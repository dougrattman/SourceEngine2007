...
class CMessageDialogHandler {
 public:
  ...
  void CloseMessageDialog(const uint32_t nType = 0);
  void CloseAllMessageDialogs();
  void CreateMessageDialog(const uint32_t nType, const char *pTitle,
                           const char *pMsg, const char *pCmdA,
                           const char *pCmdB, vgui::Panel *pCreator,
...
//-----------------------------------------------------------------------------
// Purpose: Panel that acts as background for button icons and help text in the
// UI
//-----------------------------------------------------------------------------
class CFooterPanel : public vgui::EditablePanel {
 ...
 private:
  ...
  int m_ButtonPinRight;  // if not centered, this is the distance from the right
                         // margin that we use to start drawing buttons (right
                         // to left)
  ...
  int m_ButtonOffsetFromTop;  // how far below the top the buttons should be
                              // drawn
  int m_ButtonSeparator;      // space between the button icon and text
  int m_TextAdjust;  // extra adjustment for the text (vertically)...text is
                     // centered on the button icon and then this value is
                     // applied
...
class CBasePanel : public vgui::Panel {
 ...
 public:
  ...
  class CAsyncJobContext {
   public:
    CAsyncJobContext(float flLeastExecuteTime = 0.0f)
        : m_flLeastExecuteTime(flLeastExecuteTime), m_hThreadHandle(NULL) {}
   ...
   public:
    void
        *volatile m_hThreadHandle;  // Handle to an async job thread waiting for
    float m_flLeastExecuteTime;     // Least amount of time this job should keep
                                    // executing
  };
 ...
 public:
  ...
  void ShowMessageDialog(const uint32_t nType, vgui::Panel *pParent = NULL);
  void CloseMessageDialog(const uint32_t nType);
  ...
  // forces any changed options dialog settings to be applied immediately, if
  // it's open
  void ApplyOptionsDialogSettings();

  vgui::AnimationController *GetAnimationController(void) {
    return m_pConsoleAnimationController;
  }
  void RunCloseAnimation(const char *animName);
  void RunAnimationWithCallback(vgui::Panel *parent, const char *animName,
                                KeyValues *msgFunc);
  ...
  bool IsWaitingForConsoleUI(void) {
 ...
 private:
  enum EBackgroundState {
    ...
    BACKGROUND_EXITING,  // Console has started an exiting state, cannot be
                         // stopped
  };
  ...
  uint32_t m_iGameID;  // matches context value in hl2orange.spa.h
  ...
  CPanelAnimationVar(float, m_flBackgroundFillAlpha, "m_flBackgroundFillAlpha",
                     "0");
