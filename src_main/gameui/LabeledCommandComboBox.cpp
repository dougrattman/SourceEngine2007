...
CLabeledCommandComboBox::CLabeledCommandComboBox(vgui::Panel *parent,
...
void CLabeledCommandComboBox::AddItem(char const *text,
                                      char const *engineCommand) {
...
void CLabeledCommandComboBox::OnTextChanged(char const *text) {
  ...
  //	PostMessage( GetParent()->GetVPanel(), new vgui::KeyValues(
  //"TextChanged", "text", text ) );
}

const char *CLabeledCommandComboBox::GetActiveItemCommand() {
  if (m_iCurrentSelection == -1) return NULL;
...
void CLabeledCommandComboBox::ApplyChanges() {
  if (m_iCurrentSelection == -1) return;
