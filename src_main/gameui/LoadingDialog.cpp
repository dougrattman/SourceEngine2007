...
void CLoadingDialog::PaintBackground() {
  ...
  if (m_pLoadingBackground) {
    ...
    Color color = GetSchemeColor("TanDarker", Color(255, 255, 255, 255),
  ...
  DrawBox(x + 2, y + 2, barWide - 4, barTall - 4, Color(100, 100, 100, 255),
          1.0f);
...
//-----------------------------------------------------------------------------
void CLoadingDialog::SetupControlSettingsForErrorDisplay(
...
void CLoadingDialog::DisplayGenericError(const char *failureReason,
  ...
  if (extendedReason && strlen(extendedReason) > 0) {
    ...
    g_pVGuiLocalize->ConstructString(finalMsg, sizeof(finalMsg), formatStr, 1,
                                     compositeReason);
...
void CLoadingDialog::DisplayVACBannedError() {
  ...
  SetupControlSettingsForErrorDisplay(
...
void CLoadingDialog::OnThink() {
  ...
  if (!m_bConsoleStyle && m_bShowingSecondaryProgress) {
    ...
    } else if (ProgressBar::ConstructTimeRemainingString(
                   unicode, sizeof(unicode), m_flSecondaryProgressStartTime,
...
CLoadingDialog *LoadingDialog() { return g_hLoadingDialog.Get(); }
