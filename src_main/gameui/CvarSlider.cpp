...
// Purpose:
//-----------------------------------------------------------------------------
CCvarSlider::CCvarSlider(Panel *parent, const char *panelName,
                         char const *caption, float minValue, float maxValue,
  ...
  // For backwards compatability. Ignore .res file settings for forced setup
  // sliders.
...
// Purpose:
//-----------------------------------------------------------------------------
void CCvarSlider::SetupSlider(float minValue, float maxValue,
                              const char *cvarname, bool bAllowOutOfRange) {
...
void CCvarSlider::ApplySettings(KeyValues *inResourceData) {
  ...
  if (!m_bCreatedInCode) {
    ...
    if (GetParent()) {
      // HACK: If our parent is a property page, we want the dialog containing
      // it
      if (dynamic_cast<vgui::PropertyPage *>(GetParent()) &&
          GetParent()->GetParent()) {
