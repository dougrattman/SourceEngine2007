...
ConVar x360_resolution_widescreen_mode(
    "x360_resolution_widescreen_mode", "0", 0,
    "This is only used for reference. Changing this value does nothing");
ConVar x360_resolution_width(
    "x360_resolution_width", "640", 0,
    "This is only used for reference. Changing this value does nothing");
ConVar x360_resolution_height(
    "x360_resolution_height", "480", 0,
    "This is only used for reference. Changing this value does nothing");
ConVar x360_resolution_interlaced(
    "x360_resolution_interlaced", "0", 0,
    "This is only used for reference. Changing this value does nothing");

ConVar x360_audio_english(
    "x360_audio_english", "0", 0,
...
struct OptionData_t {
  ...
  char szConvar2[OPTION_STRING_LENGTH];  // Used for choice types that use 2
                                         // convars
...
bool ActionsAreTheSame(const char *pchAction1, const char *pchAction2) {
  if (Q_stricmp(pchAction1, pchAction2) == 0) return true;

  if ((Q_stricmp(pchAction1, "+duck") == 0 ||
       Q_stricmp(pchAction1, "toggle_duck") == 0) &&
      (Q_stricmp(pchAction2, "+duck") == 0 ||
       Q_stricmp(pchAction2, "toggle_duck") == 0)) {
    // +duck and toggle_duck are interchangable
    return true;
  }

  if ((Q_stricmp(pchAction1, "+zoom") == 0 ||
       Q_stricmp(pchAction1, "toggle_zoom") == 0) &&
      (Q_stricmp(pchAction2, "+zoom") == 0 ||
       Q_stricmp(pchAction2, "toggle_zoom") == 0)) {
...
COptionsDialogXbox::COptionsDialogXbox(vgui::Panel *parent,
                                       bool bControllerOptions)
    : BaseClass(parent, "OptionsDialog") {
  // Figure out which way duck is bound, and set the option_duck_method convar
  // the correct way.
  ...
  m_pOptionsSelectionLeft =
      SETUP_PANEL(new Panel(this, "OptionsSelectionLeft"));
  m_pOptionsSelectionLeft2 =
      SETUP_PANEL(new Panel(this, "OptionsSelectionLeft2"));
...
void COptionsDialogXbox::InitializeSliderDefaults(void) {
  for (int iOption = 0; iOption < m_pOptions->Count(); ++iOption) {
    ...
    if (pOption->eOptionType != OPTION_TYPE_SLIDER) continue;

    if (pOption->eSliderHomeType != SLIDER_HOME_PREV) continue;
...
COptionsDialogXbox::~COptionsDialogXbox() {
  if (m_bOldForceEnglishAudio != x360_audio_english.GetBool()) {
    PostMessage(BasePanel()->GetVPanel(),
                new KeyValues("command", "command", "QuitRestartNoConfirm"),
                0.0f);
  }
...
void COptionsDialogXbox::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  KeyValues *pControlSettings =
      BasePanel()->GetConsoleControlSettings()->FindKey("OptionsDialog.res");
  LoadControlSettings("0", NULL, pControlSettings);

  if (m_pFooter) {
    KeyValues *pFooterControlSettings =
        BasePanel()->GetConsoleControlSettings()->FindKey("OptionsFooter.res");
    m_pFooter->LoadControlSettings("0", NULL, pFooterControlSettings);
  }

  m_SelectedColor = pScheme->GetColor("SectionedListPanel.SelectedBgColor",
  ...
  m_pValueBars[0]->SetHomeColor(
      Color(m_SelectedColor[0], m_SelectedColor[1], m_SelectedColor[2], 96));

  // Get proper setting for items for orginal in res file
  vgui::Panel *(pPanelList[3]) = {m_pOptionLabels[0], m_pValueLabels[0],
                                  m_pValueBars[0]};
  ...
  // Don't fade the background for options so that brightness can be easily
  // adjusted
  if (!m_bControllerOptions)
    vgui::GetAnimationController()->RunAnimationCommand(
        BasePanel(), "m_flBackgroundFillAlpha", 0.01f, 0.0f, 1.0f,
...
void COptionsDialogXbox::OnClose(void) {
  ...
  for (int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode) {
    ...
    if (!pDuckKeyName || !pDuckKeyName[0]) continue;
    ...
    if (ActionsAreTheSame(pDuckKeyName, "+duck")) {
      if (varOption.GetBool()) {
        ...
        Q_snprintf(szCommand, sizeof(szCommand), "bind \"%s\" \"%s\"",
                   g_pInputSystem->ButtonCodeToString(code), "toggle_duck");
        engine->ClientCmd_Unrestricted(szCommand);
      } else {
        // Bind DUCK key to +DUCK
        Q_snprintf(szCommand, sizeof(szCommand), "bind \"%s\" \"%s\"",
                   g_pInputSystem->ButtonCodeToString(code), "+DUCK");
        engine->ClientCmd_Unrestricted(szCommand);
      }
    }
  }
  ...
  // Save these settings!
  if (m_bOptionsChanged) engine->ClientCmd_Unrestricted("host_writeconfig");

  BasePanel()->RunCloseAnimation("CloseOptionsDialog_OpenMainMenu");
  if (!m_bControllerOptions)
    vgui::GetAnimationController()->RunAnimationCommand(
        BasePanel(), "m_flBackgroundFillAlpha", 120.0f, 0.0f, 1.0f,
        AnimationController::INTERPOLATOR_LINEAR);
    ...
    case KEY_XBUTTON_A:
      if (!(*m_pOptions)[m_iSelection]->bUnchangeable) {
        ...
        if (eOptionType == OPTION_TYPE_BIND &&
            ((*m_pOptions)[m_iSelection]->iNumBinds <
                 binds_per_command.GetInt() ||
             binds_per_command.GetInt() == 1)) {
          ActivateSelection();
          vgui::surface()->PlaySound("UI/buttonclickrelease.wav");
        } else if (eOptionType == OPTION_TYPE_BINARY ||
                   eOptionType == OPTION_TYPE_CHOICE) {
          ActivateSelection();
          ChangeValue(1);
          DeactivateSelection();
        }
      } else if (!(*m_pOptions)[m_iSelection]->bVocalsLanguage) {
        BasePanel()->ShowMessageDialog(MD_OPTION_CHANGE_FROM_X360_DASHBOARD,
                                       this);
      }
      break;
...
void COptionsDialogXbox::UpdateFooter(void) {
  ...
  if (!m_bSelectionActive) {
    if (!(*m_pOptions)[m_iSelection]->bUnchangeable) {
      switch ((*m_pOptions)[m_iSelection]->eOptionType) {
        ...
        case OPTION_TYPE_CHOICE:
          m_pFooter->AddNewButtonLabel(
              ((*m_pOptions)[m_iSelection]->m_Choices.Count() == 2)
                  ? ("#GameUI_Toggle")
                  : ("#GameUI_Modify"),
              "#GameUI_Icons_DPAD");
          break;
        ...
        case OPTION_TYPE_BIND: {
          if ((*m_pOptions)[m_iSelection]->iNumBinds <
                  binds_per_command.GetInt() ||
              binds_per_command.GetInt() == 1)
            m_pFooter->AddNewButtonLabel("#GameUI_Modify",
                                         "#GameUI_Icons_A_BUTTON");
          if ((*m_pOptions)[m_iSelection]->iNumBinds != 0)
            m_pFooter->AddNewButtonLabel("#GameUI_ClearButton",
    ...
    if (m_bControllerOptions)
      m_pFooter->AddNewButtonLabel("#GameUI_DefaultButtons",
                                   "#GameUI_Icons_Y_BUTTON");
    else
      m_pFooter->AddNewButtonLabel("#GameUI_Console_StorageChange",
                                   "#GameUI_Icons_Y_BUTTON");
...
void COptionsDialogXbox::UpdateScroll(void) {
  for (int iLabel = 0; iLabel < m_iNumItems; ++iLabel) {
    ...
    if (iOption >= m_pOptions->Count()) break;
  ...
  m_pOptionsDownArrow->SetAlpha(
      (m_iScroll + m_iNumItems < m_pOptions->Count()) ? (255) : (64));
}
...
void COptionsDialogXbox::UncacheChoices(void) {
  for (int iOption = 0; iOption < m_pOptions->Count(); ++iOption) {
    ...
    if (pOption->eOptionType != OPTION_TYPE_CHOICE) continue;
...
void COptionsDialogXbox::GetChoiceFromConvar(OptionData_t *pOption) {
  if (pOption->iCurrentChoice < 0) {
    // Don't have a proper choice yet, so see if the convars value matches one
    // of our choices
    if (pOption->szConvar2[0] == '\0') {
      ...
      for (int iChoice = 0; iChoice < pOption->m_Choices.Count(); ++iChoice) {
        ...
        Q_strncpy(szOptionValue, pOption->m_Choices[iChoice].szValue,
                  sizeof(szOptionValue));

        char *pchOptionValue2 = const_cast<char *>(
            Q_strnchr(szOptionValue, ';', sizeof(szOptionValue)));

        AssertMsg(pchOptionValue2,
                  "Option uses 2 convars but an option doesn't have 2 ; "
                  "separated values!");

        pchOptionValue2[0] = '\0';  // Set this as the end of the other value
        ++pchOptionValue2;          // Point at next char

        if (Q_stricmp(szOptionValue, pchValue) == 0 &&
            Q_stricmp(pchOptionValue2, pchValue2) == 0) {
          pOption->iCurrentChoice = iChoice;
          break;
        }
      }
...
#if !defined(_X360)
// BUGBUG: This won't compile because it includes windows.h and this interface
// function is #defined to something else see below system()->GetCurrentTime
#undef GetCurrentTime
#endif
  ...
  switch (m_pSelectedOption->eOptionType) {
    ...
    case OPTION_TYPE_SLIDER: {
      if (system()->GetCurrentTime() > m_fNextChangeTime) {
        ...
        if (fNumSegments <= 0.0f) fNumSegments = 1.0f;

        float fIncValue;

        if (m_pSelectedOption->fIncValue == 0.0f)
          fIncValue =
              (m_pSelectedOption->fMaxValue - m_pSelectedOption->fMinValue) *
              (1.0f / fNumSegments);
        else
          fIncValue =
              m_pSelectedOption->fIncValue *
              (m_pSelectedOption->fMaxValue - m_pSelectedOption->fMinValue) *
              (1.0f / fNumSegments);
    ...
    case OPTION_TYPE_CHOICE: {
      ...
      if (m_pSelectedOption->iCurrentChoice >= 0) {
        ...
        while (m_pSelectedOption->iCurrentChoice >=
               m_pSelectedOption->m_Choices.Count())
          m_pSelectedOption->iCurrentChoice -=
              m_pSelectedOption->m_Choices.Count();
        ...
        if (m_pSelectedOption->szConvar2[0] == '\0') {
          ...
          ConVarRef varOption(m_pSelectedOption->szConvar);
          varOption.SetValue(
              m_pSelectedOption->m_Choices[m_pSelectedOption->iCurrentChoice]
                  .szValue);
        } else {
          ...
          AssertMsg(pchOptionValue2,
                    "Option uses to convars but an option doesn't have 2 ; "
                    "separated values!");
        ...
        DevWarning(
            "ConVar \"%s\" set to value that's not a choice used by \"%s\" "
            "option.",
            m_pSelectedOption->szConvar, m_pSelectedOption->szDisplayName);
      }
    ...
    case OPTION_TYPE_BIND: {
      ...
      Q_snprintf(szCommand, sizeof(szCommand), "bind \"%s\" \"%s\"",
                 ...
                 m_pSelectedOption->szCommand);
      engine->ClientCmd_Unrestricted(szCommand);

      // After binding we need to update all bindings so they display the
      // correct keys
      UpdateAllBinds(code);
...
void COptionsDialogXbox::UnbindOption(OptionData_t *pOption, int iLabel) {
  if (pOption->eOptionType != OPTION_TYPE_BIND) return;
  ...
  for (int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode) {
    ...
    // Check if there's a binding for this key
    if (!pBinding || !pBinding[0]) continue;

    // If we use this binding, display the key in our list
    if (ActionsAreTheSame(pBinding, pOption->szCommand)) {
      char szCommand[256];
      Q_snprintf(szCommand, sizeof(szCommand), "unbind %s",
                 g_pInputSystem->ButtonCodeToString(code));
      engine->ClientCmd_Unrestricted(szCommand);
    }
  }
  ...
  if (iLabel < 0 || iLabel >= m_iNumItems) return;
...
void COptionsDialogXbox::UpdateValue(OptionData_t *pOption, int iLabel) {
  ...
  switch (pOption->eOptionType) {
    case OPTION_TYPE_BINARY: {
      ...
      m_pValueLabels[iLabel]->SetFont(m_hLabelFont);
      m_pValueLabels[iLabel]->SetText(
          (varOption.GetBool()) ? ("#GameUI_Enable") : ("#GameUI_Disable"));
      break;
    }
    ...
    case OPTION_TYPE_SLIDER: {
      ...
      if (pOption->fIncValue == 0.0f)
        fIncValue =
            (pOption->fMaxValue - pOption->fMinValue) * (1.0f / fNumSegments);
      else
        fIncValue = pOption->fIncValue *
                    (pOption->fMaxValue - pOption->fMinValue) *
                    (1.0f / fNumSegments);

      float fNormalizeHome =
          fabsf((pOption->fMinValue - pOption->fSliderHomeValue) /
                (pOption->fMaxValue - pOption->fMinValue));
      if (pOption->fIncValue < 0.0f) fNormalizeHome = 1.0f - fNormalizeHome;
      m_pValueBars[iLabel]->SetHomeValue(fNormalizeHome);

      float fNormalizeValue =
          fabsf((pOption->fMinValue - varOption.GetFloat()) /
                (pOption->fMaxValue - pOption->fMinValue));
      if (pOption->fIncValue < 0.0f) fNormalizeValue = 1.0f - fNormalizeValue;

      // atof accuracy for convar isn't so good, so snap to the home value if
      // we're near it.
    ...
    case OPTION_TYPE_CHOICE: {
      ...
      if (pOption->iCurrentChoice >= 0) {
        m_pValueLabels[iLabel]->SetText(
            pOption->m_Choices[pOption->iCurrentChoice].szName);
      } else {
        if (pOption->bUnchangeable) {
          if (pOption->szConvar2[0] == '\0') {
            ConVarRef varOption(pOption->szConvar);
            ...
            ConVarRef varOption2(pOption->szConvar2);
            char szBuff[256];
            Q_snprintf(szBuff, sizeof(szBuff), "%sx%s%s", varOption.GetString(),
                       varOption2.GetString(),
                       (x360_resolution_interlaced.GetBool()) ? ("i") : ("p"));
          ...
          DevWarning(
              "ConVar \"%s\" set to value that's not a choice used by \"%s\" "
              "option.",
              pOption->szConvar, pOption->szDisplayName);
...
void COptionsDialogXbox::UpdateBind(OptionData_t *pOption, int iLabel,
                                    ButtonCode_t codeIgnore,
                                    ButtonCode_t codeAdd) {
  int iNumBinds = 0;
  ...
  for (int iCode = 0; iCode < BUTTON_CODE_LAST; ++iCode) {
    ...
    if (!bUseThisKey) {
      ...
      // Only check against bind name if we haven't already forced this binding
      // to be used
      const char *pBinding = gameuifuncs->GetBindingForButtonCode(code);
    ...
    if (!bUseThisKey) continue;

    // Turn localized string into icon character
    Q_snprintf(
        szBuff, sizeof(szBuff), "#GameUI_Icons_%s",
        g_pInputSystem->ButtonCodeToString(static_cast<ButtonCode_t>(iCode)));
    g_pVGuiLocalize->ConstructString(szWideBuff, sizeof(szWideBuff),
                                     g_pVGuiLocalize->Find(szBuff), 0);
...
void COptionsDialogXbox::FillInDefaultBindings(void) {
  ...
  while (data != NULL) {
    ...
    if (!stricmp(cmd, "bind")) {
      ...
      if (strlen(szKeyName) <= 0) break;  // Error
      ...
      // Bind it
      char szCommand[256];
      Q_snprintf(szCommand, sizeof(szCommand), "bind \"%s\" \"%s\"", szKeyName,
                 szBinding);
      engine->ClientCmd_Unrestricted(szCommand);
      ...
      for (int iLabel = 0; iLabel < m_iNumItems; ++iLabel) {
        ...
        if (pOption->eOptionType == OPTION_TYPE_BIND &&
            ActionsAreTheSame(pOption->szCommand, szBinding)) {
          char szBuff[512];
          wchar_t szWideBuff[64];
          char szBinds[OPTION_STRING_LENGTH];
...
bool COptionsDialogXbox::ShouldSkipOption(KeyValues *pKey) {
  // Skip the option if not in developer mode and this is a developer only
  // option
  ConVarRef developer("developer");
  if (!developer.GetBool() && (pKey->GetInt("dev", 0) != 0)) return true;

  // Skip the option if it doesn't match the controller/non-controller list
  if (m_bControllerOptions != (pKey->GetInt("control", 0) != 0)) return true;

  // Skip portal options if this mod doesn't have portals (defined in
  // gameinfo.txt)
  if (!ModInfo().HasPortals() && (pKey->GetInt("portals", 0) != 0)) return true;

  // Skip multiplayer only options for single player (or combo) games
  if (ModInfo().IsSinglePlayerOnly() && !ModInfo().IsMultiplayerOnly() &&
      (pKey->GetInt("multiplayer", 0) != 0))
    return true;

  // Skip difficulty options for games that don't want them
  if (!(ModInfo().IsSinglePlayerOnly() && !ModInfo().NoDifficulty()) &&
      (pKey->GetInt("difficulty", 0) != 0))
    return true;

  // Skip voice options for single player games
  if (ModInfo().IsSinglePlayerOnly() && !ModInfo().IsMultiplayerOnly() &&
      (pKey->GetInt("voice", 0) != 0))
    return true;
  ...
  for (int iOption = 0; iOption < m_pOptions->Count(); ++iOption) {
    OptionData_t *pOption = (*m_pOptions)[iOption];
    if (Q_strcmp(pOption->szName, szName) == 0) return true;
  }
...
void COptionsDialogXbox::ReadOptionsFromFile(const char *pchFileName) {
  ...
  KeyValues *pKey = NULL;
  for (pKey = pOptionKeys->GetFirstTrueSubKey(); pKey;
    ...
    if (pKey->GetInt("disable", 0) != 0) {
      ...
      OptionChoiceData_t *pDisabledOption =
          &(s_DisabledOptions[iDisabledOption]);
      Q_strncpy(pDisabledOption->szName, pKey->GetName(),
                sizeof(pDisabledOption->szName));

      continue;
    }

    if (ShouldSkipOption(pKey)) continue;
    ...
    *pNewOption = new OptionData_t;
    ...
    // Get common values
    Q_strncpy((*pNewOption)->szName, pKey->GetName(),
              sizeof((*pNewOption)->szName));
    Q_strncpy((*pNewOption)->szDisplayName, pKey->GetString("name", ""),
              sizeof((*pNewOption)->szDisplayName));

    Q_strncpy((*pNewOption)->szConvar, pKey->GetString("convar", ""),
              sizeof((*pNewOption)->szConvar));
    if ((*pNewOption)->szConvar[0] == '\0')
      Q_strncpy((*pNewOption)->szCommand, pKey->GetString("command", ""),
                sizeof((*pNewOption)->szCommand));

    Q_strncpy((*pNewOption)->szConvarDef, pKey->GetString("convar_def", ""),
              sizeof((*pNewOption)->szConvarDef));
    ...
    switch ((*pNewOption)->eOptionType) {
      case OPTION_TYPE_SLIDER: {
        ...
        Q_strncpy(szSliderHomeType, pKey->GetString("sliderhome", "none"),
                  sizeof(szSliderHomeType));
        ...
        switch ((*pNewOption)->eSliderHomeType) {
          ...
          case SLIDER_HOME_CENTER:
            (*pNewOption)->fSliderHomeValue =
                ((*pNewOption)->fMaxValue + (*pNewOption)->fMinValue) * 0.5f;
            break;
      ...
      case OPTION_TYPE_CHOICE: {
        Q_strncpy((*pNewOption)->szConvar2, pKey->GetString("convar2", ""),
                  sizeof((*pNewOption)->szConvar));

        if ((*pNewOption)->bVocalsLanguage) {
          (*pNewOption)->iCurrentChoice = -1;
          int iChoice = (*pNewOption)->m_Choices.AddToTail();
          OptionChoiceData_t *pNewOptionChoice =
              &((*pNewOption)->m_Choices[iChoice]);

          Q_strncpy(pNewOptionChoice->szName, "#GameUI_Language_English",
                    sizeof(pNewOptionChoice->szName));
          Q_strncpy(pNewOptionChoice->szValue, "1",
                    sizeof(pNewOptionChoice->szValue));

          iChoice = (*pNewOption)->m_Choices.AddToTail();
          pNewOptionChoice = &((*pNewOption)->m_Choices[iChoice]);

          char szLanguage[256];
          Q_snprintf(szLanguage, sizeof(szLanguage), "#GameUI_Language_%s",
                     XBX_GetLanguageString());
          Q_strncpy(pNewOptionChoice->szName, szLanguage,
                    sizeof(pNewOptionChoice->szName));
          Q_strncpy(pNewOptionChoice->szValue, "0",
                    sizeof(pNewOptionChoice->szValue));
        } else {
          (*pNewOption)->iCurrentChoice = -1;
          ...
          if (pChoicesKey) {
            ...
            for (pSubKey = pChoicesKey->GetFirstSubKey(); pSubKey;
                 pSubKey = pSubKey->GetNextKey()) {
              int iChoice = (*pNewOption)->m_Choices.AddToTail();
              OptionChoiceData_t *pNewOptionChoice =
                  &((*pNewOption)->m_Choices[iChoice]);

              Q_strncpy(pNewOptionChoice->szName, pSubKey->GetName(),
                        sizeof(pNewOptionChoice->szName));
              Q_strncpy(pNewOptionChoice->szValue, pSubKey->GetString(),
                        sizeof(pNewOptionChoice->szValue));
            }
...
int __cdecl SortByPriority(OptionData_t *const *pLeft,
                           OptionData_t *const *pRight) {
  return (*pLeft)->iPriority - (*pRight)->iPriority;
}

void COptionsDialogXbox::SortOptions(void) { m_pOptions->Sort(SortByPriority); }
