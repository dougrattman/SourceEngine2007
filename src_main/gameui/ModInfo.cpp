...
#ifndef _XBOX
  return (_stricmp(m_pModData->GetString("type", ""), "singleplayer_only") ==
          0);
#else
...
const wchar_t *CModInfo::GetGameTitle() {
  if (!m_wcsGameTitle[0]) {
    // for some reason, the standard ILocalize::ConvertANSIToUnicode() strips
    // off the '²' character in 'HALF-LIFE²' - so just do a straight upconvert
    // to unicode
...
bool CModInfo::HasPortals() {
  return (_stricmp(m_pModData->GetString("hasportals", "0"), "1") == 0);
}
...
bool CModInfo::NoDifficulty() {
  return (_stricmp(m_pModData->GetString("nodifficulty", "0"), "1") == 0);
}
...
bool CModInfo::NoCrosshair() {
  return (_stricmp(m_pModData->GetString("nocrosshair", "1"), "1") == 0);
}
...
bool CModInfo::AdvCrosshair() {
  return (_stricmp(m_pModData->GetString("advcrosshair", "0"), "1") == 0);
}
...
//-----------------------------------------------------------------------------
// Purpose: loads file from 0-terminated buffer
//-----------------------------------------------------------------------------
