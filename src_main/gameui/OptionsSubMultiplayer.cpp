...
static ColorItem_t itemlist[] = {
    {"#Valve_Orange", 255, 120, 24}, {"#Valve_Yellow", 225, 180, 24},
    {"#Valve_Blue", 0, 60, 255},     {"#Valve_Ltblue", 0, 167, 255},
    {"#Valve_Green", 0, 167, 0},     {"#Valve_Red", 255, 43, 0},
    {"#Valve_Brown", 123, 73, 0},    {"#Valve_Ltgray", 100, 100, 100},
    {"#Valve_Dkgray", 36, 36, 36},
};

static ColorItem_t s_crosshairColors[] = {{"#Valve_Green", 50, 250, 50},
                                          {"#Valve_Red", 250, 50, 50},
                                          {"#Valve_Blue", 50, 50, 250},
                                          {"#Valve_Yellow", 250, 250, 50},
                                          {"#Valve_Ltblue", 50, 250, 250}};
static const int NumCrosshairColors =
    sizeof(s_crosshairColors) / sizeof(s_crosshairColors[0]);
...
class CrosshairImagePanel : public ImagePanel {
 ...
 public:
  CrosshairImagePanel(Panel *parent, const char *name,
...
//-----------------------------------------------------------------------------
CrosshairImagePanel::CrosshairImagePanel(Panel *parent, const char *name,
                                         CCvarToggleCheckButton *pAdditive)
  ...
  vgui::surface()->DrawSetTextureFile(m_iCrosshairTextureID,
                                      "vgui/white_additive", true, false);
}
...
void CrosshairImagePanel::UpdateCrosshair(int r, int g, int b, int size) {
  ...
  if (size == 0) {
    if (screenWide <= 640) {
      ...
      // if the screen width is between 640 and 1024, set the crosshair num to 2
      // (medium)
      size = 2;
    } else {
      // if the screen width is 1024 or greater, set the crosshair num to 1
      // (small)
      size = 3;
    }
  }
...
void CrosshairImagePanel::Paint() {
  ...
  DrawCrosshairRect((wide / 2 - m_barGap - m_barSize) + 1, tall / 2, m_barSize,
                    1, additive);
  DrawCrosshairRect(wide / 2 + m_barGap, tall / 2, m_barSize, 1, additive);
  DrawCrosshairRect(wide / 2, tall / 2 - m_barGap - m_barSize, 1, m_barSize,
                    additive);
...
class AdvancedCrosshairImagePanel : public ImagePanel {
 ...
 public:
  ...
  void UpdateCrosshair(int r, int g, int b, float scale,
...
//-----------------------------------------------------------------------------
AdvancedCrosshairImagePanel::AdvancedCrosshairImagePanel(Panel *parent,
                                                         const char *name)
    : ImagePanel(parent, name) {
...
//-----------------------------------------------------------------------------
void AdvancedCrosshairImagePanel::UpdateCrosshair(int r, int g, int b,
                                                  float scale,
                                                  const char *crosshairname) {
  m_R = r;
  m_G = g;
  ...
  vgui::surface()->DrawSetTextureFile(m_iCrosshairTextureID, crosshairname,
                                      true, false);
  ...
  m_pAdvCrosshair =
      vgui::surface()->DrawGetTextureMatInfoFactory(m_iCrosshairTextureID);
...
COptionsSubMultiplayer::COptionsSubMultiplayer(vgui::Panel *parent)
  ...
  // crosshair controls
  m_pCrosshairColorComboBox =
      new ComboBox(this, "CrosshairColorComboBox", 6, false);
  m_pCrosshairSize = new CLabeledCommandComboBox(this, "CrosshairSizeComboBox");
  m_pCrosshairTranslucencyCheckbox =
      new CCvarToggleCheckButton(this, "CrosshairTranslucencyCheckbox",
                                 "#GameUI_Translucent", "cl_crosshairusealpha");
  m_pCrosshairImage = new CrosshairImagePanel(this, "CrosshairImage",
                                              m_pCrosshairTranslucencyCheckbox);

  // advanced crosshair controls
  //==========
  m_pAdvCrosshairRedSlider =
      new CCvarSlider(this, "Red Color Slider", "#GameUI_CrosshairColor_Red",
                      0.0f, 255.0f, "cl_crosshair_red");
  m_pAdvCrosshairGreenSlider = new CCvarSlider(
      this, "Green Color Slider", "#GameUI_CrosshairColor_Green", 0.0f, 255.0f,
      "cl_crosshair_green");
  m_pAdvCrosshairBlueSlider =
      new CCvarSlider(this, "Blue Color Slider", "#GameUI_CrosshairColor_Blue",
                      0.0f, 255.0f, "cl_crosshair_blue");

  m_pAdvCrosshairScaleSlider =
      new CCvarSlider(this, "Scale Slider", "#GameUI_CrosshairScale", 16.0f,
                      48.0f, "cl_crosshair_scale");
  ...
  m_pAdvCrosshairImage =
      new AdvancedCrosshairImagePanel(this, "AdvCrosshairImage");
  ...
  // turn off the crosshair stuff if the mod specifies "nocrosshair" in the
  // gameinfo.txt file
  ...
  // turn off model selection stuff if the mod specifies "nomodels" in the
  // gameinfo.txt file
  if (ModInfo().NoModels()) {
    Panel *pTempPanel = NULL;
  ...
  // turn off the himodel stuff if the mod specifies "nohimodel" in the
  // gameinfo.txt file
  ...
  if (!ModInfo().AdvCrosshair()) {
    ...
    // #GameUI_AdvCrosshairDescription (from
...
void COptionsSubMultiplayer::OnCommand(const char *command) {
  if (!stricmp(command, "Advanced")) {
    ...
    if (m_hImportSprayDialog == NULL) {
      ...
      m_hImportSprayDialog->AddFilter("*.tga,*.jpg,*.bmp,*.vtf",
                                      "#GameUI_All_Images", true);
...
// file selected.  This can only happen when someone selects an image to be
// imported as a spray logo.
void COptionsSubMultiplayer::OnFileSelected(const char *fullpath) {
    ...
    if (!stricmp(extension, "jpg") || !stricmp(extension, "jpeg")) {
      ...
      if (errcode == CE_SUCCESS) {
        ...
        if (errcode == CE_MEMORY_ERROR) {
          errorDialog =
              new vgui::MessageBox("#GameUI_Spray_Import_Error_Title",
          ...
          errorDialog = new vgui::MessageBox(
  ...
  if (convertTGAToVTF && !failed) {
    ...
    errcode = ConvertTGA(tgaPath);  // resize TGA so that it has power-of-two
                                    // dimensions with a max size of 256x256.
    if (errcode != CE_SUCCESS) {
      failed = true;
      vgui::MessageBox *errorDialog = NULL;
  ...
  if (!failed) {
    ...
    *c = 0;
    ...
    while ((vtfFilename > fullpath) && (*(vtfFilename - 1) != '\\') &&
           (*(vtfFilename - 1) != '/')) {
      --vtfFilename;
    }

    Q_strncpy(finalPath, engine->GetGameDirectory(), sizeof(finalPath));
    Q_strncat(finalPath, "\\materials\\VGUI\\logos\\", sizeof(finalPath),
              COPY_ALL_CHARACTERS);
  ...
  if (deleteIntermediateVTF) {
    ...
    // the TGA->VTF conversion process generates a .txt file if one wasn't
    // already there. in this case, delete the .txt file.
    c = vtfPath + strlen(vtfPath);
    while ((c > vtfPath) && (*(c - 1) != '.')) {
      --c;
    }
...
//-----------------------------------------------------------------------------
// Purpose: We'll override the default error handler so we can deal with errors
// without having to exit the engine
//-----------------------------------------------------------------------------
static void ValveJpegErrorHandler(j_common_ptr cinfo) {
  ValveJpegErrorHandler_t *pError =
      reinterpret_cast<ValveJpegErrorHandler_t *>(cinfo->err);
...
ConversionErrorType COptionsSubMultiplayer::ConvertJPEGToTGA(
  ...
  // Check for valid width and height (ie. power of 2 and print out an error and
  // exit if not).
  if (!IsPowerOfTwo(jpegInfo.image_height) ||
      !IsPowerOfTwo(jpegInfo.image_width)) {
    jpeg_destroy_decompress(&jpegInfo);
    fclose(infile);
  ...
  while (working && (jpegInfo.output_scanline < jpegInfo.output_height)) {
    ...
    if (jpeg_read_scanlines(&jpegInfo, row_pointer, 1) == 0) {
      working = false;
    }
    ++cur_row;
  }
  ...
  // ok, at this point we have read in the JPEG image to our buffer, now we need
  // to write it out as a TGA file.
  CUtlBuffer outBuf;
  bool bRetVal =
      TGAWriter::WriteToBuffer(buf, outBuf, image_width, image_height,
...
ConversionErrorType COptionsSubMultiplayer::ConvertBMPToTGA(
  ...
  if (bitmap.bmBitsPixel == 24) {
    ...
    bitmapInfo =
        (BITMAPINFO *)malloc(colorsUsed * sizeof(RGBQUAD) + sizeof(BITMAPINFO));
  }
  ...
  if (bitmap.bmBitsPixel != 24) {
    bitmapInfo->bmiHeader.biBitCount =
        bitmap.bmBitsPixel;  // need to specify the bits per pixel so GDI will
                             // generate a color table for us.
  }

  HDC dc = CreateCompatibleDC(NULL);

  int retcode = GetDIBits(dc, hBitmap, 0, bitmap.bmHeight, NULL, bitmapInfo,
                          DIB_RGB_COLORS);
  ...
  int mem_required =
      3 * bitmap.bmWidth *
      bitmap
          .bmHeight;  // mem required for copying the data out into RGB format.
  ...
  if (bitmapInfo->bmiHeader.biBitCount == 24) {
    if ((bitmap.bmWidth * 3) == bitmap.bmWidthBytes) {
      ...
      for (row = 0; row < bitmap.bmHeight; ++row) {
        memcpy(buf + (row * 3 * bitmap.bmWidth),
               (unsigned char *)(bitmap.bmBits) + (row * bitmap.bmWidthBytes),
               3 * bitmap.bmWidth);
      }
    }
  ...
  for (index = 0; index < (bitmap.bmHeight / 2); ++index) {
    ...
    memcpy(buf + (index * rowWidth),
           buf + (((bitmap.bmHeight - 1) - index) * rowWidth), rowWidth);
  ...
  CUtlBuffer outBuf;
  bool retval =
      TGAWriter::WriteToBuffer(buf, outBuf, bitmap.bmWidth, bitmap.bmHeight,
                               IMAGE_FORMAT_RGB888, IMAGE_FORMAT_RGB888);
  if (retval) {
    if (!g_pFullFileSystem->WriteFile(tgaPath, NULL, outBuf)) {
      retval = false;
...
// reads in a TGA file and converts it to 32 bit RGBA color values in a memory
// buffer.
...
ConversionErrorType COptionsSubMultiplayer::ConvertTGA(const char *tgaPath) {
  ...
  unsigned char *srcBuffer =
      ReadTGAAsRGBA(tgaPath, tgaWidth, tgaHeight, errcode, tgaHeader);
  ...
  if (!IsPowerOfTwo(paddedImageWidth)) {
    // width is not a power of two, calculate the next highest power of two
    // value.
    int i = 1;
  ...
  if (!IsPowerOfTwo(paddedImageHeight)) {
    // height is not a power of two, calculate the next highest power of two
    // value.
    int i = 1;
  ...
  // compute the amount of stretching that needs to be done to both width and
  // height to get the image to fit.
  ...
  if (widthRatio < heightRatio) {
    ...
    // i.e.  for 1x1 size pixels in the resized image we will take color from
    // sourceRatio x sourceRatio sized pixels in the source image.
  ...
  unsigned char *resizeBuffer =
      (unsigned char *)malloc(finalWidth * finalHeight * 4);

  // do the actual stretching
  StretchRGBAImage(srcBuffer, tgaWidth, tgaHeight, resizeBuffer, finalWidth,
                   finalHeight);
  ...
  unsigned char *finalBuffer =
      (unsigned char *)malloc(paddedImageWidth * paddedImageHeight * 4);
...
// resize by stretching (or compressing) an RGBA image pointed to by srcBuf into
// the buffer pointed to by destBuf. the buffers are assumed to be sized
// appropriately to accomidate RGBA images of the given widths and heights.
ConversionErrorType COptionsSubMultiplayer::StretchRGBAImage(
  ...
  for (destRow = 0; destRow < destHeight; ++destRow) {
    for (destColumn = 0; destColumn < destWidth; ++destColumn) {
      ...
      // calculate the starting and ending coords for this destination pixel in
      // the source image.
      ...
      // Calculate the percentage of each source pixels' contribution to the
      ...
      //// loop for the parts of the source image that will contribute color to
      ...
      while (srcCurrentY < srcEndY) {
        ...
        while (srcCurrentX < srcEndX) {
          ...
          // compute the percentage of the destination pixel's color this source
          // pixel will contribute.
          float srcColorPercentage =
              (srcCurrentWidth / ratioX) * (srcCurrentHeight / ratioY);

          int srcCurrentPixelX = (int)srcCurrentX;
          int srcCurrentPixelY = (int)srcCurrentY;

          // get the color values for this source pixel.
          unsigned char srcCurrentRed =
              srcBuf[(srcCurrentPixelY * srcWidth * 4) +
                     (srcCurrentPixelX * 4)];
          unsigned char srcCurrentGreen =
              srcBuf[(srcCurrentPixelY * srcWidth * 4) +
                     (srcCurrentPixelX * 4) + 1];
          unsigned char srcCurrentBlue =
              srcBuf[(srcCurrentPixelY * srcWidth * 4) +
                     (srcCurrentPixelX * 4) + 2];
          unsigned char srcCurrentAlpha =
              srcBuf[(srcCurrentPixelY * srcWidth * 4) +
                     (srcCurrentPixelX * 4) + 3];

          // add the color contribution from this source pixel to the
          // destination pixel.
...
ConversionErrorType COptionsSubMultiplayer::PadRGBAImage(
    const unsigned char *srcBuf, const int srcWidth, const int srcHeight,
    unsigned char *destBuf, const int destWidth, const int destHeight) {
  if ((srcBuf == NULL) || (destBuf == NULL)) {
    return CE_CANT_OPEN_SOURCE_FILE;
  ...
  if ((srcWidth == destWidth) && (srcHeight == destHeight)) {
    // no padding is needed, just copy the buffer straight over and call it
    // done.
  ...
  if (destWidth == srcWidth) {
    ...
    // do this separately since we can do this more efficiently than the other
    // cases.
    int numRowsToPad = (destHeight - srcHeight) / 2;
    memcpy(destBuf + (numRowsToPad * destWidth * 4), srcBuf,
    ...
    for (row = numRowsToPad; row < lastRow; ++row) {
      unsigned char *destOffset =
          destBuf + (row * destWidth * 4) + (numColumnsToPad * 4);
      const unsigned char *srcOffset =
          srcBuf + ((row - numRowsToPad) * srcWidth * 4);
...
// convert TGA file at the given location to a VTF file of the same root name at
// the same location.
...
void FindVMTFilesInFolder(const char *pFolder, const char *pFolderName,
                          CLabeledCommandComboBox *cb, int &iCount,
                          int &iInitialItem) {
  ...
  while (fn) {
    ...
    if (strlen(filename) >= 4) {
      ...
      if (g_pFullFileSystem->FileExists(filename)) {
        ...
        StripStringOutOfString(MODEL_MATERIAL_BASE_FOLDER, filename,
                               texturepath);

        displayname[strlen(displayname) - 4] = 0;

        cb->AddItem(displayname,
                    texturepath + 1);  // ignore the initial "/" in texture path
...
void COptionsSubMultiplayer::InitCrosshairColorEntries() {
  // parse the string for the custom color settings and get the initial
  // settings.
  ...
  RedrawCrosshairImage();  // don't need to do this since we're not drawing
                           // anything.
}

//-----------------------------------------------------------------------------
// Purpose: takes the settings from the crosshair settings combo boxes and
// sliders
...
void COptionsSubMultiplayer::RedrawCrosshairImage() {
  ...
  if (m_pCrosshairImage && m_pCrosshairSize) {
    ...
    m_pCrosshairImage->UpdateCrosshair(s_crosshairColors[selectedVal].r,
                                       s_crosshairColors[selectedVal].g,
                                       s_crosshairColors[selectedVal].b, size);
  }
}
...
//-----------------------------------------------------------------------------
void COptionsSubMultiplayer::InitAdvCrosshairStyleList(
    CLabeledCommandComboBox *cb) {
  // Find out images
  FileFindHandle_t fh;
  char directory[512];
  ...
  if (!cl_crosshair_file.IsValid()) return;

  cb->DeleteAllItems();

  char crosshairfile[256];
  Q_snprintf(crosshairfile, sizeof(crosshairfile),
             "materials/vgui/crosshairs/%s.vtf", cl_crosshair_file.GetString());
...
void COptionsSubMultiplayer::RemapLogo() {
  ...
  Q_snprintf(fullLogoName, sizeof(fullLogoName),
             "materials/VGUI/logos/UI/%s.vmt", logoname);
...
// Purpose:
//-----------------------------------------------------------------------------
void COptionsSubMultiplayer::RemapPalette(char *filename, int topcolor,
                                          int bottomcolor) {
  char infile[256];
  char outfile[256];
  ...
  Q_snprintf(infile, sizeof(infile), "models/player/%s/%s.bmp", filename,
             filename);
  ...
  if (bmfHeader.bfType == DIB_HEADER_MARKER) {
    ...
    char *pDIB = (LPSTR)GlobalLock((HGLOBAL)hDIB);
    {
      ...
      // Remap palette
      PaletteHueReplace(lpbmi->bmiColors, topcolor, SUIT_HUE_START,
                        SUIT_HUE_END);
      PaletteHueReplace(lpbmi->bmiColors, bottomcolor, PLATE_HUE_START,
                        PLATE_HUE_END);
...
// Purpose:
//-----------------------------------------------------------------------------
void COptionsSubMultiplayer::ColorForName(char const *pszColorName, int &r,
                                          int &g, int &b) {
  r = g = b = 0;
...
void COptionsSubMultiplayer::OnApplyChanges() {
  ...
  if (m_pModelList && m_pModelList->IsVisible() &&
      m_pModelList->GetActiveItemCommand()) {
    Q_strncpy(m_ModelName, m_pModelList->GetActiveItemCommand(),
              sizeof(m_ModelName));
...
Panel *COptionsSubMultiplayer::CreateControlByName(const char *controlName) {
  if (!Q_stricmp("CCvarToggleCheckButton", controlName)) {
    CCvarToggleCheckButton *newButton =
        new CCvarToggleCheckButton(this, controlName, "", "");
