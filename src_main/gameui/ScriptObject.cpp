...
objtypedesc_t objtypes[] = {
    {O_BOOL, "BOOL"},     {O_NUMBER, "NUMBER"},     {O_LIST, "LIST"},
    {O_STRING, "STRING"}, {O_OBSOLETE, "OBSOLETE"},
};
...
Removes any possible formatting codes and double quote characters from the input
string
===================
*/
...
char *CleanFloat(float val) {
  ...
  char *tmp = str;
  if (!str || !*str || !strchr(str, '.')) return str;
...
void CScriptObject::WriteToFile(FileHandle_t fp) {
  ...
  switch (type) {
    ...
    case O_NUMBER:
      fVal = fcurValue;
      if (fMin != -1.0) fVal = __max(fVal, fMin);
      if (fMax != -1.0) fVal = __min(fVal, fMax);
...
CDescription::~CDescription() {
  ...
  if (m_pszHintText) free(m_pszHintText);
  if (m_pszDescriptionType) free(m_pszDescriptionType);
}

CScriptObject *CDescription::FindObject(const char *pszObjectName) {
  if (!pszObjectName) return NULL;
...
void CDescription::TransferCurrentValues(const char *pszConfigFile) {
  ...
  while (pObj) {
    ...
    use LocalPlayerInfo because PlayerInfo strips keys prefixed with "_"
    }
    else
    {
