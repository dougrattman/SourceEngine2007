...
class CGammaDialog : public vgui::Frame {
 ...
 public:
  CGammaDialog(vgui::VPANEL hParent)
    ...
    // parent is ignored, since we want look like we're steal focus from the
    // parent (we'll become modal below)
    SetTitle("#GameUI_AdjustGamma_Title", true);
    SetSize(400, 260);
    SetDeleteSelfOnClose(true);

    m_pGammaSlider = new CCvarSlider(this, "Gamma", "#GameUI_Gamma", 1.6f, 2.6f,
                                     "mat_monitorgamma");
  ...
  MESSAGE_FUNC_PTR(OnControlModified, "ControlModified", panel) {
    // the HasBeenModified() check is so that if the value is outside of the
    // range of the slider, it won't use the slider to determine the display
    // value but leave the real value that we determined in the constructor
...
class COptionsSubVideoAdvancedDlg : public vgui::Frame {
 ...
 public:
  COptionsSubVideoAdvancedDlg(vgui::Panel *parent)
    ...
    for (int i = 0; i < ARRAYSIZE(g_DirectXLevels); i++) {
      ...
      if (g_DirectXLevels[i] == pKeyValues->GetInt("ConVar.mat_dxlevel") ||
          materials->GetRecommendedConfigurationInfo(g_DirectXLevels[i],
                                                     pTempKV)) {
        // add the configuration in the combo
        char szDXLevelName[64];
        GetNameForDXLevel(g_DirectXLevels[i], szDXLevelName,
                          sizeof(szDXLevelName));
        m_pDXLevel->AddItem(szDXLevelName, new KeyValues("dxlevel", "dxlevel",
                                                         g_DirectXLevels[i]));
      }
    ...
    // Build list of MSAA and CSAA modes, based upon those which are supported
    // by the device
    //
  ...
  int FindMSAAMode(int nAASamples, int nAAQuality) {
    ...
    for (int nAAMode = 0; nAAMode < m_nNumAAModes; nAAMode++) {
      // If we found the mode that matches what we're looking for, return the
      // index
      if ((m_nAAModes[nAAMode].m_nNumSamples == nAASamples) &&
          (m_nAAModes[nAAMode].m_nQualityLevel == nAAQuality)) {
        return nAAMode;
      }
    }
  ...
  MESSAGE_FUNC(OnGameUIHidden,
               "GameUIHidden")  // called when the GameUI is hidden
  {
    Close();
  }
  ...
  void MarkDefaultSettingsAsRecommended() {
    // Pull in data from dxsupport.cfg database (includes fine-grained
    // per-vendor/per-device config data)
    KeyValues *pKeyValues = new KeyValues("config");
    materials->GetRecommendedConfigurationInfo(0, pKeyValues);

    // Read individual values from keyvalues which came from dxsupport.cfg
    // database
    ...
    if (nShadowDepthTextureShadows)
      SetComboItemAsRecommended(
          m_pShadowDetail,
          2);  // Shadow depth mapping (in addition to RTT shadows)
  ...
  virtual void ApplyChanges() {
    if (!m_bUseChanges) return;

    ApplyChangesToConVar(
        "mat_dxlevel", m_pDXLevel->GetActiveItemUserData()->GetInt("dxlevel"));
    ...
    // reset everything tied to the filtering mode, then the switch sets the
    // appropriate one
    ...
    ApplyChangesToConVar("mat_antialias",
                         m_nAAModes[nActiveAAItem].m_nNumSamples);
    ApplyChangesToConVar("mat_aaquality",
                         m_nAAModes[nActiveAAItem].m_nQualityLevel);
    ...
    {
      ApplyChangesToConVar("r_shadowrendertotexture",
                           0);  // Turn off RTT shadows
      ApplyChangesToConVar("r_flashlightdepthtexture",
                           0);  // Turn off shadow depth textures
    } else if (m_pShadowDetail->GetActiveItem() == 1)  // RTT shadows only
    {
      ApplyChangesToConVar("r_shadowrendertotexture",
                           1);  // Turn on RTT shadows
      ApplyChangesToConVar("r_flashlightdepthtexture",
                           0);  // Turn off shadow depth textures
    } else if (m_pShadowDetail->GetActiveItem() == 2)  // Shadow depth textures
    {
      ApplyChangesToConVar("r_shadowrendertotexture",
                           1);  // Turn on RTT shadows
      ApplyChangesToConVar("r_flashlightdepthtexture",
                           1);  // Turn on shadow depth textures
    }

    ApplyChangesToConVar("mat_reducefillrate",
                         (m_pShaderDetail->GetActiveItem() > 0) ? 0 : 1);
    ...
    ApplyChangesToConVar("mat_colorcorrection",
                         m_pColorCorrection->GetActiveItem());

    ApplyChangesToConVar("mat_motion_blur_enabled",
                         m_pMotionBlur->GetActiveItem());
  }
  ...
  virtual void OnResetData() {
    ...
    {
      r_shadowrendertotexture.SetValue(
          1);  // ...be sure render to texture shadows are also on
      m_pShadowDetail->ActivateItem(2);
    } else if (r_shadowrendertotexture
                   .GetBool())  // RTT shadows, but not shadow depth texturing
    {
  ...
  bool RequiresRestart() {
    if (GameUI().IsInLevel()) {
      if (GameUI().IsInBackgroundLevel()) return false;
      if (!GameUI().IsInMultiplayer()) return false;
      ...
      if (m_pHDR->IsEnabled()) {
        ConVarRef mat_hdr_level("mat_hdr_level");
        Assert(mat_hdr_level.IsValid());
        if (mat_hdr_level.GetInt() != m_pHDR->GetActiveItem()) return true;
      }
    }
    return false;
  }
 ...
 private:
  ...
  vgui::ComboBox *m_pModelDetail, *m_pTextureDetail, *m_pAntialiasingMode,
      *m_pFilteringMode;
  vgui::ComboBox *m_pShadowDetail, *m_pHDR, *m_pWaterDetail, *m_pVSync,
      *m_pShaderDetail;
...
COptionsSubVideo::COptionsSubVideo(vgui::Panel *parent)
  ...
  m_pThirdPartyCredits = new URLButton(this, "ThirdPartyVideoCredits",
                                       "#GameUI_ThirdPartyTechCredits");
  m_pThirdPartyCredits->SetCommand(
      new KeyValues("OpenThirdPartyVideoCreditsDialog"));
  ...
  int iAspectMode = GetScreenAspectMode(config.m_VideoMode.m_Width,
...
void COptionsSubVideo::OnApplyChanges() {
  if (RequiresRestart()) {
    ...
    if (nci) {
      ...
      if (pAddr) {
        if (Q_strncmp(pAddr, "127.0.0.1", 9) &&
            Q_strncmp(pAddr, "localhost", 9)) {
  ...
  if (config.m_VideoMode.m_Width != width ||
      config.m_VideoMode.m_Height != height || config.Windowed() != windowed) {
    // set mode
    char szCmd[256];
  ...
  if (!m_hOptionsSubVideoAdvancedDlg.Get()) {
    m_hOptionsSubVideoAdvancedDlg =
        new COptionsSubVideoAdvancedDlg(BasePanel()->FindChildByName(
            "OptionsDialog"));  // we'll parent this to the OptionsDialog
                                // directly
  }
...
void COptionsSubVideo::OpenThirdPartyVideoCreditsDialog() {
  if (!m_OptionsSubVideoThirdPartyCreditsDlg.Get()) {
    m_OptionsSubVideoThirdPartyCreditsDlg =
        new COptionsSubVideoThirdPartyCreditsDlg(GetVParent());
  }
