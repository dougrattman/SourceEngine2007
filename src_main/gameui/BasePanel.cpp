...
ConVar vgui_message_dialog_modal("vgui_message_dialog_modal", "1",
                                 FCVAR_ARCHIVE);
...
//-----------------------------------------------------------------------------
CBasePanel *BasePanel() { return g_pBasePanel; }
...
//-----------------------------------------------------------------------------
VPANEL GetGameUIBasePanel() { return BasePanel()->GetVPanel(); }
...
void CGameMenuItem::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetDefaultColor(GetSchemeColor("MainMenu.TextColor", pScheme),
                  Color(0, 0, 0, 0));
  SetArmedColor(GetSchemeColor("MainMenu.ArmedTextColor", pScheme),
                Color(0, 0, 0, 0));
  SetDepressedColor(GetSchemeColor("MainMenu.DepressedTextColor", pScheme),
                    Color(0, 0, 0, 0));
  ...
  vgui::HFont hMainMenuFont =
      pScheme->GetFont("MainMenuFont", IsProportional());
  ...
  if (GameUI().IsConsoleUI()) {
    SetArmedColor(GetSchemeColor("MainMenu.ArmedTextColor", pScheme),
                  GetSchemeColor("Button.ArmedBgColor", pScheme));
...
void CGameMenuItem::PaintBackground() {
  if (!GameUI().IsConsoleUI()) {
    ...
    DrawBoxFade(2, 2, wide - 4, tall - 4, Color(0, 0, 0, 96), 1.0f, 255, 0,
                true);
  }
}

void CGameMenuItem::SetRightAlignedText(bool state) { m_bRightAligned = state; }
...
class CGameMenu : public vgui::Menu {
 ...
 public:
  ...
  virtual int AddMenuItem(const char *itemName, const char *itemText,
                          KeyValues *command, Panel *target,
                          KeyValues *userData = NULL) {
  ...
  virtual void SetMenuItemBlinkingState(const char *itemName, bool state) {
    for (int i = 0; i < GetChildCount(); i++) {
      ...
      if (menuItem) {
        if (Q_strcmp(menuItem->GetCommand()->GetString("command", ""),
                     itemName) == 0) {
  ...
  void UpdateMenuItemState(bool isInGame, bool isMultiplayer) {
    ...
    if (m_pConsoleFooter) {
      ...
      if (!m_pConsoleFooter->GetHelpName() ||
          V_stricmp(pHelpName, m_pConsoleFooter->GetHelpName())) {
        // game menu must re-establish its own help once it becomes re-active
        m_pConsoleFooter->SetHelpNameAndReset(pHelpName);
        m_pConsoleFooter->AddNewButtonLabel("#GameUI_Action",
                                            "#GameUI_Icons_A_BUTTON");
        if (isInGame) {
          m_pConsoleFooter->AddNewButtonLabel("#GameUI_Close",
...
static CBackgroundMenuButton *CreateMenuButton(CBasePanel *parent,
                                               const char *panelName,
                                               const wchar_t *panelText) {
...
CBasePanel::CBasePanel() : Panel(NULL, "BaseGameUIPanel") {
  ...
  if (GameUI().IsConsoleUI()) {
    ...
    m_pConsoleAnimationController->SetScriptFile(
        GetVPanel(), "scripts/GameUIAnimations.txt");
    m_pConsoleAnimationController->SetAutoReloadScript(IsDebug());

    m_pConsoleControlSettings = new KeyValues("XboxDialogs.res");
    if (!m_pConsoleControlSettings->LoadFromFile(
            g_pFullFileSystem, "resource/UI/XboxDialogs.res", "GAME")) {
      Error("Failed to load UI control settings!\n");
    } else {
      m_pConsoleControlSettings->ProcessResolutionKeys(
          surface()->GetResolutionKey());
    }
  }

  m_pGameMenuButtons.AddToTail(
      CreateMenuButton(this, "GameMenuButton", ModInfo().GetGameTitle()));
  m_pGameMenuButtons.AddToTail(
  ...
  if (SteamClient()) {
    ...
    ISteamUtils *pUtils =
        SteamClient()->GetISteamUtils(steamPipe, "SteamUtils002");
    if (pUtils) {
...
static void CC_GameMenuCommand(const CCommand &args) {
  ...
  vgui::ivgui()->PostMessage(g_pBasePanel->GetVPanel(),
                             new KeyValues("Command", "command", args[1]),
                             NULL);
}
...
static int CC_GameMenuCompletionFunc(
  ...
  int slot = 0;
  for (i = symbols.FirstInorder(); i != symbols.InvalidIndex();
...
static ConCommand gamemenucommand("gamemenucommand", CC_GameMenuCommand,
                                  "Issue game menu command.", 0,
                                  CC_GameMenuCompletionFunc);
...
void CBasePanel::UpdateBackgroundState() {
  ...
  if (GameUI().IsConsoleUI()) {
    if (!m_ExitingFrameCount && !m_bLevelLoading && !g_hLoadingDialog.Get() &&
        GameUI().IsInLevel()) {
      // paused
  ...
  for (i = 0; i < GetChildCount(); ++i) {
    ...
    if (child && ipanel()->IsVisible(child) && ipanel()->IsPopup(child) &&
  ...
  if (m_bHaveDarkenedBackground != bNeedDarkenedBackground) {
    ...
    vgui::GetAnimationController()->RunAnimationCommand(
        this, "m_flBackgroundFillAlpha", targetAlpha, 0.0f, duration,
  ...
  if (m_bHaveDarkenedTitleText != bNeedDarkenedTitleText ||
      m_bForceTitleTextUpdate) {
    ...
    if (m_pGameLogo) {
      vgui::GetAnimationController()->RunAnimationCommand(
          m_pGameLogo, "alpha", targetTitleAlpha, 0.0f, duration,
          AnimationController::INTERPOLATOR_LINEAR);
    }
    ...
    for (i = 0; i < m_pGameMenuButtons.Count(); ++i) {
      vgui::GetAnimationController()->RunAnimationCommand(
          m_pGameMenuButtons[i], "alpha", targetTitleAlpha, 0.0f, duration,
          AnimationController::INTERPOLATOR_LINEAR);
    }
...
void CBasePanel::SetBackgroundRenderState(EBackgroundState state) {
  ...
  if (state == BACKGROUND_EXITING) {
    ...
    if (GameUI().IsConsoleUI()) {
      RunAnimationWithCallback(this, "InstantHideMainMenu",
                               new KeyValues("LoadMap"));
    }
...
void CBasePanel::DrawBackgroundImage() {
  ...
  // plaque
  if ((m_bRenderingBackgroundTransition ||
       m_eBackgroundState == BACKGROUND_LOADING)) {
...
CGameMenu *CBasePanel::RecursiveLoadGameMenu(KeyValues *datafile) {
  ...
  for (KeyValues *dat = datafile->GetFirstSubKey(); dat != NULL;
       dat = dat->GetNextKey()) {
    ...
    //		if ( !Q_stricmp( cmd, "OpenFriendsDialog" ) &&
    // bSteamCommunityFriendsVersion ) 			continue;
...
void CBasePanel::PerformLayout() {
  ...
  for (int i = 0; i < m_pGameMenuButtons.Count(); ++i) {
    ...
    m_pGameMenuButtons[i]->SetPos(m_iGameTitlePos[i].x,
                                  m_iGameTitlePos[i].y + yDiff);
  ...
  if (m_pGameLogo) {
    ...
    m_pGameLogo->SetPos(
        m_iGameMenuPos.x + m_pGameLogo->GetOffsetX(),
        idealMenuY - m_pGameLogo->GetTall() + m_pGameLogo->GetOffsetY());
  }
...
void CBasePanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  IScheme *pClientScheme =
      vgui::scheme()->GetIScheme(vgui::scheme()->GetScheme("ClientScheme"));
  CUtlVector<Color> buttonColor;
  if (pClientScheme) {
    m_iGameTitlePos.RemoveAll();
    for (i = 0; i < m_pGameMenuButtons.Count(); ++i) {
      m_pGameMenuButtons[i]->SetFont(
          pClientScheme->GetFont("ClientTitleFont", true));
      m_iGameTitlePos.AddToTail(coord());
      m_iGameTitlePos[i].x = atoi(
          pClientScheme->GetResourceString(CFmtStr("Main.Title%d.X", i + 1)));
      m_iGameTitlePos[i].x =
          scheme()->GetProportionalScaledValue(m_iGameTitlePos[i].x);
      m_iGameTitlePos[i].y = atoi(
          pClientScheme->GetResourceString(CFmtStr("Main.Title%d.Y", i + 1)));
      m_iGameTitlePos[i].y =
          scheme()->GetProportionalScaledValue(m_iGameTitlePos[i].y);

      if (GameUI().IsConsoleUI()) m_iGameTitlePos[i].x += MAIN_MENU_INDENT_X360;

      buttonColor.AddToTail(pClientScheme->GetColor(
          CFmtStr("Main.Title%d.Color", i + 1), Color(255, 255, 255, 255)));
    }
#ifdef CS_BETA
    if (!ModInfo()
             .NoCrosshair())  // hack to not show the BETA for HL2 or HL1Port
    {
      m_pGameMenuButtons[m_pGameMenuButtons.Count() - 1]->SetFont(
          pClientScheme->GetFont("BetaFont", true));
    }
#endif  // CS_BETA
    ...
    m_iGameMenuInset =
        atoi(pClientScheme->GetResourceString("Main.BottomBorder"));
  ...
  m_flFrameFadeInTime =
      atof(pScheme->GetResourceString("Frame.TransitionEffectTime"));
  ...
  surface()->DrawSetTextureFile(m_iLoadingImageID, "console/startup_loading",
                                false, false);
}
  ...
  if (!Q_stricmp(command, "OpenGameMenu")) {
    ...
    if (!SteamUser() || !SteamUser()->BLoggedOn()) {
      vgui::MessageBox *pMessageBox =
          new vgui::MessageBox("#GameUI_Achievements_SteamRequired_Title",
    ...
    if (!IsX360()) {
      ...
      if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Valve\\Source", NULL,
        ...
        RegSetValueEx(hKey, "Relaunch URL", 0, REG_SZ,
                      (const unsigned char *)szSteamURL, sizeof(szSteamURL));
...
//-----------------------------------------------------------------------------
void CBasePanel::ClearQueuedCommands() { m_CommandQueue.Purge(); }
...
static unsigned PanelJobWrapperFn(void *pvContext) {
  CBasePanel::CAsyncJobContext *pAsync =
      reinterpret_cast<CBasePanel::CAsyncJobContext *>(pvContext);
...
#ifdef _WIN32
  ThreadHandle_t hHandle =
      CreateSimpleThread(PanelJobWrapperFn, reinterpret_cast<void *>(pAsync));
...
class CAsyncCtxOnDeviceAttached : public CBasePanel::CAsyncJobContext {
 public:
  ...
  uint32_t GetContainerOpenResult(void) { return m_ContainerOpenResult; }

 private:
  uint32_t m_ContainerOpenResult;
};
...
void CBasePanel::OnCompletedAsyncDeviceAttached(
    CAsyncCtxOnDeviceAttached *job) {
  uint32_t nRet = job->GetContainerOpenResult();
...
//-----------------------------------------------------------------------------
bool CBasePanel::HandleSignInRequest(const char *command) { return true; }
...
void CBasePanel::OnCommand(const char *command) {
  if (GameUI().IsConsoleUI()) {
    RunAnimationWithCallback(
        this, command, new KeyValues("RunMenuCommand", "command", command));
...
//-----------------------------------------------------------------------------
void CBasePanel::RunAnimationWithCallback(vgui::Panel *parent,
                                          const char *animName,
                                          KeyValues *msgFunc) {
  ...
  float sequenceLength =
      m_pConsoleAnimationController->GetAnimationSequenceLength(animName);
  if (sequenceLength) {
...
class CSaveBeforeQuitQueryDialog : public vgui::Frame {
 ...
 public:
  CSaveBeforeQuitQueryDialog(vgui::Panel *parent, const char *name)
  ...
  virtual void OnCommand(const char *command) {
    if (!Q_stricmp(command, "Quit")) {
      PostMessage(GetVParent(),
                  new KeyValues("Command", "command", "QuitNoConfirm"));
...
void CBasePanel::OnOpenQuitConfirmationDialog() {
  if (GameUI().IsConsoleUI()) {
    if (!GameUI().HasSavedThisMenuSession() && GameUI().IsInLevel() &&
  ...
  if (GameUI().IsInLevel() && engine->GetMaxClients() == 1) {
    ...
    CSaveBeforeQuitQueryDialog *box =
        new CSaveBeforeQuitQueryDialog(this, "SaveBeforeQuitQueryDialog");
    box->DoModal();
  } else {
    // simple ok/cancel prompt
    QueryBox *box = new CQuitQueryBox("#GameUI_QuitConfirmationTitle",
                                      "#GameUI_QuitConfirmationText", this);
    box->SetOKButtonText("#GameUI_Quit");
    box->SetOKCommand(new KeyValues("Command", "command", "QuitNoConfirm"));
    box->SetCancelCommand(
        new KeyValues("Command", "command", "ReleaseModalWindow"));
...
CMatchmakingBasePanel *CBasePanel::GetMatchmakingBasePanel() {
  ...
  if (m_bUseMatchmaking) {
    pBase =
        dynamic_cast<CMatchmakingBasePanel *>(m_hMatchmakingBasePanel.Get());
  }
  return pBase;
}
...
//-----------------------------------------------------------------------------
void CBasePanel::ShowMessageDialog(const uint32_t nType, vgui::Panel *pOwner) {
  if (pOwner == NULL) {
    pOwner = this;
  }
...
// Purpose: Add an Xbox 360 message dialog to a dialog stack
//-----------------------------------------------------------------------------
void CBasePanel::CloseMessageDialog(const uint32_t nType) {
...
void CBasePanel::SystemNotification(const int notification) {
  ...
  if (notification == SYSTEMNOTIFY_USER_SIGNEDIN) {
    ...
    uint32_t state = XUserGetSigninState(XBX_GetPrimaryUserId());
    ...
    if (m_bWaitingForStorageDeviceHandle) {
      DWORD ret = xboxsystem->GetOverlappedResult(m_hStorageDeviceChangeHandle,
                                                  NULL, true);
      if (ret != ERROR_IO_INCOMPLETE) {
        ...
        if (m_iStorageID != XBX_INVALID_STORAGE_ID) {
          ...
          if (xboxsystem->DeviceCapacityAdequate(
                  m_iStorageID, COM_GetModDirectory()) == false) {
...
//-----------------------------------------------------------------------------
void CBasePanel::UpdatePlayerInfo(uint64_t nPlayerId, const char *pName,
                                  int nTeam, uint8_t cVoiceState,
                                  int nPlayersNeeded, bool bHost) {
  CMatchmakingBasePanel *pBase = GetMatchmakingBasePanel();
  if (pBase) {
    pBase->UpdatePlayerInfo(nPlayerId, pName, nTeam, cVoiceState,
                            nPlayersNeeded, bHost);
  }
}
...
//-----------------------------------------------------------------------------
void CBasePanel::SessionSearchResult(int searchIdx, void *pHostData,
...
void CBasePanel::OnChangeStorageDevice(void) {
  if (m_bWaitingForStorageDeviceHandle == false) {
    ...
    m_iStorageID = XBX_INVALID_STORAGE_ID;
    xboxsystem->ShowDeviceSelector(true, &m_iStorageID,
                                   &m_hStorageDeviceChangeHandle);
  }
}
...
void CFooterPanel::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_hButtonFont = pScheme->GetFont(
      (m_szButtonFont[0] != '\0') ? m_szButtonFont : "GameUIButtons");
  m_hTextFont =
      pScheme->GetFont((m_szTextFont[0] != '\0') ? m_szTextFont : "MenuLarge");
...
void CFooterPanel::ApplySettings(KeyValues *inResourceData) {
  ...
  Q_strncpy(m_szTextFont, inResourceData->GetString("fonttext", "MenuLarge"),
            sizeof(m_szTextFont));
  Q_strncpy(m_szButtonFont,
            inResourceData->GetString("fontbutton", "GameUIButtons"),
            sizeof(m_szButtonFont));

  // fg and bg colors
  Q_strncpy(m_szFGColor, inResourceData->GetString("fgcolor", "White"),
            sizeof(m_szFGColor));
  Q_strncpy(m_szBGColor, inResourceData->GetString("bgcolor", "Black"),
            sizeof(m_szBGColor));

  for (KeyValues *pButton = inResourceData->GetFirstSubKey(); pButton != NULL;
       pButton = pButton->GetNextKey()) {
...
void CFooterPanel::AddButtonsFromMap(vgui::Frame *pMenu) {
  ...
  CControllerMap *pMap =
      dynamic_cast<CControllerMap *>(pMenu->FindChildByName("ControllerMap"));
  if (pMap) {
...
//-----------------------------------------------------------------------------
const char *CFooterPanel::GetHelpName() { return m_pHelpName; }
...
void CFooterPanel::Paint(void) {
  ...
  if (!m_bCenterHorizontal) {
    ...
    for (int i = 0; i < m_ButtonLabels.Count(); ++i) {
      ...
      if (iTextWidth == 0)
        x +=
            m_nButtonGap;  // There's no text, so remove the gap between buttons
      else
        x -= iTextWidth;
      ...
      // and text
      x -=
          (vgui::surface()->GetCharacterWidth(m_hButtonFont, pButton->icon[0]) +
           m_ButtonSeparator);
    ...
    totalWidth += (nButtonCount - 1) *
                  m_nButtonGap;  // add in the gaps between the buttons
    x -= (totalWidth / 2);
    ...
    for (i = 0; i < m_ButtonLabels.Count(); ++i) {
      ...
      vgui::surface()->DrawSetTextPos(x, y);
      vgui::surface()->DrawPrintText(pButton->icon, 1);
      x += vgui::surface()->GetCharacterWidth(m_hButtonFont, pButton->icon[0]) +
           m_ButtonSeparator;
...
CMessageDialogHandler::CMessageDialogHandler() { m_iDialogStackTop = -1; }
void CMessageDialogHandler::ShowMessageDialog(int nType, vgui::Panel *pOwner) {
  ...
  switch (nType) {
    case MD_SEARCHING_FOR_GAMES:
      CreateMessageDialog(MD_CANCEL | MD_RESTRICTPAINT, NULL,
                          "#TF_Dlg_SearchingForGames", NULL, "CancelOperation",
                          pOwner, true);
      break;
    ...
    case MD_SESSION_SEARCH_FAILED:
      CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, NULL,
                          "#TF_Dlg_NoGamesFound", "ShowSessionOptionsDialog",
                          "ReturnToMainMenu", pOwner);
      break;

    case MD_SESSION_CREATE_FAILED:
      CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_CreateFailed",
                          "ReturnToMainMenu", NULL, pOwner);
      break;

    case MD_SESSION_CONNECTING:
      CreateMessageDialog(0, NULL, "#TF_Dlg_Connecting", NULL, NULL, pOwner,
                          true);
      break;

    case MD_SESSION_CONNECT_NOTAVAILABLE:
      CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_JoinRefused",
                          "ReturnToMainMenu", NULL, pOwner);
      break;

    case MD_SESSION_CONNECT_SESSIONFULL:
      CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_GameFull", "ReturnToMainMenu",
                          NULL, pOwner);
      break;

    case MD_SESSION_CONNECT_FAILED:
      CreateMessageDialog(MD_OK, NULL, "#TF_Dlg_JoinFailed", "ReturnToMainMenu",
                          NULL, pOwner);
      break;

    case MD_LOST_HOST:
      CreateMessageDialog(MD_OK | MD_RESTRICTPAINT, NULL, "#TF_Dlg_LostHost",
                          "ReturnToMainMenu", NULL, pOwner);
      break;

    case MD_LOST_SERVER:
      CreateMessageDialog(MD_OK | MD_RESTRICTPAINT, NULL, "#TF_Dlg_LostServer",
                          "ReturnToMainMenu", NULL, pOwner);
      break;

    case MD_MODIFYING_SESSION:
      CreateMessageDialog(MD_RESTRICTPAINT, NULL, "#TF_Dlg_ModifyingSession",
                          NULL, NULL, pOwner, true);
      break;

    case MD_SAVE_BEFORE_QUIT:
      CreateMessageDialog(MD_YESNO | iSimpleFrame | MD_RESTRICTPAINT,
                          "#GameUI_QuitConfirmationTitle",
                          "#GameUI_Console_QuitWarning", "QuitNoConfirm",
    ...
    case MD_QUIT_CONFIRMATION:
      CreateMessageDialog(MD_YESNO | iSimpleFrame | MD_RESTRICTPAINT,
                          "#GameUI_QuitConfirmationTitle",
                          "#GameUI_QuitConfirmationText", "QuitNoConfirm",
                          "CloseQuitDialog_OpenMainMenu", pOwner);
      break;
    ...
    case MD_DISCONNECT_CONFIRMATION:
      CreateMessageDialog(MD_YESNO | MD_RESTRICTPAINT, "",
                          ...
                          "DisconnectNoConfirm", "close_dialog", pOwner);
      break;
    ...
    case MD_KICK_CONFIRMATION:
      CreateMessageDialog(MD_YESNO, "", "#TF_Dlg_ConfirmKick", "KickPlayer",
                          "close_dialog", pOwner);
      break;

    case MD_CLIENT_KICKED:
      CreateMessageDialog(MD_OK | MD_RESTRICTPAINT, "", "#TF_Dlg_ClientKicked",
                          "close_dialog", NULL, pOwner);
      break;

    case MD_EXIT_SESSION_CONFIRMATION:
      CreateMessageDialog(MD_YESNO, "", "#TF_Dlg_ExitSessionText",
                          "ReturnToMainMenu", "close_dialog", pOwner);
      break;

    case MD_STORAGE_DEVICES_NEEDED:
      CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE |
                              iSimpleFrame | MD_RESTRICTPAINT,
                          "#GameUI_Console_StorageRemovedTitle",
                          "#GameUI_Console_StorageNeededBody",
                          "ShowDeviceSelector", "QuitNoConfirm", pOwner);
      break;
    ...
    case MD_STORAGE_DEVICES_CHANGED:
      CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE |
                          ...
                          "ShowDeviceSelector", "clear_storage_deviceID",
                          pOwner);
      break;
    ...
    case MD_STORAGE_DEVICES_TOO_FULL:
      CreateMessageDialog(MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE |
                          ...
                          "ShowDeviceSelector", "StorageDeviceDenied", pOwner);
      break;
    ...
    case MD_PROMPT_STORAGE_DEVICE_REQUIRED:
      CreateMessageDialog(
          MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | MD_SIMPLEFRAME,
          "#GameUI_Console_NoStorageDeviceSelectedTitle",
          "#GameUI_Console_StorageDeviceRequiredBody", "ShowDeviceSelector",
          "RequiredStorageDenied", pOwner);
      break;

    case MD_PROMPT_SIGNIN:
      CreateMessageDialog(
          MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame,
    ...
    case MD_PROMPT_SIGNIN_REQUIRED:
      CreateMessageDialog(
          MD_YESNO | MD_WARNING | MD_COMMANDAFTERCLOSE | iSimpleFrame,
          "#GameUI_Console_NoUserProfileSelectedTitle",
          "#GameUI_Console_UserProfileRequiredBody", "ShowSignInUI",
          "RequiredSignInDenied", pOwner);
      break;

    case MD_NOT_ONLINE_ENABLED:
      CreateMessageDialog(MD_YESNO | MD_WARNING, "", "#TF_Dlg_NotOnlineEnabled",
                          "ShowSigninUI", "close_dialog", pOwner);
      break;

    case MD_NOT_ONLINE_SIGNEDIN:
      CreateMessageDialog(MD_YESNO | MD_WARNING, "",
                          "#TF_Dlg_NotOnlineSignedIn", "ShowSigninUI",
                          "close_dialog", pOwner);
      break;

    case MD_DEFAULT_CONTROLS_CONFIRM:
      CreateMessageDialog(
          MD_YESNO | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT,
          "#GameUI_RestoreDefaults", "#GameUI_ControllerSettingsText",
    ...
    case MD_COMMENTARY_EXPLANATION_MULTI:
      CreateMessageDialog(MD_OK | MD_WARNING, "#GameUI_CommentaryDialogTitle",
                          "#GAMEUI_Commentary_Console_Explanation",
                          "StartNewGameNoCommentaryExplanation", NULL, pOwner);
      break;
    ...
    case MD_COMMENTARY_CHAPTER_UNLOCK_EXPLANATION:
      CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT,
                          "#GameUI_CommentaryDialogTitle",
                          "#GameUI_CommentaryUnlock", "close_dialog", NULL,
                          pOwner);
      break;

    case MD_SAVE_BEFORE_LANGUAGE_CHANGE:
      CreateMessageDialog(MD_YESNO | MD_WARNING | MD_SIMPLEFRAME |
                              MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
                          "#GameUI_ChangeLanguageRestart_Title",
                          "#GameUI_ChangeLanguageRestart_Info",
                          "AcceptVocalsLanguageChange",
                          "CancelVocalsLanguageChange", pOwner);

    case MD_SAVE_BEFORE_NEW_GAME:
      CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame |
                              MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
                          "#GameUI_ConfirmNewGame_Title",
                          "#GameUI_NewGameWarning", "StartNewGame",
                          "close_dialog", pOwner);
      break;
    ...
    case MD_SAVE_BEFORE_LOAD:
      CreateMessageDialog(MD_OKCANCEL | MD_WARNING | iSimpleFrame |
                          ...
                          "#GameUI_LoadWarning", "LoadGame",
                          "LoadGameCancelled", pOwner);
      break;

    case MD_DELETE_SAVE_CONFIRM:
      CreateMessageDialog(
          MD_OKCANCEL | MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE,
          "#GameUI_ConfirmDeleteSaveGame_Title",
          "#GameUI_ConfirmDeleteSaveGame_Info", "DeleteGame",
          "DeleteGameCancelled", pOwner);
      break;
    ...
    case MD_SAVE_OVERWRITE:
      CreateMessageDialog(
          ...
          "#GameUI_ConfirmOverwriteSaveGame_Info", "SaveGame",
          "OverwriteGameCancelled", pOwner);
      break;

    case MD_SAVING_WARNING:
      CreateMessageDialog(MD_WARNING | iSimpleFrame | MD_COMMANDONFORCECLOSE,
                          "", "#GameUI_SavingWarning", "SaveSuccess", NULL,
                          pOwner, true);
      break;
    ...
    case MD_SAVE_COMPLETE:
      CreateMessageDialog(MD_OK | iSimpleFrame | MD_COMMANDAFTERCLOSE,
                          "#GameUI_ConfirmOverwriteSaveGame_Title",
                          "#GameUI_GameSaved", "CloseAndSelectResume", NULL,
                          pOwner);
      break;

    case MD_LOAD_FAILED_WARNING:
      CreateMessageDialog(
          MD_OK | MD_WARNING | iSimpleFrame, "#GameUI_LoadFailed",
          "#GameUI_LoadFailed_Description", "close_dialog", NULL, pOwner);
      break;
    ...
    case MD_STANDARD_SAMPLE:
      CreateMessageDialog(MD_OK, "Standard Dialog", "This is a standard dialog",
                          "close_dialog", NULL, pOwner);
      break;

    case MD_WARNING_SAMPLE:
      CreateMessageDialog(MD_OK | MD_WARNING, "#GameUI_Dialog_Warning",
                          "This is a warning dialog", "close_dialog", NULL,
                          pOwner);
      break;

    case MD_ERROR_SAMPLE:
      CreateMessageDialog(MD_OK | MD_ERROR, "Error Dialog",
                          "This is an error dialog", "close_dialog", NULL,
                          pOwner);
      break;

    case MD_STORAGE_DEVICES_CORRUPT:
      CreateMessageDialog(MD_OK | MD_WARNING | iSimpleFrame | MD_RESTRICTPAINT,
                          "", "#GameUI_Console_FileCorrupt", "close_dialog",
                          NULL, pOwner);
      break;

    case MD_CHECKING_STORAGE_DEVICE:
      CreateMessageDialog(iSimpleFrame | MD_RESTRICTPAINT, NULL,
                          "#GameUI_Dlg_CheckingStorageDevice", NULL, NULL,
                          pOwner, true);
      break;
...
void CMessageDialogHandler::CloseMessageDialog(const uint32_t nType) {
  int nStackIdx = 0;
  if (nType & MD_WARNING) {
...
void CMessageDialogHandler::CreateMessageDialog(
    const uint32_t nType, const char *pTitle, const char *pMsg,
    const char *pCmdA, const char *pCmdB, vgui::Panel *pCreator,
  ...
  // Show the new dialog
  m_hMessageDialogs[nStackIdx] = new CMessageDialog(
      BasePanel(), nType, pTitle, pMsg, pCmdA, pCmdB, pCreator, bShowActivity);

  m_hMessageDialogs[nStackIdx]->SetControlSettingsKeys(
      BasePanel()->GetConsoleControlSettings()->FindKey("MessageDialog.res"));

  if (nType & MD_RESTRICTPAINT) {
    vgui::surface()->RestrictPaintToSinglePanel(
        m_hMessageDialogs[nStackIdx]->GetVPanel());
  }
...
void CMessageDialogHandler::ActivateMessageDialog(int nStackIdx) {
  ...
  uint32_t nType = m_hMessageDialogs[nStackIdx]->GetType();
  if (nType & MD_WARNING) {
...
void CMessageDialogHandler::PositionDialog(vgui::PHandle dlg, int wide,
                                           int tall) {
  int w, t;
  dlg->GetSize(w, t);
...
static ConCommand gameui_show_dialog("gameui_show_dialog", CC_GameUIShowDialog,
                                     "Show an arbitrary Dialog.", 0);
...
static ConCommand gameui_hide_dialog("gameui_hide_dialog", CC_GameUIHideDialog,
                                     "asdf", 0);
