...
COptionsSubKeyboard::~COptionsSubKeyboard() { DeleteSavedBindings(); }
...
void COptionsSubKeyboard::OnApplyChanges() { ApplyAllBindings(); }
...
// Purpose: binds double-clicking or hitting enter in the keybind list to
// changing the key
...
char *UTIL_va(const char *fmt, ...) {
  ...
  va_start(argptr, fmt);
  Q_vsnprintf(string[index], 1024, fmt, argptr);
  va_end(argptr);
...
void COptionsSubKeyboard::ParseActionDescriptions(void) {
  ...
  if (!g_pFullFileSystem->ReadFile("scripts/kb_act.lst", NULL, buf)) return;
    ...
    if (szDescription[0] != '=') {
      ...
      if (!_stricmp(szBinding, "blank")) {
        ...
        m_pKeyBindList->AddColumnToSection(
            sectionIndex, "Action", szDescription,
            SectionedListPanel::COLUMN_BRIGHT, 286);
        m_pKeyBindList->AddColumnToSection(
            sectionIndex, "Key", "#GameUI_KeyButton",
            SectionedListPanel::COLUMN_BRIGHT, 128);
      } else {
...
// Purpose: Search current data set for item which has the specified binding
// string Input  : *binding - string to find Output : KeyValues or NULL on
// failure
KeyValues *COptionsSubKeyboard::GetItemForBinding(const char *binding) {
  ...
  for (int i = 0; i < m_pKeyBindList->GetItemCount(); i++) {
    KeyValues *item =
        m_pKeyBindList->GetItemData(m_pKeyBindList->GetItemIDFromRow(i));
    if (!item) continue;
...
void COptionsSubKeyboard::RemoveKeyFromBindItems(KeyValues *org_item,
                                                 const char *key) {
  Assert(key && key[0]);
  if (!key || !key[0]) return;
  ...
  for (int i = 0; i < m_pKeyBindList->GetItemCount(); i++) {
    ...
    if (!_stricmp(pszKey, item->GetString("Key", ""))) {
      ...
      if (org_item) {
        ...
        // us to have the same key bound to multiple entries in the keybinding
...
void COptionsSubKeyboard::FillInCurrentBindings(void) {
  ...
  for (int i = 0; i < BUTTON_CODE_LAST; i++) {
    ...
    if (item) {
      ...
      // Already in list, means user had two keys bound to this item.  We'll
      // only note the first one we encounter
      char const *currentKey = item->GetString("Key", "");
      if (currentKey && currentKey[0]) {
        ButtonCode_t currentBC =
            (ButtonCode_t)gameuifuncs->GetButtonCodeForBind(currentKey);

        // If we're using a joystick, joystick bindings override keyboard ones
        bool bShouldOverride = bJoystick && IsJoystickCode((ButtonCode_t)i) &&
                               !IsJoystickCode(currentBC);

        if (!bShouldOverride) continue;
...
void COptionsSubKeyboard::Finish(ButtonCode_t code) {
  ...
  if (item) {
    ...
    if (code != BUTTON_CODE_NONE && code != KEY_ESCAPE &&
...
void COptionsSubKeyboard::OpenKeyboardAdvancedDialog() {
  if (!m_OptionsSubKeyboardAdvancedDlg.Get()) {
    m_OptionsSubKeyboardAdvancedDlg =
        new COptionsSubKeyboardAdvancedDlg(GetVParent());
  }
