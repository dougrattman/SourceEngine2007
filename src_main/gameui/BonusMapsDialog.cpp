...
class CBonusMapPanel : public vgui::EditablePanel {
 ...
 public:
  CBonusMapPanel(PanelListPanel *parent, const char *name,
    ...
    m_pBonusMapScreenshotBackground =
        SETUP_PANEL(new ImagePanel(this, "BonusMapScreenshotBackground"));
  ...
  void SetBonusMapInfo(const char *pCurrentPath, BonusMapDescription_t &map) {
    ...
    if (map.bIsFolder) {
      if (map.szImageName[0] == '\0') {
        ...
        Q_snprintf(szImageFileName, sizeof(szImageFileName),
                   "%s/foldericon.tga", map.szFileName);
        bIsTGA = true;
  ...
  virtual void OnMouseDoublePressed(vgui::MouseCode code) {
    ...
    PostMessage(m_pParent->GetParent(),
                new KeyValues("Command", "command", "loadbonusmap"));
  }
...
// Purpose:Constructor
//-----------------------------------------------------------------------------
CBonusMapsDialog::CBonusMapsDialog(vgui::Panel *parent)
    : BaseClass(parent, "BonusMapsDialog") {
...
CBonusMapsDialog::~CBonusMapsDialog() {
  BonusMapsDatabase()
      ->WriteSaveData();  // Closing this dialog is a good time to save
...
bool CBonusMapsDialog::ImportZippedBonusMaps(const char *pchZippedFileName) {
  ...
  // If there's already a folder by the same name we're going to tack a number
  // onto the end
  ...
  while (g_pFullFileSystem->FileExists(szOutFilename, "MOD")) {
    ...
    Q_snprintf(szOutFilename, sizeof(szOutFilename), "%s%02i", szOutFilename,
               iSameFolderCount);
  }
...
void CBonusMapsDialog::BuildMapsList(void) {
  ...
  // add to the list
  for (int iMapIndex = 0;
       iMapIndex < iMapCount && iMapIndex < MAX_LISTED_BONUS_MAPS;
       ++iMapIndex) {
    CBonusMapPanel *bonusMapPanel =
        new CBonusMapPanel(m_pGameList, "BonusMapPanel", iMapIndex);
    bonusMapPanel->SetBonusMapInfo(
        BonusMapsDatabase()->GetPath(),
        *(BonusMapsDatabase()->GetBonusData(iMapIndex)));
    m_pGameList->AddItem(NULL, bonusMapPanel);
  }

  // display a message if there are no save games
  if (iMapCount <= 0) {
    vgui::Label *pNoSavesLabel = SETUP_PANEL(new Label(
        m_pGameList, "NoBonusMapsLabel", "#GameUI_NoBonusMapsToDisplay"));
...
void CBonusMapsDialog::CreateBonusMapsList() {
  ...
  m_pChallengeSelection =
      new vgui::ComboBox(this, "ChallengeSelection", 0, false);
}
...
int CBonusMapsDialog::GetSelectedItemBonusMapIndex() {
  CBonusMapPanel *panel =
      dynamic_cast<CBonusMapPanel *>(m_pGameList->GetSelectedPanel());
  if (panel &&
      panel->GetBonusMapListItemID() < BonusMapsDatabase()->BonusCount())
    return panel->GetBonusMapListItemID();

  return BonusMapsDatabase()->InvalidIndex();
}

void CBonusMapsDialog::SetSelectedBooleanStatus(const char *pchName,
                                                bool bValue) {
  CBonusMapPanel *pSelectedBonusMapPanel =
      (CBonusMapPanel *)m_pGameList->GetSelectedPanel();
  if (!pSelectedBonusMapPanel) return;

  BonusMapsDatabase()->SetBooleanStatus(
      pchName, pSelectedBonusMapPanel->GetBonusMapListItemID(), bValue);

  // Change the status in the dialog
  BonusMapDescription_t *pMap = BonusMapsDatabase()->GetBonusData(
      pSelectedBonusMapPanel->GetBonusMapListItemID());
  pSelectedBonusMapPanel->SetBonusMapInfo(BonusMapsDatabase()->GetPath(),
                                          *pMap);
...
void CBonusMapsDialog::RefreshData(void) {
  for (int iMap = 0; iMap < m_pGameList->GetItemCount(); ++iMap) {
    CBonusMapPanel *pBonusMapPanel =
        (CBonusMapPanel *)m_pGameList->GetItemPanel(iMap);

    if (pBonusMapPanel)
      pBonusMapPanel->SetBonusMapInfo(
          BonusMapsDatabase()->GetPath(),
          *(BonusMapsDatabase()->GetBonusData(
              pBonusMapPanel->GetBonusMapListItemID())));
  }
...
int CBonusMapsDialog::GetSelectedChallenge(void) {
  if (!m_pChallengeSelection->IsEnabled()) return -1;
...
void CBonusMapsDialog::RefreshDialog(BonusMapDescription_t *pMap) {
  if (!pMap || pMap->bLocked ||
      (m_pChallengeSelection->IsEnabled() && GetSelectedChallenge() == -1)) {
...
void CBonusMapsDialog::RefreshMedalDisplay(BonusMapDescription_t *pMap) {
  ...
  if (iChallenge < 0) {
    ...
    for (int iChallengeIndex = 0;
         ...
         ++iChallengeIndex) {
      KeyValues *pUserDataKey =
          m_pChallengeSelection->GetItemUserData(iChallengeIndex);
      ...
      if (iChallengeNum >= 0) {
        ...
        if (pChallengeDescription) {
          ...
          GetChallengeMedals(pChallengeDescription, iBest, iEarnedMedal, iNext,
                             iNextMedal);

          if (iChallengeNum < 10)
            Q_snprintf(szBuff, 256, "medals/medal_0%i_%s", iChallengeNum,
                       g_pszMedalNames[iEarnedMedal]);
          else
            Q_snprintf(szBuff, 256, "medals/medal_%i_%s", iChallengeNum,
                       g_pszMedalNames[iEarnedMedal]);
        }

        CBitmapImagePanel *pBitmap = dynamic_cast<CBitmapImagePanel *>(
            GetChild(iFirstChildIndex + iChallengeNum));
  ...
  if (iEarnedMedal > -1) {
    if (iChallenge < 10)
      Q_snprintf(szBuff, sizeof(szBuff), "medals/medal_0%i_%s", iChallenge,
                 g_pszMedalNames[iEarnedMedal]);
    else
      Q_snprintf(szBuff, sizeof(szBuff), "medals/medal_%i_%s", iChallenge,
                 g_pszMedalNames[iEarnedMedal]);

    CBitmapImagePanel *pBitmap = dynamic_cast<CBitmapImagePanel *>(
        FindChildByName("ChallengeEarnedMedal"));
    pBitmap->SetVisible(true);
    pBitmap->setTexture(szBuff);
  }
  ...
  if (iNextMedal > 0) {
    if (iChallenge < 10)
      Q_snprintf(szBuff, sizeof(szBuff), "medals/medal_0%i_%s", iChallenge,
                 g_pszMedalNames[iNextMedal]);
    else
      Q_snprintf(szBuff, sizeof(szBuff), "medals/medal_%i_%s", iChallenge,
                 g_pszMedalNames[iNextMedal]);

    CBitmapImagePanel *pBitmap = dynamic_cast<CBitmapImagePanel *>(
        FindChildByName("ChallengeNextMedal"));
    pBitmap->SetVisible(true);
    pBitmap->setTexture(szBuff);
  } else {
    SetControlVisible("ChallengeNextMedal", false);
  ...
  if (iBest != -1) {
    ...
    g_pVGuiLocalize->ConstructString(
        szWideBuff, sizeof(szWideBuff),
        g_pVGuiLocalize->Find("#GameUI_BonusMapsBest"), 1, szWideBuff2);
  ...
  if (iNext != -1) {
    ...
    g_pVGuiLocalize->ConstructString(
        szWideBuff, sizeof(szWideBuff),
        g_pVGuiLocalize->Find("#GameUI_BonusMapsGoal"), 1, szWideBuff2);
    g_pVGuiLocalize->ConvertUnicodeToANSI(szWideBuff, szBuff, sizeof(szBuff));
...
void CBonusMapsDialog::RefreshCompletionPercentage(void) {
  ...
  if (fPercentage > 0.0f) {
    ...
    if (fPercentage * 100.0f < 1.0f)
      Q_snprintf(szBuff, 256, "%.2f%%",
                 fPercentage * 100.0f);  // Show decimal places if less than 1%
    else
    ...
    Color cProgressBar =
        Color(static_cast<float>(m_PercentageBarBackgroundColor.r()) *
                      (1.0f - fPercentage) +
                  static_cast<float>(m_PercentageBarColor.r()) * fPercentage,
              static_cast<float>(m_PercentageBarBackgroundColor.g()) *
                      (1.0f - fPercentage) +
                  static_cast<float>(m_PercentageBarColor.g()) * fPercentage,
              static_cast<float>(m_PercentageBarBackgroundColor.b()) *
                      (1.0f - fPercentage) +
                  static_cast<float>(m_PercentageBarColor.b()) * fPercentage,
              static_cast<float>(m_PercentageBarBackgroundColor.a()) *
                      (1.0f - fPercentage) +
                  static_cast<float>(m_PercentageBarColor.a()) * fPercentage);
  ...
  if (!stricmp(command, "loadbonusmap")) {
    ...
    if (BonusMapsDatabase()->IsValidIndex(mapIndex)) {
      BonusMapDescription_t *pBonusMap =
          BonusMapsDatabase()->GetBonusData(mapIndex);
        ...
        if (pBonusMap->bIsFolder) {
          ...
          if (iChallenge > 0) {
            ...
            ChallengeDescription_t *pChallengeDescription =
                &((*pBonusMap->m_pChallenges)[iChallenge - 1]);

            // Set up medal goals
            BonusMapsDatabase()->SetCurrentChallengeObjectives(
                pChallengeDescription->iBronze, pChallengeDescription->iSilver,
                pChallengeDescription->iGold);
            BonusMapsDatabase()->SetCurrentChallengeNames(
                pBonusMap->szFileName, pBonusMap->szMapName,
                pChallengeDescription->szName);
          }
          ...
          if (pBonusMap->szMapFileName[0] != '.') {
            ...
            const char *pchSubDir =
                Q_strnchr(BonusMapsDatabase()->GetPath(), '/',
                          Q_strlen(BonusMapsDatabase()->GetPath()));
    ...
    m_pChallengeSelection->AddItem(
        "<Select A Challenge>",
        new KeyValues("ChallengeSelection", "challenge", -1));
...
void CBonusMapsDialog::OnPanelSelected() {
  ...
  if (pMap->m_pChallenges) {
    // Get the name of the previously selected challenge so that we can select a
    // matching challenge (if one exists)
    ...
    for (iNumChallenges; iNumChallenges < pMap->m_pChallenges->Count();
         ++iNumChallenges) {
      ChallengeDescription_t *pChallenge =
          &(*pMap->m_pChallenges)[iNumChallenges];
      int iType = iNumChallenges;

      // If the challenge type was specified then use that instead of (legacy)
      // the order the challenges were listed
      if (pChallenge->iType != -1) iType = pChallenge->iType;

      m_pChallengeSelection->AddItem(
          pChallenge->szName,
          new KeyValues("ChallengeSelection", "challenge", iType));
...
void CBonusMapsDialog::OnFileSelected(const char *fullpath) {
  if (fullpath == NULL || fullpath[0] == '\0') return;
