...
void CLoadGameDialogXbox::PerformDeletion(void) {
  ...
  if (pPanel == NULL || (pPanel && pPanel->IsAutoSaveType())) return;
...
//-----------------------------------------------------------------------------
void CLoadGameDialogXbox::DeleteSaveGame(
  ...
  // If we're deleting our more recent save game, we need to make sure we setup
  // the engine to properly load the last most recent
  if (Q_stristr(engine->GetMostRecentSaveGame(), pSaveDesc->szShortName)) {
    ...
    if (GetNumPanels() > 1) {
      // The panels are sorted by how recent they are, so the first element is
      // the most recent
      const SaveGameDescription_t *pDesc = GetPanelSaveDecription(0);
      if (pDesc == pSaveDesc) {
        // If we're deleting our most recent, we need to pick the next most
        // recent
