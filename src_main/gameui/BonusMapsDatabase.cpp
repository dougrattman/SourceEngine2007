...
const char g_pszMedalNames[4][8] = {"none", "bronze", "silver", "gold"};

const char *COM_GetModDirectory();

bool WriteBonusMapSavedData(KeyValues *data) {
  CUtlBuffer file_buffer(0, 0, CUtlBuffer::TEXT_BUFFER);
  ...
  bool was_file_saved =
      g_pFullFileSystem->WriteFile(file_name, MOD_DIR, file_buffer);

  xboxsystem->FinishContainerWrites();

  return was_file_saved;
}
...
void GetBooleanStatus(KeyValues *pBonusFilesKey, BonusMapDescription_t &map) {
  ...
  for (pFileKey = pBonusFilesKey->GetFirstSubKey(); pFileKey;
       pFileKey = pFileKey->GetNextTrueSubKey()) {
    if (Q_strcmp(pFileKey->GetName(), map.szFileName) == 0) {
      for (pBonusKey = pFileKey->GetFirstSubKey(); pBonusKey;
           pBonusKey = pBonusKey->GetNextKey()) {
...
bool SetBooleanStatus(KeyValues *pBonusFilesKey, const char *pchName,
...
float GetChallengeBests(KeyValues *pBonusFilesKey,
                        BonusMapDescription_t &challenge) {
  ...
  for (int iChallenge = 0; iChallenge < challenge.m_pChallenges->Count();
       ++iChallenge) {
    ChallengeDescription_t *pChallengeDescription =
        &((*challenge.m_pChallenges)[iChallenge]);
    pChallengeDescription->iBest =
        ((pBonusKey) ? (pBonusKey->GetInt(pChallengeDescription->szName, -1))
                     : (-1));
...
bool UpdateChallengeBest(KeyValues *pBonusFilesKey,
                         const BonusMapChallenge_t &challenge) {
  // Don't create entries for files that don't exist
  ...
  if (!pBonusKey) {
    pBonusKey =
        new KeyValues(challenge.szMapName, challenge.szChallengeName, -1);
    pFileKey->AddSubKey(pBonusKey);
    bChanged = true;
  }
...
void GetChallengeMedals(ChallengeDescription_t *pChallengeDescription,
                        int &iBest, int &iEarnedMedal, int &iNext,
                        int &iNextMedal) {
...
CBonusMapsDatabase *BonusMapsDatabase(void) {
  if (!g_pBonusMapsDatabase) static CBonusMapsDatabase StaticBonusMapsDatabase;
...
CBonusMapsDatabase::CBonusMapsDatabase(void) {
  Assert(g_pBonusMapsDatabase ==
         NULL);  // There should only be 1 bonus maps database
  g_pBonusMapsDatabase = this;

  RootPath();

  m_pBonusMapsManifest = new KeyValues("bonus_maps_manifest");
  m_pBonusMapsManifest->LoadFromFile(g_pFullFileSystem,
                                     "scripts/bonus_maps_manifest.txt", NULL);
...
bool CBonusMapsDatabase::ReadBonusMapSaveData(void) {
  ...
  if (g_bIsCreatingNewGameMenuForPreFetching) {
    // Although we may have a storage device it's not going to be able to find
    // our file at this point! BAIL!
    return false;
  }
...
void CBonusMapsDatabase::AppendPath(const char *pchAppend) {
  ++m_iDirDepth;
  Q_snprintf(m_szCurrentPath, sizeof(m_szCurrentPath), "%s/%s", m_szCurrentPath,
             pchAppend);
}

void CBonusMapsDatabase::BackPath(void) {
  if (m_iDirDepth == 0) return;
...
void CBonusMapsDatabase::ScanBonusMaps(void) {
  ...
  if (!m_bHasLoadedSaveData) {
    if (!ReadBonusMapSaveData()) return;
  }
  ...
  if (Q_strcmp(m_szCurrentPath, ".") == 0) {
    ...
    KeyValues *pKey = NULL;
    for (pKey = m_pBonusMapsManifest->GetFirstSubKey(); pKey;
...
void CBonusMapsDatabase::RefreshMapData(void) {
  ...
  for (int iMap = 0; iMap < m_BonusMaps.Count(); ++iMap) {
    ...
    float fCompletion = GetChallengeBests(
        m_pBonusMapSavedData->FindKey("bonusfiles", true), *pMap);
...
int CBonusMapsDatabase::BonusCount(void) {
  if (m_BonusMaps.Count() == 0) ScanBonusMaps();
...
bool CBonusMapsDatabase::BonusesUnlocked(void) {
  ...
  if (m_iX360BonusesUnlocked == 0) {
    ...
    for (int iBonusMap = 0; iBonusMap < BonusMapsDatabase()->BonusCount();
      ...
      if (Q_strcmp(pMap->szMapName, "#Bonus_Map_AdvancedChambers") == 0 &&
          !pMap->bLocked) {
        // All bonuses unlocked, remember this and set up the proper X360 scan
        // to get info.
...
void CBonusMapsDatabase::SetCurrentChallengeNames(
    const char *pchFileName, const char *pchMapName,
...
void CBonusMapsDatabase::GetCurrentChallengeObjectives(int &iBronze,
                                                       int &iSilver,
                                                       int &iGold) {
...
bool CBonusMapsDatabase::SetBooleanStatus(const char *pchName,
                                          const char *pchFileName,
                                          const char *pchMapName, bool bValue) {
  bool bChanged =
      ::SetBooleanStatus(m_pBonusMapSavedData->FindKey("bonusfiles", true),
                         pchName, pchFileName, pchMapName, bValue);
  if (bChanged) m_bSavedDataChanged = true;

  return bChanged;
}

bool CBonusMapsDatabase::SetBooleanStatus(const char *pchName, int iIndex,
                                          bool bValue) {
  BonusMapDescription_t *pMap = &(m_BonusMaps[iIndex]);

  bool bChanged =
      SetBooleanStatus(pchName, pMap->szFileName, pMap->szMapName, bValue);
  GetBooleanStatus(m_pBonusMapSavedData->FindKey("bonusfiles", true), *pMap);

  return bChanged;
}

bool CBonusMapsDatabase::UpdateChallengeBest(const char *pchFileName,
  ...
  bool bChanged = ::UpdateChallengeBest(
      m_pBonusMapSavedData->FindKey("bonusfiles", true), challenge);
  if (bChanged) m_bSavedDataChanged = true;
...
float CBonusMapsDatabase::GetCompletionPercentage(void) {
  ...
  if (m_iCompletableLevels <= 0) return 0.0f;
...
int CBonusMapsDatabase::NumAdvancedComplete(void) {
  ...
  for (int iBonusMap = 0; iBonusMap < BonusMapsDatabase()->BonusCount();
    ...
    if (pMap && Q_strstr(pMap->szMapName, "Advanced") != NULL) {
      ...
      if (pMap->bComplete) ++iNumComplete;
    }
  }
...
void CBonusMapsDatabase::NumMedals(int piNumMedals[3]) {
  ...
  for (int i = 0; i < 3; ++i) piNumMedals[i] = 0;
  ...
  for (int iBonusMap = 0; iBonusMap < BonusMapsDatabase()->BonusCount();
    ...
    if (pMap && pMap->m_pChallenges) {
      for (int iChallenge = 0; iChallenge < pMap->m_pChallenges->Count();
           ++iChallenge) {
        ChallengeDescription_t *pChallengeDescription =
            &((*pMap->m_pChallenges)[iChallenge]);
...
void CBonusMapsDatabase::AddBonus(const char *pCurrentPath,
  ...
  if (!g_pFullFileSystem->FileExists(szFileName, "MOD")) return;
...
void CBonusMapsDatabase::BuildSubdirectoryList(const char *pCurrentPath,
  ...
  const char *pDirFileName =
      g_pFullFileSystem->FindFirst(szDirectory, &dirHandle);

  while (pDirFileName) {
    // Skip it if it's not a directory, is the root, is back, or is an invalid
    // folder
    if (!g_pFullFileSystem->FindIsDirectory(dirHandle) ||
        Q_strcmp(pDirFileName, ".") == 0 || Q_strcmp(pDirFileName, "..") == 0 ||
...
void CBonusMapsDatabase::BuildBonusMapsList(const char *pCurrentPath,
  ...
  const char *pMapFileName =
      g_pFullFileSystem->FindFirst(szDirectory, &mapHandle);

  while (pMapFileName) {
    // Skip it if it's a directory or is the folder info
    if (g_pFullFileSystem->FindIsDirectory(mapHandle) ||
        Q_strstr(pMapFileName, "folderinfo.bns")) {
...
//-----------------------------------------------------------------------------
void CBonusMapsDatabase::ParseBonusMapData(char const *pszFileName,
  ...
  while (kv) {
    ...
    if (pChallenges) {
      for (KeyValues *pChallengeKey = pChallenges->GetFirstSubKey();
           pChallengeKey; pChallengeKey = pChallengeKey->GetNextKey()) {
    ...
    // Get boolean status last because it can be altered if all the challenges
    // were completed
    GetBooleanStatus(m_pBonusMapSavedData->FindKey("bonusfiles", true), *pMap);
...
static ConCommand sv_bonus_map_unlock("sv_bonus_map_unlock", CC_BonusMapUnlock,
                                      "Locks a bonus map.", FCVAR_CHEAT);
...
static ConCommand sv_bonus_map_complete("sv_bonus_map_complete",
                                        CC_BonusMapComplete,
                                        "Completes a bonus map.", FCVAR_CHEAT);
...
static ConCommand sv_bonus_map_challenge_update(
    "sv_bonus_map_challenge_update", CC_BonusMapChallengeUpdate,
    "Updates a bonus map challenge score.", FCVAR_CHEAT);
