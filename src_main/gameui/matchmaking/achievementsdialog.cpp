...
//-----------------------------------------------------------------------------
// Purpose: Sets the parameter pIconPanel to display the specified achievement's
// icon file.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// The bias is to ensure the percentage bar gets plenty orange before it reaches
// the text, as the white-on-grey is hard to read.
//-----------------------------------------------------------------------------
Color LerpColors(Color cStart, Color cEnd, float flPercent) {
  float r = (float)((float)(cStart.r()) +
                    (float)(cEnd.r() - cStart.r()) * Bias(flPercent, 0.75));
  float g = (float)((float)(cStart.g()) +
                    (float)(cEnd.g() - cStart.g()) * Bias(flPercent, 0.75));
  float b = (float)((float)(cStart.b()) +
                    (float)(cEnd.b() - cStart.b()) * Bias(flPercent, 0.75));
  float a = (float)((float)(cStart.a()) +
                    (float)(cEnd.a() - cStart.a()) * Bias(flPercent, 0.75));
  return Color(r, g, b, a);
}

//-----------------------------------------------------------------------------
// Purpose: Shares common percentage bar calculations/color settings between
// xbox and pc.
//			Not really intended for robustness or reuse across many
// panels.
// Input  : pFrame - assumed to have certain child panels (see below)
//			*pAchievement - source achievement to poll for progress.
// Non  progress achievements will not show a percentage bar.
//-----------------------------------------------------------------------------
void UpdateProgressBar(vgui::EditablePanel *pPanel, IAchievement *pAchievement,
                       Color clrProgressBar) {
  if (pAchievement->GetGoal() > 1) {
    bool bShowProgress = true;

    // if this achievement gets saved with game and we're not in a level and
    // have not achieved it, then we do not have any state for this achievement,
    // don't show progress
    if ((pAchievement->GetFlags() & ACH_SAVE_WITH_GAME) &&
        !GameUI().IsInLevel() && !pAchievement->IsAchieved()) {
      bShowProgress = false;
    }

    float flCompletion = 0.0f;

    // Once achieved, we can't rely on count. If they've completed the
    // achievement just set to 100%.
    int iCount = pAchievement->GetCount();
    if (pAchievement->IsAchieved()) {
      flCompletion = 1.0f;
      iCount = pAchievement->GetGoal();
    } else if (bShowProgress) {
      flCompletion = (((float)pAchievement->GetCount()) /
                      ((float)pAchievement->GetGoal()));
      // In rare cases count can exceed goal and not be achieved (switch local
    ...
    if (bShowProgress) {
      Q_snprintf(szPercentageText, 256, "%d/%d", iCount,
    ...
    vgui::ImagePanel *pPercentageBar =
        (vgui::ImagePanel *)pPanel->FindChildByName("PercentageBar");
    vgui::ImagePanel *pPercentageBarBkg =
        (vgui::ImagePanel *)pPanel->FindChildByName("PercentageBarBackground");
    ...
    if (pPercentageBar && pPercentageBarBkg) {
      ...
      pPanel->SetControlVisible("PercentageBarBackground",
                                IsX360() ? bShowProgress : true);
...
CAchievementsDialog_XBox::CAchievementsDialog_XBox(vgui::Panel *pParent)
    : BaseClass(pParent, "AchievementsDialog") {
...
void CAchievementsDialog_XBox::PerformLayout(void) {
  ...
  if (m_nTotalAchievements) {
    ...
    m_pProgressPercent->SetPos(
        GetWide() - m_pProgressPercent->GetWide() - m_nBorderWidth,
        y + tall + 3);
    ...
    wchar_t *wzNumberingFmt =
        g_pVGuiLocalize->Find("#GameUI_Achievement_Menu_Range");
  ...
  if (m_pFooterInfo &&
      (m_pFooterInfo->GetInt("hide_regular_footer", 0) > 0) == false) {
    for (KeyValues *pButton = m_pFooterInfo->GetFirstSubKey(); pButton != NULL;
...
void CAchievementsDialog_XBox::ApplySettings(KeyValues *pResourceData) {
  ...
  if (!achievementmgr) {
    AssertMsgOnce(0, "No achievement manager interface in gameui.dll");
    return;
  }
  ...
  for (int i = 0; i < iAllAchievements; ++i) {
    IAchievement *pCurAchievement =
        (IAchievement *)achievementmgr->GetAchievementByIndex(i);
    Assert(pCurAchievement);

    if (pCurAchievement->IsAchieved()) ++m_nUnlocked;

    // don't show hidden achievements if not achieved
    if (pCurAchievement->ShouldHideUntilAchieved() &&
        !pCurAchievement->IsAchieved())
      continue;

    CAchievementItem *pItem = m_Menu.AddAchievementItem(
        ACHIEVEMENT_LOCALIZED_NAME(pCurAchievement),
        ACHIEVEMENT_LOCALIZED_DESC(pCurAchievement),
        pCurAchievement->GetPointValue(), pCurAchievement->IsAchieved(),
        pCurAchievement);
...
void CAchievementsDialog_XBox::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pProgressPercent->SetFgColor(pScheme->GetColor(
      "MatchmakingMenuItemDescriptionColor", Color(64, 64, 64, 255)));
  m_pNumbering->SetFgColor(pScheme->GetColor(
      "MatchmakingMenuItemDescriptionColor", Color(64, 64, 64, 255)));
}

//-----------------------------------------------------------------------------
// Purpose: Scrolling up and down with up/down, exit to main menu with back
// button
//-----------------------------------------------------------------------------
void CAchievementsDialog_XBox::OnKeyCodePressed(vgui::KeyCode code) {
  // Handle close here, CBasePanel parent doesn't support "DialogClosing"
  // command
...
//-----------------------------------------------------------------------------
// Purpose: creates child panels, passes down name to pick up any settings from
// res files.
//-----------------------------------------------------------------------------
CAchievementsDialog::CAchievementsDialog(vgui::Panel *parent)
    : BaseClass(parent, "AchievementsDialog"), m_iFixedWidth{0} {
  SetDeleteSelfOnClose(true);
  ...
  m_pAchievementsList =
      new vgui::PanelListPanel(this, "listpanel_achievements");
  ...
  m_pPercentageBarBackground =
      SETUP_PANEL(new ImagePanel(this, "PercentageBarBackground"));
  m_pPercentageBar = SETUP_PANEL(new ImagePanel(this, "PercentageBar"));

  m_pAchievementPackCombo =
      new ComboBox(this, "achievement_pack_combo", 10, false);
  ...
  if (achievementmgr) {
    ...
    for (int i = 0; i < iCount; ++i) {
      ...
      // don't show hidden achievements if not achieved
      if (pCur->ShouldHideUntilAchieved() && !pCur->IsAchieved()) continue;
      ...
      CAchievementDialogItemPanel *achievementItemPanel =
          new CAchievementDialogItemPanel(m_pAchievementsList,
                                          "AchievementDialogItemPanel", i);
  ...
  for (int i = 0; i < m_iNumAchievementGroups; i++) {
    ...
    if (i == 0) {
      ...
      Q_snprintf(buf, sizeof(buf), "#Achievement_Group_%d",
...
void CAchievementsDialog::CreateNewAchievementGroup(int iMinRange,
                                                    int iMaxRange) {
...
//			Sets up progress bar displaying total achievement
// unlocking  progress by the user.
//-----------------------------------------------------------------------------
void CAchievementsDialog::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (achievementmgr->GetAchievementCount() > 0) {
    flCompletion =
        (((float)m_nUnlocked) / ((float)achievementmgr->GetAchievementCount()));
  }

  char szPercentageText[64];
  Q_snprintf(szPercentageText, 256, "%d / %d ( %d%% )", m_nUnlocked,
             achievementmgr->GetAchievementCount(),
             (int)(flCompletion * 100.0f));
  ...
  Color cProgressBar =
      Color(static_cast<float>(clrHighlight.r()) * (1.0f - flCompletion) +
                static_cast<float>(clrWhite.r()) * flCompletion,
            static_cast<float>(clrHighlight.g()) * (1.0f - flCompletion) +
                static_cast<float>(clrWhite.g()) * flCompletion,
            static_cast<float>(clrHighlight.b()) * (1.0f - flCompletion) +
                static_cast<float>(clrWhite.b()) * flCompletion,
            static_cast<float>(clrHighlight.a()) * (1.0f - flCompletion) +
                static_cast<float>(clrWhite.a()) * flCompletion);

  m_pPercentageBar->SetFgColor(cProgressBar);
  m_pPercentageBar->SetWide(m_pPercentageBarBackground->GetWide() *
                            flCompletion);
  ...
  if (m_iNumAchievementGroups <= 2) {
    // we have no achievement packs. Hide the combo and bump the achievement
    // list up a bit
    ...
    m_pAchievementsList->SetAutoResize(corner, AUTORESIZE_DOWN, pinX, comboY,
                                       resizeOffsetX, resizeOffsetY);

    m_pAchievementsList->SetBounds(x, comboY, w, h + (y - comboY));

    m_pListBG->SetAutoResize(corner, AUTORESIZE_DOWN, pinX, comboY,
                             resizeOffsetX, resizeOffsetY);
...
void CAchievementsDialog::UpdateAchievementDialogInfo(void) {
  for (int i = 0; i < m_pAchievementsList->GetItemCount(); i++) {
    CAchievementDialogItemPanel *pPanel =
        (CAchievementDialogItemPanel *)m_pAchievementsList->GetItemPanel(i);
    if (pPanel) {
...
CAchievementDialogItemPanel::CAchievementDialogItemPanel(
    vgui::PanelListPanel *parent, const char *name, int iListItemID)
  ...
  m_pAchievementIcon =
      SETUP_PANEL(new vgui::ImagePanel(this, "AchievementIcon"));
...
//-----------------------------------------------------------------------------
// Purpose: Updates displayed achievement data. In applyschemesettings, and when
// gameui activates.
//-----------------------------------------------------------------------------
void CAchievementDialogItemPanel::UpdateAchievementInfo() {
  if (m_pSourceAchievement && m_pSchemeSettings) {
    ...
    m_pAchievementNameLabel->SetText(
        ACHIEVEMENT_LOCALIZED_NAME(m_pSourceAchievement));
    m_pAchievementDescLabel->SetText(
        ACHIEVEMENT_LOCALIZED_DESC(m_pSourceAchievement));

    // Setup icon
    // get the vtfFilename from the path.

    // Display percentage completion for progressive achievements
    // Set up total completion percentage bar. Goal > 1 means its a progress
    // achievement.
...
//-----------------------------------------------------------------------------
// Purpose: Makes a local copy of a pointer to the achievement entity stored on
// the client.
//-----------------------------------------------------------------------------
void CAchievementDialogItemPanel::SetAchievementInfo(
    IAchievement *pAchievement) {
  if (!pAchievement) {
    Assert(0);
    return;
...
void CAchievementDialogItemPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (m_pSourceAchievement->IsAchieved()) {
    SetBgColor(
        pScheme->GetColor("AchievementsLightGrey", Color(255, 0, 0, 255)));

    m_pAchievementNameLabel->SetFgColor(
        pScheme->GetColor("SteamLightGreen", Color(255, 255, 255, 255)));
  } else {
    SetBgColor(
        pScheme->GetColor("AchievementsDarkGrey", Color(255, 0, 0, 255)));

    Color fgColor =
        pScheme->GetColor("AchievementsInactiveFG", Color(255, 255, 255, 255));
