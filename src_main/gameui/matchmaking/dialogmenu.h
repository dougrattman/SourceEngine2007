...
class CCommandItem : public CMenuItem {
 ...
 public:
  CCommandItem(CDialogMenu *pParent, const char *pTitle,
               const char *pDescription, const char *pCommand);
...
class CPlayerItem : public CCommandItem {
 ...
 public:
  CPlayerItem(CDialogMenu *pParent, const char *pTitle, int64_t nId,
...
class CBrowserItem : public CCommandItem {
 ...
 public:
  CBrowserItem(CDialogMenu *pParent, const char *pHost, const char *pPlayers,
               const char *pScenario, const char *pPing);
...
class CAchievementItem : public CMenuItem {
 ...
 public:
  CAchievementItem(CDialogMenu *pParent, const wchar_t *pName,
                   const wchar_t *pDesc, uint32_t points, bool bUnlocked,
 ...
 private:
  ...
  CPanelAnimationVar(Color, m_clrProgressBar, "ProgressBarColor",
                     "140 140 140 255");
};
...
class CDialogMenu : public vgui::Panel {
 ...
 public:
  ...
  void SetMaxVisibleItems(uint32_t nMaxVisibleItems);
  void SetParent(CBaseDialog *pParent);

  // Menu items
  CCommandItem *AddCommandItem(const char *pTitleLabel, const char *pDescLabel,
                               const char *pCommand);
  CPlayerItem *AddPlayerItem(const char *pTitleLabel, int64_t nId,
                             uint8_t bVoice, bool bReady);
  CBrowserItem *AddBrowserItem(const char *pHost, const char *pPlayers,
                               const char *pScenario, const char *pPing);
  COptionsItem *AddOptionsItem(const char *pLabel);
  CSectionedItem *AddSectionedItem(const char **ppEntries, int ct);
  CAchievementItem *AddAchievementItem(const wchar_t *pName,
                                       const wchar_t *pDesc, uint32_t cred,
                                       bool bUnlocked,
                                       IAchievement *pSourceAchievement);
 ...
 private:
  ...
  char m_szFilter[MAX_COMMAND_LEN];  // string to use as a keyvalues filter when
                                     // reading in menu items

  int m_nItemSpacing;  // gap between menu items
  int m_nMinWide;      // minimum width - final menu width will always be >=
                       // m_nMinWide
