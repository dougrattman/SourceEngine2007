...
// CSessionLobbyDialog
//--------------------------------
CSessionLobbyDialog::CSessionLobbyDialog(vgui::Panel *pParent)
    : BaseClass(pParent, "SessionLobbyDialog") {
  ...
  m_pTeamInfos[BLUE_TEAM_LOBBY] =
      new CScenarioInfoPanel(this, "BlueTeamDescription");
  m_pTeamInfos[RED_TEAM_LOBBY] =
      new CScenarioInfoPanel(this, "RedTeamDescription");
...
void CSessionLobbyDialog::PerformLayout() {
  ...
  if (pName) {
    ...
    if (pScenario) {
      pDiskName =
          pName->GetString(pScenario->GetString("displaystring"), "unknown");
    }
  }
  ...
  if (pGoals) {
    ...
    if (pTeam) {
      m_pTeamInfos[BLUE_TEAM_LOBBY]->m_pDescOne->SetText(
          pTeam->GetString(pDiskName, ""));
    }
    pTeam = pGoals->FindKey("Red");
    if (pTeam) {
      m_pTeamInfos[RED_TEAM_LOBBY]->m_pDescOne->SetText(
          pTeam->GetString(pDiskName, ""));
    }
  }
  ...
  CMatchmakingBasePanel *pBase =
      dynamic_cast<CMatchmakingBasePanel *>(m_pParent);
  if (pBase) {
    pBase->SetFooterButtonVisible(
        "#GameUI_PlayerReview",
        pBase->GetGameType() != GAMETYPE_SYSTEMLINK_MATCH);
    ...
    if (m_pHostOptionsPanel) {
      ...
      vgui::Label *pSettingsLabel =
          (vgui::Label *)m_pHostOptionsPanel->FindChildByName(
              "ChangeSettingsButton", true);
      if (pSettingsLabel) {
        pSettingsLabel->SetVisible(bVisible);
      }
      pSettingsLabel = (vgui::Label *)m_pHostOptionsPanel->FindChildByName(
          "ChangeSettingsText", true);
      if (pSettingsLabel) {
        pSettingsLabel->SetVisible(bVisible);
      }
    }
...
void CSessionLobbyDialog::ApplySettings(KeyValues *pResourceData) {
  ...
  Q_strncpy(m_szCommand, pResourceData->GetString("commandstring", "NULL"),
            sizeof(m_szCommand));
}
...
void CSessionLobbyDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  Color cLabelColor =
      pScheme->GetColor("MatchmakingDialogTitleColor", Color(0, 0, 0, 255));

  m_pLobbyStateLabel->SetFgColor(cLabelColor);
  m_pHostLabel->SetFgColor(cLabelColor);

  m_pLobbyStateBg->SetBgColor(
      pScheme->GetColor("TanDarker", Color(0, 0, 0, 255)));
  m_pLobbyStateBg->SetPaintBackgroundType(2);

  m_pHostOptionsPanel->SetBgColor(
      pScheme->GetColor("TanDarker", Color(0, 0, 0, 255)));
  m_pHostOptionsPanel->SetPaintBackgroundType(2);

  m_pScenarioInfo->SetBgColor(
      pScheme->GetColor("TanDarker", Color(0, 0, 0, 255)));
  m_pTeamInfos[BLUE_TEAM_LOBBY]->SetBgColor(
      pScheme->GetColor("HudBlueTeam", Color(0, 0, 0, 255)));
  m_pTeamInfos[RED_TEAM_LOBBY]->SetBgColor(
      pScheme->GetColor("HudRedTeam", Color(0, 0, 0, 255)));
  ...
  // Lets set all the labels this panel owns to be the right fgcolor. hooray
  // vgui!
  int iChildren = m_pHostOptionsPanel->GetChildCount();

  for (int i = 0; i < iChildren; i++) {
    vgui::Label *pLabel =
        dynamic_cast<vgui::Label *>(m_pHostOptionsPanel->GetChild(i));
    if (pLabel) {
      SETUP_PANEL(pLabel);
  ...
  vgui::Label *pPlayerReviewLabel =
      (vgui::Label *)FindChildByName("PlayerReviewLabel");
...
void CSessionLobbyDialog::ActivateNextMenu() {
  ...
  } while (m_Menus[m_iActiveMenu].GetItemCount() == 0 &&
           m_iActiveMenu != startIndex);
...
void CSessionLobbyDialog::UpdatePlayerInfo(uint64_t nPlayerId,
                                           const char *pName, int iTeam,
                                           uint8_t cVoiceState,
    ...
    if (iFoundTeam == m_iActiveMenu) {
      ...
      if (iActive == iFoundItem) {
        ...
        if (iTeam >= 0) {
          ...
          m_iActiveMenu = iTeam;
          m_Menus[m_iActiveMenu].SetFocus(
              m_Menus[m_iActiveMenu].GetItemCount() - 1);
        } else {
  ...
  switch (code) {
    ...
    case KEY_XBUTTON_Y:
      if (m_bHostLobby) {
        ...
        CMatchmakingBasePanel *pBase =
            dynamic_cast<CMatchmakingBasePanel *>(m_pParent);
        if (pBase && pBase->GetGameType() == GAMETYPE_RANKED_MATCH) break;
...
void CSessionLobbyDialog::SetStartGame(bool bStartGame) {
  ...
  if (m_bStartingGame != bStartGame) {
    ...
    // If we can start the game but haven't started yet, show the "Start Game"
    // label
    bool bShowStartGame = bCanStart && !m_bStartingGame;

    // show/hide the "start game" and countdown hint label based on state
    vgui::Label *pStartGame =
        (vgui::Label *)m_pHostOptionsPanel->FindChildByName("StartGameText");
    if (pStartGame) {
      pStartGame->SetVisible(m_bStartingGame == false);
    }

    vgui::Label *pCancelCountdown =
        (vgui::Label *)m_pHostOptionsPanel->FindChildByName("CancelGameText");
    if (pCancelCountdown) {
...
void CSessionLobbyDialog::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "ReturnToMainMenu")) {
    ...
    CDialogMenu *pMenu = &m_Menus[m_iActiveMenu];
    CPlayerItem *pItem =
        (CPlayerItem *)pMenu->GetItem(pMenu->GetActiveItemIndex());
    if (pItem) {
...
CON_COMMAND(mm_add_player, "Add a player") {
  if (args.ArgC() >= 5) {
    ...
    uint8_t cVoiceState = atoi(args[4]) != 0;
    int nPlayersNeeded = atoi(args[5]);
    g_pLobbyDialog->UpdatePlayerInfo(id, pName, team, cVoiceState,
                                     nPlayersNeeded, false);
  } else if (args.ArgC() >= 1) {
    char name[32];
    int team = pnum & 0x1;
    int id = pnum++;
