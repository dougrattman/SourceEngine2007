...
//---------------------------------------------------------------------
CSessionOptionsDialog::CSessionOptionsDialog(vgui::Panel *pParent)
    : BaseClass(pParent, "SessionOptions") {
...
void CSessionOptionsDialog::ApplyCommonProperties(KeyValues *pKeys) {
  for (KeyValues *pProperty = pKeys->GetFirstSubKey(); pProperty != NULL;
       pProperty = pProperty->GetNextKey()) {
    ...
    if (!Q_stricmp(pName, "SessionContext")) {
      ...
      Q_strncpy(ctx.szValue, pProperty->GetString("value", "NULL"),
                sizeof(ctx.szValue));
      ...
      Q_strncpy(prop.szID, pProperty->GetString("id", "NULL"),
                sizeof(prop.szID));
      Q_strncpy(prop.szValue, pProperty->GetString("value", "NULL"),
                sizeof(prop.szValue));
      Q_strncpy(prop.szValueType, pProperty->GetString("valuetype", "NULL"),
...
void CSessionOptionsDialog::ApplySettings(KeyValues *pResourceData) {
  ...
  if (pSettings) {
    Q_strncpy(m_szCommand, pSettings->GetString("commandstring", "NULL"),
              sizeof(m_szCommand));
  ...
  if (pScenarios) {
    for (KeyValues *pScenario = pScenarios->GetFirstSubKey(); pScenario != NULL;
         pScenario = pScenario->GetNextKey()) {
      CScenarioInfoPanel *pScenarioInfo =
          new CScenarioInfoPanel(this, "ScenarioInfoPanel");
...
void CSessionOptionsDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  for (int i = 0; i < m_pScenarioInfos.Count(); ++i) {
    m_pScenarioInfos[i]->SetBgColor(
        pScheme->GetColor("TanDark", Color(0, 0, 0, 255)));
  }

  m_pRecommendedLabel =
      dynamic_cast<vgui::Label *>(FindChildByName("RecommendedLabel"));
}
...
void CSessionOptionsDialog::OverrideMenuItem(KeyValues *pItemKeys) {
  ...
  //
  if (!m_bModifySession && (!Q_stricmp(m_szGametype, "hoststandard") ||
                            !Q_stricmp(m_szGametype, "hostranked"))) {
    if (!Q_stricmp(pItemKeys->GetName(), "OptionsItem")) {
      const char *pID = pItemKeys->GetString("id", "NULL");
