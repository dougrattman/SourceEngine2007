...
//
// Purpose: Present a list of sessions from which the player can choose a game
// to join.
...
//--------------------------------
CSessionBrowserDialog::CSessionBrowserDialog(vgui::Panel *pParent,
...
void CSessionBrowserDialog::SessionSearchResult(int searchIdx, void *pHostData,
  ...
  int nScenarioId = 0;
  uint32_t nContextId = m_pDialogKeys->GetInt("scenario", -1);
  for (uint32_t i = 0; i < pResult->cContexts; ++i) {
  ...
  int filledSlots =
      pResult->dwFilledPublicSlots + pResult->dwFilledPrivateSlots;
  int totalSlots =
      filledSlots + pResult->dwOpenPublicSlots + pResult->dwOpenPrivateSlots;
...
void CSessionBrowserDialog::UpdateScenarioDisplay(void) {
  if (!m_ScenarioIndices.Count()) return;
