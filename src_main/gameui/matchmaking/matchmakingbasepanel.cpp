...
//--------------------------------
CMatchmakingBasePanel::CMatchmakingBasePanel(vgui::Panel *pParent)
    : BaseClass(pParent, "MatchmakingBasePanel") {
...
void CMatchmakingBasePanel::SetFooterButtons(CBaseDialog *pOwner,
  ...
  if (m_pFooter) {
    ...
    if (pKeyValues) {
      for (KeyValues *pButton = pKeyValues->GetFirstSubKey(); pButton != NULL;
...
void CMatchmakingBasePanel::SetFooterButtonVisible(const char *pszText,
                                                   bool bVisible) {
  if (m_pFooter) {
...
void CMatchmakingBasePanel::OnCommand(const char *pCommand) {
  ...
  if (!Q_stricmp("OpenRankingsDialog", pCommand)) {
    ...
    matchmaking->AddSessionProperty(SESSION_FLAG, "SESSION_CREATE_SYSTEMLINK",
                                    NULL, NULL);
    ...
    if (Q_stristr(pCommand, "_Ranked")) {
      ...
      matchmaking->AddSessionProperty(SESSION_CONTEXT, "CONTEXT_GAME_MODE",
                                      "CONTEXT_GAME_MODE_MULTIPLAYER", NULL);
      matchmaking->AddSessionProperty(SESSION_CONTEXT, "CONTEXT_GAME_TYPE",
                                      "CONTEXT_GAME_TYPE_RANKED", NULL);
      matchmaking->AddSessionProperty(
          SESSION_FLAG, "SESSION_CREATE_LIVE_MULTIPLAYER_RANKED", NULL, NULL);
      ...
      matchmaking->AddSessionProperty(SESSION_CONTEXT, "CONTEXT_GAME_MODE",
                                      "CONTEXT_GAME_MODE_MULTIPLAYER", NULL);
      matchmaking->AddSessionProperty(SESSION_CONTEXT, "CONTEXT_GAME_TYPE",
                                      "CONTEXT_GAME_TYPE_STANDARD", NULL);
      matchmaking->AddSessionProperty(
          SESSION_FLAG, "SESSION_CREATE_LIVE_MULTIPLAYER_STANDARD", NULL, NULL);
      m_nGameType = GAMETYPE_STANDARD_MATCH;
    }
    ...
    // This covers six command strings: *_Host[Standard|Ranked|Systemlink],
    // *_Client[Standard|Ranked|Systemlink] Each command has a unique options
    // menu - the command string is used as the name of the .res file.
    OnOpenSessionOptionsDialog(pCommand);
    ...
    // Need to close the client options dialog and open the host options
    // equivalent
    PopDialog();
...
//-----------------------------------------------------------------------------
void CMatchmakingBasePanel::SessionNotification(const int notification,
                                                const int param) {
  switch (notification) {
    ...
    case SESSION_NOTIFY_COUNTDOWN: {
      CSessionLobbyDialog *pDlg =
          (CSessionLobbyDialog *)m_hSessionLobbyDialog.Get();
      if (pDlg) {
        pDlg->UpdateCountdown(param);
      }

      if (param == 0) {
        BasePanel()->RunAnimationWithCallback(this, "CloseMatchmakingUI",
...
//-----------------------------------------------------------------------------
// Purpose: Check whether a player meets the signin requirements for a
// multiplayer game
//-----------------------------------------------------------------------------
bool CMatchmakingBasePanel::ValidateSigninAndStorage(
...
//-----------------------------------------------------------------------------
void CMatchmakingBasePanel::UpdatePlayerInfo(uint64_t nPlayerId,
                                             const char *pName, int nTeam,
                                             uint8_t cVoiceState,
                                             int nPlayersNeeded, bool bHost) {
  CSessionLobbyDialog *pLobby =
      dynamic_cast<CSessionLobbyDialog *>(m_hSessionLobbyDialog.Get());
  if (pLobby) {
    pLobby->UpdatePlayerInfo(nPlayerId, pName, nTeam, cVoiceState,
...
//-----------------------------------------------------------------------------
void CMatchmakingBasePanel::SessionSearchResult(int searchIdx, void *pHostData,
                                                XSESSION_SEARCHRESULT *pResult,
                                                int ping) {
  CSessionBrowserDialog *pBrowser =
      dynamic_cast<CSessionBrowserDialog *>(m_hSessionBrowserDialog.Get());
  if (pBrowser) {
...
//-----------------------------------------------------------------------------
// Purpose: Close the current dialog, pop it from the top of the stack, and
...
void CMatchmakingBasePanel::LoadSessionProperties() {
  ...
  m_pSessionKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("PropertyDisplayKeys");
  if (m_pSessionKeys) {
...
void CMatchmakingBasePanel::OnOpenRankingsDialog() {
  if (!ValidateSigninAndStorage(true, "OpenRankingDialog")) return;
...
void CMatchmakingBasePanel::OnOpenSystemLinkDialog() {
  if (!ValidateSigninAndStorage(false, "OpenSystemLinkDialog")) return;
...
void CMatchmakingBasePanel::OnOpenPlayerMatchDialog() {
  if (!ValidateSigninAndStorage(true, "OpenPlayerMatchDialog")) return;
...
void CMatchmakingBasePanel::OnOpenRankedMatchDialog() {
  if (!ValidateSigninAndStorage(true, "OpenRankedMatchDialog")) return;
...
void CMatchmakingBasePanel::OnOpenAchievementsDialog() {
  if (!ValidateSigninAndStorage(false, "OpenAchievementsDialog")) return;
...
void CMatchmakingBasePanel::OnOpenSessionOptionsDialog(
  ...
  CSessionOptionsDialog *pDlg =
      ((CSessionOptionsDialog *)m_hSessionOptionsDialog.Get());
...
void CMatchmakingBasePanel::OnOpenSessionLobbyDialog(
    const char *pResourceName) {
...
void CMatchmakingBasePanel::OnOpenSessionBrowserDialog(
    const char *pResourceName) {
