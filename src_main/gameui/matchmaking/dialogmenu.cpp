...
//-----------------------------------------------------------------------
CMenuItem::CMenuItem(CDialogMenu *pParent, const char *pTitle,
...
//-----------------------------------------------------------------------
void CMenuItem::PerformLayout() { BaseClass::PerformLayout(); }
...
void CMenuItem::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_BgColor = pScheme->GetColor("MatchmakingMenuItemBackground",
                                Color(46, 43, 42, 255));
  m_BgColorActive = pScheme->GetColor("MatchmakingMenuItemBackgroundActive",
                                      Color(150, 71, 0, 255));

  m_pTitle->SetFgColor(
      pScheme->GetColor("MatchmakingMenuItemTitleColor", Color(0, 0, 0, 255)));

  if (m_pDescription) {
    m_pDescription->SetFgColor(pScheme->GetColor(
        "MatchmakingMenuItemDescriptionColor", Color(0, 0, 0, 255)));
  }

  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("MenuItem.res");
  ApplySettings(pKeys);
}
...
// Set an item as having input focus
//-----------------------------------------------------------------------
bool CMenuItem::IsEnabled() { return m_bEnabled; }
...
//-----------------------------------------------------------------------
CCommandItem::CCommandItem(CDialogMenu *pParent, const char *pTitleLabel,
                           const char *pDescLabel, const char *pCommand)
...
//-----------------------------------------------------------------------
CPlayerItem::CPlayerItem(CDialogMenu *pParent, const char *pTitleLabel,
                         int64_t nId, uint8_t bVoice, bool bReady)
...
void CPlayerItem::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("PlayerItem.res");
  ApplySettings(pKeys);
}

void CPlayerItem::OnClick() { BaseClass::OnClick(); }
...
//-----------------------------------------------------------------------
CBrowserItem::CBrowserItem(CDialogMenu *pParent, const char *pHost,
                           const char *pPlayers, const char *pScenario,
                           const char *pPing)
...
void CBrowserItem::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  Color fgcolor = pScheme->GetColor("MatchmakingMenuItemDescriptionColor",
                                    Color(64, 64, 64, 255));
  ...
  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("BrowserItem.res");
  ApplySettings(pKeys);
...
void COptionsItem::PerformLayout() {
  ...
  m_pLeftArrow->SetPos(m_nOptionsXPos - m_nArrowGap - m_pLeftArrow->GetWide(),
                       ly);
...
void COptionsItem::ApplySettings(KeyValues *pSettings) {
  ...
  Q_strncpy(m_szOptionsFont, pSettings->GetString("optionsfont", "Default"),
            sizeof(m_szOptionsFont));
}
...
void COptionsItem::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pTitle->SetFgColor(pScheme->GetColor("MatchmakingMenuItemTitleColor",
                                         Color(200, 184, 151, 255)));

  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("OptionsItem.res");
  ApplySettings(pKeys);
...
void COptionsItem::AddOption(const char *pLabelText,
                             const sessionProperty_t &option) {
  // Add a new option to this item's list of options
  m_Options.AddToTail(option);

  int idx = m_OptionLabels.AddToTail(
      new vgui::Label(this, "Option Value", pLabelText));
  ...
  if (Q_stristr(pLabelText, "Fmt")) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(option.szValue, wzNumber,
                                          sizeof(wzNumber));
    g_pVGuiLocalize->ConstructString(wszString, sizeof(wszString), wzFmt, 1,
                                     wzNumber);
  ...
  pOption->SetFgColor(m_pTitle ? m_pTitle->GetFgColor()
                               : Color(200, 184, 151, 255));
...
//-----------------------------------------------------------------------
int COptionsItem::GetActiveOptionIndex() { return m_nActiveOption; }
...
//-----------------------------------------------------------------------
CAchievementItem::CAchievementItem(CDialogMenu *pParent, const wchar_t *pName,
                                   const wchar_t *pDesc, uint32_t points,
                                   bool bUnlocked,
                                   IAchievement *pSourceAchievement)
    : BaseClass(pParent, "", "") {
  ...
  m_pPercentageBarBackground =
      SETUP_PANEL(new vgui::ImagePanel(this, "PercentageBarBackground"));
...
void CAchievementItem::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("AchievementItem.res");
  ApplySettings(pKeys);
  ...
  m_pPoints->SetFgColor(pScheme->GetColor("MatchmakingMenuItemDescriptionColor",
                                          Color(64, 64, 64, 255)));
...
//-----------------------------------------------------------------------
CSectionedItem::CSectionedItem(CDialogMenu *pParent, const char **ppEntries,
                               int ct)
...
CSectionedItem::~CSectionedItem() { ClearSections(); }
...
void CSectionedItem::PerformLayout() {
  ...
  for (int i = 0; i < m_Sections.Count(); ++i) {
    ...
    pLabel->SetBounds(m_pParent->GetColumnXPos(i), 0,
                      m_pParent->GetColumnWide(i), tall);
    pLabel->SetTextInset(
        10,
        m_bHeader
            ? 5
            : m_pParent->GetColumnYPos(
                  i));  // only use ypos for the y-inset if we're not a header
  }
}
...
void CSectionedItem::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  KeyValues *pKeys =
      BasePanel()->GetConsoleControlSettings()->FindKey("SectionedItem.res");
  ApplySettings(pKeys);
...
//--------------------------------------------------------------------------------------
// Generic menu for Xbox 360 matchmaking dialogs. Contains a list of CMenuItems
// arranged vertically. The user can navigate the list using the controller and
// click on any item. A clicked item may send a command to the dialog and the
// dialog responds accordingly.
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
CCommandItem *CDialogMenu::AddCommandItem(const char *pTitleLabel,
                                          const char *pDescLabel,
                                          const char *pCommand) {
  return (CCommandItem *)AddItemInternal(
      new CCommandItem(this, pTitleLabel, pDescLabel, pCommand));
}

CBrowserItem *CDialogMenu::AddBrowserItem(const char *pHost,
                                          const char *pPlayers,
                                          const char *pScenario,
                                          const char *pPing) {
  // Results are added to the menu at runtime, so the layout needs to be updated
  // after each addition.
  CBrowserItem *pItem = (CBrowserItem *)AddItemInternal(
      new CBrowserItem(this, pHost, pPlayers, pScenario, pPing));
  PerformLayout();
  return pItem;
}
...
CAchievementItem *CDialogMenu::AddAchievementItem(
    const wchar_t *pName, const wchar_t *pDesc, uint32_t points, bool bUnlocked,
    IAchievement *pSourceAchievement) {
  return (CAchievementItem *)AddItemInternal(new CAchievementItem(
      this, pName, pDesc, points, bUnlocked, pSourceAchievement));
}

CSectionedItem *CDialogMenu::AddSectionedItem(const char **ppEntries, int ct) {
  CSectionedItem *pItem = (CSectionedItem *)AddItemInternal(
      new CSectionedItem(this, ppEntries, ct));
  PerformLayout();
  return pItem;
}

CPlayerItem *CDialogMenu::AddPlayerItem(const char *pTitleLabel, int64_t nId,
                                        uint8_t bVoice, bool bReady) {
  // Players are added to the lobby at runtime, so the layout needs to be
  // updated after each addition.
  CPlayerItem *pItem = (CPlayerItem *)AddItemInternal(
      new CPlayerItem(this, pTitleLabel, nId, bVoice, bReady));
  PerformLayout();
  return pItem;
}
...
void CDialogMenu::ApplySettings(KeyValues *pResourceData) {
  ...
  m_nMaxVisibleItems =
      pResourceData->GetInt("maxvisibleitems", 1000);  // arbitrarily large
  ...
  if (pColumnData) {
    ...
    for (KeyValues *pColumn = pColumnData->GetFirstSubKey(); pColumn != NULL;
         pColumn = pColumn->GetNextKey()) {
      if (!Q_stricmp(pColumn->GetName(), "Column")) {
        ...
        col.color = m_pScheme->GetColor(pColumn->GetString("fgcolor"),
                                        Color(0, 0, 0, 255));
    ...
    vgui::HFont headerFont =
        m_pScheme->GetFont(pColumnData->GetString("headerfont", "default"));
    Color headerColor = m_pScheme->GetColor(
        pColumnData->GetString("headerfgcolor"), Color(0, 0, 0, 255));
    for (int i = 0; i < idx; ++i) {
  ...
  for (KeyValues *pMenuData = pResourceData->GetFirstSubKey();
       pMenuData != NULL; pMenuData = pMenuData->GetNextKey()) {
    // See if we should skip over this block
    if (m_bUseFilter) {
      if (pMenuData->GetInt(m_szFilter, 0) == 0) continue;
    }
    ...
    if (!Q_stricmp(pMenuData->GetName(), "CommandItem")) {
      ...
      // New Options Item
      COptionsItem *pItem =
          AddOptionsItem(pMenuData->GetString("label", "<unknown>"));
      ...
      // Add all the options
      for (KeyValues *pValue = pMenuData->GetFirstSubKey(); pValue != NULL;
        ...
        if (!Q_stricmp(pValue->GetName(), "Option")) {
          ...
          Q_strncpy(prop.szValue, pValue->GetString("value", "NULL"),
                    sizeof(prop.szValue));
      ...
      KeyValues *kv =
          new KeyValues("MenuItemChanged", "item", GetItemCount() - 1);
      PostActionSignal(kv);
    }
  }
...
void CDialogMenu::SortMenuItems() {
  ...
  for (int i = 1; i <= m_MenuItems.Count(); ++i) {
    for (int j = 0; j < m_MenuItems.Count() - i; ++j) {
      ((CSectionedItem *)m_MenuItems[j])
          ->m_Sections[m_nActiveColumn]
          .pLabel->GetText(szBufferOne, sizeof(szBufferOne));
      ((CSectionedItem *)m_MenuItems[j + 1])
          ->m_Sections[m_nActiveColumn]
          .pLabel->GetText(szBufferTwo, sizeof(szBufferTwo));
...
void CDialogMenu::SetFocusPrev() {
  if (m_MenuItems.Count()) {
    ...
    if (iNewIndex < 0) iNewIndex = m_MenuItems.Count() - 1;
...
//--------------------------------------------------------------------------------------
// For OptionsItems: Move focus to the next option in the menu item - does not
// wrap
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
// For OptionsItems: Move focus to the previous option in the menu item - does
// not wrap
//--------------------------------------------------------------------------------------
...
void CDialogMenu::UpdateBaseColumnIndex() {
  if (m_iUnlocked + m_nActiveColumn - m_nBaseColumnIdx >=
      m_nMaxVisibleColumns) {
...
void CDialogMenu::SetColumnFocusNext() {
  if (m_nActiveColumn == -1) return;

  if (m_Columns.Count()) {
    int iNewColumn = m_nActiveColumn + 1;
    if (iNewColumn >= m_Columns.Count()) return;
...
void CDialogMenu::SetColumnFocusPrev() {
  ...
  if (m_Columns.Count()) {
    int iNewColumn = m_nActiveColumn - 1;
    if (iNewColumn < 0 || m_Columns[iNewColumn].bLocked) return;
...
//-----------------------------------------------------------------------
int CDialogMenu::GetActiveItemIndex() { return m_nActive; }

//-----------------------------------------------------------------------
// Return the index of the current active menu column
//-----------------------------------------------------------------------
int CDialogMenu::GetActiveColumnIndex() { return m_nActiveColumn; }

//-----------------------------------------------------------------------
// Return the number of menu items
//-----------------------------------------------------------------------
int CDialogMenu::GetItemCount() { return m_MenuItems.Count(); }
...
//-----------------------------------------------------------------------
int CDialogMenu::GetVisibleColumnCount() { return m_nMaxVisibleColumns; }

//-----------------------------------------------------------------------
// Return the index of the first unlocked column
//-----------------------------------------------------------------------
int CDialogMenu::GetFirstUnlockedColumnIndex() { return m_iUnlocked; }

//-----------------------------------------------------------------------
// Return the first visible index in the menu
//-----------------------------------------------------------------------
int CDialogMenu::GetBaseRowIndex() { return m_nBaseRowIdx; }

//-----------------------------------------------------------------------
// Set the first visible index in the menu
//-----------------------------------------------------------------------
void CDialogMenu::SetBaseRowIndex(int idx) { m_nBaseRowIdx = idx; }
...
//-----------------------------------------------------------------------
int CDialogMenu::GetColumnYPos(int idx) { return m_Columns[idx].ypos; }

//-----------------------------------------------------------------------
// Return the specified column width
//-----------------------------------------------------------------------
int CDialogMenu::GetColumnWide(int idx) { return m_Columns[idx].wide; }

//-----------------------------------------------------------------------
// Return the specified column alignment
//-----------------------------------------------------------------------
int CDialogMenu::GetColumnAlignment(int idx) { return m_Columns[idx].align; }

//-----------------------------------------------------------------------
// Return the specified column font
//-----------------------------------------------------------------------
HFont CDialogMenu::GetColumnFont(int idx) { return m_Columns[idx].hFont; }

//-----------------------------------------------------------------------
// Return the specified column fgcolor
//-----------------------------------------------------------------------
Color CDialogMenu::GetColumnColor(int idx) { return m_Columns[idx].color; }
...
//--------------------------------------------------------------------------------------
// Receive the command from a clicked menu item and forwards it to the parent
// dialog
//--------------------------------------------------------------------------------------
