...
// CLeaderboardDialog
//----------------------------------------------------------
CLeaderboardDialog::CLeaderboardDialog(vgui::Panel *pParent)
    : BaseClass(pParent, "LeaderboardDialog") {
...
void CLeaderboardDialog::PerformLayout(void) {
  ...
  if (m_cColumns) {
    ...
    m_pProgressBar->SetBounds(x + columnWide * m_iRangeBase, y,
                              columnWide * visibleColumns, tall);
  } else {
...
void CLeaderboardDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pBestMoments->SetFgColor(pScheme->GetColor(
      "MatchmakingMenuItemDescriptionColor", Color(64, 64, 64, 255)));
}
...
bool CLeaderboardDialog::GetPlayerStats(int rank, bool bFriends) {
  ...
  if (!bRanked) {
    ...
    spec.rgwColumnIds[8] =
        STATS_COLUMN_PLAYER_MAX_UNRANKED_HEALTH_POINTS_HEALED;
    ...
    spec.rgwColumnIds[12] =
        STATS_COLUMN_PLAYER_MAX_UNRANKED_HEALTH_POINTS_LEACHED;
  ...
  if (!bFriends) {
    if (rank == -1) {
      ...
      ret = XUserCreateStatsEnumeratorByXuid(0, xuid, NUM_ROWS_PER_QUERY, 1,
                                             &spec, &cbResults, &handle);
    } else {
      // Start at the requested rank
      ret = XUserCreateStatsEnumeratorByRank(0, rank, NUM_ROWS_PER_QUERY, 1,
                                             &spec, &cbResults, &handle);
    }
    ...
    for (uint32_t i = 0; i < cpReturned; ++i) {
    ...
    ret = XUserReadStats(0, xuidCount, xuidFriends, 1, &spec, &cbResults,
                         m_pStats, NULL);
  }
  ...
  if (ret == ERROR_SUCCESS) {
    ...
    for (uint32_t i = 0; i < m_pStats->pViews[0].dwNumRows; ++i) {
      ...
      for (uint32_t j = 0; j < row.dwNumColumns; ++j) {
...
void CLeaderboardDialog::UpdateLeaderboard(int iNewRank) {
  ...
  // Set these "new" values to the current values - they will be conditionally
  // updated.
  ...
  if (iNewActiveItemIndex < iNewBaseItemIndex) {
    ...
    // Since we shifted the menu down, both base and active item are at the
