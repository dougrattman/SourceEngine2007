...
static int __cdecl ChapterSortFunc(const void *elem1, const void *elem2) {
  ...
  // compare length second (longer string show up later in the list, eg.
  // chapter9 before chapter9a)
...
class CSelectionOverlayPanel : public vgui::Panel {
 ...
 public:
  ...
  virtual void OnMouseDoublePressed(vgui::MouseCode code) {
    ...
    OnMousePressed(code);
    if (GetParent()->IsEnabled()) {
      PostMessage(m_pSelectionTarget,
                  new KeyValues("Command", "command", "play"));
    }
  }
};

//-----------------------------------------------------------------------------
// Purpose: selectable item with screenshot for an individual chapter in the
// dialog
//-----------------------------------------------------------------------------
class CGameChapterPanel : public vgui::EditablePanel {
 ...
 public:
  CGameChapterPanel(CNewGameDialog *parent, const char *name,
                    const char *chapterName, int chapterIndex,
                    const char *chapterNumber, const char *chapterConfigFile,
                    bool bCommentary)
    ...
    if (ModInfo().IsSinglePlayerOnly()) {
      ...
      m_pChapterNameLabel =
          new Label(this, "ChapterNameLabel", "#GameUI_LoadCommentary");
    }
    ...
    CSelectionOverlayPanel *overlay =
        new CSelectionOverlayPanel(this, parent, chapterIndex);
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    m_DisabledColor =
        pScheme->GetColor("NewGame.DisabledColor", Color(255, 255, 255, 255));
    m_SelectedColor =
    ...
    m_pCommentaryIcon =
        dynamic_cast<ImagePanel *>(FindChildByName("CommentaryIcon"));
    if (m_pCommentaryIcon) m_pCommentaryIcon->SetVisible(m_bCommentaryMode);
  }
  ...
  const char *GetConfigFile() { return m_szConfigFile; }

  const char *GetChapter() { return m_szChapter; }

  bool IsTeaserChapter() { return m_bTeaserChapter; }

  bool HasBonus() { return m_bHasBonus; }
...
//-----------------------------------------------------------------------------
CNewGameDialog::CNewGameDialog(vgui::Panel *parent, bool bCommentaryMode)
  ...
  if (GameUI().IsConsoleUI()) {
    ...
    m_pChapterTitleLabels[0] =
        SETUP_PANEL(new Label(this, "ChapterTitleLabel", ""));
    m_pChapterTitleLabels[0]->SetVisible(true);
    m_pChapterTitleLabels[0]->SetFgColor(Color(255, 255, 255, 255));

    m_pChapterTitleLabels[1] =
        SETUP_PANEL(new Label(this, "ChapterTitleLabel2", ""));
    ...
    m_pBonusSelection = SETUP_PANEL(
        new Label(this, "BonusSelectionLabel", "#GameUI_BonusMapsStandard"));
    m_pBonusSelectionBorder =
        SETUP_PANEL(new ImagePanel(this, "BonusSelectionBorder"));
  ...
  for (int i = 0; i < chapterIndex; i++) {
    ...
    char chapterName[64];
    Q_snprintf(chapterName, sizeof(chapterName), "#%s_Chapter%s_Title",
               pGameDir, chapterID);
    ...
    if (GameUI().IsConsoleUI()) {
      if (bBonusesUnlocked) {
        ...
        for (int iBonusMap = 0; iBonusMap < BonusMapsDatabase()->BonusCount();
             ++iBonusMap) {
          BonusMapDescription_t *pMap =
              BonusMapsDatabase()->GetBonusData(iBonusMap);
          if (Q_stricmp(pMap->szChapterName, szFullFileName) == 0 &&
              !pMap->bLocked) {
  ...
  m_pCenterBg->SetPos(
      m_PanelXPos[2] - 8,
      m_PanelYPos[2] - (m_pCenterBg->GetTall() - panelHeight) + 8);
...
void CNewGameDialog::Activate(void) {
  ...
  // Commentary stuff is set up on activate because in XBox the new game menu is
  // never deleted
  SetTitle(
      ((m_bCommentaryMode) ? ("#GameUI_LoadCommentary") : ("#GameUI_NewGame")),
      true);
...
void CNewGameDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pCommentaryLabel =
      dynamic_cast<vgui::Label *>(FindChildByName("CommentaryUnlock"));
  if (m_pCommentaryLabel) m_pCommentaryLabel->SetVisible(m_bCommentaryMode);

  if (GameUI().IsConsoleUI()) {
    if (!m_bCommentaryMode && BonusMapsDatabase()->BonusesUnlocked() &&
        !m_ChapterPanels[m_PanelIndex[SLOT_CENTER]]->HasBonus()) {
...
void CNewGameDialog::UpdateMenuComponents(EScrollDirection dir) {
  ...
  if (GameUI().IsConsoleUI()) {
    ...
    if (bHasBonus) {
      ...
      for (int iBonus = 0; iBonus < BonusMapsDatabase()->BonusCount();
           ++iBonus) {
        m_pBonusMapDescription = BonusMapsDatabase()->GetBonusData(iBonus);
        if (Q_stricmp(
                m_pBonusMapDescription->szChapterName,
                m_ChapterPanels[m_PanelIndex[centerIdx]]->GetConfigFile()) == 0)
          break;
      }
    } else {
    ...
    if (upArrow) upArrow->SetVisible(bHasBonus);
    if (downArrow) downArrow->SetVisible(bHasBonus);
  ...
  if (!GameUI().IsConsoleUI()) {
    ...
    if (m_ChapterPanels.Count() < 4)  // if there are less than 4 chapters show
                                      // the next button but disabled
    {
      m_pNextButton->SetVisible(true);
      m_pNextButton->SetEnabled(false);
    } else if (m_PanelIndex[rightIdx] == INVALID_INDEX ||
               m_PanelIndex[rightIdx] == m_ChapterPanels.Count() - 1) {
      m_pNextButton->SetVisible(false);
      m_pNextButton->SetEnabled(false);
    } else {
      m_pNextButton->SetVisible(true);
...
void CNewGameDialog::UpdateBonusSelection(void) {
  ...
  if (m_iBonusSelection == 0) {
    ...
    if (m_pBonusMapDescription) {
      Q_snprintf(szMapAdvancedName, sizeof(szMapAdvancedName), "%s_advanced",
    ...
    for (int iBonus = 0; iBonus < BonusMapsDatabase()->BonusCount(); ++iBonus) {
      ...
      if (Q_stricmp(szMapAdvancedName, pAdvancedDescription->szMapFileName) ==
          0)
        break;
    }
    ...
    ChallengeDescription_t *pChallengeDescription =
        &((*m_pBonusMapDescription->m_pChallenges)[iChallenge]);
...
void CNewGameDialog::SetSelectedChapterIndex(int index) {
  ...
  currIdx = index + 1;
  for (int i = selectedSlot + 1;
       i < NUM_SLOTS && currIdx < m_ChapterPanels.Count(); ++i) {
    m_PanelIndex[i] = currIdx;
    ++currIdx;
    InitPanelIndexForDisplay(i);
  }
...
//-----------------------------------------------------------------------------
void CNewGameDialog::UpdatePanelLockedStatus(int iUnlockedChapter, int i,
                                             CGameChapterPanel *pChapterPanel) {
...
void CNewGameDialog::PostScroll(EScrollDirection dir) {
  ...
  if (index != INVALID_INDEX) {
    ...
    GetAnimationController()->RunAnimationCommand(
        panel, "alpha", 255, 0, m_ScrollSpeed,
...
void CNewGameDialog::AnimateSelectionPanels(void) {
  ...
  if (m_ScrollCt >= SCROLL_LEFT &&
      (m_PanelIndex[SLOT_CENTER] < m_ChapterPanels.Count() - 1 ||
       !GameUI().IsConsoleUI())) {
    idxOffset = -1;
    endIdx = SLOT_OFFRIGHT;
    m_ScrollDirection = SCROLL_LEFT;
  } else if (m_ScrollCt <= SCROLL_RIGHT &&
             (m_PanelIndex[SLOT_CENTER] > 0 || !GameUI().IsConsoleUI())) {
    idxOffset = 1;
  ...
  for (int i = startIdx; i <= endIdx; ++i) {
    if (m_PanelIndex[i] != INVALID_INDEX) {
      ...
      GetAnimationController()->RunAnimationCommand(
          panel, "xpos", m_PanelXPos[nextIdx], 0, m_ScrollSpeed,
          vgui::AnimationController::INTERPOLATOR_LINEAR);
      GetAnimationController()->RunAnimationCommand(
          panel, "ypos", m_PanelYPos[nextIdx], 0, m_ScrollSpeed,
          vgui::AnimationController::INTERPOLATOR_LINEAR);
      GetAnimationController()->RunAnimationCommand(
          panel, "alpha", m_PanelAlpha[nextIdx], 0, m_ScrollSpeed,
          vgui::AnimationController::INTERPOLATOR_LINEAR);
    }
  }
  ...
  if (GameUI().IsConsoleUI()) {
    ...
    GetAnimationController()->RunAnimationCommand(
        m_pChapterTitleLabels[m_ActiveTitleIdx], "alpha", 255, 0, m_ScrollSpeed,
        vgui::AnimationController::INTERPOLATOR_LINEAR);
    GetAnimationController()->RunAnimationCommand(
        m_pChapterTitleLabels[inactiveTitleIdx], "alpha", 0, 0, m_ScrollSpeed,
        vgui::AnimationController::INTERPOLATOR_LINEAR);
...
// slot so
//			it holds the index of the panel that is actually in that
// slot's position.
//-----------------------------------------------------------------------------
...
bool CNewGameDialog::IsValidPanel(const int idx) {
  if (idx < 0 || idx >= m_ChapterPanels.Count()) return false;
  return true;
}
...
void CNewGameDialog::StartGame(void) {
  if (m_ChapterPanels.IsValidIndex(m_iSelectedChapter)) {
    char mapcommand[512];
    mapcommand[0] = 0;
    Q_snprintf(mapcommand, sizeof(mapcommand),
               "disconnect\ndeathmatch 0\nprogress_enable\nexec %s\n",
    ...
    if (IsPC()) {
      // If commentary is on, we go to the explanation dialog (but not for
      // teaser trailers)
      if (m_bCommentaryMode &&
          !m_ChapterPanels[m_iSelectedChapter]->IsTeaserChapter()) {
        DHANDLE<CCommentaryExplanationDialog> hCommentaryExplanationDialog;
        if (!hCommentaryExplanationDialog.Get()) {
          hCommentaryExplanationDialog =
              new CCommentaryExplanationDialog(BasePanel(), mapcommand);
        }
  ...
  if (!stricmp(command, "Play")) {
    ...
    if (GameUI().IsConsoleUI()) {
      if (m_ChapterPanels[m_iSelectedChapter]->IsEnabled()) {
        ...
        if (m_bCommentaryMode) {
          BasePanel()->ShowMessageDialog(
              MD_COMMENTARY_CHAPTER_UNLOCK_EXPLANATION, this);
        }
      }
    } else {
      StartGame();
