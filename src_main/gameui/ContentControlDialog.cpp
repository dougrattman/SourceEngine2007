...
//-----------------------------------------------------------------------------
CContentControlDialog::CContentControlDialog(vgui::Panel *parent)
    : vgui::Frame(parent, "ContentControlDialog") {
  ...
  m_pPasswordLabel =
      new vgui::Label(this, "PasswordPrompt", "#GameUI_PasswordPrompt");
  m_pPassword2Label = new vgui::Label(this, "PasswordReentryPrompt",
                                      "#GameUI_PasswordReentryPrompt");
...
void CContentControlDialog::ResetPassword() {
  ...
  HKEY key;
  if (ERROR_SUCCESS ==
      VCRHook_RegOpenKeyEx(HKEY_CURRENT_USER,
                           "Software\\Valve\\Half-Life\\Settings", 0, KEY_READ,
                           &key)) {
    DWORD type;
    DWORD bufSize = sizeof(m_szGorePW);

    VCRHook_RegQueryValueEx(key, "User Token 2", NULL, &type,
                            (unsigned char *)m_szGorePW, &bufSize);
...
// Purpose:
//-----------------------------------------------------------------------------
void CContentControlDialog::ApplyPassword() { WriteToken(m_szGorePW); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CContentControlDialog::Explain(char const *fmt, ...) {
  if (!m_pExplain) return;
...
void CContentControlDialog::WriteToken(const char *str) {
  ...
  if (ERROR_SUCCESS ==
      VCRHook_RegOpenKeyEx(HKEY_CURRENT_USER,
                           "Software\\Valve\\Half-Life\\Settings", 0, KEY_WRITE,
                           &key)) {
    DWORD type = REG_SZ;
    DWORD bufSize = strlen(str) + 1;

    VCRHook_RegSetValueEx(key, "User Token 2", 0, type,
                          (const unsigned char *)str, bufSize);
...
//-----------------------------------------------------------------------------
/*
bool CContentControlDialog::CheckPassword( char const *oldPW, char const *newPW,
bool enableContentControl )
{
        char digestedPW[ 128 ];
...
void CContentControlDialog::UpdateContentControlStatus(void) {
  bool enabled =
      IsPasswordEnabledInDialog();  //( m_szGorePW[0]!=0 ) ? true : false;
  m_pStatus->SetText(enabled ? "#GameUI_ContentStatusEnabled"
                             : "#GameUI_ContentStatusDisabled");
