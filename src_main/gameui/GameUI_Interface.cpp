...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CGameUI, IGameUI, GAMEUI_INTERFACE_VERSION,
                                  g_GameUI);
...
void CGameUI::Initialize(CreateInterfaceFn factory) {
  ...
  enginesound =
      (IEngineSound *)factory(IENGINESOUND_CLIENT_INTERFACE_VERSION, NULL);
  ...
  enginesurfacefuncs =
      (vgui::ISurface *)factory(VGUI_SURFACE_INTERFACE_VERSION, NULL);
...
//		dialog.  If NULL, default background is used.  If you set a
// panel, 		it should be full-screen with an opaque background, and
// must be a VGUI popup.
//-----------------------------------------------------------------------------
...
void CGameUI::BonusMapUnlock(const char *pchFileName, const char *pchMapName) {
  if (!pchFileName || pchFileName[0] == '\0' || !pchMapName ||
      pchMapName[0] == '\0') {
    if (!g_pBonusMapsDialog) return;

    g_pBonusMapsDialog->SetSelectedBooleanStatus("lock", false);
    return;
  }

  if (BonusMapsDatabase()->SetBooleanStatus("lock", pchFileName, pchMapName,
                                            false)) {
...
void CGameUI::BonusMapComplete(const char *pchFileName,
  ...
  if (BonusMapsDatabase()->SetBooleanStatus("complete", pchFileName, pchMapName,
                                            true)) {
    BonusMapsDatabase()->RefreshMapData();

    // Update the open dialog
    if (g_pBonusMapsDialog) g_pBonusMapsDialog->RefreshData();
  }
}

void CGameUI::BonusMapChallengeUpdate(const char *pchFileName,
                                      const char *pchMapName,
                                      const char *pchChallengeName, int iBest) {
  if (!pchFileName || pchFileName[0] == '\0' || !pchMapName ||
      pchMapName[0] == '\0' || !pchChallengeName ||
      pchChallengeName[0] == '\0') {
    return;
  } else {
    if (BonusMapsDatabase()->UpdateChallengeBest(pchFileName, pchMapName,
                                                 pchChallengeName, iBest)) {
...
void CGameUI::Connect(CreateInterfaceFn gameFactory) {
  g_pGameClientExports = (IGameClientExports *)gameFactory(
      GAMECLIENTEXPORTS_INTERFACE_VERSION, NULL);
...
//-----------------------------------------------------------------------------
// Purpose: Searches for GameStartup*.mp3 files in the sound/ui folder and plays
// one
//-----------------------------------------------------------------------------
...
void CGameUI::Start() {
  ...
  if (IsPC()) {
    ...
    if (g_hMutex == NULL || g_hWaitMutex == NULL ||
        Sys_GetLastError() == SYS_ERROR_INVALID_HANDLE) {
    ...
    if (!(waitResult == SYS_WAIT_OBJECT_0 ||
          waitResult == SYS_WAIT_ABANDONED)) {
      // mutex locked, need to deactivate Steam (so we have the
      // Friends/ServerBrowser data files) get the wait mutex, so that Steam.exe
      // knows that we're trying to acquire ValveTrackerMutex
    ...
    // now we are set up to check every frame to see if we can friends/server
    // browser
...
void CGameUI::ValidateCDKey() {
  ...
  vgui::system()->SetRegistryString(
      ...
      "QOgi:JXrJj<Eb8abkESf4Pg;OfofJwDzRsyH>AdjtyPnV[FB");
...
bool CGameUI::FindPlatformDirectory(char *platformDir, int bufferSize) {
  ...
  if (platformDir[0] == '\0') {
    ...
    if (::GetModuleFileName((HINSTANCE)GetModuleHandle(NULL), platformDir,
                            bufferSize)) {
      char *lastslash = strrchr(
          platformDir, '\\');  // this should be just before the filename
      if (lastslash) {
        *lastslash = 0;
...
void CGameUI::RunFrame() {
  ...
  if (m_bPlayGameStartupSound) {
    PlayGameStartupSound();
    m_bPlayGameStartupSound = false;
  }

  if (m_bTryingToLoadFriends && m_iFriendsLoadPauseFrames-- < 1 && g_hMutex &&
      g_hWaitMutex) {
...
void CGameUI::OLD_OnConnectToServer(const char *game, int IP, int port) {
  // Nobody should use this anymore because the query port and the connection
  // port can be different. Use OnConnectToServer2 instead.
  Assert(false);
...
// Purpose: Called when the game connects to a server
//-----------------------------------------------------------------------------
void CGameUI::OnConnectToServer2(const char *game, int IP, int connectionPort,
                                 int queryPort) {
  m_iGameIP = IP;
...
void CGameUI::OnDisconnectFromServer(uint8_t eSteamLoginFailure) {
  ...
  g_VModuleLoader.PostMessageToAllModules(
      new KeyValues("DisconnectedFromGame"));
...
void CGameUI::OnLevelLoadingStarted(bool bShowProgressDialog) {
  ...
  // Don't play the start game sound if this happens before we get to the first
  // frame
  m_bPlayGameStartupSound = false;
}
...
//-----------------------------------------------------------------------------
void CGameUI::OnLevelLoadingFinished(bool bError, const char *failureReason,
...
bool CGameUI::ContinueProgressBar(float progressFraction) {
  if (!g_hLoadingDialog.Get()) return false;
...
//-----------------------------------------------------------------------------
void CGameUI::StopProgressBar(bool bError, const char *failureReason,
                              const char *extendedReason) {
  if (!g_hLoadingDialog.Get() && bError) {
    g_hLoadingDialog = new CLoadingDialog(staticPanel);
  }

  if (!g_hLoadingDialog.Get()) return;
...
//-----------------------------------------------------------------------------
// Purpose: returns true if we're at the main menu and a background level is
// loaded
//-----------------------------------------------------------------------------
...
void CGameUI::ShowLoadingBackgroundDialog() {
  if (g_hLoadingBackgroundDialog) {
    vgui::ipanel()->SetParent(g_hLoadingBackgroundDialog,
                              staticPanel->GetVPanel());
    ...
    vgui::ipanel()->SendMessage(g_hLoadingBackgroundDialog,
                                new KeyValues("activate"),
                                staticPanel->GetVPanel());
  }
}
...
void CGameUI::ShowMessageDialog(const uint32_t nType, vgui::Panel *pOwner) {
  BasePanel()->ShowMessageDialog(nType, pOwner);
}
void CGameUI::CloseMessageDialog(const uint32_t nType) {
  BasePanel()->CloseMessageDialog(nType);
}
void CGameUI::UpdatePlayerInfo(uint64_t nPlayerId, const char *pName, int nTeam,
                               uint8_t cVoiceState, int nPlayersNeeded,
                               bool bHost) {
  BasePanel()->UpdatePlayerInfo(nPlayerId, pName, nTeam, cVoiceState,
                                nPlayersNeeded, bHost);
}
void CGameUI::SessionSearchResult(int searchIdx, void *pHostData,
