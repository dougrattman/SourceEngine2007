...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CPlayerListDialog::CPlayerListDialog(vgui::Panel *parent)
    : BaseClass(parent, "PlayerListDialog") {
  SetSize(320, 240);
...
void CPlayerListDialog::RefreshPlayerProperties() {
  for (int i = 0; i <= m_pPlayerList->GetItemCount(); i++) {
    ...
    if (GameClientExports() &&
        GameClientExports()->IsPlayerGameVoiceMuted(playerIndex)) {
      muted = true;
    }
    if (pi.fakeplayer) {
      bot = true;
...
void CPlayerListDialog::ToggleMuteStateOfSelectedUser() {
  if (!GameClientExports()) return;

  for (int iSelectedItem = 0;
       iSelectedItem < m_pPlayerList->GetSelectedItemsCount();
       iSelectedItem++) {
    KeyValues *data =
        m_pPlayerList->GetItem(m_pPlayerList->GetSelectedItem(iSelectedItem));
    if (!data) return;
    int playerIndex = data->GetInt("index");
    Assert(playerIndex);
...
void CPlayerListDialog::OnItemSelected() {
  ...
  if (m_pPlayerList->GetSelectedItemsCount() > 0) {
    ...
    // Matt - changed this check to see if player indeces match, instead of
    // using friends ID
    if (pi.fakeplayer ||
        iPlayerIndex == iLocalPlayer)  // || pi.friendsID ==
                                       // g_pFriendsUser->GetFriendsID() )
    {
      // invalid player,
      isValidPlayer = false;
    }
