...
// Purpose:
//-----------------------------------------------------------------------------
void CC_LoadCommentary_Test(void) { OpenLoadCommentaryDialog(); }
static ConCommand commentary_testfirstrun("loadcommentary",
                                          CC_LoadCommentary_Test, 0);
...
class CCommentaryItemPanel : public vgui::EditablePanel {
 ...
 public:
  CCommentaryItemPanel(PanelListPanel *parent, const char *name,
                       int iListItemID)
      : BaseClass(parent, name) {
    m_iListItemID = iListItemID;
    m_pParent = parent;
    m_pCommentaryScreenshot =
        new CTGAImagePanel(this, "CommentaryMapScreenshot");
    m_pCommentaryScreenshotBackground =
        new ImagePanel(this, "CommentaryScreenshotBackground");
  ...
  virtual void OnMouseDoublePressed(vgui::MouseCode code) {
    ...
    PostMessage(m_pParent->GetParent(),
                new KeyValues("Command", "command", "loadcommentary"));
  }
...
// Purpose:Constructor
//-----------------------------------------------------------------------------
CLoadCommentaryDialog::CLoadCommentaryDialog(vgui::Panel *parent)
    : BaseClass(parent, "LoadCommentaryDialog") {
...
int CLoadCommentaryDialog::GetSelectedItemIndex() {
  CCommentaryItemPanel *panel =
      dynamic_cast<CCommentaryItemPanel *>(m_pGameList->GetSelectedPanel());
  if (panel) {
...
void CLoadCommentaryDialog::ScanCommentaryFiles() {
  ...
  // sort the save list
  qsort(m_CommentaryItems.Base(), m_CommentaryItems.Count(),
        sizeof(CommentaryItem_t), &SaveGameSortFunc);

  // add to the list
  for (int saveIndex = 0; saveIndex < m_CommentaryItems.Count() &&
                          saveIndex < MAX_LISTED_COMMENTARY_ITEMS;
       saveIndex++) {
  ...
  if (!m_CommentaryItems.Count()) {
    vgui::Label *pNoCommentaryItemsLabel =
        SETUP_PANEL(new Label(m_pGameList, "NoCommentaryItemsLabel",
...
void CLoadCommentaryDialog::AddCommentaryItemToList(int itemIndex) {
  ...
  CCommentaryItemPanel *commentaryItemPanel =
      new CCommentaryItemPanel(m_pGameList, "CommentaryItemPanel", itemIndex);
...
//-----------------------------------------------------------------------------
void CLoadCommentaryDialog::ParseCommentaryFile(char const *pszFileName,
  ...
  if (menuKeys) {
    for (KeyValues *track = menuKeys->GetFirstSubKey(); track != NULL;
         track = track->GetNextKey()) {
      // Msg( "track found: %s %s\n", track->GetString("map", "?"),
      // track->GetString( "description", "asdf" ) );

      CommentaryItem_t item;
      Q_strncpy(item.szMapFileName, pszFileName, sizeof(item.szMapFileName));
      Q_strncpy(item.szMapName, track->GetString("map", ""),
                sizeof(item.szMapName));
      Q_strncpy(item.szPrintName, track->GetString("printname", ""),
                sizeof(item.szPrintName));
      Q_strncpy(item.szDescription, track->GetString("description", ""),
                sizeof(item.szDescription));
    ...
    *ext = '\0';
    ...
    Q_strncpy(item.szPrintName, "No trackinfo found.",
              sizeof(item.szPrintName));
    Q_strncpy(item.szDescription, "No trackinfo found.",
              sizeof(item.szDescription));
    m_CommentaryItems.AddToTail(item);
  }
