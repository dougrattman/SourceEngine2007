...
ConVar joy_xcontroller_found(
    "joy_xcontroller_found", "1", FCVAR_NONE,
...
static LRESULT CALLBACK InputSystemWindowProc(HWND hwnd, UINT uMsg,
...
ButtonCode_t CInputSystem::XKeyToButtonCode(int nPort, int nXKey) const {
  if (m_bXController) return ButtonCode_XKeyToButtonCode(nPort, nXKey);
  return KEY_NONE;
}
...
//-----------------------------------------------------------------------------
void CInputSystem::PostEvent(int nType, int nTick, int nData, int nData2,
                             int nData3) {
...
//-----------------------------------------------------------------------------
void CInputSystem::PostButtonPressedEvent(InputEventType_t nType, int nTick,
...
//-----------------------------------------------------------------------------
void CInputSystem::PostButtonReleasedEvent(InputEventType_t nType, int nTick,
                                           ButtonCode_t scanCode,
                                           ButtonCode_t virtualCode) {
  InputState_t &state = m_InputState[m_bIsPolling];
  if (state.m_ButtonState.IsBitSet(scanCode)) {
...
//-----------------------------------------------------------------------------
void CInputSystem::CopyInputState(InputState_t *pDest, const InputState_t &src,
                                  bool bCopyEvents) {
  pDest->m_Events.RemoveAll();
  pDest->m_bDirty = false;
  if (src.m_bDirty) {
    pDest->m_ButtonState = src.m_ButtonState;
    memcpy(&pDest->m_ButtonPressedTick, &src.m_ButtonPressedTick,
           BUTTON_CODE_LAST * sizeof(int));
    memcpy(&pDest->m_ButtonReleasedTick, &src.m_ButtonReleasedTick,
           BUTTON_CODE_LAST * sizeof(int));
    memcpy(&pDest->m_pAnalogDelta, &src.m_pAnalogDelta,
           ANALOG_CODE_LAST * sizeof(int));
    memcpy(&pDest->m_pAnalogValue, &src.m_pAnalogValue,
           ANALOG_CODE_LAST * sizeof(int));
    if (bCopyEvents) {
      if (src.m_Events.Count() > 0) {
        pDest->m_Events.EnsureCount(src.m_Events.Count());
        memcpy(pDest->m_Events.Base(), src.m_Events.Base(),
               src.m_Events.Count() * sizeof(InputEvent_t));
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
//	Purpose: Sets a player as the primary user - all other controllers will
// be ignored.
//-----------------------------------------------------------------------------
...
// Poll current state
//-----------------------------------------------------------------------------
int CInputSystem::GetPollTick() const { return m_nLastPollTick; }
...
inline LRESULT CInputSystem::ChainWindowMessage(HWND hwnd, UINT uMsg,
  ...
  // engine needs to do this in order for VCR mode to play back properly.
  return 0;
}
...
void CInputSystem::ZeroAnalogState(int nFirstState, int nLastState) {
  InputState_t &state = m_InputState[m_bIsPolling];
  memset(&state.m_pAnalogDelta[nFirstState], 0,
         (nLastState - nFirstState + 1) * sizeof(int));
  memset(&state.m_pAnalogValue[nFirstState], 0,
         (nLastState - nFirstState + 1) * sizeof(int));
}
...
//-----------------------------------------------------------------------------
int CInputSystem::ButtonMaskFromMouseWParam(WPARAM wParam, ButtonCode_t code,
                                            bool bDown) const {
  int nButtonMask = 0;
...
//-----------------------------------------------------------------------------
void CInputSystem::UpdateMouseButtonState(int nButtonMask,
                                          ButtonCode_t dblClickCode) {
  for (int i = 0; i < 5; ++i) {
    ButtonCode_t code = (ButtonCode_t)(MOUSE_FIRST + i);
    bool bDown = (nButtonMask & (1 << i)) != 0;
    if (bDown) {
      InputEventType_t type =
          (code != dblClickCode) ? IE_ButtonPressed : IE_ButtonDoubleClicked;
...
void CInputSystem::SetCursorPosition(int x, int y) {
  ...
  if (bXChanged) {
    PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_X,
              state.m_pAnalogValue[MOUSE_X], state.m_pAnalogDelta[MOUSE_X]);
  }
  if (bYChanged) {
    PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_Y,
              state.m_pAnalogValue[MOUSE_Y], state.m_pAnalogDelta[MOUSE_Y]);
  }
  if (bXChanged || bYChanged) {
    PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_XY,
              state.m_pAnalogValue[MOUSE_X], state.m_pAnalogValue[MOUSE_Y]);
  }
}
...
// Handles input messages
//-----------------------------------------------------------------------------
LRESULT CInputSystem::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                 LPARAM lParam) {
  ...
  switch (uMsg) {
    ...
    case WM_SYSKEYDOWN: {
      ...
      if (!(lParam & (1 << 30))) {
        ...
        // which returns KEY_ENTER from virtual keys, and KEY_PAD_ENTER from
        // scan codes Since things like vgui care about virtual keys; we're
        // going to put both scan codes in the input message
        ...
        PostButtonPressedEvent(IE_ButtonPressed, m_nLastSampleTick, scanCode,
                               virtualCode);

        // Deal with toggles
        if (scanCode == KEY_CAPSLOCK || scanCode == KEY_SCROLLLOCK ||
          ...
          PostButtonPressedEvent(
              bToggleState ? IE_ButtonPressed : IE_ButtonReleased,
              m_nLastSampleTick, toggleCode, toggleCode);
        }
      }
    } break;
    ...
    case WM_KEYUP:
    case WM_SYSKEYUP: {
      // Don't handle key ups if the key's already up. This can happen when we
      // alt-tab back to the engine.
      ButtonCode_t virtualCode = ButtonCode_VirtualKeyToButtonCode(wParam);
      ButtonCode_t scanCode = ButtonCode_ScanCodeToButtonCode(lParam);
      PostButtonReleasedEvent(IE_ButtonReleased, m_nLastSampleTick, scanCode,
                              virtualCode);
    } break;

    case WM_MOUSEWHEEL: {
      ButtonCode_t code =
          (short)HIWORD(wParam) > 0 ? MOUSE_WHEEL_UP : MOUSE_WHEEL_DOWN;
      ...
      PostEvent(IE_AnalogValueChanged, m_nLastSampleTick, MOUSE_WHEEL,
                state.m_pAnalogValue[MOUSE_WHEEL],
