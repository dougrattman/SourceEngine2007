...
#define XBX_STICK_SCALE_LEFT(x) \
  ((float)XBX_MAX_STICKSAMPLE_LEFT / (float)(XBX_MAX_STICKSAMPLE_LEFT - (x)))
#define XBX_STICK_SCALE_RIGHT(x) \
  ((float)XBX_MAX_STICKSAMPLE_RIGHT / (float)(XBX_MAX_STICKSAMPLE_RIGHT - (x)))
#define XBX_STICK_SCALE_DOWN(x) \
  ((float)XBX_MAX_STICKSAMPLE_DOWN / (float)(XBX_MAX_STICKSAMPLE_DOWN - (x)))
#define XBX_STICK_SCALE_UP(x) \
  ((float)XBX_MAX_STICKSAMPLE_UP / (float)(XBX_MAX_STICKSAMPLE_UP - (x)))
...
void CInputSystem::InitializeXDevices(void) {
  ...
  PC_XInputSetState =
      (XInputSetState_t)GetProcAddress((HMODULE)m_pXInputDLL, "XInputSetState");
  PC_XInputGetCapabilities = (XInputGetCapabilities_t)GetProcAddress(
...
void CInputSystem::OpenXDevice(xdevice_t* pXDevice, int userId) {
  ...
  DWORD result =
      XINPUTGETCAPABILITIES(userId, XINPUT_FLAG_GAMEPAD, &capabilities);
  if (result == ERROR_SUCCESS) {
    bool bIsSupported = false;
    // Current version of XInput mistakenly returns 0 as the Type. Ignore it
    // and ensure the subtype is a gamepad.
    bIsSupported = (capabilities.SubType == XINPUT_DEVSUBTYPE_GAMEPAD);

    if (!bIsSupported) {
      // TBD: This may not be sufficient to not crash us later
      AssertMsg(0, "Unsupported XDevice Type");
      return;
    }
    ...
    pXDevice->stickThreshold[STICK1_AXIS_LEFT] =
        XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK1_AXIS_LEFT] =
        XBX_STICK_SCALE_LEFT(XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
    pXDevice->stickThreshold[STICK1_AXIS_RIGHT] =
        XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK1_AXIS_RIGHT] =
        XBX_STICK_SCALE_RIGHT(XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
    pXDevice->stickThreshold[STICK1_AXIS_DOWN] =
        XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK1_AXIS_DOWN] =
        XBX_STICK_SCALE_DOWN(XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);
    pXDevice->stickThreshold[STICK1_AXIS_UP] =
        XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK1_AXIS_UP] =
        XBX_STICK_SCALE_UP(XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE);

    // right stick, default to narrow zone
    pXDevice->stickThreshold[STICK2_AXIS_LEFT] =
        XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK2_AXIS_LEFT] =
        XBX_STICK_SCALE_LEFT(XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
    pXDevice->stickThreshold[STICK2_AXIS_RIGHT] =
        XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK2_AXIS_RIGHT] =
        XBX_STICK_SCALE_RIGHT(XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
    pXDevice->stickThreshold[STICK2_AXIS_DOWN] =
        XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK2_AXIS_DOWN] =
        XBX_STICK_SCALE_DOWN(XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
    pXDevice->stickThreshold[STICK2_AXIS_UP] =
        XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE;
    pXDevice->stickScale[STICK2_AXIS_UP] =
        XBX_STICK_SCALE_UP(XINPUT_GAMEPAD_RIGHT_THUMB_DEADZONE);
...
void CInputSystem::PollXDevices(void) {
  ...
  for (int userId = 0; userId < XUSER_MAX_COUNT; ++userId, ++pXDevice) {
    DWORD result =
        XINPUTGETSTATE(userId, &pXDevice->states[pXDevice->newState]);
    switch (result) {
      case ERROR_SUCCESS:
        if (!pXDevice->active) {
          // just inserted
        ...
        // See if primary user has been set, and if so then block all other
        // input
        if (pXDevice->userId == m_PrimaryUserId ||
      ...
      case ERROR_DEVICE_NOT_CONNECTED:
        if (pXDevice->active) {
          ...
          if (m_PrimaryUserId == INVALID_USER_ID || m_PrimaryUserId == userId) {
            ...
            // TODO: Change this to release buttons of the specific joystick
            // that unplugged
            ReleaseAllButtons(JOYSTICK_FIRST, JOYSTICK_LAST);
            ZeroAnalogState(JOYSTICK_FIRST_AXIS, JOYSTICK_LAST_AXIS);
            memset(m_appXKeys, 0,
                   XUSER_MAX_COUNT * XK_MAX_KEYS * sizeof(appKey_t));
...
void CInputSystem::PostXKeyEvent(int userId, xKey_t xKey, int nSample) {
  ...
  if (nSample) {
    PostButtonPressedEvent(IE_ButtonPressed, m_nLastSampleTick, buttonCode,
                           buttonCode);
  } else {
    PostButtonReleasedEvent(IE_ButtonReleased, m_nLastSampleTick, buttonCode,
                            buttonCode);
  }
}
...
void CInputSystem::ReadXDevice(xdevice_t* pXDevice) {
  ...
  // forceful exit
  if (newStatePtr->Gamepad.wButtons ==
  ...
  if (oldStatePtr->dwPacketNumber == newStatePtr->dwPacketNumber) return;
  ...
  if (buttons) {
    ...
    bool bDpadIsAxial =
        IsPowerOfTwo(newStatePtr->Gamepad.wButtons &
                     (XINPUT_GAMEPAD_DPAD_UP | XINPUT_GAMEPAD_DPAD_DOWN |
                      XINPUT_GAMEPAD_DPAD_LEFT | XINPUT_GAMEPAD_DPAD_RIGHT));

    // determine digital difference - up or down?
    for (int i = 0;
         i < sizeof(g_digitalXKeyTable) / sizeof(g_digitalXKeyTable[0]); ++i) {
      ...
      // the last down axial event trumps, preventing diagonals causing multiple
      // events
      if (mask & (XINPUT_GAMEPAD_DPAD_UP | XINPUT_GAMEPAD_DPAD_DOWN |
                  XINPUT_GAMEPAD_DPAD_LEFT | XINPUT_GAMEPAD_DPAD_RIGHT)) {
        if (!bDpadIsAxial) {
          // diagonal dpad event, discard any dpad event that isn't the lock
          if (pXDevice->dpadLock && pXDevice->dpadLock != mask) continue;
        } else if (sample) {
      ...
      PostXKeyEvent(pXDevice->userId, (xKey_t)g_digitalXKeyTable[i].xkey,
                    sample);
    }
  }
  ...
  if (sample != oldStatePtr->Gamepad.bLeftTrigger) {
    PostXKeyEvent(pXDevice->userId, XK_BUTTON_LTRIGGER,
                  (int)(sample * (float)XBX_MAX_BUTTONSAMPLE /
                        (float)XBX_MAX_ANALOGSAMPLE));
  }
  ...
  if (sample != oldStatePtr->Gamepad.bRightTrigger) {
    PostXKeyEvent(pXDevice->userId, XK_BUTTON_RTRIGGER,
                  (int)(sample * (float)XBX_MAX_BUTTONSAMPLE /
                        (float)XBX_MAX_ANALOGSAMPLE));
  }
  ...
  if (x < -pXDevice->stickThreshold[STICK1_AXIS_LEFT]) {
    x = (int)((-x - pXDevice->stickThreshold[STICK1_AXIS_LEFT]) *
              pXDevice->stickScale[STICK1_AXIS_LEFT]);
    PostXKeyEvent(pXDevice->userId, XK_STICK1_LEFT, x);
    key = XK_STICK1_LEFT;
  } else if (x > pXDevice->stickThreshold[STICK1_AXIS_RIGHT]) {
    x = (int)((x - pXDevice->stickThreshold[STICK1_AXIS_RIGHT]) *
              pXDevice->stickScale[STICK1_AXIS_RIGHT]);
  ...
  if (lastStickKeys[LASTKEY_STICK1_X] &&
  ...
  if (y < -pXDevice->stickThreshold[STICK1_AXIS_DOWN]) {
    y = (int)((-y - pXDevice->stickThreshold[STICK1_AXIS_DOWN]) *
              pXDevice->stickScale[STICK1_AXIS_DOWN]);
    PostXKeyEvent(pXDevice->userId, XK_STICK1_DOWN, y);
    key = XK_STICK1_DOWN;
  } else if (y > pXDevice->stickThreshold[STICK1_AXIS_UP]) {
    y = (int)((y - pXDevice->stickThreshold[STICK1_AXIS_UP]) *
              pXDevice->stickScale[STICK1_AXIS_UP]);
  ...
  if (lastStickKeys[LASTKEY_STICK1_Y] &&
  ...
  if (x < -pXDevice->stickThreshold[STICK2_AXIS_LEFT]) {
    x = (int)((-x - pXDevice->stickThreshold[STICK2_AXIS_LEFT]) *
              pXDevice->stickScale[STICK2_AXIS_LEFT]);
    PostXKeyEvent(pXDevice->userId, XK_STICK2_LEFT, x);
    key = XK_STICK2_LEFT;
  } else if (x > pXDevice->stickThreshold[STICK2_AXIS_RIGHT]) {
    x = (int)((x - pXDevice->stickThreshold[STICK2_AXIS_RIGHT]) *
              pXDevice->stickScale[STICK2_AXIS_RIGHT]);
  ...
  if (lastStickKeys[LASTKEY_STICK2_X] &&
  ...
  if (y < -pXDevice->stickThreshold[STICK2_AXIS_DOWN]) {
    y = (int)((-y - pXDevice->stickThreshold[STICK2_AXIS_DOWN]) *
              pXDevice->stickScale[STICK2_AXIS_DOWN]);
    PostXKeyEvent(pXDevice->userId, XK_STICK2_DOWN, y);
    key = XK_STICK2_DOWN;
  } else if (y > pXDevice->stickThreshold[STICK2_AXIS_UP]) {
    y = (int)((y - pXDevice->stickThreshold[STICK2_AXIS_UP]) *
              pXDevice->stickScale[STICK2_AXIS_UP]);
  ...
  if (lastStickKeys[LASTKEY_STICK2_Y] &&
...
//-----------------------------------------------------------------------------
void CInputSystem::SetXDeviceRumble(float fLeftMotor, float fRightMotor,
                                    int userId) {
  ...
  if (pXDevice->userId == INVALID_USER_ID) return;
