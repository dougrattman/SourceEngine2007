...
ConVar joy_wwhack1("joy_wingmanwarrior_centerhack", "0", FCVAR_ARCHIVE,
                   "Wingman warrior centering hack.");
ConVar joy_axisbutton_threshold(
    "joy_axisbutton_threshold", "0.3", FCVAR_ARCHIVE,
...
void CInputSystem::InitializeJoysticks(void) {
  ...
  for (int i = 0; i < nMaxJoysticks; i++) {
    ...
    info.m_nFlags =
        JOY_RETURNCENTERED | JOY_RETURNBUTTONS | JOY_RETURNX | JOY_RETURNY;
    info.m_nAxisFlags = 0;
    if (jc.wNumAxes >= 2) {
...
void CInputSystem::JoystickButtonEvent(ButtonCode_t button, int sample) {
  ...
  if (sample) {
    ...
    PostButtonReleasedEvent(IE_ButtonReleased, m_nLastSampleTick, button,
                            button);
  }
}
...
void CInputSystem::UpdateJoystickButtonState(int nJoystick) {
  ...
  for (int j = 0; j < MAX_JOYSTICK_AXES; ++j) {
    if ((info.m_nAxisFlags & (1 << j)) == 0) continue;
...
void CInputSystem::UpdateJoystickPOVControl(int nJoystick) {
  ...
  if (!info.m_bHasPOVControl) return;
...
void CInputSystem::PollJoystick(void) {
  ...
  for (int i = 0; i < m_nJoystickCount; ++i) {
    if (!m_JoysticksEnabled.IsFlagSet(1 << i)) continue;
    ...
    // 32668 go figure -- anyway, now we get the full resolution out of the
    // device
