...
void CInsertionTracker::InsertBytes(void *pos, int length) {
  ...
  Assert(m_map.InvalidIndex() == m_map.Find((u8 *)pos));
  m_map.InsertOrReplace((u8 *)pos, length);
}

int CInsertionTracker::GetNumBytesInserted() const {
  int iInserted = 0;

  for (Map::IndexType_t idx = m_map.FirstInorder(); idx != m_map.InvalidIndex();
       idx = m_map.NextInorder(idx)) {
...
void CInsertionTracker::MemMove(void *ptrBase, int &length) const {
  ...
  u8 *pbBlockEnd = BYTE_OFF_PTR(ptrBase, length);
  length += numBytesInsertReq;

  for (Map::IndexType_t idx = m_map.LastInorder(); idx != m_map.InvalidIndex();
       idx = m_map.PrevInorder(idx)) {
    u8 *ptr = m_map.Key(idx);
    int numBytes = m_map.Element(idx);

    // Move [ptr, pbBlockEnd) ->> + numBytesInsertReq
    memmove(BYTE_OFF_PTR(ptr, numBytesInsertReq), ptr,
            BYTE_DIFF_PTR(ptr, pbBlockEnd));
...
void *CInsertionTracker::ComputePointer(void *ptrNothingInserted) const {
  ...
  // Iterate the map and find all the data that would be inserted before the
  // given pointer
  for (Map::IndexType_t idx = m_map.FirstInorder(); idx != m_map.InvalidIndex();
       idx = m_map.NextInorder(idx)) {
...
void CRemoveTracker::RemoveBytes(void *pos, int length) {
  ...
  // Insert new
  Assert(m_map.InvalidIndex() == m_map.Find((u8 *)pos));
  Map::IndexType_t idx = m_map.InsertOrReplace((u8 *)pos, length);
...
void CRemoveTracker::MemMove(void *ptrBase, int &length) const {
  ...
  for (Map::IndexType_t idx = m_map.FirstInorder(); idx != m_map.InvalidIndex();
       idx = m_map.NextInorder(idx)) {
    u8 *ptr = m_map.Key(idx);
    u8 *ptrDest = BYTE_OFF_PTR(ptr, -iRemoved);
    int numBytes = m_map.Element(idx);
    memmove(ptrDest, BYTE_OFF_PTR(ptrDest, numBytes),
            BYTE_DIFF_PTR(BYTE_OFF_PTR(ptr, numBytes),
                          BYTE_OFF_PTR(ptrBase, length)));
    iRemoved += numBytes;
  }
...
void *CRemoveTracker::ComputePointer(void *ptrNothingRemoved) const {
  ...
  // Iterate the map and find all the data that would be removed before the
  // given pointer
  for (Map::IndexType_t idx = m_map.FirstInorder(); idx != m_map.InvalidIndex();
       idx = m_map.NextInorder(idx)) {
