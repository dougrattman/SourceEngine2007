...
class CMdlStripInfo : public IMdlStripInfo {
 ...
 public:
  ...
  // Load the strip info from the buffer (reads from the current position as
  // much as needed)
 ...
 public:
  ...
  //the  original model submitted for stripping
  //	plChecksumStripped		if non-NULL will hold the resulting
  //checksum  of the stripped model
  virtual bool GetCheckSum(long *plChecksumOriginal,
 ...
 public:
  struct MdlRangeItem {
    /* implicit */ MdlRangeItem(int offOld = 0, int numOld = 0, int offNew = 0,
                                int numNew = 0)
        : m_offOld(offOld),
          m_offNew(offNew),
          m_numOld(numOld),
  ...
  CUtlSortVector<CMdlStripInfo::MdlRangeItem,
                 CLessSimple<CMdlStripInfo::MdlRangeItem> >
      m_vtxMdlOffsets;
};

#endif  // SOURCE_MDLLIB_STRIPINFO_H_