...
CMdlStripInfo::CMdlStripInfo()
    : m_eMode(MODE_UNINITIALIZED), m_lChecksumOld(0), m_lChecksumNew(0) {
  NULL;
}
...
bool CMdlStripInfo::Serialize(CUtlBuffer &bufStorage) const {
  ...
  switch (m_eMode) {
    ...
    case MODE_STRIP_LOD_1N:
      ...
      for (u32 const *pdwBase = m_vtxVerts.Base(),
                     *pdwEnd = pdwBase + m_vtxVerts.GetNumDWords();
           pdwBase < pdwEnd; ++pdwBase)
        bufStorage.PutUnsignedInt(*pdwBase);

      bufStorage.PutInt(m_vtxIndices.Count());
      for (unsigned short const *pusBase = m_vtxIndices.Base(),
                                *pusEnd = pusBase + m_vtxIndices.Count();
           pusBase < pusEnd; ++pusBase)
        bufStorage.PutUnsignedShort(*pusBase);

      bufStorage.PutInt(m_vtxMdlOffsets.Count());
      for (MdlRangeItem const *pmri = m_vtxMdlOffsets.Base(),
                              *pmriEnd = pmri + m_vtxMdlOffsets.Count();
           pmri < pmriEnd; ++pmri)
        bufStorage.PutInt(pmri->m_offOld), bufStorage.PutInt(pmri->m_offNew),
...
bool CMdlStripInfo::UnSerialize(CUtlBuffer &bufData) {
  ...
  switch (chHeader[3]) {
    ...
    case MODE_STRIP_LOD_1N:
      ...
      for (u32 *pdwBase = m_vtxVerts.Base(),
               *pdwEnd = pdwBase + m_vtxVerts.GetNumDWords();
           pdwBase < pdwEnd; ++pdwBase)
        *pdwBase = bufData.GetUnsignedInt();

      m_vtxIndices.SetCount(bufData.GetInt());
      for (unsigned short *pusBase = m_vtxIndices.Base(),
                          *pusEnd = pusBase + m_vtxIndices.Count();
           pusBase < pusEnd; ++pusBase)
        *pusBase = bufData.GetUnsignedShort();

      m_vtxMdlOffsets.SetCount(bufData.GetInt());
      for (MdlRangeItem *pmri = m_vtxMdlOffsets.Base(),
                        *pmriEnd = pmri + m_vtxMdlOffsets.Count();
           pmri < pmriEnd; ++pmri)
        pmri->m_offOld = bufData.GetInt(), pmri->m_offNew = bufData.GetInt(),
        pmri->m_numOld = bufData.GetInt(), pmri->m_numNew = bufData.GetInt();
...
// the  original model submitted for stripping 	plChecksumStripped  if non-NULL
// will hold the resulting checksum of the stripped model
bool CMdlStripInfo::GetCheckSum(long *plChecksumOriginal,
                                long *plChecksumStripped) const {
  if (m_eMode == MODE_UNINITIALIZED) return false;
...
bool CMdlStripInfo::StripHardwareVertsBuffer(CUtlBuffer &vhvBuffer) {
  ...
  // Recover vhv header
  //
  DECLARE_PTR(HardwareVerts::FileHeader_t, vhvHdr,
              BYTE_OFF_PTR(vhvBuffer.Base(), vhvBuffer.TellGet()));
  ...
  size_t vhvVertOffset = ~size_t(0),
         vhvEndMeshOffset = sizeof(HardwareVerts::FileHeader_t);
  int numMeshesRemoved = 0, numVertsRemoved = 0;

  ITERATE_CHILDREN(HardwareVerts::MeshHeader_t, vhvMesh, vhvHdr, pMesh,
                   m_nMeshes)
  if (vhvMesh->m_nOffset < vhvVertOffset) vhvVertOffset = vhvMesh->m_nOffset;
  if (BYTE_DIFF_PTR(vhvHdr, vhvMesh + 1) > vhvEndMeshOffset)
    vhvEndMeshOffset = BYTE_DIFF_PTR(vhvHdr, vhvMesh + 1);
  if (!vhvMesh->m_nLod) continue;
  vhvRemove.RemoveBytes(BYTE_OFF_PTR(vhvHdr, vhvMesh->m_nOffset),
                        vhvMesh->m_nVertexes * vhvHdr->m_nVertexSize);
  vhvRemove.RemoveElements(vhvMesh);
  numVertsRemoved += vhvMesh->m_nVertexes;
  ++numMeshesRemoved;
  ITERATE_END
  vhvRemove.RemoveBytes(BYTE_OFF_PTR(vhvHdr, vhvEndMeshOffset),
                        vhvVertOffset - vhvEndMeshOffset);  // Padding
  vhvRemove.RemoveBytes(
      BYTE_OFF_PTR(vhvHdr,
                   vhvVertOffset + vhvHdr->m_nVertexes * vhvHdr->m_nVertexSize),
      vhvLength -
          (vhvVertOffset + vhvHdr->m_nVertexes * vhvHdr->m_nVertexSize));
  ...
  ITERATE_CHILDREN(HardwareVerts::MeshHeader_t, vhvMesh, vhvHdr, pMesh,
                   m_nMeshes)
  vhvMesh->m_nOffset = vhvRemove.ComputeOffset(vhvHdr, vhvMesh->m_nOffset) +
                       vhvAlignedVertOffset - vhvNewVertOffset;
  ITERATE_END
  ...
  size_t numBytesNewLength =
      vhvLength + vhvAlignedVertOffset - vhvNewVertOffset;
  size_t numAlignedNewLength = AlignValue(numBytesNewLength, 4);

  // Now reinsert the padding
  CInsertionTracker vhvInsertPadding;
  vhvInsertPadding.InsertBytes(BYTE_OFF_PTR(vhvHdr, vhvNewVertOffset),
                               vhvAlignedVertOffset - vhvNewVertOffset);
  vhvInsertPadding.InsertBytes(BYTE_OFF_PTR(vhvHdr, vhvLength),
                               numAlignedNewLength - numBytesNewLength);
  ...
  vhvBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT,
                    vhvBuffer.TellGet() + vhvLength - vhvBuffer.TellPut());
...
bool CMdlStripInfo::StripModelBuffer(CUtlBuffer &mdlBuffer) {
  ...
  // Recover mdl header
  //
  DECLARE_PTR(studiohdr_t, mdlHdr,
              BYTE_OFF_PTR(mdlBuffer.Base(), mdlBuffer.TellGet()));
  ...
  CUtlSortVector<unsigned short, CLessSimple<unsigned short> > &srcIndices =
      m_vtxIndices;

  ITERATE_CHILDREN(mstudiobodyparts_t, mdlBodyPart, mdlHdr, pBodypart,
                   numbodyparts)
  ITERATE_CHILDREN(mstudiomodel_t, mdlModel, mdlBodyPart, pModel, nummodels)

  DLog("mdllib", 3, " Stripped %d vertexes (was: %d, now: %d).\n",
       mdlModel->numvertices - srcIndices.Count(), mdlModel->numvertices,
       srcIndices.Count());
  ...
  mdlMesh->numvertices =
      srcIndices.FindLess(mdlMesh->vertexoffset + mdlMesh->numvertices);
  ...
  for (usize k = 0; k < std::size(mdlMesh->vertexdata.numLODVertexes); ++k)
...
bool CMdlStripInfo::StripVertexDataBuffer(CUtlBuffer &vvdBuffer) {
  ...
  // Recover vvd header
  //
  DECLARE_PTR(vertexFileHeader_t, vvdHdr,
              BYTE_OFF_PTR(vvdBuffer.Base(), vvdBuffer.TellGet()));
  ...
  vvdHdr->numLODs = 1;
  for (usize k = 0; k < std::size(vvdHdr->numLODVertexes); ++k)
    vvdHdr->numLODVertexes[k] = srcIndices.Count();

  DECLARE_PTR(mstudiovertex_t, vvdVertexSrc,
              BYTE_OFF_PTR(vvdHdr, vvdHdr->vertexDataStart));
  DECLARE_PTR(Vector4D, vvdTangentSrc,
  ...
  if (vvdHdr->numFixups) {
    CArrayAutoPtr<u8> memTempVVD(new u8[vvdLength]);
    DECLARE_PTR(mstudiovertex_t, vvdVertexNew,
                BYTE_OFF_PTR(memTempVVD.Get(), vvdHdr->vertexDataStart));
    DECLARE_PTR(Vector4D, vvdTangentNew,
                BYTE_OFF_PTR(memTempVVD.Get(), vvdHdr->tangentDataStart));
    DECLARE_PTR(vertexFileFixup_t, vvdFixup,
                BYTE_OFF_PTR(vvdHdr, vvdHdr->fixupTableStart));
    for (int k = 0; k < vvdHdr->numFixups; ++k) {
      memcpy(vvdVertexNew, vvdVertexSrc + vvdFixup[k].sourceVertexID,
             vvdFixup[k].numVertexes * sizeof(*vvdVertexNew));
      vvdVertexNew += vvdFixup[k].numVertexes;
      if (vvdTangentSrc) {
        memcpy(vvdTangentNew, vvdTangentSrc + vvdFixup[k].sourceVertexID,
               vvdFixup[k].numVertexes * sizeof(*vvdTangentNew));
    ...
    vvdVertexSrc
        ? memcpy(vvdVertexSrc,
                 BYTE_OFF_PTR(memTempVVD.Get(), vvdHdr->vertexDataStart),
                 mdlNumVerticesOld * sizeof(*vvdVertexSrc))
        : 0;
    vvdTangentSrc
        ? memcpy(vvdTangentSrc,
                 BYTE_OFF_PTR(memTempVVD.Get(), vvdHdr->tangentDataStart),
                 mdlNumVerticesOld * sizeof(*vvdTangentSrc))
        : 0;
  }

  vvdHdr->vertexDataStart -=
      AlignValue(sizeof(vertexFileFixup_t) * vvdHdr->numFixups, 16);
  vvdHdr->numFixups = 0;
  DECLARE_PTR(mstudiovertex_t, vvdVertexNew,
              BYTE_OFF_PTR(vvdHdr, vvdHdr->vertexDataStart));
  ...
  vvdBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT,
                    vvdBuffer.TellGet() + vvdLength - vvdBuffer.TellPut());
...
bool CMdlStripInfo::StripOptimizedModelBuffer(CUtlBuffer &vtxBuffer) {
  ...
  // Recover vtx header
  //
  DECLARE_PTR(OptimizedModel::FileHeader_t, vtxHdr,
              BYTE_OFF_PTR(vtxBuffer.Base(), vtxBuffer.TellGet()));
  int vtxLength = vtxBuffer.TellPut() - vtxBuffer.TellGet();

  if (vtxHdr->checkSum != m_lChecksumOld) {
    DLog("mdllib", 1,
         "ERROR: [StripOptimizedModelBuffer] checksum mismatch!\n");
    return false;
  }
  ...
  CUtlSortVector<unsigned short, CLessSimple<unsigned short> > &srcIndices =
      m_vtxIndices;
  CUtlSortVector<CMdlStripInfo::MdlRangeItem,
                 CLessSimple<CMdlStripInfo::MdlRangeItem> > &arrMdlOffsets =
      m_vtxMdlOffsets;
  ...
  vtxRemove.RemoveElements(CHILD_AT(vtxHdr, pMaterialReplacementList, 1),
                           vtxHdr->numLODs - 1);
  ITERATE_CHILDREN(OptimizedModel::MaterialReplacementListHeader_t, vtxMatList,
                   vtxHdr, pMaterialReplacementList, numLODs)
  if (!vtxMatList_idx) continue;
  vtxRemove.RemoveElements(CHILD_AT(vtxMatList, pMaterialReplacement, 0),
                           vtxMatList->numReplacements);
  ITERATE_CHILDREN(OptimizedModel::MaterialReplacementHeader_t, vtxMat,
                   vtxMatList, pMaterialReplacement, numReplacements)
  char const *szName = vtxMat->pMaterialReplacementName();
  vtxRemove.RemoveElements(szName, szName ? strlen(szName) + 1 : 0);
  ITERATE_END
  ITERATE_END

  ITERATE_CHILDREN(OptimizedModel::BodyPartHeader_t, vtxBodyPart, vtxHdr,
                   pBodyPart, numBodyParts)
  ITERATE_CHILDREN(OptimizedModel::ModelHeader_t, vtxModel, vtxBodyPart, pModel,
                   numModels)

  vtxRemove.RemoveElements(CHILD_AT(vtxModel, pLOD, 1), vtxModel->numLODs - 1);
  ITERATE_CHILDREN(OptimizedModel::ModelLODHeader_t, vtxLod, vtxModel, pLOD,
                   numLODs)
  if (!vtxLod_idx)  // Process only lod1-N
    continue;

  vtxRemove.RemoveElements(CHILD_AT(vtxLod, pMesh, 0), vtxLod->numMeshes);
  ITERATE_CHILDREN(OptimizedModel::MeshHeader_t, vtxMesh, vtxLod, pMesh,
                   numMeshes)
  vtxRemove.RemoveElements(CHILD_AT(vtxMesh, pStripGroup, 0),
                           vtxMesh->numStripGroups);
  ITERATE_CHILDREN(OptimizedModel::StripGroupHeader_t, vtxStripGroup, vtxMesh,
                   pStripGroup, numStripGroups)
  vtxRemove.RemoveElements(CHILD_AT(vtxStripGroup, pStrip, 0),
                           vtxStripGroup->numStrips);
  ITERATE_CHILDREN(OptimizedModel::StripHeader_t, vtxStrip, vtxStripGroup,
                   pStrip, numStrips)
  vtxRemove.RemoveElements(CHILD_AT(vtxStrip, pBoneStateChange, 0),
                           vtxStrip->numBoneStateChanges);
  ITERATE_END
  ITERATE_END
  ITERATE_END
  ...
  // We rely on the fact that vertex and index buffers are laid out as one solid
  // memory block for all lods.
  ITERATE_CHILDREN(OptimizedModel::ModelLODHeader_t, vtxLod, vtxModel, pLOD,
                   numLODs)
  ITERATE_CHILDREN(OptimizedModel::MeshHeader_t, vtxMesh, vtxLod, pMesh,
                   numMeshes)
  ...
  size_t offIndexEnd = BYTE_DIFF_PTR(
      vtxHdr, CHILD_AT(vtxStripGroup, pIndex, vtxStripGroup->numIndices));
  size_t offVertex = BYTE_DIFF_PTR(vtxHdr, CHILD_AT(vtxStripGroup, pVertex, 0));
  size_t offVertexEnd = BYTE_DIFF_PTR(
      vtxHdr, CHILD_AT(vtxStripGroup, pVertex, vtxStripGroup->numVerts));

  if (offIndex < vtxOffIndexBuffer) vtxOffIndexBuffer = offIndex;
  if (offIndexEnd > vtxOffIndexBufferEnd) vtxOffIndexBufferEnd = offIndexEnd;
  if (offVertex < vtxOffVertexBuffer) vtxOffVertexBuffer = offVertex;
  ...
  DECLARE_PTR(OptimizedModel::Vertex_t, vtxVertexBuffer,
              BYTE_OFF_PTR(vtxHdr, vtxOffVertexBuffer));
  DECLARE_PTR(OptimizedModel::Vertex_t, vtxVertexBufferEnd,
              BYTE_OFF_PTR(vtxHdr, vtxOffVertexBufferEnd));
  CUtlVector<int> vtxIndexDeltas;
  vtxIndexDeltas.EnsureCapacity(vtxVertexBufferEnd - vtxVertexBuffer);
  int vtxNumVertexRemoved = 0;
  for (OptimizedModel::Vertex_t *vtxVertexElement = vtxVertexBuffer;
       vtxVertexElement < vtxVertexBufferEnd; ++vtxVertexElement) {
    ...
    if (!bUsed) {
      ...
      vtxNumVertexRemoved++;
    } else {  // Index is in use and must be remapped
      // Find the mesh where this index belongs
      int iMesh = arrMdlOffsets.FindLessOrEqual(
          MdlRangeItem(0, 0, vtxVertexElement - vtxVertexBuffer));
      Assert(iMesh >= 0 && iMesh < arrMdlOffsets.Count());

      MdlRangeItem &mri = arrMdlOffsets[iMesh];
      Assert(
          (vtxVertexElement - vtxVertexBuffer >= mri.m_offNew) &&
          (vtxVertexElement - vtxVertexBuffer < mri.m_offNew + mri.m_numNew));

      Assert(
          m_vtxVerts.IsBitSet(vtxVertexElement->origMeshVertID + mri.m_offOld));
      vtxVertexElement->origMeshVertID =
          srcIndices.Find(vtxVertexElement->origMeshVertID + mri.m_offOld) -
          mri.m_offNew;
  ...
  DECLARE_PTR(unsigned short, vtxIndexBuffer,
              BYTE_OFF_PTR(vtxHdr, vtxOffIndexBuffer));
  DECLARE_PTR(unsigned short, vtxIndexBufferEnd,
              BYTE_OFF_PTR(vtxHdr, vtxOffIndexBufferEnd));
  for (unsigned short *vtxIndexElement = vtxIndexBuffer;
       vtxIndexElement < vtxIndexBufferEnd; ++vtxIndexElement) {
  ...
  ITERATE_CHILDREN(OptimizedModel::MaterialReplacementHeader_t, vtxMat,
                   vtxMatList, pMaterialReplacement, numReplacements)
  vtxMat->replacementMaterialNameOffset =
      vtxRemove.ComputeOffset(vtxMat, vtxMat->replacementMaterialNameOffset);
  ITERATE_END
  vtxMatList->replacementOffset =
      vtxRemove.ComputeOffset(vtxMatList, vtxMatList->replacementOffset);
  ITERATE_END
  ITERATE_CHILDREN(OptimizedModel::BodyPartHeader_t, vtxBodyPart, vtxHdr,
                   pBodyPart, numBodyParts)
  ITERATE_CHILDREN(OptimizedModel::ModelHeader_t, vtxModel, vtxBodyPart, pModel,
  ...
  vtxStrip->indexOffset =
      vtxRemove.ComputeOffset(
          vtxStripGroup, vtxStripGroup->indexOffset + vtxStrip->indexOffset) -
      vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->indexOffset);
  vtxStrip->vertOffset =
      vtxRemove.ComputeOffset(
          vtxStripGroup, vtxStripGroup->vertOffset + vtxStrip->vertOffset) -
      vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->vertOffset);
  vtxStrip->boneStateChangeOffset =
      vtxRemove.ComputeOffset(vtxStrip, vtxStrip->boneStateChangeOffset);
  ITERATE_END

  vtxStripGroup->vertOffset =
      vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->vertOffset);
  vtxStripGroup->indexOffset =
      vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->indexOffset);
  vtxStripGroup->stripOffset =
      vtxRemove.ComputeOffset(vtxStripGroup, vtxStripGroup->stripOffset);
  ITERATE_END
  vtxMesh->stripGroupHeaderOffset =
      vtxRemove.ComputeOffset(vtxMesh, vtxMesh->stripGroupHeaderOffset);
  ITERATE_END
  ...
  vtxModel->numLODs = 1;
  ITERATE_END
  vtxBodyPart->modelOffset =
      vtxRemove.ComputeOffset(vtxBodyPart, vtxBodyPart->modelOffset);
  ITERATE_END
  vtxHdr->materialReplacementListOffset =
      vtxRemove.ComputeOffset(vtxHdr, vtxHdr->materialReplacementListOffset);
  vtxHdr->bodyPartOffset =
      vtxRemove.ComputeOffset(vtxHdr, vtxHdr->bodyPartOffset);
  vtxHdr->numLODs = 1;

  // Perform final memory move
  vtxRemove.MemMove(vtxHdr, vtxLength);

  vtxBuffer.SeekPut(CUtlBuffer::SEEK_CURRENT,
                    vtxBuffer.TellGet() + vtxLength - vtxBuffer.TellPut());
