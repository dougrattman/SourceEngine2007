...
//
bool CMdlLib::StripModelBuffers(CUtlBuffer &mdlBuffer, CUtlBuffer &vvdBuffer,
                                CUtlBuffer &vtxBuffer,
                                IMdlStripInfo **ppStripInfo) {
  DECLARE_PTR(u8, mdl, BYTE_OFF_PTR(mdlBuffer.Base(), mdlBuffer.TellGet()));
  DECLARE_PTR(u8, vvd, BYTE_OFF_PTR(vvdBuffer.Base(), vvdBuffer.TellGet()));
  DECLARE_PTR(u8, vtx, BYTE_OFF_PTR(vtxBuffer.Base(), vtxBuffer.TellGet()));
  ...
  //
  // ===================
  // =================== Modify the checksum and check if further processing is
  // needed
  // ===================
  //
  ...
  ITERATE_CHILDREN2(OptimizedModel::BodyPartHeader_t, mstudiobodyparts_t,
                    vtxBodyPart, mdlBodyPart, vtxHdr, mdlHdr, pBodyPart,
                    pBodypart, numBodyParts)
  ITERATE_CHILDREN2(OptimizedModel::ModelHeader_t, mstudiomodel_t, vtxModel,
                    mdlModel, vtxBodyPart, mdlBodyPart, pModel, pModel,
                    numModels)

  OptimizedModel::ModelLODHeader_t *vtxLod = CHILD_AT(vtxModel, pLOD, 0);
  ITERATE_CHILDREN2(OptimizedModel::MeshHeader_t, mstudiomesh_t, vtxMesh,
                    mdlMesh, vtxLod, mdlModel, pMesh, pMesh, numMeshes)
  ...
  if (!(vtxStrip->flags & OptimizedModel::STRIP_IS_TRILIST)) continue;
  for (int i = 0; i < vtxStrip->numIndices; ++i) {
    unsigned short *vtxIdx =
        CHILD_AT(vtxStripGroup, pIndex, vtxStrip->indexOffset + i);
    OptimizedModel::Vertex_t *vtxVertex =
        CHILD_AT(vtxStripGroup, pVertex, *vtxIdx);
  ...
  //
  // Now having the table of which vertexes to keep we will construct a
  // remapping table
  //
  ...
  //  - for every index we know if it belongs to lod0 "mapVtxIndex.IsBitSet(
  //  oldVertIdx )"
  //  - for every new vertex we know its old index "srcIndices[ newVertIdx ]"
  //  - for every old vertex if it's in lod0 we know its new index
  //  "srcIndices.Find( oldVertIdx )"
  ...
  for (OptimizedModel::Vertex_t *vtxVertexElement = vtxVertexBuffer;
    ...
    if (!bUsed) {
      ...
      int iMesh = arrMdlOffsets.FindLessOrEqual(CMdlStripInfo::MdlRangeItem(
          0, 0, vtxVertexElement - vtxVertexBuffer));
      ...
      Assert(mapVtxIndex.IsBitSet(vtxVertexElement->origMeshVertID +
                                  mri.m_offOld));
