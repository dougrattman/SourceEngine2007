...
#ifndef SOURCE_MDLLIB_UTILS_H_
#define SOURCE_MDLLIB_UTILS_H_
...
// Declare a pointer and automatically do the cast of initial value to the
// pointer type
...
#define BYTE_OFF_PTR(initval, offval) (((u8 *)(initval)) + (offval))

// Compute difference in bytes between two pointers
#define BYTE_DIFF_PTR(begin, end) (((u8 *)(end)) - ((u8 *)(begin)))
...
#define ITERATE_CHILDREN2(type, type2, name, name2, parent, parent2, accessor, \
                          accessor2, count)                                    \
  for (int name##_idx = 0; name##_idx < (parent)->count; ++name##_idx) {       \
    type *name = (parent)->accessor(name##_idx);                               \
...
//
// CInsertionTracker
//	Class that is tracking insertions that are scheduled to happen at given
// points. 	Use policy: 		InsertBytes / InsertElements	[*]
//-- schedule insertions 		Finalize
//-- finalize insertion information 		ComputePointer / ComputeOffset
//[*]		-- compute new pointers/offsets that will happen after
// insertions
//		MemMove
//--  perform memory moves to apply insertions
//
class CInsertionTracker {
 public:
  CInsertionTracker() : m_map(DefLessFunc(u8 *)) {}
 ...
 public:
  ...
  void InsertElements(T *ptr, int count = 1) {
    InsertBytes((u8 *)ptr, count * sizeof(T));
  }
 ...
 public:
  ...
  // Perform memory moves, the buffer should be large enough to accommodate
  // inserted bytes
 public:
  void MemMove(void *ptrBase, int &length) const;

 protected:
  typedef CUtlMap<u8 *, int, unsigned int> Map;
  Map m_map;  // pos -> length
};

//
// CRemoveTracker
//	Class that is tracking removals that are scheduled to happen at given
// points. 	Use policy: 		RemoveBytes / RemoveElements	[*]
//-- schedule removals
//		Finalize
//--  finalize removal information 		ComputePointer / ComputeOffset
//[*]		-- compute new  pointers/offsets that will happen after removals
// MemMove
//-- perform memory moves to apply removals
//
class CRemoveTracker {
 public:
  CRemoveTracker() : m_map(DefLessFunc(u8 *)) {}
 ...
 public:
  ...
  void RemoveElements(T *ptr, int count = 1) {
    RemoveBytes((u8 *)ptr, count * sizeof(T));
  }
...
class CGrowableBitVec : public CVarBitVec {
 public:
  void GrowSetBit(int iBit) {
    if (iBit >= GetNumBits()) Resize(iBit + 1, false);
    Set(iBit);
  }
...
class CGrowableVector : public CUtlVector<T> {
 public:
  T &operator[](int idx) {
    while (idx >= this->Count()) this->AddToTail(T());
    return this->CUtlVector<T>::operator[](idx);
  }
};

#endif  // SOURCE_MDLLIB_UTILS_H_
