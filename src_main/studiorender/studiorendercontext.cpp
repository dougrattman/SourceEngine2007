...
//-----------------------------------------------------------------------------
void StudioChangeCallback(IConVar *var, const char *pOldValue,
                          float flOldValue) {
...
static ConVar studio_queue_mode("studio_queue_mode", "1", 0, "",
                                StudioChangeCallback);
...
CStudioRenderContext::CStudioRenderContext() {
  ...
  m_FlexWeights[0].Init(2 * sizeof(float) * MAXSTUDIOFLEXDESC,
                        512 * sizeof(float));
  m_FlexWeights[1].Init(2 * sizeof(float) * MAXSTUDIOFLEXDESC,
                        512 * sizeof(float));
  m_nCurrentStack = 0;
}
...
bool CStudioRenderContext::Connect(CreateInterfaceFn factory) {
  ...
  g_pStudioDataCache =
      (IStudioDataCache *)factory(STUDIO_DATA_CACHE_INTERFACE_VERSION, NULL);
  if (!g_pMaterialSystem || !g_pMaterialSystemHardwareConfig ||
      !g_pStudioDataCache) {
    Msg("StudioRender failed to connect to a required system\n");
  }
  return (g_pMaterialSystem && g_pMaterialSystemHardwareConfig &&
          g_pStudioDataCache);
}
...
//-----------------------------------------------------------------------------
matrix3x4_t *CStudioRenderContext::CreateQueuedMatrices(
    int nMatrixCount, const matrix3x4_t *pBoneToWorld) {
...
//-----------------------------------------------------------------------------
float *CStudioRenderContext::CreateQueuedFlexWeights(int nWeightCount,
                                                     float *pWeights) {
...
//-----------------------------------------------------------------------------
void *CStudioRenderContext::CreateQueuedShadowData(int nSizeInBytes,
                                                   void *pSrc) {
  MEM_ALLOC_CREDIT_("CStudioRenderContext::m_FlexWeights");
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::ComputeMaterialFlags(studiohdr_t *phdr,
  ...
  if (pMaterial->NeedsPowerOfTwoFrameBufferTexture(false))  // The false checks
                                                            // if it will ever
                                                            // need the frame
                                                            // buffer, not just
                                                            // this frame
  ...
  if (pBumpMatVar && pBumpMatVar->IsDefined() &&
      pMaterial->NeedsTangentSpace()) {
  ...
  if (pPhongMatVar && pPhongMatVar->IsDefined() &&
      (pPhongMatVar->GetIntValue() != 0)) {
    phdr->flags |= STUDIOHDR_FLAGS_USES_BUMPMAPPING;
  }
}
...
//-----------------------------------------------------------------------------
static const char *GetTextureName(studiohdr_t *phdr,
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::LoadMaterials(
    studiohdr_t *phdr, OptimizedModel::FileHeader_t *pVtxHeader,
  ...
  for (i = 0; i < phdr->numtextures; i++) {
    ...
    for (j = 0; j < phdr->numcdtextures && IsErrorMaterial(pMaterial); j++) {
      ...
      if (phdr->flags & STUDIOHDR_FLAGS_OBSOLETE) {
        pMaterial = g_pMaterialSystem->FindMaterial("models/obsolete/obsolete",
                                                    TEXTURE_GROUP_MODEL, false);
        if (IsErrorMaterial(pMaterial)) {
          Warning(
              "StudioRender: OBSOLETE material missing: "
              "\"models/obsolete/obsolete\"\n");
        }
      } else {
        pMaterial =
            g_pMaterialSystem->FindMaterial(szPath, TEXTURE_GROUP_MODEL, false);
      }
    }
    if (IsErrorMaterial(pMaterial)) {
      // hack - if it isn't found, go through the motions of looking for it
      // again so that the materialsystem will give an error.
      char szPrefix[256];
      ...
      for (j = 0; j < phdr->numcdtextures; j++) {
        ...
        g_pMaterialSystem->FindMaterial(szPath, TEXTURE_GROUP_MODEL, true,
                                        szPrefix);
      }
    }
...
//-----------------------------------------------------------------------------
static void SuppressAllHWMorphs(mstudiomodel_t *pModel,
...
//-----------------------------------------------------------------------------
int CStudioRenderContext::CountFlexedVertices(
    mstudiomesh_t *pMesh, OptimizedModel::StripGroupHeader_t *pStripGroup) {
  if (!pMesh->numflexes) return 0;

  // an inverse mapping from mesh index to strip group index
  unsigned short *pMeshIndexToGroupIndex = (unsigned short *)_alloca(
      pMesh->pModel()->numvertices * sizeof(unsigned short));
  memset(pMeshIndexToGroupIndex, 0xFF,
         pMesh->pModel()->numvertices * sizeof(unsigned short));
  ...
  for (int i = 0; i < pMesh->numflexes; ++i) {
    ...
    uint8_t *pVAnim = pFlex->pBaseVertanim();
    int nVAnimSizeBytes = pFlex->VertAnimSizeBytes();
    for (int j = 0; j < pFlex->numverts; ++j) {
      mstudiovertanim_t *pAnim =
          (mstudiovertanim_t *)(pVAnim + j * nVAnimSizeBytes);
...
static int SortVertCount(const void *arg1, const void *arg2) {
  ...
  return s_pVertexCount[*(const int *)arg2] -
         s_pVertexCount[*(const int *)arg1];
}

#define MIN_HWMORPH_FLEX_COUNT 200

void CStudioRenderContext::DetermineHWMorphing(
    mstudiomodel_t *pModel, OptimizedModel::ModelLODHeader_t *pVtxLOD) {
  if (!g_pMaterialSystemHardwareConfig->HasFastVertexTextures()) return;

  // There is fixed cost to using HW morphing in the form of setting
  // rendertargets. Therefore if there is a low chance of there being enough
  // work, then do it in software.
  int nTotalFlexCount = ComputeTotalFlexCount(pModel);
  if (nTotalFlexCount == 0) return;
  ...
  // If we have less meshes than the most morphs we can do in a batch, we're
  // done.
  int nMaxHWMorphBatchCount =
      g_pMaterialSystemHardwareConfig->MaxHWMorphBatchCount();
  bool bHWMorph = (pModel->nummeshes <= nMaxHWMorphBatchCount);
  if (bHWMorph) return;

  // If we have less flexed strip groups than the most we can do in a batch,
  // we're done.
  int nFlexedStripGroup = CountDeltaFlexedStripGroups(pModel, pVtxLOD);
  if (nFlexedStripGroup <= nMaxHWMorphBatchCount) return;
  ...
  // Bleah. Pretty lame. We should change StripGroupHeader_t to store the flex
  // vertex count
  int nIndex = 0;
  for (int k = 0; k < pModel->nummeshes; ++k) {
    Assert(pModel->nummeshes == pVtxLOD->numMeshes);
...
//-----------------------------------------------------------------------------
// Adds a vertex to the meshbuilder.  Returns false if boneweights did not sum
// to 1.0
//-----------------------------------------------------------------------------
template <VertexCompressionType_t T>
bool CStudioRenderContext::R_AddVertexToMesh(
    const char *pModelName, bool bNeedsTangentSpace, CMeshBuilder &meshBuilder,
    OptimizedModel::Vertex_t *pVertex, mstudiomesh_t *pMesh,
  ...
  if (hwSkin) {
    ...
    // NOTE: We use pVertex->numbones because that's the number of bones
    // actually influencing this vertex. Note that pVertex->numBones is not
    // necessary the *desired* # of bones influencing this vertex; we could have
    // collapsed some of those bones out. pBoneWeight->numbones stures the
    // desired #
    float totalWeight = 0;
    ...
    // The only way we should not add up to 1 is if there's more than 3
    // *desired* bones
    // and more than 1 *actual* bone (we can have 0	vertex bones in the case
    // of static props
    if ((pVertex->numBones > 0) && (pBoneWeight->numbones <= 3) &&
        fabs(totalWeight - 1.0f) > 1e-3) {
    ...
    // are set correctly, even though the last bone weight is computed in a
    // shader program
    for (i = 0; i < pVertex->numBones; ++i) {
  ...
  // Set all the weights at once (the meshbuilder performs additional,
  // post-compression, normalization):
...
// Builds the group
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioBuildMeshGroup(
    const char *pModelName, bool bNeedsTangentSpace,
  ...
  pMeshGroup->m_pMesh = pRenderContext->CreateStaticMesh(
      vertexFormat, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_MODELS);
  ...
  meshBuilder.Begin(pMeshGroup->m_pMesh, MATERIAL_HETEROGENOUS,
                    hwSkin ? pStripGroup->numVerts : 0,
                    pStripGroup->numIndices);

  int i;
  bool bBadBoneWeights = false;
  if (hwSkin) {
    const mstudio_meshvertexdata_t *vertData =
        GetFatVertexData(pMesh, pStudioHdr);
    Assert(vertData);
    ...
    for (i = 0; i < pStripGroup->numVerts; ++i) {
      ...
      switch (compressionType) {
        case VERTEX_COMPRESSION_ON:
          success = R_AddVertexToMesh<VERTEX_COMPRESSION_ON>(
              pModelName, bNeedsTangentSpace, meshBuilder,
              pStripGroup->pVertex(i), pMesh, vertData, hwSkin);
          break;
        case VERTEX_COMPRESSION_NONE:
        default:
          success = R_AddVertexToMesh<VERTEX_COMPRESSION_NONE>(
              pModelName, bNeedsTangentSpace, meshBuilder,
              pStripGroup->pVertex(i), pMesh, vertData, hwSkin);
          break;
      }
  ...
  for (i = 0; i < pStripGroup->numStrips; ++i) {
    ...
    if (pStripGroup->pStrip(i)->flags & OptimizedModel::STRIP_IS_TRISTRIP) {
      ...
      while (curr != end) {
        ...
        if (idx != last[0] && idx != last[1] && last[0] != last[1] &&
            last[0] != -1)
          ++numUnique;
        last[0] = last[1];
        last[1] = idx;
...
// Builds the group
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioBuildMorph(
    studiohdr_t *pStudioHdr, studiomeshgroup_t *pMeshGroup,
    mstudiomesh_t *pMesh, OptimizedModel::StripGroupHeader_t *pStripGroup) {
  ...
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  MorphFormat_t morphType =
      MORPH_POSITION | MORPH_NORMAL | MORPH_SPEED | MORPH_SIDE;
  for (int i = 0; i < pMesh->numflexes; ++i) {
    ...
    for (int j = 0; j < pFlex->numverts; ++j) {
      ...
      if (pFlex->vertanimtype == STUDIO_VERT_ANIM_WRINKLE) {
        mstudiovertanim_wrinkle_t *pWrinkleAnim =
            static_cast<mstudiovertanim_wrinkle_t *>(pAnim);
...
void CStudioRenderContext::R_StudioBuildMeshStrips(
  ...
  pMeshGroup->m_pStripData =
      (OptimizedModel::StripHeader_t *)malloc(stripDataSize);
  ...
  for (i = 0; i < pStripGroup->numStrips; ++i) {
    ...
    // Fixup the bone state change offset, since we have it right after the
    // strip data
    pMeshGroup->m_pStripData[i].boneStateChangeOffset =
        boneStateChangeOffset - i * sizeof(OptimizedModel::StripHeader_t);
    ...
    if (boneWeightSize != 0) {
      unsigned char *pBoneStateChange =
          (unsigned char *)pMeshGroup->m_pStripData + boneStateChangeOffset;
      memcpy(pBoneStateChange, pStripGroup->pStrip(i)->pBoneStateChange(0),
             boneWeightSize);
...
//-----------------------------------------------------------------------------
int CStudioRenderContext::GetNumBoneWeights(
    const OptimizedModel::StripGroupHeader_t *pGroup) {
...
//-----------------------------------------------------------------------------
// Determine an actual model vertex format for a mesh based on its material
// usage. Bypasses the homegenous model vertex format in favor of the actual
// format. Ideally matches 1:1 the shader's data requirements without any bloat.
//-----------------------------------------------------------------------------
VertexFormat_t CStudioRenderContext::CalculateVertexFormat(
    const studiohdr_t *pStudioHdr, const studioloddata_t *pStudioLodData,
    const mstudiomesh_t *pMesh, OptimizedModel::StripGroupHeader_t *pGroup,
    bool bIsHwSkinned) {
  ...
  if (bIsDX7) {
    ...
    for (int i = 0; i < pStudioHdr->numskinfamilies; i++) {
      ...
      //        Ep2, but should fix it ASAP afterwards. To make catching such
      //        errors easier, we should Assert in draw calls that the
      //        vertexdecl matches vertex shader inputs (note that D3D debug
      //        DLLs will do that on PC, though it's not as informative as if we
      //        do it ourselves). So, in the absence of reliable material vertex
      //        formats, use the old 'standard' elements (we can still omit
      //        skinning data - and COLOR for DX8+, where it should come from
      //        the second static lighting stream):
      VertexFormat_t vertexFormat =
          bIsDX7 ? MATERIAL_VERTEX_FORMAT_MODEL_DX7
                 : (MATERIAL_VERTEX_FORMAT_MODEL & ~VERTEX_COLOR);
      ...
      {
              bool bFound = false;
              IMaterialVar *pEnvmapMatVar = pMaterial->FindVar( "$envmap",
      &bFound, false ); if ( bFound && pEnvmapMatVar->IsDefined() )
              {
                      IMaterialVar *pBumpMatVar = pMaterial->FindVar(
      "$bumpmap", &bFound, false ); if ( bFound && pBumpMatVar->IsDefined() )
                      {
                              bBumpmapping = true;
                      }
              }
    ...
    // Add skinning elements for non-rigid models (with more than one bone
    // weight)
    if (bSkinnedMesh) {
      if (nBoneWeights > 0) {
...
bool CStudioRenderContext::MeshNeedsTangentSpace(
    studiohdr_t *pStudioHdr, studioloddata_t *pStudioLodData,
    mstudiomesh_t *pMesh) {
  // iterate each skin table
  ...
  for (int i = 0; i < pStudioHdr->numskinfamilies; i++) {
    IMaterial *pMaterial =
        pStudioLodData->ppMaterials[pSkinref[pMesh->material]];
    Assert(pMaterial);
    if (!pMaterial) {
      continue;
    }
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioCreateSingleMesh(
    studiohdr_t *pStudioHdr, studioloddata_t *pStudioLodData,
  ...
  bool bNeedsTangentSpace =
      MeshNeedsTangentSpace(pStudioHdr, pStudioLodData, pMesh);

  // Each strip group represents a locking group, it's a set of vertices
  // that are locked together, and, potentially, software light + skinned
  // together
  ...
  for (int i = 0; i < pVtxMesh->numStripGroups; ++i) {
    ...
    bool bIsHwSkinned =
        !!(pStripGroup->flags & OptimizedModel::STRIPGROUP_IS_HWSKINNED);
    if (bIsHwSkinned) {
      pMeshGroup->m_Flags |= MESHGROUP_IS_HWSKINNED;
    }

    // get the minimal vertex format for this mesh
    VertexFormat_t vertexFormat = CalculateVertexFormat(
        pStudioHdr, pStudioLodData, pMesh, pStripGroup, bIsHwSkinned);

    // Build the vertex + index buffers
    R_StudioBuildMeshGroup(pStudioHdr->pszName(), bNeedsTangentSpace,
                           pMeshGroup, pStripGroup, pMesh, pStudioHdr,
                           vertexFormat);
    ...
    pMeshGroup->m_pGroupIndexToMeshIndex =
        new unsigned short[pStripGroup->numVerts + PREFETCH_VERT_COUNT];
    pMeshGroup->m_NumVertices = pStripGroup->numVerts;

    int j;
    for (j = 0; j < pStripGroup->numVerts; ++j) {
      pMeshGroup->m_pGroupIndexToMeshIndex[j] =
          pStripGroup->pVertex(j)->origMeshVertID;
    }

    // Extra copies are for precaching...
    for (j = pStripGroup->numVerts;
         j < pStripGroup->numVerts + PREFETCH_VERT_COUNT; ++j) {
      pMeshGroup->m_pGroupIndexToMeshIndex[j] =
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioCreateStaticMeshes(
    studiohdr_t *pStudioHdr, OptimizedModel::FileHeader_t *pVtxHdr,
    studiohwdata_t *pStudioHWData, int nLodID, int *pColorMeshID) {
  int i, j, k;

  Assert(pStudioHdr && pVtxHdr && pStudioHWData);

  pStudioHWData->m_pLODs[nLodID].m_pMeshData =
      new studiomeshdata_t[pStudioHWData->m_NumStudioMeshes];
  ...
  for (i = 0; i < pStudioHdr->numbodyparts; i++) {
    ...
    for (j = 0; j < pBodyPart->nummodels; ++j) {
      ...
      if (g_VBAllocTracker) {
        if ((pStudioHdr->numbones > 8) || (pStudioHdr->numflexdesc > 0)) {
          g_VBAllocTracker->TrackMeshAllocations(
              "R_StudioCreateStaticMeshes (character)");
        } else {
          if (pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) {
            g_VBAllocTracker->TrackMeshAllocations(
                "R_StudioCreateStaticMeshes (prop_static)");
          } else {
            g_VBAllocTracker->TrackMeshAllocations(
                "R_StudioCreateStaticMeshes (prop_dynamic)");
          }
        }
      }
      ...
      for (k = 0; k < pModel->nummeshes; ++k) {
        ...
        R_StudioCreateSingleMesh(
            pStudioHdr, &pStudioHWData->m_pLODs[nLodID], pMesh, pVtxMesh,
            pVtxHdr->maxBonesPerVert,
            &pStudioHWData->m_pLODs[nLodID].m_pMeshData[pMesh->meshid],
            pColorMeshID);
      }
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::R_StudioDestroyStaticMeshes(
    int numStudioMeshes, studiomeshdata_t **ppStudioMeshes) {
  if (!*ppStudioMeshes) return;
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::BuildDecalBoneMap(
    studiohdr_t *pStudioHdr, int *pUsedBones, int *pBoneRemap,
    int *pMaxBoneCount, mstudiomesh_t *pMesh,
    OptimizedModel::StripGroupHeader_t *pStripGroup) {
  ...
  for (int i = 0; i < pStripGroup->numVerts; ++i) {
    ...
    mstudioboneweight_t &boneWeight =
        pVertData->Vertex(nMeshVert)->m_BoneWeights;
    int nBoneCount = boneWeight.numbones;
    for (int j = 0; j < nBoneCount; ++j) {
      if (boneWeight.weight[j] == 0.0f) continue;

      if (pBoneRemap[boneWeight.bone[j]] >= 0) continue;
...
// under
//-----------------------------------------------------------------------------
void CStudioRenderContext::ComputeHWMorphDecalBoneRemap(
    studiohdr_t *pStudioHdr, OptimizedModel::FileHeader_t *pVtxHdr,
    studiohwdata_t *pStudioHWData, int nLOD) {
  if (pStudioHdr->numbones == 0) return;

  // Remaps sw bones to hw bones during decal rendering
  // NOTE: Only bones affecting vertices which have hw flexes will be add to
  // this map.
    ...
    for (int j = 0; j < pBodyPart->nummodels; ++j) {
      ...
      for (int k = 0; k < pModel->nummeshes; ++k) {
        ...
        for (int l = 0; l < pVtxMesh->numStripGroups; ++l) {
          ...
          if (!pMeshGroup->m_pMorph) continue;

          OptimizedModel::StripGroupHeader_t *pStripGroup =
              pVtxMesh->pStripGroup(l);
          BuildDecalBoneMap(pStudioHdr, &nUsedBones, pBoneRemap, &nMaxBoneCount,
                            pMesh, pStripGroup);
        }
      }
    }
  ...
  if (nUsedBones > 1) {
    if (nUsedBones >
        g_pMaterialSystemHardwareConfig->MaxVertexShaderBlendMatrices()) {
      Warning(
          "Hardware morphing of decals will be busted! Too many unique bones "
          "on flexed vertices!\n");
    }
...
//-----------------------------------------------------------------------------
bool CStudioRenderContext::LoadModel(studiohdr_t *pStudioHdr, void *pVtxBuffer,
  ...
  OptimizedModel::FileHeader_t *pVertexHdr =
  ...
  memset(pStudioHWData->m_pLODs, 0,
         pVertexHdr->numLODs * sizeof(studioloddata_t));
  ...
  int nColorMeshID = 0;
  int nLodID;
  for (nLodID = pStudioHWData->m_RootLOD; nLodID < pStudioHWData->m_NumLODs;
       nLodID++) {
    // Load materials and determine material dependent mesh requirements
    LoadMaterials(pStudioHdr, pVertexHdr, pStudioHWData->m_pLODs[nLodID],
                  nLodID);

    // build the meshes
    R_StudioCreateStaticMeshes(pStudioHdr, pVertexHdr, pStudioHWData, nLodID,
                               &nColorMeshID);
    ...
    pStudioHWData->m_pLODs[nLodID].m_SwitchPoint =
        pVertexHdr->pBodyPart(0)->pModel(0)->pLOD(nLodID)->switchPoint;
...
void CStudioRenderContext::UnloadModel(studiohwdata_t *pHardwareData) {
  ...
  for (i = pHardwareData->m_RootLOD; i < pHardwareData->m_NumLODs; i++) {
    R_StudioDestroyStaticMeshes(pHardwareData->m_NumStudioMeshes,
                                &pHardwareData->m_pLODs[i].m_pMeshData);
  }
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::RefreshStudioHdr(studiohdr_t *pStudioHdr,
                                            studiohwdata_t *pHardwareData) {}

//-----------------------------------------------------------------------------
// Set the eye view target
//-----------------------------------------------------------------------------
void CStudioRenderContext::SetEyeViewTarget(const studiohdr_t *pStudioHdr,
                                            int nBodyIndex,
...
float CStudioRenderContext::GetLODSwitchValue(
    const studiohwdata_t &hardwareData, int nLOD) const {
  return hardwareData.m_pLODs[nLOD].m_SwitchPoint;
}

void CStudioRenderContext::SetLODSwitchValue(studiohwdata_t &hardwareData,
                                             int nLOD, float flSwitchValue) {
...
// for all lods.
//-----------------------------------------------------------------------------
int CStudioRenderContext::GetMaterialList(studiohdr_t *pStudioHdr, int count,
  ...
  for (i = 0; i < pStudioHdr->numtextures; i++) {
    ...
    // is found
    for (j = 0; j < pStudioHdr->numcdtextures && IsErrorMaterial(pMaterial);
         j++) {
    ...
    if (found < count) {
      int k;
      for (k = 0; k < found; k++) {
        if (ppMaterials[k] == pMaterial) break;
      }
      if (k >= found) {
        // add uniquely
...
int CStudioRenderContext::GetMaterialListFromBodyAndSkin(
  ...
  if (pStudioHWData == NULL) return 0;
  ...
  for (int lodID = pStudioHWData->m_RootLOD; lodID < pStudioHWData->m_NumLODs;
    ...
    for (int i = 0; i < pStudioHdr->numbodyparts; i++) {
      ...
      for (int k = 0; k < pModel->nummeshes; ++k) {
        ...
        int m;
        for (m = 0; m < found; m++) {
          if (ppOutputMaterials[m] == pMaterial) break;
        }
        if (m >= found) {
          // add uniquely
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::GetPerfStats(DrawModelResults_t *pResults,
                                        const DrawModelInfo_t &info,
                                        CUtlBuffer *pSpewBuf) const {
  pResults->m_ActualTriCount = pResults->m_TextureMemoryBytes = 0;
  pResults->m_Materials.RemoveAll();

  Assert(info.m_Lod >= 0);
  if (info.m_Lod < 0 || !info.m_pHardwareData->m_pLODs) return;

  studiomeshdata_t *pStudioMeshes =
      info.m_pHardwareData->m_pLODs[info.m_Lod].m_pMeshData;

  // Set up an array that keeps up with the number of used hardware bones in the
  // models.
  ...
  for (i = 0; i < info.m_pStudioHdr->numbodyparts; i++) {
    ...
    for (k = 0; k < pModel->nummeshes; ++k) {
      ...
      int numPasses = m_RC.m_pForcedMaterial
                          ? m_RC.m_pForcedMaterial->GetNumPasses()
                          : pMaterial->GetNumPasses();
      if (pSpewBuf) {
      ...
      if (pSpewBuf) {
        pSpewBuf->Printf(
            "        texture memory: %d (Only valid in a rendering app)\n",
            bytes);
      }

      // Iterate over all stripgroups
      int stripGroupID;
      for (stripGroupID = 0; stripGroupID < pMeshData->m_NumGroup;
           stripGroupID++) {
        ...
        for (stripID = 0; stripID < pMeshGroup->m_NumStrips; stripID++) {
          ...
          if (bIsHWSkinned) {
            // Only count bones as hardware bones if we are using hardware
            // skinning here.
            int boneID;
            for (boneID = 0; boneID < pStripData->numBoneStateChanges;
                 boneID++) {
              OptimizedModel::BoneStateChangeHeader_t *pBoneStateChange =
                  pStripData->pBoneStateChange(boneID);
...
void CStudioRenderContext::BeginFrame(void) {
  ...
  m_RC.m_Config.m_bSupportsVertexAndPixelShaders =
      g_pMaterialSystemHardwareConfig->SupportsVertexAndPixelShaders();
  m_RC.m_Config.m_bSupportsOverbright =
      g_pMaterialSystemHardwareConfig->SupportsOverbright();
...
// Material overrides
//-----------------------------------------------------------------------------
void CStudioRenderContext::ForcedMaterialOverride(
    IMaterial *newMaterial, OverrideType_t nOverrideType) {
...
void CStudioRenderContext::SetLocalLights(int nLightCount,
  ...
  if (m_RC.m_Config.bSoftwareLighting || m_RC.m_NumLocalLights == 0) {
    render_context->DisableAllLocalLights();
  } else {
    int i;
    ...
    for (; i < max_light_count; i++) {
      ...
      render_context->SetLight(i, desc);
    }
  }
}
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::LockFlexWeights(int nWeightCount,
                                           float **ppFlexWeights,
                                           float **ppFlexDelayedWeights) {
  MEM_ALLOC_CREDIT_("CStudioRenderContext::m_FlexWeights");
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::GenerateRandomFlexWeights(int nWeightCount,
                                                     float *pWeights,
                                                     float *pDelayedWeights) {
  int nRandomFlex = r_randomflex.GetInt();
  if (nRandomFlex <= 0) return;
...
void CStudioRenderContext::InvokeBindProxies(const DrawModelInfo_t &info) {
  if (m_RC.m_pForcedMaterial) {
    if (m_RC.m_nForcedMaterialType == OVERRIDE_NORMAL &&
        m_RC.m_pForcedMaterial->HasProxy()) {
  ...
  for (int i = 0; i < info.m_pStudioHdr->numbodyparts; ++i) {
    R_StudioSetupModel(i, info.m_Body, &pModel, info.m_pStudioHdr);
    for (int somethingOtherThanI = 0; somethingOtherThanI < pModel->nummeshes;
         ++somethingOtherThanI) {
      mstudiomesh_t *pMesh = pModel->pMesh(somethingOtherThanI);
      int nMaterialIndex = pSkinRef[pMesh->material];
      if (pProxyCalled[nMaterialIndex]) continue;
...
//-----------------------------------------------------------------------------
bool CStudioRenderContext::IsInternallyAllocated(
    const matrix3x4_t *pBoneToWorld) {
  int nBaseAddress = (int)m_BoneToWorldMatrices[m_nCurrentStack].GetBase();
  int nLastAddress =
      nBaseAddress + m_BoneToWorldMatrices[m_nCurrentStack].GetUsed();
  return ((int)pBoneToWorld >= nBaseAddress &&
          (int)pBoneToWorld < nLastAddress);
}
...
bool CStudioRenderContext::IsInternallyAllocated(const float *pFlexWeights) {
  if (pFlexWeights == s_pZeroFlexWeights) return true;
  ...
  return ((int)pFlexWeights >= nBaseAddress &&
          (int)pFlexWeights < nLastAddress);
}

//-----------------------------------------------------------------------------
// Draws a model
//-----------------------------------------------------------------------------
void CStudioRenderContext::DrawModel(DrawModelResults_t *pResults,
                                     const DrawModelInfo_t &info,
                                     matrix3x4_t *pBoneToWorld,
                                     float *pFlexWeights,
                                     float *pFlexDelayedWeights,
                                     const Vector &origin, int flags) {
  ...
  GenerateRandomFlexWeights(info.m_pStudioHdr->numflexdesc, pFlexWeights,
                            pFlexDelayedWeights);

  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  float flMetric;
  const_cast<DrawModelInfo_t *>(&info)->m_Lod =
      ComputeRenderLOD(pRenderContext, info, origin, &flMetric);
  if (pResults) {
  ...
  flex.m_pFlexDelayedWeights =
      pFlexDelayedWeights ? pFlexDelayedWeights : flex.m_pFlexWeights;
  ...
  if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
    ...
    if (!IsInternallyAllocated(pBoneToWorld)) {
      pBoneToWorld =
          CreateQueuedMatrices(info.m_pStudioHdr->numbones, pBoneToWorld);
    }
    if (info.m_pStudioHdr->numflexdesc != 0) {
      if (!IsInternallyAllocated(flex.m_pFlexWeights)) {
        flex.m_pFlexWeights = CreateQueuedFlexWeights(
            info.m_pStudioHdr->numflexdesc, flex.m_pFlexWeights);
        if (!pFlexDelayedWeights) {
          flex.m_pFlexDelayedWeights = flex.m_pFlexWeights;
        } else {
          flex.m_pFlexDelayedWeights = CreateQueuedFlexWeights(
              info.m_pStudioHdr->numflexdesc, flex.m_pFlexDelayedWeights);
        }
      }
    }
    pCallQueue->QueueCall(g_pStudioRenderImp, &CStudioRender::DrawModel, info,
                          m_RC, pBoneToWorld, flex, flags);
  }
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::DrawModelStaticProp(const DrawModelInfo_t &info,
                                               const matrix3x4_t &modelToWorld,
                                               int flags) {
  if (info.m_Lod < info.m_pHardwareData->m_RootLOD) {
    const_cast<DrawModelInfo_t *>(&info)->m_Lod =
        info.m_pHardwareData->m_RootLOD;
  }
  ...
  if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
    ...
    InvokeBindProxies(info);
    pCallQueue->QueueCall(g_pStudioRenderImp,
                          &CStudioRender::DrawModelStaticProp, info, m_RC,
                          modelToWorld, flags);
  }
}

void CStudioRenderContext::DrawStaticPropDecals(
    const DrawModelInfo_t &info, const matrix3x4_t &modelToWorld) {
  QUEUE_STUDIORENDER_CALL(DrawStaticPropDecals, CStudioRender,
                          g_pStudioRenderImp, info, m_RC, modelToWorld);
}

void CStudioRenderContext::DrawStaticPropShadows(
    const DrawModelInfo_t &info, const matrix3x4_t &modelToWorld, int flags) {
  QUEUE_STUDIORENDER_CALL(DrawStaticPropShadows, CStudioRender,
                          g_pStudioRenderImp, info, m_RC, modelToWorld, flags);
}
...
void CStudioRenderContext::AddShadow(IMaterial *pMaterial, void *pProxyData,
  ...
  ICallQueue *pCallQueue = pRenderContext->GetCallQueue();
  if (!pCallQueue || studio_queue_mode.GetInt() == 0) {
    g_pStudioRenderImp->AddShadow(pMaterial, pProxyData, pFlashlightState,
                                  pWorldToTexture, pFlashlightDepthTexture);
  } else {
    // NOTE: We don't need to make proxies work, because proxies are only ever
    // used when casting shadows onto props, which we don't do..that feature is
    // disabled. When casting flashlights onto mdls, which we *do* use, the
    // proxy is NULL.
    Assert(pProxyData == NULL);
    if (pProxyData != NULL) {
      Warning(
          "Cannot call CStudioRenderContext::AddShadows w/ proxies in queued "
          "mode!\n");
      return;
    }

    FlashlightState_t *pQueuedFlashlight =
        CreateQueuedShadowData(pFlashlightState);
    VMatrix *pQueuedMatrix = CreateQueuedShadowData(pWorldToTexture);
    pCallQueue->QueueCall(g_pStudioRenderImp, &CStudioRender::AddShadow,
                          pMaterial, (void *)NULL, pQueuedFlashlight,
                          pQueuedMatrix, pFlashlightDepthTexture);
  }
}
...
void CStudioRenderContext::DestroyDecalList(StudioDecalHandle_t handle) {
  QUEUE_STUDIORENDER_CALL(DestroyDecalList, CStudioRender, g_pStudioRenderImp,
                          handle);
}

void CStudioRenderContext::AddDecal(StudioDecalHandle_t handle,
                                    studiohdr_t *pStudioHdr,
                                    matrix3x4_t *pBoneToWorld, const Ray_t &ray,
                                    const Vector &decalUp,
                                    IMaterial *pDecalMaterial, float radius,
                                    int body, bool noPokethru,
                                    int maxLODToDecal) {
  Assert(((int)pBoneToWorld >=
          (int)m_BoneToWorldMatrices[m_nCurrentStack].GetBase()) &&
         ((int)pBoneToWorld <
          (int)m_BoneToWorldMatrices[m_nCurrentStack].GetBase() +
              m_BoneToWorldMatrices[m_nCurrentStack].GetUsed()));

  QUEUE_STUDIORENDER_CALL(AddDecal, CStudioRender, g_pStudioRenderImp, handle,
                          m_RC, pBoneToWorld, pStudioHdr, ray, decalUp,
                          pDecalMaterial, radius, body, noPokethru,
                          maxLODToDecal);
}
