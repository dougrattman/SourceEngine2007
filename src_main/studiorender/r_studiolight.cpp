...
//-----------------------------------------------------------------------------
int CopyLocalLightingState(int nMaxLights, LightDesc_t* pDest, int nLightCount,
                           const LightDesc_t* pSrc) {
...
//-----------------------------------------------------------------------------
void R_LightAmbient_4D(const Vector& normal, Vector4D* pLightBoxColor,
                       Vector& lv) {
  VectorScale(normal[0] > 0.f ? pLightBoxColor[0].AsVector3D()
                              : pLightBoxColor[1].AsVector3D(),
              normal[0] * normal[0], lv);
  VectorMA(lv, normal[1] * normal[1],
           normal[1] > 0.f ? pLightBoxColor[2].AsVector3D()
                           : pLightBoxColor[3].AsVector3D(),
           lv);
  VectorMA(lv, normal[2] * normal[2],
           normal[2] > 0.f ? pLightBoxColor[4].AsVector3D()
                           : pLightBoxColor[5].AsVector3D(),
           lv);
}
...
void R_LightAmbient_4D(const FourVectors& normal, Vector4D* pLightBoxColor,
  ...
  fltx4 DirectionalColor =
      OrSIMD(AndSIMD(ColorSelect1, NegMask), AndNotSIMD(NegMask, ColorSelect0));
...
//-----------------------------------------------------------------------------
void R_LightAmbient_3D(const Vector& normal, const Vector* pLightBoxColor,
                       Vector& lv) {
  VectorScale(normal[0] > 0.f ? pLightBoxColor[0] : pLightBoxColor[1],
              normal[0] * normal[0], lv);
  VectorMA(lv, normal[1] * normal[1],
           normal[1] > 0.f ? pLightBoxColor[2] : pLightBoxColor[3], lv);
  VectorMA(lv, normal[2] * normal[2],
           normal[2] > 0.f ? pLightBoxColor[4] : pLightBoxColor[5], lv);
}

//-----------------------------------------------------------------------------
// Set up light[i].dot, light[i].falloff, and light[i].delta for all lights
// given a vertex position "vert".
//-----------------------------------------------------------------------------
void R_LightStrengthWorld(const Vector& vert, int lightcount,
                          LightDesc_t* pDesc, lightpos_t* light) {
  //	VPROF( "R_LightStrengthWorld" );

  // NJS: note to self, maybe switch here based on lightcount, so multiple
  // squareroots can be done simeltaneously?
...
//-----------------------------------------------------------------------------
void R_WorldLightDelta(const LightDesc_t* wl, const Vector& org,
                       Vector& delta) {
  switch (wl->m_Type) {
...
TEMPLATE_FUNCTION_TABLE(void, R_LightEffectsWorldFunctionTable,
...
inline int R_LightEffectsWorldIndex(const LightDesc_t* pLightDesc,
                                    int nNumLights) {
  if (nNumLights > 4) {
    nNumLights = 4;
  }

  int nIndex = ((pLightDesc[0].m_Type & 0x3) << 6) |
               ((pLightDesc[1].m_Type & 0x3) << 4) |
               ((pLightDesc[2].m_Type & 0x3) << 2) |
               (pLightDesc[3].m_Type & 0x3);
...
TEMPLATE_FUNCTION_TABLE(float, R_WorldLightDistanceFalloffFunctionTable,
  ...
  if (wl->m_Range != 0.f) {
    if (dist2 > wl->m_Range * wl->m_Range) return 0.0f;
  }
...
float FASTCALL R_WorldLightDistanceFalloff(const LightDesc_t* wl,
  ...
  Assert(!(wl->m_Flags & ~(LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION0 |
                           ...
                           LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION2)));

  // calculate falloff
  int flags = wl->m_Flags & (LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION0 |
                             LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION1 |
                             LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION2);
...
fltx4 FASTCALL R_WorldLightDistanceFalloff(const LightDesc_t* wl,
  ...
  if (wl->m_Flags & LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION1) {
    fTotal = AddSIMD(
        fTotal, MulSIMD(ReplicateX4(wl->m_Attenuation1), SqrtEstSIMD(dist2)));
  }
  ...
  // Cull out light beyond this radius
  // now, zero out elements for which dist2 was > range^2. !!speed!! lights
  // should store dist^2 in sse format
  if (wl->m_Range != 0.f) {
...
int CStudioRender::R_LightGlintPosition(int index, const Vector& org,
                                        Vector& delta, Vector& intensity) {
...
void CStudioRender::R_InitLightEffectsWorld3() {
  ...
  int index =
      R_LightEffectsWorldIndex(m_pRC->m_LocalLights, m_pRC->m_NumLocalLights);
  R_LightEffectsWorld3 = R_LightEffectsWorldFunctionTable::functions[index];
}

//-----------------------------------------------------------------------------
// Performs lighting functions common to the ComputeLighting and
// ComputeLightingConstantDirectional returns the index of the
// LightEffectsWorldFunction to use
//-----------------------------------------------------------------------------
static int ComputeLightingCommon(const Vector* pAmbient, int lightCount,
                                 LightDesc_t* pLights, const Vector& pt,
                                 const Vector& normal, lightpos_t* pLightPos,
                                 Vector& lighting) {
...
//-----------------------------------------------------------------------------
void CStudioRenderContext::ComputeLighting(const Vector* pAmbient,
  ...
  int index = ComputeLightingCommon(pAmbient, lightCount, pLights, pt, normal,
                                    m_pLightPos, lighting);
  R_LightEffectsWorldFunctionTable::functions[index](pLights, m_pLightPos,
                                                     normal, lighting);
}
...
void CStudioRenderContext::ComputeLightingConstDirectional(
  ...
  int index = ComputeLightingCommon(pAmbient, lightCount, pLights, pt, normal,
                                    m_pLightPos, lighting);
  R_LightEffectsWorldFunctionTableConstDirectional::functions[index](
      pLights, m_pLightPos, normal, lighting, flDirectionalAmount);
}