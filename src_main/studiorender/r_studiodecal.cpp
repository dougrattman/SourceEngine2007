...
//-----------------------------------------------------------------------------
StudioDecalHandle_t CStudioRender::CreateDecalList(
  ...
  for (int i = 0; i < pHardwareData->m_NumLODs; i++) {
    m_DecalList[handle].m_pLod[i].m_FirstMaterial =
        m_DecalMaterial.InvalidIndex();
  }
...
void CStudioRender::DestroyDecalList(StudioDecalHandle_t hDecal) {
  if (hDecal == STUDIORENDER_DECAL_INVALID) return;
...
inline bool CStudioRender::IsFrontFacing(
    const Vector* pnorm, const mstudioboneweight_t* pboneweight) {
...
inline bool CStudioRender::TransformToDecalSpace(
    DecalBuildInfo_t& build, const Vector& pos,
  ...
  if (!build.m_NoPokeThru) return true;
...
//-----------------------------------------------------------------------------
bool CStudioRender::ProjectDecalOntoMesh(DecalBuildInfo_t& build,
  ...
  const mstudio_meshvertexdata_t* vertData =
      pMesh->GetVertexData(build.m_pStudioHdr);
  const thinModelVertices_t* thinVertData = NULL;

  if (!vertData) {
    // For most models (everything that's not got flex data), the vertex data is
    // 'thinned' on load to save memory
    thinVertData = pMesh->GetThinVertexData(build.m_pStudioHdr);
    if (!thinVertData) return false;
  }
  ...
  for (int j = 0; j < pMesh->numvertices; ++j) {
    ...
    if (!IsFrontFacing(vecNormal, boneWeights)) continue;

    pVertexInfo[j].m_Flags |= DecalBuildVertexInfo_t::FRONT_FACING;

    bool inValidArea = TransformToDecalSpace(build, *vecPosition, boneWeights,
                                             pVertexInfo[j].m_UV);
...
inline int CStudioRender::ComputeClipFlags(DecalBuildVertexInfo_t* pVertexInfo,
                                           int i) {
...
static int IntersectPlane(DecalClipState_t& state, int start, int end,
  ...
  float t =
      (val - GetVecTexCoord(startVert.m_TexCoord)[normalInd]) / dir[normalInd];
  ...
  out.m_Position[0] =
      startVert.m_Position[0] * (1.0 - t) + endVert.m_Position[0] * t;
  out.m_Position[1] =
      startVert.m_Position[1] * (1.0 - t) + endVert.m_Position[1] * t;
  out.m_Position[2] =
      startVert.m_Position[2] * (1.0 - t) + endVert.m_Position[2] * t;
  ...
  Vector2D vTexCoord;
  Vector2DLerp(GetVecTexCoord(startVert.m_TexCoord),
               GetVecTexCoord(endVert.m_TexCoord), t, vTexCoord);
...
void CStudioRender::ConvertMeshVertexToDecalVertex(DecalBuildInfo_t& build,
  ...
  if (build.m_pMeshVertexData) {
    VectorCopy(*build.m_pMeshVertexData->Position(meshIndex),
               decalVertex.m_Position);
    VectorCopy(*build.m_pMeshVertexData->Normal(meshIndex),
               GetVecNormal(decalVertex.m_Normal));
  } else {
    // At this point in the code, we should definitely have either compressed or
    // uncompressed vertex data
    Assert(build.m_pMeshThinVertexData);
    Vector position;
    Vector normal;
    build.m_pMeshThinVertexData->GetMeshPosition(build.m_pMesh, meshIndex,
                                                 &position);
    build.m_pMeshThinVertexData->GetMeshNormal(build.m_pMesh, meshIndex,
                                               &normal);
    VectorCopy(position, decalVertex.m_Position);
    VectorCopy(normal, GetVecNormal(decalVertex.m_Normal));
  }
  Vector2DCopy(build.m_pVertexInfo[meshIndex].m_UV,
               GetVecTexCoord(decalVertex.m_TexCoord));
...
//-----------------------------------------------------------------------------
inline unsigned short CStudioRender::AddVertexToDecal(DecalBuildInfo_t& build,
                                                      int nMeshIndex,
                                                      int nGroupIndex) {
  ...
  if (pVertexInfo[nMeshIndex].m_UniqueID != build.m_nGlobalMeshIndex) {
    ...
    ConvertMeshVertexToDecalVertex(
        build, nMeshIndex, build.m_pDecalMaterial->m_Vertices[v], nGroupIndex);

#ifdef _DEBUG
    // Make sure clipped vertices are in the right range...
    if (build.m_UseClipVert) {
      Assert((decalVertexList[v].m_TexCoord[0] >= -1e-3) &&
             (decalVertexList[v].m_TexCoord[0] - 1.0f < 1e-3));
      Assert((decalVertexList[v].m_TexCoord[1] >= -1e-3) &&
             (decalVertexList[v].m_TexCoord[1] - 1.0f < 1e-3));
    }
#endif
...
inline unsigned short CStudioRender::AddVertexToDecal(DecalBuildInfo_t& build,
  ...
  for (i = build.m_FirstVertex; i != decalVertexList.InvalidIndex();
    ...
    if (decalVertexList[i].GetMesh(build.m_pStudioHdr)) continue;
    ...
    if ((fabs(temp[0]) > 1e-3) || (fabs(temp[1]) > 1e-3) ||
        (fabs(temp[2]) > 1e-3))
      continue;
...
void CStudioRender::AddClippedDecalToTriangle(DecalBuildInfo_t& build,
  ...
  for (i = 0; i < clipState.m_VertCount; ++i) {
    ...
    if (vertIdx < 3) {
      indices[i] = AddVertexToDecal(
          build, clipState.m_ClipVerts[vertIdx].m_MeshVertexIndex);
    } else {
...
//-----------------------------------------------------------------------------
bool CStudioRender::ClipDecal(DecalBuildInfo_t& build, int i1, int i2, int i3,
                              int* pClipFlags) {
  int i;
  ...
  if (clipState.m_VertCount < 3) return false;
...
//-----------------------------------------------------------------------------
void CStudioRender::AddTriangleToDecal(DecalBuildInfo_t& build, int i1, int i2,
                                       int i3, int gi1, int gi2, int gi3) {
  DecalBuildVertexInfo_t* pVertexInfo = build.m_pVertexInfo;
  ...
  int nAllNotInValidArea = pVertexInfo[i1].m_Flags | pVertexInfo[i2].m_Flags |
                           pVertexInfo[i3].m_Flags;
  if ((nAllNotInValidArea & DecalBuildVertexInfo_t::VALID_AREA) == 0) return;
  ...
  if ((clipFlags[0] & (clipFlags[1] & clipFlags[2])) != 0) return;
  ...
  if ((!build.m_UseClipVert) &&
      (!clipFlags[0] || !clipFlags[1] || !clipFlags[2])) {
    doClip = false;
  }
  ...
  if (doClip && (clipFlags[0] || clipFlags[1] || clipFlags[2])) {
    ...
    if (build.m_UseClipVert || (!validTri)) return;
  }
...
void CStudioRender::AddDecalToMesh(DecalBuildInfo_t& build) {
  MeshVertexInfo_t& vertexInfo =
      build.m_pMeshVertices[build.m_nGlobalMeshIndex];
  if (vertexInfo.m_nIndex < 0) return;
    ...
    for (int k = 0; k < pGroup->m_NumStrips; ++k) {
      ...
      if (pStrip->flags & OptimizedModel::STRIP_IS_TRISTRIP) {
        for (int i = 0; i < pStrip->numIndices - 2; ++i) {
          ...
          AddTriangleToDecal(build, i1, i2, i3, pGroup->m_pIndices[ti1],
                             pGroup->m_pIndices[ti2], pGroup->m_pIndices[ti3]);
        }
      } else {
        ...
        for (int i = 0; i < pStrip->numIndices; i += 3) {
          ...
          AddTriangleToDecal(build, i1, i2, i3, pGroup->m_pIndices[idx],
                             pGroup->m_pIndices[idx + 1],
                             pGroup->m_pIndices[idx + 2]);
        }
      }
    }
  }
...
bool CStudioRender::AddDecalToModel(DecalBuildInfo_t& buildInfo) {
  ...
  for (int i = 0; i < m_pSubModel->nummeshes; ++i) {
    ...
    buildInfo.m_pMeshVertexData =
        buildInfo.m_pMesh->GetVertexData(buildInfo.m_pStudioHdr);
    if (buildInfo.m_pMeshVertexData == NULL) {
      buildInfo.m_pMeshThinVertexData =
          buildInfo.m_pMesh->GetThinVertexData(buildInfo.m_pStudioHdr);
      if (!buildInfo.m_pMeshThinVertexData) return false;
    }
...
bool CStudioRender::ComputePoseToDecal(const Ray_t& ray, const Vector& up) {
  ...
  if (VectorNormalize(decalN) == 0.0f) return false;
  ...
  if (VectorNormalize(decalU) < 1e-3) {
    ...
    if (VectorNormalize(decalU) < 1e-3) return false;
  }
...
int CStudioRender::GetDecalMaterial(DecalLod_t& decalLod,
                                    IMaterial* pDecalMaterial) {
  // Grab the material for this lod...
  unsigned short j;
  for (j = decalLod.m_FirstMaterial; j != m_DecalMaterial.InvalidIndex();
       j = m_DecalMaterial.Next(j)) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CStudioRender::RetireDecal(DecalModelList_t& list, DecalId_t nRetireID,
                                int iLOD, int iMaxLOD) {
  // Remove it from the global LRU...
  DecalLRUListIndex_t i;
  for (i = m_DecalLRU.Head(); i != m_DecalLRU.InvalidIndex();
       i = m_DecalLRU.Next(i)) {
  ...
  for (int iLod = (iMaxLOD - 1); iLod >= list.m_pHardwareData->m_RootLOD;
    ...
    while (iList != pHistoryList->InvalidIndex()) {
      ...
      if (pMaterial) {
        // @Note!! Decals must be removed in the reverse order they are added.
        // This code assumes that the decal to remove is the oldest one on the
        // model, and therefore its vertices start at the head of the list
...
#else
          if (list.m_pLod[iLOD].m_FirstMaterial ==
              maybe_retired_decal_history->m_Material) {
            list.m_pLod[iLOD].m_FirstMaterial =
                m_DecalMaterial.Next(maybe_retired_decal_history->m_Material);
          }
          m_DecalMaterial.Free(maybe_retired_decal_history->m_Material);
#endif
        }
      }
...
//-----------------------------------------------------------------------------
int CStudioRender::ComputeTotalMeshCount(int iRootLOD, int iMaxLOD,
                                         int body) const {
  int nMeshCount = 0;
...
//-----------------------------------------------------------------------------
int CStudioRender::ComputeVertexAllocation(int iMaxLOD, int body,
                                           studiohwdata_t* pHardwareData,
                                           MeshVertexInfo_t* pMeshVertices) {
  bool bSuppressTlucDecal =
      (m_pStudioHdr->flags & STUDIOHDR_FLAGS_TRANSLUCENT_TWOPASS) != 0;
  ...
  for (int i = iMaxLOD - 1; i >= pHardwareData->m_RootLOD; i--) {
    ...
    for (int k = 0; k < m_pStudioHdr->numbodyparts; k++) {
      ...
      for (int meshID = 0; meshID < pSubModel->nummeshes;
           ++meshID, ++nCurrMesh) {
...
//-----------------------------------------------------------------------------
void CStudioRender::ProjectDecalsOntoMeshes(DecalBuildInfo_t& build,
                                            int nMeshCount) {
  ...
  for (int i = 0; i < nMeshCount; ++i) {
    ...
    if ((nIndex < 0) || (nIndex <= nMaxVertexIndex)) continue;

    nMaxVertexIndex = nIndex;

    // Project all vertices for this group into decal space
    ProjectDecalOntoMesh(build, &build.m_pVertexBuffer[nIndex],
                         build.m_pMeshVertices[i].m_pMesh);
  }
}
...
//-----------------------------------------------------------------------------
void CStudioRender::AddDecal(StudioDecalHandle_t hDecal,
  ...
  if (list.m_pHardwareData->m_NumStudioMeshes == 0) return;
  ...
  ComputePoseToWorld(m_PoseToWorld, pStudioHdr, BONE_USED_BY_ANYTHING,
                     m_pRC->m_ViewOrigin, m_pBoneToWorld);
  ...
  bool found;
  IMaterialVar* pModelMaterialVar =
      pDecalMaterial->FindVar("$modelmaterial", &found, false);
  if (found) {
    IMaterial* pModelMaterial = g_pMaterialSystem->FindMaterial(
        pModelMaterialVar->GetStringValue(), TEXTURE_GROUP_DECAL, false);
  ...
  buildInfo.m_UseClipVert =
      (m_pStudioHdr->numbones <= 1) && (m_pStudioHdr->numflexdesc == 0);
  ...
  int nMeshCount =
      ComputeTotalMeshCount(list.m_pHardwareData->m_RootLOD, iMaxLOD - 1, body);

  // NOTE: This is a consequence of the sizeof (m_UniqueID)
  if (nMeshCount >= 255) {
    Warning(
        "Unable to apply decals to model (%s), it has more than 255 unique "
        "meshes!\n",
  ...
  buildInfo.m_pMeshVertices =
      (MeshVertexInfo_t*)stackalloc(nMeshCount * sizeof(MeshVertexInfo_t));
  int nVertexCount = ComputeVertexAllocation(
      iMaxLOD, body, list.m_pHardwareData, buildInfo.m_pMeshVertices);
  buildInfo.m_pVertexBuffer = (DecalBuildVertexInfo_t*)stackalloc(
      nVertexCount * sizeof(DecalBuildVertexInfo_t));
  ...
  DecalHistoryList_t* pHistoryList =
      &list.m_pLod[list.m_pHardwareData->m_RootLOD].m_DecalHistory;
  if (m_DecalLRU.Count() >= m_pRC->m_Config.maxDecalsPerModel * 1.5) {
    ...
    RetireDecal(modelList, nRetireID, modelList.m_pHardwareData->m_RootLOD,
                modelList.m_pHardwareData->m_NumLODs);
  }

  if (pHistoryList->Count() >= m_pRC->m_Config.maxDecalsPerModel) {
    DecalHistory_t* pDecalHistory =
        &pHistoryList->Element(pHistoryList->Head());
  ...
  for (int i = iMaxLOD - 1; i >= list.m_pHardwareData->m_RootLOD; i--) {
    ...
    DecalMaterial_t* maybe_retired_decal_material =
        &m_DecalMaterial[materialIdx];
    while (maybe_retired_decal_material->m_Indices.Count() >
           MAX_DECAL_INDICES_PER_MODEL) {
      DecalHistoryList_t* maybe_retired_decal_history_list =
          &list.m_pLod[i].m_DecalHistory;
      DecalHistory_t* maybe_retired_decal_history =
          &maybe_retired_decal_history_list->Element(
              maybe_retired_decal_history_list->Head());
      RetireDecal(list, maybe_retired_decal_history->m_nId, nRootLOD,
                  nFinalLOD);
    }
  }
  ...
  for (int i = iMaxLOD - 1; i >= list.m_pHardwareData->m_RootLOD; i--) {
    ...
    buildInfo.m_FirstVertex =
        buildInfo.m_pDecalMaterial->m_Vertices.InvalidIndex();
    ...
    for (k = 0; k < m_pStudioHdr->numbodyparts; k++) {
      ...
      if (!AddDecalToModel(buildInfo)) break;
    }

    if (k != m_pStudioHdr->numbodyparts) continue;
    ...
    if (buildInfo.m_VertexCount) {
      ...
      int decalIndexCount =
          buildInfo.m_pDecalMaterial->m_Indices.Count() - prevIndexCount;
      ...
      int history_index = list.m_pLod[i].m_DecalHistory.AddToTail();
      list.m_pLod[i].m_DecalHistory[history_index].m_Material = materialIdx;
      list.m_pLod[i].m_DecalHistory[history_index].m_Decal = decalIndex;
      list.m_pLod[i].m_DecalHistory[history_index].m_nId = m_nDecalId;
      list.m_pLod[i].m_DecalHistory[history_index].m_nPad = 0;
    }
  }

  // Add to LRU
  if (bAddedDecals) {
    DecalLRUListIndex_t lru_index = m_DecalLRU.AddToTail();
    m_DecalLRU[lru_index].m_nDecalId = m_nDecalId;
    m_DecalLRU[lru_index].m_hDecalHandle = hDecal;
...
void CStudioRender::DrawSingleBoneDecals(CMeshBuilder& meshBuilder,
  ...
  for (DecalVertexList_t::IndexLocalType_t i = verts.Head();
       i != verts.InvalidIndex(); i = verts.Next(i)) {
...
void CStudioRender::DrawSingleBoneFlexedDecals(
  ...
  for (DecalVertexList_t::IndexLocalType_t i = verts.Head();
    ...
    // Clipped verts shouldn't come through here, only static props should use
    // clipped
    Assert(vertex.m_MeshVertexIndex >= 0);

    m_VertexCache.SetBodyModelMesh(vertex.m_Body, vertex.m_Model,
                                   vertex.m_Mesh);
    if (m_VertexCache.IsVertexFlexed(vertex.m_MeshVertexIndex)) {
      CachedPosNormTan_t* pFlexedVertex =
          m_VertexCache.GetFlexVertex(vertex.m_MeshVertexIndex);
...
//-----------------------------------------------------------------------------
bool CStudioRender::DrawMultiBoneDecals(CMeshBuilder& meshBuilder,
                                        DecalMaterial_t& decalMaterial,
  ...
  for (DecalVertexList_t::IndexLocalType_t i = verts.Head();
    ...
    if (m_VertexCache.IsVertexPositionCached(n)) {
      ...
      if (vertData) {
        ...
        R_StudioTransform(position, &boneWeights,
                          pCachedVert->m_Position.AsVector3D());
        R_StudioRotate(normal, &boneWeights,
                       pCachedVert->m_Normal.AsVector3D());
      } else {
        return false;
      }
...
bool CStudioRender::DrawMultiBoneFlexedDecals(IMatRenderContext* pRenderContext,
  ...
  for (DecalVertexList_t::IndexLocalType_t i = verts.Head();
    ...
    if (!vertData) return false;

    IMorph* pMorph =
        pBoneRemap ? vertex.GetMorph(m_pStudioHdr, m_pStudioMeshes) : NULL;
    Vector2D morphUV;
    if (pMorph) {
      Assert(pBoneRemap);
      Assert(vertex.m_GroupIndex != 0xFFFF);
      if (!pRenderContext->GetMorphAccumulatorTexCoord(&morphUV, pMorph,
                                                       vertex.m_GroupIndex)) {
        pMorph = NULL;
      }
    }
    ...
    if (!pMorph) {
      ...
      if (m_VertexCache.IsVertexPositionCached(n)) {
        ...
        if (m_VertexCache.IsThinVertexFlexed(n)) {
          ...
          VectorAdd(*vertData->Position(n),
                    pFlexedVertex->m_Position.AsVector3D(), vecPosition);
          VectorAdd(*vertData->Normal(n), pFlexedVertex->m_Normal.AsVector3D(),
                    vecNormal);
          R_StudioTransform(vecPosition, pBoneWeights,
                            pCachedVert->m_Position.AsVector3D());
          R_StudioRotate(vecNormal, pBoneWeights,
                         pCachedVert->m_Normal.AsVector3D());
          ...
          R_StudioTransform(pFlexedVertex->m_Position, pBoneWeights,
                            pCachedVert->m_Position.AsVector3D());
          R_StudioRotate(pFlexedVertex->m_Normal, pBoneWeights,
                         pCachedVert->m_Normal.AsVector3D());
        } else {
          Assert(pMesh);
          R_StudioTransform(*vertData->Position(n), pBoneWeights,
      ...
      // NOTE: Even if HW morphing is active, since we're using bone 0, it will
      // multiply by identity in the shader
      meshBuilder.BoneWeight(0, 1.0f);
      meshBuilder.BoneWeight(1, 0.0f);
      meshBuilder.BoneWeight(2, 0.0f);
      meshBuilder.BoneWeight(3, 0.0f);
      ...
      // NOTE: We should be renormalizing bone weights here like
      // R_AddVertexToMesh does.. It's too expensive. Tough noogies.
      mstudioboneweight_t* pBoneWeights = vertData->BoneWeights(n);
      ...
      meshBuilder.BoneWeight(
          2, 1.0f - pBoneWeights->weight[1] - pBoneWeights->weight[0]);
      meshBuilder.BoneWeight(3, 0.0f);
...
//-----------------------------------------------------------------------------
void CStudioRender::DrawDecalMaterial(IMatRenderContext* pRenderContext,
                                      DecalMaterial_t& decalMaterial,
                                      studiohdr_t* pStudioHdr,
                                      studioloddata_t* pStudioLOD) {
  ...
  bool bUseHWMorphing =
      (pStudioLOD && (pStudioLOD->m_pHWMorphDecalBoneRemap != NULL));
  if (bUseHWMorphing) {
  ...
  if (m_pStudioHdr->numbones <= 1) {
    ...
    if (m_pStudioHdr->numflexdesc != 0) {
      if (!DrawMultiBoneFlexedDecals(pRenderContext, meshBuilder, decalMaterial,
                                     pStudioHdr, pStudioLOD)) {
        bDraw = false;
      }
    } else {
...
//-----------------------------------------------------------------------------
static Vector s_pWhite[6] = {Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0),
                             Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0),
                             Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0)};

bool CStudioRender::PreDrawDecal(IMatRenderContext* pRenderContext,
  ...
  if (m_pRC->m_Config.fullbright) {
    ...
    m_pRC->m_NumLocalLights = CopyLocalLightingState(
        MAXLOCALLIGHTS, m_pRC->m_LocalLights, drawInfo.m_nLocalLightCount,
        drawInfo.m_LocalLightDescs);
  }
...
//-----------------------------------------------------------------------------
void CStudioRender::DrawDecal(const DrawModelInfo_t& drawInfo, int lod,
                              int body) {
  ...
  for (; mat != m_DecalMaterial.InvalidIndex();
       mat = m_DecalMaterial.Next(mat)) {
    DecalMaterial_t& decalMaterial = m_DecalMaterial[mat];
    DrawDecalMaterial(pRenderContext, decalMaterial, m_pStudioHdr, pStudioLOD);
  }
}

void CStudioRender::DrawStaticPropDecals(const DrawModelInfo_t& drawInfo,
                                         const StudioRenderContext_t& rc,
  ...
  for (; mat != m_DecalMaterial.InvalidIndex();
       mat = m_DecalMaterial.Next(mat)) {
    DecalMaterial_t& decalMaterial = m_DecalMaterial[mat];
    DrawDecalMaterial(pRenderContext, decalMaterial, drawInfo.m_pStudioHdr,
                      NULL);
  }
