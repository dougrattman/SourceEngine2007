...
//			Assumes that the wildcard character '*' marks the end of
// comparison, 			in other words, these are identical:
//
//			test*
//			test*stuff
//
...
//-----------------------------------------------------------------------------
static void ReplaceNodeIDRecursive(CMapClass *pRoot, int nOldNodeID,
                                   int nNewNodeID) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
static void ReplaceNodeIDRefs(CMapObjectList &newList, int nOldNodeID,
                              int nNewNodeID) {
  // If they are the same, do nothing. This can happen when pasting from one
  // map to another map.
  if (nOldNodeID == nNewNodeID) return;
...
// Purpose: Destructor.
//-----------------------------------------------------------------------------
CMapEntity::~CMapEntity(void) { SignalChanged(); }
...
void CMapEntity::AddBoundBoxForClass(GDclass *pClass, bool bLoading) {
  ...
  //		 This is not correct because it bypasses the doc's
  // AddObjectToWorld code.
  //
...
void CMapEntity::AddChild(CMapClass *pChild) {
  ...
  if (dynamic_cast<CMapSolid *>(pChild) == NULL) {
    for (int i = GetFirstKeyValue(); i != GetInvalidKeyValue();
         i = GetNextKeyValue(i)) {
...
//			bLoading - True if this is being called from Postload,
// false otherwise.
//-----------------------------------------------------------------------------
void CMapEntity::AddHelper(CMapClass *pHelper, bool bLoading) {
  ...
  //
  // Note: in AddChild, where it calls OnParentKeyChanged for everything, the
  // color in the helper can get set to something else based on the entity's
  // properties (CMapLightCone does this, for example).
  //
  ...
  //
  // dvs: HACK for animator children. Better for CMapEntity to have a
  // SetAnimatorChild
  //		function that the CMapAnimator could call. Better still,
  // eliminate the knowledge 		that CMapEntity has about its animator
  // child.
  //
...
//			children of this entity. Helper objects perform
// rendering,
// UI, and 			bookkeeping functions for their parent entities.
// If
// the class 			definition does not specify any helpers, or none
// of the helpers could be added, a box helper is added so that the entity has
// some visual representation. Inputs : pClass -
//			bLoading - True if this is being called from Postload,
// false otherwise.
//-----------------------------------------------------------------------------
void CMapEntity::AddHelpersForClass(GDclass *pClass, bool bLoading) {
  ...
  //
  // If we have a game class from the FGD, add whatever helpers are declared in
  ...
  //
  // If we have no game class and we are a point entity, add an "obsolete"
  // sprite helper so level designers know to update the entity.
  //
  else if (pClass == NULL) {
    CHelperInfo HelperInfo;
  ...
  //
  // If we still haven't added any visible helpers, we need to add a bounding
  // box so that there is some visual representation for this entity. We also
  // add the bounding box if the entity's class specifies a bounding box.
  //
  if (!bAddedOneVisual) {
    AddBoundBoxForClass(pClass, bLoading);
  }
...
CMapClass *CMapEntity::CopyFrom(CMapClass *pobj, bool bUpdateDependencies) {
  ...
  //
  // Copy our keys. If our targetname changed we must relink all targetname
  // pointers.
  //
...
//-----------------------------------------------------------------------------
// Purpose: If this entity has a name key, returns a string with "<name>
// <classname>"
//			in it. Otherwise returns a buffer with "<classname>" in
// it.
...
//-----------------------------------------------------------------------------
void CMapEntity::GetRenderColor(unsigned char &red, unsigned char &green,
                                unsigned char &blue) {
  if (IsSelected()) {
...
//-----------------------------------------------------------------------------
size_t CMapEntity::GetSize(void) { return (sizeof(*this)); }
...
ChunkFileResult_t CMapEntity::LoadVMF(CChunkFile *pFile) {
  ...
  Handlers.AddHandler("connections", (ChunkHandler_t)LoadConnectionsCallback,
                      (CEditGameClass *)this);

  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadKeyCallback, this);
  pFile->PopHandlers();
...
ChunkFileResult_t CMapEntity::LoadKeyCallback(const char *szKey,
  ...
  if (!stricmp(szKey, "id")) {
    ...
    //
    // While loading, set key values directly rather than via SetKeyValue. This
    // avoids all the unnecessary bookkeeping that goes on in SetKeyValue.
    //
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapEntity::LoadHiddenCallback(CChunkFile *pFile,
                                                 CMapEntity *pEntity) {
  //
  // Set up handlers for the subchunks that we are interested in.
  //
...
// Purpose:
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapEntity::LoadEditorCallback(CChunkFile *pFile,
                                                 CMapEntity *pObject) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapEntity::LoadSolidCallback(CChunkFile *pFile,
                                                CMapEntity *pEntity) {
...
// Input  : bRemoveSolidChildren - Whether to also remove any solid children.
// This
//			is true when changing from a solid entity to a point
// entity.
//-----------------------------------------------------------------------------
void CMapEntity::RemoveHelpers(bool bRemoveSolids) {
  for (int pos = m_Children.Count() - 1; pos >= 0; pos--) {
    ...
    // LEAKLEAK: need to KeepForDestruction to avoid undo crashes, but how?
    // where?
    // delete pChild;
  }
}
...
//-----------------------------------------------------------------------------
static inline void BuildNewTargetName(const char *pOldName,
                                      const char *pNewName, char *pBuffer) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapEntity::ReplaceTargetname(const char *szOldName,
                                   const char *szNewName) {
  // NOTE: Case-sensitive compare because people might want to replace one case
  // with another.
...
//			to perform any linking with other map objects or to do
// other operations 			that require all world objects to be
// present.
//-----------------------------------------------------------------------------
void CMapEntity::PostloadWorld(CMapWorld *pWorld) {
  ...
  //
  if (IsPlaceholder() &&
      (!IsClass() || GetClass()->VarForName("origin") == NULL)) {
    const char *pszValue = m_KeyValues.GetValue("origin", &nIndex);
    if (pszValue != NULL) {
      RemoveKey(nIndex);
    }
...
//-----------------------------------------------------------------------------
// Purpose: Insures that the entity has all the helpers that it needs (and no
// more
...
//-----------------------------------------------------------------------------
// Safely sets the move parent. Will assert and not set it if pEnt is equal to
// this ent, or if this ent is already a parent of pEnt.
//-----------------------------------------------------------------------------
...
//			of its children. The child exposes the property as a key
// value pair.
...
//-----------------------------------------------------------------------------
void CMapEntity::NotifyChildKeyChanged(CMapClass *pChild, const char *szKey,
                                       const char *szValue) {
  ...
  FOR_EACH_OBJ(m_Children, pos) {
    ...
    if ((pObject != pChild) && (pChild != NULL) &&
        (dynamic_cast<CMapSolid *>(pObject) == NULL)) {
...
// Input  : pClone -
//-----------------------------------------------------------------------------
void CMapEntity::OnPreClone(CMapClass *pClone, CMapWorld *pWorld,
                            const CMapObjectList &OriginalList,
  ...
  if (OriginalList.Count() == 1) {
    ...
    if (!stricmp(GetClassName(), "path_corner") ||
      ...
      if (!pNewEntity) return;

      // create a new targetname for the clone
      char newName[128];
      const char *oldName = GetKeyValue("targetname");
      if (!oldName || oldName[0] == 0) oldName = "path";

      pWorld->GenerateNewTargetname(oldName, newName, sizeof(newName), true,
                                    NULL);
...
//			NewList -
//-----------------------------------------------------------------------------
void CMapEntity::OnClone(CMapClass *pClone, CMapWorld *pWorld,
                         const CMapObjectList &OriginalList,
                         CMapObjectList &NewList) {
...
//			pDestWorld - The world that the copies are being added
// to. 			OriginalList - The list of original objects that were
// copied. 			NewList - The list of copied.
//-----------------------------------------------------------------------------
void CMapEntity::OnPrePaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
...
//			NewList -
//-----------------------------------------------------------------------------
void CMapEntity::OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
                         CMapWorld *pDestWorld,
                         const CMapObjectList &OriginalList,
                         CMapObjectList &NewList) {
  if (IsNodeClass()) {
...
//			pszValue -
//-----------------------------------------------------------------------------
void CMapEntity::OnKeyValueChanged(const char *pszKey, const char *pszOldValue,
                                   const char *pszValue) {
  ...
  if (!stricmp(pszKey, "parentname")) {
    ...
    if (pWorld != NULL) {
      CMapEntity *pMoveParent = (CMapEntity *)UpdateDependency(
          m_pMoveParent, pWorld->FindEntityByName(pszValue));
...
//			classes that are not in the FGD are considered solid
// entities if 			they have at least one solid child, point
// entities if not.
//-----------------------------------------------------------------------------
...
// Use
// PostloadWorld 				  to do similar bookkeeping
// after map load.
//
...
//-----------------------------------------------------------------------------
void CMapEntity::OnNotifyDependent(CMapClass *pObject,
  ...
  if (eNotifyType == Notify_Removed) {
    ...
    if (pObject == m_pMoveParent) {
      ...
      if ((pWorld != NULL) && (pszParentName != NULL)) {
        CMapEntity *pMoveParent = (CMapEntity *)UpdateDependency(
            m_pMoveParent, pWorld->FindEntityByName(pszParentName));
        SetMoveParent(pMoveParent);
      } else {
        CMapEntity *pMoveParent =
            (CMapEntity *)UpdateDependency(m_pMoveParent, NULL);
        SetMoveParent(pMoveParent);
      }
    }
  }
...
// Input  : matrix -
// Output : returns true if a new matrix is returned, false if it is just the
// identity
//-----------------------------------------------------------------------------
bool CMapEntity::GetTransformMatrix(VMatrix &matrix) {
  ...
  if (m_pAnimatorChild) {
    ...
    if (gotMatrix) {
      ...
      if (!gotAnimMatrix) {
        // since we didn't get a new matrix from our child just return our
        // parent's
        return true;
      }
...
#ifndef SDK_BUILD
  ChunkFileResult_t eResult =
      pFile->WriteKeyValueVector2("logicalpos", m_vecLogicalPosition);
...
ChunkFileResult_t CMapEntity::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo) {
  ...
  if (IsPlaceholder() &&
      (!IsClass() || GetClass()->VarForName("origin") == NULL)) {
    char szOrigin[80];
    sprintf(szOrigin, "%g %g %g", (double)m_Origin[0], (double)m_Origin[1],
            (double)m_Origin[2]);
...
void CMapEntity::UpdateDependencies(CMapWorld *pWorld, CMapClass *pObject) {
  ...
  if (pDoc && !pDoc->IsLoading()) {
    // Update any downstream/upstream connections objects associated with this
    // entity
    Connections_FixBad();
    Upstream_FixBad();
  }
...
//-----------------------------------------------------------------------------
// Purpose: Looks for an input with a given name in the entity list. ALL
// entities
//			in the list must have the given input for a match to be
// found.
// Input  : szInput - Name of the input.
// Output : Returns true if the input name was found in all entities, false if
// not.
//-----------------------------------------------------------------------------
bool MapEntityList_HasInput(const CMapEntityList *pList, const char *szInput,
...
//-----------------------------------------------------------------------------
// Purpose: Returns a pointer to the object that should be added to the
// selection
//			list because this object was clicked on with a given
// selection mode.
// Input  : eSelectMode -
//-----------------------------------------------------------------------------
CMapClass *CMapEntity::PrepareSelection(SelectMode_t eSelectMode) {
  //
  // Select up the hierarchy when in Groups selection mode if we belong to a
  // group.
  //
  if ((eSelectMode == selectGroups) && (m_pParent != NULL) &&
      !IsWorldObject(m_pParent)) {
...
void CMapEntity::Render2D(CRender2D *pRender) {
  ...
  if (s_bShowEntityNames && pView->GetZoom() >= 1) {
    ...
    if (pszTargetName != NULL) {
      pRender->DrawText(pszTargetName, pt.x, pt.y + 2,
                        CRender2D::TEXT_JUSTIFY_BOTTOM);
    }

    const char *pszClassName = GetClassName();
    if (pszClassName != NULL) {
      pRender->DrawText(pszClassName, pt.x, pt2.y - 2,
                        CRender2D::TEXT_JUSTIFY_TOP);
    }
  }
  ...
  if (s_bShowEntityConnections) {
    ...
    if (pszTarget != NULL) {
      ...
      pWorld->EnumChildren((ENUMMAPCHILDRENPROC)FindKeyValue, (DWORD)&kv,
...
void CMapEntity::GetRenderLogicalBox(Vector2D &mins, Vector2D &maxs) {
  ...
  maxs.x = m_vecLogicalPosition.x + LOGICAL_BOX_WIDTH +
           LOGICAL_BOX_CONNECTOR_INPUT_WIDTH +
           LOGICAL_BOX_CONNECTOR_OUTPUT_WIDTH;
...
//-----------------------------------------------------------------------------
void CMapEntity::GetLogicalConnectionPosition(LogicalConnection_t i,
...
void CMapEntity::RenderLogical(CRender2D *pRender) {
  ...
  color32 rgbLowlight = {static_cast<u8>(5 * rgbColor.r / 8),
                         static_cast<u8>(5 * rgbColor.g / 8),
                         static_cast<u8>(5 * rgbColor.b / 8), UINT8_MAX};
  color32 rgbEdgeColor = {static_cast<u8>(3 * rgbColor.r / 8),
                          static_cast<u8>(3 * rgbColor.g / 8),
                          static_cast<u8>(3 * rgbColor.b / 8), UINT8_MAX};
  color32 rgbInterior = {static_cast<u8>(2 * rgbColor.r / 8),
                         static_cast<u8>(2 * rgbColor.g / 8),
                         static_cast<u8>(2 * rgbColor.b / 8), UINT8_MAX};
  ...
  pRender->DrawRectangle(Vector(vecBoxMins.x, vecBoxMins.y, 0.0f),
                         Vector(vecBoxMaxs.x, vecBoxMaxs.y, 0.0f), true, 0);
  ...
  pRender->DrawRectangle(Vector(vecInnerMins.x, vecBoxMins.y, 0.0f),
                         Vector(vecBoxMaxs.x, vecInnerMaxs.y, 0.0f), true, 0);
  ...
  pRender->DrawRectangle(Vector(vecBoxMins.x, vecBoxMins.y, 0.0f),
                         Vector(vecBoxMaxs.x, vecBoxMaxs.y, 0.0f), false, 0);

  // Draw the small diagonals connecting the outer and inner corners
  pRender->DrawLine(Vector(vecBoxMins.x, vecBoxMins.y, 0.0f),
                    Vector(vecBoxMaxs.x, vecBoxMaxs.y, 0.0f));
  pRender->DrawLine(Vector(vecBoxMins.x, vecBoxMaxs.y, 0.0f),
                    Vector(vecBoxMaxs.x, vecBoxMins.y, 0.0f));

  // Draw interior background first
  pRender->SetDrawColor(rgbInterior.r, rgbInterior.g, rgbInterior.b);
  pRender->DrawRectangle(Vector(vecInnerMins.x, vecInnerMins.y, 0.0f),
                         Vector(vecInnerMaxs.x, vecInnerMaxs.y, 0.0f), true, 0);
  ...
  if (!bFoundSpriteHelper) {
    ...
    pRender->SetDrawColor(rgbColor.r, rgbColor.g, rgbColor.b);
    pRender->DrawRectangle(Vector(vecInnerMins.x, vecInnerMins.y, 0.0f),
                           Vector(vecInnerMaxs.x, vecInnerMaxs.y, 0.0f), true,
                           0);

    // Put an inner border around the entity color block
    pRender->SetDrawColor(rgbEdgeColor.r, rgbEdgeColor.g, rgbEdgeColor.b);
    pRender->DrawRectangle(Vector(vecInnerMins.x, vecInnerMins.y, 0.0f),
                           Vector(vecInnerMaxs.x, vecInnerMaxs.y, 0.0f), false,
                           0);
  }
  ...
  pRender->DrawCircle(
      Vector(vecMins.x + LOGICAL_BOX_CONNECTOR_RADIUS, flConnectorY, 0.0f),
      LOGICAL_BOX_CONNECTOR_RADIUS);
  pRender->MoveTo(
      Vector(vecMins.x + 2 * LOGICAL_BOX_CONNECTOR_RADIUS, flConnectorY, 0.0f));
  pRender->DrawLineTo(Vector(vecBoxMins.x, flConnectorY, 0.0f));

  pRender->MoveTo(Vector(vecBoxMaxs.x, flConnectorY, 0.0f));
  pRender->DrawLineTo(
      Vector(vecMaxs.x - LOGICAL_BOX_ARROW_LENGTH, flConnectorY, 0.0f));
  pRender->DrawLineTo(Vector(vecMaxs.x - LOGICAL_BOX_ARROW_LENGTH,
                             flConnectorY + LOGICAL_BOX_ARROW_HEIGHT, 0.0f));
  ...
  Vector2D pt, pt2;
  pRender->GetView()->WorldToClient(pt,
                                    Vector(vecBoxMins.x, vecBoxMins.y, 0.0f));
  pRender->GetView()->WorldToClient(pt2,
                                    Vector(vecBoxMaxs.x, vecBoxMaxs.y, 0.0f));
  if (fabs(pt.y - pt2.y) < 32) return;
  ...
  if (pszTargetName != NULL) {
    pRender->DrawText(
        pszTargetName, Vector2D((vecMins.x + vecMaxs.x) / 2, vecMaxs.y), 0, -1,
        CRender2D::TEXT_JUSTIFY_TOP | CRender2D::TEXT_JUSTIFY_HORZ_CENTER);
  }

  if (fabs(pt.y - pt2.y) < 50) return;

  const char *pszClassName = GetClassName();
  if (pszClassName != NULL) {
    pRender->DrawText(
        pszClassName, Vector2D((vecMins.x + vecMaxs.x) / 2, vecMins.y), 0, 1,
        CRender2D::TEXT_JUSTIFY_BOTTOM | CRender2D::TEXT_JUSTIFY_HORZ_CENTER);
  }
}

//-----------------------------------------------------------------------------
// Purpose: Returns whether this entity snaps to half grid or not. Some
// entities,
//			such as hinges, need to snap to a 0.5 grid to center on
// geometry.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CMapEntity::IsCulledByCordon(const Vector &vecMins,
                                  const Vector &vecMaxs) {
  // Point entities are culled by their origin, not by their bounding box.
  // An exception to that is swept hulls, such as ladders, that are more like
  // solid ents.
...
// Output :
//-----------------------------------------------------------------------------
bool CMapEntity::HitTest2D(CMapView2D *pView, const Vector2D &point,
                           HitInfo_t &HitData) {
  if (!IsVisible()) return false;

  if (BaseClass::HitTest2D(pView, point, HitData)) return true;

  //
  // Only check point entities; brush entities are selected via their brushes.
  //
  if (!IsPointClass()) return false;
  ...
  if (pView->CheckDistance(point, vecClientCenter, HANDLE_RADIUS)) {
    ...
    //
    // See if any edges of the bbox are within a certain distance from the the
    // point.
    //
    int iSelUnits = 2;
...
//-----------------------------------------------------------------------------
bool CMapEntity::HitTestLogical(CMapViewLogical *pView,
                                const Vector2D &vecPoint, HitInfo_t &hitData) {
  if (!IsVisible() || !IsLogical() || !IsVisibleLogical()) return false;

  if (BaseClass::HitTestLogical(pView, vecPoint, hitData)) return true;
...
bool CMapEntity::IsLogical(void) {
  GDclass *pClass = GetClass();
  return pClass && ((pClass->GetInputCount() > 0) ||
                    (pClass->GetOutputCount() > 0)) ||
         (m_Connections.Count() || m_Upstream.Count());
}

//-----------------------------------------------------------------------------
// Is it visible in the logical view?
//-----------------------------------------------------------------------------
bool CMapEntity::IsVisibleLogical(void) { return IsVisible(); }

//-----------------------------------------------------------------------------
// Purpose: Returns true if this entity's name matches the given name,
// considering
//			wildcards.
// Input  : szName -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if this entity's classname matches the given name,
// considering
//			wildcards.
// Input  : szName -
//-----------------------------------------------------------------------------
