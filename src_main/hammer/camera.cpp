...
//-----------------------------------------------------------------------------
// Purpose: Returns the distance from the camera to the near clipping plane in
// world units.
//-----------------------------------------------------------------------------
float CCamera::GetNearClip(void) { return (m_fNearClip); }

//-----------------------------------------------------------------------------
// Purpose: Returns the distance from the camera to the far clipping plane in
// world units.
//-----------------------------------------------------------------------------
...
//			fFarClip -
//-----------------------------------------------------------------------------
void CCamera::SetPerspective(float fHorizontalFOV, float fNearClip,
                             float fFarClip) {
  m_bIsOrthographic = false;
...
void CCamera::GetFrustumPlanes(Vector4D Planes[6]) {
  ...
  Planes[4][3] = DotProduct(m_ViewPoint + ViewForward * m_fNearClip,
                            Planes[4].AsVector3D());
  ...
  Planes[5][3] = DotProduct(m_ViewPoint + ViewForward * m_fFarClip,
                            Planes[5].AsVector3D());
}
...
//-----------------------------------------------------------------------------
// Purpose: Sets the distance from the camera to the near clipping plane in
// world units.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Sets the distance from the camera to the far clipping plane in world
// units.
//-----------------------------------------------------------------------------
...
// Input  : ViewTarget - the point in world space that the camera should look
// at.
//-----------------------------------------------------------------------------
void CCamera::SetViewTarget(const Vector& ViewTarget) {
  ...
  //
  // Ideally we could replace the math below with standard VectorAngles stuff,
  // but Hammer camera matrices use a different convention from QAngle (sadly).
  //
...
void CCamera::BuildRay(const Vector2D& vView, Vector& vStart, Vector& vEnd) {
  ...
  // Build a ray from the viewpoint through the point on the near clipping
  // plane.
