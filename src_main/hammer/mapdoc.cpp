...
ON_COMMAND(ID_ENABLE_LIGHT_PREVIEW_CUSTOM_FILENAME,
           OnEnableLightPreviewCustomFilename)
...
ON_UPDATE_COMMAND_UI(ID_VIEW_HIDEUNCONNECTED,
                     OnUpdateViewHideUnconnectedEntities)
...
ON_COMMAND(ID_TOOLS_SNAP_SELECTED_TO_GRID_INDIVIDUALLY,
           OnToolsSnapSelectedToGridIndividually)
ON_UPDATE_COMMAND_UI(ID_TOOLS_SNAP_SELECTED_TO_GRID_INDIVIDUALLY,
                     OnUpdateEditSelection)
...
ON_UPDATE_COMMAND_UI(ID_TOOLS_TOGGLETEXLOCKSCALE,
                     OnUpdateToolsToggletexlockScale)
...
ON_UPDATE_COMMAND_UI(ID_VIEW_HIDENONSELECTEDOBJECTS,
                     OnUpdateViewHideUnselectedObjects)
...
ON_UPDATE_COMMAND_UI(ID_TOOLS_DISP_DRAWBUILDABLE,
                     OnUpdateToggleDispDrawBuildable)
ON_COMMAND(ID_TOOLS_DISP_DRAWREMOVEDVERTS, OnToggleDispDrawRemovedVerts)
ON_UPDATE_COMMAND_UI(ID_TOOLS_DISP_DRAWREMOVEDVERTS,
                     OnUpdateToggleDispDrawRemovedVerts)
...
struct BatchReplaceTextures_t {
  ...
  char szReplaceTexName[MAX_REPLACE_LINE_LENGTH];  // Texture to replace the
                                                   // found texture with.
};
...
void CMapDoc::AssignToGroups() {
  ...
  //
  // Remove all the objects that were added to groups from the world, since they
  // are now children of CMapGroup objects that are already in the world.
  //
  int nPairCount = GroupedObjects.Count();
  for (int i = 0; i < nPairCount; i++) {
    m_pWorld->RemoveChild(GroupedObjects.Element(i).pObject1);
    GroupedObjects.Element(i).pObject2->AddChild(
        GroupedObjects.Element(i).pObject1);
  }
}

//-----------------------------------------------------------------------------
// Purpose: Called after loading a VMF file. Assigns all objects to their proper
//			visgroups. It also finds the next unique ID to use when
    ...
    for (int i = 0; i < nKeyCount; i++) {
      ...
      if (!_stricmp(pszKey, "visgroupid")) {
        ...
        if (pszVisGroupID != NULL) {
          ...
          // If an object is assigned to a nonexistent visgroup, we do nothing
          // here, which effectively discards the bogus visgroup assignment.
          if (pVisGroup) {
            pChild->AddVisGroup(pVisGroup);
            if (CVisGroup::IsConvertingOldVisGroups() &&
                (pVisGroup->GetVisible() != VISGROUP_SHOWN)) {
...
void CMapDoc::VisGroups_Validate() {
  ...
  while (pChild) {
    ...
    // Old versions of Hammer had wacky visgroup assignments -- solid children
    // of groups or entities could belong to visgroups, even if those visgroups
    // no longer existed.

    // For new versions of Hammer, just make sure that the object is allowed to
    // belong to a visgroup.
    if (((g_nFileFormatVersion < 100) && (!IsWorldObject(pParent))) ||
      ...
      if (nCount != 0) {
        Msg(mwWarning,
            "'%s', child of '%s', was in visgroups illegally. Removed from "
            "visgroups.",
            pChild->GetDescription(), pParent->GetDescription());
...
void CMapDoc::GetSelectedCenter(Vector &vCenter) {
  Morph3D *pMorphTool =
      dynamic_cast<Morph3D *>(m_pToolManager->GetActiveTool());
  CToolBlock *pBlockTool =
      dynamic_cast<CToolBlock *>(m_pToolManager->GetActiveTool());
...
void CMapDoc::CountGUIDs(void) {
  ...
  // This increments the CMapWorld face ID but it doesn't matter since we're
  // setting it below.
  ...
  while (pChild != NULL) {
    ...
    if (pEntity != NULL) {
      ...
      // look at our classname keyvalue to determine whether we are a node
      // class.
      //
...
//-----------------------------------------------------------------------------
CMapEntity *CMapDoc::CreateEntity(const char *pszClassName, float x, float y,
                                  float z) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
// dvs: Seems better to move some of this functionality into DeleteObject and
// replace calls to this with FindEntity/DeleteObject calls.
//      Probably need to replicate all functions in the doc as follows:
//      DeleteObject, DeleteObjectList to optimize updates. Either that
//		or we need a way to lock and unlock updates.
bool CMapDoc::DeleteEntity(const char *pszClassName, float x, float y,
                           float z) {
...
//-----------------------------------------------------------------------------
CMapEntity *CMapDoc::FindEntity(const char *pszClassName, float x, float y,
                                float z) {
  ...
  if (pszClassName != NULL) {
    ...
    m_pWorld->EnumChildren((ENUMMAPCHILDRENPROC)FindEntityCallback,
                           (DWORD)&FindInfo, MAPCLASS_TYPE(CMapEntity));
...
//-----------------------------------------------------------------------------
bool CMapDoc::FindEntitiesByClassName(CMapEntityList &Found,
...
//
//-----------------------------------------------------------------------------
bool CMapDoc::FindEntitiesByKeyValue(CMapEntityList &Found, const char *pszKey,
                                     const char *pszValue, bool bVisiblesOnly) {
  return m_pWorld->FindEntitiesByKeyValue(Found, pszKey, pszValue,
                                          bVisiblesOnly);
}
...
//
//-----------------------------------------------------------------------------
bool CMapDoc::FindEntitiesByName(CMapEntityList &Found, const char *pszName,
                                 bool bVisiblesOnly) {
...
//
//-----------------------------------------------------------------------------
bool CMapDoc::FindEntitiesByNameOrClassName(CMapEntityList &Found,
                                            const char *pszName,
                                            bool bVisiblesOnly) {
...
//			bVisible - true to make visible, false to make
// invisible.
...
// Purpose:
//-----------------------------------------------------------------------------
int CMapDoc::GetDocumentCount(void) { return s_ActiveDocs.Count(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CMapDoc *CMapDoc::GetDocument(int index) { return s_ActiveDocs.Element(index); }
...
void CMapDoc::OnViewGotoCoords() {
  CStrDlg dlg(0, "",
              "Coordinates to go to (x y z), ex: 200 -4096 1154\nex: setpos -1 "
              "2 3; setang 4 5 6",
              "Go to coordinates");
    ...
    if (sscanf(dlg.m_string, "%f %f %f", &posVec.x, &posVec.y, &posVec.z) ==
      ...
      if ((posVec.x == 200) && (posVec.y == -4096) && (posVec.z == 1154)) {
        if (AfxMessageBox("Seriously?\n\n(I mean, that was just an example, "
                          "you were supposed to type in your own numbers)",
...
void CMapDoc::OnMapShowSelectedBrushNumber() {
  ...
  m_pWorld->EnumChildrenRecurseGroupsOnly(
      (ENUMMAPCHILDRENPROC)&CMapDoc::GetBrushNumberCallback, (DWORD)&info,
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::LoadEntityCallback(CChunkFile *pFile,
                                              CMapDoc *pDoc) {
  CMapEntity *pEntity = new CMapEntity;
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::LoadHiddenCallback(CChunkFile *pFile,
                                              CMapDoc *pDoc) {
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("entity", (ChunkHandler_t)CMapDoc::LoadEntityCallback,
                      pDoc);
...
//-----------------------------------------------------------------------------
bool CMapDoc::HandleLoadError(CChunkFile *pFile, const char *szChunkName,
                              ChunkFileResult_t eError, CMapDoc *pDoc) {
  return (false);
}
...
bool CMapDoc::LoadVMF(const char *pszFileName) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    //
    CChunkHandlerMap Handlers;
    Handlers.AddHandler("world", (ChunkHandler_t)CMapDoc::LoadWorldCallback,
                        this);
    Handlers.AddHandler("hidden", (ChunkHandler_t)CMapDoc::LoadHiddenCallback,
                        this);
    Handlers.AddHandler("entity", (ChunkHandler_t)CMapDoc::LoadEntityCallback,
                        this);
    Handlers.AddHandler("versioninfo",
                        (ChunkHandler_t)CMapDoc::LoadVersionInfoCallback, this);
    Handlers.AddHandler("autosave",
                        (ChunkHandler_t)CMapDoc::LoadAutosaveCallback, this);
    Handlers.AddHandler("visgroups",
                        (ChunkHandler_t)CVisGroup::LoadVisGroupsCallback, this);
    Handlers.AddHandler("viewsettings",
                        (ChunkHandler_t)CMapDoc::LoadViewSettingsCallback,
                        this);
    Handlers.AddHandler("cordon", (ChunkHandler_t)CMapDoc::LoadCordonCallback,
                        this);

    m_pToolManager->AddToolHandlers(&Handlers);

    Handlers.SetErrorHandler((ChunkErrorHandler_t)CMapDoc::HandleLoadError,
                             this);
    ...
    //
    // Read the sub-chunks. We ignore keys in the root of the file, so we don't
    // pass a key value callback to ReadChunk.
    //
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    GetMainWnd()->MessageBox(File.GetErrorText(eResult), "Error loading file",
                             MB_OK | MB_ICONEXCLAMATION);
  }

  if (bLocked) VisGroups_LockUpdates(false);
  ...
  while (pChild != NULL) {
    ...
    if (pSolid != NULL) {
      ...
      for (int i = 0; i < nFaces; i++) {
        ...
        if (pFace) DetailObjects::BuildAnyDetailObjects(pFace);
      }
    }
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::LoadVersionInfoCallback(CChunkFile *pFile,
                                                   CMapDoc *pDoc) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::LoadVersionInfoKeyCallback(const char *szKey,
                                                      const char *szValue,
                                                      CMapDoc *pDoc) {
...
ChunkFileResult_t CMapDoc::LoadAutosaveCallback(CChunkFile *pFile,
                                                CMapDoc *pDoc) {
...
ChunkFileResult_t CMapDoc::LoadCordonCallback(CChunkFile *pFile,
                                              CMapDoc *pDoc) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::LoadViewSettingsKeyCallback(const char *szKey,
                                                       const char *szValue,
                                                       CMapDoc *pDoc) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::LoadViewSettingsCallback(CChunkFile *pFile,
                                                    CMapDoc *pDoc) {
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadViewSettingsKeyCallback, pDoc);
  if (eResult == ChunkFile_Ok) {
...
void CMapDoc::Postload(void) {
  ...
  if (CMapSolid::GetBadSolidCount() > 0) {
    char szError[80];
    sprintf(szError,
            "For your information, %d solid(s) were not loaded due to errors "
            "in the file.",
            CMapSolid::GetBadSolidCount());
    GetMainWnd()->MessageBox(szError, "Warning", MB_ICONINFORMATION);
  }

  //
  // Count GUIDs before calling PostLoadWorld because objects that need to
  // generate GUIDs may do so in PostLoadWorld.
  //
  CountGUIDs();
  ...
  // Do this after AssignToVisGroups, because deleting objects causes empty
  // visgroups to be purged, and until AssignToVisGroups is called all the
  // visgroups are empty!
  ...
  // We disabled building detail objects above to prevent it from generating
  // them extra times. Now generate the ones that need to be generated.
...
BOOL CMapDoc::SelectDocType(void) {
  ...
  //	{
  //		APP()->OpenURL(ID_HELP_FIRST_TIME_SETUP,
...
//-----------------------------------------------------------------------------
// Purpose: set up this document to edit a prefab data .. when the object is
// saved,
...
BOOL CMapDoc::Serialize(std::fstream &file, BOOL fIsStoring, BOOL bRMF) {
  ...
  if (m_bEditingPrefab) {
    ...
    if (!pLibrary) {
      ...
      AfxMessageBox(
          "The library this prefab object belongs to has been\n"
  ...
  if (bRMF) {
    ...
    Camera3D *pCamTool =
        dynamic_cast<Camera3D *>(m_pToolManager->GetToolForID(TOOL_CAMERA));
    ...
    if (pCamTool) {
      ...
      if (fIsStoring) {
        ...
        if (memcmp(buf, sig, sizeof sig)) goto Done;
  ...
  Done:;
    ...
    if (m_bIsCordoning) {
      //
      // Create "cordon world", add its objects to our real world, create a list
      // in CordonList so we can remove them again.
      //
      ...
      //
      // HACK: (not mine) - make the cordon bounds bigger so that the cordon
      // brushes overlap the cordon bounds during serialization.
    ...
    if (m_pWorld->SerializeMAP(file, fIsStoring,
                               m_bIsCordoning ? &CordonBox : NULL) < 0) {
      AfxMessageBox("There was a file error.", MB_OK | MB_ICONEXCLAMATION);
      return (FALSE);
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::AssertValid(void) const { CDocument::AssertValid(); }

//-----------------------------------------------------------------------------
// Purpose:
// Input  : dc -
//-----------------------------------------------------------------------------
void CMapDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif  //_DEBUG
...
CVisGroup *CMapDoc::VisGroups_GroupForName(const char *pszName, bool bIsAuto) {
  ...
  for (int i = 0; i < nCount; i++) {
    CVisGroup *pGroup = m_VisGroups.Element(i);
    if (!Q_stricmp(pGroup->GetName(), pszName) &&
        (pGroup->IsAutoVisGroup() == bIsAuto)) {
      return pGroup;
    }
  }
  return NULL;
...
//-----------------------------------------------------------------------------
void CMapDoc::ShowNewVisGroupsDialog(CMapObjectList &Objects,
                                     bool bUnselectObjects) {
  ...
  VisGroups_AddObjectsToVisGroup(Objects, pVisGroup, dlg.GetHideObjectsOption(),
                                 dlg.GetRemoveFromOtherGroups());
...
//-----------------------------------------------------------------------------
void CMapDoc::VisGroups_CreateNamedVisGroup(CMapObjectList &Objects,
                                            const char *szName, bool bHide,
                                            bool bRemoveFromOtherVisGroups) {
  CVisGroup *pVisGroup = VisGroups_AddGroup(szName);
  VisGroups_AddObjectsToVisGroup(Objects, pVisGroup, bHide,
                                 bRemoveFromOtherVisGroups);
}

//-----------------------------------------------------------------------------
// Purpose: Adds the objects to the given visgroup and does hiding as specified.
//-----------------------------------------------------------------------------
void CMapDoc::VisGroups_AddObjectsToVisGroup(CMapObjectList &Objects,
                                             CVisGroup *pVisGroup, bool bHide,
                                             bool bRemoveFromOtherVisGroups) {
  //
  ...
  if (m_bVisGroupUpdatesLocked) return;
...
//-----------------------------------------------------------------------------
// Purpose: Returns whether or not this object is eligible for inclusion in a
// visgroup.
//-----------------------------------------------------------------------------
bool CMapDoc::VisGroups_ObjectCanBelongToVisGroup(CMapClass *pObject) {
  ...
  if (pParent) {
    if (IsWorldObject(pParent)) return true;

    if (pParent->IsGroup()) return true;

    // Children of entities cannot belong to visgroups independent of their
    // parent.
...
void CMapDoc::VisGroups_UpdateParents(void) {
  ...
  for (int i = 0; i < nVisGroupCount; i++) {
    ...
    if (pTempGroup->GetVisible() != VISGROUP_UNDEFINED &&
        pTempGroup->GetParent() != NULL) {
...
BOOL CMapDoc::SaveModified(void) {
  ...
  if (m_bEditingPrefab) {
    switch (
        AfxMessageBox("Do you want to save the changes to this prefab object?",
                      MB_YESNOCANCEL)) {
      case IDYES: {
        std::fstream file;
        Serialize(file, 0, 0);
...
BOOL CMapDoc::OnOpenDocument(LPCTSTR lpszPathName) {
  ...
  // things), but visgroups aren't available until after map load. We have to
  // refresh the visgroups here or they won't be correct.
  //
  GetMainWnd()->GlobalNotify(WM_MAPDOC_CHANGED);
...
void CMapDoc::OnCloseDocument(void) {
  //
  // Deactivate the current tool now because doing it later can cause
  // side-effects.
  //
...
BOOL CMapDoc::OnSaveDocument(LPCTSTR lpszPathName) {
  if (m_pBSPLighting) m_pBSPLighting->Serialize();
  ...
  if (access(lpszPathName, 0) != -1) {
    if (!CopyFile(lpszPathName, szFile, FALSE)) {
      ...
      wsprintf(szError,
               ...
               "drive and that the path still exists.",
               lpszPathName, dwError);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::UpdateAllCameras(const Vector *vecViewPos,
                               const Vector *vecLookAt, const float *fZoom) {
  POSITION pos = GetFirstViewPosition();
  while (pos != NULL) {
...
void CMapDoc::UpdateAnimation(void) {
  ...
  // iterate through all CMapEntity object and update their animation frame
  // matrix
  m_pWorld->EnumChildren(ENUMMAPCHILDRENPROC(_UpdateAnimation), animTime.dw,
                         MAPCLASS_TYPE(CMapAnimator));
}
...
//-----------------------------------------------------------------------------
// Purpose: Gets the current time and stores it in the doc, for use during the
// frame
//-----------------------------------------------------------------------------
...
static BOOL SelectInBox(CMapClass *pObject, SelectBoxInfo_t *pInfo) {
  ...
  //
  // Skip groups. Groups are selected via their members through
  // PrepareSelection.
  //
  if (pObject->IsGroup()) {
  ...
  if (pSelObject) {
    ...
    if (pInfo->bInside) {
      ...
      bSelect =
          pObject->IsIntersectingBox(pInfo->pBox->bmins, pInfo->pBox->bmaxs);
    }
...
// Purpose:
//-----------------------------------------------------------------------------
static BOOL SelectInLogicalBox(CMapClass *pObject,
                               SelectLogicalBoxInfo_t *pInfo) {
  // Skip hidden objects.
  if (!pObject->IsVisible() || !pObject->IsLogical() ||
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::SelectLogicalRegion(const Vector2D &vecMins,
                                  const Vector2D &vecMaxs, bool bInsideOnly) {
...
void CMapDoc::UpdateStatusbar(void) {
  if (m_pToolManager->GetActiveToolID() == TOOL_FACEEDIT_MATERIAL) {
    CString str;
    str.Format("%d faces selected",
               GetMainWnd()->m_pFaceEditSheet->GetFaceListCount());
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::BuildCascadingSelectionList(
    CMapClass *pObj, CUtlRBTree<CMapClass *, unsigned short> &list,
    bool bRecursive) {
...
void CMapDoc::SetActiveMapDoc(CMapDoc *pDoc) {
  ...
  if (m_pMapDoc != NULL) {
    ...
    GetMainWnd()->pObjectProperties->SetObjectList(
        m_pMapDoc->GetSelection()->GetList());
...
void CMapDoc::DeleteObject(CMapClass *pObject) {
  ...
  // a group, delete the parent object also. This avoids ghost objects at the
  // origin.
  ...
  if (pParent) {
    if (pParent->IsGroup()) {
      ...
      if (pParentEntity != NULL) {
        if (!pParentEntity->IsPlaceholder() &&
            !pParentEntity->HasSolidChildren()) {
...
//-----------------------------------------------------------------------------
void CMapDoc::OnEditReplace(void) { GetMainWnd()->ShowSearchReplaceDialog(); }
...
void CMapDoc::UpdateStatusBarSnap(void) {
  CString strSnap;
  strSnap.Format(" Snap: %s Grid: %d ", m_bSnapToGrid ? "On" : "Off",
                 m_nGridSpacing);
...
void CMapDoc::OnEditClearselection(void) {
  if (m_pToolManager->GetActiveToolID() == TOOL_MORPH) {
    ...
    if (m_pSelection->GetCount() > 2) {
      GetHistory()->MarkUndoPosition(m_pSelection->GetList(),
                                     "Clear Selection");
    }
...
//-----------------------------------------------------------------------------
// Purpose: Applies the current default texture to all faces of all selected
// solids.
//-----------------------------------------------------------------------------
void CMapDoc::OnEditApplytexture(void) {
  ...
  for (int i = 0; i < pSelList->Count(); i++) {
    ...
    pobj->EnumChildren((ENUMMAPCHILDRENPROC)ApplyTextureToSolid,
                       (DWORD)GetDefaultTextureName(),
                       MAPCLASS_TYPE(CMapSolid));
  }
...
//			class will be the default solid class from the game
// configuration.
//-----------------------------------------------------------------------------
void CMapDoc::OnEditToEntity(void) {
  ...
  //
  // Build a list of every solid in the selection, whether part of a solid
  // entity or not.
  //
  ...
  for (int i = 0; i < pSelList->Count(); i++) {
    ...
    //
      ...
      pObject->EnumChildren(ENUMMAPCHILDRENPROC(CopyObjectsToList),
                            DWORD(&newobjects), MAPCLASS_TYPE(CMapSolid));
    }
    //
    // If the object is an entity, add any solid children of the entity to our
    // list.
    //
      ...
      //
      // See if there is more than one solid entity selected. If so, we'll need
      // to prompt the user to pick one.
      //
  ...
  if (pNewEntity && !bUseSelectionDialog) {
    CString str;
    str.Format(
        "You have selected an existing entity (a '%s'.)\n"
  ...
  //
  // If there were multiple solid entities selected, bring up the selection
  // dialog.
  //
  ...
  FOR_EACH_OBJ(newobjects, pos) {
    ...
    if (pOldParent != pNewEntity) {
      ...
      if (pOldParent != NULL) {
        ...
        //
        // If this solid was the child of a solid entity, check to see if the
        // entity has any children left - if not, we remove it from the world
        // because it's useless without solid children.
        //
        CMapEntity *pOldParentEnt = dynamic_cast<CMapEntity *>(pOldParent);
        if (pOldParentEnt && (!pOldParentEnt->IsPlaceholder()) &&
            (!pOldParentEnt->HasSolidChildren())) {
      ...
      for (int nVisGroup = 0; nVisGroup < nVisGroupCount; nVisGroup++) {
        ...
        if (pVisGroup->IsAutoVisGroup()) continue;
    ...
    FOR_EACH_OBJ(SelList, pos) {
      ...
      if ((pEntity != NULL) && (!pEntity->IsPlaceholder())) {
        ...
        for (int i = 0; i < nChildCount; i++) {
          ...
          pChild->SetRenderColor(0, 100 + (random() % 156),
                                 100 + (random() % 156));
...
void CMapDoc::OnEditCopy(void) {
  ...
  for (int i = 0; i < pSelList->Count(); i++) {
    ...
    //
    // Prune the object from the world tree without calling
    // RemoveObjectFromWorld. This prevents CopyChildrenFrom from updating the
    // culling tree.
    //
...
void CMapDoc::GetBestVisiblePoint(Vector &ptOrg) {
  ...
  for (int i = 0; i < 3; i++) {
    if (ptOrg[i] == COORD_NOTINIT) ptOrg[i] = 0;
  }
}
...
void CMapDoc::GetBestPastePoint(Vector &vecPasteOrigin) {
  ...
  if (pView) {
    ...
    if (ptClient.x >= 0 && ptClient.x < width && ptClient.y >= 0 &&
        ptClient.y < height) {
      ...
      if (p3DView) {
        ...
        if (p3DView->ObjectsAt(vPoint, &Hits, 1)) {
          // If they clicked on a solid, the index of the face they clicked on
          // is stored in array index [1].
...
//			pParent -
//-----------------------------------------------------------------------------
void CMapDoc::Paste(CMapObjectList &Objects, CMapWorld *pSourceWorld,
                    CMapWorld *pDestWorld, Vector vecOffset, QAngle vecRotate,
                    CMapClass *pParent, bool bMakeEntityNamesUnique,
  ...
  if (bMakeEntityNamesUnique ||
      (pszEntityNamePrefix && (pszEntityNamePrefix[0] != '\0'))) {
    //
    // Make all the pasted objects with names have new, unique names within the
    // destination world.
    //

    // Stick the objects into a temporary world object for renaming the
    // entities.
    CMapWorld temp;
    ...
    RenameEntities(&temp, pDestWorld, bMakeEntityNamesUnique,
                   pszEntityNamePrefix);
  ...
  //
  // Notification happens in two-passes. The first pass lets objects generate
  // new unique IDs in the destination world, the second pass lets objects fixup
  // references to other objects in the clipboard.
  //
  ...
  //
  // Do the second pass of notification. The second pass of notification lets
  // objects fixup references to other objects that were pasted. We don't do it
  // in the loop above because then not all the pasted objects would be in the
  // world yet.
  //
  Assert(Objects.Count() == PasteList.Count());
  ...
  FOR_EACH_OBJ(Objects, pos) {
    ...
    //
    // Semi-HACK: If we aren't pasting into a group, keep the new object in the
    // Undo stack. Otherwise, we'll keep the group in OnEditPasteSpecial.
    //
...
void CMapDoc::OnEditPaste(void) {
  ...
  //
  // Build a translation that will put the pasted objects in the center of the
  // view.
  //
...
void CMapDoc::OnUpdateEditPaste(CCmdUI *pCmdUI) {
  pCmdUI->Enable(
      s_Clipboard.Objects.Count() &&
      (m_pToolManager->GetActiveToolID() != TOOL_FACEEDIT_MATERIAL) &&
      !GetMainWnd()->IsShellSessionActive());
}

//-----------------------------------------------------------------------------
// Purpose: Handles the Edit | Cut command. Copies the selection to the
// clipboard,
...
void CMapDoc::OnUpdateGroupEditFunction(CCmdUI *pCmdUI) {
  //
  // Edit functions are disabled when we're applying textures or editing via a
  // shell session.
  //
  pCmdUI->Enable(
      (m_pToolManager->GetActiveToolID() != TOOL_FACEEDIT_MATERIAL) &&
...
void CMapDoc::OnToolsGroup(void) {
  ...
  if ((m_pSelection->GetMode() == selectSolids) &&
      !Options.general.bGroupWhileIgnore) {
    return;
  }

  const CMapObjectList *pSelList = m_pSelection->GetList();

  // First see if grouping these objects will remove them from an existing
  // entity or group.
  for (int i = 0; i < pSelList->Count(); i++) {
    CMapClass *pobj = pSelList->Element(i);
    if ((pobj->GetParent() != NULL) && (!IsWorldObject(pobj->GetParent()))) {
      if (GetMainWnd()->MessageBox(
              "Some selected objects are part of an entity or belong to a "
              "group. Grouping them now will remove them from the entity or "
              "group! Continue?",
...
void CMapDoc::OnFileSaveAs(void) {
  ...
  do {
    ...
    extension.MakeLower();
    if (extension != wantedExtension) str += wantedExtension;
 ...
 public:
  CExportDlg(CString &strFile, LPCTSTR pszExt, LPCTSTR pszDesc)
      : CFileDialog(FALSE, pszExt, strFile,
                    OFN_NOCHANGEDIR | OFN_LONGNAMES | OFN_HIDEREADONLY |
  ...
  afx_msg void OnToggleVisibles() { bVisibles = m_Visibles.GetCheck(); }
...
void CMapDoc::OnFileExport(void) {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    if (strFile.CompareNoCase(dlg.GetPathName()) == 0) {
      if (GetMainWnd()->MessageBox(
              "You are about to export over your current work file. Some data "
              "loss will occur if you have any objects hidden. Continue?",
              "Export Warning", MB_YESNO | MB_ICONEXCLAMATION) != IDYES) {
        return;
      }
    }
...
//-----------------------------------------------------------------------------
// Purpose: Exports using the last exported pathname. Brings up the Export
// dialog
...
void CMapDoc::OnUpdateFileConvertWAD(CCmdUI *pCmdUI) {
  pCmdUI->Enable((m_pWorld != NULL) &&
                 (g_pGameConfig->GetTextureFormat() == tfVMT));
}
...
void CMapDoc::OnFileRunmap(void) {
  ...
  //
  if ((g_pGameConfig->GetTextureFormat() == tfWAD) &&
      !Options.textures.nTextureFiles) {
    AfxMessageBox(
        "There are no texture files defined yet. Add some texture files before "
        "you run the map.");
  ...
  CString strCompileExtension;
  GetFileExtensions(g_pGameConfig->GetMapFormat(), strEditExtension,
                    strCompileExtension);
  ...
  Assert(iPos != -1);
  if ((iPos != -1) &&
      (strEditExtension.CompareNoCase(strCompileExtension) != 0)) {
  ...
  while (1) {
    if (AfxGetApp()->GetProfileInt("Run Map", "Mode", 0) == 0) {
      ...
      // run expert dialog
      if (dlgExpert.DoModal() == IDCANCEL) return;
  ...
  // Q_snprintf( cmd.szParms, sizeof(cmd.szParms), "\"%s\"",
  // m_pGame->m_szGameExeDir); strcpy(cmd.szRun, "Change Directory");
  // cmds.Add(cmd);
  // cmd.iSpecialCmd = 0;
  ...
  if ((dlg.m_iQBSP) && (m_pGame->szBSP[0] != '\0')) {
    ...
    sprintf(cmd.szParms, "-game $gamedir %s$path\\$file",
  ...
  if ((dlg.m_iLight) && (m_pGame->szLIGHT[0] != '\0')) {
    ...
    sprintf(cmd.szParms, "%s -game $gamedir %s$path\\$file",
            dlg.m_bHDRLight ? "-both" : "",
  ...
  if (dlg.m_bNoQuake == FALSE) {
    ...
    //{
    //	sprintf(cmd.szParms, "-applaunch %s -game $gamedir %s +map $file",
    // strSteamAppID, dlg.m_strQuakeParms);
    //}
    // else
    //{
    //	sprintf(cmd.szParms, "-game $moddir %s +map $file",
    // dlg.m_strQuakeParms);
    //}
    //}
    // else
...
void CMapDoc::UpdateTitle(CView *pView) {
  ...
  // Leaving the partial-filename code in here in case we want to make it an
  // option someday.
...
// Input  : *pView -
//-----------------------------------------------------------------------------
void CMapDoc::RemoveMRU(CMapView2D *pView) { MRU2DViews.FindAndRemove(pView); }
...
//-----------------------------------------------------------------------------
// Purpose: This is called for each doc when the texture application mode
// changes. Input  : bApplicator - TRUE if entering texture applicator mode,
// FALSE if
...
void CMapDoc::UpdateForApplicator(BOOL bApplicator) {
  if (bApplicator) {
    ...
    for (int i = 0; i < pSelList->Count(); i++) {
      ...
      pObject->EnumChildren((ENUMMAPCHILDRENPROC)AddLeavesToListCallback,
    ...
    //
    // Select all faces of all solids that were selected originally. Disable
    // updates in the face properties dialog beforehand or this could take a
    // LONG time.
    //
    ...
    //
    // Remove all faces from the dialog's list and update their selection state
    // to be not selected, then update the display.
    //
    GetMainWnd()->m_pFaceEditSheet->ClickFace(NULL, -1,
                                              CFaceEditSheet::cfClear);
    UpdateStatusbar();
  }
}
...
void CMapDoc::SelectFace(CMapSolid *pSolid, int iFace, int cmd) {
  ...
  if (iFace == -1 && pSolid) {
    ...
    bool bDispSolidMask =
        CMapDoc::GetActiveMapDoc()->IsDispSolidDrawMask() && pSolid->HasDisp();
  ...
  //
  // Check the current click mode and perform the texture application if
  // appropriate.
  //
  BOOL bApply = FALSE;
  ...
  if (!(cmd & scNoApply)) {
    ...
    bApply = ((iFaceMode == CFaceEditSheet::ModeApply) ||
              (iFaceMode == CFaceEditSheet::ModeApplyAll));
...
void CMapDoc::SetView2dInfo(VIEW2DINFO &vi) {
  ...
  while (p) {
    ...
    if (!pView->IsKindOf(RUNTIME_CLASS(CMapView2D))) continue;
...
void CMapDoc::SetViewLogicalInfo(VIEW2DINFO &vi) {
  ...
  while (p) {
    CView *pView = GetNextView(p);
    if (!pView->IsKindOf(RUNTIME_CLASS(CMapViewLogical))) continue;
...
void CMapDoc::Center3DViewsOn(const Vector &vPos) {
  ...
  while (p) {
    CView *pView = GetNextView(p);
    if (!pView->IsKindOf(RUNTIME_CLASS(CMapView3D))) continue;
...
void CMapDoc::OnToolsHollow(void) {
  ...
  if (m_pSelection->GetCount() > 1) {
    if (AfxMessageBox("Do you want to turn each of the selected solids into a "
                      "hollow room?",
                      MB_YESNO) == IDNO) {
      return;
    }
  ...
  //
  // Prompt for the wall thickness, which is remembered from one hollow to
  // another.
  //
  static int iWallWidth = 32;
  char szBuf[128];
  itoa(iWallWidth, szBuf, 10);
  CStrDlg dlg(CStrDlg::Spin, szBuf,
              "How thick do you want the walls? Use a negative number to "
              "hollow outward.",
              "Hammer");
  ...
  if (abs(iWallWidth) < 2) {
    AfxMessageBox(
        "The width of the walls must be less than -1 or greater than 1.");
    return;
  }
  ...
  //
  // Carve every solid in the selection with a scaled copy of itself. This
  // accomplishes the goal of hollowing them.
  //
...
void CMapDoc::OnEditPastespecial(void) {
  ...
  const char *pszPrefix =
      (dlg.m_bAddPrefix == TRUE) ? dlg.m_strPrefix.GetString() : "";
  ...
  for (int i = 0; i < dlg.m_iCopies; i++) {
    ...
    //
    Paste(s_Clipboard.Objects, s_Clipboard.pSourceWorld, pWorld, vecPasteOffset,
          vecPasteAngles, pParent, bMakeNamesUnique, pszPrefix);
...
void CMapDoc::OnUpdateEditPastespecial(CCmdUI *pCmdUI) {
  pCmdUI->Enable((s_Clipboard.Objects.Count() != 0) &&
                 !GetMainWnd()->IsShellSessionActive());
}
...
//			pWorld -
//-----------------------------------------------------------------------------
bool CMapDoc::ExpandTargetNameKeywords(char *szNewTargetName,
    ...
    FOR_EACH_OBJ(*pEntityList, pos) {
      ...
      if (pszTargetName != NULL) {
        ...
        if (nFullLen > nPrefixLen + nSuffixLen) {
          ...
          //
          // If the prefix and the suffix match ours, extract the instance
...
bool CMapDoc::DoExpandKeywords(CMapClass *pObject, CMapWorld *pWorld,
                               char *szOldKeyword, char *szNewKeyword) {
...
static bool FindName(CUtlVector<const char *> *pList, const char *pszString) {
  for (int i = 0; i < pList->Count(); i++) {
    if (Q_stricmp(pszString, pList->Element(i)) == 0) return true;
  }
...
void CMapDoc::RenameEntities(CMapClass *pRoot, CMapWorld *pWorld,
  ...
  for (int i = 0; i < oldNames.Count(); i++) {
    if (pWorld->GenerateNewTargetname(oldNames[i], szName, sizeof(szName),
                                      bMakeUnique, szAddPrefix, pRoot)) {
...
//-----------------------------------------------------------------------------
void CMapDoc::OnEditSelnext(void) { m_pSelection->SetCurrentHit(hitNext); }

//-----------------------------------------------------------------------------
// Purpose: Selects the previous object by depth in the 3D view.
//-----------------------------------------------------------------------------
void CMapDoc::OnEditSelprev(void) { m_pSelection->SetCurrentHit(hitPrev); }
...
void CMapDoc::OnLogicalMoveBlock(void) {
  Vector2D vecLogicalCenter;
  if (!m_pSelection->GetLogicalBoundsCenter(vecLogicalCenter)) return;

  const CMapObjectList *pSelList = m_pSelection->GetList();

  if (pSelList->Count() <= 1) return;
  ...
  if (GetMainWnd()->GetActiveFrame()) {
    pCurrentView = dynamic_cast<CMapViewLogical *>(
        GetMainWnd()->GetActiveFrame()->GetActiveView());
  }
  ...
  UpdateAllViews(MAPVIEW_UPDATE_OBJECTS | MAPVIEW_UPDATE_SELECTION |
                 MAPVIEW_UPDATE_ONLY_LOGICAL);
...
void CMapDoc::OnLogicalSelectAllCascading(void) {
  if (m_pSelection->IsEmpty()) return;
  ...
  for (unsigned short h = list.FirstInorder(); h != list.InvalidIndex();
       h = list.NextInorder(h)) {
    SelectObject(list[h], scSelect);
  }
}

//-----------------------------------------------------------------------------
// Add all entities connected to inputs of all entities in the selection list
// recursively
//-----------------------------------------------------------------------------

void CMapDoc::AddConnectedNodes(
    CMapClass *pObject, CUtlRBTree<CMapClass *, unsigned short> &visited) {
...
void CMapDoc::OnLogicalSelectAllConnected(void) {
  ...
  if (nSelected) {
    ...
    CUtlRBTree<CMapClass *, unsigned short> visited(0, 0,
                                                    DefLessFunc(CMapClass *));
    ...
    for (unsigned short h = visited.FirstInorder(); h != visited.InvalidIndex();
         h = visited.NextInorder(h)) {
...
BOOL CMapDoc::OnViewHideObjects(UINT nID) {
  ...
  if (!nFinalCount) {
    CString str;
    str.Format(
        "There are no eligible %sselected objects. The only objects\n"
    ...
    AfxMessageBox(
        "Some objects could not put in the new Visible Group because\n"
...
//
// Output : Returns true if all top-level children satisfied the above criteria,
// false if not.
//-----------------------------------------------------------------------------
bool CMapDoc::GetChildrenToHide(CMapClass *pObject, bool bSelected,
                                CMapObjectList &List) {
  int nAddedCount = 0;
      ...
      if (pChild->IsVisible()) {
        ...
        if (pGroup || (pEntity && pEntity->IsSolidClass())) {
          ...
          if (GetChildrenToHide(pChild, bSelected, ChildList)) {
            // All this child's children match the criteria, so add the child to
            // the list.
            List.AddToTail(pChild);
            nAddedCount++;
          } else if (ChildList.Count()) {
            // Some of this child's children satisfy the criteria, or have
            // descendents that do. Add the children to the list.
            List.AddVectorToTail(ChildList);
          } else {
            // None of this child's children satisfy the criteria, so skip this
            // child.
          }
...
void CMapDoc::OnUpdateViewShowHelpers(CCmdUI *pCmdUI) {
  ...
  //	{
  //		pCmdUI->SetCheck(m_pToolSelection->GetHandleMode() ==
...
//-----------------------------------------------------------------------------
// Purpose: Manages the state of the View | Hide Unselected menu item and
// toolbar button.
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnMapCheck(void) { CMapCheckDlg::CheckForProblems(GetMainWnd()); }

void CMapDoc::OnMapDiff(void) { CMapDiffDlg::MapDiff(GetMainWnd(), this); }
...
void CMapDoc::OnMapEntityGallery(void) {
  if (GetMainWnd()->MessageBox(
          "This will place one of every possible point entity in the current "
          "map! Performing this operation in an empty map is recommended. "
          "Continue?",
          "Create Entity Gallery", MB_ICONEXCLAMATION | MB_YESNO) == IDYES) {
    int x = -1024;
    int y = -1024;
...
void CMapDoc::OnToolsCreateprefab(void) {
  if (m_pSelection->IsEmpty()) {
    AfxMessageBox(
        "This feature creates a prefab with the selected objects. You must "
        "select some objects before you can use it.",
  ...
  for (int i = 0; i < pSelList->Count(); i++) {
    ...
    //       this prevents CopyChildrenFrom from updating the current world's
    //       culling tree
    pNew->SetParent(NULL);
  ...
  //
  GetMainWnd()->m_ObjectBar.UpdateListForTool(
      m_pToolManager->GetActiveToolID());
}
...
void CMapDoc::OnInsertprefabOriginal(void) {
  ...
  if ((iCurTool != TOOL_POINTER) && (iCurTool != TOOL_BLOCK) &&
      (iCurTool != TOOL_ENTITY)) {
    return;
  }
  ...
  if (iCurTool != TOOL_ENTITY) {
    ...
    CToolEntity *pTool =
        dynamic_cast<CToolEntity *>(m_pToolManager->GetActiveTool());
    pTool->GetPos(pt);
  }
...
static BOOL ReplaceTexFunc(CMapSolid *pSolid, ReplaceTexInfo_t *pInfo) {
  ...
  for (int i = 0; i < nFaces; i++) {
      ...
      case 0:  // replace exact matches only:
      {
        if (!_strcmpi(pszFaceTex, pInfo->szFind)) {
          ...
          pFace->SetTexture(pInfo->szReplace,
                            pInfo->m_bRescaleTextureCoordinates);
          ++pInfo->nReplaced;
        }
        break;
      }
    ...
    if (bDoMarkSolid) {
      if (pInfo->pDoc->GetTools()->GetActiveToolID() ==
...
//			bHidden -
//-----------------------------------------------------------------------------
void CMapDoc::ReplaceTextures(LPCTSTR pszFind, LPCTSTR pszReplace,
                              BOOL bEverything, int iAction, BOOL bHidden,
  ...
  if (bEverything) {
    ...
    m_pWorld->EnumChildren((ENUMMAPCHILDRENPROC)ReplaceTexFunc, (DWORD)&info,
                           MAPCLASS_TYPE(CMapSolid));
  } else {
    // Mark/Replace textures in the selection only.

    // Copy the selection into another list since we might be changing the
    ...
    FOR_EACH_OBJ(tempSelection, pos) {
      ...
      //
      // Call the texture replacement callback for this object (if it is a
      // solid) and all of its children (no matter what).
      //
      if (pobj->IsMapClass(MAPCLASS_TYPE(CMapSolid))) {
        ReplaceTexFunc((CMapSolid *)pobj, &info);
      }
      pobj->EnumChildren((ENUMMAPCHILDRENPROC)ReplaceTexFunc, (DWORD)&info,
                         MAPCLASS_TYPE(CMapSolid));
    }
  }
  ...
  if (!info.bMarkOnly) {
    ...
    str.Format("%d %s marked.", info.nReplaced,
               (m_pToolManager->GetActiveToolID() == TOOL_FACEEDIT_MATERIAL)
                   ? "faces"
                   : "solids");
  }
...
void CMapDoc::BatchReplaceTextures(FileHandle_t fp) {
  ...
  while (g_pFullFileSystem->ReadLine(buf, sizeof(buf), fp)) {
    ...
    m_pWorld->EnumChildren((ENUMMAPCHILDRENPROC)BatchReplaceTextureCallback,
...
void CMapDoc::OnToolsTransform(void) {
  if (m_pSelection->IsEmpty()) {
    AfxMessageBox(
        "You must select some objects before you can\n"
        "transform them.");
    return;
  }
  ...
  //
  // Save any properties that may have been changed in the entity properties
  // dialog. This prevents the LoadData below from losing any changes that were
  // made in the object properties dialog.
  //
...
void CMapDoc::OnToolsToggletexlock(void) {
  ...
  SetStatusText(SBI_PROMPT, Options.IsLockingTextures()
                                ? "Texture locking on"
                                : "Texture locking off");
}
...
void CMapDoc::OnToolsToggletexlockScale(void) {
  ...
  SetStatusText(SBI_PROMPT, Options.IsScaleLockingTextures()
                                ? "Scale texture locking on"
                                : "Scale texture locking off");
}
...
//-----------------------------------------------------------------------------
bool CMapDoc::IsCordoning(void) { return m_bIsCordoning; }
...
// Purpose:
//-----------------------------------------------------------------------------
void CMapDoc::OnToggleCordon(void) { SetCordoning(!m_bIsCordoning); }
...
void CMapDoc::OnFileExporttodxf(void) {
  ...
  fprintf(fp, "9\n$UCSYDIR\n10\n0.0\n20\n1.0\n30\n0.0\n");
  fprintf(fp, "9\n$EXTMIN\n10\n%f\n20\n%f\n30\n%f\n", box.bmins[0],
          box.bmins[1], box.bmins[2]);
  fprintf(fp, "9\n$EXTMAX\n10\n%f\n20\n%f\n30\n%f\n", box.bmaxs[0],
          box.bmaxs[1], box.bmaxs[2]);
  fprintf(fp, "0\nENDSEC\n");
  ...
  m_pWorld->EnumChildren(ENUMMAPCHILDRENPROC(SaveDXF), DWORD(&info),
                         MAPCLASS_TYPE(CMapSolid));
...
void CMapDoc::AddToAutoVisGroup(CMapClass *pObject) {
  if (!pObject || !VisGroups_ObjectCanBelongToVisGroup(pObject)) return;
  ...
  if (pObject->IsMapClass(MAPCLASS_TYPE(CMapEntity))) {
    ...
    if (pMapEntity->IsPointClass()) {
      ...
      if (!Q_strnicmp(pMapEntity->GetClassName(), "trigger_",
                      strlen("trigger_"))) {
        AddChildGroupToAutoVisGroup(pObject, "Triggers", "Entities");
      }

      // Area portals and area portal windows.
      if (!Q_strnicmp(pMapEntity->GetClassName(), "func_areaportal",
                      strlen("func_areaportal"))) {
    ...
    for (int i = 0; i < pMapSolid->GetFaceCount(); i++) {
      // this check will ensure that an object with a water material on multiple
      // faces will only be added once
      if (!bWaterAdded) {
        IMaterial *pMaterial =
            pMapSolid->GetFace(i)->GetTexture()->GetMaterial();
        if (pMaterial) {
...
// Input  : *pObject -
//-----------------------------------------------------------------------------
void CMapDoc::AddChildGroupToAutoVisGroup(CMapClass *pObject,
                                          const char *pAutoVisGroup,
  ...
  for (int i = 0; i < nVisGroupCount; ++i) {
    CVisGroup *pVisGroup = pRootVisGroup->GetChild(i);
    if (!Q_stricmp(pAutoVisGroup,
                   pVisGroup->GetName()))  //&& pVisGroup->IsAutoVisGroup() )
    {
      pFoundVisGroup = pVisGroup;
      break;
    }
...
void CMapDoc::AddObjectToWorld(CMapClass *pObject, CMapClass *pParent) {
  ...
  if (pObject != NULL) {
    ...
    //
    // Update the object's visibility. This will recurse into solid children of
    // entities as well.
    //
    UpdateVisibility(pObject);
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::CordonSaveVMF(CChunkFile *pFile,
                                         CSaveInfo *pSaveInfo) {
...
bool CMapDoc::SaveVMF(const char *pszFileName, int saveFlags) {
  ...
  // We can optionally use these calls if we want the document name to go away
  // so the title bar only says "Saving...".
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    if (!m_bPrefab && !(saveFlags & SAVEFLAGS_LIGHTSONLY)) {
      ...
      if (m_bIsCordoning) {
        //
        // Create "cordon world", add its objects to our real world, create a
        // list in CordonList so we can remove them again.
        //
        pCordonWorld = CordonCreateWorld();
    ...
    //
    // Save VisGroups information. Save this first so that we can assign
    // visgroups while loading objects.
    //
    if (!m_bPrefab && !(saveFlags & SAVEFLAGS_LIGHTSONLY)) {
    ...
    if (eResult == ChunkFile_Ok) {
      eResult =
          m_pWorld->SaveVMF(&File, &SaveInfo, saveFlags & SAVEFLAGS_LIGHTSONLY);
    }
  ...
  if (eResult != ChunkFile_Ok) {
    GetMainWnd()->MessageBox(File.GetErrorText(eResult), "Error Saving File",
                             MB_OK);
  } else {
    // save filename into registry for last known good file for crash recovery
    // purposes.
...
BOOL CMapDoc::NeedsAutosave() { return m_bNeedsAutosave; }

BOOL CMapDoc::IsAutosave() { return m_bIsAutosave; }

CString *CMapDoc::AutosavedFrom() { return &m_strAutosavedFrom; }
...
//-----------------------------------------------------------------------------
// Purpose: Returns the hidden/shown state of the given object based on the
// current
//			settings including selection mode, visgroups, and cordon
// bounds.
// Input  : pObject -
//-----------------------------------------------------------------------------
bool CMapDoc::ShouldObjectBeVisible(CMapClass *pObject) {
  ...
  //
  // If the object was hidden by visgroups, hide it unless visgroup hiding is
  // disabled.
  //
  ...
  //
  // If the cordon tool is active and the object is not within the cordon
  // bounds, hide the object. The exception to this is some helpers, which only
  // hide if their parent entity is culled by the cordon.
  //
  if (m_bIsCordoning) {
...
BOOL CMapDoc::UpdateVisibilityCallback(CMapClass *pObject, CMapDoc *pDoc) {
  ...
  if (bVisible) {
    ...
    if ((dynamic_cast<CMapEntity *>(pObject)) != NULL) {
      pObject->EnumChildren((ENUMMAPCHILDRENPROC)UpdateVisibilityCallback,
                            (DWORD)pDoc);
    }
  }
...
void CMapDoc::UpdateVisibility(CMapClass *pObject) {
  ...
  if (pObject->IsGroup()) {
    pObject->EnumChildrenRecurseGroupsOnly(
        (ENUMMAPCHILDRENPROC)UpdateVisibilityCallback, (DWORD)this);
  }
}
...
void CMapDoc::UpdateVisibilityAll(void) {
  //
  // Two stage recursion: first we recurse groups only, then from the callback
  // we recurse solid children of entities.
  //
  m_pWorld->EnumChildrenRecurseGroupsOnly(
      (ENUMMAPCHILDRENPROC)UpdateVisibilityCallback, (DWORD)this);
  m_pSelection->RemoveInvisibles();
  ...
  if (pwndMain) {
    ...
    // Some entity I/O connections may have become valid/invalid after this
    // hide/show.
...
//			bAuto	- should this be an auto visgroup (default:
// false)
...
CVisGroup *CMapDoc::VisGroups_AddGroup(LPCTSTR pszName, bool bAuto) {
  ...
  //
  pGroup->SetColor(80 + (random() % 176), 80 + (random() % 176),
                   80 + (random() % 176));
...
// Input  : pObject -
//			pGroup -
// Output : Returns FALSE to stop enumerating if the object belonged to the
// given
//			visgroup. Otherwise, returns TRUE to continue
// enumerating.
//-----------------------------------------------------------------------------
BOOL CMapDoc::VisGroups_CheckForGroupCallback(CMapClass *pObject,
                                              CVisGroup *pGroup) {
  if (pObject->IsInVisGroup(pGroup)) {
    return (FALSE);
  }
...
//-----------------------------------------------------------------------------
int CMapDoc::VisGroups_GetCount(void) { return (m_VisGroups.Count()); }
...
//-----------------------------------------------------------------------------
int CMapDoc::VisGroups_GetRootCount(void) { return (m_RootVisGroups.Count()); }
...
void CMapDoc::VisGroups_PurgeGroups(void) {
  ...
  for (int i = nCount - 1; i >= 0; i--) {
    ...
    if (pGroup->GetChildCount()) continue;
...
void CMapDoc::VisGroups_DoRemoveOrCombine(CVisGroup *pFrom, CVisGroup *pTo) {
  ...
  while (pObject != NULL) {
    if (pObject->IsInVisGroup(pFrom)) {
      ...
      // this solves the problem of incorrectly forcing visibilty when an object
      // becomes temporarily orphaned.
      if (pTo) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::VisGroups_SaveVMF(CChunkFile *pFile,
                                             CSaveInfo *pSaveInfo) {
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      if (pVisGroup != NULL && !pVisGroup->IsAutoVisGroup() &&
          strcmp(pVisGroup->GetName(), "Auto")) {
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapDoc::SaveViewSettingsVMF(CChunkFile *pFile,
                                               CSaveInfo *pSaveInfo) {
...
void CMapDoc::OnEnableLightPreview() { InternalEnableLightPreview(false); }
...
//-----------------------------------------------------------------------------
bool CMapDoc::FindNotification(CMapClass *pObject,
...
bool CMapDoc::AnyNotificationsForObject(CMapClass *pObject) {
  int nCount = m_NotifyList.Count();
  for (int i = 0; i < nCount; i++) {
    if (m_NotifyList.Element(i).pObject->m_pObject == pObject) return true;
  }
...
void CMapDoc::NotifyDependents(CMapClass *pObject,
                               Notify_Dependent_t eNotifyType) {
  Assert(!s_bDispatchingNotifications);
  if (s_bDispatchingNotifications) return;
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      if (entry.pObject->m_pObject) {
        ...
        if (!bShowedWarning) {
          bShowedWarning = true;
          AfxMessageBox(
              "ProcessNotifyList: encountered a deleted object. Tell a "
              "programmer.",
              MB_OK);
        }
      }
...
void CMapDoc::DispatchNotifyDependents(CMapClass *pObject,
  ...
  for (int i = 0; i < TempDependents.Count(); i++) {
    ...
    //
    // Maybe we should give our dependents the opportunity to unlink themselves
    // here? Returning false from OnNotify could indicate a desire to sever the
    // dependency. Currently this is accomplished by calling UpdateDependency
    // from OnNotifyDependent.
    //
...
void CMapDoc::CloneObjects(const CMapObjectList &Objects) {
  ...
  // Notification happens in two-passes. The first pass lets objects generate
  // new unique IDs, the second pass lets objects fixup references to other
  // cloned objects.
  ...
  // Do the second pass of notification and add the objects to the world. The
  // second pass of notification lets objects fixup references to other cloned
  // objects.
...
//			vecNudge -
//-----------------------------------------------------------------------------
void CMapDoc::GetNudgeVector(const Vector &vHorz, const Vector &vVert,
                             int nChar, bool bSnap, Vector &vecNudge) {
  vecNudge.Init();
  ...
  switch (nChar) {
    case VK_RIGHT:
      vecNudge += fUnit * vHorz;
      break;
    case VK_LEFT:
      vecNudge -= fUnit * vHorz;
      break;
    case VK_UP:
      vecNudge += fUnit * vVert;
      break;
    case VK_DOWN:
      vecNudge -= fUnit * vVert;
      break;
...
bool CMapDoc::VisGroups_LockUpdates(bool bLock) {
  ...
  if (m_bVisGroupUpdatesLocked == bLock) return false;
...
void CMapDoc::OnLogicalobjectLayoutgeometric() {
  ...
  if (GetMainWnd()->GetActiveFrame()) {
    pCurrentView = dynamic_cast<CMapViewLogical *>(
        GetMainWnd()->GetActiveFrame()->GetActiveView());
    if (pCurrentView) bCenterView = true;
  }
