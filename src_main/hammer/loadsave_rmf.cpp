...
int CMapFace::SerializeRMF(std::fstream &file, BOOL fIsStoring) {
  ...
  if (fIsStoring) {
    //
    // After 3.3 the alignment of vec4_t's changed. We never save the new
    // format, since RMF is no longer being revved.
    //
    TEXTURE_33 OldTex33;
    ...
    //
    // Save face points. We don't serialize the Vectors directly because the
    // memory layout changed with SSE optimizations.
    //
    ...
    //
    // Save plane points. We don't serialize the Vectors directly because the
    // memory layout changed with SSE optimizations.
    //
    for (int i = 0; i < 3; i++) {
    ...
    if (fThisVersion < 0.9f) {
      ...
      file.read(
          (char *)&OldTex.rotate,
          sizeof(OldTex.rotate) + sizeof(OldTex.shift) + sizeof(OldTex.scale));
    ...
    // If reading from a pre-2.2 RMF file, copy the texture info from the old
    // format.
    ...
    //
    // Load the plane points. We don't really need them, but they can fix the
    // face if, somehow, it was saved without any points. RMF could have been
    // smaller if we only saved these plane points.
    //
    if (fThisVersion >= 0.7f) {
      //
      // Load the points into an array of float[3]'s and transfer them into
      // the array of Vectors. We can't load directly into the Vectors because
      // the memory layout changed when SSE optimizations were added.
      //
...
int CMapEntity::SerializeRMF(std::fstream &file, BOOL fIsStoring) {
  ...
  if (fIsStoring) {
    ...
    if (IsClass()) {
      ...
      flags = IsSolidClass() ? (flags & ~flagPlaceholder)
                             : (flags | flagPlaceholder);
    } else {
      // Unknown class. Determine flags by looking for children (only solid ents
      // have children at this point).
      flags = (m_Children.Count() > 0) ? (flags & ~flagPlaceholder)
                                       : (flags | flagPlaceholder);
    }
    ...
    //
    // HACK: Set our class to NULL so that it is properly set from our
    // "classname" key in PostloadWorld.
    //
    m_szClass[0] = '\0';
...
int CMapWorld::SerializeRMF(std::fstream &file, BOOL fIsStoring) {
  ...
  if (fIsStoring) {
    ...
    if (fThisVersion < fLastCompat || fThisVersion > fVersion) {
      CString str;
      str.Format(
          "Oops! SerializeRMF() v%1.1f tried to load a file v%1.1f. Aborting.",
