...
//-----------------------------------------------------------------------------
// Purpose: Callback comparison function for sorting objects clicked on while
//			in selection mode. The reverse sort is used for cards that
//return 			depth values in reverse (larger numbers are closer to the camera).
// Input  : pHit1 - First hit to compare.
//			pHit2 - Second hit to compare.
...
static bool TranslucentObjectsLessFunc(TranslucentObjects_t const &a,
                                       TranslucentObjects_t const &b) {
...
bool GetRequiredMaterial(const char *pName, IMaterial *&pMaterial) {
  ...
  if (pTex) pMaterial = pTex->GetMaterial();
  ...
  if (pMaterial) {
    ...
    Q_snprintf(str, sizeof(str),
               "Missing material '%s'. Go to Tools | Options | Game "
               "Configurations and verify that your game directory is correct.",
               pName);
...
//			routine if this rendered object is positively hit
//tested.
//-----------------------------------------------------------------------------
...
void CRender3D::EndRenderHitTarget(void) {
  if (m_Pick.bPicking) {
    ...
    if ((pRenderContext->SelectionMode(true) != 0) &&
        (m_Pick.nNumHits < MAX_PICK_HITS)) {
      if (m_Pick.uSelectionBuffer[0] == 2) {
        m_Pick.Hits[m_Pick.nNumHits].pObject =
            (CMapClass *)m_Pick.uSelectionBuffer[3];
...
void CRender3D::ComputeFrustumRenderGeometry(CCamera *pCamera) {
  ...
  for (i = 0; i < 4; ++i) {
    float t = (m_FrustumPlanes[4][3] -
               DotProduct(m_FrustumPlanes[4].AsVector3D(), viewPoint)) /
    ...
    */
    ...
    VectorMA(m_FrustumRenderPoint[i], 200.0, edges[i],
             m_FrustumRenderPoint[i + 4]);
  }
#endif
}
...
bool CRender3D::SetView(CMapView *pView) {
  if (!CRender::SetView(pView)) return false;
  ...
  if ((m_WinData.hDC = GetDCEx(m_WinData.hWnd, NULL,
                               DCX_CACHE | DCX_CLIPSIBLINGS)) == NULL) {
...
//			VIS_PARTIAL if the box is partially within the view
//frustum. 			VIS_NONE if the box is entirely outside the view frustum.
//-----------------------------------------------------------------------------
Visibility_t CRender3D::IsBoxVisible(Vector const &BoxMins,
                                     Vector const &BoxMaxs) {
  Vector NearVertex;
  Vector FarVertex;
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (DotProduct(m_FrustumPlanes[i].AsVector3D(), NearVertex) >=
        m_FrustumPlanes[i][3]) {
      return (VIS_NONE);
    }

    if (DotProduct(m_FrustumPlanes[i].AsVector3D(), FarVertex) <
        m_FrustumPlanes[i][3]) {
      nInPlanes++;
    }
  }
...
// Purpose:
// Input  : eRenderState -
// Output : Returns true if the render state is enabled, false if it is
// disabled.
//-----------------------------------------------------------------------------
...
// Output : Returns true if we are rendering for selection, false if rendering
// normally.
//-----------------------------------------------------------------------------
...
//			is at the client coordinates (x, y) and whose width and
//height are 			fWidth and fHeight.
// Input  : x - Leftmost point in the rectangle, in client coordinates.
//			y - Topmost point in the rectangle, in client
//coordinates. 			fWidth - Width of rectangle, in client coordinates. 			fHeight -
//Height of rectangle, in client coordinates. 			pObjects - Pointer to buffer to
//receive objects intersecting the rectangle. 			nMaxObjects - Maximum number of
//object pointers to place in the buffer.
// Output : Returns the number of object pointers placed in the buffer pointed
// to
//			by 'pObjects'.
//-----------------------------------------------------------------------------
int CRender3D::ObjectsAt(float x, float y, float fWidth, float fHeight,
                         HitInfo_t *pObjects, int nMaxObjects) {
  int width, height;
...
static ITexture *SetRenderTargetNamed(int nWhichTarget, char const *pRtName) {
  ...
  ITexture *dest_rt =
      materials->FindTexture(pRtName, TEXTURE_GROUP_RENDER_TARGET);
...
void CRender3D::StartRenderFrame(void) {
  ...
  if (m_Pick.bPicking) {
    ...
    pRenderContext->PickMatrix(m_Pick.fX, m_Pick.fY, m_Pick.fWidth,
                               m_Pick.fHeight);
    pRenderContext->SelectionBuffer(m_Pick.uSelectionBuffer,
                                    sizeof(m_Pick.uSelectionBuffer));
    pRenderContext->SelectionMode(true);
    pRenderContext->ClearSelectionNames();

    float aspect = (float)width / (float)height;

    pRenderContext->PerspectiveX(pCamera->GetFOV(), aspect,
...
static void SetNamedMaterialVar(IMaterial *pMat, char const *pVName,
                                float fValue) {
...
bool CompareLightPreview_Lights(CLightPreview_Light const &a,
                                CLightPreview_Light const &b) {
...
void CRender3D::SendShadowTriangles(void) {
  ...
  if (GetUpdateCounter(EVTYPE_FACE_CHANGED) != LastSendTimeStamp) {
    ...
    while (pChild) {
      if (pChild->IsVisible()) pChild->AddShadowingTriangles(*tri_list);
...
static bool LightForString(char const *pLight, Vector &intensity) {
  ...
  // 1], then we need to fall back to the non-HDR lighting since the HDR
  // lighting hasn't been defined for this light source.
  if ((argCnt == 3 && r == -1.0f && g == -1.0f && b == -1.0f) ||
      (argCnt == 4 && r == -1.0f && g == -1.0f && b == -1.0f &&
       scaler == 1.0f)) {
...
static void SetLightFalloffParams(CMapEntity *e,
...
static bool ParseLightAmbient(CMapEntity *e,
                              CLightingPreviewLightDescription &out) {
  if (LightForKey(e, "_ambient", out.m_Color) == 0) return false;
  return true;
}

static bool ParseLightGeneric(CMapEntity *e,
                              CLightingPreviewLightDescription &out) {
  ...
  if (g_bHDR) {
    if (LightForKey(e, "_lightHDR", out.m_Color) == 0 ||
        (out.m_Color.x == -1.0f && out.m_Color.y == -1.0f &&
         out.m_Color.z == -1.0f)) {
...
// when there are multiple lighting environments, we are supposed to ignore but
// the first
static bool s_bAddedLightEnvironmentAlready;

static void AddEntityLightToLightList(
    CMapEntity *e, CUtlVector<CLightingPreviewLightDescription> &listout) {
  ...
  if (pszClassName) {
    ...
    if ((!s_bAddedLightEnvironmentAlready) &&
      ...
      // now, add the ambient sphere. We will approximate as "N" directional
      // lights
...
void CRender3D::BuildLightList(
    CUtlVector<CLightingPreviewLightDescription> *pList) const {
  CMapDoc *pDoc = m_pView->GetMapDoc();
  CMapWorld *pWorld = pDoc->GetMapWorld();
  ...
  while (pChild) {
    ...
    if (pLightEntity &&
        (pLightEntity->m_EntityTypeFlags & ENTITY_FLAG_IS_LIGHT) &&
...
void CRender3D::SendLightList(void) {
  ...
  if (GetUpdateCounter(EVTYPE_LIGHTING_CHANGED) != LastSendTimeStamp) {
    ...
    CUtlVector<CLightingPreviewLightDescription> *pList =
        new CUtlVector<CLightingPreviewLightDescription>;
...
void CRender3D::EndRenderFrame(void) {
  ...
  if (m_Pick.bPicking) {
    ...
    //
    // Some OpenGL drivers, such as the ATI Rage Fury Max, return selection
    // buffer Z values in reverse order. For these cards, we must reverse the
    // selection order.
    //
  ...
  //
  // Copy the GL buffer contents to our window's device context unless we're in
  // pick mode.
  //
  if (!m_Pick.bPicking) {
    if ((m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW2) ||
      ...
      if ((pCamera->GetYaw() != m_fLastLPreviewAngles[0]) ||
        ...
        view_changed = true;
      if (m_pView->m_bUpdateView &&
          (m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED)) {
        static bool did_dump = false;
        static float Last_SendTime = 0;
        // now, lets create floatbms with the deferred rendering data, so we can
        // pass it to the lpreview thread
      ...
      // only update non-ray traced lpreview if we have no ray traced one or if
      ...
      if (m_pView->m_bUpdateView ||
        ...
        ITexture *dest_rt =
            materials->FindTexture("_rt_albedo", TEXTURE_GROUP_RENDER_TARGET);
        ...
        CUtlPriorityQueue<CLightPreview_Light> light_queue(
            0, 0, CompareLightPreview_Lights);
        ...
        for (int i = 0; i < lightList.Count(); i++) {
          ...
          if ((pLight->m_Type == MATERIAL_LIGHT_SPOT) ||
        ...
        // because of no blend support on ati, we have to ping pong. This needs
        // an nvidia-specifc path for perf
        IMaterial *add_0_to_1 = materials->FindMaterial(
            "editor/addlight0", TEXTURE_GROUP_OTHER, true);
        IMaterial *add_1_to_0 = materials->FindMaterial(
            "editor/addlight1", TEXTURE_GROUP_OTHER, true);

        IMaterial *sample_last = materials->FindMaterial(
            "editor/sample_result_0", TEXTURE_GROUP_OTHER, true);
        IMaterial *sample_other = materials->FindMaterial(
            "editor/sample_result_1", TEXTURE_GROUP_OTHER, true);

        ITexture *dest_rt_current =
            materials->FindTexture("_rt_accbuf_1", TEXTURE_GROUP_RENDER_TARGET);
        ITexture *dest_rt_other =
            materials->FindTexture("_rt_accbuf_0", TEXTURE_GROUP_RENDER_TARGET);
        ...
        for (int i = 0; i < nlights; i++) {
          ...
          pRenderContext->DrawScreenSpaceRectangle(
              src_mat, 0, 0, nTargetWidth, nTargetHeight, 0, 0,
              nTargetWidth - 1, nTargetHeight - 1, dest_rt->GetActualWidth(),
        ...
        pRenderContext->SetRenderTarget(NULL);
        pRenderContext->DrawScreenSpaceRectangle(
            sample_last, xl, yl, dest_width, dest_height, 0, 0, nTargetWidth,
            nTargetHeight, dest_rt->GetActualWidth(),
            dest_rt->GetActualHeight());
      }
    }
    ...
    if ((m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED) &&
      ...
      mybmh.biSizeImage = g_pLPreviewOutputBitmap->m_nWidth *
                          g_pLPreviewOutputBitmap->m_nHeight;
      ...
      // 				0,0,g_pLPreviewOutputBitmap->m_nWidth,
      // g_pLPreviewOutputBitmap->m_nHeight, 				g_pLPreviewOutputBitmap->m_pBits,
      // (BITMAPINFO *) &mybmh, 				DIB_RGB_COLORS, SRCCOPY);
    ...
    if (g_bShowStatistics) {
      ...
      int nLen = sprintf(szText, "FPS=%3.2f Pos=[%.f %.f %.f]", m_fFrameRate,
...
void CRender3D::Render(void) {
  ...
  if ((pCamera->GetYaw() != m_fLastLPreviewAngles[0]) ||
      (pCamera->GetPitch() != m_fLastLPreviewAngles[1]) ||
      (pCamera->GetRoll() != m_fLastLPreviewAngles[2]) ||
      (m_nLastLPreviewHeight != height) || (m_nLastLPreviewWidth != width) ||
      (new_vp != m_LastLPreviewCameraPos) ||
      (pCamera->GetZoom() != m_fLastLPreviewZoom))
    view_changed = true;

  if ((m_eCurrentRenderMode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED) &&
      g_pLPreviewOutputBitmap && (!view_changed)) {
    // blit it
    BITMAPINFOHEADER mybmh;
    mybmh.biHeight = -g_pLPreviewOutputBitmap->m_nHeight;
    mybmh.biSize = sizeof(BITMAPINFOHEADER);
    ...
    mybmh.biCompression = BI_RGB;
    mybmh.biSizeImage =
        g_pLPreviewOutputBitmap->m_nWidth * g_pLPreviewOutputBitmap->m_nHeight;
  ...
  // Purge any translucent detail objects that were added AFTER the translucent
  // rendering loop
...
void CRender3D::RenderArrow(Vector const &vStartPt, Vector const &vEndPt,
                            unsigned char chRed, unsigned char chGreen,
...
//-----------------------------------------------------------------------------
// Purpose: Renders a box in flat shaded or wireframe depending on our render
// mode. Input  : chRed -
//			chGreen -
//			chBlue -
//-----------------------------------------------------------------------------
void CRender3D::RenderBox(const Vector &Mins, const Vector &Maxs,
  ...
  int nFaces[6][4] = {{0, 2, 3, 1}, {0, 1, 5, 4}, {4, 5, 7, 6},
  ...
  if ((eBoxSelectionState != SELECT_NONE) &&
      (GetDefaultRenderMode() != RENDER_MODE_WIREFRAME)) {
    nPasses = 2;
  } else {
    nPasses = 1;
  }
  ...
  for (int nPass = 1; nPass <= nPasses; nPass++) {
    ...
    for (int nFace = 0; nFace < 6; nFace++) {
      ...
      CMatRenderContextPtr pRenderContext(MaterialSystemInterface());
      IMesh *pMesh = pRenderContext->GetDynamicMesh();
      meshBuilder.DrawQuad(pMesh, FacePoints[nP1].Base(),
                           FacePoints[nP2].Base(), FacePoints[nP3].Base(),
                           FacePoints[nP4].Base(), color, wireframe);
    }
...
//-----------------------------------------------------------------------------
void CRender3D::RenderCone(Vector const &vBasePt, Vector const &vTipPt,
                           float fRadius, int nSlices, unsigned char chRed,
                           unsigned char chGreen, unsigned char chBlue) {
...
//			chBlue -
//-----------------------------------------------------------------------------
void CRender3D::RenderSphere(Vector const &vCenter, float flRadius, int nTheta,
                             int nPhi, unsigned char chRed,
                             unsigned char chGreen, unsigned char chBlue) {
  ...
  int nTriangles =
      2 * nTheta *
      (nPhi -
       1);  // Two extra degenerate triangles per row (except the last one)
...
// Purpose:
//-----------------------------------------------------------------------------
void CRender3D::RenderWireframeSphere(Vector const &vCenter, float flRadius,
                                      int nTheta, int nPhi, unsigned char chRed,
...
void CRender3D::RenderPointsAndPortals(void) {
  ...
  if (pDoc->m_pPortalFile) {
    ...
    int nMaxIndices;
    CMatRenderContextPtr pRenderContext(MaterialSystemInterface());
    pRenderContext->GetMaxToRender(pRenderContext->GetDynamicMesh(), false,
                                   &nMaxVerts, &nMaxIndices);
    ...
    while (totalQuads > 0) {
      ...
      for (; portalIndex < pDoc->m_pPortalFile->vertCount.Count();
           portalIndex++) {
        int vertCount = pDoc->m_pPortalFile->vertCount[portalIndex];

        if ((quadOut + vertCount) > quadLimit) break;
        quadOut += vertCount;
        // compute a face normal
        Vector e0 = pDoc->m_pPortalFile->verts[baseVert + 1] -
                    pDoc->m_pPortalFile->verts[baseVert];
        Vector e1 = pDoc->m_pPortalFile->verts[baseVert + 2] -
                    pDoc->m_pPortalFile->verts[baseVert];
        ...
        for (int j = 0; j < vertCount; j++) {
          ...
          // compute the direction in the plane of the face to extrude the edge
          // toward the face interior, use that to make a wide line with a quad
          Vector e0 =
              pDoc->m_pPortalFile->verts[v1] - pDoc->m_pPortalFile->verts[v0];
          ...
          meshBuilder.Position3fv(
              (pDoc->m_pPortalFile->verts[v1] + dir).Base());
          meshBuilder.Color3ub(0, 0, 255);
          meshBuilder.AdvanceVertex();
          meshBuilder.Position3fv(
              (pDoc->m_pPortalFile->verts[v0] + dir).Base());
          meshBuilder.Color3ub(0, 0, 255);
          meshBuilder.AdvanceVertex();
        }
        baseVert += vertCount;
...
//			chRed, chGreen, chBlue - Red, green, and blue color
//compnents for the box.
//-----------------------------------------------------------------------------
  ...
  if ((pMapClass != NULL) && (pMapClass->GetRenderFrame() != m_nFrameCount)) {
    if (pMapClass->IsVisible()) {
      ...
      if (should_appear) {
        ...
        // just add it to a list of objects to render last. Otherwise, render it
        // now.
        //
...
//			bForce - If true, don't check for visibility, just render
...
void CRender3D::RenderOverlayElements(void) {
  ...
  if (m_RenderState.bCenterCrosshair) RenderCrossHair();
...
//			bEnable - true to enable, false to disable the specified
//render state.
//-----------------------------------------------------------------------------
