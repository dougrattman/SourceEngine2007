...
#if defined(ENTITY_MAINTAIN_UPSTREAM_LISTS)
  if (m_Upstream.Find(pConnection) == -1) m_Upstream.AddToTail(pConnection);
#endif
}
...
CEditGameClass *CEditGameClass::CopyFrom(CEditGameClass *pFrom) {
  ...
  for (int i = pFrom->GetFirstKeyValue(); i != pFrom->GetInvalidKeyValue();
       i = pFrom->GetNextKeyValue(i)) {
...
//			for yaw to indicate pointing straight up or straight
// down.
//
...
void CEditGameClass::SetAngles(const QAngle &vecAngles) {
  char szAngles[80];
  sprintf(szAngles, "%g %g %g", (double)vecAngles[PITCH],
          (double)vecAngles[YAW], (double)vecAngles[ROLL]);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CEditGameClass::SaveVMF(CChunkFile *pFile,
                                          CSaveInfo *pSaveInfo) {
  ...
  //
  // Determine whether we have a game data class. This will help us decide which
  // keys to write.
  //
  ...
  //
  // If we have a game data class, for each keyvalue in the class definition,
  // write out all keys that are not present in the object and whose defaults
  // are nonzero in the class definition.
  //
  if (pGameDataClass != NULL) {
    ...
    for (int i = 0; i < nVariableCount; i++) {
      ...
      if (pVar != NULL) {
        ...
        //
        // If the variable is not present in this object, write out the default
        // value.
        //
        if (p == NULL) {
          MDkeyvalue TempKey;
    ...
    if (eResult == ChunkFile_Ok) {
      ...
      for (int i = 0; i < nConnCount; i++) {
        ...
        if (pConnection != NULL) {
          ...
          sprintf(szTemp, "%s,%s,%s,%g,%d", pConnection->GetTargetName(),
                  pConnection->GetInputName(), pConnection->GetParam(),
                  pConnection->GetDelay(), pConnection->GetTimesToFire());
...
// not.
//-----------------------------------------------------------------------------
ChunkFileResult_t CEditGameClass::LoadKeyCallback(
    const char *szKey, const char *szValue, CEditGameClass *pEditGameClass) {
  CEntityConnection *pConnection = new CEntityConnection;

  // Set the "source" name to be the name of the pEditGameClass' targetname
  pConnection->SetSourceName(pEditGameClass->GetKeyValue(
      "targetname"));  // Use the classname if no targetname is defined?
  ...
  pEditGameClass->Connections_Add(
      pConnection);  // Does this belong here or SetSourceName or
                     // LinkSourceEntities??
...
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t CEditGameClass::LoadConnectionsCallback(
    CChunkFile *pFile, CEditGameClass *pEditGameClass) {
