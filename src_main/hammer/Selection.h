...
//			manipulation. Holds the current tool. Handles GUI messages
//that are 			view-independent.
//
//=============================================================================//
...
enum { hitFirst = 0, hitNext = -1, hitPrev = -2 };
...
enum {
  ...
  scNoLift = 0x20,    // Don't lift face attributes into Face Properties dialog
                      // dvs: lame!
  scNoApply = 0x40,   // Don't apply face attributes from Face Properties dialog
                      // to selected face   dvs: lame!
  scCascading = 0x80,  // Select all entities attached to outputs of this entity
  scCascadingRecursive = 0x100,  // Select all entities attached to outputs of
                                 // this entity, recursively
  scSelectAll = 0x200,
  scSaveChanges = 0x400,  // changing the selection causes changes made in the
                          // properties dialog be saved
};
...
class CSelection {
 public:
  ...
  void SelectObjectList(const CMapObjectList *pList,
                        int cmd = (scClear | scSelect | scSaveChanges));
  ...
  // Used for translations. Uses entity origins and brush bounds. That way, when
  // moving stuff, the entity origins will stay on the grid.
 ...
 protected:
  ...
  SelectMode_t m_eSelectMode;  // Controls what gets selected based on what the
                               // user clicked on.
