...
int CIconComboBox::AddIcon(LPCTSTR pIconName) {
  ...
  if ((hIcon == (HICON)1) || !hIcon) return CB_ERR;
  ...
  if ((ndx == CB_ERR) || (ndx == CB_ERRSPACE)) return ndx;
  ...
  if (result == CB_ERR) return result;
...
int CIconComboBox::InsertIcon(LPCTSTR pIconName, int ndx) {
  ...
  if ((result == CB_ERR) || (result == CB_ERRSPACE)) return result;
