...
#include "Gizmo.h"
#include "GlobalFunctions.h"  // TODO(d.rattman): For NotifyDuplicates
#include "History.h"
#include "MainFrm.h"
#include "MapDefs.h"
#include "MapDoc.h"
...
// Output :
//-----------------------------------------------------------------------------
GDinputvariable *Selection3D::ChooseEyedropperVar(
    CMapView *pView, CUtlVector<GDinputvariable *> &VarList) {
  //
  ...
  CPoint point;
  GetCursorPos(&point);
  int nID = menu.TrackPopupMenu(
      TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD, point.x,
      point.y, NULL, NULL);
  if (nID == 0) {
    return NULL;
  }
...
//-----------------------------------------------------------------------------
int Selection3D::HitTest(CMapView *pView, const Vector2D &ptClient,
                         bool bTestHandles) {
  if (!IsEmpty()) {
...
//-----------------------------------------------------------------------------
bool Selection3D::StartBoxSelection(CMapView *pView, const Vector2D &vPoint,
                                    const Vector &vStart) {
...
//-----------------------------------------------------------------------------
void Selection3D::StartLogicalBoxSelection(CMapViewLogical *pView,
                                           const Vector &vStart) {
...
void Selection3D::TransformLogicalSelection(const Vector2D &vecTranslation) {
  ...
  // The transformation may have changed some entity properties (such as the
  // "angles" key), so we must refresh the Object Properties dialog.
  GetMainWnd()->pObjectProperties->MarkDataDirty();
}

//-----------------------------------------------------------------------------
// Purpose: Draws objects when they are selected. Odd, how this code is stuck
//			in this obscure place, away from all the other 2D rendering
//code.
...
static BOOL DrawObject(CMapClass *pobj, CRender *pRender) {
  ...
  if (pRender2D) pobj->Render2D(pRender2D);

  CRender3D *pRender3D = dynamic_cast<CRender3D *>(pRender);

  if (pRender3D) pobj->Render3D(pRender3D);
...
static BOOL DrawObjectLogical(CMapClass *pObj, CRender2D *pRender2D) {
  if (!pObj->IsVisibleLogical()) return true;
...
void Selection3D::RenderToolLogical(CRender2D *pRender) {
  if (!m_pSelection->IsEmpty() && m_bIsLogicalTranslating &&
      !IsLogicalBoxSelecting()) {
    // Even if this is not the active tool, selected objects should be rendered
    // with the selection color.
    COLORREF clr = Options.colors.clrSelection;
    ...
    VMatrix matrix = GetTransformMatrix();
    MatrixBuildTranslation(matrix, m_vLogicalTranslation.x,
                           m_vLogicalTranslation.y, 0.0f);
    pRender->BeginLocalTransfrom(matrix);
    ...
    for (int i = 0; i < pSelList->Count(); i++) {
      ...
      pobj->EnumChildren((ENUMMAPCHILDRENPROC)DrawObjectLogical,
                         (DWORD)pRender);
    }
  ...
  pRender->SetDrawColor(GetRValue(Options.colors.clrToolDrag),
                        GetGValue(Options.colors.clrToolDrag),
                        GetBValue(Options.colors.clrToolDrag));
...
CBaseTool *Selection3D::GetToolObject(CMapView2D *pView, const Vector2D &vPoint,
                                      bool bAttach) {
  const CMapObjectList *pSelList = m_pSelection->GetList();
  for (int i = 0; i < pSelList->Count(); i++) {
    ...
    if (pObject->HitTest2D(pView, vPoint, HitData)) {
      ...
      //
      CBaseTool *pToolHit =
          HitData.pObject->GetToolObject(HitData.uData, bAttach);
      if (pToolHit != NULL) {
        return pToolHit;
      }
    }
...
CBaseTool *Selection3D::GetToolObjectLogical(CMapViewLogical *pView,
...
//			point -
//-----------------------------------------------------------------------------
bool Selection3D::OnContextMenu2D(CMapView2D *pView, UINT nFlags,
                                  const Vector2D &vPoint) {
  ...
  if (!IsEmpty() && !IsBoxSelecting()) {
    if (HitTest(pView, vPoint, false)) {
      ...
      menuSelection.TrackPopupMenu(
          TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_LEFTALIGN, ptScreen.x,
          ptScreen.y, pView);
      return true;
    }
  }
...
//			point -
//-----------------------------------------------------------------------------
bool Selection3D::OnContextMenuLogical(CMapViewLogical *pView, UINT nFlags,
                                       const Vector2D &vPoint) {
  // First give any selected tool helpers a chance to handle the message.
  // Don't hit test against tool helpers when shift is held down
  // (beginning a Clone operation).
  CBaseTool *pToolHit = GetToolObjectLogical(pView, vPoint, true);
  if (pToolHit) return pToolHit->OnContextMenuLogical(pView, nFlags, vPoint);
...
// Purpose:
//-----------------------------------------------------------------------------
void Selection3D::NudgeObjects(CMapView *pView, int nChar, bool bSnap,
                               bool bClone) {
  ...
  // Make sure it can still fit entirely in the view after nudging and don't
  // scroll the view if it can't. This second check is probably unnecessary, but
  // it can't hurt, and there might be cases where the selection changes size
  // after a nudge operation.
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool Selection3D::OnKeyDown2D(CMapView2D *pView, UINT nChar, UINT nRepCnt,
                              UINT nFlags) {
  bool bShift = ((GetKeyState(VK_SHIFT) & 0x8000) != 0);
  bool bCtrl = ((GetKeyState(VK_CONTROL) & 0x8000) != 0);

  if (Options.view2d.bNudge && (nChar == VK_UP || nChar == VK_DOWN ||
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool Selection3D::OnKeyDownLogical(CMapViewLogical *pView, UINT nChar,
                                   UINT nRepCnt, UINT nFlags) {
  bool bShift = ((GetKeyState(VK_SHIFT) & 0x8000) != 0);
  bool bAlt = GetKeyState(VK_MENU) < 0;

  /* TODO(d.rattman):
  if ( Options.view2d.bNudge && ( nChar == VK_UP || nChar == VK_DOWN || nChar ==
  VK_LEFT || nChar == VK_RIGHT ) )
  {
          if (!IsEmpty())
          {
  ...
  */
  switch (nChar) {
    ...
    case VK_NEXT:
      m_pDocument->OnCmdMsg(bAlt ? ID_EDIT_SELNEXT_CASCADING : ID_EDIT_SELNEXT,
                            CN_COMMAND, NULL, NULL);
      break;

    case VK_PRIOR:
      m_pDocument->OnCmdMsg(bAlt ? ID_EDIT_SELPREV_CASCADING : ID_EDIT_SELPREV,
                            CN_COMMAND, NULL, NULL);
      break;
...
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseDown2D(CMapView2D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  // First give any selected tool helpers a chance to handle the message.
  // Don't hit test against tool helpers when shift is held down
  // (beginning a Clone operation).
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnMouseMove2D(CMapView2D *pView, UINT nFlags,
                                const Vector2D &vPoint) {
  ...
  //
  char szBuf[128];
  sprintf(szBuf, " @%.0f, %.0f ", vecWorld[pView->axHorz],
          vecWorld[pView->axVert]);
  SetStatusText(SBI_COORDS, szBuf);

  //
  // If we are currently dragging the selection (moving, scaling, rotating, or
  // shearing) update that operation based on the current cursor position and
  // keyboard state.
  //
  if (IsTranslating()) {
  ...
  //
  // Else if we have just started dragging the selection, begin a new
  // translation
  //
    ...
    if (!bCtrl && HitTest(pView, m_vMouseStart[MOUSE_LEFT], true)) {
      ...
      if (uConstraints & constrainSnap) m_pDocument->Snap(ptOrg, uConstraints);
    ...
    //
    // Just in case the selection set is not empty and "selection" hasn't
    // received a left mouse click. (NOTE: this is gross, but unfortunately
    // necessary (cab))
    //
    UpdateHandleState();
    ...
    if (GetToolObject(pView, vPoint, false)) {
      // If they moused over an interactive handle, it should have set the
      // cursor.
      hCursor = vgui::dc_crosshair;
      bFoundTool = true;
    }

    // If we haven't moused over any interactive handles contained in the
    // object, see if the mouse is over one of the selection handles.
...
void Selection3D::StartTranslation(CMapView *pView, const Vector2D &vPoint,
  ...
  // If translating, redo our bounds temporarily to use the entity origins
  // rather than their bounds so things will stay on the grid correctly.
  Vector vCustomHandleBox[2];
  Vector *pCustomHandleBox = NULL;
  if (vHandleOrigin.IsZero()) {
    pCustomHandleBox = vCustomHandleBox;
    m_pSelection->GetBoundsForTranslation(vCustomHandleBox[0],
                                          vCustomHandleBox[1]);
  }
  Box3D::StartTranslation(pView, vPoint, vHandleOrigin, pRefPoint,
                          pCustomHandleBox);
  if (!m_pSelection->IsEmpty()) UpdateSelectionBounds();
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseUp2D(CMapView2D *pView, UINT nFlags,
                               const Vector2D &vPoint) {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseDownLogical(CMapViewLogical *pView, UINT nFlags,
                                      const Vector2D &vPoint) {
  // First give any selected tool helpers a chance to handle the message.
  // Don't hit test against tool helpers when shift is held down
  // (beginning a Clone operation).
  ...
  // If they weren't alt- or ctrl-clicking and we have a selection, if they
  // clicked in the selection rectangle, maintain what we got.
  bool bCtrlClick = (nFlags & MK_CONTROL) != 0;
  bool bAltClick = GetKeyState(VK_MENU) < 0;
  if (!bAltClick && !bCtrlClick && !IsEmpty()) {
    if (HitTestLogical(pView, vPoint)) return true;
  }
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnMouseMoveLogical(CMapViewLogical *pView, UINT nFlags,
                                     const Vector2D &vPoint) {
  if (m_bLButtonDown) {
    if (!m_bLeftDragged) {
      ...
      if ((abs(sizeDragged.x) > DRAG_THRESHHOLD) ||
          (abs(sizeDragged.y) > DRAG_THRESHHOLD)) {
  ...
  // update that operation based on the current cursor position and keyboard
  // state.
  ...
  if (m_bLButtonDown && (nFlags & MK_LBUTTON) && m_bLeftDragged) {
    ...
    // Check to see if the point at which we started clicking lies within the
    // selection region
  ...
  // If we are simply hovering over an object but the mouse isn't down, update
  // the cursor.
  vgui::HCursor hCursor = vgui::dc_arrow;
  if (!IsEmpty()) {
    // If the cursor is on a handle, the cursor will be set by the HitTest code.
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseUpLogical(CMapViewLogical *pView, UINT nFlags,
                                    const Vector2D &vPoint) {
  bool bShift = ((GetKeyState(VK_SHIFT) & 0x8000) != 0);
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnKeyDown3D(CMapView3D *pView, UINT nChar, UINT nRepCnt,
                              UINT nFlags) {
  bool bShift = ((GetKeyState(VK_SHIFT) & 0x8000) != 0);
  bool bCtrl = ((GetKeyState(VK_CONTROL) & 0x8000) != 0);
  ...
  switch (nChar) {
    /*
    dvs: The eyedropper is a somewhat failed experiment, an attempt to create a
    way to quickly hook entities together. I think a dedicated connection tool
    with a more rubber-band style UI might be more successful. Either that or
    relegate that work to a Logical-style view. case 'e': case 'E':
    {
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseDblClk3D(CMapView3D *pView, UINT nFlags,
                                   const Vector2D &vPoint) {
...
bool Selection3D::OnLMouseDblClkLogical(CMapViewLogical *pView, UINT nFlags,
                                        const Vector2D &vPoint) {
  if (!m_pSelection->IsEmpty()) {
    GetMainWnd()->pObjectProperties->ShowWindow(SW_SHOW);
  }
...
void Selection3D::EyedropperPick3D(CMapView3D *pView, const Vector2D &vPoint) {
  ...
  if (!m_pSelection->IsAnEntitySelected()) {
    MessageBox(
        NULL,
        "No entities are selected, so the eyedropper has nothing to assign to.",
        "No selected entities", MB_OK);
    return;
  }
...
void Selection3D::EyedropperPick(CMapView *pView, CMapClass *pObject) {
  ...
  if (pszClickName == NULL) {
    ...
    //
    MessageBox(NULL, "The chosen entity has no name.", "No name to pick",
               MB_OK);
    return;
  }

  //
  // Build a list of all the keyvalues in the selected entities that support the
  // eyedropper.
  //
  ...
  for (int i = 0; i < pSelList->Count(); i++) {
    ...
    if (pEntity != NULL) {
      ...
      for (int nVar = 0; nVar < nVarCount; nVar++) {
        ...
        if (pVar && ((pVar->GetType() == ivTargetDest) ||
  ...
  if (nCount <= 0) {
    //
    // No selected entities have keys of the appropriate type, so there's
    // nothing we can do.
    //
    MessageBox(NULL,
               "No selected entities have keyvalues that accept an entity "
               "name, so the eyedropper has nothing to assign to.",
               "No eligible keyvalues", MB_OK);
    return;
  }
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseDown3D(CMapView3D *pView, UINT nFlags,
                                 const Vector2D &vPoint) {
  ...
  //
  // If they are holding down the eyedropper hotkey, do an eyedropper pick. The
  // eyedropper fills out keyvalues in selected entities based on the object
  // they clicked on.
  //
  if (m_bEyedropper) {
  ...
  if (nFlags & MK_CONTROL) {
    ...
    if (!IsBoxSelecting() && m_LastHitTestHandle == vec3_origin) {
      //  clicked somewhere on our selection tool but maybe something else is
      //  inbetween
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnLMouseUp3D(CMapView3D *pView, UINT nFlags,
                               const Vector2D &vPoint) {
  bool bShift = nFlags & MK_SHIFT;
...
// Output : Returns true if the message was handled, false if not.
//-----------------------------------------------------------------------------
bool Selection3D::OnMouseMove3D(CMapView3D *pView, UINT nFlags,
                                const Vector2D &vPoint) {
...
void Selection3D::SetEyedropperCursor(void) {
  ...
  if (!hcurEyedropper) {
    hcurEyedropper =
        LoadCursor(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_EYEDROPPER));
  }
