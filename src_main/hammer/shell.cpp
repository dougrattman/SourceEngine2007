...
//-----------------------------------------------------------------------------
typedef bool (CShell::*ShellHandlerFunc_t)(const char *pszCommand,
...
//			and version as the active document. This prevents
// problems
// with 			editing out of sync versions of the map via the
// engine.
...
bool CShell::DoVersionCheck(const char *pszArguments) {
  if (m_pDoc != NULL) {
    ...
    if (sscanf_s(pszArguments, "%s %d", szEngineMapPath,
                 SOURCE_ARRAYSIZE(szEngineMapPath), &nEngineMapVersion) == 2) {
      ...
      if (!_stricmp(szDocName, szEngineMapName) &&
          (nDocVersion == nEngineMapVersion)) {
        return (true);
      }
    }
  }
...
//			pszArguments - Class name of entity and x, y, z
// coordinate at which 				to create it.
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CShell::EntityCreate(const char *pszCommand, const char *pszArguments) {
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    ...
    if (sscanf_s(pszArguments, "%s %f %f %f", szClassName,
                 SOURCE_ARRAYSIZE(szClassName), &x, &y, &z) == 4) {
...
bool CShell::EntityRotateIncremental(const char *pszCommand,
                                     const char *pszArguments) {
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    ...
    float z;
    QAngle rotation;
    char szArgs[NUM_ROTATE_INCREMENTAL_ARGS]
               [512];  // classname, x, y, z, ax, ay, az
    char token[1024];
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CShell::EntitySetKeyValue(const char *pszCommand,
                               const char *pszArguments) {
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    ...
    if (arg == NUM_KEY_VALUE_ARGS) {
      ...
      if (pEntity != NULL) {
        if (!Q_stricmp(szArgs[4], "origin")) {
          Vector origin;
          sscanf_s(szArgs[5], "%f %f %f", &origin[0], &origin[1], &origin[2]);
          Vector oldOrigin;
          pEntity->GetOrigin(oldOrigin);
          pEntity->TransMove(origin - oldOrigin);
        } else if (pEntity->IsSolidClass() && !Q_stricmp(szArgs[4], "angles")) {
          QAngle angles;
          sscanf_s(szArgs[5], "%f %f %f", &angles[0], &angles[1], &angles[2]);

          // build a relative transform from the previous state to the current
          // state NOTE: This only works once since solid classes destructively
          // modify transform info (GetAngles always returns identity) NOTE: Use
          // rotateIncremental instead!
          QAngle oldAngles;
...
//			pszArguments - Class name of node to create, ID to
// assign
// it, and 			x, y, z coordinate at which to create the node.
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CShell::NodeCreate(const char *pszCommand, const char *pszArguments) {
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    ...
    if (sscanf_s(pszArguments, "%s %d %f %f %f", szClassName,
                 SOURCE_ARRAYSIZE(szClassName), &nID, &x, &y, &z) == 5) {
...
bool CShell::NodeDelete(const char *pszCommand, const char *pszArguments) {
  ...
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    char szID[80];
    if (sscanf_s(pszArguments, "%s", szID, SOURCE_ARRAYSIZE(szID)) == 1) {
...
//			pszArguments - Node ids of start and end nodes, space
// delimited.
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CShell::NodeLinkCreate(const char *pszCommand, const char *pszArguments) {
  if ((m_pDoc != NULL) && m_pDoc->IsShellSessionActive()) {
    ...
    if (sscanf_s(pszArguments, "%s %s", szIDStart, SOURCE_ARRAYSIZE(szIDStart),
                 szIDEnd, SOURCE_ARRAYSIZE(szIDEnd)) == 2) {
      //
      // It doesn't matter where we place it because it will move to the
      // midpoint of the start and end entities.
      //
      ...
      if (m_pDoc->FindEntitiesByClassName(Found, "info_node_link", false)) {
        FOR_EACH_OBJ(Found, pos) {
          ...
          if ((pszNode1 != NULL) && (pszNode2 != NULL)) {
            if (((!_stricmp(pszNode1, szIDStart)) &&
                 (!_stricmp(pszNode2, szIDEnd))) ||
                ((!_stricmp(pszNode1, szIDEnd)) &&
                 (!_stricmp(pszNode2, szIDStart)))) {
              m_pDoc->DeleteObject(pEntity);
              bFound = true;
            }
          }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CShell::ReleaseVideoMemory(const char *pszCommand,
                                const char *pszArguments) {
...
bool CShell::RunCommand(const char *pszCommand) {
  for (int nCommand = 0;
       nCommand < sizeof(m_DispatchTable) / sizeof(m_DispatchTable[0]);
       nCommand++) {
    int nCommandLen = strlen(m_DispatchTable[nCommand].pszCommand);

    if (!_strnicmp(pszCommand, m_DispatchTable[nCommand].pszCommand,
                   nCommandLen)) {
      return ((this->*m_DispatchTable[nCommand].pfnHandler)(
          m_DispatchTable[nCommand].pszCommand, &pszCommand[nCommandLen]));
    }
  }
