...
//			or more views. If it contains more than one view, there
//is
// a
// splitter 			seperating the views. All views in a given MDI
// child window reflect 			a single document.
//
//===========================================================================//
...
ON_UPDATE_COMMAND_UI(ID_VIEW_MAXIMIZERESTOREACTIVEVIEW,
                     OnUpdateViewMaximizepane)
...
// Input  : b4Views -
//-----------------------------------------------------------------------------
void SetDefaultChildType(BOOL b4Views) { g_b4Views = b4Views; }
...
void CChildFrame::SetSplitterMode(BOOL bSplitter) {
  if (bSplitter == bUsingSplitter) return;  // already at this mode
  ...
  if (!bSplitter) {
    ...
    m_wndSplitter->CreateView(0, 1, RUNTIME_CLASS(CMapView2D), sizeView,
                              &context);
    m_wndSplitter->CreateView(1, 0, RUNTIME_CLASS(CMapView2D), sizeView,
                              &context);
    m_wndSplitter->CreateView(1, 1, RUNTIME_CLASS(CMapView2D), sizeView,
                              &context);

    context.m_pNewViewClass = RUNTIME_CLASS(CMapView3D);

    m_wndSplitter->CreateView(0, 0, context.m_pNewViewClass, sizeView,
                              &context);
  }
...
CView *CChildFrame::ReplaceView(CRuntimeClass *pViewClass) {
  ...
  if (bUsingSplitter) {
    if (m_wndSplitter->CreateView(iRow, iCol, pViewClass, rect.Size(),
                                  &context)) {
    ...
    if (pNewView) {
      ...
      if (!pNewView->Create(NULL, NULL, AFX_WS_DEFAULT_VIEW, r, this,
  ...
  if (bUsingSplitter) {
    ...
    for (int nRow = 0; nRow < 2; nRow++) {
      for (int nCol = 0; nCol < 2; nCol++) {
        CMapView *pView =
            dynamic_cast<CMapView *>(m_wndSplitter->GetPane(nRow, nCol));
        if (pView != NULL) {
          char szKey[30];
    ...
    sprintf(szPlacement, "(%d %d) (%d %d) (%d %d %d %d) %d", wp.ptMaxPosition.x,
...
//-----------------------------------------------------------------------------
void CChildFrame::OnView2dxy(void) { SetViewType(VIEW2D_XY); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_2DYZ command when the active view is a 3D view.
//-----------------------------------------------------------------------------
void CChildFrame::OnView2dyz(void) { SetViewType(VIEW2D_YZ); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_2DXZ command when the active view is a 3D view.
//-----------------------------------------------------------------------------
void CChildFrame::OnView2dxz(void) { SetViewType(VIEW2D_XZ); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_2DLOGICAL command when the active view is a 3D
// view.
//-----------------------------------------------------------------------------
void CChildFrame::OnViewLogical(void) { SetViewType(VIEW_LOGICAL); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_3DWIREFRAME command when the active view is a 2D
// view.
//-----------------------------------------------------------------------------
void CChildFrame::OnView3dWireframe(void) { SetViewType(VIEW3D_WIREFRAME); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_3DPOLYGON command when the active view is a 2D
// view.
//-----------------------------------------------------------------------------
void CChildFrame::OnView3dPolygon(void) { SetViewType(VIEW3D_POLYGON); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_3DTEXTURED command when the active view is a 2D
// view.
//-----------------------------------------------------------------------------
void CChildFrame::OnView3dTextured(void) { SetViewType(VIEW3D_TEXTURED); }
...
//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_3DSMOOTH command when the active view is a 3D
// view.
//-----------------------------------------------------------------------------
void CChildFrame::OnView3dSmooth(void) { SetViewType(VIEW3D_SMOOTHING_GROUP); }

//-----------------------------------------------------------------------------
// Purpose: Handles the ID_VIEW_3DENGINE command when the active view is a 2D
// view.
//-----------------------------------------------------------------------------
...
//			the splitter window is createed and one 3D view and
//three 2D views 			are added to it.
// Input  : lpcs -
//			pContext -
// Output : Returns TRUE on success, FALSE on failure.
//-----------------------------------------------------------------------------
BOOL CChildFrame::OnCreateClient(LPCREATESTRUCT lpcs,
  ...
  if (bUsingSplitter) {
    ...
    eDrawType[0][0] = (DrawType_t)APP()->GetProfileInt(
        "Splitter", "DrawType0,0", VIEW3D_WIREFRAME);
    eDrawType[0][1] =
        (DrawType_t)APP()->GetProfileInt("Splitter", "DrawType0,1", VIEW2D_XY);
    eDrawType[1][0] =
        (DrawType_t)APP()->GetProfileInt("Splitter", "DrawType1,0", VIEW2D_YZ);
    eDrawType[1][1] =
        (DrawType_t)APP()->GetProfileInt("Splitter", "DrawType1,1", VIEW2D_XZ);
    ...
    for (int nRow = 0; nRow < 2; nRow++) {
      for (int nCol = 0; nCol < 2; nCol++) {
        ...
        switch (eDrawType[nRow][nCol]) {
          ...
          case VIEW2D_XZ:
          case VIEW2D_YZ: {
            m_wndSplitter->CreateView(nRow, nCol, RUNTIME_CLASS(CMapView2D),
                                      sizeView, pContext);
            break;
          }
          ...
          case VIEW_LOGICAL: {
            m_wndSplitter->CreateView(
                nRow, nCol, RUNTIME_CLASS(CMapViewLogical), sizeView, pContext);
            break;
          }
          ...
          case VIEW3D_LIGHTING_PREVIEW_RAYTRACED:
          case VIEW3D_SMOOTHING_GROUP: {
            m_wndSplitter->CreateView(nRow, nCol, RUNTIME_CLASS(CMapView3D),
                                      sizeView, pContext);
            break;
          }
        }
...
void CChildFrame::CenterViews(void) {
  ...
  sizeView.cy = std::max(0L, sizeView.cy);
  sizeView.cx = std::max(0L, sizeView.cx);
...
int CChildFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  if (bUsingSplitter) {
    ...
    if (!str.IsEmpty()) {
      ...
      wp.length = sizeof(wp);
      wp.flags = 0;
      sscanf_s(str, "(%d %d) (%d %d) (%d %d %d %d) %d", &wp.ptMaxPosition.x,
               &wp.ptMaxPosition.y, &wp.ptMinPosition.x, &wp.ptMinPosition.y,
               &wp.rcNormalPosition.bottom, &wp.rcNormalPosition.left,
...
// Input  : *pCmdUI -
//-----------------------------------------------------------------------------
void CChildFrame::OnUpdateViewMaximizepane(CCmdUI *pCmdUI) { pCmdUI->Enable(); }
...
void CChildFrame::OnSetFocus(CWnd *pOldWnd) {
  ...
  if (pDoc) CMapDoc::SetActiveMapDoc(pDoc);
}
