...
//-----------------------------------------------------------------------------
bool GetPersistentEnvironmentVariable(const char *pName, char *pReturn,
                                      int size) {
  // Open the key
  HKEY hregkey;
  ...
  // Get the value
  DWORD dwSize = size;
  if (RegQueryValueEx(hregkey, pName, NULL, NULL, (LPBYTE)pReturn, &dwSize) !=
      ERROR_SUCCESS)
    return false;
...
void SetPersistentEnvironmentVariable(const char *pName, const char *pValue) {
  ...
  // Open the key
  if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, VPROJECT_REG_KEY, 0, KEY_ALL_ACCESS,
                   &hregkey) != ERROR_SUCCESS)
    return;

  // Set the value to the string passed in
  RegSetValueEx(hregkey, pName, 0, REG_SZ, (const unsigned char *)pValue,
                (int)strlen(pValue));
...
void EditorUtil_ConvertPath(CString &str, bool bExpand) {
  ...
  if (bExpand) {
    ...
    if (Q_StrSubst(str, szSteamUserDirToken, strSteamUserDir, szPathOut,
                   sizeof(szPathOut))) {
      str = szPathOut;
    }

    if (Q_StrSubst(str, szSteamDirToken, strSteamDir, szPathOut,
                   sizeof(szPathOut))) {
      str = szPathOut;
    }
  } else {
    // Replace the full strings with the tokens
    // Go from longest paths to shortest paths to insure the most brief
    // expression.
    if (Q_StrSubst(str, strSteamUserDir, szSteamUserDirToken, szPathOut,
                   sizeof(szPathOut))) {
      str = szPathOut;
    }

    if (Q_StrSubst(str, strSteamDir, szSteamDirToken, szPathOut,
                   sizeof(szPathOut))) {
      str = szPathOut;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Exchanges path data between a CString and an edit control,
// converting
//			the Steam path to %STEAM% and back.
//-----------------------------------------------------------------------------
void EditorUtil_TransferPath(CDialog *pDlg, int nIDC, char *szDest,
                             bool bExpand) {
...
void COPTConfigs::SaveInfo(CGameConfig *pConfig) {
  ...
  if (nIndex != CB_ERR) {
    pConfig->SetTextureFormat(
        (TEXTUREFORMAT)m_cTextureFormat.GetItemData(nIndex));
  } else {
    pConfig->SetTextureFormat(tfVMT);
  }

  m_cDefaultSolid.GetWindowText(pConfig->szDefaultSolid,
                                sizeof pConfig->szDefaultSolid);
  m_cDefaultPoint.GetWindowText(pConfig->szDefaultPoint,
                                sizeof pConfig->szDefaultPoint);
...
void COPTConfigs::OnSelchangeConfigurations(void) {
  ...
  CGameConfig *pConfig =
      Options.configs.FindConfig(m_cConfigs.GetItemData(iCurSel));
  ...
  for (int i = 0; i < nItems; i++) {
    if (pConfig->GetTextureFormat() ==
        (TEXTUREFORMAT)m_cTextureFormat.GetItemData(i)) {
  ...
  SetDlgItemInt(IDC_DEFAULT_LIGHTMAP_SCALE, pConfig->GetDefaultLightmapScale(),
                FALSE);
...
BOOL COPTConfigs::OnInitDialog(void) {
  ...
  m_pInitialSelectedConfig =
      Options.configs.FindConfig(m_cConfigs.GetItemData(nCurSel));
...
bool COPTConfigs::ConfigChanged(CGameConfig *pConfig) {
  if (m_pInitialSelectedConfig != pConfig) return true;

  if (pConfig && m_pInitialSelectedConfig) {
    if (m_pInitialSelectedConfig->dwID != pConfig->dwID) return true;

    if (m_strInitialGameDir.CompareNoCase(pConfig->m_szModDir)) return true;
  }
...
BOOL COPTConfigs::OnApply(void) {
  ...
  if (pConfig != NULL && ConfigChanged(pConfig)) {
    ...
    AfxMessageBox(
        "Your changes to the active configuration will not take effect until "
        "the next time you run Hammer.");
  }
...
//-----------------------------------------------------------------------------
// Purpose: Handles the notification from the VCONFIG that the active config has
// changed.
//-----------------------------------------------------------------------------
