...
class CPreviewImagePropertiesCache {
 public:
  //-----------------------------------------------------------------------------
  // Purpose: Anyone can call this instead of
  // IMaterial::GetPreviewImageProperties and it'll be a lot faster if there are
  // redundant calls to it.
  //-----------------------------------------------------------------------------
  static PreviewImageRetVal_t GetPreviewImageProperties(
      IMaterial *pMaterial, int *width, int *height, ImageFormat *imageFormat,
      bool *isTranslucent) {
    ...
    if (i == s_PreviewImagePropertiesCache.InvalidIndex()) {
      ...
      entry.m_RetVal = pMaterial->GetPreviewImageProperties(
          &entry.m_Width, &entry.m_Height, &entry.m_ImageFormat,
          &entry.m_bIsTranslucent);
      i = s_PreviewImagePropertiesCache.Insert(pMaterial, entry);
    }

    CPreviewImagePropertiesCache::CEntry &entry =
        s_PreviewImagePropertiesCache[i];
    *width = entry.m_Width;
 ...
 private:
  ...
  static CUtlMap<IMaterial *, CPreviewImagePropertiesCache::CEntry>
      s_PreviewImagePropertiesCache;
};
CUtlMap<IMaterial *, CPreviewImagePropertiesCache::CEntry>
    CPreviewImagePropertiesCache::s_PreviewImagePropertiesCache(
        64, 64, &CPreviewImagePropertiesCache::PreviewImageLessFunc);
...
//			client code when a pointer to the model is copied,
// making that 			reference independent.
...
//			removing it from the cache if its reference count
// becomes zero.
...
//-----------------------------------------------------------------------------
bool CMaterial::LoadMaterialsInDirectory(char const *pDirectoryName,
                                         int nDirectoryNameLen,
                                         IMaterialEnumerator *pEnum,
  ...
  const char *pFileName =
      g_pFullFileSystem->FindFirstEx(pWildCard, "GAME", &findHandle);
  while (pFileName) {
    if (!g_pFullFileSystem->FindIsDirectory(findHandle)) {
      ...
      Q_snprintf(pFileNameWithPath, nAllocSize, "%s/%s",
                 &pDirectoryName[MATERIAL_PREFIX_LEN], pFileName);
...
bool CMaterial::InitDirectoryRecursive(char const *pDirectoryName,
                                       IMaterialEnumerator *pEnum, int nContext,
                                       int nFlags) {
  ...
  if (!LoadMaterialsInDirectory(pDirectoryName, nDirectoryNameLen, pEnum,
                                nContext, nFlags))
    return false;
  ...
  while (pFileName) {
    if ((pFileName[0] != '.') || (pFileName[1] != '.' && pFileName[1] != 0)) {
      if (g_pFullFileSystem->FindIsDirectory(findHandle)) {
        ...
        char *pFileNameWithPath =
            (char *)stackalloc(nPathStrLen + fileNameStrLen + 1);
        memcpy(pFileNameWithPath, pWildCard, nPathStrLen);
        pFileNameWithPath[nPathStrLen] = '\0';
        Q_strncat(pFileNameWithPath, pFileName,
                  nPathStrLen + fileNameStrLen + 1);
...
// that we can load up at a later time
//-----------------------------------------------------------------------------
void CMaterial::EnumerateMaterials(IMaterialEnumerator *pEnum,
                                   const char *szRoot, int nContext,
                                   int nFlags) {
...
bool CMaterial::ShouldSkipMaterial(const char *pszName, int nFlags) {
  ...
  for (int i = 0; i < g_pGameConfig->m_MaterialExcludeCount; i++) {
    ...
    const char *pMatchFound =
        Q_stristr(pszName, g_pGameConfig->m_szMaterialExcludeDirs[i]);
    if (pMatchFound == pszName) return true;
  }
...
//			material did not exist.
//-----------------------------------------------------------------------------
CMaterial *CMaterial::CreateMaterial(const char *pszMaterialName,
                                     bool bLoadImmediately, bool *pFound) {
...
bool CMaterial::LoadMaterial() {
  ...
  if (!m_bLoaded) {
    ...
    IMaterial *pMat =
        materials->FindMaterial(m_szFileName, TEXTURE_GROUP_OTHER);
    if (IsErrorMaterial(pMat)) bFound = false;
    ...
    if (!LoadMaterialHeader(pMat)) {
      // dvs: yeaaaaaaaaah, we're gonna disable this until the spew can be
      // reduced
...
void CMaterial::Reload(bool bFullReload) {
  ...
  int width, height;
  retVal = m_pMaterial->GetPreviewImageProperties(
      &width, &height, &eImageFormat, &translucentBaseTexture);
  if (retVal == MATERIAL_PREVIEW_IMAGE_BAD) return;
...
void CMaterial::DrawIcon(CDC *pDC, CMaterial *pIcon, RECT &dstRect) {
  ...
  RECT rect, dst;
  rect.left = 0;
  rect.right = pIcon->GetWidth();

  // TODO(d.rattman): Workaround the fact that materials must be power of 2, I
  // want 12 bite
  rect.top = 2;
  rect.bottom = pIcon->GetHeight() - 2;
...
void CMaterial::DrawBrowserIcons(CDC *pDC, RECT &dstRect, bool detectErrors) {
  ...
  if (!pTranslucentIcon) {
    ...
    pBaseAlphaEnvMapMaskIcon =
        CreateMaterial("editor/basealphaenvmapmaskicon", true);
    pErrorIcon = CreateMaterial("editor/erroricon", true);

    Assert(pTranslucentIcon && pOpaqueIcon && pSelfIllumIcon &&
           pBaseAlphaEnvMapMaskIcon && pErrorIcon);
  }
...
void CMaterial::DrawBitmap(CDC *pDC, RECT &srcRect, RECT &dstRect) {
  ...
  if (StretchDIBits(pDC->m_hDC, dstRect.left, dstRect.top, dest_width,
                    dest_height, srcRect.left, -srcRect.top, srcWidth,
                    srcHeight, m_pData, (BITMAPINFO *)&bmi, DIB_RGB_COLORS,
                    SRCCOPY) == GDI_ERROR) {
...
//-----------------------------------------------------------------------------
void CMaterial::Draw(
    CDC *pDC, RECT &rect, int iFontHeight, int iIconHeight,
    DrawTexData_t &DrawTexData)  //, BrowserData_t *pBrowserData)
{
...
//			material.
// Input  : pszKeywords - Buffer to receive keywords, NULL to query string
// length. Output : Returns the number of characters in the keyword string.
//-----------------------------------------------------------------------------
...
bool CMaterial::LoadMaterialHeader(IMaterial *pMat) {
  ...
  ImageFormat eImageFormat;
  int width, height;
  retVal = CPreviewImagePropertiesCache::GetPreviewImageProperties(
      pMat, &width, &height, &eImageFormat, &translucentBaseTexture);
  if (retVal == MATERIAL_PREVIEW_IMAGE_BAD) return false;
...
//-----------------------------------------------------------------------------
// Purpose: Returns the full path of the file from which this material was
// loaded.
//-----------------------------------------------------------------------------
...
bool CMaterial::IsWater(void) const {
  ...
  if (bFound) {
    if (!strcmp("water", pVar->GetStringValue())) return true;
  }
...
//				pointer is NULL, no data is copied, only the
//data size is returned.
...
int CMaterial::GetImageDataRGB(void *pImageRGB) {
  ...
  if (pImageRGB != NULL) {
    ...
    for (; src < (unsigned char *)m_pData + m_nWidth * m_nHeight * 3;
         src += 3, dst += 3) {
      dst[0] = src[2];
      dst[1] = src[1];
      dst[2] = src[0];
...
bool CMaterial::LoadMaterialImage(void) {
  ...
  PreviewImageRetVal_t retVal;
  retVal = m_pMaterial->GetPreviewImage((unsigned char *)m_pData, m_nWidth,
                                        m_nHeight, imageFormat);
...
static char const *s_rt_names[] = {"_rt_albedo",   "_rt_normal",
                                   "_rt_position", "_rt_flags",
...
static CTextureReference sg_ExtraFP16Targets[std::size(s_rt_names)];
...
void AllocateLightingPreviewtextures(void) {
  ...
  if (!bHaveAllocated) {
    ...
    for (int idx = 0; idx < std::size(sg_ExtraFP16Targets); idx++)
      sg_ExtraFP16Targets[idx].Init(
          materials->CreateNamedRenderTargetTextureEx2(
              s_rt_names[idx], 512, 512, RT_SIZE_DEFAULT, s_rt_formats[idx],
...
bool CMaterial::Initialize(HWND hwnd) {
  ...
  // Create a cache for material images (for browsing and uploading to the
  // driver).
  if (g_pMaterialImageCache == NULL) {
    g_pMaterialImageCache = new CMaterialImageCache(500);
    if (g_pMaterialImageCache == NULL) return false;
  }
  ...
  g_materialSystemConfig.m_VideoMode.m_Width =
      g_materialSystemConfig.m_VideoMode.m_Height = 0;
...
void CMaterial::ShutDown(void) {
  for (usize i = 0; i < std::size(sg_ExtraFP16Targets); ++i) {
