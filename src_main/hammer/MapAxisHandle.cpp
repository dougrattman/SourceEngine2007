...
//			When selected, it exposes handles for the endpoints of the
//axis.
//
...
//-----------------------------------------------------------------------------
CMapClass *CMapAxisHandle::Create(CHelperInfo *pHelperInfo,
                                  CMapEntity *pParent) {
  static const char *pszDefaultKeyName = "axis";
...
//			pfMaxs -
//-----------------------------------------------------------------------------
CMapAxisHandle::CMapAxisHandle(void) { Initialize(); }
...
// Output : CMapClass
//-----------------------------------------------------------------------------
CMapClass *CMapAxisHandle::CopyFrom(CMapClass *pObject,
...
CBaseTool *CMapAxisHandle::GetToolObject(int nHitData, bool bAttachObject) {
  ...
  // move points around, but that would require some way for the CMapAxisHandle
  // to know when the CMapPointHandle's position changes. This way the
  // CToolAxisHandle can handle the notification. In general, we need a better
  // system for building complex objects from simple ones and handling changes
  // to the simple objects in the complex one.
  //
  // If we DID use a CToolPointHandle, we'd need to reconcile the status bar
  // updates that are done in OnMouseMove2D, because points and axes cause
  // different status bar text to be displayed as they are dragged around.
  CToolAxisHandle *pTool =
      (CToolAxisHandle *)ToolManager()->GetToolForID(TOOL_AXIS_HANDLE);
...
//-----------------------------------------------------------------------------
bool CMapAxisHandle::HitTest2D(CMapView2D *pView, const Vector2D &point,
                               HitInfo_t &HitData) {
  if (IsVisible()) {
    for (int i = 0; i < 2; i++) {
...
void CMapAxisHandle::Render2D(CRender2D *pRender) {
  ...
  if (eState == SELECT_MODIFY) {
    ...
    pRender->SetDrawColor(GetRValue(Options.colors.clrSelection),
                          GetGValue(Options.colors.clrSelection),
                          GetBValue(Options.colors.clrSelection));
  } else {
    pRender->PushRenderMode(RENDER_MODE_FLAT);
    pRender->SetDrawColor(GetRValue(Options.colors.clrToolHandle),
...
// Purpose:
//-----------------------------------------------------------------------------
int CMapAxisHandle::SerializeRMF(std::fstream &File, BOOL bRMF) { return (0); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
int CMapAxisHandle::SerializeMAP(std::fstream &File, BOOL bRMF) { return (0); }
...
// Purpose: Overridden to chain down to our endpoints, which are not children.
//-----------------------------------------------------------------------------
SelectionState_t CMapAxisHandle::SetSelectionState(
...
//-----------------------------------------------------------------------------
SelectionState_t CMapAxisHandle::SetSelectionState(
    SelectionState_t eSelectionState, int nHandle) {
  SelectionState_t eState = BaseClass::SetSelectionState(eSelectionState);
...
//-----------------------------------------------------------------------------
void CMapAxisHandle::SetRenderColor(unsigned char red, unsigned char green,
                                    unsigned char blue) {}
...
void CMapAxisHandle::UpdateParentKey(void) {
  if (m_szKeyName[0]) {
    ...
    if (pEntity != NULL) {
      ...
      sprintf(szValue, "%g %g %g, %g %g %g", (double)vecOrigin1.x,
              (double)vecOrigin1.y, (double)vecOrigin1.z, (double)vecOrigin2.x,
              (double)vecOrigin2.y, (double)vecOrigin2.z);
