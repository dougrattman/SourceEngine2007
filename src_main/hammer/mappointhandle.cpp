...
//-----------------------------------------------------------------------------
CMapClass *CMapPointHandle::Create(CHelperInfo *pHelperInfo,
                                   CMapEntity *pParent) {
...
//			pfMaxs -
//-----------------------------------------------------------------------------
CMapPointHandle::CMapPointHandle(void) { Initialize(); }
...
// Output :
//-----------------------------------------------------------------------------
CMapClass *CMapPointHandle::CopyFrom(CMapClass *pObject,
...
CBaseTool *CMapPointHandle::GetToolObject(int nHitData, bool bAttachObject) {
  CToolPointHandle *pTool =
      (CToolPointHandle *)ToolManager()->GetToolForID(TOOL_POINT_HANDLE);
...
// Output :
//-----------------------------------------------------------------------------
bool CMapPointHandle::HitTest2D(CMapView2D *pView, const Vector2D &point,
                                HitInfo_t &HitData) {
...
void CMapPointHandle::Render2D(CRender2D *pRender) {
  ...
  // Draw a line from origin helpers to their parent while they are being
  // dragged.
  if ((m_pParent != NULL) &&
  ...
  if (eState == SELECT_MODIFY) {
    ...
    pRender->SetTextColor(GetRValue(Options.colors.clrToolHandle),
                          GetGValue(Options.colors.clrToolHandle),
                          GetBValue(Options.colors.clrToolHandle));
...
// Purpose:
//-----------------------------------------------------------------------------
int CMapPointHandle::SerializeRMF(std::fstream &File, BOOL bRMF) { return (0); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
int CMapPointHandle::SerializeMAP(std::fstream &File, BOOL bRMF) { return (0); }
...
//-----------------------------------------------------------------------------
void CMapPointHandle::SetRenderColor(unsigned char red, unsigned char green,
                                     unsigned char blue) {}
...
void CMapPointHandle::UpdateParentKey(void) {
  ...
  if (m_szKeyName[0]) {
    ...
    if (pEntity != NULL) {
      ...
      sprintf(szValue, "%g %g %g", (double)m_Origin.x, (double)m_Origin.y,
              (double)m_Origin.z);
