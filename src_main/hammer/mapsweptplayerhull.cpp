...
//-----------------------------------------------------------------------------
CMapClass *CMapSweptPlayerHull::Create(CHelperInfo *pHelperInfo,
                                       CMapEntity *pParent) {
...
// Output : CMapClass
//-----------------------------------------------------------------------------
CMapClass *CMapSweptPlayerHull::CopyFrom(CMapClass *pObject,
...
CBaseTool *CMapSweptPlayerHull::GetToolObject(int nHitData,
  ...
  // need a better system for building complex objects from simple ones and
  // handling changes to the simple objects in the complex one.
  //
  ...
  CToolSweptPlayerHull *pTool =
      (CToolSweptPlayerHull *)ToolManager()->GetToolForID(TOOL_SWEPT_HULL);
...
//-----------------------------------------------------------------------------
bool CMapSweptPlayerHull::HitTest2D(CMapView2D *pView, const Vector2D &point,
                                    HitInfo_t &HitData) {
  if (IsVisible()) {
...
// Purpose: Overridden to chain down to our endpoints, which are not children.
//-----------------------------------------------------------------------------
SelectionState_t CMapSweptPlayerHull::SetSelectionState(
...
//-----------------------------------------------------------------------------
void CMapSweptPlayerHull::SetRenderColor(unsigned char red, unsigned char green,
                                         unsigned char blue) {}
...
//			szValue -
//-----------------------------------------------------------------------------
void CMapSweptPlayerHull::OnParentKeyChanged(const char *szKey,
                                             const char *szValue) {
  if (!_stricmp(szKey, "point0")) {
    Vector vecOrigin;
    sscanf_s(szValue, "%f %f %f", &vecOrigin.x, &vecOrigin.y, &vecOrigin.z);

    vecOrigin += playerFixup;

    m_Point[0]->SetOrigin(vecOrigin);
    PostUpdate(Notify_Changed);
  } else if (!_stricmp(szKey, "point1")) {
    Vector vecOrigin;
    sscanf_s(szValue, "%f %f %f", &vecOrigin.x, &vecOrigin.y, &vecOrigin.z);
...
void CMapSweptPlayerHull::UpdateParentKey(void) {
  ...
  if (pEntity != NULL) {
    ...
    sprintf(szValue, "%g %g %g", (double)vecOrigin1.x, (double)vecOrigin1.y,
            (double)vecOrigin1.z);
    pEntity->NotifyChildKeyChanged(this, "point0", szValue);
    pEntity->NotifyChildKeyChanged(this, "origin", szValue);

    sprintf(szValue, "%g %g %g", (double)vecOrigin2.x, (double)vecOrigin2.y,
            (double)vecOrigin2.z);
