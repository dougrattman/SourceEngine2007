...
extern "C" void Sys_Error(char *error, ...) { Error("%s", error); }

static int s_BoxFaces[6][3] = {
    {0, 4, 2}, {4, 5, 6}, {5, 1, 7}, {1, 0, 3}, {2, 6, 3}, {5, 4, 1},
};

void polyMake(float x1, float y1, float x2, float y2, int npoints,
              float start_ang, Vector *pmPoints) {
  int point;
...
//			y2 -
//-----------------------------------------------------------------------------
bool IsLineInside(const Vector2D &pt1, const Vector2D &pt2, int x1, int y1,
                  int x2, int y2) {
  int lx1 = pt1.x;
  int ly1 = pt1.y;
  int lx2 = pt2.x;
  ...
  if ((lx2 > x2 && lx1 > x2) || (lx2 < x1 && lx1 < x1) ||
      (ly2 > y2 && ly1 > y2) || (ly2 < y1 && ly1 < y1))
    return false;
  ...
  if ((ly1 > y1) != (ly2 > y1)) {
    ...
    if (i >= x1 && i <= x2) return true;  // the line crosses the y1 side (left)
  }
  ...
  if ((lx1 > x1) != (lx2 > x1)) {
    ...
    if (i >= y1 && i <= y2) return true;  // the line crosses the x1 side (down)
  }

  if ((lx1 > x2) != (lx2 > x2)) {
    i = ly1 + (int)((long)(x2 - lx1) * (long)(ly2 - ly1) / (long)(lx2 - lx1));
    if (i >= y1 && i <= y2) return true;  // the line crosses the x2 side (up)
  }
...
bool IsPointInside(const Vector2D &pt, const Vector2D &mins,
                   const Vector2D &maxs) {
  return (pt.x >= mins.x) && (pt.y >= mins.y) && (pt.x <= maxs.x) &&
         (pt.y <= maxs.y);
}

// Is box 1 inside box 2?
bool IsBoxInside(const Vector2D &min1, const Vector2D &max1,
                 const Vector2D &min2, const Vector2D &max2) {
  if ((min1.x < min2.x) || (max1.x > max2.x)) return false;
...
bool IsBoxIntersecting(const Vector2D &min1, const Vector2D &max1,
                       const Vector2D &min2, const Vector2D &max2) {
...
float IntersectionLineAABBox(const Vector &mins, const Vector &maxs,
...
const Vector &GetNormalFromFace(int nFace) {
  ...
  return GetNormalFromPoints(points[s_BoxFaces[nFace][0]],
                             points[s_BoxFaces[nFace][1]],
                             points[s_BoxFaces[nFace][2]]);
}

const Vector &GetNormalFromPoints(const Vector &p0, const Vector &p1,
                                  const Vector &p2) {
...
// solve equation v0 = x*v1 + y*v2 + z*v3
bool SolveLinearEquation(const Vector &v0, const Vector &v1, const Vector &v2,
                         const Vector &v3, Vector &vOut) {
  VMatrix matrix, inverse;
  matrix.Init(v1.x, v1.y, v1.z, 0, v2.x, v2.y, v2.z, 0, v3.x, v3.y, v3.z, 0,
              0.0f, 0.0f, 0.0f, 1);

  if (!matrix.InverseGeneral(inverse)) return false;
