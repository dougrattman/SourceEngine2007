...
//			entities. Examples of these are: beams and special node
//connections.
//
//			The helper factory parameters are:
//
//			<red> <green> <blue> <start key> <start key value> <end key>
//<end key value>
//
//			The line helper looks in the given keys in its parent entity
//and 			attaches itself to the entities with those key values. If only one
//			endpoint entity is specified, the other end is assumed to be
//the parent 			entity.
//
//=============================================================================//
...
CMapClass *CMapCylinder::Create(CHelperInfo *pHelperInfo, CMapEntity *pParent) {
  ...
  pCylinder = new CMapCylinder(pszStartKey, pszStartValueKey, pszStartRadiusKey,
                               pszEndKey, pszEndValueKey, pszEndRadiusKey);
...
// Purpose:
//-----------------------------------------------------------------------------
CMapCylinder::CMapCylinder(void) { Initialize(); }

//-----------------------------------------------------------------------------
// Purpose: Constructor. Initializes data members.
// Input  : pszStartKey - The key to search in other entities for a match
// against the value of pszStartValueKey, ex 'targetname'.
//			pszStartValueKey - The key in our parent entity from which
//to get a search term for the start entity ex 'beamstart01'. 			pszEndKey - The
//key to search in other entities for a match against the value of
//pszEndValueKey ex 'targetname'. 			pszEndValueKey - The key in our parent entity
//from which to get a search term for the end entity ex 'beamend01'.
//-----------------------------------------------------------------------------
CMapCylinder::CMapCylinder(const char *pszStartKey,
                           const char *pszStartValueKey,
                           const char *pszStartRadiusKey, const char *pszEndKey,
...
void CMapCylinder::BuildCylinder(void) {
  if ((m_pStartEntity != NULL) && (m_pEndEntity != NULL)) {
    //
    // Set our origin to our midpoint. This moves our selection handle box to
    // the midpoint.
    //
    Vector Start;
    Vector End;
...
// Output :
//-----------------------------------------------------------------------------
CMapClass *CMapCylinder::CopyFrom(CMapClass *pObject,
  ...
  if (pFrom != NULL) {
    ...
    if (bUpdateDependencies) {
      m_pStartEntity =
          (CMapEntity *)UpdateDependency(m_pStartEntity, pFrom->m_pStartEntity);
      m_pEndEntity =
          (CMapEntity *)UpdateDependency(m_pEndEntity, pFrom->m_pEndEntity);
    } else {
...
//-----------------------------------------------------------------------------
// Purpose: Our start or end entity has changed; recalculate our bounds and
// midpoint. Input  : pObject - Entity that changed.
//-----------------------------------------------------------------------------
void CMapCylinder::OnNotifyDependent(CMapClass *pObject,
...
//-----------------------------------------------------------------------------
void CMapCylinder::ComputeCylinderPoints(int nCount, Vector *pStartVerts,
                                         Vector *pEndVerts) {
  ...
  for (i = 0; i < nCount; ++i) {
    ...
    VectorMA(pStartVerts[i], flSinAngle * m_flStartRadius, yvec,
             pStartVerts[i]);
...
bool CMapCylinder::ShouldDrawAsLine() {
  return !IsSelected() ||
         ((m_flStartRadius == 0.0f) && (m_flEndRadius == 0.0f)) ||
         !Options.GetShowHelpers();
}
...
void CMapCylinder::Render2D(CRender2D *pRender) {
  if ((m_pStartEntity != NULL) && (m_pEndEntity != NULL)) {
    if (!ShouldDrawAsLine()) {
      pRender->SetDrawColor(SELECT_FACE_RED, SELECT_FACE_GREEN,
                            SELECT_FACE_BLUE);
...
void CMapCylinder::Render3D(CRender3D *pRender) {
  if ((m_pStartEntity == NULL) || (m_pEndEntity == NULL)) return;
  ...
  if (!ShouldDrawAsLine()) {
    ...
    for (int i = 0; i < CYLINDER_VERTEX_COUNT; j = i++) {
      ...
      meshBuilder.Position3f(pStartVerts[i].x, pStartVerts[i].y,
                             pStartVerts[i].z);
      meshBuilder.AdvanceVertex();

      meshBuilder.Color3ubv(color);
      meshBuilder.Position3f(pStartVerts[j].x, pStartVerts[j].y,
                             pStartVerts[j].z);
      meshBuilder.AdvanceVertex();
...
// Output : int
//-----------------------------------------------------------------------------
int CMapCylinder::SerializeRMF(std::fstream &File, BOOL bRMF) { return (0); }
...
// Output : int
//-----------------------------------------------------------------------------
int CMapCylinder::SerializeMAP(std::fstream &File, BOOL bRMF) { return (0); }
...
void CMapCylinder::UpdateDependencies(CMapWorld *pWorld, CMapClass *pObject) {
  ...
  if (pEntity != NULL) {
    ...
    m_pStartEntity = (CMapEntity *)UpdateDependency(
        m_pStartEntity, pWorld->FindChildByKeyValue(m_szStartKey, pszValue));

    if (m_szEndValueKey[0] != '\0') {
      pszValue = pEntity->GetKeyValue(m_szEndValueKey);
      m_pEndEntity = (CMapEntity *)UpdateDependency(
          m_pEndEntity, pWorld->FindChildByKeyValue(m_szEndKey, pszValue));
    } else {
    ...
    if (pObject == m_pStartEntity) {
      ...
      if (m_pStartEntity && m_szStartRadiusKey[0] != '\0') {
        const char *pRadiusKey =
            m_pStartEntity->GetKeyValue(m_szStartRadiusKey);
        m_flStartRadius = pRadiusKey ? atof(pRadiusKey) : 0.0f;
      }
    }
