...
// Actually, this is the max per map, but for now this is better than no limit
// at all.
...
// Defaults to match the parsing defaults in ParseDetailGroup -- code path
// defaults may/may not execute
...
CUtlVector<DetailObjects::DetailObject_t>
    DetailObjects::s_DetailObjectDict;  // static members?
...
void DetailObjects::AddDetailModelToFace(const char *pModelName,
                                         const Vector &pt, const QAngle &angles,
                                         int nOrientation) {
...
void DetailObjects::AddDetailSpriteToFace(const Vector &vecOrigin,
                                          const QAngle &vecAngles,
                                          DetailModel_t const &model,
                                          float flScale) {
...
void DetailObjects::PlaceDetail(DetailModel_t const &model, const Vector &pt,
                                const Vector &normal) {
  ...
  switch (model.m_Type) {
    case DETAIL_PROP_TYPE_MODEL:
      AddDetailModelToFace(model.m_ModelName.String(), pt, angles,
                           model.m_Orientation);
      break;
...
//-----------------------------------------------------------------------------
void DetailObjects::EmitDetailObjectsOnFace(CMapFace *pMapFace,
  ...
  // Get next points on the face in pairs -- ie get points necessary to
  // tesselate the face into triangles
  for (int i = 1; i < nPoints - 1; i++) {
    ...
    // Calculate the area of the tesselated triange using half the crossproduct
    // of the edges
    Vector areaVec;
    ...
    int numSamples = clamp(area * detail.m_Density * 0.000001, 0,
    ...
    for (int i = 0; i < numSamples; ++i) {
      ...
      // Make sure the u,v coordinate stay within the triangle boundaries (ie
      // they NOT in the far half of the parallelogram)
      if (v > 1.0f - u) {
        // Triangle is out of bounds, flip the coordinates so they are in the
        // near half of the parallelogram
        u = 1.0f - u;
        v = 1.0f - v;
        assert(u + v <= 1.0f);
      }

      // Compute alpha - assumed to be 1.0 across entire face for
      // non-displacement map faces, since there is no alpha channel
      float alpha = 1.0f;
...
// Places Detail Objects on a face
//-----------------------------------------------------------------------------
void DetailObjects::EmitDetailObjectsOnDisplacementFace(
    CMapFace *pMapFace, DetailObject_t &detail) {
