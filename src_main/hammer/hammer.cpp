...
// Expose singleton
//-----------------------------------------------------------------------------
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CHammer, IHammer, INTERFACEVERSION_HAMMER,
                                  theApp);
...
int WrapFunctionWithMinidumpHandler(int (*pfn)(void *pParam), void *pParam,
                                    int errorRetVal) {
  int nRetVal;
...
class CHammerCmdLine : public CCommandLineInfo {
 public:
  ...
  bool m_bConfigDir;  // Used to find and parse the "-configdir blah" parameter
                      // pair.
  CString m_strGame;  // The name of the game to use for this session, ie "hl2"
                      // or "cstrike". This should match the mod dir, not the
                      // config name.
};
...
bool CHammer::Connect(CreateInterfaceFn factory) {
  ...
  g_pFileSystem =
      (IBaseFileSystem *)factory(BASEFILESYSTEM_INTERFACE_VERSION, NULL);
  g_pStudioRender =
      (IStudioRender *)factory(STUDIO_RENDER_INTERFACE_VERSION, NULL);
  ...
  if (!g_pMDLCache || !g_pFileSystem || !g_pFullFileSystem || !materials ||
      !g_pMaterialSystemHardwareConfig || !g_pStudioRender)
    return false;
  ...
  // This does NOT create the window itself. That happens later in
  // CMainFrame::Create.
  ...
  // NOTE: SetRegistryKey will cause hammer to look into the registry for its
  // values
...
void *CHammer::QueryInterface(const char *pInterfaceName) {
  ...
  if (!Q_strncmp(pInterfaceName, INTERFACEVERSION_HAMMER,
                 Q_strlen(INTERFACEVERSION_HAMMER) + 1))
    return (IHammer *)this;
...
bool CHammer::HammerPreTranslateMessage(MSG *pMsg) {
  ...
  return (/*pMsg->message == WM_KICKIDLE ||*/ PreTranslateMessage(pMsg) !=
          FALSE);
}
...
//-----------------------------------------------------------------------------
// Purpose: Tweaks our data members to enable us to import old Valve Hammer
// Editor
...
//			p - Pointer to buffer that receives the full path to the
// directory.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
COLORREF CHammer::GetProfileColor(const char *pszSection, const char *pszKey,
                                  int r, int g, int b) {
  int newR, newG, newB;
...
void CHammer::Help(const char *pszTopic) {
  ...
  if (hResult > (HINSTANCE)32)
  {
          ...
          //
          hResult = ShellExecute(NULL, "open", szHelpExe, szParam, szHelpDir,
  SW_SHOW);
  }
...
static SpewRetval_t HammerDbgOutput(SpewType_t spewType, char const *pMsg) {
  ...
  switch (spewType) {
    case SPEW_ERROR:
      MessageBox(NULL, (LPCTSTR)pMsg, "Fatal Error",
  ...
  if (dwChangeHandle == NULL) {
    ...
    dwChangeHandle = FindFirstChangeNotification(
        ...
        FILE_NOTIFY_CHANGE_DIR_NAME |
            FILE_NOTIFY_CHANGE_FILE_NAME);  // watch file and dir name changes
...
bool CHammer::InitSessionGameConfig(const char *szGame) {
  ...
  if (szGame && szGame[0] != '\0') {
    ...
    if (!pConfig) {
      Msg(mwError,
          "Invalid game \"%s\" specified on the command-line, ignoring.",
          szGame);
    }
  } else {
    ...
    if (pszGameDir) {
      ...
      if (!pConfig) {
        Msg(mwError,
            "Invalid game \"%s\" found in VPROJECT environment variable, "
            "ignoring.",
            pszGameDir);
      }
    }
...
bool CHammer::Check16BitColor() {
  ...
  if (hDC) {
    ...
    if (bpp < 15) {
      AfxMessageBox(
          "Your screen must be in 16-bit color or higher to run Hammer.");
      return false;
    }
    ::DeleteDC(hDC);
  }
...
InitReturnVal_t CHammer::Init() {
  return (InitReturnVal_t)WrapFunctionWithMinidumpHandler(
      &CHammer::StaticHammerInternalInit, this, INIT_FAILED);
}
...
DLL #else Enable3dControlsStatic();	// Call this when linking to MFC
statically #endif
  */
  ...
  pMapDocTemplate = new CMultiDocTemplate(
      IDR_MAPDOC, RUNTIME_CLASS(CMapDoc),
  ...
  //
  // Initialize the rich edit control so we can use it in the entity help
  // dialog.
  //
  AfxInitRichEdit();

  //
  // Create main MDI Frame window. Must be done AFTER registering the multidoc
  // template!
  //
  CMainFrame *pMainFrame = new CMainFrame;
  if (!pMainFrame->LoadFrame(IDR_MAINFRAME)) return INIT_FAILED;
  ...
  // Now that we've initialized the file system, we can parse this config's
  // gameinfo.txt for the additional settings there.
  ...
  if (!g_Textures.Initialize(m_pMainWnd->m_hWnd)) {
    ...
    //
    // Initialize studio model rendering (must happen after
    // g_Textures.Initialize since g_Textures.Initialize kickstarts the material
    // system and sets up g_MaterialSystemClientFactory)
    //
  ...
  if (Options.general.bClosedCorrectly == FALSE) {
    ...
    if (strLastGoodSave.GetLength() != 0) {
      ...
      V_snprintf(msg, sizeof(msg),
                 "Hammer did not shut down correctly the last time it was "
                 "used.\nWould you like to load the last saved file?\n(%s)",
                 (const char *)strLastGoodSave);
...
RequestRetval_t CHammer::RequestNewConfig() {
  if (!Options.RunConfigurationDialog()) return REQUEST_QUIT;
...
#elif DEMO_VERSION == 3
    20, 10, 9,  23,  16, 84, 12, 1, 0,  38, 65, 25,
    6,  1,  11, 119, 50, 11, 21, 9, 68, 0  // Encodes "Computer Gaming World
                                           // Demo"
#elif DEMO_VERSION == 4
    25, 0,  28, 19, 72, 103, 12, 29,
    69, 19, 65, 0,  6,  0,   2,  0  // Encodes "Next-Generation Demo"
#elif DEMO_VERSION == 5
    20, 10, 9, 23, 16, 84, 12, 1,  0,  38, 65, 25, 10, 79, 41,
    57, 17, 1, 21, 17, 65, 0,  29, 77, 4,  78, 0,  0  // Encodes "Computer Game
                                                      // Entertainment"
#elif DEMO_VERSION == 6
    20, 10, 9,  23, 16, 84, 12, 1,  0,  0, 78, 16, 79,
    33, 9,  35, 69, 52, 11, 4,  89, 12, 1, 0  // Encodes "Computer and Net
                                              // Player"
#elif DEMO_VERSION == 7
    50, 72, 52, 43, 36, 121, 0  // Encodes "e-PLAY"
#elif DEMO_VERSION == 8
    4,  17, 22, 6,  17, 69, 14,
    10, 0,  49, 76, 1,  28, 0  // Encodes "Strategy Plus"
#elif DEMO_VERSION == 9
    7, 38, 68, 42, 4, 71, 8, 9, 73, 15, 69, 0  // Encodes "PC Magazine"
#elif DEMO_VERSION == 10
    5,  10,  8,  11, 12, 78, 14,
    83, 115, 21, 79, 26, 10, 0  // Encodes "Rolling Stone"
...
void Encode(char *pstring, char *pkey, int strLen) {
  int i, len;
  len = strlen(pkey);
  for (i = 0; i < strLen; i++) pstring[i] ^= pkey[i % len];
}
#endif  // DEMO_VERSION
...
void CHammer::OnFileOpen(void) {
  ...
  // TODO: need to prevent (or handle) opening VMF files when using old map file
  // formats
  CFileDialog dlg(TRUE, NULL, NULL,
                  OFN_LONGNAMES | OFN_HIDEREADONLY | OFN_NOCHANGEDIR,
                  "Valve Map Files (*.vmf)|*.vmf|Valve Map Files Autosave "
                  "(*.vmf_autosave)|*.vmf_autosave|Worldcraft RMFs "
                  "(*.rmf)|*.rmf|Worldcraft Maps (*.map)|*.map||");
...
//-----------------------------------------------------------------------------
// Purpose: Performs idle processing. Runs the frame and does MFC idle
...
//			is minimized and unminimized. This allows us to stop
// rendering the 			3D view when we are minimized.
...
//			Also sets the total amount of space used by the
// directory.
...
int CHammer::GetNextAutosaveNumber(
    ...
    const CString *pstrMapTitle) const {
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
    // go through and for each file check to see if it is an autosave for this
    // map; also keep track of total file size for directory.
    while (GetLastError() != ERROR_NO_MORE_FILES &&
      ...
      // every autosave file ends in three digits; this code separates the name
      // from the digits
      CString strMapName =
          currentFilename.Left(currentFilename.GetLength() - 17);
      CString strCurrentNumber =
          currentFilename.Mid(currentFilename.GetLength() - 16, 3);
      int nMapNumber = atoi((char *)strCurrentNumber.GetBuffer());

      if (strMapName.CompareNoCase((*pstrMapTitle)) == 0) {
        // keep track of real number of autosaves with map name; deals with
        // instance where older maps get deleted and create sequence holes in
        // autosave map names.
...
void CHammer::Autosave(void) {
  ...
  if (pDoc && pDoc->NeedsAutosave()) {
    ...
    // this will hold the name of the map w/o leading directory info or file
    // extension
    CString strMapTitle;
    ...
    // the map hasn't been saved before and doesn't have a filename; using
    // default: 'autosave'
    ...
    // the map already has a filename
      ...
      strMapTitle = strMapFilename.Mid(
          nFilenameBeginOffset, nFilenameEndOffset - nFilenameBeginOffset);
    }

    nCurrentAutosaveNumber = GetNextAutosaveNumber(
        &autosaveFiles, &dwTotalAutosaveDirectorySize, &strMapTitle);
    ...
    CString strSaveName = strAutosaveDirectory + strMapTitle +
                          strAutosaveNumber + strExtension + "_autosave";
    ...
    // if there is too much space used for autosaves, delete the oldest file
...
//-----------------------------------------------------------------------------
// Purpose: Verifies that the autosave directory exists and attempts to create
// it if
...
bool CHammer::VerifyAutosaveDirectory(char *szAutosaveDirectory) const {
  ...
  if (szRootDir[0] == 0) {
    AfxMessageBox(
        "No autosave directory has been selected.\nThe autosave feature will "
        "be disabled until a directory is entered.",
        MB_OK);
    return false;
  }
  if ((szRootDir[1] != ':') || (szRootDir[2] != '\\')) {
    AfxMessageBox(
        "The current autosave directory does not have an absolute path.\nThe "
        "autosave feature will be disabled until a new directory is entered.",
        MB_OK);
    return false;
  }
  ...
  hDir = CreateFile(strAutosaveDirectory, GENERIC_READ,
  ...
  if (hDir == INVALID_HANDLE_VALUE) {
    ...
    if (!bDirResult) {
      AfxMessageBox(
          "The current autosave directory does not exist and could not be "
          "created.  \nThe autosave feature will be disabled until a new "
    ...
    hTestFile =
        CreateFile(strAutosaveDirectory + "test.txt", GENERIC_READ,
    ...
    if (hTestFile == INVALID_HANDLE_VALUE) {
      if (GetLastError() == ERROR_ACCESS_DENIED) {
        AfxMessageBox(
            "The autosave directory is marked as read only.  Please remove the "
            "read only attribute or select a new directory in "
            "Tools->Options->General.\nThe autosave feature will be disabled.",
            MB_OK);
        return false;
      } else {
        AfxMessageBox(
            "There is a problem with the autosave directory.  Please select a "
            "new directory in Tools->Options->General.\nThe autosave feature "
            "will be disabled.",
            MB_OK);
        return false;
      }
...
void CHammer::LoadLastGoodSave(void) {
  ...
  if (!strLastGoodSave.IsEmpty()) {
    ...
    if (!((CMapDoc *)pCurrentDoc)->IsAutosave() &&
        Q_stristr(pCurrentDoc->GetPathName(), szAutoSaveDir)) {
