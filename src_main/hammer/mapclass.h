...
//
//			Pointers to other objects in the object tree may be
// stored, but 			they should be set via UpdateDependency rather
// than directly. This 			insures proper linkage to the other
// object so that if it moves, is 			removed from the world,
// or changes in any other way, the dependent 			object is
// properly notified.
//
//=============================================================================//
...
enum VisGroupSelection { AUTO = 0, USER };
...
class CSafeObject {
 ...
 private:
  int m_RefCount;  // This object goes away when all smart pointers to it go
                   // away.
};
...
class CMapClass : public CMapPoint {
 public:
  ...
  virtual CMapClass *GetParent(void) {
    Assert((m_pParent == NULL) ||
           (dynamic_cast<CMapClass *>(m_pParent) != NULL));
  ...
  virtual void OnClone(CMapClass *pClone, CMapWorld *pWorld,
                       const CMapObjectList &OriginalList,
                       CMapObjectList &NewList);
  virtual void OnPreClone(CMapClass *pClone, CMapWorld *pWorld,
                          const CMapObjectList &OriginalList,
                          CMapObjectList &NewList);
  virtual void OnPrePaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
                          CMapWorld *pDestWorld,
                          const CMapObjectList &OriginalList,
                          CMapObjectList &NewList);
  virtual void OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld,
                       CMapWorld *pDestWorld,
                       const CMapObjectList &OriginalList,
                       CMapObjectList &NewList);
  virtual void OnNotifyDependent(CMapClass *pObject,
  ...
  void GetBoundsCenter(Vector &vecCenter) {
    m_Render2DBox.GetBoundsCenter(vecCenter);
  }
  void GetBoundsSize(Vector &vecSize) { m_Render2DBox.GetBoundsSize(vecSize); }
  inline bool IsInsideBox(Vector const &Mins, Vector const &Maxs) const {
    return (m_Render2DBox.IsInsideBox(Mins, Maxs));
  }
  inline bool IsIntersectingBox(const Vector &vecMins,
                                const Vector &vecMaxs) const {
    return (m_Render2DBox.IsIntersectingBox(vecMins, vecMaxs));
  }
  ...
  virtual bool HitTest2D(CMapView2D *pView, const Vector2D &point,
                         HitInfo_t &HitData);
  virtual bool HitTestLogical(CMapViewLogical *pView, const Vector2D &point,
                              HitInfo_t &HitData);

  // Objects that can be clicked on and activated as tools implement this and
  // return a CBaseTool-derived object.
  ...
  virtual bool IsClutter(void) {
    return false;
  }  // Whether this object should be hidden when the user hides helpers.
  virtual bool IsCulledByCordon(
      const Vector &vecMins,
      const Vector &vecMaxs);  // Whether this object is hidden based on its own
                               // intersection with the cordon, independent of
                               // its parent's intersection.
  ...
  BOOL EnumChildren(ENUMMAPCHILDRENPROC pfn, unsigned int dwParam = 0,
                    MAPCLASSTYPE Type = NULL);
  BOOL EnumChildrenRecurseGroupsOnly(ENUMMAPCHILDRENPROC pfn,
  ...
  inline bool IsVisGroupShown(void) {
    return m_bVisGroupShown && m_bVisGroupAutoShown;
  }
  ...
  //
  virtual void SetRenderColor(unsigned char red, unsigned char green,
                              unsigned char blue);
  ...
  inline void SetRenderFrame(int nRenderFrame) {
    m_nRenderFrame = nRenderFrame;
  }
 ...
 public:
  // Set to true while loading a VMF file so it can delay certain calls like
  // UpdateBounds. Drastically speeds up load times.
 ...
 protected:
  ...
  //
  static ChunkFileResult_t LoadEditorCallback(CChunkFile *pFile,
                                              CMapClass *pObject);
  static ChunkFileResult_t LoadEditorKeyCallback(const char *szKey,
                                                 const char *szValue,
                                                 CMapClass *pObject);
  ...
  BoundBox m_CullBox;      // Our bounds for culling in the 3D views and
                           // intersecting with the cordon.
  BoundBox m_Render2DBox;  // Our bounds for rendering in the 2D views.

  CMapObjectList m_Children;  // Each object can have many children. Children
                              // usually transform with their parents, etc.
  CMapObjectList
      m_Dependents;  // Objects that this object should notify if it changes.

  int m_nID;           // This object's unique ID.
  bool m_bTemporary;   // Whether to track this object for Undo/Redo.
  int m_nRenderFrame;  // Frame counter used to avoid rendering the same object
                       // twice in a 3D frame.

  bool m_bVisible2D;  // Whether this object is visible in the 2D view.
                      // Currently only used for morphing.
  bool m_bVisible;  // Whether this object is currently visible in the 2D and 3D
                    // views based on ALL factors: visgroups, cordon, etc.

  bool m_bVisGroupShown;  // Whether this object is shown or hidden by user
                          // visgroups. Kept separate from m_bVisible so we can
  // reflect this state in the visgroups list independent of the cordon, hide
  // entities state, etc.

  bool m_bVisGroupAutoShown;  // Whether this object is shown or hidden by auto
                              // visgroups.

  CVisGroupList
      m_VisGroups;  // Visgroups to which this object belongs, EMPTY if none.
  CVisGroup *m_pColorVisGroup;  // The visgroup from which we get our color,
                                // NULL if none.

  WCKeyValuesT<WCKVBase_Vector>
      *m_pEditorKeys;  // Temporary storage for keys loaded from the "editor"
                       // chunk of the VMF file, freed after loading.

  friend class CTrackEntry;  // Friends with Undo/Redo system so that parentage
                             // can be changed.
  friend void FixHiddenObject(
      MapError *pError);  // So that the Check for Problems dialog can fix
                          // visgroups problems.
};
...
//-----------------------------------------------------------------------------
int CMapClass::GetID(void) { return (m_nID); }
...
#define IMPLEMENT_MAPCLASS(class_name)                              \
  ...
  CMapClass *class_name##_CreateObject() { return new class_name; } \
