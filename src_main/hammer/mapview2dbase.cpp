...
void CMapView2DBase::OnInitialUpdate(void) {
  // CMainFrame::LoadWindowStates calls InitialUpdateFrame which causes us to
  // get two OnInitialUpdate messages! Check for a NULL renderer to avoid
  // processing twice.
  ...
  vgui::EditablePanel *pMainPanel =
      new CMapView2DBasePanel(this, "MapView2DPanel");
...
//-----------------------------------------------------------------------------
// Purpose: Called by the tools to scroll the 2D view so that a point is
// visible.
//			Sets a timer to do the scroll so that we don't much with the
//view state 			while the tool is handling a mouse message.
...
void CMapView2DBase::ToolScrollToPoint(const Vector2D &ptClient) {
  ...
  if ((GetCapture() == this) &&
      (ptClient.x < 0 || ptClient.y < 0 || ptClient.x >= m_ClientWidth ||
       ptClient.y >= m_ClientHeight)) {
    // reset these
    m_xScroll = m_yScroll = 0;
    if (ptClient.x < 0) {
      // scroll left
...
//			nIntensity - Percentage of original color intensity to keep
//(0 - 100). 			bReverse - True ramps toward black, false ramps toward the given
//color.
...
void CMapView2DBase::AdjustColorIntensity(Color &color, int nIntensity) {
  ...
  color.SetColor(std::min((color.r() * nIntensity) / 100, 255),
                 ...
                 std::min((color.b() * nIntensity) / 100, 255), color.a());
}
...
void CMapView2DBase::SetColorMode(bool bWhiteOnBlack) {
  ...
  // Dotted grid color. No need to create a pen since all we do is SetPixel with
  // it.
...
//-----------------------------------------------------------------------------
void CMapView2DBase::DrawGrid(CRender2D *pRender, int xAxis, int yAxis,
                              float depth, bool bNoSmallGrid) {
  CMapDoc *pDoc = GetMapDoc();
  ...
  if ((((float)nGridSpacing * m_fZoom) < 4.0f) &&
      Options.view2d.bHideSmallGrid) {
    bNoSmallGrid = true;
  }
  ...
  int xMin = SnapToGrid(
      (int)std::max(g_MIN_MAP_COORD, m_ViewMin[xAxis] - nGridSpacing),
      nGridSpacing);
  int xMax = SnapToGrid(
      (int)std::min(g_MAX_MAP_COORD, m_ViewMax[xAxis] + nGridSpacing),
      nGridSpacing);
  int yMin = SnapToGrid(
      (int)std::max(g_MIN_MAP_COORD, m_ViewMin[yAxis] - nGridSpacing),
      nGridSpacing);
  int yMax = SnapToGrid(
      (int)std::min(g_MAX_MAP_COORD, m_ViewMax[yAxis] + nGridSpacing),
      nGridSpacing);
  ...
  for (int y = yMin; y <= yMax; y += nGridSpacing) {
    ...
    if ((!s_bGridDots) || (bHighligh) || (y == g_MAX_MAP_COORD) ||
        (y == g_MIN_MAP_COORD)) {
  ...
  for (int x = xMin; x <= xMax; x += nGridSpacing) {
    ...
    if ((!s_bGridDots) || (bHighligh) || (x == g_MAX_MAP_COORD) ||
        (x == g_MIN_MAP_COORD)) {
...
//-----------------------------------------------------------------------------
bool CMapView2DBase::CheckDistance(const Vector2D &vecCheck,
                                   const Vector2D &vecRef, int nDist) {
...
void CMapView2DBase::SetViewOrigin(float fHorz, float fVert, bool bRelative) {
  ...
  if (bRelative) {
    if (fHorz == 0 && fVert == 0) return;

    vCurPos[axHorz] += fHorz;
    vCurPos[axVert] += fVert;
  } else {
    if (fHorz == vCurPos[axHorz] && fVert == vCurPos[axVert]) return;
  ...
  // Msg("SetViewOrigin: (%i,%i) %s (%i,%i) \n", x, y, bRelative?"rel":"abs",
  // m_ptViewOrigin.x, m_ptViewOrigin.y );
...
void CMapView2DBase::UpdateClientView(void) {
  ...
  m_flMinZoom =
      std::min(m_ClientWidth / flMaxExtents, m_ClientHeight / flMaxExtents);
...
void CMapView2DBase::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif  //_DEBUG
...
void CMapView2DBase::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnKeyDownLogical(static_cast<CMapViewLogical *>(this), nChar,
                                  nRepCnt, nFlags))
        return;
    } else {
      if (pTool->OnKeyDown2D(static_cast<CMapView2D *>(this), nChar, nRepCnt,
                             nFlags))
        return;
    }
  }
...
void CMapView2DBase::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (nChar == VK_SPACE) {
    ...
    if (pTool) {
      if (IsLogical()) {
        if (pTool->OnKeyUpLogical(static_cast<CMapViewLogical *>(this), nChar,
                                  nRepCnt, nFlags))
          return;
      } else {
        if (pTool->OnKeyUp2D(static_cast<CMapView2D *>(this), nChar, nRepCnt,
                             nFlags))
          return;
      }
    }
...
void CMapView2DBase::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnCharLogical(static_cast<CMapViewLogical *>(this), nChar,
                               nRepCnt, nFlags))
        return;
    } else {
      if (pTool->OnChar2D(static_cast<CMapView2D *>(this), nChar, nRepCnt,
                          nFlags))
        return;
    }
  }
...
// Hit test
//-----------------------------------------------------------------------------
bool CMapView2DBase::HitTest(const Vector2D &vPoint, const Vector &mins,
                             const Vector &maxs) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
int CMapView2DBase::ObjectsAt(const Vector2D &vPoint, HitInfo_t *pHitData,
                              int nMaxObjects) {
  CMapDoc *pDoc = GetMapDoc();
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    ...
    if (IsLogical()) {
      if (pChild->HitTestLogical(static_cast<CMapViewLogical *>(this), vPoint,
                                 pHitData[nIndex])) {
        nIndex++;
      }
    } else {
      if (pChild->HitTest2D(static_cast<CMapView2D *>(this), vPoint,
                            pHitData[nIndex])) {
        nIndex++;
      }
    }
...
void CMapView2DBase::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnLMouseDownLogical(static_cast<CMapViewLogical *>(this),
                                     nFlags, Vector2D(point.x, point.y)))
        return;
    } else {
      if (pTool->OnLMouseDown2D(static_cast<CMapView2D *>(this), nFlags,
...
void CMapView2DBase::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  //
  if (m_bMouseDrag) {
    if (point == m_ptLDownClient) return;
  ...
  if (pTool) {
    ...
    if (IsLogical()) {
      if (pTool->OnMouseMoveLogical(static_cast<CMapViewLogical *>(this),
                                    nFlags, vPoint))
        return;
    } else {
...
BOOL CMapView2DBase::OnMouseWheel(UINT nFlags, short zDelta, CPoint point) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnMouseWheelLogical(static_cast<CMapViewLogical *>(this),
                                     nFlags, zDelta,
                                     Vector2D(point.x, point.y)))
        return TRUE;
    } else {
      if (pTool->OnMouseWheel2D(static_cast<CMapView2D *>(this), nFlags, zDelta,
                                Vector2D(point.x, point.y)))
        return TRUE;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Scrolls the view to make sure that the position in world space is
// visible. Input  : vecPos -
//-----------------------------------------------------------------------------
...
void CMapView2DBase::OnLButtonUp(UINT nFlags, CPoint point) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnLMouseUpLogical(static_cast<CMapViewLogical *>(this), nFlags,
                                   Vector2D(point.x, point.y)))
        return;
    } else {
      if (pTool->OnLMouseUp2D(static_cast<CMapView2D *>(this), nFlags,
                              Vector2D(point.x, point.y)))
        return;
    }
  }
...
void CMapView2DBase::OnLButtonDblClk(UINT nFlags, CPoint point) {
  ...
  if (pTool != NULL) {
    Vector2D vPoint(point.x, point.y);
    if (IsLogical()) {
      pTool->OnLMouseDblClkLogical(static_cast<CMapViewLogical *>(this), nFlags,
                                   vPoint);
      pTool->OnLMouseDownLogical(static_cast<CMapViewLogical *>(this), nFlags,
                                 vPoint);
    } else {
...
void CMapView2DBase::UpdateView(int nFlags) {
  if (nFlags & MAPVIEW_UPDATE_ONLY_3D) return;

  if (IsLogical()) {
    if (nFlags & MAPVIEW_UPDATE_ONLY_2D) return;
  } else {
    if (nFlags & MAPVIEW_UPDATE_ONLY_LOGICAL) return;
  }
  ...
  if (nFlags & (MAPVIEW_UPDATE_OBJECTS | MAPVIEW_UPDATE_VISGROUP_STATE |
                MAPVIEW_UPDATE_VISGROUP_ALL)) {
...
//			pScrollBar -
//-----------------------------------------------------------------------------
void CMapView2DBase::OnHScroll(UINT nSBCode, UINT nPos,
...
//			pScrollBar -
//-----------------------------------------------------------------------------
void CMapView2DBase::OnVScroll(UINT nSBCode, UINT nPos,
                               CScrollBar *pScrollBar) {
  int iPos = int(nPos);
...
void CMapView2DBase::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnRMouseDownLogical(static_cast<CMapViewLogical *>(this),
                                     nFlags, Vector2D(point.x, point.y)))
        return;
    } else {
      if (pTool->OnRMouseDown2D(static_cast<CMapView2D *>(this), nFlags,
                                Vector2D(point.x, point.y)))
        return;
    }
  }
...
void CMapView2DBase::OnContextMenu(UINT nFlags, const Vector2D &vPoint) {
  ...
  if (pTool) {
    if (IsLogical()) {
      if (pTool->OnContextMenuLogical(static_cast<CMapViewLogical *>(this),
                                      nFlags, vPoint))
        return;
    } else {
      if (pTool->OnContextMenu2D(static_cast<CMapView2D *>(this), nFlags,
                                 vPoint))
        return;
    }
  }
  ...
  menuDefault.TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_LEFTALIGN,
...
void CMapView2DBase::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  if (pTool) {
    if (IsLogical()) {
      pTool->OnRMouseUpLogical(static_cast<CMapViewLogical *>(this), nFlags,
                               Vector2D(point.x, point.y));
    } else {
      pTool->OnRMouseUp2D(static_cast<CMapView2D *>(this), nFlags,
                          Vector2D(point.x, point.y));
    }
  }
...
void CMapView2DBase::WorldToClient(Vector2D &ptClient, const Vector &vecWorld) {
  ...
  ptClient.x = (m_fZoom * (vecWorld[axHorz] - m_vViewOrigin[axHorz])) +
               m_fClientWidthHalf;
  ptClient.y = (m_fZoom * (m_vViewOrigin[axVert] - vecWorld[axVert])) +
               m_fClientHeightHalf;
...
void CMapView2DBase::BuildRay(const Vector2D &ptClient, Vector &vStart,
                              Vector &vEnd) {
...
void CMapView2DBase::GetBestTransformPlane(Vector &horzAxis, Vector &vertAxis,
                                           Vector &thirdAxis) {
  horzAxis.Init();
  horzAxis[axHorz] = 1;
  vertAxis.Init();
...
//-----------------------------------------------------------------------------
bool CMapView2DBase::IsBoxFullyVisible(const Vector &minsWorld,
...
// Purpose: Returns true if the entire 3D box is visible in this 2D view.
//-----------------------------------------------------------------------------
bool CMapView2DBase::CanBoxFitInView(const Vector &minsWorld,
                                     const Vector &maxsWorld) {
  Vector2D minsClient;
  Vector2D maxsClient;
  WorldToClient(minsClient, minsWorld);
...
bool CMapView2DBase::IsInClientView(const Vector &vecMin,
                                    const Vector &vecMax) {
...
bool CMapView2DBase::IsInClientView(const Vector2D &vecMin,
                                    const Vector2D &vecMax) {
  // check render view bounds in world space, dont translate every object
  if ((vecMin.x > m_ViewMax.x) || (vecMax.x < m_ViewMin.x)) return false;
