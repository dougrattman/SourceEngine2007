...
//			of prefabs implemented here: Half-Life style prefabs, and
//Half-Life 2 			style prefabs.
//
//			For Half-Life, prefab libraries are stored as binary .OL
//files, each of 			which contains multiple .RMF files that are the prefabs.
//
//			For Half-Life 2, prefabs are stored in a tree of folders,
//each folder 			representing a library, and the each .VMF file in the folder
//containing 			a single prefab.
//
//=============================================================================//
...
static const char *pLibHeader = "Worldcraft Prefab Library\r\n\x1a";
...
CPrefab *CPrefab::FindID(DWORD dwID) {
  ...
  while (p) {
    ...
    if (pPrefab->dwID == dwID) return pPrefab;
  }
...
// Input  : b -
//-----------------------------------------------------------------------------
void CPrefab::EnableCaching(BOOL b) { bCacheEnabled = b; }
...
// Purpose:
//-----------------------------------------------------------------------------
CPrefabLibrary::~CPrefabLibrary() { FreePrefabs(); }
...
void CPrefabLibrary::Sort(void) {
  ...
  //
  qsort(TmpPrefabArray, nPrefabs, sizeof(CPrefab *),
        (int(__cdecl *)(const void *, const void *))SortPrefabs);
...
void CPrefabLibrary::LoadAllLibraries() {
  ...
  //
  // Add one prefab library for the root prefabs folder in case they put
  // something there.
  //
...
void CPrefabLibrary::Add(CPrefab *pPrefab) {
  if (!Prefabs.Find(pPrefab)) Prefabs.AddTail(pPrefab);
...
CPrefab *CPrefabLibrary::EnumPrefabs(POSITION &p) {
  if (p == ENUM_START) p = Prefabs.GetHeadPosition();
  if (!p) return NULL;
...
CPrefabLibrary *CPrefabLibrary::FindID(DWORD dwID) {
  ...
  while (p) {
    ...
    if (pPrefabLibrary->dwID == dwID) return pPrefabLibrary;
  }
...
// Input  : p - Iterator.
//			eType - Type of library to return, LibType_None returns
//all 				library types.
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if this prefab represents the given filename, false if
...
int CPrefabLibraryRMF::SetName(LPCTSTR pszName) {
  ...
  if (m_file.is_open()) {
    ...
    if (GetFileAttributes(szNewFilename) != 0xFFFFFFFF) return 0;  // exists.
  }
...
// Output : int
//-----------------------------------------------------------------------------
int CPrefabLibraryVMF::Save(LPCTSTR pszFilename, BOOL bIndexOnly) { return 1; }
