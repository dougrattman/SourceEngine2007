...
//
#define COPY_FACE_PLANE \
  0x00000001  // Copies only the face's plane. Used for carving.
...
// contains a point that represents a local extreme along a particular
// dimension.
//
enum {
  EXTENTS_XMIN = 0,
...
//
// Both an enumeration and bitflags. Used as bitflags when querying a face for
// its texture alignment because it could be world aligned and face aligned at
// the same time.
//
...
#define INIT_TEXTURE_ALL                                            \
  (INIT_TEXTURE_AXES | INIT_TEXTURE_ROTATION | INIT_TEXTURE_SHIFT | \
   INIT_TEXTURE_SCALE)
...
#define CREATE_FACE_PRESERVE_PLANE \
  0x0001  // Hack to prevent plane from being recalculated while building a
          // solid from its planes.
...
class CMapFace : public CMapAtom {
 public:
  ...
  // If bRescaleTextureCoordinates is true, then it will rescale and reoffset
  // the texture coordinates so that the texture is in the same apparent spot as
  // the old texture (if they are different sizes).
  void SetTexture(const char *pszNewTex,
                  bool bRescaleTextureCoordinates = false);
  ...
  void JustifyTextureUsingExtents(TextureJustification_t eJustification,
                                  Extents_t Extents);
  void GetFaceBounds(Vector &pfMins, Vector &pfMaxs) const;
  void GetFaceExtents(Extents_t Extents) const;
  void GetTextureExtents(Extents_t Extents, Vector2D &TopLeft,
  ...
  CMapFace *CopyFrom(const CMapFace *pFrom, DWORD dwFlags = COPY_FACE_POINTS,
  ...
  inline void SetRenderAlpha(unsigned char uchAlpha) {
    m_uchAlpha = uchAlpha;
  }  // HACK: should be in CMapAtom

  inline void GetFaceNormal(Vector &normal);
  bool TraceLine(Vector &HitPos, Vector &HitNormal, Vector const &Start,
                 Vector const &End);
  bool TraceLineInside(Vector &HitPos, Vector &HitNormal, Vector const &Start,
                       Vector const &End, bool bNoDisp = false);
 ...
 protected:
  void ComputeColor(CRender3D *pRender, bool bRenderAsSelected,
                    SelectionState_t faceSelectionState, bool ignoreLighting,
                    Color &pColor);
  ...
  void AddFaceVertices(CMeshBuilder &builder, CRender3D *pRender,
                       bool bRenderSelected,
                       SelectionState_t faceSelectionState);

  // render texture axes
  static void RenderTextureAxes(CRender3D *pRender, int nCount,
                                CMapFace **ppFaces);
  static void RenderGridsIfCloseEnough(CRender3D *pRender, int nCount,
                                       CMapFace **ppFaces);
  static void Render3DGrids(CRender3D *pRender, int nCount, CMapFace **ppFaces);
  static void RenderWireframeFaces(CRender3D *pRender, int nCount,
                                   MapFaceRender_t **ppFaces);
  static void RenderFacesBatch(CMeshBuilder &MeshBuilder, IMesh *pMesh,
                               CRender3D *pRender, MapFaceRender_t **ppFaces,
                               int nFaceCount, int nVertexCount,
                               int nIndexCount, bool bWireframe);
  static void RenderFaces(CRender3D *pRender, int nCount,
                          MapFaceRender_t **ppFaces);

  void RenderFace3D(CRender3D *pRender, EditorRenderMode_t renderMode,
                    bool renderSelected, SelectionState_t faceSelectionState);

  //
  // Serialization (chunk handlers).
  //
  static ChunkFileResult_t LoadDispInfoCallback(CChunkFile *pFile,
                                                CMapFace *pFace);
  static ChunkFileResult_t LoadKeyCallback(const char *szKey,
                                           const char *szValue,
  ...
  static IEditorTexture *m_pLightmapGrid;  // Lightmap grid texture for use in
                                           // viewing lightmap scales.
  EditDispHandle_t
      m_DispHandle;  // Displacement map applied to this face, NULL if none.

  static bool m_bShowFaceSelection;  // Whether to render faces with a special
                                     // color when they are selected.

  Vector2D *
      m_pTextureCoords;  // An array of texture coordinates, one per face point.
  Vector2D *m_pLightmapCoords;  // An array of lightmap coordinates, one per
                                // face point.
...
// Output : inline void
//-----------------------------------------------------------------------------
inline void CMapFace::SetLightmapCoord(const Vector2D &LightmapCoord,
                                       int nIndex) {
  Assert(nIndex < nPoints);
...
//-----------------------------------------------------------------------------
// Purpose: Returns a pointer to the texture that is applied to this face, NULL
// if none.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
inline void CMapFace::SetFaceID(int nID) { m_nFaceID = nID; }

//-----------------------------------------------------------------------------
// Purpose: Attaches a displacement surface to this face.
// Input  : handle - Displacement surface handle of surface attached to this
// face
//-----------------------------------------------------------------------------
inline void CMapFace::SetDisp(EditDispHandle_t handle, bool bDestroyPrevious) {
  ...
  Assert((handle == EDITDISPHANDLE_INVALID) ||
         (EditDispMgr()->GetDisp(handle) != NULL));
...
//-----------------------------------------------------------------------------
inline EditDispHandle_t CMapFace::GetDisp(void) { return m_DispHandle; }
...
inline bool CMapFace::IsCordonFace() const { return m_bIsCordonFace; }
...
class CMapFaceList : public CUtlVector<CMapFace *> {
 public:
  ...
  void Intersect(CMapFaceList &IntersectWith, CMapFaceList &In,
                 CMapFaceList &Out);
};
...
//-----------------------------------------------------------------------------
CMapFaceList::CMapFaceList(CMapFaceList const &other) { *this = other; }
...
class CMapFaceIDList : public CUtlVector<int> {
 public:
  ...
  void Intersect(CMapFaceIDList &IntersectWith, CMapFaceIDList &In,
                 CMapFaceIDList &Out);
};
...
// Purpose: Copy constructor.
//-----------------------------------------------------------------------------
CMapFaceIDList::CMapFaceIDList(CMapFaceIDList const &other) { *this = other; }
