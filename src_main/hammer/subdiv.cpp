        ...
        for (int j = 0; j < 2; j++) {
          if (m_pEdges[i]->m_pQuads[j]) {
            for (int k = 0; k < quadCount; k++) {
              if (m_pEdges[i]->m_pQuads[j] == quadList[k]) break;
            }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CompareSubdivPoints(const CSubdivPoint *pPoint1,
                         const CSubdivPoint *pPoint2, float tolerance) {
  for (int i = 0; i < 3; i++) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CompareSubdivPointToPoint(const CSubdivPoint *pSubdivPoint,
                               const Vector &point, float tolerance) {
  for (int i = 0; i < 3; i++) {
    if (fabs(pSubdivPoint->m_Point[i] - point[i]) > tolerance) return false;
  }
...
bool CompareSubdivEdges(const CSubdivEdge *pEdge1, const CSubdivEdge *pEdge2) {
  if (((pEdge1->m_ndxPoint[0] == pEdge2->m_ndxPoint[0]) &&
       (pEdge1->m_ndxPoint[1] == pEdge2->m_ndxPoint[1])) ||
      ((pEdge1->m_ndxPoint[0] == pEdge2->m_ndxPoint[1]) &&
       (pEdge1->m_ndxPoint[1] == pEdge2->m_ndxPoint[0])))
    return true;
...
int CSubdivMesh::AddPoint(const Vector &point, const Vector &normal) {
  ...
  for (int i = 0; i < m_PointCount; i++) {
    if (CompareSubdivPointToPoint(&m_pPoints[i], point, 0.01f)) return i;
  }
...
void CSubdivMesh::RemovePoint(Vector &point) {
  ...
  //
  for (int i = 0; i < m_PointCount; i++) {
    if (!CompareSubdivPointToPoint(&m_pPoints[i], point, 0.01f)) continue;
...
int CSubdivMesh::AddEdge(CSubdivEdge *edge) {
  ...
  for (int i = 0; i < m_EdgeCount; i++) {
    if (CompareSubdivEdges(edge, &m_pEdges[i])) {
      ...
      //
      if ((!m_pEdges[i].m_pQuads[1]) &&
          (edge->m_pQuads[0] != m_pEdges[i].m_pQuads[0])) {
...
void CSubdivMesh::CatmullClarkSubdivide(void) {
  ...
  //
  // if point index if part of edge, add to point edge list and increment
  // valence
  //
  for (i = 0; i < m_PointCount; i++) {
    for (int j = 0; j < m_EdgeCount; j++) {
      if (!m_pEdges[j].m_Active) continue;

      if ((i == m_pEdges[j].m_ndxPoint[0]) ||
          (i == m_pEdges[j].m_ndxPoint[1])) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSubdivMesh::SetEdgeData(CSubdivQuad *pRoot, int index, int parentIndex,
                              int subdivIndex) {
  for (int i = 0; i < 4; i++) {
    CSubdivEdge edge;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSubdivMesh::CreateChildQuad4(CSubdivQuad *pRoot, int index,
                                   int parentIndex) {
  //
  // set quad indices -- displacement index values
  //
  pRoot[index].m_ndxQuad[0] =
      ((pRoot[parentIndex].m_ndxQuad[0] + pRoot[parentIndex].m_ndxQuad[3]) / 2);
  pRoot[index].m_ndxQuad[1] =
      ((pRoot[parentIndex].m_ndxQuad[0] + pRoot[parentIndex].m_ndxQuad[2]) / 2);
  pRoot[index].m_ndxQuad[2] =
      ((pRoot[parentIndex].m_ndxQuad[2] + pRoot[parentIndex].m_ndxQuad[3]) / 2);
  pRoot[index].m_ndxQuad[3] = pRoot[parentIndex].m_ndxQuad[3];

  //
  // set vert indices
  //
  pRoot[index].m_ndxVert[0] =
      AddPoint(m_pEdges[pRoot[parentIndex].m_ndxEdge[3]].m_NewEdgePoint,
               m_pEdges[pRoot[parentIndex].m_ndxEdge[3]].m_NewEdgeNormal);
  pRoot[index].m_ndxVert[1] =
      AddPoint(pRoot[parentIndex].m_Centroid, pRoot[parentIndex].m_Normal);
  pRoot[index].m_ndxVert[2] =
      AddPoint(m_pEdges[pRoot[parentIndex].m_ndxEdge[2]].m_NewEdgePoint,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSubdivMesh::CreateChildQuad3(CSubdivQuad *pRoot, int index,
                                   int parentIndex) {
  //
  // set quad indices -- displacement index values
  //
  pRoot[index].m_ndxQuad[0] =
      ((pRoot[parentIndex].m_ndxQuad[0] + pRoot[parentIndex].m_ndxQuad[2]) / 2);
  pRoot[index].m_ndxQuad[1] =
      ((pRoot[parentIndex].m_ndxQuad[1] + pRoot[parentIndex].m_ndxQuad[2]) / 2);
  pRoot[index].m_ndxQuad[2] = pRoot[parentIndex].m_ndxQuad[2];
  pRoot[index].m_ndxQuad[3] =
      ((pRoot[parentIndex].m_ndxQuad[2] + pRoot[parentIndex].m_ndxQuad[3]) / 2);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSubdivMesh::CreateChildQuad2(CSubdivQuad *pRoot, int index,
                                   int parentIndex) {
  //
  // set quad indices -- displacement index values
  //
  pRoot[index].m_ndxQuad[0] =
      ((pRoot[parentIndex].m_ndxQuad[0] + pRoot[parentIndex].m_ndxQuad[1]) / 2);
  ...
  //
  pRoot[index].m_ndxVert[0] =
      AddPoint(m_pEdges[pRoot[parentIndex].m_ndxEdge[0]].m_NewEdgePoint,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CSubdivMesh::CreateChildQuad1(CSubdivQuad *pRoot, int index,
                                   int parentIndex) {
  //
  // set quad indices -- displacement index values
  //
