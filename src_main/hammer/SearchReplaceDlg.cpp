...
struct FindObject_t {
  ...
  CMapWorld *pWorld;
  EnumChildrenPos_t
      WorldPos;  // A position in the world tree for world searches.

  CUtlVector<CMapClass *> SelectionList;  // A copy of the selection list for
                                          // selection only searches.
  int nSelectionIndex;  // The index into the selection list for iterating the
                        // selection list.
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the string matches the search criteria, false if
// not. Input  : pszString - String to check.
//			FindObject - Search criteria, including string to search
// for.
//-----------------------------------------------------------------------------
...
// for.
//-----------------------------------------------------------------------------
bool ReplaceString(char *pszOut, const char *pszIn, FindObject_t &FindObject,
                   const char *pszReplace) {
  //
  // Whole matches are simple, just strcpy the replacement string into the out
  // buffer.
  //
  if (FindObject.bWholeWord) {
...
// Input  : pObject -
//			FindObject -
// Output : Returns true if the object matches the search criteria, false if
// not.
//-----------------------------------------------------------------------------
bool FindCheck(CMapClass *pObject, FindObject_t &FindObject) {
...
//			FindObject -
//			pszReplaceText -
// Output : Returns the number of occurrences of the find text that were
// replaced.
//-----------------------------------------------------------------------------
int FindReplace(CMapEntity *pEntity, FindObject_t &FindObject,
                const char *pszReplace) {
  ...
  for (int i = 0; i < nConnCount; i++) {
    ...
    if (pConn) {
      ...
      if (ReplaceString(szNewValue, pConn->GetParam(), FindObject,
                        pszReplace)) {
...
// Input  : FindObject -
//-----------------------------------------------------------------------------
void CSearchReplaceDlg::GetFindCriteria(FindObject_t &FindObject,
                                        CMapDoc *pDoc) {
...
//-----------------------------------------------------------------------------
// Purpose: Called when they hit the Find, the Replace, or the Replace All
// button. Input  : uCmd - The ID of the button the user hit, IDC_FIND_NEXT or
// IDC_REPLACE. Output : Returns TRUE to indicate that the message was handled.
//-----------------------------------------------------------------------------
BOOL CSearchReplaceDlg::OnFindReplace(UINT uCmd) {
  ...
  do {
    ...
    if (m_bNewSearch) {
      ...
      //
      // We have to keep track of the last object in the iteration for
      // replacement, because replacement is done when me advance to the next
      // object.
      //
      pLastFound = NULL;
      nReplaceCount = 0;
    ...
    //
    // Replace All is undone as single operation. Mark the undo position the
    // first time we find a match during a Replace All.
    //
    ...
    if (pLastFound && ((uCmd == IDC_REPLACE) || (uCmd == IDC_REPLACE_ALL))) {
      ...
      //
      // Do the replace on the last matching object we found. This lets the user
      // see what object will be modified before it is done.
      //
      GetHistory()->Keep(pLastFound);
      nReplaceCount +=
          FindReplace((CMapEntity *)pLastFound, FindObject, m_strReplaceText);
    ...
    if (pObject) {
      ...
      if ((m_bNewSearch) || (uCmd != IDC_REPLACE_ALL)) {
        ...
        CString str;
        str.Format("Replaced %d occurrences of the string '%s' with '%s'.",
                   nReplaceCount, m_strFindText.GetString(),
                   m_strReplaceText.GetString());
        MessageBox(str, "Find/Replace Text", MB_OK);
      }
