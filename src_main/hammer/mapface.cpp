...
//
// Components of the texture axes are rounded to integers within this tolerance.
// This tolerance corresponds to an angle of about 0.06 degrees.
//
...
static const char *InvisToolTextures[] = {
    "occluder", "areaportal", "invisible", "skip",        "trigger",
    "hint",     "fog",        "origin",    "toolsnodraw",
};
...
// Output : CMapFace
//-----------------------------------------------------------------------------
CMapFace *CMapFace::CopyFrom(const CMapFace *pObject, DWORD dwFlags,
  ...
  if (pFrom != NULL) {
    ...
    if (dwFlags & COPY_FACE_POINTS) {
      ...
      if (pFrom->Points && nPoints) {
        ...
        memcpy(m_pTextureCoords, pFrom->m_pTextureCoords,
               sizeof(Vector2D) * nPoints);
        memcpy(m_pLightmapCoords, pFrom->m_pLightmapCoords,
               sizeof(Vector2D) * nPoints);
        memcpy(m_pTangentAxes, pFrom->m_pTangentAxes,
               sizeof(TangentSpaceAxes_t) * nPoints);
      }
    } else {
      Points = NULL;
      m_pTextureCoords = 0;
    ...
    // a face that does not have a displacement, because you cannot undo a
    // Generate Displacement operation.
    //
    if (pFrom->HasDisp()) {
      //
...
//-----------------------------------------------------------------------------
// Purpose: Determines the general orientation of a face based on its normal
// vector. Output : FaceOrientation_t
//-----------------------------------------------------------------------------
FaceOrientation_t CMapFace::GetOrientation(void) const {
  // The normal must have a nonzero length!
  if ((plane.normal[0] == 0) && (plane.normal[1] == 0) &&
      (plane.normal[2] == 0)) {
...
//			dwFlags -
//-----------------------------------------------------------------------------
void CMapFace::InitializeTextureAxes(TextureAlignment_t eAlignment,
                                     DWORD dwFlags) {
  FaceOrientation_t eOrientation;

  //
  // If the texture axis information has been initialized, don't reinitialize
  // unless the FORCE flag is set.
  //
  if ((!(dwFlags & INIT_TEXTURE_FORCE)) &&
      ((texture.UAxis[0] != 0) || (texture.UAxis[1] != 0) ||
       (texture.UAxis[2] != 0) || (texture.VAxis[0] != 0) ||
       (texture.VAxis[1] != 0) || (texture.VAxis[2] != 0))) {
    return;
  }
  ...
  if (dwFlags & INIT_TEXTURE_AXES) {
    ...
    // Determine the general orientation of this face (floor, ceiling, n wall,
    // etc.)
    ...
    if (eAlignment == TEXTURE_ALIGN_FACE) {
      ...
      CrossProduct(plane.normal, texture.VAxis.AsVector3D(),
                   texture.UAxis.AsVector3D());
      VectorNormalize(texture.UAxis.AsVector3D());

      // Now use the true U axis to calculate the true V axis.
      CrossProduct(texture.UAxis.AsVector3D(), plane.normal,
                   texture.VAxis.AsVector3D());
      VectorNormalize(texture.VAxis.AsVector3D());
    }
    //
    // If we are using world (or "natural") aligned textures, use the V axis as
    // is and pick the corresponding U axis from the table.
    //
...
BOOL CMapFace::IsTextureAxisValid(void) const {
  ...
  CrossProduct(texture.VAxis.AsVector3D(), texture.UAxis.AsVector3D(),
               TexNormalAxis);
  return (DotProduct(plane.normal, TexNormalAxis) != 0);
}

//-----------------------------------------------------------------------------
// Purpose: Normalize the U/V shift values to be less than the texture
// width/height.
//-----------------------------------------------------------------------------
...
//			BottomRight -
//-----------------------------------------------------------------------------
void CMapFace::GetFaceTextureExtents(Vector2D &TopLeft,
  ...
  for (int nPoint = 0; nPoint < nPoints; nPoint++) {
    ...
    Test[0] = DotProduct(Points[nPoint], texture.UAxis.AsVector3D()) /
              texture.scale[0];
    Test[1] = DotProduct(Points[nPoint], texture.VAxis.AsVector3D()) /
              texture.scale[1];
...
int CMapFace::GetTextureAlignment(void) const {
  ...
  if (eOrientation != FACE_ORIENTATION_INVALID) {
    ...
    CrossProduct(DownVectors[eOrientation], RightVectors[eOrientation],
                 WorldTexNormal);
...
//			BottomRight -
//-----------------------------------------------------------------------------
void CMapFace::GetTextureExtents(Extents_t Extents, Vector2D &TopLeft,
                                 Vector2D &BottomRight) const {
  BOOL bFirst = TRUE;
  ...
  for (int nPoint = 0; nPoint < NUM_EXTENTS_DIMS; nPoint++) {
    ...
    Test[0] = DotProduct(Extents[nPoint], texture.UAxis.AsVector3D()) /
              texture.scale[0];
    Test[1] = DotProduct(Extents[nPoint], texture.VAxis.AsVector3D()) /
              texture.scale[1];
...
//			Extents -
//-----------------------------------------------------------------------------
void CMapFace::JustifyTextureUsingExtents(TextureJustification_t eJustification,
                                          Extents_t Extents) {
  Vector2D Center;
  ...
  switch (eJustification) {
    // Align the top left corner of the texture with the top left corner of the
    // face.
    ...
    case TEXTURE_JUSTIFY_FIT: {
      ...
      if (m_pTexture && m_pTexture->GetWidth() && m_pTexture->GetHeight()) {
        texture.scale[0] =
            (BottomRight[0] - TopLeft[0]) / m_pTexture->GetWidth();
        texture.scale[1] =
            (BottomRight[1] - TopLeft[1]) / m_pTexture->GetHeight();
      } else {
        texture.scale[0] = g_pGameConfig->GetDefaultTextureScale();
        texture.scale[1] = g_pGameConfig->GetDefaultTextureScale();
      }
...
void CMapFace::OffsetTexture(const Vector &Delta) {
  ...
  //
  texture.UAxis[3] -=
      DotProduct(Delta, texture.UAxis.AsVector3D()) / texture.scale[0];
  texture.VAxis[3] -=
      DotProduct(Delta, texture.VAxis.AsVector3D()) / texture.scale[1];
...
void CMapFace::CalcPlane(void) {
  ...
  //
  plane.normal = GetNormalFromPoints(plane.planepts[0], plane.planepts[1],
                                     plane.planepts[2]);
...
//			the corresponding texture and lightmap coordinates
//(Vector2D's). Frees 			current points if there are any.
// Input  : _nPoints - number of points needed.
// Output : Total size of memory used by the points, texture, and lightmap
// coordinates.
//-----------------------------------------------------------------------------
...
void CMapFace::SetTexture(IEditorTexture *pTexture,
  ...
  if (m_pTexture != NULL) {
    ...
    bTexValid = !(m_pTexture->GetWidth() == 0 || m_pTexture->GetHeight() == 0 ||
                  m_pTexture->GetImageWidth() == 0 ||
                  m_pTexture->GetImageHeight() == 0 || !m_pTexture->HasData());
  }
...
void CMapFace::CalcTextureCoordAtPoint(const Vector &pt, Vector2D &texCoord) {
  ...
  //
  float s = DotProduct(texture.UAxis.AsVector3D(), pt) / texture.scale[0] +
            texture.UAxis[3];
  float t = DotProduct(texture.VAxis.AsVector3D(), pt) / texture.scale[1] +
            texture.VAxis[3];
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMapFace::CalcLightmapCoordAtPoint(const Vector &pt,
                                        Vector2D &lightCoord) {
  lightCoord[0] =
      DotProduct(texture.UAxis.AsVector3D(), pt) / texture.nLightmapScale +
      0.5f;
  lightCoord[1] =
      DotProduct(texture.VAxis.AsVector3D(), pt) / texture.nLightmapScale +
      0.5f;
}
...
void CMapFace::CalcTextureCoords(void) {
  ...
  for (i = 0; i < nPoints; i++) {
    ...
    //
    s = DotProduct(texture.UAxis.AsVector3D(), Points[i]) / texture.scale[0] +
        texture.UAxis[3];
    t = DotProduct(texture.VAxis.AsVector3D(), Points[i]) / texture.scale[1] +
        texture.VAxis[3];
    ...
    //
    // Generate lightmap coordinates.  Lightmap coordinates for displacements
    // happens below.
    //
    if (m_DispHandle == EDITDISPHANDLE_INVALID) {
      ...
      m_pLightmapCoords[i][0] =
          DotProduct(texture.UAxis.AsVector3D(), Points[i]) /
              texture.nLightmapScale +
          texture.UAxis[3] * shiftScaleU + 0.5;
      m_pLightmapCoords[i][1] =
          DotProduct(texture.VAxis.AsVector3D(), Points[i]) /
              texture.nLightmapScale +
          texture.VAxis[3] * shiftScaleV + 0.5;
    }
  }

  //
  // update the displacement map with new texture coordinates and calculate
...
int CMapFace::MaxLightmapSize() const {
  return HasDisp() ? MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER
  ...
  for (int nPlane = 0; nPlane < 3; nPlane++) {
    for (int nPlaneCheck = 0; nPlaneCheck < 3; nPlaneCheck++) {
      if (nPlane != nPlaneCheck) {
        if (VectorCompare(plane.planepts[nPlane],
                          plane.planepts[nPlaneCheck])) {
          if (pInfo != NULL) {
...
//-----------------------------------------------------------------------------
// Purpose: Included for loading old (quake-style) maps. This sets up the
// texture axes
//			the same way QCSG and pre-2.2 Hammer did.
// Input  : UAxis -
//			VAxis -
//-----------------------------------------------------------------------------
void CMapFace::InitializeQuakeStyleTextureAxes(Vector4D &UAxis,
                                               Vector4D &VAxis) {
...
void CMapFace::DrawFace(Color &pColor, EditorRenderMode_t mode) {
  ...
  if (mode == RENDER_MODE_LIGHT_PREVIEW_RAYTRACED) {
    if (m_nFaceFlags & FACE_FLAGS_NODRAW_IN_LPREVIEW) return;
  }

  MaterialPrimitiveType_t type =
      (mode == RENDER_MODE_WIREFRAME) ? MATERIAL_LINE_LOOP : MATERIAL_POLYGON;
  ...
  for (int nPoint = 0; nPoint < nPoints; nPoint++) {
    if (ModeUsesTextureCoords(mode)) {
      meshBuilder.TexCoord2f(0, m_pTextureCoords[nPoint][0],
                             m_pTextureCoords[nPoint][1]);
      meshBuilder.TexCoord2f(1, m_pLightmapCoords[nPoint][0],
                             m_pLightmapCoords[nPoint][1]);
    }
    ...
    if (hasParent) {
      ...
      meshBuilder.Position3f(Points[nPoint][0], Points[nPoint][1],
                             Points[nPoint][2]);
    }
...
bool CMapFace::ShouldRenderLast() {
  if (!m_pTexture || !m_pTexture->GetMaterial()) return false;
...
static int SortVal(EditorRenderMode_t mode) {
  if (mode == RENDER_MODE_WIREFRAME) return 2;
  if (mode == RENDER_MODE_SELECTION_OVERLAY) return 1;
  return 0;
}
...
// Output :
//-----------------------------------------------------------------------------
static bool OpaqueFacesLessFunc(const MapFaceRender_t &s1,
                                const MapFaceRender_t &s2) {
  // Render texture first, overlay second, wireframe 3rd
  int nSort1 = SortVal(s1.m_RenderMode);
  int nSort2 = SortVal(s2.m_RenderMode);
  if (nSort1 < nSort2) return true;
  if (nSort1 > nSort2) return false;
  return s1.m_pTexture < s2.m_pTexture;
}

static CUtlRBTree<MapFaceRender_t, int> g_OpaqueFaces(0, 0,
                                                      OpaqueFacesLessFunc);
...
static void AddFaceToQueue(CMapFace *pMapFace, IEditorTexture *pTexture,
                           EditorRenderMode_t renderMode, bool selected,
                           SelectionState_t faceSelectionState) {
...
// render texture axes
//-----------------------------------------------------------------------------
void CMapFace::RenderTextureAxes(CRender3D *pRender, int nCount,
                                 CMapFace **ppFaces) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    meshBuilder.Position3f(
        Center[0] + ppFaces[i]->texture.UAxis[0] * TEXTURE_AXIS_LENGTH,
    ...
    meshBuilder.Color3ub(0, 255, 0);
    meshBuilder.Position3f(
        Center[0] + ppFaces[i]->texture.VAxis[0] * TEXTURE_AXIS_LENGTH,
...
// Render grids
//-----------------------------------------------------------------------------
void CMapFace::RenderGridsIfCloseEnough(CRender3D *pRender, int nCount,
                                        CMapFace **ppFaces) {
...
//-----------------------------------------------------------------------------
void CMapFace::AddFaceVertices(CMeshBuilder &meshBuilder, CRender3D *pRender,
  ...
  ComputeColor(pRender, bRenderSelected, faceSelectionState, m_bIgnoreLighting,
               color);
...
//-----------------------------------------------------------------------------
void CMapFace::RenderWireframeFaces(CRender3D *pRender, int nCount,
...
//-----------------------------------------------------------------------------
void CMapFace::RenderFacesBatch(CMeshBuilder &meshBuilder, IMesh *pMesh,
                                CRender3D *pRender, MapFaceRender_t **ppFaces,
                                int nFaceCount, int nVertexCount,
                                int nIndexCount, bool bWireframe) {
  if (bWireframe) {
  ...
  for (int i = 0; i < nFaceCount; ++i) {
    ...
    pMapFace->AddFaceVertices(meshBuilder, pRender,
                              ppFaces[i]->m_RenderSelected,
                              ppFaces[i]->m_FaceSelectionState);
...
//-----------------------------------------------------------------------------
void CMapFace::RenderFaces(CRender3D *pRender, int nCount,
                           MapFaceRender_t **ppFaces) {
  if (nCount == 0) return;
  ...
  for (int nFace = 0; nFace < nCount; nFace++) {
    ...
    if (((nVertexCount + newVertices) > nMaxVerts) ||
        ((nIndexCount + newIndices) > nMaxIndices)) {
      // If we hit this assert, there's a single face that's too big for the
      // meshbuilder to handle!
      Assert((nFace - nBatchStart) > 0);

      // We have a full batch, render it.

      RenderFacesBatch(meshBuilder, pMesh, pRender, &ppFaces[nBatchStart],
                       nFace - nBatchStart, nVertexCount, nIndexCount,
                       bWireframe);
  ...
  RenderFacesBatch(meshBuilder, pMesh, pRender, &ppFaces[nBatchStart],
                   nCount - nBatchStart, nVertexCount, nIndexCount, bWireframe);
...
//-----------------------------------------------------------------------------
void CMapFace::RenderFace3D(CRender3D *pRender, EditorRenderMode_t renderMode,
                            bool renderSelected,
                            SelectionState_t faceSelectionState) {
  pRender->PushRenderMode(renderMode);

  if (HasDisp() && CMapDoc::GetActiveMapDoc() &&
      CMapDoc::GetActiveMapDoc()->IsDispDraw3D()) {
    CMapDisp *pDisp = EditDispMgr()->GetDisp(m_DispHandle);
    pDisp->Render3D(pRender, renderSelected, faceSelectionState);
  } else {
    Color color;
    ComputeColor(pRender, renderSelected, faceSelectionState, m_bIgnoreLighting,
                 color);
  ...
  if (renderMode == RENDER_MODE_WIREFRAME) {
    if (faceSelectionState != SELECT_NONE) RenderTextureAxes(pRender);

    // Draw the grid
    RenderGridIfCloseEnough(pRender);
  } else if (m_pDetailObjects && Options.general.bShowDetailObjects) {
    // Only draw the detailed objects if the displacement/face is not currently
    // selected.
...
void CMapFace::RenderOpaqueFaces(CRender3D *pRender) {
  MapFaceRender_t **ppMapFaces = (MapFaceRender_t **)_alloca(
      g_OpaqueFaces.Count() * sizeof(MapFaceRender_t *));
  int nFaceCount = 0;

  int nLastRenderMode = RENDER_MODE_NONE;
  IEditorTexture *pLastTexture = NULL;

  for (int i = g_OpaqueFaces.FirstInorder(); i != g_OpaqueFaces.InvalidIndex();
       i = g_OpaqueFaces.NextInorder(i)) {
    MapFaceRender_t &mapFace = g_OpaqueFaces[i];

    if ((mapFace.m_RenderMode != nLastRenderMode) ||
        (mapFace.m_pTexture != pLastTexture)) {
    ...
    if (mapFace.m_pMapFace->HasDisp()) {
      ...
      mapFace.m_pMapFace->RenderFace3D(pRender, mapFace.m_RenderMode,
                                       mapFace.m_RenderSelected,
                                       mapFace.m_FaceSelectionState);
    } else {
...
void CMapFace::Render2D(CRender2D *pRender) {
  ...
  bRenderSelected = bRenderSelected || ((eFaceSelectionState != SELECT_NONE) &&
                                        (CMapFace::m_bShowFaceSelection));

  Vector vViewNormal;
  pRender->GetCamera()->GetViewForward(vViewNormal);
  Vector vNormal;
  GetFaceNormal(vNormal);
  ...
  if (HasDisp() && (bIsParallel || bRenderSelected)) {
    ...
    bool bDrawDispMap =
        Options.view2d.bDrawModels && ((sizeX + sizeY) > 50 || bRenderSelected);
...
void CMapFace::RenderVertices(CRender *pRender) {
  for (int i = 0; i < nPoints; i++) pRender->DrawHandle(Points[i]);
}
...
void CMapFace::Render3D(CRender3D *pRender) {
  ...
  if (!Options.general.bShowNoDrawBrushes &&
      eSolidSelectionState == SELECT_NONE &&
      m_pTexture == g_Textures.GetNoDrawTexture())
    return;
  ...
  renderSelected = renderSelected || ((eFaceSelectionState != SELECT_NONE) &&
                                      (CMapFace::m_bShowFaceSelection));

  if (pRender->DeferRendering()) {
    AddFaceToQueue(this, m_pTexture, eCurrentRenderMode, renderSelected,
                   eFaceSelectionState);
    if (renderSelected && pRender->NeedsOverlay()) {
      AddFaceToQueue(this, m_pTexture, RENDER_MODE_SELECTION_OVERLAY,
    ...
    RenderFace3D(pRender, eCurrentRenderMode, renderSelected,
                 eFaceSelectionState);
    if (renderSelected && pRender->NeedsOverlay()) {
      RenderFace3D(pRender, RENDER_MODE_SELECTION_OVERLAY, renderSelected,
                   eFaceSelectionState);
    }
  }
}
      ...
      while (fGridPoint < fEnd) {
        ...
        for (int nPoint = 0; nPoint < nPoints; nPoint++) {
          ...
          if (Point2[nDim] != Point1[nDim]) {
            ...
            //
            float fScale =
                (fGridPoint - Point1[nDim]) / (Point2[nDim] - Point1[nDim]);
            if ((fScale >= 0) && (fScale <= 1)) {
              PointFound[nPointsFound][0] =
                  Point1[0] + (Point2[0] - Point1[0]) * fScale;
              PointFound[nPointsFound][1] =
                  Point1[1] + (Point2[1] - Point1[1]) * fScale;
              PointFound[nPointsFound][2] =
                  Point1[2] + (Point2[2] - Point1[2]) * fScale;
              ...
              if (nPointsFound == 2) {
                ...
                meshBuilder.Position3f(RenderPoint[0], RenderPoint[1],
                                       RenderPoint[2]);
                meshBuilder.Color3ub(Normal[0] * 255, Normal[1] * 255,
                                     Normal[2] * 255);
                meshBuilder.AdvanceVertex();
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapFace::LoadDispInfoCallback(CChunkFile *pFile,
                                                 CMapFace *pFace) {
  SignalUpdate(EVTYPE_FACE_CHANGED);
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CMapFace::LoadKeyCallback(const char *szKey,
                                            const char *szValue,
                                            LoadFace_t *pLoadFace) {
  SignalUpdate(EVTYPE_FACE_CHANGED);
...
ChunkFileResult_t CMapFace::LoadVMF(CChunkFile *pFile) {
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadKeyCallback, &LoadFace);
  pFile->PopHandlers();
...
ChunkFileResult_t CMapFace::SaveVMF(CChunkFile *pFile, CSaveInfo *pSaveInfo) {
  ...
  if (eResult == ChunkFile_Ok) {
    sprintf(szBuf, "(%g %g %g) (%g %g %g) (%g %g %g)",
            (double)plane.planepts[0][0], (double)plane.planepts[0][1],
            (double)plane.planepts[0][2], (double)plane.planepts[1][0],
            (double)plane.planepts[1][1], (double)plane.planepts[1][2],
            (double)plane.planepts[2][0], (double)plane.planepts[2][1],
            (double)plane.planepts[2][2]);
  ...
  if (eResult == ChunkFile_Ok) {
    sprintf(szBuf, "[%g %g %g %g] %g", (double)texture.UAxis[0],
            (double)texture.UAxis[1], (double)texture.UAxis[2],
            (double)texture.UAxis[3], (double)texture.scale[0]);
    eResult = pFile->WriteKeyValue("uaxis", szBuf);
  }

  if (eResult == ChunkFile_Ok) {
    sprintf(szBuf, "[%g %g %g %g] %g", (double)texture.VAxis[0],
            (double)texture.VAxis[1], (double)texture.VAxis[2],
            (double)texture.VAxis[3], (double)texture.scale[1]);
  ...
  if (eResult == ChunkFile_Ok) {
    eResult =
        pFile->WriteKeyValueFloat("lightmapscale", texture.nLightmapScale);
  }
...
bool CMapFace::GetRender2DBox(Vector &boundMin, Vector &boundMax) {
  ...
  for (int i = 0; i < nPoints; i++) {
    if (Points[i][0] < boundMin[0]) {
      boundMin[0] = Points[i][0];
    }
    if (Points[i][1] < boundMin[1]) {
      boundMin[1] = Points[i][1];
    }
    if (Points[i][2] < boundMin[2]) {
      boundMin[2] = Points[i][2];
    }

    if (Points[i][0] > boundMax[0]) {
      boundMax[0] = Points[i][0];
    }
    if (Points[i][1] > boundMax[1]) {
      boundMax[1] = Points[i][1];
    }
    if (Points[i][2] > boundMax[2]) {
      boundMax[2] = Points[i][2];
    }
  }
...
bool CMapFace::GetCullBox(Vector &boundMin, Vector &boundMax) {
  ...
  if (HasDisp()) {
    ...
    for (int i = 0; i < 2; i++) {
      if (bbox[i][0] < boundMin[0]) {
        boundMin[0] = bbox[i][0];
      }
      if (bbox[i][1] < boundMin[1]) {
        boundMin[1] = bbox[i][1];
      }
      if (bbox[i][2] < boundMin[2]) {
        boundMin[2] = bbox[i][2];
      }

      if (bbox[i][0] > boundMax[0]) {
        boundMax[0] = bbox[i][0];
      }
      if (bbox[i][1] > boundMax[1]) {
        boundMax[1] = bbox[i][1];
      }
      if (bbox[i][2] > boundMax[2]) {
        boundMax[2] = bbox[i][2];
      }
    }
  }
...
//-----------------------------------------------------------------------------
bool CMapFace::TraceLine(Vector &HitPos, Vector &HitNormal, Vector const &Start,
                         Vector const &End) {
  if (HasDisp()) {
    CMapDisp *pDisp = EditDispMgr()->GetDisp(m_DispHandle);
...
bool CMapFace::TraceLineInside(Vector &HitPos, Vector &HitNormal,
  ...
  for (int ndxEdge = 0; ndxEdge < nPoints; ndxEdge++) {
    ...
    float dist =
        edgePlane.normal.Dot(Points[(ndxEdge + 2) % nPoints]) - edgePlane.dist;
    if (dist > 0.0f) {
      // flip
...
bool CMapFace::AllocTangentSpaceAxes(int count) {
  ...
  if (!m_pTangentAxes) return false;
...
bool CMapFace::InSmoothingGroup(int iGroup) {
  if ((m_fSmoothingGroups & (1 << (iGroup - 1))) != 0) return true;
...
void CMapFace::DoTransform(const VMatrix &matrix) {
  ...
  if (HasDisp()) {
    ...
    if (pDisp) pDisp->GetBoundingBox(bbDispOld[0], bbDispOld[1]);
  }
  ...
  if (mTrans.IsIdentity()) {
    ...
    if (pDisp) {
      ...
      // Update the neighbors of displacements that intersect the old as well as
      // the new bbox. Without this, there can be errors if you drag > 2 edges
      // to interset each other, then move one of the intersectors (cloning can
      // easily cause this case to happen).
      Vector bbDispNew[2];
      pDisp->GetBoundingBox(bbDispNew[0], bbDispNew[1]);

      CMapDisp::UpdateNeighborsOfDispsIntersectingBox(bbDispOld[0],
                                                      bbDispOld[1], 1.0);
      CMapDisp::UpdateNeighborsOfDispsIntersectingBox(bbDispNew[0],
                                                      bbDispNew[1], 1.0);
    }
  ...
  if (GetTexture()) {
    ...
    // check is the transformation would destory the UV axis (both normals &
    // perpendicular)
    bool bUVAxisSameScale =
        fequal(fScaleV, 1, 0.0001) && fequal(fScaleU, 1, 0.0001);
    ...
    if (bUVAxisSameScale) {
      // scale is fine
      if (!bIsLocking) {
        // If we are not texture locking, recalculate the texture axes based on
        // current texture alignment setting. This prevents the axes from
        // becoming normal to the face.
        InitializeTextureAxes(Options.GetTextureAlignment(),
                              INIT_TEXTURE_AXES | INIT_TEXTURE_FORCE);
      }
  ...
  if (pDisp) {
    ...
    // Update the neighbors of displacements that intersect the old as well as
    // the new bbox. Without this, there can be errors if you drag > 2 edges to
    // interset each other, then move one of the intersectors (cloning can
    // easily cause this case to happen).
    Vector bbDispNew[2];
    pDisp->GetBoundingBox(bbDispNew[0], bbDispNew[1]);

    CMapDisp::UpdateNeighborsOfDispsIntersectingBox(bbDispOld[0], bbDispOld[1],
                                                    1.0);
    CMapDisp::UpdateNeighborsOfDispsIntersectingBox(bbDispNew[0], bbDispNew[1],
                                                    1.0);
  }
  // Create any detail objects if appropriate
  DetailObjects::BuildAnyDetailObjects(this);
