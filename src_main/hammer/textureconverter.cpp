...
void CTextureConverter::ConvertWorldTextures(CMapWorld *pWorld) {
  ...
  AfxMessageBox(
      "Conversion complete.  Check the Hammer \"Messages\" window for complete "
      "details.");
}
...
void CTextureConverter::ConvertSolids(CMapWorld *pWorld) {
  // Count total map solids so we know how many we have to do (for progress
  // meter).
  pWorld->EnumChildren(ENUMMAPCHILDRENPROC(CountMapSolids), 0,
  ...
  pWorld->EnumChildren(ENUMMAPCHILDRENPROC(CheckSolidTextures), 0,
                       MAPCLASS_TYPE(CMapSolid));
}
...
//-----------------------------------------------------------------------------
// Purpose: Enumeration function, check all the faces of a solid for texture
// conversion Input  : pSolid - map solid to be checked. Output : Always return
// true to continue enumerating.
//-----------------------------------------------------------------------------
bool CTextureConverter::CheckSolidTextures(CMapSolid *pSolid, DWORD) {
  ...
  if (m_nCurrentSolid % 100 == 0) m_pProgDlg->SetPos(m_nCurrentSolid);
...
void CTextureConverter::CheckFaceTexture(CMapFace *pFace) {
  ...
  // Criteria for needing conversion is a) being a dummy texture AND b) having
  // no slashes in the texture name.
...
//-----------------------------------------------------------------------------
// Purpose: Determine if any materials match the old texture of a face and
// replace
//          appropriately.
...
void CTextureConverter::ConvertFaceTexture(CMapFace *pFace) {
  ...
  const char *replacements[][2] = {
      {"sky", "tools/toolsskybox"},         {"skip", "tools/toolsskip"},
      {"aaatrigger", "tools/toolstrigger"}, {"hint", "tools/toolshint"},
      {"clip", "tools/toolsclip"},          {"null", "tools/toolsnodraw"}};
...
void CTextureConverter::ConvertDecals(CMapWorld *pWorld) {
  // Count total map decals so we know how many we have to do (for progress
  // meter).
  pWorld->EnumChildren(ENUMMAPCHILDRENPROC(CountMapDecals), 0,
  ...
  // Cycle through the solids again and convert as necessary.
  pWorld->EnumChildren(ENUMMAPCHILDRENPROC(CheckDecalTextures), 0,
                       MAPCLASS_TYPE(CMapEntity));
}

//-----------------------------------------------------------------------------
// Purpose: Enumeration function, increment the decals counter if entity is a
// decal. Input  : Output : Always return true to continue enumerating.
//-----------------------------------------------------------------------------
bool CTextureConverter::CountMapDecals(CMapEntity *pEnt, DWORD) {
  if (!strcmp(pEnt->GetClassName(), "infodecal")) m_nDecalCount++;
...
//-----------------------------------------------------------------------------
// Purpose: Determine if any materials match the old texture of a decal and
// replace
//          appropriately.
...
void CTextureConverter::ConvertDecalTexture(CMapEntity *pEnt) {
  ...
  switch (tlMatches.Count()) {
    ...
    default:
      ...
      MsgConvertDecal(
          pEnt, "WARNING: Multiple matches found.  Using first match (%s).",
          pNewTexture->GetName());
      m_nWarnings++;
...
// Output : pMatchList - texture list is filled in with matching material
// textures.
//-----------------------------------------------------------------------------
void CTextureConverter::GetNewTextureMatches(const char *pszOldName,
                                             EditorTextureList_t &tlMatchList) {
  ...
  while (pTexture != NULL) {
    if (TextureNameMatchesMaterialName(
            pszOldName, pTexture->GetName()))  // check for a match
    {
...
//          as the last token (delimiter '/') of the new material name,
//          otherwise return false.
//-----------------------------------------------------------------------------
bool CTextureConverter::TextureNameMatchesMaterialName(
    const char *pszTextureName, const char *pszMaterialName) {
  ...
  if (pszPartialMaterialName != NULL) {
    ...
    pszPartialMaterialName =
        pszMaterialName;  // No slashes found, just point to the name
  }

  // No '/' found in the VMT name, or the name ended in a '/'.  This shouldn't
  // happen.
...
//-----------------------------------------------------------------------------
void CTextureConverter::ReplaceFaceTexture(CMapFace *pFace,
                                           IEditorTexture *pNewTexture) {
  if (!pNewTexture->Load())  // make sure new texture is loaded
  {
    MsgConvertFace(pFace,
                   "WARNING: Couldn't load new material.  Texture converted "
                   "but not re-scaled.");
    m_nWarnings++;
  }
...
//-----------------------------------------------------------------------------
void CTextureConverter::ReplaceDecalTexture(CMapEntity *pEnt,
                                            IEditorTexture *pNewTexture) {
...
//-----------------------------------------------------------------------------
void CTextureConverter::RescaleFaceTexture(CMapFace *pFace,
                                           IEditorTexture *pNewTexture) {
  int nNewWidth;
  int nNewHeight;
  ...
  // First look for the .resizeinfo in the mod dir (hl2\dod), then the game dir
  // (hl2\hl2).
  char resizeInfoFilename[512];
  Q_snprintf(resizeInfoFilename, sizeof(resizeInfoFilename),
             "materials\\%s.resizeinfo", pNewTexture->GetName());
  ...
  if ((nOldWidth == 0) || (nOldHeight == 0)) {
    MsgConvertFace(pFace,
                   "WARNING: Invalid old texture dimensions (%dx%d).  Texture "
                   "converted but not re-scaled.",
  ...
  if ((nNewWidth == 0) || (nNewHeight == 0)) {
    MsgConvertFace(pFace,
                   "WARNING: Invalid new material dimensions (%dx%d).  Texture "
                   "converted but not re-scaled.",
  ...
  pFace->CalcTextureCoords();  // recompute internals base on the new scaling
                               // and shifting.
}
...
//			... - The remaining arguments of the *printf style
//message
...
void CTextureConverter::MsgConvertFace(CMapFace *pFace, const char *format,
  ...
  Msg(mwStatus, "[face] %s at (%d,%d,%d):  %s", pFace->GetTexture()->GetName(),
      (int)vecFaceCenter[0], (int)vecFaceCenter[1], (int)vecFaceCenter[2],
      message);
}
...
void CTextureConverter::MsgConvertDecal(CMapEntity *pEnt, const char *format,
  ...
  Msg(mwStatus, "[decal] %s at (%d,%d,%d):  %s", pEnt->GetKeyValue("texture"),
      (int)vecOrigin.x, (int)vecOrigin.y, (int)vecOrigin.z, message);
}
...
void CTextureConverter::DisplayStatistics(void) {
  ...
  Msg(mwStatus, "Total conversions:            %10d",
      m_nFaceCount + m_nDecalCount);
