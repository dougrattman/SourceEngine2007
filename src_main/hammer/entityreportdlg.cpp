...
CEntityReportDlg::CEntityReportDlg(CMapDoc *pDoc, CWnd *pParent /*=NULL*/)
  ...
  m_bFilterByKeyvalue =
      pApp->GetProfileInt(pszIniSection, "FilterByKeyvalue", FALSE);
  m_bFilterByClass = pApp->GetProfileInt(pszIniSection, "FilterByClass", FALSE);
  m_bFilterByHidden =
      pApp->GetProfileInt(pszIniSection, "FilterByHidden", TRUE);
...
void CEntityReportDlg::OnFilterbytype() {
  ...
  do {
    ...
    if (::SendMessage(hWndCtrl, BM_GETCHECK, 0, 0L) != 0) break;
    iButton++;
...
void CEntityReportDlg::MarkSelectedEntities() {
  ...
  for (int i = 0; i < m_cEntities.GetCount(); i++) {
    if (!m_cEntities.GetSel(i)) continue;
    CMapEntity *pEntity = (CMapEntity *)m_cEntities.GetItemDataPtr(i);
...
void CEntityReportDlg::OnTimer(UINT nIDEvent) {
  ...
  // check filters
  if (!m_bFilterTextChanged) return;
...
BOOL AddEntityToList(CMapEntity *pEntity, CEntityReportDlg *pDlg) {
  ...
  */
  ...
  for (int i = pEntity->GetFirstKeyValue(); i != pEntity->GetInvalidKeyValue();
    ...
    sprintf(szString + strlen(szString), "\t%s \"%s\"", pszName,
            pEntity->GetKeyValue(i));
...
void CEntityReportDlg::UpdateEntityList(void) {
  ...
  pWorld->EnumChildren(ENUMMAPCHILDRENPROC(AddEntityToList), DWORD(this),
...
void CEntityReportDlg::GenerateReport() {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (!pc->IsBaseClass()) {
      str = pc->GetName();
      if (str != "worldspawn") m_cFilterClass.AddString(str);
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CEntityReportDlg::OnSelChangeEntityList() { MarkSelectedEntities(); }
...
// Purpose:
//-----------------------------------------------------------------------------
void CEntityReportDlg::OnOK() { DestroyWindow(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CEntityReportDlg::OnClose() { DestroyWindow(); }
