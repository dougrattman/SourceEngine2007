...
//			the mouse cursor moves over it. This enables stacking of
//the 			bars with only a small portion of each visible.
//
// $Workfile:     $
// $Date:         $
//
...
//			calls the function to move the bar's controls around on a
//resize. 			also calls the CWnd OnSize so it can do what it needs to.
...
//			DWORD dwMode - type of dialog/movement we are dealing
//with
...
CSize CHammerBar::CalcDynamicLayout(int nLength, DWORD dwMode) {
  ...
  // it should not be possible that a bar with no name could be dynamic; this
  // check is just to be safe
...
//			makes sure a bar created with this old function isn't
//dyanmic 			initializes size variables.
...
//-----------------------------------------------------------------------------
BOOL CHammerBar::Create(CWnd *pParentWnd, UINT nIDTemplate, UINT nStyle,
                        UINT nID) {
  // cannot have a dynamic bar with this Create function; must use the one that
  // takes the window name
...
//-----------------------------------------------------------------------------
BOOL CHammerBar::Create(CWnd *pParentWnd, UINT nIDTemplate, UINT nStyle,
                        UINT nID, const char *pszName) {
  UINT nStyleFixed = nStyle;
  if (*pszName == 0) {
    // did not give a title and cannot have a dynamic bar.  Routing back through
    // old create
    ...
    m_sizeFloating.cx =
        APP()->GetProfileInt(textTitle, "floatX", m_sizeDefault.cx);
    m_sizeFloating.cy =
        APP()->GetProfileInt(textTitle, "floatY", m_sizeDefault.cy);
    ...
    if (pControl != NULL) {
      if (dwPlacement & GROUP_BOX) {
        pControl->SetWindowPos(NULL, 0, 0,
                               nHammerBarWidth - nControlWidthDifference,
                               nHammerBarHeight - nControlHeightDifference,
      ...
      if (dwPlacement & BOTTOM_JUSTIFY) {
        ...
        pControl->SetWindowPos(NULL, controlPos.left - HammerBarPos.left,
                               HammerBarPos.Height() - currentControl.m_nPosY,
      ...
      if (dwPlacement & RIGHT_JUSTIFY) {
        ...
        pControl->SetWindowPos(
            NULL, HammerBarPos.Width() - currentControl.m_nPosX,
...
//			determines position and stretch settings based on default
//design /
...
//				BOTTOM_JUSTIFY - will move with the bottom of the
//dialog 				RIGHT_JUSTIFY - will move with the right side of the dialog//
// Output : void
//-----------------------------------------------------------------------------
