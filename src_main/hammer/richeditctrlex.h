...
class CBoolString {
 ...
 public:
  ...
  operator CString() const { return m_b ? m_strOn : m_strOff; }
};
...
class CTextAttributes {
 ...
 public:
  ...
  operator CString() const {
    CString s;
    s.Format(
        "\\plain%s%s%s%s\\f%d\\fs%d\\cb%d\\cf%d ",
        ((CString)m_bsBold).GetString(), ((CString)m_bsUnderline).GetString(),
        ((CString)m_bsItalic).GetString(), ((CString)m_bsStrike).GetString(),
...
class CFontList : public list<CString> {
 public:
  ...
  void add(const CString &s) { push_back(s); }
};
...
class RICHED_DECL CControlManip : public CManip {
 ...
 public:
  CControlManip(RTFSM_CONTROLPFUNC p, CRichEditCtrl &c)
      : m_control(c), CManip((LPVOID)p, (CString) ""){};
...
class RICHED_DECL CRTFBuilder {
 ...
 public:
  ...
  void addFont(const CString &s) { m_fontList.add(s); }

  void addColor(COLORREF c) { m_colorList.add(c); }

  CRTFBuilder &operator+=(LPCTSTR p);

  operator CString() const { return m_string; }

  void write(CRichEditCtrl &);

  int colorCount() const { return m_colorList.size(); }
...
class RICHED_DECL CRichEditCtrlEx : public CRichEditCtrl {
 ...
 public:
  ...
  void disable(bool b = true) { enable(!b); }

  void readOnly(bool b = true) { SetReadOnly(b); }

  void writable(bool b = true) { readOnly(!b); }
