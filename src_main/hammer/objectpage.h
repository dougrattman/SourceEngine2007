...
class CObjectPage : public CPropertyPage {
 ...
 public:
  ...
  // Called by the sheet to update the selected objects. pData points to the
  // object being added to the selection.
  virtual void UpdateData(int Mode, PVOID pData) {}

  // Called by the sheet to store this page's data into the objects being
  // edited.
  virtual bool SaveData(void) { return (true); }

  // Called by the sheet to let the dialog remember its state before a refresh
  // of the data.
  ...
  CRuntimeClass *GetEditObjectRuntimeClass(void) {
    return m_pEditObjectRuntimeClass;
  }
  PVOID GetEditObject();
  ...
  bool m_bFirstTimeActive;  // Used to detect the first time this page becomes
                            // active.
  bool m_bHasUpdatedData;   // Used to prevent SaveData() called on pages that
                            // haven't had loaded the data yet.
 ...
 protected:
  const CMapObjectList
      *m_pObjectList;  // The list of objects that we are editing.
  bool m_bMultiEdit;   // Set to true if we are editing more than one object.

  CRuntimeClass *
      m_pEditObjectRuntimeClass;  // The type of object that this page can edit.
