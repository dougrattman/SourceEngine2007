...
class CIncrementalLightInfo {
 public:
  ...
  void DiscardResults(void) {
    ...
    if (m_eIncrState != INCR_STATE_NEW) m_eIncrState = INCR_STATE_NO_RESULTS;
  }
...
class CLightingPreviewThread {
 public:
  ...
  // check if the master has new work for us to do, meaning we should abort
  // rendering
  bool ShouldAbort(void) { return g_HammerToLPreviewMsgQueue.MessageWaiting(); }
  ...
  // calculate m_MinViewCoords, m_MaxViewCoords - the bounding box of the
  // rendered pixels+the eye
  ...
  void CalculateForLightTask(int nLineMask, int nLineMatch,
                             CLightingPreviewLightDescription &l, int calc_mask,
  ...
  void DiscardResults(void) {
    ...
    for (CIncrementalLightInfo *i = m_pIncrementalLightInfoList; i;
         i = i->m_pNext) {
      i->DiscardResults();
    }
  ...
  Vector EstimatedUnshotAmbient(void) {
    ...
    if (m_pLightList)
      for (int i = 0; i < m_pLightList->Count(); i++) {
        CLightingPreviewLightDescription &l = (*m_pLightList)[i];
        CIncrementalLightInfo *l_info = l.m_pIncrementalInfo;
        if (l_info && (l_info->m_eIncrState == INCR_STATE_HAVE_FULL_RESULTS) ||
...
bool CIncrementalLightInfo::IsLowerPriorityThan(
    CLightingPreviewThread *pLPV, CIncrementalLightInfo const &other) const {
  ...
  if (highpriority && (!other_highpriority)) return false;
  if (other_highpriority && (!highpriority)) return true;
  ...
  switch (state_combo) {
    ...
    case INCR_STATE_PARTIAL_RESULTS + 16 * INCR_STATE_PARTIAL_RESULTS: {
      // if incrmental vs incremental, and no light from either, do most
      // recently lit one
      if ((m_fTotalContribution == 0.0) &&
          (other.m_fTotalContribution == 0.0) &&
          (other.m_nMostRecentNonZeroContributionTimeStamp >
           m_nMostRecentNonZeroContributionTimeStamp))
        return true;
    ...
    case INCR_STATE_PARTIAL_RESULTS + 16 * INCR_STATE_NO_RESULTS: {
      if (other.m_fTotalContribution) return true;
      if ((m_fTotalContribution == 0.0) && (other.m_fTotalContribution == 0.0))
        return (other.m_nMostRecentNonZeroContributionTimeStamp >
                m_nMostRecentNonZeroContributionTimeStamp);
      return (m_fTotalContribution < other.m_fTotalContribution);
    }
    case INCR_STATE_NO_RESULTS + 16 * INCR_STATE_PARTIAL_RESULTS: {
      if (m_fTotalContribution) return false;
      if ((m_fTotalContribution == 0.0) && (other.m_fTotalContribution == 0.0))
        return (other.m_nMostRecentNonZeroContributionTimeStamp >
                m_nMostRecentNonZeroContributionTimeStamp);
      return (m_fTotalContribution < other.m_fTotalContribution);
...
void CLightingPreviewThread::HandleGeomMessage(MessageToLPreview &msg_in) {
  ...
  if (tris.Count()) {
    ...
    for (int i = 0; i < tris.Count(); i += 3) {
      ...
      m_pRtEnv->AddTriangle(i, tris[i], tris[1 + i], tris[2 + i],
                            Vector(.5, .5, .5));
    }
    //		fclose( fp );
  }
...
void CLightingPreviewThread::Run(void) {
  ...
  while (!should_quit) {
    while ((!should_quit) && ((!AnyUsefulWorkToDo()) ||
                              (g_HammerToLPreviewMsgQueue.MessageWaiting())))
      should_quit |= HandleAMessage();
    if ((!should_quit) && (AnyUsefulWorkToDo())) DoWork();
...
bool CLightingPreviewThread::AnyUsefulWorkToDo(void) {
  if (m_pLightList) {
    for (int i = 0; i < m_pLightList->Count(); i++) {
      CLightingPreviewLightDescription &l = (*m_pLightList)[i];
      CIncrementalLightInfo *l_info = l.m_pIncrementalInfo;
      if (l_info->HasWorkToDo()) return true;
    }
  }
  return false;
}
...
void CLightingPreviewThread::HandleGBuffersMessage(MessageToLPreview &msg_in) {
  m_Albedos.CreateFromRGBA_FloatImageData(
      msg_in.m_pDefferedRenderingBMs[0]->Width,
      msg_in.m_pDefferedRenderingBMs[0]->Height,
      msg_in.m_pDefferedRenderingBMs[0]->RGBAData);
  m_Normals.CreateFromRGBA_FloatImageData(
      msg_in.m_pDefferedRenderingBMs[1]->Width,
      msg_in.m_pDefferedRenderingBMs[1]->Height,
      msg_in.m_pDefferedRenderingBMs[1]->RGBAData);
  m_Positions.CreateFromRGBA_FloatImageData(
      msg_in.m_pDefferedRenderingBMs[2]->Width,
      msg_in.m_pDefferedRenderingBMs[2]->Height,
...
void CLightingPreviewThread::CalculateForLightTask(
    int nLineMask, int nLineMatch, CLightingPreviewLightDescription &l,
    int calc_mask, float *fContributionOut) {
      ...
      if ((work_line_number & nLineMatch) == nLineMatch) {
        for (int x = 0; x < rslt.m_nPaddedWidth; x++) {
          ...
          if (!IsAllZeros(v_or)) {
            ...
            m_pRtEnv->Trace4Rays(myray, Four_Zeros, ReplicateX4(1.0e9),
                                 &r_rslt);
  ...
  *(fContributionOut) =
      lmag.m128_f32[0] + lmag.m128_f32[1] + lmag.m128_f32[2] + lmag.m128_f32[3];
}

void CLightingPreviewThread::CalculateForLight(
  ...
  CIncrementalLightInfo *l_info = l.m_pIncrementalInfo;
  Assert(l_info);
  l_info->m_CalculatedContribution.SetSize(m_Albedos.m_nWidth,
                                           m_Albedos.m_nHeight);
...
void CLightingPreviewThread::SendVectorMatrixAsRendering(
    CSIMDVectorMatrix const &src) {
      ...
      case LPREVIEW_MSG_DISPLAY_RESULT: {
        ...
        // g_nBitmapGenerationCounter )
        {
          ...
          if (g_pLPreviewOutputBitmap &&
              (g_pLPreviewOutputBitmap->m_nWidth > 10)) {
            SignalUpdate(EVTYPE_BITMAP_RECEIVED_FROM_LPREVIEW);
            CLightingPreviewResultsWindow *w =
                GetMainWnd()->m_pLightingPreviewOutputWindow;
            if (!w) {
              w = new CLightingPreviewResultsWindow;
              GetMainWnd()->m_pLightingPreviewOutputWindow = w;
              w->Create(GetMainWnd());
            }
            if (!w->IsWindowVisible()) w->ShowWindow(SW_SHOW);
            RECT existing_rect;
            w->GetClientRect(&existing_rect);
            if ((existing_rect.right !=
                 g_pLPreviewOutputBitmap->m_nWidth - 1) ||
                (existing_rect.bottom !=
                 g_pLPreviewOutputBitmap->m_nHeight - 1)) {
              CRect myRect;
              myRect.top = 0;
              myRect.left = 0;
              ...
              w->SetWindowPos(NULL, 0, 0, myRect.Width(), myRect.Height(),
