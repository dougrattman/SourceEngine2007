...
static int g_DumbEditControls[] = {IDC_DELETEKEYVALUE, IDC_KEY,
                                   IDC_VALUE,          IDC_ADDKEYVALUE,
                                   IDC_KEY_LABEL,      IDC_VALUE_LABEL};

//-----------------------------------------------------------------------------
// Purpose: Returns true if the string specifies the name of an entity in the
// world.
//-----------------------------------------------------------------------------
static bool IsValidTargetName(const char *pTestName) {
  ...
  for (int i = 0; i < pList->Count(); i++) {
    ...
    if (pszTargetName && Q_stricmp(pszTargetName, pTestName) == 0) return true;
  }
...
class CMyEdit : public CEdit {
 public:
  void SetParentPage(COP_Entity *pPage) { m_pParent = pPage; }
...
void CPickAnglesTarget::OnNotifyPickAngles(const Vector &vecPos) {
  ...
  FOR_EACH_OBJ(*m_pDlg->m_pObjectList, pos) {
    ...
    if (pEntity != NULL) {
      ...
      char szAngles[80];
      sprintf(szAngles, "%.0f %.0f %.0f", angFace[PITCH], angFace[YAW],
              angFace[ROLL]);
...
typedef int(CALLBACK *ColumnSortFn)(LPARAM iItem1, LPARAM iItem2,
                                    LPARAM lpParam);

int InternalSortByColumn(COP_Entity *pDlg, const char *pShortName1,
                         const char *pShortName2, int iColumn) {
...
int CALLBACK SortByItemEditedState(LPARAM iItem1, LPARAM iItem2,
                                   LPARAM lpParam) {
  COP_Entity *pDlg = (COP_Entity *)lpParam;
  if (!pDlg->m_pDisplayClass) return 0;
...
static int CALLBACK SortByColumn0(LPARAM iItem1, LPARAM iItem2,
                                  LPARAM lpParam) {
  return InternalSortByColumn((COP_Entity *)lpParam, (const char *)iItem1,
                              (const char *)iItem2, 0);
}

static int CALLBACK SortByColumn1(LPARAM iItem1, LPARAM iItem2,
                                  LPARAM lpParam) {
  return InternalSortByColumn((COP_Entity *)lpParam, (const char *)iItem1,
                              (const char *)iItem2, 1);
}

ColumnSortFn g_ColumnSortFunctions[] = {SortByItemEditedState, SortByColumn0,
                                        SortByColumn1};
...
// Purpose: Destructor.
//-----------------------------------------------------------------------------
COP_Entity::~COP_Entity(void) { DestroySmartControls(); }
...
//-----------------------------------------------------------------------------
void COP_Entity::GetKeyState(const char *pShortName, EKeyState *pState,
                             bool *pMissingTarget) {
  *pMissingTarget = false;

  // If we're in multiedit mode with various types of entities selected, then
  // don't look at m_pDisplayClass.
  if (!m_pDisplayClass) {
  ...
  // Now we know it's in the FGD, so see if the value has been modified from the
  // default.
...
void COP_Entity::MergeObjectKeyValues(CEditGameClass *pEdit) {
  ...
  for (int i = pEdit->GetFirstKeyValue(); i != pEdit->GetInvalidKeyValue();
       i = pEdit->GetNextKeyValue(i)) {
      ...
      switch (pVar->GetType()) {
        case ivSideList: {
          ...
          CMapWorld::FaceID_FaceIDListsToString(
              szValue, sizeof(szValue), &FaceIDListFull, &FaceIDListPartial);
        ...
        case ivAngle: {
          //
          // We can't merge angles, so set the appropriate angles control to
...
void COP_Entity::UpdateData(int Mode, PVOID pData) {
  ...
  if (Mode == LoadFirstData) {
...
#ifdef NAMES
    if (pEdit->pClass) {
      sprintf(szBuf, "%s (%s)", pEdit->pClass->GetDescription(),
              pEdit->pClass->GetName());
    } else
#endif

      strcpy(szBuf, pEdit->GetClassName());
    m_cClasses.AddSuggestion(
        szBuf);  // If we don't make sure it has this item in its list, it will
                 // do Bad Things later on. This only happens when the FGD is
                 // missing an entity that is in the map file. In that case,
                 // just let it be. Check For Problems should ID the problem for
                 // them.
    ...
    //
    // Mark the comments field as "(different)" if it isn't the same as this
    // entity's comments.
    //
...
BOOL COP_Entity::OnApply(void) {
  m_pLastSmartControlVar =
      NULL;  // Force it to recreate the target name combo if need be,
...
//			pszValue -
//-----------------------------------------------------------------------------
void COP_Entity::ApplyKeyValueToObject(CEditGameClass *pObject,
                                       const char *pszKey,
                                       const char *pszValue) {
  ...
  if (pClass != NULL) {
    ...
    if (pVar != NULL) {
      if ((pVar->GetType() == ivSideList) || (pVar->GetType() == ivSide)) {
        ...
        // 2. In the partial selection set (inside the parentheses) AND are in
        // the
        //    original face list.
        //
        ...
        CMapWorld::FaceID_FaceIDListsToString(szSetValue, sizeof(szSetValue),
                                              &FullFaceList, NULL);
...
//			refresh of the data.
//-----------------------------------------------------------------------------
void COP_Entity::RememberState(void) { GetCurKey(m_strLastKey); }
...
bool COP_Entity::SaveData(void) {
  ...
  // If we were multiselecting entities and they haven't chosen a new classname
  // yet, we need to know that here so we don't force them all to be the last
  // selection in the classname combo.
  if (m_bClassSelectionEmpty) strClassName = "";
  ...
  FOR_EACH_OBJ(*m_pObjectList, pos) {
    ...
    if (pEdit != NULL) {
      ...
      for (int i = pEdit->GetFirstKeyValue(); i != pEdit->GetInvalidKeyValue();
        ...
        //
        // If this key is in not in our local storage, delete it from the
        // object.
        //
...
//-----------------------------------------------------------------------------
// Purpose: Given the short name of a key, find which row it's at in the list
// control.
//-----------------------------------------------------------------------------
...
void COP_Entity::RefreshKVListValues(const char *pOnlyThisVar) {
  ...
  // 1. In smartedit mode, the raw name of the property in the first column is
  // matched to m_kv.
  ...
  for (int i = 0; i < m_VarList.GetItemCount(); i++) {
    ...
    if (pOnlyThisVar && Q_stricmp(pVarName, pOnlyThisVar) != 0) continue;

    if (m_bSmartedit) {
      GDinputvariable *pVar =
          (m_pDisplayClass ? m_pDisplayClass->VarForName(pVarName) : NULL);
      if (pVar) {
          ...
          if (eType == ivChoices) {
            if (pUnformattedValue) {
              const char *pTestValue =
                  pVar->ItemStringForValue(pUnformattedValue);
              if (pTestValue) pValue = pTestValue;
            }
          } else if ((eType == ivStudioModel) || (eType == ivSprite) ||
                     (eType == ivSound) || (eType == ivDecal) ||
        ...
        pValue = m_kv.GetValue(
            pVarName);  // This was probably added in dumbedit mode.
      }
    } else {
...
void COP_Entity::PresentProperties() {
  if (!m_bAllowPresentProperties) return;
  ...
  if (!m_bSmartedit || !m_pDisplayClass) {
    ...
    // Too many entity variables! Increase GD_MAX_VARIABLES if you get this
    // assertion.
    ...
    //
    // Add all the keys from the entity's class to the listbox. If the key is
    // not already in the entity, add it to the m_kvAdded list as well.
    //
    ...
    // Also add any keyvalues they added in dumbedit mode. These will show up in
    // red.
    for (int i = m_kv.GetFirst(); i != m_kv.GetInvalidIndex();
         i = m_kv.GetNext(i)) {
      MDkeyvalue &KeyValue = m_kv.GetKeyValue(i);
...
void COP_Entity::LoadClassList(void) {
  ...
  // Ok, we've already initialized the list with the same list. Don't do it over
  // again.
  if (m_iLastClassListSolidClasses == iSolidClasses) return;
...
BOOL COP_Entity::OnInitDialog(void) {
  ...
  dwStyle |=
      LVS_REPORT | LVS_SINGLESEL | LVS_SHOWSELALWAYS | LVS_OWNERDRAWFIXED;
  SetWindowLong(m_VarList.GetSafeHwnd(), GWL_STYLE, dwStyle);

  m_VarList.SetExtendedStyle(m_VarList.GetExtendedStyle() |
...
void COP_Entity::UpdateAnchors() {
  CAnchorDef anchorDefs[] = {
      ...
      CAnchorDef(IDC_KEYVALUE_HELP, k_eAnchorRight, k_eAnchorTop,
                 k_eAnchorRight, k_eAnchorBottom),
      CAnchorDef(IDC_COMMENTS_LABEL, k_eSimpleAnchorBottomRight),
      CAnchorDef(IDC_ENTITY_COMMENTS, k_eAnchorRight, k_eAnchorBottom,
                 k_eAnchorRight, k_eAnchorBottom),
...
void COP_Entity::OnDblClickKeyValues(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (!m_bSmartedit) return;

  int iSel = GetCurVarListSelection();
  if (iSel == LB_ERR || m_VarMap[iSel] < 0) return;
  ...
  if (pVar->GetType() == ivColor255 || pVar->GetType() == ivColor1) {
    ...
    } else if (dynamic_cast<CMyComboBox *>(m_pSmartControl) ||
...
void COP_Entity::CreateSmartControls(GDinputvariable *pVar,
  ...
  // If this is the same var that our smart controls are already setup for, then
  // don't do anything.
  ...
  // which makes us do things like assume the user changed a key when they
  // didn't.
  //
  ...
  //
  m_cPickColor.ShowWindow((eType == ivColor255 || eType == ivColor1) ? SW_SHOW
                                                                     : SW_HIDE);
  ...
  //
  if ((eType == ivChoices) || (eType == ivNPCClass) ||
      (eType == ivFilterClass) || (eType == ivPointEntityClass)) {
    ...
    if ((eType == ivTargetSrc) || (eType == ivTargetDest)) {
      ...
      CreateSmartControls_BasicEditControl(pVar, ctrlrect, hControlFont,
                                           pHelperType);
    }
    ...
    //
    if ((eType == ivStudioModel) || (eType == ivSprite) || (eType == ivSound) ||
        (eType == ivDecal) || (eType == ivMaterial) || (eType == ivScene)) {
      CreateSmartControls_BrowseAndPlayButtons(pVar, ctrlrect, hControlFont);
    } else if ((eType == ivTargetDest) || (eType == ivTargetNameOrClass) ||
               (eType == ivTargetSrc) || (eType == ivNodeDest)) {
      CreateSmartControls_MarkAndEyedropperButtons(pVar, ctrlrect,
                                                   hControlFont);
  ...
  m_pSmartControl->SetWindowPos(&m_VarList, 0, 0, 0, 0,
...
void COP_Entity::CreateSmartControls_Angle(GDinputvariable *pVar,
  ...
  if (stricmp(pVar->GetName(), "angles")) {
    *bShowSmartAngles = true;
    ...
    m_SmartAngle.SetWindowPos(NULL, ctrlrect.left + rectAngleEdit.Width() + 4,
                              ctrlrect.bottom + 10, 0, 0,
                              SWP_NOSIZE | SWP_NOZORDER);
    m_SmartAngleEdit.SetWindowPos(
        NULL, ctrlrect.left,
        (ctrlrect.bottom + rectAngleBox.Height() + 10) - rectAngleEdit.Height(),
  ...
  pButton->CreateEx(
      ...
      ButtonRect.left, ButtonRect.top, 58, ButtonRect.Height() + 2,
      GetSafeHwnd(), (HMENU)IDC_PICK_ANGLES);
...
void COP_Entity::CreateSmartControls_Choices(GDinputvariable *pVar,
                                             CRect &ctrlrect,
                                             HFONT hControlFont) {
  CMyComboBox *pCombo = new CMyComboBox;
  pCombo->SetParentPage(this);
  ctrlrect.bottom += 150;
  pCombo->Create(CBS_DROPDOWN | CBS_HASSTRINGS | WS_TABSTOP | WS_CHILD |
                     WS_BORDER | WS_VSCROLL | CBS_AUTOHSCROLL |
  ...
  //
  // If this is a choices field, give combo box text choices from GameData
  // variable
  //
  ...
  //
  // For filterclass display a list of all the names of filters that are in the
  // map
  //
  ...
  //
  // For pointentity fields, fill with all the point entity classes from the
  // FGD.
  //
  else {
    if (pGD != NULL) {
  ...
  //
  // If the current value is the "different" string, display that in the combo
  // box.
  //
  LPCTSTR pszValue = m_kv.GetValue(pVar->GetName());
  if (pszValue != NULL) {
    if (strcmp(pszValue, VALUE_DIFFERENT_STRING) == 0) {
      ...
      //
      // If this is a choices field and the current value corresponds to one of
      // our choices, display the friendly name of the choice in the edit
      // control.
      //
      if (pVar->GetType() == ivChoices) {
...
void COP_Entity::CreateSmartControls_TargetName(GDinputvariable *pVar,
                                                CRect &ctrlrect,
                                                HFONT hControlFont) {
  //
  ...
  CTargetNameComboBox *pCombo = CTargetNameComboBox::Create(
      &m_SmartControlTargetNameRouter,
      CBS_DROPDOWN | WS_TABSTOP | WS_CHILD | WS_BORDER | CBS_AUTOHSCROLL |
          WS_VSCROLL | CBS_SORT,
...
void COP_Entity::CreateSmartControls_BasicEditControl(
    GDinputvariable *pVar, CRect &ctrlrect, HFONT hControlFont,
    CUtlVector<const char *> *pHelperType) {
  //
  ...
  pEdit->CreateEx(WS_EX_CLIENTEDGE, "EDIT", "",
                  ...
                  ctrlrect.left, ctrlrect.top, ctrlrect.Width(),
                  ctrlrect.Height(), GetSafeHwnd(), HMENU(IDC_SMARTCONTROL));
  pEdit->SendMessage(WM_SETFONT, (WPARAM)hControlFont);

  const char *pValue = m_kv.GetValue(pVar->GetName());
  if (pValue) pEdit->SetWindowText(pValue);
  ...
  for (int i = 0; i < pHelperType->Count(); i++) {
    if (!Q_strcmp(pHelperType->Element(i), "sphere")) {
      ...
      pButton->CreateEx(
          0, "Button", "",
          WS_TABSTOP | WS_CHILD | WS_VISIBLE | BS_ICON | BS_PUSHLIKE | disabled,
          ButtonRect.left, ButtonRect.top, ButtonRect.Width(),
          ButtonRect.Height(), GetSafeHwnd(), (HMENU)IDC_CAMERA_DISTANCE);
...
void COP_Entity::CreateSmartControls_BrowseAndPlayButtons(GDinputvariable *pVar,
                                                          CRect &ctrlrect,
                                                          HFONT hControlFont) {
  CRect ButtonRect = ctrlrect;
  ...
  CButton *pButton = new CButton;
  pButton->CreateEx(0, "Button", "Browse...",
                    WS_TABSTOP | WS_CHILD | WS_VISIBLE, ButtonRect.left,
                    ButtonRect.top, ButtonRect.Width(), ButtonRect.Height(),
  ...
  if (pVar->GetType() == ivSound || pVar->GetType() == ivScene) {
    ...
    pButton->CreateEx(0, "Button", "Play", WS_TABSTOP | WS_CHILD | WS_VISIBLE,
                      ButtonRect.left, ButtonRect.top, ButtonRect.Width(),
                      ButtonRect.Height(), GetSafeHwnd(),
                      (HMENU)IDC_PLAY_SOUND);
    pButton->SendMessage(WM_SETFONT, (WPARAM)hControlFont);
...
void COP_Entity::CreateSmartControls_MarkAndEyedropperButtons(
    GDinputvariable *pVar, CRect &ctrlrect, HFONT hControlFont) {
  CRect ButtonRect = ctrlrect;
  ButtonRect.top = ctrlrect.bottom + 4;
  ButtonRect.bottom = ctrlrect.bottom + ctrlrect.Height() + 4;
  ...
  if ((eType == ivTargetDest) || (eType == ivTargetNameOrClass) ||
      (eType == ivTargetSrc)) {
    //
    ...
    pButton->CreateEx(0, "Button", "Mark", WS_TABSTOP | WS_CHILD | WS_VISIBLE,
                      ButtonRect.left, ButtonRect.top, ButtonRect.Width(),
                      ButtonRect.Height(), GetSafeHwnd(), (HMENU)IDC_MARK);
    pButton->SendMessage(WM_SETFONT, (WPARAM)hControlFont);
    ...
    pButton = new CButton;
    pButton->CreateEx(0, "Button", "Mark+Add",
                      WS_TABSTOP | WS_CHILD | WS_VISIBLE, ButtonRect.left,
                      ButtonRect.top, ButtonRect.Width(), ButtonRect.Height(),
  ...
  pButton = new CButton;
  pButton->CreateEx(0, "Button", "",
                    WS_TABSTOP | WS_CHILD | WS_VISIBLE | BS_ICON |
                        BS_AUTOCHECKBOX | BS_PUSHLIKE,
                    ButtonRect.left, ButtonRect.top, ButtonRect.Width(),
                    ButtonRect.Height(), GetSafeHwnd(), (HMENU)IDC_PICK_ENTITY);
...
void COP_Entity::CreateSmartControls_PickButton(GDinputvariable *pVar,
                                                CRect &ctrlrect,
                                                HFONT hControlFont) {
  //
...
void COP_Entity::OnChangeKeyorValue(void) {
  ...
  // If they changed spawnflags, notify the flags page so its changes don't
  ...
  if (m_bEnableControlUpdate) {
    // Update any controls that are displaying the same data as the edit
    // control.
...
void COP_Entity::OnAddkeyvalue(void) {
  ...
  newkv.m_Value = "value";
  if (newkv.DoModal() == IDCANCEL) return;
  ...
  if (m_kv.GetValue(newkv.m_Key)) {
    ...
    for (int i = 1;; i++) {
      ...
      if (!m_kv.GetValue(strTemp)) break;
    }
    newkv.m_Key = strTemp;
  }
...
//-----------------------------------------------------------------------------
static unsigned long GetMatchingFlagsMask(GDinputvariable *pVar1,
                                          GDinputvariable *pVar2) {
  ...
  for (int i = 0; i < pVar1->GetFlagCount(); i++) {
    for (int j = 0; j < pVar2->GetFlagCount(); j++) {
      if (pVar1->GetFlagMask(i) == pVar2->GetFlagMask(j)) {
        if (V_stricmp(pVar1->GetFlagCaption(i), pVar2->GetFlagCaption(j)) ==
            0) {
...
//-----------------------------------------------------------------------------
// Assign default values to keys that are in the FGD but missing from the
// entity.
//-----------------------------------------------------------------------------
  ...
  for (int i = 0; i < pClass->GetVariableCount(); i++) {
    ...
    if (!strcmpi(pVar->GetName(), SPAWNFLAGS_KEYNAME)) {
      ...
      if (pOldClass && (pOldClass != pClass)) {
        ...
        // But.. if the old class and the new class have any flags with the same
        // name and value, then keep the current value for that flag.
        ...
        if (p && pOldVar) {
          ...
          nCurrent |=
              (nOriginalFlagsValue & mask);  // Add back in the current values.
        }
...
void COP_Entity::UpdateEditClass(const char *pszClass, bool bForce) {
  ...
  if (!bForce && (m_pEditClass == pOldEditClass)) return;
...
void COP_Entity::SetSmartedit(bool bSet) {
  ...
  GetDlgItem(IDC_KEYVALUE_HELP_GROUP)
      ->ShowWindow(m_bSmartedit ? SW_SHOW : SW_HIDE);
...
void COP_Entity::UpdateDisplayClass(GDclass *pClass) {
  // The outer check here is lame, but somewhere along the line, all the
  // controls get enabled behind our backs. So verify that our state information
  // is sane. If it's not, then we'll redo some state in here.
  ...
  // In case we're not allowed to present the properties yet, make sure nobody
  // uses m_VarMap.
  ...
  if (!m_pDisplayClass) {
    ...
    if (m_bSmartedit || bForceRefresh) {
      m_bSmartedit = true;  // In case bForceRefresh was on, force it to refresh
                            // the controls.
      SetSmartedit(false);
    ...
    if (bForceRefresh) m_bSmartedit = !m_bWantSmartedit;

    SetSmartedit(m_bWantSmartedit);
  }

  // No need to call PresentProperties an extra time if it was called because of
  // SetSmartedit..
  if (bNeedsSetupForMode &&
      (m_nPresentPropertiesCalls == lastNumPresentPropertiesCalls)) {
    PresentProperties();
  }
...
void COP_Entity::InternalOnChangeSmartcontrol(const char *szValue) {
  ...
  if (m_bEnableControlUpdate) {
    // Update any controls that are displaying the same data as the edit
    // control. This code should only be hit as a result of user input in the
    // edit control!
  ...
  // We have to do this because it's an owner draw control and we're redoing the
  // background colors. Normally, Windows will only invalidate the second
  // column.
  RECT rc;
...
void COP_Entity::OnChangeSmartcontrolSel(void) {
  ...
  int iSel = GetCurVarListSelection();
  if (m_VarMap[iSel] < 0 || !m_pDisplayClass) return;

  GDinputvariable *pVar = m_pDisplayClass->GetVariableAt(m_VarMap[iSel]);
  if ((pVar->GetType() != ivTargetSrc) && (pVar->GetType() != ivTargetDest) &&
      (pVar->GetType() != ivTargetNameOrClass) &&
      (pVar->GetType() != ivChoices) && (pVar->GetType() != ivNPCClass) &&
...
void COP_Entity::OnBrowse(void) {
  ...
  if (!g_FSDialogFactory) return;

  IFileSystemOpenDialog *pDlg;
  pDlg = (IFileSystemOpenDialog *)g_FSDialogFactory(
      FILESYSTEMOPENDIALOG_VERSION, NULL);
  if (!pDlg) {
    char str[512];
    Q_snprintf(str, sizeof(str), "Can't create %s interface.",
  ...
  //
  // Based on the type of file that we are picking, set up the default
  // extension, default directory, and filters. Each type of file remembers its
  // last directory.
  //
  switch (m_eEditType) {
    case ivStudioModel: {
    ...
    case ivSound: {
      ...
      if (soundDlg.m_SoundType != SOUND_TYPE_RAW &&
    ...
    case ivScene: {
      ...
      int nRet = soundDlg.DoModal();
      if (nRet == IDOK) {
        m_pSmartControl->SetWindowText(CString("scenes\\") +
                                       soundDlg.GetSelectedSound());
      }
      goto Cleanup;
    }
  ...
  // into the SmartEdit control. If there is no backslash, put the whole
  // filename.
  //
  int ret;
...
void COP_Entity::OnKillfocusKey(void) {
  ...
  // No change
  if (strOutput == m_szOldKeyName) return;

  char szSaveValue[KEYVALUE_MAX_VALUE_LENGTH];
  memset(szSaveValue, 0, sizeof(szSaveValue));
  strncpy(szSaveValue, m_kv.GetValue(m_szOldKeyName, NULL),
          sizeof(szSaveValue) - 1);
...
//-----------------------------------------------------------------------------
void COP_Entity::PerformMark(const char *szTargetName, bool bClear,
                             bool bNameOrClass) {
  CMapDoc *pDoc = CMapDoc::GetActiveMapDoc();
  ...
  if (pDoc != NULL) {
    if (szTargetName[0] != '\0') {
      ...
      if (Found.Count() != 0) {
        ...
        MessageBox("No entities were found with that targetname.",
                   "No entities found", MB_ICONINFORMATION | MB_OK);
      }
    }
  }
}
...
void COP_Entity::OnMarkAndAdd(void) {
  ...
  CMapEntity **pTemp =
      (CMapEntity **)stackalloc(m_pObjectList->Count() * sizeof(CMapEntity *));
  CUtlVector<CMapEntity *> temp(pTemp, m_pObjectList->Count());

  FOR_EACH_OBJ(*m_pObjectList, pos) {
    CMapEntity *pEntity =
        static_cast<CMapEntity *>(m_pObjectList->Element(pos));
    temp.AddToTail(pEntity);
  }

  // Mark all the entities referred to by the current keyvalue in the selected
  // entities.
  bool bNameOrClass = false;
  GDinputvariable *pVar = m_pDisplayClass->GetVariableAt(m_VarMap[iSel]);
  if (pVar && (pVar->GetType() == ivTargetNameOrClass)) {
...
void COP_Entity::OnPaste(void) {
  if (bKvClipEmpty) return;

  CString str;
  GetCurKey(str);

  // copy entity keyvalues
  for (int i = kvClipboard.GetFirst(); i != kvClipboard.GetInvalidIndex();
       i = kvClipboard.GetNext(i)) {
...
//-----------------------------------------------------------------------------
void COP_Entity::GetFaceIDListsForKey(CMapFaceIDList &FullFaces,
  ...
  if (pButton != NULL) {
    if (pButton->GetCheck()) {
      ...
      if (nSel != LB_ERR && m_VarMap[nSel] >= 0) {
        ...
        // FACEID TODO: make the faces highlight even when the tool is not
        // active
        ...
        // Save the old tool so we can reset to the correct tool when we stop
        // picking.
        ...
        CToolPickFace *pTool =
            (CToolPickFace *)ToolManager()->GetToolForID(TOOL_PICK_FACE);
  ...
  if (pButton != NULL) {
    if (pButton->GetCheck()) {
      ...
      if (nSel != LB_ERR && m_VarMap[nSel] >= 0) {
        ...
        //
        CToolPickAngles *pTool =
            (CToolPickAngles *)ToolManager()->GetToolForID(TOOL_PICK_ANGLES);
    ...
    if (pButton->GetCheck()) {
      ...
      if (nSel != LB_ERR && m_VarMap[nSel] >= 0) {
        ...
        CToolPickEntity *pTool =
            (CToolPickEntity *)ToolManager()->GetToolForID(TOOL_PICK_ENTITY);
...
void COP_Entity::OnPickColor(void) {
  ...
  while (pTmp) {
    if (pTmp[0]) {
      if (iCurToken == 3) {
        ...
        if (iCurToken == 0) r = BYTE(atol(pTmp));
        if (iCurToken == 1) g = BYTE(atol(pTmp));
        if (iCurToken == 2) b = BYTE(atol(pTmp));
      } else {
        if (iCurToken == 0) r = BYTE(atof(pTmp) * 255.f);
        if (iCurToken == 1) g = BYTE(atof(pTmp) * 255.f);
        if (iCurToken == 2) b = BYTE(atof(pTmp) * 255.f);
      }
      ++iCurToken;
    }
    pTmp = strtok(NULL, " ");
  ...
  if (pVar->GetType() == ivColor255) {
    ...
    sprintf(szTmp, "%.3f %.3f %.3f", float(r) / 255.f, float(g) / 255.f,
            float(b) / 255.f);
  }
...
void COP_Entity::OnSetfocusKey(void) {
  ...
  if (m_szOldKeyName.IsEmpty()) return;
...
void COP_Entity::UpdatePickFaceText(CToolPickFace *pTool) {
  ...
  if (!CMapWorld::FaceID_FaceListsToString(szList, sizeof(szList),
                                           &FaceListFull, &FaceListPartial)) {
    MessageBox(
        "Too many faces selected for this keyvalue to hold. Deselect some "
        "faces.",
        "Error", MB_OK);
  }
...
void COP_Entity::OnCameraDistance(void) {
  ...
  if (nSel != LB_ERR && m_VarMap[nSel] >= 0) {
    ...
    if (iSelectionCount == 1) {
      // Only 1 entity selected.. we can just set our SmartControl text and the
      // change will get applied when they close the properties dialog or click
      // Apply.
      ...
      // Multiple entities selected. We have to apply the current set of
      // changes, Set the value in each entity and set the kv text to
      // VALUE_DIFFERENT_STRING so it doesn't overwrite anything when we
      // Apply().
      GDinputvariable *pVar = m_pDisplayClass->GetVariableAt(m_VarMap[nSel]);
      if (!pVar) return;

      int index = m_kv.FindByKeyName(pVar->GetName());
      if (index == m_kv.GetInvalidIndex()) return;
      ...
      FOR_EACH_OBJ(*m_pObjectList, pos) {
        CMapClass *pObject = m_pObjectList->Element(pos);
        if (pObject && !IsWorldObject(pObject) &&
            dynamic_cast<CEditGameClass *>(pObject))
      ...
      if (objectList.Count() > 0) {
        // Setup undo stuff.
        GetHistory()->MarkUndoPosition(pDoc->GetSelection()->GetList(),
                                       "Change Properties");
...
bool COP_Entity::OnUnknownEntry(const char *pText) {
  ...
  // It's about to call OnTextChanged, and we'll null m_pDisplayClass, disable
  // SmartEdit, etc.
  m_kv.RemoveAll();
  return true;
}
...
void COP_Entity::OnSmartControlTargetNameChanged(const char *pText) {
  CFilteredComboBox *pCombo =
      dynamic_cast<CFilteredComboBox *>(m_pSmartControl);
  Assert(pCombo);
  if (pCombo) {
...
void COP_Entity::GetItemColor(int iItem, COLORREF *pBackgroundColor,
                              COLORREF *pTextColor) {
  // Setup the background color.
  EKeyState eState;
  bool bMissingTarget;
  GetKeyState((const char *)m_VarList.GetItemData(iItem), &eState,
              &bMissingTarget);
...
// Input  : preserveMask tells the bits you should NOT change in the spawnflags
// value.
//          newValues is the values of the other bits.
//-----------------------------------------------------------------------------
void COP_Entity::OnUpdateSpawnFlags(unsigned long preserveMask,
  ...
  // We print the string ourselves here because the int version of SetValue will
  // show a negative number if we exceed 1<<31.
  char str[512];
  V_snprintf(str, sizeof(str), "%lu", newValue);
  m_kv.SetValue(SPAWNFLAGS_KEYNAME, str);

  RefreshKVListValues(SPAWNFLAGS_KEYNAME);
  OnSelchangeKeyvalues();  // Refresh the control with its value in case it's
                           // selected currently.
}

void COP_Entity::OnSize(UINT nType, int cx, int cy) { m_AnchorMgr.OnSize(); }
