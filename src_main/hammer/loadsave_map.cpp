...
#pragma warning(disable : 4748)  // buffer overrung with optimizations off
                                 // - remove if we turn "g" back on
...
// Output : int
//-----------------------------------------------------------------------------
static int SaveSolidChildrenOf(CMapClass *pObject, std::fstream &file,
                               BoundBox *pIntersecting = NULL) {
  CMapWorld *pWorld = (CMapWorld *)CMapClass::GetWorldObject(pObject);

  //
  // If we are only saving visible objects and this object isn't visible, don't
  // save it.
  //
  ...
  //
  // If we are only saving objects within a particular bounding box and this
  // object isn't, don't save it.
  //
  if (pIntersecting &&
      !pObject->IsIntersectingBox(pIntersecting->bmins, pIntersecting->bmaxs)) {
    return fileOk;
  }
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    ...
    if (!pIntersecting ||
        pChild->IsIntersectingBox(pIntersecting->bmins, pIntersecting->bmaxs)) {
...
// Output : int
//-----------------------------------------------------------------------------
static int SaveEntityChildrenOf(CMapClass *pObject, std::fstream &file,
                                BoundBox *pIntersecting) {
  CMapWorld *pWorld = (CMapWorld *)CMapClass::GetWorldObject(pObject);
...
int CMapFace::SerializeMAP(std::fstream &file, BOOL fIsStoring) {
  ...
  if (fIsStoring) {
    ...
    sprintf(szBuf,
            ...
            (double)texture.UAxis[0], (double)texture.UAxis[1],
            (double)texture.UAxis[2], (double)texture.UAxis[3],
            (double)texture.VAxis[0], (double)texture.VAxis[1],
            (double)texture.VAxis[2], (double)texture.VAxis[3],

            (double)texture.rotate, (double)texture.scale[0],
    ...
    if (uMapVersion >= 340) {
      nRead = sscanf_s(
          ...
          &szTexName, SOURCE_ARRAYSIZE(szTexName),

          &texture.UAxis[0], &texture.UAxis[1], &texture.UAxis[2],
          &texture.UAxis[3], &texture.VAxis[0], &texture.VAxis[1],
          &texture.VAxis[2], &texture.VAxis[3],
      ...
      nRead = sscanf_s(
          ...
          &texture.UAxis[3], &texture.VAxis[3], &texture.rotate,
...
int CMapSolid::SerializeMAP(std::fstream &file, BOOL fIsStoring) {
  ...
  if (fIsStoring) {
    ...
    if (!file.fail()) {
      ...
      //
      // If we are reading from an old map file, the texture axes will need to
      // be set up. Leave the rotation and shifts alone; they were read from the
      // MAP file.
      //
      if (uMapVersion < 220) {
...
int CEditGameClass::SerializeMAP(std::fstream &file, BOOL fIsStoring) {
  ...
  if (!fIsStoring) {
    ...
    //
    // Determine whether we have a game data class. This will help us decide
    // which keys to write.
    //
    ...
    //
    for (int z = m_KeyValues.GetFirst(); z != m_KeyValues.GetInvalidIndex();
         z = m_KeyValues.GetNext(z)) {
    ...
    //
    // If we have a base class, for each keyvalue in the class definition, write
    // out all keys that are not present in the object and whose defaults are
    // nonzero in the class definition.
    //
    if (pGameDataClass != NULL) {
      ...
      for (int i = 0; i < nVariableCount; i++) {
        ...
        if (pVar != NULL) {
          ...
          //
          // If the variable is not present in this object, write out the
          // default value.
          //
          if (p == NULL) {
            pKey = &tmpkv;
            ...
            //
            if ((pKey->szKey[0] != 0) && (pKey->szValue[0] != 0) &&
                (_stricmp(pKey->szValue, "0"))) {
...
// Output : int
//-----------------------------------------------------------------------------
int CMapWorld::SerializeMAP(std::fstream &file, BOOL fIsStoring,
                            BoundBox *pIntersecting) {
  int iRvl;
  ...
  if (fIsStoring) {
    ...
    if (bErrors) {
      if (nInvalidSolids) {
        CString str;
        str.Format(
            "For your information, %d solids were not loaded\n"
            "due to errors in the file.",
            nInvalidSolids);
        AfxMessageBox(str);
      } else if (AfxMessageBox(
                     "There was a problem loading the MAP file. Do you\n"
    ...
    if (g_pGameConfig->GetTextureFormat() == tfVMT) {
      ...
      if (CMapDoc::GetActiveMapDoc()) {
        FileHandle_t searchReplaceFP =
            g_pFileSystem->Open(translationFilename, "r");
        if (searchReplaceFP) {
