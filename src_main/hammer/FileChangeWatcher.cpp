...
CFileChangeWatcher::CFileChangeWatcher() { m_pCallbacks = NULL; }

CFileChangeWatcher::~CFileChangeWatcher() { Term(); }
...
bool CFileChangeWatcher::AddDirectory(const char *pSearchPathBase,
                                      const char *pDirName, bool bRecursive) {
  char fullDirName[SOURCE_MAX_PATH];
  V_ComposeFileName(pSearchPathBase, pDirName, fullDirName,
                    sizeof(fullDirName));

  HANDLE hDir = CreateFile(
      fullDirName, GENERIC_READ,
      FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL,
      OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED, NULL);
  if (hDir == INVALID_HANDLE_VALUE) {
    Warning(
        "CFileChangeWatcher::AddDirectory - can't get a handle to directory "
        "%s.\n",
        pDirName);
    return false;
  }

  // Call this once to start the ball rolling.. Next time we call it, it'll tell
  // us the changes that have happened since this call.
  CDirWatch *pDirWatch = new CDirWatch;
  V_strncpy(pDirWatch->m_SearchPathBase, pSearchPathBase,
            sizeof(pDirWatch->m_SearchPathBase));
  V_strncpy(pDirWatch->m_DirName, pDirName, sizeof(pDirWatch->m_DirName));
  V_strncpy(pDirWatch->m_FullDirName, fullDirName,
            sizeof(pDirWatch->m_FullDirName));
...
int CFileChangeWatcher::Update() {
  ...
  while (i < m_DirWatches.Count()) {
    ...
    DWORD dwBytes = 0;
    if (GetOverlappedResult(pDirWatch->m_hDir, &pDirWatch->m_Overlapped,
                            &dwBytes, FALSE)) {
      ...
      while (nBytesLeft >= sizeof(FILE_NOTIFY_INFORMATION)) {
        ...
        if (m_pCallbacks) {
          ...
          int nBytesToCopy = std::min(pNotify->FileNameLength, 2047UL);
          ...
          // Now add it to the queue.	We use this queue because sometimes
          // Windows will give us multiple of the same modified notification
          // back to back, and we want to reduce redundant calls.
      ...
      continue;  // Check again because sometimes it queues up duplicate
                 // notifications.
    }

    // Process all the entries in the queue.
    for (int iQueuedChange = queuedChanges.First();
...
void CFileChangeWatcher::SendNotification(
    CFileChangeWatcher::CDirWatch *pDirWatch, const char *pRelativeFilename) {
...
BOOL CFileChangeWatcher::CallReadDirectoryChanges(
    CFileChangeWatcher::CDirWatch *pDirWatch) {
  return ReadDirectoryChangesW(
      pDirWatch->m_hDir, pDirWatch->m_Buffer, sizeof(pDirWatch->m_Buffer), true,
      FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_SIZE |
          FILE_NOTIFY_CHANGE_LAST_WRITE,
      NULL, &pDirWatch->m_Overlapped, NULL);
}
