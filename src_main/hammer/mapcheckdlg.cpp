...
inline bool IsCheckVisible(CMapClass *pClass) {
  return (Options.general.bCheckVisibleMapErrors == FALSE) ||
         pClass->IsVisible();
}
...
//-----------------------------------------------------------------------------
// Purpose: Builds the listbox string for the given error and adds it to the
// list.
//-----------------------------------------------------------------------------
...
static BOOL FindPlayer(CMapEntity *pObject, DWORD) {
  if (!IsCheckVisible(pObject)) return TRUE;
...
static BOOL _CheckMixedFaces(CMapSolid *pSolid, CListBox *pList) {
  if (!IsCheckVisible(pSolid)) return TRUE;
...
static void CheckMixedFaces(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckMixedFaces, (DWORD)pList,
...
bool FindDuplicateNodeID(CMapEntity *pNode, CMapWorld *pWorld) {
  if (!IsCheckVisible(pNode)) return false;

  EnumChildrenPos_t pos;
  CMapClass *pChild = pWorld->GetFirstDescendent(pos);
  while (pChild != NULL) {
    CMapEntity *pEntity = dynamic_cast<CMapEntity *>(pChild);
    if (pEntity && IsCheckVisible(pEntity) && (pEntity != pNode) &&
        pEntity->IsNodeClass()) {
...
static void CheckDuplicatePlanes(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckDuplicatePlanes, (DWORD)pList,
                       MAPCLASS_TYPE(CMapSolid));
}
...
//-----------------------------------------------------------------------------
static BOOL _CheckDuplicateFaceIDs(CMapSolid *pSolid,
                                   FindDuplicateFaceIDs_t *pData) {
  if (!IsCheckVisible(pSolid)) return TRUE;
...
static void CheckDuplicateFaceIDs(CListBox *pList, CMapWorld *pWorld) {
  ...
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckDuplicateFaceIDs,
                       (DWORD)&Lists, MAPCLASS_TYPE(CMapSolid));

  for (int i = 0; i < Lists.Duplicates.Count(); i++) {
    CMapFace *pFace = Lists.Duplicates.Element(i);
    AddError(pList, ErrorDuplicateFaceIDs, (DWORD)pFace,
             (CMapSolid *)pFace->GetParent());
  }
}
...
//-----------------------------------------------------------------------------
static void CheckValidTarget(CMapEntity *pEntity, const char *pFieldName,
                             const char *pTargetName, CListBox *pList,
                             bool bCheckClassNames) {
  if (!pTargetName) return;

  // These procedural names are always assumed to exist.
  if (!stricmp(pTargetName, "!activator") || !stricmp(pTargetName, "!caller") ||
      !stricmp(pTargetName, "!player") || !stricmp(pTargetName, "!self"))
    return;
  ...
  bool bFound = pDoc->FindEntitiesByName(
      Found, pTargetName, (Options.general.bCheckVisibleMapErrors == TRUE));
  if (!bFound && bCheckClassNames) {
    // Not found, search by classname.
    bFound = pDoc->FindEntitiesByClassName(
        Found, pTargetName, (Options.general.bCheckVisibleMapErrors == TRUE));
  }
...
static BOOL _CheckMissingTargets(CMapEntity *pEntity, CListBox *pList) {
  ...
  if (!pClass) {
    ...
    for (int i = 0; i < pClass->GetVariableCount(); i++) {
      ...
      CheckValidTarget(pEntity, pVar->GetName(), pszValue, pList,
                       (pVar->GetType() == ivTargetNameOrClass));
    }
  }
...
static void CheckMissingTargets(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckMissingTargets, (DWORD)pList,
                       MAPCLASS_TYPE(CMapEntity));
}
...
static void CheckSolidIntegrity(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckSolidIntegrity, (DWORD)pList,
                       MAPCLASS_TYPE(CMapSolid));
}
...
static void CheckSolidContents(CListBox *pList, CMapWorld *pWorld) {
  if (CMapDoc::GetActiveMapDoc() && CMapDoc::GetActiveMapDoc()->GetGame() &&
      CMapDoc::GetActiveMapDoc()->GetGame()->mapformat == mfQuake2) {
    pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckSolidContents, (DWORD)pList,
                         MAPCLASS_TYPE(CMapSolid));
  }
}
...
static BOOL _CheckInvalidTextures(CMapSolid *pSolid, CListBox *pList) {
  ...
  for (int i = 0; i < nFaces; i++) {
    ...
    if (pTex->IsDummy()) {
      AddError(pList, ErrorInvalidTexture, (DWORD)pFace->texture.texture,
               pSolid);
      return TRUE;
    }
...
static void CheckInvalidTextures(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckInvalidTextures, (DWORD)pList,
                       MAPCLASS_TYPE(CMapSolid));
}
...
static void CheckUnusedKeyvalues(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckUnusedKeyvalues, (DWORD)pList,
                       MAPCLASS_TYPE(CMapEntity));
}
...
static void CheckEmptyEntities(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckEmptyEntities, (DWORD)pList,
                       MAPCLASS_TYPE(CMapEntity));
}
...
static BOOL _CheckBadConnections(CMapEntity *pEntity, CListBox *pList) {
  ...
  if (CEntityConnection::ValidateOutputConnections(
          pEntity, (Options.general.bCheckVisibleMapErrors == TRUE)) ==
      CONNECTION_BAD) {
    AddError(pList, ErrorBadConnections, (DWORD)pEntity->GetClassName(),
             pEntity);
  }
...
static void CheckBadConnections(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckBadConnections, (DWORD)pList,
                       MAPCLASS_TYPE(CMapEntity));
}
...
static bool HasVisGroupHiddenChildren(CMapClass *pObject) {
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    if (!pChildren->Element(pos)->IsVisGroupShown()) return true;
  }
...
static BOOL _CheckVisGroups(CMapClass *pObject, CListBox *pList) {
  ...
  if (!pObject->IsVisGroupShown()) {
    ...
    // Check for unanticipated objects that are hidden but forbidden from
    // visgroup membership.
...
static void CheckVisGroups(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildrenRecurseGroupsOnly((ENUMMAPCHILDRENPROC)_CheckVisGroups,
                                        (DWORD)pList);
}
...
static BOOL _CheckOverlayFaceList(CMapEntity *pEntity, CListBox *pList) {
  ...
  FOR_EACH_OBJ(*pChildren, pos) {
    CMapOverlay *pOverlay =
        dynamic_cast<CMapOverlay *>(pChildren->Element(pos));
    if (pOverlay) {
...
static void CheckOverlayFaceList(CListBox *pList, CMapWorld *pWorld) {
  pWorld->EnumChildren((ENUMMAPCHILDRENPROC)_CheckOverlayFaceList, (DWORD)pList,
                       MAPCLASS_TYPE(CMapEntity));
}
...
static void FixInvalidTextureAxes(MapError *pError) {
  ...
  for (int i = 0; i < nFaces; i++) {
    CMapFace *pFace = pSolid->GetFace(i);
    if (!pFace->IsTextureAxisValid()) {
      pFace->InitializeTextureAxes(Options.GetTextureAlignment(),
                                   INIT_TEXTURE_FORCE | INIT_TEXTURE_AXES);
    }
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Removes any bad connections from the entity associated with the
// error. Input  : pError -
//-----------------------------------------------------------------------------
static void FixBadConnections(MapError *pError) {
  CMapEntity *pEntity = (CMapEntity *)pError->pObjects[0];
...
void FixHiddenObject(MapError *pError) {
  ...
  // Create a new visgroup to out the objects in (for hiding or
  // inspection/deletion).
  CMapObjectList Objects;
  ...
  if ((pError->Type == ErrorHiddenGroupHiddenChildren) ||
    ...
    pDoc->VisGroups_CreateNamedVisGroup(
        Objects, "_hidden by Check for Problems", true, false);
    ...
    // The objects either ARE in the compile, or they can't be hidden in a
    // visgroup. Don't hide them, just stick them in a visgroup for inspection
    pDoc->VisGroups_CreateNamedVisGroup(Objects, "found by Check for Problems",
                                        false, false);
  }
}
