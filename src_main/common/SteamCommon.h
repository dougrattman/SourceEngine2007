...
typedef void(STEAM_CALL *KeyValueIteratorCallback_t)(const char *Key,
                                                     const char *Val,
                                                     void *pvParam);
...
/* DEPRECATED -- these are ignored now, all API access is granted on
 * SteamStartup */
...
typedef struct {
  int bIsDir; /* If non-zero, element is a directory; if zero, element is a file
               */
  unsigned int uSizeOrCount; /* If element is a file, this contains size of file
                                in bytes */
  int bIsLocal; /* If non-zero, reported item is a standalone element on local
                   filesystem */
  char cszName[STEAM_MAX_PATH]; /* Base element name (no path) */
  long
      lLastAccessTime; /* Seconds since 1/1/1970 (like time_t) when element was
                          last accessed */
  long lLastModificationTime; /* Seconds since 1/1/1970 (like time_t) when
                                 element was last modified */
  long lCreationTime; /* Seconds since 1/1/1970 (like time_t) when element was
                         created */
} TSteamElemInfo;
...
typedef struct TSteamProgress {
  ...
  unsigned int
      uPercentDone; /* 0 to 100 * STEAM_PROGRESS_PERCENT_SCALE if bValid */
...
typedef void (*SteamNotificationCallback_t)(
    ESteamNotificationCallbackEvent eEvent, unsigned int nData);
...
typedef struct {
  ...
  /* A ProofOfPurchase token is not necessarily a nul-terminated string; it may
     be binary data (perhaps encrypted). Hence we need a length and an array of
     bytes.  	*/
...
typedef enum {
  ...
  eExternalBillingInfo =
      4, /* indirect billing via ISP etc (not supported yet) */
...
typedef enum {
  ...
  eSteamSubscriptionPaymentCardUseLimit =
      10, /* Card used too many times, potential fraud */
  eSteamSubscriptionPaymentCardAlert =
      11, /* Got a "pick up card" or the like from bank */
  eSteamSubscriptionFailed =
      12, /* Other Error in subscription data or transaction failed/lost */
  eSteamSubscriptionPaymentCardAVSFailure =
      13, /* Card failed Address Verification check */
  eSteamSubscriptionPaymentCardInsufficientFunds =
      14, /* Card failed due to insufficient funds */
  eSteamSubscriptionRestrictedCountry =
      15 /* The subscription is not available in the user's country */
...
typedef struct {
  ...
  // The following are only available after the subscription leaves "pending"
  // status
...
typedef enum {
  ...
  eSteamUserCyberCafe =
      0x00000004 /* CyberCafe, school, etc -- UI should ask for password */
...
typedef enum {
  ...
  /* Note: Mask value 0x2 is reserved for future use. (Some, but not all, public
     accounts already have this set.) */
...
const SteamUserIDTicketValidationHandle_t
    STEAM_INACTIVE_USERIDTICKET_VALIDATION_HANDLE = 0;
...
#define STEAM_INACTIVE_USERIDTICKET_VALIDATION_HANDLE \
  ((SteamUserIDTicketValidationHandle_t)(0))
#define STEAM_USE_LATEST_VERSION (0xFFFFFFFFu);

#endif

/******************************************************************************
** Each Steam instance (licensed Steam Service Provider) has a unique
*SteamInstanceID_t.
**
** Each Steam instance as its own DB of users.
** Each user in the DB has a unique SteamLocalUserID_t (a serial number, with
*possible
...
** So a SteamIDTicket contains SteamGlobalUserID, which is a unique combination
*of
** instance and user id.
**
** SteamLocalUserID is an unsigned 64-bit integer.
** For platforms without 64-bit int support, we provide access via a union that
*splits it into
** high and low unsigned 32-bit ints.  Such platforms will only need to compare
*LocalUserIDs
