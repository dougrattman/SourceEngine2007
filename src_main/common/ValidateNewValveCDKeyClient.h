...
// Note: this function expects the CDkey to contain dashes to split it into
// groups of 5 characters.
//
// e.g.		5WV2E-CDL82-89AQ4-ZZKH8-L27IB		(note: this example is
// not a valid key)
//
ESteamError SteamWeakVerifyNewValveCDKey(
    const char* pszCDKeyFormattedForCDLabel, uint* pReceiveGameCode,
    uint* pReceiveSalesTerritoryCode, uint* pReceiveUniqueSerialNumber);
...
ESteamError SteamGetEncryptedNewValveCDKey(
    const char* pszCDKeyFormattedForCDLabel, unsigned int ClientLocalIPAddr,
    const void* pEncryptionKeyReceivedFromAppServer,
    unsigned int uEncryptionKeyLength, void* pOutputBuffer,
...
// properties of this machine.  The resulting encrypted data can only be
// (easily) decrypted on the same machine.  Note: it is NOT impossible to
// decrypt the data elsewhere, just inconvenient.
ESteamError SteamEncryptDataForThisMachine(
    const char* pDataToEncrypt, unsigned int uSizeOfDataToEncrypt,
    void* pOutputBuffer, unsigned int uSizeOfOutputBuffer,
    unsigned int* pReceiveSizeOfEncryptedData);

ESteamError SteamDecryptDataForThisMachine(
    const char* pDataToDecrypt, unsigned int uSizeOfDataToDecrypt,
    void* pOutputBuffer, unsigned int uSizeOfOutputBuffer,
