...
the_interface INetChannel {
 public:
  ...
  virtual bool AddNetMsg(INetworkMessage * msg,
                         bool bForceReliable = false) = 0;
...
the_interface INetworkSystem : public IAppSystem {
 public:
  ...
  virtual bool StartServer(unsigned short nServerListenPort =
                               NETWORKSYSTEM_DEFAULT_SERVER_PORT) = 0;
  ...
  virtual bool StartClient(unsigned short nClientListenPort =
                               NETWORKSYSTEM_DEFAULT_CLIENT_PORT) = 0;
  ...
  virtual INetChannel *ConnectClientToServer(
      const char *pServer,
