...
bool CSteamID::SetFromSteam2String(const char *pchSteam2ID,
  ...
  // Customer support is fond of entering steam IDs in the following form:
  // STEAM_n:x:y
  const char *pchOptionalLeadString = "STEAM_";
  if (Q_strnicmp(pchSteam2ID, pchOptionalLeadString,
                 Q_strlen(pchOptionalLeadString)) == 0)
  ...
  if (cExtraCharCheck != 0 || cFieldConverted == EOF || cFieldConverted < 2 ||
      (cFieldConverted < 3 && steam2ID.m_SteamInstanceID != 1))
    return false;
...
char *CSteamID::Render() const {
  ...
  const int k_cBufs = 4;  // # of static bufs to use (so people can compose
                          // output with multiple calls to Render() )
  ...
  if (k_EAccountTypeAnonGameServer == m_EAccountType) {
    Q_snprintf(pchBuf, k_cBufLen, "[A-%u:%u(%u)]", m_EUniverse, m_unAccountID,
               m_unAccountInstance);
    ...
    Q_snprintf(pchBuf, k_cBufLen, "[%u:%u(%u%)]", m_EUniverse, m_unAccountID,
               m_unAccountInstance);
  } else if (k_EAccountTypePending == m_EAccountType) {
    Q_snprintf(pchBuf, k_cBufLen, "[%u:%u(pending)]", m_EUniverse,
               m_unAccountID);
...
bool CSteamID::BValidExternalSteamID() const {
  ...
  if (m_EAccountType != k_EAccountTypeAnonGameServer &&
    ...
    if (m_unAccountID == 0 && m_unAccountInstance == 0) return false;
  }
  return true;
}
