...
#define SERVER_STARTUP_DATA_TABLENAME \
  "server_query_info"  // the name is a remnant...
...
// Max number of history commands to send ( 2 by default ) in case of dropped
// packets
#define NUM_BACKUP_COMMAND_BITS 3
#define MAX_BACKUP_COMMANDS ((1 << NUM_BACKUP_COMMAND_BITS) - 1)

#define PROTOCOL_AUTHCERTIFICATE \
  0x01  // Connection from client is using a WON authenticated certificate
#define PROTOCOL_HASHEDCDKEY \
  0x02  // Connection from client is using hashed CD key because WON comm.
        // channel was unreachable
#define PROTOCOL_STEAM 0x03      // Steam certificates
#define PROTOCOL_LASTVALID 0x03  // Last valid protocol

#define CONNECTIONLESS_HEADER \
  0xFFFFFFFF  // all OOB packet start with this sequence
#define STEAM_KEYSIZE \
  2048  // max size needed to contain a steam authentication key (both server
        // and client)

// each channel packet has 1 uint8_t of FLAG bits
...
// NOTE:  Bits 5, 6, and 7 are used to specify the # of padding bits at the end
// of the packet!!!
...
#define svc_ClassInfo \
  10  // Info about classes (first uint8_t is a CLASSINFO_ define).
...
#define SIGNONSTATE_FULL \
  6  // we are fully connected, first non-delta packet received
