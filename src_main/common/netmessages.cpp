...
const char *CLC_ListenEvents::ToString(void) const {
  ...
  Q_snprintf(s_text, sizeof(s_text), "%s: registered events %i", GetName(),
             count);
  return s_text;
}
...
const char *g_MostCommonPrefixes[] = {"materials", "models", "sounds",
                                      "scripts"};
...
bool CLC_FileCRCCheck::WriteToBuffer(bf_write &buffer) {
  ...
  if (iCode == -1) {
    ...
    buffer.WriteString(
        &m_szFilename[V_strlen(g_MostCommonPrefixes[iCode]) + 1]);
  }
...
bool SVC_ServerInfo::WriteToBuffer(bf_write &buffer) {
  ...
  buffer.WriteLong(
      m_nClientCRC);  // To prevent cheating with hacked client dlls
...
bool SVC_ServerInfo::ReadFromBuffer(bf_read &buffer) {
  ...
  m_nClientCRC =
      buffer.ReadLong();  // To prevent cheating with hacked client dlls
...
bool SVC_UserMessage::WriteToBuffer(bf_write &buffer) {
  ...
  buffer.WriteUBitLong(m_nLength,
                       11);  // max 256 * 8 bits, see MAX_USER_MSG_DATA
...
bool SVC_UserMessage::ReadFromBuffer(bf_read &buffer) {
  ...
  m_nLength =
      buffer.ReadUBitLong(11);  // max 256 * 8 bits, see MAX_USER_MSG_DATA
  m_DataIn = buffer;
  return buffer.SeekRelative(m_nLength);
}
...
bool SVC_CreateStringTable::WriteToBuffer(bf_write &buffer) {
  ...
  /*
  JASON: this code is no longer needed; the ':' is prepended to the table name
  at table creation time. if ( m_bIsFilenames )
  {
...
const char *SVC_CreateStringTable::ToString(void) const {
  Q_snprintf(
      ...
      GetName(), m_szTableName, m_nNumEntries, Bits2Bytes(m_nLength),
      m_nUserDataSize, m_nUserDataSizeBits);
  return s_text;
}
...
bool SVC_ClassInfo::ReadFromBuffer(bf_read &buffer) {
  ...
  for (int i = 0; i < m_nNumServerClasses; i++) {
    ...
    buffer.ReadString(serverclass.datatablename,
                      sizeof(serverclass.datatablename));
...
bool SVC_SendTable::ReadFromBuffer(bf_read &buffer) {
  ...
  m_nLength =
      buffer.ReadShort();  // TODO do we have a maximum length ? check that
...
const char *SVC_PacketEntities::ToString(void) const {
  Q_snprintf(s_text, sizeof(s_text),
             ...
             m_bUpdateBaseline ? " BL update," : "", Bits2Bytes(m_nLength));
  return s_text;
}
