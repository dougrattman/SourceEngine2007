...
bool Pack_CreateFile(const char *pakfile) {
  ...
  int fh = _open(pakfile, _O_CREAT | _O_RDWR | _O_BINARY | _O_TRUNC,
                 _S_IREAD | _S_IWRITE);
  if (fh == -1) return false;

  // create empty pak header & end signature
  packheader64_t header = {{'P', 'K', '6', '4'}, 0, 0};
  packappenededheader_t appendheader = {
      {'P', 'A', 'C', 'K', 'A', 'P', 'P', 'E'}};
...
void Pack_Clear(const char *pakfile) {
  ...
  if (appended.id[0] == 'P' && appended.id[1] == 'A' && appended.id[2] == 'C' &&
      appended.id[3] == 'K' && appended.id[4] == 'A' && appended.id[5] == 'P' &&
      appended.id[6] == 'P' && appended.id[7] == 'E') {
  ...
  assert(_filelengthi64(fh) ==
         iOriginalFileSize + sizeof(header) + sizeof(appendheader));
...
bool Pack_AppendFile(const char *pakfile, const char *file) {
  if (!Pack_StartAppendingFile(pakfile, file)) return false;
...
bool Pack_StartAppendingFile(const char *pakfile, const char *pFileName) {
  ...
  if (strlen(pFileName) >= 112) {
    printf(
        "Couldn't append file '%s', filename is too long (> 128 characters)\n",
        pFileName);
    return false;
  }
  ...
  // jump to the end of the file, see if it has a header
  packappenededheader_t appended;
  _lseeki64(g_Pack.m_hPack, (long)(0 - sizeof(packappenededheader_t)),
            SEEK_END);
  _read(g_Pack.m_hPack, &appended, sizeof(appended));

  if (!(appended.id[0] == 'P' && appended.id[1] == 'A' &&
        appended.id[2] == 'C' && appended.id[3] == 'K' &&
        appended.id[4] == 'A' && appended.id[5] == 'P' &&
        appended.id[6] == 'P' && appended.id[7] == 'E')) {
  ...
  // we're going to put the new file where the current packheader is, and move
  // the header forward in the file
...
void Pack_FinishAppendingFile() {
  if (!g_Pack.m_hPack || g_Pack.m_hPack == -1) return;
  ...
  g_Pack.m_pPackFiles[g_Pack.m_iFilesInPack - 1].filelen =
      g_Pack.m_iNewFileLength;
  g_Pack.m_pPackFiles[g_Pack.m_iFilesInPack - 1].filepos =
      (0 - g_Pack.m_iNewFileLength);
  strncpy(g_Pack.m_pPackFiles[g_Pack.m_iFilesInPack - 1].name,
          g_Pack.m_szNewFileName,
          sizeof(g_Pack.m_pPackFiles[g_Pack.m_iFilesInPack - 1].name));
  ...
  assert(_filelengthi64(g_Pack.m_hPack) == g_Pack.m_iOldPakFileSize +
                                               sizeof(packfile64_t) +
                                               g_Pack.m_iNewFileLength);

  // header + file
  assert(_filelengthi64(g_Pack.m_hPack) ==
         (appended.packheaderpos + sizeof(header) + header.dirlen +
          sizeof(appended)));
...
bool Pack_Validate(const char *pakfile) {
  ...
  // header + file
  assert(iFileSize == (appended.packheaderpos + sizeof(header) + header.dirlen +
                       sizeof(appended)));
