...
#define M2A_SERVERS 'd'  // + 6 uint8_t IP/Port list.
...
#define M2A_SERVER_BATCH \
  'f'  // + int32_t next uniqueID( -1 for last batch ) + 6 uint8_t IP/Port list.
...
#define A2M_GETMASTERSERVERS \
  'v'  // + uint8_t (type of request, TYPE_CLIENT_MASTER or TYPE_SERVER_MASTER)

// Master server list response
#define M2A_MASTERSERVERS 'w'  // + uint8_t type + 6 uint8_t IP/Port List
