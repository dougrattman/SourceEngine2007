...
the_interface IGameServerData : public IBaseInterface {
 public:
  ...
  // is authConnection is true the SERVERDATA_AUTH command needs to succeed
  // before other commands
  virtual ra_listener_id GetNextListenerID(bool authConnection = true,
                                           const netadr_t *adr = NULL) = 0;
  // tell the remote access class that this ID is the special dedicated server
  // UI callback (and not an rcon one)
