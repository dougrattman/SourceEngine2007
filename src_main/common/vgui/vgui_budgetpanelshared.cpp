...
// Global ConVars.
ConVar budget_history_range_ms("budget_history_range_ms", "66.666666667",
                               FCVAR_ARCHIVE,
                               "budget history range in milliseconds",
                               BUDGET_CVAR_CALLBACK);
ConVar budget_panel_bottom_of_history_fraction(
    "budget_panel_bottom_of_history_fraction", ".25", FCVAR_ARCHIVE,
    "number between 0 and 1", BUDGET_CVAR_CALLBACK);
ConVar budget_bargraph_range_ms("budget_bargraph_range_ms", "16.6666666667",
                                FCVAR_ARCHIVE,
                                "budget bargraph range in milliseconds",
                                BUDGET_CVAR_CALLBACK);
ConVar budget_background_alpha("budget_background_alpha", "128", FCVAR_ARCHIVE,
                               "how translucent the budget panel is");

ConVar budget_panel_x("budget_panel_x", "0", FCVAR_ARCHIVE,
                      "number of pixels from the left side of the game screen "
                      "to draw the budget panel",
                      BUDGET_CVAR_CALLBACK);
ConVar budget_panel_y("budget_panel_y", "50", FCVAR_ARCHIVE,
                      "number of pixels from the top side of the game screen "
                      "to draw the budget panel",
                      BUDGET_CVAR_CALLBACK);
ConVar budget_panel_width("budget_panel_width", "512", FCVAR_ARCHIVE,
                          "width in pixels of the budget panel",
                          BUDGET_CVAR_CALLBACK);
ConVar budget_panel_height("budget_panel_height", "384", FCVAR_ARCHIVE,
                           "height in pixels of the budget panel",
                           BUDGET_CVAR_CALLBACK);

static CUtlVector<IVProfExport::CExportedBudgetGroupInfo>
...
void PanelGeometryChangedCallBack(IConVar *var, const char *pOldString,
                                  float flOldValue) {
...
CBudgetPanelShared::CBudgetPanelShared(vgui::Panel *pParent,
  ...
  if (g_pVProfExport) g_pVProfExport->SetBudgetFlagsFilter(budgetFlagsFilter);
...
void CBudgetPanelShared::OnNumBudgetGroupsChanged() { SendConfigDataToBase(); }
...
void CBudgetPanelShared::SendConfigDataToBase() {
  ...
  data.m_flBottomOfHistoryFraction =
      budget_panel_bottom_of_history_fraction.GetFloat();
...
void CBudgetPanelShared::DrawColoredText(vgui::HFont font, int x, int y, int r,
                                         int g, int b, int a, const char *pText,
                                         ...) {
  char msg[4096];
  va_list marker;
...
void CBudgetPanelShared::PaintBackground() {
  if (g_pVProfExport) g_pVProfExport->PauseProfile();
...
void CBudgetPanelShared::PostChildPaint() {
  ...
  if (g_pVProfExport) g_pVProfExport->ResumeProfile();
}
