...
ConVar budget_bargraph_background_alpha("budget_bargraph_background_alpha",
                                        "128", FCVAR_ARCHIVE,
                                        "how translucent the budget panel is");

ConVar budget_peaks_window(
    "budget_peaks_window", "30", FCVAR_ARCHIVE,
    "number of frames to look at when figuring out peak frametimes");
ConVar budget_averages_window(
    "budget_averages_window", "30", FCVAR_ARCHIVE,
    "number of frames to look at when figuring out average frametimes");
ConVar budget_show_peaks("budget_show_peaks", "1", FCVAR_ARCHIVE,
                         "enable/disable peaks in the budget panel");
ConVar budget_show_averages("budget_show_averages", "0", FCVAR_ARCHIVE,
                            "enable/disable averages in the budget panel");

CBudgetBarGraphPanel::CBudgetBarGraphPanel(CBaseBudgetPanel *pParent,
...
void CBudgetBarGraphPanel::GetBudgetGroupTopAndBottom(int id, int &top,
                                                      int &bottom) {
...
void CBudgetBarGraphPanel::DrawBarAtIndex(int id, float percent) {
  ...
  m_pBudgetPanel->GetConfigData().m_BudgetGroupInfo[id].m_Color.GetColor(
...
void CBudgetBarGraphPanel::DrawTickAtIndex(int id, float percent, int red,
...
void CBudgetBarGraphPanel::DrawTimeLines(void) {
  ...
  if (config.m_nLinesPerTimeLabel != 0.0f) {
    flValueInterval =
        config.m_flTimeLabelInterval / config.m_nLinesPerTimeLabel;
  }
  ...
  for (i = 0; i < nTotalLines; i++) {
    ...
    float flTemp = (config.m_flBarGraphRange != 0.0f)
                       ? (flValueInterval / config.m_flBarGraphRange)
                       : flValueInterval;
...
void CBudgetBarGraphPanel::DrawInstantaneous() {
  ...
  const double *pBudgetGroupTimes = m_pBudgetPanel->GetBudgetGroupData(
      nGroups, nSamplesPerGroup, nSampleOffset);
  ...
  int i;
  for (i = 0; i < nGroups; i++) {
    float percent = m_pBudgetPanel->GetBudgetGroupPercent(
        pBudgetGroupTimes[nSamplesPerGroup * i + nSampleOffset]);
...
void CBudgetBarGraphPanel::DrawPeaks() {
  ...
  for (i = 0; i < nGroups; i++) {
    ...
    for (j = 0; j < numSamples; j++) {
      double tmp;
      int offset =
          (nSampleOffset - j + BUDGET_HISTORY_COUNT) % BUDGET_HISTORY_COUNT;
...
void CBudgetBarGraphPanel::DrawAverages() {
  ...
  for (i = 0; i < nGroups; i++) {
    uint8_t red, green, blue, alpha;
    m_pBudgetPanel->GetConfigData().m_BudgetGroupInfo[i].m_Color.GetColor(
        red, green, blue, alpha);
