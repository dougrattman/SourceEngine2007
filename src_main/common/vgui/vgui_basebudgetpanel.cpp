...
CBaseBudgetPanel::CBaseBudgetPanel(vgui::Panel *pParent,
                                   const char *pElementName)
    : vgui::Panel(pParent, pElementName) {
  m_BudgetHistoryOffset = 0;
  m_CachedNumTimeLabels = 0;
  ...
  vgui::HScheme scheme =
      vgui::scheme()->LoadSchemeFromFile("resource/SourceScheme.res", "Client");
  SetScheme(scheme);
...
float CBaseBudgetPanel::GetBudgetGroupPercent(float value) {
  if (m_ConfigData.m_flBarGraphRange == 0.0f) return 1.0f;
  return value / m_ConfigData.m_flBarGraphRange;
}

const double *CBaseBudgetPanel::GetBudgetGroupData(int &nGroups,
...
void CBaseBudgetPanel::OnConfigDataChanged(const CBudgetPanelConfigData &data) {
  ...
  if (m_ConfigData.m_BudgetGroupInfo.Count() > m_BudgetGroupTimes.Count()) {
    ...
    for (int i = oldNumGroups; i < m_ConfigData.m_BudgetGroupInfo.Count();
         i++) {
...
void CBaseBudgetPanel::Rebuild(const CBudgetPanelConfigData &data) {
  ...
  if (m_ConfigData.m_BudgetGroupInfo.Count() > m_GraphLabels.Count()) {
    ...
    for (i = oldNumBudgetGroups; i < m_ConfigData.m_BudgetGroupInfo.Count();
         i++) {
      const char *pBudgetGroupName =
          m_ConfigData.m_BudgetGroupInfo[i].m_Name.String();
      m_GraphLabels[i] =
          new vgui::Label(this, pBudgetGroupName, pBudgetGroupName);
    }
  } else {
  ...
  if (m_ConfigData.m_HistoryLabelValues.Count() > m_HistoryLabels.Count()) {
    ...
    for (i = oldNumHistoryLabels; i < m_HistoryLabels.Count(); i++) {
      m_HistoryLabels[i] =
          new vgui::Label(this, "history label", "history label");
    }
  } else {
    while (m_HistoryLabels.Count() >
           m_ConfigData.m_HistoryLabelValues.Count()) {
  ...
  // Note: the time lines still use milliseconds for the computations about
  // where to draw them, but each BudgetGroupDataType_t has its own scale.
...
void CBaseBudgetPanel::PerformLayout() {
  if (!m_pBudgetHistoryPanel || !m_pBudgetBarGraphPanel) return;
  ...
  m_pBudgetBarGraphPanel->SetPos(
      maxLabelWidth, totalHeightMinusTimeLabels * bottomOfHistoryPercentage);
  m_pBudgetBarGraphPanel->SetSize(
      totalWidth - maxLabelWidth,
      totalHeightMinusTimeLabels * (1 - bottomOfHistoryPercentage));

  for (i = 0; i < m_GraphLabels.Count(); i++) {
    m_GraphLabels[i]->SetPos(
        0,
        (bottomOfHistoryPercentage * totalHeightMinusTimeLabels) +
            (i * totalHeightMinusTimeLabels * (1 - bottomOfHistoryPercentage)) /
                m_ConfigData.m_BudgetGroupInfo.Count());
    // fudge height by 1 for rounding
    m_GraphLabels[i]->SetSize(
        maxLabelWidth,
        1 + (totalHeightMinusTimeLabels * (1 - bottomOfHistoryPercentage)) /
                m_ConfigData.m_BudgetGroupInfo.Count());
  ...
  for (i = 0; i < m_TimeLabels.Count(); i++) {
    int labelWidth, labelHeight;
    m_TimeLabels[i]->GetContentSize(labelWidth, labelHeight);
    int xp = maxLabelWidth + (i * m_ConfigData.m_flTimeLabelInterval) / fRange *
                                 (totalWidth - maxLabelWidth);

    m_TimeLabels[i]->SetPos(xp - (labelWidth * 0.5), totalHeight - labelHeight);
  ...
  for (i = 0; i < m_HistoryLabels.Count(); i++) {
    ...
    float yp = (fRange != 0)
                   ? budgetHistoryHeight *
                         m_ConfigData.m_HistoryLabelValues[i] / (float)fRange
                   : 0.0f;
    int top = (int)(budgetHistoryHeight - yp - 1 - labelHeight * 0.5f);
...
void CBaseBudgetPanel::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  for (i = 0; i < m_ConfigData.m_BudgetGroupInfo.Count(); i++) {
    ...
    m_GraphLabels[i]->SetFont(
        pScheme->GetFont("BudgetLabel", IsProportional()));
    if (m_bDedicated) {
      m_GraphLabels[i]->SetBgColor(
          pScheme->GetColor("ControlBG", Color(0, 0, 0, 255)));
    }
  }
  ...
  for (i = 0; i < m_HistoryLabels.Count(); i++) {
    ...
    m_HistoryLabels[i]->SetFont(
        pScheme->GetFont("BudgetLabel", IsProportional()));
    if (m_bDedicated) {
      m_HistoryLabels[i]->SetBgColor(
          pScheme->GetColor("ControlBG", Color(0, 0, 0, 255)));
    }
  }
...
void CBaseBudgetPanel::Paint() {
  m_pBudgetHistoryPanel->SetData(&m_BudgetGroupTimes[0].m_Time[0],
                                 GetNumCachedBudgetGroups(),
                                 BUDGET_HISTORY_COUNT, m_BudgetHistoryOffset);
  BaseClass::Paint();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseBudgetPanel::GetGraphLabelScreenSpaceTopAndBottom(int id, int &top,
                                                            int &bottom) {
  int x = 0;
  int y = 0;
