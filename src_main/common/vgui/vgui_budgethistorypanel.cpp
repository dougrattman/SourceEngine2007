...
ConVar budget_history_numsamplesvisible("budget_history_numsamplesvisible",
                                        "100", FCVAR_ARCHIVE,
                                        "number of samples to draw in the "
                                        "budget history window.  The lower the "
                                        "better as far as rendering overhead "
                                        "of the budget panel");

CBudgetHistoryPanel::CBudgetHistoryPanel(CBaseBudgetPanel *pParent,
  ...
  // For some reason, vgui::Frame likes to set the minimum size to (128,66) and
  // we may want to get smaller.
  SetMinimumSize(0, 0);
}
...
void CBudgetHistoryPanel::Paint() {
  ...
  for (j = 0; j < m_nGroups; j++) {
    ...
    m_pBudgetPanel->GetConfigData().m_BudgetGroupInfo[j].m_Color.GetColor(
  ...
  for (int i = 0;
       i < m_pBudgetPanel->GetConfigData().m_HistoryLabelValues.Count(); i++) {
...
void CBudgetHistoryPanel::DrawBudgetLine(float val) {
  ...
  GetSize(width, height);
  double y =
      (val - m_fRangeMin) * (1.0f / (m_fRangeMax - m_fRangeMin)) * height;
...
void CBudgetHistoryPanel::SetData(double *pData, int nGroups,
                                  int nSamplesPerGroup, int nSampleOffset) {
  m_pData = pData;
  m_nGroups = nGroups;
