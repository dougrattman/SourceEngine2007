...
class HtmlWindow : public vgui::IHTML, vgui::IHTMLEvents {
 ...
 private:
  ...
  HWND m_oleObjectHWND;  // the oleObjects window (which is inside the main
                         // window)
  HWND m_ieHWND;         // the handle to the IE browser itself, which is inside
                         // oleObject
  vgui::VPANEL m_vcontext;  // the vpanel of our frame, used to find out the abs
                            // pos of the panel
  ...
  vgui::MemoryBitmap
      *m_Bitmap;  // the vgui image that the page is rendered into
  ...
  DWORD m_adviseCookie;  // cookies (magic numbers) used to hook into ActiveX
                         // events
  DWORD m_HtmlEventsAdviseCookie;

  HHOOK m_hHook;       // hook for window events
  bool m_bHooked;      // whether a hook has been installed yet
  LPARAM m_iMousePos;  // the lParam value for the last mouse movement on the
                       // object, used in GetMsgProc()
