...
class CBudgetPanelConfigData {
 public:
  ...
  CUtlVector<float>
      m_HistoryLabelValues;  // A label will be placed at each of these values.
...
class CBaseBudgetPanel : public vgui::Panel {
 ...
 public:
  ...
  // The derived class should implement this and set the text in the time
  // labels.
  ...
  const double *GetBudgetGroupData(int &nGroups, int &nSamplesPerGroup,
 ...
 protected:
  ...
  CUtlVector<BudgetGroupTimeData_t>
      m_BudgetGroupTimes;  // [m_CachedNumBudgetGroups][BUDGET_HISTORY_COUNT]
