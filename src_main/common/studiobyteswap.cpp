...
//
// Purpose: Swaps the bytes in all file types generated by studiomdl (.vvd,
// .vtx, .mdl, .phy, .ani) so the files can be loaded on a big-endian machine.
...
#undef ALIGN4
#define ALIGN4(a) a = (u8 *)((uintptr_t)((u8 *)a + 3) & ~3)
#undef ALIGN16
#define ALIGN16(a) a = (u8 *)((uintptr_t)((u8 *)a + 15) & ~15)
#undef ALIGN32
#define ALIGN32(a) a = (u8 *)((uintptr_t)((u8 *)a + 31) & ~31)
#undef ALIGN64
#define ALIGN64(a) a = (u8 *)((uintptr_t)((u8 *)a + 63) & ~63)
...
// Helper to write a chunk of objects of the same type, and increment the buffer
// pointers.
template <typename T>
inline void WriteObjects(u8 **out, u8 **base, usize objectCount = 1) {
  T tempObject;
  for (usize i = 0; i < objectCount; ++i) {
    memcpy(&tempObject, *base, sizeof(T));
    g_Swap.SwapFieldsToTargetEndian(&tempObject, &tempObject);
    memcpy(*out, &tempObject, sizeof(T));
...
// pointers.
template <typename T>
inline void WriteObjects(T **out, T **base, usize objectCount = 1) {
  T tempObject;
  for (usize i = 0; i < objectCount; ++i) {
    memcpy(&tempObject, *base, sizeof(T));
    g_Swap.SwapFieldsToTargetEndian(&tempObject, &tempObject);
...
// Helper to write a buffer of some integral type, and increment the buffer
// pointers.
template <typename T>
inline void WriteBuffer(u8 **pOutputBuffer, u8 **pBaseData,
                        usize objectCount = 1) {
  T tempObject;
  for (usize i = 0; i < objectCount; ++i) {
    memcpy(&tempObject, *pBaseData, sizeof(T));
    g_Swap.SwapBufferToTargetEndian(&tempObject, &tempObject);
    memcpy(*pOutputBuffer, &tempObject, sizeof(T));
...
template <typename T>
inline T SrcNative(T *idx) {
  T ret = *idx;

  if (!g_bNativeSrc) g_Swap.SwapBuffer(&ret, idx);

  return ret;
}

template <typename T>
inline T DestNative(T *idx) {
  T ret = *idx;
...
// Declares src/dest u8 pointers and sets them to some index offset in
// the buffers.
#define DECLARE_INDEX_POINTERS(ptr, base, index)                 \
  u8 *ptr##Src = (u8 *)base##Src + SrcNative(&base##Src->index); \
  u8 *ptr##Dest = (u8 *)base##Dest + SrcNative(&base##Src->index);

// Declares src/dest u8 pointers and sets them to some index offset in
// the buffers.  If src pointer is misaligned, the fixup method is
// called.
...
//	Same as DECLARE_INDEX_POINTERS, but reuses existing u8 pointers.
#define SET_INDEX_POINTERS(ptr, base, index)                 \
  ptr##Src = (u8 *)base##Src + SrcNative(&base##Src->index); \
  ptr##Dest = (u8 *)base##Dest + SrcNative(&base##Src->index);
...
#define ITERATE_BLOCK(objPtr, count)                           \
  for (int objPtr##_idx = 0; objPtr##_idx < SrcNative(&count); \
       ++objPtr##_idx, ++objPtr, ++objPtr##Src, ++objPtr##Dest)

//	Checks for misaligned source pointer, then calculates the necessary
// fixup,
//  calls the fixup function, and sets the src pointer to the new position.
#define FIXUP_OFFSETS(ptr, base, index)                                \
  {                                                                    \
    u8 *ptr##Fixup = ptr##Src;                                         \
    ALIGN4(ptr##Fixup);                                                \
    if (ptr##Fixup != ptr##Src) {                                      \
      ptrdiff_t nShiftBytes = ptr##Fixup - ptr##Src;                   \
      if (g_bVerbose)                                                  \
        Warning("Shifting misaligned data block by %d bytes at " #base \
                "->" #index "\n",                                      \
                nShiftBytes);                                          \
      ptrdiff_t prevBytes = (u8 *)ptr##Src - (u8 *)g_pDataSrcBase;     \
      Q_memmove((u8 *)ptr##Src + nShiftBytes, ptr##Src,                \
                fixedFileSize - prevBytes);                            \
      g_pFixPoint = ptr##Src;                                          \
      g_nFixupBytes = nShiftBytes;                                     \
      fixedFileSize += nShiftBytes;                                    \
      if (fixedFileSize > fileSize + BYTESWAP_ALIGNMENT_PADDING) {     \
        Error(                                                         \
            "Byteswap buffer overrun - increase "                      \
            "BYTESWAP_ALIGNMENT_PADDING!\n");                          \
        return 0;                                                      \
      }                                                                \
...
typedef void (*datadescProcessFunc_t)(void *pBase, void *pData,
...
int GetIntegerFromField(void *pData, int fieldType) {
  ...
  if (fieldType == FIELD_SHORT) {
    return SrcNative((short *)pData);
  }

  Error("GetIntegerFromField using unsupported fieldType %d\n", fieldType);
...
void PutIntegerInField(void *pData, int index, int fieldType) {
  if (fieldType == FIELD_INTEGER) {
    ...
    *(short *)pData = SrcNative(&index);
    ...
    Error("PutIntegerInField using unsupported fieldType %d\n", fieldType);
  }
}
...
void ProcessField(void *pBase, void *pData, typedescription_t *pField,
                  datadescProcessFunc_t pfn) {
  if (pfn) pfn(pBase, pData, pField);
}

// Process the fields of a datadesc.
void ProcessFields(void *pBaseAddress, void *pData, datamap_t *pDataMap,
                   datadescProcessFunc_t pfnProcessFunc) {
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    ProcessField(pBaseAddress,
                 (u8 *)pData + pField->fieldOffset[TD_OFFSET_NORMAL], pField,
                 pfnProcessFunc);
  }
}

// Process the fields of a datadesc.
void ProcessFields(void *pData, datamap_t *pDataMap,
                   datadescProcessFunc_t pfnProcessFunc) {
  ProcessFields(pData, pData, pDataMap, pfnProcessFunc);
}

// Process a datadesc field by name
void ProcessFieldByName(void *pBaseAddress, void *pData, datamap_t *pDataMap,
                        const char *pName,
                        datadescProcessFunc_t pfnProcessFunc) {
  // deal with base class first
  if (pDataMap->baseMap) {
    ProcessFieldByName(pBaseAddress, pData, pDataMap->baseMap, pName,
                       pfnProcessFunc);
  }
  ...
  for (int i = 0; i < fieldCount; ++i) {
    typedescription_t *pField = &pFields[i];
    if (!_stricmp(pField->fieldName, pName)) {
      ProcessField(pBaseAddress,
                   (u8 *)pData + pField->fieldOffset[TD_OFFSET_NORMAL], pField,
                   pfnProcessFunc);
      break;
...
// Process a datadesc field by name.
void ProcessFieldByName(void *pData, datamap_t *pDataMap, const char *pName,
                        datadescProcessFunc_t pfnProcessFunc) {
...
int ByteswapPHY(void *pDestBase, const void *pSrcBase, const int fileSize) {
  ...
  if (g_bNativeSrc) {
    ...
    pSrc = (u8 *)pSrcBase + pHdr->size;
    pDest = (u8 *)pDestBase + pHdr->size;

    int bufSize = fileSize - pHdr->size;
    pCollision->VCollideLoad(&collide, pHdr->solidCount, (const char *)pSrc,
                             bufSize, false);
  }

  // Swap the collision data headers
  for (int i = 0; i < pHdr->solidCount; ++i) {
    swapcompactsurfaceheader_t *baseHdr =
        (swapcompactsurfaceheader_t *)(g_bNativeSrc ? pSrc : pDest);
    WriteObjects<swapcompactsurfaceheader_t>(pDest, pSrc);

    int srcIncrement =
        baseHdr->surfaceSize + sizeof(swapcompactsurfaceheader_t);
    int destIncrement = srcIncrement;
    bool bCopyToSrc = !g_bNativeSrc;

    if (baseHdr->vphysicsID != MAKEID<int32_t>('V', 'P', 'H', 'Y')) {
      // May be old phy format
      legacysurfaceheader_t *legacyHdr =
          (legacysurfaceheader_t *)(g_bNativeSrc ? pSrc : pDest);
      WriteObjects<legacysurfaceheader_t>(pDest, pSrc);
      if (legacyHdr->dummy[2] == MAKEID<int32_t>('I', 'V', 'P', 'S') ||
          legacyHdr->dummy[2] == 0) {
        srcIncrement = legacyHdr->byte_size + sizeof(int);
        destIncrement =
            legacyHdr->byte_size + sizeof(swapcompactsurfaceheader_t);
        bCopyToSrc = false;

        if (!g_bNativeSrc) {
          // src needs the size member to be native to load vcollides
          memcpy(pSrc, pDest, sizeof(int));
        }
      } else {
        // Not recognized
        Assert(0);
    ...
    if (bCopyToSrc) {
      ...
      memcpy(pSrc, pDest, sizeof(swapcompactsurfaceheader_t));
    }
  ...
  for (int i = 0; i < collide.solidCount; ++i) {
    // skip over the size
    pDest += sizeof(int);
    int offset = pCollision->CollideWrite((char *)pDest, collide.solids[i],
                                          g_bNativeSrc);
    int destSize = g_bNativeSrc ? SwapLong(offset) : offset;
    memcpy(pDest - sizeof(int), &destSize, sizeof(int));
    pDest += offset;
  }
  ...
  int newFileSize = pDest - (u8 *)pDestBase + remainingBytes;
...
// Swap a .vvd file.  Doesn't do any alignment fixups
int ByteswapVVD(void *pDestBase, const void *pSrcBase, const int fileSize) {
  memset(pDestBase, 0, fileSize);

  u8 *pDataSrc = (u8 *)pSrcBase;
  u8 *pDataDest = (u8 *)pDestBase;
  ...
  WriteObjects<vertexFileFixup_t>(&pDataDest, &pDataSrc,
                                  SrcNative(&pHdr->numFixups));

  /** VERTEX DATA **/

  SET_INDEX_POINTERS(pData, pHdr, vertexDataStart)
  WriteObjects<mstudiovertex_t>(&pDataDest, &pDataSrc,
                                SrcNative(&pHdr->numLODVertexes[0]));

  /** TANGENT DATA **/

  if (pHdr->tangentDataStart != 0) {
    SET_INDEX_POINTERS(pData, pHdr, tangentDataStart)
    WriteBuffer<float>(&pDataDest, &pDataSrc,
                       4 * SrcNative(&pHdr->numLODVertexes[0]));
  }

  int newFileSize = pDataDest - (u8 *)pDestBase;

  if (g_pCompressFunc) {
    void *pInput = (u8 *)pDestBase + sizeof(vertexFileHeader_t);
...
// Swap a .vtx file.  Doesn't do any alignment fixups
int ByteswapVTX(void *pDestBase, const void *pSrcBase, const int fileSize) {
  ...
  DECLARE_OBJECT_POINTERS(pBodyPartHeader, pData,
  ...
  ITERATE_BLOCK(pBodyPartHeader, pVtxHeader->numBodyParts) {
    ...
    ITERATE_BLOCK(pModelHeader, pBodyPartHeader->numModels) {
      ...
      DECLARE_OBJECT_POINTERS(pModelLODHeader, pData,
      ...
      ITERATE_BLOCK(pModelLODHeader, pModelHeader->numLODs) {
        ...
        if (prevOffset - sizeof(OptimizedModel::ModelLODHeader_t) ==
            meshOffset) {
        ...
        DECLARE_OBJECT_POINTERS(pMeshHeader, pData,
        ...
        ITERATE_BLOCK(pMeshHeader, pModelLODHeader->numMeshes) {
          ...
          DECLARE_OBJECT_POINTERS(pStripGroupHeader, pData,
          ...
          ITERATE_BLOCK(pStripGroupHeader, pMeshHeader->numStripGroups) {
            ...
            WriteObjects<OptimizedModel::Vertex_t>(
                pDataDest, pDataSrc, SrcNative(&pStripGroupHeader->numVerts));

            /** VERT INDICES **/

            SET_INDEX_POINTERS(pData, pStripGroupHeader, indexOffset)
            WriteBuffer<short>(pDataDest, pDataSrc,
                               SrcNative(&pStripGroupHeader->numIndices));

            /** STRIPS **/

            SET_INDEX_POINTERS(pData, pStripGroupHeader, stripOffset)
            DECLARE_OBJECT_POINTERS(pStripHeader, pData,
            ...
            ITERATE_BLOCK(pStripHeader, pStripGroupHeader->numStrips) {
              ...
              WriteObjects<OptimizedModel::BoneStateChangeHeader_t>(
                  pDataDest, pDataSrc,
                  SrcNative(&pStripHeader->numBoneStateChanges));
            }
          }
        }
      }
  ...
  DECLARE_OBJECT_POINTERS(pMatRepListHeader, pData,
  ...
  ITERATE_BLOCK(pMatRepListHeader, pVtxHeader->numLODs) {
    ...
    WriteObjects<OptimizedModel::MaterialReplacementHeader_t>(
        &pDataDest, &pDataSrc, SrcNative(&pMatRepListHeader->numReplacements));
  }

  int newFileSize = fileSize;

  if (g_pCompressFunc) {
    void *pInput = (u8 *)pDestBase + sizeof(OptimizedModel::FileHeader_t);
...
// Swap animation data.  Fixes alignment errors
void ByteswapAnimData(mstudioanimdesc_t *pAnimDesc, int section, u8 *&pDataSrc,
                      u8 *&pDataDest) {
  /** ANIMATIONS **/
  ...
  while (pAnimation) {
    if (pAnimation->flags &
        (STUDIO_ANIM_RAWROT | STUDIO_ANIM_RAWPOS | STUDIO_ANIM_RAWROT2)) {
      if (pAnimation->flags & STUDIO_ANIM_RAWROT) {
        int offset = (u8 *)pAnimation->pQuat48() - (u8 *)pAnimation;
        pDataSrc = (u8 *)pAnimationSrc + offset;
        pDataDest = (u8 *)pAnimationDest + offset;
      ...
      if (pAnimation->flags & STUDIO_ANIM_RAWROT2) {
        int offset = (u8 *)pAnimation->pQuat64() - (u8 *)pAnimation;
        pDataSrc = (u8 *)pAnimationSrc + offset;
        pDataDest = (u8 *)pAnimationDest + offset;

        // Write the quaternion (bit fields contained in 1 64 bit int
        WriteBuffer<int64_t>(&pDataDest, &pDataSrc, 1);
      }

      if (pAnimation->flags & STUDIO_ANIM_RAWPOS) {
        int offset = (u8 *)pAnimation->pPos() - (u8 *)pAnimation;
        pDataSrc = (u8 *)pAnimationSrc + offset;
        pDataDest = (u8 *)pAnimationDest + offset;
      ...
      int offset = (u8 *)pAnimation->pRotV() - (u8 *)pAnimation;
      pDataSrc = (u8 *)pAnimationSrc + offset;
      pDataDest = (u8 *)pAnimationDest + offset;
      ...
      if (pAnimation->flags & STUDIO_ANIM_ANIMPOS) {
        int anim_offset = (u8 *)pAnimation->pPosV() - (u8 *)pAnimation;
        pDataSrc = (u8 *)pAnimationSrc + anim_offset;
        pDataDest = (u8 *)pAnimationDest + anim_offset;
      ...
      // Note: destanimvalue_t is a union that can be either two bytes or a
      // short. This structure is used to compress animation data using RLE. The
      // first object of a chunk acts as the header, and uses the two bytes to
      ...
      // The total number of chunks has been determined by counting the number
      // of valid (non-zero) offsets.
      for (int animValue = 0; animValue < animValueCt; ++animValue) {
        ...
        if (sectionFrames) {
          ...
          iStartFrame = std::min(iStartFrame, totalFrames - 1);
          iEndFrame = std::min(iEndFrame, totalFrames - 1);
        ...
        while (encodedFrames < totalFrames) {
          ...
          mstudioanimvalue_t *pDestAnimvalue =
              (mstudioanimvalue_t *)(g_bNativeSrc ? pDataSrc : pDataDest);
    ...
    // TODOKD: Could add a fixup here with some more work, hasn't been necessary
    // yet
    if (pAnimation->nextoffset) {
      // Set pointers to the next animation
      pAnimationSrc = (mstudioanim_t *)((u8 *)pAnimationSrc +
                                        SrcNative(&pAnimation->nextoffset));
      pAnimationDest = (mstudioanim_t *)((u8 *)pAnimationDest +
                                         SrcNative(&pAnimation->nextoffset));
...
int ByteswapIKRules(studiohdr_t *&pHdrSrc, int numikrules, int numFrames,
                    u8 *&pDataSrc, u8 *&pDataDest, int &fixedFileSize,
                    const int fileSize) {
  ...
  ITERATE_BLOCK(pIKRule, numikrules) {
    ...
    // Calculate the number of ikerrors by converting the ikerror start and end
    // float values to frame numbers. (See the generation of these values in
    // simplify.cpp: ProcessIKRules()).
    ...
    if (pIKRule->compressedikerrorindex) {
      ...
      mstudiocompressedikerror_t *pCompressed =
          (mstudiocompressedikerror_t *)pDataSrc;

      // Write the animvalues.
      for (int idx = 0; idx < 6; ++idx) {
        if (pCompressed->offset[idx]) {
          u8 *pAnimvalueSrc = pDataSrc + SrcNative(&pCompressed->offset[idx]);
          u8 *pAnimvalueDest = pDataDest + SrcNative(&pCompressed->offset[idx]);

          int numerror = 0;
          while (numerror < totalerror) {
            // Write the first animation value (struct of 2 bytes)
            mstudioanimvalue_t *pDestAnimvalue =
                (mstudioanimvalue_t *)(g_bNativeSrc ? pAnimvalueSrc
                                                    : pAnimvalueDest);
            WriteBuffer<char>(&pAnimvalueDest, &pAnimvalueSrc, 2);

            // Write the remaining animation values from this group (shorts)
            WriteBuffer<short>(&pAnimvalueDest, &pAnimvalueSrc,
                               pDestAnimvalue->num.valid);
      ...
      if (pIKRule->szattachmentindex) {
        ...
        usize size = strlen((char *)pDataSrc) + 1;
...
// Swap an .ani file.  Fixes alignment errors
int ByteswapANIFile(studiohdr_t *pHdr, void *pDestBase, const void *pSrcBase,
                    const int fileSize) {
  ...
  // swap file header
  {
    u8 *pHeaderSrc = (u8 *)pSrcBase;
    u8 *pHeaderDest = (u8 *)pDestBase;
  ...
  // The animdesc_t header is always contained in the mdl file, but its data may
  // be in the mdl or the ani. When the data is contained in the mdl, the
  // animdesc index fields represent offsets from the location of the animdesc
  // header. When the data is in the ani, the index fields contain offsets from
  // the start of the animblock in which the animdesc data is contained.

  mstudioanimdesc_t *pAnimDesc = pHdr->pLocalAnimdesc(0);
  for (int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc) {
    // printf("anim %d : %d : %d\n", i, pAnimDesc->animblock,
    // pAnimDesc->sectionframes );
    ...
    if (pAnimDesc->sectionframes == 0) {
      ...
      // pAnimBlock->datastart, pAnimDesc->animindex ); Base address of the
      // animblock
      u8 *pBlockBaseSrc = (u8 *)pSrcBase + pAnimBlock->datastart;
      u8 *pBlockBaseDest = (u8 *)pDestBase + pAnimBlock->datastart;

      // Base address of the animation in the animblock
      u8 *pDataSrc = pBlockBaseSrc + pAnimDesc->animindex;
      u8 *pDataDest = pBlockBaseDest + pAnimDesc->animindex;
      ...
      for (int z = 0; z < numsections; ++z) {
        int block = pAnimDesc->pSection(z)->animblock;
        int index = pAnimDesc->pSection(z)->animindex;
        ...
        if (block != 0) {
          ...
          // Base address of the animation in the animblock
          u8 *pDataSrc = pBlockBaseSrc + index;
          u8 *pDataDest = pBlockBaseDest + index;

          ByteswapAnimData(pAnimDesc, z, pDataSrc, pDataDest);
        }
      }
    }
    ...
    if (pAnimDesc->animblockikruleindex) {
      pDataSrc = (u8 *)pBlockBaseSrc + pAnimDesc->animblockikruleindex;
      pDataDest = (u8 *)pBlockBaseDest + pAnimDesc->animblockikruleindex;
      FIXUP_OFFSETS(pData, pAnimDesc, animblockikruleindex)

      int numikrules = SrcNative(&pAnimDesc->numikrules);
      ByteswapIKRules(pHdrSrc, pAnimDesc->numikrules, pAnimDesc->numframes,
                      pDataSrc, pDataDest, fixedFileSize, fileSize);
    }

    /** LOCAL HIERARCHY **/

    if (pAnimDesc->localhierarchyindex) {
      pDataSrc = (u8 *)pBlockBaseSrc + pAnimDesc->localhierarchyindex;
      pDataDest = (u8 *)pBlockBaseDest + pAnimDesc->localhierarchyindex;
      DECLARE_OBJECT_POINTERS(pLocalHierarchy, pData, mstudiolocalhierarchy_t)

      // HACK: Since animdescs are already native, pre-swap
      // pAnimDesc->numlocalhierarchy here so the automatic swap inside
      // ITERATE_BLOCK will restore it
      int numlocalhierarchy = SrcNative(&pAnimDesc->numlocalhierarchy);
      ITERATE_BLOCK(pLocalHierarchy, numlocalhierarchy) {
        WriteObjects<mstudiolocalhierarchy_t>(pLocalHierarchyDest,
                                              pLocalHierarchySrc,
                                              pAnimDesc->numlocalhierarchy);

        /** COMPRESSED IK ERRORS **/

        if (pLocalHierarchy->localanimindex != 0) {
          // Calculate the number of ikerrors by converting the ikerror start
          // and end float values to frame numbers. (See the generation of these
          // values in simplify.cpp: ProcessIKRules()).
          int numFrames = pAnimDesc->numframes;
          float start = floorf(
              SrcNative(&pLocalHierarchy->start) * (numFrames - 1) + 0.5f);
          float end =
              floorf(SrcNative(&pLocalHierarchy->end) * (numFrames - 1) + 0.5f);
          int totalerror = (int)(end - start + 1);
          if (end >= numFrames) totalerror += 2;
          ...
          for (int idx = 0; idx < 6; ++idx) {
            if (pCompressed->offset[idx]) {
              u8 *pAnimvalueSrc =
                  pDataSrc + SrcNative(&pCompressed->offset[idx]);
              u8 *pAnimvalueDest =
                  pDataDest + SrcNative(&pCompressed->offset[idx]);
...
int ByteswapANI(studiohdr_t *pHdr, void *pDestBase, const void *pSrcBase,
  ...
  if (fixedFileSize != fileSize) {
    int finalSize =
        ByteswapANIFile(pHdr, pDestBase, pNewSrcBase, fixedFileSize);
  ...
  if (g_pCompressFunc && pHdr->numanimblocks >= 2 && fixedFileSize) {
    ...
    for (int i = 1; i < pHdr->numanimblocks; ++i) {
      ...
      void *pInput = (u8 *)pDestBase + pAnimBlock->datastart;
      int inputSize = pAnimBlock->dataend - pAnimBlock->datastart;

      pAnimBlock->datastart = pNewDest - pNewDestBase;
      ...
      padding = AlignValue(pNewDest - pNewDestBase, 2048);
      padding -= pNewDest - pNewDestBase;
      pNewDest += padding;

      pAnimBlock->dataend = pNewDest - pNewDestBase;
    }
...
int ByteswapMDLFile(void *pDestBase, void *pSrcBase, const int fileSize) {
    ...
    if (pStudioBone->procindex) {
      ...
      switch (index) {
        case STUDIO_PROC_AXISINTERP: {
          /** AXIS-INTERP BONES **/
          DECLARE_OBJECT_POINTERS(pAxisInterpBone, pData,
        ...
        case STUDIO_PROC_QUATINTERP: {
          /** QUAT-INTERP BONES **/
          DECLARE_OBJECT_POINTERS(pQuatInterpBone, pData,
          ...
          WriteObjects<mstudioquatinterpinfo_t>(
              pDataDest, pDataSrc, SrcNative(&pQuatInterpBone->numtriggers));
          break;
        }
  ...
  WriteObjects<mstudiobonecontroller_t>(pDataDest, pDataSrc,
                                        SrcNative(&pHdr->numbonecontrollers));

  /** ATTACHMENTS **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, localattachmentindex)
  WriteObjects<mstudioattachment_t>(pDataDest, pDataSrc,
                                    SrcNative(&pHdr->numlocalattachments));
  ...
  ITERATE_BLOCK(pHitboxSet, pHdr->numhitboxsets) {
    ...
    WriteObjects<mstudiobbox_t>(pDataDest, pDataSrc,
                                SrcNative(&pHitboxSet->numhitboxes));
  }
  ...
  ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim) {
    ...
    if (pAnimDesc->animblock == 0) {
      if (numsections == 0) {
        ...
        for (int i = 0; i < numsections; ++i) {
          if (pAnimDesc->pSection(i)->animblock == 0) {
            ...
            // Base address of the animation in the animblock
            u8 *src = (u8 *)pAnimDescSrc + index;
            u8 *dest = (u8 *)pAnimDescDest + index;

            ByteswapAnimData(pAnimDesc, i, src, dest);
          }
        }
      }
      ...
      if (pAnimDesc->ikruleindex) {
        ...
        ByteswapIKRules(pHdrSrc, pAnimDesc->numikrules, numframes, pDataSrc,
                        pDataDest, fixedFileSize, fileSize);
      }
      ...
      if (pAnimDesc->localhierarchyindex) {
        ...
        ITERATE_BLOCK(pLocalHierarchy, pAnimDesc->numlocalhierarchy) {
          WriteObjects<mstudiolocalhierarchy_t>(
              pLocalHierarchyDest, pLocalHierarchySrc,
              SrcNative(&pAnimDesc->numlocalhierarchy));

          /** COMPRESSED IK ERRORS **/

          if (pLocalHierarchy->localanimindex != 0) {
            // Calculate the number of ikerrors by converting the ikerror start
            // and end float values to frame numbers. (See the generation of
            // these values in simplify.cpp: ProcessIKRules()).
            int numFrames = SrcNative(&pAnimDesc->numframes);
            float start = floorf(
                SrcNative(&pLocalHierarchy->start) * (numFrames - 1) + 0.5f);
            float end = floorf(
                SrcNative(&pLocalHierarchy->end) * (numFrames - 1) + 0.5f);
            int totalerror = (int)(end - start + 1);
            if (end >= numFrames) totalerror += 2;
            ...
            for (int idx = 0; idx < 6; ++idx) {
              if (pCompressed->offset[idx]) {
                ...
                while (numerror < totalerror) {
                  ...
                  // Write the remaining animation values from this group
                  // (shorts)
                  WriteBuffer<short>(&pAnimvalueDest, &pAnimvalueSrc,
                                     pDestAnimvalue->num.valid);
  ...
  ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim) {
    if (pAnimDesc->nummovements) {
      ...
      WriteObjects<mstudiomovement_t>(pDataDest, pDataSrc,
                                      SrcNative(&pAnimDesc->nummovements));
    }
  }

  /** IK RULES (2nd pass) **/

  // This is required to support older models that had these lumps in a
  // different order - the model version didn't change, so there's no reliable
  // way to detect it.
  SET_INDEX_POINTERS(pData, pHdr, localanimindex)
  SET_OBJECT_POINTERS(pAnimDesc, pData, mstudioanimdesc_t)
  ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim) {
    if (pAnimDesc->ikruleindex) {
      ...
      u8 *pTest = (u8 *)pAnimDesc + SrcNative(&pAnimDesc->ikruleindex);
      SET_INDEX_POINTERS_FIXUP(pData, pAnimDesc, ikruleindex)
      if (pTest == pDataSrc) continue;
      ...
      ITERATE_BLOCK(pIKRule, pAnimDesc->numikrules) {
        ...
        // Calculate the number of ikerrors by converting the ikerror start and
        // end float values to frame numbers. (See the generation of these
        // values in simplify.cpp: ProcessIKRules()).
        int numFrames = SrcNative(&pAnimDesc->numframes);
        float start =
            floorf(SrcNative(&pIKRule->start) * (numFrames - 1) + 0.5f);
        float end = floorf(SrcNative(&pIKRule->end) * (numFrames - 1) + 0.5f);
        int totalerror = (int)(end - start + 1);
        if (end >= numFrames) totalerror += 2;
  ...
  ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim) {
    if (pAnimDesc->pZeroFrameData() != NULL) {
      ...
      u8 *pZeroFrameDest = (u8 *)pAnimDescDest + offset;
      ...
      ITERATE_BLOCK(pStudioBone, pHdr->numbones) {
        ...
        if (pStudioBone->flags & BONE_HAS_SAVEFRAME_ROT) {
          for (int j = 0; j < pAnimDesc->zeroframecount; j++) {
            WriteBuffer<int64_t>(&pZeroFrameDest, &pZeroFrameSrc, 1);
          }
        }
      }
    }
  ...
  ITERATE_BLOCK(pSequence, pHdr->numlocalseq) {
    ...
    if (pSequence->posekeyindex) {
      ...
      WriteBuffer<float>(pDataDest, pDataSrc,
                         SrcNative(&pSequence->groupsize[0]) +
                             SrcNative(&pSequence->groupsize[1]));
    }

    /** STUDIO EVENTS **/

    SET_INDEX_POINTERS_FIXUP(pData, pSequence, eventindex)
    WriteObjects<mstudioevent_t>(pDataDest, pDataSrc,
                                 SrcNative(&pSequence->numevents));

    /** AUTOLAYERS **/

    SET_INDEX_POINTERS_FIXUP(pData, pSequence, autolayerindex)
    WriteObjects<mstudioautolayer_t>(pDataDest, pDataSrc,
                                     SrcNative(&pSequence->numautolayers));
    ...
    if (pWeightSrc >= pDataSrc) {
      int numBoneWeights = (SrcNative(&pSequence->iklockindex) -
                            SrcNative(&pSequence->weightlistindex)) /
                           sizeof(float);
    ...
    WriteObjects<mstudioiklock_t>(pDataDest, pDataSrc,
                                  SrcNative(&pSequence->numiklocks));
    ...
    ITERATE_BLOCK(pModel, pBodypart->nummodels) {
      ...
      ITERATE_BLOCK(pMesh, pModel->nummeshes) {
        ...
        ITERATE_BLOCK(pFlex, pMesh->numflexes) {
          ...
          WriteObjects<mstudiovertanim_t>(pDataDest, pDataSrc,
                                          SrcNative(&pFlex->numverts));
        }
      }
      ...
      WriteObjects<mstudioeyeball_t>(pDataDest, pDataSrc,
                                     SrcNative(&pModel->numeyeballs));
    }
  }
  ...
  WriteObjects<mstudioflexdesc_t>(pDataDest, pDataSrc,
                                  SrcNative(&pHdr->numflexdesc));

  /** GLOBAL FLEX CONTROLLERS **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, flexcontrollerindex)
  WriteObjects<mstudioflexcontroller_t>(pDataDest, pDataSrc,
                                        SrcNative(&pHdr->numflexcontrollers));

  /** GLOBAL FLEX CONTROLLER REMAPS **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, flexcontrolleruiindex)
  WriteObjects<mstudioflexcontrollerui_t>(
      pDataDest, pDataSrc, SrcNative(&pHdr->numflexcontrollerui));

  // TODOKD: The remap indices after the flex controller remap headers need to
  // be swapped as well?
  ...
  ITERATE_BLOCK(pFlexRule, pHdr->numflexrules) {
    ...
    WriteObjects<mstudioflexop_t>(pDataDest, pDataSrc,
                                  SrcNative(&pFlexRule->numops));
  }
  ...
  ITERATE_BLOCK(pIKChain, pHdr->numikchains) {
    ...
    WriteObjects<mstudioiklink_t>(pDataDest, pDataSrc,
                                  SrcNative(&pIKChain->numlinks));
  }
  ...
  WriteObjects<mstudioiklock_t>(pDataDest, pDataSrc,
                                SrcNative(&pHdr->numlocalikautoplaylocks));

  /** MOUTH INFO **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, mouthindex)
  WriteObjects<mstudiomouth_t>(pDataDest, pDataSrc,
                               SrcNative(&pHdr->nummouths));

  /** POSE PARAMATERS **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, localposeparamindex)
  WriteObjects<mstudioposeparamdesc_t>(
      pDataDest, pDataSrc, SrcNative(&pHdr->numlocalposeparameters));

  /** MODEL GROUPS **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, includemodelindex)
  WriteObjects<mstudiomodelgroup_t>(pDataDest, pDataSrc,
                                    SrcNative(&pHdr->numincludemodels));

  /** ANIMBLOCK GROUP INFO **/

  SET_INDEX_POINTERS_FIXUP(pData, pHdr, animblockindex)
  WriteObjects<mstudioanimblock_t>(pDataDest, pDataSrc,
                                   SrcNative(&pHdr->numanimblocks));
  ...
  int nameOffset = 0;
  for (int i = 0; i < SrcNative(&pHdr->numtextures);
       ++i, ++pTexture, ++pTextureSrc) {
  ...
  WriteBuffer<short>(
      &pDataDest, &pDataSrc,
      SrcNative(&pHdr->numskinfamilies) * SrcNative(&pHdr->numskinref));
  ...
  if (pHdr->studiohdr2index) {
    ...
    // HACK: Pre-swap the constant "1" here so the automatic swap inside
    // ITERATE_BLOCK will restore it
    int studiohdr2ct = 1;
    ...
    ITERATE_BLOCK(pStudioHdr2, studiohdr2ct) {
      ...
      if (pStudioHdr2->numsrcbonetransform) {
        // Note, srcbonetransformindex is an offset from the start of the file,
        // not the start of the studiohdr2 as is the convention.  That's why the
        // macros can't be used here.
        pDataSrc =
            (u8 *)pHdrSrc + SrcNative(&pStudioHdr2->srcbonetransformindex);
        pDataDest =
            (u8 *)pHdrDest + SrcNative(&pStudioHdr2->srcbonetransformindex);
        WriteObjects<mstudiosrcbonetransform_t>(
            &pDataDest, &pDataSrc,
            SrcNative(&pStudioHdr2->numsrcbonetransform));
      }
  ...
  // NOTE: The block of data (above) swapped immediately before the string table
  // MUST update the pDataSrc pointer position, in order for this string table
  // offset calculation to work correctly. To update the pointer position, pass
  // the pointer address to WriteObjects().
  int offset = pDataSrc - (u8 *)pSrcBase;
  ...
  for (int i = 0; i < numCdTextures; ++i) {
    ...
    usize len = strlen(pPath);
    if (len >= 2 && (pPath[len - 1] == '\\' || pPath[len - 1] == '/') &&
        (pPath[len - 2] == '\\' || pPath[len - 2] == '/')) {
      pPath[len - 1] = '\0';
    }
  }
...
// called with the process func "UpdateSrcIndexFields" to update every datadesc
// field that was marked as FIELD_INDEX.  Any index that "points" across the
// shifted data location will have its value incremented or decremented by the
// appropriate number of bytes.  This misalignment issue only applies to some
// pre-EP2 models, as all newly compiled models are guaranteed to be aligned
// correctly.  Therefore, this function should not need to change when model
// formats are updated, as any model compiled with a new format will naturally
// not require this fixup function to be called.
void ProcessANIFields(void *pDataBase, datadescProcessFunc_t pfnProcessFunc) {
  studiohdr_t *pHdr = g_pHdr;
  u8 *pData = (u8 *)pDataBase;
  u8 *pSrcBase = (u8 *)g_pDataSrcBase;
  ...
  for (int i = 1; i < pHdr->numanimblocks; ++i, ++pAnimBlock) {
    ProcessFields(pSrcBase, pAnimBlock, &mstudioanimblock_t::m_DataMap,
                  pfnProcessFunc);
  }

  // Update the .mdl's animindex offsets into the .ani file
  // Don't bother with local hierarchy keys - they only exist in newer, properly
  // aligned models
  mstudioanimdesc_t *pAnimDesc = pHdr->pLocalAnimdesc(0);
  for (int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc) {
    mstudioanimblock_t *block = pHdr->pAnimBlock(pAnimDesc->animblock);
    u8 *pBlockBase = (u8 *)pSrcBase + block->datastart;

    ProcessFieldByName(pBlockBase, pAnimDesc, &mstudioanimdesc_t::m_DataMap,
                       "animindex", pfnProcessFunc);
    ProcessFieldByName(pBlockBase, pAnimDesc, &mstudioanimdesc_t::m_DataMap,
                       "animblockikruleindex", pfnProcessFunc);
  }
  ...
  for (int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc) {
    ...
    u8 *pBlockBase = (u8 *)pSrcBase + pAnimBlock->datastart;

    u8 *pData = pBlockBase + pAnimDesc->animindex;
    ...
    while (pAnimation) {
      ...
      if (pAnimation->nextoffset) {
        pData = (u8 *)pAnimation + SrcNative(&pAnimation->nextoffset);
    ...
    if (pAnimDesc->animblockikruleindex) {
      pData = (u8 *)pBlockBase + pAnimDesc->animblockikruleindex;

      mstudioikrule_t *pIKRule = (mstudioikrule_t *)pData;
      for (int z = 0; z < pAnimDesc->numikrules; ++z, ++pIKRule) {
...
void ProcessMDLFields(void *pDataBase, datadescProcessFunc_t pfnProcessFunc) {
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->boneindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numbones); ++i, ++pBone) {
    ...
    if (pBone->procindex) {
      pData = (u8 *)pBone + SrcNative(&pBone->procindex);
      ...
      switch (proctype) {
        case STUDIO_PROC_AXISINTERP:
          ...
          /** AXIS-INTERP BONES **/
          ProcessFields(pData, &mstudioaxisinterpbone_t::m_DataMap,
                        pfnProcessFunc);
          break;
        ...
        case STUDIO_PROC_QUATINTERP:
          ...
          /** QUAT-INTERP BONES **/
          ProcessFields(pData, &mstudioquatinterpbone_t::m_DataMap,
                        pfnProcessFunc);
          break;
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->localattachmentindex);
  mstudioattachment_t *pAttachment = (mstudioattachment_t *)pData;
  for (int i = 0; i < SrcNative(&pHdr->numlocalattachments);
       ++i, ++pAttachment) {
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->hitboxsetindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numhitboxsets); ++i, ++pHitboxSet) {
    ...
    /** HITBOXES **/
    pData = (u8 *)pHitboxSet + SrcNative(&pHitboxSet->hitboxindex);
    mstudiobbox_t *pBBox = (mstudiobbox_t *)pData;
    for (int z = 0; z < SrcNative(&pHitboxSet->numhitboxes); ++z, ++pBBox) {
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->localanimindex);
  mstudioanimdesc_t *pAnimDesc = (mstudioanimdesc_t *)pData;
  for (int i = 0; i < SrcNative(&pHdr->numlocalanim); ++i, ++pAnimDesc) {
    // Can't update animindex or animblockindex for animations that are in a
    // separate .ani file
    ProcessFieldByName(pAnimDesc, &mstudioanimdesc_t::m_DataMap, "baseptr",
                       pfnProcessFunc);
    ProcessFieldByName(pAnimDesc, &mstudioanimdesc_t::m_DataMap, "sznameindex",
                       pfnProcessFunc);
    ProcessFieldByName(pAnimDesc, &mstudioanimdesc_t::m_DataMap,
                       "movementindex", pfnProcessFunc);
    ProcessFieldByName(pAnimDesc, &mstudioanimdesc_t::m_DataMap, "ikruleindex",
                       pfnProcessFunc);
    ...
    if (pAnimDesc->animblock == 0) {
      ...
      ProcessFieldByName(pAnimDesc, &mstudioanimdesc_t::m_DataMap, "animindex",
                         pfnProcessFunc);

      pData = (u8 *)pAnimDesc + SrcNative(&pAnimDesc->animindex);
      mstudioanim_t *pAnimation = (mstudioanim_t *)pData;
      while (pAnimation) {
        ProcessFields(pAnimation, &mstudioanim_t::m_DataMap, pfnProcessFunc);
    ...
    if (pAnimDesc->ikruleindex) {
      pData = (u8 *)pAnimDesc + SrcNative(&pAnimDesc->ikruleindex);

      mstudioikrule_t *pIKRule = (mstudioikrule_t *)pData;
      for (int z = 0; z < SrcNative(&pAnimDesc->numikrules); ++z, ++pIKRule) {
        ProcessFields(pIKRule, &mstudioikrule_t::m_DataMap, pfnProcessFunc);
      }
    }
  }
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->localseqindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numlocalseq); ++i, ++pSequenceHdr) {
    ...
    pData = (u8 *)pHdr + SrcNative(&pSequenceHdr->eventindex);
    mstudioevent_t *pEvent = (mstudioevent_t *)pData;
    for (int z = 0; z < SrcNative(&pSequenceHdr->numevents); ++z, ++pEvent) {
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->bodypartindex);
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->flexdescindex);
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->flexcontrollerindex);
  mstudioflexcontroller_t *pFlexController = (mstudioflexcontroller_t *)pData;
  for (int i = 0; i < SrcNative(&pHdr->numflexcontrollers);
       ++i, ++pFlexController) {
    ProcessFields(pFlexController, &mstudioflexcontroller_t::m_DataMap,
                  pfnProcessFunc);
  }

  /** GLOBAL FLEX CONTROLLER REMAPS **/

  pData = (u8 *)pHdr + SrcNative(&pHdr->flexcontrolleruiindex);
  mstudioflexcontrollerui_t *pFlexControllerRemap =
      (mstudioflexcontrollerui_t *)pData;
  for (int i = 0; i < SrcNative(&pHdr->numflexcontrollerui);
       ++i, ++pFlexControllerRemap) {
    ProcessFields(pFlexControllerRemap, &mstudioflexcontrollerui_t::m_DataMap,
                  pfnProcessFunc);
  }

  /** FLEX RULES **/

  pData = (u8 *)pHdr + SrcNative(&pHdr->flexruleindex);
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->ikchainindex);
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->localposeparamindex);
  mstudioposeparamdesc_t *pPoseParam = (mstudioposeparamdesc_t *)pData;
  for (int i = 0; i < SrcNative(&pHdr->numlocalposeparameters);
       ++i, ++pPoseParam) {
    ProcessFields(pPoseParam, &mstudioposeparamdesc_t::m_DataMap,
                  pfnProcessFunc);
  }

  /** MODEL GROUPS **/

  pData = (u8 *)pHdr + SrcNative(&pHdr->includemodelindex);
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->textureindex);
  ...
  pData = (u8 *)pHdr + SrcNative(&pHdr->cdtextureindex);
  int *pIdx = (int *)pData;
  ...
  if (pHdr->studiohdr2index) {
    pData = (u8 *)pHdr + SrcNative(&pHdr->studiohdr2index);
...
// Data descriptions for u8 swapping - only needed
...
BEGIN_BYTESWAP_DATADESC(studiohdr_t)
  DEFINE_FIELD(id, FIELD_INTEGER), DEFINE_FIELD(version, FIELD_INTEGER),
      ...
      DEFINE_FIELD(activitylistversion, FIELD_INTEGER),  // initialization flag
                                                         // - have the sequences
                                                         // been indexed?
...
BEGIN_BYTESWAP_DATADESC(mstudiobone_t)
  DEFINE_INDEX(sznameindex, FIELD_INTEGER), DEFINE_FIELD(parent, FIELD_INTEGER),
      ...
      DEFINE_FIELD(pos, FIELD_VECTOR), DEFINE_FIELD(quat, FIELD_QUATERNION),
      ...
      DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(proctype, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudioaxisinterpbone_t)
  DEFINE_FIELD(control, FIELD_INTEGER), DEFINE_FIELD(axis, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudiojigglebone_t)
  DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(length, FIELD_FLOAT),
      ...
      DEFINE_FIELD(angleLimit, FIELD_FLOAT), DEFINE_FIELD(minYaw, FIELD_FLOAT),
      DEFINE_FIELD(maxYaw, FIELD_FLOAT), DEFINE_FIELD(yawFriction, FIELD_FLOAT),
      DEFINE_FIELD(yawBounce, FIELD_FLOAT), DEFINE_FIELD(minPitch, FIELD_FLOAT),
...
BEGIN_BYTESWAP_DATADESC(mstudioaimatbone_t)
  DEFINE_FIELD(parent, FIELD_INTEGER), DEFINE_FIELD(aim, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudioquatinterpinfo_t)
  DEFINE_FIELD(inv_tolerance, FIELD_FLOAT),
      DEFINE_FIELD(trigger, FIELD_QUATERNION), DEFINE_FIELD(pos, FIELD_VECTOR),
      DEFINE_FIELD(quat, FIELD_QUATERNION),
END_BYTESWAP_DATADESC()

BEGIN_BYTESWAP_DATADESC(mstudiobonecontroller_t)
  DEFINE_FIELD(bone, FIELD_INTEGER), DEFINE_FIELD(type, FIELD_INTEGER),
      DEFINE_FIELD(start, FIELD_FLOAT), DEFINE_FIELD(end, FIELD_FLOAT),
...
BEGIN_BYTESWAP_DATADESC(mstudiobbox_t)
  DEFINE_FIELD(bone, FIELD_INTEGER), DEFINE_FIELD(group, FIELD_INTEGER),
      DEFINE_FIELD(bbmin, FIELD_VECTOR), DEFINE_FIELD(bbmax, FIELD_VECTOR),
...
BEGIN_BYTESWAP_DATADESC(mstudiolocalhierarchy_t)
  DEFINE_FIELD(iBone, FIELD_INTEGER), DEFINE_FIELD(iNewParent, FIELD_INTEGER),
      DEFINE_FIELD(start, FIELD_FLOAT), DEFINE_FIELD(peak, FIELD_FLOAT),
      DEFINE_FIELD(tail, FIELD_FLOAT), DEFINE_FIELD(end, FIELD_FLOAT),
...
BEGIN_BYTESWAP_DATADESC(mstudioanimdesc_t)
  DEFINE_INDEX(baseptr, FIELD_INTEGER),
      DEFINE_INDEX(sznameindex, FIELD_INTEGER), DEFINE_FIELD(fps, FIELD_FLOAT),
      DEFINE_FIELD(flags, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudioanim_t)
  DEFINE_FIELD(bone, FIELD_CHARACTER), DEFINE_FIELD(flags, FIELD_CHARACTER),
...
BEGIN_BYTESWAP_DATADESC(mstudioikrule_t)
  DEFINE_FIELD(index, FIELD_INTEGER), DEFINE_FIELD(type, FIELD_INTEGER),
      DEFINE_FIELD(chain, FIELD_INTEGER), DEFINE_FIELD(bone, FIELD_INTEGER),
      DEFINE_FIELD(slot, FIELD_INTEGER), DEFINE_FIELD(height, FIELD_FLOAT),
      DEFINE_FIELD(radius, FIELD_FLOAT), DEFINE_FIELD(floor, FIELD_FLOAT),
      DEFINE_FIELD(pos, FIELD_VECTOR), DEFINE_FIELD(q, FIELD_QUATERNION),
      ...
      DEFINE_FIELD(start, FIELD_FLOAT), DEFINE_FIELD(peak, FIELD_FLOAT),
      DEFINE_FIELD(tail, FIELD_FLOAT), DEFINE_FIELD(end, FIELD_FLOAT),
      DEFINE_FIELD(unused3, FIELD_FLOAT), DEFINE_FIELD(contact, FIELD_FLOAT),
      DEFINE_FIELD(drop, FIELD_FLOAT), DEFINE_FIELD(top, FIELD_FLOAT),
...
BEGIN_BYTESWAP_DATADESC(mstudiomovement_t)
  DEFINE_FIELD(endframe, FIELD_INTEGER),
      DEFINE_FIELD(motionflags, FIELD_INTEGER), DEFINE_FIELD(v0, FIELD_FLOAT),
      DEFINE_FIELD(v1, FIELD_FLOAT), DEFINE_FIELD(angle, FIELD_FLOAT),
      DEFINE_FIELD(vector, FIELD_VECTOR), DEFINE_FIELD(position, FIELD_VECTOR),
END_BYTESWAP_DATADESC()
...
BEGIN_BYTESWAP_DATADESC(mstudioseqdesc_t)
  DEFINE_INDEX(baseptr, FIELD_INTEGER),
      ...
      DEFINE_FIELD(
          activity,
          FIELD_INTEGER),  // initialized at loadtime to game DLL values
      ...
      DEFINE_FIELD(fadeintime,
                   FIELD_FLOAT),  // ideal cross fate in time (0.2 default)
      DEFINE_FIELD(fadeouttime,
                   FIELD_FLOAT),  // ideal cross fade out time (0.2 default)
      ...
      DEFINE_FIELD(lastframe,
                   FIELD_FLOAT),  // frame that should generation EndOfSequence
      DEFINE_FIELD(nextseq, FIELD_INTEGER),  // auto advancing sequences
      DEFINE_FIELD(
          pose,
          FIELD_INTEGER),  // index of delta animation between end and nextseq
      DEFINE_FIELD(numikrules, FIELD_INTEGER),
      ...
      DEFINE_ARRAY(unused, FIELD_INTEGER, 7),  // remove/add as appropriate
                                               // (grow back to 8 ints on
                                               // version change!)
END_BYTESWAP_DATADESC()

BEGIN_BYTESWAP_DATADESC(mstudioevent_t)
  DEFINE_FIELD(cycle, FIELD_FLOAT), DEFINE_FIELD(event, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudioautolayer_t)
  DEFINE_FIELD(iSequence, FIELD_SHORT), DEFINE_FIELD(iPose, FIELD_SHORT),
      DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(start, FIELD_FLOAT),
      DEFINE_FIELD(peak, FIELD_FLOAT), DEFINE_FIELD(tail, FIELD_FLOAT),
...
BEGIN_BYTESWAP_DATADESC(mstudiobodyparts_t)
  DEFINE_INDEX(sznameindex, FIELD_INTEGER),
      DEFINE_FIELD(nummodels, FIELD_INTEGER), DEFINE_FIELD(base, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudiomodel_t)
  DEFINE_ARRAY(name, FIELD_CHARACTER, 64), DEFINE_FIELD(type, FIELD_INTEGER),
      ...
      DEFINE_INDEX(eyeballindex, FIELD_INTEGER), DEFINE_EMBEDDED(vertexdata),
      DEFINE_ARRAY(unused, FIELD_INTEGER, 8),
END_BYTESWAP_DATADESC()
...
BEGIN_BYTESWAP_DATADESC(mstudioflexcontroller_t)
  DEFINE_INDEX(sztypeindex, FIELD_INTEGER),
      ...
      DEFINE_FIELD(min, FIELD_FLOAT), DEFINE_FIELD(max, FIELD_FLOAT),
END_BYTESWAP_DATADESC()
...
BEGIN_BYTESWAP_DATADESC(mstudioflexrule_t)
  DEFINE_FIELD(flex, FIELD_INTEGER), DEFINE_FIELD(numops, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudioiklink_t)
  DEFINE_FIELD(bone, FIELD_INTEGER), DEFINE_FIELD(kneeDir, FIELD_VECTOR),
      DEFINE_FIELD(unused0, FIELD_VECTOR),
END_BYTESWAP_DATADESC()

BEGIN_BYTESWAP_DATADESC(mstudiomouth_t)
  DEFINE_FIELD(bone, FIELD_INTEGER), DEFINE_FIELD(forward, FIELD_VECTOR),
...
BEGIN_BYTESWAP_DATADESC(mstudiomesh_t)
  DEFINE_FIELD(material, FIELD_INTEGER),
      ...
      DEFINE_FIELD(meshid, FIELD_INTEGER), DEFINE_FIELD(center, FIELD_VECTOR),
      DEFINE_EMBEDDED(vertexdata), DEFINE_ARRAY(unused, FIELD_INTEGER, 8),
END_BYTESWAP_DATADESC()

BEGIN_BYTESWAP_DATADESC(mstudio_meshvertexdata_t)
  DEFINE_FIELD(modelvertexdata,
               FIELD_INTEGER),  // mstudio_modelvertexdata_t*
      DEFINE_ARRAY(numLODVertexes, FIELD_INTEGER, MAX_NUM_LODS),
END_BYTESWAP_DATADESC()

BEGIN_BYTESWAP_DATADESC(mstudioeyeball_t)
  DEFINE_INDEX(sznameindex, FIELD_INTEGER), DEFINE_FIELD(bone, FIELD_INTEGER),
      DEFINE_FIELD(org, FIELD_VECTOR), DEFINE_FIELD(zoffset, FIELD_FLOAT),
      DEFINE_FIELD(radius, FIELD_FLOAT), DEFINE_FIELD(up, FIELD_VECTOR),
      DEFINE_FIELD(forward, FIELD_VECTOR), DEFINE_FIELD(texture, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudioflex_t)
  DEFINE_FIELD(flexdesc, FIELD_INTEGER), DEFINE_FIELD(target0, FIELD_FLOAT),
      DEFINE_FIELD(target1, FIELD_FLOAT), DEFINE_FIELD(target2, FIELD_FLOAT),
      DEFINE_FIELD(target3, FIELD_FLOAT), DEFINE_FIELD(numverts, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudiovertanim_t)
  DEFINE_FIELD(index, FIELD_SHORT), DEFINE_FIELD(speed, FIELD_CHARACTER),
...
BEGIN_BYTESWAP_DATADESC(mstudiotexture_t)
  DEFINE_INDEX(sznameindex, FIELD_INTEGER), DEFINE_FIELD(flags, FIELD_INTEGER),
      DEFINE_FIELD(used, FIELD_INTEGER), DEFINE_FIELD(unused1, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(mstudiovertex_t)
  DEFINE_EMBEDDED(m_BoneWeights), DEFINE_FIELD(m_vecPosition, FIELD_VECTOR),
...
BEGIN_BYTESWAP_DATADESC(phyheader_t)
  DEFINE_FIELD(size, FIELD_INTEGER), DEFINE_FIELD(id, FIELD_INTEGER),
