...
// specifically the Xbox360. A new file is generated with an extra extension in
// the form of <filename>.360.<ext>
...
#undef ALIGN32
#undef ALIGN64
#define ALIGN4(a) a = (uint8_t *)((int)((uint8_t *)a + 3) & ~3)
#define ALIGN16(a) a = (uint8_t *)((int)((uint8_t *)a + 15) & ~15)
#define ALIGN32(a) a = (uint8_t *)((int)((uint8_t *)a + 31) & ~31)
#define ALIGN64(a) a = (uint8_t *)((int)((uint8_t *)a + 63) & ~63)
...
//----------------------------------------------------------------------
template <class T>
inline void WriteObjects(uint8_t **pOutputBuffer, uint8_t **pBaseData,
                         int objectCount = 1) {
  T tempObject;
...
//----------------------------------------------------------------------
template <class T>
inline void WriteObjects(T **pOutputBuffer, T **pBaseData,
                         int objectCount = 1) {
  T tempObject;
  for (int i = 0; i < objectCount; ++i) {
    Q_memcpy(&tempObject, *pBaseData, sizeof(T));
...
//----------------------------------------------------------------------
template <class T>
inline void WriteBuffer(uint8_t **pOutputBuffer, uint8_t **pBaseData,
                        int objectCount = 1) {
  T tempObject;
  for (int i = 0; i < objectCount; ++i) {
    Q_memcpy(&tempObject, *pBaseData, sizeof(T));
...
//----------------------------------------------------------------------
//	Declares src/dest uint8_t pointers and sets them to some index offset in
// the buffers.
//----------------------------------------------------------------------
#define DECLARE_INDEX_POINTERS(ptr, base, index)                           \
  uint8_t *ptr##Src = (uint8_t *)base##Src + SrcNative(&base##Src->index); \
  uint8_t *ptr##Dest = (uint8_t *)base##Dest + SrcNative(&base##Src->index);
...
//----------------------------------------------------------------------
//	Same as DECLARE_INDEX_POINTERS, but reuses existing uint8_t pointers.
//----------------------------------------------------------------------
#define SET_INDEX_POINTERS(ptr, base, index)                      \
  ptr##Src = (uint8_t *)base##Src + SrcNative(&base##Src->index); \
  ptr##Dest = (uint8_t *)base##Dest + SrcNative(&base##Src->index);
    ...
    if (ptr##Fixup != ptr##Src) {                                      \
      ...
      if (g_bVerbose)                                                  \
        Warning("Shifting misaligned data block by %d bytes at " #base \
                "->" #index "\n",                                      \
                nShiftBytes);                                          \
      int prevBytes = (uint8_t *)ptr##Src - (uint8_t *)g_pDataSrcBase; \
      Q_memmove((uint8_t *)ptr##Src + nShiftBytes, ptr##Src,           \
...
bool UpdateIndex(void *pBase, int *indexMember) {
  ...
  if (pBase < g_pFixPoint) {
    if ((uint8_t *)pBase + idx >= g_pFixPoint) {
      bUpdateIndex = true;
    }
  } else {
    if ((uint8_t *)pBase + idx < g_pFixPoint) {
...
void ProcessFields(void *pBaseAddress, void *pData, datamap_t *pDataMap,
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    ProcessField(pBaseAddress,
                 (uint8_t *)pData + pField->fieldOffset[TD_OFFSET_NORMAL],
                 pField, pfnProcessFunc);
  }
}
...
int ByteswapPHY(void *pDestBase, const void *pSrcBase, const int fileSize) {
  ...
  uint8_t *pSrc = (uint8_t *)pSrcBase;
  uint8_t *pDest = (uint8_t *)pDestBase;
  ...
  if (g_bNativeSrc) {
    ...
    pSrc = (uint8_t *)pSrcBase + pHdr->size;
    pDest = (uint8_t *)pDestBase + pHdr->size;
  ...
  int currPos = pSrc - (uint8_t *)pSrcBase;
  ...
  int newFileSize = pDest - (uint8_t *)pDestBase + remainingBytes;
...
int ByteswapVVD(void *pDestBase, const void *pSrcBase, const int fileSize) {
  ...
  uint8_t *pDataSrc = (uint8_t *)pSrcBase;
  uint8_t *pDataDest = (uint8_t *)pDestBase;
  ...
  int newFileSize = pDataDest - (uint8_t *)pDestBase;

  if (g_pCompressFunc) {
    void *pInput = (uint8_t *)pDestBase + sizeof(vertexFileHeader_t);
...
int ByteswapVTX(void *pDestBase, const void *pSrcBase, const int fileSize) {
  ...
  if (g_pCompressFunc) {
    void *pInput = (uint8_t *)pDestBase + sizeof(OptimizedModel::FileHeader_t);
...
void ByteswapAnimData(mstudioanimdesc_t *pAnimDesc, int section,
                      uint8_t *&pDataSrc, uint8_t *&pDataDest) {
  /** ANIMATIONS **/
  ...
  while (pAnimation) {
    if (pAnimation->flags &
      ...
      if (pAnimation->flags & STUDIO_ANIM_RAWROT) {
        int offset = (uint8_t *)pAnimation->pQuat48() - (uint8_t *)pAnimation;
        pDataSrc = (uint8_t *)pAnimationSrc + offset;
        pDataDest = (uint8_t *)pAnimationDest + offset;
      ...
      if (pAnimation->flags & STUDIO_ANIM_RAWROT2) {
        int offset = (uint8_t *)pAnimation->pQuat64() - (uint8_t *)pAnimation;
        pDataSrc = (uint8_t *)pAnimationSrc + offset;
        pDataDest = (uint8_t *)pAnimationDest + offset;
      ...
      if (pAnimation->flags & STUDIO_ANIM_RAWPOS) {
        int offset = (uint8_t *)pAnimation->pPos() - (uint8_t *)pAnimation;
        pDataSrc = (uint8_t *)pAnimationSrc + offset;
        pDataDest = (uint8_t *)pAnimationDest + offset;
      ...
      int offset = (uint8_t *)pAnimation->pRotV() - (uint8_t *)pAnimation;
      pDataSrc = (uint8_t *)pAnimationSrc + offset;
      pDataDest = (uint8_t *)pAnimationDest + offset;
      ...
      if (pAnimation->flags & STUDIO_ANIM_ANIMPOS) {
        int offset = (uint8_t *)pAnimation->pPosV() - (uint8_t *)pAnimation;
        pDataSrc = (uint8_t *)pAnimationSrc + offset;
        pDataDest = (uint8_t *)pAnimationDest + offset;
    ...
    if (pAnimation->nextoffset) {
      ...
      pAnimationSrc = (mstudioanim_t *)((uint8_t *)pAnimationSrc +
                                        SrcNative(&pAnimation->nextoffset));
      pAnimationDest = (mstudioanim_t *)((uint8_t *)pAnimationDest +
                                         SrcNative(&pAnimation->nextoffset));
...
int ByteswapIKRules(studiohdr_t *&pHdrSrc, int numikrules, int numFrames,
                    uint8_t *&pDataSrc, uint8_t *&pDataDest, int &fixedFileSize,
                    const int fileSize) {
    ...
    if (pIKRule->compressedikerrorindex) {
      ...
      for (int idx = 0; idx < 6; ++idx) {
        if (pCompressed->offset[idx]) {
          uint8_t *pAnimvalueSrc =
              pDataSrc + SrcNative(&pCompressed->offset[idx]);
          uint8_t *pAnimvalueDest =
...
int ByteswapANIFile(studiohdr_t *pHdr, void *pDestBase, const void *pSrcBase,
  ...
  // swap file header
  {
    uint8_t *pHeaderSrc = (uint8_t *)pSrcBase;
    uint8_t *pHeaderDest = (uint8_t *)pDestBase;
  ...
  for (int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc) {
    ...
    if (pAnimDesc->sectionframes == 0) {
      ...
      // animblock
      uint8_t *pBlockBaseSrc = (uint8_t *)pSrcBase + pAnimBlock->datastart;
      uint8_t *pBlockBaseDest = (uint8_t *)pDestBase + pAnimBlock->datastart;

      // Base address of the animation in the animblock
      uint8_t *pDataSrc = pBlockBaseSrc + pAnimDesc->animindex;
      uint8_t *pDataDest = pBlockBaseDest + pAnimDesc->animindex;
      ...
      for (int i = 0; i < numsections; ++i) {
        ...
        if (block != 0) {
          ...
          uint8_t *pBlockBaseSrc = (uint8_t *)pSrcBase + pAnimBlock->datastart;
          uint8_t *pBlockBaseDest =
              (uint8_t *)pDestBase + pAnimBlock->datastart;
          FIXUP_OFFSETS(pBlockBase, pAnimBlock, datastart)

          // Base address of the animation in the animblock
          uint8_t *pDataSrc = pBlockBaseSrc + index;
          uint8_t *pDataDest = pBlockBaseDest + index;
    ...
    if (pAnimDesc->animblockikruleindex) {
      pDataSrc = (uint8_t *)pBlockBaseSrc + pAnimDesc->animblockikruleindex;
      pDataDest = (uint8_t *)pBlockBaseDest + pAnimDesc->animblockikruleindex;
    ...
    if (pAnimDesc->localhierarchyindex) {
      pDataSrc = (uint8_t *)pBlockBaseSrc + pAnimDesc->localhierarchyindex;
      pDataDest = (uint8_t *)pBlockBaseDest + pAnimDesc->localhierarchyindex;
...
int ByteswapANI(studiohdr_t *pHdr, void *pDestBase, const void *pSrcBase,
  ...
  if (g_pCompressFunc && pHdr->numanimblocks >= 2 && fixedFileSize) {
    ...
    uint8_t *pNewDestBase = (uint8_t *)malloc(fixedFileSize);
    Q_memset(pNewDestBase, 0, fixedFileSize);
    uint8_t *pNewDest = pNewDestBase;
    ...
    int padding =
        AlignValue((unsigned int)pNewDest - (unsigned int)pNewDestBase, 2048);
    ...
    for (int i = 1; i < pHdr->numanimblocks; ++i) {
      ...
      void *pInput = (uint8_t *)pDestBase + pAnimBlock->datastart;
      int inputSize = pAnimBlock->dataend - pAnimBlock->datastart;

      pAnimBlock->datastart =
          (unsigned int)pNewDest - (unsigned int)pNewDestBase;
...
int ByteswapMDLFile(void *pDestBase, void *pSrcBase, const int fileSize) {
      ...
      if (numsections == 0) {
        ...
        for (int i = 0; i < numsections; ++i) {
          if (pAnimDesc->pSection(i)->animblock == 0) {
            ...
            // Base address of the animation in the animblock
            uint8_t *pDataSrc = (uint8_t *)pAnimDescSrc + index;
            uint8_t *pDataDest = (uint8_t *)pAnimDescDest + index;
  ...
  ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim) {
    if (pAnimDesc->ikruleindex) {
      ...
      uint8_t *pTest =
          (uint8_t *)pAnimDesc + SrcNative(&pAnimDesc->ikruleindex);
      SET_INDEX_POINTERS_FIXUP(pData, pAnimDesc, ikruleindex)
  ...
  ITERATE_BLOCK(pAnimDesc, pHdr->numlocalanim) {
    if (pAnimDesc->pZeroFrameData() != NULL) {
      int offset = pAnimDesc->pZeroFrameData() - (uint8_t *)pAnimDesc;

      // Base address of the animation in the animblock
      uint8_t *pZeroFrameSrc = (uint8_t *)pAnimDescSrc + offset;
      uint8_t *pZeroFrameDest = (uint8_t *)pAnimDescDest + offset;
  ...
  if (pHdr->studiohdr2index) {
    ...
    ITERATE_BLOCK(pStudioHdr2, studiohdr2ct) {
      ...
      if (pStudioHdr2->numsrcbonetransform) {
        ...
        pDataSrc =
            (uint8_t *)pHdrSrc + SrcNative(&pStudioHdr2->srcbonetransformindex);
        pDataDest = (uint8_t *)pHdrDest +
                    SrcNative(&pStudioHdr2->srcbonetransformindex);
  ...
  int offset = pDataSrc - (uint8_t *)pSrcBase;
...
void ProcessANIFields(void *pDataBase, datadescProcessFunc_t pfnProcessFunc) {
  ...
  uint8_t *pData = (uint8_t *)pDataBase;
  uint8_t *pSrcBase = (uint8_t *)g_pDataSrcBase;
  ...
  for (int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc) {
    mstudioanimblock_t *pAnimBlock = pHdr->pAnimBlock(pAnimDesc->animblock);
    uint8_t *pBlockBase = (uint8_t *)pSrcBase + pAnimBlock->datastart;
  ...
  for (int i = 0; i < pHdr->numlocalanim; ++i, ++pAnimDesc) {
    ...
    uint8_t *pData = pBlockBase + pAnimDesc->animindex;
    ...
    while (pAnimation) {
      ...
      if (pAnimation->nextoffset) {
        pData = (uint8_t *)pAnimation + SrcNative(&pAnimation->nextoffset);
    ...
    if (pAnimDesc->animblockikruleindex) {
      pData = (uint8_t *)pBlockBase + pAnimDesc->animblockikruleindex;
...
void ProcessMDLFields(void *pDataBase, datadescProcessFunc_t pfnProcessFunc) {
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->boneindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numbones); ++i, ++pBone) {
    ...
    if (pBone->procindex) {
      pData = (uint8_t *)pBone + SrcNative(&pBone->procindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->localattachmentindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->hitboxsetindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numhitboxsets); ++i, ++pHitboxSet) {
    ...
    /** HITBOXES **/
    pData = (uint8_t *)pHitboxSet + SrcNative(&pHitboxSet->hitboxindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->localanimindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numlocalanim); ++i, ++pAnimDesc) {
    ...
    if (pAnimDesc->animblock == 0) {
      ...
      pData = (uint8_t *)pAnimDesc + SrcNative(&pAnimDesc->animindex);
      mstudioanim_t *pAnimation = (mstudioanim_t *)pData;
      while (pAnimation) {
        ProcessFields(pAnimation, &mstudioanim_t::m_DataMap, pfnProcessFunc);
    ...
    if (pAnimDesc->ikruleindex) {
      pData = (uint8_t *)pAnimDesc + SrcNative(&pAnimDesc->ikruleindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->localseqindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numlocalseq); ++i, ++pSequenceHdr) {
    ...
    pData = (uint8_t *)pHdr + SrcNative(&pSequenceHdr->eventindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->bodypartindex);
  ...
  for (int i = 0; i < SrcNative(&pHdr->numbodyparts); ++i, ++pBodypart) {
    ...
    uint8_t *pData = (uint8_t *)pBodypart + SrcNative(&pBodypart->modelindex);
    ...
    for (int i = 0; i < SrcNative(&pBodypart->nummodels); ++i, ++pModel) {
      ...
      pData = (uint8_t *)pModel + SrcNative(&pModel->meshindex);
      ...
      for (int i = 0; i < SrcNative(&pModel->nummeshes); ++i, ++pMesh) {
        ...
        pData = (uint8_t *)pMesh + SrcNative(&pMesh->flexindex);
      ...
      pData = (uint8_t *)pModel + SrcNative(&pModel->eyeballindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->flexdescindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->flexcontrollerindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->flexcontrolleruiindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->flexruleindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->ikchainindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->localposeparamindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->includemodelindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->textureindex);
  ...
  pData = (uint8_t *)pHdr + SrcNative(&pHdr->cdtextureindex);
  int *pIdx = (int *)pData;
  ...
  if (pHdr->studiohdr2index) {
    pData = (uint8_t *)pHdr + SrcNative(&pHdr->studiohdr2index);
    ...
    for (int i = 0; i < 1; ++i, ++pStudioHdr2) {
      ...
      pData = (uint8_t *)pHdr + SrcNative(&pStudioHdr2->srcbonetransformindex);
