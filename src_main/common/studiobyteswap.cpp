...
#undef ALIGN16
#define ALIGN16(a) a = (u8 *)((int)((u8 *)a + 15) & ~15)
#undef ALIGN32
#define ALIGN32(a) a = (u8 *)((int)((u8 *)a + 31) & ~31)
#undef ALIGN64
#define ALIGN64(a) a = (u8 *)((int)((u8 *)a + 63) & ~63)
...
//----------------------------------------------------------------------
template <class T>
inline void WriteObjects(u8 **pOutputBuffer, u8 **pBaseData,
                         int objectCount = 1) {
  T tempObject;
...
//----------------------------------------------------------------------
template <class T>
inline void WriteBuffer(u8 *pOutputBuffer, u8 *pBaseData, int objectCount = 1) {
  T tempObject;
  for (int i = 0; i < objectCount; ++i) {
    Q_memcpy(&tempObject, pBaseData, sizeof(T));
    g_Swap.SwapBufferToTargetEndian(&tempObject, &tempObject);
    ...
    if (ptr##Fixup != ptr##Src) {                                      \
      ...
      if (g_bVerbose)                                                  \
        Warning("Shifting misaligned data block by %d bytes at " #base \
                "->" #index "\n",                                      \
                nShiftBytes);                                          \
      int prevBytes = (u8 *)ptr##Src - (u8 *)g_pDataSrcBase;           \
      ...
      if (numsections == 0) {
        ...
        for (int i = 0; i < numsections; ++i) {
          if (pAnimDesc->pSection(i)->animblock == 0) {
            ...
            // Base address of the animation in the animblock
            u8 *pDataSrc = (u8 *)pAnimDescSrc + index;
            u8 *pDataDest = (u8 *)pAnimDescDest + index;
