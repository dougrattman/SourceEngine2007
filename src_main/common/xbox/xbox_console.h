...
#define XBXCONSOLE_INTERFACE SOURCE_API_EXPORT
#define XBXCONSOLE_OVERLOAD SOURCE_API_GLOBAL_EXPORT
#else
#define XBXCONSOLE_INTERFACE SOURCE_API_IMPORT
#define XBXCONSOLE_OVERLOAD SOURCE_API_GLOBAL_IMPORT
#endif
...
class IXboxConsole {
 public:
  ...
  virtual int SetProfileAttributes(const char *pProfileName, int numCounters,
                                   const char *names[],
                                   unsigned int colors[]) = 0;
  virtual void SetProfileData(const char *pProfileName, int numCounters,
  ...
  virtual int MaterialList(int nMaterials,
                           const xMaterialList_t *pXMaterialList) = 0;
  virtual int TextureList(int nTextures,
                          const xTextureList_t *pXTextureList) = 0;
  virtual int SoundList(int nSounds, const xSoundList_t *pXSoundList) = 0;
  virtual int MapInfo(const xMapInfo_t *pXMapInfo) = 0;
  virtual int AddCommands(int numCommands, const char *commands[],
                          const char *help[]) = 0;
};
...
class CXboxConsole : public IXboxConsole {
 public:
  ...
  int SetProfileAttributes(const char *pProfileName, int numCounters,
                           const char *names[], unsigned int colors[]);
  void SetProfileData(const char *pProfileName, int numCounters,
...
#define XBX_SendRemoteCommand \
  if (!g_pXboxConsole)        \
    ;                         \
  else                        \
    g_pXboxConsole->SendRemoteCommand
#define XBX_DebugString \
  if (!g_pXboxConsole)  \
    ;                   \
  else                  \
    g_pXboxConsole->DebugString
#define XBX_IsConsoleConnected \
  (!g_pXboxConsole) ? false : g_pXboxConsole->IsConsoleConnected
#define XBX_InitConsoleMonitor \
  if (!g_pXboxConsole)         \
    ;                          \
  else                         \
    g_pXboxConsole->InitConsoleMonitor
#define XBX_DisconnectConsoleMonitor \
  if (!g_pXboxConsole)               \
    ;                                \
  else                               \
    g_pXboxConsole->DisconnectConsoleMonitor
#define XBX_FlushDebugOutput \
  if (!g_pXboxConsole)       \
    ;                        \
  else                       \
    g_pXboxConsole->FlushDebugOutput
#define XBX_GetXboxName (!g_pXboxConsole) ? false : g_pXboxConsole->GetXboxName
#define XBX_CrashDump  \
  if (!g_pXboxConsole) \
    ;                  \
  else                 \
    g_pXboxConsole->CrashDump
#define XBX_DumpDllInfo \
  if (!g_pXboxConsole)  \
    ;                   \
  else                  \
    g_pXboxConsole->DumpDllInfo
#define XBX_OutputDebugString \
  if (!g_pXboxConsole)        \
    ;                         \
  else                        \
    g_pXboxConsole->OutputDebugString
#define XBX_IsDebuggerPresent \
  (!g_pXboxConsole) ? false : g_pXboxConsole->IsDebuggerPresent

#define XBX_rSetProfileAttributes \
  (!g_pXboxConsole) ? 0 : g_pXboxConsole->SetProfileAttributes
#define XBX_rSetProfileData \
  if (!g_pXboxConsole)      \
    ;                       \
  else                      \
    g_pXboxConsole->SetProfileData
