...
abstract_class IRunGameEngine : public IBaseInterface {
 public:
  ...
  virtual bool RunEngine(const char *gameDir,
                         const char *commandLineParams) = 0;
  ...
  // returns the engine build number and mod version string for server
  // versioning
  virtual unsigned int GetEngineBuildNumber() = 0;
  virtual const char *GetProductVersionString() = 0;

  // new interface to RunEngine (done so we don't have to roll the interface
  // version)
