...
// Half-Life 2
defaultConfigInfo_t HL2Info = {"Half-Life 2",
                               "hl2",
                               "halflife2.fgd",
                               "half-life 2",
                               "info_player_start",
...
defaultConfigInfo_t HL2DMInfo = {"Half-Life 2: Deathmatch",
                                 "hl2mp",
                                 "hl2mp.fgd",
                                 "half-life 2 deathmatch",
...
defaultConfigInfo_t DODInfo = {"Day of Defeat: Source",
                               "dod",
                               "dod.fgd",
                               "day of defeat source",
                               "info_player_allies",
...
defaultConfigInfo_t Episode1Info = {
    "Half-Life 2: Episode One",     "episodic",          "halflife2.fgd",
    "half-life 2 episode one",      "info_player_start", "hl2.exe",
    GetAppSteamAppId(k_App_HL2_EP1)};

// Half-Life 2 Episode 2
defaultConfigInfo_t Episode2Info = {"Half-Life 2: Episode Two",
                                    "ep2",
                                    "halflife2.fgd",
...
// Team Fortress 2
defaultConfigInfo_t TF2Info = {"Team Fortress 2",
                               "tf",
                               "tf.fgd",
                               "team fortress 2",
...
// Portal
defaultConfigInfo_t PortalInfo = {"Portal",
                                  "portal",
                                  "portal.fgd",
                                  "portal",
                                  "info_player_start",
...
CGameConfigManager::CGameConfigManager(void)
    : m_pData(NULL), m_LoadStatus(LOADSTATUS_NONE) {
  // Start with default directory
  GetModuleFileName((HINSTANCE)GetModuleHandle(NULL), m_szBaseDirectory,
                    sizeof(m_szBaseDirectory));
  Q_StripLastDir(m_szBaseDirectory,
                 sizeof(m_szBaseDirectory));  // Get rid of the filename.
  ...
  if (!bLoaded) {
    ...
    if (ConvertGameConfigsINI() == false) {
      ...
      if (CreateAllDefaultConfigs()) {
        ...
        if (!bRecursiveCall) return LoadConfigsInternal(baseDir, true);
...
void CGameConfigManager::UpdateConfigsInternal() {
  ...
  // Check for version first.  If the version is up to date, it is assumed to be
  // accurate
  if (IsConfigCurrent()) return;
...
void CGameConfigManager::VersionConfig() {
  ...
  if (pKeyVersion) {
    if (pKeyVersion->GetInt() == m_eSDKEpoch) return;
...
bool CGameConfigManager::IsConfigCurrent() {
  ...
  if (nVersion == m_eSDKEpoch) return true;

  return false;
}

//-----------------------------------------------------------------------------
// Purpose: Get the base path for a default config's install (handling steam's
// paths)
//-----------------------------------------------------------------------------
void CGameConfigManager::GetRootGameDirectory(char *out, size_t outLen,
                                              const char *rootDir,
                                              const char *steamDir) {
  // Steam install is different
  if (g_pFullFileSystem && g_pFullFileSystem->IsSteam() &&
      (steamDir != NULL && steamDir[0] != NULL)) {
...
//-----------------------------------------------------------------------------
// Purpose: Get the base path for a default config's content sources (handling
// steam's paths)
//-----------------------------------------------------------------------------
void CGameConfigManager::GetRootContentDirectory(char *out, size_t outLen,
                                                 const char *rootDir) {
  // Steam install is different
  if (g_pFullFileSystem && g_pFullFileSystem->IsSteam()) {
...
// NOTE: This function could use some re-write, it can't handle non-retail paths
// well
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CGameConfigManager::AddDefaultConfig(const defaultConfigInfo_t &info,
                                          KeyValues *out,
  ...
  if (hammerBlock == NULL) return false;
...
bool CGameConfigManager::ConvertGameConfigsINI() {
  ...
  // Load our INI file
  int nNumConfigs =
      GetPrivateProfileInt("Configs", "NumConfigs", 0, iniFilePath);
  if (nNumConfigs <= 0) return false;
  ...
  for (int nConfig = 0; nConfig < nNumConfigs; nConfig++) {
    // Each came configuration is stored in a different section, named
    // "GameConfig0..GameConfigN". If the "Name" key exists in this section, try
    // to load the configuration from this section.
    sprintf(szSectionName, "GameConfig%d", nConfig);

    int nCount = GetPrivateProfileString(szSectionName, "Name", "", textBuffer,
                                         sizeof(textBuffer), iniFilePath);
    if (nCount > 0) {
      // Make a new section
      KeyValues *subGame = gamesBlock->CreateNewKey();
      subGame->SetName(textBuffer);

      GetPrivateProfileString(szSectionName, "ModDir", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      ...
      do {
        ...
        nStrlen =
            GetPrivateProfileString(szSectionName, szGameData, "", textBuffer,
                                    sizeof(textBuffer), iniFilePath);
      ...
      GetPrivateProfileString(szSectionName, "DefaultTextureScale", "1",
                              textBuffer, sizeof(textBuffer), iniFilePath);
      ...
      GetPrivateProfileString(szSectionName, "DefaultSolidEntity", "",
                              textBuffer, sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("DefaultSolidEntity", textBuffer);

      GetPrivateProfileString(szSectionName, "DefaultPointEntity", "",
                              textBuffer, sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("DefaultPointEntity", textBuffer);

      GetPrivateProfileString(szSectionName, "BSP", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("BSP", textBuffer);

      GetPrivateProfileString(szSectionName, "Vis", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("Vis", textBuffer);

      GetPrivateProfileString(szSectionName, "Light", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("Light", textBuffer);

      GetPrivateProfileString(szSectionName, "GameExeDir", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("GameExeDir", textBuffer);

      GetPrivateProfileString(szSectionName, "MapDir", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("MapDir", textBuffer);

      GetPrivateProfileString(szSectionName, "BSPDir", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("BSPDir", textBuffer);

      GetPrivateProfileString(szSectionName, "CordonTexture", "", textBuffer,
                              sizeof(textBuffer), iniFilePath);
      hammerBlock->SetString("CordonTexture", textBuffer);

      GetPrivateProfileString(szSectionName, "MaterialExcludeCount", "0",
                              textBuffer, sizeof(textBuffer), iniFilePath);
      ...
      for (i = 0; i < materialExcludeCount; i++) {
        ...
        GetPrivateProfileString(szSectionName, excludeDir, "", textBuffer,
                                sizeof(textBuffer), iniFilePath);
...
const char *CGameConfigManager::GetRootDirectory() {
  ...
  if (path[0] == 0) {
    ...
    if (g_pFullFileSystem && g_pFullFileSystem->IsSteam()) {
      Q_StripLastDir(path,
                     sizeof(path));  // // Get rid of the 'orangebox' directory
      Q_StripTrailingSlash(path);
      Q_StripLastDir(path, sizeof(path));  // Get rid of the 'bin' directory
      Q_StripTrailingSlash(path);
      Q_StripLastDir(path,
                     sizeof(path));  // Get rid of the 'sourcesdk' directory
      Q_StripTrailingSlash(path);
    }
  }
  return path;
...
KeyValues *CGameConfigManager::GetGameSubBlock(const char *keyName) {
  ...
  KeyValues *pGameBlock = GetGameBlock();
  if (pGameBlock == NULL) return NULL;
...
bool CGameConfigManager::ResetConfigs(const char *baseDir /*= NULL*/) {
  ...
  // Delete the file
  if (unlink(szPath)) return false;

  // Load the file again (causes defaults to be created)
  if (LoadConfigsInternal(baseDir, false) == false) return false;
