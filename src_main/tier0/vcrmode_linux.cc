...
VCRMode_t g_OldVCRMode = (VCRMode_t)-1;  // Stored temporarily between
...
// VCR interface.
static int VCR_Start(char const *pFilename, bool bRecord,
                     IVCRHelpers *pHelpers) {
...
void VCR_Hook_RecordGameMsg(unsigned int uMsg, unsigned int wParam,
                            long lParam) {
  if (g_VCRMode == VCR_Record) {
...
bool VCR_Hook_PlaybackGameMsg(unsigned int &uMsg, unsigned int &wParam,
                              long &lParam) {
...
void VCR_GenericRecord(const char *pEventName, const void *pData, int len) {
  ...
  if (pEventName) {
    ...
    if (nameLen >= 255) {
      VCR_Error("VCR_GenericRecord( %s ): nameLen too long (%d)", pEventName,
                nameLen);
      return;
    }
  }
