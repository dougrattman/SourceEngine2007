...
// we use malloc & free internally in our validation code; turn off the
// deprecation #defines
#undef malloc
#undef free
...
CValidator::CValidator() {
  ...
  // Mark all memory blocks as unclaimed, prior to starting the validation
  // process
  CMemBlockHdr *pMemBlockHdr = CMemBlockHdr::PMemBlockHdrFirst();
  pMemBlockHdr =
      pMemBlockHdr->PMemBlockHdrNext();  // Head is just a placeholder
...
// Input:	pchType -		The caller's type (typically a class
// name)
//			pvObj -			The caller (typically an object
//pointer) 			pchName -		The caller's individual name (typically a member
//var of another class)
//-----------------------------------------------------------------------------
void CValidator::Push(tchar *pchType, void *pvObj, tchar *pchName) {
  ...
  pValObjectNew->Init(pchType, pvObj, pchName, m_pValObjectCur,
                      m_pValObjectLast);
  m_pValObjectLast = pValObjectNew;
  if (NULL == m_pValObjectFirst) m_pValObjectFirst = pValObjectNew;
...
// Input:	cubThreshold -		Only render object whose children have at
// least
//								cubThreshold bytes
//allocated
//-----------------------------------------------------------------------------
void CValidator::RenderObjects(int cubThreshold) {
  ...
  while (NULL != pValObject) {
    if (pValObject->CubMemTree() >= cubThreshold) {
      for (int ich = 0; ich < pValObject->NLevel(); ich++) ConMsg(2, _T(" "));

      ConMsg(2, _T("%s at 0x%x--> %d blocks = %d bytes\n"),
             pValObject->PchType(), pValObject->PvObj(),
             pValObject->CpubMemTree(), pValObject->CubMemTree());
    }
...
CValObject *CValidator::FindObject(void *pvObj) {
  ...
  while (NULL != pValObject) {
    pValObjectNext = pValObject->PValObjectNext();
    if (pvObj == pValObject->PvObj()) return pValObject;
                                  ...
                                  // that are also present in the other.
{
  ...
  while (NULL != pValObject) {
    pValObjectNext = pValObject->PValObjectNext();
    pValObject->SetBNewSinceSnapshot(
        pOtherValidator->FindObject(pValObject->PvObj()) == NULL);
