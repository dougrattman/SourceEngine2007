...
CVProfNode *CVProfNode::GetSubNode(const char *pszName, int detailLevel,
...
int g_TotalFrames;
map<const char *, size_t> g_TimeSumsMap;
vector<TimeSums_t> g_TimeSums;
CVProfNode *g_pStartNode;
const char *g_pszSumNode;
    ...
    if (bAverageAndCountOnly) {
      Msg("  %10.3f %6.2f      %6d",
          (pNode->GetTotalCalls() > 0)
              ...
              : 0,
          (pNode->GetTotalCalls() > 0)
              ? iterTimeLessChildren->second / (double)NumFramesSampled()
              : 0,
          pNode->GetTotalCalls());
    } else {
      Msg("  %10.3f %6.2f      %10.3f %6.2f      %10.3f %6.2f   %6d %6.2f",
          ...
          (pNode->GetTotalCalls() > 0)
              ...
              : 0,
          (pNode->GetTotalCalls() > 0)
              ? iterTimeLessChildren->second / (double)pNode->GetTotalCalls()
              : 0,
...
//-------------------------------------
static void DumpSorted(const char *pszHeading, double totalTime,
...
void CVProfile::OutputReport(int type, const char *pszStartNode,
                             int budgetGroupID) {
...
int CVProfile::AddBudgetGroupName(const char *pBudgetGroupName,
                                  int budgetFlags) {
  MEM_ALLOC_CREDIT();
  char *pNewString = new char[strlen(pBudgetGroupName) + 1];
...
int *CVProfile::FindOrCreateCounter(const char *pName,
  ...
  MEM_ALLOC_CREDIT();
  char *pNewName = new char[strlen(pName) + 1];
...
const char *CVProfile::GetCounterNameAndValue(int index, int &val) const {
  Assert(index >= 0 && index < m_NumCounters);
