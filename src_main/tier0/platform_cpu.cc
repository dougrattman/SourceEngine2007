...
#error Please, define CPU architecture for your CPU in tier0/cpu.cc
    ...
    memcpy(vendor_id + sizeof(registers[0]) + sizeof(registers[1]),  //-V119
...
i64 CalculateClockSpeed() {
  ...
  i64 wait_time = Plat_PerformanceFrequency() >> scale;
  ...
  } while (end_count.QuadPart - start_count.QuadPart < wait_time);
  end_cycle.Sample();
