...
class CValidateAlloc : public IMemAlloc {
 public:
  ...
  void *CrtSetReportFile(int nRptType, void *file);
 ...
 private:
  ...
  // Updates stats
  void RegisterAllocation(const char *pFileName, int nLine, size_t nLogicalSize,
                          size_t nActualSize, unsigned nTime);
  void RegisterDeallocation(const char *pFileName, int nLine,
...
inline CValidateAlloc::HeapSuffix_t *CValidateAlloc::Suffix(
  ...
  return reinterpret_cast<HeapSuffix_t *>((unsigned char *)(pPrefix + 1) +
                                          pPrefix->m_nSize);
}
...
void *CValidateAlloc::Realloc(void *pMem, size_t nSize, const char *pFileName,
                              int nLine) {
  Assert(heapchk() == _HEAPOK);
  Assert(CrtCheckMemory());
  HeapPrefix_t *pHeap = PrefixFromAllocation(pMem);
...
int CValidateAlloc::CrtIsValidPointer(const void *pMem, unsigned int size,
                                      int access) {
  const HeapPrefix_t *pHeap = PrefixFromAllocation(pMem);
  return g_pActualAlloc->CrtIsValidPointer(pHeap, size, access);
}

int CValidateAlloc::CrtCheckMemory() {
...
void *CValidateAlloc::CrtSetReportFile(int nRptType, void *file) {
...
int CValidateAlloc::CrtDbgReport(int nRptType, const char *szFile, int nLine,
                                 const char *szModule, const char *pMsg) {
...
// Updates stats
void CValidateAlloc::RegisterAllocation(const char *pFileName, int nLine,
                                        size_t nLogicalSize, size_t nActualSize,
                                        unsigned nTime) {
...
void CValidateAlloc::RegisterDeallocation(const char *pFileName, int nLine,
                                          size_t nLogicalSize,
                                          size_t nActualSize, unsigned nTime) {
