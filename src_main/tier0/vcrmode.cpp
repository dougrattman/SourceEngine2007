...
#include <cstring>
#include <ctime>
#include "inputsystem/inputenums.h"  // FIXME: We totally have a bad tier dependency here
#include "tier0/dbg.h"
...
// wrappers.
inline DWORD Wrap_WaitForSingleObject(HANDLE handle, DWORD milliseconds) {
  return WaitForSingleObject(handle, milliseconds);
}

inline DWORD Wrap_WaitForMultipleObjects(DWORD handles_count,
                                         const HANDLE *handles,
                                         BOOL is_wait_all, DWORD milliseconds) {
...
void VCR_Debug(const char *message, ...) {
  va_list marker;
...
struct CVCRThreadInfo {
  ...
  CVCRThreadInfo(DWORD the_thread_id, HANDLE the_wait_event,
                 bool the_is_enabled)
...
uint16_t GetCurrentVCRThreadIndex() {
  const DWORD tid = GetCurrentThreadId();

  for (uint16_t i = 0; i < g_nVCRThreads; i++) {
...
// Internal functions.
static void VCR_Error(const char *error, ...) {
#ifdef _DEBUG
  // Figure out which thread we're in, for the debugger.
  uint32_t current_thread_index = GetCurrentVCRThreadIndex();
...
static void VCR_RuntimeAssertFn(bool assert, char const *message) {
  if (!assert) {
...
static void VCR_WriteEvent(VCREvent event) {
  uint8_t the_event = static_cast<uint8_t>(event);
  const uint16_t tid = GetCurrentVCRThreadIndex();
...
static BOOL VCR_StartWrite(char const *vcr_file_path) {
  ...
  if (g_pVCRFile) {
    // Write the version.
    uint32_t version = VCRFILE_VERSION;
...
static BOOL VCR_StartRead(char const *vcr_file_path) {
...
// VCR interface.
static BOOL VCR_Start(char const *vcr_file_path, bool should_record,
...
static void VCR_SyncToken(char const *pToken) {
  ...
  if (g_VCRMode == VCR_Record) {
    ...
    len = static_cast<uint8_t>(intLen);
    ...
    VCR_RuntimeAssert(len == static_cast<uint8_t>(strlen(pToken)));
...
static int VCR_Hook_PeekMessage(struct tagMSG *msg, void *hWnd,
...
static int VCR_Hook_recvfrom(int s, char *buf, int len, int flags,
...
static long VCR_Hook_RegOpenKeyEx(void *hKey, const char *lpSubKey,
...
static long VCR_Hook_RegSetValueEx(void *hKey, char const *lpValueName,
                                   unsigned long Reserved, unsigned long dwType,
                                   unsigned char const *lpData,
...
static long VCR_Hook_RegQueryValueEx(void *hKey, char const *lpValueName,
...
static long VCR_Hook_RegCreateKeyEx(void *hKey, char const *lpSubKey,
                                    unsigned long, char *lpClass,
...
int VCR_Hook_GetNumberOfConsoleInputEvents(void *hInput,
...
int VCR_Hook_ReadConsoleInput(void *hInput, void *pRecs, int nMaxRecs,
  ...
  if (g_VCRMode == VCR_Playback) {
    ...
    ret = (char)ReadConsoleInput((HANDLE)hInput, (INPUT_RECORD *)pRecs,
                                 nMaxRecs, pNumRead);
...
void VCR_GenericRecord(const char *pEventName, const void *pData, int len) {
  ...
  uint8_t ucNameLen = (uint8_t)nameLen;
...
int VCR_GenericPlaybackInternal(const char *pEventName, void *pOutData,
                                int maxLen, bool bForceSameLen,
...
int VCR_GenericPlayback(const char *pEventName, void *pOutData, int maxLen,
                        bool bForceSameLen) {
...
void VCR_GenericValueVerify(const char *pEventName, const void *pData,
                            int maxLen) {
  // Preamble.
  if (!IsVCRModeEnabledForThisThread()) return;
...
void WriteShortString(const char *pStr) {
  ...
  uint16_t twobytes = (uint16_t)len;
...
unsigned long VCR_WaitForMultipleObjects(uint32_t handles_count,
