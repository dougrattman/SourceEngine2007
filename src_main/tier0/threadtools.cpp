...
#include "winlite.h"
#elif _LINUX
typedef int (*PTHREAD_START_ROUTINE)(void *lpThreadParameter);
...
ThreadHandle_t CreateSimpleThread(ThreadFunc_t pfnThread, void *pParam,
                                  ThreadId_t *pID, unsigned stackSize) {
#ifdef _WIN32
...
uint32_t g_ThreadMainThreadID = InitMainThread();
...
bool CThreadSyncObject::Wait(uint32_t dwTimeout) {
  ...
  if (m_cSet > 0) {
    ...
    volatile uint64_t nSec =
        (uint64_t)tv.tv_usec * 1000 + (uint64_t)dwTimeout * 1000000;
...
_Acquires_lock_(this->m_hSyncObject) CThreadFullMutex::CThreadFullMutex(
    bool bEstablishInitialOwnership, const char *pszName) {
...
int64_t ThreadInterlockedCompareExchange64(int64_t volatile *pDest,
                                           int64_t value, int64_t comperand) {
...
bool ThreadInterlockedAssignIf64(volatile int64_t *pDest, int64_t value,
                                 int64_t comperand) {
  Assert((size_t)pDest % 8 == 0);
...
#ifndef _LINUX
void CThreadFastMutex::Lock(const uint32_t threadId,
...
uint32_t CThread::GetThreadId() { return m_threadId; }
...
int CWorkerThread::CallWorker(unsigned dw, unsigned timeout,
...
static uint32_t __stdcall DefaultWaitFunc(uint32_t nHandles,
...
int CWorkerThread::Call(unsigned dwParam, unsigned timeout, bool fBoostPriority,
                        WaitFunc_t pfnWait) {
