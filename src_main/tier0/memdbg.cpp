  ...
  for (int k = 0; k < nMaxAddresses; ++k) {
    ...
    if (SymGetLineFromAddr64(GetCurrentProcess(), (DWORD64)ppAddresses[k],
      ...
      char const *pchFileName =
          hlpLine.FileName ? hlpLine.FileName + strlen(hlpLine.FileName)
                           : nullptr;
      for (size_t numSlashesAllowed = 2; pchFileName > hlpLine.FileName;
           --pchFileName) {
        if (*pchFileName == '\\') {
...
{
  size_t nLogicalSize;
  uint8_t reserved[12];  // MS allocator always returns mem aligned on 16 bytes,
...
class CDbgMemAlloc : public IMemAlloc {
 ...
 private:
  ...
  struct MemInfoKey_t {
    MemInfoKey_t(const char *pFileName, int line)
  ...
  typedef std::set<const char *, CStringLess,
                   CNoRecurseAllocator<const char *> >
      Filenames_t;
