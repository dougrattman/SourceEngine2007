...
bool WriteMiniDumpUsingExceptionInfo(
    ...
    wchar_t *ptchMinidumpFileNameBuffer /* = nullptr */
) {
  ...
  if (miniDumpWriteDump) {
    ...
    wchar_t *pch = wcsrchr(module_name, L'.');
    if (pch) {
      *pch = L'\0';
    }
    ...
    if (minidump_file != INVALID_HANDLE_VALUE) {
      ...
      if (bMinidumpResult) {
        ...
        if (ptchMinidumpFileNameBuffer) {
          ...
          wchar_t *pTgt = ptchMinidumpFileNameBuffer;
          wchar_t const *pSrc = file_name;
          while ((*(pTgt++) = *(pSrc++)) != char(0)) continue;
        }
      }
    ...
    if (!bMinidumpResult) {
      wchar_t failed_file_name[_MAX_PATH];
...
void InternalWriteMiniDumpUsingExceptionInfo(
    unsigned int unstructured_exception_code,
    _EXCEPTION_POINTERS *exception_infos) {
