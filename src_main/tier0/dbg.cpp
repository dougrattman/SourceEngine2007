...
static const char* s_pGroupNames[GROUP_COUNT] = {s_pDeveloper, s_pConsole,
                                                 s_pNetwork};
...
DBG_INTERFACE void AssertValidStringPtr(const char* ptr,
...
static SpewRetval_t _SpewMessage(SpewType_t spewType, const char* pGroupName,
                                 int nLevel, const Color* pColor,
...
FORCEINLINE SpewRetval_t _SpewMessage(SpewType_t spewType,
                                      const char* pMsgFormat, va_list args) {
...
SpewRetval_t _DSpewMessage(const char* pGroupName, int level,
...
bool Plat_SimpleLog(const char* file, int line) {
...
void COM_TimestampedLog(char const* fmt, ...) {
  ...
  const double curent_stamp = Plat_FloatTime();
