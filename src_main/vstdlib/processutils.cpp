...
int CProcessUtils::GetActualProcessOutput(ProcessHandle_t hProcess, char *pBuf,
                                          int nBufLen) {
  ProcessInfo_t &info = m_Processes[hProcess];
  if (info.m_hChildStdoutRd == INVALID_HANDLE_VALUE) return 0;
...
int CProcessUtils::GetProcessOutput(ProcessHandle_t hProcess, char *pBuf,
                                    int nBufLen) {
  Assert(m_bInitialized);
  ...
  if (info.m_hChildStdoutRd == INVALID_HANDLE_VALUE) {
    ...
    while (WaitForSingleObject(info.m_hProcess, 100) == WAIT_TIMEOUT) {
      ...
      if (nLen > 0) {
        ...
        int nBytesRead = GetActualProcessOutput(
