...
BOOL SetConsoleCXCY(HANDLE std_out, int cx, int cy) {
  CONSOLE_SCREEN_BUFFER_INFO info;
  COORD coordMax;

  coordMax = GetLargestConsoleWindowSize(std_out);

  if (cy > coordMax.Y) cy = coordMax.Y;

  if (cx > coordMax.X) cx = coordMax.X;
...
LPVOID GetMappedBuffer(HANDLE file_buffer) {
  LPVOID pBuffer =
      MapViewOfFile(file_buffer, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0);
  return pBuffer;
}
