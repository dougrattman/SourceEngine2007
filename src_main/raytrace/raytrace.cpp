...
void CacheOptimizedTriangle::ChangeIntoIntersectionFormat(void) {
  ...
  int drop_axis = 0;
  for (int c = 1; c < 3; c++)
    if (fabs(N[c]) > fabs(N[drop_axis])) drop_axis = c;
...
int CacheOptimizedTriangle::ClassifyAgainstAxisSplit(int split_plane,
  ...
  if (maxc <= split_value) return PLANECHECK_NEGATIVE;
...
float RayTracingEnvironment::CalculateCostsOfSplit(
  ...
  if (nleft && (nboth == 0) && (nright == 0)) split_value = max_coord;
  if (nright && (nboth == 0) && (nleft == 0)) split_value = min_coord;
...
void RayTracingEnvironment::RefineNode(int node_number, int32_t const *tri_list,
  ...
  if (ntris < 3)  // never split empty lists
  {
    ...
    for (int t = 0; t < ntris; t++) TriangleIndexList.AddToTail(tri_list[t]);
    return;
  }
  ...
  if ((cost_of_no_split <= best_cost) || NEVER_SPLIT ||
    ...
    OptimizedKDTree.AddToTail(newnode);
    // now, recurse!
    if ((ntris < 20) && ((best_nleft == 0) || (best_nright == 0))) depth += 100;
