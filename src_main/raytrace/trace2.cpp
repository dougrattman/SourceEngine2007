...
static ALIGN16 int32_t signmask[4] = {0x80000000, 0x80000000, 0x80000000,
                                      0x80000000};
...
void RayTracingEnvironment::RenderScene(
  ...
  // block_offsets-relative offsets for eahc of the 4 pixels in the block, in
  // sse format
  FourVectors block_offsets;
  block_offsets.LoadAndSwizzle(Vector(0, 0, 0), dxvector, dyvector,
                               dxvector + dyvector);
  ...
  for (int y = 0; y < height; y += 2) {
    ...
    for (int x = 0; x < width; x += 2) {
      ...
      if ((rslt.HitIds[0] == -1) && (rslt.HitIds[1] == -1) &&
        ...
        MapLinearIntensities(BackgroundColor, dest, dest + 1, dest + stride,
                             dest + stride + 1);
      else {
        ...
        switch (lmode) {
          case DIRECT_LIGHTING: {
            ...
            for (int l = 0; l < LightList.Count(); l++) {
              LightList[l].ComputeLightAtPoints(surface_pos,
                                                rslt.surface_normal, intens);
            }
          } break;
        ...
        MapLinearIntensities(intens, dest, dest + 1, dest + stride,
                             dest + stride + 1);
      }
      dest += 2;
      SLoc += dxvectortimes2;
      ...
      for (int try1 = 0; try1 < n_desired; try1++) {
        ...
        if (li.IsDirectionWithinLightCone(trial_dir)) {
          ...
          if ((rslt.HitIds[0] != -1)) {
            ...
            l1.m_Position =
                Vector(surface_pos.X(0), surface_pos.Y(0), surface_pos.Z(0));
            l1.m_Direction =
                Vector(rslt.surface_normal.X(0), rslt.surface_normal.Y(0),
...
void RayTracingEnvironment::AddToRayStream(RayStream &s, Vector const &start,
                                           Vector const &end,
