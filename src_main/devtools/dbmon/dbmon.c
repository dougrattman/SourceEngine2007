...
--*/
{
  ...
  SharedFile = CreateFileMapping((HANDLE)-1, &sa, PAGE_READWRITE, 0, 4096,
                                 "DBWIN_BUFFER");

  if (!SharedFile) {
    fprintf(stderr, "dbmon: Unable to create file mapping object, err == %d\n",
            GetLastError());
    exit(1);
  }
  ...
  if (!SharedMem) {
    fprintf(stderr, "dbmon: Unable to map shared memory, err == %d\n",
            GetLastError());
    exit(1);
  }
