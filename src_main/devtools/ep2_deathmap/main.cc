...
bool ReadBitmapRGB(const u8 *raw, size_t rawlen, Image_t *image) {
  ...
  if (!(bmih.biSize == sizeof(bmih) && bmih.biPlanes == 1)) return false;

  // Bogus bit depth?  Only 8-bit supported.
  if (bmih.biBitCount != 24) return false;

  // Bogus compression?  Only non-compressed supported.
  if (bmih.biCompression != BI_RGB) return false;
...
void ParseBugs(CUtlDict<CUtlVector<Vector> *, int> &list,
               char const *filename) {
  FileHandle_t fh = g_pFullFileSystem->Open(filename, "rb");
  if (FILESYSTEM_INVALID_HANDLE == fh) return;
...
bool WriteBitmap(char const *filename, Image_t *image) {
  ...
  if (FILESYSTEM_INVALID_HANDLE == fh) return false;
  ...
  hdr.bfSize =
      (DWORD)(sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + imageSize);
  hdr.bfReserved1 = 0;
  hdr.bfReserved2 = 0;
...
inline void DrawColoredRect(Image_t *image, int x, int y, int w, int h,
  ...
  for (int xx = x; xx < x + w; ++xx) {
    if (xx < 0 || xx >= image->w) continue;

    for (int yy = y; yy < y + h; ++yy) {
      if (yy < 0 || yy >= image->h) continue;
...
void BuildAggregateStats(IMySQL *mysql, char const *pszMapName,
                         char const *whereClause) {
  char q[2048];
  ...
  Q_snprintf(q, sizeof(q),
             "select Sum(Count), Avg(Count), Avg(Seconds)/60.0, Avg(Deaths) "
             "from ep2_maps where MapName = \'%s\' %s and Seconds < 15000;",
  ...
  if (mysql->SeekToFirstRow() && mysql->NextRow()) {
    ...
    Msg("Sessions %d, average sessions %.2f avg time %.2f (minutes) avgdeaths "
        "%.2f\n",
        SumCount, AverageCount, AvgTime, AvgDeaths);
  } else {
    Msg("No data for %s\n", mapname.c_str());
    return;
  }
  ...
  Q_snprintf(q, sizeof(q),
             "select Entity, Sum(BodyCount), avg(BodyCount), "
             "Sum(KilledPlayer), avg(KilledPlayer) from ep2_entities where "
             "mapname = \'%s\' %s group by Entity;",
             mapname.c_str(), whereClause);
  ...
  Q_snprintf(q, sizeof(q),
             "select Weapon, Sum(Shots), avg(Shots), sum(Hits), avg(Hits), "
             "Sum(Damage), Avg(Damage) from ep2_weapons where mapname = \'%s\' "
             "%s group by Weapon;",
             mapname.c_str(), whereClause);
  ...
  if (mysql->SeekToFirstRow()) {
    while (mysql->NextRow()) {
      Msg("  %32s %10d %10.2f %15d %10.2f %15d %10.2f\n",
          mysql->GetColumnValue_String(0), mysql->GetColumnValue_Int(1),
          Q_atof(mysql->GetColumnValue_String(2)), mysql->GetColumnValue_Int(3),
          Q_atof(mysql->GetColumnValue_String(4)), mysql->GetColumnValue_Int(5),
  ...
  // Now show weapon stats
  Q_snprintf(q, sizeof(q),
             "select count(*)/count(distinct(userid) ) from ep2_saves where "
             "mapname = \'%s\' %s;",
             mapname.c_str(), whereClause);
  ...
  Q_snprintf(q, sizeof(q),
             ...
             "from ep2_counters where mapname = \'%s\' %s;",
             mapname.c_str(), whereClause);
...
#define INCHES_TO_MILES (1.0 / (5280.0 * 12.0))
      ...
      while (counters[i] != nullptr) {
        ...
        if (Q_stristr(counters[i], "DISTANCE_")) {
          Msg("  %32s %20.2f %20.2f [miles]\n", counters[i],
              (double)sum * INCHES_TO_MILES, avg * INCHES_TO_MILES);
        } else {
  ...
  Q_snprintf(q, sizeof(q),
             "select StatName, Sum(Count), avg(Count), sum(Value), avg(Value) "
             "from ep2_generic where mapname = \'%s\' %s group by StatName;",
             mapname.c_str(), whereClause);
...
void printusage(int argc, char *argv[]) {
  Msg("%s:\n"
      ...
      "  -stats                { spew per level stats for entities, weapons, "
      "etc. }\n",
      argv[0]);
}
...
int main(int argc, char *argv[]) {
  ...
  for (KeyValues *map = kv->GetFirstSubKey(); map; map = map->GetNextKey()) {
    ...
    if (bBuildImages) {
      ...
      if (FILESYSTEM_INVALID_HANDLE == fh) continue;
      ...
      if (ReadBitmapRGB(buf, size, &image)) {
        ...
        for (int i = 0; i < c; ++i) {
          ...
          for (int y = std::max(0, ypos - nRadius);
            ...
            for (int x = std::max(0, xpos - nRadius); x <= xpos + nRadius;
              ...
              if (flScale <= 0.0f) continue;

              *slot += flScale;
              if (*slot > flMaxValue) flMaxValue = *slot;
            }
          }
        }
        ...
        if (flMaxValue > 0.0f) {
          ...
          for (int y = 0; y < image.h; ++y) {
            ...
            for (int x = 0; x < image.w; ++x, ++slot) {
              ...
              DrawColoredRect(&image, x, y, 1, 1, col,
                              fastSquareRoot[colorIndex]);
            }
          }
        }
        ...
        if (bStressTest) {
          Msg("Processing took %f msec %f msec per thousand deaths [%d "
              "deaths]\n",
