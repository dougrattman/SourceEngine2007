...
static CounterInfo_t g_IntCounterNames[] = {
    ...
    {Ep2LevelStats_t::COUNTER_DISTANCE_INVEHICLE,
     "Distance in Vehicles (inches)"},
    {Ep2LevelStats_t::COUNTER_DISTANCE_ONFOOT, "Distance on Foot (inches)"},
    {Ep2LevelStats_t::COUNTER_DISTANCE_ONFOOTSPRINTING,
     "Distance on Foot Sprint (inches)"},
    {Ep2LevelStats_t::COUNTER_FALLINGDEATHS, "Falling Death"},
    {Ep2LevelStats_t::COUNTER_VEHICLE_OVERTURNED, "Flipped Vehicles"},
    {Ep2LevelStats_t::COUNTER_LOADGAME_STILLALIVE,
     "Loadgame while player alive"},
...
void DescribeEp2Stats() {
  int i;
  int iMap;
  for (iMap = g_dictMapStats.First(); iMap != g_dictMapStats.InvalidIndex();
       iMap = g_dictMapStats.Next(iMap)) {
    // Get the current map.
    Ep2LevelStats_t *pCurrentMap = &g_dictMapStats[iMap];

    Msg("map version[ %d ], user tag[ %s ]\n", pCurrentMap->m_Tag.m_nMapVersion,
        pCurrentMap->m_Tag.m_szTagText);
    ...
    CUtlDict<Ep2LevelStats_t::WeaponLump_t, int> &wdict =
        pCurrentMap->m_dictWeapons;
    for (i = wdict.First(); i != wdict.InvalidIndex(); i = wdict.Next(i)) {
      ...
      Msg("   %32.32s:  shots %5d hits %5d damage %8.2f [accuracy %8.2f %%]\n",
          wdict.GetElementName(i), lump.m_nShots, lump.m_nHits,
          lump.m_flDamageInflicted, acc);
    }
    Msg("  -- NPC Stats -- \n");

    CUtlDict<Ep2LevelStats_t::EntityDeathsLump_t, int> &dict =
        pCurrentMap->m_dictEntityDeaths;
    ...
    for (i = 0; i < Ep2LevelStats_t::NUM_INTCOUNTER_TYPES; ++i) {
      if (i == Ep2LevelStats_t::COUNTER_DISTANCE_INVEHICLE ||
        ...
        Msg("   %32.32s:  %8I64u [%8.3f feet] [%8.3f miles]\n",
            ...
            INCHES_TO_FEET * (double)pCurrentMap->m_IntCounters[i],
            INCHES_TO_MILES * (double)pCurrentMap->m_IntCounters[i]);
      } else {
        Msg("   %32.32s:  %8I64u\n", g_IntCounterNames[i].counterName,
            pCurrentMap->m_IntCounters[i]);
      }
    }
    for (i = 0; i < Ep2LevelStats_t::NUM_FLOATCOUNTER_TYPES; ++i) {
      Msg("   %32.32s:  %8.2f\n", g_FloatCounterNames[i].counterName,
          pCurrentMap->m_FloatCounters[i]);
    }
    ...
    for (i = 0; i < sg->m_Records.Count(); ++i) {
      ...
      Msg("  %3d deaths, saved at (%5d %5d %5d) with health %3d %s\n",
          rec.m_nNumDeaths, (int)rec.m_nSavePos[0], (int)rec.m_nSavePos[1],
          (int)rec.m_nSavePos[2], (int)rec.m_nSaveHealth,
          SaveType((int)rec.m_SaveType));
    }

    Msg("  -- Generic Stats --\n");

    CUtlDict<Ep2LevelStats_t::GenericStatsLump_t, int> &gdict =
        pCurrentMap->m_dictGeneric;
    for (i = gdict.First(); i != gdict.InvalidIndex(); i = gdict.Next(i)) {
      ...
      Msg("   %32.32s:  count %u total %f @[%d, %d, %d]\n",
          gdict.GetElementName(i), lump.m_unCount, lump.m_flCurrentValue,
          lump.m_Pos[0], lump.m_Pos[1], lump.m_Pos[2]);
    }
...
void InsertCustomData(CUtlDict<int, unsigned short> &mapOrder, IMySQL *sql,
                      const char *szStatsFileUserID, int iStatsFileVersion,
                      const char *gamename) {
  if (!sql) return;
  ...
  for (iMap = g_dictMapStats.First(); iMap != g_dictMapStats.InvalidIndex();
    ...
    for (i = 0; i < pCurrentMap->m_aPlayerDeaths.Count(); ++i) {
      Q_snprintf(q, sizeof(q),
                 "REPLACE into %s_deaths "
                 "(UserID,Tag,LastUpdate,MapName,MapVersion,DeathIndex,X,Y,Z) "
                 "values (\"%s\",\"%s\",Now(),\"%-.20s\",%d,%d,%d,%d,%d);",
                 gamename, userid.c_str(), tag.c_str(), mapname.c_str(),
                 mapversion, i, pCurrentMap->m_aPlayerDeaths[i].nPosition[0],
                 pCurrentMap->m_aPlayerDeaths[i].nPosition[1],
                 pCurrentMap->m_aPlayerDeaths[i].nPosition[2]);
    ...
    for (i = wdict.First(); i != wdict.InvalidIndex(); i = wdict.Next(i)) {
      ...
      Q_snprintf(
          q, sizeof(q),
          "REPLACE into %s_weapons "
          "(UserID,Tag,LastUpdate,MapName,MapVersion,Weapon,Shots,Hits,Damage) "
          "values (\"%s\",\"%s\",Now(),\"%-.20s\",%d,\"%-.32s\",%d,%d,%g);",
          gamename, userid.c_str(), tag.c_str(), mapname.c_str(), mapversion,
          wname.c_str(), lump.m_nShots, lump.m_nHits, lump.m_flDamageInflicted);
    ...
    for (i = dict.First(); i != dict.InvalidIndex(); i = dict.Next(i)) {
      ...
      Q_snprintf(q, sizeof(q),
                 "REPLACE into %s_entities "
                 "(UserID,Tag,LastUpdate,MapName,MapVersion,Entity,BodyCount,"
                 "KilledPlayer) values "
                 "(\"%s\",\"%s\",Now(),\"%-.20s\",%d,\"%-.32s\",%d,%d);",
                 gamename, userid.c_str(), tag.c_str(), mapname.c_str(),
                 mapversion, ename.c_str(), lump.m_nBodyCount,
    ...
    Q_snprintf(
        q, sizeof(q),
        "REPLACE into %s_counters "
        "(UserID,Tag,LastUpdate,MapName,MapVersion,%s) values "
        "(\"%s\",\"%s\",Now(),\"%-.20s\",%d,"
        "%u,"
        "%u,"
        "%u,"
    ...
    */
    for (i = 0; i < sg->m_Records.Count(); ++i) {
      ...
      Q_snprintf(q, sizeof(q),
                 "REPLACE into %s_saves "
                 "(UserID,Tag,LastUpdate,MapName,MapVersion,FirstDeath,"
                 "NumDeaths,X,Y,Z,Health,SaveType) values "
                 "(\"%s\",\"%s\",Now(),\"%-.20s\",%d,%d,%d,%d,%d,%d,%d,%d);",
                 gamename, userid.c_str(), tag.c_str(), mapname.c_str(),
                 mapversion, rec.m_nFirstDeathIndex, rec.m_nNumDeaths,
                 (int)rec.m_nSavePos[0], (int)rec.m_nSavePos[1],
                 (int)rec.m_nSavePos[2], (int)rec.m_nSaveHealth,
                 (int)rec.m_SaveType);
    ...
    for (i = gdict.First(); i != gdict.InvalidIndex(); i = gdict.Next(i)) {
      ...
      Q_snprintf(
          q, sizeof(q),
          "REPLACE into %s_generic "
          "(UserID,Tag,LastUpdate,MapName,MapVersion,StatName,Count,Value,X,Y,"
          "Z) values "
          "(\"%s\",\"%s\",Now(),\"%-.20s\",%d,\"%-.16s\",%d,%f,%d,%d,%d);",
          gamename, userid.c_str(), tag.c_str(), mapname.c_str(), mapversion,
          statname.c_str(), lump.m_unCount, lump.m_flCurrentValue,
          lump.m_Pos[0], lump.m_Pos[1], lump.m_Pos[2]);
...
int GetMaxLumpCount() { return EP2_MAX_LUMP_COUNT; }

void LoadCustomDataFromBuffer(CUtlBuffer &LoadBuffer, char *tag,
                              size_t tagsize) {
  tag[0] = 0;
  Ep2LevelStats_t::LoadData(g_dictMapStats, LoadBuffer);
  if (g_dictMapStats.Count() > 0) {
    Q_strncpy(tag, g_dictMapStats[g_dictMapStats.First()].m_Tag.m_szTagText,
              tagsize);
  }
  ...
  if (fp) {
    ...
    if (bHasCustomData) {
      ...
      if (ctx->describeonly) {
        ...
        InsertCustomData(g_mapOrder, ctx->mysql, szCurrentStatsFileUserID,
                         iCurrentStatsFileVersion, ctx->gamename);
      }
    }

    // Do base data after custom since we need the custom to parse out the "tag"
    // used for this user
    if (valid) {
      if (!ctx->describeonly) {
        InsertData(g_mapOrder, ctx->mysql, stats, szCurrentStatsFileUserID,
                   iCurrentStatsFileVersion, ctx->gamename, tag);
      }
    } else {
      ++ctx->skipcount;
    }
