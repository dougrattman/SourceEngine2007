...
typedef bool (*ParseCurrentUserIDFunc)(char const *pchDataFile, char *pchUserID,
...
extern bool Ep2_ParseCurrentUserID(char const *pchDataFile, char *pchUserID,
                                   size_t bufsize, time_t &modifiedtime);
...
void DescribeData(BasicGameStats_t &stats, const char *szStatsFileUserID,
                  int iStatsFileVersion) {
  double averageSession = 0.0f;
  if (stats.m_Summary.m_nCount > 0) {
    averageSession =
        (double)stats.m_Summary.m_nSeconds / (double)stats.m_Summary.m_nCount;
  }
  ...
  Msg("  %16.16s:  %8d seconds\n", "Completion time ",
      stats.m_nSecondsToCompleteGame);
  Msg("  %16.16s:  %8d\n", "Number of deaths", stats.m_Summary.m_nDeaths);

  Msg(" -- Maps played --\n");

  for (int i = stats.m_MapTotals.First(); i != stats.m_MapTotals.InvalidIndex();
       i = stats.m_MapTotals.Next(i)) {
    ...
    Msg(" %16.16s:  %5d seconds in %3d sessions (%4d deaths)\n", mapname,
        rec.m_nSeconds, rec.m_nCount, rec.m_nDeaths);
  }
}
...
void InsertData(CUtlDict<int, unsigned short> &mapOrder, IMySQL *sql,
                BasicGameStats_t &gs, const char *szStatsFileUserID,
                int iStatsFileVersion, const char *gamename,
                char const *tag = NULL) {
  if (!sql) return;
  ...
  // Deal with the maps
  for (int i = gs.m_MapTotals.First(); i != gs.m_MapTotals.InvalidIndex();
       i = gs.m_MapTotals.Next(i)) {
    ...
    if (slot != mapOrder.InvalidIndex()) {
      ...
      if (Q_stricmp(pszMapName, "devtest")) continue;
    }
    ...
    if (tag) {
      Q_snprintf(
          q, sizeof(q),
          "REPLACE into %s_maps "
          "(UserID,LastUpdate,Version,MapName,Tag,Count,Seconds,HDR,Captions,"
          "Commentary,Easy,Medium,Hard,nonsteam,cybercafe,Deaths) values "
          "(\"%s\",Now(),%d,\"%s\",\"%s\",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d);",
          gamename, userid.c_str(), iStatsFileVersion, finalname,
          finaltag.c_str(), rec.m_nCount, rec.m_nSeconds, rec.m_nHDR,
          rec.m_nCaptions, rec.m_nCommentary, rec.m_nSkill[0], rec.m_nSkill[1],
          rec.m_nSkill[2], rec.m_bSteam ? 0 : 1, rec.m_bCyberCafe ? 1 : 0,
          rec.m_nDeaths);
    } else {
      Q_snprintf(
          q, sizeof(q),
          "REPLACE into %s_maps "
          "(UserID,LastUpdate,Version,MapName,Count,Seconds,HDR,Captions,"
          "Commentary,Easy,Medium,Hard,nonsteam,cybercafe,Deaths) values "
          "(\"%s\",Now(),%d,\"%s\",%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d);",
          gamename, userid.c_str(), iStatsFileVersion, finalname, rec.m_nCount,
          rec.m_nSeconds, rec.m_nHDR, rec.m_nCaptions, rec.m_nCommentary,
          rec.m_nSkill[0], rec.m_nSkill[1], rec.m_nSkill[2],
          rec.m_bSteam ? 0 : 1, rec.m_bCyberCafe ? 1 : 0, rec.m_nDeaths);
    }
  ...
  if (tag) {
    Q_snprintf(q, sizeof(q),
               "REPLACE into %s "
               "(UserID,LastUpdate,Version,Tag,Count,Seconds,HDR,Captions,"
               "Commentary,Easy,Medium,Hard,SecondsToCompleteGame,HighestMap,"
               "nonsteam,cybercafe,hl2_chapter,dxlevel,Deaths) values "
               "(\"%s\",Now(),%d,\"%s\",%d,%d,%d,%d,%d,%d,%d,%d,%d,\"%s\",%d,%"
               "d,%d,%d,%d);",

               gamename, userid.c_str(), iStatsFileVersion, finaltag.c_str(),
               gs.m_Summary.m_nCount, gs.m_Summary.m_nSeconds,
               gs.m_Summary.m_nHDR, gs.m_Summary.m_nCaptions,
               gs.m_Summary.m_nCommentary, gs.m_Summary.m_nSkill[0],
               gs.m_Summary.m_nSkill[1], gs.m_Summary.m_nSkill[2],
               gs.m_nSecondsToCompleteGame, finalname, gs.m_bSteam ? 0 : 1,
               gs.m_bCyberCafe ? 1 : 0, gs.m_nHL2ChaptureUnlocked,
               gs.m_nDXLevel, gs.m_Summary.m_nDeaths);
  } else {
    Q_snprintf(
        q, sizeof(q),
        "REPLACE into %s "
        "(UserID,LastUpdate,Version,Count,Seconds,HDR,Captions,Commentary,Easy,"
        "Medium,Hard,SecondsToCompleteGame,HighestMap,nonsteam,cybercafe,hl2_"
        "chapter,dxlevel,Deaths) values "
        "(\"%s\",Now(),%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,\"%s\",%d,%d,%d,%d,%d);",

        gamename, userid.c_str(), iStatsFileVersion, gs.m_Summary.m_nCount,
        gs.m_Summary.m_nSeconds, gs.m_Summary.m_nHDR, gs.m_Summary.m_nCaptions,
        gs.m_Summary.m_nCommentary, gs.m_Summary.m_nSkill[0],
        gs.m_Summary.m_nSkill[1], gs.m_Summary.m_nSkill[2],
        gs.m_nSecondsToCompleteGame, finalname, gs.m_bSteam ? 0 : 1,
        gs.m_bCyberCafe ? 1 : 0, gs.m_nHL2ChaptureUnlocked, gs.m_nDXLevel,
...
int Default_ParseCustomGameStatsData(ParseContext_t *ctx) {
  ...
  if (fp) {
    ...
    if (iCheckIfStandardDataSaved != GAMESTATS_STANDARD_NOT_SAVED) {
      // standard data was saved, rewind so the stats can read in time to
      // completion
      statsBuffer.SeekGet(CUtlBuffer::SEEK_CURRENT,
      ...
      if (describeonly) {
        ...
        if (valid) {
          InsertData(g_mapOrder, ctx->mysql, stats, szCurrentStatsFileUserID,
                     iCurrentStatsFileVersion, ctx->gamename);
        } else {
          ++ctx->skipcount;
        }
      }
    ...
    bool bHasCustomData =
        (valid && (statsBuffer.TellPut() != statsBuffer.TellGet()));

    if (bHasCustomData) {
      if (describeonly) {
        // separate out the custom data and store it off for processing by other
        // applications, since they only wanted to 'describe' the data, just use
        // a local temp and overwrite it each time
        ...
        // separate out the custom data and store it off for processing by other
        // applications, assume we will have multiple input stats files from the
        // same user, so store custom data under their userid name and overwrite
        // old data to avoid bloat
        ...
        Q_snprintf(szCustomDataOutputFileName,
...
int main(int argc, char *argv[]) {
  ...
  // Cull list of files by looking for most recent version of user's stats and
  // only keeping around those files
  if (parseUserIDFunc) {
    struct CUserIDFileMapping {
      ...
      static bool Less(const CUserIDFileMapping &lhs,
                       const CUserIDFileMapping &rhs) {
        return Q_stricmp(lhs.userid, rhs.userid) < 0;
      }
    };

    CUtlRBTree<CUserIDFileMapping, int> userIDToFileMap(
        0, 0, CUserIDFileMapping::Less);
    ...
    for (int i = 0; i < c; ++i) {
      ...
      if ((*parseUserIDFunc)(fn, search.userid, sizeof(search.userid),
                             search.filemodifiedtime)) {
        // Find map index
    ...
    Msg("discarded %d of %d, remainder %d [%d skipped] max mod %d\n", nDiscards,
        c, userIDToFileMap.Count(), nSkips, nMaxMod);

    // Now re-write files and count with pared down listing
    files.Purge();
    for (int i = userIDToFileMap.FirstInorder();
         i != userIDToFileMap.InvalidIndex();
         i = userIDToFileMap.NextInorder(i)) {
  ...
  if (bTrySql) {
    ...
    if (sql) {
      ...
      if (factory) {
        ...
        if (mysql) {
          if (mysql->InitMySQL(argv[dbArg], argv[hostArg], argv[usernameArg],
                               argv[pwArg])) {
            bSqlOkay = true;
            if (batchMode) {
          ...
          Msg("Unable to get MYSQL_WRAPPER_VERSION_NAME(%s) from "
              "mysql_wrapper\n",
  ...
  if (bSqlOkay || describeonly) {
    ...
    if (ctx.skipcount > 0) {
      printf(
          "Skipped %d samples which appear to be malformed or contain bogus "
          "data\n",
          ctx.skipcount);
    }

    // if this game has a post-import function to call after all the files have
    // been imported, call it now
...
void InsertKeyDataIntoTable(IMySQL *pSQL, char const *pTableName,
                            char const *pPerfData, char const *pKeyWhiteList[],
                            int nNumFields) {
  // we don't need to worry about semicolons embedded in string fields because
  // we supressed them on the client.  if some malicious person inserts them,
  // the mandled field names will fail the whitelist check, causing the record
  // to be ignored.
  ...
  for (int i = 0; i < tokens.Count(); i++) {
    ...
    *pEqualsSign = 0;  // *semicolon->null
    ...
    if (pValue[0] == ' ') pValue++;
    if (pValue[strlen(pValue) - 1] == ' ') pValue[strlen(pValue) - 1] = 0;
...
char const *s_PerfKeyList[] = {
    "AvgFps",      "MinFps",           "MaxFps",         "CPUID",
    "CPUGhz",      "NumCores",         "GPUDrv",         "GPUVendor",
    "GPUDeviceID", "GPUDriverVersion", "DxLvl",          "Width",
    "Height",      "MapName",          "TotalLevelTime", "NumLevels"};
