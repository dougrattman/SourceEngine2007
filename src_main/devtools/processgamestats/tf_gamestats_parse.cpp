...
static const char *s_aClassNames[] = {
    ...
    "HEAVYWEAPONS", "PYRO",  "SPY",    "ENGINEER", "CIVILIAN",
};
...
static const char *ClassIdToAlias(int iClass) {
  if ((iClass >= std::size(s_aClassNames)) || (iClass < 0)) return NULL;
...
const char *WeaponIdToAlias(int iWeapon) {
  if ((iWeapon >= std::size(s_aWeaponNames)) || (iWeapon < 0)) return NULL;
...
// Purpose:
//-----------------------------------------------------------------------------
void InsertTF2Data(bool bDeathOnly, char const *szStatsFileUserID,
                   time_t fileTime, IMySQL *sql, int iStatsFileVersion) {
  if (!sql) return;
  ...
  {
    int iMap;
    for (iMap = g_reportedStats.m_dictMapStats.First();
         iMap != g_reportedStats.m_dictMapStats.InvalidIndex();
         iMap = g_reportedStats.m_dictMapStats.Next(iMap)) {
      // Get the current map.
      TF_Gamestats_LevelStats_t *pCurrentMap =
          &g_reportedStats.m_dictMapStats[iMap];
      ...
      */
      ...
      for (int i = 0; i < pCurrentMap->m_aPlayerDeaths.Count(); ++i) {
        Q_snprintf(q, sizeof(q),
                   ...
                   "tf", userid.c_str(), tag.c_str(), mapname.c_str(),
  ...
  for (iMap = g_reportedStats.m_dictMapStats.First();
    ...
    Q_snprintf(
        q, sizeof(q),
        "INSERT into tf_mapdata "
        "(TimeSubmitted,MapName,RoundsPlayed,TotalTime,BlueWins,RedWins,"
        "Stalemates,BlueSuddenDeathWins,RedSuddenDeathWins) "
        "values (%s,\"%s\",%d,%d,%d,%d,%d,%d,%d);",
        szDate, mapname.c_str(), pCurrentMap->m_Header.m_iRoundsPlayed,
        pCurrentMap->m_Header.m_iTotalTime, pCurrentMap->m_Header.m_iBlueWins,
        pCurrentMap->m_Header.m_iRedWins, pCurrentMap->m_Header.m_iStalemates,
        pCurrentMap->m_Header.m_iBlueSuddenDeathWins,
        pCurrentMap->m_Header.m_iRedSuddenDeathWins);
    ...
    for (int i = TF_CLASS_UNDEFINED + 1; i < TF_CLASS_CIVILIAN; i++) {
      ...
      Q_snprintf(q, sizeof(q),
                 "INSERT into tf_classdata (MapName, "
                 "TimeSubmitted,Class,Spawns,TotalTime,Score,Kills,Deaths,"
                 "Assists,Captures) "
                 "values (\"%s\",%s,%d,%d,%d,%d,%d,%d,%d,%d);",
                 mapname.c_str(), szDate, i, classStats.iSpawns,
                 classStats.iTotalTime, classStats.iScore, classStats.iKills,
                 classStats.iDeaths, classStats.iAssists, classStats.iCaptures);
    ...
    for (int i = TF_WEAPON_NONE + 1; i < TF_WEAPON_COUNT; i++) {
      ...
      Q_snprintf(q, sizeof(q),
                 "INSERT into tf_weapondata (MapName, "
                 "TimeSubmitted,WeaponID,ShotsFired,ShotsFiredCrit,ShotsHit,"
                 "DamageTotal,"
                 "HitsWithKnownDistance,DistanceTotal) "
                 "values (\"%s\",%s,%d,%d,%d,%d,%d,%d,%I64d)",
                 mapname.c_str(), szDate, i, weaponStats.iShotsFired,
                 weaponStats.iCritShotsFired, weaponStats.iHits,
                 weaponStats.iTotalDamage, weaponStats.iHitsWithKnownDistance,
                 weaponStats.iTotalDistance);
...
int TF_ParseCustomGameStatsData(ParseContext_t *ctx) {
  ...
  if (memcmp(statsBuffer.Base(), "PERFDATA:", 9) == 0) {
    ProcessPerfData(ctx->mysql, "tf_perfdata",
                    ((char *)statsBuffer.Base()) + 9);
  ...
  if (ctx->describeonly) {
    ...
    InsertTF2Data(g_bDeathOnly, szCurrentStatsFileUserID, sb.st_mtime,
                  ctx->mysql, iCurrentStatsFileVersion);
  }
