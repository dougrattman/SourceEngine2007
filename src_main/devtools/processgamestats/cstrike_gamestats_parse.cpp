...
const char *pValidStatLevels[] = {
    ...
    "cs_office",  "de_aztec",    "de_cbble",  "de_chateau",  "de_dust2",
    "de_dust",    "de_inferno",  "de_nuke",   "de_piranesi", "de_port",
...
static const char *s_WeaponAliasInfo[] = {
    ...
    "kevlar",    "assaultsuit", "nightvision",
    NULL,  // WEAPON_NONE
};
...
void DescribeData(cs_gamestats_t &stats) {
  ...
  for (int i = 0; i < CS_NUM_LEVELS; i++) {
    Msg("%s - Terrorists Wins: %d | Counter-Terrorists Wins: %d\n",
        pValidStatLevels[i], stats.iTerroristVictories[i],
        stats.iCounterTVictories[i]);
  }

  for (int i = 0; i < WEAPON_MAX; i++) {
    Msg("%s was purchased %d time(s)\n", s_WeaponAliasInfo[i],
        stats.iBlackMarketPurchases[i]);
  }
...
int CS_ParseCustomGameStatsData(ParseContext_t *ctx) {
  if (g_pFullFileSystem == NULL) return CUSTOMDATA_FAILED;
  ...
  if (Q_stricmp(stats.header.szGameName, "cstrike")) return CUSTOMDATA_FAILED;
  ...
  for (int i = 0; i < CS_NUM_LEVELS; i++) {
    Q_snprintf(q, sizeof(q),
               "update maps set TerroristWins=TerroristWins+%d, "
               "CTWins=CTWins+%d where MapName = \"%s\";",
               stats.iTerroristVictories[i], stats.iCounterTVictories[i],
               pValidStatLevels[i]);
  ...
  for (int i = 0; i < WEAPON_MAX; i++) {
    ...
    Q_snprintf(q, sizeof(q),
               "update weapons set Count=Count+%d where WeaponID = %d;",
               stats.iBlackMarketPurchases[i], iWeaponID);
  ...
  Q_snprintf(q, sizeof(q),
             "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy\";",
             stats.iAutoBuyPurchases);
  ctx->mysql->Execute(q);

  Q_snprintf(q, sizeof(q),
             "update autobuy set Count=Count+%d where Purchase = \"Re-Buy\";",
             stats.iReBuyPurchases);
  ctx->mysql->Execute(q);

  Q_snprintf(
      q, sizeof(q),
      "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy: M4A1\";",
      stats.iAutoBuyM4A1Purchases);
  ctx->mysql->Execute(q);

  Q_snprintf(
      q, sizeof(q),
      "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy: AK47\";",
      stats.iAutoBuyAK47Purchases);
  ctx->mysql->Execute(q);

  Q_snprintf(
      q, sizeof(q),
      "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy: Famas\";",
      stats.iAutoBuyFamasPurchases);
  ctx->mysql->Execute(q);

  Q_snprintf(
      q, sizeof(q),
      "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy: Galil\";",
      stats.iAutoBuyGalilPurchases);
  ctx->mysql->Execute(q);

  Q_snprintf(
      q, sizeof(q),
      "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy: Suit\";",
      stats.iAutoBuyVestHelmPurchases);
  ctx->mysql->Execute(q);

  Q_snprintf(
      q, sizeof(q),
      "update autobuy set Count=Count+%d where Purchase = \"Auto-Buy: Kev\";",
      stats.iAutoBuyVestPurchases);
  ctx->mysql->Execute(q);
