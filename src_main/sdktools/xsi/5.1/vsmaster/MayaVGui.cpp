...
class CMayaVGui : public CTier3AppSystem<IMayaVGui> {
 ...
 public:
  ...
  virtual void InstallVguiWindowFactory(const char *pWindowTypeName,
                                        IMayaVguiWindowFactory *pFactory);
  ...
  virtual bool CreateVGuiWindow(HWND in_hParent, const char *pWindowTypeName,
                                const char *pPanelName);
  virtual void DestroyVGuiWindow(const char *pWindowTypeName,
                                 const char *pPanelName);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMayaVGui, IMayaVGui,
                                  MAYA_VGUI_INTERFACE_VERSION, s_MayaVGui);
...
// Connection
//-----------------------------------------------------------------------------
bool CMayaVGui::Connect() { return true; }
...
InitReturnVal_t CMayaVGui::Init() {
  ...
  // load scheme
  if (!vgui::scheme()->LoadSchemeFromFile("Resource/BoxRocket.res",
                                          "MayaPlugins"))
    return INIT_FAILED;
...
//-----------------------------------------------------------------------------
void CMayaVGui::InstallVguiWindowFactory(const char *pWindowTypeName,
                                         IMayaVguiWindowFactory *pFactory) {
  if (m_WindowFactories.Find(pWindowTypeName) !=
      m_WindowFactories.InvalidIndex()) {
...
void CMayaVGui::RemoveVguiWindowFactory(const char *pWindowTypeName,
                                        IMayaVguiWindowFactory *pFactory) {
  if (m_WindowFactories.Find(pWindowTypeName) ==
      m_WindowFactories.InvalidIndex()) {
...
//-----------------------------------------------------------------------------
bool CMayaVGui::CreateVGuiWindow(HWND in_hParent, const char *pWindowTypeName,
                                 const char *pPanelName) {
  unsigned short hFactory = m_WindowFactories.Find(pWindowTypeName);
...
void CMayaVGui::DestroyVGuiWindow(const char *pWindowTypeName,
                                  const char *pPanelName) {
  // Have to iterate over all factories to see if that factory is the one that
  // created it
  unsigned short hFactory = m_WindowFactories.Find(pWindowTypeName);
  if (hFactory != m_WindowFactories.InvalidIndex()) {
...
void CMayaVGui::SetFocus(void *hWnd, int hVGuiContext) {
  if (hWnd == m_pActiveWindow) return;
...
//-----------------------------------------------------------------------------
bool CMayaVGui::HasFocus(void *pWnd) { return m_pActiveWindow == pWnd; }
...
XSIPLUGINCALLBACK void ValveViewer_SetAttributeValue(
    XSI::CRef in_pViewCtx, XSI::CString in_sAttribName, XSI::CValue in_vValue) {
  XSI::ViewContext l_vViewContext = in_pViewCtx;
  ...
  if (in_sAttribName.IsEqualNoCase(L"attach")) {
    ...
    // TODO(d.rattman) :l_vViewContext.PutUserData ( (void*)l_pVGui );
  }
}

XSIPLUGINCALLBACK XSI::CValue ValveViewer_GetAttributeValue(
    XSI::CRef in_pViewCtx, XSI::CString in_sAttribName) {
