...
double VectorLength(vec3_t v) {
  ...
  length = 0;
  for (i = 0; i < 3; i++) length += v[i] * v[i];
  length = sqrt(length);  // TODO(d.rattman) :
...
winding_t *BasePolyForPlane(face_t *f) {
  ...
  if (x == -1) OutputDebugString("BasePolyForPlane: no axis found");
  ...
  if (!td->scale[0]) td->scale[0] = 1;
  if (!td->scale[1]) td->scale[1] = 1;
...
void CheckFace(face_t *f) {
  ...
  w = f->w;
  if (!w) OutputDebugString("CheckFace: no winding");

  if (w->numpoints < 3) OutputDebugString("CheckFace greater that 3 points");
  ...
  for (i = 0; i < w->numpoints; i++) {
    ...
    for (j = 0; j < w->numpoints; j++) {
      ...
      if (d > edgedist) OutputDebugString("CheckFace: non-convex");
    }
  }
}
...
winding_t *NewWinding(int points) {
  ...
  if (points > MAX_POINTS_ON_WINDING) OutputDebugString("NewWinding error");
...
void GetUV(vec3_t in_vNormal, vec3_t in_vVertex, float &s, float &t,
           float in_fAngle, float in_fOffsetX, float in_fOffsetY,
           float in_fScaleX, float in_fScaleY) {
  ...
  // Scale it.
  if (in_fScaleX != 0.0f) s /= in_fScaleX;
  if (in_fScaleY != 0.0f) t /= in_fScaleY;
