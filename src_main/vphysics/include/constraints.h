...
#ifndef SOURCE_VPHYSICS_INCLUDE_CONSTRAINTS_H_
#define SOURCE_VPHYSICS_INCLUDE_CONSTRAINTS_H_
...
struct constraint_groupparams_t {
  int additionalIterations;  // additional solver iterations make the constraint
                             // system more stable
...
struct constraint_breakableparams_t {
  ...
  float bodyMassScale[2];  // scale applied to mass of reference/attached object
                           // before solving constriant
  bool isActive;
...
// Constraint limit on a single rotation axis
...
// Special limited ballsocket constraint for ragdolls.  Has axis limits for all
// 3 axes.
struct constraint_ragdollparams_t {
  ...
  matrix3x4_t
      constraintToAttached;  // xform constraint space to attached object space
...
// orientation of two objects to rotation around a single axis
...
struct constraint_limitedhingeparams_t : public constraint_hingeparams_t {
  Vector
      referencePerpAxisDirection;  // unit direction vector vector perpendicular
                                   // to the hinge axis in world space
  Vector
      attachedPerpAxisDirection;  // unit direction vector vector perpendicular
                                  // to the hinge axis in world space
...
// Same parameters as fixed constraint, but torqueLimit has no effect
struct constraint_ballsocketparams_t {
  Vector constraintPosition[2];  // position of the constraint in each object's
                                 // space
  constraint_breakableparams_t constraint;
  inline void Defaults() {
    constraint.Defaults();
...
struct constraint_slidingparams_t {
  matrix3x4_t
      attachedRefXform;  // xform attached object space to ref object space
  Vector slideAxisRef;  // unit direction vector of the slide axis in ref object
                        // space
  constraint_breakableparams_t constraint;
...
struct constraint_pulleyparams_t {
  constraint_breakableparams_t constraint;
  Vector pulleyPosition[2];  // These are the pulley positions for the reference
                             // and attached objects in world space
...
struct constraint_lengthparams_t {
  ...
  void InitWorldspace(IPhysicsObject *pRef, IPhysicsObject *pAttached,
                      const Vector &refPosition, const Vector &attachedPosition,
                      bool rigid = false) {
...
#endif  // SOURCE_VPHYSICS_INCLUDE_CONSTRAINTS_H_
