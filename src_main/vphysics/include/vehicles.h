...
#ifndef SOURCE_VPHYSICS_INCLUDE_VEHICLES_H_
#define SOURCE_VPHYSICS_INCLUDE_VEHICLES_H_
...
struct vehicle_bodyparams_t {
  ...
  float
      tiltForceHeight;  // where the tilt force pulls relative to center of mass
...
struct vehicle_axleparams_t {
  ...
  Vector
      raytraceCenterOffset;  // offset to center of axle for the raytrace data.
...
struct vehicle_steeringparams_t {
  ...
  float steeringRateSlow;  // this is the speed the wheels are steered when the
                           // vehicle is slow
  float steeringRateFast;  // this is the speed the wheels are steered when the
                           // vehicle is "fast"
  float steeringRestRateSlow;  // this is the speed at which the wheels move
                               // toward their resting state (straight ahead) at
                               // slow speed
  ...
  float turnThrottleReduceSlow;   // this is the amount of throttle reduction to
                                  // apply at the maximum steering angle
  float turnThrottleReduceFast;   // this is the amount of throttle reduction to
                                  // apply at the maximum steering angle
  ...
  float boostSteeringRestRateFactor;  // this scales the steering rest rate when
                                      // boosting
...
#endif  // SOURCE_VPHYSICS_INCLUDE_VEHICLES_H_
