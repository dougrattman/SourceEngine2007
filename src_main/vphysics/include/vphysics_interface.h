...
//
// Public interfaces to vphysics DLL

#ifndef SOURCE_VPHYSICS_INCLUDE_VPHYSICS_INTERFACE_H_
#define SOURCE_VPHYSICS_INCLUDE_VPHYSICS_INTERFACE_H_
...
// NOTE:  Coordinates are in HL units.  1 unit == 1 inch.  X is east (forward),
// Y is north (left), Z is up (up) QAngle are pitch (around y), Yaw (around Z),
// Roll (around X) AngularImpulse are exponetial maps (an axis in HL units
// scaled by a "twist" angle in degrees)
//		They can be transformed like normals/covectors and added
// linearly
// mass is kg, volume is in^3, acceleration is in/s^2, velocity is in/s

// density is kg/m^3 (water ~= 998 at room temperature)
// preferably, these would be in kg/in^3, but the range of those numbers makes
// them not very human readable having water be about 1000 is really convenient
// for data entry. Since volume is in in^3 and density is in kg/m^3:
...
#define CUBIC_METERS_PER_CUBIC_INCH \
  (METERS_PER_INCH * METERS_PER_INCH * METERS_PER_INCH)
// 2.2 lbs / kg
...
the_interface IVPhysicsDebugOverlay {
 public:
  ...
  virtual void AddTriangleOverlay(const Vector &p1, const Vector &p2,
                                  const Vector &p3, int r, int g, int b, int a,
  ...
  virtual void AddTextOverlay(const Vector &origin, float duration,
                              const char *format, ...) = 0;
  virtual void AddTextOverlay(const Vector &origin, int line_offset,
                              float duration, const char *format, ...) = 0;
  ...
  virtual void AddTextOverlayRGB(const Vector &origin, int line_offset,
...
the_interface IPhysics : public IAppSystem {
 public:
  ...
  // More elements will return nullptr in create. NOTE: id is not allowed to be
  // zero.
...
the_interface IPhysicsCollision {
 public:
  ...
  virtual CPhysConvex *ConvexFromConvexPolyhedron(
      const CPolyhedron &ConvexPolyhedron) = 0;
  // produce a set of convex triangles from a convex polygon, normal is assumed
  // to be on the side with forward point ordering, which should be clockwise,
  // output will need to be able to hold exactly (iPointCount-2) convexes
  virtual void ConvexesFromConvexPolygon(const Vector &vPolyNormal,
                                         const Vector *pPoints, int iPointCount,
  ...
  virtual int CollideWrite(char *pDest, CPhysCollide *pCollide,
                           bool bSwap = false) = 0;
  // unserialize the collide from a block of memory
  virtual CPhysCollide *UnserializeCollide(char *pBuffer, int size,
                                           int index) = 0;
  ...
  virtual void CollideGetAABB(
      Vector * pMins, Vector * pMaxs, const CPhysCollide *pCollide,
  ...
  // get the approximate cross-sectional area projected orthographically on the
  // bbox of the collide NOTE: These are fractional areas - unitless.  Basically
  // this is the fraction of the OBB on each axis that would be visible if the
  // object were rendered orthographically. NOTE: This has been precomputed when
  // the collide was built or this function will return 1,1,1
  virtual Vector CollideGetOrthographicAreas(const CPhysCollide *pCollide) = 0;
  virtual void CollideSetOrthographicAreas(CPhysCollide * pCollide,
                                           const Vector &areas) = 0;
  ...
  virtual void TraceCollide(
      const Vector &start, const Vector &end, const CPhysCollide *pSweepCollide,
      const QAngle &sweepAngles, const CPhysCollide *pCollide,
  ...
  virtual CPhysCollide *CreateVirtualMesh(
      const virtualmeshparams_t &params) = 0;
  ...
  virtual CPolyhedron *PolyhedronFromConvex(CPhysConvex *const pConvex,
                                            bool bUseTempPolyhedron) = 0;
...
the_interface IPhysicsConstraintEvent {
 public:
...
the_interface IPhysicsShadowController {
 public:
  ...
  // Basically get the last inputs to IPhysicsShadowController::Update(),
  // returns last input to timeOffset in Update()
  virtual float GetTargetPosition(Vector * pPositionOut,
                                  QAngle * pAnglesOut) = 0;
...
class IMotionEvent {
 public:
  // These constants instruct the simulator as to how to apply the values copied
  // to linear & angular GLOBAL/LOCAL refer to the coordinate system of the
  // values, whereas acceleration/force determine whether or not mass is divided
  // out (forces must be divided by mass to compute acceleration)
  enum simresult_e {
    SIM_NOTHING = 0,
...
the_interface IPhysicsCollisionSolver {
 public:
  ...
  // pObject has already done the max number of collisions this tick, should we
  // freeze it to save CPU?
  ...
  // This list of objects is in a connected contact graph that is too large to
  // solve quickly return true to freeze the system, false to solve it
  virtual bool ShouldFreezeContacts(IPhysicsObject * *pObjectList,
                                    int objectCount) = 0;
};
...
the_interface IPhysicsEnvironment {
 public:
  ...
  virtual IPhysicsShadowController *CreateShadowController(
      IPhysicsObject * pObject, bool allowTranslation, bool allowRotation) = 0;
  ...
  // Manage the timestep (period) of the simulator.  The main functions are all
  // integrated with this period as dt.
  ...
  // returns the current simulation clock's value at the next frame.  This is an
  // absolute time.
  ...
  virtual const IPhysicsObject **GetObjectList(int *pOutputObjectCount)
      const = 0;
  virtual bool TransferObject(
      IPhysicsObject * pObject,
      IPhysicsEnvironment * pDestinationEnvironment) = 0;
  ...
  // performance tuning
  virtual void GetPerformanceSettings(physics_performanceparams_t * pOutput)
      const = 0;
  ...
  virtual unsigned int GetObjectSerializeSize(IPhysicsObject * pObject)
      const = 0;
  virtual void SerializeObjectToBuffer(IPhysicsObject * pObject,
                                       unsigned char *pBuffer,
                                       unsigned int bufferSize) = 0;
  virtual IPhysicsObject *UnserializeObjectFromBuffer(
      void *pGameData, unsigned char *pBuffer, unsigned int bufferSize,
      bool enableCollisions) = 0;
...
enum callbackflags {
  ...
  CALLBACK_ENABLING_COLLISION =
      0x0800,  // This is active during the time an object is enabling
               // collisions allows us to skip collisions between "new" objects
               // and objects marked for delete
...
the_interface IPhysicsObject {
 public:
  ...
  virtual bool IsFluid()
      const = 0;  // fluids are special triggers with fluid controllers
  ...
  virtual bool IsMoveable()
      const = 0;  // legacy: IsMotionEnabled() && !IsStatic()
  ...
  // NOTE: Contact points aren't updated when collision rules change, call this
  // to force an update UNDONE: Force this in RecheckCollisionFilter() ?
  ...
  // NOTE: velocity is in worldspace, angularVelocity is relative to the
  // object's local axes (just like pev->velocity, pev->avelocity)
  virtual void GetVelocity(Vector * velocity, AngularImpulse * angularVelocity)
      const = 0;
  ...
  virtual void AddVelocity(const Vector *velocity,
                           const AngularImpulse *angularVelocity) = 0;
  // gets a velocity in the object's local frame of reference at a specific
  // point
  virtual void GetVelocityAtPoint(const Vector &worldPosition,
                                  Vector *pVelocity) const = 0;
  // gets the velocity actually moved by the object in the last simulation
  // update
  virtual void GetImplicitVelocity(Vector * velocity,
                                   AngularImpulse * angularVelocity) const = 0;
  // NOTE:	These are here for convenience, but you can do them yourself by
  // using the matrix returned from GetPositionMatrix() convenient coordinate
  // system transformations (params - dest, src)
  virtual void LocalToWorld(Vector * worldPosition, const Vector &localPosition)
      const = 0;
  virtual void WorldToLocal(Vector * localPosition, const Vector &worldPosition)
      const = 0;

  // transforms a vector (no translation) from object-local to world space
  virtual void LocalToWorldVector(Vector * worldVector,
                                  const Vector &localVector) const = 0;
  // transforms a vector (no translation) from world to object-local space
  virtual void WorldToLocalVector(Vector * localVector,
                                  const Vector &worldVector) const = 0;
  ...
  virtual void CalculateForceOffset(
      ...
      Vector *centerForce, AngularImpulse *centerTorque) const = 0;
  ...
  virtual void CalculateVelocityOffset(
      const Vector &forceVector, const Vector &worldPosition,
      Vector *centerVelocity, AngularImpulse *centerAngularVelocity) const = 0;
  ...
  virtual bool GetContactPoint(Vector * contactPoint,
                               IPhysicsObject * *contactObject) const = 0;
  ...
  virtual void UpdateShadow(const Vector &targetPosition,
                            const QAngle &targetAngles, bool tempDisableGravity,
                            float timeOffset) = 0;
...
struct surfacephysicsparams_t {
  ...
  float
      thickness;  // material thickness if not solid (sheet materials) in inches
  float dampening;
};
...
struct surfaceaudioparams_t {
  ...
  float
      hardnessFactor;  // like elasticity, but only affects impact sound choices
  float
      roughnessFactor;  // like friction, but only affects scrape sound choices

  // audio thresholds
  float roughThreshold;  // surface roughness > this causes "rough" scrapes, <
                         // this causes "smooth" scrapes
  float hardThreshold;  // surface hardness > this causes "hard" impacts, < this
                        // causes "soft" impacts
  float hardVelocityThreshold;  // collision velocity > this causes "hard"
                                // impacts, < this causes "soft" impacts
...
the_interface IPhysicsSurfaceProps {
 public:
  ...
  virtual void GetPhysicsParameters(
      int surfaceDataIndex, surfacephysicsparams_t *pParamsOut) const = 0;
};
...
#endif  // SOURCE_VPHYSICS_INCLUDE_VPHYSICS_INTERFACE_H_
