...
class CVguiMatInfo : public IVguiMatInfo {
 public:
  CVguiMatInfo(IMaterial *pMaterial) { m_pMaterial = pMaterial; }
  ...
  virtual IVguiMatInfoVar *FindVarFactory(const char *varName, bool *found) {
    ...
    if (pMaterialVar == NULL) return NULL;
...
CMatSystemSurface::CMatSystemSurface()
    : m_pEmbeddedPanel(NULL), m_pWhite(NULL), m_BatchedCharVerts(0, 256) {
  m_iBoundTexture = -1;
  m_HWnd = NULL;
...
bool CMatSystemSurface::Connect(CreateInterfaceFn factory) {
  ...
  g_pIInput =
      (IInputInternal *)factory(VGUI_INPUTINTERNAL_INTERFACE_VERSION, NULL);
  if (!g_pIInput) {
  ...
  if (!vgui::VGui_InitInterfacesList("MATSURFACE", &factory, 1)) return false;
...
//-----------------------------------------------------------------------------
InitReturnVal_t CMatSystemSurface::Init() {
  ...
  m_DrawTextColor[0] = m_DrawTextColor[1] = m_DrawTextColor[2] =
...
// Purpose:
//-----------------------------------------------------------------------------
void CMatSystemSurface::Shutdown() {
  for (int i = m_FileTypeImages.First(); i != m_FileTypeImages.InvalidIndex();
       i = m_FileTypeImages.Next(i)) {
  ...
  // we need to delete these BEFORE we close our window down, as the browser is
  // using it if this DOESN'T run then it will crash when we close the main
  // window
  ...
  for (int i = 0; i < m_CustomFontFileNames.Count(); i++) {
    // dvs: Keep removing the font until we get an error back. After
    // consulting with Microsoft, it appears that RemoveFontResourceEx must
    // sometimes be called multiple times to work. Doing this insures that
...
// hierarchy root
//-----------------------------------------------------------------------------
VPANEL CMatSystemSurface::GetEmbeddedPanel() { return m_pEmbeddedPanel; }
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawPanelIn3DSpace(vgui::VPANEL pRootPanel,
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::StartDrawingIn3DSpace(const VMatrix &screenToWorld,
                                              int pw, int ph, float sw,
                                              float sh) {
  g_bInDrawing = true;
  m_iBoundTexture = -1;
  ...
  // concatenate it with the panelCenterToWorld to produce pixelToWorld
  // transform
  ...
  // Always enable scissoring (translate to origin because of the glTranslatef
  // call above..)
  EnableScissor(true);
...
// we may need to offset by 0.5 texels to account for the different in pixel vs.
// texel centers in dx7-9 however, we do this fixup already when we set up the
// texture coordinates for all materials/fonts so in theory we shouldn't need to
// do any adjustments for setting up the screen HOWEVER, we must do the offset,
// else the driver will think the text is something that should be antialiased,
// so the text will look broken if antialiasing is turned on (usually forced on
// in the driver)
float g_flPixelOffsetX = 0.5f;
float g_flPixelOffsetY = 0.5f;
bool g_bCheckedCommandLine = false;

void CMatSystemSurface::StartDrawing() {
  if (!g_bCheckedCommandLine) {
    g_bCheckedCommandLine = true;

    const char *pX =
        CommandLine()->ParmValue("-pixel_offset_x", (const char *)NULL);
    if (pX) g_flPixelOffsetX = atof(pX);

    const char *pY =
        CommandLine()->ParmValue("-pixel_offset_y", (const char *)NULL);
    if (pY) g_flPixelOffsetY = atof(pY);
  }
  ...
  pRenderContext->Ortho(g_flPixelOffsetX, g_flPixelOffsetY,
                        width + g_flPixelOffsetX, height + g_flPixelOffsetY,
                        -1.0f, 1.0f);
...
// Purpose:
//-----------------------------------------------------------------------------
void CMatSystemSurface::FinishDrawing() {
...
void CMatSystemSurface::RunFrame() {
  ...
  if (m_nLastInputPollCount == nPollCount) return;
...
void CMatSystemSurface::PushMakeCurrent(VPANEL pPanel, bool useInSets) {
  ...
  g_pVGuiPanel->GetClipRect(pPanel, clipRect[0], clipRect[1], clipRect[2],
                            clipRect[3]);
...
void CMatSystemSurface::PopMakeCurrent(VPANEL pPanel) {
  ...
  if (top > 0) SetupPaintState(m_PaintStateStack[top - 1]);
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::InitVertex(vgui::Vertex_t &vertex, int x, int y,
                                   float u, float v) {
...
// Draws a line!
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawTexturedLineInternal(const Vertex_t &a,
                                                 const Vertex_t &b) {
  Assert(!m_bIn3DPaintMode);

  // Don't bother drawing fully transparent lines
  if (m_DrawColor[3] == 0) return;
  ...
  if (!ClipLine(verts, clippedVerts)) return;
  ...
  meshBuilder.Position3f(clippedVerts[0].m_Position.x,
                         clippedVerts[0].m_Position.y, m_flZPos);
  meshBuilder.AdvanceVertex();

  meshBuilder.Color4ubv(m_DrawColor);
  meshBuilder.TexCoord2fv(0, clippedVerts[1].m_TexCoord.Base());
  meshBuilder.Position3f(clippedVerts[1].m_Position.x,
                         clippedVerts[1].m_Position.y, m_flZPos);
  meshBuilder.AdvanceVertex();
...
void CMatSystemSurface::DrawTexturedLine(const Vertex_t &a, const Vertex_t &b) {
  IMaterial *pMaterial =
      TextureDictionary()->GetTextureMaterial(m_iBoundTexture);
...
void CMatSystemSurface::DrawPolyLine(int *px, int *py, int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!ClipLine(verts, clippedVerts)) continue;

    meshBuilder.Color4ubv(m_DrawColor);
    meshBuilder.TexCoord2fv(0, clippedVerts[0].m_TexCoord.Base());
    meshBuilder.Position3f(clippedVerts[0].m_Position.x + g_flPixelOffsetX,
                           clippedVerts[0].m_Position.y + g_flPixelOffsetY,
                           m_flZPos);
    meshBuilder.AdvanceVertex();

    meshBuilder.Color4ubv(m_DrawColor);
    meshBuilder.TexCoord2fv(0, clippedVerts[1].m_TexCoord.Base());
    meshBuilder.Position3f(clippedVerts[1].m_Position.x + g_flPixelOffsetX,
                           clippedVerts[1].m_Position.y + g_flPixelOffsetY,
                           m_flZPos);
    meshBuilder.AdvanceVertex();
  }
...
// Draws a quad:
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawQuad(const vgui::Vertex_t &ul,
                                 const vgui::Vertex_t &lr,
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawQuadArray(int quadCount, vgui::Vertex_t *pVerts,
                                      unsigned char *pColor) {
  Assert(!m_bIn3DPaintMode);
  ...
  for (int i = 0; i < quadCount; ++i) {
    ...
    if (!ClipRect(ul, lr, &ulc, &lrc)) continue;
...
void CMatSystemSurface::DrawFilledRect(int x0, int y0, int x1, int y1) {
  ...
  // Fully clipped?
  if (!ClipRect(rect[0], rect[1], &clippedRect[0], &clippedRect[1])) return;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawFilledRectFade(int x0, int y0, int x1, int y1,
                                           unsigned int alpha0,
                                           unsigned int alpha1,
                                           bool bHorizontal) {
  Assert(g_bInDrawing);
  ...
  meshBuilder.Position3f(clippedRect[0].m_Position.x,
                         clippedRect[0].m_Position.y, m_flZPos);
  meshBuilder.TexCoord2f(0, clippedRect[0].m_TexCoord.x,
                         clippedRect[0].m_TexCoord.y);
  meshBuilder.AdvanceVertex();

  meshBuilder.Color4ubv(colors[1]);
  meshBuilder.Position3f(clippedRect[1].m_Position.x,
                         clippedRect[0].m_Position.y, m_flZPos);
  meshBuilder.TexCoord2f(0, clippedRect[1].m_TexCoord.x,
                         clippedRect[0].m_TexCoord.y);
  meshBuilder.AdvanceVertex();

  meshBuilder.Color4ubv(colors[2]);
  meshBuilder.Position3f(clippedRect[1].m_Position.x,
                         clippedRect[1].m_Position.y, m_flZPos);
  meshBuilder.TexCoord2f(0, clippedRect[1].m_TexCoord.x,
                         clippedRect[1].m_TexCoord.y);
  meshBuilder.AdvanceVertex();
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawOutlinedCircle(int x, int y, int radius,
                                           int segments) {
  Assert(g_bInDrawing);
  ...
  for (int i = 1; i <= segments; ++i) {
    ...
    if (ClipLine(vertex, renderVertex)) {
      ...
      meshBuilder.Position3f(renderVertex[0].m_Position.x,
                             renderVertex[0].m_Position.y, m_flZPos);
      meshBuilder.AdvanceVertex();

      meshBuilder.Color4ubv(m_DrawColor);
      meshBuilder.TexCoord2fv(0, renderVertex[1].m_TexCoord.Base());
      meshBuilder.Position3f(renderVertex[1].m_Position.x,
                             renderVertex[1].m_Position.y, m_flZPos);
      meshBuilder.AdvanceVertex();
    }
...
bool CMatSystemSurface::DrawGetTextureFile(int id, char *filename, int maxlen) {
  if (!TextureDictionary()->IsValidId(id)) return false;
...
IVguiMatInfo *CMatSystemSurface::DrawGetTextureMatInfoFactory(int id) {
  if (!TextureDictionary()->IsValidId(id)) return NULL;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawSetTextureFile(int id, const char *pFileName,
                                           int hardwareFilter,
...
void CMatSystemSurface::ReferenceProceduralMaterial(int id, int referenceId,
                                                    IMaterial *pMaterial) {
  TextureDictionary()->BindTextureToMaterialReference(id, referenceId,
                                                      pMaterial);
}
...
// Draws a textured rectangle
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawTexturedSubRect(int x0, int y0, int x1, int y1,
                                            float texs0, float text0,
                                            float texs1, float text1) {
  Assert(g_bInDrawing);
...
void CMatSystemSurface::DrawTexturedPolygon(int n, Vertex_t *pVertices) {
  ...
  // Clip vertices...
  iCount =
      ClipPolygon(n, pVertices, m_nTranslateX, m_nTranslateY, &ppClippedVerts);
  if (iCount <= 0) return;
...
//-----------------------------------------------------------------------------
HFont CMatSystemSurface::CreateFont() { return FontManager().CreateFont(); }

//-----------------------------------------------------------------------------
// Purpose: adds glyphs to a font created by CreateFont()
//-----------------------------------------------------------------------------
bool CMatSystemSurface::SetFontGlyphSet(HFont font, const char *windowsFontName,
                                        int tall, int weight, int blur,
                                        int scanlines, int flags) {
  return FontManager().SetFontGlyphSet(font, windowsFontName, tall, weight,
...
// Purpose: adds glyphs to a font created by CreateFont()
//-----------------------------------------------------------------------------
bool CMatSystemSurface::SetBitmapFontGlyphSet(HFont font,
                                              const char *windowsFontName,
                                              float scalex, float scaley,
                                              int flags) {
  return FontManager().SetBitmapFontGlyphSet(font, windowsFontName, scalex,
                                             scaley, flags);
}
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetCharABCwide(HFont font, int ch, int &a, int &b,
                                       int &c) {
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetTextSize(HFont font, const wchar_t *text, int &wide,
                                    int &tall) {
  FontManager().GetTextSize(font, text, wide, tall);
}

//-----------------------------------------------------------------------------
// Purpose: adds a custom font file (only supports true type font files (.ttf)
// for now)
//-----------------------------------------------------------------------------
bool CMatSystemSurface::AddCustomFontFile(const char *fontFileName) {
  ...
  Q_strlower(full_font_path);
  CUtlSymbol sym(full_font_path);

  int i;
  for (i = 0; i < m_CustomFontFileNames.Count(); i++) {
    if (m_CustomFontFileNames[i] == sym) break;
  }

  if (!m_CustomFontFileNames.IsValidIndex(i)) {
    m_CustomFontFileNames.AddToTail(full_font_path);
    ...
    g_pFullFileSystem->GetLocalCopy(full_font_path);
  }

  const int fonts_added_count =
      AddFontResourceEx(full_font_path, FR_PRIVATE, nullptr);
  const bool is_font_added = fonts_added_count > 0;

  // add to windows
  if (!is_font_added) {
    Msg("Failed to load custom font file '%s'\n", full_font_path);
  }
  Assert(is_font_added);

  return is_font_added;
}
...
bool CMatSystemSurface::AddBitmapFontFile(const char *fontFileName) {
  bool bFound = false;
  bFound = ((g_pFullFileSystem->GetDVDMode() == DVDMODE_STRICT) ||
            g_pFullFileSystem->FileExists(fontFileName, nullptr));
  if (!bFound) {
  ...
  for (i = 0; i < m_BitmapFontFileNames.Count(); i++) {
    if (m_BitmapFontFileNames[i] == sym) break;
  }
...
void CMatSystemSurface::ClearTemporaryFontCache() {
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::PrecacheFontCharacters(HFont font,
                                               wchar_t *pCharacterString) {
  if (!pCharacterString || !pCharacterString[0]) return;
  ...
  g_FontTextureCache.GetTextureForChars(m_hCurrentFont, FONT_DRAW_DEFAULT,
                                        pCharacterString, pTextureIDs_ignored,
                                        pTexCoords_ignored, numChars);
...
void CMatSystemSurface::DrawFlushText() {
  if (!m_BatchedCharVerts.Count()) return;

  IMaterial *pMaterial =
      TextureDictionary()->GetTextureMaterial(m_iBoundTexture);
  InternalSetMaterial(pMaterial);
  DrawQuadArray(m_BatchedCharVerts.Count() / 2, m_BatchedCharVerts.Base(),
                m_DrawTextColor);
...
void CMatSystemSurface::DrawSetTextColor(int r, int g, int b, int a) {
  ...
  if (r != m_DrawTextColor[0] || g != m_DrawTextColor[1] ||
      b != m_DrawTextColor[2] || adjustedAlpha != m_DrawTextColor[3]) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawUnicodeChar(
    wchar_t ch, FontDrawType_t drawType /*= FONT_DRAW_DEFAULT */) {
  CharRenderInfo info;
...
// Purpose:
//-----------------------------------------------------------------------------
bool CMatSystemSurface::DrawGetUnicodeCharRenderInfo(wchar_t ch,
                                                     CharRenderInfo &info) {
  Assert(g_bInDrawing);
  info.valid = false;
  ...
  if (!g_FontTextureCache.GetTextureForChar(m_hCurrentFont, info.drawType, ch,
                                            &info.textureId, &texCoords)) {
    info.valid = false;
    return info.valid;
  }
  ...
  InitVertex(info.verts[1], info.x + fontWide, info.y + info.fontTall,
...
void CMatSystemSurface::DrawRenderCharInternal(const CharRenderInfo &info) {
  ...
  if (m_DrawTextColor[3] == 0) return;

  vgui::Vertex_t clippedRect[2];

  if (info.shouldclip) {
    // Fully clipped?
    if (!ClipRect(info.verts[0], info.verts[1], &clippedRect[0],
                  &clippedRect[1]))
      return;
  } else {
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawPrintText(
    const wchar_t *text, int iTextLen,
    FontDrawType_t drawType /*= FONT_DRAW_DEFAULT */) {
  Assert(g_bInDrawing);
  if (!text) return;

  if (!m_hCurrentFont) return;
  ...
  for (int i = 0; i < iTextLen; ++i) {
    ...
    if (!iswspace(ch) || bUnderlined) {
      ...
      int iTexId = 0;
      float *texCoords = NULL;
      if (!g_FontTextureCache.GetTextureForChar(m_hCurrentFont, drawType, ch,
                                                &iTexId, &texCoords))
        continue;
      ...
      if (iTexId != iLastTexId) {
        ...
        if (iCount) {
          IMaterial *pMaterial =
              TextureDictionary()->GetTextureMaterial(iLastTexId);
          InternalSetMaterial(pMaterial);
...
void CMatSystemSurface::OnScreenSizeChanged(int nOldWidth, int nOldHeight) {
  ...
  ivgui()->PostMessage(panel,
                       new KeyValues("OnScreenSizeChanged", "oldwide",
                                     nOldWidth, "oldtall", nOldHeight),
                       NULL);
}
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetWorkspaceBounds(int &x, int &y, int &iWide,
                                           int &iTall) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CMatSystemSurface::SetWorkspaceInsets(int left, int top, int right,
                                           int bottom) {
...
void CMatSystemSurface::SetAsToolBar(VPANEL panel, bool state)  // removes the
                                                                // window's task
                                                                // bar entry
                                                                // (for context
                                                                // menu's, etc.)
...
// notify icons?!?
VPANEL CMatSystemSurface::GetNotifyPanel() { return NULL; }

void CMatSystemSurface::SetNotifyIcon(VPANEL context, HTexture icon,
                                      VPANEL panelToReceiveMessages,
                                      const char *text) {}

bool CMatSystemSurface::IsWithin(int x, int y) { return true; }
...
bool CMatSystemSurface::RecreateContext(VPANEL panel) { return false; }

//-----------------------------------------------------------------------------
// Focus-related methods
//-----------------------------------------------------------------------------
bool CMatSystemSurface::HasFocus() { return true; }
...
void CMatSystemSurface::SetTopLevelFocus(VPANEL pSubFocus) {
  ...
  while (pSubFocus) {
    if (ipanel()->IsPopup(pSubFocus) &&
        ipanel()->IsMouseInputEnabled(pSubFocus)) {
...
void CMatSystemSurface::PlaySound(const char *pFileName) {
  if (m_PlaySoundFunc) m_PlaySoundFunc(pFileName);
}
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::CreatePopup(VPANEL panel, bool minimized,
                                    bool showTaskbarIcon, bool disabled,
                                    bool mouseInput, bool kbInput) {
...
void CMatSystemSurface::ReleasePanel(VPANEL panel) {
  // Remove from popup list if needed and remove any dead popups while we're at
  // it
  RemovePopup(panel);
...
//-----------------------------------------------------------------------------
int CMatSystemSurface::GetPopupCount() { return m_PopupList.Count(); }
...
void CMatSystemSurface::ResetPopupList() { m_PopupList.RemoveAll(); }
...
void CMatSystemSurface::AddPopupsToList(VPANEL panel) {
  if (!g_pVGuiPanel->IsVisible(panel)) return;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::InternalSchemeSettingsTraverse(
    VPANEL panel, bool forceApplySchemeSettings) {
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::SolveTraverse(VPANEL panel,
...
void CMatSystemSurface::RestrictPaintToSinglePanel(VPANEL panel) {
  if (panel && m_pRestrictedPanel &&
      m_pRestrictedPanel == input()->GetAppModalSurface()) {
    return;  // don't restrict drawing to a panel other than the modal one -
             // that's a good way to hang the game.
  }

  m_pRestrictedPanel = panel;

  if (!input()->GetAppModalSurface()) {
    input()->SetAppModalSurface(panel);  // if painting is restricted to this
                                         // panel, it had better be modal, or
                                         // else you can get in some bad
                                         // state...
  }
}
...
bool CMatSystemSurface::IsPanelUnderRestrictedPanel(VPANEL panel) {
  if (!m_pRestrictedPanel) return true;

  while (panel) {
    if (panel == m_pRestrictedPanel) return true;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::PaintTraverseEx(VPANEL panel,
                                        bool paintPopups /*= false*/) {
  ...
  // NOTE You might expect we'd have to draw these under the popups so they
  // would occlude them, but there are a few things we do have to draw on top
  // of, esp. the black panel that draws over the top of the engine to darken
  // everything.
  m_flZPos = 0.0f;
  ...
  if (paintPopups) {
    ...
    // since depth-test and depth-write are on, the front panels will occlude
    // the underlying ones
    {
      ...
      for (int i = popups - 1; i >= 0; --i) {
        ...
        if (!popupPanel) continue;

        if (!ipanel()->IsFullyVisible(popupPanel)) continue;

        if (!IsPanelUnderRestrictedPanel(popupPanel)) continue;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::Begin3DPaint(int iLeft, int iTop, int iRight,
                                     int iBottom) {
  Assert(iRight > iLeft);
  Assert(iBottom > iTop);
  ...
  // For 3d painting, use the off-screen render target the material system
  // allocates NOTE: We have to grab it here, as opposed to during init, because
  // the mode hasn't been set by now.
  if (!m_FullScreenBuffer) {
    m_FullScreenBuffer.Init(
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetFullScreenTexCoords(int x, int y, int w, int h,
                                               float *pMinU, float *pMinV,
                                               float *pMaxU, float *pMaxV) {
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawFullScreenBuffer(int nLeft, int nTop, int nRight,
                                             int nBottom) {
  ...
  if (m_nFullScreenBufferMaterialId == -1) {
    ...
    DrawSetTextureMaterial(m_nFullScreenBufferMaterialId,
                           m_FullScreenBufferMaterial);
  }
  ...
  float u0, u1, v0, v1;
  GetFullScreenTexCoords(0, 0, nRight - nLeft, nBottom - nTop, &u0, &v0, &u1,
                         &v1);
...
//-----------------------------------------------------------------------------
float CMatSystemSurface::GetZPos() const { return m_flZPos; }
...
void CMatSystemSurface::DrawColoredCircle(int centerx, int centery,
  ...
  for (int i = 0; i < CIRCLE_POINTS; i++) {
    ...
    // Draw the point, if it's not on the previous point, to avoid smaller
    // circles being brighter
...
//-----------------------------------------------------------------------------
int CMatSystemSurface::DrawColoredText(vgui::HFont font, int x, int y, int r,
                                       int g, int b, int a, const char *fmt,
                                       va_list argptr) {
  Assert(g_bInDrawing);
  int len;
  char data[1024];
...
//-----------------------------------------------------------------------------
// Draws text with current font at position and wordwrapped to the rect using
// color values specified
//-----------------------------------------------------------------------------
void CMatSystemSurface::SearchForWordBreak(vgui::HFont font, char *text,
                                           int &chars, int &pixels) {
  chars = pixels = 0;
  while (1) {
    char ch = text[chars];
    int a, b, c;
...
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawTextHeight(vgui::HFont font, int w, int &h,
                                       char *fmt, ...) {
  if (!font) return;
...
// color values specified
//-----------------------------------------------------------------------------
void CMatSystemSurface::DrawColoredTextRect(vgui::HFont font, int x, int y,
...
//-----------------------------------------------------------------------------
bool CMatSystemSurface::IsCursorLocked() const { return ::IsCursorLocked(); }
...
void CMatSystemSurface::MovePopupToFront(VPANEL panel) {
  ...
  if (index == m_PopupList.InvalidIndex()) return;

  m_PopupList.Remove(index);
  m_PopupList.AddToTail(p);

  // If the modal panel isn't a parent, restore it to the top, to prevent a hard
  // lock
  if (input()->GetAppModalSurface()) {
...
bool CMatSystemSurface::IsInThink(VPANEL panel) {
  if (m_bInThink) {
    if (panel == m_CurrentThinkPanel)  // HasParent() returns true if you pass
                                       // yourself in
    {
      return false;
    }
...
IHTML *CMatSystemSurface::CreateHTMLWindow(vgui::IHTMLEvents *events,
                                           VPANEL context) {
#if defined(ENABLE_HTMLWINDOW)
  HtmlWindow *IE =
      new HtmlWindow(events, context, reinterpret_cast<HWND>(m_HWnd),
                     m_bAllowJavaScript, false);
  IE->Show(false);
...
void CMatSystemSurface::DrawSetTextureRGBA(int id, const unsigned char *rgba,
                                           int wide, int tall,
                                           int hardwareFilter,
                                           bool forceUpload) {
...
void CMatSystemSurface::DrawSetTextureRGBAEx(int id, const unsigned char *rgba,
                                             int wide, int tall,
                                             ImageFormat format) {
  TextureDictionary()->SetTextureRGBAEx(id, (const char *)rgba, wide, tall,
                                        format);
}

void CMatSystemSurface::DrawSetSubTextureRGBA(int textureID, int drawX,
                                              int drawY,
                                              unsigned const char *rgba,
                                              int subTextureWide,
                                              int subTextureTall) {
  TextureDictionary()->SetSubTextureRGBA(textureID, drawX, drawY, rgba,
                                         subTextureWide, subTextureTall);
}

void CMatSystemSurface::SetModalPanel(VPANEL) {}

VPANEL CMatSystemSurface::GetModalPanel() { return 0; }

void CMatSystemSurface::UnlockCursor() { ::LockCursor(false); }

void CMatSystemSurface::LockCursor() { ::LockCursor(true); }

void CMatSystemSurface::SetTranslateExtendedKeys(bool state) {}

VPANEL CMatSystemSurface::GetTopmostPopup() { return 0; }

//-----------------------------------------------------------------------------
// Purpose: gets the absolute coordinates of the screen (in screen space)
//-----------------------------------------------------------------------------
void CMatSystemSurface::GetAbsoluteWindowBounds(int &x, int &y, int &wide,
                                                int &tall) {
...
bool CMatSystemSurface::NeedKBInput() { return m_bNeedsKeyboard; }
...
// Output : vgui::HCursor
//-----------------------------------------------------------------------------
vgui::HCursor CMatSystemSurface::CreateCursorFromFile(char const *curOrAniFile,
                                                      char const *pPathID) {
...
static bool GetIconBits(HDC hdc, ICONINFO &iconInfo, int &w, int &h,
                        unsigned char *rgba, size_t &bufsize) {
  if (!iconInfo.hbmColor || !iconInfo.hbmMask) return false;

  if (!rgba) {
    if (!GetIconSize(iconInfo, w, h)) return false;
  ...
  if (GetDIBits(hdc, iconInfo.hbmMask, 0, h, maskData, &bmInfo,
                DIB_RGB_COLORS) == h &&
      GetDIBits(hdc, iconInfo.hbmColor, 0, h, colorData, &bmInfo,
                DIB_RGB_COLORS) == h) {
    bret = true;
...
static bool ShouldMakeUnique(char const *extension) {
  if (!Q_stricmp(extension, "cur")) return true;
  if (!Q_stricmp(extension, "ani")) return true;
  return false;
}
#endif  // !_X360

vgui::IImage *CMatSystemSurface::GetIconImageForFullPath(
    char const *pFullPath) {
  vgui::IImage *newIcon = NULL;

#if !defined(_X360)
  SHFILEINFO info = {0};
  DWORD_PTR dwResult = SHGetFileInfo(
      pFullPath, 0, &info, sizeof(info),
  ...
  if (dwResult) {
    if (info.szTypeName[0] != 0) {
      ...
      char lookup[512];
      Q_snprintf(lookup, sizeof(lookup), "%s",
