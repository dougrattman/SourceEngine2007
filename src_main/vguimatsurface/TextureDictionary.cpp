...
class CMatSystemTexture {
 public:
  ...
  // This is used when we want different rendering state sharing the same
  // procedural texture (fonts)
  void ReferenceOtherProcedural(CMatSystemTexture *pTexture,
                                IMaterial *pMaterial);
  ...
  void SetSubTextureRGBA(int drawX, int drawY, unsigned const char *rgba,
                         int subTextureWide, int subTextureTall);
 ...
 private:
  enum { TEXTURE_IS_PROCEDURAL = 0x1, TEXTURE_IS_REFERENCE = 0x2 };
...
class CTextureDictionary : public ITextureDictionary {
 public:
  ...
  virtual void BindTextureToMaterialReference(int id, int referenceId,
                                              IMaterial *pMaterial);
  ...
  void SetTextureRGBAEx(int id, const char *rgba, int wide, int tall,
                        ImageFormat format);
  void SetSubTextureRGBA(int id, int drawX, int drawY,
                         unsigned const char *rgba, int subTextureWide,
                         int subTextureTall);
 ...
 private:
  CUtlLinkedList<CMatSystemTexture, u16> m_Textures;
};
...
class CFontTextureRegen : public ITextureRegenerator {
 public:
  CFontTextureRegen(int nWidth, int nHeight, ImageFormat format) {
    ...
    if (IsPC()) {
      int size =
          ImageLoader::GetMemRequired(m_nWidth, m_nHeight, 1, m_nFormat, false);
  ...
  void UpdateBackingBits(Rect_t &subRect, const unsigned char *pBits) {
    ...
    if (ImageLoader::SizeInBytes(m_nFormat) == 4) {
      ...
      Assert((subRect.x + subRect.width <= m_nWidth) &&
             (subRect.y + subRect.height <= m_nHeight));
      ...
      for (y = 0; y < subRect.height; ++y) {
        ...
        const unsigned int *pSrc =
            (const unsigned int *)(&pBits[(y * subRect.width) << 2]);
  ...
  virtual void RegenerateTextureBits(ITexture *pTexture,
    ...
    if (nFormatBytes == 4) {
      ...
      for (y = pSubRect->y; y < ymax; ++y) {
        ...
        unsigned char *rgba =
            &m_pTextureBits[(y * m_nWidth + pSubRect->x) * nFormatBytes];
...
// Purpose:
//-----------------------------------------------------------------------------
CMatSystemTexture::~CMatSystemTexture(void) { CleanUpMaterial(); }
...
void CMatSystemTexture::SetTextureRGBA(const char *rgba, int wide, int tall,
  ...
  if (!m_pMaterial) {
    ...
    Q_snprintf(pTextureName, sizeof(pTextureName), "__vgui_texture_%d",
               nTextureId);
    ++nTextureId;
    ...
    ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
        ...
        TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_NOMIP |
            ...
            TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_POINTSAMPLE);
    ...
    IMaterial *pMaterial =
        g_pMaterialSystem->CreateMaterial(pTextureName, pVMTKeyValues);
    pMaterial->Refresh();
...
ITexture *CMatSystemTexture::GetTextureValue(void) {
  Assert(IsProcedural());
  if (!m_pMaterial) return NULL;

  return m_pTexture;
}

void CMatSystemTexture::SetSubTextureRGBA(int drawX, int drawY,
  ...
  if (IsX360()) {
    ...
    // the texture (commonly font page) "backing bits" are allocated during
    // UpdateBackingBits() which get blitted into by procedural regeneration in
    // preparation for download() which then subrect blits out of and into
    // target texture (d3d upload) the "backing bits" are then no longer
    // required
...
// procedural texture (fonts)
//-----------------------------------------------------------------------------
void CMatSystemTexture::ReferenceOtherProcedural(CMatSystemTexture *pTexture,
                                                 IMaterial *pMaterial) {
  // Decrement references to old texture
  CleanUpMaterial();
  ...
  Assert((pMaterial->GetMappingWidth() == m_iWide) &&
         (pMaterial->GetMappingHeight() == m_iTall));
...
void CMatSystemTexture::SetMaterial(const char *pFileName) {
  ...
  IMaterial *pMaterial =
      g_pMaterialSystem->FindMaterial(pFileName, TEXTURE_GROUP_VGUI);
...
void CTextureDictionary::SetTextureRGBA(int id, const char *rgba, int wide,
                                        int tall) {
  SetTextureRGBAEx(id, rgba, wide, tall, IMAGE_FORMAT_RGBA8888);
}

void CTextureDictionary::SetTextureRGBAEx(int id, const char *rgba, int wide,
                                          int tall, ImageFormat format) {
  if (!IsValidId(id)) {
...
void CTextureDictionary::SetSubTextureRGBA(int id, int drawX, int drawY,
                                           unsigned const char *rgba,
                                           int subTextureWide,
                                           int subTextureTall) {
  if (!IsValidId(id)) {
...
void CTextureDictionary::BindTextureToFile(int id, const char *pFileName) {
  ...
  // Reload from file if the material was never loaded, or if the filename has
  // changed at all
...
//-----------------------------------------------------------------------------
void CTextureDictionary::BindTextureToMaterialReference(int id, int referenceId,
                                                        IMaterial *pMaterial) {
...
void CTextureDictionary::GetTextureTexCoords(int id, float &s0, float &t0,
                                             float &s1, float &t1) {
  if (!IsValidId(id)) {
    s0 = t0 = 0.0f;
    s1 = t1 = 1.0f;
