...
//-----------------------------------------------------------------------------
bool CFontTextureCache::CacheEntryLessFunc(CacheEntry_t const &lhs,
                                           CacheEntry_t const &rhs) {
...
//-----------------------------------------------------------------------------
bool CFontTextureCache::GetTextureForChar(vgui::HFont font,
                                          vgui::FontDrawType_t type,
                                          wchar_t wch, int *textureID,
                                          float **texCoords) {
...
//-----------------------------------------------------------------------------
bool CFontTextureCache::GetTextureForChars(vgui::HFont font,
                                           vgui::FontDrawType_t type,
                                           wchar_t *wch, int *textureID,
                                           float **texCoords, int numChars) {
  Assert(wch && textureID && texCoords);
  Assert(numChars >= 1);

  if (type == vgui::FONT_DRAW_DEFAULT) {
    type = g_MatSystemSurface.IsFontAdditive(font)
               ? vgui::FONT_DRAW_ADDITIVE
  ...
  if (FontManager().IsBitmapFont(font)) {
    ...
    for (int i = 0; i < numChars; i++) {
      ...
      pWinFont = reinterpret_cast<CBitmapFont *>(
          FontManager().GetFontForChar(font, wch[i]));
      if (!pWinFont) {
        // bad font handle
        return false;
      }
    ...
    newPageEntry_t *newEntries =
        (newPageEntry_t *)_alloca(numChars * sizeof(newPageEntry_t));
    ...
    for (int i = 0; i < numChars; i++) {
      ...
      if (!m_CharCache.IsValidIndex(cacheHandle)) {
        ...
        if (!AllocatePageForChar(fontWide, fontTall, page, drawX, drawY, twide,
                                 ttall))
          return false;
        ...
        double adjust = 0.0f;  // the 0.5 texel offset is done in
                               // CMatSystemTexture::SetMaterial()

        cacheItem.texCoords[0] =
            (float)((double)drawX / ((double)twide + adjust));
        cacheItem.texCoords[1] =
            (float)((double)drawY / ((double)ttall + adjust));
        cacheItem.texCoords[2] =
            (float)((double)(drawX + fontWide) / (double)twide);
        cacheItem.texCoords[3] =
            (float)((double)(drawY + fontTall) / (double)ttall);
    ...
    if (numNewChars > 0) {
      ...
      for (int i = 0; i < numNewChars; i++) {
        ...
        winFont->GetCharRGBA(newChar.wch, newChar.fontWide, newChar.fontTall,
                             pRGBA);

        // upload the new sub texture
        // NOTE: both textureIDs reference the same ITexture, so we're ok)
        g_MatSystemSurface.DrawSetTexture(
            m_PageList[newEntry.page].textureID[typePage]);
        g_MatSystemSurface.DrawSetSubTextureRGBA(
            m_PageList[newEntry.page].textureID[typePage], newEntry.drawX,
            newEntry.drawY, pRGBA, newChar.fontWide, newChar.fontTall);
      }
    }
  }
...
//-----------------------------------------------------------------------------
void CFontTextureCache::CreateFontMaterials(Page_t &page,
  ...
  IMaterial *pMaterial =
      g_pMaterialSystem->CreateMaterial("__fontpage", pVMTKeyValues);
  pMaterial->Refresh();

  int typePageNonAdditive = (int)vgui::FONT_DRAW_NONADDITIVE - 1;
  g_MatSystemSurface.DrawSetTextureMaterial(page.textureID[typePageNonAdditive],
                                            pMaterial);
  ...
  pVMTKeyValues->SetString("$basetexture", pFontTexture->GetName());
  pMaterial =
      g_pMaterialSystem->CreateMaterial("__fontpage_additive", pVMTKeyValues);
  pMaterial->Refresh();

  int typePageAdditive = (int)vgui::FONT_DRAW_ADDITIVE - 1;
  if (bitmapFont) {
    g_MatSystemSurface.DrawSetTextureMaterial(page.textureID[typePageAdditive],
                                              pMaterial);
  } else {
    g_MatSystemSurface.ReferenceProceduralMaterial(
        page.textureID[typePageAdditive], page.textureID[typePageNonAdditive],
        pMaterial);
  }
  pMaterial->DecrementReferenceCount();
}
...
int CFontTextureCache::ComputePageType(int charTall) const {
  for (int i = 0; i < FONT_PAGE_SIZE_COUNT; ++i) {
    if (charTall < s_pFontPageSize[i]) return i;
  }
...
//-----------------------------------------------------------------------------
bool CFontTextureCache::AllocatePageForChar(int charWide, int charTall,
                                            int &pageIndex, int &drawX,
                                            int &drawY, int &twide,
                                            int &ttall) {
  ...
  if (bNeedsNewPage) {
    ...
    ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
        pTextureName, TEXTURE_GROUP_VGUI, newPage.wide, newPage.tall,
    ...
    int typePageNonAdditive = (int)(vgui::FONT_DRAW_NONADDITIVE)-1;
    g_MatSystemSurface.DrawSetTextureRGBA(
        newPage.textureID[typePageNonAdditive], pRGBA, newPage.wide,
        newPage.tall, false, false);
  }
