...
class CMatSystemSurface : public CTier3AppSystem<IMatSystemSurface> {
 ...
 public:
  ...
  virtual void DrawTexturedLine(const vgui::Vertex_t &a,
                                const vgui::Vertex_t &b);
  ...
  virtual vgui::IHTML *CreateHTMLWindow(vgui::IHTMLEvents *events,
                                        vgui::VPANEL context);
  ...
  virtual void SetAsToolBar(vgui::VPANEL panel,
                            bool state);  // removes the window's task bar entry
                                          // (for context menu's, etc.)

  // windows stuff
  virtual void CreatePopup(VPANEL panel, bool minimized,
                           bool showTaskbarIcon = true, bool disabled = false,
                           bool mouseInput = true, bool kbInput = true);
  ...
  virtual bool SetFontGlyphSet(vgui::HFont font, const char *windowsFontName,
                               int tall, int weight, int blur, int scanlines,
                               int flags);
  virtual bool SetBitmapFontGlyphSet(vgui::HFont font,
                                     const char *windowsFontName, float scalex,
  ...
  void DrawSetSubTextureRGBA(int textureID, int drawX, int drawY,
  ...
  virtual void SetNotifyIcon(vgui::VPANEL context, vgui::HTexture icon,
                             vgui::VPANEL panelToReceiveMessages,
                             const char *text);
  ...
  virtual int DrawColoredText(vgui::HFont font, int x, int y, int r, int g,
                              int b, int a, const char *fmt, ...);
  virtual void DrawColoredTextRect(vgui::HFont font, int x, int y, int w, int h,
  ...
  virtual void DrawPanelIn3DSpace(vgui::VPANEL pRootPanel,
                                  const VMatrix &panelCenterToWorld, int pw,
                                  int ph, float sw, float sh);

  // Only visible within vguimatsurface
  void DrawSetTextureMaterial(int id, IMaterial *pMaterial);
  void ReferenceProceduralMaterial(int id, int referenceId,
                                   IMaterial *pMaterial);
 ...
 private:
  ...
  // Helper method to initialize vertices (transforms them into screen space
  // too)
  void InitVertex(vgui::Vertex_t &vertex, int x, int y, float u, float v);

  // Draws a quad + quad array
  void DrawQuad(const vgui::Vertex_t &ul, const vgui::Vertex_t &lr,
                unsigned char *pColor);
  void DrawQuadArray(int numQuads, vgui::Vertex_t *pVerts,
                     unsigned char *pColor);

  // Necessary to wrap the rendering
  void StartDrawing(void);
  void StartDrawingIn3DSpace(const VMatrix &screenToWorld, int pw, int ph,
                             float sw, float sh);
  ...
  int DrawColoredText(vgui::HFont font, int x, int y, int r, int g, int b,
                      int a, const char *fmt, va_list argptr);
  void SearchForWordBreak(vgui::HFont font, char *text, int &chars,
                          int &pixels);
  ...
  void GetFullScreenTexCoords(int x, int y, int w, int h, float *pMinU,
                              float *pMinV, float *pMaxU, float *pMaxV);
  ...
  // List of pop-up panels based on the type enum above (draw order vs last
  // clicked)
