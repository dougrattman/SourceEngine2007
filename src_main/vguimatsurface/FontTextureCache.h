...
class CFontTextureCache {
 public:
  ...
  // returns a texture ID and a pointer to an array of 4 texture coords for the
  // given character & font generates+uploads more texture if necessary
  bool GetTextureForChar(vgui::HFont font, vgui::FontDrawType_t type,
                         wchar_t wch, int *textureID, float **texCoords);

  // for each character in an array (not assumed to be a NULL-terminated
  // string), returns a texture ID and a pointer to an array of 4 texture coords
  // for the given character & font generates+uploads more texture if necessary
  bool GetTextureForChars(vgui::HFont font, vgui::FontDrawType_t type,
                          wchar_t *wch, int *textureID, float **texCoords,
                          int numChars = 1);
 ...
 private:
  ...
  bool AllocatePageForChar(int charWide, int charTall, int &pageIndex,
                           int &drawX, int &drawY, int &twide, int &ttall);
  ...
  static bool CacheEntryLessFunc(const CacheEntry_t &lhs,
