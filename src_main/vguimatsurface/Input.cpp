...
//-----------------------------------------------------------------------------
static LRESULT CALLBACK MatSurfaceWindowProc(HWND hwnd, UINT uMsg,
                                             WPARAM wParam, LPARAM lParam) {
  if (!s_bInputEnabled) goto chainWndProc;
  ...
  switch (uMsg) {
    ...
    case WM_CLOSE:
      ...
      if ((LONG)GetWindowLongPtr(hwnd, GWLP_WNDPROC) ==
          (LONG)MatSurfaceWindowProc) {
    ...
    case WM_IME_NOTIFY: {
      switch (wParam) {
        ...
        case IMN_SETOPENSTATUS:
          ...
          g_pInputSystem->PostUserEvent(event);
          if (wParam == IMN_SETOPENSTATUS) return 0;
          break;
    ...
    case WM_IME_CHAR:
      // We need to process this message so that the IME doesn't double convert
      // the unicode IME characters into garbage characters and post
...
//-----------------------------------------------------------------------------
void EnableInput(bool bEnable) { s_bInputEnabled = bEnable; }
