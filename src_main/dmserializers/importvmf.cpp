...
class CImportVMF : public CImportKeyValueBase {
 ...
 private:
  ...
  bool SerializeOther(CUtlBuffer &buf, CDmAttribute *pOther,
                      const char **ppFilter = 0);
  ...
  bool SerializeAttribute(CUtlBuffer &buf, CDmAttribute *pAttribute,
                          bool bElementArrays);
...
// Writes out a single attribute recursively
//-----------------------------------------------------------------------------
bool CImportVMF::SerializeAttribute(CUtlBuffer &buf, CDmAttribute *pAttribute,
                                    bool bElementArrays) {
  if (pAttribute->IsFlagSet(FATTRIB_STANDARD | FATTRIB_DONTSAVE)) return true;

  const char *pFieldName = GetRemapName(pAttribute->GetName(), true);
  if (!Q_stricmp(pFieldName, "editorType")) return true;
  ...
  if (!IsArrayType(pAttribute->GetType())) {
    ...
    if (bElementArrays) {
      ...
      if (!SerializeOther(buf, pAttribute)) return false;
    }
  }
...
// Writes out all everything other than entities
//-----------------------------------------------------------------------------
bool CImportVMF::SerializeOther(CUtlBuffer &buf, CDmAttribute *pOther,
                                const char **ppFilter) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (ppFilter) {
      ...
      for (j = 0; ppFilter[j]; ++j) {
        if (!Q_stricmp(pElementName, ppFilter[j])) break;
      }

      if (!ppFilter[j]) continue;
    }
    ...
    for (CDmAttribute *pAttribute = pElement->FirstAttribute(); pAttribute;
      ...
      if (!SerializeAttribute(buf, pAttribute, false)) return false;
    }

    // Subkeys later
    for (CDmAttribute *pAttribute = pElement->FirstAttribute(); pAttribute;
         pAttribute = pAttribute->NextAttribute()) {
      if (!SerializeAttribute(buf, pAttribute, true)) return false;
    }
...
// Writes entity editor keys
//-----------------------------------------------------------------------------
bool CImportVMF::SerializeEntityEditorKey(CUtlBuffer &buf,
                                          DmElementHandle_t hEditor) {
  CDmElement *pEditorElement = g_pDataModel->GetElement(hEditor);
  if (!pEditorElement) return true;

  buf.Printf("editor\n");
  buf.Printf("{\n");
  buf.PushTab();

  CDmAttribute *colorAttribute = pEditorElement->GetAttribute("color");
  if (colorAttribute) {
    Color c = colorAttribute->GetValue<Color>();
  ...
  for (CDmAttribute *pAttribute = pEditorElement->FirstAttribute();
       pAttribute != NULL; pAttribute = pAttribute->NextAttribute()) {
    if (pAttribute->IsFlagSet(FATTRIB_STANDARD | FATTRIB_DONTSAVE)) continue;

    const char *pKeyName = pAttribute->GetName();
    if (Q_stricmp(pKeyName, "color") && Q_stricmp(pKeyName, "id") &&
        Q_stricmp(pKeyName, "comments") &&
        Q_stricmp(pKeyName, "visgroupshown") &&
...
bool CImportVMF::SerializeEntities(CUtlBuffer &buf, CDmAttribute *pEntities) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    for (CDmAttribute *pAttribute = pElement->FirstAttribute(); pAttribute;
      ...
      if (!Q_stricmp(pAttribute->GetName(), "editor")) continue;
...
bool CImportVMF::Serialize(CUtlBuffer &buf, CDmElement *pRoot) {
  // This is done in this strange way (namely, serializing other twice) to
  // minimize diffs
  const char *pOtherFilter1[] = {"versioninfo", "visgroups", "viewsettings",
                                 "world", NULL};

  const char *pOtherFilter2[] = {"cameras", "cordon", "hidden", NULL};

  CDmAttribute *pOther = pRoot->GetAttribute("other");
  if (pOther && pOther->GetType() == AT_ELEMENT_ARRAY) {
    if (!SerializeOther(buf, pOther, pOtherFilter1)) return false;
  }

  // Serialize entities
  CDmAttribute *pEntities = pRoot->GetAttribute("entities");
  if (pEntities && pEntities->GetType() == AT_ELEMENT_ARRAY) {
    if (!SerializeEntities(buf, pEntities)) return false;
  }

  if (pOther && pOther->GetType() == AT_ELEMENT_ARRAY) {
    if (!SerializeOther(buf, pOther, pOtherFilter2)) return false;
  }
...
// Reads entity editor keys
//-----------------------------------------------------------------------------
bool CImportVMF::UnserializeEntityEditorKey(CDmAttribute *pEditorAttribute,
  ...
  for (KeyValues *pUserKey = pKeyValues->GetFirstValue(); pUserKey != NULL;
       pUserKey = pUserKey->GetNextValue()) {
...
// Reads a single entity
//-----------------------------------------------------------------------------
bool CImportVMF::UnserializeEntityKey(CDmAttribute *pEntities,
                                      KeyValues *pKeyValues) {
  CDmElement *pEntity =
      CreateDmElement("DmeVMFEntity", pKeyValues->GetString("id", "-1"), NULL);
  if (!pEntity) return false;

  CDmrElementArray<> array(pEntities);
  array.AddToTail(pEntity);

  // Each act busy needs to have an editortype associated with it so it displays
  // nicely in editors
  ...
  for (KeyValues *pField = pKeyValues->GetFirstValue(); pField != NULL;
    ...
    Assert(Q_stricmp(pFieldName, "type") && Q_stricmp(pFieldName, "name") &&
           Q_stricmp(pFieldName, "editortype"));

    switch (pField->GetDataType()) {
      case KeyValues::TYPE_INT:
        if (!AddIntAttributeFlags(pEntity, pKeyValues, pFieldName,
                                  FATTRIB_USERDEFINED))
          return false;
        break;

      case KeyValues::TYPE_FLOAT:
        if (!AddFloatAttributeFlags(pEntity, pKeyValues, pFieldName,
                                    FATTRIB_USERDEFINED))
          return false;
        break;
      ...
      case KeyValues::TYPE_STRING: {
        ...
        if (!pString || !pString[0]) return false;
        ...
        if (sscanf_s(pString, "%f %f %f %f", &v.x, &v.y, &v.z, &v.w) == 4) {
          ...
          if (!AddStringAttributeFlags(pEntity, pKeyValues, pFieldName,
                                       FATTRIB_USERDEFINED))
            return false;
        }
      } break;
  ...
  CDmrElementArray<> otherKeys(
      pEntity->AddAttribute("other", AT_ELEMENT_ARRAY));
  for (KeyValues *pSubKey = pKeyValues->GetFirstTrueSubKey(); pSubKey != NULL;
    ...
    if (!Q_stricmp(pSubKey->GetName(), "editor")) {
      ...
      CDmElement *pOther =
          CreateDmElement("DmElement", pSubKey->GetName(), NULL);
...
// Reads keys that we currently do nothing with
//-----------------------------------------------------------------------------
bool CImportVMF::UnserializeUnusedKeys(DmElementHandle_t hOther,
                                       KeyValues *pKeyValues) {
  ...
  for (KeyValues *pSubKey = pKeyValues->GetFirstTrueSubKey(); pSubKey != NULL;
       pSubKey = pSubKey->GetNextTrueSubKey()) {
    CDmElement *pSubElement =
        CreateDmElement("DmElement", pSubKey->GetName(), NULL);
    subKeys.AddToTail(pSubElement);
    if (!UnserializeUnusedKeys(pSubElement->GetHandle(), pSubKey)) return false;
  }
  return true;
}
...
//-----------------------------------------------------------------------------
bool CImportVMF::UnserializeCordonKey( IDmAttributeElement *pCordon, KeyValues
*pKeyValues )
{
...
CDmElement *CImportVMF::UnserializeFromKeyValues(KeyValues *pKeyValues) {
  ...
  // Each vmf needs to have an editortype associated with it so it displays
  // nicely in editors
  ...
  CDmAttribute *pEntityArray =
      pElement->AddAttribute("entities", AT_ELEMENT_ARRAY);

  // All main keys are root keys
  CDmrElementArray<> otherKeys(
      pElement->AddAttribute("other", AT_ELEMENT_ARRAY));
  for (; pKeyValues != NULL; pKeyValues = pKeyValues->GetNextKey()) {
    ...
    if (!Q_stricmp(pKeyValues->GetName(), "entity")) {
      ...
      CDmElement *pOther =
          CreateDmElement("DmElement", pKeyValues->GetName(), NULL);
      otherKeys.AddToTail(pOther);
