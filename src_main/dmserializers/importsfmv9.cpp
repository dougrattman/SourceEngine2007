...
// creating the updater between 3 and 4 fortunately, since all updates happen on
// untyped elements, it's reasonably safe to do this out of order
...
void CImportSFMV9::FixupElement(CDmElement *pElement) {
  ...
  if (!V_stricmp(pType, "DmeLight") ||
      ...
      !V_stricmp(pType, "DmePointLight") || !V_stricmp(pType, "DmeSpotLight") ||
    ...
    {  // scoping this section of code since vecColor is invalid after
       // RemoveAttribute
...
// Fixes up all elements
//-----------------------------------------------------------------------------
void CImportSFMV9::BuildList(CDmElement *pElement,
