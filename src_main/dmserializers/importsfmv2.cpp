...
template <class T>
void CopyValues(int layerType, CDmElement *pElement, CDmElement *pLayer,
                CDmAttribute *pInTimeAttribute,
                CDmAttribute *pInCurveTypeAttribute) {
  ...
  CDmrArray<T> outValues(
      pLayer->AddAttribute("values", (DmAttributeType_t)layerType));
...
void CImportSFMV2::FixupElement(CDmElement *pElement) {
  ...
  if (layerType != AT_UNKNOWN) {
    ...
    */
    ...
    CDmrElementArray<> layers(
        pElement->AddAttribute("layers", AT_ELEMENT_ARRAY));
    CDmElement *layer = NULL;
    if (layers.Count() == 0) {
      DmElementHandle_t hElement = g_pDataModel->CreateElement(
          GetLogLayerType(layerType), GetLogLayerType(layerType),
          pElement->GetFileId());
...
// Fixes up all elements
//-----------------------------------------------------------------------------
void CImportSFMV2::BuildList(CDmElement *pElement,
