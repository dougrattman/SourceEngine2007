...
void CImportActBusy::SerializeHeader(CUtlBuffer &buf) {
  buf.Printf(
      "// \"act busy name\"\t\tThis is the name that a mapmaker must specify "
      "in the hint node.\n");
  buf.Printf("// {\n");
  ...
  buf.Printf(
      "// \t\"busy_sequence\"\t\t\"Sequence Name\". If specified, this is used "
      "over the activity name. Specify it in the hint node.\n");
  buf.Printf(
      "// \t\"entry_sequence\"\t\"Sequence Name\". If specified, this is used "
      "over the entry anim.\n");
  buf.Printf(
      "// \t\"exit_sequence\"\t\t\"Sequence Name\". If specified, this is used "
      "over the exit anim.\n");
  buf.Printf(
      "// \t\"min_time\"\t\t\t\"Minimum time to spend in this busy anim\"\n");
  buf.Printf(
      "// \t\"max_time\"\t\t\t\"Maximum time to spend in this busy "
      "anim\"	0 = only stop when interrupted by external event\n");
  buf.Printf("// \t\"interrupts\"\t\tOne of:\n");
  buf.Printf(
      "// \t\t\t\t\t\t\"BA_INT_NONE\"\t\tbreak out only when time runs out. No "
      "external influence will break me out.\n");
  buf.Printf(
      "// \t\t\t\t\t\t\"BA_INT_DANGER\"\t\tbreak out of this anim only if "
      "threatened\n");
  buf.Printf(
      "// \t\t\t\t\t\t\"BA_INT_PLAYER\"\t\tbreak out of this anim if I can see "
      "the player, or I'm threatened\n");
  buf.Printf(
      "// \t\t\t\t\t\t\"BA_INT_AMBUSH\"\t\tsomeone please define this - I have "
      "no idea what it does\n");
  buf.Printf(
      "// \t\t\t\t\t\t\"BA_INT_COMBAT\"\t\tbreak out of this anim if combat "
      "occurs in my line of sight (bullet hits, grenades, etc), -OR- the max "
      "time is reached\n");
  buf.Printf("// }\n");
  buf.Printf("//\n");
}
...
bool CImportActBusy::Serialize(CUtlBuffer &buf, CDmElement *pRoot) {
  ...
  if (!pChildren || pChildren->GetType() != AT_ELEMENT_ARRAY) return NULL;
...
// Reads a single element
//-----------------------------------------------------------------------------
bool CImportActBusy::UnserializeActBusyKey(CDmAttribute *pChildren,
                                           KeyValues *pKeyValues) {
  CDmElement *pActBusy =
      CreateDmElement("DmElement", pKeyValues->GetName(), NULL);
  if (!pActBusy) return false;
...
CDmElement *CImportActBusy::UnserializeFromKeyValues(KeyValues *pKeyValues) {
  ...
  // Each act busy list needs to have an editortype associated with it so it
  // displays nicely in editors
  pElement->SetValue("editorType", "actBusyList");

  // All actbusy keys are elements of a single element array attribute
  // 'children'
  CDmAttribute *pChildren =
      pElement->AddAttribute("children", AT_ELEMENT_ARRAY);
  if (!pChildren) return NULL;

  // Under the root are all the actbusy keys
  for (KeyValues *pActBusyKey = pKeyValues->GetFirstTrueSubKey();
       pActBusyKey != NULL; pActBusyKey = pActBusyKey->GetNextTrueSubKey()) {
