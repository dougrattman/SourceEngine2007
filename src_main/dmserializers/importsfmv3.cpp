...
const char *GetCurveInfoTypeFromLogType(const char *pLogType) {
  for (const auto &type : g_typeMap) {
    if (!Q_stricmp(pLogType, type.pLogType)) return type.pCurveInfoType;
  }
  return NULL;
}
...
bool IsLogLayerType(const char *pLogLayerType) {
  for (const auto &type : g_typeMap) {
    if (!Q_stricmp(pLogLayerType, type.pLogLayerType)) return true;
  }
  return false;
}

void MoveAttribute(CDmElement *pFromElement, const char *pFromAttrName,
                   CDmElement *pToElement = NULL,
                   const char *pToAttrName = NULL,
                   DmAttributeType_t toType = AT_UNKNOWN) {
  if (!pToAttrName) {
  ...
  if (pToElement) {
    ...
    if (!pToAttr) {
      ...
      Warning(
          "*** Element \"%s\" has attribute \"%s\" with an unexpected type!\n",
...
void CImportSFMV3::FixupElement(CDmElement *pElement) {
  ...
  if (pUseCurveTypeAttr && pUseCurveTypeAttr->GetValue<bool>()) {
    DmElementHandle_t hElement = g_pDataModel->CreateElement(
        "curve info", pCurveInfoType, pElement->GetFileId());
    pCurveInfo = g_pDataModel->GetElement(hElement);
  }
  pElement->RemoveAttribute("usecurvetypes");

  MoveAttribute(pElement, "defaultcurvetype", pCurveInfo, "defaultCurveType",
                AT_INT);
  MoveAttribute(pElement, "defaultedgezerovalue", pCurveInfo,
                "defaultEdgeZeroValue");
  ...
  MoveAttribute(pElement, "left_edge_active", pCurveInfo, "leftEdgeActive",
                AT_BOOL);
  MoveAttribute(pElement, "right_edge_active", pCurveInfo, "rightEdgeActive",
                AT_BOOL);
  MoveAttribute(pElement, "left_edge_curvetype", pCurveInfo,
                "leftEdgeCurveType", AT_INT);
  MoveAttribute(pElement, "right_edge_curvetype", pCurveInfo,
                "rightEdgeCurveType", AT_INT);
...
// Fixes up all elements
//-----------------------------------------------------------------------------
void CImportSFMV3::BuildList(CDmElement *pElement,
                             CUtlRBTree<CDmElement *, int> &list) {
  if (!pElement) return;
...
bool CImportSFMV3::DoFixup(CDmElement *pSourceRoot) {
  ...
  for (int i = fixlist.FirstInorder(); i != fixlist.InvalidIndex();
       i = fixlist.NextInorder(i)) {
