...
//-----------------------------------------------------------------------------
CDmElement *CImportKeyValueBase::CreateDmElement(const char *pElementType,
                                                 const char *pElementName,
                                                 DmObjectId_t *pId) {
  // See if we can create an element of that type
  DmElementHandle_t hElement = g_pDataModel->CreateElement(
      pElementType, pElementName, DMFILEID_INVALID, pId);
...
//-----------------------------------------------------------------------------
void CImportKeyValueBase::PrintBoolAttribute(CDmElement *pElement,
                                             CUtlBuffer &outBuf,
                                             const char *pKeyName) {
...
void CImportKeyValueBase::PrintIntAttribute(CDmElement *pElement,
                                            CUtlBuffer &outBuf,
                                            const char *pKeyName) {
  if (pElement->HasAttribute(pKeyName)) {
    CDmAttribute *pAttribute = pElement->GetAttribute(pKeyName);
...
void CImportKeyValueBase::PrintFloatAttribute(CDmElement *pElement,
                                              CUtlBuffer &outBuf,
                                              const char *pKeyName) {
  if (pElement->HasAttribute(pKeyName)) {
    CDmAttribute *pAttribute = pElement->GetAttribute(pKeyName);
...
void CImportKeyValueBase::PrintStringAttribute(CDmElement *pElement,
                                               CUtlBuffer &outBuf,
                                               const char *pKeyName,
...
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::AddBoolAttribute(CDmElement *pElement,
...
// Used to add typed attributes from keyvalues
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::AddIntAttribute(CDmElement *pElement,
                                          KeyValues *pKeyValues,
                                          const char *pKeyName, int *pDefault) {
  KeyValues *pKey = pKeyValues->FindKey(pKeyName);
  int nValue;
  if (pKey) {
    nValue = pKey->GetInt();
...
bool CImportKeyValueBase::AddFloatAttribute(CDmElement *pElement,
                                            KeyValues *pKeyValues,
                                            const char *pKeyName,
                                            float *pDefault) {
  KeyValues *pKey = pKeyValues->FindKey(pKeyName);
...
bool CImportKeyValueBase::AddStringAttribute(CDmElement *pElement,
                                             KeyValues *pKeyValues,
                                             const char *pKeyName,
                                             const char *pDefault) {
  KeyValues *pKey = pKeyValues->FindKey(pKeyName);
...
// Used to add typed attributes from keyvalues
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::AddBoolAttributeFlags(CDmElement *pElement,
                                                KeyValues *pKeyValue,
                                                const char *pKeyName,
                                                int nFlags, bool *pDefault) {
  if (!AddBoolAttribute(pElement, pKeyValue, pKeyName, pDefault)) return false;
...
bool CImportKeyValueBase::AddIntAttributeFlags(CDmElement *pElement,
                                               KeyValues *pKeyValue,
                                               const char *pKeyName, int nFlags,
                                               int *pDefault) {
  if (!AddIntAttribute(pElement, pKeyValue, pKeyName, pDefault)) return false;
...
bool CImportKeyValueBase::AddFloatAttributeFlags(CDmElement *pElement,
                                                 KeyValues *pKeyValue,
                                                 const char *pKeyName,
                                                 int nFlags, float *pDefault) {
  if (!AddFloatAttribute(pElement, pKeyValue, pKeyName, pDefault)) return false;
...
bool CImportKeyValueBase::AddStringAttributeFlags(CDmElement *pElement,
                                                  KeyValues *pKeyValue,
                                                  const char *pKeyName,
                                                  int nFlags,
                                                  const char *pDefault) {
...
void CImportKeyValueBase::RecursivelyResolveElement(CDmElement *pElement) {
  ...
  while (pAttribute) {
    switch (pAttribute->GetType()) {
      case AT_ELEMENT: {
        CDmElement *atElement = pAttribute->GetValueElement<CDmElement>();
        RecursivelyResolveElement(atElement);
      } break;
      ...
      case AT_ELEMENT_ARRAY: {
        ...
        for (int i = 0; i < nCount; ++i) {
          CDmElement *arElement = array[i];
          RecursivelyResolveElement(arElement);
        }
      } break;
    }
...
//-----------------------------------------------------------------------------
bool CImportKeyValueBase::Unserialize(
