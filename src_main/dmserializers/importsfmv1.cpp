...
class CImportSFMV1 : public CSFMBaseImporter {
 ...
 private:
  ...
  // Fixes up a single time attribute - converting from float seconds to int
  // tenths-of-a-millisecond
  void ConvertTimeAttribute(CDmElement *pElementInternal, const char *pOldName,
                            const char *pNewName);

  // Fixes up a single timeframe
  void FixupTimeframe(CDmElement *pElementInternal);

  // Fixes up a single log - converting from int milliseconds to int
  // tenths-of-a-millisecond
...
bool CImportSFMV1::DoFixup(CDmElement *pElementInternal) {
  if (!pElementInternal) return true;
  ...
  } else if (!Q_strcmp(pType, "DmeLog") || !Q_strcmp(pType, "DmeIntLog") ||
  ...
  for (CDmAttribute *pAttribute = pElementInternal->FirstAttribute();
       pAttribute; pAttribute = pAttribute->NextAttribute()) {
...
// tenths-of-a-millisecond
//-----------------------------------------------------------------------------
void CImportSFMV1::ConvertTimeAttribute(CDmElement *pElementInternal,
                                        const char *pOldName,
                                        const char *pNewName) {
  float time = 0.0f;
  ...
  if (!pOldAttr) {
    ...
    Warning("*** Problem in file encountered!\n");
    Warning(
        "*** TimeFrame \"%s\" has attribute \"%s\" with an unexpected type "
        "(expected float)!\n",
        pElementInternal->GetName(), pOldName);
  } else {
  ...
  // this is disabled because even dmxconvert installs *some* movieobjects
  // factories, when it probably shouldn't the method of installing movieobjects
  // factories will change at some point in the future, and we can turn on this
  // safety check then
#if 0
  ...
  // ie we're just working with CDmElement subclasses, so it's safe to add
  // attributes
  pNewAttr = pElementInternal->AddAttribute(pNewName, AT_INT);
  if (!pNewAttr) {
    Assert(0);
    Warning(
        "*** Converter error - element \"%s\" already has a non-int attribute "
        "\"%s\"!\n",
        pElementInternal->GetName(), pNewName);
    return;
  }
...
void CImportSFMV1::FixupLog(CDmElement *pElementInternal) {
  ...
  if (pAttr->GetType() != AT_INT_ARRAY) {
    Warning("*** Problem in file encountered!\n");
    Warning(
        "*** Log \"%s\" has attribute \"%s\" with an unexpected type (expected "
        "int array)!\n",
        pElementInternal->GetName(), "times");
    return;
  }
  ...
  int c = array.Count();
  for (int i = 0; i < c; ++i) {
    // convert all log times from int milliseconds to int
    // tenths-of-a-millisecond
