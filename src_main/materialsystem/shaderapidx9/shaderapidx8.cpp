...
/*
DX9 todo:
-make the transforms in the older shaders match the transforms in
lightmappedgeneric -fix polyoffset for hardware that doesn't support
D3DRS_SLOPESCALEDEPTHBIAS and D3DRS_DEPTHBIAS
        - code is there, I think matrix offset just needs tweaking
-fix forcehardwaresync - implement texture locking for hardware that doesn't
support async query -get the format for GetAdapterModeCount and EnumAdapterModes
from somewhere (shaderapidx8.cpp, GetModeCount, GetModeInfo) -record frame sync
...
ConVar mat_texture_limit(
    ...
    "If this value is not -1, the material system will limit the amount of "
    "texture memory it uses in a frame."
...
struct DynamicState_t {
  ...
  float
      m_DestAlphaDepthRange;  // Dest alpha writes compress the depth to get
                              // better results. This holds the default setting
                              // that can be overriden with r_destalpharange
...
#define ADD_VERTEX_TEXTURE_FUNC(_func, _shader, _func_name, _stage, _state, \
                                _val)                                       \
...
#define SetSupportedRenderState(_state, _val)                              \
  {                                                                        \
    MSVC_SCOPED_DISABLE_WARNING(4127, if (_state != D3DRS_NOTSUPPORTED)) { \
...
struct Texture_t {
  ...
  int *
      m_pTextureGroupCounterGlobal;  // Global counter for this texture's group.
  int *m_pTextureGroupCounterFrame;  // Per-frame global counter for this
                                     // texture's group.
  ...
  enum Flags_t {
    ...
    IS_RENDER_TARGET_SURFACE =
        0x0200,  // 360: marks a render target surface target
  ...
  IDirect3DBaseTexturePtr GetTexture() {
  ...
  IDirect3DBaseTexturePtrPtr &GetTextureArray() {
    Assert(m_NumCopies > 1);
    Assert(!(m_Flags & IS_DEPTH_STENCIL));
    return m_ppTexture;
  }

  IDirect3DSurfacePtr &GetDepthStencilSurface() {
    Assert(m_NumCopies == 1);
    Assert((m_Flags & IS_DEPTH_STENCIL));
  ...
  void SetTexture(IDirect3DBaseTexturePtr pPtr) { m_pTexture = pPtr; }
  void SetTexture(int copy, IDirect3DBaseTexturePtr pPtr) {
    m_ppTexture[copy] = pPtr;
  }

  int GetMemUsage() const { return m_SizeBytes; }

  int GetWidth() const { return (int)m_Width; }

  int GetHeight() const { return (int)m_Height; }

  int GetDepth() const { return (int)m_Depth; }

  void SetImageFormat(ImageFormat format) { m_ImageFormat = format; }
 ...
 private:
  union {
    ...
    IDirect3DBaseTexture *
        *m_ppTexture;  // used when there are more than one copies
    IDirect3DSurface
        *m_pDepthStencilSurface;  // used when there's one depth stencil surface
...
//-----------------------------------------------------------------------------
class CShaderAPIDx8 : public CShaderDeviceDx8,
                      public IShaderAPIDX8,
                      public IDebugTextureInfo {
 ...
 public:
  ...
  IMesh *GetDynamicMeshEx(IMaterial *pMaterial, VertexFormat_t vertexFormat,
                          int nHWSkinBoneCount, bool bBuffered,
  ...
  void SetIntegerVertexShaderConstant(int var, int const *pVec,
                                      int numIntVecs = 1, bool bForce = false);
  ...
  virtual void BindVertexTexture(VertexTextureSampler_t nStage,
                                 ShaderAPITextureHandle_t textureHandle);
  void DeleteTexture(ShaderAPITextureHandle_t textureHandle);

  void WriteTextureToFile(ShaderAPITextureHandle_t hTexture,
  ...
  void SetRenderTarget(ShaderAPITextureHandle_t colorTextureHandle =
                       ...
                       ShaderAPITextureHandle_t depthTextureHandle =
                           SHADER_RENDERTARGET_DEPTHBUFFER);
  // Set the render target to a texID.
  // Set to SHADER_RENDERTARGET_BACKBUFFER if you want to use the regular
  // framebuffer.
  ...
  ShaderAPITextureHandle_t CreateTexture(int width, int height, int depth,
                                         ...
                                         int numMipLevels, int numCopies,
                                         int creationFlags,
  ...
  ShaderAPITextureHandle_t CreateDepthTexture(ImageFormat renderTargetFormat,
  ...
  void TexSubImage2D(int level, int cubeFaceID, int xOffset, int yOffset,
                     int zOffset, int width, int height, ImageFormat srcFormat,
                     int srcStride, bool bSrcIsTiled, void *imageData);
  ...
  void UpdateFastClipUserClipPlane();
  ...
  int GetPixelFogCombo();  // 0 is either range fog, or no fog simulated with
  ...
  TextureStageState_t &TextureStage(int stage) {
    return m_DynamicState.m_TextureStage[stage];
  }
  const TextureStageState_t &TextureStage(int stage) const {
    return m_DynamicState.m_TextureStage[stage];
  }
  SamplerState_t &SamplerState(int nSampler) {
    return m_DynamicState.m_SamplerState[nSampler];
  }
  const SamplerState_t &SamplerState(int nSampler) const {
    return m_DynamicState.m_SamplerState[nSampler];
  }

  void SetAmbientLight(float r, float g, float b);
  void SetLight(int lightNum, const LightDesc_t &desc);
  void SetLightingOrigin(const Vector &vLightingOrigin);
  ...
  void CopyRenderTargetToTextureEx(ShaderAPITextureHandle_t textureHandle,
                                   int nRenderTargetID, Rect_t *pSrcRect = NULL,
  ...
  void ApplyFogMode(ShaderFogMode_t fogMode, bool bSRGBWritesEnabled,
                    bool bDisableFogGammaCorrection);
  ...
  void SetTextureStageState(int stage, D3DTEXTURESTAGESTATETYPE state,
                            DWORD val);
  ...
  virtual int OcclusionQuery_GetNumPixelsRendered(ShaderAPIOcclusionQuery_t h,
                                                  bool bFlush);
  ...
  // For dealing with device lost in cases where Present isn't called all the
  // time (Hammer)
  ...
  virtual void GetDXLevelDefaults(uint32_t &max_dxlevel,
                                  uint32_t &recommended_dxlevel);

  virtual bool OwnGPUResources(bool bEnable);

  // ------------ New Vertex/Index Buffer interface ----------------------------
  void BindVertexBuffer(int streamID, IVertexBuffer *pVertexBuffer,
                        int nOffsetInBytes, int nFirstVertex, int nVertexCount,
                        VertexFormat_t fmt, int nRepetitions = 1);
  ...
  void DrawWithVertexAndIndexBuffers();
  // ------------ End ----------------------------
  ...
  // can be used to set the dynamic combo
 ...
 private:
  ...
  enum { INVALID_TRANSITION_OP = 0xFFFF };

  // State transition table for the device is as follows:

  // Other app init causes transition from OK to OtherAppInit, during transition
  // we must release resources !Other app init causes transition from
  // OtherAppInit to OK, during transition we must restore resources Minimized
  // or device lost or device not reset causes transition from OK to
  // LOST_DEVICE, during transition we must release resources Minimized or
  // device lost or device not reset causes transition from OtherAppInit to
  // LOST_DEVICE

  // !minimized AND !device lost causes transition from LOST_DEVICE to
  // NEEDS_RESET minimized or device lost causes transition from NEEDS_RESET to
  // LOST_DEVICE

  // Successful TryDeviceReset and !Other app init causes transition from
  // NEEDS_RESET to OK, during transition we must restore resources Successful
  // TryDeviceReset and Other app init causes transition from NEEDS_RESET to
  // OtherAppInit

  void ExportTextureList();
  void AddBufferToTextureList(const char *pName, D3DSURFACE_DESC &desc);

  void SetupTextureGroup(ShaderAPITextureHandle_t hTexture,
                         const char *pTextureGroupName);
  ...
  void ComputeStatsInfo(ShaderAPITextureHandle_t hTexture, bool isCubeMap,
  ...
  // If mat_texture_limit is enabled, then this tells us if binding the
  // specified texture would take us over the limit.
  bool WouldBeOverTextureLimit(ShaderAPITextureHandle_t hTexture);

  // Sets the texture state
  void SetTextureState(Sampler_t sampler, ShaderAPITextureHandle_t hTexture,
                       bool force = false);

  // Grab/release the internal render targets such as the back buffer and the
  // save game thumbnail
  void AcquireInternalRenderTargets();
  void ReleaseInternalRenderTargets();

  // create/release linear->gamma table texture lookups. Only used by hardware
  // supporting pixel shader 2b and up
  ...
  // Compute and save the projection atrix with polyoffset built in if we need
  // it.
  ...
  IDirect3DSurface *GetBackBufferImage(Rect_t *pSrcRect, Rect_t *pDstRect,
                                       ImageFormat &format);
  IDirect3DSurface *GetBackBufferImageHDR(Rect_t *pSrcRect, Rect_t *pDstRect,
                                          ImageFormat &format);

  // Copy bits from a host-memory surface
  void CopyBitsFromHostSurface(IDirect3DSurface *pSurfaceBits,
                               const Rect_t &dstRect, unsigned char *pData,
  ...
  void SetStandardTextureHandle(StandardTextureId_t nId,
  ...
  void CallCommitFuncs(CommitFuncType_t func, CommitShaderType_t shader,
                       bool bForce);
  bool IsCommitFuncInUse(CommitFuncType_t func, CommitShaderType_t shader,
                         int nFunc) const;
  void MarkCommitFuncInUse(CommitFuncType_t func, CommitShaderType_t shader,
                           int nFunc);
  void AddCommitFunc(CommitFuncType_t func, CommitShaderType_t shader,
                     StateCommitFunc_t f);
  void CallCommitFuncs(CommitFuncType_t func, bool bUsingFixedFunction,
                       bool bForce = false);

  // Commits transforms and lighting
  void CommitStateChanges();

  // Commits transforms that have to be dealt with on a per pass basis (ie.
  // projection matrix for polyoffset)
  ...
  // Recomputes the fast-clip plane matrices based on the current fast-clip
  // plane
  void CommitFastClipPlane();

  // Computes a matrix which includes the poly offset given an initial
  // projection matrix
  void ComputePolyOffsetMatrix(const D3DXMATRIX &matProjection,
                               D3DXMATRIX &matProjectionOffset);
  ...
  // Gets the surface associated with a texture (refcount of surface is
  // increased)
  IDirect3DSurface *GetTextureSurface(ShaderAPITextureHandle_t textureHandle);
  IDirect3DSurface *GetDepthTextureSurface(
      ShaderAPITextureHandle_t textureHandle);

  //
  // Methods related to hardware config
  //
  void SetDefaultConfigValuesForDxLevel(int dxLevelFromCaps,
                                        ShaderDeviceInfo_t &info,
                                        unsigned int nFlagsUsed);

  // Determines hardware capabilities
  bool DetermineHardwareCaps();

  // Alpha To Coverage entrypoints and states - much of this involves
  // vendor-dependent paths and states...
  bool CheckVendorDependentAlphaToCoverage();
  void EnableAlphaToCoverage();
  void DisableAlphaToCoverage();

  // Vendor-dependent shadow mapping detection
  void CheckVendorDependentShadowMappingSupport(
      bool &bSupportsShadowDepthTextures, bool &bSupportsFetch4);
  ...
  D3DCOLOR ComputeGammaCorrectedFogColor(unsigned char r, unsigned char g,
                                         unsigned char b,
                                         bool bSRGBWritesEnabled);
  ...
  // The *desired* dynamic state. Most dynamic state is committed into actual
  // hardware state at either per-pass or per-material time.	This can also be
  // used to force the hardware to match the desired state after returning from
  // alt-tab.
  DynamicState_t m_DesiredState;

  // A list of state commit functions to run as per-draw call commit time
  unsigned char m_pCommitFlags[COMMIT_FUNC_TYPE_COUNT][COMMIT_SHADER_TYPE_COUNT]
                              [COMMIT_FUNC_BYTE_COUNT];
  ...
  void SetDepthFeatheringPixelShaderConstant(int iConstant,
                                             float fDepthBlendScale);

  void SetDisallowAccess(bool b) { g_bShaderAccessDisallowed = b; }
  ...
  void ShaderLock() { g_ShaderMutex.Lock(); }

  void ShaderUnlock() { g_ShaderMutex.Unlock(); }

  // Vendor-dependent slim texture format
  ImageFormat GetNullTextureFormat();

  // The idea behind a delayed constant is this.
  // Some shaders set constants based on rendering states, and some rendering
  // states aren't updated until after a shader's already called Draw().
  //  So, for some functions that are state based, we save the constant we set
  //  and if the state changes between when it's set in the shader setup code
  //   and when the shader is drawn, we update that constant.
  struct DelayedConstants_t {
    int iPixelShaderFogParams;

    void Invalidate() { iPixelShaderFogParams = -1; }
    DelayedConstants_t() { this->Invalidate(); }
  };
...
void CShaderAPIDx8::GetDXLevelDefaults(uint32_t &max_dxlevel,
                                       uint32_t &recommended_dxlevel) {
...
bool CShaderAPIDx8::OnDeviceInit() {
  ...
  g_pHardwareConfig->CapsForEdit().m_TextureMemorySize =
      g_pShaderDeviceMgrDx8->GetVidMemBytes(m_nAdapter);
...
bool CShaderAPIDx8::SetMode(void *hWnd, int nAdapter,
  ...
  const HardwareCaps_t &actualCaps =
      g_pShaderDeviceMgr->GetHardwareCaps(nAdapter);

  ShaderDeviceInfo_t actualInfo = info;
  int nDXLevel = actualInfo.m_nDXLevel ? actualInfo.m_nDXLevel
  ...
  if (!g_pShaderDeviceMgrDx8->ValidateMode(nAdapter, actualInfo)) return false;
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::EnableAlphaToCoverage() {
  if (!g_pHardwareConfig->ActualCaps().m_bSupportsAlphaToCoverage ||
      !IsAAEnabled())
    return;

  D3DRENDERSTATETYPE renderState =
      (D3DRENDERSTATETYPE)g_pHardwareConfig->Caps().m_AlphaToCoverageState;
  SetRenderState(renderState,
                 g_pHardwareConfig->Caps()
                     .m_AlphaToCoverageEnableValue);  // Vendor dependent state
}
...
void CShaderAPIDx8::DisableAlphaToCoverage() {
  ...
  SetRenderState(renderState,
                 g_pHardwareConfig->Caps()
                     .m_AlphaToCoverageDisableValue);  // Vendor dependent state
}
...
bool CShaderAPIDx8::DetermineHardwareCaps() {
  ...
  g_pShaderDeviceMgrDx8->ReadHardwareCaps(actualCaps,
  ...
  if (pShaderParam) {
    Q_strncpy(actualCaps.m_pShaderDLL, pShaderParam,
              sizeof(actualCaps.m_pShaderDLL));
  }
...
void CShaderAPIDx8::OverrideCaps(int nForcedDXLevel) {
  ...
  g_pHardwareConfig->SetupHardwareCaps(nForcedDXLevel,
...
// per-pass
//-----------------------------------------------------------------------------
bool CShaderAPIDx8::IsCommitFuncInUse(CommitFuncType_t func,
                                      CommitShaderType_t shader,
                                      int nFunc) const {
  Assert(nFunc < COMMIT_FUNC_COUNT);
  return (m_pCommitFlags[func][shader][nFunc >> 3] & (1 << (nFunc & 0x7))) != 0;
}

void CShaderAPIDx8::MarkCommitFuncInUse(CommitFuncType_t func,
                                        CommitShaderType_t shader, int nFunc) {
  m_pCommitFlags[func][shader][nFunc >> 3] |= 1 << (nFunc & 0x7);
}

void CShaderAPIDx8::AddCommitFunc(CommitFuncType_t func,
                                  CommitShaderType_t shader,
                                  StateCommitFunc_t f) {
...
inline void CShaderAPIDx8::SetSamplerState(int stage, D3DSAMPLERSTATETYPE state,
                                           DWORD val) {
  if (IsDeactivated()) return;
...
static void CommitSetScissorRect(Direct3DDevice9Wrapper *pDevice,
  ...
  bool bEnableChanged = desiredState.m_RenderState[D3DRS_SCISSORTESTENABLE] !=
                        currentState.m_RenderState[D3DRS_SCISSORTESTENABLE];
  if (bEnableChanged) {
    Dx9Device()->SetRenderState(
        D3DRS_SCISSORTESTENABLE,
        desiredState.m_RenderState[D3DRS_SCISSORTESTENABLE]);
    currentState.m_RenderState[D3DRS_SCISSORTESTENABLE] =
        desiredState.m_RenderState[D3DRS_SCISSORTESTENABLE];
  }
  ...
  if (desiredState.m_RenderState[D3DRS_SCISSORTESTENABLE]) {
    ...
    Assert((desiredState.m_ScissorRect.left <= nWidth) &&
           (desiredState.m_ScissorRect.bottom <= nHeight) &&
...
inline void CShaderAPIDx8::SetScissorRect(int nLeft, int nTop, int nRight,
  ...
  if (!m_bResettingRenderState) {
    bool bEnableChanged =
        m_DesiredState.m_RenderState[D3DRS_SCISSORTESTENABLE] !=
        dwEnableScissor;
    bool bRectChanged = memcmp(&newScissorRect, &m_DesiredState.m_ScissorRect,
                               sizeof(RECT)) != 0;

    if (!bEnableChanged && !bRectChanged) return;
  }
...
void CShaderAPIDx8::SetStandardVertexShaderConstants(float fOverbright) {
  LOCK_SHADERAPI();
  if (g_pHardwareConfig->GetDXSupportLevel() < 80) return;

  // Set a couple standard constants....
  Vector4D standardVertexShaderConstant(0.0f, 1.0f, 2.0f, 0.5f);
  SetVertexShaderConstant(VERTEX_SHADER_MATH_CONSTANTS0,
                          standardVertexShaderConstant.Base(), 1);

  // [ gamma, overbright, 1/3, 1/overbright ]
  standardVertexShaderConstant.Init(1.0f / 2.2f, fOverbright, 1.0f / 3.0f,
                                    1.0f / fOverbright);
  SetVertexShaderConstant(VERTEX_SHADER_MATH_CONSTANTS1,
                          standardVertexShaderConstant.Base(), 1);

  int nModelIndex = g_pHardwareConfig->Caps().m_nDXSupportLevel < 90
                        ? VERTEX_SHADER_MODEL - 10
                        : VERTEX_SHADER_MODEL;

  // These point to the lighting and the transforms
  standardVertexShaderConstant.Init(
      VERTEX_SHADER_LIGHTS, VERTEX_SHADER_LIGHTS + 5,
  ...
  if ( g_pHardwareConfig->Caps().m_SupportsVertexShaders_3_0 )
  {
          ...
          SetVertexShaderConstant( VERTEX_SHADER_DOT_PRODUCT_FACTORS,
  factors[0].Base(), 4 );
  }
*/
...
void CShaderAPIDx8::InitVertexAndPixelShaders() {
  ...
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    ...
    {
      delete[] m_DynamicState.m_pVectorPixelShaderConstant;
      m_DynamicState.m_pVectorPixelShaderConstant =
          new Vector4D[g_pHardwareConfig->Caps().m_NumPixelShaderConstants];
      ...
      delete[] m_DynamicState.m_pBooleanPixelShaderConstant;
      m_DynamicState.m_pBooleanPixelShaderConstant =
          new BOOL[g_pHardwareConfig->Caps().m_NumBooleanPixelShaderConstants];

      delete[] m_DesiredState.m_pBooleanPixelShaderConstant;
      m_DesiredState.m_pBooleanPixelShaderConstant =
          new BOOL[g_pHardwareConfig->Caps().m_NumBooleanPixelShaderConstants];

      delete[] m_DynamicState.m_pIntegerPixelShaderConstant;
      m_DynamicState.m_pIntegerPixelShaderConstant =
          new IntVector4D[g_pHardwareConfig->Caps()
      ...
      int i;
      for (i = 0; i < g_pHardwareConfig->Caps().m_NumPixelShaderConstants;
           ++i) {
        m_DesiredState.m_pVectorPixelShaderConstant[i].Init();
      }
      SetPixelShaderConstant(
          0, m_DesiredState.m_pVectorPixelShaderConstant[0].Base(),
          g_pHardwareConfig->Caps().m_NumPixelShaderConstants, true);

      // force reset boolean pixel constants
      int nNumBooleanPixelShaderConstants =
          g_pHardwareConfig->Caps().m_NumBooleanPixelShaderConstants;
      if (nNumBooleanPixelShaderConstants) {
        ...
        SetBooleanPixelShaderConstant(
            0, m_DesiredState.m_pBooleanPixelShaderConstant,
            nNumBooleanPixelShaderConstants, true);
      }

      // force reset integer pixel constants
      int nNumIntegerPixelShaderConstants =
          g_pHardwareConfig->Caps().m_NumIntegerPixelShaderConstants;
      if (nNumIntegerPixelShaderConstants) {
        ...
        SetIntegerPixelShaderConstant(
            0, m_DesiredState.m_pIntegerPixelShaderConstant[0].Base(),
            nNumIntegerPixelShaderConstants, true);
      }
    }
    ...
    {
      delete[] m_DynamicState.m_pVectorVertexShaderConstant;
      m_DynamicState.m_pVectorVertexShaderConstant =
          new Vector4D[g_pHardwareConfig->Caps().m_NumVertexShaderConstants];

      delete[] m_DesiredState.m_pVectorVertexShaderConstant;
      m_DesiredState.m_pVectorVertexShaderConstant =
          new Vector4D[g_pHardwareConfig->Caps().m_NumVertexShaderConstants];

      delete[] m_DynamicState.m_pBooleanVertexShaderConstant;
      m_DynamicState.m_pBooleanVertexShaderConstant =
          new BOOL[g_pHardwareConfig->Caps().m_NumBooleanVertexShaderConstants];

      delete[] m_DesiredState.m_pBooleanVertexShaderConstant;
      m_DesiredState.m_pBooleanVertexShaderConstant =
          new BOOL[g_pHardwareConfig->Caps().m_NumBooleanVertexShaderConstants];

      delete[] m_DynamicState.m_pIntegerVertexShaderConstant;
      m_DynamicState.m_pIntegerVertexShaderConstant =
          new IntVector4D[g_pHardwareConfig->Caps()
...
static void CommitVertexTextures(Direct3DDevice9Wrapper *pDevice,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    ShaderAPITextureHandle_t textureHandle =
        desiredState.m_VertexTextureState[i].m_BoundTexture;

    Texture_t *pTexture = (textureHandle != INVALID_SHADERAPI_TEXTURE_HANDLE)
                              ? &g_ShaderAPIDX8.GetTexture(textureHandle)
                              : NULL;
    if (pTexture && (pTexture->m_Flags & Texture_t::IS_VERTEX_TEXTURE) == 0) {
      Warning(
          "Attempting to bind a vertex texture (%s) which was not created as a "
          "vertex texture!\n",
    ...
    if (bForce || (currentVTState.m_BoundTexture != textureHandle)) {
      ...
      // 			RECORD_INT( pTexture ? pTexture->GetUniqueID() :
      // 0xFFFF ); 			RECORD_INT( 0 );

      IDirect3DBaseTexture *pD3DTexture =
          (textureHandle >= 0) ? g_ShaderAPIDX8.GetD3DTexture(textureHandle)
                               : NULL;
    ...
    if (bForce || (currentVTState.m_UTexWrap != nNewUTexWrap)) {
      ...
      SetSamplerState(pDevice, D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_ADDRESSU,
    ...
    if (bForce || (currentVTState.m_VTexWrap != nNewVTexWrap)) {
      ...
      SetSamplerState(pDevice, D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_ADDRESSV,
                      currentVTState.m_VTexWrap);
    }
    ...
    if ( bForce || ( currentVTState.m_WTexWrap != nNewWTexWrap ) )
    {
            ...
            SetSamplerState( pDevice, D3DVERTEXTEXTURESAMPLER0 + i,
    D3DSAMP_ADDRESSW, currentVTState.m_WTexWrap );
    }
    */
    ...
    if (bForce || (currentVTState.m_MinFilter != nNewMinFilter)) {
      ...
      SetSamplerState(pDevice, D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_MINFILTER,
                      currentVTState.m_MinFilter);
    }
    ...
    if (bForce || (currentVTState.m_MagFilter != nNewMagFilter)) {
      ...
      SetSamplerState(pDevice, D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_MAGFILTER,
                      currentVTState.m_MagFilter);
    }
    ...
    if (bForce || (currentVTState.m_MipFilter != nNewMipFilter)) {
      ...
      SetSamplerState(pDevice, D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_MIPFILTER,
                      currentVTState.m_MipFilter);
    }
  }
}
...
bool CShaderAPIDx8::UsesVertexAndPixelShaders(StateSnapshot_t id) const {
  LOCK_SHADERAPI();
  return m_TransitionTable.GetSnapshotShader(id).m_VertexShader !=
         INVALID_SHADER;
}
...
void CShaderAPIDx8::ResetDXRenderState() {
  float zero = 0.0f;
  float one = 1.0f;
  ...
  if (IsPC()) {
    if (g_pHardwareConfig->Caps().m_bNeedsATICentroidHack) {
      SetSupportedRenderStateForce(D3DRS_POINTSIZE,
                                   MAKEFOURCC('C', 'E', 'N', 'T'));
    } else if (g_pHardwareConfig->Caps().m_VendorID == VENDORID_NVIDIA) {
      // NVIDIA has a bug in their driver, so we can't call this on DX10
      // hardware right now
      if (!g_pHardwareConfig->UsesSRGBCorrectBlending() ||
          (g_pHardwareConfig->Caps().m_nDXSupportLevel < 90)) {
        // This helps the driver to know to turn on fast z reject for NVidia
        // dx80 hardware
        SetSupportedRenderStateForce(D3DRS_POINTSIZE,
                                     MAKEFOURCC('H', 'L', '2', 'A'));
      }
    ...
    if (g_pHardwareConfig->Caps().m_bDisableShaderOptimizations) {
      SetSupportedRenderStateForce(D3DRS_ADAPTIVETESS_Y,
                                   MAKEFOURCC('C', 'O', 'P', 'M'));
    }
  }
...
//-----------------------------------------------------------------------------
bool CShaderAPIDx8::OwnGPUResources(bool bEnable) { return false; }
...
void CShaderAPIDx8::ResetRenderState(bool bFullReset) {
  ...
  if (bFullReset) {
    ...
    // just need to dirty the dynamic state, desired state gets copied into
    // below
    Q_memset(m_DynamicState.m_pVectorPixelShaderConstant, 0,
             caps.m_NumPixelShaderConstants * sizeof(Vector4D));
    Q_memset(m_DynamicState.m_pBooleanPixelShaderConstant, 0,
             caps.m_NumBooleanPixelShaderConstants * sizeof(BOOL));
    Q_memset(m_DynamicState.m_pIntegerPixelShaderConstant, 0,
             caps.m_NumIntegerPixelShaderConstants * sizeof(IntVector4D));

    Q_memset(m_DynamicState.m_pVectorVertexShaderConstant, 0,
             caps.m_NumVertexShaderConstants * sizeof(Vector4D));
    Q_memset(m_DynamicState.m_pBooleanVertexShaderConstant, 0,
             caps.m_NumBooleanVertexShaderConstants * sizeof(BOOL));
    Q_memset(m_DynamicState.m_pIntegerVertexShaderConstant, 0,
             caps.m_NumIntegerVertexShaderConstants * sizeof(IntVector4D));

    SetStandardVertexShaderConstants(OVERBRIGHT);
  }

  // Set the default compressed depth range written to dest alpha. Only need to
  // compress it for 8bit alpha to get a useful gradient.
  m_DynamicState.m_DestAlphaDepthRange =
      (g_pHardwareConfig->GetHDRType() == HDR_TYPE_FLOAT) ? 8192.0f : 192.0f;
  ...
  m_DynamicState.m_PixelFogColor[0] = m_DynamicState.m_PixelFogColor[1] =
      m_DynamicState.m_PixelFogColor[2] = m_DynamicState.m_PixelFogColor[3] =
          0.0f;
  ...
  SetSupportedRenderState(D3DRS_NORMALIZENORMALS,
                          m_DynamicState.m_NormalizeNormals);
  ...
  for (i = 0; i < g_pHardwareConfig->ActualCaps().m_NumTextureStages; ++i) {
    ...
    SetTextureStageState(i, D3DTSS_TEXTURETRANSFORMFLAGS,
                         TextureStage(i).m_TextureTransformFlags);
    SetTextureStageState(i, D3DTSS_BUMPENVMAT00,
                         *((LPDWORD)(&TextureStage(i).m_BumpEnvMat00)));
    SetTextureStageState(i, D3DTSS_BUMPENVMAT01,
                         *((LPDWORD)(&TextureStage(i).m_BumpEnvMat01)));
    SetTextureStageState(i, D3DTSS_BUMPENVMAT10,
                         *((LPDWORD)(&TextureStage(i).m_BumpEnvMat10)));
    SetTextureStageState(i, D3DTSS_BUMPENVMAT11,
                         *((LPDWORD)(&TextureStage(i).m_BumpEnvMat11)));
  }
  ...
  for (i = 0; i < g_pHardwareConfig->ActualCaps().m_nVertexTextureCount; i++) {
    ...
    //		m_DynamicState.m_VertexTextureState[i].m_WTexWrap =
    // D3DTADDRESS_CLAMP;
    ...
    SetSamplerState(D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_ADDRESSU,
                    m_DynamicState.m_VertexTextureState[i].m_UTexWrap);
    SetSamplerState(D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_ADDRESSV,
                    m_DynamicState.m_VertexTextureState[i].m_VTexWrap);
    //		SetSamplerState( D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_ADDRESSW,
    // m_DynamicState.m_VertexTextureState[i].m_WTexWrap );
    SetSamplerState(D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_MINFILTER,
                    m_DynamicState.m_VertexTextureState[i].m_MinFilter);
    SetSamplerState(D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_MAGFILTER,
                    m_DynamicState.m_VertexTextureState[i].m_MagFilter);
    SetSamplerState(D3DVERTEXTEXTURESAMPLER0 + i, D3DSAMP_MIPFILTER,
                    m_DynamicState.m_VertexTextureState[i].m_MipFilter);
  }
  ...
  SetSupportedRenderState(D3DRS_COLORVERTEX,
                          TRUE);  // This defaults to true anyways. . .
  ...
  fFakePlane[0] = fFakePlane[1] = fFakePlane[2] = fFakePlane[3] =
      *((float *)&iFakePlaneVal);
  SetFastClipPlane(
      fFakePlane);  // doing this to better wire up plane change detection
  ...
  Assert(m_DynamicState.m_UserClipPlaneChanged ==
         ((1 << g_pHardwareConfig->MaxUserClipPlanes()) - 1));
  ...
  Vector4D *pVectorPixelShaderConstants =
      m_DesiredState.m_pVectorPixelShaderConstant;
  int *pBooleanPixelShaderConstants =
      m_DesiredState.m_pBooleanPixelShaderConstant;
  IntVector4D *pIntegerPixelShaderConstants =
      m_DesiredState.m_pIntegerPixelShaderConstant;
  Vector4D *pVectorVertexShaderConstants =
      m_DesiredState.m_pVectorVertexShaderConstant;
  int *pBooleanVertexShaderConstants =
      m_DesiredState.m_pBooleanVertexShaderConstant;
  IntVector4D *pIntegerVertexShaderConstants =
      m_DesiredState.m_pIntegerVertexShaderConstant;
  ...
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    if (!bFullReset) {
      // Full resets init the values to defaults. Normal resets just leave them
      // dirty.
      ...
      if (g_pHardwareConfig->Caps().m_NumIntegerPixelShaderConstants != 0)
        SetIntegerPixelShaderConstant(
            0, (int *)m_DesiredState.m_pIntegerPixelShaderConstant,
...
void CShaderAPIDx8::SetDefaultState() {
  ...
  // NOTE: This used to be in the material system, but I want to avoid all the
...
// Sets the vertex
//-----------------------------------------------------------------------------
inline void CShaderAPIDx8::SetVertexDecl(VertexFormat_t vertexFormat,
                                         bool bHasColorMesh, bool bUsingFlex,
                                         bool bUsingMorph) {
  VPROF("CShaderAPIDx8::SetVertexDecl");
  IDirect3DVertexDeclaration9 *pDecl = FindOrCreateVertexDecl(
      vertexFormat, bHasColorMesh, bUsingFlex, bUsingMorph);
  Assert(pDecl);
...
// Gets the dynamic mesh
//-----------------------------------------------------------------------------
IMesh *CShaderAPIDx8::GetDynamicMesh(IMaterial *pMaterial, int nHWSkinBoneCount,
                                     bool buffered, IMesh *pVertexOverride,
                                     IMesh *pIndexOverride) {
  ...
  LOCK_SHADERAPI();
  return MeshMgr()->GetDynamicMesh(pMaterial, 0, nHWSkinBoneCount, buffered,
                                   pVertexOverride, pIndexOverride);
}

IMesh *CShaderAPIDx8::GetDynamicMeshEx(IMaterial *pMaterial,
                                       VertexFormat_t vertexFormat,
                                       int nHWSkinBoneCount, bool bBuffered,
  ...
  LOCK_SHADERAPI();
  return MeshMgr()->GetDynamicMesh(pMaterial, vertexFormat, nHWSkinBoneCount,
...
// Returns the number of vertices we can render using the dynamic mesh
//-----------------------------------------------------------------------------
void CShaderAPIDx8::GetMaxToRender(IMesh *pMesh, bool bMaxUntilFlush,
                                   int *pMaxVerts, int *pMaxIndices) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::MarkUnusedVertexFields(unsigned int nFlags,
                                           int nTexCoordCount,
                                           bool *pUnusedTexCoords) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::DrawMesh(CMeshBase *pMesh) {
  ...
  if (ShaderUtil()->GetConfig().m_bSuppressRendering) return;
  ...
  SetVertexDecl(vertexFormat, m_pRenderMesh->HasColorMesh(),
                m_pRenderMesh->HasFlexMesh(), m_pMaterial->IsUsingVertexID());
  CommitStateChanges();
...
void CShaderAPIDx8::DrawWithVertexAndIndexBuffers() {
  ...
  SetVertexDecl(vertexFormat, false /*m_pRenderMesh->HasColorMesh()*/,
                ...
                false /*m_pRenderMesh->IsUsingMorphData()*/);
  CommitStateChanges();
  if (m_pMaterial) {
    m_pMaterial->DrawMesh(CompressionType(vertexFormat));
  } else {
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr)) {
        ...
        Dx9Device()->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST,
                                            0,        // Min vertex index
                                            3,        // Num vertices used
                                            1,        // # primitives
                                            indices,  // indices
...
void CShaderAPIDx8::ForceHardwareSync() {
  LOCK_SHADERAPI();
  VPROF("CShaderAPIDx8::ForceHardwareSync");

  if (!mat_frame_sync_enable.GetInt()) return;
  ...
  // simple transaction with the driver through the GetDriverState, trivial for
  // the drivers to lie.  We came up with a much better scheme for tracking
  // pending frames where the driver can not lie without a possible performance
  // loss: use the asynchronous query system with D3DQUERYTYPE_EVENT and data
  // size 0.  When GetData returns S_OK for the query, you know that frame has
  // finished.

  if (!mat_frame_sync_force_texture.GetInt() && m_DeviceSupportsCreateQuery &&
    ...
    // On such boards, sometimes the driver has to reset itself (an event which
    // takes several seconds) and when that happens, the frame sync query object
    // gets lost
    double flStartTime = Plat_FloatTime();
    do {
      hr = m_pFrameSyncQueryObject->GetData(&bDummy, sizeof(bDummy),
                                            D3DGETDATA_FLUSH);
      double flCurrTime = Plat_FloatTime();
      if (flCurrTime - flStartTime > 3.0f) break;
    } while (hr == S_FALSE);
...
void CShaderAPIDx8::AddBufferToTextureList(const char *pName,
  ...
  pSubKey->SetInt(
      "Size",
      //		ImageLoader::SizeInBytes( imageFormat ) * desc.Width *
      // desc.Height );
      4 * desc.Width * desc.Height);
  pSubKey->SetString(
      "Format",
      "32 bit buffer (hack)");  // ImageLoader::GetName( imageFormat ) );
...
void CShaderAPIDx8::ExportTextureList() {
  if (!m_bEnableDebugTextureList) return;
...
void CShaderAPIDx8::EndPIXEvent() {
#if defined(PIX_INSTRUMENTATION)
  LOCK_SHADERAPI();
  ...
  if (r_pix_recordframes.GetInt()) {
    if (m_nPixFrame == 0)  // First frame to record
    {
      ...
      m_nPixFrame++;
               ...
               r_pix_recordframes.GetInt())  // Last frame to record
    {
...
void CShaderAPIDx8::IncrementPIXError() {
  ...
  if (m_nPIXErrorCount >= MAX_PIX_ERRORS) {
    Warning(
        "Source engine built with PIX instrumentation, but PIX doesn't seem to "
        "have been used to instantiate the game, which is necessary on PC.\n");
  }
#endif
}
...
void CShaderAPIDx8::ComputeFillRate() {
  ...
  if (!pBuf) {
    int memSize = ShaderUtil()->GetMemRequired(width, height, 1,
...
void CShaderAPIDx8::ApplyZBias(const ShadowState_t &shaderState) {
  ...
  float a = (config.m_SlopeScaleDepthBias_Decal != 0.0f)
                ? 1.0f / config.m_SlopeScaleDepthBias_Decal
                : 0.0f;
  float b = (config.m_SlopeScaleDepthBias_Normal != 0.0f)
                ? 1.0f / config.m_SlopeScaleDepthBias_Normal
                : 0.0f;
  float c = (config.m_DepthBias_Decal != 0.0f) ? 1.0f / config.m_DepthBias_Decal
                                               : 0.0f;
  float d = (config.m_DepthBias_Normal != 0.0f)
  ...
  if ((g_pHardwareConfig->Caps().m_VendorID == 0x1002) && bPS14Only) {
    ...
    // NOTE: Slamming to 0 instead of -1.0 / 4096 because on Cat 4.9, WinXP,
    // 8500, this causes the z values to be more than 50 units away from the
    // original z values
  ...
  // bias = (s * D3DRS_SLOPESCALEDEPTHBIAS) + D3DRS_DEPTHBIAS, where s is the
  // maximum depth slope of the triangle being rendered
...
void CShaderAPIDx8::ApplyTextureEnable(const ShadowState_t &state,
                                       int nSampler) {
  if (state.m_SamplerState[nSampler].m_TextureEnable ==
      SamplerState(nSampler).m_TextureEnable)
    return;
  ...
  if (state.m_SamplerState[nSampler].m_TextureEnable) {
    ...
    SetTextureState((Sampler_t)nSampler, SamplerState(nSampler).m_BoundTexture,
                    true);
  } else {
...
//-----------------------------------------------------------------------------
VertexFormat_t CShaderAPIDx8::ComputeVertexUsage(int num,
                                                 StateSnapshot_t *pIds) const {
  LOCK_SHADERAPI();
  if (num == 0) return 0;
  ...
  if (num == 1) {
    const ShadowShaderState_t &state =
        m_TransitionTable.GetSnapshotShader(pIds[0]);
  ...
  for (int i = num; --i >= 0;) {
    const ShadowShaderState_t &state =
        m_TransitionTable.GetSnapshotShader(pIds[i]);
    VertexFormat_t fmt = state.m_VertexUsage;
    flags |= VertexFlags(fmt);

    VertexCompressionType_t newCompression = CompressionType(fmt);
    if ((compression != newCompression) &&
        (compression != VERTEX_COMPRESSION_INVALID)) {
      Warning(
          "Encountered a material with two passes that specify different "
          "vertex compression types!\n");
    ...
    if ((numBones != newNumBones) && (newNumBones != 0)) {
      if (numBones != 0) {
        Warning(
            "Encountered a material with two passes that use different numbers "
            "of bones!\n");
      }
    ...
    if ((userDataSize != newUserSize) && (newUserSize != 0)) {
      if (userDataSize != 0) {
        Warning(
            "Encountered a material with two passes that use different user "
            "data sizes!\n");
      }
    ...
    for (int j = 0; j < VERTEX_MAX_TEXTURE_COORDINATES; ++j) {
      ...
      if ((texCoordSize[j] != newSize) && (newSize != 0)) {
        if (texCoordSize[j] != 0) {
          Warning(
              "Encountered a material with two passes that use different "
              "texture coord sizes!\n");
        }
...
VertexFormat_t CShaderAPIDx8::ComputeVertexFormat(int num,
                                                  StateSnapshot_t *pIds) const {
  LOCK_SHADERAPI();
...
// What fields in the morph do we actually use?
//-----------------------------------------------------------------------------
MorphFormat_t CShaderAPIDx8::ComputeMorphFormat(int numSnapshots,
                                                StateSnapshot_t *pIds) const {
  LOCK_SHADERAPI();
  MorphFormat_t format = 0;
  for (int i = 0; i < numSnapshots; ++i) {
    MorphFormat_t fmt =
        m_TransitionTable.GetSnapshotShader(pIds[i]).m_MorphUsage;
...
static void CommitVertexShaderConstantRange(Direct3DDevice9Wrapper *pDevice,
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    bool bDifferentValue =
        bForce || (desiredState.m_pVectorVertexShaderConstant[nVar] !=
                   currentState.m_pVectorVertexShaderConstant[nVar]);
    if (!bDifferentValue) {
      if (nCommitCount != 0) {
        // flush the prior range
        pDevice->SetVertexShaderConstantF(
            nFirstCommit,
            desiredState.m_pVectorVertexShaderConstant[nFirstCommit].Base(),
            nCommitCount);

        memcpy(&currentState.m_pVectorVertexShaderConstant[nFirstCommit],
               &desiredState.m_pVectorVertexShaderConstant[nFirstCommit],
               nCommitCount * 4 * sizeof(float));
      }
...
void CShaderAPIDx8::GetBufferedState(BufferedState_t &state) {
  memcpy(&state.m_Transform[0], &GetTransform(MATERIAL_MODEL),
         sizeof(D3DXMATRIX));
  memcpy(&state.m_Transform[1], &GetTransform(MATERIAL_VIEW),
         sizeof(D3DXMATRIX));
  memcpy(&state.m_Transform[2], &GetTransform(MATERIAL_PROJECTION),
         sizeof(D3DXMATRIX));
  memcpy(&state.m_Viewport, &m_DynamicState.m_Viewport,
         sizeof(state.m_Viewport));
...
void CShaderAPIDx8::Color3f(float r, float g, float b) {
  unsigned int color =
      D3DCOLOR_ARGB(255, (int)(r * 255), (int)(g * 255), (int)(b * 255));
...
void CShaderAPIDx8::Color3fv(float const *c) {
  Assert(c);
  unsigned int color = D3DCOLOR_ARGB(255, (int)(c[0] * 255), (int)(c[1] * 255),
                                     (int)(c[2] * 255));
  if (color != m_DynamicState.m_ConstantColor) {
    m_DynamicState.m_ConstantColor = color;
    SetSupportedRenderState(D3DRS_TEXTUREFACTOR, color);
...
void CShaderAPIDx8::Color3ub(unsigned char r, unsigned char g,
                             unsigned char b) {
...
void CShaderAPIDx8::Color4ub(unsigned char r, unsigned char g, unsigned char b,
                             unsigned char a) {
...
void CShaderAPIDx8::Color4ubv(unsigned char const *pColor) {
  Assert(pColor);
  unsigned int color =
      D3DCOLOR_ARGB(pColor[3], pColor[0], pColor[1], pColor[2]);
  if (color != m_DynamicState.m_ConstantColor) {
    m_DynamicState.m_ConstantColor = color;
    SetSupportedRenderState(D3DRS_TEXTUREFACTOR, color);
  }
...
void CShaderAPIDx8::ShadeMode(ShaderShadeMode_t mode) {
  ...
  D3DSHADEMODE shadeMode =
      (mode == SHADER_FLAT) ? D3DSHADE_FLAT : D3DSHADE_GOURAUD;
...
void CShaderAPIDx8::SetDepthFeatheringPixelShaderConstant(
    int iConstant, float fDepthBlendScale) {
  float fConstantValues[4] = {
      m_DynamicState.m_DestAlphaDepthRange / fDepthBlendScale, 0.0f, 0.0f,
      0.0f};  // empty
...
void CShaderAPIDx8::ApplyCullEnable(bool bEnable) {
  ...
  SetCullModeState(m_DynamicState.m_bCullEnabled,
                   m_DynamicState.m_DesiredCullMode);
}
...
//-----------------------------------------------------------------------------
// Returns the current cull mode of the current material (for selection mode
// only)
//-----------------------------------------------------------------------------
D3DCULL CShaderAPIDx8::GetCullMode() const {
  Assert(m_pMaterial);
  if (m_pMaterial->GetMaterialVarFlag(MATERIAL_VAR_NOCULL)) return D3DCULL_NONE;
...
void CShaderAPIDx8::UpdateFastClipUserClipPlane() {
  float plane[4];
...
void CShaderAPIDx8::SetClipPlane(int index, const float *pPlane) {
  ...
  // NOTE: This is done because they assume Ax+By+Cz+Dw = 0 (where w = 1 in real
  // space) while we use Ax+By+Cz=D
  D3DXPLANE plane;
  plane.a = pPlane[0];
  plane.b = pPlane[1];
  plane.c = pPlane[2];
...
void CShaderAPIDx8::MarkAllUserClipPlanesDirty() {
  m_DynamicState.m_UserClipPlaneChanged |=
      (1 << g_pHardwareConfig->MaxUserClipPlanes()) - 1;
  m_DynamicState.m_bFastClipPlaneChanged = true;
}
...
void CShaderAPIDx8::EnableClipPlane(int index, bool bEnable) {
  ...
  if ((m_DynamicState.m_UserClipPlaneEnabled & (1 << index) ? true : false) !=
    ...
    SetRenderState(D3DRS_CLIPPLANEENABLE,
                   m_DynamicState.m_UserClipPlaneEnabled);
  }
}
...
void CShaderAPIDx8::CommitFastClipPlane() {
  ...
  if (!m_DynamicState.m_bFastClipPlaneChanged ||
      !m_DynamicState.m_FastClipEnabled)
    return;
  ...
  projection(3, 2) *=
      0.5f;  // pull in zNear because the shear in effect
...
#define ALLOW_FOR_FASTCLIPDUMPS 0
  ...
  static ConVar shader_dumpfastclipprojectioncoords(
      "shader_dumpfastclipprojectioncoords", "0", 0,
      "dump fast clip projected matrix");
  if (shader_dumpfastclipprojectioncoords.GetBool())
    DevMsg("Fast clip plane projected coordinates: %f %f %f %f", clipPlane.x,
           clipPlane.y, clipPlane.z, clipPlane.w);
#endif

  if ((clipPlane.z * clipPlane.w) <= -0.4f)  // a plane with (z*w) > -0.4 at
                                             // this point is behind the camera
                                             // and will cause graphical
                                             // glitches. Toss it. (0.4 found
                                             // through experimentation)
  {
#if (ALLOW_FOR_FASTCLIPDUMPS == 1)
    if (shader_dumpfastclipprojectioncoords.GetBool())
      DevMsg("    %f %f %f %f\n", clipPlane.x, clipPlane.y, clipPlane.z,
             clipPlane.w);
#endif
  ...
  m_CachedFastClipProjectionMatrix =
      projection * m_CachedFastClipProjectionMatrix;

  // Update the cached polyoffset matrix (with clip) too:
  ComputePolyOffsetMatrix(m_CachedFastClipProjectionMatrix,
                          m_CachedFastClipPolyOffsetProjectionMatrix);
}
...
void CShaderAPIDx8::SetFastClipPlane(const float *pPlane) {
  ...
  if (plane != m_DynamicState.m_FastClipPlane) {
    ...
    // Mark a dirty bit so when it comes time to commit view + projection
    // transforms, we also update the fast clip matrices
    m_DynamicState.m_bFastClipPlaneChanged = true;
...
void CShaderAPIDx8::SetNumBoneWeights(int numBones) {
  ...
  if (m_DynamicState.m_NumBones != numBones) {
    ...
    const ShadowState_t *current_shadow_state{
        m_TransitionTable.CurrentShadowState()};
    if (current_shadow_state) {
      SetVertexBlendState(current_shadow_state->m_VertexBlendEnable ? -1 : 0);
    }
  }
}
...
// Fog methods...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::UpdatePixelFogColorConstant() {
  Assert(HardwareConfig()->SupportsPixelShaders_2_b());
  float fogColor[4];

  switch (GetPixelFogMode()) {
    case MATERIAL_FOG_NONE: {
      for (int i = 0; i != 3; ++i) fogColor[i] = 0.0f;
    } break;

    case MATERIAL_FOG_LINEAR: {
      // setup the fog for mixing linear fog in the pixel shader so that it
      // emulates ff range fog
      for (int i = 0; i != 3; ++i)
        fogColor[i] = m_DynamicState.m_PixelFogColor[i];

      if (m_DynamicState.m_bSRGBWritesEnabled) {
        // since the fog color will assuredly get converted from linear to
        // gamma, we should probably convert it from gamma to linear
        for (int i = 0; i != 3; ++i)
          fogColor[i] = GammaToLinear_HardwareSpecific(fogColor[i]);
      }

      if ((!m_DynamicState.m_bFogGammaCorrectionDisabled) &&
    ...
    case MATERIAL_FOG_LINEAR_BELOW_FOG_Z: {
      // water fog has been around a while and has never tonemap scaled, and has
      // always been in linear space
      if (g_pHardwareConfig->NeedsShaderSRGBConversion()) {
        ...
        for (int i = 0; i != 3; ++i)
          fogColor[i] = GammaToLinear_HardwareSpecific(
              m_DynamicState.m_PixelFogColor[i]);  // this is how water fog
  ...
  SetPixelShaderConstant(LINEAR_FOG_COLOR, fogColor);  //-V614
}

void CShaderAPIDx8::ApplyFogMode(ShaderFogMode_t fogMode,
                                 bool bSRGBWritesEnabled,
                                 bool bDisableFogGammaCorrection) {
  ...
  if (fogMode == SHADER_FOGMODE_DISABLED) {
    ...
    if (m_DelayedShaderConstants.iPixelShaderFogParams != -1)
      SetPixelShaderFogParams(m_DelayedShaderConstants.iPixelShaderFogParams,
                              fogMode);
...
void CShaderAPIDx8::SceneFogMode(MaterialFogMode_t fogMode) {
  ...
  if (m_SceneFogMode != fogMode) {
    ...
    if (current_shadow_state) {
      ...
      ApplyFogMode(current_shadow_state->m_FogMode,
                   current_shadow_state->m_SRGBWriteEnable,
                   current_shadow_state->m_bDisableFogGammaCorrection);
    }
  }
}

MaterialFogMode_t CShaderAPIDx8::GetSceneFogMode() { return m_SceneFogMode; }
...
int CShaderAPIDx8::GetPixelFogCombo() {
  if ((m_SceneFogMode != MATERIAL_FOG_NONE) &&
      ShouldUsePixelFogForMode(m_SceneFogMode))
    return m_SceneFogMode - 1;  // PIXELFOGTYPE dynamic combos are shifted down
                                // one. MATERIAL_FOG_NONE is simulated by range
                                // fog with rigged parameters. Gets rid of a
                                // dynamic combo across the ps2x set
  else
    return MATERIAL_FOG_NONE;
}
...
bool CShaderAPIDx8::ShouldUsePixelFogForMode(MaterialFogMode_t fogMode) {
  ...
  switch (m_SceneFogMode) {
    case MATERIAL_FOG_LINEAR:
      return (
          g_pHardwareConfig
              ->SupportsPixelShaders_2_b() &&  // lightmappedgeneric_ps20.fxc
                                               // can't handle the instruction
                                               // count
...
void CShaderAPIDx8::FogMaxDensity(float flMaxDensity) {
  ...
  if (flMaxDensity != m_DynamicState.m_FogMaxDensity) {
    ...
    //		SetRenderState(D3DRS_FOGDENSITY, *((DWORD*)(&flMaxDensity))); //
    //???  do I need to to this ???
...
void CShaderAPIDx8::GetFogDistances(float *fStart, float *fEnd, float *fFogZ) {
  LOCK_SHADERAPI();
  if (fStart) *fStart = m_DynamicState.m_FogStart;

  if (fEnd) *fEnd = m_DynamicState.m_FogEnd;

  if (fFogZ) *fFogZ = m_DynamicState.m_FogZ;
}

void CShaderAPIDx8::SceneFogColor3ub(unsigned char r, unsigned char g,
                                     unsigned char b) {
  LOCK_SHADERAPI();
  if (m_SceneFogColor[0] != r || m_SceneFogColor[1] != g ||
      m_SceneFogColor[2] != b) {
    FlushBufferedPrimitives();
    m_SceneFogColor[0] = r;
    m_SceneFogColor[1] = g;
...
void CShaderAPIDx8::GetSceneFogColor(unsigned char *r, unsigned char *g,
                                     unsigned char *b) {
  *r = m_SceneFogColor[0];
  *g = m_SceneFogColor[1];
  *b = m_SceneFogColor[2];
...
// Gamma correction of fog color, or not...
//-----------------------------------------------------------------------------
D3DCOLOR CShaderAPIDx8::ComputeGammaCorrectedFogColor(unsigned char r,
                                                      unsigned char g,
                                                      unsigned char b,
                                                      bool bSRGBWritesEnabled) {
#ifdef _DEBUG
...
#endif
  bool bLinearSpace =
      g_pHardwareConfig->Caps().m_bFogColorAlwaysLinearSpace ||
      (bSRGBWritesEnabled &&
       (g_pHardwareConfig->Caps().m_bFogColorSpecifiedInLinearSpace ||
        g_pHardwareConfig->GetHDRType() == HDR_TYPE_FLOAT));

  bool bScaleFogByToneMappingScale =
      g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER;
...
void CShaderAPIDx8::DestroyVertexBuffers(bool bExitingLevel) {
  ...
  // (VGUI shouldn't need much), so that we have more memory free during map
  // loading
  m_nDynamicVBSize = bExitingLevel ? DYNAMIC_VERTEX_BUFFER_MEMORY_SMALL
...
// Sets the constant register for vertex and pixel shaders
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetVertexShaderConstant(int var, float const *pVec,
                                            int numVecs, bool bForce) {
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetBooleanVertexShaderConstant(int var, int const *pVec,
                                                   int numBools, bool bForce) {
  Assert(pVec);
  Assert(var + numBools <=
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetIntegerVertexShaderConstant(int var, int const *pVec,
                                                   int numIntVecs,
                                                   bool bForce) {
  Assert(pVec);
...
void CShaderAPIDx8::SetPixelShaderConstant(int var, float const *pVec,
                                           int numVecs, bool bForce) {
...
void CShaderAPIDx8::ExecuteCommandBuffer(uint8_t *pCmdBuf) {
  ...
  for (;;) {
    ...
    switch (nCmd) {
      ...
      case CBCMD_JUMP:
        pCmdBuf = GetData<uint8_t *>(pCmdBuf + sizeof(int));
        break;

      case CBCMD_JSR: {
        Assert(pSP > &(pReturnStack[0]));
        // 				*(--pSP ) = pCmdBuf + sizeof( int ) +
        // sizeof( uint8_t *);
        // 				pCmdBuf = GetData<uint8_t *>(  pCmdBuf +
        // sizeof( int ) );
        ExecuteCommandBuffer(GetData<uint8_t *>(pCmdBuf + sizeof(int)));
        pCmdBuf = pCmdBuf + sizeof(int) + sizeof(uint8_t *);
        break;
      }
      ...
      case CBCMD_SET_PIXEL_SHADER_FLOAT_CONST: {
        ...
        float const *pValues =
            reinterpret_cast<float const *>(pCmdBuf + 3 * sizeof(int));
      ...
      case CBCMD_STORE_EYE_POS_IN_PSCONST: {
        int nReg = GetData<int>(pCmdBuf + sizeof(int));
        pCmdBuf += 2 * sizeof(int);
        SetPixelShaderConstantInternal(nReg, m_WorldSpaceCameraPositon.Base(),
                                       1, false);
        break;
      }
      ...
      case CBCMD_SET_VERTEX_SHADER_FLOAT_CONST: {
        ...
        float const *pValues =
            reinterpret_cast<float const *>(pCmdBuf + 3 * sizeof(int));
        pCmdBuf += nNumConsts * 4 * sizeof(float) + 3 * sizeof(int);
        SetVertexShaderConstantInternal(nStartConst, pValues, nNumConsts,
                                        false);
        break;
      }
      ...
      case CBCMD_BIND_STANDARD_TEXTURE: {
        ...
        if (m_StdTextureHandles[nTextureID] !=
            INVALID_SHADERAPI_TEXTURE_HANDLE) {
          BindTexture((Sampler_t)nSampler, m_StdTextureHandles[nTextureID]);
        } else {
          ShaderUtil()->BindStandardTexture((Sampler_t)nSampler,
      ...
      case CBCMD_BIND_SHADERAPI_TEXTURE_HANDLE: {
        int nSampler = GetData<int>(pCmdBuf + sizeof(int));
        ShaderAPITextureHandle_t hTexture =
            GetData<ShaderAPITextureHandle_t>(pCmdBuf + 2 * sizeof(int));
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetBooleanPixelShaderConstant(int var, int const *pVec,
                                                  int numBools, bool bForce) {
  Assert(pVec);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetIntegerPixelShaderConstant(int var, int const *pVec,
                                                  int numIntVecs, bool bForce) {
  Assert(pVec);
  Assert(var + numIntVecs <=
...
inline IDirect3DBaseTexture *CShaderAPIDx8::GetD3DTexture(
    ShaderAPITextureHandle_t hTexture) {
...
void CShaderAPIDx8::SetModifyTexture(IDirect3DBaseTexture *pTex) {
  if (m_ModifyTextureHandle == INVALID_SHADERAPI_TEXTURE_HANDLE) return;
...
inline ShaderAPITextureHandle_t CShaderAPIDx8::GetBoundTextureBindId(
    Sampler_t sampler) const {
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetTextureState(Sampler_t sampler,
                                    ShaderAPITextureHandle_t hTexture,
                                    bool force) {
  ...
  if ((samplerState.m_BoundTexture == hTexture) && !force) return;

  // Disabling texturing
  if (hTexture == INVALID_SHADERAPI_TEXTURE_HANDLE ||
      WouldBeOverTextureLimit(hTexture)) {
  ...
  if (!samplerState.m_TextureEnable) return;
...
void CShaderAPIDx8::BindTexture(Sampler_t sampler,
                                ShaderAPITextureHandle_t textureHandle) {
  LOCK_SHADERAPI();
  SetTextureState(sampler, textureHandle);
}

void CShaderAPIDx8::BindVertexTexture(VertexTextureSampler_t nStage,
                                      ShaderAPITextureHandle_t textureHandle) {
  ...
  ADD_VERTEX_TEXTURE_FUNC(COMMIT_PER_PASS, COMMIT_VERTEX_SHADER,
                          CommitVertexTextures, nStage, m_BoundTexture,
                          textureHandle);
}
...
ShaderAPITextureHandle_t CShaderAPIDx8::CreateDepthTexture(
  ...
  pTexture->m_SizeBytes =
      ImageLoader::GetMemRequired(width, height, 1, renderFormat, false);
  ...
  if (!bTexture) {
    ...
    hr = Dx9Device()->CreateTexture(width, height, 1, D3DUSAGE_DEPTHSTENCIL,
                                    format, D3DPOOL_DEFAULT, &pTex, NULL);
  ...
  if (FAILED(hr)) {
    switch (hr) {
      ...
      case D3DERR_OUTOFVIDEOMEMORY:
        Warning(
            "ShaderAPIDX8::CreateDepthStencilSurface: "
...
void CShaderAPIDx8::CreateTextureHandles(ShaderAPITextureHandle_t *handles,
                                         int count) {
  if (count <= 0) return;
  ...
  for (hTexture = m_Textures.Head(); hTexture != m_Textures.InvalidIndex();
    ...
    if (!(m_Textures[hTexture].m_Flags & Texture_t::IS_ALLOCATED)) {
      ...
      if (idxCreating >= count) return;
    }
  }

  while (idxCreating < count) handles[idxCreating++] = m_Textures.AddToTail();
}
...
ShaderAPITextureHandle_t CShaderAPIDx8::CreateTexture(
    ...
    int numMipLevels, int numCopies, int creationFlags, const char *pDebugName,
    const char *pTextureGroupName) {
  ShaderAPITextureHandle_t handle = 0;
  CreateTextures(&handle, 1, width, height, depth, dstImageFormat, numMipLevels,
                 numCopies, creationFlags, pDebugName, pTextureGroupName);
  return handle;
}

void CShaderAPIDx8::CreateTextures(ShaderAPITextureHandle_t *pHandles,
                                   int count, int width, int height, int depth,
                                   ImageFormat dstImageFormat, int numMipLevels,
                                   int numCopies, int creationFlags,
                                   const char *pDebugName,
                                   const char *pTextureGroupName) {
  LOCK_SHADERAPI();
  ...
  for (int idxFrame = 0; idxFrame < count; ++idxFrame) {
    ...
    RECORD_INT(ImageLoader::ImageFormatToD3DFormat(
        FindNearestSupportedFormat(dstImageFormat)));
    ...
    if (numCopies <= 1) {
      pTexture->m_NumCopies = 1;
      pD3DTex = CreateD3DTexture(width, height, depth, dstImageFormat,
                                 numMipLevels, creationFlags);
...
void CShaderAPIDx8::SetupTextureGroup(ShaderAPITextureHandle_t hTexture,
                                      const char *pTextureGroupName) {
  ...
  pTexture->m_pTextureGroupCounterGlobal =
      g_VProfCurrentProfile.FindOrCreateCounter(counterName,
                                                COUNTER_GROUP_TEXTURE_GLOBAL);

  Q_snprintf(counterName, sizeof(counterName), "TexGroup_frame_%s",
             pTexture->m_TextureGroupName.String());
  pTexture->m_pTextureGroupCounterFrame =
      g_VProfCurrentProfile.FindOrCreateCounter(
          counterName, COUNTER_GROUP_TEXTURE_PER_FRAME);
#else
...
void CShaderAPIDx8::DeleteD3DTexture(ShaderAPITextureHandle_t hTexture) {
  ...
  if (texture.m_Flags & Texture_t::IS_DEPTH_STENCIL) {
    // garymcthack - need to make sure that playback knows how to deal with
    // these.
...
void CShaderAPIDx8::UnbindTexture(ShaderAPITextureHandle_t hTexture) {
  ...
  for (int unit = 0; unit < g_pHardwareConfig->GetSamplerCount(); ++unit) {
    if (hTexture == SamplerState(unit).m_BoundTexture) {
      // Gotta set this here because INVALID_SHADERAPI_TEXTURE_HANDLE means
      // don't actually set bound texture (it's used for disabling
      // texturemapping)
  ...
  for (int nSampler = 0; nSampler < nVertexTextureCount; ++nSampler) {
    if (hTexture ==
        m_DynamicState.m_VertexTextureState[nSampler].m_BoundTexture) {
      // Gotta set this here because INVALID_SHADERAPI_TEXTURE_HANDLE means
      // don't actually set bound texture (it's used for disabling
      // texturemapping)
      BindVertexTexture((VertexTextureSampler_t)nSampler,
                        INVALID_SHADERAPI_TEXTURE_HANDLE);
    }
  }
}
...
void CShaderAPIDx8::WriteTextureToFile(ShaderAPITextureHandle_t hTexture,
                                       const char *szFileName) {
  ...
  hr = pD3DTexture->GetLevelDesc(0, &surfaceDesc);
  if (FAILED(hr)) return;
  ...
  // if( pTexInt->m_Flags & Texture_t::IS_RENDER_TARGET )
  {
    ...
    hr = Dx9Device()->CreateOffscreenPlainSurface(
        ...
        D3DPOOL_SYSTEMMEM, &pSystemSurface, NULL);
    Assert(SUCCEEDED(hr));
    ...
    // pretend this is the texture level we originally grabbed with
    // GetSurfaceLevel() and continue on
  ...
  TGAWriter::WriteTGAFile(szFileName, surfaceDesc.Width, surfaceDesc.Height,
                          pTexInt->GetImageFormat(),
                          (const uint8_t *)lockedRect.pBits, lockedRect.Pitch);
...
//-----------------------------------------------------------------------------
IDirect3DSurface *CShaderAPIDx8::GetTextureSurface(
    ShaderAPITextureHandle_t textureHandle) {
  MEM_ALLOC_D3D_CREDIT();
...
// Gets the surface associated with a texture (refcount of surface is increased)
//-----------------------------------------------------------------------------
IDirect3DSurface *CShaderAPIDx8::GetDepthTextureSurface(
    ShaderAPITextureHandle_t textureHandle) {
  AssertValidTextureHandle(textureHandle);
  if (!TextureIsAllocated(textureHandle)) {
    return NULL;
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetRenderTargetEx(
    int nRenderTargetID, ShaderAPITextureHandle_t colorTextureHandle,
    ShaderAPITextureHandle_t depthTextureHandle) {
  LOCK_SHADERAPI();
  if (IsDeactivated()) return;
  ...
  IDirect3DSurface *pColorSurface, *pZSurface;
  ...
  // NOTE!!!!  If this code changes, also change Dx8SetRenderTarget in
  // playback.cpp
  ...
  if (depthTextureHandle == SHADER_RENDERTARGET_DEPTHBUFFER) {
    ...
    if (tex.m_Flags & Texture_t::IS_DEPTH_STENCIL) {
      ...
      if (pZSurface) pZSurface->AddRef();
    } else {
      hr = ((IDirect3DTexture9 *)tex.GetTexture())
               ->GetSurfaceLevel(0, &pZSurface);
      Assert(!FAILED(hr));
    }
  ...
  if (pColorSurface == m_pBackBufferSurface && nRenderTargetID > 0) {
    // SetRenderTargetEx is overloaded so that if you pass NULL in for
    // anything that isn't the zeroth render target, you effectively disable
    // that MRT index. (Passing in NULL for the zeroth render target means
...
// Changes the render target
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetRenderTarget(
    ShaderAPITextureHandle_t colorTextureHandle,
    ShaderAPITextureHandle_t depthTextureHandle) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::AdvanceCurrentCopy(ShaderAPITextureHandle_t hTexture) {
  ...
  Texture_t &tex = GetTexture(hTexture);
  if (tex.m_NumCopies > 1) {
    if (++tex.m_CurrentCopy >= tex.m_NumCopies) tex.m_CurrentCopy = 0;
...
bool CShaderAPIDx8::TexLock(int level, int cubeFaceID, int xOffset, int yOffset,
  ...
  if (!m_Textures.IsValidIndex(hTexture)) return false;

  // Blow off mip levels if we don't support mipmapping
  if (!g_pHardwareConfig->SupportsMipmapping() && (level > 0)) return false;
...
void CShaderAPIDx8::TexUnlock() {
  ...
  if (m_ModifyTextureLockedLevel >= 0) {
    ...
    UnlockTexture(GetModifyTextureHandle(), tex.m_CurrentCopy,
                  GetModifyTexture(), m_ModifyTextureLockedLevel,
                  (D3DCUBEMAP_FACES)m_ModifyTextureLockedFace);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::TexImage2D(int level, int cubeFaceID, ImageFormat dstFormat,
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::TexSubImage2D(int level, int cubeFaceID, int xOffset,
...
void CShaderAPIDx8::SetAnisotropicLevel(int nAnisotropyLevel) {
  ...
  //       anisotropic can be set per-texture to force it on! This will also
  //       avoid a possible infinite loop that existed before.
  g_pShaderUtil->NoteAnisotropicLevel(nAnisotropyLevel);

  // Never set this to 1. In the case we want it set to 1, we will use this to
  // override
  ...
  for (int i = 0; i < g_pHardwareConfig->Caps().m_NumSamplers; ++i) {
    ...
    SetSamplerState(i, D3DSAMP_MAXANISOTROPY,
                    SamplerState(i).m_nAnisotropicLevel);
  }
}
...
void CShaderAPIDx8::TexSetPriority(int priority) {
  ...
  if (hModifyTexture == INVALID_SHADERAPI_TEXTURE_HANDLE) return;
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::TexWrap(ShaderTexCoordComponent_t coord,
  ...
  switch (texFilterMode) {
    ...
    case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_NEAREST:
      ...
      GetTexture(hModifyTexture).m_MipFilter =
          GetTexture(hModifyTexture).m_NumLevels != 1 ? D3DTEXF_POINT
                                                      : D3DTEXF_NONE;
      break;
    ...
    case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_LINEAR:
      GetTexture(hModifyTexture).m_MinFilter = D3DTEXF_POINT;
      GetTexture(hModifyTexture).m_MipFilter =
          GetTexture(hModifyTexture).m_NumLevels != 1 ? D3DTEXF_LINEAR
                                                      : D3DTEXF_NONE;
      break;
...
void CShaderAPIDx8::TexMagFilter(ShaderTexFilterMode_t texFilterMode) {
  ...
  switch (texFilterMode) {
    ...
    case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_NEAREST:
      Warning(
          "CShaderAPIDx8::TexMagFilter: "
          "SHADER_TEXFILTERMODE_NEAREST_MIPMAP_NEAREST is invalid\n");
      break;
    case SHADER_TEXFILTERMODE_LINEAR_MIPMAP_NEAREST:
      Warning(
          "CShaderAPIDx8::TexMagFilter: "
          "SHADER_TEXFILTERMODE_LINEAR_MIPMAP_NEAREST is invalid\n");
      break;
    case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_LINEAR:
      Warning(
          "CShaderAPIDx8::TexMagFilter: "
          "SHADER_TEXFILTERMODE_NEAREST_MIPMAP_LINEAR is invalid\n");
      break;
    case SHADER_TEXFILTERMODE_LINEAR_MIPMAP_LINEAR:
      Warning(
          "CShaderAPIDx8::TexMagFilter: "
          "SHADER_TEXFILTERMODE_LINEAR_MIPMAP_LINEAR is invalid\n");
      break;
    case SHADER_TEXFILTERMODE_ANISOTROPIC:
      GetTexture(hModifyTexture).m_MagFilter =
          g_pHardwareConfig->Caps().m_bSupportsMagAnisotropicFiltering
              ? D3DTEXF_ANISOTROPIC
              : D3DTEXF_LINEAR;
      break;
    default:
...
void CShaderAPIDx8::SetLightingOrigin(const Vector &vLightingOrigin) {
...
void CShaderAPIDx8::SetLight(int lightNum, const LightDesc_t &desc_) {
  ...
  memcpy(&light.Diffuse, &desc.m_Color[0], 3 * sizeof(float));   //-V512
  memcpy(&light.Specular, &desc.m_Color[0], 3 * sizeof(float));  //-V512
...
void CShaderAPIDx8::SetAmbientLightCube(Vector4D cube[6]) {
  ...
  if (memcmp(&m_DynamicState.m_AmbientLightCube[0][0], cube,
             6 * sizeof(Vector4D))) {
    memcpy(&m_DynamicState.m_AmbientLightCube[0][0], cube,
           6 * sizeof(Vector4D));
...
void CShaderAPIDx8::SetPixelShaderStateAmbientLightCube(int pshReg,
                                                        bool bForceToBlack) {
  float *pCubeBase;
  Vector4D tempCube[6];

  if (bForceToBlack) {
    for (int i = 0; i < 6; i++) tempCube[i].Init();
...
float CShaderAPIDx8::GetAmbientLightCubeLuminance() {
...
void CShaderAPIDx8::CopyRenderTargetToTextureEx(
    ShaderAPITextureHandle_t textureHandle, int nRenderTargetID,
    Rect_t *pSrcRect, Rect_t *pDstRect) {
  LOCK_SHADERAPI();
  VPROF_BUDGET("CShaderAPIDx8::CopyRenderTargetToTexture",
               "Refraction overhead");

  if (!TextureIsAllocated(textureHandle)) return;
  ...
  HRESULT hr =
      Dx9Device()->GetRenderTarget(nRenderTargetID, &pRenderTargetSurface);
  if (FAILED(hr)) {
    Assert(0);
    return;
  }
  ...
  hr = Dx9Device()->StretchRect(pRenderTargetSurface,
                                RectToRECT(pSrcRect, srcRect), pDstSurf,
                                RectToRECT(pDstRect, dstRect), D3DTEXF_LINEAR);
  Assert(!FAILED(hr));
...
void CShaderAPIDx8::CopyRenderTargetToTexture(
    ShaderAPITextureHandle_t textureHandle) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::RenderPass(int nPass, int nPassCount) {
  ...
  // NOTE: not enabled/finished yet... see comment in
  // CShaderAPIDx8::ApplyTextureEnable
  //	int nSampler;
  //	for ( nSampler = 0; nSampler < g_pHardwareConfig->GetSamplerCount();
  // nSampler++ )
  //	{
...
void CShaderAPIDx8::MatrixMode(MaterialMatrixMode_t matrixMode) {
  // NOTE!!!!!!
  // The only time that m_MatrixMode is used is for texture matrices.  Do not
  // use it for anything else unless you change this code!
  if (matrixMode >= MATERIAL_TEXTURE0 && matrixMode <= MATERIAL_TEXTURE7) {
    m_MatrixMode = (D3DTRANSFORMSTATETYPE)(matrixMode - MATERIAL_TEXTURE0 +
                                           D3DTS_TEXTURE0);
  } else {
...
void CShaderAPIDx8::CacheWorldSpaceCameraPosition() {
  ...
  m_WorldSpaceCameraPositon[0] =
      -(view(3, 0) * view(0, 0) + view(3, 1) * view(0, 1) +
        view(3, 2) * view(0, 2));
  m_WorldSpaceCameraPositon[1] =
      -(view(3, 0) * view(1, 0) + view(3, 1) * view(1, 1) +
        view(3, 2) * view(1, 2));
  m_WorldSpaceCameraPositon[2] =
      -(view(3, 0) * view(2, 0) + view(3, 1) * view(2, 1) +
        view(3, 2) * view(2, 2));
  m_WorldSpaceCameraPositon[3] = 1.0f;
}

//-----------------------------------------------------------------------------
// Computes a matrix which includes the poly offset given an initial projection
// matrix
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ComputePolyOffsetMatrix(const D3DXMATRIX &matProjection,
                                            D3DXMATRIX &matProjectionOffset) {
  // We never need to do this on hardware that can handle zbias
  if (g_pHardwareConfig->Caps().m_ZBiasAndSlopeScaledDepthBiasSupported) return;

  float offsetVal =
      -1.0f *
      (m_DesiredState.m_Viewport.MaxZ - m_DesiredState.m_Viewport.MinZ) /
      16384.0f;
...
void CShaderAPIDx8::CachePolyOffsetProjectionMatrix() {
  ComputePolyOffsetMatrix(GetTransform(MATERIAL_PROJECTION),
                          m_CachedPolyOffsetProjectionMatrix);
}
...
bool CShaderAPIDx8::MatrixIsChanging(TransformType_t type) {
  ...
  if ((type != TRANSFORM_IS_GENERAL) &&
      (type == m_DynamicState.m_TransformType[m_CurrStack]))
    return false;
...
void CShaderAPIDx8::SetTextureTransformDimension(TextureStage_t textureMatrix,
                                                 int dimension,
                                                 bool projected) {
  ...
  if (TextureStage(textureMatrix).m_TextureTransformFlags !=
      textureTransformFlags) {
    SetTextureStageState(textureMatrix, D3DTSS_TEXTURETRANSFORMFLAGS,
                         textureTransformFlags);
...
void CShaderAPIDx8::SetBumpEnvMatrix(TextureStage_t textureStage, float m00,
...
void CShaderAPIDx8::UpdateMatrixTransform(TransformType_t type) {
  ...
  if ((textureMatrix >= 0) && (textureMatrix < NUM_TEXTURE_TRANSFORMS)) {
    ...
    transformFlags =
        (type == TRANSFORM_IS_IDENTITY) ? D3DTTFF_DISABLE : D3DTTFF_COUNT3;
  ...
  // Any time the view or projection matrix changes, the user clip planes need
  // recomputing.... Assuming we're not overriding the user clip transform
  if ((m_CurrStack == MATERIAL_PROJECTION) ||
      ((m_CurrStack == MATERIAL_VIEW) &&
       (!m_DynamicState.m_bUserClipTransformOverride))) {
    MarkAllUserClipPlanesDirty();
  }

  // Set the state if it's a texture transform
  if ((m_CurrStack >= MATERIAL_TEXTURE0) &&
      (m_CurrStack <= MATERIAL_TEXTURE7)) {
...
void CShaderAPIDx8::PushDeformation(DeformationBase_t const *pDef) {
  ...
  switch (pDef->m_eType) {
    case DEFORMATION_CLAMP_TO_BOX_IN_WORLDSPACE: {
      BoxDeformation_t const *pBox =
          reinterpret_cast<const BoxDeformation_t *>(pDef);
      m_pDeformationStackPtr->m_nNumParameters = 16;
      memcpy(m_pDeformationStackPtr->m_flDeformationParameters,  //-V512
             &(pBox->m_SourceMins.x), 16 * sizeof(float));
    } break;
...
void CShaderAPIDx8::PopDeformation() {
  Assert(m_pDeformationStackPtr !=
         m_DeformationStack + DEFORMATION_STACK_DEPTH);
  ++m_pDeformationStackPtr;
}
...
// for shaders to set vertex shader constants. returns a packed state which can
// be used to set the dynamic combo
int CShaderAPIDx8::GetPackedDeformationInformation(
    int nMaskOfUnderstoodDeformations, float *pConstantValuesOut,
    int nBufferSize, int nMaximumDeformations, int *pDefCombosOut) const {
  int nCombosFound = 0;
  memset(pDefCombosOut, 0, sizeof(pDefCombosOut[0]) * nMaximumDeformations);
  size_t nRemainingBufferSize = nBufferSize;

  for (const Deformation_t *i =
           m_DeformationStack + DEFORMATION_STACK_DEPTH - 1;
       i >= m_pDeformationStackPtr; i--) {
    int nFloatsOut = 4 * ((i->m_nNumParameters + 3) >> 2);
    if (((1 << i->m_nDeformationType) & nMaskOfUnderstoodDeformations) &&
        (nRemainingBufferSize >= (nFloatsOut * sizeof(float)))) {
      memcpy(pConstantValuesOut, i->m_flDeformationParameters,
             nFloatsOut * sizeof(float));
...
void CShaderAPIDx8::LoadMatrix(float *m) {
  // Check for identity...
  if ((fabs(m[0] - 1.0f) < 1e-3) && (fabs(m[5] - 1.0f) < 1e-3) &&
      (fabs(m[10] - 1.0f) < 1e-3) && (fabs(m[15] - 1.0f) < 1e-3) &&
...
void CShaderAPIDx8::SetFlexWeights(int nFirstWeight, int nCount,
                                   const MorphWeight_t *pWeights) {
  LOCK_SHADERAPI();
  if (g_pHardwareConfig->Caps().m_NumVertexShaderConstants <
  ...
  float *pDest = m_DesiredState
                     .m_pVectorVertexShaderConstant[VERTEX_SHADER_FLEX_WEIGHTS +
                                                    nFirstWeight]
                     .Base();
...
void CShaderAPIDx8::Ortho(double left, double top, double right, double bottom,
  ...
  if (MatrixIsChanging()) {
    ...
    // Our projection transform needs to transform from this space to a LH
    // coordinate system that looks thusly:
    //
    //	y|  /z
    //	 | /
    //	 |/____ x	Z is going into the screen
    ...
    // This is because the viewport transformation from projection space to
    // pixels introduces a -1 scale in the y coordinates
    //		D3DXMatrixOrthoOffCenterLH( &matrix, left, right, bottom, top,
    // zNear,  zFar );
...
void CShaderAPIDx8::PerspectiveX(double fovx, double aspect, double zNear,
                                 double zFar) {
  if (MatrixIsChanging()) {
...
void CShaderAPIDx8::PerspectiveOffCenterX(double fovx, double aspect,
                                          double zNear, double zFar,
  ...
  if (MatrixIsChanging()) {
    ...
    float flFrontPlaneLeft =
        -(width / 2.0f) * (1.0f - left) + left * (width / 2.0f);
    float flFrontPlaneRight =
        -(width / 2.0f) * (1.0f - right) + right * (width / 2.0f);
    float flFrontPlaneBottom =
        -(height / 2.0f) * (1.0f - bottom) + bottom * (height / 2.0f);
    float flFrontPlaneTop =
        -(height / 2.0f) * (1.0f - top) + top * (height / 2.0f);

    Assert(m_CurrStack == MATERIAL_PROJECTION);
    D3DXMATRIX rh;
    D3DXMatrixPerspectiveOffCenterRH(&rh, flFrontPlaneLeft, flFrontPlaneRight,
                                     flFrontPlaneBottom, flFrontPlaneTop, zNear,
                                     zFar);
    m_pMatrixStack[m_CurrStack]->MultMatrixLocal(&rh);
    UpdateMatrixTransform();
  }
...
inline bool CShaderAPIDx8::VertexShaderTransformChanged(int i) {
  return (m_DynamicState.m_TransformChanged[i] & STATE_CHANGED_VERTEX_SHADER) !=
         0;
}

inline bool CShaderAPIDx8::FixedFunctionTransformChanged(int i) {
  return (m_DynamicState.m_TransformChanged[i] &
          STATE_CHANGED_FIXED_FUNCTION) != 0;
}

const D3DXMATRIX &CShaderAPIDx8::GetProjectionMatrix() {
  ...
  if (!m_DynamicState.m_FastClipEnabled) {
    if (bUsingZBiasProjectionMatrix) return m_CachedPolyOffsetProjectionMatrix;
...
void CShaderAPIDx8::SetupSelectionModeVisualizationState() {
  ...
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    ...
    SetVertexShaderConstant(VERTEX_SHADER_MODEL, ident,
                            3 * NUM_MODEL_TRANSFORMS);
  }
}
...
void CShaderAPIDx8::SetVertexShaderViewProj() {
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    D3DXMATRIX transpose = GetTransform(MATERIAL_VIEW) * GetProjectionMatrix();
...
void CShaderAPIDx8::SetVertexShaderModelViewProjAndModelView() {
  if (g_pHardwareConfig->Caps().m_SupportsPixelShaders) {
    ...
    D3DXMatrixMultiply(&modelView, &GetTransform(MATERIAL_MODEL),
                       &GetTransform(MATERIAL_VIEW));
...
void CShaderAPIDx8::UpdateVertexShaderMatrix(int iMatrix) {
  if (iMatrix == 0) {
    ...
    SetVertexShaderConstant(VERTEX_SHADER_MODEL + iMatrix,
                            m_boneMatrix[iMatrix].Base(), 3);
  }
}
...
void CShaderAPIDx8::SetVertexShaderStateSkinningMatrices() {
  ...
  SetVertexShaderConstant(VERTEX_SHADER_MODEL, m_boneMatrix[0].Base(),
                          matricesLoaded * 3, true);
}
...
void CShaderAPIDx8::CommitVertexShaderTransforms() {
  ...
  m_DynamicState.m_TransformChanged[MATERIAL_MODEL] &=
      ~STATE_CHANGED_VERTEX_SHADER;
  m_DynamicState.m_TransformChanged[MATERIAL_VIEW] &=
      ~STATE_CHANGED_VERTEX_SHADER;
  m_DynamicState.m_TransformChanged[MATERIAL_PROJECTION] &=
      ~STATE_CHANGED_VERTEX_SHADER;
}
...
//-----------------------------------------------------------------------------
// Commits transforms for the fixed function pipeline that can happen on a per
// pass basis
//-----------------------------------------------------------------------------
...
void CShaderAPIDx8::CommitFixedFunctionTransforms() {
  ...
  for (i = MATERIAL_VIEW; i <= MATERIAL_PROJECTION; ++i) {
    if (FixedFunctionTransformChanged(i)) {
      D3DTRANSFORMSTATETYPE matrix =
          (i == MATERIAL_VIEW) ? D3DTS_VIEW : D3DTS_PROJECTION;
...
inline bool CShaderAPIDx8::VertexShaderLightingEnableChanged(int i) {
  return (m_DynamicState.m_LightEnableChanged[i] &
          STATE_CHANGED_VERTEX_SHADER) != 0;
}
...
VertexShaderLightTypes_t CShaderAPIDx8::ComputeLightType(int i) const {
  if (!m_DynamicState.m_LightEnable[i]) return LIGHT_NONE;

  switch (m_DynamicState.m_Lights[i].Type) {  //-V719
    case D3DLIGHT_POINT:
      return LIGHT_POINT;
...
void CShaderAPIDx8::SortLights(int *index) {
  ...
  for (int i = 0; i < MAX_NUM_LIGHTS; ++i) {
    VertexShaderLightTypes_t type =
        ComputeLightType(i);  // returns LIGHT_NONE if the light is disabled
    int j = m_DynamicState.m_NumLights;
    if (type != LIGHT_NONE) {
      while (--j >= 0) {
        if (m_DynamicState.m_LightType[j] <= type) break;
...
void CShaderAPIDx8::CommitVertexShaderLighting() {
  ...
  for (i = 0; i < m_DynamicState.m_NumLights; ++i) {
    ...
    float w =
        (light.Type == D3DLIGHT_DIRECTIONAL) && bAtLeastDX90 ? 1.0f : 0.0f;
    ...
    lightState[1].Init(light.Direction.x, light.Direction.y, light.Direction.z,
                       w);

    // The next constant holds the light position
    lightState[2].Init(light.Position.x, light.Position.y, light.Position.z,
                       1.0f);

    // The next constant holds exponent, stopdot, stopdot2, 1 / (stopdot -
    // stopdot2)
    ...
    lightState[4].Init(light.Attenuation0, light.Attenuation1,
                       light.Attenuation2, 0.0f);

    // Set the state
    SetVertexShaderConstant(VERTEX_SHADER_LIGHTS + i * 5, lightState[0].Base(),
                            5);
  }

  if (g_pHardwareConfig->NumIntegerVertexShaderConstants() > 0 &&
      g_pHardwareConfig->NumBooleanVertexShaderConstants() > 0) {
    // Vertex Shader loop counter for number of lights (Only the .x component is
    // used by our shaders) .x is the iteration count, .y is the initial value
    // and .z is the increment step
    ...
    int nLightEnable[VERTEX_SHADER_LIGHT_ENABLE_BOOL_CONST_COUNT] = {0, 0, 0,
                                                                     0};
    for (i = 0; i < m_DynamicState.m_NumLights; ++i) {
      nLightEnable[i] = 1;
    }

    SetBooleanVertexShaderConstant(VERTEX_SHADER_LIGHT_ENABLE_BOOL_CONST,
                                   nLightEnable,
                                   VERTEX_SHADER_LIGHT_ENABLE_BOOL_CONST_COUNT);
  }
}
...
void CShaderAPIDx8::CommitPixelShaderLighting(int pshReg) {
  ...
  for (int i = 0; i < 6; i++) lightState[i].Init();
  ...
  if (nNumLights > 0) {
    ...
    if (light->Type == D3DLIGHT_DIRECTIONAL) {
      ...
      lightState[1].Init(light->Position.x, light->Position.y,
    ...
    if (nNumLights > 1)  // At least two lights
    {
      ...
      lightState[2].Init(light->Diffuse.r, light->Diffuse.g, light->Diffuse.b,
                         0.0f);
      ...
      if (light->Type == D3DLIGHT_DIRECTIONAL) {
        ...
        lightState[3].Init(light->Position.x, light->Position.y,
                           light->Position.z, 0.0f);
      }
      ...
      if (nNumLights > 2)  // At least three lights
      {
        ...
        lightState[4].Init(light->Diffuse.r, light->Diffuse.g, light->Diffuse.b,
                           0.0f);

        if (light->Type == D3DLIGHT_DIRECTIONAL) {
          Vector vDir(light->Direction.x, light->Direction.y,
                      light->Direction.z);
          Vector vPos = m_DynamicState.m_vLightingOrigin - vDir * fFarAway;
          lightState[5].Init(vPos.x, vPos.y, vPos.z, 0.0f);
        } else {
          lightState[5].Init(light->Position.x, light->Position.y,
                             light->Position.z, 0.0f);
        }

        if (nNumLights > 3)  // At least four lights (our current max)
        {
          light = &m_DynamicState.m_Lights[lightIndex[3]];  // Spread 4th
                                                            // light's constants
                                                            // across w
                                                            // components
...
void CShaderAPIDx8::CommitUserClipPlanes(bool bUsingFixedFunction) {
  ...
  if (bUsingFixedFunction !=
      m_DynamicState.m_UserClipLastUpdatedUsingFixedFunction) {
    // fixed function clip planes are in world space, vertex shader clip planes
    // are in clip space, so we need to update every clip plane whenever there's
    // a flip
    m_DynamicState.m_UserClipPlaneChanged =
        (1 << g_pHardwareConfig->MaxUserClipPlanes()) - 1;
    m_DynamicState.m_UserClipLastUpdatedUsingFixedFunction =
...
#ifndef _DEBUG
  if (m_DynamicState.m_UserClipPlaneChanged &
      m_DynamicState.m_UserClipPlaneEnabled &
      ((1 << g_pHardwareConfig->MaxUserClipPlanes()) - 1))
#endif
  {
    ...
    if (bUsingFixedFunction) {
      if (m_DynamicState.m_bUserClipTransformOverride) {
        // D3DXMatrixIdentity( &worldToProjectionInvTrans ); //TODO: Test user
        // clip transforms with this  Since GetUserClipTransform() returns the
        // view matrix if a user supplied transform doesn't exist, the general
        // solution to this should be to transform the user transform by the
        // inverse view matrix  Since we don't know if the user clip is
        // invertable, we'll premultiply by inverse view and cross our fingers
        // that it's right more often than wrong
        D3DXMATRIX viewInverse = GetTransform(MATERIAL_VIEW);
        D3DXMatrixInverse(&viewInverse, NULL, &viewInverse);
        worldToProjectionInvTrans =
            viewInverse * GetUserClipTransform();  // taking a cue from the
                                                   // multiplication below,
                                                   // multiplication goes left
                                                   // into right

        D3DXMatrixInverse(&worldToProjectionInvTrans, NULL,
                          &worldToProjectionInvTrans);
        D3DXMatrixTranspose(&worldToProjectionInvTrans,
                            &worldToProjectionInvTrans);
      } else {
      ...
      worldToProjectionInvTrans =
          GetUserClipTransform() * GetTransform(MATERIAL_PROJECTION);
      D3DXMatrixInverse(&worldToProjectionInvTrans, NULL,
                        &worldToProjectionInvTrans);
      D3DXMatrixTranspose(&worldToProjectionInvTrans,
                          &worldToProjectionInvTrans);
  ...
  for (int i = 0; i < g_pHardwareConfig->MaxUserClipPlanes(); ++i) {
    ...
    if ((m_DynamicState.m_UserClipPlaneEnabled & (1 << i)) == 0) continue;
    ...
    if ((m_DynamicState.m_UserClipPlaneChanged & (1 << i)) == 0) {
      ...
      D3DXPlaneTransform(&clipPlaneProj,
                         &m_DynamicState.m_UserClipPlaneWorld[i],
                         &worldToProjectionInvTrans);
    ...
    D3DXPLANE clipPlaneProj;
    D3DXPlaneTransform(&clipPlaneProj, &m_DynamicState.m_UserClipPlaneWorld[i],
                       &worldToProjectionInvTrans);
...
static void CommitSetViewports(Direct3DDevice9Wrapper *pDevice,
                               ...
                               DynamicState_t &currentState, bool bForce) {
  bool bChanged =
      bForce || memcmp(&desiredState.m_Viewport, &currentState.m_Viewport,
                       sizeof(D3DVIEWPORT9));

  // The width + height can be zero at startup sometimes.
  if (bChanged && (desiredState.m_Viewport.Width != 0) &&
      (desiredState.m_Viewport.Height != 0)) {
    if (ReverseDepthOnX360())  // reverse depth on 360 for better perf through
                               // hierarchical z
    {
    ...
    memcpy(&currentState.m_Viewport, &desiredState.m_Viewport,
           sizeof(D3DVIEWPORT9));
  }
}

void CShaderAPIDx8::SetViewports(int nCount,
...
// Buffer clear	color
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ClearColor3ub(unsigned char r, unsigned char g,
                                  unsigned char b) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::ClearColor4ub(unsigned char r, unsigned char g,
...
D3DCOLOR CShaderAPIDx8::GetActualClearColor(D3DCOLOR clearColor) {
  ...
  if (bConvert) {
    ...
    clearColor |= D3DCOLOR_COLORVALUE(vecLinearColor.x, vecLinearColor.y,
                                      vecLinearColor.z, 0.0f);
  }
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ClearBuffersObeyStencil(bool bClearColor,
                                            bool bClearDepth) {
  LOCK_SHADERAPI();

  if (!bClearColor && !bClearDepth) return;
...
//-------------------------------------------------------------------------
void CShaderAPIDx8::PerformFullScreenStencilOperation() {
  LOCK_SHADERAPI();

  FlushBufferedPrimitives();

  // We'll be drawing a large quad in altered worldspace, user clip planes must
  // be disabled
  SetRenderState(D3DRS_CLIPPLANEENABLE, 0);
...
// Buffer clear
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ClearBuffers(bool bClearColor, bool bClearDepth,
                                 bool bClearStencil, int renderTargetWidth,
                                 int renderTargetHeight) {
  LOCK_SHADERAPI();
  if (ShaderUtil()->GetConfig().m_bSuppressRendering) return;
  ...
  float depth = (ShaderUtil()->GetConfig().bReverseDepth ^ ReverseDepthOnX360())
                    ? 0.0f
                    : 1.0f;
  DWORD mask = 0;
  ...
  // Restore state
  if (bSRGBWriteEnable) {
    // sRGBWriteEnable shouldn't be true if we have no shadow state. . . Assert
    // just in case.
    Assert(m_TransitionTable.CurrentShadowState());
    m_TransitionTable.ApplySRGBWriteEnable(
        *m_TransitionTable.CurrentShadowState());
  }
}
...
IDirect3DSurface *CShaderAPIDx8::GetFrontBufferImage(ImageFormat &format) {
  ...
  hr = Dx9Device()->CreateOffscreenPlainSurface(
      w, h, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &pFullScreenSurfaceBits, NULL);
  ...
  hr = Dx9Device()->CreateOffscreenPlainSurface(
      windowWidth, windowHeight, D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH,
      &pSurfaceBits, NULL);
  Assert(hr == D3D_OK);
  ...
  hr = pFullScreenSurfaceBits->LockRect(&lockedSrcRect, &srcRect,
                                        D3DLOCK_READONLY);
  Assert(hr == D3D_OK);
...
// Lets the shader know about the full-screen texture so it can
//-----------------------------------------------------------------------------
void CShaderAPIDx8::SetLinearToGammaConversionTextures(
    ShaderAPITextureHandle_t hSRGBWriteEnabledTexture,
    ShaderAPITextureHandle_t hIdentityTexture) {
  LOCK_SHADERAPI();
...
IDirect3DSurface *CShaderAPIDx8::GetBackBufferImageHDR(Rect_t *pSrcRect,
  ...
  if ((pDstRect->x + pDstRect->width <= SMALL_BACK_BUFFER_SURFACE_WIDTH) &&
    ...
    if (!m_pSmallBackBufferFP16TempSurface) {
      hr = Dx9Device()->CreateRenderTarget(
          ...
          desc.Format, D3DMULTISAMPLE_NONE, 0, TRUE,
          &m_pSmallBackBufferFP16TempSurface, NULL);
      if (FAILED(hr)) goto CleanUp;
    }
    ...
    // Normally we would only have to create a separate render target here and
    // StretchBlt to it first if AA was enabled, but certain machines/drivers
    // get reboots if we do GetRenderTargetData straight off the backbuffer.
  ...
  hr = Dx9Device()->GetRenderTargetData(pTmpSurface ? pTmpSurface : pBackBuffer,
                                        pSurfaceBits);
...
IDirect3DSurface *CShaderAPIDx8::GetBackBufferImage(Rect_t *pSrcRect,
  ...
  if ((desc.MultiSampleType == D3DMULTISAMPLE_NONE) &&
      (pRenderTarget != m_pBackBufferSurface) &&
      (pSrcRect->width == pDstRect->width) &&
      (pSrcRect->height == pDstRect->height)) {
    // Don't bother to blit through the full-screen texture if we don't
    // have to stretch, we're not coming from the backbuffer, and we don't have
    // to do AA resolve
    pTmpSurface = pRenderTarget;
    pTmpSurface->AddRef();
  } else {
    Texture_t *pTex = &GetTexture(m_hFullScreenTexture);
    IDirect3DTexture *pFullScreenTexture =
        (IDirect3DTexture *)pTex->GetTexture();
...
// Copy bits from a host-memory surface
//-----------------------------------------------------------------------------
void CShaderAPIDx8::CopyBitsFromHostSurface(
    IDirect3DSurface *pSurfaceBits, const Rect_t &dstRect, unsigned char *pData,
    ImageFormat srcFormat, ImageFormat dstFormat, int nDstStride) {
  // Copy out the bits...
  RECT rect;
  rect.left = dstRect.x;
  ...
  if (!FAILED(hr)) {
    ...
    ShaderUtil()->ConvertImageFormat((unsigned char *)pImage, srcFormat, pData,
                                     dstFormat, dstRect.width, dstRect.height,
                                     lockedRect.Pitch, nDstStride);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect,
                               unsigned char *pData, ImageFormat dstFormat,
                               int nDstStride) {
  LOCK_SHADERAPI();
  Assert(pDstRect);

  static Rect_t srcRect;
  if (!pSrcRect) {
  ...
  ImageFormat format;
  IDirect3DSurface *pSurfaceBits =
      GetBackBufferImage(pSrcRect, pDstRect, format);
  if (pSurfaceBits) {
    CopyBitsFromHostSurface(pSurfaceBits, *pDstRect, pData, format, dstFormat,
                            nDstStride);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx8::ReadPixels(int x, int y, int width, int height,
                               unsigned char *pData, ImageFormat dstFormat) {
  Rect_t rect;
  rect.x = x;
  rect.y = y;
...
void CShaderAPIDx8::Bind(IMaterial *pMaterial) {
  ...
  if (m_pMaterial && pMatInt && m_pMaterial->InMaterialPage() &&
      pMatInt->InMaterialPage()) {
    bMaterialChanged =
        (m_pMaterial->GetMaterialPage() != pMatInt->GetMaterialPage());
  } else {
    bMaterialChanged = (m_pMaterial != pMatInt) ||
                       (m_pMaterial && m_pMaterial->InMaterialPage()) ||
                       (pMatInt && pMatInt->InMaterialPage());
  }
...
IMaterialInternal *CShaderAPIDx8::GetBoundMaterial() { return m_pMaterial; }

//-----------------------------------------------------------------------------
// Binds a standard texture
//-----------------------------------------------------------------------------
void CShaderAPIDx8::BindStandardTexture(Sampler_t sampler,
                                        StandardTextureId_t id) {
...
void CShaderAPIDx8::BindStandardVertexTexture(VertexTextureSampler_t sampler,
                                              StandardTextureId_t id) {
  ShaderUtil()->BindStandardVertexTexture(sampler, id);
}

void CShaderAPIDx8::GetStandardTextureDimensions(int *pWidth, int *pHeight,
                                                 StandardTextureId_t id) {
...
bool CShaderAPIDx8::IsInSelectionMode() const { return m_InSelectionMode; }
...
void CShaderAPIDx8::WriteHitRecord() {
  ...
  if (m_SelectionNames.Count() && (m_SelectionMinZ != FLT_MAX)) {
    Assert(m_pCurrSelectionRecord + m_SelectionNames.Count() + 3 <
           m_pSelectionBufferEnd);
    *m_pCurrSelectionRecord++ = m_SelectionNames.Count();
    *m_pCurrSelectionRecord++ =
        (int)((double)m_SelectionMinZ * (double)0xFFFFFFFF);
    *m_pCurrSelectionRecord++ =
        (int)((double)m_SelectionMaxZ * (double)0xFFFFFFFF);
...
void CShaderAPIDx8::RegisterSelectionHit(float minz, float maxz) {
  if (minz < 0) minz = 0;
  if (maxz > 1) maxz = 1;
  if (m_SelectionMinZ > minz) m_SelectionMinZ = minz;
...
// NOTE: These should match g_lightType* in vsh_prep.pl!
// They also correspond to the parallel g_LocalLightTypeXArray[] arrays in
// common_vs_fxc.h
static int g_DX9LightCombinations[][MAX_LIGHTS] = {
    // local0				local1				local2
    // local3
    {LIGHT_NONE, LIGHT_NONE, LIGHT_NONE,
     LIGHT_NONE},  // Zero lights		[ Combo 0]

    {LIGHT_SPOT, LIGHT_NONE, LIGHT_NONE,
     LIGHT_NONE},  // One light		[ Combo 1]
    {LIGHT_POINT, LIGHT_NONE, LIGHT_NONE, LIGHT_NONE},
    {LIGHT_DIRECTIONAL, LIGHT_NONE, LIGHT_NONE, LIGHT_NONE},

    {LIGHT_SPOT, LIGHT_SPOT, LIGHT_NONE,
     LIGHT_NONE},  // Two lights		[ Combo 4]
    ...
    {LIGHT_SPOT, LIGHT_SPOT, LIGHT_SPOT,
     LIGHT_NONE},  // Three lights		[ Combo 10]
    ...
    {LIGHT_SPOT, LIGHT_SPOT, LIGHT_SPOT,
     LIGHT_SPOT},  // Four lights		[ Combo 20]
...
int CShaderAPIDx8::GetCurrentLightCombo() const {
  ...
  // hack - skip the first two for now since we don't know if the ambient cube
  // is needed or not.
  for (int i = 2; i < DX8_LIGHTING_COMBINATION_COUNT; ++i) {
    int j;
    for (j = 0; j < m_DynamicState.m_NumLights; ++j) {
      if (pLightType[j] != g_DX8LightCombinations[i][j + 2]) break;
    }
    if (j == m_DynamicState.m_NumLights) {
      while (j < 2) {
        if (g_DX8LightCombinations[i][j + 2] != LIGHT_NONE) break;
        ++j;
      }
      if (j == 2) {
        if (m_pRenderMesh->HasColorMesh()) {
...
void CShaderAPIDx8::GetDX9LightState(LightState_t *state) const {
  ...
  if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
    Assert(m_DynamicState.m_NumLights <=
           MAX_LIGHTS);  // 2b hardware gets four lights
  } else {
    Assert(m_DynamicState.m_NumLights <=
           (MAX_LIGHTS - 2));  // 2.0 hardware gets two less
  }
...
void CShaderAPIDx8::RecordString(const char *pStr) { RECORD_STRING(pStr); }
...
void CShaderAPIDx8::EvictManagedResources() {
  LOCK_SHADERAPI();
...
bool CShaderAPIDx8::IsDebugTextureListFresh(int numFramesAllowed /* = 1 */) {
  return (m_nDebugDataExportFrame <= m_CurrentFrame) &&
         (m_nDebugDataExportFrame >= m_CurrentFrame - numFramesAllowed);
}
...
KeyValues *CShaderAPIDx8::GetDebugTextureList() { return m_pDebugTextureList; }
...
// Allocate and delete query objects.
ShaderAPIOcclusionQuery_t CShaderAPIDx8::CreateOcclusionQueryObject() {
  // don't allow this on <80 because it falls back to wireframe in that case
  if (m_DeviceSupportsCreateQuery == 0 ||
      g_pHardwareConfig->Caps().m_nDXSupportLevel < 80)
    return INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE;

  // While we're deactivated, m_OcclusionQueryObjects just holds NULL pointers.
  // Create a dummy one here and let ReacquireResources create the actual D3D
  // object.
  if (IsDeactivated()) return INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE;

  IDirect3DQuery9 *pQuery = NULL;
  HRESULT hr = Dx9Device()->CreateQuery(D3DQUERYTYPE_OCCLUSION, &pQuery);
  return (hr == D3D_OK) ? (ShaderAPIOcclusionQuery_t)pQuery
                        : INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE;
}

void CShaderAPIDx8::DestroyOcclusionQueryObject(
...
// Bracket drawing with begin and end so that we can get counts next frame.
void CShaderAPIDx8::BeginOcclusionQueryDrawing(
    ShaderAPIOcclusionQuery_t handle) {
  IDirect3DQuery9 *pQuery = (IDirect3DQuery9 *)handle;
...
int CShaderAPIDx8::OcclusionQuery_GetNumPixelsRendered(
    ShaderAPIOcclusionQuery_t handle, bool bFlush) {
  LOCK_SHADERAPI();
  IDirect3DQuery9 *pQuery = (IDirect3DQuery9 *)handle;

  DWORD nPixels;
  HRESULT hResult =
      pQuery->GetData(&nPixels, sizeof(nPixels), bFlush ? D3DGETDATA_FLUSH : 0);

  // This means that the query will not finish and resulted in an error, game
  // should use the previous query's results and not reissue the query
  if (hResult == D3DERR_DEVICELOST) return OCCLUSION_QUERY_RESULT_ERROR;

  // This means the query isn't finished yet, game will have to use the previous
  // query's results and not reissue the query; wait for query to finish later.
  if (hResult == S_FALSE) return OCCLUSION_QUERY_RESULT_PENDING;
...
void CShaderAPIDx8::SetPixelShaderFogParams(int reg, ShaderFogMode_t fogMode) {
  m_DelayedShaderConstants.iPixelShaderFogParams =
      reg;  // save it off in case the ShaderFogMode_t disables fog. We only
            // find out later.
  float fogParams[4];

  if ((GetPixelFogMode() != MATERIAL_FOG_NONE) &&
    ...
    // CalcRangeFog() always returns 0. Gets rid of a dynamic combo across the
    // ps2x set.
    fogParams[0] = 1.0f;
    fogParams[1] = m_DynamicState.m_FogZ;  // water height
...
void CShaderAPIDx8::SetPixelShaderFogParams(int reg) {
  SetPixelShaderFogParams(reg,
                          m_TransitionTable.CurrentShadowState()->m_FogMode);
}

void CShaderAPIDx8::SetFlashlightState(const FlashlightState_t &state,
                                       const VMatrix &worldToTexture) {
  LOCK_SHADERAPI();
  SetFlashlightStateEx(state, worldToTexture, NULL);
}

void CShaderAPIDx8::SetFlashlightStateEx(const FlashlightState_t &state,
                                         const VMatrix &worldToTexture,
                                         ITexture *pFlashlightDepthTexture) {
  LOCK_SHADERAPI();
...
const FlashlightState_t &CShaderAPIDx8::GetFlashlightState(
    VMatrix &worldToTexture) const {
  worldToTexture = m_FlashlightWorldToTexture;
  return m_FlashlightState;
}

const FlashlightState_t &CShaderAPIDx8::GetFlashlightStateEx(
    VMatrix &worldToTexture, ITexture **ppFlashlightDepthTexture) const {
  worldToTexture = m_FlashlightWorldToTexture;
...
bool CShaderAPIDx8::SupportsMSAAMode(int nMSAAMode) {
  return (D3D_OK == D3D()->CheckDeviceMultiSampleType(
                        m_DisplayAdapter, m_DeviceType,
...
bool CShaderAPIDx8::SupportsCSAAMode(int nNumSamples, int nQualityLevel) {
  ...
  if (g_pHardwareConfig->Caps().m_VendorID != VENDORID_NVIDIA) return false;

  DWORD dwQualityLevels = 0;
  HRESULT hr = D3D()->CheckDeviceMultiSampleType(
      m_DisplayAdapter, m_DeviceType, m_PresentParameters.BackBufferFormat,
      m_PresentParameters.Windowed, ComputeMultisampleType(nNumSamples),
      &dwQualityLevels);

  return ((D3D_OK == hr) && ((int)dwQualityLevels >= nQualityLevel));
}

bool CShaderAPIDx8::SupportsShadowDepthTextures() {
...
ImageFormat CShaderAPIDx8::GetNullTextureFormat() {
  return g_pHardwareConfig->Caps().m_NullTextureFormat;
}

void CShaderAPIDx8::SetShadowDepthBiasFactors(float fShadowSlopeScaleDepthBias,
                                              float fShadowDepthBias) {
...
void CShaderAPIDx8::SetToneMappingScaleLinear(const Vector &scale) {
  if (g_pHardwareConfig->SupportsPixelShaders_2_0()) {
    ...
    SetPixelShaderConstant(TONE_MAPPING_SCALE_PSH_CONSTANT,
                           m_ToneMappingScale.Base());

    // We have to change the fog color since we tone map directly in the shaders
    // in integer HDR mode.
    if (HardwareConfig()->GetHDRType() == HDR_TYPE_INTEGER &&
        m_TransitionTable.CurrentShadowState()) {
      // Get the shadow state in sync since it depends on
      // SetToneMappingScaleLinear.
      ApplyFogMode(
          m_TransitionTable.CurrentShadowState()->m_FogMode, mode_uses_srgb,
          m_TransitionTable.CurrentShadowState()->m_bDisableFogGammaCorrection);
    }
  }
}
...
void CShaderAPIDx8::SetStencilCompareFunction(
    StencilComparisonFunction_t cmpfn) {
  LOCK_SHADERAPI();
...
void CShaderAPIDx8::SetStencilTestMask(uint32_t msk) {
  LOCK_SHADERAPI();
  SetRenderState(D3DRS_STENCILMASK, msk, true);
}

void CShaderAPIDx8::SetStencilWriteMask(uint32_t msk) {
  LOCK_SHADERAPI();
  SetRenderState(D3DRS_STENCILWRITEMASK, msk, true);
}

void CShaderAPIDx8::ClearStencilBufferRectangle(int xmin, int ymin, int xmax,
                                                int ymax, int value) {
  LOCK_SHADERAPI();
  D3DRECT clear;
  clear.x1 = xmin;
  ...
  Dx9Device()->Clear(1, &clear, D3DCLEAR_STENCIL, 0, 0, value);
}

int CShaderAPIDx8::CompareSnapshots(StateSnapshot_t snapshot0,
                                    StateSnapshot_t snapshot1) {
  LOCK_SHADERAPI();
  ...
  const ShadowShaderState_t &shader0 =
      m_TransitionTable.GetSnapshotShader(snapshot0);
  const ShadowShaderState_t &shader1 =
      m_TransitionTable.GetSnapshotShader(snapshot1);

  int dVertex = shader0.m_VertexShader - shader1.m_VertexShader;
  if (dVertex) return dVertex;
  int dVCombo = shader0.m_nStaticVshIndex - shader1.m_nStaticVshIndex;
  if (dVCombo) return dVCombo;
...
void CShaderAPIDx8::BindVertexBuffer(int streamID, IVertexBuffer *pVertexBuffer,
                                     int nOffsetInBytes, int nFirstVertex,
                                     int nVertexCount, VertexFormat_t fmt,
                                     int nRepetitions) {
  LOCK_SHADERAPI();
  MeshMgr()->BindVertexBuffer(streamID, pVertexBuffer, nOffsetInBytes,
                              nFirstVertex, nVertexCount, fmt, nRepetitions);
}

void CShaderAPIDx8::BindIndexBuffer(IIndexBuffer *pIndexBuffer,
                                    int nOffsetInBytes) {
  LOCK_SHADERAPI();
  MeshMgr()->BindIndexBuffer(pIndexBuffer, nOffsetInBytes);
}

void CShaderAPIDx8::Draw(MaterialPrimitiveType_t primitiveType, int nFirstIndex,
                         int nIndexCount) {
  LOCK_SHADERAPI();
...
// to track the actual RT state.  Returns true if the RT gets set, otherwise
// false.
bool CShaderAPIDx8::SetRenderTargetInternalXbox(
    ShaderAPITextureHandle_t hRenderTargetTexture, bool bForce) {
  Assert(0);  // XBOX only
  return false;
}
