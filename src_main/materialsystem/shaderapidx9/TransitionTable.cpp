...
inline unsigned char TextureOp(TextureStateFunc_t func, int stage) {
  ...
  return ((func << TEXTURE_OP_SHIFT) & TEXTURE_OP_MASK) |
         (stage & TEXTURE_STAGE_MASK);
}

inline void GetTextureOp(unsigned char nBits, TextureStateFunc_t* pFunc,
                         int* pStage) {
...
inline ShadowState_t& BoardState() { return g_pTransitionTable->BoardState(); }
#endif
...
// Less functions
//-----------------------------------------------------------------------------
bool CTransitionTable::ShadowStateDictLessFunc::Less(
    const CTransitionTable::ShadowStateDictEntry_t& src1,
    const CTransitionTable::ShadowStateDictEntry_t& src2, void* pCtx) {
  return src1.m_nChecksum < src2.m_nChecksum;
}

bool CTransitionTable::SnapshotDictLessFunc::Less(
    const CTransitionTable::SnapshotDictEntry_t& src1,
    const CTransitionTable::SnapshotDictEntry_t& src2, void* pCtx) {
  return src1.m_nChecksum < src2.m_nChecksum;
}

bool CTransitionTable::UniqueSnapshotLessFunc::Less(
    const CTransitionTable::TransitionList_t& src1,
    const CTransitionTable::TransitionList_t& src2, void* pCtx) {
...
StateSnapshot_t CTransitionTable::CreateStateSnapshot(
  ...
  memcpy(&shaderState.m_ShaderState, &currentShaderState,
         sizeof(ShadowShaderState_t));
  memset(shaderState.m_ShaderState.m_nReserved, 0,
         sizeof(shaderState.m_ShaderState.m_nReserved));
...
CTransitionTable::ShadowStateId_t CTransitionTable::CreateShadowState(
    const ShadowState_t& currentState) {
  int newShaderState = m_ShadowStateList.AddToTail();

  // Copy our snapshot into the list
  memcpy(&m_ShadowStateList[newShaderState], &currentState,
         sizeof(ShadowState_t));
  ...
  for (i = 0; i <= newShaderState; ++i) {
    ...
    m_TransitionTable[newShaderState][newElem].m_FirstOperation =
        INVALID_TRANSITION_OP;
...
//-----------------------------------------------------------------------------
CTransitionTable::ShadowStateId_t CTransitionTable::FindShadowState(
    const ShadowState_t& currentState) const {
  ...
  if (i < 0) return (ShadowStateId_t)-1;
  ...
  for (; i < nDictCount; ++i) {
    ...
    if (!memcmp(&m_ShadowStateList[nShadowState], &currentState,
                sizeof(ShadowState_t)))
      return nShadowState;
  }
...
// Finds a snapshot, if it exists. Or creates a new one if it doesn't.
//-----------------------------------------------------------------------------
StateSnapshot_t CTransitionTable::FindStateSnapshot(
    ShadowStateId_t id, const ShadowShaderState_t& currentState) const {
  SnapshotShaderState_t temp;
  temp.m_ShaderState = currentState;
  temp.m_ShadowStateId = id;
  memset(temp.m_ShaderState.m_nReserved, 0,
         sizeof(temp.m_ShaderState.m_nReserved));
  ...
  if (i < 0) return (StateSnapshot_t)-1;
  ...
  for (; i < nDictCount; ++i) {
    ...
    if ((id == m_SnapshotList[nShapshot].m_ShadowStateId) &&
        !memcmp(&m_SnapshotList[nShapshot].m_ShaderState, &currentState,
                sizeof(ShadowShaderState_t))) {
      return nShapshot;
    }
  }
...
#define SetSamplerState(_stage, _state, _val)                                  \
  {                                                                            \
    if (MSVC_SCOPED_DISABLE_WARNING(4127, (_state != D3DSAMP_NOTSUPPORTED))) { \
      Assert(!g_pShaderDeviceDx8->IsDeactivated());                            \
...
#define SetRenderState(_state, _val)                                       \
  {                                                                        \
    if (MSVC_SCOPED_DISABLE_WARNING(4127, _state != D3DRS_NOTSUPPORTED)) { \
      Assert(!g_pShaderDeviceDx8->IsDeactivated());                        \
...
#define UPDATE_BOARD_TEXTURE_STAGE_STATE(_d3dState, _state, _stage) \
  {                                                                 \
    BoardState().m_TextureStage[_stage].m_##_state =                \
...
#define APPLY_TEXTURE_STAGE_STATE_FUNC(_d3dState, _state)               \
  void Apply##_state(const ShadowState_t& shaderState, int stage) {     \
    SetTextureStageState(stage, _d3dState,                              \
                         shaderState.m_TextureStage[stage].m_##_state); \
    UPDATE_BOARD_TEXTURE_STAGE_STATE(_d3dState, _state, stage);         \
  }

#define APPLY_SAMPLER_STATE_FUNC(_d3dState, _state)                 \
  void Apply##_state(const ShadowState_t& shaderState, int stage) { \
    SetSamplerState(stage, _d3dState,                               \
                    shaderState.m_SamplerState[stage].m_##_state);  \
...
void ApplyFetch4Enable(const ShadowState_t& shaderState, int stage) {
  if (ShaderAPI()->SupportsFetch4()) {
    SetSamplerState(stage, ATISAMP_FETCH4,
                    shaderState.m_SamplerState[stage].m_Fetch4Enable
                        ? ATI_FETCH4_ENABLE
                        : ATI_FETCH4_DISABLE);
  }
...
void CTransitionTable::ApplySRGBWriteEnable(const ShadowState_t& shaderState) {
  // ApplySRGBWriteEnable set to true means that the shader is writing linear
  // values.
  if (CurrentState().m_bLinearColorSpaceFrameBufferEnable) {
    // The shader had better be writing linear values since we can't convert to
    // gamma here. Can't leave this assert here since there are cases where the
    // shader is doing the right thing. This is good to test occasionally to
    // make sure that the shaders are doing the right thing.
    ...
    // fog isn't fixed-function with linear frame buffers, so don't bother with
    // that here.
  } else {
    ...
    if (HardwareConfig()->NeedsShaderSRGBConversion()) {
      if (HardwareConfig()
              ->SupportsPixelShaders_2_b())  // in 2b supported
                                             // devices, we never
                                             // actually enable SRGB
                                             // writes, but instead
                                             // handle the conversion
                                             // in the pixel shader.
                                             // But we want all other
                                             // code to be unaware.
      {
        SetRenderState(D3DRS_SRGBWRITEENABLE, 0);
      } else {
    ...
    if (HardwareConfig()->SpecifiesFogColorInLinearSpace()) {
      ShaderAPI()->ApplyFogMode(shaderState.m_FogMode,
                                shaderState.m_SRGBWriteEnable,
                                shaderState.m_bDisableFogGammaCorrection);
    }
  }
...
void ApplyDisableFogGammaCorrection(const ShadowState_t& shadowState,
                                    int stageUnused) {
  ShaderAPI()->ApplyFogMode(shadowState.m_FogMode,
                            shadowState.m_SRGBWriteEnable,
                            shadowState.m_bDisableFogGammaCorrection);

#ifdef DEBUG_BOARD_STATE
  g_pTransitionTable->BoardState().m_bDisableFogGammaCorrection =
      shadowState.m_bDisableFogGammaCorrection;
#endif
}
...
void CTransitionTable::ApplyDepthTest(const ShadowState_t& state) {
  ...
  if (state.m_ZEnable != D3DZB_FALSE) {
    ...
    if (m_CurrentState.m_ZBias != state.m_ZBias) {
      ...
      m_CurrentState.m_ZBias =
          (PolygonOffsetMode_t)state.m_ZBias;  // Cast two bits from m_ZBias
    }
  }
...
void CTransitionTable::ApplySeparateAlphaBlend(const ShadowState_t& state) {
  if (m_CurrentState.m_SeparateAlphaBlendEnable !=
      state.m_SeparateAlphaBlendEnable) {
    SetRenderState(D3DRS_SEPARATEALPHABLENDENABLE,
                   state.m_SeparateAlphaBlendEnable);
    m_CurrentState.m_SeparateAlphaBlendEnable =
        state.m_SeparateAlphaBlendEnable;
  }
...
void CTransitionTable::ApplyColorTextureStage(const ShadowState_t& state,
                                              int stage) {
...
void CTransitionTable::ApplyAlphaTextureStage(const ShadowState_t& state,
                                              int stage) {
...
#ifdef DEBUG_BOARD_STATE
    BoardState().m_SamplerState[i].m_TextureEnable =
        state.m_SamplerState[i].m_TextureEnable;
#endif
  }
...
void ApplyFogMode(const ShadowState_t& state, int arg) {
  ...
  ShaderAPI()->ApplyFogMode(state.m_FogMode, state.m_SRGBWriteEnable,
...
inline void CTransitionTable::AddTextureTransition(TextureStateFunc_t func,
                                                   int stage) {
  int nElem = m_TransitionOps.AddToTail();
  TransitionOp_t& op = m_TransitionOps[nElem];
...
#define ADD_TEXTURE_STAGE_STATE_TRANSITION(_stage, _state)       \
  if (bForce || (toState.m_TextureStage[_stage].m_##_state !=    \
                 fromState.m_TextureStage[_stage].m_##_state)) { \
...
#define ADD_SAMPLER_STATE_TRANSITION(_stage, _state)             \
  if (bForce || (toState.m_SamplerState[_stage].m_##_state !=    \
                 fromState.m_SamplerState[_stage].m_##_state)) { \
    Assert(_stage < MAX_SAMPLERS);                               \
    AddTextureTransition(TEXTURE_STATE_##_state, _stage);        \
    ++numOps;                                                    \
  }

int CTransitionTable::CreateNormalTransitions(const ShadowState_t& fromState,
                                              const ShadowState_t& toState,
                                              bool bForce) {
  int numOps = 0;

  // Special case for alpha blending to eliminate extra transitions
  bool blendEnableDifferent =
      (toState.m_AlphaBlendEnable != fromState.m_AlphaBlendEnable);
  bool srcBlendDifferent = toState.m_AlphaBlendEnable &&
                           (toState.m_SrcBlend != fromState.m_SrcBlend);
  bool destBlendDifferent = toState.m_AlphaBlendEnable &&
                            (toState.m_DestBlend != fromState.m_DestBlend);
  if (bForce || blendEnableDifferent || srcBlendDifferent ||
      destBlendDifferent) {
    AddTransition(RENDER_STATE_AlphaBlend);
    ++numOps;
  }

  bool blendSeparateAlphaEnableDifferent =
      (toState.m_SeparateAlphaBlendEnable !=
       fromState.m_SeparateAlphaBlendEnable);
  bool srcBlendAlphaDifferent =
      toState.m_SeparateAlphaBlendEnable &&
      (toState.m_SrcBlendAlpha != fromState.m_SrcBlendAlpha);
  bool destBlendAlphaDifferent =
      toState.m_SeparateAlphaBlendEnable &&
      (toState.m_DestBlendAlpha != fromState.m_DestBlendAlpha);
  if (bForce || blendSeparateAlphaEnableDifferent || srcBlendAlphaDifferent ||
      destBlendAlphaDifferent) {
    AddTransition(RENDER_STATE_SeparateAlphaBlend);
    ++numOps;
  }

  bool bAlphaTestEnableDifferent =
      (toState.m_AlphaTestEnable != fromState.m_AlphaTestEnable);
  bool bAlphaFuncDifferent = toState.m_AlphaTestEnable &&
                             (toState.m_AlphaFunc != fromState.m_AlphaFunc);
  bool bAlphaRefDifferent =
      toState.m_AlphaTestEnable && (toState.m_AlphaRef != fromState.m_AlphaRef);
  if (bForce || bAlphaTestEnableDifferent || bAlphaFuncDifferent ||
      bAlphaRefDifferent) {
  ...
  bool bDepthFuncDifferent = (toState.m_ZEnable != D3DZB_FALSE) &&
                             (toState.m_ZFunc != fromState.m_ZFunc);
  bool bDepthBiasDifferent = (toState.m_ZEnable != D3DZB_FALSE) &&
                             (toState.m_ZBias != fromState.m_ZBias);
  if (bForce || bDepthTestEnableDifferent || bDepthFuncDifferent ||
      bDepthBiasDifferent) {
  ...
  if (bForce || (toState.m_bDisableFogGammaCorrection !=
                 fromState.m_bDisableFogGammaCorrection)) {
  ...
  for (int i = 0; i < nStageCount; ++i) {
    ...
    if (!bForce && toState.m_UsingFixedFunction &&
        fromState.m_UsingFixedFunction) {
      const TextureStageShadowState_t& fromTexture =
          fromState.m_TextureStage[i];
...
void CTransitionTable::CreateTransitionTableEntry(int to, int from) {
  ...
  const ShadowState_t& fromState =
      (from >= 0) ? m_ShadowStateList[from] : m_ShadowStateList[to];
  bool bForce = (from < 0);
  ...
  // This is another reason to try to have very little fixed function in the
  // dx8/dx9 path.
  ...
  int nSamplerCount = HardwareConfig()->GetSamplerCount();
  for (int i = 0; i < nSamplerCount; ++i) {
    if (toState.m_SamplerState[i].m_TextureEnable !=
        fromState.m_SamplerState[i].m_TextureEnable) {
  ...
  TransitionList_t& transition =
      (from >= 0) ? m_TransitionTable[to][from] : m_DefaultTransition;
  Assert(numOps <= 255);
  ...
  unsigned short identicalListFirstElem =
      FindIdenticalTransitionList(firstElem, numOps, nFirstTest);
...
bool CTransitionTable::TestShadowState(const ShadowState_t& state,
                                       const ShadowShaderState_t& shaderState) {
...
//-----------------------------------------------------------------------------
unsigned short CTransitionTable::FindIdenticalTransitionList(
    unsigned short firstElem, unsigned short numOps,
    unsigned short nFirstTest) const {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    for (nPotentialMatch = list.m_FirstOperation; nPotentialMatch <= nLastTest;
         ++nPotentialMatch) {
      // Find the first match
      const TransitionOp_t& testOp = m_TransitionOps[nPotentialMatch];
      if (testOp.m_nBits == op.m_nBits) break;
    }
...
//-----------------------------------------------------------------------------
void CTransitionTable::ApplyTransitionList(int snapshot, int nFirstOp,
                                           int nOpCount) {
  ...
  if (nOpCount > 0) {
    ...
    for (int i = 0; i < nOpCount; ++i) {
      ...
      if (pTransitionOp->m_nInfo.m_bIsTextureCode) {
        ...
        (*s_pRenderFunctionTable[pTransitionOp->m_nInfo.m_nOpCode])(shadowState,
                                                                    0);
      }
      ++pTransitionOp;
    }
...
void CTransitionTable::ApplyTransition(TransitionList_t& list, int snapshot) {
  ...
  // dynamic state at all; followed by states that *are* affected by dynamic
  // state
  ...
  for (i = nTextureStageCount; i < MAX_TEXTURE_STAGES; ++i) {
    memcpy(&m_BoardState.m_TextureStage[i],
           &CurrentShadowState()->m_TextureStage[i],
           sizeof(TextureStageShadowState_t));
  }
  m_BoardState.m_UsingFixedFunction =
      CurrentShadowState()->m_UsingFixedFunction;

  // State blocks bypass the code that sets the board state
#ifdef _DEBUG
  // NOTE: A memcmp here isn't enough since we don't set alpha args in cases
  // where the op is nothing. Assert( !memcmp( &m_BoardState,
  // &CurrentShadowState(), sizeof(m_BoardState) ) );
  ...
  for (i = 0; i < nTextureStageCount; i++) {
    if (!testState1.m_UsingFixedFunction) {
      ...
      testState2.m_TextureStage[i].m_ColorArg1 =
          testState1.m_TextureStage[i].m_ColorArg1;
      testState2.m_TextureStage[i].m_ColorArg2 =
          testState1.m_TextureStage[i].m_ColorArg2;
      testState2.m_TextureStage[i].m_AlphaOp =
          testState1.m_TextureStage[i].m_AlphaOp;
      testState2.m_TextureStage[i].m_AlphaArg1 =
          testState1.m_TextureStage[i].m_AlphaArg1;
      testState2.m_TextureStage[i].m_AlphaArg2 =
          testState1.m_TextureStage[i].m_AlphaArg2;
    } else {
...
StateSnapshot_t CTransitionTable::TakeSnapshot() {
  ...
  const ShadowShaderState_t& currentShaderState =
      ShaderShadow()->GetShadowShaderState();
  StateSnapshot_t snapshotId =
      FindStateSnapshot(shadowStateId, currentShaderState);
  if (snapshotId == -1) {
    // Create entry in state transition table
...
//-----------------------------------------------------------------------------
void CTransitionTable::ApplyShaderState(
    const ShadowState_t& shadowState, const ShadowShaderState_t& shaderState) {
...
void CTransitionTable::UseSnapshot(StateSnapshot_t snapshotId) {
  ...
  // NOTE: This occurs regardless of whether the snapshot changed because it
  // depends on dynamic state (namely, the dynamic vertex + pixel shader index)
  // Followed by things that are not
  ApplyShaderState(m_ShadowStateList[id],
                   m_SnapshotList[snapshotId].m_ShaderState);

#ifdef _DEBUG
  // NOTE: We can't ship with this active because mod makers may well violate
  // this rule We don't want no stinking fixed-function on hardware that has
  // vertex and pixel shaders. . This could cause a serious perf hit.
...
void CTransitionTable::UseDefaultState() {
  ...
  for (int i = 0; i < nSamplerCount; ++i) {
    ...
    if (ShaderAPI()->SupportsFetch4()) {
      SetSamplerState(i, ATISAMP_FETCH4,
                      SamplerState(i).m_Fetch4Enable ? ATI_FETCH4_ENABLE
                                                     : ATI_FETCH4_DISABLE);
    }
  }
...
void CTransitionTable::OverrideDepthEnable(bool bEnable, bool bDepthEnable) {
  if (bEnable != m_CurrentState.m_bOverrideDepthEnable) {
    ...
    m_CurrentState.m_OverrideZWriteEnable =
        bDepthEnable ? D3DZB_TRUE : D3DZB_FALSE;
...
void CTransitionTable::EnableLinearColorSpaceFrameBuffer(bool bEnable) {
  if (m_CurrentState.m_bLinearColorSpaceFrameBufferEnable != bEnable &&
      CurrentShadowState()) {
    ShaderAPI()->FlushBufferedPrimitives();
