...
class PushBuffer {
  ...
  uint32_t m_BufferData[PUSHBUFFER_NELEMS];

 public:
  PushBuffer(void) { m_State = PUSHBUFFER_AVAILABLE; }
};
...
struct LockedBufferContext {
  PushBuffer *m_pPushBuffer;  // if a push buffer was used to hold
                              // the temporary data, this will be non-0
  void *m_pMallocedMemory;    // if memory had to be malloc'd, this will be set.

  size_t m_MallocSize;  // # of bytes malloced if mallocedmem ptr non-0
...
class D3DDeviceWrapper {
 private:
  ...
  void HandleAsynchronousLockVBCommand(uint32_t const *dptr);
  void HandleAsynchronousUnLockVBCommand(uint32_t const *dptr);
  void HandleAsynchronousLockIBCommand(uint32_t const *dptr);
  void HandleAsynchronousUnLockIBCommand(uint32_t const *dptr);
 ...
 public:
  ...
  bool IsActive(void) const { return m_pD3DDevice != NULL; }
  ...
  void SetDevicePtr(IDirect3DDevice9 *pD3DDev) { m_pD3DDevice = pD3DDev; }
  ...
  HRESULT CreateOffscreenPlainSurface(UINT Width, UINT Height, D3DFORMAT Format,
                                      ...
                                      HANDLE *pSharedHandle) {
    Synchronize();
    return m_pD3DDevice->CreateOffscreenPlainSurface(
  ...
  HRESULT CreateDepthStencilSurface(UINT Width, UINT Height, D3DFORMAT Format,
                                    ...
                                    HANDLE *pSharedHandle) {
    Synchronize();
    return m_pD3DDevice->CreateDepthStencilSurface(
  ...
  FORCEINLINE void SetVertexShaderConstantF(UINT StartRegister,
    ...
    if (ASyncMode()) {
      ...
      memcpy(m_pOutputPtr + 3, pConstantData,
             sizeof(float) * 4 * Vector4fCount);
      m_pOutputPtr += 3 + 4 * Vector4fCount;
    } else
      DO_D3D(SetVertexShaderConstantF(StartRegister, pConstantData,
                                      Vector4fCount));
  }
  ...
  FORCEINLINE void SetPixelShaderConstantI(UINT StartRegister,
    ...
    if (ASyncMode()) {
      ...
      DO_D3D(SetPixelShaderConstantI(StartRegister, pConstantData,
                                     Vector4IntCount));
  }
  ...
  HRESULT StretchRect(IDirect3DSurface9 *pSourceSurface,
    ...
    if (ASyncMode()) {
      ...
      *(m_pOutputPtr++) = (pDestRect != NULL);
      if (pDestRect) memcpy(m_pOutputPtr, pDestRect, sizeof(RECT));
      m_pOutputPtr += N_DWORDS(RECT);
      *(m_pOutputPtr++) = Filter;
      return S_OK;  // !bug!
    } else
  ...
  HRESULT Present(CONST RECT *pSourceRect, CONST RECT *pDestRect,
    ...
    if (ASyncMode()) {
      ...
      *(m_pOutputPtr++) = (pSourceRect != NULL);
      if (pSourceRect) memcpy(m_pOutputPtr, pSourceRect, sizeof(RECT));
      m_pOutputPtr += N_DWORDS(RECT);
      *(m_pOutputPtr++) = (pDestRect != NULL);
      if (pDestRect) memcpy(m_pOutputPtr, pDestRect, sizeof(RECT));
      m_pOutputPtr += N_DWORDS(RECT);
      *(m_pOutputPtr++) = (uint32_t)hDestWindowOverride;
      *(m_pOutputPtr++) = (pDirtyRegion != NULL);
      if (pDirtyRegion) memcpy(m_pOutputPtr, pDirtyRegion, sizeof(RGNDATA));
