...
class CTransitionTable {
 public:
  ...
  struct CurrentState_t {
    // Everything in this 'CurrentState' structure is a state whose value we
    // don't care about under certain circumstances, (which therefore can
    // diverge from the shadow state), or states which we override in the
    // dynamic pass.
 ...
 private:
  ...
  class ShadowStateDictLessFunc {
   public:
    bool Less(const ShadowStateDictEntry_t &src1,
              const ShadowStateDictEntry_t &src2, void *pCtx);
  };

  class SnapshotDictLessFunc {
   public:
    bool Less(const SnapshotDictEntry_t &src1, const SnapshotDictEntry_t &src2,
              void *pCtx);
  };

  class UniqueSnapshotLessFunc {
   public:
    bool Less(const TransitionList_t &src1, const TransitionList_t &src2,
              void *pCtx);
  };

  CurrentTextureStageState_t &TextureStage(int stage) {
    return m_CurrentState.m_TextureStage[stage];
  }
  const CurrentTextureStageState_t &TextureStage(int stage) const {
    return m_CurrentState.m_TextureStage[stage];
  }

  CurrentSamplerState_t &SamplerState(int stage) {
    return m_CurrentState.m_SamplerState[stage];
  }
  const CurrentSamplerState_t &SamplerState(int stage) const {
    return m_CurrentState.m_SamplerState[stage];
  }
  ...
  StateSnapshot_t FindStateSnapshot(
  ...
  bool TestShadowState(const ShadowState_t &state,
  ...
  void ApplyShaderState(const ShadowState_t &shadowState,
                        const ShadowShaderState_t &shaderState);

  // Wrapper for the non-standard transitions for stateblock + non-stateblock
  // cases
  int CreateNormalTransitions(const ShadowState_t &fromState,
                              const ShadowState_t &toState, bool bForce);
 ...
 private:
  ...
  CUtlSortVector<ShadowStateDictEntry_t, ShadowStateDictLessFunc>
      m_ShadowStateDict;
