...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(
    CHardwareConfig, IMaterialSystemHardwareConfig,
...
void CHardwareConfig::ForceCapsToDXLevel(HardwareCaps_t *pCaps, int nDxLevel,
  ...
  switch (nDxLevel) {
    ...
    case 70:
      ...
      pCaps->m_UseFastClipping = true;
      pCaps->m_bNeedsATICentroidHack = false;
      //		pCaps->m_bColorOnSecondStream = false;	// dont'
      // force this!
      pCaps->m_bSupportsStreamOffset = false;
      pCaps->m_bFogColorSpecifiedInLinearSpace = false;
      pCaps->m_bFogColorAlwaysLinearSpace = false;
...
//-----------------------------------------------------------------------------
void CHardwareConfig::SetupHardwareCaps(int nDXLevel,
                                        const HardwareCaps_t &actualCaps) {
  Assert(nDXLevel != 0);
  ...
  if (m_Caps.m_nDXSupportLevel != m_Caps.m_nMaxDXSupportLevel ||
      CommandLine()->ParmValue("-maxdxlevel", 0) > 0) {
...
// Sets up the hardware caps given the specified DX level
//-----------------------------------------------------------------------------
void CHardwareConfig::SetupHardwareCaps(const ShaderDeviceInfo_t &mode,
                                        const HardwareCaps_t &actualCaps) {
  memcpy(&m_ActualCaps, &actualCaps, sizeof(HardwareCaps_t));
  SetupHardwareCaps(mode.m_nDXLevel, actualCaps);
}

void CHardwareConfig::OverrideStreamOffsetSupport(bool bOverrideEnabled,
                                                  bool bEnableSupport) {
  if (bOverrideEnabled) {
...
bool CHardwareConfig::SupportsCompressedTextures() const {
  Assert(m_Caps.m_SupportsCompressedTextures !=
...
int CHardwareConfig::GetShadowFilterMode() const {
  if (!m_Caps.m_bSupportsShadowDepthTextures ||
      !ShaderUtil()->GetConfig().ShadowDepthTexture())
    return 0;
...
static ConVar r_shader_srgb(
    "r_shader_srgb", "0", 0,
    "-1 = use hardware caps. 0 = use hardware srgb. 1 = use shader "
    "srgb(software lookup)");  // -1=use caps 0=off 1=on
...
int CHardwareConfig::NeedsShaderSRGBConversion() const {
  ...
  switch (cValue) {
    ...
    default:
      return m_Caps.m_bDX10Card ? TRUE
                                : FALSE;  // !!! change to return false after
                                          // portal deport built!!!!!
  }
}
...
static ConVar mat_disablehwmorph("mat_disablehwmorph", "0", 0,
...
bool CHardwareConfig::SupportsHDR() const {
  // This is a deprecated function. . use GetHDRType instead.  For shipping HL2,
  // this always being false is correct.
  Assert(0);
  return false;
}
...
bool CHardwareConfig::SupportsHDRMode(HDRType_t nHDRType) const {
  switch (nHDRType) {
    ...
    case HDR_TYPE_INTEGER:
      return (m_Caps.m_MaxHDRType == HDR_TYPE_INTEGER) ||
             (m_Caps.m_MaxHDRType == HDR_TYPE_FLOAT);
...
bool CHardwareConfig::IsAAEnabled() const {
  ...
  //	bool bAntialiasing = ( m_PresentParameters.MultiSampleType !=
  // D3DMULTISAMPLE_NONE ); 	return bAntialiasing;
}
...
HDRType_t CHardwareConfig::GetHDRType() const {
  if (m_bHDREnabled && (GetDXSupportLevel() >= 90)) return m_Caps.m_HDRType;
