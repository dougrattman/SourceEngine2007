...
class CFunctionCommit {
 public:
  ...
  void CallCommitFuncs(ID3D10Device *pDevice,
                       const ShaderStateDx10_t &desiredState,
                       ShaderStateDx10_t &currentState, bool bForce = false);
...
class CShaderAPIDx10 : public CShaderAPIBase, public IDebugTextureInfo {
 ...
 private:
  ...
  virtual void SetPixelShaderStateAmbientLightCube(int pshReg,
  ...
  float GammaToLinear_HardwareSpecific(float fGamma) const { return 0.; }

  float LinearToGamma_HardwareSpecific(float fLinear) const { return 0.; }
