...
//-----------------------------------------------------------------------------
void CFunctionCommit::CallCommitFuncs(ID3D10Device *pDevice,
...
#define IMPLEMENT_RENDERSTATE_FUNC(_func_name, _state, _d3dFunc)         \
  static void _func_name(ID3D10Device *pDevice,                          \
                         const ShaderStateDx10_t &desiredState,          \
...
IMPLEMENT_RENDERSTATE_FUNC(CommitSetTopology, m_Topology,
                           IASetPrimitiveTopology)
IMPLEMENT_RENDERSTATE_FUNC(CommitSetVertexShader, m_pVertexShader, VSSetShader)
IMPLEMENT_RENDERSTATE_FUNC(CommitSetGeometryShader, m_pGeometryShader,
                           GSSetShader)
IMPLEMENT_RENDERSTATE_FUNC(CommitSetPixelShader, m_pPixelShader, PSSetShader)

static void CommitSetInputLayout(ID3D10Device *pDevice,
                                 const ShaderStateDx10_t &desiredState,
                                 ShaderStateDx10_t &currentState, bool bForce) {
...
static void CommitSetViewports(ID3D10Device *pDevice,
                               const ShaderStateDx10_t &desiredState,
                               ShaderStateDx10_t &currentState, bool bForce) {
  ...
  pDevice->RSSetViewports(desiredState.m_nViewportCount,
...
static void CommitSetIndexBuffer(ID3D10Device *pDevice,
                                 const ShaderStateDx10_t &desiredState,
                                 ShaderStateDx10_t &currentState, bool bForce) {
  const ShaderIndexBufferStateDx10_t &newState = desiredState.m_IndexBuffer;
  bool bChanged = bForce || memcmp(&newState, &currentState.m_IndexBuffer,
                                   sizeof(ShaderIndexBufferStateDx10_t));
  if (!bChanged) return;

  pDevice->IASetIndexBuffer(newState.m_pBuffer, newState.m_Format,
                            newState.m_nOffset);
  memcpy(&currentState.m_IndexBuffer, &newState,
         sizeof(ShaderIndexBufferStateDx10_t));
}

static void CommitSetVertexBuffer(ID3D10Device *pDevice,
                                  const ShaderStateDx10_t &desiredState,
                                  ShaderStateDx10_t &currentState,
                                  bool bForce) {
  ...
  for (int i = 0; i < MAX_DX10_STREAMS; ++i) {
    const ShaderVertexBufferStateDx10_t &newState =
        desiredState.m_pVertexBuffer[i];
    bool bMatch =
        !bForce && !memcmp(&newState, &currentState.m_pVertexBuffer[i],
                           sizeof(ShaderVertexBufferStateDx10_t));
    if (!bMatch) {
...
static void GenerateRasterizerDesc(D3D10_RASTERIZER_DESC *pDesc,
...
static void CommitSetRasterState(ID3D10Device *pDevice,
                                 const ShaderStateDx10_t &desiredState,
                                 ShaderStateDx10_t &currentState, bool bForce) {
...
void CShaderAPIDx10::CommitStateChanges(bool bForce) {
  ...
  m_Commit.CallCommitFuncs(D3D10Device(), m_DesiredState, m_CurrentState,
                           bForce);
}
...
//-----------------------------------------------------------------------------
void CShaderAPIDx10::SetViewports(int nCount,
                                  const ShaderViewport_t *pViewports) {
  nCount = std::min(nCount, MAX_DX10_VIEWPORTS);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx10::ClearColor3ub(unsigned char r, unsigned char g,
                                   unsigned char b) {
...
void CShaderAPIDx10::ClearColor4ub(unsigned char r, unsigned char g,
...
void CShaderAPIDx10::ClearBuffers(bool bClearColor, bool bClearDepth,
...
void CShaderAPIDx10::BindVertexShader(VertexShaderHandle_t hVertexShader) {
  ID3D10VertexShader *pVertexShader =
      g_pShaderDeviceDx10->GetVertexShader(hVertexShader);
...
void CShaderAPIDx10::BindGeometryShader(
    GeometryShaderHandle_t hGeometryShader) {
  ID3D10GeometryShader *pGeometryShader =
      g_pShaderDeviceDx10->GetGeometryShader(hGeometryShader);
  ADD_RENDERSTATE_FUNC(CommitSetGeometryShader, m_pGeometryShader,
                       pGeometryShader);
}

void CShaderAPIDx10::BindPixelShader(PixelShaderHandle_t hPixelShader) {
  ID3D10PixelShader *pPixelShader =
      g_pShaderDeviceDx10->GetPixelShader(hPixelShader);
  ADD_RENDERSTATE_FUNC(CommitSetPixelShader, m_pPixelShader, pPixelShader);
}

void CShaderAPIDx10::BindVertexBuffer(int nStreamID,
...
void CShaderAPIDx10::BindIndexBuffer(IIndexBuffer *pIndexBuffer,
                                     int nOffsetInBytes) {
  CIndexBufferDx10 *pIndexBufferDx10 =
      static_cast<CIndexBufferDx10 *>(pIndexBuffer);

  ShaderIndexBufferStateDx10_t state;
  if (pIndexBufferDx10) {
    state.m_pBuffer = pIndexBufferDx10->GetDx10Buffer();
    state.m_Format =
        (pIndexBufferDx10->GetIndexFormat() == MATERIAL_INDEX_FORMAT_16BIT)
...
void CShaderAPIDx10::Unbind(GeometryShaderHandle_t hShader) {
  ID3D10GeometryShader *pShader =
      g_pShaderDeviceDx10->GetGeometryShader(hShader);
  Assert(pShader);
...
//-----------------------------------------------------------------------------
void CShaderAPIDx10::Draw(MaterialPrimitiveType_t primitiveType,
...
bool CShaderAPIDx10::CanDownloadTextures() const { return false; }
...
StateSnapshot_t CShaderAPIDx10::TakeSnapshot() {
  ...
  if (g_pShaderShadowDx10->m_IsTranslucent) id |= TRANSLUCENT;
  if (g_pShaderShadowDx10->m_IsAlphaTested) id |= ALPHATESTED;
  if (g_pShaderShadowDx10->m_bUsesVertexAndPixelShaders)
    id |= VERTEX_AND_PIXEL_SHADERS;
  if (g_pShaderShadowDx10->m_bIsDepthWriteEnabled) id |= DEPTHWRITE;
  return id;
}
...
VertexFormat_t CShaderAPIDx10::ComputeVertexFormat(
    int numSnapshots, StateSnapshot_t *pIds) const {
  return 0;
}

// Gets the vertex format for a set of snapshot ids
VertexFormat_t CShaderAPIDx10::ComputeVertexUsage(int numSnapshots,
...
void CShaderAPIDx10::Color3ub(unsigned char r, unsigned char g,
                              unsigned char b) {}

void CShaderAPIDx10::Color3ubv(unsigned char const *rgb) {}

void CShaderAPIDx10::Color4ub(unsigned char r, unsigned char g, unsigned char b,
                              unsigned char a) {}

void CShaderAPIDx10::Color4ubv(unsigned char const *rgba) {}

void CShaderAPIDx10::GetStandardTextureDimensions(int *pWidth, int *pHeight,
...
void CShaderAPIDx10::SetHeightClipMode(
...
IMesh *CShaderAPIDx10::CreateStaticMesh(VertexFormat_t fmt,
...
IMesh *CShaderAPIDx10::GetDynamicMesh(IMaterial *pMaterial,
...
IMesh *CShaderAPIDx10::GetDynamicMeshEx(IMaterial *pMaterial,
                                        VertexFormat_t fmt,
                                        int nHWSkinBoneCount, bool buffered,
                                        IMesh *pVertexOverride,
                                        IMesh *pIndexOverride) {
...
IMesh *CShaderAPIDx10::GetFlexMesh() { return &m_Mesh; }
...
void CShaderAPIDx10::PerspectiveX(double fovx, double aspect, double zNear,
                                  double zFar) {}

void CShaderAPIDx10::PerspectiveOffCenterX(double fovx, double aspect,
                                           double zNear, double zFar,
...
void CShaderAPIDx10::SceneFogColor3ub(unsigned char r, unsigned char g,
                                      unsigned char b) {}
...
void CShaderAPIDx10::FogColor3ub(unsigned char r, unsigned char g,
                                 unsigned char b) {}
...
void CShaderAPIDx10::GetViewport(int &x, int &y, int &width,
                                 int &height) const {}
...
void CShaderAPIDx10::SetVertexShaderConstant(int var, float const *pVec,
...
void CShaderAPIDx10::SetLinearToGammaConversionTextures(
...
ImageFormat CShaderAPIDx10::GetNearestRenderTargetFormat(
    ImageFormat fmt) const {
  return fmt;
}

// Sets the texture state
void CShaderAPIDx10::BindTexture(Sampler_t stage,
...
void CShaderAPIDx10::TexImage2D(int level, int cubeFace, ImageFormat dstFormat,
                                int zOffset, int width, int height,
                                ImageFormat srcFormat, bool bSrcIsTiled,
                                void *imageData) {}

void CShaderAPIDx10::TexSubImage2D(int level, int cubeFace, int xOffset,
                                   int yOffset, int zOffset, int width,
                                   int height, ImageFormat srcFormat,
                                   int srcStride, bool bSrcIsTiled,
                                   void *imageData) {}

bool CShaderAPIDx10::TexLock(int level, int cubeFaceID, int xOffset,
...
void CShaderAPIDx10::TexWrap(ShaderTexCoordComponent_t coord,
...
void CShaderAPIDx10::CreateTextures(ShaderAPITextureHandle_t *pHandles,
...
ShaderAPITextureHandle_t CShaderAPIDx10::CreateDepthTexture(
...
void CShaderAPIDx10::ReadPixels(Rect_t *pSrcRect, Rect_t *pDstRect,
                                unsigned char *data, ImageFormat dstFormat,
                                int nDstStride) {}
...
int CShaderAPIDx10::SelectionMode(bool selectionMode) { return 0; }
...
CMeshBuilder *CShaderAPIDx10::GetVertexModifyBuilder() { return 0; }
...
double CShaderAPIDx10::CurrentTime() const { return Plat_FloatTime(); }
...
void CShaderAPIDx10::SetTextureTransformDimension(TextureStage_t textureStage,
                                                  int dimension,
                                                  bool projected) {}

void CShaderAPIDx10::SetBumpEnvMatrix(TextureStage_t textureStage, float m00,
