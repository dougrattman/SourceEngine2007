...
bool CShaderDeviceMgrDx10::ComputeCapsFromD3D(HardwareCaps_t *pCaps,
  ...
  Q_UnicodeToUTF8(desc.Description, pCaps->m_pDriverName,
...
IDXGIOutput *CShaderDeviceMgrDx10::GetAdapterOutput(int nAdapter) const {
  ...
  IDXGIOutput *pOutput;
  for (UINT i = 0; pAdapter->EnumOutputs(i, &pOutput) != DXGI_ERROR_NOT_FOUND;
       ++i) {
...
int CShaderDeviceMgrDx10::GetModeCount(int nAdapter) const {
  ...
  // get the number of available display mode for the given format and scanline
  // order
...
//-----------------------------------------------------------------------------
void CShaderDeviceMgrDx10::GetModeInfo(ShaderDisplayMode_t *pInfo, int nAdapter,
                                       int nMode) const {
  // Default error state
  ...
  DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM_SRGB;  // desired color format
  UINT flags =
      DXGI_ENUM_MODES_INTERLACED;  // desired scanline order and/or scaling
  ...
  if ((UINT)nMode >= num) return;

  DXGI_MODE_DESC *pDescs =
      (DXGI_MODE_DESC *)_alloca(num * sizeof(DXGI_MODE_DESC));
  ...
  //	pInfo->m_Format      = ImageLoader::D3DFormatToImageFormat(
  // pDescs[nMode].Format );
...
bool CShaderDeviceDx10::InitDevice(void *hWnd, int nAdapter,
  ...
  if (m_nAdapter != -1) {
    Warning(
        "CShaderDeviceDx10::SetMode: Previous mode has not been shut down!\n");
    return false;
  }
  ...
  hr = m_pSwapChain->GetBuffer(0, __uuidof(ID3D10Texture2D),
                               (LPVOID *)&pBackBuffer);
  if (FAILED(hr)) return FALSE;
  ...
  g_pHardwareConfig->SetupHardwareCaps(
      mode, g_ShaderDeviceMgrDx10.GetHardwareCaps(nAdapter));
...
//-----------------------------------------------------------------------------
void CShaderDeviceDx10::SpewDriverInfo() const { Warning("Dx10 Driver!\n"); }
...
// Camma ramp
//-----------------------------------------------------------------------------
void CShaderDeviceDx10::SetHardwareGammaRamp(float fGamma,
  ...
  if (FAILED(hr)) {
    Warning(
        "CShaderDeviceDx10::SetHardwareGammaRamp: Unable to set gamma "
        "controls!\n");
  }
}
...
//-----------------------------------------------------------------------------
IShaderBuffer *CShaderDeviceDx10::CompileShader(const char *pProgram,
                                                size_t nBufLen,
  ...
  CShaderBuffer<ID3D10Blob> *pShaderBuffer =
      new CShaderBuffer<ID3D10Blob>(pCompiledShader);
  if (pErrorMessages) {
    pErrorMessages->Release();
  }
...
VertexShaderHandle_t CShaderDeviceDx10::CreateVertexShader(
  ...
  hr = D3D10ReflectShader(pShaderBuffer->GetBits(), pShaderBuffer->GetSize(),
                          &pInfo);
  if (FAILED(hr) || !pInfo) {
    pShader->Release();
...
ID3D10InputLayout *CShaderDeviceDx10::GetInputLayout(
  ...
  insert.m_pInputLayout = CreateInputLayout(
      format, shader.m_pInfo, shader.m_pByteCode, shader.m_nByteCodeLen);
  dict.Insert(insert);
...
//-----------------------------------------------------------------------------
IMesh *CShaderDeviceDx10::CreateStaticMesh(VertexFormat_t vertexFormat,
...
//-----------------------------------------------------------------------------
IVertexBuffer *CShaderDeviceDx10::CreateVertexBuffer(ShaderBufferType_t type,
                                                     VertexFormat_t fmt,
                                                     int nVertexCount,
                                                     const char *pBudgetGroup) {
  LOCK_SHADERAPI();
  CVertexBufferDx10 *pVertexBuffer =
      new CVertexBufferDx10(type, fmt, nVertexCount, pBudgetGroup);
...
void CShaderDeviceDx10::DestroyVertexBuffer(IVertexBuffer *pVertexBuffer) {
  LOCK_SHADERAPI();
  if (pVertexBuffer) {
    CVertexBufferDx10 *pVertexBufferBase =
        assert_cast<CVertexBufferDx10 *>(pVertexBuffer);
    g_pShaderAPIDx10->UnbindVertexBuffer(pVertexBufferBase->GetDx10Buffer());
    delete pVertexBufferBase;
  }
}

IIndexBuffer *CShaderDeviceDx10::CreateIndexBuffer(ShaderBufferType_t type,
                                                   MaterialIndexFormat_t fmt,
                                                   int nIndexCount,
                                                   const char *pBudgetGroup) {
  LOCK_SHADERAPI();
  CIndexBufferDx10 *pIndexBuffer =
      new CIndexBufferDx10(type, fmt, nIndexCount, pBudgetGroup);
  return pIndexBuffer;
}
...
void CShaderDeviceDx10::DestroyIndexBuffer(IIndexBuffer *pIndexBuffer) {
  LOCK_SHADERAPI();
  if (pIndexBuffer) {
    CIndexBufferDx10 *pIndexBufferBase =
        assert_cast<CIndexBufferDx10 *>(pIndexBuffer);
