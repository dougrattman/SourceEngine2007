...
static bool TestTextureFormat(D3DFORMAT format, bool bIsRenderTarget,
  ...
  if (bIsRenderTarget && ((format == NVFMT_RAWZ) || (format == NVFMT_INTZ) ||
    ...
    nUsage = bIsRenderTarget ? D3DUSAGE_RENDERTARGET : 0;
    if (bIsVertexTexture) {
  ...
  HRESULT hr{D3D()->CheckDeviceFormat(
      ...
      ImageLoader::ImageFormatToD3DFormat(g_DeviceFormat), nUsage,
...
static D3DFORMAT GetNearestD3DColorFormat(ImageFormat fmt, bool isRenderTarget,
  ...
  switch (fmt) {
    ...
    case IMAGE_FORMAT_BGRA8888:
      if (TestTextureFormat(D3DFMT_A8R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_A8R8G8B8;
      if (TestTextureFormat(D3DFMT_A4R4G4B4, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_BGRX8888:
      ...
      if (TestTextureFormat(D3DFMT_X8R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_BGR888:
      if (TestTextureFormat(D3DFMT_R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
      ...
      if (TestTextureFormat(D3DFMT_A8R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_A8R8G8B8;
      if (TestTextureFormat(D3DFMT_R5G6B5, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_I8:
      if (TestTextureFormat(D3DFMT_L8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_L8;
      if (TestTextureFormat(D3DFMT_A8R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_IA88:
      if (TestTextureFormat(D3DFMT_A8L8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_A8L8;
      if (TestTextureFormat(D3DFMT_A8R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_A8:
      if (TestTextureFormat(D3DFMT_A8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_A8;
      if (TestTextureFormat(D3DFMT_A8R8G8B8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_DXT1_ONEBITALPHA:
      if (TestTextureFormat(D3DFMT_DXT1, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_DXT1;
      break;

    case IMAGE_FORMAT_DXT3:
      if (TestTextureFormat(D3DFMT_DXT3, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_DXT3;
      break;

    case IMAGE_FORMAT_DXT5:
      if (TestTextureFormat(D3DFMT_DXT5, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_DXT5;
      break;

    case IMAGE_FORMAT_UV88:
      if (TestTextureFormat(D3DFMT_V8U8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_V8U8;
      break;

    case IMAGE_FORMAT_UVWQ8888:
      if (TestTextureFormat(D3DFMT_Q8W8V8U8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_Q8W8V8U8;
      break;

    case IMAGE_FORMAT_UVLX8888:
      if (TestTextureFormat(D3DFMT_X8L8V8U8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_RGBA16161616F:
      if (TestTextureFormat(D3DFMT_A16B16G16R16F, isRenderTarget,
                            bIsVertexTexture, bIsFilterableRequired))
    ...
    case IMAGE_FORMAT_R32F:
      if (TestTextureFormat(D3DFMT_R32F, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_R32F;
      break;

    case IMAGE_FORMAT_RGBA32323232F:
      if (TestTextureFormat(D3DFMT_A32B32G32R32F, isRenderTarget,
                            bIsVertexTexture, bIsFilterableRequired))
        return D3DFMT_A32B32G32R32F;
      break;
    // nVidia overloads DST formats as texture formats
    case IMAGE_FORMAT_NV_DST16:
      if (TestTextureFormat(D3DFMT_D16, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_D16;
      break;

    case IMAGE_FORMAT_NV_DST24:
      if (TestTextureFormat(D3DFMT_D24S8, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return D3DFMT_D24S8;
      break;
    case IMAGE_FORMAT_NV_INTZ:
      if (TestTextureFormat(NVFMT_INTZ, isRenderTarget, bIsVertexTexture,
                            false))
        return NVFMT_INTZ;
      break;

    case IMAGE_FORMAT_NV_RAWZ:
      if (TestTextureFormat(NVFMT_RAWZ, isRenderTarget, bIsVertexTexture,
                            false))
        return NVFMT_RAWZ;
      break;

    case IMAGE_FORMAT_NV_NULL:
      if (TestTextureFormat(NVFMT_NULL, isRenderTarget, bIsVertexTexture,
                            false))
        return NVFMT_NULL;
      break;

    case IMAGE_FORMAT_ATI_DST16:
      if (TestTextureFormat(ATIFMT_D16, isRenderTarget, bIsVertexTexture,
                            false))
        return ATIFMT_D16;
      break;
    ...
    case IMAGE_FORMAT_ATI2N:
      if (TestTextureFormat(ATIFMT_ATI2N, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return ATIFMT_ATI2N;
      break;

    case IMAGE_FORMAT_ATI1N:
      if (TestTextureFormat(ATIFMT_ATI1N, isRenderTarget, bIsVertexTexture,
                            bIsFilterableRequired))
        return ATIFMT_ATI1N;
      break;
  }
  ...
  while (fmt < NUM_IMAGE_FORMATS) {
    for (int nVertexTexture = 0; nVertexTexture <= 1; ++nVertexTexture) {
      for (int nRenderTarget = 0; nRenderTarget <= 1; ++nRenderTarget) {
        g_D3DColorFormat[fmt][nVertexTexture][nRenderTarget][0] =
            GetNearestD3DColorFormat((ImageFormat)fmt, nRenderTarget != 0,
                                     nVertexTexture != 0, false);

        g_D3DColorFormat[fmt][nVertexTexture][nRenderTarget][1] =
            GetNearestD3DColorFormat((ImageFormat)fmt, nRenderTarget != 0,
                                     nVertexTexture != 0, true);
      }
    }
    ++fmt;
  }
...
ImageFormat FindNearestSupportedFormat(ImageFormat format,
  ...
  return ImageLoader::D3DFormatToImageFormat(
      g_D3DColorFormat[format][bIsVertexTexture][bIsRenderTarget]
                      [bFilterableRequired]);
}
...
//-----------------------------------------------------------------------------
static inline bool IsDepthFormatCompatible(int nAdapter,
                                           ImageFormat displayFormat,
                                           ImageFormat renderTargetFormat,
                                           D3DFORMAT depthFormat) {
  D3DFORMAT d3dDisplayFormat =
      ImageLoader::ImageFormatToD3DFormat(displayFormat);
  D3DFORMAT d3dRenderTargetFormat =
      ImageLoader::ImageFormatToD3DFormat(renderTargetFormat);

  // Verify that the depth format is compatible.
  HRESULT hr{D3D()->CheckDepthStencilMatch(nAdapter, SOURCE_DX9_DEVICE_TYPE,
                                           d3dDisplayFormat,
...
D3DFORMAT FindNearestSupportedDepthFormat(u32 nAdapter,
  ...
  Assert(displayFormat != IMAGE_FORMAT_UNKNOWN &&
...
//-----------------------------------------------------------------------------
static inline bool IsFrameBufferFormatValid(UINT displayAdapter,
...
ImageFormat FindNearestSupportedBackBufferFormat(UINT displayAdapter,
  ...
  switch (backBufferFormat) {
    ...
    case IMAGE_FORMAT_ARGB8888:
    case IMAGE_FORMAT_BGRA8888:
    case IMAGE_FORMAT_BGRA4444:  // This is not supported ever; bump up to 32
                                 // bit
      if (IsFrameBufferFormatValid(displayAdapter, deviceType, d3dDisplayFormat,
                                   D3DFMT_A8R8G8B8, bIsWindowed))
        return IMAGE_FORMAT_BGRA8888;

      // Bye, bye dest alpha
      if (IsFrameBufferFormatValid(displayAdapter, deviceType, d3dDisplayFormat,
                                   D3DFMT_X8R8G8B8, bIsWindowed))
        return IMAGE_FORMAT_BGRX8888;

      if (IsFrameBufferFormatValid(displayAdapter, deviceType, d3dDisplayFormat,
                                   D3DFMT_R5G6B5, bIsWindowed))
        return IMAGE_FORMAT_BGR565;

      if (IsFrameBufferFormatValid(displayAdapter, deviceType, d3dDisplayFormat,
                                   D3DFMT_A1R5G5B5, bIsWindowed))
        return IMAGE_FORMAT_BGRA5551;

      if (IsFrameBufferFormatValid(displayAdapter, deviceType, d3dDisplayFormat,
                                   D3DFMT_X1R5G5B5, bIsWindowed))
