...
#ifndef MATERIALSYSTEM_SHADERAPIDX9_STUBD3DDEVICE_H_
#define MATERIALSYSTEM_SHADERAPIDX9_STUBD3DDEVICE_H_
...
class CStubD3DTexture : public IDirect3DTexture8 {
 ...
 public:
  ...
  HRESULT SOURCE_STDCALL SetPrivateData(REFGUID refguid, CONST void* pData,
  ...
  HRESULT SOURCE_STDCALL GetPrivateData(REFGUID refguid, void* pData,
                                        DWORD* pSizeOfData) {
  ...
  HRESULT SOURCE_STDCALL GetSurfaceLevel(UINT Level,
  ...
  HRESULT SOURCE_STDCALL LockRect(UINT Level, D3DLOCKED_RECT* pLockedRect,
...
class CStubD3DDevice : public IDirect3DDevice8 {
 ...
 public:
  ...
  HRESULT SOURCE_STDCALL SetCursorProperties(UINT XHotSpot, UINT YHotSpot,
  ...
  void SOURCE_STDCALL SetCursorPosition(UINT XScreenSpace, UINT YScreenSpace,
                                        DWORD Flags) {
  ...
  CreateAdditionalSwapChain(D3DPRESENT_PARAMETERS* pPresentationParameters,
    ...
    return m_pD3DDevice->CreateAdditionalSwapChain(pPresentationParameters,
                                                   pSwapChain);
  }
  ...
  HRESULT SOURCE_STDCALL Present(CONST RECT* pSourceRect, CONST RECT* pDestRect,
                                 HWND hDestWindowOverride,
                                 CONST RGNDATA* pDirtyRegion) {
    FPRINTF(s_FileHandle, "Present\n");
    return m_pD3DDevice->Present(pSourceRect, pDestRect, hDestWindowOverride,
                                 pDirtyRegion);
  }

  HRESULT SOURCE_STDCALL GetBackBuffer(UINT BackBuffer, D3DBACKBUFFER_TYPE Type,
  ...
  HRESULT SOURCE_STDCALL CreateTexture(UINT Width, UINT Height, UINT Levels,
 ...
 *ppTexture = new CStubD3DTexture( *ppTexture, this );
 ...
 return ret;
  ...
  CreateVolumeTexture(UINT Width, UINT Height, UINT Depth, UINT Levels,
    ...
    return m_pD3DDevice->CreateVolumeTexture(
        Width, Height, Depth, Levels, Usage, Format, Pool, ppVolumeTexture);
  }

  HRESULT SOURCE_STDCALL
  CreateCubeTexture(UINT EdgeLength, UINT Levels, DWORD Usage, D3DFORMAT Format,
                    D3DPOOL Pool, IDirect3DCubeTexture8** ppCubeTexture) {
    FPRINTF(s_FileHandle, "CreateCubeTexture\n");
    return m_pD3DDevice->CreateCubeTexture(EdgeLength, Levels, Usage, Format,
                                           Pool, ppCubeTexture);
  }
  ...
  CreateIndexBuffer(UINT Length, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool,
    ...
    return m_pD3DDevice->CreateIndexBuffer(Length, Usage, Format, Pool,
                                           ppIndexBuffer);
  }

  HRESULT SOURCE_STDCALL CreateRenderTarget(UINT Width, UINT Height,
                                            D3DFORMAT Format,
  ...
  HRESULT SOURCE_STDCALL CreateDepthStencilSurface(
    ...
    return m_pD3DDevice->CreateDepthStencilSurface(Width, Height, Format,
                                                   MultiSample, ppSurface);
  }

  HRESULT SOURCE_STDCALL CreateImageSurface(UINT Width, UINT Height,
                                            D3DFORMAT Format,
                                            IDirect3DSurface8** ppSurface) {
  ...
  HRESULT SOURCE_STDCALL CopyRects(IDirect3DSurface8* pSourceSurface,
                                   CONST RECT* pSourceRectsArray, UINT cRects,
                                   IDirect3DSurface8* pDestinationSurface,
                                   CONST POINT* pDestPointsArray) {
    FPRINTF(s_FileHandle, "CopyRects\n");
    return m_pD3DDevice->CopyRects(pSourceSurface, pSourceRectsArray, cRects,
  ...
  HRESULT SOURCE_STDCALL SetRenderTarget(IDirect3DSurface8* pRenderTarget,
  ...
  HRESULT SOURCE_STDCALL
  GetDepthStencilSurface(IDirect3DSurface8** ppZStencilSurface) {
  ...
  HRESULT SOURCE_STDCALL Clear(DWORD Count, CONST D3DRECT* pRects, DWORD Flags,
                               D3DCOLOR Color, float Z, DWORD Stencil) {
    FPRINTF(s_FileHandle, "Clear\n");
    return m_pD3DDevice->Clear(Count, pRects, Flags, Color, Z, Stencil);
  }

  HRESULT SOURCE_STDCALL SetTransform(D3DTRANSFORMSTATETYPE State,
                                      CONST D3DMATRIX* pMatrix) {
    FPRINTF(s_FileHandle, "SetTransform\n");
    return m_pD3DDevice->SetTransform(State, pMatrix);
  }

  HRESULT SOURCE_STDCALL GetTransform(D3DTRANSFORMSTATETYPE State,
                                      D3DMATRIX* pMatrix) {
    FPRINTF(s_FileHandle, "GetTransform\n");
    return m_pD3DDevice->GetTransform(State, pMatrix);
  }

  HRESULT SOURCE_STDCALL MultiplyTransform(D3DTRANSFORMSTATETYPE transformState,
                                           CONST D3DMATRIX* pMatrix) {
  ...
  HRESULT SOURCE_STDCALL GetRenderState(D3DRENDERSTATETYPE State,
                                        DWORD* pValue) {
  ...
  HRESULT SOURCE_STDCALL CreateStateBlock(D3DSTATEBLOCKTYPE Type,
                                          DWORD* pToken) {
  ...
  HRESULT SOURCE_STDCALL SetTextureStageState(DWORD Stage,
                                              D3DTEXTURESTAGESTATETYPE Type,
                                              DWORD Value) {
  ...
  HRESULT SOURCE_STDCALL GetInfo(DWORD DevInfoID, void* pDevInfoStruct,
  ...
  HRESULT SOURCE_STDCALL SetPaletteEntries(UINT PaletteNumber,
                                           CONST PALETTEENTRY* pEntries) {
    FPRINTF(s_FileHandle, "SetPaletteEntries\n");
    return m_pD3DDevice->SetPaletteEntries(PaletteNumber, pEntries);
  }

  HRESULT SOURCE_STDCALL GetPaletteEntries(UINT PaletteNumber,
  ...
  HRESULT SOURCE_STDCALL DrawPrimitive(D3DPRIMITIVETYPE PrimitiveType,
                                       UINT StartVertex, UINT PrimitiveCount) {
    FPRINTF(s_FileHandle, "DrawPrimitive\n");
    return m_pD3DDevice->DrawPrimitive(PrimitiveType, StartVertex,
                                       PrimitiveCount);
  }

  HRESULT SOURCE_STDCALL DrawIndexedPrimitive(D3DPRIMITIVETYPE primitiveType,
  ...
  HRESULT SOURCE_STDCALL DrawPrimitiveUP(D3DPRIMITIVETYPE PrimitiveType,
                                         UINT PrimitiveCount,
  ...
  HRESULT SOURCE_STDCALL DrawIndexedPrimitiveUP(
      D3DPRIMITIVETYPE PrimitiveType, UINT MinVertexIndex,
      UINT NumVertexIndices, UINT PrimitiveCount, CONST void* pIndexData,
      D3DFORMAT IndexDataFormat, CONST void* pVertexStreamZeroData,
      UINT VertexStreamZeroStride) {
    FPRINTF(s_FileHandle, "DrawIndexedPrimitiveUP\n");
    return m_pD3DDevice->DrawIndexedPrimitiveUP(
        PrimitiveType, MinVertexIndex, NumVertexIndices, PrimitiveCount,
        pIndexData, IndexDataFormat, pVertexStreamZeroData,
        VertexStreamZeroStride);
  }

  HRESULT SOURCE_STDCALL ProcessVertices(UINT SrcStartIndex, UINT DestIndex,
                                         UINT VertexCount,
                                         IDirect3DVertexBuffer8* pDestBuffer,
                                         DWORD Flags) {
    FPRINTF(s_FileHandle, "ProcessVertices\n");
    return m_pD3DDevice->ProcessVertices(SrcStartIndex, DestIndex, VertexCount,
                                         pDestBuffer, Flags);
  }

  HRESULT SOURCE_STDCALL CreateVertexShader(CONST DWORD* pDeclaration,
                                            CONST DWORD* pFunction,
                                            DWORD* pHandle, DWORD Usage) {
    FPRINTF(s_FileHandle, "CreateVertexShader\n");
    return m_pD3DDevice->CreateVertexShader(pDeclaration, pFunction, pHandle,
                                            Usage);
  }
  ...
  HRESULT SOURCE_STDCALL SetVertexShaderConstant(DWORD Register,
                                                 CONST void* pConstantData,
                                                 DWORD ConstantCount) {
    FPRINTF(s_FileHandle, "SetVertexShaderConstant\n");
    return m_pD3DDevice->SetVertexShaderConstant(Register, pConstantData,
                                                 ConstantCount);
  }

  HRESULT SOURCE_STDCALL GetVertexShaderConstant(DWORD Register,
                                                 void* pConstantData,
                                                 DWORD ConstantCount) {
    FPRINTF(s_FileHandle, "GetVertexShaderConstant\n");
    return m_pD3DDevice->GetVertexShaderConstant(Register, pConstantData,
                                                 ConstantCount);
  }

  HRESULT SOURCE_STDCALL GetVertexShaderDeclaration(DWORD Handle, void* pData,
                                                    DWORD* pSizeOfData) {
    FPRINTF(s_FileHandle, "GetVertexShaderDeclaration\n");
    return m_pD3DDevice->GetVertexShaderDeclaration(Handle, pData, pSizeOfData);
  }

  HRESULT SOURCE_STDCALL GetVertexShaderFunction(DWORD Handle, void* pData,
                                                 DWORD* pSizeOfData) {
    FPRINTF(s_FileHandle, "GetVertexShaderFunction\n");
    return m_pD3DDevice->GetVertexShaderFunction(Handle, pData, pSizeOfData);
  }

  HRESULT SOURCE_STDCALL SetStreamSource(UINT StreamNumber,
  ...
  HRESULT SOURCE_STDCALL GetStreamSource(UINT StreamNumber,
  ...
  HRESULT SOURCE_STDCALL SetIndices(IDirect3DIndexBuffer8* pIndexData,
                                    UINT BaseVertexIndex) {
    FPRINTF(s_FileHandle, "SetIndices\n");
    return m_pD3DDevice->SetIndices(pIndexData, BaseVertexIndex);
  }

  HRESULT SOURCE_STDCALL GetIndices(IDirect3DIndexBuffer8** ppIndexData,
                                    UINT* pBaseVertexIndex) {
    FPRINTF(s_FileHandle, "GetIndices\n");
    return m_pD3DDevice->GetIndices(ppIndexData, pBaseVertexIndex);
  }

  HRESULT SOURCE_STDCALL CreatePixelShader(CONST DWORD* pFunction,
                                           DWORD* pHandle) {
  ...
  HRESULT SOURCE_STDCALL SetPixelShaderConstant(DWORD Register,
                                                CONST void* pConstantData,
                                                DWORD ConstantCount) {
    FPRINTF(s_FileHandle, "SetPixelShaderConstant\n");
    return m_pD3DDevice->SetPixelShaderConstant(Register, pConstantData,
                                                ConstantCount);
  }

  HRESULT SOURCE_STDCALL GetPixelShaderConstant(DWORD Register,
                                                void* pConstantData,
                                                DWORD ConstantCount) {
    FPRINTF(s_FileHandle, "GetPixelShaderConstant\n");
    return m_pD3DDevice->GetPixelShaderConstant(Register, pConstantData,
                                                ConstantCount);
  }

  HRESULT SOURCE_STDCALL GetPixelShaderFunction(DWORD Handle, void* pData,
                                                DWORD* pSizeOfData) {
  ...
  HRESULT SOURCE_STDCALL DrawTriPatch(UINT Handle, CONST float* pNumSegs,
...
#endif  // MATERIALSYSTEM_SHADERAPIDX9_STUBD3DDEVICE_H_
