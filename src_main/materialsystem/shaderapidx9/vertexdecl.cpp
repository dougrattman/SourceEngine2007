...
static D3DDECLTYPE VertexElementToDeclType(
    VertexElement_t element, VertexCompressionType_t compressionType) {
  ...
  switch (element) {
    ...
    case VERTEX_ELEMENT_WRINKLE:
      // Wrinkle is packed into Position.W, it is not specified as a separate
      // vertex element
      Assert(0);
...
void PrintVertexDeclaration(const D3DVERTEXELEMENT9 *pDecl) {
  ...
  for (i = 0;; i++) {
    ...
    Msg("%d: Stream: %d, Offset: %d, Type: %s, Method: %s, Usage: %s, "
        "UsageIndex: %d\n",
        i, (int)pDecl[i].Stream, (int)pDecl[i].Offset,
        DeclTypeToString(pDecl[i].Type), DeclMethodToString(pDecl[i].Method),
        DeclUsageToString(pDecl[i].Usage), (int)pDecl[i].UsageIndex);
  }
}
...
void ComputeVertexSpec(VertexFormat_t fmt, D3DVERTEXELEMENT9 *pDecl,
  ...
  if (fmt & VERTEX_POSITION) {
    ...
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_POSITION, compressionType);
  ...
  if (numBones > 0) {
    ...
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_BONEWEIGHTS2, compressionType);
    offset +=
        GetVertexElementSize(VERTEX_ELEMENT_BONEWEIGHTS2, compressionType);
    ++i;
  }
  ...
  if (fmt & VERTEX_BONE_INDEX) {
    ...
    pDecl[i].UsageIndex = 0;
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_BONEINDEX, compressionType);
  ...
  if (fmt & VERTEX_NORMAL) {
    ...
    pDecl[i].UsageIndex = 0;
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_NORMAL, compressionType);
  ...
  if (fmt & VERTEX_COLOR) {
    ...
    pDecl[i].UsageIndex = 0;
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_COLOR, compressionType);
  ...
  if (fmt & VERTEX_SPECULAR) {
    ...
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_SPECULAR, compressionType);
    offset += GetVertexElementSize(VERTEX_ELEMENT_SPECULAR, compressionType);
    ++i;
  }

  VertexElement_t texCoordDimensions[4] = {
  ...
  for (int j = 0; j < VERTEX_MAX_TEXTURE_COORDINATES; ++j) {
    ...
    VertexElement_t texCoordElement =
        (VertexElement_t)(texCoordDimensions[nCoordSize - 1] + j);
  ...
  if (fmt & VERTEX_TANGENT_S) {
    ...
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_TANGENT_S, compressionType);
  ...
  if (fmt & VERTEX_TANGENT_T) {
    ...
    // NOTE: this is currently *not* compressed
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_TANGENT_T, compressionType);
  ...
  if (userDataSize > 0) {
    ...
    pDecl[i].Usage = D3DDECLUSAGE_TANGENT;
    pDecl[i].UsageIndex = 0;
    VertexElement_t userDataElement =
        (VertexElement_t)(VERTEX_ELEMENT_USERDATA1 + (userDataSize - 1));
  ...
  if (bStaticLit) {
    // force stream 1 to have specular color in it, which is used for baked
    // static lighting
    pDecl[i].Stream = 1;
    pDecl[i].Offset = 0;
    pDecl[i].Method = D3DDECLMETHOD_DEFAULT;
  ...
  if (HardwareConfig()->SupportsVertexAndPixelShaders()) {
    ...
    if (bUseWrinkle) {
      normalOffset +=
          GetVertexElementSize(VERTEX_ELEMENT_WRINKLE, compressionType);
    }
    ...
    // NOTE: this is currently *not* compressed
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_NORMAL, VERTEX_COMPRESSION_NONE);
    ++i;
  }

  if (bUsingMorph) {
    // force stream 3 to have vertex index in it, which is used for doing vertex
    // texture reads
    pDecl[i].Stream = 3;
    pDecl[i].Offset = 0;
    pDecl[i].Method = D3DDECLMETHOD_DEFAULT;
    ...
    pDecl[i].Type =
        VertexElementToDeclType(VERTEX_ELEMENT_USERDATA1, compressionType);
    ++i;
  }
...
static CUtlRBTree<VertexDeclLookup_t, int> s_VertexDeclDict(0, 256,
                                                            VertexDeclLessFunc);
...
IDirect3DVertexDeclaration9 *FindOrCreateVertexDecl(VertexFormat_t fmt,
  ...
  HRESULT hr = Dx9Device()->CreateVertexDeclaration(decl, &lookup.m_pDecl);
  ...
  if (hr != D3D_OK) {
    Warning(
        " ERROR: failed to create vertex decl for vertex format %x! You'll "
        "probably see messed-up mesh rendering - to diagnose, build "
        "shaderapidx9.dll in debug.\n",
        (int)fmt);
  }
