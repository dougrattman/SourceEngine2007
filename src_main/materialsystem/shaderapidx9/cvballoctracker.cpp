...
class CounterData {
 public:
  ...
  int m_vertCount;
  int m_paddingCount;
  int m_elementsCompressed[VERTEX_ELEMENT_NUMELEMENTS];  // Number of compressed
                                                         // verts using each
                                                         // element
  int m_elementsUncompressed[VERTEX_ELEMENT_NUMELEMENTS];  // Number of
                                                           // uncompressed verts
                                                           // using each element
...
class AllocData {
 public:
  AllocData(void *buffer, int bufferSize, VertexFormat_t fmt, int numVerts,
            int allocatorHash)
      : m_buffer(buffer),
        m_bufferSize(bufferSize),
        m_fmt(fmt),
        m_numVerts(numVerts),
        m_allocatorHash(allocatorHash) {}
  AllocData()
      : m_buffer(NULL),
        m_bufferSize(0),
        m_fmt(0),
...
class CVBAllocTracker : public IVBAllocTracker {
 ...
 private:
  UtlHashFixedHandle_t TrackAlloc(void *buffer, int bufferSize,
                                  VertexFormat_t fmt, int numVerts,
                                  short allocatorHash);
  bool KillAlloc(void *buffer, int &bufferSize, VertexFormat_t &fmt,
                 int &numVerts, short &allocatorHash);

  UtlHashFixedHandle_t GetCounterHandle(const char *allocatorName,
                                        short allocatorHash);

  void SpewElements(const char *allocatorName, short nameHash);
  int ComputeVertexSize(VertexElementMap_t map, VertexFormat_t fmt,
                        bool compressed);
  VertexElementMap_t ComputeElementMap(VertexFormat_t fmt, int vertexSize,
                                       bool isDynamic);
  void UpdateElements(CounterData &data, VertexFormat_t fmt, int numVerts,
                      int vertexSize, bool isDynamic, bool isCompressed);

  int ComputeAlignmentWastage(int bufferSize);
  void AddSaving(int &alreadySaved, int &yetToSave, const char *allocatorName,
                 VertexElement_t element, Saving_t savingType);
  void SpewExpectedSavings(void);
  void UpdateData(const char *allocatorName, short allocatorKey, int bufferSize,
                  VertexFormat_t fmt, int numVerts, int vertexSize,
  ...
  // We use a mutex since allocation tracking is accessed from multiple loading
  // threads. CThreadFastMutex is used as contention is expected to be low
  // during loading.
...
static const ElementData specularElement = {
    VERTEX_ELEMENT_SPECULAR, 4, 4, 4, "SPECULAR    "};  // (already minimal)
static const ElementData tangentSElement = {
    VERTEX_ELEMENT_TANGENT_S, 12, 12, 4, "TANGENT_S   "};  // (all-but-unused)
static const ElementData tangentTElement = {
    VERTEX_ELEMENT_TANGENT_T, 12, 12, 4, "TANGENT_T   "};  // (all-but-unused)
static const ElementData wrinkleElement = {VERTEX_ELEMENT_WRINKLE, 4, 4, 0,
                                           "WRINKLE     "};  // (UNDONE:
                                                             // compress it as a
                                                             // SHORTN in
                                                             // Position.w - is
                                                             // it [0,1]?)
static const ElementData boneIndexElement = {
    VERTEX_ELEMENT_BONEINDEX, 4, 4, 4, "BONEINDEX   "};  // (already minimal)
static const ElementData boneWeight1Element = {
    VERTEX_ELEMENT_BONEWEIGHTS1, 4, 4, 4, "BONEWEIGHT1 "};  // (unused)
static const ElementData boneWeight2Element = {
    VERTEX_ELEMENT_BONEWEIGHTS2, 8, 8, 4,
    "BONEWEIGHT2 "};  // (UNDONE: take care w.r.t cracking in flex regions)
static const ElementData boneWeight3Element = {
    VERTEX_ELEMENT_BONEWEIGHTS3, 12, 12, 8, "BONEWEIGHT3 "};  // (unused)
static const ElementData boneWeight4Element = {
    VERTEX_ELEMENT_BONEWEIGHTS4, 16, 16, 8, "BONEWEIGHT4 "};  // (unused)
static const ElementData userData1Element = {VERTEX_ELEMENT_USERDATA1, 4, 4, 4,
                                             "USERDATA1   "};  // (unused)
static const ElementData userData2Element = {VERTEX_ELEMENT_USERDATA2, 8, 8, 4,
                                             "USERDATA2   "};  // (unused)
...
static const ElementData texCoord1D0Element = {
    ...
    "TEXCOORD1D_0"};  // (not worth compressing)
static const ElementData texCoord1D1Element = {
    VERTEX_ELEMENT_TEXCOORD1D_1, 4, 4, 4,
    "TEXCOORD1D_1"};  // (not worth compressing)
static const ElementData texCoord1D2Element = {
    VERTEX_ELEMENT_TEXCOORD1D_2, 4, 4, 4,
    "TEXCOORD1D_2"};  // (not worth compressing)
static const ElementData texCoord1D3Element = {
    VERTEX_ELEMENT_TEXCOORD1D_3, 4, 4, 4,
    "TEXCOORD1D_3"};  // (not worth compressing)
static const ElementData texCoord1D4Element = {
    VERTEX_ELEMENT_TEXCOORD1D_4, 4, 4, 4,
    "TEXCOORD1D_4"};  // (not worth compressing)
static const ElementData texCoord1D5Element = {
    VERTEX_ELEMENT_TEXCOORD1D_5, 4, 4, 4,
    "TEXCOORD1D_5"};  // (not worth compressing)
static const ElementData texCoord1D6Element = {
    VERTEX_ELEMENT_TEXCOORD1D_6, 4, 4, 4,
    "TEXCOORD1D_6"};  // (not worth compressing)
...
static const ElementData texCoord2D3Element = {
    VERTEX_ELEMENT_TEXCOORD2D_3, 8, 8, 4, "TEXCOORD2D_3"};  // (unused)
static const ElementData texCoord2D4Element = {
    VERTEX_ELEMENT_TEXCOORD2D_4, 8, 8, 4, "TEXCOORD2D_4"};  // (unused)
static const ElementData texCoord2D5Element = {
    VERTEX_ELEMENT_TEXCOORD2D_5, 8, 8, 4, "TEXCOORD2D_5"};  // (unused)
static const ElementData texCoord2D6Element = {
...
static const ElementData elementTable[VERTEX_ELEMENT_NUMELEMENTS] = {
    ...
    wrinkleElement,     boneIndexElement,   boneWeight1Element,
    boneWeight2Element, boneWeight3Element, boneWeight4Element,
    userData1Element,   userData2Element,   userData3Element,
    userData4Element,   texCoord1D0Element, texCoord1D1Element,
    texCoord1D2Element, texCoord1D3Element, texCoord1D4Element,
    texCoord1D5Element, texCoord1D6Element, texCoord1D7Element,
    texCoord2D0Element, texCoord2D1Element, texCoord2D2Element,
    texCoord2D3Element, texCoord2D4Element, texCoord2D5Element,
    texCoord2D6Element, texCoord2D7Element, texCoord3D0Element,
    texCoord3D1Element, texCoord3D2Element, texCoord3D3Element,
    texCoord3D4Element, texCoord3D5Element, texCoord3D6Element,
    texCoord3D7Element, texCoord4D0Element, texCoord4D1Element,
    texCoord4D2Element, texCoord4D3Element, texCoord4D4Element,
    texCoord4D5Element, texCoord4D6Element, texCoord4D7Element,
};

static ConVar mem_vballocspew("mem_vballocspew", "0", FCVAR_CHEAT,
                              "How often to spew vertex buffer allocation "
                              "stats - 1: every alloc, 2+: every 2+ allocs, 0: "
                              "off");
...
UtlHashFixedHandle_t CVBAllocTracker::TrackAlloc(void *buffer, int bufferSize,
                                                 VertexFormat_t fmt,
                                                 int numVerts,
                                                 short allocatorHash) {
...
bool CVBAllocTracker::KillAlloc(void *buffer, int &bufferSize,
                                VertexFormat_t &fmt, int &numVerts,
                                short &allocatorHash) {
...
UtlHashFixedHandle_t CVBAllocTracker::GetCounterHandle(
    const char *allocatorName, short allocatorHash) {
  ...
  if (handle == m_VBCountTable.InvalidHandle()) {
    ...
    V_strncpy(newData.m_AllocatorName, allocatorName,
              CounterData::MAX_NAME_SIZE);
...
void CheckForElementTableUpdates(const ElementData &element) {
  ...
  if (tableIndex != element.element) {
    ...
    if (timesToSpew > 0) {
      Warning(
          "VertexElement_t structure has changed, ElementData table in "
...
void CVBAllocTracker::SpewElements(const char *allocatorName, short nameHash) {
  ...
  if (handle != m_VBCountTable.InvalidHandle()) {
    ...
    for (int i = 0; i < VERTEX_ELEMENT_NUMELEMENTS; i++) {
      ...
      currentSum += numCompressed * elementTable[i].currentCompressed +
                    numUncompressed * elementTable[i].uncompressed;
    ...
    if (originalSum > 0) {
      ...
      for (int i = 0; i < VERTEX_ELEMENT_NUMELEMENTS; i++) {
        ...
        if (numVerts > 0) {
          Msg("              element:  %5.2f MB 'U', %5.2f MB 'C', %5.2f MB "
              "'I', %6.2f MB 'D', %s\n",
              numVerts * elementData.uncompressed / (1024.0f * 1024.0f),
              (numCompressed * elementData.currentCompressed +
               numUncompressed * elementData.uncompressed) /
                  (1024.0f * 1024.0f),
              numVerts * elementData.idealCompressed / (1024.0f * 1024.0f),
              -(numCompressed * elementData.currentCompressed +
                numUncompressed * elementData.uncompressed -
                numVerts * elementData.idealCompressed) /
                  (1024.0f * 1024.0f),
              elementData.name);
        }
      }
      Msg("[VBMEM]         total:  %5.2f MB 'U', %5.2f MB 'C', %5.2f MB 'I', "
          "%6.2f MB 'D'\n",
          originalSum / (1024.0f * 1024.0f), currentSum / (1024.0f * 1024.0f),
...
int CVBAllocTracker::ComputeVertexSize(VertexElementMap_t map,
                                       VertexFormat_t fmt, bool compressed) {
  int vertexSize = 0;
  for (int i = 0; i < VERTEX_ELEMENT_NUMELEMENTS; i++) {
    ...
    if (map & (LSB << i)) {
      vertexSize +=
          compressed ? element.currentCompressed : element.uncompressed;
    }
  }
...
VertexElementMap_t CVBAllocTracker::ComputeElementMap(VertexFormat_t fmt,
                                                      int vertexSize,
                                                      bool isDynamic) {
  ...
  if (userDataSize > 0)
    map |= LSB << (VERTEX_ELEMENT_USERDATA1 + userDataSize - 1);
  ...
  if (map == 0) {
    ...
    if (vertexSize != 0) {
      ...
      // size
      // TODO(d.rattman): make this assert dependent on whether the current VB
      ...
      // TODO(d.rattman): once we've finalised which elements we're compressing
      // for ship, update
...
void CVBAllocTracker::UpdateElements(CounterData &data, VertexFormat_t fmt,
                                     int numVerts, int vertexSize,
                                     bool isDynamic, bool isCompressed) {
...
void CVBAllocTracker::AddSaving(int &alreadySaved, int &yetToSave,
                                const char *allocatorName,
                                VertexElement_t element, Saving_t savingType) {
  UtlHashFixedHandle_t handle =
      GetCounterHandle(allocatorName, HashString(allocatorName));
  if (handle != m_VBCountTable.InvalidHandle()) {
    ...
    switch (savingType) {
      case SAVING_COMPRESSION:
        alreadySaved += numCompressed * (elementData.uncompressed -
                                         elementData.currentCompressed);
        yetToSave += numUncompressed *
                     (elementData.uncompressed - elementData.currentCompressed);
        break;
      case SAVING_REMOVAL:
        alreadySaved += elementData.uncompressed *
                        (numVerts - (numUncompressed + numCompressed));
        yetToSave += numUncompressed * elementData.uncompressed +
                     numCompressed * elementData.uncompressed;
        break;
...
void CVBAllocTracker::SpewExpectedSavings() {
  ...
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (prop_static)",
            VERTEX_ELEMENT_BONEWEIGHTS2, SAVING_REMOVAL);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (prop_static)",
            VERTEX_ELEMENT_BONEINDEX, SAVING_REMOVAL);
  // We have removed vertex colors from all models (color should only ever be in
  // stream1, for static vertex lighting)
  AddSaving(alreadySaved, yetToSave,
            "R_StudioCreateStaticMeshes (prop_dynamic)", VERTEX_ELEMENT_COLOR,
            SAVING_REMOVAL);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (prop_static)",
            VERTEX_ELEMENT_COLOR, SAVING_REMOVAL);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (character)",
            VERTEX_ELEMENT_COLOR, SAVING_REMOVAL);

  // We expect to compress texcoords (DONE: normals+tangents, boneweights) for
  // all studiomdls
  AddSaving(alreadySaved, yetToSave,
            "R_StudioCreateStaticMeshes (prop_dynamic)", VERTEX_ELEMENT_NORMAL,
            SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (prop_static)",
            VERTEX_ELEMENT_NORMAL, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (character)",
            VERTEX_ELEMENT_NORMAL, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave,
            "R_StudioCreateStaticMeshes (prop_dynamic)",
            VERTEX_ELEMENT_USERDATA4, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (prop_static)",
            VERTEX_ELEMENT_USERDATA4, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (character)",
            VERTEX_ELEMENT_USERDATA4, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave,
            "R_StudioCreateStaticMeshes (prop_dynamic)",
            VERTEX_ELEMENT_TEXCOORD2D_0, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (prop_static)",
            VERTEX_ELEMENT_TEXCOORD2D_0, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (character)",
            VERTEX_ELEMENT_TEXCOORD2D_0, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (character)",
            VERTEX_ELEMENT_BONEWEIGHTS1, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave, "R_StudioCreateStaticMeshes (character)",
            VERTEX_ELEMENT_BONEWEIGHTS2, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave,
            "R_StudioCreateStaticMeshes (prop_dynamic)",
            VERTEX_ELEMENT_BONEWEIGHTS1, SAVING_COMPRESSION);
  AddSaving(alreadySaved, yetToSave,
  ...
  // UNDONE: compress bone weights for studiomdls?			(issue:
  // possible flex artifacts, but 2xSHORTN probably ok) UNDONE: compress
  // positions
  // (+wrinkle) for studiomdls?	(issue: possible flex artifacts) UNDONE: disable
  // tangents for non-bumped models			(issue:
  // forcedmaterialoverride support... don't think that needs tangents, though
  //                                                                  however,
  //                                                                  if we use
  //                                                                  UBYTE4
  //                                                                  normal+tangent
  ...
  Msg("[VBMEM]\n");
  Msg("[VBMEM] Total expected memory saving by disabling/compressing vertex "
      "elements: %6.2f MB\n",
      yetToSave / (1024.0f * 1024.0f));
  Msg("[VBMEM] ( total memory already saved: %6.2f MB)\n",
      alreadySaved / (1024.0f * 1024.0f));
  Msg("[VBMEM]  - compression of model texcoords, [DONE: normals+tangents, "
      "bone weights]\n");
  Msg("[VBMEM]  - avoidance of 4-KB alignment wastage for color meshes (on "
      "360)\n");
  Msg("[VBMEM]  - [DONE: removal of unneeded bone weights+indices on "
      "models]\n");
  Msg("[VBMEM]\n");
}

void CVBAllocTracker::UpdateData(const char *allocatorName, short allocatorKey,
                                 int bufferSize, VertexFormat_t fmt,
                                 int numVerts, int vertexSize, bool isDynamic,
                                 bool isCompressed) {
  ...
  if (handle != m_VBCountTable.InvalidHandle()) {
    ...
    data.m_paddingCount +=
        (bufferSize < 0 ? -1 : +1) * ComputeAlignmentWastage(abs(bufferSize));
  ...
  if (handle != m_VBCountTable.InvalidHandle()) {
    ...
    if (data.m_memCount > 0) {
      Msg("[VBMEM]    running mem usage: (%5.2f M-verts) %6.2f MB | '%s'\n",
          ...
          data.m_memCount / (1024.0f * 1024.0f), data.m_AllocatorName);
    }
    if (data.m_paddingCount > 0) {
      Msg("[VBMEM]    4KB  VB  alignment  wastage:       %6.2f MB | '%s'\n",
          data.m_paddingCount / (1024.0f * 1024.0f), data.m_AllocatorName);
    }
  }
}
...
void CVBAllocTracker::SpewDataSometimes(int inc) {
  ...
  if (period >= 1) {
    if ((count % period) == 0) {
      ...
      // Msg( "[VBMEM]    Conservative estimate of mem used to track allocs:
      // %d\n", 4096*ROUND_UP( 4 + sizeof( CUtlPtrLinkedList<AllocData> ) ) +
      // count*ROUND_UP( sizeof( AllocData ) + 8 ) );
...
void CVBAllocTracker::DumpVBAllocs() {
  ...
  Msg("[VBMEM]   'D' - difference between C and I (-> how much more "
      "compression could save)\n");
...
void CVBAllocTracker::CountVB(void *buffer, bool isDynamic, int bufferSize,
                              int vertexSize, VertexFormat_t fmt) {
#if ENABLE_VB_ALLOC_TRACKER
  m_VBAllocMutex.Lock();
  ...
  const char *allocatorName =
      (m_MeshAllocatorName[0] == 0) ? "unknown" : m_MeshAllocatorName;
  ...
  if (!isDynamic) {
    ...
    UpdateData("total_static", totalStaticKey, bufferSize, fmt, numVerts,
               vertexSize, isDynamic, isCompressed);
  }
  UpdateData(allocatorName, key, bufferSize, fmt, numVerts, vertexSize,
             isDynamic, isCompressed);
...
void CVBAllocTracker::UnCountVB(void *buffer) {
  ...
  // We have to store allocation data because the caller often doesn't know what
  // it alloc'd :o/
  if (KillAlloc(buffer, bufferSize, fmt, numVerts, key)) {
    ...
    if (!isDynamic) {
      UpdateData(NULL, totalKey, -bufferSize, fmt, -numVerts, 0, isDynamic,
                 isCompressed);
    }
    UpdateData(NULL, key, -bufferSize, fmt, -numVerts, 0, isDynamic,
               isCompressed);
...
static ConCommand mem_dumpvballocs("mem_dumpvballocs", CC_DumpVBMemAllocs,
