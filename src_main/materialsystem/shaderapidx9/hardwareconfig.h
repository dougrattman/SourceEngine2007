...
struct HardwareCaps_t : public MaterialAdapterInfo_t {
  // *****************************NOTE*********************************************
  // If you change any members, make sure and reflect the changes in
  // CHardwareConfig::ForceCapsToDXLevel for every dxlevel!!!!! If you change
  // any members, make sure and reflect the changes in
  // CHardwareConfig::ForceCapsToDXLevel for every dxlevel!!!!! If you change
  // any members, make sure and reflect the changes in
  // CHardwareConfig::ForceCapsToDXLevel for every dxlevel!!!!! If you change
  // any members, make sure and reflect the changes in
  // CHardwareConfig::ForceCapsToDXLevel for every dxlevel!!!!! If you change
  // any members, make sure and reflect the changes in
  // CHardwareConfig::ForceCapsToDXLevel for every dxlevel!!!!!
  // *****************************NOTE*********************************************
  //
  // NOTE: Texture stages are an obsolete concept; used by fixed-function
  // hardware Samplers are dx9+, indicating how many textures we can
  // simultaneously bind In Dx8, samplers didn't exist and texture stages were
  // used to indicate the number of simultaneously bound textures; we'll emulate
  // that by slamming the number of samplers to == the number of texture stages.
  ...
  unsigned long m_AlphaToCoverageState;  // State to ping to toggle Alpha To
                                         // Coverage (vendor-dependent)
  unsigned long
      m_AlphaToCoverageEnableValue;  // Value to set above state to turn on
                                     // Alpha To Coverage  (vendor-dependent)
  unsigned long
      m_AlphaToCoverageDisableValue;  // Value to set above state to turn off
                                      // Alpha To Coverage (vendor-dependent)
  int m_nMaxViewports;
