...
// "stdshaders" on the host machine and is completely compatible with -dvd.
// Ensure that the share is writable if you plan on generating UPDB's.
...
// uncomment and fill in with a path to use a specific set of shader source
// files. Meant for network use.
//		PC path format is of style
//"\\\\somemachine\\sourcetreeshare\\materialsystem\\stdshaders"
//		Xbox path format is of style
//"net:\\smb\\somemachine\\sourcetreeshare\\materialsystem\\stdshaders"
//			- Xbox dynamic compiles without a custom path default to
// look directly for "stdshaders" share on host pc
...
static CUtlMap<IDirect3DVertexShader9 *, CRC32_t, int> s_VSLookup(
    0, 0, DefLessFunc(IDirect3DVertexShader9 *));
static CUtlMap<IDirect3DPixelShader9 *, CRC32_t, int> s_PSLookup(
    0, 0, DefLessFunc(IDirect3DPixelShader9 *));
#endif

static void RegisterVS(const void *pShaderBits, int nShaderSize,
  ...
  if (nIndex != s_UniqueVS.InvalidIndex()) {
    ...
    VPROF_INCREMENT_GROUP_COUNTER("vs driver mem", COUNTER_GROUP_NO_RESET,
                                  nMemUsed);
...
static void RegisterPS(const void *pShaderBits, int nShaderSize,
  ...
  if (nIndex != s_UniquePS.InvalidIndex()) {
    ...
    VPROF_INCREMENT_GROUP_COUNTER("ps driver mem", COUNTER_GROUP_NO_RESET,
                                  nMemUsed);
    VPROF_INCREMENT_GROUP_COUNTER("total driver mem", COUNTER_GROUP_NO_RESET,
                                  nMemUsed);
  }
...
static void UnregisterVS(IDirect3DVertexShader9 *pShader) {
  ...
  if (nCRCIndex == s_VSLookup.InvalidIndex()) return;
  ...
  if (nIndex != s_UniqueVS.InvalidIndex()) {
    if (--s_UniqueVS[nIndex] <= 0) {
      int nMemUsed = 23 * 1024;
      VPROF_INCREMENT_GROUP_COUNTER("unique vs count", COUNTER_GROUP_NO_RESET,
                                    -1);
      VPROF_INCREMENT_GROUP_COUNTER("vs driver mem", COUNTER_GROUP_NO_RESET,
                                    -nMemUsed);
      VPROF_INCREMENT_GROUP_COUNTER("total driver mem", COUNTER_GROUP_NO_RESET,
...
static void UnregisterPS(IDirect3DPixelShader9 *pShader) {
  ...
  if (nCRCIndex == s_PSLookup.InvalidIndex()) return;
  ...
  if (nIndex != s_UniquePS.InvalidIndex()) {
    if (--s_UniquePS[nIndex] <= 0) {
      int nMemUsed = 400;
      VPROF_INCREMENT_GROUP_COUNTER("unique ps count", COUNTER_GROUP_NO_RESET,
                                    -1);
      VPROF_INCREMENT_GROUP_COUNTER("ps driver mem", COUNTER_GROUP_NO_RESET,
                                    -nMemUsed);
      VPROF_INCREMENT_GROUP_COUNTER("total driver mem", COUNTER_GROUP_NO_RESET,
...
static HardwareShader_t CreateD3DVertexShader(DWORD *pByteCode, int numBytes) {
  ...
  HRESULT hr = Dx9Device()->CreateVertexShader(
      pByteCode, (IDirect3DVertexShader9 **)&hShader);
...
static void PatchPixelShaderForAtiMsaaHack(DWORD *pShader,
                                           DWORD dwTexCoordMask) {
  if (IsPC()) {
    bool bIsSampler, bIsTexCoord;

    // Should be able to patch only ps2.0
    if (*pShader != 0xFFFF0200) return;
    ...
    while (pShader) {
      switch (*pShader & D3DSI_OPCODE_MASK) {
        ...
        case D3DSIO_DCL:
          bIsSampler =
              (*(pShader + 1) & D3DSP_TEXTURETYPE_MASK) != D3DSTT_UNKNOWN;
          bIsTexCoord =
              (((*(pShader + 2) & D3DSP_REGTYPE_MASK) >> D3DSP_REGTYPE_SHIFT) +
               ((*(pShader + 2) & D3DSP_REGTYPE_MASK2) >>
                D3DSP_REGTYPE_SHIFT2)) == D3DSPR_TEXTURE;
        ...
        default:
          ...
          pShader =
              pShader +
              ((*pShader & D3DSI_INSTLENGTH_MASK) >> D3DSI_INSTLENGTH_SHIFT) +
              1;
      }
    }
  }
...
//-----------------------------------------------------------------------------
static HardwareShader_t CreateD3DPixelShader(DWORD *pByteCode,
  ...
  HRESULT hr = Dx9Device()->CreatePixelShader(pByteCode,
                                              (IDirect3DPixelShader **)&shader);

  // NOTE: We have to do this after creating the pixel shader since we don't
  // know lookup.m_PixelShader yet!!!!!!!
...
template <class T>
int BinarySearchCombos(uint32_t nStaticComboID, int nCombos,
                       T const *pRecords) {
  // Use binary search - data is sorted
  int nLowerIdx = 1;
  int nUpperIdx = nCombos;
  for (;;) {
    if (nUpperIdx < nLowerIdx) return -1;

    int nMiddleIndex = (nLowerIdx + nUpperIdx) / 2;
    uint32_t nProbe = pRecords[nMiddleIndex - 1].m_nStaticComboID;
...
inline int FindShaderStaticCombo(uint32_t nStaticComboID,
                                 const ShaderHeader_t &header,
                                 StaticComboRecord_t *pRecords) {
  if (header.m_nVersion < 5) return -1;

  return BinarySearchCombos(nStaticComboID, header.m_nNumStaticCombos,
                            pRecords);
}
...
struct ShaderFileCache_t {
  ...
  // valid for diff version only - contains the microcode used as the reference
  // for diff algorithm
  ...
  int FindCombo(uint32_t nStaticComboID) {
    int nSearchAliases =
        BinarySearchCombos(nStaticComboID, m_StaticComboDupRecords.Count(),
                           m_StaticComboDupRecords.Base());
    if (nSearchAliases != -1)
      nStaticComboID =
          m_StaticComboDupRecords[nSearchAliases].m_nSourceStaticCombo;
    return FindShaderStaticCombo(nStaticComboID, m_Header,
                                 m_StaticComboRecords.Base());
  }
...
class CShaderManager : public IShaderManager {
 public:
  ...
  bool CreateDynamicCombos_Ver4(void *pContext, uint8_t *pComboBuffer);
  bool CreateDynamicCombos_Ver5(void *pContext, uint8_t *pComboBuffer);

  static void QueuedLoaderCallback(void *pContext, void *pContext2,
                                   const void *pData, int nSize,
                                   LoaderError_t loaderError);

 private:
  typedef CUtlFixedLinkedList<IDirect3DVertexShader9 *>::IndexType_t
      VertexShaderIndex_t;
  typedef CUtlFixedLinkedList<IDirect3DPixelShader9 *>::IndexType_t
      PixelShaderIndex_t;

  struct ShaderStaticCombos_t {
    int m_nCount;

    // Can't use CUtlVector here since you CUtlLinkedList<CUtlVector<>> doesn't
    // work.
    HardwareShader_t *m_pHardwareShaders;
    struct ShaderCreationData_t {
      CUtlVector<uint8_t> ByteCode;
      uint32_t iCentroidMask;
    };
  ...
  struct ShaderLookup_t {
    ...
    // for queued loading, bias an aligned optimal buffer forward to correct
    // location
    int m_nDataOffset;
 ...
 private:
  ...
  void SetVertexShaderState(
      HardwareShader_t shader,
      DataCacheHandle_t hCachedShader = DC_INVALID_HANDLE);

  // The low-level dx call to set the pixel shader state
  void SetPixelShaderState(HardwareShader_t shader,
                           DataCacheHandle_t hCachedShader = DC_INVALID_HANDLE);
  ...
  FileHandle_t OpenFileAndLoadHeader(const char *pFileName,
  ...
  HardwareShader_t CompileShader(const char *pShaderName, int nStaticIndex,
                                 int nDynamicIndex, bool bVertexShader);
#endif

  void DisassembleShader(ShaderLookup_t *pLookup, int dynamicCombo,
                         uint8_t *pByteCode);
  ...
  // true, creates on access, helps reduce d3d memory for tools, but causes i/o
  // hitches.
...
void CShaderManager::Init() {
  ...
  m_bCreateShadersOnDemand = ShaderUtil()->InEditorMode() ||
                             CommandLine()->CheckParm("-shadersondemand");
  ...
  if (m_pShaderCompiler30) {
    m_ShaderCompileFileFunc30 = (ShaderCompileFromFileFunc_t)GetProcAddress(
        (HMODULE)m_pShaderCompiler30, "D3DXCompileShaderFromFileA");
  }
#endif
...
// Compiles shaders
//-----------------------------------------------------------------------------
IShaderBuffer *CShaderManager::CompileShader(const char *pProgram,
                                             size_t nBufLen,
                                             const char *pShaderVersion) {
  ...
  HRESULT hr =
      D3DXCompileShader(pProgram, nBufLen, NULL, NULL, "main", pShaderVersion,
                        nCompileFlags, &pCompiledShader, &pErrorMessages, NULL);
  ...
  CShaderBuffer<ID3DXBuffer> *pShaderBuffer =
      new CShaderBuffer<ID3DXBuffer>(pCompiledShader);
  if (pErrorMessages) {
    pErrorMessages->Release();
  }
...
VertexShaderHandle_t CShaderManager::CreateVertexShader(
    IShaderBuffer *pShaderBuffer) {
  // Create the vertex shader
  IDirect3DVertexShader9 *pVertexShader = NULL;
  HRESULT hr = Dx9Device()->CreateVertexShader(
      (const DWORD *)pShaderBuffer->GetBits(), &pVertexShader);
...
PixelShaderHandle_t CShaderManager::CreatePixelShader(
    IShaderBuffer *pShaderBuffer) {
  // Create the vertex shader
  IDirect3DPixelShader9 *pPixelShader = NULL;
  HRESULT hr = Dx9Device()->CreatePixelShader(
      (const DWORD *)pShaderBuffer->GetBits(), &pPixelShader);
...
void CShaderManager::CreateStaticShaders() {
  ...
  if (IsPC()) {
    ...
    Dx9Device()->CreatePixelShader(
        psIllegalMaterial, (IDirect3DPixelShader9 **)&s_pIllegalMaterialPS);
  }
}
...
#ifdef DYNAMIC_SHADER_COMPILE
const CShaderManager::ShaderCombos_t *CShaderManager::FindOrCreateShaderCombos(
  ...
  while (char *line = bffr.InplaceGetLinePtr()) {
    ...
    if (Q_stristr(line, "[360]") || Q_stristr(line, "[XBOX]")) continue;
    ...
    if (Q_stristr(pShaderName, "_ps20") && !Q_stristr(pShaderName, "_ps20b") &&
      ...
      continue;
    if (Q_stristr(pShaderName, "_ps20b") && Q_stristr(line, "[ps") &&
        !Q_stristr(line, "[ps20b]"))
      continue;
    if (Q_stristr(pShaderName, "_ps30") && Q_stristr(line, "[ps") &&
        !Q_stristr(line, "[ps30]"))
      continue;
    if (Q_stristr(pShaderName, "_vs20") && Q_stristr(line, "[vs") &&
        !Q_stristr(line, "[vs20]"))
      continue;
    if (Q_stristr(pShaderName, "_vs30") && Q_stristr(line, "[vs") &&
        !Q_stristr(line, "[vs30]"))
      continue;
    ...
    // slam a NULL at the end quote of the string so that we have the string at
    // pBeginningOfName.
    *pScan = '\0';
    pScan++;
    ...
    //		char buf[1024];
    //		sprintf( buf, "\"%s\" \"%s\" %d %d\n", bDynamic ? "DYNAMIC" :
    //"STATIC", pBeginningOfName, begin, end );
...
#endif
{
  ...
  if (!g_pFullFileSystem->ReadFile(pFileName, NULL, buf)) return E_FAIL;
...
static const char *FileNameToShaderModel(const char *pShaderName,
                                         bool bVertexShader) {
...
#ifdef _DEBUG
      Error(
          "Failed dynamic shader compiled\nBuild shaderapidx9.dll in debug to "
          "find problem\n");
#else
      Assert(0);
#endif
...
HardwareShader_t CShaderManager::CompileShader(const char *pShaderName,
  ...
  for (i = 0; i < combos.m_DynamicCombos.Count(); i++) {
    int countForCombo =
        combos.m_DynamicCombos[i].m_nMax - combos.m_DynamicCombos[i].m_nMin + 1;
    int val = nCombo % countForCombo + combos.m_DynamicCombos[i].m_nMin;
    nCombo /= countForCombo;
    macros[macroIndex].Name =
        m_ShaderSymbolTable.String(combos.m_DynamicCombos[i].m_ComboName);
    char buf[16];
    sprintf(buf, "%d", val);
  ...
  for (i = 0; i < combos.m_StaticCombos.Count(); i++) {
    int countForCombo =
        combos.m_StaticCombos[i].m_nMax - combos.m_StaticCombos[i].m_nMin + 1;
    int val = nCombo % countForCombo + combos.m_StaticCombos[i].m_nMin;
    nCombo /= countForCombo;
    macros[macroIndex].Name =
        m_ShaderSymbolTable.String(combos.m_StaticCombos[i].m_ComboName);
    char buf[16];
    sprintf(buf, "%d", val);
    CUtlSymbol valSymbol(buf);
    macros[macroIndex].Definition = valSymbol.String();
  ...
  // (i.e. give developers a chance to fix the shader code w/out restarting the
  // game)
#ifndef _DEBUG
  int retriesLeft = 20;
retry_compile:
  ...
  HRESULT hr;
  bool b30Shader =
      !Q_stricmp(pShaderModel, "vs_3_0") || !Q_stricmp(pShaderModel, "ps_3_0");
  if (m_ShaderCompileFileFunc30 && b30Shader) {
    CDxInclude dxInclude(filename);
    hr = m_ShaderCompileFileFunc30(
        filename, macros.Base(), &dxInclude, "main", pShaderModel,
...
#if (!defined(_X360))
    {
      if (b30Shader) {
        Warning(
            "Compiling with a stale version of d3dx. Should have d3d9x_33.dll "
            "installed (Apr 2007)\n");
      }
  ...
  if (hr != D3D_OK) {
    const char *pErrorMessageString =
        (const char *)pErrorMessages->GetBufferPointer();
    ...
    if (retriesLeft-- > 0) {
      DevMsg(0,
             "Failed dynamic shader compiled - fix the shader while the "
             "debugger is at the breakpoint, then continue\n");
      DebuggerBreakIfDebugging();
      goto retry_compile;
    }
    if (!IsX360())  // errors make the 360 puke and die. We have a better
                    // solution for this particular error
      Error(
          "Failed dynamic shader compile\nBuild shaderapidx9.dll in debug to "
          "find problem\n");
#else
    Assert(0);
#endif
    ...
    for (i = 0; macros[i].Name; i++) {
      ...
      Q_strncat(exampleCommandLine, macros[i].Definition,
                sizeof(exampleCommandLine));
      Q_strncat(exampleCommandLine, " ", sizeof(exampleCommandLine));
    }
    ...
    HRESULT hr;
    hr = D3DXDisassembleShader((DWORD *)pShader->GetBufferPointer(), false,
                               NULL, &pd3dxBuffer);
    Assert(hr == D3D_OK);
    ...
    tempBuffer.EnsureCapacity(pd3dxBuffer->GetBufferSize() +
                              exampleCommandLineLength);
    memcpy(tempBuffer.Base(), exampleCommandLine, exampleCommandLineLength);
    memcpy((char *)tempBuffer.Base() + exampleCommandLineLength,
...
#endif
    if (bVertexShader) {
      return CreateD3DVertexShader((DWORD *)pShader->GetBufferPointer(),
                                   pShader->GetBufferSize());
    } else {
      return CreateD3DPixelShader((DWORD *)pShader->GetBufferPointer(), 0,
                                  pShader->GetBufferSize());  // hack hack hack!
                                                              // need to get
                                                              // centroid info
                                                              // from the source
    }
...
#ifdef DYNAMIC_SHADER_COMPILE
bool CShaderManager::LoadAndCreateShaders_Dynamic(ShaderLookup_t &lookup,
                                                  bool bVertexShader) {
  ...
  lookup.m_ShaderStaticCombos.m_pHardwareShaders =
      new HardwareShader_t[numDynamicCombos];
  lookup.m_ShaderStaticCombos.m_nCount = numDynamicCombos;
  lookup.m_ShaderStaticCombos.m_pCreationData =
      new ShaderStaticCombos_t::ShaderCreationData_t[numDynamicCombos];
...
//-----------------------------------------------------------------------------
FileHandle_t CShaderManager::OpenFileAndLoadHeader(const char *pFileName,
                                                   ShaderHeader_t *pHeader) {
    ...
    switch (pHeader->m_nVersion) {
      ...
      default:
        ...
        Warning("Shader %s is the wrong version %d, expecting %d\n", pFileName,
                pHeader->m_nVersion, SHADER_VCS_VERSION_NUMBER);
        g_pFullFileSystem->Close(fp);
        return FILESYSTEM_INVALID_HANDLE;
    }
  }
...
//-----------------------------------------------------------------------------
void CShaderManager::DisassembleShader(ShaderLookup_t *pLookup,
                                       int dynamicCombo, uint8_t *pByteCode) {
  ...
  tempBuffer.SetBufferType(true, false);
  tempBuffer.EnsureCapacity(pd3dxBuffer->GetBufferSize());
  memcpy((char *)tempBuffer.Base(), pd3dxBuffer->GetBufferPointer(),
...
//-----------------------------------------------------------------------------
bool CShaderManager::CreateDynamicCombos_Ver4(void *pContext,
                                              uint8_t *pComboBuffer) {
  ShaderLookup_t *pLookup = (ShaderLookup_t *)pContext;

  ShaderFileCache_t *pFileCache =
      &m_ShaderFileCache[pLookup->m_hShaderFileCache];
  ShaderHeader_t *pHeader = &pFileCache->m_Header;

  int nReferenceComboSizeForDiffs =
      ((ShaderHeader_t_v4 *)pHeader)->m_nDiffReferenceSize;

  uint8_t *pReferenceShader = NULL;
  uint8_t *pDiffOutputBuffer = NULL;
  if (nReferenceComboSizeForDiffs) {
    // reference combo is *always* the largest combo, so safe worst case size
    // for uncompression buffer
    pReferenceShader = (uint8_t *)pFileCache->m_ReferenceCombo.Base();
    pDiffOutputBuffer = (uint8_t *)_alloca(nReferenceComboSizeForDiffs);
  }
  ...
  for (int i = 0; i < pHeader->m_nDynamicCombos; i++) {
    ...
    uint8_t *pByteCode = (uint8_t *)pComboBuffer + pLookup->m_nDataOffset +
                         pLookup->m_pComboDictionary[i].m_Offset -
                         nStartingOffset;
    int nByteCodeSize = pLookup->m_pComboDictionary[i].m_Size;

    if (pReferenceShader) {
      // reference combo better be the largest combo, otherwise memory
      // corruption
      Assert(nReferenceComboSizeForDiffs >= nByteCodeSize);

      // use the differencing algorithm to recover the full shader
      int nOriginalSize;
      ApplyDiffs(pReferenceShader, pByteCode, nReferenceComboSizeForDiffs,
                 nByteCodeSize, nOriginalSize, pDiffOutputBuffer,
    ...
    if (m_bCreateShadersOnDemand) {
      ...
      pLookup->m_ShaderStaticCombos.m_pCreationData[i].ByteCode.SetSize(
          nByteCodeSize);
      V_memcpy(pLookup->m_ShaderStaticCombos.m_pCreationData[i].ByteCode.Base(),
               pByteCode, nByteCodeSize);
      pLookup->m_ShaderStaticCombos.m_pCreationData[i].iCentroidMask =
          pFileCache->m_bVertexShader ? 0 : pHeader->m_nCentroidMask;
    } else {
      if (pFileCache->m_bVertexShader) {
        hardwareShader = CreateD3DVertexShader(
            reinterpret_cast<DWORD *>(pByteCode), nByteCodeSize);
      } else {
        hardwareShader =
            CreateD3DPixelShader(reinterpret_cast<DWORD *>(pByteCode),
                                 pHeader->m_nCentroidMask, nByteCodeSize);
      }
...
// Create dynamic combos
//-----------------------------------------------------------------------------
static uint32_t NextULONG(uint8_t *&pData) {
...
bool CShaderManager::CreateDynamicCombos_Ver5(void *pContext,
                                              uint8_t *pComboBuffer) {
  ShaderLookup_t *pLookup = (ShaderLookup_t *)pContext;
  ShaderFileCache_t *pFileCache =
      &m_ShaderFileCache[pLookup->m_hShaderFileCache];
  uint8_t *pCompressedShaders = pComboBuffer + pLookup->m_nDataOffset;

  uint8_t *pUnpackBuffer = new uint8_t[MAX_SHADER_UNPACKED_BLOCK_SIZE];

  // now, loop through all blocks
  bool bOK = true;
  while (bOK) {
    uint32_t nBlockSize = NextULONG(pCompressedShaders);
    ...
    switch (nBlockSize & 0xc0000000) {
      ...
      {
        // uncompress
        uint32_t nOutsize = MAX_SHADER_UNPACKED_BLOCK_SIZE;
        int nRslt = BZ2_bzBuffToBuffDecompress(
            reinterpret_cast<char *>(pUnpackBuffer), &nOutsize,
            reinterpret_cast<char *>(pCompressedShaders), nBlockSize, 1, 0);
        if (nRslt < 0) {
    ...
    uint8_t *pReadPtr = pUnpackBuffer;
    while (pReadPtr < pUnpackBuffer + nBlockSize) {
      uint32_t nCombo_ID = NextULONG(pReadPtr);
      uint32_t nShaderSize = NextULONG(pReadPtr);
      ...
      if (m_bCreateShadersOnDemand) {
        ...
        pLookup->m_ShaderStaticCombos.m_pCreationData[iIndex].ByteCode.SetSize(
            nShaderSize);
        V_memcpy(pLookup->m_ShaderStaticCombos.m_pCreationData[iIndex]
                     .ByteCode.Base(),
                 pReadPtr, nShaderSize);
        pLookup->m_ShaderStaticCombos.m_pCreationData[iIndex].iCentroidMask =
            pFileCache->m_bVertexShader ? 0
                                        : pFileCache->m_Header.m_nCentroidMask;
      } else {
        if (pFileCache->m_bVertexShader) {
          hardwareShader = CreateD3DVertexShader(
              reinterpret_cast<DWORD *>(pReadPtr), nShaderSize);
        } else {
          hardwareShader = CreateD3DPixelShader(
              reinterpret_cast<DWORD *>(pReadPtr),
              pFileCache->m_Header.m_nCentroidMask, nShaderSize);
        }
        if (hardwareShader == INVALID_HARDWARE_SHADER) {
...
//-----------------------------------------------------------------------------
// Static method, called by thread, don't call anything non-threadsafe from
// handler!!!
//-----------------------------------------------------------------------------
void CShaderManager::QueuedLoaderCallback(void *pContext, void *pContext2,
                                          const void *pData, int nSize,
                                          LoaderError_t loaderError) {
  ShaderLookup_t *pLookup = (ShaderLookup_t *)pContext;
  ...
  if (bOK) {
    if (pContext2) {
      ...
      bOK =
          s_ShaderManager.CreateDynamicCombos_Ver4(pContext, (uint8_t *)pData);
    } else {
      bOK =
          s_ShaderManager.CreateDynamicCombos_Ver5(pContext, (uint8_t *)pData);
    }
  }
  if (!bOK) {
...
// Loads all shaders
//-----------------------------------------------------------------------------
bool CShaderManager::LoadAndCreateShaders(ShaderLookup_t &lookup,
                                          bool bVertexShader) {
#ifdef DYNAMIC_SHADER_COMPILE
  ...
  // a cache hit prevents costly i/o for static components, i.e. header, ref
  // combo, etc.
  ...
  if (pFileCache->IsValid()) {
    ...
    hFile = OpenFileAndLoadHeader(
        m_ShaderSymbolTable.String(pFileCache->m_Filename), NULL);
      ...
      if (pFileCache->IsVersion6()) {
        ...
        if (nNumDups) {
          ...
          g_pFullFileSystem->Read(pFileCache->m_StaticComboDupRecords.Base(),
                                  nNumDups * sizeof(StaticComboAliasRecord_t),
                                  hFile);
        }
      }
    }
  ...
  if (pFileCache->IsOldVersion()) {
    int nDictionaryOffset =
        sizeof(ShaderHeader_t) +
        ((ShaderHeader_t_v4 *)pHeader)->m_nDiffReferenceSize;

    // read in shader's dynamic combos directory
    lookup.m_pComboDictionary =
        new ShaderDictionaryEntry_t[pHeader->m_nDynamicCombos];
    g_pFullFileSystem->Seek(
        hFile,
        nDictionaryOffset +
            lookup.m_nStaticIndex * sizeof(ShaderDictionaryEntry_t),
        FILESYSTEM_SEEK_HEAD);
    g_pFullFileSystem->Read(
        lookup.m_pComboDictionary,
        pHeader->m_nDynamicCombos * sizeof(ShaderDictionaryEntry_t), hFile);
    ...
    for (i = 0; i < pHeader->m_nDynamicCombos; i++) {
      ...
      Assert(lookup.m_pComboDictionary[i].m_Offset >= nStartingOffset &&
             lookup.m_pComboDictionary[i].m_Size >= 0);

      if (!nStartingOffset) {
        nStartingOffset = lookup.m_pComboDictionary[i].m_Offset;
      }
      nEndingOffset = lookup.m_pComboDictionary[i].m_Offset +
                      lookup.m_pComboDictionary[i].m_Size;
    }
    if (!nStartingOffset) {
    ...
    int nStaticComboIdx = pFileCache->FindCombo(
    ...
    nStartingOffset =
        pFileCache->m_StaticComboRecords[nStaticComboIdx].m_nFileOffset;
    nEndingOffset =
        pFileCache->m_StaticComboRecords[nStaticComboIdx + 1].m_nFileOffset;
  }

  // align offsets for unbuffered optimal i/o - fastest i/o possible
  unsigned nOffsetAlign, nSizeAlign, nBufferAlign;
  g_pFullFileSystem->GetOptimalIOConstraints(hFile, &nOffsetAlign, &nSizeAlign,
                                             &nBufferAlign);
  unsigned int nAlignedOffset =
      AlignValue((nStartingOffset - nOffsetAlign) + 1, nOffsetAlign);
  unsigned int nAlignedBytesToRead =
      AlignValue(nEndingOffset - nAlignedOffset, nSizeAlign);
  ...
  uint8_t *pOptimalBuffer =
      (uint8_t *)g_pFullFileSystem->AllocOptimalReadBuffer(
          hFile, nAlignedBytesToRead, nAlignedOffset);
...
//-----------------------------------------------------------------------------
VertexShader_t CShaderManager::CreateVertexShader(const char *pFileName,
                                                  int nStaticVshIndex) {
  MEM_ALLOC_CREDIT();
...
// Create pixel shader
//-----------------------------------------------------------------------------
PixelShader_t CShaderManager::CreatePixelShader(const char *pFileName,
                                                int nStaticPshIndex) {
  MEM_ALLOC_CREDIT();
...
// The low-level dx call to set the vertex shader state
//-----------------------------------------------------------------------------
void CShaderManager::SetVertexShaderState(HardwareShader_t shader,
                                          DataCacheHandle_t hCachedShader) {
...
void CShaderManager::BindVertexShader(VertexShaderHandle_t hVertexShader) {
  HardwareShader_t hHardwareShader =
      m_RawVertexShaderDict[(VertexShaderIndex_t)hVertexShader];
...
void CShaderManager::SetVertexShader(VertexShader_t shader) {
  ...
  HardwareShader_t &dxshader =
      m_VertexShaderDict[shader]
          .m_ShaderStaticCombos.m_pHardwareShaders[vshIndex];
  ...
  Q_strncpy(vshDebugName[vshDebugIndex],
            m_ShaderSymbolTable.String(lookup.m_Name), sizeof(vshDebugName[0]));
#endif
  HardwareShader_t dxshader =
  ...
  if ((dxshader == INVALID_HARDWARE_SHADER) && m_bCreateShadersOnDemand) {
    ...
    ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
        &m_VertexShaderDict[shader]
             .m_ShaderStaticCombos.m_pCreationData[vshIndex];
#else
    ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
        &lookup.m_ShaderStaticCombos.m_pCreationData[vshIndex];
#endif

    dxshader = CreateD3DVertexShader((DWORD *)pCreationData->ByteCode.Base(),
                                     pCreationData->ByteCode.Count());

#ifdef DYNAMIC_SHADER_COMPILE
    // copy the compiled shader handle back to wherever it's supposed to be
    // stored
    m_VertexShaderDict[shader]
        .m_ShaderStaticCombos.m_pHardwareShaders[vshIndex] = dxshader;
#else
  ...
  if (!dxshader) {
    Error(
        "!!!!!Using invalid shader combo!!!!!  Consult a programmer and tell "
        "them to build debug materialsystem.dll and stdshader*.dll.  Run with "
        "\"mat_bufferprimitives 0\" and look for CMaterial in the call stack "
        "and see what m_pDebugName is.  You are likely using a shader combo "
...
// The low-level dx call to set the pixel shader state
//-----------------------------------------------------------------------------
void CShaderManager::SetPixelShaderState(HardwareShader_t shader,
                                         DataCacheHandle_t hCachedShader) {
...
void CShaderManager::BindPixelShader(PixelShaderHandle_t hPixelShader) {
  HardwareShader_t hHardwareShader =
      m_RawPixelShaderDict[(PixelShaderIndex_t)hPixelShader];
...
// lookup.m_Name ), 		lookup.m_nStaticIndex, m_nPixelShaderIndex );
  ...
  HardwareShader_t &dxshader =
      m_PixelShaderDict[shader]
          .m_ShaderStaticCombos.m_pHardwareShaders[pshIndex];
  if (dxshader == INVALID_HARDWARE_SHADER) {
    // compile it since we haven't already!
  ...
  Q_strncpy(pshDebugName[pshDebugIndex],
            m_ShaderSymbolTable.String(lookup.m_Name), sizeof(pshDebugName[0]));
#endif
  HardwareShader_t dxshader =
  ...
  if ((dxshader == INVALID_HARDWARE_SHADER) && m_bCreateShadersOnDemand) {
    ...
    ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
        &m_PixelShaderDict[shader]
             .m_ShaderStaticCombos.m_pCreationData[pshIndex];
#else
    ShaderStaticCombos_t::ShaderCreationData_t *pCreationData =
        &lookup.m_ShaderStaticCombos.m_pCreationData[pshIndex];
#endif

    dxshader = CreateD3DPixelShader((DWORD *)pCreationData->ByteCode.Base(),
                                    pCreationData->iCentroidMask,
                                    pCreationData->ByteCode.Count());
    ...
    // stored
    m_PixelShaderDict[shader]
        .m_ShaderStaticCombos.m_pHardwareShaders[pshIndex] = dxshader;
#else
...
void CShaderManager::ResetShaderState() {
  // This will force the calls to SetVertexShader + SetPixelShader to actually
  // set the state
...
void CShaderManager::DestroyVertexShader(VertexShader_t shader) {
  ShaderStaticCombos_t &combos =
      m_VertexShaderDict[shader].m_ShaderStaticCombos;
  int i;
  for (i = 0; i < combos.m_nCount; i++) {
    if (combos.m_pHardwareShaders[i] != INVALID_HARDWARE_SHADER) {
      IDirect3DVertexShader9 *pShader =
          (IDirect3DVertexShader9 *)combos.m_pHardwareShaders[i];
...
void CShaderManager::DestroyPixelShader(PixelShader_t pixelShader) {
  ShaderStaticCombos_t &combos =
      m_PixelShaderDict[pixelShader].m_ShaderStaticCombos;
  int i;
  for (i = 0; i < combos.m_nCount; i++) {
    if (combos.m_pHardwareShaders[i] != INVALID_HARDWARE_SHADER) {
      IDirect3DPixelShader *pShader =
          (IDirect3DPixelShader *)combos.m_pHardwareShaders[i];
...
void CShaderManager::SpewVertexAndPixelShaders(void) {
  ...
  for (VertexShader_t vshIndex = m_VertexShaderDict.Head();
    ...
    const char *pName = m_ShaderSymbolTable.String(lookup.m_Name);
    Msg("vsh 0x%8.8x: static combo:%9d dynamic combos:%6d refcount:%4d "
        "\"%s\"\n",
  ...
  for (PixelShader_t pshIndex = m_PixelShaderDict.Head();
    ...
    const char *pName = m_ShaderSymbolTable.String(lookup.m_Name);
    Msg("psh 0x%8.8x: static combo:%9d dynamic combos:%6d refcount:%4d "
        "\"%s\"\n",
...
void CShaderManager::FlushShaders(void) {
  for (VertexShader_t shader = m_VertexShaderDict.Head();
    ...
    ShaderStaticCombos_t &combos =
        m_VertexShaderDict[shader].m_ShaderStaticCombos;
    if (!(m_VertexShaderDict[shader].m_Flags &
          SHADER_DYNAMIC_COMPILE_IS_HLSL)) {
  ...
  for (PixelShader_t shader = m_PixelShaderDict.Head();
    ...
    int i;
    ShaderStaticCombos_t &combos =
        m_PixelShaderDict[shader].m_ShaderStaticCombos;
...
#ifdef DYNAMIC_SHADER_COMPILE
CON_COMMAND(mat_flushshaders,
            "flush all hardware shaders when using DYNAMIC_SHADER_COMPILE") {
