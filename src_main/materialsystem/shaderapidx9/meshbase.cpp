...
inline float *BoneWeight(VertexDesc_t const &desc, int vert) {
  ...
  return (float *)((unsigned char *)desc.m_pBoneWeight +
...
inline Vector &Normal(VertexDesc_t const &desc, int vert) {
  Assert(desc.m_CompressionType == VERTEX_COMPRESSION_NONE);
  return *(Vector *)((unsigned char *)desc.m_pNormal +
...
void CVertexBufferBase::PrintVertexFormat(VertexFormat_t vertexFormat) {
  ...
  if (NumBoneWeights(vertexFormat) > 0) {
    Q_snprintf(pBuf, sizeof(pBuf), "VERTEX_BONEWEIGHT(%d)%s|",
               NumBoneWeights(vertexFormat),
               (compression ? "[COMPRESSED]" : ""));
    Msg(pBuf);
  }
  if (UserDataSize(vertexFormat) > 0) {
    Q_snprintf(pBuf, sizeof(pBuf), "VERTEX_USERDATA_SIZE(%d)|",
               UserDataSize(vertexFormat));
    Msg(pBuf);
  }
  int i;
  ...
  for (int i = 0; i < nVertexCount; ++i) {
    ...
    if (nBoneWeightCount) {
      ...
      for (int j = 0; j < nBoneWeightCount; ++j) {
        nLen += Q_snprintf(&pTempBuf[nLen], sizeof(pTempBuf) - nLen, "%1.2f ",
                           pWeight[j]);
      }
    }
...
//-----------------------------------------------------------------------------
void CVertexBufferBase::ValidateData(int nVertexCount,
                                     const VertexDesc_t &spewDesc) {
  LOCK_SHADERAPI();
...
void CIndexBufferBase::Spew(int nIndexCount, const IndexDesc_t &indexDesc) {
  ...
  Q_snprintf(pTempBuf, sizeof(pTempBuf),
             ...
             indexDesc.m_nFirstIndex, indexDesc.m_nOffset);
  Warning(pTempBuf);
  for (int i = 0; i < nIndexCount; ++i) {
    nLen += Q_snprintf(pTemp, sizeof(pTempBuf) - nLen - 1, "%d ",
                       (int)indexDesc.m_pIndices[i]);
    pTemp = pTempBuf + nLen;
    if ((i & 0x0F) == 0x0F) {
