...
#ifndef MATERIALSYSTEM_SHADERAPIDX9_RECORDING_H_
#define MATERIALSYSTEM_SHADERAPIDX9_RECORDING_H_
...
// number of actual frames between each keyframe.
#define KEYFRAME_INTERVAL 100
...
// Use this to record debug strings . .these are only useful if you are doing
// "playback -list".
#define RECORD_DEBUG_STRINGS

// Recording state, if you change this, change the table in
...
enum RecordingCommands_t {
  ...
  // Isn't actually a dx8 command, used to help find particular frames.
  DX8_KEYFRAME,
  ...
  // Isn't actually a dx8 command. Let's playback know what format a buffer is
  // for listing info.
  DX8_SET_VERTEX_BUFFER_FORMAT,
...
inline void RecordInt(int i) { RecordArgument(&i, sizeof(i)); }

template <typename T>
inline void RecordPtr(T* p) {
  RecordArgument(&p, sizeof(p));
}

inline void RecordFloat(float f) { RecordArgument(&f, sizeof(f)); }

#define RECORD_COMMAND(_cmd, _numargs) RecordCommand(_cmd, _numargs)
#define RECORD_INT(_int) RecordInt(_int)
#define RECORD_PTR(_ptr) RecordPtr(_ptr)
...
#endif  // MATERIALSYSTEM_SHADERAPIDX9_RECORDING_H_
