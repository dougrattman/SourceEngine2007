...
class CVertexBufferDx8 : public CVertexBufferBase {
 ...
 public:
  // constructor
  CVertexBufferDx8(ShaderBufferType_t type, VertexFormat_t fmt,
 ...
 protected:
  ...
  int m_nVertexCount;
  int m_nBufferSize;
  int m_nFirstUnwrittenOffset;  // Used only for dynamic buffers, indicates
                                // where it's safe to write (nooverwrite)

  // Is it locked?
  bool m_bIsLocked : 1;
  bool m_bIsDynamic : 1;
  bool m_bFlush : 1;  // Used only for dynamic buffers, indicates to discard the
                      // next time
...
class CIndexBufferDx8 : public CIndexBufferBase {
 ...
 public:
  // constructor
  CIndexBufferDx8(ShaderBufferType_t bufferType, MaterialIndexFormat_t fmt,
...
abstract_class CBaseMeshDX8 : public CMeshBase {
 public:
  ...
  void ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount,
                     int nFirstIndex, int nIndexCount, MeshDesc_t &desc);
  void ModifyBegin(int nFirstVertex, int nVertexCount, int nFirstIndex,
                   int nIndexCount, MeshDesc_t &desc);
...
class CMeshDX8 : public CBaseMeshDX8 {
 ...
 protected:
  ...
  bool SetRenderState(int nVertexOffsetInBytes, int nFirstVertexIdx,
                      VertexFormat_t vertexFormat = VERTEX_FORMAT_INVALID);
  ...
  // Pass in nFirstIndex=-1 to lock wherever the index buffer is. Pass in a
  // value
  // >= 0 to specify where to lock.
  int Lock(bool bReadOnly, int nFirstIndex, int nIndexCount,
           IndexDesc_t &pIndices);
  ...
  static int s_nPrims;
  static unsigned int
      s_FirstVertex;  // Gets reset during CMeshDX8::DrawInternal
...
//-----------------------------------------------------------------------------
// A mesh that stores temporary vertex data in the correct format (for
// modification)
//-----------------------------------------------------------------------------
class CTempMeshDX8 : public CBaseMeshDX8 {
 public:
  ...
  // Locks mesh for modifying
  virtual void ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount,
                             int nFirstIndex, int nIndexCount,
                             MeshDesc_t &desc);
  virtual void ModifyBegin(int nFirstVertex, int nVertexCount, int nFirstIndex,
                           int nIndexCount, MeshDesc_t &desc);
...
#endif
//-----------------------------------------------------------------------------
// This is a version that buffers up vertex data so we can blast through it
// later
//-----------------------------------------------------------------------------
class CBufferedMeshDX8 : public CBaseMeshDX8 {
 public:
  ...
  void ValidateData(int nVertexCount, int nIndexCount,
                    const MeshDesc_t &spewDesc);
...
class CMeshMgr : public IMeshMgr {
 public:
  ...
  IMesh *CreateStaticMesh(VertexFormat_t vertexFormat,
                          const char *pTextureBudgetGroup,
                          IMaterial *pMaterial = NULL);
  void DestroyStaticMesh(IMesh *pMesh);

  // Gets at the dynamic mesh	(spoofs it though)
  IMesh *GetDynamicMesh(IMaterial *pMaterial, VertexFormat_t vertexFormat,
  ...
  // I don't think we use bSoftwareVertexShader anymore. .need to look into
  // bForceTempMesh.
  IVertexBuffer *CreateVertexBuffer(ShaderBufferType_t type, VertexFormat_t fmt,
                                    int nVertexCount, const char *pBudgetGroup);
  ...
  VertexFormat_t ComputeVertexFormat(unsigned int flags, int numTexCoords,
  ...
  CVertexBuffer *FindOrCreateVertexBuffer(int nDynamicBufferId,
                                          VertexFormat_t fmt);
  ...
  IVertexBuffer *GetDynamicVertexBuffer(IMaterial *pMaterial,
                                        bool buffered = true);
  IIndexBuffer *GetDynamicIndexBuffer(IMaterial *pMaterial,
                                      bool buffered = true);
 ...
 private:
  ...
  void SetIndexStreamState(int firstVertexIdx);
  bool SetRenderState(int nVertexOffsetInBytes, int nFirstVertexIdx,
                      VertexFormat_t vertexFormat, int nVertexStride);
  ...
  void CopyStaticMeshIndexBufferToTempMeshIndexBuffer(
      CTempMeshDX8 *pDstIndexMesh, CMeshDX8 *pSrcIndexMesh);
...
IMeshMgr *MeshMgr() { return &g_MeshMgr; }
...
inline void D3DSetStreamSource(unsigned int streamNumber,
                               IDirect3DVertexBuffer9 *pStreamData,
                               unsigned int nVertexOffsetInBytes,
                               unsigned int stride) {
  Dx9Device()->SetStreamSource(streamNumber, pStreamData, nVertexOffsetInBytes,
                               stride);
}
...
static int NumTextureCoordinates(VertexFormat_t vertexFormat) {
  ...
  for (int i = 0; i < VERTEX_MAX_TEXTURE_COORDINATES; ++i) {
    if (TexCoordSize(i, vertexFormat) == 0) continue;
    ++nTexCoordCount;
  }
...
void CIndexBufferDx8::BeginCastBuffer(MaterialIndexFormat_t format) {
  // NOTE: This should have no effect under Dx9, since we can't recast index
  // buffers.
...
// Locks/unlocks the index buffer
//-----------------------------------------------------------------------------
bool CIndexBufferDx8::Lock(int nMaxIndexCount, bool bAppend,
                           IndexDesc_t &desc) {
  ...
  if (bAppend) {
    ...
    nLockFlags |=
        (m_nFirstUnwrittenOffset == 0) ? D3DLOCK_DISCARD : D3DLOCK_NOOVERWRITE;
  } else {
    ...
    if (!m_bFlush && bHasEnoughMemory && m_bIsDynamic) {
      nLockFlags |= (m_nFirstUnwrittenOffset == 0) ? D3DLOCK_DISCARD
                                                   : D3DLOCK_NOOVERWRITE;
    } else {
      if (m_bIsDynamic) {
...
#ifdef CHECK_INDICES
  memcpy((unsigned char *)m_pShadowIndices + m_nLockIndexOffset,
         m_pLockIndexBuffer, nWrittenIndexCount * IndexSize());
#endif  // CHECK_INDICES
...
// constructor
//-----------------------------------------------------------------------------
CVertexBufferDx8::CVertexBufferDx8(ShaderBufferType_t type, VertexFormat_t fmt,
                                   int nVertexCount,
                                   const char *pBudgetGroupName)
    : BaseClass(pBudgetGroupName) {
...
void CVertexBufferDx8::BeginCastBuffer(VertexFormat_t format) {
  ...
  // so append can safely guarantee nooverwrite regardless of a format growth or
  // shrinkage
  if (!g_pHardwareConfig->SupportsStreamOffset()) {
    m_nFirstUnwrittenOffset =
        (m_nFirstUnwrittenOffset + nVertexSize - 1) / nVertexSize;
...
//-----------------------------------------------------------------------------
bool CVertexBufferDx8::Lock(int nMaxVertexCount, bool bAppend,
                            VertexDesc_t &desc) {
...
inline float Wrinkle(MeshDesc_t const &desc, int vert) {
  return *(float *)((unsigned char *)desc.m_pWrinkle +
                    vert * desc.m_VertexSize_Wrinkle);
}

inline D3DXVECTOR3 &BoneWeight(MeshDesc_t const &desc, int vert) {
  Assert(desc.m_CompressionType == VERTEX_COMPRESSION_NONE);
  return *(D3DXVECTOR3 *)((unsigned char *)desc.m_pBoneWeight +
                          vert * desc.m_VertexSize_BoneWeight);
}
...
inline D3DXVECTOR3 &Normal(MeshDesc_t const &desc, int vert) {
  Assert(desc.m_CompressionType == VERTEX_COMPRESSION_NONE);
  return *(D3DXVECTOR3 *)((unsigned char *)desc.m_pNormal +
                          vert * desc.m_VertexSize_Normal);
}
...
inline D3DXVECTOR2 &TexCoord(MeshDesc_t const &desc, int vert, int stage) {
  return *(D3DXVECTOR2 *)((unsigned char *)desc.m_pTexCoord[stage] +
                          vert * desc.m_VertexSize_TexCoord[stage]);
}

inline D3DXVECTOR3 &TangentS(MeshDesc_t const &desc, int vert) {
  return *(D3DXVECTOR3 *)((unsigned char *)desc.m_pTangentS +
                          vert * desc.m_VertexSize_TangentS);
}

inline D3DXVECTOR3 &TangentT(MeshDesc_t const &desc, int vert) {
  return *(D3DXVECTOR3 *)((unsigned char *)desc.m_pTangentT +
                          vert * desc.m_VertexSize_TangentT);
}
...
#ifdef _DEBUG
  if (m_pMaterial) return m_pMaterial->PerformDebugTrace();
#endif
...
bool CBaseMeshDX8::IsUsingMorphData() const {
  LOCK_SHADERAPI();
  // We're not using a morph unless the bound morph is a superset of what the
  // rendermesh needs
  MorphFormat_t morphFormat = GetMorphFormat();
  if (!morphFormat) return false;
...
// Locks mesh for modifying
//-----------------------------------------------------------------------------
void CBaseMeshDX8::ModifyBeginEx(bool bReadOnly, int nFirstVertex,
...
void CBaseMeshDX8::ModifyBegin(int nFirstVertex, int nVertexCount,
                               int nFirstIndex, int nIndexCount,
                               MeshDesc_t &desc) {
  LOCK_SHADERAPI();
  // for the time being, disallow for most cases
...
// Spews the mesh data
//-----------------------------------------------------------------------------
void CBaseMeshDX8::Spew(int nVertexCount, int nIndexCount,
                        const MeshDesc_t &spewDesc) {
  LOCK_SHADERAPI();
  // This has regressed.
  int i;
  ...
  for (i = 0; i < nVertexCount; ++i) {
    ...
    if (fmt & VERTEX_BONE_INDEX) {
      ...
      temp += sprintf(temp, "BI %d %d %d %d ", (int)pIndex[0], (int)pIndex[1],
    ...
    if (fmt & VERTEX_NORMAL) {
      ...
      temp += sprintf(temp, "N %1.2f %1.2f %1.2f ", normal[0], normal[1],
                      normal[2]);
    }

    if (fmt & VERTEX_COLOR) {
      unsigned char *pColor = Color(desc, i);
      temp += sprintf(temp, "C b %3d g %3d r %3d a %3d ", pColor[0], pColor[1],
                      pColor[2], pColor[3]);
    }
    ...
    if (fmt & VERTEX_TANGENT_S) {
      ...
      temp += sprintf(temp, "S %1.2f %1.2f %1.2f ", tangentS[0], tangentS[1],
                      tangentS[2]);
    }

    if (fmt & VERTEX_TANGENT_T) {
      D3DXVECTOR3 &tangentT = TangentT(desc, i);
      temp += sprintf(temp, "T %1.2f %1.2f %1.2f ", tangentT[0], tangentT[1],
                      tangentT[2]);
    }
...
void CBaseMeshDX8::ValidateData(int nVertexCount, int nIndexCount,
                                const MeshDesc_t &spewDesc) {
  LOCK_SHADERAPI();
  ...
  for (i = 0; i < nVertexCount; ++i) {
    ...
    if (fmt & VERTEX_TANGENT_S) {
      D3DXVECTOR3 &tangentS = TangentS(desc, i);
      Assert(IsFinite(tangentS[0]) && IsFinite(tangentS[1]) &&
             IsFinite(tangentS[2]));
    }

    if (fmt & VERTEX_TANGENT_T) {
      D3DXVECTOR3 &tangentT = TangentT(desc, i);
      Assert(IsFinite(tangentT[0]) && IsFinite(tangentT[1]) &&
             IsFinite(tangentT[2]));
    }
  }
#endif  // _DEBUG
...
void CMeshDX8::SetFlexMesh(IMesh *pMesh, int nVertexOffsetInBytes) {
  if (!ShaderUtil()->OnSetFlexMesh(this, pMesh, nVertexOffsetInBytes)) return;

  LOCK_SHADERAPI();
  m_nFlexVertOffsetInBytes =
      nVertexOffsetInBytes;  // Offset into dynamic mesh (in bytes)
...
bool CMeshDX8::Lock(int nVertexCount, bool bAppend, VertexDesc_t &desc) {
  ...
  if (!m_pVertexBuffer) {
    ...
    m_pVertexBuffer = new CVertexBuffer(
        Dx9Device(), m_VertexFormat, 0, size, nVertexCount, m_pTextureGroupName,
  ...
  if (!g_pHardwareConfig->SupportsStreamOffset()) {
    // Without stream offset, we can't use VBs greater than 65535 verts (due to
    // our using 16-bit indices)
    Assert(nVertexCount <= nMaxVerts);
  }

  unsigned char *pVertexMemory =
      m_pVertexBuffer->Lock(nVertexCount, desc.m_nFirstVertex);
  if (!pVertexMemory) {
    if (nVertexCount > nMaxVerts) {
      Assert(0);
      Error(
          "Too many verts for a dynamic vertex buffer (%d>%d) Tell a "
          "programmer to up VERTEX_BUFFER_SIZE.\n",
  ...
  // Set up the vertex descriptor
  CVertexBufferBase::ComputeVertexDescription(pVertexMemory, m_VertexFormat,
                                              desc);
  m_IsVBLocked = true;
...
// Locks/unlocks the index buffer
//-----------------------------------------------------------------------------
int CMeshDX8::Lock(bool bReadOnly, int nFirstIndex, int nIndexCount,
                   IndexDesc_t &desc) {
  Assert(!m_IsIBLocked);
  ...
  if (!m_pIndexBuffer) {
    m_pIndexBuffer = new CIndexBuffer(
        Dx9Device(), nIndexCount, ShaderAPI()->UsingSoftwareVertexProcessing());
  }

  int startIndex;
  desc.m_pIndices =
      m_pIndexBuffer->Lock(bReadOnly, nIndexCount, startIndex, nFirstIndex);
  if (!desc.m_pIndices) {
    desc.m_pIndices = (unsigned short *)(&g_nScratchIndexBuffer);
    desc.m_nIndexSize = 0;

    Assert(0);
    Error("failed to lock index buffer in CMeshDX8::Lock\n");
...
// Locks mesh for modifying
//-----------------------------------------------------------------------------
void CMeshDX8::ModifyBeginEx(bool bReadOnly, int nFirstVertex, int nVertexCount,
                             int nFirstIndex, int nIndexCount,
                             MeshDesc_t &desc) {
  ...
  // Lock it baby
  unsigned char *pVertexMemory =
      m_pVertexBuffer->Modify(bReadOnly, nFirstVertex, nVertexCount);
  if (pVertexMemory) {
    m_IsVBLocked = true;
...
void CMeshDX8::ModifyBegin(int nFirstVertex, int nVertexCount, int nFirstIndex,
                           int nIndexCount, MeshDesc_t &desc) {
  ModifyBeginEx(false, nFirstVertex, nVertexCount, nFirstIndex, nIndexCount,
                desc);
}
...
static void OutputVertexFormat(VertexFormat_t format) {
  ...
  Warning("\nBone weights: %d (%s)\n", NumBoneWeights(format),
          (CompressionType(format) == VERTEX_COMPRESSION_ON ? "compressed"
                                                            : "uncompressed"));
...
bool CMeshDX8::IsValidVertexFormat(VertexFormat_t vertexFormat) {
  ...
  bool bCheckCompression = (m_VertexFormat & VERTEX_FORMAT_COMPRESSED) &&
                           ((vertexFormat == VERTEX_FORMAT_INVALID) ||
                            ((vertexFormat & VERTEX_FORMAT_COMPRESSED) == 0));

  if (bCheckCompression || IsPC() || IsDebug()) {
    IMaterialInternal *pMaterial = ShaderAPI()->GetBoundMaterial();
    Assert(pMaterial);

    // the material format should match the vertex usage, unless another format
    // is passed in
    if (vertexFormat == VERTEX_FORMAT_INVALID) {
      vertexFormat =
          pMaterial->GetVertexUsage() &
          ~(VERTEX_FORMAT_VERTEX_SHADER | VERTEX_FORMAT_USE_EXACT_FORMAT);
    ...
    bool bIsValid = ((VERTEX_FORMAT_FIELD_MASK & vertexFormat) &
                     (VERTEX_FORMAT_FIELD_MASK & ~m_VertexFormat)) == 0;

    if (m_VertexFormat & VERTEX_FORMAT_COMPRESSED) {
      // We shouldn't get compressed verts if this material doesn't support
      // them!
      if ((vertexFormat & VERTEX_FORMAT_COMPRESSED) == 0) {
        static int numWarnings = 0;
        if (numWarnings++ == 0) {
          // NOTE: ComputeVertexFormat() will make sure no materials support
          // VERTEX_FORMAT_COMPRESSED
          //       if vertex compression is disabled in the config
          if (g_pHardwareConfig->SupportsCompressedVertices() ==
              VERTEX_COMPRESSION_NONE)
            Warning(
                "ERROR: Compressed vertices in use but vertex compression is "
                "disabled (or not supported on this hardware)!\n");
          else
            Warning(
                "ERROR: Compressed vertices in use but material does not "
                "support them!\n");
        }
        Assert(0);
        bIsValid = false;
    ...
    bIsValid =
        bIsValid && UserDataSize(m_VertexFormat) >= UserDataSize(vertexFormat);
    ...
    // NOTE: It can totally be valid to have more weights than the current
    // number of bones. The -1 here is because if we have N bones, we can have
    // only (N-1) weights, since the Nth is implied (the weights sum to 1).
    int nWeightCount = NumBoneWeights(m_VertexFormat);
    bIsValid =
        bIsValid && (nWeightCount >= (g_pShaderAPI->GetCurrentNumBones() - 1));
...
void CMeshDX8::SetVertexIDStreamState() {
  ...
  if (bUsingVertexID != g_bUsingVertexID) {
    if (bUsingVertexID) {
      ...
      // NOTE: At the moment, vertex id is only used for hw morphing. I've got
      // it set up so that a shader that supports hw morphing always says it
      // uses vertex id. If we ever use vertex id for something other than hw
      // morphing, we're going to have to revisit how those shaders say they
      // want to use vertex id or  fix this some other way
      Assert(!g_pShaderAPI->IsHWMorphingEnabled() ||
             !m_pVertexBuffer->IsDynamic());
      ...
      D3DSetStreamSource(3, pVertexIDBuffer->GetInterface(), 0,
                         pVertexIDBuffer->VertexSize());
      pVertexIDBuffer->HandlePerFrameTextureStats(
          ShaderAPI()->GetCurrentFrameCounter());
    } else {
      RECORD_COMMAND(DX8_SET_STREAM_SOURCE, 4);
...
void CMeshDX8::SetColorStreamState() {
  if ((m_pColorMesh != g_pLastColorMesh) ||
      (m_nColorMeshVertOffsetInBytes != g_nLastColorMeshVertOffsetInBytes)) {
    if (m_pColorMesh) {
      RECORD_COMMAND(DX8_SET_STREAM_SOURCE, 4);
      ...
      CVertexBuffer *vertex_buffer{m_pColorMesh->GetVertexBuffer()};
      RECORD_INT(vertex_buffer->VertexSize());

      D3DSetStreamSource(1, vertex_buffer->GetInterface(),
                         m_nColorMeshVertOffsetInBytes,
                         vertex_buffer->VertexSize());
      vertex_buffer->HandlePerFrameTextureStats(
          ShaderAPI()->GetCurrentFrameCounter());
    } else {
      RECORD_COMMAND(DX8_SET_STREAM_SOURCE, 4);
      RECORD_INT(-1);  // vertex buffer id
...
void CMeshDX8::SetVertexStreamState(int nVertOffsetInBytes) {
  ...
  if (HardwareConfig()->SupportsVertexAndPixelShaders()) {
    if (HasFlexMesh()) {
      // m_pFlexVertexBuffer is the flex buffer down inside the CMeshMgr
      // singleton
      D3DSetStreamSource(2, m_pFlexVertexBuffer->GetInterface(),
                         m_nFlexVertOffsetInBytes,
                         m_pFlexVertexBuffer->VertexSize());
      ...
      // NOTE: D3D debug DLLs will RIP if stream 0 has a smaller stride than the
      // largest
      //       offset in the stream 2 vertex decl elements (which are
      //       position(12)+wrinkle(4)+normal(12))
      // If this fires, go find the material/shader which is requesting a really
      // 'thin' stream 0 vertex, and fatten it up slightly (e.g. add a D3DCOLOR
      // element)
      ...
      if (m_pVertexBuffer->VertexSize() < minimumStreamZeroStride) {
        ...
        if (!bWarned) {
          Warning(
              "Shader specifying too-thin vertex format, should be at least %d "
              "bytes! (Supressing furthur warnings)\n",
              minimumStreamZeroStride);
          bWarned = true;
        }
      }
      D3DSetStreamSource(2, m_pVertexBuffer->GetInterface(), nVertOffsetInBytes,
                         m_pVertexBuffer->VertexSize());
  ...
  if ((g_pLastVertex != m_pVertexBuffer) ||
    ...
    D3DSetStreamSource(0, m_pVertexBuffer->GetInterface(), nVertOffsetInBytes,
                       m_pVertexBuffer->VertexSize());
    m_pVertexBuffer->HandlePerFrameTextureStats(
        ShaderAPI()->GetCurrentFrameCounter());
...
void CMeshDX8::SetIndexStreamState(int firstVertexIdx) {
  if ((g_pLastIndexBuffer != NULL) || (g_pLastIndex != m_pIndexBuffer) ||
      (firstVertexIdx != g_LastVertexIdx)) {
    Assert(m_pIndexBuffer);
    ...
    m_pIndexBuffer->HandlePerFrameTextureStats(
        ShaderAPI()->GetCurrentFrameCounter());
...
bool CMeshDX8::SetRenderState(int nVertexOffsetInBytes, int nFirstVertexIdx,
                              VertexFormat_t vertexFormat) {
...
void CMeshDX8::DrawInternal(CPrimList *pLists, int nLists) {
  ...
  int i;
  for (i = 0; i < nLists; i++) {
    if (pLists[i].m_NumIndices > 0) break;
  }
  if (i == nLists) return;
...
void CMeshDX8::RenderPass() {
  ...
  if (!IsValidVertexFormat(g_LastVertexFormat)) {
    Warning(
        "Material %s does not support vertex format used by the mesh (maybe "
        "missing fields or mismatched vertex compression?), mesh will not be "
        "rendered. Grab a programmer!\n",
  ...
  for (int iPrim = 0; iPrim < s_nPrims; iPrim++) {
    ...
    if ((m_Type == MATERIAL_POINTS) || (m_Type == MATERIAL_INSTANCED_QUADS)) {
      // (For point/instanced-quad lists, we don't actually fill in indices, but
      // we treat it as though there are indices for the list up until here).
...
#endif  // CHECK_INDICES
      {
        ...
        Dx9Device()->DrawIndexedPrimitive(
            m_Mode,  // Member of the D3DPRIMITIVETYPE enumerated type,
                     // describing the type of primitive to render.
                     // D3DPT_POINTLIST is not supported with this method.

            m_FirstIndex,  // Offset from the start of the vertex buffer to the
                           // first vertex index. An index of 0 in the index
                           // buffer refers to this location in the vertex
                           // buffer.

            s_FirstVertex,  // Minimum vertex index for vertices used during
                            // this call. This is a zero based index relative to
                            // BaseVertexIndex. The first Vertex in the
                            // vertexbuffer that we are currently using for the
                            // current batch.

            s_NumVertices,  // Number of vertices used during this call. The
                            // first vertex is located at index: BaseVertexIndex
                            // + MinIndex.

            pPrim->m_FirstIndex,  // Index of the first index to use when
                                  // accesssing the vertex buffer. Beginning at
                                  // StartIndex to index vertices from the
                                  // vertex buffer.

            numPrimitives);  // Number of primitives to render. The number of
                             // vertices used is a function of the primitive
                             // count and the primitive type.
      }
    }
  }
...
bool CDynamicMeshDX8::HasEnoughRoom(int nVertexCount, int nIndexCount) const {
  if (g_pShaderDeviceDx8->IsDeactivated()) return false;
...
// Locks/unlocks the entire mesh
//-----------------------------------------------------------------------------
void CDynamicMeshDX8::LockMesh(int nVertexCount, int nIndexCount,
                               MeshDesc_t &desc) {
  ShaderUtil()->SyncMatrices();
  ...
  // When we're using a static index buffer or a flex mesh, the indices assume
  // vertices start at 0
...
// Unlocks the mesh
//-----------------------------------------------------------------------------
void CDynamicMeshDX8::UnlockMesh(int nVertexCount, int nIndexCount,
                                 MeshDesc_t &desc) {
...
void CDynamicMeshDX8::Draw(int nFirstIndex, int nIndexCount) {
  ...
  if (m_IndexOverride || m_VertexOverride ||
      ((m_TotalVertices > 0) &&
       (m_TotalIndices > 0 || m_Type == MATERIAL_POINTS ||
        m_Type == MATERIAL_INSTANCED_QUADS))) {
    Assert(!m_IsDrawing);
    ...
    int nVertexOffsetInBytes =
        HasFlexMesh()
            ? nFirstVertex * g_MeshMgr.VertexFormatSize(GetVertexFormat())
            : 0;
    int baseIndex = m_IndexOverride ? 0 : m_FirstIndex;

    // Overriding with the dynamic index buffer, preserve state!
    if (m_IndexOverride &&
        m_pIndexBuffer == g_MeshMgr.GetDynamicIndexBuffer()) {
      baseIndex = m_FirstIndex;
    }

    VertexFormat_t fmt =
        m_VertexOverride ? GetVertexFormat() : VERTEX_FORMAT_INVALID;
    if (!SetRenderState(nVertexOffsetInBytes, actualFirstVertex, fmt)) return;

    // Draws a portion of the mesh
    int numVertices =
        m_VertexOverride ? m_pVertexBuffer->VertexCount() : m_TotalVertices;
    if ((nFirstIndex != -1) && (nIndexCount != 0)) {
...
void CDynamicMeshDX8::DrawSinglePassImmediately() {
  if ((m_TotalVertices > 0) || (m_TotalIndices > 0)) {
    ...
    if (SetRenderState(0, 0)) {
      ...
      static CPrimList prim(m_FirstIndex, m_TotalIndices);
...
CTempMeshDX8::CTempMeshDX8(bool isDynamic)
    : m_VertexSize(0xFFFF), m_IsDynamic(isDynamic) {
#ifdef _DEBUG
  m_Locked = false;
  m_InPass = false;
#endif
...
void CTempMeshDX8::ModifyBeginEx(bool bReadOnly, int nFirstVertex,
                                 int nVertexCount, int nFirstIndex,
                                 int nIndexCount, MeshDesc_t &desc) {
  Assert(!m_Locked);
...
void CTempMeshDX8::ModifyBegin(int nFirstVertex, int nVertexCount,
                               int nFirstIndex, int nIndexCount,
                               MeshDesc_t &desc) {
  ModifyBeginEx(false, nFirstVertex, nVertexCount, nFirstIndex, nIndexCount,
                desc);
...
// Locks/unlocks the mesh
//-----------------------------------------------------------------------------
void CTempMeshDX8::LockMesh(int nVertexCount, int nIndexCount,
                            MeshDesc_t &desc) {
  ShaderUtil()->SyncMatrices();
  ...
  if (nVertexCount > 0) {
    int vertexByteOffset =
        m_VertexData.AddMultipleToTail(m_VertexSize * nVertexCount);
...
void CTempMeshDX8::UnlockMesh(int nVertexCount, int nIndexCount,
                              MeshDesc_t &desc) {
  Assert(m_Locked);
  ...
  if (verticesToRemove != 0) {
    m_VertexData.RemoveMultiple(m_VertexData.Count() - verticesToRemove,
                                verticesToRemove);
  }

  int indicesToRemove = m_LockedIndices - nIndexCount;
  if (indicesToRemove != 0) {
    m_IndexData.RemoveMultiple(m_IndexData.Count() - indicesToRemove,
                               indicesToRemove);
  }
...
CDynamicMeshDX8 *CTempMeshDX8::GetDynamicMesh() {
  return static_cast<CDynamicMeshDX8 *>(
      g_MeshMgr.GetActualDynamicMesh(m_VertexFormat));
}
...
static bool PointInsidePlane(D3DXVECTOR3 *pVert, int normalInd, float val,
                             bool nearClip) {
  if ((val > 0) || nearClip)
...
static int ClipTriangleAgainstPlane(D3DXVECTOR3 **ppVert, int nVertexCount,
                                    D3DXVECTOR3 **ppOutVert, int normalInd,
                                    float val, bool nearClip = false) {
  ...
  for (int i = 0; i < nVertexCount; ++i) {
    ...
    if (endInside) {
      if (!startInside) {
        IntersectPlane(pStart, pEnd, normalInd, val,
                       &g_ClipVerts[g_NumClipVerts]);
...
void CTempMeshDX8::ClipTriangle(D3DXVECTOR3 **ppVert, float zNear,
  ...
  nVertexCount = ClipTriangleAgainstPlane(ppVert, nVertexCount, ppClipVert1, 2,
                                          zNear, true);
  if (nVertexCount < 3) return;
  ...
  for (i = 0; i < nVertexCount; ++i) {
    if (ppClipVert1[i] == &g_ClipVerts[0]) {
      ...
      D3DXVec3TransformCoord(&g_ClipVerts[g_NumClipVerts], ppClipVert1[i],
                             &projection);
  ...
  nVertexCount =
      ClipTriangleAgainstPlane(ppClipVert1, nVertexCount, ppClipVert2, 2, 1.0f);
  if (nVertexCount < 3) return;

  nVertexCount =
      ClipTriangleAgainstPlane(ppClipVert2, nVertexCount, ppClipVert1, 0, 1.0f);
  if (nVertexCount < 3) return;

  nVertexCount = ClipTriangleAgainstPlane(ppClipVert1, nVertexCount,
                                          ppClipVert2, 0, -1.0f);
  if (nVertexCount < 3) return;

  nVertexCount =
      ClipTriangleAgainstPlane(ppClipVert2, nVertexCount, ppClipVert1, 1, 1.0f);
  if (nVertexCount < 3) return;
...
void CTempMeshDX8::TestSelection() {
  ...
  for (int i = 0; i < numTriangles; ++i) {
    ...
    // negative w coords; negative coords throw off the projection-space
    // clipper.
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      D3DXVECTOR3 *pPosition =
          (D3DXVECTOR3 *)&m_VertexData[index * m_VertexSize];
    ...
    if (dot * cullFactor > 0.0f) continue;
...
void CTempMeshDX8::BeginPass() {
  ...
  memcpy(pMeshBuilder->BaseVertexData(), m_VertexData.Base(),
         m_VertexData.Count());
...
CBufferedMeshDX8::CBufferedMeshDX8()
    : m_IsFlushing(false), m_WasRendered(true) {
  m_pMesh = NULL;
...
// Spews the mesh data
//-----------------------------------------------------------------------------
void CBufferedMeshDX8::Spew(int nVertexCount, int nIndexCount,
                            const MeshDesc_t &spewDesc) {
  if (m_pMesh) {
...
void CBufferedMeshDX8::ValidateData(int nVertexCount, int nIndexCount,
                                    const MeshDesc_t &spewDesc) {
#if _DEBUG
  Assert(m_pMesh);
...
// Locks/unlocks the entire mesh
//-----------------------------------------------------------------------------
void CBufferedMeshDX8::LockMesh(int nVertexCount, int nIndexCount,
                                MeshDesc_t &desc) {
  ShaderUtil()->SyncMatrices();
...
// Locks/unlocks the entire mesh
//-----------------------------------------------------------------------------
void CBufferedMeshDX8::UnlockMesh(int nVertexCount, int nIndexCount,
                                  MeshDesc_t &desc) {
  Assert(m_pMesh);

  // Gotta fix up the first index to batch strips reasonably
  if ((m_pMesh->GetPrimitiveType() == MATERIAL_TRIANGLE_STRIP) &&
      desc.m_nIndexSize) {
    if (m_ExtraIndices > 0) {
...
CMeshMgr::CMeshMgr()
    : m_pDynamicIndexBuffer(0),
      ...
      m_DynamicIndexBuffer(SHADER_BUFFER_TYPE_DYNAMIC,
                           MATERIAL_INDEX_FORMAT_16BIT, INDEX_BUFFER_SIZE,
                           "dynamic"),
      m_DynamicVertexBuffer(SHADER_BUFFER_TYPE_DYNAMIC, VERTEX_FORMAT_UNKNOWN,
                            DYNAMIC_VERTEX_BUFFER_MEMORY, "dynamic") {
...
void CMeshMgr::Init() {
  ...
  // The dynamic index buffer
  m_pDynamicIndexBuffer =
      new CIndexBuffer(Dx9Device(), INDEX_BUFFER_SIZE,
                       ShaderAPI()->UsingSoftwareVertexProcessing(), true);
...
void CMeshMgr::CreateVertexIDBuffer() {
  ...
  if (g_pHardwareConfig->HasFastVertexTextures()) {
    m_pVertexIDBuffer =
        new CVertexBuffer(Dx9Device(), 0, 0, sizeof(float), VERTEX_BUFFER_SIZE,
...
//-----------------------------------------------------------------------------
void CMeshMgr::MarkUnusedVertexFields(unsigned int nFlags, int nTexCoordCount,
                                      bool *pUnusedTexCoords) {
...
void CMeshMgr::DiscardVertexBuffers() {
  VPROF_BUDGET("CMeshMgr::DiscardVertexBuffers",
               VPROF_BUDGETGROUP_SWAP_BUFFERS);
...
//-----------------------------------------------------------------------------
void CMeshMgr::CopyStaticMeshIndexBufferToTempMeshIndexBuffer(
    CTempMeshDX8 *pDstIndexMesh, CMeshDX8 *pSrcIndexMesh) {
  Assert(!pSrcIndexMesh->IsDynamic());
  int nIndexCount = pSrcIndexMesh->IndexCount();

  CMeshBuilder dstMeshBuilder;
  dstMeshBuilder.Begin(pDstIndexMesh, pSrcIndexMesh->GetPrimitiveType(), 0,
                       nIndexCount);
  CIndexBuffer *srcIndexBuffer = pSrcIndexMesh->GetIndexBuffer();
  int dummy = 0;
  unsigned short *srcIndexArray =
      srcIndexBuffer->Lock(false, nIndexCount, dummy, 0);
  int i;
  for (i = 0; i < nIndexCount; i++) {
...
//-----------------------------------------------------------------------------
IMesh *CMeshMgr::GetDynamicMesh(IMaterial *pMaterial,
                                VertexFormat_t vertexFormat,
                                int nHWSkinBoneCount, bool buffered,
                                IMesh *pVertexOverride, IMesh *pIndexOverride) {
  ...
  if (needTempMesh) {
    ...
    if (pIndexOverride) {
      CopyStaticMeshIndexBufferToTempMeshIndexBuffer(
          &m_DynamicTempMesh, (CMeshDX8 *)pIndexOverride);
    }
  ...
  if (!pVertexOverride) {
    // Remove VERTEX_FORMAT_COMPRESSED from the material's format (dynamic
    // meshes don't support compression, and all materials should support
    // uncompressed verts too)
    VertexFormat_t materialFormat =
        pMatInternal->GetVertexFormat() & ~VERTEX_FORMAT_COMPRESSED;
...
VertexFormat_t CMeshMgr::ComputeVertexFormat(unsigned int flags,
                                             ...
                                             int numBoneWeights,
                                             int userDataSize) const {
  // Construct a bitfield that makes sense and is unique from the standard FVF
  // formats
  ...
  fmt |= VERTEX_USERDATA_SIZE((VertexFormat_t)userDataSize);
...
// Returns the number of vertices we can render using the dynamic mesh
//-----------------------------------------------------------------------------
void CMeshMgr::GetMaxToRender(IMesh *pMesh, bool bMaxUntilFlush, int *pMaxVerts,
                              int *pMaxIndices) {
  CBaseMeshDX8 *pBaseMesh = static_cast<CBaseMeshDX8 *>(pMesh);
  if (!pBaseMesh) {
    *pMaxVerts = 0;
  ...
  if (IsBufferedDynamicMesh(pMesh)) {
    pBaseMesh =
        (CBaseMeshDX8 *)static_cast<CBufferedMeshDX8 *>(pBaseMesh)->GetMesh();
    pMesh = pBaseMesh;
  }
  ...
  if (!bMaxUntilFlush) {
    *pMaxVerts =
        ShaderAPI()->GetCurrentDynamicVBSize() / pVertexBuffer->VertexSize();
    if (*pMaxVerts > 65535) {
      *pMaxVerts = 65535;
    }
  ...
  *pMaxIndices =
      pIndexBuffer ? pIndexBuffer->IndexCount() - pIndexBuffer->IndexPosition()
                   : 0;
  if (*pMaxVerts == 0) {
    *pMaxVerts =
        ShaderAPI()->GetCurrentDynamicVBSize() / pVertexBuffer->VertexSize();
...
int CMeshMgr::GetMaxVerticesToRender(IMaterial *pMaterial) {
  ...
  int nMaxVerts =
      ShaderAPI()->GetCurrentDynamicVBSize() / VertexFormatSize(fmt);
  if (nMaxVerts > 65535) {
    nMaxVerts = 65535;
  }
  return nMaxVerts;
...
// Returns a vertex buffer appropriate for the flags
//-----------------------------------------------------------------------------
CVertexBuffer *CMeshMgr::FindOrCreateVertexBuffer(int nDynamicBufferId,
                                                  VertexFormat_t vertexFormat) {
  int vertexSize = VertexFormatSize(vertexFormat);

  while (m_DynamicVertexBuffers.Count() <= nDynamicBufferId) {
    // Track VB allocations (override any prior allocator string set higher up
    // on the callstack)
    g_VBAllocTracker->TrackMeshAllocations(NULL);
    g_VBAllocTracker->TrackMeshAllocations(
        "CMeshMgr::FindOrCreateVertexBuffer (dynamic VB)");

    // create the single 1MB dynamic vb that will be shared amongst all
    // consumers the correct thing is to use the largest expected vertex format
    // size of max elements, but this creates an undesirably large buffer -
    // instead create the buffer we want, and fix consumers that bork NOTE:
    // GetCurrentDynamicVBSize returns a smaller value during level transitions
    ...
    m_DynamicVertexBuffers[nIndex].m_pBuffer = new CVertexBuffer(
        Dx9Device(), 0, 0, nBufferMemory / VERTEX_BUFFER_SIZE,
        VERTEX_BUFFER_SIZE, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_OTHER,
        ShaderAPI()->UsingSoftwareVertexProcessing(), true);
  ...
  if (m_DynamicVertexBuffers[nDynamicBufferId].m_VertexSize != vertexSize) {
    ...
    // NOTE: GetCurrentDynamicVBSize returns a smaller value during level
    // transitions
    int nBufferMemory = ShaderAPI()->GetCurrentDynamicVBSize();
    m_DynamicVertexBuffers[nDynamicBufferId].m_VertexSize = vertexSize;
    m_DynamicVertexBuffers[nDynamicBufferId].m_pBuffer->ChangeConfiguration(
        vertexSize, nBufferMemory);
...
IVertexBuffer *CMeshMgr::GetDynamicVertexBuffer(IMaterial *pMaterial,
                                                bool buffered) {
  Assert(0);
  return NULL;
...
IIndexBuffer *CMeshMgr::GetDynamicIndexBuffer(IMaterial *pMaterial,
                                              bool buffered) {
  Assert(0);
  return NULL;
  //	return ( IMeshDX8 * )GetDynamicMesh( pMaterial, buffered, NULL, NULL );
...
//-----------------------------------------------------------------------------
IVertexBuffer *CMeshMgr::CreateVertexBuffer(ShaderBufferType_t type,
                                            VertexFormat_t fmt,
                                            int nVertexCount,
                                            const char *pBudgetGroup) {
  // FIXME: Use a fixed-size allocator
...
IIndexBuffer *CMeshMgr::CreateIndexBuffer(ShaderBufferType_t bufferType,
  ...
  switch (bufferType) {
    ...
    case SHADER_BUFFER_TYPE_DYNAMIC: {
      CIndexBufferDx8 *pIndexBuffer =
          new CIndexBufferDx8(bufferType, fmt, nIndexCount, pBudgetGroup);
      return pIndexBuffer;
    }
...
// Do we need to specify the stream here in the case of locking multiple dynamic
// VBs on different streams?
IVertexBuffer *CMeshMgr::GetDynamicVertexBuffer(int streamID,
                                                VertexFormat_t vertexFormat,
                                                bool bBuffered) {
...
IIndexBuffer *CMeshMgr::GetDynamicIndexBuffer(MaterialIndexFormat_t fmt,
                                              bool bBuffered) {
  m_BufferedMode = bBuffered;
...
void CMeshMgr::SetVertexStreamState(int nVertOffsetInBytes, int nVertexStride) {
  ...
  if (HardwareConfig()->SupportsVertexAndPixelShaders()) {
    ...
    D3DSetStreamSource(2, m_pCurrentVertexBuffer->GetDx9Buffer(),
                       nVertOffsetInBytes, nVertexStride);
  ...
  if (g_pLastVertex ||
    ...
    D3DSetStreamSource(0, m_pCurrentVertexBuffer->GetDx9Buffer(),
                       nVertOffsetInBytes, nVertexStride);
    m_pCurrentVertexBuffer->HandlePerFrameTextureStats(
        ShaderAPI()->GetCurrentFrameCounter());
...
bool CMeshMgr::SetRenderState(int nVertexOffsetInBytes, int nFirstVertexIdx,
                              VertexFormat_t vertexFormat, int nVertexStride) {
  // Can't set the state if we're deactivated
  if (g_pShaderDeviceDx8->IsDeactivated()) {
    ResetMeshRenderState();
...
void CMeshMgr::BindVertexBuffer(int nStreamID, IVertexBuffer *pVertexBuffer,
...
void CMeshMgr::Draw(MaterialPrimitiveType_t primitiveType, int nFirstIndex,
                    int nIndexCount) {
  // MESHFIXME : This path is only used for the new index/vertex buffer
  // interfaces. make sure we aren't using a morph stream for this path.
  //	Assert( !IsUsingMorphData() );
  //	Assert( !m_pColorMesh );

  SetRenderState(m_pVertexBufferOffset[0], /* nFirstVertexIdx */ 0,
                 m_CurrentVertexFormat, m_pCurrentVertexStride[0]);
...
void CMeshMgr::RenderPassWithVertexAndIndexBuffers(void) {
  ...
  //	for ( int iPrim=0; iPrim < s_nPrims; iPrim++ )
  {
    ...
    if (m_PrimitiveType == MATERIAL_POINTS) {
      ...
      Assert(0);
      //			Dx9Device()->DrawPrimitive( ComputeMode(
      // m_PrimitiveType ), s_FirstVertex, pPrim->m_NumIndices );
    } else {
      //			int numPrimitives = NumPrimitives(
      // s_NumVertices, pPrim->m_NumIndices );

      //			Warning(
      //"CMeshMgr::RenderPassWithVertexAndIndexBuffers: DrawIndexedPrimitive:
      // m_nFirstIndex = %d numPrimitives = %d\n", ( int )( (
      // CDynamiCIndexBufferDx8 * )m_pCurrentIndexBuffer )->m_FirstIndex, ( int
      //)( m_nNumIndices / 3 ) );
      {
        VPROF("Dx9Device()->DrawIndexedPrimitive");
        //				VPROF_INCREMENT_COUNTER(
...
#endif  // CHECK_INDICES
        Dx9Device()->DrawIndexedPrimitive(
            ComputeMode(m_PrimitiveType),  // Member of the D3DPRIMITIVETYPE
                                           // enumerated type, describing the
                                           // type of primitive to render.
                                           // D3DPT_POINTLIST is not supported
                                           // with this method.

            /*m_FirstIndex*/ 0,  // Offset from the start of the vertex buffer
                                 // to the first vertex index. An index of 0 in
                                 // the index buffer refers to this location in
                                 // the vertex buffer.

            /*s_FirstVertex*/ m_pFirstVertex[0],  // Minimum vertex index for
                                                  // vertices used during this
                                                  // call. This is a zero based
                                                  // index relative to
                                                  // BaseVertexIndex. This is
                                                  // zero for now since we don't
                                                  // do more than one batch yet
                                                  // with the new mesh
                                                  // interface.

            /*s_NumVertices*/ m_pVertexCount[0],
            // Number of vertices used during this call. The first vertex is
            // located at index: BaseVertexIndex + MinIndex. This is simple the
            // number of verts in the current vertex buffer for now since we
            // don't do more than one batch with the new mesh interface.

            m_nFirstIndex /*pPrim->m_FirstIndex*/,  // Index of the first index
                                                    // to use when accesssing
                                                    // the vertex buffer.
                                                    // Beginning at StartIndex
                                                    // to index vertices from
                                                    // the vertex buffer.

            m_nNumIndices /
                3 /*numPrimitives*/  // Number of primitives to render. The
                                     // number of vertices used is a function of
                                     // the primitive count and the primitive
                                     // type.
        );
...
void CMeshMgr::SetIndexStreamState(int firstVertexIdx) {
  CIndexBufferDx8 *pIndexBuffer =
      assert_cast<CIndexBufferDx8 *>(m_pCurrentIndexBuffer);
  IDirect3DIndexBuffer9 *pDx9Buffer =
      pIndexBuffer ? pIndexBuffer->GetDx9Buffer() : NULL;
  if (g_pLastIndex || g_pLastIndexBuffer != pDx9Buffer) {
    Dx9Device()->SetIndices(pDx9Buffer);
    pIndexBuffer->HandlePerFrameTextureStats(
        ShaderAPI()->GetCurrentFrameCounter());
