...
class CShaderShadowDX8 : public IShaderShadowDX8 {
 public:
  ...
  virtual void VertexShaderVertexFormat(unsigned int nFlags, int nTexCoordCount,
...
// Global instance
//-----------------------------------------------------------------------------
IShaderShadowDX8* ShaderShadow() { return &g_ShaderShadow; }
...
void CShaderShadowDX8::Init() {
  ...
  // NOTE: If you change these defaults, change the code in
  // ComputeAggregateShadowState + CreateTransitionTableEntry
  int i;
  for (i = 0; i < MAX_TEXTURE_STAGES; ++i) {
    m_ShadowState.m_TextureStage[i].m_ColorOp = D3DTOP_DISABLE;
    m_ShadowState.m_TextureStage[i].m_ColorArg1 = D3DTA_TEXTURE;
    m_ShadowState.m_TextureStage[i].m_ColorArg2 =
        (i == 0) ? D3DTA_DIFFUSE : D3DTA_CURRENT;
...
void CShaderShadowDX8::SetDefaultState() {
  ...
  for (i = 0; i < nTextureStageCount; i++) {
    ...
    CustomTextureOperation((TextureStage_t)i, SHADER_TEXCHANNEL_COLOR,
                           SHADER_TEXOP_DISABLE, SHADER_TEXARG_TEXTURE,
...
void CShaderShadowDX8::DepthFunc(ShaderDepthFunc_t depthFunc) {
  ...
  switch (depthFunc) {
    ...
    case SHADER_DEPTHFUNC_NEARER:
      zFunc = (ShaderUtil()->GetConfig().bReverseDepth ^ ReverseDepthOnX360())
                  ? D3DCMP_GREATER
                  : D3DCMP_LESS;
      break;
...
void CShaderShadowDX8::EnableColorWrites(bool bEnable) {
  if (bEnable) {
    ...
    m_ShadowState.m_ColorWriteEnable &=
...
void CShaderShadowDX8::AlphaFunc(ShaderAlphaFunc_t alphaFunc,
                                 float alphaRef /* [0-1] */) {
...
void CShaderShadowDX8::BlendFunc(ShaderBlendFactor_t srcFactor,
                                 ShaderBlendFactor_t dstFactor) {
...
void CShaderShadowDX8::BlendFuncSeparateAlpha(ShaderBlendFactor_t srcFactor,
                                              ShaderBlendFactor_t dstFactor) {
  D3DBLEND d3dSrcFactor = BlendFuncValue(srcFactor);
  D3DBLEND d3dDstFactor = BlendFuncValue(dstFactor);
...
// Indicates we've got a constant color specified
//-----------------------------------------------------------------------------
bool CShaderShadowDX8::HasConstantColor() const { return m_HasConstantColor; }
...
// A simpler method of dealing with alpha modulation
//-----------------------------------------------------------------------------
void CShaderShadowDX8::EnableAlphaPipe(bool bEnable) { m_AlphaPipe = bEnable; }
...
//-----------------------------------------------------------------------------
void CShaderShadowDX8::TextureCoordinate(TextureStage_t stage,
                                         int useTexCoord) {
...
void CShaderShadowDX8::TexGen(TextureStage_t stage, ShaderTexGenParam_t param) {
  if (stage >= m_pHardwareConfig->GetTextureStageCount()) return;
  ...
  switch (param) {
    ...
    case SHADER_TEXGENPARAM_SPHERE_MAP:
      if (m_pHardwareConfig->SupportsSpheremapping()) {
        ...
        m_TextureStage[stage].m_TexCoordIndex =
...
//-----------------------------------------------------------------------------
// alternate method of specifying per-texture unit stuff, more flexible and more
// complicated Can be used to specify different operation per channel
// (alpha/color)...
//-----------------------------------------------------------------------------
...
void CShaderShadowDX8::VertexShaderVertexFormat(unsigned int nFlags,
                                                ...
                                                int* pTexCoordDimensions,
                                                int nUserDataSize) {
  // Code that creates a Mesh should specify whether it contains bone
  // weights+indices, *not* the shader.
  ...
  m_ShadowShaderState.m_VertexUsage = MeshMgr()->ComputeVertexFormat(
      nFlags, nTexCoordCount, pTexCoordDimensions, 0, nUserDataSize);
  ...
  if (CVertexBufferBase::VertexFormatSize(m_ShadowShaderState.m_VertexUsage) <=
    ...
    //          data
    //        ...it would be far more sensible to not add stream 2 to all vertex
    //        decls.
    static bool bComplained = false;
    if (!bComplained) {
      Warning(
          "ERROR: shader asking for a too-narrow vertex format - you will see "
          "errors if running with debug D3D DLLs!\n\tPadding the vertex format "
          "with extra texcoords\n\tWill not warn again.\n");
      bComplained = true;
    }
    // All vertex formats should contain position...
    Assert(nFlags & VERTEX_POSITION);
    nFlags |= VERTEX_POSITION;
    // This error should occur only if we have zero texcoords, or if we have a
    // single, 1-D texcoord
    Assert((nTexCoordCount == 0) ||
           ((nTexCoordCount == 1) && pTexCoordDimensions &&
            (pTexCoordDimensions[0] == 1)));
    nTexCoordCount = 1;
    m_ShadowShaderState.m_VertexUsage = MeshMgr()->ComputeVertexFormat(
        nFlags, nTexCoordCount, NULL, 0, nUserDataSize);
  }
}
...
// Pixel and vertex shader methods
//-----------------------------------------------------------------------------
void CShaderShadowDX8::SetVertexShader(const char* pFileName,
                                       int nStaticVshIndex) {
  //	Assert( nStaticVshIndex == 0 );
  m_ShadowShaderState.m_VertexShader =
      ShaderManager()->CreateVertexShader(pFileName, nStaticVshIndex);
  m_ShadowShaderState.m_nStaticVshIndex = nStaticVshIndex;
}

void CShaderShadowDX8::SetPixelShader(const char* pFileName,
                                      int nStaticPshIndex) {
  m_ShadowShaderState.m_PixelShader =
      ShaderManager()->CreatePixelShader(pFileName, nStaticPshIndex);
...
//-----------------------------------------------------------------------------
inline bool CShaderShadowDX8::IsUsingTextureCoordinates(
    Sampler_t sampler) const {
  return m_SamplerState[sampler].m_TextureEnable;
}

inline D3DTEXTUREOP CShaderShadowDX8::OverbrightBlendValue(
...
void CShaderShadowDX8::ConfigureCustomFVFVertexShader(unsigned int flags) {
  ...
  for (i = 0; i < m_CustomTextureStages; ++i) {
    m_ShadowState.m_TextureStage[i].m_ColorArg1 =
        ComputeArg(m_TextureStage[i].m_Arg[0][0]);
    m_ShadowState.m_TextureStage[i].m_ColorArg2 =
        ComputeArg(m_TextureStage[i].m_Arg[0][1]);
    m_ShadowState.m_TextureStage[i].m_AlphaArg1 =
        ComputeArg(m_TextureStage[i].m_Arg[1][0]);
    m_ShadowState.m_TextureStage[i].m_AlphaArg2 =
        ComputeArg(m_TextureStage[i].m_Arg[1][1]);
    m_ShadowState.m_TextureStage[i].m_ColorOp =
        ComputeOp(m_TextureStage[i].m_Op[0]);
    m_ShadowState.m_TextureStage[i].m_AlphaOp =
        ComputeOp(m_TextureStage[i].m_Op[1]);
  }

  // Deal with texture stage 1 -> n
  for (i = m_CustomTextureStages; i < m_pHardwareConfig->GetTextureStageCount();
       ++i) {
    m_ShadowState.m_TextureStage[i].m_ColorArg1 = D3DTA_TEXTURE;
...
void CShaderShadowDX8::ConfigureAlphaPipe(unsigned int flags) {
  // Are we using color?
  bool isUsingVertexAlpha =
      m_HasVertexAlpha && ((flags & SHADER_DRAW_COLOR) != 0);
  ...
  while (lastTextureStage >= 0) {
    if (m_TextureStage[lastTextureStage].m_TextureAlphaEnable) break;
    --lastTextureStage;
  }
  ...
  for (int i = 0; i < m_pHardwareConfig->GetTextureStageCount(); ++i) {
    ...
    if (m_TextureStage[i].m_TextureAlphaEnable) {
      ...
      if ((i > lastTextureStage) && !isUsingVertexAlpha &&
          !isUsingConstantAlpha) {
        m_ShadowState.m_TextureStage[i].m_AlphaArg1 = D3DTA_TEXTURE;
        m_ShadowState.m_TextureStage[i].m_AlphaArg2 = D3DTA_CURRENT;
        m_ShadowState.m_TextureStage[i].m_AlphaOp = D3DTOP_DISABLE;
        continue;
...
void CShaderShadowDX8::ConfigureFVFVertexShader(unsigned int flags) {
  ...
  if (IsUsingTextureCoordinates(SHADER_SAMPLER0)) {
    if (isUsingVertexColor) {
      m_ShadowState.m_TextureStage[0].m_ColorOp =
          OverbrightBlendValue(SHADER_TEXTURE_STAGE0);
    ...
    if (isUsingVertexColor) {
      ...
      if (m_TextureStage[0].m_OverbrightVal < 2.0f) {
        ...
        if (isUsingConstantColor) {
          ...
          m_ShadowState.m_TextureStage[0].m_ColorOp =
              OverbrightBlendValue((TextureStage_t)0);
          m_ShadowState.m_TextureStage[0].m_AlphaOp = D3DTOP_MODULATE;
  ...
  if (isUsingConstantColor) {
    ...
    if (lastUsedTextureStage >= m_pHardwareConfig->GetTextureStageCount()) {
      ...
      // stage, but we can't because there aren't enough. In this case, we're
      // gonna need to do the modulation in the per-vertex color.
      ...
      m_ShadowState.m_TextureStage[lastUsedTextureStage].m_ColorArg1 =
          D3DTA_TFACTOR;
      m_ShadowState.m_TextureStage[lastUsedTextureStage].m_ColorArg2 =
          D3DTA_CURRENT;
      m_ShadowState.m_TextureStage[lastUsedTextureStage].m_AlphaArg1 =
          D3DTA_TFACTOR;
      m_ShadowState.m_TextureStage[lastUsedTextureStage].m_AlphaArg2 =
          D3DTA_CURRENT;
      m_ShadowState.m_TextureStage[lastUsedTextureStage].m_ColorOp =
          D3DTOP_MODULATE;
      m_ShadowState.m_TextureStage[lastUsedTextureStage].m_AlphaOp =
          D3DTOP_MODULATE;
    }
  }
...
VertexFormat_t CShaderShadowDX8::FlagsToVertexFormat(int flags) const {
  ...
  if (flags == -1) {
    ...
    if (flags & SHADER_DRAW_POSITION) formatFlags |= VERTEX_POSITION;

    if (flags & SHADER_DRAW_NORMAL) formatFlags |= VERTEX_NORMAL;

    if (flags & SHADER_DRAW_COLOR) formatFlags |= VERTEX_COLOR;

    if (flags & SHADER_DRAW_SPECULAR) formatFlags |= VERTEX_SPECULAR;
  ...
  return MeshMgr()->ComputeVertexFormat(formatFlags,
...
void CShaderShadowDX8::ComputeAggregateShadowState() {
  ...
  if (m_ShadowState.m_AlphaTestEnable) {
    ...
    // If not alpha testing and doing a standard alpha blend, force on alpha
    // testing
    if (m_ShadowState.m_AlphaBlendEnable) {
      if ((m_ShadowState.m_SrcBlend == D3DBLEND_SRCALPHA) &&
          (m_ShadowState.m_DestBlend == D3DBLEND_INVSRCALPHA)) {
        m_ShadowState.m_AlphaFunc = D3DCMP_GREATEREQUAL;
  ...
  if (m_ShadowState.m_UsingFixedFunction) {
    ...
    if (HasConstantColor()) flags |= SHADER_HAS_CONSTANT_COLOR;
    ...
    // SHADER_DRAW_TEXCOORD or SHADER_DRAW_SECONDARY_TEXCOORD0 on the same
    // stage)
    if (flags & (SHADER_DRAW_TEXCOORD0 | SHADER_DRAW_SECONDARY_TEXCOORD0)) {
      Assert((m_ShadowState.m_TextureStage[0].m_TexCoordIndex & 0xFFFF0000) ==
             0);
    }
    if (flags & (SHADER_DRAW_TEXCOORD1 | SHADER_DRAW_SECONDARY_TEXCOORD1)) {
      Assert((m_ShadowState.m_TextureStage[1].m_TexCoordIndex & 0xFFFF0000) ==
             0);
    }
    if (flags & (SHADER_DRAW_TEXCOORD2 | SHADER_DRAW_SECONDARY_TEXCOORD2)) {
      Assert((m_ShadowState.m_TextureStage[2].m_TexCoordIndex & 0xFFFF0000) ==
             0);
    }
    if (flags & (SHADER_DRAW_TEXCOORD3 | SHADER_DRAW_SECONDARY_TEXCOORD3)) {
      Assert((m_ShadowState.m_TextureStage[3].m_TexCoordIndex & 0xFFFF0000) ==
             0);
    }
...
//#ifdef _DEBUG
    ...
    // Pixel shaders, disable everything so as to prevent unnecessary state
    // changes....
    for (int i = 0; i < m_pHardwareConfig->GetTextureStageCount(); ++i) {
      m_ShadowState.m_TextureStage[i].m_ColorArg1 = D3DTA_TEXTURE;
      m_ShadowState.m_TextureStage[i].m_ColorArg2 =
  ...
  if (m_ShadowState.m_EnableAlphaToCoverage) {
    // Only allow this to be enabled if blending is disabled and testing is
    // enabled
    if ((m_ShadowState.m_AlphaBlendEnable == true) ||
        (m_ShadowState.m_AlphaTestEnable == false)) {
...
void CShaderShadowDX8::SetDiffuseMaterialSource(
  ...
  m_ShadowState.m_DiffuseMaterialSource =
      (D3DMATERIALCOLORSOURCE)materialSource;
}
