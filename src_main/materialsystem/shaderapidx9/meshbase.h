...
abstract_class CVertexBufferBase : public IVertexBuffer {
 ...
 public:
  ...
  static void ComputeVertexDescription(
      unsigned char *pBuffer, VertexFormat_t vertexFormat, VertexDesc_t &desc);
...
inline void ComputeVertexDesc(unsigned char *pBuffer,
  ...
  // Determine which vertex compression type this format specifies (affects
  // element sizes/decls):
  ...
  int offset = 0;
  // NOTE: At the moment, we assume that if you specify wrinkle, you also
  // specify position
  Assert(((vertexFormat & VERTEX_WRINKLE) == 0) ||
         ((vertexFormat & VERTEX_POSITION) != 0));
  if (vertexFormat & VERTEX_POSITION) {
    // UNDONE: compress position+wrinkle to SHORT4N, and roll the scale into the
    // transform matrices
  ...
  // We assume that if you have any indices/weights, you have exactly two of
  // them
  Assert(((desc.m_NumBoneWeights == 2) &&
          ((vertexFormat & VERTEX_BONE_INDEX) != 0)) ||
  ...
  if (vertexFormat & VERTEX_NORMAL) {
    ...
    // See PackNormal_[SHORT2|UBYTE4|HEND3N] in mathlib.h for the compression
    // algorithm
  ...
  for (i = 0; i < VERTEX_MAX_TEXTURE_COORDINATES; ++i) {
    ...
    if (nSize != 0) {
      ...
      VertexElement_t texCoordElement =
          (VertexElement_t)(texCoordElements[nSize - 1] + i);
  ...
  if (vertexFormat & VERTEX_TANGENT_S) {
    // UNDONE: use normal compression here (use mem_dumpvballocs to see if this
    // uses much memory)
  ...
  Assert(nVertexSizesToSet <
         sizeof(pVertexSizesToSet) / sizeof(pVertexSizesToSet[0]));
