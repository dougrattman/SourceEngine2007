...
void FastCopy(uint8_t *pDest, const uint8_t *pSrc, size_t nBytes) {
  ...
  if ((size_t)pDest % 16 == 0 && (size_t)pSrc % 16 == 0) {
    ...
    for (uint8_t *pLimit = pDest + nBytesFull; pDest < pLimit;
         pDest += BYTES_PER_FULL, pSrc += BYTES_PER_FULL) {
  ...
  if ((size_t)pDest % 4 == 0 && nBytes % 4 == 0) {
    ...
    if ((size_t)pDest % 2 == 0 && nBytes == 4) {
      *(reinterpret_cast<short *>(pDest)) =
          *(reinterpret_cast<const short *>(pSrc));
      *(reinterpret_cast<short *>(pDest) + 1) =
          *(reinterpret_cast<const short *>(pSrc) + 1);
    } else {
      memcpy(pDest, pSrc, nBytes);
    }
  }
...
class CMatQueuedMesh : public IMesh {
 public:
  CMatQueuedMesh(CMatQueuedRenderContext *pOwner,
  ...
  CLateBoundPtr<IMesh> &AccessLateBoundMesh() { return m_pLateBoundMesh; }

  uint8_t *GetVertexData() { return m_pVertexData; }
  uint16_t *GetIndexData() { return m_pIndexData; }
  IMesh *DetachActualMesh() {
  ...
  void DeferredGetDynamicMesh(VertexFormat_t vertexFormat, unsigned flags,
                              IMesh *pVertexOverride, IMesh *pIndexOverride,
                              IMaterialInternal *pMaterial) {
    if (!(flags & MQM_FLEX)) {
      if (vertexFormat == 0) {
        m_pActualMesh = m_pHardwareContext->GetDynamicMesh(
            ((flags & MQM_BUFFERED) != 0), pVertexOverride, pIndexOverride,
            pMaterial);
      } else {
        m_pActualMesh = m_pHardwareContext->GetDynamicMeshEx(
            vertexFormat, ((flags & MQM_BUFFERED) != 0), pVertexOverride,
            pIndexOverride, pMaterial);
      }
    } else {
  ...
  bool OnGetDynamicMesh(VertexFormat_t vertexFormat, unsigned flags,
                        IMesh *pVertexOverride, IMesh *pIndexOverride,
                        IMaterialInternal *pMaterial, int nHWSkinBoneCount) {
    ...
    if (!(flags & MQM_FLEX)) {
      if (pVertexOverride) {
        ...
        m_VertexFormat |= VERTEX_BONEWEIGHT((VertexFormat_t)nHWSkinBoneCount);
      ...
      m_VertexFormat =
          VERTEX_POSITION | VERTEX_NORMAL | VERTEX_FORMAT_USE_EXACT_FORMAT;
    ...
    m_pCallQueue->QueueCall(this, &CMatQueuedMesh::DeferredGetDynamicMesh,
                            vertexFormat, flags, pVertexOverride,
                            pIndexOverride, pMaterial);
    return true;
  }
  ...
  int VertexCount() const { return m_VertexSize ? m_nVerts : 0; }

  int IndexCount() const { return m_nIndices; }
  ...
  void SetColorMesh(IMesh *pColorMesh, int nVertexOffset) {
    m_pCallQueue->QueueCall(m_pLateBoundMesh, &IMesh::SetColorMesh, pColorMesh,
                            nVertexOffset);
  }

  void Draw(CPrimList *pLists, int nLists) { CannotSupport(); }

  void CopyToMeshBuilder(int iStartVert, int nVerts, int iStartIndex,
                         int nIndices, int indexOffset, CMeshBuilder &builder) {
    CannotSupport();
  }
  ...
  void LockMesh(int numVerts, int numIndices, MeshDesc_t &desc) {
    ...
    if (numVerts > 0) {
      ...
      m_pVertexData =
          (uint8_t *)m_pOwner->AllocVertices(numVerts, m_VertexSize);
      ...
      g_pShaderAPI->ComputeVertexDescription(m_pVertexData, m_VertexFormat,
                                             desc);
    } else {
      desc.m_nFirstVertex = 0;
      static_cast<VertexDesc_t *>(&desc)->m_nOffset = 0;
  ...
  void UnlockMesh(int numVerts, int numIndices, MeshDesc_t &desc) {
    if (m_pVertexData && numVerts < m_nVerts) {
      m_pVertexData = m_pOwner->ReallocVertices(m_pVertexData, m_nVerts,
    ...
    if (m_pIndexData && numIndices < m_nIndices) {
      m_pIndexData =
          m_pOwner->ReallocIndices(m_pIndexData, m_nIndices, numIndices);
    }
    m_nIndices = numIndices;
  }

  void SetFlexMesh(IMesh *pMesh, int nVertexOffset) {
    m_pCallQueue->QueueCall(m_pLateBoundMesh, &IMesh::SetFlexMesh, pMesh,
                            nVertexOffset);
  }
  ...
  void ExecuteDefferredBuild(uint8_t *pVertexData, int nVerts, int nBytesVerts,
                             uint16_t *pIndexData, int nIndices) {
    Assert(m_pActualMesh);
    MeshDesc_t desc;
    m_pActualMesh->LockMesh(nVerts, nIndices, desc);
    m_nActualVertexOffsetInBytes =
        desc.m_nFirstVertex * desc.m_ActualVertexSize;
    if (pVertexData &&
        desc.m_ActualVertexSize)  // if !desc.m_ActualVertexSize, device lost
    {
      void *pDest;
...
#undef FindMin
      ...
      Assert(pDest);
      if (pDest) {
        FastCopy((uint8_t *)pDest, pVertexData, nBytesVerts);
      }
    }
    ...
    if (pIndexData && pIndexData != &gm_ScratchIndexBuffer &&
      ...
      if (!desc.m_nFirstVertex) {
        // AssertMsg(desc.m_pIndices & 0x03 == 0,"desc.m_pIndices is misaligned
        // in CMatQueuedMesh::ExecuteDefferedBuild\n");
        ...
        while (i < nIndices) {
          ...
          FastCopy((uint8_t *)(desc.m_pIndices + i), (uint8_t *)tempIndices,
                   nToCopy * sizeof(uint16_t));
          i += nToCopy;
        }
      }
    }
  ...
  void QueueBuild(bool bDetachBuffers = true) {
    m_pCallQueue->QueueCall(this, &CMatQueuedMesh::ExecuteDefferredBuild,
                            m_pVertexData, m_nVerts, m_nVerts * m_VertexSize,
                            m_pIndexData, m_nIndices);
    if (bDetachBuffers) {
      DetachBuffers();
      m_Type = MATERIAL_TRIANGLES;
    }
  ...
  void Draw(int firstIndex = -1, int numIndices = 0) {
    ...
    void (IMesh::*pfnDraw)(int, int) =
        &IMesh::Draw;  // need assignment to disambiguate overloaded function
    bool bDetachBuffers;
    if (firstIndex == -1 || numIndices == 0) {
      bDetachBuffers = true;
    } else if (m_pIndexOverride) {
      bDetachBuffers =
          (firstIndex + numIndices == m_pIndexOverride->IndexCount());
  ...
  void ModifyEnd(IndexDesc_t &desc) { CannotSupport(); }
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::Init(CMaterialSystem *pMaterialSystem,
...
void CMatQueuedRenderContext::Bind(IMaterial *iMaterial, void *proxyData) {
  if (!iMaterial) {
    ...
    iMaterial = ((IMaterialInternal *)iMaterial)
  ...
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::Bind, iMaterial,
                    proxyData);
}
...
void CMatQueuedRenderContext::CallQueued(bool bTermAfterCall) {
  if (mat_report_queue_status.GetBool()) {
    Msg("%d calls queued for %d bytes in parameters and overhead, %d bytes "
        "verts, %d bytes indices, %d bytes primitives\n",
        m_queue.Count(), m_queue.GetMemoryUsed(), m_Vertices.GetUsed(),
        m_Indices.GetUsed(), m_Primitives.GetUsed());
  }
  ...
  if (bTermAfterCall) Term();
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::FlushQueued() { m_queue.Flush(); }
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::SetRenderTargetEx(int nRenderTargetID,
                                                ITexture *pNewTarget) {
  CMatRenderContextBase::SetRenderTargetEx(nRenderTargetID, pNewTarget);

  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::SetRenderTargetEx,
                    nRenderTargetID, pNewTarget);
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::GetRenderTargetDimensions(int &width,
                                                        int &height) const {
...
void CMatQueuedRenderContext::Viewport(int x, int y, int width, int height) {
  ...
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::Viewport, x, y,
                    width, height);
}
...
void CMatQueuedRenderContext::SetLight(int i, const LightDesc_t &desc) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::SetLight, i,
                    RefToVal(desc));
}
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::SetLightingOrigin(const Vector &vLightingOrigin) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::SetLightingOrigin,
                    vLightingOrigin);
}
...
void CMatQueuedRenderContext::SetNumBoneWeights(int nBoneCount) {
  ...
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::SetNumBoneWeights,
                    nBoneCount);
}

int CMatQueuedRenderContext::GetCurrentNumBones() const { return m_nBoneCount; }
...
void CMatQueuedRenderContext::FogMaxDensity(float flMaxDensity) {
  ...
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::FogMaxDensity,
                    flMaxDensity);
}
...
void CMatQueuedRenderContext::FogColor3ub(unsigned char r, unsigned char g,
  ...
  m_FogColor.g = g;
  m_FogColor.b = b;
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::FogColor3ub, r, g,
                    b);
}
...
void CMatQueuedRenderContext::GetFogDistances(float *fStart, float *fEnd,
                                              float *fFogZ) {
  if (fStart) *fStart = m_flFogStart;

  if (fEnd) *fEnd = m_flFogEnd;

  if (fFogZ) *fFogZ = m_flFogZ;
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::GetViewport(int &x, int &y, int &width,
                                          int &height) const {
...
void CMatQueuedRenderContext::SyncToken(const char *p) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::SyncToken,
                    CUtlEnvelope<const char *>(p));
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
IMesh *CMatQueuedRenderContext::GetDynamicMesh(bool buffered,
  ...
  m_pQueuedMesh->OnGetDynamicMesh(
      0, (buffered) ? MQM_BUFFERED : 0, pVertexOverride, pIndexOverride,
      GetCurrentMaterialInternal(), nCurrentBoneCount);
  return m_pQueuedMesh;
}

IMesh *CMatQueuedRenderContext::GetDynamicMeshEx(VertexFormat_t vertexFormat,
                                                 bool bBuffered,
                                                 IMesh *pVertexOverride,
                                                 IMesh *pIndexOverride,
                                                 IMaterial *pAutoBind) {
  ...
  m_pQueuedMesh->OnGetDynamicMesh(
      vertexFormat, (bBuffered) ? MQM_BUFFERED : 0, pVertexOverride,
      pIndexOverride, GetCurrentMaterialInternal(), nCurrentBoneCount);
  return m_pQueuedMesh;
}
...
int CMatQueuedRenderContext::GetMaxVerticesToRender(IMaterial *pMaterial) {
  ...
  int maxVerts =
      g_pShaderAPI->GetCurrentDynamicVBSize() / temp.m_ActualVertexSize;
  if (maxVerts > 65535) {
    maxVerts = 65535;
  }
  return maxVerts;
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::GetMaxToRender(IMesh *pMesh, bool bMaxUntilFlush,
                                             int *pMaxVerts, int *pMaxIndices) {
  Assert(!bMaxUntilFlush);
  *pMaxVerts =
      g_pShaderAPI->GetCurrentDynamicVBSize() / m_pQueuedMesh->GetVertexSize();
  if (*pMaxVerts > 65535) {
    *pMaxVerts = 65535;
  }
...
CMatQueuedRenderContext::CreateOcclusionQueryObject() {
  OcclusionQueryObjectHandle_t h =
      g_pOcclusionQueryMgr->CreateOcclusionQueryObject();
  m_queue.QueueCall(g_pOcclusionQueryMgr,
                    &COcclusionQueryMgr::OnCreateOcclusionQueryObject, h);
  return h;
}

int CMatQueuedRenderContext::OcclusionQuery_GetNumPixelsRendered(
    OcclusionQueryObjectHandle_t h) {
  m_queue.QueueCall(
      g_pOcclusionQueryMgr,
      &COcclusionQueryMgr::OcclusionQuery_IssueNumPixelsRenderedQuery, h);
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::SetFlashlightState(const FlashlightState_t &s,
                                                 const VMatrix &m) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::SetFlashlightState,
                    RefToVal(s), RefToVal(m));
}
...
bool CMatQueuedRenderContext::EnableClipping(bool bEnable) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::EnableClipping,
                    bEnable);
...
void CMatQueuedRenderContext::UserClipTransform(const VMatrix &m) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::UserClipTransform,
                    RefToVal(m));
}
...
{
  IMaterial *pRealTimeVersionMaterial =
  ...
  m_queue.QueueCall(m_pHardwareContext,
                    &IMatRenderContext::DrawScreenSpaceRectangle, pMaterial,
                    destx, desty, width, height, src_texture_x0, src_texture_y0,
                    src_texture_x1, src_texture_y1, src_texture_width,
                    src_texture_height, pClientRenderable, nXDice, nYDice);
}
...
void CMatQueuedRenderContext::LoadBoneMatrix(int i, const matrix3x4_t &m) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::LoadBoneMatrix, i,
                    RefToVal(m));
}
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::CopyRenderTargetToTextureEx(ITexture *pTexture,
                                                          int i, Rect_t *pSrc,
                                                          Rect_t *pDst) {
  m_queue.QueueCall(m_pHardwareContext,
                    &IMatRenderContext::CopyRenderTargetToTextureEx, pTexture,
                    i, CUtlEnvelope<Rect_t>(pSrc), CUtlEnvelope<Rect_t>(pDst));
}

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
bool CMatQueuedRenderContext::OnDrawMesh(IMesh *pMesh, int firstIndex,
                                         int numIndices) {
  void (IMesh::*pfnDraw)(int, int) =
      &IMesh::Draw;  // need assignment to disambiguate overloaded function
...
//
//-----------------------------------------------------------------------------
bool CMatQueuedRenderContext::OnDrawMesh(IMesh *pMesh, CPrimList *pLists,
                                         int nLists) {
  CPrimList *pCopyPrimList = AllocPrimLists(nLists);
  memcpy(pCopyPrimList, pLists, sizeof(CPrimList) * nLists);
  m_queue.QueueCall(this, &CMatQueuedRenderContext::DeferredDrawPrimList, pMesh,
                    pCopyPrimList, nLists);
  return false;
}

void CMatQueuedRenderContext::DeferredDrawPrimList(IMesh *pMesh,
                                                   CPrimList *pLists,
                                                   int nLists) {
...
//
//-----------------------------------------------------------------------------
void CMatQueuedRenderContext::DeferredSetFlexMesh(IMesh *pStaticMesh,
                                                  int nVertexOffsetInBytes) {
  pStaticMesh->SetFlexMesh(m_pQueuedMesh->GetActualMesh(),
                           m_pQueuedMesh->GetActualVertexOffsetInBytes());
}

bool CMatQueuedRenderContext::OnSetFlexMesh(IMesh *pStaticMesh, IMesh *pMesh,
                                            int nVertexOffsetInBytes) {
  Assert(pMesh == m_pQueuedMesh || !pMesh);
  if (pMesh) {
    m_pQueuedMesh->QueueBuild();
    m_queue.QueueCall(this, &CMatQueuedRenderContext::DeferredSetFlexMesh,
                      pStaticMesh, nVertexOffsetInBytes);
  } else {
...
//
//-----------------------------------------------------------------------------
bool CMatQueuedRenderContext::OnSetColorMesh(IMesh *pStaticMesh, IMesh *pMesh,
                                             int nVertexOffsetInBytes) {
  m_queue.QueueCall(pStaticMesh, &IMesh::SetColorMesh, pMesh,
...
//
//-----------------------------------------------------------------------------
bool CMatQueuedRenderContext::OnSetPrimitiveType(IMesh *pMesh,
...
inline void CMatQueuedRenderContext::QueueMatrixSync() {
  void (IMatRenderContext::*pfnLoadMatrix)(const VMatrix &) =
      &IMatRenderContext::LoadMatrix;  // need assignment to disambiguate
                                       // overloaded function
  m_queue.QueueCall(m_pHardwareContext, pfnLoadMatrix,
                    RefToVal(AccessCurrentMatrix()));
}
...
void CMatQueuedRenderContext::Ortho(double left, double top, double right,
                                    double bottom, double zNear, double zFar) {
  CMatRenderContextBase::Ortho(left, top, right, bottom, zNear, zFar);
  QueueMatrixSync();
}

void CMatQueuedRenderContext::PerspectiveX(double flFovX, double flAspect,
                                           double flZNear, double flZFar) {
  CMatRenderContextBase::PerspectiveX(flFovX, flAspect, flZNear, flZFar);
  QueueMatrixSync();
}

void CMatQueuedRenderContext::PerspectiveOffCenterX(
    double flFovX, double flAspect, double flZNear, double flZFar,
...
void CMatQueuedRenderContext::PickMatrix(int x, int y, int nWidth,
                                         int nHeight) {
...
void CMatQueuedRenderContext::BeginBatch(IMesh *pIndices) {
  ...
  m_queue.QueueCall(this, &CMatQueuedRenderContext::DeferredBeginBatch,
                    m_pQueuedMesh->GetIndexData(), m_pQueuedMesh->IndexCount());
  m_pQueuedMesh->DetachBuffers();
}

void CMatQueuedRenderContext::BindBatch(IMesh *pVertices,
                                        IMaterial *pAutoBind) {
  Assert(pVertices != (IMesh *)m_pQueuedMesh);
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::BindBatch,
                    pVertices, pAutoBind);
}

void CMatQueuedRenderContext::DrawBatch(int firstIndex, int numIndices) {
  m_queue.QueueCall(m_pHardwareContext, &IMatRenderContext::DrawBatch,
...
void CMatQueuedRenderContext::DeferredBeginBatch(uint16_t *pIndexData,
                                                 int nIndices) {
  m_pQueuedMesh->DeferredGetDynamicMesh(0, 0, NULL, NULL, NULL);
...
//-----------------------------------------------------------------------------
uint8_t *CMatQueuedRenderContext::AllocVertices(int nVerts, int nVertexSize) {
  MEM_ALLOC_CREDIT();
  return (uint8_t *)m_Vertices.Alloc(nVerts * nVertexSize, false);
}

uint16_t *CMatQueuedRenderContext::AllocIndices(int nIndices) {
  MEM_ALLOC_CREDIT();
  return (uint16_t *)m_Indices.Alloc(nIndices * sizeof(uint16_t), false);
}

uint8_t *CMatQueuedRenderContext::ReallocVertices(uint8_t *pVerts,
                                                  int nVertsOld, int nVertsNew,
                                                  int nVertexSize) {
  Assert(nVertsNew <= nVertsOld);

  if (nVertsNew < nVertsOld) {
    unsigned nBytes = ((nVertsOld - nVertsNew) * nVertexSize);
    m_Vertices.FreeToAllocPoint(
        AlignValue(m_Vertices.GetCurrentAllocPoint() - nBytes, 16),
        false);  // memstacks 128 bit aligned
  }
  return pVerts;
}

uint16_t *CMatQueuedRenderContext::ReallocIndices(uint16_t *pIndices,
                                                  int nIndicesOld,
                                                  int nIndicesNew) {
  Assert(nIndicesNew <= nIndicesOld);
  if (nIndicesNew < nIndicesOld) {
    unsigned nBytes = ((nIndicesOld - nIndicesNew) * sizeof(uint16_t));
    m_Indices.FreeToAllocPoint(
        AlignValue(m_Indices.GetCurrentAllocPoint() - nBytes, 16),
        false);  // memstacks 128 bit aligned
  }
  return pIndices;
}

void CMatQueuedRenderContext::FreeVertices(uint8_t *pVerts, int nVerts,
                                           int nVertexSize) {
  // free at end of call dispatch
}

void CMatQueuedRenderContext::FreeIndices(uint16_t *pIndices, int nIndices) {
  // free at end of call dispatch
}
...
void CMatQueuedRenderContext::LoadLookup(ColorCorrectionHandle_t handle,
