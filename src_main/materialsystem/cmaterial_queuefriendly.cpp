...
#define USE_QUEUED_MATERIAL_CALLS  // uncomment to queue up material changing
                                   // calls. Comment out to always use instant
                                   // calls.
...
#define QUEUE_MATERIAL_CALL(FuncName, ...)                                    \
  {                                                                           \
    ...
    if (!pCallQueue) {                                                        \
      ...
      pCallQueue->QueueCall(m_pRealTimeVersion, &IMaterialInternal::FuncName, \
                            ##__VA_ARGS__);                                   \
    }                                                                         \
  }
...
PreviewImageRetVal_t CMaterial_QueueFriendly::GetPreviewImageProperties(
    int *width, int *height, ImageFormat *imageFormat,
    bool *isTranslucent) const {
  return m_pRealTimeVersion->GetPreviewImageProperties(
      width, height, imageFormat, isTranslucent);
}

PreviewImageRetVal_t CMaterial_QueueFriendly::GetPreviewImage(
    unsigned char *data, int width, int height, ImageFormat imageFormat) const {
...
void CMaterial_QueueFriendly::GetLowResColorSample(float s, float t,
                                                   float *color) const {
  m_pRealTimeVersion->GetLowResColorSample(s, t, color);
}

IMaterialVar *CMaterial_QueueFriendly::FindVar(const char *varName, bool *found,
                                               bool complain) {
  // TODO: return a queue friendly variable that can be get/set
  return m_pRealTimeVersion->FindVar(varName, found, complain);
}

IMaterialVar *CMaterial_QueueFriendly::FindVarFast(char const *pVarName,
                                                   unsigned int *pToken) {
  // TODO: return a queue friendly variable that can be get/set
...
bool CMaterial_QueueFriendly::IsTranslucent() {
  ...
  return m_pRealTimeVersion->IsTranslucentInternal(
      m_fAlphaModulationOnQueueCompletion);
}

bool CMaterial_QueueFriendly::NeedsPowerOfTwoFrameBufferTexture(
    bool bCheckSpecificToThisFrame) {
  // bCheckSpecificToThisFrame scares me a bit.
  return m_pRealTimeVersion->NeedsPowerOfTwoFrameBufferTexture(
      bCheckSpecificToThisFrame);
}

bool CMaterial_QueueFriendly::NeedsFullFrameBufferTexture(
    bool bCheckSpecificToThisFrame) {
  // bCheckSpecificToThisFrame scares me a bit.
  return m_pRealTimeVersion->NeedsFullFrameBufferTexture(
      bCheckSpecificToThisFrame);
}
...
void CMaterial_QueueFriendly::SetMaterialVarFlag(MaterialVarFlags_t flag,
                                                 bool on) {
  QUEUE_MATERIAL_CALL(SetMaterialVarFlag, flag, on);
}
bool CMaterial_QueueFriendly::GetMaterialVarFlag(
    MaterialVarFlags_t flag) const {
...
const char *CMaterial_QueueFriendly::GetShaderName() const {
  // TODO: return as if the queue is up to date. Someone could have set the
  // shader very recently
...
void CMaterial_QueueFriendly::UpdateToRealTime(void) {
  m_fAlphaModulationOnQueueCompletion =
      m_pRealTimeVersion->GetAlphaModulation();
  m_pRealTimeVersion->GetColorModulation(
      &m_vColorModulationOnQueueCompletion.x,
