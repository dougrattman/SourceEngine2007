...
BEGIN_SHADER(LightmappedGeneric_DX6, "Help for LightmappedGeneric_DX6")
...
SHADER_PARAM(ENVMAPOPTIONAL, SHADER_PARAM_TYPE_BOOL, "90",
...
int GetDrawFlagsPass1(IMaterialVar** params) {
  ...
  if (params[BASETEXTURE]->IsTexture()) flags |= SHADER_DRAW_TEXCOORD1;
  return flags;
}

void DrawLightmapOnly(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
...
void DrawBaseTimesLightmap(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                           IShaderShadow* pShaderShadow) {
  // Base times lightmap
  SHADOW_STATE {
    // alpha test
...
//-----------------------------------------------------------------------------
BEGIN_INHERITED_SHADER(Water_DX60, LightmappedGeneric_DX6,
                       "Help for Water_DX60")
