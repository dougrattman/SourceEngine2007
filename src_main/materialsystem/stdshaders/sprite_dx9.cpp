...
BEGIN_VS_SHADER(Sprite_DX9, "Help for Sprite_DX9")
...
BEGIN_SHADER_PARAMS
  ...
  SHADER_PARAM(NOSRGB, SHADER_PARAM_TYPE_BOOL, "0",
               "do not operate in srgb space")
  SHADER_PARAM(HDRCOLORSCALE, SHADER_PARAM_TYPE_FLOAT, "1.0", "hdr color scale")
END_SHADER_PARAMS

SHADER_FALLBACK {
  if (g_pHardwareConfig->GetDXSupportLevel() < 90) return "Sprite_DX8";
  return 0;
}
SHADER_INIT_PARAMS() {
...
void SetSpriteCommonShadowState(unsigned int shaderFlags) {
  ...
  SET_STATIC_VERTEX_SHADER_COMBO(
      VERTEXCOLOR, (shaderFlags & SHADER_USE_VERTEX_COLOR) ? true : false);
      ...
      DYNAMIC_STATE {
        ...
        if (bSRGB)
          color[0] = color[1] = color[2] =
              GammaToLinear(flFade * frameBlendAlpha);
        else
