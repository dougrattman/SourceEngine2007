...
int GetDefaultDepthFeatheringValue(void)  // Allow the command-line to go
                                          // against the default soft-particle
                                          // value
{
...
#else
BEGIN_VS_SHADER_FLAGS(Spritecard_DX8, "Help for Spritecard_DX8",
                      SHADER_NOT_EDITABLE)
#endif
...
SHADER_FALLBACK {
#ifdef STDSHADER_DX9_DLL_EXPORT
  if (g_pHardwareConfig->GetDXSupportLevel() < 90) return "SpriteCard_DX8";
#endif
#ifdef STDSHADER_DX8_DLL_EXPORT
  // STDSHADER_DX8_DLL_EXPORT
  if (g_pHardwareConfig->GetDXSupportLevel() < 80) return "Wireframe";
#endif
  return 0;
}
...
SHADER_DRAW {
  ...
  bool bZoomSeq2 =
      (!bDX8) && ((params[ZOOMANIMATESEQ2]->GetFloatValue()) > 1.0);
  ...
  SHADOW_STATE {
    ...
    if (nSplineType) bUseInstancing = false;
    ...
    if (bDX8) pShaderShadow->EnableTexture(SHADER_SAMPLER1, true);
    ...
    if (bSecondSequence) {
      // the whole shebang - 2 sequences, with a possible multi-image sequence
      // first
      numTexCoords = 8;
    }
    ...
    if (bDX8) {
      ...
      if (nSplineType) {
        ...
        if (bSecondSequence) bAdditive2ndTexture = false;
      ...
      if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
        ...
        SET_STATIC_PIXEL_SHADER_COMBO(MAXLUMFRAMEBLEND1,
      ...
      if (!bDX8) pShaderShadow->EnableSRGBWrite(true);
  ...
  DYNAMIC_STATE {
    ...
    float VC0[8] = {params[MINSIZE]->GetFloatValue(),
                    ...
                    params[STARTFADESIZE]->GetFloatValue(),
                    params[ENDFADESIZE]->GetFloatValue(),
                    flStartFade,
                    1.0f / (flMaxDistance - flStartFade),
                    0,
                    0};
    ...
    if (!bDX8) {
      BOOL bShaderConstants[3] = {bZoomSeq2, bExtractGreenAlpha,
                                  bUseInstancing};
      pShaderAPI->SetBooleanVertexShaderConstant(
