...
BEGIN_SHADER_PARAMS
SHADER_PARAM(BUMPMAP, SHADER_PARAM_TYPE_TEXTURE, "cable/cablenormalmap",
             "bumpmap texture")
SHADER_PARAM(MINLIGHT, SHADER_PARAM_TYPE_FLOAT, "0.1",
             "Minimum amount of light (0-1 value)")
SHADER_PARAM(MAXLIGHT, SHADER_PARAM_TYPE_FLOAT, "0.3",
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    pShaderShadow->VertexShaderVertexFormat(
        VERTEX_POSITION | VERTEX_COLOR | VERTEX_TANGENT_S | VERTEX_TANGENT_T, 2,
        tCoordDimensions, 0);
    ...
    // This is kinda wrong.  We are writing linear or gamma depending on
    // "IsHDREnabled" below. The COLOR really decides if we are gamma or linear.
    if (g_pHardwareConfig->GetDXSupportLevel() >= 90) {
  ...
  DYNAMIC_STATE {
    ...
    if (g_pHardwareConfig->GetDXSupportLevel() >= 90) {
      SetVertexShaderConstantGammaToLinear(
          VERTEX_SHADER_SHADER_SPECIFIC_CONST_0, lightDir);
    } else {
