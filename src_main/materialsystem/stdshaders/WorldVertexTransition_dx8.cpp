...
BEGIN_VS_SHADER(WorldVertexTransition_DX8, "Help for WorldVertexTransition_DX8")

BEGIN_SHADER_PARAMS
  SHADER_PARAM(BASETEXTURE2, SHADER_PARAM_TYPE_TEXTURE,
               "shadertest/BaseTexture2", "base texture2 help")
  SHADER_PARAM(FRAME2, SHADER_PARAM_TYPE_INTEGER, "0",
               "frame number for $baseTexture")
  SHADER_PARAM(BASETEXTURETRANSFORM2, SHADER_PARAM_TYPE_MATRIX,
               "center .5 .5 scale 1 1 rotate 0 translate 0 0",
               "$baseTexture texcoord transform")
  ...
  SHADER_PARAM(BUMPMAP, SHADER_PARAM_TYPE_TEXTURE,
               "models/shadertest/shader1_normal", "bump map for BASETEXTURE")
  ...
  SHADER_PARAM(FRESNELREFLECTION, SHADER_PARAM_TYPE_FLOAT, "0.0",
  ...
  SHADER_PARAM(SEAMLESS_SCALE, SHADER_PARAM_TYPE_FLOAT, "0",
               ...
               "ordinary mapping")
  SHADER_PARAM(BLENDMODULATETEXTURE, SHADER_PARAM_TYPE_TEXTURE, "",
               "texture to use r/g channels for blend range for")
  SHADER_PARAM(BLENDMASKTRANSFORM, SHADER_PARAM_TYPE_MATRIX,
               "center .5 .5 scale 1 1 rotate 0 translate 0 0",
...
SHADER_INIT_PARAMS() {
  ...
  if (params[SEAMLESS_SCALE]->IsDefined() &&
    ...
    // seamless scale is going to be used, so kill some other features. . might
    // implement with these features later.
...
SHADER_INIT {
  ...
  if (g_pHardwareConfig->SupportsPixelShaders_1_4() &&
      params[BLENDMODULATETEXTURE]->IsDefined()) {
  ...
  if (params[ENVMAP]->IsDefined() && !params[BUMPMAP]->IsDefined()) {
    Warning(
        "must have $bumpmap if you have $envmap for worldvertextransition\n");
    params[ENVMAP]->SetUndefined();
    params[BUMPMAP]->SetUndefined();
  }
  if (g_pConfig->UseBumpmapping() && params[BUMPMAP]->IsDefined()) {
...
void WriteVertexAlphaToDestAlpha(IMaterialVar** params,
  ...
  if (pShaderShadow) {
    ...
    pShaderShadow->SetVertexShader("writevertexalphatodestalpha_vs11",
                                   vshIndex.GetIndex());

    pShaderShadow->SetPixelShader("writevertexalphatodestalpha_ps11");
    pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_COLOR, 2,
                                            0, 0);
  } else {
...
void DrawFlashlightPass(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                        IShaderShadow* pShaderShadow, int passID) {
  bool bBump = (passID == 0) && ShouldUseBumpmapping(params) &&
  ...
  DrawFlashlight_dx80(params, pShaderAPI, pShaderShadow, bBump, BUMPMAP,
                      ...
                      FLASHLIGHTTEXTUREFRAME, true, true, passID, BASETEXTURE2,
                      FRAME2);
}
...
void DrawFlashlight(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                    IShaderShadow* pShaderShadow) {
  ...
  if (hasFlashlight) {
    ...
    SHADOW_STATE {
      ...
      if (!bSupports14) {
        ...
        pShaderShadow->SetVertexShader("WorldVertexTransition_vs14",
                                       vshIndex.GetIndex());
    ...
    DYNAMIC_STATE {
      ...
      // you'll always have a detailscale. go ahead and set this even if we
      // don't have a detail texture so the vertex shader doesn't barf chunks
      // with unitialized data.
      SetVertexShaderTextureScaledTransform(
          VERTEX_SHADER_SHADER_SPECIFIC_CONST_4, BASETEXTURETRANSFORM,
          DETAILSCALE);
    ...
    if (params[BUMPBASETEXTURE2WITHBUMPMAP]->GetIntValue()) {
      DrawWorldBumpedUsingVertexShader(
          ...
          true, BASETEXTURE2, BASETEXTURETRANSFORM2, FRAME2, false);
    } else {
