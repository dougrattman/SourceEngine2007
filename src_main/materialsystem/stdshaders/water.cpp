...
SHADER_PARAM(NORMALMAP, SHADER_PARAM_TYPE_TEXTURE, "dev/water_normal",
             "normal map")
...
SHADER_PARAM(CHEAPWATERSTARTDISTANCE, SHADER_PARAM_TYPE_FLOAT, "",
             "This is the distance from the eye in inches that the shader "
             "should start transitioning to a cheaper water shader.")
SHADER_PARAM(CHEAPWATERENDDISTANCE, SHADER_PARAM_TYPE_FLOAT, "",
             "This is the distance from the eye in inches that the shader "
             "should finish transitioning to a cheaper water shader.")
...
SHADER_PARAM(BLURREFRACT, SHADER_PARAM_TYPE_BOOL, "0",
             "Cause the refraction to be blurry on ps2b hardware")
END_SHADER_PARAMS
...
SHADER_INIT_PARAMS() {
  ...
  if (params[FORCEEXPENSIVE]->GetIntValue() &&
      params[FORCECHEAP]->GetIntValue()) {
    params[FORCEEXPENSIVE]->SetIntValue(0);
  }
...
inline void DrawReflectionRefraction(IMaterialVar** params,
                                     IShaderShadow* pShaderShadow,
                                     IShaderDynamicAPI* pShaderAPI,
                                     bool bReflection, bool bRefraction) {
  SHADOW_STATE {
    ...
    if (params[NORMALMAP]->IsTexture() &&
        g_pHardwareConfig->SupportsNormalMapCompression()) {
      ITexture* pNormalMap = params[NORMALMAP]->GetTextureValue();
      if (pNormalMap) {
        // Clamp this to 0 or 1 since that's how we've authored the water shader
        // (i.e. no separate alpha map/channel)
        nNormalDecodeMode =
            pNormalMap->GetNormalDecodeMode() == NORMAL_DECODE_NONE
                ? NORMAL_DECODE_NONE
                : NORMAL_DECODE_ATI2N;
      }
    }
    ...
    SET_STATIC_VERTEX_SHADER_COMBO(BASETEXTURE,
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(ABOVEWATER,
                                    params[ABOVEWATER]->GetIntValue());
      SET_STATIC_PIXEL_SHADER_COMBO(MULTITEXTURE, fabs(Scroll1.x) > 0.0);
      SET_STATIC_PIXEL_SHADER_COMBO(BASETEXTURE,
                                    params[BASETEXTURE]->IsTexture());
      SET_STATIC_PIXEL_SHADER_COMBO(BLURRY_REFRACT,
                                    params[BLURREFRACT]->GetIntValue());
  ...
  DYNAMIC_STATE {
    ...
    if (bRefraction) {
      // HDRFIXME: add comment about binding.. Specify the number of MRTs in the
      // enable
    ...
    if (bReflection) {
      if (g_pHardwareConfig->GetHDRType() == HDR_TYPE_INTEGER) {
        ...
        // the render target by .25 to get overbright data in the reflection
        // render target.
    ...
    float c5[4] = {params[REFLECTAMOUNT]->GetFloatValue(),
                   params[REFLECTAMOUNT]->GetFloatValue(),
...
inline void DrawCheapWater(IMaterialVar** params, IShaderShadow* pShaderShadow,
  ...
  SHADOW_STATE {
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
      ...
      SET_STATIC_PIXEL_SHADER_COMBO(FRESNEL,
  ...
  DYNAMIC_STATE {
    ...
    float cheapWaterStartDistance =
        params[CHEAPWATERSTARTDISTANCE]->GetFloatValue();
    float cheapWaterEndDistance =
        params[CHEAPWATERENDDISTANCE]->GetFloatValue();
    float cheapWaterParams[4] = {
        ...
        cheapWaterStartDistance /
            (cheapWaterEndDistance - cheapWaterStartDistance),
    };
    ...
    if (g_pConfig->bShowSpecular) {
      ...
      float zero[4] = {0.0f, 0.0f, 0.0f,
                       params[REFLECTBLENDFACTOR]->GetFloatValue()};
...
SHADER_DRAW {
  // TODO: fit the cheap water stuff into the water shader so that we don't have
  // to do 2 passes.
#ifdef _X360
...
#endif
  bool bForceCheap =
      (params[FORCECHEAP]->GetIntValue() != 0) || UsingEditor(params);
  if (bForceCheap) {
    bForceExpensive = false;
  } else {
    bForceExpensive =
        bForceExpensive || (params[FORCEEXPENSIVE]->GetIntValue() != 0);
  }
  ...
  if (!bForceCheap && (bReflection || bRefraction)) {
    bDrewSomething = true;
    DrawReflectionRefraction(params, pShaderShadow, pShaderAPI, bReflection,
                             bRefraction);
  }
...
#ifdef _X360
  if (params[ENVMAP]->IsTexture() && !IS_FLAG_SET(MATERIAL_VAR_DECAL) &&
      !bForceExpensive)
#else
