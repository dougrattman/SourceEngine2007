...
void InitParamsEyes_DX8_DX9(CBaseVSShader *pShader, IMaterialVar **params,
...
void InitEyes_DX8_DX9(CBaseVSShader *pShader, IMaterialVar **params,
                      Eyes_DX8_DX9_Vars_t &info) {
...
#ifdef STDSHADER_DX9_DLL_EXPORT
static void SetDepthFlashlightParams(CBaseVSShader *pShader,
                                     IShaderDynamicAPI *pShaderAPI,
                                     const VMatrix &worldToTexture,
                                     const FlashlightState_t &flashlightState) {
  float atten[4], pos[4], tweaks[4];
  atten[0] = flashlightState
  ...
  pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_POSITION_RIM_BOOST, pos,
                                     1);

  pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_TO_WORLD_TEXTURE,
                                     worldToTexture.Base(), 4);

  // Tweaks associated with a given flashlight
  tweaks[0] = ShadowFilterFromState(flashlightState);
  tweaks[1] = ShadowAttenFromState(flashlightState);
  pShader->HashShadow2DJitter(flashlightState.m_flShadowJitterSeed, &tweaks[2],
                              &tweaks[3]);
...
static void DrawFlashlight(bool bDX9, CBaseVSShader *pShader,
  ...
  if (pShaderShadow) {
    ...
    pShader->EnableAlphaBlending(
        SHADER_BLEND_ONE,
        SHADER_BLEND_ONE);  // Write over the eyes that were already there
    ...
    if (bDX9) {
      int nShadowFilterMode =
          g_pHardwareConfig->GetShadowFilterMode();  // Based upon vendor and
                                                     // device dependent formats
      ...
      if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
        pShaderShadow->EnableTexture(SHADER_SAMPLER4,
                                     true);  // Shadow depth map
        pShaderShadow->SetShadowDepthFiltering(SHADER_SAMPLER4);
        pShaderShadow->EnableTexture(SHADER_SAMPLER5,
                                     true);  // Shadow noise rotation map
      }
    } else
#endif
    {
      ...
      pShaderShadow->SetVertexShader("eyes_flashlight_vs11",
                                     vshIndex.GetIndex());

      eyes_flashlight_ps11_Static_Index pshIndex;
      pShaderShadow->SetPixelShader("eyes_flashlight_ps11",
                                    pshIndex.GetIndex());
    }
    ...
    pShader->BindTexture(SHADER_SAMPLER0, flashlightState.m_pSpotlightTexture,
    ...
    if (bDX9) {
      if (!g_pHardwareConfig->HasFastVertexTextures()) {
        ...
        SET_DYNAMIC_VERTEX_SHADER_COMBO(SKINNING,
                                        pShaderAPI->GetCurrentNumBones() > 0);
      ...
      VMatrix theWorldToTexture;
      ITexture *theFlashlightDepthTexture;
      FlashlightState_t theFlashlightState = pShaderAPI->GetFlashlightStateEx(
          theWorldToTexture, &theFlashlightDepthTexture);
      SetFlashLightColorFromState(theFlashlightState, pShaderAPI);

      if (theFlashlightDepthTexture && g_pConfig->ShadowDepthTexture() &&
          theFlashlightState.m_bEnableShadows) {
        pShader->BindTexture(SHADER_SAMPLER4, theFlashlightDepthTexture, 0);
      ...
      if (!g_pHardwareConfig->HasFastVertexTextures()) {
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          ...
          SetDepthFlashlightParams(pShader, pShaderAPI, theWorldToTexture,
                                   theFlashlightState);
        } else {
    ...
    // This uses from VERTEX_SHADER_SHADER_SPECIFIC_CONST_0 to
    // VERTEX_SHADER_SHADER_SPECIFIC_CONST_5
    pShader->SetFlashlightVertexShaderConstants(false, -1, false, -1, false);

    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_6,
                                     info.m_nEyeOrigin);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_7,
                                     info.m_nEyeUp);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_8,
                                     info.m_nIrisU);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_9,
                                     info.m_nIrisV);
  }
  pShader->Draw();
}

static void DrawUsingVertexShader(bool bDX9, CBaseVSShader *pShader,
                                  IMaterialVar **params,
                                  IShaderDynamicAPI *pShaderAPI,
                                  IShaderShadow *pShaderShadow,
                                  Eyes_DX8_DX9_Vars_t &info,
  ...
  SHADOW_STATE {
    ...
    pShaderShadow->EnableAlphaWrites(true);  // we end up hijacking destination
                                             // alpha for opaques most of the
                                             // time.
    ...
    if (bDX9) {
      if (!g_pHardwareConfig->HasFastVertexTextures()) {
        ...
        SET_STATIC_VERTEX_SHADER_COMBO(HALFLAMBERT,
                                       IS_FLAG_SET(MATERIAL_VAR_HALFLAMBERT));
        SET_STATIC_VERTEX_SHADER_COMBO(
            INTRO, params[info.m_nIntro]->GetIntValue() ? 1 : 0);
  ...
  DYNAMIC_STATE {
    ...
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                                     info.m_nEyeOrigin);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_1,
                                     info.m_nEyeUp);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_2,
                                     info.m_nIrisU);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_3,
                                     info.m_nIrisV);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_4,
                                     info.m_nGlintU);
    pShader->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_5,
                                     info.m_nGlintV);
    ...
    if (bDX9) {
      ...
      float fGlintDamping = std::max(
          0.0f, std::min(pShaderAPI->GetAmbientLightCubeLuminance(), 1.0f));
      ...
      if (fGlintDamping > fDimGlint)
        ...
        fGlintDamping *=
            SimpleSplineRemapVal(fGlintDamping, 0.0f, fDimGlint, 0.0f, 1.0f);

      // Special constant for DX9 eyes: { Dilation, ambient, x, x };
      float vPSConst[4] = {params[info.m_nDilation]->GetFloatValue(),
                           fGlintDamping, 0.0f, 0.0f};
      ...
      if (!g_pHardwareConfig->HasFastVertexTextures()) {
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          ...
          SET_DYNAMIC_PIXEL_SHADER_COMBO(
              ...
              pShaderAPI->ShouldWriteDepthToDestAlpha());
      ...
      if (params[info.m_nIntro]->GetIntValue()) {
        ...
        pShaderAPI->SetVertexShaderConstant(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_6, timeVec, 1);
      }
    } else
#endif
    {
...
static void DrawEyes_DX8_DX9_Internal(
    bool bDX9, CBaseVSShader *pShader, IMaterialVar **params,
    IShaderDynamicAPI *pShaderAPI, IShaderShadow *pShaderShadow,
    bool bHasFlashlight, Eyes_DX8_DX9_Vars_t &info,
    VertexCompressionType_t vertexCompression) {
  if (!bHasFlashlight) {
    DrawUsingVertexShader(bDX9, pShader, params, pShaderAPI, pShaderShadow,
                          info, vertexCompression);
  } else {
    DrawFlashlight(bDX9, pShader, params, pShaderAPI, pShaderShadow, info,
                   vertexCompression);
  }
}

extern ConVar r_flashlight_version2;
void DrawEyes_DX8_DX9(bool bDX9, CBaseVSShader *pShader, IMaterialVar **params,
                      IShaderDynamicAPI *pShaderAPI,
                      IShaderShadow *pShaderShadow, Eyes_DX8_DX9_Vars_t &info,
                      VertexCompressionType_t vertexCompression) {
  SHADOW_STATE { SET_FLAGS2(MATERIAL_VAR2_LIGHTING_VERTEX_LIT); }
  bool bHasFlashlight = pShader->UsingFlashlight(params);

  if (bHasFlashlight && (IsX360() || r_flashlight_version2.GetInt())) {
    DrawEyes_DX8_DX9_Internal(bDX9, pShader, params, pShaderAPI, pShaderShadow,
                              false, info, vertexCompression);
    if (pShaderShadow) pShader->SetInitialShadowState();
  }

  DrawEyes_DX8_DX9_Internal(bDX9, pShader, params, pShaderAPI, pShaderShadow,
                            bHasFlashlight, info, vertexCompression);
}
