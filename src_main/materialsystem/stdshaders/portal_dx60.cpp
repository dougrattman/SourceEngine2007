...
SHADER_PARAM(RENDERFIXZ, SHADER_PARAM_TYPE_INTEGER, "0",
             "Special depth handling, intended for rendering bug workarounds "
             "for extremely close polygons")
END_SHADER_PARAMS
...
SHADER_DRAW {
  ...
  SHADOW_STATE {
    ...
    if (params[RENDERFIXZ]->GetIntValue() == 0) {
      // pShaderShadow->EnablePolyOffset( SHADER_POLYOFFSET_DECAL ); //a portal
      // is effectively a decal on top of a wall
  ...
  if (bAlphaMaskTexture)
    StaticPass_WithAlphaMask(pShaderShadow, pShaderAPI,
  ...
  SHADOW_STATE {
    ...
    if (bStaticBlendTexture) {
      pShaderShadow->CustomTextureOperation(
          ...
          SHADER_TEXOP_SELECTARG1, SHADER_TEXARG_TEXTURE,
          SHADER_TEXARG_TEXTURE);
    } else {
      pShaderShadow->CustomTextureOperation(
          SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_COLOR,
          SHADER_TEXOP_SELECTARG1, SHADER_TEXARG_CONSTANTCOLOR,
