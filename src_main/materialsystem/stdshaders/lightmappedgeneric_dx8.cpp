...
BEGIN_VS_SHADER(LightmappedGeneric_DX8, "Help for LightmappedGeneric_DX8")
...
BEGIN_SHADER_PARAMS
  ...
  SHADER_PARAM(DETAILBLENDFACTOR, SHADER_PARAM_TYPE_FLOAT, "1",
               "amount of detail texture to apply")
  ...
  SHADER_PARAM(ENVMAPMASKSCALE, SHADER_PARAM_TYPE_FLOAT, "1",
               "envmap mask scale")
  ...
  SHADER_PARAM(ENVMAPOPTIONAL, SHADER_PARAM_TYPE_INTEGER, "90",
               "Do specular pass only on dxlevel or higher (ie.80, 81, 90)")
  SHADER_PARAM(NODIFFUSEBUMPLIGHTING, SHADER_PARAM_TYPE_BOOL, "0",
               "0 == Use diffuse bump lighting, 1 = No diffuse bump lighting")
  SHADER_PARAM(FORCEBUMP, SHADER_PARAM_TYPE_BOOL, "0",
               "0 == Do bumpmapping if the card says it can handle it. 1 == "
               "Always do bumpmapping.")
  SHADER_PARAM(ALPHATESTREFERENCE, SHADER_PARAM_TYPE_FLOAT, "0.0", "")
  SHADER_PARAM(SSBUMP, SHADER_PARAM_TYPE_INTEGER, "0",
               "whether or not to use alternate bumpmap format with height")
  SHADER_PARAM(SEAMLESS_SCALE, SHADER_PARAM_TYPE_FLOAT, "0",
               "Scale factor for 'seamless' texture mapping. 0 means to use "
               "ordinary mapping")
END_SHADER_PARAMS
...
SHADER_INIT_PARAMS() {
  ...
  if (params[ENVMAPOPTIONAL]->IsDefined() &&
      (params[ENVMAPOPTIONAL]->GetIntValue() >
       g_pHardwareConfig->GetDXSupportLevel())) {
    params[ENVMAP]->SetUndefined();
  }

  // If mat_specular 0, then get rid of envmap
  if (!g_pConfig->UseSpecular() && params[ENVMAP]->IsDefined() &&
      params[BASETEXTURE]->IsDefined()) {
    params[ENVMAP]->SetUndefined();
  }

  if (params[SEAMLESS_SCALE]->IsDefined() &&
      params[SEAMLESS_SCALE]->GetFloatValue() != 0.0f) {
    if (params[BUMPMAP]->IsDefined()) {
      Warning(
          "Can't use $bumpmap with $seamless_scale for lightmappedgeneric_dx8. "
          " Implicitly disabling $bumpmap: %s\n",
          pMaterialName);
      params[BUMPMAP]->SetUndefined();
    }
    if (params[ENVMAP]->IsDefined()) {
      Warning(
          "Can't use $envmap with $seamless_scale for lightmappedgeneric_dx8. "
          "Implicitly disabling $envmap: %s\n",
          pMaterialName);
      params[ENVMAP]->SetUndefined();
    }
  }
  ...
  // *and* we have normalmapalphaenvmapmask *and* we don't have envmapmask
  // elsewhere
  if (params[ENVMAP]->IsDefined() && params[BUMPMAP]->IsDefined() &&
      IS_FLAG_SET(MATERIAL_VAR_NORMALMAPALPHAENVMAPMASK) &&
      !ShouldUseBumpmapping(params)) {
    if (!IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK) &&
        !params[ENVMAPMASK]->IsDefined()) {
      params[ENVMAP]->SetUndefined();
    }
  }
}
...
inline const char* GetPixelShaderName(IMaterialVar** params,
  ...
  if (!params[BASETEXTURE]->IsTexture()) {
    ...
    if (params[ENVMAP]->IsTexture() && !bBumpedEnvMap) {
      int pshIndex = 0;
      if (IS_FLAG_SET(MATERIAL_VAR_SELFILLUM)) pshIndex |= 0x1;
      if (IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK)) pshIndex |= 0x2;
      if (params[ENVMAPMASK]->IsTexture()) pshIndex |= 0x4;
...
void DrawUnbumpedUsingVertexShader(IMaterialVar** params,
  ...
  if (hasEnvmap || hasBaseTexture || hasVertexColor || !bBumpedEnvMap) {
    SHADOW_STATE {
      ...
      if (params[ALPHATESTREFERENCE]->GetFloatValue() > 0.0f) {
        pShaderShadow->AlphaFunc(SHADER_ALPHAFUNC_GEQUAL,
      ...
      pShaderShadow->SetVertexShader("LightmappedGeneric_vs11",
                                     vshIndex.GetIndex());
      ...
      if (hasEnvmap) {
        ...
        if (params[ENVMAPMASK]->IsTexture() ||
          ...
          SetVertexShaderTextureScaledTransform(
              VERTEX_SHADER_SHADER_SPECIFIC_CONST_2, BASETEXTURETRANSFORM,
              ENVMAPMASKSCALE);
        }
  ...
  if (bBumpedEnvMap) {
    DrawWorldBumpedSpecularLighting(
        BUMPMAP, ENVMAP, BUMPFRAME, ENVMAPFRAME, ENVMAPTINT, ALPHA,
        ENVMAPCONTRAST, ENVMAPSATURATION, BUMPTRANSFORM, FRESNELREFLECTION,
        hasEnvmap || hasBaseTexture || hasVertexColor);
  }
}

void DrawDetailNoEnvmap(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                        IShaderShadow* pShaderShadow, bool doSelfIllum) {
  SHADOW_STATE {
    // Alpha test
    pShaderShadow->EnableAlphaTest(IS_FLAG_SET(MATERIAL_VAR_ALPHATEST));
    ...
    if (IS_FLAG_SET(MATERIAL_VAR_VERTEXCOLOR)) fmt |= VERTEX_COLOR;
    ...
    if (!params[BASETEXTURE]->IsTexture()) {
      ...
      if (!IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) || (!doSelfIllum)) {
        ...
        pShaderShadow->SetPixelShader(
            "LightmappedGeneric_DetailSelfIlluminated");
      }
    }
    DefaultFog();
  }
  DYNAMIC_STATE {
    ...
    SetVertexShaderTextureScaledTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_4,
                                          BASETEXTURETRANSFORM, DETAILSCALE);
...
inline const char* GetAdditiveEnvmapPixelShaderName(
    bool usingMask, bool usingBaseTexture, bool usingBaseAlphaEnvmapMask) {
  static char const* s_pPixelShaders[] = {
  ...
  int pshIndex = 0;
  if (usingMask) pshIndex |= 0x1;
  return s_pPixelShaders[pshIndex];
}

void DrawAdditiveEnvmap(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                        IShaderShadow* pShaderShadow) {
  ...
  SHADOW_STATE {
    ...
    s_pShaderShadow->SetVertexShader("LightmappedGeneric_vs11",
                                     vshIndex.GetIndex());

    const char* pshName = GetAdditiveEnvmapPixelShaderName(
...
void DrawDetailMode1(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                     IShaderShadow* pShaderShadow, bool bBumpedEnvMap) {
  // Mode 1 :
  ...
  if (!bBumpedEnvMap) {
    ...
    DrawWorldBumpedSpecularLighting(BUMPMAP, ENVMAP, BUMPFRAME, ENVMAPFRAME,
                                    ENVMAPTINT, ALPHA, ENVMAPCONTRAST,
                                    ENVMAPSATURATION, BUMPTRANSFORM,
                                    FRESNELREFLECTION, true);
  }
}

void DrawDetailUsingVertexShader(IMaterialVar** params,
                                 IShaderDynamicAPI* pShaderAPI,
                                 IShaderShadow* pShaderShadow,
                                 bool bBumpedEnvMap) {
  // We don't have enough textures; gotta do this in two passes if there's
  // envmapping
  if (!params[ENVMAP]->IsTexture()) {
    DrawDetailNoEnvmap(params, pShaderAPI, pShaderShadow,
...
void DrawUnbumpedSeamlessUsingVertexShader(IMaterialVar** params,
                                           IShaderDynamicAPI* pShaderAPI,
                                           IShaderShadow* pShaderShadow) {
  ...
  SHADOW_STATE {
    ...
    pShaderShadow->SetVertexShader("WorldVertexTransition_Seamless",
                                   vshIndex.GetIndex());
  ...
  DYNAMIC_STATE {
    ...
    float map_scale[4] = {fSeamlessScale, fSeamlessScale, fSeamlessScale,
                          fSeamlessScale};
...
SHADER_DRAW {
  ...
  if (hasFlashlight) {
    DrawFlashlight_dx80(params, pShaderAPI, pShaderShadow, bBump, BUMPMAP,
                        BUMPFRAME, BUMPTRANSFORM, FLASHLIGHTTEXTURE,
                        FLASHLIGHTTEXTUREFRAME, true, false, 0, -1, -1);
  } else if (bBump) {
    DrawWorldBumpedUsingVertexShader(
        BASETEXTURE, BASETEXTURETRANSFORM, BUMPMAP, BUMPFRAME, BUMPTRANSFORM,
        ENVMAPMASK, ENVMAPMASKFRAME, ENVMAP, ENVMAPFRAME, ENVMAPTINT, COLOR,
        ALPHA, ENVMAPCONTRAST, ENVMAPSATURATION, FRAME, FRESNELREFLECTION,
        false, -1, -1, -1, bSSBump);
  } else {
    bool bBumpedEnvMap = ShouldUseBumpmapping(params) &&
                         params[BUMPMAP]->IsTexture() &&
                         params[ENVMAP]->IsTexture();
    if (!params[DETAIL]->IsTexture()) {
      if (params[SEAMLESS_SCALE]->GetFloatValue() != 0.0f) {
        DrawUnbumpedSeamlessUsingVertexShader(params, pShaderAPI,
                                              pShaderShadow);
      } else {
        DrawUnbumpedUsingVertexShader(params, pShaderAPI, pShaderShadow,
                                      bBumpedEnvMap);
...
virtual bool ShouldUseBumpmapping(IMaterialVar** params) {
  if (!g_pConfig->UseBumpmapping()) return false;
