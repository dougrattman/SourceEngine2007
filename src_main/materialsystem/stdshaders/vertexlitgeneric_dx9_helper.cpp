...
static inline bool WantsSkinShader(IMaterialVar **params,
                                   const VertexLitGeneric_DX9_Vars_t &info) {
  if (info.m_nPhong == -1)  // Don't use skin without Phong
    return false;

  if (params[info.m_nPhong]->GetIntValue() ==
      0)  // Don't use skin without Phong turned on
    return false;

  if ((info.m_nDiffuseWarpTexture != -1) &&
      params[info.m_nDiffuseWarpTexture]
          ->IsTexture())  // If there's Phong and diffuse warp do skin
    return true;

  if ((info.m_nBaseMapAlphaPhongMask != -1) &&
      params[info.m_nBaseMapAlphaPhongMask]->GetIntValue() != 1) {
    if (info.m_nBumpmap == -1)  // Don't use without a bump map
      return false;

    if (!params[info.m_nBumpmap]
             ->IsTexture())  // Don't use if the texture isn't specified
      return false;
  }
...
void InitParamsVertexLitGeneric_DX9(CBaseVSShader *pShader,
  ...
  if ((info.m_nSelfIllumTint != -1) &&
      (!params[info.m_nSelfIllumTint]->IsDefined())) {
  ...
  // Write over $basetexture with $info.m_nBumpmap if we are going to be using
  // diffuse normal mapping.
  if (info.m_nAlbedo != -1 && g_pConfig->UseBumpmapping() &&
      info.m_nBumpmap != -1 && params[info.m_nBumpmap]->IsDefined() &&
      params[info.m_nAlbedo]->IsDefined() &&
      params[info.m_nBaseTexture]->IsDefined()) {
    params[info.m_nBaseTexture]->SetStringValue(
        params[info.m_nAlbedo]->GetStringValue());
  }
  ...
  if (((info.m_nBumpmap != -1) && g_pConfig->UseBumpmapping() &&
       params[info.m_nBumpmap]->IsDefined())
      // we don't need a tangent space if we have envmap without bumpmap
      //		|| ( info.m_nEnvmap != -1 &&
      // params[info.m_nEnvmap]->IsDefined() )
  ) {
    SET_FLAGS2(MATERIAL_VAR2_NEEDS_TANGENT_SPACES);
  } else if ((info.m_nDiffuseWarpTexture != -1) &&
             params[info.m_nDiffuseWarpTexture]
                 ->IsDefined())  // diffuse warp goes down bump path...
  {
    SET_FLAGS2(MATERIAL_VAR2_NEEDS_TANGENT_SPACES);
  ...
  if (IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK) && info.m_nBumpmap != -1 &&
    ...
    Warning(
        "material %s has a normal map and $basealphaenvmapmask.  Must use "
  ...
  if (info.m_nEnvmapMask != -1 && params[info.m_nEnvmapMask]->IsDefined() &&
    ...
    if (!hasNormalMapAlphaEnvmapMask) {
      Warning(
          "material %s has a normal map and an envmapmask.  Must use "
  ...
  if (!g_pConfig->UseSpecular() && info.m_nEnvmap != -1 &&
      params[info.m_nEnvmap]->IsDefined() &&
      params[info.m_nBaseTexture]->IsDefined()) {
    params[info.m_nEnvmap]->SetUndefined();
  }
...
void InitVertexLitGeneric_DX9(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  // bool bHasBump = ( info.m_nBumpmap != -1 ) &&
  // params[info.m_nBumpmap]->IsTexture();  if ( bHasBump )
  //{
  ...
  if (bNeedsSkinBecauseOfDetail ||
      (info.m_nPhong != -1 && params[info.m_nPhong]->GetIntValue()) &&
  ...
  bool bHasSelfIllumMask = IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) &&
                           ...
                           params[info.m_nSelfIllumMask]->IsDefined();

  // No alpha channel in any of the textures? No self illum or envmapmask
  if (!bIsBaseTextureTranslucent) {
    bool bHasSelfIllumFresnel =
        IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) &&
        (info.m_nSelfIllumFresnel != -1) &&
        (params[info.m_nSelfIllumFresnel]->GetIntValue() != 0);

    // Can still be self illum with no base alpha if using one of these
    // alternate modes
  ...
  if (info.m_nDetail != -1 && params[info.m_nDetail]->IsDefined()) {
    int nDetailBlendMode =
        (info.m_nDetailTextureCombineMode == -1)
            ? 0
  ...
  if (g_pConfig->UseBumpmapping()) {
    if ((info.m_nBumpmap != -1) && params[info.m_nBumpmap]->IsDefined()) {
      ...
      SET_FLAGS2(MATERIAL_VAR2_DIFFUSE_BUMPMAPPED_MODEL);
               ...
               params[info.m_nDiffuseWarpTexture]->IsDefined()) {
      SET_FLAGS2(MATERIAL_VAR2_DIFFUSE_BUMPMAPPED_MODEL);
    }
  }
  ...
  if (IS_FLAG_SET(MATERIAL_VAR_SELFILLUM) ||
      IS_FLAG_SET(MATERIAL_VAR_BASEALPHAENVMAPMASK)) {
...
//     is set to zero."
// This change caused shader problems in HLMV (use ep2/Hunter model as an
// example) and I was told to fix it. -Jeep
//
...
// Draws the shader
//-----------------------------------------------------------------------------
static void DrawVertexLitGeneric_DX9_Internal(
    CBaseVSShader *pShader, IMaterialVar **params,
...
{
  CVertexLitGeneric_DX9_Context *pContextData =
      reinterpret_cast<CVertexLitGeneric_DX9_Context *>(*pContextDataPtr);
  ...
  bool bHasDiffuseWarp = (!bHasFlashlight || IsX360()) && hasDiffuseLighting &&
                         (info.m_nDiffuseWarpTexture != -1) &&
                         params[info.m_nDiffuseWarpTexture]->IsTexture();

  // bool bNoCull = IS_FLAG_SET( MATERIAL_VAR_NOCULL );
  bool bFlashlightNoLambert = false;
  if ((info.m_nFlashlightNoLambert != -1) &&
      params[info.m_nFlashlightNoLambert]->GetIntValue()) {
  ...
  float fBlendFactor =
      GetFloatParam(info.m_nDetailTextureBlendFactor, params, 1.0);
  bool bHasDetailTexture = IsTextureSet(info.m_nDetail, params);
  if (bHasDetailTexture && (fBlendFactor > 0.0)) {
    if ((nDetailBlendMode == 3) || (nDetailBlendMode == 8) ||
        (nDetailBlendMode == 9))
  ...
  if (bHasBaseTexture) {
    nBlendType = pShader->EvaluateBlendRequirements(info.m_nBaseTexture, true,
                                                    nDetailTranslucencyTexture);
  } else {
    nBlendType = pShader->EvaluateBlendRequirements(info.m_nEnvmapMask, false);
  }
  bool bFullyOpaque =
      (nBlendType != BT_BLENDADD) && (nBlendType != BT_BLEND) &&
      !bIsAlphaTested &&
      (!bHasFlashlight || IsX360());  // dest alpha is free for special use

  bool bHasEnvmap = (!bHasFlashlight || IsX360()) && info.m_nEnvmap != -1 &&
                    params[info.m_nEnvmap]->IsTexture();
  ...
  if (pShader->IsSnapshotting() || (!pContextData) ||
      (pContextData->m_bMaterialVarsChanged)) {
    ...
    bool bHasSelfIllum =
        (!bHasFlashlight || IsX360()) && IS_FLAG_SET(MATERIAL_VAR_SELFILLUM);

    bool bHasSelfIllumMask =
        bHasSelfIllum && IsTextureSet(info.m_nSelfIllumMask, params);
    ...
    if (pShader->IsSnapshotting()) {
      ...
      if (info.m_nVertexAlphaTest != -1 &&
          params[info.m_nVertexAlphaTest]->GetIntValue() > 0) {
      ...
      bool bHasSelfIllumFresnel =
          (!bHasDetailTexture) && (bHasSelfIllum) &&
          (info.m_nSelfIllumFresnel != -1) &&
          (params[info.m_nSelfIllumFresnel]->GetIntValue() != 0);
      ...
      bool bHasEnvmap = (!bHasFlashlight || IsX360()) &&
                        (info.m_nEnvmap != -1) &&
                        params[info.m_nEnvmap]->IsTexture();
      bool bHasEnvmapMask =
          (!bHasFlashlight || IsX360()) &&
          (info.m_nEnvmapMask != -1 && params[info.m_nEnvmapMask]->IsTexture());
      bool bHasNormal = bVertexLitGeneric || bHasEnvmap || bHasFlashlight ||
                        bSeamlessBase || bSeamlessDetail;

      // On PC, LIGHTING_PREVIEW requires normals (they won't use much memory
      // - unlitgeneric isn't used on many models)
      bHasNormal = true;
      ...
      if (bHasFlashlight) {
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          nShadowFilterMode =
              g_pHardwareConfig->GetShadowFilterMode();  // Based upon vendor
        ...
        if (!IsX360()) {
          ...
          if (bIsAlphaTested) {
            ...
            // guaranteed to be the same on both the regular pass and the
            // flashlight pass.
      ...
      if (bHasBaseTexture) {
        if ((info.m_nGammaColorRead != -1) &&
            (params[info.m_nGammaColorRead]->GetIntValue() == 1))
      ...
      if (bHasFlashlight) {
        ...
        pShaderShadow->EnableTexture(SHADER_SAMPLER7,
                                     true);  // Flashlight cookie
      ...
      if (bHasDiffuseWarp && (!bHasFlashlight || IsX360()) &&
          !bHasSelfIllumFresnel) {
        pShaderShadow->EnableTexture(SHADER_SAMPLER9,
                                     true);  // Diffuse warp texture
      }

      if ((info.m_nDepthBlend != -1) &&
          (params[info.m_nDepthBlend]->GetIntValue())) {
        if (bHasBump)
          Warning(
              "DEPTHBLEND not supported by bump mapped variations of "
              "vertexlitgeneric to avoid shader bloat. Either remove the bump "
              "map or convince a graphics programmer that it's worth it.\n");

        pShaderShadow->EnableTexture(SHADER_SAMPLER10, true);
      }

      if (bHasSelfIllum) {
        pShaderShadow->EnableTexture(SHADER_SAMPLER11,
                                     true);  // self illum mask
      }

      bool bSRGBWrite = true;
      if ((info.m_nLinearWrite != -1) &&
          (params[info.m_nLinearWrite]->GetIntValue() == 1)) {
        bSRGBWrite = false;
      }
      ...
      pShaderShadow->VertexShaderVertexFormat(flags, nTexCoordCount,
                                              pTexCoordDim, userDataSize);
      ...
      if (bHasBump || bHasDiffuseWarp) {
...
#endif
        {
          ...
          SET_STATIC_VERTEX_SHADER_COMBO(
              USE_WITH_2B, g_pHardwareConfig->SupportsPixelShaders_2_b());
#ifdef _X360
          ...
          if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
            ...
            SET_STATIC_PIXEL_SHADER_COMBO(
                LIGHTWARPTEXTURE, bHasDiffuseWarp && !bHasSelfIllumFresnel);
            SET_STATIC_PIXEL_SHADER_COMBO(SELFILLUM, bHasSelfIllum);
            SET_STATIC_PIXEL_SHADER_COMBO(SELFILLUMFRESNEL,
                                          bHasSelfIllumFresnel);
            ...
            SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE,
                                          nShadowFilterMode);
      ...
      } else  // !(bHasBump || bHasDiffuseWarp)
      {
        ...
        if (bDistanceAlpha) {
          bDistanceAlphaFromDetail =
              IsBoolSet(info.m_nDistanceAlphaFromDetail, params);
...
#endif
        {
          ...
          SET_STATIC_VERTEX_SHADER_COMBO(VERTEXCOLOR,
                                         bHasVertexColor || bHasVertexAlpha);
          ...
          if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
            ...
            SET_STATIC_PIXEL_SHADER_COMBO(
                ...
                (hasSelfIllumInEnvMapMask && (bHasEnvmapMask)));
            SET_STATIC_PIXEL_SHADER_COMBO(DETAILTEXTURE, bHasDetailTexture);
            SET_STATIC_PIXEL_SHADER_COMBO(CUBEMAP, bHasEnvmap);
            SET_STATIC_PIXEL_SHADER_COMBO(DIFFUSELIGHTING, hasDiffuseLighting);
            SET_STATIC_PIXEL_SHADER_COMBO(ENVMAPMASK, bHasEnvmapMask);
            SET_STATIC_PIXEL_SHADER_COMBO(BASEALPHAENVMAPMASK,
            ...
            SET_STATIC_PIXEL_SHADER_COMBO(DISTANCEALPHAFROMDETAIL,
                                          bDistanceAlphaFromDetail);
            ...
            SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE,
                                          nShadowFilterMode);
            SET_STATIC_PIXEL_SHADER_COMBO(
                DEPTHBLEND, IsBoolSet(info.m_nDepthBlend, params));
          ...
          SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE,
                                        nShadowFilterMode);
          SET_STATIC_PIXEL_SHADER_COMBO(DEPTHBLEND,
                                        IsBoolSet(info.m_nDepthBlend, params));
    ...
    if (pShaderAPI &&
        ((!pContextData) || (pContextData->m_bMaterialVarsChanged))) {
      ...
      if (bHasBaseTexture) {
        pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER0,
                                                      info.m_nBaseTexture,
                                                      info.m_nBaseTextureFrame);
      } else {
        if (bHasEnvmap) {
          // if we only have an envmap (no basetexture), then we want the albedo
          // to be black.
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(SHADER_SAMPLER0,
                                                                TEXTURE_BLACK);
        } else {
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(SHADER_SAMPLER0,
                                                                TEXTURE_WHITE);
      ...
      if (bHasDetailTexture) {
        pContextData->m_SemiStaticCmdsOut.BindTexture(
            pShader, SHADER_SAMPLER2, info.m_nDetail, info.m_nDetailFrame);
      }
      if (bHasSelfIllum) {
        if (bHasSelfIllumMask)  // Separate texture for self illum?
        {
          pContextData->m_SemiStaticCmdsOut.BindTexture(
              pShader, SHADER_SAMPLER11, info.m_nSelfIllumMask, -1);  // Bind it
        } else                                                        // else
        {
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
              SHADER_SAMPLER11, TEXTURE_BLACK);  // Bind dummy
        }
      }
      ...
      if ((info.m_nDepthBlend != -1) &&
          (params[info.m_nDepthBlend]->GetIntValue())) {
        pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
            SHADER_SAMPLER10, TEXTURE_FRAME_BUFFER_FULL_DEPTH);
      }
      if (bSeamlessDetail || bSeamlessBase) {
        float flSeamlessData[4] = {
            params[info.m_nSeamlessScale]->GetFloatValue(), 0, 0, 0};
        pContextData->m_SemiStaticCmdsOut.SetVertexShaderConstant(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_2, flSeamlessData);
      }
      ...
      if (bHasDetailTexture) {
        pContextData->m_SemiStaticCmdsOut.SetVertexShaderTextureScaledTransform(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_4, info.m_nBaseTextureTransform,
            info.m_nDetailScale);
        // Assert( !bHasBump );
        if (info.m_nDetailTint != -1)
          pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstantGammaToLinear(
              10, info.m_nDetailTint);
        else {
          pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant4(10, 1, 1, 1,
                                                                    1);
        }
      }
      if (bDistanceAlpha) {
        ...
        bool bScaleEdges =
            IsBoolSet(info.m_nScaleEdgeSoftnessBasedOnScreenRes, params);
        bool bScaleOutline =
            IsBoolSet(info.m_nScaleOutlineSoftnessBasedOnScreenRes, params);
        ...
        pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant(5, flConsts,
                                                                 5);
      }
      if (!g_pConfig->m_bFastNoBump) {
        if (bHasBump) {
          pContextData->m_SemiStaticCmdsOut.BindTexture(
              pShader, SHADER_SAMPLER3, info.m_nBumpmap, info.m_nBumpFrame);
        } else if (bHasDiffuseWarp) {
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
              SHADER_SAMPLER3, TEXTURE_NORMALMAP_FLAT);
        }
      } else {
        if (bHasBump) {
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
      ...
      if (info.m_nEnvmapSaturation != -1)
        params[info.m_nEnvmapSaturation]->GetVecValue(
            vEnvMapSaturation_SelfIllumMask, 3);

      vEnvMapSaturation_SelfIllumMask[3] = bHasSelfIllumMask ? 1.0f : 0.0f;
      pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant(
          3, vEnvMapSaturation_SelfIllumMask, 1);
      if (bHasEnvmap) {
        pContextData->m_SemiStaticCmdsOut
            .SetEnvMapTintPixelShaderDynamicStateGammaToLinear(
                0, info.m_nEnvmapTint);
      }
      bool bHasEnvmapMask = (!bHasFlashlight || IsX360()) &&
                            info.m_nEnvmapMask != -1 &&
                            params[info.m_nEnvmapMask]->IsTexture();
      if (bHasEnvmapMask) {
        pContextData->m_SemiStaticCmdsOut.BindTexture(pShader, SHADER_SAMPLER4,
                                                      info.m_nEnvmapMask,
      ...
      if (bHasSelfIllumFresnel && (!bHasFlashlight || IsX360())) {
        ...
        float flMin =
            IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
                ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[0]
                : 0.0f;
        float flMax =
            IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
                ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[1]
                : 1.0f;
        float flExp =
            IS_PARAM_DEFINED(info.m_nSelfIllumFresnelMinMaxExp)
                ? params[info.m_nSelfIllumFresnelMinMaxExp]->GetVecValue()[2]
                : 1.0f;

        vConstScaleBiasExp[1] =
            (flMax != 0.0f) ? (flMin / flMax) : 0.0f;          // Bias
      ...
      if (bHasDiffuseWarp && (!bHasFlashlight || IsX360()) &&
        ...
        if (r_lightwarpidentity.GetBool()) {
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
              SHADER_SAMPLER9, TEXTURE_IDENTITY_LIGHTWARP);
        } else {
          pContextData->m_SemiStaticCmdsOut.BindTexture(
              pShader, SHADER_SAMPLER9, info.m_nDiffuseWarpTexture, -1);
        }
      }
      ...
      if (bHasFlashlight) {
        ...
        float tweaks[4];
        tweaks[0] = flashlightState.m_flShadowFilterSize /
                    flashlightState.m_flShadowMapResolution;
        tweaks[1] = ShadowAttenFromState(flashlightState);
        pShader->HashShadow2DJitter(flashlightState.m_flShadowJitterSeed,
                                    &tweaks[2], &tweaks[3]);
      ...
      if ((!bHasFlashlight || IsX360()) && (info.m_nEnvmapContrast != -1))
        pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant(
            2, info.m_nEnvmapContrast);

      // mat_fullbright 2 handling
      bool bLightingOnly = bVertexLitGeneric && mat_fullbright.GetInt() == 2 &&
                           !IS_FLAG_SET(MATERIAL_VAR_NO_DEBUG_OVERRIDE);
      if (bLightingOnly) {
        if (bHasBaseTexture) {
          if ((bHasSelfIllum && !hasSelfIllumInEnvMapMask)) {
            pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
                SHADER_SAMPLER0, TEXTURE_GREY_ALPHA_ZERO);
          } else {
            pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
                SHADER_SAMPLER0, TEXTURE_GREY);
          }
        }
        if (bHasDetailTexture) {
          pContextData->m_SemiStaticCmdsOut.BindStandardTexture(SHADER_SAMPLER2,
                                                                TEXTURE_GREY);
        }
      }
      ...
      if (bHasBump || bHasDiffuseWarp) {
        pContextData->m_SemiStaticCmdsOut.BindStandardTexture(
            SHADER_SAMPLER5, TEXTURE_NORMALIZATION_CUBEMAP_SIGNED);
        pContextData->m_SemiStaticCmdsOut.SetPixelShaderStateAmbientLightCube(
            5);
        pContextData->m_SemiStaticCmdsOut.CommitPixelShaderLighting(13);
      }
      pContextData->m_SemiStaticCmdsOut.SetPixelShaderConstant_W(
          4, info.m_nSelfIllumTint, fBlendFactor);
      pContextData->m_SemiStaticCmdsOut
  ...
  if (pShaderAPI) {
    ...
    if (bHasEnvmap) {
      DynamicCmdsOut.BindTexture(pShader, SHADER_SAMPLER1, info.m_nEnvmap,
                                 info.m_nEnvmapFrame);
    }
    ...
    if (bHasFlashlight) {
      ...
      if (pFlashlightDepthTexture && g_pConfig->ShadowDepthTexture() &&
        ...
        DynamicCmdsOut.BindStandardTexture(SHADER_SAMPLER6,
      ...
      pShader->BindTexture(SHADER_SAMPLER7, state.m_pSpotlightTexture,
                           state.m_nSpotlightTextureFrame);
    }
    ...
    if (bFullyOpaque) {
      ...
      // can't write a special value to dest alpha if we're actually using
      // as-intended alpha
    ...
    if (bHasBump || bHasDiffuseWarp) {
...
#endif
      {
        ...
        SET_DYNAMIC_VERTEX_SHADER_CMD(DynamicCmdsOut,
        ...
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          ...
          SET_DYNAMIC_PIXEL_SHADER_CMD(DynamicCmdsOut,
        ...
        pShader->SetHWMorphVertexShaderState(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_10,
        ...
        bool bUnusedTexCoords[3] = {
            false, false, !pShaderAPI->IsHWMorphingEnabled() || !bIsDecal};
    ...
    } else  // !( bHasBump || bHasDiffuseWarp )
    {
...
#endif
      {
        ...
        SET_DYNAMIC_VERTEX_SHADER_COMBO(DYNAMIC_LIGHT,
                                        lightState.HasDynamicLight());
        SET_DYNAMIC_VERTEX_SHADER_COMBO(STATIC_LIGHT,
                                        lightState.m_bStaticLight ? 1 : 0);
        SET_DYNAMIC_VERTEX_SHADER_COMBO(DOWATERFOG, fogIndex);
        SET_DYNAMIC_VERTEX_SHADER_COMBO(SKINNING, numBones > 0);
        SET_DYNAMIC_VERTEX_SHADER_COMBO(
            LIGHTING_PREVIEW, pShaderAPI->GetIntRenderingParameter(
        ...
        if (g_pHardwareConfig->SupportsPixelShaders_2_b()) {
          ...
          SET_DYNAMIC_PIXEL_SHADER_COMBO(
              LIGHTING_PREVIEW, pShaderAPI->GetIntRenderingParameter(
                                    INT_RENDERPARM_ENABLE_FIXED_LIGHTING));
          SET_DYNAMIC_PIXEL_SHADER_CMD(DynamicCmdsOut,
        ...
        SET_DYNAMIC_PIXEL_SHADER_COMBO(
            LIGHTING_PREVIEW, pShaderAPI->GetIntRenderingParameter(
                                  INT_RENDERPARM_ENABLE_FIXED_LIGHTING));
        SET_DYNAMIC_PIXEL_SHADER_CMD(DynamicCmdsOut,
                                     vertexlit_and_unlit_generic_ps30);
    ...
    if (!bHasBump && !bHasDiffuseWarp) {
      DynamicCmdsOut.SetDepthFeatheringPixelShaderConstant(
          13, GetFloatParam(info.m_nDepthBlendScale, params, 50.0f));
    }
    ...
    // Controls for lerp-style paths through shader code (bump and non-bump have
    // use different register)
    float vShaderControls[4] = {fPixelFogType, fWriteDepthToAlpha,
                                fWriteWaterFogToDestAlpha, fVertexAlpha};
    ...
    if (bHasFlashlight) {
      ...
      SetFlashLightColorFromState(flashlightState, pShaderAPI, 28,
                                  bFlashlightNoLambert);

      pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_6,
                                          worldToTexture.Base(), 4);

      pShader->BindTexture(SHADER_SAMPLER7, flashlightState.m_pSpotlightTexture,
                           flashlightState.m_nSpotlightTextureFrame);

      float atten_pos[8];
      atten_pos[0] =
          flashlightState
              .m_fConstantAtten;  // Set the flashlight attenuation factors
      ...
      atten_pos[4] =
          flashlightState.m_vecLightOrigin[0];  // Set the flashlight origin
...
void DrawVertexLitGeneric_DX9(CBaseVSShader *pShader, IMaterialVar **params,
  ...
  if (WantsSkinShader(params, info) &&
      g_pHardwareConfig
          ->SupportsPixelShaders_2_b() /*&& mat_bumpmap.GetBool()*/) {
    DrawSkin_DX9(pShader, params, pShaderAPI, pShaderShadow, info,
                 vertexCompression);
    return;
  }
  ...
  if (bNewFlashlight) {
    bReceiveFlashlight = bReceiveFlashlight ||
                         (GetIntParam(info.m_nReceiveFlashlight, params) != 0);
  }
  bool bHasFlashlight = bReceiveFlashlight && pShader->UsingFlashlight(params);

  DrawVertexLitGeneric_DX9_Internal(pShader, params, pShaderAPI, pShaderShadow,
                                    bVertexLitGeneric, bHasFlashlight, info,
