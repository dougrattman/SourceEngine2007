...
BEGIN_SHADER_PARAMS
SHADER_PARAM_OVERRIDE(COLOR, SHADER_PARAM_TYPE_COLOR, "{255 255 255}", "unused",
                      SHADER_PARAM_NOT_EDITABLE)
SHADER_PARAM_OVERRIDE(ALPHA, SHADER_PARAM_TYPE_FLOAT, "1.0", "unused",
                      SHADER_PARAM_NOT_EDITABLE)
SHADER_PARAM(
    STATICAMOUNT, SHADER_PARAM_TYPE_FLOAT, "0.0",
    "Amount of the static blend texture to blend into the base texture")
SHADER_PARAM(STATICBLENDTEXTURE, SHADER_PARAM_TYPE_TEXTURE, "",
...
SHADER_PARAM(ALPHAMASKTEXTURE, SHADER_PARAM_TYPE_TEXTURE, "",
...
SHADER_INIT {
  if (params[STATICBLENDTEXTURE]->IsDefined()) LoadTexture(STATICBLENDTEXTURE);
  if (params[ALPHAMASKTEXTURE]->IsDefined()) LoadTexture(ALPHAMASKTEXTURE);
  ...
  if (!params[NOCOLORWRITE]->IsDefined()) params[NOCOLORWRITE]->SetIntValue(0);
}
...
SHADER_DRAW {
  SHADOW_STATE {
    ...
    // pShaderShadow->EnablePolyOffset( SHADER_POLYOFFSET_DECAL ); //a portal is
    // effectively a decal on top of a wall
  ...
  if (params[ALPHAMASKTEXTURE]->IsTexture())
    StaticPass_WithAlphaMask(
        pShaderShadow, pShaderAPI,
        params);  // portal static texture blending, with an alpha mask
  else
    StaticPass_NoAlphaMask(pShaderShadow, pShaderAPI,
                           params);  // portal static texture blending
}

void StaticPass_NoAlphaMask(IShaderShadow *pShaderShadow,
  ...
  SHADOW_STATE {
    ...
    pShaderShadow->CustomTextureOperation(
        ...
        SHADER_TEXOP_BLEND_CONSTANTALPHA, SHADER_TEXARG_ZERO,
        SHADER_TEXARG_ONE);
  }
  DYNAMIC_STATE {
    BindTexture(SHADER_SAMPLER0, STATICBLENDTEXTURE, STATICBLENDTEXTUREFRAME);
    pShaderAPI->Color4f(0.0f, 0.0f, 0.0f,
                        params[STATICAMOUNT]->GetFloatValue());
  }
  Draw();
  SHADOW_STATE { pShaderShadow->EnableCustomPixelPipe(false); }
}

void StaticPass_WithAlphaMask(IShaderShadow *pShaderShadow,
                              IShaderDynamicAPI *pShaderAPI,
                              IMaterialVar **params) {
  SHADOW_STATE {
    pShaderShadow->DrawFlags(SHADER_DRAW_POSITION | SHADER_DRAW_TEXCOORD0 |
    ...
    // alpha mask
    pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_COLOR, SHADER_TEXOP_SELECTARG1,
