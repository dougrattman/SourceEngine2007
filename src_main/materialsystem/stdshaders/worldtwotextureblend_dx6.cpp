...
BEGIN_SHADER(WorldTwoTextureBlend_DX6, "Help for WorldTwoTextureBlend")

BEGIN_SHADER_PARAMS
SHADER_PARAM_OVERRIDE(BASETEXTURE, SHADER_PARAM_TYPE_TEXTURE,
                      "shadertest/WorldTwoTextureBlend", "iris texture", 0)
SHADER_PARAM(DETAIL, SHADER_PARAM_TYPE_TEXTURE,
             "shadertest/WorldTwoTextureBlend_detail", "detail texture")
SHADER_PARAM(DETAILSCALE, SHADER_PARAM_TYPE_FLOAT, "1.0",
...
SHADER_DRAW {
  ...
  if (hasFlashlight) {
    DrawFlashlight_dx70(params, pShaderAPI, pShaderShadow, FLASHLIGHTTEXTURE,
...
void NormalModePass1(IShaderShadow *pShaderShadow,
...
void NormalModePass2(IShaderShadow *pShaderShadow,
                     IShaderDynamicAPI *pShaderAPI, IMaterialVar **params,
                     float detailScale) {
  SHADOW_STATE {
    pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
...
void NormalModePass3(IShaderShadow *pShaderShadow,
                     IShaderDynamicAPI *pShaderAPI, IMaterialVar **params,
                     float detailScale) {
  SHADOW_STATE {
    SET_FLAGS2(MATERIAL_VAR2_LIGHTING_LIGHTMAP);
  ...
  DYNAMIC_STATE {
    if (detailScale != 1.0f) pShaderAPI->LoadIdentity();
...
void DetailAlphaMaskPass1(IShaderShadow *pShaderShadow,
                          IShaderDynamicAPI *pShaderAPI, IMaterialVar **params,
                          float detailScale) {
  ...
  SHADOW_STATE {
    ...
    // the first 2x is here. The second is in SingleTextureLightmapBlendMode in
    // the 2nd pass.
    pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_COLOR, SHADER_TEXOP_MODULATE2X,
    ...
    pShaderShadow->CustomTextureOperation(
        ...
        SHADER_TEXOP_MODULATEINVCOLOR_ADDALPHA, SHADER_TEXARG_INVTEXTUREALPHA,
...
void DetailAlphaMaskPass2(IShaderShadow *pShaderShadow,
                          IShaderDynamicAPI *pShaderAPI, float detailScale) {
  SHADOW_STATE {
    ...
    // Make sure the texgen transform is applied to the texture coordinates and
    // not to an auto-generated reflection vector or whatever.
