...
BEGIN_SHADER(Sprite_DX6, "Help for Sprite_DX6")
  ...
  switch (params[SPRITERENDERMODE]->GetIntValue()) {
    ...
    case kRenderTransAddFrameBlend: {
      ...
      DYNAMIC_STATE {
        ...
        pShaderAPI->Color3f(flFade * frameBlendAlpha, flFade * frameBlendAlpha,
                            flFade * frameBlendAlpha);
        ITexture *pTexture = params[BASETEXTURE]->GetTextureValue();
        BindTexture(SHADER_SAMPLER0, pTexture, (int)flFrame);
      }
      Draw();
      SHADOW_STATE { FogToBlack(); }
      DYNAMIC_STATE {
