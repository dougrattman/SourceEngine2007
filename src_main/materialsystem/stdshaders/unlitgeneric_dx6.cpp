...
BEGIN_SHADER(UnlitGeneric_DX6, "Help for UnlitGeneric_DX6")
...
int GetDrawFlagsPass1(IMaterialVar** params, bool doDetail) {
  ...
  if (doDetail) flags |= SHADER_DRAW_TEXCOORD1;
  return flags;
}
...
void DrawAdditiveNonTextured(IMaterialVar** params,
                             IShaderDynamicAPI* pShaderAPI,
                             IShaderShadow* pShaderShadow, bool doDetail) {
  SHADOW_STATE {
    SetModulationShadowState();
    SetAdditiveBlendingShadowState();
    if (doDetail) SetDetailShadowState(pShaderShadow);
    pShaderShadow->DrawFlags(GetDrawFlagsPass1(params, doDetail));
    FogToBlack();
  }
  DYNAMIC_STATE {
    SetModulationDynamicState();
    if (doDetail) SetDetailDymamicState(pShaderShadow);
  }
  Draw();
}

void DrawAdditiveTextured(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                          IShaderShadow* pShaderShadow, bool doDetail) {
  SHADOW_STATE {
...
void DrawNonTextured(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                     IShaderShadow* pShaderShadow, bool doDetail) {
  SHADOW_STATE {
    SetModulationShadowState();
...
void DrawTextured(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
                  IShaderShadow* pShaderShadow, bool doDetail) {
  SHADOW_STATE {
    pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
    SetModulationShadowState();
...
SHADER_DRAW {
  ...
  bool doFirstPass =
      isTextureDefined || hasVertexColor || (!params[ENVMAP]->IsTexture());

  if (doFirstPass) {
    SHADOW_STATE {
      pShaderShadow->EnableAlphaTest(IS_FLAG_SET(MATERIAL_VAR_ALPHATEST));
      if (params[ALPHATESTREFERENCE]->IsDefined() &&
          params[ALPHATESTREFERENCE]->GetFloatValue() > 0.0f) {
    ...
    if (IS_FLAG_SET(MATERIAL_VAR_ADDITIVE)) {
      if (!isTextureDefined) {
        ...
        DrawAdditiveNonTextured(params, pShaderAPI, pShaderShadow,
                                hasDetailTexture);
      } else {
        ...
        bool onePassDetail =
            hasDetailTexture && (!hasVertexColor || !isModulating);
