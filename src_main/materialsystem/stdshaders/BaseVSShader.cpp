...
//
// Purpose: This is what all vs/ps (dx8+) shaders inherit from.

#if !defined(_STATIC_LINKED) || defined(STDSHADER_DX8_DLL_EXPORT) || \
    defined(STDSHADER_DX9_DLL_EXPORT)
...
void CBaseVSShader::SetPixelShaderConstant(int pixelReg, int constantVar,
  ...
  if ((!s_ppParams) || (constantVar == -1) || (constantVar2 == -1)) return;
...
void CBaseVSShader::SetPixelShaderConstantGammaToLinear(int pixelReg,
                                                        int constantVar,
                                                        int constantVar2) {
  Assert(!IsSnapshotting());
  if ((!s_ppParams) || (constantVar == -1) || (constantVar2 == -1)) return;
...
void CBaseVSShader::SetPixelShaderConstant_W(int pixelReg, int constantVar,
                                             float fWValue) {
  Assert(!IsSnapshotting());
  if ((!s_ppParams) || (constantVar == -1)) return;
...
void CBaseVSShader::SetVertexShaderConstantGammaToLinear(int var,
                                                         float const* pVec,
                                                         int numConst,
                                                         bool bForce) {
  int i;
  for (i = 0; i < numConst; i++) {
    float vec[4];
    vec[0] = pVec[i * 4 + 0] > 1.0f ? pVec[i * 4 + 0]
                                    : GammaToLinear(pVec[i * 4 + 0]);
    vec[1] = pVec[i * 4 + 1] > 1.0f ? pVec[i * 4 + 1]
                                    : GammaToLinear(pVec[i * 4 + 1]);
    vec[2] = pVec[i * 4 + 2] > 1.0f ? pVec[i * 4 + 2]
                                    : GammaToLinear(pVec[i * 4 + 2]);
    vec[3] = pVec[i * 4 + 3];
...
void CBaseVSShader::SetPixelShaderConstantGammaToLinear(int var,
                                                        float const* pVec,
                                                        int numConst,
                                                        bool bForce) {
  int i;
...
void CBaseVSShader::SetPixelShaderConstantFudge(int pixelReg, int constantVar) {
  ...
  if (pPixelVar->GetType() == MATERIAL_VAR_TYPE_VECTOR) {
    ...
    val[0] = val[1] = val[2] = val[3] =
        pPixelVar->GetFloatValue() * 0.992f + 0.0078f;
  s_pShaderAPI->SetPixelShaderConstant(pixelReg, val);
}
...
void CBaseVSShader::SetPixelShaderLightColors(int pixelReg) {
  ...
  for (i = 0; i < maxLights; i++) {
    ...
    if (lightDesc.m_Type != MATERIAL_LIGHT_DISABLE) {
      Vector color(lightDesc.m_Color[0], lightDesc.m_Color[1],
                   lightDesc.m_Color[2]);
...
//-----------------------------------------------------------------------------
void CBaseVSShader::SetVertexShaderTextureTranslation(int vertexReg,
                                                      int translationVar) {
  float offset[2] = {0, 0};
...
void CBaseVSShader::SetVertexShaderTextureTransform(int vertexReg,
                                                    int transformVar) {
...
void CBaseVSShader::SetVertexShaderTextureScaledTransform(int vertexReg,
                                                          int transformVar,
                                                          int scaleVar) {
  Vector4D transformation[2];
  IMaterialVar* pTransformationVar = s_ppParams[transformVar];
...
//-----------------------------------------------------------------------------
void CBaseVSShader::SetPixelShaderTextureTranslation(int pixelReg,
                                                     int translationVar) {
  float offset[2] = {0, 0};
...
void CBaseVSShader::SetPixelShaderTextureTransform(int pixelReg,
                                                   int transformVar) {
  Vector4D transformation[2];
  IMaterialVar* pTransformationVar = s_ppParams[transformVar];
  if (pTransformationVar &&
...
void CBaseVSShader::SetPixelShaderTextureScaledTransform(int pixelReg,
                                                         int transformVar,
                                                         int scaleVar) {
  Vector4D transformation[2];
  IMaterialVar* pTransformationVar = s_ppParams[transformVar];
...
void CBaseVSShader::SetVertexShaderMatrix3x4(int vertexReg, int matrixVar) {
  ...
  if (pTranslationVar) {
    s_pShaderAPI->SetVertexShaderConstant(
        vertexReg, &pTranslationVar->GetMatrixValue()[0][0], 3);
  } else {
    VMatrix matrix;
...
void CBaseVSShader::SetVertexShaderMatrix4x4(int vertexReg, int matrixVar) {
  ...
  if (pTranslationVar) {
    s_pShaderAPI->SetVertexShaderConstant(
        vertexReg, &pTranslationVar->GetMatrixValue()[0][0], 4);
  } else {
    VMatrix matrix;
    MatrixSetIdentity(matrix);
...
//-----------------------------------------------------------------------------
void CBaseVSShader::LoadProjectionMatrixIntoVertexShaderConstant(
    int vertexReg) {
  VMatrix mat, transpose;
...
//-----------------------------------------------------------------------------
void CBaseVSShader::EnablePixelShaderOverbright(int reg, bool bEnable,
                                                bool bDivideByTwo) {
...
void CBaseVSShader::SetModulationPixelShaderDynamicState_LinearColorSpace(
    int modulationVar) {
  float color[4] = {1.0, 1.0, 1.0, 1.0};
  ComputeModulationColor(color);
...
void CBaseVSShader::
    SetModulationPixelShaderDynamicState_LinearColorSpace_LinearScale(
        int modulationVar, float flScale) {
  float color[4] = {1.0, 1.0, 1.0, 1.0};
  ComputeModulationColor(color);
...
//-----------------------------------------------------------------------------
void CBaseVSShader::ColorVarsToVector(int colorVar, int alphaVar,
                                      Vector4D& color) {
...
//-----------------------------------------------------------------------------
void CBaseVSShader::SetColorVertexShaderConstant(int nVertexReg, int colorVar,
                                                 int alphaVar) {
  Vector4D color;
...
void CBaseVSShader::SetColorPixelShaderConstant(int nPixelReg, int colorVar,
                                                int alphaVar) {
  Vector4D color;
  ColorVarsToVector(colorVar, alphaVar, color);
...
void CBaseVSShader::SetEnvMapTintPixelShaderDynamicState(
    int pixelReg, int tintVar, int alphaVar, bool bConvertFromGammaToLinear) {
...
const char* CBaseVSShader::UnlitGeneric_ComputePixelShaderName(
    bool bMask, bool bEnvmap, bool bBaseTexture, bool bBaseAlphaEnvmapMask,
    bool bDetail, bool bDetailMultiplyMode, bool bMaskBaseByDetailAlpha) {
  ...
  // handle hud elements
  if (bDetail & bDetailMultiplyMode) return "alphadist_ps11";
  ...
  if (!bMask && bEnvmap && bBaseTexture && bBaseAlphaEnvmapMask) {
    ...
    int pshIndex = 0;
    if (bBaseTexture) pshIndex |= 0x1;
    if (bEnvmap) pshIndex |= 0x2;
    if (bMask) pshIndex |= 0x4;
    if (bDetail) pshIndex |= 0x8;
...
//-----------------------------------------------------------------------------
void CBaseVSShader::SetHWMorphVertexShaderState(
    int nDimConst, int nSubrectConst, VertexTextureSampler_t morphSampler) {
#ifndef _X360
  if (!s_pShaderAPI->IsHWMorphingEnabled()) return;

  int nMorphWidth, nMorphHeight;
  s_pShaderAPI->GetStandardTextureDimensions(&nMorphWidth, &nMorphHeight,
                                             TEXTURE_MORPH_ACCUMULATOR);

  float nDim = s_pShaderAPI->GetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_4TUPLE_COUNT);
  float pMorphAccumSize[4] = {nMorphWidth * 1.0f, nMorphHeight * 1.0f, nDim,
                              0.0f};
  s_pShaderAPI->SetVertexShaderConstant(nDimConst, pMorphAccumSize);

  float nXOffset = s_pShaderAPI->GetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_X_OFFSET);
  float nYOffset = s_pShaderAPI->GetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_Y_OFFSET);
  float nWidth = s_pShaderAPI->GetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_SUBRECT_WIDTH);
  float nHeight = s_pShaderAPI->GetIntRenderingParameter(
...
//-----------------------------------------------------------------------------
void CBaseVSShader::VertexShaderUnlitGenericPass(
    ...
    int alphaTestReferenceVar, int nDetailBlendModeVar, int nOutlineVar,
    int nOutlineColorVar, int nOutlineStartVar, int nOutlineEndVar,
  ...
  bool bBaseTexture =
      (baseTextureVar >= 0) && params[baseTextureVar]->IsTexture();
  ...
  bool bDetailMultiply = (nDetailBlendModeVar >= 0) &&
                         (params[nDetailBlendModeVar]->GetIntValue() == 8);
  bool bMaskBaseByDetailAlpha =
      (nDetailBlendModeVar >= 0) &&
      (params[nDetailBlendModeVar]->GetIntValue() == 9);
  bool bSeparateDetailUVs = (nSeparateDetailUVsVar >= 0) &&
                            (params[nSeparateDetailUVsVar]->GetIntValue() != 0);

  if (IsSnapshotting()) {
    // Alpha test
    s_pShaderShadow->EnableAlphaTest(IS_FLAG_SET(MATERIAL_VAR_ALPHATEST));

    if (alphaTestReferenceVar != -1 &&
        params[alphaTestReferenceVar]->GetFloatValue() > 0.0f) {
      s_pShaderShadow->AlphaFunc(
          SHADER_ALPHAFUNC_GEQUAL,
          params[alphaTestReferenceVar]->GetFloatValue());
    }
    ...
    if (bEnvmap) fmt |= VERTEX_NORMAL;
    if (bVertexColor) fmt |= VERTEX_COLOR;
    ...
    const char* pshName = UnlitGeneric_ComputePixelShaderName(
        bMask, bEnvmap, bBaseTexture, bBaseAlphaEnvmapMask, bDetail,
        bDetailMultiply, bMaskBaseByDetailAlpha);
    ...
    if (s_pShaderAPI
            ->InFlashlightMode())  // Not snapshotting && flashlight pass
    {
      Draw(false);
      return;
    }
    ...
    if (bDetail) {
      ...
      if (bDetailTransformIsScale) {
        SetVertexShaderTextureScaledTransform(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_4, baseTextureTransformVar,
            detailTransform);
      } else {
    ...
    if (bEnvmap) {
      ...
      if (bMask || bBaseAlphaEnvmapMask) {
        ...
        SetVertexShaderTextureScaledTransform(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_2, baseTextureTransformVar,
            envmapMaskScaleVar);
      }
    ...
    if (bDetailMultiply && (nOutlineVar != -1) &&
        (params[nOutlineVar]->GetIntValue())) {
    ...
    vshIndex.SetDOWATERFOG(s_pShaderAPI->GetSceneFogMode() ==
...
void CBaseVSShader::DrawWorldBaseTexture(int baseTextureVar,
                                         int baseTextureTransformVar,
                                         int frameVar, int colorVar,
                                         int alphaVar) {
  if (IsSnapshotting()) {
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
    s_pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION, 1, 0, 0);
    ...
    IMaterialVar** params = s_ppParams;
    bool bLightingOnly = mat_fullbright.GetInt() == 2 &&
                         !IS_FLAG_SET(MATERIAL_VAR_NO_DEBUG_OVERRIDE);
    if (bLightingOnly) {
...
void CBaseVSShader::DrawWorldBumpedDiffuseLighting(int bumpmapVar,
                                                   int bumpFrameVar,
  ...
  if (IsSnapshotting()) {
    ...
    if (bSSBump)
      s_pShaderShadow->SetPixelShader(
          "LightmappedGeneric_SSBumpmappedLightmap");
    else
    ...
    if (!g_pConfig->m_bFastNoBump) {
      ...
      s_pShaderAPI->BindStandardTexture(SHADER_SAMPLER0,
...
void CBaseVSShader::DrawWorldBumpedDiffuseLighting_Base_ps14(
    int bumpmapVar, int bumpFrameVar, int bumpTransformVar, int baseTextureVar,
    int baseTextureTransformVar, int frameVar) {
  if (IsSnapshotting()) {
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER1, true);
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER2, true);
    ...
    s_pShaderShadow->SetVertexShader(
        "LightmappedGeneric_BumpmappedLightmap_Base_ps14", vshIndex.GetIndex());

    s_pShaderShadow->SetPixelShader(
        "LightmappedGeneric_BumpmappedLightmap_Base_ps14");
    FogToFogColor();
  } else {
    if (!g_pConfig->m_bFastNoBump) {
      BindTexture(SHADER_SAMPLER0, bumpmapVar, bumpFrameVar);
    ...
    SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                                    bumpTransformVar);
    SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_2,
                                    baseTextureTransformVar);
    SetModulationPixelShaderDynamicState(3);
...
void CBaseVSShader::DrawWorldBumpedDiffuseLighting_Blend_ps14(
    int bumpmapVar, int bumpFrameVar, int bumpTransformVar, int baseTextureVar,
    int baseTextureTransformVar, int baseTextureFrameVar, int baseTexture2Var,
    int baseTextureTransform2Var, int baseTextureFrame2Var) {
  if (IsSnapshotting()) {
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER0, true);
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER1, true);
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER2, true);
    ...
    s_pShaderShadow->SetVertexShader(
        "LightmappedGeneric_BumpmappedLightmap_Blend_ps14",
        vshIndex.GetIndex());
...
void CBaseVSShader::DrawWorldBumpedSpecularLighting(
    int bumpmapVar, int envmapVar, int bumpFrameVar, int envmapFrameVar,
    int envmapTintVar, int alphaVar, int envmapContrastVar,
    int envmapSaturationVar, int bumpTransformVar, int fresnelReflectionVar,
  ...
  if (IsSnapshotting()) {
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_1_4()) {
      s_pShaderAPI->BindStandardTexture(SHADER_SAMPLER4,
...
void CBaseVSShader::DrawModelBumpedSpecularLighting(
    int bumpMapVar, int bumpMapFrameVar, int envMapVar, int envMapVarFrame,
    int envMapTintVar, int alphaVar, int envMapContrastVar,
    int envMapSaturationVar, int bumpTransformVar, bool bBlendSpecular,
    bool bNoWriteZ) {
  IMaterialVar** params = s_ppParams;
  ...
  if (IsSnapshotting()) {
    ...
    s_pShaderShadow->VertexShaderVertexFormat(VERTEX_POSITION | VERTEX_NORMAL,
                                              1, 0, 4 /* userDataSize */);
    ...
    if (g_pHardwareConfig->SupportsPixelShaders_1_4()) {
      ...
      if (bHasNormalMapAlphaEnvMapMask) {
        s_pShaderShadow->SetPixelShader(
            "VertexLitGeneric_EnvmappedBumpmapV2_MultByAlpha_ps14");
      } else {
        s_pShaderShadow->SetPixelShader(
      ...
      s_pShaderShadow->SetVertexShader(
          "VertexLitGeneric_EnvmappedBumpmap_NoLighting", vshIndex.GetIndex());
      // This version does not multiply by lighting
      // NOTE: We don't support multiplying by lighting for bumped specular
      // stuff.
      if (bHasNormalMapAlphaEnvMapMask) {
        s_pShaderShadow->SetPixelShader(
            "VertexLitGeneric_EnvmappedBumpmapV2_MultByAlpha");
      } else {
...
void CBaseVSShader::DrawBaseTextureBlend(
...
void CBaseVSShader::DrawWorldBumpedUsingVertexShader(
    int baseTextureVar, int baseTextureTransformVar, int bumpmapVar,
    int bumpFrameVar, int bumpTransformVar, int envmapMaskVar,
    int envmapMaskFrame, int envmapVar, int envmapFrameVar, int envmapTintVar,
    int colorVar, int alphaVar, int envmapContrastVar, int envmapSaturationVar,
    int frameVar, int fresnelReflectionVar, bool doBaseTexture2,
    int baseTexture2Var, int baseTextureTransform2Var, int baseTextureFrame2Var,
    bool bSSBump) {
  IMaterialVar** params = s_ppParams;
  // Draw base texture
  ...
  if (doBaseTexture2 && params[baseTexture2Var]->IsTexture() &&
      params[baseTextureVar]->IsTexture()) {
    DrawBaseTextureBlend(baseTextureVar, baseTextureTransformVar, frameVar,
                         baseTexture2Var, baseTextureTransform2Var,
                         baseTextureFrame2Var, colorVar, alphaVar);
  ...
  if (params[envmapVar]->IsTexture()) {
    DrawWorldBumpedSpecularLighting(
        bumpmapVar, envmapVar, bumpFrameVar, envmapFrameVar, envmapTintVar,
        alphaVar, envmapContrastVar, envmapSaturationVar, bumpTransformVar,
        fresnelReflectionVar, bBlendSpecular);
  }
}
...
//-----------------------------------------------------------------------------
BlendType_t CBaseVSShader::EvaluateBlendRequirements(int textureVar,
                                                     bool isBaseTexture,
  ...
  isTranslucent =
      isTranslucent || (CurrentMaterialVarFlags() & MATERIAL_VAR_VERTEXALPHA);
...
void CBaseVSShader::SetFlashlightVertexShaderConstants(
    bool bBump, int bumpTransformVar, bool bDetail, int detailScaleVar,
  ...
  pos[3] = 1.0f / ((0.6f * flashlightState.m_FarZ) -
                   flashlightState.m_FarZ);  // DX8 needs this

  s_pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                                        pos, 1);

  s_pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_1,
                                        worldToTexture.Base(), 4);
  ...
  s_pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_5,
                                        atten, 1);

  if (bDetail) {
    SetVertexShaderTextureScaledTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_8,
                                          BASETEXTURETRANSFORM, detailScaleVar);
  }

  if (bSetTextureTransforms) {
    SetVertexShaderTextureTransform(VERTEX_SHADER_SHADER_SPECIFIC_CONST_6,
                                    BASETEXTURETRANSFORM);
    if (!bDetail && bBump && bumpTransformVar != -1) {
      SetVertexShaderTextureTransform(
          VERTEX_SHADER_SHADER_SPECIFIC_CONST_8,
          bumpTransformVar);  // aliased on top of detail transform
    }
  }
}

#if SUPPORT_DX8
void CBaseVSShader::DrawFlashlight_dx80(
    IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
    IShaderShadow* pShaderShadow, bool bBump, int bumpmapVar, int bumpmapFrame,
    int bumpTransform, int flashlightTextureVar, int flashlightTextureFrameVar,
    bool bLightmappedGeneric, bool bWorldVertexTransition,
    int nWorldVertexTransitionPassID, int baseTexture2Var,
    int baseTexture2FrameVar, bool bTeeth, int nTeethForwardVar,
    int nTeethIllumFactorVar) {
  // FLASHLIGHTFIXME: hack . . need to fix the vertex shader so that it can deal
  // with and without bumps for vertexlitgeneric
  ...
  if (pShaderShadow) {
    ...
    if (bWorldVertexTransition) {
      // use separate alpha blend to make sure that we aren't adding alpha from
      // source
    ...
    if (bLightmappedGeneric) {
      ...
      pShaderShadow->SetVertexShader("lightmappedgeneric_flashlight_vs11",
                                     vshIndex.GetIndex());
      ...
      pShaderShadow->SetVertexShader("vertexlitgeneric_flashlight_vs11",
                                     vshIndex.GetIndex());
    ...
    // Specify that we have XYZ texcoords that need to be divided by W before
    // the pixel shader. NOTE Tried to divide XY by Z, but doesn't work. The
    // dx9.0c runtime says that we shouldn't have a non-zero dimension when
    // using vertex and pixel shaders.
    pShaderAPI->SetTextureTransformDimension(SHADER_TEXTURE_STAGE0, 0, true);
    BindTexture(SHADER_SAMPLER0, flashlightTextureVar,
                flashlightTextureFrameVar);
    ...
    pShaderAPI->BindStandardTexture(SHADER_SAMPLER2,
                                    TEXTURE_NORMALIZATION_CUBEMAP);
    if (bBump) {
    ...
    if (bLightmappedGeneric) {
      ...
      vshIndex.SetDOWATERFOG(pShaderAPI->GetSceneFogMode() ==
                             MATERIAL_FOG_LINEAR_BELOW_FOG_Z);
      ...
      if (bTeeth) {
        ...
        pShaderAPI->SetVertexShaderConstant(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_10, lighting.Base());
      }
...
#ifdef STDSHADER_DX9_DLL_EXPORT
void CBaseVSShader::DrawFlashlight_dx90(IMaterialVar** params,
  ...
  bool bBump2 = vars.m_bWorldVertexTransition && vars.m_bBump &&
                vars.m_nBumpmap2Var != -1 &&
                params[vars.m_nBumpmap2Var]->IsTexture();
  bool bSeamless = vars.m_fSeamlessScale != 0.0;
  bool bDetail = vars.m_bLightmappedGeneric && (vars.m_nDetailVar != -1) &&
                 params[vars.m_nDetailVar]->IsDefined() &&
  ...
  if (pShaderShadow) {
    ...
    if (vars.m_nAlphaTestReference != -1 &&
        params[vars.m_nAlphaTestReference]->GetFloatValue() > 0.0f) {
      pShaderShadow->AlphaFunc(
          SHADER_ALPHAFUNC_GEQUAL,
          params[vars.m_nAlphaTestReference]->GetFloatValue());
    }
    ...
    if (vars.m_bLightmappedGeneric) {
      ...
      vshIndex.SetDETAIL(bDetail);
      pShaderShadow->SetVertexShader("lightmappedgeneric_flashlight_vs20",
                                     vshIndex.GetIndex());
      ...
      unsigned int flags = VERTEX_POSITION | VERTEX_NORMAL;
      int numTexCoords = 1;
      pShaderShadow->VertexShaderVertexFormat(flags, numTexCoords, 0,
                                              vars.m_bBump ? 4 : 0);
    }
    ...
    FlashlightState_t flashlightState = pShaderAPI->GetFlashlightStateEx(
        worldToTexture, &pFlashlightDepthTexture);

    SetFlashLightColorFromState(flashlightState, pShaderAPI);

    BindTexture(SHADER_SAMPLER0, flashlightState.m_pSpotlightTexture,
    ...
    if (pFlashlightDepthTexture && g_pConfig->ShadowDepthTexture() &&
      ...
      HashShadow2DJitter(flashlightState.m_flShadowJitterSeed, &tweaks[2],
                         &tweaks[3]);
      pShaderAPI->SetPixelShaderConstant(PSREG_ENVMAP_TINT__SHADOW_TWEAKS,
                                         tweaks, 1);
      ...
      pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_SCREEN_SCALE,
                                         vScreenScale, 1);
    }
    ...
    if (vars.m_bWorldVertexTransition) {
      ...
      BindTexture(SHADER_SAMPLER4, vars.m_nBaseTexture2Var,
                  vars.m_nBaseTexture2FrameVar);
    }
    pShaderAPI->BindStandardTexture(SHADER_SAMPLER2,
                                    TEXTURE_NORMALIZATION_CUBEMAP);
    if (vars.m_bBump) {
    ...
    if (vars.m_bWorldVertexTransition) {
      if (bBump2) {
        BindTexture(SHADER_SAMPLER6, vars.m_nBumpmap2Var,
                    vars.m_nBumpmap2Frame);
      }
    }
    ...
    if (vars.m_bLightmappedGeneric) {
      ...
      SET_DYNAMIC_VERTEX_SHADER_COMBO(
          DOWATERFOG,
          pShaderAPI->GetSceneFogMode() == MATERIAL_FOG_LINEAR_BELOW_FOG_Z);
      SET_DYNAMIC_VERTEX_SHADER(lightmappedgeneric_flashlight_vs20);
      if (bSeamless) {
        float const0[4] = {vars.m_fSeamlessScale, 0, 0, 0};
        pShaderAPI->SetVertexShaderConstant(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_6, const0);
      }
      ...
      if (bDetail) {
        ...
        if (vars.m_nDetailTextureBlendFactor != -1) {
          vDetailConstants[3] =
              params[vars.m_nDetailTextureBlendFactor]->GetFloatValue();
        }
      ...
      if (vars.m_bTeeth) {
        ...
        pShaderAPI->SetVertexShaderConstant(
            VERTEX_SHADER_SHADER_SPECIFIC_CONST_0, lighting.Base());
      }
    }
    ...
    pShaderAPI->SetPixelShaderConstant(PSREG_EYEPOS_SPEC_EXPONENT,
                                       vEyePos_SpecExponent, 1);
    ...
    atten[2] = flashlightState.m_fQuadraticAtten;
    atten[3] = flashlightState.m_FarZ;
    s_pShaderAPI->SetPixelShaderConstant(PSREG_FLASHLIGHT_ATTENUATION, atten,
                                         1);

    SetFlashlightVertexShaderConstants(vars.m_bBump, vars.m_nBumpTransform,
                                       bDetail, vars.m_nDetailScale,
                                       bSeamless ? false : true);
  }
  Draw();
}

// Take 0..1 seed and map to (u, v) coordinate to be used in shadow filter
// jittering...
void CBaseVSShader::HashShadow2DJitter(const float fJitterSeed, float* fU,
                                       float* fV) {
...
void CBaseVSShader::InitParamsUnlitGeneric_DX8(int baseTextureVar,
                                               int detailScaleVar,
                                               int envmapOptionalVar,
                                               int envmapVar, int envmapTintVar,
  ...
  if (envmapOptionalVar >= 0 && params[envmapOptionalVar]->IsDefined() &&
      params[envmapOptionalVar]->GetIntValue()) {
    if (envmapVar >= 0) {
  ...
  if (envmapVar >= 0 && baseTextureVar >= 0 && !g_pConfig->UseSpecular() &&
      params[envmapVar]->IsDefined() && params[baseTextureVar]->IsDefined()) {
    params[envmapVar]->SetUndefined();
  }
}

void CBaseVSShader::InitUnlitGeneric_DX8(int baseTextureVar, int detailVar,
                                         int envmapVar, int envmapMaskVar) {
  IMaterialVar** params = s_ppParams;
