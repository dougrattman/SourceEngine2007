...
BEGIN_VS_SHADER_FLAGS(MorphAccumulate_DX9, "Help for MorphAccumulate",
                      SHADER_NOT_EDITABLE)
...
SHADER_DRAW {
  bool bUseConstantBasedAccum =
      (g_pHardwareConfig->NumVertexShaderConstants() >=
  ...
  SHADOW_STATE {
    ...
    int pTexCoord[2] = {4, 1};
    pShaderShadow->VertexShaderVertexFormat(VERTEX_FORMAT_USE_EXACT_FORMAT, 2,
                                            pTexCoord, 0);
  }
  DYNAMIC_STATE {
    ...
    if (!bUseConstantBasedAccum) {
      ...
      float nXOffset = pShaderAPI->GetIntRenderingParameter(
          INT_RENDERPARM_MORPH_WEIGHT_X_OFFSET);
      float nYOffset = pShaderAPI->GetIntRenderingParameter(
          INT_RENDERPARM_MORPH_WEIGHT_Y_OFFSET);
      float nSubrectWidth = pShaderAPI->GetIntRenderingParameter(
          INT_RENDERPARM_MORPH_WEIGHT_SUBRECT_WIDTH);
      float nSubrectHeight = pShaderAPI->GetIntRenderingParameter(
          INT_RENDERPARM_MORPH_WEIGHT_SUBRECT_HEIGHT);
      float pMorphWeightSubrect[4] = {nXOffset, nYOffset, nSubrectWidth,
                                      nSubrectHeight};
      pShaderAPI->SetVertexShaderConstant(VERTEX_SHADER_SHADER_SPECIFIC_CONST_0,
                                          pMorphWeightSubrect);

      int nWidth, nHeight;
      pShaderAPI->GetStandardTextureDimensions(&nWidth, &nHeight,
                                               TEXTURE_MORPH_WEIGHTS);
      float pMorphWeightDim[4] = {nWidth * 1.0f, nHeight * 1.0f, 0.0f, 0.0f};
