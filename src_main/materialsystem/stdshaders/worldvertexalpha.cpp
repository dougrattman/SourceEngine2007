...
SHADER_FALLBACK { return "WorldVertexAlpha_DX8"; }
  ...
  if (g_pHardwareConfig->SupportsVertexAndPixelShaders() &&
    ...
    if (g_pHardwareConfig->GetDXSupportLevel() < 90) {
      ...
      SHADOW_STATE {
        ...
        // Looks backwards, but this is done so that lightmap alpha = 1 when
        // only using 1 texture (needed for translucent displacements).
      ...
      SHADOW_STATE {
        ...
        pShaderShadow->BlendFuncSeparateAlpha(SHADER_BLEND_ZERO,
                                              SHADER_BLEND_SRC_ALPHA);
      ...
      SHADOW_STATE {
        ...
        pShaderShadow->BlendFunc(SHADER_BLEND_ONE_MINUS_SRC_ALPHA,
                                 SHADER_BLEND_SRC_ALPHA);
        pShaderShadow->EnableBlendingSeparateAlpha(true);
        pShaderShadow->BlendFuncSeparateAlpha(SHADER_BLEND_ONE,
                                              SHADER_BLEND_ONE);
    ...
    SHADOW_STATE {
      ...
      // SHADER_BLEND_ONE_MINUS_SRC_ALPHA, SHADER_BLEND_SRC_ALPHA );

      // Use vertex color for Hammer because it puts the blending alpha in the
      // vertices.
      ...
      pShaderShadow->DrawFlags(colorFlag | SHADER_DRAW_POSITION |
