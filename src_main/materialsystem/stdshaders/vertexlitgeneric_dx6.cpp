...
SHADER_PARAM(ENVMAPOPTIONAL, SHADER_PARAM_TYPE_BOOL, "0",
             "Make the envmap only apply to dx9 and higher hardware")
END_SHADER_PARAMS
...
int GetDrawFlagsPass1(IMaterialVar** params) {
  ...
  if (params[BASETEXTURE]->IsTexture()) flags |= SHADER_DRAW_TEXCOORD0;
  return flags;
}

void DrawVertexLightingOnly(IMaterialVar** params,
  ...
  DYNAMIC_STATE { SetModulationDynamicState(); }
  Draw();
}

void MultiplyByVertexLighting(IMaterialVar** params,
                              IShaderDynamicAPI* pShaderAPI,
                              IShaderShadow* pShaderShadow) {
  SHADOW_STATE {
    ...
    // This here will perform color = vertex light * (cc alpha) + 1 * (1 - cc
    // alpha)
    pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_COLOR,
        SHADER_TEXOP_BLEND_CONSTANTALPHA, SHADER_TEXARG_VERTEXCOLOR,
...
void DrawBaseTimesVertexLighting(IMaterialVar** params,
                                 IShaderDynamicAPI* pShaderAPI,
                                 IShaderShadow* pShaderShadow) {
...
void DrawEnvmapTimesVertexLighting(IMaterialVar** params,
                                   IShaderDynamicAPI* pShaderAPI,
                                   IShaderShadow* pShaderShadow) {
  SHADOW_STATE {
    ...
    pShaderShadow->OverbrightValue(
        hasEnvMapMask ? SHADER_TEXTURE_STAGE1 : SHADER_TEXTURE_STAGE0,
        OVERBRIGHT);
    ...
    pShaderShadow->EnableAlphaPipe(true);
    pShaderShadow->EnableConstantAlpha(IsAlphaModulating());
    pShaderShadow->EnableVertexAlpha(
        (materialVarFlags & MATERIAL_VAR_VERTEXALPHA) != 0);
    if (hasEnvMapMask)
  ...
  DYNAMIC_STATE {
    SetDynamicEnvMappingState(ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME,
                              ENVMAPMASKFRAME, FRAME, BASETEXTURETRANSFORM,
                              ENVMAPMASKSCALE);
  }
  Draw();
...
void DrawMode1(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
               IShaderShadow* pShaderShadow) {
  ...
  if (texDefined) {
    FixedFunctionBaseTimesDetailPass(BASETEXTURE, FRAME, BASETEXTURETRANSFORM,
                                     DETAIL, DETAILSCALE);
  } else {
    FixedFunctionMaskedEnvmapPass(ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME,
                                  ENVMAPMASKFRAME, FRAME, BASETEXTURETRANSFORM,
                                  ENVMAPMASKSCALE, ENVMAPTINT);
  }
  ...
  if (IS_FLAG_SET(MATERIAL_VAR_MULTIPASS)) {
    if (texDefined && envDefined) {
      FixedFunctionAdditiveMaskedEnvmapPass(
          ENVMAP, ENVMAPMASK, BASETEXTURE, ENVMAPFRAME, ENVMAPMASKFRAME, FRAME,
  ...
  if (IS_FLAG_SET(MATERIAL_VAR_SELFILLUM)) {
    FixedFunctionSelfIlluminationPass(SHADER_SAMPLER0, BASETEXTURE, FRAME,
                                      BASETEXTURETRANSFORM, SELFILLUMTINT);
  }
}

void DrawMode0(IMaterialVar** params, IShaderDynamicAPI* pShaderAPI,
               IShaderShadow* pShaderShadow) {
  ...
  if (params[BASETEXTURE]->IsTexture()) {
    ...
    // Detail map
    FixedFunctionMultiplyByDetailPass(BASETEXTURE, FRAME, BASETEXTURETRANSFORM,
                                      DETAIL, DETAILSCALE);
