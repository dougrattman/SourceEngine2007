...
BEGIN_VS_SHADER(Eyes_dx8, "Help for Eyes")

BEGIN_SHADER_PARAMS
SHADER_PARAM_OVERRIDE(BASETEXTURE, SHADER_PARAM_TYPE_TEXTURE,
                      "models/alyx/eyeball_l", "iris texture", 0)
SHADER_PARAM(IRIS, SHADER_PARAM_TYPE_TEXTURE, "models/alyx/pupil_l",
             "iris texture")
SHADER_PARAM(IRISFRAME, SHADER_PARAM_TYPE_INTEGER, "0",
             "frame for the iris texture")
SHADER_PARAM(GLINT, SHADER_PARAM_TYPE_TEXTURE, "models/humans/male/glint",
             "glint texture")
...
SHADER_PARAM(IRISU, SHADER_PARAM_TYPE_VEC4, "[0 1 0 0]",
             "U projection vector for the iris")
SHADER_PARAM(IRISV, SHADER_PARAM_TYPE_VEC4, "[0 0 1 0]",
             "V projection vector for the iris")
SHADER_PARAM(GLINTU, SHADER_PARAM_TYPE_VEC4, "[0 1 0 0]",
             "U projection vector for the glint")
SHADER_PARAM(GLINTV, SHADER_PARAM_TYPE_VEC4, "[0 0 1 0]",
             "V projection vector for the glint")
...
bool NeedsPowerOfTwoFrameBufferTexture(IMaterialVar **params,
                                       bool bCheckSpecificToThisFrame) const {
  if (params[CLOAKPASSENABLED]->GetIntValue())  // If material supports cloaking
  {
    if (bCheckSpecificToThisFrame ==
        false)  // For setting model flag at load time
      return true;
    else if ((params[CLOAKFACTOR]->GetFloatValue() > 0.0f) &&
             (params[CLOAKFACTOR]->GetFloatValue() < 1.0f))  // Per-frame check
      return true;
    // else, not cloaking this frame, so check flag2 in case the base material
    // still needs it
  }
...
bool IsTranslucent(IMaterialVar **params) const {
  if (params[CLOAKPASSENABLED]->GetIntValue())  // If material supports cloaking
  {
    if ((params[CLOAKFACTOR]->GetFloatValue() > 0.0f) &&
        (params[CLOAKFACTOR]->GetFloatValue() < 1.0f))  // Per-frame check
      return true;
    // else, not cloaking this frame, so check flag in case the base material
    // still needs it
  }
...
SHADER_INIT_PARAMS() {
  ...
  if (!params[CLOAKPASSENABLED]->IsDefined()) {
    ...
    CloakBlendedPassVars_t local_info;
    SetupVarsCloakBlendedPass(local_info);
    InitParamsCloakBlendedPass(this, params, pMaterialName, local_info);
  }
}
...
SHADER_INIT {
  ...
  if (params[CLOAKPASSENABLED]->GetIntValue()) {
    CloakBlendedPassVars_t local_info;
    SetupVarsCloakBlendedPass(local_info);
    InitCloakBlendedPass(this, params, local_info);
  }
}
...
SHADER_DRAW {
  ...
  if (params[CLOAKPASSENABLED]->GetIntValue() &&
      (pShaderShadow == NULL))  // && not snapshotting
  {
    CloakBlendedPassVars_t info;
    SetupVarsCloakBlendedPass(info);
    if (CloakBlendedPassIsFullyOpaque(params, info)) {
      // There is some strangeness in DX8 when trying to skip the main pass, so
  ...
  if (bDrawStandardPass) {
    Eyes_DX8_DX9_Vars_t info;
    SetupVars(info);
    DrawEyes_DX8_DX9(false, this, params, pShaderAPI, pShaderShadow, info,
                     vertexCompression);
  } else {
    // Skip this pass!
    Draw(false);
  ...
  if (params[CLOAKPASSENABLED]->GetIntValue()) {
    ...
    if ((pShaderShadow != NULL) ||
        ((params[CLOAKFACTOR]->GetFloatValue() > 0.0f) &&
         (params[CLOAKFACTOR]->GetFloatValue() < 1.0f))) {
      CloakBlendedPassVars_t info;
      SetupVarsCloakBlendedPass(info);
