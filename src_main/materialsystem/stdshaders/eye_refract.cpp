...
BEGIN_SHADER_PARAMS
  ...
  SHADER_PARAM(CORNEATEXTURE, SHADER_PARAM_TYPE_TEXTURE,
               "shadertest/BaseTexture", "cornea texture")
  SHADER_PARAM(AMBIENTOCCLTEXTURE, SHADER_PARAM_TYPE_TEXTURE,
               "shadertest/BaseTexture", "reflection texture")
...
SHADER_INIT_PARAMS() {
  ...
  if (!params[CLOAKPASSENABLED]->IsDefined()) {
    ...
    CloakBlendedPassVars_t theInfo;
    SetupVarsCloakBlendedPass(theInfo);
    InitParamsCloakBlendedPass(this, params, pMaterialName, theInfo);
  }
  ...
  if (!params[EMISSIVEBLENDENABLED]->IsDefined()) {
    ...
    EmissiveScrollBlendedPassVars_t theInfo;
    SetupVarsEmissiveScrollBlendedPass(theInfo);
    InitParamsEmissiveScrollBlendedPass(this, params, pMaterialName, theInfo);
  }
}
...
SHADER_INIT {
  ...
  if (params[CLOAKPASSENABLED]->GetIntValue()) {
    CloakBlendedPassVars_t theInfo;
    SetupVarsCloakBlendedPass(theInfo);
    InitCloakBlendedPass(this, params, theInfo);
  }
  ...
  if (params[EMISSIVEBLENDENABLED]->GetIntValue()) {
    EmissiveScrollBlendedPassVars_t theInfo;
    SetupVarsEmissiveScrollBlendedPass(theInfo);
    InitEmissiveScrollBlendedPass(this, params, theInfo);
  }
}
...
SHADER_DRAW {
  ...
  if (params[EMISSIVEBLENDENABLED]->GetIntValue()) {
    // If ( snapshotting ) or ( we need to draw this frame )
    if ((pShaderShadow != NULL) ||
        (params[EMISSIVEBLENDSTRENGTH]->GetFloatValue() > 0.0f)) {
      EmissiveScrollBlendedPassVars_t info;
      SetupVarsEmissiveScrollBlendedPass(info);
      DrawEmissiveScrollBlendedPass(this, params, pShaderAPI, pShaderShadow,
