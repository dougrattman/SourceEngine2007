...
the_interface ITextureInternal : public ITexture {
 public:
  ...
  virtual void Bind(Sampler_t sampler1, int nFrame,
                    Sampler_t sampler2 = (Sampler_t)-1) = 0;
  ...
  // Called before Download() on restore. Gives render targets a change to
  // change whether or not they force themselves to have a separate depth buffer
  // due to AA.
  ...
  virtual ShaderAPITextureHandle_t GetTextureHandle(
      int nFrame, int nTextureChannel = 0) = 0;
  ...
  static ITextureInternal *CreateFileTexture(const char *pFileName,
  ...
  static void ChangeRenderTarget(
      ITextureInternal * pTexture, int w, int h,
      RenderTargetSizeMode_t sizeMode, ImageFormat fmt, RenderTargetType_t type,
  ...
  virtual void BindVertexTexture(VertexTextureSampler_t sampler,
                                 int frameNum = 0) = 0;
...
//-----------------------------------------------------------------------------
inline char *NormalizeTextureName(const char *pName, char *pOutName,
                                  int nOutNameSize) {
  ...
  Q_strlower(pOutName, nOutNameSize);
