...
class CMaterialSubRect : public IMaterialInternal {
 public:
  ...
  CMaterialSubRect(char const *pMaterialName, const char *pTextureGroupName,
                   KeyValues *pVMTKeyValues, KeyValues *pPatchKeyValues,
  ...
  void GetReflectivity(Vector &reflect) {
    m_pMaterialPage->GetReflectivity(reflect);
  }
  ...
  bool PrecacheVars(KeyValues *pKeyValues = NULL,
  ...
  PreviewImageRetVal_t GetPreviewImageProperties(int *width, int *height,
    ...
    return m_pMaterialPage->GetPreviewImageProperties(
        width, height, imageFormat, isTranslucent);
  }
  ...
  ShaderRenderState_t *GetRenderState() {
    return m_pMaterialPage->GetRenderState();
  }
  int GetNumAnimationFrames() {
    return m_pMaterialPage->GetNumAnimationFrames();
  }
  ...
  bool GetPropertyFlag(MaterialPropertyTypes_t type) {
    return m_pMaterialPage->GetPropertyFlag(type);
  }
  ...
  int GetMinLightmapPageID() const {
    return m_pMaterialPage->GetMinLightmapPageID();
  }
  int GetMaxLightmapPageID() const {
    return m_pMaterialPage->GetMaxLightmapPageID();
  }
  ...
  bool GetNeedsWhiteLightmap() const {
    return m_pMaterialPage->GetNeedsWhiteLightmap();
  }
  ...
  void SetShader(const char *pShaderName) {
    m_pMaterialPage->SetShader(pShaderName);
  }
  ...
  VertexFormat_t GetVertexFormat() const {
    return m_pMaterialPage->GetVertexFormat();
  }

  // Gets the morph format
  virtual MorphFormat_t GetMorphFormat() const {
    return m_pMaterialPage->GetMorphFormat();
  }
  ...
  VertexFormat_t GetVertexUsage() const {
    return m_pMaterialPage->GetVertexUsage();
  }

  // Debugs this material
  bool PerformDebugTrace() const {
    return m_pMaterialPage->PerformDebugTrace();
  }
  ...
  void RefreshPreservingMaterialVars() {
    m_pMaterialPage->RefreshPreservingMaterialVars();
  }
  ...
  void SetUseFixedFunctionBakedLighting(bool bEnable) {
    m_pMaterialPage->SetUseFixedFunctionBakedLighting(bEnable);
  }
  bool NeedsFixedFunctionFlashlight() const {
    return m_pMaterialPage->NeedsFixedFunctionFlashlight();
  }

  virtual void DecideShouldReloadFromWhitelist(IFileList *pFileList) {
    m_pMaterialPage->DecideShouldReloadFromWhitelist(pFileList);
  }
  virtual void ReloadFromWhitelistIfMarked() {
    return m_pMaterialPage->ReloadFromWhitelistIfMarked();
  }
  virtual bool WasReloadedFromWhitelist() {
    return m_pMaterialPage->WasReloadedFromWhitelist();
  }

  bool IsUsingVertexID() const { return m_pMaterialPage->IsUsingVertexID(); }

  virtual void ReportVarChanged(IMaterialVar *pVar) {
    m_pMaterialPage->ReportVarChanged(pVar);
  }
...
//-----------------------------------------------------------------------------
IMaterialInternal *IMaterialInternal::CreateMaterialSubRect(
    char const *pMaterialName, const char *pTextureGroupName,
    KeyValues *pVMTKeyValues, KeyValues *pPatchKeyValues,
    bool bAssumeCreateFromFile) {
  return new CMaterialSubRect(pMaterialName, pTextureGroupName, pVMTKeyValues,
                              pPatchKeyValues, bAssumeCreateFromFile);
}
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMaterialSubRect::CMaterialSubRect(const char *pMaterialName,
...
//-----------------------------------------------------------------------------
int CMaterialSubRect::GetMappingWidth() { return m_vecSize.x; }

//-----------------------------------------------------------------------------
// Purpose: Return the size of the subrect not the texture page size (height).
//-----------------------------------------------------------------------------
int CMaterialSubRect::GetMappingHeight() { return m_vecSize.y; }
...
void CMaterialSubRect::ParseMaterialVars(KeyValues &keyValues) {
  ...
  if (!pShaderName) {
    DevWarning(1,
               "CMaterialSubRect::InitializeShader: Shader not specified in "
               "material %s.\n",
               GetName());
    Assert(0);
  ...
  if (!Q_strcmp(pShaderName, "Subrect")) {
    ...
    while (pVar) {
      ...
      if (!Q_strcmp(pVar->GetName(), "$Material")) {
        m_pMaterialPage =
            static_cast<IMaterialInternal *>(MaterialSystem()->FindMaterial(
                pVar->GetString(), TEXTURE_GROUP_DECAL));
        m_pMaterialPage =
            m_pMaterialPage
                ->GetRealTimeVersion();  // always work with the realtime
                                         // material internally
      }
...
void CMaterialSubRect::SetupMaterialVars() {
  ...
  // Add 0.5f to push the image "in" by 1/2 a texel, and subtract 1.0f to push
  // it "in" by 1/2 a texel on the other side.
  m_vecOffset.x += 1.0f;
  m_vecOffset.y += 1.0f;
...
IMaterialVar *CMaterialSubRect::FindVarFast(char const *pVarName,
                                            unsigned int *pToken) {
  // Look for the var in the material page - it has precedence.
...
//-----------------------------------------------------------------------------
static IMaterialVar *CreateVectorMaterialVarFromKeyValue(IMaterial *pMaterial,
                                                         KeyValues *pKeyValue) {
  float vecVal[4];
...
static IMaterialVar *CreateMaterialVarFromKeyValue(IMaterial *pMaterial,
  ...
  switch (pKeyValue->GetDataType()) {
    case KeyValues::TYPE_INT: {
      return IMaterialVar::Create(pMaterial, pKeyValue->GetName(),
                                  pKeyValue->GetInt());
    }
