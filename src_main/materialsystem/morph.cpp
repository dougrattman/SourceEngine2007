...
class CVertexMorphDict {
 public:
  ...
  const MorphVertexInfo_t &GetMorphVertexInfo(int nMorphTargetIndex,
                                              int nIndex) const;
 ...
 private:
  ...
  class CMorphVertexListLess {
   public:
    bool Less(const MorphVertexInfo_t &src1, const MorphVertexInfo_t &src2,
              void *pCtx) {
  ...
  struct MorphVertexList_t {
    MorphVertexList_t() {}
    MorphVertexList_t(const MorphVertexList_t &src)
  ...
  class VertexMorphDictLess {
   public:
    bool Less(const MorphVertexList_t &src1, const MorphVertexList_t &src2,
              void *pCtx);
  };

  // For each morph, store all target vertex indices
  // List of all morphs affecting all vertices, used for constructing the morph
  // only
...
//-----------------------------------------------------------------------------
bool CVertexMorphDict::VertexMorphDictLess::Less(
    const CVertexMorphDict::MorphVertexList_t &src1,
    const CVertexMorphDict::MorphVertexList_t &src2, void *pCtx) {
...
// Sets up, cleans up the morph information
//-----------------------------------------------------------------------------
void CVertexMorphDict::Setup() { m_MorphLists.Purge(); }
...
//-----------------------------------------------------------------------------
int CVertexMorphDict::MorphCount() const { return m_MorphLists.Count(); }

int CVertexMorphDict::GetMorphTargetId(int i) const {
  if (i >= m_MorphLists.Count()) return -1;
...
const MorphVertexInfo_t &CVertexMorphDict::GetMorphVertexInfo(int nMorphTarget,
                                                              int j) const {
...
class CMorph : public IMorphInternal, public ITextureRegenerator {
 public:
  ...
  bool RenderMorphWeights(IMatRenderContext *pRenderContext, int nRenderId,
                          int nWeightCount, const MorphWeight_t *pWeights);
 ...
 private:
  ...
  // both in the src texture and destination texture (which is the morph
  ...
  typedef void (CMorph::*MorphPixelWriter_t)(CPixelWriter &pixelWriter, int x,
                                             int y,
 ...
 private:
  ...
  // Builds the list of segments to render, returns total # of src texels to
  // read from
  ...
  void ComputeTextureDimensions(
      const CUtlVector<MorphSegmentList_t> &morphSegments);

  // Writes a morph delta into the texture
  void WriteDeltaPositionNormalToTexture(CPixelWriter &pixelWriter, int x,
                                         int y, const MorphVertexInfo_t &info);
  void WriteSideSpeedToTexture(CPixelWriter &pixelWriter, int x, int y,
                               const MorphVertexInfo_t &info);
  ...
  void RenderMorphQuads(IMatRenderContext *pRenderContext, int nRenderId,
                        int nTotalQuadCount, int nWeightCount,
                        int *pWeightLookup, const MorphWeight_t *pWeights);
  ...
  void AccumulateMorphStats(int nActiveMorphCount, int nQuadsRendered,
                            int nTexelsRendered);
  void ReportMorphStats();
  void HandleMorphStats(int nActiveMorphCount, int nQuadsRendered,
                        int nTexelsRendered);
  ...
  int BuildNonZeroMorphList(int *pWeightIndices, int nWeightCount,
                            const MorphWeight_t *pWeights);

  // Determines the total number of deltas
  int DetermineTotalDeltaCount(
      const CUtlVector<MorphSegmentList_t> &morphSegments) const;
 ...
 private:
  ...
  // Caches off the morph weights when in the middle of performing morph
  // accumulation
...
class CMorphMgr : public IMorphMgr {
 public:
  ...
  virtual void AccumulateMorph(IMorphMgrRenderContext *pIRenderContext,
                               IMorph *pMorph, int nMorphCount,
                               const MorphWeight_t *pWeights);
  virtual void AdvanceFrame();
  virtual bool GetMorphAccumulatorTexCoord(
      IMorphMgrRenderContext *pRenderContext, Vector2D *pTexCoord,
      IMorph *pMorph, int nVertex);
  virtual IMorphMgrRenderContext *AllocateRenderContext();
  virtual void FreeRenderContext(IMorphMgrRenderContext *pRenderContext);

  // Other public methods
 public:
  // Computes texel offsets for the upper corner of the morph accumulator for a
  // particular block
  void ComputeAccumulatorSubrect(int *pXOffset, int *pYOffset, int *pWidth,
                                 int *pHeight, int nMorphAccumBlockId);
  void GetAccumulatorSubrectDimensions(int *pWidth, int *pHeight);
  int GetAccumulator4TupleCount() const;

  // Computes texel offsets for the upper corner of the morph weight texture for
  // a particular block
  void ComputeWeightSubrect(int *pXOffset, int *pYOffset, int *pWidth,
                            int *pHeight, int nMorphAccumBlockId);
 ...
 private:
  ...
  void Display32FTextureData(float *pBuf, int nTexelID, int *pSubRect,
                             ITexture *pTexture, int n4TupleCount);
  ...
  void DrawMorphTempTexture(IMatRenderContext *pRenderContext,
                            IMaterial *pMaterial, ITexture *pTexture);
...
//-----------------------------------------------------------------------------
MorphFormat_t CMorph::GetMorphFormat() const { return m_Format; }

//-----------------------------------------------------------------------------
// Binds morph accumulator, morph weights
//-----------------------------------------------------------------------------
void CMorph::Bind(IMorphMgrRenderContext *pIRenderContext) {
  CMorphMgrRenderContext *pMorphRenderContext =
      static_cast<CMorphMgrRenderContext *>(pIRenderContext);
  int nRenderId = pMorphRenderContext->GetRenderId(this);
  if (nRenderId < 0) return;

  int nXOffset, nYOffset, nWidth, nHeight;
  s_MorphMgr.ComputeAccumulatorSubrect(&nXOffset, &nYOffset, &nWidth, &nHeight,
                                       nRenderId);

  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_4TUPLE_COUNT,
      s_MorphMgr.GetAccumulator4TupleCount());
  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_X_OFFSET, nXOffset);
  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_Y_OFFSET, nYOffset);
  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_SUBRECT_WIDTH, nWidth);
  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_ACCUMULATOR_SUBRECT_HEIGHT, nHeight);
}

void CMorph::BindMorphWeight(int nRenderId) {
  int nXOffset, nYOffset, nWidth, nHeight;
  s_MorphMgr.ComputeWeightSubrect(&nXOffset, &nYOffset, &nWidth, &nHeight,
                                  nRenderId);

  g_pShaderAPI->SetIntRenderingParameter(INT_RENDERPARM_MORPH_WEIGHT_X_OFFSET,
                                         nXOffset);
  g_pShaderAPI->SetIntRenderingParameter(INT_RENDERPARM_MORPH_WEIGHT_Y_OFFSET,
                                         nYOffset);
  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_WEIGHT_SUBRECT_WIDTH, nWidth);
  g_pShaderAPI->SetIntRenderingParameter(
      INT_RENDERPARM_MORPH_WEIGHT_SUBRECT_HEIGHT, nHeight);
}
...
int CMorph::ComputeMorphTextureSizeInBytes() const {
  ...
  if (m_pMorphTexture[MORPH_TEXTURE_POS_NORMAL_DELTA]) {
    ...
    nSize += m_nTextureWidth * m_nTextureHeight * nTotal4Tuples *
             ImageLoader::SizeInBytes(IMAGE_FORMAT_RGBA16161616);
  }

  if (m_pMorphTexture[MORPH_TEXTURE_SPEED_SIDE_MAP]) {
    nSize += m_nTextureWidth * m_nTextureHeight *
...
// Is the morph locked?
//-----------------------------------------------------------------------------
bool CMorph::IsLocked() const { return m_bLocked; }
...
void CMorph::CreateAccumulatorMaterial(int nMaterialIndex) {
  // NOTE: Delta scale is a little tricky. The numbers are store in fixed-point
  // 16 bit. The pixel shader will interpret 65536 as 1.0, and 0 as 0.0. In the
  // pixel shader, we will read the delta, multiply it by 2, and subtract 1 to
  // get a -1 to 1 range. The float to fixed scale is applied prior to writing
  // it in (delta * scale + 32768). Therefore the max representable positive
  // value =
  ...
  pVMTKeyValues->SetFloat("$deltascale", (m_flFloatToFixedScale != 0.0f)
                                             ? 32768.0f / m_flFloatToFixedScale
                                             : 1.0f);
  if (m_pMorphTexture[MORPH_TEXTURE_POS_NORMAL_DELTA]) {
    pVMTKeyValues->SetString(
        "$delta", m_pMorphTexture[MORPH_TEXTURE_POS_NORMAL_DELTA]->GetName());
  }
  if (m_pMorphTexture[MORPH_TEXTURE_SPEED_SIDE_MAP]) {
    pVMTKeyValues->SetString(
        "$sidespeed", m_pMorphTexture[MORPH_TEXTURE_SPEED_SIDE_MAP]->GetName());
  }
  Q_snprintf(pTemp, sizeof(pTemp), "[%d %d %d]", m_nTextureWidth,
             m_nTextureHeight, Get4TupleCount(m_Format));
...
// Determines the total number of deltas
//-----------------------------------------------------------------------------
int CMorph::DetermineTotalDeltaCount(
    const CUtlVector<MorphSegmentList_t> &morphSegments) const {
  int nDeltaCount = 0;
...
//-----------------------------------------------------------------------------
void CMorph::ComputeTextureDimensions(
    const CUtlVector<MorphSegmentList_t> &morphSegments) {
  ...
  m_nTextureWidth =
      (m_nTextureHeight != 0)
          ? (nTotalDeltas + (m_nTextureHeight - 1)) / m_nTextureHeight
          : 0;
  ...
  while (m_nTextureWidth * nTotal4Tuples > nMaxTextureWidth) {
    ...
    if (m_nTextureHeight > nMaxTextureHeight) {
      Warning(
          "Morph texture is too big!!! Make brian add support for morphs "
          "having multiple textures.\n");
      Assert(0);
  ...
  for (int i = 0; i < nMorphCount; ++i) {
    ...
    for (int j = 0; j < nQuadCount; ++j) {
      ...
      if (j != 0) {
        ...
        if ((list[j].m_nFirstDest / nDestTextureHeight ==
             list[j - 1].m_nFirstDest / nDestTextureHeight) &&
            (list[j].m_nFirstSrc / m_nTextureHeight ==
             list[j - 1].m_nFirstSrc / m_nTextureHeight)) {
          int nGapSize = list[j].m_nFirstDest -
                         (list[j - 1].m_nFirstDest + list[j - 1].m_nCount);
...
void CMorph::PackMorphData() {
  ...
  if (m_nTextureWidth == 0 || m_nTextureHeight == 0) return;

  char pTemp[512];
  if (m_Format & (MORPH_POSITION | MORPH_WRINKLE | MORPH_NORMAL)) {
    Q_snprintf(pTemp, sizeof(pTemp), "__morphtarget[%d]: pos/norm",
               s_nUniqueId);

    int nTotal4Tuples = Get4TupleCount(m_Format);
    ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
        pTemp, TEXTURE_GROUP_MORPH_TARGETS, m_nTextureWidth * nTotal4Tuples,
        m_nTextureHeight, IMAGE_FORMAT_RGBA16161616,
        TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE |
            TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_CLAMPS |
            TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE);
    m_pMorphTexture[MORPH_TEXTURE_POS_NORMAL_DELTA] =
        static_cast<ITextureInternal *>(pTexture);
  }

  if (m_Format & (MORPH_SIDE | MORPH_SPEED)) {
    Q_snprintf(pTemp, sizeof(pTemp), "__morphtarget[%d]: side/speed",
               s_nUniqueId);

    ITexture *pTexture = g_pMaterialSystem->CreateProceduralTexture(
        pTemp, TEXTURE_GROUP_MORPH_TARGETS, m_nTextureWidth, m_nTextureHeight,
        IMAGE_FORMAT_RGBA8888,
        TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE |
            TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_CLAMPS |
            TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_POINTSAMPLE);
    m_pMorphTexture[MORPH_TEXTURE_SPEED_SIDE_MAP] =
        static_cast<ITextureInternal *>(pTexture);
  }
...
// Writes a morph delta into the texture
//-----------------------------------------------------------------------------
void CMorph::WriteDeltaPositionNormalToTexture(CPixelWriter &pixelWriter, int x,
                                               int y,
                                               const MorphVertexInfo_t &info) {
...
void CMorph::WriteSideSpeedToTexture(CPixelWriter &pixelWriter, int x, int y,
                                     const MorphVertexInfo_t &info) {
...
void CMorph::BuildSegmentList(CUtlVector<MorphSegmentList_t> &morphSegments) {
  ...
  s_MorphMgr.GetAccumulatorSubrectDimensions(&nDestTextureWidth,
                                             &nDestTextureHeight);
  ...
  // Allocate space to cache off the morph weights when in the middle of
  // performing morph accumulation
  Assert(!m_pRenderMorphWeight);
  m_pRenderMorphWeight = new MorphWeight_t[m_nMaxMorphTargetCount];

  Assert(m_nMaxMorphTargetCount < 1024);  // This algorithm of storing a full
                                          // array is bogus if this isn't true
  m_MorphTargetIdToQuadIndex.SetCount(m_nMaxMorphTargetCount);
  memset(m_MorphTargetIdToQuadIndex.Base(), 0xFF,
         m_nMaxMorphTargetCount * sizeof(int));
  ...
  for (int i = 0; i < nMorphTargetCount; ++i) {
    ...
    for (int j = 0; j < nVertexCount; ++j) {
      ...
      if (segment.m_nCount) {
        // Vertical overflow, non-contiguous destination verts, or contiguous
        // dest verts which happen to lie in different columns are the break
        // conditions
        if ((nLastDestIndex < 0) ||
            (info.m_nVertexId > nLastDestIndex + MIN_SEGMENT_GAP_SIZE) ||
            (info.m_nVertexId / nDestTextureHeight !=
             nLastDestIndex / nDestTextureHeight)) {
...
VertexFormat_t CMorph::ComputeVertexFormat(IMaterial *pMaterial) const {
  // We believe this material's vertex format is reliable (unlike many others as
  // of June 07)
  VertexFormat_t vertexFormat = pMaterial->GetVertexFormat();

  // UNDONE: optimize the vertex format to compress or remove elements where
  // possible
...
void CMorph::CreateStaticMesh() {
  ...
  VertexFormat_t vertexFormat =
      ComputeVertexFormat(m_MorphAccumulationMaterial);
  m_pMorphBuffer = pRenderContext->CreateStaticMesh(
      vertexFormat, TEXTURE_GROUP_MORPH_TARGETS, m_MorphAccumulationMaterial);
  ...
  for (int i = 0; i < nMorphCount; ++i) {
    ...
    for (int j = 0; j < nQuadCount; ++j) {
      ...
      meshBuilder.TexCoord4f(
          0, sx * flOOTexWidth, sy * flOOTexHeight, (dx - 0.5f) * flOODestWidth,
          (dy - 0.5f) * flOODestHeight);  // Stores the source to read from
      meshBuilder.TexCoord1f(1, i);
      meshBuilder.AdvanceVertex();

      meshBuilder.TexCoord4f(
          0, sx * flOOTexWidth, (sy + quad.m_nCount) * flOOTexHeight,
          (dx - 0.5f) * flOODestWidth,
          (dy + quad.m_nCount - 0.5f) *
              flOODestHeight);  // Stores the source to read from
      meshBuilder.TexCoord1f(1, i);
      meshBuilder.AdvanceVertex();

      meshBuilder.TexCoord4f(
          0, (sx + n4TupleCount) * flOOTexWidth,
          (sy + quad.m_nCount) * flOOTexHeight,
          (dx + n4TupleCount - 0.5f) * flOODestWidth,
          (dy + quad.m_nCount - 0.5f) *
              flOODestHeight);  // Stores the source to read from
      meshBuilder.TexCoord1f(1, i);
      meshBuilder.AdvanceVertex();
      ...
      meshBuilder.TexCoord4f(
          0, (sx + n4TupleCount) * flOOTexWidth, sy * flOOTexHeight,
          (dx + n4TupleCount - 0.5f) * flOODestWidth,
          (dy - 0.5f) * flOODestHeight);  // Stores the source to read from
      meshBuilder.TexCoord1f(1, i);
      meshBuilder.AdvanceVertex();
...
// Inherited from ITextureRegenerator
//-----------------------------------------------------------------------------
void CMorph::RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture,
                                   Rect_t *pRect) {
  ...
  for (nTextureType = 0; nTextureType < MORPH_TEXTURE_COUNT; ++nTextureType) {
    if (pTexture == m_pMorphTexture[nTextureType]) break;
  }
  ...
  for (int i = 0; i < nQuadListCount; ++i) {
    ...
    for (int j = 0; j < nQuadCount; ++j) {
      ...
      for (int k = 0; k < quad.m_nCount; ++k) {
        const MorphVertexInfo_t &info =
            m_MorphDict.GetMorphVertexInfo(i, nVertIndex);
...
void CMorph::AccumulateMorphStats(int nActiveMorphCount, int nQuadsRendered,
                                  int nTexelsRendered) {
...
// Renders to the morph accumulator texture
//-----------------------------------------------------------------------------
void CMorph::RenderMorphQuads(IMatRenderContext *pRenderContext, int nRenderId,
                              int nTotalQuadCount, int nWeightCount,
                              int *pWeightLookup,
                              const MorphWeight_t *pWeights) {
  if (s_MorphMgr.IsUsingConstantRegisters()) {
    pRenderContext->SetFlexWeights(0, m_nMaxMorphTargetCount,
                                   m_pRenderMorphWeight);
  } else {
  ...
  for (int i = 0; i < nWeightCount; ++i) {
    ...
    for (int j = 0; j < nQuadCount; ++j) {
      ...
      if (s_nMinDest >= 0 && quad.m_nFirstDest + quad.m_nCount <= s_nMinDest)
        continue;
      if (s_nMaxDest >= 0 && quad.m_nFirstDest > s_nMaxDest) continue;
#endif
...
static inline bool IsMorphWeightZero(const MorphWeight_t &weight) {
  return (FloatMakePositive(weight.m_pWeight[MORPH_WEIGHT]) < 0.001 &&
          ...
          FloatMakePositive(weight.m_pWeight[MORPH_WEIGHT_STEREO_LAGGED]) <
              0.001);
}
...
// Builds a list of non-zero morph targets
//-----------------------------------------------------------------------------
int CMorph::BuildNonZeroMorphList(int *pWeightIndices, int nWeightCount,
                                  const MorphWeight_t *pWeights) {
  ...
  for (int i = 0; i < m_nMaxMorphTargetCount; ++i) {
    ...
    if (m_MorphTargetIdToQuadIndex[i] < 0) continue;

    // Don't bother with small weights
    if (IsMorphWeightZero(weight)) continue;
...
//-----------------------------------------------------------------------------
bool CMorph::RenderMorphWeights(IMatRenderContext *pRenderContext,
                                int nRenderId, int nWeightCount,
                                const MorphWeight_t *pWeights) {
  VPROF_BUDGET("CMorph::RenderMorphWeights", "HW Morphing");
  ...
  if (nCountToClear > 0) {
    memset(&m_pRenderMorphWeight[nCountToCopy], 0,
           nCountToClear * sizeof(MorphWeight_t));
  }

  int *pWeightIndices = (int *)_alloca(nCountToCopy * sizeof(int));
  int nIndexCount =
      BuildNonZeroMorphList(pWeightIndices, nCountToCopy, pWeights);
  if (nIndexCount == 0) return false;

  if (s_MorphMgr.IsUsingConstantRegisters()) return true;
  ...
  for (int i = 0; i < nIndexCount; ++i) {
    ...
    meshBuilder.TexCoord2f(0, (nLocalX + x) * flOOWidth,
                           (nLocalY + y) * flOOHeight);
...
void CMorph::AccumulateMorph(int nRenderId) {
  VPROF_BUDGET("CMorph::AccumulateMorph", "HW Morphing");

  // Build a non-zero weight list and a total quad count
  int *pTargets = (int *)_alloca(m_nMaxMorphTargetCount * sizeof(int));
  int nTargetCount = BuildNonZeroMorphList(pTargets, m_nMaxMorphTargetCount,
                                           m_pRenderMorphWeight);
  ...
  RenderMorphQuads(pRenderContext, nRenderId, nTotalQuadCount, nTargetCount,
                   pTargets, m_pRenderMorphWeight);
}
...
void CMorphMgr::AllocateScratchTextures() {
  ...
  // because this will rarely change. Just hard code it to something that will
  // fit it.
  m_nAccumulatorWidth = 256;
  m_nAccumulatorHeight = 256;
  Assert(m_nAccumulatorWidth * m_nAccumulatorHeight ==
         MAX_MORPH_ACCUMULATOR_VERTICES * MORPH_ACCUMULATOR_4TUPLES);
  ...
  m_pMorphAccumTexture = TextureManager()->CreateRenderTargetTexture(
      "_rt_MorphAccumulator", m_nAccumulatorWidth * nMultFactor,
      m_nAccumulatorHeight * nMultFactor, RT_SIZE_OFFSCREEN, fmt,
      RENDER_TARGET_NO_DEPTH,
      TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE |
  ...
  if (!m_bUsingConstantRegisters) {
    m_pMorphWeightTexture = TextureManager()->CreateRenderTargetTexture(
        "_rt_MorphWeight", m_nWeightWidth * nMultFactor,
        m_nWeightHeight * nMultFactor, RT_SIZE_OFFSCREEN, fmt,
        RENDER_TARGET_NO_DEPTH,
        TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NODEBUGOVERRIDE |
...
void CMorphMgr::AllocateMaterials() {
  ...
  m_pVisualizeMorphAccum = g_pMaterialSystem->CreateMaterial(
  ...
  if (!m_bUsingConstantRegisters) {
    ...
    pVMTKeyValues->SetString("$ignorez", "1");
    m_pRenderMorphWeight =
        g_pMaterialSystem->CreateMaterial("___morphweight.vmt", pVMTKeyValues);
    ...
    pVMTKeyValues->SetString("$nocull", "1");
    pVMTKeyValues->SetString("$ignorez", "1");
    m_pVisualizeMorphWeight = g_pMaterialSystem->CreateMaterial(
...
//-----------------------------------------------------------------------------
ITextureInternal *CMorphMgr::MorphAccumulator() { return m_pMorphAccumTexture; }

ITextureInternal *CMorphMgr::MorphWeights() { return m_pMorphWeightTexture; }

//-----------------------------------------------------------------------------
// Class factory
//-----------------------------------------------------------------------------
IMorphInternal *CMorphMgr::CreateMorph() { return new CMorph; }
...
//-----------------------------------------------------------------------------
void CMorphMgr::AdvanceFrame() { ++m_nFrameCount; }

//-----------------------------------------------------------------------------
// Computes texel offsets for the upper corner of the morph weight texture for a
// particular block
//-----------------------------------------------------------------------------
void CMorphMgr::ComputeWeightSubrect(int *pXOffset, int *pYOffset, int *pWidth,
                                     int *pHeight, int nMorphAccumBlockId) {
...
// particular block
//-----------------------------------------------------------------------------
void CMorphMgr::ComputeAccumulatorSubrect(int *pXOffset, int *pYOffset,
                                          int *pWidth, int *pHeight,
                                          int nMorphAccumBlockId) {
  *pXOffset = nMorphAccumBlockId / m_nSubrectVerticalCount;
  *pYOffset = nMorphAccumBlockId - m_nSubrectVerticalCount * (*pXOffset);
...
int CMorphMgr::GetTotalMemoryUsage() const {
  int nSize = 0;
  if (m_pMorphAccumTexture) {
    nSize += m_pMorphAccumTexture->GetActualWidth() *
             m_pMorphAccumTexture->GetActualHeight() *
             ImageLoader::SizeInBytes(m_pMorphAccumTexture->GetImageFormat());
  }
  if (m_pMorphWeightTexture) {
    nSize += m_pMorphWeightTexture->GetActualWidth() *
             m_pMorphWeightTexture->GetActualHeight() *
...
// Displays 32bit float texture data
//-----------------------------------------------------------------------------
void CMorphMgr::Display32FTextureData(float *pBuf, int nTexelID, int *pSubRect,
                                      ITexture *pTexture, int n4TupleCount) {
...
void CMorphMgr::DebugMorphAccumulator(IMatRenderContext *pRenderContext) {
  ...
  ITexture *pDest = g_pMorphMgr->MorphAccumulator();
  if (pDest->GetImageFormat() != IMAGE_FORMAT_RGBA32323232F) return;
  ...
  pRenderContext->ReadPixels(0, 0, nDestWidth, nDestHeight,
                             (unsigned char *)pBuf, IMAGE_FORMAT_RGBA32323232F);
  ...
  int pSubRect[4];
  ComputeAccumulatorSubrect(&pSubRect[0], &pSubRect[1], &pSubRect[2],
                            &pSubRect[3], s_nMorphIndex);

  if (s_nMaxDisplay < 0) {
    Display32FTextureData(pBuf, s_nMinDisplay, pSubRect, pDest,
                          MORPH_ACCUMULATOR_4TUPLES);
  } else {
    for (int i = s_nMinDisplay; i <= s_nMaxDisplay; ++i) {
      Display32FTextureData(pBuf, i, pSubRect, pDest,
                            MORPH_ACCUMULATOR_4TUPLES);
    }
  }
  free(pBuf);
...
void CMorphMgr::DebugMorphWeights(IMatRenderContext *pRenderContext) {
  ...
  if (pTexture->GetImageFormat() != IMAGE_FORMAT_RGBA32323232F) return;

  pRenderContext->Flush();
  float *pBuf = (float *)malloc(nWidth * nHeight * 4 * sizeof(float));
  pRenderContext->ReadPixels(0, 0, nWidth, nHeight, (unsigned char *)pBuf,
  ...
  int pSubRect[4];
  ComputeWeightSubrect(&pSubRect[0], &pSubRect[1], &pSubRect[2], &pSubRect[3],
                       s_nMorphIndex);
...
void CMorphMgr::DrawMorphTempTexture(IMatRenderContext *pRenderContext,
                                     IMaterial *pMaterial, ITexture *pTexture) {
  pMaterial = ((IMaterialInternal *)pMaterial)
                  ->GetRealTimeVersion();  // always work
                                           // with the real
                                           // time version of
                                           // materials
                                           // internally.
  ...
  int nWidth = pTexture->GetActualWidth();
  int nHeight = pTexture->GetActualHeight();
  ::DrawScreenSpaceRectangle(pMaterial, s_nX, s_nY, nWidth, nHeight, 0, 0,
                             nWidth - 1, nHeight - 1, nWidth, nHeight);
...
//-----------------------------------------------------------------------------
bool CMorphMgr::GetMorphAccumulatorTexCoord(
    IMorphMgrRenderContext *pRenderContext, Vector2D *pTexCoord, IMorph *pMorph,
    int nVertex) {
  CMorphMgrRenderContext *pMorphRenderContext =
      static_cast<CMorphMgrRenderContext *>(pRenderContext);
  int nRenderId =
      pMorphRenderContext->GetRenderId(static_cast<CMorph *>(pMorph));
  if (nRenderId < 0) {
    pTexCoord->Init();
    return false;
  }
...
// Starts, ends morph accumulation.
//-----------------------------------------------------------------------------
void CMorphMgr::BeginMorphAccumulation(
    IMorphMgrRenderContext *pIRenderContext) {
  VPROF_BUDGET("CMorph::BeginMorphAccumulation", "HW Morphing");
  ...
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  IMatRenderContextInternal *pRenderContextInternal =
      static_cast<IMatRenderContextInternal *>(
  ...
  // may cause it to be unbound; therefore we must force a rebind of the
  // material.
...
void CMorphMgr::EndMorphAccumulation(IMorphMgrRenderContext *pIRenderContext) {
  VPROF_BUDGET("CMorph::EndMorphAccumulation", "HW Morphing");
  ...
  if (mat_drawmorphweights.GetInt()) {
    if (!m_bUsingConstantRegisters) {
      DrawMorphTempTexture(pRenderContext, m_pVisualizeMorphWeight,
                           MorphWeights());
    }
  }
  if (mat_drawmorphaccumulator.GetInt()) {
    DrawMorphTempTexture(pRenderContext, m_pVisualizeMorphAccum,
                         MorphAccumulator());
  }
#endif
...
//-----------------------------------------------------------------------------
void CMorphMgr::AccumulateMorph(IMorphMgrRenderContext *pIRenderContext,
                                IMorph *pMorph, int nMorphCount,
                                const MorphWeight_t *pWeights) {
  CMorphMgrRenderContext *pMorphRenderContext =
      static_cast<CMorphMgrRenderContext *>(pIRenderContext);
  ...
  Assert(pMorphRenderContext->m_nMorphCount <
         CMorphMgrRenderContext::MAX_MODEL_MORPHS);
  if (pMorphRenderContext->m_nMorphCount >=
      CMorphMgrRenderContext::MAX_MODEL_MORPHS) {
  ...
  if (pMorphInternal->RenderMorphWeights(pRenderContext,
                                         pMorphRenderContext->m_nMorphCount,
                                         nMorphCount, pWeights)) {
    pMorphRenderContext
        ->m_pMorphsToAccumulate[pMorphRenderContext->m_nMorphCount] =
        pMorphInternal;
    ++pMorphRenderContext->m_nMorphCount;
  }
}
