...
enum InternalTextureFlags {
  ...
  TEXTUREFLAGSINTERNAL_PRELOADED =
      0x00000004,  // 360: textures that went through the preload process
  TEXTUREFLAGSINTERNAL_QUEUEDLOAD =
      0x00000008,  // 360: load using the queued loader
  ...
  TEXTUREFLAGSINTERNAL_TEMPRENDERTARGET =
      0x00000080,  // 360: should only allocate texture bits upon first resolve,
                   // destroy at level end
};

namespace TextureLodOverride {
struct OverrideInfo {
  OverrideInfo() : x(0), y(0) {}
  OverrideInfo(int8_t x_, int8_t y_) : x(x_), y(y_) {}
  int8_t x, y;
};
...
class CTexture : public ITextureInternal {
 public:
  ...
  void InitProceduralTexture(const char *pTextureName,
                             const char *pTextureGroupName, int w, int h, int d,
  ...
  // Returns:
  //		pointer to the resource data, or NULL. Note that the data from
  // this  pointer can disappear when
  ...
  // Set this texture as a render target (optionally set depth texture as depth
  // buffer as well)
 ...
 protected:
  ...
  void WriteDataToShaderAPITexture(int nFrameCount, int nFaceCount,
                                   int nFirstFace, int nMipCount,
                                   IVTFTexture *pVTFTexture, ImageFormat fmt);
  ...
  // Calculates info about whether we can make the texture smaller and by how
  // much Returns the number of skipped mip levels
  ...
  IVTFTexture *LoadTextureBitsFromFile(char *pCacheFileName,
                                       char **pResolvedFilename, bool bPaired);
  ...
  IVTFTexture *ReconstructPartialProceduralBits(const Rect_t *pRect,
                                                Rect_t *pActualRect);
  ...
  void ComputeMipLevelSubRect(const Rect_t *pSrcRect, int nMipLevel,
                              Rect_t *pSubRect);

  IVTFTexture *GetScratchVTFTexture(bool bPaired = false);

  int GetOptimalReadBuffer(FileHandle_t hFile, int nFileSize,
                           CUtlBuffer &optimalBuffer);
 ...
 protected:
  ...
  // What texture group this texture is in (winds up setting counters based on
  // the group name, then the budget panel views the counters).
  ...
  // Used to help decide whether or not to recreate the render target if AA
  // changes.
  ...
  // Indicates that this CTexture contains a pair of CTexture data (two surface
  // format types...useful for normal compression with alpha)
  bool m_bPaired;
 ...
 public:
  void InitRenderTarget(const char *pRTName, int w, int h,
                        RenderTargetSizeMode_t sizeMode, ImageFormat fmt,
                        RenderTargetType_t type, unsigned int textureFlags,
...
class CReferenceToHandleTexture : public ITextureInternal {
 public:
  ...
  virtual NormalDecodeMode_t GetNormalDecodeMode() const {
 ...
 public:
  ...
  void InitFromHandle(const char *pTextureName, const char *pTextureGroupName,
...
CReferenceToHandleTexture::~CReferenceToHandleTexture() {
  ...
  if (m_nRefCount != 0) {
    Warning(
        "Reference Count(%d) != 0 in ~CReferenceToHandleTexture for texture "
        "\"%s\"\n",
...
void CReferenceToHandleTexture::InitFromHandle(
    const char *pTextureName, const char *pTextureGroupName,
    ShaderAPITextureHandle_t hTexture) {
...
void CReferenceToHandleTexture::Bind(Sampler_t sampler1, int nFrame,
...
void CReferenceToHandleTexture::BindVertexTexture(
    VertexTextureSampler_t sampler, int nFrame) {
  if (g_pShaderDevice->IsUsingGraphics()) {
...
//-----------------------------------------------------------------------------
ITextureInternal *ITextureInternal::CreateFileTexture(
...
ITextureInternal *ITextureInternal::CreateProceduralTexture(
    ...
    int d, ImageFormat fmt, int nFlags) {
  CTexture *pTex = new CTexture;
  pTex->InitProceduralTexture(pTextureName, pTextureGroupName, w, h, d, fmt,
                              nFlags);
...
ITextureInternal *ITextureInternal::CreateRenderTarget(
    const char *pRTName, int w, int h, RenderTargetSizeMode_t sizeMode,
    ImageFormat fmt, RenderTargetType_t type, unsigned int textureFlags,
    unsigned int renderTargetFlags) {
  CTexture *pTex = new CTexture;
  pTex->InitRenderTarget(pRTName, w, h, sizeMode, fmt, type, textureFlags,
                         renderTargetFlags);
...
//-----------------------------------------------------------------------------
void ITextureInternal::ChangeRenderTarget(ITextureInternal *pTex, int w, int h,
                                          ...
                                          unsigned int renderTargetFlags) {
  pTex->Release();
  dynamic_cast<CTexture *>(pTex)->InitRenderTarget(
      pTex->GetName(), w, h, sizeMode, fmt, type, textureFlags,
      renderTargetFlags);
}
...
//-----------------------------------------------------------------------------
void CTexture::Init(int w, int h, int d, ImageFormat fmt, int iFlags,
                    int iFrameCount, bool bPaired) {
  Assert(iFrameCount > 0);

  // Assume unpaired was initialized first, as we piggyback off of the shared
  // values such as height, width etc
#if defined(COMPRESSED_NORMAL_FORMATS)
  if (bPaired) {
...
//-----------------------------------------------------------------------------
int CTexture::GetOptimalReadBuffer(FileHandle_t hFile, int nSize,
                                   CUtlBuffer &optimalBuffer) {
  ...
  if (nBytesOptimalRead > s_nOptimalReadBufferSize) {
    ...
    s_pOptimalReadBuffer =
        g_pFullFileSystem->AllocOptimalReadBuffer(hFile, nSize);
  }

  // set external buffer and reset to empty
  optimalBuffer.SetExternalBuffer(s_pOptimalReadBuffer,
...
void CTexture::ApplyRenderTargetSizeMode(int &width, int &height,
                                         ImageFormat fmt) {
  ...
  switch (m_RenderTargetSizeMode) {
    ...
    case RT_SIZE_OFFSCREEN: {
      ...
      // On 360, don't do this resizing for formats related to the shadow depth
      // texture
#if defined(_X360)
      if (!((fmt == IMAGE_FORMAT_X360_DST16) ||
            (fmt == IMAGE_FORMAT_X360_DST24) ||
            (fmt == IMAGE_FORMAT_X360_DST24F) || (fmt == IMAGE_FORMAT_BGR565)))
#endif
      {
        // Shrink the buffer if it's bigger than back buffer.  Otherwise, don't
        // mess with it.
    ...
    default: {
      ...
      Assert(m_nOriginalRenderTargetType ==
             RENDER_TARGET_NO_DEPTH);  // Only can use NO_CHANGE if we don't
                                       // have a depth buffer.
    } break;
  }
}
...
//-----------------------------------------------------------------------------
void CTexture::InitRenderTarget(const char *pRTName, int w, int h,
                                RenderTargetSizeMode_t sizeMode,
                                ImageFormat fmt, RenderTargetType_t type,
                                unsigned int textureFlags,
                                unsigned int renderTargetFlags) {
  if (pRTName) {
    SetName(pRTName);
  ...
  if (type == RENDER_TARGET_NO_DEPTH) {
    nFlags |= TEXTUREFLAGS_NODEPTHBUFFER;
             ...
             g_pShaderAPI->DoRenderTargetsNeedSeparateDepthBuffer()) {
...
void CTexture::OnRestore() {
  ...
  if ((m_nFlags & TEXTUREFLAGS_RENDERTARGET)) {
    ...
    // If we didn't recreate it up above, then we may need to resize it anyway
    // if the framebuffer got smaller than we are.
    int newWidth, newHeight;
    ApplyRenderTargetSizeMode(newWidth, newHeight, m_ImageFormat);
...
//-----------------------------------------------------------------------------
void CTexture::InitProceduralTexture(const char *pTextureName,
                                     const char *pTextureGroupName, int w,
                                     int h, int d, ImageFormat fmt,
                                     int nFlags) {
  ...
  Assert((nFlags &
          (TEXTUREFLAGS_RENDERTARGET | TEXTUREFLAGS_DEPTHRENDERTARGET)) == 0);

  SetName(pTextureName);

  // Eliminate flags that are inappropriate...
  nFlags &= ~TEXTUREFLAGS_HINT_DXT5 | TEXTUREFLAGS_ONEBITALPHA |
            TEXTUREFLAGS_EIGHTBITALPHA | TEXTUREFLAGS_RENDERTARGET |
...
void CTexture::AllocateShaderAPITextures() {
  ...
  if ((m_nFlags & TEXTUREFLAGS_ENVMAP) &&
      HardwareConfig()->SupportsCubeMaps()) {
  ...
  int nShaderApiCreateTextureDepth =
  ...
  g_pShaderAPI->CreateTextures(m_pTextureHandles, nCount, m_nActualWidth,
                               m_nActualHeight, nShaderApiCreateTextureDepth,
                               m_ImageFormat, m_nActualMipCount, nCopies,
                               nCreateFlags, GetName(), GetTextureGroupName());

#if defined(COMPRESSED_NORMAL_FORMATS)
  if (m_bPaired) {
    g_pShaderAPI->CreateTextures(
        m_pTextureHandlesPaired, nCount, m_nActualWidth, m_nActualHeight,
        nShaderApiCreateTextureDepth, m_ImageFormatPaired, m_nActualMipCount,
...
ImageFormat CTexture::ComputeActualFormat(ImageFormat srcFormat) {
  ...
  if (g_config.bCompressedTextures &&
      HardwareConfig()->SupportsCompressedTextures() && bIsCompressed) {
...
int CTexture::ComputeActualSize(bool bIgnorePicmip, IVTFTexture *pVTFTexture) {
  ...
  if (IsPC()) {
    ...
    // Check for LOD control override
    {
      ...
      if (oi.x && oi.y && !pLODInfo)  // If overriding texture that doesn't have
                                      // lod info yet, then use default
        lcs = default_lod_settings;

      lcs.m_ResolutionClampX += oi.x;
      lcs.m_ResolutionClampY += oi.y;
      if (int8_t(lcs.m_ResolutionClampX) < 0) lcs.m_ResolutionClampX = 0;
      if (int8_t(lcs.m_ResolutionClampY) < 0) lcs.m_ResolutionClampY = 0;
    }
  ...
  if (!bIgnorePicmip) {
    ...
    if (g_config.skipMipLevels < 0) {
      for (int iUpgrade = 0; iUpgrade < -g_config.skipMipLevels; ++iUpgrade) {
        if (nClampX < m_nActualWidth && nClampY < m_nActualHeight) {
          nClampX <<= 1;
          nClampY <<= 1;
        } else
          break;
  ...
  //
  int numMipsSkipped =
      0;  // will compute now when clamping how many mips we drop
  while ((m_nActualWidth > nClampX) || (m_nActualHeight > nClampY) ||
         (m_nActualDepth > nClampZ)) {
    m_nActualWidth >>= 1;
    m_nActualHeight >>= 1;
    m_nActualDepth >>= 1;
    if (m_nActualDepth < 1) m_nActualDepth = 1;
...
#if defined(COMPRESSED_NORMAL_FORMATS)
  g_pShaderAPI->ModifyTexture(bPaired ? m_pTextureHandlesPaired[iFrame]
                                      : m_pTextureHandles[iFrame]);
#else
...
void CTexture::SetFilterState() {
  ...
  // NOTE: config.bMipMapTextures and config.bFilterTextures is handled in
  // ShaderAPIDX8
  ...
  if (HardwareConfig()->GetDXSupportLevel() >= 80 &&
      (g_config.m_nForceAnisotropicLevel > 1) &&
      (HardwareConfig()->MaximumAnisotropicLevel() > 1)) {
    bIsAnisotropic = true;
  } else if (g_config.ForceTrilinear()) {
    bIsAnisotropic = ((m_nFlags & TEXTUREFLAGS_ANISOTROPIC) != 0) &&
                     (HardwareConfig()->MaximumAnisotropicLevel() > 1);
    bIsTrilinear = true;
  } else {
    bIsAnisotropic = ((m_nFlags & TEXTUREFLAGS_ANISOTROPIC) != 0) &&
...
void CTexture::DownloadTexture(Rect_t *pRect) {
  ...
  // Iterate over all the frames and set the appropriate wrapping + filtering
  // state
...
//-----------------------------------------------------------------------------
void CTexture::Bind(Sampler_t sampler1, int nFrame,
                    Sampler_t sampler2 /* = -1 */) {
  if (g_pShaderDevice->IsUsingGraphics()) {
...
bool CTexture::SetRenderTarget(int nRenderTargetID, ITexture *pDepthTexture) {
  ...
  ShaderAPITextureHandle_t depthTextureHandle =
      (unsigned int)SHADER_RENDERTARGET_DEPTHBUFFER;
  ...
  if (pDepthTexture) {
    depthTextureHandle =
        static_cast<ITextureInternal *>(pDepthTexture)->GetTextureHandle(0);
  }

  g_pShaderAPI->SetRenderTargetEx(nRenderTargetID, textureHandle,
                                  depthTextureHandle);
  return true;
}
...
bool CTexture::IsTranslucent() const {
  return (m_nFlags & (TEXTUREFLAGS_ONEBITALPHA | TEXTUREFLAGS_EIGHTBITALPHA)) !=
         0;
}
...
void CTexture::ForceLODOverride(int iNumLodsOverrideUpOrDown) {
  TextureLodOverride::OverrideInfo oi(iNumLodsOverrideUpOrDown,
                                      iNumLodsOverrideUpOrDown);
...
void CTexture::SetFilteringAndClampingMode() {
  ...
  for (int iFrame = 0; iFrame < nCount; ++iFrame) {
    Modify(iFrame);    // Indicate we're changing state with respect to a
                       // particular frame
    SetWrapState();    // Send the appropriate wrap/clamping modes to the
                       // shaderapi.
    SetFilterState();  // Set the filtering mode for the texture after
                       // downloading it. NOTE: Apparently, the filter state
                       // cannot be set until after download
  }

#if defined(COMPRESSED_NORMAL_FORMATS)
  if (m_bPaired) {
    for (int iFrame = 0; iFrame < nCount; ++iFrame) {
      Modify(iFrame, true);  // Indicate we're changing state with respect to a
                             // particular frame
      SetWrapState();        // Send the appropriate wrap/clamping modes to the
                             // shaderapi.
      SetFilterState();      // Set the filtering mode for the texture after
...
void CTexture::Precache() {
  ...
  if (!Q_strnicmp(m_Name.String(), "env_cubemap", 12)) return;
  ...
  Q_snprintf(pCacheFileName, sizeof(pCacheFileName),
             "materials/%s" TEXTURE_FNAME_EXTENSION, m_Name.String());
...
#if !defined(_X360)
  m_pLowResImage =
      new unsigned char[m_LowResImageWidth * m_LowResImageHeight * 3];
#ifdef _DEBUG
  bool retVal =
#endif
      ImageLoader::ConvertImageFormat(
          pTexture->LowResImageData(), pTexture->LowResFormat(), m_pLowResImage,
          IMAGE_FORMAT_RGB888, m_LowResImageWidth, m_LowResImageHeight);
  Assert(retVal);
#else
  *(unsigned int *)m_LowResImageSample =
      *(unsigned int *)pTexture->LowResImageSample();
#endif
}

void *CTexture::GetResourceData(uint32_t eDataType, size_t *pnumBytes) const {
  for (DataChunk const *pDataChunk = m_arrDataChunks.Base(),
                       *pDataChunkEnd = pDataChunk + m_arrDataChunks.Count();
       pDataChunk < pDataChunkEnd; ++pDataChunk) {
    if ((pDataChunk->m_eType & ~RSRCF_MASK) == eDataType) {
      if ((pDataChunk->m_eType & RSRCF_HAS_NO_DATA_CHUNK) == 0) {
        if (pnumBytes) *pnumBytes = pDataChunk->m_numBytes;
        return pDataChunk->m_pvData;
      } else {
        if (pnumBytes) *pnumBytes = sizeof(pDataChunk->m_numBytes);

        return (void *)(&pDataChunk->m_numBytes);
      }
    }
  }
  if (pnumBytes) pnumBytes = 0;
  return NULL;
}
...
static void FillCompressedTextureWithSingleColor(
    int red, int green, int blue, int alpha, unsigned char *pImageData,
  ...
  switch (imageFormat) {
    ...
    case IMAGE_FORMAT_ATI1N:  // Invalid block data, but correct memory
                              // footprint
    {
      int i;
    ...
    case IMAGE_FORMAT_ATI2N: {
      ...
      for (i = 0; i < numBlocks; i++) {
        ...
        //				memset( pImageData + i * 16 + 8, 0xffff,
        // 8
        //);
        //// alpha block
      }
...
void CTexture::GenerateShowMipLevelsTextures(IVTFTexture *pTexture) {
  if (pTexture->FaceCount() > 1) return;

  switch (pTexture->Format()) {
    // These are formats that we don't bother with for generating mip level
    // textures.
        ...
        for (int iFace = 0; iFace < pTexture->FaceCount(); ++iFace) {
          for (int iMip = 0; iMip < pTexture->MipCount(); ++iMip) {
            ...
            pTexture->ComputeMipLevelDimensions(iMip, &nWidth, &nHeight,
                                                &nDepth);
            if (pTexture->Format() == IMAGE_FORMAT_DXT1 ||
                pTexture->Format() == IMAGE_FORMAT_DXT5 ||
                pTexture->Format() == IMAGE_FORMAT_ATI1N ||
                pTexture->Format() == IMAGE_FORMAT_ATI2N) {
              unsigned char *pImageData =
                  pTexture->ImageData(iFrame, iFace, iMip, 0, 0, 0);
              int alpha = 255;
              FillCompressedTextureWithSingleColor(
                  red, green, blue, alpha, pImageData, pTexture->Width(),
                  pTexture->Height(), nDepth, pTexture->Format());
            } else {
              for (int z = 0; z < nDepth; ++z) {
                CPixelWriter pixelWriter;
                pixelWriter.SetPixelMemory(
                    pTexture->Format(),
                    pTexture->ImageData(iFrame, iFace, iMip, 0, 0, z),
                    pTexture->RowSizeInBytes(iMip));
...
void CTexture::CopyLowResImageToTexture(IVTFTexture *pTexture) {
  ...
  nFlags &= ~(TEXTUREFLAGS_TRILINEAR | TEXTUREFLAGS_ANISOTROPIC |
  ...
  Init(pTexture->Width(), pTexture->Height(), 1, IMAGE_FORMAT_BGR888, nFlags, 1,
       false);
  pTexture->Init(m_LowResImageWidth, m_LowResImageHeight, 1,
                 IMAGE_FORMAT_BGR888, nFlags, 1);

  // Don't bother computing the actual size; it's actually equal to the low-res
  // size With only one mip level
  ...
  CPixelWriter pixelWriter;
  pixelWriter.SetPixelMemory(pTexture->Format(), pTexture->ImageData(0, 0, 0),
                             pTexture->RowSizeInBytes(0));
...
bool CTexture::SetupDebuggingTextures(IVTFTexture *pVTFTexture) {
  ...
  if (g_config.nShowMipLevels) {
    ...
    if ((g_config.nShowMipLevels == 1) &&
        (pVTFTexture->Flags() & (TEXTUREFLAGS_NORMAL | TEXTUREFLAGS_SSBUMP)))
      return false;

    // mat_showmiplevels 2 means don't do base textures
    if ((g_config.nShowMipLevels == 2) &&
        !(pVTFTexture->Flags() & (TEXTUREFLAGS_NORMAL | TEXTUREFLAGS_SSBUMP)))
      return false;
    ...
    return true;
             ...
             pVTFTexture->FaceCount() == 1 &&
             ((pVTFTexture->Flags() & TEXTUREFLAGS_NORMAL) == 0) &&
...
bool CTexture::ConvertToActualFormat(IVTFTexture *pVTFTexture,
  ...
  if (fmt != dstFormat) {
    ...
    // This is to force at most the precision of int16_t for fp16 texture when
    // running the integer path.
...
void CTexture::GetFilename(char *pOut, int maxLen) const {
  ...
  if (!bIsUNCName) {
    Q_snprintf(pOut, maxLen, "materials/%s" TEXTURE_FNAME_EXTENSION, pName);
  } else {
...
//-----------------------------------------------------------------------------
IVTFTexture *CTexture::LoadTextureBitsFromFile(char *pCacheFileName,
                                               char **ppResolvedFilename,
                                               bool bPaired) {
  int nHeaderSize;
  int nFileSize;
  ...
  while (fileHandle ==
         FILESYSTEM_INVALID_HANDLE)  // run until found a file or out of rules
  {
    fileHandle = g_pFullFileSystem->OpenEx(
    ...
    if (fileHandle == FILESYSTEM_INVALID_HANDLE) {
      ...
      if (pHdrExt) {
        DevWarning(
            "A custom HDR cubemap \"%s\": cannot be found on disk.\n"
  ...
  nBytesRead = g_pFullFileSystem->ReadEx(buf.Base(), nBytesOptimalRead,
                                         nHeaderSize, fileHandle);
  ...
  Init(pVTFTexture->Width(), pVTFTexture->Height(), pVTFTexture->Depth(),
       pVTFTexture->Format(), pVTFTexture->Flags(), pVTFTexture->FrameCount(),
       bPaired);
  ...
  if (unsigned int uiRsrcCount = pVTFTexture->GetResourceTypes(NULL, 0)) {
    uint32_t *arrRsrcTypes =
        (uint32_t *)_alloca(uiRsrcCount * sizeof(unsigned int));
    pVTFTexture->GetResourceTypes(arrRsrcTypes, uiRsrcCount);

    m_arrDataChunks.EnsureCapacity(uiRsrcCount);
    for (uint32_t *arrRsrcTypesEnd = arrRsrcTypes + uiRsrcCount;
      ...
      switch (*arrRsrcTypes) {
        ...
        default: {
          ...
          size_t numBytes;
          if (void *pvData =
                  pVTFTexture->GetResourceData(dc.m_eType, &numBytes)) {
            Assert(numBytes >= sizeof(uint32_t));
  ...
  ConvertToActualFormat(pVTFTexture, bPaired);
...
//-----------------------------------------------------------------------------
void CTexture::ComputeMipLevelSubRect(const Rect_t *pSrcRect, int nMipLevel,
                                      Rect_t *pSubRect) {
  if (nMipLevel == 0) {
    *pSubRect = *pSrcRect;
    return;
...
//-----------------------------------------------------------------------------
void CTexture::FixupTexture(const void *pData, int nSize,
...
// Generates the procedural bits
//-----------------------------------------------------------------------------
IVTFTexture *CTexture::ReconstructPartialProceduralBits(const Rect_t *pRect,
                                                        Rect_t *pActualRect) {
  ...
  pVTFTexture->Init(m_nActualWidth, m_nActualHeight, m_nActualDepth,
                    ComputeActualFormat(m_ImageFormat), m_nFlags,
                    m_nFrameCount);

  // Generate the bits from the installed procedural regenerator
  if (m_pTextureRegenerator) {
    m_pTextureRegenerator->RegenerateTextureBits(this, pVTFTexture,
                                                 pActualRect);
  } else {
  ...
  for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
    ...
    for (int iFace = 0; iFace < nFaceCount; ++iFace) {
      for (int iMip = 0; iMip < m_nActualMipCount; ++iMip) {
        ...
        unsigned char *pBits = pVTFTexture->ImageData(
            iFrame, iFace + nFirstFace, iMip, mipRect.x, mipRect.y, 0);
        g_pShaderAPI->TexSubImage2D(
            iMip, iFace, mipRect.x, mipRect.y, 0, mipRect.width, mipRect.height,
            pVTFTexture->Format(), nStride, false, pBits);
      }
    }
  }
...
void CTexture::WriteDataToShaderAPITexture(int nFrameCount, int nFaceCount,
                                           int nFirstFace, int nMipCount,
  ...
  for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
    ...
    for (int iFace = 0; iFace < nFaceCount; ++iFace) {
      for (int iMip = 0; iMip < nMipCount; ++iMip) {
        ...
        for (int z = 0; z < nDepth; ++z) {
          pBits =
              pVTFTexture->ImageData(iFrame, iFace + nFirstFace, iMip, 0, 0, z);

          g_pShaderAPI->TexImage2D(iMip, iFace, fmt, z, nWidth, nHeight,
...
void CTexture::ReconstructTexture() {
  ...
  if (IsProcedural()) {
    ...
    if (!bIsUNCName) {
      ...
      Q_snprintf(pCacheFileName, sizeof(pCacheFileName),
                 "%s" TEXTURE_FNAME_EXTENSION, pName);
    }
    ...
    // Currently 360 only
    // If we support normal compression and file ends in _normal, see if there's
    // a compressed version on disk
    if (IsX360() && g_pShaderAPI->SupportsNormalMapCompression() &&
        V_strstr(pCacheFileName, TEXTURE_FNAME_EXTENSION_NORMAL)) {
      V_strncpy(pFileNameATI2N, pCacheFileName,
                strlen(pCacheFileName) - TEXTURE_FNAME_EXTENSION_LEN + 1);
      ...
      if (g_pFullFileSystem->FileExists(
              pFileNameATI2N,
              MaterialSystem()->GetForcedTextureLoadPathID()))  // Both _xya and
                                                                // _a are there
      {
        V_strncpy(pFileNameATI1N, pCacheFileName,
                  strlen(pCacheFileName) - TEXTURE_FNAME_EXTENSION_LEN + 1);
        ...
        if (g_pFullFileSystem->FileExists(
                pFileNameATI2N,
                MaterialSystem()
                    ->GetForcedTextureLoadPathID()))  // Only _xy is there
        {
          bHasATI2N = true;
        }
      }
    ...
    pVTFTexture = LoadTextureBitsFromFile(
        bHasATI2N ? pFileNameATI2N : pCacheFileName, &pResolvedFilename, false);
    if (bHasATI1N) {
    ...
    // format, pretty much *anything* can change.
    pVTFTexture =
        LoadTextureBitsFromFile(pCacheFileName, &pResolvedFilename, false);
#endif
  }

  if (!HasBeenAllocated() || m_nActualWidth != oldWidth ||
      m_nActualHeight != oldHeight || m_nActualDepth != oldDepth ||
      m_nActualMipCount != oldMipCount || m_nFrameCount != oldFrameCount) {
    if (HasBeenAllocated()) {
  ...
  if (IsRenderTarget()) {
    ...
    pRenderContext->PushRenderTargetAndViewport(
        pThisTexture);  // Push this texture on the stack
    g_pShaderAPI->ClearColor4ub(0, 0, 0,
                                0xFF);  // Set the clear color to opaque black
    g_pShaderAPI->ClearBuffers(true, false, false, m_nActualWidth,
                               m_nActualHeight);  // Clear the target
    // Pop back to previous target
  ...
  WriteDataToShaderAPITexture(m_nFrameCount, nFaceCount, nFirstFace,
                              m_nActualMipCount, pVTFTexture, m_ImageFormat);
#if defined(COMPRESSED_NORMAL_FORMATS)
  if (bHasATI1N) {
    WriteDataToShaderAPITexture(m_nFrameCount, nFaceCount, nFirstFace,
                                m_nActualMipCount, pVTFTextureA,
                                m_ImageFormatPaired);
  }
#endif
  ...
  FreeOptimalReadBuffer(4 * 1024 * 1024);
}

// Get the shaderapi texture handle associated w/ a particular frame
ShaderAPITextureHandle_t CTexture::GetTextureHandle(int nFrame,
                                                    int nTextureChannel) {
...
#if defined(COMPRESSED_NORMAL_FORMATS)
  if (nTextureChannel)
    return m_pTextureHandlesPaired[nFrame];  // Don't get paired handle
                                             // here...callers of this function
                                             // don't know about paired textures
#endif

  return m_pTextureHandles[nFrame];  // Don't get paired handle here...callers
                                     // of this function don't know about paired
                                     // textures
}

void CTexture::GetLowResColorSample(float s, float t, float *color) const {
  if (m_LowResImageWidth <= 0 || m_LowResImageHeight <= 0) {
    //		Warning( "Programming error: GetLowResColorSample \"%s\":
  ...
  sColor[0][0] =
      (1.0f - fracS) *
      (m_pLowResImage[(wholeS + wholeT * m_LowResImageWidth) * 3 + 0] *
       (1.0f / 255.0f));
  sColor[0][1] =
      (1.0f - fracS) *
      (m_pLowResImage[(wholeS + wholeT * m_LowResImageWidth) * 3 + 1] *
       (1.0f / 255.0f));
  sColor[0][2] =
      (1.0f - fracS) *
      (m_pLowResImage[(wholeS + wholeT * m_LowResImageWidth) * 3 + 2] *
       (1.0f / 255.0f));
  sColor[0][0] +=
      fracS *
      (m_pLowResImage[(wholeSPlusOne + wholeT * m_LowResImageWidth) * 3 + 0] *
       (1.0f / 255.0f));
  sColor[0][1] +=
      fracS *
      (m_pLowResImage[(wholeSPlusOne + wholeT * m_LowResImageWidth) * 3 + 1] *
       (1.0f / 255.0f));
  sColor[0][2] +=
      fracS *
      (m_pLowResImage[(wholeSPlusOne + wholeT * m_LowResImageWidth) * 3 + 2] *
       (1.0f / 255.0f));

  sColor[1][0] =
      (1.0f - fracS) *
      (m_pLowResImage[(wholeS + wholeTPlusOne * m_LowResImageWidth) * 3 + 0] *
       (1.0f / 255.0f));
  sColor[1][1] =
      (1.0f - fracS) *
      (m_pLowResImage[(wholeS + wholeTPlusOne * m_LowResImageWidth) * 3 + 1] *
       (1.0f / 255.0f));
  sColor[1][2] =
      (1.0f - fracS) *
      (m_pLowResImage[(wholeS + wholeTPlusOne * m_LowResImageWidth) * 3 + 2] *
       (1.0f / 255.0f));
  sColor[1][0] +=
      fracS *
      (m_pLowResImage[(wholeSPlusOne + wholeTPlusOne * m_LowResImageWidth) * 3 +
                      0] *
       (1.0f / 255.0f));
  sColor[1][1] +=
      fracS *
...
int CTexture::GetApproximateVidMemBytes(void) const {
  ...
  return numFrames *
         ImageLoader::GetMemRequired(width, height, depth, format, isMipmapped);
}

void CTexture::CopyFrameBufferToMe(int nRenderTargetID, Rect_t *pSrcRect,
                                   Rect_t *pDstRect) {
  Assert(m_pTextureHandles && m_nFrameCount >= 1);

  if (m_pTextureHandles && m_nFrameCount >= 1) {
    g_pShaderAPI->CopyRenderTargetToTextureEx(
        m_pTextureHandles[0], nRenderTargetID, pSrcRect, pDstRect);
  }
}
...
// Swap everything about a texture except the name. Created to support Portal
// mod's need for swapping out water render targets in recursive stencil views
void CTexture::SwapContents(ITexture *pOther) {
  if ((pOther == NULL) || (pOther == this)) return;

  AssertMsg(dynamic_cast<CTexture *>(pOther) != NULL,
...
bool CTexture::UpdateExcludedState(void) {
  ...
  if ((bDesired == bActual) &&
      (m_nDesiredDimensionLimit == m_nActualDimensionLimit)) {
    return false;
  }
  ...
  // force the texture to re-download, causes the texture bits to match its
  // desired exclusion state
  Download();
...
static bool SetBufferValue(char *chTxtFileBuffer, char const *szLookupKey,
  ...
  size_t lenLookupKey = strlen(szLookupKey);

  for (char *pch = chTxtFileBuffer; (NULL != (pch = strstr(pch, szLookupKey)));
       ++pch) {
    char *val = pch + lenLookupKey;
    if (!isspace(*val))
      continue;
    else
      ++val;
    ...
    memmove(pValStart + lenTmp, pValEnd,
            chTxtFileBuffer + nTxtFileBufferLen + 1 - pValEnd);
...
static char *BufferReplace(char *buf, char const *szFindData,
                           char const *szNewData) {
  size_t len = strlen(buf), lFind = strlen(szFindData),
         lNew = strlen(szNewData);
...
CP4Requirement::CP4Requirement() : m_bLoadedModule(false), m_pP4Module(NULL) {
  if (p4) return;
...
CON_COMMAND_F(mat_texture_list_txlod_sync,
              "'reset' - resets all run-time changes to LOD overrides, 'save' "
  ...
  if (!stricmp(szCmd, "reset")) {
    ...
    for (int k = 0; k < s_OverrideMap.GetNumStrings(); ++k) {
      ...
      // 1. look for TGA - if found, get the txt file (if txt file missing,
      // create one)
...
usage:
  Warning(
      "Usage:\n"
      "  mat_texture_list_txlod_sync reset - resets all run-time changes to "
      "LOD overrides;\n"
      "  mat_texture_list_txlod_sync save  - saves all changes to material "
      "content files.\n");
}
#endif
