...
class CMatNullRenderContext : public CMatRenderContextBase {
 public:
  CMatNullRenderContext() : m_WidthBackBuffer(0), m_HeightBackBuffer(0) {}

  virtual void InitializeFrom(CMatRenderContextBase *pInitialState) {
    CMatRenderContextBase::InitializeFrom(pInitialState);
    g_pShaderAPI->GetBackBufferDimensions(m_WidthBackBuffer,
                                          m_HeightBackBuffer);
  }
  ...
  virtual IMesh *GetDynamicMeshEx(VertexFormat_t, bool, IMesh *, IMesh *,
                                  IMaterial *) {
    AssertMsg(0,
              "CMatNullRenderContext only provides base features, not a stub "
              "(right now)");
  ...
  void ClearColor4ub(unsigned char, unsigned char, unsigned char,
                     unsigned char) {
    AssertMsg(0,
              "CMatNullRenderContext only provides base features, not a stub "
              "(right now)");
  ...
  void DrawScreenSpaceRectangle(IMaterial *, int, int, int, int, float, float,
                                float, float, int, int, void *, int, int) {
    AssertMsg(0,
              "CMatNullRenderContext only provides base features, not a stub "
              "(right now)");
  }
  ...
  void SetStencilTestMask(uint32_t) {
    AssertMsg(0,
              "CMatNullRenderContext only provides base features, not a stub "
              "(right now)");
  }
  ...
  void SetFlashlightStateEx(const FlashlightState_t &, const VMatrix &,
                            ITexture *) {
    AssertMsg(0,
              "CMatNullRenderContext only provides base features, not a stub "
              "(right now)");
  ...
  void DestroyVertexBuffer(IVertexBuffer *) { Assert(0); }
  void DestroyIndexBuffer(IIndexBuffer *) { Assert(0); }
  ...
  virtual void BeginMorphAccumulation() { Assert(0); }
  virtual void EndMorphAccumulation() { Assert(0); }
