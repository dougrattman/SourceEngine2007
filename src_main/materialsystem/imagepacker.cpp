...
float CImagePacker::GetEfficiency(void) {
  return (float)m_AreaUsed /
         (float)(m_MaxLightmapWidth * CeilPow2(m_MinimumHeight));
}

bool CImagePacker::Reset(int nSortId, int maxLightmapWidth,
...
bool CImagePacker::AddBlock(int width, int height, int *returnX, int *returnY) {
  ...
  *returnY = outerMinY + 1;
  ...
  if (*returnY + height > m_MinimumHeight) m_MinimumHeight = *returnY + height;
