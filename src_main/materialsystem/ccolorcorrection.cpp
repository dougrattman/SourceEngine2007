...
struct ColorCorrectionLookup_t {
  ...
  color24 m_pColorCorrection[COLOR_CORRECTION_TEXTURE_SIZE *
...
ColorCorrectionLookup_t::~ColorCorrectionLookup_t() {
  ...
  if (m_pColorCorrectionTexture) {
    for (int i = 0; i < COLOR_CORRECTION_MAX_TEXTURES; i++) {
      if (m_pColorCorrectionTexture ==
          TextureManager()->ColorCorrectionTexture(i)) {
...
void ColorCorrectionLookup_t::AllocTexture() {
  ...
  m_pColorCorrectionTexture = ITextureInternal::CreateProceduralTexture(
      ...
      COLOR_CORRECTION_TEXTURE_SIZE, COLOR_CORRECTION_TEXTURE_SIZE,
  ...
  extern void CreateColorCorrectionTexture(ITextureInternal * pTexture,
                                           ColorCorrectionHandle_t handle);
...
void ColorCorrectionLookup_t::RestoreTexture() {
  ...
  m_pColorCorrectionTexture->OnRestore();  // Give render targets a chance to
                                           // reinitialize themselves if
                                           // necessary (due to AA changes).
  m_pColorCorrectionTexture->Download();
}
...
class CColorCorrectionSystem : public IColorCorrectionSystem {
 public:
  ...
  virtual void SetLookup(ColorCorrectionHandle_t handle, RGBX5551_t inColor,
                         color24 outColor);
  ...
  virtual void CopyLookup(ColorCorrectionHandle_t handle,
 ...
 protected:
  ...
  void SetLookupPtr(ColorCorrectionLookup_t *lookup, RGBX5551_t inColor,
                    color24 outColor);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CColorCorrectionSystem,
                                  IColorCorrectionSystem,
                                  COLORCORRECTION_INTERFACE_VERSION,
...
//-----------------------------------------------------------------------------
void ReleaseColorCorrection() { g_pColorCorrectionSystem->ReleaseTextures(); }
...
void CColorCorrectionSystem::SortLookups() {
  ...
  for (int i = 0;
       i < COLOR_CORRECTION_MAX_TEXTURES && i < m_ColorCorrectionList.Count();
       i++) {
    TextureManager()->SetColorCorrectionTexture(
        i, m_ColorCorrectionList[i]->m_pColorCorrectionTexture);
  }

  for (int i = m_ColorCorrectionList.Count(); i < COLOR_CORRECTION_MAX_TEXTURES;
       i++) {
    TextureManager()->SetColorCorrectionTexture(i, nullptr);
  }
}
...
ColorCorrectionLookup_t *CColorCorrectionSystem::FindLookup(
    ColorCorrectionHandle_t handle) {
  for (int i = 0; i < m_ColorCorrectionList.Count(); i++) {
...
ColorCorrectionHandle_t CColorCorrectionSystem::GetLookupHandle(
    const char *pName) {
...
ColorCorrectionHandle_t CColorCorrectionSystem::AddLookup(const char *pName) {
  ...
  if (handle == m_DefaultColorCorrectionHandle) return handle;
...
bool CColorCorrectionSystem::RemoveLookup(ColorCorrectionHandle_t handle) {
  if (handle == m_DefaultColorCorrectionHandle) return false;
...
void CColorCorrectionSystem::SetLookupWeight(ColorCorrectionHandle_t handle,
                                             float flWeight) {
...
float CColorCorrectionSystem::GetLookupWeight(int i) {
  if (i < m_ColorCorrectionList.Count()) {
    if (i == -1) return m_DefaultColorCorrectionWeight;
...
void CColorCorrectionSystem::SetLookup(ColorCorrectionHandle_t handle,
                                       RGBX5551_t inColor, color24 outColor) {
  if (handle == m_DefaultColorCorrectionHandle) return;
...
//  Set the mapping for the specified color correction lookup
void CColorCorrectionSystem::SetLookupPtr(ColorCorrectionLookup_t *lookup,
                                          RGBX5551_t inColor,
                                          color24 outColor) {
  Assert(lookup->m_bLocked);
  lookup->m_pColorCorrection[inColor.r +
                             inColor.g * COLOR_CORRECTION_TEXTURE_SIZE +
                             inColor.b * COLOR_CORRECTION_TEXTURE_SIZE *
                                 COLOR_CORRECTION_TEXTURE_SIZE] = outColor;
}

//  Get the mapping for the specified color correction lookup
color24 CColorCorrectionSystem::GetLookup(ColorCorrectionHandle_t handle,
                                          RGBX5551_t inColor) {
  if (handle == m_DefaultColorCorrectionHandle) {
  ...
  return lookup->m_pColorCorrection[inColor.r +
                                    inColor.g * COLOR_CORRECTION_TEXTURE_SIZE +
                                    inColor.b * COLOR_CORRECTION_TEXTURE_SIZE *
                                        COLOR_CORRECTION_TEXTURE_SIZE];
}
...
void CColorCorrectionSystem::LoadLookup(ColorCorrectionHandle_t handle,
                                        const char *pLookupName) {
  if (handle == m_DefaultColorCorrectionHandle) {
    return;
  }
  ...
  int res =
      (int)powf((float)(colorBuff.TellMaxPut() / sizeof(color24)), 1.0f / 3.0f);
  if (res != COLOR_CORRECTION_TEXTURE_SIZE ||
...
void CColorCorrectionSystem::CopyLookup(ColorCorrectionHandle_t handle,
                                        const color24 *pSrcColorCorrection) {
  if (handle == m_DefaultColorCorrectionHandle) {
    return;
  }
  ...
  Q_memcpy(lookup->m_pColorCorrection, pSrcColorCorrection,
           sizeof(color24) * COLOR_CORRECTION_TEXTURE_SIZE *
               COLOR_CORRECTION_TEXTURE_SIZE * COLOR_CORRECTION_TEXTURE_SIZE);
}
...
int CColorCorrectionSystem::GetNumLookups() {
  int i;
  for (i = 0;
       i < m_ColorCorrectionList.Count() && i < COLOR_CORRECTION_MAX_TEXTURES;
       i++) {
    if (m_ColorCorrectionList[i]->m_flWeight <= 0.0f) break;
  }
...
void CColorCorrectionSystem::SetResetable(ColorCorrectionHandle_t handle,
                                          bool bResetable) {
  ColorCorrectionLookup_t *lookup = FindLookup(handle);

  if (lookup) lookup->m_bResetable = bResetable;
}
