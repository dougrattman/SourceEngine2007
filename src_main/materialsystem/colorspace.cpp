...
// static float			texLightToLinear[256];	// texlight (0..255) to
// linear (0..4)
...
static int linearToScreen[1024];    // linear (0..1) to gamma corrected vertex
                                    // light (0..255)
float g_LinearToVertex[4096];  // linear (0..4) to screen corrected vertex space
                               // (0..1?)
static int linearToLightmap[4096];  // linear (0..4) to screen corrected texture
                                    // value (0..255)

void ColorSpace::SetGamma(float screenGamma, float texGamma, float overbright,
  ...
  // pow( textureColor, g1 ) converts from on-disk texture space to framebuffer
  // space
  ...
  for (i = 0; i < 256; i++) {
    ...
    textureToLinear[i] = pow(i / 255.0f, texGamma);
  }
  ...
  for (i = 0; i < 4096; i++) {
    ...
    f = pow(i / 1024.0f, 1.0f / screenGamma);

    g_LinearToVertex[i] = f * overbrightFactor;
    if (g_LinearToVertex[i] > 1) g_LinearToVertex[i] = 1;

    linearToLightmap[i] = (int)(f * 255 * overbrightFactor);
    if (linearToLightmap[i] > 255) linearToLightmap[i] = 255;
  }
}
...
float ColorSpace::TexLightToLinear(int c, int exponent) {
  ...
  // optimize me
  return (float)c * pow(2.0f, exponent) * (1.0f / 255.0f);
}
...
u16 ColorSpace::LinearFloatToCorrectedShort(float in) {
