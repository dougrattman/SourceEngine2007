...
#ifndef SOURCE_MATERIALSYSTEM_CMATERIALLIGHTMAPS_H_
#define SOURCE_MATERIALSYSTEM_CMATERIALLIGHTMAPS_H_
...
class CMatLightmaps {
 public:
  ...
  // NOTE: This returns a lightmap page ID, not a sortID like
  // AllocateLightmap!!!!
  int AllocateDynamicLightmap(int lightmapSize[2], int *pOutOffsetIntoPage,
                              int frameID);
  ...
  ShaderAPITextureHandle_t GetLightmapPageTextureHandle(int lightmap) {
    return m_LightmapPageTextureHandles[lightmap];
  }
  bool IsDynamicLightmap(int lightmap) const {
    return (lightmap >= m_firstDynamicLightmap) ? true : false;
  }
 ...
 private:
  ...
  void BumpedLightmapBitsToPixelWriter_LDR(
  ...
  void BumpedLightmapBitsToPixelWriter_HDRF(
      float *pFloatImage, float *pFloatImageBump1, float *pFloatImageBump2,
      float *pFloatImageBump3, int pLightmapSize[2],
      int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut);

  void BumpedLightmapBitsToPixelWriter_HDRI(
      float *pFloatImage, float *pFloatImageBump1, float *pFloatImageBump2,
      float *pFloatImageBump3, int pLightmapSize[2],
      int pOffsetIntoLightmapPage[2], FloatBitMap_t *pfmOut);

  void LightmapBitsToPixelWriter_LDR(float *pFloatImage, int pLightmapSize[2],
                                     int pOffsetIntoLightmapPage[2],
                                     FloatBitMap_t *pfmOut);
  void LightmapBitsToPixelWriter_HDRF(float *pFloatImage, int pLightmapSize[2],
                                      int pOffsetIntoLightmapPage[2],
                                      FloatBitMap_t *pfmOut);
  void LightmapBitsToPixelWriter_HDRI(float *pFloatImage, int pLightmapSize[2],
                                      int pOffsetIntoLightmapPage[2],
                                      FloatBitMap_t *pfmOut);

  // For computing sort info
  void ComputeSortInfo(MaterialSystem_SortInfo_t *pInfo, int &sortId,
                       bool alpha);
  void ComputeWhiteLightmappedSortInfo(MaterialSystem_SortInfo_t *pInfo,
  ...
  enum LightmapsState {
    ...
    STATE_RELEASED  //	Lightmaps in released state - usually due to
                    // lost/released D3D device, D3D operations disallowed,
                    // Restore() call to follow
  };
  LightmapsState m_eLightmapsState;
};

#endif  // SOURCE_MATERIALSYSTEM_CMATERIALLIGHTMAPS_H_
