...
ConVar mat_supportflashlight("mat_supportflashlight", "-1", 0,
                             "0 - do not support flashlight (don't load "
                             "flashlight shader combos), 1 - flashlight is "
                             "supported");
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(MaterialSystem_Config_t,
                                  MaterialSystem_Config_t,
                                  MATERIALSYSTEM_CONFIG_VERSION, g_config);
...
void *ShaderFactory(const char *pName, int *pReturnCode) {
  ...
  if (!Q_stricmp(pName, FILESYSTEM_INTERFACE_VERSION)) return g_pFullFileSystem;
...
class CResourcePreloadMaterial : public CResourcePreload {
  virtual bool CreateResource(const char *pName) {
    IMaterial *pMaterial =
        g_MaterialSystem.FindMaterial(pName, TEXTURE_GROUP_WORLD, false);
    IMaterialInternal *pMatInternal =
        static_cast<IMaterialInternal *>(pMaterial);
    if (pMatInternal) {
  ...
  //-----------------------------------------------------------------------------
  // Called before queued loader i/o jobs are actually performed. Must free up
  // memory to ensure i/o requests have enough memory to succeed.  The materials
  // that were touched by the CreateResource() are inhibited from purging (as is
  // their textures, by virtue of ref counts), all others are candidates.  The
  // preloaded materials are by definition zero ref'd until owned by the normal
  // loading process. Any material that stays zero ref'd is a candidate for the
  // post load purge.
  //-----------------------------------------------------------------------------
  virtual void PurgeUnreferencedResources() {
    ...
    for (MaterialHandle_t hMaterial = g_MaterialSystem.FirstMaterial();
         hMaterial != g_MaterialSystem.InvalidMaterial(); hMaterial = hNext) {
      hNext = g_MaterialSystem.NextMaterial(hMaterial);

      IMaterialInternal *pMatInternal =
          g_MaterialSystem.GetMaterialInternal(hMaterial);
...
class CResourcePreloadCubemap : public CResourcePreload {
  virtual bool CreateResource(const char *pName) {
    ITexture *pTexture =
        g_MaterialSystem.FindTexture(pName, TEXTURE_GROUP_CUBE_MAP, true);
    ITextureInternal *pTexInternal = static_cast<ITextureInternal *>(pTexture);
    if (pTexInternal) {
      // There can be cubemaps that are unbound by materials. To prevent an
      // unwanted purge, mark and increase the ref count. Otherwise the
      // pre-purge discards these zero ref'd textures, and then the normal
      // loading process hitches on the miss. The zombie cubemaps DO get
      // discarded after the normal loading process completes if no material
      // references them.
  ...
  //-----------------------------------------------------------------------------
  // All valid cubemaps should have been owned by their materials.  Undo the
  // preloaded cubemap locks. Any zero ref'd cubemaps will be purged by the
  // normal loading path conclusion.
  //-----------------------------------------------------------------------------
...
void CMaterialSystem::CreateDebugMaterials() {
  if (!m_pDrawFlatMaterial) {
    ...
    pVMTKeyValues->SetString("$basetexture",
                             "error");  // This is the "error texture"
    g_pErrorMaterial = static_cast<IMaterialInternal *>(
                           CreateMaterial("___error.vmt", pVMTKeyValues))
                           ->GetRealTimeVersion();

    pVMTKeyValues = new KeyValues("UnlitGeneric");
    pVMTKeyValues->SetInt("$flat", 1);
    pVMTKeyValues->SetInt("$vertexcolor", 1);
    m_pDrawFlatMaterial = static_cast<IMaterialInternal *>(
                              CreateMaterial("___flat.vmt", pVMTKeyValues))
                              ->GetRealTimeVersion();

    pVMTKeyValues = new KeyValues("BufferClearObeyStencil");
    pVMTKeyValues->SetInt("$nocull", 1);
    m_pBufferClearObeyStencil[BUFFER_CLEAR_NONE] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil0.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
    ...
    pVMTKeyValues->SetInt("$vertexcolor", 1);
    m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil1.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();

    pVMTKeyValues = new KeyValues("BufferClearObeyStencil");
    pVMTKeyValues->SetInt("$nocull", 1);
    pVMTKeyValues->SetInt("$cleardepth", 1);
    m_pBufferClearObeyStencil[BUFFER_CLEAR_DEPTH] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil2.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
    ...
    pVMTKeyValues->SetInt("$clearcolor", 1);
    pVMTKeyValues->SetInt("$vertexcolor", 1);
    m_pBufferClearObeyStencil[BUFFER_CLEAR_COLOR_AND_DEPTH] =
        static_cast<IMaterialInternal *>(
            CreateMaterial("___buffer_clear_obey_stencil3.vmt", pVMTKeyValues))
            ->GetRealTimeVersion();
...
CMaterialSystem::~CMaterialSystem() { delete[] m_pShaderDLL; }
...
void CMaterialSystem::SetShaderAPI(char const *pShaderAPIDLL) {
  ...
  if (!m_ShaderAPIFactory) {
    DestroyShaderAPI();
    Error("Can't set the shader API from '%s'!\n", pShaderAPIDLL);
  }
}
...
bool CMaterialSystem::Connect(CreateInterfaceFn factory) {
  ...
  g_pShaderDeviceMgr = (IShaderDeviceMgr *)m_ShaderAPIFactory(
      SHADER_DEVICE_MGR_INTERFACE_VERSION, 0);
  if (!g_pShaderDeviceMgr) return false;
  g_pHWConfig = (IHardwareConfigInternal *)m_ShaderAPIFactory(
  ...
  g_pShaderDevice =
      (IShaderDevice *)m_ShaderAPIFactory(SHADER_DEVICE_INTERFACE_VERSION, 0);
  if (!g_pShaderDevice) return false;
  g_pShaderShadow =
      (IShaderShadow *)m_ShaderAPIFactory(SHADERSHADOW_INTERFACE_VERSION, 0);
  if (!g_pShaderShadow) return false;
...
void *CMaterialSystem::QueryInterface(const char *pInterfaceName) {
  ...
  CreateInterfaceFn factory =
      Sys_GetFactoryThis();  // This silly construction is necessary
  return factory(pInterfaceName,
                 NULL);  // to prevent the LTCG compiler from crashing.
}
...
InitReturnVal_t CMaterialSystem::Init() {
  ...
  // HACKHACK: <sigh> This horrible hack is possibly the only way to reliably
  // detect an old version of hammer initializing the material system. We need
...
//-----------------------------------------------------------------------------
CreateInterfaceFn CMaterialSystem::Init(
    char const *pShaderAPIDLL, IMaterialProxyFactory *pMaterialProxyFactory,
  ...
  if (!Connect(TempCreateInterface)) return 0;
...
//-----------------------------------------------------------------------------
IMaterialSystemHardwareConfig *CMaterialSystem::GetHardwareConfig(
    const char *pVersion, int *returnCode) {
  return (IMaterialSystemHardwareConfig *)m_ShaderAPIFactory(pVersion,
                                                             returnCode);
}
...
//
//-----------------------------------------------------------------------------
void CMaterialSystem::SetThreadMode(MaterialThreadMode_t nextThreadMode,
                                    int nServiceThread) {
  m_IdealThreadMode = nextThreadMode;
  m_nServiceThread = nServiceThread;
}

MaterialThreadMode_t CMaterialSystem::GetThreadMode() { return m_ThreadMode; }
...
//
//-----------------------------------------------------------------------------
IMatRenderContext *CMaterialSystem::CreateRenderContext(
...
//
//-----------------------------------------------------------------------------
IMatRenderContext *CMaterialSystem::SetRenderContext(
    IMatRenderContext *pNewContext) {
...
void CMaterialSystem::GetDisplayAdapterInfo(int adapter,
...
//-----------------------------------------------------------------------------
static void ConvertModeStruct(ShaderDeviceInfo_t *pMode,
  ...
  pMode->m_DisplayMode.m_nRefreshRateNumerator =
      config.m_VideoMode.m_RefreshRate;
  pMode->m_DisplayMode.m_nRefreshRateDenominator =
      config.m_VideoMode.m_RefreshRate ? 1 : 0;
...
static void ConvertModeStruct(MaterialVideoMode_t *pMode,
                              const ShaderDisplayMode_t &info) {
  pMode->m_Width = info.m_nWidth;
  pMode->m_Height = info.m_nHeight;
  pMode->m_Format = info.m_Format;
  pMode->m_RefreshRate =
      info.m_nRefreshRateNumerator / info.m_nRefreshRateDenominator;
}

//-----------------------------------------------------------------------------
// Returns mode information..
//-----------------------------------------------------------------------------
void CMaterialSystem::GetModeInfo(int nAdapter, int nMode,
                                  MaterialVideoMode_t &info) const {
...
bool CMaterialSystem::SetMode(void *hwnd,
  ...
  if (!bPreviouslyUsingGraphics) {
    if (IsPC()) {
      ...
      if (MaterialSystem()->CanUseEditorMaterials()) {
        // We are in Hammer.  Allocate these here since we aren't going to
        // allocate lightmaps. HACK! NOTE! : Overbright is 1.0 so that Hammer
        // will work properly with the white bumped and unbumped lightmaps.
  ...
  g_pShaderDevice->SetHardwareGammaRamp(
      config.m_fMonitorGamma, config.m_fGammaTVRangeMin,
      config.m_fGammaTVRangeMax, config.m_fGammaTVExponent,
      config.m_bGammaTVEnabled);
...
void CMaterialSystem::RestoreShaderObjects(CreateInterfaceFn shaderFactory,
                                           int nChangeFlags) {
  if (shaderFactory) {
    g_pShaderAPI =
        (IShaderAPI *)shaderFactory(SHADERAPI_INTERFACE_VERSION, NULL);
    g_pShaderDevice =
        (IShaderDevice *)shaderFactory(SHADER_DEVICE_INTERFACE_VERSION, NULL);
    g_pShaderShadow =
        (IShaderShadow *)shaderFactory(SHADERSHADOW_INTERFACE_VERSION, NULL);
  }

  for (MaterialHandle_t i = m_MaterialDict.FirstMaterial();
       i != m_MaterialDict.InvalidMaterial();
       i = m_MaterialDict.NextMaterial(i)) {
    IMaterialInternal *pMat = m_MaterialDict.GetMaterialInternal(i);
    if (pMat) pMat->ReportVarChanged(NULL);
  }
  ...
  g_pShaderAPI->SetAnisotropicLevel(
      GetCurrentConfigForVideoCard().m_nForceAnisotropicLevel);
...
bool CMaterialSystem::ConvertImageFormat(unsigned char *src,
                                         enum ImageFormat srcImageFormat,
                                         unsigned char *dst,
                                         enum ImageFormat dstImageFormat,
                                         int width, int height, int srcStride,
                                         int dstStride) {
  return ImageLoader::ConvertImageFormat(src, srcImageFormat, dst,
                                         dstImageFormat, width, height,
                                         srcStride, dstStride);
}
...
//-----------------------------------------------------------------------------
int CMaterialSystem::GetMemRequired(int width, int height, int depth,
                                    ImageFormat format, bool mipmap) {
...
//-----------------------------------------------------------------------------
ImageFormatInfo_t const &CMaterialSystem::ImageFormatInfo(
...
//-----------------------------------------------------------------------------
static void ReadInt(KeyValues *pGroup, const char *pName, int nDefaultVal,
                    int nUndefinedVal, int *pDest) {
...
//-----------------------------------------------------------------------------
void CMaterialSystem::GenerateConfigFromConfigKeyValues(
    MaterialSystem_Config_t *pConfig, bool bOverwriteCommandLineValues) {
...
//-----------------------------------------------------------------------------
static void MatProxyCallback(IConVar *pConVar, const char *old,
                             float flOldValue) {
...
//-----------------------------------------------------------------------------
static ConVar mat_vsync("mat_vsync", "0", 0, "Force sync to vertical retrace",
                        true, 0.0, true, 1.0);
static ConVar mat_forcehardwaresync("mat_forcehardwaresync",
                                    IsPC() ? "1" : "0");
...
static void mat_showmiplevels_Callback_f(IConVar *var, const char *pOldValue,
                                         float flOldValue) {
  // turn off texture filtering if we are showing mip levels.
  mat_filtertextures.SetValue(((ConVar *)var)->GetInt() == 0);
}
// Debugging textures
static ConVar mat_showmiplevels(
    "mat_showmiplevels", "0", 0,
    "color-code miplevels 2: normalmaps, 1: everything else",
    mat_showmiplevels_Callback_f);

static ConVar mat_specular("mat_specular", "1", 0,
                           "Enable/Disable specularity for perf testing.  Will "
                           "cause a material reload upon change.");
...
static ConVar mat_slopescaledepthbias_normal("mat_slopescaledepthbias_normal",
                                             "0.0f", FCVAR_CHEAT);
static ConVar mat_depthbias_normal("mat_depthbias_normal", "0.0f", FCVAR_CHEAT);
static ConVar mat_slopescaledepthbias_decal(
    "mat_slopescaledepthbias_decal", "-0.5",
    FCVAR_CHEAT);  // Reciprocals of these biases sent to API
static ConVar mat_depthbias_decal("mat_depthbias_decal", "-262144",
                                  FCVAR_CHEAT);  //

static ConVar mat_slopescaledepthbias_shadowmap(
    "mat_slopescaledepthbias_shadowmap", "16", FCVAR_CHEAT);
static ConVar mat_depthbias_shadowmap("mat_depthbias_shadowmap", "0.0005",
                                      FCVAR_CHEAT);

static ConVar mat_monitorgamma(
    "mat_monitorgamma", "2.2", 0,
    "monitor gamma (typically 2.2 for CRT and 1.7 for LCD)", true, 1.6f, true,
    2.6f);
static ConVar mat_monitorgamma_tv_range_min("mat_monitorgamma_tv_range_min",
                                            "16");
static ConVar mat_monitorgamma_tv_range_max("mat_monitorgamma_tv_range_max",
                                            "255");
// TV's generally have a 2.5 gamma, so we need to convert our 2.2 frame buffer
// into a 2.5 frame buffer for display on a TV
static ConVar mat_monitorgamma_tv_exp("mat_monitorgamma_tv_exp", "2.5", 0, "",
                                      true, 1.0f, true, 4.0f);
static ConVar mat_monitorgamma_tv_enabled("mat_monitorgamma_tv_enabled", "0",
                                          FCVAR_ARCHIVE, "");
...
static ConVar mat_fastspecular("mat_fastspecular", "1", 0,
                               "Enable/Disable specularity for visual testing. "
                               " Will not reload materials and will not affect "
                               "perf.");
static ConVar mat_fastnobump(
    "mat_fastnobump", "0",
    FCVAR_CHEAT);  // Binds 1-texel normal map for quick internal testing

// These are not controlled by the material system, but are limited by settings
...
uint32_t g_nDebugVarsSignature = 0;
...
void CMaterialSystem::ReadConfigFromConVars(MaterialSystem_Config_t *pConfig) {
  ...
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_FORCE_TRILINEAR,
                   mat_trilinear.GetBool());
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_DISABLE_SPECULAR,
                   !mat_specular.GetBool());
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_DISABLE_BUMPMAP, !mat_bumpmap.GetBool());
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_ENABLE_PARALLAX_MAPPING,
                   mat_parallaxmap.GetBool());
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_REDUCE_FILLRATE,
                   mat_reducefillrate.GetBool());
  pConfig->m_nForceAnisotropicLevel = std::max(mat_forceaniso.GetInt(), 1);
  pConfig->dxSupportLevel = mat_dxlevel.GetInt();
  pConfig->skipMipLevels = mat_picmip.GetInt();
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_FORCE_HWSYNC,
                   mat_forcehardwaresync.GetBool());
  pConfig->m_SlopeScaleDepthBias_Decal =
      mat_slopescaledepthbias_decal.GetFloat();
  pConfig->m_DepthBias_Decal = mat_depthbias_decal.GetFloat();
  pConfig->m_SlopeScaleDepthBias_Normal =
      mat_slopescaledepthbias_normal.GetFloat();
  pConfig->m_DepthBias_Normal = mat_depthbias_normal.GetFloat();
  pConfig->m_SlopeScaleDepthBias_ShadowMap =
  ...
  pConfig->SetFlag(MATSYS_VIDCFG_FLAGS_ENABLE_HDR,
                   HardwareConfig() && HardwareConfig()->GetHDREnabled());

  // Render-to-texture shadows are disabled for dxlevel 70 because of material
  // issues
...
//-----------------------------------------------------------------------------
void CMaterialSystem::WriteConfigurationInfoToConVars(
  ...
  for (KeyValues *pKey = pKeyValues->GetFirstSubKey(); pKey;
    ...
    if (bOverwriteCommandLineValues ||
        !WasConVarSpecifiedOnCommandLine(pConfigName)) {
...
// This is called when the config changes
//-----------------------------------------------------------------------------
void CMaterialSystem::WriteConfigIntoConVars(
    const MaterialSystem_Config_t &config) {
  if (!g_pCVar) return;
  ...
  mat_slopescaledepthbias_shadowmap.SetValue(
      config.m_SlopeScaleDepthBias_ShadowMap);
...
//-----------------------------------------------------------------------------
bool CMaterialSystem::OverrideConfig(const MaterialSystem_Config_t &_config,
                                     bool forceUpdate) {
  Assert(m_bGeneratedConfig);
  ...
  if (config.dxSupportLevel != g_config.dxSupportLevel) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: Setting dxSupportLevelChanged, bResetAnisotropy, "
          "and bReloadMaterials because new dxlevel = %d and old dxlevel = "
          "%d\n",
  ...
  if (config.HDREnabled() != g_config.HDREnabled()) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: Setting forceUpdate, bReloadMaterials, and "
          "bForceAltTab because new hdr level = %d and old hdr level = %d\n",
  ...
  if (config.ShadowDepthTexture() != g_config.ShadowDepthTexture()) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: Setting forceUpdate, bReloadMaterials and "
          "recomputeSnapshots (ShadowDepthTexture changed: %d -> %d)\n",
          g_config.ShadowDepthTexture() ? 1 : 0,
          config.ShadowDepthTexture() ? 1 : 0);
    }
  ...
  if (forceUpdate) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: forceUpdate is true, therefore setting "
          "recomputeSnapshots, bRedownloadLightmaps, bRedownloadTextures, "
          "bResetAnisotropy, and bSetStandardVertexShaderConstants\n");
    }
  ...
  if (config.UseSpecular() != g_config.UseSpecular()) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new usespecular=%d, old usespecular=%d, setting "
          "recomputeSnapshots, bReloadMaterials, and bResetAnisotropy\n",
  ...
  if (config.UseParallaxMapping() != g_config.UseParallaxMapping()) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new UseParallaxMapping=%d, old "
          "UseParallaxMapping=%d, setting bReloadMaterials\n",
  ...
  if (config.ReduceFillrate() != g_config.ReduceFillrate()) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new ReduceFillrate=%d, old ReduceFillrate=%d, "
          "setting bReloadMaterials\n",
  ...
  if (config.bNoTransparency != g_config.bNoTransparency) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new bNoTransparency=%d, old bNoTransparency=%d, "
          "setting recomputeSnapshots and bResetAnisotropy\n",
  ...
  if (config.bFilterLightmaps != g_config.bFilterLightmaps) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new bFilterLightmaps=%d, old bFilterLightmaps=%d, "
          "setting EnableLightmapFiltering\n",
  ...
  if (config.bSoftwareLighting != g_config.bSoftwareLighting) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new bSoftwareLighting=%d, old "
          "bSoftwareLighting=%d, setting bReloadMaterials\n",
          (int)config.bFilterLightmaps, (int)g_config.bFilterLightmaps);
    }
    bReloadMaterials = true;
  }
...
#ifndef _X360
  if (config_internal.r_waterforceexpensive !=
      g_config_internal.r_waterforceexpensive) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new r_waterforceexpensive=%d, old "
          "r_waterforceexpensive=%d, setting bReloadMaterials\n",
          (int)config_internal.r_waterforceexpensive,
          (int)g_config_internal.r_waterforceexpensive);
    }
    bReloadMaterials = true;
  }
#endif
  ...
  if (config.bAllowCheats != g_config.bAllowCheats) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new bAllowCheats=%d, old bAllowCheats=%d, setting "
  ...
  if (config.bAllowCheats != g_config.bAllowCheats ||
      ...
      ((config.bCompressedTextures != g_config.bCompressedTextures) &&
       HardwareConfig()->SupportsCompressedTextures()) ||
      config.bShowLowResImage != g_config.bShowLowResImage) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: setting bRedownloadTextures, recomputeSnapshots, "
          "and bResetAnisotropy\n");
    }
    bRedownloadTextures = true;
    recomputeSnapshots = true;
    bResetAnisotropy = true;
  ...
  if (config.ForceTrilinear() != g_config.ForceTrilinear()) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new forcetrilinear: %d, old forcetrilinear: %d, "
          "setting bResetTextureFilter\n",
  ...
  if (config.m_nForceAnisotropicLevel != g_config.m_nForceAnisotropicLevel) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new m_nForceAnisotropicLevel: %d, old "
          "m_nForceAnisotropicLevel: %d, setting bResetAnisotropy and "
          "bResetTextureFilter\n",
          (int)config.ForceTrilinear(), (int)g_config.ForceTrilinear());
    }
    bResetAnisotropy = true;
  ...
  if (config.m_fMonitorGamma != g_config.m_fMonitorGamma ||
    ...
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: new monitorgamma: %f, old monitorgamma: %f, "
          "setting bMonitorGammaChanged\n",
  ...
  if (config.m_VideoMode.m_Width != g_config.m_VideoMode.m_Width ||
    ...
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: video mode changed for one of various reasons\n");
    }
    bVideoModeChange = true;
  }

  // toggle wait for vsync
  if ((IsX360() || !config.Windowed()) &&
      (config.WaitForVSync() != g_config.WaitForVSync())) {
#if (!defined(_X360))
    {
      if (mat_debugalttab.GetBool()) {
        Warning(
            "mat_debugalttab: video mode changed due to toggle of wait for "
            "vsync\n");
      }
      bVideoModeChange = true;
    }
  ...
  if (dxSupportLevelChanged) {
    if (mat_debugalttab.GetBool()) {
      Warning(
          "mat_debugalttab: dx support level changed, clearing snapshots\n");
    }
  ...
  // 360 does not support various configuration changes and cannot reload
  // materials
  ...
  // 360 does not support various configuration changes and cannot reload
  // textures 360 has no reason to reload textures, it's unnecessary and
  // massively expensive 360 does not use this path as an init affect to get its
...
ITexture *CMaterialSystem::CreateProceduralTexture(
  ...
  ITextureInternal *pTex = TextureManager()->CreateProceduralTexture(
      pTextureName, pTextureGroupName, w, h, 1, fmt, nFlags);
  return pTex;
}
...
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::CreateMaterial(const char *pMaterialName,
                                           KeyValues *pVMTKeyValues) {
  // For not, just create a material with no default settings
  IMaterialInternal *pMaterial = IMaterialInternal::CreateMaterial(
      pMaterialName, TEXTURE_GROUP_OTHER, pVMTKeyValues);
...
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::FindProceduralMaterial(
...
// Search by name
//-----------------------------------------------------------------------------
IMaterial *CMaterialSystem::FindMaterial(char const *pMaterialName,
                                         const char *pTextureGroupName,
                                         bool bComplain,
                                         const char *pComplainPrefix) {
  // Hammer incorrectly puts a slash in front of the material name if it's in
  // the materials\ folder. If we don't get rid of it, we'll get a filename like
  // materials\\blah.vmt and it can cause problems in other systems.
  ...
  IMaterialInternal *pExistingMaterial = m_MaterialDict.FindMaterial(
      pTemp,
      false);  // 'false' causes the search to find only file-created materials
  ...
  if (!LoadVMTFile(*pKeyValues, *pPatchKeyValues, vmtName, true, &includes)) {
    ...
    if (!Q_stricmp(pKeyValues->GetName(), "subrect")) {
      ...
      pMat =
          m_MaterialDict.AddMaterial(matNameWithExtension, pTextureGroupName);
      if (g_pShaderDevice->IsUsingGraphics()) {
        if (!bIsUNC) {
...
static char const *TextureAliases[] = {
    // this table is only here for backwards compatibility where a render target
    // change was made, and we wish to redirect an existing old client.dll for
    // hl2 to reference this texture. It's not meant as a general texture
    // aliasing system.
    "_rt_FullFrameFB1", "_rt_FullScreen"};

ITexture *CMaterialSystem::FindTexture(char const *pTextureName,
                                       const char *pTextureGroupName,
                                       bool bComplain) {
...
void CMaterialSystem::AddTextureAlias(const char *pAlias,
...
void CMaterialSystem::UpdateExcludedTextures(void) {
  ...
  for (MaterialHandle_t i = FirstMaterial(); i != InvalidMaterial();
       i = NextMaterial(i)) {
...
void CMaterialSystem::UncacheUnusedMaterials(bool bRecomputeStateSnapshots) {
  ...
  // We need two loops to make sure we don't reset the snapshots if nothing got
  // removed, otherwise the snapshot recomputation is expensive and avoided at
  // load time
  bool bDidUncacheMaterial = false;
  for (MaterialHandle_t i = FirstMaterial(); i != InvalidMaterial();
       i = NextMaterial(i)) {
...
void CMaterialSystem::ReloadMaterials(const char *pSubString) {
  ...
  if (pSubString == NULL) {
    ...
    for (MaterialHandle_t i = FirstMaterial(); i != InvalidMaterial();
         i = NextMaterial(i)) {
      if (GetMaterialInternal(i)->GetReferenceCount() <= 0) continue;
      ...
      if (arrSearchItems.Count() > 1) {
        ...
        for (int k = 0; !bMatched && (k < arrSearchItems.Count()); ++k)
          if (Q_stristr(szMatName, arrSearchItems[k])) bMatched = true;

        if (!bMatched) continue;
      } else {
        if (!Q_stristr(szMatName, pSubString)) continue;
      }
      ...
      if (!GetMaterialInternal(i)->IsPrecached()) {
        ...
        VertexFormat_t oldVertexFormat =
            GetMaterialInternal(i)->GetVertexFormat();
        GetMaterialInternal(i)->Uncache();
        GetMaterialInternal(i)->Precache();
...
void CMaterialSystem::AllocateStandardTextures() {
  if (m_StandardTexturesAllocated) return;
  ...
  m_FullbrightLightmapTextureHandle = g_pShaderAPI->CreateTexture(
      1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
      "[FULLBRIGHT_LIGHTMAP_TEXID]", TEXTURE_GROUP_LIGHTMAP);
  ...
  float tmpVect[3] = {nominal_lightmap_value, nominal_lightmap_value,
                      nominal_lightmap_value};
  ColorSpace::LinearToLightmap(texel, tmpVect);
  g_pShaderAPI->TexImage2D(0, 0, IMAGE_FORMAT_BGRX8888, 0, 1, 1,
                           IMAGE_FORMAT_BGRX8888, false, texel);

  // allocate a black single texel texture
#if !defined(_X360)
  m_BlackTextureHandle =
      g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
                                  "[BLACK_TEXID]", TEXTURE_GROUP_OTHER);
...
#else
  m_BlackTextureHandle =
      ((ITextureInternal *)FindTexture("black", TEXTURE_GROUP_OTHER, true))
          ->GetTextureHandle(0);
#endif
  ...
  m_WhiteTextureHandle =
      g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
                                  "[WHITE_TEXID]", TEXTURE_GROUP_OTHER);
...
#else
  m_WhiteTextureHandle =
      ((ITextureInternal *)FindTexture("white", TEXTURE_GROUP_OTHER, true))
          ->GetTextureHandle(0);
#endif
  g_pShaderAPI->SetStandardTextureHandle(TEXTURE_WHITE, m_WhiteTextureHandle);

  // allocate a grey single texel texture with an alpha of zero (for
  // mat_fullbright 2)
#if !defined(_X360)
  m_GreyTextureHandle =
      g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
                                  "[GREY_TEXID]", TEXTURE_GROUP_OTHER);
  g_pShaderAPI->ModifyTexture(m_GreyTextureHandle);
  g_pShaderAPI->TexMinFilter(SHADER_TEXFILTERMODE_LINEAR);
  g_pShaderAPI->TexMagFilter(SHADER_TEXFILTERMODE_LINEAR);
  texel[0] = texel[1] = texel[2] = 128;
  texel[3] =
      255;  // needs to be 255 so that mat_fullbright 2 stuff isn't translucent.
  g_pShaderAPI->TexImage2D(0, 0, IMAGE_FORMAT_BGRX8888, 0, 1, 1,
                           IMAGE_FORMAT_BGRX8888, false, texel);
#else
  m_GreyTextureHandle =
      ((ITextureInternal *)FindTexture("grey", TEXTURE_GROUP_OTHER, true))
          ->GetTextureHandle(0);
#endif
  ...
  m_GreyAlphaZeroTextureHandle =
      g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_RGBA8888, 1, 1, tcFlags,
                                  "[GREY_TEXID]", TEXTURE_GROUP_OTHER);
  ...
  texel[0] = texel[1] = texel[2] = 128;
  texel[3] =
      0;  // needs to be 0 so that self-illum doens't affect mat_fullbright 2
  g_pShaderAPI->TexImage2D(0, 0, IMAGE_FORMAT_RGBA8888, 0, 1, 1,
                           IMAGE_FORMAT_RGBA8888, false, texel);
  texel[3] = 255;  // set back to default value so we don't affect the rest of
                   // this code.'
#else
  m_GreyAlphaZeroTextureHandle =
      ((ITextureInternal *)FindTexture("greyalphazero", TEXTURE_GROUP_OTHER,
                                       true))
          ->GetTextureHandle(0);
#endif
  g_pShaderAPI->SetStandardTextureHandle(TEXTURE_GREY_ALPHA_ZERO,
                                         m_GreyAlphaZeroTextureHandle);

  // allocate a single texel flat normal texture lightmap
  m_FlatNormalTextureHandle =
      g_pShaderAPI->CreateTexture(1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
                                  "[FLAT_NORMAL_TEXTURE]", TEXTURE_GROUP_OTHER);
  ...
  g_pShaderAPI->TexImage2D(0, 0, IMAGE_FORMAT_BGRX8888, 0, 1, 1,
                           IMAGE_FORMAT_BGRX8888, false, texel);
  g_pShaderAPI->SetStandardTextureHandle(TEXTURE_NORMALMAP_FLAT,
                                         m_FlatNormalTextureHandle);

  // allocate a single texel fullbright 1 lightmap for use with bump textures
  m_FullbrightBumpedLightmapTextureHandle = g_pShaderAPI->CreateTexture(
      1, 1, 1, IMAGE_FORMAT_BGRX8888, 1, 1, tcFlags,
      "[FULLBRIGHT_BUMPED_LIGHTMAP_TEXID]", TEXTURE_GROUP_LIGHTMAP);
  g_pShaderAPI->ModifyTexture(m_FullbrightBumpedLightmapTextureHandle);
  g_pShaderAPI->TexMinFilter(SHADER_TEXFILTERMODE_LINEAR);
  g_pShaderAPI->TexMagFilter(SHADER_TEXFILTERMODE_LINEAR);
  float linearColor[3] = {nominal_lightmap_value, nominal_lightmap_value,
                          nominal_lightmap_value};
  unsigned char dummy[3];
  ColorSpace::LinearToBumpedLightmap(linearColor, linearColor, linearColor,
                                     linearColor, dummy, texel, dummy, dummy);
  g_pShaderAPI->TexImage2D(0, 0, IMAGE_FORMAT_BGRX8888, 0, 1, 1,
                           IMAGE_FORMAT_BGRX8888, false, texel);
...

  {
    ...
    // generate the linear->gamma conversion table texture.
    {
      ...
      m_LinearToGammaTableTextureHandle = g_pShaderAPI->CreateTexture(
          LINEAR_TO_GAMMA_TABLE_WIDTH, 1, 1, gammalookupfmt, 1, 1,
          iGammaLookupFlags, "[LINEAR_TO_GAMMA_LOOKUP_SRGBON_TEXID]",
      ...
      float pixelData[LINEAR_TO_GAMMA_TABLE_WIDTH];  // sometimes used as float,
                                                     // sometimes as uint8_t,
                                                     // sizeof(float) >
                                                     // sizeof(uint8_t)
      for (int i = 0; i != LINEAR_TO_GAMMA_TABLE_WIDTH; ++i) {
        float fLookupResult =
            ((float)i) / ((float)(LINEAR_TO_GAMMA_TABLE_WIDTH - 1));
        fLookupResult =
            g_pShaderAPI->LinearToGamma_HardwareSpecific(fLookupResult);

        // do an extra srgb conversion because we'll be converting back on
        // texture read
        fLookupResult = g_pShaderAPI->LinearToGamma_HardwareSpecific(
            fLookupResult);  // that's right, linear->gamma->gamma2x so that
                             // that gamma->linear srgb read still ends up in
                             // gamma

        int iColor = RoundFloatToInt(fLookupResult * 255.0f);
        if (iColor > 255) iColor = 255;

        ((uint8_t *)pixelData)[i] = (uint8_t)iColor;
      }

      g_pShaderAPI->TexImage2D(0, 0, gammalookupfmt, 0,
                               LINEAR_TO_GAMMA_TABLE_WIDTH, 1, gammalookupfmt,
                               false, (void *)pixelData);
    }

    // generate the identity conversion table texture.
    {
      const int LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH = 256;
      m_LinearToGammaIdentityTableTextureHandle = g_pShaderAPI->CreateTexture(
          LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH, 1, 1, gammalookupfmt, 1, 1,
          tcFlags, "[LINEAR_TO_GAMMA_LOOKUP_SRGBOFF_TEXID]",
          TEXTURE_GROUP_PIXEL_SHADERS);
      ...
      float pixelData[LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH];  // sometimes used
                                                              // as float,
                                                              // sometimes as
                                                              // uint8_t,
                                                              // sizeof(float) >
      ...
      for (int i = 0; i != LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH; ++i) {
        float fLookupResult =
            ((float)i) / ((float)(LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH - 1));
      ...
      g_pShaderAPI->TexImage2D(0, 0, gammalookupfmt, 0,
                               LINEAR_TO_GAMMA_IDENTITY_TABLE_WIDTH, 1,
                               gammalookupfmt, false, (void *)pixelData);
    }
  }

  // create the maximum depth texture
  {
    m_MaxDepthTextureHandle = g_pShaderAPI->CreateTexture(
        1, 1, 1, IMAGE_FORMAT_RGBA8888, 1, 1, tcFlags, "[MAXDEPTH_TEXID]",
        TEXTURE_GROUP_OTHER);
    g_pShaderAPI->ModifyTexture(m_MaxDepthTextureHandle);
    g_pShaderAPI->TexMinFilter(SHADER_TEXFILTERMODE_LINEAR);
    g_pShaderAPI->TexMagFilter(SHADER_TEXFILTERMODE_LINEAR);

    // 360 gets depth out of the red channel (which doubles as depth in D24S8)
    // and may be 0/1 depending on REVERSE_DEPTH_ON_X360  PC gets depth out of
    // the alpha channel
  ...
  // only the shaderapi can handle switching between textures correctly, so pass
  // off the textures to it.
  g_pShaderAPI->SetLinearToGammaConversionTextures(
      m_LinearToGammaTableTextureHandle,
...
void CMaterialSystem::ReleaseStandardTextures() {
  if (m_StandardTexturesAllocated) {
    ...
    g_pShaderAPI->SetLinearToGammaConversionTextures(
        INVALID_SHADERAPI_TEXTURE_HANDLE, INVALID_SHADERAPI_TEXTURE_HANDLE);
...
void CMaterialSystem::BeginFrame(float frameTime) {
  ...
  // check debug vars. we will use these to setup g_nDebugVarsSignature so that
  // materials will rebuild their draw lists when debug modes changed.
  g_nDebugVarsSignature =
      ((mat_specular.GetInt() != 0) + (mat_normalmaps.GetInt() << 1) +
       (mat_fullbright.GetInt() << 2) + (mat_fastnobump.GetInt() << 4))
      << 24;
...
bool CMaterialSystem::IsInFrame() const { return m_bInFrame; }

ConVar mat_queue_mode("mat_queue_mode", "-1", 0,
                      "The queue/thread mode the material system should use: "
                      "-1=default, 0=synchronous single thread, 1=queued "
                      "single thread, 2=queued multithreaded");

void CMaterialSystem::ThreadExecuteQueuedContext(
    CMatQueuedRenderContext *pContext) {
...
void CMaterialSystem::EndFrame(void) {
  ...
  MaterialThreadMode_t nextThreadMode =
      (iConVarThreadMode >= 0) ? (MaterialThreadMode_t)iConVarThreadMode
                               : m_IdealThreadMode;
  ...
  switch (m_ThreadMode) {
    ...
    case MATERIAL_QUEUED_THREADED: {
      ...
      CMatQueuedRenderContext *pPrevContext =
      ...
      m_QueuedRenderContexts[m_iCurQueuedContext]
          .GetCallQueueInternal()
          ->QueueCall(g_pShaderAPI, &IShaderAPI::AcquireThreadOwnership);
      m_pRenderContext.Set(&m_QueuedRenderContexts[m_iCurQueuedContext]);

      m_pActiveAsyncJob = new CFunctorJob(CreateFunctor(
          this, &CMaterialSystem::ThreadExecuteQueuedContext, pPrevContext));
  ...
  if (m_ThreadMode != nextThreadMode) {
    ...
    g_pShaderAPI->EnableShaderShaderMutex(
        m_ThreadMode != MATERIAL_SINGLE_THREADED);  // use mutex even for queued
                                                    // to allow "disalow access"
    ...
    switch (m_ThreadMode) {
      ...
      case MATERIAL_QUEUED_THREADED:
        ...
        m_iCurQueuedContext = 0;
        m_QueuedRenderContexts[m_iCurQueuedContext].BeginQueue(
...
bool CMaterialSystem::IsInStubMode() { return m_bInStubMode; }
...
int __cdecl MaterialNameCompareFunc(const void *elem1, const void *elem2) {
  IMaterialInternal *pMaterialA =
      g_MaterialSystem.GetMaterialInternal(*(MaterialHandle_t *)elem1);
  IMaterialInternal *pMaterialB =
...
void CMaterialSystem::DebugPrintUsedMaterials(const char *pSearchSubString,
  ...
  qsort(pSorted, nSortedMaterials, sizeof(MaterialHandle_t),
        MaterialNameCompareFunc);
              ...
              switch (var->GetType()) {
                case MATERIAL_VAR_TYPE_TEXTURE: {
                  ITextureInternal *texture =
                      static_cast<ITextureInternal *>(var->GetTextureValue());
                  if (!texture) {
                    DevWarning(
                        "Programming error: "
                        "CMaterialSystem::DebugPrintUsedMaterialsCallback: "
                        "NULL texture\n");
                    continue;
                  }
                  ...
                  if (IsTextureInternalEnvCubemap(texture)) {
                    ...
                    DevMsg("        %dx%d refCount: %d numframes: %d\n",
                           texture->GetActualWidth(),
                           texture->GetActualHeight(),
  ...
  if (nNumErrors) {
    for (i = 0; i < nSortedMaterials; i++) {
      ...
      if (nRefCount < 0) {
        ...
        DevWarning(
            "DebugPrintUsedMaterials: refCount (%d) < 0 for material: \"%s\"\n",
        ...
        if (pMaterial->IsPrecached() || pMaterial->IsPrecachedVars()) {
          DevWarning(
              "DebugPrintUsedMaterials: material: \"%s\" didn't unache\n",
              pMaterial->GetName());
        }
      }
    }
...
void CMaterialSystem::ToggleSuppressMaterial(char const *pMaterialName) {
  ...
  */
  ...
  // TEXTURE_GROUP_OTHER, it'll go in TEXTURE_GROUP_SHARED.
  IMaterial *pMaterial =
      FindMaterial(pMaterialName, TEXTURE_GROUP_OTHER, true, NULL);
  if (!IsErrorMaterial(pMaterial)) {
    IMaterialInternal *pMatInt = static_cast<IMaterialInternal *>(pMaterial);
    pMatInt = pMatInt->GetRealTimeVersion();  // always work with the realtime
                                              // material internally
...
void CMaterialSystem::ToggleDebugMaterial(char const *pMaterialName) {
  ...
  // TEXTURE_GROUP_OTHER, it'll go in TEXTURE_GROUP_SHARED.
  IMaterial *pMaterial =
      FindMaterial(pMaterialName, TEXTURE_GROUP_OTHER, false, NULL);
  if (!IsErrorMaterial(pMaterial)) {
    IMaterialInternal *pMatInt = static_cast<IMaterialInternal *>(pMaterial);
    pMatInt = pMatInt->GetRealTimeVersion();  // always work with the realtime
                                              // material internally
...
int CMaterialSystem::GetShaders(int nFirstShader, int nMaxCount,
                                IShader **ppShaderList) const {
...
void CMaterialSystem::GetShaderFallback(const char *pShaderName,
  ...
  do {
    int i;
    for (i = 0; i < nCount; ++i) {
      if (!Q_stricmp(pShaderName, ppShaderList[i]->GetName())) break;
    }
...
// Get the current config for this video card (as last set by control panel or
// the default if not)
const MaterialSystem_Config_t &CMaterialSystem::GetCurrentConfigForVideoCard()
    const {
  Assert(m_bGeneratedConfig);
  return g_config;
}
...
void CMaterialSystem::ReloadFilesInList(IFileList *pFilesToReload) {
  ...
  // We have to flush the materials in 2 steps because they have recursive
  // dependencies. The problem case is if you have two materials, A and B, that
  // depend on C. You tell A to reload and it also reloads C. Then the
  // filesystem thinks C doesn't need to be reloaded anymore. So when you get to
  // B, it decides not to reload either since C doesn't need to be reloaded. To
  // fix this, we ask all materials if they want to reload in one stage, then in
  // the next stage we actually reload the appropriate ones.
  MaterialHandle_t hNext;
  for (MaterialHandle_t h = m_MaterialDict.FirstMaterial();
       h != m_MaterialDict.InvalidMaterial(); h = hNext) {
...
void CMaterialSystem::SetShadowDepthBiasFactors(
    float fShadowSlopeScaleDepthBias, float fShadowDepthBias) {
  g_pShaderAPI->SetShadowDepthBiasFactors(fShadowSlopeScaleDepthBias,
                                          fShadowDepthBias);
}
...
//-----------------------------------------------------------------------------
// Gets configuration information associated with the display card, and
// optionally for a particular DX level. It will return a list of ConVars and
// values to set.
//-----------------------------------------------------------------------------
bool CMaterialSystem::GetRecommendedConfigurationInfo(int nDXLevel,
                                                      KeyValues *pKeyValues) {
  MaterialLock_t hLock = Lock();
  bool bResult = g_pShaderDeviceMgr->GetRecommendedConfigurationInfo(
      m_nAdapter, nDXLevel, pKeyValues);
  Unlock(hLock);
  return bResult;
}

//-----------------------------------------------------------------------------
// For dealing with device lost in cases where SwapBuffers isn't called all the
// time (Hammer)
//-----------------------------------------------------------------------------
...
ITexture *CMaterialSystem::CreateRenderTargetTexture(
  ...
  return CreateNamedRenderTargetTextureEx(
      NULL, w, h, sizeMode, format, depth,
...
ITexture *CMaterialSystem::CreateNamedRenderTargetTexture(
    ...
    RenderTargetSizeMode_t sizeMode,  // Controls how size is generated (and
                                      // regenerated on video mode change).
    ImageFormat format, MaterialRenderTargetDepth_t depth, bool bClampTexCoords,
    bool bAutoMipMap) {
  ...
  return CreateNamedRenderTargetTextureEx(
      pRTName, w, h, sizeMode, format, depth, textureFlags, renderTargetFlags);
}
...
ITexture *CMaterialSystem::CreateNamedRenderTargetTextureEx(
  ...
  ITextureInternal *pTex = TextureManager()->CreateRenderTargetTexture(
      pRTName, w, h, sizeMode, format, rtType, textureFlags, renderTargetFlags);
  ...
  // If we're not in a BeginRenderTargetAllocation-EndRenderTargetAllocation
  // block because we're being called by a legacy path (i.e. a mod), force an
  // Alt-Tab after every RT allocation to ensure that all RTs get priority
  // during allocation
...
//-----------------------------------------------------------------------------------------------------
// New version which must be called inside
// BeginRenderTargetAllocation-EndRenderTargetAllocation block
//-----------------------------------------------------------------------------------------------------
ITexture *CMaterialSystem::CreateNamedRenderTargetTextureEx2(
    ...
    ImageFormat format, MaterialRenderTargetDepth_t depth,
    unsigned int textureFlags, unsigned int renderTargetFlags) {
  // Only proceed if we are between BeginRenderTargetAllocation and
  // EndRenderTargetAllocation
  if (!m_bAllocatingRenderTargets) {
    Warning(
        "Tried to create render target outside of "
        "CMaterialSystem::BeginRenderTargetAllocation/"
        "EndRenderTargetAllocation block\n");
    return NULL;
  }

  ITexture *pTexture = CreateNamedRenderTargetTextureEx(
      pRTName, w, h, sizeMode, format, depth, textureFlags, renderTargetFlags);

  pTexture->DecrementReferenceCount();  // Follow the same convention as
                                        // CTextureManager::LoadTexture (return
                                        // refcount of 0).
  return pTexture;
}
...
void CMaterialSystem::Unlock(MaterialLock_t hMaterialLock) {
  IMatRenderContextInternal *pRenderContext =
      (IMatRenderContextInternal *)hMaterialLock;
...
void CMaterialSystem::ReloadMaterials(const CCommand &args) {
  if (args.ArgC() != 2) {
    ConWarning(
        "Usage: mat_reloadmaterial material_name_substring\n"
...
void CMaterialSystem::ReloadTextures(const CCommand &args) { ReloadTextures(); }
