...
// assume 0..4 range
void LinearToBumpedLightmap(const float *linearColor,
                            const float *linearBumpColor1,
                            const float *linearBumpColor2,
                            const float *linearBumpColor3, unsigned char *ret,
                            unsigned char *retBump1, unsigned char *retBump2,
...
SOURCE_FORCEINLINE void LinearToBumpedLightmap(
  ...
  // find a scale factor which makes the average of the 3 bumped mapped vectors
  // match the straight up vector (if possible), so that flat bumpmapped areas
  // match non-bumpmapped areas. Note: According to Alex, this code is
  // completely wrong. Because the bump vectors constitute a orthonormal basis,
  // one does not simply average them to get the straight-up result. In fact
  // they are added together then multiplied by 0.575
  Vector bumpAverage = linearBump1;
  bumpAverage += linearBump2;
  bumpAverage += linearBump3;
  bumpAverage *= (1.0f / 3.0f);
...
{
  // preload 3.0f onto the returns so that we don't need to multiply the
  // bumpAverage by it straight away (eg, reschedule this dependent op)
  ...
  // find a scale factor which makes the average of the 3 bumped mapped vectors
  // match the straight up vector (if possible), so that flat bumpmapped areas
  // match non-bumpmapped areas.
  fltx4 bumpAverage = AddSIMD(AddSIMD(linearBumpColor1, linearBumpColor2),
                              linearBumpColor3);  // actually average * 3

  // find the zero terms so that we can quash their channels in the output
  fltx4 zeroTerms = CmpEqSIMD(bumpAverage, Four_Zeros);
  fltx4 correctionScale = ReciprocalEstSIMD(
      bumpAverage);  // each channel is now 1.0f / (average[x] * 3.0f)

  // divide unbumped linear by the average to get the correction scale
  correctionScale =
      MulSIMD(AndNotSIMD(zeroTerms,
                         linearColor),  // crush values that were zero in
                                        // bumpAverage. (saves on dep latency)
