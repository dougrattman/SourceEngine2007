...
class CMatRenderContextBase : public CRefCounted1<IMatRenderContextInternal> {
 public:
  ...
  virtual void SetCurrentMaterialInternal(IMaterialInternal *pCurrentMaterial) {
    ...
    Assert((m_pCurrentMaterial == NULL) ||
           ((IMaterialInternal *)m_pCurrentMaterial)->IsRealTimeVersion());
  }
  IMaterial *GetCurrentMaterial() { return GetCurrentMaterialInternal(); }
  virtual void *GetCurrentProxy() { return m_pCurrentProxyData; }
  virtual void SetCurrentProxy(void *pProxyData) {
    m_pCurrentProxyData = pProxyData;
  }
  ...
  void PushRenderTargetAndViewport(ITexture *pTexture, int nViewX, int nViewY,
                                   int nViewW, int nViewH);
  void PushRenderTargetAndViewport(ITexture *pTexture, ITexture *pDepthTexture,
  ...
  const VMatrix &AccessCurrentMatrix() const {
  ...
  virtual void GetWorldSpaceCameraVectors(Vector *pVecForward,
                                          Vector *pVecRight, Vector *pVecUp);
 ...
 protected:
  ...
  struct PlaneStackElement {
    float fValues[4];
    bool bHack_IsHeightClipPlane;  // used to hack in compatibility between the
                                   // user clip planes and the existing height
                                   // clip plane code
    // I'm doing the hack this way to retain modder's flexibility to mess with
    // clip plane ordering so that they can make special hacks on their side if
    // necessary
...
class CMatRenderContext : public CMatRenderContextBase {
 public:
  ...
  IMesh *GetDynamicMesh(bool buffered, IMesh *pVertexOverride = 0,
                        IMesh *pIndexOverride = 0, IMaterial *pAutoBind = 0);
  virtual IMesh *GetDynamicMeshEx(VertexFormat_t vertexFormat,
                                  bool bBuffered = true,
  ...
  IVertexBuffer *CreateStaticVertexBuffer(VertexFormat_t fmt, int nVertexCount,
                                          const char *pBudgetGroup) {
    return g_pShaderDevice->CreateVertexBuffer(SHADER_BUFFER_TYPE_STATIC, fmt,
                                               nVertexCount, pBudgetGroup);
  }

  IIndexBuffer *CreateStaticIndexBuffer(MaterialIndexFormat_t fmt,
                                        int nIndexCount,
                                        const char *pBudgetGroup) {
    return g_pShaderDevice->CreateIndexBuffer(SHADER_BUFFER_TYPE_STATIC, fmt,
                                              nIndexCount, pBudgetGroup);
  }
  ...
  DELEGATE_TO_OBJECT_1V(SetLightingOrigin, const Vector &, g_pShaderAPI);
  ...
  DELEGATE_TO_OBJECT_3V(ClearColor3ub, unsigned char, unsigned char,
                        unsigned char, g_pShaderAPI);
  DELEGATE_TO_OBJECT_4V(ClearColor4ub, unsigned char, unsigned char,
                        unsigned char, unsigned char, g_pShaderAPI);
  ...
  DELEGATE_TO_OBJECT_6V(ReadPixels, int, int, int, int, unsigned char *,
                        ImageFormat, g_pShaderAPI);
  void ReadPixelsAndStretch(Rect_t *pSrcRect, Rect_t *pDstRect,
                            unsigned char *pBuffer, ImageFormat dstFormat,
                            int nDstStride) {
    g_pShaderAPI->ReadPixels(pSrcRect, pDstRect, pBuffer, dstFormat,
                             nDstStride);
  }
  ...
  DELEGATE_TO_OBJECT_3V(SetFlexWeights, int, int, const MorphWeight_t *,
                        g_pShaderAPI);
  ...
  // projection-space user clip planes Occasionally (for the particle system in
  // hl2, for example), we want to override that behavior and explictly specify
  // a ViewProj transform for user clip planes
  ...
  DELEGATE_TO_OBJECT_1V(DestroyOcclusionQueryObject,
                        OcclusionQueryObjectHandle_t, g_pOcclusionQueryMgr);
  DELEGATE_TO_OBJECT_1V(BeginOcclusionQueryDrawing,
                        OcclusionQueryObjectHandle_t, g_pOcclusionQueryMgr);
  DELEGATE_TO_OBJECT_1V(EndOcclusionQueryDrawing, OcclusionQueryObjectHandle_t,
                        g_pOcclusionQueryMgr);
  DELEGATE_TO_OBJECT_1V(ResetOcclusionQueryObject, OcclusionQueryObjectHandle_t,
                        g_pOcclusionQueryMgr);
  ...
  void DrawScreenSpaceRectangle(
      ...
      float src_texture_x0, float src_texture_y0,  // which texel you want to
                                                   // appear at destx/y
  ...
  // custom clip planes, beware that only the most recently pushed plane will
  // actually be used in some hardware configurations
  void PushCustomClipPlane(const float *fPlane);
  void PopCustomClipPlane(void);
  void ApplyCustomClipPlanes(
      void);  // updates the clip planes based on how many are supported by the
              // hardware using the top of the stack first, at the end of the
              // stack, the height clip plane will be evaluated
  ...
  void DrawClearBufferQuad(unsigned char r, unsigned char g, unsigned char b,
                           unsigned char a, bool bClearColor, bool bClearDepth);

  void UpdateHeightClipUserClipPlane(void);

  // Private routine called by render target stack push and pop routines to
  // commit new top of stack to the device
  ...
  void CopyRenderTargetToTextureEx(ITexture *pTexture, int nRenderTargetID,
                                   Rect_t *pSrcRect, Rect_t *pDstRect = NULL);
  ...
  DELEGATE_TO_OBJECT_2V(SetVectorRenderingParameter, int, const Vector &,
                        g_pShaderAPI);
  ...
  DELEGATE_TO_OBJECT_4V(GetMaxToRender, IMesh *, bool, int *, int *,
                        g_pShaderAPI);
  ...
  DELEGATE_TO_OBJECT_1V(SetStencilFailOperation, StencilOperation_t,
                        g_pShaderAPI);
  DELEGATE_TO_OBJECT_1V(SetStencilZFailOperation, StencilOperation_t,
                        g_pShaderAPI);
  DELEGATE_TO_OBJECT_1V(SetStencilPassOperation, StencilOperation_t,
                        g_pShaderAPI);
  DELEGATE_TO_OBJECT_1V(SetStencilCompareFunction, StencilComparisonFunction_t,
                        g_pShaderAPI);
  DELEGATE_TO_OBJECT_1V(SetStencilReferenceValue, int, g_pShaderAPI);
  DELEGATE_TO_OBJECT_1V(SetStencilTestMask, uint32_t, g_pShaderAPI);
  DELEGATE_TO_OBJECT_1V(SetStencilWriteMask, uint32_t, g_pShaderAPI);
  DELEGATE_TO_OBJECT_5V(ClearStencilBufferRectangle, int, int, int, int, int,
                        g_pShaderAPI);

  // PIX support
  DELEGATE_TO_OBJECT_2V(BeginPIXEvent, unsigned long, const char *,
                        g_pShaderAPI);
  DELEGATE_TO_OBJECT_0V(EndPIXEvent, g_pShaderAPI);
  DELEGATE_TO_OBJECT_2V(SetPIXMarker, unsigned long, const char *,
                        g_pShaderAPI);
  ...
  virtual bool GetMorphAccumulatorTexCoord(Vector2D *pTexCoord, IMorph *pMorph,
                                           int nVertex);

  DELEGATE_TO_OBJECT_1V(PushDeformation, const DeformationBase_t *,
                        g_pShaderAPI);
  ...
  DELEGATE_TO_OBJECT_1(ColorCorrectionHandle_t, AddLookup, const char *,
                       g_pColorCorrectionSystem);
  DELEGATE_TO_OBJECT_1(bool, RemoveLookup, ColorCorrectionHandle_t,
                       g_pColorCorrectionSystem);
  DELEGATE_TO_OBJECT_1V(LockLookup, ColorCorrectionHandle_t,
                        g_pColorCorrectionSystem);
  DELEGATE_TO_OBJECT_2V(LoadLookup, ColorCorrectionHandle_t, const char *,
                        g_pColorCorrectionSystem);
  DELEGATE_TO_OBJECT_1V(UnlockLookup, ColorCorrectionHandle_t,
                        g_pColorCorrectionSystem);
  ...
  DELEGATE_TO_OBJECT_2V(SetResetable, ColorCorrectionHandle_t, bool,
                        g_pColorCorrectionSystem);
 ...
 protected:
  ...
  CUtlVector<PlaneStackElement>
      m_CustomClipPlanes;  // implemented as a vector so we can remove in
                           // special ways

  IMesh *m_pBatchIndices;
  IMesh *m_pBatchMesh;
  IIndexBuffer *m_pCurrentIndexBuffer;
  CTextureReference m_pNonInteractiveTempFullscreenBuffer
...
inline void CMatRenderContext::LoadBoneMatrix(int boneIndex,
...
inline float CMatRenderContextBase::ComputePixelWidthOfSphere(
...
//-----------------------------------------------------------------------------
inline IMesh *CMatRenderContext::CreateStaticMesh(
    VertexFormat_t vertexFormat, const char *pTextureBudgetGroup,
    IMaterial *pMaterial) {
  return g_pShaderDevice->CreateStaticMesh(vertexFormat, pTextureBudgetGroup,
                                           pMaterial);
}
...
inline enum MaterialHeightClipMode_t CMatRenderContextBase::GetHeightClipMode(
    void) {
