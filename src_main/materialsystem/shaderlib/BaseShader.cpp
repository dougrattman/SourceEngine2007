...
static ShaderParamInfo_t s_StandardParams[NUM_SHADER_MATERIAL_VARS] = {
    ...
    {"$basetexture", "Base Texture with lighting built in",
     SHADER_PARAM_TYPE_TEXTURE, "shadertest/BaseTexture", 0},
    {"$frame", "Animation Frame", SHADER_PARAM_TYPE_INTEGER, "0", 0},
    {"$basetexturetransform", "Base Texture Texcoord Transform",
     SHADER_PARAM_TYPE_MATRIX, "center .5 .5 scale 1 1 rotate 0 translate 0 0",
     0},
    {"$flashlighttexture", "flashlight spotlight shape texture",
     SHADER_PARAM_TYPE_TEXTURE, "effects/flashlight001",
     SHADER_PARAM_NOT_EDITABLE},
    {"$flashlighttextureframe", "Animation Frame for $flashlight",
     SHADER_PARAM_TYPE_INTEGER, "0", SHADER_PARAM_NOT_EDITABLE},
...
void CBaseShader::InitShaderInstance(IMaterialVar** ppParams,
...
void CBaseShader::DrawElements(IMaterialVar** ppParams, int nModulationFlags,
  ...
  OnDrawElements(ppParams, pShaderShadow, pShaderAPI, vertexCompression,
                 pContextDataPtr);
...
void CBaseShader::SetInitialShadowState() {
  ...
  if (flags & MATERIAL_VAR_WIREFRAME) {
    s_pShaderShadow->PolyMode(SHADER_POLYMODEFACE_FRONT_AND_BACK,
                              SHADER_POLYMODE_LINE);
  }

  // Set alpha to coverage
  if (flags & MATERIAL_VAR_ALLOWALPHATOCOVERAGE) {
    // Force the bit on and then check against alpha blend and test states in
...
void CBaseShader::Draw(bool bMakeActualDrawCall) {
  if (IsSnapshotting()) {
    ...
    if (g_pConfig->bNoTransparency && ((s_ppParams[FLAGS]->GetIntValue() &
                                        MATERIAL_VAR_NO_DEBUG_OVERRIDE) == 0)) {
...
ShaderAPITextureHandle_t CBaseShader::GetShaderAPITextureBindHandle(
    int nTextureVar, int nFrameVar, int nTextureChannel) {
  ...
  return GetShaderSystem()->GetShaderAPITextureBindHandle(
...
void CBaseShader::BindTexture(Sampler_t sampler1, Sampler_t sampler2,
                              int nTextureVar, int nFrameVar /* = -1 */) {
  Assert(!IsSnapshotting());
  Assert(nTextureVar != -1);
  Assert(s_ppParams);
  ...
  if (pTextureVar) {
    ...
    if (sampler2 == -1) {
      GetShaderSystem()->BindTexture(sampler1, pTextureVar->GetTextureValue(),
                                     nFrame);
    } else {
      GetShaderSystem()->BindTexture(sampler1, sampler2,
                                     pTextureVar->GetTextureValue(), nFrame);
    }
  }
}

void CBaseShader::BindTexture(Sampler_t sampler1, ITexture* pTexture,
                              int nFrame /* = 0 */) {
...
bool CBaseShader::TextureIsTranslucent(int textureVar, bool isBaseTexture) {
  ...
  if (params[textureVar]->GetType() == MATERIAL_VAR_TYPE_TEXTURE) {
    if (!isBaseTexture) {
      ...
      if ((CurrentMaterialVarFlags() &
           (MATERIAL_VAR_SELFILLUM | MATERIAL_VAR_BASEALPHAENVMAPMASK)) == 0) {
...
void CBaseShader::GetColorParameter(IMaterialVar** params,
                                    float* pColorOut) const {
  float flColor2[3];
...
void CBaseShader::ApplyColor2Factor(
    float* pColorOut) const  // (*pColorOut) *= COLOR2
{
...
void CBaseShader::SetNormalBlendingShadowState(int textureVar,
                                               bool isBaseTexture) {
  Assert(IsSnapshotting());
...
void CBaseShader::SetAdditiveBlendingShadowState(int textureVar,
                                                 bool isBaseTexture) {
  Assert(IsSnapshotting());
...
void CBaseShader::SetDefaultBlendingShadowState(int textureVar,
                                                bool isBaseTexture) {
...
void CBaseShader::LoadCameraSpaceSphereMapTransform(
...
void CBaseShader::SetFixedFunctionTextureScale(
    MaterialMatrixMode_t textureTransform, int scaleVar) {
  Assert(!IsSnapshotting());
...
void CBaseShader::SetFixedFunctionTextureTransform(
    MaterialMatrixMode_t textureTransform, int transformVar) {
  Assert(!IsSnapshotting());
...
void CBaseShader::SetFixedFunctionTextureScaledTransform(
...
void CBaseShader::FixedFunctionMultiplyByDetailPass(int baseTextureVar,
  ...
  if (IsSnapshotting()) {
    ...
    // We can never get the perfect alpha (vertex alpha * cc alpha * texture
    // alpha) so we'll just choose to use cc alpha * texture alpha

    int flags = SHADER_DRAW_POSITION | SHADER_DRAW_TEXCOORD1;

    // Compute alpha, stage 0 is used, stage 1 isn't.
    if (translucentTexture) {
      s_pShaderShadow->CustomTextureOperation(
          SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_ALPHA, SHADER_TEXOP_MODULATE,
          SHADER_TEXARG_TEXTURE, SHADER_TEXARG_CONSTANTCOLOR);
      flags |= SHADER_DRAW_TEXCOORD0;
    } else {
      bool hasVertexAlpha =
          (CurrentMaterialVarFlags() & MATERIAL_VAR_VERTEXALPHA) != 0;
      if (hasVertexAlpha) {
        flags |= SHADER_DRAW_COLOR;
      }

      s_pShaderShadow->CustomTextureOperation(
          SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_ALPHA,
          hasVertexAlpha ? SHADER_TEXOP_MODULATE : SHADER_TEXOP_SELECTARG1,
          SHADER_TEXARG_CONSTANTCOLOR, SHADER_TEXARG_VERTEXCOLOR);
    }

    s_pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_ALPHA, SHADER_TEXOP_SELECTARG1,
    ...
    s_pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_COLOR,
        SHADER_TEXOP_BLEND_PREVIOUSSTAGEALPHA, SHADER_TEXARG_TEXTURE,
    ...
    if (TextureIsTranslucent(baseTextureVar, true)) {
      ...
      // Unnecessary... but we get strange colors if we don't put something on
      // stage 0
    ...
    SetFixedFunctionTextureScaledTransform(MATERIAL_TEXTURE1,
                                           textureTransformVar, detailScaleVar);
...
void CBaseShader::FixedFunctionMultiplyByLightmapPass(
  ...
  if (IsSnapshotting()) {
    ...
    if (TextureIsTranslucent(baseTextureVar, true)) {
      ...
      s_pShaderShadow->CustomTextureOperation(
          SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_ALPHA,
          SHADER_TEXOP_SELECTARG2, SHADER_TEXARG_TEXTURE,
          SHADER_TEXARG_CONSTANTCOLOR);
    }
    ...
    //		s_pShaderAPI->Color4f( OO_OVERBRIGHT, OO_OVERBRIGHT,
    // OO_OVERBRIGHT,  alpha );
    s_pShaderAPI->Color4ub(128, 128, 128, (int)(alpha * 255));

    if (TextureIsTranslucent(baseTextureVar, true)) {
      SetFixedFunctionTextureTransform(MATERIAL_TEXTURE0,
...
void CBaseShader::FixedFunctionBaseTimesDetailPass(int baseTextureVar,
  ...
  if (IsSnapshotting()) {
    ...
    s_pShaderShadow->EnableTextureAlpha(
        SHADER_TEXTURE_STAGE0, TextureIsTranslucent(baseTextureVar, true));
    ...
    if (hasDetail && (!detailInSecondPass)) {
      BindTexture(SHADER_SAMPLER1, detailVar, frameVar);
      SetFixedFunctionTextureScaledTransform(
          MATERIAL_TEXTURE1, baseTextureTransformVar, detailScaleVar);
    }
  ...
  if (detailInSecondPass) {
    FixedFunctionMultiplyByDetailPass(baseTextureVar, frameVar,
                                      baseTextureTransformVar, detailVar,
                                      detailScaleVar);
  }
}
...
int CBaseShader::SetShadowEnvMappingState(int envMapMaskVar, int tintVar) {
  ...
  if ((varFlags & MATERIAL_VAR_ENVMAPSPHERE) == 0)
    s_pShaderShadow->TexGen(SHADER_TEXTURE_STAGE0,
  ...
  if (params[envMapMaskVar]->IsDefined() ||
      (varFlags & MATERIAL_VAR_BASEALPHAENVMAPMASK)) {
    s_pShaderShadow->EnableTexture(SHADER_SAMPLER1, true);
    flags |= SHADER_DRAW_TEXCOORD1;
  } else {
  ...
  if (varFlags & MATERIAL_VAR_BASEALPHAENVMAPMASK) {
    ...
    s_pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_COLOR, SHADER_TEXOP_MODULATE,
        SHADER_TEXARG_TEXTURE, SHADER_TEXARG_CONSTANTCOLOR);
    s_pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_COLOR, SHADER_TEXOP_MODULATE,
        SHADER_TEXARG_PREVIOUSSTAGE, SHADER_TEXARG_INVTEXTUREALPHA);

    // Use alpha modulation * vertex alpha * env map alpha
    s_pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE0, SHADER_TEXCHANNEL_ALPHA, SHADER_TEXOP_MODULATE,
        SHADER_TEXARG_VERTEXCOLOR, SHADER_TEXARG_TEXTURE);
    s_pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_ALPHA, SHADER_TEXOP_SELECTARG1,
    ...
    // Alpha = vertex alpha * constant alpha * env map alpha * mask alpha (only
    // if it's not a base alpha mask)
    s_pShaderShadow->EnableConstantAlpha(IsAlphaModulating());
    s_pShaderShadow->EnableVertexAlpha((varFlags & MATERIAL_VAR_VERTEXALPHA) !=
                                       0);
    s_pShaderShadow->EnableTextureAlpha(SHADER_TEXTURE_STAGE0, true);
    s_pShaderShadow->EnableTextureAlpha(SHADER_TEXTURE_STAGE1,
                                        params[envMapMaskVar]->IsTexture());
  }
...
void CBaseShader::SetDynamicEnvMappingState(int envMapVar, int envMapMaskVar,
                                            ...
                                            int frameVar, int maskOffsetVar,
                                            int maskScaleVar, int tintVar) {
  Assert(!IsSnapshotting());
  ...
  if (params[envMapMaskVar]->IsTexture()) {
    SetFixedFunctionTextureScaledTransform(MATERIAL_TEXTURE1, maskOffsetVar,
                                           maskScaleVar);
...
//-----------------------------------------------------------------------------
void CBaseShader::FixedFunctionMaskedEnvmapPass(
    int envMapVar, int envMapMaskVar, int baseTextureVar, int envMapFrameVar,
...
//-----------------------------------------------------------------------------
void CBaseShader::FixedFunctionAdditiveMaskedEnvmapPass(
    int envMapVar, int envMapMaskVar, int baseTextureVar, int envMapFrameVar,
    int envMapMaskFrameVar, int frameVar, int maskOffsetVar, int maskScaleVar,
    int envMapTintVar) {
  //	IMaterialVar** params = ShaderState().m_ppParams;
...
void CBaseShader::DrawFlashlight_dx70(IMaterialVar** params,
                                      IShaderDynamicAPI* pShaderAPI,
                                      IShaderShadow* pShaderShadow,
  ...
  SHADOW_STATE {
    ...
    int flags = SHADER_DRAW_POSITION | SHADER_DRAW_TEXCOORD1 |
                SHADER_DRAW_COLOR | SHADER_DRAW_NORMAL;
    ...
    pShaderShadow->CustomTextureOperation(
        SHADER_TEXTURE_STAGE1, SHADER_TEXCHANNEL_ALPHA, SHADER_TEXOP_MODULATE,
        SHADER_TEXARG_TEXTURE, SHADER_TEXARG_PREVIOUSSTAGE);
  ...
  DYNAMIC_STATE {
    ...
    // NOTE: This has to come after the loadmatrix since the loadmatrix screws
    // with the transform flags!!!!!! Specify that we have XYZ texcoords that
    // need to be divided by W before the pixel shader. NOTE Tried to divide XY
    // by Z, but doesn't work.
...
void CBaseShader::SetFlashlightFixedFunctionTextureTransform(
