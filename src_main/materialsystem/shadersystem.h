...
#ifndef SOURCE_MATERIALSYSTEM_SHADERSYSTEM_H_
#define SOURCE_MATERIALSYSTEM_SHADERSYSTEM_H_
...
inline int SnapshotTypeCount() {
  return MaterialSystem()->CanUseEditorMaterials() ? SNAPSHOT_COUNT_EDITOR
                                                   : SNAPSHOT_COUNT_NORMAL;
}
...
the_interface IShaderSystemInternal : public IShaderInit, public IShaderSystem {
 public:
  ...
  virtual bool InitRenderState(
      IShader * pShader, int numParams, IMaterialVar **params,
      ShaderRenderState_t *pRenderState, char const *pMaterialName) = 0;
  ...
  virtual int GetShaders(int nFirstShader, int nCount, IShader **ppShaderList)
      const = 0;
};

#endif  // SOURCE_MATERIALSYSTEM_SHADERSYSTEM_H_
