      ...
      for (int iFace = 0; iFace < pVTFTexture->FaceCount(); ++iFace) {
        ...
        for (int z = 0; z < nDepth; ++z) {
          ...
          pixelWriter.SetPixelMemory(
              pVTFTexture->Format(),
              pVTFTexture->ImageData(iFrame, iFace, 0, 0, 0, z),
          ...
          for (int y = 0; y < nHeight; ++y) {
            ...
            for (int x = 0; x < nWidth; ++x) {
              if (((x & m_nCheckerSize) ^ (y & m_nCheckerSize)) ^
                  (z & m_nCheckerSize)) {
                pixelWriter.WritePixel(m_Color1.r, m_Color1.g, m_Color1.b,
                                       m_Color1.a);
              } else {
                pixelWriter.WritePixel(m_Color2.r, m_Color2.g, m_Color2.b,
                                       m_Color2.a);
              }
            }
          }
...
static void CreateCheckerboardTexture(ITextureInternal *pTexture,
                                      int nCheckerSize, color32 color1,
                                      color32 color2) {
  ITextureRegenerator *pRegen =
      new CCheckerboardTexture(nCheckerSize, color1, color2);
      ...
      for (int iFace = 0; iFace < pVTFTexture->FaceCount(); ++iFace) {
        for (int iMip = 0; iMip < nMipCount; ++iMip) {
          ...
          for (int z = 0; z < nDepth; ++z) {
            ...
            pixelWriter.SetPixelMemory(
                pVTFTexture->Format(),
                pVTFTexture->ImageData(iFrame, iFace, iMip, 0, 0, z),
                pVTFTexture->RowSizeInBytes(iMip));

            for (int y = 0; y < nHeight; ++y) {
              pixelWriter.Seek(0, y);
              for (int x = 0; x < nWidth; ++x) {
                pixelWriter.WritePixel(m_Color.r, m_Color.g, m_Color.b,
                                       m_Color.a);
              }
            }
          }
  ...
  virtual void RegenerateTextureBits(ITexture *pTexture,
    ...
    for (int iFace = 0; iFace < 6; ++iFace) {
      pixelWriter.SetPixelMemory(pVTFTexture->Format(),
                                 pVTFTexture->ImageData(0, iFace, 0),
                                 pVTFTexture->RowSizeInBytes(0));
...
class CColorCorrectionTexture : public ITextureRegenerator {
 public:
  CColorCorrectionTexture(ColorCorrectionHandle_t handle)
      : m_ColorCorrectionHandle(handle) {}
  ...
  virtual void RegenerateTextureBits(ITexture *pTexture,
    ...
    int nHeight = pVTFTexture->Height();
    int nDepth = pVTFTexture->Depth();
    Assert(nWidth == COLOR_CORRECTION_TEXTURE_SIZE &&
           nHeight == COLOR_CORRECTION_TEXTURE_SIZE &&
           nDepth == COLOR_CORRECTION_TEXTURE_SIZE);
    ...
    for (int z = 0; z < nDepth; ++z) {
      ...
      for (int y = 0; y < nHeight; ++y) {
        ...
        for (int x = 0; x < nWidth; ++x) {
          ...
          color24 col = ColorCorrectionSystem()->GetLookup(
              m_ColorCorrectionHandle, inColor);
...
void CreateColorCorrectionTexture(ITextureInternal *pTexture,
                                  ColorCorrectionHandle_t handle) {
...
class CTextureManager : public ITextureManager {
 public:
  ...
  virtual ITextureInternal *FindOrLoadTexture(const char *textureName,
  ...
  virtual void GenerateErrorTexture(ITexture *pTexture,
                                    IVTFTexture *pVTFTexture);
 ...
 protected:
  ...
  ITextureInternal *LoadTexture(const char *textureName,
                                const char *pTextureGroupName);
...
CTextureManager::CTextureManager(void)
    : m_TextureList(true), m_TextureAliases(true), m_TextureExcludes(true) {
...
void CTextureManager::Init(int nFlags) {
  ...
  color.a = 128;
  color2.r = color2.b = color2.a = 255;
  color2.g = 0;
  m_pErrorRegen = new CCheckerboardTexture(4, color, color2);

  // Create an error texture
  m_pErrorTexture = CreateProceduralTexture(
      "error", TEXTURE_GROUP_OTHER, ERROR_TEXTURE_SIZE, ERROR_TEXTURE_SIZE, 1,
      IMAGE_FORMAT_BGRA8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
  CreateCheckerboardTexture(m_pErrorTexture, 4, color, color2);

  // Create a white texture
  m_pWhiteTexture = CreateProceduralTexture(
      "white", TEXTURE_GROUP_OTHER, WHITE_TEXTURE_SIZE, WHITE_TEXTURE_SIZE, 1,
      IMAGE_FORMAT_BGRX8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
  color.r = color.g = color.b = color.a = 255;
  CreateSolidTexture(m_pWhiteTexture, color);

  // Create a black texture
  m_pBlackTexture = CreateProceduralTexture(
      "black", TEXTURE_GROUP_OTHER, BLACK_TEXTURE_SIZE, BLACK_TEXTURE_SIZE, 1,
      IMAGE_FORMAT_BGRX8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
  color.r = color.g = color.b = 0;
  CreateSolidTexture(m_pBlackTexture, color);

  // Create a grey texture
  m_pGreyTexture = CreateProceduralTexture(
      "grey", TEXTURE_GROUP_OTHER, GREY_TEXTURE_SIZE, GREY_TEXTURE_SIZE, 1,
      IMAGE_FORMAT_BGRA8888, TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_SINGLECOPY);
  ...
  // Create a grey texture
  m_pGreyAlphaZeroTexture = CreateProceduralTexture(
      "greyalphazero", TEXTURE_GROUP_OTHER, GREY_TEXTURE_SIZE,
      GREY_TEXTURE_SIZE, 1, IMAGE_FORMAT_BGRA8888,
  ...
  if (HardwareConfig()->GetMaxDXSupportLevel() >= 80) {
    ...
    m_pNormalizationCubemap = CreateProceduralTexture(
        "normalize", TEXTURE_GROUP_CUBE_MAP, NORMALIZATION_CUBEMAP_SIZE,
        NORMALIZATION_CUBEMAP_SIZE, 1, IMAGE_FORMAT_BGRX8888,
  ...
  if (HardwareConfig()->GetMaxDXSupportLevel() >= 90) {
    // Create a normalization cubemap
    m_pSignedNormalizationCubemap = CreateProceduralTexture(
        "normalizesigned", TEXTURE_GROUP_CUBE_MAP, NORMALIZATION_CUBEMAP_SIZE,
        NORMALIZATION_CUBEMAP_SIZE, 1, IMAGE_FORMAT_UVWQ8888,
        TEXTUREFLAGS_ENVMAP | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD |
            TEXTUREFLAGS_SINGLECOPY | TEXTUREFLAGS_CLAMPS |
            TEXTUREFLAGS_CLAMPT);
    CreateSignedNormalizationCubemap(m_pSignedNormalizationCubemap);
    m_pIdentityLightWarp =
        FindOrLoadTexture("dev/IdentityLightWarp", TEXTURE_GROUP_OTHER);
  ...
  if (HardwareConfig()->ActuallySupportsPixelShaders_2_b()) {
    m_pShadowNoise2D = FindOrLoadTexture("engine/NormalizedRandomDirections2D",
                                         TEXTURE_GROUP_OTHER);
...
void CTextureManager::Shutdown() {
  ...
  // These checks added because it's possible for shutdown to be called before
  // the material system is fully initialized.
  if (m_pWhiteTexture) {
  ...
  for (int i = m_TextureAliases.First(); i != m_TextureAliases.InvalidIndex();
       i = m_TextureAliases.Next(i)) {
...
void CTextureManager::AllocateStandardRenderTargets() {
  bool bAllocateFullscreenTexture =
      (m_nFlags & MATERIAL_INIT_ALLOCATE_FULLSCREEN_TEXTURE) != 0;
  ...
  if (IsPC() && (bAllocateFullscreenTexture || bAllocateMorphAccumTexture)) {
    ...
    if (bAllocateFullscreenTexture) {
      m_pFullScreenTexture = CreateRenderTargetTexture(
          "_rt_FullScreen", 1, 1, RT_SIZE_FULL_FRAME_BUFFER_ROUNDED_UP,
          MaterialSystem()->GetBackBufferFormat(), RENDER_TARGET,
...
void CTextureManager::CacheExternalStandardRenderTargets() {
  m_pFullScreenDepthTexture =
      FindTexture("_rt_FullFrameDepth");  // created/destroyed in
                                          // engine/matsys_interface.cpp to
                                          // properly track hdr changes
}

//-----------------------------------------------------------------------------
// Generates an error texture pattern
//-----------------------------------------------------------------------------
void CTextureManager::GenerateErrorTexture(ITexture *pTexture,
                                           IVTFTexture *pVTFTexture) {
...
void CTextureManager::SetColorCorrectionTexture(int i,
  ...
  if (pTexture) pTexture->IncrementReferenceCount();
}
...
void CTextureManager::RestoreTexture(ITextureInternal *pTexture) {
  ...
  pTexture->OnRestore();  // Give render targets a chance to reinitialize
                          // themselves if necessary (due to AA changes).
  pTexture->Download();
}
...
void CTextureManager::RestoreRenderTargets() {
  ...
  if (m_pFullScreenTexture) {
    g_pShaderAPI->SetFullScreenTextureHandle(
        m_pFullScreenTexture->GetTextureHandle(0));
  }
...
static void ForceTextureIntoHardware(ITexture *pTexture, IMaterial *pMaterial,
                                     IMaterialVar *pBaseTextureVar) {
  if (IsX360()) return;
...
ITextureInternal *CTextureManager::CreateProceduralTexture(
    const char *pTextureName, const char *pTextureGroupName, int w, int h,
    int d, ImageFormat fmt, int nFlags) {
  ITextureInternal *pNewTexture = ITextureInternal::CreateProceduralTexture(
      pTextureName, pTextureGroupName, w, h, d, fmt, nFlags);
...
ITextureInternal *CTextureManager::LoadTexture(const char *pTextureName,
                                               const char *pTextureGroupName) {
  ITextureInternal *pNewTexture =
      ITextureInternal::CreateFileTexture(pTextureName, pTextureGroupName);
  if (pNewTexture) {
...
ITextureInternal *CTextureManager::FindTexture(const char *pTextureName) {
  ...
  if (char const *szLightMapNum = StringAfterPrefix(szCleanName, "[lightmap")) {
    ...
    if (iLightMapNum >= 0 && iLightMapNum < plm->GetNumLightmapPages()) {
      ShaderAPITextureHandle_t hTex =
          plm->GetLightmapPageTextureHandle(iLightMapNum);
      if (hTex != INVALID_SHADERAPI_TEXTURE_HANDLE) {
        // Establish the lookup linking in the dictionary
        ITextureInternal *pTxInt =
            ITextureInternal::CreateReferenceTextureFromHandle(
                pTextureName, TEXTURE_GROUP_LIGHTMAP, hTex);
...
void CTextureManager::AddTextureAlias(const char *pAlias,
  ...
  if (index != m_TextureAliases.InvalidIndex()) {
    AssertMsg(Q_stricmp(pRealName, m_TextureAliases[index]) == 0,
              "Trying to use one name to alias two different textures.");
    RemoveTextureAlias(
        pAlias);  // remove the old alias to make room for the new one.
  }
...
void CTextureManager::SetExcludedTextures(const char *pScriptName) {
  ...
  for (int i = m_TextureExcludes.First(); i != m_TextureExcludes.InvalidIndex();
       i = m_TextureExcludes.Next(i)) {
    ITextureInternal *pTexture =
        FindTexture(m_TextureExcludes.GetElementName(i));
    if (pTexture) {
  ...
  if (g_pFullFileSystem->ReadFile(pScriptName, NULL, excludeBuffer)) {
    ...
    while (1) {
      ...
      if (m_TextureExcludes.Find(szCleanName) !=
          m_TextureExcludes.InvalidIndex()) {
        // avoid duplicates
        continue;
      }
      ...
      // textures that don't exist yet will get caught during their creation
      // path
...
ITextureInternal *CTextureManager::FindOrLoadTexture(
...
ITextureInternal *CTextureManager::CreateRenderTargetTexture(
  ...
  if (pRTName) {
    ...
    if (pTexture) {
      // Changing the underlying render target, but leaving the pointer and
      // refcount alone fixes callers that have exisiting references to this
      // object.
  ...
  pTexture = ITextureInternal::CreateRenderTarget(
      pRTName, w, h, sizeMode, fmt, type, textureFlags, renderTargetFlags);
...
void CTextureManager::DebugPrintUsedTextures(void) {
  for (int i = m_TextureList.First(); i != m_TextureList.InvalidIndex();
    ...
    Msg("Texture: '%s' RefCount: %d\n", pTexture->GetName(),
        pTexture->GetReferenceCount());
  }

  if (m_TextureExcludes.Count()) {
    Msg("\nExcluded Textures: (%d)\n", m_TextureExcludes.Count());
    for (int i = m_TextureExcludes.First();
         i != m_TextureExcludes.InvalidIndex(); i = m_TextureExcludes.Next(i)) {
      char buff[256];
