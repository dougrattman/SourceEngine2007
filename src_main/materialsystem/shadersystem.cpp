...
class CShaderSystem : public IShaderSystemInternal {
 public:
  ...
  virtual ShaderAPITextureHandle_t GetShaderAPITextureBindHandle(
      ITexture *pTexture, int nFrameVar, int nTextureChannel = 0);

  virtual void BindTexture(Sampler_t sampler1, ITexture *pTexture,
                           int nFrame = 0);
  virtual void BindTexture(Sampler_t sampler1, Sampler_t sampler2,
                           ITexture *pTexture, int nFrame = 0);
  ...
  virtual void InitShaderParameters(IShader *pShader, IMaterialVar **params,
                                    const char *pMaterialName);
  virtual void InitShaderInstance(IShader *pShader, IMaterialVar **params,
                                  const char *pMaterialName,
                                  const char *pTextureGroupName);
  virtual bool InitRenderState(IShader *pShader, int numParams,
                               IMaterialVar **params,
                               ShaderRenderState_t *pRenderState,
                               char const *pMaterialName);
  virtual void CleanupRenderState(ShaderRenderState_t *pRenderState);
  virtual void DrawElements(IShader *pShader, IMaterialVar **params,
                            ShaderRenderState_t *pShaderState,
                            VertexCompressionType_t vertexCompression,
                            uint32_t nVarChangeID);
 ...
 private:
  ...
  void InitStateSnapshots(IShader *pShader, IMaterialVar **params,
                          ShaderRenderState_t *pRenderState);

  // Compute snapshots for all combinations of alpha + color modulation
  void InitRenderStateFlags(ShaderRenderState_t *pRenderState, int numParams,
                            IMaterialVar **params);
  ...
  bool ComputeVertexFormatFromSnapshot(IMaterialVar **params,
                                       ShaderRenderState_t *pRenderState);
  ...
  void CopyMaterialVarToDebugShader(IMaterialInternal *pDebugMaterial,
                                    IShader *pShader, IMaterialVar **ppParams,
                                    const char *pSrcVarName,
                                    const char *pDstVarName = NULL);
  ...
  void DrawNormalMap(IShader *pShader, IMaterialVar **ppParams,
                     VertexCompressionType_t vertexCompression);
  bool DrawEnvmapMask(IShader *pShader, IMaterialVar **ppParams,
                      ShaderRenderState_t *pRenderState,
                      VertexCompressionType_t vertexCompression);
 ...
 private:
  ...
  // Debugging materials
  // If you add to this, add to the list of debug shader names
  // (s_pDebugShaderName) below
  enum {
...
void CShaderSystem::LoadAllShaderDLLs() {
  ...
  // Load up standard shader DLLs...
  int dx_support_level = HardwareConfig()->GetMaxDXSupportLevel();
  Assert(dx_support_level >= 60);
  dx_support_level /= 10;
...
void CShaderSystem::UnloadAllShaderDLLs() {
  if (m_ShaderDLLs.Count() == 0) return;
...
bool CShaderSystem::LoadShaderDLL(const char *pFullPath, const char *pPathID,
                                  bool bModShaderDLL) {
  if (!pFullPath || !pFullPath[0]) return true;
    ...
    if (info.m_bModShaderDLL) {
      for (int iTestDLL = 0; iTestDLL < m_ShaderDLLs.Count(); iTestDLL++) {
        ...
        if (!pTestDLL->m_bModShaderDLL) {
          if (pTestDLL->m_ShaderDict.Find(pShaderName) !=
              pTestDLL->m_ShaderDict.InvalidIndex()) {
            Error("Game shader '%s' trying to override a base shader '%s'.",
                  info.m_pFileName, pShaderName);
          }
        }
      }
    }
...
int CShaderSystem::GetShaders(int nFirstShader, int nMaxCount,
                              IShader **ppShaderList) const {
  ...
  for (int i = m_ShaderDLLs.Count(); --i >= 0;) {
    ...
    for (unsigned short j = info.m_ShaderDict.First();
      ...
      if (uniqueNames.Find(pShaderName) != UTL_INVAL_SYMBOL) continue;
      ...
      if (nActualCount > nFirstShader) {
        ...
        ++nCount;
        if (nCount >= nMaxCount) return nCount;
      }
    }
  }
...
inline StateSnapshot_t CShaderSystem::CurrentStateSnapshot() {
  ...
  Assert(m_nRenderPass <
         m_pRenderState->m_pSnapshots[m_nModulation].m_nPassCount);
...
void CShaderSystem::CreateDebugMaterials() {
  ...
  for (i = 0; i < MATERIAL_DEBUG_COUNT; ++i) {
    char shaderName[64];
    Q_snprintf(shaderName, sizeof(shaderName), "___%s_%d.vmt",
               s_pDebugShaderName[i], i);
    m_pDebugMaterials[i] = static_cast<IMaterialInternal *>(
        MaterialSystem()->CreateMaterial(shaderName, pVMTKeyValues[i]));
...
void CShaderSystem::BufferSpew(SpewType_t spewType, const Color &c,
                               const char *pMsg) {
...
void CShaderSystem::PrintBufferedSpew() {
  AUTO_LOCK(g_StgoredSpewMutex);
...
//-----------------------------------------------------------------------------
void CShaderSystem::PrepForShaderDraw(IShader *pShader, IMaterialVar **ppParams,
                                      ShaderRenderState_t *pRenderState,
                                      int nModulation) {
  Assert(!m_pRenderState);
...
//-----------------------------------------------------------------------------
void CShaderSystem::InitShaderParameters(IShader *pShader,
                                         IMaterialVar **params,
                                         const char *pMaterialName) {
...
//-----------------------------------------------------------------------------
void CShaderSystem::InitShaderInstance(IShader *pShader, IMaterialVar **params,
                                       const char *pMaterialName,
                                       const char *pTextureGroupName) {
  // Let the derived class do its thing
  PrepForShaderDraw(pShader, params, 0, 0);
...
// Compute snapshots for all combinations of alpha + color modulation
//-----------------------------------------------------------------------------
void CShaderSystem::InitRenderStateFlags(ShaderRenderState_t *pRenderState,
                                         int numParams, IMaterialVar **params) {
  // Compute vertex format and flags
  pRenderState->m_Flags = 0;

  // Make sure the shader don't force these flags. . they are automatically
  // computed.
  ...
  // If we are in release mode, just go ahead and clear in case the above is
  // screwed up.
  pRenderState->m_Flags &= ~SHADER_OPACITY_MASK;

  /*
          // HACK: Also kind of gross; turn off bump lightmapping for low-end
          if (g_config.bUseGraphics &&
     !HardwareConfig()->SupportsVertexAndPixelShaders())
          {
                  pRenderState->m_Flags &= ~SHADER_NEEDS_BUMPED_LIGHTMAPS;
          }
  */
  /*
          // HACK: more grossness!!!  turn off bump lightmapping if we don't
     have a bumpmap
          // Shaders should specify SHADER_NEEDS_BUMPED_LIGHTMAPS if they might
     need a bumpmap,
          ...
          if( pRenderState->m_Flags & SHADER_NEEDS_BUMPED_LIGHTMAPS )
          {
                  ...
                  for( int i = 0; i < numParams; i++ )
                  {
     ...
     )
                          {
                                  ...
                                  {
                                          const char *blah =
     params[i]->GetStringValue(); pRenderState->m_Flags |=
     SHADER_NEEDS_BUMPED_LIGHTMAPS; break;
                                  }
                          }
                  }
          }
...
// Computes flags from a particular snapshot
//-----------------------------------------------------------------------------
void CShaderSystem::ComputeRenderStateFlagsFromSnapshot(
    ShaderRenderState_t *pRenderState) {
...
void CShaderSystem::InitStateSnapshots(IShader *pShader, IMaterialVar **params,
                                       ShaderRenderState_t *pRenderState) {
#ifdef _DEBUG
  ...
  // If the current mod does not use the flashlight, skip all flashlight
  // snapshots (saves a ton of memory)
  bool bModUsesFlashlight = (mat_supportflashlight.GetInt() != 0);

  for (int i = 0; i < nSnapshotCount; ++i) {
    if ((i & SHADER_USING_FLASHLIGHT) && !bModUsesFlashlight) {
    ...
    if (i & SHADER_USING_FLASHLIGHT) {
      ...
      //			{
      //				pRenderState->m_pSnapshots[i].m_nPassCount
      //= 0; 				continue;
      //			}
    ...
    pShader->DrawElements(params, i, g_pShaderShadow, 0,
                          ...
                          &(pRenderState->m_pSnapshots[i].m_pContextData[0]));
    DoneWithShaderDraw();
  }
...
#ifdef _DEBUG
static bool IsVertexFormatSubsetOfVertexformat(VertexFormat_t subset,
                                               VertexFormat_t superset) {
  ...
  // Test bone weights
  if (NumBoneWeights(subset) > NumBoneWeights(superset)) return false;
  ...
  for (int i = 0; i < VERTEX_MAX_TEXTURE_COORDINATES; i++) {
    if (TexCoordSize(i, subset) > TexCoordSize(i, superset)) return false;
  }
...
//-----------------------------------------------------------------------------
static void AddSnapshotsToList(RenderPassList_t *pPassList, int &nSnapshotID,
...
// Computes vertex format + usage from a particular snapshot
//-----------------------------------------------------------------------------
bool CShaderSystem::ComputeVertexFormatFromSnapshot(
    IMaterialVar **params, ShaderRenderState_t *pRenderState) {
  ...
  if (nModulationSnapshotCount >= SHADER_USING_FLASHLIGHT) {
    numSnapshots +=
        pRenderState->m_pSnapshots[SHADER_USING_FLASHLIGHT].m_nPassCount;
  }
  if (MaterialSystem()->CanUseEditorMaterials()) {
    numSnapshots +=
  ...
  if (nModulationSnapshotCount >= SHADER_USING_FLASHLIGHT) {
    AddSnapshotsToList(&pRenderState->m_pSnapshots[SHADER_USING_FLASHLIGHT],
                       snapshotID, pSnapshots);
  }
  if (MaterialSystem()->CanUseEditorMaterials()) {
    AddSnapshotsToList(&pRenderState->m_pSnapshots[SHADER_USING_EDITOR],
                       snapshotID, pSnapshots);
  }

  Assert(snapshotID == numSnapshots);

  pRenderState->m_VertexUsage =
      g_pShaderAPI->ComputeVertexUsage(numSnapshots, pSnapshots);
  pRenderState->m_MorphFormat =
      g_pShaderAPI->ComputeMorphFormat(numSnapshots, pSnapshots);
  ...
  for (int mod = 1; mod < nModulationSnapshotCount; ++mod) {
    ...
    StateSnapshot_t *pSnapshotsTest =
        (StateSnapshot_t *)_alloca(numSnapshotsTest * sizeof(StateSnapshot_t));

    for (int i = 0; i < numSnapshotsTest; ++i) {
      pSnapshotsTest[i] = pRenderState->m_pSnapshots[mod].m_Snapshot[i];
    }

    VertexFormat_t usageTest =
        g_pShaderAPI->ComputeVertexUsage(numSnapshotsTest, pSnapshotsTest);
    Assert(IsVertexFormatSubsetOfVertexformat(usageTest,
                                              pRenderState->m_VertexUsage));
  }
#endif

  pRenderState->m_VertexFormat =
      g_pShaderAPI->ComputeVertexFormat(numSnapshots, pSnapshots);
...
//-----------------------------------------------------------------------------
bool CShaderSystem::InitRenderState(IShader *pShader, int numParams,
                                    IMaterialVar **params,
                                    ShaderRenderState_t *pRenderState,
                                    char const *pMaterialName) {
  Assert(!m_pRenderState);
...
void CShaderSystem::DrawElements(IShader *pShader, IMaterialVar **params,
                                 ShaderRenderState_t *pRenderState,
                                 VertexCompressionType_t vertexCompression,
                                 uint32_t nMaterialVarChangeTimeStamp) {
  ...
  if (((materialVarFlags & MATERIAL_VAR_MODEL) != 0) ||
      (IsFlag2Set(params, MATERIAL_VAR2_SUPPORTS_HW_SKINNING) &&
       (g_pShaderAPI->GetCurrentNumBones() > 0))) {
  ...
  if ((g_config.bMeasureFillRate || g_config.bVisualizeFillRate) &&
    ...
    IMaterialInternal *pDebugMaterial =
        m_pDebugMaterials[MATERIAL_DEBUG_DEPTH + nIndex];
    if (!g_pShaderAPI->IsDepthWriteEnabled(
            pRenderState->m_Snapshots[mod].m_Snapshot[0])) {
    ...
    CBasePerMaterialContextData **pContextDataPtr =
        &(m_pRenderState->m_pSnapshots[m_nModulation]
              .m_pContextData[m_nRenderPass]);
    ...
    pShader->DrawElements(params, mod, 0, g_pShaderAPI, vertexCompression,
                          &(m_pRenderState->m_pSnapshots[m_nModulation]
                                .m_pContextData[m_nRenderPass]));
    DoneWithShaderDraw();
  }
...
bool CShaderSystem::IsUsingGraphics() const {
  // YWB Hack if running with -noshaderapi/-makereslists this forces materials
  // to "precache" which means they will resolve their .vtf files for
...
void CShaderSystem::TakeSnapshot() {
  ...
  // Take a snapshot...
  snapshotList.m_Snapshot[snapshotList.m_nPassCount] =
...
//-----------------------------------------------------------------------------
void CShaderSystem::DrawUsingMaterial(
    IMaterialInternal *pMaterial, VertexCompressionType_t vertexCompression) {
...
// Copies material vars
//-----------------------------------------------------------------------------
void CShaderSystem::CopyMaterialVarToDebugShader(
    IMaterialInternal *pDebugMaterial, IShader *pShader,
    IMaterialVar **ppParams, const char *pSrcVarName, const char *pDstVarName) {
  bool bFound;
  IMaterialVar *pMaterialVar =
      pDebugMaterial->FindVar(pDstVarName ? pDstVarName : pSrcVarName, &bFound);
  Assert(bFound);
...
//-----------------------------------------------------------------------------
void CShaderSystem::DrawMeasureFillRate(
    ShaderRenderState_t *pRenderState, int mod,
    VertexCompressionType_t vertexCompression) {
  int nPassCount = pRenderState->m_pSnapshots[mod].m_nPassCount;

  bool bUsesVertexShader = (VertexFlags(pRenderState->m_VertexFormat) &
                            VERTEX_FORMAT_VERTEX_SHADER) != 0;
  IMaterialInternal *pMaterial =
      m_pDebugMaterials[bUsesVertexShader ? MATERIAL_FILL_RATE
                                          : MATERIAL_FILL_RATE_DX6];
...
void CShaderSystem::DrawNormalMap(IShader *pShader, IMaterialVar **ppParams,
                                  VertexCompressionType_t vertexCompression) {
  IMaterialInternal *pDebugMaterial =
      m_pDebugMaterials[MATERIAL_DEBUG_NORMALMAP];

  if (!g_config.m_bFastNoBump) {
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams, "$bumpmap");
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$bumpframe");
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$bumptransform");
  } else {
...
bool CShaderSystem::DrawEnvmapMask(IShader *pShader, IMaterialVar **ppParams,
  ...
  IMaterialInternal *pDebugMaterial =
      m_pDebugMaterials[MATERIAL_DEBUG_ENVMAPMASK];
  ...
  if (IsFlagSet(ppParams, MATERIAL_VAR_NORMALMAPALPHAENVMAPMASK)) {
    ...
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$bumpframe", "$frame");
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$bumptransform", "$basetexturetransform");
    ...
    // $envmapmask
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$envmapmask", "$basetexture");
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$envmapmaskframe", "$frame");
    CopyMaterialVarToDebugShader(pDebugMaterial, pShader, ppParams,
                                 "$envmapmasktransform",
                                 "$basetexturetransform");
...
ShaderAPITextureHandle_t CShaderSystem::GetShaderAPITextureBindHandle(
    ITexture *pTexture, int nFrame, int nTextureChannel) {
  Assert(
      !IsTextureInternalEnvCubemap(static_cast<ITextureInternal *>(pTexture)));

  // Bind away baby
  if (pTexture) {
    return static_cast<ITextureInternal *>(pTexture)->GetTextureHandle(
        nFrame, nTextureChannel);
  } else
...
// Binds a texture
//-----------------------------------------------------------------------------
void CShaderSystem::BindTexture(Sampler_t sampler1, ITexture *pTexture,
                                int nFrame /* = 0 */) {
  // The call to IMaterialVar::GetTextureValue should have converted this to a
  // real thing
  Assert(
      !IsTextureInternalEnvCubemap(static_cast<ITextureInternal *>(pTexture)));
...
void CShaderSystem::BindTexture(Sampler_t sampler1, Sampler_t sampler2,
                                ITexture *pTexture, int nFrame /* = 0 */) {
  // The call to IMaterialVar::GetTextureValue should have converted this to a
  // real thing
  Assert(
...
void CShaderSystem::LoadTexture(IMaterialVar *pTextureVar,
  ...
  // Fix cases where people stupidly put a slash at the front of the vtf
  // filename in a vmt. Causes trouble elsewhere.
...
// Loads a cubemap
//-----------------------------------------------------------------------------
void CShaderSystem::LoadCubeMap(IMaterialVar **ppParams,
                                IMaterialVar *pTextureVar) {
  if (!HardwareConfig()->SupportsCubeMaps()) return;
