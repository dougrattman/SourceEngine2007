...
void COcclusionQueryMgr::OnCreateOcclusionQueryObject(
    OcclusionQueryObjectHandle_t h) {
  m_OcclusionQueryObjects[(int)h].m_QueryHandle =
      g_pShaderAPI->CreateOcclusionQueryObject();
}

// Flushes an outstanding query
void COcclusionQueryMgr::FlushQuery(
    OcclusionQueryObjectHandle_t hOcclusionQuery) {
...
void COcclusionQueryMgr::AllocOcclusionQueryObjects(void) {
  FOR_EACH_LL(m_OcclusionQueryObjects, iterator) {
    m_OcclusionQueryObjects[iterator].m_QueryHandle =
        g_pShaderAPI->CreateOcclusionQueryObject();
    m_OcclusionQueryObjects[iterator].m_bHasBeenIssued =
        false;  // any in-flight queries are never returning
  }
}

void COcclusionQueryMgr::FreeOcclusionQueryObjects(void) {
  FOR_EACH_LL(m_OcclusionQueryObjects, iterator) {
    if (m_OcclusionQueryObjects[iterator].m_QueryHandle !=
        INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE) {
...
//-----------------------------------------------------------------------------
void COcclusionQueryMgr::ResetOcclusionQueryObject(
    OcclusionQueryObjectHandle_t hOcclusionQuery) {
  int h = (int)hOcclusionQuery;
  Assert(m_OcclusionQueryObjects.IsValidIndex(h));
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
    // Flush out any previous queries
    if (m_OcclusionQueryObjects[h].m_bHasBeenIssued) {
      ShaderAPIOcclusionQuery_t hQuery =
          m_OcclusionQueryObjects[h].m_QueryHandle;
      int nPixels;
      do {
        nPixels =
...
//-----------------------------------------------------------------------------
void COcclusionQueryMgr::BeginOcclusionQueryDrawing(
    OcclusionQueryObjectHandle_t hOcclusionQuery) {
  int h = (int)hOcclusionQuery;
  Assert(m_OcclusionQueryObjects.IsValidIndex(h));
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
    ...
    if (hQuery != INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE) {
      // If it's been issued, but we haven't gotten a result when we polled last
      // time, try polling one last time, since we can't poll again after we
      // issue again.
      if (m_OcclusionQueryObjects[h].m_bHasBeenIssued) {
        int nPixels =
            g_pShaderAPI->OcclusionQuery_GetNumPixelsRendered(hQuery, false);
        if ((nPixels == OCCLUSION_QUERY_RESULT_PENDING) &&
...
void COcclusionQueryMgr::EndOcclusionQueryDrawing(
    OcclusionQueryObjectHandle_t hOcclusionQuery) {
  int h = (int)hOcclusionQuery;
  Assert(m_OcclusionQueryObjects.IsValidIndex(h));
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
...
//-----------------------------------------------------------------------------
void COcclusionQueryMgr::OcclusionQuery_IssueNumPixelsRenderedQuery(
    OcclusionQueryObjectHandle_t hOcclusionQuery) {
  int h = (int)hOcclusionQuery;
  Assert(m_OcclusionQueryObjects.IsValidIndex(h));
  if (m_OcclusionQueryObjects.IsValidIndex(h)) {
    ShaderAPIOcclusionQuery_t hQuery = m_OcclusionQueryObjects[h].m_QueryHandle;
    if (hQuery != INVALID_SHADERAPI_OCCLUSION_QUERY_HANDLE &&
        m_OcclusionQueryObjects[h].m_bHasBeenIssued) {
...
int COcclusionQueryMgr::OcclusionQuery_GetNumPixelsRendered(
    OcclusionQueryObjectHandle_t h, bool bDoQuery) {
  if (bDoQuery) {
