...
the_interface IMaterialInternal : public IMaterial {
 public:
  ...
  static IMaterialInternal *CreateMaterialSubRect(
      char const *pMaterialName, const char *pTextureGroupName,
      KeyValues *pKeyValues = NULL, KeyValues *pPatchKeyValues = NULL,
      bool bAssumeCreateFromFile = false);
  ...
  virtual bool PrecacheVars(KeyValues *pKeyValues = NULL,
                            ...
                            CUtlVector<FileNameHandle_t> *pIncludes = NULL) = 0;
  ...
  virtual uint32_t GetChangeID() const = 0;
  ...
  // easy swapping between the queue friendly and realtime versions of the
  // material
  ...
  // These are used when a new whitelist is passed in. First materials to be
  // reloaded are flagged, then they are reloaded.
  virtual void DecideShouldReloadFromWhitelist(IFileList * pFileList) = 0;
  virtual void ReloadFromWhitelistIfMarked() = 0;
};

extern void InsertKeyValues(KeyValues &dst, KeyValues &src,
...
// patchKeyValues accumulates keys applied by VMT patch files (this is necessary
// to make $fallbackmaterial work properly - the patch keys need to be reapplied
// when the fallback VMT is loaded). It may contain previously accumulated patch
// keys on entry, and may contain more encountered patch keys on exit.
extern bool LoadVMTFile(KeyValues &vmtKeyValues, KeyValues &patchKeyValues,
