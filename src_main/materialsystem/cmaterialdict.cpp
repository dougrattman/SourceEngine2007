...
void CMaterialDict::RemoveMaterialFromMaterialList(
    IMaterialInternal *pMaterial) {
  // Gotta iterate over this manually; name-based lookup is bogus if there are
  // two materials with the same name, which can happen for procedural materials
  // First remove all the subrect materials, because they'll point at their
  // material pages.
  MaterialHandle_t i;
...
//-----------------------------------------------------------------------------
IMaterialInternal *CMaterialDict::AddMaterial(char const *pName,
                                              const char *pTextureGroupName) {
  IMaterialInternal *pMaterial =
      IMaterialInternal::CreateMaterial(pName, pTextureGroupName);
...
IMaterialInternal *CMaterialDict::AddMaterialSubRect(
    const char *pName, const char *pTextureGroupName, KeyValues *pKeyValues,
    KeyValues *pPatchKeyValues) {
  IMaterialInternal *pMaterial = IMaterialInternal::CreateMaterialSubRect(
      pName, pTextureGroupName, pKeyValues, pPatchKeyValues, true);
  Assert(pMaterial);
  AddMaterialToMaterialList(pMaterial);
  return pMaterial;
}
