...
the_interface ITextureManager {
 public:
  ...
  // Some render targets are managed by code outside of the materialsystem but
  // are used by the materialsystem all the time.
  ...
  virtual ITextureInternal *CreateProceduralTexture(
      ...
      int d, ImageFormat fmt, int nFlags) = 0;
  ...
  virtual ITextureInternal *FindOrLoadTexture(
      const char *pTextureName, const char *pTextureGroupName) = 0;
  ...
  // Releases texture memory bits for temporary render targets, does NOT destroy
  // the CTexture entirely
  virtual void ReleaseTempRenderTargetBits(void) = 0;

  // See CL_HandlePureServerWhitelist for a description of the pure server
  // stuff.
