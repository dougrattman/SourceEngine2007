...
struct COperatorNode {
  COperatorNode(IDmeOperator *pOp = NULL)
      : m_state(TS_NOT_VISITED), m_operator(pOp), m_bInList(false) {}
...
class CAttributeNode {
 public:
  CAttributeNode(CDmAttribute *attribute = NULL)
      : m_attribute(attribute), m_bIsOutputToOperator(false) {}
...
bool HashEntryCompareFunc(CAttributeNode *const &lhs,
                          CAttributeNode *const &rhs) {
  return lhs->m_attribute == rhs->m_attribute;
}

uint32_t HashEntryKeyFunc(CAttributeNode *const &keyinfo) {
  uint32_t i = (uint32_t)keyinfo->m_attribute;
...
void CDependencyGraph::Reset(const CUtlVector<IDmeOperator *> &operators) {
  ...
  CUtlVector<CDmAttribute *> attrs;  // moved outside the loop to function as a
                                     // temporary memory pool for performance
  int on = operators.Count();
  CUtlRBTree<IDmeOperator *> operatorDict(0, on * 2,
                                          DefLessFunc(IDmeOperator *));
  for (int i = 0; i < on; ++i) {
    ...
    for (int ai = 0; ai < an; ++ai) {
      ...
      if (pOperator) {
        ...
        if (operatorDict.Find(pOperator) == operatorDict.InvalidIndex()) {
          ...
          Warning(
              "Found dependent operator '%s' referenced by operator '%s' that "
              "wasn't in the scene or trackgroups!\n",
...
//-----------------------------------------------------------------------------
CDependencyGraph::~CDependencyGraph() { Cleanup(); }
...
void CDependencyGraph::FindRoots() {
  ...
  for (oi = 0; oi < on; ++oi) {
    ...
    if (!pOp->IsDirty()) continue;

    m_opRoots.AddToTail(pOpNode);
    pOpNode->m_bInList = true;
  }

  // Do we have an attribute which is an input to us which is not an output to
  // some other op?
  UtlHashHandle_t h = m_attrNodes.GetFirstHandle();
  for (; h != m_attrNodes.InvalidHandle(); h = m_attrNodes.GetNextHandle(h)) {
    CAttributeNode *pAttrNode = m_attrNodes[h];
    // Msg( "attrib %s %p\n", pAttrNode->m_attribute->GetName(),
    // pAttrNode->m_attribute );
...
bool CDependencyGraph::CullAndSortOperators() {
  ...
  bool cycle = GetOperatorOrdering(
      m_opRoots, m_operators);  // leaves to roots (outputs to inputs)
  ...
  //	return GetOperatorOrdering( m_opLeaves, operators ); // roots to leaves
  //(inputs to outputs)
}

//-----------------------------------------------------------------------------
// GetOperatorOrdering is just a recursive post-order depth-first-search
//   since we have two types of nodes, we manually traverse to the opnodes
//   grandchildren, which are opnodes (skipping children, which are attrnodes)
//   returns true if a cycle found - in this case, an arbitrary link of the
//   cycle will be ignored
//-----------------------------------------------------------------------------
bool CDependencyGraph::GetOperatorOrdering(
    CUtlVector<COperatorNode *> &pOpNodes,
    CUtlVector<IDmeOperator *> &operators) {
  bool cycle = false;

  uint32_t on = pOpNodes.Count();
  for (uint32_t oi = 0; oi < on; ++oi) {
    ...
    uint32_t an = pOpNode->m_OutputAttributes.Count();
    for (uint32_t ai = 0; ai < an; ++ai) {
      CAttributeNode *pAttrNode = pOpNode->m_OutputAttributes[ai];
      if (GetOperatorOrdering(pAttrNode->m_InputDependentOperators,
                              operators)) {
        cycle = true;
      }
    }
...
//-----------------------------------------------------------------------------
void CDependencyGraph::DBG_PrintOperator(const char *pIndent,
                                         IDmeOperator *pOp) {
