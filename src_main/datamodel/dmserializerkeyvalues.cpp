...
class CDmSerializerKeyValues : public IDmSerializer {
 ...
 private:
  ...
  DmElementHandle_t UnserializeElement(KeyValues *pKeyValues,
                                       int iNestingLevel);
  void UnserializeAttribute(CDmElement *pElement, KeyValues *pKeyValues);
  DmElementHandle_t CreateDmElement(const char *pElementType,
...
//-----------------------------------------------------------------------------
void CDmSerializerKeyValues::SerializeSubKeys(CUtlBuffer &buf,
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues::SerializeAttributes(CUtlBuffer &buf,
                                                 CDmElement *pElement) {
  ...
  for (int i = nAttributes - 1; i >= 0; --i) {
    ...
    for (int j = 0; s_pAttributeRemap[j].m_pKeyValuesName; ++j) {
      if (!Q_stricmp(pName, s_pAttributeRemap[j].m_pDmeName)) {
        pName = s_pAttributeRemap[j].m_pKeyValuesName;
        break;
      }
    }
    ...
    switch (nAttrType) {
      ...
      case AT_ELEMENT_ARRAY:
        Warning(
            "KeyValues: Can't serialize attribute of type %s into KeyValues "
            "files!\n",
...
bool CDmSerializerKeyValues::SerializeElement(CUtlBuffer &buf,
                                              CDmElement *pElement) {
  buf.Printf("\"%s\"\n{\n", pElement->GetName());
  buf.PushTab();
...
//-----------------------------------------------------------------------------
DmElementHandle_t CDmSerializerKeyValues::CreateDmElement(
    const char *pElementType, const char *pElementName) {
  // See if we can create an element of that type
  DmElementHandle_t hElement =
      g_pDataModel->CreateElement(pElementType, pElementName, m_fileid);
...
//-----------------------------------------------------------------------------
void CDmSerializerKeyValues::UnserializeAttribute(CDmElement *pElement,
                                                  KeyValues *pKeyValues) {
  // It's an attribute
  ...
  if (type == AT_UNKNOWN) {
    ...
    Warning(
        "Dm Unserialize: Attempted to read an attribute (\"%s\") of an "
        "inappropriate type!\n",
        pLowerName.String());
    return;
  }
  ...
  switch (type) {
    case AT_STRING: {
      ...
      // so let's just directly copy the string instead of going through
      // unserialize
    ...
    default: {
      ...
      CUtlBuffer buf(pAttributeValue, nLen,
...
//-----------------------------------------------------------------------------
DmElementHandle_t CDmSerializerKeyValues::UnserializeElement(
    KeyValues *pKeyValues, int iNestingLevel) {
  const char *pElementName = pKeyValues->GetName();
  const char *pszKeyValuesElement =
      g_pDataModel->GetKeyValuesElementName(pElementName, iNestingLevel);
  ...
  for (KeyValues *pSub = pKeyValues->GetFirstSubKey(); pSub != NULL;
...
//-----------------------------------------------------------------------------
CDmElement *CDmSerializerKeyValues::UnserializeFromKeyValues(
    KeyValues *pKeyValues) {
  ...
  for (KeyValues *pElementKey = pKeyValues; pElementKey != NULL;
       pElementKey = pElementKey->GetNextKey()) {
...
// Main entry point for the unserialization
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues::Unserialize(
