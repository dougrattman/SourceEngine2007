...
class CElementIdHash : public CUtlHash<DmElementHandle_t> {
 public:
  CElementIdHash(int nBucketCount = 0, int nGrowCount = 0, int nInitCount = 0)
      : CUtlHash<DmElementHandle_t>(nBucketCount, nGrowCount, nInitCount,
                                    CompareFunc, KeyFunc) {}

 protected:
  typedef CUtlHash<DmElementHandle_t> BaseClass;

  static bool CompareFunc(DmElementHandle_t const &a,
                          DmElementHandle_t const &b) {
    return a == b;
  }
  static bool IdCompareFunc(DmElementHandle_t const &hElement,
                            DmObjectId_t const &id) {
 ...
 protected:
  bool DoFind(DmObjectId_t const &src, unsigned int *pBucket, int *pIndex) {
    ...
    for (ndxKeyData = 0; ndxKeyData < keyDataCount; ndxKeyData++) {
      if (IdCompareFunc(bucket.Element(ndxKeyData), src)) break;
    }
 ...
 public:
  UtlHashHandle_t Find(DmElementHandle_t const &src) {
    return BaseClass::Find(src);
  }
...
struct FileElementSet_t {
  FileElementSet_t(UtlSymId_t filename = UTL_INVAL_SYMBOL,
                   UtlSymId_t format = UTL_INVAL_SYMBOL)
      : m_filename(filename),
        m_format(format),
  ...
  FileElementSet_t(const FileElementSet_t &that)
    ...
    // the only time this should be copy constructed is when passing in an empty
    // set to the parent array otherwise it could get prohibitively expensive
...
class CDataModel : public CBaseAppSystem<IDataModel> {
 ...
 public:
  ...
  virtual void AddElementFactory(const char *pClassName,
  ...
  virtual DmElementHandle_t CreateElement(UtlSymId_t typeSymbol,
                                          const char *pElementName,
                                          DmFileId_t fileid,
                                          const DmObjectId_t *pObjectID = NULL);
  virtual DmElementHandle_t CreateElement(const char *pTypeName,
                                          const char *pElementName,
                                          DmFileId_t fileid,
                                          const DmObjectId_t *pObjectID = NULL);
  ...
  virtual bool SaveToFile(char const *pFileName, char const *pPathID,
  ...
  virtual void StartUndo(const char *undodesc, const char *redodesc,
                         int nChainingID = 0);
  ...
  virtual void TraceUndo(
      bool state);  // if true, undo records spew as they are added
 ...
 public:
  ...
  // CreateElement references the attribute list passed in via ref, so don't
  // edit or purge ref's attribute list afterwards
  CDmElement *CreateElement(const DmElementReference_t &ref,
                            const char *pElementType, const char *pElementName,
                            DmFileId_t fileid, const DmObjectId_t *pObjectID);
  void DeleteElement(DmElementHandle_t hElement,
  ...
  // Returns false if the mailing list is empty now (can happen owing to stale
  // attributes)
  ...
  // unserialization
  DmElementHandle_t ChangeElementId(DmElementHandle_t hElement,
                                    const DmObjectId_t &oldId,
                                    const DmObjectId_t &newId);

  DmElementReference_t *FindElementReference(DmElementHandle_t hElement,
                                             DmObjectId_t **ppId = NULL);
  ...
  bool IsCreatingUntypedElements() const {
    return m_bOnlyCreateUntypedElements;
  }
 ...
 private:
  ...
  struct ElementIdHandlePair_t {
    ...
    ElementIdHandlePair_t(const DmObjectId_t &id,
                          const DmElementReference_t &ref)
        : m_ref(ref) {
      CopyUniqueId(id, &m_id);
    }
    ElementIdHandlePair_t(const ElementIdHandlePair_t &that)
        : m_ref(that.m_ref) {
    ...
    static bool Compare(const ElementIdHandlePair_t &a,
                        const ElementIdHandlePair_t &b) {
 ...
 private:
  ...
  void OnElementReferenceAdded(DmElementHandle_t hElement,
                               CDmAttribute *pAttribute);
  void OnElementReferenceRemoved(DmElementHandle_t hElement,
                                 CDmAttribute *pAttribute);
...
class CDmeElementAccessor {
 public:
  ...
  static void SetId(CDmElement *pElement, const DmObjectId_t &id) {
    pElement->SetId(id);
  }
  static bool IsDirty(const CDmElement *pElement) {
    return pElement->IsDirty();
  }
  static void MarkDirty(CDmElement *pElement, bool dirty = true) {
    pElement->MarkDirty(dirty);
  }
  static void MarkAttributesClean(CDmElement *pElement) {
    pElement->MarkAttributesClean();
  }
  static void MarkBeingUnserialized(CDmElement *pElement,
                                    bool beingUnserialized = true) {
    pElement->MarkBeingUnserialized(beingUnserialized);
  }
  static bool IsBeingUnserialized(const CDmElement *pElement) {
    return pElement->IsBeingUnserialized();
  }
  static void AddAttributeByPtr(CDmElement *pElement, CDmAttribute *ptr) {
    pElement->AddAttributeByPtr(ptr);
  }
  static void RemoveAttributeByPtrNoDelete(CDmElement *pElement,
                                           CDmAttribute *ptr) {
    pElement->RemoveAttributeByPtrNoDelete(ptr);
  }
  static void ChangeHandle(CDmElement *pElement, DmElementHandle_t handle) {
    pElement->ChangeHandle(handle);
  }
  static DmElementReference_t *GetReference(CDmElement *pElement) {
    return pElement->GetReference();
  }
  static void SetReference(CDmElement *pElement,
                           const DmElementReference_t &ref) {
    pElement->SetReference(ref);
  }
  static int EstimateMemoryUsage(CDmElement *pElement,
                                 CUtlHash<DmElementHandle_t> &visited,
                                 TraversalDepth_t depth, int *pCategories) {
    return pElement->EstimateMemoryUsage(visited, depth, pCategories);
  }
  static void PerformConstruction(CDmElement *pElement) {
    pElement->PerformConstruction();
  }
  static void PerformDestruction(CDmElement *pElement) {
    pElement->PerformDestruction();
  }
};
