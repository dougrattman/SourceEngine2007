...
// Current line control
//-----------------------------------------------------------------------------
void CKeyValues2ErrorStack::IncrementCurrentLine() { ++m_nFileLine; }

void CKeyValues2ErrorStack::SetCurrentLine(int nLine) { m_nFileLine = nLine; }

int CKeyValues2ErrorStack::GetCurrentLine() const { return m_nFileLine; }
...
class CDmSerializerKeyValues2 : public IDmSerializer {
 ...
 private:
  ...
  bool UnserializeAttributeValueFromToken(CDmAttribute *pAttribute,
                                          CUtlBuffer &tokenBuf);
  ...
  bool UnserializeAttribute(CUtlBuffer &buf, DmElementDictHandle_t hElement,
                            const char *pAttributeName,
                            DmAttributeType_t nAttrType);
  ...
  bool UnserializeElements(CUtlBuffer &buf, DmFileId_t fileid,
                           DmConflictResolution_t idConflictResolution,
                           CDmElement **ppRoot);
...
//-----------------------------------------------------------------------------
void CDmSerializerKeyValues2::SerializeElementAttribute(
...
//-----------------------------------------------------------------------------
void CDmSerializerKeyValues2::SerializeElementArrayAttribute(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
    CDmAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
void CDmSerializerKeyValues2::SerializeArrayAttribute(
    CUtlBuffer &buf, CDmAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::SerializeAttributes(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
    CDmElement *pElement) {
  ...
  for (CDmAttribute *pAttribute = pElement->FirstAttribute(); pAttribute;
    ...
    if (pAttribute->IsFlagSet(FATTRIB_DONTSAVE)) continue;
...
bool CDmSerializerKeyValues2::SaveElement(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
  ...
  buf.Printf("\"id\" \"%s\" ",
             g_pDataModel->GetAttributeNameForType(AT_OBJECTID));
  buf.PutChar('\"');
  ::Serialize(buf, pElement->GetId());
  buf.PutString("\"\n");
...
//-----------------------------------------------------------------------------
CDmSerializerKeyValues2::TokenType_t CDmSerializerKeyValues2::ReadToken(
...
//-----------------------------------------------------------------------------
DmElementDictHandle_t CDmSerializerKeyValues2::CreateDmElement(
    const char *pElementType) {
  // See if we can create an element of that type
  DmElementHandle_t hElement =
      g_pDataModel->CreateElement(pElementType, "", m_fileid);
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeElementAttribute(
    CUtlBuffer &buf, DmElementDictHandle_t hElement, const char *pAttributeName,
    const char *pElementType) {
  ...
  if (!pAttribute) {
    g_KeyValues2ErrorStack.ReportError(
        "Attempted to read an attribute (\"%s\") of unknown type %s!\n",
  ...
  if (bOk) {
    ...
    pAttribute->SetValue(pNewElement ? pNewElement->GetHandle()
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeElementArrayAttribute(
    CUtlBuffer &buf, DmElementDictHandle_t hElement,
    const char *pAttributeName) {
  CDmElement *pElement = m_ElementDict.GetElement(hElement);
  CDmAttribute *pAttribute =
      pElement->AddAttribute(pAttributeName, AT_ELEMENT_ARRAY);
  if (!pAttribute) {
    g_KeyValues2ErrorStack.ReportError(
        "Attempted to read an attribute (\"%s\") of an inappropriate type!\n",
        pAttributeName);
    return false;
  }
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeAttributeValueFromToken(
    CDmAttribute *pAttribute, CUtlBuffer &tokenBuf) {
...
// Reads an attribute for an element array
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeArrayAttribute(
    CUtlBuffer &buf, DmElementDictHandle_t hElement, const char *pAttributeName,
    DmAttributeType_t nAttrType) {
  CDmElement *pElement = m_ElementDict.GetElement(hElement);
...
// Reads an attribute for an element
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeAttribute(
    CUtlBuffer &buf, DmElementDictHandle_t hElement, const char *pAttributeName,
    DmAttributeType_t nAttrType) {
  ...
  switch (nAttrType) {
    ...
    default:
      if (UnserializeAttributeValueFromToken(pAttribute, tokenBuf)) return true;
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeElement(
    CUtlBuffer &buf, const char *pElementType, DmElementDictHandle_t *pHandle) {
  *pHandle = ELEMENT_DICT_HANDLE_INVALID;

  // Create the element
  DmElementDictHandle_t hElement = CreateDmElement(pElementType);
  if (hElement == ELEMENT_DICT_HANDLE_INVALID) return false;
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::UnserializeElement(
    CUtlBuffer &buf, DmElementDictHandle_t *pHandle) {
  *pHandle = ELEMENT_DICT_HANDLE_INVALID;
...
//-----------------------------------------------------------------------------
bool CDmSerializerKeyValues2::Unserialize(
    CUtlBuffer &buf, const char *pEncodingName, int nEncodingVersion,
  ...
  bool bSuccess =
      UnserializeElements(buf, fileid, idConflictResolution, ppRoot);
...
bool CDmSerializerKeyValues2::UnserializeElements(
    CUtlBuffer &buf, DmFileId_t fileid,
    DmConflictResolution_t idConflictResolution, CDmElement **ppRoot) {
  *ppRoot = NULL;
