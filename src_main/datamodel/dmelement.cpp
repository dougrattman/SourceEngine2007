...
void DmElementReference_t::AddAttribute(CDmAttribute *pAttribute) {
  if (m_attributes.m_hAttribute != DMATTRIBUTE_HANDLE_INVALID) {
    DmAttributeList_t *pLink =
        new DmAttributeList_t;  // TODO - create a fixed size allocator for
                                // these
...
void DmElementReference_t::RemoveAttribute(CDmAttribute *pAttribute) {
  ...
  for (DmAttributeList_t *pLink = &m_attributes; pLink->m_pNext;
       pLink = pLink->m_pNext) {
...
CDmElementFactoryHelper g_CDmeElement_Helper("DmeElement",
                                             &g_CDmeElement_Factory, true);

//-----------------------------------------------------------------------------
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmElement::CDmElement(DmElementHandle_t handle, const char *pElementType,
  ...
  m_Name.InitAndSet(this, "name", pElementName,
                    FATTRIB_TOPOLOGICAL | FATTRIB_STANDARD);
...
void CDmElement::PerformConstruction() { OnConstruction(); }

void CDmElement::PerformDestruction() { OnDestruction(); }
...
void CDmElement::SetId(const DmObjectId_t &id) { CopyUniqueId(id, &m_Id); }

//-----------------------------------------------------------------------------
// RTTI implementation
//-----------------------------------------------------------------------------
void CDmElement::SetTypeSymbol(CUtlSymbol sym) { m_classType = sym; }
...
//-----------------------------------------------------------------------------
bool CDmElement::IsDirty() const { return m_bDirty; }
...
void CDmElement::MarkAttributesClean() {
  for (CDmAttribute *pAttr = m_pAttributes; pAttr;
       pAttr = pAttr->NextAttribute()) {
...
void CDmElement::MarkBeingUnserialized(bool beingUnserialized) {
  if (m_bBeingUnserialized != beingUnserialized) {
    ...
    // After we finish unserialization, call OnAttributeChanged; assume
    // everything changed
    if (!beingUnserialized) {
      for (CDmAttribute *pAttribute = m_pAttributes; pAttribute;
           pAttribute = pAttribute->NextAttribute()) {
        pAttribute->OnUnserializationFinished();
      }

      // loop referencing attributes, and call OnAttributeChanged on them as
      // well
      if (m_ref.m_attributes.m_hAttribute != DMATTRIBUTE_HANDLE_INVALID) {
        for (DmAttributeList_t *pAttrLink = &m_ref.m_attributes; pAttrLink;
             pAttrLink = pAttrLink->m_pNext) {
          CDmAttribute *pAttr =
              g_pDataModel->GetAttribute(pAttrLink->m_hAttribute);
          if (!pAttr || pAttr->GetOwner()->GetFileId() == m_fileId)
            continue;  // attributes in this file will already have
                       // OnAttributeChanged called on them
...
bool CDmElement::IsBeingUnserialized() const { return m_bBeingUnserialized; }
...
DmElementReference_t *CDmElement::GetReference() { return &m_ref; }
...
int CDmElement::EstimateMemoryUsage(CUtlHash<DmElementHandle_t> &visited,
                                    TraversalDepth_t depth, int *pCategories) {
  if (visited.Find(m_ref.m_hElement) != visited.InvalidHandle()) return 0;
  ...
  int nInternalUsage =
      sizeof(*this) -
      sizeof(CUtlString);  // NOTE: The utlstring is the 'name' attribute var
  ...
  for (CDmAttribute *pAttr = m_pAttributes; pAttr;
       pAttr = pAttr->NextAttribute()) {
    nAttributeDataUsage +=
        pAttr->EstimateMemoryUsageInternal(visited, depth, pCategories);
  }

  return nInternalUsage + nDataModelUsage + nReferenceUsage + nOuterUsage +
         nAttributeDataUsage;
}

//-----------------------------------------------------------------------------
// these functions are here for the mark and sweep algorithm for deleting
// orphaned subtrees it's conservative, so it can return true for orphaned
// elements but false really means it isn't accessible
//-----------------------------------------------------------------------------
bool CDmElement::IsAccessible() const { return m_bIsAcessible; }
...
void CDmElement::MarkAccessible(TraversalDepth_t depth /* = TD_ALL */) {
  if (m_bIsAcessible) return;

  m_bIsAcessible = true;

  for (const CDmAttribute *pAttr = FirstAttribute(); pAttr != NULL;
       pAttr = pAttr->NextAttribute()) {
    if (!ShouldTraverse(pAttr, depth)) continue;
...
// do we want a true visited set to avoid retraversing the same subtree over and
// over again? for most dag trees, it's probably a perf loss, since multiple
// instances are rare, (and searching the visited set costs log(n)) for trees
// that include channels, it's probably a perf win, since many channels link
// into the same element most of the time
bool CDmElement::FindElement(const CDmElement *pElement,
                             CUtlVector<ElementPathItem_t> &elementPath,
                             TraversalDepth_t depth) const {
  if (this == pElement) return true;

  ElementPathItem_t search(GetHandle());
  if (elementPath.Find(search) != elementPath.InvalidIndex()) return false;
...
bool CDmElement::FindReferer(DmElementHandle_t hElement,
                             CUtlVector<ElementPathItem_t> &elementPath,
                             TraversalDepth_t depth /* = TD_SHALLOW */) const {
  DmElementHandle_t hThis = GetHandle();

  DmAttributeReferenceIterator_t hAttr =
      g_pDataModel->FirstAttributeReferencingElement(hThis);
  for (; hAttr != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID;
       hAttr = g_pDataModel->NextAttributeReferencingElement(hAttr)) {
    ...
    if (elementPath.Find(ElementPathItem_t(hOwner)) !=
        elementPath.InvalidIndex())
      return false;
    ...
    if (hOwner == hElement) return true;

    CDmElement *pOwner = GetElement<CDmElement>(hOwner);
    if (pOwner->FindReferer(hElement, elementPath, depth)) return true;
...
void CDmElement::RemoveAllReferencesToElement(CDmElement *pElement) {
  for (CDmAttribute *pAttr = FirstAttribute(); pAttr != NULL;
       pAttr = pAttr->NextAttribute()) {
    if (pAttr->GetType() == AT_ELEMENT) {
      CDmElement *pChild = pAttr->GetValueElement<CDmElement>();
      if (pChild == pElement) {
...
//
//-----------------------------------------------------------------------------
CDmElement *CDmElement::CopyInternal(
    TraversalDepth_t depth /* = TD_DEEP */) const {
  CDmElement *pCopy = GetElement<CDmElement>(
      g_pDataModel->CreateElement(GetType(), GetName(), GetFileId()));
  if (pCopy) {
...
//-----------------------------------------------------------------------------
void CDmElement::CopyAttributesTo(CDmElement *pCopy,
                                  TraversalDepth_t depth) const {
  CDisableUndoScopeGuard sg;

  CUtlMap<DmElementHandle_t, DmElementHandle_t, int> refmap(
      DefLessFunc(DmElementHandle_t));
  CopyAttributesTo(pCopy, refmap, depth);

  CUtlHashFast<DmElementHandle_t> visited;
  uint32_t nPow2Size = 1;
...
//-----------------------------------------------------------------------------
void CDmElement::CopyElementAttribute(const CDmAttribute *pSrcAttr,
                                      CDmAttribute *pDestAttr, CRefMap &refmap,
                                      TraversalDepth_t depth) const {
  ...
  if (ShouldTraverse(pSrcAttr, depth)) {
    ...
    if (hDest == DMELEMENT_HANDLE_INVALID) {
      hDest = g_pDataModel->CreateElement(pSrc->GetType(), pSrc->GetName(),
                                          pSrc->GetFileId());
...
//-----------------------------------------------------------------------------
void CDmElement::CopyElementArrayAttribute(const CDmAttribute *pAttr,
                                           CDmAttribute *pCopyAttr,
                                           CRefMap &refmap,
                                           TraversalDepth_t depth) const {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (bCopy) {
      DmElementHandle_t hDest = g_pDataModel->CreateElement(
          pSrc->GetType(), pSrc->GetName(), pSrc->GetFileId());
...
// builds refmap of old element's handle -> copy's handle, and uses it to fixup
// references
//-----------------------------------------------------------------------------
void CDmElement::CopyAttributesTo(CDmElement *pCopy, CRefMap &refmap,
                                  TraversalDepth_t depth) const {
  refmap.Insert(this->GetHandle(), pCopy->GetHandle());

  // loop attrs, copying - element (and element array) attrs can be marked to
  // always copy deep(er)
  for (const CDmAttribute *pAttr = FirstAttribute(); pAttr != NULL;
       pAttr = pAttr->NextAttribute()) {
...
//	but the targets parent might decide to copy it, (later on in the
// travesal) 	so the channel needs to change to refer to the copy
//-----------------------------------------------------------------------------
void CDmElement::FixupReferences(CUtlHashFast<DmElementHandle_t> &visited,
                                 const CRefMap &refmap,
                                 TraversalDepth_t depth) {
  if (visited.Find(GetHandle()) != visited.InvalidHandle()) return;

  visited.Insert(GetHandle(),
                 DMELEMENT_HANDLE_INVALID);  // ignore data arguement - we're
  ...
  for (CDmAttribute *pAttr = FirstAttribute(); pAttr != NULL;
    ...
    if (type == AT_ELEMENT) {
      ...
      if (idx == refmap.InvalidIndex()) {
        ...
        if (pElement == NULL || !bCopy) continue;
...
//-----------------------------------------------------------------------------
// recursively set fileid's, with option to only change elements in the matched
// file
//-----------------------------------------------------------------------------
void CDmElement::SetFileId(DmFileId_t fileid, TraversalDepth_t depth,
                           bool bOnlyIfMatch /* = false */) {
  if (depth != TD_NONE) {
    CUtlHashFast<DmElementHandle_t> visited;
    visited.Init(4096);  // this will make visited behave reasonably (perf-wise)
                         // for trees w/ around 4k elements in them
...
void CDmElement::SetFileId_R(CUtlHashFast<DmElementHandle_t> &visited,
                             DmFileId_t fileid, TraversalDepth_t depth,
                             DmFileId_t match, bool bOnlyIfMatch) {
  if (bOnlyIfMatch && match != GetFileId()) return;
...
CDmAttribute *CDmElement::FirstAttribute() { return m_pAttributes; }

const CDmAttribute *CDmElement::FirstAttribute() const { return m_pAttributes; }

bool CDmElement::HasAttribute(const char *pAttributeName,
                              DmAttributeType_t type) const {
...
class CUndoAttributeRemove : public CUndoElement {
 ...
 public:
  CUndoAttributeRemove(CDmElement *pElement, CDmAttribute *pOldAttribute,
                       const char *attributeName, DmAttributeType_t type)
  ...
  virtual void Undo() {
    ...
    CDmAttribute *pAtt =
        m_pElement->AddAttribute(m_symAttribute.String(), m_Type);
    if (pAtt) {
      // Copy data from old version
      pAtt->SetValue(m_pOldAttribute);
    }
  }
  virtual void Redo() { m_pElement->RemoveAttribute(m_symAttribute.String()); }
...
void CDmElement::RemoveAttributeByPtrNoDelete(CDmAttribute *ptr) {
  for (CDmAttribute **ppAttr = &m_pAttributes; *ppAttr;
       ppAttr = (*ppAttr)->GetNextAttributeRef()) {
    if (ptr == *ppAttr) {
      MarkDirty();
...
void CDmElement::RemoveAttribute(CDmAttribute **pAttrRef) {
  ...
  if (pAttrToDelete->IsFlagSet(FATTRIB_EXTERNAL)) return;
  ...
  if (g_pDataModel->UndoEnabledForElement(this)) {
    ...
    CUndoAttributeRemove *pUndo =
        new CUndoAttributeRemove(this, pAttrToDelete, pAttrToDelete->GetName(),
                                 pAttrToDelete->GetType());
...
//-----------------------------------------------------------------------------
void CDmElement::SetValueFromString(const char *pAttributeName,
                                    const char *pValue) {
  CDmAttribute *pAttribute = FindAttribute(pAttributeName);
  if (pAttribute) {
...
//-----------------------------------------------------------------------------
const char *CDmElement::GetValueAsString(const char *pAttributeName,
                                         char *pBuffer, size_t nBufLen) const {
  Assert(pBuffer);

  const CDmAttribute *pAttribute = FindAttribute(pAttributeName);
  if (pAttribute) return pAttribute->GetValueAsString(pBuffer, nBufLen);
...
class CUndoAttributeAdd : public CUndoElement {
 ...
 public:
  ...
  void SetAttributePtr(CDmAttribute *pAttribute) { m_pAttribute = pAttribute; }

  virtual void Undo() {
    if (GetElement()) {
      CDmeElementAccessor::RemoveAttributeByPtrNoDelete(GetElement(),
                                                        m_pAttribute);
      m_bHoldingPtr = true;
    }
  }
 ...
 private:
  CDmElement *GetElement() { return g_pDataModel->GetElement(m_hElement); }
...
CDmAttribute *CDmElement::CreateAttribute(const char *pAttributeName,
...
CDmAttribute *CDmElement::AddExternalAttribute(const char *pAttributeName,
                                               DmAttributeType_t type,
                                               void *pMemory) {
  MarkDirty();
  ...
  {
    CDisableUndoScopeGuard guard;
    pAttribute = CDmAttribute::CreateExternalAttribute(this, type,
                                                       pAttributeName, pMemory);
    *(pAttribute->GetNextAttributeRef()) = m_pAttributes;
    m_pAttributes = pAttribute;
    if (pUndo) {
      pUndo->SetAttributePtr(pAttribute);
...
CDmAttribute *CDmElement::FindAttribute(const char *pAttributeName) const {
  ...
  for (CDmAttribute *pAttr = m_pAttributes; pAttr;
       pAttr = pAttr->NextAttribute()) {
    if (find == pAttr->GetNameSymbol()) return pAttr;
  }
...
// attribute renaming
//-----------------------------------------------------------------------------
void CDmElement::RenameAttribute(const char *pAttributeName,
                                 const char *pNewName) {
...
void InvokeOnAttributeChangedOnReferrers(DmElementHandle_t hElement,
  ...
  DmAttributeReferenceIterator_t ai =
      g_pDataModel->FirstAttributeReferencingElement(hElement);
  for (; ai != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID;
       ai = g_pDataModel->NextAttributeReferencingElement(ai)) {
    CDmAttribute *pAttr = g_pDataModel->GetAttribute(ai);
    Assert(pAttr);
    if (!pAttr) continue;

    if (pAttr->IsFlagSet(FATTRIB_NEVERCOPY)) continue;
...
void DestroyElement(CDmElement *pElement, TraversalDepth_t depth) {
  ...
  for (pAttribute = pElement->FirstAttribute(); pAttribute;
       pAttribute = pAttribute->NextAttribute()) {
    if (!ShouldTraverse(pAttribute, depth)) continue;
...
void CopyElements(const CUtlVector<CDmElement *> &from,
  ...
  for (int i = 0; i < c; ++i) {
    ...
    CDmElement *pCopy = GetElement<CDmElement>(g_pDataModel->CreateElement(
        pFrom->GetType(), pFrom->GetName(), pFrom->GetFileId()));
    if (pCopy) {
...
struct ElementArrayNameAccessor {
  ElementArrayNameAccessor(const CUtlVector<DmElementHandle_t> &array)
      : m_array(array) {}
  int Count() const { return m_array.Count(); }
...
int GenerateUniqueNameIndex(const char *prefix,
                            ...
                            int startindex) {
  return V_GenerateUniqueNameIndex(prefix, ElementArrayNameAccessor(array),
                                   startindex);
}

bool GenerateUniqueName(char *name, int memsize, const char *prefix,
                        const CUtlVector<DmElementHandle_t> &array) {
  return V_GenerateUniqueName(name, memsize, prefix,
                              ElementArrayNameAccessor(array));
}

void MakeElementNameUnique(CDmElement *pElement, const char *prefix,
                           const CUtlVector<DmElementHandle_t> &array,
                           bool forceIndex) {
  if (pElement == NULL || prefix == NULL) return;
  ...
  if (i <= 0) {
    ...
    i = 1;  // 1 means that no names of "prefix*" were found, but we want to
...
void RemoveElementFromRefereringAttributes(CDmElement *pElement,
  ...
  for (DmAttributeReferenceIterator_t i =
           g_pDataModel->FirstAttributeReferencingElement(
               pElement->GetHandle());
       i != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID;
       i = g_pDataModel->FirstAttributeReferencingElement(
           pElement->GetHandle()))  // always re-get the FIRST attribute, since
                                    // we're removing from this list
  {
    ...
    if (IsArrayType(pAttribute->GetType())) {
      CDmrElementArray<> array(pAttribute);
      int j = array.Find(pElement);
      Assert(j != array.InvalidIndex());
      if (bPreserveOrder) {
        array.Remove(j);
      } else {
        array.FastRemove(j);
