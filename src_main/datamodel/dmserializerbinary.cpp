...
class CDmSerializerBinary : public IDmSerializer {
 public:
  ...
  virtual bool Unserialize(CUtlBuffer &buf, const char *pEncodingName,
                           ...
                           int nFormatVersion, DmFileId_t fileid,
                           DmConflictResolution_t idConflictResolution,
                           CDmElement **ppRoot);

 private:
  // Methods related to serialization
  void SerializeElementIndex(CUtlBuffer &buf,
                             CDmElementSerializationDictionary &list,
  ...
  bool SerializeAttributes(CUtlBuffer &buf,
                           ...
                           CDmElement *pElement);
  bool SaveElementDict(CUtlBuffer &buf, unsigned short *symbolToIndexMap,
                       CDmElement *pElement);
  bool SaveElement(CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
                   unsigned short *symbolToIndexMap, CDmElement *pElement);

  // Methods related to unserialization
  DmElementHandle_t UnserializeElementIndex(
      CUtlBuffer &buf, CUtlVector<CDmElement *> &elementList);
  void UnserializeElementAttribute(CUtlBuffer &buf, CDmAttribute *pAttribute,
                                   CUtlVector<CDmElement *> &elementList);
  void UnserializeElementArrayAttribute(CUtlBuffer &buf,
                                        CDmAttribute *pAttribute,
                                        CUtlVector<CDmElement *> &elementList);
  bool UnserializeAttributes(CUtlBuffer &buf, CDmElement *pElement,
                             CUtlVector<CDmElement *> &elementList,
                             UtlSymId_t *symbolTable);
  bool UnserializeElements(CUtlBuffer &buf, DmFileId_t fileid,
                           DmConflictResolution_t idConflictResolution,
                           CDmElement **ppRoot, UtlSymId_t *symbolTable);
};
...
//-----------------------------------------------------------------------------
void CDmSerializerBinary::SerializeElementIndex(
    CUtlBuffer &buf, CDmElementSerializationDictionary &list,
    DmElementHandle_t hElement, DmFileId_t fileid) {
  if (hElement == DMELEMENT_HANDLE_INVALID) {
    ...
    if (pElement) {
      ...
      DmElementReference_t *pRef =
          g_pDataModelImp->FindElementReference(hElement, &pId);
      if (pRef && pId) {
        buf.PutInt(ELEMENT_INDEX_EXTERNAL);
        char idstr[40];
...
//-----------------------------------------------------------------------------
void CDmSerializerBinary::SerializeElementAttribute(
    CUtlBuffer &buf, CDmElementSerializationDictionary &list,
    CDmAttribute *pAttribute) {
  SerializeElementIndex(buf, list, pAttribute->GetValue<DmElementHandle_t>(),
                        pAttribute->GetOwner()->GetFileId());
}

//-----------------------------------------------------------------------------
// Writes out element array attributes
//-----------------------------------------------------------------------------
void CDmSerializerBinary::SerializeElementArrayAttribute(
    CUtlBuffer &buf, CDmElementSerializationDictionary &list,
    CDmAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
bool CDmSerializerBinary::SerializeAttributes(
    CUtlBuffer &buf, CDmElementSerializationDictionary &list,
    unsigned short *symbolToIndexMap, CDmElement *pElement) {
...
bool CDmSerializerBinary::SaveElement(CUtlBuffer &buf,
                                      CDmElementSerializationDictionary &list,
                                      unsigned short *symbolToIndexMap,
                                      CDmElement *pElement) {
...
bool CDmSerializerBinary::SaveElementDict(CUtlBuffer &buf,
                                          unsigned short *symbolToIndexMap,
                                          CDmElement *pElement) {
...
void MarkSymbol(UtlSymId_t *indexToSymbolMap, unsigned short *symbolToIndexMap,
                unsigned short &nSymbols, UtlSymId_t sym) {
  if (symbolToIndexMap[sym] != 0xffff) return;
...
void MarkSymbols(UtlSymId_t *indexToSymbolMap, unsigned short *symbolToIndexMap,
                 unsigned short &nSymbols, CDmElement *pElement) {
  MarkSymbol(indexToSymbolMap, symbolToIndexMap, nSymbols, pElement->GetType());
  for (CDmAttribute *pAttr = pElement->FirstAttribute(); pAttr;
       pAttr = pAttr->NextAttribute()) {
    MarkSymbol(indexToSymbolMap, symbolToIndexMap, nSymbols,
               pAttr->GetNameSymbol());
  }
}
...
bool CDmSerializerBinary::Serialize(CUtlBuffer &outBuf, CDmElement *pRoot) {
  ...
  // TODO - consider allowing dmxconvert to skip the collection step, since the
  ...
  for (i = dict.FirstRootElement(); i != ELEMENT_DICT_HANDLE_INVALID;
    ...
    MarkSymbols(indexToSymbolMap, symbolToIndexMap, nUsedSymbols,
                dict.GetRootElement(i));
  }
  Assert(nUsedSymbols <= nTotalSymbols);

  // write out the symbol table for this file (may be significantly smaller than
  // datamodel's full symbol table)
  ...
  // First write out the dictionary of all elements (to avoid later stitching up
  // in unserialize)
...
//-----------------------------------------------------------------------------
DmElementHandle_t CDmSerializerBinary::UnserializeElementIndex(
    CUtlBuffer &buf, CUtlVector<CDmElement *> &elementList) {
...
//-----------------------------------------------------------------------------
void CDmSerializerBinary::UnserializeElementAttribute(
...
//-----------------------------------------------------------------------------
void CDmSerializerBinary::UnserializeElementArrayAttribute(
    CUtlBuffer &buf, CDmAttribute *pAttribute,
    CUtlVector<CDmElement *> &elementList) {
...
//-----------------------------------------------------------------------------
bool CDmSerializerBinary::UnserializeAttributes(
    CUtlBuffer &buf, CDmElement *pElement,
    CUtlVector<CDmElement *> &elementList, UtlSymId_t *symbolTable) {
  char nameBuf[1024];
  ...
  for (int i = 0; i < nAttributeCount; ++i) {
    ...
    CDmAttribute *pAttribute =
        pElement ? pElement->AddAttribute(pName, nAttributeType) : NULL;
    if (pElement && !pAttribute) {
      Warning(
          "Dm: Attempted to read an attribute (\"%s\") of an inappropriate "
          "type!\n",
          pName);
      return false;
    }
...
DmElementHandle_t CreateElementWithFallback(const char *pType,
                                            const char *pName,
                                            DmFileId_t fileid,
                                            const DmObjectId_t &id) {
  DmElementHandle_t hElement =
      g_pDataModel->CreateElement(pType, pName, fileid, &id);
  if (hElement == DMELEMENT_HANDLE_INVALID) {
...
bool CDmSerializerBinary::Unserialize(
  ...
  if (V_stricmp(pEncodingName, GetName()) != 0) return false;

  Assert(nEncodingVersion >= 0 && nEncodingVersion <= 2);
  if (nEncodingVersion < 0 || nEncodingVersion > 2) return false;
...
bool CDmSerializerBinary::UnserializeElements(
    ...
    if (hExistingElement != DMELEMENT_HANDLE_INVALID) {
      ...
      if (idConflictResolution == CR_DELETE_NEW) {
        ...
        g_pDataModelImp->DeleteElement(
            hExistingElement, HR_NEVER);  // keep the handle around until
        ...
        hElement =
            CreateElementWithFallback(pType, pName, fileid, idpair.m_newId);
      } else
        Assert(0);
    }
  ...
  *ppRoot = elementList[0];
  ...
  for (int i = 0; i < nElementCount; ++i) {
    ...
    UnserializeAttributes(buf,
                          pInternal->GetFileId() == fileid ? pInternal : NULL,
                          elementList, symbolTable);
  }

  for (int i = 0; i < nElementCount; ++i) {
    CDmElement *pElement = elementList[i];
    if (pElement->GetFileId() == fileid) {
      // mark all unserialized elements as done unserializing, and call
      // Resolve()
