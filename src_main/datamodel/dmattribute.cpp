...
the_interface IDmAttributeOp {
 public:
  ...
  virtual void SetValue(CDmAttribute * pAttribute, DmAttributeType_t valueType,
                        const void *pValue) = 0;
  virtual void SetMultiple(CDmAttribute * pAttribute, int i, int nCount,
                           DmAttributeType_t valueType, const void *pValue) = 0;
  virtual void Set(CDmAttribute * pAttribute, int i,
                   DmAttributeType_t valueType, const void *pValue) = 0;
  ...
  virtual bool SerializeElement(const CDmAttribute *pAttribute, int nElement,
                                CUtlBuffer &buf) = 0;
  virtual bool UnserializeElement(CDmAttribute * pAttribute,
                                  CUtlBuffer & buf) = 0;
  virtual bool UnserializeElement(CDmAttribute * pAttribute, int nElement,
                                  CUtlBuffer &buf) = 0;
...
class CDmAttributeOp : public IDmAttributeOp {
 public:
  ...
  virtual void SetValue(CDmAttribute *pAttribute, DmAttributeType_t valueType,
                        const void *pValue);
  virtual void SetMultiple(CDmAttribute *pAttribute, int i, int nCount,
                           DmAttributeType_t valueType, const void *pValue);
  virtual void Set(CDmAttribute *pAttribute, int i, DmAttributeType_t valueType,
                   const void *pValue);
...
//-----------------------------------------------------------------------------
CMemoryPool g_DataAlloc4(sizeof(CDmAttribute), 4, CMemoryPool::GROW_SLOW,
                         "4-byte data pool");
CMemoryPool g_DataAlloc8(sizeof(CDmAttribute), 8, CMemoryPool::GROW_SLOW,
                         "8-byte data pool");
...
#define USE_SPECIAL_ALLOCATOR(_className, _allocator)                          \
  template <>                                                                  \
  void *NewData<_className>() {                                                \
    void *pData =                                                              \
        _allocator.Alloc(sizeof(CDmAttributeInfo<_className>::StorageType_t)); \
    return ::new (pData) CDmAttributeInfo<_className>::StorageType_t();        \
  }                                                                            \
  template <>                                                                  \
  void DeleteData<_className>(void *pData) {                                   \
...
template <>
void *CDmAttributeOp<DmUnknownAttribute_t>::CreateAttributeData() {
...
template <class T>
void CDmAttributeOp<T>::SetMultiple(CDmAttribute *pAttribute, int i, int nCount,
...
template <class T>
void CDmAttributeOp<T>::Set(CDmAttribute *pAttribute, int i,
                            DmAttributeType_t valueType, const void *pValue) {
  Assert(0);
}

template <class T>
void CDmAttributeOp<T>::SetValue(CDmAttribute *pAttribute,
                                 DmAttributeType_t valueType,
                                 const void *pValue) {
...
template <>
void CDmAttributeOp<int>::SetValue(CDmAttribute *pAttribute,
                                   DmAttributeType_t valueType,
                                   const void *pValue) {
  switch (valueType) {
    SET_VALUE_TYPE(int);
...
template <>
void CDmAttributeOp<float>::SetValue(CDmAttribute *pAttribute,
                                     DmAttributeType_t valueType,
                                     const void *pValue) {
  switch (valueType) {
    SET_VALUE_TYPE(int);
...
template <>
void CDmAttributeOp<bool>::SetValue(CDmAttribute *pAttribute,
                                    DmAttributeType_t valueType,
                                    const void *pValue) {
  switch (valueType) {
    SET_VALUE_TYPE(int);
...
template <>
void CDmAttributeOp<QAngle>::SetValue(CDmAttribute *pAttribute,
                                      DmAttributeType_t valueType,
                                      const void *pValue) {
  switch (valueType) {
...
template <>
void CDmAttributeOp<Quaternion>::SetValue(CDmAttribute *pAttribute,
                                          DmAttributeType_t valueType,
                                          const void *pValue) {
  switch (valueType) {
    SET_VALUE_TYPE(QAngle);
...
template <class T>
bool CDmAttributeOp<T>::Serialize(const CDmAttribute *pAttribute,
                                  CUtlBuffer &buf) {
...
template <class T>
bool CDmAttributeOp<T>::SerializeElement(const CDmAttribute *pData,
                                         int nElement, CUtlBuffer &buf) {
  Assert(0);
  return false;
}

template <class T>
bool CDmAttributeOp<T>::UnserializeElement(CDmAttribute *pData,
                                           CUtlBuffer &buf) {
  Assert(0);
  return false;
}
...
template <class T>
bool CDmAttributeOp<T>::UnserializeElement(CDmAttribute *pData, int nElement,
                                           CUtlBuffer &buf) {
  Assert(0);
  return false;
}
...
class CDmArrayAttributeOp : public CDmAttributeOp<CUtlVector<T> > {
 ...
 public:
  ...
  virtual void Set(CDmAttribute *pAttribute, int i, DmAttributeType_t valueType,
                   const void *pValue);
  virtual bool Unserialize(CDmAttribute *pAttribute, CUtlBuffer &buf);
  virtual bool SerializeElement(const CDmAttribute *pData, int nElement,
                                CUtlBuffer &buf);
  virtual bool UnserializeElement(CDmAttribute *pData, CUtlBuffer &buf);
  virtual bool UnserializeElement(CDmAttribute *pData, int nElement,
                                  CUtlBuffer &buf);
  virtual void OnUnserializationFinished(CDmAttribute *pAttribute);

  // Other methods used by CDmaArrayBase
  CDmArrayAttributeOp() : m_pAttribute(NULL), m_pData(NULL) {}
  CDmArrayAttributeOp(CDmAttribute *pAttribute)
      : m_pAttribute(pAttribute),
        m_pData((D *)m_pAttribute->GetAttributeData()) {}
  ...
  void OnAttributeArrayElementAdded(int nFirstElem, int nLastElem,
                                    bool bUpdateElementReferences = true);
...
class CUndoAttributeRenameElement : public CUndoElement {
 ...
 public:
  CUndoAttributeRenameElement(CDmAttribute *pAttribute, const char *newName)
    ...
    Assert(pAttribute->GetOwner() &&
           pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
  ...
  virtual void Undo() {
    ...
    if (pOwner) {
      pOwner->RenameAttribute(m_symAttributeNew.String(),
                              m_symAttributeOld.String());
    }
  }
  ...
  virtual void Redo() {
    CDmElement *pOwner = GetOwner();
    if (pOwner) {
      pOwner->RenameAttribute(m_symAttributeOld.String(),
                              m_symAttributeNew.String());
    }
  }
  ...
  virtual const char *GetDesc() {
    ...
    Q_snprintf(buf, sizeof(buf), "%s (%s -> %s)", base,
               m_symAttributeOld.String(), m_symAttributeNew.String());
    return buf;
  }
...
class CUndoAttributeSetValueElement : public CUndoElement {
 ...
 public:
  ...
  virtual const char *GetDesc() {
    ...
    Q_snprintf(buf, sizeof(buf), "%s(%s) = %s", base,
               g_pDataModel->GetString(m_symAttribute),
               serialized.Base() ? serialized.Base() : "\"\"");
    return buf;
  }
...
class CUndoAttributeArrayBase : public CUndoElement {
 ...
 public:
  CUndoAttributeArrayBase(CDmAttribute *pAttribute, const char *pUndoName)
      : BaseClass(pUndoName) {
    m_hOwner = pAttribute->GetOwner()->GetHandle();
    m_symAttribute = pAttribute->GetNameSymbol();
  }
 ...
 protected:
  typedef
      typename CDmAttributeUndoStorageType<T>::UndoStorageType StorageType_t;
  ...
  CDmAttribute *GetAttribute() {
    ...
    CDmElement *pOwner = GetOwner();
    if (pOwner) return pOwner->GetAttribute(pAttributeName);
    Assert(0);
    return NULL;
  }
...
class CUndoArrayAttributeSetValueElement : public CUndoAttributeArrayBase<T> {
 ...
 public:
  CUndoArrayAttributeSetValueElement(CDmAttribute *pAttribute, int slot,
                                     const T &newValue)
...
//-----------------------------------------------------------------------------
template <class T>
class CUndoArrayAttributeSetMultipleValueElement
    : public CUndoAttributeArrayBase<T> {
  typedef CUndoAttributeArrayBase<T> BaseClass;

 public:
  CUndoArrayAttributeSetMultipleValueElement(CDmAttribute *pAttribute,
                                             int nSlot, int nCount,
                                             const T *pNewValue)
    ...
    Assert(pAttribute->GetOwner() &&
           pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
    m_pOldValue =
        new typename CUndoAttributeArrayBase<T>::StorageType_t[nCount];
  ...
  ~CUndoArrayAttributeSetMultipleValueElement() {
    ...
    // member template support (ie otherwise I'd just create a
    // CUndoArrayAttributeSetMultipleValueElement< DmElementHandle_t,BaseClass>
    // version with this code) anyways, the casting hackiness only happens when
    // the value is actually a DmElementHandle_t, so it's completely safe
...
class CUndoAttributeArrayInsertBefore : public CUndoAttributeArrayBase<T> {
 ...
 public:
  CUndoAttributeArrayInsertBefore(CDmAttribute *pAttribute, int slot,
                                  int count = 1)
...
class CUndoAttributeArrayInsertCopyBefore : public CUndoAttributeArrayBase<T> {
 ...
 public:
  CUndoAttributeArrayInsertCopyBefore(CDmAttribute *pAttribute, int slot,
                                      const T &newValue)
...
class CUndoAttributeArrayRemoveElement : public CUndoAttributeArrayBase<T> {
 ...
 public:
  CUndoAttributeArrayRemoveElement(CDmAttribute *pAttribute, bool fastRemove,
                                   int elem, int count)
      : BaseClass(pAttribute, "CUndoAttributeArrayRemoveElement"),
        m_bFastRemove(fastRemove),
        m_nIndex(elem),
        m_nCount(count) {
    Assert(pAttribute->GetOwner() &&
           pAttribute->GetOwner()->GetFileId() != DMFILEID_INVALID);
    ...
    if (array.IsValid()) {
      if (m_bFastRemove) {
        ...
        if (array.Count() > m_nIndex) {
          // Get value at previous index (it was moved down from the "end"
          // before
  ...
  virtual const char *GetDesc() {
    ...
    const char *base = BaseClass::GetDesc();
    Q_snprintf(buf, sizeof(buf), "%s (%s) = remove( pos %i, count %i )", base,
               this->GetAttributeName(), m_nIndex, m_nCount);
    return buf;
  }
...
class CUndoAttributeArrayCopyAllElement : public CUndoAttributeArrayBase<T> {
 ...
 public:
  CUndoAttributeArrayCopyAllElement(CDmAttribute *pAttribute,
                                    const T *pNewValues, int nNewSize,
                                    bool purgeOnRemove = false)
    ...
    if (pNewValues != NULL && nNewSize > 0) {
      m_pNewValues =
          new typename CUndoAttributeArrayBase<T>::StorageType_t[nNewSize];
    ...
    if (pOldValues != NULL && nOldSize > 0) {
      m_pOldValues =
          new typename CUndoAttributeArrayBase<T>::StorageType_t[nOldSize];
...
//-----------------------------------------------------------------------------
template <class T>
void CDmArrayAttributeOp<T>::OnAttributeArrayElementAdded(
    int nFirstElem, int nLastElem, bool bUpdateElementReferences) {
  CDmElement *pOwner = m_pAttribute->GetOwner();
  if (m_pAttribute->IsFlagSet(FATTRIB_HAS_ARRAY_CALLBACK) &&
      !CDmeElementAccessor::IsBeingUnserialized(pOwner)) {
...
template <>
inline void
CDmArrayAttributeOp<DmElementHandle_t>::OnAttributeArrayElementAdded(
    int nFirstElem, int nLastElem, bool bUpdateElementReferences) {
  CDmElement *pOwner = m_pAttribute->GetOwner();
  if (m_pAttribute->IsFlagSet(FATTRIB_HAS_ARRAY_CALLBACK) &&
      !CDmeElementAccessor::IsBeingUnserialized(pOwner)) {
...
template <class T>
void CDmArrayAttributeOp<T>::OnAttributeArrayElementRemoved(int nFirstElem,
                                                            int nLastElem) {
  CDmElement *pOwner = m_pAttribute->GetOwner();
  if (m_pAttribute->IsFlagSet(FATTRIB_HAS_ARRAY_CALLBACK) &&
      !CDmeElementAccessor::IsBeingUnserialized(pOwner)) {
...
template <>
void CDmArrayAttributeOp<DmElementHandle_t>::OnAttributeArrayElementRemoved(
...
template <>
inline bool CDmArrayAttributeOp<DmElementHandle_t>::ShouldInsertElement(
  ...
  if (m_pAttribute->IsFlagSet(FATTRIB_NODUPLICATES)) {
    ...
    if (idx != Data().InvalidIndex()) return false;
  }
...
inline bool CDmArrayAttributeOp<T>::ShouldInsert(const T &src) {
  if (!ShouldInsertElement(src)) return false;
...
int CDmArrayAttributeOp<T>::InsertBefore(int elem, const T &src) {
  if (!ShouldInsert(src)) return Data().InvalidIndex();

  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayInsertCopyBefore<T> *pUndo =
        new CUndoAttributeArrayInsertCopyBefore<T>(m_pAttribute, elem, src);
...
int CDmArrayAttributeOp<T>::InsertMultipleBefore(int elem, int num) {
  if (!m_pAttribute->MarkDirty()) return Data().InvalidIndex();

  // UNDO HOOK
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayInsertBefore<T> *pUndo =
        new CUndoAttributeArrayInsertBefore<T>(m_pAttribute, elem, num);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
void CDmArrayAttributeOp<T>::FastRemove(int elem) {
  ...
  // UNDO HOOK
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayRemoveElement<T> *pUndo =
        new CUndoAttributeArrayRemoveElement<T>(m_pAttribute, true, elem, 1);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
void CDmArrayAttributeOp<T>::Remove(int elem) {
  if (!Data().IsValidIndex(elem)) return;
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayRemoveElement<T> *pUndo =
        new CUndoAttributeArrayRemoveElement<T>(m_pAttribute, false, elem, 1);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
void CDmArrayAttributeOp<T>::RemoveAll() {
  ...
  // UNDO HOOK
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayCopyAllElement<T> *pUndo =
        new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, NULL, 0);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
void CDmArrayAttributeOp<T>::RemoveMultiple(int elem, int num) {
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayRemoveElement<T> *pUndo =
        new CUndoAttributeArrayRemoveElement<T>(m_pAttribute, false, elem, num);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
void CDmArrayAttributeOp<T>::Purge() {
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayCopyAllElement<T> *pUndo =
        new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, NULL, true);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
template <>
void CDmArrayAttributeOp<DmElementHandle_t>::PerformCopyArray(
    const DmElementHandle_t *pArray, int nCount) {
  Data().RemoveAll();
  for (int i = 0; i < nCount; ++i) {
...
void CDmArrayAttributeOp<T>::CopyArray(const T *pArray, int nCount) {
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayCopyAllElement<T> *pUndo =
        new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, pArray, nCount);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
void CDmArrayAttributeOp<T>::SwapArray(CUtlVector<T> &src) {
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoAttributeArrayCopyAllElement<T> *pUndo =
        new CUndoAttributeArrayCopyAllElement<T>(m_pAttribute, src.Base(),
                                                 src.Count());
    g_pDataModel->AddUndoElement(pUndo);
  }
...
template <>
void CDmArrayAttributeOp<DmElementHandle_t>::SwapArray(
    CUtlVector<DmElementHandle_t> &src) {
  // This feature doesn't work for elements..
  // Can't do it owing to typesafety reasons as well as supporting the
  // NODUPLICATES feature.
  Assert(0);
}
...
void CDmArrayAttributeOp<T>::Set(int i, const T &value) {
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoArrayAttributeSetValueElement<T> *pUndo =
        new CUndoArrayAttributeSetValueElement<T>(m_pAttribute, i, value);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
template <class T>
void CDmArrayAttributeOp<T>::Set(CDmAttribute *pAttribute, int i,
                                 DmAttributeType_t valueType,
                                 const void *pValue) {
...
void CDmArrayAttributeOp<T>::SetMultiple(int i, int nCount, const T *pValue) {
  ...
  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoArrayAttributeSetMultipleValueElement<T> *pUndo =
        new CUndoArrayAttributeSetMultipleValueElement<T>(m_pAttribute, i,
                                                          nCount, pValue);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
template <class T>
void CDmArrayAttributeOp<T>::SetMultiple(CDmAttribute *pAttribute, int i,
                                         int nCount,
                                         DmAttributeType_t valueType,
                                         const void *pValue) {
  if (valueType == ArrayTypeToValueType(pAttribute->GetType())) {
...
//-----------------------------------------------------------------------------
template <class T>
void CDmArrayAttributeOp<T>::SetValue(CDmAttribute *pAttribute,
                                      DmAttributeType_t valueType,
                                      const void *pValue) {
  Assert(pAttribute->GetType() == valueType);
  if (pAttribute->GetType() == valueType) {
    CDmArrayAttributeOp<T> accessor(pAttribute);
    const CUtlVector<T> *pArray =
        reinterpret_cast<const CUtlVector<T> *>(pValue);
...
void CDmArrayAttributeOp<T>::Swap(int i, int j) {
  ...
  T vk = Data()[i];
  if (IsAttributeEqual(vk, Data()[j])) return;

  if (!m_pAttribute->MarkDirty()) return;

  if (g_pDataModel->UndoEnabledForElement(m_pAttribute->GetOwner())) {
    CUndoArrayAttributeSetValueElement<T> *pUndo =
        new CUndoArrayAttributeSetValueElement<T>(m_pAttribute, i, Data()[j]);
    g_pDataModel->AddUndoElement(pUndo);
...
//-----------------------------------------------------------------------------
template <class T>
bool CDmArrayAttributeOp<T>::Unserialize(CDmAttribute *pAttribute,
                                         CUtlBuffer &buf) {
  if (!pAttribute->MarkDirty()) return false;
...
template <>
bool CDmArrayAttributeOp<DmElementHandle_t>::Unserialize(
    CDmAttribute *pAttribute, CUtlBuffer &buf) {
  // Need to specialize this because element handles can't use SwapArray
  // because it's incapable of doing type safety checks or looking for
...
template <class T>
bool CDmArrayAttributeOp<T>::SerializeElement(const CDmAttribute *pAttribute,
                                              int nElement, CUtlBuffer &buf) {
...
template <class T>
bool CDmArrayAttributeOp<T>::UnserializeElement(CDmAttribute *pAttribute,
                                                CUtlBuffer &buf) {
  if (!CDmAttributeAccessor::MarkDirty(pAttribute)) return false;
...
bool CDmArrayAttributeOp<T>::UnserializeElement(CDmAttribute *pAttribute,
  ...
  CDmrArray<T> array(pAttribute);
  if (array.Count() <= nElement) return false;
...
template <class T>
void CDmArrayAttributeOp<T>::OnUnserializationFinished(
...
//-----------------------------------------------------------------------------
CMemoryPool g_AttrAlloc(sizeof(CDmAttribute), 32, CMemoryPool::GROW_SLOW,
                        "CDmAttribute pool");
...
CDmAttribute *CDmAttribute::CreateExternalAttribute(CDmElement *pOwner,
                                                    DmAttributeType_t type,
                                                    const char *pAttributeName,
  ...
  switch (type) {
    ...
    default: {
      void *pMem = g_AttrAlloc.Alloc(sizeof(CDmAttribute));
      return ::new (pMem)
          CDmAttribute(pOwner, type, pAttributeName, pExternalMemory);
    }
  }
}
...
//-----------------------------------------------------------------------------
CDmAttribute::CDmAttribute(CDmElement *pOwner, DmAttributeType_t type,
                           const char *pAttributeName)
    : m_pData(NULL) {
...
CDmAttribute::~CDmAttribute() {
  switch (GetType()) {
    case AT_ELEMENT:
      g_pDataModelImp->OnElementReferenceRemoved(GetValue<DmElementHandle_t>(),
                                                 this);
      break;
...
void CDmAttribute::SetElementTypeSymbol(UtlSymId_t typeSymbol) {
  switch (GetType()) {
    case AT_ELEMENT: {
      ...
      Assert(pData->m_Handle == DMELEMENT_HANDLE_INVALID ||
             ::IsA(pData->m_Handle, typeSymbol));
    ...
    case AT_ELEMENT_ARRAY: {
      ...
      if (array.GetElementType() != UTL_INVAL_SYMBOL) {
        ...
        int c = array.Count();
        for (i = 0; i < c; ++i) {
          Assert(array.GetHandle(i) == DMELEMENT_HANDLE_INVALID ||
                 ::IsA(array.GetHandle(i), typeSymbol));
        }
      }
#endif
...
bool CDmAttribute::ModificationAllowed() const {
  if (IsFlagSet(FATTRIB_READONLY)) return false;

  DmPhase_t phase = g_pDmElementFramework->GetPhase();
  if (phase == PH_EDIT) return true;
  if ((phase == PH_OPERATE) && !IsFlagSet(FATTRIB_TOPOLOGICAL)) return true;
...
void CDmAttribute::PreChanged() {
  if (IsFlagSet(FATTRIB_HAS_PRE_CALLBACK) &&
      !CDmeElementAccessor::IsBeingUnserialized(m_pOwner)) {
...
void CDmAttribute::OnChanged(bool bArrayCountChanged, bool bIsTopological) {
  ...
  if (bIsTopological || IsTopological(GetType())) {
    ...
    g_pDataModelImp->NotifyState(bArrayCountChanged
...
//-----------------------------------------------------------------------------
template <class T>
bool CDmAttribute::IsTypeConvertable() const {
  return (CDmAttributeInfo<T>::ATTRIBUTE_TYPE == GetType());
}

template <>
bool CDmAttribute::IsTypeConvertable<bool>() const {
  DmAttributeType_t type = GetType();
  return (type == AT_BOOL || type == AT_INT || type == AT_FLOAT);
}

template <>
bool CDmAttribute::IsTypeConvertable<int>() const {
  DmAttributeType_t type = GetType();
  return (type == AT_INT || type == AT_BOOL || type == AT_FLOAT);
}

template <>
bool CDmAttribute::IsTypeConvertable<float>() const {
  DmAttributeType_t type = GetType();
  return (type == AT_FLOAT || type == AT_INT || type == AT_BOOL);
}

template <>
bool CDmAttribute::IsTypeConvertable<QAngle>() const {
  DmAttributeType_t type = GetType();
  return (type == AT_QANGLE || type == AT_QUATERNION);
}

template <>
bool CDmAttribute::IsTypeConvertable<Quaternion>() const {
  DmAttributeType_t type = GetType();
  return (type == AT_QUATERNION || type == AT_QANGLE);
}

template <class T>
void CDmAttribute::CopyData(const T &value) {
  *reinterpret_cast<T *>(m_pData) = value;
}

template <class T>
void CDmAttribute::CopyDataOut(T &value) const {
  value = *reinterpret_cast<const T *>(m_pData);
}

template <>
void CDmAttribute::CopyData(const bool &value) {
  switch (GetType()) {
    case AT_BOOL:
...
template <>
void CDmAttribute::CopyDataOut(bool &value) const {
  switch (GetType()) {
    case AT_BOOL:
...
template <>
void CDmAttribute::CopyData(const int &value) {
  switch (GetType()) {
    case AT_BOOL:
...
template <>
void CDmAttribute::CopyDataOut(int &value) const {
  switch (GetType()) {
    case AT_BOOL:
...
template <>
void CDmAttribute::CopyData(const float &value) {
  switch (GetType()) {
    case AT_BOOL:
...
template <>
void CDmAttribute::CopyDataOut(float &value) const {
  switch (GetType()) {
    case AT_BOOL:
...
template <>
void CDmAttribute::CopyData(const QAngle &value) {
  switch (GetType()) {
    case AT_QANGLE:
...
template <>
void CDmAttribute::CopyDataOut(QAngle &value) const {
  switch (GetType()) {
    case AT_QANGLE:
...
template <>
void CDmAttribute::CopyData(const Quaternion &value) {
  switch (GetType()) {
    case AT_QANGLE: {
      QAngle aValue;
...
template <>
void CDmAttribute::CopyDataOut(Quaternion &value) const {
  switch (GetType()) {
    case AT_QANGLE:
...
template <>
void CDmAttribute::CopyData(const DmElementHandle_t &value) {
  g_pDataModelImp->OnElementReferenceRemoved(GetValue<DmElementHandle_t>(),
                                             this);
...
bool CDmAttribute::ShouldModify(const T &value) {
  if (!IsTypeConvertable<T>()) return false;

  if ((GetType() == CDmAttributeInfo<T>::ATTRIBUTE_TYPE) &&
      IsAttributeEqual(GetValue<T>(), value))
    return false;
...
template <>
bool CDmAttribute::ShouldModify(const DmElementHandle_t &value) {
  if (!IsTypeConvertable<DmElementHandle_t>()) return false;

  if (IsAttributeEqual(GetValue<DmElementHandle_t>(), value)) return false;

  DmElementAttribute_t *pData = GetData<DmElementHandle_t>();
  if (pData->m_ElementType != UTL_INVAL_SYMBOL &&
      !::IsA(value, pData->m_ElementType))
    return false;
...
void CDmAttribute::SetValue(const T &value) {
  if (!ShouldModify(value)) return;

  // UNDO Hook
  if (g_pDataModel->UndoEnabledForElement(m_pOwner)) {
    CUndoAttributeSetValueElement<T> *pUndo =
        new CUndoAttributeSetValueElement<T>(this, value);
    g_pDataModel->AddUndoElement(pUndo);
  }

  bool bIsBeingUnserialized =
      CDmeElementAccessor::IsBeingUnserialized(m_pOwner);
  ...
  g_pDataModelImp->NotifyState(IsTopological(GetType())
...
#define ATTRIBUTE_SET_VALUE_ARRAY(_type)                        \
  template <>                                                   \
  void CDmAttribute::SetValue(const CUtlVector<_type> &value) { \
...
void CDmAttribute::SetValue(const CDmAttribute *pAttribute) {
  s_pAttrInfo[GetType()]->SetValue(this, pAttribute->GetType(),
                                   pAttribute->GetAttributeData());
}
...
const char *CDmAttribute::GetValueAsString(char *pBuffer,
...
void CDmAttribute::SetName(const char *pNewName) {
  ...
  // UNDO Hook
  if (g_pDataModel->UndoEnabledForElement(m_pOwner)) {
    CUndoAttributeRenameElement *pUndo =
        new CUndoAttributeRenameElement(this, pNewName);
    g_pDataModel->AddUndoElement(pUndo);
  }
...
DmAttributeHandle_t CDmAttribute::GetHandle(bool bCreate) {
  ...
  Assert((m_Handle == DMATTRIBUTE_HANDLE_INVALID) ||
         g_pDataModel->IsAttributeHandleValid(m_Handle));
  return m_Handle;
}
...
int CDmAttribute::EstimateMemoryUsageInternal(
    CUtlHash<DmElementHandle_t> &visited, TraversalDepth_t depth,
  ...
  if (IsArrayType(GetType())) {
    ...
    nAttributeExtraDataSize =
        nCount * s_pAttrInfo[GetType()]->ValueSize();  // Data in the UtlVector
    int nMallocOverhead =
        (array.Count() == 0) ? 0 : 8;  // malloc overhead inside the vector
  ...
  if (pCategories) {
    ...
    pCategories[MEMORY_CATEGORY_ATTRIBUTE_DATA] +=
        nAttributeDataSize + nAttributeExtraDataSize;
  ...
  switch (GetType()) {
    case AT_STRING: {
      ...
      return nTotalMemory + value.Length() + 1 +
    ...
    case AT_VOID_ARRAY: {
      const CUtlVector<CUtlBinaryBlock> &array =
          GetValue<CUtlVector<CUtlBinaryBlock> >();
      for (int i = 0; i < array.Count(); ++i) {
        if (pCategories) {
    ...
    case AT_ELEMENT:
      if (ShouldTraverse(this, depth)) {
        ...
        if (pElement) {
          nTotalMemory += CDmeElementAccessor::EstimateMemoryUsage(
              pElement, visited, depth, pCategories);
        }
      }
      return nTotalMemory;
...
//-----------------------------------------------------------------------------
template <class T, class B>
void CDmaDecorator<T, B>::Init(CDmElement *pOwner, const char *pAttributeName,
                               int nFlags) {
  Assert(pOwner);
...
void CDmrDecoratorConst<T, BaseClass>::Init(const CDmAttribute *pAttribute) {
  if (pAttribute && pAttribute->GetType() ==
                        CDmAttributeInfo<CUtlVector<T> >::AttributeType()) {
...
template <class T, class BaseClass>
void CDmrDecoratorConst<T, BaseClass>::Init(const CDmElement *pElement,
                                            const char *pAttributeName) {
...
template <class T, class BaseClass>
void CDmrDecorator<T, BaseClass>::Init(CDmElement *pElement,
                                       const char *pAttributeName,
                                       bool bAddAttribute) {
  CDmAttribute *pAttribute = NULL;
  if (pElement && pAttributeName && pAttributeName[0]) {
    if (bAddAttribute) {
      pAttribute = pElement->AddAttribute(
          pAttributeName, CDmAttributeInfo<CUtlVector<T> >::AttributeType());
    } else {
      pAttribute = pElement->GetAttribute(pAttributeName);
    }
...
#define ARRAY_METHOD_VOID(_type, _func)                        \
  case CDmAttributeInfo<CUtlVector<_type> >::ATTRIBUTE_TYPE: { \
    CDmrArray<_type> &array =                                  \
        *reinterpret_cast<CDmrArray<_type> *>(&arrayShared);   \
...
CDmrGenericArrayConst::CDmrGenericArrayConst(const CDmElement *pElement,
                                             const char *pAttributeName) {
...
void CDmrGenericArrayConst::Init(const CDmElement *pElement,
                                 const char *pAttributeName) {
  const CDmAttribute *pAttribute =
      (pElement && pAttributeName && pAttributeName[0])
          ? pElement->GetAttribute(pAttributeName)
          : NULL;
  Init(pAttribute);
}
...
const char *CDmrGenericArrayConst::GetAsString(int i, char *pBuffer,
                                               size_t nBufLen) const {
  if ((Count() > i) && (i >= 0)) {
    CUtlBuffer buf(pBuffer, nBufLen, CUtlBuffer::TEXT_BUFFER);
...
CDmrGenericArray::CDmrGenericArray(CDmElement *pElement,
                                   const char *pAttributeName) {
  Init(pElement, pAttributeName);
}
...
void CDmrGenericArray::SetMultiple(int i, int nCount,
                                   DmAttributeType_t valueType,
                                   const void *pValue) {
  s_pAttrInfo[m_pAttribute->GetType()]->SetMultiple(m_pAttribute, i, nCount,
                                                    valueType, pValue);
}

void CDmrGenericArray::Set(int i, DmAttributeType_t valueType,
                           const void *pValue) {
...
//-----------------------------------------------------------------------------
// Skip unserialization for an attribute type (unserialize into a dummy
// variable)
//-----------------------------------------------------------------------------
bool SkipUnserialize(CUtlBuffer &buf, DmAttributeType_t type) {
  if (type == AT_UNKNOWN) return false;
...
#define INSTANTIATE_GENERIC_OPS(_className)                                    \
  template <>                                                                  \
  class CInstantiateOp<CUtlVector<_className> > {                              \
   public:                                                                     \
    CInstantiateOp() {                                                         \
      s_pAttrInfo[CDmAttributeInfo<CUtlVector<_className> >::ATTRIBUTE_TYPE] = \
          new CDmArrayAttributeOp<_className>;                                 \
    }                                                                          \
  };                                                                           \
...
#define DEFINE_ATTRIBUTE_TYPE(_type)                                          \
  ...
  template class CDmaArrayConstBase<_type,                                    \
                                    CDmaDataInternal<CUtlVector<_type> > >;   \
  template class CDmaArrayConstBase<_type,                                    \
                                    CDmaDataExternal<CUtlVector<_type> > >;   \
  template class CDmaDecorator<                                               \
      _type, CDmaArrayBase<_type, CDmaDataInternal<CUtlVector<_type> > > >;   \
  template class CDmrDecorator<                                               \
      _type, CDmaArrayBase<_type, CDmaDataExternal<CUtlVector<_type> > > >;   \
  template class CDmrDecoratorConst<                                          \
      _type,                                                                  \
      CDmaArrayConstBase<_type, CDmaDataExternal<CUtlVector<_type> > > >;
