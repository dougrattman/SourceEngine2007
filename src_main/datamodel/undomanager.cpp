...
bool CUndoManager::InstallNotificationCallback(IDmNotify *pNotify) {
  if (m_Notifiers.Find(pNotify) >= 0) return false;
...
bool CUndoManager::IsSuppressingNotify() const { return m_bSuppressingNotify; }
...
void CUndoManager::EnableUndo() { m_bEnabled = true; }

void CUndoManager::DisableUndo() { m_bEnabled = false; }

bool CUndoManager::HasUndoData() const { return m_UndoList.Count() != 0; }

bool CUndoManager::UndoDataDiscarded() const { return m_bDiscarded; }

bool CUndoManager::HasRedoData() const { return m_RedoStack.Count() > 0; }

void CUndoManager::PushNotificationScope(const char *pReason, int nNotifySource,
                                         int nNotifyFlags) {
  ...
  if (m_nNotifyNesting == 0) {
    if (!m_bSuppressingNotify && ((m_nNotifyFlags & NOTIFY_CHANGE_MASK) != 0)) {
      ...
      for (int i = 0; i < nNotifyCount; ++i) {
        m_Notifiers[i]->NotifyDataChanged(m_pNotifyReason, m_nNotifySource,
                                          m_nNotifyFlags);
      }
    }
    m_nNotifySource = 0;
...
void CUndoManager::PushUndo(const char *udesc, const char *rdesc,
                            int nChainingID) {
  if (!IsEnabled()) return;
  ...
  if (m_nNesting++ == 0) {
    ...
    m_RedoDesc =
        (udesc == rdesc) ? m_UndoDesc : s_UndoSymbolTable.AddString(rdesc);
    m_bStreamStart = true;
    m_nItemsAddedSinceStartOfStream = 0;
  }
}
...
void CUndoManager::PushRedo() {
  ...
  Trace("[%d] Popping undo '%s'\n", m_nNesting,
        s_UndoSymbolTable.String(m_UndoDesc));
  ...
  if (m_nNesting == 0) {
    if (m_nItemsAddedSinceStartOfStream > 0) {
      ...
      // Accumulate this operation into the previous "undo" operation if there
      // is one
      if (m_nChainingID != 0 && m_PreviousChainingID == m_nChainingID) {
        // Walk undo list backward looking for previous end of stream and unmark
        // that indicator
...
void CUndoManager::Undo() {
  CNotifyScopeGuard notify("CUndoManager::Undo", NOTIFY_SOURCE_UNDO,
                           NOTIFY_SETDIRTYFLAG);
...
void CUndoManager::Redo() {
  CNotifyScopeGuard notify("CUndoManager::Redo", NOTIFY_SOURCE_UNDO,
                           NOTIFY_SETDIRTYFLAG);
...
UtlSymId_t CUndoManager::GetUndoDescInternal(const char *context) {
  if (m_nNesting <= 0) {
    ...
    if (s_DescErrors.Find(sym) == s_DescErrors.InvalidIndex()) {
      Warning(
          "CUndoManager::GetUndoDescInternal:  undoable operation missing "
          "CUndoScopeGuard in application\nContext( %s )\n",
          context);
...
UtlSymId_t CUndoManager::GetRedoDescInternal(const char *context) {
  if (m_nNesting <= 0) {
    // Warning( "CUndoManager::GetRedoDescInternal:  undoable operation missing
    // CUndoScopeGuard in application\nContext( %s )", context );
    return s_UndoSymbolTable.AddString(context);
  }
  return m_RedoDesc;
...
void CUndoManager::GetUndoInfo(CUtlVector<UndoInfo_t> &list) {
  ...
  int ops = 0;
  for (int i = m_UndoList.Tail(); i != m_UndoList.InvalidIndex();
       i = m_UndoList.Previous(i)) {
    ++ops;
    IUndoElement *action = m_UndoList[i];
    Assert(action);
    ...
    // This is a hack because GetDesc() returns a static char buf[] and so the
    // last one will clobber them all So we have the requester pass in a
    // temporary string table so we can get a ptr to a CUtlSymbol in the table
    // and use that.  Sigh.
...
void CUndoManager::TraceUndo(bool state) { m_bTrace = state; }
