...
//-----------------------------------------------------------------------------
BEGIN_CHAR_CONVERSION(s_XMLCharConversion, "", '&'){'&', "amp;"}, {'<', "lt;"},
    {'>', "gt;"}, {'\"', "quot;"}, {'\'', "apos;"},
 ...
 public:
  CXMLInputStringConversion(char nEscapeChar, const char *pDelimiter,
...
class CXMLSerializer : public IDmSerializer {
 public:
  ...
  virtual const char *GetName() const {
    return m_bFlatMode ? "xml_flat" : "xml";
  }
  virtual const char *GetDescription() const {
    return m_bFlatMode ? "XML (flat)" : "XML";
  }
  ...
  virtual bool Unserialize(CUtlBuffer &buf, const char *pEncodingName,
                           int nEncodingVersion, const char *pSourceFormatName,
                           int nSourceFormatVersion, DmFileId_t fileid,
                           DmConflictResolution_t idConflictResolution,
                           CDmElement **ppRoot);

 private:
  void SerializeElementReference(CUtlBuffer &buf,
                                 CDmElementSerializationDictionary &dict,
                                 CDmElement *pElement);
  void SerializeElementArrayAttribute(CUtlBuffer &buf,
                                      CDmElementSerializationDictionary &dict,
                                      const char *pName,
                                      CDmAttribute *pAttribute);
  void SerializeElementAttribute(CUtlBuffer &buf,
                                 CDmElementSerializationDictionary &dict,
                                 const char *pName, CDmAttribute *pAttribute);
  void SerializeArrayAttribute(CUtlBuffer &buf, CDmAttribute *pAttribute);
  bool SerializeAttributes(CUtlBuffer &buf,
                           CDmElementSerializationDictionary &dict,
                           CDmElement *pElement);
  bool SaveElement(CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
                   CDmElement *pElement, bool bWriteDelimiters = true);
  bool UnserializeElements(CUtlBuffer &buf, const char *pFormatName,
                           DmFileId_t fileid,
                           DmConflictResolution_t idConflictResolution,
                           CDmElement **ppRoot);
...
//-----------------------------------------------------------------------------
void CXMLSerializer::SerializeElementReference(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
    CDmElement *pElement) {
...
// Serializes a single element attribute
//-----------------------------------------------------------------------------
void CXMLSerializer::SerializeElementAttribute(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict, const char *pName,
    CDmAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
void CXMLSerializer::SerializeElementArrayAttribute(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict, const char *pName,
    CDmAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
void CXMLSerializer::SerializeArrayAttribute(CUtlBuffer &buf,
                                             CDmAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
bool CXMLSerializer::SerializeAttributes(
    CUtlBuffer &buf, CDmElementSerializationDictionary &dict,
    CDmElement *pElement) {
  // Collect the attributes to be written
  CDmAttribute **ppAttributes = (CDmAttribute **)_alloca(
      pElement->AttributeCount() * sizeof(CDmAttribute *));
  int nAttributes = 0;
  for (CDmAttribute *pAttribute = pElement->FirstAttribute(); pAttribute;
       pAttribute = pAttribute->NextAttribute()) {
    if (pAttribute->IsFlagSet(FATTRIB_DONTSAVE | FATTRIB_STANDARD)) continue;

    ppAttributes[nAttributes++] = pAttribute;
  }

  // Now write them all out in reverse order, since FirstAttribute is actually
  // the *last* attribute for perf reasons
...
//-----------------------------------------------------------------------------
bool CXMLSerializer::SaveElement(CUtlBuffer &buf,
                                 CDmElementSerializationDictionary &dict,
...
bool CXMLSerializer::Serialize(CUtlBuffer &outBuf, CDmElement *pRoot) {
  ...
  for (i = dict.FirstRootElement(); i != ELEMENT_DICT_HANDLE_INVALID;
       i = dict.NextRootElement(i)) {
...
class CXMLUnserializationState {
 public:
  ...
  bool Unserialize(DmFileId_t fileid, CUtlBuffer &buf,
                   DmConflictResolution_t idConflictResolution,
                   CDmElement **ppRoot);

  int StartElement(const XMLCH *uri, const XMLCH *localName, const char *qName,
                   LPXMLVECTOR atts);
 ...
 private:
  ...
  void PushAttribute(const char *pAttributeName, CDmAttribute *pAttribute,
                     DmElementDictHandle_t hElement,
                     ParseState_t::ParseType_t type);
  void PopAttribute();
  ...
  int StartAttributeElement(const char *pAttributeName,
                            const char *pAttributeType);
...
int StartElement(void *pUserData, const XMLCH *uri, const XMLCH *localName,
...
int EndElement(void *pUserData, const XMLCH *uri, const XMLCH *localName,
               const XMLCH *qName) {
  CXMLUnserializationState *pState = (CXMLUnserializationState *)pUserData;
...
//-----------------------------------------------------------------------------
bool CXMLUnserializationState::Unserialize(
    DmFileId_t fileid, CUtlBuffer &buf,
  ...
  *ppRoot = NULL;
  ...
  if (!bOk) {
    Warning("Error: %s\nLine: %d Col: %d\n", m_Parser->ErrorString,
            m_Parser->ErrorLine, m_Parser->ErrorColumn);
  }

  XMLParser_Free(m_Parser);

  if (bOk) {
    // do this *before* getting the root, since the first element might be
    // deleted due to id conflicts
    ...
    *ppRoot = m_ElementDict.GetElement(m_hRootElement);
    ...
    for (DmElementDictHandle_t h = m_ElementDict.FirstElement();
         h != ELEMENT_DICT_HANDLE_INVALID; h = m_ElementDict.NextElement(h)) {
...
inline CDmAttribute *CXMLUnserializationState::GetTopAttribute() {
  Assert(
      (m_ParseStack.Top().m_Type == ParseState_t::ATTRIBUTE) ||
...
CDmElement *CXMLUnserializationState::GetTopmostElement() {
  if (m_hTopmostDictHandle == ELEMENT_DICT_HANDLE_INVALID) return NULL;
...
void CXMLUnserializationState::PushAttribute(const char *pAttributeName,
                                             ...
                                             DmElementDictHandle_t hElement,
                                             ParseState_t::ParseType_t type) {
  Assert(type == ParseState_t::ATTRIBUTE ||
         type == ParseState_t::ATTRIBUTE_ARRAY_VALUE ||
         type == ParseState_t::ATTRIBUTE_ELEMENT_REFERENCE);
...
void CXMLUnserializationState::PopAttribute() {
  Assert(m_ParseStack.Top().m_Type == ParseState_t::ATTRIBUTE ||
         m_ParseStack.Top().m_Type == ParseState_t::ATTRIBUTE_ARRAY_VALUE ||
         m_ParseStack.Top().m_Type ==
             ParseState_t::ATTRIBUTE_ELEMENT_REFERENCE);
...
int CXMLUnserializationState::StartFileScopeElement(const char *pElementName) {
  ...
  if (Q_stricmp(pElementName, DMX_FILE_STARTING_TOKEN)) return XML_ABORT;
...
int CXMLUnserializationState::EndFileScopeElement(const char *pElementName) {
  // First thing in the file has to be the starting token
  if (Q_stricmp(pElementName, DMX_FILE_ENDING_TOKEN)) return XML_ABORT;
...
// Creates a scene object, adds it to the element dictionary
//-----------------------------------------------------------------------------
DmElementDictHandle_t CXMLUnserializationState::CreateDmElement(
  ...
  DmElementHandle_t hElement =
      g_pDataModel->CreateElement(pElementType, pName, m_fileid);
...
//-----------------------------------------------------------------------------
int CXMLUnserializationState::StartAttributeElement(
    const char *pAttributeName, const char *pAttributeType) {
  if (m_hTopmostDictHandle == ELEMENT_DICT_HANDLE_INVALID) {
    Warning(
        "XML: (%d) Tried to add attribute %s outside of an element definition "
        "scope\n",
  ...
  // If the requested type doesn't exist, then it must be an installed element
  // type.
  if (nAttrType == AT_UNKNOWN) {
    // See if we can create an element of that type
    DmElementDictHandle_t hElement = CreateDmElement(pAttributeType);
    if (hElement == ELEMENT_DICT_HANDLE_INVALID) return XML_ABORT;

    CDmAttribute *pAttribute =
        GetTopmostElement()->AddAttribute(pAttributeName, AT_ELEMENT);
    if (!pAttribute) {
    ...
    pAttribute->SetValue(pElement ? pElement->GetHandle()
                                  : DMELEMENT_HANDLE_INVALID);
    PushAttribute(pAttributeName, pAttribute, hElement,
                  ParseState_t::ATTRIBUTE);
    return XML_OK;
  }

  CDmAttribute *pAttribute =
      GetTopmostElement()->AddAttribute(pAttributeName, nAttrType);
  if (!pAttribute) {
    Warning("XML: (%d) Attribute %s is not requested type %s\n",
            GetCurrentLine(), pAttributeName, pAttributeType);
    return XML_ABORT;
  ...
  ParseState_t::ParseType_t type =
      (nAttrType != AT_ELEMENT) ? ParseState_t::ATTRIBUTE
                                : ParseState_t::ATTRIBUTE_ELEMENT_REFERENCE;
...
int CXMLUnserializationState::EndAttributeElement(const char *pAttributeName) {
  ...
  //Q_stricmp( pAttributeName, pActualAttributeName ) )
  //	{
  //		Warning("XML: (%d) Attribute (%s) has mismatched ending attribute
  //name %s\n", GetCurrentLine(), pAttributeName ); 		return XML_ABORT;
  //	}
...
int CXMLUnserializationState::StartChildElement(const char *pElementType) {
  // If the current top of the stack is an array attribute, then add it to the
  // array...
  bool bIsElementArray =
      (m_ParseStack.Top().m_Type == ParseState_t::ATTRIBUTE) &&
      (GetTopAttribute()->GetType() == AT_ELEMENT_ARRAY);

  // Look for an external child reference first
  if (!Q_stricmp(pElementType,
                 g_pDataModel->GetAttributeNameForType(AT_ELEMENT))) {
  ...
  if (hChild == ELEMENT_DICT_HANDLE_INVALID) return XML_ABORT;
...
int CXMLUnserializationState::EndChildElement(const char *pElementType) {
  if (m_ParseStack.Top().m_Type ==
      ParseState_t::ATTRIBUTE_ELEMENT_ARRAY_REFERENCE) {
    m_ParseStack.Pop();
    return XML_OK;
  }
  ...
  if (Q_stricmp(pElementType, pActualElementType)) {
    Warning(
        "XML: (%d) Child element (%s) has mismatched ending element type %s\n",
        GetCurrentLine(), pElementType);
    return XML_ABORT;
  }
...
int CXMLUnserializationState::StartElement(const XMLCH *uri,
  ...
  if (!m_ParseStack.Count()) return StartFileScopeElement(pElementName);

  // See if we're reading in an attribute. We are if there's an XML attribute
  // called 'type'.
  ...
  // We're reading in an array attribute if there's no XML attribute called
  // 'type'.

  // If the current top of the stack is an array attribute, then add it to the
  // array... To make this work, we need do
  if ((m_ParseStack.Top().m_Type == ParseState_t::ATTRIBUTE) &&
      (GetTopAttribute()->GetType() != AT_ELEMENT_ARRAY)) {
    PushAttribute(NULL, GetTopAttribute(), ELEMENT_DICT_HANDLE_INVALID,
                  ParseState_t::ATTRIBUTE_ARRAY_VALUE);
    return XML_OK;
  }
...
//-----------------------------------------------------------------------------
int CXMLUnserializationState::CharactersElementArrayReference(
    const XMLCH *pChars, int cbChars) {
  Assert(m_ParseStack.Count() > 1);
  ParseState_t &state = m_ParseStack[m_ParseStack.Count() - 2];
  Assert((state.m_Type == ParseState_t::ATTRIBUTE) ||
         (state.m_Type == ParseState_t::ATTRIBUTE_ELEMENT_REFERENCE));
...
// Called by the parser when we get characters inside an element array reference
//-----------------------------------------------------------------------------
int CXMLUnserializationState::CharactersElementReference(const XMLCH *pChars,
                                                         int cbChars) {
  DmObjectId_t id;
  UniqueIdFromString(&id, (const char *)pChars, cbChars);
...
//-----------------------------------------------------------------------------
int CXMLUnserializationState::CharactersAttribute(const XMLCH *pChars,
                                                  int cbChars) {
  // Not sure how to handle this one...
  CDmAttribute *pTopAttribute = GetTopAttribute();
  if (pTopAttribute->GetType() == AT_ELEMENT) {
    Warning(
        "XML (%d) : Error parsing attribute data (can't have non-attribute "
        "data in an element block)!\n",
        GetCurrentLine());
    Assert(0);
    return XML_ABORT;
  }
  ...
  if (pTopAttribute->GetType() >= AT_FIRST_ARRAY_TYPE) {
    ...
    Assert(V_stricmp(pTopAttribute->GetName(),
                     "id"));  // this shouldn't be legal - I don't know why this
                              // code was here before...
...
int CXMLUnserializationState::Characters(const XMLCH *pChars, int cbChars) {
  ...
  if (m_ParseStack.Count() == 0) return XML_OK;
...
//-----------------------------------------------------------------------------
int CXMLUnserializationState::EndElement(const XMLCH *uri,
                                         const XMLCH *localName,
                                         const XMLCH *qName) {
  switch (m_ParseStack.Top().m_Type) {
...
//-----------------------------------------------------------------------------
bool CXMLSerializer::Unserialize(CUtlBuffer &buf, const char *pEncodingName,
  ...
  bool bSuccess =
      unserializer.Unserialize(fileid, buf, idConflictResolution, ppRoot);
  if (!bSuccess) return false;

  return g_pDataModel->UpdateUnserializedElements(pSourceFormatName,
                                                  nSourceFormatVersion, fileid,
