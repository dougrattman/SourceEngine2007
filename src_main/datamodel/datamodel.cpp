...
class CDmElementFactoryDefault : public IDmElementFactory {
 public:
  ...
  virtual CDmElement *Create(DmElementHandle_t handle, const char *pElementType,
...
CDataModel::CDataModel()
    : m_elementIds(4096),
      m_unloadedIdElementMap(16, 0, 0, ElementIdHandlePair_t::Compare,
                             ElementIdHandlePair_t::HashKey) {
...
void CDataModel::Shutdown() {
  ...
  for (int i = g_typeHistogram.FirstInorder(); g_typeHistogram.IsValidIndex(i);
       i = g_typeHistogram.NextInorder(i)) {
    Msg("%d\t%s\n", g_typeHistogram.Element(i),
        GetString(g_typeHistogram.Key(i)));
  }
  Msg("\n");
#endif _ELEMENT_HISTOGRAM_
...
//-----------------------------------------------------------------------------
void CDataModel::SetUndoDepth(int nSize) { m_UndoMgr.SetUndoDepth(nSize); }
...
int CDataModel::GetMaxNumberOfElements() { return m_nMaxNumberOfElements; }
...
DmElementHandle_t CDataModel::FirstAllocatedElement() {
  ...
  for (int i = 0; i < nHandles; ++i) {
    DmElementHandle_t hElement =
        (DmElementHandle_t)m_Handles.GetHandleFromIndex(i);
    if (hElement != DMELEMENT_HANDLE_INVALID) return hElement;
  }
...
DmElementHandle_t CDataModel::NextAllocatedElement(DmElementHandle_t hElement) {
  ...
  for (int i = m_Handles.GetIndexFromHandle(hElement) + 1; i < nHandles; ++i) {
    DmElementHandle_t nextElement =
        (DmElementHandle_t)m_Handles.GetHandleFromIndex(i);
    if (nextElement != DMELEMENT_HANDLE_INVALID) return nextElement;
  }
...
int CDataModel::EstimateMemoryOverhead() const {
  ...
  int nElementIdsOverhead =
      sizeof(DmElementHandle_t);  // this also has a 80k static overhead, since
                                  // hash tables can't grow
  return nHandlesOverhead + nElementIdsOverhead;
}

static bool HandleCompare(const DmElementHandle_t &a,
                          const DmElementHandle_t &b) {
  return a == b;
}
...
int CDataModel::EstimateMemoryUsage(DmElementHandle_t hElement,
  ...
  return CDmeElementAccessor::EstimateMemoryUsage(pElement, visited, depth,
                                                  NULL);
}
...
void CDataModel::DisplayMemoryStats() {
  CUtlMap<UtlSymId_t, DmMemoryInfo_t> typeHistogram(0, 100,
                                                    DefLessFunc(UtlSymId_t));
  CUtlHash<DmElementHandle_t> visited(1024, 0, 0, HandleCompare, HandleHash);

  int c = (int)m_Handles.GetHandleCount();
  for (int i = 0; i < c; ++i) {
    DmElementHandle_t h = (DmElementHandle_t)m_Handles.GetHandleFromIndex(i);
    if (!m_Handles.IsHandleValid(h)) continue;
    ...
    if (!typeHistogram.IsValidIndex(j)) {
      ...
      memset(typeHistogram[j].m_pCategories, 0,
             sizeof(typeHistogram[j].m_pCategories));
    }

    int nMemory = CDmeElementAccessor::EstimateMemoryUsage(
        pElement, visited, TD_NONE, typeHistogram[j].m_pCategories);
  ...
  // Sort
  DmMemorySortInfo_t *pSortInfo = (DmMemorySortInfo_t *)_alloca(
      typeHistogram.Count() * sizeof(DmMemorySortInfo_t));
  int nCount = 0;
  for (int i = typeHistogram.FirstInorder(); typeHistogram.IsValidIndex(i);
       i = typeHistogram.NextInorder(i)) {
  ...
  ConMsg(
      "Dm Memory usage: type\t\t\t\tcount\ttotalsize\twastage "
      "%%\touter\t\tinner\t\tdatamodel\trefs\t\ttree\t\tatts\t\tdata\t(att "
      "count)\n");
  for (int i = 0; i < nCount; ++i) {
    const DmMemoryInfo_t &info = typeHistogram.Element(pSortInfo[i].m_nIndex);
    float flPercentOverhead =
        1.0f -
        ((info.m_nSize != 0)
             ? ((float)info.m_pCategories[MEMORY_CATEGORY_ATTRIBUTE_DATA] /
                (float)info.m_nSize)
             : 0.0f);
    flPercentOverhead *= 100.0f;

    ConMsg("%-40s\t%6d\t%9d\t\t%5.2f",
           GetString(typeHistogram.Key(pSortInfo[i].m_nIndex)), info.m_nCount,
           info.m_nSize, flPercentOverhead);
    int nTotal = 0;
...
unsigned short CDataModel::GetSymbolCount()
    const  // this can't ever overflow a ushort, since UtlSymId_t is a ushort,
           // and one of its entries is invalid (0xffff)
{
  return m_SymbolTable.GetNumStrings();  // this is only useful because symbols
                                         // are never removed
}
...
int CDataModel::GetFormatCount() const { return m_FormatUpdaters.Count(); }
...
void CDataModel::AddSerializer(IDmSerializer *pSerializer) {
  ...
  if (FindSerializer(pSerializer->GetName())) {
    Warning(
        "Attempted to add two serializers with the same file encoding (%s)!\n",
        pSerializer->GetName());
    return;
  }
...
void CDataModel::AddLegacyUpdater(IDmLegacyUpdater *pUpdater) {
  ...
  if (FindLegacyUpdater(pUpdater->GetName())) {
    Warning(
        "Attempted to add two legacy updaters with the same file format "
        "(%s)!\n",
        pUpdater->GetName());
    return;
  }
...
void CDataModel::AddFormatUpdater(IDmFormatUpdater *pUpdater) {
  ...
  if (FindFormatUpdater(pUpdater->GetName())) {
    Warning(
        "Attempted to add two format updaters with the same file format "
        "(%s)!\n",
        pUpdater->GetName());
    return;
  }
...
//-----------------------------------------------------------------------------
int CDataModel::GetEncodingCount() const { return m_Serializers.Count(); }
...
bool CDataModel::DoesEncodingStoreVersionInFile(
...
IDmSerializer *CDataModel::FindSerializer(const char *pEncodingName) const {
  ...
  for (int i = 0; i < nSerializers; ++i) {
    ...
    Assert(pSerializer);
    if (!pSerializer) continue;

    if (!V_strcmp(pEncodingName, pSerializer->GetName())) return pSerializer;
  }

  return NULL;
}

IDmLegacyUpdater *CDataModel::FindLegacyUpdater(
    const char *pLegacyFormatName) const {
  int nUpdaters = m_LegacyUpdaters.Count();
  for (int i = 0; i < nUpdaters; ++i) {
    IDmLegacyUpdater *pUpdater = m_LegacyUpdaters[i];
    Assert(pUpdater);
    if (!pUpdater) continue;

    if (!V_strcmp(pLegacyFormatName, pUpdater->GetName())) return pUpdater;
  }
...
IDmFormatUpdater *CDataModel::FindFormatUpdater(const char *pFormatName) const {
  ...
  for (int i = 0; i < nUpdaters; ++i) {
    ...
    if (!V_strcmp(pFormatName, pUpdater->GetName())) return pUpdater;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Sets the name of the DME element to create in keyvalues
// serialization
//-----------------------------------------------------------------------------
void CDataModel::SetKeyValuesElementCallback(
    IElementForKeyValueCallback *pCallbackInterface) {
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CDataModel::GetKeyValuesElementName(const char *pszKeyName,
                                                int iNestingLevel) {
  if (m_pKeyvaluesCallbackInterface)
    return m_pKeyvaluesCallbackInterface->GetElementForKeyValue(pszKeyName,
                                                                iNestingLevel);
...
bool GenerateFullPath(const char *pFileName, char const *pPathID, char *pBuf,
                      int nBufLen) {
  ...
  const char *pFullPath = g_pFullFileSystem->RelativePathToFullPath(
...
bool CDataModel::SaveToFile(char const *pFileName, char const *pPathID,
  ...
  CUtlStreamBuffer buf(pFullPath, pPathID,
                       bIsBinary ? 0 : CUtlBuffer::TEXT_BUFFER, true);
  if (!buf.IsValid()) {
...
DmFileId_t CDataModel::RestoreFromFile(
    char const *pFileName, char const *pPathID, const char *pFormatHint,
    CDmElement **ppRoot,
  ...
  if (!g_pFullFileSystem->ReadFile(pFullPath, pPathID, typeBuf,
                                   DMX_MAX_HEADER_LENGTH)) {
    Warning("CDataModel: Unable to open file %s\n", pFullPath);
    return DMFILEID_INVALID;
  }
  ...
  if (!bSuccess) {
    ...
    V_strncpy(pHeader->encodingName, pFormatHint,
              sizeof(pHeader->encodingName));
  ...
  if (!Unserialize(buf, pHeader->encodingName, pHeader->formatName, pFormatHint,
                   pFullPath, idConflictResolution, hRootElement))
    return DMFILEID_INVALID;
...
//-----------------------------------------------------------------------------
bool CDataModel::Serialize(CUtlBuffer &outBuf, const char *pEncodingName,
                           const char *pFormatName, DmElementHandle_t hRoot) {
  ...
  if (pSerializer->IsBinaryFormat()) {
    if (outBuf.IsText()) {
      if (!outBuf.ContainsCRLF()) {
        Warning(
            "Serialize: Format %s expects to be written to a binary format, "
            "but the buffer is a text-format buffer\n");
        return false;
      }
      outBuf.SetBufferType(false, false);
    }
  } else {
    // If we want text, but the binbuf is binary; recast it to a text buffer w/
    // CRLF
    if (!outBuf.IsText()) {
    ...
    if (outBuf.ContainsCRLF()) {
      // If we want text, but the binbuf expects CRLF, then we must do a
      // conversion pass
  ...
  if (pSerializer->StoresVersionInFile()) {
    ...
    pActualOutBuf->Printf(
        "%s encoding %s %d format %s %d %s\n", DMX_VERSION_STARTING_TOKEN,
        pEncodingName, pSerializer->GetCurrentVersion(), pFormatName,
        GetCurrentFormatVersion(pFormatName), DMX_VERSION_ENDING_TOKEN);
  }
...
bool CDataModel::ReadDMXHeader(CUtlBuffer &inBuf, DmxHeader_t *pHeader) const {
  ...
  bool bOk =
      inBuf.ParseToken(DMX_VERSION_STARTING_TOKEN, DMX_VERSION_ENDING_TOKEN,
                       headerStr, sizeof(headerStr));
  if (bOk) {
#ifdef _WIN32
    int nAssigned =
  ...
  if (!bOk) {
    ...
    bOk = inBuf.ParseToken(DMX_LEGACY_VERSION_STARTING_TOKEN,
                           DMX_LEGACY_VERSION_ENDING_TOKEN, pHeader->formatName,
                           DMX_MAX_FORMAT_NAME_MAX_LENGTH);
    if (bOk) {
      const char *pEncoding = GetEncodingFromLegacyFormat(pHeader->formatName);
      if (pEncoding) {
        V_strncpy(pHeader->encodingName, pEncoding,
                  DMX_MAX_FORMAT_NAME_MAX_LENGTH);
        pHeader->nEncodingVersion = 0;  // the first encoding version
        pHeader->nFormatVersion =
            -1;  // this value is ignored for legacy formats
      } else {
        bOk = false;
      }
    }
...
const char *CDataModel::GetEncodingFromLegacyFormat(
    const char *pLegacyFormatName) const {
  if (StringHasPrefixCaseSensitive(pLegacyFormatName, "binary_v"))
    return "binary";
  if (StringHasPrefixCaseSensitive(pLegacyFormatName, "sfm_v")) return "binary";
  ...
  if (!V_strcmp(pLegacyFormatName, "xml")) return "xml";
  if (!V_strcmp(pLegacyFormatName, "xml_flat")) return "xml_flat";
  return NULL;
}
...
bool CDataModel::IsUnserializing() { return m_bIsUnserializing; }

int CDataModel::GetCurrentFormatVersion(const char *pFormatName) {
  if (IsValidNonDMXFormat(pFormatName)) return 0;  // unversioned format

  IDmFormatUpdater *pUpdater = FindFormatUpdater(pFormatName);
  if (!pUpdater) return -1;  // invalid version #
...
//-----------------------------------------------------------------------------
bool CDataModel::Unserialize(CUtlBuffer &inBuf, const char *pEncodingName,
  ...
  if (!pEncodingName || !*pEncodingName) return false;
  Assert(pSourceFormatName && *pSourceFormatName);
  if (!pSourceFormatName || !*pSourceFormatName) return false;
  ...
  bool bIsCurrentVersion =
      true;  // for formats that don't store a format, files are currently
  ...
  if (bStoresVersionInFile) {
    ...
    if (IsLegacyFormat(header.formatName)) {
      if (GetCurrentFormatVersion(GENERIC_DMX_FORMAT) == 1) {
        ...
        bIsCurrentVersion =
            !pLegacyUpdater || pLegacyUpdater->IsLatestVersion();
      } else {
        bIsCurrentVersion = false;
      }
    } else {
      bIsCurrentVersion =
          GetCurrentFormatVersion(header.formatName) == header.nFormatVersion;
    }
  }

  // if we're not in dmxconvert, and we're not at the latest version, call
  // dmxconvert and unserialize from the converted file
  if (!m_bOnlyCreateUntypedElements && !bIsCurrentVersion) {
    ...
    const char *pDestFormatName = IsLegacyFormat(header.formatName)
                                      ? GENERIC_DMX_FORMAT
                                      : header.formatName;
    char cmdline[256];
    V_snprintf(cmdline, sizeof(cmdline),
               "dmxconvert -allowdebug -i %s -o %s -oe %s -of %s", pFileName,
               tempFileName, pDestEncodingName, pDestFormatName);

    ProcessHandle_t hProcess = g_pProcessUtils->StartProcess(cmdline, false);
    if (hProcess == PROCESS_HANDLE_INVALID) return false;
    ...
    bool bSuccess;
    {
      ...
      // yes, this passes in pFileName, even though it read from tempFileName -
      // pFileName is only used for marking debug messages and setting fileid
      bSuccess =
          Unserialize(strbuf, pDestEncodingName, pDestFormatName,
                      pDestFormatName, pFileName, idConflictResolution, hRoot);
    }
  ...
  if (bStoresVersionInFile) {
    if (V_strcmp(pEncodingName, header.encodingName) != 0) return false;
    if (V_strcmp(pSourceFormatName, header.formatName) != 0) return false;

    if (pSerializer->IsBinaryFormat()) {
      // For binary formats, we gotta keep reading until we hit the string
      // terminator that occurred after the version line.
      while (inBuf.GetChar() != 0) {
        if (!inBuf.IsValid()) break;
      }
    }
  }
  ...
  CDmElement *pRoot;
  bool bOk = pSerializer->Unserialize(
      inBuf, pEncodingName, header.nEncodingVersion, pSourceFormatName,
      header.nFormatVersion, fileid, idConflictResolution, &pRoot);
...
bool CDataModel::UpdateUnserializedElements(
  ...
  if (IsLegacyFormat(pSourceFormatName)) {
    ...
    if (pLegacyUpdater) {
      if (!pLegacyUpdater->Update(ppRoot)) return false;
    }

    // if there's no legacy updater found, then this is already the latest
    // legacy format
    pSourceFormatName = GENERIC_DMX_FORMAT;
  }

  IDmFormatUpdater *pFormatUpdater = FindFormatUpdater(pSourceFormatName);
  if (!pFormatUpdater) return false;
...
int CDataModel::NumFileIds() { return m_openFiles.GetHandleCount(); }

DmFileId_t CDataModel::GetFileId(int i) {
  Assert(i >= 0 && i < (int)m_openFiles.GetHandleCount());
  if (i < 0 || i >= (int)m_openFiles.GetHandleCount()) return DMFILEID_INVALID;
...
DmFileId_t CDataModel::FindOrCreateFileId(const char *pFilename) {
  ...
  if (!pFilename || !*pFilename) return DMFILEID_INVALID;

  DmFileId_t fileid = GetFileId(pFilename);
  if (fileid != DMFILEID_INVALID) {
    //		Assert( IsFileLoaded( fileid ) );
    MarkFileLoaded(fileid);  // this is sort of a hack, but I'm planning a
                             // rewrite phase on all this anyways - joe
    return fileid;
  }
...
DmFileId_t CDataModel::GetFileId(const char *pFilename) {
  ...
  for (int i = 0; i < nFiles; ++i) {
    ...
    if (fes && fes->m_filename == filenameSym) return fileid;
  }
...
void CDataModel::UnloadFile(DmFileId_t fileid, bool bDeleteElements) {
  ...
  for (int i = 0; i < nHandles; ++i) {
    DmElementHandle_t hElement =
        (DmElementHandle_t)m_Handles.GetHandleFromIndex(i);
    if (hElement == DMELEMENT_HANDLE_INVALID) continue;

    CDmElement *pElement = GetElement(hElement);
    if (!pElement || pElement->GetFileId() != fileid) continue;
...
void CDataModel::UnloadFile(DmFileId_t fileid) { UnloadFile(fileid, false); }
...
//-----------------------------------------------------------------------------
void CDataModel::RemoveElementFromFile(DmElementHandle_t hElement,
                                       DmFileId_t fileid) {
  if (fileid == DMFILEID_INVALID) return;
...
void CDataModel::AddElementToFile(DmElementHandle_t hElement,
                                  DmFileId_t fileid) {
  if (fileid == DMFILEID_INVALID) return;
...
// search id->handle table (both loaded and unloaded) for id, and if not found,
// create a new handle, map it to the id and return it
DmElementHandle_t CDataModel::FindOrCreateElementHandle(
    const DmObjectId_t &id) {
  UtlHashHandle_t h = m_elementIds.Find(id);
  if (h != m_elementIds.InvalidHandle()) return m_elementIds[h];

  h = m_unloadedIdElementMap.Find(ElementIdHandlePair_t(
      id));  // TODO - consider optimizing find to take just an id
  ...
  m_unloadedIdElementMap.Insert(
      ElementIdHandlePair_t(id, DmElementReference_t(hElement)));
  MarkHandleInvalid(hElement);
  return hElement;
}

// changes an element's id and associated mappings - generally during
// unserialization
DmElementHandle_t CDataModel::ChangeElementId(DmElementHandle_t hElement,
                                              const DmObjectId_t &oldId,
                                              const DmObjectId_t &newId) {
  UtlHashHandle_t oldHash = m_elementIds.Find(oldId);
  Assert(oldHash != m_elementIds.InvalidHandle());
  if (oldHash == m_elementIds.InvalidHandle()) return hElement;
  ...
  if (newHash != m_elementIds.InvalidHandle())
    return DMELEMENT_HANDLE_INVALID;  // can't change an element's id to the id
  ...
  if (newHash == m_unloadedIdElementMap.InvalidHandle()) {
    // the newId has never been seen before - keep the element handle the same
    // and rehash into the id->handle map
    m_elementIds.Insert(hElement);
    return hElement;
  }

  // else, the newId is being referenced by some other element
  // change element to use newId and the associated handle from an element
  // reference
  ...
  Assert(!m_Handles.IsHandleValid(
      newHandle));  // unloaded elements shouldn't have valid handles
...
DmElementReference_t *CDataModel::FindElementReference(
    DmElementHandle_t hElement, DmObjectId_t **ppId /* = NULL */) {
  if (ppId) {
    *ppId = NULL;
  }

  CDmElement *pElement = GetElement(hElement);
  if (pElement) return CDmeElementAccessor::GetReference(pElement);

  for (UtlHashHandle_t h = m_unloadedIdElementMap.GetFirstHandle();
       h != m_unloadedIdElementMap.InvalidHandle();
       h = m_unloadedIdElementMap.GetNextHandle(h)) {
...
void CDataModel::DontAutoDelete(DmElementHandle_t hElement) {
  // this artificially adds a strong reference to the element, so it won't ever
  // get unref'ed to 0 the only ways for this element to go away are explicit
  // deletion, or file unload
  OnElementReferenceAdded(hElement, true);
}

void CDataModel::OnElementReferenceAdded(DmElementHandle_t hElement,
                                         CDmAttribute *pAttribute) {
  Assert(pAttribute);
  if (!pAttribute) return;

  if (hElement == DMELEMENT_HANDLE_INVALID) return;
  ...
  //		GetString( GetElementType( hElement ) ), hElement,
  // GetElementName(
  // hElement ), 		pAttribute->GetOwner()->GetHandle(),
  // pAttribute->GetName() );
...
void CDataModel::OnElementReferenceAdded(DmElementHandle_t hElement,
  ...
  //	Msg( "OnElementReferenceAdded: %s 0x%x \"%s\" referenced by %s
  // handle\n", 		GetString( GetElementType( hElement ) ),
  // hElement,
  // GetElementName( hElement ), 		bRefCount ? "refcounted" :
  // "weak"
  // );
...
void CDataModel::OnElementReferenceRemoved(DmElementHandle_t hElement,
                                           CDmAttribute *pAttribute) {
  MEM_ALLOC_CREDIT();
  ...
  //	Msg( "OnElementReferenceRemoved: %s 0x%x '%s' referenced by 0x%x
  //'%s'\n", 		GetString( GetElementType( hElement ) ), hElement,
  // GetElementName(
  // hElement ), 		pAttribute->GetOwner()->GetHandle(),
  // pAttribute->GetName() );
...
void CDataModel::OnElementReferenceRemoved(DmElementHandle_t hElement,
  ...
  //	Msg( "OnElementReferenceRemoved: %s 0x%x \"%s\" referenced by %s
  // handle\n", 		GetString( GetElementType( hElement ) ),
  // hElement,
  // GetElementName( hElement ), 		bRefCount ? "refcounted" :
  // "weak"
...
void CDataModel::RemoveUnreferencedElements() {
  ...
  for (int i = 0; i < nElementIds; ++i) {
    UtlHashHandle_t h = m_unloadedIdElementMap.Find(
        ElementIdHandlePair_t(m_unreferencedElementIds[i]));
    if (h == m_unloadedIdElementMap.InvalidHandle()) continue;

    if (m_unloadedIdElementMap[h].m_ref.IsWeaklyReferenced())
      continue;  // don't remove if it's been referenced again - this allows an
                 // unref followed by a ref in the same edit phase

    //		Msg( "Removing reference: 0x%x\n", m_unloadedIdElementMap[ h
    //].m_ref.m_hElement );
  ...
  // this is intentionally calling Count() every time through, since
  // DestroyElement may cause more elements to be added to the list
  for (int i = 0; i < m_unreferencedElementHandles.Count(); ++i) {
    ...
    // pElement->GetName(), pElement->GetHandle() );
...
void CDataModel::FindAndDeleteOrphanedElements() {
  ...
  for (DmElementHandle_t hElement = FirstAllocatedElement();
       ...
       hElement = NextAllocatedElement(hElement)) {
    CDmElement *pElement = GetElement(hElement);
    if (!pElement) continue;

    DmFileId_t fileid = pElement->GetFileId();
    if (fileid == DMFILEID_INVALID) continue;
  ...
  for (DmElementHandle_t hElement = FirstAllocatedElement();
    ...
    if (pElement->IsAccessible()) continue;
  ...
  // JDTODO - this incorrectly deletes elements that are referenced by a counted
  // handle, but aren't under the file root
...
DmElementHandle_t CDataModel::FindElement(const DmObjectId_t &id) {
  UtlHashHandle_t h = m_elementIds.Find(id);
  if (h == m_elementIds.InvalidHandle()) return DMELEMENT_HANDLE_INVALID;

  return m_elementIds[h];
}

DmAttributeReferenceIterator_t CDataModel::FirstAttributeReferencingElement(
    DmElementHandle_t hElement) {
...
DmAttributeReferenceIterator_t CDataModel::NextAttributeReferencingElement(
    DmAttributeReferenceIterator_t hAttrIter) {
  DmAttributeList_t *pList = (DmAttributeList_t *)hAttrIter;
  if (!pList) return DMATTRIBUTE_REFERENCE_ITERATOR_INVALID;

  return (DmAttributeReferenceIterator_t)(int)pList->m_pNext;
}

CDmAttribute *CDataModel::GetAttribute(
    DmAttributeReferenceIterator_t hAttrIter) {
  DmAttributeList_t *pList = (DmAttributeList_t *)hAttrIter;
...
//-----------------------------------------------------------------------------
CDmElement *CDataModel::Unserialize(CUtlBuffer &buf) { return NULL; }
...
//			factory -
//-----------------------------------------------------------------------------
void CDataModel::AddElementFactory(const char *pClassName,
                                   IDmElementFactory *pFactory) {
...
int CDataModel::GetFirstFactory() const { return m_Factories.First(); }
...
//-----------------------------------------------------------------------------
DmElementHandle_t CDataModel::CreateElement(UtlSymId_t typeSymbol,
...
DmElementHandle_t CDataModel::CreateElement(const char *pElementType,
                                            const char *pElementName,
                                            DmFileId_t fileid,
                                            const DmObjectId_t *pObjectID) {
  Assert(!pObjectID ||
         m_elementIds.Find(*pObjectID) == m_elementIds.InvalidHandle());

  UtlHashHandle_t h =
      pObjectID ? m_unloadedIdElementMap.Find(ElementIdHandlePair_t(*pObjectID))
                : m_unloadedIdElementMap.InvalidHandle();
  if (h != m_unloadedIdElementMap.InvalidHandle()) {
    CDmElement *pElement =
        CreateElement(m_unloadedIdElementMap[h].m_ref, pElementType,
                      pElementName, fileid, pObjectID);
    if (pElement) {
      m_unloadedIdElementMap.Remove(h);
    ...
    DmElementHandle_t hElement = AcquireElementHandle();
    CDmElement *pElement =
        CreateElement(DmElementReference_t(hElement), pElementType,
                      pElementName, fileid, pObjectID);
    if (pElement) return pElement->GetHandle();
...
class CUndoCreateElement : public CUndoElement {
 ...
 public:
  CUndoCreateElement()
      : BaseClass("CUndoCreateElement"), m_bKill(false), m_hElement() {}
  ...
  void SetElement(DmElementHandle_t hElement) {
    Assert(GetElement<CDmElement>(hElement) &&
           GetElement<CDmElement>(hElement)->GetFileId() != DMFILEID_INVALID);
    m_hElement = hElement;  // this has to be delayed so that the element's ref
...
//-----------------------------------------------------------------------------
// CreateElement references the attribute list passed in via ref, so don't edit
// or purge ref's attribute list afterwards this is kosher because the ref
// either is created on the fly and has no attributes, or is being removed from
// m_unloadedIdElementMap
//-----------------------------------------------------------------------------
CDmElement *CDataModel::CreateElement(const DmElementReference_t &ref,
                                      const char *pElementType,
                                      const char *pElementName,
                                      DmFileId_t fileid,
                                      const DmObjectId_t *pObjectID) {
  //	Msg( "Creating %s 0x%x '%s' in file \"%s\" - %d elements loaded\n",
  // pElementType, ref.m_hElement, pElementName ? pElementName : "",
  // GetFileName( fileid ), m_elementIds.Count() );
  ...
  if (g_pDataModel->IsUndoEnabled() &&
      fileid != DMFILEID_INVALID)  // elements not in any file don't participate
                                   // in undo
  {
  ...
  CDmElement *pElement = pFactory->Create(ref.m_hElement, pElementType,
                                          pElementName, fileid, *pObjectID);
  if (pElement) {
...
class CUndoDestroyElement : public CUndoElement {
 ...
 public:
  CUndoDestroyElement(DmElementHandle_t hElement)
      : BaseClass("CUndoDestroyElement"), m_bKill(true), m_hElement(hElement) {
    Assert(GetElement<CDmElement>(hElement) &&
           GetElement<CDmElement>(hElement)->GetFileId() != DMFILEID_INVALID);
    g_pDataModelImp->MarkHandleInvalid(m_hElement);
  }
...
void CDataModel::DestroyElement(DmElementHandle_t hElement) {
  ...
  if (phase != PH_EDIT &&
      phase !=
          PH_EDIT_APPLY)  // need to allow edit_apply to delete elements, so
                          // that cascading deletes can occur in one phase
  {
    Assert(0);
    return;
  }
  ...
  if (UndoEnabledForElement(GetElement(hElement))) {
    ...
    g_pDataModel->AddUndoElement(pUndo);
    NotifyState(NOTIFY_CHANGE_TOPOLOGICAL);
    return;  // if undo is enabled, just toss this onto the undo stack, rather
             // than actually destroying it
  }

  DeleteElement(hElement);
}

void CDataModel::DeleteElement(DmElementHandle_t hElement,
                               DmHandleReleasePolicy hrp /* = HR_ALWAYS */) {
  DmPhase_t phase = g_pDmElementFramework->GetPhase();
  ...
  // In order for DestroyElement to work, then, we need to cache off the element
  // type because that's stored in an attribute

  const char *pElementType = pElement->GetTypeString();
  Assert(pElementType);

  //	Msg( "Deleting %s element 0x%x \'%s\' in file \"%s\" - %d elements
  // loaded\n", pElementType, hElement, pInternal->GetName(), GetFileName(
  // pInternal->GetFileId() ), m_elementIds.Count() );
  ...
  bool bReleaseHandle = hrp == HR_ALWAYS || (hrp == HR_IF_NOT_REFERENCED &&
                                             !pRef->IsWeaklyReferenced());
  if (!bReleaseHandle) {
    m_unloadedIdElementMap.Insert(
        ElementIdHandlePair_t(GetElementId(hElement), *pRef));
  }
  ...
  if (m_bOnlyCreateUntypedElements) {
    ...
    int idx = m_Factories.Find(pElementType);
    pFactory = idx == m_Factories.InvalidIndex() ? m_pDefaultFactory
                                                 : m_Factories[idx];
  }

  CDmeElementAccessor::PerformDestruction(pElement);

  // NOTE: Attribute destruction has to happen before the containing object is
  // destroyed because the inline optimization will crash otherwise, and after
  // PerformDestruction or else PerformDestruction will crash
...
CDmElement *CDataModel::GetElement(DmElementHandle_t hElement) const {
  return (hElement != DMELEMENT_HANDLE_INVALID) ? m_Handles.GetHandle(hElement)
                                                : NULL;
}

UtlSymId_t CDataModel::GetElementType(DmElementHandle_t hElement) const {
  CDmElement *pElement = (hElement != DMELEMENT_HANDLE_INVALID)
                             ? m_Handles.GetHandle(hElement)
                             : NULL;
  if (pElement == NULL) return UTL_INVAL_SYMBOL;
...
// Attribute types
//-----------------------------------------------------------------------------
const char *CDataModel::GetAttributeNameForType(
    DmAttributeType_t attType) const {
...
void CDataModel::AddElementToMailingList(DmMailingList_t list,
                                         DmElementHandle_t h) {
...
bool CDataModel::RemoveElementFromMailingList(DmMailingList_t list,
                                              DmElementHandle_t h) {
...
bool CDataModel::PostAttributeChanged(DmMailingList_t list,
                                      CDmAttribute *pAttribute) {
  MailingList_t &mailingList = m_MailingLists[list];
...
void CDataModel::PushNotificationScope(const char *pReason, int nNotifySource,
                                       int nNotifyFlags) {
...
bool CDataModel::IsUndoEnabled() const { return GetUndoMgr()->IsEnabled(); }

bool CDataModel::UndoEnabledForElement(const CDmElement *pElement) const {
  // elements not in any file don't participate in undo
  Assert(pElement);
  return IsUndoEnabled() && pElement &&
         pElement->GetFileId() != DMFILEID_INVALID;
}

bool CDataModel::IsDirty() const { return GetUndoMgr()->HasUndoData(); }

bool CDataModel::CanUndo() const { return GetUndoMgr()->HasUndoData(); }

bool CDataModel::CanRedo() const { return GetUndoMgr()->HasRedoData(); }

void CDataModel::StartUndo(const char *undodesc, const char *redodesc,
                           int nChainingID /* = 0 */) {
  GetUndoMgr()->PushUndo(undodesc, redodesc, nChainingID);
}

void CDataModel::FinishUndo() { GetUndoMgr()->PushRedo(); }
...
const char *CDataModel::GetUndoDesc() { return GetUndoMgr()->UndoDesc(); }

const char *CDataModel::GetRedoDesc() { return GetUndoMgr()->RedoDesc(); }

// From the UI, perform the Undo operation
void CDataModel::Undo() { GetUndoMgr()->Undo(); }

void CDataModel::Redo() { GetUndoMgr()->Redo(); }

// if true, undo records spew as they are added
void CDataModel::TraceUndo(bool state) { GetUndoMgr()->TraceUndo(state); }
...
void CDataModel::ClearUndo() {
  ...
  m_bDeleteOrphanedElements =
      true;  // next time we delete unreferenced elements, delete orphaned
             // subtrees as well
}
...
//
//-----------------------------------------------------------------------------
DmAttributeHandle_t CDataModel::AcquireAttributeHandle(
    CDmAttribute *pAttribute) {
  DmAttributeHandle_t hAttribute =
      (DmAttributeHandle_t)m_AttributeHandles.AddHandle();
...
//
//-----------------------------------------------------------------------------
void CDataModel::EmptyClipboard() { GetClipboardMgr()->EmptyClipboard(true); }

void CDataModel::SetClipboardData(
    CUtlVector<KeyValues *> &data,
    IClipboardCleanup *pfnOptionalCleanuFunction /*= 0*/) {
