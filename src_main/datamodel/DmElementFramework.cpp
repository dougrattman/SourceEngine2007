...
// Constructor
//-----------------------------------------------------------------------------
CDmElementFramework::CDmElementFramework()
    : m_phase(PH_EDIT), m_dirtyElements(128, 256) {}
...
void CDmElementFramework::Resolve(bool clearDirtyFlags) {
  ...
  for (int ei = 0; ei < nCount; ++ei) {
    ...
    if (clearDirtyFlags) {
      ...
      CDmeElementAccessor::MarkAttributesClean(
          pElement);  // marks all attributes clean
    }
  }
...
void CDmElementFramework::SetOperators(
    const CUtlVector<IDmeOperator *> &operators) {
...
void CDmElementFramework::Operate(bool bResolve) {
...

  {
    ...
    m_phase = PH_OPERATE;
    const CUtlVector<IDmeOperator *> &operatorsToRun =
        m_dependencyGraph.GetSortedOperators();
    uint32_t on = operatorsToRun.Count();
