...
//-----------------------------------------------------------------------------
DmElementDictHandle_t CDmElementDictionary::InsertElement(
    CDmElement *pElement) {
  // Insert it into the reconnection table
  return m_Dict.AddToTail(pElement ? pElement->GetHandle()
...
//-----------------------------------------------------------------------------
void CDmElementDictionary::AddAttribute(CDmAttribute *pAttribute,
                                        const DmObjectId_t &objectId) {
  if (m_elementsToDelete.Find(pAttribute->GetOwner()->GetHandle()) !=
      m_elementsToDelete.InvalidIndex())
...
//-----------------------------------------------------------------------------
void CDmElementDictionary::AddArrayAttribute(CDmAttribute *pAttribute,
...
//
//-----------------------------------------------------------------------------
void CDmElementDictionary::RemoveAttributeInfosOfElement(
    AttributeList_t &attributes, DmElementHandle_t hElement) {
  while (attributes.Count() > 0 &&
         attributes.Tail().m_pAttribute->GetOwner()->GetHandle() == hElement) {
    attributes.Remove(attributes.Count() - 1);
  }
}

DmElementHandle_t CDmElementDictionary::SetElementId(
    DmElementDictHandle_t hDictHandle, const DmObjectId_t &newId,
  ...
  if (idConflictResolution == CR_FORCE_COPY) {
    m_idmap.Insert(DmIdPair_t(
        newId,
        oldId));  // map the newId back to the old id, and keep the old id
    return hElement;
  }

  DmElementHandle_t newHandle =
      g_pDataModelImp->ChangeElementId(hElement, oldId, newId);
  if (newHandle != DMELEMENT_HANDLE_INVALID) {
    ...
    return newHandle;  // either keeping the old handle, with the new id, or
                       // found a new handle associated with that new id
  }
  ...
  if (idConflictResolution == CR_DELETE_OLD) {
    ...
    if (hExistingElement == DMELEMENT_HANDLE_INVALID)
      return DMELEMENT_HANDLE_INVALID;  // unexpected error in ChangeElementId
                                        // (failed due to something other than a
                                        // conflict)

    g_pDataModelImp->DeleteElement(
        hExistingElement,
        HR_NEVER);  // need to keep the handle around until ChangeElemendId
...
DmElementDictHandle_t CDmElementDictionary::FindElement(CDmElement *pElement) {
  return m_Dict.Find(pElement ? pElement->GetHandle()
                              : DMELEMENT_HANDLE_INVALID);
}
...
void CDmElementDictionary::HookUpElementAttributes() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    DmObjectId_t &id = h == m_idmap.InvalidHandle() ? m_Attributes[i].m_ObjectId
                                                    : m_idmap[h].m_newId;
...
void CDmElementDictionary::HookUpElementArrayAttributes() {
  // Find unique array attributes; we need to clear them all before adding
  // stuff. This clears them of stuff added during their construction phase.
  int n = m_ArrayAttributes.Count();
  CUtlRBTree<CDmAttribute *, unsigned short> lookup(
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (m_ArrayAttributes[i].m_nType == AT_ELEMENT) {
      ...
      UtlHashHandle_t h =
          m_idmap.Find(DmIdPair_t(m_ArrayAttributes[i].m_ObjectId));
      DmObjectId_t &id = (h == m_idmap.InvalidHandle())
                             ? m_ArrayAttributes[i].m_ObjectId
                             : m_idmap[h].m_newId;
...
//-----------------------------------------------------------------------------
bool CDmElementSerializationDictionary::LessFunc(const ElementInfo_t &lhs,
                                                 const ElementInfo_t &rhs) {
...
//-----------------------------------------------------------------------------
DmElementDictHandle_t CDmElementSerializationDictionary::Find(
    CDmElement *pElement) {
  ElementInfo_t find;
...
//-----------------------------------------------------------------------------
void CDmElementSerializationDictionary::BuildElementList_R(CDmElement *pElement,
                                                           bool bFlatMode,
                                                           bool bIsRoot) {
  if (!pElement) return;
  ...
  for (CDmAttribute *pAttribute = pElement->FirstAttribute(); pAttribute;
    ...
    switch (pAttribute->GetType()) {
      case AT_ELEMENT: {
        ...
        if (!pChild || pChild->GetFileId() != pElement->GetFileId()) break;
...
void CDmElementSerializationDictionary::BuildElementList(CDmElement *pElement,
                                                         bool bFlatMode) {
...
//-----------------------------------------------------------------------------
bool CDmElementSerializationDictionary::ShouldInlineElement(
    CDmElement *pElement) {
  // This means we've already encountered this guy.
  // Therefore, he can never be a root element
  DmElementDictHandle_t h = Find(pElement);
  if (h != m_Dict.InvalidIndex()) return !m_Dict[h].m_bRoot;
...
// Clears the dictionary
//-----------------------------------------------------------------------------
void CDmElementSerializationDictionary::Clear() { m_Dict.RemoveAll(); }
...
//-----------------------------------------------------------------------------
DmElementDictHandle_t CDmElementSerializationDictionary::FirstRootElement()
    const {
  ...
  // Also, using inorder traversal won't get my actual root element to be first
  // in the file
  int nCount = m_Dict.Count();
  for (DmElementDictHandle_t h = 0; h < nCount; ++h) {
    if (m_Dict[h].m_bRoot) return h;
  }
  return ELEMENT_DICT_HANDLE_INVALID;
}

DmElementDictHandle_t CDmElementSerializationDictionary::NextRootElement(
    DmElementDictHandle_t h) const {
  // NOTE - this code only works with BlockMemory or Memory (NOT FixedMemory)
...
CDmElement *CDmElementSerializationDictionary::GetRootElement(
