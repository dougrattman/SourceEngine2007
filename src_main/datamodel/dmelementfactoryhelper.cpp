...
// Constructor
//-----------------------------------------------------------------------------
CDmElementFactoryHelper::CDmElementFactoryHelper(
    const char *classname, IDmElementFactoryInternal *pFactory,
...
//-----------------------------------------------------------------------------
// NOTE: The name of this extern is defined by the macro
...
void CDmElementFactoryHelper::InstallFactories() {
  ...
  while (p) {
    ...
    p->GetFactory()->SetElementTypeSymbol(
        g_pDataModel->GetSymbol(p->GetClassname()));
    p = p->GetNext();
  }
