...
enum { ELEMENT_DICT_HANDLE_INVALID = (DmElementDictHandle_t)~0 };
...
class CDmElementDictionary {
 public:
  ...
  void AddArrayAttribute(CDmAttribute *pAttribute,
                         DmElementDictHandle_t hChild);
  void AddArrayAttribute(CDmAttribute *pAttribute,
 ...
 private:
  ...
  struct DmIdPair_t {
    ...
    DmIdPair_t() {}
    DmIdPair_t(const DmObjectId_t &id) { CopyUniqueId(id, &m_oldId); }
  ...
  struct DeletionInfo_t {
    ...
    bool operator==(const DeletionInfo_t &src) const {
      return m_hElementToDelete == src.m_hElementToDelete;
    }
  ...
  void RemoveAttributeInfosOfElement(AttributeList_t &attributes,
