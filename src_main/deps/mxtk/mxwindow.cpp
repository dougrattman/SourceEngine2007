...
mxWindow::mxWindow(mxWindow *parent, int x, int y, int w, int h,
  ...
  void *handle = (void *)CreateWindowEx(0, "mx_class", label, dwStyle, x, y, w,
                                        h, (HWND)parentHandle, (HMENU)NULL,
  ...
  if (!parent && !g_mainWindow) g_mainWindow = this;
}
...
int mxWindow::handleEvent(mxEvent * /*event*/) { return 0; }
...
void mxWindow::setTimer(int milliSeconds) {
  ...
  if (milliSeconds > 0) {
    ...
    d_this->d_uTimer =
        (UINT)SetTimer((HWND)getHandle(), d_this->d_uTimer, milliSeconds, NULL);
  }
}
