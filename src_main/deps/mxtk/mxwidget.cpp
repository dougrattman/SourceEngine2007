...
mxWidget::mxWidget(mxWindow *parent, int x, int y, int w, int h,
                   const char *label) {
...
mxWidget::~mxWidget() {
  ...
  if (d_this->d_type == MX_MENU || d_this->d_type == MX_MENUBAR ||
...
void mxWidget::setHandle(void *handle) { d_this->d_hwnd = (HWND)handle; }

void mxWidget::setType(int type) { d_this->d_type = type; }
...
void mxWidget::setBounds(int x, int y, int w, int h) {
  ...
  if (!strcmp(str, WC_TABCONTROL)) mxTab_resizeChild(d_this->d_hwnd);
}
...
void mxWidget::setVisible(bool b) {
  if (b)
    SetWindowPos(d_this->d_hwnd, HWND_TOP, 0, 0, 0, 0,
                 SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
  else
    ShowWindow(d_this->d_hwnd, SW_HIDE);
}

void mxWidget::setEnabled(bool b) { EnableWindow(d_this->d_hwnd, b); }

void mxWidget::setId(int id) {
  SetWindowLong(d_this->d_hwnd, GWL_ID, (LONG)id);
}

void mxWidget::setUserData(void *userData) { d_this->d_userData = userData; }

void *mxWidget::getHandle() const { return (void *)d_this->d_hwnd; }

int mxWidget::getType() const { return d_this->d_type; }

mxWindow *mxWidget::getParent() const { return d_this->d_parent_p; }
