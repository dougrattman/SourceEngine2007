...
mxProgressBar::mxProgressBar(mxWindow *parent, int x, int y, int w, int h,
                             int style)
  ...
  if (style == Smooth) dwStyle |= PBS_SMOOTH;

  void *handle = (void *)CreateWindowEx(0, PROGRESS_CLASS, "", dwStyle, x, y, w,
                                        h, hwndParent, (HMENU)NULL,
...
mxProgressBar::~mxProgressBar() { delete d_this; }
...
int mxProgressBar::getValue() const { return d_this->d_value; }

int mxProgressBar::getTotalSteps() const { return d_this->d_steps; }
