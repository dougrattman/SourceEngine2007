...
typedef LRESULT(CALLBACK *WndProc_t)(HWND hwnd, UINT uMessage, WPARAM wParam,
                                     LPARAM lParam);

static WndProc_t s_OldWndProc = 0;

static LRESULT CALLBACK EditWndProc(HWND hwnd, UINT uMessage, WPARAM wParam,
                                    LPARAM lParam) {
  int iret = 0;
...
mxLineEdit::mxLineEdit(mxWindow *parent, int x, int y, int w, int h,
                       const char *label, int id, int style)
    : mxWidget(parent, x, y, w, h, label) {
  if (!parent) return;

  DWORD dwStyle = WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL |
                  WS_TABSTOP;  //  | ES_WANTRETURN | ES_MULTILINE;
  ...
  void *handle = (void *)CreateWindowEx(
      WS_EX_CLIENTEDGE, "mx_edit", label,
      dwStyle,  // WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL,
      x, y, w, h, hwndParent, (HMENU)id, (HINSTANCE)GetModuleHandle(NULL),
      NULL);
