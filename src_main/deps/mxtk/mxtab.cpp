...
void mxTab_resizeChild(HWND hwnd) {
  ...
  if (index >= 0) {
    ...
    if (widget) {
      ...
      DeferWindowPos(hdwp, (HWND)widget->getHandle(), HWND_TOP, rc.left, rc.top,
...
mxTab::mxTab(mxWindow *parent, int x, int y, int w, int h, int id)
  ...
  void *handle = (void *)CreateWindowEx(0, WC_TABCONTROL, "", dwStyle, x, y, w,
...
void mxTab::add(mxWidget *widget, const char *text) {
  ...
  TabCtrl_InsertItem((HWND)getHandle(), TabCtrl_GetItemCount((HWND)getHandle()),
                     &ti);
  mxTab_resizeChild((HWND)getHandle());
}

void mxTab::remove(int index) { TabCtrl_DeleteItem((HWND)getHandle(), index); }

void mxTab::select(int index) { TabCtrl_SetCurSel((HWND)getHandle(), index); }
