...
mxListView::mxListView(mxWindow *parent, int x, int y, int w, int h, int id)
  ...
  DWORD dwStyle =
      LVS_NOSORTHEADER | LVS_REPORT | LVS_SHOWSELALWAYS | WS_VISIBLE | WS_CHILD;
  HWND hwndParent = (HWND)((mxWidget *)parent)->getHandle();

  d_this->d_hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, WC_LISTVIEW, "", dwStyle, x,
                                  y, w, h, hwndParent, (HMENU)id,
                                  (HINSTANCE)GetModuleHandle(NULL), NULL);

  SendMessage(d_this->d_hwnd, WM_SETFONT,
              (WPARAM)(HFONT)GetStockObject(ANSI_VAR_FONT),
              MAKELPARAM(TRUE, 0));
...
void mxListView::remove(int index) {
  if (!d_this) return;

  ListView_DeleteItem(d_this->d_hwnd, index);
}

void mxListView::removeAll() { ListView_DeleteAllItems(d_this->d_hwnd); }

void mxListView::setLabel(int item, int column, const char *label) {
  if (!d_this) return;
...
void mxListView::setLabel(int item, int column, const wchar_t *label) {
  ...
  SendMessage(d_this->d_hwnd, LVM_SETITEMW, 0,
              (LPARAM)(const LV_ITEMW FAR *)(&lvItem));
}

void mxListView::setUserData(int item, int column, void *userData) {
  if (!d_this) return;
...
void mxListView::setSelected(int item, bool b) {
  if (!d_this) return;

  ListView_SetItemState(d_this->d_hwnd, item,
                        b ? (LVIS_SELECTED | LVIS_FOCUSED) : 0,
...
int mxListView::getNextSelectedItem(int startitem /*= 0*/) const {
  ...
  if (ListView_GetSelectedCount(d_this->d_hwnd) == 0) return -1;

  int c = getItemCount();
  int start = startitem + 1;

  while (start < c) {
    if (isSelected(start)) return start;
    start++;
  }
...
const char *mxListView::getLabel(int item, int column) const {
  static char label[256];
  strcpy(label, "");

  if (!d_this) return label;
...
void *mxListView::getUserData(int item, int column) const {
  if (!d_this) return 0;
...
bool mxListView::isSelected(int index) const {
  if (!d_this) return false;

  int state = ListView_GetItemState(d_this->d_hwnd, index, LVIS_SELECTED);
  if (state & LVIS_SELECTED) return true;
...
void mxListView::setDrawingEnabled(bool draw) {
  ...
  SendMessage(d_this->d_hwnd, WM_SETREDRAW, (WPARAM)draw ? TRUE : FALSE,
              (LPARAM)0);
}
