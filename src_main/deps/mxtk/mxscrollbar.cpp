...
mxScrollbar::mxScrollbar(mxWindow *parent, int x, int y, int w, int h, int id,
                         int style)
  ...
  void *handle = (void *)CreateWindowEx(0, "SCROLLBAR", "", dwStyle, x, y, w, h,
                                        hwndParent, (HMENU)id,
...
void mxScrollbar::setValue(int ivalue) {
  SetScrollPos((HWND)getHandle(), SB_CTL, ivalue, FALSE);
}

void mxScrollbar::setRange(int min, int max) {
  SCROLLINFO si = {sizeof(SCROLLINFO), SIF_RANGE, min, max, 0, 0, 0};

  SetScrollInfo((HWND)getHandle(), SB_CTL, &si, TRUE);
}

void mxScrollbar::setPagesize(int size) {
  SCROLLINFO si = {sizeof(SCROLLINFO), SIF_PAGE, 0, 0, (UINT)size, 0, 0};

  SetScrollInfo((HWND)getHandle(), SB_CTL, &si, TRUE);
}

int mxScrollbar::getValue() const {
  // SCROLLINFO si = { sizeof( SCROLLINFO ), SIF_POS | SIF_TRACKPOS, 0, 0, 0, 0,
  // 0 }; GetScrollInfo( (HWND) getHandle (), SB_CTL, &si ); return si.nPos;
  return GetScrollPos((HWND)getHandle(), SB_CTL);
}

int mxScrollbar::getMinValue() const {
...
int mxScrollbar::getMaxValue() const {
  SCROLLINFO si = {sizeof(SCROLLINFO), SIF_RANGE, 0, 0, 0, 0, 0};
...
int mxScrollbar::getPagesize() const {
