...
void mx::createAccleratorTable(int numentries, Accel_t *entries) {
  ...
  for (int i = 0; i < numentries; ++i) {
    ...
    accelentries.push_back(add);
  }

  g_hAcceleratorTable =
      ::CreateAcceleratorTable(accelentries.data(), accelentries.size());
}

void mx_addWidget(mxWidget *widget) {
  if (g_widgetList) g_widgetList->add((void *)widget);
}

void mx_removeWidget(mxWidget *widget) {
  if (g_widgetList) g_widgetList->remove((void *)widget);
}
  ...
  if (!g_hwndToolTipControl) {
    if (g_mainWindow) {
      g_hwndToolTipControl =
          CreateWindowEx(0, TOOLTIPS_CLASS, "", WS_POPUP | WS_EX_TOPMOST, 0, 0,
                         0, 0, (HWND)g_mainWindow->getHandle(), (HMENU)NULL,
...
static void RecursiveHandleEvent(mxWindow *window, mxEvent *event) {
  while (window) {
    if (window->handleEvent(event)) break;
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT uMessage, WPARAM wParam,
  ...
  switch (uMessage) {
    ...
    case WM_COMMAND: {
      ...
      if (LOWORD(wParam) > 0 && window) {
        ...
        if (!strcmpi(className, "edit")) {
          ...
          if (wNotifyCode != LBN_SELCHANGE) break;
        }
    ...
    case WM_NOTIFY: {
      ...
      if (nmhdr->code == TVN_SELCHANGED) {
        if (nmhdr->idFrom > 0) {
          ...
          event.event = mxEvent::Action;
          event.widget =
              (mxWidget *)GetWindowLong(nmhdr->hwndFrom, GWL_USERDATA);
    ...
    case WM_ERASEBKGND: {
      mxWindow *window = (mxWindow *)GetWindowLong(hwnd, GWL_USERDATA);
      if (window) {
        if (window->getType() == MX_GLWINDOW) return 0;
    ...
    case WM_RBUTTONDOWN: {
      ...
      if (window) {
        ...
        if (wParam & MK_LBUTTON) event.buttons |= mxEvent::MouseLeftButton;

        if (wParam & MK_RBUTTON) event.buttons |= mxEvent::MouseRightButton;

        if (wParam & MK_MBUTTON) event.buttons |= mxEvent::MouseMiddleButton;

        if (wParam & MK_CONTROL) event.modifiers |= mxEvent::KeyCtrl;

        if (wParam & MK_SHIFT) event.modifiers |= mxEvent::KeyShift;
    ...
    case WM_SYSKEYDOWN: {
      ...
      if (window) {
        ...
        if (window->handleEvent(&event)) return 0;
      }
    } break;
  ...
  while (1) {
    ...
    if (PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE) || !g_idleWindow) {
      ...
      if (!g_hAcceleratorTable ||
          !TranslateAccelerator((HWND)g_mainWindow->getHandle(),
                                g_hAcceleratorTable, &msg)) {
...
void mx::setIdleWindow(mxWindow *window) { g_idleWindow = window; }

int mx::getDisplayWidth() { return (int)GetSystemMetrics(SM_CXSCREEN); }

int mx::getDisplayHeight() { return (int)GetSystemMetrics(SM_CYSCREEN); }

mxWindow *mx::getMainWindow() { return g_mainWindow; }
...
int mx::getTickCount() { return (int)GetTickCount(); }
