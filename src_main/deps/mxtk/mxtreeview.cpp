...
mxTreeView::mxTreeView(mxWindow *parent, int x, int y, int w, int h, int id)
  ...
  DWORD dwStyle = TVS_HASBUTTONS | TVS_HASLINES | TVS_LINESATROOT |
                  TVS_SHOWSELALWAYS | WS_VISIBLE | WS_CHILD;
  HWND hwndParent = (HWND)((mxWidget *)parent)->getHandle();

  d_this->d_hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, WC_TREEVIEW, "", dwStyle, x,
...
void mxTreeView::removeAll() { remove(0); }
...
void mxTreeView::setOpen(mxTreeViewItem *item, bool b) {
  ...
  if (item)
    TreeView_Expand(d_this->d_hwnd, (HTREEITEM)item,
                    b ? TVE_EXPAND : TVE_COLLAPSE);
}

void mxTreeView::setSelected(mxTreeViewItem *item, bool b) {
  if (!d_this) return;

  if (item) TreeView_SelectItem(d_this->d_hwnd, (HTREEITEM)item);
}

mxTreeViewItem *mxTreeView::getFirstChild(mxTreeViewItem *item) const {
  if (!d_this) return 0;

  return (mxTreeViewItem *)TreeView_GetChild(d_this->d_hwnd,
                                             item ? (HTREEITEM)item : TVI_ROOT);
}
...
mxTreeViewItem *mxTreeView::getNextChild(mxTreeViewItem *item) const {
  ...
  if (item)
    return (mxTreeViewItem *)TreeView_GetNextSibling(d_this->d_hwnd,
                                                     (HTREEITEM)item);
  else
    return 0;
}
...
mxTreeViewItem *mxTreeView::getParent(mxTreeViewItem *item) const {
  ...
  if (item)
    return (mxTreeViewItem *)TreeView_GetParent(d_this->d_hwnd,
                                                (HTREEITEM)item);
...
void mxTreeView::setImages(mxTreeViewItem *item, int imagenormal,
                           int imageselected) {
  if (!d_this) return;
...
void mxTreeView::sortTree(mxTreeViewItem *parent, bool recurse, void *func,
                          int parameter) {
  if (!d_this) return;
