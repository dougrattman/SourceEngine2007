...
void mxMenuBar::addMenu(const char *item, mxMenu *menu) {
  AppendMenu((HMENU)getHandle(), MF_POPUP,
             (UINT)((mxWidget *)menu)->getHandle(), item);
}
...
void mxMenuBar::modify(int id, int newId, const char *newItem) {
  ModifyMenu((HMENU)getHandle(), (UINT)id, MF_BYCOMMAND | MF_STRING,
             (UINT)newId, (LPCTSTR)newItem);
}
...
int mxMenuBar::getHeight() const { return 0; }
