...
const char *mxGetOpenFileName(mxWindow *parent, const char *path,
                              const char *filter) {
  ...
  if (parent) ofn.hwndOwner = (HWND)parent->getHandle();
  ofn.hInstance = (HINSTANCE)GetModuleHandle(NULL);
  ofn.lpstrFilter = szFilter;
  ofn.nFilterIndex = 1;
  ofn.lpstrFile = sd_path;
  ofn.nMaxFile = _MAX_PATH;
  if (path && strlen(path)) ofn.lpstrInitialDir = szPath;
...
const char *mxGetSaveFileName(mxWindow *parent, const char *path,
                              const char *filter) {
  CHAR szPath[_MAX_PATH], szFilter[_MAX_PATH];
