...
static bool DoGetToken(TokenReader &tr, char **ppszStore, int nSize,
  ...
  if (bBadTokenType && (pszExpecting == NULL)) {
    ...
    return false;
  } else if (bBadTokenType ||
             ((pszExpecting != NULL) && !IsToken(pszStore, pszExpecting))) {
    //
...
// error.
//			If there was an error, the error will be reported in the
// message window.
//-----------------------------------------------------------------------------
...
// message window.
//-----------------------------------------------------------------------------
bool GDSkipToken(TokenReader &tr, trtoken_t ttexpecting,
  ...
  return DoGetToken(tr, &pszDiscardBuf, sizeof(szDiscardBuf), ttexpecting,
                    pszExpecting);
}
...
// Output :
//-----------------------------------------------------------------------------
bool GDGetTokenDynamic(TokenReader &tr, char **ppszStore, trtoken_t ttexpecting,
                       const char *pszExpecting) {
  if (ppszStore == NULL) {
    return false;
  }
...
BOOL GameData::Load(const char *pszFilename) {
  ...
  if (GetFileAttributes(pszFilename) == 0xffffffff) return FALSE;

  if (!tr.Open(pszFilename)) return FALSE;
  ...
  while (1) {
    ...
    if (ttype == TOKENEOF) break;

    if (ttype != OPERATOR || !IsToken(szToken, "@")) {
      if (!GDError(tr, "expected @")) return FALSE;
    }

    // check what kind it is, and parse a new object
    if (tr.NextToken(szToken, sizeof(szToken)) != IDENT) {
      if (!GDError(tr, "expected identifier after @")) return FALSE;
    }

    if (IsToken(szToken, "baseclass") || IsToken(szToken, "pointclass") ||
        IsToken(szToken, "solidclass") || IsToken(szToken, "keyframeclass") ||
        IsToken(szToken, "moveclass") || IsToken(szToken, "npcclass") ||
        IsToken(szToken, "filterclass")) {
      //
      // New class.
      //
      ...
      if (!pNewClass->InitFromTokens(tr, this)) {
        ...
        } else if (IsToken(szToken, "solidclass"))  // Tied to solids.
        {
          pNewClass->SetSolidClass(true);
                       ...
                       szToken,
                       "npcclass"))  // NPC class - can be spawned by npc_maker.
        {
          pNewClass->SetPointClass(true);
          pNewClass->SetNPCClass(true);
        } else if (IsToken(szToken, "filterclass"))  // Filter class - can be
                                                     // used as a filter
        {
          pNewClass->SetPointClass(true);
        ...
        // Check and see if this new class matches an existing one. If so we
        // will override the previous definition.
        int nExistingClassIndex = 0;
        GDclass *pExistingClass =
            ClassForName(pNewClass->GetName(), &nExistingClassIndex);
...
// These are 'standard' keys that every entity uses, but they aren't specified
// that way in the .fgd
static const char *RequiredKeys[] = {"Origin", "Angles", NULL};
...
//			pszInstancePrefix - the prefix to be used for all name
// fields 			Origin - the origin offset of the instance
// Angles - the angle rotation of the instance
// Output : if successful, will return the game data class of the class name
//-----------------------------------------------------------------------------
GDclass *GameData::BeginInstanceRemap(const char *pszClassName,
...
//			AllowNameRemapping - only do name remapping if this
// parameter is true. 				this is generally only false on
// the instance level. Output : returns true if the value changed
...
bool GameData::RemapKeyValue(const char *pszKey, const char *pszInValue,
  ...
  switch (RemapOperation[KVRemapIndex]) {
    ...
    case REMAP_ANGLE:
      if (m_InstanceAngle.x != 0.0f || m_InstanceAngle.y != 0.0f ||
          m_InstanceAngle.z != 0.0f) {
  ...
  while (1) {
    ...
    if (tr.PeekTokenType(szToken, sizeof(szToken)) == OPERATOR) {
      ...
      if (bMatchFound == false) {
        ...
        Q_strncpy(m_FGDMaterialExclusions[index].szDirectory, szToken,
                  sizeof(m_FGDMaterialExclusions[index].szDirectory));
...
bool GameData::LoadFGDAutoVisGroups(TokenReader &tr) {
  ...
  if (GDSkipToken(tr, OPERATOR, "=")) {
    ...
    Q_strncpy(m_FGDAutoVisGroups[gindex].szParent, szToken,
              sizeof(m_FGDAutoVisGroups[gindex].szParent));
  ...
  while (1) {
    if (GDGetToken(tr, szToken, sizeof(szToken), STRING)) {
      ...
      Q_strncpy(m_FGDAutoVisGroups[gindex].m_Classes[cindex].szClass, szToken,
                sizeof(m_FGDAutoVisGroups[gindex].m_Classes[cindex].szClass));
