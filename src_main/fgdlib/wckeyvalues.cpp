...
MDkeyvalue &MDkeyvalue::operator=(const MDkeyvalue &other) {
  ...
  V_strcpy(szValue, other.szValue);
...
//
//			NOTE: This should only be used for keyvalue lists that
// do
// not require 			unique key names! If you use this function then
// you should use GetCount 			and GetKey/Value by index rather
// than GetValue by key name.
void WCKVBase_Vector::AddKeyValue(const char *pszKey, const char *pszValue) {
  ...
  V_strcpy(szTmpKey, pszKey);
  V_strcpy(szTmpValue, pszValue);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void WCKVBase_Dict::RemoveKeyAt(int nIndex) { m_KeyValues.RemoveAt(nIndex); }
...
//-----------------------------------------------------------------------------
template <class Base>
const char *WCKeyValuesT<Base>::GetValue(const char *pszKey,
                                         int *piIndex) const {
...
void WCKeyValuesT<Base>::SetValue(const char *pszKey, const char *pszValue) {
  ...
  if (i == this->GetInvalidIndex()) {
    ...
    if (pszValue != NULL) {
      V_strncpy(this->m_KeyValues[i].szValue, szTmpValue,
                sizeof(this->m_KeyValues[i].szValue));
    }
    //
