...
//			iBaseIndex -
//			iVarIndex -
// Output : Returns TRUE if the pVar pointer was copied directly into this
// GDClass,
//			FALSE if not. If this function returns TRUE, pVar should
// not be 			deleted by the caller.
//-----------------------------------------------------------------------------
BOOL GDclass::AddVariable(GDinputvariable *pVar, GDclass *pBase, int iBaseIndex,
                          int iVarIndex) {
  int iThisIndex;
  ...
  if (pThisVar != NULL) {
    ...
    if (m_VariableMap[iThisIndex][0] == -1) {
      ...
      //
      // "pThisVar" was declared in a base class - we can replace the reference
      // in our variable map with the new variable.
      //
  ...
  //
  // We added the pointer to our list of items (see Variables.AddToTail, above)
  // so we must return true here.
  //
  return (true);
}
...
//			FGD file. This controls the onscreen representation of
// any entities 			derived from this class.
// Input  : pfMins - Receives minimum X, Y, and Z coordinates for the class.
//			pfMaxs - Receives maximum X, Y, and Z coordinates for
// the class.
// Output : Returns TRUE if this class has a specified bounding box, FALSE if
// not.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CClassInput *GDclass::GetInput(int nIndex) { return m_Inputs.Element(nIndex); }
...
BOOL GDclass::InitFromTokens(TokenReader &tr, GameData *pGD) {
  ...
  if ((tr.PeekTokenType(szToken, sizeof(szToken)) == OPERATOR) &&
    ...
    //
    // Free any existing description and set the pointer to NULL so that
    // GDGetToken allocates memory for us.
    //
    delete m_pszDescription;
    m_pszDescription = NULL;
...
//-----------------------------------------------------------------------------
// Purpose: Parses a helper from the FGD file. Helpers are of the following
// format:
//
//			<helpername>(<parameter 0> <parameter 1> ... <parameter
// n>)
//
//			When this function is called, the helper name has
// already been parsed.
...
//			read successfully, the input is added to this class. If
// not, a 			parsing failure is returned.
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool GDclass::ParseInputOutput(TokenReader &tr,
                               CClassInputOutputBase *pInputOutput) {
  char szToken[MAX_TOKEN];
...
//			read successfully, the output is added to this class. If
// not, a 			parsing failure is returned.
// Input  : tr - Token reader to use for parsing.
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
...
GDinputvariable *GDclass::GetVariableAt(int iIndex) {
  if (iIndex < 0 || iIndex >= m_nVariables) return NULL;
...
void GDclass::GetHelperForGDVar(GDinputvariable *pVar,
                                CUtlVector<const char *> *pszHelperName) {
