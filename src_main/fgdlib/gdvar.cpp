...
static TypeMap_t TypeMap[] = {
    ...
    {ivTargetNameOrClass, "target_name_or_class",
     STRING},  // Another version of target_destination that accepts class names
...
BOOL GDinputvariable::InitFromTokens(TokenReader &tr) {
  ...
  if (ttype == OPERATOR && IsToken(szToken, ":")) {
    ...
    if (ttype == OPERATOR && IsToken(szToken, ":")) {
      ...
      if (ttype == OPERATOR && IsToken(szToken, ":")) {
        ...
        //
        // Determine how to parse the default value. If this is a choices field,
        // the default could either be a string or an integer, so we must look
        // ahead and use whichever is there.
        //
  ...
  if ((ttype == OPERATOR && IsToken(szToken, "]")) || ttype != OPERATOR) {
    if (m_eType == ivFlags || m_eType == ivChoices) {
      //
      // Can't define a flags or choices variable without providing any flags or
      // choices.
      //
  ...
  if (m_eType == ivFlags) {
    ...
    for (int i = 0; i < m_Items.Count(); i++) {
      if (m_Items[i].bDefault) nDefault |= m_Items[i].iValue;
    }
...
// Input  : pkv - Pointer to the key value object containing string encoded
// value.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Determines whether the given flag is set (assuming this is an
// ivFlags). Input  : uCheck - Flag to check. Output : Returns TRUE if flag is
// set, FALSE if not.
//-----------------------------------------------------------------------------
...
//			list of flags or choices. Ours take priority if
// collisions occur.
...
//-----------------------------------------------------------------------------
// Purpose: Determines whether the given flag is set (assuming this is an
// ivFlags). Input  : uFlags - Flags to set.
...
//-----------------------------------------------------------------------------
// Purpose: this function will let you iterate through the text names of the
// variable types Input  : eType - the type to get the text of Output : returns
// the textual name
//-----------------------------------------------------------------------------
