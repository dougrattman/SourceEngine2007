...
class ScopedWinsockInitializer {
 public:
  ...
  [[nodiscard]] windows_errno_code error_code() const noexcept { return error_code_; }
 ...
 private:
  ...
  static WSAData Initialize(ScopedWinsockInitializer* me, WinsockVersion version) noexcept {
    ...
    static_assert(std::is_same_v<WORD, std::underlying_type_t<decltype(version)>>,
    ...
    windows_errno_code error_code{win32_to_windows_errno_code(WSAStartup(the_version, &wsa_data))};
    if (error_code == S_OK) {
