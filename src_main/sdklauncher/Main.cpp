...
void CreateMessageWindow(void) {
  ...
  staticHwnd = CreateWindowEx(0, "SDKLauncher_Window", "Hidden Window", 0, 0, 0,
...
CUtlVector<char> *GetFileStringWithReplacements(const char *pInputFilename,
  ...
  if (nReplacements) {
    ...
    for (int i = 0; i < nReplacements / 2; i++) {
      for (int iTestCount = 0; iTestCount < 64; iTestCount++) {
        if (Q_StrSubst(pCurData->Base(), ppReplacements[i * 2],
                       ppReplacements[i * 2 + 1],
                       g_ReplacementData[i & 1].Base(),
                       g_ReplacementData[i & 1].Count()))
          break;

        // Ok, we would overflow the string.. add more space to do the string
        // substitution into.
        iCurCount += 2048;
        g_ReplacementData[0].EnsureCount(iCurCount);
        g_ReplacementData[1].EnsureCount(iCurCount);
...
bool CopyWithReplacements(const char *pInputFilename,
                          const char **ppReplacements, int nReplacements,
                          const char *pOutputFilenameFormat, ...) {
  int dataWriteLen;
  CUtlVector<char> *pCurData = GetFileStringWithReplacements(
      pInputFilename, ppReplacements, nReplacements, dataWriteLen);
  if (!pCurData) {
    char msg[512];
  ...
  // Write it out. I'd like to use IFileSystem, but Steam lowercases all
  // filenames, which screws case-sensitive linux (since the linux makefiles are
  // tuned to the casing in Perforce).
...
void QuickLaunchCommandLine(char *pCommandLine) {
  ...
  if (!CreateProcess(0, pCommandLine,
                     ...
                     &si, &pi)) {
    ::MessageBoxA(NULL, GetLastWindowsErrorString(), "Error",
...
bool RunQuickLaunch() {
  ...
  if (CommandLine()->FindParm("-runhammer")) {
    Q_snprintf(cmdLine, sizeof(cmdLine), "\"%s\\%s\\bin\\hammer.exe\"",
               GetSDKLauncherBinDirectory(), g_engineDir);
    QuickLaunchCommandLine(cmdLine);
    return true;
  } else if (CommandLine()->FindParm("-runmodelviewer")) {
    Q_snprintf(cmdLine, sizeof(cmdLine), "\"%s\\%s\\bin\\hlmv.exe\"",
               GetSDKLauncherBinDirectory(), g_engineDir);
    QuickLaunchCommandLine(cmdLine);
    return true;
  } else if (CommandLine()->FindParm("-runfaceposer")) {
    Q_snprintf(cmdLine, sizeof(cmdLine), "\"%s\\%s\\bin\\hlfaceposer.exe\"",
               GetSDKLauncherBinDirectory(), g_engineDir);
    QuickLaunchCommandLine(cmdLine);
    return true;
  }
...
void CheckCreateModParameters() {
  if (CommandLine()->FindParm("-AutoHL2Mod")) g_bAutoHL2Mod = true;

  size_t iParm = CommandLine()->FindParm("-CreateMod");
  if (iParm == 0) return;
  ...
  if ((iParm + 2) < CommandLine()->ParmCount()) {
    ...
    Q_strncpy(g_ModWizard_CmdLine_ModDir, CommandLine()->GetParm(iParm + 1),
              sizeof(g_ModWizard_CmdLine_ModDir));
    Q_strncpy(g_ModWizard_CmdLine_ModName, CommandLine()->GetParm(iParm + 2),
              sizeof(g_ModWizard_CmdLine_ModName));
...
bool CSDKLauncherApp::PreInit() {
  ...
  g_pFullFileSystem->AddSearchPath(GetSDKLauncherBaseDirectory(),
                                   SDKLAUNCHER_MAIN_PATH_ID);
