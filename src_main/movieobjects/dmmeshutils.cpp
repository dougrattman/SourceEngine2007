...
bool CDmMeshUtils::RemoveLargeAxisAlignedPlanarFaces(CDmeMesh *pMesh) {
  ...
  const CUtlVector<Vector> &posData(
      CDmrArrayConst<Vector>(pBase->GetVertexData(posIndex)).Get());
  if (posData.Count() <= 0) return false;

  const CUtlVector<int> &posIndices(
      CDmrArrayConst<int>(pBase->GetIndexData(posIndex)).Get());
  if (posIndices.Count() <= 0) return false;
  ...
  for (int i = 0; i < nFaceSets; ++i) {
    ...
    if (nFaceIndices <= 0) continue;

    faceStartIndex = 0;

    faceCurrentIndex = pFaceSet->GetIndex(0);
    if (faceCurrentIndex < 0) continue;
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::RemoveFacesWithMaterial(CDmeMesh *pMesh,
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::RemoveFacesWithMoreThanNVerts(CDmeMesh *pMesh,
...
//-----------------------------------------------------------------------------
void ComputeVertexIndexMap(CDmeMesh *pMesh, int nMaxVertexCount,
                           CUtlVector<int> &vertexIndexMap) {
  bool *pVertexFound =
      reinterpret_cast<bool *>(alloca(nMaxVertexCount * sizeof(bool)));
  ...
  int *pReverseVertexIndexMap =
      reinterpret_cast<int *>(alloca(nMaxVertexCount * sizeof(int)));
  for (int i = 0; i < nFaceSetCount; ++i) {
  ...
  // Fix up the face set indices to compensate for the ones which are going to
  // be removed
  for (int i = 0; i < nFaceSetCount; ++i) {
    CDmeFaceSet *pFaceSet = pMesh->GetFaceSet(i);
    const int nFaceSetIndices = pFaceSet->NumIndices();
...
//-----------------------------------------------------------------------------
template <class T_t>
void RemapData(CDmrArray<T_t> data, const CUtlVector<int> &newToOldMap) {
  const int nNewToOldMapCount = newToOldMap.Count();

  T_t *pNewData =
      reinterpret_cast<T_t *>(alloca(nNewToOldMapCount * sizeof(T_t)));
...
//-----------------------------------------------------------------------------
void RemoveUnusedData(CDmeMesh *pMesh, CDmeVertexData *pVertexData, bool bBind,
                      const char *pFieldName, int *pIndices, int nIndicesCount,
                      const CDmrGenericArray &data) {
  const int nDataCount = data.Count();

  bool *pDataIndexFound =
      reinterpret_cast<bool *>(alloca(nDataCount * sizeof(bool)));
  ...
  int *pOldToNewDataMap =
      reinterpret_cast<int *>(alloca(nDataCount * sizeof(int)));
  for (int i = 0; i < nDataCount; ++i) {
...
void RemoveUnusedVerticesFromBaseState(
    CDmeMesh *pMesh, CDmeVertexData *pVertexData,
    const CUtlVector<int> &newToOldIndexMap) {
  const int nNewToOldIndexMapCount = newToOldIndexMap.Count();
  int *pNewVertexIndices =
      reinterpret_cast<int *>(alloca(nNewToOldIndexMapCount * sizeof(int)));
  ...
  for (int i = 0; i < nFieldCount; ++i) {
    ...
    // TODO: Checking by name is lame... should be a lookup to map fieldIndex to
    // a standard field index
    if (!Q_strcmp(pFieldName, "jointWeights") ||
        !Q_strcmp(pFieldName, "jointIndices")) {
    ...
    RemoveUnusedData(pMesh, pVertexData, bBind, pFieldName, pNewVertexIndices,
                     nNewToOldIndexMapCount,
                     CDmrGenericArray(pVertexData->GetVertexData(i)));

    // Shrink the indices array
    indices.RemoveMultiple(nNewToOldIndexMapCount,
                           indices.Count() - nNewToOldIndexMapCount);
...
bool CDmMeshUtils::PurgeUnusedData(CDmeMesh *pMesh) {
  ...
  for (int i = pMesh->BaseStateCount() - 1; i >= 0; --i) {
    RemoveUnusedVerticesFromBaseState(pMesh, pMesh->GetBaseState(i),
                                      vertexIndexMap);
  }
...
bool CDmMeshUtils::Mirror(CDmeMesh *pMesh, int axis /*= kXAxis */) {
  ...
  if (!MirrorVertices(pMesh, pBase, axis, mirrorMap)) return false;
  ...
  for (int i = 0; i < nFaceSets; ++i) {
    ...
    if (nNameLen >= 2) {
      ...
      if (!Q_stricmp(pSrcMaterialName + nNameLen - 2, "_l")) {
        ...
        materialName += "_l";
                 ...
                 !Q_stricmp(pSrcMaterialName + nNameLen - 5, "_left")) {
        materialName = pSrcMaterialName;
        materialName.SetLength(nNameLen - 5);
        materialName += "_right";
      } else if (nNameLen >= 6 &&
                 !Q_stricmp(pSrcMaterialName + nNameLen - 6, "_right")) {
        materialName = pSrcMaterialName;
      ...
      if (materialName.Length()) {
        pDstFaceSet =
            CreateElement<CDmeFaceSet>(materialName, pMesh->GetFileId());
        CDmeMaterial *pDstMaterial =
            CreateElement<CDmeMaterial>(materialName, pDstFaceSet->GetFileId());
    ...
    for (int j = 0; j < nFaceSetIndices; ++j) {
      ...
      if (vertexIndex < 0) {
        ...
        pDstFaceSet->SetIndices(oldNumIndices, newFaceIndices.Count(),
                                newFaceIndices.Base());
...
//-----------------------------------------------------------------------------
const int *CDmMeshUtils::BuildDataMirrorMap(
    CDmeVertexData *pBase, int axis,
    CDmeVertexData::StandardFields_t standardField,
    CUtlVector<int> &dataMirrorMap) {
  const FieldIndex_t fieldIndex = pBase->FindFieldIndex(standardField);
  if (fieldIndex < 0) return NULL;

  const CUtlVector<int> &indices(
      CDmrArrayConst<int>(pBase->GetIndexData(fieldIndex)).Get());

  CDmAttribute *pData = pBase->GetVertexData(fieldIndex);
  if (standardField == CDmeVertexData::FIELD_POSITION ||
      standardField == CDmeVertexData::FIELD_NORMAL) {
    ...
    const CUtlVector<Vector2D> &data(
        CDmrArrayConst<Vector2D>(pBase->GetVertexData(fieldIndex)).Get());
    const int nDataCount = data.Count();
    dataMirrorMap.SetCount(nDataCount);
...
// version of vertex
//-----------------------------------------------------------------------------
bool CDmMeshUtils::MirrorVertices(CDmeMesh *pMesh, CDmeVertexData *pBase,
                                  int axis, CUtlVector<int> &mirrorMap) {
  mirrorMap.RemoveAll();
  ...
  if (!BuildDataMirrorMap(pBase, axis, CDmeVertexData::FIELD_POSITION,
                          posMirrorMap))
    return false;

  CUtlVector<int> normalMirrorMap;
  const int *pNormalIndices = BuildDataMirrorMap(
      pBase, axis, CDmeVertexData::FIELD_NORMAL, normalMirrorMap);

  CUtlVector<int> uvMirrorMap;
  const int *pUVIndices = BuildDataMirrorMap(
      pBase, axis, CDmeVertexData::FIELD_TEXCOORD, uvMirrorMap);
  ...
  int mirrorCount = 0;
  {
    ...
    for (int i = 0; i < nIndices; ++i) {
      ...
      if (pNormalIndices &&
          normalMirrorMap[pNormalIndices[i]] != pNormalIndices[i]) {
        mirror = true;
      }
  ...
  for (int i = 0; i < nBaseState; ++i) {
    ...
    MirrorVertices(pBase, axis, nVertexCount, mirrorCount, mirrorMap,
                   posMirrorMap, normalMirrorMap, uvMirrorMap);
  }
...
//-----------------------------------------------------------------------------
template <class T_t>
void MirrorVertexData(CDmeVertexData *pBase, FieldIndex_t fieldIndex, int axis,
                      int nOrigVertexCount, int nMirrorCount,
  ...
  T_t *pMirrorData =
      reinterpret_cast<T_t *>(alloca(nMirrorCount * sizeof(T_t)));
  int *pMirrorIndices =
      reinterpret_cast<int *>(alloca(nMirrorCount * sizeof(int)));
  ...
  for (int i = 0; i < nOrigVertexCount; ++i) {
    if (mirrorMap[i] != i) {
      ...
      if (dataMirrorMap[origIndices[i]] != origIndices[i]) {
        // Data referred to by vertex i must be mirror (this may be done a
        // redundant number of times)
  ...
  const DmAttributeType_t dmAttributeType =
      ArrayTypeToValueType(origData.GetAttribute()->GetType());

  pBase->AddVertexData(fieldIndex, nMirrorDataCount);
  pBase->SetVertexData(fieldIndex, nData, nMirrorDataCount, dmAttributeType,
                       pMirrorData);

  pBase->SetVertexIndices(fieldIndex, nOrigVertexCount, nMirrorCount,
                          pMirrorIndices);
}
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::MirrorVertices(CDmeVertexData *pBase, int axis,
                                  int nOldVertexCount, int nMirrorCount,
                                  const CUtlVector<int> &mirrorMap,
                                  const CUtlVector<int> &posMirrorMap,
                                  const CUtlVector<int> &normalMirrorMap,
                                  const CUtlVector<int> &uvMirrorMap) {
  if (!pBase || axis < kXAxis || axis > kZAxis) return false;

  pBase->AddVertexIndices(nMirrorCount);

  const int posFieldIndex =
      pBase->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
  const int normalFieldIndex =
      pBase->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
  const int uvFieldIndex =
      pBase->FindFieldIndex(CDmeVertexData::FIELD_TEXCOORD);
  ...
  for (int i = 0; i < nFields; ++i) {
    ...
    switch (pBaseData->GetType()) {
      case AT_VECTOR2_ARRAY:
        if (i == uvFieldIndex) {
          MirrorVertexData(pBase, i, axis % 2, nOldVertexCount, nMirrorCount,
                           CDmrArrayConst<Vector2D>(pBaseData), baseIndices,
                           mirrorMap, uvMirrorMap);
          continue;
        }
        break;
      case AT_VECTOR3_ARRAY:
        if (i == posFieldIndex) {
          MirrorVertexData(pBase, i, axis, nOldVertexCount, nMirrorCount,
                           CDmrArrayConst<Vector>(pBaseData), baseIndices,
                           mirrorMap, posMirrorMap);
          continue;
        } else if (i == normalFieldIndex) {
          MirrorVertexData(pBase, i, axis, nOldVertexCount, nMirrorCount,
                           CDmrArrayConst<Vector>(pBaseData), baseIndices,
                           mirrorMap, normalMirrorMap);
          continue;
        }
        break;
      default:
    ...
    MirrorVertices(pBase, i, nOldVertexCount, nMirrorCount, baseIndices,
                   mirrorMap);
  }
...
//-----------------------------------------------------------------------------
void CDmMeshUtils::MirrorVertices(CDmeVertexData *pBase,
                                  FieldIndex_t fieldIndex, int nOldVertexCount,
                                  int nMirrorCount,
...
//-----------------------------------------------------------------------------
template <class T_t>
void MirrorDeltaData(CDmeVertexDeltaData *pDelta, FieldIndex_t fieldIndex,
                     int axis, const CDmrArrayConst<T_t> &origData,
                     const CUtlVector<int> &origIndices,
  ...
  T_t *pMirrorData =
      reinterpret_cast<T_t *>(alloca(nOrigDataCount * sizeof(T_t)));
  int *pMirrorIndices =
      reinterpret_cast<int *>(alloca(nOrigDataCount * sizeof(int)));
  ...
  pDelta->SetVertexData(fieldIndex, nOrigDataCount, nMirrorDataCount,
                        dmAttributeType, pMirrorData);
  pDelta->SetVertexIndices(fieldIndex, nOrigDataCount, nMirrorDataCount,
                           pMirrorIndices);
}
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::MirrorDelta(CDmeVertexDeltaData *pDelta, int axis,
                               const CUtlVector<int> &posMirrorMap,
                               const CUtlVector<int> &normalMirrorMap,
                               const CUtlVector<int> &uvMirrorMap) {
  if (!pDelta || axis < kXAxis || axis > kZAxis) return false;

  const int posFieldIndex =
      pDelta->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
  const int normalFieldIndex =
      pDelta->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
  const int uvFieldIndex =
      pDelta->FindFieldIndex(CDmeVertexData::FIELD_TEXCOORD);
  ...
  for (int i = 0; i < nFields; ++i) {
    ...
    switch (pDeltaData->GetType()) {
      case AT_VECTOR2_ARRAY:
        if (i == uvFieldIndex) {
          MirrorDeltaData(pDelta, i, axis % 2,
                          CDmrArrayConst<Vector2D>(pDeltaData), deltaIndices,
                          uvMirrorMap);
          continue;
        }
        break;
      case AT_VECTOR3_ARRAY:
        if (i == posFieldIndex) {
          MirrorDeltaData(pDelta, i, axis, CDmrArrayConst<Vector>(pDeltaData),
                          deltaIndices, posMirrorMap);
          continue;
        } else if (i == normalFieldIndex) {
          MirrorDeltaData(pDelta, i, axis, CDmrArrayConst<Vector>(pDeltaData),
                          deltaIndices, normalMirrorMap);
          continue;
        }
        break;
      default:
...
//-----------------------------------------------------------------------------
bool CDmMeshUtils::RemapMaterial(CDmeMesh *pMesh, const CUtlString &src,
...
//-----------------------------------------------------------------------------
bool CDmMeshUtils::RemapMaterial(CDmeMesh *pMesh, const int nMaterialIndex,
                                 const CUtlString &dst) {
  const int nFaceSets = pMesh->FaceSetCount();
  if (nMaterialIndex >= nFaceSets) return false;

  CDmeFaceSet *pFaceSet = pMesh->GetFaceSet(nMaterialIndex);
  if (!pFaceSet) return false;
...
int CDmMeshUtils::FindMergeSocket(
  ...
  for (int i = srcBorderEdgesList.Count() - 1; i >= 0; --i) {
    const CUtlVector<CDmMeshComp::CEdge *> &srcBorderEdges =
        srcBorderEdgesList[i];

    int nEdgeMatch = 0;

    for (int j = edgeList.Head(); j != edgeList.InvalidIndex();
         j = edgeList.Next(j)) {
...
bool CDmMeshUtils::Merge(CDmeMesh *pSrcMesh, CDmElement *pRoot) {
  ...
  if (srcComp.GetBorderEdges(srcBorderEdgesList) == 0) return false;
  ...
  while (traverseStack.Count()) {
    ...
    for (int nChildIndex = pDag->GetChildCount() - 1; nChildIndex >= 0;
         --nChildIndex) {
  ...
  Msg("Error: Merge() - No Merge Socket Found - i.e. A Set Of Border Edges On "
      "The Source Model That Are Found On The Merge Model");
...
//-----------------------------------------------------------------------------
template <class T_t>
void AppendData(const CDmrArrayConst<T_t> &srcData, CDmrArray<T_t> &dstData,
...
//-----------------------------------------------------------------------------
template <>
void AppendData(const CDmrArrayConst<Vector> &srcData,
                CDmrArray<Vector> &dstData, const matrix3x4_t *pMat) {
  const int nSrcCount = srcData.Count();
  const int nDstCount = dstData.Count();
...
//-----------------------------------------------------------------------------
int MergeBaseState(CDmeVertexData *pSrcBase, CDmeVertexData *pDstBase,
                   const matrix3x4_t &pMat, const matrix3x4_t &nMat,
                   int nSkinningJointIndex, int &nPositionOffset,
                   int &nNormalOffset, int &nWrinkleOffset) {
  int nRetVal = -1;

  const int nSrcPositionIndex =
      pSrcBase->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
  const int nSrcNormalIndex =
      pSrcBase->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
  const int nSrcWrinkleIndex =
      pSrcBase->FindFieldIndex(CDmeVertexData::FIELD_WRINKLE);
  const int nSrcJointWeightsIndex =
      pSrcBase->FindFieldIndex(CDmeVertexData::FIELD_JOINT_WEIGHTS);
  const int nSrcJointIndicesIndex =
      pSrcBase->FindFieldIndex(CDmeVertexData::FIELD_JOINT_INDICES);

  const int nDstJointWeightsIndex =
      pDstBase->FindFieldIndex(CDmeVertexData::FIELD_JOINT_WEIGHTS);
  const int nDstJointIndicesIndex =
      pDstBase->FindFieldIndex(CDmeVertexData::FIELD_JOINT_INDICES);
  ...
  // specific joint specified to skin to, the first joint is used and a warning
  // issued

  if (nDstJointWeightsIndex >= 0 && nDstJointIndicesIndex >= 0) {
    if (nSkinningJointIndex < 0) {
      Msg("Warning: Destination mesh is skinned but no valid joint specified "
          "to skin to, using first joint\n");
  ...
  for (int i = 0; i < pSrcBase->FieldCount(); ++i) {
    ...
    for (int j = 0; j < pDstBase->FieldCount(); ++j) {
      if (i == nSrcJointWeightsIndex || i == nSrcJointIndicesIndex ||
          Q_strcmp(pSrcBase->FieldName(i), pDstBase->FieldName(j)))
        continue;
      ...
      switch (pSrcData->GetType()) {
        case AT_FLOAT_ARRAY:
          AppendData(CDmrArrayConst<float>(pSrcData),
                     CDmrArray<float>(pDstData));
          break;
        case AT_VECTOR2_ARRAY:
          AppendData(CDmrArrayConst<Vector2D>(pSrcData),
                     CDmrArray<Vector2D>(pDstData));
          break;
        case AT_VECTOR3_ARRAY:
          if (i == nSrcPositionIndex) {
            AppendData(CDmrArrayConst<Vector>(pSrcData),
                       CDmrArray<Vector>(pDstData), &pMat);
          } else if (i == nSrcNormalIndex) {
            AppendData(CDmrArrayConst<Vector>(pSrcData),
                       CDmrArray<Vector>(pDstData), &nMat);
          } else {
            AppendData(CDmrArrayConst<Vector>(pSrcData),
                       CDmrArray<Vector>(pDstData));
          }
          break;
        case AT_VECTOR4_ARRAY:
          AppendData(CDmrArrayConst<Vector4D>(pSrcData),
                     CDmrArray<Vector4D>(pDstData));
          break;
        case AT_QUATERNION_ARRAY:
          AppendData(CDmrArrayConst<Quaternion>(pSrcData),
                     CDmrArray<Quaternion>(pDstData));
          break;
        case AT_COLOR_ARRAY:
          AppendData(CDmrArrayConst<Color>(pSrcData),
                     CDmrArray<Color>(pDstData));
          break;
        default:
          Assert(0);
          break;
  ...
  const int nDstSpeedIndex =
      pDstBase->FindFieldIndex(CDmeVertexData::FIELD_MORPH_SPEED);
  ...
  for (int i = 0; i < pDstBase->FieldCount(); ++i) {
    ...
    for (int j = 0; j < pSrcBase->FieldCount(); ++j) {
      if (Q_strcmp(pDstBase->FieldName(i), pSrcBase->FieldName(j))) continue;
    ...
    if (!bFound) {
      ...
      if (i == nDstSpeedIndex) {
        // Pad data with a 1
        nDstIndex =
            CDmrArray<float>(pDstBase->GetVertexData(i)).AddToTail(1.0f);
      } else {
        // Pad data with a 0
...
//
//-----------------------------------------------------------------------------
void MergeDeltaState(CDmeVertexDeltaData *pSrcDelta,
                     CDmeVertexDeltaData *pDstDelta, int &nPositionOffset,
                     int &nNormalOffset, int &nWrinkleOffset) {
  const int nSrcPositionIndex =
      pSrcDelta->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
  const int nSrcNormalIndex =
      pSrcDelta->FindFieldIndex(CDmeVertexData::FIELD_NORMAL);
  const int nSrcWrinkleIndex =
      pSrcDelta->FindFieldIndex(CDmeVertexData::FIELD_WRINKLE);

  for (int i = 0; i < pSrcDelta->FieldCount(); ++i) {
    bool bFound = false;

    for (int j = 0; j < pDstDelta->FieldCount(); ++j) {
      if (Q_strcmp(pSrcDelta->FieldName(i), pDstDelta->FieldName(j))) continue;
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::Merge(CDmeMesh *pSrcMesh, CDmeMesh *pDstMesh,
                         int nSkinningJointIndex) {
  if (!pSrcMesh || !pDstMesh) return false;
  ...
  for (int i = 0; i < pSrcMesh->BaseStateCount(); ++i) {
    ...
    for (int j = 0; j < pDstMesh->BaseStateCount(); ++j) {
      ...
      bMerged = true;
      const int nTmpVertexOffset =
          MergeBaseState(pSrcBase, pDstBase, pMat, nMat, nSkinningJointIndex,
                         nPositionOffset, nNormalOffset, nWrinkleOffset);
      if (nVertexOffset < 0) {
    ...
    if (!bMerged) {
      Msg("Error: Merge( %s, %s ) - Can't Find Base State %s On %s\n",
          pSrcMesh->GetName(), pDstMesh->GetName(), pSrcBase->GetName(),
          pDstMesh->GetName());
    }
  }
  ...
  for (int i = 0; i < pSrcMesh->DeltaStateCount(); ++i) {
    ...
    for (int j = 0; j < pDstMesh->DeltaStateCount(); ++j) {
      ...
      if (Q_strcmp(pSrcDelta->GetName(), pDstDelta->GetName())) continue;

      bMerged = true;
      MergeDeltaState(pSrcDelta, pDstDelta, nPositionOffset, nNormalOffset,
                      nWrinkleOffset);
    }

    if (!bMerged) {
      MergeDeltaState(pSrcDelta,
                      pDstMesh->FindOrCreateDeltaState(pSrcDelta->GetName()),
                      nPositionOffset, nNormalOffset, nWrinkleOffset);
    }
  }
...
struct VertexWeightMap_s {
  struct VertexWeight_s {
    int m_vertexDataIndex;  // Index into the CDmeVertexData data (only used for
                            // joint weights & indices)
    const CUtlVector<int>
        *m_pVertexIndices;  // Index into the CDmeVertexData vertex indices
...
//
//-----------------------------------------------------------------------------
bool CopyJointWeights(CDmeVertexData *pSrcData, CDmeVertexData *pDstData,
                      const CUtlVector<VertexWeightMap_s> &vertexWeightMap) {
  const int nJointCount = pSrcData->GetValue<int>("jointCount");

  const FieldIndex_t nSrcJointWeightsField =
      pSrcData->FindFieldIndex(CDmeVertexData::FIELD_JOINT_WEIGHTS);
  const FieldIndex_t nSrcJointIndicesField =
      pSrcData->FindFieldIndex(CDmeVertexData::FIELD_JOINT_INDICES);

  if (nJointCount <= 0 || nSrcJointWeightsField < 0 ||
      nSrcJointIndicesField < 0)
    return false;

  const CUtlVector<float> &srcJointWeights =
      CDmrArrayConst<float>(pSrcData->GetVertexData(nSrcJointWeightsField))
          .Get();
  const float *const pSrcJointWeights = srcJointWeights.Base();

  const CUtlVector<int> &srcJointIndices =
      CDmrArrayConst<int>(pSrcData->GetVertexData(nSrcJointIndicesField)).Get();
  ...
  pDstData->CreateJointWeightsAndIndices(nJointCount, &nDstJointWeightsField,
                                         &nDstJointIndicesField);

  const int nDstCount = vertexWeightMap.Count();

  float *pDstJointWeights = reinterpret_cast<float *>(
      alloca(nDstCount * nJointCount * sizeof(float)));
  memset(pDstJointWeights, 0, nDstCount * nJointCount);

  int *pDstJointIndices =
      reinterpret_cast<int *>(alloca(nDstCount * nJointCount * sizeof(int)));
  ...
  for (int i = 0; i < nDstCount; ++i) {
    ...
    if (nVertexWeights > 0) {
      ...
      memcpy(pDstJointWeights + i * nJointCount,
             pSrcJointWeights + nMatchIndex * nJointCount,
             nJointCount * sizeof(float));
      memcpy(pDstJointIndices + i * nJointCount,
             pSrcJointIndices + nMatchIndex * nJointCount,
             nJointCount * sizeof(int));
    }
  }

  pDstData->AddVertexData(nDstJointIndicesField, nDstCount * nJointCount);
  pDstData->SetVertexData(nDstJointIndicesField, 0, nDstCount * nJointCount,
                          AT_INT, pDstJointIndices);

  pDstData->AddVertexData(nDstJointWeightsField, nDstCount * nJointCount);
  pDstData->SetVertexData(nDstJointWeightsField, 0, nDstCount * nJointCount,
                          AT_FLOAT, pDstJointWeights);
...
//-----------------------------------------------------------------------------
CDmeMesh *ReplaceMesh(CDmeMesh *pSrcMesh, CDmeMesh *pDstMesh) {
  if (!pSrcMesh || !pDstMesh) return NULL;
  ...
  CDmeCombinationOperator *pSrcComboOp =
      FindReferringElement<CDmeCombinationOperator>(pSrcMesh, "targets");
  if (pSrcComboOp) {
    CDmeCombinationOperator *pDstComboOp =
        FindReferringElement<CDmeCombinationOperator>(pDstMesh, "targets");
    CDmElement *pDstRoot = NULL;
    if (pDstComboOp) {
      // Find the root the easy way
      pDstRoot =
          FindReferringElement<CDmElement>(pDstComboOp, "combinationOperator");
      ...
      for (;;) {
        ...
        if (pNextDag) {
          ...
          // Can't find anyone referring to this via "children" so, hopefully
          // it's the DmeModel referred to by "model"
...
//-----------------------------------------------------------------------------
template <class T_t>
void CopyFieldData(const CDmrArrayConst<T_t> &srcData,
                   const CUtlVector<int> &srcIndices,
                   CDmeVertexData *pDstVertexData, FieldIndex_t dstFieldIndex,
                   const CUtlVector<VertexWeightMap_s> &vertexWeightMap) {
  ...
  for (int i = 0; i < nDstData; ++i) {
    ...
    for (int j = 0; j < vertexWeight.m_nVertexWeights; ++j) {
      const VertexWeightMap_s::VertexWeight_s &vWeight =
          vertexWeight.m_vertexWeights[j];
  ...
  const DmAttributeType_t dmAttributeType =
      ArrayTypeToValueType(srcData.GetAttribute()->GetType());
  CDmrArray<T_t> dstData(pDstVertexData->GetVertexData(dstFieldIndex));
  dstData.EnsureCount(nDstData);
  pDstVertexData->SetVertexData(dstFieldIndex, 0, nDstData, dmAttributeType,
                                pDstData);
}
...
//
//-----------------------------------------------------------------------------
bool CopyField(CDmeVertexData::StandardFields_t field, CDmeVertexData *pSrcData,
               CDmeVertexData *pDstData,
               const CUtlVector<VertexWeightMap_s> &vertexWeightMap) {
  FieldIndex_t srcFieldIndex = pSrcData->FindFieldIndex(field);
  if (srcFieldIndex < 0) return false;

  FieldIndex_t dstFieldIndex = pDstData->CreateField(field);
  if (dstFieldIndex < 0) return false;

  CDmAttribute *pSrcVertexData = pSrcData->GetVertexData(srcFieldIndex);
  const CUtlVector<int> &srcIndices =
      pSrcData->GetVertexIndexData(srcFieldIndex);

  // Everything on dst has to be indexed the same as position
  const CUtlVector<int> &dstPosIndices =
      pDstData->GetVertexIndexData(CDmeVertexData::FIELD_POSITION);
  CDmrArray<int> dstIndices(pDstData->GetIndexData(dstFieldIndex));
  dstIndices.EnsureCount(dstPosIndices.Count());
  pDstData->SetVertexIndices(dstFieldIndex, 0, dstPosIndices.Count(),
                             dstPosIndices.Base());

  switch (pSrcVertexData->GetType()) {
    case AT_FLOAT_ARRAY:
      CopyFieldData(CDmrArrayConst<float>(pSrcVertexData), srcIndices, pDstData,
                    dstFieldIndex, vertexWeightMap);
      break;
    case AT_VECTOR2_ARRAY:
      CopyFieldData(CDmrArrayConst<Vector2D>(pSrcVertexData), srcIndices,
                    pDstData, dstFieldIndex, vertexWeightMap);
      break;
    case AT_VECTOR3_ARRAY:
      CopyFieldData(CDmrArrayConst<Vector>(pSrcVertexData), srcIndices,
                    pDstData, dstFieldIndex, vertexWeightMap);
      break;
    default:
      break;
...
//
//-----------------------------------------------------------------------------
bool CDmMeshUtils::Merge(CDmMeshComp &srcComp,
  ...
  for (int i = 0; i < nSrcCount; ++i) {
    ...
    for (int j = 0; j < nDstCount; ++j) {
      ...
      if (vSrc.DistToSqr(vDst) < FLT_EPSILON * 10.0f) {
        ...
        vertexWeight.m_vertexWeights[0].m_pVertexIndices =
            &pDstData->FindVertexIndicesFromDataIndex(
                CDmeVertexData::FIELD_POSITION, j);
        break;
      }
    ...
    if (vertexWeight.m_nVertexWeights == 0) {
      Warning(
          "Warning: Merge() - No Match For Src Vertex: %f %f %f, Using "
          "Closest: %f %f %f\n",
          vSrc.x, vSrc.y, vSrc.z, dstPosData[nClosestIndex].x,
          dstPosData[nClosestIndex].y, dstPosData[nClosestIndex].z);
      ...
      vertexWeight.m_vertexWeights[0].m_pVertexIndices =
          &pDstData->FindVertexIndicesFromDataIndex(
              CDmeVertexData::FIELD_POSITION, nClosestIndex);
  ...
  if (pNewData) {
    ...
    CopyField(CDmeVertexData::FIELD_BALANCE, pDstData, pNewData,
              vertexWeightMap);

    CopyField(CDmeVertexData::FIELD_MORPH_SPEED, pDstData, pNewData,
              vertexWeightMap);

    if (pNewData->FindFieldIndex(CDmeVertexData::FIELD_MORPH_SPEED) >= 0) {
      CDmeCombinationOperator *pComboOp(
          FindReferringElement<CDmeCombinationOperator>(pNewMesh, "targets"));
      if (pComboOp) {
...
//
//-----------------------------------------------------------------------------
bool CreateExpressionFile(const char *pExpressionFile,
                          const CUtlVector<CUtlString> *pPurgeAllButThese,
                          CDmeCombinationOperator *pComboOp,
                          CDmePresetGroup *pPresetGroup) {
  if (!pPresetGroup) return false;
  ...
  CDmePresetGroup *pDstPresetGroup =
      CreateElement<CDmePresetGroup>(expName, CreateUniqueFileId());
  if (!pDstPresetGroup) return false;
  ...
  for (int i = 0; i < nPresetsCount; ++i) {
    ...
    const CDmaElementArray<CDmElement> &controlValues =
        pPreset->GetControlValues();
    ...
    for (int j = 0; j < nControlCount; ++j) {
      ...
      if (!Q_strcmp(pControlName, pPresetName)) {
        ...
        // These shouldn't really happen because these are presets which were
        // made into deltas so they are never stereo nor multi-controls
        if (bStereo) {
      ...
      for (int k = 0; k < nControlValueCount; ++k) {
        ...
        if (!Q_strcmp(pControlName, pControlPreset->GetName())) {
          pDstControlValue = pDstPreset->FindOrAddControlValue(pControlName);
          pDstControlValue->SetValue("value",
                                     pControlPreset->GetValue("value", 0.0f));
          if (bStereo) {
            pDstControlValue->SetValue(
                "balance", pControlPreset->GetValue("balance", 0.5f));
          }
          if (bMulti) {
            pDstControlValue->SetValue(
                "multilevel", pControlPreset->GetValue("multilevel", 0.5f));
          }
          break;
        }
      }
      ...
      if (!pDstControlValue) {
        pDstControlValue = pDstPreset->FindOrAddControlValue(pControlName);
        pDstControlValue->SetValue("value",
                                   pComboOp->GetControlDefaultValue(j));
...
bool CDmMeshUtils::CreateDeltasFromPresets(
    CDmeMesh *pMesh, CDmeVertexData *pPassedDst,
    const CUtlStringMap<CUtlString> &presetExpressionMap, bool bPurge,
  ...
  if (!pDst || !pBind || pDst == pBind) return false;

  CDmeCombinationOperator *pComboOp =
      FindReferringElement<CDmeCombinationOperator>(pMesh, "targets");
  if (!pComboOp) return false;
  ...
  for (int i = 0; i < presetExpressionMap.GetNumStrings(); ++i) {
    ...
    CreateDeltasFromPresetGroup(pPresetGroup, pComboOp, pPurgeAllButThese,
                                pMesh, pDst, conflictingNames, presetMap);
  }
    ...
    if (nControlIndex < 0) {
      ...
      if (bPurge) {
        ...
        for (int j = 0; j < nRawControls; ++j) {
          if (!Q_strcmp(pComboOp->GetRawControlName(nControlIndex, j),
                        pPresetName)) {
            bFound = true;
            break;
          }
  ...
  for (int i = 0; i < presetExpressionMap.GetNumStrings(); ++i) {
    ...
    if (expressionFile.IsEmpty()) continue;

    CreateExpressionFile(expressionFile.Get(), pPurgeAllButThese, pComboOp,
                         presetGroups[i]);
  }
...
bool CDmMeshUtils::PurgeUnusedDeltas(CDmeMesh *pMesh) {
  ...
  for (int i = 0; i < nControlCount; ++i) {
    ...
    for (int j = 0; j < nRawControlCount; ++j) {
      const int nDeltaIndex =
          pMesh->FindDeltaStateIndex(pCombo->GetRawControlName(i, j));
...
bool CDmMeshUtils::CreateWrinkleDeltaFromBaseState(
    CDmeVertexDeltaData *pDelta, float flScale /* = 1.0f */,
    CDmeMesh *pPassedMesh /* = NULL */,
    CDmeVertexData *pPassedBind /* = NULL */,
    CDmeVertexData *pPassedCurrent /* = NULL */) {
  CDmeVertexData *pBind =
      pPassedBind ? pPassedBind
                  : pPassedMesh ? pPassedMesh->GetBindBaseState() : NULL;
  CDmeVertexData *pCurr =
      pPassedCurrent ? pPassedCurrent
                     : pPassedMesh ? pPassedMesh->GetCurrentBaseState() : NULL;

  const CDmeMesh *pMesh =
      pPassedMesh
          ? pPassedMesh
          : pBind ? FindReferringElement<CDmeMesh>(pBind, "baseStates") : NULL;
  const CDmeMesh *pBindMesh =
      pBind ? FindReferringElement<CDmeMesh>(pBind, "baseStates") : NULL;
  const CDmeMesh *pCurrMesh =
      pCurr ? FindReferringElement<CDmeMesh>(pCurr, "baseStates") : NULL;
  const CDmeMesh *pDeltaMesh =
      pDelta ? FindReferringElement<CDmeMesh>(pDelta, "deltaStates") : NULL;

  if (!pDelta || !pBind || !pCurr || pBind == pCurr || !pMesh ||
      pMesh != pBindMesh || pMesh != pCurrMesh || pMesh != pDeltaMesh) {
    return false;
  }

  const FieldIndex_t nBindPosIndex =
      pBind->FindFieldIndex(CDmeVertexData::FIELD_POSITION);
  const FieldIndex_t nBindTexIndex =
      pBind->FindFieldIndex(CDmeVertexData::FIELD_TEXCOORD);
  const FieldIndex_t nCurrPosIndex =
      pCurr->FindFieldIndex(CDmeVertexData::FIELD_POSITION);

  if (nBindPosIndex < 0 || nBindTexIndex < 0 || nCurrPosIndex < 0) return false;

  const CUtlVector<Vector> &bindPos =
      CDmrArrayConst<Vector>(pBind->GetVertexData(nBindPosIndex)).Get();
  const CUtlVector<Vector> &currPos =
      CDmrArrayConst<Vector>(pCurr->GetVertexData(nCurrPosIndex)).Get();
  const CUtlVector<int> &baseTexCoordIndices =
      pBind->GetVertexIndexData(nBindTexIndex);

  const int nPosCount = bindPos.Count();
  if (nPosCount != currPos.Count()) return false;

  FieldIndex_t nWrinkleIndex =
      pDelta->FindFieldIndex(CDmeVertexDeltaData::FIELD_WRINKLE);
  if (nWrinkleIndex < 0) {
    nWrinkleIndex = pDelta->CreateField(CDmeVertexDeltaData::FIELD_WRINKLE);
  }

  pDelta->RemoveAllVertexData(nWrinkleIndex);
  if (flScale == 0.0f) return true;

  float flMaxDeflection = 0.0f;
  int *pWrinkleIndices =
      reinterpret_cast<int *>(alloca(nPosCount * sizeof(int)));
  float *pWrinkleDelta =
      reinterpret_cast<float *>(alloca(nPosCount * sizeof(float)));
  int nWrinkleCount = 0;
  ...
  unsigned char *const pUsedBits = reinterpret_cast<unsigned char *>(
      alloca(nBufSize * sizeof(unsigned char)));
  memset(pUsedBits, 0, nBufSize);

  for (int i = 0; i < nWrinkleCount; ++i) {
    float flWrinkleDelta = static_cast<float>(
        static_cast<double>(pWrinkleDelta[i]) * scaledInverseMaxDeflection);
    Assert(fabs(flWrinkleDelta) <= fabs(flScale));

    // NOTE: This will produce bad behavior in cases where two positions share
    // the same texcoord, which shouldn't theoretically happen.
    const CUtlVector<int> &baseVerts = pBind->FindVertexIndicesFromDataIndex(
        CDmeVertexData::FIELD_POSITION, pWrinkleIndices[i]);
    ...
    for (int j = 0; j < nBaseVertCount; ++j) {
      ...
      pDelta->SetVertexData(nWrinkleIndex, nDeltaIndex, 1, AT_FLOAT,
                            &flWrinkleDelta);
    }
  }
...
//
//-----------------------------------------------------------------------------
CDmMeshFaceIt::CDmMeshFaceIt(const CDmeMesh *pMesh,
                             const CDmeVertexData *pVertexData /* = NULL */) {
...
//
//-----------------------------------------------------------------------------
bool CDmMeshFaceIt::Reset(const CDmeMesh *pMesh,
                          const CDmeVertexData *pVertexData /* = NULL */) {
  m_nFaceIndex = 0;
  ...
  if (pMesh) {
    ...
    m_pVertexData =
        pVertexData
            ? pVertexData
            : m_pVertexData ? m_pVertexData : m_pMesh->GetCurrentBaseState();
    ...
    if (m_nFaceSetCount <= 0) return false;

    // Get number of faces in current mesh
    for (m_nFaceSetIndex = 0; m_nFaceSetIndex < m_nFaceSetCount;
         ++m_nFaceSetIndex) {
  ...
  // Set indices to point to first index of first face of first face set,
  // accounting for NULL face sets and NULL faces
  for (m_nFaceSetIndex = 0; m_nFaceSetIndex < m_nFaceSetCount;
       ++m_nFaceSetIndex) {
    if (SetFaceSet()) return true;
  }
...
bool CDmMeshFaceIt::Next() {
  ...
  while (m_nFaceSetIndex < m_nFaceSetCount) {
    ...
    while (m_nFaceSetIndexIndex < m_nFaceSetIndexCount) {
      if (m_pFaceSet->GetIndex(m_nFaceSetIndexIndex) >= 0) break;
...
bool CDmMeshFaceIt::SetFaceSet() {
  ...
  for (m_nFaceSetIndexIndex = 0; m_nFaceSetIndexIndex < m_nFaceSetIndexCount;
       ++m_nFaceSetIndexIndex) {
    if (m_pFaceSet->GetIndex(m_nFaceSetIndex) >= 0) return true;
  }
...
int CDmMeshFaceIt::GetVertexIndex(int nFaceRelativeVertexIndex) const {
  ...
  if (nVertexCount <= 0 || nFaceRelativeVertexIndex < 0 ||
      nFaceRelativeVertexIndex >= nVertexCount)
    return -1;

  int *pVertexIndices =
      reinterpret_cast<int *>(alloca(nVertexCount * sizeof(int)));
  if (!GetVertexIndices(pVertexIndices, nVertexCount)) return -1;
...
void CDmMeshUtils::CreateDeltasFromPresetGroup(
    CDmePresetGroup *pPresetGroup, CDmeCombinationOperator *pComboOp,
    const CUtlVector<CUtlString> *pPurgeAllButThese, CDmeMesh *pMesh,
    CDmeVertexData *pDst, CUtlStringMap<CUtlString> &conflictingNames,
    CUtlStringMap<CDmePreset *> &presetMap) {
  const CDmaElementArray<CDmePreset> &presets = pPresetGroup->GetPresets();
  const int nPresetsCount = presets.Count();
  ...
  for (int i = 0; i < nPresetsCount; ++i) {
    ...
    for (int j = 0; j < nControlValues; ++j) {
      ...
      const ControlIndex_t nControlIndex =
          pComboOp->FindControlIndex(pControlPreset->GetName());
      if (nControlIndex < 0) continue;

      bool bSkip = false;

      if (pPurgeAllButThese) {
        for (int k = 0; k < pPurgeAllButThese->Count(); ++k) {
          if (!Q_strcmp(pControlPreset->GetName(),
                        pPurgeAllButThese->Element(k).Get())) {
            bSkip = true;
          }
        }
      }
      ...
      if (pComboOp->IsStereoControl(nControlIndex)) {
        pComboOp->SetControlValue(
            nControlIndex, pControlPreset->GetValue<float>("value", 0.0),
      ...
      if (pComboOp->IsMultiControl(nControlIndex)) {
        pComboOp->SetMultiControlLevel(
            nControlIndex, pControlPreset->GetValue<float>("multilevel", 0.5));
      }
    }
    ...
    for (int presetSuffix = 1;
         pComboOp->FindControlIndex(presetName) >= 0 ||
         pMesh->FindDeltaState(presetName) != NULL ||
         conflictingNames.Defined(presetName) || presetMap.Defined(presetName);
         ++presetSuffix) {
...
//
//-----------------------------------------------------------------------------
void CDmMeshUtils::PurgeUnreferencedDeltas(
    CDmeMesh *pMesh, CUtlStringMap<CDmePreset *> &presetMap,
    const CUtlVector<CUtlString> *pPurgeAllButThese,
    CDmeCombinationOperator *pComboOp) {
    ...
    for (int i = 0; i < pMesh->DeltaStateCount(); ++i) {
      ...
      if (pPurgeAllButThese) {
        for (int j = 0; j < pPurgeAllButThese->Count(); ++j) {
          ...
          const ControlIndex_t nControlIndex =
              pComboOp->FindControlIndex(pPurgeAllButThese->Element(j));
          if (nControlIndex < 0) continue;

          for (int k = 0; k < pComboOp->GetRawControlCount(nControlIndex);
               ++k) {
            if (!Q_strcmp(pDeltaStateName,
                          pComboOp->GetRawControlName(nControlIndex, k))) {
              bDelete = false;
              break;
            }
          }
  ...
  for (int i = 0; i < presetMap.GetNumStrings(); ++i) {
    ...
    if (Q_strcmp(pPreset->GetName(), pPresetName)) {
      const ControlIndex_t nOrigIndex =
          pComboOp->FindControlIndex(pPreset->GetName());
      const ControlIndex_t nRenamedIndex =
          pComboOp->FindControlIndex(pPresetName);
      CDmeVertexDeltaData *pOrigDelta =
          pMesh->FindDeltaState(pPreset->GetName());
      CDmeVertexDeltaData *pRenamedDelta = pMesh->FindDeltaState(pPresetName);

      if (nOrigIndex < 0 && nRenamedIndex >= 0 && pOrigDelta == NULL &&
          pRenamedDelta != NULL) {
