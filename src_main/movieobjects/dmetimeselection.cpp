...
void CDmeTimeSelection::OnConstruction() {
  ...
  m_nFalloffInterpolatorType[0].InitAndSet(this, "interpolator_left",
                                           INTERPOLATE_LINEAR_INTERP);
  m_nFalloffInterpolatorType[1].InitAndSet(this, "interpolator_right",
                                           INTERPOLATE_LINEAR_INTERP);

  m_threshold.InitAndSet(this, "threshold", 0.0005f);

  m_nRecordingState.InitAndSet(
      this, "recordingstate",
      3 /*AS_PLAYBACK :  HACK THIS SHOULD MOVE TO A PUBLIC HEADER*/);
}
...
float CDmeTimeSelection::AdjustFactorForInterpolatorType(float factor,
                                                         int side) {
  Vector points[4];
  points[0].Init();
  ...
  Vector out;
  Interpolator_CurveInterpolate(GetFalloffInterpolatorType(side),
                                points[0],  // unused
                                points[1], points[2],
                                points[3],  // unused
                                factor, out);
...
float CDmeTimeSelection::GetAmountForTime(DmeTime_t t, DmeTime_t curtime) {
  ...
  if (t >= times[0] && t < times[1]) {
    ...
    Interpolator_CurveInterpolate(GetFalloffInterpolatorType(0),
                                  points[0],  // unused
                                  points[1], points[2],
                                  points[3],  // unused
                                  f, out);
  ...
  if (t > times[2] && t <= times[3]) {
    ...
    Interpolator_CurveInterpolate(GetFalloffInterpolatorType(1),
                                  points[0],  // unused
                                  points[1], points[2],
                                  points[3],  // unused
                                  f, out);
...
void CDmeTimeSelection::GetAlphaForTime(DmeTime_t t, DmeTime_t curtime,
                                        u8& alpha) {
  Assert(IsEnabled());
  ...
  if (dt1 > DmeTime_t(0) && t >= times[0] && t < times[1]) {
    float frac = GetFractionOfTime(t - times[0], dt1);
    alpha = std::clamp((u8)(alpha * frac), minAlpha, (u8)255);
    return;
  }
  if (dt2 > DmeTime_t(0) && t > times[2] && t <= times[3]) {
...
void CDmeTimeSelection::SetFalloffInterpolatorType(int side,
...
void CDmeTimeSelection::SetAbsFalloff(DmeTime_t time, int side,
                                      DmeTime_t absfallofftime) {
  DmeTime_t newTime;
  if (m_bRelative) {
...
void CDmeTimeSelection::SetAbsHold(DmeTime_t time, int side,
                                   DmeTime_t absholdtime) {
  DmeTime_t newTime;
  if (m_bRelative) {
...
float CDmeTimeSelection::GetThreshold() { return m_threshold; }
