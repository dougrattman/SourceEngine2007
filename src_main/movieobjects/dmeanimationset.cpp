...
int CDmePreset::FindControlValueIndex(const char *pControlName) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!Q_stricmp(e->GetName(), pControlName)) return i;
  }
  return -1;
}
...
bool CDmePreset::IsReadOnly() {
  DmAttributeReferenceIterator_t h =
      g_pDataModel->FirstAttributeReferencingElement(GetHandle());
  while (h != DMATTRIBUTE_REFERENCE_ITERATOR_INVALID) {
    CDmAttribute *pAttribute = g_pDataModel->GetAttribute(h);
    CDmePresetGroup *pOwner =
        CastElement<CDmePresetGroup>(pAttribute->GetOwner());
    if (pOwner && pOwner->m_bIsReadOnly) return true;
...
void CDmePreset::CopyControlValuesFrom(CDmePreset *pSource) {
  ...
  const CDmaElementArray<CDmElement> &sourceValues =
      pSource->GetControlValues();
...
int CDmePreset::GetProceduralPresetType() const { return m_nProceduralType; }

IMPLEMENT_ELEMENT_FACTORY(DmeProceduralPresetSettings,
                          CDmeProceduralPresetSettings);
...
const char *CDmePresetRemap::FindSourcePreset(const char *pDestPresetName) {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(pDestPresetName, m_DestPresets[i])) return m_SrcPresets[i];
  }
  return NULL;
}

void CDmePresetRemap::AddRemap(const char *pSourcePresetName,
                               const char *pDestPresetName) {
...
// Iteration
//-----------------------------------------------------------------------------
int CDmePresetRemap::GetRemapCount() { return m_SrcPresets.Count(); }

const char *CDmePresetRemap::GetRemapSource(int i) { return m_SrcPresets[i]; }

const char *CDmePresetRemap::GetRemapDest(int i) { return m_DestPresets[i]; }
...
CDmePreset *CDmePresetGroup::FindPreset(const char *pPresetName) {
  ...
  for (i = 0; i < c; ++i) {
    CDmePreset *e = m_Presets.Get(i);
    if (!Q_stricmp(e->GetName(), pPresetName)) return e;
  }
  return NULL;
}

CDmePreset *CDmePresetGroup::FindOrAddPreset(
    const char *pPresetName, int nType /*=PROCEDURAL_PRESET_NOT*/) {
...
// Reorder presets
//-----------------------------------------------------------------------------
void CDmePresetGroup::MovePresetInFrontOf(CDmePreset *pPreset,
                                          CDmePreset *pInFrontOf) {
  if (pPreset == pInFrontOf) return;
...
//-----------------------------------------------------------------------------
static int FindExportedControlIndex(
    const char *pControlName, CUtlVector<ExportedControl_t> &uniqueControls) {
  int nCount = uniqueControls.Count();
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(pControlName, uniqueControls[i].m_Name)) return i;
  }
  return -1;
}
...
// Builds a unique list of controls found in the presets
//-----------------------------------------------------------------------------
static int BuildExportedControlList(
    CDmeAnimationSet *pAnimationSet, const CDmePresetGroup *pPresetGroup,
    CUtlVector<ExportedControl_t> &uniqueControls) {
  int nGlobalIndex = 0;
  ...
  for (int i = 0; i < nPresetCount; ++i) {
    ...
    for (int k = 0; k < nControlCount; ++k) {
      const char *pControlName = controls[k]->GetName();
      int nIndex = FindExportedControlIndex(pControlName, uniqueControls);
      if (nIndex >= 0) continue;
      CDmAttribute *pValueAttribute = controls[k]->GetAttribute("value");
      if (!pValueAttribute || pValueAttribute->GetType() != AT_FLOAT) continue;
      ...
      if (pAnimationSet) {
        ...
        ExportedControl_t &control = uniqueControls[j];
        control.m_Name = pControlName;
        // this isn't quite as reliable as querying the animation set but if we
        // don't have one...
        control.m_bIsStereo =
            controls[k]->GetAttribute("balance") ? true : false;
        control.m_bIsMulti =
            controls[k]->GetAttribute("multilevel") ? true : false;
        control.m_nFirstIndex = nGlobalIndex;
        nGlobalIndex += 1 + control.m_bIsStereo + control.m_bIsMulti;
      }
    }
...
//-----------------------------------------------------------------------------
bool CDmePresetGroup::ExportToTXT(
    const char *pFileName, CDmeAnimationSet *pAnimationSet /* = NULL */,
    CDmeCombinationOperator *pComboOp /* = NULL */) const {
  ...
  for (int i = 0; i < nPresetCount; ++i) {
    ...
    for (int k = 0; k < nExportedControlCount; ++k) {
      ExportedControl_t &control = exportedControls[k];
      CDmElement *pControlValue = pPreset->FindControlValue(control.m_Name);
      if (!pControlValue) {
        CDmElement *pControl =
            pAnimationSet ? pAnimationSet->FindControl(control.m_Name) : NULL;
        if (!pControl) {
          const ControlIndex_t nIndex =
              pComboOp ? pComboOp->FindControlIndex(control.m_Name) : -1;
          if (nIndex >= 0) {
            buf.Printf("%.5f\t0.000\t",
                       pComboOp->GetControlDefaultValue(nIndex));
            if (control.m_bIsStereo) {
              buf.Printf("%.5f\t0.000\t",
                         pComboOp->GetControlDefaultValue(nIndex));
            }
        ...
        if (!control.m_bIsStereo) {
          buf.Printf("%.5f\t1.000\t",
                     pControl->GetValue<float>("defaultValue"));
        } else {
...
#define ALIGN4(a) a = (u8 *)((int)((u8 *)a + 3) & ~3)

//-----------------------------------------------------------------------------
// Exports this preset group to a faceposer .vfe expression file
//-----------------------------------------------------------------------------
bool CDmePresetGroup::ExportToVFE(
    const char *pFileName, CDmeAnimationSet *pAnimationSet /* = NULL */,
    CDmeCombinationOperator *pComboOp /* = NULL */) const {
  const CDmePresetGroup *pPresetGroup = this;
  ...
  if (!g_pFullFileSystem->FullPathToRelativePathEx(
          pFileName, "GAME", fhdr->name, sizeof(fhdr->name))) {
  ...
  for (i = 0; i < fhdr->numflexsettings; i++) {
    ...
    pSetting[i].index = i;
    pSetting[i].settingindex = pData - (u8 *)(&pSetting[i]);
  ...
  for (i = 0; i < fhdr->numflexsettings; i++) {
    ...
    pSetting[i].nameindex = pData - (u8 *)(&pSetting[i]);
...
int CDmeAnimationSet::FindPresetGroupIndex(const char *pGroupName) {
  ...
  for (int i = 0; i < c; ++i) {
    CDmePresetGroup *e = m_PresetGroups.Get(i);
    if (e && !Q_stricmp(e->GetName(), pGroupName)) return i;
  }
  return -1;
}
...
CDmePresetGroup *CDmeAnimationSet::FindPresetGroup(const char *pGroupName) {
  ...
  if (nIndex >= 0) return m_PresetGroups[nIndex];
  return NULL;
}
...
// Find or add by name
//-----------------------------------------------------------------------------
CDmePresetGroup *CDmeAnimationSet::FindOrAddPresetGroup(
...
//-----------------------------------------------------------------------------
void CDmeAnimationSet::MovePresetGroupInFrontOf(CDmePresetGroup *pPresetGroup,
                                                CDmePresetGroup *pInFrontOf) {
  if (pPresetGroup == pInFrontOf) return;
  ...
  int nEnd =
      pInFrontOf ? FindPresetGroupIndex(pInFrontOf) : m_PresetGroups.Count();
...
CDmePreset *CDmeAnimationSet::FindOrAddPreset(
    const char *pGroupName, const char *pPresetName,
    int nType /*=PROCEDURAL_PRESET_NOT*/) {
...
bool CDmeAnimationSet::RemovePreset(CDmePreset *pPreset) {
  int c = m_PresetGroups.Count();
  for (int i = 0; i < c; ++i) {
    if (m_PresetGroups[i]->RemovePreset(pPreset)) return true;
  }
  return false;
}
...
void CDmeAnimationSet::RestoreDefaultPhonemeMap() {
  ...
  for (i = 0; i < phonemeCount; ++i) {
    ...
    CDmePhonemeMapping *mapping =
        CreateElement<CDmePhonemeMapping>(pName, GetFileId());
    char presetName[256];
...
CDmePhonemeMapping *CDmeAnimationSet::FindMapping(const char *pRawPhoneme) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!Q_stricmp(e->GetName(), pRawPhoneme)) return e;
  }
  return NULL;
}
...
CDmElement *CDmeAnimationSet::FindControl(const char *pControlName) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!Q_stricmp(e->GetName(), pControlName)) return e;
  }
  return NULL;
}
...
CDmElement *CDmeAnimationSet::FindSelectionGroup(
    const char *pSelectionGroupName) {
  int c = m_SelectionGroups.Count();
  for (int i = 0; i < c; ++i) {
    CDmElement *e = m_SelectionGroups.Get(i);
    if (!Q_stricmp(e->GetName(), pSelectionGroupName)) return e;
  }
  return NULL;
}

CDmElement *CDmeAnimationSet::FindOrAddSelectionGroup(
    const char *pSelectionGroupName) {
  CDmElement *pSelectionGroup = FindSelectionGroup(pSelectionGroupName);
  if (!pSelectionGroup) {
    // Create the default groups in order
    pSelectionGroup =
        CreateElement<CDmElement>(pSelectionGroupName, GetFileId());
...
void CDmeAnimationSet::CollectOperators(
    CUtlVector<DmElementHandle_t> &operators) {
...
class CModelPresetGroupManager : public IModelPresetGroupManager {
 public:
  ...
  virtual void ApplyModelPresets(const char *pModelName,
...
// Loads model presets from .pre files matching the model name
//-----------------------------------------------------------------------------
void CModelPresetGroupManager::LoadModelPresets(const char *pModelName,
  ...
  for (; pFileName; pFileName = g_pFullFileSystem->FindNext(fh)) {
    ...
    DmFileId_t fileid = g_pDataModel->RestoreFromFile(
        pRelativePresetPath, "GAME", NULL, &pRoot, CR_FORCE_COPY);
    if (fileid == DMFILEID_INVALID || !pRoot) continue;
...
//-----------------------------------------------------------------------------
void CModelPresetGroupManager::ApplyModelPresets(
    const char *pModelName, CDmeAnimationSet *pAnimationSet) {
  if (m_FileId == DMFILEID_INVALID) {
