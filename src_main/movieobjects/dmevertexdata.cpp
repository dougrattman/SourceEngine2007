...
static const char *g_pStandardFieldNames[] = {
    "positions", "normals",      "tangents",     "textureCoordinates",
    "colors",    "jointWeights", "jointIndices", "balance",
    "speed",     "wrinkle",      "weight"};

static DmAttributeType_t g_pStandardFieldTypes[] = {
    AT_VECTOR3_ARRAY, AT_VECTOR3_ARRAY, AT_VECTOR4_ARRAY, AT_VECTOR2_ARRAY,
    AT_COLOR_ARRAY,   AT_FLOAT_ARRAY,   AT_INT_ARRAY,     AT_FLOAT_ARRAY,
    AT_FLOAT_ARRAY,   AT_FLOAT_ARRAY,   AT_FLOAT_ARRAY};
...
void CDmeVertexDataBase::UpdateStandardFieldInfo(int nFieldIndex,
  ...
  static_assert(std::size(g_pStandardFieldNames) == STANDARD_FIELD_COUNT);
  static_assert(std::size(g_pStandardFieldTypes) == STANDARD_FIELD_COUNT);
...
void CDmeVertexDataBase::ComputeFieldInfo() {
  ...
  for (int i = 0; i < nFieldCount; ++i) {
    ...
    if (!pVerticesArray || !IsArrayType(pVerticesArray->GetType())) continue;
    ...
    if (Q_stricmp(pFieldName, g_pStandardFieldNames[FIELD_JOINT_WEIGHTS]) &&
      ...
      if (!pIndicesArray || pIndicesArray->GetType() != AT_INT_ARRAY) continue;
    }
    ...
    if (nFieldIndex < 0) {
      ...
      UpdateStandardFieldInfo(nFieldIndex, pFieldName,
                              pVerticesArray->GetType());
    }
...
void CDmeVertexDataBase::ComputeVertexCount() {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (!m_FieldInfo[i].m_pIndexData) continue;
...
void CDmeVertexDataBase::Resolve() {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (m_FieldInfo[i].m_pIndexData &&
        m_FieldInfo[i].m_pIndexData->IsFlagSet(FATTRIB_DIRTY)) {
...
const Vector &CDmeVertexDataBase::GetPosition(int nIndex) const {
  ...
  if (nFieldIndex < 0) return vec3_origin;
...
const float *CDmeVertexDataBase::GetJointWeights(int nVertexIndex) const {
  ...
  if (nPosFieldIndex < 0 || nFieldIndex < 0) return NULL;
...
//-----------------------------------------------------------------------------
const float *CDmeVertexDataBase::GetJointPositionWeights(
    int nPositionIndex) const {
  Assert(!IsVertexDeltaData());
  FieldIndex_t nFieldIndex = m_pStandardFieldIndex[FIELD_JOINT_WEIGHTS];
  if (nFieldIndex < 0) return NULL;
...
// the vertex index to access the data
//-----------------------------------------------------------------------------
const int *CDmeVertexDataBase::GetJointPositionIndices(
    int nPositionIndex) const {
  Assert(!IsVertexDeltaData());
  FieldIndex_t nJointIndicesField = m_pStandardFieldIndex[FIELD_JOINT_INDICES];
  if (nJointIndicesField < 0) return NULL;
...
const Vector4D &CDmeVertexDataBase::GetTangent(int nIndex) const {
  ...
  if (nFieldIndex < 0) return vec4_origin;
...
const Vector2D &CDmeVertexDataBase::GetTexCoord(int nIndex) const {
  ...
  if (nFieldIndex < 0) return vec2_origin;
...
const Color &CDmeVertexDataBase::GetColor(int nIndex) const {
  ...
  if (nFieldIndex < 0) return s_Black;
...
void CDmeVertexDataBase::FindOrAddVertexField(const char *pFieldName) {
  ...
  for (i = 0; i < nFormatCount; ++i) {
    if (!Q_stricmp(pFieldName, m_VertexFormat[i])) return;
  }
...
//-----------------------------------------------------------------------------
FieldIndex_t CDmeVertexDataBase::CreateField(const char *pFieldName,
...
FieldIndex_t CDmeVertexDataBase::CreateField(StandardFields_t fieldId) {
  return CreateField(g_pStandardFieldNames[fieldId],
                     g_pStandardFieldTypes[fieldId]);
}

//-----------------------------------------------------------------------------
// Use this to create vertex fields for joint weights + indices
//-----------------------------------------------------------------------------
void CDmeVertexDataBase::CreateJointWeightsAndIndices(
    int nJointCount, FieldIndex_t *pJointWeightsField,
  ...
  ComputeFieldInfo();
  *pJointWeightsField =
      FindFieldIndex(g_pStandardFieldNames[FIELD_JOINT_WEIGHTS]);
  *pJointIndicesField =
      FindFieldIndex(g_pStandardFieldNames[FIELD_JOINT_INDICES]);
}
...
// Sets vertex data
//-----------------------------------------------------------------------------
void CDmeVertexDataBase::SetVertexData(FieldIndex_t nFieldIndex,
                                       int nFirstVertex, int nCount,
...
void CDmeVertexDataBase::SetVertexIndices(FieldIndex_t nFieldIndex,
...
FieldIndex_t CDmeVertexDataBase::FindFieldIndex(const char *pFieldName) const {
  int nCount = m_FieldInfo.Count();
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(m_FieldInfo[i].m_Name, pFieldName)) return i;
  }
  return -1;
}
...
const float *CDmeVertexDataBase::GetJointWeightData(int nDataIndex) const {
  FieldIndex_t nFieldIndex = m_pStandardFieldIndex[FIELD_JOINT_WEIGHTS];
  if (nFieldIndex < 0 || nFieldIndex >= m_FieldInfo.Count()) return NULL;
...
const CUtlVector<int> &CDmeVertexDataBase::GetVertexIndexData(
    FieldIndex_t nFieldIndex) const {
  if (nFieldIndex < 0 || nFieldIndex >= m_FieldInfo.Count()) return s_EmptyInt;
...
const CUtlVector<int> &CDmeVertexDataBase::GetVertexIndexData(
    StandardFields_t fieldId) const {
...
//-----------------------------------------------------------------------------
const CUtlVector<int> &CDmeVertexDataBase::FindVertexIndicesFromDataIndex(
    FieldIndex_t nFieldIndex, int nDataIndex) {
  if (nFieldIndex < 0) return s_EmptyInt;
...
const CUtlVector<int> &CDmeVertexDataBase::FindVertexIndicesFromDataIndex(
    StandardFields_t fieldId, int nDataIndex) {
  // NOTE! Wrinkles don't exist in the base state, therefore we use the index to
  // index into the TEXCOORD base state fields instead of the wrinkle fields
  if (fieldId == FIELD_WRINKLE) {
    fieldId = FIELD_TEXCOORD;
  }

  return FindVertexIndicesFromDataIndex(m_pStandardFieldIndex[fieldId],
                                        nDataIndex);
}
...
//-----------------------------------------------------------------------------
// Do we need tangent data? (Utility method for applications to know if they
// should call ComputeDefaultTangentData)
//-----------------------------------------------------------------------------
bool CDmeVertexDataBase::NeedsTangentData() const {
  FieldIndex_t posField =
      m_pStandardFieldIndex[CDmeVertexDataBase::FIELD_POSITION];
  FieldIndex_t normalField =
      m_pStandardFieldIndex[CDmeVertexDataBase::FIELD_NORMAL];
  FieldIndex_t uvField =
      m_pStandardFieldIndex[CDmeVertexDataBase::FIELD_TEXCOORD];
  FieldIndex_t tangentField =
      m_pStandardFieldIndex[CDmeVertexDataBase::FIELD_TANGENT];
  return (posField >= 0 && uvField >= 0 && normalField >= 0 &&
          tangentField < 0);
}
...
//
//-----------------------------------------------------------------------------
int CDmeVertexDataBase::FieldCount() const { return m_VertexFormat.Count(); }

//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
const char *CDmeVertexDataBase::FieldName(int i) const {
  if (i < 0 || i >= m_VertexFormat.Count()) return NULL;
...
//-----------------------------------------------------------------------------
void CDmeVertexDeltaData::GenerateWrinkleDelta(CDmeVertexData *pBindState,
                                               float flScale, bool bOverwrite) {
  FieldIndex_t nPosIndex = FindFieldIndex(FIELD_POSITION);
  if (nPosIndex < 0) return;

  FieldIndex_t nBaseTexCoordIndex = pBindState->FindFieldIndex(FIELD_TEXCOORD);
  if (nBaseTexCoordIndex < 0) return;
  ...
  if (flScale == 0.0f) return;

  const float flMaxDeflection(ComputeMaxDeflection());
  if (flMaxDeflection == 0.0f) return;
  ...
  const CUtlVector<int> &baseTexCoordIndices =
      pBindState->GetVertexIndexData(nBaseTexCoordIndex);

  CDmrArrayConst<Vector2D> texData(
      pBindState->GetVertexData(nBaseTexCoordIndex));
  ...
  for (int i = 0; i < nCount; ++i) {
    float flWrinkleDelta = static_cast<float>(
        static_cast<double>(pos[i].Length()) * scaledInverseMaxDeflection);
    ...
    const CUtlVector<int> &baseVerts =
        pBindState->FindVertexIndicesFromDataIndex(FIELD_POSITION,
                                                   positionIndices[i]);
...
float CDmeVertexDeltaData::GenerateWeightDelta(CDmeVertexData *pBindState) {
  ...
  if (maxDeflection == 0.0) return maxDeflection;
