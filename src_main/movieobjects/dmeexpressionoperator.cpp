...
// (although we'll want to implement elerp as the standard 3x^2 - 2x^3 with
// rescale)
...
bool ParseString(const char *&expr, const char *str) {
  const char *startExpr = expr;
  while ((*expr == ' ') || (*expr == '\t')) expr++;  // skip whitespace
...
bool ParseStringList(const char *&expr, const char **pOps, int &nOp) {
  while (nOp--) {
    if (ParseString(expr, pOps[nOp])) return true;
  }
  return false;
}

bool ParseStringList(const char *&expr, const CUtlVector<CUtlString> &strings,
                     int &nOp) {
  while (nOp--) {
    if (ParseString(expr, strings[nOp])) return true;
  }
  return false;
}
...
int FindString(const CUtlVector<CUtlString> &strings, const char *str) {
  ...
  for (u32 si = 0; si < sn; ++si) {
    if (!Q_strcmp(str, strings[si])) return si;
  }
  return -1;
}
...
//-----------------------------------------------------------------------------
int CExpressionCalculator::VariableCount() { return m_varNames.Count(); }
...
bool CExpressionCalculator::ParseConditional(const char *&expr) {
  ...
  if (ParseString(expr, "?") && ParseExpr(expr) && ParseString(expr, ":") &&
      ParseExpr(expr)) {
...
bool CExpressionCalculator::ParseOr(const char *&expr) {
  ...
  ParseState_t ps1(m_stack, expr);
  if (ParseString(expr, "||") && ParseOr(expr)) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
    m_stack.Pop();
...
bool CExpressionCalculator::ParseAnd(const char *&expr) {
  ...
  ParseState_t ps1(m_stack, expr);
  if (ParseString(expr, "&&") && ParseAnd(expr)) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
    m_stack.Pop();
...
bool CExpressionCalculator::ParseEquality(const char *&expr) {
  ...
  ParseState_t ps1(m_stack, expr);
  if (ParseStringList(expr, pOps, nOp) && ParseEquality(expr)) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
    m_stack.Pop();
...
bool CExpressionCalculator::ParseLessGreater(const char *&expr) {
  ...
  ParseState_t ps1(m_stack, expr);
  if (ParseStringList(expr, pOps, nOp) && ParseLessGreater(expr)) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
    m_stack.Pop();
...
bool CExpressionCalculator::ParseAddSub(const char *&expr) {
  ...
  ParseState_t ps1(m_stack, expr);
  if (ParseStringList(expr, pOps, nOp) && ParseAddSub(expr)) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
    m_stack.Pop();
...
bool CExpressionCalculator::ParseDivMul(const char *&expr) {
  ...
  ParseState_t ps1(m_stack, expr);
  if (ParseStringList(expr, pOps, nOp) && ParseDivMul(expr)) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
    m_stack.Pop();
...
bool CExpressionCalculator::ParseUnary(const char *&expr) {
  ...
  if (ParseStringList(expr, pOps, nOp) && ParseUnary(expr)) {
    float f1 = m_stack.Top();
    m_stack.Pop();
    switch (nOp) {
      case 0:  // +
  ...
  if (ParsePrimary(expr)) return true;
...
bool CExpressionCalculator::ParsePrimary(const char *&expr) {
  ...
  ps.Reset(m_stack, expr);
  if (Parse1ArgFunc(expr) || Parse2ArgFunc(expr) || Parse3ArgFunc(expr) ||
      //		Parse4ArgFunc( expr ) ||
      Parse5ArgFunc(expr)) {
    return true;
  }
  ...
  if (!m_bIsBuildingArgumentList) return false;
...
bool CExpressionCalculator::Parse1ArgFunc(const char *&expr) {
  ...
  const char *pFuncs[] = {"abs",   "sqr",     "sqrt",  "sin", "asin", "cos",
                          "acos",  "tan",     "exp",   "log", "dtor", "rtod",
                          "floor", "ceiling", "round", "sign"};
  int nFunc = 16;

  if (ParseStringList(expr, pFuncs, nFunc) && ParseString(expr, "(") &&
      ParseExpr(expr) && ParseString(expr, ")")) {
    float f1 = m_stack.Top();
    m_stack.Pop();
    switch (nFunc) {
      case 0:  // abs
...
bool CExpressionCalculator::Parse2ArgFunc(const char *&expr) {
  ...
  if (ParseStringList(expr, pFuncs, nFunc) && ParseString(expr, "(") &&
      ParseExpr(expr) && ParseString(expr, ",") && ParseExpr(expr) &&
      ParseString(expr, ")")) {
    float f2 = m_stack.Top();
    m_stack.Pop();
    float f1 = m_stack.Top();
...
noise(a,b,c) : { solid noise pattern (improved perlin noise) indexed with three
numbers }
*/
...
bool CExpressionCalculator::Parse3ArgFunc(const char *&expr) {
  ...
  const char *pFuncs[] = {"inrange", "clamp", "ramp",  "lerp",
                          "cramp",   "clerp", "elerp", "noise"};
  int nFunc = 8;

  if (ParseStringList(expr, pFuncs, nFunc) && ParseString(expr, "(") &&
      ParseExpr(expr) && ParseString(expr, ",") && ParseExpr(expr) &&
      ParseString(expr, ",") && ParseExpr(expr) && ParseString(expr, ")")) {
    float f3 = m_stack.Top();
    m_stack.Pop();
    float f2 = m_stack.Top();
    m_stack.Pop();
    ...
    switch (nFunc) {
      ...
      case 3:  // lerp
        ...
        break;
      case 4:  // cramp
        m_stack.Push(std::clamp(ramp(f1, f2, f3), 0.0f, 1.0f));
        break;
      case 5:  // clerp
...
void TestCalculator(const char *expr, float answer, const char *var,
                    float value) {
  CExpressionCalculator calc(expr);
...
void TestCalculator() {
  ...
  TestCalculator("noise( 0.123, 4.56, 78.9 )",
                 ImprovedPerlinNoise(Vector(0.123, 4.56, 78.9)));
}
...
#if 0  // skip this test, since none of these are float attributes, but leave
...
void CDmeExpressionOperator::Operate() {
  ...
  for (CDmAttribute *pAttribute = FirstAttribute(); pAttribute;
...
void CDmeExpressionOperator::GetInputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
  for (CDmAttribute *pAttribute = FirstAttribute(); pAttribute;
       pAttribute = pAttribute->NextAttribute()) {
    if (IsInputAttribute(pAttribute)) {
...
void CDmeExpressionOperator::GetOutputAttributes(
    CUtlVector<CDmAttribute *> &attrs) {
