...
//-----------------------------------------------------------------------------
IMPLEMENT_FX_CLIP_ELEMENT_FACTORY(DmeMaterialOverlayFXClip,
                                  CDmeMaterialOverlayFXClip,
                                  "Material Overlay Effect");
...
//-----------------------------------------------------------------------------
void CDmeMaterialOverlayFXClip::ApplyEffect(
    DmeTime_t time, Rect_t &currentRect, Rect_t &totalRect,
    ITexture *pTextures[MAX_FX_INPUT_TEXTURES]) {
  if (!m_OverlayMaterial || m_Color.a() == 0) return;
  ...
  if (m_bFullScreen) {
    ...
    x = std::clamp(m_nLeft.operator const int &(), currentRect.x,
                   currentRect.x + currentRect.width);
    y = std::clamp(m_nTop.operator const int &(), currentRect.y,
                   currentRect.y + currentRect.height);
    int x1 = std::clamp(m_nLeft + m_nWidth, currentRect.x,
                        currentRect.x + currentRect.width);
    int y1 = std::clamp(m_nTop + m_nHeight, currentRect.y,
                        currentRect.y + currentRect.height);
    w = x1 - x;
    h = y1 - y;
  ...
  // Now determine the subrange we should use given we're rendering a portion of
  // the image
  float u0, v0, u1, v1, f;
  ...
  m_OverlayMaterial->ColorModulate(m_Color.r() / 255.0f, m_Color.g() / 255.0f,
                                   m_Color.b() / 255.0f);
