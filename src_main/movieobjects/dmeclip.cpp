...
DmeClipType_t ClipTypeFromString(const char *pName) {
  for (DmeClipType_t i = DMECLIP_FIRST; i <= DMECLIP_LAST; ++i) {
    if (!Q_stricmp(pName, s_pClipTypeNames[i])) return i;
  }
...
void CDmeClip::OnConstruction() {
  ...
  m_TrackGroups.Init(this, "trackGroups",
                     FATTRIB_MUSTCOPY | FATTRIB_HAS_ARRAY_CALLBACK);
}
...
//-----------------------------------------------------------------------------
void CDmeClip::OnAttributeArrayElementAdded(CDmAttribute *pAttribute,
...
void CDmeClip::OnAttributeArrayElementRemoved(CDmAttribute *pAttribute,
                                              int nFirstElem, int nLastElem) {
...
void CDmeClip::AddTrackGroupBefore(CDmeTrackGroup *pTrackGroup,
...
CDmeTrackGroup *CDmeClip::AddTrackGroup(const char *pTrackGroupName) {
  CDmeTrackGroup *pTrackGroup =
      CreateElement<CDmeTrackGroup>(pTrackGroupName, GetFileId());
...
// Swap track groups
//-----------------------------------------------------------------------------
void CDmeClip::SwapOrder(CDmeTrackGroup *pTrackGroup1,
                         CDmeTrackGroup *pTrackGroup2) {
  if (pTrackGroup1 == pTrackGroup2) return;
...
CDmeTrackGroup *CDmeClip::FindTrackGroup(const char *pTrackGroupName) const {
  ...
  for (int i = 0; i < c; ++i) {
    CDmeTrackGroup *pTrackGroup = m_TrackGroups[i];
    if (!pTrackGroup) continue;

    if (!Q_strcmp(pTrackGroup->GetName(), pTrackGroupName)) return pTrackGroup;
  }
  return NULL;
}
...
int CDmeClip::GetTrackGroupIndex(CDmeTrackGroup *pTrackGroup) const {
  ...
  for (int i = 0; i < nTrackGroups; ++i) {
    if (pTrackGroup == m_TrackGroups[i]) return i;
  }
  return -1;
}
...
//-----------------------------------------------------------------------------
CDmeTrack *CDmeClip::FindTrackForClip(
    CDmeClip *pClip, CDmeTrackGroup **ppTrackGroup /*= NULL*/) const {
...
bool CDmeClip::FindMultiTrackGroupForClip(CDmeClip *pClip,
                                          int *pTrackGroupIndex,
                                          int *pTrackIndex,
                                          int *pClipIndex) const {
  int nTrackGroups = m_TrackGroups.Count();
...
//-----------------------------------------------------------------------------
void CDmeClip::FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time,
                               DmeClipSkipFlag_t flags,
                               CUtlVector<CDmeClip *> &clips) const {
  if (clipType == DMECLIP_FILM) return;
...
void CDmeClip::FindClipsWithinTime(DmeClipType_t clipType, DmeTime_t startTime,
                                   DmeTime_t endTime, DmeClipSkipFlag_t flags,
                                   CUtlVector<CDmeClip *> &clips) const {
  if (clipType == DMECLIP_FILM) return;
  ...
  for (int i = 0; i < gc; ++i) {
    ...
    pTrackGroup->FindClipsWithinTime(clipType, startTime, endTime, flags,
                                     clips);
  }
}
...
//-----------------------------------------------------------------------------
static int BuildReferringClipList(CDmeClip *pClip, CDmeClip **ppParents,
                                  int nMaxCount) {
  int nCount = 0;
  ...
  for (it = g_pDataModel->FirstAttributeReferencingElement(pClip->GetHandle());
    ...
    for (it2 = g_pDataModel->FirstAttributeReferencingElement(
             pTrack->GetHandle());
      ...
      for (it3 = g_pDataModel->FirstAttributeReferencingElement(
               pTrackGroup->GetHandle());
...
// Clip stack
//-----------------------------------------------------------------------------
static bool BuildClipStack_R(DmeClipStack_t *pStack, CDmeClip *pMovie,
                             CDmeClip *pShot, CDmeClip *pCurrent) {
  ...
  if (pCurrent == pMovie) {
    if (!pShot) return true;
  } else {
    // NOTE: This algorithm assumes a clip can never appear twice under another
    // clip at a single level of hierarchy.
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      if (BuildClipStack_R(pStack, pMovie, pShot, ppParents[i])) return true;
    }
  }
...
bool CDmeClip::BuildClipStack(DmeClipStack_t *pStack, CDmeClip *pMovie,
                              CDmeClip *pShot) {
  // Walk through each shot in the movie and look for the subClip, if don't find
  // it recurse into each shot
  return BuildClipStack_R(pStack, pMovie, pShot, this);
}

DmeTime_t CDmeClip::ToChildMediaTime(const DmeClipStack_t &stack,
...
DmeTime_t CDmeClip::FromChildMediaTime(const DmeClipStack_t &stack,
                                       DmeTime_t localTime,
                                       bool bClamp /* = true */) {
...
DmeTime_t CDmeClip::ToChildMediaDuration(const DmeClipStack_t &stack,
                                         DmeTime_t globalDuration) {
...
DmeTime_t CDmeClip::FromChildMediaDuration(const DmeClipStack_t &stack,
                                           DmeTime_t localDuration) {
...
void CDmeClip::ToChildMediaTime(DmeLog_TimeSelection_t &params,
                                const DmeClipStack_t &stack) {
...
CDmeChannel *CDmeChannelsClip::CreatePassThruConnection(
    char const *passThruName, CDmElement *pFrom, char const *pFromAttribute,
...
void CDmeFXClip::InstallFXClipType(const char *pElementType,
...
void CDmeFilmClip::OnConstruction() {
  ...
  m_FilmTrackGroup.Init(this, "subClipTrackGroup",
                        FATTRIB_HAS_CALLBACK | FATTRIB_HAS_PRE_CALLBACK);
...
CDmeTrack *CDmeFilmClip::GetFilmTrack() const {
  ...
  if (pTrackGroup) return pTrackGroup->GetFilmTrack();
  return NULL;
}
...
CDmeTrackGroup *CDmeFilmClip::FindOrCreateFilmTrackGroup() {
  if (!m_FilmTrackGroup) {
    m_FilmTrackGroup =
        CreateElement<CDmeTrackGroup>("subClipTrackGroup", GetFileId());
...
// Finding clips in track groups
//-----------------------------------------------------------------------------
CDmeTrack *CDmeFilmClip::FindTrackForClip(
    CDmeClip *pClip, CDmeTrackGroup **ppTrackGroup /*= NULL*/) const {
...
// Finding clips in tracks by time
//-----------------------------------------------------------------------------
void CDmeFilmClip::FindClipsAtTime(DmeClipType_t clipType, DmeTime_t time,
                                   DmeClipSkipFlag_t flags,
                                   CUtlVector<CDmeClip *> &clips) const {
...
void CDmeFilmClip::FindClipsWithinTime(DmeClipType_t clipType,
                                       DmeTime_t startTime, DmeTime_t endTime,
                                       DmeClipSkipFlag_t flags,
                                       CUtlVector<CDmeClip *> &clips) const {
  if ((clipType == DMECLIP_FILM) || (clipType == DMECLIP_UNKNOWN)) {
    if (m_FilmTrackGroup) {
      m_FilmTrackGroup->FindClipsWithinTime(clipType, startTime, endTime, flags,
                                            clips);
    }
  }
...
// Volume
//-----------------------------------------------------------------------------
void CDmeFilmClip::SetVolume(float state) { m_Volume = state; }
float CDmeFilmClip::GetVolume() const { return m_Volume.Get(); }
...
void CDmeFilmClip::OnElementUnserialized() {
  ...
  // I'm hoping we don't care about any files that old - if we ever hit this, we
  // should move this code into an unserialization converter
  Assert(!HasAttribute("overlay") && !HasAttribute("overlayalpha"));
  if (HasAttribute("overlay") || HasAttribute("overlayalpha")) {
    Warning(
        "CDmeFilmClip %s is an old version that has overlay and/or "
        "overlayalpha attributes!\n",
        GetName());
    ...
    // If this is an older file with an overlay attribute, strip it out into
    // materialoverlay
    CDmAttribute *pOverlayAttribute = GetAttribute("overlay");
    if (!pOverlayAttribute) goto cleanUp;

    pName = pOverlayAttribute->GetValueString();
    if (!pName || !pName[0]) goto cleanUp;

    // If we don't yet have a material overlay, create one
    if (m_MaterialOverlayEffect.GetElement() == NULL) {
      m_MaterialOverlayEffect = CreateElement<CDmeMaterialOverlayFXClip>(
          "materialOverlay", GetFileId());
    }

    m_MaterialOverlayEffect->SetOverlayEffect(pName);

    // If this is an older file with an overlayalpha attribute, strip it out
    // into materialoverlay
...
IMaterial *CDmeFilmClip::GetOverlayMaterial() {
  return m_MaterialOverlayEffect.GetElement()
             ? m_MaterialOverlayEffect->GetMaterial()
             : NULL;
}

float CDmeFilmClip::GetOverlayAlpha() {
  return m_MaterialOverlayEffect.GetElement()
             ? m_MaterialOverlayEffect->GetAlpha()
             : 0.0f;
}
...
void CDmeFilmClip::DrawOverlay(DmeTime_t time, Rect_t &currentRect,
                               Rect_t &totalRect) {
  if (m_MaterialOverlayEffect.GetElement()) {
    m_MaterialOverlayEffect->ApplyEffect(ToChildMediaTime(time), currentRect,
                                         totalRect, NULL);
  }
...
AVIMaterial_t CDmeFilmClip::GetCachedAVI() {
  if (m_bReloadCachedVersion) {
    if (g_pAVI) {
      ...
      if (m_AVIFile[0]) {
        m_hCachedVersion =
            g_pAVI->CreateAVIMaterial(m_AVIFile, m_AVIFile, "MOD");
      }
    }
    m_bReloadCachedVersion = false;
  }
...
//-----------------------------------------------------------------------------
// Returns the monitor camera associated with the clip (for now, only 1
// supported)
//-----------------------------------------------------------------------------
CDmeCamera *CDmeFilmClip::GetMonitorCamera() {
  if (m_nActiveMonitor < 0) return NULL;
...
//-----------------------------------------------------------------------------
int CDmeFilmClip::GetLightCount() { return m_Lights.Count(); }

CDmeLight *CDmeFilmClip::GetLight(int nIndex) {
  if ((nIndex < 0) || (nIndex >= m_Lights.Count())) return NULL;

  return m_Lights[nIndex];
}

void CDmeFilmClip::AddLight(CDmeLight *pLight) { m_Lights.AddToTail(pLight); }

//-----------------------------------------------------------------------------
// Scene / Dag helper methods
//-----------------------------------------------------------------------------
CDmeDag *CDmeFilmClip::GetScene() { return m_Scene.GetElement(); }
...
CDmeInput *CDmeFilmClip::GetInput(int nIndex) {
  if (nIndex < 0 || nIndex >= m_Inputs.Count()) return NULL;
...
int CDmeFilmClip::GetAnimationSetCount() {
  // yes, this is nasty perf-wise, but since we only have a dozen or so
  // animation sets, and we're about to rewrite the entire structure of
  // animation sets vs. clips vs. scene it's not worth polluting the leaf code
  // just to save a couple cycles short term
  ...
  for (int i = 0; i < nElements; ++i) {
    ...
    if (pAnimSet) {
      ...
      const CDmAttribute *pChildren =
          pElement->GetAttribute("children", AT_ELEMENT_ARRAY);
...
CDmeAnimationSet *CDmeFilmClip::GetAnimationSet(int idx) {
  ...
  for (int i = 0; i < nElements; ++i) {
    ...
    if (pAnimSet) {
      ...
      if (idx < nChildren) return CastElement<CDmeAnimationSet>(array[idx]);
      idx -= nChildren;
    }
  }
...
void CDmeFilmClip::RemoveAllAnimationSets() { m_AnimationSets.RemoveAll(); }
...
//-----------------------------------------------------------------------------
void CDmeFilmClip::BuildClipAssociations(
    CUtlVector<ClipAssociation_t> &association, bool bHandleGaps) {
  association.RemoveAll();

  CDmeTrack *pFilmTrack = GetFilmTrack();
  if (!pFilmTrack) return;
  ...
  if (bHandleGaps) {
    pFilmTrack->FillAllGapsWithSlugs("__tempSlug__", clipStartTime,
                                     clipEndTime);
  }
  ...
  for (int i = 0; i < gc; ++i) {
    ...
    for (int j = 0; j < tc; ++j) {
      ...
      for (int k = 0; k < cc; ++k) {
        ...
        CDmeClip *pFilmClip =
            pFilmTrack->FindFilmClipAtTime(pClip->GetStartTime());

        int nIndex = association.AddToTail();
        association[nIndex].m_hClip = pClip;
        association[nIndex].m_hAssociation = pFilmClip;
        if (pFilmClip) {
          association[nIndex].m_offset =
              pClip->GetStartTime() - pFilmClip->GetStartTime();
...
//-----------------------------------------------------------------------------
void CDmeFilmClip::RollAssociatedClips(
    CDmeClip *pClip, CUtlVector<ClipAssociation_t> &association, DmeTime_t dt) {
  int c = association.Count();
  for (int i = 0; i < c; ++i) {
    if (association[i].m_nType != ClipAssociation_t::HAS_CLIP) continue;

    if (association[i].m_hAssociation.Get() != pClip) continue;
...
// Rolls associated clips so they remain in the same relative time
//-----------------------------------------------------------------------------
void CDmeFilmClip::ScaleAssociatedClips(
    CDmeClip *pClip, CUtlVector<ClipAssociation_t> &association, float ratio,
    DmeTime_t oldOffset) {
  int c = association.Count();
  for (int i = 0; i < c; ++i) {
    if (association[i].m_nType != ClipAssociation_t::HAS_CLIP) continue;
    ...
    DmeTime_t newStartTime =
        (oldStartTime - clipStartTime + oldOffset) / ratio + clipStartTime -
        pClip->GetTimeOffset();
    association[i].m_hClip->SetStartTime(newStartTime);
  }
}

void CDmeFilmClip::UpdateAssociatedClips(
    CUtlVector<ClipAssociation_t> &association) {
  int i;
  ...
  if (c > 0) {
    ...
    for (i = 0; i < c; ++i) {
      ...
      if (!curr.m_hClip.Get()) continue;

      switch (curr.m_nType) {
        case ClipAssociation_t::HAS_CLIP:
          if (curr.m_hAssociation.Get()) {
            curr.m_hClip->SetStartTime(curr.m_hAssociation->GetStartTime() +
                                       curr.m_offset);
          }
          break;
...
// Creates a slug clip
//-----------------------------------------------------------------------------
CDmeFilmClip *CreateSlugClip(const char *pClipName, DmeTime_t startTime,
                             DmeTime_t endTime, DmFileId_t fileid) {
...
CDmeTrack *GetParentTrack(CDmeClip *pClip) {
  DmAttributeReferenceIterator_t hAttr =
      g_pDataModel->FirstAttributeReferencingElement(pClip->GetHandle());
...
//-----------------------------------------------------------------------------
CDmeChannel *FindChannelTargetingElement(CDmeChannelsClip *pChannelsClip,
                                         CDmElement *pElement,
  ...
  for (int i = 0; i < nChannels; ++i) {
    ...
    if (toElement != pElement) continue;

    if (pAttributeName &&
        (Q_stricmp(pChannel->GetToAttribute()->GetName(), pAttributeName) != 0))
      continue;
...
CDmeChannel *FindChannelTargetingElement(CDmeFilmClip *pClip,
                                         CDmElement *pElement,
                                         const char *pAttributeName,
                                         CDmeChannelsClip **ppChannelsClip,
                                         CDmeTrack **ppTrack,
                                         CDmeTrackGroup **ppTrackGroup) {
  int gc = pClip->GetTrackGroupCount();
  for (int i = 0; i < gc; ++i) {
    CDmeTrackGroup *pTrackGroup = pClip->GetTrackGroup(i);
    DMETRACKGROUP_FOREACH_CLIP_TYPE_START(CDmeChannelsClip, pTrackGroup, pTrack,
                                          pChannelsClip)

    CDmeChannel *pChannel =
        FindChannelTargetingElement(pChannelsClip, pElement, pAttributeName);
