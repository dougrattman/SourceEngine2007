...
//
//-----------------------------------------------------------------------------
void CFaceSetData::Clear() { m_faceSetIndices.Clear(); }
  ...
  for (int i(0); i < nFaceSets; ++i) {
    ...
    if (faceSetIndices.Count()) {
      CDmeFaceSet *pFaceSet =
          CreateElement<CDmeFaceSet>(pName, pMesh->GetFileId());

      CDmeMaterial *pMaterial =
          CreateElement<CDmeMaterial>(pName, pMesh->GetFileId());
...
class CVertexData {
 public:
  ...
  inline void AddUV(const Vector2D &uv) {
    AddUniqueValue(uv, m_uvs, m_uvIndexMap, FLT_EPSILON * 0.1f);
  }

  inline void AddUVIndex(int i) {
    Assert(i < m_uvIndexMap.Count());
    m_uvIndices.AddToTail(m_uvIndexMap[i]);
  }

  inline int VertexCount() const { return m_pIndices.Count(); }

  CDmeVertexDataBase *AddToMesh(CDmeMesh *pMesh, bool bAbsolute,
                                const char *pName, bool bDelta);

 protected:
  template <class T_t>
  void AddUniqueValue(const T_t &v, CUtlVector<T_t> &vs, CUtlVector<int> &map,
  ...
  CDmeVertexDeltaData *AddDelta(CDmeMesh *pMesh, bool bAbsolute,
                                const char *pName = "bind");
...
//
//-----------------------------------------------------------------------------
CDmeVertexDataBase *CVertexData::AddToMesh(CDmeMesh *pMesh, bool bAbsolute,
                                           const char *pName, bool delta) {
  if (delta) return AddDelta(pMesh, bAbsolute, pName);
...
CDmeVertexDataBase *CVertexData::Add(CDmeMesh *pMesh, const char *pName) {
  ...
  if (m_positions.Count() && m_pIndices.Count()) {
    ...
    const FieldIndex_t pIndex(
        pVertexData->CreateField(CDmeVertexData::FIELD_POSITION));
    pVertexData->AddVertexData(pIndex, m_positions.Count());
    pVertexData->SetVertexData(pIndex, 0, m_positions.Count(), AT_VECTOR3,
                               m_positions.Base());
    pVertexData->SetVertexIndices(pIndex, 0, m_pIndices.Count(),
                                  m_pIndices.Base());

    if (pVertexData && m_normals.Count() && m_nIndices.Count()) {
      Assert(m_pIndices.Count() == m_nIndices.Count());

      const FieldIndex_t nIndex(
          pVertexData->CreateField(CDmeVertexData::FIELD_NORMAL));
      pVertexData->AddVertexData(nIndex, m_normals.Count());
      pVertexData->SetVertexData(nIndex, 0, m_normals.Count(), AT_VECTOR3,
                                 m_normals.Base());
      pVertexData->SetVertexIndices(nIndex, 0, m_nIndices.Count(),
                                    m_nIndices.Base());
    }
    ...
    if (pVertexData && m_uvs.Count() && m_uvIndices.Count()) {
      ...
      const FieldIndex_t uvIndex(
          pVertexData->CreateField(CDmeVertexData::FIELD_TEXCOORD));
      pVertexData->AddVertexData(uvIndex, m_uvs.Count());
      pVertexData->SetVertexData(uvIndex, 0, m_uvs.Count(), AT_VECTOR2,
                                 m_uvs.Base());
      pVertexData->SetVertexIndices(uvIndex, 0, m_uvIndices.Count(),
                                    m_uvIndices.Base());
    }
  }
...
//
//-----------------------------------------------------------------------------
CDmeVertexDeltaData *CVertexData::AddDelta(CDmeMesh *pMesh, bool bAbsolute,
                                           const char *pName) {
  ...
  if (m_positions.Count()) {
    ...
    if (pBindData.Count() != pCount) return NULL;
    ...
    const FieldIndex_t pIndex(
        pDelta->CreateField(CDmeVertexData::FIELD_POSITION));
    ...
    if (nBindNormalIndex >= 0) {
      CDmrArrayConst<Vector> bindNormalData(
          pBind->GetVertexData(nBindNormalIndex));
      ...
      if (bindNormalData.Count() == nNormalCount) {
        ...
        for (int i = 0; i < nNormalCount; ++i) {
          ...
          // Kind of a magic number but it's because of 16 bit compression of
          // the delta values
          if (fabs(n.x) >= (1 / 4096.0f) || fabs(n.y) >= (1 / 4096.0f) ||
              fabs(n.z) >= (1 / 4096.0f)) {
...
//-----------------------------------------------------------------------------
bool CDmObjSerializer::Unserialize(CUtlBuffer &buf, const char *pEncodingName,
...
//-----------------------------------------------------------------------------
CDmElement *CDmObjSerializer::ReadOBJ(const char *pFilename,
  ...
  CUtlBuffer utlBuf;
  if (!g_pFullFileSystem->ReadFile(filename, NULL, utlBuf)) return NULL;
  ...
  CDmElement *pRoot = ReadOBJ(utlBuf, DMFILEID_INVALID, baseFile, filename,
                              NULL, &pMesh, bAbsolute);
  if (pRoot && pMesh) {
    if (ppCreatedMesh) {
      *ppCreatedMesh = pMesh;
    }
    ...
    // But only if the rest of the file is <prefix>=<suffix>.obj or is
    // <prefix>_zero.obj

    char *pSuffix = Q_strrchr(baseFile, '=');
    if (!pSuffix || !*pSuffix) {
      pSuffix = Q_strrchr(baseFile, '_');
      if (!pSuffix || !*pSuffix) return pRoot;
    ...
    *(pSuffix + 1) = '*';
    *(pSuffix + 2) = '.';
    Q_strncpy(pSuffix + 3, Q_GetFileExtension(filename),
              sizeof(findGlob) - (pSuffix - findGlob + 3));
    ...
    for (const char *pFindFile(g_pFullFileSystem->FindFirst(findPath, &hFind));
         pFindFile && *pFindFile;
         pFindFile = g_pFullFileSystem->FindNext(hFind)) {
      ...
      if (Q_stricmp(baseFile, deltaFile)) {
        ...
        if (pControlName && *(pControlName + 1)) {
          ...
          for (char *pPlus(strchr(pDeltaName, '+')); pPlus;
               pPlus = strchr(pPlus, '+')) {
            *pPlus = '_';
          }
        }
        ...
        if (!strchr(pControlName, '_')) {
          if (pCombo == NULL) {
            pCombo = CreateElement<CDmeCombinationOperator>(
                "combinationOperator", pRoot->GetFileId());
...
//-----------------------------------------------------------------------------
CDmElement *CDmObjSerializer::ReadOBJ(CUtlBuffer &buf, DmFileId_t dmFileId,
                                      const char *pName,
  ...
  while (buf.IsValid()) {
    ...
    if (sscanf_s(tmpBuf0, "v %f %f %f", &p.x, &p.y, &p.z) == 3) {
      if (pDmeDag) {
    ...
    if (sscanf_s(pBuf, "vn %f %f %f", &p.x, &p.y, &p.z) == 3) {
      vertexData.AddNormal(p);
      continue;
    }

    if (!pBaseMesh) {
      if (sscanf_s(pBuf, "vt %f %f", &uv.x, &uv.y) == 2) {
        vertexData.AddUV(uv);
        continue;
      }
      ...
      if (*pBuf == 'f' && (*(pBuf + 1) == ' ' || *(pBuf + 1) == '\t')) {
        if (pDmeDag == NULL) {
          pDmeDag = CreateElement<CDmeDag>(
              pName ? pName : (groupName.IsEmpty() ? "obj" : groupName.Get()),
              pRoot->GetFileId());
          Assert(pDmeDag);
          pDmeMesh = CreateElement<CDmeMesh>(
              pName ? pName : (groupName.IsEmpty() ? "obj" : groupName.Get()),
              pRoot->GetFileId());
        ...
        CUtlBuffer bufParse(pBuf, nLen,
                            CUtlBuffer::TEXT_BUFFER | CUtlBuffer::READ_ONLY);

        while (bufParse.IsValid()) {
          if (!ParseVertex(bufParse, breakSet, v, t, n)) break;
  ...
  if (pBaseMesh) return pVertexData;
...
//
//-----------------------------------------------------------------------------
int CDmObjSerializer::OutputVectors(CUtlBuffer &b, const char *pPrefix,
...
//
//-----------------------------------------------------------------------------
void CDmObjSerializer::MeshToObj(CUtlBuffer &b,
                                 const matrix3x4_t &parentWorldMatrix,
                                 CDmeMesh *pMesh, const char *pDeltaName,
                                 bool absolute) {
  ...
  if (nCompList) {
    ...
    if (absolute) {
      for (int i(0); i < nCompList; ++i) {
        CDmeVertexDeltaData *pTmpDeltaState(
            pMesh->GetDeltaState(compList[i].m_nDeltaIndex));
        if (Q_strcmp(pTmpDeltaState->GetName(), pDeltaName)) continue;

        b << "# Delta: " << pTmpDeltaState->GetName() << "\n";
        pMesh->AddDelta(pTmpDeltaState, pData.Base(), pData.Count(),
                        CDmeVertexData::FIELD_POSITION, 1.0f);
    ...
    if (nCompList) {
      ...
      if (absolute) {
        for (int i(0); i < nCompList; ++i) {
          ...
          b << "# Delta: " << pTmpDeltaState->GetName() << "\n";
          pMesh->AddDelta(pTmpDeltaState, nData.Base(), nData.Count(),
                          CDmeVertexData::FIELD_NORMAL, 1.0f);
    ...
    if (pCount == uvCount && pCount == nCount) {
      ...
      while (pIndex < pEnd) {
        ...
        b << pFaceSep << (pIndices[fIndex] + m_nPositionOffset) << '/'
          << (uvIndices[fIndex] + m_nTextureOffset) << '/'
          << (nIndices[fIndex] + m_nNormalOffset);
        pFaceSep = pFaceNext;
      }
...
//
//-----------------------------------------------------------------------------
void CDmObjSerializer::DagToObj(CUtlBuffer &b,
                                const matrix3x4_t &parentWorldMatrix,
                                CDmeDag *pDag, const char *pDeltaName,
                                bool absolute) {
...
//
//-----------------------------------------------------------------------------
void CDmObjSerializer::FindDeltaMeshes(CDmeDag *pDag,
                                       CUtlVector<CDmeMesh *> &meshes) {
  CDmeMesh *pMesh(CastElement<CDmeMesh>(pDag->GetShape()));
...
// Convert from OBJ -> DME
//-----------------------------------------------------------------------------
bool CDmObjSerializer::WriteOBJ(const char *pFilename, CDmElement *pRoot,
                                bool bWriteOBJs, const char *pDeltaName,
                                bool absolute) {
    ...
    for (int i(0); i < nDeltaMeshes; ++i) {
      ...
      for (int j(0); j < nDeltas; ++j) {
        ...
        if (!pDeltaName || !Q_strcmp(pDeltaName, pDelta->GetName())) {
          ...
          // On WIN32 filesystem changes all of the characters to lowercase
          // grrrr.....
...
//
//-----------------------------------------------------------------------------
bool CDmObjSerializer::ParseVertex(CUtlBuffer &bufParse,
                                   characterset_t &breakSet, int &v, int &t,
                                   int &n) {
  char cmd[1024];
...
//
//-----------------------------------------------------------------------------
const char *CDmObjSerializer::SkipSpace(const char *pBuf) {
...
//
//-----------------------------------------------------------------------------
CDmeVertexDeltaData *CDmObjSerializer::GetDelta(const char *pDeltaName,
                                                bool bAbsolute) {
  if (!m_deltas.Defined(pDeltaName)) return NULL;
  ...
  char deltaPath[SOURCE_MAX_PATH];
  Q_ComposeFileName(m_objDirectory, deltaInfo.m_filename, deltaPath,
                    sizeof(deltaPath));
  Q_FixSlashes(deltaPath);

  if (!g_pFullFileSystem->ReadFile(deltaPath, NULL, utlBuf)) return NULL;

  if (deltaInfo.m_pComboOp && !strchr(pDeltaName, '_')) {
    deltaInfo.m_pComboOp->FindOrCreateControl(pDeltaName, false, true);
  }

  deltaInfo.m_pDeltaData = CastElement<CDmeVertexDeltaData>(
      ReadOBJ(utlBuf, deltaInfo.m_pMesh->GetFileId(), pDeltaName, deltaPath,
              deltaInfo.m_pMesh, NULL, bAbsolute));
...
//-----------------------------------------------------------------------------
void CDmObjSerializer::ComputeDeltaStateComputationList( CUtlVector< CUtlVector<
int > > &dependentDeltaList )
{
        // Do all combinations in order of dimensionality, lowest dimension
first

        for ( int i = 0; i < nDeltas; ++i )
        {
                compList[i].m_nDeltaIndex = i;
                compList[i].m_nDimensionality = ComputeDeltaStateDimensionality(
i );
        }
...

{
        ...
        for ( nCurrentDelta = 0; nCurrentDelta < nCount; ++nCurrentDelta )
        {
                if ( pInfo[nCurrentDelta].m_nDimensionality != 1 )
                        break;
                int j = atomicControls.AddToTail( GetDeltaState(
pInfo[nCurrentDelta].m_nDeltaIndex )->GetName() ); deltaStateUsage[
nCurrentDelta ].AddToTail( j );
        }

        for ( ; nCurrentDelta < nCount; ++nCurrentDelta )
        {
                CDmeVertexDeltaData *pDeltaState = GetDeltaState(
pInfo[nCurrentDelta].m_nDeltaIndex ); int nLen = Q_strlen(
pDeltaState->GetName() ); char *pTempBuf = (char*)_alloca( nLen + 1 ); memcpy(
pTempBuf, pDeltaState->GetName(), nLen+1 ); char *pNext; for ( char *pUnderBar =
pTempBuf; pUnderBar; pUnderBar = pNext )
                {
...
//
//-----------------------------------------------------------------------------
void CDmObjSerializer::ComputeDependentUsage( CUtlVector< CUtlVector< int > >
&deltaUsage )
{
        ...
        for ( int i = 1; i < nCount; ++i )
        {
                ...
                for ( int j = 0; j < i; ++j )
                {
                        // At the point they have the same dimensionality, no
more need to check if ( compList[j].m_nDimensionality ==
compList[i].m_nDimensionality ) break;
                        ...
                        {
                                for ( bSubsetFound = false; ii < nUsageCount1;
++ii )
                                {
                                        if ( deltaStateUsage[j][ji] ==
deltaStateUsage[i][ii] )
                                        {
                                                ++ii;
                                                bSubsetFound = true;
                                                break;
...
deltaStateUsage[i][ii] ) break;
                        ...
                        if ( bSubsetFound )
                        {
                                compList[i].m_DependentDeltas.AddToTail(
compList[j].m_nDeltaIndex );
                        }
                }
        }
}
