...
void CAudioSourceWave::Init(const char *pHeaderBuffer, int headerSize) {
  ...
  // For non-standard waves (like ADPCM) store the header, it has some useful
  // data
...
int CAudioSourceWave::SampleCount(void) {
  if (m_format == WAVE_FORMAT_ADPCM) {
    ...
    if (blockRem) {
      sampleCount += pFormat->wSamplesPerBlock -
                     (((blockSize - blockRem) * 2) / m_channels);
    }
    return sampleCount;
  }
...
void CAudioSourceMemWave::ParseDataChunk(IterateRIFF &walk) {
  ...
  if (m_format == WAVE_FORMAT_PCM) {
    ...
    m_sampleSize = 1;
    // Since each "sample" is a byte (this is a flat file), the number of
