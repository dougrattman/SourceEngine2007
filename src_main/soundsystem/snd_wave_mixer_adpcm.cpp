...
class CAudioMixerWaveADPCM : public CAudioMixerWave {
 public:
  ...
  virtual void Mix(IAudioDevice *pDevice, channel_t *pChannel, void *pData,
                   ...
                   int outCount, int timecompress, bool forward = true);
...
CAudioMixerWaveADPCM::CAudioMixerWaveADPCM(CWaveData *data)
...
void CAudioMixerWaveADPCM::Mix(IAudioDevice *pDevice, channel_t *pChannel,
  ...
  if (NumChannels() == 1)
    pDevice->Mix16Mono(pChannel, (short *)pData, outputOffset, inputOffset,
                       fracRate, outCount, timecompress, forward);
  else
...
bool CAudioMixerWaveADPCM::DecodeBlock() {
  ...
  int available =
      m_pData->ReadSourceData((void **)(&pData), m_offset, m_blockSize);
...
//-----------------------------------------------------------------------------
int CAudioMixerWaveADPCM::GetOutputData(void **pData, int samplePosition,
                                        int sampleCount,
                                        bool forward /*= true*/) {
