...
class CAudioDeviceWave : public IAudioDevice {
 public:
  ...
  virtual void Mix8Mono(channel_t *pChannel, char *pData, int outputOffset,
                        ...
                        int timecompress, bool forward = true);
 ...
 private:
  ...
  // This is a single allocation for all wave headers (there are
  // OUTPUT_BUFFER_COUNT of them)
  HGLOBAL m_hWaveHdr;

  // This is a single allocation for all wave data (there are
  // OUTPUT_BUFFER_COUNT of them)
  HANDLE m_hWaveData;
...
void CAudioDeviceWave::OpenWaveOut() {
  ...
  MMRESULT errorCode = waveOutOpen(&m_waveOutHandle, WAVE_MAPPER, &waveFormat,
                                   0, 0L, CALLBACK_NULL);
  while (errorCode != MMSYSERR_NOERROR) {
    ...
    if (nRetVal != IDRETRY) {
      DWarning("soundsystem", 1,
               "waveOutOpen failure--hardware already in use\n");
      m_waveOutHandle = 0;
      return;
    }

    errorCode = waveOutOpen(&m_waveOutHandle, WAVE_MAPPER, &waveFormat, 0, 0L,
                            CALLBACK_NULL);
  }
...
void CAudioDeviceWave::AllocateOutputBuffers() {
  ...
  for (int i = 0; i < OUTPUT_BUFFER_COUNT; i++) {
    ...
    MMRESULT nResult =
        waveOutPrepareHeader(m_waveOutHandle, lpHdr, sizeof(WAVEHDR));
...
void CAudioDeviceWave::FreeOutputBuffers() {
  ...
  for (int i = 0; i < OUTPUT_BUFFER_COUNT; i++) {
    if (m_buffers[i].hdr) {
      waveOutUnprepareHeader(m_waveOutHandle, m_buffers[i].hdr,
                             sizeof(WAVEHDR));
...
void CAudioDeviceWave::Mix16Mono(channel_t *pChannel, short *pData,
                                 int outputOffset, int inputOffset,
                                 fixedint rateScaleFix, int outCount,
                                 int timecompress, bool forward) {
  int sampleIndex = 0;
...
void CAudioDeviceWave::Mix16Stereo(channel_t *pChannel, short *pData,
                                   int outputOffset, int inputOffset,
                                   fixedint rateScaleFix, int outCount,
                                   int timecompress, bool forward) {
  int sampleIndex = 0;
  ...
  for (int i = 0; i < outCount; i++, fixup += fixupstep) {
    ...
    m_paintbuffer[dest].right +=
        (pChannel->rightvol * pData[sampleIndex + 1]) >> 8;
...
void CAudioDeviceWave::AddToReferencedList(CAudioMixer *mixer,
                                           CAudioBuffer *buffer) {
  // Already in list
  for (int i = 0; i < buffer->m_Referenced.Size(); i++) {
    if (buffer->m_Referenced[i].mixer == mixer) return;
  }
...
void CAudioDeviceWave::RemoveFromReferencedList(CAudioMixer *mixer,
                                                CAudioBuffer *buffer) {
  for (int i = 0; i < buffer->m_Referenced.Size(); i++) {
...
bool CAudioDeviceWave::IsSoundInReferencedList(CAudioMixer *mixer,
                                               CAudioBuffer *buffer) {
  for (int i = 0; i < buffer->m_Referenced.Size(); i++) {
    if (buffer->m_Referenced[i].mixer == mixer) {
      return true;
...
bool CAudioDeviceWave::IsSourceReferencedByActiveBuffer(CAudioMixer *mixer) {
  if (!ValidWaveOut()) return false;

  CAudioBuffer *buffer;
  for (int i = 0; i < OUTPUT_BUFFER_COUNT; i++) {
    buffer = &m_buffers[i];
    if (!buffer->submitted) continue;
...
CAudioDeviceWave::CAudioBuffer *CAudioDeviceWave::GetEmptyBuffer() {
  ...
  if (ValidWaveOut()) {
    for (int i = 0; i < OUTPUT_BUFFER_COUNT; i++) {
      if (!(m_buffers[i].submitted) || m_buffers[i].hdr->dwFlags & WHDR_DONE) {
        pOutput = &m_buffers[i];
...
void CAudioDeviceWave::Update(float time) {
  ...
  if (time > m_mixTime) {
    ...
    while (tempCount > 0) {
      ...
      for (int i = 0; i < MAX_CHANNELS; i++) {
        ...
        if (pSource->GetActive()) {
          if (!pSource->MixDataToDevice(this, pSource->GetChannel(),
                                        currentsample, sampleCount,
                                        DeviceSampleRate(), forward)) {
          ...
          if (!IsSourceReferencedByActiveBuffer(pSource)) {
            ...
            pSource->IncrementSamples(pSource->GetChannel(), currentsample,
                                      sampleCount, DeviceSampleRate(), forward);
          }
        }
      }
    ...
    // if the buffers aren't aligned on sample boundaries, this will hard-lock
    // the machine!
