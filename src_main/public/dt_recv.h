...
// need to dereference the pointer here. NOTE: don't ever return 0 from a
...
#define BEGIN_RECV_TABLE_NOBASE(className, tableName)                \
  ...
  int ClientClassInit<tableName::ignored>(tableName::ignored *) {    \
    ...
    static RecvProp RecvProps[] = {                                  \
      RecvPropInt("should_never_see_this", 0,                        \
...
#define END_RECV_TABLE()                                                       \
  ...
  ;                                                                            \
  RecvTable.Construct(RecvProps + 1, sizeof(RecvProps) / sizeof(RecvProp) - 1, \
                      pRecvTableName);                                         \
  return 1;                                                                    \
  }
...
RecvProp RecvPropFloat(const char *pVarName, int offset,
                       int sizeofVar = SIZEOF_IGNORE,  // Handled by RECVINFO
                                                       // macro, but set to
                                                       // SIZEOF_IGNORE if you
                                                       // don't want to bother.
                       int flags = 0,
                       RecvVarProxyFn varProxy = RecvProxy_FloatToFloat);

RecvProp RecvPropVector(const char *pVarName, int offset,
                        int sizeofVar = SIZEOF_IGNORE,  // Handled by RECVINFO
                                                        // macro, but set to
                                                        // SIZEOF_IGNORE if you
                                                        // don't want to bother.
...
RecvProp RecvPropInt(const char *pVarName, int offset,
                     int sizeofVar = SIZEOF_IGNORE,  // Handled by RECVINFO
                                                     // macro, but set to
                                                     // SIZEOF_IGNORE if you
                                                     // don't want to bother.
...
#define RecvPropVariableLengthArray(arrayLengthProxy, varTemplate, arrayName) \
  varTemplate,                                                                \
      InternalRecvPropArray(sizeof(((currentRecvDTClass *)0)->arrayName) /    \
...
#define RecvPropArray2(arrayLengthProxy, varTemplate, elementCount,           \
  ...
  varTemplate, InternalRecvPropArray(elementCount, elementStride, #arrayName, \
                                     arrayLengthProxy)
