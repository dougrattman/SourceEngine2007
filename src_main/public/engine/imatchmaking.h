...
abstract_class IMatchmaking {
 public:
  ...
  virtual void AddSessionProperty(const uint32_t nType, const char *pID,
                                  const char *pValue,
                                  const char *pValueType) = 0;
  virtual void SetSessionProperties(KeyValues * pPropertyKeys) = 0;
  virtual void SelectSession(uint32_t idx) = 0;
  ...
  virtual void KickPlayerFromSession(uint64_t id) = 0;
  ...
  // For voice chat
  virtual uint64_t PlayerIdToXuid(int playerId) = 0;
  virtual bool IsPlayerMuted(int iUserId, XUID id) = 0;

  // To determine host Quality-of-Service
  virtual MM_QOS_t GetQosWithLIVE() = 0;

  // Used by non-'host' local machines which are starting a map to "prime" the
  // caches.  Will sit at near completion indefinitely --
