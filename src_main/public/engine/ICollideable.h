...
abstract_class ICollideable {
 public:
  ...
  // Returns the bounds of a world-space box used when the collideable is being
  // traced against as a trigger. It's only valid to call these methods if the
  // solid flags have the FSOLID_USE_TRIGGER_BOUNDS flag set.
  ...
  // Returns a world-aligned box guaranteed to surround *everything* in the
  // collision representation Note that this will surround hitboxes, trigger
  // bounds, physics. It may or may not be a tight-fitting box and its volume
  // may suddenly change
  ...
  // returns NULL unless this collideable has specified
