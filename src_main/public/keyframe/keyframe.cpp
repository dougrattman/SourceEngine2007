...
KeyFrame_t *g_KeyFramePtr = &g_KeyFrames[-LOWEST_KEYFRAME];  // points to the
                                                             // middle keyframe,
                                                             // keyframe 0

bool Motion_SetKeyAngles(int keyNum, Quaternion &quatAngles) {
  if (keyNum > HIGHEST_KEYFRAME || keyNum < LOWEST_KEYFRAME) return false;
...
float TimeModifierFunc_TimeSquared(float time) { return (time * time); }
...
bool Motion_GetTimeModifierDetails(int timeInterpNum, const char **outName) {
  ...
  if (outName) *outName = g_TimeModifiers[0].szName;
...
class CPositionInterpolator_Linear : public IPositionInterpolator {
 public:
  ...
  virtual void GetDetails(const char **outName, int *outMinKeyReq,
                          int *outMaxKeyReq);
...
IPositionInterpolator *GetLinearInterpolator() { return &g_LinearInterpolator; }

void CPositionInterpolator_Linear::Release() {}

void CPositionInterpolator_Linear::GetDetails(const char **outName,
                                              int *outMinKeyReq,
                                              int *outMaxKeyReq) {
  *outName = "Linear";
  *outMinKeyReq = 0;
...
void CPositionInterpolator_Linear::SetKeyPosition(int keyNum,
                                                  Vector const &vPos) {
  Assert(keyNum <= HIGHEST_KEYFRAME && keyNum >= LOWEST_KEYFRAME);
  VectorCopy(vPos, g_KeyFramePtr[keyNum].vPos);
}

void CPositionInterpolator_Linear::InterpolatePosition(float time,
                                                       Vector &vOut) {
...
void CPositionInterpolator_CatmullRom::GetDetails(const char **outName,
                                                  int *outMinKeyReq,
                                                  int *outMaxKeyReq) {
...
void CPositionInterpolator_CatmullRom::SetKeyPosition(int keyNum,
                                                      Vector const &vPos) {
  Assert(keyNum <= HIGHEST_KEYFRAME && keyNum >= LOWEST_KEYFRAME);
  VectorCopy(vPos, g_KeyFramePtr[keyNum].vPos);
}

void CPositionInterpolator_CatmullRom::InterpolatePosition(float time,
                                                           Vector &vOut) {
  Catmull_Rom_Spline(g_KeyFramePtr[-1].vPos, g_KeyFramePtr[0].vPos,
                     g_KeyFramePtr[1].vPos, g_KeyFramePtr[2].vPos, time, vOut);
}
...
void CRopeDelegate::GetNodeForces(CSimplePhysics::CNode *pNodes, int iNode,
                                  Vector *pAccel) {
  // Gravity.
  pAccel->Init(0, 0, -1500);
}

void CRopeDelegate::ApplyConstraints(CSimplePhysics::CNode *pNodes,
                                     int nNodes) {
  if (nNodes >= 2) {
...
void CPositionInterpolator_Rope::GetDetails(const char **outName,
                                            int *outMinKeyReq,
                                            int *outMaxKeyReq) {
  *outName = "Rope";
  *outMinKeyReq = 0;
...
void CPositionInterpolator_Rope::SetKeyPosition(int keyNum,
  ...
  if (keyNum == 0 || keyNum == 1) {
    if (vPos != m_Delegate.m_CurEndPoints[keyNum]) m_bChange = true;
...
void CPositionInterpolator_Rope::InterpolatePosition(float time, Vector &vOut) {
  ...
  if (m_bChange) {
    ...
    for (int i = 0; i < m_RopePhysics.NumNodes(); i++)
      m_RopePhysics.GetNode(i)->m_vPos = m_RopePhysics.GetNode(i)->m_vPrevPos =
          m_Delegate.m_CurEndPoints[0];

    float flDist =
        (m_Delegate.m_CurEndPoints[0] - m_Delegate.m_CurEndPoints[1]).Length();
    flDist += m_flSlack;

    m_RopePhysics.Restart();
    m_RopePhysics.SetupSimulation(flDist / (m_RopePhysics.NumNodes() - 1),
                                  &m_Delegate);
  ...
  VectorLerp(m_RopePhysics.GetNode(iNode)->m_vPredicted,
             m_RopePhysics.GetNode(iNode + 1)->m_vPredicted, flNode - iNode,
             vOut);
}
...
bool Motion_GetRotationInterpolatorDetails(int rotInterpNum,
                                           ...
                                           int *outMaxKeyReq) {
  if (rotInterpNum < 0 ||
      rotInterpNum >= Motion_GetNumberOfRotationInterpolators()) {
    return false;
  }

  if (!g_RotationInterpolators[rotInterpNum].szName ||
      !g_RotationInterpolators[rotInterpNum].pFunc) {
    return false;
  }

  if (outName) *outName = g_RotationInterpolators[rotInterpNum].szName;
...
//			Time is assumed to have already been modified by the
// TimeModifyFunc (above)
