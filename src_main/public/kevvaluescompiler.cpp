...
bool CRunTimeKeyValuesStringTable::ReadStringTable(int numStrings,
                                                   CUtlBuffer &buf) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (info.IsSubTree()) {
      Msg("%d:  %s -> subtree at parent %i\n", file.firstElement + i,
          m_StringTable.String(info.key), info.GetParent());
    } else {
      Msg("%d:  %s -> %s at parent %i\n", file.firstElement + i,
          m_StringTable.String(info.key), m_StringTable.String(info.value),
          info.GetParent());
    }
  }
}
...
void CCompiledKeyValuesReader::GetFileName(int index, char *buf,
                                           size_t bufsize) {
  Assert(buf);
  buf[0] = 0;
...
bool CCompiledKeyValuesReader::LoadFile(char const *filename) {
  ...
  for (i = 0; i < fileCount; ++i) {
    ...
    kvf.hFile = g_pFullFileSystem->FindOrAddFileName(
        m_StringTable.Lookup(fileNameString));
...
bool CCompiledKeyValuesReader::CreateInPlaceFromData(KeyValues &head,
  ...
  for (int i = 0; i < num; ++i) {
    int offset = first + i;
    KVInfo_t &kv_info = m_Data[offset];

    if (kv_info.GetParent() != -1) {
      CreateHelper_t search;
      search.index = kv_info.GetParent();
      ...
      KeyValues *sub = new KeyValues(m_StringTable.Lookup(kv_info.key));

      if (!kv_info.IsSubTree()) {
        sub->SetStringValue(m_StringTable.Lookup(kv_info.value));
      }
      ...
      if (!root) {
        root = &head;
        root->SetName(m_StringTable.Lookup(kv_info.key));
        tail = root;
        ...
        CreateHelper_t insert;
        insert.index = offset;
        insert.kv = new KeyValues(m_StringTable.Lookup(kv_info.key));
        insert.tail = nullptr;
        helper.Insert(insert);
...
bool CCompiledKeyValuesReader::InstanceInPlace(KeyValues &head,
                                               char const *kvfilename) {
  char sz[512];
...
bool CCompiledKeyValuesReader::LookupKeyValuesRootKeyName(
    char const *kvfilename, char *outbuf, size_t bufsize) {
  char sz[512];
  strcpy_s(sz, kvfilename);
  Q_FixSlashes(sz);
  ...
  strcpy_s(outbuf, bufsize, m_StringTable.Lookup(m_Data[info.nFirstIndex].key));
  return true;
}
