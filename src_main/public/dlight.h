...
enum {
  ...
  // displacements which is a special effect for selecting which texture to use.
  // If we ever change how alpha is stored for displacements, we'll have to kill
  // this feature
...
// culle from lighting because it is close enough to black to be virtually
// black.
//#define MIN_LIGHTING_VALUE	(1.0f/256.0f)

// This is the broken value of MIN_LIGHTING_VALUE that we have to take into
// consideration to make sure that the lighting for dlights look the same as
// they did in HL2. We'll use the real MIN_LIGHTING_VALUE above to calculate
// larger radii for dynamic light sources.
...
struct dlight_t {
  ...
  float GetRadius() const {
    //		return FastSqrt( radius * radius * (
    //HL2_BROKEN_MIN_LIGHTING_VALUE
    ///  MIN_LIGHTING_VALUE ) );
    return radius;
  }
  ...
  bool IsRadiusGreaterThanZero() const {
    // don't bother calculating the new radius if you just want to know if it is
    // greater than zero.
    return radius > 0.0f;
  }
};
