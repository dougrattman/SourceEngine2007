...
void CBaseRopePhysics::ResetNodeSpringLength(int iStartNode,
                                             float flSpringDist) {
  m_flNodeSpringDistsSqr[iStartNode] = flSpringDist * flSpringDist;
}

void CBaseRopePhysics::SetupSimulation(float flSpringDist,
...
void CBaseRopePhysics::GetNodeForces(CSimplePhysics::CNode *pNodes, int iNode,
                                     Vector *pAccel) {
  if (m_pDelegate)
...
void CBaseRopePhysics::ApplyConstraints(CSimplePhysics::CNode *pNodes,
                                        int nNodes) {
  // Handle springs..
  //
  ...
  for (int iIteration = 0; iIteration < nIterations; iIteration++) {
    for (int i = 0; i < NumSprings(); i++) {
      ...
      // If we don't have an overall spring distance, see if we have a per-node
      // one
