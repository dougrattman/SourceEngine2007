...
#define MAX_LEVEL_CONNECTIONS \
  16  // These are encoded in the lower 16bits of entitytable_t->flags

//-------------------------------------

struct EHandlePlaceholder_t  // Engine does some of the game writing (alas,
                             // probably shouldn't), but can't see ehandle.h
{
  unsigned long i;
};
...
struct entitytable_t {
  ...
  int id;          // Ordinal ID of this entity (used for entity <--> pointer
                   // conversions)
  int edictindex;  // saved for if the entity requires a certain edict number
                   // when restored (players, world)

  int saveentityindex;     // the entity index the entity had at save time ( for
                           // fixing up client side entities )
  int restoreentityindex;  // the entity index given to this entity at restore
                           // time
  ...
  int flags;     // This could be a short -- bit mask of transitions that this
                 // entity is in the PVS of
...
#define FENTTABLE_PLAYERCHILD \
  0x08000000  // this entity is connected to a player, so it must be spawned
              // with it
...
struct saverestorelevelinfo_t {
  ...
  levellist_t
      levelList[MAX_LEVEL_CONNECTIONS];  // List of connections from this level
...
class CGameSaveRestoreInfo {
 public:
  ...
  void PurgeEntityHash() { m_EntityToIndex.Purge(); }
  ...
  Vector modelSpaceOffset;  // used only for globaly entity brushes modelled in
                            // different coordinate systems.

 private:
  int tableCount;         // Number of elements in the entity table
  entitytable_t *pTable;  // Array of entitytable_t elements (1 for each entity)
  CBaseEntity *m_pCurrentEntity;  // only valid during the save functions of
                                  // this entity, NULL otherwise
...
inline char *CSaveRestoreSegment::GetBuffer() { return pBaseData; }

inline int CSaveRestoreSegment::BytesAvailable() const {
  return (bufferSize - size);
}

inline int CSaveRestoreSegment::SizeBuffer() const { return bufferSize; }
...
inline bool CSaveRestoreSegment::Read(void *pOutput, int nBytes) {
  ...
  if (pOutput) memcpy(pOutput, pCurrentData, nBytes);
  MoveCurPos(nBytes);
  return true;
}

inline int CSaveRestoreSegment::GetCurPos() { return size; }

inline char *CSaveRestoreSegment::AccessCurPos() { return pCurrentData; }

inline bool CSaveRestoreSegment::Seek(int absPosition) {
  if (absPosition < 0 || absPosition >= bufferSize) return false;
...
inline void CSaveRestoreSegment::InitSymbolTable(char **pNewTokens,
                                                 int sizeTable) {
  Assert(!pTokens);
...
inline int CSaveRestoreSegment::SizeSymbolTable() { return tokenCount; }
...
inline unsigned short CSaveRestoreSegment::FindCreateSymbol(
  ...
  for (int i = 0; i < tokenCount; i++) {
    ...
    int index = hash + i;
    if (index >= tokenCount) index -= tokenCount;
  ...
  // [Consider doing overflow table(s) after the main table & limiting linear
  // hash table search]
...
inline const char *CSaveRestoreSegment::StringFromSymbol(int token) {
  if (token >= 0 && token < tokenCount) return pTokens[token];
  Assert(0);
  return "<<illegal>>";
}
...
negative shifts work */
  ...
  while (shift--) {
    ...
    if (lobit) num |= 0x80000000; /* set hi bit if lo bit was set */
  }
...
inline unsigned int CSaveRestoreSegment::HashString(const char *pszToken) {
  ...
  while (*pszToken) hash = _rotr(hash, 4) ^ *pszToken++;
