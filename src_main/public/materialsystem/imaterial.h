...
enum VertexFormatFlags_t {
  ...
  // Indicates that compressed vertex elements are to be used (see also
...
#define VERTEX_TEXCOORD_MASK(_coord) \
  ((0x7ULL) << (TEX_COORD_SIZE_BIT + 3 * (_coord)))
...
inline int TexCoordSize(int nTexCoordIndex, VertexFormat_t vertexFormat) {
  return static_cast<int>(
      (vertexFormat >> (TEX_COORD_SIZE_BIT + 3 * nTexCoordIndex)) & 0x7);
}
...
inline VertexCompressionType_t CompressionType(VertexFormat_t vertexFormat) {
  // This is trivial now, but we may add multiple flavours of compressed vertex
  // later on
...
enum VertexElement_t {
  ...
  // Deliberately explicitly numbered so it's a pain in the ass to change, so
  // you read this:
...
inline void Detect_VertexElement_t_Changes(
    VertexElement_t element)  // GREPs for VertexElement_t will hit this
{
...
inline int GetVertexElementSize(VertexElement_t element,
...
// Also make sure you add/remove a string associated with each flag below to
// CShaderSystem::ShaderStateString in ShaderSystem.cpp
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Internal flags not accessible from outside the material system. Stored in
// Flags2
//-----------------------------------------------------------------------------
...
abstract_class IMaterial {
 public:
  ...
  virtual PreviewImageRetVal_t GetPreviewImageProperties(
      int *width, int *height, ImageFormat *imageFormat, bool *isTranslucent)
      const = 0;
