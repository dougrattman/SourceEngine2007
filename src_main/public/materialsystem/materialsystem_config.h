...
struct MaterialSystem_Config_t {
  ...
  unsigned char proxiesTestMode;  // 0 = normal, 1 = no proxies, 2 = alpha test
                                  // all, 3 = color mod all
  ...
  // debug modes
  bool bShowSpecular;  // This is the fast version that doesn't require
                       // reloading materials
  bool bShowDiffuse;  // This is the fast version that doesn't require reloading
                      // materials
  ...
  uint32_t m_WindowedSizeLimitWidth;
  uint32_t m_WindowedSizeLimitHeight;
  int m_nAAQuality;
