...
struct FlashlightState_t {
  FlashlightState_t() {
    m_bEnableShadows = false;  // Provide reasonable defaults for shadow depth
                               // mapping parameters
...
enum RenderTargetSizeMode_t {
  ...
  RT_SIZE_DEFAULT = 1,  // Don't play with the specified width and height other
                        // than making sure it fits in the framebuffer.
  ...
  RT_SIZE_FULL_FRAME_BUFFER_ROUNDED_UP =
      6  // Same size as the frame buffer, rounded up if necessary for systems
         // that can't do non-power of two textures.
};
...
abstract_class IMaterialSystem : public IAppSystem {
 public:
  ...
  // recommended dx level. The function returns false if an invalid dxlevel was
  // specified

  // UNDONE: To find out all convars affected by configuration, we'll need to
  // change the dxsupport.pl program to output all column headers into a single
  // keyvalue block and then we would read that in, and send it back to the
  // client
  ...
  virtual void GetDXLevelDefaults(uint32_t & max_dxlevel,
                                  uint32_t & recommended_dxlevel) = 0;
  ...
  //
  // Note: if the material can't be found, this returns a checkerboard material.
  // You can find out if you have that material by calling
  // IMaterial::IsErrorMaterial(). (Or use the global IsErrorMaterial function,
  ...
  virtual void EndRenderTargetAllocation() = 0;  // Simulate an Alt-Tab in here,
                                                 // which causes a
  ...
  virtual int AllocateDynamicLightmap(int lightmapSize[2],
                                      int *pOutOffsetIntoPage, int frameID) = 0;
...
abstract_class IMatRenderContext : public IRefCounted {
 public:
  ...
  // fractional values can be used for the src_texture coordinates to get linear
  // sampling - integer values should produce 1:1 mappings for non-scaled
  // operations.
  ...
  virtual void PushRenderTargetAndViewport(
      ITexture * pTexture, int nViewX, int nViewY, int nViewW, int nViewH) = 0;
  ...
  // rendering clip planes, beware that only the most recently pushed plane will
  ...
  // mesh Passing true in the second parameter will return the max # of vertices
  // + indices we can use before a flush is provoked and may return different
  // values if called multiple times in succession. Passing false into the
  // second parameter will return the maximum possible vertices + indices that
  // can be rendered in a single batch
  ...
  // lights to points)
  virtual void SetLightingOrigin(const Vector &vLightingOrigin) = 0;

  // Set scissor rect for rendering
  virtual void SetScissorRect(const int nLeft, const int nTop, const int nRight,
                              const int nBottom, const bool bEnableScissor) = 0;
...
static void DoMatSysQueueMark(IMaterialSystem *pMaterialSystem,
  ...
  if (pRenderContext->GetCallQueue())
    pRenderContext->GetCallQueue()->QueueCall(Plat_DebugString,
                                              CUtlEnvelope<const char *>(psz));
}
