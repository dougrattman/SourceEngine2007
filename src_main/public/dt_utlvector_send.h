...
#define SENDINFO_UTLVECTOR(varName)                  \
...
#define SendPropUtlVectorDataTable(varName, nMaxElements, dataTableName) \
  SendPropUtlVector(                                                     \
      SENDINFO_UTLVECTOR(varName), nMaxElements,                         \
...
//
// pArrayProp doesn't need a name, offset, or size. You can pass 0 for all
// those. Example usage:
//
...
SendProp SendPropUtlVector(
    // Use SENDINFO_UTLVECTOR to generate these first 4 parameters.
    const char *pVarName, int offset, int sizeofVar, EnsureCapacityFn ensureFn,
    // Max # of elements in the array. Keep this as low as possible.
    int nMaxElements,
    // Describe the data inside of each element in the array.
    SendProp pArrayProp,
    // This can be overridden to control who the array is sent to.
