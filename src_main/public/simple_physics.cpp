...
void CSimplePhysics::Simulate(CSimplePhysics::CNode *pNodes, int nNodes,
                              CSimplePhysics::IHelper *pHelper, float dt,
                              float flDamp) {
  ...
  for (int iTimeStep = 0; iTimeStep < nTimeSteps; iTimeStep++) {
    ...
    for (int iNode = 0; iNode < nNodes; iNode++) {
      ...
      pNode->m_vPos = pNode->m_vPos +
                      (pNode->m_vPos - pNode->m_vPrevPos) * flDamp +
                      vAccel * m_flTimeStepMul;
  ...
  float flInterpolant =
      (m_flPredictedTime - (GetCurTime() - m_flTimeStep)) / m_flTimeStep;
  for (int iNode = 0; iNode < nNodes; iNode++) {
    CSimplePhysics::CNode *pNode = &pNodes[iNode];
    VectorLerp(pNode->m_vPrevPos, pNode->m_vPos, flInterpolant,
               pNode->m_vPredicted);
  }
}
