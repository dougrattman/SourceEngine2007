...
class CDmeRenderable : public T,
 ...
 public:
  ...
  //vec3_origin; } 	virtual const QAngle &		GetRenderAngles( void )
  //{ return vec3_angle; }
 ...
 protected:
  ...
  virtual CDmAttribute *GetDrawnInEngineAttribute() {
    return m_bWantsToBeDrawnInEngine.GetAttribute();
  }
...
void CDmeRenderable<T>::OnConstruction() {
  ...
  m_bWantsToBeDrawnInEngine.InitAndSet(this, "wantsToBeDrawnInEngine", false,
                                       FATTRIB_DONTSAVE | FATTRIB_HAS_CALLBACK);
...
void CDmeRenderable<T>::OnAttributeChanged(CDmAttribute *pAttribute) {
  ...
  if (pAttribute == pVisibilityAttribute ||
      pAttribute == m_bWantsToBeDrawnInEngine.GetAttribute()) {
    bool bIsVisible =
        pVisibilityAttribute ? pVisibilityAttribute->GetValue<bool>() : true;
      ...
      if (clienttools) {
        if (m_bIsDrawingInEngine) {
          clienttools->AddClientRenderable(
              this, IsTransparent() ? RENDER_GROUP_TRANSLUCENT_ENTITY
...
//-----------------------------------------------------------------------------
template <class T>
bool CDmeRenderable<T>::GetAttachment(int number, Vector &origin,
                                      QAngle &angles) {
...
//-----------------------------------------------------------------------------
template <class T>
void CDmeRenderable<T>::GetShadowRenderBounds(Vector &mins, Vector &maxs,
                                              ShadowType_t shadowType) {
...
template <class T>
void CDmeRenderable<T>::GetRenderBoundsWorldspace(Vector &absMins,
                                                  Vector &absMaxs) {
  Vector mins, maxs;
  GetRenderBounds(mins, maxs);
...
class CDmeVisibilityControl : public T {
 ...
 private:
  virtual CDmAttribute *GetVisibilityAttribute() {
    return m_bIsVisible.GetAttribute();
  }
