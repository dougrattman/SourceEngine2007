...
enum SaveDocumentCloseType_t {
  ...
  SAVEDOC_LEAVEOPEN_POSTCOMMAND_AFTER_SAVE,  // Leaves open, then posts a
                                             // command
};
...
// and the ui can be smaller than the toolsystem panel. The reason these are
// decoupled is so that you can get the 'action' menu no matter where you click
// on the screen
//-----------------------------------------------------------------------------
class CBaseToolSystem : public vgui::EditablePanel,
                        public IToolSystem,
                        public vgui::IFileOpenStateMachineClient {
 ...
 public:
  // Other methods
  // NOTE: This name here is 'general' strictly so 'general' shows up in the
  // keybinding dialog
  CBaseToolSystem(char const *toolName = "#ToolGeneral");

  // Gets the action target to sent to panels so that the tool system's
  // OnCommand is called
  ...
  // Derived classes implement this to create status bar, can return NULL for no
  // status bar in tool...
 ...
 protected:
  ...
  // Derived classes can implement this to get a new scheme to be applied to
  // this tool
  ...
  void OpenFile(const char *pOpenFileType, const char *pSaveFileName = NULL,
  ...
  // Posts the keyvalues
  void SaveFile(const char *pFileName, const char *pFileType, int nFlags,
                KeyValues *pKeyValues = NULL);

  KEYBINDING_FUNC_NODECLARE(editkeybindings, KEY_E,
                            vgui::MODIFIER_SHIFT | vgui::MODIFIER_CONTROL |
                                vgui::MODIFIER_ALT,
                            OnEditKeyBindings, "#editkeybindings_help", 0);
  KEYBINDING_FUNC(keybindinghelp, KEY_H, 0, OnKeyBindingHelp,
                  "#keybindinghelp_help", 0);
  ...
  void ShowKeyBindingsEditor(vgui::Panel *panel,
                             vgui::KeyBindingContextHandle_t handle);
  void ShowKeyBindingsHelp(vgui::Panel *panel,
                           vgui::KeyBindingContextHandle_t handle,
                           vgui::KeyCode boundKey, int modifiers);
 ...
 private:
  ...
  MESSAGE_FUNC_PARAMS(OnFileStateMachineFinished, "FileStateMachineFinished",
                      kv);
