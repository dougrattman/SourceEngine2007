...
the_interface ISave {
 public:
  ...
  // Block support
  //
  // Using these, one doesn't have to worry about queuing up the read pointer on
  // restore if only a subset of the data is read
  //
  ...
  virtual void WriteString(
      const char *pstring) = 0;  // Save a 0-terminated string
  virtual void WriteString(
      const string_t *stringId,
      int count = 1) = 0;  // Save a 0-terminated string (engine string)
  ...
  virtual void WritePositionVector(const char *pname, const Vector *value,
...
the_interface IRestore {
 public:
  ...
  virtual void ReadString(char *pDest, int nSizeDest,
                          int nBytesAvailable) = 0;  // A 0-terminated string
  ...
  virtual int ReadPositionVector(Vector * pValue, int count = 1,
                                 int nBytesAvailable = 0) = 0;
  virtual int ReadFunction(datamap_t * pMap, void **pValue, int count = 1,
                           int nBytesAvailable = 0) = 0;
