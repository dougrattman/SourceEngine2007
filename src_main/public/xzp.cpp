...
BEGIN_BYTESWAP_DATADESC(xZipDirectoryEntry_t)
  DEFINE_FIELD(FilenameCRC, FIELD_INTEGER), DEFINE_FIELD(Length, FIELD_INTEGER),
...
BEGIN_BYTESWAP_DATADESC(xZipFooter_t)
  DEFINE_FIELD(Size, FIELD_INTEGER), DEFINE_FIELD(Magic, FIELD_INTEGER),
END_BYTESWAP_DATADESC()
...
CXZip::CXZip(FILE* handle, int offset,
             int size)  // file handle and offset of the zip file
{
  m_pRead = NULL;
  m_hUser = 0;
  m_nPreloadStart = 0;
...
bool CXZip::InstallAlternateIO(int (*read)(void* buffer, int offset, int length,
                                           int nDestLength, int hUser),
                               int hUser) {
  m_pRead = read;
  m_hUser = hUser;
  return true;
...
bool CXZip::Load(FILE* handle, int nOffset, int nSize,
                 bool bPreload)  // Load a pack file into this instance. Returns
                                 // true on success.
{
  Unload();
  ...
  // Validate the Magic number and at the same time determine if I am reading a
  // regular or swappped xZip file:
  switch (
      m_Swap.SourceIsNativeEndian<int>(m_Header.Magic, xZipHeader_t::MAGIC)) {
    ...
    case 0:
      m_bByteSwapped = true;
      m_Swap.ActivateByteSwapping(
          true);  // We must be reading the opposite endianness.
  ...
  // Read the directory:
  {
    ...
    m_pDirectory = (xZipDirectoryEntry_t*)malloc(sizeof(xZipDirectoryEntry_t) *
                                                 m_Header.DirectoryEntries);
    m_pRead(m_pDirectory, m_Header.HeaderLength, -1,
            sizeof(xZipDirectoryEntry_t) * m_Header.DirectoryEntries, m_hUser);

    // Swap the directory entries if nessecary
    if (m_bByteSwapped) {
      for (unsigned nDirectoryEntry = 0;
           nDirectoryEntry < m_Header.DirectoryEntries; nDirectoryEntry++) {
        m_Swap.SwapFieldsToTargetEndian<xZipDirectoryEntry_t>(
            &(m_pDirectory[nDirectoryEntry]));
      }
    }

    m_nPreloadStart = m_Header.HeaderLength + (sizeof(xZipDirectoryEntry_t) *
                                               m_Header.DirectoryEntries);
  }
...
void CXZip::PreloadData() {
  ...
  if (m_bByteSwapped) {
    for (unsigned nDirectoryEntry = 0;
         nDirectoryEntry < m_Header.PreloadDirectoryEntries;
         nDirectoryEntry++) {
      m_Swap.SwapFieldsToTargetEndian<xZipDirectoryEntry_t>(
          &(m_pPreloadDirectory[nDirectoryEntry]));
    }
  }

  // Set up the regular 2 preload mapping section:
  m_nRegular2PreloadEntryMapping =
      (unsigned short*)(((unsigned char*)m_pPreloadDirectory) +
                        (sizeof(xZipDirectoryEntry_t) *
                         m_Header.PreloadDirectoryEntries));

  // Swap the regular to preload mapping
  if (m_bByteSwapped) {
    m_Swap.SwapBufferToTargetEndian<short>(
        (short*)m_nRegular2PreloadEntryMapping,
        (short*)m_nRegular2PreloadEntryMapping, m_Header.DirectoryEntries);
  }
}
...
int CXZip::defaultRead(void* buffer, int offset, int destLength, int length,
                       int hUser) {
...
char* CXZip::GetEntryFileName(unsigned CRC, char* pDefault) {
  ...
  if (IsRetail()) {
    ...
    if (m_Header.FilenameStringsOffset == 0 || m_Header.FilenameEntries == 0 ||
        CRC == 0) {
      return pDefault;
    }
    ...
    if (!m_pFilenames) {
      MEM_ALLOC_CREDIT_("xZip");
      m_pFilenames =
          (xZipFilenameEntry_t*)malloc(m_Header.FilenameStringsLength);
      m_pRead(m_pFilenames, m_Header.FilenameStringsOffset, -1,
              m_Header.FilenameStringsLength, m_hUser);
    ...
    xZipFilenameEntry_t* found = (xZipFilenameEntry_t*)bsearch(
        &entry, m_pFilenames, m_Header.FilenameEntries,
        sizeof(xZipFilenameEntry_t),
        xZipFilenameEntry_t::xZipFilenameEntryCompare);

    if (!found) return pDefault;

    return (((char*)m_pFilenames) + found->FilenameOffset) -
           m_Header.FilenameStringsOffset;
  }
}

// Sanity checks that the zip file is ready and readable:
bool CXZip::IsValid() {
  if (m_Header.Magic != xZipHeader_t::MAGIC) return false;

  if (m_Header.Version > xZipHeader_t::VERSION) return false;
...
int CXZip::ReadIndex(int nEntryIndex, int nFileOffset, int nDestBytes,
                     int nLength, void* pBuffer) {
  Assert(IsValid());

  if (nLength <= 0 || nEntryIndex < 0) return 0;

  // HACK HACK HACK - convert the pack file index to a local file index (ie,
  // assuming the full file index is being passed in)
  ...
  if (pPreloadEntry) {
    ...
    if (nFileOffset + nLength <= (int)pPreloadEntry->Length) {
      if (m_nMonitorLevel >= 2) {
        char* filename = GetEntryFileName(m_pDirectory[nEntryIndex].FilenameCRC,
                                          "(!!! unknown !!!)");

        Msg("PACK(preload) %s: length:%i offset:%i", filename, nLength,
            nFileOffset);
      }

      memcpy(pBuffer,
             (char*)m_pPreloadedData + pPreloadEntry->StoredOffset +
                 nFileOffset - m_nPreloadStart,
             nLength);
      return nLength;
    }
  }
  ...
  if (m_nMonitorLevel) {
    ...
    unsigned preload = 0;
    if (m_pPreloadedData &&
        m_nRegular2PreloadEntryMapping[nEntryIndex] != 0xFFFF) {
      // Find this entry in the preload directory
      xZipDirectoryEntry_t* entry =
          &(m_pPreloadDirectory[m_nRegular2PreloadEntryMapping[nEntryIndex]]);
      Assert(entry->FilenameCRC == m_pDirectory[nEntryIndex].FilenameCRC);

      preload = entry->Length;
    }

    Msg("PACK %s: length:%i offset:%i (preload bytes:%i)", filename, nLength,
        nFileOffset, preload);
  }
...
bool CXZip::GetSimpleFileOffsetLength(const char* FileName, int& nBaseIndex,
                                      int& nFileOffset, int& nLength) {
  Assert(IsValid());

  xZipDirectoryEntry_t entry;
  entry.FilenameCRC = xZipCRCFilename(FileName);

  xZipDirectoryEntry_t* found = (xZipDirectoryEntry_t*)bsearch(
      &entry, m_pDirectory, m_Header.DirectoryEntries,
      sizeof(xZipDirectoryEntry_t),
      xZipDirectoryEntry_t::xZipDirectoryEntryFindCompare);

  if (found == NULL) return false;

  nFileOffset = found[0].StoredOffset;
  nLength = found[0].Length;
  nBaseIndex = (((int)((char*)found - (char*)m_pDirectory)) /
                sizeof(xZipDirectoryEntry_t));

  return true;
}

bool CXZip::ExtractFile(const char* FileName) { return false; }
...
//		StoredOffset
//
// The sort function may look overly complex, but it is actually useful for
// locating different pieces of the same file in a meaningful order.
//
int __cdecl xZipDirectoryEntry_t::xZipDirectoryEntrySortCompare(
...
int __cdecl xZipDirectoryEntry_t::xZipDirectoryEntryFindCompare(
    const void* left, const void* right) {
  xZipDirectoryEntry_t *l = (xZipDirectoryEntry_t*)left,
                       *r = (xZipDirectoryEntry_t*)right;
...
int __cdecl xZipFilenameEntry_t::xZipFilenameEntryCompare(const void* left,
                                                          const void* right) {
...
bool CopyFileBytes(FILE* hDestination, FILE* hSource, unsigned nBytes) {
  ...
  while (nBytes > 0) {
    int nBytesRead = fread(
        buffer, 1, nBytes > sizeof(buffer) ? sizeof(buffer) : nBytes, hSource);
...
void AddFilename(const char* filename) {
  ...
  pFilenameEntries = (xZipFilenameEntry_t*)realloc(
      pFilenameEntries, sizeof(xZipFilenameEntry_t) * Header.FilenameEntries);

  int filenameLength = (int)strlen(filename) + 1;
  pFilenameEntries[Header.FilenameEntries - 1].FilenameCRC = CRCfilename;
  pFilenameEntries[Header.FilenameEntries - 1].FilenameOffset =
      nFilenameDataLength;
  ...
  memcpy(pFilenameData + nFilenameDataLength - filenameLength, filename,
         filenameLength);
}
...
bool xZipAddFile(const char* filename, CUtlBuffer& fileBuff,
  ...
  if (customPreloadSize > 0) {
    ...
    pPreloadDirectoryEntries = (xZipDirectoryEntry_t*)realloc(
        pPreloadDirectoryEntries,
        sizeof(xZipDirectoryEntry_t) * Header.PreloadDirectoryEntries);
    memcpy(pPreloadDirectoryEntries + Header.PreloadDirectoryEntries - 1,
           &entry, sizeof(entry));
  ...
  pDirectoryEntries = (xZipDirectoryEntry_t*)realloc(
      pDirectoryEntries,
      sizeof(xZipDirectoryEntry_t) * Header.DirectoryEntries);
  memcpy(pDirectoryEntries + Header.DirectoryEntries - 1, &entry,
         sizeof(entry));

  WriteFileBytes(hTempFileData, fileBuff, entry.Length);

  // Align the data region to a 512 byte boundry:  (has to be on last entry as
  // well to ensure enough space to perform the final read, and initial
  // alignment is taken careof by assembexzip)
  int nPadding =
      (XBOX_HDD_SECTORSIZE - (ftell(hTempFileData) % XBOX_HDD_SECTORSIZE)) %
      XBOX_HDD_SECTORSIZE;
...
bool xZipAddFile(const char* zipname, bool bPrecacheEntireFile,
  ...
  return xZipAddFile(zipname, fileBuff, bPrecacheEntireFile,
                     bProcessPrecacheHeader, bProcessPrecacheHeaderOnly);
}
...
bool xZipEnd() {
  ...
  if (Header.PreloadDirectoryEntries) {
    ...
    Header.PreloadBytes = ftell(hTempFilePreload) + nPreloadDirectorySize +
                          nRegular2PreloadSize;  // Raw# of bytes to preload
  ...
  int nPreloadOffset =
      sizeof(Header) + (sizeof(xZipDirectoryEntry_t) * Header.DirectoryEntries);

  // Number of bytes to pad between the end of the preload section and the start
  // of the data section:
  int nPadding = (512 - ((nPreloadOffset + Header.PreloadBytes) % 512)) %
                 512;  // Number of alignment bytes after the preload section
  ...
  // note: not even bothering to byteswap at this point
  fwrite(&Header, sizeof(Header), 1, hOutputFile);

  // Fixup each of the directory entries to make them relative to the beginning
  // of the file.
  for (unsigned i = 0; i < Header.DirectoryEntries; i++) {
  ...
  printf("Sorting and writing %i directory entries...\n",
         Header.DirectoryEntries);
  qsort(pDirectoryEntries, Header.DirectoryEntries,
        sizeof(xZipDirectoryEntry_t),
        &xZipDirectoryEntry_t::xZipDirectoryEntrySortCompare);

  // Swap the directory entries:
  for (unsigned i = 0; i < Header.DirectoryEntries; i++) {
    g_xzpSwap.SwapFieldsToTargetEndian<xZipDirectoryEntry_t>(
        &pDirectoryEntries[i]);
  }

  fwrite(pDirectoryEntries,
         Header.DirectoryEntries * sizeof(xZipDirectoryEntry_t), 1,
         hOutputFile);
  ...
  if (Header.PreloadBytes > 0) {
    ...
    // Fixup each of the directory entries to make them relative to the
    // beginning of the file.
    for (unsigned i = 0; i < Header.PreloadDirectoryEntries; i++) {
      xZipDirectoryEntry_t* pDir = &(pPreloadDirectoryEntries[i]);

      // Shift preload data down by preload bytes (and skipping over the
      // directory):
      pDir->StoredOffset +=
          nPreloadOffset + nPreloadDirectorySize + nRegular2PreloadSize;
    }

    printf("Sorting %u preload directory entries...\n",
           Header.PreloadDirectoryEntries);
    qsort(pPreloadDirectoryEntries, Header.PreloadDirectoryEntries,
          sizeof(xZipDirectoryEntry_t),
          &xZipDirectoryEntry_t::xZipDirectoryEntrySortCompare);

    printf("Building regular to preload mapping table for %u entries...\n",
           Header.DirectoryEntries);
    unsigned short* Regular2Preload =
        (unsigned short*)malloc(nRegular2PreloadSize);
    for (unsigned i = 0; i < Header.DirectoryEntries; i++) {
      unsigned short j;
      for (j = 0; j < Header.PreloadDirectoryEntries; j++) {
        if (pDirectoryEntries[i].FilenameCRC ==
            pPreloadDirectoryEntries[j].FilenameCRC)
          break;
      }
    ...
    for (unsigned i = 0; i < Header.PreloadDirectoryEntries; i++) {
      g_xzpSwap.SwapFieldsToTargetEndian<xZipDirectoryEntry_t>(
          &pPreloadDirectoryEntries[i]);
    }

    fwrite(pPreloadDirectoryEntries,
           Header.PreloadDirectoryEntries * sizeof(xZipDirectoryEntry_t), 1,
           hOutputFile);
    ...
    g_xzpSwap.SwapBufferToTargetEndian<short>(
        (short*)Regular2Preload, (short*)Regular2Preload,
        nRegular2PreloadSize / sizeof(short));
  ...
  if (nFilenameDataLength && Header.FilenameEntries) {
    ...
    Header.FilenameStringsLength =
        (Header.FilenameEntries * sizeof(xZipFilenameEntry_t)) +
        nFilenameDataLength;

    // Adjust the offset in each of the filename offsets to absolute position in
    // the file.
    for (unsigned i = 0; i < Header.FilenameEntries; i++) {
      pFilenameEntries[i].FilenameOffset +=
          (Header.FilenameStringsOffset +
    ...
    for (unsigned int i = 0; i < Header.FilenameEntries; i++) {
      g_xzpSwap.SwapFieldsToTargetEndian<xZipFilenameEntry_t>(
          &pFilenameEntries[i]);
    }

    fwrite(pFilenameEntries, 1,
           Header.FilenameEntries * sizeof(xZipFilenameEntry_t), hOutputFile);
  ...
  // Write the footer:  (block used to keep possibly swapped footer from being
  // used later)
  {
    xZipFooter_t footer;
    ...
    g_xzpSwap.SwapFieldsToTargetEndian<xZipFooter_t>(
        &footer);  // Swap the footer
  ...
  g_xzpSwap.SwapFieldsToTargetEndian<xZipHeader_t>(
      &Header);  // Swap it to write out:
  fwrite(&Header, 1, sizeof(Header), hOutputFile);
  g_xzpSwap.SwapFieldsToTargetEndian<xZipHeader_t>(
      &Header);  // But then swap it back so we can use it in  memory
  ...
  // Print the summary
  printf(
      "\n\nSummary: Input:%u, XZip:%u, Directory Entries:%u (%u preloaded), "
      "Preloaded Bytes:%u\n\n",
      InputFileBytes, OutputFileBytes, Header.DirectoryEntries,
      Header.PreloadDirectoryEntries, Header.PreloadBytes);
...
//
// Returns the number of bytes from a xbox compliant WAV file that should go
// into the preload section:
//-----------------------------------------------------------------------------
...
unsigned xZipComputeXWVPreload(const char* filename) {
  ...
  if (header.id != XWV_ID || header.headerSize != sizeof(header)) return 0;
...
unsigned xZipComputeVMTPreload(const char* filename) {
  ...
  if (!strstr(filename, ".vmt")) return 0;
...
unsigned xZipComputeVHVPreload(const char* filename) {
  // Store VMT's entirely
  if (!strstr(filename, ".vhv")) return 0;
...
unsigned xZipComputeXCSPreload(const char* filename) {
  if (!strstr(filename, ".vcs")) return 0;
