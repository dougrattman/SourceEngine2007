...
#define INVALID_STEAM_LOGGED_IN_ELSEWHERE \
...
// the 360 only allows 15 char + 0, but stick with the larger PC size for
...
#define NUM_NETWORKED_EHANDLE_BITS \
...
// This is the maximum amount of data a PackedEntity can have. Having a limit
// allows us to use static arrays sometimes instead of allocating memory all
// over the place.
#define MAX_PACKEDENTITY_DATA (16384)

// This is the maximum number of properties that can be delta'd. Must be evenly
// divisible by 8.
...
#define FL_FLY \
  (1 << 10)  // Changes the SV_Movestep() behavior to not need to be on ground
#define FL_SWIM \
...
#define FL_AIMTARGET \
  (1 << 16)  // set if the crosshair needs to aim onto the entity
#define FL_PARTIALGROUND (1 << 17)  // not all corners are valid
#define FL_STATICPROP (1 << 18)     // Eetsa static prop!
#define FL_GRAPHED \
  (1 << 19)  // worldgraph has this ent listed as something that blocks a
             // connection
#define FL_GRENADE (1 << 20)
#define FL_STEPMOVEMENT \
  (1 << 21)  // Changes the SV_Movestep() behavior to not do any processing
#define FL_DONTTOUCH \
  (1 << 22)  // Doesn't generate touch functions, generates Untouch() for
             // anything it was touching when this flag was set
#define FL_BASEVELOCITY \
  (1 << 23)  // Base velocity has been applied this frame (used to convert base
             // velocity into momentum)
#define FL_WORLDBRUSH \
  (1 << 24)  // Not moveable/removeable brush entity (really part of the world,
             // but represented as an entity for transparency or something)
#define FL_OBJECT \
  (1 << 25)  // Terrible name. This is an object that NPCs should see. Missiles,
             // for example.
#define FL_KILLME \
  (1 << 26)  // This entity is marked for death -- will be freed by game DLL
...
enum MoveCollide_t {
  ...
  MOVECOLLIDE_FLY_BOUNCE,  // bounces, reflects, based on elasticity of surface
                           // and object - applies friction (adjust velocity)
  MOVECOLLIDE_FLY_CUSTOM,  // Touch() will modify the velocity however it likes
  MOVECOLLIDE_FLY_SLIDE,   // slides along surfaces (no bounce) - applies
                           // friciton (adjusts velocity)
...
// SOLID_NOT/SOLID_TRIGGER when the entity moves SOLID only effects OTHER
// entities colliding with this one when they move - UGH!

// Solid type basically describes how the bounding volume of the object is
// represented NOTE: SOLID_BBOX MUST BE 2, and SOLID_VPHYSICS MUST BE 6 NOTE:
// These numerical values are used in the FGD by the prop code (see
// prop_dynamic)
enum SolidType_t {
...
enum SolidFlags_t {
  FSOLID_CUSTOMRAYTEST =
      0x0001,  // Ignore solid type + always call into the entity for ray tests
  FSOLID_CUSTOMBOXTEST = 0x0002,  // Ignore solid type + always call into the
                                  // entity for swept box tests
  FSOLID_NOT_SOLID = 0x0004,      // Are we currently not solid?
  FSOLID_TRIGGER = 0x0008,  // This is something may be collideable but fires
                            // touch functions even when it's not collideable
                            // (when the FSOLID_NOT_SOLID flag is set)
  FSOLID_NOT_STANDABLE = 0x0010,    // You can't stand on this
  FSOLID_VOLUME_CONTENTS = 0x0020,  // Contains volumetric contents (like water)
  FSOLID_FORCE_WORLD_ALIGNED = 0x0040,  // Forces the collision rep to be
                                        // world-aligned even if it's SOLID_BSP
                                        // or SOLID_VPHYSICS
  FSOLID_USE_TRIGGER_BOUNDS =
      0x0080,  // Uses a special trigger bounds separate from the normal OBB
  FSOLID_ROOT_PARENT_ALIGNED =
      0x0100,  // Collisions are defined in root parent's local coordinate space
  FSOLID_TRIGGER_TOUCH_DEBRIS =
      0x0200,  // This trigger will touch debris objects
...
#define LIFE_DYING \
  1  // playing death animation or still falling off of a ledge waiting to hit
     // ground
...
enum {
  ...
  EF_BONEMERGE_FASTCULL = 0x080,  // For use with EF_BONEMERGE. If this is set,
                                  // then it places this ent's origin at its
  // parent and uses the parent's bbox + the max extents of the aiment.
  // Otherwise, it sets up the parent's bones every frame to figure out where to
  // place the aiment, which is inefficient because it'll setup the parent's
  // bones even if the parent is not in the PVS.
...
// If this is set, then we share a lighting origin with the last non-slave
// breakable sent down to the client
...
enum RenderMode_t {
  ...
  kRenderGlow,  // src*a+dest -- No Z buffer checks -- Fixed size in screen
                // space
  kRenderTransAlpha,          // src*srca+dest*(1-srca)
  kRenderTransAdd,            // src*a+dest
  kRenderEnvironmental,       // not drawn, used for environmental effects
  kRenderTransAddFrameBlend,  // use a fractional frame value to blend between
                              // animation frames
...
enum RenderFx_t {
  ...
  kRenderFxClampMinScale,  // Keep this sprite from getting very small (SPRITES
                           // only!)
  ...
  kRenderFxRagdoll,  // HACKHACK: TEST CODE for signalling death of a ragdoll
                     // character
...
enum Collision_Group_t {
  ...
  COLLISION_GROUP_INTERACTIVE_DEBRIS,  // Collides with everything except other
                                       // interactive debris or debris
  COLLISION_GROUP_INTERACTIVE,  // Collides with everything except interactive
                                // debris or debris
  ...
  COLLISION_GROUP_VEHICLE_CLIP,   // vehicle clip brush to restrict vehicle
                                  // movement
  COLLISION_GROUP_PROJECTILE,     // Projectiles!
  COLLISION_GROUP_DOOR_BLOCKER,   // Blocks entities not permitted to get near
                                  // moving doors
  COLLISION_GROUP_PASSABLE_DOOR,  // Doors that the player shouldn't collide
                                  // with
  COLLISION_GROUP_DISSOLVING,  // Things that are dissolving are in this group
  COLLISION_GROUP_PUSHAWAY,    // Nonsolid on client and server, pushaway in
                               // player code

  COLLISION_GROUP_NPC_ACTOR,     // Used so NPCs in scripts ignore the player.
  COLLISION_GROUP_NPC_SCRIPTED,  // USed for NPCs in scripts that should not
...
// user message max payload size (note, this value is used by the engine, so
// MODs cannot change it)
