...
enum FilesystemOpenExFlags_t {
  ...
  FSOPEN_FORCE_TRACK_CRC = (1 << 1),  // This makes it calculate a CRC for the
                                      // file (if the file came from disk)
                                      // regardless of the IFileList passed to
...
enum FSAsyncFlags_t {
  ...
  FSASYNC_FLAGS_SYNC = (1 << 2),  // Actually perform the operation
                                  // synchronously. Used to simplify client code
                                  // paths
  FSASYNC_FLAGS_NULLTERMINATE = (1 << 3),  // allocate an extra uint8_t and 0
                                           // terminate the buffer read in
};
...
struct FileAsyncRequest_t {
  ...
  FSAllocFunc_t pfnAlloc;  // custom allocator. can be 0. not compatible with
                           // FSASYNC_FLAGS_FREEDATAPTR
};
...
the_interface IFileSystem : public IAppSystem, public IBaseFileSystem {
 public:
  ...
  //
  // If pFilesToReload is non-0, the filesystem will hand back a list of
