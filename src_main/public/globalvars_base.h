...
class CGlobalVarsBase {
 public:
  ...
  // This can be used to filter debug output or to catch the client or server in
  // the act.
  bool IsClient() const { return m_bClient; }

  // for encoding m_flSimulationTime, m_flAnimTime
  int GetNetworkBase(int nTick, int nEntity) const {
 ...
 public:
  // Absolute time (per frame still - Use Plat_FloatTime() for a high precision
  // real time perf clock, but not that it doesn't obey
  // host_timescale/host_framerate)
  float realtime;
  ...
  // Current time
  //
  // On the client, this (along with tickcount) takes a different meaning based
  // on what piece of code you're in:
  //
 ...
 private:
  ...
  // 100 (i.e., tickcount is rounded down to this base and then the "delta" from
  // this base is networked
