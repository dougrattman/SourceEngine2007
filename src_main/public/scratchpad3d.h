...
class CScratchPad3D : public IScratchPad3D {
 ...
 public:
  ...
  class CBaseCommand {
   public:
    ...
    virtual ~CBaseCommand() { ReleaseCachedRenderData(); }

    virtual void Read(CFileRead *pFile) = 0;
    virtual void Write(IFileSystem *pFileSystem, FileHandle_t fp) = 0;

    // Release cached render data. Usually used for releasing things like
    // textures when the app is resizing..
   ...
   public:
    ...
    // The renderer can cache data with the commands to speedup the rendering
    // after the first time (text uses this big time).
    ICachedRenderData *m_pCachedRenderData;
  };

  class CCommand_Point : public CBaseCommand {
   public:
    CCommand_Point() : CBaseCommand(COMMAND_POINT), m_flPointSize{0.0f} {}
 ...
 public:
  CScratchPad3D(char const *pFilename, IFileSystem *pFileSystem,
                bool bAutoClear);

  void AutoFlush();
  void DrawRectGeneric(int iPlane, int otherDim1, int otherDim2,
                       float planeDist, Vector2D const &vMin,
                       Vector2D const &vMax, CSPColor const &vColor);
  void DeleteCommands();
 ...
 public:
  ...
  virtual void SetMapping(Vector const &vInputMin, Vector const &vInputMax,
                          Vector const &vOutputMin, Vector const &vOutputMax);
  ...
  virtual void DrawImageBW(unsigned char const *pData, int width, int height,
                           ...
                           bool bOutlineImage = false, Vector *vCorners = NULL);
  ...
  void DrawPolygonsForPixels(SPRGBA *pData, int width, int height,
