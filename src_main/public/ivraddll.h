...
class CBSPInfo {
 public:
  ...
  dface_t *dfaces;
  unsigned char
      *m_pFacesTouched;  // If non-null, then this has 1 byte for each face and
...
abstract_class IVRadDLL {
 public:
  ...
  // NOTE: if DoIncrementalLight never finished, this will do nothing and return
  // false.
  ...
  // This can be called from a separate thread than the DoIncrementalLight
  // thread. It asynchronously tells DoIncrementalLight to stop as soon as
  // possible and exit.
