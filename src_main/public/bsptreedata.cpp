...
class CBSPTreeData : public IBSPTreeData, public ISpatialLeafEnumerator {
 public:
  ...
  BSPTreeDataHandle_t Insert(int userId, Vector const& mins,
                             Vector const& maxs);
  void Remove(BSPTreeDataHandle_t handle);
  void ElementMoved(BSPTreeDataHandle_t handle, Vector const& mins,
                    Vector const& maxs);

  // Enumerate elements in a particular leaf
  bool EnumerateElementsInLeaf(int leaf, IBSPTreeDataEnumerator* pEnum,
                               int context);
 ...
 private:
  ...
  void InsertIntoTree(BSPTreeDataHandle_t handle, Vector const& mins,
                      Vector const& maxs);
 ...
 private:
  ...
  struct HandleInfo_t {
    ...
    u16 m_LeafList;  // What leafs is it in?
  };
  ...
  struct HandleInLeaf_t {
    ...
    u16
        m_LeafElementIndex;  // what's the m_LeafElements index of the entry?
  };
  ...
  CUtlLinkedList<HandleInfo_t, u16> m_Handles;

  // Maintains the list of all handles in a particular leaf
  CUtlLinkedList<BSPTreeDataHandle_t, u16, true> m_LeafElements;

  // Maintains the list of all leaves a particular handle spans
  CUtlLinkedList<HandleInLeaf_t, u16, true> m_HandleLeafList;
...
IBSPTreeData* CreateBSPTreeData() { return new CBSPTreeData; }

void DestroyBSPTreeData(IBSPTreeData* pTreeData) { delete pTreeData; }
...
BSPTreeDataHandle_t CBSPTreeData::Insert(int userId, Vector const& mins,
                                         Vector const& maxs) {
...
void CBSPTreeData::Remove(BSPTreeDataHandle_t handle) {
  if (!m_Handles.IsValidIndex(handle)) return;
...
void CBSPTreeData::AddHandleToLeaf(int leaf, BSPTreeDataHandle_t handle) {
  ...
  u16 leafElement = m_LeafElements.Alloc(true);
  ...
  u16 handleElement = m_HandleLeafList.Alloc(true);
...
void CBSPTreeData::InsertIntoTree(BSPTreeDataHandle_t handle,
...
void CBSPTreeData::RemoveFromTree(BSPTreeDataHandle_t handle) {
  ...
  u16 i = m_Handles[handle].m_LeafList;
  while (i != m_HandleLeafList.InvalidIndex()) {
    int leaf = m_HandleLeafList[i].m_Leaf;
    u16 leafElement = m_HandleLeafList[i].m_LeafElementIndex;
...
//-----------------------------------------------------------------------------
void CBSPTreeData::ElementMoved(BSPTreeDataHandle_t handle, Vector const& mins,
                                Vector const& maxs) {
...
int CBSPTreeData::CountElementsInLeaf(int leaf) {
  int i;
  int nCount = 0;
  for (i = m_Leaf[leaf].m_FirstElement; i != m_LeafElements.InvalidIndex();
       i = m_LeafElements.Next(i)) {
    ++nCount;
  }
...
// Enumerate elements in a particular leaf
//-----------------------------------------------------------------------------
bool CBSPTreeData::EnumerateElementsInLeaf(int leaf,
  ...
  u16 idx = m_Leaf[leaf].m_FirstElement;
...
// For convenience, enumerates the leaves along a ray, box, etc.
//-----------------------------------------------------------------------------
bool CBSPTreeData::EnumerateLeavesAtPoint(Vector const& pt,
                                          ISpatialLeafEnumerator* pEnum,
                                          int context) {
  return m_pBSPTree->EnumerateLeavesAtPoint(pt, pEnum, context);
}

bool CBSPTreeData::EnumerateLeavesInBox(Vector const& mins, Vector const& maxs,
                                        ISpatialLeafEnumerator* pEnum,
                                        int context) {
  return m_pBSPTree->EnumerateLeavesInBox(mins, maxs, pEnum, context);
}

bool CBSPTreeData::EnumerateLeavesInSphere(Vector const& center, float radius,
                                           ISpatialLeafEnumerator* pEnum,
                                           int context) {
  return m_pBSPTree->EnumerateLeavesInSphere(center, radius, pEnum, context);
}

bool CBSPTreeData::EnumerateLeavesAlongRay(Ray_t const& ray,
                                           ISpatialLeafEnumerator* pEnum,
                                           int context) {
