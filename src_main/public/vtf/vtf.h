...
the_interface IVTFTexture {
 public:
  ...
  virtual void ImageFileInfo(int nFrame, int nFace, int nMip,
                             int *pStartLocation, int *pSizeInBytes) const = 0;
  ...
  // Blends adjacent pixels on cubemap borders, since the card doesn't do it. If
  // the texture is S3TC compressed, then it has to do it AFTER the texture has
  // been compressed to prevent artifacts along the edges.
  //
  // If bSkybox is true, it assumes the faces are oriented in the way the engine
  // draws the skybox (which happens to be different from the way cubemaps have
  // their faces).
