...
inline void *MemAlloc_AllocAligned(size_t size, size_t align) {
  ...
  if ((pAlloc = (unsigned char *)g_pMemAlloc->Alloc(
           sizeof(void *) + align + size)) == (unsigned char *)nullptr)
    return nullptr;
...
inline void *MemAlloc_AllocAligned(size_t size, size_t align,
  ...
  if ((pAlloc = (unsigned char *)g_pMemAlloc->Alloc(
           sizeof(void *) + align + size, pszFile, nLine)) ==
...
inline void *MemAlloc_ReallocAligned(void *ptr, size_t size, size_t align) {
  ...
  if (((size_t)ptr & (align - 1)) != 0) return nullptr;
...
inline size_t MemAlloc_GetSizeAligned(void *pMemBlock) {
  ...
  return g_pMemAlloc->GetSize(pAlloc) -
         ((uint8_t *)pMemBlock - (uint8_t *)pAlloc);
}
...
#define MEMALLOC_DEFINE_EXTERNAL_TRACKING(tag)                     \
  static CUtlMap<void *, MemAllocFileLine_t, int> g_##tag##Allocs( \
      DefLessFunc(void *));                                        \
  static const char *g_psz##tag##Alloc =                           \
      strcpy((char *)g_pMemAlloc->Alloc(strlen(#tag "Alloc") + 1,  \
                                        "intentional leak", 0),    \
             #tag "Alloc");
