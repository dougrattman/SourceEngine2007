...
// a string table to speed up searching for sequences in the current virtual
// model
...
CModelLookupContext::CModelLookupContext(int group,
...
void virtualmodel_t::AppendSequences(int group, const studiohdr_t *pStudioHdr) {
  ...
  for (j = 0; j < pStudioHdr->numlocalseq; j++) {
    ...
    if (k == numCheck) {
      ...
      k = seq.AddToTail(tmp);
    } else if (m_group[seq[k].group]
                   .GetStudioHdr()
                   ->pLocalSeqdesc(seq[k].index)
                   ->flags &
               STUDIO_OVERRIDE) {
...
void virtualmodel_t::AppendAnimations(int group,
                                      const studiohdr_t *pStudioHdr) {
  AUTO_LOCK_(CThreadTerminalMutex<CThreadFastMutex>, m_Lock);
  ...
  if (HasLookupTable()) {
    for (j = numCheck; j < anim.Count(); j++) {
      const char *s1 = m_group[anim[j].group]
                           .GetStudioHdr()
                           ->pLocalAnimdesc(anim[j].index)
                           ->pszName();
        ...
        if ((pStudioHdr->pBone(j)->parent == -1) ||
          ...
          if ((pStudioHdr->pBone(j)->parent != -1) ||
            ...
            Warning("%s/%s : missmatched parent bones on \"%s\"\n",
                    pBaseStudioHdr->pszName(), pStudioHdr->pszName(),
                    pStudioHdr->pBone(j)->pszName());
          }
...
void virtualmodel_t::AppendAttachments(int group,
                                       const studiohdr_t *pStudioHdr) {
  AUTO_LOCK_(CThreadTerminalMutex<CThreadFastMutex>, m_Lock);
  ...
  for (j = 0; j < pStudioHdr->numlocalattachments; j++) {
    ...
    for (k = 0; k < numCheck; k++) {
      char *s2 = m_group[attachment[k].group]
                     .GetStudioHdr()
    ...
    if (k == numCheck) {
      ...
      if ((m_group[0].GetStudioHdr()->pBone(n)->flags &
           BONE_USED_BY_ATTACHMENT) == 0) {
        while (n != -1) {
          m_group[0].GetStudioHdr()->pBone(n)->flags |= BONE_USED_BY_ATTACHMENT;

          if (m_group[0].GetStudioHdr()->pLinearBones()) {
            *m_group[0].GetStudioHdr()->pLinearBones()->pflags(n) |=
...
void virtualmodel_t::AppendPoseParameters(int group,
                                          const studiohdr_t *pStudioHdr) {
  AUTO_LOCK_(CThreadTerminalMutex<CThreadFastMutex>, m_Lock);
...
void virtualmodel_t::AppendIKLocks(int group, const studiohdr_t *pStudioHdr) {
  ...
  for (j = 0; j < pStudioHdr->numlocalikautoplaylocks; j++) {
    ...
    for (k = 0; k < numCheck; k++) {
      int chain2 = m_group[iklock[k].group]
                       .GetStudioHdr()
                       ->pLocalIKAutoplayLock(iklock[k].index)
                       ->chain;
