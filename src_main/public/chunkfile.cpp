...
//-----------------------------------------------------------------------------
void CChunkHandlerMap::AddHandler(const char *pszChunkName,
                                  ChunkHandler_t pfnHandler, void *pData) {
  ChunkHandlerInfoNode_t *pNew = new ChunkHandlerInfoNode_t;

  Q_strncpy(pNew->Handler.szChunkName, pszChunkName,
            sizeof(pNew->Handler.szChunkName));
...
//      pData -
//-----------------------------------------------------------------------------
void CChunkHandlerMap::SetErrorHandler(ChunkErrorHandler_t pfnHandler,
                                       void *pData) {
...
//      ppfnHandler - Receives the address of the callback
// function.       ppData - Receives the context data for the given
// chunk.
// Output : Returns true if a handler was found, false if not.
//-----------------------------------------------------------------------------
ChunkHandler_t CChunkHandlerMap::GetHandler(const char *pszChunkName,
                                            void **ppData) {
  ChunkHandlerInfoNode_t *pNode = m_pHandlers;
  while (pNode != NULL) {
...
// Input  : eError -
//-----------------------------------------------------------------------------
void CChunkFile::HandleError(const char *szChunkName,
                             ChunkFileResult_t eError) {
...
ChunkFileResult_t CChunkFile::HandleChunk(const char *szChunkName) {
  ...
  if (m_DefaultChunkHandler) {
    ChunkFileResult_t testResult =
        m_DefaultChunkHandler(this, m_pDefaultChunkHandlerData, szChunkName);
  ...
  if (m_nHandlerStackDepth > 0) {
    ...
    if (pfnHandler != NULL) {
      ...
      do {
        ...
        while ((eResult = ReadNext(szKey, szValue, sizeof(szValue),
                                   eChunkType)) == ChunkFile_Ok) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::Open(const char *pszFileName,
                                   ChunkFileOpenMode_t eMode) {
  if (eMode == ChunkFile_Read) {
    // UNDONE: TokenReader encapsulates file - unify reading and writing to use
    // the same file I/O. UNDONE: Support in-memory parsing.
...
void CChunkFile::SetDefaultChunkHandler(DefaultChunkHandler_t pHandler,
                                        void *pData) {
...
//      szValue - If eChunkType is ChunkType_Key, contains the
// value of the key.       nValueSize - Size of the buffer pointed
// to by szValue.       eChunkType
//- ChunkType_Key or ChunkType_Chunk.
// Output : Returns ChunkFile_Ok on success, an error code if a parsing error
// occurs.
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::ReadNext(char *szName, char *szValue,
                                       int nValueSize,
                                       ChunkType_t &eChunkType) {
...
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::ReadChunk(KeyHandler_t pfnKeyHandler,
                                        void *pData) {
  //
  ...
  //
  // Cover up ChunkFile_EndOfChunk results because the caller doesn't want to
  // see them.
  //
  if (eResult == ChunkFile_EndOfChunk) {
    eResult = ChunkFile_Ok;
...
// Output :
//-----------------------------------------------------------------------------
bool CChunkFile::ReadKeyValueColor(const char *pszValue, unsigned char &chRed,
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValue(const char *pszKey,
                                            const char *pszValue) {
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValueBool(const char *pszKey,
                                                bool bValue) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValueFloat(const char *pszKey,
                                                 float fValue) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValueColor(const char *pszKey,
                                                 unsigned char r,
                                                 unsigned char g,
                                                 unsigned char b) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
    Q_snprintf(szBuf, sizeof(szBuf), "\"%s\" \"%d %d %d\"", pszKey, (int)r,
               (int)g, (int)b);
    return (WriteLine(szBuf));
  }
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValuePoint(const char *pszKey,
                                                 const Vector &Point) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
    Q_snprintf(szBuf, sizeof(szBuf), "\"%s\" \"(%g %g %g)\"", pszKey,
...
// Purpose:
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValueVector2(const char *pszKey,
                                                   const Vector2D &vec) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
    Q_snprintf(szBuf, sizeof(szBuf), "\"%s\" \"[%g %g]\"", pszKey,
               (double)vec.x, (double)vec.y);
    return (WriteLine(szBuf));
  }
...
// Purpose:
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValueVector3(const char *pszKey,
                                                   const Vector &vec) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
    Q_snprintf(szBuf, sizeof(szBuf), "\"%s\" \"[%g %g %g]\"", pszKey,
...
// Output :
//-----------------------------------------------------------------------------
ChunkFileResult_t CChunkFile::WriteKeyValueVector4(const char *pszKey,
                                                   const Vector4D &vec) {
  if (pszKey != NULL) {
    char szBuf[MAX_KEYVALUE_LEN];
    Q_snprintf(szBuf, sizeof(szBuf), "\"%s\" \"[%g %g %g %g]\"", pszKey,
               (double)vec.x, (double)vec.y, (double)vec.z, (double)vec.w);
    return (WriteLine(szBuf));
  }
