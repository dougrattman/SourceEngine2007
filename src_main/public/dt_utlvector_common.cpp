...
char *AllocateStringHelper2(const char *pFormat, va_list marker) {
  char str[512];
  _vsnprintf_s(str, sizeof(str), pFormat, marker);
...
char *AllocateUniqueDataTableName(bool bSendTable, const char *pFormat, ...) {
...
#ifndef NDEBUG
  // Have to allocate them here because if they're declared as straight global
  // variables, their constructors won't have been called yet by the time we get
  // in here.
  if (!g_STDict) {
  ...
  if (pDict->Find(pRet) != pDict->InvalidIndex()) {
    // If it hits this, then they have 2 utlvectors in different data tables
    // with the same name and the same size limit. The names of
    Assert(false);
  }
  pDict->Insert(pRet, 0);
#endif
