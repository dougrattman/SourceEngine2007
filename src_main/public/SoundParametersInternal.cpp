...
static SoundChannels g_pChannelNames[] = {
    {CHAN_AUTO, "CHAN_AUTO"},     {CHAN_WEAPON, "CHAN_WEAPON"},
    {CHAN_VOICE, "CHAN_VOICE"},   {CHAN_ITEM, "CHAN_ITEM"},
    {CHAN_BODY, "CHAN_BODY"},     {CHAN_STREAM, "CHAN_STREAM"},
...
static SoundLevelLookup g_pSoundLevels[] = {
    {SNDLVL_NONE, "SNDLVL_NONE"},       {SNDLVL_20dB, "SNDLVL_20dB"},
    {SNDLVL_25dB, "SNDLVL_25dB"},       {SNDLVL_30dB, "SNDLVL_30dB"},
    {SNDLVL_35dB, "SNDLVL_35dB"},       {SNDLVL_40dB, "SNDLVL_40dB"},
    {SNDLVL_45dB, "SNDLVL_45dB"},       {SNDLVL_50dB, "SNDLVL_50dB"},
    {SNDLVL_55dB, "SNDLVL_55dB"},       {SNDLVL_IDLE, "SNDLVL_IDLE"},
    {SNDLVL_TALKING, "SNDLVL_TALKING"}, {SNDLVL_60dB, "SNDLVL_60dB"},
    {SNDLVL_65dB, "SNDLVL_65dB"},       {SNDLVL_STATIC, "SNDLVL_STATIC"},
    {SNDLVL_70dB, "SNDLVL_70dB"},       {SNDLVL_NORM, "SNDLVL_NORM"},
    {SNDLVL_75dB, "SNDLVL_75dB"},       {SNDLVL_80dB, "SNDLVL_80dB"},
    {SNDLVL_85dB, "SNDLVL_85dB"},       {SNDLVL_90dB, "SNDLVL_90dB"},
    {SNDLVL_95dB, "SNDLVL_95dB"},       {SNDLVL_100dB, "SNDLVL_100dB"},
    {SNDLVL_105dB, "SNDLVL_105dB"},     {SNDLVL_110dB, "SNDLVL_110dB"},
    {SNDLVL_120dB, "SNDLVL_120dB"},     {SNDLVL_130dB, "SNDLVL_130dB"},
    {SNDLVL_GUNFIRE, "SNDLVL_GUNFIRE"}, {SNDLVL_140dB, "SNDLVL_140dB"},
    {SNDLVL_150dB, "SNDLVL_150dB"},     {SNDLVL_180dB, "SNDLVL_180dB"},
};
...
static const char *_ChannelToString(int channel) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (entry->channel == channel) return entry->name;
  }
...
static const char *_VolumeToString(float volume) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (entry->volume == volume) return entry->name;
  }
...
static const char *_PitchToString(float pitch) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (entry->pitch == pitch) return entry->name;
  }
...
soundlevel_t TextToSoundLevel(const char *key) {
  ...
  for (i = 0; i < c; i++) {
    SoundLevelLookup *entry = &g_pSoundLevels[i];
    if (!Q_strcasecmp(key, entry->name)) return entry->level;
  }
...
int TextToChannel(const char *name) {
  ...
  DevMsg(
      "CSoundEmitterSystem:  Warning, unknown channel type in sounds.txt "
      "(%s)\n",
      name);
...
CSoundParametersInternal::CSoundParametersInternal(
    const CSoundParametersInternal &src) {
  m_pSoundNames = NULL;
...
CSoundParametersInternal::~CSoundParametersInternal() {
  if (m_nSoundNames > 1) delete m_pSoundNames;
  if (m_nConvertedNames > 1) delete m_pConvertedNames;
}
...
void CSoundParametersInternal::CopyFrom(const CSoundParametersInternal &src) {
  ...
  if (m_nSoundNames) {
    if (m_nSoundNames > 1) {
      ...
      memcpy(m_pSoundNames, src.m_pSoundNames,
             m_nSoundNames * sizeof(SoundFile));
    } else {
  ...
  if (m_nConvertedNames) {
    if (m_nConvertedNames > 1) {
      ...
      memcpy(m_pConvertedNames, src.m_pConvertedNames,
             m_nConvertedNames * sizeof(SoundFile));
    } else {
...
bool CSoundParametersInternal::operator==(
    const CSoundParametersInternal &other) const {
  if (this == &other) return true;

  if (channel != other.channel) return false;
  if (!CompareInterval(volume, other.volume)) return false;
  if (!CompareInterval(pitch, other.pitch)) return false;
  if (!CompareInterval(soundlevel, other.soundlevel)) return false;
  if (delay_msec != other.delay_msec) return false;
  if (play_to_owner_only != other.play_to_owner_only) return false;
...
const char *CSoundParametersInternal::VolumeToString(void) const {
  ...
  static char sz[64];
  Q_snprintf(sz, sizeof(sz), "%.3f, %.3f", (float)volume.start,
             (float)volume.start + (float)volume.range);
  return sz;
}
...
const char *CSoundParametersInternal::SoundLevelToString(void) const {
  ...
  static char sz[64];
  Q_snprintf(sz, sizeof(sz), "%i, %i", (soundlevel_t)(int)soundlevel.start,
             (soundlevel_t)(int)(soundlevel.start + soundlevel.range));
  return sz;
}
...
const char *CSoundParametersInternal::PitchToString(void) const {
  ...
  static char sz[64];
  Q_snprintf(sz, sizeof(sz), "%i, %i", (int)pitch.start,
             (int)(pitch.start + pitch.range));
  return sz;
}
...
void CSoundParametersInternal::AddToTail(SoundFile **pDest,
                                         uint16_t *pDestCount,
