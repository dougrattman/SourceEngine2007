...
enum VoiceTweakControl {
  ...
  MicBoost,
  SpeakingVolume,  // values 0-1.  Current voice volume received through Voice
                   // Tweak mode
};

struct IVoiceTweak {
  // These turn voice tweak mode on and off. While in voice tweak mode, the
  // user's voice is echoed back without sending to the server.
  ...
  bool (*IsStillTweaking)();  // This can return false if the user restarts the
                              // sound system during voice tweak mode
};
