...
struct SoundInfo_t {
  ...
  void Set(int newEntity, int newChannel, const char *pszNewName,
           const Vector &newOrigin, const Vector &newDirection, float newVolume,
           soundlevel_t newSoundLevel, bool newLooping, int newPitch,
           const Vector &vecListenerOrigin, int speakerentity) {
  ...
  void WriteDelta(SoundInfo_t *delta, bf_write &buffer) {
    ...
    buffer.WriteOneBit(bIsSentence ? 1
                                   : 0);  // NOTE: SND_STOP behavior is
    ...
    if (nFlags != SND_STOP) {
      ...
      if (fDelay == delta->fDelay) {
        ...
        // bias results so that we only incur the precision loss on relatively
        // large skipaheads
  ...
  void ReadDelta(SoundInfo_t *delta, bf_read &buffer) {
    ...
    if (nFlags != SND_STOP) {
      ...
      if (buffer.ReadOneBit() != 0) {
        ...
        fDelay = (float)buffer.ReadSBitLong(MAX_SOUND_DELAY_MSEC_ENCODE_BITS) /
                 1000.0f;
        ;
