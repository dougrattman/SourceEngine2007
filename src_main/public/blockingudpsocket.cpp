...
CBlockingUDPSocket::CBlockingUDPSocket()
    : m_cserIP(), m_Socket(0), m_pImpl(new CImpl) {
  CreateSocket();
}
...
bool CBlockingUDPSocket::WaitForMessage(float timeOutInSeconds) {
  ...
  if (SOCKET_ERROR ==
      select((int)m_Socket + 1, &m_pImpl->m_FDSet, NULL, NULL, &tv)) {
    return false;
  }
...
unsigned int CBlockingUDPSocket::ReceiveSocketMessage(
    struct sockaddr_in *packet_from, unsigned char *buf, size_t bufsize) {
  memset(packet_from, 0, sizeof(*packet_from));

  struct sockaddr fromaddress;
  int fromlen = sizeof(fromaddress);

  int packet_length = VCRHook_recvfrom(m_Socket, (char *)buf, (int)bufsize, 0,
                                       &fromaddress, &fromlen);
...
bool CBlockingUDPSocket::SendSocketMessage(const struct sockaddr_in &rRecipient,
                                           const unsigned char *buf,
                                           size_t bufsize) {
  // Send data
  int bytesSent = sendto(m_Socket, (const char *)buf, (int)bufsize, 0,
