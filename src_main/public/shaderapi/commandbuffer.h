...
//
// This file defines a number of constants and structured which are used to
// build up a command buffer to pass to ShaderAPI for state setting and other
// operations.  Since the prupose of these command buffers is to minimize and
// optimize calls into shaderapi, their structure is not abstract - they are
// built out by the calling process.
...
enum CommandBufferCommand_t {
  ...
  CBCMD_SET_PIXEL_SHADER_FLOAT_CONST =
      256,  // int cmd,int first_reg, int nregs, float values[nregs*4]

  CBCMD_SET_VERTEX_SHADER_FLOAT_CONST =
      257,  // int cmd,int first_reg, int nregs, float values[nregs*4]
  CBCMD_SET_VERTEX_SHADER_FLOAT_CONST_REF =
      258,  // int cmd,int first_reg, int nregs, &float values[nregs*4]
  ...
  CBCMD_SET_DEPTH_FEATHERING_CONST =
      264,  // int cmd, int constant register, float blend scale
