...
enum StandardTextureId_t {
  ...
  // A texture in which morph data gets accumulated (vs30, fast vertex textures
  // required)
...
abstract_class IShaderDynamicAPI {
 public:
  ...
  virtual void SetTextureTransformDimension(TextureStage_t textureStage,
                                            int dimension, bool projected) = 0;
  ...
  virtual const FlashlightState_t &GetFlashlightState(VMatrix & worldToTexture)
      const = 0;
  ...
  virtual const FlashlightState_t &GetFlashlightStateEx(
      VMatrix & worldToTexture, ITexture * *pFlashlightDepthTexture) const = 0;
  ...
  // Are we in a configuration that needs access to depth data through the alpha
  // channel later?
  ...
  virtual void ExecuteCommandBuffer(uint8_t * pCmdBuffer) = 0;
