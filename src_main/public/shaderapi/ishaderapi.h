...
abstract_class IShaderAPI : public IShaderDynamicAPI {
 public:
  ...
  virtual IMesh *GetDynamicMeshEx(
      IMaterial * pMaterial, VertexFormat_t vertexFormat, int nHWSkinBoneCount,
      bool bBuffered = true, IMesh *pVertexOverride = 0,
  ...
  // framebuffer. Set to SHADER_RENDERTARGET_DEPTHBUFFER if you want to use the
  // regular z buffer.
  ...
  //	iQueryResult >= 0 		-	iQueryResult is the number of
  // pixels  rendered
  //	OCCLUSION_QUERY_RESULT_PENDING		-	query results are not
  // available yet 	OCCLUSION_QUERY_RESULT_ERROR		-	query
  // failed
  ...
  // a View transform for user clip planes. The PROJECTION will be mutliplied
  // against this instead of the normal VIEW matrix.
