...
typedef int __cdecl SortFunc(ListPanel *pPanel, const ListPanelItem &item1,
...
class ListPanel : public Panel {
 ...
 public:
  ...
  enum ColumnFlags_e {
    ...
    COLUMN_RESIZEWITHWINDOW =
        0x02,  // set to have the column grow with the parent dialog growing
    COLUMN_IMAGE = 0x04,  // set if the column data is not text, but instead the
  ...
  virtual void AddColumnHeader(int index, const char *columnName,
                               const char *columnText, int startingWidth,
                               int minWidth, int maxWidth, int columnFlags = 0);
  virtual void AddColumnHeader(int index, const char *columnName,
  ...
  // data sub items are matched against column header name to be used in the
  // table
  virtual int AddItem(const KeyValues *data, unsigned int userData,
                      bool bScrollToItem,
                      bool bSortOnAdd);  // Takes a copy of the data for use in
                                         // the table. Returns the index the
                                         // item is at.
  void SetItemDragData(int itemID,
                       const KeyValues *data);  // Makes a copy of the keyvalues
                                                // to store in the table. Used
                                                // when dragging from the table.
                                                // Only used if the caller
                                                // enables drag support
  virtual int GetItemCount(void);  // returns the number of VISIBLE items
  virtual int GetItem(const char *itemName);  // gets the row index of an item
                                              // by name (data->GetName())
  virtual KeyValues *GetItem(
      int itemID);  // returns pointer to data the row holds
  virtual int GetItemCurrentRow(
      int itemID);  // returns -1 if invalid index or item not visible
  ...
  virtual void ApplyItemChanges(int itemID);  // applies any changes to the
                                              // data, performed by modifying
                                              // the return of GetItem() above
  virtual void RemoveItem(int itemID);        // removes an item from the table
                                              // (changing the indices of all
                                              // following items)
  virtual void RereadAllItems();  // updates the view with the new data

  virtual void
  RemoveAll();  // clears and deletes all the memory used by the data items
  virtual void DeleteAllItems();  // obselete, use RemoveAll();

  virtual void GetCellText(
      int itemID, int column, wchar_t *buffer,
      int bufferSize);  // returns the data held by a specific cell
  virtual IImage *GetCellImage(int itemID, int column);  //, ImagePanel
                                                         //*&buffer); // returns
                                                         //the image held by a
                                                         //specific cell
  ...
  // it is removed from the row list when it becomes invisible, but stays in the
  // indexes this is much faster than a normal remove
  ...
  // returns the selected item by selection index, valid in range [0,
  // GetNumSelectedRows)
  ...
  virtual bool GetCellAtPos(int x, int y, int &row,
                            int &column);  // returns true if any found, row and
                                           // column are filled out. x, y are in
                                           // screen space
  virtual bool GetCellBounds(int row, int column, int &x, int &y, int &wide,
                             int &tall);
 ...
 public:
  virtual void SetSortColumnEx(int iPrimarySortColumn, int iSecondarySortColumn,
                               bool bSortAscending);
  void GetSortColumnEx(int &iPrimarySortColumn, int &iSecondarySortColumn,
 ...
 private:
  ...
  // persistent list of all columns ever created, indexes into m_ColumnsData -
  // used for matching up DATAITEM m_SortedTreeIndexes
  ...
  int m_iColumnDraggerMoved;  // which column dragger was moved->which header to
                              // resize
  int m_lastBarWidth;
  ...
  static bool RBTreeLessFunc(vgui::ListPanel::IndexItem_t &item1,
  ...
  int m_LastItemSelected;  // remember the last row selected for future shift
                           // clicks
