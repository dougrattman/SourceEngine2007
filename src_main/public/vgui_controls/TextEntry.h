...
//  TextEntry classes support several box styles, horizontal scrolling with no
//  scrollbar vertical scrolling with or without a scrollbar, single line,
//  multiline, editable and noneditable.
//
// Shared behaviour:
//  URL's are a different text color and are clickable. Clicking them brings up
//  a web browser. For vertical scroll bars, up and down arrows scroll one line
//  at a time. Clicking and dragging the nob scrolls through text lines. Mouse
//  wheel also moves the nob. User can select and highlight text in the window.
//  Double clicking on a word selects it.
//
// Non editable:
//	No blinking cursor in non editable windows.
//  Right clicking mouse opens copy menu. Menu's top left corner is where mouse
//  is. Ctrl-c will also copy the text.
// Editable:
...
//  ctrl-enter delete the selected text (and inserts a newline if _catchEnterKey
//  is true) insert delete selected text and pastes text from the clipboard
...
//  Trying to move to the prev/next char/line/word when there is none moves the
//  cursor to the start/end of the text.
// Horizontal scrolling:
//  Trying to move to the prev/next char/line/word when there is none scrolls
//  the text horizontally in the window so the new text displays at the correct
//  side. When moving to prev chars scrolling is staggered. To next chars it is
//  one char at a time.
// Cut/Copy/Paste Menu:
//  Right clicking mouse brings up cut/copy/paste menu.
//  If no text is highlighted the cut/copy options are dimmed. Cut is dimmed in
//  non editable panels If there is no text in the clipboard or panel is not
//  editable the paste option is dimmed. If the mouse is right clicked over
//  selected text, the text stays selected. If the mouse is right clicked over
//  unselected text, any selected text is deselected.
//
//
//-----------------------------------------------------------------------------
class TextEntry : public Panel {
 ...
 public:
  ...
  // sets limit of number of characters insertable into field; set to -1 to
  // remove maximum only works with if rich-edit is NOT enabled
  ...
  // sets whether to wrap text once maxChars is reached (on a line by line
  // basis)
...
input:	"text"	- text is set to be this string
  */
  ...
  /* MESSAGE SENDING (to action signal targets)
          ...
          "TextNewLine" - sent when the end key is pressed in the text entry AND
_sendNewLines is true
  ...
  */
  ...
  void SetDrawWidth(
      int width);  // width from right side of window we have to draw in
  int GetDrawWidth();
  void SetHorizontalScrolling(
      bool status);  // turn horizontal scrolling on or off.

  // sets whether non-asci characters (unicode chars > 127) are allowed in the
  ...
  // By default, we draw the language shortname on the right hand side of the
  // control
 ...
 protected:
  ...
  virtual void SetCharAt(
      wchar_t ch, int index);  // set the value of a char in the text buffer
  ...
  virtual bool SelectCheck(
      bool fromMouse = false);  // check if we are in text selection mode
  MESSAGE_FUNC_WCHARPTR(OnSetText, "SetText", text);
  MESSAGE_FUNC(OnSliderMoved,
               "ScrollBarSliderMoved");  // respond to scroll bar events
  ...
  virtual void OnCursorMoved(
      int x, int y);  // respond to moving the cursor with mouse button down
 ...
 protected:
  ...
  bool IsCursorOffRightSideOfWindow(
      int cursorPos);  // check if cursor is off right side of window
  bool IsCursorOffLeftSideOfWindow(
      int cursorPos);  // check if cursor is off left side of window
 ...
 private:
  CUtlVector<wchar_t>
      m_TextStream;  // the text in the text window is stored in this buffer
  CUtlVector<wchar_t>
      m_UndoTextStream;          // a copy of the text buffer to revert changes
  CUtlVector<int> m_LineBreaks;  // an array that holds the index in the buffer
                                 // to wrap lines at
  ...
  bool _mouseSelection;  // whether we are highlighting text or not (selecting
                         // text)
  bool _mouseDragSelection;  // tells weather mouse is outside window and button
                             // is down so we select text
  int _mouseSelectCursorStart;  // where mouse button was pressed down in text
                                // window
  long _cursorNextBlinkTime;    // time of next cursor blink
  int _cursorBlinkRate;         // speed of cursor blinking
  int _select[2];  // select[1] is the offset in the text to where the cursor is
                   // currently
  // select[0] is the offset to where the cursor was dragged to. or -1 if no
  // drag.
  int _pixelsIndent;
  int _charCount;
  ...
  int _currentStartLine;   // use for checking vertical text scrolling
                           // (multiline)
  int _currentStartIndex;  // use for horizontal text scrolling (!multiline)
  bool _horizScrollingAllowed;  // use to disable horizontal text scrolling
                                // period.
  ...
  int _recalculateBreaksIndex;      // tells next linebreakindex index to Start
                                    // recalculating line breaks
  bool _selectAllOnFirstFocus : 1;  // highlights all text in window when focus
                                    // is gained.
