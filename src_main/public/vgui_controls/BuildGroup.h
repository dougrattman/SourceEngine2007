...
namespace vgui {
//-----------------------------------------------------------------------------
// Purpose: a BuildGroup is a list of panels contained in a window (the
// contextPanel) Members of this group are viewable and editable in Build Mode,
// via the BuildModeDialog wizard
//-----------------------------------------------------------------------------
class BuildGroup {
 ...
 public:
  ...
  virtual void LoadControlSettings(const char *controlResourceName,
  ...
  void GetNewFieldName(char *newFieldName, int newFieldNameSize,
                       Panel *newPanel);
  ...
  // Set the panel from which the build group gets all it's object creation
  // information
  ...
  virtual const char *GetResourceName() { return m_pResourceName; }
  ...
  // use when the dialog may have multiple states and the editor will need to be
  // able to switch between them
  void RegisterControlSettingsFile(const char *controlResourceName,
                                   const char *pathID = NULL);
 ...
 private:
  ...
  Panel *m_pBuildContext;  // the panel from which the build dialog gets all the
                           // information it needs
  Panel *m_pParentPanel;   // panel to create new controls in
  CUtlVector<PHandle> _controlGroup;  // grouped panels
  CUtlVector<int>
      _groupDeltaX;  // x offsets of panels in group from the selected panel
  CUtlVector<int>
      _groupDeltaY;  // y offsets of panels in group from the selected panel
