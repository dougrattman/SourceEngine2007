...
// Parameter data type enumeration used internal but the shortcut macros require
// this to be exposed
enum DataType_t {
  DATATYPE_VOID,
...
// Single item in a message map.  Contains the information to map a string
// message name with parameters to a function call
struct MessageMapItem_t {
  const char *name;
  // VC6 aligns this to 16-bytes.  Since some of the code has been compiled with
  // VC6, we need to enforce the alignment on later compilers to remain
  // compatible.
...
#define MESSAGE_FUNC_CHARPTR(name, scriptname, p1)                             \
  _MessageFuncCommon(name, scriptname, 1, vgui::DATATYPE_CONSTCHARPTR, #p1, 0, \
                     0);                                                       \
...
#define MESSAGE_FUNC_PARAMS(name, scriptname, p1)                            \
  _MessageFuncCommon(name, scriptname, 1, vgui::DATATYPE_KEYVALUES, nullptr, \
                     0, 0);                                                  \
...
#define MAP_MESSAGE_CONSTCHARPTR(type, name, func, param1)                    \
  {                                                                           \
    name, (vgui::MessageFunc_t)(&type::func), 1, vgui::DATATYPE_CONSTCHARPTR, \
        param1                                                                \
  }
#define MAP_MESSAGE_CONSTWCHARPTR(type, name, func, param1)                    \
  {                                                                            \
    name, (vgui::MessageFunc_t)(&type::func), 1, vgui::DATATYPE_CONSTWCHARPTR, \
        param1                                                                 \
  }
...
#define MAP_MESSAGE_CONSTCHARPTR_CONSTCHARPTR(type, name, func, param1,     \
                                              param2)                       \
  {                                                                         \
    name, (vgui::MessageFunc_t)&type::func, 2, vgui::DATATYPE_CONSTCHARPTR, \
        param1, vgui::DATATYPE_CONSTCHARPTR, param2                         \
  }

// if more parameters are needed, just use MAP_MESSAGE_PARAMS() and pass the
// keyvalue set into the function

// Stores the list of objects in the hierarchy used to iterate through an
// object's message maps
struct PanelMap_t {
...
// For use in class declarations.  declares the static variables and functions
// needed for the data description iteration
...
// Used by DECLARE_BUILD_FACTORY macro to create a linked list of instancing
// functions
...
// This is the macro which implements creation of each type of panel.  It
// creates a function which instances an object of the specified type.  It them
...
#define DECLARE_BUILD_FACTORY_CUSTOM_ALIAS(className, factoryName, createFunc) \
  static vgui::CBuildFactoryHelper g_##factoryName##_Helper(#factoryName,      \
                                                            createFunc);       \
  className *g_##factoryName##LinkerHack = nullptr;
