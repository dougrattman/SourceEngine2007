...
class RichText : public Panel {
 ...
 public:
  ...
  // sets the RichText control interactive or not (meaning you can select/copy
  // text in the window)
  ...
  // inserts a string that needs to be scanned for urls/mailto commands to be
  // made clickable
  void InsertPossibleURLString(const char *text, Color URLTextColor,
                               Color normalTextColor);

  void InsertFade(float flSustain, float flLength);

  void ResetAllFades(bool bHold, bool bOnlyExpired = false,
                     float flNewSustain = -1.0f);
...
returns:
 ...
 protected:
  ...
  int ParseTextStringForUrls(const char *text, int startPos, char *pchURLText,
                             int cchURLText, char *pchURL, int cchURL,
                             bool &clickable);
 ...
 private:
  const wchar_t *ResolveLocalizedTextAndVariables(char const *pchLookup,
                                                  wchar_t *outbuf,
                                                  size_t outbufsizeinbytes);
  ...
  int _select[2];  // select[1] is the offset in the text to where the cursor is
                   // currently select[0] is the offset to where the cursor was
                   // dragged to. or -1 if no drag.
  int _pixelsIndent;
  ...
  TRenderState
      m_CachedRenderState;  // cached render state for the beginning of painting
  ...
  void GenerateRenderStateForTextStreamIndex(int textStreamIndex,
                                             TRenderState &renderState);
  int FindFormatStreamIndexForTextStreamPos(int textStreamIndex);

  // draws a string of characters with the same formatting using the current
  // render state
