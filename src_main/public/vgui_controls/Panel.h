...
//
//  SETUP_PANEL - will make a panel ready for use right now (i.e setup its
...
//
//   example of use = to set the FG Color of a panel inside of a constructor
//   (i.e before ApplySchemeSettings() has been run on the child)
//
...
class IPanelAnimationPropertyConverter {
 public:
  virtual void GetData(Panel *panel, KeyValues *kv,
                       PanelAnimationMapEntry *entry) = 0;
...
// low-level access to vgui functions use the IPanel/IClientPanel interfaces
// directly
//-----------------------------------------------------------------------------
class Panel : public IClientPanel {
 ...
 public:
  ...
  static void AddPropertyConverter(char const *typeName,
                                   IPanelAnimationPropertyConverter *converter);

  //-----------------------------------------------------------------------------
  // CONSTRUCTORS
  // these functions deal with the creation of the Panel
  // the Panel automatically gets a handle to a vgui-internal panel, the
  // ipanel(), upon construction vgui interfaces deal only with ipanel(), not
  // Panel directly
  Panel();
  Panel(Panel *parent);
  ...
  void SetName(const char *panelName);  // sets the name of the panel - used as
                                        // an identifier
  ...
  // panel visibility
  // invisible panels and their children do not drawn, updated, or receive input
  // messages
  virtual void SetVisible(bool state);
  virtual bool IsVisible();

  // painting
  virtual VPANEL IsWithinTraverse(int x, int y,
                                  bool traversePopups);  // recursive; returns a
  ...
  void MakePopup(bool showTaskbarIcon = true,
                 bool disabled = false);  // turns the panel into a popup window
                                          // (ie. can draw outside of it's
                                          // parents space)
  virtual void OnMove();
  ...
  virtual void SetAutoDelete(bool state);  // if set to true, panel
                                           // automatically frees itself when
                                           // parent is deleted
  ...
  virtual void PostActionSignal(KeyValues *message);  // sends a message to the
                                                      // current
                                                      // actionSignalTarget(s)
  ...
  virtual void PostMessage(Panel *target, KeyValues *message,
                           float delaySeconds = 0.0f);
  virtual bool RequestInfo(KeyValues *outputData);  // returns true if output is
                                                    // successfully written.
                                                    // You should always chain
                                                    // back to the base class if
                                                    // info request is not
                                                    // handled
  virtual bool SetInfo(KeyValues *inputData);  // sets a specified value in the
                                               // control - inverse of the above
  virtual void SetSilentMode(bool bSilent);  // change the panel's silent mode;
                                             // if silent, the panel will not
                                             // post any action signals
  ...
  // Gets the relative offset of the control from the pinned + non-pinned corner
  // (for resizing)
  ...
  virtual void SetPaintBackgroundType(int type);  // 0 for normal(opaque), 1 for
                                                  // single texture from
                                                  // Texture1, and 2 for rounded
                                                  // box w/ four corner textures
  ...
  MESSAGE_FUNC(OnKillFocus,
               "KillFocus");  // called after the panel loses the keyboard focus
  MESSAGE_FUNC(OnDelete, "Delete");  // called to delete the panel;
                                     // Panel::OnDelete() does simply { delete
                                     // this; }
  ...
  //  sort of like setting the SetAllowKeyBindingChainToParent flag to false for
  //  specific keys
  ...
  // over the panel for that amount of time the Show hover context menu function
  // will be invoked
  ...
  // drop data and show an appropriate menu Return false if not using context
  // menu
 ...
 private:
  ...
  uint8_t _autoResizeDirection : 4;  // the directions in which the panel will
                                     // auto-resize to
  ...
  CPanelAnimationVarAliasType(int, m_nBgTextureId3, "Texture3",
                              "vgui/hud/800corner3", "textureid");
  CPanelAnimationVarAliasType(int, m_nBgTextureId4, "Texture4",
                              "vgui/hud/800corner4", "textureid");
