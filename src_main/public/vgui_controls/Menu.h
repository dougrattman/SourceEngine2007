...
// navigated with arrow keys and/or hot keys, and have a lit behavior when mouse
// over. It is NOT the button which opens the menu, but only the menu itself.
//
// Behaviour spec:
...
class Menu : public Panel {
 ...
 public:
  ...
  // Activates item in the menu list, as if that menu item had been selected by
  // the user
 ...
 protected:
  // helper functions
  int AddMenuItemCharCommand(MenuItem *item, const char *command, Panel *target,
 ...
 private:
  ...
  int m_iFixedWidth;
  int m_iMinimumWidth;  // a minimum width the menu has to be if it is not fixed
                        // width
