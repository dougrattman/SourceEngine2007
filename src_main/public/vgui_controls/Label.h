...
class Label : public Panel {
 ...
 public:
  ...
  // alignment code, used to determine how the images are layed out within the
  // Label
  enum Alignment {
    a_northwest = 0,
    a_north,
  ...
  // Additional offset at the Start of the text (from whichever sides it is
  // aligned)
  ...
  // Labels can be associated with controls, and alter behaviour based on the
  // associates behaviour If the associate is disabled, so are we If the
  // associate has focus, we may alter how we draw If we get a hotkey press or
  // focus message, we forward the focus to the associate
  virtual void SetAssociatedControl(Panel *control);

  // Multiple image handling
  // Images are drawn from left to right across the label, ordered by index
  // By default there is a TextImage in position 0 (see
  // GetTextImage()/SetTextImageIndex())
  virtual int AddImage(
      IImage *image,
      int preOffset);  // Return the index the image was placed in
  virtual void SetImageAtIndex(int index, IImage *image, int preOffset);
  virtual void SetImagePreOffset(
      int index, int preOffset);  // Set the offset in pixels before the image
  ...
  // Moves where the default text image is within the image array (it starts in
  // position 0) Setting it to -1 removes it from the image list Returns the
  // index the default text image was previously in
...
input:	"text"	- label text is set to be this string
 ...
 protected:
  ...
  virtual void DrawDashedLine(int x0, int y0, int x1, int y1, int dashLen,
                              int gapLen);
  ...
  MESSAGE_FUNC_PARAMS(OnDialogVariablesChanged, "DialogVariables",
                      dialogVariables);
 ...
 private:
  ...
  int _textImageIndex;  // index in the image array that the default _textimage
                        // resides
