...
class Button : public Label {
 ...
 public:
  // You can optionally pass in the panel to send the click message to and the
  // name of the command to send to that panel.
  Button(Panel *parent, const char *panelName, const char *text,
         Panel *pActionSignalTarget = NULL, const char *pCmd = NULL);
 ...
 public:
  ...
  MESSAGE_FUNC(OnHotkey, "Hotkey") { DoClick(); }
  ...
  // Set this button to be the button that is accessed by default when the user
  // hits ENTER or SPACE
  MESSAGE_FUNC_INT(SetAsDefaultButton, "SetAsDefaultButton", state);
  // Set this button to be the button that is currently accessed by default when
  // the user hits ENTER or SPACE
  MESSAGE_FUNC_INT(SetAsCurrentDefaultButton, "SetAsCurrentDefaultButton",
                   state);
  ...
  /* CUSTOM MESSAGE HANDLING
          "PressButton"	- makes the button act as if it had just been
pressed by the user (just like DoClick()) input: none
  */
 ...
 private:
  ...
  CUtlFlags<u16> _buttonFlags;  // see ButtonFlags_t
  int _mouseClickMask;
  ...
  u16 m_sArmedSoundName, m_sDepressedSoundName, m_sReleasedSoundName;
