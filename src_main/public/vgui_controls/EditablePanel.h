...
class EditablePanel : public Panel {
 ...
 public:
  ...
  // If you pass in pPreloadedKeyValues, it won't actually load the file. That
  // way, you can cache the keyvalues outside of here if you want to prevent
  // file accesses in the middle of the game.
  virtual void LoadControlSettings(const char *dialogResourceName,
                                   const char *pathID = NULL,
                                   KeyValues *pPreloadedKeyValues = NULL);
  ...
  virtual void LoadControlSettingsAndUserConfig(const char *dialogResourceName,
                                                int dialogID = 0);
  ...
  virtual const char *GetControlString(const char *controlName,
                                       const char *defaultString = "");
  // as above, but copies the result into the specified buffer instead of a
  // static buffer
  virtual void GetControlString(const char *controlName, char *buf, int bufSize,
                                const char *defaultString = "");
...
returns:
  "PanelPtr" - pointer to the newly created panel, or NULL if no such class
exists
  */
  // registers a file in the list of control settings, so the vgui dialog can
  // choose between them to edit
  virtual void RegisterControlSettingsFile(const char *dialogResourceName,
                                           const char *pathID = NULL);

  // localization variables - only use this if you need to iterate the
  // variables, use the SetLoc*() to set them
 ...
 protected:
  ...
  // optimization for text rendering, returns true if text should be rendered
  // immediately after Paint() disabled for now virtual bool ShouldFlushText();
 ...
 private:
  ...
  // the wide and tall to which all controls are locked - used for autolayout
  // deltas
