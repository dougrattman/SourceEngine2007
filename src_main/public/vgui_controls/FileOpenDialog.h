...
//
// Purpose: Declaration of FileOpenDialog class, a generic open/save as file
// dialog
...
class FileOpenDialog : public vgui::Frame {
 ...
 public:
  ...
  FileOpenDialog(Panel *parent, const char *title, bool bOpenFile,
                 KeyValues *pContextKeyValues = 0);

  // The context keyvalues are added to all messages sent by this dialog if they
  // are specified
  FileOpenDialog(Panel *parent, const char *title, FileOpenDialogType_t type,
                 KeyValues *pContextKeyValues = 0);
  ~FileOpenDialog();
  ...
  // process) NOTE: If you specify a startdir context, then if you've already
  // opened a file with that same start dir context before, it will start in the
  ...
  // The filter info, if specified, gets sent back to the app in the
  // FileSelected message
  void AddFilter(const char *filter, const char *filterName, bool bActive,
                 const char *pFilterInfo = NULL);
...
"FileSelectionCancelled"
 ...
 protected:
  ...
  MESSAGE_FUNC(OnListItemSelected, "ListItemSelected") { OnItemSelected(); }
