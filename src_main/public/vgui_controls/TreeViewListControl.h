...
class CTreeViewListControl : public vgui::Panel {
 ...
 public:
  ...
  // Set the tree view to be displayed on the left. If this isn't set, then
  // nothing displays in here.
  ...
  // Initialize the column headers.. This info includes the tree view on the
  // left, so this
  virtual void SetNumColumns(int nColumns);
  virtual int GetNumColumns() const;
  // ciFlags is a combination of CI_ flags.
  virtual void SetColumnInfo(int iColumn, const char *pTitle, int width,
                             int ciFlags = 0);

  // Use this to render your stuff. Iterate over the rows in the tree view and
  virtual int GetNumRows();
  virtual int GetTreeItemAtRow(int iRow);  // You can use m_pTree->GetItemData
                                           // to get at the data for the row.

  // Use this to find out the client area to render in for each grid element.
  // The returned box is inclusive.
  // The rule is that the the top and left pixels in each grid element are
  // reserved for lines.
  virtual void GetGridElementBounds(int iColumn, int iRow, int &left, int &top,
 ...
 public:
  ...
  // Usually, you'll want to override paint. After calling the base, use
  // GetNumRows() to iterate over the data in the tree control and fill in the
  // other columns.
  virtual void Paint();
 ...
 private:
  ...
  class CColumnInfo {
   public:
    CColumnInfo() { m_Width = m_Left = m_Right = m_ciFlags = 0; }
