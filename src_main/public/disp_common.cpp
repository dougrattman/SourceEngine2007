...
static CNodeVert const g_NodeTriWinding[9] = {
    CNodeVert(0, 1), CNodeVert(0, 0), CNodeVert(1, 0),
    CNodeVert(2, 0), CNodeVert(2, 1), CNodeVert(2, 2),
    CNodeVert(1, 2), CNodeVert(0, 2), CNodeVert(0, 1)};

// Indexed by CORNER_. These store NEIGHBOREDGE_ defines and tell which edges
// butt up against the corner.
...
static inline void RotateVertIndex(NeighborOrientation neighor,
                                   int sideLengthMinus1, CVertIndex const &in,
                                   CVertIndex &out) {
...
static inline void RotateVertIncrement(NeighborOrientation neighor,
                                       CVertIndex const &in, CVertIndex &out) {
  if (neighor == ORIENTATION_CCW_0) {
    out = in;
  } else if (neighor == ORIENTATION_CCW_90) {
    out.x = in.y;
...
int GetNeighborEdgePower(CDispUtilsHelper *pDisp, int iEdge, int iSub) {
  ...
  if (!pSub->IsValid()) return -1;

  CDispUtilsHelper *pNeighbor =
      pDisp->GetDispUtilsByIndex(pSub->GetNeighborIndex());
...
CDispUtilsHelper *SetupEdgeIncrements(CDispUtilsHelper *pDisp, int iEdge,
  ...
  int nbPower =
      pNeighbor->GetPowerInfo()->m_Power + pShiftInfo->m_PowerShiftAdd;
...
int GetSubNeighborIndex(CDispUtilsHelper *pDisp, int iEdge,
  ...
  if (iFreeIndex == pPowerInfo->m_MidPoint) {
    ...
    // that touch the midpoint, then we have no need to return them because it
    // would touch their corner verts which are always active.
    if (pSide->m_SubNeighbors[0].m_Span != CORNER_TO_CORNER) return -1;
  ...
  if (!pSide->m_SubNeighbors[iSub].IsValid()) {
    if (iSub == 1 && pSide->m_SubNeighbors[0].IsValid() &&
...
void SetupSpan(int iPower, int iEdge, NeighborSpan span, CVertIndex &viStart,
               CVertIndex &viEnd) {
  ...
  if (iEdge == NEIGHBOREDGE_RIGHT || iEdge == NEIGHBOREDGE_BOTTOM) {
    // CORNER_TO_MIDPOINT and MIDPOINT_CORNER are defined where the edge moves
    // up or right, but pPowerInfo->GetCornerPointIndex walks around the edges
    // clockwise, so on the bottom and right edges (where GetCornerPointIndex
    // has us moving down and left) we need to reverse the sense here to make
    // sure we return the right span.
...
CDispUtilsHelper *TransformIntoSubNeighbor(CDispUtilsHelper *pDisp, int iEdge,
                                           ...
                                           CVertIndex const &nodeIndex,
                                           CVertIndex &out) {
  const CDispSubNeighbor *pSub =
      &pDisp->GetEdgeNeighbor(iEdge)->m_SubNeighbors[iSub];
  ...
  SetupSpan(pNeighbor->GetPower(), iNBEdge, pSub->GetNeighborSpan(), viDestEnd,
            viDestStart);

  // Now map the one into the other.
  int iFreeDim = !g_EdgeDims[iEdge];
  int fixedPercent =
      ((nodeIndex[iFreeDim] - viSrcStart[iFreeDim]) * (1 << 16)) /
      (viSrcEnd[iFreeDim] - viSrcStart[iFreeDim]);
  ...
  out[!nbDim] =
      viDestStart[!nbDim] +
      ((viDestEnd[!nbDim] - viDestStart[!nbDim]) * fixedPercent) / (1 << 16);
...
CDispUtilsHelper *TransformIntoNeighbor(CDispUtilsHelper *pDisp, int iEdge,
                                        CVertIndex const &nodeIndex,
                                        CVertIndex &out) {
  if (iEdge == -1) iEdge = GetEdgeIndexFromPoint(nodeIndex, pDisp->GetPower());

  int iSub = GetSubNeighborIndex(pDisp, iEdge, nodeIndex);
  if (iSub == -1) return NULL;

  CDispUtilsHelper *pRet =
      TransformIntoSubNeighbor(pDisp, iEdge, iSub, nodeIndex, out);
...
bool DoesPointHaveAnyNeighbors(CDispUtilsHelper *pDisp,
  ...
  CVertIndex dummy;
  if (TransformIntoNeighbor(pDisp, -1, index, dummy)) return true;

  // See if it connects to a neighbor on a corner.
  int iCorner = GetCornerIndexFromPoint(index, pDisp->GetPower());
  if (iCorner == -1) return false;

  // If there are any neighbors on the specified corner, then the point has
  // neighbors.
  if (pDisp->GetCornerNeighbors(iCorner)->m_nNeighbors > 0) return true;

  // Since points on corners touch two edges, we actually want to test two edges
  // to see if the point has a neighbor on either edge.
  for (int i = 0; i < 2; i++) {
...
void CDispSubEdgeIterator::Start(CDispUtilsHelper *pDisp, int iEdge, int iSub,
                                 bool bTouchCorners) {
  m_pNeighbor = SetupEdgeIncrements(pDisp, iEdge, iSub, m_Index, m_Inc,
                                    m_NBIndex, m_NBInc, m_End, m_FreeDim);
  if (m_pNeighbor) {
    if (bTouchCorners) {
      // Back up our current position by 1 so we hit the corner first, and
      // extend the endpoint so we hit the other corner too.
      m_Index -= m_Inc;
      m_NBIndex -= m_NBInc;
    ...
    m_FreeDim = m_Index.x = m_Inc.x = m_End =
        0;  // Setup so Next returns false.
  }
}
...
bool CDispCircumferenceIterator::Next() {
  switch (m_iCurEdge) {
    ...
    case NEIGHBOREDGE_LEFT: {
      ++m_VertIndex.y;
      if (m_VertIndex.y == m_SideLengthM1) m_iCurEdge = NEIGHBOREDGE_TOP;
    } break;

    case NEIGHBOREDGE_TOP: {
      ++m_VertIndex.x;
      if (m_VertIndex.x == m_SideLengthM1) m_iCurEdge = NEIGHBOREDGE_RIGHT;
    } break;

    case NEIGHBOREDGE_RIGHT: {
      --m_VertIndex.y;
      if (m_VertIndex.y == 0) m_iCurEdge = NEIGHBOREDGE_BOTTOM;
    } break;

    case NEIGHBOREDGE_BOTTOM: {
      --m_VertIndex.x;
      if (m_VertIndex.x == 0) return false;  // Done!
    } break;
  }
...
static inline void SetupCoordXY(CNodeVert &out, CNodeVert const &bottomLeft,
                                CNodeVert const &topRight,
                                CNodeVert const &info) {
  for (int i = 0; i < 2; i++) {
    if (info[i] == 0)
      out[i] = bottomLeft[i];
    else if (info[i] == 1)
...
static unsigned short *DispCommon_GenerateTriIndices_R(
    CNodeVert const &bottomLeft, CNodeVert const &topRight,
    unsigned short *indices, int power, int sideLength) {
  if (power == 1) {
    // Ok, add triangles. All we do here is follow a list of verts
    // (g_NodeTriWinding) around the center vert of this node and make
    // triangles.
    int iCurTri = 0;
    CNodeVert verts[3];
    ...
    for (int i = 0; i < 4; i++) {
      ...
      SetupCoordXY(childBottomLeft, bottomLeft, topRight,
                   g_NodeChildLookup[i][0]);
      SetupCoordXY(childTopRight, bottomLeft, topRight,
                   g_NodeChildLookup[i][1]);

      indices = DispCommon_GenerateTriIndices_R(childBottomLeft, childTopRight,
                                                indices, power - 1, sideLength);
    }
  }
...
int CDispUtilsHelper::VertIndexToInt(const CVertIndex &i) const {
  Assert(i.x >= 0 && i.x < GetSideLength() && i.y >= 0 &&
         i.y < GetSideLength());
...
void DispCommon_GenerateTriIndices(int power, unsigned short *indices) {
  ...
  DispCommon_GenerateTriIndices_R(CNodeVert(0, 0),
                                  CNodeVert(sideLength, sideLength), indices,
                                  power, sideLength + 1);
}
...
static NeighborSpan g_SpanFlip[3] = {CORNER_TO_CORNER, MIDPOINT_TO_CORNER,
                                     CORNER_TO_MIDPOINT};
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SetupDispBoxes(CCoreDispInfo **ppListBase, int nListSize,
                    CUtlVector<CDispBox> &out) {
...
inline bool DoBBoxesTouch(const CDispBox &a, const CDispBox &b) {
  for (int i = 0; i < 3; i++) {
    if (a.m_Max[i] < b.m_Min[i]) return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool FindEdge(CCoreDispInfo *pInfo, Vector const &vPoint1,
              Vector const &vPoint2, int &iEdge) {
...
void AddNeighbor(CCoreDispInfo *pMain,
                 ...
                 CCoreDispInfo *pOther, int iNeighborEdge,
                 NeighborSpan nbSpan) {
  // The edge iteration before coming in here goes 0-1, 1-2, 2-3, 3-4.
  // This flips the sense of CORNER_TO_MIDPOINT/MIDPOINT_TO_CORNER on the right
  // and bottom edges and is undone here.
  ...
  pSub->m_NeighborOrientation =
      g_CoreDispNeighborOrientationMap[iEdge][iNeighborEdge];
  pSub->m_Span = span;
  pSub->m_NeighborSpan = nbSpan;

  pNeighborSub->m_iNeighbor = pMain->GetListIndex();
  pNeighborSub->m_NeighborOrientation =
      g_CoreDispNeighborOrientationMap[iNeighborEdge][iEdge];
...
//-----------------------------------------------------------------------------
// This function is symmetric wrt pMain and pOther. It sets up valid neighboring
// data for the relationship between both of them.
//-----------------------------------------------------------------------------
void SetupEdgeNeighbors(CCoreDispInfo *pMain, CCoreDispInfo *pOther) {
  ...
  for (int iEdge = 0; iEdge < 4; iEdge++) {
    ...
    if (FindEdge(pOther, pt[1], pt[0], iNBEdge)) {
      AddNeighbor(pMain, iEdge, 0, CORNER_TO_CORNER, pOther, iNBEdge,
                  CORNER_TO_CORNER);
    } else {
      ...
      if (FindEdge(pOther, pt[1], pt[0] * 2 - pt[1], iNBEdge)) {
        ...
        if (FindEdge(pOther, mid, pt[0], iNBEdge)) {
          AddNeighbor(pMain, iEdge, g_bEdgeNeighborFlip[iEdge],
                      CORNER_TO_MIDPOINT, pOther, iNBEdge, CORNER_TO_CORNER);
        }

        if (FindEdge(pOther, pt[1], mid, iNBEdge)) {
          AddNeighbor(pMain, iEdge, !g_bEdgeNeighborFlip[iEdge],
                      MIDPOINT_TO_CORNER, pOther, iNBEdge, CORNER_TO_CORNER);
        }
      }
    }
  }
...
bool HasEdgeNeighbor(const CCoreDispInfo *pMain, int iNeighbor) {
  for (int i = 0; i < 4; i++) {
    ...
    for (int iNB = 0; iNB < pCorner->m_nNeighbors; iNB++)
      if (pCorner->m_Neighbors[iNB] == iNeighbor) return true;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SetupCornerNeighbors(CCoreDispInfo *pMain, CCoreDispInfo *pOther,
                          int *nOverflows) {
  if (HasEdgeNeighbor(pMain, pOther->GetListIndex())) return;
  ...
  if (nShared == 1) {
    CDispCornerNeighbors *pMainCorner =
        pMain->GetCornerNeighbors(iMainSharedCorner);
    CDispCornerNeighbors *pOtherCorner =
        pOther->GetCornerNeighbors(iOtherSharedCorner);

    if (pMainCorner->m_nNeighbors < MAX_DISP_CORNER_NEIGHBORS &&
        pOtherCorner->m_nNeighbors < MAX_DISP_CORNER_NEIGHBORS) {
      pMainCorner->m_Neighbors[pMainCorner->m_nNeighbors++] =
          pOther->GetListIndex();
      pOtherCorner->m_Neighbors[pOtherCorner->m_nNeighbors++] =
          pMain->GetListIndex();
    } else {
      ++(*nOverflows);
    }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool VerifyNeighborVertConnection(CDispUtilsHelper *pDisp,
  ...
  if ((pNeighbor = TransformIntoNeighbor(pDisp, mySide, nodeIndex, nbIndex)) !=
    ...
    int iSide =
        GetEdgeIndexFromPoint(nbIndex, pNeighbor->GetPowerInfo()->m_Power);
    if (iSide == -1) {
      return false;
    }

    CDispUtilsHelper *pTest =
        TransformIntoNeighbor(pNeighbor, iSide, nbIndex, testIndex);
  ...
  while (1) {
    ...
    for (iDisp = 0; iDisp < nDisps; ++iDisp) {
      ...
      for (int iEdge = 0; iEdge < 4; iEdge++) {
        ...
        while (it.Next()) {
          if (!VerifyNeighborVertConnection(pHelper, it.GetVertIndex(),
                                            it.GetCurrentNeighbor(),
                                            it.GetNBVertIndex(), iEdge)) {
            pDisp->GetEdgeNeighbor(iEdge)->SetInvalid();
            Warning(
                "Warning: invalid neighbor connection on displacement near "
                "(%.2f %.2f %.2f)\n",
...
void FindNeighboringDispSurfs(CCoreDispInfo **ppListBase, int nListSize) {
  ...
  // Now test all pairs of displacements and setup neighboring relations between
  // them.
  for (iDisp = 0; iDisp < nListSize; ++iDisp) {
    ...
    for (int iDisp2 = iDisp + 1; iDisp2 < nListSize; ++iDisp2) {
      ...
      // NOTE: this must come after SetupEdgeNeighbors because it makes sure not
      // to add corner neighbors for disps that are already edge neighbors.
      SetupCornerNeighbors(pMain, pOther, &nCornerOverflows);
    }
  }

  if (nCornerOverflows) {
    Warning("Warning: overflowed %d displacement corner-neighbor lists.",
            nCornerOverflows);
  }

  // Debug check.. make sure the neighbor connections are intact (make sure that
  // any edge vert that gets mapped into a neighbor gets mapped back the same
  // way).
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool IsVertAllowed(CDispUtilsHelper *pDisp, CVertIndex const &sideVert,
                   int iLevel) {
  if (IsCorner(sideVert, pDisp->GetPowerInfo()->GetSideLength())) return true;

  int iSide =
      GetEdgeIndexFromPoint(sideVert, pDisp->GetPowerInfo()->GetPower());
  if (iSide == -1) return true;
  ...
  if ((pNeighbor->GetPowerInfo()->GetPower() + pShiftInfo->m_PowerShiftAdd) <
      (iLevel + 1)) {
    return false;
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void DisableUnallowedVerts_R(CDispUtilsHelper *pDisp,
  ...
  for (int iSide = 0; iSide < 4; iSide++) {
    CVertIndex const &sideVert =
        pDisp->GetPowerInfo()->m_pSideVerts[iNodeIndex].m_Verts[iSide];
...
void SetupAllowedVerts(CCoreDispInfo **ppListBase, int nListSize) {
  ...
  // Disable verts that need to be disabled so higher-powered displacements
  // remove the necessary triangles when bordering lower-powered displacements.
  // It is necessary to loop around here because disabling verts can accumulate
  // into neighbors.
  bool bContinue;
  do {
    bContinue = false;
    for (iDisp = 0; iDisp < nListSize; ++iDisp) {
      ...
      int nUnallowed = 0;
      DisableUnallowedVerts_R(pDisp, pDisp->GetPowerInfo()->m_RootNode, 0,
                              nUnallowed);
      if (nUnallowed) bContinue = true;
    }
  } while (bContinue);
}
