  ...
  class NetworkVar_##name : public type {                                      \
   ...
   public:                                                                     \
    ...
    virtual void NetworkStateChanged(void *pVar) {                             \
      DispatchNetworkStateChanged(                                             \
          (ThisClass_##name *)(((char *)this) - GetOffset_##name()), pVar);    \
    }                                                                          \
  };                                                                           \
...
class CNetworkColor32Base : public CNetworkVarBase<Type, Changer> {
 public:
  inline void Init(uint8_t rVal, uint8_t gVal, uint8_t bVal) {
    SetR(rVal);
    SetG(gVal);
    SetB(bVal);
  }
  inline void Init(uint8_t rVal, uint8_t gVal, uint8_t bVal, uint8_t aVal) {
    SetR(rVal);
    SetG(gVal);
    SetB(bVal);
    SetA(aVal);
