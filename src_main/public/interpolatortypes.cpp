...
static InterpolatorNameMap_t g_InterpolatorNameMap[] = {
    ...
    {INTERPOLATE_CATMULL_ROM_NORMALIZEX, "catmullrom_normalize_x",
     "Catmull-Rom (Norm X)"},
    ...
    {INTERPOLATE_KOCHANEK_BARTELS_EARLY, "kochanek_early",
     "Kochanek-Bartels Early"},
    {INTERPOLATE_KOCHANEK_BARTELS_LATE, "kochanek_late",
     "Kochanek-Bartels Late"},
    {INTERPOLATE_SIMPLE_CUBIC, "simple_cubic", "Simple Cubic"},
    {INTERPOLATE_CATMULL_ROM, "catmullrom", "Catmull-Rom"},
    {INTERPOLATE_CATMULL_ROM_NORMALIZE, "catmullrom_normalize",
     "Catmull-Rom (Norm)"},
    {INTERPOLATE_CATMULL_ROM_TANGENT, "catmullrom_tangent",
     "Catmull-Rom (Tangent)"},
...
int Interpolator_InterpolatorForName(char const *name) {
  for (int i = 0; i < NUM_INTERPOLATE_TYPES; ++i) {
    ...
    if (!_stricmp(name, slot->name)) return slot->type;
  }
...
char const *Interpolator_NameForInterpolator(int type, bool printname) {
  usize i = type;
  if (i >= std::size(g_InterpolatorNameMap)) {
    Assert("!Interpolator_NameForInterpolator:  bogus type!");
    // returns "unspecified!!!";
    return printname ? g_InterpolatorNameMap[0].printname
                     : g_InterpolatorNameMap[0].name;
  }

  return printname ? g_InterpolatorNameMap[i].printname
                   : g_InterpolatorNameMap[i].name;
}
...
int Interpolator_CurveTypeForName(const char *name) {
  ...
  constexpr usize skip = std::size("curve_") - 1;

  if (!_strnicmp(sz, "curve_", skip)) {
    char *p = sz + skip;
...
int Interpolator_CurveTypeForHotkey(int key) {
  for (usize i = 0; i < std::size(g_CurveNameMap); ++i) {
    CurveNameMap_t *slot = &g_CurveNameMap[i];
    if (slot->hotkey == key) return slot->type;
  }

  return -1;
}

void Interpolator_GetKochanekBartelsParams(int interpolationType,
...
void Interpolator_CurveInterpolate(int interpolationType, const Vector &vPre,
  ...
  switch (interpolationType) {
    ...
    case INTERPOLATE_CATMULL_ROM_NORMALIZEX:
      Catmull_Rom_Spline_NormalizeX(vPre, vStart, vEnd, vNext, f, vOut);
      break;
    case INTERPOLATE_CATMULL_ROM:
      Catmull_Rom_Spline(vPre, vStart, vEnd, vNext, f, vOut);
      break;
    case INTERPOLATE_CATMULL_ROM_NORMALIZE:
      Catmull_Rom_Spline_Normalize(vPre, vStart, vEnd, vNext, f, vOut);
      break;
    case INTERPOLATE_CATMULL_ROM_TANGENT:
      Catmull_Rom_Spline_Tangent(vPre, vStart, vEnd, vNext, f, vOut);
      break;
    case INTERPOLATE_EASE_IN: {
      f = sin(M_PI * f * 0.5f);
      // Fixme, since this ignores vPre and vNext we could omit computing them
      // aove
    ...
    case INTERPOLATE_KOCHANEK_BARTELS_LATE: {
      ...
      Kochanek_Bartels_Spline_NormalizeX(t, b, c, vPre, vStart, vEnd, vNext, f,
                                         vOut);
    } break;
    case INTERPOLATE_SIMPLE_CUBIC:
      Cubic_Spline_NormalizeX(vPre, vStart, vEnd, vNext, f, vOut);
      break;
    case INTERPOLATE_BSPLINE:
      BSpline(vPre, vStart, vEnd, vNext, f, vOut);
      break;
...
void Interpolator_CurveInterpolate_NonNormalized(
  ...
  switch (interpolationType) {
    ...
    case INTERPOLATE_KOCHANEK_BARTELS_LATE: {
      float t, b, c;
      Interpolator_GetKochanekBartelsParams(interpolationType, t, b, c);
      Kochanek_Bartels_Spline(t, b, c, vPre, vStart, vEnd, vNext, f, vOut);
    } break;
    case INTERPOLATE_SIMPLE_CUBIC:
      Cubic_Spline(vPre, vStart, vEnd, vNext, f, vOut);
      break;
    case INTERPOLATE_BSPLINE:
      BSpline(vPre, vStart, vEnd, vNext, f, vOut);
      break;
...
void Interpolator_CurveInterpolate_NonNormalized(int interpolationType,
  ...
  switch (interpolationType) {
    ...
    case INTERPOLATE_SIMPLE_CUBIC:
    case INTERPOLATE_BSPLINE:
      // TODO(d.rattman): since this ignores vPre and vNext we could omit
      // computing them aove
