...
class CSheet;
class CMeshBuilder;
extern float s_pRandomFloats[4096];

// Random numbers

#define MAX_RANDOM_FLOATS std::size(s_pRandomFloats)
...
#define ATTRIBUTES_WHICH_ARE_VEC3S_MASK                             \
  (PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK | \
   PARTICLE_ATTRIBUTE_TINT_RGB_MASK |                               \
...
class IParticleSystemQuery : public IAppSystem {
 public:
  ...
  virtual void GetRandomPointsOnControllingObjectHitBox(
      ...
      float flBBoxScale, int nNumTrysToGetAPointInsideTheModel,
      Vector *pPntsOut, Vector vecDirectionBias,
      Vector *pHitBoxRelativeCoordOut = NULL, int *pHitBoxIndexOut = NULL);
...
class CParticleSystemMgr {
 public:
  ...
  // won't be able to sim or draw. These are done separately from Init, so that
  ...
  // format
  bool WriteParticleConfigFile(const char *pParticleSystemName, CUtlBuffer &buf,
                               bool bPreventNameBasedLookup = false);
  bool WriteParticleConfigFile(const DmObjectId_t &id, CUtlBuffer &buf,
                               bool bPreventNameBasedLookup = false);

  // create a particle system by name. returns 0 if one of that name does not
  // exist
  CParticleCollection *CreateParticleCollection(const char *pParticleSystemName,
                                                float flDelay = 0.0f,
                                                int nRandomSeed = 0);
 ...
 private:
  ...
  // attaching and dtaching killlists. when simulating, a particle system gets a
  // kill list. after simulating, the memory for that will be used for the next
  // particle system.  This matters for threaded particles, because we don't
  // want to share the same kill list between simultaneously simulating particle
  // systems.
...
class IParticleOperatorDefinition {
 public:
  ...
  SOURCE_FORCEINLINE void RecordExecutionTime(float flETime) {
    m_flUncomittedTime += flETime;
    m_flMaxExecutionTime = std::max(m_flMaxExecutionTime, flETime);
  }

  SOURCE_FORCEINLINE float TotalRecordedExecutionTime(void) const {
    return m_flTotalExecutionTime;
  }

  SOURCE_FORCEINLINE float MaximumRecordedExecutionTime(void) const {
...
class CParticleOperatorInstance {
 public:
  ...
  virtual uint32_t GetWrittenAttributes(void) const = 0;
  virtual uint32_t GetReadAttributes(void) const = 0;
  virtual uint64_t GetReadControlPointMask() const { return 0; }

  //  Used when an operator needs to read the attributes of a particle at spawn
  //  time
  virtual uint32_t GetReadInitialAttributes(void) const { return 0; }
  ...
  virtual uint32_t Emit(CParticleCollection *pParticles, float flOpCurStrength,
                        void *pContext) const {
    return 0;
  }
  ...
  // splits particle initialization up into scalar and block sections, callingt
  // he right code
  void InitNewParticles(CParticleCollection *pParticles, int nFirstParticle,
                        int n_particles, int attribute_write_mask,
                        void *pContext) const;

  // this function is queried to determine if a particle system is over and doen
  // with. A particle system is done with when it has noparticles and no
  ...
  // RandomFloat inside operators Use CParticleCollection::RandomInt/RandomFloat
  // instead
 ...
 protected:
  ...
  void InitScalarAttributeRandomRangeExpBlock(int nAttributeId, float fMinValue,
...
class CParticleOperatorDefinition : public IParticleOperatorDefinition {
 public:
  CParticleOperatorDefinition(const char *pFactoryName, ParticleOperatorId_t id,
                              bool bIsObsolete)
...
{
  uint16_t m_nPass2SortKey;  // value to sort by. bastardized fp16
  uint16_t m_nIndex;         // index or fudged index (for child particles)
...
struct CParticleSIMDTransformation {
  ...
  SOURCE_FORCEINLINE void VectorRotate(FourVectors &InPnt) {
  ...
  SOURCE_FORCEINLINE void VectorTransform(FourVectors &InPnt) {
    VectorRotate(InPnt);
...
class CParticleCollection {
 public:
  ...
  void StopEmission(bool bInfiniteOnly = false,
                    bool bRemoveAllParticles = false, bool bWakeOnStop = false);
  ...
  // NOTE: These versions will produce the *same random numbers* if you give it
  // the same random sample id. do not use these methods.
  ...
  const ParticleRenderData_t *GetRenderList(IMatRenderContext *pRenderContext,
  ...
  void CalculatePathValues(CPathParameters const &PathIn, float flTimeStamp,
                           Vector *pStartPnt, Vector *pMidPnt, Vector *pEndPnt);
 ...
 public:
  ...
  // control point data.  Don't set these directly, or they won't propagate down
  // to children particle control points can act as emitter centers, repulsions
  // points, etc.  what they are used for depends on what operators and
  // parameters your system has.
  ...
  uint8_t *m_pOperatorContextData;
  ...
  struct CWorldCollideContextData
      *m_pCollisionCacheData[NUM_COLLISION_CACHE_MODES];  // children can share
                                                          // collision caches w/
                                                          // parent
 ...
 protected:
  ...
  Vector m_MinBounds;
  Vector m_MaxBounds;
  int m_nHighestCP;  // Highest CP set externally.  Needs to assert if a system
                     // calls to an unassigned CP.
 ...
 private:
  ...
  int m_nPerParticleReadInitialAttributeMask;  // What fields do operators want
                                               // to see initial attribute
                                               // values for?
  ...
  uint64_t m_nControlPointReadMask;  // Mask indicating which control points
                                     // have been accessed
  ...
  bool m_bIsTwoPass : 1;
  bool m_bAnyUsesPowerOfTwoFrameBufferTexture : 1;  // whether or not we or any
                                                    // children use this
...
class CStridedPtr {
 ...
 public:
  T *operator->(void)const { return m_pData; }

  T &operator*(void)const { return *m_pData; }

  SOURCE_FORCEINLINE operator T *(void) { return m_pData; }

  SOURCE_FORCEINLINE CStridedPtr<T> &operator++(void) {
    m_pData += m_nStride;
    return *this;
  }

  SOURCE_FORCEINLINE void operator+=(size_t nNumElements) {
...
class CStridedConstPtr {
 ...
 public:
  ...
  SOURCE_FORCEINLINE operator const T *(void)const { return m_pData; }

  SOURCE_FORCEINLINE CStridedConstPtr<T> &operator++(void) {
    m_pData += m_nStride;
    return *this;
  }
  SOURCE_FORCEINLINE void operator+=(size_t nNumElements) {
...
class CM128InitialAttributeIterator : public CStridedConstPtr<fltx4> {
 public:
  SOURCE_FORCEINLINE CM128InitialAttributeIterator(
      int nAttribute, CParticleCollection *pParticles) {
...
class CM128AttributeIterator : public CStridedConstPtr<fltx4> {
 public:
  SOURCE_FORCEINLINE CM128AttributeIterator(int nAttribute,
                                            CParticleCollection *pParticles) {
...
class C4IAttributeIterator : public CStridedConstPtr<FourInts> {
 public:
  SOURCE_FORCEINLINE C4IAttributeIterator(int nAttribute,
                                          CParticleCollection *pParticles) {
...
class CM128AttributeWriteIterator : public CStridedPtr<fltx4> {
 public:
  SOURCE_FORCEINLINE CM128AttributeWriteIterator(
      int nAttribute, CParticleCollection *pParticles) {
...
class C4VAttributeIterator : public CStridedConstPtr<FourVectors> {
 public:
  SOURCE_FORCEINLINE C4VAttributeIterator(int nAttribute,
                                          CParticleCollection *pParticles) {
...
class C4VInitialAttributeIterator : public CStridedConstPtr<FourVectors> {
 public:
  SOURCE_FORCEINLINE C4VInitialAttributeIterator(
      int nAttribute, CParticleCollection *pParticles) {
...
class C4VAttributeWriteIterator : public CStridedPtr<FourVectors> {
 public:
  SOURCE_FORCEINLINE C4VAttributeWriteIterator(
      int nAttribute, CParticleCollection *pParticles) {
...
inline void CParticleCollection::SwapPosAndPrevPos(void) {
  ...
  swap(m_pParticleAttributes[PARTICLE_ATTRIBUTE_XYZ],
...
inline const int32_t *CParticleCollection::GetIntAttributePtr(
    int nAttribute, size_t *pStrideOut) const {
  *(pStrideOut) = m_nParticleFloatStrides[nAttribute];
  return reinterpret_cast<int32_t *>(m_pParticleAttributes[nAttribute]);
}
...
SOURCE_FORCEINLINE void CParticleCollection::KillParticle(int nPidx) {
...
SOURCE_FORCEINLINE void SetVectorAttribute(float *pAttribute, float x, float y,
                                           float z) {
  pAttribute[0] = x;
  pAttribute[4] = y;
  pAttribute[8] = z;
...
SOURCE_FORCEINLINE void SetVectorAttribute(float *pAttribute, const Vector &v) {
  pAttribute[0] = v.x;
  pAttribute[4] = v.y;
  pAttribute[8] = v.z;
}

SOURCE_FORCEINLINE void SetVectorFromAttribute(Vector &v,
...
SOURCE_FORCEINLINE float CParticleCollection::ComputeSqrDistanceToParticle(
...
SOURCE_FORCEINLINE void CParticleCollection::GrowDistSqrBounds(
    float flDistSqr) {
...
class CParticleSystemDefinition {
 ...
 private:
  ...
  char m_pszMaterialName[SOURCE_MAX_PATH];
  ...
  float m_flMinimumSimTime;  // minimum time to sim before drawing first frame -
                             // prevents all capped particles from drawing at 0
                             // time.
 ...
 public:
  ...
  float m_flNoDrawTimeToGoToSleep;  // after not beeing seen for this long, the
                                    // system will sleep
...
SOURCE_FORCEINLINE const Vector &
CParticleCollection::GetControlPointAtCurrentTime(int nControlPoint) const {
...
SOURCE_FORCEINLINE void
CParticleCollection::GetControlPointOrientationAtCurrentTime(
...
SOURCE_FORCEINLINE int CParticleCollection::GetControlPointParent(
