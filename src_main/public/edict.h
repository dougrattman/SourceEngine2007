...
class CBaseEdict {
 ...
 public:
  ...
  int m_NetworkSerialNumber;  // Game DLL sets this when it gets a serial number
                              // for its EHANDLE.
...
inline void CBaseEdict::StateChanged(unsigned short offset) {
  ...
  if (accessor->GetChangeInfoSerialNumber() ==
    ...
    if (g_pSharedChangeInfo->m_nChangeInfos == MAX_EDICT_CHANGE_INFOS) {
      // Shucks.. have to mark the edict as fully changed because we don't have
      // room to remember this change.
      accessor->SetChangeInfoSerialNumber(0);
...
inline ICollideable *edict_t::GetCollideable() {
  ...
  if (pEnt) return pEnt->GetCollideable();
