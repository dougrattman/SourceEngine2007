 ...
 public:
  T *Alloc() {
    ...
    if (!p) {
      ...
      if (((size_t)p) % TSLIST_NODE_ALIGNMENT != 0) DebuggerBreak();
    }
...
CBoneCache *CBoneCache::CreateResource(const bonecacheparams_t &params) {
  ...
  int size = (sizeof(CBoneCache) + tableSizeStudio + tableSizeCached +
              matrixSize + 3) &
             ~3;

  CBoneCache *pMem = (CBoneCache *)malloc(size);
  Construct(pMem);
  pMem->Init(params, size, studioToCachedIndex, cachedToStudioIndex,
             cachedBoneCount);
  return pMem;
}
...
unsigned int CBoneCache::EstimatedSize(const bonecacheparams_t &params) {
  ...
  return (params.pStudioHdr->numbones() *
              (sizeof(short) + sizeof(short) + sizeof(matrix3x4_t)) +
          3) &
         ~3;
}
...
void CBoneCache::Init(const bonecacheparams_t &params, unsigned int size,
  ...
  UpdateBones(params.pBoneToWorld, params.pStudioHdr->numbones(),
              params.curtime);
}

void CBoneCache::UpdateBones(const matrix3x4_t *pBoneToWorld, int numbones,
                             float curtime) {
...
bool CBoneCache::IsValid(float curtime, float dt) {
  if (curtime - m_timeValid <= dt) return true;
  return false;
}
...
static CDataManager<CBoneCache, bonecacheparams_t, CBoneCache *,
                    CThreadFastMutex>
    g_StudioBoneCache(24 * 1024L);
...
void BuildBoneChain(const CStudioHdr *pStudioHdr, const matrix3x4_t &rootxform,
  ...
  BuildBoneChain(pStudioHdr, rootxform, pos, q, iBone, pBoneToWorld,
                 boneComputed);
  return;
}
...
//-----------------------------------------------------------------------------
void ExtractAnimValue(int frame, mstudioanimvalue_t *panimvalue, float scale,
                      float &v1, float &v2) {
  if (!panimvalue) {
    v1 = v2 = 0;
    return;
  ...
  // There is probably a better long-term solution; Ken is going to look into
  // it.
...
// Purpose: return a sub frame rotation for a single bone
//-----------------------------------------------------------------------------
void CalcBoneQuaternion(int frame, float s, const Quaternion &baseQuat,
                        const RadianEuler &baseRot, const Vector &baseRotScale,
  ...
  if (s > 0.001f) {
    ...
    ExtractAnimValue(frame, pValuesPtr->pAnimvalue(0), baseRotScale.x, angle1.x,
                     angle2.x);
    ExtractAnimValue(frame, pValuesPtr->pAnimvalue(1), baseRotScale.y, angle1.y,
                     angle2.y);
    ExtractAnimValue(frame, pValuesPtr->pAnimvalue(2), baseRotScale.z, angle1.z,
                     angle2.z);
  ...
  if (!(panim->flags & STUDIO_ANIM_DELTA) &&
      (iBaseFlags & BONE_FIXED_ALIGNMENT)) {
    QuaternionAlign(baseAlignment, q, q);
  }
}

inline void CalcBoneQuaternion(int frame, float s, const mstudiobone_t *pBone,
                               const mstudiolinearbone_t *pLinearBones,
                               const mstudioanim_t *panim, Quaternion &q) {
  if (pLinearBones) {
    CalcBoneQuaternion(frame, s, pLinearBones->quat(panim->bone),
                       pLinearBones->rot(panim->bone),
                       pLinearBones->rotscale(panim->bone),
                       pLinearBones->flags(panim->bone),
                       pLinearBones->qalignment(panim->bone), panim, q);
  } else {
    CalcBoneQuaternion(frame, s, pBone->quat, pBone->rot, pBone->rotscale,
                       pBone->flags, pBone->qAlignment, panim, q);
  }
}
...
//-----------------------------------------------------------------------------
void CalcBonePosition(int frame, float s, const Vector &basePos,
                      const Vector &baseBoneScale, const mstudioanim_t *panim,
                      Vector &pos) {
...
inline void CalcBonePosition(int frame, float s, const mstudiobone_t *pBone,
                             const mstudiolinearbone_t *pLinearBones,
                             const mstudioanim_t *panim, Vector &pos) {
  if (pLinearBones) {
    CalcBonePosition(frame, s, pLinearBones->pos(panim->bone),
                     pLinearBones->posscale(panim->bone), panim, pos);
  } else {
...
static void CalcDecompressedAnimation(
    const mstudiocompressedikerror_t *pCompressed, int iFrame, float fraq,
    Vector &pos, Quaternion &q) {
  if (fraq > 0.0001f) {
    Vector p1, p2;
    ExtractAnimValue(iFrame, pCompressed->pAnimvalue(0), pCompressed->scale[0],
                     p1.x, p2.x);
    ExtractAnimValue(iFrame, pCompressed->pAnimvalue(1), pCompressed->scale[1],
                     p1.y, p2.y);
    ExtractAnimValue(iFrame, pCompressed->pAnimvalue(2), pCompressed->scale[2],
                     p1.z, p2.z);
    pos = p1 * (1 - fraq) + p2 * fraq;

    Quaternion q1, q2;
    RadianEuler angle1, angle2;
    ExtractAnimValue(iFrame, pCompressed->pAnimvalue(3), pCompressed->scale[3],
                     angle1.x, angle2.x);
    ExtractAnimValue(iFrame, pCompressed->pAnimvalue(4), pCompressed->scale[4],
                     angle1.y, angle2.y);
    ExtractAnimValue(iFrame, pCompressed->pAnimvalue(5), pCompressed->scale[5],
                     angle1.z, angle2.z);
...
static void CalcLocalHierarchyAnimation(
    const CStudioHdr *pStudioHdr, matrix3x4_t *boneToWorld,
    CBoneBitList &boneComputed, Vector *pos, Quaternion *q,
    // const mstudioanimdesc_t &animdesc,
    const mstudiobone_t *pbone, mstudiolocalhierarchy_t *pHierarchy, int iBone,
    int iNewParent, float cycle, int iFrame, float flFraq, int boneMask) {
  Vector localPos;
  Quaternion localQ;
  ...
  // TODO(d.rattman): missing check to see if seq has a weight for this bone
  float weight = 1.0f;
  ...
  if (pHierarchy->tail - pHierarchy->peak < 1.0f) {
    ...
    if (index < pHierarchy->start) return;
    if (index >= pHierarchy->end) return;

    if (index < pHierarchy->peak && pHierarchy->start != pHierarchy->peak) {
      weight =
          (index - pHierarchy->start) / (pHierarchy->peak - pHierarchy->start);
    } else if (index > pHierarchy->tail &&
               pHierarchy->end != pHierarchy->tail) {
  ...
  CalcDecompressedAnimation(pHierarchy->pLocalAnim(),
                            iFrame - pHierarchy->iStart, flFraq, localPos,
                            localQ);
...
static void CalcZeroframeData(const CStudioHdr *pStudioHdr,
                              const studiohdr_t *pAnimStudioHdr,
                              const virtualgroup_t *pAnimGroup,
                              const mstudiobone_t *pAnimbone,
                              mstudioanimdesc_t &animdesc, float fFrame,
                              Vector *pos, Quaternion *q, int boneMask,
                              float flWeight) {
  uint8_t *pData = animdesc.pZeroFrameData();
  ...
  if (animdesc.zeroframecount == 1) {
    ...
    if (index >= animdesc.zeroframecount - 1) {
      ...
      s1 = std::clamp(
          (fFrame - index * animdesc.zeroframespan) / animdesc.zeroframespan,
          0.0f, 1.0f);
    }
...
//-----------------------------------------------------------------------------
// Purpose: Find and decode a sub-frame of animation, remapping the skeleton
// bone indexes
//-----------------------------------------------------------------------------
static void CalcVirtualAnimation(virtualmodel_t *pVModel,
                                 const CStudioHdr *pStudioHdr, Vector *pos,
  ...
  if (!panim) {
    CalcZeroframeData(pStudioHdr, pAnimStudioHdr, pAnimGroup, pAnimbone,
                      animdesc, fFrame, pos, q, boneMask, 1.0);
    return;
  }

  // TODO(d.rattman): change encoding so that bone -1 is never the case
    ...
    if (j >= 0 && (pStudioHdr->boneFlags(j) & boneMask)) {
      ...
      if (k >= 0 && pweight[k] > 0.0f) {
        CalcBoneQuaternion(iLocalFrame, s, &pAnimbone[panim->bone],
                           pAnimLinearBones, panim, q[j]);
        CalcBonePosition(iLocalFrame, s, &pAnimbone[panim->bone],
                         pAnimLinearBones, panim, pos[j]);
#ifdef STUDIO_ENABLE_PERF_COUNTERS
  ...
  if (flStall > 0.0f) {
    CalcZeroframeData(pStudioHdr, pAnimStudioHdr, pAnimGroup, pAnimbone,
                      animdesc, fFrame, pos, q, boneMask, flStall);
  }
    ...
    for (i = 0; i < animdesc.numlocalhierarchy; i++) {
      ...
      if (iBone >= 0 && (pStudioHdr->boneFlags(iBone) & boneMask)) {
        ...
        if (iNewParent >= 0 && (pStudioHdr->boneFlags(iNewParent) & boneMask)) {
          CalcLocalHierarchyAnimation(pStudioHdr, boneToWorld, boneComputed,
                                      pos, q, pbone, pHierarchy, iBone,
                                      iNewParent, cycle, iFrame, s, boneMask);
        }
      }
    }
...
static void CalcAnimation(const CStudioHdr *pStudioHdr, Vector *pos,
                          Quaternion *q, mstudioseqdesc_t &seqdesc,
                          int sequence, int animation, float cycle,
                          int boneMask) {
#ifdef STUDIO_ENABLE_PERF_COUNTERS
  ...
  if (pVModel) {
    CalcVirtualAnimation(pVModel, pStudioHdr, pos, q, seqdesc, sequence,
                         animation, cycle, boneMask);
    return;
  }
  ...
  if (!panim) {
    ...
    CalcZeroframeData(pStudioHdr, pStudioHdr->GetRenderHdr(), NULL,
                      pStudioHdr->pBone(0), animdesc, fFrame, pos, q, boneMask,
                      1.0);
    ...
    for (int i = 0; i < animdesc.numlocalhierarchy; i++) {
      ...
      if (pStudioHdr->boneFlags(pHierarchy->iBone) & boneMask) {
        if (pStudioHdr->boneFlags(pHierarchy->iNewParent) & boneMask) {
          CalcLocalHierarchyAnimation(pStudioHdr, boneToWorld, boneComputed,
                                      pos, q, pbone, pHierarchy,
                                      pHierarchy->iBone, pHierarchy->iNewParent,
...
// Purpose: qt = p + s * q
//-----------------------------------------------------------------------------
void QuaternionAccumulate(const Quaternion &p, float s, const Quaternion &q,
                          Quaternion &qt) {
  Quaternion q2;
...
//-----------------------------------------------------------------------------
// Purpose: blend together in world space q1,pos1 with q2,pos2.  Return result
// in q1,pos1.
//			0 returns q1, pos1.  1 returns q2, pos2
//-----------------------------------------------------------------------------

void WorldSpaceSlerp(const CStudioHdr *pStudioHdr,
                     Quaternion q1[MAXSTUDIOBONES], Vector pos1[MAXSTUDIOBONES],
                     mstudioseqdesc_t &seqdesc, int sequence,
  ...
  for (i = 0; i < pStudioHdr->numbones(); i++) {
    ...
    if (s1 == 1.0 && s2 == 1.0) {
      ...
      BuildBoneChain(pStudioHdr, rootXform, pos1, q1, i, destBoneToWorld,
                     destBoneComputed);
      BuildBoneChain(pStudioHdr, rootXform, pos2, q2, i, srcBoneToWorld,
                     srcBoneComputed);
...
void SlerpBones(const CStudioHdr *pStudioHdr, Quaternion q1[MAXSTUDIOBONES],
                ...
                int sequence, const QuaternionAligned q2[MAXSTUDIOBONES],
                const Vector pos2[MAXSTUDIOBONES], float s, int boneMask) {
  if (s <= 0.0f) return;
  if (s > 1.0f) {
    s = 1.0f;
  ...
  if (seqdesc.flags & STUDIO_WORLD) {
    WorldSpaceSlerp(pStudioHdr, q1, pos1, seqdesc, sequence, q2, pos2, s,
                    boneMask);
    return;
  }
  ...
  float *pS2 = stack_alloc<float>(nBoneCount);
...
//			blend together q1,pos1 with q2,pos2.  Return result in
// q1,pos1. 			0 returns q1, pos1.  1 returns q2, pos2
//-----------------------------------------------------------------------------
void BlendBones(const CStudioHdr *pStudioHdr, Quaternion q1[MAXSTUDIOBONES],
                Vector pos1[MAXSTUDIOBONES], mstudioseqdesc_t &seqdesc,
...
//-----------------------------------------------------------------------------
void ScaleBones(const CStudioHdr *pStudioHdr, Quaternion q1[MAXSTUDIOBONES],
                Vector pos1[MAXSTUDIOBONES], int sequence, float s,
                int boneMask) {
  int i, j;
  Quaternion q3;
...
//-----------------------------------------------------------------------------
// Purpose: resolve a global pose parameter to the specific setting for this
// sequence
//-----------------------------------------------------------------------------
void Studio_LocalPoseParameter(const CStudioHdr *pStudioHdr,
  ...
  int iPose = pStudioHdr->GetSharedPoseParameter(
      iSequence, seqdesc.paramindex[iLocalIndex]);
  ...
  if (seqdesc.posekeyindex == 0) {
    float flLocalStart = ((float)seqdesc.paramstart[iLocalIndex] - Pose.start) /
                         (Pose.end - Pose.start);
    float flLocalEnd = ((float)seqdesc.paramend[iLocalIndex] - Pose.start) /
                       (Pose.end - Pose.start);
    ...
    if (flSetting < 0) flSetting = 0;
    if (flSetting > 1) flSetting = 1;
    ...
    // TODO(d.rattman): this needs to be 2D
    // TODO(d.rattman): this shouldn't be a linear search

    while (1) {
      flSetting = (flValue - seqdesc.poseKey(iLocalIndex, index)) /
                  (seqdesc.poseKey(iLocalIndex, index + 1) -
                   seqdesc.poseKey(iLocalIndex, index));
      /*
    ...
    // clamp.
    if (flSetting < 0.0f) flSetting = 0.0f;
    if (flSetting > 1.0f) flSetting = 1.0f;
  }
}
...
void Studio_CalcBoneToBoneTransform(const CStudioHdr *pStudioHdr,
  ...
  ConcatTransforms(pStudioHdr->pBone(outputBoneIndex)->poseToBone, inputToPose,
                   matrixOut);
}
...
//-----------------------------------------------------------------------------
void InitPose(const CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[],
              int boneMask) {
...
inline bool PoseIsAllZeros(const CStudioHdr *pStudioHdr, int sequence,
                           mstudioseqdesc_t &seqdesc, int i0, int i1) {
  int baseanim;
...
// Returns: returns the animination indices and barycentric coordinates of a
// triangle
//			the triangle is a right triangle, and the diagonal is
// between elements [0] and [2]
//-----------------------------------------------------------------------------

static ConVar anim_3wayblend("anim_3wayblend", "1", FCVAR_REPLICATED,
                             "Toggle the 3-way animation blending code.");

void Calc3WayBlendIndices(int i0, int i1, float s0, float s1,
                          const mstudioseqdesc_t &seqdesc, int *pAnimIndices,
                          float *pWeight) {
  ...
  /*
  float w0 = ((x2-x3)*(y3-s1) - (x3-s0)*(y2-y3)) / ((x1-x3)*(y2-y3) -
  (x2-x3)*(y1-y3)); float w1 = ((x1-x3)*(y3-s1) - (x3-s0)*(y1-y3)) /
  ((x2-x3)*(y1-y3) - (x1-x3)*(y2-y3)); Assert( pWeight[0] == w0 && pWeight[1] ==
  w1 );
  */
...
// Purpose: calculate a pose for a single sequence
//-----------------------------------------------------------------------------
bool CalcPoseSingle(const CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[],
                    mstudioseqdesc_t &seqdesc, int sequence, float cycle,
                    const float poseParameter[], int boneMask, float flTime) {
  bool bResult = true;
  ...
  Studio_LocalPoseParameter(pStudioHdr, poseParameter, seqdesc, sequence, 0, s0,
                            i0);
  Studio_LocalPoseParameter(pStudioHdr, poseParameter, seqdesc, sequence, 1, s1,
                            i1);
  ...
  if (seqdesc.flags & STUDIO_REALTIME) {
    ...
    int iPose =
        pStudioHdr->GetSharedPoseParameter(sequence, seqdesc.cycleposeindex);
    if (iPose != -1) {
      /*
      const mstudioposeparamdesc_t &Pose = pStudioHdr->pPoseParameter( iPose );
  ...
  if (s0 < 0.001) {
    if (s1 < 0.001) {
      if (PoseIsAllZeros(pStudioHdr, sequence, seqdesc, i0, i1)) {
        ...
        CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence,
                      seqdesc.anim(i0, i1), cycle, boneMask);
      }
    } else if (s1 > 0.999) {
      CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence,
                    seqdesc.anim(i0, i1 + 1), cycle, boneMask);
    } else {
      CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence, seqdesc.anim(i0, i1),
                    cycle, boneMask);
      CalcAnimation(pStudioHdr, pos2, q2, seqdesc, sequence,
                    seqdesc.anim(i0, i1 + 1), cycle, boneMask);
    ...
    if (s1 < 0.001) {
      if (PoseIsAllZeros(pStudioHdr, sequence, seqdesc, i0 + 1, i1)) {
        ...
        CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence,
                      seqdesc.anim(i0 + 1, i1), cycle, boneMask);
      }
    } else if (s1 > 0.999) {
      CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence,
                    seqdesc.anim(i0 + 1, i1 + 1), cycle, boneMask);
    } else {
      CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence,
                    seqdesc.anim(i0 + 1, i1), cycle, boneMask);
      CalcAnimation(pStudioHdr, pos2, q2, seqdesc, sequence,
    ...
    if (s1 < 0.001) {
      if (PoseIsAllZeros(pStudioHdr, sequence, seqdesc, i0 + 1, i1)) {
        ...
        BlendBones(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2, s0,
                   boneMask);
      }
    } else if (s1 > 0.999) {
      CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence,
      ...
      CalcAnimation(pStudioHdr, pos2, q2, seqdesc, sequence,
                    seqdesc.anim(i0, i1 + 1), cycle, boneMask);
      CalcAnimation(pStudioHdr, pos3, q3, seqdesc, sequence,
                    seqdesc.anim(i0 + 1, i1 + 1), cycle, boneMask);
      BlendBones(pStudioHdr, q2, pos2, seqdesc, sequence, q3, pos3, s0,
                 boneMask);
      ...
      if (weight[1] < 0.001) {
        ...
        CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence, iAnimIndices[0],
                      cycle, boneMask);
        CalcAnimation(pStudioHdr, pos2, q2, seqdesc, sequence, iAnimIndices[2],
                      cycle, boneMask);
        BlendBones(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2,
                   weight[2] / (weight[0] + weight[2]), boneMask);
      } else {
        CalcAnimation(pStudioHdr, pos, q, seqdesc, sequence, iAnimIndices[0],
                      cycle, boneMask);
        CalcAnimation(pStudioHdr, pos2, q2, seqdesc, sequence, iAnimIndices[1],
                      cycle, boneMask);
        BlendBones(pStudioHdr, q, pos, seqdesc, sequence, q2, pos2,
                   weight[1] / (weight[0] + weight[1]), boneMask);

        CalcAnimation(pStudioHdr, pos3, q3, seqdesc, sequence, iAnimIndices[2],
                      cycle, boneMask);
        BlendBones(pStudioHdr, q, pos, seqdesc, sequence, q3, pos3, weight[2],
                   boneMask);
      }
    }
  }
...
//-----------------------------------------------------------------------------
void AddSequenceLayers(const CStudioHdr *pStudioHdr, CIKContext *pIKContext,
                       Vector pos[], Quaternion q[], mstudioseqdesc_t &seqdesc,
                       int sequence, float cycle, const float poseParameter[],
                       int boneMask, float flWeight, float flTime) {
  for (int i = 0; i < seqdesc.numautolayers; i++) {
    ...
    if (pLayer->start != pLayer->end) {
      ...
      if (!(pLayer->flags & STUDIO_AL_POSE)) {
        ...
        int iPose =
            pStudioHdr->GetSharedPoseParameter(iSequence, pLayer->iPose);
        if (iPose != -1) {
          const mstudioposeparamdesc_t &Pose =
              pStudioHdr->pPoseParameter(iPose);
      ...
      if (index < pLayer->start) continue;
      if (index >= pLayer->end) continue;
    ...
    int iSequence = pStudioHdr->iRelativeSeq(sequence, pLayer->iSequence);
    AccumulatePose(pStudioHdr, pIKContext, pos, q, iSequence, layerCycle,
                   poseParameter, boneMask, layerWeight, flTime);
  }
}
...
//			adds autolayers, runs local ik rukes
//-----------------------------------------------------------------------------
void AddLocalLayers(const CStudioHdr *pStudioHdr, CIKContext *pIKContext,
                    Vector pos[], Quaternion q[], mstudioseqdesc_t &seqdesc,
                    int sequence, float cycle, const float poseParameter[],
                    int boneMask, float flWeight, float flTime) {
...
void CalcPose(const CStudioHdr *pStudioHdr, CIKContext *pIKContext,
              ...
              const float poseParameter[], int boneMask, float flWeight,
              float flTime) {
  mstudioseqdesc_t &seqdesc = pStudioHdr->pSeqdesc(sequence);

  Assert(flWeight >= 0.0f && flWeight <= 1.0f);
  // This shouldn't be necessary, but the Assert should help us catch whoever is
  // screwing this up
  flWeight = std::clamp(flWeight, 0.0f, 1.0f);

  // add any IK locks to prevent numautolayers from moving extremities
  CIKContext seq_ik;
  if (seqdesc.numiklocks) {
    seq_ik.Init(
        pStudioHdr, vec3_angle, vec3_origin, 0.0, 0,
        boneMask);  // local space relative so absolute position doesn't mater
    seq_ik.AddSequenceLocks(seqdesc, pos, q);
  }

  CalcPoseSingle(pStudioHdr, pos, q, seqdesc, sequence, cycle, poseParameter,
                 boneMask, flTime);

  if (pIKContext) {
    pIKContext->AddDependencies(seqdesc, sequence, cycle, poseParameter,
                                flWeight);
  }

  AddSequenceLayers(pStudioHdr, pIKContext, pos, q, seqdesc, sequence, cycle,
                    poseParameter, boneMask, flWeight, flTime);
...
//			adds autolayers, runs local ik rukes
//-----------------------------------------------------------------------------
void AccumulatePose(const CStudioHdr *pStudioHdr, CIKContext *pIKContext,
                    Vector pos[], Quaternion q[], int sequence, float cycle,
                    const float poseParameter[], int boneMask, float flWeight,
                    float flTime) {
  ...
  g_pPoseDebugger->AccumulatePose(pStudioHdr, pIKContext, pos, q, sequence,
                                  cycle, poseParameter, boneMask, flWeight,
                                  flTime);
#endif
  ...
  if (CalcPoseSingle(pStudioHdr, pos2, q2, seqdesc, sequence, cycle,
                     poseParameter, boneMask, flTime)) {
    // this weight is wrong, the IK rules won't composite at the correct
    // intensity
    AddLocalLayers(pStudioHdr, pIKContext, pos2, q2, seqdesc, sequence, cycle,
                   poseParameter, boneMask, 1.0, flTime);
...
//			0 returns q1, pos1.  1 returns q2, pos2
//-----------------------------------------------------------------------------
void CalcBoneAdj(const CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[],
                 const float controllers[], int boneMask) {
  int i, j, k;
  float value;
  ...
  for (j = 0; j < pStudioHdr->numbonecontrollers(); j++) {
    ...
    if (pStudioHdr->boneFlags(k) & boneMask) {
      ...
      value =
          (1.0 - value) * pbonecontroller->start + value * pbonecontroller->end;
...
class CIKSolver {
 public:
  ...
  static void rot(float const M[3][3], float const src[], float dst[]) {
    for (int i = 0; i < 3; i++) dst[i] = dot(M[i], src);
  }
};
...
//-----------------------------------------------------------------------------
#if 1
inline void debugLine(const Vector &origin, const Vector &dest, int r, int g,
                      int b, bool noDepthTest, float duration){};
#else
extern void drawLine(const Vector &p1, const Vector &p2, int r = 0, int g = 0,
                     int b = 1, bool noDepthTest = true, float duration = 0.1);
...
//-----------------------------------------------------------------------------
bool Studio_SolveIK(mstudioikchain_t *pikchain, Vector &targetFoot,
                    matrix3x4_t *pBoneToWorld) {
  if (pikchain->pLink(0)->kneeDir.LengthSqr() > 0.0) {
    Vector targetKneeDir, targetKneePos;
    // TODO(d.rattman): knee length should be as long as the legs
    ...
    return Studio_SolveIK(pikchain->pLink(0)->bone, pikchain->pLink(1)->bone,
                          pikchain->pLink(2)->bone, targetFoot, targetKneePos,
                          targetKneeDir, pBoneToWorld);
  } else {
    return Studio_SolveIK(pikchain->pLink(0)->bone, pikchain->pLink(1)->bone,
                          pikchain->pLink(2)->bone, targetFoot, pBoneToWorld);
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Solve Knee position for a known hip and foot location, but no
// specific knee direction preference
//-----------------------------------------------------------------------------
...
bool Studio_SolveIK(int iThigh, int iKnee, int iFoot, Vector &targetFoot,
  ...
  return Studio_SolveIK(iThigh, iKnee, iFoot, targetFoot, worldKnee, ikKneeDir,
                        pBoneToWorld);
}
...
void Studio_AlignIKMatrix(matrix3x4_t &mMat, const Vector &vAlignTo) {
  ...
  // TODO(d.rattman): check for X being too near to Z
...
//-----------------------------------------------------------------------------
// Purpose: Solve Knee position for a known hip and foot location, and a known
// knee direction
//-----------------------------------------------------------------------------
...
bool Studio_SolveIK(int iThigh, int iKnee, int iFoot, Vector &targetFoot,
  ...
  if (ik.solve(l1, l2, ikFoot.Base(), ikTargetKnee.Base(), ikKnee.Base())) {
    ...
    // true, 0 ); debugLine( ikKnee + worldThigh, ikFoot + worldThigh, color[0],
    // color[1], color[2], true,0 );
    ...
    debugLine( worldThigh + ikKnee, worldThigh + ikTargetKnee, 255, 0, 0, true,
    0 );
    */
    return false;
  }
...
float Studio_IKRuleWeight(mstudioikrule_t &ikRule,
                          const mstudioanimdesc_t *panim, float flCycle,
                          int &iFrame, float &fraq) {
...
bool Studio_IKAnimationError(const CStudioHdr *pStudioHdr,
                             mstudioikrule_t *pRule,
                             const mstudioanimdesc_t *panim, float flCycle,
                             Vector &pos, Quaternion &q, float &flWeight) {
  float fraq;
  int iFrame;
  ...
  if (pCompressed != NULL) {
    CalcDecompressedAnimation(pCompressed, iFrame - pRule->iStart, fraq, pos,
                              q);
    return true;
  }
...
bool Studio_IKSequenceError(const CStudioHdr *pStudioHdr,
                            mstudioseqdesc_t &seqdesc, int iSequence,
                            float flCycle, int iRule,
                            const float poseParameter[],
  ...
  for (i = 0; i < 4; i++) {
    if (weight[i]) {
      if (iRule >= panim[i]->numikrules ||
          panim[i]->numikrules != panim[0]->numikrules) {
        Assert(0);
        return false;
      }
  ...
  if (ikRule.flWeight <= 0.001f) {
    ...
    if (panim[0]->pIKRule(iRule) == NULL) return false;
  ...
  for (i = 0; i < 4; i++) {
    if (weight[i]) {
      ...
      ikRule.chain = pRule->chain;  // TODO(d.rattman): this is anim local
      ikRule.bone = pRule->bone;    // TODO(d.rattman): this is anim local
      ...
      switch (ikRule.type) {
        ...
        case IK_GROUND:
        case IK_ATTACHMENT: {
          int bResult = Studio_IKAnimationError(pStudioHdr, pRule, panim[i],
                                                flCycle, pos1, q1, w);
  ...
  if (ikRule.type == IK_SELF && ikRule.bone != -1) {
    // TODO(d.rattman): this is anim local, not seq local!
    ikRule.bone = pStudioHdr->RemapSeqBone(iSequence, ikRule.bone);
    if (ikRule.bone == -1) return false;
  }
...
void CIKContext::Init(const CStudioHdr *pStudioHdr, const QAngle &angles,
                      const Vector &pos, float flTime, int iFramecounter,
                      int boneMask) {
  ...
  if (pStudioHdr->numikchains()) {
    ...
    // TODO(d.rattman): Brutal hackery to prevent a crash
    if (m_target.Count() == 0) {
      m_target.SetSize(12);
...
void CIKContext::AddDependencies(mstudioseqdesc_t &seqdesc, int iSequence,
  ...
  Studio_SeqAnims(m_pStudioHdr, seqdesc, iSequence, poseParameters, panim,
                  weight);

  // TODO(d.rattman): add proper number of rules!!!
  for (i = 0; i < seqdesc.numikrules; i++) {
    if (!Studio_IKSequenceError(m_pStudioHdr, seqdesc, iSequence, flCycle, i,
                                poseParameters, panim, weight, ikrule))
      continue;
    ...
    if (ikrule.bone >= 0 &&
        !(m_pStudioHdr->boneFlags(ikrule.bone) & m_boneMask))
      continue;
...
void CIKContext::AddAutoplayLocks(Vector pos[], Quaternion q[]) {
  ...
  int ikOffset =
      m_ikLock.AddMultipleToTail(m_pStudioHdr->GetNumIKAutoplayLocks());
  memset(&m_ikLock[ikOffset], 0,
         sizeof(ikcontextikrule_t) * m_pStudioHdr->GetNumIKAutoplayLocks());
  ...
  for (int i = 0; i < m_pStudioHdr->GetNumIKAutoplayLocks(); i++) {
    ...
    if (pchain->pLink(0)->kneeDir.LengthSqr() > 0.0) {
      ...
      VectorRotate(pchain->pLink(0)->kneeDir,
                   boneToWorld[pchain->pLink(0)->bone], ikrule.kneeDir);
...
void CIKContext::AddSequenceLocks(mstudioseqdesc_t &seqdesc, Vector pos[],
                                  Quaternion q[]) {
  ...
  memset(&m_ikLock[ikOffset], 0,
         sizeof(ikcontextikrule_t) * seqdesc.numiklocks);
...
//-----------------------------------------------------------------------------
void CIKContext::BuildBoneChain(const Vector pos[], const Quaternion q[],
                                int iBone, matrix3x4_t *pBoneToWorld,
                                CBoneBitList &boneComputed) {
  Assert(m_pStudioHdr->boneFlags(iBone) & m_boneMask);
  ::BuildBoneChain(m_pStudioHdr, m_rootxform, pos, q, iBone, pBoneToWorld,
                   boneComputed);
}
...
//-----------------------------------------------------------------------------
// Purpose: turn a specific bones boneToWorld transform into a pos and q in
// parents bonespace
//-----------------------------------------------------------------------------
void SolveBone(const CStudioHdr *pStudioHdr, int iBone,
               matrix3x4_t *pBoneToWorld, Vector pos[], Quaternion q[]) {
...
void CIKTarget::SetOwner(int entindex, const Vector &pos,
                         const QAngle &angles) {
...
void CIKTarget::ClearOwner() { latched.owner = -1; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------

int CIKTarget::GetOwner() { return latched.owner; }

//-----------------------------------------------------------------------------
// Purpose: update the latched IK values that are in a moving frame of reference
//-----------------------------------------------------------------------------

void CIKTarget::UpdateOwner(int entindex, const Vector &pos,
                            const QAngle &angles) {
  if (pos == latched.absOrigin && angles == latched.absAngles) return;
...
//			normal of the ground and the desired ground identity
// orientation
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: estimates the ground impact at the center location assuming a the
// edge of
//			an Z axis aligned disc collided with it the surface.
//-----------------------------------------------------------------------------

void CIKTarget::SetPosWithNormalOffset(const Vector &pos,
                                       const Vector &normal) {
  // assume it's a disc edge intersecting with the floor, so try to estimate the
...
//-----------------------------------------------------------------------------
// Purpose: Run through the rules that survived and turn a specific bones
// boneToWorld
//			transform into a pos and q in parents bonespace
//-----------------------------------------------------------------------------

void CIKContext::UpdateTargets(Vector pos[], Quaternion q[],
      ...
      switch (pRule->type) {
        ...
        case IK_GROUND:
          // case IK_SELF:
          {
            ...
            if (pRule->flRuleWeight == 1.0f || pTarget->est.flWeight == 0.0f) {
              ...
              QuaternionSlerp(pTarget->offset.q, pRule->q, pRule->flRuleWeight,
                              pTarget->offset.q);
              pTarget->offset.pos =
                  Lerp(pRule->flRuleWeight, pTarget->offset.pos, pRule->pos);
              pTarget->est.height =
                  Lerp(pRule->flRuleWeight, pTarget->est.height, pRule->height);
              pTarget->est.floor =
                  Lerp(pRule->flRuleWeight, pTarget->est.floor, pRule->floor);
              pTarget->est.radius =
                  Lerp(pRule->flRuleWeight, pTarget->est.radius, pRule->radius);
              // pTarget->est.latched = Lerp( pRule->flRuleWeight,
              // pTarget->est.latched, pRule->latched );
              pTarget->est.latched =
                  std::min(pTarget->est.latched, pRule->latched);
              pTarget->est.release = Lerp(pRule->flRuleWeight,
                                          pTarget->est.release, pRule->release);
              pTarget->est.flWeight = Lerp(
                  pRule->flRuleWeight, pTarget->est.flWeight, pRule->flWeight);
            }
        ...
        case IK_UNLATCH: {
          ...
          if (pRule->latched > 0.0)
            ...
            pTarget->est.latched =
                std::min(pTarget->est.latched, 1.0f - pRule->flWeight);
        } break;
        case IK_RELEASE: {
          CIKTarget *pTarget = &m_target[pRule->slot];
          if (pRule->latched > 0.0)
          ...
          pTarget->est.flWeight = (pTarget->est.flWeight) *
                                  (1 - pRule->flWeight * pRule->flRuleWeight);
        } break;
      }
    }
  }
    ...
    if (pTarget->est.flWeight > 0.0) {
      ...
      if (pTarget->latched.bHasLatch) {
        if (pTarget->est.latched == 1.0) {
          ...
          QuaternionSM(-1, pTarget->est.q, pTarget->latched.q,
          ...
          if (latched > 0 && latched < pTarget->latched.influence) {
            ...
            VectorScale(pTarget->latched.deltaPos, (1 - dt),
                        pTarget->latched.deltaPos);
            QuaternionScale(pTarget->latched.deltaQ, (1 - dt),
                            pTarget->latched.deltaQ);
          }

          // move ideal contact position by latched error factor
          pTarget->est.pos = pTarget->est.pos + pTarget->latched.deltaPos;
          QuaternionMA(pTarget->est.q, 1, pTarget->latched.deltaQ,
                       pTarget->est.q);
      ...
      if (pTarget->latched.bHasLatch) {
        ...
        if ((d4 < fabs(d1 - d2) || d4 * 0.95 > d1 + d2) &&
            pTarget->est.latched > 0.2) {
          pTarget->error.flTime = m_flTime;
        }

        // unstick feet when angle is too great
        if (pTarget->est.latched > 0.2) {
          float d = fabs(pTarget->latched.deltaQ.w) * 2.0f -
                    1.0f;  // QuaternionDotProduct( pTarget->latched.q,
                           // pTarget->est.q );

          // TODO(d.rattman): cos(45), make property of chain
          if (d < 0.707) {
            pTarget->error.flTime = m_flTime;
          }
        }
  ...
  for (i = 0; i < m_target.Count(); i++) {
    ...
    if (pTarget->error.bInError || dt < 0.5) {
      ...
      if (pTarget->error.ramp > 0.0) {
        ...
        // don't bother with AutoIKRelease if the bone isn't going to be
        // calculated this code is crashing for some unknown reason.
        if (pTarget->chain >= 0 &&
          ...
          if (pchain != NULL) {
            ...
            if (bone >= 0 && bone < m_pStudioHdr->numbones()) {
              ...
              DevWarning(
                  1, "AutoIKRelease (%s) got an out of range bone %d (%d)\n",
...
void CIKContext::SolveDependencies(Vector pos[], Quaternion q[],
                                   matrix3x4_t boneToWorld[],
                                   CBoneBitList &boneComputed) {
  //	ASSERT_NO_REENTRY();
    ...
    for (i = 0; i < m_ikChainRule.Element(j).Count(); i++) {
      ...
      switch (pRule->type) {
        case IK_SELF: {
          ...
          pChainResult->flWeight =
              pChainResult->flWeight * (1 - flWeight) + flWeight;
          ...
          pChainResult->pos =
              pChainResult->pos * (1.0 - flWeight) + p2 * flWeight;
        ...
        case IK_UNLATCH: {
          /*
          pChainResult->flWeight = pChainResult->flWeight * (1 -
          pRule->flWeight) + pRule->flWeight;

          pChainResult->pos = pChainResult->pos * (1.0 - pRule->flWeight ) +
          pChainResult->local.pos * pRule->flWeight; QuaternionSlerp(
          pChainResult->q, pChainResult->local.q, pRule->flWeight,
          pChainResult->q );
          */
        } break;
      }
  ...
  for (i = 0; i < m_target.Count(); i++) {
    ...
    if (m_target[i].est.flWeight > 0.0) {
      ...
      pChainResult->pos = pChainResult->pos * (1.0 - pChainResult->flWeight) +
                          p2 * pChainResult->flWeight;
      QuaternionSlerp(pChainResult->q, q2, pChainResult->flWeight,
                      pChainResult->q);
    }
  ...
  for (i = 0; i < m_pStudioHdr->numikchains(); i++) {
    ...
    if (pChainResult->flWeight > 0.0) {
      ...
      if (Studio_SolveIK(pchain, pChainResult->pos, boneToWorld)) {
        ...
        QuaternionMatrix(pChainResult->q, p3,
...
void CIKContext::SolveSequenceLocks(mstudioseqdesc_t &seqdesc, Vector pos[],
                                    Quaternion q[]) {
  matrix3x4_t *boneToWorld = g_MatrixPool.Alloc();
  CBoneBitList boneComputed;
  int i;
...
void CIKContext::AddAllLocks(Vector pos[], Quaternion q[]) {
  ...
  memset(&m_ikLock[ikOffset], 0,
         sizeof(ikcontextikrule_t) * m_pStudioHdr->GetNumIKChains());
...
void CIKContext::SolveLock(const mstudioiklock_t *plock, int i, Vector pos[],
  ...
  if (m_ikLock[i].kneeDir.LengthSqr() > 0) {
    Studio_SolveIK(pchain->pLink(0)->bone, pchain->pLink(1)->bone,
                   pchain->pLink(2)->bone, p3, m_ikLock[i].kneePos,
...
//-----------------------------------------------------------------------------
// Purpose: run all animations that automatically play and are driven off of
// poseParameters
//-----------------------------------------------------------------------------
void CalcAutoplaySequences(const CStudioHdr *pStudioHdr, CIKContext *pIKContext,
                           Vector pos[], Quaternion q[],
                           const float poseParameters[], int boneMask,
                           float realTime) {
  //	ASSERT_NO_REENTRY();
  ...
  for (i = 0; i < count; i++) {
    ...
    if (seqdesc.flags & STUDIO_AUTOPLAY) {
      float cycle = 0;
      float cps =
          Studio_CPS(pStudioHdr, seqdesc, sequenceIndex, poseParameters);
      cycle = realTime * cps;
      cycle = cycle - (int)cycle;
  ...
  for (j = chainlength - 1; j >= 0; j--) {
    ...
    if (pStudioHdr->boneFlags(i) & boneMask) {
      ...
      if (pStudioHdr->boneParent(i) == -1) {
        ...
        ConcatTransforms(bonetoworld[pStudioHdr->boneParent(i)], bonematrix,
                         bonetoworld[i]);
      }
    }
  }
...
//			and generate a procedural transformation based on how
// that  column 			points down the 6 cardinal axis (all
// negative
// weights are clamped to 0).
//-----------------------------------------------------------------------------

void DoAxisInterpBone(mstudiobone_t *pbones, int ibone,
                      CBoneAccessor &bonetoworld) {
  matrix3x4_t bonematrix;
  Vector control;

  mstudioaxisinterpbone_t *pProc =
      (mstudioaxisinterpbone_t *)pbones[ibone].pProcedure();
  ...
  if (pProc && pbones[pProc->control].parent != -1) {
    ...
    VectorIRotate(tmp, bonetoworld.GetBone(pbones[pProc->control].parent),
                  control);
#if 0
  ...
  if (a1 + a2 > 0) {
    ...
    // TODO(d.rattman): do a proper 3-way Quat blend!
  ...
  ConcatTransforms(bonetoworld.GetBone(pbones[ibone].parent), bonematrix,
                   bonetoworld.GetBoneForWrite(ibone));
}

//-----------------------------------------------------------------------------
// Purpose: Generate a procedural transformation based on how that another bones
//			local transformation matches a set of target
// orientations.
//-----------------------------------------------------------------------------
void DoQuatInterpBone(mstudiobone_t *pbones, int ibone,
                      CBoneAccessor &bonetoworld) {
  matrix3x4_t bonematrix;
  Vector control;

  mstudioquatinterpbone_t *pProc =
      (mstudioquatinterpbone_t *)pbones[ibone].pProcedure();
  if (pProc && pbones[pProc->control].parent != -1) {
    Quaternion src;
    float weight[32];
    float scale = 0.0;
    ...
    for (i = 0; i < pProc->numtriggers; i++) {
      ...
      // TODO(d.rattman): a fast acos should be acceptable
      dot = std::clamp(dot, -1.0f, 1.0f);
      weight[i] = 1 - (2 * acos(dot) * pProc->pTrigger(i)->inv_tolerance);
      weight[i] = std::max(0.0f, weight[i]);
      scale += weight[i];
    }

    if (scale <= 0.001)  // EPSILON?
    {
      AngleMatrix(pProc->pTrigger(0)->quat, pProc->pTrigger(0)->pos,
                  bonematrix);
      ConcatTransforms(bonetoworld.GetBone(pbones[ibone].parent), bonematrix,
                       bonetoworld.GetBoneForWrite(ibone));
      return;
...
static ConVar aim_constraint( "aim_constraint", "1", FCVAR_REPLICATED, "Toggle
<aimconstraint> Helper Bones" );
...
//-----------------------------------------------------------------------------
void DoAimAtBone(mstudiobone_t *pBones, int iBone, CBoneAccessor &bonetoworld,
  ...
  {
          // If the aim constraint is turned off then just copy the parent
  transform
  ...
  ); MatrixSetColumn( boneWorldPosition, 3, boneToWorldSpace ); MatrixCopy(
  boneToWorldSpace, bonetoworld.GetBoneForWrite( iBone ) );
  ...
  */
  ...
  if (pStudioHdr) {
    ...
    ConcatTransforms(bonetoworld.GetBone(attachment.localbone),
                     attachment.local, aimAtSpace);
  } else {
  ...
  ConcatTransforms(bonetoworld.GetBone(pProc->parent), bonematrix,
                   boneLocalToWorld);
...
static mstudiobonecontroller_t *FindController(const CStudioHdr *pStudioHdr,
                                               int iController) {
...
float Studio_SetController(const CStudioHdr *pStudioHdr, int iController,
  ...
  if (!pStudioHdr) return flValue;

  mstudiobonecontroller_t *pbonecontroller =
      FindController(pStudioHdr, iController);
  if (!pbonecontroller) {
    ctlValue = 0;
    return flValue;
  }
  ...
  if (pbonecontroller->type & (STUDIO_XR | STUDIO_YR | STUDIO_ZR)) {
    ...
    if (pbonecontroller->start + 359.0 >= pbonecontroller->end) {
      if (flValue >
          ((pbonecontroller->start + pbonecontroller->end) / 2.0) + 180)
        flValue = flValue - 360;
      if (flValue <
          ((pbonecontroller->start + pbonecontroller->end) / 2.0) - 180)
        flValue = flValue + 360;
  ...
  ctlValue = (flValue - pbonecontroller->start) /
             (pbonecontroller->end - pbonecontroller->start);
  if (ctlValue < 0) ctlValue = 0;
  if (ctlValue > 1) ctlValue = 1;

  float flReturnVal = ((1.0 - ctlValue) * pbonecontroller->start +
                       ctlValue * pbonecontroller->end);
...
float Studio_GetController(const CStudioHdr *pStudioHdr, int iController,
                           float ctlValue) {
  if (!pStudioHdr) return 0.0;

  mstudiobonecontroller_t *pbonecontroller =
      FindController(pStudioHdr, iController);
  if (!pbonecontroller) return 0;

  return ctlValue * (pbonecontroller->end - pbonecontroller->start) +
         pbonecontroller->start;
}
...
float Studio_SetPoseParameter(const CStudioHdr *pStudioHdr, int iParameter,
  ...
  const mstudioposeparamdesc_t &PoseParam =
      pStudioHdr->pPoseParameter(iParameter);
  ...
  if (PoseParam.loop) {
    ...
    flValue =
        flValue - PoseParam.loop * floor((flValue + shift) / PoseParam.loop);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
static int ClipRayToHitbox(const Ray_t &ray, mstudiobbox_t *pbox,
                           matrix3x4_t &matrix, trace_t &tr) {
  // scale by current t so hits shorten the ray and increase the likelihood of
  // early outs
  Vector delta2;
  ...
  // OPTIMIZE: This is optimized for world space.  If the transform is fast
  // enough, it may make more sense to just xform and call UTIL_ClipToBox()
  ...
  for (int j = 0; j < 3; j++) {
    extent[j] = delta2.x * matrix[0][j] + delta2.y * matrix[1][j] +
                delta2.z * matrix[2][j];
    uextent[j] = fabsf(extent[j]);
    float coord = segmentCenter.x * matrix[0][j] +
                  segmentCenter.y * matrix[1][j] +
                  segmentCenter.z * matrix[2][j];
    coord = fabsf(coord);
  ...
  cextent =
      cross.x * matrix[0][0] + cross.y * matrix[1][0] + cross.z * matrix[2][0];
  cextent = fabsf(cextent);
  tmp = boxextents[1] * uextent[2] + boxextents[2] * uextent[1];
  if (cextent > tmp) return -1;

  cextent =
      cross.x * matrix[0][1] + cross.y * matrix[1][1] + cross.z * matrix[2][1];
  cextent = fabsf(cextent);
  tmp = boxextents[0] * uextent[2] + boxextents[2] * uextent[0];
  if (cextent > tmp) return -1;

  cextent =
      cross.x * matrix[0][2] + cross.y * matrix[1][2] + cross.z * matrix[2][2];
  cextent = fabsf(cextent);
  ...
  // delta was prescaled by the current t, so no need to see if this
  // intersection is closer
  trace_t boxTrace;
  if (!IntersectRayWithBox(start, extent, pbox->bbmin, pbox->bbmax, 0.0f,
                           &boxTrace))
    return -1;
...
// Purpose:
//-----------------------------------------------------------------------------
bool SweepBoxToStudio(IPhysicsSurfaceProps *pProps, const Ray_t &ray,
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    if ((fBoneContents & fContentsMask) == 0) continue;

    trace_t obbTrace;
    if (IntersectRayWithOBB(clippedRay, *hitboxbones[pbox->bone], pbox->bbmin,
                            pbox->bbmax, 0.0f, &obbTrace)) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool TraceToStudio(IPhysicsSurfaceProps *pProps, const Ray_t &ray,
                   CStudioHdr *pStudioHdr, mstudiohitboxset_t *set,
                   matrix3x4_t **hitboxbones, int fContentsMask, trace_t &tr) {
  if (!ray.m_IsRay) {
    return SweepBoxToStudio(pProps, ray, pStudioHdr, set, hitboxbones,
                            fContentsMask, tr);
  }
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    // columns are axes of the bones in world space, translation is in world
    // space
  ...
  if (hitbox >= 0) {
    ...
    if (hitside >= 3) {
      ...
      // tr.plane.dist = DotProduct( tr.plane.normal, Vector(matrix[0][3],
      // matrix[1][3], matrix[2][3] ) ) + pbox->bbmax[hitside];
    } else {
      tr.plane.normal[0] = -matrix[0][hitside];
      tr.plane.normal[1] = -matrix[1][hitside];
      tr.plane.normal[2] = -matrix[2][hitside];
      // tr.plane.dist = DotProduct( tr.plane.normal, Vector(matrix[0][3],
      // matrix[1][3], matrix[2][3] ) ) - pbox->bbmin[hitside];
    }
...
//-----------------------------------------------------------------------------
// Purpose: returns array of animations and weightings for a sequence based on
...
void Studio_SeqAnims(const CStudioHdr *pStudioHdr, mstudioseqdesc_t &seqdesc,
  ...
  Studio_LocalPoseParameter(pStudioHdr, poseParameter, seqdesc, iSequence, 0,
                            s0, i0);
  Studio_LocalPoseParameter(pStudioHdr, poseParameter, seqdesc, iSequence, 1,
                            s1, i1);

  panim[0] = &pStudioHdr->pAnimdesc(
      pStudioHdr->iRelativeAnim(iSequence, seqdesc.anim(i0, i1)));
  weight[0] = (1 - s0) * (1 - s1);

  panim[1] = &pStudioHdr->pAnimdesc(
      pStudioHdr->iRelativeAnim(iSequence, seqdesc.anim(i0 + 1, i1)));
  weight[1] = (s0) * (1 - s1);

  panim[2] = &pStudioHdr->pAnimdesc(
      pStudioHdr->iRelativeAnim(iSequence, seqdesc.anim(i0, i1 + 1)));
  weight[2] = (1 - s0) * (s1);

  panim[3] = &pStudioHdr->pAnimdesc(
      pStudioHdr->iRelativeAnim(iSequence, seqdesc.anim(i0 + 1, i1 + 1)));
  weight[3] = (s0) * (s1);

  Assert(weight[0] >= 0.0f && weight[1] >= 0.0f && weight[2] >= 0.0f &&
         weight[3] >= 0.0f);
}
...
//-----------------------------------------------------------------------------
// Purpose: calculate changes in position and angle relative to the start of an
// animations cycle Output:	updated position and angle, relative to the
// origin
//			returns false if animation is not a movement animation
//-----------------------------------------------------------------------------

bool Studio_AnimPosition(mstudioanimdesc_t *panim, float flCycle,
                         Vector &vecPos, QAngle &vecAngle) {
  float prevframe = 0;
  vecPos.Init();
  vecAngle.Init();
...
bool Studio_AnimVelocity(mstudioanimdesc_t *panim, float flCycle,
                         Vector &vecVelocity) {
  float prevframe = 0;
...
//-----------------------------------------------------------------------------
// Purpose: calculate changes in position and angle between two points in an
// animation cycle
// Output:	updated position and angle, relative to CycleFrom being at the
// origin
//			returns false if animation is not a movement animation
//-----------------------------------------------------------------------------

bool Studio_AnimMovement(mstudioanimdesc_t *panim, float flCycleFrom,
                         float flCycleTo, Vector &deltaPos,
                         QAngle &deltaAngle) {
  if (panim->nummovements == 0) return false;
...
float Studio_FindAnimDistance(mstudioanimdesc_t *panim, float flDist) {
  ...
  for (int i = 0; i < panim->nummovements; i++) {
    ...
    if (flMove >= flDist) {
      ...
      if (SolveQuadratic(0.5 * (pmove->v1 - pmove->v0), pmove->v0, -flDist,
                         root1, root2)) {
...
//-----------------------------------------------------------------------------
// Purpose: calculate changes in position and angle between two points in a
// sequences cycle
// Output:	updated position and angle, relative to CycleFrom being at the
// origin
...
bool Studio_SeqMovement(const CStudioHdr *pStudioHdr, int iSequence,
                        ...
                        const float poseParameter[], Vector &deltaPos,
                        QAngle &deltaAngles) {
  mstudioanimdesc_t *panim[4];
  float weight[4];
  ...
  for (int i = 0; i < 4; i++) {
    if (weight[i]) {
      ...
      if (Studio_AnimMovement(panim[i], flCycleFrom, flCycleTo, localPos,
                              localAngles)) {
        found = true;
        deltaPos = deltaPos + localPos * weight[i];
        // TODO(d.rattman): this makes no sense
        deltaAngles = deltaAngles + localAngles * weight[i];
      } else if (!(panim[i]->flags & STUDIO_DELTA) &&
                 panim[i]->nummovements == 0 && seqdesc.weight(0) > 0.0) {
        found = true;
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: calculate instantaneous velocity in ips at a given point in the
// sequence's cycle Output:	velocity vector, relative to identity
// orientation
//			returns false if sequence is not a movement sequence
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: lookup attachments by substring. Randomly return one of the matching
// attachments.
//-----------------------------------------------------------------------------
...
int Studio_BoneIndexByName(const CStudioHdr *pStudioHdr, const char *pName) {
  ...
  const uint8_t *pBoneTable = pStudioHdr->GetBoneTableSortedByName();
  mstudiobone_t *pbones = pStudioHdr->pBone(0);
  while (start <= end) {
    int mid = (start + end) >> 1;
...
const char *Studio_GetKeyValueText(const CStudioHdr *pStudioHdr,
                                   int iSequence) {
  if (pStudioHdr && pStudioHdr->SequencesAvailable()) {
