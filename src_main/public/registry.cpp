...
int CRegistry::ReadInt(const char *key, int defaultValue /*= 0*/) {
  ...
  lResult = VCRHook_RegQueryValueEx(m_hKey,          // handle to key
                                    key,             // value name
                                    0,               // reserved
                                    &dwType,         // type buffer
...
void CRegistry::WriteInt(const char *key, int value) {
  ...
  VCRHook_RegSetValueEx(m_hKey,          // handle to key
                        key,             // value name
                        0,               // reserved
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CRegistry::ReadString(const char *key,
                                  const char *defaultValue /* = NULL */) {
  LONG lResult;
  // Type of key
  DWORD dwType;
...
// Read/write helper methods
//-----------------------------------------------------------------------------
int CRegistry::ReadInt(const char *pKeyBase, const char *pKey,
                       int defaultValue) {
...
const char *CRegistry::ReadString(const char *pKeyBase, const char *pKey,
                                  const char *defaultValue) {
  int nLen = strlen(pKeyBase);
  int nKeyLen = strlen(pKey);
  char *pFullKey = (char *)_alloca(nLen + nKeyLen + 2);
...
void CRegistry::WriteString(const char *pKeyBase, const char *pKey,
                            const char *value) {
  int nLen = strlen(pKeyBase);
  int nKeyLen = strlen(pKey);
  char *pFullKey = (char *)_alloca(nLen + nKeyLen + 2);
