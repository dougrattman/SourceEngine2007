...
struct CacheOptimizedKDNode {
  ...
  int32_t Children;           // child idx, or'ed with flags above
  ...
  inline int32_t TriangleIndexStart(void) const {
  ...
  inline int NumberOfTrianglesInLeaf(void) const {
    assert(NodeType() == KDNODE_STATE_LEAF);
    return *((int32_t *)&SplittingPlaneValue);
  }

  inline void SetNumberOfTrianglesInLeafNode(int n) {
    *((int32_t *)&SplittingPlaneValue) = n;
  }
...
struct RayTracingSingleResult {
  ...
  int32_t HitID;          // -1=no hit. otherwise, triangle index
...
class RayTracingEnvironment {
 public:
  uint32_t Flags;  // RTE_FLAGS_xxx above
  Vector m_MinBound;
  Vector m_MaxBound;
  ...
  CUtlVector<int32_t> TriangleIndexList;  //< the list of triangle indices.
  CUtlVector<LightDesc_t> LightList;      //< the list of lights
  CUtlVector<Vector> TriangleColors;      //< color of tries
  CUtlVector<int32_t> TriangleMaterials;  //< material index of tries
 ...
 public:
  ...
  void AddTriangle(int32_t id, const Vector &v1, const Vector &v2,
                   const Vector &v3, const Vector &color);

  // Adds a triangle with flags & material
  void AddTriangle(int32_t id, const Vector &v1, const Vector &v2,
                   const Vector &v3, const Vector &color, uint16_t flags,
                   int32_t materialIndex);

  void AddQuad(int32_t id, const Vector &v1, const Vector &v2, const Vector &v3,
  ...
  void Trace4Rays(const FourRays &rays, fltx4 TMin, fltx4 TMax,
                  RayTracingResult *rslt_out, int32_t skip_id = -1,
                  ITransparentTriangleCallback *pCallback = NULL);
  ...
  float CalculateCostsOfSplit(int split_plane, int32_t const *tri_list,
                              int ntris, Vector MinBound, Vector MaxBound,
                              float &split_value, int &nleft, int &nright,
                              int &nboth);

  void RefineNode(int node_number, int32_t const *tri_list, int ntris,
                  Vector MinBound, Vector MaxBound, int depth);

  void CalculateTriangleListBounds(int32_t const *tris, int ntris,
  ...
  const CacheOptimizedTriangle &GetTriangle(int32_t triID) {
    return OptimizedTriangleList[triID];
  }

  int32_t GetTriangleMaterial(int32_t triID) {
