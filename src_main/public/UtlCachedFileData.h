...
class CUtlCachedFileData {
 public:
  ...
  // If you create a cache and don't call init/shutdown, you can call this to do
  // a quick check to see if the checksum/version
  ...
  // Iterates all entries and gets filesystem info and optionally causes rebuild
  // on any existing items which are out of date
...
bool CUtlCachedFileData<T>::IsUpToDate() {
  ...
  if (!m_pszRepositoryFileName || !m_pszRepositoryFileName[0]) {
    Error(
        "CUtlCachedFileData:  Can't IsUpToDate, no repository file specified.");
    return false;
  }
  ...
  // This is a checksum based on any meta data files which the cache depends on
  // (supplied by a passed in
  //  meta data function
...
void CUtlCachedFileData<T>::InitLargeBuffer(FileHandle_t &fh,
  ...
  if (UTL_CACHE_SYSTEM_VERSION == cacheversion) {
    ...
    if (version == m_nVersion) {
      ...
      g_pFullFileSystem->Read(&cache_meta_checksum, sizeof(cache_meta_checksum),
                              fh);
...
bool CUtlCachedFileData<T>::Init() {
  ...
  long fileTime =
      g_pFullFileSystem->GetFileTime(m_pszRepositoryFileName, "MOD");
