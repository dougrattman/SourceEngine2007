...
wchar_t *ReadToEndOfLine(wchar_t *start) {
  ...
  while (*start) {
    if (*start == 0x0D || *start == 0x0A) break;
    start++;
  }

  while (*start == 0x0D || *start == 0x0A) start++;
...
//-----------------------------------------------------------------------------
void WriteUnicodeString(CUtlBuffer &buf, const wchar_t *string,
                        bool addQuotes) {
  if (addQuotes) {
...
// Purpose: file writing
//-----------------------------------------------------------------------------
void WriteAsciiStringAsUnicode(CUtlBuffer &buf, const char *string,
                               bool addQuotes) {
  if (addQuotes) {
    buf.PutUnsignedShort('\"');
  }
