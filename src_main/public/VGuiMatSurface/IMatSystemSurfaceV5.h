...
class IMatSystemSurface : public SurfaceV30::ISurface {
 public:
  ...
  // it can set bLetAppDriveInput to true and call HandleWindowMessage for the
  // Windows messages.
  virtual void AttachToWindow(void *hwnd, bool bLetAppDriveInput = false) = 0;

  // If you specified true for bLetAppDriveInput, then call this for each window
  // message that comes in.
  virtual void HandleWindowMessage(void *hwnd, unsigned int uMsg,
