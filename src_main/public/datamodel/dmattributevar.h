...
// a  member of a element class
//		CDmrArray<T> var;	<- Used to reference an existing array
// attribute + read/modify it
//		CDmrArrayConst<T> var;	<- Used to reference an existing array
// attribute + read it (no modify)
//
...
// a member of a element class
//		CDmrStringArray var;	<- Used to reference an existing array
// attribute + read/modify it 		CDmrStringArrayConst var; <- Used to
...
// attribute var as a member of a element class
//		CDmrElementArray< element_type >  var;	<- Used to reference
// an existing array attribute + read/modify it
// CDmrElementArrayConst<  element_type > var; <- Used to reference an existing
// array attribute + read it (no modify)
//-----------------------------------------------------------------------------
...
#define DECLARE_ATTRIBUTE_ARRAY_REFERENCE(_className, _elementType)          \
 public:                                                                     \
  ...
  _className(CDmaArray<_className> &var) { this->Init(var.GetAttribute()); } \
  _className(CDmrArray<_className> &var) { this->Init(var.GetAttribute()); }
