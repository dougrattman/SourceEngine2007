...
the_interface IDmElementFramework : public IAppSystem {
 public:
  ...
  virtual void BeginEdit() = 0;  // ends in edit phase, forces apply/resolve if
                                 // from edit phase
...
class IDmSerializer {
 public:
  ...
  // Read from the UtlBuffer, return true if successful, and return the read-in
  // root in ppRoot.
...
class IDmLegacyUpdater {
 public:
  ...
  // Updates ppRoot to first non-legacy generic dmx format, returns false if the
  // conversion fails
...
enum DmNotifyFlags_t {
  ...
  NOTIFY_CHANGE_OTHER =
      (1 << (NOTIFY_SOURCE_BITS +
             7)),  // Non attribute related change (a change in UI, for example)
...
class IDataModel : public IAppSystem {
 public:
  ...
  virtual bool Serialize(CUtlBuffer &outBuf, const char *pEncodingName,
                         const char *pFormatName, DmElementHandle_t hRoot) = 0;
  ...
  virtual void AbortUndoableOperation() = 0;  // called instead of FinishUndo,
                                              // essentially performs and Undo()
                                              // and WipeRedo() if any undo
...
class CDisableUndoScopeGuard : public CChangeUndoScopeGuard {
 ...
 public:
  ...
  CDisableUndoScopeGuard(const char *pDesc, int nNotifySource, int nNotifyFlags,
...
class CEnableUndoScopeGuard : public CChangeUndoScopeGuard {
 ...
 public:
  ...
  CEnableUndoScopeGuard(const char *pDesc, int nNotifySource, int nNotifyFlags,
                        IDmNotify *pNotify = NULL)
   ...
   public:                                                                    \
    C##_classnameprefix##UndoScopeGuard(int nNotifyFlags,                     \
        ...
        : BaseClass(_source, nNotifyFlags, pUndoDesc, pRedoDesc,              \
                    nChainingID) {}                                           \
    C##_classnameprefix##UndoScopeGuard(int nNotifyFlags, IDmNotify *pNotify, \
                                        const char *pUndoDesc,                \
                                        const char *pRedoDesc = NULL,         \
                                        int nChainingID = 0)                  \
