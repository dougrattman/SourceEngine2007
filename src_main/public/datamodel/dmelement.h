...
struct DmElementReference_t {
  ...
  unsigned short
      m_nStrongHandleCount;  // CDmeCountedElementRef - for preventing elements
                             // from being truly deleted, mainly used by undo
                             // and file root
...
class CDmElement {
 ...
 protected:
  // NOTE: These are protected to ensure that the factory is the only thing that
  // can create these
  CDmElement(DmElementHandle_t handle, const char *objectType,
             const DmObjectId_t &id, const char *objectName, DmFileId_t fileid);
  virtual ~CDmElement();
 ...
 private:
  ...
  void CopyAttributesTo(CDmElement *pCopy, CRefMap &refmap,
                        TraversalDepth_t depth) const;
  void CopyElementAttribute(const CDmAttribute *pAttr, CDmAttribute *pCopyAttr,
                            CRefMap &refmap, TraversalDepth_t depth) const;
  ...
  void SetFileId_R(CUtlHashFast<DmElementHandle_t> &visited, DmFileId_t fileid,
                   TraversalDepth_t depth, DmFileId_t match, bool bOnlyIfMatch);
...
inline CDmAttribute *CDmElement::SetValue(const char *pAttributeName,
  ...
  if (!pAttribute) {
    pAttribute =
        CreateAttribute(pAttributeName, CDmAttributeInfo<T>::AttributeType());
  }
  if (pAttribute) {
...
void FindAncestorsReferencingElement(CDmElement *target,
  ...
  for (DmAttributeReferenceIterator_t it =
           g_pDataModel->FirstAttributeReferencingElement(target->GetHandle());
...
#define DEFINE_ELEMENT(className, baseClassName)                             \
 ...
 protected:                                                                  \
  ...
  static int GetInheritanceDepth_Implementation(CUtlSymbol typeSymbol,       \
    ...
    return BaseClass::GetInheritanceDepth_Implementation(typeSymbol,         \
                                                         nCurrentDepth + 1); \
  }                                                                          \
  virtual int AllocatedSize() const { return DmeEstimateMemorySize(this); }  \
                                                                             \
