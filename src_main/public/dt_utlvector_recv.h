...
#define RECVINFO_UTLVECTOR(varName)                                   \
...
// )
#define RECVINFO_UTLVECTOR_SIZEFN(varName, resizeFn)           \
#varName, offsetof(currentRecvDTClass, varName),             \
      sizeof(((currentRecvDTClass *)0)->varName[0]), resizeFn, \
      GetEnsureCapacityTemplate(((currentRecvDTClass *)0)->varName)

#define RecvPropUtlVectorDataTable(varName, nMaxElements, dataTableName) \
  RecvPropUtlVector(                                                     \
      RECVINFO_UTLVECTOR(varName), nMaxElements,                         \
      RecvPropDataTable(NULL, 0, 0, &REFERENCE_RECV_TABLE(dataTableName)))
...
//
//	RecvPropUtlVector( RECVINFO_UTLVECTOR( m_FloatArray ), 16,
//RecvPropFloat(NULL,0,0) )
//
RecvProp RecvPropUtlVector(
    const char *pVarName,  // Use RECVINFO_UTLVECTOR to generate these first 5
                           // parameters.
    int offset, int sizeofVar, ResizeUtlVectorFn fn, EnsureCapacityFn ensureFn,

    int nMaxElements,  // Max # of elements in the array. Keep this as low as
                       // possible.
    RecvProp
        pArrayProp  // The definition of the property you're receiving into.
