...
abstract_class IVEngineServer {
 public:
  ...
  // memory before level startup, otherwise
  //  it'll only load when actually used (which can cause a disk i/o hitch if it
  //  occurs during play of a level).
  ...
  // Check's if the name is precached, but doesn't actually precache the name if
  // not...
  ...
  // Get the PVS bits for a specified cluster and copy the bits into outputpvs.
  // Returns the number of bytes needed to pack the PVS
  ...
  // Check whether the specified worldspace bounding box is inside the specified
  // PVS
  ...
  // with that index. If that edict index is already used, it'll return 0.
  ...
  // Save/restore uses a special memory allocator (which zeroes newly allocated
  // memory, etc.)
  ...
  //  change place in memory (use MAKE_STRING) for anything that's not compiled
  //  into your mod.
  virtual void LightStyle(int style, const char *val) = 0;

  // Project a static decal onto the specified entity / model (for level placed
  // decals in the .bsp)
  ...
  // Locks/unlocks the network string tables (.e.g, when adding bots to server,
  // this needs to happen). Be sure to reset the lock after executing your
  // code!!!
  ...
  // Merge the pvs bits into the current accumulated pvs based on the specified
  // origin ( not that each pvs origin has an 8 world unit fudge factor )
  ...
  // PVS
  virtual int CheckHeadnodeVisible(int nodenum, const uint8_t *pvs,
                                   int vissize) = 0;
  ...
  // Marks the material (vmt file) for consistency checking.  If the client and
  // server have different contents for the file, the client's vmt can only use
  // the VertexLitGeneric shader, and can only contain $baseTexture and $bumpmap
  // vars.
  ...
  virtual const CCheckTransmitInfo *GetPrevCheckTransmitInfo(edict_t *
                                                             pPlayerEdict) = 0;
  ...
  // Returns true if the engine is an internal build. i.e. is using the internal
  // bugreporter.
  ...
  // OnQueryCvarValueFinished call. Returns InvalidQueryCvarCookie if the entity
  // is invalid.
...
abstract_class IServerGameDLL {
 public:
  ...
  // Give the list of datatable classes to the engine.  The engine matches class
  // names from here with
...
abstract_class IServerGameEnts {
 public:
  ...
  //
  // This is also where an entity can force other entities to be transmitted if
  // it refers to them with ehandles.
...
abstract_class IServerGameClients {
 public:
  ...
  // A block of CUserCmds has arrived from the user, decode them and buffer for
  // execution during player simulation
