...
the_interface IDmeMakefileUtils : public IAppSystem {
 public:
  // Methods related to compilation
  virtual void PerformCompile(CDmElement * pElement,
                              bool bBuildAllDependencies) = 0;
  ...
  virtual CompilationState_t UpdateCompilation(char* pOutputBuf,
                                               int nBufLen) = 0;
  ...
  // Returns the exit code of the failed compilation (if COMPILATION_FAILED
  // occurred)
