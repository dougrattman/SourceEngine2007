...
class CDmeGameModel : public CDmeDag {
 ...
 public:
  ...
  void AddBones(studiohdr_t *pStudioHdr, const char *pBaseName, int nFirstBone,
                int nCount);
  void SetBone(u32 index, const Vector &pos, const Quaternion &rot);
  u32 NumBones() const;
  CDmeTransform *GetBone(u32 index) const;
  int FindBone(CDmeTransform *pTransform) const;
  void RemoveAllBones();

  // A src bone transform transforms pre-compiled data (.dmx or .smd files, for
  // example) into post-compiled data (.mdl or .ani files) Returns false if
  // there is no transform (or if the transforms are identity)
  bool GetSrcBoneTransforms(matrix3x4_t *pPreTransform,
                            matrix3x4_t *pPostTransform, int nBoneIndex) const;

  bool IsRootTransform(int nBoneIndex) const;

  u32 NumFlexWeights() const;
  const CUtlVector<float> &GetFlexWeights() const;
  // We drive these through the operators instead
  // void SetFlexWeights( u32 nFlexWeights, const float* flexWeights );
  void SetNumFlexWeights(u32 nFlexWeights);
  void SetFlexWeights(u32 nFlexWeights, const float *flexWeights);
  ...
  CDmeGlobalFlexControllerOperator *AddGlobalFlexController(
      char const *controllerName, int globalIndex);
  ...
  CDmeGlobalFlexControllerOperator *GetGlobalFlexController(
      int localIndex);  // localIndex is in order of calls to
 ...
 protected:
  ...
  CDmaArray<float> m_flexWeights;  // These are global flex weights (so there
                                   // can be gaps, unused indices)
  ...
  // this is different than m_Children - this is ALL transforms in the tree that
  // are used by the model m_Children holds the roots of that tree
...
class CDmeGameSprite : public CDmeDag {
 ...
 public:
  ...
  void SetState(bool bVisible, float nFrame, int nRenderMode, int nRenderFX,
                float flRenderScale, float flProxyRadius, const Vector &pos,
