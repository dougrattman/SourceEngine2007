...
class CDmeCombinationInputControl : public CDmElement {
 ...
 private:
  ...
  // TODO(d.rattman): Remove soon! Used to autogenerate wrinkle deltas
...
class CDmeCombinationOperator : public CDmeOperator {
 ...
 public:
  ...
  // Adds a control, returns the control index. Also adds a raw control with the
  // same name to this control
  ControlIndex_t FindOrCreateControl(const char *pControlName, bool bStereo,
                                     bool bAutoAddRawControl = false);
  ...
  float GetRawControlWrinkleScale(ControlIndex_t nControl,
                                  const char *pRawControlName) const;
  ...
  void SetWrinkleScale(ControlIndex_t nControlIndex,
                       const char *pRawControlName, float flWrinkleScale);

  // Sets the value of a control
  void SetControlValue(ControlIndex_t nControlIndex, float flValue,
                       CombinationControlType_t type = COMBO_CONTROL_NORMAL);

  // Sets the value of a stereo control
  void SetControlValue(ControlIndex_t nControlIndex, float flLevel,
                       float flBalance,
                       CombinationControlType_t type = COMBO_CONTROL_NORMAL);
  void SetControlValue(ControlIndex_t nControlIndex, const Vector2D &vec,
                       CombinationControlType_t type = COMBO_CONTROL_NORMAL);
  ...
  void SetMultiControlLevel(
      ControlIndex_t nControlIndex, float flLevel,
      CombinationControlType_t type = COMBO_CONTROL_NORMAL);
  float GetMultiControlLevel(
      ControlIndex_t nControlIndex,
      CombinationControlType_t type = COMBO_CONTROL_NORMAL) const;

  // Reordering controls
  void MoveControlUp(const char *pControlName);
  void MoveControlDown(const char *pControlName);
  void MoveControlBefore(const char *pDragControlName,
                         const char *pDropControlName);
  void MoveControlAfter(const char *pDragControlName,
                        const char *pDropControlName);

  void MoveRawControlUp(ControlIndex_t nControlIndex,
                        const char *pRawControlName);
  void MoveRawControlDown(ControlIndex_t nControlIndex,
                          const char *pRawControlName);

  // Returns true if a control is a multi control (a control w/ 3 or more
  // remappings)
  ...
  float GetControlValue(
      ControlIndex_t nControlIndex,
      CombinationControlType_t type = COMBO_CONTROL_NORMAL) const;
  const Vector2D &GetStereoControlValue(
      ControlIndex_t nControlIndex,
      CombinationControlType_t type = COMBO_CONTROL_NORMAL) const;
  ...
  void AttachChannelToControlValue(ControlIndex_t nControlIndex,
                                   CombinationControlType_t type,
                                   CDmeChannel *pChannel);

  // Adds a domination rule. Domination rules are specified using raw control
  // names
  CDmeCombinationDominationRule *AddDominationRule();
  CDmeCombinationDominationRule *AddDominationRule(
      int nDominatorCount, const char **ppDominatorOutputControlNames,
      int nSuppressedCount, const char **ppSuppressedOutputControlNames);
  CDmeCombinationDominationRule *AddDominationRule(
      const CUtlVector<const char *> dominators,
      const CUtlVector<const char *> suppressed);
  CDmeCombinationDominationRule *AddDominationRule(
      CDmeCombinationDominationRule *pSrcRule);
  ...
  const CUtlVector<int> &GetOperationControls(int nTargetIndex,
                                              int nOpIndex) const;
  int GetOperationDominatorCount(int nTargetIndex, int nOpIndex) const;
  const CUtlVector<int> &GetOperationDominator(int nTargetIndex, int nOpIndex,
                                               int nDominatorIndex) const;
  ...
  // Would a particular delta state attached to this combination operator end up
  // stero?
  bool IsDeltaStateStereo(const char *pDeltaStateName);

  void CopyControls(CDmeCombinationOperator *pSrc);

  // TODO(d.rattman): Remove soon!
  ...
  // The base values are different from the default values see
  // CDmeCombinationInputControl
  void SetToBase();

  // Remove all controls and domination rules which are not referring to
  // anything
  void Purge();
 ...
 private:
  ...
  int FindDeltaStateIndex(CDmAttribute *pDeltaArray,
  ...
  void ComputeInternalControlValue(RawControlIndex_t nRawControlIndex,
                                   CombinationControlType_t type,
                                   Vector2D &value);
  ...
  RawControlIndex_t FindRawControlIndex(
      const char *pControlName, bool bIgnoreDefaultControls = false) const;
...
//-----------------------------------------------------------------------------
//
// A class used to edit combination operators in Maya.. doesn't connect to
// targets
//
//-----------------------------------------------------------------------------
class CDmeMayaCombinationOperator : public CDmeCombinationOperator {
 ...
 public:
  ...
  const Vector2D &GetDeltaStateWeight(int nIndex,
                                      CombinationControlType_t type) const;
