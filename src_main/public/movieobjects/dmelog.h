 ...
 public:
  DmeLog_TimeSelection_t()
      : m_flIntensity(1.0f),
        ...
        m_nResampleInterval(
            DmeTime_t(.05f)),  // 50 msec sampling interval by default
    ...
    m_nFalloffInterpolatorTypes[0] = m_nFalloffInterpolatorTypes[1] =
  ...
  inline void StartTimeAdvancing() { m_bTimeAdvancing = true; }

  inline bool IsTimeAdvancing() const { return m_bTimeAdvancing; }

  inline RecordingMode_t GetRecordingMode() const { return m_RecordingMode; }

  void SetRecordingMode(RecordingMode_t mode) { m_RecordingMode = mode; }

  float GetAmountForTime(DmeTime_t curtime) const;
  float AdjustFactorForInterpolatorType(float factor, int side) const;

  // NOTE: See DmeTimeSelectionTimes_t for return values, 0 means before, 1=
  // left fallof, 2=hold, 3=right falloff, 4=after
  ...
  float m_flIntensity;            // How much to drive values toward m_HeadValue
                                  // (generally 1.0f)
  float m_flThreshold;
  CDmAttribute *m_pPresetValue;

  bool m_bAttachedMode : 1;  // Is the current time "attached" to the head
                             // position
...
abstract_class CDmeLogLayer : public CDmElement {
 ...
 public:
  ...
  virtual void CopyPartialLayer(const CDmeLogLayer *src, DmeTime_t startTime,
                                DmeTime_t endTime, bool bRebaseTimestamps) = 0;
  virtual void ExplodeLayer(const CDmeLogLayer *src, DmeTime_t startTime,
                            DmeTime_t endTime, bool bRebaseTimestamps,
                            DmeTime_t tResampleInterval) = 0;
  virtual void InsertKeyFromLayer(DmeTime_t keyTime, const CDmeLogLayer *src,
                                  DmeTime_t srcKeyTime) = 0;
  ...
  virtual void SetKey(DmeTime_t time, const CDmAttribute *pAttr, u32 index = 0,
                      int curveType = CURVE_DEFAULT) = 0;
  virtual bool SetDuplicateKeyAtTime(DmeTime_t time) = 0;
  // This inserts a key using the current values to construct the proper value
  // for the time
  virtual int InsertKeyAtTime(DmeTime_t nTime,
                              int curveType = CURVE_DEFAULT) = 0;

  // Sets the interpolated value of the log at the specified time into the
  // attribute
  virtual void GetValue(DmeTime_t time, CDmAttribute * pAttr, u32 index = 0)
      const = 0;
...
abstract_class CDmeLog : public CDmElement {
 ...
 public:
  ...
  virtual void FinishTimeSelection(
      DmeTime_t tHeadPosition,
      DmeLog_TimeSelection_t &
          params) = 0;  // in attached, timeadvancing mode, we need to blend out
                        // of the final sample over the fadeout interval
  virtual void StampKeyAtHead(DmeTime_t tHeadPosition,
                              DmeTime_t tPreviousHeadPosition,
                              const DmeLog_TimeSelection_t &params,
                              const CDmAttribute *pAttr, u32 index = 0) = 0;
  virtual void FilterUsingTimeSelection(
      IUniformRandomStream * random, float flScale,
      const DmeLog_TimeSelection_t &params, int filterType, bool bResample,
      bool bApplyFalloff, const CDmeLogLayer *baseLayer,
      CDmeLogLayer *writeLayer) = 0;
  virtual void FilterUsingTimeSelection(
      IUniformRandomStream * random, const DmeLog_TimeSelection_t &params,
      int filterType, bool bResample, bool bApplyFalloff) = 0;
  virtual void StaggerUsingTimeSelection(
      const DmeLog_TimeSelection_t &params, DmeTime_t tStaggerAmount,
      const CDmeLogLayer *baseLayer, CDmeLogLayer *writeLayer) = 0;
  virtual void RevealUsingTimeSelection(const DmeLog_TimeSelection_t &params,
                                        CDmeLogLayer *savedLayer) = 0;
  virtual void BlendLayersUsingTimeSelection(
      const DmeLog_TimeSelection_t &params) = 0;
  virtual void BlendLayersUsingTimeSelection(
      const CDmeLogLayer *firstLayer, const CDmeLogLayer *secondLayer,
      CDmeLogLayer *outputLayer, const DmeLog_TimeSelection_t &params,
      bool bUseBaseLayerSamples, DmeTime_t tStartOffset) = 0;
  virtual void BlendTimesUsingTimeSelection(
      const CDmeLogLayer *firstLayer, const CDmeLogLayer *secondLayer,
      CDmeLogLayer *outputLayer, const DmeLog_TimeSelection_t &params,
      DmeTime_t tStartOffset) = 0;
  virtual void PasteAndRescaleSamples(const CDmeLogLayer *src,
                                      const DmeLog_TimeSelection_t &srcParams,
                                      const DmeLog_TimeSelection_t &destParams,
                                      bool bBlendAreaInFalloffRegion) = 0;
  virtual void PasteAndRescaleSamples(
      const CDmeLogLayer *pBaseLayer, const CDmeLogLayer *pDataLayer,
  ...
  virtual void BuildCorrespondingLayer(const CDmeLogLayer *pReferenceLayer,
                                       const CDmeLogLayer *pDataLayer,
                                       CDmeLogLayer *pOutputLayer) = 0;
  ...
  virtual void GetValue(DmeTime_t time, CDmAttribute * pAttr, u32 index = 0)
      const = 0;
  virtual void GetValueSkippingTopmostLayer(
      DmeTime_t time, CDmAttribute * pAttr, u32 index = 0) const = 0;
  ...
  enum {
    FLATTEN_NODISCONTINUITY_FIXUP =
        (1 << 0),  // Don't add "helper" samples to preserve discontinuities.
                   // This occurs when the time selection is "detached" from the
                   // head position
    FLATTEN_SPEW = (1 << 1),
  };
...
class CDmeTypedCurveInfo : public CDmeCurveInfo {
 ...
 public:
  // For "faceposer" style left/right edges, this controls whether interpolators
  // try to mimic faceposer left/right edge behavior
...
class CDmeTypedLogLayer : public CDmeLogLayer {
 ...
 public:
  ...
  // Finds a key within tolerance, or adds one. Unlike SetKey, this will *not*
  // delete keys after the specified time
  int FindOrAddKey(DmeTime_t nTime, DmeTime_t nTolerance, const T &value,
  ...
  virtual void GetValue(DmeTime_t time, CDmAttribute *pAttr,
                        u32 index = 0) const;
 ...
 protected:
  ...
  void GetBoundedSample(int keyindex, DmeTime_t &time, T &val,
                        int &curveType) const;

  void CurveSimplify_R(float thresholdSqr, int startPoint, int endPoint,
...
class CDmeTypedLog : public CDmeLog {
 ...
 public:
  ...
  void StampKeyAtHead(DmeTime_t tHeadPosition, DmeTime_t tPreviousHeadPosition,
                      const DmeLog_TimeSelection_t &params, const T &value);
  void StampKeyAtHead(DmeTime_t tHeadPosition, DmeTime_t tPreviousHeadPosition,
                      const DmeLog_TimeSelection_t &params,
                      const CDmAttribute *pAttr, u32 index = 0);
  void FinishTimeSelection(
      DmeTime_t tHeadPosition,
      DmeLog_TimeSelection_t
          &params);  // in attached, timeadvancing mode, we need to blend out of
                     // the final sample over the fadeout interval
  void FilterUsingTimeSelection(IUniformRandomStream *random, float flScale,
                                const DmeLog_TimeSelection_t &params,
                                int filterType, bool bResample,
                                bool bApplyFalloff,
                                const CDmeLogLayer *baseLayer,
                                CDmeLogLayer *writeLayer);
  void FilterUsingTimeSelection(IUniformRandomStream *random,
                                const DmeLog_TimeSelection_t &params,
                                int filterType, bool bResample,
                                bool bApplyFalloff);
  void StaggerUsingTimeSelection(const DmeLog_TimeSelection_t &params,
                                 DmeTime_t tStaggerAmount,
                                 const CDmeLogLayer *baseLayer,
                                 CDmeLogLayer *writeLayer);
  void RevealUsingTimeSelection(const DmeLog_TimeSelection_t &params,
                                CDmeLogLayer *savedLayer);
  void BlendLayersUsingTimeSelection(const DmeLog_TimeSelection_t &params);
  void BlendLayersUsingTimeSelection(const CDmeLogLayer *firstLayer,
                                     const CDmeLogLayer *secondLayer,
                                     CDmeLogLayer *outputLayer,
                                     const DmeLog_TimeSelection_t &params,
                                     bool bUseBaseLayerSamples,
                                     DmeTime_t tStartOffset);
  void BlendTimesUsingTimeSelection(const CDmeLogLayer *firstLayer,
                                    const CDmeLogLayer *secondLayer,
                                    CDmeLogLayer *outputLayer,
                                    const DmeLog_TimeSelection_t &params,
                                    DmeTime_t tStartOffset);
  ...
  virtual void PasteAndRescaleSamples(const CDmeLogLayer *pBaseLayer,
                                      const CDmeLogLayer *pDataLayer,
  ...
  virtual void GetValue(DmeTime_t time, CDmAttribute *pAttr,
                        u32 index = 0) const;
  virtual void GetValueSkippingTopmostLayer(DmeTime_t time, CDmAttribute *pAttr,
                                            u32 index = 0) const;
  virtual float GetComponent(DmeTime_t time, int componentIndex) const;
  virtual DmAttributeType_t GetDataType() const;
 ...
 protected:
  ...
  void _StampKeyAtHeadResample(DmeTime_t tHeadPosition,
                               const DmeLog_TimeSelection_t &params,
                               const T &value, bool bSkipToHead,
                               bool bClearPreviousKeys);
  void _StampKeyAtHeadFilteredByTimeSelection(
      DmeTime_t tHeadPosition, DmeTime_t tPreviousHeadPosition,
      const DmeLog_TimeSelection_t &params, const T &value);
  void _StampKeyFilteredByTimeSelection(CDmeTypedLogLayer<T> *pWriteLayer,
                                        DmeTime_t t,
                                        const DmeLog_TimeSelection_t &params,
                                        const T &value, bool bForce = false);

 protected:
  // this really only makes sense for some of our subclasses, basically those
  // which have float data anything else's threshhold is almost certainly 0, and
  // that class just ignores m_threshold
  float m_threshold;
...
bool CDmeTypedLogLayer<T>::IsConstantValued() const {
  if (m_values.Count() < 2) return true;

  if (m_values.Count() == 2 &&
      !GetTypedOwnerLog()->ValuesDiffer(m_values[0], m_values[1]))
    return true;

  // we're throwing away duplicate values during recording, so this is generally
  // correct although there are paths to set keys that don't use the duplicate
  // test, so it's not 100%
  return false;
}
...
bool CDmeTypedLog<T>::IsConstantValued() const {
  ...
  for (int i = 0; i < c; ++i) {
    if (!GetLayer(i)->IsConstantValued()) return false;
  }
...
template <class T>
inline void CDmeTypedLog<T>::BuildNormalizedLayer(
    CDmeTypedLogLayer<float> *pTarget) {
  Assert(pTarget);
  Assert(GetDataType() != AT_FLOAT);

  CDmeTypedLogLayer<T> *pBaseLayer =
      static_cast<CDmeTypedLogLayer<T> *>(GetLayer(0));
  if (!pBaseLayer) return;
...
template <class T>
void CDmeTypedLogLayer<T>::GetValueUsingCurveInfo(DmeTime_t time,
                                                  T &out) const {
  Assert(0);
}
template <class T>
void CDmeTypedLogLayer<T>::GetValueUsingCurveInfoSkippingKey(int nKeyToSkip,
                                                             T &out) const {
  Assert(0);
}
template <>
...
void CDmeTypedLogLayer<T>::CurveSimplify_R(float thresholdSqr, int startPoint,
                                           ...
                                           CDmeTypedLogLayer<T> *output);
template <>
void CDmeTypedLogLayer<bool>::CurveSimplify_R(float thresholdSqr,
...
// template<> void CDmeTypedLog< float >::FinishTimeSelection( DmeTime_t
// tHeadPosition, DmeLog_TimeSelection_t& params ); template<> void
// CDmeTypedLog< bool >::_StampKeyAtHeadResample( const DmeLog_TimeSelection_t&
// params, const bool& value ) { Assert( 0 ); }

//-----------------------------------------------------------------------------
// typedefs for convenience (and so the user-supplied names match the programmer
// names)
//-----------------------------------------------------------------------------
...
void RotateOrientationLog(CDmeQuaternionLogLayer *pOrientationLog,
                          const matrix3x4_t &matrix, bool bPreMultiply);
