...
typedef ChunkFileResult_t (*DefaultChunkHandler_t)(CChunkFile *pFile,
                                                   void *pData,
                                                   char const *pChunkName);

typedef ChunkFileResult_t (*ChunkHandler_t)(CChunkFile *pFile, void *pData);
typedef ChunkFileResult_t (*KeyHandler_t)(const char *szKey,
                                          const char *szValue, void *pData);
typedef bool (*ChunkErrorHandler_t)(CChunkFile *pFile, const char *szChunkName,
                                    void *pData);
...
//	virtual ChunkFileResult_t HandleKey(const char *szKey, const char
//*szValue); 	virtual bool HandleError(const char *szChunkName,
...
class CChunkHandlerMap {
 public:
  ...
  void AddHandler(const char *pszChunkName, ChunkHandler_t pfnHandler,
                  void *pData);
...
class CChunkFile {
 public:
  ...
  ChunkFileResult_t WriteKeyValueColor(const char *pszKey, unsigned char r,
  ...
  ChunkFileResult_t WriteKeyValueVector2(const char *pszKey,
                                         const Vector2D &vec);
  ChunkFileResult_t WriteKeyValueVector3(const char *pszKey, const Vector &vec);
  ChunkFileResult_t WriteKeyValueVector4(const char *pszKey,
                                         const Vector4D &vec);
  ...
  //
  ChunkFileResult_t ReadChunk(KeyHandler_t pfnKeyHandler = NULL,
                              void *pData = NULL);
  ChunkFileResult_t ReadNext(char *szKey, char *szValue, int nValueSize,
  ...
  // These functions should more really be named Parsexxx and possibly moved
  // elsewhere.
  static bool ReadKeyValueBool(const char *pszValue, bool &bBool);
  static bool ReadKeyValueColor(const char *pszValue, unsigned char &chRed,
