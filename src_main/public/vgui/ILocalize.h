...
abstract_class ILocalize {
 public:
  ...
  // terminator
  virtual int ConvertANSIToUnicode(const char *ansi, wchar_t *unicode,
  ...
  // terminator
  virtual int ConvertUnicodeToANSI(const wchar_t *unicode, char *ansi,
                                   int ansiBufferSize) = 0;
  ...
  virtual void ConstructString(
      wchar_t * unicodeOutput, int unicodeBufferSizeInBytes,
      const char *tokenName, KeyValues *localizationVariables) = 0;
...
#endif  // SOURCE_VGUI_ILOCALIZE_H_
