...
#ifndef SOURCE_VGUI_ISURFACE_V30_H_
#define SOURCE_VGUI_ISURFACE_V30_H_
...
the_interface ISurface : public IAppSystem {
 public:
  ...
  virtual void DrawFlushText() = 0;  // flushes any buffered text (for rendering
                                     // optimizations)
  virtual vgui::IHTML *CreateHTMLWindow(vgui::IHTMLEvents *events,
                                        vgui::VPANEL context) = 0;
  ...
  virtual void SetAsToolBar(vgui::VPANEL panel,
                            bool state) = 0;  // removes the window's task bar
                                              // entry (for context menu's,
                                              // etc.)

  // windows stuff
  virtual void CreatePopup(vgui::VPANEL panel, bool minimised,
  ...
  virtual void GetCharABCwide(vgui::HFont font, int ch, int &a, int &b,
                              int &c) = 0;
  virtual int GetCharacterWidth(vgui::HFont font, int ch) = 0;
  virtual void GetTextSize(vgui::HFont font, const wchar_t *text, int &wide,
                           int &tall) = 0;
  ...
  virtual void SolveTraverse(vgui::VPANEL panel,
  ...
  virtual void PaintTraverseEx(vgui::VPANEL panel,
                               bool paintPopups = false) = 0;
  ...
  virtual void DrawSetTextureRGBAEx(int id, const unsigned char *rgba, int wide,
                                    int tall, ImageFormat imageFormat) = 0;
  ...
  virtual void DrawUnicodeString(
      const wchar_t *pwString, FontDrawType_t drawType = FONT_DRAW_DEFAULT) = 0;
};
}  // namespace SurfaceV30

// TODO(d.rattman): This works around using scoped interfaces w/
// EXPOSE_SINGLE_INTERFACE
the_interface ISurfaceV30 : public SurfaceV30::ISurface {
};

#define VGUI_SURFACE_INTERFACE_VERSION_30 "VGUI_Surface030"

#endif  // SOURCE_VGUI_ISURFACE_V30_H_