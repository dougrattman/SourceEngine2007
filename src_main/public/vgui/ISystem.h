...
class ISystem : public IBaseInterface {
 public:
  ...
  // use this with the "open" command to launch web browsers/explorer windows,
  // eg. ShellExecute("open", "www.valvesoftware.com")
  ...
  virtual bool GetRegistryString(const char *key, char *value,
                                 int valueLen) = 0;
  ...
  virtual KeyValues *GetUserConfigFileData(const char *dialogName,
                                           int dialogID) = 0;
  // sets the name of the config file to save/restore from.  Settings are loaded
  // immediately.
  virtual void SetUserConfigFile(const char *fileName,
  ...
  virtual bool GetShortcutTarget(const char *linkFileName, char *targetPath,
                                 char *arguments, int destBufferSizes) = 0;
  ...
  //!! move this function next to other registry function on changing interface
  //! version number
  ...
  virtual void SetClipboardImage(void *pWnd, int x1, int y1, int x2,
                                 int y2) = 0;
};
}  // namespace vgui
