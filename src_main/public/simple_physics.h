...
class CSimplePhysics {
 public:
  class CNode {
   public:
    ...
    void Init(const Vector &vPos) { m_vPos = m_vPrevPos = m_vPredicted = vPos; }
 ...
 public:
  ...
  void Simulate(CNode *pNodes, int nNodes, IHelper *pHelper, float dt,
                float flDamp);
 ...
 private:
  double m_flPredictedTime;  // (GetCurTime()-m_flTimeStep) <= m_flPredictedTime
                             // <= GetCurTime()
  int m_iCurTimeStep;
