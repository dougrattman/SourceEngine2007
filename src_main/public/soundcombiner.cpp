...
class CSoundCombiner : public ISoundCombiner {
 public:
  CSoundCombiner()
      : m_pWaveOutput(NULL), m_pOutRIFF(NULL), m_pOutIterator(NULL) {
    m_szOutFile[0] = 0;
  }

  virtual bool CombineSoundFiles(IFileSystem *filesystem, char const *outfile,
                                 CUtlVector<CombinerEntry> &info);
  virtual bool IsCombinedFileChecksumValid(IFileSystem *filesystem,
                                           char const *outfile,
                                           CUtlVector<CombinerEntry> &info);

 private:
  struct CombinerWork {
    CombinerWork() : sentence(), duration(0.0), wave(0), mixer(0), entry(0) {}
    CSentence sentence;
    float duration;
    CAudioSource *wave;
    CAudioMixer *mixer;
  ...
  bool InternalCombineSoundFiles(IFileSystem *filesystem, char const *outfile,
                                 CUtlVector<CombinerEntry> &info);
  bool VerifyFilesExist(IFileSystem *filesystem,
                        CUtlVector<CombinerEntry> &info);
...
bool CSoundCombiner::CreateWorkList(IFileSystem *filesystem,
                                    CUtlVector<CombinerEntry> &info) {
  m_Work.RemoveAll();
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!LoadSentenceFromWavFile(fullpath, workitem->sentence)) {
      Warning(
          "CSoundCombiner::CreateWorkList couldn't load %s for work item "
          "(%d)\n",
          fullpath, i);
      return false;
    }
...
bool CSoundCombiner::InternalCombineSoundFiles(
    IFileSystem *filesystem, char const *outfile,
    CUtlVector<CombinerEntry> &info) {
  Q_strncpy(m_szOutFile, outfile, sizeof(m_szOutFile));
  if (info.Count() <= 0) {
    Warning(
        "CSoundCombiner::InternalCombineSoundFiles:  work item count is "
        "zero\n");
    return false;
  }
...
bool CSoundCombiner::CombineSoundFiles(IFileSystem *filesystem,
                                       char const *outfile,
                                       CUtlVector<CombinerEntry> &info) {
...
unsigned int CSoundCombiner::ComputeChecksum() {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    CRC32_ProcessBuffer(&crc, curitem->entry->wavefile,
                        Q_strlen(curitem->entry->wavefile));
  }
...
unsigned int CSoundCombiner::CheckSumWork(IFileSystem *filesystem,
                                          CUtlVector<CombinerEntry> &info) {
  if (info.Count() <= 0) {
...
bool CSoundCombiner::IsCombinedFileChecksumValid(
    IFileSystem *filesystem, char const *outfile,
    CUtlVector<CombinerEntry> &info) {
  ...
  if (LoadSentenceFromWavFile(fullpath, sentence)) {
    ...
    Warning(
        "CSoundCombiner::IsCombinedFileChecksumValid:  Unabled to load %s\n",
        fullpath);
  }
...
bool CSoundCombiner::VerifyFilesExist(IFileSystem *filesystem,
                                      CUtlVector<CombinerEntry> &info) {
  int c = info.Count();
  for (int i = 0; i < c; ++i) {
...
bool CSoundCombiner::LoadSentenceFromWavFileUsingIO(char const *wavfile,
                                                    CSentence &sentence,
                                                    IFileReadBinary &io) {
  sentence.Reset();
...
bool CSoundCombiner::LoadSentenceFromWavFile(char const *wavfile,
                                             CSentence &sentence) {
  return CSoundCombiner::LoadSentenceFromWavFileUsingIO(wavfile, sentence,
                                                        io_in);
}
...
/*
bool CSoundCombiner::SaveSentenceToWavFile( char const *wavfile, CSentence&
sentence )
{
        char tempfile[ 512 ];
...
wavfile ); return false;
        ...
        // NOTE:  Put this in it's own scope so that the destructor for
outfileRFF actually closes the file!!!!
        {
                // Read from Temp
                ...
                while ( walk.ChunkAvailable() )
                {
                        ...
                        case WAVE_VALVEDATA:
                                {
                                        // Overwrite data
                                        CSoundCombiner::StoreValveDataChunk(
sentence ); wordtrackwritten = true;
                                }
                                break;
                        default:
...
bool CSoundCombiner::InitSplicer(IFileSystem *filesystem, int samplerate,
...
bool CSoundCombiner::LoadSpliceAudioSources() {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!pMixer) {
      Warning(
          "CSoundCombiner::LoadSpliceAudioSources  CreateMixer failed '%s'\n",
          item->entry->wavefile);
      return false;
    }
...
bool CSoundCombiner::AppendWaveData(int &currentsample, CAudioSource *wave,
                                    CAudioMixer *mixer) {
  // need a bit of space
  short samples[2];
  channel_t channel;
  ...
  while (1) {
    ...
    if (!mixer->MixDataToDevice(&m_pWaveOutput->m_audioDevice, &channel,
                                currentsample, 1, wave->SampleRate(), true))
      break;
...
int CSoundCombiner::ComputeBestNumChannels() {
  // We prefer mono output unless one of the source wav files is stereo, then
  // we'll do stereo output
  int c = m_Work.Count();
  for (int i = 0; i < c; ++i) {
    CombinerWork *curitem = m_Work[i];
...
bool CSoundCombiner::PerformSplicingOnWorkItems(IFileSystem *filesystem) {
  ...
  if (!InitSplicer(filesystem, WAVEOUTPUT_FREQUENCY, bestNumChannels,
                   bitsPerChannel)) {
    return false;
  }
