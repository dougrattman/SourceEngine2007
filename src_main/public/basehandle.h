...
class CBaseHandle {
 ...
 public:
  ...
  // Even if this returns true, Get() still can return return a non-0 value.
 ...
 protected:
  // The low NUM_SERIAL_BITS hold the index. If this value is less than
  // MAX_EDICTS, then the entity is networkable. The high NUM_SERIAL_NUM_BITS
  // bits are the serial number.
...
inline bool CBaseHandle::operator<(const IHandleEntity *pEntity) const {
  unsigned long otherIndex =
      (pEntity) ? pEntity->GetRefEHandle().m_Index : INVALID_EHANDLE_INDEX;
