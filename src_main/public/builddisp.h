...
class CCoreDispSurface {
 ...
 protected:
  ...
  int m_PointStartIndex;  // index to the starting point -- for saving starting
                          // point
...
class CCoreDispNode {
 ...
 protected:
  ...
  int m_NeighborVertIndices
      [MAX_NEIGHBOR_VERT_COUNT];  // all other vertex indices in node (maximally
                                  // creates 8 trianglar surfaces)
  Vector m_SurfBBoxes[MAX_SURF_AT_NODE_COUNT]
                     [2];  // surface bounding boxes - old method
  cplane_t
      m_SurfPlanes[MAX_SURF_AT_NODE_COUNT];  // surface plane info - old method
...
class CCoreDispInfo : public CDispUtilsHelper {
 ...
 public:
  ...
  void InitSurf(int parentIndex, Vector points[4], Vector normals[4],
                Vector2D texCoords[4], Vector2D lightCoords[4][4], int contents,
                int flags, bool bGenerateSurfPointStart, Vector &startPoint,
                bool bHasMappingAxes, Vector &uAxis, Vector &vAxis);

  void InitDispInfo(int power, int minTess, float smoothingAngle, float *alphas,
                    Vector *dispVectorField, float *dispDistances);
 ...
 private:
  ...
  void DispUVToSurf_TriTLToBR(Vector &vecPoint, Vector *pNormal, float *pAlpha,
                              float flU, float flV,
                              const Vector &vecIntersectPoint);
  void DispUVToSurf_TriBLToTR(Vector &vecPoint, Vector *pNormal, float *pAlpha,
                              float flU, float flV,
                              const Vector &vecIntersectPoint);
