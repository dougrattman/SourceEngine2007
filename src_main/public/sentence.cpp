...
#if PHONEME_EDITOR
void CEmphasisSample::SetSelected(bool isSelected) { selected = isSelected; }
void CPhonemeTag::SetSelected(bool isSelected) { m_bSelected = isSelected; }
bool CPhonemeTag::GetSelected() const { return m_bSelected; }
void CPhonemeTag::SetStartAndEndBytes(unsigned int start, unsigned int end) {
  m_uiStartByte = start;
  m_uiEndByte = end;
}
unsigned int CPhonemeTag::GetStartByte() const { return m_uiStartByte; }
unsigned int CPhonemeTag::GetEndByte() const { return m_uiEndByte; }
void CWordTag::SetSelected(bool isSelected) { m_bSelected = isSelected; }
bool CWordTag::GetSelected() const { return m_bSelected; }
void CWordTag::SetStartAndEndBytes(unsigned int start, unsigned int end) {
  m_uiStartByte = start;
  m_uiEndByte = end;
}
unsigned int CWordTag::GetStartByte() const { return m_uiStartByte; }
unsigned int CWordTag::GetEndByte() const { return m_uiEndByte; }
#else
...
void CSentence::ColorForLanguage(int language, unsigned char &r,
...
char const *CSentence::NameForLanguage(int language) {
  if (language < 0 || language >= CC_NUM_LANGUAGES) return "unknown_language";
...
void CSentence::ParseWords(CUtlBuffer &buf) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      buf.GetString(token);
      float tagend = atof(token);
      buf.GetString(token);
      float volume = atof(token);

      CPhonemeTag *pt = new CPhonemeTag();
      assert(pt);
      pt->SetPhonemeCode(code);
      pt->SetTag(phonemename);
      pt->SetStartTime(tagstart);
      pt->SetEndTime(tagend);
...
// This is a compressed save of just the data needed to drive phonemes in the
// engine (no word / sentence text, etc )
//-----------------------------------------------------------------------------
// Purpose:
// Input  : buf -
//-----------------------------------------------------------------------------
...
void CSentence::SaveToBuffer(CUtlBuffer &buf) {
  ...
  for (i = 0; i < m_Words.Size(); i++) {
    ...
    buf.Printf("WORD %s %.3f %.3f\n", word->GetWord(), word->m_flStartTime,
               word->m_flEndTime);
    ...
    for (j = 0; j < word->m_Phonemes.Size(); j++) {
      ...
      buf.Printf("%i %s %.3f %.3f 1\n", phoneme->GetPhonemeCode(),
                 phoneme->GetTag(), phoneme->GetStartTime(),
                 phoneme->GetEndTime());
    }
...
bool CSentence::ShouldSplitWord(char in) {
  if (in <= 32) return true;

  if (in >= 128) return true;
...
void CSentence::CreateEventWordDistribution(char const *pszText,
                                            float flSentenceDuration) {
  Assert(pszText);
  if (!pszText) return;
  ...
  float wordLength =
      (flSentenceDuration - 2 * STARTEND_TIMEGAP) / (float)wordCount;
