...
mstudioanim_t *mstudioanimdesc_t::pAnimBlock(int block, int index) const {
  ...
  if (block == 0) {
    return (mstudioanim_t *)(((uint8_t *)this) + index);
  }

  uint8_t *pAnimBlock = pStudiohdr()->GetAnimBlock(block);
  if (pAnimBlock) {
...
static ConVar mod_load_showstall("mod_load_showstall", "0", 0,
  ...
  if (panim == NULL) {
    if (section > 0 && mod_load_showstall.GetInt() > 0) {
      Msg("[%8.3f] hitch on %s:%s:%d:%d\n", Plat_FloatTime(),
          pStudiohdr()->pszName(), pszName(), section, block);
    }
  ...
  if (panim == NULL && section <= 0) {
    ...
    if (flStall == 0.0f) {
      ...
      Msg("[%8.3f] stall blend %.2f on %s:%s:%d:%d\n", Plat_FloatTime(),
          flStall, pStudiohdr()->pszName(), pszName(), section, block);
    }
  }
...
mstudioikrule_t *mstudioanimdesc_t::pIKRule(int i) const {
  if (ikruleindex) {
    return (mstudioikrule_t *)(((uint8_t *)this) + ikruleindex) + i;
  } else if (animblockikruleindex) {
    if (animblock == 0) {
      return (mstudioikrule_t *)(((uint8_t *)this) + animblockikruleindex) + i;
    } else {
      uint8_t *pAnimBlocks = pStudiohdr()->GetAnimBlock(animblock);
...
mstudiolocalhierarchy_t *mstudioanimdesc_t::pHierarchy(int i) const {
  if (localhierarchyindex) {
    if (animblock == 0) {
      return (mstudiolocalhierarchy_t *)(((uint8_t *)this) +
                                         localhierarchyindex) +
             i;
    } else {
      uint8_t *pAnimBlocks = pStudiohdr()->GetAnimBlock(animblock);
      ...
      if (pAnimBlocks) {
        return (mstudiolocalhierarchy_t *)(pAnimBlocks + localhierarchyindex) +
               i;
      }
    }
  }
...
void studiohdr_t::SetAttachmentBone(int iAttachment, int iBone) {
  mstudioattachment_t &attachment =
      (mstudioattachment_t &)pAttachment(iAttachment);

  // remap bone
  virtualmodel_t *pVModel = GetVirtualModel();
  if (pVModel) {
    virtualgroup_t *pGroup =
        &pVModel->m_group[pVModel->m_attachment[iAttachment].group];
...
const char *studiohdr_t::pszNodeName(int iNode) const {
  ...
  return pVModel->m_group[pVModel->m_node[iNode - 1].group]
      .GetStudioHdr()
      ->pszLocalNodeName(pVModel->m_node[iNode - 1].index);
}
...
int studiohdr_t::GetActivityListVersion(void) const {
  ...
  int activity_list_version = activitylistversion;
...
void studiohdr_t::SetActivityListVersion(int activity_list_version) const {
  activitylistversion = activity_list_version;
  ...
  for (i = 1; i < pVModel->m_group.Count(); i++) {
    ...
    pStudioHdr->SetActivityListVersion(activity_list_version);
  }
}
...
CStudioHdr::CStudioHdr(const studiohdr_t *pStudioHdr, IMDLCache *mdlcache) {
  // preset pointer to bogus value (it may be overwritten with legitimate data
  // later)
  m_nFrameUnlockCounter = 0;
  m_pFrameUnlockCounter = &m_nFrameUnlockCounter;
...
void CStudioHdr::Init(const studiohdr_t *pStudioHdr, IMDLCache *mdlcache) {
  ...
  if (mdlcache) {
    m_pFrameUnlockCounter =
        mdlcache->GetFrameUnlockCounterPtr(MDLCACHE_STUDIOHDR);
...
const virtualmodel_t *CStudioHdr::ResetVModel(
    const virtualmodel_t *pVModel) const {
  if (pVModel != NULL) {
...
const studiohdr_t *CStudioHdr::GroupStudioHdr(int i) const {
  if (m_nFrameUnlockCounter != *m_pFrameUnlockCounter) {
    ...
    if (*m_pFrameUnlockCounter !=
        m_nFrameUnlockCounter)  // i.e., this thread got the mutex
    {
      memset(m_pStudioHdrCache.Base(), 0,
             m_pStudioHdrCache.Count() * sizeof(studiohdr_t *));
...
const studiohdr_t *CStudioHdr::pSeqStudioHdr(int sequence) const {
  ...
  const studiohdr_t *pStudioHdr =
      GroupStudioHdr(m_pVModel->m_seq[sequence].group);
...
const studiohdr_t *CStudioHdr::pAnimStudioHdr(int animation) const {
  ...
  const studiohdr_t *pStudioHdr =
      GroupStudioHdr(m_pVModel->m_anim[animation].group);
...
int CStudioHdr::GetSharedPoseParameter(int iSequence, int iLocalPose) const {
  ...
  virtualgroup_t *pGroup =
      &m_pVModel->m_group[m_pVModel->m_seq[iSequence].group];
...
const mstudioattachment_t &CStudioHdr::pAttachment(int i) const {
  ...
  const studiohdr_t *pStudioHdr =
      GroupStudioHdr(m_pVModel->m_attachment[i].group);
...
int CStudioHdr::GetAttachmentBone(int i) const {
  ...
  virtualgroup_t *pGroup =
      &m_pVModel->m_group[m_pVModel->m_attachment[i].group];
  const mstudioattachment_t &attachment = pAttachment(i);
  int iBone = pGroup->masterBone[attachment.localbone];
  if (iBone == -1) return 0;
  return iBone;
...
void CStudioHdr::SetAttachmentBone(int iAttachment, int iBone) {
  mstudioattachment_t &attachment =
      (mstudioattachment_t &)m_pStudioHdr->pAttachment(iAttachment);

  // remap bone
  if (m_pVModel) {
    virtualgroup_t *pGroup =
        &m_pVModel->m_group[m_pVModel->m_attachment[iAttachment].group];
    iBone = pGroup->boneMap[iBone];
  }
  attachment.localbone = iBone;
}
...
const char *CStudioHdr::pszNodeName(int iNode) const {
  ...
  const studiohdr_t *pStudioHdr =
      GroupStudioHdr(m_pVModel->m_node[iNode - 1].group);
...
int CStudioHdr::GetTransition(int iFrom, int iTo) const {
  if (m_pVModel == NULL) {
    return *m_pStudioHdr->pLocalTransition(
        (iFrom - 1) * m_pStudioHdr->numlocalnodes + (iTo - 1));
  }
...
void CStudioHdr::RunFlexRules(const float *src, float *dest) {
  ...
  for (i = 0; i < numflexrules(); i++) {
    ...
    // 0 means run them all const char *pszExpression =
    flex_maxrule.GetString(); if ( pszExpression )
                    {
                            nFlexRulesToRun = atoi(pszExpression);
    // 0 will be returned if not a numeric string
                    }
                    // end JasonM hack
    //*/
    // debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), i + 1,
    // 0, "%2d:%d\n", i, prule->flex );
    ...
    for (j = 0; j < prule->numops; j++) {
      switch (pops->op) {
        ...
        case STUDIO_CONST:
          ...
          break;
        case STUDIO_FETCH1: {
          int m = pFlexcontroller((LocalFlexController_t)pops->d.index)
                      ->localToGlobal;
          stack[k] = src[m];
          k++;
          break;
        ...
        case STUDIO_NWAY: {
          LocalFlexController_t valueControllerIndex =
              static_cast<LocalFlexController_t>((int)stack[k - 1]);
          int m = pFlexcontroller(valueControllerIndex)->localToGlobal;
          float flValue = src[m];
          int v = pFlexcontroller((LocalFlexController_t)pops->d.index)
                      ->localToGlobal;

          const Vector4D filterRamp(stack[k - 5], stack[k - 4], stack[k - 3],
                                    stack[k - 2]);
          ...
          if (flValue <= filterRamp.x || flValue >= filterRamp.w) {
            ...
            flValue = RemapValClamped(flValue, filterRamp.x, filterRamp.y, 0.0f,
                                      1.0f);
          } else if (flValue > filterRamp.z) {
            flValue = RemapValClamped(flValue, filterRamp.z, filterRamp.w, 1.0f,
                                      0.0f);
          } else {
            flValue = 1.0f;
          }
        ...
        case STUDIO_DME_LOWER_EYELID: {
          ...
          const float flCloseLidV =
              RemapValClamped(src[pCloseLidV->localToGlobal], pCloseLidV->min,
                              pCloseLidV->max, 0.0f, 1.0f);

          const mstudioflexcontroller_t *const pCloseLid = pFlexcontroller(
              static_cast<LocalFlexController_t>((int)stack[k - 1]));
          const float flCloseLid =
              RemapValClamped(src[pCloseLid->localToGlobal], pCloseLid->min,
                              pCloseLid->max, 0.0f, 1.0f);

          int nBlinkIndex = static_cast<int>(stack[k - 2]);
          float flBlink = 0.0f;
          if (nBlinkIndex >= 0) {
            const mstudioflexcontroller_t *const pBlink = pFlexcontroller(
                static_cast<LocalFlexController_t>((int)stack[k - 2]));
            flBlink = RemapValClamped(src[pBlink->localToGlobal], pBlink->min,
                                      pBlink->max, 0.0f, 1.0f);
          }

          int nEyeUpDownIndex = static_cast<int>(stack[k - 3]);
          float flEyeUpDown = 0.0f;
          if (nEyeUpDownIndex >= 0) {
            const mstudioflexcontroller_t *const pEyeUpDown = pFlexcontroller(
                static_cast<LocalFlexController_t>((int)stack[k - 3]));
            flEyeUpDown =
                RemapValClamped(src[pEyeUpDown->localToGlobal], pEyeUpDown->min,
                                pEyeUpDown->max, -1.0f, 1.0f);
          }

          if (flEyeUpDown > 0.0) {
            stack[k - 3] =
                (1.0f - flEyeUpDown) * (1.0f - flCloseLidV) * flCloseLid;
          } else {
    ...
    /*
                    // JasonM hack
                    if ( nFlexRulesToRun == 0) // 0 means run all rules
    correctly
                    {
                            dest[prule->flex] = stack[0];
                    }
                    else // run only up to nFlexRulesToRun correctly...zero out
    the rest
                    {
...
void CStudioHdr::CActivityToSequenceMapping::Initialize(
    CStudioHdr *__restrict pstudiohdr) {
  // Algorithm: walk through every sequence in the model, determine to which
  // activity it corresponds, and keep a count of sequences per activity. Once
  // the total count is available, allocate an array large enough to contain
  // them all, update the starting indices for every activity's section in the
  // array, and go back through, populating the array with its data.

  AssertMsg1(m_pSequenceTuples == NULL,
             "Tried to double-initialize sequence mapping for %s",
             pstudiohdr->pszName());
  if (m_pSequenceTuples != NULL) return;  // don't double initialize.
  ...
  for (int i = 0; i < NumSeq; ++i) {
    ...
    if (!(seqdesc.flags & STUDIO_ACTIVITY)) {
      // AssertMsg2( false, "Sequence %d on studiohdr %s didn't have its
      // activity initialized!", i, pstudiohdr->pszName() );
    ...
    if (seqdesc.activity >= 0) {
      ...
      // look up if we already have an entry. First we need to make a
      // speculative one --
  ...
  int sequenceCount = 0;
  int topActivity = 0;  // this will store the highest seen activity number
                        // (used later to make an ad hoc map on the stack)
  ...
  // restrict; this is an important optimization, but means that you must never
  // refer to this array through m_pSequenceTuples in the scope of this
  // function.
  SequenceTuple *__restrict tupleList = new SequenceTuple[sequenceCount];
  m_pSequenceTuples = tupleList;  // save it off -- NEVER USE m_pSequenceTuples
                                  // in this function!
  m_iSequenceTuplesCount = sequenceCount;

  // Now we're going to actually populate that list with the relevant data.
  // First, create an array on the stack to store how many sequences we've
  ...
  for (int i = 0; i < NumSeq; ++i) {
    const mstudioseqdesc_t &seqdesc = pstudiohdr->pSeqdesc(i);
    if (seqdesc.activity >= 0) {
      const HashValueType &element = m_ActToSeqHash[m_ActToSeqHash.Find(
          HashValueType(seqdesc.activity, 0, 0, 0))];

      // If this assert trips, we've written more sequences per activity than we
      // allocated (therefore there must have been a miscount in the first for
      // loop above).
      int tupleOffset = seqsPerAct[seqdesc.activity];
      Assert(tupleOffset < element.count);

      // You might be tempted to collapse this pointer math into a single
      // pointer -- don't! the tuple list is marked __restrict above.
      (tupleList + element.startingIdx + tupleOffset)->seqnum =
          i;  // store sequence number
      (tupleList + element.startingIdx + tupleOffset)->weight =
...
void CStudioHdr::CActivityToSequenceMapping::Reinitialize(
...
// Look up relevant data for an activity's sequences. This isn't terribly
// efficient, due to the load-hit-store on the output parameters, so the most
// common case -- SelectWeightedSequence -- is specially implemented.
const CStudioHdr::CActivityToSequenceMapping::SequenceTuple *
CStudioHdr::CActivityToSequenceMapping::GetSequences(
    int forActivity, int *__restrict outSequenceCount,
    int *__restrict outTotalWeight) {
  // Construct a dummy entry so we can do a hash lookup (the UtlHash does not
  // divorce keys from values)
...
int CStudioHdr::CActivityToSequenceMapping::NumSequencesForActivity(
    int forActivity) {
...
bool CStudioHdr::CActivityToSequenceMapping::ValidateAgainst(
  ...
  if (m_bIsInitialized) {
    ...
    return true;  // Allow an ordinary initialization to take place without
                  // printing a panicky assert.
  }
}
