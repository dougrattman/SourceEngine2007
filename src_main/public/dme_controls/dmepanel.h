...
//-----------------------------------------------------------------------------
SOURCE_DECLARE_POINTER_HANDLE(DmeFactoryHandle_t);
...
class CDmePanel : public vgui::EditablePanel {
 ...
 public:
  ...
  CDmePanel(vgui::Panel *pParent, const char *pPanelName,
            bool bComboBoxVisible = true);
  virtual ~CDmePanel();

  virtual void PerformLayout();
  virtual void ApplySchemeSettings(vgui::IScheme *pScheme);

  void SetDmeElement(CDmElement *pDmeElement, bool bForce = false,
                     const char *pPanelName = NULL);
 ...
 private:
  ...
  static bool CreateDmePanel(vgui::Panel *pParent, const char *pPanelName,
                             CDmElement *pElement, const char *pEditorName,
                             EditorPanelMap_t *pMap);
...
class CBaseDmePanelFactory {
 public:
  virtual vgui::EditablePanel *CreateDmePanel(vgui::Panel *pParent,
                                              const char *pPanelName,
                                              CDmElement *pElement) = 0;
  virtual void SetDmeElement(vgui::EditablePanel *pPanel,
                             CDmElement *pElement) = 0;

 protected:
  // Constructor, protected because these should never be instanced directly
  CBaseDmePanelFactory(const char *pElementType, const char *pEditorName,
                       const char *pEditorDisplayName, bool bIsDefault,
                       bool bIsOverride);
...
class CDmePanelFactory : public CBaseDmePanelFactory {
 ...
 public:
  // Constructor
  CDmePanelFactory(const char *pElementType, const char *pEditorName,
                   const char *pEditorDisplayName, bool bIsDefault,
                   bool bIsOverride)
      : BaseClass(pElementType, pEditorName, pEditorDisplayName, bIsDefault,
                  bIsOverride) {}
...
class CDmePanelConverterFactory : public CBaseDmePanelFactory {
 ...
 public:
  // Constructor
  CDmePanelConverterFactory(const char *pElementType, const char *pDisplayType,
  ...
  virtual vgui::EditablePanel *CreateDmePanel(vgui::Panel *pParent,
    ...
    DisplayType *pDisplayElement = CastElement<DisplayType>(
        pTypedElement->GetDmePanelElement(m_pDisplayType));
...
//-----------------------------------------------------------------------------
#define IMPLEMENT_DMEPANEL_FACTORY(_panelClassName, _dmeLookupName,     \
                                   _editorName, _editorDisplayName,     \
                                   _isDefault)                          \
  CDmePanelFactory<_panelClassName, C##_dmeLookupName>                  \
      g_##_panelClassName##_##_dmeLookupName##_Factory(                 \
          #_dmeLookupName, _editorName, _editorDisplayName, _isDefault, \
          false);                                                       \
  _panelClassName *g_##_panelClassName##_##_dmeLookupName##LinkerHack = NULL;

#define IMPLEMENT_DMEPANEL_FACTORY_OVERRIDE(_panelClassName, _dmeLookupName,   \
                                            _editorName, _editorDisplayName,   \
                                            _isDefault)                        \
  CDmePanelFactory<_panelClassName, C##_dmeLookupName>                         \
      g_##_panelClassName##_##_dmeLookupName##_Factory(                        \
          #_dmeLookupName, _editorName, _editorDisplayName, _isDefault, true); \
  _panelClassName *g_##_panelClassName##_##_dmeLookupName##LinkerHack = NULL;
...
#define USING_DMEPANEL_FACTORY(_panelClassName, _dmeLookupName)               \
  ...
  _panelClassName *g_##_panelClassName##_##_dmeLookupName##PullInModule =     \
      g_##_panelClassName##_##_dmeLookupName##LinkerHack;

//-----------------------------------------------------------------------------
// Helper macro to create the converter panel factory
// IMPLEMENT_DMEPANEL_CONVERSION_FACTORY_OVERRIDE is used by applications to
// override DmePanels implemented in libraries
//-----------------------------------------------------------------------------
#define IMPLEMENT_DMEPANEL_CONVERSION_FACTORY(_panelClassName, _dmeLookupName, \
                                              _dmeDisplayName, _editorName,    \
                                              _editorDisplayName, _isDefault)  \
  CDmePanelConverterFactory<_panelClassName, C##_dmeLookupName,                \
                            C##_dmeDisplayName>                                \
      g_##_panelClassName##_##_dmeLookupName##_Factory(                        \
...
#define IMPLEMENT_DMEPANEL_CONVERSION_FACTORY_OVERRIDE(                       \
    _panelClassName, _dmeLookupName, _dmeDisplayName, _editorName,            \
    _editorDisplayName, _isDefault)                                           \
  CDmePanelConverterFactory<_panelClassName, C##_dmeLookupName,               \
                            C##_dmeDisplayName>                               \
      g_##_panelClassName##_##_dmeLookupName##_Factory(                       \
...
DmeFactoryHandle_t DmePanelNextFactory(DmeFactoryHandle_t h,
...
// Dme Panel factory methods
//-----------------------------------------------------------------------------
vgui::EditablePanel *CreateDmePanel(vgui::Panel *pParent,
                                    const char *pPanelName,
                                    CDmElement *pElement,
                                    const char *pEditorName = NULL);
