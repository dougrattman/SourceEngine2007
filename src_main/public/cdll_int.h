...
the_interface IVEngineClient {
 public:
  ...
  //  returns the string name of the key to which this string is bound. Returns
  //  NULL if no such binding exists
  virtual const char *Key_LookupBinding(const char *pBinding) = 0;

  // Given the name of the key "mouse1", "e", "tab", etc., return the string it
  // is bound to "+jump", "impulse 50", etc.
  ...
  // Returns true if the specified box is outside of the view frustum and should
  // be culled
  ...
  // The .bsp file can have mod-specified data lumps. These APIs are for working
  // with such game lumps.
  ...
  // Loads a game lump off disk, writing the data into the buffer pointed to bye
  // pBuffer Returns false if the data can't be read or the destination buffer
  // is too small
  ...
  // This tells the engine to fire any events (temp entity messages) that it has
  // queued up this frame. It should only be called once per frame.
  virtual void FireEvents() = 0;

  // Returns an area index if all the leaves are in the same area. If they span
  // multple areas, then it returns -1.
  ...
  // This can be used to notify test scripts that we're at a particular spot in
  // the code.
  ...
  // is this level loaded as just the background to the main menu? (active, but
  // unplayable)
  ...
  // allow other modules to know about engine versioning (one use is a proxy for
  ...
  // Inserts szCmdString into the command buffer as if it was typed by the
  // client to his/her console. And then executes the command string immediately
  // (vs ClientCmd() which executes in the next frame)
  //
  ...
  // commands marked with FCVAR_SERVER_CAN_EXECUTE. By default, mods are allowed
  // to execute any server commands, and they can restrict the server's ability
  // to execute client commands with this function.
...
the_interface IBaseClientDLL {
 public:
  ...
  //  side of a windowed version of the engine if rendering or other processing
  //  is taking too long
  ...
  // Raw keyboard signal, if the client .dll returns 1, the engine processes the
  // key as usual, otherwise,
  ...
  // If the game is running faster than the tick_interval framerate, then we do
  // extra mouse sampling to avoid jittery input
  //  This code path is much like the normal move creation code, except no move
  //  is created
  virtual void ExtraMouseSample(float frametime, bool active) = 0;

  // Encode the delta (changes) between the CUserCmd in slot from vs the one in
  // slot to.  The game code will have
  ...
  // The engine has parsed a crosshair angle message, this function is called to
  // dispatch the new crosshair angle
  ...
  // entindex is -1 to represent the local client talking (before the data comes
  // back from the server). entindex is -2 to represent the local client's voice
  // being acked by the server. entindex is GetPlayer() when the server
  // acknowledges that the local client is talking.
  ...
  // returns false, an error message of up to length bytes should be returned in
  // errorMsg.
  ...
  // save game screenshot writing
  virtual void WriteSaveGameScreenshotOfSize(const char *pFilename, int width,
                                             int height) = 0;
  ...
  virtual void GetPropertyDisplayString(uint32_t id, uint32_t value,
                                        char *pOutput, int nBytes) = 0;
#ifdef _WIN32
