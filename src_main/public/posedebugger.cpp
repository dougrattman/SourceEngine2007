...
static ConVar ui_posedebug_fade_in_time(
    "ui_posedebug_fade_in_time", "0.2", FCVAR_CHEAT | FCVAR_DONTRECORD,
    "Time during which a new pose activity layer is shown in green in "
    "+posedebug UI");
static ConVar ui_posedebug_fade_out_time(
    "ui_posedebug_fade_out_time", "0.8", FCVAR_CHEAT | FCVAR_DONTRECORD,
    "Time to keep a no longer active pose activity layer in red until removing "
    "it from +posedebug UI");
...
class CPoseDebuggerStub : public IPoseDebugger {
 public:
  ...
  virtual void AccumulatePose(const CStudioHdr *pStudioHdr,
                              CIKContext *pIKContext, Vector pos[],
                              Quaternion q[], int sequence, float cycle,
  ...
  if (pOld) {
    ...
    if (k == o.m_arrTxt.Count()) {
      if (!x) {
        ...
        // Don't update the current when insertion happens and don't have
        // finished commands
    ...
    for (int iFinished = iFinishedRange[0]; iFinished < iFinishedRange[1];
         ++iFinished) {
...
void ModelPoseDebugInfo::PrintPendingInfoText(int &rnPosPrint) {
  ...
  for (int k = m_iCurrentText; k < m_arrTxt.Count(); ++k) {
    ...
    for (int iLine = 1; iLine < ModelPoseDebugInfo::InfoText::MAX_TEXT_LINES;
         ++iLine) {
      if (!prntxt.m_chTextLines[iLine][0]) break;
...
void CPoseDebuggerImpl::ShowModel(int iEntNum, bool bShow) {
  ...
  if (iEntNum < MAX_EDICTS) m_uiMaskShowModels.Set(iEntNum, bShow);
}
...
void CPoseDebuggerImpl::StartBlending(IClientNetworkable *pEntity,
                                      const CStudioHdr *pStudioHdr) {
  ...
  if (!pRMdl || !pRMdl->numincludemodels) return;

  // Entity number
  int iEntNum = pEntity->entindex();
  if (!IsModelShown(iEntNum)) return;
  ...
  if (m_mapModel.Find(pStudioHdr) != m_mapModel.InvalidIndex()) {
    ...
    for (int k = m_mapModelOld.FirstInorder();
         k != m_mapModelOld.InvalidIndex(); k = m_mapModelOld.NextInorder(k)) {
  ...
  engine->Con_NXPrintf(&nxPrn, "[ %2d  ]    Model: %s", iEntNum,
                       pRMdl->pszName());
  m_nPosPrint += 3;
}

void CPoseDebuggerImpl::AccumulatePose(const CStudioHdr *pStudioHdr,
                                       CIKContext *pIKContext, Vector pos[],
                                       Quaternion q[], int sequence,
                                       float cycle, const float poseParameter[],
                                       int boneMask, float flWeight,
                                       float flTime) {
  //	virtualmodel_t const *pVMdl = pStudioHdr->GetVirtualModel();
  // 	if ( !pVMdl )
  // 		return;
  ...
  if (idxMapModel == m_mapModel.InvalidIndex()) return;

  ModelPoseDebugInfo &mpi = m_mapModel.Element(idxMapModel);
  if (!IsModelShown(mpi.m_iEntNum)) return;
  ...
  ModelPoseDebugInfo::InfoText *pOldTxt =
      pMpiOld ? pMpiOld->LookupInfoText(&txt) : NULL;
  sprintf(txt.m_chTextLines[numLines],
          "%-*s  %-*s  %*.2f  %*.1f/%-*d  %*.0f%% ", widthActivity,
          seqdesc.pszActivityName(), widthLayer, seqdesc.pszLabel(), 7,
          pOldTxt ? pOldTxt->m_flTimeAlive : 0.f, 5,
          cycle * (pStudioHdr->pAnimdesc(seqdesc.anim(0, 0)).numframes - 1), 3,
          pStudioHdr->pAnimdesc(seqdesc.anim(0, 0)).numframes, widthPercent,
          flWeight * 100.0f);
  ++numLines;

  if (seqdesc.numiklocks) {
    sprintf(chBuffer, "iklocks : %-2d : ", seqdesc.numiklocks);
    ...
    sprintf(txt.m_chTextLines[numLines], "%-*s", widthIks, chBuffer);
    ++numLines;
  }
...
static ConCommand posedebuggerstart(
    "+posedebug", IN_PoseDebuggerStart,
    "Turn on pose debugger or add ents to pose debugger UI", FCVAR_CHEAT);
static ConCommand posedebuggerend(
    "-posedebug", IN_PoseDebuggerEnd,
    "Turn off pose debugger or hide ents from pose debugger UI", FCVAR_CHEAT);
