...
class CManagedDataCacheClient : public CDefaultDataCacheClient {
 public:
  ...
  void Init(IDataCache *pSharedCache, const char *pszSectionName,
    ...
    if (!m_pCache) {
      m_pCache = pSharedCache->AddSection(this, pszSectionName, limits,
                                          bSupportFastFind);
    }
  }
  ...
  LOCK_TYPE CacheGet(DataCacheHandle_t handle, bool bFrameLock = true) {
    return (LOCK_TYPE)(
        ((STORAGE_TYPE *)m_pCache->Get(handle, bFrameLock))->GetData());
  }

  LOCK_TYPE CacheGetNoTouch(DataCacheHandle_t handle) {
    return (LOCK_TYPE)(
        ((STORAGE_TYPE *)m_pCache->GetNoTouch(handle))->GetData());
  }
  ...
  DataCacheHandle_t CacheCreate(const CREATE_PARAMS &createParams,
                                unsigned flags = DCAF_DEFAULT) {
    ...
    m_pCache->AddEx((DataCacheClientID_t)pStore, pStore, pStore->Size(), flags,
                    &handle);
    return handle;
  }
