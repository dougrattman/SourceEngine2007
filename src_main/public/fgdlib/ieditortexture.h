...
//			implementations are for world textures (WADTexture.cpp)
//and sprite 			textures (Texture.cpp).
...
class IEditorTexture {
 public:
  ...
  virtual bool Load(void) = 0;  // ensure that texture is loaded. could this be
                                // done internally?
  ...
  // textures
  //			serve as placeholders for textures that were found in the
  //map, but 			not in the WAD (or the materials tree). The
  // dummy texture enables us 			to bind the texture, find it by
  // name, etc.
  //-----------------------------------------------------------------------------
  virtual bool IsDummy(void) const = 0;  // dvs: perhaps not the best name?

  //-----------------------------------------------------------------------------
  // Purpose: Returns the unique texture ID for this texture object. The texture
  // ID
  //			identifies the texture object across all renderers, and
  //is
  // assigned 			by the first renderer that actually binds the texture
  // thru BindTexture.
  //
