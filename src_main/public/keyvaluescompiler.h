...
#define COMPILED_KEYVALUES_ID MAKEID<int32_t>('V', 'K', 'V', 'F')
...
#pragma pack(1)
struct KVFile_t {
  KVFile_t() : filename(0), firstElement(0), numElements(0) {}
  short filename;
  short firstElement;
  short numElements;
};
...
struct KVInfo_t {
  KVInfo_t() : key(0), value(0), parentIndex(-1), issubtree(false) {}
  ...
  inline short GetParent() const { return parentIndex; }

  inline void SetSubTree(bool state) { issubtree = state; }

  inline bool IsSubTree() const { return issubtree; }
...
class CCompiledKeyValuesWriter {
 public:
  CCompiledKeyValuesWriter() { m_StringTable.AddString(""); }
...
class CRunTimeKeyValuesStringTable {
 public:
  ...
  inline char const *Lookup(short index) { return m_Strings[index]; }
...
class CCompiledKeyValuesReader {
 public:
  ...
  bool LookupKeyValuesRootKeyName(char const *kvfilename, char *outbuf,
                                  size_t bufsize);
 ...
 private:
  struct FileInfo_t {
    FileInfo_t() : hFile(0), nFirstIndex(0), nCount(0) {}
