...
struct MatchMakingKeyValuePair_t {
  ...
  MatchMakingKeyValuePair_t(const char *pchKey, const char *pchValue) {
    strcpy_s(m_szKey, pchKey);  // this is a public header, use basic c library
                                // string funcs only!
    strcpy_s(m_szValue, pchValue);
  }
  char m_szKey[256];
  char m_szValue[256];
};
...
#ifdef NETADR_H
inline void servernetadr_t::Init(const netadr_t &ipAndQueryPort,
                                 uint16 usConnectionPort) {
...
inline uint16 servernetadr_t::GetQueryPort() const { return m_usQueryPort; }
...
inline uint32 servernetadr_t::GetIP() const { return m_unIP; }

inline void servernetadr_t::SetIP(uint32 unIP) { m_unIP = unIP; }

inline const char *servernetadr_t::ToString(uint32 unIP, uint16 usPort) const {
  static char s[4][64];
  static int nBuf = 0;
  unsigned char *ipByte = (unsigned char *)&unIP;
  _snprintf_s(s[nBuf], sizeof(s[nBuf]), "%u.%u.%u.%u:%i", (int)(ipByte[3]),
...
class gameserveritem_t {
 ...
 public:
  ...
  bool m_bHadSuccessfulResponse;  // server has responded successfully in the
                                  // past
  bool m_bDoNotRefresh;  // server is marked as not responding and should no
                         // longer be refreshed
  ...
  uint32 m_ulTimeLastPlayed;  // time (in unix time) when this server was last
