...
// This header file defines the interface between the calling application and
// the code that knows how to communicate with the connection manager (CM) from
// the Steam service

// This header file is intended to be portable; ideally this 1 header file plus
// a lib or dll is all you need to integrate the client library into some other
// tree.  So please avoid including or requiring other header files if possible.
// This header should only describe the interface layer, no need to include
...
enum EAccountType {
  ...
  k_EAccountTypeP2PSuperSeeder = 9,  // a fake steamid used by superpeers to
...
enum EChatEntryType {
  ...
  k_EChatEntryTypeInviteGame =
      3,  // DEPRECATED Invite from other user into that users current game
  k_EChatEntryTypeEmote = 4,  // text emote message
  // Above are previous FriendMsgType entries, now merged into more generic chat
  // entry types
};
...
enum EChatRoomEnterResponse {
  ...
  k_EChatRoomEnterResponseNotAllowed = 3,  // General Denied - You don't have
                                           // the permissions needed to join the
                                           // chat
...
typedef void (*PFNLegacyKeyRegistration)(const char *pchCDKey,
                                         const char *pchInstallPath);
...
class CSteamID {
 public:
  ...
  // eAccountType -	Type of account
  //-----------------------------------------------------------------------------
  CSteamID(uint32_t unAccountID, unsigned int unAccountInstance,
    ...
    Assert(!((k_EAccountTypeIndividual == eAccountType) &&
             (1 != unAccountInstance)));  // enforce that for individual
                                          // accounts, instance is always 1
#endif                                    // _SERVER
  ...
  //-----------------------------------------------------------------------------
  void FullSet(uint64_t ulIdentifier, EUniverse eUniverse,
               EAccountType eAccountType) {
    m_unAccountID = (ulIdentifier & 0xFFFFFFFF);  // account ID is low 32 bits
    m_unAccountInstance =
        ((ulIdentifier >> 32) & 0xFFFFF);  // account instance is next 20 bits
    m_EUniverse = eUniverse;
    m_EAccountType = eAccountType;
  }
  ...
  void SetFromUint64(uint64_t ulSteamID) {
    ...
    m_unAccountInstance =
        ((ulSteamID >> 32) & 0xFFFFF);  // account instance is next 20 bits

    m_EAccountType =
        (EAccountType)((ulSteamID >> 52) & 0xF);  // type is next 4 bits
    m_EUniverse =
        (EUniverse)((ulSteamID >> 56) & 0xFF);  // universe is next 8 bits
  }
  ...
  void SetFromSteam2(TSteamGlobalUserID *pTSteamGlobalUserID,
    ...
    m_EAccountType = k_EAccountTypeIndividual;  // Steam 2 accounts always map
                                                // to account type of individual
    m_unAccountInstance =
        1;  // individual accounts always have an account instance ID of 1
  }
  ...
  void ConvertToSteam2(TSteamGlobalUserID *pTSteamGlobalUserID) const {
    // only individual accounts have any meaning in Steam 2, only they can be
    // mapped Assert( m_EAccountType == k_EAccountTypeIndividual );
  ...
  // representation. For multiseat accounts, all instances of that account will
  // have the same static account key, so they can be grouped together by the
  // static account key. Output : 64-bit static account key
  //-----------------------------------------------------------------------------
  ...
  bool BGameServerAccount() const {
    return m_EAccountType == k_EAccountTypeGameServer ||
           m_EAccountType == k_EAccountTypeAnonGameServer;
  }
  ...
  //-----------------------------------------------------------------------------
  bool BClanAccount() const { return m_EAccountType == k_EAccountTypeClan; }

  //-----------------------------------------------------------------------------
  // Purpose: Is this a chat account id?
  //-----------------------------------------------------------------------------
  bool BChatAccount() const { return m_EAccountType == k_EAccountTypeChat; }
  ...
  // simple accessors
  void SetAccountID(uint32_t unAccountID) { m_unAccountID = unAccountID; }
  uint32_t GetAccountID() const { return m_unAccountID; }
  uint32_t GetUnAccountInstance() const { return m_unAccountInstance; }
  ...
  bool IsValid() const {
    return (m_EAccountType != k_EAccountTypeInvalid &&
            m_EUniverse != k_EUniverseInvalid);
  }
  ...
  bool operator==(const CSteamID &val) const {
    return ((val.m_unAccountID == m_unAccountID) &&
            (val.m_unAccountInstance == m_unAccountInstance) &&
            (val.m_EAccountType == m_EAccountType) &&
            (val.m_EUniverse == m_EUniverse));
  }

  bool operator!=(const CSteamID &val) const { return !operator==(val); }
  bool operator<(const CSteamID &val) const {
    return ConvertToUint64() < val.ConvertToUint64();
  }
 ...
 private:
  ...
  // u32AccountID to a CSteamID. If you get a compiler error about an ambiguous
  // constructor/function then it may be because you're passing a 32-bit int to
  // a function that takes a CSteamID. You should explicitly create the SteamID
...
const CSteamID k_steamIDOutofDateGS(0, 0, k_EUniverseInvalid,
                                    k_EAccountTypeInvalid);
// This steamID comes from a user game connection to an sv_lan GS
const CSteamID k_steamIDLanModeGS(0, 0, k_EUniversePublic,
                                  k_EAccountTypeInvalid);
...
// logging on.
const CSteamID k_steamIDNotInitYetGS(1, 0, k_EUniverseInvalid,
                                     k_EAccountTypeInvalid);
...
class CGameID {
 public:
  ...
  explicit CGameID(uint64_t ulGameID) { m_ulGameID = ulGameID; }
  ...
  uint64_t *GetUint64Ptr() { return &m_ulGameID; }

  bool IsMod() const { return (m_gameID.m_nType == k_EGameIDTypeGameMod); }

  bool IsShortcut() const {
    return (m_gameID.m_nType == k_EGameIDTypeShortcut);
  }

  bool IsP2PFile() const { return (m_gameID.m_nType == k_EGameIDTypeP2P); }

  bool IsSteamApp() const { return (m_gameID.m_nType == k_EGameIDTypeApp); }

  uint32_t ModID() const { return m_gameID.m_nModID; }

  uint32_t AppID() const { return m_gameID.m_nAppID; }

  bool operator==(const CGameID &rhs) const {
    return m_ulGameID == rhs.m_ulGameID;
  }

  bool operator!=(const CGameID &rhs) const { return !(*this == rhs); }

  bool operator<(const CGameID &rhs) const {
    return (m_ulGameID < rhs.m_ulGameID);
  }

  bool IsValid() const { return (m_ulGameID != 0); }

  void Reset() { m_ulGameID = 0; }
...
#define QUERY_PORT_NOT_INITIALIZED \
  0xFFFF  // We haven't asked the GS for this query port's actual value yet.
#define QUERY_PORT_ERROR \
  0xFFFE  // We were unable to get the query port for this server.
