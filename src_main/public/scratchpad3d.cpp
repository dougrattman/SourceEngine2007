...
void CScratchPad3D::CCommand_Point::Write(IFileSystem *pFileSystem,
                                          FileHandle_t fp) {
...
void CScratchPad3D::CCommand_Line::Write(IFileSystem *pFileSystem,
                                         FileHandle_t fp) {
...
void CScratchPad3D::CCommand_Polygon::Read(CFileRead *pFile) {
  ...
  if (count) pFile->Read(&m_Verts[0], sizeof(CSPVert) * count);
}

void CScratchPad3D::CCommand_Polygon::Write(IFileSystem *pFileSystem,
                                            FileHandle_t fp) {
  int count = m_Verts.Size();
  pFileSystem->Write(&count, sizeof(count), fp);

  if (count) pFileSystem->Write(&m_Verts[0], sizeof(CSPVert) * count, fp);
}
...
void CScratchPad3D::CCommand_Matrix::Write(IFileSystem *pFileSystem,
                                           FileHandle_t fp) {
...
void CScratchPad3D::CCommand_RenderState::Write(IFileSystem *pFileSystem,
                                                FileHandle_t fp) {
...
void CScratchPad3D::CCommand_Text::Write(IFileSystem *pFileSystem,
                                         FileHandle_t fp) {
...
CScratchPad3D::CScratchPad3D(char const *pFilename, IFileSystem *pFileSystem,
                             bool bAutoClear) {
  m_pFileSystem = pFileSystem;
  m_pFilename = pFilename;
  m_bAutoFlush = true;

  if (bAutoClear) Clear();  // Clear whatever is in the file..
}
...
void CScratchPad3D::DrawRectGeneric(int iPlane, int otherDim1, int otherDim2,
                                    float planeDist, const Vector2D &vMin,
                                    const Vector2D &vMax,
                                    const CSPColor &vColor) {
  Vector verts[4];

  verts[0][iPlane] = verts[1][iPlane] = verts[2][iPlane] = verts[3][iPlane] =
      planeDist;
...
void CScratchPad3D::DeleteCommands() {
  for (int i = 0; i < m_Commands.Size(); i++) delete m_Commands[i];
...
void CScratchPad3D::SetMapping(const Vector &vInputMin, const Vector &vInputMax,
  ...
  for (int i = 0; i < 3; i++)
    vDivisor[i] = fabs(vInputMax[i] - vInputMin[i]) < 0.0001f
                      ? 0.001f
                      : (vInputMax[i] - vInputMin[i]);

  Vector vScale = (vOutputMax - vOutputMin) / vDivisor;
  Vector vShift = -vInputMin * vScale + vOutputMin;

  cmd->m_mMatrix.Init(vScale.x, 0, 0, vShift.x, 0, vScale.y, 0, vShift.y, 0, 0,
                      vScale.z, vShift.z, 0, 0, 0, 1);

  AutoFlush();
}

bool CScratchPad3D::GetAutoFlush() { return m_bAutoFlush; }
...
void CScratchPad3D::DrawRectYZ(float xPos, const Vector2D &vMin,
                               const Vector2D &vMax, const CSPColor &vColor) {
  DrawRectGeneric(0, 1, 2, xPos, vMin, vMax, vColor);
}

void CScratchPad3D::DrawRectXZ(float yPos, const Vector2D &vMin,
                               const Vector2D &vMax, const CSPColor &vColor) {
  DrawRectGeneric(1, 0, 2, yPos, vMin, vMax, vColor);
}

void CScratchPad3D::DrawRectXY(float zPos, const Vector2D &vMin,
                               const Vector2D &vMax, const CSPColor &vColor) {
...
void CScratchPad3D::DrawWireframeBox(const Vector &vMin, const Vector &vMax,
                                     const Vector &vColor) {
  // Bottom 4.
  DrawLine(CSPVert(Vector(vMin.x, vMin.y, vMin.z), vColor),
           CSPVert(Vector(vMax.x, vMin.y, vMin.z), vColor));

  DrawLine(CSPVert(Vector(vMin.x, vMin.y, vMin.z), vColor),
           CSPVert(Vector(vMin.x, vMax.y, vMin.z), vColor));

  DrawLine(CSPVert(Vector(vMax.x, vMin.y, vMin.z), vColor),
           CSPVert(Vector(vMax.x, vMax.y, vMin.z), vColor));

  DrawLine(CSPVert(Vector(vMax.x, vMax.y, vMin.z), vColor),
           CSPVert(Vector(vMin.x, vMax.y, vMin.z), vColor));

  // Top 4.
  DrawLine(CSPVert(Vector(vMin.x, vMin.y, vMax.z), vColor),
           CSPVert(Vector(vMax.x, vMin.y, vMax.z), vColor));

  DrawLine(CSPVert(Vector(vMin.x, vMin.y, vMax.z), vColor),
           CSPVert(Vector(vMin.x, vMax.y, vMax.z), vColor));

  DrawLine(CSPVert(Vector(vMax.x, vMin.y, vMax.z), vColor),
           CSPVert(Vector(vMax.x, vMax.y, vMax.z), vColor));

  DrawLine(CSPVert(Vector(vMax.x, vMax.y, vMax.z), vColor),
           CSPVert(Vector(vMin.x, vMax.y, vMax.z), vColor));
  ...
  DrawLine(CSPVert(Vector(vMin.x, vMax.y, vMin.z), vColor),
           CSPVert(Vector(vMin.x, vMax.y, vMax.z), vColor));
...
void CScratchPad3D::Flush() {
  ...
  for (int i = 0; i < m_Commands.Size(); i++) {
    m_pFileSystem->Write(&m_Commands[i]->m_iCommand,
                         sizeof(m_Commands[i]->m_iCommand), fp);
...
void CScratchPad3D::DrawImageBW(unsigned char const *pData, int width,
                                int height, int pitchInBytes,
                                bool bOutlinePixels, bool bOutlineImage,
                                Vector *vCorners) {
  ...
  DrawImageRGBA(pRGBA, width, height, width * sizeof(SPRGBA), bOutlinePixels,
                bOutlineImage, vCorners);
  delete[] pRGBA;
}

void CScratchPad3D::DrawPolygonsForPixels(SPRGBA *pData, int width, int height,
                                          int pitchInBytes, Vector *vCorners) {
  // Scan top-down.
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      if (pData)
        DrawPolygon(CSPVertList(
            vPolyBox, 4,
            Vector(pSrc->r / 255.1f, pSrc->g / 255.1f, pSrc->b / 255.1f)));
      else
...
void CScratchPad3D::DrawImageRGBA(SPRGBA *pData, int width, int height,
                                  int pitchInBytes, bool bOutlinePixels,
                                  bool bOutlineImage, Vector *vCorners) {
  ...
  // Draw wireframe.
  if (bOutlinePixels) {
    SetRenderState(IScratchPad3D::RS_FillMode,
                   IScratchPad3D::FillMode_Wireframe);
...
IFileSystem *ScratchPad3D_SetupFileSystem() {
  ...
  if (!fn || (pFileSystem = (IFileSystem *)fn(FILESYSTEM_INTERFACE_VERSION,
                                              NULL)) == NULL) {
