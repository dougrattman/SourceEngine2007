...
#define SOFTWARE_VERTEX_SHADER(name)                                 \
  static void SoftwareVertexShader_##name(CMeshBuilder &meshBuilder, \
                                          IMaterialVar **params,     \
...
#define SHADER_INIT_PARAMS()                             \
  virtual void OnInitShaderParams(IMaterialVar **params, \
...
#define CLEAR_FLAGS(_flag) \
  params[FLAGS]->SetIntValue(params[FLAGS]->GetIntValue() & ~(_flag))
#define IS_FLAG_SET(_flag) CShader_IsFlagSet(params, _flag)
#define IS_FLAG_DEFINED(_flag) \
  ((params[FLAGS_DEFINED]->GetIntValue() & (_flag)) != 0)

#define IS_PARAM_DEFINED(_param) \
  ((((_param) >= 0) && (params[(_param)]->IsDefined())))
...
// Typesafe flag setting
inline void CShader_SetFlags2(IMaterialVar **params,
                              MaterialVarFlags2_t _flag) {
  params[FLAGS2]->SetIntValue(params[FLAGS2]->GetIntValue() | (_flag));
}

inline bool CShader_IsFlag2Set(IMaterialVar **params,
                               MaterialVarFlags2_t _flag) {
  return ((params[FLAGS2]->GetIntValue() & (_flag)) != 0);
}

#define SET_FLAGS2(_flag) CShader_SetFlags2(params, _flag)
#define CLEAR_FLAGS2(_flag) \
  params[FLAGS2]->SetIntValue(params[FLAGS2]->GetIntValue() & ~(_flag))
#define IS_FLAG2_SET(_flag) CShader_IsFlag2Set(params, _flag)
#define IS_FLAG2_DEFINED(_flag) \
  ((params[FLAGS_DEFINED2]->GetIntValue() & (_flag)) != 0)
...
#define __BEGIN_SHADER_INTERNAL(_baseclass, name, help, flags)               \
  ...
  class CShaderParam {                                                       \
   public:                                                                   \
    CShaderParam(ShaderMaterialVars_t var, ShaderParamType_t type,           \
                 const char *pDefaultParam, const char *pHelp, int nFlags) { \
      m_Info.m_pName = "override";                                           \
      m_Info.m_Type = type;                                                  \
      m_Info.m_pDefaultValue = pDefaultParam;                                \
      m_Info.m_pHelp = pHelp;                                                \
      m_Info.m_nFlags = nFlags;                                              \
      AssertMsg(!s_pShaderParamOverrides[var],                               \
                ("Shader parameter override duplicately defined!"));         \
      s_pShaderParamOverrides[var] = this;                                   \
      m_Index = var;                                                         \
    }                                                                        \
    CShaderParam(const char *pName, ShaderParamType_t type,                  \
                 const char *pDefaultParam, const char *pHelp, int nFlags) { \
...
#define BEGIN_SHADER(name, help) \
  __BEGIN_SHADER_INTERNAL(CBaseShader, name, help, 0)
#define BEGIN_SHADER_FLAGS(name, help, flags) \
  __BEGIN_SHADER_INTERNAL(CBaseShader, name, help, flags)
...
#define SHADER_PARAM_OVERRIDE(param, paramtype, paramdefault, paramhelp, \
                              flags)                                     \
  static CShaderParam param((ShaderMaterialVars_t)param, paramtype,      \
                            paramdefault, paramhelp, flags);
...
#define SHADER_INIT                                                          \
  ...
  void OnInitShaderInstance(IMaterialVar **params, IShaderInit *pShaderInit, \
                            const char *pMaterialName)
...
// params, pMaterialName ) #define CHAIN_SHADER_FALLBACK()
// CBaseClass::GetFallbackShader( params ) #define CHAIN_SHADER_INIT()
// CBaseClass::OnInitShaderInstance( params, pShaderInit, pMaterialName )
// #define  CHAIN_SHADER_DRAW()	CBaseClass::OnDrawElements( params,
// pShaderShadow,  pShaderAPI )
...
#define BEGIN_INHERITED_SHADER(_name, _base, _help) \
  BEGIN_INHERITED_SHADER_FLAGS(_name, _base, _help, 0)
#define END_INHERITED_SHADER \
  END_SHADER                 \
  }

// psh ## shader is used here to generate a warning if you don't ever call
// SET_DYNAMIC_PIXEL_SHADER
#define DECLARE_DYNAMIC_PIXEL_SHADER(shader)                 \
  int declaredynpixshader_##shader##_missingcurlybraces = 0; \
  declaredynpixshader_##shader##_missingcurlybraces =        \
      declaredynpixshader_##shader##_missingcurlybraces;     \
  shader##_Dynamic_Index _pshIndex;                          \
  int psh##shader = 0

// vsh ## shader is used here to generate a warning if you don't ever call
...
#define DECLARE_DYNAMIC_VERTEX_SHADER(shader)                 \
  ...
  declaredynvertshader_##shader##_missingcurlybraces =        \
      declaredynvertshader_##shader##_missingcurlybraces;     \
...
#define DECLARE_STATIC_PIXEL_SHADER(shader)                     \
  ...
  declarestaticpixshader_##shader##_missingcurlybraces =        \
      declarestaticpixshader_##shader##_missingcurlybraces;     \
...
#define DECLARE_STATIC_VERTEX_SHADER(shader)                     \
  ...
  declarestaticvertshader_##shader##_missingcurlybraces =        \
      declarestaticvertshader_##shader##_missingcurlybraces;     \
...
// SET_DYNAMIC_PIXEL_SHADER_COMBO so that an error is generated if they aren't
// set. psh_testAllCombos is set to itself to avoid an unused variable warning.
...
#define SET_DYNAMIC_PIXEL_SHADER(shader)                  \
  ...
  dynamicpixshader_##shader##_missingcurlybraces =        \
      dynamicpixshader_##shader##_missingcurlybraces;     \
...
// SET_DYNAMIC_VERTEX_SHADER_COMBO so that an error is generated if they aren't
// set. vsh_testAllCombos is set to itself to avoid an unused variable warning.
...
#define SET_DYNAMIC_VERTEX_SHADER(shader)                  \
  ...
  dynamicvertshader_##shader##_missingcurlybraces =        \
      dynamicvertshader_##shader##_missingcurlybraces;     \
...
// SET_STATIC_PIXEL_SHADER_COMBO so that an error is generated if they aren't
// set. psh_testAllCombos is set to itself to avoid an unused variable warning.
...
#define SET_STATIC_PIXEL_SHADER(shader)                  \
  ...
  staticpixshader_##shader##_missingcurlybraces =        \
      staticpixshader_##shader##_missingcurlybraces;     \
...
// SET_STATIC_VERTEX_SHADER_COMBO so that an error is generated if they aren't
// set. vsh_testAllCombos is set to itself to avoid an unused variable warning.
...
#define SET_STATIC_VERTEX_SHADER(shader)                  \
  ...
  staticvertshader_##shader##_missingcurlybraces =        \
      staticvertshader_##shader##_missingcurlybraces;     \
