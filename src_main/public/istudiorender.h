...
enum MaterialVertexFormat_t {
  MATERIAL_VERTEX_FORMAT_MODEL_SKINNED =
      (VertexFormat_t)VERTEX_POSITION | VERTEX_COLOR | VERTEX_NORMAL |
      VERTEX_TEXCOORD0_2D | VERTEX_BONEWEIGHT(2) | VERTEX_BONE_INDEX |
      VERTEX_USERDATA_SIZE(4),
  MATERIAL_VERTEX_FORMAT_MODEL_SKINNED_DX7 =
      (VertexFormat_t)VERTEX_POSITION | VERTEX_COLOR | VERTEX_NORMAL |
      VERTEX_TEXCOORD0_2D | VERTEX_BONEWEIGHT(2) | VERTEX_BONE_INDEX,
  MATERIAL_VERTEX_FORMAT_MODEL = (VertexFormat_t)VERTEX_POSITION |
                                 VERTEX_COLOR | VERTEX_NORMAL |
                                 VERTEX_TEXCOORD0_2D | VERTEX_USERDATA_SIZE(4),
...
struct ColorMeshInfo_t {
  ...
  // (in which case it uses a sub-range defined by m_nVertOffset and
  // m_nNumVerts)
  IMesh *m_pMesh;
...
struct DrawModelInfo_t {
  ...
  Vector
      m_vecAmbientCube[6];  // ambient, and lights that aren't in locallight[]
...
// implementation can either statically persist data (tools) or lru cache
// (engine) it. caller returns base pointer to resident data. code expectes data
// to be dynamic and invokes cache callback prior to iterative access.
// virtualModel is member passed in via studiohdr_t and passed back for model
// identification.
//-----------------------------------------------------------------------------
...
the_interface IStudioRender : public IAppSystem {
 public:
  ...
  virtual StudioDecalHandle_t CreateDecalList(studiohwdata_t *
                                              pHardwareData) = 0;
