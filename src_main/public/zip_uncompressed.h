...
struct ZIP_PreloadHeader {
  ...
  unsigned int PreloadDirectoryEntries;  // Number of preloaded directory
...
struct ZIP_PreloadDirectoryEntry {
  ...
  unsigned int DataOffset;  // Offset the file data in the .zip, relative to the
                            // logical beginning of the preload file.
};

struct ZIP_PreloadRemapTable {
  DECLARE_BYTESWAP_DATADESC();
  unsigned short PreloadIndex;  // Index into preload directory, entry marked
