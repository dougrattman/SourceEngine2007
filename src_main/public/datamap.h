...
#define DEFINE_INPUTFUNC(fieldtype, inputname, inputfunc)                     \
  {                                                                           \
    fieldtype, #inputfunc, {NULL, NULL}, 1, FTYPEDESC_INPUT, inputname, NULL, \
...
#define FTYPEDESC_OUTPUT \
  0x0010  // This field propogates it's value to all targets whenever it changes
...
#define FTYPEDESC_PRIVATE \
  0x0200  // The field is local to the client or server only (not referenced by
...
#define FTYPEDESC_VIEW_OTHER_PLAYER \
  0x2000  // By default you can only view fields on the local player (yourself),
//   but if this is set, then we allow you to see fields on other players
#define FTYPEDESC_VIEW_OWN_TEAM \
  0x4000  // Only show this data if the player is on the same team as the local
          // player
#define FTYPEDESC_VIEW_NEVER \
  0x8000  // Never show this field to anyone, even the local player (unusual)

#define TD_MSECTOLERANCE \
  0.001f  // This is a FIELD_FLOAT and should only be checked to be within 0.001
...
#define BEGIN_DATADESC_GUTS(className)                                        \
  ...
  namespace className##_DataDescInit {                                        \
    datamap_t *g_DataMapHolder = DataMapInit(                                 \
        (className *)                                                         \
  ...
  datamap_t *DataMapInit<className>(className *) {                            \
    ...
    static typedescription_t dataDesc[] = {                                   \
      {FIELD_VOID, 0, {0, 0}, 0, 0, 0, 0, 0, 0}, /* so you can define "empty" \
                                                    tables */
...
#define END_DATADESC()                                                       \
  ...
  ;                                                                          \
                                                                             \
  MSVC_SCOPED_DISABLE_WARNING(4127,                                          \
...
class CDatadescGeneratedNameHolder {
 public:
  ...
  const char *GenerateName(const char *pszIdentifier) {
    size_t len = m_nLenBase + strlen(pszIdentifier) + 1;
    char *pBuf = new char[len];
    strcpy_s(pBuf, len, m_pszBase);
    strcat_s(pBuf, len, pszIdentifier);
