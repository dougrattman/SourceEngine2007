...
enum ShakeCommand_t {
  SHAKE_START =
      0,       // Starts the screen shake for all players within the radius.
  SHAKE_STOP,  // Stops the screen shake for all players within the radius.
  SHAKE_AMPLITUDE,  // Modifies the amplitude of an active screen shake for all
                    // players within the radius.
  SHAKE_FREQUENCY,  // Modifies the frequency of an active screen shake for all
                    // players within the radius.
  SHAKE_START_RUMBLEONLY,  // Starts a shake effect that only rumbles the
                           // controller, no screen effect.
...
#define FFADE_STAYOUT \
  0x0008  // ignores the duration, stays faded out until new ScreenFade message
          // received
#define FFADE_PURGE \
  0x0010  // Purges all other fades, replacing them with this one
