...
bool CalcBarycentricCooefs(Vector const &v0, Vector const &v1, Vector const &v2,
  ...
  if (FloatMakePositive(1.0f - cTotal) < 1e-3) return true;

  return false;
}

// For some reason, the global optimizer screws up the recursion here.  disable
// the global optimizations to fix this. IN VC++ 6.0
...
void CCoreDispSurface::SetNeighborData(
    const CDispNeighbor edgeNeighbors[4],
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCoreDispSurface::GeneratePointStartIndexFromMappingAxes(
    Vector const &sAxis, Vector const &tAxis) {
...
// Input  :  -
//-----------------------------------------------------------------------------
bool CCoreDispSurface::CalcLuxelCoords(int nLuxels, bool bAdjust,
                                       const Vector &vecU, const Vector &vecV) {
  // Valid value?
  ...
  float flULength =
      (m_Points[(3 + iOffset) % 4] - m_Points[(0 + iOffset) % 4]).Length();
  flLengthTemp =
      (m_Points[(2 + iOffset) % 4] - m_Points[(1 + iOffset) % 4]).Length();
  ...
  float flVLength =
      (m_Points[(1 + iOffset) % 4] - m_Points[(0 + iOffset) % 4]).Length();
  flLengthTemp =
      (m_Points[(2 + iOffset) % 4] - m_Points[(3 + iOffset) % 4]).Length();
  ...
  float flUValue =
      static_cast<float>(static_cast<int>(flULength * flOOLuxelScale) + 1);
  if (flUValue > MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER) {
    flUValue = MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER;
  }

  float flVValue =
      static_cast<float>(static_cast<int>(flVLength * flOOLuxelScale) + 1);
  ...
  for (int iBump = 0; iBump < NUM_BUMP_VECTS + 1; ++iBump) {
    ...
    m_LuxelCoords[iBump][(2 + iOffset) % 4].Init(flUValue + 0.5,
                                                 flVValue + 0.5);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void GetDispNodeTriVerts(CCoreDispInfo *pDisp, int nodeIndex, int triIndex,
...
void CCoreDispInfo::SetDispUtilsHelperInfo(CCoreDispInfo **ppListBase,
                                           int listSize) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCoreDispInfo::CalcMinMaxBoundingBoxAtNode(int nodeIndex, Vector &bMin,
                                                Vector &bMax) {
...
void CCoreDispInfo::CalcErrorTermAtNode(int nodeIndex, int level) {
  ...
  VectorAdd(m_pVerts[neighborVertIndices[5]].m_Vert,
            m_pVerts[neighborVertIndices[4]].m_Vert, v);
  ...
  VectorAdd(m_pVerts[neighborVertIndices[5]].m_Vert,
            m_pVerts[neighborVertIndices[6]].m_Vert, v);
  VectorScale(v, 0.5f, v);
  ...
  VectorAdd(m_pVerts[neighborVertIndices[6]].m_Vert,
            m_pVerts[neighborVertIndices[7]].m_Vert, v);
  VectorScale(v, 0.5f, v);
  VectorSubtract(m_pVerts[neighborVertIndices[2]].m_Vert, v, segment);
  if (errorTerm < (float)VectorLength(segment))
    errorTerm = (float)VectorLength(segment);

  VectorAdd(m_pVerts[neighborVertIndices[7]].m_Vert,
            m_pVerts[neighborVertIndices[4]].m_Vert, v);
  VectorScale(v, 0.5f, v);
  ...
  VectorAdd(m_pVerts[neighborVertIndices[4]].m_Vert,
            m_pVerts[neighborVertIndices[6]].m_Vert, v);
  VectorScale(v, 0.5f, v);
...
void CCoreDispInfo::CalcNeighborVertIndicesAtNode(int nodeIndex, int level) {
  ...
  for (int direction = 0; direction < 8; direction++) {
    ...
    if (bError) {
      ...
      m_Nodes[nodeIndex].SetNeighborVertIndex(direction,
                                              ((posY * extent) + posX));
    }
  }
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CCoreDispInfo::GetNodeVertIndexFromParentIndex(int level,
                                                   int parentVertIndex,
                                                   int direction) {
  // calculate the "shift"
...
void CCoreDispInfo::CalcVertIndicesAtNodes(int nodeIndex) {
  ...
  for (i = 0, j = 4; i < 4; i++, j++) {
    ...
    int centerIndex = GetNodeVertIndexFromParentIndex(
        level, m_Nodes[nodeIndex].GetCenterVertIndex(), j);
  ...
  for (i = 0; i < postSpacing; i++) {
    ...
    for (int j = 0; j < postSpacing; j++) {
      ...
      if (!bLightMap) {
        ...
        Vector2DAdd(endPts[0], seg,
                    m_pVerts[i * postSpacing + j].m_LuxelCoords[lightmapID]);
      }
    }
  }
}
...
void CCoreDispInfo::GenerateDispSurfTangentSpaces(void) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    CrossProduct(m_pVerts[i].m_Normal, m_pVerts[i].m_TangentT,
                 m_pVerts[i].m_TangentS);
    VectorNormalize(m_pVerts[i].m_TangentS);
    CrossProduct(m_pVerts[i].m_TangentS, m_pVerts[i].m_Normal,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCoreDispInfo::CalcNormalFromEdges(int indexRow, int indexCol,
                                        bool bIsEdge[4], Vector &normal) {
  ...
  if (bIsEdge[1] && bIsEdge[2]) {
    // tri i
    VectorSubtract(m_pVerts[(indexCol + 1) * postSpacing + indexRow].m_Vert,
                   m_pVerts[indexCol * postSpacing + indexRow].m_Vert,
                   tmpVect[0]);
    VectorSubtract(m_pVerts[indexCol * postSpacing + (indexRow + 1)].m_Vert,
                   m_pVerts[indexCol * postSpacing + indexRow].m_Vert,
                   tmpVect[1]);
    ...
    // tri 2
    VectorSubtract(m_pVerts[(indexCol + 1) * postSpacing + indexRow].m_Vert,
                   m_pVerts[indexCol * postSpacing + (indexRow + 1)].m_Vert,
                   tmpVect[0]);
    VectorSubtract(
        m_pVerts[(indexCol + 1) * postSpacing + (indexRow + 1)].m_Vert,
        m_pVerts[indexCol * postSpacing + (indexRow + 1)].m_Vert, tmpVect[1]);
    CrossProduct(tmpVect[1], tmpVect[0], tmpNormal);
    VectorNormalize(tmpNormal);
    VectorAdd(accumNormal, tmpNormal, accumNormal);
    normalCount++;
  ...
  if (bIsEdge[0] && bIsEdge[1]) {
    ...
    VectorSubtract(
        m_pVerts[(indexCol + 1) * postSpacing + (indexRow - 1)].m_Vert,
        m_pVerts[indexCol * postSpacing + (indexRow - 1)].m_Vert, tmpVect[0]);
    VectorSubtract(m_pVerts[indexCol * postSpacing + indexRow].m_Vert,
                   m_pVerts[indexCol * postSpacing + (indexRow - 1)].m_Vert,
                   tmpVect[1]);
    CrossProduct(tmpVect[1], tmpVect[0], tmpNormal);
    VectorNormalize(tmpNormal);
    ...
    VectorSubtract(
        m_pVerts[(indexCol + 1) * postSpacing + (indexRow - 1)].m_Vert,
        m_pVerts[indexCol * postSpacing + indexRow].m_Vert, tmpVect[0]);
    VectorSubtract(m_pVerts[(indexCol + 1) * postSpacing + indexRow].m_Vert,
                   m_pVerts[indexCol * postSpacing + indexRow].m_Vert,
                   tmpVect[1]);
    CrossProduct(tmpVect[1], tmpVect[0], tmpNormal);
  ...
  if (bIsEdge[0] && bIsEdge[3]) {
    ...
    VectorSubtract(
        ...
        tmpVect[0]);
    VectorSubtract(
        m_pVerts[(indexCol - 1) * postSpacing + indexRow].m_Vert,
        m_pVerts[(indexCol - 1) * postSpacing + (indexRow - 1)].m_Vert,
        tmpVect[1]);
    CrossProduct(tmpVect[1], tmpVect[0], tmpNormal);
    VectorNormalize(tmpNormal);
  ...
  if (bIsEdge[2] && bIsEdge[3]) {
    ...
    VectorSubtract(
        m_pVerts[(indexCol - 1) * postSpacing + (indexRow + 1)].m_Vert,
        m_pVerts[(indexCol - 1) * postSpacing + indexRow].m_Vert, tmpVect[1]);
    CrossProduct(tmpVect[1], tmpVect[0], tmpNormal);
    VectorNormalize(tmpNormal);
    VectorAdd(accumNormal, tmpNormal, accumNormal);
    normalCount++;
...
//-----------------------------------------------------------------------------
bool CCoreDispInfo::DoesEdgeExist(int indexRow, int indexCol, int direction,
                                  int postSpacing) {
  switch (direction) {
    case 0:
      // left edge
...
void CCoreDispInfo::GenerateDispSurf(void) {
  ...
  //
  // get the spacing (interval = width/height, are equal because it is uniform)
  // along the edge
  //
  int postSpacing = GetPostSpacing();
  float ooInt = 1.0f / (float)(postSpacing - 1);
...
//   Input: power - the size in one dimension of the displacement map (2^power +
//   1 )
//          index - the "base" node index
//          direction - the direction of the neighbor { W = 1, N = 2, E = 3, S =
//          4 }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int GetNodeNeighborNodeFromNeighborSurf(int power, int index, int direction,
                                        int level, int neighborOrient) {
  // adjust the index to range [0...?]
  int minNodeIndex = GetNodeMinNodeAtLevel(level);
  ...
  switch (direction) {
    case CCoreDispInfo::WEST: {
      switch (neighborOrient) {
        case CCoreDispInfo::WEST:
          return -(
              (GetNodeIndexFromComponents(posX, ((nodeExtent - 1) - posY))) +
              minNodeIndex);
        case CCoreDispInfo::NORTH:
          return -((GetNodeIndexFromComponents((nodeExtent - 1) - posY,
                                               (nodeExtent - 1))) +
                   minNodeIndex);
        case CCoreDispInfo::EAST:
          return -((GetNodeIndexFromComponents((nodeExtent - 1), posY)) +
    ...
    case CCoreDispInfo::NORTH: {
      switch (neighborOrient) {
        ...
        case CCoreDispInfo::NORTH:
          return -(
              (GetNodeIndexFromComponents(((nodeExtent - 1) - posX), posY)) +
              minNodeIndex);
        case CCoreDispInfo::EAST:
          return -((GetNodeIndexFromComponents(posY, posX)) + minNodeIndex);
        case CCoreDispInfo::SOUTH:
    ...
    case CCoreDispInfo::SOUTH: {
      switch (neighborOrient) {
        case CCoreDispInfo::WEST:
          return -((GetNodeIndexFromComponents(posY, posX)) + minNodeIndex);
        case CCoreDispInfo::NORTH:
          return -((GetNodeIndexFromComponents(posX, (nodeExtent - 1))) +
                   minNodeIndex);
        case CCoreDispInfo::EAST:
          return -((GetNodeIndexFromComponents((nodeExtent - 1),
                                               ((nodeExtent - 1) - posX))) +
                   minNodeIndex);
        case CCoreDispInfo::SOUTH:
          return -(
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CCoreDispInfo::BaseFacePlaneToDispUV(Vector const &planePt,
                                          Vector2D &dispUV) {
  ...
  PointInQuadToBarycentric(vecPoints[0], vecPoints[3], vecPoints[2],
                           vecPoints[1], planePt, dispUV);
}
...
void CCoreDispInfo::DispUVToSurf_TriTLToBR_1(const Vector &vecIntersectPoint,
                                             int nSnapU, int nNextU, int nSnapV,
                                             int nNextV, Vector &vecPoint,
                                             Vector *pNormal, float *pAlpha,
                                             bool bBackup) {
  int nWidth = GetWidth();
  ...
  if (nSnapU == nNextU) {
    ...
    float flCfs[3];
    if (CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1], vecFlatVerts[2],
                              vecIntersectPoint, flCfs[0], flCfs[1],
                              flCfs[2])) {
      vecPoint = (vecVerts[0] * flCfs[0]) + (vecVerts[1] * flCfs[1]) +
                 (vecVerts[2] * flCfs[2]);

      if (pAlpha) {
        *pAlpha = (flAlphas[0] * flCfs[0]) + (flAlphas[1] * flCfs[1]) +
                  (flAlphas[2] * flCfs[2]);
      }
      ...
      if (!bBackup) {
        DispUVToSurf_TriTLToBR_2(vecIntersectPoint, nSnapU, nNextU, nSnapV,
                                 nNextV, vecPoint, pNormal, pAlpha, true);
      }
    }
  }
}
...
void CCoreDispInfo::DispUVToSurf_TriTLToBR_2(const Vector &vecIntersectPoint,
  ...
  if (nSnapU == nNextU) {
    ...
    if (CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1], vecFlatVerts[2],
      ...
      if (!bBackup) {
        DispUVToSurf_TriTLToBR_1(vecIntersectPoint, nSnapU, nNextU, nSnapV,
                                 nNextV, vecPoint, pNormal, pAlpha, true);
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CCoreDispInfo::DispUVToSurf_TriTLToBR(Vector &vecPoint, Vector *pNormal,
  ...
  if ((flFracU + flFracV) >= (1.0f + TRIEDGE_EPSILON)) {
    DispUVToSurf_TriTLToBR_1(vecIntersectPoint, nSnapU, nNextU, nSnapV, nNextV,
                             vecPoint, pNormal, pAlpha, false);
  } else {
    DispUVToSurf_TriTLToBR_2(vecIntersectPoint, nSnapU, nNextU, nSnapV, nNextV,
                             vecPoint, pNormal, pAlpha, false);
  }
}
...
void CCoreDispInfo::DispUVToSurf_TriBLToTR_1(const Vector &vecIntersectPoint,
  ...
  if (nSnapU == nNextU) {
    ...
    if (CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1], vecFlatVerts[2],
      ...
      if (!bBackup) {
        DispUVToSurf_TriBLToTR_2(vecIntersectPoint, nSnapU, nNextU, nSnapV,
                                 nNextV, vecPoint, pNormal, pAlpha, true);
      }
    }
  }
...
void CCoreDispInfo::DispUVToSurf_TriBLToTR_2(const Vector &vecIntersectPoint,
  ...
  if (nSnapU == nNextU) {
    ...
    if (CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1], vecFlatVerts[2],
      ...
      if (!bBackup) {
        DispUVToSurf_TriBLToTR_1(vecIntersectPoint, nSnapU, nNextU, nSnapV,
                                 nNextV, vecPoint, pNormal, pAlpha, true);
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CCoreDispInfo::DispUVToSurf_TriBLToTR(Vector &vecPoint, Vector *pNormal,
                                           float *pAlpha, float flU, float flV,
                                           const Vector &vecIntersectPoint) {
  int nWidth = GetWidth();
  int nHeight = GetHeight();
  ...
  if (flFracU < flFracV) {
    DispUVToSurf_TriBLToTR_1(vecIntersectPoint, nSnapU, nNextU, nSnapV, nNextV,
                             vecPoint, pNormal, pAlpha, false);
  } else {
    DispUVToSurf_TriBLToTR_2(vecIntersectPoint, nSnapU, nNextU, nSnapV, nNextV,
                             vecPoint, pNormal, pAlpha, false);
  }
}
...
void CCoreDispInfo::DispUVToSurf(Vector2D const &dispUV, Vector &vecPoint,
  ...
  CCoreDispSurface *pSurf = GetSurface();
  PointInQuadFromBarycentric(pSurf->GetPoint(0), pSurf->GetPoint(3),
                             pSurf->GetPoint(2), pSurf->GetPoint(1), dispUV,
                             vecIntersectPoint);
  ...
  // Top Left to Bottom Right
  if (bOdd) {
    DispUVToSurf_TriTLToBR(vecPoint, pNormal, pAlpha, flU, flV,
                           vecIntersectPoint);
  }
  // Bottom Left to Top Right
  else {
    DispUVToSurf_TriBLToTR(vecPoint, pNormal, pAlpha, flU, flV,
                           vecIntersectPoint);
  }
}
...
inline bool PointInDispBBox(CoreDispBBox_t *pBox, const Vector &vecPoint) {
  ...
  if ((vecPoint.x < pBox->vMin.x) || (vecPoint.x > pBox->vMax.x)) return false;

  if ((vecPoint.y < pBox->vMin.y) || (vecPoint.y > pBox->vMax.y)) return false;

  if ((vecPoint.z < pBox->vMin.z) || (vecPoint.z > pBox->vMax.z)) return false;
...
bool CCoreDispInfo::SurfToBaseFacePlane(Vector const &surfPt, Vector &planePt) {
  ...
  for (int iBox = 0; iBox < nBoxCount; ++iBox) {
    ...
    for (int iTri = 0; iTri < 2; ++iTri) {
      ...
      float c[3];
      if (CalcBarycentricCooefs(vecPoints[0], vecPoints[1], vecPoints[2],
                                surfPt, c[0], c[1], c[2])) {
        ...
        planePt = (vecFlatPoints[0] * c[0]) + (vecFlatPoints[1] * c[1]) +
                  (vecFlatPoints[2] * c[2]);
