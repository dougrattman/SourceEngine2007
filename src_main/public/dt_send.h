...
// If the proxy returns 0, it's the same as if pRecipients->ClearAllRecipients()
// was called.
...
class CSendProxyRecipients {
 public:
  void SetAllRecipients();  // Note: recipients are all set by default when each
                            // proxy is called.
  void ClearAllRecipients();

  void SetRecipient(int iClient);  // Note: these are CLIENT indices, not entity
                                   // indices (so the first player's index is
                                   // 0).
...
class SendProp {
 ...
 public:
  ...
  union {
    // If this is an exclude prop, then this is the name of the datatable to
    // exclude a prop from.
 ...
 private:
  ...
  // SENDPROP_VECTORELEM makes this negative to start with so we can detect that
  // and set the SPROP_IS_VECTOR_ELEM flag.
  int m_Offset;
...
class SendTable {
 ...
 protected:
  ...
  bool m_bHasPropsEncodedAgainstCurrentTickCount : 1;  // m_flSimulationTime and
                                                       // m_flAnimTime, e.g.
};
...
#define BEGIN_SEND_TABLE_NOBASE(className, tableName)                \
  ...
  int ServerClassInit<tableName::ignored>(tableName::ignored *) {    \
    ...
    static SendProp g_SendProps[] = {                                \
      SendPropInt("should_never_see_this", 0,                        \
...
#varName, offsetof(currentSendDTClass::MakeANetworkVar_##varName, varName), \
...
inline SendProp SendPropModelIndex(const char *pVarName, int offset,
                                   int sizeofVar = SIZEOF_IGNORE) {
  return SendPropInt((char *)pVarName, offset, sizeofVar, SP_MODEL_INDEX_BITS,
                     0);
}
...
#define SendPropVariableLengthArray(arrayLengthSendProxy, varTemplate,        \
                                    arrayName)                                \
  varTemplate,                                                                \
      InternalSendPropArray(sizeof(((currentSendDTClass *)0)->arrayName) /    \
...
SendProp SendPropExclude(
    const char *pDataTableName,  // Data table name (given to
                                 // BEGIN_SEND_TABLE and
                                 // BEGIN_RECV_TABLE).
