...
// Clears the trace
//-----------------------------------------------------------------------------
static void Collision_ClearTrace(const Vector &vecRayStart,
...
float IntersectRayWithTriangle(const Ray_t &ray, const Vector &v1,
  ...
  if (FloatMakePositive(denom) < 1e-6) return -1.0f;
  denom = 1.0f / denom;
  ...
  if ((v < 0.0f) || (v + u > 1.0f)) return -1.0f;
...
bool ComputeIntersectionBarycentricCoordinates(const Ray_t &ray,
  ...
  //		| -Dz E1z E2z |
  float denom = DotProduct(dirCrossEdge2, edge1);
  if (FloatMakePositive(denom) < 1e-6) return false;
  denom = 1.0f / denom;
  ...
  if (t) {
    ...
    *t = DotProduct(orgCrossEdge1, edge2) * denom;
    if ((*t < -boxt) || (*t > 1.0f + boxt)) return false;
  }
...
bool IsRayIntersectingSphere(const Vector &vecRayOrigin,
                             const Vector &vecRayDelta, const Vector &vecCenter,
                             float flRadius, float flTolerance) {
  // For this algorithm, find a point on the ray  which is closest to the sphere
  // origin Do this by making a plane passing through the sphere origin whose
  // normal is parallel to the ray. Intersect that plane with the ray. Plane: N
  // dot P = I, N = D (ray direction), I = C dot N = C dot D Ray: P = O + D * t
  // D dot ( O + D * t ) = C dot D
  ...
  // NOTE: This in an alternate algorithm which I didn't use because I'd have to
  // use a sqrt So it's probably faster to do this other algorithm. I'll leave
  // the comments here for how to go back if we want to
  ...
  // c = DotProduct(vecRayOrigin - vecCenter, vecRayOrigin - vecCenter) -
  // flRadius * flRadius; Valid solutions are possible only if b^2 - 4ac >= 0
...
bool IntersectInfiniteRayWithSphere(const Vector &vecRayOrigin,
  ...
  if (flDiscrim < 0.0f) return false;
...
bool IntersectRayWithSphere(const Vector &vecRayOrigin,
                            ...
                            float *pT1, float *pT2) {
  if (!IntersectInfiniteRayWithSphere(vecRayOrigin, vecRayDelta,
                                      vecSphereCenter, flRadius, pT1, pT2))
    return false;

  if ((*pT1 > 1.0f) || (*pT2 < 0.0f)) return false;

  // Clamp it!
  if (*pT1 < 0.0f) *pT1 = 0.0f;
  if (*pT2 > 1.0f) *pT2 = 1.0f;
...
//-----------------------------------------------------------------------------
bool IsPointInBox(const Vector &pt, const Vector &boxMin,
                  const Vector &boxMax) {
  Assert(boxMin[0] <= boxMax[0]);
  Assert(boxMin[1] <= boxMax[1]);
  Assert(boxMin[2] <= boxMax[2]);

  if ((pt[0] > boxMax[0]) || (pt[0] < boxMin[0])) return false;
  if ((pt[1] > boxMax[1]) || (pt[1] < boxMin[1])) return false;
  if ((pt[2] > boxMax[2]) || (pt[2] < boxMin[2])) return false;
  return true;
}
...
bool IsPointInCone(const Vector &pt, const Vector &origin, const Vector &axis,
  ...
  if (dot < cosAngle) return false;
  if (dist * dot > length) return false;
...
bool IsBoxIntersectingBox(const Vector &boxMin1, const Vector &boxMax1,
  ...
  if ((boxMin1[0] > boxMax2[0]) || (boxMax1[0] < boxMin2[0])) return false;
  if ((boxMin1[1] > boxMax2[1]) || (boxMax1[1] < boxMin2[1])) return false;
  if ((boxMin1[2] > boxMax2[2]) || (boxMax1[2] < boxMin2[2])) return false;
  return true;
}
...
bool IsOBBIntersectingOBB(const Vector &vecOrigin1, const QAngle &vecAngles1,
  ...
  /*if ( vecAngles1 == vecAngles2 )
  {
          ...
          return IsBoxIntersectingBox( boxMin1, boxMax1, vecOtherMins,
  vecOtherMaxs );
  }*/

  // OBB test...
  cplane_t plane;
  bool bFoundPlane = ComputeSeparatingPlane(
      vecOrigin1, vecAngles1, boxMin1, boxMax1, vecOrigin2, vecAngles2, boxMin2,
      boxMax2, flTolerance, &plane);
...
bool FASTCALL IsBoxIntersectingRay(const Vector &boxMin, const Vector &boxMax,
  ...
  // Check to see if both the origin (start point) and the end point (delta) are
  // on the front side of any of the box sides - if so there can be no
  // intersection
  ...
  if (IsAnyNegative(SetWToZeroSIMD(OrSIMD(minsMask, maxsMask)))) return false;
  ...
  fltx4 crossPlane = OrSIMD(XorSIMD(startOutMins, endOutMins),
                            XorSIMD(startOutMaxs, endOutMaxs));
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (FloatMakePositive(vecDelta[i]) < 1e-8) {
      ...
      if ((origin[i] < boxMin[i] - flTolerance) ||
          (origin[i] > boxMax[i] + flTolerance))
        return false;
...
bool FASTCALL IsBoxIntersectingRay(const Vector &vecBoxMin,
  ...
  return IsBoxIntersectingRay(vecExpandedBoxMin, vecExpandedBoxMax, ray.m_Start,
                              ray.m_Delta, flTolerance);
}
...
bool FASTCALL IsBoxIntersectingRay(
  ...
  // Check to see if the origin (start point) and the end point (delta) are on
  // the same side of any of the box sides - if so there can be no intersection
  fltx4 startOutMins =
      AndSIMD(CmpLtSIMD(Four_Zeros, boxMin), CmpLtSIMD(delta, boxMin));
  fltx4 startOutMaxs =
      AndSIMD(CmpGtSIMD(Four_Zeros, boxMax), CmpGtSIMD(delta, boxMax));
...
bool FASTCALL IsBoxIntersectingRay(const fltx4 &boxMin, const fltx4 &boxMax,
  ...
  if (!ray.m_IsSwept) {
    fltx4 rayMins = SubSIMD(rayStart, rayExtents),
          ...
          vTolerance = ReplicateX4(flTolerance);
  ...
  return IsBoxIntersectingRay(vecExpandedBoxMin, vecExpandedBoxMax, rayStart,
                              rayDelta, ReciprocalSIMD(rayDelta),
...
bool IntersectRayWithRay(const Ray_t &ray0, const Ray_t &ray1, float &t,
  ...
  i0 += ray0.m_Start;
  i1 += ray1.m_Start;
  if (i0.x == i1.x && i0.y == i1.y && i0.z == i1.z) return true;
...
bool IntersectRayWithBox(const Vector &vecRayStart, const Vector &vecRayDelta,
  ...
  for (i = 0; i < 6; ++i) {
    ...
    if (d1 > 0 && d2 > 0) {
      ...
      // UNDONE: Refactor to have only 2 return points (true/false) from this
      // function
      pTrace->startsolid = false;
      return false;
    }
...
bool IntersectRayWithBox(const Vector &vecRayStart, const Vector &vecRayDelta,
  ...
  if (IntersectRayWithBox(vecRayStart, vecRayDelta, boxMins, boxMaxs,
                          flTolerance, &trace)) {
...
bool IntersectRayWithBox(const Ray_t &ray, const Vector &boxMins,
                         const Vector &boxMaxs, float flTolerance,
                         CBaseTrace *pTrace, float *pFractionLeftSolid) {
  if (!ray.m_IsRay) {
    Vector vecExpandedMins = boxMins - ray.m_Extents;
    Vector vecExpandedMaxs = boxMaxs + ray.m_Extents;
    bool bIntersects = IntersectRayWithBox(
        ray.m_Start, ray.m_Delta, vecExpandedMins, vecExpandedMaxs, flTolerance,
        pTrace, pFractionLeftSolid);
    pTrace->startpos += ray.m_StartOffset;
    pTrace->endpos += ray.m_StartOffset;
    return bIntersects;
  }
  return IntersectRayWithBox(ray.m_Start, ray.m_Delta, boxMins, boxMaxs,
                             flTolerance, pTrace, pFractionLeftSolid);
}
...
bool IntersectRayWithOBB(const Vector &vecRayStart, const Vector &vecRayDelta,
  ...
  return IntersectRayWithBox(start, delta, vecOBBMins, vecOBBMaxs, flTolerance,
                             pTrace);
}
...
bool IntersectRayWithOBB(const Vector &vecRayStart, const Vector &vecRayDelta,
  ...
  for (int j = 0; j < 3; j++) {
    extent[j] = vecRayDelta.x * matOBBToWorld[0][j] +
                vecRayDelta.y * matOBBToWorld[1][j] +
                vecRayDelta.z * matOBBToWorld[2][j];
    uextent[j] = fabsf(extent[j]);
    float coord = segmentCenter.x * matOBBToWorld[0][j] +
                  segmentCenter.y * matOBBToWorld[1][j] +
                  segmentCenter.z * matOBBToWorld[2][j];
    coord = fabsf(coord);
  ...
  cextent = cross.x * matOBBToWorld[0][0] + cross.y * matOBBToWorld[1][0] +
            cross.z * matOBBToWorld[2][0];
  cextent = fabsf(cextent);
  tmp = vecBoxExtents[1] * uextent[2] + vecBoxExtents[2] * uextent[1];
  if (cextent > tmp) return false;

  cextent = cross.x * matOBBToWorld[0][1] + cross.y * matOBBToWorld[1][1] +
            cross.z * matOBBToWorld[2][1];
  cextent = fabsf(cextent);
  tmp = vecBoxExtents[0] * uextent[2] + vecBoxExtents[2] * uextent[0];
  if (cextent > tmp) return false;

  cextent = cross.x * matOBBToWorld[0][2] + cross.y * matOBBToWorld[1][2] +
            cross.z * matOBBToWorld[2][2];
  cextent = fabsf(cextent);
  ...
  trace_t boxTrace;
  if (!IntersectRayWithBox(start, extent, vecOBBMins, vecOBBMaxs, flTolerance,
                           pTrace))
    return false;
...
bool IntersectRayWithOBB(const Vector &vecRayOrigin, const Vector &vecRayDelta,
  ...
  if (angBoxRotation == vec3_angle) {
    ...
    return IntersectRayWithBox(vecRayOrigin, vecRayDelta, vecAbsMins,
                               vecAbsMaxs, flTolerance, pTrace);
  }

  matrix3x4_t obbToWorld;
  AngleMatrix(angBoxRotation, vecBoxOrigin, obbToWorld);
  return IntersectRayWithOBB(vecRayOrigin, vecRayDelta, obbToWorld, vecOBBMins,
                             vecOBBMaxs, flTolerance, pTrace);
}

//-----------------------------------------------------------------------------
// Box support map
//-----------------------------------------------------------------------------
inline void ComputeSupportMap(const Vector &vecDirection,
...
bool IntersectRayWithOBB(const Ray_t &ray, const matrix3x4_t &matOBBToWorld,
  ...
  if (!IsRayIntersectingSphere(ray.m_Start, ray.m_Delta, vecOBBCenter, flRadius,
                               flTolerance))
    return false;

  // Ok, we passed the trivial reject, so lets do the dirty deed.
  // Basically we're going to do the GJK thing explicitly. We'll shrink the ray
  // down to a point, and bloat the OBB by the ray's extents. This will generate
  // facet planes which are perpendicular to all of the separating axes
  // typically seen in a standard seperating axis implementation.

  // We're going to create a number of planes through various vertices in the
  // OBB which represent all of the separating planes. Then we're going to bloat
  // the planes by the ray extents.
  ...
  for (i = 0; i < 3; ++i) {
    ...
    // and use the direction of the OBB (the ith column of matOBBToWorld) in
    // world space vs extents
    pPlaneNormal[i].Init();
    ...
    ComputeSupportMap(pPlaneNormal[i + 3], vecOBBMins, vecOBBMaxs,
                      ppPlaneDist[i + 3]);
    ppPlaneDist[i + 3][0] -= ray.m_Extents[i];
    ppPlaneDist[i + 3][1] += ray.m_Extents[i];

    // Now the edge cases... (take the cross product of x,y,z axis w/ ray extent
    // axes given by the rows of the obb to world matrix. Compute the ray extent
    // bloat in world space because it's easier...
    ...
    ComputeSupportMap(pPlaneNormal[i + 6], 1, 2, vecOBBMins, vecOBBMaxs,
                      ppPlaneDist[i + 6]);
    flExtentDotNormal = FloatMakePositive(pMatRow0[0]) * flRayExtent0 +
    ...
    ComputeSupportMap(pPlaneNormal[i + 9], 0, 2, vecOBBMins, vecOBBMaxs,
                      ppPlaneDist[i + 9]);
    flExtentDotNormal = FloatMakePositive(pMatRow0[1]) * flRayExtent0 +
    ...
    ComputeSupportMap(pPlaneNormal[i + 12], 0, 1, vecOBBMins, vecOBBMaxs,
                      ppPlaneDist[i + 12]);
    flExtentDotNormal = FloatMakePositive(pMatRow0[2]) * flRayExtent0 +
...
bool IntersectRayWithOBB(const Ray_t &ray, const Vector &vecBoxOrigin,
  ...
  if (angBoxRotation == vec3_angle) {
    ...
    return IntersectRayWithBox(ray, vecWorldMins, vecWorldMaxs, flTolerance,
                               pTrace);
  }

  if (ray.m_IsRay) {
    return IntersectRayWithOBB(ray.m_Start, ray.m_Delta, vecBoxOrigin,
                               angBoxRotation, vecOBBMins, vecOBBMaxs,
                               flTolerance, pTrace);
  }

  matrix3x4_t matOBBToWorld;
  AngleMatrix(angBoxRotation, vecBoxOrigin, matOBBToWorld);
  return IntersectRayWithOBB(ray, matOBBToWorld, vecOBBMins, vecOBBMaxs,
                             flTolerance, pTrace);
}
...
QuadBarycentricRetval_t QuadWithParallelEdges(const Vector &vecOrigin,
                                              const Vector &vecU, float lengthU,
                                              const Vector &vecV, float lengthV,
                                              const Vector &pt,
                                              Vector2D &vecUV) {
  Ray_t rayAxis;
  Ray_t rayPt;
  ...
  // inside of the quad??
  if ((vecUV[0] < 0.0f) || (vecUV[0] > 1.0f) || (vecUV[1] < 0.0f) ||
      (vecUV[1] > 1.0f))
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void ResolveQuadratic(double tPlus, double tMinus, const Vector &axisU0,
                      const Vector &axisU1, const Vector &axisV0,
                      const Vector &axisV1, const Vector &axisOrigin,
                      const Vector &pt, int projU, double &s, double &t) {
  // calculate the sPlus, sMinus pair(s)
  double sDenomPlus = (axisU0[projU] * (1 - tPlus)) + (axisU1[projU] * tPlus);
  double sDenomMinus =
      (axisU0[projU] * (1 - tMinus)) + (axisU1[projU] * tMinus);

  double sPlus = UNINIT, sMinus = UNINIT;
  if (FloatMakePositive(sDenomPlus) >= 1e-5) {
    sPlus =
        (pt[projU] - axisOrigin[projU] - (axisV0[projU] * tPlus)) / sDenomPlus;
  }

  if (FloatMakePositive(sDenomMinus) >= 1e-5) {
    sMinus = (pt[projU] - axisOrigin[projU] - (axisV0[projU] * tMinus)) /
             sDenomMinus;
  }
  ...
  if ((tMinus >= 0.0) && (tMinus <= 1.0) && (sMinus >= 0.0) &&
      (sMinus <= 1.0)) {
    s = sMinus;
    t = tMinus;
    return;
...
#define PIQ_DOT_EPSILON 0.99f
  ...
  if (axisUNorm[0].Dot(axisUNorm[1]) > PIQ_DOT_EPSILON) {
    if (FloatMakePositive(lengthU[0] - lengthU[1]) < PIQ_PLANE_EPSILON) {
      return QuadWithParallelEdges(v1, axisUNorm[0], lengthU[0], axisVNorm[0],
                                   lengthV[0], point, uv);
    }
  }
  ...
  //
  if (FloatMakePositive(axisU[0][projAxes[0]]) <
      FloatMakePositive(axisU[0][projAxes[1]])) {
    int tmp = projAxes[0];
  ...
  negB = C - (v1[projAxes[1]] * axisU[1][projAxes[0]]) +
  ...
  if ((A > -PIQ_PLANE_EPSILON) && (A < PIQ_PLANE_EPSILON)) {
    // shouldn't be here -- this should have been take care of in the "early
    // out"
    //		Assert( 0 );
    ...
    return QuadWithParallelEdges(v1, vecUAvg, fLengthUAvg, vecVAvg, fLengthVAvg,
                                 point, uv);
  } else {
    // (-b +/- sqrt( b^2 - 4ac )) / 2a
    ...
    ResolveQuadratic(QPlus, QMinus, axisU[0], axisU[1], axisV[0], axisV[1], v1,
                     point, projAxes[0], s, t);
  }
...
static void ComputeCenterIMatrix(const Vector &origin, const QAngle &angles,
  ...
  // is T = R * C + O, (R = rotation matrix, C = centroid in local space, O =
  // origin in world space) The IMatrix translation = - transpose(R) * T = -C -
  // transpose(R) * 0
  Vector localOrigin;
...
//-----------------------------------------------------------------------------
static bool ComputeSeparatingPlane(const matrix3x4_t &worldToBox1,
                                   const matrix3x4_t &box2ToWorld,
                                   const Vector &box1Size,
                                   const Vector &box2Size, float tolerance,
                                   cplane_t *pPlane) {
  ...
  // First side of box 1
  boxProjectionSum =
      box1Size.x + MatrixRowDotProduct(absBox2ToBox1, 0, box2Size);
  ...
  boxProjectionSum =
      box1Size.y + MatrixRowDotProduct(absBox2ToBox1, 1, box2Size);
  ...
  // Third side of box 1
  boxProjectionSum =
      box1Size.z + MatrixRowDotProduct(absBox2ToBox1, 2, box2Size);
  ...
  // First side of box 2
  boxProjectionSum =
      box2Size.x + MatrixColumnDotProduct(absBox2ToBox1, 0, box1Size);
  originProjection =
      FloatMakePositive(MatrixColumnDotProduct(box2ToBox1, 0, box2Origin)) +
      tolerance;
  if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
  ...
  boxProjectionSum =
      box2Size.y + MatrixColumnDotProduct(absBox2ToBox1, 1, box1Size);
  originProjection =
      FloatMakePositive(MatrixColumnDotProduct(box2ToBox1, 1, box2Origin)) +
      tolerance;
  if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
  ...
  // Third side of box 2
  boxProjectionSum =
      box2Size.z + MatrixColumnDotProduct(absBox2ToBox1, 2, box1Size);
  originProjection =
      FloatMakePositive(MatrixColumnDotProduct(box2ToBox1, 2, box2Origin)) +
      tolerance;
  if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
  ...
  // axis aligned; therefore the crossproducts are simplified. Let's walk
  // through the example of b1e1 x b2e1:
  ...
  // The projection of box1 onto this line = the absolute dot product of the box
  // size against the line, which = AbsDot( box1Size, l1 ) = abs( -ez * box1.y )
  // + abs( ey * box1.z )

  // To compute the projection of box2 onto this line, we'll do it in the space
  // of box 2
  //
  // [  i  j  k ]
  ...
  // The projection of box2 onto this line = the absolute dot product of the box
  // size against the line, which = AbsDot( box2Size, l2 ) = abs( fz * box2.y )
  // + abs ( fy * box2.z )
  ...
  if (absBox2ToBox1[0][0] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(-box2Origin.y * box2ToBox1[2][0] +
                                         box2Origin.z * box2ToBox1[1][0]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
  ...
  if (absBox2ToBox1[0][1] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(-box2Origin.y * box2ToBox1[2][1] +
                                         box2Origin.z * box2ToBox1[1][1]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
  ...
  if (absBox2ToBox1[0][2] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(-box2Origin.y * box2ToBox1[2][2] +
                                         box2Origin.z * box2ToBox1[1][2]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
  ...
  if (absBox2ToBox1[1][0] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(box2Origin.x * box2ToBox1[2][0] -
                                         box2Origin.z * box2ToBox1[0][0]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
      MatrixGetColumn(box2ToWorld, 0, tmp);
  ...
  if (absBox2ToBox1[1][1] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(box2Origin.x * box2ToBox1[2][1] -
                                         box2Origin.z * box2ToBox1[0][1]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
      MatrixGetColumn(box2ToWorld, 1, tmp);
      CrossProduct(worldToBox1[1], tmp.Base(), pPlane->normal.Base());
  ...
  if (absBox2ToBox1[1][2] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(box2Origin.x * box2ToBox1[2][2] -
                                         box2Origin.z * box2ToBox1[0][2]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
      MatrixGetColumn(box2ToWorld, 2, tmp);
      CrossProduct(worldToBox1[1], tmp.Base(), pPlane->normal.Base());
  ...
  if (absBox2ToBox1[2][0] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(-box2Origin.x * box2ToBox1[1][0] +
                                         box2Origin.y * box2ToBox1[0][0]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
      MatrixGetColumn(box2ToWorld, 0, tmp);
  ...
  if (absBox2ToBox1[2][1] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(-box2Origin.x * box2ToBox1[1][1] +
                                         box2Origin.y * box2ToBox1[0][1]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
      MatrixGetColumn(box2ToWorld, 1, tmp);
      CrossProduct(worldToBox1[2], tmp.Base(), pPlane->normal.Base());
  ...
  if (absBox2ToBox1[2][2] < 1.0f - 1e-3f) {
    ...
    originProjection = FloatMakePositive(-box2Origin.x * box2ToBox1[1][2] +
                                         box2Origin.y * box2ToBox1[0][2]) +
                       tolerance;
    if (FloatBits(originProjection) > FloatBits(boxProjectionSum)) {
      MatrixGetColumn(box2ToWorld, 2, tmp);
      CrossProduct(worldToBox1[2], tmp.Base(), pPlane->normal.Base());
...
bool ComputeSeparatingPlane(const Vector &org1, const QAngle &angles1,
  ...
  return ComputeSeparatingPlane(worldToBox1, box2ToWorld, box1Size, box2Size,
                                tolerance, pPlane);
}
...
bool IsRayIntersectingOBB(const Ray_t &ray, const Vector &org,
  ...
  if (!ray.m_IsSwept) {
    cplane_t plane;
    return ComputeSeparatingPlane(ray.m_Start, vec3_angle, -ray.m_Extents,
                                  ray.m_Extents, org, angles, mins, maxs, 0.0f,
                                  &plane) == false;
  }
  ...
  cplane_t plane;
  if (ComputeSeparatingPlane(worldToBox1, box2ToWorld, box1Size, box2Size, 0.0f,
                             &plane))
    return false;

  // Now deal with the planes which are the cross products of the ray sweep
  // direction vs box edges
  Vector vecRayDirection = ray.m_Delta;
  VectorNormalize(vecRayDirection);

  // Need a vector between ray center vs box center measured in the space of the
  // ray (world)
  ...
  // Now do the work for the planes which are perpendicular to the edges of the
  // AABB and the sweep direction edges...

  // In this example, the line to check is perpendicular to box edge x + ray
  // delta we can compute this line by taking the cross-product:
  //
  // [  i  j  k ]
  // [  1  0  0 ] = - dz j + dy k = l1
  // [ dx dy dz ]
  ...
  // The projection of the box onto this line = the absolute dot product of the
  // box size against the line, which = AbsDot( vecBoxHalfDiagonal, l1 ) = abs(
  // -dz * vecBoxHalfDiagonal.y ) + abs( dy * vecBoxHalfDiagonal.z )
  ...
  // AbsDot( ray.m_Extents, l1 ) = abs( -dz * ray.m_Extents.y ) + abs( dy *
  // ray.m_Extents.z )

  Vector vecPlaneNormal;

  // box x x ray delta
  CrossProduct(vecRayDirection,
               Vector(box2ToWorld[0][0], box2ToWorld[1][0], box2ToWorld[2][0]),
               vecPlaneNormal);
  float flCenterDeltaProjection =
      FloatMakePositive(DotProduct(vecPlaneNormal, vecCenterDelta));
  float flBoxProjectionSum = vecAbsRayDirBox2.z * box2Size.y +
                             vecAbsRayDirBox2.y * box2Size.z +
  ...
  // box y x ray delta
  CrossProduct(vecRayDirection,
               Vector(box2ToWorld[0][1], box2ToWorld[1][1], box2ToWorld[2][1]),
               vecPlaneNormal);
  flCenterDeltaProjection =
      FloatMakePositive(DotProduct(vecPlaneNormal, vecCenterDelta));
  flBoxProjectionSum = vecAbsRayDirBox2.z * box2Size.x +
                       vecAbsRayDirBox2.x * box2Size.z +
                       DotProductAbs(vecPlaneNormal, ray.m_Extents);
  if (FloatBits(flCenterDeltaProjection) > FloatBits(flBoxProjectionSum))
    return false;
  ...
  // box z x ray delta
  CrossProduct(vecRayDirection,
               Vector(box2ToWorld[0][2], box2ToWorld[1][2], box2ToWorld[2][2]),
               vecPlaneNormal);
  flCenterDeltaProjection =
      FloatMakePositive(DotProduct(vecPlaneNormal, vecCenterDelta));
  flBoxProjectionSum = vecAbsRayDirBox2.y * box2Size.x +
                       vecAbsRayDirBox2.x * box2Size.y +
                       DotProductAbs(vecPlaneNormal, ray.m_Extents);
  if (FloatBits(flCenterDeltaProjection) > FloatBits(flBoxProjectionSum))
    return false;
...
// Purpose:
//-----------------------------------------------------------------------------
inline bool AxisTestEdgeCrossX2(float flEdgeZ, float flEdgeY, float flAbsEdgeZ,
                                float flAbsEdgeY, const Vector &p1,
                                const Vector &p3, const Vector &vecExtents,
                                float flTolerance) {
  ...
  // Extents are symmetric: dist = abs( normal.y ) * extents.y + abs( normal.z )
  // * extents.z
...
// Purpose:
//--------------------------------------------------------------------------
inline bool AxisTestEdgeCrossX3(float flEdgeZ, float flEdgeY, float flAbsEdgeZ,
                                float flAbsEdgeY, const Vector &p1,
                                const Vector &p2, const Vector &vecExtents,
                                float flTolerance) {
  // Cross Product( axialX(1,0,0) x edge ): x = 0.0f, y = edge.z, z = -edge.y
  // Triangle Point Distances: dist(x) = normal.y * pt(x).y + normal.z * pt(x).z
  float flDist1 = flEdgeZ * p1.y - flEdgeY * p1.z;
...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
inline bool AxisTestEdgeCrossY2(float flEdgeZ, float flEdgeX, float flAbsEdgeZ,
                                float flAbsEdgeX, const Vector &p1,
                                const Vector &p3, const Vector &vecExtents,
                                float flTolerance) {
  ...
  // Extents are symmetric: dist = abs( normal.x ) * extents.x + abs( normal.z )
  // * extents.z
...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
inline bool AxisTestEdgeCrossY3(float flEdgeZ, float flEdgeX, float flAbsEdgeZ,
                                float flAbsEdgeX, const Vector &p1,
                                const Vector &p2, const Vector &vecExtents,
                                float flTolerance) {
  // Cross Product( axialY(0,1,0) x edge ): x = -edge.z, y = 0.0f, z = edge.x
...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
inline bool AxisTestEdgeCrossZ1(float flEdgeY, float flEdgeX, float flAbsEdgeY,
                                float flAbsEdgeX, const Vector &p2,
                                const Vector &p3, const Vector &vecExtents,
                                float flTolerance) {
  ...
  // Extents are symmetric: dist = abs( normal.x ) * extents.x + abs( normal.y )
  // * extents.y
...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
inline bool AxisTestEdgeCrossZ2(float flEdgeY, float flEdgeX, float flAbsEdgeY,
                                float flAbsEdgeX, const Vector &p1,
                                const Vector &p3, const Vector &vecExtents,
                                float flTolerance) {
  // Cross Product( axialZ(0,0,1) x edge ): x = edge.y, y = -edge.x, z = 0.0f
...
bool IsBoxIntersectingTriangle(const Vector &vecBoxCenter,
  ...
  if (!AxisTestEdgeCrossX2(vecEdge.z, vecEdge.y, vecAbsEdge.z, vecAbsEdge.y, p1,
                           p3, vecBoxExtents, flTolerance))
    return false;

  vecAbsEdge.x = FloatMakePositive(vecEdge.x);
  if (!AxisTestEdgeCrossY2(vecEdge.z, vecEdge.x, vecAbsEdge.z, vecAbsEdge.x, p1,
                           p3, vecBoxExtents, flTolerance))
    return false;

  if (!AxisTestEdgeCrossZ1(vecEdge.y, vecEdge.x, vecAbsEdge.y, vecAbsEdge.x, p2,
                           p3, vecBoxExtents, flTolerance))
    return false;
  ...
  if (!AxisTestEdgeCrossZ2(vecEdge.y, vecEdge.x, vecAbsEdge.y, vecAbsEdge.x, p1,
                           p3, vecBoxExtents, flTolerance))
    return false;
  ...
  vecAbsEdge.y = FloatMakePositive(vecEdge.y);
  vecAbsEdge.z = FloatMakePositive(vecEdge.z);
  if (!AxisTestEdgeCrossX3(vecEdge.z, vecEdge.y, vecAbsEdge.z, vecAbsEdge.y, p1,
                           p2, vecBoxExtents, flTolerance))
    return false;

  vecAbsEdge.x = FloatMakePositive(vecEdge.x);
  if (!AxisTestEdgeCrossY3(vecEdge.z, vecEdge.x, vecAbsEdge.z, vecAbsEdge.x, p1,
                           p2, vecBoxExtents, flTolerance))
    return false;
  ...
  if (BoxOnPlaneSide(vecMin, vecMax, &plane) != 3) return false;

  return true;
}

// NOTE: JAY: This is untested code based on Real-time Collision Detection by
// Ericson
#if 0
...
bool OBBHasFullyContainedIntersectionWithQuad(
  ...
  Vector ptOBB[8];  // this specific ordering helps us web out from a point to
                    // its 3 connecting points with some bit math (most
                    // importantly, no if's)
  ptOBB[0] = ptOBBCenter - vOBBExtent1_Scaled - vOBBExtent2_Scaled -
             vOBBExtent3_Scaled;
  ptOBB[1] = ptOBBCenter - vOBBExtent1_Scaled - vOBBExtent2_Scaled +
             vOBBExtent3_Scaled;
  ptOBB[2] = ptOBBCenter - vOBBExtent1_Scaled + vOBBExtent2_Scaled +
             vOBBExtent3_Scaled;
  ptOBB[3] = ptOBBCenter - vOBBExtent1_Scaled + vOBBExtent2_Scaled -
             vOBBExtent3_Scaled;
  ptOBB[4] = ptOBBCenter + vOBBExtent1_Scaled - vOBBExtent2_Scaled -
             vOBBExtent3_Scaled;
  ptOBB[5] = ptOBBCenter + vOBBExtent1_Scaled - vOBBExtent2_Scaled +
             vOBBExtent3_Scaled;
  ptOBB[6] = ptOBBCenter + vOBBExtent1_Scaled + vOBBExtent2_Scaled +
             vOBBExtent3_Scaled;
  ptOBB[7] = ptOBBCenter + vOBBExtent1_Scaled + vOBBExtent2_Scaled -
             vOBBExtent3_Scaled;
  ...
  Vector ptPlaneIntersections[12];  // only have 12 lines, can only possibly
                                    // generate 12 split points
  ...
  for (int i = 0; i != 8; ++i) {
    if (iSides[i] == 2)  // point behind the plane
    {
      ...
      iAxisCrossings[1] =
          ((i + 1) & 3) + (i & 4);  // cycle to the next element while staying
                                    // within the upper 4 or lower 4, this will
                                    // cross either vOBBExtent2 or vOBBExtent3
                                    // axis, we don't care which
      iAxisCrossings[2] =
          ((i - 1) & 3) + (i & 4);  // cylce to the previous element while
                                    // staying within the upper 4 or lower 4,
                                    // this will cross the axis
                                    // iAxisCrossings[1] didn't cross

      for (int j = 0; j != 3; ++j) {
        if (iSides[iAxisCrossings[j]] == 1)  // point in front of the plane
        {
          // line between ptOBB[i] and ptOBB[iAxisCrossings[j]] intersects the
          // plane, generate a point at the intersection for further testing
          float fTotalDist =
              fDists[iAxisCrossings[j]] -
              fDists[i];  // remember that fDists[i] is a negative value
          ptPlaneIntersections[iPlaneIntersectionsCount] =
              (ptOBB[iAxisCrossings[j]] * (-fDists[i] / fTotalDist)) +
              (ptOBB[i] * (fDists[iAxisCrossings[j]] / fTotalDist));

          Assert(fabs(ptPlaneIntersections[iPlaneIntersectionsCount].Dot(
                          vQuadNormal) -
                      fQuadPlaneDist) <
                 0.1f);  // intersection point is on plane
  ...
  for (int i = 0; i != iPlaneIntersectionsCount; ++i) {
    // these points are guaranteed to be on the plane, now just check to see if
    // they're within the quad's extents
    ...
    // vToPointFromQuadCenter -= vQuadExtent1_Normalized * fExt1Dist; //to
    // handle diamond shaped quads
  ...
  return true;  // there were lines crossing the quad plane, and every line
                // crossing that plane had its intersection with the plane
                // within the quad's boundaries
}
...
bool RayHasFullyContainedIntersectionWithQuad(
  ...
  Vector ptPlaneIntersections[(12 + 12 + 8)];  // absolute max possible: 12
                                               // lines to connect the start
                                               // box, 12 more to connect the
                                               // end box, 8 to connect the
                                               // boxes to eachother

  // 8 points to make an AABB, 8 lines to connect each point from it's start to
  // end point along the ray, 8 possible intersections
  int iPlaneIntersectionsCount = 0;
  ...
  if (ray.m_IsRay) {
    ...
    if (ray.m_IsSwept) {
      ...
      for (i = 0; i != 2; ++i) {
        if (fDists[i] <= 0.0f) {
          ...
          if (fDists[j] >= 0.0f) {
            float fInvTotalDist =
                1.0f / (fDists[j] - fDists[i]);  // fDists[i] <= 0, ray is swept
                                                 // so no chance that the denom
                                                 // was 0
            ptPlaneIntersections[0] =
                (ptEndPoints[i] * (fDists[j] * fInvTotalDist)) -
                (ptEndPoints[j] *
                 (fDists[i] * fInvTotalDist));  // fDists[i] <= 0
            Assert(fabs(ptPlaneIntersections[iPlaneIntersectionsCount].Dot(
                            vQuadNormal) -
                        fQuadPlaneDist) <
                   0.1f);  // intersection point is on plane
    ...
    // this specific ordering helps us web out from a point to its 3 connecting
    // points with some bit math (most importantly, no if's)
    ...
    // generate intersections for boxes split by the plane at either end of the
    // ray
    for (int k = 0; k != 2; ++k) {
      if (iSideMask[k] == 3)  // box is split by the plane
      {
        for (int i = 0; i != 8; ++i) {
          if (iSides[k][i] == 2)  // point behind the plane
          {
            ...
            iAxisCrossings[1] =
                ((i + 1) & 3) + (i & 4);  // cycle to the next element while
                                          // staying within the upper 4 or lower
                                          // 4, this will cross either Y or Z
                                          // axis, we don't care which
            iAxisCrossings[2] =
                ((i - 1) & 3) + (i & 4);  // cylce to the previous element while
                                          // staying within the upper 4 or lower
                                          // 4, this will cross the axis
                                          // iAxisCrossings[1] didn't cross

            for (int j = 0; j != 3; ++j) {
              if (iSides[k][iAxisCrossings[j]] ==
                  1)  // point in front of the plane
              {
                // line between ptEndPoints[i] and
                // ptEndPoints[iAxisCrossings[j]] intersects the plane, generate
                // a point at the intersection for further testing
                float fInvTotalDist =
                    1.0f / (fDists[k][iAxisCrossings[j]] -
                            fDists[k][i]);  // remember that
                                            // fDists[k][i] is
                                            // a negative value
                ptPlaneIntersections[iPlaneIntersectionsCount] =
                    (ptEndPoints[k][iAxisCrossings[j]] *
                     (-fDists[k][i] * fInvTotalDist)) +
                    (ptEndPoints[k][i] *
                     (fDists[k][iAxisCrossings[j]] * fInvTotalDist));
    ...
    if (ray.m_IsSwept) {
      for (int i = 0; i != 8; ++i) {
        if (iSides[0][i] != iSides[1][i]) {
          ...
          Assert((fDists[iPosSide][i] >= 0.0f) &&
                 (fDists[iNegSide][i] <= 0.0f));

          float fInvTotalDist =
              1.0f / (fDists[iPosSide][i] -
                      fDists[iNegSide][i]);  // remember that
                                             // fDists[iNegSide][i] is a
                                             // negative value
          ptPlaneIntersections[iPlaneIntersectionsCount] =
              (ptEndPoints[iPosSide][i] *
               (-fDists[iNegSide][i] * fInvTotalDist)) +
              (ptEndPoints[iNegSide][i] *
               (fDists[iPosSide][i] * fInvTotalDist));
  ...
  // down here, we should simply have a collection of plane intersections, now
  // we see if they reside within the quad
  Assert(iPlaneIntersectionsCount != 0);
