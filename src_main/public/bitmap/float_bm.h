...
#define SSBUMP_MOD2X_DETAIL_TEXTURE \
  2  // scale so that a flat unshadowed
...
class FloatBitMap_t {
 public:
  ...
  // compress to 8 bits converts the hdr texture to an 8 bit texture, encoding a
  // scale factor in the alpha channel. upon return, the original pixel can be
  // (approximately) recovered by the formula rgb*alpha*overbright. this
  // function performs special numerical optimization on the texture to minimize
  // the error when using bilinear filtering to read the texture.
  void CompressTo8Bits(float overbright);
  // decompress a bitmap converted by CompressTo8Bits
  void Uncompress(float overbright);

  Vector AverageColor();   // average rgb value of all pixels
  float BrightestColor();  // highest vector magnitude
  ...
  void ComputeVertexPositionsAndNormals(float flHeightScale, Vector **ppPosOut,
                                        Vector **ppNormalOut) const;

  // generate a normal map with height stored in alpha.  uses hl2 tangent basis
  // to support baked self shadowing.  the bump scale maps the height of a pixel
  // relative to the edges of the pixel. This function may take a while - many
  // millions of rays may be traced.  applications using this method need to
  // link w/ raytrace.lib
  FloatBitMap_t *ComputeSelfShadowedBumpmapFromHeightInAlphaChannel(
      float bump_scale, int nrays_to_trace_per_pixel = 100,
      uint32_t nOptionFlags = 0  // SSBUMP_OPTION_XXX
      ) const;
  ...
  // bilateral (edge preserving) smoothing filter. edge_threshold_value defines
  // the difference in values over which filtering will not occur. Each channel
  // is filtered independently. large radii will run slow, since the bilateral
  // filter is neither separable, nor is it a convolution that can be done via
  // fft.
...
class FloatImagePyramid_t {
 public:
  ...
  // read or write a Pixel from a given level. All coordinates are specified in
  // the same domain as the base level.
