...
class IFriendsUser : public IBaseInterface {
 public:
  ...
  // gets the IP address of the server the buddy is on, returns false if
  // couldn't get
  virtual bool GetBuddyGameAddress(unsigned int friendsID, int *ip,
                                   int *port) = 0;

  // returns the number of buddies
  virtual int GetNumberOfBuddies() = 0;

  // returns the FriendsID of a buddy - buddyIndex is valid in the range [0,
  // GetNumberOfBuddies)
