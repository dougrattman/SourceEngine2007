...
#ifndef SOURCE_TIER1_RANGECHECKEDVAR_H_
#define SOURCE_TIER1_RANGECHECKEDVAR_H_
...
class CRangeCheckedVar {
 public:
  inline CRangeCheckedVar() { m_Val = startValue; }

  inline CRangeCheckedVar(const T &value) { *this = value; }

  // Clamp the value to its limits. Interpolation code uses this after
  // interpolating.
  inline void Clamp() {
    if (m_Val < minValue)
      m_Val = minValue;
  ...
  inline operator const T &() const { return m_Val; }

  inline CRangeCheckedVar<T, minValue, maxValue, startValue> &operator=(
      const T &value) {
...
#endif  // SOURCE_TIER1_RANGECHECKEDVAR_H_
