...
#ifndef SOURCE_TIER1_UTLPRIORITYQUEUE_H_
#define SOURCE_TIER1_UTLPRIORITYQUEUE_H_
...
void CUtlPriorityQueue<T>::RemoveAtHead() {
  ...
  int larger = index;
  while (index < half) {
    int child = ((index + 1) * 2) - 1;  // if we wasted an element, this math
                                        // would be more compact (1 based array)
    if (child < count) {
...
#endif  // SOURCE_TIER1_UTLPRIORITYQUEUE_H_
