...
//
// Purpose: Utilities for serialization/unserialization buffer.

#ifndef SOURCE_TIER1_UTLBUFFERUTIL_H_
#define SOURCE_TIER1_UTLBUFFERUTIL_H_
  ...
  if (!buf.IsText()) {
    ...
    if (nCount) {
      ...
      for (int i = 0; i < nCount; ++i) {
        ...
        if (!::Unserialize(buf, dest[i])) return false;
      }
    }
    return buf.IsValid();
  }
...
#endif  // SOURCE_TIER1_UTLBUFFERUTIL_H_
