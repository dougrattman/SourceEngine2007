...
//
// Purpose: A growable memory class.

#ifndef SOURCE_TIER1_UTLMEMORY_H_
...
class CUtlMemoryFixed {
 public:
  ...
  T* Base() {
    MSVC_SCOPED_DISABLE_WARNING(4127, if (nAlignment == 0))
    return (T*)(&m_Memory[0]);
    else return (T*)AlignValue(&m_Memory[0], nAlignment);
  }
  const T* Base() const {
    MSVC_SCOPED_DISABLE_WARNING(4127, if (nAlignment == 0))
    return (T*)(&m_Memory[0]);
...
void CUtlMemory<T, I>::ConvertToGrowableMemory(int nGrowSize) {
  if (!this->IsExternallyAllocated()) return;
...
void CUtlMemory<T, I>::Grow(int num) {
  ...
  m_nAllocationCount = UtlMemory_CalcNewAllocationCount(
      m_nAllocationCount, m_nGrowSize, nAllocationRequested, sizeof(T));

  // if m_nAllocationRequested wraps index type I, recalculate
  if ((int)(I)m_nAllocationCount < nAllocationRequested) {
    if ((int)(I)m_nAllocationCount == 0 &&
        (int)(I)(m_nAllocationCount - 1) >= nAllocationRequested) {
      --m_nAllocationCount;  // deal w/ the common case of m_nAllocationCount ==
                             // MAX_USHORT + 1
    } else {
...
void CUtlMemory<T, I>::Purge(int numElements) {
  ...
  if (!m_pMemory) {
    // Allocation count is non zero, but memory is 0.
    Assert(m_pMemory);
    return;
  }
