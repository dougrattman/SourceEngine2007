...
//
// Purpose: A growable array class that keeps all elements in order using binary
// search

#ifndef SOURCE_TIER1_UTLSORTVECTOR_H_
#define SOURCE_TIER1_UTLSORTVECTOR_H_
...
template <class T, class LessFunc>
void CUtlSortVector<T, LessFunc>::QuickSort(LessFunc& less, int nLower,
                                            int nUpper) {
#ifdef _WIN32
  ...
  if (this->Count() > 1) {
    ...
    qsort_s(this->Base(), this->Count(), sizeof(T),
...
void CUtlSortVector<T, LessFunc>::RedoSort(bool bForceSort /*= false */) {
  ...
  m_bNeedsSort = false;
  LessFunc less;
  this->QuickSort(less, 0, this->Count() - 1);
}
...
#endif  // SOURCE_TIER1_UTLSORTVECTOR_H_
