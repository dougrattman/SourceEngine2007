...
#ifndef SOURCE_TIER1_ICONVAR_H_
#define SOURCE_TIER1_ICONVAR_H_
...
#define FCVAR_PROTECTED \
  (1 << 5)  // It's a server cvar, but we don't send the data since it's a
            ...
            // value
#define FCVAR_SPONLY \
  (1 << 6)  // This cvar cannot be changed by clients connected to a multiplayer
            // server.
...
#define FCVAR_PRINTABLEONLY \
  (1 << 10)  // This cvar's string cannot contain unprintable characters ( e.g.,
...
#define FCVAR_NOT_CONNECTED \
  (1 << 22)  // cvar cannot be changed by a client that is connected to a server
...
#define FCVAR_SERVER_CANNOT_QUERY \
  (1 << 29)  // If this is set, then the server is not allowed to query this
