...
//
// Purpose: Intrusive linked list templates, both for singly and doubly linked
// lists.
...
//
// These templates are used for intrusive linked list classes. Intrusive linked
// list templates force the structs and classes contained within them to have
// their own m_pNext, (optionally), m_pPrev, and other fileds contained within.
// All memory management is up to the caller and their classes. No data will
// ever be copied. Nodes can only exist on one list at a time, because of only
// having on m_Next field, and manipulating the list while walking it requires
// that care on the part of the caller. All accessing and searching functions
// work by passing and returning pointers.
//
//
//
// naming and field conventions:
// functions referring to a DList are for doubly linked lists. nodes must have
// m_pHead and m_pPrev pointer fields. Functions using Priority require an
// m_Priority field, which must be comparable.
//
// Some functions are meanto for use with lists which maintain both a head and
// tail pointer in order to support fast adding to the end.
...
// Add a node to a d-list, keeping the highest priority nodes first. This is a
// simple linear search to insert, NOT a O(logn) heap.
template <class T>
inline void AddToDPriority(T *&head, T *which) {
  T *prevnode = 0;
  for (T *curnode = head; curnode; curnode = curnode->m_pNext) {
    if (which->m_Priority >= curnode->m_Priority) break;
    prevnode = curnode;
  }
  ...
  if (!prevnode) {
    ...
    if (which->m_pNext) which->m_pNext->m_pPrev = which;
  }
}
...
// find the Nth node in a list. 0 if index too high.
template <class T>
...
static inline void AddToTail(T *&head, V *node) {
  ...
  if (!head)
    ...
    T *pLastNode = head;
    while (pLastNode->m_pNext) pLastNode = pLastNode->m_pNext;
...
// return the node in head before in a singly linked list. returns 0 if head
// is empty, n is 0, or if n is the first node. not fast.
template <class T>
static inline T *PrevNode(T *head, T *node) {
  for (T *i = head; i; i = i->m_pNext) {
    if (i->m_pNext == node) break;
  }
  return i;
}
...
static inline T *FindNodeByField(T *head, U data, U V::*field) {
  while (head) {
    if (data == (*head).*field) return head;
    head = head->m_pNext;
  }
  return 0;
}
...
template <class T, class U, class V>
static inline T *FindNodeByFieldWithPrev(T *head, U data, U V::*field,
                                         T *&prev) {
  prev = 0;
  for (T *i = head; i; i = i->m_pNext) {
    if (data == (*i).*field) return i;
    prev = i;
  }
  prev = 0;
  return 0;
...
/// sort a list. comparefn should return 0 if the items are equal, 1 if A goes
/// first, and -1 if A goes last.
// NOT fast.
template <class T>
...
// reverse a singly linked list. not recommended for anything other than valve
...
class CUtlIntrusiveList {
 public:
  ...
  // return the node in head before in a singly linked list. returns 0 if
  // head is empty, n is 0, or if n is the first node. not fast. Fast for
  // dlists
  ...
  int Count(void) { return IntrusiveList::ListLength(m_pHead); }
};
...
class CUtlIntrusiveDList : public CUtlIntrusiveList<T> {
 public:
  ...
  T *PrevNode(T *node) { return (node) ? node->m_Prev : NULL; }
};

#endif  // SOURCE_TIER1_UTILINTRUSIVELIST_H_
