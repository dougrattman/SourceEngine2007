...
void CBidirectionalSet<CBucketHandle, CElementHandle, S,
  ...
  while (elementInBucket != m_ElementsInBucket.InvalidIndex()) {
    // If you hit an Assert here, fix the calling code.  It's too expensive to
    // ensure that each item only shows up once here.  Hopefully you can do
    // something better outside of here.
...
#endif  // SOURCE_TIER1_UTLBIDIRECTIONALSET_H_
