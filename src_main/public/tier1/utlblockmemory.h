...
#ifndef SOURCE_TIER1_UTLBLOCKMEMORY_H_
...
void CUtlBlockMemory<T, I>::ChangeSize(int nBlocks) {
  UTLMEMORY_TRACK_FREE();  // this must stay before the recalculation of
                           // m_nBlocks, since it implicitly uses the old value

  int nBlocksOld = m_nBlocks;
  m_nBlocks = nBlocks;

  UTLMEMORY_TRACK_ALLOC();  // this must stay after the recalculation of
                            // m_nBlocks, since it implicitly uses the new value
  ...
  if (m_pMemory) {
    ...
    T** reallocated_memory = (T**)realloc(m_pMemory, m_nBlocks * sizeof(T*));
