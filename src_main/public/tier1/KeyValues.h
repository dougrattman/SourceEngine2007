...
// Data is stored in key (string names) - (string/int/float)value pairs called
// nodes.
//
//	About KeyValues Text File Format:

//	It has 3 control characters '{', '}' and '"'. Names and values may be
// quoted or 	not. The quote '"' charater must not be used within name or
// values, only for 	quoting whole tokens. You may use escape sequences wile
// parsing and add within a 	quoted token a \" to add quotes within your name
// or token. When using Escape 	Sequence the parser must now that by setting
...
class KeyValues {
 public:
  ...
  // Simply construct it with the keyvalues you allocated and it will free them
  // when falls out of scope. When you decide that keyvalues shouldn't be
  ...
  // Read from a buffer...  Note that the buffer must be 0 terminated
 ...
 private:
  ...
  // NOTE: If both filesystem and pBuf are non-0, it'll save to both of them.
  // If filesystem is 0, it'll ignore f.
...
struct KeyValuesUnpackStructure {
  char const *m_pKeyName;                         // 0 to terminate tbl
  char const *m_pKeyDefault;                      // 0 ok
...
#endif  // SOURCE_TIER1_KEYVALUES_H_
