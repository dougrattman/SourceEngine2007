...
//
// Purpose: Serialization/deserialization buffer.

#ifndef SOURCE_TIER1_UTLBUFFER_H_
#define SOURCE_TIER1_UTLBUFFER_H_
...
class CUtlInplaceBuffer : public CUtlBuffer {
 ...
 public:
  ...
  // because the trailing newline characters are also included as part of the
  // line.
  //
  // Note:	the pointer returned points into the local memory of this
  // buffer, in case the buffer gets relocated or destroyed the pointer becomes
  // invalid.
  //
  // e.g.:	-------------
  //
  ...
  // read and buffer modified NULL  	when EOF is reached or error occurs
  //
...
inline void CUtlBuffer::GetTypeBin<float>(float &dest) {
  if (CheckGet(sizeof(float))) {
    const void *data = PeekGet();
    // aligned read
    memcpy(&dest, data, sizeof(float));
...
#endif  // SOURCE_TIER1_UTLBUFFER_H_
