...
class CMemoryPool {
 public:
  ...
  enum MemoryPoolGrowType_t {
    ...
    GROW_FAST = 1,  // New blob size is numElements * (i+1)  (ie: the blocks it
                    // allocates
...
class CMemoryPoolMT : public CMemoryPool {
 public:
  CMemoryPoolMT(int blockSize, int numElements, int growMode = GROW_FAST,
      ...
      : CMemoryPool(blockSize, numElements, growMode, pszAllocOwner) {}
  ...
  void *Alloc(size_t amount) {
    AUTO_LOCK(m_mutex);
    return CMemoryPool::Alloc(amount);
  }
  void *AllocZero() {
    AUTO_LOCK(m_mutex);
    return CMemoryPool::AllocZero();
  }
  void *AllocZero(size_t amount) {
    AUTO_LOCK(m_mutex);
    return CMemoryPool::AllocZero(amount);
  }
  void Free(void *pMem) {
    AUTO_LOCK(m_mutex);
...
class CAlignedMemPool {
 ...
 private:
  struct FreeBlock_t {
    FreeBlock_t *pNext;
    uint8_t reserved[BLOCK_SIZE - sizeof(FreeBlock_t *)];
  };

  CUtlVector<void *> m_Chunks;  // Chunks are tracked outside blocks (unlike
                                // CMemoryPool) to simplify alignment issues
