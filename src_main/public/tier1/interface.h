...
//    1. The interface must be ALL pure virtual, and have no data members.
...
// This is achieved through unique namespace 'InterfaceReg' via symbol
...
// interface exposure) thereby stitching all of that lib's code/data together
// for eventual final executable link inclusion.
...
using CreateInterfaceFn = void *(*)(const char *interface_name,
                                    int *return_code);
...
class InterfaceReg {
 public:
  InterfaceReg(InstantiateInterfaceFn instantiate_interface_func,
...
// names/virtual tables to communicate.
//
...
extern CreateInterfaceFn Sys_GetFactory(const char *module_name);
...
// When the singleton goes out of scope (.dll unload if at module scope), then
// it'll call Sys_UnloadModule on the module so that the reference count is
// decremented and the .dll actually can unload from memory.
class CDllDemandLoader {
 public:
  CDllDemandLoader(char const *module_name);
  virtual ~CDllDemandLoader();
  CreateInterfaceFn GetFactory();

 private:
  char const *module_path_;
  CSysModule *module_;
  bool is_load_attempted_;
};

#endif  // SOURCE_TIER1_INTERFACE_H_
