...
void SendProxy_UInt8ToInt32(const SendProp *pProp, const void *pStruct,
                            const void *pData, DVariant *pOut, int iElement,
                            int objectID);
void SendProxy_UInt16ToInt32(const SendProp *pProp, const void *pStruct,
                             const void *pData, DVariant *pOut, int iElement,
                             int objectID);
void SendProxy_UInt32ToInt32(const SendProp *pProp, const void *pStruct,
                             const void *pData, DVariant *pOut, int iElement,
                             int objectID);
...
static void SendProxy_Empty(const SendProp *pProp, const void *pStruct,
                            const void *pData, DVariant *pOut, int iElement,
                            int objectID) {}
...
SendProp SendPropFloat(
  ...
  if (nBits <= 0 || nBits == 32) {
    ...
    if (fHighValue == HIGH_DEFAULT) fHighValue = (1 << nBits);
  ...
  ret.m_fHighLowMul =
      AssignRangeMultiplier(ret.m_nBits, ret.m_fHighValue - ret.m_fLowValue);
  ret.SetProxyFn(varProxy);
  if (ret.GetFlags() &
      (SPROP_COORD | SPROP_NOSCALE | SPROP_NORMAL | SPROP_COORD_MP |
       SPROP_COORD_MP_LOWPRECISION | SPROP_COORD_MP_INTEGRAL))
    ret.m_nBits = 0;
...
SendProp SendPropVector(
    const char *pVarName, int offset, int sizeofVar,
    int nBits,  // Number of bits to use when encoding.
    int flags,
    float fLowValue,   // For floating point, low and high values.
    float fHighValue,  // High value. If HIGH_DEFAULT, it's (1<<nBits).
  ...
  if (nBits == 32) flags |= SPROP_NOSCALE;
...
SendProp SendPropAngle(const char *pVarName, int offset, int sizeofVar,
                       int nBits, int flags, SendVarProxyFn varProxy) {
  SendProp ret;
...
SendProp SendPropQAngles(const char *pVarName, int offset, int sizeofVar,
                         int nBits, int flags, SendVarProxyFn varProxy) {
  SendProp ret;
...
SendProp SendPropInt(const char *pVarName, int offset, int sizeofVar, int nBits,
                     int flags, SendVarProxyFn varProxy) {
  SendProp ret;
  ...
  // the values correctly, but it lets us check the ranges of the data to make
  // sure they're valid.
  ret.SetProxyFn(varProxy);
...
SendProp SendPropString(const char *pVarName, int offset, int bufferLen,
  ...
  Assert(bufferLen <=
         DT_MAX_STRING_BUFFERSIZE);  // You can only have strings with 8-bits
                                     // worth of length.
...
SendProp SendPropArray3(const char *pVarName, int offset, int sizeofVar,
                        int elements, SendProp pArrayProp,
  ...
  *pArrayPropAllocated = pArrayProp;
  ...
  SendTable *pTable =
      new SendTable(pProps, elements, pVarName);  // TODO free that again
...
SendProp SendPropDataTable(const char *pVarName, int offset, SendTable *pTable,
                           SendTableProxyFn varProxy) {
  SendProp ret;
...
SendProp InternalSendPropArray(const int elementCount, const int elementStride,
  ...
  ret.m_pArrayProp =
      NULL;  // This gets set in SendTable_InitTable. It always points at the
             // property that precedes this one in the datatable's list.
...
void SendTable::Construct(SendProp *pProps, int nProps,
