...
class CBoneBitList : public CBitVec<MAXSTUDIOBONES> {
 public:
  ...
  inline bool IsBoneMarked(int iBone) { return Get(iBone) != 0 ? true : false; }
};
...
void CalcPose(const CStudioHdr *pStudioHdr,
              CIKContext *pIKContext,  // optional
              Vector pos[], Quaternion q[], int sequence, float cycle,
              const float poseParameter[], int boneMask, float flWeight = 1.0f,
              float flTime = 0.0f);
...
void AccumulatePose(const CStudioHdr *pStudioHdr,
                    CIKContext *pIKContext,  // optional
                    Vector pos[], Quaternion q[], int sequence, float cycle,
                    const float poseParameter[], int boneMask, float flWeight,
                    float flTime);

// takes a "controllers[]" array normalized to 0..1 and adds in the adjustments
// to pos[], and q[].
...
class CIKTarget {
 ...
 public:
  ...
  struct x5 {
    ...
    Vector closest;    // closest valid location from hip to foot that the foot
                       // can move to
    Vector knee;       // pre-ik location of knee
    Vector farthest;   // farthest valid location from hip to foot that the foot
                       // can move to
    Vector
        lowest;  // lowest position directly below hip that the foot can drop to
  } trace;
...
class CIKContext {
 public:
  ...
  void AddDependencies(mstudioseqdesc_t &seqdesc, int iSequence, float flCycle,
                       const float poseParameters[], float flWeight = 1.0f);
  ...
  void SolveDependencies(Vector pos[], Quaternion q[],
                         matrix3x4_t boneToWorld[], CBoneBitList &boneComputed);
...
// takes a "poseparameters[]" array normalized to 0..1 and layers on the
// sequences driven by them
void CalcAutoplaySequences(const CStudioHdr *pStudioHdr,
                           CIKContext *pIKContext,  // optional
