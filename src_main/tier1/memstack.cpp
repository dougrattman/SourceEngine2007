...
bool CMemoryStack::Init(unsigned maxSize, unsigned commitSize,
                        unsigned initialCommit, unsigned alignment) {
  Assert(!m_pBase);
...
bool CMemoryStack::CommitTo(uint8_t *RESTRICT pNextAlloc) {
  ...
  if (!VirtualAlloc(m_pCommitLimit, commitSize, VA_COMMIT_FLAGS,
                    PAGE_READWRITE)) {
    Assert(0);
    return false;
  }
...
void CMemoryStack::FreeToAllocPoint(MemoryStackMark_t mark, bool bDecommit) {
  ...
  if (pAllocPoint >= m_pBase && pAllocPoint < m_pNextAlloc) {
    if (bDecommit) {
#if defined(_WIN32)
      unsigned char *pDecommitPoint =
          AlignValue((unsigned char *)pAllocPoint, m_commitSize);
