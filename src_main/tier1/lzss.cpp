...
unsigned char *CLZSS::CompressNoAlloc(unsigned char *pInput, int inputLength,
  ...
  m_pHashTarget =
      (lzss_node_t *)stackalloc(m_nWindowSize * sizeof(lzss_node_t));
  ...
  // prevent compression failure (inflation), leave enough to allow dribble eof
  // bytes
  ...
  while (inputLength > 0) {
    ...
    int encodedLength = 0;
    int lookAheadLength =
        inputLength < LZSS_LOOKAHEAD ? inputLength : LZSS_LOOKAHEAD;
