...
void CUtlBuffer::EnsureCapacity(int num) {
  // Add one extra for the 0 termination
  num += 1;
...
void CUtlBuffer::EatWhiteSpace() {
  if (IsText() && IsValid()) {
    while (CheckGet(sizeof(char))) {
      if (!isspace(*(const unsigned char *)PeekGet())) break;
      m_Get += sizeof(char);
    }
  }
}
...
int CUtlBuffer::PeekWhiteSpace(int nOffset) {
  ...
  while (CheckPeekGet(nOffset, sizeof(char))) {
    if (!isspace(*(unsigned char *)PeekGet(nOffset))) break;
    nOffset += sizeof(char);
  }
...
void CUtlBuffer::GetString(char *pString, int nMaxChars) {
  ...
  // Remember, this *includes* the 0 character
        ...
        switch (type) {
          ...
          case 'd': {
            ...
            *i = strtol((char *)PeekGet(), &pEnd, 10);
            ...
            if (nBytesRead == 0) return numScanned;
            m_Get += nBytesRead;
          } break;
...
bool CUtlBuffer::ParseToken(const char *pStartingDelim,
  ...
  if (nCharsToCopy > 0) {
    ...
    for (; nCharsToCopy > 0; --nCharsToCopy) {
      if (!isspace((unsigned char)pString[nCharsToCopy - 1])) break;
    }
  }
...
void CUtlBuffer::PutString(const char *pString) {
  if (!IsText()) {
    if (pString) {
      // Not text? append a 0 at the end.
...
bool CUtlBuffer::IsBigEndian() { return m_Byteswap.IsTargetBigEndian(); }
...
bool CUtlInplaceBuffer::InplaceGetLinePtr(char **ppszInBufferPtr,
  ...
  --nLineLen;  // because it accounts for putting a terminating 0-character
