...
CDataManagerBase::~CDataManagerBase() { Assert(m_listsAreFreed); }

void CDataManagerBase::NotifySizeChanged(memhandle_t handle,
...
// Frees everything!  The LRU AND the LOCKED items.  This is only used to
// forcibly free the resources, not to make space.
...
void CDataManagerBase::DestroyResource(memhandle_t handle) {
  ...
  if (m_memoryLists[index].lockCount) BreakLock(handle);
...
memhandle_t CDataManagerBase::StoreResourceInHandle(unsigned short memoryIndex,
                                                    void *pStore,
...
void CDataManagerBase::GetLockHandleList(CUtlVector<memhandle_t> &list) {
  for (int node = m_memoryLists.Head(m_lockList);
       node != m_memoryLists.InvalidIndex(); node = m_memoryLists.Next(node)) {
    list.AddToTail(ToHandle(node));
  }
}
