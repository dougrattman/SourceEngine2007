...
bool CCommandBuffer::ParseArgV0(CUtlBuffer &buf, char *pArgV0, int nMaxLen,
  ...
  *pArgS = NULL;
  ...
  if ((nSize <= 0) || (nMaxLen == nSize)) return false;
...
void CCommandBuffer::InsertCommandAtAppropriateTime(int hCommand) {
  ...
  for (i = m_Commands.Head(); i != m_Commands.InvalidIndex();
       i = m_Commands.Next(i)) {
    if (m_Commands[i].m_nTick > command.m_nTick) break;
  }
...
void CCommandBuffer::GetNextCommandLength(const char *pText, int nMaxLen,
  ...
  for (nNextCommandOffset = 0; nNextCommandOffset < nMaxLen;
       ++nNextCommandOffset, nCommandLength += bIsCommented ? 0 : 1) {
    ...
    if (!bIsCommented) {
      ...
      if (!bIsQuoted && c == '/') {
        bIsCommented = (nNextCommandOffset < nMaxLen - 1) &&
                       pText[nNextCommandOffset + 1] == '/';
        if (bIsCommented) {
          ++nNextCommandOffset;
          continue;
        }
...
bool CCommandBuffer::AddText(const char *pText, int nTickDelay) {
  ...
  for (; nLen > 0; nLen -= nOffsetToNextCommand + 1,
                   pCurrentCommand += nOffsetToNextCommand + 1) {
    // find a \n or ; line break
    int nCommandLength;
    GetNextCommandLength(pCurrentCommand, nLen, &nCommandLength,
                         &nOffsetToNextCommand);
    if (nCommandLength <= 0) continue;

    const char *pArgS;
    char *pArgV0 = (char *)_alloca(nCommandLength + 1);
    CUtlBuffer bufParse(pCurrentCommand, nCommandLength,
                        CUtlBuffer::TEXT_BUFFER | CUtlBuffer::READ_ONLY);
    ParseArgV0(bufParse, pArgV0, nCommandLength + 1, &pArgS);
    if (pArgV0[0] == 0) continue;
...
void CCommandBuffer::DelayAllQueuedCommands(int nDelay) {
  ...
  for (int i = m_Commands.Head(); i != m_Commands.InvalidIndex();
       i = m_Commands.Next(i)) {
...
bool CCommandBuffer::DequeueNextCommand() {
  ...
  Assert(m_bIsProcessingCommands);
  if (m_Commands.Count() == 0) return false;

  int nHead = m_Commands.Head();
  Command_t &command = m_Commands[nHead];
  if (command.m_nTick > m_nLastTickToProcess) return false;
...
void CCommandBuffer::Compact() {
  ...
  for (int i = m_Commands.Head(); i != m_Commands.InvalidIndex();
    ...
    memcpy(&pTempBuffer[m_nArgSBufferSize],
           &m_pArgSBuffer[command.m_nFirstArgS], command.m_nBufferSize);
    command.m_nFirstArgS = m_nArgSBufferSize;
...
void CCommandBuffer::EndProcessingCommands() {
  ...
  while (i != m_Commands.InvalidIndex()) {
    if (m_Commands[i].m_nTick >= m_nCurrentTick) break;

    AssertMsgOnce(false,
                  "CCommandBuffer::EndProcessingCommands() called before all "
                  "appropriate commands were dequeued.\n");
    int nNext = i;
