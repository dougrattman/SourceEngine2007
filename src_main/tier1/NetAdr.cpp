...
bool netadr_t::CompareAdr(const netadr_t &a, bool onlyBase) const {
  ...
  if (type == NA_IP) {
    if (!onlyBase && (port != a.port)) return false;
...
bool netadr_t::CompareClassBAdr(const netadr_t &a) const {
  ...
  if (type == NA_IP) {
    if (a.ip[0] == ip[0] && a.ip[1] == ip[1]) return true;
  }
...
bool netadr_t::CompareClassCAdr(const netadr_t &a) const {
  ...
  if (type == NA_IP) {
    if (a.ip[0] == ip[0] && a.ip[1] == ip[1] && a.ip[2] == ip[2]) return true;
  }
...
const char *netadr_t::ToString(bool baseOnly) const {
  ...
  if (type == NA_LOOPBACK) {
    ...
    if (baseOnly) {
      ...
      Q_snprintf(s, sizeof(s), "%i.%i.%i.%i:%i", ip[0], ip[1], ip[2], ip[3],
                 ntohs(port));
    }
  }
...
void netadr_t::SetIP(uint8_t b1, uint8_t b2, uint8_t b3, uint8_t b4) {
  ip[0] = b1;
  ip[1] = b2;
  ip[2] = b3;
  ip[3] = b4;
...
void netadr_t::SetIP(uint32_t unIP) { *((uint32_t *)ip) = BigLong(unIP); }
...
unsigned long netadr_t::addr_ntohl() const { return ntohl(GetIP()); }

unsigned long netadr_t::addr_htonl() const { return htonl(GetIP()); }
...
bool netadr_t::operator<(const netadr_t &netadr) const {
  if (*((uint32_t *)netadr.ip) < *((uint32_t *)ip))
    return true;
  else if (*((uint32_t *)netadr.ip) > *((uint32_t *)ip))
    return false;
