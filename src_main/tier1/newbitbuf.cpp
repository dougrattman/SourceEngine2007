...
void CBitWrite::StartWriting(void *pData, int nBytes, int iStartBit,
                             int nBits) {
...
void CBitWrite::WriteBitCoordMP(const float f, bool bIntegral,
  ...
  if (bIntegral) {
    ...
    WriteUBitLong((unsigned int)fractval,
                  bLowPrecision ? COORD_FRACTIONAL_BITS_MP_LOWPRECISION
...
bool CBitRead::Seek(int nPosition) {
  ...
  int nHead =
      m_nDataBytes &
      3;  // non-multiple-of-4 bytes at head of buffer. We put the "round off"
  ...
  if ((m_nDataBytes < 4) || (nHead && (nByteOfs < nHead))) {
    ...
    if (m_pData) {
      ...
      if (nHead > 1) m_nInBufWord |= (*pPartial++) << 8;
      if (nHead > 2) m_nInBufWord |= (*pPartial++) << 16;
    }
    ...
    m_pDataIn = reinterpret_cast<uint32_t const *>(
        reinterpret_cast<uint8_t const *>(m_pData) +
...
void CBitRead::StartReading(const void *pData, int nBytes, int iStartBit,
                            int nBits) {
  // Make sure it's dword aligned and padded.
  Assert(((unsigned long)pData & 3) == 0);
  m_pData = (uint32_t *)pData;
  ...
  m_bOverflow = false;
  m_pBufferEnd = reinterpret_cast<uint32_t const *>(
      reinterpret_cast<uint8_t const *>(m_pData) + nBytes);
  if (m_pData) Seek(iStartBit);
}

bool CBitRead::ReadString(char *pStr, int maxLen, bool bLine,
                          int *pOutNumChars) {
  Assert(maxLen != 0);
...
int64_t CBitRead::ReadLongLong(void) {
  int64_t retval;
...
float CBitRead::ReadBitCoordMP(bool bIntegral, bool bLowPrecision) {
  ...
  if (bIntegral) {
    ...
    // Calculate the correct floating point value
    value = intval +
            ((float)fractval * (bLowPrecision ? COORD_RESOLUTION_LOWPRECISION
                                              : COORD_RESOLUTION));
  }
