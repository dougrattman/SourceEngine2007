...
int _V_wcscmp(const char *file, int line, const wchar_t *s1,
              const wchar_t *s2) {
  while (1) {
...
int V_strncmp(const char *s1, const char *s2, int count) {
  ...
  while (count-- > 0) {
    ...
    if (*s1 == '\0') return 0;  // 0 terminator hit - strings the same
    s1++;
    s2++;
  }
...
char *V_strnlwr(char *s, ptrdiff_t count) {
  Assert(count >= 0);
...
int V_strncasecmp(const char *s1, const char *s2, int n) {
  ...
  while (n-- > 0) {
    ...
    if (c1 == '\0') return 0;  // 0 terminator hit - strings the same
  }
  ...
  while (*pLetter != 0) {
    ...
    if (tolower(*pLetter) == tolower(*pSearch)) {
      ...
      while (*pTest != 0) {
        ...
        if (tolower(*pMatch) != tolower(*pTest)) break;
...
//			max_chars_to_copy - COPY_ALL_CHARACTERS in pSrc or max #
// to  copy
// Output : char * the copied buffer

char *V_strncat(char *pDest, const char *pSrc, size_t destBufferSize,
...
char *V_pretifynum(int64_t value) {
  ...
  *out = 0;
  ...
  if (value >= 1000000000000000) {
    char *pchRender = out + V_strlen(out);
    V_snprintf(pchRender, 32, "%d,", value / 1000000000000000);
  }
...
#elif OS_POSIX
  int cchResult =
      mbstowcs(pwchDest, pUTF8, cubDestSizeInBytes / sizeof(wchar_t));
#endif
...
void V_hextobinary(char const *in, int numchars, uint8_t *out,
                   int maxoutputbytes) {
  int len = V_strlen(in);
  ...
  *pOut = 0;
  ...
  while (*pIn) {
    if (pIn[0] == '.' && pIn[1] == '.' &&
        ...
        (pIn[2] == 0 || PATHSEPARATOR(pIn[2])))  // Following character must be
                                                 // a slash or the end of the
                                                 // string.
    {
...
char *AllocString(const char *pStr, int nMaxChars) {
  int allocLen, len = strlen(pStr);
  if (nMaxChars == -1)
    allocLen = len + 1;
  else
    allocLen = std::min(len, nMaxChars) + 1;
...
char *V_AddBackSlashesToSpecialChars(char const *pSrc) {
  ...
  char *pRet = new char[nSpaceNeeded + 1];  // +1 for 0
  char *pOut = pRet;
