...
#define LzmaGetNumProbs(Properties) \
  (LZMA_BASE_SIZE + (LZMA_LIT_SIZE << ((Properties)->lc + (Properties)->lp)))
...
int LzmaDecode(CLzmaDecoderState *vs,
...
#endif
  int state = vs->State;
  UInt32 rep0 = vs->Reps[0], rep1 = vs->Reps[1], rep2 = vs->Reps[2],
         rep3 = vs->Reps[3];
  ...
  *outSizeProcessed = 0;
  ...
  if (len == kLzmaNeedInitId) {
    {
      UInt32 numProbs =
          Literal + ((UInt32)LZMA_LIT_SIZE << (lc + vs->Properties.lp));
      UInt32 i;
  ...
  *outSizeProcessed = 0;
  ...
  while (nowPos < outSize) {
    ...
    IfBit0(prob) {
      int symbol = 1;
      UpdateBit0(prob) prob = p + Literal +
...
unsigned int CLZMA::Uncompress(unsigned char *pInput, unsigned char *pOutput) {
  ...
  if (LzmaDecodeProperties(&state.Properties,
                           ((lzma_header_t *)pInput)->properties,
  ...
  SizeT inProcessed;
  SizeT outProcessed;
  int result = LzmaDecode(&state, pInput + sizeof(lzma_header_t), lzmaSize,
                          &inProcessed, pOutput, actualSize, &outProcessed);
