...
class CKeyErrorContext {
 public:
  CKeyErrorContext(KeyValues *pKv) { Init(pKv->GetNameSymbol()); }

  ~CKeyErrorContext() { g_KeyValuesErrorStack.Pop(); }
  CKeyErrorContext(int symName) { Init(symName); }
      ...
      switch (dat->m_iDataType) {
        ...
        case TYPE_WSTRING: {
          ...
          if (dat->m_wsValue) {
            ...
            Assert(::WideCharToMultiByte(CP_UTF8, 0, dat->m_wsValue, -1, NULL,
                                         0, NULL, NULL) < KEYVALUES_TOKEN_SIZE);
            int result =
                ::WideCharToMultiByte(CP_UTF8, 0, dat->m_wsValue, -1, buf,
                                      KEYVALUES_TOKEN_SIZE, NULL, NULL);
            if (result) {
              WriteIndents(filesystem, f, pBuf, indentLevel + 1);
              INTERNALWRITE("\"", 1);
  ...
  if (dat) {
    ...
    switch (dat->m_iDataType) {
      ...
      case TYPE_WSTRING: {
        ...
        char wideBuf[512];
        int result = ::WideCharToMultiByte(CP_UTF8, 0, dat->m_wsValue, -1,
                                           wideBuf, 512, NULL, NULL);
        if (result) {
  ...
  if (dat) {
    ...
    switch (dat->m_iDataType) {
      ...
      case TYPE_STRING: {
        ...
        int result =
            ::MultiByteToWideChar(CP_UTF8, 0, dat->m_sValue, -1, wbuftemp, 512);
        if (result) {
