...
//-----------------------------------------------------------------------------
CToolMenuButton *CreateToolFileMenuButton(vgui::Panel *parent,
                                          const char *panelName,
                                          const char *text,
                                          vgui::Panel *pActionTarget,
                                          IFileMenuCallbacks *pCallbacks) {
  return new CToolFileMenuButton(parent, panelName, text, pActionTarget,
                                 pCallbacks);
}
...
// Constructor
//-----------------------------------------------------------------------------
CToolFileMenuButton::CToolFileMenuButton(vgui::Panel *pParent,
                                         const char *panelName,
                                         const char *text,
                                         vgui::Panel *pActionSignalTarget,
                                         IFileMenuCallbacks *pFileMenuCallback)
    : BaseClass(pParent, panelName, text, pActionSignalTarget),
      m_pFileMenuCallback(pFileMenuCallback) {
  Assert(pFileMenuCallback);

  AddMenuItem("new", "#ToolFileNew", new KeyValues("OnNew"),
              pActionSignalTarget, NULL, "file_new");
  AddMenuItem("open", "#ToolFileOpen", new KeyValues("OnOpen"),
              pActionSignalTarget, NULL, "file_open");
  AddMenuItem("save", "#ToolFileSave", new KeyValues("OnSave"),
              pActionSignalTarget, NULL, "file_save");
  AddMenuItem("saveas", "#ToolFileSaveAs", new KeyValues("OnSaveAs"),
              pActionSignalTarget);
  AddMenuItem("close", "#ToolFileClose", new KeyValues("OnClose"),
              pActionSignalTarget);
  AddSeparator();

  m_pPerforce = new vgui::Menu(this, "Perforce");
  m_pMenu->AddCascadingMenuItem("#ToolPerforce", this, m_pPerforce);
  m_nPerforceAdd = m_pPerforce->AddMenuItem(
      "perforce_add", "#ToolPerforceAdd", new KeyValues("OnPerforceAdd"), this);
  m_nPerforceOpen =
      m_pPerforce->AddMenuItem("perforce_open", "#ToolPerforceOpen",
                               new KeyValues("OnPerforceOpen"), this);
  m_nPerforceRevert =
      m_pPerforce->AddMenuItem("perforce_revert", "#ToolPerforceRevert",
                               new KeyValues("OnPerforceRevert"), this);
  m_nPerforceSubmit =
      m_pPerforce->AddMenuItem("perforce_submit", "#ToolPerforceSubmit",
                               new KeyValues("OnPerforceSubmit"), this);
  m_pPerforce->AddSeparator();
  m_nPerforceP4Win =
      m_pPerforce->AddMenuItem("perforce_p4win", "#ToolPerforceP4Win",
                               new KeyValues("OnPerforceP4Win"), this);
  m_nPerforceListOpenFiles = m_pPerforce->AddMenuItem(
      "perforce_listopenfiles", "#ToolPerforceListOpenFiles",
      new KeyValues("OnPerforceListOpenFiles"), this);

  m_pRecentFiles = new vgui::Menu(this, "RecentFiles");
  m_nRecentFiles = m_pMenu->AddCascadingMenuItem(
      "#ToolFileRecent", pActionSignalTarget, m_pRecentFiles);

  AddMenuItem("clearrecent", "#ToolFileClearRecent",
              new KeyValues("OnClearRecent"), pActionSignalTarget);
  AddSeparator();
  AddMenuItem("exit", "#ToolFileExit", new KeyValues("OnExit"),
              pActionSignalTarget);
...
void CToolFileMenuButton::OnShowMenu(vgui::Menu *menu) {
  ...
  SetItemEnabled(id,
                 (nEnableMask & IFileMenuCallbacks::FILE_CLEAR_RECENT) != 0);
  ...
  if (bP4Connected && m_pFileMenuCallback->GetPerforceFileName(
                          pPerforceFile, sizeof(pPerforceFile))) {
    ...
    m_pPerforce->SetItemEnabled(
        m_nPerforceAdd, !bIsUnnamed && !bFileInPerforce && !bOpenedForEdit);
    m_pPerforce->SetItemEnabled(
        m_nPerforceOpen, !bIsUnnamed && bFileInPerforce && !bOpenedForEdit);
    m_pPerforce->SetItemEnabled(m_nPerforceRevert,
                                !bIsUnnamed && bOpenedForEdit);
    m_pPerforce->SetItemEnabled(m_nPerforceSubmit,
                                !bIsUnnamed && bOpenedForEdit);
    m_pPerforce->SetItemEnabled(
        m_nPerforceP4Win, !bIsUnnamed && bFileInPerforce || bOpenedForEdit);
...
void CToolFileMenuButton::OnPerforceAdd() {
  char pPerforceFile[SOURCE_MAX_PATH];
  if (m_pFileMenuCallback->GetPerforceFileName(pPerforceFile,
                                               sizeof(pPerforceFile))) {
    CPerforceFileListFrame *pPerforceFrame = new CPerforceFileListFrame(
        m_pFileMenuCallback->GetRootPanel(), "Add Movie File to Perforce?",
        "Movie File", PERFORCE_ACTION_FILE_ADD);
...
void CToolFileMenuButton::OnPerforceOpen() {
  ...
  if (m_pFileMenuCallback->GetPerforceFileName(pPerforceFile,
                                               sizeof(pPerforceFile))) {
    CPerforceFileListFrame *pPerforceFrame =
        new CPerforceFileListFrame(m_pFileMenuCallback->GetRootPanel(),
                                   "Check Out Movie File from Perforce?",
                                   "Movie File", PERFORCE_ACTION_FILE_EDIT);
    pPerforceFrame->AddFile(pPerforceFile);
    pPerforceFrame->DoModal();
  }
}
...
void CToolFileMenuButton::OnPerforceRevert() {
  ...
  if (m_pFileMenuCallback->GetPerforceFileName(pPerforceFile,
    ...
    CPerforceFileListFrame *pPerforceFrame =
        new CPerforceFileListFrame(m_pFileMenuCallback->GetRootPanel(),
                                   "Revert Movie File Changes from Perforce?",
                                   "Movie File", PERFORCE_ACTION_FILE_REVERT);
    pPerforceFrame->AddFile(pPerforceFile);
    pPerforceFrame->DoModal();
  }
}
...
void CToolFileMenuButton::OnPerforceSubmit() {
  ...
  if (m_pFileMenuCallback->GetPerforceFileName(pPerforceFile,
    ...
    CPerforceFileListFrame *pPerforceFrame =
        new CPerforceFileListFrame(m_pFileMenuCallback->GetRootPanel(),
                                   "Submit Movie File Changes to Perforce?",
                                   "Movie File", PERFORCE_ACTION_FILE_SUBMIT);
    pPerforceFrame->AddFile(pPerforceFile);
    pPerforceFrame->DoModal();
  }
}
...
void CToolFileMenuButton::OnPerforceListOpenFiles() {
  ...
  COperationFileListFrame *pOpenedFiles = new COperationFileListFrame(
      m_pFileMenuCallback->GetRootPanel(), "Opened Files In Perforce",
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pOpenType) {
      pOpenedFiles->AddOperation(pOpenType,
                                 p4->String(openedFiles[i].m_sLocalFile));
    }
  }
