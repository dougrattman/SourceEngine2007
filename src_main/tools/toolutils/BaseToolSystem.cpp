...
//-----------------------------------------------------------------------------
// Called at the end of engine startup (after client .dll and server .dll have
// been loaded)
//-----------------------------------------------------------------------------
bool CBaseToolSystem::Init() {
  ...
  m_KeyBindingsHandle =
      Panel::CreateKeyBindingsContext(GetBindingsContextFile(), "GAME");
  ...
  if (pszBackground) {
    m_pBackground =
        materials->FindMaterial(GetBackgroundTextureName(), TEXTURE_GROUP_VGUI);
...
void CBaseToolSystem::ShowMiniViewport(bool state) {
  if (!m_hMiniViewport.Get()) return;

  m_hMiniViewport->SetVisible(state);
}

void CBaseToolSystem::SetMiniViewportBounds(int x, int y, int width,
                                            int height) {
  if (m_hMiniViewport) {
...
void CBaseToolSystem::GetMiniViewportEngineBounds(int &x, int &y, int &width,
                                                  int &height) {
  if (m_hMiniViewport) {
...
vgui::Panel *CBaseToolSystem::GetMiniViewport(void) { return m_hMiniViewport; }
...
// Can the tool quit?
//-----------------------------------------------------------------------------
bool CBaseToolSystem::CanQuit() { return true; }
...
bool CBaseToolSystem::ServerInit(CreateInterfaceFn serverFactory) {
  servertools =
      (IServerTools *)serverFactory(VSERVERTOOLS_INTERFACE_VERSION, NULL);
  if (!servertools) {
    Error(
        "CBaseToolSystem::PostInit:  Unable to get '%s' interface from game "
        ".dll\n",
...
bool CBaseToolSystem::ClientInit(CreateInterfaceFn clientFactory) {
  clienttools =
      (IClientTools *)clientFactory(VCLIENTTOOLS_INTERFACE_VERSION, NULL);
  if (!clienttools) {
    Error(
        "CBaseToolSystem::PostInit:  Unable to get '%s' interface from client "
        ".dll\n",
        VCLIENTTOOLS_INTERFACE_VERSION);
  } else {
    g_pGlobalFlexController = &g_GlobalFlexController;  // don't set this until
                                                        // clienttools is
                                                        // connected
  }
...
void CBaseToolSystem::ServerShutdown() { servertools = NULL; }

void CBaseToolSystem::ClientShutdown() { clienttools = NULL; }
...
void CBaseToolSystem::OnToolActivate() {
  ...
  // TODO(d.rattman): Note that this is necessary because IsGameInputEnabled
  // depends on m_bIsActive at the moment
  OnModeChanged();

  input()->SetModalSubTree(VGui_GetToolRootPanel(), GetVPanel(),
                           IsGameInputEnabled());
...
bool CBaseToolSystem::TrapKey(ButtonCode_t key, bool down) {
  ...
  if (!m_bIsActive) return false;  // didn't trap, continue processing
...
void CBaseToolSystem::UpdateUIVisibility() {
  bool bIsVisible =
      m_bIsActive && (!IsGameInputEnabled() || !m_bFullscreenMode);
  ShowUI(bIsVisible);
}
...
void CBaseToolSystem::SetMode(bool bGameInputEnabled, bool bFullscreen) {
  ...
  if (bOldGameInputEnabled != m_bGameInputEnabled) {
    ...
    // The subtree starts at the tool system root panel.  If game input is
    // enabled then
    //  the subtree should not receive or process input messages, otherwise it
    //  should
...
void CBaseToolSystem::ShowKeyBindingsEditor(Panel *panel,
                                            KeyBindingContextHandle_t handle) {
  if (!m_hKeyBindingsEditor.Get()) {
    // Show the editor
    m_hKeyBindingsEditor =
        new CKeyBoardEditorDialog(GetClientArea(), panel, handle);
    m_hKeyBindingsEditor->DoModal();
  }
}

void CBaseToolSystem::ShowKeyBindingsHelp(Panel *panel,
                                          KeyBindingContextHandle_t handle,
                                          vgui::KeyCode boundKey,
                                          int modifiers) {
  ...
  m_hKeyBindingsHelp = new CKeyBindingHelpDialog(GetClientArea(), panel, handle,
                                                 boundKey, modifiers);
}
...
void CBaseToolSystem::OnKeyBindingHelp() {
  ...
  if (tool) {
    ...
    if (list.Count() > 0) {
      ShowKeyBindingsHelp(tool, tool->GetKeyBindingsContext(),
                          (KeyCode)list[0]->keycode, list[0]->modifiers);
    }
  }
}
...
void CBaseToolSystem::PostMessageToActiveTool(KeyValues *pKeyValues,
                                              float flDelay) {
...
void CBaseToolSystem::PostMessageToAllTools(KeyValues *message) {
  ...
  for (int i = 0; i < nCount; ++i) {
    IToolSystem *pToolSystem =
        const_cast<IToolSystem *>(enginetools->GetToolSystem(i));
...
void CBaseToolSystem::OnThink() {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!hasFocus && !focusOnChild) continue;
...
//-----------------------------------------------------------------------------
void CBaseToolSystem::AdjustEngineViewport(int &x, int &y, int &width,
                                           int &height) {
  if (!m_hMiniViewport.Get()) return;
...
//-----------------------------------------------------------------------------
bool CBaseToolSystem::SetupEngineView(Vector &origin, QAngle &angles,
                                      float &fov) {
  return false;
}
...
bool CBaseToolSystem::ShouldGameRenderView() {
  ...
  if (!m_hMiniViewport.Get()) return true;
...
bool CBaseToolSystem::IsThirdPersonCamera() { return false; }

bool CBaseToolSystem::IsToolRecording() { return false; }

IMaterialProxy *CBaseToolSystem::LookupProxy(const char *proxyName) {
  return NULL;
}

bool CBaseToolSystem::GetSoundSpatialization(int iUserData, int guid,
...
void CBaseToolSystem::RenderFrameBegin() {
  ...
  if (!IsVisible() || !IsActiveTool()) return;

  if (!m_hMiniViewport.Get() || !m_hMiniViewport->IsVisible()) return;
...
void CBaseToolSystem::VGui_PreSimulate() {
  ...
  if (gameui != 0) {
    bool wantsToBeSeen =
        IsGameInputEnabled() &&
        (enginetools->IsGamePaused() || !enginetools->IsInGame() ||
         enginetools->IsConsoleVisible());
    vgui::ipanel()->SetVisible(gameui, wantsToBeSeen);
  }

  // if there's no map loaded and we're in fullscreen toolMode, switch to
  // gameMode otherwise there's nothing to see or do...
...
bool CBaseToolSystem::ShowUI(bool bVisible) {
  ...
  if (bPrevVisible == bVisible) return bPrevVisible;

  SetMouseInputEnabled(bVisible);
  SetVisible(bVisible);

  // Hide loading image if using bx movie UI
  // A bit of a hack because it more or less tunnels through to the client .dll,
  // but the moviemaker assumes
...
//-----------------------------------------------------------------------------
// Gets the action target to sent to panels so that the tool system's OnCommand
// is called
//-----------------------------------------------------------------------------
vgui::Panel *CBaseToolSystem::GetActionTarget() { return this; }
...
//-----------------------------------------------------------------------------
// Purpose: Derived classes implement this to create a custom status bar, or
...
//-----------------------------------------------------------------------------
vgui::Menu *CBaseToolSystem::GetActionMenu() { return m_hActionMenu; }
...
void CBaseToolSystem::OnFileStateMachineFinished(KeyValues *pKeyValues) {
  ...
  vgui::FileOpenStateMachine::CompletionState_t state =
      (vgui::FileOpenStateMachine::CompletionState_t)pKeyValues->GetInt(
          "completionState", vgui::FileOpenStateMachine::IN_PROGRESS);
...
//-----------------------------------------------------------------------------
void CBaseToolSystem::OpenFile(const char *pOpenFileType,
                               const char *pSaveFileName,
                               const char *pSaveFileType, int nFlags,
                               KeyValues *pContextKeyValues) {
  m_pFileOpenStateMachine->OpenFile(pOpenFileType, pContextKeyValues,
                                    pSaveFileName, pSaveFileType, nFlags);
}

void CBaseToolSystem::OpenFile(const char *pOpenFileName,
                               const char *pOpenFileType,
                               const char *pSaveFileName,
                               const char *pSaveFileType, int nFlags,
                               KeyValues *pContextKeyValues) {
  m_pFileOpenStateMachine->OpenFile(pOpenFileName, pOpenFileType,
                                    pContextKeyValues, pSaveFileName,
                                    pSaveFileType, nFlags);
}

//-----------------------------------------------------------------------------
// Used to save a specified file, and deal with all the lovely dialogs
//-----------------------------------------------------------------------------
void CBaseToolSystem::SaveFile(const char *pFileName, const char *pFileType,
                               int nFlags, KeyValues *pContextKeyValues) {
  m_pFileOpenStateMachine->SaveFile(pContextKeyValues, pFileName, pFileType,
                                    nFlags);
}
...
void CBaseToolSystem::PaintBackground() {
  ...
  if (m_pBackground) {
    ...
    RenderQuad(m_pBackground, x, y, w, h, surface()->GetZPos(), 0.0f, 0.0f,
               maxu, maxv, Color(255, 255, 255, 255));
  }
  ...
  if (m_pLogo) {
    ...
    RenderQuad(m_pLogo, x, y, w, h, surface()->GetZPos(), 0.0f, 0.0f, 1.0f,
               1.0f, Color(255, 255, 255, alpha));
  }
}
...
bool CBaseToolSystem::HasDocument() { return false; }
...
CMiniViewport *CBaseToolSystem::CreateMiniViewport(vgui::Panel *parent) {
  ...
  int offset = 4;
  vp->SetBounds((2 * w / 3) - offset, menuBarHeight + offset, w / 3,
                h / 3 + titleBarHeight);
  return vp;
}
...
void CBaseToolSystem::ComputeMenuBarTitle(char *buf, size_t buflen) {
  Q_snprintf(buf, buflen, ": %s [ %s - Switch Mode ] [ %s - Full Screen ]",
             IsGameInputEnabled() ? "Game Mode" : "Tool Mode",
             TOGGLE_INPUT_KEY_NAME, TOGGLE_WINDOWED_KEY_NAME);
}
