...
void CMiniViewportEngineRenderArea::RenderFrameBegin() {
  if (!enginetools->IsInGame()) return;
...
void CMiniViewportEngineRenderArea::InitSceneMaterials() {
  ...
  if (g_pMaterialSystem->IsTextureLoaded("_rt_LayoffResult")) {
    ITexture* pTexture = g_pMaterialSystem->FindTexture(
        "_rt_LayoffResult", TEXTURE_GROUP_RENDER_TARGET);
    ...
    g_pMaterialSystem
        ->BeginRenderTargetAllocation();  // Begin allocating RTs which IFM can
                                          // scribble into
    ...
    g_pMaterialSystem->EndRenderTargetAllocation();  // End allocating RTs which
                                                     // IFM can scribble into
  }
  ...
  m_ScreenMaterial.Init("MiniViewportEngineRenderAreaSceneMaterial",
                        pVMTKeyValues);
...
// Paints the black borders around the engine window
//-----------------------------------------------------------------------------
void CMiniViewportEngineRenderArea::PaintEngineBorders(int x, int y, int w,
                                                       int h) {
...
void CMiniViewportEngineRenderArea::PaintOverlayText() {
  if (!m_OverlayText.Length()) return;
  ...
  g_pMatSystemSurface->GetTextSize(m_OverlayTextFont, pTemp, nTextWidth,
                                   nTextHeight);
  ...
  g_pMatSystemSurface->DrawColoredTextRect(m_OverlayTextFont, lx, ly, cw - lx,
                                           ch - ly, 255, 255, 255, 255,
                                           m_OverlayText.Get());
}
...
// Paints the engine window	itself
//-----------------------------------------------------------------------------
void CMiniViewportEngineRenderArea::PaintEngineWindow(int x, int y, int w,
                                                      int h) {
  if (!enginetools->IsInGame()) {
    ...
    g_pMatSystemSurface->DrawSetTextureMaterial(m_nEngineOutputTexture,
                                                m_ScreenMaterial);
    surface()->DrawSetColor(Color(0, 0, 0, 255));
...
void CMiniViewportEngineRenderArea::GetEngineBounds(int& x, int& y, int& w,
                                                    int& h) {
  x = 0;
  y = 0;
  GetSize(w, h);
...
CMiniViewportPropertyPage::CMiniViewportPropertyPage(Panel* parent,
...
void CMiniViewportPropertyPage::GetEngineBounds(int& x, int& y, int& w,
                                                int& h) {
...
CMiniViewport::CMiniViewport(vgui::Panel* parent, bool contextLabel,
                             vgui::IToolWindowFactory* factory /*= 0*/,
                             vgui::Panel* page /*= NULL*/,
                             char const* title /*= NULL*/,
                             bool contextMenu /*= false*/)
    : BaseClass(parent, contextLabel, factory, page, title, contextMenu,
                false) {
