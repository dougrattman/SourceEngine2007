...
// Constructor
//-----------------------------------------------------------------------------
CToolMenuButton::CToolMenuButton(Panel *parent, const char *panelName,
                                 const char *text, Panel *actionTarget)
    : BaseClass(parent, panelName, text), m_pActionTarget(actionTarget) {
...
int CToolMenuButton::AddMenuItem(char const *itemName, const char *itemText,
...
int CToolMenuButton::AddCheckableMenuItem(
    char const *itemName, const char *itemText, KeyValues *message,
...
int CToolMenuButton::AddMenuItem(char const *itemName, const wchar_t *itemText,
                                 KeyValues *message, Panel *target,
                                 const KeyValues *userData /*= NULL*/,
                                 char const *kbcommandname /*= NULL*/) {
...
int CToolMenuButton::AddCheckableMenuItem(
    char const *itemName, const wchar_t *itemText, KeyValues *message,
    Panel *target, const KeyValues *userData /*= NULL*/,
    char const *kbcommandname /*= NULL*/) {
  int id = m_pMenu->AddCheckableMenuItem(itemName, itemText, message, target,
                                         userData);
  MenuItem_t item;
  item.m_ItemID = id;
  if (kbcommandname) {
...
void CToolMenuButton::AddSeparator() { m_pMenu->AddSeparator(); }
...
int CToolMenuButton::FindMenuItem(char const *itemName) {
  ...
  if (id == m_Items.InvalidIndex()) return -1;
...
void CToolMenuButton::SetCurrentKeyBindingLabel(char const *itemName,
                                                char const *binding) {
  int id = FindMenuItem(itemName);
  if (id != -1) {
...
void CToolMenuButton::UpdateMenuItemKeyBindings() {
  if (!m_pActionTarget) return;

  int c = m_Items.Count();
  for (int i = 0; i < c; ++i) {
    if (!m_Items[i].m_KeyBinding.IsValid()) continue;
    ...
    wchar_t sz[256];
    wcsncpy(sz,
            Panel::KeyCodeModifiersToDisplayString((KeyCode)kb->keycode,
                                                   kb->modifiers),
            256);
    sz[255] = L'\0';
...
vgui::Menu *CToolMenuButton::GetMenu() { return m_pMenu; }
