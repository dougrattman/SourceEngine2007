...
//		2) Forward all mouse messages to the tool workspace so action
//menus work
//		3) Forward all commands to the tool system so all smarts can
//reside there 		4) Control the size of the menu bar + the working area
//=============================================================================
...
// Constructor
//-----------------------------------------------------------------------------
CToolUI::CToolUI(vgui::Panel *pParent, const char *panelName,
                 CBaseToolSystem *pBaseToolSystem)
    : BaseClass(pParent, panelName),
      m_pClientArea(0),
      m_pBaseToolSystem(pBaseToolSystem) {
...
vgui::Panel *CToolUI::GetClientArea() { return m_pClientArea; }
...
void CToolUI::PerformLayout() {
  ...
  int iWidth, iHeight;
  vgui::VPANEL parent = GetParent() ? GetParent()->GetVPanel()
                                    : vgui::surface()->GetEmbeddedPanel();
  ...
  m_pClientArea->SetBounds(0, insettop, iWidth,
                           iHeight - insettop - insetbottom);
}

//-----------------------------------------------------------------------------
// Returns the menu bar
//-----------------------------------------------------------------------------
vgui::MenuBar *CToolUI::GetMenuBar() { return m_pMenuBar; }

vgui::Panel *CToolUI::GetStatusBar() { return m_pStatusBar; }
...
void CToolUI::OnCommand(const char *cmd) { m_pBaseToolSystem->OnCommand(cmd); }
