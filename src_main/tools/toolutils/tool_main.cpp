...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CToolDictionary, IToolDictionary,
                                  VTOOLDICTIONARY_INTERFACE_VERSION,
                                  g_ToolDictionary);
...
bool CToolDictionary::Connect(CreateInterfaceFn factory) {
  ...
  debugoverlay =
      (IVDebugOverlay *)factory(VDEBUG_OVERLAY_INTERFACE_VERSION, NULL);

  if (!enginevgui || !debugoverlay || !g_pCVar || !enginetools ||
      !g_pFileSystem || !p4)
    return false;
