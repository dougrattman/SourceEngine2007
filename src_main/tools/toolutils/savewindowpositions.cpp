...
//-----------------------------------------------------------------------------
// Purpose: This will save the bounds and the visibility state of UI elements
...
class CWindowPositionMgr : public IWindowPositionMgr {
 public:
  ...
  virtual bool LoadPositions(char const *filename, Panel *parent,
                             vgui::IToolWindowFactory *factory, char const *key,
                             bool force = false);
  virtual void RegisterPanel(char const *saveName, Panel *panel,
                             bool contextMenu);
...
CWindowPositionMgr::LoadInfo_t *CWindowPositionMgr::Find(Panel *panel) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (info->m_hPanel.Get() == panel) return info;
  }
  return NULL;
}

CWindowPositionMgr::LoadInfo_t *CWindowPositionMgr::Find(
  ...
  for (int i = 0; i < c; ++i) {
    LoadInfo_t *info = &m_Panels[i];
    if (!Q_stricmp(info->m_Name.String(), panelName)) return info;
  }
  return NULL;
}
...
void CWindowPositionMgr::SavePositions(char const *filename, char const *key) {
  ...
  FileHandle_t h =
      g_pFullFileSystem->Open(filename, "wb", "DEFAULT_WRITE_PATH");
...
bool CWindowPositionMgr::LoadPositions(char const *filename,
                                       vgui::Panel *parent,
                                       vgui::IToolWindowFactory *factory,
                                       char const *key, bool force /*=false*/) {
  bool success = false;
  ...
  if (kv->LoadFromFile(g_pFullFileSystem, filename, "GAME")) {
    // Walk through tools
    for (KeyValues *tw = kv->GetFirstSubKey(); tw != NULL;
         tw = tw->GetNextKey()) {
      if (Q_stricmp(tw->GetName(), "toolwindow")) continue;

      // read bounds
      float fx, fy, fw, fh;
      int x, y, w, h;
      char const *bounds = tw->GetString("bounds", "");
      if (!bounds || !bounds[0]) continue;

      if (4 != sscanf(bounds, "%f %f %f %f", &fx, &fy, &fw, &fh)) continue;
      ...
      ToolWindow *newTool =
          factory->InstanceToolWindow(parent, true, NULL, NULL, false);
      newTool->SetBounds(x, y, w, h);

      for (KeyValues *page = pages->GetFirstSubKey(); page != NULL;
           page = page->GetNextKey()) {
        if (Q_stricmp(page->GetName(), "panel")) continue;

        char const *pageName = page->GetString();
        if (!pageName || !pageName[0]) continue;

        LoadInfo_t *info = Find(pageName);
        if (!info) continue;

        newTool->AddPage(info->m_hPanel.Get(), info->m_Name.String(),
                         info->m_bContextMenu);
        success = true;
      }
...
void CWindowPositionMgr::RegisterPanel(char const *saveName, Panel *panel,
                                       bool contextMenu) {
  char const *panelName = panel->GetName();
  if (!panelName || !panelName[0]) {
    Warning(
        "CWindowPositionMgr::RegisterPanel:  Panel has NULL or blank "
        "name!!!\n");
    return;
  }
...
void CWindowPositionMgr::UnregisterPanel(vgui::Panel *panel) {
  int c = m_Panels.Count();
  for (int i = c - 1; i >= 0; --i) {
    if (m_Panels[i].m_hPanel.Get() != panel) continue;
