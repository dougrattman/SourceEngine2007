...
static StandardParam_t g_pStandardParams[] = {
    {"$surfaceprop", SHADER_PARAM_TYPE_STRING, "default",
     "surfacepropertypicker", "surfacePropertyName"},
    {"%detailtype", SHADER_PARAM_TYPE_STRING, "", "detailtypepicker",
     "detailTypeName"},
...
//-----------------------------------------------------------------------------
void CVMTDoc::NotifyDataChanged(const char *pReason, int nNotifySource,
                                int nNotifyFlags) {
...
bool CVMTDoc::CreateRootElement() {
  ...
  if (m_hRoot == DMELEMENT_HANDLE_INVALID) return false;

  g_pDataModel->SetFileRoot(fileid, m_hRoot);

  // Each VMT list needs to have an editortype associated with it so it displays
  // nicely in editors
  ...
  for (int i = 0; g_pStandardParams[i].m_pParamName; ++i) {
    AddNewShaderParam(m_hRoot, g_pStandardParams[i].m_pParamName,
                      g_pStandardParams[i].m_ParamType,
                      g_pStandardParams[i].m_pDefaultValue);

    if (g_pStandardParams[i].m_pParamName[0] == '%') {
      m_pCallback->AddToolParameter(g_pStandardParams[i].m_pParamName,
                                    g_pStandardParams[i].m_pWidgetType,
                                    g_pStandardParams[i].m_pTextType);
    } else if (g_pStandardParams[i].m_pWidgetType ||
               g_pStandardParams[i].m_pTextType) {
      m_pCallback->RemoveShaderParameter(g_pStandardParams[i].m_pParamName);
      m_pCallback->AddShaderParameter(g_pStandardParams[i].m_pParamName,
                                      g_pStandardParams[i].m_pWidgetType,
                                      g_pStandardParams[i].m_pTextType);
    }
  }
...
void CVMTDoc::CopyParamsFromVMT(CDmElement *pVMT) {
    ...
    switch (dstType) {
      ...
      case AT_COLOR:
        if (srcType == AT_VECTOR3) {
          ...
          v *= 255.0f;
          u8 r = std::clamp(v[0], 0.f, 255.f);
          u8 g = std::clamp(v[1], 0.f, 255.f);
          u8 b = std::clamp(v[2], 0.f, 255.f);
          pDst->SetValue(Color{r, g, b, 255});
        }
        break;
    }
  }
  ...
  for (pDst = m_hRoot->FirstAttribute(); pDst; pDst = pDst->NextAttribute()) {
    if (!IsShaderParam(pDst)) continue;
    ...
    if (!pVMT->HasAttribute(pDst->GetName())) {
      ...
      if (!Q_stricmp(pDst->GetName(), "$alpha")) {
        ...
        pDst->SetValue(Color{255, 255, 255, 255});
      } else {
...
void CVMTDoc::SetupPreviewMaterial() {
  ...
  if (!g_pFileSystem->FullPathToRelativePath(m_pFileName, pLocalName,
                                             sizeof(pLocalName))) {
  ...
  pMaterial = g_pMaterialSystem->FindMaterial(pMaterialName,
                                              "Editable material", false);
  if (!pMaterial || pMaterial->IsErrorMaterial()) goto noMaterialConnection;
...
bool CVMTDoc::LoadFromFile(const char *pFileName) {
  ...
  // change the filename of all the elements under the root, so we can unload
  // the imported elements later
...
CDmElement *CVMTDoc::ExtractDefaultParameters() {
  ...
  for (; pAttribute; pAttribute = pNextAttribute) {
    ...
    for (i = 0; i < nCount; ++i) {
      ...
      if (!Q_stricmp(pShaderParam, pFlagName)) break;
    }
    ...
    // TODO(d.rattman): We can't do this.. the defaults in the strings need to
    // be changed to make it so they actually match the true defaults
    continue;
    ...
    for (i = 0; i < nCount; ++i) {
      ...
      CUtlBuffer buf(temp, sizeof(temp),
                     CUtlBuffer::TEXT_BUFFER | CUtlBuffer::EXTERNAL_GROWABLE);
      CUtlBuffer buf2(temp2, sizeof(temp2),
                      CUtlBuffer::TEXT_BUFFER | CUtlBuffer::EXTERNAL_GROWABLE);
      pAttribute->Serialize(buf);
      SetAttributeValueFromDefault(pMaterial, pAttribute,
                                   m_pCurrentIShader->GetParamDefault(i));
      pAttribute->Serialize(buf2);

      if ((buf.TellMaxPut() == buf2.TellMaxPut()) &&
          !memcmp(buf.Base(), buf2.Base(), buf.TellMaxPut())) {
        // Buffers match! Therefore it's still using the default parameter
        pMaterial->RemoveAttributeByPtr(pAttribute);
      } else {
...
bool CVMTDoc::SaveToFile() {
  if (m_hRoot.Get() && m_pFileName && m_pFileName[0]) {
    ...
    bool bOk = g_pDataModel->SaveToFile(m_pFileName, NULL, "keyvalues", "vmt",
                                        pSaveRoot);
...
bool CVMTDoc::IsShaderParam(CDmAttribute *pAttribute) {
  ...
  for (int i = 0; g_pStandardParams[i].m_pParamName; ++i) {
    if (!Q_stricmp(g_pStandardParams[i].m_pParamName, pName)) return false;
  }
...
void CVMTDoc::RemoveAllShaderParams(CDmElement *pMaterial) {
  ...
  CDmAttribute *pNextAttribute = NULL;
  for (pAttribute = pMaterial->FirstAttribute(); pAttribute;
...
//-----------------------------------------------------------------------------
void CVMTDoc::RemoveUnusedShaderParams(CDmElement *pMaterial, IShader *pShader,
                                       IShader *pOldShader) {
  CDmAttribute *pAttribute = pMaterial->FirstAttribute();
  CDmAttribute *pNextAttribute = NULL;
  for (; pAttribute; pAttribute = pNextAttribute) {
    ...
    for (i = 0; i < nCount; ++i) {
      const char *pFlagName = g_pMaterialSystem->ShaderFlagName(i);
      if (!Q_stricmp(pAttribute->GetName(), pFlagName)) break;
    }
    ...
    // Remove parameters we've currently got but which don't exist in the new
    // shader
    ...
    if (pOldShader) {
      ...
      for (i = 0; i < nCount; ++i) {
        ...
        SetAttributeValueFromDefault(pMaterial, pAttribute,
                                     pOldShader->GetParamDefault(i));
        pAttribute->Serialize(buf2);

        if ((buf1.TellMaxPut() == buf2.TellMaxPut()) &&
            !memcmp(buf1.Base(), buf2.Base(), buf1.TellMaxPut())) {
          // Buffers match! Therefore it's still using the default parameter
          m_pCallback->RemoveShaderParameter(pAttribute->GetName());
          pMaterial->RemoveAttributeByPtr(pAttribute);
        } else {
...
//-----------------------------------------------------------------------------
CDmAttribute *CVMTDoc::AddAttributeForShaderParameter(
    CDmElement *pMaterial, const char *pParamName,
...
//-----------------------------------------------------------------------------
bool CVMTDoc::SetVMatrixParamValue(CDmAttribute *pAttribute,
                                   const char *pValue) {
  // TODO(d.rattman): Change default strings to match DME?
  // Then we could remove this crap
  VMatrix mat;
  int count =
      sscanf(pValue, " [ %f %f %f %f  %f %f %f %f  %f %f %f %f  %f %f %f %f ]",
...
// A couple methods to set vmatrix param values from strings (OLD METHOD!)
//-----------------------------------------------------------------------------
bool CVMTDoc::SetVector2DParamValue(CDmAttribute *pAttribute,
                                    const char *pValue) {
  Vector2D vec;
...
// A couple methods to set vmatrix param values from strings (OLD METHOD!)
//-----------------------------------------------------------------------------
bool CVMTDoc::SetVector3DParamValue(CDmAttribute *pAttribute,
                                    const char *pValue) {
  Vector vec;
...
// A couple methods to set vmatrix param values from strings (OLD METHOD!)
//-----------------------------------------------------------------------------
bool CVMTDoc::SetVector4DParamValue(CDmAttribute *pAttribute,
                                    const char *pValue) {
  Vector4D vec;
  int count =
      sscanf(pValue, " [ %f %f %f %f ]", &vec[0], &vec[1], &vec[2], &vec[3]);
  if (count == 4) {
    pAttribute->SetValue(vec);
    return true;
  }
...
bool CVMTDoc::SetColorParamValue(CDmAttribute *pAttribute, const char *pValue) {
  ...
  if (count == 3) {
    vec *= 255.0f;
    u8 r = std::clamp(vec[0], 0.f, 255.f);
    u8 g = std::clamp(vec[1], 0.f, 255.f);
    u8 b = std::clamp(vec[2], 0.f, 255.f);
    pAttribute->SetValue(Color(r, g, b, 255));
    return true;
  }
...
//-----------------------------------------------------------------------------
void CVMTDoc::SetAttributeValueFromDefault(CDmElement *pMaterial,
                                           CDmAttribute *pAttribute,
                                           const char *pValue) {
  // TODO(d.rattman): Change default strings to match DME?
  // Then we could remove this crap
  switch (pAttribute->GetType()) {
    case AT_VMATRIX:
      if (SetVMatrixParamValue(pAttribute, pValue)) return;
      break;
    case AT_COLOR:
      if (SetColorParamValue(pAttribute, pValue)) return;
      break;
    case AT_VECTOR2:
      if (SetVector2DParamValue(pAttribute, pValue)) return;
      break;
    case AT_VECTOR3:
      if (SetVector3DParamValue(pAttribute, pValue)) return;
      break;
    case AT_VECTOR4:
      if (SetVector4DParamValue(pAttribute, pValue)) return;
      break;
  }
...
//-----------------------------------------------------------------------------
void CVMTDoc::AddNewShaderParam(CDmElement *pMaterial, const char *pParamName,
                                ShaderParamType_t paramType,
                                const char *pValue) {
  char temp[512];
  ...
  for (pAttribute = pMaterial->FirstAttribute(); pAttribute;
    ...
    if (!Q_stricmp(pAttributeName, pParamName)) return;
  }
...
void CVMTDoc::SetShader(const char *pShaderName) {
  ...
  g_pMaterialSystem->GetShaderFallback(pShaderName, pActualShaderName,
                                       sizeof(pActualShaderName));
...
void CVMTDoc::UpdatePreviewMaterial() {
  ...
  char buf[1024];
  CUtlBuffer vmtBuf(buf, sizeof(buf),
                    CUtlBuffer::TEXT_BUFFER | CUtlBuffer::EXTERNAL_GROWABLE);
...
//-----------------------------------------------------------------------------
void CVMTDoc::OnDataChanged(const char *pReason, int nNotifySource,
                            int nNotifyFlags) {
