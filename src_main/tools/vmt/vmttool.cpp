...
//-----------------------------------------------------------------------------
char const *GetVGuiControlsModuleName() { return "VMTTool"; }

//-----------------------------------------------------------------------------
// Connect, disconnect
//-----------------------------------------------------------------------------
bool ConnectTools(CreateInterfaceFn factory) {
  return (g_pMDLCache != NULL) && (studiorender != NULL) &&
         (materials != NULL) && (g_pMatSystemSurface != NULL);
}
...
//-----------------------------------------------------------------------------
class CVMTTool : public CBaseToolSystem,
                 public IFileMenuCallbacks,
                 public IVMTDocCallback {
 ...
 public:
  ...
  virtual void AddToolParameter(const char *pParam, const char *pWidget,
                                const char *pTextType);
  ...
  virtual bool OnReadFileFromDisk(const char *pFileName,
                                  ...
                                  KeyValues *pContextKeyValues);
  virtual bool OnWriteFileToDisk(const char *pFileName, const char *pFileFormat,
                                 KeyValues *pContextKeyValues);
  virtual void OnFileOperationCompleted(
      const char *pFileType, bool bWroteFile,
 ...
 public:
  ...
  KEYBINDING_FUNC(redo, KEY_Z, vgui::MODIFIER_CONTROL | vgui::MODIFIER_SHIFT,
                  OnRedo, "#redo_help", 0);
  void OnDescribeUndo();
...
void CreateTools() { g_pVMTTool = new CVMTTool(); }
...
//-----------------------------------------------------------------------------
void CVMTTool::OnToolActivate() { BaseClass::OnToolActivate(); }

void CVMTTool::OnToolDeactivate() { BaseClass::OnToolDeactivate(); }

//-----------------------------------------------------------------------------
// Hides standard fields
//-----------------------------------------------------------------------------
void CVMTTool::HideStandardFields(CDmeEditorType *pEditorType,
                                  int nFieldFlags) {
  CDmeEditorAttributeInfo *pInfo;

  if (nFieldFlags & EDITOR_FIELD_NAME) {
    pInfo = CreateElement<CDmeEditorAttributeInfo>("name info",
                                                   pEditorType->GetFileId());
  ...
  if (nFieldFlags & EDITOR_FIELD_TYPE) {
    pInfo = CreateElement<CDmeEditorAttributeInfo>("type info",
                                                   pEditorType->GetFileId());
    pInfo->m_bIsVisible = false;
  ...
  if (nFieldFlags & EDITOR_FIELD_ID) {
    pInfo = CreateElement<CDmeEditorAttributeInfo>("id info",
                                                   pEditorType->GetFileId());
    pInfo->m_bIsVisible = false;
  ...
  if (nFieldFlags & EDITOR_FIELD_EDITORTYPE) {
    pInfo = CreateElement<CDmeEditorAttributeInfo>("editor type info",
                                                   pEditorType->GetFileId());
    pInfo->m_bIsVisible = false;
...
//-----------------------------------------------------------------------------
void CVMTTool::AddShaderParameter(const char *pParam, const char *pWidget,
                                  const char *pTextType) {
  // anims only accept activity names
  CDmeEditorAttributeInfo *pInfo = CreateElement<CDmeEditorAttributeInfo>(
      "shader param info", m_pVMTType->GetFileId());
...
void CVMTTool::AddFlagParameter(const char *pParam) {
  ...
  CDmeEditorAttributeInfo *pInfo = CreateElement<CDmeEditorAttributeInfo>(
      "flag param info", m_pVMTType->GetFileId());
  m_pVMTType->AddAttributeInfo(pParam, pInfo);
...
void CVMTTool::AddToolParameter(const char *pParam, const char *pWidget,
                                const char *pTextType) {
  Assert(m_pVMTType->GetAttributeInfo(pParam) == NULL);
  ...
  CDmeEditorAttributeInfo *pInfo = CreateElement<CDmeEditorAttributeInfo>(
      "tool param info", m_pVMTType->GetFileId());
  m_pVMTType->AddAttributeInfo(pParam, pInfo);
...
void CVMTTool::InitEditorDict() {
  ...
  g_pEditorTypeDict = CreateElement<CDmeEditorTypeDictionary>(
      "DmeEditorTypeDictionary", DMFILEID_INVALID);
  m_toolElements.AddToTail(g_pEditorTypeDict->GetHandle());

  m_pVMTType = CreateElement<CDmeEditorType>("vmt", DMFILEID_INVALID);
  HideStandardFields(m_pVMTType, EDITOR_FIELD_NAME | EDITOR_FIELD_TYPE |
                                     EDITOR_FIELD_ID | EDITOR_FIELD_EDITORTYPE);
  ...
  pInfo = CreateElement<CDmeEditorAttributeInfo>("shader name info",
                                                 DMFILEID_INVALID);
...
class CVMTMaterialMenuButton : public CToolMenuButton {
 ...
 public:
  CVMTMaterialMenuButton(vgui::Panel *parent, const char *panelName,
                         const char *text, vgui::Panel *pActionSignalTarget);
  virtual void OnShowMenu(vgui::Menu *menu);
};

CVMTMaterialMenuButton::CVMTMaterialMenuButton(vgui::Panel *parent,
                                               const char *panelName,
                                               const char *text,
                                               vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  AddMenuItem("default", "#VMTSetToDefault",
              new KeyValues("OnSetParamsToDefault"), pActionSignalTarget);
  SetMenu(m_pMenu);
}
...
class CVMTViewMenuButton : public CToolMenuButton {
 ...
 public:
  CVMTViewMenuButton(vgui::Panel *parent, const char *panelName,
                     const char *text, vgui::Panel *pActionSignalTarget);
  virtual void OnShowMenu(vgui::Menu *menu);
};

CVMTViewMenuButton::CVMTViewMenuButton(vgui::Panel *parent,
                                       const char *panelName, const char *text,
                                       vgui::Panel *pActionSignalTarget)
    : BaseClass(parent, panelName, text, pActionSignalTarget) {
  AddCheckableMenuItem("properties", "#VMTProperties",
                       new KeyValues("OnToggleProperties"),
                       pActionSignalTarget);
  AddCheckableMenuItem("preview", "#VMTPreview",
                       new KeyValues("OnToggleVMTPreview"),
                       pActionSignalTarget);

  AddSeparator();

  AddCheckableMenuItem("showflagparams", "#VMTShowFlags",
                       new KeyValues("OnShowFlags"), pActionSignalTarget);
  AddCheckableMenuItem("showtoolparams", "#VMTShowToolParams",
                       new KeyValues("OnShowToolParams"), pActionSignalTarget);

  AddSeparator();

  AddMenuItem("defaultlayout", "#VMTViewDefault",
              new KeyValues("OnDefaultLayout"), pActionSignalTarget);
...
vgui::MenuBar *CVMTTool::CreateMenuBar(CBaseToolSystem *pParent) {
  ...
  // Add menu buttons
  CToolMenuButton *pFileButton = CreateToolFileMenuButton(
      m_pMenuBar, "File", "&File", GetActionTarget(), this);
  CToolMenuButton *pEditButton =
      CreateToolEditMenuButton(m_pMenuBar, "Edit", "&Edit", GetActionTarget());
  CToolMenuButton *pMaterialButton = new CVMTMaterialMenuButton(
      m_pMenuBar, "Material", "&Material", GetActionTarget());
  CToolMenuButton *pSwitchButton = CreateToolSwitchMenuButton(
      m_pMenuBar, "Switcher", "&Tools", GetActionTarget());
  CVMTViewMenuButton *pViewButton =
      new CVMTViewMenuButton(m_pMenuBar, "View", "&View", GetActionTarget());
...
//-----------------------------------------------------------------------------
// Derived classes can implement this to get a new scheme to be applied to this
// tool
//-----------------------------------------------------------------------------
vgui::HScheme CVMTTool::GetToolScheme() {
  return vgui::scheme()->LoadSchemeFromFile("Resource/BoxRocket.res",
                                            "BoxRocket");
}
...
vgui::Menu *CVMTTool::CreateActionMenu(vgui::Panel *pParent) {
  ...
  pActionMenu->AddMenuItem(
      "#ToolHide", new KeyValues("Command", "command", "HideActionMenu"),
      GetActionTarget());
  return pActionMenu;
}
...
void CVMTTool::PostMessage(HTOOLHANDLE hEntity, KeyValues *message) {
  if (!Q_stricmp(message->GetName(), "EditMaterial")) {
    const char *pMaterialName =
        message->GetString("material", "debug/debugempty");
    ...
    if (pMaterialName[0] == '/' && pMaterialName[1] == '/' &&
      ...
      g_pFileSystem->RelativePathToFullPath(pLocalPath, "GAME", pAbsPath,
                                            sizeof(pAbsPath));
    }
...
//-----------------------------------------------------------------------------
// Derived classes can implement this to get notified when files are
// saved/loaded
//-----------------------------------------------------------------------------
void CVMTTool::OnFileOperationCompleted(
    const char *pFileType, bool bWroteFile,
    vgui::FileOpenStateMachine::CompletionState_t state,
    KeyValues *pContextKeyValues) {
  if (bWroteFile) {
...
//-----------------------------------------------------------------------------
void CVMTTool::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                   bool bOpenFile, const char *pFileFormat,
                                   KeyValues *pContextKeyValues) {
...
//-----------------------------------------------------------------------------
bool CVMTTool::OnReadFileFromDisk(const char *pFileName,
                                  const char *pFileFormat,
                                  KeyValues *pContextKeyValues) {
  OnCloseNoSave();
  if (!LoadDocument(pFileName)) return false;
...
// Called by SaveFile to allow clients to actually write the file out
//-----------------------------------------------------------------------------
bool CVMTTool::OnWriteFileToDisk(const char *pFileName, const char *pFileFormat,
                                 KeyValues *pContextKeyValues) {
  if (!m_pDoc) return true;

  m_pDoc->SetFileName(pFileName);
  if (!m_pDoc->SaveToFile()) return false;
...
void CVMTTool::OnNew() {
  if (m_pDoc && m_pDoc->IsDirty()) {
    SaveFile(m_pDoc->GetFileName(), "vmt",
...
void CVMTTool::OpenSpecificFile(const char *pFileName) {
  ...
  if (m_pDoc) {
    ...
    if (!Q_stricmp(m_pDoc->GetFileName(), pFileName)) return;
...
// Inherited from IVMTDocCallback
//-----------------------------------------------------------------------------
void CVMTTool::OnDocChanged(const char *pReason, int nNotifySource,
                            int nNotifyFlags) {
  UpdateMenuBar();
...
// Output : CVMTDoc
//-----------------------------------------------------------------------------
CVMTDoc *CVMTTool::GetDocument() { return m_pDoc; }
...
CVMTPanel *CVMTTool::GetVMTPreview() { return m_hVMTPreview.Get(); }
...
void CVMTTool::InitTools() {
  ...
  if (!windowposmgr->LoadPositions("cfg/vmt.txt", this, &m_ToolWindowFactory,
                                   "VMT")) {
    OnDefaultLayout();
  }
}
...
void CVMTTool::ShowElementProperties() {
  ...
  if (!m_pDoc->GetRootObject()) return;
...
void CVMTTool::CreateTools(CVMTDoc *doc) {
  if (!m_hProperties.Get()) {
    m_hProperties =
        new CBasePropertiesContainer(NULL, m_pDoc, g_pEditorTypeDict);
  }
  if (!m_hVMTPreview.Get()) {
...
void CVMTTool::OnDefaultLayout() {
  ...
  ToolWindow *pPropertyWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, properties, "#VMTProperties", false);
  ToolWindow *pVMTPreviewWindow = m_ToolWindowFactory.InstanceToolWindow(
      GetClientArea(), false, pVMTPreview, "#VMTPreview", false);
...
void CVMTTool::OnShowFlags() {
  ...
  for (int i = 0; i < nCount; ++i) {
    CDmeEditorAttributeInfo *pInfo =
        m_pVMTType->GetAttributeInfo(m_FlagParams[i]);
    Assert(pInfo);
    pInfo->m_bIsVisible = m_bFlagParamsVisible;
  }
  if (m_hProperties.Get()) {
...
void CVMTTool::OnShowToolParams() {
  ...
  for (int i = 0; i < nCount; ++i) {
    CDmeEditorAttributeInfo *pInfo =
        m_pVMTType->GetAttributeInfo(m_ToolParams[i]);
    Assert(pInfo);
    pInfo->m_bIsVisible = m_bToolParamsVisible;
  }
  if (m_hProperties.Get()) {
...
char const *CVMTTool::GetLogoTextureName() { return "vgui/tools/vmt/vmt_logo"; }