...
static void PR_CheckEmptyString(const char *s) {
  if (s[0] <= ' ') Host_Error("Bad string: %s", s);
}
...
class CVEngineServer : public IVEngineServer {
 public:
  virtual void ChangeLevel(const char *s1, const char *s2) {
    ...
    if (sv.GetSpawnCount() == last_spawncount) return;
    ...
    if (!s2)  // no indication of where they are coming from;  so just do a
              // standard old changelevel
    {
  ...
  virtual int PrecacheDecal(const char *name, bool preload /*=false*/) {
    ...
    Host_Error("CVEngineServer::PrecacheDecal: '%s' overflow, too many decals",
               name);
    return 0;
  }
  ...
  virtual int PrecacheModel(const char *s, bool preload /*= false*/) {
    ...
    Host_Error("CVEngineServer::PrecacheModel: '%s' overflow, too many models",
               s);
    return 0;
  }
  ...
  //			*outputpvs - If 0, then return value is the needed
  // length
  ...
  virtual int GetPVSForCluster(int clusterIndex, int outputpvslength,
                               unsigned char *outputpvs) {
    int length = (CM_NumClusters() + 7) >> 3;

    if (outputpvs) {
      if (outputpvslength < length) {
        Sys_Error(
            "GetPVSForOrigin called with inusfficient sized pvs array, need %i "
            "bytes!",
            length);
        return length;
      }
  ...
  virtual bool CheckOriginInPVS(const Vector &org,
    ...
    if (offset > checkpvssize) {
      Sys_Error(
          "CheckOriginInPVS:  cluster would read past end of pvs data "
          "(%i:%i)\n",
          offset, checkpvssize);
      return false;
    }
  ...
  virtual bool CheckBoxInPVS(const Vector &mins, const Vector &maxs,
                             const unsigned char *checkpvs, int checkpvssize) {
  ...
  virtual int IndexOfEdict(const edict_t *pEdict) {
    ...
    if (index < 0 || index > sv.max_edicts) {
      Sys_Error(
          "Bad entity in IndexOfEdict() index %i pEdict %p sv.edicts %p\n",
  ...
  virtual void RemoveEdict(edict_t *ed) { ED_Free(ed); }

  //
  // Request engine to allocate "cb" bytes on the entity's private data pointer.
  //
  virtual void *PvAllocEntPrivateData(long cb) { return calloc(1, cb); }
  ...
  EmitAmbientSound
    ...
    =================
  */
  virtual void EmitAmbientSound(int entindex, const Vector &pos,
                                const char *samp, float vol,
    ...
    if (TestSoundChar(samp, CHAR_SENTENCE)) {
      ...
      if (sound.nSoundNum >= VOX_SentenceCount()) {
        ConMsg("EmitAmbientSound: invalid sentence number: %s",
               PSkipSoundChars(samp));
        return;
      }
    } else {
    ...
    if ((fFlags & SND_SPAWNING) && sv.allowsignonwrites) {
      ...
      if (fFlags & SND_SPAWNING) {
        DevMsg(
            "EmitAmbientSound: warning, broadcasting sound labled as "
            "SND_SPAWNING.\n");
      }
  ...
  virtual void FadeClientVolume(const edict_t *clientent, float fadePercent,
    ...
    NET_StringCmd sndMsg(va("soundfade	%.1f %.1f %.1f %.1f", fadePercent,
                            holdTime, fadeOutSeconds, fadeInSeconds));
  ...
  virtual int CheckHeadnodeVisible(int nodenum, const uint8_t *visbits,
                                   int vissize) {
  ...
  */
  ...
  virtual void StaticDecal(const Vector &origin, int decalIndex,
                           int entityIndex, int modelIndex, bool lowpriority) {
    SVC_BSPDecal decal;
  ...
  */
  ...
  virtual bf_write *EntityMessageBegin(int ent_index, ServerClass *ent_class,
    ...
    if (s_MsgData.started) {
      Sys_Error(
          "EntityMessageBegin:  New message started before matching call to "
          "EndMessage.\n ");
      return NULL;
    }
  ...
  virtual bf_write *UserMessageBegin(IRecipientFilter *filter, int msg_index) {
    if (s_MsgData.started) {
      Sys_Error(
          "UserMessageBegin:  New message started before matching call to "
          "EndMessage.\n ");
      return NULL;
    }
  ...
  int Message_CheckMessageLength() {
    if (s_MsgData.currentMsg == &s_MsgData.userMsg) {
      ...
      if (!serverGameDLL->GetUserMessageInfo(msgtype, msgname, sizeof(msgname),
                                             msgsize)) {
      ...
      if (msgsize == -1) {
        if (bytesWritten > MAX_USER_MSG_DATA) {
          Warning(
              "DLL_MessageEnd:  Refusing to send user message %s of %i bytes "
              "to client, user message size limit is %i bytes\n",
    ...
    if (s_MsgData.currentMsg == &s_MsgData.entityMsg) {
      ...
      if (bytesWritten > MAX_ENTITY_MSG_DATA)  // TODO use a define or so
      {
        Warning("Entity Message to %i, %i bytes written (max is %d)\n",
                s_MsgData.entityMsg.m_nEntityIndex, bytesWritten,
                MAX_ENTITY_MSG_DATA);
        return -1;
      }
  ...
  virtual void ResetPVS(byte *pvs, int pvssize) { ::SV_ResetPVS(pvs, pvssize); }

  virtual void SetAreaPortalState(int portalNumber, int isOpen) {
    CM_SetAreaPortalState(portalNumber, isOpen);
  }

  virtual void SetAreaPortalStates(const int *portalNumbers, const int *isOpen,
                                   int nPortals) {
  ...
  virtual void DrawMapToScratchPad(IScratchPad3D *pPad, unsigned long iFlags) {
    ...
    SurfaceHandle_t surfID = SurfaceHandleFromIndex(
        host_state.worldmodel->brush.firstmodelsurface, pData);
    for (int i = 0; i < host_state.worldmodel->brush.nummodelsurfaces;
         ++i, ++surfID) {
  ...
  const CBitVec<MAX_EDICTS> *GetEntityTransmitBitsForClient(int iClientIndex) {
    ...
    if (!deltaFrame) return NULL;

    return &deltaFrame->transmit_entity;
  }

  virtual bool IsPaused() { return sv.IsPaused(); }
  ...
  virtual void AllowImmediateEdictReuse() { ED_AllowImmediateReuse(); }
  ...
  virtual IAchievementMgr *GetAchievementMgr() { return g_pAchievementMgr; }

  virtual int GetAppID() { return g_iSteamAppID; }
  ...
  */
  ...
  bool IsClientFullyAuthenticated(edict_t *pEdict) {
    int entnum = NUM_FOR_EDICT(pEdict);
    if (entnum < 1 || entnum > sv.GetClientCount()) return false;

    // Entity numbers are offset by 1 from the player numbers
    CGameClient *client = sv.Client(entnum - 1);
    if (client) return client->IsFullyAuthenticated();
 ...
 private:
  ...
  // original MESSAGE_BEGIN stuff from HL1
  ...
  virtual ISpatialPartition *CreateSpatialPartition(const Vector &worldmin,
                                                    const Vector &worldmax) {
    return ::CreateSpatialPartition(worldmin, worldmax);
  }
  virtual void DestroySpatialPartition(ISpatialPartition *pPartition) {
    ::DestroySpatialPartition(pPartition);
  }
};
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVEngineServer, IVEngineServer,
...
void CVEngineServer::PlaybackTempEntity(IRecipientFilter &filter, float delay,
  ...
  if (!SendTable_Encode(pST, pSender, &buffer, classID, NULL, false)) {
    Host_Error(
        "PlaybackTempEntity: SendTable_Encode returned false (ent %d), "
        "overflow? %i\n",
...
bool CVEngineServer::GetAreaPortalPlane(Vector const &vViewOrigin,
                                        int portalKey, VPlane *pPlane) {
...
void CVEngineServer::LogPrint(const char *msg) { g_Log.Print(msg); }
...
void CVEngineServer::LoadAdjacentEnts(const char *pOldLevel,
                                      const char *pLandmarkName) {
#ifndef SWDS
...
const char *CVEngineServer::GetMapEntitiesString() { return CM_EntityString(); }
...
void CVEngineServer::BuildEntityClusterList(edict_t *pEdict,
                                            PVSInfo_t *pPVSInfo) {
  int i, j;
  int topnode;
  int leafCount;
  ...
  leafCount = CM_BoxLeafnums(vecWorldMins, vecWorldMaxs, leafs,
  ...
  for (i = 0; i < leafCount; i++) {
    ...
    if (pPVSInfo->m_nAreaNum && pPVSInfo->m_nAreaNum != area) {
      if (pPVSInfo->m_nAreaNum2 && pPVSInfo->m_nAreaNum2 != area &&
          sv.IsLoading()) {
        ConDMsg("Object touching 3 areas at %f %f %f\n", vecWorldMins[0],
                vecWorldMins[1], vecWorldMins[2]);
      }
...
//-----------------------------------------------------------------------------
void CVEngineServer::SolidMoved(edict_t *pSolidEnt, ICollideable *pSolidCollide,
                                const Vector *pPrevAbsOrigin,
                                bool accurateBboxTriggerChecks) {
  SV_SolidMoved(pSolidEnt, pSolidCollide, pPrevAbsOrigin,
                accurateBboxTriggerChecks);
}

void CVEngineServer::TriggerMoved(edict_t *pTriggerEnt,
                                  bool accurateBboxTriggerChecks) {
