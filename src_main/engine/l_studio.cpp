...
ConVar r_drawmodelstatsoverlaydistance("r_drawmodelstatsoverlaydistance", "500",
                                       FCVAR_CHEAT);
...
static ConVar r_lightaverage("r_lightaverage", "1", 0,
                             "Activates/deactivate light averaging");
static ConVar r_lightinterp(
    "r_lightinterp", "5", FCVAR_CHEAT,
    "Controls the speed of light interpolation, 0 turns off interpolation");
static ConVar r_eyeglintlodpixels("r_eyeglintlodpixels", "20.0", 0,
                                  "The number of pixels wide an eyeball has to "
                                  "be before rendering an eyeglint.  Is a "
                                  "floating point value.");
ConVar r_rootlod("r_rootlod", "0", 0, "Root LOD", true, 0, true,
                 MAX_NUM_LODS - 1, SetRootLOD_f);
static ConVar r_decalstaticprops("r_decalstaticprops", "1", 0,
                                 "Decal static props test");
static ConCommand r_flushlod("r_flushlod", FlushLOD_f,
                             "Flush and reload LODs.");
ConVar r_debugrandomstaticlighting("r_debugrandomstaticlighting", "0",
                                   FCVAR_CHEAT,
                                   "Set to 1 to randomize static lighting for "
                                   "debugging.  Must restart for change to "
                                   "take affect.");
ConVar r_proplightingfromdisk("r_proplightingfromdisk", "1", 0,
...
static ConVar r_proplightingpooling("r_proplightingpooling", "-1.0",
                                    FCVAR_CHEAT,
                                    "0 - off, 1 - static prop color meshes are "
                                    "allocated from a single shared vertex "
                                    "buffer (on hardware that supports stream "
                                    "offset)");
...
static ConVar r_eyeshift_x("r_eyeshift_x", "0",
                           FCVAR_ARCHIVE);  // eye X position
static ConVar r_eyeshift_y("r_eyeshift_y", "0",
                           FCVAR_ARCHIVE);  // eye Y position
static ConVar r_eyeshift_z("r_eyeshift_z", "0",
                           FCVAR_ARCHIVE);  // eye Z position
static ConVar r_eyesize("r_eyesize", "0",
                        FCVAR_ARCHIVE);  // adjustment to iris textures
...
void UpdateStudioRenderConfig(void) {
  ...
  s_StudioRenderConfig.bSoftwareLighting =
      g_pMaterialSystemConfig->bSoftwareLighting;

  s_StudioRenderConfig.bShowEnvCubemapOnly =
      r_showenvcubemap.GetInt() ? true : false;
  s_StudioRenderConfig.fEyeGlintPixelWidthLODThreshold =
      r_eyeglintlodpixels.GetFloat();
  ...
  if (pWorldLight->radius != 0) {
    ...
    if (light.m_Attenuation2 == 0.0f) {
      if (light.m_Attenuation1 == 0.0f) {
        ...
        light.m_Range = (intensity / MIN_LIGHT_VALUE - light.m_Attenuation0) /
                        light.m_Attenuation1;
      }
    } else {
      ...
      if (discrim < 0.0f)
        ...
        if (light.m_Range < 0) light.m_Range = 0;
      }
    }
  }
  light.m_Flags = 0;
...
static void R_SetNonAmbientLightingState(
    int numLights, dworldlight_t *locallight[MAXLOCALLIGHTS],
    int *pNumLightDescs, LightDesc_t *pLightDescs,
    bool bUpdateStudioRenderLights) {
  Assert(numLights >= 0 && numLights <= MAXLOCALLIGHTS);

  // convert dworldlight_t's to LightDesc_t's and send 'em down to
  // g_pStudioRender->
  *pNumLightDescs = 0;
  ...
  for (int i = 0; i < numLights; i++) {
    ...
    if (!WorldLightToMaterialLight(locallight[i], *pLightDesc)) continue;
...
float Engine_WorldLightDistanceFalloff(const dworldlight_t *wl,
  ...
  switch (wl->type) {
    case emit_surface:
      ...
      if (wl->radius != 0) {
        if (DotProduct(delta, delta) > (wl->radius * wl->radius)) return 0.0f;
      }
    ...
    case emit_spotlight:  // directional & positional
    {
      ...
      return 1.f / (wl->constant_attn + wl->linear_attn * dist +
                    wl->quadratic_attn * dist2);
    }
...
float Engine_WorldLightAngle(const dworldlight_t *wl, const Vector &lnormal,
  ...
  switch (wl->type) {
    case emit_surface:
      ...
      if (dot2 <= ON_EPSILON / 10) return 0;  // behind light surface
    ...
    case emit_spotlight:
      ...
      dot2 = -DotProduct(delta, lnormal);
      if (dot2 <= wl->stopdot2) return 0;  // outside light cone

      ratio = dot;
      if (dot2 >= wl->stopdot) return ratio;  // inside inner cone

      if ((wl->exponent == 1) || (wl->exponent == 0)) {
        ratio *= (dot2 - wl->stopdot2) / (wl->stopdot - wl->stopdot2);
      } else {
        ratio *= pow((dot2 - wl->stopdot2) / (wl->stopdot - wl->stopdot2),
                     wl->exponent);
      }
      return ratio;
...
class CPooledVBAllocator_ColorMesh : public IPooledVBAllocator {
 public:
  ...
  // Allocate a sub-range of 'numVerts' from free space in the shared vertex
  // buffer (returns the uint8_t offset from the start of the VB to the new
  // allocation)
 ...
 private:
  // Assert/warn that the allocator is in a clear/empty state (returns FALSE if
  // not)
  bool CheckIsClear(void);

  IMesh *m_pMesh;  // The shared mesh (vertex buffer) from which sub-allocations
                   // are made
  void
      *m_pVertexBufferBase;  // A pointer to the start of the vertex buffer data
  ...
  int m_nextFreeOffset;     // The offset to be returned by the next call to
                            // Allocate()
  // (incremented as a simple stack)
  bool m_bStartedDeallocation;  // This is set when Deallocate() is called for
                                // the first time,
...
struct colormeshparams_t {
  ...
  // Given memory alignment (VBs must be 4-KB aligned on X360, for example), it
  // can be more efficient to allocate many color meshes out of a single shared
  // vertex buffer (using vertex 'stream offset')
...
class CColorMeshData {
 public:
  void DestroyResource() {
    ...
    for (int i = 0; i < m_nMeshes; i++) {
      if (m_pMeshInfos[i].m_pPooledVBAllocator) {
        // Let the pooling allocator dealloc this sub-range of the shared vertex
        // buffer
        m_pMeshInfos[i].m_pPooledVBAllocator->Deallocate(
            m_pMeshInfos[i].m_nVertOffsetInBytes, m_pMeshInfos[i].m_nNumVerts);
      } else {
  ...
  static CColorMeshData *CreateResource(const colormeshparams_t &params) {
    ...
    data->m_nTotalSize =
        params.m_nMeshes * sizeof(IMesh *) + params.m_nTotalVertexes * 4;
    ...
    for (int i = 0; i < params.m_nMeshes; i++) {
      ...
      if (params.m_pPooledVBAllocator != NULL) {
        ...
        data->m_pMeshInfos[i].m_nVertOffsetInBytes =
            params.m_pPooledVBAllocator->Allocate(params.m_nVertexes[i]);
        ...
        if (data->m_pMeshInfos[i].m_nVertOffsetInBytes == -1) {
          ...
          data->m_pMeshInfos[i].m_pMesh =
              params.m_pPooledVBAllocator->GetSharedMesh();
          data->m_ppTargets[i] =
              ((unsigned char *)
                   params.m_pPooledVBAllocator->GetVertexBufferBase()) +
              data->m_pMeshInfos[i].m_nVertOffsetInBytes;
        }
      }
      ...
      if (data->m_pMeshInfos[i].m_pMesh == NULL) {
        ...
        data->m_pMeshInfos[i].m_pMesh = pRenderContext->CreateStaticMesh(
            vertexFormat, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_COLOR);
  ...
  static unsigned int EstimatedSize(const colormeshparams_t &params) {
    // each vertex is a 4 uint8_t color
...
class CModelRender
 ...
 public:
  ...
  virtual void ForcedMaterialOverride(
      IMaterial *newMaterial, OverrideType_t nOverrideType = OVERRIDE_NORMAL);
  virtual int DrawModel(int flags, IClientRenderable *cliententity,
                        ModelInstanceHandle_t instance, int entity_index,
                        const model_t *model, const Vector &origin,
                        QAngle const &angles, int skin, int body, int hitboxset,
  ...
  virtual void AddDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
                        const Vector &decalUp, int decalIndex, int body,
  ...
  virtual matrix3x4_t *DrawModelShadowSetup(IClientRenderable *pRenderable,
                                            int body, int skin,
  ...
  unsigned short &FirstShadowOnModelInstance(ModelInstanceHandle_t handle) {
    return m_ModelInstances[handle].m_FirstShadow;
  }

  // This gets called when overbright, etc gets changed to recompute static prop
  // lighting.
  ...
  virtual bool DrawModelSetup(ModelRenderInfo_t &pInfo,
  ...
  virtual int DrawStaticPropArrayFast(StaticPropRenderInfo_t *pProps, int count,
                                      bool bShadowDepth);
  ...
  inline vertexFileHeader_t *CacheVertexData() {
    return g_pMDLCache->GetVertexData(
        (MDLHandle_t)(uintptr_t)m_pStudioHdr->virtualModel);
  }

  bool Init();
  void Shutdown();

  bool GetItemName(DataCacheClientID_t clientId, const void *pItem, char *pDest,
                   unsigned nMaxLen);
 ...
 private:
  enum { CURRENT_LIGHTING_UNINITIALIZED = -999999 };
  ...
  void InitColormeshParams(ModelInstance_t &instance,
                           studiohwdata_t *pStudioHWData,
                           colormeshparams_t *pColorMeshParams);
  ...
  bool UpdateStaticPropColorData(IHandleEntity *pEnt,
  ...
  bool LoadStaticPropColorData(IHandleEntity *pProp,
                               DataCacheHandle_t colorMeshHandle,
                               studiohwdata_t *pStudioHWData);

  // Returns true if the model instance is valid
  bool IsModelInstanceValid(ModelInstanceHandle_t handle);

  void DebugDrawLightingOrigin(const DrawModelState_t &state,
                               const ModelRenderInfo_t &pInfo);
  ...
  void SnapCurrentLightingState(ModelInstance_t &inst,
                                LightingState_t *pLightingState);

  // Sets up lighting state for rendering
  void StudioSetupLighting(const DrawModelState_t &state,
                           const Vector &absEntCenter,
                           LightCacheHandle_t *pLightcache, bool bVertexLit,
                           bool bNeedsEnvCubemap, bool &bStaticLighting,
                           DrawModelInfo_t &drawInfo,
                           const ModelRenderInfo_t &pInfo, int drawFlags);

  // Time average the ambient term
  void TimeAverageAmbientLight(LightingState_t &actualLightingState,
                               ModelInstance_t &inst, float flAttenFactor,
                               LightingState_t *pLightingState,
                               const Vector *pLightingOrigin);
  ...
  void ComputeModelVertexLighting(IHandleEntity *pProp, mstudiomodel_t *pModel,
                                  OptimizedModel::ModelLODHeader_t *pVtxLOD,
                                  matrix3x4_t &matrix, Vector4D *pTempMem,
                                  color24 *pLighting);
  ...
  // Allocator for static prop color mesh vertex buffers (all are pooled into
  // one VB)
  CPooledVBAllocator_ColorMesh m_colorMeshVBAllocator;
};

static CModelRender s_ModelRender;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CModelRender, IVModelRender,
                                  VENGINE_HUDMODEL_INTERFACE_VERSION,
                                  s_ModelRender);
...
class CResourcePreloadPropLighting : public CResourcePreload {
  virtual bool CreateResource(const char *pName) {
    ...
    if (g_pQueuedLoader->IsSameMapLoading() &&
        s_ModelRender.IsStaticPropColorDataCached(szFilename)) {
    ...
    // create an anonymous job to get the lighting data in memory, claim during
    // static prop instancing
  ...
  virtual void PurgeUnreferencedResources() {
    if (g_pQueuedLoader->IsSameMapLoading()) {
      // do nothing, same map is loading, correct disk prop lighting will still
      // be in data cache
      return;
    }
...
//
//-----------------------------------------------------------------------------
bool CModelRender::GetItemName(DataCacheClientID_t clientId, const void *pItem,
...
// Cause the current lighting state to match the given one
//-----------------------------------------------------------------------------
void CModelRender::SnapCurrentLightingState(ModelInstance_t &inst,
                                            LightingState_t *pLightingState) {
...
void CModelRender::TimeAverageAmbientLight(LightingState_t &actualLightingState,
  ...
  for (int i = 0; i < 6; ++i) {
    VectorSubtract(pLightingState->r_boxcolor[i],
                   inst.m_CurrentLightingState.r_boxcolor[i], vecDelta);
    vecDelta *= flAttenFactor;
    inst.m_CurrentLightingState.r_boxcolor[i] =
        pLightingState->r_boxcolor[i] - vecDelta;
    ...
    if (pLightingOrigin) {
      ...
      CDebugOverlay::AddLineOverlay(
          *pLightingOrigin, *pLightingOrigin + vecDir * 20,
      ...
      CDebugOverlay::AddLineOverlay(
          *pLightingOrigin + Vector(5, 5, 5), *pLightingOrigin + vecDir * 50,
          255 * pLightingState->r_boxcolor[i].x,
          255 * pLightingState->r_boxcolor[i].y,
          255 * pLightingState->r_boxcolor[i].z, 255, true, 5.0f);
    }
#endif
    // haven't been able to find this rare bug which results in ambient light
    // getting "stuck" on the viewmodel extremely rarely , presumably with
    // infinities. So, mask the bug (hopefully) and warn by clamping.
#ifndef NDEBUG
  ...
  memcpy(&actualLightingState.r_boxcolor,
         ...
         sizeof(inst.m_CurrentLightingState.r_boxcolor));
}

//-----------------------------------------------------------------------------
// Do time averaging of the lighting state to avoid popping...
//-----------------------------------------------------------------------------
LightingState_t *CModelRender::TimeAverageLightingState(
    ModelInstanceHandle_t handle, LightingState_t *pLightingState,
    int nEntIndex, const Vector *pLightingOrigin) {
  if (r_lightaverage.GetInt() == 0) return pLightingState;

#ifndef SWDS

  float flInterpFactor = r_lightinterp.GetFloat();
  if (flInterpFactor == 0) return pLightingState;
  ...
  // where vf = this frame's lighting value, vi = current time averaged lighting
  // value
  int i;
  Vector vecDelta;
  float flAttenFactor = exp(-flInterpFactor * dt);
  TimeAverageAmbientLight(actualLightingState, inst, flAttenFactor,
                          pLightingState, pLightingOrigin);
  ...
  for (i = 0; i < pLightingState->numlights; ++i) {
    // By default, assume the light doesn't match an existing light, so blend up
    // from 0
    pLight[i].Init(0.0f, 0.0f, 0.0f);
    int j;
    for (j = 0; j < inst.m_CurrentLightingState.numlights; ++j) {
      if (pLightingState->locallight[i] ==
          inst.m_CurrentLightingState.locallight[j]) {
        // Ok, we found a matching light, so use the intensity of that light at
        // the moment
        ++nMatchCount;
        pMatch[j] = true;
  ...
  // For the lights in the current lighting state, attenuate them toward their
  // actual value
  for (i = 0; i < pLightingState->numlights; ++i) {
    actualLightingState.locallight[i] = &s_WorldLights[i];
    memcpy(&s_WorldLights[i], pLightingState->locallight[i],
           sizeof(dworldlight_t));

    // Light already exists? Attenuate to it...
    VectorSubtract(pLightingState->locallight[i]->intensity, pLight[i],
                   vecDelta);
    vecDelta *= flAttenFactor;

    s_WorldLights[i].intensity =
        pLightingState->locallight[i]->intensity - vecDelta;
  ...
  for (i = 0; i < inst.m_CurrentLightingState.numlights; ++i) {
    if (pMatch[i]) continue;

    // Has it faded out to black? Then remove it.
    if (inst.m_flLightIntensity[i].LengthSqr() < 1) continue;

    if (nCurrLight >= MAXLOCALLIGHTS) break;

    actualLightingState.locallight[nCurrLight] = &s_WorldLights[nCurrLight];
    memcpy(&s_WorldLights[nCurrLight],
           inst.m_CurrentLightingState.locallight[i], sizeof(dworldlight_t));
    ...
    if ((nCurrLight >= nWorldLights) && pLightingOrigin) {
      AddWorldLightToAmbientCube(&s_WorldLights[nCurrLight], *pLightingOrigin,
                                 actualLightingState.r_boxcolor);
    }
  ...
  for (i = 0; i < nCurrLight; ++i) {
    ...
    float flRatio = inst.m_flLightIntensity[i].Length() /
                    pSourceLight[i]->intensity.Length();
    vecColor *= flRatio;
    CDebugOverlay::AddLineOverlay(*pLightingOrigin, pSourceLight[i]->origin,
                                  vecColor.x, vecColor.y, vecColor.z, 255,
                                  false, 5.0f);
#endif
  }
...
static ConVar r_ambientboost(
    "r_ambientboost", "1", FCVAR_ARCHIVE,
    "Set to boost ambient term if it is totally swamped by local lights");
static ConVar r_ambientmin("r_ambientmin", "0.3", FCVAR_ARCHIVE,
                           "Threshold above which ambient cube will not boost "
                           "(i.e. it's already sufficiently bright");
static ConVar r_ambientfraction("r_ambientfraction", "0.1", FCVAR_CHEAT,
                                "Fraction of direct lighting that ambient cube "
                                "must be below to trigger boosting");
static ConVar r_ambientfactor("r_ambientfactor", "5", FCVAR_ARCHIVE,
                              "Boost ambient cube by no more than this factor");
static ConVar r_lightcachemodel("r_lightcachemodel", "-1", FCVAR_CHEAT, "");
static ConVar r_drawlightcache(
    "r_drawlightcache", "0", FCVAR_CHEAT,
    "0: off\n1: draw light cache entries\n2: draw rays\n");

//-----------------------------------------------------------------------------
// Sets up lighting state for rendering
//-----------------------------------------------------------------------------
void CModelRender::StudioSetupLighting(
    const DrawModelState_t &state, const Vector &absEntCenter,
    LightCacheHandle_t *pLightcache, bool bVertexLit, bool bNeedsEnvCubemap,
    bool &bStaticLighting, DrawModelInfo_t &drawInfo,
    const ModelRenderInfo_t &pInfo, int drawFlags) {
  if (m_bSuppressEngineLighting) return;
  ...
  if (pLightcache) {
    ...
    if (bStaticLighting) {
      if (g_pMaterialSystemHardwareConfig
              ->SupportsStaticPlusDynamicLighting()) {
        LightingState_t *pLightingState = NULL;
        // dx8 and dx9 case. . .hardware can do baked lighting plus other
        // dynamic lighting We already have the static part baked into a color
        // mesh, so just get the dynamic stuff.
        if (StaticLightCacheAffectedByDynamicLight(*pLightcache)) {
          pLightingState =
              LightcacheGetStatic(*pLightcache, &pEnvCubemapTexture);
          Assert(lightingState.numlights >= 0 &&
                 lightingState.numlights <= MAXLOCALLIGHTS);
        } else {
          pLightingState = LightcacheGetStatic(
              *pLightcache, &pEnvCubemapTexture,
        ...
        // dx6 and dx7 case. . .hardware can either do software lighting or
        // baked lighting only.
        if (StaticLightCacheAffectedByDynamicLight(*pLightcache) ||
          ...
          UpdateStaticPropColorData(state.m_pRenderable->GetIClientUnknown(),
                                    pInfo.instance);
        }
      }
    }
    ...
    if (r_decalstaticprops.GetBool() && pModelInst &&
        drawInfo.m_bStaticLighting && bHasDecals) {
      for (int iCube = 0; iCube < 6; ++iCube) {
        drawInfo.m_vecAmbientCube[iCube] =
            pModelInst->m_AmbientLightingState.r_boxcolor[iCube] +
            lightingState.r_boxcolor[iCube];
      }

      lightingDecalState.CopyLocalLights(pModelInst->m_AmbientLightingState);
      lightingDecalState.AddAllLocalLights(lightingState);

      Assert(lightingDecalState.numlights >= 0 &&
             lightingDecalState.numlights <= MAXLOCALLIGHTS);
    }
  } else  // !pLightcache
  {
    ...
    // If we don't have a lightcache entry, but we have bStaticLighting, that
    // means that we are a prop_physics that has fallen asleep.
    if (bStaticLighting) {
      LightcacheGetDynamic_Stats stats;
      pEnvCubemapTexture = LightcacheGetDynamic(
          vLightingOrigin, lightingState, stats,
          LIGHTCACHEFLAGS_DYNAMIC | LIGHTCACHEFLAGS_LIGHTSTYLE);
      Assert(lightingState.numlights >= 0 &&
             lightingState.numlights <= MAXLOCALLIGHTS);

      // Deal with all the dx6/dx7 issues (ie. can't do anything besides either
      // baked lighting or software dynamic lighting.
      if (!g_pMaterialSystemHardwareConfig
               ->SupportsStaticPlusDynamicLighting()) {
        if ((stats.m_bHasDLights || stats.m_bHasNonSwitchableLightStyles)) {
          // We either have a light switch, or a dynamic light. . do it in
          // software. We'll reget the cache entry with different flags below.
          bStaticLighting = false;
    ...
    if (!bStaticLighting) {
      ...
      // For special r_drawlightcache mode, we only draw models containing the
      // substring set in r_lightcachemodel
      bool bDebugModel = false;
      if (r_drawlightcache.GetInt() == 5) {
        if (pModelInst && pModelInst->m_pModel) {
          const char *szModelName = r_lightcachemodel.GetString();
          bDebugModel =
              V_stristr(pModelInst->m_pModel->szName, szModelName) != NULL;
        }
      }

      pEnvCubemapTexture = LightcacheGetDynamic(
          vLightingOrigin, lightingState, stats,
          LIGHTCACHEFLAGS_STATIC | LIGHTCACHEFLAGS_DYNAMIC |
              LIGHTCACHEFLAGS_LIGHTSTYLE | LIGHTCACHEFLAGS_ALLOWFAST,
          bDebugModel);
    ...
    if (pInfo.pLightingOffset && !pInfo.pLightingOrigin) {
      for (int i = 0; i < lightingState.numlights; ++i) {
        ...
        VectorITransform(pSaveLightPos[i], *pInfo.pLightingOffset,
                         lightingState.locallight[i]->origin);
      }
    }
    ...
    if (pModelInst && drawInfo.m_bStaticLighting && bHasDecals) {
      ...
      LightcacheGetDynamic(vLightingOrigin, lightingDecalState, stats,
                           LIGHTCACHEFLAGS_STATIC | LIGHTCACHEFLAGS_DYNAMIC |
                               ...
                               LIGHTCACHEFLAGS_ALLOWFAST);

      Assert(lightingDecalState.numlights >= 0 &&
             lightingDecalState.numlights <= MAXLOCALLIGHTS);

      for (int iCube = 0; iCube < 6; ++iCube) {
        VectorCopy(lightingDecalState.r_boxcolor[iCube],
                   drawInfo.m_vecAmbientCube[iCube]);
      }
      ...
      if (pInfo.pLightingOffset && !pInfo.pLightingOrigin) {
        for (int i = 0; i < lightingDecalState.numlights; ++i) {
          ...
          VectorITransform(pSaveLightPos[i], *pInfo.pLightingOffset,
                           lightingDecalState.locallight[i]->origin);
        }
      }
    }
  }
  ...
  if (!bStaticLighting && !pLightcache) {
    pState = TimeAverageLightingState(pInfo.instance, &lightingState,
                                      pInfo.entity_index, pDebugLightingOrigin);
  } else {
  ...
  if (g_pMaterialSystemConfig->nFullbright == 1) {
    ...
    static Vector white[6] = {
        Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0),
        Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0), Vector(1.0, 1.0, 1.0),
    };
    ...
    if (drawFlags & STUDIORENDER_DRAW_ITEM_BLINK) {
      ...
      if ((pState->numlights > 0) &&
          (pInfo.pModel->flags & MODELFLAG_STUDIOHDR_AMBIENT_BOOST) &&
          r_ambientboost.GetBool()) {
        ...
        // Compute the amount of direct light reaching the center of the model
        ...
        // If ambient cube is sufficiently dim in absolute terms and ambient
        // cube is swamped by direct lights
        if (avgCubeLuminance < r_ambientmin.GetFloat() &&
            (avgCubeLuminance <
          ...
          g_pStudioRender->SetAmbientLightColors(
              pState->r_boxcolor);  // No Boost
        }
      } else  // Don't bother with ambient boost, just use the ambient cube as
              // is
      {
    ...
    R_SetNonAmbientLightingState(pState->numlights, pState->locallight,
                                 &drawInfo.m_nLocalLightCount,
                                 &drawInfo.m_LocalLightDescs[0], true);

    // Cache lighting for decals.
    if (pModelInst && drawInfo.m_bStaticLighting && bHasDecals) {
      R_SetNonAmbientLightingState(
          lightingDecalState.numlights, lightingDecalState.locallight,
...
int GetLightingConditions(const Vector &vecLightingOrigin, Vector *pColors,
  ...
  for (int i = 0; i < state.numlights; ++i) {
    ...
    if (!WorldLightToMaterialLight(state.locallight[i], *pLightDesc)) continue;
    ...
    if (++nLightCount >= nMaxLocalLights) break;
  }
  return nLightCount;
#endif
}
...
void CModelRender::SetupLighting(const Vector &vecCenter) {
  ...
  ITexture *pEnvCubemapTexture = NULL;
  int nLightCount =
      GetLightingConditions(vecCenter, vecAmbient, 2, desc, pEnvCubemapTexture);
...
matrix3x4_t *CModelRender::SetupModelState(IClientRenderable *pRenderable) {
  ...
  studiohdr_t *pStudioHdr =
      modelinfo->GetStudiomodel(const_cast<model_t *>(pModel));
  if (pStudioHdr->numbodyparts == 0) return NULL;
  ...
  Assert(pRenderable);
  {
    ...
    pRenderable->SetupBones(pBoneMatrices, nBoneCount, BONE_USED_BY_ANYTHING,
                            cl.GetTime());  // hack hack
...
void DrawModelDebugOverlay(const DrawModelInfo_t &info,
                           const DrawModelResults_t &results,
                           const Vector &origin, float r = 1.0f, float g = 1.0f,
                           float b = 1.0f) {
#ifndef SWDS
  float alpha = 1;
  if (r_drawmodelstatsoverlaydistance.GetFloat() == 1) {
    ...
    // The view model keeps throwing up its data and it looks like garbage, so I
  ...
  float duration = 0.0f;
  int lineOffset = 0;
  if (!info.m_pStudioHdr || !info.m_pStudioHdr->pszName() ||
      !info.m_pHardwareData) {
    CDebugOverlay::AddTextOverlay(
        origin, lineOffset++, duration,
        "This model has problems. . see a programmer.");
    return;
  }

  char buf[1024];
  CDebugOverlay::AddTextOverlay(origin, lineOffset++, duration, r, g, b, alpha,
                                info.m_pStudioHdr->pszName());
  Q_snprintf(buf, sizeof(buf), "lod: %d/%d\n", results.m_nLODUsed + 1,
             (int)info.m_pHardwareData->m_NumLODs);
  CDebugOverlay::AddTextOverlay(origin, lineOffset++, duration, r, g, b, alpha,
                                buf);
  ...
  Q_snprintf(buf, sizeof(buf), "has shadow lod: %s\n",
             (info.m_pStudioHdr->flags & STUDIOHDR_FLAGS_HASSHADOWLOD)
                 ? "true"
                 : "false");
  CDebugOverlay::AddTextOverlay(origin, lineOffset++, duration, r, g, b, alpha,
                                buf);
  ...
  for (i = 0; i < results.m_Materials.Count(); i++) {
    ...
    if (pMaterial) {
      ...
      Q_snprintf(buf, sizeof(buf), "\t%s (%d %s)\n",
                 results.m_Materials[i]->GetName(), numPasses,
                 numPasses > 1 ? "passes" : "pass");
      CDebugOverlay::AddTextOverlay(origin, lineOffset++, duration, r, g, b,
                                    alpha, buf);
    }
  }
  if (results.m_Materials.Count() > results.m_NumMaterials) {
    CDebugOverlay::AddTextOverlay(origin, lineOffset++, duration, r, g, b,
                                  alpha, "(Remaining materials not shown)\n");
  }
...
void AddModelDebugOverlay(const DrawModelInfo_t &info,
                          const DrawModelResults_t &results,
                          const Vector &origin) {
...
int SavedModelInfo_Compare_f(const void *l, const void *r) {
  ...
  return left->m_ModelResults.m_RenderTime.GetDuration().GetSeconds() <
         right->m_ModelResults.m_RenderTime.GetDuration().GetSeconds();
}

static ConVar r_drawmodelstatsoverlaymin("r_drawmodelstatsoverlaymin", "0.1",
                                         FCVAR_ARCHIVE,
                                         "time in milliseconds that a model "
                                         "must take to render before showing "
                                         "an overlay in "
                                         "r_drawmodelstatsoverlay 2");
static ConVar r_drawmodelstatsoverlaymax("r_drawmodelstatsoverlaymax", "1.5",
                                         FCVAR_ARCHIVE,
                                         "time in milliseconds beyond which a "
                                         "model overlay is fully red in "
                                         "r_drawmodelstatsoverlay 2");
...
void DrawSavedModelDebugOverlays(void) {
  ...
  for (i = 0; i < s_SavedModelInfo.Count(); i++) {
    float r, g, b;
    float t = s_SavedModelInfo[i]
                  .m_ModelResults.m_RenderTime.GetDuration()
                  .GetMillisecondsF();
    if (t > min) {
      if (t >= max) {
        r = 1.0f;
      ...
      DrawModelDebugOverlay(s_SavedModelInfo[i].m_ModelInfo,
                            s_SavedModelInfo[i].m_ModelResults,
                            s_SavedModelInfo[i].m_Origin, r, g, b);
    }
  }
  ClearSaveModelDebugOverlays();
}

void CModelRender::DebugDrawLightingOrigin(const DrawModelState_t &state,
                                           const ModelRenderInfo_t &pInfo) {
#ifndef SWDS
  ...
  if (pInfo.pLightingOrigin) {
    ...
    R_ComputeLightingOrigin(state.m_pRenderable, state.m_pStudioHdr,
                            *state.m_pModelToWorld, illumPosition);
    lightOrigin = illumPosition;
    if (pInfo.pLightingOffset) {
  ...
  CDebugOverlay::AddLineOverlay(lightOrigin, pt1, 100, 100, 150, 255, true,
                                0.0f);
  pt0.x = state.m_pStudioHdr->hull_min.x;
  ...
  CDebugOverlay::AddLineOverlay(lightOrigin, pt1, 100, 100, 150, 255, true,
                                0.0f);
  pt0.x = state.m_pStudioHdr->hull_min.x;  //-V778
  pt0.y = state.m_pStudioHdr->hull_max.y;
  pt0.z = state.m_pStudioHdr->hull_max.z;
  VectorTransform(pt0, *state.m_pModelToWorld, pt1);
  CDebugOverlay::AddLineOverlay(lightOrigin, pt1, 100, 100, 150, 255, true,
...
//-----------------------------------------------------------------------------
void CModelRender::DrawModelExecute(const DrawModelState_t &state,
                                    const ModelRenderInfo_t &pInfo,
                                    matrix3x4_t *pBoneToWorld) {
#ifndef SWDS
  bool bShadowDepth = (pInfo.flags & STUDIO_SHADOWDEPTHTEXTURE) != 0;

  // Bail if we're rendering into shadow depth map and this model doesn't cast
  // shadows
  if (bShadowDepth &&
      ((pInfo.pModel->flags & MODELFLAG_STUDIOHDR_DO_NOT_CAST_SHADOWS) != 0))
    return;
  ...
  if (state.m_pStudioHdr->numflexdesc > 0) {
    ...
    g_pStudioRender->LockFlexWeights(
        state.m_pStudioHdr->numflexdesc, &pFlexWeights,
        bUsesDelayedWeights ? &pFlexDelayedWeights : NULL);
    state.m_pRenderable->SetupWeights(pBoneToWorld,
                                      state.m_pStudioHdr->numflexdesc,
                                      pFlexWeights, pFlexDelayedWeights);
    g_pStudioRender->UnlockFlexWeights();
  }

  // OPTIMIZE: Try to precompute part of this mess once a frame at the very
  // least.
  bool bUsesBumpmapping =
      (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80) &&
      (pInfo.pModel->flags & MODELFLAG_STUDIOHDR_USES_BUMPMAPPING);
  ...
  bool bStaticLighting =
      ...
      (!bUsesBumpmapping) && (pInfo.instance != MODEL_INSTANCE_INVALID) &&
  ...
  bool bNeedsEnvCubemap =
      r_showenvcubemap.GetInt() ||
      (pInfo.pModel->flags & MODELFLAG_STUDIOHDR_USES_ENV_CUBEMAP);
  ...
  if ((bVertexLit || bNeedsEnvCubemap) && !bShadowDepth) {
    ...
    if (pInfo.instance != MODEL_INSTANCE_INVALID) {
      if ((m_ModelInstances[pInfo.instance].m_nFlags &
           MODEL_INSTANCE_HAS_STATIC_LIGHTING) &&
          m_ModelInstances[pInfo.instance].m_LightCacheHandle) {
    ...
    Vector entOrigin;
    R_ComputeLightingOrigin(state.m_pRenderable, state.m_pStudioHdr,
                            *state.m_pModelToWorld, entOrigin);

    // Set up lighting based on the lighting origin
    StudioSetupLighting(state, entOrigin, pLightCache, bVertexLit,
                        bNeedsEnvCubemap, bStaticLighting, info, pInfo,
                        state.m_drawFlags);
  }

  // Set up the camera state
  g_pStudioRender->SetViewState(CurrentViewOrigin(), CurrentViewRight(),
                                CurrentViewUp(), CurrentViewForward());
...
//-----------------------------------------------------------------------------
int CModelRender::DrawModel(int flags, IClientRenderable *pRenderable,
                            ModelInstanceHandle_t instance, int entity_index,
                            const model_t *pModel, const Vector &origin,
                            const QAngle &angles, int skin, int body,
                            int hitboxset, const matrix3x4_t *pModelToWorld,
                            const matrix3x4_t *pLightingOffset) {
...
// Input  : &pInfo -
//-----------------------------------------------------------------------------
bool CModelRender::DrawModelSetup(ModelRenderInfo_t &pInfo,
                                  DrawModelState_t *pState,
  ...
  *ppBoneToWorldOut = NULL;
  ...
  if (!state.m_pStudioHWData) return false;
  ...
  if ((pInfo.flags & STUDIO_RENDER) == 0) {
    ...
    bool bOk = pInfo.pRenderable->SetupBones(NULL, MAXSTUDIOBONES, boneMask,
                                             cl.GetTime());
    return bOk;
  }
  ...
  const bool bOk = pInfo.pRenderable->SetupBones(pBoneToWorld, nBoneCount,
                                                 boneMask, cl.GetTime());
  if (!pCustomBoneToWorld) {
    g_pStudioRender->UnlockBoneMatrices();
  }
...
int CModelRender::DrawModelEx(ModelRenderInfo_t &pInfo) {
  ...
  matrix3x4_t *pBoneToWorld;
  if (!DrawModelSetup(pInfo, &state, NULL, &pBoneToWorld)) return 0;
...
int CModelRender::DrawModelExStaticProp(ModelRenderInfo_t &pInfo) {
  ...
  // Can only deal with studio models
  if (pInfo.pModel->type != mod_studio) return 0;
#endif
  Assert(modelloader->IsLoaded(pInfo.pModel));
  ...
  state.m_pStudioHWData = g_pMDLCache->GetHardwareData(pInfo.pModel->studio);
  if (!state.m_pStudioHWData) return 0;
 ...
 public:
  bool Less(const robject_t &lhs, const robject_t &rhs, void *pContext) {
    ...
    if (lhs.modelIndex == rhs.modelIndex) {
      if (lhs.skin != rhs.skin) return lhs.skin < rhs.skin;
...
inline int FindModel(const CUtlVector<rmodel_t> &list, const model_t *pModel) {
  for (int j = list.Count(); --j >= 0;) {
    if (list[j].pModel == pModel) return j;
  }
  return -1;
}

// NOTE: UNDONE: This is a work in progress of a new static prop rendering
// pipeline UNDONE: Expose drawing commands from studiorender and draw here
...
int CModelRender::DrawStaticPropArrayFast(StaticPropRenderInfo_t *pProps,
                                          int count, bool bShadowDepth) {
#ifndef SWDS
  ...
  for (int i = 0; i < modelList.Count(); i++) {
    ...
    modelList[i].bStaticLighting =
        ((modelList[i].pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) != 0) &&
        !bBumpMapped;
    modelList[i].bVertexLit = (flags & MODELFLAG_VERTEXLIT) != 0;
    modelList[i].bNeedsCubemap =
        (flags & MODELFLAG_STUDIOHDR_USES_ENV_CUBEMAP) != 0;
  ...
  // UNDONE: Don't sort if rendering transparent objects - for now this isn't
  // called in the transparent case sort by model, then by lod
  ...
  if (!bShadowDepth) {
    for (int i = 0; i < objectList.Count(); i++) {
      ...
      bool bStaticLighting =
          (model.bStaticLighting && obj.instance != MODEL_INSTANCE_INVALID);
      bool bVertexLit = model.bVertexLit;
      bool bNeedsEnvCubemap = bForceCubemap || model.bNeedsCubemap;
      bool bHasDecals = (m_ModelInstances[obj.instance].m_DecalHandle !=
      ...
      // for now we skip models that have shadows - will update later to include
      // them in a post-pass
      ...
      if (bStaticLighting) {
        ...
        if (!pColorMeshData || pColorMeshData->m_bNeedsRetry) {
          ...
          if (UpdateStaticPropColorData(obj.pRenderable->GetIClientUnknown(),
                                        obj.instance)) {
            pColorMeshData = CacheGet(hColorMeshData);
          } else if (!pColorMeshData || !pColorMeshData->m_bNeedsRetry) {
            // can't draw
        ...
        if (pColorMeshData && pColorMeshData->m_bColorMeshValid) {
          ...
          if (pCallQueue) {
            if (CacheLock(hColorMeshData))  // CacheCreate above will call
                                            // functions that won't take place
                                            // until later. If color mesh isn't
                                            // used right away, it could get
                                            // dumped
            {
              pCallQueue->QueueCall(this, &CModelRender::CacheUnlock,
                                    hColorMeshData);
            }
          }
        } else {
      ...
      if ((bVertexLit || bNeedsEnvCubemap)) {
        ...
        if (obj.instance != MODEL_INSTANCE_INVALID) {
          if ((m_ModelInstances[obj.instance].m_nFlags &
               MODEL_INSTANCE_HAS_STATIC_LIGHTING) &&
              m_ModelInstances[obj.instance].m_LightCacheHandle) {
        ...
        if (pLightCache) {
          ...
          // mesh, so just get the dynamic stuff.
          if (!bStaticLighting ||
              StaticLightCacheAffectedByDynamicLight(*pLightCache)) {
            pState = LightcacheGetStatic(*pLightCache, &pEnvCubemapTexture,
                                         LIGHTCACHEFLAGS_STATIC |
                                             LIGHTCACHEFLAGS_DYNAMIC |
                                             LIGHTCACHEFLAGS_LIGHTSTYLE);
            Assert(pState->numlights >= 0 &&
                   pState->numlights <= MAXLOCALLIGHTS);
          } else {
            pState = LightcacheGetStatic(
                *pLightCache, &pEnvCubemapTexture,
                LIGHTCACHEFLAGS_DYNAMIC | LIGHTCACHEFLAGS_LIGHTSTYLE);
            Assert(pState->numlights >= 0 &&
                   pState->numlights <= MAXLOCALLIGHTS);
          ...
          if (bHasDecals) {
            for (int iCube = 0; iCube < 6; ++iCube) {
              pDecalLightState->r_boxcolor[iCube] =
                  m_ModelInstances[obj.instance]
                      .m_AmbientLightingState.r_boxcolor[iCube] +
                  pState->r_boxcolor[iCube];
            }
            pDecalLightState->CopyLocalLights(
                m_ModelInstances[obj.instance].m_AmbientLightingState);
        ...
        } else  // !pLightcache
        {
          ...
          LightcacheGetDynamic_Stats stats;
          pEnvCubemapTexture = LightcacheGetDynamic(
              vLightingOrigin, lightingState, stats, lightCacheFlags, false);
          Assert(lightingState.numlights >= 0 &&
                 lightingState.numlights <= MAXLOCALLIGHTS);
          pState = &lightingState;
          ...
          if (bHasDecals) {
            ...
            LightcacheGetDynamic(
                vLightingOrigin, *pDecalLightState, stats,
                LIGHTCACHEFLAGS_STATIC | LIGHTCACHEFLAGS_DYNAMIC |
                    LIGHTCACHEFLAGS_LIGHTSTYLE | LIGHTCACHEFLAGS_ALLOWFAST);
          }
        }
        ...
        if (bVertexLit) {
          // if we have any real lighting state we need to save it for this
          // object
  ...
  pRenderContext->DisableAllLocalLights();
  DrawModelInfo_t info;
  for (int i = 0; i < 6; i++) info.m_vecAmbientCube[i].Init();
  ...
  int drawFlags =
      STUDIORENDER_DRAW_ENTIRE_MODEL | STUDIORENDER_DRAW_STATIC_LIGHTING;
  if (bShadowDepth) drawFlags |= STUDIO_SHADOWDEPTHTEXTURE;
  info.m_Decals = STUDIORENDER_DECAL_INVALID;
  info.m_Body = 0;
  info.m_HitboxSet = 0;
  for (int i = 0; i < objectList.Count(); i++) {
    robject_t &obj = objectList[i];
    if (obj.lightIndex >= 0) continue;
    rmodel_t &model = modelList[obj.modelIndex];
    if (obj.pEnvCubeMap) {
  ...
  if (lightObjects.Count()) {
    for (int i = 0; i < lightObjects.Count(); i++) {
      ...
      R_SetNonAmbientLightingState(pState->numlights, pState->locallight,
                                   &nLocalLightCount, localLightDescs, true);
      info.m_pStudioHdr = model.pStudioHdr;
      info.m_pHardwareData = model.pStudioHWData;
      info.m_Skin = obj.skin;
      info.m_pClientEntity = static_cast<void *>(obj.pRenderable);
  ...
  if (!IsX360() && (r_flashlight_version2.GetInt() == 0) &&
      shadowObjects.Count()) {
...
// Shadow rendering
//-----------------------------------------------------------------------------
matrix3x4_t *CModelRender::DrawModelShadowSetup(
    IClientRenderable *pRenderable, int body, int skin, DrawModelInfo_t *pInfo,
    matrix3x4_t *pCustomBoneToWorld) {
#ifndef SWDS
  ...
  if (!info.m_pHardwareData) return NULL;
  ...
  if (info.m_pStudioHdr->flags & STUDIOHDR_FLAGS_HASSHADOWLOD) {
    ...
    float factor = r_shadowlodbias.GetFloat() > 0.0f
                       ? 1.0f / r_shadowlodbias.GetFloat()
                       : 1.0f;
    float screenSize = factor * pRenderContext->ComputePixelWidthOfSphere(
                                    pRenderable->GetRenderOrigin(), 0.5f);
    info.m_Lod =
        g_pStudioRender->ComputeModelLod(info.m_pHardwareData, screenSize);
  ...
  const bool bOk = pRenderable->SetupBones(
      pBoneToWorld, info.m_pStudioHdr->numbones,
      BONE_USED_BY_VERTEX_AT_LOD(info.m_Lod), cl.GetTime());
  g_pStudioRender->UnlockBoneMatrices();
  if (!bOk) return NULL;
  return pBoneToWorld;
#else
...
void CModelRender::DrawModelShadow(IClientRenderable *pRenderable,
                                   const DrawModelInfo_t &info,
                                   matrix3x4_t *pBoneToWorld) {
#ifndef SWDS
  // Needed because we don't call SetupWeights
  g_pStudioRender->SetEyeViewTarget(info.m_pStudioHdr, info.m_Body,
                                    vec3_origin);
  ...
  if ((info.m_pStudioHdr->flags & STUDIOHDR_FLAGS_USE_SHADOWLOD_MATERIALS) ==
      0) {
    g_pStudioRender->ForcedMaterialOverride(g_pMaterialShadowBuild,
                                            OVERRIDE_BUILD_SHADOWS);
  }

  g_pStudioRender->DrawModel(
      NULL, info, pBoneToWorld, NULL, NULL, pRenderable->GetRenderOrigin(),
      STUDIORENDER_DRAW_NO_SHADOWS | STUDIORENDER_DRAW_ENTIRE_MODEL |
          STUDIORENDER_DRAW_NO_FLEXES);
  g_pStudioRender->ForcedMaterialOverride(0);
#endif
}

void CModelRender::SetViewTarget(const CStudioHdr *pStudioHdr, int nBodyIndex,
                                 const Vector &target) {
  g_pStudioRender->SetEyeViewTarget(pStudioHdr->GetRenderHdr(), nBodyIndex,
                                    target);
}

void CModelRender::InitColormeshParams(ModelInstance_t &instance,
                                       studiohwdata_t *pStudioHWData,
                                       colormeshparams_t *pColorMeshParams) {
  ...
  if ((instance.m_nFlags & MODEL_INSTANCE_HAS_DISKCOMPILED_COLOR) &&
    ...
    pColorMeshParams->m_pPooledVBAllocator =
        (IPooledVBAllocator *)&m_colorMeshVBAllocator;
  }

  for (int lodID = pStudioHWData->m_RootLOD; lodID < pStudioHWData->m_NumLODs;
       lodID++) {
...
CColorMeshData *CModelRender::FindOrCreateStaticPropColorData(
  ...
  studiohwdata_t *pStudioHWData =
      g_pMDLCache->GetHardwareData(instance.m_pModel->studio);
...
void CModelRender::ProtectColorDataIfQueued(DataCacheHandle_t hColorMesh) {
  if (hColorMesh != DC_INVALID_HANDLE) {
    ...
    ICallQueue *pCallQueue = pRenderContext->GetCallQueue();
    if (pCallQueue) {
      if (CacheLock(hColorMesh))  // CacheCreate above will call functions that
                                  // won't take place until later. If color mesh
                                  // isn't used right away, it could get dumped
      {
...
//-----------------------------------------------------------------------------
void CModelRender::ComputeModelVertexLightingOld(
    mstudiomodel_t *pModel, matrix3x4_t &matrix,
    const LightingState_t &lightingState, color24 *pLighting,
  ...
  R_SetNonAmbientLightingState(pLightingState->numlights,
                               pLightingState->locallight, &nNumLightDesc,
                               lightDesc, false);
  ...
  for (int i = 0; i < pModel->numvertices; ++i) {
    ...
    if (bUseConstDirLighting) {
      g_pStudioRender->ComputeLightingConstDirectional(
          pLightingState->r_boxcolor, nNumLightDesc, lightDesc, worldPos,
          worldNormal, destColor, flConstDirLightAmount);
    } else {
      g_pStudioRender->ComputeLighting(pLightingState->r_boxcolor,
                                       nNumLightDesc, lightDesc, worldPos,
                                       worldNormal, destColor);
    }
...
//-----------------------------------------------------------------------------
void CModelRender::ComputeModelVertexLighting(
    IHandleEntity *pProp, mstudiomodel_t *pModel,
    OptimizedModel::ModelLODHeader_t *pVtxLOD, matrix3x4_t &matrix,
    Vector4D *pTempMem, color24 *pLighting) {
#ifndef SWDS
  if (IsX360()) return;

  int i;
  unsigned char *pInSolid = (unsigned char *)stackalloc(
      ((pModel->numvertices + 7) >> 3) * sizeof(unsigned char));
  ...
  for (int meshID = 0; meshID < pModel->nummeshes; ++meshID) {
    ...
    for (int stripGroupID = 0; stripGroupID < pVtxMesh->numStripGroups;
         ++stripGroupID) {
      ...
      for (i = 0; i < pStripGroup->numIndices; i += 3) {
        ...
        int v[3];
        v[0] =
            pStripGroup->pVertex(nIndex1)->origMeshVertID + pMesh->vertexoffset;
        v[1] =
            pStripGroup->pVertex(nIndex2)->origMeshVertID + pMesh->vertexoffset;
        v[2] =
            pStripGroup->pVertex(nIndex3)->origMeshVertID + pMesh->vertexoffset;
        ...
        int nAverage[3];
        if (!bSolid[0]) {
          nAverage[nValidCount++] = v[0];
        }
        if (!bSolid[1]) {
          nAverage[nValidCount++] = v[1];
        }
        if (!bSolid[2]) {
          nAverage[nValidCount++] = v[2];
        }
        ...
        if (bSolid[0]) {
          pTempMem[v[0]].AsVector3D() += vecAverage;
          pTempMem[v[0]].w += 1.0f;
        }
        if (bSolid[1]) {
          pTempMem[v[1]].AsVector3D() += vecAverage;
          pTempMem[v[1]].w += 1.0f;
        }
        if (bSolid[2]) {
          pTempMem[v[2]].AsVector3D() += vecAverage;
          pTempMem[v[2]].w += 1.0f;
        }
      }
    }
...
void CModelRender::ValidateStaticPropColorData(ModelInstanceHandle_t handle) {
  ...
  if (!g_pMaterialSystemHardwareConfig->SupportsColorOnSecondStream() ||
      !StaticPropMgr()->IsStaticProp(pProp)) {
    // can't support it or not a static prop
    return;
  }

  if (!g_bLoadedMapHasBakedPropLighting ||
      StaticPropMgr()->PropHasBakedLightingDisabled(pProp)) {
    return;
  }
  ...
  if (!g_pFileSystem->ReadFile(fileName, "GAME", utlBuf,
                               sizeof(HardwareVerts::FileHeader_t), 0)) {
    // not available
    return;
  }

  studiohdr_t *pStudioHdr =
      g_pMDLCache->GetStudioHdr(pInstance->m_pModel->studio);
...
//-----------------------------------------------------------------------------
void CModelRender::StaticPropColorMeshCallback(void *pContext,
                                               const void *pData,
                                               int numReadBytes,
                                               FSAsyncStatus_t asyncStatus) {
  // get our preserved data
  Assert(pContext);
  staticPropAsyncContext_t *pStaticPropContext =
      (staticPropAsyncContext_t *)pContext;

  HardwareVerts::FileHeader_t *pVhvHdr;
  uint8_t *pOriginalData = NULL;
  ...
  for (meshID = startMesh; meshID < pVhvHdr->m_nMeshes; meshID++) {
    ...
    if (numVertexes !=
        pStaticPropContext->m_pColorMeshData->m_pMeshInfos[meshID - startMesh]
            .m_nNumVerts) {
      // meshes are out of sync, discard data
      break;
    }
    V_memcpy((void *)pStaticPropContext->m_pColorMeshData
                 ->m_ppTargets[meshID - startMesh],
             pVhvHdr->pVertexBase(meshID), numVertexes * 4);
  }
...
// Called from async i/o thread - must spend minimal cycles in this context
//-----------------------------------------------------------------------------
static void StaticPropColorMeshCallback(const FileAsyncRequest_t &request,
                                        int numReadBytes,
                                        FSAsyncStatus_t asyncStatus) {
  s_ModelRender.StaticPropColorMeshCallback(request.pContext, request.pData,
                                            numReadBytes, asyncStatus);
}
...
//-----------------------------------------------------------------------------
bool CModelRender::LoadStaticPropColorData(IHandleEntity *pProp,
                                           DataCacheHandle_t colorMeshHandle,
                                           studiohwdata_t *pStudioHWData) {
  ...
  // the color meshes should already have low quality data to be used during
  // rendering
  ...
  // can't optimal async yet, because need flat ppColorMesh[], so use callback
  // to distribute create our private context of data for the callback
...
//-----------------------------------------------------------------------------
bool CModelRender::UpdateStaticPropColorData(IHandleEntity *pProp,
                                             ModelInstanceHandle_t handle) {
  MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
  ...
  if (r_debugrandomstaticlighting.GetBool()) {
    ...
    // purposely not deterministic to catch bugs with excessive re-baking (i.e.
    // disco)
    Vector fRandomColor;
  ...
  Assert(modelloader->IsLoaded(inst.m_pModel) &&
         (inst.m_pModel->type == mod_studio));

  if (r_proplightingfromdisk.GetInt() == 2) {
    // This visualization debug mode is strictly to debug which static prop
    // models have valid disk based lighting. There should be no red models,
    // only green or yellow. Yellow models denote the legacy lower quality
  ...
  studiohwdata_t *pStudioHWData =
      g_pMDLCache->GetHardwareData(inst.m_pModel->studio);
  ...
  // calculation may abort due to lack of async requested data, caller should
  // retry
  pColorMeshData->m_bColorMeshValid = false;
  ...
  if (!bDebugColor) {
    ...
    vertexFileHeader_t *pVertexHdr = g_pMDLCache->GetVertexData(
        (MDLHandle_t)(uintptr_t)pStudioHdr->virtualModel);
    if (!pVertexHdr) {
  ...
  matrix3x4_t matrix;
  AngleMatrix(inst.m_pRenderable->GetRenderAngles(),
              inst.m_pRenderable->GetRenderOrigin(), matrix);

  // Get static lighting only!!  We'll add dynamic and lightstyles in in the
  // vertex shader. . .
  unsigned int lightCacheFlags = LIGHTCACHEFLAGS_STATIC;
  if (!g_pMaterialSystemHardwareConfig->SupportsStaticPlusDynamicLighting()) {
    // . . . unless we can't do anything but static or dynamic simulaneously. .
    // then we'll bake the lightstyle info here.
    lightCacheFlags |= LIGHTCACHEFLAGS_LIGHTSTYLE;
  }

  LightingState_t lightingState;
  if ((inst.m_nFlags & MODEL_INSTANCE_HAS_STATIC_LIGHTING) &&
      inst.m_LightCacheHandle) {
    lightingState =
        *(LightcacheGetStatic(inst.m_LightCacheHandle, NULL, lightCacheFlags));
  } else {
    // Choose the lighting origin
    Vector entOrigin;
  ...
  if (pStudioHdr->flags & STUDIOHDR_FLAGS_CONSTANT_DIRECTIONAL_LIGHT_DOT) {
    ...
    flConstDirLightingAmount =
        (float)(pStudioHdr->constdirectionallightdot) / 255.0;
  }

  CUtlMemory<color24> tmpLightingMem;

  // Iterate over every body part...
  for (int bodyPartID = 0; bodyPartID < pStudioHdr->numbodyparts;
       ++bodyPartID) {
    ...
    for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
      ...
      for (int lodID = pStudioHWData->m_RootLOD;
           lodID < pStudioHWData->m_NumLODs; ++lodID) {
        ...
        for (int meshID = 0; meshID < pModel->nummeshes; ++meshID) {
          ...
          // Iterate over all strip groups.
          for (int stripGroupID = 0;
               stripGroupID < pStudioMeshData[pMesh->meshid].m_NumGroup;
               ++stripGroupID) {
            studiomeshgroup_t *pMeshGroup =
                &pStudioMeshData[pMesh->meshid].m_pMeshGroup[stripGroupID];
            ColorMeshInfo_t *pColorMeshInfo =
                &pColorMeshData->m_pMeshInfos[pMeshGroup->m_ColorMeshID];

            CMeshBuilder meshBuilder;
            meshBuilder.Begin(pColorMeshInfo->m_pMesh, MATERIAL_HETEROGENOUS,
                              pMeshGroup->m_NumVertices, 0);

            if (!meshBuilder.VertexSize()) {
              meshBuilder.End();
              return false;  // Aborting processing, since something was wrong
                             // with D3D
            }

            // We need to account for the stream offset used by pool-allocated
            // (static-lit) color meshes:
            int streamOffset =
                pColorMeshInfo->m_nVertOffsetInBytes / meshBuilder.VertexSize();
            meshBuilder.AdvanceVertices(streamOffset);

            // Iterate over all vertices
            for (int i = 0; i < pMeshGroup->m_NumVertices; ++i) {
              int nVertIndex =
                  pMesh->vertexoffset + pMeshGroup->m_pGroupIndexToMeshIndex[i];
...
void CModelRender::ReleaseAllStaticPropColorData(void) {
  FOR_EACH_LL(m_ModelInstances, i) { DestroyStaticPropColorData(i); }
}
...
void CModelRender::RestoreAllStaticPropColorData(void) {
  ...
  FOR_EACH_LL(m_ModelInstances, i) {
    UpdateStaticPropColorData(
        m_ModelInstances[i].m_pRenderable->GetIClientUnknown(), i);
  }
#endif
}
...
// Creates, destroys instance data to be associated with the model
//-----------------------------------------------------------------------------
ModelInstanceHandle_t CModelRender::CreateInstance(
    IClientRenderable *pRenderable, LightCacheHandle_t *pCache) {
  Assert(pRenderable);
...
void CModelRender::SetStaticLighting(ModelInstanceHandle_t handle,
  ...
  // reasons It was a real pain to do this from within the static prop mgr,
  // where the lightcache handle used to reside
  if (handle != MODEL_INSTANCE_INVALID) {
    ModelInstance_t &instance = m_ModelInstances[handle];
    if (pCache) {
...
LightCacheHandle_t CModelRender::GetStaticLighting(
    ModelInstanceHandle_t handle) {
  if (handle != MODEL_INSTANCE_INVALID) {
    ModelInstance_t &instance = m_ModelInstances[handle];
...
//-----------------------------------------------------------------------------
// This gets called when overbright, etc gets changed to recompute static prop
// lighting. Returns FALSE if needed async data not available to complete
// computation or an error (don't draw). Returns TRUE if operation succeeded or
// computation skipped (ok to draw). Callers use this to track state in a retry
// pattern, so the expensive computation only happens once as needed or can
// continue to be polled until success.
//-----------------------------------------------------------------------------
bool CModelRender::RecomputeStaticLighting(ModelInstanceHandle_t handle) {
  ...
  studiohdr_t *pStudioHdr =
      g_pMDLCache->GetStudioHdr(instance.m_pModel->studio);
  if (!pStudioHdr) {
    // data not available
    return false;
  }
  ...
  if (pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP) {
    ...
    if (r_decalstaticprops.GetBool() && instance.m_LightCacheHandle) {
      instance.m_AmbientLightingState = *(LightcacheGetStatic(
          instance.m_LightCacheHandle, NULL, LIGHTCACHEFLAGS_STATIC));
    }

    return UpdateStaticPropColorData(
        instance.m_pRenderable->GetIClientUnknown(), handle);
  }
...
DataCacheHandle_t CModelRender::GetCachedStaticPropColorData(
    const char *pName) {
  // valid for 360 only
  Assert(IsX360());
  if (IsPC()) {
...
void CModelRender::SetupColorMeshes(int nTotalVerts) {
  ...
  if (r_proplightingpooling.GetInt() == -1) {
    // This is useful on X360 because VBs are 4-KB aligned, so using a shared VB
  ...
  if (r_proplightingpooling.GetInt() == 1) {
    if (m_colorMeshVBAllocator.GetNumVertsAllocated() == 0) {
      if (nTotalVerts) {
        // Allocate a mesh (vertex buffer) big enough to accommodate all static
        // prop color meshes (which are allocated inside
        // CModelRender::FindOrCreateStaticPropColorData() ):
...
void CModelRender::DestroyInstance(ModelInstanceHandle_t handle) {
  ...
  // 360 holds onto static prop disk color data only, to avoid redundant work
  // during same map load can only persist props with disk based lighting check
  // for dvd mode as a reasonable assurance that the queued loader will be
  // responsible for a possible purge if the queued loader doesn't run, the
  // purge will get caught later than intended
  bool bPersistLighting = IsX360() &&
                          (m_ModelInstances[handle].m_nFlags &
                           MODEL_INSTANCE_HAS_DISKCOMPILED_COLOR) &&
...
bool CModelRender::ChangeInstance(ModelInstanceHandle_t handle,
                                  IClientRenderable *pRenderable) {
  if (handle == MODEL_INSTANCE_INVALID || !pRenderable) return false;
...
bool CModelRender::IsModelInstanceValid(ModelInstanceHandle_t handle) {
  if (handle == MODEL_INSTANCE_INVALID) return false;

  ModelInstance_t inst = m_ModelInstances[handle];
  if (inst.m_DecalHandle == STUDIORENDER_DECAL_INVALID) return false;
...
void CModelRender::AddDecal(ModelInstanceHandle_t handle, Ray_t const &ray,
                            const Vector &decalUp, int decalIndex, int body,
                            bool noPokeThru, int maxLODToDecal) {
  if (handle == MODEL_INSTANCE_INVALID) return;
  ...
  g_pStudioRender->AddDecal(inst.m_DecalHandle,
                            g_pMDLCache->GetStudioHdr(inst.m_pModel->studio),
                            pBoneToWorld, ray, decalUp, pDecalMaterial, radius,
...
void CModelRender::RemoveAllDecals(ModelInstanceHandle_t handle) {
  ...
  ModelInstance_t &inst = m_ModelInstances[handle];
  if (!IsModelInstanceValid(handle)) return;
...
void CModelRender::RemoveAllDecalsFromAllModels() {
  for (ModelInstanceHandle_t i = m_ModelInstances.Head();
       i != m_ModelInstances.InvalidIndex(); i = m_ModelInstances.Next(i)) {
    RemoveAllDecals(i);
  }
}
...
//-----------------------------------------------------------------------------
CPooledVBAllocator_ColorMesh::CPooledVBAllocator_ColorMesh() : m_pMesh(NULL) {
  Clear();
}
...
bool CPooledVBAllocator_ColorMesh::Init(VertexFormat_t format, int numVerts) {
  ...
  CMatRenderContextPtr pRenderContext(materials);
  m_pMesh = pRenderContext->CreateStaticMesh(
      format, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_COLOR);
  if (m_pMesh) {
    // Build out the underlying vertex buffer
    CMeshBuilder meshBuilder;
    int numIndices = 0;
    meshBuilder.Begin(m_pMesh, MATERIAL_HETEROGENOUS, numVerts, numIndices);
    {
      ...
      // Probably good to catch any change to vertex size... there may be
...
void CPooledVBAllocator_ColorMesh::Clear(void) {
  if (m_pMesh != NULL) {
    if (m_numAllocations > 0) {
      Warning(
          "ERROR: CPooledVBAllocator_ColorMesh::Clear should not be called "
          "until all allocations released!");
...
bool CPooledVBAllocator_ColorMesh::CheckIsClear(void) {
  if (m_pMesh) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh's internal mesh (vertex buffer) "
...
//  - Allocate a sub-range of 'numVerts' from free space in the shared vertex
//  buffer
//    (returns the uint8_t offset from the start of the VB to the new
//    allocation)
...
int CPooledVBAllocator_ColorMesh::Allocate(int numVerts) {
  if (m_pMesh == NULL) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh::Allocate cannot be called before "
        "Init (expect a crash)");
    Assert(m_pMesh);
    return -1;
  }

  // Once we start deallocating, we have to keep going until everything has been
  // freed
  if (m_bStartedDeallocation) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh::Allocate being called after some "
        "(but not all) calls to Deallocate have been called - invalid! (expect "
        "visual artifacts)");
  ...
  if (numVerts > (m_totalVerts - m_numVertsAllocated)) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh::Allocate failing - not enough "
        "space left in the vertex buffer!");
...
void CPooledVBAllocator_ColorMesh::Deallocate(int offset, int numVerts) {
  if (m_pMesh == NULL) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh::Deallocate cannot be called "
        "before Init");
    Assert(m_pMesh != NULL);
    return;
  }
  ...
  if (m_numAllocations == 0) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh::Deallocate called too many "
        "times! (bug in calling code)");
    Assert(m_numAllocations > 0);
    return;
  }
  ...
  if (numVerts > m_numVertsAllocated) {
    Warning(
        "ERROR: CPooledVBAllocator_ColorMesh::Deallocate called with too many "
        "verts, trying to free more than were allocated (bug in calling code)");
    Assert(numVerts <= m_numVertsAllocated);
    numVerts =
        m_numVertsAllocated;  // Hack (avoid counters ever going below zero)
  }
  ...
  m_numAllocations -= 1;
  m_numVertsAllocated -= numVerts;
  m_nextFreeOffset = 0;  // (we shouldn't be returning this until everything's
                         // free, at which point 0 is valid)

  // Are we empty?
  if (m_numAllocations == 0) {
    if (m_numVertsAllocated != 0) {
      Warning(
          "ERROR: CPooledVBAllocator_ColorMesh::Deallocate, after all "
          "allocations have been freed too few verts total have been "
          "deallocated (bug in calling code)");
