...
typedef uint32_t TraceCounter_t;
...
struct TraceInfo_t {
  ...
  bool Visit(cbrush_t *pBrush, int ndxBrush, TraceCounter_t cachedCount,
             TraceCounter_t *pCachedCounters);
  bool Visit(int dispIndex, TraceCounter_t cachedCount,
             TraceCounter_t *pCachedCounters);
};
...
// 48-bytes, aligned to 16-uint8_t boundary
// this is a brush that is an AABB.  It's encoded this way instead of with 6
// brushsides
struct cboxbrush_t {
  VectorAligned mins;
  VectorAligned maxs;
 ...
 public:
  ...
  T *Get() {
    if (!m_buf.TellPut()) {
      ...
      if (!g_pFileSystem->ReadFile(m_pFilename, NULL, m_buf,
                                   sizeof(T) * m_nCount, m_nOffset)) {
        return NULL;
      }
    }
...
class CCollisionBSPData {
 public:
  // This is sort of a hack, but it was a little too painful to do this any
  // other way The goal of this dude is to allow us to override the tree with
  // some other tree (or a subtree)
...
//=============================================================================
//
// This might eventually become a class/interface when we want multiple
// instances etc.......for now....
//
...
//
// UNDONE: Find a way to overlap the counter/contents bits with mins.w/maxs.w
// without hosing performance
struct alignedbbox_t {
  ...
  int dispContents;
  int pad[2];
  void SetCounter(int counter) { dispCounter = counter; }
  int GetCounter() { return dispCounter; }
  void SetContents(int contents) { dispContents = contents; }
  int GetContents() { return dispContents; }
  void Init(const Vector &minsIn, const Vector &maxsIn, int counterIn,
            int contentsIn) {
    mins = minsIn;
...
void CM_TestInDispTree(TraceInfo_t *pTraceInfo, cleaf_t *pLeaf,
                       Vector const &traceStart, Vector const &boxMin,
