...
IMaterialProxy *CMaterialProxyFactory::CreateProxy(const char *proxyName) {
  ...
  IMaterialProxy *materialProxy =
      LookupProxy(proxyName, Sys_GetFactory(g_ClientDLLModule));
...
// Look up proxy
//-----------------------------------------------------------------------------
IMaterialProxy *CMaterialProxyFactory::LookupProxy(const char *proxyName,
  ...
  int buflen =
      Q_strlen(proxyName) + Q_strlen(IMATERIAL_PROXY_INTERFACE_VERSION) + 1;

  proxyVersionedName = (char *)_alloca(buflen);
  Q_strncpy(proxyVersionedName, proxyName, buflen);
  Q_strncat(proxyVersionedName, IMATERIAL_PROXY_INTERFACE_VERSION, buflen,
            COPY_ALL_CHARACTERS);
