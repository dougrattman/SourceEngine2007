...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerRemoteAccess, IGameServerData,
                                  GAMESERVERDATA_INTERFACE_VERSION,
...
static ConCommand stats("stats", Host_Stats_f,
...
//-----------------------------------------------------------------------------
ra_listener_id CServerRemoteAccess::GetNextListenerID(bool authConnection,
                                                      const netadr_t *adr) {
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::WriteDataRequest(CRConServer *pNetworkListener,
                                           ra_listener_id listener,
  ...
  if (bufferSize <
      2 * sizeof(
              int))  // check that the buffer contains at least the id and type
  {
    return;
  }
  ...
  while (invalidRequest == false &&
         (int)cmd.TellGet() <
             (int)(cmd.Size() -
                   2 * sizeof(int)))  // while there is commands to read
  {
    // parse out the buffer
    int requestID = cmd.GetInt();
    pNetworkListener->SetRequestID(
        listener, requestID);  // tell the rcon server the ID so it can reflect
                               // it when the console redirect flushes
    ...
    switch (requestType) {
      ...
      case SERVERDATA_SEND_CONSOLE_LOG: {
        ...
        if (g_pFullFileSystem->ReadFile(pLogFile, "GAME", buf)) {
          ...
          SendResponseToClient(listener, SERVERDATA_CONSOLE_LOG_RESPONSE, pMem,
                               nLen);
          CloseZip(hZip);
        } else {
...
void CServerRemoteAccess::WriteDataRequest(ra_listener_id listener,
                                           const void *buffer, int bufferSize) {
...
#ifndef SWDS
  if (m_nScreenshotListener < 0) return;
...
void CServerRemoteAccess::LogCommand(ra_listener_id listener, const char *msg) {
  if (!sv_rcon_log.GetBool()) return;

  if (listener >= 0 && listener < (ra_listener_id)m_ListenerIDs.Count() &&
      m_ListenerIDs[listener].m_bHasAddress) {
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::CheckPassword(CRConServer *pNetworkListener,
                                        ra_listener_id listener, int requestID,
                                        const char *password) {
  ...
  m_ResponsePackets[i].responderID =
      listener;  // record who we need to respond to
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::BadPassword(CRConServer *pNetworkListener,
                                      ra_listener_id listener) {
  // allocate a spot in the list for the response
  int i = m_ResponsePackets.AddToTail();
  m_ResponsePackets[i].responderID =
...
int CServerRemoteAccess::GetDataResponseSize(ra_listener_id listener) {
  for (int i = m_ResponsePackets.Head(); m_ResponsePackets.IsValidIndex(i);
       i = m_ResponsePackets.Next(i)) {
    // copy response into buffer
    if (m_ResponsePackets[i].responderID !=
        listener)  // not for us, skip to the next entry
      continue;
...
int CServerRemoteAccess::ReadDataResponse(ra_listener_id listener, void *buffer,
                                          int bufferSize) {
  for (int i = m_ResponsePackets.Head(); m_ResponsePackets.IsValidIndex(i);
       i = m_ResponsePackets.Next(i)) {
    // copy response into buffer
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::RequestValue(ra_listener_id listener, int requestID,
                                       const char *variable) {
  // look up the cvar
...
// Purpose: looks up a cvar and posts a return value
//-----------------------------------------------------------------------------
void CServerRemoteAccess::RespondString(ra_listener_id listener, int requestID,
                                        const char *pString) {
  // allocate a spot in the list for the response
  int i = m_ResponsePackets.AddToTail();
  m_ResponsePackets[i].responderID =
...
void CServerRemoteAccess::SetValue(const char *variable, const char *value) {
  ...
  if (!_stricmp(variable, "map")) {
    // push a map change command
    Cbuf_AddText(va("changelevel %s\n", value));
    Cbuf_Execute();
  } else if (!_stricmp(variable, "mapcycle")) {
    ...
    if (mapcycle.IsValid()) {
      ...
      if (!f) {
        ...
        Msg("Couldn't write to read-only file %s, using file "
            "_temp_mapcycle.txt instead.\n",
            mapcycle.GetString());
    ...
    // Stick the cvar set in the command string, so client notification,
    // replication, etc happens
...
bool CServerRemoteAccess::LookupValue(const char *variable, CUtlBuffer &value) {
  ...
  if (strval) {
    ...
    value.PutChar(0);
  } else if (!_stricmp(variable, "stats")) {
    char stats[512];
    GetStatsString(stats, sizeof(stats));
    value.PutString(stats);
    value.PutChar(0);
  } else if (!_stricmp(variable, "banlist")) {
    // returns a list of banned users and ip's
    GetUserBanList(value);
  } else if (!_stricmp(variable, "playerlist")) {
    GetPlayerList(value);
  } else if (!_stricmp(variable, "maplist")) {
    GetMapList(value);
  } else if (!_stricmp(variable, "uptime")) {
    int timeSeconds = (int)(Plat_FloatTime());
    value.PutInt(timeSeconds);
    value.PutChar(0);
  } else if (!_stricmp(variable, "ipaddress")) {
    char addr[25];
    Q_snprintf(addr, sizeof(addr), "%s:%i", net_local_adr.ToString(true),
               sv.GetUDPPort());
...
//-----------------------------------------------------------------------------
// Purpose: Finds the value of a particular server variable for simple string
// values
//-----------------------------------------------------------------------------
const char *CServerRemoteAccess::LookupStringValue(const char *variable) {
  ...
  if (pVar) {
    ConVarRef var(pVar);
    if (var.IsValid()) return var.GetString();
  }

  // special types
  if (!Q_stricmp(variable, "map")) return sv.GetMapName();

  if (!Q_stricmp(variable, "playercount")) {
    Q_snprintf(s_ReturnBuf, sizeof(s_ReturnBuf) - 1, "%d",
...
void CServerRemoteAccess::GetUserBanList(CUtlBuffer &value) {
  ...
  for (i = 0; i < g_IPFilters.Count(); i++) {
    ...
    *(unsigned *)b = g_IPFilters[i].compare;
    value.Printf("%i %i.%i.%i.%i : %.3f min\n", i + 1 + g_UserFilters.Count(),
                 b[0], b[1], b[2], b[3], g_IPFilters[i].banTime);
  }
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    ...
    if (client->IsFakeClient()) {
      value.Printf("\"%s\" %s 0 0 0 %d 0\n", client->GetClientName(),
                   client->GetNetworkIDString(), pl->frags);
    } else {
      const INetChannel *net_channel{client->GetNetChannel()};

      value.Printf("\"%s\" %s %s %d %d %d %d\n", client->GetClientName(),
                   client->GetNetworkIDString(), net_channel->GetAddress(),
                   (int)(net_channel->GetAvgLatency(FLOW_OUTGOING) * 1000.0f),
                   (int)(net_channel->GetAvgLoss(FLOW_INCOMING)), pl->frags,
                   (int)(net_channel->GetTimeConnected()));
    }
  }
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::SendMessageToAdminUI(ra_listener_id listenerID,
                                               const char *message) {
  if (listenerID != m_AdminUIID) {
    Warning(
        "ServerRemoteAccess: Sending AdminUI message to non-AdminUI "
        "listener\n");
  }
  ...
  int i = m_ResponsePackets.AddToTail();
  m_ResponsePackets[i].responderID =
      listenerID;  // record who we need to respond to
  CUtlBuffer &response = m_ResponsePackets[i].packet;
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::SendResponseToClient(ra_listener_id listenerID,
...
//-----------------------------------------------------------------------------
void CServerRemoteAccess::SendVProfData(ra_listener_id listenerID,
                                        bool bGroupData, void *data, int len) {
  Assert(listenerID !=
         m_AdminUIID);  // only RCON clients support this right now
  SendResponseToClient(
      listenerID, bGroupData ? SERVERDATA_VPROF_GROUPS : SERVERDATA_VPROF_DATA,
      data, len);
}
...
extern "C" void NotifyDedicatedServerUI(const char *message) {
  if (g_ServerRemoteAccess.GetAdminUIID() !=
      INVALID_LISTENER_ID)  // if we have an admin UI actually registered
  {
    g_ServerRemoteAccess.SendMessageToAdminUI(
        g_ServerRemoteAccess.GetAdminUIID(), message);
  }
}
