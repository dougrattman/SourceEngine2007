...
class CPureServerWhitelist {
 public:
  ...
  // Get the file lists.
  IFileList *
  GetAllowFromDiskList();  // This is the list of files that can be loaded off
                           // disk. Make sure to Release() it when finished!
  IFileList *
  GetForceMatchList();  // This is the list of files that must match CRCs. Make
                        // sure to Release() it when finished!
  ...
  // Calls IFileSystem::CacheFileCRCs for all files and directories marked
  // check_crc.
  void CacheFileCRCs();

  // Instead of loading keyvalues, you can call this to make it force all files
  // to come from Steam. This is used with sv_pure 2.
 ...
 private:
  ...
  class CCommand {
   ...
   public:
    ...
    bool m_bCheckCRC;
    unsigned short m_LoadOrder;  // What order this thing was specified in the
                                 // whitelist file.
  };
 ...
 private:
  ...
  void PrintCommand(const char *pFileSpec, const char *pExt, int maxPathnameLen,
                    CPureServerWhitelist::CCommand *pCommand);
  int FindCommandByLoadOrder(
      CUtlDict<CPureServerWhitelist::CCommand *, int> &commands,
      int iLoadOrder);

  void InternalCacheFileCRCs(CUtlDict<CCommand *, int> &theList,
                             ECacheCRCType eType);
 ...
 private:
  unsigned short m_LoadCounter;  // Incremented as we load things so their
                                 // m_LoadOrder increases.
  int m_RefCount;
