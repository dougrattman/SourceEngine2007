...
void CL_StartMovie(const char *filename, int flags, int nWidth, int nHeight,
                   float flFrameRate, int jpeg_quality);
void CL_EndMovie();
...
class CEngineTool : public IEngineToolInternal {
 ...
 public:
  // Retrieve factories from server.dll and client.dll to get at specific
  // interfaces defined within
  ...
  // Method for causing engine to call client to render scene with no view model
  // or overlays
  virtual void RenderView(CViewSetup &view, int nFlags, int nWhatToRender);

  // Returns true if the player is fully connected and active in game (i.e, not
  // still loading)
  virtual bool IsInGame();
  // Returns true if the player is connected, but not necessarily active in game
  // (could still be loading)
  virtual bool IsConnected();

  virtual int
  GetMaxClients();  // Tools might want to ensure single player, e.g.
  ...
  virtual void SetClientFrameTime(
      float frametime);  // gpGlobals->frametime on client

  // Currently the engine doesn't like to do networking when it's paused, but if
  // a tool changes entity state, it can be useful to force
  ...
  virtual int StartSound(int iUserData, bool staticsound, int iEntIndex,
                         ...
                         int iPitch = PITCH_NORM, bool bUpdatePositions = true,
                         float delay = 0.0f, int speakerentity = -1);
  ...
  // Even if game is paused, force networking to update to get new server state
  // down to client
  virtual void ForceSend();

  virtual bool IsRecordingMovie();

  // NOTE: Params can contain file name, frame rate, output avi, output raw, and
  // duration
  ...
  virtual void StartRecordingVoiceToFile(const char *filename,
  ...
  virtual void TraceRay(const Ray_t &ray, unsigned int fMask,
                        ITraceFilter *pTraceFilter, CBaseTrace *pTrace);
  ...
  virtual int GetLightingConditions(const Vector &vecPosition, Vector *pColors,
                                    int nMaxLocalLights,
...
//-----------------------------------------------------------------------------
bool EngineTool_CheckQuitHandlers() { return g_EngineTool.CanQuit(); }

void EngineTool_UpdateScreenshot() { g_EngineTool.UpdateScreenshot(); }
...
// Singleton
//-----------------------------------------------------------------------------
void CEngineTool::ShowCursor(bool show) { Assert(0); }
...
int CEngineTool::GetLightingConditions(const Vector &vecLightingOrigin,
...
void CEngineTool::Command(const char *cmd) { Cbuf_AddText(cmd); }

void CEngineTool::Execute() { Cbuf_Execute(); }

const char *CEngineTool::GetCurrentMap() {
  if (sv.IsDedicated()) return "Dedicated Server";

  if (!cl.IsConnected()) {
    if (sv.IsLoading()) return sv.GetMapName();
...
void CEngineTool::SetIsInGame(bool bIsInGame) { m_bIsInGame = bIsInGame; }

bool CEngineTool::IsInGame() { return m_bIsInGame && cl.IsConnected(); }

bool CEngineTool::IsConnected() { return cl.IsConnected(); }

int CEngineTool::GetMaxClients() { return cl.m_nMaxClients; }

bool CEngineTool::IsGamePaused() { return cl.IsPaused(); }
...
void CEngineTool::SetGamePaused(bool paused) { sv.SetPaused(paused); }

float CEngineTool::GetTimescale() { return host_timescale.GetFloat(); }

void CEngineTool::SetTimescale(float scale) { host_timescale.SetValue(scale); }

float CEngineTool::Time() { return Plat_FloatTime(); }

// Real time is unscaled, but is updated once per frame
float CEngineTool::GetRealTime() { return realtime; }

float CEngineTool::GetRealFrameTime() { return host_frametime; }

float CEngineTool::HostFrameTime() { return host_frametime; }

float CEngineTool::HostTime() { return host_time; }

int CEngineTool::HostTick() { return host_tickcount; }

int CEngineTool::HostFrameCount() { return host_framecount; }

float CEngineTool::ServerTime() { return g_ServerGlobalVariables.curtime; }

float CEngineTool::ServerFrameTime() {
  return g_ServerGlobalVariables.frametime;
}
int CEngineTool::ServerTick() { return g_ServerGlobalVariables.tickcount; }

float CEngineTool::ServerTickInterval() {
  return g_ServerGlobalVariables.interval_per_tick;
}

float CEngineTool::ClientTime() { return g_ClientGlobalVariables.curtime; }

float CEngineTool::ClientFrameTime() {
  return g_ClientGlobalVariables.frametime;
}

int CEngineTool::ClientTick() { return g_ClientGlobalVariables.tickcount; }

void CEngineTool::SetClientFrameTime(float frametime) {
  g_ClientGlobalVariables.frametime = frametime;
}

void CEngineTool::ForceUpdateDuringPause() { SV_ForceSend(); }
...
// Output : int
//-----------------------------------------------------------------------------
int CEngineTool::GetToolCount() const { return toolframework->GetToolCount(); }
...
int CEngineTool::StartSound(int iUserData, bool staticsound, int iEntIndex,
                            int iChannel, const char *pSample, float flVolume,
                            soundlevel_t iSoundlevel, const Vector &origin,
                            const Vector &direction, int iFlags /*= 0*/,
...
void CEngineTool::StopSoundByGuid(int guid) { S_StopSoundByGuid(guid); }
...
void CEngineTool::ReloadSound(const char *pSample) { S_ReloadSound(pSample); }

void CEngineTool::StopAllSounds() { S_StopAllSounds(true); }
...
void CEngineTool::TraceRay(const Ray_t &ray, unsigned int fMask,
                           ITraceFilter *pTraceFilter, CBaseTrace *pTrace) {
  trace_t tempTrace;
...
void CEngineTool::TraceRayServer(const Ray_t &ray, unsigned int fMask,
...
bool CEngineTool::GetPlayerView(CViewSetup &viewSetup, int x, int y, int w,
                                int h) {
  if (g_ClientDLL) {
    if (!g_ClientDLL->GetPlayerView(viewSetup)) return false;
    ...
    viewSetup.width = w;
    viewSetup.height = h;
    viewSetup.m_flAspectRatio =
        (viewSetup.height != 0)
            ? (float)viewSetup.width / (float)viewSetup.height
            : 4.0f / 3.0f;
    viewSetup.m_bRenderToSubrectOfLargerScreen = true;
    viewSetup.fov = ScaleFOVByWidthRatio(
        viewSetup.fov, viewSetup.m_flAspectRatio / (4.0f / 3.0f));
    viewSetup.fovViewmodel = ScaleFOVByWidthRatio(
        viewSetup.fovViewmodel, viewSetup.m_flAspectRatio / (4.0f / 3.0f));
    return true;
  }
  return false;
}
...
void CEngineTool::CreatePickingRay(const CViewSetup &viewSetup, int x, int y,
                                   Vector &org, Vector &forward) {
  ...
  // Create two points at the normalized mouse x, y pos and at the near and far
  // z planes (0 and 1 depth)
  Vector v1, v2;
  v1.Init(xf, yf, 0.0f);
  v2.Init(xf, yf, 1.0f);
...
void CEngineTool::TakeTGAScreenShot(const char *filename, int width,
                                    int height) {
...
void CEngineTool::UpdateScreenshot() {
  ...
  if (m_szScreenshotFile[0]) {
    g_ClientDLL->WriteSaveGameScreenshotOfSize(
        m_szScreenshotFile, m_nScreenshotWidth, m_nScreenshotHeight);
    m_szScreenshotFile[0] = 0;
  }
}
...
AVIHandle_t CEngineTool::GetRecordingAVIHandle() {
  if (!CL_IsRecordingMovie()) return AVIHANDLE_INVALID;
...
void CEngineTool::StartRecordingVoiceToFile(const char *filename,
                                            const char *pPathID /*= 0*/) {
  ...
  g_pFileSystem->RelativePathToFullPath(filename, pPathID, m_szVoiceoverFile,
                                        sizeof(m_szVoiceoverFile));
...
float CEngineTool::GetMono16Samples(const char *pszName,
                                    CUtlVector<short> &sampleList) {
  return S_GetMono16Samples(pszName, sampleList);
}

void CEngineTool::GetWorldToScreenMatrixForView(const CViewSetup &view,
                                                VMatrix *pVMatrix) {
  VMatrix worldToView, viewToProjection;
  ComputeViewMatrices(&worldToView, &viewToProjection, pVMatrix, view);
}

SpatialPartitionHandle_t CEngineTool::CreatePartitionHandle(
    IHandleEntity *pEntity, SpatialPartitionListMask_t listMask,
    const Vector &mins, const Vector &maxs) {
...
void CEngineTool::InstallPartitionQueryCallback(
    IPartitionQueryCallback *pQuery) {
  SpatialPartition()->InstallQueryCallback(pQuery);
}

void CEngineTool::RemovePartitionQueryCallback(
    IPartitionQueryCallback *pQuery) {
  SpatialPartition()->RemoveQueryCallback(pQuery);
}

void CEngineTool::ElementMoved(SpatialPartitionHandle_t handle,
                               const Vector &mins, const Vector &maxs) {
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTool, IEngineTool,
                                  VENGINETOOL_INTERFACE_VERSION, g_EngineTool);
// Expose simple interface
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTool, IEngineToolFramework,
                                  VENGINETOOLFRAMEWORK_INTERFACE_VERSION,
                                  g_EngineTool);
