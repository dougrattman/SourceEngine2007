...
struct leafvis_t {
  ...
  bool IsValid() {
    CCollisionBSPData *pBSP = GetCollisionBSPData();
    if (!pBSP || numbrushes != pBSP->numbrushes ||
        numentitychars != pBSP->numentitychars)
      return false;
...
static void AddPlaneToList(CUtlVector<cplane_t> &list, const Vector &normal,
                           float dist, int invert) {
  cplane_t plane;
...
void TranslatePlaneList(cplane_t *pPlanes, int planeCount,
                        const Vector &offset) {
  for (int i = 0; i < planeCount; i++) {
...
void CSGPlaneList(leafvis_t *pVis, CUtlVector<cplane_t> &planeList) {
  ...
  // Build the CSG solid of this leaf given that the planes in the list define a
  // convex solid
  for (int i = 0; i < planeCount; i++) {
    // Build a big-ass poly in this plane
    int vertCount = PolyFromPlane(vertsIn, planeList[i].normal,
                                  planeList[i].dist);  // BaseWindingForPlane()
    ...
    for (j = 0; j < planeCount; j++) {
      ...
      vertCount = ClipPolyToPlane(vertsIn, vertCount, vertsOut,
                                  planeList[j].normal, planeList[j].dist);

      // Just copy the verts each time, don't bother swapping pointers
      // (efficiency is not a goal here)
...
ConVar mat_leafvis("mat_leafvis", "0", FCVAR_CHEAT,
                   "Draw wireframe of current leaf", LeafvisChanged);
ConVar r_visambient(
    "r_visambient", "0", 0,
...
void LeafVisBuild(const Vector &p) {
  if (!mat_leafvis.GetInt()) {
    ...
    if (g_LeafVis && last_leaf == leafIndex) return;

    DevMsg(1, "Leaf %d, Area %d, Cluster %d\n", leafIndex,
           CM_LeafArea(leafIndex), CM_LeafCluster(leafIndex));
    last_leaf = leafIndex;
...
void DrawLeafvis(leafvis_t *pVis) {
  ...
  int vert = 0;
  g_materialLeafVisWireframe->ColorModulate(pVis->color[0], pVis->color[1],
                                            pVis->color[2]);
    ...
    if (pVis->polyVertCount[i] >= 3) {
      ...
      for (int j = 0; j < pVis->polyVertCount[i]; j++) {
        ...
        meshBuilder.Position3fv(
            pVis->verts[vert + ((j + 1) % pVis->polyVertCount[i])].Base());
        meshBuilder.AdvanceVertex();
      }
      meshBuilder.End();
      pMesh->Draw();
...
void RecomputeClipbrushes(bool bEnabled) {
  ...
  for (int v = 0; v < 3; v++) {
    int contents[3] = {CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP,
                       CONTENTS_MONSTERCLIP, CONTENTS_PLAYERCLIP};
    ...
    for (int i = 0; i < lastBrush; i++) {
      ...
      if ((pBrush->contents & (CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP)) ==
          contents[v]) {
        CUtlVector<cplane_t> planeList;
        if (pBrush->IsBox()) {
          ...
          for (int j = 0; j < pBrush->numsides; j++) {
            cbrushside_t *pSide =
                &pBSP->map_brushsides[pBrush->firstbrushside + j];
            if (pSide->bBevel) continue;
            AddPlaneToList(planeList, pSide->plane->normal, pSide->plane->dist,
                           true);
          }
        }
...
static ConVar r_drawclipbrushes(
    "r_drawclipbrushes", "0", FCVAR_CHEAT,
    "Draw clip brushes (red=NPC+player, pink=player, purple=NPC)", ClipChanged);

static Vector LeafAmbientSamplePos(int leafIndex,
                                   const mleafambientlighting_t &sample) {
...
// some simple helpers to draw a cube in the special way the ambient
// visualization wants
...
static void CubeFace(CMeshBuilder &meshBuilder, const Vector &org, int v0,
  ...
  meshBuilder.Color4ubv((uint8_t *)&color);
  meshBuilder.AdvanceVertex();
...
void LeafVisDraw(void) {
  ...
  if (g_LeafVis && r_visambient.GetBool()) {
    ...
    if (!pAmbient->ambientSampleCount && pAmbient->firstAmbientSample) {
      // this leaf references another leaf, move there (this leaf is a solid
      // leaf so it borrows samples from a neighbor)
    ...
    for (int i = 0; i < pAmbient->ambientSampleCount; i++) {
      ...
      const mleafambientlighting_t &sample =
          host_state.worldbrush
              ->m_pAmbientSamples[pAmbient->firstAmbientSample + i];
