...
class CPerfUIChildPanel : public vgui::EditablePanel {
 ...
 public:
  CPerfUIChildPanel(vgui::Panel *parent, const char *pName)
...
const char *CPropFadeUIPanel::s_pFadeVisualizeLabel
    [CPropFadeUIPanel::VISUALIZE_TYPE_COUNT] = {
        "No visualization", "Show Fade Distance", "Show Fade Screen Width"};

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
CPropFadeUIPanel::CPropFadeUIPanel(vgui::Panel *parent)
...
// Purpose:
//-----------------------------------------------------------------------------
CAreaPortalsUIPanel::CAreaPortalsUIPanel(vgui::Panel *parent)
    : BaseClass(parent, "AreaPortalUIPanel") {}
...
const char *COcclusionUIPanel::s_pOccVisualizeLabel
    [COcclusionUIPanel::VISUALIZE_TYPE_COUNT] = {
        "No visualization",
        "View occluders",
};
...
// Purpose:
//-----------------------------------------------------------------------------
COcclusionUIPanel::COcclusionUIPanel(vgui::Panel *parent)
    : BaseClass(parent, "AreaPortalUIPanel") {
  m_pVisualization =
      new ComboBox(this, "VisualizeMode", VISUALIZE_TYPE_COUNT, false);
  int i;
  ...
  m_pDeactivateOcclusion =
      new vgui::CheckButton(this, "DeactivateOcclusion", "");
...
//-----------------------------------------------------------------------------
CPerfUIPanel::CPerfUIPanel(vgui::Panel *parent)
    : BaseClass(parent, "PerfUIPanel") {
  ...
  int y =
      (videomode->GetModeHeight() - h) / 2 + videomode->GetModeHeight() * 0.2;
  SetBounds(x, y, w, h);
...
void CPerfUIPanel::OnTextChanged(KeyValues *data) {
  ...
  if (pBox ==
      m_pPerformanceTool)  // don't change the text in the config setting combo
  {
    OnPerfToolSelected();
  }
}
...
static ConCommand perfui("perfui", ShowHidePerfUI,
