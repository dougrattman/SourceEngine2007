...
// ------------------------------------------------------------------------------------
// // CClientSendTable and CClientSendProp. This is the data that we receive a
// SendTable into.
//
...
class CClientSendProp {
 ...
 private:
  char* m_pTableName;  // For DPT_DataTable properties.. this tells the table
                       // name.
};

//
// CClientSendTables are the client's version of the SendTables that are sent
// down from the server. It stores these, then builds CRecvDecoders that allow
// it to decode packets of data.
//
...
inline const char* CRecvDecoder::GetName() const { return m_pTable->GetName(); }

inline SendTable* CRecvDecoder::GetSendTable() const {
  return m_Precalc.GetSendTable();
}

inline RecvTable* CRecvDecoder::GetRecvTable() const { return m_pTable; }

inline int CRecvDecoder::GetNumProps() const { return m_Props.Count(); }

inline const RecvProp* CRecvDecoder::GetProp(int i) const { return m_Props[i]; }
