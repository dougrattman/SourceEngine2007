...
// multiplayer game, tracking new players and dropped players during the
// game, and reporting game results and stats after the game is complete.
...
static ConVar mm_max_spectators("mm_max_spectators", "4", 0,
                                "Max players allowed on the spectator team");
...
void CMatchmaking::StartHost(bool bSystemLink) {
  ...
  for (int i = 0; i < m_SessionProperties.Count(); ++i) {
    ...
    m_Session.SetProperty(prop.dwPropertyId, prop.value.type, &prop.value.nData,
                          false);
  }
...
void CMatchmaking::OnHostSessionCreated() {
  ...
  if (pScenario) {
    Q_strncpy(m_HostData.scenario,
              pScenario->GetString("displaystring", "Unknown"),
              sizeof(m_HostData.scenario));
  }
  ...
  // Session has been created and advertised. Start listening for connection
  // requests.
...
void CMatchmaking::HandleSystemLinkSearch(netpacket_t *pPacket) {
  ...
  if (!m_Session.IsSystemLink() || !m_Session.IsHost() || m_Session.IsFull() ||
  ...
  msg.WriteByte(m_Session.GetSessionSlots(SLOTS_TOTALPUBLIC) -
                m_Session.GetSessionSlots(SLOTS_FILLEDPUBLIC));
  msg.WriteByte(m_Session.GetSessionSlots(SLOTS_TOTALPRIVATE) -
  ...
  uint32_t nScenarioId = g_ClientDLL->GetPresenceID("CONTEXT_SCENARIO");
  uint32_t nScenarioValue = 0;
  ...
  for (int i = 0; i < m_SessionContexts.Count(); ++i) {
    ...
    // Get the scenario id so the correct info can be displayed in the session
    // browser
...
//			request is accepted, transmit session info to the new
// client, and 			notify all connected clients of the new
// addition.
//-----------------------------------------------------------------------------
void CMatchmaking::HandleJoinRequest(netpacket_t *pPacket) {
  ...
  if (!IsAcceptingConnections()) {
    ...
    int nSlotsOpen = m_Session.GetSessionSlots(SLOTS_TOTALPUBLIC) -
                     m_Session.GetSessionSlots(SLOTS_FILLEDPUBLIC);
    if (tempClient.m_bInvited) {
      // Only invited clients can take private slots
      nSlotsOpen += m_Session.GetSessionSlots(SLOTS_TOTALPRIVATE) -
                    m_Session.GetSessionSlots(SLOTS_FILLEDPRIVATE);
    }
    ...
    if (tempClient.m_cPlayers > nSlotsOpen) {
      ...
      // There is room for the new client - fill out the rest of the response
      // fields
      ...
      if (!GameIsActive()) {
        // If the game is in progress, the new client will choose a team after
        // connecting
        joinResponse.m_iTeam = ChooseTeam();
      } else {
        // Tell the client to join the game in progress
        joinResponse.m_iTeam = -1;
        joinResponse.m_ResponseType =
            joinResponse.JOINRESPONSE_APPROVED_JOINGAME;
      }
    }
  }
  ...
  if (joinResponse.m_ResponseType == joinResponse.JOINRESPONSE_APPROVED ||
    ...
    for (int i = 0; i < tempClient.m_cPlayers; ++i) {
      ...
      pPacket->message.ReadBytes(&pNewClient->m_cVoiceState,
                                 sizeof(pNewClient->m_cVoiceState));
      pNewClient->m_iTeam[i] = joinResponse.m_iTeam;
      pPacket->message.ReadString(pNewClient->m_szGamertags[i],
                                  sizeof(pNewClient->m_szGamertags[i]), true);
    }
...
//-----------------------------------------------------------------------------
void CMatchmaking::UpdateSessionReplyData(uint32_t flags) {
#if defined(_X360)
  if (m_Session.GetSessionHandle() == INVALID_HANDLE_VALUE) return;

  // Enable listening for client Quality Of Service probes
  XNKID id = m_Session.GetSessionId();
  uint32_t ret =
      XNetQosListen(&id, (BYTE *)&m_HostData, sizeof(m_HostData), 0, flags);
  if (ret) {
...
bool CMatchmaking::StartGame() {
  ...
  if (GetPlayersNeeded() != 0) return false;
...
void CMatchmaking::SwitchToNextOpenTeam(CClientInfo *pClient) {
  ...
  do {
    ...
    if (CountPlayersOnTeam(iTeam) < maxPlayersPerTeam) break;
...
int CMatchmaking::GetPlayersNeeded() {
  ...
  if (m_Session.IsSystemLink()) return 0;
