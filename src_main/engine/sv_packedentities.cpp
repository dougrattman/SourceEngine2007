...
ConVar sv_debugmanualmode("sv_debugmanualmode", "0", 0,
                          ...
                          "their network data has changed.");
...
void SV_EnsureInstanceBaseline(ServerClass *pServerClass, int iEdict,
  ...
  if (pClass->m_InstanceBaselineIndex == INVALID_STRING_INDEX) {
...

    {
      ...
      pClass->m_InstanceBaselineIndex =
          sv.GetInstanceBaselineTable()->AddString(
              true,
              idString,  // Note we're sending a string with the ID number, not
                         // the class name.
              nBytes, pData);
    }
...
static inline void SV_PackEntity(int edictIdx, edict_t *edict,
  ...
  if (!edict->HasStateChanged()) {
    ...
    bUsedPrev = framesnapshotmanager->UsePreviouslySentPacket(
        pSnapshot, edictIdx, iSerialNum);
  }
  ...
  CUtlMemory<CSendProxyRecipients> recip(
      (CSendProxyRecipients *)tempData,
      pSendTable->m_pPrecalc->GetNumDataTableProxies());

  if (!SendTable_Encode(pSendTable, edict->GetUnknown(), &writeBuf, edictIdx,
                        &recip, false)) {
  ...
  if (vcr_verbose.GetInt() && writeBuf.GetNumBytesWritten() > 0)
    VCRGenericValueVerify("writebuf", writeBuf.GetBasePointer(),
                          writeBuf.GetNumBytesWritten() - 1);
#endif

  SV_EnsureInstanceBaseline(pServerClass, edictIdx, packedData,
                            writeBuf.GetNumBytesWritten());  //-V614

  int nFlatProps = SendTable_GetNumFlatProps(pSendTable);
  IChangeFrameList *pChangeFrame = NULL;

  // If this entity was previously in there, then it should have a valid
  ...
  PackedEntity *pPrevFrame = framesnapshotmanager->GetPreviouslySentPacket(
      edictIdx, pSnapshot->m_pEntities[edictIdx].m_nSerialNumber);
  if (pPrevFrame) {
    // Calculate a delta.
    ...
    int nChanges = SendTable_CalcDelta(pSendTable, pPrevFrame->GetData(),
    ...
    // If it's non-manual-mode, but we detect that there are no changes here,
    // then just use the previous pSnapshot if it's available (as though the
    // entity were manual mode). It would be interesting to hook here and see
    // how many non-manual-mode entities are winding up with no changes.
    if (nChanges == 0) {
      if (pPrevFrame->CompareRecipients(recip)) {
        if (framesnapshotmanager->UsePreviouslySentPacket(pSnapshot, edictIdx,
                                                          iSerialNum)) {
          edict->ClearStateChanged();
          return;
        }
      ...
      if (!edict->HasStateChanged()) {
        for (int iDeltaProp = 0; iDeltaProp < nChanges; iDeltaProp++) {
          ...
          Assert(deltaProps[iDeltaProp] <
                 pSendTable->m_pPrecalc->GetNumProps());

          const SendProp *pProp =
              pSendTable->m_pPrecalc->GetProp(deltaProps[iDeltaProp]);
          // If a field changed, but it changed because it encoded against
          // tickcount,
          //   then it's just like the entity changed the underlying field, not
          //   an error, that is.
          if (pProp->GetFlags() & SPROP_ENCODED_AGAINST_TICKCOUNT) continue;

          Msg("Entity %d (class '%s') reported ENTITY_CHANGE_NONE but '%s' "
              "changed.\n",
              edictIdx, edict->GetClassName(), pProp->GetName());
        }
      }
    }
...
#endif
    {
      // Ok, now snag the changeframe from the previous frame and update the
      // 'last frame changed' for the properties in the delta.
  ...
  // Now make a PackedEntity and store the new packed data in there.
  {
    PackedEntity *pPackedEntity =
        framesnapshotmanager->CreatePackedEntity(pSnapshot, edictIdx);
    pPackedEntity->SetChangeFrameList(pChangeFrame);
    pPackedEntity->SetServerAndClientClass(pServerClass, NULL);
    pPackedEntity->AllocAndCopyPadded(packedData,
...
// in HLTV mode we ALWAYS have to store position and PVS info, even if entity
// didnt change
void SV_FillHLTVData(CFrameSnapshot *pSnapshot, edict_t *edict,
                     int iValidEdict) {
#if !defined(_XBOX)
...
void PackEntities_NetworkBackDoor(int clientCount, CGameClient **clients,
                                  CFrameSnapshot *snapshot) {
  Assert(clientCount == 1);

  VPROF_BUDGET("PackEntities_NetworkBackDoor",
               VPROF_BUDGETGROUP_OTHER_NETWORKING);

  CGameClient *client =
      clients[0];  // update variables cl, pInfo, frame for current client
  CCheckTransmitInfo *pInfo = &client->m_PackInfo;

  for (int iValidEdict = 0; iValidEdict < snapshot->m_nValidEntities;
       iValidEdict++) {
    ...
    // If we're using the fast path for a single-player game, just pass the
    // entity directly over to the client.
    Assert(index < snapshot->m_nNumEntities);
    ServerClass *pSVClass = snapshot->m_pEntities[index].m_pClass;
    g_pLocalNetworkBackdoor->EntState(
        index, edict->m_NetworkSerialNumber, pSVClass->m_ClassID,
        pSVClass->m_pTable, edict->GetUnknown(), edict->HasStateChanged(),
        bShouldTransmit);
    edict->ClearStateChanged();
  }
...
struct PackWork_t {
  ...
  static void Process(PackWork_t &item) {
    SV_PackEntity(item.nIdx, item.pEdict,
                  item.pSnapshot->m_pEntities[item.nIdx].m_pClass,
                  item.pSnapshot);
  }
};

void PackEntities_Normal(int clientCount, CGameClient **clients,
                         CFrameSnapshot *snapshot) {
  ...
  for (int iValidEdict = 0; iValidEdict < snapshot->m_nValidEntities;
    ...
    for (int iClient = 0; iClient < clientCount; ++iClient) {
      ...
      CGameClient *client = clients[iClient];  // update variables cl, pInfo,
                                               // frame for current client
  ...
  if (sv_parallel_packentities.GetBool()) {
    ...
    for (int i = 0; i < c; ++i) {
      ...
      SV_PackEntity(w.nIdx, w.pEdict, w.pSnapshot->m_pEntities[w.nIdx].m_pClass,
                    w.pSnapshot);
    }
  }
...
void SV_ComputeClientPacks(int clientCount, CGameClient **clients,
  ...
  {
    VPROF_BUDGET_FLAGS("SV_ComputeClientPacks", "CheckTransmit",
                       BUDGETFLAG_SERVER);

    for (int iClient = 0; iClient < clientCount; ++iClient) {
      CCheckTransmitInfo *pInfo = &clients[iClient]->m_PackInfo;
      clients[iClient]->SetupPackInfo(snapshot);
      serverGameEnts->CheckTransmit(pInfo, snapshot->m_pValidEntities,
                                    snapshot->m_nValidEntities);
      clients[iClient]->SetupPrevPackInfo();
    }
  }

  VPROF_BUDGET_FLAGS("SV_ComputeClientPacks", "ComputeClientPacks",
                     BUDGETFLAG_SERVER);
...
// If the table's ID is -1, writes its info into the buffer and increments
// curID.
void SV_MaybeWriteSendTable(SendTable *pTable, bf_write &pBuf,
                            bool bNeedDecoder) {
  // Already sent?
  if (pTable->GetWriteFlag()) return;
...
void SV_WriteSendTables(ServerClass *pClasses, bf_write &pBuf) {
  ...
  // First, we send all the leaf classes. These are the ones that will need
  // decoders on the client.
...
void SV_ComputeClassInfosCRC(CRC32_t *crc) {
  ...
  for (ServerClass *pClass = pClasses; pClass; pClass = pClass->m_pNext) {
    CRC32_ProcessBuffer(crc, (void *)pClass->m_pNetworkName,
                        Q_strlen(pClass->m_pNetworkName));
    CRC32_ProcessBuffer(crc, (void *)pClass->m_pTable->GetName(),
...
void SV_WriteClassInfos(ServerClass *pClasses, bf_write &pBuf) {
  ...
  for (ServerClass *pClass = pClasses; pClass; pClass = pClass->m_pNext) {
    ...
    Q_strncpy(svclass.datatablename, pClass->m_pTable->GetName(),
              sizeof(svclass.datatablename));
    Q_strncpy(svclass.classname, pClass->m_pNetworkName,
              sizeof(svclass.classname));

    classinfomsg.m_Classes.AddToTail(
        svclass);  // add all known classes to message
  }

  classinfomsg.WriteToBuffer(pBuf);
}

// This is implemented for the datatable code so its warnings can include an
// object's classname.
