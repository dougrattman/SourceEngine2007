...
class CNetworkStringTable : public INetworkStringTable {
 public:
  ...
  CNetworkStringTable(TABLEID id, const char *tableName, int maxentries,
                      int userdatafixedsize, int userdatanetworkbits,
                      bool bIsFilenames);
 ...
 public:
  ...
  int AddString(bool bIsServer, const char *value, int length = -1,
 ...
 public:
  ...
  bool WriteBaselines(SVC_CreateStringTable &msg, char *msg_buffer,
                      int msg_buffer_size);
#endif
 ...
 protected:
  ...
  CNetworkStringTable(
      const CNetworkStringTable &);  // not implemented, not allowed

  TABLEID m_id;
  char *m_pszTableName;
  // Must be a power of 2, so encoding can determine # of bits to use based on
  // log2
  int m_nMaxEntries;
  int m_nEntryBits;
  int m_nTickCount;
  ...
  INetworkStringDict
      *m_pItemsClientSide;  // For m_bAllowClientSideAddString, these items are
                            // non-networked and are referenced by a negative
                            // string index!!!
};
...
class CNetworkStringTableContainer : public INetworkStringTableContainer {
 ...
 public:
  ...
  INetworkStringTable *CreateStringTable(const char *tableName, int maxentries,
    ...
    return CreateStringTableEx(tableName, maxentries, userdatafixedsize,
                               userdatanetworkbits, false);
  }
  INetworkStringTable *CreateStringTableEx(const char *tableName,
                                           int maxentries,
                                           int userdatafixedsize = 0,
                                           int userdatanetworkbits = 0,
