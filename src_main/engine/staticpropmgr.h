...
abstract_class IStaticPropMgrEngine {
 public:
  ...
  // Call this when there's a client, *after* LevelInit, and after the world
  // entity is loaded
  ...
  virtual bool IsPropInPVS(IHandleEntity * pHandleEntity, const uint8_t *pVis)
      const = 0;
  ...
  virtual bool PropHasBakedLightingDisabled(IHandleEntity * pHandleEntity)
      const = 0;
};
