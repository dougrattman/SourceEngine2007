...
struct mtexinfo_t {
  Vector4D textureVecsTexelsPerWorldUnits[2];  // [s/t] unit vectors in world
                                               // space. [i][3] is the s/t
...
#define MODELFLAG_TRANSLUCENT_TWOPASS \
  0x0008  // render opaque part in opaque pass
#define MODELFLAG_FRAMEBUFFER_TEXTURE \
  0x0010                             // we need the framebuffer as a texture
...
#define MODELFLAG_STUDIOHDR_DO_NOT_CAST_SHADOWS \
  0x1000  // persisted from studiohdr
...
struct decallist_t {
  ...
  // This is the surface plane that we hit so that we can move certain decals
  // across
  //  transitions if they hit similar geometry
  Vector impactPlaneNormal;
};

inline class IDispInfo *MLeaf_Disaplcement(
    mleaf_t *pLeaf, int index,
...
#define SURFDRAW_BUMPLIGHT \
  0x00000008  // Has multiple lightmaps for bump-mapping
#define SURFDRAW_NODRAW \
  0x00000010  // don't draw this surface, not really visible
#define SURFDRAW_TRANS 0x00000020  // sort this surface from back to front
#define SURFDRAW_PLANEBACK \
  0x00000040  // faces away from plane of the node that stores this face
...
#define SURFDRAW_HASDLIGHT \
  0x00080000  // Has some kind of dynamic light that must be checked
...
struct msurface1_t {
  ...
  short textureExtents[2];  // ?? s/t texture size, 1..512 for all non-sky
                            // surfaces
...
struct msurfacelighting_t {
  ...
  ColorRGBExp32 *AvgLightColor(int nLightStyleIndex) {
    return m_pSamples - (nLightStyleIndex + 1);
  }
  ...
  int m_nDLightFrame;  // Indicates the last frame in which dlights illuminated
                       // this surface

  unsigned char
      m_nStyles[MAXLIGHTMAPS];  // index into d_lightstylevalue[] for animated
                                // lights no one surface can be effected by more
                                // than 4 animated lights.

  // NOTE: This is tricky. To get this to fit in a single cache line,
  // and to save the extra memory of not having to store average light colors
  // for lightstyles that are not used, I store between 0 and 4 average light
  // colors +before+ the samples, depending on how many lightstyles there are.
  // Naturally, accessing an average color for an undefined lightstyle on the
  // surface results in undefined results. 0->4 avg light colors, *in reverse
  // order from m_nStyles* + [numstyles*surfsize]
...
inline const SurfaceHandle_t SurfaceHandleFromIndex(
    int surfaceIndex, const worldbrushdata_t *pData) {
  return &pData->surfaces2[surfaceIndex];
}

inline SurfaceHandle_t SurfaceHandleFromIndex(
    int surfaceIndex, worldbrushdata_t *pData = host_state.worldbrush) {
  return &pData->surfaces2[surfaceIndex];
}
...
inline void MSurf_SetSortGroup(SurfaceHandle_t surfID, int sortGroup) {
  unsigned int flags =
      (sortGroup << SURFDRAW_SORTGROUP_SHIFT) & SURFDRAW_SORTGROUP_MASK;
  surfID->flags |= flags;
}

/*
inline int& MSurf_DLightFrame( SurfaceHandle_t surfID, worldbrushdata_t *pData =
...
inline cplane_t &MSurf_Plane(SurfaceHandle_t surfID) { return *surfID->plane; }
...
inline short *MSurf_TextureExtents(
...
inline short *MSurf_LightmapMins(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline short *MSurf_LightmapExtents(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline short MSurf_MaxLightmapSizeWithBorder(SurfaceHandle_t surfID) {
  //	ASSERT_SURF_VALID( surfID );
  return SurfaceHasDispInfo(surfID) ? MAX_DISP_LIGHTMAP_DIM_INCLUDING_BORDER
                                    : MAX_BRUSH_LIGHTMAP_DIM_INCLUDING_BORDER;
}

inline short MSurf_MaxLightmapSizeWithoutBorder(SurfaceHandle_t surfID) {
  //	ASSERT_SURF_VALID( surfID );
  return SurfaceHasDispInfo(surfID) ? MAX_DISP_LIGHTMAP_DIM_WITHOUT_BORDER
                                    : MAX_BRUSH_LIGHTMAP_DIM_WITHOUT_BORDER;
}

inline mtexinfo_t *MSurf_TexInfo(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
...
inline ColorRGBExp32 *MSurf_AvgLightColor(
    SurfaceHandle_t surfID, int nIndex,
    worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline uint8_t *MSurf_Styles(SurfaceHandle_t surfID,
                             worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
/*
inline int *MSurf_CachedLight( SurfaceHandle_t surfID, worldbrushdata_t *pData =
host_state.worldbrush )
{
//	ASSERT_SURF_VALID( surfID );
        Assert( pData );
...
inline short& MSurf_CachedDLight( SurfaceHandle_t surfID, worldbrushdata_t
...
inline ColorRGBExp32 *MSurf_Samples(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline IDispInfo *MSurf_DispInfo(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
...
inline unsigned int &MSurf_FirstVertNormal(
  ...
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
  Assert(pData->surfacenormals[surfaceIndex].firstvertnormal <
...
inline short &MSurf_MaterialSortID(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  return surfID->materialSortID;
}

inline short *MSurf_OffsetIntoLightmapPage(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  //	ASSERT_SURF_VALID( surfID );
  Assert(pData);
...
inline OverlayFragmentHandle_t &MSurf_OverlayFragmentList(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  return surfID->m_nFirstOverlayFragment;
}

inline msurfacelighting_t *SurfaceLighting(
    SurfaceHandle_t surfID, worldbrushdata_t *pData = host_state.worldbrush) {
  int surfaceIndex = MSurf_Index(surfID, pData);
  Assert(pData);
