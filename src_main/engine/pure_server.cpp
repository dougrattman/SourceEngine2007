...
CPureServerWhitelist::~CPureServerWhitelist() { Term(); }
...
bool CPureServerWhitelist::LoadFromKeyValues(KeyValues *kv) {
  for (KeyValues *pCurItem = kv->GetFirstValue(); pCurItem;
    ...
    if (bFromSteam && (bAllowFromDisk || bCheckCRC)) {
      ...
      Warning(
          "Whitelist: from_steam not compatible with other modifiers (used on "
          "%s).\n",
          pValue);
    ...
    CPureServerWhitelist::CCommand *pCommand =
        new CPureServerWhitelist::CCommand;
    pCommand->m_LoadOrder = m_LoadCounter++;
    pCommand->m_bAllowFromDisk = bAllowFromDisk;
    pCommand->m_bCheckCRC = bCheckCRC;

    // Figure out if they're referencing a file, a recursive directory, or a
    // nonrecursive directory.
...
void CPureServerWhitelist::UpdateCommandStats(
    CUtlDict<CPureServerWhitelist::CCommand *, int> &commands, int *pHighest,
...
void CPureServerWhitelist::PrintCommand(
  ...
  if (len > 0 &&
      (tempFileSpec[len - 1] == '/' || tempFileSpec[len - 1] == '\\'))
...
int CPureServerWhitelist::FindCommandByLoadOrder(
    CUtlDict<CPureServerWhitelist::CCommand *, int> &commands, int iLoadOrder) {
  for (int i = commands.First(); i != commands.InvalidIndex();
       i = commands.Next(i)) {
    if (commands[i]->m_LoadOrder == iLoadOrder) return i;
  }
  return -1;
}
...
void CPureServerWhitelist::PrintWhitelistContents() {
  ...
  UpdateCommandStats(m_RecursiveDirCommands, &highestLoadOrder,
                     &longestPathName);
  UpdateCommandStats(m_NonRecursiveDirCommands, &highestLoadOrder,
                     &longestPathName);
  ...
  for (int iLoadOrder = 0; iLoadOrder <= highestLoadOrder; iLoadOrder++) {
    ...
    if (iCommand != -1) {
      PrintCommand(m_FileCommands.GetElementName(iCommand), NULL,
                   longestPathName, m_FileCommands[iCommand]);
    } else {
      // Check recursive commands.
      iCommand = FindCommandByLoadOrder(m_RecursiveDirCommands, iLoadOrder);
      if (iCommand != -1) {
        PrintCommand(m_RecursiveDirCommands.GetElementName(iCommand), "...",
                     longestPathName, m_RecursiveDirCommands[iCommand]);
      } else {
        // Check *.* commands.
        iCommand =
            FindCommandByLoadOrder(m_NonRecursiveDirCommands, iLoadOrder);
        if (iCommand != -1) {
          PrintCommand(m_NonRecursiveDirCommands.GetElementName(iCommand),
                       "*.*", longestPathName,
                       m_NonRecursiveDirCommands[iCommand]);
        }
      }
    }
  }
...
void CPureServerWhitelist::EncodeCommandList(
    CUtlDict<CPureServerWhitelist::CCommand *, int> &theList, CUtlBuffer &buf) {
  buf.PutInt(theList.Count());
  for (int i = theList.First(); i != theList.InvalidIndex();
       i = theList.Next(i)) {
    CPureServerWhitelist::CCommand *pCommand = theList[i];

    unsigned char val = 0;
    if (pCommand->m_bAllowFromDisk) val |= 0x01;
    if (pCommand->m_bCheckCRC) val |= 0x02;
...
void CPureServerWhitelist::CacheFileCRCs() {
  ...
  InternalCacheFileCRCs(m_RecursiveDirCommands,
                        k_eCacheCRCType_Directory_Recursive);
}

void CPureServerWhitelist::InternalCacheFileCRCs(
    CUtlDict<CCommand *, int> &theList, ECacheCRCType eType) {
  for (int i = theList.First(); i != theList.InvalidIndex();
       i = theList.Next(i)) {
...
void CPureServerWhitelist::DecodeCommandList(
    CUtlDict<CPureServerWhitelist::CCommand *, int> &theList, CUtlBuffer &buf) {
...
CPureServerWhitelist::CCommand *CPureServerWhitelist::GetBestEntry(
    const char *pFilename) {
  // NOTE: Since this is a user-specified file, we don't have the added
  // complexity of path IDs in here. So when the filesystem asks if a file is in
  ...
  if (!V_IsAbsolutePath(relativeFilename) ||
      m_pFileSystem->FullPathToRelativePath(pFilename, relativeFilename,
                                            sizeof(relativeFilename))) {
    ...
    // Check each of our dictionaries to see if there is an entry for this
    // thing.
    ...
    if (relativeDir[0] != 0) {
      pBestEntry =
          CheckEntry(m_NonRecursiveDirCommands, relativeDir, pBestEntry);

      while (relativeDir[0] != 0) {
        // Check for this directory.
        pBestEntry =
            CheckEntry(m_RecursiveDirCommands, relativeDir, pBestEntry);
        if (!V_StripLastDir(relativeDir, sizeof(relativeDir))) break;
      }
    }
  ...
  // Either we couldn't find an entry, or they specified an absolute path that
  // we could not convert to a relative path.
  return NULL;
}

CPureServerWhitelist::CCommand *CPureServerWhitelist::CheckEntry(
    CUtlDict<CPureServerWhitelist::CCommand *, int> &dict,
    const char *pEntryName, CPureServerWhitelist::CCommand *pBestEntry) {
  int i = dict.Find(pEntryName);
  if (i != dict.InvalidIndex() &&
      (!pBestEntry || dict[i]->m_LoadOrder > pBestEntry->m_LoadOrder))
    pBestEntry = dict[i];
...
bool CPureServerWhitelist::CAllowFromDiskList::IsFileInList(
    const char *pFilename) {
...
bool CPureServerWhitelist::CForceMatchList::IsFileInList(
    const char *pFilename) {
  // In "fully pure" mode, all files must come from disk.
  // We shouldn't need this because all the relevant files should be loaded from
  // Steam, but just in case, early out here.
  if (m_pWhitelist->m_bFullyPureMode) return false;
