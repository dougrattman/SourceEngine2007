...
#ifdef _DEBUG
ConVar cl_debug_respect_cheat_vars(
    "cl_debug_respect_cheat_vars", "0", 0,
    "(debug builds only) - when set to 0, the client can change cheat vars.");
#endif
...
int g_iFilterCommandsByServerCanExecute =
    0;  // If this is nonzero, then they want us to only run commands marked
        // with FCVAR_SERVER_CAN_EXECUTE.
int g_iFilterCommandsByClientCmdCanExecute =
    0;  // If this is nonzero, then they want us to only run commands marked
        // with FCVAR_CLIENTCMD_CAN_EXECUTE.

// This is a list of cvars that are in the client DLL that we want
// FCVAR_CLIENTCMD_CAN_EXECUTE set on. In order to avoid patching the client
// DLL, we setup this list. Whenever the client DLL has gone out with the
...
void Cmd_AddClientCmdCanExecuteVar(const char *pName) {
  if (g_ExtraClientCmdCanExecuteCvars.Find(pName) ==
      g_ExtraClientCmdCanExecuteCvars.InvalidIndex())
...
static int CreateExecutionMarker() {
  if (g_ExecutionMarkers.Count() > 2048) g_ExecutionMarkers.Remove(0);
...
static bool FindAndRemoveExecutionMarker(int iCode) {
  ...
  if (i == g_ExecutionMarkers.InvalidIndex()) return false;
...
void Cmd_SetRptActive(bool bActive) { g_bRPTActive = bActive; }
...
//-----------------------------------------------------------------------------
// Just translates BindToggle <key> <cvar> into: bind <key> "increment var
// <cvar> 0 1 1"
//-----------------------------------------------------------------------------
CON_COMMAND(BindToggle,
...
void Cbuf_AddExecutionMarker(ECmdExecutionMarker marker) {
  ...
  // CMDCHAR_ADD_EXECUTION_MARKER tells us there's a special execution thing
  // here. (char)marker tells it what to turn on iRandomCode is for security, so
  // only our code can stuff this command into the buffer.
  char str[512];
  Q_snprintf(str, sizeof(str), ";%s %c %d;", CMDSTR_ADD_EXECUTION_MARKER,
             (char)marker, iMarkerCode);
...
void Cbuf_Execute() {
  ...
  // If text was added with Cbuf_AddText and then Cbuf_Execute gets called from
  // within handler, we're going
  //  to execute the new commands anyway, so we can ignore this extra execute
  //  call here.
  if (s_CommandBuffer.IsProcessingCommands()) return;
...
static char const *Cmd_TranslateFileAssociation(char const *param) {
  ...
  // return 0 if no translation, otherwise return commands
  return retval;
}
...
// Also automatically converts .dem, .bsp, and .sav files to +playdemo etc
// command line options hl2 +cmd amlev1 hl2 -nosound +cmd amlev1 Output : void
// Cmd_StuffCmds_f
//-----------------------------------------------------------------------------
CON_COMMAND(stuffcmds,
...
bool IsValidFileExtension(const char *pszFilename) {
  ...
  if (Q_strstr(pszFilename, ".exe") || Q_strstr(pszFilename, ".vbs") ||
      Q_strstr(pszFilename, ".com") || Q_strstr(pszFilename, ".bat") ||
      Q_strstr(pszFilename, ".dll") || Q_strstr(pszFilename, ".ini") ||
      Q_strstr(pszFilename, ".gcf") || Q_strstr(pszFilename, ".sys") ||
...
void Cmd_Exec_f(const CCommand &args) {
  ...
  if (!f) {
    if (!V_stristr(s, "autoexec.cfg") && !V_stristr(s, "joystick.cfg") &&
        !V_stristr(s, "game.cfg")) {
...
static void HandleExecutionMarker(const char *pCommand,
                                  const char *pMarkerCode) {
  ...
  if (FindAndRemoveExecutionMarker(iMarkerCode)) {
    ...
    static int cnt = 0;
    if (++cnt < 3) Warning("Invalid execution marker code.\n");
  }
}
...
static bool ShouldPreventServerCommand(const ConCommandBase *pCommand) {
  if (!Host_IsSinglePlayerGame()) {
    if (g_iFilterCommandsByServerCanExecute > 0) {
      // If we don't understand the command, and it came from a server command,
      // then forward it back to the server. Lots of server plugins use this.
      // They use engine->ClientCommand() to have a client execute a command
      // that they have hooked on the server. We disabled it once and they
      // freaked. It IS redundant since they could just code the call to their
      // command on the server, but they complained loudly enough that we're
      // adding it back in since there's no exploit that we know of by allowing
      // it.
...
static bool ShouldPreventClientCommand(const ConCommandBase *pCommand) {
  if (g_iFilterCommandsByClientCmdCanExecute > 0 &&
      ...
      g_ExtraClientCmdCanExecuteCvars.Find(pCommand->GetName()) ==
          g_ExtraClientCmdCanExecuteCvars.InvalidIndex()) {
    // If this command is in the game DLL, don't mention it because we're going
    // to forward this request to the server and let the server handle it.
...
const ConCommandBase *Cmd_ExecuteCommand(const CCommand &command,
  ...
  // If we prevent a server command due to FCVAR_SERVER_CAN_EXECUTE not being
  // set, then we get out immediately.
  if (ShouldPreventServerCommand(pCommand)) return NULL;
  ...
  if (pCommand && pCommand->IsCommand()) {
    if (!ShouldPreventClientCommand(pCommand) && pCommand->IsCommand()) {
      ...
      // Hook to allow game .dll to figure out who type the message on a listen
      // server
      if (serverGameClients) {
        ...
        // We're not the server, but we've been a listen server (game .dll
        // loaded)
        //  forward this command tot he server instead of running it locally if
        ...
        // Otherwise, things like "say" won't work unless you quit and restart
          ...
          // It's a server command, but we're not connected to a server.  Don't
          // try to execute it.
          return NULL;
        }
      }
      ...
      if (pCommand->IsFlagSet(FCVAR_CHEAT)) {
        if (!Host_IsSinglePlayerGame() && !CanCheat()) {
          // But.. if the server is allowed to run this command and the server
          // DID run this command, then let it through. (used by
          // soundscape_flush)
          if (g_iFilterCommandsByServerCanExecute == 0 ||
              !pCommand->IsFlagSet(FCVAR_SERVER_CAN_EXECUTE)) {
            Msg("Can't use cheat command %s in multiplayer, unless the server "
  ...
  // check cvars
  if (cv->IsCommand(command)) return pCommand;
...
#ifndef _XBOX
  if (demoplayer->IsPlayingBack()) return;  // not really connected
#endif
