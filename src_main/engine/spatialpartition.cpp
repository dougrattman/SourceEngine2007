...
struct EntityInfo_t {
  ...
  int m_iLeafList[NUM_TREES];  // Index into the leaf pool - leaf list for
                               // entity (m_aLeafList).
};
...
static Color s_pVoxelColor[9] = {
    ...
    Color(255, 255, 255, 255), Color(192, 192, 0, 255),
...
class CVoxelHash {
 public:
  ...
  // Call this to clear out the spatial partition and to re-initialize it given
  // a particular world size (ISpatialPartitionInternal)
  void Init(CVoxelTree *pPartition, const Vector &worldmin,
            const Vector &worldmax, int nLevel);
  void Shutdown();

  // Gets all entities in a particular volume...
  // returns false if the enumerator broke early
  bool EnumerateElementsInBox(SpatialPartitionListMask_t listMask, Voxel_t vmin,
                              Voxel_t vmax, const Vector &mins,
                              const Vector &maxs,
                              IPartitionEnumerator *pIterator);
  bool EnumerateElementsAlongRay(SpatialPartitionListMask_t listMask,
                                 const Ray_t &ray, const Vector &vecInvDelta,
                                 const Vector &vecEnd,
                                 IPartitionEnumerator *pIterator);
  bool EnumerateElementsAtPoint(SpatialPartitionListMask_t listMask, Voxel_t v,
                                const Vector &pt,
                                IPartitionEnumerator *pIterator);

  // Inserts/Removes a handle from the tree.
  void InsertIntoTree(SpatialPartitionHandle_t hPartition, const Vector &vecMin,
                      const Vector &vecMax);
  void RemoveFromTree(SpatialPartitionHandle_t hPartition);

  // Debug!
  void RenderAllObjectsInTree(float flTime);
  void RenderObjectsInPlayerLeafs(const Vector &vecPlayerMin,
                                  const Vector &vecPlayerMax, float flTime);
  void RenderVoxel(Voxel_t voxel, float flTime);
  void RenderObjectInVoxel(SpatialPartitionHandle_t hPartition,
                           CPartitionVisitor *pVisitor, float flTime);
  void RenderObjectsInVoxel(Voxel_t voxel, CPartitionVisitor *pVisitor,
                            bool bRenderVoxel, float flTime);
  ...
  void LeafListRaySetup(const Ray_t &ray, const Vector &vecEnd,
                        const Vector &vecInvDelta, Voxel_t voxel, int *pStep,
                        float *pMax, float *pDelta);
  void LeafListExtrudedRaySetup(const Ray_t &ray, const Vector &vecInvDelta,
                                const Vector &vecMin, const Vector &vecMax,
                                int iVoxelMin[3], int iVoxelMax[3], int *pStep,
                                float *pMin, float *pMax, float *pDelta);

  // Main enumeration method
  template <class T>
  bool EnumerateElementsInVoxel(Voxel_t voxel, const T &intersectTest,
                                SpatialPartitionListMask_t listMask,
                                IPartitionEnumerator *pIterator);

  // Enumeration method when only 1 voxel is ever visited
  template <class T>
  bool EnumerateElementsInSingleVoxel(Voxel_t voxel, const T &intersectTest,
                                      SpatialPartitionListMask_t listMask,
                                      IPartitionEnumerator *pIterator);

  bool EnumerateElementsAlongRay_ExtrudedRaySlice(
      SpatialPartitionListMask_t listMask, IPartitionEnumerator *pIterator,
      const CIntersectSweptBox &intersectSweptBox, int voxelMin[3],
      int voxelMax[3], int iAxis, int *pStep);

 private:
  bool EnumerateElementsAlongRay_Ray(SpatialPartitionListMask_t listMask,
                                     const Ray_t &ray,
                                     const Vector &vecInvDelta,
                                     const Vector &vecEnd,
                                     IPartitionEnumerator *pIterator);
  bool EnumerateElementsAlongRay_ExtrudedRay(
      SpatialPartitionListMask_t listMask, const Ray_t &ray,
      const Vector &vecInvDelta, const Vector &vecEnd,
      IPartitionEnumerator *pIterator);

  inline void PackVoxel(int iX, int iY, int iZ, Voxel_t &voxel);

  typedef CUtlHashFixed<int, SPHASH_BUCKET_COUNT,
                        CUtlHashFixedGenericHash<SPHASH_BUCKET_COUNT> >
      CHashTable;

  Vector m_vecVoxelOrigin;  // Voxel space (hash) origin.
  CHashTable m_aVoxelHash;  // Voxel tree (hash) - data = entity list head
                            // handle (m_aEntityList)
  int m_nVoxelDelta[3];     // Voxel world - width(Dx), height(Dy), depth(Dz)
  CUtlFixedLinkedList<SpatialPartitionHandle_t>
      m_aEntityList;  // Pool - Linked list(multilist) of entities per leaf.
  CVoxelTree *m_pTree;
  int m_nLevel;
};
...
class CVoxelTree {
 public:
  ...
  virtual void Init(CSpatialPartition *pOwner, int iTree,
                    const Vector &worldmin, const Vector &worldmax);

  virtual void ElementMoved(SpatialPartitionHandle_t handle, const Vector &mins,
                            const Vector &maxs);
  virtual void EnumerateElementsInBox(SpatialPartitionListMask_t listMask,
                                      const Vector &mins, const Vector &maxs,
                                      bool coarseTest,
                                      IPartitionEnumerator *pIterator);
  virtual void EnumerateElementsInSphere(SpatialPartitionListMask_t listMask,
  ...
  void InsertIntoTree(SpatialPartitionHandle_t hPartition, const Vector &mins,
                      const Vector &maxs);
  ...
  // Purpose:
  void ComputeSweptRayBounds(const Ray_t &ray, const Vector &vecStartMin,
                             const Vector &vecStartMax, Vector *pVecMin,
                             Vector *pVecMax);
...
class CSpatialPartition : public ISpatialPartitionInternal {
 public:
  ...
  virtual void UnhideElement(SpatialPartitionHandle_t handle,
                             SpatialTempHandle_t tempHandle);
  ...
  virtual void InsertIntoTree(SpatialPartitionHandle_t hPartition,
                              const Vector &mins, const Vector &maxs);
 ...
 protected:
  ...
  typedef CUtlLinkedList<
      EntityInfo_t, SpatialPartitionHandle_t, false, SpatialPartitionHandle_t,
      CUtlMemoryStack<
          UtlLinkedListElem_t<EntityInfo_t, SpatialPartitionHandle_t>,
          SpatialPartitionHandle_t, 0xffff, 1024> >
      CHandleList;
 ...
 private:
  ...
  IPartitionQueryCallback
      *m_pQueryCallback[MAX_QUERY_CALLBACK];  // Query callbacks.
...
//-----------------------------------------------------------------------------
// Gets entity info (for enumerations).
inline EntityInfo_t &CSpatialPartition::EntityInfo(
    SpatialPartitionHandle_t hPartition) {
  return m_aHandles[hPartition];
}

inline EntityInfo_t &CVoxelTree::EntityInfo(
    SpatialPartitionHandle_t hPartition) {
  return m_pOwner->EntityInfo(hPartition);
}

inline CLeafList &CVoxelTree::LeafList() { return m_aLeafList; }

inline int CVoxelTree::GetTreeId() const { return m_TreeId; }

inline CPartitionVisits *CVoxelTree::GetVisits() { return m_pVisits; }
...
inline CVoxelTree *CSpatialPartition::VoxelTree(
    SpatialPartitionListMask_t listMask) {
  int iTree = ((listMask & PARTITION_ALL_CLIENT_EDICTS) == 0) ? SERVER_TREE
                                                              : CLIENT_TREE;
  return &m_VoxelTrees[iTree];
}

inline CVoxelTree *CSpatialPartition::VoxelTreeForHandle(
    SpatialPartitionHandle_t handle) {
...
CVoxelHash::~CVoxelHash() { Shutdown(); }
...
//  Output: voxel index
//-----------------------------------------------------------------------------
inline void CVoxelHash::VoxelIndexFromPoint(const Vector &vecWorldPoint,
                                            int pPoint[3]) {
  pPoint[0] = static_cast<int>(vecWorldPoint.x - m_vecVoxelOrigin.x) >>
              (SPHASH_VOXEL_SHIFT + SPHASH_LEVEL_SKIP * m_nLevel);
  pPoint[1] = static_cast<int>(vecWorldPoint.y - m_vecVoxelOrigin.y) >>
              (SPHASH_VOXEL_SHIFT + SPHASH_LEVEL_SKIP * m_nLevel);
  pPoint[2] = static_cast<int>(vecWorldPoint.z - m_vecVoxelOrigin.z) >>
              (SPHASH_VOXEL_SHIFT + SPHASH_LEVEL_SKIP * m_nLevel);
}
...
inline Voxel_t CVoxelHash::VoxelIndexFromPoint(const Vector &vecWorldPoint) {
  ...
  voxel.bitsVoxel.x = static_cast<int>(vecWorldPoint.x - m_vecVoxelOrigin.x) >>
                      (SPHASH_VOXEL_SHIFT + SPHASH_LEVEL_SKIP * m_nLevel);
  voxel.bitsVoxel.y = static_cast<int>(vecWorldPoint.y - m_vecVoxelOrigin.y) >>
                      (SPHASH_VOXEL_SHIFT + SPHASH_LEVEL_SKIP * m_nLevel);
  voxel.bitsVoxel.z = static_cast<int>(vecWorldPoint.z - m_vecVoxelOrigin.z) >>
                      (SPHASH_VOXEL_SHIFT + SPHASH_LEVEL_SKIP * m_nLevel);
...
//          worldmax -
//-----------------------------------------------------------------------------
void CVoxelHash::Init(CVoxelTree *pPartition, const Vector &worldmin,
                      const Vector &worldmax, int nLevel) {
  m_pTree = pPartition;
  m_nLevel = nLevel;
...
//-----------------------------------------------------------------------------
void CVoxelHash::InsertIntoTree(SpatialPartitionHandle_t hPartition,
                                const Vector &vecMin, const Vector &vecMax) {
...
inline void ClampStartPoint(Ray_t &ray, const Vector &vecEnd) {
  ...
  for (int iAxis = 0; iAxis < 3; ++iAxis) {
    if (fabs(ray.m_Delta[iAxis]) < 1e-10) continue;
...
class CIntersectPoint : public CPartitionVisitor {
 public:
  CIntersectPoint(CVoxelTree *pPartition, const Vector &pt)
      : CPartitionVisitor(pPartition), m_vecPoint(pt) {}
...
class CIntersectBox : public CPartitionVisitor {
 public:
  CIntersectBox(CVoxelTree *pPartition, const Vector &vecMins,
                const Vector &vecMaxs)
      : CPartitionVisitor(pPartition), m_vecMins(vecMins), m_vecMaxs(vecMaxs) {}
...
class CIntersectRay : public CPartitionVisitor {
 public:
  CIntersectRay(CVoxelTree *pPartition, const Ray_t &ray,
                const Vector &vecInvDelta)
      : CPartitionVisitor(pPartition), m_Ray(ray), m_vecInvDelta(vecInvDelta) {}
  ...
  bool Intersects(const Vector &vecMins, const Vector &vecMaxs) const {
    ...
    // Does the ray intersect the box?
    return IsBoxIntersectingRay(vecMins, vecMaxs, m_Ray.m_Start, m_Ray.m_Delta,
                                m_vecInvDelta);
  }
...
class CIntersectSweptBox : public CPartitionVisitor {
 public:
  CIntersectSweptBox(CVoxelTree *pPartition, const Ray_t &ray,
                     const Vector &vecInvDelta)
      : CPartitionVisitor(pPartition), m_Ray(ray), m_vecInvDelta(vecInvDelta) {}
  ...
  bool Intersects(const Vector &vecMins, const Vector &vecMaxs) const {
    ...
    // Does the ray intersect the box?
    return IsBoxIntersectingRay(vecTestMin, vecTestMax, m_Ray.m_Start,
                                m_Ray.m_Delta, m_vecInvDelta);
  }
...
//-----------------------------------------------------------------------------
template <class T>
bool CVoxelHash::EnumerateElementsInVoxel(Voxel_t voxel, const T &intersectTest,
                                          SpatialPartitionListMask_t listMask,
                                          IPartitionEnumerator *pIterator) {
  // If the voxel doesn't exist, nothing to iterate over
  UtlHashFastHandle_t hHash = m_aVoxelHash.Find(voxel.uiVoxel);
  if (hHash == m_aVoxelHash.InvalidHandle()) return true;

  SpatialPartitionHandle_t hPartition;
  for (int i = m_aVoxelHash.Element(hHash); i != m_aEntityList.InvalidIndex();
       i = m_aEntityList.Next(i)) {
    ...
    if (!(listMask & hInfo.m_fList)) continue;

    if (hInfo.m_flags & ENTITY_HIDDEN) continue;

    // Has this handle already been visited?
    if (!intersectTest.Visit(hPartition, hInfo)) continue;

    // Intersection test
    if (!intersectTest.Intersects(hInfo.m_vecMin, hInfo.m_vecMax)) continue;
...
//-----------------------------------------------------------------------------
template <class T>
bool CVoxelHash::EnumerateElementsInSingleVoxel(
    Voxel_t voxel, const T &intersectTest, SpatialPartitionListMask_t listMask,
    IPartitionEnumerator *pIterator) {
...
bool CVoxelHash::EnumerateElementsAlongRay(SpatialPartitionListMask_t listMask,
  ...
  if (ray.m_IsRay)
    return EnumerateElementsAlongRay_Ray(listMask, ray, vecInvDelta, vecEnd,
                                         pIterator);
  return EnumerateElementsAlongRay_ExtrudedRay(listMask, ray, vecInvDelta,
                                               vecEnd, pIterator);
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVoxelHash::EnumerateElementsAlongRay_Ray(
    SpatialPartitionListMask_t listMask, const Ray_t &ray,
    const Vector &vecInvDelta, const Vector &vecEnd,
    IPartitionEnumerator *pIterator) {
#ifdef _DEBUG
  ...
  if (bSingleVoxel)
    return EnumerateElementsInSingleVoxel(voxelStart, intersectRay, listMask,
                                          pIterator);
  ...
  while (1) {
    if (!EnumerateElementsInVoxel(voxelCurrent, intersectRay, listMask,
                                  pIterator))
      return false;

    if (tMax[0] >= 1.0f && tMax[1] >= 1.0f && tMax[2] >= 1.0f) break;
...
void CVoxelHash::LeafListRaySetup(const Ray_t &ray, const Vector &vecEnd,
                                  const Vector &vecInvDelta, Voxel_t voxel,
                                  int *pStep, float *pMax, float *pDelta) {
  int iVoxel[3];
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVoxelHash::EnumerateElementsAlongRay_ExtrudedRay(
    SpatialPartitionListMask_t listMask, const Ray_t &ray,
    const Vector &vecInvDelta, const Vector &vecEnd,
    IPartitionEnumerator *pIterator) {
  ...
  for (iX = voxelMin[0]; iX <= voxelMax[0]; ++iX) {
    ...
    for (iY = voxelMin[1]; iY <= voxelMax[1]; ++iY) {
      ...
      for (iZ = voxelMin[2]; iZ <= voxelMax[2]; ++iZ) {
        voxel.bitsVoxel.z = iZ;
        if (!EnumerateElementsInVoxel(voxel, intersectSweptBox, listMask,
                                      pIterator))
          return false;
      }
    }
  ...
  if ((endVoxelMin[0] >= voxelMin[0]) && (endVoxelMin[1] >= voxelMin[1]) &&
      (endVoxelMin[2] >= voxelMin[2]) && (endVoxelMax[0] <= voxelMax[0]) &&
      (endVoxelMax[1] <= voxelMax[1]) && (endVoxelMax[2] <= voxelMax[2]))
    return true;

  // Setup.
  int nStep[3];
  float tMax[3];  // amount of change in t along ray until we hit the next new
                  // voxel
  float
      tMin[3];  // amount of change in t along ray until we leave the last voxel
  float tDelta[3];
  LeafListExtrudedRaySetup(ray, vecInvDelta, vecMin, vecMax, voxelMin, voxelMax,
                           nStep, tMin, tMax, tDelta);
  ...
  while (tMax[0] < 1.0f || tMax[1] < 1.0f || tMax[2] < 1.0f) {
    ...
    if (tMin[iMinAxis] < tMax[iAxis]) {
      ...
      if (!EnumerateElementsAlongRay_ExtrudedRaySlice(
              listMask, pIterator, intersectSweptBox, voxelMin, voxelMax, iAxis,
              nStep))
        return false;
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CVoxelHash::LeafListExtrudedRaySetup(
    const Ray_t &ray, const Vector &vecInvDelta, const Vector &vecMin,
    const Vector &vecMax, int iVoxelMin[3], int iVoxelMax[3], int *pStep,
    float *pMin, float *pMax, float *pDelta) {
  ...
  for (int iAxis = 0; iAxis < 3; ++iAxis) {
    ...
    if (ray.m_Delta[iAxis] < 0.0f) {
      ...
      pStep[iAxis] = 1;
      flDistStart =
          -vecVoxelMax[iAxis] - ((iVoxelMax[iAxis] + 1) * -VoxelSize());
      flDistMinStart =
          -vecVoxelMin[iAxis] - ((iVoxelMin[iAxis] + 1) * -VoxelSize());
      flDist = ray.m_Delta[iAxis];
      flRecipDist = vecInvDelta[iAxis];
    }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVoxelHash::EnumerateElementsAlongRay_ExtrudedRaySlice(
    SpatialPartitionListMask_t listMask, IPartitionEnumerator *pIterator,
    const CIntersectSweptBox &intersectSweptBox, int voxelMin[3],
    int voxelMax[3], int iAxis, int *pStep) {
...
bool CVoxelHash::EnumerateElementsAtPoint(SpatialPartitionListMask_t listMask,
  ...
  if (hHash != m_aVoxelHash.InvalidHandle()) {
    ...
    while (iEntityList != m_aEntityList.InvalidIndex()) {
      ...
      // Keep going if there's no collision
      if (!IsPointInBox(pt, hInfo.m_vecMin, hInfo.m_vecMax)) continue;
...
void CVoxelHash::RenderVoxel(Voxel_t voxel, float flTime) {
  ...
  CDebugOverlay::AddBoxOverlay(
      vec3_origin, vecMin, vecMax, vec3_angle, s_pVoxelColor[m_nLevel][0],
      s_pVoxelColor[m_nLevel][1], s_pVoxelColor[m_nLevel][2], 30, flTime);
  ...
  CDebugOverlay::AddLineOverlay(vecPoints[0], vecPoints[1],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[1], vecPoints[2],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[2], vecPoints[3],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[3], vecPoints[0],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);

  CDebugOverlay::AddLineOverlay(vecPoints[4], vecPoints[5],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[5], vecPoints[6],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[6], vecPoints[7],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[7], vecPoints[4],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);

  CDebugOverlay::AddLineOverlay(vecPoints[0], vecPoints[4],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[3], vecPoints[7],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[1], vecPoints[5],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
  CDebugOverlay::AddLineOverlay(vecPoints[2], vecPoints[6],
                                s_pVoxelColor[m_nLevel][0],
                                s_pVoxelColor[m_nLevel][1],
                                s_pVoxelColor[m_nLevel][2], 255, true, flTime);
#endif
...
//-----------------------------------------------------------------------------
void CVoxelHash::RenderObjectInVoxel(SpatialPartitionHandle_t hPartition,
  ...
  CDebugOverlay::AddBoxOverlay(vec3_origin, info.m_vecMin, info.m_vecMax,
                               vec3_angle, s_pVoxelColor[m_nLevel][0],
                               s_pVoxelColor[m_nLevel][1],
                               s_pVoxelColor[m_nLevel][2], 75, flTime);
...
//-----------------------------------------------------------------------------
// Purpose: Debug! Render the objects in a voxel (optionally, render the
// voxel!).
//-----------------------------------------------------------------------------
void CVoxelHash::RenderObjectsInVoxel(Voxel_t voxel,
                                      CPartitionVisitor *pVisitor,
                                      bool bRenderVoxel, float flTime) {
  UtlHashFastHandle_t hHash = m_aVoxelHash.Find(voxel.uiVoxel);
  if (hHash == m_aVoxelHash.InvalidHandle()) return;
...
int CVoxelHash::EntityCount() {
  ...
  for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
    if (m_aVoxelHash.m_aBuckets[iBucket].Count() == 0) continue;
...
// Purpose:
//-----------------------------------------------------------------------------
void CVoxelHash::RenderObjectsInPlayerLeafs(const Vector &vecPlayerMin,
                                            const Vector &vecPlayerMax,
                                            float flTime) {
  // Visit each voxel in the box and enumerate its elements.
  Voxel_t voxelMin, voxelMax;
...
CVoxelTree::CVoxelTree()
    : m_pVoxelHash(NULL), m_pOwner(NULL), m_nNextVisitBit(0) {
...
//
//-----------------------------------------------------------------------------
CVoxelTree::~CVoxelTree() { delete[] m_pVoxelHash; }
...
//          worldmax -
//-----------------------------------------------------------------------------
void CVoxelTree::Init(CSpatialPartition *pOwner, int iTree,
                      const Vector &worldmin, const Vector &worldmax) {
  m_pOwner = pOwner;
  m_TreeId = iTree;
...
// Insert into the appropriate tree
//-----------------------------------------------------------------------------
void CVoxelTree::InsertIntoTree(SpatialPartitionHandle_t hPartition,
                                const Vector &mins, const Vector &maxs) {
  bool bWasReading = (m_pVisits != NULL);
  if (bWasReading) {
    // If we're recursing in this thread, need to release our read lock to allow
    // ourselves to write
    UnlockRead();
  }
  ...
  for (nLevel = 0; nLevel < m_nLevelCount - 1; ++nLevel) {
    ...
    if ((nVoxelSize > vecSize.x) && (nVoxelSize > vecSize.y) &&
        (nVoxelSize > vecSize.z))
      break;
  }
...
void CVoxelTree::RemoveFromTree(SpatialPartitionHandle_t hPartition) {
  ...
  if (nLevel >= 0) {
    bool bWasReading = (m_pVisits != NULL);
    if (bWasReading) {
      // If we're recursing in this thread, need to release our read lock to
...
//-----------------------------------------------------------------------------
void CVoxelTree::ElementMoved(SpatialPartitionHandle_t hPartition,
                              const Vector &mins, const Vector &maxs) {
  if (hPartition != PARTITION_INVALID_HANDLE) {
    ...
    Vector vecEpsMin(mins.x - SPHASH_EPS, mins.y - SPHASH_EPS,
                     mins.z - SPHASH_EPS);
    Vector vecEpsMax(maxs.x + SPHASH_EPS, maxs.y + SPHASH_EPS,
                     maxs.z + SPHASH_EPS);
...
void CVoxelTree::EnumerateElementsInBox(SpatialPartitionListMask_t listMask,
  ...
  // If this assertion fails, you're using a list at a point where the spatial
  // partition elements aren't set up!
  ...
  if (!m_pVoxelHash[0].EnumerateElementsInBox(listMask, vs, ve, mins, maxs,
                                              pIterator)) {
    m_lock.UnlockRead();
  ...
  if (!m_pVoxelHash[1].EnumerateElementsInBox(listMask, vs, ve, mins, maxs,
                                              pIterator)) {
    m_lock.UnlockRead();
    EndVisit(pPrevVisits);
    return;
  ...
  vs = ConvertToNextLevel(vs);
  ve = ConvertToNextLevel(ve);
  if (!m_pVoxelHash[2].EnumerateElementsInBox(listMask, vs, ve, mins, maxs,
                                              pIterator)) {
    m_lock.UnlockRead();
    EndVisit(pPrevVisits);
    return;
  ...
  vs = ConvertToNextLevel(vs);
  ve = ConvertToNextLevel(ve);
  m_pVoxelHash[3].EnumerateElementsInBox(listMask, vs, ve, mins, maxs,
                                         pIterator);
...
void CVoxelTree::EnumerateElementsInSphere(SpatialPartitionListMask_t listMask,
  ...
  return EnumerateElementsInBox(listMask, vecMin, vecMax, coarseTest,
                                pIterator);
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVoxelTree::EnumerateElementsAlongRay_Ray(
    SpatialPartitionListMask_t listMask, const Ray_t &ray,
    const Vector &vecInvDelta, const Vector &vecEnd,
    IPartitionEnumerator *pIterator) {
  // Find the voxel start + end
  ...
  // Optimization: Look for single voxel rays
  if (bSingleVoxel) {
    if (!m_pVoxelHash[0].EnumerateElementsInSingleVoxel(
            voxelStart, intersectRay, listMask, pIterator))
      return false;
    voxelStart = ConvertToNextLevel(voxelStart);
    if (!m_pVoxelHash[1].EnumerateElementsInSingleVoxel(
            voxelStart, intersectRay, listMask, pIterator))
      return false;
    voxelStart = ConvertToNextLevel(voxelStart);
    if (!m_pVoxelHash[2].EnumerateElementsInSingleVoxel(
            voxelStart, intersectRay, listMask, pIterator))
      return false;
    voxelStart = ConvertToNextLevel(voxelStart);
    return m_pVoxelHash[3].EnumerateElementsInSingleVoxel(
        voxelStart, intersectRay, listMask, pIterator);
  }
  ...
  float tMax[3];
  float tDelta[3];
  m_pVoxelHash[0].LeafListRaySetup(ray, vecEnd, vecInvDelta, voxelStart, nStep,
                                   tMax, tDelta);
  ...
  while (1) {
    if (!m_pVoxelHash[0].EnumerateElementsInVoxel(voxelCurrent, intersectRay,
                                                  listMask, pIterator))
      return false;

    if (v1.uiVoxel != ov1.uiVoxel) {
      if (!m_pVoxelHash[1].EnumerateElementsInVoxel(v1, intersectRay, listMask,
                                                    pIterator))
        return false;
    }
    if (v2.uiVoxel != ov2.uiVoxel) {
      if (!m_pVoxelHash[2].EnumerateElementsInVoxel(v2, intersectRay, listMask,
                                                    pIterator))
        return false;
    }
    if (v3.uiVoxel != ov3.uiVoxel) {
      if (!m_pVoxelHash[3].EnumerateElementsInVoxel(v3, intersectRay, listMask,
                                                    pIterator))
        return false;
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void CVoxelTree::ComputeSweptRayBounds(const Ray_t &ray,
                                       const Vector &vecStartMin,
                                       const Vector &vecStartMax,
                                       Vector *pVecMin, Vector *pVecMax) {
  if (ray.m_Delta.x < 0) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CVoxelTree::EnumerateElementsAlongRay_ExtrudedRay(
    SpatialPartitionListMask_t listMask, const Ray_t &ray,
    const Vector &vecInvDelta, const Vector &vecEnd,
    IPartitionEnumerator *pIterator) {
  // Check the starting position, then proceed with the sweep.
    ...
    for (iX = voxelBounds[i][0][0]; iX <= voxelBounds[i][1][0]; ++iX) {
      ...
      for (iY = voxelBounds[i][0][1]; iY <= voxelBounds[i][1][1]; ++iY) {
        ...
        for (iZ = voxelBounds[i][0][2]; iZ <= voxelBounds[i][1][2]; ++iZ) {
          voxel.bitsVoxel.z = iZ;
          if (!m_pVoxelHash[i].EnumerateElementsInVoxel(
                  voxel, intersectSweptBox, listMask, pIterator))
            return false;
        }
      }
    }
  ...
  if ((endVoxelMin[0] >= voxelBounds[0][0][0]) &&
      (endVoxelMin[1] >= voxelBounds[0][0][1]) &&
      (endVoxelMin[2] >= voxelBounds[0][0][2]) &&
      (endVoxelMax[0] <= voxelBounds[0][1][0]) &&
      (endVoxelMax[1] <= voxelBounds[0][1][1]) &&
      (endVoxelMax[2] <= voxelBounds[0][1][2]))
    return true;
  ...
  float
      tMin[3];  // amount of change in t along ray until we leave the last voxel
  float tDelta[3];
  m_pVoxelHash[0].LeafListExtrudedRaySetup(ray, vecInvDelta, vecMin, vecMax,
                                           voxelBounds[0][0], voxelBounds[0][1],
                                           nStep, tMin, tMax, tDelta);
  ...
  while (tMax[0] < 1.0f || tMax[1] < 1.0f || tMax[2] < 1.0f) {
    ...
    if (tMin[iMinAxis] < tMax[iAxis]) {
      ...
      voxelBounds[1][nIndex][iMinAxis] =
          voxelBounds[0][nIndex][iMinAxis] >> SPHASH_LEVEL_SKIP;
      voxelBounds[2][nIndex][iMinAxis] =
          voxelBounds[0][nIndex][iMinAxis] >> (2 * SPHASH_LEVEL_SKIP);
      voxelBounds[3][nIndex][iMinAxis] =
          voxelBounds[0][nIndex][iMinAxis] >> (3 * SPHASH_LEVEL_SKIP);
    } else {
      tMax[iAxis] += tDelta[iAxis];
      int nIndex = (nStep[iAxis] > 0) ? 1 : 0;
      voxelBounds[0][nIndex][iAxis] += nStep[iAxis];
      voxelBounds[1][nIndex][iAxis] =
          voxelBounds[0][nIndex][iAxis] >> SPHASH_LEVEL_SKIP;
      voxelBounds[2][nIndex][iAxis] =
          voxelBounds[0][nIndex][iAxis] >> (2 * SPHASH_LEVEL_SKIP);
      voxelBounds[3][nIndex][iAxis] =
          voxelBounds[0][nIndex][iAxis] >> (3 * SPHASH_LEVEL_SKIP);

      if (!m_pVoxelHash[0].EnumerateElementsAlongRay_ExtrudedRaySlice(
              listMask, pIterator, intersectSweptBox, voxelBounds[0][0],
              voxelBounds[0][1], iAxis, nStep))
        return false;

      if (nLastVoxel1[iAxis] != voxelBounds[1][nIndex][iAxis]) {
        nLastVoxel1[iAxis] = voxelBounds[1][nIndex][iAxis];
        if (!m_pVoxelHash[1].EnumerateElementsAlongRay_ExtrudedRaySlice(
                listMask, pIterator, intersectSweptBox, voxelBounds[1][0],
                voxelBounds[1][1], iAxis, nStep))
          return false;
      }

      if (nLastVoxel2[iAxis] != voxelBounds[2][nIndex][iAxis]) {
        nLastVoxel2[iAxis] = voxelBounds[2][nIndex][iAxis];
        if (!m_pVoxelHash[2].EnumerateElementsAlongRay_ExtrudedRaySlice(
                listMask, pIterator, intersectSweptBox, voxelBounds[2][0],
                voxelBounds[2][1], iAxis, nStep))
          return false;
      }

      if (nLastVoxel3[iAxis] != voxelBounds[3][nIndex][iAxis]) {
        nLastVoxel3[iAxis] = voxelBounds[3][nIndex][iAxis];
        if (!m_pVoxelHash[3].EnumerateElementsAlongRay_ExtrudedRaySlice(
                listMask, pIterator, intersectSweptBox, voxelBounds[3][0],
                voxelBounds[3][1], iAxis, nStep))
          return false;
      }
    }
  }
...
void CVoxelTree::EnumerateElementsAlongRay(SpatialPartitionListMask_t listMask,
  ...
  vecInvDelta[0] =
      (clippedRay.m_Delta[0] != 0.0f) ? 1.0f / clippedRay.m_Delta[0] : FLT_MAX;
  vecInvDelta[1] =
      (clippedRay.m_Delta[1] != 0.0f) ? 1.0f / clippedRay.m_Delta[1] : FLT_MAX;
  vecInvDelta[2] =
      (clippedRay.m_Delta[2] != 0.0f) ? 1.0f / clippedRay.m_Delta[2] : FLT_MAX;

  CPartitionVisits *pPrevVisits = BeginVisit();

  m_lock.LockForRead();
  if (ray.m_IsRay) {
    EnumerateElementsAlongRay_Ray(listMask, clippedRay, vecInvDelta, vecEnd,
                                  pIterator);
  } else {
    EnumerateElementsAlongRay_ExtrudedRay(listMask, clippedRay, vecInvDelta,
                                          vecEnd, pIterator);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CVoxelTree::RenderObjectsInPlayerLeafs(const Vector &vecPlayerMin,
                                            const Vector &vecPlayerMax,
                                            float flTime) {
  MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
  m_lock.LockForRead();
  for (int i = 0; i < m_nLevelCount; ++i) {
    m_pVoxelHash[i].RenderObjectsInPlayerLeafs(vecPlayerMin, vecPlayerMax,
                                               flTime);
  }
  m_lock.UnlockRead();
}
...
//-----------------------------------------------------------------------------
ISpatialPartitionInternal *SpatialPartition() { return &g_SpatialPartition; }

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
CSpatialPartition::CSpatialPartition() { m_nQueryCallbackCount = 0; }

CSpatialPartition::~CSpatialPartition() { Shutdown(); }
...
//-----------------------------------------------------------------------------
void CSpatialPartition::InstallQueryCallback(
    IPartitionQueryCallback *pCallback) {
  // Verify data.
  Assert(pCallback && m_nQueryCallbackCount < MAX_QUERY_CALLBACK);
  if (!pCallback || (m_nQueryCallbackCount >= MAX_QUERY_CALLBACK)) return;
...
//   Input: pCallback - pointer to the callback function to add
//-----------------------------------------------------------------------------
void CSpatialPartition::InstallQueryCallback_V1(
    IPartitionQueryCallback *pCallback) {
  // Verify data.
  Assert(pCallback && m_nQueryCallbackCount < MAX_QUERY_CALLBACK);
  if (!pCallback || (m_nQueryCallbackCount >= MAX_QUERY_CALLBACK)) return;

  // NOTE: the query callbacks are not mutexed. Only add and remove when threads
  // are joined
...
//-----------------------------------------------------------------------------
void CSpatialPartition::RemoveQueryCallback(
    IPartitionQueryCallback *pCallback) {
  // Verify data.
  if (!pCallback) return;
...
//-----------------------------------------------------------------------------
void CSpatialPartition::InvokeQueryCallbacks(
    SpatialPartitionListMask_t listMask, bool bDone) {
...
//   Input: pHandleEntity - entity handle of the object to create a spatial
//   partition handle for
//-----------------------------------------------------------------------------
SpatialPartitionHandle_t CSpatialPartition::CreateHandle(
  ...
  for (int i = 0; i < NUM_TREES; i++) {
    ...
    m_aHandles[hPartition].m_nLevel[i] = (uint8_t)-1;
...
SpatialPartitionHandle_t CSpatialPartition::CreateHandle(
    IHandleEntity *pHandleEntity, SpatialPartitionListMask_t listMask,
    const Vector &mins, const Vector &maxs) {
  //	CDebugOverlay::AddBoxOverlay( vec3_origin, mins, maxs, vec3_angle, 0,
  // 255, 0, 75, 3600 );
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::RemoveAndInsert(SpatialPartitionListMask_t removeMask,
                                        SpatialPartitionListMask_t insertMask,
                                        SpatialPartitionHandle_t handle) {
  Assert(m_aHandles.IsValidIndex(handle));
...
//-----------------------------------------------------------------------------
// Purpose: Fast way to re-add (set) a group that was removed (and saved) via
// the
//          HideElement call.
//-----------------------------------------------------------------------------
void CSpatialPartition::UnhideElement(SpatialPartitionHandle_t handle,
                                      SpatialTempHandle_t tempHandle) {
  Assert(m_aHandles.IsValidIndex(handle));
...
//-----------------------------------------------------------------------------
SpatialTempHandle_t CSpatialPartition::HideElement(
    SpatialPartitionHandle_t handle) {
  Assert(m_aHandles.IsValidIndex(handle));
  m_HandlesMutex.Lock();
  m_aHandles[handle].m_flags |= ENTITY_HIDDEN;
...
//-----------------------------------------------------------------------------
void CSpatialPartition::SuppressLists(SpatialPartitionListMask_t nListMask,
                                      bool bSuppress) {
  if (bSuppress) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::EnumerateElementsInBox(
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::EnumerateElementsInSphere(
    SpatialPartitionListMask_t listMask, const Vector &origin, float radius,
    bool coarseTest, IPartitionEnumerator *pIterator) {
  MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
  CVoxelTree *pTree = VoxelTree(listMask);
  InvokeQueryCallbacks(listMask);
  pTree->EnumerateElementsInSphere(listMask, origin, radius, coarseTest,
                                   pIterator);
  InvokeQueryCallbacks(listMask, true);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::EnumerateElementsAlongRay(
    SpatialPartitionListMask_t listMask, const Ray_t &ray, bool coarseTest,
    IPartitionEnumerator *pIterator) {
  MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
  CVoxelTree *pTree = VoxelTree(listMask);
  InvokeQueryCallbacks(listMask);
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::EnumerateElementsAtPoint(
    SpatialPartitionListMask_t listMask, const Vector &pt, bool coarseTest,
    IPartitionEnumerator *pIterator) {
  MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
  CVoxelTree *pTree = VoxelTree(listMask);
  InvokeQueryCallbacks(listMask);
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::InsertIntoTree(SpatialPartitionHandle_t hPartition,
                                       const Vector &mins, const Vector &maxs) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::RenderObjectsInBox(const Vector &vecMin,
                                           const Vector &vecMax, float flTime) {
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CSpatialPartition::RenderObjectsInSphere(const Vector &vecCenter,
                                              float flRadius, float flTime) {}
...
void CSpatialPartition::RenderObjectsInPlayerLeafs(const Vector &vecPlayerMin,
                                                   const Vector &vecPlayerMax,
                                                   float flTime) {
  for (int i = 0; i < NUM_TREES; i++) {
    m_VoxelTrees[i].RenderObjectsInPlayerLeafs(vecPlayerMin, vecPlayerMax,
                                               flTime);
  }
}
...
void CSpatialPartition::ReportStats(const char *pFileName) {
  Msg("Handle Count %d (%d bytes, %d total)\n", m_aHandles.Count(),
      m_aHandles.Count() *
          (sizeof(EntityInfo_t) + 2 * sizeof(SpatialPartitionHandle_t)));
  for (int i = 0; i < NUM_TREES; i++) {
    m_VoxelTrees[i].ReportStats(pFileName);
  }
}

static ConVar r_partition_level("r_partition_level", "-1", FCVAR_CHEAT,
                                "Displays a particular level of the spatial "
                                "partition system. Use -1 to disable it.");
...
//=============================================================================
ISpatialPartition *CreateSpatialPartition(const Vector &worldmin,
