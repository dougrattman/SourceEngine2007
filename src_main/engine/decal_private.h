...
struct decal_t {
  ...
  float dx;  // Offsets into surface texture (in texture coordinates, so we
             // don't need floats)
  float dy;
  float scale;        // Pixel scale
  short flags;        // Decal flags  DECAL_*		!!!SAVED AS A BYTE (SEE
                      // HOST_CMD.C)
...
#define FDECAL_PERMANENT \
  0x01  // This decal should not be removed in favor of any new decals
#define FDECAL_REFERENCE \
  0x02  // This is a decal that's been moved from another level
#define FDECAL_CUSTOM \
  0x04  // This is a custom clan logo and should not be saved/restored
...
#define FDECAL_SECONDPASS \
  0x200  // Decals that have to be drawn after everything else
#define FDECAL_DONTSAVE \
  0x800  // Decal was loaded from adjacent level, don't save out to save file
         // for this level
#define FDECAL_PLAYERSPRAY 0x1000  // Decal is a player spray
#define FDECAL_DISTANCESCALE \
  0x2000  // Decal is dynamically scaled based on distance.
