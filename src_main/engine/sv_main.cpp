...
static void SV_CheatsChanged_f(IConVar *pConVar, const char *pOldString,
                               float flOldValue) {
...
static void SV_Pure_f(const CCommand &args) {
  ...
  if (pure_mode == 0 || pure_mode == 1 || pure_mode == 2) {
    ...
    if (pure_mode == g_sv_pure_mode) {
      ...
      if (sv.IsActive()) {
        Msg("Note: Changes to sv_pure take effect when the next map is "
            "loaded.\n");
      }
    }
  } else {
  ...
  if (pure_mode == -1) {
    // If we're a client on a server with sv_pure = 1, display the current
    // whitelist.
#ifndef SWDS
    if (cl.IsConnected()) {
      Msg("\n\n");
...
ConVar sv_pure_kick_clients("sv_pure_kick_clients", "1", 0,
                            "If set to 1, the server will kick clients with "
                            "mismatching files. Otherwise, it will issue a "
                            "warning to the client.");
ConVar sv_pure_trace("sv_pure_trace", "0", 0,
                     "If set to 1, the server will print a message whenever a "
                     "client is verifying a CRC for a file.");

ConVar sv_cheats("sv_cheats", "0", FCVAR_NOTIFY | FCVAR_REPLICATED,
                 "Allow cheats on server", SV_CheatsChanged_f);
ConVar sv_lan("sv_lan", "0", 0,
...
static ConVar sv_pausable("sv_pausable", "0", FCVAR_NOTIFY,
                          "Is the server pausable.");
static ConVar sv_contact("sv_contact", "", FCVAR_NOTIFY,
...
static ConVar sv_voiceenable(
    "sv_voiceenable", "1",
    FCVAR_ARCHIVE | FCVAR_NOTIFY);  // set to 0 to disable all voice forwarding.
ConVar sv_downloadurl("sv_downloadurl", "", FCVAR_REPLICATED,
                      "Location from which clients can download missing files");
ConVar sv_consistency(
    "sv_consistency", "1", FCVAR_REPLICATED,
    "Whether the server enforces file consistency for critical files");
ConVar sv_maxreplay("sv_maxreplay", "0", 0, "Maximum replay time in seconds",
                    true, 0, true, 15);

ConVar sv_mincmdrate(
    "sv_mincmdrate", "0", FCVAR_REPLICATED,
    "This sets the minimum value for cl_cmdrate. 0 == unlimited.");
ConVar sv_maxcmdrate(
    "sv_maxcmdrate", "40", FCVAR_REPLICATED,
    "(If sv_mincmdrate is > 0), this sets the maximum value for cl_cmdrate.");
ConVar sv_client_cmdrate_difference(
    "sv_client_cmdrate_difference", "20", FCVAR_REPLICATED,
    "cl_cmdrate is moved to within sv_client_cmdrate_difference units of "
    "cl_updaterate before it "
    "is clamped between sv_mincmdrate and sv_maxcmdrate.");

ConVar sv_client_min_interp_ratio(
    "sv_client_min_interp_ratio", "1", FCVAR_REPLICATED,
    "This can be used to limit the value of cl_interp_ratio for connected "
    "clients "
...
ConVar sv_client_max_interp_ratio(
    "sv_client_max_interp_ratio", "5", FCVAR_REPLICATED,
    "This can be used to limit the value of cl_interp_ratio for connected "
    "clients "
    "(only while they are connected). If sv_client_min_interp_ratio is -1, "
    "then this cvar has no effect.");
ConVar sv_client_predict(
    "sv_client_predict", "-1", FCVAR_REPLICATED,
...
void SV_ForceSend() { s_bForceSend = true; }
...
#if defined(_DEBUG)
ConVar sv_deltatrace(
    "sv_deltatrace", "0", 0,
    "For debugging, print entity creation/deletion info to console.");
#define TRACE_DELTA(text)       \
  if (sv_deltatrace.GetInt()) { \
    ConMsg(text);               \
  };
#else
...
void CGameServer::Clear(void) {
  ...
  if (serverGameDLL) {
    for (ServerClass *pCur = serverGameDLL->GetAllServerClasses(); pCur;
         pCur = pCur->m_pNext) {
...
void CGameServer::CreateEngineStringTables(void) {
  ...
  m_pDownloadableFileTable = m_StringTables->CreateStringTableEx(
      szDownloadableFileTablename, MAX_DOWNLOADABLE_FILES, 0, 0,
      bUseFilenameTables);

  m_pModelPrecacheTable = m_StringTables->CreateStringTableEx(
      szModelPrecacheTablename, MAX_MODELS, sizeof(CPrecacheUserData),
      PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

  m_pGenericPrecacheTable = m_StringTables->CreateStringTableEx(
      szGenericPrecacheTablename, MAX_GENERIC, sizeof(CPrecacheUserData),
      PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

  m_pSoundPrecacheTable = m_StringTables->CreateStringTableEx(
      szSoundPrecacheTablename, MAX_SOUNDS, sizeof(CPrecacheUserData),
      PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

  m_pDecalPrecacheTable = m_StringTables->CreateStringTableEx(
      szDecalPrecacheTablename, MAX_BASE_DECALS, sizeof(CPrecacheUserData),
      PRECACHE_USER_DATA_NUMBITS, bUseFilenameTables);

  m_pInstanceBaselineTable = m_StringTables->CreateStringTable(
      INSTANCE_BASELINE_TABLENAME, MAX_DATATABLES);

  m_pLightStyleTable = m_StringTables->CreateStringTable(LIGHT_STYLES_TABLENAME,
                                                         MAX_LIGHTSTYLES);
  ...
  m_pServerStartupTable =
      m_StringTables->CreateStringTable(SERVER_STARTUP_DATA_TABLENAME, 4);
  SetQueryPortFromSteamServer();
  CopyPureServerWhitelistToStringTable();

  Assert(m_pModelPrecacheTable && m_pGenericPrecacheTable &&
         m_pSoundPrecacheTable && m_pDecalPrecacheTable &&
         m_pInstanceBaselineTable && m_pLightStyleTable && m_pUserInfoTable &&
         m_pServerStartupTable && m_pDownloadableFileTable);
...
void CGameServer::SetQueryPortFromSteamServer() {
  if (!m_pServerStartupTable) return;

  int queryPort = Steam3Server().GetQueryPort();
  m_pServerStartupTable->AddString(true, "QueryPort", sizeof(queryPort),
                                   &queryPort);
}

void CGameServer::CopyPureServerWhitelistToStringTable() {
  if (!m_pPureServerWhitelist) return;

  CUtlBuffer buf;
  m_pPureServerWhitelist->Encode(buf);
  m_pServerStartupTable->AddString(true, "PureServerWhitelist", buf.TellPut(),
                                   buf.Base());
}
...
void SV_InstallClientStringTableMirrors(void) {
  ...
  for (int i = 0; i < numTables; i++) {
    ...
    CNetworkStringTable *clientTable =
        (CNetworkStringTable *)networkStringTableContainerClient->FindTable(
            serverTable->GetTableName());

    if (!clientTable) {
      DevMsg(
          "SV_InstallClientStringTableMirrors! Missing client table \"%s\".\n ",
          serverTable->GetTableName());
      continue;
    }
...
CON_COMMAND(user, "Show user data.") {
  ...
  for (i = 0; i < sv.GetClientCount(); i++) {
    ...
    if ((cl->GetPlayerSlot() == uid) ||
        !Q_strcmp(cl->GetClientName(), args[1])) {
...
void CGameServer::InitMaxClients(void) {
  ...
  if (serverGameClients) {
    ...
    if (minmaxplayers < 1) {
      Sys_Error("GetPlayerLimits:  min maxplayers must be >= 1 (%i)",
                minmaxplayers);
    } else if (defaultmaxplayers < 1) {
      Sys_Error("GetPlayerLimits:  default maxplayers must be >= 1 (%i)",
                minmaxplayers);
    }

    if (minmaxplayers > maxmaxplayers || defaultmaxplayers > maxmaxplayers) {
      Sys_Error("GetPlayerLimits:  min maxplayers %i > max %i", minmaxplayers,
                maxmaxplayers);
    }

    if (maxmaxplayers > ABSOLUTE_PLAYER_LIMIT) {
      Sys_Error("GetPlayerLimits:  max players limited to %i",
...
int SV_BuildSendTablesArray(ServerClass *pClasses, SendTable **pTables,
                            int nMaxTables) {
  int nTables = 0;

  for (ServerClass *pCur = pClasses; pCur; pCur = pCur->m_pNext) {
    ErrorIfNot(nTables < nMaxTables,
...
// Builds an alternate copy of the datatable for any classes that have
...
void SV_TermSendTables(ServerClass *pClasses) { SendTable_Term(); }
...
void ServerDLL_Unload() { UnloadEntityDLLs(); }
...
void SV_InitGameDLL(void) {
  ...
  if (CL_IsHL2Demo() && !sv.IsDedicated() &&
      Q_stricmp(COM_GetModDirectory(), "hl2")) {
    Error("The HL2 demo is unable to run Mods.\n");
    return;
  }

  if (CL_IsPortalDemo() && !sv.IsDedicated() &&
      Q_stricmp(COM_GetModDirectory(), "portal")) {
    Error("The Portal demo is unable to run Mods.\n");
    return;
  }
#ifndef NO_STEAM
  // check permissions
  if (SteamApps() && g_pFileSystem->IsSteam() && !CL_IsHL2Demo() &&
      !CL_IsPortalDemo()) {
    ...
    for (int i = 0; i < SOURCE_ARRAYSIZE(g_ModDirPermissions); i++) {
      if (!Q_stricmp(COM_GetModDirectory(),
        ...
        char szSub[10];
        if (SteamApps()->GetAppData(g_ModDirPermissions[i].m_iAppID,
                                    "subscribed", szSub, sizeof(szSub)) > 0 &&
            Q_atoi(szSub) > 0) {
          bVerifiedMod = true;
        } else {
    ...
    if (!bVerifiedMod) {
      ...
      char szSub[10];
      if (SteamApps()->GetAppData(215, "subscribed", szSub, sizeof(szSub)) ==
              0 ||
          Q_atoi(szSub) == 0) {
  ...
  if (!serverGameDLL->DLLInit(g_AppSystemFactory, g_AppSystemFactory,
                              g_AppSystemFactory, &g_ServerGlobalVariables)) {
  ...
  if (host_state.interval_per_tick < MINIMUM_TICK_INTERVAL ||
    ...
    Sys_Error(
        "GetTickInterval returned bogus tick interval (%f)[%f to %f is valid "
        "range]",
        host_state.interval_per_tick, MINIMUM_TICK_INTERVAL,
...
void SV_ShutdownGameDLL(void) {
  ...
  // Delete any extra SendTable copies we've attached to the game DLL's classes,
  // if any.
...
ServerClass *SV_FindServerClass(const char *pName) {
  ...
  while (pCur) {
    if (Q_stricmp(pCur->GetName(), pName) == 0) return pCur;
...
==================
*/
void SV_StartSound(IRecipientFilter &filter, edict_t *pSoundEmittingEntity,
                   int iChannel, const char *pSample, float flVolume,
                   soundlevel_t iSoundLevel, int iFlags, int iPitch,
                   const Vector *pOrigin, float soundtime, int speakerentity,
                   CUtlVector<Vector> *pUtlVecOrigins) {
  SoundInfo_t sound;
  sound.SetDefault();

  sound.nEntityIndex =
      pSoundEmittingEntity ? NUM_FOR_EDICT(pSoundEmittingEntity) : 0;
  ...
  if (pSample && TestSoundChar(pSample, CHAR_SENTENCE)) {
    ...
    if (sound.nSoundNum >= VOX_SentenceCount()) {
      ConMsg("SV_StartSound: invalid sentence number: %s",
...
void SV_DetermineMulticastRecipients(
  ...
  const uint8_t *pMask = CM_Vis(pvs, sizeof(pvs), cluster, visType);
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    ...
    if (!pClient->IsActive()) continue;

    // HACK:  Should above also check pClient->spawned instead of this
    if (!pClient->edict || pClient->edict->IsFree() ||
        pClient->edict->GetUnknown() == NULL)
      continue;
    ...
    if (!(pMask[iBitNumber >> 3] & (1 << (iBitNumber & 7)))) continue;
...
void SV_ReplicateConVarChange(ConVar const *var, const char *newValue) {
  ...
  if (!sv.IsActive() || !sv.IsMultiplayer()) return;

  NET_SetConVar cvarMsg(var->GetName(),
                        Host_CleanupConVarStringValue(newValue));
...
CGameServer::~CGameServer() {
  if (m_pPureServerWhitelist) m_pPureServerWhitelist->Release();
}

//-----------------------------------------------------------------------------
// Purpose: Disconnects the client and cleans out the m_pEnt CBasePlayer
// container object Input  : *clientedict -
//-----------------------------------------------------------------------------
void CGameServer::RemoveClientFromGame(CBaseClient *client) {
  ...
  if (!cl->edict || !cl->IsSpawned() || !IsActive()) return;
...
CBaseClient *CGameServer::CreateNewClient(int slot) {
  ...
  if (cl && CommandLine()->CheckParm("-netspike", &pszValue) && pszValue) {
...
For LAN connections, make sure we don't have too many people with same cd key
hash For Authenticated net connections, check the certificate and also double
check won userid from that certificate
================
*/
bool CGameServer::FinishCertificateCheck(netadr_t &adr, int nAuthProtocol,
  ...
  switch (nAuthProtocol) {
    ...
    case PROTOCOL_HASHEDCDKEY:
      ...
      if ((g_pFileSystem->IsSteam() && !Host_IsSinglePlayerGame()) ||
          sv.IsDedicated())  // PROTOCOL_HASHEDCDKEY isn't allowed for
                             // multiplayer servers
      {
        RejectConnection(
            adr, "CD Key authentication invalid for internet servers.\n");
        return false;
      }
      ...
      for (int i = 0; i < GetClientCount(); i++) {
        ...
        if (Q_strnicmp(szRawCertificate, cl->m_GUID, SIGNED_GUID_LEN)) continue;
...
//-----------------------------------------------------------------------------
// Purpose: Zeroes out pvs, this way we can or together multiple pvs's for a
// player
//-----------------------------------------------------------------------------
...
void CGameServer::BroadcastSound(SoundInfo_t &sound, IRecipientFilter &filter) {
  ...
  if (IsPaused() && !developer.GetInt()) return;

  for (int i = 0; i < num; i++) {
    int index = filter.GetRecipientIndex(i);

    if (index < 1 || index > GetClientCount()) {
      Msg("CGameServer::BroadcastSound:  Recipient Filter for sound (reliable: "
          "%s, init: %s) with bogus client index (%i) in list of %i clients\n",
...
void CGameServer::FinishRestore() {
  ...
  if (g_ServerGlobalVariables.eLoadType == MapLoad_Transition) {
    for (int i = 0; i < currentLevelData.levelInfo.connectionCount; i++) {
      saverestore->RestoreAdjacenClientState(
...
void CGameServer::CopyTempEntities(CFrameSnapshot *pSnapshot) {
  ...
  if (m_TempEntities.Count() > 0) {
    ...
    Q_memcpy(pSnapshot->m_pTempEntities, m_TempEntities.Base(),
             m_TempEntities.Count() * sizeof(CEventInfo *));
...
void CGameServer::SendClientMessages(bool bSendSnapshots) {
  ...
  for (int i = 0; i < GetClientCount(); i++) {
    ...
    if (bSendSnapshots && client->IsActive()) {
      ...
      if (NET_IsMultiplayer() &&
          client->m_NetChannel->GetSequenceNr(FLOW_INCOMING) == 0) {
        NET_OutOfBandPrintf(m_Socket, client->m_NetChannel->GetRemoteAddress(),
                            "%c00000000000000", S2C_CONNECTION);
      }
  ...
  if (receivingClientCount) {
    ...
    if (receivingClientCount > 1 && sv_parallel_sendsnapshot.GetBool()) {
      ParallelProcess(pReceivingClients, receivingClientCount,
  ...
  // Allow game .dll to run code, including unsetting EF_MUZZLEFLASH and
  // EF_NOINTERP on effects fields etc.
...
void SV_WriteVoiceCodec(bf_write &pBuf) {
  ...
  SVC_VoiceInit voiceinit(sv.IsMultiplayer() ? sv_voicecodec.GetString() : "",
                          5);

  voiceinit.WriteToBuffer(pBuf);
}

// Gets voice data from a client and forwards it to anyone who can hear this
// client.
ConVar voice_debugfeedbackfrom("voice_debugfeedbackfrom", "0");

void SV_BroadcastVoiceData(IClient *cl, int nBytes, char *data, int64_t xuid) {
  // Disable voice?
  if (!sv_voiceenable.GetInt()) return;
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    ...
    voiceData.m_bProximity =
        pDestClient->IsProximityHearingClient(voiceData.m_nFromClient);
...
void SV_CreateBaseline(void) {
  ...
  if (sv_sendtables.GetInt()) {
    ...
    sv.m_FullSendTables.StartWriting(sv.m_FullSendTablesBuffer.Base(),
                                     sv.m_FullSendTablesBuffer.Count());
  ...
  // If we're using the local network backdoor, we'll never use the instance
  // baselines.
  if (!g_pLocalNetworkBackdoor) {
    ...
    for (int entnum = 0; entnum < sv.num_edicts; entnum++) {
      ...
      if (edict->IsFree() || !edict->GetUnknown()) continue;

      ServerClass *pClass = edict->GetNetworkable()
                                ? edict->GetNetworkable()->GetServerClass()
                                : 0;
      ...
      if (!SendTable_Encode(pSendTable, edict->GetUnknown(), &writeBuf, entnum,
                            NULL, false)) {
        Host_Error(
      ...
      SV_EnsureInstanceBaseline(pClass, entnum, packedData,  //-V614
...
bool SV_ActivateServer() {
  ...
  g_pServerPluginHandler->ServerActivate(sv.edicts, sv.num_edicts,
                                         sv.GetMaxClients());
  ...
  if (IsUsingMasterLegacyMode()) master->Heartbeat_Legacy_f();
#ifndef NO_STEAM
  else
...
static void SV_AllocateEdicts() {
  sv.edicts =
      (edict_t *)Hunk_AllocName(sv.max_edicts * sizeof(edict_t), "edicts");
...
void CGameServer::ReloadWhitelist(const char *pMapName) {
  ...
  if (g_sv_pure_mode == 2) {
    // sv_pure 2 means to ignore the pure_server_whitelist.txt file and force
    // everything to come from Steam.
    ...
    bool bLoaded =
        kv->LoadFromFile(g_pFileSystem, pGlobalWhitelistFilename, "game");
    if (bLoaded) bLoaded = m_pPureServerWhitelist->LoadFromKeyValues(kv);
...
bool CGameServer::SpawnServer(char *mapname, char *startspot) {
  ...
  // Any partially connected client will be restarted if the spawncount is not
  // matched.
  ...
  // Assign class ids to server classes here so we can encode temp ents into
  // signon
  //  if needed
  AssignClassIds();
  ...
  if (!CommandLine()->FindParm("-allowstalezip")) {
    if (g_pFileSystem->FileExists("stale.txt", "GAME")) {
      Warning(
          "This map is not final!!  Needs to be rebuilt without -keepstalezip "
  ...
  for (i = 1; i < host_state.worldbrush->numsubmodels; i++) {
    ...
    PrecacheModel(localmodel, RES_FATALIFMISSING | RES_PRELOAD,
                  modelloader->GetModelForName(
                      localmodel, IModelLoader::FMODELLOADER_SERVER));
  }
...
void CGameServer::UpdateMasterServerPlayers() {
  for (int i = 0; i < GetClientCount(); i++) {
    ...
    if (!client->m_SteamID || !client->m_SteamID->IsValid()) continue;
#ifndef NO_STEAM
    SteamGameServer()->GSSetUserData(*client->m_SteamID,
                                     client->GetClientName(), pl->frags);
#endif
  }
}
...
#ifndef SWDS
  // Don't simulate in single player if console is down or the bug UI is active
  // and we're in a game
  if (!sv.IsMultiplayer()) {
...
//-----------------------------------------------------------------------------
// Purpose: Run physics code (simulating == false means we're paused, but we'll
// still
...
void SV_Think(bool bIsSimulating) {
  ...
  g_ServerGlobalVariables.frametime =
      bIsSimulating ? host_state.interval_per_tick : 0;
...
void SV_Frame(bool finalTick) {
  ...
  bool bSendDuringPause =
      sv_noclipduringpause ? sv_noclipduringpause->GetBool() : false;
  ...
  if (SV_HasPlayers()) {
    bool serverCanSimulate =
        (serverGameDLL && !serverGameDLL->IsRestoring()) ? true : false;
  ...
  if (finalTick) {
    if (!IsEngineThreaded() || sv.IsMultiplayer())
      ...
      g_pDeferredServerWork =
          CreateFunctor(SV_SendClientUpdates, bIsSimulating, bSendDuringPause);
  }
