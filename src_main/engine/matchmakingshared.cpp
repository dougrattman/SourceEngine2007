...
// the game, and reporting game results and stats after the game is
// complete.
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMatchmaking, IMatchmaking,
                                  VENGINE_MATCHMAKING_VERSION, s_Matchmaking);

bool Channel_LessFunc(const uint32_t &a, const uint32_t &b) { return a < b; }
...
int CMatchmaking::FindOrCreateContext(const uint32_t id) {
...
int CMatchmaking::FindOrCreateProperty(const uint32_t id) {
...
//-----------------------------------------------------------------------------
void CMatchmaking::AddSessionProperty(const uint32_t nType, const char *pID,
                                      const char *pValue,
...
void CMatchmaking::SetSessionProperties(KeyValues *pPropertyKeys) {
  ...
  for (KeyValues *pProperty = m_pSessionKeys->GetFirstSubKey();
       pProperty != NULL; pProperty = pProperty->GetNextKey()) {
    AddSessionPropertyInternal(pProperty);
  }
}
  ...
  switch (pProperty->GetInt("type")) {
    case SESSION_CONTEXT: {
      ...
      if (m_CurrentState != MMSTATE_INITIAL &&
          !Q_stricmp(pID, "CONTEXT_SCENARIO")) {
    ...
    case SESSION_PROPERTY: {
      ...
      if (m_CurrentState != MMSTATE_INITIAL &&
          !Q_stricmp(pID, "PROPERTY_MAX_GAME_TIME")) {
    ...
    default:
      Warning("Session option type #d not recognized/n",
              pProperty->GetInt("type"));
      break;
  }
}
...
bool CMatchmaking::ProcessVoiceData(CLC_VoiceData *pVoice) {
  ...
  if (pVoice->m_xuid != m_Local.m_xuids[0]) {
    Audio_GetXVoice()->PlayIncomingVoiceData(pVoice->m_xuid, (byte *)chReceived,
                                             dwLength);
  }
#endif
  return true;
...
INetChannel *CMatchmaking::CreateNetChannel(netadr_t *adr) {
  ...
  if (!pNewChannel) {
    pNewChannel =
        NET_CreateNetChannel(NS_MATCHMAKING, adr, "MATCHMAKING", this);
  }
...
//-----------------------------------------------------------------------------
void CMatchmaking::SendMessage(INetMessage *msg, CClientInfo *pClient,
                               bool bVoice) {
  // Find the matching net channel
...
//-----------------------------------------------------------------------------
void CMatchmaking::SendToRemoteClients(INetMessage *msg, bool bVoice,
                                       XUID excludeXUID) {
  for (int i = 0; i < m_Remote.Count(); ++i) {
...
bool CMatchmaking::SendHeartbeat(CClientInfo *pClient) {
  if (pClient->m_adr.GetIP() == 0) return false;
...
void CMatchmaking::SendHeartbeat() {
  double time = GetTime();
  if (time < m_fNextHeartbeatTime) return;
...
//-----------------------------------------------------------------------------
static uint64_t FindPlayerByName(CClientInfo *pClient, const char *pName) {
...
//-----------------------------------------------------------------------------
uint64_t CMatchmaking::PlayerIdToXuid(int playerId) {
  uint64_t ret = 0;
...
bool CMatchmaking::IsAcceptingConnections() {
  if (!m_Session.IsHost() || m_CurrentState < MMSTATE_ACCEPTING_CONNECTIONS ||
      m_CurrentState == MMSTATE_PREGAME || m_CurrentState == MMSTATE_LOADING ||
      GameIsLocked()) {
    return false;
  }
  return true;
...
//-----------------------------------------------------------------------------
void CMatchmaking::ClientInfoToNetMessage(MM_ClientInfo *pInfo,
                                          const CClientInfo *pClient) {
  ...
  Q_memcpy(pInfo->m_cVoiceState, pClient->m_cVoiceState,
           sizeof(pInfo->m_cVoiceState));
  Q_memcpy(pInfo->m_iTeam, pClient->m_iTeam, sizeof(pInfo->m_iTeam));
  Q_memcpy(pInfo->m_iControllers, pClient->m_iControllers,
           sizeof(pInfo->m_iControllers));
  Q_memcpy(pInfo->m_szGamertags, pClient->m_szGamertags,
           sizeof(pInfo->m_szGamertags));
}

void CMatchmaking::NetMessageToClientInfo(CClientInfo *pClient,
                                          const MM_ClientInfo *pInfo) {
  ...
  Q_memcpy(pClient->m_cVoiceState, pInfo->m_cVoiceState,
           sizeof(pClient->m_cVoiceState));
  Q_memcpy(pClient->m_iTeam, pInfo->m_iTeam, sizeof(pClient->m_iTeam));
  Q_memcpy(pClient->m_iControllers, pInfo->m_iControllers,
           sizeof(pClient->m_iControllers));
  Q_memcpy(pClient->m_szGamertags, pInfo->m_szGamertags,
           sizeof(pClient->m_szGamertags));
}
...
bool CMatchmaking::InitializeLocalClient(bool bIsHost) {
  ...
  // Set up the players
  for (uint32_t i = 0; i < MAX_PLAYERS_PER_CLIENT; ++i) {
    ...
    // xuid
    uint32_t ret = XUserGetXUID(i, &m_Local.m_xuids[m_Local.m_cPlayers]);
    ...
    // gamertag
    ret = XUserGetName(XBX_GetPrimaryUserId(),
                       m_Local.m_szGamertags[m_Local.m_cPlayers],
                       MAX_PLAYER_NAME_LENGTH);
    if (ret != ERROR_SUCCESS) {
      return false;
    }
    m_Local.m_szGamertags[m_Local.m_cPlayers][MAX_PLAYER_NAME_LENGTH - 1] =
        '\0';

    // Set the player's name in the game
    char szNameCmd[MAX_PLAYER_NAME_LENGTH + 16];
    Q_snprintf(szNameCmd, sizeof(szNameCmd), "name %s",
               m_Local.m_szGamertags[m_Local.m_cPlayers]);
  ...
  Q_strncpy(m_HostData.hostName, m_Local.m_szGamertags[0],
            sizeof(m_HostData.hostName));
...
//			add the local players to the teams that were setup in
// the lobby.
//-----------------------------------------------------------------------------
void CMatchmaking::AddLocalPlayersToTeams() {
  if (!m_bInitialized || XBX_GetPrimaryUserId() == INVALID_USER_ID) return;

  if (m_Local.m_iTeam[0] == -1) return;

  // Convert the team number into a team name
  char szTeamName[32];
  uint32_t id = g_ClientDLL->GetPresenceID("PROPERTY_TEAM");
  g_ClientDLL->GetPropertyDisplayString(id, m_Local.m_iTeam[0], szTeamName,
                                        sizeof(szTeamName));
...
bool CMatchmaking::ProcessClientInfo(MM_ClientInfo *pInfo) {
  ...
  if (!pClient && pInfo->m_cPlayers != 0) {
    ...
    if (pInfo->m_cPlayers) {
      // Cache off the old client info, as pClient gets updated through this
      // function
      CClientInfo tempClient = *pClient;

      // Check for player changes
      if (Q_memcmp(&tempClient.m_xuids, pInfo->m_xuids,
                   sizeof(tempClient.m_xuids))) {
      ...
      for (int i = 0; i < pInfo->m_cPlayers; ++i) {
        if (pInfo->m_iTeam[i] != tempClient.m_iTeam[i] ||
            pInfo->m_cVoiceState[i] != tempClient.m_cVoiceState[i]) {
          // X360TBD: send real "ready" setting, or remove entirely?
          EngineVGui()->UpdatePlayerInfo(
              pInfo->m_xuids[i], pInfo->m_szGamertags[i], pInfo->m_iTeam[i],
              pInfo->m_cVoiceState[i], GetPlayersNeeded(), bHost);
        }
      }
      ...
      for (int i = 0; i < sv.GetClientCount(); i++) {
        ...
        if (pIClient) {
          for (int j = 0; j < pClient->m_cPlayers; ++j) {
            if (pClient->m_xuids[j] == 0) continue;

            if (Q_stricmp(pIClient->GetClientName(),
                          pClient->m_szGamertags[j]) == 0) {
              bFound = true;
...
// Purpose: A player is leaving the session
//-----------------------------------------------------------------------------
void CMatchmaking::KickPlayerFromSession(uint64_t id) {
  MM_ClientInfo droppedPlayer;
...
bool CMatchmaking::IsPlayerMuted(int iUserId, XUID playerId) {
  ...
  if (m_MutedBy[iUserId].HasElement(playerId)) return true;
...
static ConCommand voice_printtalkers("voice_printtalkers", Con_PrintTalkers,
                                     "voice debug.", FCVAR_DONTRECORD);
...
void CMatchmaking::ChangeTeam(const char *pTeamName) {
  if (!pTeamName) {
    ...
    for (int iTeam = 0; iTeam < m_nTotalTeams; ++iTeam) {
      g_ClientDLL->GetPropertyDisplayString(id, iTeam, szTeamName,
                                            sizeof(szTeamName));
...
bool CMatchmaking::ProcessCheckpoint(MM_Checkpoint *pMsg) {
  ...
  switch (pMsg->m_Checkpoint) {
    ...
    case MM_Checkpoint::CHECKPOINT_REPORT_STATS:
      ...
      g_ClientDLL->StartStatsReporting(m_Session.GetSessionHandle(),
                                       m_Session.IsArbitrated());
    ...
    case MM_Checkpoint::CHECKPOINT_POSTGAME:
      ...
      if (m_Session.IsArbitrated()) {
        ...
        if (m_Session.IsHost()) {
          ...
          m_HostData.gameState = GAMESTATE_INLOBBY;
          UpdateSessionReplyData(XNET_QOS_LISTEN_ENABLE |
                                 XNET_QOS_LISTEN_SET_DATA);
        }

        // Tell gameui to activate the lobby
        SessionNotification(m_Session.IsHost() ? SESSION_NOTIFY_ENDGAME_HOST
...
void CMatchmaking::SendPlayerInfoToLobby(CClientInfo *pClient, int iHostIdx) {
  for (int i = 0; i < pClient->m_cPlayers; ++i) {
    EngineVGui()->UpdatePlayerInfo(
        ...
        pClient->m_cVoiceState[i], GetPlayersNeeded(), iHostIdx == i);
  }
}
...
void CMatchmaking::UpdatePregame() {
  ...
  if (elapsedTime > REGISTRATION_MAXWAITTIME) {
    ...
    if (m_Session.IsHost() && m_Session.IsArbitrated() &&
        !m_Local.m_bRegistered) {
  ...
  if (elapsedTime < STARTGAME_COUNTDOWN) {
    SessionNotification(SESSION_NOTIFY_COUNTDOWN,
                        STARTGAME_COUNTDOWN - elapsedTime);
    return;
  }
  ...
  if (!IsServer()) {
    SetPreventFullServerStartup(
        true, "SESSION_NOTIFY_COUNTDOWN == 0 and not the host\n");
  }
...
//-----------------------------------------------------------------------------
void CMatchmaking::SessionNotification(const SESSION_NOTIFY notification,
                                       const int param) {
  // Notify GameUI
...
void CMatchmaking::RunFrame() {
  ...
  switch (m_CurrentState) {
    ...
    case MMSTATE_ACCEPTING_CONNECTIONS:
      // Host is sitting in the Lobby waiting for connection requests. Once the
      // game is full enough (player count >= min players) the host will be able
      // to start the game.
...
//-----------------------------------------------------------------------------
// Purpose: Let the invite system determine a good moment to start connecting to
// inviter's host
//-----------------------------------------------------------------------------
...
void CMatchmaking::SetPreventFullServerStartup(bool bState, char const *fmt,
  ...
  DevMsg(1, "Setting state from prevent %s to prevent %s:  %s",
         m_bPreventFullServerStartup ? "yes" : "no", bState ? "yes" : "no",
         desc);
