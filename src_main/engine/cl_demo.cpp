...
static ConVar demo_quitafterplayback("demo_quitafterplayback", "0", 0,
                                     "Quits game after demo playback.");
static ConVar demo_debug("demo_debug", "0", 0, "Demo debug info.");
static ConVar demo_interpolateview(
    "demo_interpolateview", "1", 0,
    "Do view interpolation during dem playback.");
static ConVar demo_pauseatservertick("demo_pauseatservertick", "0", 0,
                                     "Pauses demo playback at server tick");

// singeltons:
static ch g_pStatsFile[MAX_OSPATH] = {NULL};
...
// This is the number of units under which we are allowed to interpolate,
// otherwise pop. This fixes problems with in-level transitions.
static ConVar demo_interplimit("demo_interplimit", "4000", 0,
                               "How much origin velocity before it's "
                               "considered to have 'teleported' causing "
                               "interpolation to reset.");
static ConVar demo_avellimit(
    "demo_avellimit", "2000", 0,
...
static ConVar demo_fastforwardstartspeed(
    "demo_fastforwardstartspeed", "2", 0,
    "Go this fast when starting to hold FF button.");
static ConVar demo_fastforwardfinalspeed(
    "demo_fastforwardfinalspeed", "20", 0,
    "Go this fast when starting to hold FF button.");
static ConVar demo_fastforwardramptime(
    "demo_fastforwardramptime", "5", 0,
...
//  other kinds of data items that are also included in the demo file:
//  specifically commands that the client .dll itself issued to the engine are
//  recorded, though they probably were not the result of network traffic.
// At the start of a connection to a map/server, all of the signon, etc. network
// packets
//  are buffered.  This allows us to actually decide to start recording the demo
//  at a later time.  Once we actually issue the recording command, we don't
//  actually start recording network traffic, but instead we ask the server for
//  an "uncompressed" packet (otherwise we wouldn't be able to deal with the
//  incoming packets during playback because we'd be missing the data to delta
//  from ) and go into a waiting state.  Once an uncompressed packet is
//  received, we unset the waiting state and start recording network packets
//  from that point forward.
// Demo's record the elapsed time based on the current client clock minus the
// time the demo was started During playback, the elapsed time for playback (
// based on the host_time, which is subject to the
//  host_frametime cvar ) is compared with the elapsed time on the message from
//  the demo file.
// If it's not quite time for the message yet, the demo input stream is rewound
// The demo system sits at the point where the client is checking to see if any
// network messages
//  have arrived from the server.  If the message isn't ready for processing,
//  the demo system just responds that there are no messages waiting and the
//  client continues on
// Once a true network message with entity data is read from the demo stream, a
// couple of other
//  actions occur.  First, the timestamp in the demo file and the view
//  origin/angles corresponding to the message are cached off.  Then, we search
//  ahead (into the future) to find out the next true network message we are
//  going to read from the demo file.  We store of it's elapsed time and view
//  origin/angles
// Every frame that the client is rendering, even if there is no data from the
// demo system,
//  the engine asks the demo system to compute an interpolated origin and view
//  angles.  This is done by taking the current time on the host and figuring
//  out how far that puts us between the last read origin from the demo file and
//  the time when we'll actually read out and use the next origin
// We use Quaternions to avoid gimbel lock on interpolating the view angles
// To make a movie recorded at a fixed frame rate you would simply set the
// host_framerate to the
//  desired playback fps ( e.g., 0.02 == 50 fps ), then issue the startmovie
//  command, and then play the demo.  The demo system will think that the engine
//  is running at 50 fps and will pull messages accordingly, even though movie
//  recording kills the actually framerate.
// It will also render frames with render origin/angles interpolated in-between
// the previous and next origins
//  even if the recording framerate was not 50 fps or greater.  The
//  interpolation provides a smooth visual playback of the demo information to
//  the client without actually adding latency to the view position (because we
//  are looking into the future for the position, not buffering the past data ).
//-----------------------------------------------------------------------------

static bool IsControlCommand(unsigned char cmd) {
  return ((cmd == dem_signon) || (cmd == dem_stop) || (cmd == dem_synctick) ||
          (cmd == dem_datatables));
}

// Puts a flashing overlay on the screen during demo recording/playback
static ConVar cl_showdemooverlay("cl_showdemooverlay", "0", 0,
                                 "How often to flash demo recording/playback "
                                 "overlay (0 - disable overlay, -1 - show "
                                 "always)");
...
DemoOverlay::DemoOverlay()
    : m_fLastTickTime(0.f),
      m_fLastTickOverlay(0.f),
      m_bTick(false),
      m_maskDrawnOverlay(OVR_NONE) {}
...
void DemoOverlay::Tick() {
  if (!m_bTick) {
    ...
    if (m_fLastTickTime != fRealTime) {
      ...
      if (fSettingDelta <= 0.f || fDelta >= fSettingDelta) {
...
void DemoOverlay::DrawOverlay(float fSetting) {
  ...
  if (fSetting < 0.f) {
    // Keep drawing
    maskDrawnOverlay = (demorecorder->IsRecording() ? OVR_REC : 0) |
  ...
  if (OVR_NONE == maskDrawnOverlay && OVR_NONE != m_maskDrawnOverlay) {
    con_nprint_s xprn;
...
int CDemoRecorder::GetRecordingTick() {
...
void CDemoRecorder::WriteBSPDecals() {
  decallist_t *decalList =
      (decallist_t *)malloc(sizeof(decallist_t) * Draw_DecalMax());

  int decalcount = DecalListCreate(decalList);

  ch data[NET_MAX_PAYLOAD];
  bf_write msg;
  ...
  for (int i = 0; i < decalcount; i++) {
    ...
    IClientEntity *clientEntity =
        entitylist->GetClientEntity(entry->entityIndex);
...
void CDemoRecorder::RecordServerClasses(ServerClass *pClasses) {
  ...
  ch *pBigBuffer = stack_alloc<ch>(buffSize);
...
void CDemoRecorder::RecordUserInput(int cmdnumber) {
  ...
  m_DemoFile.WriteUserCmd(cmdnumber, buffer, msg.GetNumBytesWritten(),  //-V614
                          GetRecordingTick());
}

void CDemoRecorder::ResetDemoInterpolation() { m_bResetInterpolation = true; }
...
void CDemoRecorder::WriteDemoCvars() {
  ...
  for (var = g_pCVar->GetCommands(); var; var = var->GetNext()) {
    ...
    const ConVar *con_var = (const ConVar *)var;

    if (!con_var->IsFlagSet(FCVAR_DEMO)) continue;

    ch cvarcmd[MAX_OSPATH];

    Q_snprintf(cvarcmd, sizeof(cvarcmd), "%s \"%s\"", con_var->GetName(),
               Host_CleanupConVarStringValue(con_var->GetString()));
...
// Input  : *cmdname -
//-----------------------------------------------------------------------------
void CDemoRecorder::RecordCommand(const ch *cmdstring) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CDemoRecorder::StartupDemoHeader() {
  ...
  m_MessageData.StartWriting(new unsigned char[NET_MAX_PAYLOAD],
                             NET_MAX_PAYLOAD);
...
void CDemoRecorder::StartupDemoFile() {
  ...
  ch demoFileName[MAX_OSPATH];

  if (m_nDemoNumber <= 1) {
    sprintf_s(demoFileName, "%s.dem", m_szDemoBaseName);
  } else {
    sprintf_s(demoFileName, "%s_%i.dem", m_szDemoBaseName, m_nDemoNumber);
  }
  ...
  if (m_DemoFile.IsOpen()) {
    if (!m_bIsDemoHeader) {
      ...
      m_DemoFile.m_DemoHeader.playback_time =
          host_state.interval_per_tick * GetRecordingTick();
      m_DemoFile.m_DemoHeader.playback_frames = m_nFrameCount;

      // go back to header and write demoHeader with correct time and #frame
      // again
      m_DemoFile.WriteDemoHeader();

      ConMsg("Completed demo, recording time %.1f, game frames %i.\n",
             m_DemoFile.m_DemoHeader.playback_time,
             m_DemoFile.m_DemoHeader.playback_frames);
    }
...
void CDemoRecorder::WriteMessages(bf_write &message) {
  ...
  // fill last bits in last uint8_t with NOP if necessary
...
//-----------------------------------------------------------------------------
void CDemoRecorder::StopRecording() {
  if (!IsRecording()) {
    return;
  }
...
//			track -
//-----------------------------------------------------------------------------
void CDemoRecorder::StartRecording(const ch *name, bool bContinuously) {
...
void CDemoPlayer::StopPlayback() {
  ...
  if (m_bTimeDemo) {
    ...
    if (demotime > 0.0f) {
      DevMsg(
          "Demo playback finished ( %.1f seconds, %i render frames, %.2f "
          "fps).\n",
...
CDemoFile *CDemoPlayer::GetDemoFile() { return &m_DemoFile; }

#define SKIP_TO_TICK_FLAG uint32_t(uint32_t(0x88) << 24)

bool CDemoPlayer::IsSkipping() {
  return m_bPlayingBack && (m_nSkipToTick != -1);
}

int CDemoPlayer::GetTotalTicks() {
...
void CDemoPlayer::SkipToTick(int tick, bool bRelative, bool bPause) {
  ...
  if (tick < GetPlaybackTick()) {
    ...
    if (tick > 0) tick |= SKIP_TO_TICK_FLAG;
  }

  m_nSkipToTick = tick;

  if (bPause) PausePlayback(-1);
}

//-----------------------------------------------------------------------------
// Purpose: Read in next demo message and send to local client over network
// channel, if it's time. Output : bool
//-----------------------------------------------------------------------------
  ...
  while (true) {
    ...
    do {
      ...
      switch (cmd) {
        ...
        default: { swallowmessages = false; } break;
      }
...
// channel, if it's time. Output : bool
//-----------------------------------------------------------------------------
netpacket_t *CDemoPlayer::ReadPacket() {
  int tick = 0;
  byte cmd = dem_signon;
  long curpos = 0;
  ...
  // If game is still shutting down, then don't read any demo messages from file
  // quite yet
  ...
  while (!bStopReading) {
    ...
    if (!IsControlCommand(cmd)) {
      ...
      if (!m_bTimeDemo) {
        // Time demo ignores clocks and tries to synchronize frames to what was
        // recorded
        //  I.e., while frame is the same, read messages, otherwise, skip out.
        // If we're still signing on, then just parse messages until fully
        // connected no matter what
        if (cl.IsActive() && (tick > playbacktick) && !IsSkipping()) {
          // is not time yet
          bStopReading = true;
        }
      } else {
        if (m_nTimeDemoCurrentFrame == host_framecount) {
          ...
          //  frame update for this host_frame tag, then we'll just skip this
          //  mess
          bStopReading = true;
        }
      }
      ...
      if (bStopReading) {
        ...
        m_DemoFile.SeekTo(curpos,
                          true);  // go back to start of current demo command
    ...
    switch (cmd) {
      ...
      case dem_consolecmd: {
        const ch *command = m_DemoFile.ReadConsoleCommand();
      ...
      case dem_datatables: {
        ...
        if (!DataTable_LoadDataTablesFromBuffer(
                &buf, m_DemoFile.m_DemoHeader.demoprotocol)) {
      ...
      case dem_usercmd: {
        ...
        // Note, we need to have the current outgoing sequence correct so we can
        // do prediction
  ...
  int length = m_DemoFile.ReadRawData((ch *)m_DemoPacket.data, NET_MAX_PAYLOAD);
...
void CDemoPlayer::InterpolateDemoCommand(int targettick, DemoCommandQueue &prev,
  ...
  while (true) {
    ...
    // If any command between the previous target and now has the
    // FDEMO_NOINTERP, we need to stop at the command just before that (entry),
    // so we save off the I We can't just return since we need to see if we
    // actually get to a spanning pair (though we always should).  Also, we only
    // latch this final interp spot on
    ///  the first FDEMO_NOINTERP we see
    if (savedI == -1 && entry2->tick > m_nPreviousTick &&
        entry2->tick <= targettick && entry2->info.flags & FDEMO_NOINTERP) {
      savedI = i;
    }
...
static ConVar demo_legacy_rollback(
    "demo_legacy_rollback", "1", 0,
    "Use legacy view interpolation rollback amount in demo playback.");

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CDemoPlayer::InterpolateViewpoint() {
  if (!IsPlayingBack()) return;

  democmdinfo_t outinfo;
  outinfo.Reset();

  bool bHasValidData = m_LastCmdInfo.viewOrigin != vec3_origin ||
  ...
  // Player view needs to be one tick interval in the past like the client DLL
  // entities
  if (cl.m_nMaxClients == 1) {
  ...
  if (m_bInterpolateView && demo_interpolateview.GetBool() && bHasValidData) {
    ...
    // check for teleporting - since there can be multiple cmd packets between a
    // game frame, we need to check from the last actually ran command to see if
    ...
    if (dt > 0.0f) {
      ...
      for (int i = 0; i < 3; ++i) {
        float dAng = AngleNormalizePositive(destang[i]) -
                     AngleNormalizePositive(startang[i]);
  ...
  if (!bHasValidData) return;  // no validate data & no override, exit
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CDemoPlayer::IsPlayingTimeDemo() { return m_bTimeDemo && m_bPlayingBack; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CDemoPlayer::IsPlayingBack() { return m_bPlayingBack; }
...
CDemoPlayer::~CDemoPlayer() { StopPlayback(); }

//-----------------------------------------------------------------------------
// Purpose: Start's demo playback
// Input  : *name -
//-----------------------------------------------------------------------------
bool CDemoPlayer::StartPlayback(const ch *filename, bool bAsTimeDemo) {
...
void CDemoPlayer::WriteTimeDemoResults() {
  int frames;
  float time;
  ...
  g_pFileSystem->FPrintf(fileHandle, "%s,",
                         ImageLoader::GetName(backBufferFormat));
  ...
  g_pFileSystem->FPrintf(
      fileHandle, "%s,",
      g_pMaterialSystemConfig->ForceTrilinear() ? "enabled" : "disabled");
  g_pFileSystem->FPrintf(fileHandle, "%s,",
  ...
  g_pFileSystem->FPrintf(
      fileHandle, "%s,",
      CommandLine()->CheckParm("-nosound") ? "disabled" : "enabled");
  g_pFileSystem->FPrintf(
      fileHandle, "%s,",
      !CommandLine()->CheckParm("-mat_vsync") ? "disabled" : "enabled");
  g_pFileSystem->FPrintf(fileHandle, "\n");
...
int CDemoPlayer::GetPlaybackTick() { return host_tickcount - m_nStartTick; }
...
void CDemoPlayer::SetBenchframe(int tick, const ch *filename) {
...
static bool ComputeNextIncrementalDemoFilename(ch *name, int namesize) {
  FileHandle_t test;
  ...
  ConMsg(
      "Unable to find a valid incremental demo filename for %s, try clearing "
      "the directory of %snnn.dem\n",
      name, basename);
  return false;
}
...
CON_COMMAND_F(record, "Record a demo.", FCVAR_DONTRECORD) {
  ...
  if (!g_ClientDLL->CanRecordDemo(name, sizeof(name))) {
    ConMsg("%s\n", name);  // re-use name as the error string if the client
                           // prevents us from starting a demo
    return;
  }
...
void CL_TimeDemo_f(const CCommand &args) {
  ...
  if (args.ArgC() < 2 || args.ArgC() > 4) {
    ConMsg(
        "timedemo <demoname> <optional stats.txt> : gets demo speeds, starting "
        "from optional frame\n");
    return;
  }
...
void CL_BenchFrame_f(const CCommand &args) {
  ...
  if (args.ArgC() != 4) {
    ConMsg(
        "benchframe <demoname> <frame> <tgafilename>: takes a snapshot of a "
        "particular frame in a demo\n");
    return;
  }
...
CON_COMMAND_AUTOCOMPLETEFILE(playdemo, CL_PlayDemo_f,
                             "Play a recorded demo file (.dem ).", NULL, dem);
CON_COMMAND_AUTOCOMPLETEFILE(timedemo, CL_TimeDemo_f,
                             "Play a demo and report performance info.", NULL,
                             dem);
CON_COMMAND_AUTOCOMPLETEFILE(
    timedemoquit, CL_TimeDemoQuit_f,
    "Play a demo, report performance info, and then exit", NULL, dem);
CON_COMMAND_AUTOCOMPLETEFILE(listdemo, CL_ListDemo_f,
                             "List demo file contents.", NULL, dem);
CON_COMMAND_AUTOCOMPLETEFILE(
    benchframe, CL_BenchFrame_f,
    "Takes a snapshot of a particular frame in a time demo.", NULL, dem);
...
void CDemoPlayer::ResetDemoInterpolation() { m_bResetInterpolation = true; }
