...
CNewActionButton::CNewActionButton(Panel *parent, const ch *panelName,
  ...
  for (i = 1; i < count; i++) {
    ...
    m_pMenu->SetItemEnabled(actionType,
                            CBaseDemoAction::HasEditorFactory((DEMOACTION)i));
  }
...
//-----------------------------------------------------------------------------
CDemoEditorPanel::CDemoEditorPanel(vgui::Panel *parent)
    : Frame(parent, "DemoEditorPanel") {
  int w = 440;
  int h = 300;
  ...
  m_pActions->AddColumnHeader(0, "actionname", "Action",
                              m_pActions->GetWide() / 3);
  m_pActions->AddColumnHeader(1, "actiontype", "Type",
                              m_pActions->GetWide() / 3);
  m_pActions->AddColumnHeader(2, "actionstart", "Start",
                              m_pActions->GetWide() / 3);
  OnRefresh();
...
void CDemoEditorPanel::CreateNewAction(ch const *actiontype) {
  ...
  if (type == DEMO_ACTION_UNKNOWN) return;
  ...
  if (action) {
    ...
    m_hCurrentEditor = CBaseDemoAction::CreateActionEditor(action->GetType(),
                                                           this, action, true);
...
void CDemoEditorPanel::PopulateActionList() {
  ...
  for (i = 0; i < count; i++) {
    ...
    KeyValues *item =
        new KeyValues("data", "actionname", action->GetActionName());
    item->SetString("actiontype",
                    CBaseDemoAction::NameForType(action->GetType()));
...
void CDemoEditorPanel::OnSave() { demoaction->SaveToFile(); }
...
CBaseDemoAction *CDemoEditorPanel::FindActionByName(ch const *name) {
  ...
  for (i = 0; i < count; i++) {
    CBaseDemoAction *action = demoaction->GetAction(i);
    Assert(action);
    if (!Q_strcasecmp(name, action->GetActionName())) return action;
  }
  return NULL;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CDemoEditorPanel::OnRefresh() { PopulateActionList(); }
