...
const char *COM_Parse(const char *data) {
  ...
  if (com_ignorecolons) breaks = &g_BreakSet;
...
char *vstr(Vector &v) {
  ...
  Q_snprintf(string[idx], sizeof(string[idx]), "%.2f %.2f %.2f", v[0], v[1],
             v[2]);
  return string[idx];
}
...
===========
*/
int COM_FileSize(const char *filename) { return g_pFileSystem->Size(filename); }
...
uint8_t *COM_LoadFile(const char *path, int usehunk, int *pLength) {
  ...
  switch (usehunk) {
    case 1:
      buf = (uint8_t *)Hunk_AllocName(bufSize, base);
      break;
    case 2:
...
void COM_StringFree(const char *in) { delete[] in; }
...
/*
================
COM_GetModDirectory - return the final directory in the game dir (i.e "cstrike",
"hl2", rather than c:\blah\cstrike )
================
*/
...
void COM_InitFilesystem(const char *pFullModPath) {
  ...
  // Set com_basedir.
  Q_strncpy(com_basedir, GetBaseDirectory(),
            sizeof(com_basedir));  // the "root" directory where hl2.exe is
