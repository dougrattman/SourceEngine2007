...
// You tell it the number of samples to hang onto and it will average that
// number of samples as you feed it data.
...
inline CLowPassStream<DataType, nSamples>::CLowPassStream() {
  for (int i = 0; i < nSamples; i++) SetToZero(m_Samples[i]);
...
template <class DataType, int nSamples>
inline void CLowPassStream<DataType, nSamples>::AddSample(
    DataType const &data) {
  m_Samples[m_iOutSample] = data;

  ++m_nSamplesGotten;
  if (m_nSamplesGotten >= nSamples) m_nSamplesGotten = nSamples;

  ++m_iOutSample;
  if (m_iOutSample >= nSamples) m_iOutSample = 0;
}
...
inline DataType CLowPassStream<DataType, nSamples>::GetCurrentAverage() {
  ...
  SetToZero(data);
  for (int i = 0; i < nSamples; i++) AddSamples(data, m_Samples[i], data);
