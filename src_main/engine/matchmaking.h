...
enum MMPACKETS { PTH_CONNECT, PTH_SYSTEMLINK_SEARCH, HTP_SYSTEMLINK_REPLY };
...
enum eGameState { GAMESTATE_INLOBBY, GAMESTATE_INPROGRESS };

#define HEARTBEAT_INTERVAL_LONG \
  1.0  // send a heartbeat every second during gameplay
#define HEARTBEAT_INTERVAL_SHORT \
  0.1  // send a heartbeat ten times a second in the lobby
#define HEARTBEAT_TIMEOUT \
  10.0  // time out if a heartbeat isn't recieved for ten seconds
#if defined(_DEBUG)
#define HEARTBEAT_TIMEOUT_LOADING 300  // in debug loads take much longer
#else
#define HEARTBEAT_TIMEOUT_LOADING \
  100  // allow for longer communication gaps during map load
#endif

#define STARTGAME_COUNTDOWN 15.0  // start game countdown timer
#define DISCONNECT_WAITTIME \
  1.0  // wait for the server to reply to our disconnect notification
#define QOSLOOKUP_WAITTIME \
  20.0  // wait to get quality of service data about session hosts
#define JOINREPLY_WAITTIME \
  15.0  // time to wait for the host to reply to our join request
#define REPORTSTATS_WAITTIME \
  20.0  // time to wait for clients to report their stats to live
...
#define SESSIONMODIRY_MAXWAITTIME \
  10  // max time for clients to update their session properties
...
#define HOSTMIGRATION_MAXWAITTIME \
  10  // time to wait for a new host to contact us

#define MAX_SEARCHRESULTS \
  20  // Maximum number of results when searching for a session.
...
class CMatchmaking : public IMatchmaking,
                     public IMatchmakingMessageHandler,
 ...
 public:
  ...
  virtual void SessionNotification(const SESSION_NOTIFY notification,
                                   const int param = 0);
  virtual void AddSessionProperty(const uint32_t nType, const char *pID,
                                  const char *pValue, const char *pValueType);
  virtual void SetSessionProperties(KeyValues *pPropertyKeys);
  virtual void SelectSession(uint32_t sessionIdx);
  ...
  virtual void KickPlayerFromSession(uint64_t id);

  // For GameUI
  virtual KeyValues *GetSessionProperties();

  // For voice chat
  virtual uint64_t PlayerIdToXuid(int playerId);
  ...
  virtual void ConnectionStart(
      INetChannel *chan);    // called first time network channel is established
  ...
  virtual void ConnectionClosing(
      const char *reason){};  // network channel is being closed by remote site
  virtual void ConnectionCrashed(
      const char *reason){};  // network error occurred
  virtual void PacketStart(
      int incoming_sequence,
      int outgoing_acknowledged){};  // called each time a new packet arrived
  virtual void FileRequested(
      const char *fileName,
      unsigned int transferID){};  // other side request a file for download
  virtual void FileReceived(const char *fileName,
                            unsigned int transferID){};  // we received a file
  virtual void FileDenied(
      const char *fileName,
      unsigned int transferID){};  // a file request was denied by other side

  // Debugging helpers
  void ShowSessionInfo();
  void SendDevMessage(const char *message);

  void SetSessionSlots(const uint32_t nSlotsTotal,
                       const uint32_t nSlotsPrivate);
  void RunFrame();
  void EndGame();
 ...
 private:
  ...
  void SendToRemoteClients(INetMessage *msg, bool bVoice = false,
                           XUID excludeXUID = -1);
  ...
  void UpdateSessionReplyData(uint32_t flags);
  ...
  void ApplySessionProperties(int numContexts, int numProperties,
  ...
  int FindOrCreateContext(const uint32_t id);
  int FindOrCreateProperty(const uint32_t id);
  ...
  *m_pSearchResults;  // dynamic buffer to hold session search results
  ...
  uint64_t m_Nonce;  // used in system link queries
