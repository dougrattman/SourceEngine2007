...
// ----------------------------------------------------------------------------------------
// // This is a datatable test case. It is run in debug mode when the engine
// starts to catch any bugs in datatable code. This can also serve as a simple
// example of how datatables work separately from the intricacies of the entity
// system.
// ----------------------------------------------------------------------------------------
// // This is also a good place to test new code since it's run right when the
// engine starts up. It can also be put into standalone apps easily.
// ----------------------------------------------------------------------------------------
// // Things it tests:
...
// - A function that initializes the server structure and tries random changes
// to it and
...
void SendProxy_DTTestServerSubString(const SendProp *pProp, const void *pStruct,
...
SendPropInt(SENDINFO_NOCHECK(a)), SendPropInt(SENDINFO_NOCHECK(b)),
 ...
 public:
  ...
  char m_String[64];
  int m_Int;
  int m_IntArray[32];  // Note that the server and client array length are
                       // different.
  char m_CharArray[8];
...
void SendProxy_DTTestServerFloat(const SendProp *pProp, void *pStruct,
                                 void *pData, DVariant *pOut, int iElement,
                                 int objectID) {
  SendProxy_FloatToFloat(pProp, pStruct, pData, pOut, iElement, objectID);
}

void SendProxy_DTTestServerVector(const SendProp *pProp, void *pStruct,
                                  void *pData, DVariant *pOut, int iElement,
                                  int objectID) {
  SendProxy_VectorToVector(pProp, pStruct, pData, pOut, iElement, objectID);
}

void SendProxy_DTTestServerString(const SendProp *pProp, void *pStruct,
                                  void *pData, DVariant *pOut, int iElement,
                                  int objectID) {
  SendProxy_StringToString(pProp, pStruct, pData, pOut, iElement, objectID);
}

void SendProxy_DTTestServerInt(const SendProp *pProp, void *pStruct,
                               void *pData, DVariant *pOut, int iElement,
                               int objectID) {
...
void *SendProxy_DTTestServerSub(const SendProp *pProp, const void *pStruct,
                                const void *pData,
                                CSendProxyRecipients *pRecipients,
                                int objectID) {
  if (!g_bSendSub) return NULL;

  return SendProxy_DataTableToDataTable(pProp, pStruct, pData, pRecipients,
                                        objectID);
}
...
void RecvProxy_DTTestClientSubString(const CRecvProxyData *pData, void *pStruct,
                                     void *pOut) {
...
RecvPropInt(RECVINFO(a)), RecvPropInt(RECVINFO(b)), RecvPropFloat(RECVINFO(f)),
    ...
    RecvPropUtlVector(RECVINFO_UTLVECTOR(m_UtlVectorFloat),
                      MAX_FLOATARRAY_ELEMENTS, RecvPropFloat(NULL, 0, 0)),
    RecvPropUtlVectorDataTable(m_UtlVectorStruct, MAX_STRUCTARRAY_ELEMENTS,
                               DT_TestStruct),

    RecvPropUtlVector(RECVINFO_UTLVECTOR(m_UtlVectorChar),
                      MAX_CHARARRAY_ELEMENTS,
...
void RecvProxyArrayLength_VLA(void *pStruct, int objectID,
...
#if defined(SUPPORT_ARRAYS_OF_DATATABLES)
RecvPropArray(RecvPropDataTable(RECVINFO_DT(m_SubArray[0]), 0,
                                &REFERENCE_RECV_TABLE(DT_DTTestSub),
                                RecvProxy_DTTestClientSub),
              m_SubArray),
#endif
    ...
    RecvPropDataTable(RECVINFO_DT(m_Sub), 0,
                      &REFERENCE_RECV_TABLE(DT_DTTestSub)),
    RecvPropDataTable(RECVINFO_DT(m_Sub2), 0,
                      &REFERENCE_RECV_TABLE(DT_DTTestSub2)),
    ...
    typedef bool (*CompareElementFn)(DTTestClient *pClient,
                                     DTTestServer *pServer);
...
void RandomlyChangeStringGeneric(char *str, int size) {
  for (int i = 0; i < size - 1; i++) str[i] = (char)rand();
...
void RandomlyChangeSubString0(DTTestServer *pServer) {
  if (g_bSendSub)
    RandomlyChangeStringGeneric(pServer->m_Sub.m_Strings[0],
                                sizeof(pServer->m_Sub.m_Strings[0]));
}
...
void RandomlyChangeSubString1(DTTestServer *pServer) {
  if (g_bSendSub)
    RandomlyChangeStringGeneric(pServer->m_Sub.m_Strings[1],
                                sizeof(pServer->m_Sub.m_Strings[1]));
}
...
void RandomlyChangeInt(DTTestServer *pServer) { pServer->m_Int = (int)rand(); }

bool CompareIntArray(DTTestClient *pClient, DTTestServer *pServer) {
  // Just verify however much of the data we can.
  int leastElements =
      (sizeof(pClient->m_IntArray) < sizeof(pServer->m_IntArray))
          ? (sizeof(pClient->m_IntArray) / sizeof(pClient->m_IntArray[0]))
          : (sizeof(pServer->m_IntArray) / sizeof(pServer->m_IntArray[0]));
  return memcmp(pClient->m_IntArray, pServer->m_IntArray,
                leastElements * sizeof(int)) == 0;
}
...
bool CompareFloatArray(DTTestClient *pClient, DTTestServer *pServer) {
  ...
  /*
          int leastElements = (sizeof(pClient->m_Sub.m_FloatArray) <
     sizeof(pServer->m_Sub.m_FloatArray)) ?
     (sizeof(pClient->m_Sub.m_FloatArray)/sizeof(pClient->m_Sub.m_FloatArray[0]))
     :
     (sizeof(pServer->m_Sub.m_FloatArray)/sizeof(pServer->m_Sub.m_FloatArray[0]));
          for(int i=0; i < leastElements; i++)
          {
                  if(pClient->m_Sub.m_FloatArray[i] !=
     pServer->m_Sub.m_FloatArray[i]) return false;
          }
  */
  return true;
}
void RandomlyChangeFloatArray(DTTestServer *pServer) {
  // Change a random subset of the array.
  int nElements = sizeof(pServer->m_Sub.m_FloatArray) /
                  sizeof(pServer->m_Sub.m_FloatArray[0]);
  int nChanges = 4 + rand() % nElements;
...
bool CompareCharArray(DTTestClient *pClient, DTTestServer *pServer) {
  return memcmp(pClient->m_CharArray, pServer->m_CharArray,
                sizeof(pClient->m_CharArray)) == 0;
}
void RandomlyChangeCharArray(DTTestServer *pServer) {
  for (int i = 0;
       i < (sizeof(pServer->m_CharArray) / sizeof(pServer->m_CharArray[0]));
       i++)
...
#if defined(SUPPORT_ARRAYS_OF_DATATABLES)
  for (int i = 0; i < 2; i++) {
    for (int z = 0; z < sizeof(pServer->m_SubArray[0].m_FloatArray) /
                            sizeof(pServer->m_SubArray[0].m_FloatArray[0]);
         z++) {
      if (pServer->m_SubArray[i].m_FloatArray[z] !=
          pClient->m_SubArray[i].m_FloatArray[z])
        return false;
    }

    for (int iString = 0;
         iString < sizeof(pServer->m_SubArray[0].m_Strings) /
                       sizeof(pServer->m_SubArray[0].m_Strings[0]);
         iString++) {
      for (z = 0; z < sizeof(pServer->m_SubArray[0].m_Strings[0]) /
                          sizeof(pServer->m_SubArray[0].m_Strings[0][0]);
           z++) {
        if (pServer->m_SubArray[i].m_Strings[iString][z] !=
            pClient->m_SubArray[i].m_Strings[iString][z])
          return false;

        // Check for 0 termination.
        if (pServer->m_SubArray[i].m_Strings[iString][z] == 0) break;
      }
    }
  }
#endif
  ...
  for (int i = 0; i < 2; i++) {
    ...
    for (int iString = 0;
         iString < sizeof(pServer->m_SubArray[0].m_Strings) /
                       sizeof(pServer->m_SubArray[0].m_Strings[0]);
         iString++) {
      int stringLen = sizeof(pServer->m_SubArray[0].m_Strings[0]) /
                      sizeof(pServer->m_SubArray[0].m_Strings[0][0]);
...
bool CompareSub2(DTTestClient *pClient, DTTestServer *pServer) {
  return memcmp(&pClient->m_Sub2, &pServer->m_Sub2, sizeof(pClient->m_Sub2)) ==
         0;
}
...
bool CompareVLA(DTTestClient *pClient, DTTestServer *pServer) {
  if (pClient->m_VLALength != pServer->m_VLALength) return false;

  for (int i = 0; i < pClient->m_VLALength; i++) {
    if (pClient->m_VLA[i] != pServer->m_VLA[i]) return false;
  }
...
bool CompareUtlVectorStruct(DTTestClient *pClient, DTTestServer *pServer) {
  ...
  for (int i = 0; i < c.Count(); i++) {
    if (c[i].a != s[i].a || c[i].b != s[i].b || c[i].f != s[i].f) return false;
  }
  return true;
}
...
int g_GuardOffsets[] = {
    offsetof(DTTestClient, m_Guard1), offsetof(DTTestClient, m_Guard2),
    offsetof(DTTestClient, m_Guard3), offsetof(DTTestClient, m_Guard4),
    offsetof(DTTestClient, m_Guard5), offsetof(DTTestClient, m_Guard6),
    offsetof(DTTestClient, m_Guard7), offsetof(DTTestClient, m_Guard8),
...
bool CompareDTTest(DTTestClient *pClient, DTTestServer *pServer) {
  for (int iVar = 0; iVar < NUMVARTESTINFOS; iVar++) {
    if (!g_VarTestInfos[iVar].m_CompareFn(pClient, pServer)) {
      Assert(
          !"CompareDTTest: comparison failed. There is a new datatable bug.");
      return false;
    }
  }
  return true;
...
bool WriteSendTable_R(SendTable *pTable, bf_write &bfWrite,
                      bool bNeedsDecoder) {
  if (pTable->GetWriteFlag()) return true;
  ...
  if (!SendTable_WriteInfos(pTable, &bfWrite)) return false;
...
void RunDataTableTest() {
  ...
  // Now loop around, changing the data a little bit each time and send/recv
  // deltas.
  int nIterations = 25;
  for (int iIteration = 0; iIteration < nIterations; iIteration++) {
    ...
    if (iIteration == 0) {
      ...
      // Figure out the delta between the newly encoded one and the previously
      // encoded one.
      int deltaProps[MAX_DATATABLE_PROPS];

      bf_read fullEncodedRead("RunDataTableTest->fullEncodedRead", fullEncoded,
                              sizeof(fullEncoded),
      ...
      int nDeltaProps =
          SendTable_CalcDelta(pSendTable, prevEncoded, sizeof(prevEncoded) * 8,
                              ...
                              deltaProps, ARRAYSIZE(deltaProps), -1);

      Assert(nDeltaProps != -1);  // BAD: buffer overflow

      // Reencode with just the delta. This is what is actually sent to the
      // client.
      SendTable_WritePropList(pSendTable, fullEncoded,
                              bfFullEncoded.GetNumBitsWritten(),
                              &bfDeltaEncoded, -1111, deltaProps, nDeltaProps);
    }

    memcpy(prevEncoded, fullEncoded, sizeof(prevEncoded));

    // This step isn't necessary to have the client decode the data but it's
    // here to test RecvTable_CopyEncoding (and RecvTable_MergeDeltas). This
    // call should just make an exact copy of the encoded data.
    unsigned char copyEncoded[4096];
    bf_read bfReadDeltaEncoded("RunDataTableTest->bfReadDeltaEncoded",
                               deltaEncoded, sizeof(deltaEncoded));
    ...
    // Verify that only the changed properties were sent and that they were
    // received correctly.
