...
abstract_class IModelLoader {
 public:
  enum REFERENCETYPE {
    ...
    FMODELLOADER_REFERENCEMASK =
        (FMODELLOADER_SERVER | FMODELLOADER_CLIENT | FMODELLOADER_CLIENTDLL |
         FMODELLOADER_STATICPROP | FMODELLOADER_DETAILPROP),
  ...
  // reference flags
  virtual model_t *GetModelForName(const char *name,
  ...
  // On the client only, there is some information that is computed at the time
  // we are just
  //  about to render the map the first time.  If we don't change/unload the
  //  map, then we shouldn't have to recompute it each time we reconnect to the
  //  same map
  virtual bool Map_GetRenderInfoAllocated(void) = 0;
  virtual void Map_SetRenderInfoAllocated(bool allocated) = 0;

  // Load all the displacements for rendering. Set bRestoring to true if we're
  // recovering from an alt+tab.
...
class CMapLoadHelper {
 public:
  ...
  static void InitFromMemory(model_t *pMapModel, const void *pData,
                             int nDataSize);
