...
abstract_class IRender {
 public:
  ...
  virtual void ViewDrawFade(uint8_t * color, IMaterial * pFadeMaterial) = 0;
  ...
  virtual void BuildWorldLists(
      IWorldRenderList * pList, WorldListInfo_t * pInfo, int iForceViewLeaf,
      const VisOverrideData_t *pVisData, bool bShadowDepth,
      float *pReflectionWaterHeight) = 0;
  ...
  virtual void UpdateBrushModelLightmap(model_t * model,
                                        IClientRenderable * Renderable) = 0;
