...
float ComputeLightRadius(dworldlight_t *pLight, bool bIsHDR) {
  ...
  if (flLightRadius == 0.0f) {
    ...
    // This keeps the behavior of the cutoff radius consistent between LDR and
    // HDR
    float minLightValue =
        bIsHDR ? (LIGHT_MIN_LIGHT_VALUE * 0.5f) : LIGHT_MIN_LIGHT_VALUE;
    ...
    if (pLight->quadratic_attn == 0.0f) {
      if (pLight->linear_attn == 0.0f) {
        ...
        flLightRadius = (flIntensity / minLightValue - pLight->constant_attn) /
                        pLight->linear_attn;
      }
    } else {
...
static void DrawLightSprite(dworldlight_t *pLight, float angleAttenFactor) {
  ...
  /*
          ...
          up *= 1.0f / sqrt( pLight->constant_attn + dist * pLight->linear_attn
     + dist * dist * pLight->quadratic_attn ); right *= 1.0f / sqrt(
     pLight->constant_attn + dist * pLight->linear_attn + dist * dist *
     pLight->quadratic_attn );
  */

  //	float distFactor = 1.0f / ( pLight->constant_attn + dist *
  // pLight->linear_attn + dist * dist * pLight->quadratic_attn ); float
  // distFactor = 1.0f;
...
void DrawSpotLight(dworldlight_t *pLight) {
  ...
  nMinIndex =
      fabs(pLight->normal[nMinIndex]) < fabs(pLight->normal[2]) ? nMinIndex : 2;
...
void DrawLightDebuggingInfo(void) {
  ...
  for (i = 0; i < host_state.worldbrush->numworldlights; i++) {
    if ((nLight > 0) && (i != nLight - 1)) continue;
