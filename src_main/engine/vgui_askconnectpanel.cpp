...
class CAskConnectPanel : public EditablePanel {
 ...
 private:
  ...
  int m_HostNameLabelRightSidePadding;  // Grow the whole panel to make sure
                                        // there's this much padding on the
                                        // right of the hostname label.
...
void CAskConnectPanel::SetHostName(const char *pHostName) {
  ...
  g_pVGuiLocalize->ConstructString(
      wcMessage, sizeof(wcMessage),
      g_pVGuiLocalize->Find("#Valve_ServerOfferingToConnect"), 0);
...
void CAskConnectPanel::StartSlideAnimation(float flDuration) {
  ...
  if (pKeyName) {
    ...
    g_pVGuiLocalize->ConstructString(
        wcMessage, sizeof(wcMessage),
        g_pVGuiLocalize->Find("#Valve_PressKeyToAccept"), 1, wcKeyName);
...
void CAskConnectPanel::UpdateCurrentPosition() {
  ...
  if (g_PluginManager) g_PluginManager->GetHudMessagePosition(x, y, wide, tall);
