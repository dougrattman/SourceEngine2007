...
void DevShotGenerator_Usage() {
  ...
  Msg("  [ -usedevshotsfile filename ] -- get map list from specified file, "
  ...
  Msg("  [ -condebug ] -- prepend console.log entries with mapname or engine "
      "if not in a map\n");
  Msg("  [ +map mapname ] -- generate devshots for specified map and exit "
      "after that map\n");
}
...
CDevShotGenerator &DevShotGenerator() { return g_DevShotGenerator; }

void CL_DevShots_NextMap() { DevShotGenerator().NextMap(); }

static ConCommand devshots_nextmap("devshots_nextmap", CL_DevShots_NextMap,
                                   "Used by the devshots system to go to the "
                                   "next map in the devshots maplist.");
...
void CDevShotGenerator::BuildMapList() {
  if (!m_bDevShotsEnabled) return;

  DevShotGenerator_Usage();

  // Get the maplist file, if any
  const ch *pMapFile = NULL;
  CommandLine()->CheckParm("-usedevshotsfile", &pMapFile);

  // Build the map list
  if (!BuildGeneralMapList(&m_Maps,
                           CommandLine()->FindParm("-usedevshotsfile") != 0,
                           pMapFile, "devshots", &m_iCurrentMap)) {
    m_bDevShotsEnabled = false;
  }
}
