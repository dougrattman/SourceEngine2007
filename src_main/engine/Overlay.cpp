...
struct overlayvert_t {
  ...
  Vector normal;
  Vector2D
      texCoord[NUM_OVERLAY_TEXCOORDS];  // texcoord 0 = the mapped tex coord
                                        // from worldcraft texcoord 1 is used
                                        // for alpha and maps the whole texture
                                        // into the whole overlay
  float lightCoord[2];
  float flAlpha;
...
struct moverlay_t {
  ...
  void *m_pBindProxy;    // client renderable for an overlay's material proxy to
                         // bind to
  float m_flFadeDistMinSq;   // Distance from the overlay's origin at which we
                             // start fading (-1 = use max dist)
  float m_flFadeDistMaxSq;   // Distance from the overlay's origin at which we
                             // fade out completely
  float m_flInvFadeRangeSq;  // Precomputed 1.0f / ( m_flFadeDistMaxSq -
                             // m_flFadeDistMinSq )
};

// Going away!
void Overlay_BuildBasisOrigin(Vector &vecBasisOrigin, SurfaceHandle_t surfID);
void Overlay_BuildBasis(const Vector &vecBasisNormal, Vector &vecBasisU,
                        Vector &vecBasisV, bool bFlip);
void Overlay_OverlayUVToOverlayPlane(const Vector &vecBasisOrigin,
                                     const Vector &vecBasisU,
                                     const Vector &vecBasisV,
                                     const Vector &vecUVPoint,
                                     Vector &vecPlanePoint);
void Overlay_WorldToOverlayPlane(const Vector &vecBasisOrigin,
                                 const Vector &vecBasisNormal,
                                 const Vector &vecWorldPoint,
                                 Vector &vecPlanePoint);
void Overlay_OverlayPlaneToWorld(const Vector &vecBasisNormal,
                                 SurfaceHandle_t surfID,
                                 const Vector &vecPlanePoint,
                                 Vector &vecWorldPoint);
void Overlay_DispUVToWorld(CDispInfo *pDisp, CMeshReader *pReader,
                           const Vector2D &vecUV, Vector &vecWorld,
...
class COverlayMgr : public IOverlayMgr {
 ...
 public:
  ...
  virtual void AddFragmentListToRenderList(int nSortGroup,
                                           OverlayFragmentHandle_t iFragment,
                                           bool bDisp);
 ...
 private:
  ...
  bool Surf_PreClipFragment(moverlay_t *pOverlay,
                            moverlayfragment_t &overlayFrag,
                            SurfaceHandle_t surfID,
                            moverlayfragment_t &surfaceFrag);
  void Surf_PostClipFragment(moverlay_t *pOverlay,
                             moverlayfragment_t &overlayFrag,
                             SurfaceHandle_t surfID);
  void Surf_ClipFragment(moverlay_t *pOverlay, moverlayfragment_t &overlayFrag,
                         SurfaceHandle_t surfID,
                         moverlayfragment_t &surfaceFrag);

  // Displacements
  void Disp_CreateFragments(moverlay_t *pOverlay, SurfaceHandle_t surfID);
  bool Disp_PreClipFragment(moverlay_t *pOverlay,
                            OverlayFragmentVector_t &aDispFragments,
                            SurfaceHandle_t surfID);
  void Disp_PostClipFragment(CDispInfo *pDisp, CMeshReader *pReader,
                             moverlay_t *pOverlay,
                             OverlayFragmentVector_t &aDispFragments,
                             SurfaceHandle_t surfID);
  void Disp_ClipFragment(CDispInfo *pDisp,
                         OverlayFragmentVector_t &aDispFragments);
  void Disp_DoClip(CDispInfo *pDisp, OverlayFragmentVector_t &aCurrentFragments,
  ...
  void BuildClipPlanes(SurfaceHandle_t surfID, moverlayfragment_t &surfaceFrag,
                       const Vector &vecBasisNormal,
                       CUtlVector<cplane_t> &m_ClipPlanes);
  void DoClipFragment(moverlayfragment_t *pFragment, cplane_t *pClipPlane,
 ...
 private:
  enum { RENDER_QUEUE_INVALID = 0xFFFF };
...
//-----------------------------------------------------------------------------
inline moverlayfragment_t *COverlayMgr::GetOverlayFragment(
    OverlayFragmentHandle_t iFragment) {
...
int COverlayMgr::GetMaterialSortID(IMaterial *pMaterial, int nLightmapPage) {
  ...
  for (i = m_RenderQueue.Head(); i != m_RenderQueue.InvalidIndex();
    ...
    if ((m_RenderQueue[i].m_pMaterial == pMaterial) &&
        (m_RenderQueue[i].m_nLightmapPage == nLightmapPage)) {
...
bool COverlayMgr::FadeOverlayFragmentGlobal(moverlayfragment_t *pFragment) {
  ...
  for (iVert = 0; iVert < nVertexCount; ++iVert) {
    Vector vecSegment;
    VectorSubtract(MainViewOrigin(), pFragment->m_aPrimVerts.Element(iVert).pos,
                   vecSegment);
...
//-----------------------------------------------------------------------------
bool COverlayMgr::FadeOverlayFragment(moverlay_t *pOverlay,
...
//-----------------------------------------------------------------------------
// Adds the fragment list to the list of fragments to render when RenderOverlays
// is called
//-----------------------------------------------------------------------------
void COverlayMgr::AddFragmentListToRenderList(int nSortGroup,
                                              OverlayFragmentHandle_t iFragment,
                                              bool bDisp) {
  ...
  for (i = iFragment; i != OVERLAY_FRAGMENT_INVALID; i = m_aFragments.Next(i)) {
    ...
    if (r_overlayfadeenable.GetBool()) {
      ...
      if (!FadeOverlayFragmentGlobal(pFragment)) continue;
    } else if (pOverlay->m_flFadeDistMaxSq > 0) {
      // Fade using per-overlay fade distances, configured by the level
      // designer.
      if (!FadeOverlayFragment(pOverlay, pFragment)) continue;
    }
    ...
    // because they are stored in the list in *reverse* order in which they
    // should be rendered.
...
void COverlayMgr::ClearRenderLists(int nSortGroup) {
  ...
  for (int i = m_nFirstRenderQueue[nSortGroup]; i != RENDER_QUEUE_INVALID;
       i = nNextRenderQueue) {
  ...
  while (iCurrentRenderOrder <= iHighestRenderOrder) {
    ...
    for (int i = m_nFirstRenderQueue[nSortGroup]; i != RENDER_QUEUE_INVALID;
      ...
      while (hStartFragment != OVERLAY_FRAGMENT_INVALID) {
        void *pCurrentBindProxy =
            m_aOverlays[m_aFragments[hStartFragment].m_iOverlay].m_pBindProxy;
        ...
        // We just need to make sure there's a unique sort ID for that. Then we
        // bind once per queue
        OverlayFragmentHandle_t hFragment = hStartFragment;
        hStartFragment = OVERLAY_FRAGMENT_INVALID;

        for (; hFragment != OVERLAY_FRAGMENT_INVALID;
             hFragment = m_aFragments[hFragment].m_hNextRender) {
          ...
          if (pOverlay->m_pBindProxy != pCurrentBindProxy) {
            ...
            if (hStartFragment == OVERLAY_FRAGMENT_INVALID) {
              // Start at the first different bind proxy when we rerun the
              // fragment list
          ...
          int iVert;
          if (!pMesh)  // have we output any vertices yet? if first verts, init
                       // material and meshbuilder
          {
            if (!bWireframeFragments) {
              pRenderContext->Bind(renderQueueHead.m_pMaterial,
                                   pOverlay->m_pBindProxy /*proxy*/);
              pRenderContext->BindLightmapPage(renderQueueHead.m_nLightmapPage);
              bLightmappedMaterial =
                  renderQueueHead.m_pMaterial->GetPropertyFlag(
            ...
            meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES,
                              renderQueue.m_nVertexCount,
                              renderQueue.m_nIndexCount);
          }
...
void COverlayMgr::SetOverlayBindProxy(int iOverlayID, void *pBindProxy) {
  ...
  if (pOverlay) pOverlay->m_pBindProxy = pBindProxy;
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool COverlayMgr::Surf_PreClipFragment(moverlay_t *pOverlay,
                                       moverlayfragment_t &overlayFrag,
                                       SurfaceHandle_t surfID,
                                       moverlayfragment_t &surfaceFrag) {
  MEM_ALLOC_CREDIT();
  ...
  for (int iVert = 0; iVert < 4; ++iVert) {
    Overlay_OverlayUVToOverlayPlane(
        pOverlay->m_vecOrigin, pOverlay->m_vecBasis[0], pOverlay->m_vecBasis[1],
        pOverlay->m_vecUVPoints[iVert], overlayFrag.m_aPrimVerts[iVert].pos);
  }
  ...
  for (int iVert = 0; iVert < nVertCount; ++iVert) {
    ...
    Overlay_WorldToOverlayPlane(pOverlay->m_vecOrigin, pOverlay->m_vecBasis[2],
                                surfaceFrag.m_aPrimVerts[iVert].pos,
                                surfaceFrag.m_aPrimVerts[iVert].pos);
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::Surf_PostClipFragment(moverlay_t *pOverlay,
                                        moverlayfragment_t &overlayFrag,
                                        SurfaceHandle_t surfID) {
#ifndef SWDS
  ...
  for (int iPoint = 0; iPoint < 4; ++iPoint) {
    Overlay_OverlayUVToOverlayPlane(
        pOverlay->m_vecOrigin, pOverlay->m_vecBasis[0], pOverlay->m_vecBasis[1],
        pOverlay->m_vecUVPoints[iPoint], origOverlay.m_aPrimVerts[iPoint].pos);
  }
  ...
  for (int iVert = 0; iVert < nVertCount; ++iVert) {
    ...
    PointInQuadToBarycentric(
        origOverlay.m_aPrimVerts[0].pos, origOverlay.m_aPrimVerts[3].pos,
        origOverlay.m_aPrimVerts[2].pos, origOverlay.m_aPrimVerts[1].pos,
        overlayFrag.m_aPrimVerts[iVert].pos, vecUV);

    overlayvert_t &overlay_vertex{pFragment->m_aPrimVerts[iVert]};
    ...
    for (int iTexCoord = 0; iTexCoord < NUM_OVERLAY_TEXCOORDS; iTexCoord++) {
      TexCoordInQuadFromBarycentric(
          ...
          origOverlay.m_aPrimVerts[2].texCoord[iTexCoord],
          origOverlay.m_aPrimVerts[1].texCoord[iTexCoord], vecUV, vecTexCoord);

      overlay_vertex.texCoord[iTexCoord][0] = vecTexCoord.x;
    ...
    Vector2D uv;
    SurfComputeLightmapCoordinate(ctx, surfID, overlay_vertex.pos, uv);
    overlay_vertex.lightCoord[0] = uv.x;
    overlay_vertex.lightCoord[1] = uv.y;

    // Push -just- off the surface to avoid z-clipping errors.
    overlay_vertex.pos += vNormal * OVERLAY_AVOID_FLICKER_NORMAL_OFFSET;
  }

  // Create the sort ID for this fragment
  const MaterialSystem_SortInfo_t &sortInfo =
      materialSortInfoArray[MSurf_MaterialSortID(surfID)];
  mtexinfo_t *pTexInfo = &host_state.worldbrush->texinfo[pOverlay->m_nTexInfo];
  pFragment->m_nMaterialSortID =
      GetMaterialSortID(pTexInfo->material, sortInfo.lightmapPageID);
  ...
  // rendering, we can do *that* in reverse order too? Reduces the amount of
  // iteration necessary Therefore, we need to add to the head of the list
...
//-----------------------------------------------------------------------------
void COverlayMgr::Surf_ClipFragment(moverlay_t *pOverlay,
                                    moverlayfragment_t &overlayFrag,
                                    SurfaceHandle_t surfID,
                                    moverlayfragment_t &surfaceFrag) {
  MEM_ALLOC_CREDIT();
  ...
  if (pClippedFrag) {
    ...
    for (int iVert = 0; iVert < pClippedFrag->m_aPrimVerts.Count(); ++iVert) {
      overlayvert_t &overlay_vertex{overlayFrag.m_aPrimVerts[iVert]};
      overlay_vertex.pos = pClippedFrag->m_aPrimVerts[iVert].pos;
      memcpy(overlay_vertex.texCoord,
             pClippedFrag->m_aPrimVerts[iVert].texCoord,
             sizeof(overlay_vertex.texCoord));
    }
  }
...
inline float TriangleArea(const Vector &v0, const Vector &v1,
                          const Vector &v2) {
...
//-----------------------------------------------------------------------------
void COverlayMgr::Surf_CreateFragments(moverlay_t *pOverlay,
                                       SurfaceHandle_t surfID) {
  ...
  for (int iTri = 0; iTri < nSurfTriangleCount; ++iTri) {
    ...
    if (TriangleArea(surfaceFrag.m_aPrimVerts[0].pos,
                     surfaceFrag.m_aPrimVerts[1].pos,
                     surfaceFrag.m_aPrimVerts[2].pos) > 1.0f) {
...
void COverlayMgr::CreateFragments(void) {
  ...
  for (int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay) {
    ...
    if (nFaceCount == 0) continue;

    // Build the overlay basis.
    bool bFlip = (pOverlay->m_vecUVPoints[3].z == 1.0f);
    pOverlay->m_vecUVPoints[3].z = 0.0f;
    Overlay_BuildBasis(pOverlay->m_vecBasis[2], pOverlay->m_vecBasis[0],
                       pOverlay->m_vecBasis[1], bFlip);
    ...
    while (hFrag != OVERLAY_FRAGMENT_INVALID) {
      ...
      for (int iVert = 0; iVert < nVertCount; ++iVert) {
        ...
        if (!pVert->pos.IsValid()) {
          Assert(0);
          mtexinfo_t *pTexInfo =
              &host_state.worldbrush->texinfo[pOverlay->m_nTexInfo];
          DevMsg(
              1, "Bad overlay vert - %d at (%f, %f, %f) with material '%s'\n",
              iOverlay, pOverlay->m_vecOrigin.x, pOverlay->m_vecOrigin.y,
        ...
        if (!pVert->normal.IsValid()) {
          ...
          mtexinfo_t *pTexInfo =
              &host_state.worldbrush->texinfo[pOverlay->m_nTexInfo];
          DevMsg(
              1, "Bad overlay normal - %d at (%f, %f, %f) with material '%s'\n",
              iOverlay, pOverlay->m_vecOrigin.x, pOverlay->m_vecOrigin.y,
              pOverlay->m_vecOrigin.z,
              (pTexInfo && pTexInfo->material) ? pTexInfo->material->GetName()
                                               : "");
        ...
        if (!pVert->texCoord[0].IsValid() || !pVert->texCoord[1].IsValid()) {
          ...
          mtexinfo_t *pTexInfo =
              &host_state.worldbrush->texinfo[pOverlay->m_nTexInfo];
          DevMsg(1,
                 "Bad overlay texture coords - %d at (%f, %f, %f) with "
                 "material '%s'\n",
                 iOverlay, pOverlay->m_vecOrigin.x, pOverlay->m_vecOrigin.y,
                 pOverlay->m_vecOrigin.z,
                 (pTexInfo && pTexInfo->material)
                     ? pTexInfo->material->GetName()
                     : "");
        }
      }
      hFrag = m_OverlayFragments.Next(hFrag);
...
void COverlayMgr::ReSortMaterials(void) {
  ...
  for (int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay) {
    moverlay_t *pOverlay = &m_aOverlays.Element(iOverlay);
    if (!pOverlay) continue;

    mtexinfo_t *pTexInfo =
        &host_state.worldbrush->texinfo[pOverlay->m_nTexInfo];
    if (!pTexInfo) continue;
    ...
    while (hFrag != OVERLAY_FRAGMENT_INVALID) {
      ...
      if (pFrag) {
        const MaterialSystem_SortInfo_t &sortInfo =
            materialSortInfoArray[MSurf_MaterialSortID(pFrag->m_SurfId)];
        pFrag->m_nMaterialSortID =
            GetMaterialSortID(pTexInfo->material, sortInfo.lightmapPageID);
        ...
        int nVertCount = pFrag->m_aPrimVerts.Count();
        for (int iVert = 0; iVert < nVertCount; ++iVert) {
          overlayvert_t &overlay_vertex{pFrag->m_aPrimVerts[iVert]};
          // Lightmap coordinates.
          Vector2D uv;
          SurfComputeLightmapCoordinate(ctx, pFrag->m_SurfId,
                                        overlay_vertex.pos, uv);
          overlay_vertex.lightCoord[0] = uv.x;
          overlay_vertex.lightCoord[1] = uv.y;
        }
      }
...
bool COverlayMgr::LoadOverlays() {
  ...
  if (lh.LumpSize() % sizeof(doverlay_t)) return false;

  pWaterOverlayIn = (dwateroverlay_t *)lh2.LumpBase();
  if (lh2.LumpSize() % sizeof(dwateroverlay_t)) return false;

  // Fade distances are in a parallel lump
  doverlayfade_t *pOverlayFadesIn = (doverlayfade_t *)lhOverlayFades.LumpBase();
  if (lhOverlayFades.LumpSize() % sizeof(doverlayfade_t)) return false;
  ...
  for (int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay, ++pOverlayIn) {
    ...
    if (pOverlayOut->m_nRenderOrder >= OVERLAY_NUM_RENDER_ORDERS)
      Error(
          "COverlayMgr::LoadOverlays: invalid render order (%d) for an "
          "overlay.",
          pOverlayOut->m_nRenderOrder);
    ...
    if (pOverlayFadesIn) {
      ...
      pOverlayOut->m_flInvFadeRangeSq =
          1.0f /
          (pOverlayFadesIn->flFadeDistMaxSq - pOverlayFadesIn->flFadeDistMinSq);
    ...
    if (pOverlayOut->m_vecBasis[0].x == 0.0f &&
        pOverlayOut->m_vecBasis[0].y == 0.0f &&
        pOverlayOut->m_vecBasis[0].z == 0.0f) {
      Warning("Bad overlay basis at (%f %f %f)!\n", pOverlayOut->m_vecOrigin.x,
              pOverlayOut->m_vecOrigin.y, pOverlayOut->m_vecOrigin.z);
    }

    CrossProduct(pOverlayOut->m_vecBasis[2], pOverlayOut->m_vecBasis[0],
                 pOverlayOut->m_vecBasis[1]);
    ...
    for (int iFace = 0; iFace < pOverlayIn->GetFaceCount(); ++iFace) {
      pOverlayOut->m_aFaces[iFace] =
          SurfaceHandleFromIndex(pOverlayIn->aFaces[iFace], lh.GetMap());
    }
  ...
  for (int iWaterOverlay = 0; iWaterOverlay < nWaterOverlayCount;
       ++iWaterOverlay, ++pWaterOverlayIn) {
    moverlay_t *pOverlayOut =
        &m_aOverlays.Element(nOverlayCount + iWaterOverlay);
    ...
    for (int iFace = 0; iFace < pWaterOverlayIn->GetFaceCount(); ++iFace) {
      pOverlayOut->m_aFaces[iFace] =
          SurfaceHandleFromIndex(pWaterOverlayIn->aFaces[iFace], lh2.GetMap());
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void COverlayMgr::Disp_CreateFragments(moverlay_t *pOverlay,
                                       SurfaceHandle_t surfID) {
  OverlayFragmentVector_t aDispFragments;
  ...
  if (Disp_PreClipFragment(pOverlay, aDispFragments, surfID)) {
    ...
    if (pDisp) {
      ...
      Disp_PostClipFragment(pDisp, &pDisp->m_MeshReader, pOverlay,
                            aDispFragments, surfID);
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
bool COverlayMgr::Disp_PreClipFragment(moverlay_t *pOverlay,
                                       OverlayFragmentVector_t &aDispFragments,
                                       SurfaceHandle_t surfID) {
  MEM_ALLOC_CREDIT();
  ...
  for (int iVert = 0; iVert < nVertCount; ++iVert) {
    PointInQuadToBarycentric(
        surfaceFrag.m_aPrimVerts[iPointStart].pos,
        ...
        overlayFrag.m_aPrimVerts[iVert].pos, vecTmpUV);
    if (!vecTmpUV.IsValid()) {
      mtexinfo_t *pTexInfo =
          &host_state.worldbrush->texinfo[pOverlay->m_nTexInfo];
    ...
    Vector &overlay_vertex_pos{pFragment->m_aPrimVerts[iVert].pos};

    overlay_vertex_pos.x = vecTmpUV.x;
    overlay_vertex_pos.y = vecTmpUV.y;
    overlay_vertex_pos.z = 0.0f;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void COverlayMgr::Disp_PostClipFragment(CDispInfo *pDisp, CMeshReader *pReader,
                                        moverlay_t *pOverlay,
                                        OverlayFragmentVector_t &aDispFragments,
                                        SurfaceHandle_t surfID) {
#ifndef SWDS
  ...
  pReader->TexCoord2f(0, DISP_LMCOORDS_STAGE, lightCoords[0].x,
                      lightCoords[0].y);
  pReader->TexCoord2f(nInterval - 1, DISP_LMCOORDS_STAGE, lightCoords[1].x,
                      lightCoords[1].y);
  pReader->TexCoord2f((nInterval * nInterval) - 1, DISP_LMCOORDS_STAGE,
                      lightCoords[2].x, lightCoords[2].y);
  pReader->TexCoord2f(nInterval * (nInterval - 1), DISP_LMCOORDS_STAGE,
                      lightCoords[3].x, lightCoords[3].y);
  ...
  for (int iFrag = 0; iFrag < nFragCount; ++iFrag) {
    ...
    if (!pDispFragment) continue;

    int nVertCount = pDispFragment->m_aPrimVerts.Count();
    if (nVertCount < 3) continue;
    ...
    for (int iVert = 0; iVert < nVertCount; ++iVert) {
      ...
      // Texture coordinates.
      pFragment->m_aPrimVerts[iVert].texCoord[0] =
          pDispFragment->m_aPrimVerts[iVert].texCoord[0];
      pFragment->m_aPrimVerts[iVert].texCoord[1] =
          pDispFragment->m_aPrimVerts[iVert].texCoord[1];

      // Lightmap coordinates.
      Vector2D uv;
      TexCoordInQuadFromBarycentric(lightCoords[0], lightCoords[1],
                                    lightCoords[2], lightCoords[3], vecTmpUV,
                                    uv);
    ...
    Vector vecEdges[2];
    VectorSubtract(pFragment->m_aPrimVerts[1].pos,
                   pFragment->m_aPrimVerts[0].pos, vecEdges[0]);
    VectorSubtract(pFragment->m_aPrimVerts[2].pos,
                   pFragment->m_aPrimVerts[0].pos, vecEdges[1]);
...
// Purpose:
//-----------------------------------------------------------------------------
void COverlayMgr::Disp_ClipFragment(CDispInfo *pDisp,
                                    OverlayFragmentVector_t &aDispFragments) {
  cplane_t clipPlane;
  ...
  Disp_DoClip(pDisp, aDispFragments, clipPlane, 1.0f, nInterval, 1, nInterval,
              1);
  ...
  Disp_DoClip(pDisp, aDispFragments, clipPlane, 0.707f, nInterval, 2,
              (nInterval * 2 - 1), 2);

  // Displacement-space clipping UV from bottom-left to top-right.
  clipPlane.normal.Init(-0.707f, 0.707f, 0.0f);  // 135 degrees
  Disp_DoClip(pDisp, aDispFragments, clipPlane, 0.707f, nInterval,
              -(nInterval - 2), (nInterval - 1), 2);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::Disp_DoClip(CDispInfo *pDisp,
                              OverlayFragmentVector_t &aDispFragments,
                              cplane_t &clipPlane, float clipDistStart,
                              int nInterval, int nLoopStart, int nLoopEnd,
                              int nLoopInc) {
  ...
  for (int iInterval = nLoopStart; iInterval < nLoopEnd;
       iInterval += nLoopInc) {
...
// Purpose:
//-----------------------------------------------------------------------------
void COverlayMgr::InitTexCoords(moverlay_t *pOverlay,
                                moverlayfragment_t &overlayFrag) {
  // Overlay texture coordinates.
  overlayFrag.m_aPrimVerts[0].texCoord[0].Init(pOverlay->m_flU[0],
                                               pOverlay->m_flV[0]);
  overlayFrag.m_aPrimVerts[1].texCoord[0].Init(pOverlay->m_flU[0],
                                               pOverlay->m_flV[1]);
  overlayFrag.m_aPrimVerts[2].texCoord[0].Init(pOverlay->m_flU[1],
                                               pOverlay->m_flV[1]);
  overlayFrag.m_aPrimVerts[3].texCoord[0].Init(pOverlay->m_flU[1],
                                               pOverlay->m_flV[0]);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::DoClipFragment(moverlayfragment_t *pFragment,
                                 cplane_t *pClipPlane,
                                 moverlayfragment_t **ppFront,
                                 moverlayfragment_t **ppBack) {
  ...
  for (; iVert < pFragment->m_aPrimVerts.Count(); ++iVert) {
    flDists[iVert] =
        pClipPlane->normal.Dot(pFragment->m_aPrimVerts[iVert].pos) -
        pClipPlane->dist;
  ...
  for (iVert = 0; iVert < nVertCount; ++iVert) {
    ...
    vert.pos =
        ...
        fraction * (pFragment->m_aPrimVerts[(iVert + 1) % nVertCount].pos -
                    pFragment->m_aPrimVerts[iVert].pos);
    for (int iTexCoord = 0; iTexCoord < NUM_OVERLAY_TEXCOORDS; iTexCoord++) {
      vert.texCoord[iTexCoord][0] =
          pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][0] +
          fraction * (pFragment->m_aPrimVerts[(iVert + 1) % nVertCount]
                          .texCoord[iTexCoord][0] -
                      pFragment->m_aPrimVerts[iVert].texCoord[iTexCoord][0]);
      vert.texCoord[iTexCoord][1] =
...
// Copies a fragment into the main fragment list
//-----------------------------------------------------------------------------
OverlayFragmentHandle_t COverlayMgr::AddFragmentToFragmentList(
    moverlayfragment_t *pSrc) {
  MEM_ALLOC_CREDIT();
  ...
  frag.m_aPrimVerts.CopyArray(pSrc->m_aPrimVerts.Base(),
                              pSrc->m_aPrimVerts.Count());
...
moverlayfragment_t *COverlayMgr::CopyTempFragment(moverlayfragment_t *pSrc) {
  ...
  if (pDst) {
    ...
    pDst->m_aPrimVerts.CopyArray(pSrc->m_aPrimVerts.Base(),
                                 pSrc->m_aPrimVerts.Count());
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void COverlayMgr::BuildClipPlanes(SurfaceHandle_t surfID,
                                  moverlayfragment_t &surfaceFrag,
                                  const Vector &vecBasisNormal,
                                  CUtlVector<cplane_t> &m_ClipPlanes) {
  int nVertCount = surfaceFrag.m_aPrimVerts.Count();
  for (int iVert = 0; iVert < nVertCount; ++iVert) {
    Vector vecEdge;
    vecEdge = surfaceFrag.m_aPrimVerts[(iVert + 1) % nVertCount].pos -
              surfaceFrag.m_aPrimVerts[iVert].pos;
    ...
    float flDistance =
        pPlane->normal.Dot(
            surfaceFrag.m_aPrimVerts[(iVert + 2) % nVertCount].pos) -
        pPlane->dist;
    if (flDistance > 0.0) {
      // Flip
...
void Overlay_BuildBasisOrigin(Vector &vecBasisOrigin, SurfaceHandle_t surfID) {
  ...
  // Move the basis origin to the position of the entity projected into the face
  // plane.
...
bool Overlay_IsBasisFlipped(int *pFlip, int iAxis, int iComponent) {
  if (iAxis < 0 || iAxis > 2 || iComponent < 0 || iComponent > 2) return false;
...
void Overlay_BuildBasis(const Vector &vecBasisNormal, Vector &vecBasisU,
  ...
  if (!vecBasisNormal.IsValid()) return;
...
// Purpose:
//-----------------------------------------------------------------------------
void Overlay_TriTLToBR(CDispInfo *pDisp, CMeshReader *pReader, Vector &vecWorld,
                       float flU, float flV, int nWidth,
  ...
  if ((flFracU + flFracV) >= (1.0f + TRIEDGE_EPSILON)) {
    ...
    if (nSnapU == nNextU) {
      if (nSnapV == nNextV) {
        ...
        float flFrac = (vecIntersectPoint - vecFlatVerts[0]).Length() /
      ...
      float flCfs[3];
      if (CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1],
                                vecFlatVerts[2], vecIntersectPoint, flCfs[0],
                                flCfs[1], flCfs[2])) {
        vecWorld = (vecVerts[0] * flCfs[0]) + (vecVerts[1] * flCfs[1]) +
                   (vecVerts[2] * flCfs[2]);
      } else {
        int nIndices[3];
        ...
        if (nSnapU == nNextU) {
          ...
          float flCfs[3];
          CalcBarycentricCooefs(vecFlatVerts[0], vecFlatVerts[1],
                                vecFlatVerts[2], vecIntersectPoint, flCfs[0],
                                flCfs[1], flCfs[2]);
          vecWorld = (vecVerts[0] * flCfs[0]) + (vecVerts[1] * flCfs[1]) +
                     (vecVerts[2] * flCfs[2]);
...
// Purpose:
//-----------------------------------------------------------------------------
void Overlay_TriBLToTR(CDispInfo *pDisp, CMeshReader *pReader, Vector &vecWorld,
                       float flU, float flV, int nWidth,
                       const Vector &vecIntersectPoint) {
  int nHeight = nWidth;
  ...
  // The fractions are not correct all the time - but they are a good first
  // guess!
  Vector vecVerts[3], vecFlatVerts[3];
  if (flFracU < flFracV) {
    int nIndices[3];
...
void Overlay_DispUVToWorld(CDispInfo *pDisp, CMeshReader *pReader,
  ...
  PointInQuadFromBarycentric(
      surfaceFrag.m_aPrimVerts[(0 + pDisp->m_iPointStart) % 4].pos,
      surfaceFrag.m_aPrimVerts[(3 + pDisp->m_iPointStart) % 4].pos,
      surfaceFrag.m_aPrimVerts[(2 + pDisp->m_iPointStart) % 4].pos,
      surfaceFrag.m_aPrimVerts[(1 + pDisp->m_iPointStart) % 4].pos, vecUV,
      vecIntersectPoint);
  ...
  if (bOdd) {
    Overlay_TriTLToBR(pDisp, pReader, vecWorld, flU, flV, nWidth,
                      vecIntersectPoint);
  }
  // Bottom Left to Top Right
  else {
    Overlay_TriBLToTR(pDisp, pReader, vecWorld, flU, flV, nWidth,
                      vecIntersectPoint);
  }
}
...
void Overlay_OverlayPlaneToWorld(const Vector &vecBasisNormal,
  ...
  cplane_t surfacePlane = MSurf_Plane(surfID);
  VectorNormalize(surfacePlane.normal);
  float flDistanceToSurface =
      surfacePlane.normal.Dot(vecPlanePoint) - surfacePlane.dist;
