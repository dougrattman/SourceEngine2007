...
void MapReslistGenerator_Usage() {
  ...
  Msg("  [ -makereslists <optionalscriptfile> ] -- script file to control more "
      "complex makereslists operations (multiple passes, etc.)\n");
  Msg("  [ -usereslistfile filename ] -- get map list from specified file, "
      "default is to build for maps/*.bsp\n");
  Msg("  [ -startmap mapname ] -- restart generation at specified map (after "
      "crash, implies resume)\n");
  Msg("  [ -condebug ] -- prepend console.log entries with mapname or engine "
      "if not in a map\n");
  Msg("  [ +map mapname ] -- generate reslists for specified map and exit "
      "after that map\n");
  Msg("  [ -rebuildaudio ] -- force rebuild of _other_rebuild.cache "
      "(metacache) file at exit\n");
  Msg("  [ -forever ] -- when you get to the end of the maplist, start over "
      "from the top\n");
  Msg("  [ -reslistdir ] -- default is 'reslists', use this to override\n");
  Msg("  [ -startstage nnn ] -- when running from script file, this starts at "
      "specified stage\n");
  Msg("  [ -collate ] -- skip everything, just merge the reslist from temp "
      "folders to the final folder again\n");
}
...
CMapReslistGenerator &MapReslistGenerator() { return g_MapReslistGenerator; }
...
void CMapReslistGenerator::BuildMapList() {
  ...
  bool bUseMapListFile =
      bUseMap ? false : CommandLine()->FindParm("-usereslistfile") != 0;

  // Build the map list
  if (!BuildGeneralMapList(&m_Maps, bUseMapListFile, pMapFile, "reslists",
                           &m_iCurrentMap)) {
    m_bLoggingEnabled = false;
  }
}

bool BuildGeneralMapList(CUtlVector<maplist_map_t> *aMaps, bool bUseMapListFile,
                         const char *pMapFile, const char *pSystemMsg,
                         int *iCurrentMap) {
  if (!bUseMapListFile) {
...
void CMapReslistGenerator::BuildEngineLogFromReslist() {
  ...
  if (!g_pFileSystem->ReadFile(
          CFmtStr("%s\\%s", m_sResListDir.String(), ENGINE_RESLIST_FILE),
          "DEFAULT_WRITE_PATH", buffer)) {
    // does not exist
    return;
  }
...
void CMapReslistGenerator::LogToEngineReslist(char const *pLine) {
  ...
  FileHandle_t fh = g_pFileSystem->Open(
      CFmtStr("%s\\%s", m_sResListDir.String(), ENGINE_RESLIST_FILE), "at",
      "DEFAULT_WRITE_PATH");
...
void CMapReslistGenerator::EnableReslistGeneration(bool usemaplistfile) {
  // hackhack !!!! This is a work-around until CS precaches things on level
  // start, not player spawn
  ...
  g_pFileSystem->CreateDirHierarchy(m_sResListDir.String(),
                                    "DEFAULT_WRITE_PATH");

  // Leave the existing one if resuming from a specific map, otherwise, blow it
  // away
  if (!CommandLine()->FindParm("-startmap")) {
    g_pFileSystem->RemoveFile(
        CFmtStr("%s\\%s", m_sResListDir.String(), ENGINE_RESLIST_FILE),
        "DEFAULT_WRITE_PATH");
    m_EngineLog.RemoveAll();
  } else {
...
bool CMapReslistGenerator::ShouldRebuildCaches() {
  ...
  if (!CommandLine()->FindParm("-norebuildaudio")) return true;
  return false;
}
...
//-----------------------------------------------------------------------------
// Purpose: call every frame if we're enabled, just so that the next map can be
...
//-----------------------------------------------------------------------------
void CMapReslistGenerator::OnResourcePrecached(
...
void CMapReslistGenerator::OnResourcePrecachedFullPath(
  ...
  if (filename != UTL_INVAL_SYMBOL) return;
...
void CMapReslistGenerator::WriteMapLog() {
  ...
  for (int i = m_MapLog.FirstInorder(); i != m_MapLog.InvalidIndex();
       i = m_MapLog.NextInorder(i)) {
...
//-----------------------------------------------------------------------------
void CMapReslistGenerator::FileSystemLoggingFunc(const char *fullPathFileName,
                                                 const char *options) {
...
void CMapReslistGenerator::EnableDeletionsTracking() {
  ...
  if (FILESYSTEM_INVALID_HANDLE != deletionsfile) {
    ...
    if (length > 0) {
      char *pStart = (char *)new char[length + 1];
      if (pStart &&
          (length == g_pFileSystem->Read(pStart, length, deletionsfile))) {
        pStart[length] = 0;
        const char *pFileList = pStart;
        ...
        while (1) {
          ...
          Q_snprintf(filename, sizeof(filename), "%s/%s", com_gamedir,
                     com_token);
  ...
  FileHandle_t warningsfile =
      g_pFileSystem->Open(DELETIONS_WARNINGS_FILE, "rb");
  if (FILESYSTEM_INVALID_HANDLE != warningsfile) {
    ...
    if (length > 0) {
      char *pStart = (char *)new char[length + 1];
      if (pStart &&
          (length == g_pFileSystem->Read(pStart, length, warningsfile))) {
        pStart[length] = 0;
        const char *pFileList = pStart;
  ...
  Msg("Tracking deletions (%u files in deletion list in '%s', %u previous "
      "warnings loaded from '%s'\n",
...
void CMapReslistGenerator::TrackDeletions(const char *fullPathFileName) {
  ...
  if (UTL_INVAL_SYMBOL != sym) {
    ...
    uint32_t idx = m_DeletionListWarnings.Find(warningSymbol);
    if (idx == m_DeletionListWarnings.InvalidIndex()) {
...
// Purpose: callback function from filesystem
//-----------------------------------------------------------------------------
void CMapReslistGenerator::TrackDeletionsLoggingFunc(
    const char *fullPathFileName, const char *options) {
...
void CMapReslistGenerator::SpewTrackedDeletionsLog() {
  if (!m_bTrackingDeletions) return;

  FileHandle_t hUndeleteFile =
      g_pFileSystem->Open(DELETIONS_WARNINGS_FILE, "wt", "DEFAULT_WRITE_PATH");
  if (FILESYSTEM_INVALID_HANDLE == hUndeleteFile) {
    return;
  }

  for (int i = m_DeletionListWarnings.FirstInorder();
       i != m_DeletionListWarnings.InvalidIndex();
       i = m_DeletionListWarnings.NextInorder(i)) {
    char const *filename =
        m_DeletionListWarningsSymbols.String(m_DeletionListWarnings[i]);
