...
ConVar g_CV_DTWatchClass("dtwatchclass", "", 0,
...
CDeltaBitsWriter::~CDeltaBitsWriter() {
  // Write a final zero bit, signifying that there are no more properties in the
  // buffer.
...
void CDeltaBitsWriter::WritePropIndex(int iProp) {
  ...
  if (g_bTrackProps) {
    ...
    if (diff > g_MaxPropDiff) g_MaxPropDiff = diff;

    if (diff < g_MinPropDiff) g_MinPropDiff = diff;
  }
...
CDeltaBitsReader::~CDeltaBitsReader() {
  // Make sure they read to the end unless they specifically said they don't
  // care.
  if (m_pBuf) {
    Assert(m_bFinished);
  }
...
CSendNode::CSendNode() {
  ...
  m_DataTableProxyIndex =
      DATATABLE_PROXY_INDEX_INVALID;  // set it to a questionable value.
}
...
CSendTablePrecalc::~CSendTablePrecalc() {
  if (m_pSendTable) m_pSendTable->m_pPrecalc = 0;
}

const ExcludeProp *FindExcludeProp(char const *pTableName,
...
static bool SendTable_GetPropsExcluded(const SendTable *pTable,
  ...
  for (int i = 0; i < pTable->m_nProps; i++) {
    ...
    if (pProp->IsExcludeProp()) {
      ...
      ErrorIfNot(pName, ("Found an exclude prop missing a name."));

      ErrorIfNot(
          nExcludeProps < nMaxExcludeProps,
      ...
      if (!SendTable_GetPropsExcluded(pProp->GetDataTable(), pExcludeProps,
                                      nExcludeProps, nMaxExcludeProps))
        return false;
    }
  }
...
static void SetDataTableProxyIndices_R(CSendTablePrecalc *pMainTable,
                                       CSendNode *pCurTable,
  ...
  for (int i = 0; i < pCurTable->GetNumChildren(); i++) {
    ...
    if (pProp->GetFlags() & SPROP_PROXY_ALWAYS_YES) {
      ...
      pMainTable->SetNumDataTableProxies(pMainTable->GetNumDataTableProxies() +
                                         1);
    }
...
// Set the datatable proxy indices in all datatable SendProps.
static void SetRecursiveProxyIndices_R(SendTable *pBaseTable,
                                       CSendNode *pCurTable,
                                       int &iCurProxyIndex) {
...
void SendTable_BuildHierarchy(CSendNode *pNode, const SendTable *pTable,
                              CBuildHierarchyStruct *bhs);

void SendTable_BuildHierarchy_IterateProps(
    CSendNode *pNode, const SendTable *pTable, CBuildHierarchyStruct *bhs,
  ...
  for (i = 0; i < pTable->m_nProps; i++) {
    ...
    if (pProp->IsExcludeProp() || pProp->IsInsideArray() ||
        FindExcludeProp(pTable->GetName(), pProp->GetName(),
                        bhs->m_pExcludeProps, bhs->m_nExcludeProps)) {
      continue;
    }

    if (pProp->GetType() == DPT_DataTable) {
      if (pProp->GetFlags() & SPROP_COLLAPSIBLE) {
        // This is a base class.. no need to make a new CSendNode (and trigger a
        // bunch of unnecessary send proxy calls in the datatable stacks).
        SendTable_BuildHierarchy_IterateProps(pNode, pProp->GetDataTable(), bhs,
                                              pNonDatatableProps,
                                              nNonDatatableProps);
      } else {
      ...
      if (nNonDatatableProps >= MAX_TOTAL_SENDTABLE_PROPS)
        Error(
            "SendTable_BuildHierarchy: overflowed non-datatable props with "
            "'%s'.",
            pProp->GetName());
...
void SendTable_BuildHierarchy(CSendNode *pNode, const SendTable *pTable,
  ...
  SendTable_BuildHierarchy_IterateProps(pNode, pTable, bhs, pNonDatatableProps,
                                        nNonDatatableProps);
...
void CalcPathLengths_R(CSendNode *pNode, CUtlVector<int> &pathLengths,
                       int curPathLength, int &totalPathLengths) {
  pathLengths[pNode->GetRecursiveProxyIndex()] = curPathLength;
  totalPathLengths += curPathLength;

  for (int i = 0; i < pNode->GetNumChildren(); i++) {
    CalcPathLengths_R(pNode->GetChild(i), pathLengths, curPathLength + 1,
                      totalPathLengths);
  }
}

void FillPathEntries_R(CSendTablePrecalc *pPrecalc, CSendNode *pNode,
                       CSendNode *pParent, int &iCurEntry) {
  ...
  if (pParent) {
    CSendTablePrecalc::CProxyPath &parentProxyPath =
        pPrecalc->m_ProxyPaths[pParent->GetRecursiveProxyIndex()];
    outProxyPath.m_nEntries = parentProxyPath.m_nEntries + 1;

    for (int i = 0; i < parentProxyPath.m_nEntries; i++)
      pPrecalc->m_ProxyPathEntries[iCurEntry++] =
          pPrecalc->m_ProxyPathEntries[parentProxyPath.m_iFirstEntry + i];

    // Now add this node's own proxy.
    pPrecalc->m_ProxyPathEntries[iCurEntry].m_iProxy =
        pNode->GetRecursiveProxyIndex();
    pPrecalc->m_ProxyPathEntries[iCurEntry].m_iDatatableProp =
        pNode->m_iDatatableProp;
    ++iCurEntry;
  } else {
...
void SendTable_GenerateProxyPaths(CSendTablePrecalc *pPrecalc,
                                  int nProxyIndices) {
  // Initialize the array.
  pPrecalc->m_ProxyPaths.SetSize(nProxyIndices);
  for (int i = 0; i < nProxyIndices; i++)
    pPrecalc->m_ProxyPaths[i].m_iFirstEntry =
        pPrecalc->m_ProxyPaths[i].m_nEntries = 0xFFFF;
...
bool CSendTablePrecalc::SetupFlatPropertyArray() {
  ...
  // First go through and set SPROP_INSIDEARRAY when appropriate, and set array
  // prop pointers.
  ...
  int nExcludeProps = 0;
  if (!SendTable_GetPropsExcluded(pTable, excludeProps, nExcludeProps,
                                  MAX_EXCLUDE_PROPS))
    return false;
...
bool AreBitArraysEqual(void const *pvBits1, void const *pvBits2, int nBits) {
  ...
  // Compare bytes.
  nBytes = nBits >> 3;
  if (memcmp(pBits1, pBits2, nBytes) != 0) return false;
...
// Does a fast memcmp-based test to determine if the two bit arrays are
// different. Returns true if they are equal.
bool CompareBitArrays(void const *pPacked1, void const *pPacked2, int nBits1,
                      int nBits2) {
...
bool ShouldWatchThisProp(const SendTable *pTable, int objectID,
  ...
  if (g_CV_DTWatchClass.GetString()[0] &&
      Q_stristr(pTable->GetName(), g_CV_DTWatchClass.GetString()))
    return true;
...
bool Sendprop_UsingDebugWatch() {
  if (g_CV_DTWatchEnt.GetInt() != -1) return true;
