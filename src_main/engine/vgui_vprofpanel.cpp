...
static ConVar vprof_verbose("vprof_verbose", "1", FCVAR_ARCHIVE,
                            "Set to one to show average and peak times");
static ConVar vprof_unaccounted_limit("vprof_unaccounted_limit", "0.3",
                                      FCVAR_ARCHIVE,
                                      "number of milliseconds that a node must "
                                      "exceed to turn red in the vprof panel");
static ConVar vprof_warningmsec(
    "vprof_warningmsec", "10", FCVAR_ARCHIVE,
...
CON_COMMAND(vprof_expand_group,
...
void ChangeVProfScopeCallback(IConVar *pConVar, const char *pOldString,
                              float flOldValue) {
...
ConVar vprof_scope("vprof_scope", "", 0,
                   "Set a specific scope to start showing vprof tree", 0, 0, 0,
                   0, ChangeVProfScopeCallback);
...
void CProfileHierarchyPanel::ColumnPanels_t::AddColumn(int index,
                                                       const char *name,
                                                       vgui::Label *label) {
...
CProfileHierarchyPanel::CProfileHierarchyPanel(vgui::Panel *parent,
                                               const char *panelName)
    : BaseClass(parent, panelName), m_Panels(0, 0, PanelsLessFunc) {}
...
void CProfileHierarchyPanel::SetItemColors(int id, const Color &fg,
                                           const Color &bg) {
...
void CProfileHierarchyPanel::SetItemColumnColors(int id, int col,
                                                 const Color &fg,
                                                 const Color &bg) {
...
int CProfileHierarchyPanel::AddItem(KeyValues *data, int parentItemIndex,
                                    ColumnPanels_t &columnPanels) {
...
void CProfileHierarchyPanel::PerformLayout() {
  ...
  int vbar_top, visitems;
  bool hbarVisible = false;
  GetTree()->GetVBarInfo(vbar_top, visitems, hbarVisible);

  int offset = -vbar_top * rowheight;
  ...
  for (int row = 0; row < rows; ++row) {
    ...
    for (int col = 0; col < numColumns; ++col) {
      ...
      if (idx != m_Panels.InvalidIndex()) {
        ...
        if (col >= info.m_Columns.Count()) continue;
...
void CProfileHierarchyPanel::HideAll() {
  for (int i = m_Panels.FirstInorder(); i != m_Panels.InvalidIndex();
       i = m_Panels.NextInorder(i)) {
...
CVProfPanel::CVProfPanel(vgui::Panel *pParent, const char *pElementName)
  ...
  m_pHierarchy->SetBounds(X_BORDER, VPROF_TITLE_SIZE_Y, wide - X_BORDER * 2,
                          tall - Y_BORDER * 2 - VPROF_TITLE_SIZE_Y);
  ...
  m_pHierarchicalView =
      new vgui::CheckButton(this, "HierarchicalViewSelection", "");
  ...
  m_pStepForward =
      new vgui::Button(this, "StepForward", "", this, "StepForward");
...
void CVProfPanel::PerformLayout() {
  ...
  m_pHierarchy->SetBounds(inset, topoffset, w - 2 * inset,
                          h - inset - topoffset);
  ...
  Q_snprintf(header, sizeof(header), DATA_FMT_STR,
             "Frame Calls + Time + NoChild", "Avg Calls   + Time + NoChild",
             "Sum Calls   + Time + NoChild + Peak", "L2Miss");

  m_pHierarchy->SetColumnInfo(2, header, 775,
                              CTreeViewListControl::CI_HEADER_LEFTALIGN);
}
...
bool ChildCostSortFunc(KeyValues *pNode1, KeyValues *pNode2) {
  switch (g_nSortType) {
    ...
    case 1: {
      ...
      if (Q_stricmp(t1, t2) <= 0) return true;
      return false;
    } break;
    case 2: {
...
void CVProfPanel::OnCheckButtonChecked(Panel *panel) {
  if (panel == m_pHierarchicalView) {
    ...
    if (bSelected != m_bHierarchicalView) {
      ...
      m_pHierarchy->GetTree()->SetSortFunc(
          m_bHierarchicalView ? NULL : ChildCostSortFunc);
      m_pRedoSort->SetVisible(!m_bHierarchicalView);
      Reset();
    }
    return;
...
void CVProfPanel::ExpandAll(void) {
  ...
  for (int i = 0; i < count; i++) {
    if (tree_view->IsItemIDValid(i)) {
      tree_view->ExpandItem(i, true);
    }
  }
}
...
// Populate the tree
//-----------------------------------------------------------------------------
int CVProfPanel::UpdateVProfTreeEntry(KeyValues *pKeyValues, CVProfNode *pNode,
                                      int parent) {
  ...
  double avgLessChildren =
      (pNode->GetTotalCalls() > 0)
          ? pNode->GetTotalTimeLessChildren() / (double)pNode->GetTotalCalls()
          : 0;

  pKeyValues->SetString("Text", pNode->GetName());
  pKeyValues->SetString(
      "group", m_pVProfile->GetBudgetGroupName(pNode->GetBudgetGroupID()));
  ...
  frame.sprintf("[%4d] %7.2f %7.2f", pNode->GetCurCalls(), pNode->GetCurTime(),
                curTimeLessChildren);

  pKeyValues->SetString("frame", msg);

  if (vprof_verbose.GetBool()) {
    double avgCalls = (m_pVProfile->NumFramesSampled() > 0)
                          ? (double)pNode->GetTotalCalls() /
                                (double)m_pVProfile->NumFramesSampled()
                          : 0;
    double avg = (m_pVProfile->NumFramesSampled() > 0)
                     ? (double)pNode->GetTotalTime() /
                           (double)m_pVProfile->NumFramesSampled()
                     : 0;
    double verbose_avgLessChildren =
        (m_pVProfile->NumFramesSampled() > 0)
            ? (double)pNode->GetTotalTimeLessChildren() /
                  (double)m_pVProfile->NumFramesSampled()
            : 0;

    avgstr.sprintf("[%6.2f] %6.3f %6.3f", avgCalls, avg,
                   verbose_avgLessChildren);
    sum.sprintf("[%7d] %9.2f %9.2f %8.2fp", pNode->GetTotalCalls(),
                pNode->GetTotalTime(), pNode->GetTotalTimeLessChildren(),
                pNode->GetPeakTime());
  }
  ...
  msg.sprintf(DATA_FMT_STR, (const char *)frame, (const char *)avgstr,
              (const char *)sum, (const char *)l2miss);
  ...
  if (pNode->GetBudgetGroupID() == VPROF_BUDGET_GROUP_ID_UNACCOUNTED) {
    if (curTimeLessChildren > vprof_unaccounted_limit.GetFloat()) {
      m_pHierarchy->SetItemColors(id, Color(255, 0, 0, 255),
                                  Color(0, 0, 0, 255));
    }
  }
  ...
  if (pNode->GetCurTime() > vprof_warningmsec.GetFloat() ||
    ...
    m_pHierarchy->SetItemColumnColors(id, 2, Color(255, 0, 0, 255),
                                      Color(63, 0, 0, 255));
  }
  return id;
}
...
// Populate the tree
//-----------------------------------------------------------------------------
void CVProfPanel::FillTree(KeyValues *pKeyValues, CVProfNode *pNode,
                           int parent) {
#ifdef VPROF_ENABLED
  ...
  int id = parent;
  if ((m_nCurrentBudgetGroup < 0) ||
      (pNode->GetBudgetGroupID() == m_nCurrentBudgetGroup)) {
...
void CVProfPanel::PopulateBudgetGroupComboBox() {
  ...
  while (m_nLastBudgetGroupCount < nBudgetGroupCount) {
    m_pVProfCategory->AddItem(
        m_pVProfile->GetBudgetGroupName(m_nLastBudgetGroupCount), NULL);
...
void CVProfPanel::UpdateProfile(float filteredtime) {
  ...
  if (IsVisible()) {
    ...
    if (!m_pHierarchy->GetTree()->GetItemCount()) {
      ...
      m_pHierarchy->SetItemColors(m_RootItem, Color(255, 255, 255, 255),
                                  Color(0, 0, 0, 255));
    ...
    CVProfNode *pStartNode =
        (pScope[0] == 0)
            ? m_pVProfile->GetRoot()
            : m_pVProfile->FindNode(m_pVProfile->GetRoot(), pScope);
...
void CVProfPanel::UserCmd_ShowVProf(void) {
  ...
  SetVisible(true);
  // This is hacky . . need to at least remember the previous value to set it
  // back.
...
void CVProfPanel::OnCommand(const char *pCommand) {
  ...
  if (!Q_stricmp(pCommand, "StepForward")) {
    ...
    int shouldReset = VProfPlayback_SeekToPercent(
        (float)m_pPlaybackScroll->GetValue() / 1000.0);
    if (shouldReset == 2) {
      Reset();
    }
