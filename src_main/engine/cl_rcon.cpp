...
CRConClient &RCONClient() { return g_RCONClient; }
...
void CRConVProfExport::GetBudgetGroupInfos(CExportedBudgetGroupInfo *pInfos) {
  memcpy(pInfos, m_Info.Base(),
         GetNumBudgetGroups() * sizeof(CExportedBudgetGroupInfo));
}
...
CON_COMMAND(vprof_remote_start,
            "Request a VProf data stream from the remote server (requires "
            "authentication)") {
...
void CRConClient::SetAddress(const netadr_t &netAdr) {
  ...
  if (m_Address.GetPort() == 0) {
    m_Address.SetPort(PORT_SERVER);  // override the port setting, by default
                                     // rcon tries to bind to the same port as
                                     // the server
  }
}
...
//-----------------------------------------------------------------------------
bool CRConClient::ShouldAcceptSocket(SocketHandle_t hSocket,
                                     const netadr_t &netAdr) {
  // Can't connect if we're already connected
  return !IsConnected();
}

void CRConClient::OnSocketAccepted(SocketHandle_t hSocket,
                                   const netadr_t &netAdr, void **ppData) {}

void CRConClient::OnSocketClosed(SocketHandle_t hSocket, const netadr_t &netAdr,
                                 void *pData) {
  // reset state
  m_bAuthenticated = false;
  m_iReqID = 0;
...
void CRConClient::ParseReceivedData() {
  ...
  if (size || (m_RecvBuffer.TellPut() - m_RecvBuffer.TellGet() > 0)) {
    ...
    if (m_RecvBuffer.TellPut() - m_RecvBuffer.TellGet() > 0) {
      tmpBuf.Put(m_RecvBuffer.PeekGet(),
                 m_RecvBuffer.TellPut() - m_RecvBuffer.TellGet());
    }
    m_RecvBuffer.Purge();
    if (size > 0) {
...
void CRConClient::Authenticate() {
  ...
  // Use the otherwise-empty second string for the userid.  The server will use
  // this to exec "mp_disable_autokick <userid>" upon successful authentication.
  ...
  if (cl.IsConnected()) {
    if (cl.m_nPlayerSlot < cl.m_nMaxClients && cl.m_nPlayerSlot >= 0) {
      ...
      if (cl.m_pUserInfoTable) {
        player_info_t *pi =
            (player_info_t *)cl.m_pUserInfoTable->GetStringUserData(
                cl.m_nPlayerSlot, NULL);
        if (pi) {
          addedUserID = true;
