...
CXboxSystem::~CXboxSystem() {
  ...
  // Release any remaining handles - should have been released by the client
  // that created them.
  int ct = 0;
...
int CXboxSystem::GetOverlappedResult(AsyncHandle_t handle,
  ...
  return XGetOverlappedResult(&((AsyncResult_t *)handle)->overlapped,
                              (DWORD *)pResultCode, bWait);
}
...
// Purpose:
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::OpenContainers(void) {
  ...
  // Open the save games
  if ((m_OpenContainerResult = CreateUserSettingsContainer(
           XCONTENTFLAG_OPENALWAYS)) != ERROR_SUCCESS)
    return m_OpenContainerResult;

  // If we're TF, we don't care about save game space
  if (!Q_stricmp(GetCurrentMod(), "tf")) return m_OpenContainerResult;

  // Open the user settings
  if ((m_OpenContainerResult =
           CreateSavegameContainer(XCONTENTFLAG_OPENALWAYS)) != ERROR_SUCCESS) {
    CloseContainers();
    return m_OpenContainerResult;
  }
...
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::GetContainerOpenResult(void) {
  return m_OpenContainerResult;
}
...
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::CreateSavegameContainer(uint32_t nCreationFlags) {
  ...
  g_pXboxSystem->GetModSaveContainerNames(
      GetCurrentMod(), &pchContainerDisplayName, &pchContainerName);

  XCONTENT_DATA contentData;
  contentData.DeviceID = XBX_GetStorageDeviceId();
  contentData.dwContentType = XCONTENTTYPE_SAVEDGAME;
  Q_wcsncpy(contentData.szDisplayName, pchContainerDisplayName,
            sizeof(contentData.szDisplayName));
  Q_snprintf(contentData.szFileName, sizeof(contentData.szFileName),
             pchContainerName);
  ...
  int nRet = XContentCreateEx(XBX_GetPrimaryUserId(), GetCurrentMod(),
                              &contentData, nCreationFlags, NULL, NULL,
                              dwFileCacheSize, ulSize, NULL);
  if (nRet == ERROR_SUCCESS) {
    BOOL bUserIsCreator = false;
    XContentGetCreator(XBX_GetPrimaryUserId(), &contentData, &bUserIsCreator,
                       NULL, NULL);
...
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::CreateUserSettingsContainer(uint32_t nCreationFlags) {
  if (XBX_GetStorageDeviceId() == XBX_INVALID_STORAGE_ID ||
      XBX_GetStorageDeviceId() == XBX_STORAGE_DECLINED)
    return ERROR_INVALID_HANDLE;
  ...
  contentData.DeviceID = XBX_GetStorageDeviceId();
  contentData.dwContentType = XCONTENTTYPE_SAVEDGAME;
  Q_wcsncpy(contentData.szDisplayName,
            g_pVGuiLocalize->Find("#GameUI_Console_UserSettings"),
            sizeof(contentData.szDisplayName));
  Q_snprintf(contentData.szFileName, sizeof(contentData.szFileName),
             "UserSettings");
  ...
  int nRet = XContentCreateEx(
      XBX_GetPrimaryUserId(), XBX_USER_SETTINGS_CONTAINER_DRIVE, &contentData,
      nCreationFlags, NULL, NULL, dwFileCacheSize, ulSize, NULL);
  if (nRet == ERROR_SUCCESS) {
    BOOL bUserIsCreator = false;
    XContentGetCreator(XBX_GetPrimaryUserId(), &contentData, &bUserIsCreator,
                       NULL, NULL);
...
//			**ppchDisplayName - Display name that will be presented
// to users by the console
...
void CXboxSystem::GetModSaveContainerNames(const char *pchModName,
  ...
  if (g_szModSaveContainerDisplayName[0] == '\0') {
    if (Q_stricmp(pchModName, "episodic") == 0) {
      Q_wcsncpy(g_szModSaveContainerDisplayName,
                g_pVGuiLocalize->Find("#GameUI_Console_Ep1_Saves"),
                sizeof(g_szModSaveContainerDisplayName));
    } else if (Q_stricmp(pchModName, "ep2") == 0) {
      Q_wcsncpy(g_szModSaveContainerDisplayName,
                g_pVGuiLocalize->Find("#GameUI_Console_Ep2_Saves"),
                sizeof(g_szModSaveContainerDisplayName));
    } else if (Q_stricmp(pchModName, "portal") == 0) {
      Q_wcsncpy(g_szModSaveContainerDisplayName,
                g_pVGuiLocalize->Find("#GameUI_Console_Portal_Saves"),
                sizeof(g_szModSaveContainerDisplayName));
    } else if (Q_stricmp(pchModName, "tf") == 0) {
      Q_wcsncpy(g_szModSaveContainerDisplayName,
                g_pVGuiLocalize->Find("#GameUI_Console_TF2_Saves"),
                sizeof(g_szModSaveContainerDisplayName));
    } else {
      Q_wcsncpy(g_szModSaveContainerDisplayName,
                g_pVGuiLocalize->Find("#GameUI_Console_HL2_Saves"),
                sizeof(g_szModSaveContainerDisplayName));
    }

    // Create a filename with the format "mod_saves"
    Q_snprintf(g_szModSaveContainerName, sizeof(g_szModSaveContainerName),
               "%s_saves", pchModName);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Search the device and find out if we have adequate space to start a
// game Input  : nStorageID - Device to check
//			*pModName - Name of the mod we want to check for
//-----------------------------------------------------------------------------
bool CXboxSystem::DeviceCapacityAdequate(DWORD nStorageID,
                                         const char *pModName) {
  // If we don't have a valid user id, we can't poll the device
  if (XBX_GetPrimaryUserId() == XBX_INVALID_USER_ID) return false;

  // Must be a valid storage device to poll
  if (nStorageID == XBX_INVALID_STORAGE_ID) return false;
  ...
  const ULONGLONG nSaveGameSize =
      XContentCalculateSize(XBX_PERSISTENT_BYTES_NEEDED, 1);
  const ULONGLONG nUserSettingsSize =
      XContentCalculateSize(XBX_USER_SETTINGS_BYTES, 1);
  bool bIsTF2 = (!Q_stricmp(pModName, "tf"));
  ULONGLONG nTotalSpaceNeeded =
      (bIsTF2) ? nUserSettingsSize : (nSaveGameSize + nUserSettingsSize);
  ULONGLONG nAvailableSpace =
      deviceData.ulDeviceFreeBytes;  // Take the first device's free space to
  ...
  const wchar_t *pchContainerDisplayName;
  const char *pchContainerName;
  GetModSaveContainerNames(pModName, &pchContainerDisplayName,
                           &pchContainerName);

  // Look for a user settings block for all products
  DWORD nBufferSize;
  HANDLE hEnumerator;
  if (XContentCreateEnumerator(XBX_GetPrimaryUserId(), nStorageID,
                               XCONTENTTYPE_SAVEDGAME, fContentFlags,
                               nNumItemsToRetrieve, &nBufferSize,
                               &hEnumerator) == ERROR_SUCCESS) {
    // Allocate a buffer of the correct size
    BYTE *pBuffer = new BYTE[nBufferSize];
    if (pBuffer == NULL) return XBX_INVALID_STORAGE_ID;
    ...
    while (XEnumerate(hEnumerator, pBuffer, nBufferSize, &nNumItems, NULL) ==
      ...
      // Safely store this away (0-termination is not guaranteed by the API!)
...
//-----------------------------------------------------------------------------
// Purpose: Enumerate all devices and search for game data already present.  If
// only one device has it, we return it Input  : nUserID - User whose data we're
// searching for
//			*pModName - Name of the mod we're searching for
// Output : Device ID which contains our data (-1 if no data was found, or data
// resided on multiple devices)
//-----------------------------------------------------------------------------
DWORD CXboxSystem::DiscoverUserData(DWORD nUserID, const char *pModName) {
  // If we're entering this function without a storage device, then we must pop
  // the UI anyway to choose it!
  Assert(nUserID != XBX_INVALID_USER_ID);
  if (nUserID == XBX_INVALID_USER_ID) return XBX_INVALID_STORAGE_ID;
  ...
  ULONGLONG nAvailableSpace =
      0;  // Take the first device's free space to compare this against
  ...
  if (nFoundDevice == XBX_INVALID_STORAGE_ID) return nFoundDevice;

  // Finally, check its complete size
  if (nTotalSpaceNeeded <= nAvailableSpace) return nFoundDevice;
...
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::GetContainerRemainingSpace(void) {
  XDEVICE_DATA deviceData;
  if (XContentGetDeviceData(XBX_GetStorageDeviceId(), &deviceData) !=
      ERROR_SUCCESS)
    return 0;
...
bool CXboxSystem::ShowDeviceSelector(bool bForce, uint32_t *pStorageID,
  ...
  // We validate the size outside of this because we want to look inside our
  // packages to see what's really free
  ULARGE_INTEGER bytes;
  bytes.QuadPart = XContentCalculateSize(
      XBX_PERSISTENT_BYTES_NEEDED + XBX_USER_SETTINGS_BYTES, 1);
  ...
  DWORD ret = XShowDeviceSelectorUI(XBX_GetPrimaryUserId(),
                                    XCONTENTTYPE_SAVEDGAME, showFlags, bytes,
                                    (DWORD *)pStorageID, &pResult->overlapped);
...
int CXboxSystem::UserSetProperty(uint32_t nUserIndex, uint32_t nPropertyId,
  ...
  return XUserSetPropertyEx(nUserIndex, nPropertyId, nBytes, pData,
                            pOverlapped);
}
...
int CXboxSystem::CreateSession(uint32_t nFlags, uint32_t nUserIdx,
  ...
  // Create the session
  return XSessionCreate(nFlags, nUserIdx, nMaxPublicSlots, nMaxPrivateSlots,
                        pNonce, (XSESSION_INFO *)pSessionInfo, pOverlapped,
                        pSessionHandle);
}
...
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::DeleteSession(XboxHandle_t hSession, bool bAsync,
                                    AsyncHandle_t *pAsyncHandle) {
  XOVERLAPPED *pOverlapped = NULL;
  ...
  // Delete the session
  uint32_t ret = XSessionDelete(hSession, pOverlapped);
...
//	Purpose: Create a matchmaking session
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::SessionSearch(
    ...
    uint32_t *pcbResultsBuffer, XSESSION_SEARCHRESULT_HEADER *pSearchResults,
    bool bAsync, AsyncHandle_t *pAsyncHandle) {
  XOVERLAPPED *pOverlapped = NULL;
  ...
  return XSessionSearchEx(nProcedureIndex, nUserIndex, nNumResults, nNumUsers,
                          nNumProperties, nNumContexts, pSearchProperties,
                          pSearchContexts, (DWORD *)pcbResultsBuffer,
                          pSearchResults, pOverlapped);
}

//-----------------------------------------------------------------------------
//	Purpose: Starting a multiplayer game
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::SessionStart(XboxHandle_t hSession, uint32_t nFlags,
                                   bool bAsync, AsyncHandle_t *pAsyncHandle) {
  XOVERLAPPED *pOverlapped = NULL;
...
//-----------------------------------------------------------------------------
uint32_t CXboxSystem::SessionEnd(XboxHandle_t hSession, bool bAsync,
                                 AsyncHandle_t *pAsyncHandle) {
  XOVERLAPPED *pOverlapped = NULL;
...
int CXboxSystem::SessionJoinLocal(XboxHandle_t hSession, uint32_t nUserCount,
  ...
  return XSessionJoinLocal(hSession, nUserCount, (DWORD *)pUserIndexes,
                           (BOOL *)pPrivateSlots, pOverlapped);
}
...
int CXboxSystem::SessionJoinRemote(XboxHandle_t hSession, uint32_t nUserCount,
  ...
  return XSessionJoinRemote(hSession, nUserCount, pXuids, (BOOL *)pPrivateSlots,
                            pOverlapped);
}
...
int CXboxSystem::SessionLeaveLocal(XboxHandle_t hSession, uint32_t nUserCount,
  ...
  return XSessionLeaveLocal(hSession, nUserCount, (DWORD *)pUserIndexes,
                            pOverlapped);
}
...
int CXboxSystem::SessionMigrate(XboxHandle_t hSession, uint32_t nUserIndex,
  ...
  return XSessionMigrateHost(hSession, nUserIndex,
                             (XSESSION_INFO *)pSessionInfo, pOverlapped);
}
...
int CXboxSystem::SessionArbitrationRegister(XboxHandle_t hSession,
  ...
  return XSessionArbitrationRegister(hSession, nFlags, nonce, (DWORD *)pBytes,
                                     (XSESSION_REGISTRATION_RESULTS *)pBuffer,
                                     pOverlapped);
}
...
int CXboxSystem::WriteStats(XboxHandle_t hSession, XUID xuid, uint32_t nViews,
  ...
  return XSessionWriteStats(hSession, xuid, nViews,
                            (XSESSION_VIEW_PROPERTIES *)pViews, pOverlapped);
}
...
int CXboxSystem::EnumerateAchievements(uint32_t nUserIdx, uint64_t xuid,
  ...
  DWORD ret = XUserCreateAchievementEnumerator(
      0, nUserIdx, xuid, XACHIEVEMENT_DETAILS_ALL, nStartingIdx, nCount,
      (DWORD *)pBuffer, &hEnumerator);
  ...
  if (nBufferBytes < *(uint *)pBuffer) {
    Warning(
        "EnumerateAchievements: Buffer provided not large enough to hold "
        "results");
...
void CXboxSystem::AwardAchievement(uint32_t nUserIdx, uint32_t nAchievementId) {
  ...
  DWORD ret = XUserWriteAchievements(
      1, (XUSER_ACHIEVEMENT *)pResult->pInputData, &pResult->overlapped);
  if (ret != ERROR_IO_PENDING) {
