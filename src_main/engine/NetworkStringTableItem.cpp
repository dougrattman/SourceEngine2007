...
void CNetworkStringTableItem::UpdateChangeList(int tick, int length,
                                               const void *userData) {
  ...
  if (count > 0) {
    ...
    if (!item.data && !userData) return;  // both NULL data
...
// Input  : *string -
//-----------------------------------------------------------------------------
bool CNetworkStringTableItem::SetUserData(int tick, int length,
                                          const void *userData) {
#ifndef SHARED_NET_STRING_TABLES
  if (m_pChangeList) {
  ...
  // no old or new data
  if (!userData && !m_pUserData) return false;

  if (m_pUserData && length == m_nUserDataLength &&
