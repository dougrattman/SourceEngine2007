...
#endif
  if (pTraceInfo->m_BrushCounters[0].Count() !=
      GetCollisionBSPData()->numbrushes + 1) {
    memset(pTraceInfo->m_Count, 0, sizeof(pTraceInfo->m_Count));
    pTraceInfo->m_nCheckDepth = -1;

    for (int i = 0; i < MAX_CHECK_COUNT_DEPTH; i++) {
      pTraceInfo->m_BrushCounters[i].SetCount(
          GetCollisionBSPData()->numbrushes + 1);
      pTraceInfo->m_DispCounters[i].SetCount(g_DispCollTreeCount);

      memset(pTraceInfo->m_BrushCounters[i].Base(), 0,
             pTraceInfo->m_BrushCounters[i].Count() * sizeof(TraceCounter_t));
      memset(pTraceInfo->m_DispCounters[i].Base(), 0,
             pTraceInfo->m_DispCounters[i].Count() * sizeof(TraceCounter_t));
    }
  }
...
void PushTraceVisits(TraceInfo_t *pTraceInfo) {
  ...
  Assert((pTraceInfo->m_nCheckDepth >= 0) &&
         (pTraceInfo->m_nCheckDepth < MAX_CHECK_COUNT_DEPTH));
...
==================
*/
cmodel_t *CM_LoadMap(const char *name, bool allowReusePrevious,
                     unsigned *checksum) {
  ...
  if (name && !strcmp(pBSPData->map_name, name) && allowReusePrevious) {
  ...
  if (!name || !name[0]) {
    *checksum = 0;
    return &pBSPData->map_cmodels[0];  // cinematic servers won't have anything
                                       // at all
  }
...
==================
*/
int CM_PointLeafnumMinDistSqr_r(CCollisionBSPData *pBSPData, const Vector &p,
                                int num, float &minDistSqr) {
  float d;
  cnode_t *node;
  cplane_t *plane;
...
void CM_SnapPointToReferenceLeaf_r(CCollisionBSPData *pBSPData, const Vector &p,
                                   int num, float tolerance,
                                   Vector *pSnapPoint) {
  float d, snapDist;
  cnode_t *node;
  cplane_t *plane;
...
void CM_SnapPointToReferenceLeaf(const Vector &referenceLeafPoint,
                                 float tolerance, Vector *pSnapPoint) {
  // get the current collision bsp -- there is only one!
  CCollisionBSPData *pBSPData = GetCollisionBSPData();

  if (pBSPData->numplanes) {
    CM_SnapPointToReferenceLeaf_r(pBSPData, referenceLeafPoint, 0, tolerance,
                                  pSnapPoint);
  }
}
...
int CM_BoxLeafnums(leafnums_t &context, const Vector &center,
                   const Vector &extents, int nodenum) {
  int leafCount = 0;
  ...
  while (1) {
    if (nodenum < 0) {
      ...
      if (nodeReadIndex == nodeWriteIndex) return leafCount;
      ...
      if (d0 >= d1)
        ...
        if (context.leafTopNode == -1) context.leafTopNode = nodenum;
...
int CM_BoxLeafnums(const Vector &mins, const Vector &maxs, int *list,
  ...
  int leafCount = CM_BoxLeafnums(context, center, extents,
                                 context.pBSPData->map_cmodels[0].headnode);
...
==================
*/
int CM_TransformedPointContents(const Vector &p, int headnode,
...
bool IntersectRayWithBoxBrush(TraceInfo_t *pTraceInfo, const cbrush_t *pBrush,
  ...
  if (IsAllZeros(separation)) {
    bool bStartOut =
        IsAnyNegative(SetWToZeroSIMD(OrSIMD(startOutMins, startOutMaxs)));
    ...
    if (IsAllZeros(separation)) {
      ...
      if (bStartOut && pTraceInfo->m_ispoint &&
          pTrace->fractionleftsolid > t1) {
        bStartOut = false;
      }
      ...
      if (!bStartOut) {
        ...
        if (t1 < pTrace->fraction) {
          ...
          pTrace->surface = *pTraceInfo->m_pBSPData->GetSurfaceAtIndex(
              pBox->surfaceIndex[faceIndex]);
          if (faceIndex >= 3) {
            faceIndex -= 3;
...
// slightly different version of the above.  This folds in more of the trace_t
// output because CM_ComputeTraceEndpts() isn't called after this so this
// routine needs to properly compute start/end points and fractions in all cases
...
void FASTCALL CM_ClipBoxToBrush(TraceInfo_t *RESTRICT pTraceInfo,
  ...
  if (MSVC_SCOPED_DISABLE_WARNING(4127, IS_POINT && startout)) {
    // Add a little sludge.  The sludge should already be in the
    // fractionleftsolid (for all intents and purposes is a leavefrac value) and
    // enterfrac values. Both of these values have +/- DIST_EPSILON values
    // calculated in.  Thus, I think the test should be against "0.0."  If we
    // experience new "left solid" problems you may want to take a closer look
    // here!
  ...
  if (!startout) {  // original point was inside brush
    ...
    if (!getout) {
      ...
      // if leavefrac == 1, this means it's never been updated or we're in
      // allsolid the allsolid case was handled above
  ...
  if (enterfrac < leavefrac) {
    if (enterfrac > NEVER_UPDATED && enterfrac < trace->fraction) {
      ...
      trace->surface =
          *pTraceInfo->m_pBSPData->GetSurfaceAtIndex(leadside->surfaceIndex);
      trace->contents = brushContents;
    }
  }
}

inline bool IsTraceBoxIntersectingBoxBrush(TraceInfo_t *pTraceInfo,
                                           cboxbrush_t *pBox) {
  fltx4 start = LoadUnaligned3SIMD(pTraceInfo->m_start.Base());
...
static void FASTCALL CM_TestBoxInBrush(TraceInfo_t *pTraceInfo,
  ...
  if (brush->IsBox()) {
    cboxbrush_t *pBox =
        &pTraceInfo->m_pBSPData->map_boxbrushes[brush->GetBox()];
    if (!IsTraceBoxIntersectingBoxBrush(pTraceInfo, pBox)) return;
  } else {
    if (!brush->numsides) return;
...
void FASTCALL CM_TraceToLeaf(TraceInfo_t *RESTRICT pTraceInfo, int ndxLeaf,
  ...
  for (int ndxLeafBrush = pLeaf->firstleafbrush; ndxLeafBrush < lastleafbrush;
    ...
    // Many traces rely on CONTENTS_OPAQUE always being hit, even if it is
    // nodraw.  AI blocklos brushes need this, for instance.  CS and Terror
    // visibility checks don't want this behavior, since blocklight brushes also
    // are CONTENTS_OPAQUE and SURF_NODRAW, and are actually in the playable
    // area in several maps.
    // NOTE: This is no longer true - no traces should rely on hitting
    // CONTENTS_OPAQUE unless they actually want to hit blocklight brushes.  No
    // other brushes are marked with those bits so it should be renamed
    // CONTENTS_BLOCKLIGHT.  CONTENTS_BLOCKLOS has its own field now so there is
    // no reason to ignore nodraw opaques since you can merely remove
    // CONTENTS_OPAQUE to get that behavior
    if (releventContents == CONTENTS_OPAQUE &&
        (traceContents & CONTENTS_IGNORE_NODRAW_OPAQUE)) {
      // if the only reason we hit this brush is because it is opaque, make sure
      // it isn't nodraw
      bool isNoDraw = false;

      if (pBrush->IsBox()) {
        cboxbrush_t *pBox =
            &pTraceInfo->m_pBSPData->map_boxbrushes[pBrush->GetBox()];
        for (int i = 0; i < 6 && !isNoDraw; i++) {
          csurface_t *surface =
              pTraceInfo->m_pBSPData->GetSurfaceAtIndex(pBox->surfaceIndex[i]);
        ...
        cbrushside_t *side =
            &pTraceInfo->m_pBSPData->map_brushsides[pBrush->firstbrushside];
        for (int i = 0; i < pBrush->numsides && !isNoDraw; i++, side++) {
          csurface_t *surface =
              pTraceInfo->m_pBSPData->GetSurfaceAtIndex(side->surfaceIndex);
          if (surface->flags & SURF_NODRAW) {
            isNoDraw = true;
            break;
          }
    ...
    // NOTE: pTraceInfo->m_trace.fraction == 0.0f only when trace starts inside
    // of a brush!
    CM_ClipBoxToBrush<IS_POINT>(pTraceInfo, pBrush);
    if (!pTraceInfo->m_trace.fraction) return;
  }
  ...
  if (pLeaf->dispCount) {
    ...
    for (int i = 0; i < pLeaf->dispCount; i++) {
      ...
      if (IS_POINT && !IsBoxIntersectingRay(
                          pDispBounds->mins, pDispBounds->maxs,
                          pTraceInfo->m_start, pTraceInfo->m_delta,
                          pTraceInfo->m_invDelta, DISPCOLL_DIST_EPSILON)) {
        continue;
      }

      if (!IS_POINT && !IsBoxIntersectingRay(
                           pDispBounds->mins - pTraceInfo->m_extents,
                           pDispBounds->maxs + pTraceInfo->m_extents,
                           pTraceInfo->m_start, pTraceInfo->m_delta,
                           pTraceInfo->m_invDelta, DISPCOLL_DIST_EPSILON)) {
        continue;
      }
...
static void FASTCALL CM_TestInLeaf(TraceInfo_t *pTraceInfo, int ndxLeaf) {
  ...
  TraceCounter_t count = pTraceInfo->GetCount();
  for (int ndxLeafBrush = 0; ndxLeafBrush < pLeaf->numleafbrushes;
       ndxLeafBrush++) {
    // get the current brush
    int ndxBrush = pTraceInfo->m_pBSPData
                       ->map_leafbrushes[pLeaf->firstleafbrush + ndxLeafBrush];
  ...
  if (pLeaf->dispCount) {
    ...
    CM_TestInDispTree(pTraceInfo, pLeaf, pTraceInfo->m_start,
                      pTraceInfo->m_mins, pTraceInfo->m_maxs,
                      pTraceInfo->m_contents, &pTraceInfo->m_trace);
  }
}
...
void CM_RayLeafnums_r(const Ray_t &ray, CCollisionBSPData *pBSPData, int iNode,
                      float p1f, float p2f, const Vector &vecPoint1,
                      const Vector &vecPoint2, int *pLeafList,
                      int nMaxLeafCount, int &nLeafCount) {
  cnode_t *pNode = NULL;
  ...
  // A quick check so we don't flood the message on overflow - or keep testing
  // beyond our means!
  if (nLeafCount >= nMaxLeafCount) return;

  // Find the point distances to the seperating plane and the offset for the
  // size of the box. NJS: Hoisted loop invariant comparison to
  // pTraceInfo->m_ispoint
  if (ray.m_IsRay) {
    while (iNode >= 0) {
  ...
  CM_RayLeafnums_r(ray, pBSPData, pNode->children[nSide ^ 1], flMid, p2f,
                   vecMid, vecPoint2, pLeafList, nMaxLeafCount, nLeafCount);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CM_RayLeafnums(const Ray_t &ray, int *pLeafList, int nMaxLeafCount,
                    int &nLeafCount) {
  CCollisionBSPData *pBSPData = GetCollisionBSPData();
  if (!pBSPData->numnodes) return;

  Vector vecEnd;
  VectorAdd(ray.m_Start, ray.m_Delta, vecEnd);
  CM_RayLeafnums_r(ray, pBSPData, 0 /*headnode*/, 0.0f, 1.0f, ray.m_Start,
                   vecEnd, pLeafList, nMaxLeafCount, nLeafCount);
}
...
static void FASTCALL CM_RecursiveHullCheckImpl(TraceInfo_t *pTraceInfo, int num,
  ...
  CM_RecursiveHullCheckImpl<IS_POINT>(pTraceInfo, node->children[side], p1f,
                                      midf, p1, mid);
  ...
  CM_RecursiveHullCheckImpl<IS_POINT>(pTraceInfo, node->children[side ^ 1],
                                      midf, p2f, mid, p2);
}
...
static inline void CM_UnsweptBoxTrace(TraceInfo_t *pTraceInfo, const Ray_t &ray,
  ...
  numleafs = CM_BoxLeafnums(context, ray.m_Start,
                            ray.m_Extents + Vector(1, 1, 1), headnode);
  for (i = 0; i < numleafs; i++) {
    if ((pTraceInfo->m_pBSPData->map_leafs[leafs[i]].contents &
         CONTENTS_SOLID) == 0) {
      bFoundNonSolidLeaf = true;
    }

    CM_TestInLeaf(pTraceInfo, leafs[i]);
    if (pTraceInfo->m_trace.allsolid) break;
  }
...
//-----------------------------------------------------------------------------
void CM_BoxTraceAgainstLeafList(const Ray_t &ray, int *pLeafList,
                                int nLeafCount, int nBrushMask,
  ...
  if (!ray.m_IsSwept) {
    Vector vecBoxMin((ray.m_Start.x - ray.m_Extents.x - 1),
                     (ray.m_Start.y - ray.m_Extents.y - 1),
                     (ray.m_Start.z - ray.m_Extents.z - 1));
    Vector vecBoxMax((ray.m_Start.x + ray.m_Extents.x + 1),
                     (ray.m_Start.y + ray.m_Extents.y + 1),
                     (ray.m_Start.z + ray.m_Extents.z + 1));

    bool bFoundNonSolidLeaf = false;
    for (int iLeaf = 0; iLeaf < nLeafCount; ++iLeaf) {
      if ((pTraceInfo->m_pBSPData->map_leafs[pLeafList[iLeaf]].contents &
           CONTENTS_SOLID) == 0) {
        bFoundNonSolidLeaf = true;
      }
  ...
  Assert(!ray.m_IsRay || trace.allsolid ||
         (trace.fraction >= trace.fractionleftsolid));
}

void CM_BoxTrace(const Ray_t &ray, int headnode, int brushmask,
                 bool computeEndpt, trace_t &tr) {
  VPROF("BoxTrace");
  // for multi-check avoidance
  TraceInfo_t *pTraceInfo = BeginTrace();
...
void CM_TransformedBoxTrace(const Ray_t &ray, int headnode, int brushmask,
  ...
  if (rotated) {
    ...
    // to insure consistency at all orientations, we must rotate the origin of
    // the ray and reapply the offset to the center of the box.  That way all
    // traces with the same box centering will have the same transformation into
    // local space
...
//			*out -
//-----------------------------------------------------------------------------
void CM_NullVis(CCollisionBSPData *pBSPData, uint8_t *out) {
...
===================
*/
void CM_DecompressVis(CCollisionBSPData *pBSPData, int cluster, int visType,
                      uint8_t *out) {
  int c;
  byte *out_p;
  int numClusterBytes;
  ...
  if (cluster > pBSPData->numclusters || cluster < 0) {
    // This can happen if this is called while the level is loading. See
  ...
  uint8_t *in = ((uint8_t *)pBSPData->map_vis) +
                pBSPData->map_vis->bitofs[cluster][visType];
...
// Output : uint8_t * - pointer to the filled buffer
//-----------------------------------------------------------------------------
const uint8_t *CM_Vis(uint8_t *dest, int destlen, int cluster, int visType) {
  // get the current collision bsp -- there is only one!
  CCollisionBSPData *pBSPData = GetCollisionBSPData();
...
void CM_SetAreaPortalStates(const int *portalnums, const int *isOpen,
                            int nPortals) {
  if (nPortals == 0) return;
...
bool CM_AreasConnected(int area1, int area2) {
  ...
  if (area1 >= pBSPData->numareas || area2 >= pBSPData->numareas) {
    Sys_Error(
        "area(1==%i, 2==%i) >= numareas (%i):  Check if engine->ResetPVS() was "
        "called from ClientSetupVisibility",
        area1, area2, pBSPData->numareas);
  }

  return (pBSPData->map_areas[area1].floodnum ==
          pBSPData->map_areas[area2].floodnum);
}
...
Writes a length uint8_t followed by a bit vector of all the areas
that area in the same flood as the area parameter

This is used by the client refreshes to cull visibility
=================
*/
int CM_WriteAreaBits(uint8_t *buffer, int buflen, int area) {
  int i;
  int floodnum;
  int bytes;
...
=============
*/
bool CM_HeadnodeVisible(int nodenum, const uint8_t *visbits, int vissize) {
  int leafnum;
  int cluster;
  cnode_t *node;
  ...
  if (CM_HeadnodeVisible(node->children[0], visbits, vissize)) return true;
  return CM_HeadnodeVisible(node->children[1], visbits, vissize);
}

//-----------------------------------------------------------------------------
// Purpose: returns true if the box is in a cluster that is visible in the
// visbits Input  : mins - box extents
//			maxs -
//			*visbits - pvs or pas of some cluster
// Output : true if visible, false if not
//-----------------------------------------------------------------------------
#define MAX_BOX_LEAVES 256
int CM_BoxVisible(const Vector &mins, const Vector &maxs,
                  const uint8_t *visbits, int vissize) {
...
void CM_WorldSpaceCenter(ICollideable *pCollideable, Vector *pCenter) {
  ...
  if ((pCollideable->GetCollisionAngles() == vec3_angle) ||
    ...
    VectorTransform(vecLocalCenter, pCollideable->CollisionToWorldTransform(),
                    *pCenter);
  }
}
...
//-----------------------------------------------------------------------------
void CM_WorldAlignBounds(ICollideable *pCollideable, Vector *pMins,
                         Vector *pMaxs) {
  if (pCollideable->GetCollisionAngles() == vec3_angle) {
    *pMins = pCollideable->OBBMins();
    *pMaxs = pCollideable->OBBMaxs();
  } else {
    ITransformAABB(pCollideable->CollisionToWorldTransform(),
                   pCollideable->OBBMins(), pCollideable->OBBMaxs(), *pMins,
                   *pMaxs);
...
//-----------------------------------------------------------------------------
void CM_WorldSpaceBounds(ICollideable *pCollideable, Vector *pMins,
                         Vector *pMaxs) {
  if (pCollideable->GetCollisionAngles() == vec3_angle) {
    VectorAdd(pCollideable->GetCollisionOrigin(), pCollideable->OBBMins(),
              *pMins);
    VectorAdd(pCollideable->GetCollisionOrigin(), pCollideable->OBBMaxs(),
              *pMaxs);
  } else {
    TransformAABB(pCollideable->CollisionToWorldTransform(),
                  pCollideable->OBBMins(), pCollideable->OBBMaxs(), *pMins,
                  *pMaxs);
  }
}

void CM_SetupAreaFloodNums(uint8_t areaFloodNums[MAX_MAP_AREAS],
                           int *pNumAreas) {
  CCollisionBSPData *pBSPData = GetCollisionBSPData();
...
int CFastPointLeafNum::GetLeaf(const Vector &vPos) {
  if (vPos.DistToSqr(m_vCachedPos) > m_flDistToExitLeafSqr) {
    ...
    m_iCachedLeaf =
        CM_PointLeafnumMinDistSqr_r(pBSPData, vPos, 0, m_flDistToExitLeafSqr);
  }
...
bool FASTCALL IsBoxIntersectingRayNoLowest(
  ...
  */
  ...
  unsigned int r;
  AssertMsg((XMVectorGreaterOrEqualR(&r, SetWToZeroSIMD(boxMax),
                                     SetWToZeroSIMD(boxMin)),
             XMComparisonAllTrue(r)),
            "IsBoxIntersectingRay : boxmax < boxmin");
#endif

  // test if delta is tiny along any dimension
  fltx4 bvDeltaTinyComponents = CmpInBoundsSIMD(delta, Four_Epsilons);

  // push box extents out by tolerance (safe to do because pass by copy, not
  // ref)
  boxMin = SubSIMD(boxMin, vTolerance);
  boxMax = AddSIMD(boxMax, vTolerance);

  // for the very  short components of the ray, check if the origin is inside
  // the box; if not, then it doesn't intersect.
  fltx4 bvOriginOutsideBox =
      OrSIMD(CmpLtSIMD(origin, boxMin), CmpGtSIMD(origin, boxMax));
  bvDeltaTinyComponents = SetWToZeroSIMD(bvDeltaTinyComponents);

  // work out entry and exit points for the ray. This may produce strange
  // results for very short delta components, but those will be masked out by
  // bvDeltaTinyComponents anyway. We could early-out on bvOriginOutsideBox, but
  // it won't be ready to branch on for fourteen cycles.
  ...
  fltx4 outOfBounds = OrSIMD(CmpGtSIMD(furthestEntry, LoadOneSIMD()),
                             CmpGtSIMD(LoadZeroSIMD(), closestExit));
  fltx4 failedComponents =
      OrSIMD(tminOverTmax, outOfBounds);  // any 1's here mean return false
  // but, if a component is tiny and has its origin inside the box, ignore the
  // computation against bogus invDelta.
