...
ConCommand askconnect_accept("askconnect_accept", askconnect_accept_f,
...
ConVar cl_resend(
    ...
    true, CL_MIN_RESEND_TIME, true, CL_MAX_RESEND_TIME);
ConVar cl_name("name", "unnamed",
               FCVAR_ARCHIVE | FCVAR_USERINFO | FCVAR_PRINTABLEONLY |
                   FCVAR_SERVER_CAN_EXECUTE,
               "Current user name", CL_NameCvarChanged);
ConVar password("password", "",
                FCVAR_ARCHIVE | FCVAR_SERVER_CANNOT_QUERY | FCVAR_DONTRECORD,
                "Current server access password");
ConVar cl_interpolate("cl_interpolate", "1.0",
...
inline static bool CL_DetermineUpdateType(CEntityReadInfo &u) {
  if (!u.m_bIsEntity || (u.m_nNewEntity > u.m_nOldEntity)) {
    // If we're at the last entity, preserve whatever entities followed it in
    // the old packet. If newnum > oldnum, then the server skipped sending
    // entities that it wants to leave the state alone for.
    ...
    // Preserve entities until we reach newnum (ie: the server didn't send
    // certain entities because they haven't changed).
...
static inline void CL_ParseDeltaHeader(CEntityReadInfo &u) {
  ...
  int lastbit = u.m_pBuf->GetNumBitsRead();
  {
    ...
    SpewBitStream((uint8_t *)u.m_pBuf->m_pData, startbit, lastbit);
  }
#endif
}
...
void CBaseClientState::Clear(void) {
  ...
  if (m_NetChannel) m_NetChannel->Reset();
...
void CBaseClientState::FileRequested(const char *fileName,
  ...
  m_NetChannel->SendFile(fileName,
                         transferID);  // CBaseCLisntState always sends file
}
...
bool CBaseClientState::SetSignonState(int state, int count) {
  ...
  if ((state > SIGNONSTATE_CONNECTED) && (state <= m_nSignonState) &&
      !m_NetChannel->IsPlayback()) {
    ConMsg("Received signon %i when at %i\n", state, m_nSignonState);
    Assert(0);
    return false;
  }

  if ((count != m_nServerCount) && (count != -1) && (m_nServerCount != -1) &&
      !m_NetChannel->IsPlayback()) {
...
void CBaseClientState::SendConnectPacket(int challengeNr, int authProtocol,
  ...
  switch (authProtocol) {
    ...
    case PROTOCOL_STEAM:
      if (!PrepareSteamConnectResponse(keySize, encryptionKey, unGSSteamID,
                                       bGSSecure, adr, msg)) {
        return;
      }
      break;
    ...
    default:
      Host_Error("Unexepected authentication protocol %i!\n", authProtocol);
      return;
  }
...
bool CBaseClientState::PrepareSteamConnectResponse(
  ...
  if (!SteamUser()) {
    COM_ExplainDisconnection(
        true, "The server requires that you be running Steam.\n");
    Disconnect();
    return false;
  }
  ...
  if (steam3CookieLen > 0) msg.WriteBytes(steam3Cookie, steam3CookieLen);
#endif
...
void CBaseClientState::Disconnect(bool bShowMainMenu) {
  ...
  if (m_nSignonState == SIGNONSTATE_NONE) return;
...
#ifndef SWDS
  // Get rid of any whitelist in our filesystem and reload any files that the
  // previous whitelist forced to come from Steam. MD: This causes an annoying
  // pause when you disconnect from a server, so just leave the last whitelist
  // active until you connect to a new server.
...
void CBaseClientState::CheckForResend(void) {
  ...
  if ((net_time - m_flConnectTime) < cl_resend.GetFloat()) return;
...
bool CBaseClientState::ProcessConnectionlessPacket(netpacket_t *packet) {
  ...
  switch (c) {
    ...
    case S2C_CHALLENGE:  // Response from getchallenge we sent to the server we
                         // are connecting to
      ...
      if (m_nSignonState == SIGNONSTATE_CHALLENGE) {
        ...
        SendConnectPacket(challenge, authprotocol, keysize, keyData,
                          unGSSteamID, bGSSecure);
      }
      break;
    ...
    case S2C_CONNREJECT:
      if (m_nSignonState == SIGNONSTATE_CHALLENGE)  // Spoofed?
      {
...
bool CBaseClientState::ProcessTick(NET_Tick *msg) {
  ...
  m_NetChannel->SetRemoteFramerate(msg->m_flHostFrameTime,
                                   msg->m_flHostFrameTimeStdDeviation);

  // Note: CClientState separates the client and server clock states and drifts
  // the client's clock to match the server's, but right here, we keep the two
  // clocks in sync.
...
bool CBaseClientState::InternalProcessStringCmd(NET_StringCmd *msg,
                                                bool bIsHLTV) {
  // Don't restrict commands from the server in single player or if
  // cl_restrict_stuffed_commands is 0.
  ...
  // Run the command, but make sure the command parser knows to only execute
  // commands marked with FCVAR_SERVER_CAN_EXECUTE.
...
bool CBaseClientState::ProcessSetConVar(NET_SetConVar *msg) {
  ...
  for (int i = 0; i < msg->m_ConVars.Count(); i++) {
    ...
    if (!var.IsFlagSet(FCVAR_REPLICATED)) {
      ConMsg(
          "SetConVar: Can't set server cvar %s to %s, not marked as "
          "FCVAR_REPLICATED on client\n",
          name, value);
      continue;
    }
...
bool CBaseClientState::ProcessServerInfo(SVC_ServerInfo *msg) {
  ...
  Q_snprintf(m_szLevelName, sizeof(m_szLevelName), "maps/%s%s.bsp",
             msg->m_szMapName, GetPlatformExt());
  ...
  if (event) {
    ...
    event->SetString("address",
                     m_NetChannel->GetRemoteAddress().ToString(true));
...
bool CBaseClientState::ProcessCreateStringTable(SVC_CreateStringTable *msg) {
  ...
  CNetworkStringTable *table =
      (CNetworkStringTable *)m_StringTableContainer->CreateStringTableEx(
          msg->m_szTableName, msg->m_nMaxEntries, msg->m_nUserDataSize,
          msg->m_nUserDataSizeBits, msg->m_bIsFilenames);
...
bool CBaseClientState::ProcessUpdateStringTable(SVC_UpdateStringTable *msg) {
  ...
  CNetworkStringTable *table =
      (CNetworkStringTable *)m_StringTableContainer->GetTable(msg->m_nTableID);
...
bool CBaseClientState::ProcessPacketEntities(SVC_PacketEntities *msg) {
  ...
  // First update is the final signon stage where we actually receive an entity
...
void CBaseClientState::ReadPacketEntities(CEntityReadInfo &u) {
  ...
  // If we get an uncompressed packet, then the server is waiting for us to ack
  // the validsequence that we got the uncompressed packet on. So we stop
  // reading packets here and force ourselves to send the clc_move on the next
  // frame.
...
bool CBaseClientState::LinkClasses() {
  ...
  for (int i = 0; i < m_nServerClasses; i++) {
    ...
    // (this can be 0 in which case we just use default behavior).
    ...
    if (pServerClass->m_pClientClass) {
      ...
      if (Q_stricmp(pServerName, pClientName) != 0) {
        Host_EndGame(
            true,
            "CL_ParseClassInfo_EndClasses: server and client classes for '%s' "
            "use different datatables (server: %s, client: %s)",
...
PackedEntity *CBaseClientState::GetEntityBaseline(int iBaseline,
                                                  int nEntityIndex) {
...
bool CBaseClientState::GetClassBaseline(int iClass, void const **pData,
                                        int *pDatalen) {
  ErrorIfNot(iClass >= 0 && iClass < m_nServerClasses,
             ("GetDynamicBaseline: invalid class index '%d'", iClass));

  // We lazily update these because if you connect to a server that's already
  // got some dynamic baselines, you'll get the baselines BEFORE you get the
  // class descriptions.
  C_ServerClassInfo *pInfo = &m_pServerClasses[iClass];

  INetworkStringTable *pBaselineTable =
      GetStringTable(INSTANCE_BASELINE_TABLENAME);

  ErrorIfNot(pBaselineTable != NULL,
  ...
  if (pInfo->m_InstanceBaselineIndex == INVALID_STRING_INDEX) {
    ...
    ErrorIfNot(pInfo->m_InstanceBaselineIndex != INVALID_STRING_INDEX,
               ("GetDynamicBaseline: FindStringIndex(%s) failed.", str));
  }
  *pData = pBaselineTable->GetStringUserData(pInfo->m_InstanceBaselineIndex,
                                             pDatalen);
  ...
  if (pVar) {
    if (pVar->IsFlagSet(FCVAR_SERVER_CANNOT_QUERY)) {
      ...
      if (pVar->IsFlagSet(FCVAR_NEVER_AS_STRING)) {
        // The cvar won't store a string, so we have to come up with a string
        // for it ourselves.
    ...
    if (g_pCVar->FindCommand(msg->m_szCvarName))
      returnMsg.m_eStatusCode =
          eQueryCvarValueStatus_NotACvar;  // It's a command, not a cvar.
    else
      returnMsg.m_eStatusCode = eQueryCvarValueStatus_CvarNotFound;
  }
