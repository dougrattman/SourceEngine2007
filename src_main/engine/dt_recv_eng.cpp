   ...
   public:                                                                     \
    static void Visit_R(TableType *pTable) {                                   \
      ...
      for (int i = 0; i < pTable->GetNumProps(); i++) {                        \
        ...
        propCode;                                                              \
                                                                               \
        if (pProp->GetType() == DPT_DataTable) Visit_R(pProp->GetDataTable()); \
      }                                                                        \
    }                                                                          \
  };                                                                           \
  CPropVisitor::Visit_R(pTablePointer);

#define SENDPROP_VISIT(pTablePointer, tableCode, propCode) \
  ...
  FOR_EACH_LL(g_ClientSendTables, iClientTable) {
    ...
    for (int iProp = 0; iProp < pTable->GetNumProps(); iProp++) {
      ...
      if (pProp->m_Type == DPT_DataTable) {
        ...
        if (!pChild) {
          DataTable_Warning(
              "SetupClientSendTableHierarchy: missing SendTable '%s' "
              "(referenced by '%s').\n",
...
bool CompareRecvPropToSendProp(const RecvProp *pRecvProp,
                               const SendProp *pSendProp) {
  while (1) {
    ErrorIfNot(pRecvProp && pSendProp,
               ("CompareRecvPropToSendProp: missing a property."));

    if (pRecvProp->GetType() != pSendProp->GetType() ||
        pRecvProp->IsInsideArray() != pSendProp->IsInsideArray()) {
      return false;
    }
...
static bool MatchRecvPropsToSendProps_R(CUtlRBTree<MatchingProp_t, u16> &lookup,
  ...
  for (int i = 0; i < pSendTable->m_nProps; i++) {
    ...
    if (pSendProp->IsExcludeProp() || pSendProp->IsInsideArray()) continue;

    // Find a RecvProp by the same name and type.
    RecvProp *pRecvProp = 0;
    if (pRecvTable) pRecvProp = FindRecvProp(pRecvTable, pSendProp->GetName());
    ...
    if (pSendProp->GetType() == DPT_DataTable) {
      if (!MatchRecvPropsToSendProps_R(
              lookup, sendTableName, pSendProp->GetDataTable(),
              pRecvProp ? pRecvProp->GetDataTable() : 0))
        return false;
    }
  }
...
bool RecvTable_RecvClassInfos(bf_read *pBuf, bool bNeedsDecoder,
                              int nDemoProtocol) {
  SendTable *pSendTable = RecvTable_ReadInfos(pBuf, nDemoProtocol);

  if (!pSendTable) return false;

  bool ret =
      DataTable_SetupReceiveTableFromSendTable(pSendTable, bNeedsDecoder);
...
bool RecvTable_CreateDecoders(const CStandardSendProxies *pSendProxies) {
  ...
  if (!SetupClientSendTableHierarchy()) return false;
  ...
  FOR_EACH_LL(g_RecvDecoders, i) {
    ...
    if (!pDecoder->m_pClientSendTable) return false;
    ...
    if (!MatchRecvPropsToSendProps_R(
            PropLookup, pDecoder->GetSendTable()->m_pNetTableName,
            pDecoder->GetSendTable(), pDecoder->GetRecvTable()))
      return false;
    ...
    CopySendPropsToRecvProps(PropLookup, pPrecalc->m_DatatableProps,
                             pDecoder->m_DatatableProps);
...
bool RecvTable_Decode(RecvTable *pTable, void *pStruct, bf_read *pIn,
                      int objectID) {
  ...
  while (-1 != (iProp = deltaBitsReader.ReadNextPropIndex())) {
    ...
    decodeInfo.m_pRecvProp =
        theStack.IsCurProxyValid()
            ? pProp
            : NULL;  // Just skip the data if the proxies are screwed.
...
void RecvTable_DecodeZeros(RecvTable *pTable, void *pStruct, int objectID) {
  CRecvDecoder *pDecoder = pTable->m_pDecoder;
  ErrorIfNot(pDecoder, ("RecvTable_DecodeZeros: table '%s' missing a decoder.",
                        pTable->GetName()));
...
// Copies pProp's state from pIn to pOut. pDecodeInfo MUST be setup by calling
// InitDecodeInfoForSkippingProps with pIn.
//
// NOTE: this routine isn't optimal. If it shows up on the profiles, then it's
// easy to make this fast by adding a special routine to copy a property's state
// to PropTypeFns.
static void CopyPropState(CRecvDecoder *pDecoder, int iSendProp, bf_read *pIn,
...
int RecvTable_MergeDeltas(RecvTable *pTable,
                          ...
                          int objectID, bool bDebugWatchInfo,
                          int *pChangedProps) {
  ...
  if (pOldState) iOldProp = NextProp(&oldStateReader);
  ...
  while (1) {
    ...
    // Check if we're at the end here so the while() statement above can seek
    // the old buffer to its end too.
    if (iNewProp == PROP_SENTINEL) break;
  ...
  ErrorIfNot(!(pOldState && pOldState->IsOverflowed()) &&
                 !pNewState->IsOverflowed() && !pOut->IsOverflowed(),
...
void RecvTable_CopyEncoding(RecvTable *pTable, bf_read *pIn, bf_write *pOut,
                            int objectID) {
