...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CServerPlugin, IServerPluginHelpers,
                                  INTERFACEVERSION_ISERVERPLUGINHELPERS,
                                  s_ServerPlugin);

// Ascending value so they have a unique cookie to relate queries to the
// responses.
static int g_iQueryCvarCookie = 1;

QueryCvarCookie_t SendCvarValueQueryToClient(IClient *client,
  ...
  // If the query came from the game DLL instead of from a plugin, then we
  // negate the cookie so it knows who to callback on when the value arrives
  // back from the client.
  if (!bPluginQuery) msg.m_iCookie = -msg.m_iCookie;
  ...
  if (m_pPluginModule) {
    ...
    if (pluginFactory) {
      ...
      m_pPlugin = (IServerPluginCallbacks *)pluginFactory(
          INTERFACEVERSION_ISERVERPLUGINCALLBACKS, NULL);
      if (!m_pPlugin) {
...
//---------------------------------------------------------------------------------
const char *CPlugin::GetName() { return m_szName; }
...
// Purpose: constructor/destructor
//---------------------------------------------------------------------------------
CServerPlugin::CServerPlugin() { m_PluginHelperCheck = NULL; }
...
void CServerPlugin::LoadPlugins() {
  ...
  while (findfn) {
    ...
    if (!g_pFileSystem->FileExists(va("addons/%s", findfn),
                                   "MOD"))  // verify its in the mods directory
    {
  ...
  CreateInterfaceFn gameServerFactory = Sys_GetFactory(g_GameDLL);
  m_PluginHelperCheck = (IPluginHelpersCheck *)gameServerFactory(
      INTERFACEVERSION_PLUGINHELPERSCHECK, NULL);
}
...
void CServerPlugin::PrintDetails() {
  ...
  ConMsg("---------------------\n");
  for (int i = 0; i < m_Plugins.Count(); i++) {
    ConMsg("%i:\t\"%s\"%s\n", i, m_Plugins[i]->GetName(),
           m_Plugins[i]->IsDisabled() ? " (disabled)" : "");
  }
  ConMsg("---------------------\n");
}
...
//---------------------------------------------------------------------------------
void CServerPlugin::LevelInit(char const *pMapName, char const *pMapEntities,
  ...
  serverGameDLL->LevelInit(pMapName, pMapEntities, pOldLevel, pLandmarkName,
                           loadGame, background);
  networkStringTableContainerServer->Lock(bPrevState);
}

void CServerPlugin::ServerActivate(edict_t *pEdictList, int edictCount,
                                   int clientMax) {
  MDLCACHE_COARSE_LOCK_(g_pMDLCache);
  FORALL_PLUGINS {
    if (!m_Plugins[i]->IsDisabled()) {
      m_Plugins[i]->GetCallback()->ServerActivate(pEdictList, edictCount,
                                                  clientMax);
    }
  }
...
void CServerPlugin::ClientPutInServer(edict_t *pEntity,
...
bool CServerPlugin::ClientConnect(edict_t *pEntity, const char *pszName,
  ...
  FORALL_PLUGINS {
    if (!m_Plugins[i]->IsDisabled()) {
      result = m_Plugins[i]->GetCallback()->ClientConnect(
      ...
      if (result == PLUGIN_STOP)  // stop executing right away
      {
        ...
        return bAllowConnect;
                 ...
                 bRetValOverridden == false)  // only the first PLUGIN_OVERRIDE
                                              // return set the retval
      {
  ...
  bAllowConnect = serverGameClients->ClientConnect(pEntity, pszName, pszAddress,
                                                   reject, maxrejectlen);
...
QueryCvarCookie_t CServerPlugin::StartQueryCvarValue(edict_t *pEntity,
                                                     const char *pCvarName) {
...
void CServerPlugin::NetworkIDValidated(const char *pszUserName,
                                       const char *pszNetworkID) {
  PLUGIN_RESULT result = PLUGIN_CONTINUE;
  FORALL_PLUGINS {
    if (!m_Plugins[i]->IsDisabled()) {
      result = m_Plugins[i]->GetCallback()->NetworkIDValidated(pszUserName,
                                                               pszNetworkID);
      if (result == PLUGIN_STOP)  // stop executing right away
      {
        return;
...
void CServerPlugin::OnQueryCvarValueFinished(QueryCvarCookie_t iCookie,
  ...
  FORALL_PLUGINS {
    ...
    if (!p->IsDisabled()) {
      ...
      if (p->GetPluginInterfaceVersion() >= 2) {
        p->GetCallback()->OnQueryCvarValueFinished(
...
//---------------------------------------------------------------------------------
void CServerPlugin::CreateMessage(edict_t *pEntity, DIALOG_TYPE type,
                                  KeyValues *data,
  ...
  if (m_PluginHelperCheck &&
      !m_PluginHelperCheck->CreateMessage(plugin->GetPluginDescription(),
                                          pEntity, type, data)) {
...
void CServerPlugin::ClientCommand(edict_t *pEntity, const char *cmd) {
  ...
  if ((entnum < 1) || (entnum > sv.GetClientCount())) {
    Msg("\n!!!\nCServerPlugin::ClientCommand:  Some entity tried to stuff '%s' "
        "to console buffer of entity %i when maxclients was set to %i, "
        "ignoring\n\n",
