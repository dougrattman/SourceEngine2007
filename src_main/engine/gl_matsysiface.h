...
class CMSurfaceSortList {
 public:
  ...
  void GetSurfaceListForGroup(CUtlVector<msurface2_t *> &list,
  ...
  inline const surfacesortgroup_t &GetGroupByIndex(int groupIndex) const {
    if (!IsGroupUsed(groupIndex)) return m_emptyGroup;
    return m_groups[groupIndex];
  }
  inline const CUtlVector<surfacesortgroup_t *> &GetSortList(
      int nSortGroup) const {
  ...
  inline const surfacesortgroup_t &GetGroupForSortID(int sortGroup,
                                                     int sortID) const {
 ...
 private:
  ...
  bool IsGroupUsed(int groupIndex) const {
    return (m_groupUsed[(groupIndex >> 3)] & (1 << (groupIndex & 7))) != 0;
  }
  inline void MarkGroupUsed(int groupIndex) {
    m_groupUsed[groupIndex >> 3] |= (1 << (groupIndex & 7));
  }
  inline void MarkGroupNotUsed(int groupIndex) {
    m_groupUsed[groupIndex >> 3] &= ~(1 << (groupIndex & 7));
  }

  CUtlVector<materiallist_t> m_list;
  CUtlVector<surfacesortgroup_t>
      m_groups;  // one per sortID per MAT_SORT_GROUP, sparse
  CUtlVector<uint8_t> m_groupUsed;
...
#define MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(_sortList, _group, _pSurface)     \
  {                                                                          \
    for (short _blockIndex = (_group).listHead; _blockIndex != -1;           \
         _blockIndex = (_sortList).GetSurfaceBlock(_blockIndex).nextBlock) { \
      const materiallist_t *_pList =                                         \
          &(_sortList).GetSurfaceBlock(_blockIndex);                         \
...
#define MSL_FOREACH_GROUP_BEGIN(_sortList, _sortGroup, _group)    \
  {                                                               \
    const CUtlVector<surfacesortgroup_t *> &_groupList =          \
        (_sortList).GetSortList(_sortGroup);                      \
...
void BuildMSurfaceVerts(const struct worldbrushdata_t *pBrushData,
...
// Used for debugging - force it to release and restore all material system
// objects.
...
void MaterialSystem_RegisterPalettedLightmapSurfaces(int numPages,
                                                     void *pLightmaps);
