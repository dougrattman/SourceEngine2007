...
class CDTISendTable {
 public:
  ...
  bool HadAnyAction() const {
    return m_nCalcDeltaCalls || m_nEncodeCalls || m_nShouldTransmitCalls;
  }
...
void ServerDTI_Term() {
  if (!g_pServerDTIFilename) return;
  ServerDTI_Flush();
...
void ServerDTI_Flush() {
  ...
  if (fp != FILESYSTEM_INVALID_HANDLE) {
    ...
    // Calculate totals.
    CCycleCount totalCalcDelta, totalEncode, totalShouldTransmit,
        totalDeltaProps;
    totalCalcDelta.Init();
    totalEncode.Init();
    ...
    FOR_EACH_LL(g_DTISendTables, i) {
      ...
      CCycleCount::Add(pTable->m_nCalcDeltaCycles, totalCalcDelta,
                       totalCalcDelta);
      CCycleCount::Add(pTable->m_nEncodeCycles, totalEncode, totalEncode);
      CCycleCount::Add(pTable->m_nShouldTransmitCycles, totalShouldTransmit,
                       totalShouldTransmit);
      CCycleCount::Add(pTable->m_nWriteDeltaPropsCycles, totalDeltaProps,
                       totalDeltaProps);
    }
    ...
    FOR_EACH_LL(g_DTISendTables, j) {
      ...
      CCycleCount total;
      CCycleCount::Add(pTable->m_nEncodeCycles, pTable->m_nCalcDeltaCycles,
                       total);
      ...
      g_pFileSystem->FPrintf(
          ...
          pTable->m_nEncodeCalls, pTable->m_nEncodeCycles.GetMillisecondsF(),
          ...
          (float)pTable->m_nNoChanges * 100.0f /
              (pTable->m_nNoChanges + pTable->m_nChangeAutoDetects),
    ...
    g_pFileSystem->FPrintf(fp,
                           ...
                           totalCalcDelta.GetMillisecondsF() * 100.0 /
                               runningTime.GetMillisecondsF());
    ...
    g_pFileSystem->FPrintf(fp,
                           ...
                           totalEncode.GetMillisecondsF() * 100.0 /
                               runningTime.GetMillisecondsF());
    ...
    g_pFileSystem->FPrintf(fp,
                           ...
                           totalShouldTransmit.GetMillisecondsF() * 100.0 /
                               runningTime.GetMillisecondsF());
    ...
    g_pFileSystem->FPrintf(fp,
                           ...
                           totalDeltaProps.GetMillisecondsF() * 100.0 /
                               runningTime.GetMillisecondsF());
...
CDTISendTable *ServerDTI_HookTable(SendTable *pTable) {
  if (!g_bServerDTIEnabled) return NULL;
...
void ServerDTI_AddEntityEncodeEvent(SendTable *pSendTable, float distToPlayer) {
  ...
  if (!pPrecalc || !pPrecalc->m_pDTITable) return;
...
void _ServerDTI_HookTimer(const SendTable *pSendTable,
                          ServerDTITimerType timerType,
  ...
  if (timerType == SERVERDTI_CALCDELTA) {
    CCycleCount::Add(count, pTable->m_nCalcDeltaCycles,
                     pTable->m_nCalcDeltaCycles);
    ++pTable->m_nCalcDeltaCalls;
    ...
    CCycleCount::Add(count, pTable->m_nShouldTransmitCycles,
                     pTable->m_nShouldTransmitCycles);
    ++pTable->m_nShouldTransmitCalls;
  } else if (timerType == SERVERDTI_WRITE_DELTA_PROPS) {
    CCycleCount::Add(count, pTable->m_nWriteDeltaPropsCycles,
                     pTable->m_nWriteDeltaPropsCycles);
  }
}

void _ServerDTI_RegisterNetworkStateChange(SendTable *pSendTable,
                                           bool bStateChanged) {
  CSendTablePrecalc *pPrecalc = pSendTable->m_pPrecalc;
  if (!pPrecalc || !pPrecalc->m_pDTITable) return;
