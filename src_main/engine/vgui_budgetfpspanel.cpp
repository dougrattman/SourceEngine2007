...
void CBudgetFPSPanel::DrawTickAtIndex(int id, float percent, int red, int green,
                                      int blue, int alpha) {
  if (percent > 1.0f) {
    percent = 1.0f;
  }
...
void CBudgetFPSPanel::DrawTimeLines() {
  ...
  for (i = (int)m_fRangeMin; i < (int)(m_fRangeMax + 0.5f); i++) {
    ...
    left = -0.5f +
           panelWidth * ((float)i - m_fRangeMin) / (m_fRangeMax - m_fRangeMin);
    right = left + 1;
...
void CBudgetFPSPanel::DrawInstantaneous() {
  ...
  const double *pBudgetGroupTimes = GetBudgetPanel()->GetBudgetGroupData(
  ...
  int i;
  for (i = 0; i < nGroups; i++) {
    DrawBarAtIndex(i, (pBudgetGroupTimes[nSamplesPerGroup * i + nSampleOffset] -
                       m_fRangeMin) /
                          (m_fRangeMax - m_fRangeMin));
  }
}
...
void CBudgetFPSPanel::DrawPeaks() {
  ...
  for (i = 0; i < nGroups; i++) {
    ...
    for (j = 0; j < numSamples; j++) {
      double tmp;
      int offset =
          (nSampleOffset - j + VPROF_HISTORY_COUNT) % VPROF_HISTORY_COUNT;
