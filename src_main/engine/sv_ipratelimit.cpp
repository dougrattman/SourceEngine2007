...
static ConVar sv_max_queries_sec(
    "sv_max_queries_sec", "3.0", 0,
    "Maximum queries per second to respond to from a single IP address.");
static ConVar sv_max_queries_window(
    "sv_max_queries_window", "30", 0,
    "Window over which to average queries per second averages.");
static ConVar sv_max_queries_sec_global(
    "sv_max_queries_sec_global", "60", 0,
    "Maximum queries per second to respond to from anywhere.");
static ConVar sv_logblocks("sv_logblocks", "0", 0,
                           "If true when log when a query is blocked (can "
...
class CIPRateLimit {
 ...
 private:
  ...
  static bool LessIP(const struct CIPRateLimit::iprate_s &lhs,
                     const struct CIPRateLimit::iprate_s &rhs);
...
//-----------------------------------------------------------------------------
bool CIPRateLimit::LessIP(const struct CIPRateLimit::iprate_s &lhs,
                          const struct CIPRateLimit::iprate_s &rhs) {
...
bool CIPRateLimit::CheckIP(netadr_t adr) {
  ...
  // check the per ip rate (do this first, so one person dosing doesn't add to
  // the global max rate
  ip_t clientIP;
  ...
  if (m_IPTree.Count() > MAX_TREE_SIZE)  // if we have stored too many items
  {
    ...
    while ((m_IPTree.Count() > (2 * MAX_TREE_SIZE) / 3) &&
           i < m_IPTree.MaxElement())  // trim 1/3 the entries from the tree and
                                       // only traverse the max nodes
    {
  ...
  if (m_IPTree.IsValidIndex(entry)) {
    ...
    if ((curTime - m_IPTree[entry].lastTime) >
        sv_max_queries_window.GetFloat()) {
      m_IPTree[entry].lastTime = curTime;
      m_IPTree[entry].count = 1;
    } else {
      float query_rate =
          static_cast<float>(m_IPTree[entry].count) /
          sv_max_queries_window
              .GetFloat();  // add one so the bottom is never zero
  ...
  if ((curTime - m_lLastTime) > sv_max_queries_window.GetFloat()) {
    ...
    float query_rate = static_cast<float>(m_iGlobalCount) /
                       sv_max_queries_window
                           .GetFloat();  // add one so the bottom is never zero
