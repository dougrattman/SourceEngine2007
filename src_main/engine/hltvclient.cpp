...
static ConVar tv_maxrate(
    "tv_maxrate", "8000", 0,
    "Max SourceTV spectator bandwidth rate allowed, 0 == unlimited");
static ConVar tv_relaypassword("tv_relaypassword", "",
                               FCVAR_NOTIFY | FCVAR_PROTECTED |
                                   FCVAR_DONTRECORD,
                               "SourceTV password for relay proxies");
static ConVar tv_chattimelimit(
    "tv_chattimelimit", "8", 0,
    "Limits spectators to chat only every n seconds");
static ConVar tv_chatgroupsize("tv_chatgroupsize", "0", 0,
...
CHLTVClient::CHLTVClient(int slot, CBaseServer *pServer) {
  ...
  if (tv_chatgroupsize.GetInt() > 0) {
    Q_snprintf(m_szChatGroup, sizeof(m_szChatGroup), "group%d",
               slot % tv_chatgroupsize.GetInt());
  } else {
...
bool CHLTVClient::ProcessClientInfo(CLC_ClientInfo *msg) {
  if (!CBaseClient::ProcessClientInfo(msg)) return false;
...
bool CHLTVClient::ProcessFileCRCCheck(CLC_FileCRCCheck *msg) { return true; }
...
void CHLTVClient::PacketStart(int incoming_sequence,
...
bool CHLTVClient::ExecuteStringCommand(const char *pCommandString) {
  ...
  CCommand args;
  if (!args.Tokenize(pCommandString)) return true;

  const char *cmd = args[0];

  if (!Q_stricmp(cmd, "spec_next") || !Q_stricmp(cmd, "spec_prev") ||
  ...
  if (!Q_stricmp(cmd, "say") && args.ArgC() > 1) {
    ...
    ClientPrintf("Game Time %s, Mod \"%s\", Map \"%s\", Players %i\n",
                 COM_FormatSeconds(m_pHLTV->GetTime()), gd,
                 m_pHLTV->GetMapName(), m_pHLTV->GetNumPlayers());
...
bool CHLTVClient::ShouldSendMessages() {
  ...
  // HLTV clients use snapshot rate used by HLTV server, not given by HLTV
  // client
  ...
  if (!bSendMessage && (m_fLastSendTime + 2.0f) < net_time) {
    bSendMessage =
        true;  // force sending a message even if server didn't update
  }
...
void CHLTVClient::SetRate(int nRate, bool bForce) {
  if (!bForce) {
    if (m_bIsHLTV) {
      ...
      nRate = std::clamp(nRate, MIN_RATE, MAX_RATE);
    } else if (tv_maxrate.GetInt() > 0) {
      // restrict rate for normal clients to hltv_maxrate
      nRate = std::clamp(nRate, MIN_RATE, tv_maxrate.GetInt());
    }
  }
...
bool CHLTVClient::ProcessSetConVar(NET_SetConVar *msg) {
  if (!CBaseClient::ProcessSetConVar(msg)) return false;
  ...
  if (m_nSignonState == SIGNONSTATE_CONNECTED) {
    ...
    if (m_bIsHLTV) {
      ...
      // if client is a normal spectator, check if we can to forward him to
      // other relays
      ...
      if (!m_pHLTV->CheckIPRestrictions(m_NetChannel->GetRemoteAddress(),
...
void CHLTVClient::SendSnapshot(CClientFrame *pFrame) {
  ...
  while (pLastFrame && pLastFrame->tick_count <= pFrame->tick_count) {
    ...
    if (pDeltaFrame) {
      ...
      m_NetChannel->SendData(pLastFrame->m_Messages[HLTV_BUFFER_UNRELIABLE],
                             false);
    }
  ...
  // TODO delta cache whole snapshots, not just packet entities. then use
  // net_Align send entity update, delta compressed if deltaFrame != NULL
