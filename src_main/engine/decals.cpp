...
//
// Purpose: Functionality common to wad and decal code in gl_draw.c and draw.c
...
// This list is indexed by network id, so it maps network ids to decal
// dictionary entries
...
// Output : int
//-----------------------------------------------------------------------------
int Draw_DecalMax(void) { return g_nMaxDecals; }
...
IMaterial *Draw_DecalMaterial(int index) {
  if (index < 0 || index >= g_DecalLookup.Count()) return NULL;

  int slot = g_DecalLookup[index];
  if (slot < 0 || slot >= (int)g_DecalDictionary.MaxElement()) return NULL;
...
#if !defined(SWDS)
  return g_DecalDictionary[index].material
             ? g_DecalDictionary[index].material->GetName()
             : "";
#else
  return "";
#endif
...
void Decal_Init(void) { Decal_Shutdown(); }
