...
void CClientFrame::Init(int tickcount) { tick_count = tickcount; }
...
void CClientFrame::SetSnapshot(CFrameSnapshot *pSnapshot) {
  if (m_pSnapshot == pSnapshot) return;

  if (pSnapshot) pSnapshot->AddReference();

  if (m_pSnapshot) m_pSnapshot->ReleaseReference();
...
CClientFrame *CClientFrameManager::GetClientFrame(int nTick, bool bExact) {
  ...
  while (frame != NULL) {
    if (frame->tick_count >= nTick) {
      if (frame->tick_count == nTick) return frame;

      if (bExact) return NULL;
