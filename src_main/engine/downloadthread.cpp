...
//--------------------------------------------------------------------------------------------------------------
/**
 * Formats a string to spit out via OutputDebugString (only in debug).
 * OutputDebugString is threadsafe, so this should be fine.
 *
 * Since I don't want to be playing with the developer cvar in the other thread,
 * I'll use the presence of _DEBUG as my developer flag.
 */
inline void Thread_DPrintf(const char *fmt, ...) {
#ifdef _DEBUG
  va_list argptr;
  char msg[4096];
...
void ReadData(RequestContext &rc) {
  ...
  while (!rc.shouldStop) {
    // InternetReadFile() will block until there is data, or the socket gets
    // closed.  This means the main thread could request an abort while we're
    // blocked here.  This is okay, because the main thread will not wait for
    // this thread to finish, but will clean up the RequestContext at some later
    // point when InternetReadFile() has returned and this thread has finished.
    if (!InternetReadFile(rc.hDataResource, (LPVOID)data, BufferSize,
                          &dwSize)) {
      // if InternetReadFile() returns 0, there was a socket error (connection
      // closed, etc)
      rc.status = HTTP_ERROR;
    ...
    if (!dwSize) {
      // if InternetReadFile() succeeded, but we read 0 bytes, we're at the end
      // of the file.
      ...
      // Let the main thread know we finished reading data, and wait for it to
      // let us exit.
      rc.status = HTTP_DONE;
      return;
    } else {
...
const char *StatusString[] = {
    "HTTP_CONNECTING", "HTTP_FETCH", "HTTP_DONE", "HTTP_ABORTED", "HTTP_ERROR",
};
 ...
 * to quit.
 */
void CleanUpDownload(RequestContext &rc, HTTPStatus status,
  ...
  // and tell the main thread we're exiting, so it can delete rc.cachedData and
  // rc itself.
  rc.threadDone = true;
}

//--------------------------------------------------------------------------------------------------------------
static inline void DumpHeaders(RequestContext &rc) {
#ifdef _DEBUG
  DWORD dwSize;

  // First time we will find out the size of the headers.
  HttpQueryInfo(rc.hDataResource, HTTP_QUERY_RAW_HEADERS_CRLF, NULL, &dwSize,
                NULL);
  char *lpBuffer = new char[dwSize + 2];

  // Now we call HttpQueryInfo again to get the headers.
  if (!HttpQueryInfo(rc.hDataResource, HTTP_QUERY_RAW_HEADERS_CRLF,
                     (LPVOID)lpBuffer, &dwSize, NULL)) {
    return;
  }
  *(lpBuffer + dwSize) = '\n';
  *(lpBuffer + dwSize + 1) = '\0';
...
DWORD __stdcall DownloadThread(void *voidPtr) {
  ...
  */
  ...
  /// @TODO: Add FTP support (including restart of aborted transfers) -MDC
  /// 2004/01/08
  // We should be able to handle FTP downloads as well, but I don't have a
  // server to check against, so I'm gonna disallow it in case something bad
  // would happen.
  ...
  // The first parameter, "Half-Life", is the User-Agent that gets sent with
  // HTTP requests. INTERNET_OPEN_TYPE_PRECONFIG specifies using IE's proxy info
  // from the registry for HTTP downloads.
  rc.hOpenResource =
      InternetOpen("Half-Life 2", INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
  ...
  InternetSetStatusCallback(rc.hOpenResource,
                            (INTERNET_STATUS_CALLBACK)DownloadStatusCallback);
  ...
  flags |= INTERNET_FLAG_NO_CACHE_WRITE;  // Don't write to IE's cache, since
                                          // we're doing our own caching of
                                          // partial downloads
  flags |=
      INTERNET_FLAG_KEEP_CONNECTION;  // Use keep-alive semantics.  Since each
                                      // file downloaded is a separate
                                      // connection
                                      //   from a separate thread, I don't think
                                      //   this does much.  Can't hurt, though.
  if (url.nScheme == INTERNET_SCHEME_HTTPS) {
    // The following flags allow us to use https:// URLs, but don't provide much
    // in the way of authentication. In other words, this allows people with
  ...
  if (*rc.cachedTimestamp && rc.nBytesCached) {
    ...
    headerPtr = headers;
    headerLen =
        (DWORD)-1L;  // the DWORD cast is because we get a signed/unsigned
                     // mismatch even with an L on the -1.
  ...
  rc.hDataResource = InternetOpenUrl(rc.hOpenResource, fullURL, headerPtr,
                                     headerLen, flags, (DWORD)(&rc));
  ...
  DWORD code;
  if (!HttpQueryInfo(rc.hDataResource,
  ...
  // Only status codes we're looking for are HTTP_STATUS_OK (200) and
  ...
  // get the timestamp, and save it off for future resumes, in case we abort
  // this transfer later.
  size = BufferSize;
  if (!HttpQueryInfo(rc.hDataResource, HTTP_QUERY_LAST_MODIFIED,
                     rc.cachedTimestamp, &size, NULL)) {
    rc.cachedTimestamp[0] = 0;
  }
  rc.cachedTimestamp[BufferSize - 1] = 0;

  // If we're not getting a partial download, don't use any cached data, even if
  // we have some.
  ...
  size = sizeof(code);
  if (HttpQueryInfo(rc.hDataResource,
                    HTTP_QUERY_CONTENT_LENGTH | HTTP_QUERY_FLAG_NUMBER, &code,
                    &size, NULL)) {
