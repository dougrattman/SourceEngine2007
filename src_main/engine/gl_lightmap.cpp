...
ALIGN128 Vector4D
    blocklights[NUM_BUMP_VECTS + 1][MAX_LIGHTMAP_DIM_INCLUDING_BORDER *
                                    MAX_LIGHTMAP_DIM_INCLUDING_BORDER];
...
void R_DLightEndView() {
  ...
  for (int lnum = 0; lnum < MAX_DLIGHTS; lnum++) {
    if (r_dlightvisiblethisframe & (1 << lnum)) continue;
...
//-----------------------------------------------------------------------------
static bool AddSingleDynamicLight(dlight_t &dl, SurfaceHandle_t surfID,
                                  const Vector &lightOrigin, float perpDistSq,
                                  float lightRadiusSq) {
  ...
  local[0] = DotProduct(lightOrigin,
                        tex->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D()) +
             tex->lightmapVecsLuxelsPerWorldUnits[0][3];
  local[1] = DotProduct(lightOrigin,
                        tex->lightmapVecsLuxelsPerWorldUnits[1].AsVector3D()) +
  ...
  intensity[1] =
      TexLightToLinear(dl.color.g, dl.color.exponent) * lightStyleValue;
  intensity[2] =
  ...
  for (int t = 0; t < tmax; ++t) {
    ...
    for (int s = 0; s < smax; ++s) {
      ...
      if (totalDistSq < lightRadiusSq) {
        ...
        float scale =
            (totalDistSq != 0.0f) ? ooQuadraticAttn / totalDistSq : 1.0f;
...
//-----------------------------------------------------------------------------
static void AddSingleDynamicLightToBumpLighting(
    dlight_t &dl, SurfaceHandle_t surfID, const Vector &lightOrigin,
    float perpDistSq, float lightRadiusSq, Vector *pBumpBasis,
  ...
  // Then, I compute an actual light direction vector per luxel (FIXME:
  ...
  if (useLightDirection) VectorMultiply(dl.m_Direction, -1.0f, lightDirection);
#endif
  ...
  float fixupFactor =
      pTexInfo->worldUnitsPerLuxel * pTexInfo->worldUnitsPerLuxel;
  ...
  for (int t = 0; t < tmax; ++t) {
    ...
    VectorMA(luxelBasePosition, t * fixupFactor,
             pTexInfo->lightmapVecsLuxelsPerWorldUnits[1].AsVector3D(),
             texelWorldPosition);
    ...
    for (int s = 0; s < smax; ++s) {
      ...
      if (totalDistSq < lightRadiusSq) {
        ...
        // Compute the base lighting just as is done in the non-bump case...
        VectorMA(blocklights[0][idx].AsVector3D(), scale, intensity,
                 blocklights[0][idx].AsVector3D());
...
#else
        VectorMA(blocklights[1][idx].AsVector3D(), scale, intensity,
                 blocklights[1][idx].AsVector3D());
        VectorMA(blocklights[2][idx].AsVector3D(), scale, intensity,
                 blocklights[2][idx].AsVector3D());
        VectorMA(blocklights[3][idx].AsVector3D(), scale, intensity,
                 blocklights[3][idx].AsVector3D());
#endif
      }
    }
...
//-----------------------------------------------------------------------------
static void R_ComputeSurfaceBasis(SurfaceHandle_t surfID, Vector *pBumpNormals,
                                  Vector &luxelBasePosition) {
  // Get the bump basis vects in the space of the surface.
  Vector sVect, tVect;
  VectorCopy(
      MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D(),
      sVect);
  ...
  GetBumpNormals(sVect, tVect, MSurf_Plane(surfID).normal,
                 MSurf_Plane(surfID).normal, pBumpNormals);

  // Compute the location of the first luxel in worldspace

  // Since there's a scale factor used when going from world to luxel,
  // we gotta undo that scale factor when going from luxel to world
  float fixupFactor = MSurf_TexInfo(surfID)->worldUnitsPerLuxel *
                      MSurf_TexInfo(surfID)->worldUnitsPerLuxel;

  // The starting u of the surface is surf->lightmapMins[0];
  // since N * P + D = u, N * P = u - D, therefore we gotta move (u-D) along
  // uvec
  VectorMultiply(
      MSurf_TexInfo(surfID)->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D(),
      (MSurf_LightmapMins(surfID)[0] -
  ...
  VectorMA(luxelBasePosition, MSurf_Plane(surfID).dist,
           MSurf_Plane(surfID).normal, luxelBasePosition);
}

//-----------------------------------------------------------------------------
// Purpose: Compute the mask of which dlights affect a surface
//			NOTE: Also has the side effect of updating the surface
// lighting dlight flags!
//-----------------------------------------------------------------------------
unsigned int R_ComputeDynamicLightMask(dlight_t *pLights,
  ...
  int lightMask = 0;
  for (int lnum = 0, testBit = 1, mask = r_dlightactive; lnum < MAX_DLIGHTS;
       lnum++, mask >>= 1, testBit <<= 1) {
    if (mask & 1) {
      // not lit by this light
      if (!(pLighting->m_fDLightBits & testBit)) continue;

      // This light doesn't affect the world
      if (pLights[lnum].flags &
          (DLIGHT_NO_WORLD_ILLUMINATION | DLIGHT_DISPLACEMENT_MASK))
        continue;
      ...
      // NOTE: Dist can be negative because muzzle flashes can actually get
      // behind walls since the gun isn't checked for collision tests.
      float perpDistSq = DotProduct(lightOrigin, MSurf_Plane(surfID).normal) -
...
//			NOTE: Can be threaded, should not reference or modify
// any  global state 			other than blocklights.
//-----------------------------------------------------------------------------
void R_AddDynamicLights(dlight_t *pLights, SurfaceHandle_t surfID,
                        const matrix3x4_t &entityToWorld, bool needsBumpmap,
  ...
  for (int lnum = 0, testBit = 1, mask = lightMask;
       lnum < MAX_DLIGHTS && mask != 0; lnum++, mask >>= 1, testBit <<= 1) {
    // shift over the mask of active lights each iteration, if this one is
    // active, apply it
    if (mask & 1) {
      // Cull surface to light radius
      Vector lightOrigin;
      ...
      float perpDistSq = DotProduct(lightOrigin, MSurf_Plane(surfID).normal) -
                         MSurf_Plane(surfID).dist;
      if (perpDistSq < DLIGHT_BEHIND_PLANE_DIST) continue;

      perpDistSq *= perpDistSq;

      // If the perp distance > radius of light, blow it off
      float lightRadiusSq = pLights[lnum].GetRadiusSquared();
      if (lightRadiusSq <= perpDistSq) continue;

      if (!needsBumpmap) {
        AddSingleDynamicLight(pLights[lnum], surfID, lightOrigin, perpDistSq,
                              lightRadiusSq);
        continue;
      }
      ...
      AddSingleDynamicLightToBumpLighting(pLights[lnum], surfID, lightOrigin,
                                          perpDistSq, lightRadiusSq,
                                          bumpNormals, luxelBasePosition);
    }
  }
}
...
static int ComputeLightmapSize(SurfaceHandle_t surfID) {
  ...
  if (size > nMaxSize * nMaxSize) {
    ConMsg("Bad lightmap extents on material \"%s\"\n",
           materialSortInfoArray[MSurf_MaterialSortID(surfID)]
               .material->GetName());
    return 0;
  }
...
//-----------------------------------------------------------------------------
static void AccumulateLightstyles(ColorRGBExp32 *pLightmap, int lightmapSize,
                                  float scalar) {
  Assert(pLightmap);
  for (int i = 0; i < lightmapSize; ++i) {
    blocklights[0][i][0] +=
        scalar * TexLightToLinear(pLightmap[i].r, pLightmap[i].exponent);
    blocklights[0][i][1] +=
        scalar * TexLightToLinear(pLightmap[i].g, pLightmap[i].exponent);
    blocklights[0][i][2] +=
        scalar * TexLightToLinear(pLightmap[i].b, pLightmap[i].exponent);
  }
}

static void AccumulateLightstylesFlat(ColorRGBExp32 *pLightmap,
                                      int lightmapSize, float scalar) {
  Assert(pLightmap);
  for (int i = 0; i < lightmapSize; ++i) {
    blocklights[0][i][0] +=
        scalar * TexLightToLinear(pLightmap->r, pLightmap->exponent);
    blocklights[0][i][1] +=
        scalar * TexLightToLinear(pLightmap->g, pLightmap->exponent);
    blocklights[0][i][2] +=
        scalar * TexLightToLinear(pLightmap->b, pLightmap->exponent);
  }
}

static void AccumulateBumpedLightstyles(ColorRGBExp32 *pLightmap,
                                        int lightmapSize, float scalar) {
  ...
  for (int i = 0; i < lightmapSize; ++i) {
    ...
    blocklights[1][i][0] +=
        scalar * TexLightToLinear(pBumpedLightmaps[0][i].r,
                                  pBumpedLightmaps[0][i].exponent);
    blocklights[1][i][1] +=
        scalar * TexLightToLinear(pBumpedLightmaps[0][i].g,
                                  pBumpedLightmaps[0][i].exponent);
    blocklights[1][i][2] +=
        scalar * TexLightToLinear(pBumpedLightmaps[0][i].b,
                                  pBumpedLightmaps[0][i].exponent);
  ...
  for (int i = 0; i < lightmapSize; ++i) {
    blocklights[2][i][0] +=
        scalar * TexLightToLinear(pBumpedLightmaps[1][i].r,
                                  pBumpedLightmaps[1][i].exponent);
    blocklights[2][i][1] +=
        scalar * TexLightToLinear(pBumpedLightmaps[1][i].g,
                                  pBumpedLightmaps[1][i].exponent);
    blocklights[2][i][2] +=
        scalar * TexLightToLinear(pBumpedLightmaps[1][i].b,
                                  pBumpedLightmaps[1][i].exponent);
    ...
    blocklights[3][i][0] +=
        scalar * TexLightToLinear(pBumpedLightmaps[2][i].r,
                                  pBumpedLightmaps[2][i].exponent);
    blocklights[3][i][1] +=
        scalar * TexLightToLinear(pBumpedLightmaps[2][i].g,
                                  pBumpedLightmaps[2][i].exponent);
    blocklights[3][i][2] +=
        scalar * TexLightToLinear(pBumpedLightmaps[2][i].b,
                                  pBumpedLightmaps[2][i].exponent);
...
static const int32_t ALIGN16 g_SIMD_HalfWordMask[4] = {0x0000000, 0x0000FFFF,
...
// to be cache aligned.
// For a simpler reference implementation, see the PC version in the ifdef
// above. This function makes heavy use of the special XBOX360 opcodes for
// packing and unpacking integer d3d data. (Not available in SSE, sadly.)
static void AccumulateLightstyles_EightAtAtime(
    ColorRGBExp32 *RESTRICT
        pLightmap,  // the input lightmap (not necessarily aligned)
    int lightmapSize, fltx4 vScalar,
    Vector4D *RESTRICT bLights  // pointer to the blocklights row we'll be
  ...
  // that, we do two groups at once, because that's the length of a cache line,
  // and it helps us better hide latency.
  AssertMsg((lightmapSize & 7) == 0,
            "Input to Accumulate...EightAtATime not divisible by eight. Data "
            "corruption is the likely result.");
  ...
  for (int i = 0; i < lightmapSize; i += 8) {
    ...
    fltx4 bytePackedLightMap0 = XMLoadVector4(
        pLightmap +
        i);  // because each colorrgbexp is actually a 32-bit struct,
    ...
    // unpack rgbe 0 and 1:
    // like an unsigned unpack: { 0x00, colorLightMap[0].r, 0x00,
    // colorLightMap[0].g, 0x00, colorLightMap[0].b, 0x00, colorLightMap[0].e,
    //							  0x00,
    // colorLightMap[1].r, 0x00,  colorLightMap[1].g,  0x00, colorLightMap[1].b,
    // 0x00,
    // colorLightMap[1].e}
    fltx4 unsignedUnpackHi0 = __vmrghb(zero, bytePackedLightMap0);  // GROUP 0
    fltx4 unsignedUnpackLo0 =
        __vmrglb(zero, bytePackedLightMap0);  // rgbe words 2 and 3
    fltx4 unsignedUnpackHi1 = __vmrghb(zero, bytePackedLightMap1);  // GROUP 1
    fltx4 unsignedUnpackLo1 =
        __vmrglb(zero, bytePackedLightMap1);  // rgbe words 2 and 3

    fltx4 signedUnpackHi0 =
        __vupkhsb(bytePackedLightMap0);  // signed unpack of words 0 and 1, like
                                         // the unsigned unpack but replaces
                                         // 0x00 w/ sign extension
    fltx4 signedUnpackLo0 = __vupklsb(bytePackedLightMap0);  // GROUP 0
    fltx4 signedUnpackHi1 =
        __vupkhsb(bytePackedLightMap1);  // signed unpack of words 0 and 1, like
                                         // the unsigned unpack but replaces
                                         // 0x00 w/ sign extension
    ...
    unsignedUnpackHi0 =
        MaskedAssign(vHalfWordMask, signedUnpackHi0, unsignedUnpackHi0);
    unsignedUnpackLo0 =
        MaskedAssign(vHalfWordMask, signedUnpackLo0, unsignedUnpackLo0);
    unsignedUnpackHi1 =
        MaskedAssign(vHalfWordMask, signedUnpackHi1, unsignedUnpackHi1);
    unsignedUnpackLo1 =
        MaskedAssign(vHalfWordMask, signedUnpackLo1, unsignedUnpackLo1);

    // now complete the unpack from halfwords to words (we can just use signed
    // because there are 0x00's above the rgb channels)
    colorLightMap0[0] =
        __vupkhsh(unsignedUnpackHi0);  // vector unpack high signed halfword
    colorLightMap0[1] =
        __vupklsh(unsignedUnpackHi0);  // vector unpack low signed halfword
    ...
    colorLightMap1[0] =
        __vupkhsh(unsignedUnpackHi1);  // interleave group 1 unpacks
    ...
    // manufacture exponent splats and start normalizing the rgb channels (eg *=
    // 1/255)
    fltx4 expW0[4], expW1[4];
    expW0[0] = XMVectorSplatW(colorLightMap0[0]);
    colorLightMap0[0] = MulSIMD(
        colorLightMap0[0], vOneOverTwoFiftyFive);  // normalize the rgb channels
    expW0[1] = XMVectorSplatW(colorLightMap0[1]);
    colorLightMap0[1] = MulSIMD(
        colorLightMap0[1], vOneOverTwoFiftyFive);  // normalize the rgb channels
    expW0[2] = XMVectorSplatW(colorLightMap0[2]);
    colorLightMap0[2] = MulSIMD(
        colorLightMap0[2], vOneOverTwoFiftyFive);  // normalize the rgb channels
    expW0[3] = XMVectorSplatW(colorLightMap0[3]);
    colorLightMap0[3] = MulSIMD(
        colorLightMap0[3], vOneOverTwoFiftyFive);  // normalize the rgb channels
    ...
    expW1[0] = XMVectorSplatW(
        colorLightMap1[0]);  // interleave splats on exp group 1 (dual issue)
    colorLightMap1[0] = MulSIMD(
        colorLightMap1[0], vOneOverTwoFiftyFive);  // normalize the rgb channels
    expW0[1] = XMVectorExpEst(expW0[1]);
    expW1[1] = XMVectorSplatW(colorLightMap1[1]);
    colorLightMap1[1] = MulSIMD(
        colorLightMap1[1], vOneOverTwoFiftyFive);  // normalize the rgb channels
    expW0[2] = XMVectorExpEst(expW0[2]);
    expW1[2] = XMVectorSplatW(colorLightMap1[2]);
    colorLightMap1[2] = MulSIMD(
        colorLightMap1[2], vOneOverTwoFiftyFive);  // normalize the rgb channels
    expW0[3] = XMVectorExpEst(expW0[3]);
    expW1[3] = XMVectorSplatW(colorLightMap1[3]);
    colorLightMap1[3] = MulSIMD(
        colorLightMap1[3], vOneOverTwoFiftyFive);  // normalize the rgb channels
    ...
    for (int group = 0; group < 4; ++group) {
      Assert(colorLightMap0[group].v[0] ==
                 TexLightToLinear(pLightmap[i + group].r,
                                  pLightmap[i + group].exponent) &&
             colorLightMap0[group].v[1] ==
                 TexLightToLinear(pLightmap[i + group].g,
                                  pLightmap[i + group].exponent) &&
             colorLightMap0[group].v[2] ==
                 TexLightToLinear(pLightmap[i + group].b,
                                  pLightmap[i + group].exponent));
    }
#endif
...
static void AccumulateLightstyles(ColorRGBExp32 *pLightmap, int lightmapSize,
                                  fltx4 vScalar) {
  Assert(pLightmap);
  // crush w of the scalar to zero (so we don't overwrite blocklight[x][y][3] in
  // the madds)
  ...
  AccumulateLightstyles_EightAtAtime(pLightmap, lightmapSizeEightAligned,
                                     vScalar, blocklights[0]);
  ...
  for (int i = lightmapSizeEightAligned; i < lightmapSize; ++i) {
    ...
    fltx4 exponentiator = XMVectorExpEst(XMVectorSplatW(
        LoadSignedByte4NoAssert(reinterpret_cast<XMBYTE4 *>(pLightmap + i))));

    // scale each of the color light channels by the exponent
    colorLightMap =
        MulSIMD(MulSIMD(colorLightMap, vOneOverTwoFiftyFive), exponentiator);

    Assert(colorLightMap.v[0] ==
               TexLightToLinear(pLightmap[i].r, pLightmap[i].exponent) &&
           colorLightMap.v[1] ==
               TexLightToLinear(pLightmap[i].g, pLightmap[i].exponent) &&
           colorLightMap.v[2] ==
               TexLightToLinear(pLightmap[i].b, pLightmap[i].exponent));
...
static void AccumulateLightstylesFlat(ColorRGBExp32 *pLightmap,
  ...
  // fish out the exponent component from a signed load
  fltx4 exponentiator = XMVectorExpEst(XMVectorSplatW(
      LoadSignedByte4NoAssert(reinterpret_cast<XMBYTE4 *>(pLightmap))));
...
static void AccumulateBumpedLightstyles(ColorRGBExp32 *RESTRICT pLightmap,
                                        int lightmapSize, fltx4 vScalar) {
  COMPILE_TIME_ASSERT(
  ...
  */
  ...
  AssertMsg(
      ((reinterpret_cast<unsigned int>(pLightmap) & 0x03) == 0),
      "Lightmap was not word-aligned: AccumulateBumpedLightstyles must fail.");
  // assert vector alignment
  AssertMsg((reinterpret_cast<unsigned int>(blocklights) & 0x0F) == 0,
            "Blocklights is not vector-aligned. You're doomed.");
  AssertMsg((reinterpret_cast<unsigned int>(blocklights) & 127) == 0,
            "Blocklights is not cache-aligned. Performance will suffer.");

#if 0  // reference: This is the simple version -- four-way accumulate (no
       // interleaving)
...
// Compute the portion of the lightmap generated from lightstyles
//-----------------------------------------------------------------------------
static void ComputeLightmapFromLightstyle(msurfacelighting_t *pLighting,
                                          bool computeLightmap,
                                          bool computeBumpmap, int lightmapSize,
  ...
  for (int maps = minmap; maps < maxmap && pLighting->m_nStyles[maps] != 255;
       ++maps) {
    if (r_lightstyle.GetInt() != -1 &&
        pLighting->m_nStyles[maps] != r_lightstyle.GetInt()) {
      continue;
    }
    ...
    // skip to next lightmap. If we store lightmap data, we need to jump forward
    // 4
    pLightmap += hasBumpmapLightmapData ? lightmapSize * (NUM_BUMP_VECTS + 1)
                                        : lightmapSize;
  }
}
...
static void UpdateLightmapTextures(SurfaceHandle_t surfID, bool needsBumpmap) {
  ...
  if (materialSortInfoArray) {
    ...
    // FIXME: Should differentiate between bumped and unbumped since the perf
    // characteristics are completely different?
    //		MarkPage( materialSortInfoArray[MSurf_MaterialSortID( surfID
    //)].lightmapPageID );

    if (needsBumpmap) {
      materials->UpdateLightmap(
          materialSortInfoArray[MSurf_MaterialSortID(surfID)].lightmapPageID,
          lightmapSize, offsetIntoLightmapPage, &blocklights[0][0][0],
          &blocklights[1][0][0], &blocklights[2][0][0], &blocklights[3][0][0]);
    } else {
      materials->UpdateLightmap(
          materialSortInfoArray[MSurf_MaterialSortID(surfID)].lightmapPageID,
          lightmapSize, offsetIntoLightmapPage, &blocklights[0][0][0], NULL,
          NULL, NULL);
    }
  }
}

unsigned int R_UpdateDlightState(dlight_t *pLights, SurfaceHandle_t surfID,
                                 const matrix3x4_t &entityToWorld,
                                 bool bOnlyUseLightStyles, bool bLightmap) {
  ...
  if (!bOnlyUseLightStyles) {
    ...
    if (bLightmap && (pLighting->m_nDLightFrame == r_framecount)) {
      dlightMask =
          R_ComputeDynamicLightMask(pLights, surfID, pLighting, entityToWorld);
    }
...
//			Combine and scale multiple lightmaps into the 8.8 format
// in  blocklights
// Input  : *psurf - surface to rebuild
//			*dest - texture pointer to receive copy in lightmap
// texture  format 			stride - stride of *dest memory
//-----------------------------------------------------------------------------
void R_BuildLightMapGuts(dlight_t *pLights, SurfaceHandle_t surfID,
                         const matrix3x4_t &entityToWorld,
                         unsigned int dlightMask, bool needsBumpmap,
                         bool needsLightmap) {
  VPROF_("R_BuildLightMapGuts", 1, VPROF_BUDGETGROUP_DLIGHT_RENDERING, false,
         0);
  int bumpID;

  // Lightmap data can be dumped to save memory - this precludes any dynamic
  // lighting on the world
  ...
  if ((hasLightmap && needsLightmap) || (hasBumpmap && needsBumpmap)) {
    ComputeLightmapFromLightstyle(pLighting, (hasLightmap && needsLightmap),
                                  (hasBumpmap && needsBumpmap), size,
                                  hasBumpmap);
  } else if (!hasBumpmap && needsBumpmap && hasLightmap) {
    // make something up for the bumped lights if you need them but don't have
    // the data if you have a lightmap, use that, otherwise fullbright
    ComputeLightmapFromLightstyle(pLighting, true, false, size, hasBumpmap);

    for (bumpID = 0; bumpID < (hasBumpmap ? (NUM_BUMP_VECTS + 1) : 1);
         bumpID++) {
  ...
  if (dlightMask && (needsLightmap || needsBumpmap)) {
    R_AddDynamicLights(pLights, surfID, entityToWorld, needsBumpmap,
                       dlightMask);
  }
...
void R_BuildLightMap(dlight_t *pLights, ICallQueue *pCallQueue,
  ...
  if (!needsBumpmap && !needsLightmap) return;
  ...
  unsigned int dlightMask = R_UpdateDlightState(
      pLights, surfID, entityToWorld, bOnlyUseLightStyles, bDlightsInLightmap);

  // update the state, but don't render any dlights if only lightstyles
  // requested
  if (bOnlyUseLightStyles) dlightMask = 0;

  if (!pCallQueue) {
    R_BuildLightMapGuts(pLights, surfID, entityToWorld, dlightMask,
                        needsBumpmap, needsLightmap);
  } else {
    pCallQueue->QueueCall(R_BuildLightMapGuts, pLights, surfID,
                          RefToVal(entityToWorld), dlightMask, needsBumpmap,
                          needsLightmap);
  }
}

//-----------------------------------------------------------------------------
// Purpose: Save off the average light values, and dump the rest of the lightmap
// data. Can be used to save memory, at the expense of dynamic lights and
// lightstyles.
//-----------------------------------------------------------------------------
void CacheAndUnloadLightmapData() {
  ...
  uint8_t *pDestBase =
      (byte *)malloc(numSurfaces * MAXLIGHTMAPS * sizeof(ColorRGBExp32));
  uint8_t *pDest = pDestBase;

  for (int i = 0; i < numSurfaces; ++i, ++pLighting) {
    int nStyleCt = 0;
    for (int map = 0; map < MAXLIGHTMAPS; ++map) {
      if (pLighting->m_nStyles[map] != 255) ++nStyleCt;
    }

    const int nHdrBytes = nStyleCt * sizeof(ColorRGBExp32);
    uint8_t *pHdr = (byte *)pLighting->m_pSamples - nHdrBytes;
...
void R_RedownloadAllLightmaps() {
  ...
  if (!initializedBlockLights) {
    memset(&blocklights[0][0][0], 0,
           MAX_LIGHTMAP_DIM_INCLUDING_BORDER *
               MAX_LIGHTMAP_DIM_INCLUDING_BORDER * (NUM_BUMP_VECTS + 1) *
               sizeof(Vector));
  ...
  if (!host_state.worldbrush->unloadedlightmaps) {
    ...
    for (int surfaceIndex = 0;
         surfaceIndex < host_state.worldbrush->numsurfaces; surfaceIndex++) {
      SurfaceHandle_t surfID = SurfaceHandleFromIndex(surfaceIndex);

      ASSERT_SURF_VALID(surfID);
      R_BuildLightMap(&cl_dlights[0], pCallQueue, surfID, xform,
                      bOnlyUseLightStyles);
    }
...
void FASTCALL R_RenderDynamicLightmaps(dlight_t *pLights,
...
#ifdef UPDATE_LIGHTSTYLES_EVERY_FRAME
    if (mat_updatelightstyleseveryframe.GetBool() &&
        (pLighting->m_nStyles[0] != 0 || pLighting->m_nStyles[1] != 255)) {
      bChanged = true;
    }
#endif
    for (int maps = 0; maps < MAXLIGHTMAPS && pLighting->m_nStyles[maps] != 255;
         maps++) {
      if (d_lightstyleframe[pLighting->m_nStyles[maps]] >
          pLighting->m_nLastComputedFrame) {
        bChanged = true;
        break;
      }
    }
  ...
  bool bDLightChanged =
      (pLighting->m_nDLightFrame == r_framecount) || pLighting->m_fDLightBits;
  bool bOnlyUseLightStyles = false;
