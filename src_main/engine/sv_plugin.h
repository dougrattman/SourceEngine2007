...
class CPlugin {
 ...
 private:
  ...
  int m_iPluginInterfaceVersion;  // Tells if we got
                                  // INTERFACEVERSION_ISERVERPLUGINCALLBACKS or
                                  // an older version.
...
//---------------------------------------------------------------------------------
// Purpose: implenents passthroughs for plugins and their special helper
// functions
//---------------------------------------------------------------------------------
class CServerPlugin : public IServerPluginHelpers {
 public:
  ...
  virtual void LevelInit(char const *pMapName, char const *pMapEntities,
  ...
  virtual void CreateMessage(edict_t *pEntity, DIALOG_TYPE type,
                             KeyValues *data, IServerPluginCallbacks *plugin);
