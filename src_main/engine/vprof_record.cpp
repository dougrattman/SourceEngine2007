...
class CVProfRecorder : public CVProfile {
 public:
  ...
  void Stop() {
    if ((m_Mode == Mode_Record || m_Mode == Mode_Playback) && m_hFile != NULL) {
      if (m_Mode == Mode_Record) ++m_nQueuedStops;
    ...
    g_pVProfileForDisplay =
        &g_VProfCurrentProfile;  // Stop using us for vprofile displays.
    m_iPlaybackTick = -1;
 ...
 public:
  ...
  void Record_MatchTree_R(CVProfNode *pOut, const CVProfNode *pIn,
                          CVProfile *pInProfile) {
    // Add any new nodes at the beginning of the list..
    if (pIn->m_pChild) {
      while (!pOut->m_pChild || pIn->m_pChild->GetUniqueNodeID() !=
                                    pOut->m_pChild->GetUniqueNodeID()) {
        // Find the last new node in the list.
        const CVProfNode *pToAdd = NULL;
        const CVProfNode *pCur = pIn->m_pChild;
        while (pCur) {
          // If the out node has no children then we add the last one in the
          // input node.
          if (pOut->m_pChild &&
              pCur->GetUniqueNodeID() == pOut->m_pChild->GetUniqueNodeID())
            break;
        ...
        g_pFileSystem->Write(&parentNodeID, sizeof(parentNodeID),
                             m_hFile);  // Parent node ID.
        g_pFileSystem->Write(pToAdd->m_pszName, strlen(pToAdd->m_pszName) + 1,
                             m_hFile);  // Name of the new node.
        g_pFileSystem->Write(&budgetGroupID, sizeof(budgetGroupID), m_hFile);
        g_pFileSystem->Write(&nodeID, sizeof(nodeID), m_hFile);

        // There's a new one here.
        const char *pBudgetGroupName =
            g_VProfCurrentProfile.GetBudgetGroupName(pToAdd->m_BudgetGroupID);
        int budgetGroupFlags =
            g_VProfCurrentProfile.GetBudgetGroupFlags(pToAdd->m_BudgetGroupID);
        CVProfNode *pNewNode = pOut->GetSubNode(
            pToAdd->m_pszName, 0, pBudgetGroupName, budgetGroupFlags);
  ...
  void Record_WriteTimings_R(const CVProfNode *pIn) {
    ...
    if (curCalls >= 255) {
      ...
      // Get away with one uint8_t if we can.
      unsigned char token = (char)curCalls;
      g_pFileSystem->Write(&token, sizeof(token), m_hFile);
    }

    // This allows us to write 2 bytes unless it's > 256 milliseconds
    // (unlikely).
    ...
    for (const CVProfNode *pChild = pIn->m_pChild; pChild;
...
#else
    g_pFileSystem->Write(&g_ClientGlobalVariables.tickcount,
                         sizeof(g_ClientGlobalVariables.tickcount), m_hFile);
#endif

    // Record all the changes to get our tree and budget groups to
...
#define Playback_Assert(theTest) Playback_AssertFn(!!(theTest), __LINE__)
  ...
  bool Playback_Start(const char *pFilename) {
    ...
    if (m_hFile == NULL) {
      ...
      if (!Playback_Assert(version == VPROF_FILE_VERSION)) return false;
  ...
  bool Playback_ReadAddBudgetGroup() {
    char name[512];
    if (!Playback_ReadString(name, sizeof(name))) return false;
  ...
  bool Playback_ReadAddNode() {
    ...
    g_pFileSystem->Read(&parentNodeID, sizeof(parentNodeID),
                        m_hFile);  // Parent node ID.
    if (!Playback_ReadString(nodeName, sizeof(nodeName))) return false;
    ...
    if (!Playback_Assert(pParentNode != NULL)) return false;

    const char *pBudgetGroupName = GetBudgetGroupName(0);
    int budgetGroupFlags = 0;
    CVProfNode *pNewNode = pParentNode->GetSubNode(
        PoolString(nodeName), 0, pBudgetGroupName, budgetGroupFlags);
  ...
  bool Playback_ReadTimings_R(CVProfNode *pNode) {
    ...
    if (token == 255) {
      ...
      if (g_pFileSystem->Read(&curCalls, sizeof(curCalls), m_hFile) !=
          sizeof(curCalls))
        return false;
    ...
    if (g_pFileSystem->Read(&microsecondsToken, sizeof(microsecondsToken),
                            m_hFile) != sizeof(microsecondsToken))
      return false;

    if (microsecondsToken == 0xFFFF) {
      unsigned long nMicroseconds;
      if (g_pFileSystem->Read(&nMicroseconds, sizeof(nMicroseconds), m_hFile) !=
          sizeof(nMicroseconds))
        return false;

      pNode->m_CurFrameTime.SetMicroseconds(nMicroseconds * 4);
    } else {
      pNode->m_CurFrameTime.SetMicroseconds((unsigned long)microsecondsToken *
                                            4);
    }
    ...
    // Recurse.
    for (CVProfNode *pCur = pNode->m_pChild; pCur; pCur = pCur->m_pSibling) {
      if (!Playback_ReadTimings_R(pCur)) return false;
    }

    return true;
  }

  // Read the next tick. If iDontGoPast is set, then it will abort IF the next
  // tick's index is greater than iDontGoPast. In that case, sets
  // pWouldHaveGonePast to true, stays where it was before the call, and returns
  // true.
  bool Playback_ReadTick(int iDontGoPast = -1,
                         bool *pWouldHaveGonePast = NULL) {
    if (pWouldHaveGonePast) *pWouldHaveGonePast = false;
    ...
    if (!Playback_Assert(token == Token_StartFrame)) return false;

    int iPlaybackTick = m_iPlaybackTick;
    g_pFileSystem->Read(&iPlaybackTick, sizeof(iPlaybackTick), m_hFile);

    // First test if this tick would go past the number they don't want us to go
    // past.
    ...
    while (1) {
      int next_token = Playback_ReadToken();
      if (next_token == Token_EndOfFrame) break;

      if (next_token == Token_AddBudgetGroup) {
        if (!Playback_ReadAddBudgetGroup()) return false;
      } else if (next_token == Token_AddNode) {
        if (!Playback_ReadAddNode()) return false;
      } else if (next_token == Token_Timings) {
        if (!Playback_ReadTimings_R(GetRoot())) return false;
      } else {
        Playback_Assert(false);
        return false;
      }
  ...
  void Playback_Snapshot() {
    if (m_Mode == Mode_Playback && !m_bPlaybackPaused) Playback_ReadTick();
  }
  ...
  CNodeAverage *FindNodeAverage(CUtlVector<CNodeAverage> &averages,
                                CVProfNode *pNode) {
    for (int i = 0; i < averages.Count(); i++) {
      if (averages[i].m_pNode == pNode) return &averages[i];
    }
    return NULL;
  }
  ...
  void DumpAverages_R(CUtlVector<CNodeAverage> &averages, CVProfNode *pNode) {
    CNodeAverage *pAverage = FindNodeAverage(averages, pNode);
    if (pAverage) {
      pNode->m_CurFrameTime.m_Int64 =
          pAverage->m_CurFrameTime_Total.m_Int64 / pAverage->m_nSamples;
      pNode->m_nCurFrameCalls =
          pAverage->m_nCurFrameCalls_Total / pAverage->m_nSamples;
    }
    pNode->m_PrevFrameTime = pNode->m_CurFrameTime;
    pNode->m_nPrevFrameCalls = pNode->m_nCurFrameCalls;
  ...
  int Playback_SetPlaybackTick(int iTick) {
    if (m_Mode != Mode_Playback) return 0;

    m_bNodesChanged =
        false;  // We want to pickup changes to this, so reset it here.
    if (iTick == m_iPlaybackTick) {
      ...
      // If this tick has a smaller value than the first tick in the file, then
      // we can't seek forward to it...
      if (iTick <= m_iPlaybackTick) {
    ...
    while (m_iPlaybackTick < iTick) {
      ...
      if (!Playback_ReadTick(iTick, &bWouldHaveGonePast)) return 0;  // error

      // If reading this tick would have gone past the tick they're asking us to
      // go for, stay on the current tick.
      if (bWouldHaveGonePast) break;
  ...
  int Playback_SeekToPercent(float flWantedPercent) {
    if (m_Mode != Mode_Playback) return 0;  // error
    ...
    while (Playback_GetCurrentPercent() < flWantedPercent) {
      if (!Playback_ReadTick()) return 0;  // error
 ...
 private:
  ...
  enum { VPROF_FILE_VERSION = 1 };

  enum { Mode_None, Mode_Record, Mode_Playback };
...
CON_COMMAND(vprof_record_start,
...
CON_COMMAND(vprof_playback_start,
  ...
  // Console parser treats colons as a break, so join all the tokens together
  // here.
...
CON_COMMAND(vprof_playback_step,
            "While playing back a .vprof file, step to the next tick.") {
  VProfPlayback_Step();
}

CON_COMMAND(vprof_playback_stepback,
...
CON_COMMAND(vprof_playback_average, "Average the next N frames.") {
  if (args.ArgC() >= 2) {
    ...
    if (nFrames == -1) nFrames = 9999999;

    g_VProfRecorder.Playback_Average(nFrames);
  } else {
    Warning("vprof_playback_average [# frames]\n");
    Warning(
        "If # frames is -1, then it will average all the remaining frames in "
        "the vprof file.\n");
  }
}
...
int VProfPlayback_StepBack() {
  return g_VProfRecorder.Playback_SetPlaybackTick(
      g_VProfRecorder.Playback_GetCurrentTick() - 1);
}
