...
// This stack doesn't actually call any proxies. It uses the
// CSendProxyRecipients to tell what can be sent to the specified client.
class CPropCullStack : public CDatatableStack {
 public:
  CPropCullStack(CSendTablePrecalc *pPrecalc, int iClient,
  ...
  inline unsigned char *CallPropProxy(CSendNode *pNode, int iProp,
    ...
    bool bCur = m_pNewStateProxies[pNode->GetDataTableProxyIndex()].m_Bits.Get(
                    m_iClient) != 0;

    if (m_pOldStateProxies) {
      Assert(pNode->GetDataTableProxyIndex() < m_nOldStateProxies);
      bool bPrev =
          m_pOldStateProxies[pNode->GetDataTableProxyIndex()].m_Bits.Get(
              m_iClient) != 0;
      if (bPrev != bCur) {
        if (bPrev) {
          ...
          for (int i = 0; i < pNode->m_nRecursiveProps; i++) {
            if (m_nNewProxyProps < ARRAYSIZE(m_NewProxyProps)) {
              ...
              Error(
                  "CPropCullStack::CallPropProxy - overflowed "
                  "m_nNewProxyProps");
            }
          ...
          // Tell the outer loop that writes to m_pOutProps not to write
          // anything from this proxy since we just did.
          return 0;
        }
      }
    }
  ...
  virtual void RecurseAndCallProxies(CSendNode *pNode,
                                     unsigned char *pStructBase) {
    ...
    for (int iChild = 0; iChild < pNode->GetNumChildren(); iChild++) {
      ...
      if (pStructBase) {
        pNewStructBase =
            CallPropProxy(pCurChild, pCurChild->m_iDatatableProp, pStructBase);
      }
  ...
  void CullPropsFromProxies(const int *pStartProps, int nStartProps,
                            int *pOutProps, int nMaxOutProps) {
    m_nOutProps = 0;
    ...
    // This list will have any newly available props written into it. Write a
    // sentinel at the end.
    ...
    for (int i = 0; i < nStartProps; i++) {
      ...
      // Fill in the gaps with any properties that are newly enabled by the
      // proxies.
      ...
      // Now write this property's index if the proxies are allowing this
...
// -----------------------------------------------------------------------------
// // CDeltaCalculator encapsulates part of the functionality for calculating
// deltas between entity states.
//
// To use it, initialize it with the from and to states, then walk through the
// 'to' properties using SeekToNextToProp().
//
...
class CDeltaCalculator {
 public:
  CDeltaCalculator(CSendTablePrecalc *pPrecalc, const void *pFromState,
                   const int nFromBits, const void *pToState, const int nToBits,
                   int *pDeltaProps, int nMaxDeltaProps, const int objectID);

  ~CDeltaCalculator();

  // Returns the next prop index in the 'to' buffer. Returns PROP_SENTINEL if
  // there are no more. If a valid property index is returned, you MUST call
  // PropSkip() or PropCalcDelta() after calling this.
  int SeekToNextProp();

  // Ignore the current 'to' prop.
  void PropSkip();

  // Seek the 'from' buffer up to the current property, determine if there is a
  // delta, and write the delta (and the property data) into the buffer if there
  // is a delta.
  void PropCalcDelta();
...
CDeltaCalculator::CDeltaCalculator(CSendTablePrecalc *pPrecalc,
                                   const void *pFromState, const int nFromBits,
                                   const void *pToState, const int nToBits,
                                   int *pDeltaProps, int nMaxDeltaProps,
                                   const int objectID)

    : m_bfFromState("CDeltaCalculator->m_bfFromState", pFromState,
                    Bits2Bytes(nFromBits), nFromBits),
      m_FromBitsReader(&m_bfFromState),
      m_bfToState("CDeltaCalculator->m_bfToState", pToState,
                  Bits2Bytes(nToBits), nToBits),
...
CDeltaCalculator::~CDeltaCalculator() {
  ...
  ErrorIfNot(m_nDeltaProps <= m_nMaxDeltaProps &&
                 !m_bfFromState.IsOverflowed() && !m_bfToState.IsOverflowed(),
             ("SendTable_CalcDelta: overflowed on datatable '%s'.",
              m_pPrecalc->GetSendTable()->GetName()));
...
void CDeltaCalculator::PropCalcDelta() {
  ...
  if (m_iFromProp == m_iToProp) {
    ...
    bChange = g_PropTypeFns[pProp->m_Type].CompareDeltas(pProp, &m_bfFromState,
                                                         &m_bfToState);

    // Seek to the next properties.
    m_iFromProp = NextProp(&m_FromBitsReader);
  } else {
    // Only the 'to' state has this property, so just skip its data and register
    // a change.
...
inline int CDeltaCalculator::GetNumDeltasDetected() { return m_nDeltaProps; }

// -----------------------------------------------------------------------------
// // CEncodeInfo Used by SendTable_Encode.
// -----------------------------------------------------------------------------
// //
class CEncodeInfo : public CServerDatatableStack {
 public:
  CEncodeInfo(CSendTablePrecalc *pPrecalc, unsigned char *pStructBase,
              int objectID)
...
static inline void ShowEncodeDeltaWatchInfo(const SendTable *pTable,
                                            const SendProp *pProp,
                                            bf_read &buffer, const int objectID,
                                            const int index) {
  if (!ShouldWatchThisProp(pTable, objectID, pProp->GetName())) return;
  ...
  ConDMsg("+ %s %s, %s, index %i, bits %i, value %s\n", pTable->GetName(),
          pProp->GetName(), type, index, bits, value);
}
...
static void SendTable_EncodeProp(CEncodeInfo *pInfo, unsigned long iProp) {
  ...
  pProp->GetProxyFn()(pProp, pInfo->GetCurStructBase(),
                      pInfo->GetCurStructBase() + pProp->GetOffset(), &var,
                      0,  // iElement
                      pInfo->m_ObjectID);
  ...
  g_PropTypeFns[pProp->m_Type].Encode(pInfo->GetCurStructBase(), &var, pProp,
                                      pInfo->m_pOut, pInfo->m_ObjectID);

  pInfo->m_nDataBits += (pInfo->m_pOut->GetNumBitsWritten() -
                         iStartPos);  // record # bits written.
}
...
static bool SendTable_IsPropZero(CEncodeInfo *pInfo, unsigned long iProp) {
  ...
  return g_PropTypeFns[pProp->m_Type].IsZero(pInfo->GetCurStructBase(), &var,
                                             pProp);
}

int SendTable_CullPropsFromProxies(const SendTable *pTable,

                                   const int *pStartProps, int nStartProps,
  ...
  CPropCullStack stack(pTable->m_pPrecalc, iClient, pOldStateProxies,
                       nOldStateProxies, pNewStateProxies, nNewStateProxies);

  stack.CullPropsFromProxies(pStartProps, nStartProps, pOutProps, nMaxOutProps);

  ErrorIfNot(stack.GetNumOutProps() <= nMaxOutProps,
             ("CullPropsFromProxies: overflow in '%s'.", pTable->GetName()));
  return stack.GetNumOutProps();
}

// compares properties and writes delta properties, it ignores reciepients
int SendTable_WriteAllDeltaProps(const SendTable *pTable, const void *pFromData,
                                 const int nFromDataBits, const void *pToData,
                                 const int nToDataBits, const int nObjectID,
                                 bf_write *pBufOut) {
...
bool SendTable_Encode(const SendTable *pTable, const void *pStruct,
                      bf_write *pOut, int objectID,
  ...
  if (pRecipients) {
    ErrorIfNot(
        pRecipients->NumAllocated() >= pPrecalc->GetNumDataTableProxies(),
  ...
  info.m_pRecipients = pRecipients;  // optional buffer to store the bits for
                                     // which clients get what data.
...
void SendTable_WritePropList(const SendTable *pTable, const void *pState,
                             const int nBits, bf_write *pOut,
                             const int objectID, const int *pCheckProps,
  ...
  bf_read inputBuffer("SendTable_WritePropList->inputBuffer", pState,
                      BitByte(nBits), nBits);
...
int SendTable_CalcDelta(const SendTable *pTable,
  ...
  // Now just walk through each property in the 'to' buffer and write it in if
  // it's a new property or if it's different from the previous state.
  CDeltaCalculator deltaCalc(pTable->m_pPrecalc, pFromState, nFromBits,
                             pToState, nToBits, pDeltaProps, nMaxDeltaProps,
                             objectID);
...
bool SendTable_WriteInfos(SendTable *pTable, bf_write *pBuf) {
  ...
  for (int iProp = 0; iProp < pTable->m_nProps; iProp++) {
    ...
    unsigned int networkFlags =
        pProp->GetFlags() & ((1 << PROPINFOBITS_FLAGS) - 1);
    pBuf->WriteUBitLong(networkFlags, PROPINFOBITS_FLAGS);

    if (pProp->m_Type == DPT_DataTable) {
      // Just write the name and it will be able to reuse the table with a
      // matching name.
  ...
  for (int i = 0; i < pTable->m_nProps; i++) {
    ...
    if (pProp->GetArrayProp()) {
      if (pProp->GetArrayProp()->GetType() == DPT_DataTable) {
        Error(
            "Invalid property: %s/%s (array of datatables) [on prop %d of %d "
            "(%s)].",
            pTable->m_pNetTableName, pProp->GetName(), i, pTable->m_nProps,
            pProp->GetArrayProp()->GetName());
      }
    } else {
      ErrorIfNot(pProp->GetNumElements() == 1,
                 ("Prop %s/%s has an invalid element count for a non-array.",
                  pTable->m_pNetTableName, pProp->GetName()));
    }

    // Check for 1-bit signed properties (their value doesn't get down to the
    // client).
    if (pProp->m_nBits == 1 && !(pProp->GetFlags() & SPROP_UNSIGNED)) {
      DataTable_Warning(
          "SendTable prop %s::%s is a 1-bit signed property. Use "
          "SPROP_UNSIGNED or the client will never receive a value.\n",
          pTable->m_pNetTableName, pProp->GetName());
    }
  }
...
static bool SendTable_InitTable(SendTable *pTable) {
  if (pTable->m_pPrecalc) return true;
...
int SendTable_GetNumFlatProps(SendTable *pSendTable) {
  ...
  ErrorIfNot(pPrecalc, ("SendTable_GetNumFlatProps: missing pPrecalc."));
  return pPrecalc->GetNumProps();
}

CRC32_t SendTable_CRCTable(CRC32_t &crc, SendTable *pTable) {
  CRC32_ProcessBuffer(&crc, (void *)pTable->m_pNetTableName,
                      Q_strlen(pTable->m_pNetTableName));
  ...
  for (int iProp = 0; iProp < pTable->m_nProps; iProp++) {
    ...
    CRC32_ProcessBuffer(&crc, (void *)pProp->GetName(),
                        Q_strlen(pProp->GetName()));

    int flags = LittleLong(pProp->GetFlags());
    CRC32_ProcessBuffer(&crc, (void *)&flags, sizeof(flags));

    if (pProp->m_Type == DPT_DataTable) {
      CRC32_ProcessBuffer(&crc, (void *)pProp->GetDataTable()->m_pNetTableName,
                          Q_strlen(pProp->GetDataTable()->m_pNetTableName));
    } else {
      if (pProp->IsExcludeProp()) {
        CRC32_ProcessBuffer(&crc, (void *)pProp->GetExcludeDTName(),
                            Q_strlen(pProp->GetExcludeDTName()));
      } else if (pProp->GetType() == DPT_Array) {
...
bool SendTable_Init(SendTable **pTables, int nTables) {
  ErrorIfNot(g_SendTables.Count() == 0, ("SendTable_Init: called twice."));

  // Initialize them all.
  for (int i = 0; i < nTables; i++) {
    if (!SendTable_InitTable(pTables[i])) return false;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Computes the crc for all sendtables for the data sent in the
// class/table definitions Output : CRC32_t
//-----------------------------------------------------------------------------
CRC32_t SendTable_ComputeCRC() {
  CRC32_t result;
  CRC32_Init(&result);
...
//-----------------------------------------------------------------------------
bool SendTable_CheckIntegrity(SendTable *pTable, const void *pData,
                              const int nDataBits) {
#ifdef _DEBUG
  if (pData == NULL && nDataBits == 0) return true;

  bf_read bfRead("SendTable_CheckIntegrity", pData, Bits2Bytes(nDataBits),
                 nDataBits);
