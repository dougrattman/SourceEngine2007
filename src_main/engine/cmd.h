...
// These allow you to create blocks in the command stream where certain rules
// apply. ONLY use Cbuf_AddText in between execution markers. If you use
// Cbuf_InsertText, it will put that stuff before the execution marker and the
// execution marker won't apply.
//
// cl_restrict_server_commands uses this. It inserts a marker that says, "don't
// run anything unless it's marked with FCVAR_SERVER_CAN_EXECUTE", then inserts
// some commands, then removes the execution marker. That way, ANYTIME
// Cbuf_Execute() is called, it will apply the cl_restrict_server_commands rules
// correctly.
...
Command execution takes a 0 terminated string, breaks it into tokens,
