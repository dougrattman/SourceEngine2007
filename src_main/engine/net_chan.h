...
class CNetChan : public INetChannel {
 ...
 public:  // INetChannelInfo interface
  ...
  void GetPacketResponseLatency(int flow, int frame_number, int *pnLatencyMsecs,
                                int *pnChoke) const;
  void GetRemoteFramerate(float *pflFrameTime,
                          float *pflFrameTimeStdDeviation) const;
 ...
 public:  // INetChannel interface
  ...
  bool SendNetMsg(INetMessage &msg, bool bForceReliable = false,
                  bool bVoice = false);                 // send a net message
  bool SendData(bf_write &msg, bool bReliable = true);  // send a chunk of data
  bool SendFile(const char *filename,
                unsigned int transferID);  // transmit a local file
  void SetChoked(void);                    // choke a packet
  int SendDatagram(bf_write *data);        // build and send datagram packet
  unsigned int RequestFile(const char *filename);  // request remote file to
                                                   // upload, returns request ID
  void RequestFile_OLD(const char *filename,
                       unsigned int transferID);  // request remote file to
                                                  // upload, returns request ID
  void DenyFile(const char *filename,
                unsigned int transferID);    // deny a file request
  ...
  void GetSequenceData(int &nOutSequenceNr, int &nInSequenceNr,
                       int &nOutSequenceNrAck);
  void SetSequenceData(int nOutSequenceNr, int nInSequenceNr,
                       int nOutSequenceNrAck);
 ...
 public:
  void Setup(int sock, netadr_t *adr, const char *name,
 ...
 private:
  ...
  void FlowNewPacket(int flow, int seqnr, int acknr, int nChoked, int nDropped,
                     int nSize);
  ...
  bool CreateFragmentsFromFile(const char *filename, int stream,
 ...
 public:
  ...
  CUtlMemory<uint8_t> m_ReliableDataBuffer;

  // unreliable message buffer, cleared which each packet
  bf_write m_StreamUnreliable;
  CUtlMemory<uint8_t> m_UnreliableDataBuffer;

  bf_write m_StreamVoice;
  CUtlMemory<uint8_t> m_VoiceDataBuffer;
  ...
  CUtlVector<dataFragments_t *> m_WaitingList[MAX_STREAMS];  // waiting list for
                                                             // reliable data
                                                             // and file
                                                             // transfer
  ...
  CUtlMemory<uint8_t> m_StreamData;  // Here goes the stream data (if not file).
  ...
  IDemoRecorder
      *m_DemoRecorder;  // if != NULL points to a recording/playback demo object
