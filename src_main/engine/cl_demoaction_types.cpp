...
bool CDemoActionSkipAhead::Update(const DemoActionTimingContext &tc) {
  // Not active yet
  if (!BaseClass::Update(tc)) return false;
...
bool CDemoActionSkipAhead::Init(KeyValues *pInitData) {
  if (!BaseClass::Init(pInitData)) return false;
...
void CDemoActionSkipAhead::FireAction() {
  ...
  // Don't this this, instead wait for demo to notify us that it's finished
  // skipping SetFinishedAction( true );
  if (m_bUsingSkipTick) {
...
// Input  : track -
//-----------------------------------------------------------------------------
void CDemoActionCDTrackStart::SetTrack(int track) { m_nCDTrack = track; }
...
// Input  : buf -
//-----------------------------------------------------------------------------
void CDemoActionChangePlaybackRate::SaveKeysToBuffer(int depth,
                                                     CUtlBuffer &buf) {
  BaseClass::SaveKeysToBuffer(depth, buf);
...
void CDemoActionChangePlaybackRate::SetPlaybackRate(float rate) {
  m_flPlaybackRate = std::clamp(rate, 0.001f, 1000.0f);
}
...
// Purpose:
//-----------------------------------------------------------------------------
CDemoActionPausePlayback::CDemoActionPausePlayback() { m_flPauseTime = 0.0f; }
...
void CDemoActionPausePlayback::SetPauseTime(float t) {
  m_flPauseTime = std::clamp(t, 0.0f, 300.0f);
}
  ...
  if (GetActionFired()) {
    ...
    if (elapsed > m_flFOVRateOut) {
      ...
      scr_demo_override_fov =
          m_flOriginalFOV + frac * (m_flFinalFOV - m_flOriginalFOV);
    } else {
      float frac = 0.0f;
