...
void AddIntersectingLeafSurfaces(mleaf_t *pLeaf,
  ...
  for (int iSurf = 0; iSurf < pLeaf->nummarksurfaces; iSurf++) {
    ...
    if (MSurf_Flags(surfID) & SURFDRAW_SKY) continue;
    ...
    if (foundSurf) continue;

    // Make sure there's output room.
    if (pStruct->m_nSetInfos >= pStruct->m_nMaxInfos) return;
    ...
    for (int iVert = 0; iVert < MSurf_VertCount(surfID); iVert++) {
      int vertIndex = pStruct->m_pModel->brush.pShared
                          ->vertindices[MSurf_FirstVertIndex(surfID) + iVert];

      pOut->m_Verts[pOut->m_nVerts] =
          pStruct->m_pModel->brush.pShared->vertexes[vertIndex].position;
      vMin = vMin.Min(pOut->m_Verts[pOut->m_nVerts]);
      vMax = vMax.Max(pOut->m_Verts[pOut->m_nVerts]);

      ++pOut->m_nVerts;
      if (pOut->m_nVerts >= MAX_SURFINFO_VERTS) break;
    }
...
void GetIntersectingSurfaces_R(GetIntersectingSurfaces_Struct *pStruct,
                               mnode_t *pNode) {
  if (pStruct->m_nSetInfos >= pStruct->m_nMaxInfos) return;
  ...
  if (pNode->contents >= 0) {
    ...
    if (pStruct->m_bOnlyVisible && pStruct->m_pCenterPVS) {
      if (pLeaf->cluster < 0) return;

      if (!(pStruct->m_pCenterPVS[pLeaf->cluster >> 3] &
            (1 << (pLeaf->cluster & 7))))
        return;
    }
...
//-----------------------------------------------------------------------------
void DebugDrawPhysCollide(const CPhysCollide *pCollide, IMaterial *pMaterial,
                          matrix3x4_t &transform, const color32 &color,
                          bool drawAxes) {
  if (!pMaterial) {
    pMaterial = materials->FindMaterial("shadertest/wireframevertexcolor",
                                        TEXTURE_GROUP_OTHER);
  }
...
class CEngineClient : public IVEngineClient {
 public:
  ...
  int GetIntersectingSurfaces(const model_t *model, const Vector &vCenter,
                              const float radius, const bool bOnlyVisible,
                              SurfInfo *pInfos, const int nMaxInfos);
  ...
  virtual void ComputeDynamicLighting(Vector const &pt, Vector const *pNormal,
                                      Vector &color);

  // Computes light due to dynamic lighting at a point
  // If the normal isn't specified, then it'll return the maximum lighting
  // If pBoxColors is specified (it's an array of 6), then it'll copy the light
  // contribution at each box side.
  virtual void ComputeLighting(const Vector &pt, const Vector *pNormal,
                               bool bClamp, Vector &color, Vector *pBoxColors);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineClient, IVEngineClient,
...
int CEngineClient::GetIntersectingSurfaces(
    ...
    const bool bOnlyVisible, SurfInfo *pInfos, const int nMaxInfos) {
  if (!model) return 0;
  ...
  theStruct.m_pCenterPVS = CM_Vis(
      pvs, sizeof(pvs), CM_LeafCluster(CM_PointLeafnum(vCenter)), DVIS_PVS);
  ...
  GetIntersectingSurfaces_R(
      &theStruct, &model->brush.pShared->nodes[model->brush.firstnode]);
...
void CEngineClient::ClientCmd(const char *szCmdString) {
  ...
  if (cl.m_bRestrictClientCommands)
    Cbuf_AddExecutionMarker(
        eCmdExecutionMarker_Enable_FCVAR_CLIENTCMD_CAN_EXECUTE);
...
bool CEngineClient::GetPlayerInfo(int ent_num, player_info_t *pinfo) {
  ...
  player_info_t *pi =
      (player_info_t *)cl.m_pUserInfoTable->GetStringUserData(ent_num, NULL);
...
float CEngineClient::GetSentenceLength(CAudioSource *pAudioSource) {
  if (pAudioSource && pAudioSource->SampleRate() > 0) {
    float length =
        (float)pAudioSource->SampleCount() / (float)pAudioSource->SampleRate();
    return length;
  }
  return 0.0f;
}
...
IMaterial *CEngineClient::TraceLineMaterialAndLighting(
    const Vector &start, const Vector &end, Vector &diffuseLightColor,
    Vector &baseColor) {
  return BrushModel_GetLightingAndMaterial(start, end, diffuseLightColor,
                                           baseColor);
}
...
int CEngineClient::IsBoxInViewCluster(const Vector &mins, const Vector &maxs) {
  // See comments in Map_VisCurrentCluster for why we might get a negative
  // number.
  int curCluster = Map_VisCurrentCluster();
  if (curCluster < 0) return false;

  uint8_t pvs[MAX_MAP_LEAFS / 8];
  const uint8_t *ppvs = CM_Vis(pvs, sizeof(pvs), curCluster, DVIS_PVS);
...
void CEngineClient::Sound_ExtraUpdate(void) {
  ...
  VPROF_BUDGET("CEngineClient::Sound_ExtraUpdate()",
               VPROF_BUDGETGROUP_OTHER_SOUND);
...
// Computes light due to dynamic lighting at a point
// If the normal isn't specified, then it'll return the maximum lighting
void CEngineClient::ComputeDynamicLighting(Vector const &pt,
                                           Vector const *pNormal,
                                           Vector &color) {
...
// Computes light due to dynamic lighting at a point
// If the normal isn't specified, then it'll return the maximum lighting
void CEngineClient::ComputeLighting(const Vector &pt, const Vector *pNormal,
...
void CEngineClient::SetOcclusionParameters(const OcclusionParams_t &params) {
  OcclusionSystem()->SetOcclusionParameters(params.m_flMaxOccludeeArea,
                                            params.m_flMinOccluderArea);
}
...
int CEngineClient::GetPlayerForUserID(int userID) {
  if (!cl.m_pUserInfoTable) return 0;

  for (int i = 0; i < cl.m_nMaxClients; i++) {
    player_info_t *pi =
        (player_info_t *)cl.m_pUserInfoTable->GetStringUserData(i, NULL);
...
int CEngineClient::GetLeavesArea(int *pLeaves, int nLeaves) {
  ...
  for (int i = 1; i < nLeaves; i++) {
    ...
    if (iTestArea != iArea) return -1;
  }

  return iArea;
}

bool CEngineClient::DoesBoxTouchAreaFrustum(const Vector &mins,
                                            const Vector &maxs, int iArea) {
...
void CEngineClient::DebugDrawPhysCollide(const CPhysCollide *pCollide,
                                         IMaterial *pMaterial,
                                         matrix3x4_t &transform,
                                         const color32 &color) {
...
bool CEngineClient::IsOccluded(const Vector &vecAbsMins,
...
// Can skybox be seen from a particular point?
//-----------------------------------------------------------------------------
SkyboxVisibility_t CEngineClient::IsSkyboxVisibleFromPoint(
    const Vector &vecPoint) {
  // In the mat_fullbright 1 case, it's always visible
  // (we may have no lighting in the level, and vrad is where LEAF_FLAGS_SKY is
  // computed)
  ...
  if (nFlags & LEAF_FLAGS_SKY) return SKYBOX_3DSKYBOX_VISIBLE;
  return (nFlags & LEAF_FLAGS_SKY2D) ? SKYBOX_2DSKYBOX_VISIBLE
                                     : SKYBOX_NOT_VISIBLE;
}
...
int CEngineClient::GetAppID() { return g_iSteamAppID; }
...
bool CEngineClient::CopyFrameBufferToMaterial(const char *pMaterialName) {
  ...
  IMaterial *pMaterial =
      materials->FindMaterial(pMaterialName, TEXTURE_GROUP_OTHER);
  ...
  if (width != pTexture->GetActualWidth() ||
...
//-----------------------------------------------------------------------------
void CEngineClient::GrabPreColorCorrectedFrame(int x, int y, int width,
                                               int height) {
...
bool ClientDLL_Load() {
  ...
  // loads the client.dll, but ensures that the client dll is running under
  // Steam this will have to be undone when we want mods to be able to run
  ...
  if (g_ClientDLLModule) {
    ...
    if (g_ClientFactory) {
      g_ClientDLL =
          (IBaseClientDLL *)g_ClientFactory(CLIENT_DLL_INTERFACE_VERSION, NULL);
      // this is to ensure the old format of the string table is used for
      // clients version 13 and older. when the client version gets revved,
      // there will need to be an else that sets this bool to true
  ...
  // NOTE: Its OK if this returns NULL, as some mods won't provide the interface
  // and will just use the default behavior of the engine
  g_pClientRenderTargets = (IClientRenderTargets *)g_ClientFactory(
      CLIENTRENDERTARGETS_INTERFACE_VERSION, NULL);
  return true;
}

void InitExtraClientCmdCanExecuteVars() {
  // This can go away when we ship a client DLL with the
  // FCVAR_CLIENTCMD_CAN_EXECUTE flag set on these cvars/concommands.
  ...
  if (g_ClientDLL) {
    ...
    if (g_ClientFactory) {
      ...
      g_pClientSidePrediction = (IPrediction *)g_ClientFactory(
          VCLIENT_PREDICTION_INTERFACE_VERSION, NULL);
      if (!g_pClientSidePrediction) {
        Sys_Error("Could not get IPrediction interface from library client");
      }
      g_pClientSidePrediction->Init();

      entitylist = (IClientEntityList *)g_ClientFactory(
          VCLIENTENTITYLIST_INTERFACE_VERSION, NULL);
      if (!entitylist) {
        Sys_Error(
      ...
      centerprint =
          (ICenterPrint *)g_ClientFactory(VCENTERPRINT_INTERFACE_VERSION, NULL);
      if (!centerprint) {
        Sys_Error("Could not get centerprint interface from library client");
      }

      clientleafsystem = (IClientLeafSystemEngine *)g_ClientFactory(
          CLIENTLEAFSYSTEM_INTERFACE_VERSION, NULL);
      if (clientleafsystem) {
        g_bClientLeafSystemV1 = false;
      } else if (!clientleafsystem) {
        clientleafsystem = (IClientLeafSystemEngine *)g_ClientFactory(
            CLIENTLEAFSYSTEM_INTERFACE_VERSION_1, NULL);
        if (!clientleafsystem) {
          Sys_Error(
    ...
    if (g_pMaterialSystemHardwareConfig &&
      ...
      if ((Q_stricmp(COM_GetModDirectory(), "tf") == 0) ||
          (Q_stricmp(COM_GetModDirectory(), "ep2") == 0) ||
          (Q_stricmp(COM_GetModDirectory(), "portal") == 0)) {
        Sys_Error(
            "This game has a minimum requirement of DirectX 8.0 to run "
            "properly.");
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Called when the game initializes and whenever the vid_mode is
// changed
...
void ClientDLL_VoiceStatus(int entindex, bool bTalking) {
  if (g_ClientDLL) g_ClientDLL->VoiceStatus(entindex, bTalking);
}
