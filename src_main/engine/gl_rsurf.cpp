...
const int MAX_VERTEX_FORMAT_CHANGES = 256;
int g_MaxLeavesVisible = 1024;
...
void Shader_BrushEnd(IMatRenderContext *pRenderContext,
                     VMatrix const *brushToWorld, model_t *model,
                     bool bShadowDepth, IClientEntity *baseentity = NULL);
#ifdef NEWMESH
void BuildMSurfaceVertexArrays(worldbrushdata_t *pBrushData,
                               SurfaceHandle_t surfID, float overbright,
...
static ConVar r_drawleaf("r_drawleaf", "-1", FCVAR_CHEAT,
                         "Draw the specified leaf.");
static ConVar r_drawworld("r_drawworld", "1", FCVAR_CHEAT, "Render the world.");
static ConVar r_drawfuncdetail("r_drawfuncdetail", "1", FCVAR_CHEAT,
                               "Render func_detail");
static ConVar fog_enable_water_fog("fog_enable_water_fog", "1", FCVAR_CHEAT);
static ConVar r_fastzreject(
    "r_fastzreject", "0", 0,
    "Activate/deactivates a fast z-setting algorithm to take advantage of "
    "hardware with fast z reject. Use -1 to default to hardware settings");
static ConVar r_fastzrejectdisp(
    "r_fastzrejectdisp", "0", 0,
    "Activates/deactivates fast z rejection on displacements (360 only). Only "
    "active when r_fastzreject is on.");
...
const cplane_t &R_GetBrushModelPlane(const model_t *model, int nIndex,
                                     Vector *pOrigin) {
  SurfaceHandle_t surfID = SurfaceHandleFromIndex(
      model->brush.firstmodelsurface, model->brush.pShared);
  surfID += nIndex;
  ...
  if (pOrigin) {
    ...
    if (vertCount > 0) {
      int nFirstVertex =
          model->brush.pShared->vertindices[MSurf_FirstVertIndex(surfID)];
...
class CWorldRenderList : public CRefCounted1<IWorldRenderList> {
 public:
  ...
  static CWorldRenderList *FindOrCreateList(int nSurfaces) {
    ...
    AssertMsg(p->m_VisitedSurfs.GetNumBits() == nSurfaces,
              "World render list pool not cleared between maps");
...
bool DlightSurfaceSetQueuingFlag(SurfaceHandle_t surfID) {
  if (MSurf_Flags(surfID) & SURFDRAW_HASLIGHTSYTLES) {
    ...
    for (int maps = 1; maps < MAXLIGHTMAPS && pLighting->m_nStyles[maps] != 255;
         maps++) {
...
//-----------------------------------------------------------------------------
void Shader_TranslucentWorldSurface(CWorldRenderList *pRenderList,
                                    SurfaceHandle_t surfID) {
  Assert(!SurfaceHasDispInfo(surfID) &&
         (pRenderList->m_VisibleLeaves.Count() > 0));

  // Hook into the chain of translucent objects for this leaf
  int sortGroup = MSurf_SortGroup(surfID);
  pRenderList->m_AlphaSortList.AddSurfaceToTail(
      surfID, sortGroup, pRenderList->m_VisibleLeaves.Count() - 1);
...
inline void Shader_WorldSurface(CWorldRenderList *pRenderList,
                                SurfaceHandle_t surfID) {
  ...
  if (MSurf_Flags(surfID) & (SURFDRAW_HASLIGHTSYTLES | SURFDRAW_HASDLIGHT)) {
    ...
    if (!DlightSurfaceSetQueuingFlag(surfID)) {
      pRenderList->m_SortList.AddSurfaceToTail(surfID, nSortGroup,
                                               nMaterialSortID);
    }
  } else {
    pRenderList->m_SortList.AddSurfaceToTail(surfID, nSortGroup,
...
inline void Shader_WorldSurfaceNoCull(CWorldRenderList *pRenderList,
                                      SurfaceHandle_t surfID) {
  // Hook it into the list of surfaces to render with this material
  // Do it in a way that generates a front-to-back ordering for fast z reject
  Assert(!SurfaceHasDispInfo(surfID));
...
//-----------------------------------------------------------------------------
void Shader_TranslucentDisplacementSurface(CWorldRenderList *pRenderList,
                                           SurfaceHandle_t surfID) {
  Assert(SurfaceHasDispInfo(surfID) &&
         (pRenderList->m_VisibleLeaves.Count() > 0));
  ...
  // NOTE: You might expect some problems here when displacements cross fog
  // volume planes. However, these problems go away (I hope!) because the first
  // planes that split a scene are the fog volume planes. That means that if
  // we're in a fog volume, the closest leaf that the displacement will be in
  // will also be in the fog volume. If we're not in a fog volume, the closest
  ...
  if (MSurf_Flags(surfID) & (SURFDRAW_HASLIGHTSYTLES | SURFDRAW_HASDLIGHT)) {
    pRenderList->m_DlightSurfaces[sortGroup].AddToTail(surfID);
    if (!DlightSurfaceSetQueuingFlag(surfID)) {
      pRenderList->m_DispAlphaSortList.AddSurfaceToTail(
          surfID, sortGroup, pRenderList->m_VisibleLeaves.Count() - 1);
    }
  } else {
    pRenderList->m_DispAlphaSortList.AddSurfaceToTail(
...
void Shader_DisplacementSurface(CWorldRenderList *pRenderList,
                                SurfaceHandle_t surfID) {
  Assert(SurfaceHasDispInfo(surfID));
  ...
  if (MSurf_Flags(surfID) & (SURFDRAW_HASLIGHTSYTLES | SURFDRAW_HASDLIGHT)) {
    pRenderList->m_DlightSurfaces[nSortGroup].AddToTail(surfID);
    if (!DlightSurfaceSetQueuingFlag(surfID)) {
      pRenderList->m_DispSortList.AddSurfaceToTail(surfID, nSortGroup,
                                                   nMaterialSortID);
    }
  } else {
    pRenderList->m_DispSortList.AddSurfaceToTail(surfID, nSortGroup,
...
//-----------------------------------------------------------------------------
void Shader_DrawSurfaceDynamic(IMatRenderContext *pRenderContext,
                               SurfaceHandle_t surfID, bool bShadowDepth) {
  if (!SurfaceHasPrims(surfID)) {
    IMesh *pMesh = pRenderContext->GetDynamicMesh();
    CMeshBuilder meshBuilder;
    meshBuilder.Begin(pMesh, MATERIAL_POLYGON, MSurf_VertCount(surfID));
    BuildMSurfaceVertexArrays(host_state.worldbrush, surfID, OVERBRIGHT,
                              meshBuilder);
    meshBuilder.End();
    pMesh->Draw();
    return;
  ...
  mprimitive_t *pPrim =
      &host_state.worldbrush->primitives[MSurf_FirstPrimID(surfID)];
  ...
  if (pPrim->vertCount) {
    ...
    for (int i = 0; i < MSurf_NumPrims(surfID); i++, pPrim++) {
      ...
      switch (pPrim->type) {
        case PRIM_TRILIST:
          meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, pPrim->vertCount,
                            pPrim->indexCount);
          break;
        case PRIM_TRISTRIP:
          meshBuilder.Begin(pMesh, MATERIAL_TRIANGLE_STRIP, pPrim->vertCount,
                            pPrim->indexCount);
          break;
        default:
          Assert(0);
    ...
    IMesh *pMesh = pRenderContext->GetDynamicMesh();
    CMeshBuilder meshBuilder;
    meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, MSurf_VertCount(surfID),
                      pPrim->indexCount);
    BuildMSurfaceVertexArrays(host_state.worldbrush, surfID, OVERBRIGHT,
                              meshBuilder);
    for (int primIndex = 0; primIndex < pPrim->indexCount; primIndex++) {
      meshBuilder.FastIndex(
          host_state.worldbrush->primindices[pPrim->firstIndex + primIndex]);
    }
  ...
  if (g_pMaterialSystemConfig->nFullbright == 1) {
    if (MSurf_Flags(surfID) & SURFDRAW_BUMPLIGHT) {
      pRenderContext->BindLightmapPage(
          MATERIAL_SYSTEM_LIGHTMAP_PAGE_WHITE_BUMP);
    } else {
    ...
    pRenderContext->BindLightmapPage(
        materialSortInfoArray[MSurf_MaterialSortID(surfID)].lightmapPageID);
  }
}
...
//-----------------------------------------------------------------------------
void Shader_SetChainTextureState(IMatRenderContext *pRenderContext,
                                 SurfaceHandle_t surfID,
  ...
  if (bShadowDepth) {
    ...
    if (nAlphaTest == 1) {
      ...
      IMaterialVar *pOriginalTextureVar =
          pDrawMaterial->FindVarFast("$basetexture", &originalTextureVarCache);
      static unsigned int originalTextureFrameVarCache = 0;
      IMaterialVar *pOriginalTextureFrameVar =
          pDrawMaterial->FindVarFast("$frame", &originalTextureFrameVarCache);
      static unsigned int originalAlphaRefCache = 0;
      IMaterialVar *pOriginalAlphaRefVar = pDrawMaterial->FindVarFast(
          "$AlphaTestReference", &originalAlphaRefCache);

      static unsigned int textureVarCache = 0;
      IMaterialVar *pTextureVar =
          pDepthWriteMaterial->FindVarFast("$basetexture", &textureVarCache);
      static unsigned int textureFrameVarCache = 0;
      IMaterialVar *pTextureFrameVar =
          pDepthWriteMaterial->FindVarFast("$frame", &textureFrameVarCache);
      static unsigned int alphaRefCache = 0;
      IMaterialVar *pAlphaRefVar = pDepthWriteMaterial->FindVarFast(
          "$AlphaTestReference", &alphaRefCache);
...
void Shader_DrawChainsStatic(const CMSurfaceSortList &sortList, int nSortGroup,
  ...
  const CUtlVector<surfacesortgroup_t *> &groupList =
      sortList.GetSortList(nSortGroup);
  ...
  while (listIndex < count) {
    ...
    IIndexBuffer *pBuildIndexBuffer = pRenderContext->GetDynamicIndexBuffer(
        MATERIAL_INDEX_FORMAT_16BIT, false);
    indexBufferBuilder.Begin(pBuildIndexBuffer, nMaxIndices);
    IVertexBuffer *pLastVertexBuffer = NULL;
#else
    IMesh *pBuildMesh =
        pRenderContext->GetDynamicMesh(false, g_WorldStaticMeshes[sortID]);
    ...
    for (; listIndex < count; listIndex++) {
...
#endif
      {
        if (meshList.Count() < MAX_VERTEX_FORMAT_CHANGES - 1) {
          ...
          if (bWarn) {
            Warning(
                "Too many vertex format changes in frame, whole world not "
                "rendered\n");
            bWarn = false;
          }
          continue;
    ...
    // UNDONE: Make the traversal happen so that they are already sorted when
    // you get here. NOTE: Profiled in a fairly complex map.  This is not even
    // costing 0.01ms / frame!
...
#ifdef NEWMESH
        if (meshList[meshMap[i]].pVertexBuffer <
            meshList[meshMap[i - 1]].pVertexBuffer)
#else
    ...
    for (int m = 0; m < meshTotal; m++) {
      ...
      IMaterial *pBindMaterial =
          materialSortInfoArray[MSurf_MaterialSortID(
                                    batchList[mesh.firstbatch].surfID)]
              .material;
#ifdef NEWMESH
...
#endif
#ifdef NEWMESH
      IIndexBuffer *pIndexBuffer = pRenderContext->GetDynamicIndexBuffer(
          MATERIAL_INDEX_FORMAT_16BIT, false);
#else
      //			IMesh *pMesh = pRenderContext->GetDynamicMesh(
      // false, mesh.pMesh, pBuildMesh, pBindMaterial );
      ...
      for (int b = 0; b < mesh.numbatches; b++) {
        ...
        IMaterial *pDrawMaterial =
            materialSortInfoArray[MSurf_MaterialSortID(batch.surfID)].material;
        ...
        if (bShadowDepth) {
          ...
          IMaterial *pDepthWriteMaterial =
              g_pMaterialDepthWrite[nAlphaTest][nNoCull];

          if (nAlphaTest == 1) {
            static unsigned int originalTextureVarCache = 0;
            IMaterialVar *pOriginalTextureVar = pDrawMaterial->FindVarFast(
                "$basetexture", &originalTextureVarCache);
            static unsigned int originalTextureFrameVarCache = 0;
            IMaterialVar *pOriginalTextureFrameVar = pDrawMaterial->FindVarFast(
                "$frame", &originalTextureFrameVarCache);
            static unsigned int originalAlphaRefCache = 0;
            IMaterialVar *pOriginalAlphaRefVar = pDrawMaterial->FindVarFast(
                "$AlphaTestReference", &originalAlphaRefCache);

            static unsigned int textureVarCache = 0;
            IMaterialVar *pTextureVar = pDepthWriteMaterial->FindVarFast(
                "$basetexture", &textureVarCache);
            static unsigned int textureFrameVarCache = 0;
            IMaterialVar *pTextureFrameVar = pDepthWriteMaterial->FindVarFast(
                "$frame", &textureFrameVarCache);
            static unsigned int alphaRefCache = 0;
            IMaterialVar *pAlphaRefVar = pDepthWriteMaterial->FindVarFast(
                "$AlphaTestReference", &alphaRefCache);
          ...
          if (skipBind) {
            if (MSurf_Flags(batch.surfID) & SURFDRAW_BUMPLIGHT) {
              ...
              pRenderContext->BindLightmapPage(
                  MATERIAL_SYSTEM_LIGHTMAP_PAGE_WHITE);
            }
          } else {
            pRenderContext->BindLightmapPage(
                materialSortInfoArray[MSurf_MaterialSortID(batch.surfID)]
                    .lightmapPageID);
          }
        }
#ifdef NEWMESH
        ...
        Warning(
            "pRenderContext->Draw( MATERIAL_TRIANGLES, batch.firstIndex = %d, "
            "batch.numIndex = %d )\n",
            (int)batch.firstIndex, (int)batch.numIndex);
        pRenderContext->Draw(MATERIAL_TRIANGLES, batch.firstIndex,
                             batch.numIndex);
#else
        //				pMesh->Draw( batch.firstIndex,
        // batch.numIndex
        //);
    ...
    // if we get here and pLast mesh is NULL and we rendered somthing, we need
    // to loop
#ifdef NEWMESH
...
// surface
//-----------------------------------------------------------------------------
typedef void (*SurfaceDebugFunc_t)(SurfaceHandle_t surfID,
...
//-----------------------------------------------------------------------------
void Shader_DrawSurfaceDebuggingInfo(
    const CUtlVector<msurface2_t *> &surfaceList, SurfaceDebugFunc_t func) {
...
//-----------------------------------------------------------------------------
void Shader_DrawWireframePolygons(
    const CUtlVector<msurface2_t *> &surfaceList) {
  int nLineCount = 0;
  for (int i = 0; i < surfaceList.Count(); i++) {
  ...
  for (int i = 0; i < surfaceList.Count(); i++) {
    ...
    if (nCount >= 3) {
      int nFirstVertIndex = MSurf_FirstVertIndex(surfID);
      int nVertIndex =
          host_state.worldbrush->vertindices[nFirstVertIndex + nCount - 1];
...
//-----------------------------------------------------------------------------
static void Shader_DrawChainsWireframe(
    const CUtlVector<msurface2_t *> &surfaceList) {
...
//-----------------------------------------------------------------------------
static void Shader_DrawChainNormals(
    const CUtlVector<msurface2_t *> &surfaceList) {
  ...
  for (int i = 0; i < surfaceList.Count(); i++) {
    ...
    for (vertID = 0; vertID < MSurf_VertCount(surfID); ++vertID) {
      int vertIndex =
          pBrushData->vertindices[MSurf_FirstVertIndex(surfID) + vertID];
      Vector &pos = pBrushData->vertexes[vertIndex].position;
      Vector &norm =
          pBrushData->vertnormals[pBrushData->vertnormalindices
                                      [MSurf_FirstVertNormal(surfID) + vertID]];
...
static void Shader_DrawChainBumpBasis(
    const CUtlVector<msurface2_t *> &surfaceList) {
  Vector p, tVect, tangentS, tangentT;
  ...
  for (int i = 0; i < surfaceList.Count(); i++) {
    ...
    for (vertID = 0; vertID < MSurf_VertCount(surfID); ++vertID) {
      ...
      int i;
      for (i = 0; i < 3; i++) {
        worldSpaceBumpBasis[i][0] = g_localBumpBasis[i][0] * tangentS[0] +
                                    g_localBumpBasis[i][1] * tangentS[1] +
                                    g_localBumpBasis[i][2] * tangentS[2];
        worldSpaceBumpBasis[i][1] = g_localBumpBasis[i][0] * tangentT[0] +
                                    g_localBumpBasis[i][1] * tangentT[1] +
                                    g_localBumpBasis[i][2] * tangentT[2];
        worldSpaceBumpBasis[i][2] = g_localBumpBasis[i][0] * norm[0] +
...
static struct CShaderDebug {
  ...
  void TestAnyDebug() {
    anydebug = wireframe || normals || luxels || bumpBasis ||
               (surfaceid != 0) || surfacematerials;
  }
...
static void DrawDebugInformation(const CUtlVector<msurface2_t *> &surfaceList) {
  ...
  if (g_ShaderDebug.surfaceid) {
    ...
    Shader_DrawSurfaceDebuggingInfo(surfaceList, (g_ShaderDebug.surfaceid != 2)
                                                     ? DrawSurfaceID
                                                     : DrawSurfaceIDAsInt);
...
void AddProjectedTextureDecalsToList(CWorldRenderList *pRenderList,
                                     int nSortGroup) {
...
//-----------------------------------------------------------------------------
void Shader_DrawChains(const CWorldRenderList *pRenderList, int nSortGroup,
                       bool bShadowDepth) {
  CMatRenderContextPtr pRenderContext(materials);
...
//-----------------------------------------------------------------------------
void Shader_DrawDispChain(int nSortGroup, const CMSurfaceSortList &list,
...
#endif
    for (int i = pRenderList->m_DlightSurfaces[nSortGroup].Count() - 1; i >= 0;
         --i) {
...
static void ComputeFogVolumeInfo(FogVolumeInfo_t *pFogVolume) {
  ...
  if (leafID < 0 || leafID >= host_state.worldbrush->numleafs) return;

  mleaf_t *pLeaf = &host_state.worldbrush->leafs[leafID];
  pFogVolume->m_FogVolumeID = pLeaf->leafWaterDataID;
  if (pFogVolume->m_FogVolumeID == -1) return;

  pFogVolume->m_InFogVolume = true;

  mleafwaterdata_t *pLeafWaterData =
      &host_state.worldbrush->leafwaterdata[pLeaf->leafWaterDataID];
  ...
  mtexinfo_t *pTexInfo =
      &host_state.worldbrush->texinfo[pLeafWaterData->surfaceTexInfoID];
...
// Performs the z-fill
//-----------------------------------------------------------------------------
static void Shader_WorldZFillSurfChain(const CMSurfaceSortList &sortList,
                                       const surfacesortgroup_t &group,
                                       CMeshBuilder &meshBuilder,
                                       int &nStartVertIn,
...
//-----------------------------------------------------------------------------
static void Shader_WorldShadowDepthFill(CWorldRenderList *pRenderList,
                                        unsigned long flags) {
  ...
  int nMaxIndices = pRenderContext->GetMaxIndicesToRender();
  int nMaxVertices = pRenderContext->GetMaxVerticesToRender(
      g_pMaterialDepthWrite[0][1]);  // opaque, nocull
...
// Performs the z-fill
//-----------------------------------------------------------------------------
static void Shader_WorldZFill(CWorldRenderList *pRenderList,
                              unsigned long flags) {
  // First, count the number of vertices + indices
  int nVertexCount = 0;
  int nIndexCount = 0;
...
//-----------------------------------------------------------------------------
static void Shader_WorldEnd(CWorldRenderList *pRenderList, unsigned long flags,
                            float waterZAdjust) {
  ...
  if (flags & DRAWWORLDLISTS_DRAW_SKYBOX) {
    if (pRenderList->m_bSkyVisible || Map_VisForceFullSky()) {
      if (flags & DRAWWORLDLISTS_DRAW_CLIPSKYBOX) {
        ...
        MaterialHeightClipMode_t nClipMode =
  ...
  for (i = MAX_MAT_SORT_GROUPS; --i >= 0;) {
    ...
    for (int j = pRenderList->m_ShadowHandles[nSortGroup].Count() - 1; j >= 0;
         --j) {
      g_pShadowMgr->AddShadowsOnSurfaceToRenderList(
          pRenderList->m_ShadowHandles[nSortGroup].Element(j));
    }
    pRenderList->m_ShadowHandles[nSortGroup].RemoveAll();

    // Don't stencil or scissor the flashlight if we're rendering to an
    // offscreen view
    bool bFlashlightMask = !((flags & DRAWWORLDLISTS_DRAW_REFRACTION) ||
                             (flags & DRAWWORLDLISTS_DRAW_REFLECTION));
...
//-----------------------------------------------------------------------------
bool Shader_LeafContainsTranslucentSurfaces(IWorldRenderList *pRenderListIn,
                                            int sortIndex,
                                            unsigned long flags) {
  ...
  for (i = 0; i < MAX_MAT_SORT_GROUPS; ++i) {
    ...
    const surfacesortgroup_t &group =
        pRenderList->m_AlphaSortList.GetGroupForSortID(sortGroup, sortIndex);
    if (group.surfaceCount) return true;
    const surfacesortgroup_t &dispGroup =
        pRenderList->m_DispAlphaSortList.GetGroupForSortID(sortGroup,
                                                           sortIndex);
    if (dispGroup.surfaceCount) return true;
  }

  return false;
}

void Shader_DrawTranslucentSurfaces(IWorldRenderList *pRenderListIn,
                                    int sortIndex, unsigned long flags,
                                    bool bShadowDepth) {
  if (!r_drawtranslucentworld.GetBool()) return;
  ...
  for (i = 0; i < MAX_MAT_SORT_GROUPS; ++i) {
    ...
    const surfacesortgroup_t &dispGroup =
        pRenderList->m_DispAlphaSortList.GetGroupForSortID(sortGroup,
                                                           sortIndex);
    // Empty? skip...
    if (!group.surfaceCount && !dispGroup.surfaceCount) continue;
    ...
    for (int listIndex = surfaceList.Count(); --listIndex >= 0;) {
      ...
      //			NOTE: Since a static vb/dynamic ib IMesh doesn't
      // buffer, we shouldn't use this 			since it causes a lock
      // and drawindexedprimitive per surface! (gary)
      ...
      OverlayMgr()->AddFragmentListToRenderList(
          i, MSurf_OverlayFragmentList(surfID), false);
      ...
      g_pShadowMgr->DrawFlashlightDecalsOnSingleSurface(surfID,
                                                        bFlashlightMask);
    ...
    // non-displacement surfaces because most likely the displacement will
    // always be in front (or at least not behind) the non-displacement
    // translucent surfaces that exist in the same leaf.

    // Draws translucent displacement surfaces

    surfaceList.RemoveAll();
    surfaceList.EnsureCapacity(dispGroup.surfaceCount);
    MSL_FOREACH_SURFACE_IN_GROUP_BEGIN(pRenderList->m_DispAlphaSortList,
                                       dispGroup, surfID) {
      surfaceList.AddToTail(surfID);
    }
    MSL_FOREACH_SURFACE_IN_GROUP_END()

    DispInfo_RenderList(i, surfaceList.Base(), surfaceList.Count(),
                        g_EngineRenderer->ViewGetCurrent().m_bOrtho, flags,
                        bShadowDepth);
  }
}
...
//-----------------------------------------------------------------------------
static inline void DrawDisplacementsInLeaf(CWorldRenderList *pRenderList,
                                           mleaf_t *pLeaf) {
...
//-----------------------------------------------------------------------------
static inline void UpdateVisibleLeafLists(CWorldRenderList *pRenderList,
                                          mleaf_t *pLeaf) {
  // Consistency check...
  MEM_ALLOC_CREDIT();
...
static void SOURCE_FASTCALL R_DrawLeaf(CWorldRenderList *pRenderList,
  ...
  for (i = 0; i < nSurfaceCount; ++i) {
    ...
    // We seem to be taking a huge hit here for referencing the surface for the
    // first time.
...
// Input  : *node - BSP node
//-----------------------------------------------------------------------------
static void R_RecursiveWorldNodeNoCull(CWorldRenderList *pRenderList,
                                       mnode_t *node, int nCullMask) {
  int side;
  cplane_t *plane;
  float dot;
    ...
    if (nCullMask != FRUSTUM_SUPPRESS_CLIPPING) {
      if (node->contents >= -1) {
        if ((nCullMask != 0) || (node->area > 0)) {
          if (R_CullNode(&g_Frustum, node, nCullMask)) return;
        }
      } else {
...
static void R_RecursiveWorldNode(CWorldRenderList *pRenderList, mnode_t *node,
                                 int nCullMask) {
  int side;
  cplane_t *plane;
  float dot;
  ...
  while (true) {
    ...
    for (; i < nLastSurface; ++i, ++surfID) {
      // Only render things at this node that have previously been marked as
      // visible
      if (!VisitedSurface(visitedSurfs, i)) continue;
      ...
      // Only func_detail render at leaves. In the case of normal world
      // surfaces, we only want to render them if they intersect a visible leaf.
      ...
      if (!(nFlags & SURFDRAW_UNDERWATER) &&
          (side ^ !!(nFlags & SURFDRAW_PLANEBACK)))
        continue;  // wrong side
...
inline float R_GetWaterHeight(int nFogVolume) {
  ...
  mleafwaterdata_t *pLeafWaterData =
      &host_state.worldbrush->leafwaterdata[nFogVolume];
...
void R_SetFogVolumeState(int fogVolume, bool useHeightFog) {
  ...
  mleafwaterdata_t *pLeafWaterData =
      &host_state.worldbrush->leafwaterdata[fogVolume];
  IMaterialVar *pFogColorVar = pMaterial->FindVar("$fogcolor", NULL);
  IMaterialVar *pFogEnableVar = pMaterial->FindVar("$fogenable", NULL);
  IMaterialVar *pFogStartVar = pMaterial->FindVar("$fogstart", NULL);
  IMaterialVar *pFogEndVar = pMaterial->FindVar("$fogend", NULL);
...
static inline bool R_CullNodeTopView(mnode_t *pNode) {
  Vector2D delta, size;
  Vector2DSubtract(pNode->m_vecCenter.AsVector2D(), s_OrthographicCenter,
                   delta);
  Vector2DAdd(pNode->m_vecHalfDiagonal.AsVector2D(), s_OrthographicHalfDiagonal,
              size);
  ...
  do {
    ...
    if (s_ShaderConvars.m_bDrawWorld) {
      ...
      for (int i = 0; i < node->numsurfaces; i++, surfID++) {
        ...
        // If a surface is marked to draw at a node, then it's not a
        // func_detail. Only func_detail render at leaves. In the case of normal
        // world surfaces, we only want to render them if they intersect a
        // visible leaf.
        Assert((MSurf_Flags(surfID) & SURFDRAW_NODE));
        ...
        if (!(MSurf_Flags(surfID) & SURFDRAW_TRANS))
          //				if (
          //! surf->texinfo->material->IsTranslucent()
          //)
          Shader_WorldSurface(pRenderList, surfID);
      }
    }
...
void R_BuildWorldLists(IWorldRenderList *pRenderListIn, WorldListInfo_t *pInfo,
                       int iForceViewLeaf, const VisOverrideData_t *pVisData,
  ...
  if (!r_drawtopview) {
    ...
    if (bShadowDepth) {
      R_RecursiveWorldNodeNoCull(pRenderList, host_state.worldbrush->nodes,
                                 r_frustumcullworld.GetBool()
                                     ? FRUSTUM_CLIP_ALL
                                     : FRUSTUM_SUPPRESS_CLIPPING);
    } else {
      R_RecursiveWorldNode(pRenderList, host_state.worldbrush->nodes,
                           r_frustumcullworld.GetBool()
                               ? FRUSTUM_CLIP_ALL
                               : FRUSTUM_SUPPRESS_CLIPPING);
    }
  ...
  // UNDONE: This is done here because monitor drawing happens before
  // SceneBegin() in the client DLL investigate moving/fixing this
  if (!bShadowDepth) {
...
class CVisibleFogVolumeQuery {
 public:
  void FindVisibleFogVolume(const Vector &vecViewPoint, int *pVisibleFogVolume,
...
//-----------------------------------------------------------------------------
void CVisibleFogVolumeQuery::FindVisibleFogVolume(const Vector &vecViewPoint,
...
bool CVisibleFogVolumeQuery::RecursiveGetVisibleFogVolume(mnode_t *node) {
  ...
  if (R_CullNode(&g_Frustum, node, fixmeTempRemove)) return true;
...
//-----------------------------------------------------------------------------
void R_GetVisibleFogVolume(const Vector &vEyePoint,
  ...
  if (pLeaf->leafWaterDataID != -1) {
    ...
    pInfo->m_pFogVolumeMaterial =
        R_GetFogVolumeMaterial(pInfo->m_nVisibleFogVolume, true);
    ...
    if (fast_fogvolume.GetBool() &&
        host_state.worldbrush->numleafwaterdata == 1) {
      pInfo->m_nVisibleFogVolume = 0;
      pInfo->m_nVisibleFogVolumeLeaf =
          host_state.worldbrush->leafwaterdata[0].firstLeafIndex;
    } else {
      CVisibleFogVolumeQuery query;
      query.FindVisibleFogVolume(vEyePoint, &pInfo->m_nVisibleFogVolume,
                                 &pInfo->m_nVisibleFogVolumeLeaf);
    }

    pInfo->m_bEyeInFogVolume = false;
    pInfo->m_pFogVolumeMaterial =
        R_GetFogVolumeMaterial(pInfo->m_nVisibleFogVolume, false);
    pInfo->m_flWaterHeight = R_GetWaterHeight(pInfo->m_nVisibleFogVolume);
  } else {
    ClearFogInfo(pInfo);
  }
  ...
  if (host_state.worldbrush->m_LeafMinDistToWater) {
    pInfo->m_flDistanceToWater =
        (float)host_state.worldbrush->m_LeafMinDistToWater[nLeafID];
  } else {
...
void R_DrawWorldLists(IWorldRenderList *pRenderListIn, unsigned long flags,
                      float waterZAdjust) {
  CWorldRenderList *pRenderList =
      assert_cast<CWorldRenderList *>(pRenderListIn);
...
void Shader_DrawLightmapPageSurface(SurfaceHandle_t surfID, float red,
  ...
  materials->GetLightmapPageSize(
      SortInfoToLightmapPage(MSurf_MaterialSortID(surfID)), &lightmapPageWidth,
      &lightmapPageHeight);
...
void Shader_DrawLightmapPageChains(IWorldRenderList *pRenderListIn,
                                   int pageId) {
  CWorldRenderList *pRenderList =
      assert_cast<CWorldRenderList *>(pRenderListIn);
  for (int j = 0; j < MAX_MAT_SORT_GROUPS; ++j) {
...
class CBrushSurface : public IBrushSurface {
 public:
  ...
  virtual void ComputeTextureCoordinate(const Vector &worldPos,
                                        Vector2D &texCoord);
  virtual void ComputeLightmapCoordinate(const Vector &worldPos,
...
// Computes texture coordinates + lightmap coordinates given a world position
//-----------------------------------------------------------------------------
void CBrushSurface::ComputeTextureCoordinate(const Vector &worldPos,
                                             Vector2D &texCoord) {
  SurfComputeTextureCoordinate(m_Ctx, m_SurfaceID, worldPos, texCoord);
}

void CBrushSurface::ComputeLightmapCoordinate(const Vector &worldPos,
                                              Vector2D &lightmapCoord) {
...
bool Shader_DrawBrushSurfaceOverride(IMatRenderContext *pRenderContext,
                                     SurfaceHandle_t surfID,
  ...
  return s_pBrushRenderOverride->RenderBrushModelSurface(baseentity,
                                                         &brushSurface);
}
...
void Shader_BrushSurface(SurfaceHandle_t surfID, model_t *model,
  ...
  if (!s_pBrushRenderOverride) {
    ...
    //		NOTE: Since a static vb/dynamic ib IMesh doesn't buffer, we
    // shouldn't use this 		since it causes a lock and
    // drawindexedprimitive per surface! (gary)
    // Shader_DrawSurfaceStatic( surfID );
    Shader_DrawSurfaceDynamic(pRenderContext, surfID, false);
    ...
    drawDecals =
        Shader_DrawBrushSurfaceOverride(pRenderContext, surfID, baseentity);
  }
...
class CBrushBatchRender {
 public:
  // These are the compact structs produced by the brush render cache.  The goal
  // is to have a compact list of drawing instructions for drawing an opaque
  // brush model in the most optimal order. These structs contain ONLY the
  // opaque surfaces of a brush model.
  struct brushrendersurface_t {
    short surfaceIndex;
    short planeIndex;
  };

  // a batch is a list of surfaces with the same material - they can be drawn
  // with one call to the materialsystem
  ...
  // This is the top-level struct containing all data necessary to render an
  // opaque brush model in optimal order
  struct brushrender_t {
    // UNDONE: Compact these arrays into a single allocation
    // UNDONE: Compact entire struct to a single allocation?  Store
    // brushrender_t * in the linked list?
    void Free() {
      delete[] pPlanes;
      delete[] pMeshes;
      delete[] pBatches;
  ...
  // Surfaces are stored in a list like this temporarily for sorting purposes
  // only.  The compact structs do not store these.
  ...
  // These are the compact structs produced for translucent brush models.  These
  // structs contain only the translucent surfaces of a brush model.
  ...
  // A node is the list of batches that can be drawn without sorting errors.
  // When no decals are present, surfaces from the next node may be appended to
  // this one to improve performance without causing sorting errors.
  struct transnode_t {
    short firstBatch;
    short batchCount;
  ...
  // This is the top-level struct containing all data necessary to render a
  // translucent brush model in optimal order. NOTE: Unlike the opaque struct,
  // the order of the batches is view-dependent, so caching this is pointless
  // since the view usually changes.
  struct transrender_t {
    ...
    transbatch_t *pLastBatch;  // These are used to append surfaces to existing
  ...
  void DrawTranslucentBrushModel(model_t *model, IClientEntity *baseentity) {
    ...
    BuildTransLists_r(render, model,
  ...
  void AddSurfaceToBatch(transrender_t &render, transnode_t *pNode,
                         transbatch_t *pBatch, SurfaceHandle_t surfID) {
    pBatch->surfaceCount++;
  ...
  void AddTransBatch(transrender_t &render, SurfaceHandle_t surfID) {
    transbatch_t &batch = render.batches[render.pLastNode->firstBatch +
    ...
    if (sortList.Count()) {
      ...
      for (int i = 0; i < sortList.Count(); i++) {
        ...
        if (lastSortID == sortID) {
          // can be drawn in a single call with the current list of surfaces,
          // append
          AddSurfaceToBatch(render, render.pLastNode, render.pLastBatch,
                            surfID);
        } else {
  ...
  void DrawTransLists(transrender_t &render, void *pProxyData) {
    ...
    for (int i = 0; i < render.nodeCount; i++) {
      ...
      for (j = 0; j < node.batchCount; j++) {
        ...
        if (!skipLight) {
          pRenderContext->BindLightmapPage(
              materialSortInfoArray[batch.sortID].lightmapPageID);
        }
        ...
        IMesh *pBuildMesh = pRenderContext->GetDynamicMesh(
            false, g_WorldStaticMeshes[batch.sortID], NULL, NULL);
      ...
      if (node.decalSurfaceCount) {
        for (j = 0; j < node.decalSurfaceCount; j++) {
          SurfaceHandle_t surfID =
              render.decalSurfaces[node.firstDecalSurface + j];
          Assert(!(MSurf_Flags(surfID) & SURFDRAW_NODRAW));
          if (SurfaceHasDecals(surfID)) {
            DecalSurfaceAdd(surfID, BRUSHMODEL_DECAL_SORT_GROUP);
          }
        ...
        // This order relative to the surfaces is important for translucency to
        // work correctly.
  ...
  static int __cdecl SurfaceCmp(const surfacelist_t *s0,
                                const surfacelist_t *s1);

  void LevelInit();
  brushrender_t *FindOrCreateRenderBatch(model_t *pModel);
  void DrawOpaqueBrushModel(IClientEntity *baseentity, model_t *model,
                            const Vector &origin, bool bShadowDepth);
  void DrawTranslucentBrushModel(IClientEntity *baseentity, model_t *model,
                                 const Vector &origin, bool bShadowDepth);
...
int __cdecl CBrushBatchRender::SurfaceCmp(const surfacelist_t *s0,
                                          const surfacelist_t *s1) {
...
// Create a compact, optimal list of rendering commands for the opaque parts of
// a brush model NOTE: This just skips translucent surfaces assuming a separate
// transrender_t pass!
CBrushBatchRender::brushrender_t *CBrushBatchRender::FindOrCreateRenderBatch(
    model_t *pModel) {
  ...
  memcpy(render.pPlanes, planeList.Base(),
         sizeof(cplane_t *) * planeList.Count());
...
//-----------------------------------------------------------------------------
void CBrushBatchRender::DrawOpaqueBrushModel(IClientEntity *baseentity,
                                             model_t *model,
                                             const Vector &origin,
                                             bool bShadowDepth) {
  ...
  // NOTE: Backface culling is almost no perf gain.  Can be removed from brush
  // model rendering. Check the shared planes once
  for (i = 0; i < pRender->planeCount; i++) {
    float dot = DotProduct(modelorg, pRender->pPlanes[i]->normal) -
                pRender->pPlanes[i]->dist;
    backface[i] =
        (!bShadowDepth && (dot < BACKFACE_EPSILON))
            ? true
            : false;  // don't backface cull when rendering to shadow map
  }
  ...
  for (i = 0; i < pRender->meshCount; i++) {
    ...
    for (int j = 0; j < mesh.batchCount; j++) {
      ...
      for (k = 0; k < batch.surfaceCount; k++) {
        brushrendersurface_t &surface =
            pRender->pSurfaces[batch.firstSurface + k];
        if (!backface[surface.planeIndex]) break;
      }
      ...
      if (bShadowDepth) {
        ...
        if (nAlphaTest == 1) {
          static unsigned int originalTextureVarCache = 0;
          IMaterialVar *pOriginalTextureVar = batch.pMaterial->FindVarFast(
              "$basetexture", &originalTextureVarCache);
          static unsigned int originalTextureFrameVarCache = 0;
          IMaterialVar *pOriginalTextureFrameVar = batch.pMaterial->FindVarFast(
              "$frame", &originalTextureFrameVarCache);
          static unsigned int originalAlphaRefCache = 0;
          IMaterialVar *pOriginalAlphaRefVar = batch.pMaterial->FindVarFast(
              "$AlphaTestReference", &originalAlphaRefCache);
      ...
      for (; k < batch.surfaceCount; k++) {
        brushrendersurface_t &surface =
            pRender->pSurfaces[batch.firstSurface + k];
        if (backface[surface.planeIndex]) continue;
      ...
      indexBufferBuilder.End(
          false);  // this one is broken (opaque brush model. .tv)
      pRenderContext->BindVertexBuffer(
          0, g_WorldStaticMeshes[batch.sortID], 0,
          g_WorldStaticMeshes[batch.sortID]->GetVertexFormat());
      pRenderContext->BindIndexBuffer(pBuildIndexBuffer, 0);
      pRenderContext->Draw(
          MATERIAL_TRIANGLES, 0,
          pBuildIndexBuffer->NumIndices());  // batch.indexCount );
#else
      meshBuilder.End(false, true);
#endif
...
//-----------------------------------------------------------------------------
void CBrushBatchRender::DrawTranslucentBrushModel(IClientEntity *baseentity,
                                                  model_t *model,
                                                  const Vector &origin,
                                                  bool bShadowDepth) {
...
void CBrushBatchRender::DrawBrushModelShadow(model_t *model,
  ...
  IMesh *pMesh = pRenderContext->GetDynamicMesh();
  CMeshBuilder meshBuilder;
  meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, pRender->totalVertexCount,
                    pRender->totalIndexCount);
...
void R_Surface_LevelInit() {
  ...
  g_MaxLeavesVisible = 1024;
}

void R_Surface_LevelShutdown() { CWorldRenderList::PurgeAll(); }

//-----------------------------------------------------------------------------
static void R_DrawBrushModel_Override(IClientEntity *baseentity, model_t *model,
                                      const Vector &origin) {
  ...
  if (g_ShaderDebug.anydebug) {
    CUtlVector<msurface2_t *> surfaceList;
    surfID = SurfaceHandleFromIndex(model->brush.firstmodelsurface,
                                    model->brush.pShared);
    for (int i = 0; i < model->brush.nummodelsurfaces; i++, surfID++) {
      surfaceList.AddToTail(surfID);
    }
...
int R_MarkDlightsOnBrushModel(model_t *model, IClientRenderable *pRenderable) {
  ...
  if (g_bActiveDlights) {
    ...
    g_BrushToWorldMatrix.SetupMatrixOrgAngles(pRenderable->GetRenderOrigin(),
    ...
    for (int k = 0; k < MAX_DLIGHTS; k++) {
      ...
      if (IsBoxIntersectingSphereExtents(
              node->m_vecCenter, node->m_vecHalfDiagonal, cl_dlights[k].origin,
              cl_dlights[k].GetRadius())) {
...
void Shader_BrushEnd(IMatRenderContext *pRenderContext,
                     VMatrix const *pBrushToWorld, model_t *model,
                     bool bShadowDepth,
                     IClientEntity *baseentity /* = NULL */) {
  if (bShadowDepth) return;
...
class CBrushModelTransform {
 public:
  CBrushModelTransform(const Vector &origin, const QAngle &angles,
    ...
    if (!m_bIdentity) {
      ...
      modelorg =
          g_BrushToWorldMatrix.VMul4x3Transpose(g_EngineRenderer->ViewOrigin());
    }
  }
...
void R_DrawBrushModel(IClientEntity *baseentity, model_t *model,
  ...
  if (s_pBrushRenderOverride) {
    ...
    if (bSort) {
      if (!bShadowDepth) {
        g_BrushBatchRenderer.DrawTranslucentBrushModel(baseentity, model,
                                                       origin, bShadowDepth);
      }
    } else {
      g_BrushBatchRenderer.DrawOpaqueBrushModel(baseentity, model, origin,
                                                bShadowDepth);
    }
  }
...
static bool EnumerateLeafInBox_R(mnode_t *node, EnumLeafBoxInfo_t &info) {
  ...
  if (!IsBoxIntersectingBoxExtents(node->m_vecCenter, node->m_vecHalfDiagonal,
                                   info.m_vecBoxCenter,
                                   info.m_vecBoxHalfDiagonal)) {
    return true;
  }

  if (node->contents >= 0) {
    // if a leaf node, report it to the iterator...
    return info.m_pIterator->EnumerateLeaf(LeafToIndex((mleaf_t *)node),
                                           info.m_nContext);
  }
...
//-----------------------------------------------------------------------------
bool EnumerateLeafInSphere_R(mnode_t *node, EnumLeafSphereInfo_t &info,
                             int nTestFlags) {
  while (true) {
    // no polygons in solid nodes (don't report these leaves either)
    if (node->contents == CONTENTS_SOLID) return true;  // solid
    ...
    if (node->contents >= 0) {
      ...
      if (nTestFlags) {
        if (!IsBoxIntersectingSphereExtents(node->m_vecCenter,
                                            node->m_vecHalfDiagonal,
                                            info.m_vecCenter, info.m_flRadius))
          return true;
      }
      ...
      if (node->contents == -1) {
        ...
        if (nTestFlags & ENUM_SPHERE_TEST_X) {
          float flDelta =
              FloatMakePositive(node->m_vecCenter.x - info.m_vecBoxCenter.x);
          float flSize =
              node->m_vecHalfDiagonal.x + info.m_vecBoxHalfDiagonal.x;
          if (flDelta > flSize) return true;
        ...
        if (nTestFlags & ENUM_SPHERE_TEST_Y) {
          float flDelta =
              FloatMakePositive(node->m_vecCenter.y - info.m_vecBoxCenter.y);
          float flSize =
              node->m_vecHalfDiagonal.y + info.m_vecBoxHalfDiagonal.y;
          if (flDelta > flSize) return true;
        ...
        if (nTestFlags & ENUM_SPHERE_TEST_Z) {
          float flDelta =
              FloatMakePositive(node->m_vecCenter.z - info.m_vecBoxCenter.z);
          float flSize =
              node->m_vecHalfDiagonal.z + info.m_vecBoxHalfDiagonal.z;
          if (flDelta > flSize) return true;
        ...
        // If the box is too small to bother with testing, then blat out the
        // flags
        nTestFlags = 0;
      }
    }
    ...
    if (plane->type <= PLANE_Z) {
      ...
      // Here, we've got a plane which is not axis aligned, so we gotta do more
      // work
...
static bool EnumerateLeavesAlongRay_R(mnode_t *node, Ray_t const &ray,
  ...
  if ((back < 0) == side) {
    return EnumerateLeavesAlongRay_R(node->children[side], ray, start, end,
                                     pEnum, context);
  }
  ...
  // go down front side
  bool ok = EnumerateLeavesAlongRay_R(node->children[side], ray, start, mid,
                                      pEnum, context);
  if (!ok) return ok;

  // go down back side
  return EnumerateLeavesAlongRay_R(node->children[!side], ray, mid, end, pEnum,
                                   context);
}
...
static bool EnumerateLeavesAlongExtrudedRay_R(mnode_t *node, Ray_t const &ray,
  ...
  //	if (t1 >= offset && t2 >= offset)
  {
    return EnumerateLeavesAlongExtrudedRay_R(node->children[0], ray, start, end,
                                             pEnum, context);
  }
  if (t1 < -offset && t2 < -offset) {
    return EnumerateLeavesAlongExtrudedRay_R(node->children[1], ray, start, end,
                                             pEnum, context);
  }
  ...
  if (fabs(t1 - t2) < DIST_EPSILON) {
    ...
    bool ret = EnumerateLeavesAlongExtrudedRay_R(node->children[0], ray, start,
                                                 end, pEnum, context);
    if (!ret) return false;
    return EnumerateLeavesAlongExtrudedRay_R(node->children[1], ray, start, end,
                                             pEnum, context);
...
class CEngineBSPTree : public IEngineSpatialQuery {
 public:
  ...
  bool EnumerateLeavesAtPoint(const Vector &pt, ISpatialLeafEnumerator *pEnum,
                              int context);
  bool EnumerateLeavesInBox(const Vector &mins, const Vector &maxs,
                            ISpatialLeafEnumerator *pEnum, int context);
  bool EnumerateLeavesInSphere(const Vector &center, float radius,
                               ISpatialLeafEnumerator *pEnum, int context);
...
static ConVar opt_EnumerateLeavesFastAlgorithm(
    "opt_EnumerateLeavesFastAlgorithm", "1", FCVAR_NONE,
    "Use the new SIMD version of CEngineBSPTree::EnumerateLeavesInBox.");

bool CEngineBSPTree::EnumerateLeavesInBox(const Vector &mins,
                                          const Vector &maxs,
                                          ISpatialLeafEnumerator *pEnum,
                                          int context) {
  if (!host_state.worldmodel) return false;
...
bool CEngineBSPTree::EnumerateLeavesInSphere(const Vector &center, float radius,
  ...
  return EnumerateLeafInSphere_R(host_state.worldbrush->nodes, info,
                                 ENUM_SPHERE_TEST_ALL);
}

bool CEngineBSPTree::EnumerateLeavesAlongRay(Ray_t const &ray,
                                             ISpatialLeafEnumerator *pEnum,
                                             int context) {
  if (!ray.m_IsSwept) {
    Vector mins, maxs;
  ...
  if (ray.m_IsRay) {
    return EnumerateLeavesAlongRay_R(host_state.worldbrush->nodes, ray, 0.0f,
                                     1.0f, pEnum, context);
  } else {
    return EnumerateLeavesAlongExtrudedRay_R(host_state.worldbrush->nodes, ray,
                                             0.0f, 1.0f, pEnum, context);
  }
}
