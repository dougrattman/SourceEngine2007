...
// If pFilename is 0, no instrumentation is performed.
...
void ServerDTI_RegisterNetworkStateChange(SendTable *pTable,
                                          bool bStateChanged);
...
inline CServerDTITimer::CServerDTITimer(const SendTable *pTable,
                                        ServerDTITimerType type) {
...
inline CServerDTITimer::~CServerDTITimer() {
  if (g_bServerDTIEnabled && m_pTable) {
    m_Timer.End();
    extern void _ServerDTI_HookTimer(const SendTable *pTable,
...
inline void ServerDTI_RegisterNetworkStateChange(SendTable *pTable,
                                                 bool bStateChanged) {
  if (g_bServerDTIEnabled) {
    extern void _ServerDTI_RegisterNetworkStateChange(SendTable * pTable,
                                                      bool bStateChanged);
