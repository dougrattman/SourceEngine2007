...
class CProfileHierarchyPanel : public vgui::CTreeViewListControl {
 ...
 public:
  ...
  struct PanelEntry_t {
    PanelEntry_t() : label(0), dataname(UTL_INVAL_SYMBOL) {}
  ...
  virtual void SetItemColumnColors(int id, int col, const Color &fg,
                                   const Color &bg);
 ...
 public:
  ...
  static bool PanelsLessFunc(const ColumnPanels_t &lhs,
                             const ColumnPanels_t &rhs) {
...
class CVProfPanel : public vgui::Frame {
 ...
 private:
  ...
  int UpdateVProfTreeEntry(KeyValues *pKeyValues, CVProfNode *pNode,
                           int parent);
