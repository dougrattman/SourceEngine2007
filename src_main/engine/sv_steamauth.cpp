...
ConVar sv_master_share_game_socket(
    "sv_master_share_game_socket", "1", 0,
...
CSteam3 &Steam3Server() { return s_Steam3Server; }
...
void CSteam3::Activate() {
  ...
  // NS_SERVER port and don't tell Steam that we have a game server.
  uint16_t usGamePort;
  ...
  uint16_t usSpectatorPort = NET_GetUDPPort(NS_HLTV);
  if (!hltv) usSpectatorPort = 0;

  uint16_t usMasterServerUpdaterPort;
  ...
  if (!SteamGameServer_Init(
          m_unIP,
          m_usPort + 1,  // Steam lives on -steamport + 1, master server updater
                         // lives on -steamport.
          usGamePort, usSpectatorPort, usMasterServerUpdaterPort, m_eServerMode,
          g_iSteamAppID, gamedir, gpszVersionString)) {
    Warning("*********************************************************\n");
...
bool CSteam3::CompareUserID(const USERID_t &id1, const USERID_t &id2) {
  if (id1.idtype != id2.idtype) return false;

  switch (id1.idtype) {
    case IDTYPE_STEAM:
    case IDTYPE_VALVE: {
      return (id1.uid.steamid.m_SteamInstanceID ==
                  id2.uid.steamid.m_SteamInstanceID &&
              id1.uid.steamid.m_SteamLocalUserID.As64bits ==
                  id2.uid.steamid.m_SteamLocalUserID.As64bits);
    }
    default:
      break;
  }
...
bool CSteam3::CheckForDuplicateSteamID(const CBaseClient *client) {
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    ...
    if (!cl->IsConnected() || cl->IsFakeClient()) continue;

    if (cl->GetNetworkID().idtype != IDTYPE_STEAM) continue;

    // don't compare this client against himself in the list
    if (client == cl) continue;

    if (!CompareUserID(client->GetNetworkID(), cl->GetNetworkID())) continue;
...
void CSteam3::OnLogonSuccess(LogonSuccess_t *pLogonSuccess) {
  ...
  // OnLogonSuccess() gets called each time we logon, so if we get dropped this
  // gets called again and we get need to retell the AM our details
  SendUpdatedServerDetails();
}
...
void CSteam3::OnGSClientSteam2Deny(GSClientSteam2Deny_t *pGSClientSteam2Deny) {
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    const IClient *cl = sv.GetClient(i);
    if ((uint32_t)cl->GetUserID() == pGSClientSteam2Deny->m_UserID) {
  ...
  OnGSClientDenyHelper(client, (EDenyReason)pGSClientSteam2Deny->m_eSteamError,
                       NULL);
}

//-----------------------------------------------------------------------------
// Purpose: Called when Steam2 has processed a user login an accepts it (Steam3
// login is still pending however)
//-----------------------------------------------------------------------------
void CSteam3::OnGSClientSteam2Accept(
    GSClientSteam2Accept_t *pGSClientSteam2Accept) {
  if (!BIsActive()) return;
  ...
  for (int i = 0; i < sv.GetClientCount(); i++) {
    const IClient *cl = sv.GetClient(i);
    if ((uint32_t)cl->GetUserID() == pGSClientSteam2Accept->m_UserID) {
      client = (CBaseClient *)cl;
      break;
    }
  }
  ...
  if (Filter_IsUserBanned(client->GetNetworkID())) {
    sv.RejectConnection(client->GetNetChannel()->GetRemoteAddress(),
                        "You have been banned from this server\n");
    client->Disconnect(
        va("STEAM UserID %s is banned", client->GetNetworkIDString()));
    ...
    sprintf(msg, "\"%s<%i><%s><>\" STEAM USERID validated\n",
            client->GetClientName(), client->GetUserID(),
            client->GetNetworkIDString());

    DevMsg("%s", msg);
    g_Log.Printf("%s", msg);

    g_pServerPluginHandler->NetworkIDValidated(client->GetClientName(),
                                               client->GetNetworkIDString());

    // Tell IServerGameClients if its version is high enough.
    if (g_iServerGameClientsVersion >= 4) {
      serverGameClients->NetworkIDValidated(client->GetClientName(),
                                            client->GetNetworkIDString());
    }
  }
}
...
void CSteam3::OnGSClientDeny(GSClientDeny_t *pGSClientDeny) {
  ...
  OnGSClientDenyHelper(cl, pGSClientDeny->m_eDenyReason,
...
//			eDenyReason - reason
//			pchOptionalText - some kicks also have a string with
// them
//-----------------------------------------------------------------------------
void CSteam3::OnGSClientDenyHelper(CBaseClient *cl, EDenyReason eDenyReason,
  ...
  switch (eDenyReason) {
    ...
    case k_EDenyNotLoggedOn:
      if (!BLanOnly()) cl->Disconnect(INVALID_STEAM_LOGON);
      break;
    case k_EDenyLoggedInElseWhere:
      if (!BLanOnly()) cl->Disconnect(INVALID_STEAM_LOGGED_IN_ELSEWHERE);
      break;
    case k_EDenyNoLicense:
      ...
      break;
    case k_EDenyCheater:
      if (!BLanOnly()) cl->Disconnect(INVALID_STEAM_VACBANSTATE);
      break;
    ...
    case k_EDenyIncompatibleAnticheat:
      cl->Disconnect(
          "You are running an external tool that is\nincompatible with Secure "
          "servers");
      break;
    ...
    case k_EDenyIncompatibleSoftware:
      cl->Disconnect(
          "You are running software that is\nnot compatible with Secure "
          "servers");
      break;
...
//-----------------------------------------------------------------------------
bool CSteam3::NotifyClientConnect(CBaseClient *client, uint32_t unUserID,
                                  netadr_t &adr, const void *pvCookie,
                                  uint32_t ucbCookie) {
  if (!BIsActive()) return true;

  if (!client || client->IsFakeClient()) return false;
#ifndef NO_STEAM
  // Msg("S3: Sending client logon request for %x\n",
  // steamIDClient.ConvertToUint64( ) );
  bool bRet = SteamGameServer()->GSSendUserConnect(
      unUserID, htonl(adr.GetIP()), htons(adr.GetPort()), pvCookie, ucbCookie);
#else
  bool bRet = false;
#endif
  SendUpdatedServerDetails();
...
bool CSteam3::NotifyLocalClientConnect(CBaseClient *client) {
  ...
  if (SteamGameServer() &&
      !SteamGameServer()->GSCreateUnauthenticatedUser(&steamID))
    return false;
...
void CSteam3::NotifyClientDisconnect(CBaseClient *client) {
  if (!client || !BIsActive() || !client->IsConnected() ||
      client->IsFakeClient())
    return;
...
#ifndef NO_STEAM
    if (client->m_SteamID)
      SteamGameServer()->GSSendUserDisconnect(*client->m_SteamID,
                                              client->GetUserID());
#endif
  } else {
    USERID_t id = client->GetNetworkID();
    if (id.idtype != IDTYPE_STEAM) return;
#ifndef NO_STEAM
    CSteamID steamIDClient;
    steamIDClient.SetFromSteam2(&id.uid.steamid,
                                SteamGameServerUtils()->GetConnectedUniverse());
...
// Purpose:
//-----------------------------------------------------------------------------
void CSteam3::NotifyOfServerNameChange() { SendUpdatedServerDetails(); }
...
// Input  : bChangingLevels - true if we're going to heartbeat slowly for a
// while
//-----------------------------------------------------------------------------
void CSteam3::SendUpdatedServerDetails() {
  ...
  SteamGameServer()->GSUpdateStatus(
      sv.GetNumClients(), sv.GetMaxClients(), sv.GetNumFakeClients(),
      sv.GetName(), hltv ? hltv->GetName() : sv.GetName(),
