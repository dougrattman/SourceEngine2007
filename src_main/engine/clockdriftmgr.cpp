...
ConVar cl_clock_correction("cl_clock_correction", "1", FCVAR_CHEAT,
                           "Enable/disable clock correction on the client.");

ConVar cl_clockdrift_max_ms(
    "cl_clockdrift_max_ms", "150", FCVAR_CHEAT,
...
ConVar cl_clockdrift_max_ms_threadmode(
    "cl_clockdrift_max_ms_threadmode", "0", FCVAR_CHEAT,
    "Maximum number of milliseconds the clock is allowed to drift before the "
    "client snaps its clock to the server's.");

ConVar cl_clock_showdebuginfo("cl_clock_showdebuginfo", "0", FCVAR_CHEAT,
                              "Show debugging info about the clock drift. ");

ConVar cl_clock_correction_force_server_tick(
    "cl_clock_correction_force_server_tick", "999", FCVAR_CHEAT,
    "Force clock correction to match the server tick + this offset (-999 "
    "disables it).");

ConVar cl_clock_correction_adjustment_max_amount(
    "cl_clock_correction_adjustment_max_amount", "200", FCVAR_CHEAT,
    "Sets the maximum number of milliseconds per second it is allowed to "
    "correct the client clock. "
    "It will only correct this amount if the difference between the client and "
    "server clock is equal to or larger than "
    "cl_clock_correction_adjustment_max_offset.");

ConVar cl_clock_correction_adjustment_min_offset(
    "cl_clock_correction_adjustment_min_offset", "10", FCVAR_CHEAT,
    "If the clock offset is less than this amount (in milliseconds), then no "
    "clock correction is applied.");

ConVar cl_clock_correction_adjustment_max_offset(
    "cl_clock_correction_adjustment_max_offset", "90", FCVAR_CHEAT,
    "As the clock offset goes from cl_clock_correction_adjustment_min_offset "
    "to this value (in milliseconds), "
    "it moves towards applying cl_clock_correction_adjustment_max_amount of "
    "adjustment. That way, the response "
...
static float GetClockAdjustmentAmount(float flCurDiffInMS) {
  ...
  float flReturnValue = RemapVal(
      flCurDiffInMS, cl_clock_correction_adjustment_min_offset.GetFloat(),
      cl_clock_correction_adjustment_max_offset.GetFloat(), 0,
      cl_clock_correction_adjustment_max_amount.GetFloat() / 1000.0f);
...
CClockDriftMgr::CClockDriftMgr() { Clear(); }
...
// when running in threaded host mode, the clock drifts by a predictable
// algorithm because the client lags the server by one frame so at each update
// from the network we have lastframeticks-1 pending ticks to execute on the
// client.  If the clock has drifted by exactly that amount, allow it to drift
// temporarily NOTE: When the server gets paused the tick count is still
// incorrect for a frame NOTE: It should be possible to fix this by applying
// pause before the tick is incremented NOTE: or decrementing the client tick
// after receiving pause NOTE: This is due to the fact that currently pause is
// applied at frame start on the server NOTE: and frame end on the client
void CClockDriftMgr::SetServerTick(int nTick) {
  ...
  int clientTick =
      cl.GetClientTickCount() + g_ClientGlobalVariables.simTicksThisFrame - 1;
  if (cl_clock_correction_force_server_tick.GetInt() == 999) {
    // If this is the first tick from the server, or if we get further than
    // cl_clockdrift_max_ticks off, then use the old behavior and slam the
    // server's tick into the client tick.
    if (!IsClockCorrectionEnabled() || clientTick == 0 ||
        abs(nTick - clientTick) > nMaxDriftTicks) {
      cl.SetClientTickCount(nTick -
    ...
    // Used for testing..
    cl.SetClientTickCount(nTick +
                          cl_clock_correction_force_server_tick.GetInt());
  }
...
float CClockDriftMgr::AdjustFrameTime(float inputFrameTime) {
  ...
  if (IsClockCorrectionEnabled()
    ...
    float flCurDiffInSeconds =
        GetCurrentClockDifference() * host_state.interval_per_tick;
...
float CClockDriftMgr::GetCurrentClockDifference() const {
  ...
  float total = 0;
  for (int i = 0; i < NUM_CLOCKDRIFT_SAMPLES; i++) total += m_ClockOffsets[i];
...
void CClockDriftMgr::ShowDebugInfo(float flAdjustment) {
  ...
  if (IsClockCorrectionEnabled()) {
    ...
    Msg("Clock drift: adjustment (per sec): %.2fms, avg: %.3f, lo: %d, hi: %d, "
        "ex: %d\n",
        flAdjustment * 1000.0f, GetCurrentClockDifference(), low, high,
        exactDiff);
  } else {
...
void CClockDriftMgr::AdjustAverageDifferenceBy(float flAmountInSeconds) {
  ...
  for (int i = 0; i < NUM_CLOCKDRIFT_SAMPLES; i++) m_ClockOffsets[i] *= factor;
...
bool CClockDriftMgr::IsClockCorrectionEnabled() {
  ...
  // If we're a multiplayer listen server, we can back off of that if we have
  // zero latency (faked or due to sockets)
  if (bIsMultiplayer) {
    bool bIsListenServer = sv.IsActive();
    bool bLocalConnectionHasZeroLatency =
        (NET_GetFakeLag() <= 0.0f) && !net_usesocketsforloopback.GetBool();
  ...
  // Only in multi-threaded client/server OR in multi player, but don't use it
  // if we're the listen server w/ no fake lag
