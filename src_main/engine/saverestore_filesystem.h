...
abstract_class ISaveRestoreFileSystem {
 public:
  ...
  virtual unsigned int Size(const char *pFileName,
                            const char *pPathID = NULL) = 0;

  virtual bool FileExists(const char *pFileName,
                          const char *pPathID = NULL) = 0;
  virtual void RenameFile(char const *pOldPath, char const *pNewPath,
                          const char *pathID = NULL) = 0;
  ...
  virtual FSAsyncStatus_t AsyncWrite(
      ...
      bool bAppend, FSAsyncControl_t *pControl = NULL) = 0;
  ...
  virtual void DirectoryCopy(const char *pPath, const char *pDestFileName,
                             bool bIsXSave) = 0;
  virtual bool DirectoryExtract(FileHandle_t pFile, int fileCount,
                                bool bIsXSave) = 0;
