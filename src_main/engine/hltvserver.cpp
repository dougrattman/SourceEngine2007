...
static void tv_title_changed_f(IConVar *var, const char *pOldString,
                               float flOldValue) {
...
static void tv_name_changed_f(IConVar *var, const char *pOldValue,
                              float flOldValue) {
  Steam3Server().NotifyOfServerNameChange();
}

static ConVar tv_maxclients("tv_maxclients", "128", 0,
                            "Maximum client number on SourceTV server.", true,
                            0, true, 255);

ConVar tv_autorecord("tv_autorecord", "0", 0,
                     "Automatically records all games as SourceTV demos.");
ConVar tv_name("tv_name", "SourceTV", 0, "SourceTV host name",
               tv_name_changed_f);
static ConVar tv_password("tv_password", "",
                          FCVAR_NOTIFY | FCVAR_PROTECTED | FCVAR_DONTRECORD,
                          "SourceTV password for all clients");

static ConVar tv_overridemaster("tv_overridemaster", "0", 0,
                                "Overrides the SourceTV master root address.");
static ConVar tv_dispatchmode(
    "tv_dispatchmode", "1", 0,
    "Dispatch clients to relay proxies: 0=never, 1=if appropriate, 2=always");
ConVar tv_transmitall("tv_transmitall", "0", FCVAR_REPLICATED,
                      "Transmit all entities (not only director view)");
ConVar tv_debug("tv_debug", "0", 0, "SourceTV debug info.");
ConVar tv_title("tv_title", "SourceTV", 0,
                "Set title for SourceTV spectator UI", tv_title_changed_f);
static ConVar tv_deltacache("tv_deltacache", "2", 0,
                            "Enable delta entity bit stream cache");
static ConVar tv_relayvoice("tv_relayvoice", "1", 0,
...
unsigned char *CDeltaEntityCache::FindDeltaBits(int nEntityIndex,
                                                int nDeltaTick, int &nBits) {
  nBits = -1;

  if (nEntityIndex < 0 || nEntityIndex >= m_nMaxEntities) return NULL;
...
void CDeltaEntityCache::AddDeltaBits(int nEntityIndex, int nDeltaTick,
                                     int nBits, bf_write *pBuffer) {
  ...
  if (nBits > 0) {
    bf_read inBuffer;
    inBuffer.StartReading(pBuffer->GetData(), pBuffer->m_nDataBytes,
                          pBuffer->GetNumBitsWritten());
    bf_write outBuffer((char *)(pEntry) + sizeof(DeltaEntityEntry_s),
                       nBufferSize);
    outBuffer.WriteBitsFromBuffer(&inBuffer, nBits);
  }
}

static RecvTable *FindRecvTable(const char *pName, RecvTable **pRecvTables,
                                int nRecvTables) {
  for (int i = 0; i < nRecvTables; i++) {
    if (!Q_strcmp(pName, pRecvTables[i]->GetName())) return pRecvTables[i];
  }

  return NULL;
}

static RecvTable *AddRecvTableR(SendTable *sendt, RecvTable **pRecvTables,
                                int &nRecvTables) {
  RecvTable *recvt =
      FindRecvTable(sendt->m_pNetTableName, pRecvTables, nRecvTables);
  ...
  if (sendt->m_nProps > 0) {
    ...
    for (int i = 0; i < sendt->m_nProps; i++) {
      ...
      if (sp->GetType() == DPT_DataTable) {
        ...
        RecvTable *subTable =
            AddRecvTableR(sp->GetDataTable(), pRecvTables, nRecvTables);
        rp->SetDataTable(subTable);
      }
    }

    recvt =
        new RecvTable(receiveProps, sendt->m_nProps, sendt->m_pNetTableName);
  } else {
...
void CHLTVServer::InitClientRecvTables() {
  ...
  if (ClientDLL_GetAllClasses() != NULL) return;  // already initialized
  ...
  for (pCur = serverGameDLL->GetAllServerClasses(); pCur;
    ...
    RecvTable *recvt =
        FindRecvTable(pCur->m_pTable->GetName(), m_pRecvTables, m_nRecvTables);

    Assert(recvt);

    // register class, constructor addes clientClass to g_pClientClassHead list
    ClientClass *clientclass =
        new ClientClass(pCur->m_pNetworkName, NULL, NULL, recvt);
...
bool CHLTVFrame::HasData(void) {
  for (int i = 0; i < HLTV_BUFFER_MAX; i++) {
    if (m_Messages[i].GetNumBitsWritten() > 0) return true;
  }
...
void CHLTVFrame::CopyHLTVData(CHLTVFrame &frame) {
  ...
  if (bits > 0) {
    ...
    m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(
        ...
        frame.m_Messages[HLTV_BUFFER_UNRELIABLE].GetNumBitsWritten());
    m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(
        frame.m_Messages[HLTV_BUFFER_TEMPENTS].GetData(),
        frame.m_Messages[HLTV_BUFFER_TEMPENTS].GetNumBitsWritten());
    m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(
        frame.m_Messages[HLTV_BUFFER_SOUNDS].GetData(),
        frame.m_Messages[HLTV_BUFFER_SOUNDS].GetNumBitsWritten());

    if (tv_relayvoice.GetBool())
      m_Messages[HLTV_BUFFER_UNRELIABLE].WriteBits(
          frame.m_Messages[HLTV_BUFFER_VOICE].GetData(),
          frame.m_Messages[HLTV_BUFFER_VOICE].GetNumBitsWritten());
  }
}
...
void CHLTVServer::StartMaster(CGameClient *client) {
  ...
  CPlayerState *player =
      serverGameClients->GetPlayerState(m_MasterClient->edict);
  player->hltv = true;
  ...
  m_nGameServerMaxClients =
      m_Server->GetMaxClients();  // maxclients is different on proxy (128)
  ...
  while (modevents[j] != NULL) {
    ...
    CGameEventDescriptor *descriptor =
        g_GameEventManager.GetEventDescriptor(eventname);

    if (descriptor) {
      g_GameEventManager.AddListener(this, descriptor,
                                     CGameEventManager::CLIENTSTUB);
    } else {
  ...
  // copy signon buffers
  m_Signon.StartWriting(m_Server->m_Signon.GetBasePointer(),
                        m_Server->m_Signon.m_nDataBytes,
...
bool CHLTVServer::DispatchToRelay(CHLTVClient *pClient) {
  ...
  if ((tv_dispatchmode.GetInt() == DISPATCH_MODE_AUTO) &&
      (GetMaxClients() > 0)) {
  ...
  // increase this proxies client number in advance so this proxy isn't used
  // again next time
...
void CHLTVServer::BroadcastLocalChat(const char *pszChat,
                                     const char *pszGroup) {
...
bool CHLTVServer::ShouldUpdateMasterServer() {
  if (IsUsingMasterLegacyMode()) {
    // In the legacy master server updater, we have to call the updater with our
    // CBaseServer* for it to work right.
    return true;
  } else {
    // If the main game server is active, then we let it update Steam with the
    // server info.
    return !sv.IsActive();
  }
}
  ...
  for (int i = 0; i < numTables; i++) {
    ...
    CNetworkStringTable *hltvTable =
        (CNetworkStringTable *)m_StringTables->CreateStringTableEx(
            serverTable->GetTableName(), serverTable->GetMaxStrings(),
            serverTable->GetUserDataSize(), serverTable->GetUserDataSizeBits(),
...
void CHLTVServer::RestoreTick(int tick) {
  ...
  for (int i = 0; i < numTables; i++) {
    ...
    CNetworkStringTable *pTable =
        (CNetworkStringTable *)m_StringTables->GetTable(i);
...
void CHLTVServer::LinkInstanceBaselines(void) {
  ...
  for (ServerClass *pClass = serverGameDLL->GetAllServerClasses(); pClass;
...
CHLTVEntityData *FindHLTVDataInSnapshot(CFrameSnapshot *pSnapshot,
  ...
  while (a < z) {
    ...
    if (index == iEntIndex) return &pSnapshot->m_pHLTVEntityData[m];
...
void CHLTVServer::EntityPVSCheck(CClientFrame *pFrame) {
  ...
  if (pSnapshot && nDirectorEntity > 0) {
    CHLTVEntityData *pHLTVData =
        FindHLTVDataInSnapshot(pSnapshot, nDirectorEntity);
...
void CHLTVServer::UpdateStats(void) {
  ...
  if (!IsMasterProxy() && !m_ClientState.IsConnected()) {
    ...
    if (IsMasterProxy() || tv_overridemaster.GetBool()) {
      ...
      Q_snprintf(address, sizeof(address), "%s:%u",
                 net_local_adr.ToString(true), GetUDPPort());
    } else {
      // forward address
...
bf_write *CHLTVServer::GetBuffer(int nBuffer) {
  if (nBuffer < 0 || nBuffer >= HLTV_BUFFER_MAX) return NULL;
...
CClientFrame *CHLTVServer::GetDeltaFrame(int nTick) {
  if (!tv_deltacache.GetBool()) return GetClientFrame(nTick);  // expensive

  // TODO make that a utlmap
  FOR_EACH_VEC(m_FrameCache, iFrame) {
    if (m_FrameCache[iFrame].nTick == nTick) return m_FrameCache[iFrame].pFrame;
  }
...
void CHLTVServer::RunFrame() {
  ...
  if (!sv.IsActive()) Steam3Server().RunFrame();
...
void CHLTVServer::UpdateTick(void) {
  ...
  if (IsMasterProxy()) {
    ...
    m_DeltaCache.SetTick(m_CurrentFrame->tick_count,
                         m_CurrentFrame->last_entity + 1);
  }

  int removeTick =
      m_nTickCount - 16.0f / m_flTickInterval;  // keep 16 seconds buffer
...
void CHLTVServer::FillServerInfo(SVC_ServerInfo &serverinfo) {
  ...
  serverinfo.m_nPlayerSlot =
      m_nPlayerSlot;  // all spectators think they're the HLTV client
...
void CHLTVServer::Shutdown(void) {
  ...
  if (IsMasterProxy()) {
    if (m_MasterClient) m_MasterClient->Disconnect("SourceTV stop.");

    if (m_Director) m_Director->SetHLTVServer(NULL);
  } else {
    // do not try to reconnect to old connection
    m_ClientState.m_szRetryAddress[0] = 0;
...
bool CHLTVServer::StartPlayback(const char *filename, bool bAsTimeDemo) {
  ...
  m_ClientState.m_NetChannel =
      NET_CreateNetChannel(NS_CLIENT, NULL, "DEMO", &m_ClientState);
...
void CHLTVServer::ReadCompeleteDemoFile() {
  ...
  while (true) {
    ...
    switch (cmd) {
      ...
      case dem_usercmd: {
        char user_cmd_buffer[256];
        int length = sizeof(user_cmd_buffer);
        m_DemoFile.ReadUserCmd(user_cmd_buffer, length);
      ...
      case dem_packet: {
        ...
        m_DemoFile.ReadCmdInfo(
            m_LastCmdInfo);  // MOTODO must be stored somewhere
...
IClient *CHLTVServer::ConnectClient(netadr_t &adr, int protocol, int challenge,
                                    int authProtocol, const char *name,
                                    const char *password,
                                    const char *hashedCDkey, int cdKeyLen) {
  IClient *client = (CHLTVClient *)CBaseServer::ConnectClient(
      adr, protocol, challenge, authProtocol, name, password, hashedCDkey,
      cdKeyLen);

  if (client) {
    // remember password
    CHLTVClient *pHltvClient = (CHLTVClient *)client;
    Q_strncpy(pHltvClient->m_szPassword, password,
              sizeof(pHltvClient->m_szPassword));
  }
...
CON_COMMAND(tv_status, "Show SourceTV server status.") {
  ...
  ConMsg("Online %s, FPS %.1f, Version %i (%s)\n",
         COM_FormatSeconds(hltv->GetOnlineTime()), hltv->m_flFPS,
         build_number(),
  ...
  ConMsg("Game Time %s, Mod \"%s\", Map \"%s\", Players %i\n",
         COM_FormatSeconds(hltv->GetTime()), gd, hltv->GetMapName(),
         hltv->GetNumPlayers());
  ...
  if (hltv->m_DemoRecorder.IsRecording()) {
    ConMsg("Recording to \"%s\", length %s.\n",
           ...
           COM_FormatSeconds(host_state.interval_per_tick *
                             hltv->m_DemoRecorder.GetRecordingTick()));
  }
}
...
CON_COMMAND(tv_clients, "Shows list of connected SourceTV clients.") {
  ...
  for (int i = 0; i < hltv->GetClientCount(); i++) {
    ...
    ConMsg("ID: %i, \"%s\" %s, Time %s, %s, In %.1f, Out %.1f.\n",
           client->GetUserID(), client->GetClientName(),
           client->IsHLTV() ? "(Relay)" : "",
           COM_FormatSeconds(netchan->GetTimeConnected()),
           netchan->GetAddress(), netchan->GetAvgData(FLOW_INCOMING) / 1024,
...
CON_COMMAND_AUTOCOMPLETEFILE(editdemo, EditDemo_f,
                             "Edit a recorded demo file (.dem ).", NULL, dem);
#endif
