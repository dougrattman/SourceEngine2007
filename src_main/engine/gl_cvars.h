...
extern ConVar
    mat_bumpbasis;  // Draw the world with the bump basis vectors drawn
extern ConVar mat_envmapsize;  // Dimensions of square skybox bitmap (in 3D
                               // screen shots, not game textures)
...
#ifdef _DEBUG
  bool bRespectCheatVars =
      cl_debug_respect_cheat_vars.GetBool() && !Cmd_IsRptActive();
  if (bRespectCheatVars) return sv_cheats.GetBool();
  return true;
#else
