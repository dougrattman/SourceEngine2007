...
abstract_class CFileTransferMgr {
 public:
  ...
  // The user data is sent in the header and can include the filename, its ID,
  // or whatever.
  FileTransferID_t StartSending(INetChannel * pDest, const void *pUserData,
                                int userDataLength, const char *pFileData,
                                int fileLength, int bytesPerSecond);
 ...
 public:
  ...
  // Returns false if it was unable to send the chunk. If this happens, the file
  // transfer manager will retry the chunk a few times, and eventually cancel
  // the file transfer if the problem keeps happening.
  ...
  virtual void OnFinishedSending(INetChannel * pDest, const void *pUserData,
                                 int userDataLen, FileTransferID_t id) = 0;
