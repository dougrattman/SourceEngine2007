...
// multiplayer game, tracking new players and dropped players during the game,
// and reporting game results and stats after the game is complete.
...
void CMatchmaking::HandleSystemLinkReply(netpacket_t *pPacket) {
  if (!m_Session.IsSystemLink() || m_Session.IsHost()) return;

  uint64_t nonce = pPacket->message.ReadLongLong();
  ...
  for (int i = 0; i < m_pSystemLinkResults.Count(); ++i) {
    XSESSION_SEARCHRESULT *pCheck =
        &((systemLinkInfo_s *)m_pSystemLinkResults[i])->Result;
    if (Q_memcmp(&pCheck->info.sessionID, &pResult->info.sessionID,
                 sizeof(pResult->info.sessionID)) == 0) {
  ...
  const uint32_t propSize = pResult->cProperties * sizeof(XUSER_PROPERTY);
  const uint32_t ctxSize = pResult->cContexts * sizeof(XUSER_CONTEXT);

  pResult->pProperties =
      (XUSER_PROPERTY *)((byte *)pResult + sizeof(XSESSION_SEARCHRESULT));
  pResult->pContexts =
      (XUSER_CONTEXT *)((byte *)pResult->pProperties + propSize);
...
bool CMatchmaking::SearchForSession() {
  ...
  uint32_t cbResultsBytes = 0;
  uint32_t ret = 0;

  // Call once to get the necessary buffer size
  ret = g_pXboxSystem->SessionSearch(SESSION_MATCH_QUERY_PLAYER_MATCH,
                                     XBX_GetPrimaryUserId(), MAX_SEARCHRESULTS,
                                     1, 0, 0, NULL, NULL,
...
void CMatchmaking::UpdateSearch() {
  ...
  if (!m_Session.IsSystemLink()) {
    ...
    DWORD ret =
        g_pXboxSystem->GetOverlappedResult(m_hSearchHandle, NULL, false);
    if (ret == ERROR_IO_INCOMPLETE) {
      // Still waiting
      return;
    } else {
      if (ret == ERROR_SUCCESS && m_pSearchResults &&
        ...
        //
        // Note: XNetQosLookup requires only 2 successful probes to be received
        // from the host. This is much less than the recommended 8 probes
        // because on a 10% data loss profile it is impossible to find the host
        // when requiring 8 probes to be received.
        XNetQosLookup(m_pSearchResults->dwSearchResults, m_QoSxnaddr,
                      m_QoSxnkid, m_QoSxnkey,
    ...
    if (GetTime() - m_fSendTimer > SYSTEMLINK_RETRYINTERVAL &&
      ...
      NET_SendPacket(NULL, NS_SYSTEMLINK, adr, msg.GetData(),
      ...
      if (m_pSystemLinkResults.Count()) {
        ...
        for (int i = 0; i < m_pSystemLinkResults.Count(); ++i) {
          systemLinkInfo_s *pSearchInfo =
              (systemLinkInfo_s *)m_pSystemLinkResults[i];
          ...
          Q_strncpy(hostData.hostName, pSearchInfo->szHostName,
                    sizeof(hostData.hostName));
          Q_strncpy(hostData.scenario, pSearchInfo->szScenario,
                    sizeof(hostData.scenario));
...
void CMatchmaking::UpdateQosLookup() {
  ...
  if (GetTime() - m_fWaitTimer < QOSLOOKUP_WAITTIME) {
    for (uint32_t i = 0; i < m_pSearchResults->dwSearchResults; ++i) {
  ...
  for (unsigned int i = 0; i < m_pSearchResults->dwSearchResults; ++i) {
    ...
    if (!(m_pQoSResult->axnqosinfo[i].bFlags &
          XNET_XNQOSINFO_TARGET_CONTACTED)) {
      DevMsg(
          "Result #%d: Host unreachable (!XNET_XNQOSINFO_TARGET_CONTACTED)\n",
          i);
      continue;
    } else if ((m_pQoSResult->axnqosinfo[i].bFlags &
                XNET_XNQOSINFO_TARGET_DISABLED)) {
    ...
    unsigned short ping =
        m_pQoSResult->axnqosinfo[i]
            .wRttMinInMsecs;  // Use min ping to suit better for traffic bursts
                              // and lossy connections
    DevMsg("Result #%d: ping min %d ms, med %d ms\n", i,
           m_pQoSResult->axnqosinfo[i].wRttMinInMsecs,
           m_pQoSResult->axnqosinfo[i].wRttMedInMsecs);

    // On X360 ping calculations are reported between 4 and 5 times bigger
    // than the actual upstream/downstream latency of the connection to Xbox
    // LIVE
    ...
    EngineVGui()->SessionSearchResult(i, &hostData, &searchResult,
                                      pingDisplayedToUserIcon);
    DevMsg("Result #%d: %d open public slots, %d open private slots\n", i,
           searchResult.dwOpenPublicSlots, searchResult.dwOpenPrivateSlots);
  }
...
//-----------------------------------------------------------------------------
void CMatchmaking::CancelSearch() { SwitchToState(MMSTATE_INITIAL); }
...
//-----------------------------------------------------------------------------
// Purpose: User has selected a session to join, create a local session with the
// same properties
//-----------------------------------------------------------------------------
void CMatchmaking::SelectSession(uint32_t idx) {
  ...
  ApplySessionProperties(pResult->cContexts, pResult->cProperties,
                         pResult->pContexts, pResult->pProperties);

  m_Session.SetIsHost(false);
  m_Session.SetSessionSlots(
      SLOTS_TOTALPUBLIC,
      pResult->dwOpenPublicSlots + pResult->dwFilledPublicSlots);
  m_Session.SetSessionSlots(
      SLOTS_TOTALPRIVATE,
      pResult->dwOpenPrivateSlots + pResult->dwFilledPrivateSlots);
...
void CMatchmaking::JoinInviteSession(XSESSION_INFO *pHostInfo) {
  ...
  switch (m_InviteState) {
    case INVITE_NONE:
      // Initial invite call
      Msg("[JoinInviteSession:INVITE_NONE] Initial call to join invite "
          "session.\n");

      // Don't bother if we're invited to join the same session
      if (!Q_memcmp(&(pHostInfo->sessionID), &(nSessionID),
                    sizeof(nSessionID))) {
        Msg("[JoinInviteSession:INVITE_NONE] Rejecting invite session since it "
      ...
      if (m_bInitialized) {
        ...
        // We can be uninitialized due to the commentary mode - perform the
        // disconnect to be sure
        ConVarRef commentary("commentary");
        if (commentary.IsValid() && commentary.GetBool()) {
          Msg("[JoinInviteSession:INVITE_NONE] Stopping commentary mode "
              "first.\n");
    ...
    case INVITE_VALIDATING:
      ...
      Msg("[JoinInviteSession:INVITE_VALIDATING] Validating user storage "
          "before accepting invite.\n");

      //
      // Configure and validate waiting info in case user will have to pick
      // storage device
      //
      ...
      if (m_InviteWaitingInfo.m_UserIdx == INVALID_USER_ID) {
        Msg("[JoinInviteSession:INVITE_VALIDATING] Invalid user id, "
            "aborting.\n");
        InviteCancel();
        return;
      }

      m_InviteWaitingInfo.m_SignInState =
          XUserGetSigninState(m_InviteWaitingInfo.m_UserIdx);
      if ((m_InviteWaitingInfo.m_SignInState !=
           eXUserSigninState_SignedInToLive) ||
          (ERROR_SUCCESS !=
           XUserGetSigninInfo(m_InviteWaitingInfo.m_UserIdx, 0,
                              &m_InviteWaitingInfo.m_SignInInfo)) ||
          !(m_InviteWaitingInfo.m_SignInInfo.dwInfoFlags &
            XUSER_INFO_FLAG_LIVE_ENABLED)) {
        Msg("[JoinInviteSession:INVITE_VALIDATING] Failed to get sign in to "
            "LIVE info, aborting.\n");
        InviteCancel();
        return;
      }
      ...
      if ((ERROR_SUCCESS !=
           XUserCheckPrivilege(m_InviteWaitingInfo.m_UserIdx,
                               XPRIVILEGE_MULTIPLAYER_SESSIONS,
                               &m_InviteWaitingInfo.m_PrivilegeMultiplayer)) ||
          (!m_InviteWaitingInfo.m_PrivilegeMultiplayer)) {
        Msg("[JoinInviteSession:INVITE_VALIDATING] Privilege denied, "
            "aborting.\n");
        InviteCancel();
        return;
      }
      ...
      if (m_InviteWaitingInfo.m_bAcceptingInvite ||
          EngineVGui()->ValidateStorageDevice(
              &m_InviteWaitingInfo.m_InviteStorageDeviceSelected)) {
        m_InviteWaitingInfo.m_bAcceptingInvite = 0;
        Msg("[JoinInviteSession:INVITE_VALIDATING] Storage %s, accepting.\n",
            m_InviteWaitingInfo.m_bAcceptingInvite ? "already queried"
                                                   : "valid");
    ...
    case INVITE_AWAITING_STORAGE:
      // Wait for user to select storage, but keep an eye on user change or
      // logout or etc
      {
        ...
        if (InviteCurrentInfo.m_UserIdx != m_InviteWaitingInfo.m_UserIdx) {
          Msg("[JoinInviteSession:INVITE_AWAITING_STORAGE] User index changed, "
              "aborting.\n");
          InviteCancel();
          return;
        }

        InviteCurrentInfo.m_SignInState =
            XUserGetSigninState(InviteCurrentInfo.m_UserIdx);
        if ((InviteCurrentInfo.m_SignInState !=
             m_InviteWaitingInfo.m_SignInState) ||
            (ERROR_SUCCESS !=
             XUserGetSigninInfo(InviteCurrentInfo.m_UserIdx, 0,
                                &InviteCurrentInfo.m_SignInInfo)) ||
            !(InviteCurrentInfo.m_SignInInfo.dwInfoFlags &
              XUSER_INFO_FLAG_LIVE_ENABLED) ||
            !IsEqualXUID(InviteCurrentInfo.m_SignInInfo.xuid,
                         m_InviteWaitingInfo.m_SignInInfo.xuid)) {
          Msg("[JoinInviteSession:INVITE_AWAITING_STORAGE] User xuid changed, "
              "aborting.\n");
          InviteCancel();
          return;
        }
        ...
        if ((ERROR_SUCCESS !=
             XUserCheckPrivilege(InviteCurrentInfo.m_UserIdx,
                                 XPRIVILEGE_MULTIPLAYER_SESSIONS,
                                 &InviteCurrentInfo.m_PrivilegeMultiplayer)) ||
            (!InviteCurrentInfo.m_PrivilegeMultiplayer)) {
          Msg("[JoinInviteSession:INVITE_AWAITING_STORAGE] Privilege denied, "
              "aborting.\n");
          InviteCancel();
          return;
        }
        ...
        switch (m_InviteWaitingInfo.m_InviteStorageDeviceSelected) {
          ...
          case 1:
            ...
            Msg("[JoinInviteSession:INVITE_AWAITING_STORAGE] Device "
                "selected.\n");
            break;
          case 2:
            Msg("[JoinInviteSession:INVITE_AWAITING_STORAGE] Device "
      ...
      // Otherwise user has selected the storage device, try to accept the
      // invite once again
...
#if defined(_X360)
  // "Spoof" certain information we don't yet know about the server, knowing
  // that we'll modify it later once connected
  m_Session.SetIsSystemLink(false);
  m_Session.SetSessionInfo(pHostInfo);
  m_Session.SetIsHost(false);
  m_Session.SetContext(X_CONTEXT_GAME_TYPE, X_CONTEXT_GAME_TYPE_STANDARD,
                       false);
...
//-----------------------------------------------------------------------------
// Purpose: Search for a session by ID and connect to it (done for cross-game
// invites)
//-----------------------------------------------------------------------------
...
void CMatchmaking::SendJoinRequest(netadr_t *adr) {
  ...
  for (int i = 0; i < m_Local.m_cPlayers; ++i) {
    ...
    msg.WriteBytes(&m_Local.m_cVoiceState,
                   sizeof(m_Local.m_cVoiceState));  // TODO: has voice
    msg.WriteString(m_Local.m_szGamertags[i]);
  }

  // Send message
  NET_SendPacket(NULL, NS_MATCHMAKING, *adr, msg.GetData(),
                 msg.GetNumBytesWritten());
}
...
bool CMatchmaking::ProcessJoinResponse(MM_JoinResponse *pMsg) {
  ...
  switch (pMsg->m_ResponseType) {
    ...
    case pMsg->JOINRESPONSE_APPROVED_JOINGAME:
      ...
      ApplySessionProperties(pMsg->m_ContextCount, pMsg->m_PropertyCount,
                             pMsg->m_SessionContexts.Base(),
                             pMsg->m_SessionProperties.Base());
...
//-----------------------------------------------------------------------------
// Purpose: Apply the contexts and properties that came from the host, and build
// out keyvalues for GameUI
//-----------------------------------------------------------------------------
void CMatchmaking::ApplySessionProperties(int numContexts, int numProperties,
                                          XUSER_CONTEXT *pContexts,
                                          XUSER_PROPERTY *pProperties) {
  // Clear our existing properties, as they should be completely replaced by
  // these new ones
