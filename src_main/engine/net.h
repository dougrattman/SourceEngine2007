...
#define HEADER_BYTES 9  // 2*4 bytes seqnr, 1 uint8_t flags

// Pad this to next higher 16 uint8_t boundary
...
void NET_Shutdown();
// Read any incoming packets, dispatch to known netchannels and call handler for
...
// RunFrame must be called each system frame before reading/sending on any
// socket
...
// bForceNew (used for bots) tells it not to share INetChannels (bots will crash
// when disconnecting if they share an INetChannel).
INetChannel *NET_CreateNetChannel(int socketnumber, netadr_t *adr,
                                  const char *name, INetChannelHandler *handler,
...
const char *NET_ErrorString(
    int code);  // translate a socket error into a friendly string
