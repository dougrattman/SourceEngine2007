...
ConVar r_visocclusion("r_visocclusion", "0", FCVAR_CHEAT,
                      "Activate/deactivate wireframe rendering of what the "
                      "occlusion system is doing.");
ConVar r_occlusion("r_occlusion", "1", 0,
                   "Activate/deactivate the occlusion system.");
static ConVar r_occludermincount(
    "r_occludermincount", "0", 0,
    "At least this many occluders will be used, no matter how big they are.");
static ConVar r_occlusionspew(
    "r_occlusionspew", "0", FCVAR_CHEAT,
    "Activate/deactivates spew about what the occlusion system is doing.");
ConVar r_occluderminarea("r_occluderminarea", "0", 0,
                         "Prevents this occluder from being used if it takes "
                         "up less than X% of the screen. 0 means use whatever "
                         "the level said to use.");
ConVar r_occludeemaxarea("r_occludeemaxarea", "0", 0,
                         "Prevents occlusion testing for entities that take up "
                         "more than X% of the screen. 0 means use whatever the "
                         "level said to use.");
...
class CWingedEdgeList {
 public:
  struct WingedEdge_t {
    ...
    Vector
        m_vecPositionEnd;  // of the lower point in y, measured in screen space
 ...
 public:
  ...
  // Adds an edge
  int AddEdge();
  int AddEdge(const Vector &vecStartVert, const Vector &vecEndVert,
              int nLeaveSurfID, int nEnterSurfID);
...
int CWingedEdgeList::AddEdge(const Vector &vecStartVert,
                             const Vector &vecEndVert, int nLeaveSurfID,
                             int nEnterSurfID) {
...
inline void CWingedEdgeList::LinkActiveEdgeAfter(WingedEdge_t *pPrevEdge,
                                                 WingedEdge_t *pInsertEdge) {
...
// Returns the z value of a surface given and x,y coordinate
//-----------------------------------------------------------------------------
inline float CWingedEdgeList::ComputeZValue(const Surface_t *pSurface, float x,
                                            float y) const {
...
//-----------------------------------------------------------------------------
inline bool CWingedEdgeList::IsEdgeXGreater(const WingedEdge_t *pEdge1,
                                            const WingedEdge_t *pEdge2) {
  float flDelta = pEdge1->m_flX - pEdge2->m_flX;
  if (flDelta > 0) return true;

  if (flDelta < 0) return false;

  // NOTE: Using > instead of >= means coincident edges won't continually swap
  // places
...
// Inserts an edge into the active edge list, sorted by X
//-----------------------------------------------------------------------------
inline void CWingedEdgeList::InsertActiveEdge(WingedEdge_t *pPrevEdge,
                                              WingedEdge_t *pInsertEdge) {
...
void CWingedEdgeList::SpewActiveEdgeList(float y, bool bHex) {
  ...
  while (!AtListEnd(pEdge)) {
    if (!bHex) {
      Msg("(%d %.3f [%d/%d]) ", (int)(pEdge - m_WingedEdges.Base()),
          pEdge->m_flX, pEdge->m_nLeaveSurfID, pEdge->m_nEnterSurfID);
    } else {
      Msg("(%d %X [%d/%d]) ", (int)(pEdge - m_WingedEdges.Base()),
          *(int *)&pEdge->m_flX, pEdge->m_nLeaveSurfID, pEdge->m_nEnterSurfID);
    }
...
bool CWingedEdgeList::AdvanceActiveEdgeList(float flCurrY) {
  // Reordering is unnecessary because the winged edges are guaranteed to be
  // non-overlapping
  m_flNextDiscontinuity = FLT_MAX;

  // Advance all edges until the current Y; we don't need to re-order *any*
  // edges.
  WingedEdge_t *pCurEdge;
  WingedEdge_t *pNextEdge;
  for (pCurEdge = FirstActiveEdge(); !AtListEnd(pCurEdge);
       pCurEdge = pNextEdge) {
    ...
    pCurEdge->m_flX =
        pCurEdge->m_vecPosition.x +
        (flCurrY - pCurEdge->m_vecPosition.y) * pCurEdge->m_flDxDy;
...
// Advance the active edge list until a particular X value is reached.
//-----------------------------------------------------------------------------
inline CWingedEdgeList::WingedEdge_t *CWingedEdgeList::AdvanceActiveEdgeListToX(
    WingedEdge_t *pEdge, float x) {
...
// Returns true if this active edge list occludes another active edge list
//-----------------------------------------------------------------------------
bool CWingedEdgeList::IsOccludingActiveEdgeList(CWingedEdgeList &testList,
                                                float y) {
  ...
  if (!testList.AtListStart(pTestEdge->m_pPrevActiveEdge)) {
    // In this case, we should be on a span crossing from x <= -1.0f to x
    // > 1.0f. Do the first occlusion test at x = -1.0f.
  ...
  Surface_t *pTestSurf = (pTestEdge->m_nEnterSurfID >= 0)
                             ? &testList.m_Surfaces[pTestEdge->m_nEnterSurfID]
                             : &m_BackSurface;

  // Use the leave surface because we know the occluder has been advanced
  // *beyond* the test surf X.
  Surface_t *pOccluderSurf = (pOccluderEdge->m_nLeaveSurfID >= 0)
                                 ? &m_Surfaces[pOccluderEdge->m_nLeaveSurfID]
                                 : &m_BackSurface;
  ...
  while (true) {
    ...
    if (flTestOOz < flOccluderOOz) return false;
    ...
    if (flNextTestX <= flNextOccluderX) {
      ...
      if (pTestEdge->m_nEnterSurfID >= 0) {
        ...
        pTestSurf = (pTestEdge->m_nLeaveSurfID >= 0)
                        ? &testList.m_Surfaces[pTestEdge->m_nLeaveSurfID]
                        : &m_BackSurface;
      }
      flNextTestX = pTestEdge->m_pNextActiveEdge->m_flX;
    } else {
      ...
      pOccluderSurf = (pOccluderEdge->m_nLeaveSurfID >= 0)
                          ? &m_Surfaces[pOccluderEdge->m_nLeaveSurfID]
                          : &m_BackSurface;
...
bool CWingedEdgeList::IsOccludingEdgeList(CWingedEdgeList &testList) {
  ...
  // If, then, the first occluder edge starts *below* the first occludee edge,
  // it doesn't occlude.
  ...
  while (true) {
    if (!IsOccludingActiveEdgeList(testList, flCurrentY)) return false;
    ...
    // not necessarily a discontinuity at y == 1.0. We need to create a
    // discontinuity there so that the occluder edges are still being used.
    if (flCurrentY > 1.0f) {
      flCurrentY = 1.0f;
    }

    // If the occludee list is empty, then it's occluded!
    if (!testList.AdvanceActiveEdgeList(flCurrentY)) return true;
...
class CEdgeList {
 ...
 private:
  ...
  void ComputePointAlongEdge(const Edge_t *pEdge, int nSurfID, float y,
                             Vector *pPoint) const;
  ...
  void ReduceActiveEdgeList(CWingedEdgeList &newEdgeList, float flMinY,
                            float flMaxY);

  // Discovers the first edge crossing discontinuity
  float LocateEdgeCrossingDiscontinuity(float flNextY, float flPrevY,
                                        int &nCount, Edge_t **pInfo);
...
inline const CEdgeList::Edge_t &CEdgeList::EdgeFromSortIndex(
    int nSortIndex) const {
...
inline void CEdgeList::LinkActiveEdgeAfter(Edge_t *pPrevEdge,
                                           Edge_t *pInsertEdge) {
  pInsertEdge->m_pNextActiveEdge = pPrevEdge->m_pNextActiveEdge;
  pInsertEdge->m_pPrevActiveEdge = pPrevEdge;
  pInsertEdge->m_pNextActiveEdge->m_pPrevActiveEdge = pInsertEdge;
...
// Returns the z value of a surface given and x,y coordinate
//-----------------------------------------------------------------------------
inline float CEdgeList::ComputeZValue(const Surface_t *pSurface, float x,
                                      float y) const {
  const cplane_t &plane = pSurface->m_Plane;
  Assert(plane.normal.z == 1.0f);
  return plane.dist - plane.normal.x * x - plane.normal.y * y;
...
// Computes a point at a specified y value along an edge
//-----------------------------------------------------------------------------
inline void CEdgeList::ComputePointAlongEdge(const Edge_t *pEdge, int nSurfID,
                                             float y, Vector *pPoint) const {
  Assert((y >= pEdge->m_vecPosition.y) && (y <= pEdge->m_vecPositionEnd.y));

  float t;
  t = (y - pEdge->m_vecPosition.y) * pEdge->m_flOODy;
  pPoint->x = pEdge->m_vecPosition.x +
              (pEdge->m_vecPositionEnd.x - pEdge->m_vecPosition.x) * t;
  pPoint->y = y;
...
void CEdgeList::CullSmallOccluders() {
  ...
  // We're going to keep the greater of r_occludermin + All surfaces with a
  // screen area >= r_occluderarea
...
// If Xs match, sort by Dx/Dy
//-----------------------------------------------------------------------------
inline bool CEdgeList::IsEdgeXGreater(const Edge_t *pEdge1,
                                      const Edge_t *pEdge2) {
  float flDelta = pEdge1->m_flX - pEdge2->m_flX;
  if (flDelta > 0) return true;
...
// Inserts an edge into the active edge list, sorted by X
//-----------------------------------------------------------------------------
inline void CEdgeList::InsertActiveEdge(Edge_t *pPrevEdge,
                                        Edge_t *pInsertEdge) {
  while (!AtListStart(pPrevEdge) && IsEdgeXGreater(pPrevEdge, pInsertEdge)) {
    pPrevEdge = pPrevEdge->m_pPrevActiveEdge;
  }
...
inline void CEdgeList::UpdateCurrentSurfaceZValues(float x, float y) {
  ...
  for (Surface_t *pSurf = TopSurface(); !AtSurfListEnd(pSurf);
       pSurf = pSurf->m_pNextSurface) {
...
inline bool CEdgeList::IsSurfaceBehind(Surface_t *pTestSurf, Surface_t *pSurf) {
  if (pTestSurf->m_flOOz - pSurf->m_flOOz <= -1e-6) return true;
  if (pTestSurf->m_flOOz - pSurf->m_flOOz >= 1e-6) return false;
...
// Reduces the active edge list into a subset of ones we truly care about
//-----------------------------------------------------------------------------
void CEdgeList::ReduceActiveEdgeList(CWingedEdgeList &wingedEdgeList,
                                     float flMinY, float flMaxY) {
  ...
  for (; !AtListEnd(pCurEdge); pCurEdge = pNextEdge) {
    ...
    // If we have coincident edges, we have to introduce them at the same
    // time...
    pNextEdge = pCurEdge->m_pNextActiveEdge;
    if ((flPrevX == pNextEdge->m_flX) &&
        (pCurEdge->m_flDxDy == pNextEdge->m_flDxDy))
      continue;
    ...
    for (i = m_nPrevReduceCount; --i >= 0;) {
      CWingedEdgeList::WingedEdge_t &testEdge =
          wingedEdgeList.WingedEdge(m_pPrevReduceInfo[i].m_nWingedEdge);
      if ((testEdge.m_nLeaveSurfID != nLeaveSurfID) ||
          (testEdge.m_nEnterSurfID != nEnterSurfID))
        continue;
      ...
      ComputePointAlongEdge(m_pPrevReduceInfo[i].m_pEdge, nEdgeSurfID, flMaxY,
                            &testEdge.m_vecPositionEnd);
    ...
    if (i < 0) {
      ...
      CWingedEdgeList::WingedEdge_t &newWingedEdge =
          wingedEdgeList.WingedEdge(i);
      ...
      ComputePointAlongEdge(pCurEdge, nEdgeSurfID, flMinY,
                            &newWingedEdge.m_vecPosition);
      ComputePointAlongEdge(pCurEdge, nEdgeSurfID, flMaxY,
                            &newWingedEdge.m_vecPositionEnd);

      // Enforce sort order...
      // Required because we're computing the x position here, which can
      // introduce error.
      if (i != 0) {
        CWingedEdgeList::WingedEdge_t &prevWingedEdge =
            wingedEdgeList.WingedEdge(i - 1);
...
// Discovers the first edge crossing discontinuity
//-----------------------------------------------------------------------------
float CEdgeList::LocateEdgeCrossingDiscontinuity(float flNextY, float flPrevY,
                                                 int &nCount, Edge_t **ppInfo) {
  nCount = 0;
  ...
  for (pCurEdge = FirstActiveEdge(); !AtListEnd(pCurEdge);
    ...
    // pointer the first time through the loop, but it never hits that check
    // since the first X test is guaranteed to pass
    ...
    // N * P = d
    // Normal is perpendicular to the line, therefore N = [-dy dx] = [-1
    // (dx/dy)] d = DotProduct( N, edge.m_vecPosition ) = N dot Pon So, the t
    // that solve the equation is given by t = (d - N dot Pop) / (N dot D) Or, t
    // = (N dot Pon - N dot Pop) / (N dot D) t = (N dot (Pon - Pop)) / (N dot D)

    float flDenominator = 1.0f / (-pPrevEdge->m_flDxDy + pCurEdge->m_flDxDy);
    Vector2DSubtract(pCurEdge->m_vecPosition.AsVector2D(),
                     pPrevEdge->m_vecPosition.AsVector2D(), vecDelta);
    ...
    // if the bitpattern of flYCrossing and flNextY are the exact same, because
    // it's doing the test with the 80bit fp registers. flYCrossing is still
    // sitting in the register from the computation on the line above, but
    // flNextY isn't. Therefore it returns not equal. That's why I have to do
    // the explicit bitpattern check.
...
void CEdgeList::ReorderActiveEdgeList(int nCount, Edge_t **ppCrossings) {
  ...
  while (nCurCrossing < nCount) {
    ...
    // reverse the order of those edges. At the same time, slam the X value of
    // each crossing edge to reduce precision errors
    ...
    while (true) {
      if ((nCurCrossing < nCount) &&
          (pLastCrossing == ppCrossings[nCurCrossing])) {
    ...
    for (Edge_t *pCrossing = pFirstCrossing->m_pNextActiveEdge;
         pCrossing != pLastCrossing; pCrossing = pCrossing->m_pNextActiveEdge) {
      pCrossing->m_flX = flXCrossing;
    }
  }

  // Now re-insert everything to take into account other edges which may well
  // have crossed on this line
  Edge_t *pEdge;
  Edge_t *pNextEdge;
...
void CEdgeList::SpewActiveEdgeList(float y, bool bHex) {
  ...
  while (!AtListEnd(pEdge)) {
    if (!bHex) {
      Msg("(%d %.3f [%d]) ", (int)(pEdge - m_Edges.Base()), pEdge->m_flX,
          pEdge->m_nSurfID);
    } else {
      Msg("(%d %X [%d]) ", (int)(pEdge - m_Edges.Base()), *(int *)&pEdge->m_flX,
          pEdge->m_nSurfID);
    }
    pEdge = pEdge->m_pNextActiveEdge;
  }
...
void CEdgeList::ReduceActiveList(CWingedEdgeList &newEdgeList) {
  ...
  bool bDone = false;
  while (!bDone) {
    // Don't immediately progress to the next discontinuity if there are edge
    // crossings.
    float flNextY = LocateEdgeCrossingDiscontinuity(
        NextDiscontinuity(), flPrevY, nEdgeCrossingCount, pEdgeCrossings);
    ...
    // If we had an edge crossing, re-order the edges. Otherwise introduce new
    // active edges
    if (!nEdgeCrossingCount) {
      IntroduceNewActiveEdges(flNextY);

      // Keep advancing the active edge list until it's got no more
      // discontinuities
      if (NextDiscontinuity() == FLT_MAX) return;
    } else {
      ...
      if (m_nCurrentEdgeIndex < nEdgeCount) {
        float flNextEdgeY =
            EdgeFromSortIndex(m_nCurrentEdgeIndex).m_vecPosition.y;
...
class COcclusionSystem : public IOcclusionSystem {
 public:
  ...
  virtual void SetView(const Vector &vecCameraPos, float flFOV,
  ...
  virtual void SetOcclusionParameters(float flMaxOccludeeArea,
 ...
 private:
  ...
  void IntersectWithNearPlane(const Vector &vecStart, const Vector &vecEnd,
                              Vector &outPos) const;

  // Clips a polygon to the near clip plane
  int ClipPolygonToNearPlane(Vector **ppVertices, int nVertexCount,
                             Vector **ppOutVerts, bool *pClipped) const;

  // Project world-space verts + add into the edge list
  void AddPolygonToEdgeList(CEdgeList &edgeList, Vector **ppPolygon, int nCount,
                            int nSurfID, bool bClipped);

  // Computes the plane equation of a polygon in screen space from a
  // camera-space plane
  void ComputeScreenSpacePlane(const cplane_t &cameraSpacePlane,
                               cplane_t *pScreenSpacePlane);
  ...
  int ClipPolygonToAxisAlignedPlane(Vector **ppVertices, int nVertexCount,
                                    const AxisAlignedPlane_t &plane,
                                    Vector **ppOutVerts) const;

  // Is the point within an axis-aligned plane?
  bool IsPointInsideAAPlane(const Vector &vecPos,
                            const AxisAlignedPlane_t &plane) const;
  void IntersectWithAAPlane(const Vector &vecStart, const Vector &vecEnd,
                            const AxisAlignedPlane_t &plane,
                            Vector &outPos) const;
...
//-----------------------------------------------------------------------------
void COcclusionSystem::SetOcclusionParameters(float flMaxOccludeeArea,
                                              float flMinOccluderArea) {
  m_flMaxOccludeeArea =
      (flMaxOccludeeArea ? flMaxOccludeeArea : DEFAULT_MAX_OCCLUDEE_AREA) *
      0.01f;
  m_flMinOccluderArea =
      (flMinOccluderArea ? flMinOccluderArea : DEFAULT_MIN_OCCLUDER_AREA);
}
...
//-----------------------------------------------------------------------------
inline bool COcclusionSystem::IsPointInsideNearPlane(
    const Vector &vecPos) const {
  return DotProduct(vecPos, m_NearClipPlane.normal) >= m_NearClipPlane.dist;
}

inline void COcclusionSystem::IntersectWithNearPlane(const Vector &vecStart,
                                                     const Vector &vecEnd,
                                                     Vector &outPos) const {
  Vector vecDir;
  VectorSubtract(vecEnd, vecStart, vecDir);
  float t = IntersectRayWithPlane(vecStart, vecDir, m_NearClipPlane.normal,
                                  m_NearClipPlane.dist);
...
int COcclusionSystem::ClipPolygonToNearPlane(Vector **ppVertices,
                                             int nVertexCount,
                                             Vector **ppOutVerts,
                                             bool *pClipped) const {
  *pClipped = false;
...
// Is the point within an axis-aligned plane?
//-----------------------------------------------------------------------------
inline bool COcclusionSystem::IsPointInsideAAPlane(
    const Vector &vecPos, const AxisAlignedPlane_t &plane) const {
  return vecPos[plane.m_nAxis] * plane.m_flSign >= plane.m_flDist;
}

inline void COcclusionSystem::IntersectWithAAPlane(
    const Vector &vecStart, const Vector &vecEnd,
    const AxisAlignedPlane_t &plane, Vector &outPos) const {
  float t = IntersectRayWithAAPlane(vecStart, vecEnd, plane.m_nAxis,
                                    plane.m_flSign, plane.m_flDist);
  VectorLerp(vecStart, vecEnd, t, outPos);
}
...
int COcclusionSystem::ClipPolygonToAxisAlignedPlane(
    Vector **ppVertices, int nVertexCount, const AxisAlignedPlane_t &plane,
    Vector **ppOutVerts) const {
  // Ye Olde Sutherland-Hodgman clipping algorithm
  int nOutVertCount = 0;
...
//-----------------------------------------------------------------------------
// Computes the plane equation of a polygon in screen space from a world-space
// plane
//-----------------------------------------------------------------------------
void COcclusionSystem::ComputeScreenSpacePlane(const cplane_t &cameraSpacePlane,
                                               cplane_t *pScreenSpacePlane) {
  ...
  float ooD =
      (cameraSpacePlane.dist != 0) ? (1.0f / cameraSpacePlane.dist) : 0.0f;
  pScreenSpacePlane->normal.x =
      cameraSpacePlane.normal.x * ooD * m_flXProjScale;
  pScreenSpacePlane->normal.y =
      cameraSpacePlane.normal.y * ooD * m_flYProjScale;
  pScreenSpacePlane->normal.z = 1;
  pScreenSpacePlane->dist =
      cameraSpacePlane.normal.z * ooD * m_flProjDistScale + m_flProjDistOffset;
}
...
// Project world-space verts + add into the edge list
//-----------------------------------------------------------------------------
void COcclusionSystem::AddPolygonToEdgeList(CEdgeList &edgeList,
                                            Vector **ppPolygon, int nCount,
                                            int nSurfID, bool bClipped) {
  // Transform the verts into projection space
  // Transform into projection space (extra logic here is to simply guarantee
  ...
  int k;
  for (k = 0; k < nCount; ++k) {
    Vector3DMultiplyPositionProjective(m_WorldToProjection, *(ppPolygon[k]),
                                       pVecProjectedVertex[k]);
  ...
  nClipCount1 = ClipPolygonToAxisAlignedPlane(ppClipVertex, nClipCount, aaPlane,
                                              ppClipVertex1);
  if (nClipCount1 < 3) return;
  Assert(nClipCount1 < nMaxClipVerts);

  aaPlane.m_flSign = 1;
  nClipCount = ClipPolygonToAxisAlignedPlane(ppClipVertex1, nClipCount1,
                                             aaPlane, ppClipVertex);
  if (nClipCount < 3) return;
  ...
  for (k = 1; k < nLastClipVert; ++k) {
    ...
    float flTriArea = TriArea2DTimesTwo((*ppClipVertex[0]), (*ppClipVertex[k]),
                                        (*ppClipVertex[k + 1]));
...
void COcclusionSystem::RecomputeOccluderEdgeList() {
  if (!m_bEdgeListDirty) return;

  // Tracker 17772:  If building cubemaps can end up calling into here w/o
  // cl.pAreaBits setup yet, oh well.
  if (!cl.m_bAreaBitsValid && CommandLine()->FindParm("-buildcubemaps")) return;
  ...
  for (i = host_state.worldbrush->numoccluders; --i >= 0;) {
    ...
    for (j = 0; j < nSurfCount; ++j, ++nSurfID) {
      ...
      bool bClipped;
      int nClipCount = ClipPolygonToNearPlane(ppSurfVerts, nVertexCount,
                                              ppClipVerts, &bClipped);
      Assert(nClipCount <= (nVertexCount * 2));
      if (nClipCount < 3) continue;
      ...
      // Transform into projection space (extra logic here is to simply
      // guarantee that we project each vert exactly once)
      AddPolygonToEdgeList(m_EdgeList, ppClipVerts, nClipCount, nEdgeSurfID,
                           bClipped);
    }
  }
  ...
  //	Msg("Edge count %d -> %d\n", m_EdgeList.EdgeCount(),
  // m_WingedEdgeList.EdgeCount() );
...
void COcclusionSystem::ActivateOccluder(int nOccluderIndex, bool bActive) {
  if ((nOccluderIndex >= host_state.worldbrush->numoccluders) ||
      (nOccluderIndex < 0))
    return;

  if (bActive) {
    host_state.worldbrush->occluders[nOccluderIndex].flags &=
        ~OCCLUDER_FLAGS_INACTIVE;
  } else {
    host_state.worldbrush->occluders[nOccluderIndex].flags |=
        OCCLUDER_FLAGS_INACTIVE;
  }
...
void COcclusionSystem::SetView(const Vector &vecCameraPos, float flFOV,
                               const VMatrix &worldToCamera,
                               const VMatrix &cameraToProjection,
                               const VPlane &nearClipPlane) {
  ...
  m_bEdgeListDirty = true;
  m_flNearPlaneDist = -(DotProduct(vecCameraPos, m_NearClipPlane.normal) -
                        m_NearClipPlane.dist);
...
static unsigned char s_VisualizationColor[2][4] = {{255, 0, 0, 255},
                                                   {0, 255, 0, 255}};
...
// for the first face listed and vert1 -> vert0 is clockwise for the 2nd face
// listed
...
static int s_pFaceEdges[6][4] = {
    {4, 9, 6, 1},  {2, 7, 10, 5}, {0, 5, 8, 4},
    {6, 11, 7, 3}, {1, 3, 2, 0},  {8, 10, 11, 9},
};
...
class WingedEdgeLessFunc {
 public:
  bool Less(const int &src1, const int &src2, void *pCtx) {
    ...
    if (pV1->y < pV2->y) return true;
    if (pV1->y > pV2->y) return false;
    if (pV1->x < pV2->x) return true;
    if (pV1->x > pV2->x) return false;

    // This is the same as the following line:
    //	return (pEdge1->m_flDxDy <= pEdge2->m_flDxDy);
    Vector2D dEdge1, dEdge2;
    Vector2DSubtract(
        pVertices[pEdge1->m_nVert[1 - pEdge1->m_nMinVert]].AsVector2D(),
        pV1->AsVector2D(), dEdge1);
    Vector2DSubtract(
        pVertices[pEdge2->m_nVert[1 - pEdge2->m_nMinVert]].AsVector2D(),
        pV2->AsVector2D(), dEdge2);
...
bool COcclusionSystem::IsOccluded(const Vector &vecAbsMins,
  ...
  if (flScreenArea >= flMaxSize) return false;
  ...
  // (probably a single plane test, which tests if the box is wholly behind the
  // camera )
  ...
  //		Vector3DMultiplyPositionProjective( m_WorldToProjection,
  // vecBoxVertex, pVecProjectedVertex[ i ] );
  ...
  if (vecProjVert[0].w <= 0.0f) return false;
  float flOOW = 1.0f / vecProjVert[0].w;

  vecDeltaProj[0].Init(vecSize.x * m_WorldToProjection[0][0],
                       vecSize.x * m_WorldToProjection[1][0],
                       vecSize.x * m_WorldToProjection[2][0],
                       vecSize.x * m_WorldToProjection[3][0]);
  vecDeltaProj[1].Init(vecSize.y * m_WorldToProjection[0][1],
                       vecSize.y * m_WorldToProjection[1][1],
                       vecSize.y * m_WorldToProjection[2][1],
                       vecSize.y * m_WorldToProjection[3][1]);
  vecDeltaProj[2].Init(vecSize.z * m_WorldToProjection[0][2],
                       vecSize.z * m_WorldToProjection[1][2],
                       vecSize.z * m_WorldToProjection[2][2],
                       vecSize.z * m_WorldToProjection[3][2]);

  pVecProjectedVertex[0].Init(vecProjVert[0].x * flOOW,
                              vecProjVert[0].y * flOOW,
                              vecProjVert[0].z * flOOW);
  if (pVecProjectedVertex[0].z <= 0.0f) return false;
  ...
  for (i = 1; i < 8; ++i) {
    ...
    if (vecProjVert[i].w <= 0.0f) return false;
    flOOW = 1.0f / vecProjVert[i].w;
    pVecProjectedVertex[i].Init(vecProjVert[i].x * flOOW,
                                vecProjVert[i].y * flOOW,
                                vecProjVert[i].z * flOOW);
    if (pVecProjectedVertex[i].z <= 0.0f) return false;
  }
  ...
  for (i = 0; i < 6; ++i) {
    ...
    MatrixTransformAxisAlignedPlane(m_WorldToCamera, nDim, flSign, flPlaneDist,
                                    cameraSpacePlane);
    ComputeScreenSpacePlane(cameraSpacePlane, &projectionSpacePlane);
  ...
  for (i = 0; i < 12; ++i) {
    ...
    if (pEdge->m_nTestCount != m_nTests) continue;

    pEdge->m_nMinVert = (pVecProjectedVertex[pEdge->m_nVert[0]].y >=
                         pVecProjectedVertex[pEdge->m_nVert[1]].y);
    edgeSort.Insert(i);
  }
  ...
  for (i = 0; i < nEdgeCount; ++i) {
    ...
    const Vector &startVert =
        pVecProjectedVertex[pEdge->m_nVert[pEdge->m_nMinVert]];
    const Vector &endVert =
        pVecProjectedVertex[pEdge->m_nVert[1 - pEdge->m_nMinVert]];
    int nLeaveSurfID = pSurfInd[pEdge->m_nFace[pEdge->m_nMinVert]];
    int nEnterSurfID = pSurfInd[pEdge->m_nFace[1 - pEdge->m_nMinVert]];

    s_WingedTestEdgeList.AddEdge(startVert, endVert, nLeaveSurfID,
                                 nEnterSurfID);
  }
