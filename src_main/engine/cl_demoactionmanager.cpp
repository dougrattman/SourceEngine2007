    ...
    if (kv->LoadFromFile(g_pFullFileSystem, metafile)) {
      ...
      while (pIter) {
        ...
        if (atoi(factorytouse) > 0) {
          Q_strncpy(factorytouse, pIter->GetString("factory", ""),
                    sizeof(factorytouse));
        }

        CBaseDemoAction *action = CBaseDemoAction::CreateDemoAction(
            CBaseDemoAction::TypeForName(factorytouse));
        if (action) {
...
void CDemoActionManager::StartPlaying(char const *demfilename) {
  ...
  // If didn't change file and the timestamps are the same, don't transition to
  // new .vdm
...
// Input  : dirty -
//-----------------------------------------------------------------------------
void CDemoActionManager::SetDirty(bool dirty) { m_bDirty = true; }
...
void CDemoActionManager::InsertFireEvent(CBaseDemoAction *action) {
  // BUGBUG: Sometimes this can get called multiple times for the same event
  // before DispatchEvents() is called.  Why? If that gets fixed, remove this
  // hack.
  if (m_PendingFireActionStack.Find(action) >= 0) return;
