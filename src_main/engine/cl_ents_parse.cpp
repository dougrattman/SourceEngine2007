...
void SpewToFile(char const *pFmt, ...) {
  ...
  if (1)  // s_RecordingBuffer.Size() > 8192)
  {
    ...
    g_pFileSystem->Write(s_RecordingBuffer.Base(), s_RecordingBuffer.Size(),
                         fp);
...
void CL_DeleteDLLEntity(int iEnt, const char *reason,
  ...
  if (pNet) {
    ...
    TRACE_DELTA(va("Trace %i (%s): delete (%s)\n", iEnt,
                   pClientClass ? pClientClass->m_pNetworkName : "unknown",
                   reason));
...
inline static void CL_AddPostDataUpdateCall(CEntityReadInfo &u, int iEnt,
...
void CL_CopyNewEntity(CEntityReadInfo &u, int iClass, int iSerialNum) {
  ...
  if (ent) {
    ...
    if (ent->GetIClientUnknown()->GetRefEHandle().GetSerialNumber() !=
        iSerialNum) {
  ...
  if (!ent) {
    ...
    if (!ent) {
      Host_Error("CL_ParsePacketEntities:  Error creating entity %s(%i)\n",
                 cl.m_pServerClasses[iClass].m_pClientClass->m_pNetworkName,
                 u.m_nNewEntity);
      return;
    }
  ...
  DataUpdateType_t updateType =
      bNew ? DATA_UPDATE_CREATED : DATA_UPDATE_DATATABLE_CHANGED;
  ...
  PackedEntity *baseline =
      u.m_bAsDelta ? cl.GetEntityBaseline(u.m_nBaseline, u.m_nNewEntity) : NULL;
  if (baseline && baseline->m_pClientClass == pClass) {
    ...
    ErrorIfNot(cl.GetClassBaseline(iClass, &pFromData, &nFromBits),
  ...
  bf_read fromBuf("CL_CopyNewEntity->fromBuf", pFromData, Bits2Bytes(nFromBits),
                  nFromBits);
  ...
  if (u.m_bUpdateBaselines) {
    ...
    cl.SetEntityBaseline((u.m_nBaseline == 0) ? 1 : 0, pClass, u.m_nNewEntity,
                         packedData, writeBuf.GetNumBytesWritten());

    fromBuf.StartReading(packedData, writeBuf.GetNumBytesWritten());

    RecvTable_Decode(pRecvTable, ent->GetDataTableBasePtr(), &fromBuf,
                     u.m_nNewEntity);
    ...
    RecvTable_Decode(pRecvTable, ent->GetDataTableBasePtr(), u.m_pBuf,
                     u.m_nNewEntity);
  }
...
#ifndef _XBOX
  if (cl_entityreport.GetBool()) CL_RecordEntityBits(u.m_nNewEntity, bit_count);
#endif
...
void CL_CopyExistingEntity(CEntityReadInfo &u) {
  ...
  RecvTable_Decode(pRecvTable, pEnt->GetDataTableBasePtr(), u.m_pBuf,
                   u.m_nNewEntity);
...
static void CL_CallPostDataUpdates(CEntityReadInfo &u) {
  for (int i = 0; i < u.m_nPostDataUpdateCalls; i++) {
    ...
    ErrorIfNot(pEnt, ("CL_CallPostDataUpdates: missing ent %d", pCall->m_iEnt));
...
//  rest of the data stream.  This can be a delta from the baseline or from a
//  previous client frame for this client.
// Input  : delta -
//			*playerbits -
...
bool CL_ProcessPacketEntities(SVC_PacketEntities *entmsg) {
  ...
  // if cl_flushentitypacket is set to N, the next N entity updates will be
  // flushed
  if (cl_flushentitypacket.GetInt()) {
    // we can't use this, it is too old
    CL_FlushEntityPacket(newFrame, "Forced by cvar\n");
    cl_flushentitypacket.SetValue(cl_flushentitypacket.GetInt() -
                                  1);  // Reduce the cvar.
    return false;
  }
  ...
  if (entmsg->m_bIsDelta) {
    ...
    // Otherwise, mark where we are valid to and point to the packet entities
    // we'll be updating from.
  ...
  cl.m_NetChannel->UpdateMessageStats(INetChannelInfo::LOCALPLAYER,
                                      u.m_nLocalPlayerBits);
  cl.m_NetChannel->UpdateMessageStats(INetChannelInfo::OTHERPLAYERS,
                                      u.m_nOtherPlayerBits);
  cl.m_NetChannel->UpdateMessageStats(
      INetChannelInfo::ENTITIES,
      -(u.m_nLocalPlayerBits + u.m_nOtherPlayerBits));

  cl.DeleteClientFrames(entmsg->m_nDeltaFrom);

  // If the client has more than 64 frames, the host will start to eat too much
  // memory.
...
void CL_PreprocessEntities(void) {
  ...
  bool bLastOutgoingCommandEqualsLastAcknowledgedCommand =
      cl.lastoutgoingcommand == cl.command_ack;

  // We always want to re-run prediction when using the multiplayer networking,
  // or if we're the listen server and we get a packet
  ...
  // Anything not sent over the network from server to client must be specified
  // here.
  // if ( cl.last_command_ack  )
  {
    ...
    g_pClientSidePrediction->PreEntityPacketReceived(
        number_of_commands_executed, cl.m_nCurrentSequence);
  }
