...
class CRConServer : public ISocketCreatorListener {
 ...
 private:
  struct ConnectedRConSocket_t {
    ...
    CUtlBuffer packetbuffer;  // data buffer containing pending incoming data
                              // for this connection
    CUtlLinkedList<CUtlBuffer>
        m_OutstandingSends;  // packets pending to be send (queued because of
                             // WSAEWOULDBLOCK )
  };
  ...
  struct FailedRCon_t {
    ...
    bool operator==(const FailedRCon_t &rhs) const {
      return (adr.CompareAdr(rhs.adr) == 0);
    }
    bool operator<(const FailedRCon_t &rhs) const;
  };
  ...
  bool SendRCONResponse(int nIndex, const void *data, int len,
...
inline CRConServer::ConnectedRConSocket_t *CRConServer::GetSocketData(
    int nIndex) {
