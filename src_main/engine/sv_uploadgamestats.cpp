...
enum ESendMethod {
  ...
  eSendMethodCompressedBlocks,  // TODO: Reenable compressed sending of
                                // minidumps
...
const uint32_t cuNumCommandResponses = 5;
}  // namespace HarvestFileCommand
...
typedef void (*GAMESTATSREPORTPROGRESSFUNC)(
    u32 uContext, const TGameStatsProgress& rGameStatsProgress);

struct TGameStatsParameters {
  TGameStatsParameters() : m_uAppId(0) {}
...
#define GAMESTATSUPLOADER_CONNECT_RETRY_TIME 1.0
  ...
  void UpdateConnection(void) {
    ...
    if (CommandLine()->FindParm("-localcser") || !g_pFileSystem->IsSteam()) {
      char const* cserIP =
          CommandLine()->ParmValue("-localcser", "steambeta1:27013");
      if (cserIP && cserIP[0]) {
  ...
  bool ForceGameStats() const {
    bool bForcingGameStats =
        CommandLine()->FindParm("-gamestats") ? true : false;
    return bForcingGameStats;
  }

  virtual bool UploadGameStats(char const* mapname, unsigned int blobversion,
                               unsigned int blobsize, const void* pvBlobData) {
    ...
    Q_strncpy(params.m_sExecutableName, "hl2.exe",
              sizeof(params.m_sExecutableName));
    Q_FileBase(com_gamedir, params.m_sGameDirectory,
               sizeof(params.m_sGameDirectory));
  ...
  // Gets a non-personally identifiable unique ID for this steam user, used for
  // tracking total gameplay time across
...
EXPOSE_SINGLE_INTERFACE(CUploadGameStats, IUploadGameStats,
...
class CWin32UploadGameStats {
 public:
  explicit CWin32UploadGameStats(
      ...
      const TGameStatsParameters& rGameStatsParameters, u32 contextid);
 ...
 private:
  enum States {
    ...
    eSendWholeFile,  // This could push chunks onto the wire, but we'll just use
                     // a whole buffer for now.
  ...
  bool ConnectToHarvesterServer(EGameStatsUploadStatus& status,
                                CUtlBuffer& buf);
  ...
  bool ReceiveFileUploadSuccess(EGameStatsUploadStatus& status,
                                CUtlBuffer& buf);
  ...
  typedef bool (CWin32UploadGameStats::*pfnProtocolStateHandler)(
      EGameStatsUploadStatus& status, CUtlBuffer& buf);
  struct FSMState_t {
    FSMState_t(uint32_t f, pfnProtocolStateHandler s) : first(f), second(s) {}

    uint32_t first;
    pfnProtocolStateHandler second;
  };

  void AddState(uint32_t StateIndex, pfnProtocolStateHandler handler);
  void SetNextState(uint32_t StateIndex);
  bool DoBlockingReceive(uint32_t bytesExpected, CUtlBuffer& buf);

  CUtlVector<FSMState_t> m_States;
  uint32_t m_uCurrentState;
  struct sockaddr_in m_HarvesterSockAddr;
  uint32_t m_SocketTCP;
  const TGameStatsParameters& m_rCrashParameters;  // lint !e1725
  u32 m_ContextID;
};

CWin32UploadGameStats::CWin32UploadGameStats(
    const netadr_t& harvester, const TGameStatsParameters& rGameStatsParameters,
    u32 contextid)
    : m_States(),
  ...
  AddState(eConnectToHarvesterServer,
  ...
  AddState(eReceiveFileUploadSuccess,
           &CWin32UploadGameStats::ReceiveFileUploadSuccess);
...
//
//-----------------------------------------------------------------------------
bool CWin32UploadGameStats::DoBlockingReceive(uint32_t bytesExpected,
                                              CUtlBuffer& buf) {
  ...
  for (;;) {
    ...
    if (totalReceived >= bytesExpected) break;
  }
  return true;
}

void CWin32UploadGameStats::AddState(uint32_t StateIndex,
                                     pfnProtocolStateHandler handler) {
...
EGameStatsUploadStatus CWin32UploadGameStats::Upload(CUtlBuffer& buf) {
  UpdateProgress(m_rCrashParameters,
...
bool CWin32UploadGameStats::ConnectToHarvesterServer(
  ...
  if (connect(m_SocketTCP, (const sockaddr*)&m_HarvesterSockAddr,
              sizeof(m_HarvesterSockAddr)) == SOCKET_ERROR) {
...
bool CWin32UploadGameStats::SendProtocolVersion(EGameStatsUploadStatus& status,
                                                CUtlBuffer& buf) {
  UpdateProgress(m_rCrashParameters,
  ...
  if (send(m_SocketTCP, (const char*)buf.Base(), (int)buf.TellPut(), 0) ==
      SOCKET_ERROR) {
...
bool CWin32UploadGameStats::ReceiveProtocolOkay(EGameStatsUploadStatus& status,
                                                CUtlBuffer& buf) {
  UpdateProgress(m_rCrashParameters,
...
bool CWin32UploadGameStats::SendUploadCommand(EGameStatsUploadStatus& status,
                                              CUtlBuffer& buf) {
  UpdateProgress(m_rCrashParameters,
  ...
  buf.PutInt(static_cast<HarvestFileCommand::SendMethod_t>(
      eSendMethodWholeRawFileNoBlocks));
...
bool CWin32UploadGameStats::ReceiveOKToSendFile(EGameStatsUploadStatus& status,
                                                CUtlBuffer& buf) {
  UpdateProgress(m_rCrashParameters,
...
bool CWin32UploadGameStats::SendWholeFile(EGameStatsUploadStatus& status,
                                          CUtlBuffer& /*buf*/) {
  UpdateProgress(m_rCrashParameters, "Uploading game stats data.");
  // Send to server
  bool bret = true;
  if (send(m_SocketTCP, (const char*)m_rCrashParameters.m_pStatsBlobData,
           (int)m_rCrashParameters.m_uStatsBlobSize, 0) == SOCKET_ERROR) {
    bret = false;
    UpdateProgress(m_rCrashParameters, "Send failed.");
...
bool CWin32UploadGameStats::ReceiveFileUploadSuccess(
    EGameStatsUploadStatus& status, CUtlBuffer& buf) {
  UpdateProgress(m_rCrashParameters,
...
bool CWin32UploadGameStats::SendGracefulClose(EGameStatsUploadStatus& status,
                                              CUtlBuffer& buf) {
...
bool CWin32UploadGameStats::CloseTCPSocket(EGameStatsUploadStatus& status,
                                           CUtlBuffer& /*buf*/) {
...
EGameStatsUploadStatus Win32UploadGameStatsBlocking(
  ...
  if (0)  // This is the old protocol
  {
    ...
    buf.PutInt((int)rGameStatsParameters
                   .m_uStatsBlobVersion);  // game stats blob version
    buf.PutInt(
        (int)rGameStatsParameters.m_uStatsBlobSize);  // game stats blob size
  } else {
  ...
  UpdateProgress(rGameStatsParameters, "Sending game stats to server %s.",
                 rGameStatsParameters.m_ipCSERServer.ToString());

  bcs.SendSocketMessage(sa, (const u8*)buf.Base(), buf.TellPut());  // lint
                                                                    // !e534
  ...
  if (bcs.WaitForMessage(2.0f)) {
    ...
    uint32_t bytesReceived =
        bcs.ReceiveSocketMessage(&replyaddress, (u8*)buf.Base(), 4096);
    if (bytesReceived > 0) {
      // Fixup actual size
      ...
      if (M2C_ACKREPORT_GAMESTATISTICS != msgtype) {
        UpdateProgress(rGameStatsParameters,
      ...
      UpdateProgress(rGameStatsParameters,
                     ...
                     GameStatsHarvesterFSMIPAddress.ToString());

      // Keep using the same scratch buffer for messaging
      CWin32UploadGameStats uploader(GameStatsHarvesterFSMIPAddress,
                                     rGameStatsParameters, dumpcontext);
