...
ConVar r_visualizelighttracesshowfulltrace(
    "r_visualizelighttracesshowfulltrace", "0", FCVAR_CHEAT);
...
// Returns true if the surface has the specified dlight already set on it for
// this frame.
inline bool R_IsDLightAlreadyMarked(msurfacelighting_t *pLighting, int bit) {
  return (pLighting->m_nDLightFrame == r_framecount) &&
         (pLighting->m_fDLightBits & bit);
}

// Mark the surface as changed by the specified dlight (so its texture gets
// updated when it comes time to render).
inline void R_MarkSurfaceDLight(SurfaceHandle_t surfID,
                                msurfacelighting_t *pLighting, int bit) {
  pLighting->m_nDLightFrame = r_framecount;
  pLighting->m_fDLightBits |= bit;
  MSurf_Flags(surfID) |= SURFDRAW_HASDLIGHT;
}

int R_TryLightMarkSurface(dlight_t *light, msurfacelighting_t *pLighting,
                          SurfaceHandle_t surfID, int bit) {
  // Make sure this light actually intersects the surface cache of the surfaces
  // it hits
  mtexinfo_t *tex;
  ...
  float flInPlaneLuxelRadius = sqrtf(
      flInPlaneRadiusSq * tex->luxelsPerWorldUnit * tex->luxelsPerWorldUnit);

  // Does the circle intersect the square?
  if (!IsCircleIntersectingRectangle(mins, maxs, vecCircleCenter,
                                     flInPlaneLuxelRadius))
    return 0;
...
int R_MarkLightsLeaf(dlight_t *light, int bit, mleaf_t *pLeaf) {
  ...
  for (int i = 0; i < pLeaf->dispCount; i++) {
    ...
    if (parentSurfID) {
      // Don't redo all this work if we already hit this surface and decided
      ...
      if (!R_IsDLightAlreadyMarked(pLighting, bit)) {
        ...
        if (IsBoxIntersectingSphere(bmin, bmax, light->origin,
                                    light->GetRadius())) {
  ...
  for (int i = 0; i < pLeaf->nummarksurfaces; i++) {
    ...
    // Don't redo all this work if we already hit this surface and decided it's
    // lit by this light.
    msurfacelighting_t *pLighting = SurfaceLighting(surfID);
    if (R_IsDLightAlreadyMarked(pLighting, bit)) continue;

    float dist = DotProduct(light->origin, MSurf_Plane(surfID).normal) -
                 MSurf_Plane(surfID).dist;

    if (dist > light->GetRadius() || dist < -light->GetRadius()) continue;
...
void R_MarkDLightsOnSurface(mnode_t *pNode) {
  if (!pNode || !g_bActiveDlights) return;

  dlight_t *l = cl_dlights;
  for (int i = 0; i < MAX_DLIGHTS; i++, l++) {
    if (l->die < cl.GetTime() || !l->IsRadiusGreaterThanZero()) continue;
    if (l->flags & DLIGHT_NO_WORLD_ILLUMINATION) continue;
...
static void ComputeTextureCoordsAtIntersection(mtexinfo_t *pTex,
                                               Vector const &pt,
                                               float *textureS,
                                               float *textureT) {
  if (pTex->material && textureS && textureT) {
    ...
    *textureT =
        DotProduct(pt, pTex->textureVecsTexelsPerWorldUnits[1].AsVector3D()) +
...
// Computes s and t coords of texture at intersection pt
//-----------------------------------------------------------------------------
static void ComputeLightmapCoordsAtIntersection(msurfacelighting_t *pLighting,
                                                float ds, float dt,
                                                float *lightmapS,
                                                float *lightmapT) {
...
//-----------------------------------------------------------------------------
static void ComputeLightmapColor(SurfaceHandle_t surfID, int ds, int dt,
                                 bool bUseLightStyles, Vector &c) {
  msurfacelighting_t *pLighting = SurfaceLighting(surfID);
  ...
  for (int maps = 0; maps < nMaxMaps && pLighting->m_nStyles[maps] != 255;
       ++maps) {
...
// Computes the lightmap color at a particular point
//-----------------------------------------------------------------------------
static void ComputeLightmapColorFromAverage(msurfacelighting_t *pLighting,
                                            bool bUseLightStyles, Vector &c) {
  int nMaxMaps = bUseLightStyles ? MAXLIGHTMAPS : 1;
  for (int maps = 0; maps < nMaxMaps && pLighting->m_nStyles[maps] != 255;
       ++maps) {
...
static bool FASTCALL FindIntersectionAtSurface(SurfaceHandle_t surfID, float f,
  ...
  if (!pLighting->m_LightmapExtents[0] && !pLighting->m_LightmapExtents[1]) {
    ...
    for (i = 0; i < MSurf_VertCount(surfID); i++) {
      ...
      for (j = 0; j < 2; j++) {
        ...
        sextent =
            DotProduct(v->position,
                       pTex->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D()) +
            pTex->lightmapVecsLuxelsPerWorldUnits[0][3] -
            pLighting->m_LightmapMins[0];
        textent =
            DotProduct(v->position,
                       pTex->lightmapVecsLuxelsPerWorldUnits[1].AsVector3D()) +
            pTex->lightmapVecsLuxelsPerWorldUnits[1][3] -
            pLighting->m_LightmapMins[1];
    ...
    if (ds > lightMaxs[0] || dt > lightMaxs[1]) return false;
  } else {
    if (ds > pLighting->m_LightmapExtents[0] ||
        dt > pLighting->m_LightmapExtents[1])
      return false;
  }
...
#endif
  {
    ...
    ComputeLightmapCoordsAtIntersection(pLighting, ds, dt, state.m_pLightmapS,
                                        state.m_pLightmapT);
...
// returns surfID
static SurfaceHandle_t R_LightVecDisplacementChain(LightVecState_t &state,
  ...
  for (int i = 0; i < state.m_LightTestDisps.Count(); i++) {
    ...
    if (pDispInfo->TestRay(state.m_Ray, 0.0f, state.m_HitFrac, dist, &luv,
                           &tuv)) {
      ...
      if (state.m_pLightmapS && state.m_pLightmapT) {
        ComputeLightmapCoordsAtIntersection(SurfaceLighting(surfID), (int)luv.x,
                                            (int)luv.y, state.m_pLightmapS,
                                            state.m_pLightmapT);
      }
...
static void AddDisplacementsInLeafToTestList(mleaf_t *pLeaf,
                                             LightVecState_t &state) {
...
static SurfaceHandle_t FASTCALL FindIntersectionSurfaceAtLeaf(
  ...
  // Adds displacements in the leaf to a list of displacements to test at the
  // end
  ...
  for (int i = pLeaf->nummarknodesurfaces; i < pLeaf->nummarksurfaces; i++) {
    ...
    if (MSurf_Flags(surfID) &
        (SURFDRAW_NODE | SURFDRAW_NODRAW | SURFDRAW_WATERSURFACE))
      continue;
    ...
    if (frac >= state.m_HitFrac) continue;
...
SurfaceHandle_t RecursiveLightPoint(mnode_t *node, float start, float end,
  ...
  if (IS_SURF_VALID(surfID)) return surfID;  // hit something

  // check for impact on this node
  surfID = FindIntersectionSurfaceAtNode(node, mid, c, state);
  if (IS_SURF_VALID(surfID)) return surfID;
...
SurfaceHandle_t R_LightVec(const Vector &start, const Vector &end,
                           bool bUseLightStyles, Vector &c, float *textureS,
                           float *textureT, float *lightmapS,
                           float *lightmapT) {
  ...
  retSurfID =
      RecursiveLightPoint(&model->brush.pShared->nodes[model->brush.firstnode],
                          0.0f, 1.0f, c, state);
  ...
  if (r_visualizelighttraces.GetBool()) {
    if (r_visualizelighttracesshowfulltrace.GetBool()) {
      ...
      CDebugOverlay::AddLineOverlay(start,
                                    start + (end - start) * state.m_HitFrac, 0,
                                    255, 0, 255, true, -1.0f);
    }
  }
  ...
  if ((retSurfID == SURFACE_HANDLE_INVALID) &&
      (state.m_nSkySurfID != SURFACE_HANDLE_INVALID))
