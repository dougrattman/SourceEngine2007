...
CBaseClient::~CBaseClient() { delete m_SteamID; }
...
bool CBaseClient::SendNetMsg(INetMessage &msg, bool bForceReliable) {
  ...
  int nStartBit =
      m_NetChannel->GetNumBitsWritten(msg.IsReliable() || bForceReliable);
...
char const *CBaseClient::GetUserSetting(char const *con_var) const {
  ...
  if (value[0] == 0) {
    ...
    if (m_ConVars->GetDataType(con_var) == KeyValues::TYPE_NONE) {
...
void CBaseClient::SetUpdateRate(int udpaterate, bool bForce) {
  udpaterate = std::clamp(udpaterate, 1, 100);
...
bool CBaseClient::SetSignonState(int state, int spawncount) {
  ...
  switch (m_nSignonState) {
    case SIGNONSTATE_CONNECTED:  // client is connected, leave client in this
                                 // state and let SendPendingSignonData do the
                                 // rest
      m_bSendServerInfo = true;
      break;

    case SIGNONSTATE_NEW:  // client got server info, send prespawn
                           // datam_Client->SendServerInfo()
      if (!SendSignonData()) return false;
...
void CBaseClient::SetName(const char *name) {
  ...
  while (*pFrom && pTo < pLimit) {
    ...
    // Don't copy '#' chars across if they would go into the first position in
    // the name
    if (*pFrom != '%' && *pFrom != '~' &&
        (*pFrom != '#' || pTo != &m_Name[0])) {
      *pTo++ = *pFrom;
  ...
  *pTo = 0;
  ...
  // Don't care about duplicate names on the xbox. It can only occur when a
  // player is reconnecting after crashing, and we don't want to ever show the
  // (X) then.
  if (!IsX360()) {
    ...
    while (true) {
      for (i = 0; i < m_Server->GetClientCount(); i++) {
        ...
        if (!client->IsConnected() || client == this) continue;

        if (!Q_stricmp(client->GetClientName(), val)) break;
      }
      ...
      Q_snprintf(newname, sizeof(newname), "(%d)%-0.*s", dupc++,
                 MAX_PLAYER_NAME_LENGTH - 4, p);
...
void CBaseClient::SpawnPlayer(void) {
  ...
  NET_Tick tick(m_Server->GetTick(), host_frametime_unbounded,
...
void CBaseClient::Disconnect(const char *fmt, ...) {
  ...
  // Send the remaining reliable buffer so the client finds out the server is
  // shutting down.
  if (m_NetChannel) {
...
bool CBaseClient::SendServerInfo(void) {
  ...
  if (developer.GetBool() || m_Server->IsMultiplayer()) {
    ...
    Q_snprintf(
        devtext, sizeof(devtext),
        "\n%s\nMap: %s\nPlayers: %i / %i\nBuild: %d\nServer Number: %i\n\n",
        serverGameDLL->GetGameDescription(), m_Server->GetMapName(), curplayers,
        m_Server->GetMaxClients(), build_number(), m_Server->GetSpawnCount());
...
bool CBaseClient::ProcessBaselineAck(CLC_BaselineAck *msg) {
  if (msg->m_nBaselineTick != m_nBaselineUpdateTick) {
    // This occurs when there are multiple ack's queued up for processing from a
    // client.
    return true;
  }
  ...
  if (frame == NULL) {
    // Will get here if we have a lot of packet loss and finally receive a stale
    // ack from
    //  remote client.  Our "window" could be well beyond what it's acking, so
    //  just ignore the ack.
    return true;
  }
  ...
  while (index >= 0) {
    // get new entity
    PackedEntityHandle_t hNewEntity =
        pSnapshot->m_pEntities[index].m_pPackedData;
    ...
    PackedEntityHandle_t hOldEntity =
        m_pBaseline->m_pEntities[index].m_pPackedData;
  ...
  for (int i = 0; i < MAX_EVENT_NUMBER; i++) {
    if (msg->m_EventArray.Get(i)) {
      CGameEventDescriptor *descriptor =
          g_GameEventManager.GetEventDescriptor(i);
...
bool CBaseClient::IsTracing() const { return m_Trace.m_nMinWarningBytes != 0; }
...
void CBaseClient::EndTrace(bf_write &msg) {
  ...
  logData.Printf(
      "%f/%d Player [%s][%d][adr:%s] was sent a datagram %d bits (%8.3f "
      "bytes)\n",
      realtime, host_tickcount, GetClientName(), GetPlayerSlot(),
      GetNetChannel()->GetAddress(), bits, (float)bits / 8.0f);
...
void CBaseClient::SendSnapshot(CClientFrame *pFrame) {
  ...
  bf_write msg("CBaseClient::SendSnapshot", m_SnapshotScratchBuffer,
               sizeof(m_SnapshotScratchBuffer));

  TRACE_PACKET(("SendSnapshot(%d)\n", pFrame->tick_count));

  // now create client snapshot packet
  CClientFrame *deltaFrame =
      GetDeltaFrame(m_nDeltaTick);  // NULL if delta_tick is not found
  if (!deltaFrame) {
  ...
  // send tick time
  NET_Tick tickmsg(pFrame->tick_count, host_frametime_unbounded,
                   host_frametime_stddeviation);
...
#ifndef SHARED_NET_STRING_TABLES
  // in LocalNetworkBackdoor mode we updated the stringtables already in
  // SV_ComputeClientPacks()
  if (!g_pLocalNetworkBackdoor) {
    // Update shared client/server string tables. Must be done before sending
    // entities
    m_Server->m_StringTables->WriteUpdateMessage(this, GetMaxAckTickCount(),
                                                 msg);
  }
#endif
  ...
  m_Server->WriteTempEntities(this, pFrame->GetSnapshot(),
                              m_pLastSnapshot.GetObject(), msg, nMaxTempEnts);
  ...
  // Don't send the datagram to fakeplayers unless sv_stressbots is on (which
  // will make m_NetChannel non-0).
...
bool CBaseClient::ShouldSendMessages(void) {
  ...
  if (!bSendMessage && !IsActive()) {
    ...
    if (m_bReceivedPacket && m_NetChannel &&
        m_NetChannel->HasPendingReliableData()) {
      bSendMessage = true;
    }
  }
...
void CBaseClient::UpdateSendState(void) {
  ...
  } else if (IsActive())            // multiplayer mode
  {
    ...
    float maxDelta = std::min(m_Server->GetTickInterval(), m_fSnapshotInterval);
    float delta =
        std::clamp((float)(net_time - m_fNextMessageTime), 0.0f, maxDelta);
...
void CBaseClient::UpdateUserSettings() {
  ...
  SetMaxRoutablePayloadSize(
      m_ConVars->GetInt("net_maxroutable", MAX_ROUTABLE_PAYLOAD));
...
bool CBaseClient::UpdateAcknowledgedFramecount(int tick) {
  ...
  if (m_nForceWaitForTick > 0) {
    if (tick > m_nForceWaitForTick) {
      // we should never get here since full updates are transmitted as reliable
      // data now Disconnect("Acknowledging reliable snapshot failed.\n");
      return true;
    } else if (tick == -1) {
      if (!m_NetChannel->HasPendingReliableData()) {
        // that's strange: we sent the client a full update, and it was fully
        // received ( no reliable data in waiting buffers ) but the client is
        // requesting another full update.
        //
        // This can happen if they request full updates in succession really
        // quickly (using cl_fullupdate or "record X;stop" quickly). There was a
        // bug here where if we just return out, the client will have nuked its
        // entities and we'd send it a supposedly uncompressed update but
        // m_nDeltaTick was not -1, so it was delta'd and it'd miss lots of
        // stuff. Led to clients getting full spectator mode radar while their
        // player was not a spectator.
  ...
  if ((m_nBaselineUpdateTick > -1) && (m_nDeltaTick > m_nBaselineUpdateTick)) {
    // server sent a baseline update, but it wasn't acknowledged yet so it was
    // probably lost.
    m_nBaselineUpdateTick = -1;
  }
