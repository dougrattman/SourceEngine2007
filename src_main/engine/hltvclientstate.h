...
class CHLTVClientState : public CBaseClientState {
 ...
 public:
  ...
  CClientFrame *m_pNewClientFrame;  // not NULL if we just got a packet with a
                                    // new entity frame
  CClientFrame *m_pCurrentClientFrame;  // NULL or pointer to last entity frame
  bool m_bSaveMemory;  // compress data as much as possible to keep whole demos
                       // in memory
