...
//	bUseMapListFile: true if you want to use a maplist file, vs parsing the
//maps directory or using +map 	pMapFile: If you're using a maplist file, this
//should be the filename of it 	pSystemMsg: Used to preface and debug messages
//	iCurrentMap: The map in the list to begin at. Handles the -startmap
//parameter for you.
...
class CMapReslistGenerator {
 public:
  ...
  // call every frame if we're enabled, just so that the next map can be
 ...
 private:
  static void TrackDeletionsLoggingFunc(const char *fullPathFileName,
                                        const char *options);
  static void FileSystemLoggingFunc(const char *fullPathFileName,
                                    const char *options);
