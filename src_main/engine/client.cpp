...
static ConVar cl_timeout("cl_timeout", "30", FCVAR_ARCHIVE,
                         ...
                         "from the server, the client will disconnect itself");
static ConVar cl_logofile(
    "cl_logofile", "materials/decals/spraylogo.vtf", FCVAR_ARCHIVE,
    "Spraypoint logo decal.");  // TODO must be more generic
static ConVar cl_soundfile("cl_soundfile", "sound/player/jingle.wav",
                           FCVAR_ARCHIVE, "Jingle sound file.");
static ConVar cl_forcepreload("cl_forcepreload", "0", FCVAR_ARCHIVE,
                              "Whether we should force preloading.");
static ConVar cl_allowdownload("cl_allowdownload", "1", FCVAR_ARCHIVE,
                               "Client downloads customization files");
static ConVar cl_downloadfilter("cl_downloadfilter", "all", FCVAR_ARCHIVE,
                                "Determines which files can be downloaded from "
                                "the server (all, none, nosounds)");
...
#if !defined(NO_STEAM)
  info.m_nFriendsID =
      SteamUser() ? SteamUser()->GetSteamID().GetAccountID() : 0;
#else
  info.m_nFriendsID = 0;
#endif
...
bool CClientState::SetSignonState(int state, int count) {
  ...
  switch (m_nSignonState) {
    case SIGNONSTATE_CHALLENGE:
      m_bMarkedCRCsUnverified =
          false;  // Remember that we just connected to a new server so it'll
    ...
    case SIGNONSTATE_PRESPAWN:
      m_nSoundSequence =
          1;  // reset sound sequence number after receiving signon sounds
      break;
...
bool CClientState::IsPaused() const {
  return m_bPaused || (g_LostVideoMemory && Host_IsSinglePlayerGame()) ||
         !host_initialized || demoplayer->IsPlaybackPaused() ||
...
float CClientState::GetClientInterpAmount() {
  ...
  const ConVar_ServerBounded *pBounded =
      dynamic_cast<const ConVar_ServerBounded *>(s_cl_interp_ratio);
  if (pBounded) flInterpRatio = pBounded->GetFloat();
  //#define FIXME_INTERP_RATIO
  return std::max(flInterpRatio / cl_updaterate->GetFloat(), flInterp);
}
...
model_t *CClientState::GetModel(int index) {
  ...
  if (!m) {
    const CPrecacheUserData *data =
        CL_GetPrecacheUserData(m_pModelPrecacheTable, index);
    if (data && (data->flags & RES_FATALIFMISSING)) {
      COM_ExplainDisconnection(
          true, "Cannot continue without model %s, disconnecting\n", name);
...
void CClientState::SetModel(int tableIndex) {
  ...
  const CPrecacheUserData *data =
      CL_GetPrecacheUserData(m_pModelPrecacheTable, tableIndex);

  bool bLoadNow = (data && (data->flags & RES_PRELOAD)) || IsX360();
  if (CommandLine()->FindParm("-nopreload") ||
      CommandLine()->FindParm("-nopreloadmodels")) {
    bLoadNow = false;
...
CSfxTable *CClientState::GetSound(int index) {
  if (index <= 0 || !m_pSoundPrecacheTable) return NULL;
...
void CClientState::SetSound(int tableIndex) {
  ...
  const CPrecacheUserData *data =
      CL_GetPrecacheUserData(m_pSoundPrecacheTable, tableIndex);

  bool bLoadNow = (data && (data->flags & RES_PRELOAD)) || IsX360();
  if (CommandLine()->FindParm("-nopreload") ||
      CommandLine()->FindParm("-nopreloadsounds")) {
    bLoadNow = false;
  } else if (cl_forcepreload.GetInt() || CommandLine()->FindParm("-preload")) {
    bLoadNow = true;
  }
...
void CClientState::SetDecal(int tableIndex) {
  if (!m_pDecalPrecacheTable) return;

  if (tableIndex < 0 || tableIndex >= m_pDecalPrecacheTable->GetNumStrings()) {
    return;
  }
...
void CClientState::CheckOthersCustomFile(CRC32_t crcValue) {
  if (crcValue == 0) return;  // not a valid custom file
...
void CClientState::AddCustomFile(int slot, const char *resourceFile) {
  ...
  // Copy it into materials/downloads if it's not there yet, so the server
  // doesn't have to transmit the file back to us.
  bool bCopy = true;
  CCustomFilename filehex(crcValue);
  if (g_pFileSystem->FileExists(filehex.m_Filename)) {
...
void CClientState::CheckOwnCustomFiles() {
  ...
  if (m_nMaxClients == 1) return;  // not in singleplayer
...
void CClientState::DumpPrecacheStats(const char *name) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (!string_name || !slot || !p) continue;

    ConMsg("%03i:  %s (%s):   ", i, string_name, GetFlagString(p->flags));

    if (slot->GetReferenceCount() == 0) {
      ConMsg(" never used\n");
    } else {
      ConMsg(" %i refs, first %.2f mru %.2f\n", slot->GetReferenceCount(),
             slot->GetFirstReference(), slot->GetMostRecentReference());
    }
  }
...
void CClientState::ReadEnterPVS(CEntityReadInfo &u) {
  ...
  int iSerialNum =
      u.m_pBuf->ReadUBitLong(NUM_NETWORKED_EHANDLE_SERIAL_NUMBER_BITS);
...
void CClientState::ReadPreserveEnt(CEntityReadInfo &u) {
  ...
  // Zero overhead
  if (cl_entityreport.GetBool()) CL_RecordEntityBits(u.m_nOldEntity, 0);
...
void CClientState::StartUpdatingSteamResources() {
  ...
  m_hWaitForResourcesHandle =
      g_pFileSystem->WaitForResources(m_szLevelNameShort);
...
void CClientState::CheckUpdatingSteamResources() {
  ...
  if (m_bUpdateSteamResources) {
    ...
    g_pFileSystem->GetWaitForResourcesProgress(m_hWaitForResourcesHandle,
                                               &flProgress, &bComplete);
    ...
    if (bComplete) {
        ...
        if (allowDownloads) {
          ...
          for (int i = 0; i < m_pDownloadableFileTable->GetNumStrings(); ++i) {
            ...
            if (!allowSoundDownloads) {
              ...
              if (!Q_strcasecmp(extension, "wav") ||
                  !Q_strcasecmp(extension, "mp3")) {
                continue;
              }
            }
      ...
      EngineVGui()->UpdateCustomProgressBar(
          flProgress, g_pVGuiLocalize->Find("#Valve_UpdatingSteamResources"));
    }
  }
...
void CClientState::CheckFileCRCsWithServer() {
  VPROF_("CheckFileCRCsWithServer", 1, VPROF_BUDGETGROUP_OTHER_NETWORKING,
         false, BUDGETFLAG_CLIENT);
  ...
  if ((flCurTime - m_flLastCRCBatchTime) < flBatchInterval) return;
  ...
  for (int i = 0; i < count; i++) {
    ...
    V_strncpy(crcCheck.m_szPathID, crcFiles[i].m_PathID,
              sizeof(crcCheck.m_szPathID));
    V_strncpy(crcCheck.m_szFilename, crcFiles[i].m_Filename,
              sizeof(crcCheck.m_szFilename));
...
bool CheckSimpleMaterial(IMaterial *pMaterial) {
  ...
  if (pMaterial->GetMaterialVarFlag(MATERIAL_VAR_HALFLAMBERT)) return false;

  return true;
}

//-----------------------------------------------------------------------------
// Purpose: find a filename in the string table, ignoring case and slash
...
//-----------------------------------------------------------------------------
// Purpose: find a filename in the string table, ignoring case and slash
// mismatches. Returns the consistency type, with CONSISTENCY_NONE being a Not
// Found result.
//-----------------------------------------------------------------------------
...
void CClientState::FinishSignonState_New() {
  ...
  if (!m_bMarkedCRCsUnverified) {
    // Mark all file CRCs unverified once per server. We may have verified CRCs
    // for certain files on the previous server, but we need to reverify them on
    // the new server.
    m_bMarkedCRCsUnverified = true;
    g_pFileSystem->MarkAllCRCsUnverified();
  }

  // Check for a new whitelist. It's good to do it early in the connection
  // process here because if we wait until later, the client may have loaded
  // some files w/o the proper whitelist restrictions and we'd have to reload
  // them.
  m_bCheckCRCsWithServer =
      false;  // Don't check CRCs yet.. wait until we got a whitelist and
              // cleaned out our files based on it to send CRCs.
  CL_CheckForPureServerWhitelist();

  // Verify the map and player .mdl crc's now that we've finished downloading
  // missing resources (maps etc)
  if (!CL_CheckCRCs(m_szLevelName)) {
    Host_Error("Unabled to verify map %s\n",
               (m_szLevelName[0]) ? m_szLevelName : "unknown");
    return;
  }
  ...
  // Include server count in case server disconnects and changes level during
  // d/l
...
void CClientState::ConsistencyCheck(bool bChanged) {
  ...
  for (int i = 0; i < m_pDownloadableFileTable->GetNumStrings(); ++i) {
    ...
    // CRC Check
    //
    if (userData && userData[0] == CONSISTENCY_EXACT &&
        length == sizeof(ExactFileUserData)) {
    ...
    // Bounds Check
    //
    // This is simply asking for the model's mins and maxs.  Also, it checks
    // each material referenced by the model, to make sure it doesn't ignore Z,
    // isn't overbright, etc.
    //
    // TODO: Animations and facial expressions can still pull verts out past
    // this.
    //
    else if (userData && userData[0] == CONSISTENCY_BOUNDS &&
             length == sizeof(ModelBoundsUserData)) {
      ModelBoundsUserData *boundsData = (ModelBoundsUserData *)userData;
      model_t *pModel = modelloader->GetModelForName(
          filename, IModelLoader::FMODELLOADER_CLIENT);
      if (!pModel) {
        ...
        if (pModel->mins.x < boundsData->mins.x ||
          ...
          ConColorMsg(
              ...
              filename, pModel->mins.x, pModel->mins.y, pModel->mins.z,
        ...
        if (pModel->maxs.x > boundsData->maxs.x ||
          ...
          ConColorMsg(
              ...
              filename, pModel->maxs.x, pModel->maxs.y, pModel->maxs.z,
  ...
  if (errorFilename && *errorFilename) {
    COM_ExplainDisconnection(
        true, "Server is enforcing consistency for this file:\n%s\n",
        errorFilename);
...
void CClientState::UpdateAreaBits_BackwardsCompatible() {
  if (m_pAreaBits) {
    ...
    // The whole point of adding this array was that the client could react to
    // closed portals. If they're using the old interface to set area portal
    // bits, then we use the old behavior of assuming all portals are open on
    // the clent.
