...
//-----------------------------------------------------------------------------
// Purpose: A menu button that knows how to parse cvar/command menu data from
// gamedir\scripts\debugmenu.txt
//-----------------------------------------------------------------------------
...
CSmoothingTypeButton::CSmoothingTypeButton(Panel *parent, const char *panelName,
  ...
  m_pMenu->AddMenuItem("Smooth Selection Angles", "smoothselectionangles",
                       parent);
  m_pMenu->AddMenuItem("Smooth Selection Origin", "smoothselectionorigin",
                       parent);
  m_pMenu->AddMenuItem("Linear Interp Angles", "smoothlinearinterpolateangles",
                       parent);
  m_pMenu->AddMenuItem("Linear Interp Origin", "smoothlinearinterpolateorigin",
                       parent);
  ...
  m_pMenu->AddMenuItem("Two Point Origin Ease In/Out", "origineaseboth",
                       parent);
...
//-----------------------------------------------------------------------------
CDemoSmootherPanel::CDemoSmootherPanel(vgui::Panel *parent)
    : Frame(parent, "DemoSmootherPanel") {
  int w = 440;
  int h = 300;
  ...
  m_pPreviewOriginal =
      new vgui::Button(this, "DemoSmoothPreviewOriginal", "Show Original");
  m_pPreviewProcessed =
      new vgui::Button(this, "DemoSmoothPreviewProcessed", "Show Processed");

  m_pBackOff = new vgui::CheckButton(this, "DemoSmoothBackoff", "Back off");
  m_pHideLegend =
      new vgui::CheckButton(this, "DemoSmoothHideLegend", "Hide legend");

  m_pHideOriginal =
      new vgui::CheckButton(this, "DemoSmoothHideOriginal", "Hide original");
  m_pHideProcessed =
      new vgui::CheckButton(this, "DemoSmoothHideProcessed", "Hide processed");

  m_pSelectionInfo = new vgui::Label(this, "DemoSmoothSelectionInfo", "");
  m_pShowAllSamples =
      new vgui::CheckButton(this, "DemoSmoothShowAll", "Show All");
  ...
  m_pRevertPoint =
      new vgui::Button(this, "DemoSmoothRevertPoint", "Revert Pt.");
  m_pToggleKeyFrame =
      new vgui::Button(this, "DemoSmoothSetKeyFrame", "Mark Keyframe");
  m_pToggleLookTarget =
      new vgui::Button(this, "DemoSmoothSetLookTarget", "Mark Look Target");
  ...
  m_pMoveCameraToPoint =
      new vgui::Button(this, "DemoSmoothCameraAtPoint", "Set View");
...
void CDemoSmootherPanel::OnTick() {
  ...
  if (p) {
    ...
    m_pToggleLookTarget->SetText(p->targetpoint ? "Delete Target"
                                                : "Make Target");
...
bool CDemoSmootherPanel::CanEdit() {
  if (!m_Smoothing.active) return false;
...
void DrawVecForward(bool active, const Vector &origin, const QAngle &angles,
                    int r, int g, int b) {
  Vector fwd;
...
void GetColorForSample(bool original, bool samplepoint, bool targetpoint,
                       demosmoothing_t *sample, int &r, int &g, int &b) {
...
//			a -
//-----------------------------------------------------------------------------
void Draw_Box(const Vector &origin, const Vector &mins, const Vector &maxs,
              const QAngle &angles, int r, int g, int b, int a) {
...
//			*next -
//-----------------------------------------------------------------------------
void CDemoSmootherPanel::DrawSmoothingSample(bool original, bool processed,
                                             int samplenumber,
  ...
  if (original) {
    RenderLine(sample->info.viewOrigin + m_vecEyeOffset,
               next->info.viewOrigin + m_vecEyeOffset,
               Color(180, 180, 180, 255), true);

    GetColorForSample(true, false, false, sample, r, g, b);

    DrawVecForward(false, sample->info.viewOrigin + m_vecEyeOffset,
                   sample->info.viewAngles, r, g, b);
  }

  if (processed && sample->info.flags != 0) {
    RenderLine(sample->info.GetViewOrigin() + m_vecEyeOffset,
               next->info.GetViewOrigin() + m_vecEyeOffset,
               Color(255, 255, 180, 255), true);

    GetColorForSample(false, false, false, sample, r, g, b);

    DrawVecForward(false, sample->info.GetViewOrigin() + m_vecEyeOffset,
                   sample->info.GetViewAngles(), r, g, b);
  }
  if (sample->samplepoint) {
    GetColorForSample(false, true, false, sample, r, g, b);
    RenderBox(sample->vecmoved + m_vecEyeOffset, sample->angmoved,
              Vector(-2, -2, -2), Vector(2, 2, 2), Color(r, g, b, 127), false);
    DrawVecForward(false, sample->vecmoved + m_vecEyeOffset, sample->angmoved,
                   r, g, b);
  }

  if (sample->targetpoint) {
    GetColorForSample(false, false, true, sample, r, g, b);
    RenderBox(sample->vectarget, vec3_angle, Vector(-2, -2, -2),
              Vector(2, 2, 2), Color(r, g, b, 127), false);
  }
  ...
  if (samplenumber == m_nPreviewLastFrame + 1) {
    ...
    g = 100;
    b = 250;
    RenderBox(sample->info.GetViewOrigin() + m_vecEyeOffset,
              sample->info.GetViewAngles(), Vector(-2, -2, -2), Vector(2, 2, 2),
              Color(r, g, b, 92), false);
  }
  ...
  if (sample->targetpoint) {
    ...
    RenderLine(sample->info.GetViewOrigin() + m_vecEyeOffset, sample->vectarget,
               Color(r, g, b, 255), true);
  }
}
...
void CDemoSmootherPanel::DrawDebuggingInfo(int frame, float elapsed) {
  ...
  if (useframe < c) {
    ...
    Draw_Box(org + m_vecEyeOffset, Vector(-1, -1, -1), Vector(1, 1, 1), ang,
             100, 220, 250, 127);
  }
...
int CDemoSmootherPanel::GetFrameForTick(int tick) {
  ...
  while (true) {
    ...
    if (tick == middleTick) return middle;
...
//			frac -
//-----------------------------------------------------------------------------
static void InterpolateAngles(const QAngle &start, const QAngle &end,
                              QAngle &output, float frac) {
  Quaternion src, dest;
...
bool CDemoSmootherPanel::GetInterpolatedOriginAndAngles(bool readonly,
                                                        Vector &origin,
                                                        QAngle &angles) {
  origin.Init();
  angles.Init();
  ...
  do {
    ...
    if (TICKS_TO_TIME(endsample->frametick) >= time) {
      ...
      if (m_bPreviewOriginal) {
        VectorLerp(startsample->info.viewOrigin, endsample->info.viewOrigin,
                   frac, renderOrigin);
        InterpolateAngles(startsample->info.viewAngles,
                          endsample->info.viewAngles, renderAngles, frac);
      } else {
        VectorLerp(startsample->info.GetViewOrigin(),
                   endsample->info.GetViewOrigin(), frac, renderOrigin);
        InterpolateAngles(startsample->info.GetViewAngles(),
                          endsample->info.GetViewAngles(), renderAngles, frac);
      }
...
// Input  : t -
//-----------------------------------------------------------------------------
bool CDemoSmootherPanel::GetInterpolatedViewPoint(Vector &origin,
                                                  QAngle &angles) {
  Assert(m_bPreviewing);
...
void CDemoSmootherPanel::OnStep(bool forward) {
  ...
  SetLastFrame(false, clamp(m_nPreviewLastFrame, max(m_nSelection[0] - 10, 0),
                            min(m_nSelection[1] + 10, c - 1)));
...
void CDemoSmootherPanel::DrawLegend(int startframe, int endframe) {
  ...
  for (i = startframe; i <= endframe; i++) {
    ...
    CDebugOverlay::AddTextOverlay(sample->info.GetViewOrigin() +
                                      m_vecEyeOffset +
                                      fwd * (back_off ? 5.0f : 50.0f),
                                  0, -1.0f, sz);
  }
}
...
void CDemoSmootherPanel::PerformLinearInterpolatedAngleSmoothing(int startframe,
                                                                 int endframe) {
...
void CDemoSmootherPanel::AddSamplePoints(bool usetarget, bool includeboundaries,
...
demosmoothing_t *CDemoSmootherPanel::GetBoundedSample(
    CUtlVector<demosmoothing_t *> &points, int sample) {
  int c = points.Count();
  if (sample < 0)
    return points[0];
  else if (sample >= c)
...
//			next -
//-----------------------------------------------------------------------------
void CDemoSmootherPanel::FindSpanningPoints(
    int tick, CUtlVector<demosmoothing_t *> &points, int &prev, int &next) {
  prev = -1;
  next = 0;
  int c = points.Count();
  int i;
...
void CDemoSmootherPanel::OnSplineSampleOrigin(void) {
  ...
  for (int i = m_nSelection[0]; i <= m_nSelection[1]; i++) {
    ...
    Catmull_Rom_Spline_Normalize(earliest->vecmoved, current->vecmoved,
                                 next->vecmoved, latest->vecmoved, frac,
                                 splined);
...
void CDemoSmootherPanel::OnLookAtPoints(bool spline) {
  ...
  for (i = m_nSelection[0]; i <= m_nSelection[1]; i++) {
    ...
    if (spline) {
      Catmull_Rom_Spline_Normalize(earliest->vectarget, current->vectarget,
                                   next->vectarget, latest->vectarget, frac,
                                   splined);
    } else {
...
void CDemoSmootherPanel::OnSmoothEdges(bool left, bool right) {
  ...
  if (left && m_nSelection[0] > 0) {
    PerformLinearInterpolatedAngleSmoothing(m_nSelection[0] - 1,
                                            m_nSelection[0] + frames);
  }
  if (right && m_nSelection[1] < c - 1) {
    PerformLinearInterpolatedAngleSmoothing(m_nSelection[1] - frames,
                                            m_nSelection[1] + 1);
  }
...
void CDemoSmootherPanel::ParseSmoothingInfo(CDemoFile &demoFile,
  ...
  while (!demofinished) {
    ...
    if (!foundFirstSelectable && smoothing_entry.vecmoved.LengthSqr() > 0.0f) {
