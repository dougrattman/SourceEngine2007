...
// -----------------------------------------------------------------------------
// //
static CUtlLinkedList<CDispShadowDecal, DispShadowHandle_t, true>
    s_DispShadowDecals;
static CUtlLinkedList<CDispShadowFragment, DispShadowFragmentHandle_t, true>
    s_DispShadowFragments;
static CUtlLinkedList<CDispDecal, DispShadowHandle_t, true> s_DispDecals;
static CUtlLinkedList<CDispDecalFragment, DispShadowFragmentHandle_t, true>
    s_DispDecalFragments;

void CDispInfo::GetIntersectingSurfaces(
    GetIntersectingSurfaces_Struct *pStruct) {
  if (!m_Verts.Count() || !m_nIndices) return;
  ...
  for (int iVert = 0; iVert < m_MeshReader.NumIndices(); iVert += 3) {
    ...
    if (pStruct->m_nSetInfos >= pStruct->m_nMaxInfos) break;

    Vector const &a =
        m_MeshReader.Position(m_MeshReader.Index(iVert + 0) - m_iVertOffset);
    Vector const &b =
        m_MeshReader.Position(m_MeshReader.Index(iVert + 1) - m_iVertOffset);
    Vector const &c =
        m_MeshReader.Position(m_MeshReader.Index(iVert + 2) - m_iVertOffset);
...
unsigned int CDispInfo::ComputeDynamicLightMask(dlight_t *pLights) {
  ...
  for (int lnum = 0, testBit = 1, mask = r_dlightactive;
...
DispDecalHandle_t CDispInfo::NotifyAddDecal(decal_t *pDecal, float flSize) {
  ...
  if (h != s_DispDecals.InvalidIndex()) {
    ...
    R_SetupDecalClip(
        pOutVerts, pDispDecal->m_pDecal, MSurf_Plane(m_ParentSurfID).normal,
        pDispDecal->m_pDecal->material, pDispDecal->m_TextureSpaceBasis,
...
// Allocates fragments...
//-----------------------------------------------------------------------------
CDispShadowFragment *CDispInfo::AllocateShadowDecalFragment(
    DispShadowHandle_t h, int nCount) {
...
void CDispInfo::GenerateDecalFragments_R(CVertIndex const &nodeIndex,
  ...
  if ((iLevel + 1 < m_Power) &&
    ...
    for (int iChild = 0; iChild < 4; iChild++) {
      CVertIndex const &childNode =
          m_pPowerInfo->m_pChildVerts[iNodeIndex].m_Verts[iChild];

      bool bActiveChild = m_ActiveVerts.Get(VertIndex(childNode)) != 0;
      if (bActiveChild)
        GenerateDecalFragments_R(childNode, iChildNodeBit, decalHandle,
                                 pDispDecal, iLevel + 1);
  ...
  for (int i = 0; i < nodeInfo.m_Count; i += 3) {
    if (isShadow)
      TestAddDecalTri(index + i, decalHandle,
                      static_cast<CDispShadowDecal *>(pDispDecal));
    else
      TestAddDecalTri(index + i, decalHandle,
                      static_cast<CDispDecal *>(pDispDecal));
  }
}
...
// -----------------------------------------------------------------------------
// //
bool CDispInfo::ComputeShadowFragments(DispShadowHandle_t h, int &vertexCount,
                                       int &indexCount) {
  CDispShadowDecal *pShadowDecal = &s_DispShadowDecals[h];
  ...
  // this displacement should be taken out of the list of potential
  // displacements
  if (pShadowDecal->m_Flags & CDispDecalBase::NO_INTERSECTION) return false;
...
void DispInfo_BuildPrimLists(int nSortGroup, SurfaceHandle_t *pList,
                             int listCount, bool bDepthOnly,
  ...
  for (int i = 0; i < listCount; i++) {
    ...
    if (!pDisp->Render(pDisp->m_pMesh, bDebugConvars)) continue;
    ...
    if (bDepthOnly) continue;

#ifndef SWDS
    OverlayMgr()->AddFragmentListToRenderList(
        nSortGroup, MSurf_OverlayFragmentList(pList[i]), true);
#endif
  }
}
...
void DispInfo_DrawPrimLists(bool bShadowDepth) {
  ...
  for (int iGroup = 0; iGroup < nDispGroupsSize; iGroup++) {
    ...
    if (pGroup->m_nVisible == 0) continue;
    ...
    if (bShadowDepth) {
      ...
      if (nAlphaTest == 1) {
        ...
        IMaterialVar *pOriginalTextureVar = pGroup->m_pMaterial->FindVarFast(
        ...
        IMaterialVar *pOriginalTextureFrameVar =
            pGroup->m_pMaterial->FindVarFast("$frame",
                                             &originalTextureFrameVarCache);
        static unsigned int originalAlphaRefCache = 0;
        IMaterialVar *pOriginalAlphaRefVar = pGroup->m_pMaterial->FindVarFast(
    ...
    if (nFullbright != 1 && !bShadowDepth) {
      ...
      if (pGroup->m_pMaterial->GetPropertyFlag(
    ...
    for (int iMesh = 0; iMesh < nMeshesSize; iMesh++) {
      ...
      if (pMesh->m_nVisible == 0) continue;
...
void DispInfo_BatchDecals(CDispInfo **pVisibleDisps, int nVisibleDisps) {
  ...
  for (int iDisp = 0; iDisp < nVisibleDisps; ++iDisp) {
    ...
    if (pDisp->m_FirstDecal == DISP_DECAL_HANDLE_INVALID) continue;
    ...
    while (hDecal != DISP_DECAL_HANDLE_INVALID) {
      ...
      if ((decal.m_Flags & CDispDecalBase::FRAGMENTS_COMPUTED) == 0) {
        pDisp->GenerateDecalFragments(pDisp->m_pPowerInfo->m_RootNode, 0,
                                      hDecal, &decal);
      }
...
void DispInfo_DrawDecalsGroup(int iGroup, int iTreeType) {
  ...
  for (int iSortTree = 0; iSortTree < nSortTreeCount; ++iSortTree) {
    ...
    const CUtlVector<DecalMaterialBucket_t> &materialBucketList =
        g_aDispDecalSortTrees[iSortTree].m_aDecalSortBuckets[iGroup][iTreeType];
    int nBucketCount = materialBucketList.Count();
    for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
      if (materialBucketList.Element(iBucket).m_nCheckCount !=
          g_nDispDecalSortCheckCount)
        continue;

      int iHead = materialBucketList.Element(iBucket).m_iHead;
      if (!g_aDispDecalSortPool.IsValidIndex(iHead)) continue;
      ...
      while (iElement != g_aDispDecalSortPool.InvalidIndex()) {
        ...
        while (hFrag != DISP_DECAL_FRAGMENT_HANDLE_INVALID) {
          ...
          // Setup verts.
          float flOffset =
              ComputeDecalLightmapOffset(fragment.m_pDecal->surfID);
          for (int iVert = 0; iVert < fragment.m_nVerts; ++iVert) {
            ...
            if (bIsMaterialInPage) {
              meshBuilder.TexCoordSubRect2f(
                  0, vert.m_ctCoords.x, vert.m_ctCoords.y, matOffset[0],
                  matOffset[1], matScale[0], matScale[1]);
            } else {
...
void DispInfo_DrawDecals_Old(CDispInfo *visibleDisps[MAX_MAP_DISPINFO],
                             int nVisibleDisps) {
#ifndef SWDS
  //	VPROF("DispInfo_DrawDecals");
  if (!nVisibleDisps) return;
  ...
  for (int i = 0; i < nVisibleDisps; i++) {
    ...
    pRenderContext->BindLightmapPage(
        pDisp->m_pMesh->m_pGroup->m_LightmapPageID);
    ...
    while (d != DISP_DECAL_HANDLE_INVALID) {
      ...
      if ((decal.m_Flags & CDispDecalBase::FRAGMENTS_COMPUTED) == 0) {
        pDisp->GenerateDecalFragments(pDisp->m_pPowerInfo->m_RootNode, 0, d,
                                      &decal);
      }
      ...
      IMesh *pMesh =
          pRenderContext->GetDynamicMesh(true, NULL, NULL, pMaterial);
      ...
      CMeshBuilder meshBuilder;
      meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, decal.m_nVerts,
                        decal.m_nTris * 3);
      ...
      while (f != DISP_DECAL_FRAGMENT_HANDLE_INVALID) {
        ...
        for (v = 0; v < fragment.m_nVerts - 2; ++v) {
          ...
          if (pMaterial->InMaterialPage()) {
            meshBuilder.TexCoordSubRect2f(0, fragment.m_pVerts[v].m_ctCoords.x,
                                          fragment.m_pVerts[v].m_ctCoords.y,
                                          matOffset[0], matOffset[1],
                                          matScale[0], matScale[1]);
          } else {
            meshBuilder.TexCoord2f(0, fragment.m_pVerts[v].m_ctCoords.x,
                                   fragment.m_pVerts[v].m_ctCoords.y);
          }
          meshBuilder.TexCoord2f(1, fragment.m_pVerts[v].m_cLMCoords.x,
                                 fragment.m_pVerts[v].m_cLMCoords.y);
          meshBuilder.AdvanceVertex();
        ...
        if (pMaterial->InMaterialPage()) {
          meshBuilder.TexCoordSubRect2f(0, fragment.m_pVerts[v].m_ctCoords.x,
                                        fragment.m_pVerts[v].m_ctCoords.y,
                                        matOffset[0], matOffset[1], matScale[0],
                                        matScale[1]);
        } else {
          meshBuilder.TexCoord2f(0, fragment.m_pVerts[v].m_ctCoords.x,
                                 fragment.m_pVerts[v].m_ctCoords.y);
...
int DispInfo_AddShadowsToMeshBuilder(CMeshBuilder &meshBuilder,
  ...
  while (f != DISP_SHADOW_FRAGMENT_HANDLE_INVALID) {
    ...
    for (i = 0; i < fragment.m_nVerts - 2; ++i, ++pShadowVert) {
      ...
      Vector2DMultiply(pShadowVert->m_ShadowSpaceTexCoord.AsVector2D(),
                       info.m_vTexSize, texCoord);
      texCoord += info.m_vTexOrigin;
      c = g_pShadowMgr->ComputeDarkness(pShadowVert->m_ShadowSpaceTexCoord.z,
                                        info);
...
HDISPINFOARRAY DispInfo_CreateArray(int nElements) {
  ...
  for (int i = 0; i < nElements; i++) pRet->m_pDispInfos[i].m_pDispArray = pRet;
...
static void DispInfo_DrawDebugInformation(SurfaceHandle_t *pList,
                                          int listCount) {
#ifndef SWDS
...
//-----------------------------------------------------------------------------
void DispInfo_RenderList(int nSortGroup, SurfaceHandle_t *pList, int listCount,
                         bool bOrtho, unsigned long flags, bool bShadowDepth) {
#ifndef SWDS
  if (!r_DrawDisp.GetInt() || !listCount) return;
  ...
  DispInfo_BuildPrimLists(nSortGroup, pList, listCount, bShadowDepth,
                          visibleDisps, nVisibleDisps);
  ...
  g_pShadowMgr->DrawFlashlightDecalsOnDisplacements(
      nSortGroup, visibleDisps, nVisibleDisps, bFlashlightMask);
