...
class CServerRemoteAccess : public IGameServerData {
 public:
  ...
  virtual void WriteDataRequest(ra_listener_id listener, const void *buffer,
                                int bufferSize);
  void WriteDataRequest(CRConServer *pNetworkListener, ra_listener_id listener,
                        const void *buffer, int bufferSize);

  // gets return value from the server
  // returns the number of bytes read
  virtual int ReadDataResponse(ra_listener_id listener, void *buffer,
                               int bufferSize);
  ...
  void SendVProfData(ra_listener_id listenerID, bool bGroupData, void *data,
                     int len);

  virtual ra_listener_id GetNextListenerID(bool authConnection,
                                           const netadr_t *adr = NULL);
  virtual void RegisterAdminUIID(ra_listener_id listener) {
    m_AdminUIID = listener;
  }

  ra_listener_id GetAdminUIID() { return m_AdminUIID; }
  void GetStatsString(char *buf,
                      int bufSize);  // also used by the 'stats' command

  void UploadScreenshot(const char *pFileName);

 private:
  void RespondString(ra_listener_id listener, int requestID,
                     const char *pString);
  void RequestValue(ra_listener_id listener, int requestID,
                    const char *variable);
  ...
  void CheckPassword(CRConServer *pNetworkListener, ra_listener_id listener,
                     int requestID, const char *password);
  ...
  void SendResponseToClient(ra_listener_id listenerID,
                            ServerDataResponseType_t type, void *pData,
                            int nDataLen);
