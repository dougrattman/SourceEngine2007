...
CDispCollTree *CollisionBSPData_GetCollisionTree(int i) {
  if ((i < 0) || (i >= g_DispCollTreeCount)) return 0;
...
void CollisionBSPData_LinkPhysics(void) {
  ...
  //
  if (!physprop) {
    physprop = (IPhysicsSurfaceProps *)g_AppSystemFactory(
        VPHYSICS_SURFACEPROPS_INTERFACE_VERSION, NULL);
    physcollision = (IPhysicsCollision *)g_AppSystemFactory(
...
void CollisionBSPData_LoadTextures(CCollisionBSPData *pBSPData) {
  ...
  pBSPData->map_texturenames =
      (char *)Hunk_Alloc(lhStringData.LumpSize() * sizeof(char), false);
  ...
  for (i = 0; i < count; i++, in++) {
    ...
    material = materials->FindMaterial(pBSPData->map_surfaces[i].name,
                                       TEXTURE_GROUP_WORLD, true);
    if (!IsErrorMaterial(material)) {
      ...
      if (varFound) {
        ...
        pBSPData->map_surfaces[i].surfaceProps =
            physprop->GetSurfaceIndex(pProps);
      }
    }
  }
}
...
void CollisionBSPData_LoadTexinfo(CCollisionBSPData *pBSPData,
  ...
  count = lh.LumpSize() / sizeof(*in);
  if (count < 1) Sys_Error("Map with no texinfo");
  if (count > MAX_MAP_TEXINFO) Sys_Error("Map has too many surfaces");
  ...
  for (i = 0; i < count; i++, in++) {
    ...
    if (out >= pBSPData->numtextures) out = 0;
...
void CollisionBSPData_LoadLeafs_Version_0(CCollisionBSPData *pBSPData,
  ...
  memset(&pBSPData->map_leafs[pBSPData->emptyleaf], 0,
         sizeof(pBSPData->map_leafs[pBSPData->emptyleaf]));
  pBSPData->numleafs++;
}
...
inline bool IsBoxBrush(const cbrush_t &brush, dbrushside_t *pSides,
                       cplane_t *pPlanes) {
  int countAxial = 0;
  if (brush.numsides == 6) {
    for (int i = 0; i < brush.numsides; i++) {
      cplane_t *plane = pPlanes + pSides[brush.firstbrushside + i].planenum;
      if (plane->type > PLANE_Z) break;
      countAxial++;
    }
  }
...
void CollisionBSPData_LoadBrushSides(CCollisionBSPData *pBSPData,
  ...
  // Brushes are compressed on load to remove any AABB brushes.  The brushsides
  // for those are removed and those brushes are stored as cboxbrush_t.  But the
  // texinfo/surface data needs to be copied So the algorithm is:
  //
  // count box brushes
  ...
  pBSPData->map_brushsides.Attach(brushSideCount,
                                  (cbrushside_t *)Hunk_Alloc(nSize, false));
  pBSPData->map_boxbrushes.Attach(
      boxBrushCount,
      (cboxbrush_t *)Hunk_Alloc(boxBrushCount * sizeof(cboxbrush_t), false));
  ...
  for (i = 0; i < pBSPData->numbrushes; i++) {
    ...
    if (pBrush->IsBox()) {
      ...
      ExtractBoxBrush(pBox, *pBrush, in, pBSPData->map_planes.Base(),
                      map_texinfo);
  ...
  Assert(outBrushSide == pBSPData->numbrushsides &&
         outBoxBrush == pBSPData->numboxbrushes);
}
...
void CollisionBSPData_LoadSubmodels(CCollisionBSPData *pBSPData) {
  ...
  if (count < 1) Sys_Error("Map with no models");
  if (count > MAX_MAP_MODELS) Sys_Error("Map has too many models");
...
void CollisionBSPData_LoadNodes(CCollisionBSPData *pBSPData) {
  ...
  if (count < 1) Sys_Error("Map has no nodes");
  if (count > MAX_MAP_NODES) Sys_Error("Map has too many nodes");
...
void CollisionBSPData_LoadAreaPortals(CCollisionBSPData *pBSPData) {
  ...
  pBSPData->portalopen.Attach(
      count, (bool *)Hunk_Alloc(pBSPData->numportalopen * sizeof(bool), false));
...
void CollisionBSPData_LoadEntityString(CCollisionBSPData *pBSPData) {
  ...
  MEM_ALLOC_CREDIT();
  pBSPData->map_entitystring.Init(lh.GetDiskName(), lh.LumpOffset(),
                                  lh.LumpSize(),
                                  IsX360() ? lh.LumpBase() : NULL);
}
...
void CollisionBSPData_LoadPhysics(CCollisionBSPData *pBSPData) {
  ...
  if (!g_bServerGameDLLGreaterThanV4) {
    ...
    // mopp surfaces in it (if compiled with the current vbsp.exe or a map
    // without any displacements). The legacy server game DLLs will crash when
    // mopps are present but since they also crash with a NULL lump there's
    // nothing lost in that case.
    if (CMapLoadHelper::LumpSize(LUMP_PHYSCOLLIDESURFACE) > 0) {
      nLoadLump = LUMP_PHYSCOLLIDESURFACE;
    } else {
      DevWarning(
          "Legacy game DLL may not support terrain vphysics collisions with "
          "this BSP!\n");
    }
  }
  ...
  do {
    ...
    if (physModel.dataSize > 0) {
      ...
      physcollision->VCollideLoad(&pModel->vcollisionData, physModel.solidCount,
                                  (const char *)ptr,
...
void CollisionBSPData_LoadDispInfo(CCollisionBSPData *pBSPData) {
  ...
  g_pDispBounds = (alignedbbox_t *)Hunk_Alloc(
  ...
  for (i = 0; i < coreDispCount; ++i) {
    ...
    coreDisp.InitDispInfo(dispInfo.power, dispInfo.minTess,
                          dispInfo.smoothingAngle, tempVerts, tempTris);
    ...
    //
    // check for 0 faces, should have been taken care of in vbsp!!!
    //
    ...
    g_pDispBounds[i].Init(pDispTree->m_mins, pDispTree->m_maxs,
                          pDispTree->m_iCounter, pDispTree->GetContents());
    ...
    if (pTex->texdata >= 0) {
      IMaterial *pMaterial =
          materials->FindMaterial(pBSPData->map_surfaces[pTex->texdata].name,
                                  TEXTURE_GROUP_WORLD, true);
