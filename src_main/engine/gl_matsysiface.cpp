...
//
// Purpose: Wrapper for the material system for the engine.
...
#ifdef NEWMESH
CUtlVector<IVertexBuffer *>
    g_WorldStaticMeshes;  // fixme - rename to g_WorldStaticVertexBuffers
#else
...
void TangentSpaceComputeBasis(Vector &tangent, Vector &binormal,
...
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_edit,
              "Bring up the material under the crosshair in the editor",
              FCVAR_CHEAT) {
  if (!toolframework->InToolMode()) return;
...
//-----------------------------------------------------------------------------
// A console command to open the material under the crosshair in the associated
// editor.
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_crosshair_edit,
              "open the material under the crosshair in the editor defined by "
...
// editor.
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_crosshair_explorer,
              "open the material under the crosshair in explorer and highlight "
              "the vmt file",
              FCVAR_CHEAT) {
  IMaterial *pMaterial = GetMaterialAtCrossHair();
  if (!pMaterial) {
...
// editor.
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_crosshair_reloadmaterial,
              "reload the material under the crosshair", FCVAR_CHEAT) {
  IMaterial *pMaterial = GetMaterialAtCrossHair();
  if (!pMaterial) {
    ConMsg("no/bad material\n");
  } else {
...
CON_COMMAND_F(mat_crosshair_printmaterial,
              "print the material under the crosshair", FCVAR_CHEAT) {
  IMaterial *pMaterial = GetMaterialAtCrossHair();
  if (!pMaterial) {
    ConMsg("no/bad material\n");
  } else {
...
static void RegisterLightmappedSurface(SurfaceHandle_t surfID) {
  ...
  MSurf_MaterialSortID(surfID) = materials->AllocateLightmap(
      allocationWidth, allocationHeight, offsetIntoLightmapPage,
...
static void RegisterUnlightmappedSurface(SurfaceHandle_t surfID) {
  MSurf_MaterialSortID(surfID) =
      materials->AllocateWhiteLightmap(MSurf_TexInfo(surfID)->material);
...
static bool LightmapLess(const SurfaceHandle_t &surfID1,
  ...
  if (hasLightmap1 != hasLightmap2) return hasLightmap1 > hasLightmap2;
  ...
  if (enum1 != enum2) return enum1 < enum2;
  ...
  if (hasLightstyle1 != hasLightstyle2) return hasLightstyle1 > hasLightstyle2;

    // Then sort by lightmap area for better packing... (big areas first)
    // NOTE: Don't care about bumpmap increasing area here because it is a
    // linear factor (all surfs with the same material have the same bumpmapping
    // cost)
#if 1
  int area1 =
      MSurf_LightmapExtents(surfID1)[0] * MSurf_LightmapExtents(surfID1)[1];
  int area2 =
      MSurf_LightmapExtents(surfID2)[0] * MSurf_LightmapExtents(surfID2)[1];
  return area2 < area1;
#else
...
void MaterialSystem_RegisterLightmapSurfaces(void) {
  ...
  CUtlRBTree<SurfaceHandle_t, int> surfaces(
      0, host_state.worldbrush->numsurfaces, LightmapLess);
  ...
  for (int i = surfaces.FirstInorder(); i != surfaces.InvalidIndex();
       i = surfaces.NextInorder(i)) {
...
static void TestBumpSanity(SurfaceHandle_t surfID) {
  ...
  // use the last one to check if we need a bumped lightmap, but don't have it
  // so that we can warn.
    ...
    */
    for (j = 0; j < MSurf_NumPrims(surfID); j++) {
      ...
      assert(MSurf_FirstPrimID(surfID) + j <
             host_state.worldbrush->numprimitives);
      ...
      for (k = 0; k < pPrim->vertCount; k++) {
        ...
        for (l = 0; l < pPrim->vertCount; l++) {
          ...
          mprimvert_t &vert =
              host_state.worldbrush->primverts[l + pPrim->firstVert];
          Vector &vec = vert.pos;

          // base texture coordinate
          vert.texCoord[0] =
              DotProduct(vec, MSurf_TexInfo(surfID)
                                  ->textureVecsTexelsPerWorldUnits[0]
                                  .AsVector3D()) +
              MSurf_TexInfo(surfID)->textureVecsTexelsPerWorldUnits[0][3];
          vert.texCoord[0] /=
              MSurf_TexInfo(surfID)->material->GetMappingWidth();
          ...
          vert.texCoord[1] /=
              MSurf_TexInfo(surfID)->material->GetMappingHeight();
          ...
          if ((MSurf_Flags(surfID) & SURFDRAW_NOLIGHT)) {
            ...
            vert.lightCoord[0] += 0.5f;
            vert.lightCoord[0] /= (float)MSurf_LightmapExtents(
                surfID)[0];  // pSurf->texinfo->texture->width;
            ...
            vert.lightCoord[1] += 0.5f;
            vert.lightCoord[1] /= (float)MSurf_LightmapExtents(
                surfID)[1];  // pSurf->texinfo->texture->height;
...
int __cdecl SortMapCompareFunc(const void *pElem0, const void *pElem1) {
  ...
  return pSortMapRenderContext->CompareMaterialCombos(
      pMap0->info.material, pMap1->info.material, pMap0->info.lightmapPageID,
      pMap1->info.lightmapPageID);
}
...
void MaterialSystem_CreateSortinfo(void) {
  ...
  materialSortInfoArray =
      (MaterialSystem_SortInfo_t *)new MaterialSystem_SortInfo_t[nSortIDs];
  Assert(materialSortInfoArray);
  ...
  for (i = 0; i < nSortIDs; i++) {
    ...
    // Msg("Material %s, lightmap %d ",
    // materialSortInfoArray[i].material->GetName(),
    // materialSortInfoArray[i].lightmapPageID );
  }
...
bool SurfHasBumpedLightmaps(SurfaceHandle_t surfID) {
  ...
  if ((MSurf_TexInfo(surfID)->flags & SURF_BUMPLIGHT) &&
      ...
      (host_state.worldbrush->lightdata) && (MSurf_Samples(surfID))) {
    hasBumpmap = true;
  }
  return hasBumpmap;
}
...
bool SurfNeedsBumpedLightmaps(SurfaceHandle_t surfID) {
  ...
  return MSurf_TexInfo(surfID)->material->GetPropertyFlag(
...
bool SurfNeedsLightmap(SurfaceHandle_t surfID) {
  ...
  assert(MSurf_TexInfo(surfID));
  assert(MSurf_TexInfo(surfID)->material);
  if (MSurf_TexInfo(surfID)->flags & SURF_NOLIGHT) return false;
...
void BuildMSurfaceVerts(const worldbrushdata_t *pBrushData,
                        SurfaceHandle_t surfID, Vector *verts,
  ...
  for (int i = 0; i < vertCount; i++) {
    ...
    if (lightCoords) {
      ...
      if (MSurf_Flags(surfID) & SURFDRAW_BUMPLIGHT) {
        // bump maps appear left to right in lightmap page memory, calculate the
        // offset for the width of a single map
        for (int bumpID = 1; bumpID <= NUM_BUMP_VECTS; bumpID++) {
          lightCoords[i][bumpID][0] =
              lightCoords[i][0][0] + (bumpID * ctx.m_BumpSTexCoordOffset);
...
void BuildMSurfacePrimVerts(worldbrushdata_t *pBrushData, mprimitive_t *prim,
  ...
  //	Vector& normal = pBrushData->vertnormals[
  // pBrushData->vertnormalindices[MSurf_FirstVertNormal( surfID )] ];
  ...
  for (int i = 0; i < prim->vertCount; i++) {
    ...
    if (MSurf_Flags(surfID) & SURFDRAW_TANGENTSPACE) {
      ...
      TangentSpaceComputeBasis(tangentS, tangentT, MSurf_Plane(surfID).normal,
                               tVect, false);
...
//-----------------------------------------------------------------------------
// Here's a version of the mesh builder used to allow for client DLL to draw
// brush models
//-----------------------------------------------------------------------------

void BuildBrushModelVertexArray(worldbrushdata_t *pBrushData,
                                SurfaceHandle_t surfID, BrushVertex_t *pVerts) {
  SurfaceCtx_t ctx;
  SurfSetupSurfaceContext(ctx, surfID);
...
void CMSurfaceSortList::Reset() { Init(m_maxSortIDs, m_list.NumAllocated()); }
  ...
  if (newMaxSortIDs > m_maxSortIDs) {
    ...
    for (int i = MAX_MAT_SORT_GROUPS; --i >= 0;) {
      for (int j = newMaxSortIDs; --j >= 0;) {
        ...
        if (j < oldMax) {
          // when i == 0, the group indices overlap so they don't need to be
          // remapped
          if (i != 0) {
          ...
          if (IsGroupUsed(newIndex) && m_groups[newIndex].groupListIndex >= 0) {
            m_sortGroupLists[i][m_groups[newIndex].groupListIndex] =
                &m_groups[newIndex];
          }
        } else {
          MarkGroupNotUsed(newIndex);
...
void CMSurfaceSortList::AddSurfaceToTail(msurface2_t *pSurface, int sortGroup,
                                         int sortID) {
  ...
  if (pList) {
    ...
    if (pGroup->listHead == m_list.InvalidIndex()) {
      // UNDONE: This should really be sorted by sortID would help reduce state
      // changes NOTE: Doesn't seem to help much in benchmarks to sort this
      // vector
...
msurface2_t *CMSurfaceSortList::GetSurfaceAtHead(
    const surfacesortgroup_t &group) const {
  if (group.listHead == m_list.InvalidIndex()) return NULL;
...
IMaterial *GetMaterialAtCrossHair(void) {
  ...
  SurfaceHandle_t hitSurfID =
      R_LightVec(MainViewOrigin(), endPoint, false, lightmapColor);
...
void SaveSurfAtCrossHair() {
  ...
  s_CrossHairSurfID =
      R_LightVec(MainViewOrigin(), endPoint, false, lightmapColor, &textureS,
                 &textureT, &lightmapCoords[0], &lightmapCoords[1]);
#endif
}
