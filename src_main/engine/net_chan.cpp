...
ConVar net_blocksize("net_maxfragments", "1260", 0,
                     "Max fragment bytes per packet", true, FRAGMENT_SIZE, true,
                     MAX_ROUTABLE_PAYLOAD);

static ConVar net_showmsg("net_showmsg", "0", 0,
                          "Show incoming message: <0|1|name>");
static ConVar net_showfragments("net_showfragments", "0", 0,
                                "Show netchannel fragments");
static ConVar net_showpeaks("net_showpeaks", "0", 0,
                            "Show messages for large packets only: <size>");
static ConVar net_blockmsg("net_blockmsg", "none", FCVAR_CHEAT,
                           "Discards incoming message: <0|1|name>");
static ConVar net_showdrop("net_showdrop", "0", 0,
                           "Show dropped packets in console");
static ConVar net_drawslider("net_drawslider", "0", 0,
                             "Draw completion slider during signon");
static ConVar net_chokeloopback("net_chokeloop", "0", 0,
                                "Apply bandwidth choke to loopback packets");
static ConVar net_maxfilesize("net_maxfilesize", "16", 0,
                              "Maximum allowed file size for uploading in MB",
                              true, 0, true, 64);
static ConVar net_compresspackets("net_compresspackets", "1", 0,
                                  "Use lz compression on game packets.");
static ConVar net_compresspackets_minsize(
    "net_compresspackets_minsize", "128", 0,
    "Don't bother compressing packets below this size.");
static ConVar net_maxcleartime("net_maxcleartime", "4.0", 0,
                               "Max # of seconds we can wait for next packets "
                               "to be sent based on rate setting (0 == no "
                               "limit).");
...
// If the network connection hasn't been active in this many seconds, display
// some warning text.
...
// We only need to checksum packets on the PC and only when we're actually
...
static bool IsSafeFileToDownload(const char *pFilename) {
  ...
  if (V_strstr(pFilename, ":") || V_strstr(pFilename, "..")) {
    return false;
  }
  ...
  if (V_stricmp(pExt, ".cfg") == 0 || V_stricmp(pExt, ".lst") == 0 ||
      V_stricmp(pExt, ".exe") == 0 || V_stricmp(pExt, ".vbs") == 0 ||
      V_stricmp(pExt, ".com") == 0 || V_stricmp(pExt, ".bat") == 0 ||
      V_stricmp(pExt, ".dll") == 0 || V_stricmp(pExt, ".ini") == 0 ||
...
void CNetChan::Clear() {
  ...
  for (i = 0; i < MAX_STREAMS; i++) {
    while (m_WaitingList[i].Count()) RemoveHeadInWaitingList(i);
...
void CNetChan::CompressFragments() {
  // We don't want this to go in the VCR file, because the compressed size can
  // be different. The reason is that the bf_writes that contributed to this
  // message may have uninitialized bits at the end of the buffer (for example
  // if it uses only the first couple bits of the last uint8_t in the message).
  // If the last few bits are different, it can produce a different compressed
  // size.
  ...
  for (int i = 0; i < MAX_STREAMS; i++) {
    ...
    if (data->isCompressed || data->bytes < 512) continue;
    ...
    if (data->buffer) {
      ...
      if (NET_BufferToBufferCompress(compressedData, &compressedSize,
    ...
    } else                      // it's a file
    {
      ...
      Q_snprintf(compressedfilename, sizeof(compressedfilename), "%s.ztmp",
                 data->filename);
      ...
      if (hZipFile != FILESYSTEM_INVALID_HANDLE) {
        ...
        if (NET_BufferToBufferCompress(compressed, &compressedSize,
...
void CNetChan::UncompressFragments(dataFragments_t *data) {
  ...
  // uncompress data
  NET_BufferToBufferDecompress(newbuffer, &uncompressedSize, data->buffer,
                               data->bytes);
...
void CNetChan::Shutdown(const char *pReason) {
  ...
  if (m_bProcessingMessages) {
    NET_RemoveNetChannel(this, false);  // Delay the deletion or it'll crash in
...
==============
*/
void CNetChan::Setup(int sock, netadr_t *adr, const char *name,
...
#ifdef _XBOX
  // We don't want to go into here because it'll eat up 192k extra memory in the
  // client and server's m_StreamData.
...
void CNetChan::SetSequenceData(int nOutSequenceNr, int nInSequenceNr,
                               int nOutSequenceNrAck) {
  Assert(IsPlayback());
...
void CNetChan::FlowNewPacket(int flow, int seqnr, int acknr, int nChoked,
                             int nDropped, int nSize) {
...
void CNetChan::FlowUpdate(int flow, int addbytes) {
  ...
  for (int i = 0; i < NET_FRAMES_BACKUP; i++) {
    ...
    if (pcurr->valid) {
      if (pcurr->time < starttime) starttime = pcurr->time;

      if (pcurr->time > endtime) endtime = pcurr->time;
  ...
  if (totaltime > 0.0f) {
    ...
    pflow->avgbytespersec +=
        (1.0f - FLOW_AVG) * ((float)totalbytes / totaltime);

    pflow->avgpacketspersec *= FLOW_AVG;
    pflow->avgpacketspersec +=
        (1.0f - FLOW_AVG) * ((float)totalvalid / totaltime);
  }
  ...
  if (totalPackets > 0) {
    ...
    pflow->avgloss += (1.0f - FLOW_AVG) *
                      ((float)(totalinvalid - totalchoked) / totalPackets);

    if (pflow->avgloss < 0) pflow->avgloss = 0;
...
bool CNetChan::Transmit(bool onlyReliable) {
  if (onlyReliable) m_StreamUnreliable.Reset();
...
bool CNetChan::CreateFragmentsFromFile(const char *filename, int stream,
  ...
  if (totalBytes >= (net_maxfilesize.GetInt() * 1024 * 1024)) {
    ConMsg(
        "CreateFragmentsFromFile: '%s' size exceeds net_maxfilesize limit (%i "
        "MB).\n",
  ...
  // check if send as stream or with snapshot
  data->asTCP = false;  // m_StreamActive && ( Bits2Bytes(data->length) >
                        // m_MaxReliablePayloadSize );
...
bool CNetChan::SendSubChannelData(bf_write &buf) {
  ...
  for (i = 0; i < MAX_STREAMS; i++) {
    ...
    if ((subChan->startFraggment[i] + subChan->numFragments[i]) ==
        data->numFragments) {
      // we are sending the last fragment, adjust length
      int rest = FRAGMENT_SIZE - (data->bytes % FRAGMENT_SIZE);
      if (rest < FRAGMENT_SIZE) length -= rest;
    }

    // if all fragments can be send within a single packet, avoid overhead (if
    // not a file)
    ...
    if (bSingleBlock) {
      ...
      buf.WriteUBitLong(subChan->startFraggment[i],
                        (MAX_FILE_SIZE_BITS - FRAGMENT_BITS));
      ...
      if (offset == 0) {
        ...
        buf.WriteUBitLong(data->bytes,
                          MAX_FILE_SIZE_BITS);  // 4MB max for files
      }
    }
    ...
    if (net_showfragments.GetBool()) {
      ConMsg("Sending subchan %i: start %i, num %i\n", subChan->index,
             subChan->startFraggment[i], subChan->numFragments[i]);
    }
...
bool CNetChan::ReadSubChannelData(bf_read &buf, int stream) {
  ...
  if (!bSingleBlock) {
    startFragment =
        buf.ReadUBitLong(MAX_FILE_SIZE_BITS - FRAGMENT_BITS);  // 16 MB max
...
inline unsigned short BufferToShortChecksum(const void *pvData,
                                            size_t nLength) {
...
inline unsigned short BufferToShortChecksum(const void *pvData, size_t nSize) {
  const uint32_t *pData = (const uint32_t *)pvData;
...
int CNetChan::SendDatagram(bf_write *datagram) {
  ...
  if (vcr_verbose.GetInt() && datagram && datagram->GetNumBytesWritten() > 0)
    VCRGenericValueVerify("datagram", datagram->GetBasePointer(),
                          datagram->GetNumBytesWritten() - 1);
#endif
  ...
  // process all new and pending reliable data, return true if reliable data
  // should been send with this packet
  ...
  bf_write flagsPos = send;  // remember flags uint8_t position
  ...
  if (m_StreamUnreliable.GetNumBitsWritten() < send.GetNumBitsLeft()) {
    send.WriteBits(m_StreamUnreliable.GetData(),
                   m_StreamUnreliable.GetNumBitsWritten());
  } else {
  ...
  if (!IsX360() && m_StreamVoice.GetNumBitsWritten() > 0 &&
  ...
  // Make sure we have enough bits to read a final net_NOP opcode before
  // compressing
  ...
  // if ( IsX360() )
  {
    // Now round up to uint8_t boundary
  ...
  // Compute checksum (must be aligned to a uint8_t boundary!!)
  if (ShouldChecksumPackets()) {
  ...
  // Send the datagram
  int bytesSent = NET_SendPacket(this, m_Socket, remote_address, send.GetData(),
  ...
  if (net_showudp.GetInt() && net_showudp.GetInt() != 2) {
    int mask = 63;
    char comp[64] = {0};
    if (net_compresspackets.GetBool() && bytesSent &&
    ...
    ConMsg("UDP -> %12.12s: sz=%5i seq=%5i ack=%5i rel=%1i tm=%8.3f%s\n",
           GetName(), send.GetNumBytesWritten(), (m_nOutSequenceNr)&mask,
           m_nInSequenceNr & mask, (flags & PACKET_FLAG_RELIABLE) ? 1 : 0,
           (float)net_time, comp);
  }
  ...
  FlowNewPacket(FLOW_OUTGOING, m_nOutSequenceNr, m_nInSequenceNr,
                m_nChokedPackets, 0, nTotalSize);
  ...
  // TODO:  This doesn't exactly match size sent when packet is a "split" packet
  // (actual bytes sent is higher, etc.)
...
bool CNetChan::ProcessMessages(bf_read &buf) {
  ...
  if (net_showpeaks.GetInt() > 0 &&
      net_showpeaks.GetInt() < buf.GetNumBytesLeft()) {
    showmsgname = "1";  // show messages for this packet only
  }

  // create a copy of reading buffer state for demo recording
  bf_read democopy = buf;
  ...
  while (true) {
    ...
    if (netmsg) {
      // let message parse itself from buffer
      const char *msgname = netmsg->GetName();

      int msgstartbit = buf.GetNumBitsRead();
      ...
      UpdateMessageStats(netmsg->GetGroup(),
                         buf.GetNumBitsRead() - msgstartbit);

      if (showmsgname) {
        if ((*showmsgname == '1') ||
            !Q_stricmp(showmsgname, netmsg->GetName())) {
          ConMsg("Msg from %s: %s\n", remote_address.ToString(),
                 netmsg->ToString());
        }
      }
      ...
      if (blockmsgname) {
        if ((*blockmsgname == '1') ||
            !Q_stricmp(blockmsgname, netmsg->GetName())) {
          ConMsg("Blocking message %s\n", netmsg->ToString());
          continue;
        }
      }

      // netmessage calls the Process function that was registered by it's
      // MessageHandler
...
void CNetChan::ProcessPlayback() {
  ...
  while ((packet = demoplayer->ReadPacket()) != NULL) {
    ...
    FlowNewPacket(FLOW_INCOMING, m_nInSequenceNr, m_nOutSequenceNrAck, 0, 0,
                  packet->wiresize);
...
CNetChan::subChannel_s *CNetChan::GetFreeSubChannel() {
  for (int i = 0; i < MAX_SUBCHANNELS; i++) {
    if (m_SubChannels[i].state == SUBCHANNEL_FREE) return &m_SubChannels[i];
  }
...
int CNetChan::ProcessPacketHeader(netpacket_t *packet) {
  ...
  if (ShouldChecksumPackets()) {
    unsigned short usCheckSum =
        (unsigned short)packet->message.ReadUBitLong(16);
  ...
  for (i = 0; i < MAX_SUBCHANNELS; i++) {
    ...
    if ((m_nOutReliableState & bitmask) == (relState & bitmask)) {
      if (subchan->state == SUBCHANNEL_DIRTY) {
        // subchannel was marked dirty during changelevel, waiting list is
        // already cleared
        subchan->Free();
        ...
        for (j = 0; j < MAX_STREAMS; j++) {
          if (subchan->numFragments[j] == 0) continue;
    ...
    } else  // subchannel doesn't match
    {
      if (subchan->sendSeqNr <= sequence_ack) {
        ...
        if (subchan->state == SUBCHANNEL_WAITING) {
          ...
          int bit =
              1 << subchan->index;  // flip bit back since data was send yet
  ...
  FlowNewPacket(FLOW_INCOMING, m_nInSequenceNr, m_nOutSequenceNrAck, nChoked,
                m_PacketDrop, packet->wiresize + UDP_HEADER_SIZE);
...
void CNetChan::ProcessPacket(netpacket_t *packet, bool bHasHeader) {
  ...
  if (net_showudp.GetInt() && net_showudp.GetInt() != 3) {
    ConMsg("UDP <- %s: sz=%i seq=%i ack=%i rel=%i tm=%f wire=%i\n", GetName(),
           packet->size, m_nInSequenceNr & 63, m_nOutSequenceNrAck & 63,
           flags & PACKET_FLAG_RELIABLE ? 1 : 0, net_time, packet->wiresize);
  }
  ...
  if (flags & PACKET_FLAG_RELIABLE) {
    ...
    for (i = 0; i < MAX_STREAMS; i++) {
      if (!CheckReceivingList(i)) return;  // error while processing
    }
  }
...
bool CNetChan::SendNetMsg(INetMessage &msg, bool bForceReliable, bool bVoice) {
  ...
  if (vcr_verbose.GetInt()) {
    ...
    if (nNewBytes > nOldBytes) {
      VCRGenericValueVerify("NetMsg", &pStream->GetBasePointer()[nOldBytes],
                            nNewBytes - nOldBytes - 1);
    }
#endif
    return bRet;
  } else {
...
INetMessage *CNetChan::FindMessage(int type) {
  ...
  for (int i = 0; i < numtypes; i++) {
    if (m_NetMessages[i]->GetType() == type) return m_NetMessages[i];
  }
...
bool CNetChan::SendData(bf_write &msg, bool bReliable) {
  ...
  if (msg.GetNumBitsWritten() > buf->GetNumBitsLeft()) {
    if (bReliable) {
      ConMsg("ERROR! SendData reliabe data too big (%i)",
...
bool CNetChan::SendReliableViaStream(dataFragments_t *data) {
  ...
  if (net_showtcp.GetInt()) {
    ConMsg("TCP -> %s: sz=%i seq=%i\n", remote_address.ToString(), data->bytes,
           m_nOutSequenceNr);
  }

  NET_SendStream(m_StreamSocket, (char *)header.GetData(),
                 header.GetNumBytesWritten(), 0);
...
bool CNetChan::SendReliableAcknowledge(int seqnr) {
  ...
  return NET_SendStream(m_StreamSocket, (char *)header.GetData(),
...
bool CNetChan::ProcessStream() {
  ...
  if (m_StreamReceived < m_StreamLength) {
    ...
    int bytesRecv = NET_ReceiveStream(
        m_StreamSocket, (char *)m_StreamData.Base() + m_StreamReceived,
        bytesLeft, 0);
...
void CNetChan::GetPacketResponseLatency(int flow, int frame_number,
                                        ...
                                        int *pnChoke) const {
  const netframe_t &nf =
      m_DataFlow[flow].frames[frame_number & NET_FRAMES_MASK];
  if (pnLatencyMsecs) {
    if (nf.dropped) {
      *pnLatencyMsecs = 9999;
    } else {
...
void CNetChan::GetRemoteFramerate(float *pflFrameTime,
                                  float *pflRemoteFrameTimeStdDeviation) const {
  if (pflFrameTime) {
...
float CNetChan::GetCommandInterpolationAmount(int flow,
                                              int frame_number) const {
  return m_DataFlow[flow]
      .frames[frame_number & NET_FRAMES_MASK]
      .m_flInterpolationAmount;
}
...
int CNetChan::GetPacketBytes(int flow, int frame_number, int group) const {
  if (group >= INetChannelInfo::TOTAL) {
    ...
    return Bits2Bytes(m_DataFlow[flow]
                          .frames[frame_number & NET_FRAMES_MASK]
                          .msggroups[group]);
  }
}
...
void CNetChan::UpdateMessageStats(int msggroup, int bits) {
  ...
  Assert((msggroup >= INetChannelInfo::GENERIC) &&
         (msggroup < INetChannelInfo::TOTAL));
...
void CNetChan::DecrementQueuedPackets() {
  ...
  if (m_nQueuedPackets < 0) m_nQueuedPackets = 0;
}

bool CNetChan::HasQueuedPackets() const { return m_nQueuedPackets > 0; }

void CNetChan::SetInterpolationAmount(float flInterpolationAmount) {
  m_flInterpolationAmount = flInterpolationAmount;
}

void CNetChan::SetRemoteFramerate(float flFrameTime,
