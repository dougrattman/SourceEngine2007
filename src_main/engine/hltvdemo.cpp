...
CHLTVDemoRecorder::CHLTVDemoRecorder() { m_bIsRecording = false; }

CHLTVDemoRecorder::~CHLTVDemoRecorder() { StopRecording(); }
...
void CHLTVDemoRecorder::StartRecording(const char *filename,
                                       bool bContinuously) {
  ...
  dh->signonlength =
      WriteSignonData();  // demoheader will be written when demo is closed
...
bool CHLTVDemoRecorder::IsRecording() { return m_bIsRecording; }

void CHLTVDemoRecorder::StopRecording() {
  if (!m_bIsRecording) return;
  ...
  ConMsg("Completed SourceTV demo \"%s\", recording time %.1f\n",
         m_DemoFile.m_szFileName, m_DemoFile.m_DemoHeader.playback_time);
}
...
void CHLTVDemoRecorder::WriteServerInfo() {
  ...
  CBaseServer *pServer =
      hltv->IsMasterProxy() ? (CBaseServer *)(&sv) : (CBaseServer *)(hltv);
...
void CHLTVDemoRecorder::WriteFrame(CHLTVFrame *pFrame) {
  ...
  assert(
      hltv->IsMasterProxy());  // this works only on the master since we use sv.
  ...
  // get delta frame
  CClientFrame *deltaFrame = hltv->GetClientFrame(
      m_nDeltaTick);  // NULL if delta_tick is not found or -1
  ...
  sv.WriteTempEntities(hltv->m_MasterClient, pFrame->GetSnapshot(),
                       fromSnapshot, msg, 255);
...
void CHLTVDemoRecorder::WriteMessages(unsigned char cmd, bf_write &message) {
  ...
  // uint8_t cmd = (m_pDemoFileHeader != NULL)  ? dem_signon : dem_packet;
