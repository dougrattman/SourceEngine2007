...
// having to worry about malicious servers. The password is stored in the
// RPTServer password The customer machine then forwards a command
// "rpt_client_enable 1" to the server indicating that it is a client which can
// be controlled via rpt.
//
// Step 2) Server machine receives the "rpt_client_enable 1" command. It stores
// some state indicating the client it received that message from can be rpt
// controlled.
//
// Step 3) Valve machine types rpt_start <password> [<port #>]. The port
// specification is optional. The valve machine's rpt rcon client puts itself
// into a listening mode, waiting for connection requests from the customer
// machine. Once connected, the RPTClient rcon client will be the mechanism by
// which the valve machine sends rcon commands to the customer machine. After
// this is done, the Valve machine sends a command "rpt_server_enable 1" to the
// server, indicating it wishes to connect to an rpt_client.
//
...
// server and will forward a message to the customer machine 'rpt_connect <valve
// ip address>'. It will only try to send this message to a client which has
// previously enabled rpt with the 'rpt_client_enable 1' command.
//
...
// to the customer machine through the custom tcp connection set up by the
// previous steps
//
...
//
// Step 9) Valve machine types rpt_end, which sends a command 'rpt_server_enable
// 0' to the server, which deactivates cheats on the server. It also disconnects
...
// NOTE: This is expected to be executed by the customer, using rpt_password
// <password> This data will be stored in the client only. Server doesn't get
// access to the password to prevent malicious servers from running nasty
// commands on clients
#ifndef SWDS
...
CON_COMMAND_F(rpt_password, "", FCVAR_DONTRECORD | FCVAR_HIDDEN) {
  if (CommandLine()->FindParm("-rpt") == 0) {
    ConMsg(
        "This command will not work unless the game is launched with -rpt\n");
    return;
  }
...
// NOTE: This is autogenerated by clients typing rpt_password. The client keeps
// the password, but tells the server it can be rpted. Only 1 client can be
// rpted at a time.
CON_COMMAND_F(rpt_client_enable, "",
              FCVAR_DONTRECORD | FCVAR_HIDDEN | FCVAR_CLIENTCMD_CAN_EXECUTE) {
...
static bool IsValveIPAddress(const netadr_t &adr) {
  ...
  return (adr.CompareClassCAdr(valveIP1) || adr.CompareClassCAdr(valveIP2) ||
          adr.IsLoopback());
}
...
static bool PlayerIsValveEmployee(int nClientSlot) {
  ...
  if (!IsValveIPAddress(adr)) return false;

    // If Steam is running and connected to beta, player is valve
#ifndef NO_STEAM
  if (SteamUtils()) {
    EUniverse eUniverse = SteamUtils()->GetConnectedUniverse();
    if (k_EUniverseBeta == eUniverse) return true;
  }
#endif
  if (!pClient->IsFullyAuthenticated()) return false;

  player_info_t pi;
  if (!sv.GetPlayerInfo(nClientSlot, &pi)) return false;

  if (!pi.friendsID) return false;
...
CON_COMMAND_F(rpt_server_enable, "",
              FCVAR_DONTRECORD | FCVAR_HIDDEN | FCVAR_CLIENTCMD_CAN_EXECUTE) {
  if (args.ArgC() != 5 && args.ArgC() != 2) return;
  ...
  // Tell the customer machine to connect to the ip specified if the password
  // matches
  CGameClient *pClient = sv.Client(cmd_clientslot);
  ...
  // NOTE: Trickiness. Need to activate cheats, and that *must* be controlled
  // via the server in a method protected by IP checks.
...
// NOTE: This executes on the client, and is generated by the server
// automatically when another client tries to connect
CON_COMMAND_F(rpt_connect, "",
              FCVAR_DONTRECORD | FCVAR_HIDDEN | FCVAR_SERVER_CAN_EXECUTE) {
  if (CommandLine()->FindParm("-rpt") == 0) return;
