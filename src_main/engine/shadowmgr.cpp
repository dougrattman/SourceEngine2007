...
//		vertex cache should be used or not. The assumption is that the
// client
//		of this system should know whether the shadows are always
// changing  or not
//
...
// of elements == to the number of material enumeration IDs, which is pretty
// large).
//-----------------------------------------------------------------------------
...
ConVar r_shadows_gamecontrol("r_shadows_gamecontrol", "-1",
                             FCVAR_CHEAT);  // Shadow override controlled by
                                            // game entities (shadow_controller)
...
class CShadowMgr : public IShadowMgrInternal, ISpatialLeafEnumerator {
 public:
  ...
  virtual ShadowHandle_t CreateShadow(IMaterial* pMaterial,
                                      ...
                                      void* pBindProxy, int creationFlags);
  virtual ShadowHandle_t CreateShadowEx(IMaterial* pMaterial,
                                        IMaterial* pModelMaterial,
                                        void* pBindProxy, int creationFlags);
  virtual void DestroyShadow(ShadowHandle_t handle);
  virtual void SetShadowMaterial(ShadowHandle_t handle, IMaterial* pMaterial,
                                 IMaterial* pModelMaterial, void* pBindProxy);
  ...
  virtual void ProjectShadow(ShadowHandle_t handle, const Vector& origin,
                             ...
                             const VMatrix& worldToShadow, const Vector2D& size,
                             int nLeafCount, const int* pLeafList,
                             float maxHeight, float falloffOffset,
                             float falloffAmount,
  ...
  // Set the number of world material buckets.  This should happen exactly once
  // per level load.
  ...
  virtual void DrawFlashlightDecalsOnSingleSurface(SurfaceHandle_t surfID,
                                                   bool bDoMasking);

  virtual void DrawFlashlightOverlays(int sortGroup, bool bDoMasking);

  virtual void DrawFlashlightDepthTexture();
  virtual void SetFlashlightDepthTexture(ShadowHandle_t shadowHandle,
                                         ITexture* pFlashlightDepthTexture,
                                         unsigned char ucShadowStencilBit);

  virtual void AddFlashlightRenderable(ShadowHandle_t shadow,
                                       IClientRenderable* pRenderable);
  virtual void DrawFlashlightDecalsOnDisplacements(int sortGroup,
 ...
 private:
  ...
  typedef CUtlFixedLinkedList<ShadowDecalHandle_t>::IndexType_t
      ShadowSurfaceIndex_t;
  ...
  typedef void (*ShadowDebugFunc_t)(ShadowHandle_t shadowHandle,
                                    const Vector& vecCentroid);

  // m_FlashlightWorldMaterialBuckets is where surfaces are stored per
  // flashlight each frame.
 ...
 private:
  ...
  void ApplyFlashlightToLeaf(const Shadow_t& shadow, mleaf_t* pLeaf,
  ...
  ShadowDecalHandle_t AddShadowDecalToSurface(SurfaceHandle_t surfID,
                                              ShadowHandle_t handle);
  void RemoveShadowDecalFromSurface(SurfaceHandle_t surfID,
                                    ShadowDecalHandle_t decalHandle);

  // Adds the surface to the list for this shadow
  void AddDecalToShadowList(ShadowHandle_t handle,
                            ShadowDecalHandle_t decalHandle);

  // Removes the shadow to the list of surfaces
  void RemoveDecalFromShadowList(ShadowHandle_t handle,
                                 ShadowDecalHandle_t decalHandle);

  // Actually projects + clips vertices
  int ProjectAndClipVertices(const Shadow_t& shadow,
                             const VMatrix& worldToShadow,
                             const VMatrix* pWorldToModel, int count,
                             Vector** ppPosition,
                             ShadowVertex_t*** ppOutVertex);
  ...
  void RenderShadowList(IMatRenderContext* pRenderContext,
                        ShadowDecalHandle_t decalHandle,
  ...
  bool GenerateDispShadowRenderInfo(IMatRenderContext* pRenderContext,
                                    ShadowDecal_t& decal,
                                    ShadowRenderInfo_t& info);

  // Generates a list shadow vertices to render
  bool GenerateNormalShadowRenderInfo(IMatRenderContext* pRenderContext,
                                      ShadowDecal_t& decal,
                                      ShadowRenderInfo_t& info);

  // Adds normal shadows to the mesh builder
  int AddNormalShadowsToMeshBuilder(CMeshBuilder& meshBuilder,
                                    ShadowRenderInfo_t& info);
  ...
  bool ComputeShadowVertices(ShadowDecal_t& decal, const VMatrix* pModelToWorld,
  ...
  void CopyClippedVertices(int count, ShadowVertex_t** ppSrcVert,
                           ShadowVertex_t* pDstVert, const Vector& vToAdd);
  ...
  void RenderDebuggingInfo(const ShadowRenderInfo_t& info,
                           ShadowDebugFunc_t func);

  // Methods for dealing with world material buckets for flashlights.
  void ClearAllFlashlightMaterialBuckets(void);
  void AddSurfaceToFlashlightMaterialBuckets(ShadowHandle_t handle,
  ...
  void SetStencilAndScissor(IMatRenderContext* pRenderContext,
                            FlashlightInfo_t& flashlightInfo, bool bUseStencil);

  void EnableStencilAndScissorMasking(IMatRenderContext* pRenderContext,
                                      const FlashlightInfo_t& flashlightInfo,
                                      bool bDoMasking);
  ...
  // Generates a list shadow vertices to render
  void GenerateShadowRenderInfo(IMatRenderContext* pRenderContext,
                                ShadowDecalHandle_t decalHandle,
                                ShadowRenderInfo_t& info);
  ...
  bool IsShadowNearSurface(ShadowHandle_t h, SurfaceHandle_t nSurfID,
                           const VMatrix* pModelToWorld,
                           const VMatrix* pWorldToModel);
 ...
 private:
  ...
  CUtlLinkedList<
      Shadow_t, ShadowHandle_t, false, int,
      CUtlMemoryAligned<UtlLinkedListElem_t<Shadow_t, ShadowHandle_t>, 16> >
      m_Shadows;
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CShadowMgr, IShadowMgr,
                                  ENGINE_SHADOWMGR_INTERFACE_VERSION,
                                  s_ShadowMgr);
...
//-----------------------------------------------------------------------------
void CShadowMgr::SetMaterial(Shadow_t& shadow, IMaterial* pMaterial,
                             IMaterial* pModelMaterial, void* pBindProxy) {
...
//-----------------------------------------------------------------------------
ShadowHandle_t CShadowMgr::CreateShadow(IMaterial* pMaterial,
                                        IMaterial* pModelMaterial,
                                        void* pBindProxy, int creationFlags) {
  return CreateShadowEx(pMaterial, pModelMaterial, pBindProxy, creationFlags);
}

ShadowHandle_t CShadowMgr::CreateShadowEx(IMaterial* pMaterial,
                                          IMaterial* pModelMaterial,
                                          void* pBindProxy, int creationFlags) {
#ifndef SWDS
...
void CShadowMgr::DestroyShadow(ShadowHandle_t handle) {
  ...
  if (m_Shadows[handle].m_FlashlightHandle !=
      m_FlashlightStates.InvalidIndex()) {
...
//-----------------------------------------------------------------------------
void CShadowMgr::SetShadowMaterial(ShadowHandle_t handle, IMaterial* pMaterial,
                                   IMaterial* pModelMaterial,
                                   void* pBindProxy) {
  Shadow_t& shadow = m_Shadows[handle];
  if ((shadow.m_pMaterial != pMaterial) ||
      (shadow.m_pModelMaterial != pModelMaterial) ||
      (shadow.m_pBindProxy != pBindProxy)) {
...
// Sets the texture coordinate range for a shadow...
//-----------------------------------------------------------------------------
void CShadowMgr::SetShadowTexCoord(ShadowHandle_t handle, float x, float y,
                                   float w, float h) {
  Shadow_t& shadow = m_Shadows[handle];
...
void CShadowMgr::AddExtraClipPlane(ShadowHandle_t h, const Vector& normal,
                                   float dist) {
  Shadow_t& shadow = m_Shadows[h];
...
//-----------------------------------------------------------------------------
inline ShadowVertex_t* CShadowMgr::AllocateVertices(ShadowVertexCache_t& cache,
                                                    int count) {
  cache.m_pVerts = 0;
...
// Adds the surface to the list for this shadow
//-----------------------------------------------------------------------------
void CShadowMgr::AddDecalToShadowList(ShadowHandle_t handle,
                                      ShadowDecalHandle_t decalHandle) {
...
// Removes the shadow to the list of surfaces
//-----------------------------------------------------------------------------
void CShadowMgr::RemoveDecalFromShadowList(ShadowHandle_t handle,
                                           ShadowDecalHandle_t decalHandle) {
  ...
  m_ShadowDecals[decalHandle].m_ShadowListIndex =
      m_ShadowSurfaces.InvalidIndex();
}

//-----------------------------------------------------------------------------
// Computes spherical bounds for a surface
//-----------------------------------------------------------------------------
void CShadowMgr::ComputeSurfaceBounds(SurfaceBounds_t* pBounds,
                                      SurfaceHandle_t nSurfID) {
  ...
  for (int i = 0; i < nCount; ++i) {
    int nVertIndex =
        host_state.worldbrush->vertindices[MSurf_FirstVertIndex(nSurfID) + i];
    const Vector& position =
        host_state.worldbrush->vertexes[nVertIndex].position;
...
//-----------------------------------------------------------------------------
const CShadowMgr::SurfaceBounds_t* CShadowMgr::GetSurfaceBounds(
    SurfaceHandle_t surfID) {
  ...
  if (m_SurfaceBoundsCache.Count() >= SURFACE_BOUNDS_CACHE_COUNT) {
    ...
    m_pSurfaceBounds[m_SurfaceBoundsCache[nIndex].m_nSurfaceIndex] =
        m_SurfaceBoundsCache.InvalidIndex();
  } else {
...
bool CShadowMgr::IsShadowNearSurface(ShadowHandle_t h, SurfaceHandle_t nSurfID,
  ...
  if (!pModelToWorld) {
    ...
    Vector3DMultiplyPosition(*pModelToWorld, pBounds->m_vecCenter,
                             vecSurfCenter);
  }
  ...
  if (flDistSqr >= flMinDistSqr) return false;

  if (!pModelToWorld)
    return IsBoxIntersectingRay(pBounds->m_vecMins, pBounds->m_vecMaxs,
                                shadow.m_Ray);

  Ray_t transformedRay;
  Vector3DMultiplyPosition(*pWorldToModel, shadow.m_Ray.m_Start,
                           transformedRay.m_Start);
  Vector3DMultiply(*pWorldToModel, shadow.m_Ray.m_Delta,
                   transformedRay.m_Delta);
...
//-----------------------------------------------------------------------------
inline ShadowDecalHandle_t CShadowMgr::AddShadowDecalToSurface(
    SurfaceHandle_t surfID, ShadowHandle_t handle) {
...
inline void CShadowMgr::RemoveShadowDecalFromSurface(
    SurfaceHandle_t surfID, ShadowDecalHandle_t decalHandle) {
...
void CShadowMgr::AddSurfaceToFlashlightMaterialBuckets(ShadowHandle_t handle,
                                                       SurfaceHandle_t surfID) {
  // Make sure that this is a flashlight.
  Assert(m_Shadows[handle].m_Flags & SHADOW_FLASHLIGHT);

  // Get the flashlight id for this particular shadow handle and make sure that
  // it's valid.
  FlashlightHandle_t flashlightID = m_Shadows[handle].m_FlashlightHandle;
  Assert(flashlightID != m_FlashlightStates.InvalidIndex());

  m_FlashlightStates[flashlightID].m_MaterialBuckets.AddElement(
      MSurf_MaterialSortID(surfID), surfID);
}
...
void CShadowMgr::AddSurfaceToShadow(ShadowHandle_t handle,
  ...
  // FLASHLIGHTFIXME: get rid of some of these checks for the ones that will
  // work just fine with the flashlight.
  bool bIsFlashlight = ((m_Shadows[handle].m_Flags & SHADOW_FLASHLIGHT) != 0);
  if (!bIsFlashlight &&
      MSurf_Flags(surfID) &
          (SURFDRAW_TRANS | SURFDRAW_ALPHATEST | SURFDRAW_NOSHADOWS))
    return;

#ifdef _XBOX
  // Don't let the flashlight get on water on XBox
  if (bIsFlashlight && (MSurf_Flags(surfID) & SURFDRAW_WATERSURFACE)) return;
#endif
...
void CShadowMgr::RemoveSurfaceFromShadow(ShadowHandle_t handle,
                                         SurfaceHandle_t surfID) {
...
void CShadowMgr::RemoveAllSurfacesFromShadow(ShadowHandle_t handle) {
  ...
  while (i != m_ShadowSurfaces.InvalidIndex()) {
    ...
    RemoveShadowDecalFromSurface(m_ShadowDecals[decalHandle].m_SurfID,
                                 decalHandle);
    ...
    while (i != m_ShadowsOnModels.InvalidIndex()) {
      ...
      if (!bWireframe) {
        if (shadow.m_Flags & SHADOW_FLASHLIGHT) {
          ...
          // since we need to render the models material again with different
          // lighting. Need to add something here to specify which flashlight.
          g_pStudioRender->AddShadow(
              NULL, NULL,
              &m_FlashlightStates[shadow.m_FlashlightHandle].m_FlashlightState,
              &shadow.m_WorldToShadow, shadow.m_pFlashlightDepthTexture);
        } else if (r_shadows_gamecontrol.GetInt() != 0) {
          g_pStudioRender->AddShadow(shadow.m_pModelMaterial,
                                     shadow.m_pBindProxy);
        }
      } else if ((shadow.m_Flags & SHADOW_FLASHLIGHT) ||
...
bool CShadowMgr::ModelHasShadows(ModelInstanceHandle_t instance) {
  if (instance != MODEL_INSTANCE_INVALID) {
    if (m_ShadowsOnModels.FirstElement(instance) !=
        m_ShadowsOnModels.InvalidIndex())
      return true;
  }
  return false;
}
...
// Applies the shadow to a surface
//-----------------------------------------------------------------------------
void CShadowMgr::ApplyShadowToSurface(ShadowBuildInfo_t& build,
                                      SurfaceHandle_t surfID) {
  ...
  // To do this, we're gonna want to project the surface into the space of the
  // decal Therefore, we want to produce a surface->world transformation, and a
  // world->shadow/light space transformation
  // Then we transform the surface points into shadow space and apply the
  // projection in shadow space.
...
// Applies the shadow to a displacement
//-----------------------------------------------------------------------------
void CShadowMgr::ApplyShadowToDisplacement(ShadowBuildInfo_t& build,
                                           IDispInfo* pDispInfo,
                                           bool bIsFlashlight) {
  // Trivial bbox reject.
  Vector bbMin, bbMax;
  ...
  if (!bIsFlashlight) {
    if (!IsBoxIntersectingSphere(bbMin, bbMax, build.m_vecSphereCenter,
                                 build.m_flSphereRadius))
      return;
  } else {
    if (R_CullBox(bbMin, bbMax, GetFlashlightFrustum(build.m_Shadow))) return;
  }
...
void CShadowMgr::ProjectShadow(ShadowHandle_t handle, const Vector& origin,
                               const Vector& projectionDir,
                               const VMatrix& worldToShadow,
                               const Vector2D& size, int nLeafCount,
                               const int* pLeafList, float maxHeight,
                               float falloffOffset, float falloffAmount,
                               const Vector& vecCasterOrigin) {
  ...
  Shadow_t& shadow = m_Shadows[handle];
  if (shadow.m_Flags & SHADOW_DISABLED) return;
...
void DrawFrustum(Frustum_t& frustum) {
  ...
  for (i = 0; i < FRUSTUM_NUMPLANES; i++) {
    ...
    int numPoints = PolyFromPlane(points, frustum.GetPlane(i)->normal,
                                  frustum.GetPlane(i)->dist);
    ...
    for (j = 0; j < FRUSTUM_NUMPLANES; j++) {
      ...
      numPoints =
          ClipPolyToPlane(in, numPoints, out, frustum.GetPlane(j)->normal,
                          frustum.GetPlane(j)->dist);
      Assert(numPoints <= maxPoints);
      swap(in, out);
    }
    int c;
    for (c = 0; c < numPoints; c++) {
      CDebugOverlay::AddLineOverlay(in[c], in[(c + 1) % numPoints], 0, 255, 0,
                                    255, true, 0.0f);
    }
  }
}
...
void CShadowMgr::ProjectFlashlight(ShadowHandle_t handle,
                                   const VMatrix& worldToShadow, int nLeafCount,
                                   const int* pLeafList) {
  VPROF_BUDGET("CShadowMgr::ProjectFlashlight",
  ...
  // Set up the frustum for the flashlight so that we can cull each leaf against
  // it.
  ...
  CalculateSphereFromProjectionMatrixInverse(
      shadowToWorld, &shadow.m_vecSphereCenter, &shadow.m_flSphereRadius);
  ...
  ShadowBuildInfo_t build;
  build.m_Shadow = handle;
  build.m_RayStart = m_FlashlightStates[shadow.m_FlashlightHandle]
                         .m_FlashlightState.m_vecLightOrigin;
  build.m_pVis = NULL;
  build.m_vecSphereCenter = shadow.m_vecSphereCenter;
  build.m_flSphereRadius = shadow.m_flSphereRadius;
  ...
  if (r_flashlightdrawfrustumbbox.GetBool()) {
    ...
    CDebugOverlay::AddBoxOverlay(Vector(0.0f, 0.0f, 0.0f), mins, maxs,
                                 QAngle(0, 0, 0), 0, 0, 255, 100, 0.0f);
  }
...
//-----------------------------------------------------------------------------
void CShadowMgr::ApplyFlashlightToLeaf(const Shadow_t& shadow, mleaf_t* pLeaf,
                                       ShadowBuildInfo_t* pBuild) {
  // Get the bounds of the leaf so that we can test it against the flashlight
  // frustum.
  ...
  // The flashlight frustum didn't intersect the bounding box for this leaf!
  // Get outta here!
  ...
  SurfaceHandle_t* pHandle =
      &host_state.worldbrush->marksurfaces[pLeaf->firstmarksurface];
  for (int i = 0; i < pLeaf->nummarksurfaces; i++) {
    SurfaceHandle_t surfID = pHandle[i];

    // only process each surface once;
    if (MSurf_VisFrame(surfID) == r_surfacevisframe) continue;
    ...
    // world-space vertex
    int vertIndex =
        host_state.worldbrush->vertindices[MSurf_FirstVertIndex(surfID)];
    ...
    float flDist = DotProduct(surfPlane.normal, pBuild->m_vecSphereCenter) -
                   surfPlane.dist;
    if (fabs(flDist) >= pBuild->m_flSphereRadius) continue;
    ...
    if (bCullDepth) {
      if ((MSurf_Flags(surfID) & SURFDRAW_NOCULL) == 0) {
        if (DotProduct(surfPlane.normal, lookdir) < BACKFACE_EPSILON) continue;
      } else {
        // Avoid edge-on shadows regardless.
        float dot = DotProduct(surfPlane.normal, lookdir);
        if (fabs(dot) < BACKFACE_EPSILON) continue;
      }
    }

    FlashlightInfo_t& flashlightInfo =
        m_FlashlightStates[shadow.m_FlashlightHandle];
    flashlightInfo.m_OccluderBuckets.AddElement(MSurf_MaterialSortID(surfID),
                                                surfID);
  }
}
...
void CShadowMgr::ApplyShadowToLeaf(const Shadow_t& shadow,
  ...
  for (int i = 0; i < pLeaf->nummarksurfaces; i++) {
    ...
    // If this surface has specifically had dynamic shadows disabled on it, then
    // get out!
    ...
    // Now apply the spherical cull
    float flDist = DotProduct(pSurfPlane->normal, pBuild->m_vecSphereCenter) -
                   pSurfPlane->dist;
    if (fabs(flDist) >= pBuild->m_flSphereRadius) continue;
...
bool CShadowMgr::EnumerateLeaf(int leaf, int context) {
  ...
  if (pBuild->m_pVis) {
    ...
    if (!BIT_SET(pBuild->m_pVis, cluster)) return true;
  }
...
void CShadowMgr::AddShadowToBrushModel(ShadowHandle_t handle, model_t* pModel,
  ...
  SurfaceHandleRestrict_t surfID = SurfaceHandleFromIndex(
  ...
  for (int i = 0; i < pModel->brush.nummodelsurfaces; ++i, ++surfID) {
    ...
    if (nFlags & SURFDRAW_NODRAW) continue;
...
void CShadowMgr::RemoveAllShadowsFromBrushModel(model_t* pModel) {
  SurfaceHandle_t surfID = SurfaceHandleFromIndex(
      pModel->brush.firstmodelsurface, pModel->brush.pShared);
  for (int i = 0; i < pModel->brush.nummodelsurfaces; ++i, ++surfID) {
...
//-----------------------------------------------------------------------------
void CShadowMgr::AddShadowsOnSurfaceToRenderList(
    ShadowDecalHandle_t decalHandle) {
  // Don't compute the surface cache if shadows are off..
  if (!r_shadows.GetInt()) return;
  ...
  while (decalHandle != m_ShadowDecals.InvalidIndex()) {
    ...
    if (m_Shadows[shadowDecal.m_Shadow].m_Flags & SHADOW_FLASHLIGHT) {
      AddSurfaceToFlashlightMaterialBuckets(shadowDecal.m_Shadow,
                                            shadowDecal.m_SurfID);
    } else if (r_shadows_gamecontrol.GetInt() != 0) {
      // For shadow rendering, hook the decal into the render list based on the
      // shadow material, not the surface material.
...
void CShadowMgr::ClearShadowRenderList() {
  ...
  if (m_RenderQueue.Count() > 0) {
    memset(m_RenderQueue.Base(), 0xFF,
           m_RenderQueue.Count() * sizeof(ShadowDecalHandle_t));
  }
  m_DecalsToRender = 0;
...
void CShadowMgr::RenderShadows(const VMatrix* pModelToWorld) {
  ...
  // Iterate through all sort ids and render for regular shadows, which get
  // their materials from the shadow material.
...
void CShadowMgr::RenderProjectedTextures(const VMatrix* pModelToWorld) {
  VPROF_BUDGET("CShadowMgr::RenderProjectedTextures",
...
class CClipTop {
 public:
  static inline bool Inside(ShadowVertex_t const& vert) {
    return vert.m_ShadowSpaceTexCoord.y < 1;
  }
  static inline float Clip(const Vector& one, const Vector& two) {
    return (1 - one.y) / (two.y - one.y);
  }
...
class CClipLeft {
 public:
  static inline bool Inside(ShadowVertex_t const& vert) {
    return vert.m_ShadowSpaceTexCoord.x > 0;
  }
  static inline float Clip(const Vector& one, const Vector& two) {
    return one.x / (one.x - two.x);
  }
  static inline bool IsPlane() { return false; }
  static inline bool IsAbove() { return false; }
};
...
class CClipRight {
 public:
  static inline bool Inside(ShadowVertex_t const& vert) {
    return vert.m_ShadowSpaceTexCoord.x < 1;
  }
  static inline float Clip(const Vector& one, const Vector& two) {
    return (1 - one.x) / (two.x - one.x);
  }
  static inline bool IsPlane() { return false; }
  static inline bool IsAbove() { return false; }
};
...
class CClipBottom {
 public:
  static inline bool Inside(ShadowVertex_t const& vert) {
    return vert.m_ShadowSpaceTexCoord.y > 0;
  }
  static inline float Clip(const Vector& one, const Vector& two) {
    return one.y / (one.y - two.y);
  }
  static inline bool IsPlane() { return false; }
  static inline bool IsAbove() { return false; }
};
...
class CClipAbove {
 public:
  static inline bool Inside(ShadowVertex_t const& vert) {
    return vert.m_ShadowSpaceTexCoord.z > 0;
  }
  static inline float Clip(const Vector& one, const Vector& two) {
    return one.z / (one.z - two.z);
  }
  static inline bool IsPlane() { return false; }
...
static inline void ClampTexCoord(ShadowVertex_t* pInVertex,
                                 ShadowVertex_t* pOutVertex) {
...
static inline void Intersect(ShadowVertex_t* pStart, ShadowVertex_t* pEnd,
                             ShadowVertex_t* pOut, bool startInside,
                             Clipper& clipper) {
  ...
  if (!Clipper::IsPlane()) {
    if (!Clipper::IsAbove()) {
      ...
      t = Clipper::Clip(pStart->m_ShadowSpaceTexCoord,
                        pEnd->m_ShadowSpaceTexCoord);

      VectorLerp(pStart->m_ShadowSpaceTexCoord, pEnd->m_ShadowSpaceTexCoord, t,
                 pOut->m_ShadowSpaceTexCoord);
    } else {
      t = Clipper::Clip(pStart->m_ShadowSpaceTexCoord,
                        pEnd->m_ShadowSpaceTexCoord);
      VectorLerp(pStart->m_ShadowSpaceTexCoord, pEnd->m_ShadowSpaceTexCoord, t,
...
bool CShadowMgr::ProjectVerticesIntoShadowSpace(const VMatrix& modelToShadow,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    Vector3DMultiplyPosition(modelToShadow, *ppPosition[i],
                             clip.m_pTempVertices[i].m_ShadowSpaceTexCoord);
...
//-----------------------------------------------------------------------------
int CShadowMgr::ProjectAndClipVertices(const Shadow_t& shadow,
                                       const VMatrix& worldToShadow,
                                       const VMatrix* pWorldToModel, int count,
                                       Vector** ppPosition,
                                       ShadowVertex_t*** ppOutVertex) {
  ...
  if (!ProjectVerticesIntoShadowSpace(worldToShadow, shadow.m_MaxDist, count,
                                      ppPosition, clip))
    return 0;
...
int CShadowMgr::ProjectAndClipVertices(ShadowHandle_t handle, int count,
  ...
  return ProjectAndClipVertices(m_Shadows[handle],
                                m_Shadows[handle].m_WorldToShadow, NULL, count,
                                ppPosition, ppOutVertex);
}
...
inline void CShadowMgr::CopyClippedVertices(int count,
...
bool CShadowMgr::ComputeShadowVertices(ShadowDecal_t& decal,
  ...
  // Create vertices to clip to...
  ShadowVertex_t** ppSrcVert;
  int clipCount = ProjectAndClipVertices(
      m_Shadows[decal.m_Shadow], *pModelToShadow, pWorldToModel,
  ...
  CopyClippedVertices(clipCount, ppSrcVert, pDstVert,
                      vNormal * OVERLAY_AVOID_FLICKER_NORMAL_OFFSET);
...
// Generates a list displacement shadow vertices to render
//-----------------------------------------------------------------------------
inline bool CShadowMgr::GenerateDispShadowRenderInfo(
    IMatRenderContext* pRenderContext, ShadowDecal_t& decal,
    ShadowRenderInfo_t& info) {
  int v, i;
  if (!MSurf_DispInfo(decal.m_SurfID)
           ->ComputeShadowFragments(decal.m_DispShadow, v, i))
    return false;

  // Catch overflows....
  if ((info.m_VertexCount + v >= info.m_nMaxVertices) ||
      (info.m_IndexCount + i >= info.m_nMaxIndices))
    return true;
...
// Generates a list shadow vertices to render
//-----------------------------------------------------------------------------
inline bool CShadowMgr::GenerateNormalShadowRenderInfo(
    IMatRenderContext* pRenderContext, ShadowDecal_t& decal,
    ShadowRenderInfo_t& info) {
  ...
  if (decal.m_ShadowVerts != m_VertexCache.InvalidIndex()) {
    ...
    bool bIsNear =
        IsShadowNearSurface(decal.m_Shadow, decal.m_SurfID,
                            info.m_pModelToWorld, &info.m_WorldToModel);
    if (!bIsNear) return false;
    ...
    // If no vertices were created, indicate this surface should be removed from
    // the cache
    if (!ComputeShadowVertices(decal, info.m_pModelToWorld,
                               &info.m_WorldToModel, pVertexCache))
      return false;
  }
...
// Generates a list shadow vertices to render
//-----------------------------------------------------------------------------
void CShadowMgr::GenerateShadowRenderInfo(IMatRenderContext* pRenderContext,
                                          ShadowDecalHandle_t decalHandle,
                                          ShadowRenderInfo_t& info) {
  ...
  for (; decalHandle != m_ShadowDecals.InvalidIndex(); decalHandle = next) {
    ...
    // Skip translucent shadows [ don't add their verts + indices to the render
    // lists ]
    Shadow_t& shadow = m_Shadows[decal.m_Shadow];
    if (shadow.m_FalloffBias == 255) continue;
...
//-----------------------------------------------------------------------------
void CShadowMgr::ComputeRenderInfo(ShadowDecalRenderInfo_t* pInfo,
                                   ShadowHandle_t handle) const {
  ...
  pInfo->m_flOOZFalloffDist =
      (flFalloffDist > 0.0f) ? 1.0f / flFalloffDist : 1.0f;
}

//-----------------------------------------------------------------------------
// Adds normal shadows to the mesh builder
//-----------------------------------------------------------------------------
int CShadowMgr::AddNormalShadowsToMeshBuilder(CMeshBuilder& meshBuilder,
                                              ShadowRenderInfo_t& info) {
  ...
  for (int i = 0; i < info.m_Count; ++i) {
    ...
    for (j = 0; j < vCount; ++j, ++pVerts) {
      ...
      Vector2DMultiply(pVerts->m_ShadowSpaceTexCoord.AsVector2D(),
                       shadow.m_vTexSize, texCoord);
...
int CShadowMgr::AddDisplacementShadowsToMeshBuilder(CMeshBuilder& meshBuilder,
                                                    ShadowRenderInfo_t& info,
                                                    int baseIndex) {
  if (!r_DrawDisp.GetBool()) return baseIndex;

  // Step through the cache and add all shadows on displacement surfaces
  for (int i = 0; i < info.m_DispCount; ++i) {
    baseIndex = DispInfo_AddShadowsToMeshBuilder(
        meshBuilder, info.m_pDispCache[i], baseIndex);
  }
...
//-----------------------------------------------------------------------------
// The following methods will display debugging info in the middle of each
// shadow decal
//-----------------------------------------------------------------------------
...
void CShadowMgr::RenderDebuggingInfo(const ShadowRenderInfo_t& info,
                                     ShadowDebugFunc_t func) {
  // Step through the cache and add all shadows on normal surfaces
  for (int i = 0; i < info.m_Count; ++i) {
    ShadowVertexCache_t* pVertexCache;
...
// Renders shadows that all share a material enumeration
//-----------------------------------------------------------------------------
void CShadowMgr::RenderShadowList(IMatRenderContext* pRenderContext,
                                  ShadowDecalHandle_t decalHandle,
                                  const VMatrix* pModelToWorld) {
  ...
  info.m_nMaxVertices =
      pRenderContext->GetMaxVerticesToRender(shadow.m_pMaterial);
  ...
  meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, info.m_VertexCount,
                    info.m_IndexCount);
...
//-----------------------------------------------------------------------------
// Set the number of world material buckets.  This should get called on level
// load.
//-----------------------------------------------------------------------------
void CShadowMgr::SetNumWorldMaterialBuckets(int numMaterialSortBins) {
  ...
  for (flashlightID = m_FlashlightStates.Head();
    ...
    m_FlashlightStates[flashlightID].m_MaterialBuckets.SetNumMaterialSortIDs(
        numMaterialSortBins);
    m_FlashlightStates[flashlightID].m_OccluderBuckets.SetNumMaterialSortIDs(
        numMaterialSortBins);
  }
  ClearAllFlashlightMaterialBuckets();
}
...
//-----------------------------------------------------------------------------
// Allocate world material buckets for a particular flashlight.  This should get
// called on flashlight creation.
//-----------------------------------------------------------------------------
void CShadowMgr::AllocFlashlightMaterialBuckets(
    FlashlightHandle_t flashlightID) {
  Assert(m_FlashlightStates.MaxElementIndex() >= flashlightID);
  m_FlashlightStates[flashlightID].m_MaterialBuckets.SetNumMaterialSortIDs(
      m_NumWorldMaterialBuckets);
  m_FlashlightStates[flashlightID].m_OccluderBuckets.SetNumMaterialSortIDs(
      m_NumWorldMaterialBuckets);
}
...
//-----------------------------------------------------------------------------
void CShadowMgr::UpdateFlashlightState(ShadowHandle_t shadowHandle,
                                       const FlashlightState_t& lightState) {
  m_FlashlightStates[m_Shadows[shadowHandle].m_FlashlightHandle]
      .m_FlashlightState = lightState;
}

void CShadowMgr::SetFlashlightDepthTexture(ShadowHandle_t shadowHandle,
                                           ITexture* pFlashlightDepthTexture,
                                           unsigned char ucShadowStencilBit) {
...
bool ScreenSpaceRectFromPoints(IMatRenderContext* pRenderContext,
                               Vector vClippedPolygons[8][10], int* pNumPoints,
                               int nNumPolygons, int* nLeft, int* nTop,
                               int* nRight, int* nBottom) {
  if (nNumPolygons == 0) return false;
  ...
  int nWidth, nHeight;
  g_pMaterialSystem->GetBackBufferDimensions(
      nWidth, nHeight);  // Get render target dimensions

  *nLeft = ((fMinX * 0.5f + 0.5f) * (float)nWidth) -
           1;  // Convert to render target pixel units
  ...
  *nBottom = clamp(*nBottom, 0, nHeight);
  ...
  bool bWithinBounds = ((*nLeft > 0) || (*nTop > 0) || (*nRight < nWidth) ||
                        (*nBottom < nHeight));
...
void ExtractFrustumPlanes(Frustum frustumPlanes, float flPlaneEpsilon) {
  ...
  GeneratePerspectiveFrustum(
      view.origin, vForward, vRight, vUp, view.zNear + flPlaneEpsilon,
      view.zFar - flPlaneEpsilon,  // Apply epsilon to near and far
...
void ConstructNearAndFarPolygons(Vector* pVecNearPlane, Vector* pVecFarPlane,
                                 float flPlaneEpsilon) {
  const CViewSetup& view = g_EngineRenderer->ViewGetCurrent();
  ...
  float flHalfNearHeight =
      flTanHalfAngleRadians * (view.zNear + flPlaneEpsilon);
  ...
  pVecNearPlane[0] =
      vCenterNear - (vRight * flHalfNearWidth) - (vUp * flHalfNearHeight);
  pVecNearPlane[1] =
      vCenterNear - (vRight * flHalfNearWidth) + (vUp * flHalfNearHeight);
  pVecNearPlane[2] =
      vCenterNear + (vRight * flHalfNearWidth) + (vUp * flHalfNearHeight);
  ...
  pVecFarPlane[0] =
      vCenterNear - (vRight * flHalfFarWidth) - (vUp * flHalfFarHeight);
  pVecFarPlane[1] =
      vCenterNear + (vRight * flHalfFarWidth) - (vUp * flHalfFarHeight);
  pVecFarPlane[2] =
      vCenterNear + (vRight * flHalfFarWidth) + (vUp * flHalfFarHeight);
...
void DrawDebugPolygon(int nNumVerts, Vector* pVecPoints, bool bFrontFacing,
                      bool bNearPlane) {
  int r = 0, g = 0, b = 0;
  if (bFrontFacing)
    b = 255;
...
void DrawPolygonToStencil(IMatRenderContext* pRenderContext, int nNumVerts,
                          Vector* pVecPoints, bool bFrontFacing,
                          bool bNearPlane) {
  IMaterial* pMaterial =
      materials->FindMaterial("engine/writestencil", TEXTURE_GROUP_OTHER, true);
  ...
  for (int i = 1; i < (nNumVerts - 1); i++) {
    ...
    meshBuilder.Position3f(pVecPoints[index].x, pVecPoints[index].y,
                           pVecPoints[index].z);
    meshBuilder.AdvanceVertex();
...
// Determine if two Vectors are sufficiently close (Manhattan-ish distance, not
// Euclidean)
bool SufficientlyClose(const Vector& v1, const Vector& v2, float flEpsilon) {
  if (fabs(v1.x - v2.x) >
      flEpsilon)  // Bail if x components are sufficiently different
    return false;

  if (fabs(v1.y - v2.y) >
      flEpsilon)  // Bail if y components are sufficiently different
    return false;

  if (fabs(v1.z - v2.z) >
      flEpsilon)  // Bail if z components are sufficiently different
    return false;

  return true;
}

int ClipPlaneToFrustum(Vector* pInPoints, Vector* pOutPoints,
                       Vector* pVecWorldFrustumPoints) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    Vector* pClipPolygon =
        pVecWorldFrustumPoints + (4 * i);  // Polygon defining clip plane
    ComputeTrianglePlane(pClipPolygon[0], pClipPolygon[1], pClipPolygon[2],
                         vNormal, flDist);  // Compute plane normal and dist

    if (bPing)
      nNumPoints = ClipPolyToPlane(vClipPing, nNumPoints, vClipPong, vNormal,
                                   flDist);  // Clip Ping into Pong
    else
      nNumPoints = ClipPolyToPlane(vClipPong, nNumPoints, vClipPing, vNormal,
                                   flDist);  // Clip Pong into Ping
...
void CShadowMgr::SetStencilAndScissor(IMatRenderContext* pRenderContext,
                                      FlashlightInfo_t& flashlightInfo,
                                      bool bUseStencil) {
  VMatrix matFlashlightToWorld;
  MatrixInverseGeneral(m_Shadows[flashlightInfo.m_Shadow].m_WorldToShadow,
                       matFlashlightToWorld);

  // Eight points defining the frustum in Flashlight space
  Vector vFrustumPoints[24] = {
      Vector(0.0f, 0.0f, 0.0f), Vector(1.0f, 0.0f, 0.0f),
      Vector(1.0f, 1.0f, 0.0f), Vector(0.0f, 1.0f, 0.0f),  // Near
      Vector(0.0f, 0.0f, 1.0f), Vector(0.0f, 1.0f, 1.0f),
      Vector(1.0f, 1.0f, 1.0f), Vector(1.0f, 0.0f, 1.0f),  // Far
      Vector(1.0f, 0.0f, 0.0f), Vector(1.0f, 0.0f, 1.0f),
      Vector(1.0f, 1.0f, 1.0f), Vector(1.0f, 1.0f, 0.0f),  // Right
      Vector(0.0f, 0.0f, 0.0f), Vector(0.0f, 1.0f, 0.0f),
      Vector(0.0f, 1.0f, 1.0f), Vector(0.0f, 0.0f, 1.0f),  // Left
      Vector(0.0f, 1.0f, 0.0f), Vector(1.0f, 1.0f, 0.0f),
      Vector(1.0f, 1.0f, 1.0f), Vector(0.0f, 1.0f, 1.0f),  // Bottom
      Vector(0.0f, 0.0f, 0.0f), Vector(0.0f, 0.0f, 1.0f),
      Vector(1.0f, 0.0f, 1.0f), Vector(1.0f, 0.0f, 0.0f)};  // Top
  ...
  Vector vClippedPolygons[8][10];  // Array of up to eight polygons (10 verts is
  ...
  for (int i = 0; i < 6; i++) {
    Vector* inVerts =
        vWorldFrustumPoints + (4 * i);  // Series of quadrilateral inputs
    Vector* tempVerts = vTempFace;
    Vector* outVerts = vClippedFace;

    int nClipCount =
        ClipPolyToPlane(inVerts, 4, tempVerts, vNearNormal,
                        flNearDist);  // need to set fOnPlaneEpsilon?

    if (nClipCount >
        2)  // If the polygon survived the near clip, try the far as well
    {
      nClipCount = ClipPolyToPlane(tempVerts, nClipCount, outVerts, vFarNormal,
                                   flFarDist);  // need to set fOnPlaneEpsilon?

      if (nClipCount > 2)  // If we still have a poly after clipping to both
                           // planes, add it to the list
      {
        memcpy(vClippedPolygons[nNumPolygons], outVerts,
               nClipCount * sizeof(Vector));
  ...
  int nClipCount = ClipPlaneToFrustum(
      vNearPlane, vClippedPolygons[nNumPolygons], vWorldFrustumPoints);
  if (nClipCount > 2)  // If the near plane clipped and resulted in a polygon,
                       // take note in the polygon list
  {
    nNumVertices[nNumPolygons] = nClipCount;
    nNumPolygons++;
    bNearPlane = true;
  ...
  for (int i = 0; i < nNumPolygons; i++)  // For each polygon
  {
    for (int j = 0; j < nNumVertices[i]; j++)  // For each vertex
    {
      for (int k = i + 1; k < nNumPolygons; k++)  // For each later polygon
      {
        ...
        {
          if (SufficientlyClose(vClippedPolygons[i][j], vClippedPolygons[k][m],
                                0.1f)) {
  ...
  if (r_flashlightscissor.GetBool() && (nNumPolygons > 0)) {
    ...
    flashlightInfo.m_FlashlightState.m_bScissor = ScreenSpaceRectFromPoints(
        pRenderContext, vClippedPolygons, nNumVertices, nNumPolygons, &nLeft,
        &nTop, &nRight, &nBottom);
  ...
  if (r_flashlightdrawclip.GetBool() && r_flashlightclip.GetBool() &&
      bUseStencil) {
    ...
    /*
                    ...
                    for (int i=0; i<nNumPolygons; i++)
                    {
                            DrawDebugPolygon( nNumVertices[i],
       vClippedPolygons[i], true, bNearPlane && (i == nNumPolygons-1)  );
                    }
    */
  }
  ...
  if (r_flashlightclip.GetBool() && bUseStencil) {
    /*
       ...
       Stencil fails pRenderContext->SetStencilZFailOperation(
       STENCILOPERATION_KEEP );
       // Stencil passes but depth fails
                    pRenderContext->SetStencilPassOperation(
       STENCILOPERATION_REPLACE );					// Z and
       stencil both pass pRenderContext->SetStencilCompareFunction(
       STENCILCOMPARISONFUNCTION_ALWAYS );			// Stencil
       always pass pRenderContext->SetStencilReferenceValue(
       m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit );
                    pRenderContext->SetStencilTestMask(
       m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit );
                    pRenderContext->SetStencilWriteMask(
       m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit );
       // Bit mask which is specific to this shadow
    */

    // Just blast front faces into the stencil buffer no matter what...
    pRenderContext->SetStencilEnable(true);
    pRenderContext->SetStencilFailOperation(
        STENCILOPERATION_REPLACE);  // Stencil fails
    pRenderContext->SetStencilZFailOperation(
        STENCILOPERATION_REPLACE);  // Stencil passes but depth fails
    pRenderContext->SetStencilPassOperation(
        STENCILOPERATION_REPLACE);  // Z and stencil both pass
    pRenderContext->SetStencilCompareFunction(
        STENCILCOMPARISONFUNCTION_ALWAYS);  // Stencil always pass
    pRenderContext->SetStencilReferenceValue(
        m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit);
    pRenderContext->SetStencilTestMask(
        m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit);
    pRenderContext->SetStencilWriteMask(
        m_Shadows[flashlightInfo.m_Shadow].m_ucShadowStencilBit);  // Bit mask
                                                                   // which is
                                                                   // specific
                                                                   // to this
                                                                   // shadow

    for (int i = 0; i < nNumPolygons;
         i++)  // Set the stencil bit on front facing
    {
      DrawPolygonToStencil(pRenderContext, nNumVertices[i], vClippedPolygons[i],
                           true, false);
    }

    /*
                    pRenderContext->SetStencilReferenceValue( 0x00000000 );
       // All bits cleared

                    for (int i=0; i<nNumPolygons; i++)
       // Clear the stencil bit on back facing
                    {
                            DrawPolygonToStencil( nNumVertices[i],
       vClippedPolygons[i], false, false );
                    }
    */
...
void CShadowMgr::SetFlashlightStencilMasks(bool bDoMasking) {
  VPROF_BUDGET("CShadowMgr::RenderFlashlights",
               VPROF_BUDGETGROUP_SHADOW_RENDERING);
  ...
  if (flashlightID == m_FlashlightStates.InvalidIndex()) return;
  ...
  for (; flashlightID != m_FlashlightStates.InvalidIndex();
    ...
    SetStencilAndScissor(
        pRenderContext, flashlightInfo,
        m_Shadows[flashlightInfo.m_Shadow].m_pFlashlightDepthTexture != NULL);
  }
}

void CShadowMgr::DisableStencilAndScissorMasking(
...
//---------------------------------------------------------------------------------------
void CShadowMgr::EnableStencilAndScissorMasking(
    IMatRenderContext* pRenderContext, const FlashlightInfo_t& flashlightInfo,
    bool bDoMasking) {
  // Bail out if we're not doing any of these optimizations
  ...
  if (pRenderContext->GetRenderTarget() == NULL) {
    ...
    if (r_flashlightclip.GetBool() &&
      ...
      pRenderContext->SetStencilEnable(true);
      pRenderContext->SetStencilFailOperation(
          STENCILOPERATION_KEEP);  // Stencil fails
      pRenderContext->SetStencilZFailOperation(
          STENCILOPERATION_KEEP);  // Stencil passes but depth fails
      pRenderContext->SetStencilPassOperation(
          STENCILOPERATION_KEEP);  // Z and stencil both pass

      pRenderContext->SetStencilCompareFunction(
          STENCILCOMPARISONFUNCTION_EQUAL);  // Bit must be set
      pRenderContext->SetStencilReferenceValue(
          ucShadowStencilBit);                                 // Specific bit
    ...
    if (r_flashlightscissor.GetBool() &&
      ...
      pRenderContext->SetScissorRect(flashlightInfo.m_FlashlightState.m_nLeft,
                                     flashlightInfo.m_FlashlightState.m_nTop,
                                     flashlightInfo.m_FlashlightState.m_nRight,
                                     flashlightInfo.m_FlashlightState.m_nBottom,
                                     true);
    }
  } else  // disable
  {
...
void CShadowMgr::SetFlashlightRenderState(ShadowHandle_t handle) {
  ...
  const FlashlightInfo_t& flashlightInfo =
      m_FlashlightStates[shadow.m_FlashlightHandle];
  pRenderContext->SetFlashlightStateEx(flashlightInfo.m_FlashlightState,
                                       shadow.m_WorldToShadow,
                                       shadow.m_pFlashlightDepthTexture);
}

//---------------------------------------------------------------------------------------
// Render all of the world and displacement surfaces that need to be drawn for
// flashlights
//---------------------------------------------------------------------------------------
void CShadowMgr::RenderFlashlights(bool bDoMasking,
                                   const VMatrix* pModelToWorld) {
#ifndef SWDS
  VPROF_BUDGET("CShadowMgr::RenderFlashlights",
               VPROF_BUDGETGROUP_SHADOW_RENDERING);
  ...
  for (; flashlightID != m_FlashlightStates.InvalidIndex();
       flashlightID = m_FlashlightStates.Next(flashlightID)) {
    FlashlightInfo_t& flashlightInfo = m_FlashlightStates[flashlightID];
    CMaterialsBuckets<SurfaceHandle_t>& materialBuckets =
        flashlightInfo.m_MaterialBuckets;
    CMaterialsBuckets<SurfaceHandle_t>::SortIDHandle_t sortIDHandle =
        materialBuckets.GetFirstUsedSortID();
    if (sortIDHandle == materialBuckets.InvalidSortIDHandle()) continue;

    pRenderContext->SetFlashlightStateEx(
        flashlightInfo.m_FlashlightState,
        m_Shadows[flashlightInfo.m_Shadow].m_WorldToShadow,
        m_Shadows[flashlightInfo.m_Shadow].m_pFlashlightDepthTexture);
    ...
    for (; sortIDHandle != materialBuckets.InvalidSortIDHandle();
      ...
      if (bWireframe) {
        ...
        pRenderContext->BindLightmapPage(
            materialSortInfoArray[sortID].lightmapPageID);
      }
      ...
      if (numIndices > 0) {
...
#else
        IMesh* pMesh = pRenderContext->GetDynamicMesh(
            false, g_WorldStaticMeshes[sortID], 0);
        CMeshBuilder meshBuilder;
        ...
        pRenderContext->BindVertexBuffer(
            0, g_WorldStaticMeshes[sortID], 0,
            materialSortInfoArray[sortID]
                .material->GetVertexFormat());  // hack fixme. . . use currently
                                                // bound material format instead
                                                // of passing in?
      ...
      // NOTE: If we ever need to make this faster, we could get larger batches
      // here. Draw displacements
      for (elemHandle = materialBuckets.GetElementListHead(sortID);
           elemHandle != materialBuckets.InvalidElementHandle();
           elemHandle = materialBuckets.GetElementListNext(elemHandle)) {
        SurfaceHandle_t surfID = materialBuckets.GetElement(elemHandle);
        if (SurfaceHasDispInfo(surfID)) {
          ...
          if (bWireframe) {
            ...
            pDisp->m_pMesh->m_pMesh->Draw(pDisp->m_iIndexOffset,
                                          pDisp->m_nIndices);
          }
        }
      }
  ...
  // Tell the materialsystem that we are finished drawing additive flashlight
  // lighting.
  pRenderContext->SetFlashlightMode(false);
...
const FlashlightState_t& CShadowMgr::GetFlashlightState(ShadowHandle_t handle) {
  Assert(m_Shadows[handle].m_Flags & SHADOW_FLASHLIGHT);
  Assert(m_Shadows[handle].m_FlashlightHandle != m_Shadows.InvalidIndex());
  return m_FlashlightStates[m_Shadows[handle].m_FlashlightHandle]
      .m_FlashlightState;
}

void CShadowMgr::DrawFlashlightDecals(int sortGroup, bool bDoMasking) {
  VPROF_BUDGET("CShadowMgr::DrawFlashlightDecals",
               VPROF_BUDGETGROUP_SHADOW_RENDERING);
  ...
  for (; flashlightID != m_FlashlightStates.InvalidIndex();
       flashlightID = m_FlashlightStates.Next(flashlightID)) {
    FlashlightInfo_t& flashlightInfo = m_FlashlightStates[flashlightID];
    pRenderContext->SetFlashlightState(
        flashlightInfo.m_FlashlightState,
        m_Shadows[flashlightInfo.m_Shadow].m_WorldToShadow);
...
void CShadowMgr::DrawFlashlightDecalsOnDisplacements(
    int sortGroup, CDispInfo* visibleDisps[MAX_MAP_DISPINFO], int nVisibleDisps,
    bool bDoMasking) {
  VPROF_BUDGET("CShadowMgr::DrawFlashlightDecalsOnDisplacements",
               VPROF_BUDGETGROUP_SHADOW_RENDERING);
...
void CShadowMgr::DrawFlashlightDecalsOnSingleSurface(SurfaceHandle_t surfID,
                                                     bool bDoMasking) {
  VPROF_BUDGET("CShadowMgr::DrawFlashlightDecalsOnSingleSurface",
               VPROF_BUDGETGROUP_SHADOW_RENDERING);
...
void CShadowMgr::DrawFlashlightOverlays(int nSortGroup, bool bDoMasking) {
  VPROF_BUDGET("CShadowMgr::DrawFlashlightOverlays",
               VPROF_BUDGETGROUP_SHADOW_RENDERING);
...
void CShadowMgr::DrawFlashlightDepthTexture() {
  ...
  FlashlightHandle_t flashlightID = m_FlashlightStates.Head();
  while (flashlightID !=
         m_FlashlightStates.InvalidIndex())  // Count up the shadows
  {
    FlashlightInfo_t& flashlightInfo = m_FlashlightStates[flashlightID];
    ...
    if (m_Shadows[flashlightInfo.m_Shadow].m_pFlashlightDepthTexture) {
      ...
      IMaterialVar* BaseTextureVar =
          pMaterial->FindVar("$basetexture", &foundVar, false);
      if (!foundVar) return;
      ...
      BaseTextureVar->SetTextureValue(
          m_Shadows[flashlightInfo.m_Shadow].m_pFlashlightDepthTexture);
...
void CShadowMgr::AddFlashlightRenderable(ShadowHandle_t shadowHandle,
                                         IClientRenderable* pRenderable) {
