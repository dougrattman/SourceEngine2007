  ...
  for (int iProp = 0; iProp < pTable->m_nProps; iProp++) {
    ...
    if (pProp->m_Type == DPT_DataTable) {
      ...
      if (pProp->IsExcludeProp()) {
        pProp->m_pExcludeDTName =
            COM_StringCopy(pSendTableProp->GetExcludeDTName());
...
// curID.
void DataTable_MaybeCreateReceiveTable(CUtlVector<SendTable *> &visited,
                                       SendTable *pTable, bool bNeedDecoder) {
  // Already sent?
  if (visited.Find(pTable) != visited.InvalidIndex()) return;
...
void DataTable_MaybeCreateReceiveTable_R(CUtlVector<SendTable *> &visited,
                                         SendTable *pTable) {
...
void DataTable_CreateClientTablesFromServerTables() {
  if (!serverGameDLL) {
    Sys_Error(
        "DataTable_CreateClientTablesFromServerTables:  No serverGameDLL "
        "loaded!");
  }
...
void DataTable_CreateClientClassInfosFromServerClasses(
  ...
  if (!serverGameDLL) {
    Sys_Error(
        "DataTable_CreateClientClassInfosFromServerClasses:  No serverGameDLL "
        "loaded!");
  }
  ...
  for (ServerClass *pClass = pClasses; pClass; pClass = pClass->m_pNext) {
    ...
    pState->m_pServerClasses[pClass->m_ClassID].m_ClassName =
        COM_StringCopy(pClass->m_pNetworkName);
    pState->m_pServerClasses[pClass->m_ClassID].m_DatatableName =
        COM_StringCopy(pClass->m_pTable->GetName());
  }
}

// If the table's ID is -1, writes its info into the buffer and increments
// curID.
void DataTable_MaybeWriteSendTableBuffer(SendTable *pTable, bf_write *pBuf,
                                         bool bNeedDecoder) {
  // Already sent?
...
bool DataTable_ParseClassInfosFromBuffer(CClientState *pState, bf_read *pBuf) {
  ...
  for (int i = 0; i < pState->m_nServerClasses; i++) {
    ...
    pState->m_pServerClasses[classID].m_ClassName =
        pBuf->ReadAndAllocateString();
    pState->m_pServerClasses[classID].m_DatatableName =
        pBuf->ReadAndAllocateString();
  }
...
bool DataTable_LoadDataTablesFromBuffer(bf_read *pBuf, int nDemoProtocol) {
  // Okay, read them out of the buffer since they weren't recorded into the main
  // network stream during recording
