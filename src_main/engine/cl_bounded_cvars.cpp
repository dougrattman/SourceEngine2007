...
void CL_RateCvarChanged(IConVar *pConVar, const char *pOldValue,
                        float flOldValue) {
  ConVarRef var{pConVar};

  // write rate to registry
  char rate[128];
  sprintf_s(rate, "%u", var.GetInt());
...
class CBoundedCvar_Rate : public ConVar_ServerBounded {
 public:
  CBoundedCvar_Rate()
      : ConVar_ServerBounded("rate", "10000", FCVAR_USERINFO,
 ...
 public:
  CBoundedCvar_CmdRate()
      : ConVar_ServerBounded(
            "cl_cmdrate", "30", FCVAR_ARCHIVE | FCVAR_USERINFO,
            "Max number of command packets sent to server per second", true,
            MIN_CMD_RATE, true, MAX_CMD_RATE) {}
  ...
  float GetFloat() const override {
    ...
    if (sv_mincmdrate.GetInt() != 0 && cl.m_nSignonState >= SIGNONSTATE_FULL) {
      ...
      if (fabs(diff) > sv_client_cmdrate_difference.GetFloat()) {
        if (diff > 0)
          flCmdRate = cl_updaterate->GetFloat() +
                      sv_client_cmdrate_difference.GetFloat();
        else
          flCmdRate = cl_updaterate->GetFloat() -
                      sv_client_cmdrate_difference.GetFloat();
      }
...
class CBoundedCvar_UpdateRate : public ConVar_ServerBounded {
 public:
  CBoundedCvar_UpdateRate()
      : ConVar_ServerBounded("cl_updaterate", "20",
                             FCVAR_ARCHIVE | FCVAR_USERINFO,
                             "Number of packets per second of updates you are "
