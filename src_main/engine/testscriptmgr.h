...
class CTestScriptMgr {
 public:
  ...
  // The engine calls this when it gets to certain points during execution. The
  // 'Test_StepTo' command stops executing scripts until it reaches a checkpoint
  // with a matching names.
  void CheckPoint(const char *pName);

  // This pauses the script until execution reaches the specified checkpoint.
  // If bOnce is true, then it won't start waiting if the specified checkpoint
  // was already hit.
  void SetWaitCheckPoint(const char *pCheckPointName, bool bOnce = false);

 private:
  // Runs until it hits a command that makes the script wait, like 'Test_Wait'
  // or 'Test_StepTo'.
  void RunCommands();
 ...
 private:
  ...
  char m_NextCheckPoint[32];  // If this is > 0 length it will wait to run
                              // script until you hit the next checkpoint with
                              // this name.

  double
      m_WaitUntil;  // It waits to execute scripts until the time reaches here.
