...
ConVar sv_logo_rate(
    "sv_logo_rate", "1024", 0,
    "How fast (bytes per second) the server sends logo files to clients.");
 ...
 public:
  ...
  CGameClient *GetClientByNetChannel(INetChannel *pChan) {
    for (int i = 0; i < sv.clients.Count(); i++) {
      ...
      if (pClient && pClient->GetNetChannel() == pChan) return pClient;
    }
    return NULL;
  }

  virtual void OnFinishedSending(INetChannel *pDest, const void *pUserData,
                                 int userDataLen, FileTransferID_t id) {
  ...
  virtual void OnFileReceived(INetChannel *pChan, const void *pUserData,
                              int userDataLength, const char *pFileData,
                              int fileLength) {
    // Ok, now the server has received a file the client sent. First, validate
    // the VTF.
    if (!LogoFile_IsValidVTFFile(pFileData, fileLength)) {
      Warning(
          "CServerFileTransferMgr::OnFileReceived: received an invalid logo "
          "file from a client.\n");
      return;
    }
    ...
    if (userDataLength < sizeof(CRC32_t)) {
      Warning(
          "CServerFileTransferMgr::OnFileReceived: invalid userDataLength "
          "(%d).\n",
          userDataLength);
    }
  ...
  // If any clients are waiting on this file, mark them so they know they can be
  // sent the file now.
  void MarkPendingFilesWithCRC(CRC32_t crcValue) {
    for (int i = 0; i < sv.clients.Count(); i++) {
      CGameClient *pClient = sv.Client(i);
      if (!pClient || !pClient->m_pLogoInfo) continue;
  ...
  bool SaveCRCFileToCache(CRC32_t crcValue, const void *pFileData,
                          int fileLength) {
  ...
  void UpdatePendingFiles() {
    ...
    for (int i = 0; i < sv.clients.Count(); i++) {
      ...
      for (int iFile = 0; iFile < pClient->m_pLogoInfo->m_PendingFiles.Count();
           iFile++) {
        ...
        if (fileData.Count() == 0 || lastCRC != pFile->m_nLogoFileCRC) {
          ...
          if (!LogoFile_ReadFile(pFile->m_nLogoFileCRC, fileData)) break;
        }

        StartSending(pClient->GetNetChannel(), &lastCRC, sizeof(lastCRC),
                     fileData.Base(), fileData.Count(), sv_logo_rate.GetInt());
...
PROCESS_MSG_SERVER(CLC_LogoFileData) {
  g_ServerFileTransferMgr.HandleReceivedData(m_Client->GetNetChannel(),
                                             m_Data.Base(), m_Data.Count());
  return true;
}
}
;
...
PROCESS_MSG_SERVER(CLC_LogoFileRequest) {
  ...
  file.m_bWaitingForServerToGetFile =
      SV_LogoFile_HasLogoFile(file.m_nLogoFileCRC);
...
bool SV_LogoFile_IsDownloadingLogoFile(CRC32_t crcValue) {
  for (int i = g_ServerFileTransferMgr.FirstIncoming();
       i != g_ServerFileTransferMgr.InvalidIncoming();
       i = g_ServerFileTransferMgr.NextIncoming(i)) {
    const void *pData;
    int dataLen;
    g_ServerFileTransferMgr.GetIncomingUserData(i, pData, dataLen);

    CRC32_t *pTestValue = (CRC32_t *)pData;
    if (*pTestValue == crcValue) return true;
  }
  return false;
}

void SV_LogoFile_OnConnect(CGameClient *pSenderClient, bool bValid,
                           CRC32_t crcValue) {
  ...
  if (bValid) {
    ...
    if (!SV_LogoFile_HasLogoFile(crcValue) &&
        !SV_LogoFile_IsDownloadingLogoFile(crcValue)) {
      SVC_LogoFileRequest fileRequest;
      fileRequest.m_nLogoFileCRC = crcValue;
      if (!pSenderClient->SendNetMsg(fileRequest, true)) {
        Host_Error(
            "SV_LogoFile_OnConnect: Reliable broadcast message would overflow "
            "client");
        return;
      }
    }
    ...
    for (int i = 0; i < sv.clients.Count(); i++) {
      CGameClient *pClient = sv.Client(i);
      if (!pClient || pClient == pSenderClient) continue;
  ...
  // Also, tell this client about all other client CRCs so it can aks for the
  // one it needs.
  for (int i = 0; i < sv.clients.Count(); i++) {
    CGameClient *pClient = sv.Client(i);
    if (!pClient || pClient == pSenderClient ||
        !pClient->m_pLogoInfo->m_bLogoFileCRCValid)
      continue;
