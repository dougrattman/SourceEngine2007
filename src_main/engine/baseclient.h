...
class CNetworkStatTrace {
 public:
  CNetworkStatTrace() : m_nMinWarningBytes(0), m_nStartBit(0), m_nCurBit(0) {}
...
class CBaseClient : public IGameEventListener2,
 ...
 public:
  ...
  bool
      m_bConVarsChanged;  // true if convars updated and not changes process yet
  bool m_bSendServerInfo;  // true if we need to send server info packet to
                           // start connect
  ...
  // a client can have couple of cutomized files distributed to all other
  // players
  CustomFile_t m_nCustomFiles[MAX_CUSTOM_FILES];
  int m_nFilesDownloaded;  // counter of how many files we downloaded from this
                           // client

  //===== NETWORK ============
  INetChannel *m_NetChannel;  // The client's net connection.
  int m_nSignonState;         // connection state
  int m_nDeltaTick;   // -1 = no compression.  This is where the server is
                      // creating the
  ...
  CSmartPtr<CFrameSnapshot, CRefCountAccessorLongName>
      m_pLastSnapshot;  // last send snapshot

  CFrameSnapshot *m_pBaseline;  // current entity baselines as a snapshot
  int m_nBaselineUpdateTick;    // last tick we send client a update baseline
                                // signal or -1
  CBitVec<MAX_EDICTS> m_BaselinesSent;  // baselines sent with last update
  int m_nBaselineUsed;  // 0/1 toggling flag, singaling client what baseline to
                        // use

  // This is used when we send out a nodelta packet to put the client in a state
  // where we wait until we get an ack from them on this packet. This is for 3
  // reasons:
  // 1. A client requesting a nodelta packet means they're screwed so no point
  // in deluging them with data.
  //    Better to send the uncompressed data at a slow rate until we hear back
  //    from them (if at all).
  // 2. Since the nodelta packet deletes all client entities, we can't ever
  // delta from a packet previous to it.
  // 3. It can eat up a lot of CPU on the server to keep building nodelta
  // packets while waiting for
  //    a client to get back on its feet.
  int m_nForceWaitForTick;

  bool m_bFakePlayer;  // JAC: This client is a fake player controlled by the
                       // game DLL
  bool m_bReceivedPacket;  // true, if client received a packet after the last
                           // send packet
