...
int GetOverlayTick() {
  if (sv.IsActive()) return sv.m_nTickCount;
...
bool OverlayText_t::IsDead() {
  ...
  if (m_nCreationTick != -1) {
    if (GetOverlayTick() > m_nCreationTick) return true;
...
//-----------------------------------------------------------------------------
OverlayText_t* s_pOverlayText =
    NULL;  // text is handled differently; for backward compatibility reasons
OverlayBase_t* s_pOverlays = NULL;
Vector s_vGridPosition(0, 0, 0);
bool s_bDrawGrid = false;

//-----------------------------------------------------------------------------
// Purpose: Hack to allow this code to run on a client that's not connected to a
// server
...
void AddTextOverlay(const Vector& textPos, float duration, float alpha,
  ...
  new_overlay->g = 255;
  new_overlay->b = 255;
  new_overlay->a = (int)std::clamp(alpha * 255.f, 0.f, 255.f);
...
void AddTextOverlay(const Vector& textPos, int line_offset, float duration,
  ...
  new_overlay->lineOffset = line_offset;
  new_overlay->SetEndTime(duration);
  new_overlay->r = (int)std::clamp(r * 255.f, 0.f, 255.f);
  new_overlay->g = (int)std::clamp(g * 255.f, 0.f, 255.f);
  new_overlay->b = (int)std::clamp(b * 255.f, 0.f, 255.f);
  new_overlay->a = (int)std::clamp(alpha * 255.f, 0.f, 255.f);
  new_overlay->bUseOrigin = true;
...
void AddBoxOverlay(const Vector& origin, const Vector& mins, const Vector& maxs,
                   QAngle const& angles, int r, int g, int b, int a,
                   float flDuration) {
  if (cl.IsPaused()) return;
...
void AddBoxOverlay2(const Vector& origin, const Vector& mins,
                    const Vector& maxs, QAngle const& orientation,
...
// Output :
//-----------------------------------------------------------------------------
void AddSphereOverlay(const Vector& vOrigin, float flRadius, int nTheta,
                      int nPhi, int r, int g, int b, int a, float flDuration) {
  if (cl.IsPaused()) return;
...
// Output :
//-----------------------------------------------------------------------------
void AddLineOverlay(const Vector& origin, const Vector& dest, int r, int g,
                    int b, int a, bool noDepthTest, float flDuration) {
  if (cl.IsPaused()) return;
...
void DrawGridOverlay(void) {
  ...
  for (int i = 1; i < numVertSpaces + 1; i++) {
    ...
    for (int j = 0; j < numHorzSpaces + 1; j++) {
      ...
      RenderLine(startLine, endLine, Color(color.x, color.y, color.z, 255),
                 true);
      ...
      for (int k = 0; k < numHorzSpaces + 1; k++) {
        ...
        RenderLine(bottomStartLine, bottomEndLine,
                   Color(color.x, color.y, color.z, 255), true);
...
void DrawOverlay(OverlayBase_t* pOverlay) {
  switch (pOverlay->m_Type) {
    case OVERLAY_LINE: {
      ...
      RenderLine(pLine->origin, pLine->dest,
                 Color(pLine->r, pLine->g, pLine->b, pLine->a),
                 !pLine->noDepthTest);
    } break;
    ...
    case OVERLAY_BOX: {
      ...
      if (pCurrBox->a > 0) {
        RenderBox(
            ...
            Color(pCurrBox->r, pCurrBox->g, pCurrBox->b, pCurrBox->a), false);
      }
      RenderWireframeBox(
          pCurrBox->origin, pCurrBox->angles, pCurrBox->mins, pCurrBox->maxs,
          Color(pCurrBox->r, pCurrBox->g, pCurrBox->b, 255), true);
    } break;
    ...
    case OVERLAY_BOX2: {
      ...
      if (pCurrBox->faceColor.a() > 0) {
        RenderBox(pCurrBox->origin, pCurrBox->angles, pCurrBox->mins,
                  pCurrBox->maxs, pCurrBox->faceColor, false);
      }
      if (pCurrBox->edgeColor.a() > 0) {
        RenderWireframeBox(pCurrBox->origin, pCurrBox->angles, pCurrBox->mins,
                           pCurrBox->maxs, pCurrBox->edgeColor, false);
      }
    } break;
    ...
    case OVERLAY_SPHERE: {
      ...
      RenderSphere(pSphere->vOrigin, pSphere->flRadius, pSphere->nTheta,
                   pSphere->nPhi,
    ...
    case OVERLAY_SWEPT_BOX: {
      ...
      RenderWireframeSweptBox(pBox->start, pBox->end, pBox->angles, pBox->mins,
                              pBox->maxs,
                              Color(pBox->r, pBox->g, pBox->b, pBox->a), true);
    } break;

    case OVERLAY_TRIANGLE: {
      OverlayTriangle_t* pTriangle = static_cast<OverlayTriangle_t*>(pOverlay);
      RenderTriangle(
          pTriangle->p1, pTriangle->p2, pTriangle->p3,
...
//------------------------------------------------------------------------------
// Purpose : Remove from the linkedlist overlays that have the special "until
// next
...
class CIVDebugOverlay : public IVDebugOverlay, public IVPhysicsDebugOverlay {
 ...
 public:
  void AddEntityTextOverlay(int ent_index, int line_offset, float duration,
    ...
    CDebugOverlay::AddEntityTextOverlay(ent_index, line_offset, duration, r, g,
                                        b, a, m_text);
  }

  void AddBoxOverlay(const Vector& origin, const Vector& mins,
                     const Vector& max, QAngle const& angles, int r, int g,
                     int b, int a, float duration) {
    CDebugOverlay::AddBoxOverlay(origin, mins, max, angles, r, g, b, a,
                                 duration);
  }
  ...
  void AddSweptBoxOverlay(const Vector& start, const Vector& end,
                          ...
                          float flDuration) {
    CDebugOverlay::AddSweptBoxOverlay(start, end, mins, max, angles, r, g, b, a,
                                      flDuration);
  }

  void AddLineOverlay(const Vector& origin, const Vector& dest, int r, int g,
                      int b, bool noDepthTest, float duration) {
    CDebugOverlay::AddLineOverlay(origin, dest, r, g, b, 255, noDepthTest,
                                  duration);
  }
  ...
  void AddTriangleOverlay(const Vector& p1, const Vector& p2, const Vector& p3,
                          int r, int g, int b, int a, bool noDepthTest,
                          float duration) {
    CDebugOverlay::AddTriangleOverlay(p1, p2, p3, r, g, b, a, noDepthTest,
                                      duration);
  }
  ...
  void AddTextOverlayRGB(const Vector& origin, int line_offset, float duration,
    ...
    CDebugOverlay::AddTextOverlay(origin, line_offset, duration, r, g, b, alpha,
                                  m_text);
  }
  ...
  void AddTextOverlayRGB(const Vector& origin, int line_offset,
    ...
    CDebugOverlay::AddTextOverlay(
        origin, line_offset, flDuration, r * 1.0f / 255.0f, g * 1.0f / 255.0f,
        b * 1.0f / 255.0f, alpha * 1.0f / 255.0f, m_text);
  }
  ...
  void AddLineOverlayAlpha(const Vector& origin, const Vector& dest, int r,
                           int g, int b, int a, bool noDepthTest,
                           float duration) {
    CDebugOverlay::AddLineOverlay(origin, dest, r, g, b, a, noDepthTest,
                                  duration);
  }
  ...
  void AddBoxOverlay2(const Vector& origin, const Vector& mins,
                      ...
                      const Color& faceColor, const Color& edgeColor,
                      float duration) {
    CDebugOverlay::AddBoxOverlay2(origin, mins, maxs, orientation, faceColor,
                                  edgeColor, duration);
  }
};
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CIVDebugOverlay, IVDebugOverlay,
                                  VDEBUG_OVERLAY_INTERFACE_VERSION,
                                  g_DebugOverlay);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CIVDebugOverlay, IVPhysicsDebugOverlay,
                                  VPHYSICS_DEBUG_OVERLAY_INTERFACE_VERSION,
                                  g_DebugOverlay);
