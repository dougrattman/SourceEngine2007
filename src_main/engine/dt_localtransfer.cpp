...
static ConVar dt_UsePartialChangeEnts(
    "dt_UsePartialChangeEnts", "1", 0,
    "(SP only) - enable FL_EDICT_PARTIAL_CHANGE optimization.");

static ConVar dt_ShowPartialChangeEnts(
    "dt_ShowPartialChangeEnts", "0", 0,
    "(SP only) - show entities that were copied using small optimized lists "
    "(FL_EDICT_PARTIAL_CHANGE).");
...
template <class T>
inline void LocalTransfer_FastType(T *pBlah, CServerDatatableStack &serverStack,
...
// where they are tracked in a list when they change. If their m_pProxies
// pointers are set to 1, then it means that this property is gotten to by means
// of SendProxy_DataTableToDataTable. If it's set to 0, then we can't directly
// take the property's offset.
class CPropMapStack : public CDatatableStack {
 public:
  CPropMapStack(CSendTablePrecalc *pPrecalc,
  ...
  bool IsNonPointerModifyingProxy(SendTableProxyFn fn,
                                  const CStandardSendProxies *pSendProxies) {
    ...
    if (pSendProxies->m_ppNonModifiedPointerProxies) {
      CNonModifiedPointerProxy *pCur =
          *pSendProxies->m_ppNonModifiedPointerProxies;
      while (pCur) {
        if (pCur->m_Fn == fn) return true;
        pCur = pCur->m_pNext;
      }
    }
  ...
  inline unsigned char *CallPropProxy(CSendNode *pNode, int iProp,
    ...
    if (!pStructBase) return 0;

    const SendProp *pProp = m_pPropMapStackPrecalc->GetDatatableProp(iProp);
    if (IsNonPointerModifyingProxy(pProp->GetDataTableProxyFn(),
                                   m_pSendProxies)) {
      // Note: these are offset by 1 (see the constructor), otherwise it won't
      // recurse during the Init call because pCurStructBase is 0.
...
void BuildPropOffsetToIndexMap(CSendTablePrecalc *pPrecalc,
                               const CStandardSendProxies *pSendProxies) {
  ...
  for (int i = 0; i < pPrecalc->m_Props.Count(); i++) {
    ...
    if (pmStack.GetCurStructBase() != 0) {
      ...
      if (pProp->GetType() == DPT_Array) {
        offset = pProp->GetArrayProp()->GetOffset() +
                 (int)pmStack.GetCurStructBase() - 1;
      ...
      if (offset != 0) {
        for (int j = 0; j < elementCount; j++) {
          if (pProp->GetFlags() & SPROP_IS_A_VECTOR_ELEM) {
            AddPropOffsetToMap(pPrecalc, i | PROP_INDEX_VECTOR_ELEM_MARKER,
                               offset);
          } else {
...
void LocalTransfer_InitFastCopy(
    const SendTable *pSendTable, const CStandardSendProxies *pSendProxies,
    RecvTable *pRecvTable, const CStandardRecvProxies *pRecvProxies,
    int &nSlowCopyProps,  // These are incremented to tell you how many fast
                          // copy props it found.
    int &nFastCopyProps) {
  ...
  for (int iProp = 0; iProp < iNumProp; iProp++) {
    ...
    if (pRecvProp) {
      ...
      if (pSendProp->GetType() == DPT_Int &&
          (pSendProp->GetProxyFn() == pSendProxies->m_Int32ToInt32 ||
           pSendProp->GetProxyFn() == pSendProxies->m_UInt32ToInt32) &&
        ...
        ++nFastCopyProps;
                 ...
                 (pSendProp->GetProxyFn() == pSendProxies->m_Int16ToInt32 ||
                  pSendProp->GetProxyFn() == pSendProxies->m_UInt16ToInt32) &&
                 pRecvProp->GetProxyFn() == pRecvProxies->m_Int32ToInt16) {
        pList = &pPrecalc->m_FastLocalTransfer.m_FastInt16;
        ++nFastCopyProps;
      } else if (pSendProp->GetType() == DPT_Int &&
                 (pSendProp->GetProxyFn() == pSendProxies->m_Int8ToInt32 ||
                  pSendProp->GetProxyFn() == pSendProxies->m_UInt8ToInt32) &&
...
inline int MapPropOffsetsToIndices(const CBaseEdict *pEdict,
  ...
  for (unsigned short i = 0; i < nOffsets; i++) {
    ...
    if (index == pPrecalc->m_PropOffsetToIndexMap.InvalidIndex()) {
      ...
      // order for that NetworkStateChanged call to mean anything.
      //
      // It means that if we can't find an offset here, then there isn't a
      // SendProp associated with the CNetworkVar that triggered the change.
      // Therefore, the change doesn't matter and we can skip past it.
      //
      // If we wanted to be anal, we could force them to use
      // DISABLE_NETWORK_VAR_FOR_DERIVED appropriately in all these cases, but
      // then we'd need a ton of them for certain classes (like CBaseViewModel,
      // which has a slew of CNetworkVars in its base classes that it doesn't
      // want to transmit).
      ...
      if (dt_ShowPartialChangeEnts.GetInt()) {
        ...
        Q_snprintf(str, sizeof(str),
                   ...
                   pEdict->GetClassName(),
                   pPrecalc->m_pSendTable->m_pNetTableName, pOffsets[i]);
...
inline void FastSortList(unsigned short *pList, unsigned short nEntries) {
  ...
  while (1) {
    if (pList[i + 1] < pList[i]) {
      ...
      ++i;
      if (i >= (nEntries - 1)) return;
    }
  }
}

inline void AddToPartialChangeEntsList(int iEnt, bool bPartial) {
  if (!dt_ShowPartialChangeEnts.GetInt()) return;

#if !defined(_XBOX)
  if (!bPartial) iEnt = -iEnt;

  if (g_PartialChangeEnts.Find(iEnt) == -1) g_PartialChangeEnts.AddToTail(iEnt);
#endif
}
...
void PrintPartialChangeEntsList() {
  ...
  for (int i = 0; i < g_PartialChangeEnts.Count(); i++) {
    if (g_PartialChangeEnts[i] >= 0) {
      if (!bFirst) Q_strncat(str, ", ", sizeof(str));
...
void LocalTransfer_TransferEntity(const CBaseEdict *pEdict,
                                  const SendTable *pSendTable,
                                  const void *pSrcEnt, RecvTable *pRecvTable,
                                  void *pDestEnt, bool bNewlyCreated,
                                  bool bJustEnteredPVS, int objectID) {
  ++g_nTotalEntChanges;
  ...
  if (pEdict->GetChangeInfoSerialNumber() ==
          g_pSharedChangeInfo->m_iSerialNumber &&
      !bNewlyCreated && !bJustEnteredPVS && dt_UsePartialChangeEnts.GetInt()) {
    CSendTablePrecalc *pPrecalc = pSendTable->m_pPrecalc;

    int nChangeOffsets =
        MapPropOffsetsToIndices(pEdict, pPrecalc, pCI->m_ChangeOffsets,
                                pCI->m_nChangeOffsets, propIndices);
    if (nChangeOffsets == 0) return;
    ...
    CServerDatatableStack serverStack(pPrecalc, (unsigned char *)pSrcEnt,
                                      objectID);
    serverStack.Init(true);

    // Setup the structure to traverse the dest tree.
    CRecvDecoder *pDecoder = pRecvTable->m_pDecoder;
    ErrorIfNot(pDecoder, ("RecvTable_Decode: table '%s' missing a decoder.",
                          pRecvTable->GetName()));
    CClientDatatableStack clientStack(pDecoder, (unsigned char *)pDestEnt,
                                      objectID);
    ...
    for (int iChanged = 0; iChanged < nChangeOffsets; iChanged++) {
      ...
      if (pSendBase) {
        ...
        g_PropTypeFns[pRecvProp->GetType()].FastCopy(
            pSendProp, pRecvProp, pSendBase, pRecvBase, objectID);
      }
    }
  }
  // Whereas the below code copies _all_ fields, regardless of whether they were
  // changed or not.  We run this only newly created entities, or entities
  //  which were previously dormant/outside the pvs but are now back in the PVS
  //  since we could have missed field updates since the changeoffsets get
  //  cleared every frame.
  else {
    // Setup the structure to traverse the source tree.
    CSendTablePrecalc *pPrecalc = pSendTable->m_pPrecalc;
    ...
    CFastLocalTransferPropInfo *pPropList =
        pPrecalc->m_FastLocalTransfer.m_OtherProps.Base();
    ...
    LocalTransfer_FastType((int *)0, serverStack, clientStack,
                           pPrecalc->m_FastLocalTransfer.m_FastInt32.Base(),
                           pPrecalc->m_FastLocalTransfer.m_FastInt32.Count());
    LocalTransfer_FastType((short *)0, serverStack, clientStack,
                           pPrecalc->m_FastLocalTransfer.m_FastInt16.Base(),
                           pPrecalc->m_FastLocalTransfer.m_FastInt16.Count());
    LocalTransfer_FastType((char *)0, serverStack, clientStack,
                           pPrecalc->m_FastLocalTransfer.m_FastInt8.Base(),
                           pPrecalc->m_FastLocalTransfer.m_FastInt8.Count());
    LocalTransfer_FastType((Vector *)0, serverStack, clientStack,
                           pPrecalc->m_FastLocalTransfer.m_FastVector.Base(),
                           pPrecalc->m_FastLocalTransfer.m_FastVector.Count());
  }
