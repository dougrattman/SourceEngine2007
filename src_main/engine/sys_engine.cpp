...
//-----------------------------------------------------------------------------
static ConVar fps_max("fps_max", (IsX360()) ? "30" : "300", 0,
                      "Frame rate limiter");
#ifndef _RETAIL
...
class CEngine : public IEngine {
 public:
  ...
  bool Load(bool is_dedicated, const char *basedir);
...
//-----------------------------------------------------------------------------
bool CEngine::Load(bool is_dedicated, const char *rootdir) {
  // Activate engine
  // NOTE: We must bypass the 'next state' block here for initialization to work
  // properly.
  m_nDLLState = m_nNextDLLState = InEditMode() ? DLL_PAUSED : DLL_ACTIVE;

  const bool is_loaded = Sys_InitGame(
      g_AppSystemFactory, rootdir, game->GetMainWindowAddress(), is_dedicated);
  if (is_loaded) {
...
bool CEngine::FilterTime(float dt) {
  ...
  if (!sv.IsDedicated() && !CanCheat() && fps_max.GetFloat() < 30) {
    ...
    if (fps_max.GetFloat() != 0.0f) {
      Warning(
          "sv_cheats is 0 and fps_max is being limited to a minimum of 30 (or "
          "set to 0).\n");
