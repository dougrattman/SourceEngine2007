...
class VoiceCodec_Frame : public IVoiceCodec {
 public:
  ...
  virtual ~VoiceCodec_Frame() {
    if (m_pFrameEncoder) m_pFrameEncoder->Release();
  }

  virtual bool Init(int quality) {
    if (m_pFrameEncoder &&
        m_pFrameEncoder->Init(quality, m_nRawBytes, m_nEncodedBytes)) {
      m_nRawSamples = m_nRawBytes >> 1;
      Assert(m_nRawBytes <= MAX_FRAMEBUFFER_SAMPLES &&
             m_nEncodedBytes <= MAX_FRAMEBUFFER_SAMPLES);
      return true;
    } else {
      if (m_pFrameEncoder) m_pFrameEncoder->Release();
  ...
  virtual int Compress(const char *pUncompressedBytes, int nSamples,
                       char *pCompressed, int maxCompressedBytes, bool bFinal) {
    if (!m_pFrameEncoder) return 0;
    ...
    while ((nSamples + m_nEncodeBufferSamples) >= m_nRawSamples &&
      ...
      // Compress it.
      m_pFrameEncoder->EncodeFrame((const char *)samples,
    ...
    if (bFinal && m_nEncodeBufferSamples &&
      ...
      memset(&m_EncodeBuffer[m_nEncodeBufferSamples], 0,
             (m_nRawSamples - m_nEncodeBufferSamples) * BYTES_PER_SAMPLE);
      m_pFrameEncoder->EncodeFrame((const char *)m_EncodeBuffer,
                                   &pCompressed[nCompressedBytes]);
      nCompressedBytes += m_nEncodedBytes;
      m_nEncodeBufferSamples = 0;
    }
  ...
  virtual int Decompress(const char *pCompressed, int compressedBytes,
                         char *pUncompressed, int maxUncompressedBytes) {
    if (!m_pFrameEncoder) return 0;
    ...
    while ((compressedBytes - curCompressedByte) >= m_nEncodedBytes &&
      ...
      m_pFrameEncoder->DecodeFrame(&pCompressed[curCompressedByte],
