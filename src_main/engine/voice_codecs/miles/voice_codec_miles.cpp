...
FrameEncoder_Miles::~FrameEncoder_Miles() { Shutdown(); }

bool FrameEncoder_Miles::Init(int quality, int &rawFrameSize,
  ...
  if (!m_Encoder.Init((void *)this, ".RAW", suffix,
                      &FrameEncoder_Miles::EncodeStreamCB)) {
  ...
  if (!m_Decoder.Init((void *)this, suffix, ".RAW",
                      &FrameEncoder_Miles::DecodeStreamCB)) {
  ...
  rawFrameSize =
      m_nRawBytes *
      2;  // They'll be using 16-bit samples and we're quantizing to 8-bit.
  encodedFrameSize = m_nEncodedBytes;

  return true;
}

void FrameEncoder_Miles::Release() { delete this; }

void FrameEncoder_Miles::EncodeFrame(const char *pUncompressedBytes,
                                     char *pCompressed) {
  char samples[1024];

  if (!m_Encoder.IsActive() || m_nRawBytes > sizeof(samples)) return;
...
void FrameEncoder_Miles::DecodeFrame(const char *pCompressed,
                                     char *pDecompressed) {
  if (!m_Decoder.IsActive()) return;
...
bool FrameEncoder_Miles::ResetState() {
  if (!m_Decoder.IsActive() || !m_Encoder.IsActive()) return true;
...
void FrameEncoder_Miles::FigureOutFrameSizes() {
  // Figure out the frame sizes. It is probably not prudent in general to assume
  // fixed frame sizes with Miles codecs but it works with the voxware codec
  // right now and simplifies things a lot.
