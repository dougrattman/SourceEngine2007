...
HPROVIDER CProvider::GetProviderHandle() { return m_hProvider; }
...
bool ASISTRUCT::Init(void *pCallbackObject, const char *pInputFileType,
                     const char *pOutputFileType, AILASIFETCHCB cb) {
  // Get the provider.
  HPROVIDER hProvider =
      RIB_find_files_provider("ASI codec", "Output file types", pOutputFileType,
                              "Input file types", pInputFileType);

  if (!hProvider) return false;
  ...
  RIBRESULT result =
      RIB_request(m_pProvider->GetProviderHandle(), "ASI stream", RECVSTR);
  if (result != RIB_NOERR) return false;
  ...
  if (result != RIB_NOERR) ASI_stream_seek = NULL;
...
bool ASISTRUCT::IsActive() const { return m_stream != NULL ? true : false; }
