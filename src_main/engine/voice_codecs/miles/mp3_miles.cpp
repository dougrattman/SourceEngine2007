...
CMilesMP3::~CMilesMP3() { m_decoder.Shutdown(); }
...
void CMilesMP3::SetPosition(unsigned int position) { m_decoder.Seek(position); }
...
class CVAudio : public IVAudio {
 public:
  ...
  void DestroyMP3StreamDecoder(IAudioStream *pDecoder) { delete pDecoder; }
};
