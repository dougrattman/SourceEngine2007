...
class CSendNode;
static CSendProxyRecipients
    s_Recipients;  // avoid calling constructor each time
...
the_interface CDatatableStack {
 public:
  ...
  // The stack is meant to be used by calling SeekToProp with increasing
  // property numbers.
  ...
  // Derived classes must implement this. The server gets one and the client
  // gets one. It calls the proxy to move to the next datatable's data.
  virtual void RecurseAndCallProxies(CSendNode * pNode,
                                     unsigned char *pStructBase) = 0;

 public:
  CSendTablePrecalc *m_pPrecalc;

  enum { MAX_PROXY_RESULTS = 64 };

  // These point at the various values that the proxies returned. They are setup
  // once, then the properties index them.
...
// It is faster to use this route if you only are going to ask for a couple
// props. If you're going to ask for all the props, then you shouldn't use the
// "explicit" route.
...
inline unsigned char *UpdateRoutesExplicit_Template(DTStack *pStack,
  ...
  for (unsigned short i = 0; i < proxyPath.m_nEntries; i++) {
    ...
    if (pStack->m_pProxies[iProxy] == (unsigned char *)0xFFFFFFFF) {
      pStack->m_pProxies[iProxy] =
          ProxyCaller::CallProxy(pStack, pStructBase, pEntry->m_iDatatableProp);
...
class CClientDatatableStack : public CDatatableStack {
 public:
  CClientDatatableStack(CRecvDecoder *pDecoder, unsigned char *pStructBase,
                        int objectID)
  ...
  inline unsigned char *CallPropProxy(CSendNode *pNode, int iProp,
    ...
    pProp->GetDataTableProxyFn()(pProp, &pVal, pStructBase + pProp->GetOffset(),
                                 GetObjectID());
   ...
   public:
    static inline unsigned char *CallProxy(CClientDatatableStack *pStack,
      ...
      pProp->GetDataTableProxyFn()(
          pProp, &pVal, pStructBase + pProp->GetOffset(), pStack->m_ObjectID);
...
class CServerDatatableStack : public CDatatableStack {
 public:
  CServerDatatableStack(CSendTablePrecalc *pPrecalc, unsigned char *pStructBase,
                        int objectID)
  ...
  inline unsigned char *CallPropProxy(CSendNode *pNode, int iProp,
    ...
    if (m_pRecipients &&
        pNode->GetDataTableProxyIndex() != DATATABLE_PROXY_INDEX_NOPROXY) {
    ...
    unsigned char *pRet = (unsigned char *)pProp->GetDataTableProxyFn()(
        pProp, pStructBase, pStructBase + pProp->GetOffset(), pRecipients,
        GetObjectID());
   ...
   public:
    static inline unsigned char *CallProxy(CServerDatatableStack *pStack,
      ...
      return (unsigned char *)pProp->GetDataTableProxyFn()(
          pProp, pStructBase, pStructBase + pProp->GetOffset(), &s_Recipients,
          pStack->GetObjectID());
    }
  };
