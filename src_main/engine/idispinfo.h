...
enum { DISP_DECAL_HANDLE_INVALID = (DispDecalHandle_t)~0 };

typedef unsigned short DispShadowHandle_t;

enum { DISP_SHADOW_HANDLE_INVALID = (DispShadowHandle_t)~0 };
...
abstract_class IDispInfo {
 public:
  ...
  virtual void GetIntersectingSurfaces(GetIntersectingSurfaces_Struct *
                                       pStruct) = 0;
  ...
  virtual void AddDynamicLights(struct dlight_t * pLights,
  ...
  // flSize is like the radius of the decal so the decal isn't put on any disp
  // faces it's too far away from.
  ...
  // Compute shadow fragments for a particular shadow, return the vertex + index
  ...
  virtual bool TestRay(Ray_t const& ray, float start, float end, float& dist,
                       Vector2D* lightmapUV, Vector2D* textureUV) = 0;

  // Computes the texture + lightmap coordinate given a displacement uv
  virtual void ComputeLightmapAndTextureCoordinate(
      RayDispOutput_t const& uv, Vector2D* luv, Vector2D* tuv) = 0;
};

// -----------------------------------------------------------------------------
// // Adds shadow rendering data to a particular mesh builder The function will
...
// This should be called from Map_LoadDisplacements (while the map file is
// open). It loads the displacement data from the file and prepares the
// displacements for rendering.
//
