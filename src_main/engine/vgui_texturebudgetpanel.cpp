...
static void TextureCVarChangedCallBack(IConVar *pConVar, const char *pOldString,
                                       float flOldValue);

ConVar texture_budget_panel_global(
    "texture_budget_panel_global", "0", 0,
    "Show global times in the texture budget panel.");
ConVar showbudget_texture("showbudget_texture", "0", FCVAR_CHEAT,
...
void showbudget_texture_off_f() { showbudget_texture.SetValue(0); }
ConCommand showbudget_texture_on("+showbudget_texture", showbudget_texture_on_f,
                                 "", FCVAR_CHEAT);
ConCommand showbudget_texture_off("-showbudget_texture",
                                  showbudget_texture_off_f, "", FCVAR_CHEAT);
...
void showbudget_texture_global_off_f() { showbudget_texture.SetValue(0); }
ConCommand showbudget_texture_global_on("+showbudget_texture_global",
                                        showbudget_texture_global_on_f, "",
                                        FCVAR_CHEAT);
ConCommand showbudget_texture_global_off("-showbudget_texture_global",
                                         showbudget_texture_global_off_f, "",
                                         FCVAR_CHEAT);

ConVar texture_budget_panel_x("texture_budget_panel_x", "0", FCVAR_ARCHIVE,
                              "number of pixels from the left side of the game "
                              "screen to draw the budget panel",
                              TextureCVarChangedCallBack);
ConVar texture_budget_panel_y("texture_budget_panel_y", "450", FCVAR_ARCHIVE,
                              "number of pixels from the top side of the game "
                              "screen to draw the budget panel",
                              TextureCVarChangedCallBack);
ConVar texture_budget_panel_width("texture_budget_panel_width", "512",
                                  FCVAR_ARCHIVE,
                                  "width in pixels of the budget panel",
                                  TextureCVarChangedCallBack);
ConVar texture_budget_panel_height("texture_budget_panel_height", "284",
                                   FCVAR_ARCHIVE,
                                   "height in pixels of the budget panel",
                                   TextureCVarChangedCallBack);
ConVar texture_budget_panel_bottom_of_history_fraction(
    "texture_budget_panel_bottom_of_history_fraction", ".25", FCVAR_ARCHIVE,
    "number between 0 and 1", TextureCVarChangedCallBack);

ConVar texture_budget_background_alpha("texture_budget_background_alpha", "128",
                                       FCVAR_ARCHIVE,
...
CTextureBudgetPanel::CTextureBudgetPanel(vgui::Panel *pParent,
...
void CTextureBudgetPanel::SendConfigDataToBase() {
  ...
  for (int i = 0; i < g_VProfCurrentProfile.GetNumCounters(); i++) {
    if (g_VProfCurrentProfile.GetCounterGroup(i) == GetCurrentCounterGroup()) {
      ...
      int r, g, b, a;
      g_VProfCurrentProfile.GetBudgetGroupColor(data.m_BudgetGroupInfo.Count(),
                                                r, g, b, a);
  ...
  data.m_flBottomOfHistoryFraction =
      texture_budget_panel_bottom_of_history_fraction.GetFloat();

  data.m_flBarGraphRange = (m_MaxValue * 4) / 3.0f;
  data.m_flTimeLabelInterval = data.m_flBarGraphRange / 4;
  data.m_nLinesPerTimeLabel = 4;

  data.m_flHistoryRange = (m_SumOfValues * 4) / 3.0f;
...
void CTextureBudgetPanel::PerformLayout() {
  ...
  if (texture_budget_panel_global.GetInt()) pStr = "Global texture stats";

  m_pModeLabel->SetText(pStr);
  int width =
      g_pMatSystemSurface->DrawTextLen(m_pModeLabel->GetFont(), "%s", pStr);
  ...
  m_pModeLabel->SetBgColor(
      Color(0, 0, 0, texture_budget_background_alpha.GetInt()));
}

void CTextureBudgetPanel::OnCVarStateChanged() { SendConfigDataToBase(); }
...
void CTextureBudgetPanel::SnapshotTextureHistory() {
  ...
  if ((float)m_MaxValue > GetConfigData().m_flBarGraphRange ||
...
void CTextureBudgetPanel::SetTimeLabelText() {
  for (int i = 0; i < m_TimeLabels.Count(); i++) {
    char text[512];
    Q_snprintf(text, sizeof(text), "%.1fM",
               (float)(i * GetConfigData().m_flTimeLabelInterval) / 1024);
...
void CTextureBudgetPanel::SetHistoryLabelText() {
  for (int i = 0; i < m_HistoryLabels.Count(); i++) {
    char text[512];
    Q_snprintf(text, sizeof(text), "%.1fM",
               GetConfigData().m_HistoryLabelValues[i] / 1024);
...
void CTextureBudgetPanel::DumpGlobalTextureStats(const CCommand &args) {
  ...
  // Copy the budget group names in.
  for (int i = 0; i < g_VProfCurrentProfile.GetNumCounters(); i++) {
    if (g_VProfCurrentProfile.GetCounterGroup(i) ==
      ...
      Warning("%s: %d\n", pGroupName,
              (int)g_VProfCurrentProfile.GetCounterValue(i));
    }
  }
}
