...
void CL_Predict(int world_start_state, bool validframe, int start_command,
  ...
  g_pClientSidePrediction->Update(world_start_state, validframe, start_command,
                                  stop_command);
}
...
void CL_RunPrediction(PREDICTION_REASON reason) {
  ...
  // Msg( "%i pred reason %s\n", host_framecount, reason ==
  // PREDICTION_SIMULATION_RESULTS_ARRIVING_ON_SEND_FRAME ? 	"same frame" :
  //"normal" );

  bool valid =
      cl.m_nDeltaTick > 0;  // cl.GetReceiveList( cl.delta_tick ) != NULL;
  ...
  CL_Predict(cl.m_nDeltaTick, valid, cl.last_command_ack,
