...
void CEngineSoundServer::EmitSoundInternal(
    ...
    CUtlVector<Vector> *pUtlVecOrigins, bool bUpdatePositions,
    float soundtime /*= 0.0f*/, int speakerentity /*=-1*/) {
  AssertMsg(pDirection == NULL,
            "Direction specification not currently supported on server sounds");
  AssertMsg(bUpdatePositions,
            "Non-updated positions not currently supported on server sounds");
  ...
  SV_StartSound(filter, pEdict, iChannel, pSample, flVolume, iSoundLevel,
                iFlags, iPitch, pOrigin, soundtime, speakerentity,
                pUtlVecOrigins);
}
...
void BuildRecipientList(CUtlVector<edict_t *> &list,
                        const IRecipientFilter &filter) {
  int c = filter.GetRecipientCount();
  for (int i = 0; i < c; i++) {
    int playerindex = filter.GetRecipientIndex(i);
    if (playerindex < 1 || playerindex > sv.GetClientCount()) continue;
...
void CEngineSoundServer::SetPlayerDSP(IRecipientFilter &filter, int dspType,
                                      bool fastReset) {
  Assert(!fastReset);
  if (fastReset) {
...
float CEngineSoundServer::GetDistGainFromSoundLevel(soundlevel_t soundlevel,
                                                    float dist) {
