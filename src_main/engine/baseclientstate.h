...
the_interface CBaseClientState : public INetChannelHandler,
 ...
 public:
  ...
  int m_nServerCount;  // server identification for prespawns, must match the
                       // svs.spawncount which
  // is incremented on server spawning.  This supercedes svs.spawn_issued, in
  // that we can now spend a fair amount of time sitting connected to the server
  // but downloading models, sounds, etc.  So much time that it is possible that
  // the server might change levels again and, if so, we need to know that.
  int m_nCurrentSequence;  // this is the sequence number of the current
                           // incoming packet
  ...
  // This stuff manages the receiving of data tables and instantiating of client
  // versions of server-side classes.
  ...
  bool m_bRestrictServerCommands;  // If true, then the server is only allowed
                                   // to execute commands marked with
                                   // FCVAR_SERVER_CAN_EXECUTE on the client.
  bool m_bRestrictClientCommands;  // If true, then IVEngineClient::ClientCmd is
                                   // only allowed to execute commands marked
                                   // with FCVAR_CLIENTCMD_CAN_EXECUTE on the
                                   // client.
};
