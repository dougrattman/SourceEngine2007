...
class CClientFrame {
 public:
  ...
  // Accessors to snapshots. The data is protected because the snapshots are
  // reference-counted.
 ...
 public:
  ...
  CBitVec<MAX_EDICTS>
      transmit_entity;  // if bit n is set, entity n will be send to client
  CBitVec<MAX_EDICTS> *from_baseline;  // if bit n is set, this entity was send
                                       // as update from baseline
  CBitVec<MAX_EDICTS> *transmit_always;  // if bit is set, don't do PVS checks
                                         // before sending (HLTV only)

  CClientFrame *m_pNext;

 private:
  // Index of snapshot entry that stores the entities that were active and the
  // serial numbers for the frame number this packed entity corresponds to
  // m_pSnapshot MUST be private to force using SetSnapshot(), see reference
  // counters
  CFrameSnapshot *m_pSnapshot;
};

// TODO substitute CClientFrameManager with an intelligent structure (Tree,
// hash, cache, etc)
class CClientFrameManager {
 public:
  CClientFrameManager(void)
      : m_ClientFramePool(MAX_CLIENT_FRAMES, CMemoryPool::GROW_SLOW) {
    m_Frames = NULL;
  }
  virtual ~CClientFrameManager() { DeleteClientFrames(-1); }
