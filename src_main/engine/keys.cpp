...
//-----------------------------------------------------------------------------
static inline bool ShouldPassKeyUpToTarget(ButtonCode_t code,
                                           KeyUpTarget_t target) {
  return (s_pKeyInfo[code].m_nKeyUpTarget == target) ||
...
void Key_SetBinding(ButtonCode_t keynum, const char *pBinding) {
  ...
  if (keynum == BUTTON_CODE_INVALID) return;
...
CON_COMMAND(unbindall, "Unbind all keys.") {
  ...
  for (i = 0; i < BUTTON_CODE_LAST; i++) {
    if (!s_pKeyInfo[i].m_pKeyBinding) continue;
...
CON_COMMAND(bind, "Bind a key.") {
  ...
  ButtonCode_t b = g_pInputSystem->StringToButtonCode(args[1]);
  if (b == BUTTON_CODE_INVALID) {
    ConMsg("\"%s\" isn't a valid key\n", args[1]);
    return;
  }
...
const char *Key_BindingForKey(ButtonCode_t code) {
  if (code < 0 || code > BUTTON_CODE_LAST) return NULL;
...
static bool FilterTrappedKey(ButtonCode_t code, bool bDown) {
  // After we've trapped a key, we want to capture the button up message for
  // that key
...
static bool HandleToolKey(const InputEvent_t &event) {
  ...
  return toolsys && toolsys->TrapKey((ButtonCode_t)event.m_nData,
...
static bool HandleClientKey(const InputEvent_t &event) {
  ...
  if (g_ClientDLL &&
      g_ClientDLL->IN_KeyEvent(bDown ? 1 : 0, code,
                               s_pKeyInfo[code].m_pKeyBinding) == 0)
    return true;
...
static bool HandleEngineKey(const InputEvent_t &event) {
  ...
  // Swallow console toggle if any modifier keys are down if it's bound to
  // toggleconsole (the default)
  if (!Q_stricmp(kb, "toggleconsole")) {
    if (s_pKeyInfo[KEY_LALT].m_bKeyDown || s_pKeyInfo[KEY_LSHIFT].m_bKeyDown ||
        s_pKeyInfo[KEY_LCONTROL].m_bKeyDown ||
        s_pKeyInfo[KEY_RALT].m_bKeyDown || s_pKeyInfo[KEY_RSHIFT].m_bKeyDown ||
        s_pKeyInfo[KEY_RCONTROL].m_bKeyDown)
      return false;
  }
...
static bool FilterKey(const InputEvent_t &event, KeyUpTarget_t target,
...
void Key_Event(const InputEvent_t &event) {
  ...
  if (s_pKeyInfo[code].m_bKeyDown == bDown) return;

  s_pKeyInfo[code].m_bKeyDown = bDown;

  bool bCtrlDown = s_pKeyInfo[KEY_LCONTROL].m_bKeyDown ||
                   s_pKeyInfo[KEY_RCONTROL].m_bKeyDown;
  bool bAltDown =
      s_pKeyInfo[KEY_LALT].m_bKeyDown || s_pKeyInfo[KEY_RALT].m_bKeyDown;
  ...
  // Keep vgui's notion of which keys are down up-to-date regardless of
  // filtering Necessary because vgui has multiple input contexts, so vgui can't
  // directly ask the input system for this information.
