 ...
 *  and for code clarity (the download can be a linear sequence of blocking
 * calls).
 *
 ...
 *   3. The CL_HTTPUpdate() function is called every frame, and it does the
 * following: a) Starts a download thread if none are active, and at least one
 * is queued up. b) Checks on status of the current transfer and updates the
 * progress bar. i. If the thread has finished, the file is written out to disk
 * and another transfer is started. ii. If the thread has aborted/errored out,
 * any partial data is written to the cache, the progress bar is removed, and an
 * error message is shown, if neccessary. c) Checks for old transfers to clean
 * up where the download thread has exited.
 *   4. If the user hits Cancel on the progress bar, the current download is
 * told to abort, and the progress bar is removed.
 *
 ...
 *      uint8_t range for partial transfers (resuming an aborted transfer).  A
 * buffer for the data is allocated in this thread.
 *   4. while the main thread has not requested an abort, read data from the
 * server.
 *   5. Upon completion, abort, or an error, the thread does the following:
 *      a) close any open handles
 *      b) wait for the main thread to request an exit (so the main thread can
 * read any data.) c) delete the data buffer (previously allocated in this
 * thread) d) sets a flag letting the main thread know this thread is finished.
 *      e) exit
 *
 * -------------------
 * Thread interaction:
 *  All thread interaction is handled via a shared RequestContext.  Interaction
 * is structured so that at all times, only one thread can be writing to any
 * given variable.
 *
 ...
 *         i.   for HTTP_CONNECTING, nothing is read, and only shouldStop can be
 * set. ii.  for HTTP_FETCH, nBytesTotal and nBytesCurrent are read, and
 *              shouldStop can be set.
 *         iii. for HTTP_DONE, nBytesTotal, nBytesCurrent, and data are read.
 * Also, shouldStop can be set. iv.  for HTTP_ABORTED, nothing is read, and only
 * shouldStop can be set. v.   for HTTP_ERROR, nBytesTotal, nBytesCurrent, and
 * data are read.  Also, shouldStop can be set.
 *   3. After shouldStop is set by main thread:
 *      a) if the download thread is in status HTTP_FETCH, it will cease
 * operations and set status to HTTP_ABORTED. b) download thread will delete
 * data, if it exists. c) download thread will set threadDone. d) after the main
 * thread set shouldStop, it will only look at threadDone.
 *   4. After threadDone is set by download thread:
 *      a) download thread can safely exit, and will not access the
 * RequestContext. b) main thread can delete the cacheData, if any exists, and
 * delete the RequestContext itself.  Thus ends the download.
 *   5. SPECIAL CASE: if the user hits Cancel during a download, the main thread
 * will look at nBytesTotal and nBytesCurrent to determine if there is any data
 *      present, and read from data if there is.  The download thread will most
 * likely be in HTTP_CONNECTING or HTTP_FETCH, but could also be in HTTP_DONE or
...
typedef struct {
   ...
   * Allocated/deleted by the main thread (deleted after download thread is
   * done)
   */
