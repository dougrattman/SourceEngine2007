...
#define MAX_RETESSELATE \
  MAX_MAP_DISPINFO  // Max number of displacements that can be retesselated per
                    // frame.

#define MAX_TOTAL_DISP_DEPENDENCIES \
  (4 * 2 + 4 * MAX_DISP_CORNER_NEIGHBORS + MAX_EXTRA_DEPENDENCIES)
...
// These classes store groups of meshes which can be drawn with one
// IMesh::Draw() call.
class CGroupMesh {
 public:
  IMesh *m_pMesh;
...
class CDispDecalBase {
 public:
  ...
  // Precalculated flags on the nodes telling which nodes this decal can
  // intersect. See CPowerInfo::m_NodeIndexIncrements for a description of how
  // the node tree is walked using this bit vector.
...
// This position is about OVERLAY_AVOID_FLICKER_NORMAL_OFFSET inches off the
// surface and is used by overlays and shadows to avoid z-fighting.
inline Vector GetOverlayPos(CMeshReader *pReader, int iVertIndex) {
  Vector vNormal;
  pReader->Normal(iVertIndex, vNormal);
  return pReader->Position(iVertIndex) +
...
extern CUtlVector<unsigned char, CHunkMemory<unsigned char> >
