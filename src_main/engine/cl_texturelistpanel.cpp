...
static ConVar mat_texture_list(
    "mat_texture_list", "0", 0,
    "For debugging, show a list of used textures per frame");

enum TxListPanelRequest { TXR_NONE, TXR_SHOW, TXR_RUNNING, TXR_HIDE };

static TxListPanelRequest s_eTxListPanelRequest = TXR_NONE;

// These are here so you can bind a key to +mat_texture_list and toggle it on
// and off.
...
ConVar mat_texture_list_all("mat_texture_list_all", "0", FCVAR_NEVER_AS_STRING,
                            "If this is nonzero, then the texture list panel "
                            "will show all currently-loaded textures.");

ConVar mat_texture_list_view(
    "mat_texture_list_view", "1", FCVAR_NEVER_AS_STRING,
    "If this is nonzero, then the texture list panel will render thumbnails of "
    "currently-loaded textures.");

ConVar mat_show_texture_memory_usage(
    "mat_show_texture_memory_usage", "0", FCVAR_NEVER_AS_STRING | FCVAR_CHEAT,
...
class TileViewPanelEx : public Panel {
 ...
 public:
  enum HitTest_t { HT_NOTHING = 0, HT_TILE };
 ...
 protected:
  ...
  int m_li_colVisible,
      m_li_rowVisible;  //!< Num columns/rows fitting in the client area
  int m_li_rowNeeded;   //!< Num rows required to fit all numTiles
  int m_li_startTile,
      m_li_endTile;  //!< Start/end tile that are rendered in the client area
...
TileViewPanelEx::~TileViewPanelEx() { delete m_hbar; }
...
HFont TileViewPanelEx::GetFont() { return m_hFont; }

int TileViewPanelEx::HitTest(int x, int y, int &iTile) {
  iTile = -1;

  if (!ComputeLayoutInfo()) return HT_NOTHING;

  int hitCol = (x / m_li_wideItem);
  int hitRow = (y / m_li_tallItem);

  if (hitCol >= m_li_colVisible) return HT_NOTHING;
  if (hitRow > m_li_rowVisible) return HT_NOTHING;

  int hitTile = m_li_startTile + hitCol + hitRow * m_li_colVisible;
  if (hitTile >= m_li_endTile) return HT_NOTHING;
...
bool TileViewPanelEx::GetTileOrg(int iTile, int &x, int &y) {
  if (m_li_colVisible <= 0) return false;
  if (iTile < m_li_startTile || iTile >= m_li_endTile) return false;
...
void TileViewPanelEx::PerformLayout() {
  ...
  if (!wideItem || !tallItem) return;

  int colVisible = wide / wideItem;
  int rowVisible = tall / tallItem;
  if (rowVisible <= 0 || colVisible <= 0) return;
...
bool TileViewPanelEx::ComputeLayoutInfo() {
  ...
  if (!m_li_wideItem || !m_li_tallItem) return false;

  m_li_colVisible = m_li_wide / m_li_wideItem;
  m_li_rowVisible = m_li_tall / m_li_tallItem;
  if (m_li_rowVisible <= 0 || m_li_colVisible <= 0) return false;
  ...
  if (m_li_startTile < 0) m_li_startTile = 0;

  m_li_endTile = m_li_startTile + m_li_numVisibleTiles +
                 m_li_colVisible;  // draw the partly visible items
  if (m_li_endTile > m_li_numTiles) m_li_endTile = m_li_numTiles;
...
void TileViewPanelEx::Paint() {
  ...
  if (!ComputeLayoutInfo()) return;

  for (int iRenderTile = m_li_startTile; iRenderTile < m_li_endTile;
       ++iRenderTile) {
    int x =
        0 + ((iRenderTile - m_li_startTile) % m_li_colVisible) * m_li_wideItem;
    int y =
        0 + ((iRenderTile - m_li_startTile) / m_li_colVisible) * m_li_tallItem;
...
CAutoMatSysDebugMode::CAutoMatSysDebugMode()
  ...
  bOldDebugMode =
      g_pMaterialSystemDebugTextureInfo->SetDebugTextureRendering(true);
}
...
CAutoMatSysDebugMode::~CAutoMatSysDebugMode() {
  ...
  for (uint32_t k = 0; k < arrCleanupVars.Count(); ++k) {
...
static IMaterial *UseDebugMaterial(char const *szMaterial,
  ...
  IMaterial *pMaterial =
      materials->FindMaterial(szMaterial, TEXTURE_GROUP_OTHER, false);
  // IMaterial *pMaterial = materials->FindMaterial( "debug/debugempty",
  // TEXTURE_GROUP_OTHER, false );
  if (!pMaterial) return NULL;

  IMaterialVar *BaseTextureVar =
      pMaterial->FindVar("$basetexture", &foundVar, false);
  // IMaterialVar *BaseTextureVar = pMaterial->FindVar( "$basetexture",
  // &foundVar, false );
  if (!foundVar || !BaseTextureVar) return NULL;
  ...
  if (foundVar && FrameVar) {
    ...
    if (pMatTexture->IsRenderTarget() || pMatTexture->IsProcedural())
      numAnimFrames =
          0;  // Cannot use the stencil parts of the render targets and
              // shouldn't use the other frames of procedural textures

    FrameVar->SetIntValue(
        (numAnimFrames > 0)
            ? (g_ClientGlobalVariables.tickcount % numAnimFrames)
            : 0);
  }
...
static void RenderTexturedRect(vgui::Panel *pPanel, IMaterial *pMaterial, int x,
                               int y, int x1, int y1, int xoff = 0,
                               int yoff = 0) {
...
static bool ShallWarnTx(KeyValues *kv, ITexture *tx) {
  ...
  if (kv->GetInt(KEYNAME_WIDTH) > g_warn_texdimensions) return true;

  if (kv->GetInt(KEYNAME_HEIGHT) > g_warn_texdimensions) return true;
...
bool CanAdjustTextureSize(char const *szTextureName, bool bMoveSizeUp) {
  ...
  if (!bMoveSizeUp) {
    return (pMatTexture->GetActualWidth() > 4 ||
            pMatTexture->GetActualHeight() > 4);
  } else {
...
CON_COMMAND_F(mat_texture_list_txlod,
              "Adjust LOD of the last viewed texture +1 to inc resolution, -1 "
              "to dec resolution",
              FCVAR_DONTRECORD) {
  if (args.ArgC() == 2) {
    ...
    switch (iAdjustment) {
      ...
      case -1:
        ...
        else if (!AdjustTextureSize(s_chLastViewedTextureBuffer,
                                    iAdjustment > 0))
  ...
  Warning(
      "Usage: 'mat_texture_list_txlod +1' to inc lod | 'mat_texture_list_txlod "
      "-1' to dec lod\n");
}
...
struct ViewParamsLast {
  ViewParamsLast()
      : lstMaterials(DefLessFunc(UtlSymId_t)),
        flTime(0.f),
        bHighlighted(false) {}
  ...
  struct VarMap : public CUtlMap<UtlSymId_t, TxInfo> {
    ...
    VarMap(VarMap const &x) {
      const_cast<VarMap &>(x).Swap(*this);
      m_matInfo = x.m_matInfo;
    }  // Fast-swap data
    ...
    if (pMat) {
      ...
      for (int idxParam = 0; idxParam < numParams; ++idxParam) {
        ...
        if (idxVarsTxInfo != vars.InvalidIndex()) {
          ...
          ITexture *pTx = (s_viewParamsLast.bHighlighted && !bRemovedSelection)
                              ? txEmpty
                              : ti.pTx;
...
void CVmtTextEntry::OpenEditMenu() {
  ...
  int pos = pEditMenu->AddMenuItem("Open VMT",
                                   new KeyValues("DoOpenVmtSelected"), this);
...
CRenderTextureEditor::CRenderTextureEditor(vgui::Panel *parent,
  ...
  m_pExplore = vgui::SETUP_PANEL(
      new vgui::Button(this, "Explore", "Open", this, "Explore"));
  m_pExplore->SetVisible(true);

  m_pReload = vgui::SETUP_PANEL(
      new vgui::Button(this, "Reload", "Reload", this, "Reload"));
  m_pReload->SetVisible(true);

  m_pCopyTxt = vgui::SETUP_PANEL(
      new vgui::Button(this, "CopyTxt", "Copy Text", this, "CopyTxt"));
  m_pCopyTxt->SetVisible(true);

  m_pCopyImg = vgui::SETUP_PANEL(
      new vgui::Button(this, "CopyImg", "Copy Image", this, "CopyImg"));
  m_pCopyImg->SetVisible(true);

  m_pFlashBtn = vgui::SETUP_PANEL(
      new vgui::Button(this, "FlashBtn", "Flash in Game", this, "FlashBtn"));
  m_pFlashBtn->SetVisible(true);

  m_pSizeControls[0] =
      vgui::SETUP_PANEL(new vgui::Button(this, "--", "--", this, "size-"));
  m_pSizeControls[1] =
      vgui::SETUP_PANEL(new vgui::Button(this, "+", "+", this, "size+"));
}

CRenderTextureEditor::~CRenderTextureEditor() { SetDispInfo(NULL, 0); }
...
void CRenderTextureEditor::Activate() { BaseClass::Activate(); }
...
void CRenderTextureEditor::PerformLayout() {
  ...
  SetSize(4 * TILE_BORDER + TILE_SIZE, iRenderedHeight + 90 + TILE_BORDER);
  ...
  if (m_pInfo) {
    ...
    if (!m_pInfo->GetInt("SpecialTx")) {
      ...
      m_pSizeControls[0]->SetEnabled(
          CanAdjustTextureSize(m_pInfo->GetString(KEYNAME_NAME), false));
      m_pSizeControls[1]->SetEnabled(
          CanAdjustTextureSize(m_pInfo->GetString(KEYNAME_NAME), true));
      ...
      m_pSizeControls[1]->SetPos(posX + m_pSizeControls[0]->GetWide() + 1,
                                 posY + m_pExplore->GetTall() + 1);
      m_pSizeControls[1]->SetWide(m_pExplore->GetWide() -
                                  (m_pSizeControls[0]->GetWide() + 1));
    }
  }
...
void CRenderTextureEditor::OnCommand(const char *command) {
  ...
  if (!stricmp(command, "CopyImg")) {
    ...
    vgui::system()->SetClipboardImage(pMainWnd, x, y, x + GetWide(),
                                      y + GetTall());
  }
  ...
  if ((!stricmp(command, "size-") || !stricmp(command, "size+")) && m_pInfo) {
    ...
    if (bResult) {
      CAutoPushPop<bool> auto_g_bRecursiveRequestToShowTextureList(
...
void CRenderTextureEditor::OnMousePressed(vgui::MouseCode code) { Close(); }
...
void CRenderTextureEditor::Paint() {
  ...
  //
  {
    ...
    g_pMatSystemSurface->DrawOutlinedRect(x + 1, y + 1, x + tileWidth - 2,
                                          y + tileHeight - 2);
  }
  ...
  g_pMatSystemSurface->DrawColoredTextRect(
      GetFont(), x, y, TILE_SIZE, TILE_TEXT / 2, 255, 255, 255, 255,
      "%s%s\n"
      "%s Kb    %dx%d    %s",
      szPrintFilePrefix, szPrintFileName, chSizeBuf, iTxWidth, iTxHeight,
      szTxFormat);
  ...
  if (!kv->GetInt("SpecialTx")) {
    ...
    {
      int wmap = pMatTexture->GetMappingWidth(),
          hmap = pMatTexture->GetMappingHeight(),
          dmap = pMatTexture->GetMappingDepth();
      int wact = pMatTexture->GetActualWidth(),
          hact = pMatTexture->GetActualHeight(),
          dact = pMatTexture->GetActualDepth();
    ...
    if (chLine1[0]) {
      ...
      g_pMatSystemSurface->DrawFilledRect(
          x - TILE_BORDER / 2, y + TILE_TEXT / 2,
          x + TILE_BORDER / 2 + TILE_SIZE, y + TILE_TEXT / 2 + TILE_TEXT / 4);
      g_pMatSystemSurface->DrawColoredTextRect(GetFont(), x, y + TILE_TEXT / 2,
                                               TILE_SIZE, TILE_TEXT / 4, 255,
                                               255, 255, 255, "%s", chLine1);
    }
    if (chLine2[0]) {
      // g_pMatSystemSurface->DrawSetColor( 200, 0, 0, 255 );
      // g_pMatSystemSurface->DrawFilledRect( x - TILE_BORDER/2, y + TILE_TEXT/2
      // + TILE_TEXT/4, x + TILE_BORDER/2 + TILE_SIZE, y + TILE_TEXT );
      g_pMatSystemSurface->DrawColoredTextRect(
          GetFont(), x, y + TILE_TEXT / 2 + TILE_TEXT / 4, TILE_SIZE,
          TILE_TEXT / 4, 255, 255, 255, 255, "%s", chLine2);
    }
  }
  ...
  if (IMaterial *pMaterial = UseDebugMaterial(
          "debug/debugtexturecolor", pMatTexture, &auto_matsysdebugmode)) {
    g_pMatSystemSurface->DrawSetColor(255, 255, 255, 255);
    ...
    if (bHasAlpha) {
      ...
      orgTxY += orgTxYA;
      if (IMaterial *pMaterial = UseDebugMaterial(
              "debug/debugtexturealpha", pMatTexture, &auto_matsysdebugmode)) {
        g_pMatSystemSurface->DrawOutlinedRect(
            x + orgTxX + (extTxWidth - iDrawWidth) / 2 - IMG_FRAME_OFF,
            y + orgTxY + (extTxHeight - iDrawHeight) / 2 - IMG_FRAME_OFF,
            x + orgTxX + (extTxWidth + iDrawWidth) / 2 + IMG_FRAME_OFF,
    ...
    g_pMatSystemSurface->DrawFilledRect(
        x + orgTxX + (extTxWidth - iDrawWidth) / 2,
        y + orgTxY + (extTxWidth - iDrawHeight) / 2,
        x + orgTxX + (extTxWidth + iDrawWidth) / 2,
        y + orgTxY + (extTxWidth + iDrawHeight) / 2);
  }
...
void CRenderTexturesListViewPanel::OnMousePressed(vgui::MouseCode code) {
  ...
  if (HT_NOTHING == htResult) return;
  if (!GetTileOrg(iTile, tileX, tileY)) return;
...
void CRenderTexturesListViewPanel::RenderTile(int iTile, int x, int y) {
  ...
  fmtlenreduce:
    switch (szTxFormat[iTxFormatLen - 1]) {
      case '8': {
        while ((iTxFormatLen > 4) && (szTxFormat[iTxFormatLen - 2] == '8'))
          --iTxFormatLen;
      } break;
      case '6': {
        while ((iTxFormatLen > 4) && (szTxFormat[iTxFormatLen - 2] == '1') &&
  ...
  bool bWarnTile = (!kv->GetInt("SpecialTx")) &&
                   (g_warn_enable && ShallWarnTx(kv, pMatTexture));
  ...
  int iTextHeight = g_pMatSystemSurface->GetFontTall(GetFont());
  {
    ...
    while (chInfoText[iTextLen[1]] != ' ') ++iTextLen[1];
    ++iTextLen[1];
    iTextLen[2] = 2 + iTextLen[1] + iTxFormatLen + strlen(szTxFormatSuffix);
    iTextLen[3] = strlen(chInfoText);
    for (int k = 0; k < 4; ++k)
      iTextMargins[k] = g_pMatSystemSurface->DrawTextLen(
          GetFont(), "%.*s", iTextLen[k], chInfoText);
  }
  ...
  if (bWarnTile) {
    ...
    if (iTxSize > g_warn_texkbytes)
      g_pMatSystemSurface->DrawFilledRect(
          x - 2, y + iTextHeight + 1, x + iTextMargins[0] - 5, y + TILE_TEXT);
    if (iTxWidth > g_warn_texdimensions || iTxHeight > g_warn_texdimensions)
      g_pMatSystemSurface->DrawFilledRect(
          x + iTextMargins[0] - 2, y + iTextHeight + 1, x + iTextMargins[1] - 1,
          y + TILE_TEXT);
    if (strcmp(szTxFormat, "DXT1") && strcmp(szTxFormat, "DXT5"))
      g_pMatSystemSurface->DrawFilledRect(
          x + iTextMargins[1] + 2, y + iTextHeight + 1, x + iTextMargins[2] - 1,
          y + TILE_TEXT);
    if (pMatTexture->GetFlags() &
        (TEXTUREFLAGS_NOLOD | TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_ONEBITALPHA))
      g_pMatSystemSurface->DrawFilledRect(
          x + iTextMargins[2] + 3, y + iTextHeight + 1, x + iTextMargins[3] + 2,
          y + TILE_TEXT);
  }

  g_pMatSystemSurface->DrawColoredTextRect(
      GetFont(), x, y, TILE_SIZE, TILE_TEXT, 255, 255, 255, 255,
      "%s%s\n"
      "%s",
      szPrintFilePrefix, szPrintFileName, chInfoText);
  ...
  if (IMaterial *pMaterial = UseDebugMaterial(
    ...
    RenderTexturedRect(this, pMaterial,
                       ...
                       y + orgTxY + (extTxHeight - iDrawHeight) / 2,
                       x + orgTxX + (extTxWidth + iDrawWidth) / 2,
                       y + orgTxY + (extTxHeight + iDrawHeight) / 2, 2, 1);
...
static int __cdecl KilobytesSortFunc(vgui::ListPanel *pPanel,
...
class CSmartTextureKeyValues {
 ...
 public:
  CSmartTextureKeyValues() : m_p(NULL) {
    if (KeyValues *p = g_pMaterialSystemDebugTextureInfo->GetDebugTextureList())
      m_p = p->MakeCopy();
  }
  ~CSmartTextureKeyValues() {
    if (m_p) m_p->deleteThis();
    m_p = NULL;
  }
...
CTextureListPanel::CTextureListPanel(vgui::Panel *parent)
    : BaseClass(parent, "CTextureListPanel"), m_numDisplayedSizeKB(0) {
  ...
  // Init the buttons.
  m_pCVarListLabel = vgui::SETUP_PANEL(
      new vgui::Label(this, "m_pCVarListLabel",
                      "cvars: mat_texture_limit, mat_texture_list, mat_picmip, "
                      "mat_texture_list_txlod, mat_texture_list_txlod_sync"));
  m_pCVarListLabel->SetVisible(false);  // m_pCVarListLabel->SetVisible( true );

  m_pTotalUsageLabel =
      vgui::SETUP_PANEL(new vgui::Label(this, "m_pTotalUsageLabel", ""));
  m_pTotalUsageLabel->SetVisible(true);

  m_pSpecialTexs = vgui::SETUP_PANEL(new vgui::CheckButton(
      this, "service", "Render Targets and Special Textures"));
  m_pSpecialTexs->SetVisible(true);
  m_pSpecialTexs->AddActionSignalTarget(this);
  m_pSpecialTexs->SetCommand("service");

  m_pShowTextureMemoryUsageOption = vgui::SETUP_PANEL(new CConVarCheckButton(
      this, "m_pShowTextureMemoryUsageOption", "Show Memory Usage on HUD"));
  m_pShowTextureMemoryUsageOption->SetVisible(true);
  m_pShowTextureMemoryUsageOption->SetConVar(&mat_show_texture_memory_usage);

  m_pAllTextures = vgui::SETUP_PANEL(
      new CConVarCheckButton(this, "m_pAllTextures", "Show ALL textures"));
  ...
  m_pViewTextures = vgui::SETUP_PANEL(new CConVarCheckButton(
      this, "m_pViewTextures", "View textures thumbnails"));
  ...
  m_pCopyToClipboardButton = vgui::SETUP_PANEL(
      new vgui::Button(this, "CopyToClipboard", "Copy to Clipboard"));
  ...
  m_pCollapse =
      vgui::SETUP_PANEL(new vgui::ToggleButton(this, "Collapse", " "));
  m_pCollapse->AddActionSignalTarget(this);
  m_pCollapse->SetCommand("Collapse");
  m_pCollapse->SetSelected(true);

  m_pAlpha =
      vgui::SETUP_PANEL(new vgui::CheckButton(this, "ShowAlpha", "Alpha"));
  ...
  m_pThumbWarnings =
      vgui::SETUP_PANEL(new vgui::CheckButton(this, "ThumbWarnings", "Warns"));
  ...
  // Filtering
  m_pFilteringChk = vgui::SETUP_PANEL(
      new vgui::CheckButton(this, "FilteringChk", "Filter: "));
  m_pFilteringChk->AddActionSignalTarget(this);
  m_pFilteringChk->SetCommand("FilteringChk");
  m_pFilteringChk->SetSelected(true);

  m_pFilteringText =
      vgui::SETUP_PANEL(new vgui::TextEntry(this, "FilteringTxt"));
  ...
  m_pListPanel->AddColumnHeader(0, KEYNAME_NAME, "Texture Name", 200, 100, 700,
                                vgui::ListPanel::COLUMN_RESIZEWITHWINDOW);
  m_pListPanel->AddColumnHeader(1, KEYNAME_SIZE, "Kilobytes", 50, 50, 50, 0);
  m_pListPanel->AddColumnHeader(2, KEYNAME_TEXTURE_GROUP, "Group", 100, 100,
                                300, 0);
  ...
  m_pListPanel->AddColumnHeader(6, KEYNAME_BINDS_FRAME, "# Binds", 50, 50, 50,
                                0);
  m_pListPanel->AddColumnHeader(7, KEYNAME_BINDS_MAX, "BindsMax", 50, 50, 50,
                                0);
  ...
  m_pViewPanel =
      vgui::SETUP_PANEL(new CRenderTexturesListViewPanel(this, "View Panel"));
...
void CTextureListPanel::PerformLayout() {
  ...
  // m_pCVarListLabel->SetVisible( !bCollapsed );
  m_pCVarListLabel->SetVisible(false);
  ...
  vgui::Panel *buttons[] = {m_pSpecialTexs,  m_pShowTextureMemoryUsageOption,
  ...
  for (int i = 0; i < ARRAYSIZE(buttons); i++) {
    ...
    if (buttons[i] == m_pViewTextures) {
      ...
      m_pThumbWarnings->SetPos(x + accumw + 5,
                               yOffset - m_pViewTextures->GetTall());
    ...
    if (buttons[i] == m_pFilteringChk) {
      ...
      m_pFilteringText->SetPos(x + accumw + 5,
                               yOffset - m_pFilteringChk->GetTall());
...
void CTextureListPanel::UpdateTotalUsageLabel() {
  ...
  FmtCommaNumber(kb1, (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(
                       ...
                       511) /
                          1024);
  FmtCommaNumber(kb2, (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(
  ...
  if (bool bCollapsed = m_pCollapse->IsSelected()) {
    ...
    FmtCommaNumber(kbMip1,
                   (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(
                    ...
                    511) /
                       1024);
    FmtCommaNumber(kbMip2,
                   (g_pMaterialSystemDebugTextureInfo->GetTextureMemoryUsed(
                        IDebugTextureInfo::MEMORY_ESTIMATE_PICMIP_2) +
                    511) /
                       1024);
    Q_snprintf(data, sizeof(data),
               "%s:  frame %s Kb  /  total %s Kb ( picmip1 = %s Kb, picmip2 = "
               "%s Kb )  /  shown %s Kb",
               szTitle, kb1, kb2, kbMip1, kbMip2, kb3);
  }
...
bool CTextureListPanel::UpdateDisplayedItem(KeyValues *pDispData,
                                            KeyValues *kv) {
  // Update the item?
  bool bUpdate = false;

  // do the stuff that changes often separately.
  if (pDispData->GetInt(KEYNAME_BINDS_FRAME) !=
      kv->GetInt(KEYNAME_BINDS_FRAME)) {
  ...
  if (pDispData->GetInt(KEYNAME_SIZE) != kv->GetInt(KEYNAME_SIZE) ||
      ...
      Q_stricmp(pDispData->GetString(KEYNAME_FORMAT),
                kv->GetString(KEYNAME_FORMAT)) != 0 ||
      Q_stricmp(pDispData->GetString(KEYNAME_TEXTURE_GROUP),
                kv->GetString(KEYNAME_TEXTURE_GROUP)) != 0) {
    ...
    pDispData->SetString(KEYNAME_TEXTURE_GROUP,
...
int CTextureListPanel::AddListItem(KeyValues *kv) {
  ...
  if (iItem == -1) {
    // Set this so the GetItem() call above can use the key's name (as opposed
    // to the value of its "Name" subkey) to find this texture again.
...
void CTextureListPanel::OnTurnedOn() {
  if (g_bRecursiveRequestToShowTextureList) return;

  if (m_pListPanel) m_pListPanel->DeleteAllItems();

  if (CRenderTextureEditor *pRe = m_pViewPanel->GetRenderTxEditor())
    pRe->Close();
}

void CTextureListPanel::Close() { mat_texture_list_off_f(); }

void CTextureListPanel::EndPaint() { UpdateTotalUsageLabel(); }
...
void CTextureListPanel::Paint() {
  ...
  struct EndPaint_t {
    ...
    ~EndPaint_t() { m_p->EndPaint(); }
  } endpaint(this);

  if (!mat_texture_list.GetBool()) return;

  if (!g_pMaterialSystemDebugTextureInfo->IsDebugTextureListFresh(1)) return;

  CSmartTextureKeyValues textureList;
  if (!textureList.Get()) return;

  CRenderTextureEditor *pRte = m_pViewPanel->GetRenderTxEditor();
  if ((s_eTxListPanelRequest == TXR_RUNNING) && pRte->IsVisible()) {
    KeyValues *kv = NULL;
    int iHint = 0;
    pRte->GetDispInfo(kv, iHint);
    if (kv && iHint) {
      KeyValues *plv =
          (m_pListPanel->IsValidItemID(iHint) ? m_pListPanel->GetItem(iHint)
                                              : NULL);
      if (plv &&
          !strcmp(plv->GetString(KEYNAME_NAME), kv->GetString(KEYNAME_NAME))) {
        KeyValues *pValData = plv->GetFirstValue(),
                  *pValRendered = kv->GetFirstValue();
        for (; pValData && pValRendered;
             pValData = pValData->GetNextValue(),
             pValRendered = pValRendered->GetNextValue()) {
          if (strcmp(pValData->GetString(), pValRendered->GetString())) break;
        }
  ...
  for (KeyValues *pCur = textureList.Get()->GetFirstSubKey(); pCur;
       pCur = pCur->GetNextKey()) {
    ...
    if (numCount > 1) sizeInBytes *= numCount;
    ...
    if (iItem < itemsTouched.GetNumBits()) itemsTouched.Set(iItem);
  }
  ...
  for (int iCur = m_pListPanel->FirstItem();
       iCur != m_pListPanel->InvalidItemID(); iCur = iNext) {
...
void VGui_UpdateTextureListPanel() {
  ...
  g_pMaterialSystemDebugTextureInfo->EnableGetAllTextures(
      mat_texture_list_all.GetBool());

  g_pMaterialSystemDebugTextureInfo->EnableDebugTextureList(
      (mat_texture_list.GetInt() <= 0) ? false : true);
