...
class CClientState : public CBaseClientState, public CClientFrameManager {
 ...
 public:  // CBaseClientState overrides:
  ...
  void SetFriendsID(uint32_t friendsID, const char *friendsName);
  ...
  // copy what they've passed to us into the m_chAreaBits array (and 0xFF-out
  // the m_chAreaPortalBits array).
 ...
 public:
  ...
  CRC32_t serverCRC;  // To determine if client is playing hacked .map.
                      // (entities lump is skipped)
  CRC32_t serverClientSideDllCRC;  // To determine if client is playing on a
                                   // hacked client dll.
 ...
 public:
  // If 'insimulation', returns the time (in seconds) at the client's current
  // tick. Otherwise, returns the exact client clock.
  float GetTime() const;
  ...
  float GetClientInterpAmount();  // Formerly cl_interp, now based on
                                  // cl_interp_ratio and cl_updaterate.
  ...
  void CheckOthersCustomFile(
      CRC32_t crc);  // check if we have to download custom files from server
 ...
 public:
  ...
  // This is only kept around to print out the whitelist info if sv_pure is
  // used.
  CPureServerWhitelist *m_pPureServerWhitelist;

 private:
  // Note: This is only here for backwards compatibility. If it is set to
  // something other than NULL, then we'll copy its contents into m_chAreaBits
  // in UpdateAreaBits_BackwardsCompatible.
  uint8_t *m_pAreaBits;
