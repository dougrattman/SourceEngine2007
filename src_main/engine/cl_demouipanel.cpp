...
void CDemoUIPanel::InstallDemoUI(vgui::Panel *parent) {
  if (g_pDemoUI) return;  // UI already created
...
//-----------------------------------------------------------------------------
CDemoUIPanel::CDemoUIPanel(vgui::Panel *parent)
    : vgui::Frame(parent, "DemoUIPanel") {
  SetTitle("Demo Playback", true);

  m_pPlayPauseResume =
      new vgui::Button(this, "DemoPlayPauseResume", "PlayPauseResume");
...
bool CDemoUIPanel::IsInDriveMode() { return m_pDriveCamera->IsSelected(); }
...
void CDemoUIPanel::OnTick() {
  ...
  if (bIsPlaying) {
    m_pPlayPauseResume->SetText(demoplayer->IsPlaybackPaused() ? "Resume"
                                                               : "Pause");
  } else {
    if (bHasDemoFile) {
  ...
  Q_strncpy(curtime, COM_FormatSeconds(host_state.interval_per_tick * curtick),
            32);
  Q_strncpy(totaltime,
            COM_FormatSeconds(host_state.interval_per_tick * totalticks), 32);
  m_pProgressLabelTime->SetText(va("Time: %s / %s", curtime, totaltime));

  m_pFastForward->SetEnabled(demoplayer->IsPlayingBack() &&
...
void CDemoUIPanel::HandleInput(bool active) {
  ...
  if (active) {
    ...
    bool shiftdown = vgui::input()->IsKeyDown(KEY_LSHIFT) ||
...
static ConCommand demoui("demoui", DemoUI_f, "Show/hide the demo player UI.",
                         FCVAR_DONTRECORD);
...
void CDemoUIPanel2::Install(vgui::Panel *pParentBkgnd, vgui::Panel *pParentFgnd,
                            bool bPutToForeground) {
  if (g_pDemoUI2) return;  // UI already created
...
// Purpose: Basic help dialog
//-----------------------------------------------------------------------------
CDemoUIPanel2::CDemoUIPanel2(vgui::Panel *pParentBkgnd,
                             vgui::Panel *pParentFgnd, bool bPutToForeground)
    : vgui::Frame(bPutToForeground ? pParentFgnd : pParentBkgnd,
                  "DemoUIPanel2") {
...
bool CDemoUIPanel2::IsInDriveMode() { return false; }
...
void CDemoUIPanel2::OnTick() {
  ...
  if (!m_pProgress->IsDragged()) {
    ...
    m_pProgressLabelFrame->SetText(
        va("Tick: %i / %i", m_pProgress->GetValue(), totalticks));
  }

  // Color in red when dragging back
  m_pProgressLabelFrame->SetFgColor((m_pProgress->GetValue() < curtick)
                                        ? Color(255, 0, 0, 255)
                                        : m_pProgressLabelTime->GetFgColor());
...
void CDemoUIPanel2::OnCommand(const char *command) {
  if (!Q_strcasecmp(command, "stop")) {
    ...
    if (!demoplayer->IsPlayingBack()) {
      ...
      demoplayer->IsPlaybackPaused() ? demoplayer->ResumePlayback()
...
void CDemoUIPanel2::OnMessage(const KeyValues *params, VPANEL fromPanel) {
  ...
  if (fromPanel == m_pProgress->GetVPanel()) {
    ...
    if (!Q_strcmp("SliderDragEnd", params->GetName()) ||
        !Q_strcmp("SliderMoved", params->GetName())) {
...
static ConCommand demoui2("demoui2", DemoUI2_f,
                          "Show/hide the advanced demo player UI (demoui2).",
                          FCVAR_DONTRECORD);
static ConCommand demoui2_on(
    "+demoui2", DemoUI2_on,
    "Bring the advanced demo player UI (demoui2) to foreground.",
    FCVAR_DONTRECORD);
static ConCommand demoui2_off(
    "-demoui2", DemoUI2_off,
