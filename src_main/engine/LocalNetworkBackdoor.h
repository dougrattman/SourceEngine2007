...
class CLocalNetworkBackdoor {
 public:
  ...
  void NotifyEdictFlagsChange(int iEdict) {
    // If they newly added the dontsend flag, then we need to run it through
    // EntityDormant.
    if (sv.edicts[iEdict].m_fStateFlags & FL_EDICT_DONTSEND)
      AddToPendingDormantEntityList(iEdict);
  }

  void EntState(int iEnt, int iSerialNum, int iClass,
                const SendTable *pSendTable, const void *pSourceEnt,
                bool bChanged, bool bShouldTransmit);

  void ClearState();
  void StartBackdoorMode();
  void StopBackdoorMode();

  // This is called when the client DLL is loaded to precalculate data to let it
  // copy data faster.
 ...
 private:
  ...
  // This should correspond to which m_CachedEntState entries have a non-0
  // m_pNetworkable pointer.
  ...
  // Tell the client DLL about entities that need to be notified about being
  // dormant. Anything that EntityDormant() would care about needs to get added
  // to this list.
  CUtlLinkedList<unsigned short, unsigned short> m_PendingDormantEntities;

  // This data is cached in here so we don't have to call a lot of virtuals to
  // get it from the client DLL.
  class CCachedEntState {
   public:
    CCachedEntState() { m_iSerialNumber = -1; }
