...
ConVar mat_fastspecular("mat_fastspecular", "1", 0,
                        "Enable/Disable specularity for visual testing.  Will "
                        "not reload materials and will not affect perf.");
ConVar mat_fullbright("mat_fullbright", "0", FCVAR_CHEAT);

static ConVar mat_monitorgamma(
    "mat_monitorgamma", "2.2", FCVAR_ARCHIVE,
...
ConVar r_drawbrushmodels("r_drawbrushmodels", "1", FCVAR_CHEAT,
...
// Note: this is only here so we can ship an update without changing
// materialsystem.dll. Once we ship materialsystem.dll again, we can get rid of
// this and make the only one exist in materialsystem.dll.
...
void mat_hdr_level_Callback(IConVar *var, const char *pOldString,
  ...
  if (g_pMaterialSystemHardwareConfig->GetHardwareHDRType() != HDR_TYPE_NONE &&
      ...
      modelloader->LastLoadedMapHasHDRLighting() && sv.GetMaxClients() == 1 &&
...
ConVar mat_hdr_level("mat_hdr_level", "2", 0,
                     "Set to 0 for no HDR, 1 for LDR+bloom on HDR maps, and 2 "
...
//-----------------------------------------------------------------------------
bool MaterialConfigLightingChanged() { return s_bConfigLightingChanged; }

void ClearMaterialConfigLightingChanged() { s_bConfigLightingChanged = false; }

//-----------------------------------------------------------------------------
// List of all convars to store into the registry
//-----------------------------------------------------------------------------
static const char *s_pRegistryConVars[] = {
    "mat_forceaniso", "mat_picmip",
    //	"mat_dxlevel",
    "mat_trilinear", "mat_vsync", "mat_forcehardwaresync", "mat_parallaxmap",
    "mat_reducefillrate", "r_shadowrendertotexture", "r_rootlod",
#ifndef _X360
    "r_waterforceexpensive",
#endif
    "r_waterforcereflectentities", "mat_antialias", "mat_aaquality",
    "mat_specular", "mat_bumpmap", "mat_hdr_level", "mat_colorcorrection",
...
//-----------------------------------------------------------------------------
static void ReadMaterialSystemConfigFromRegistry(
...
#define READ_VIDCFG_ENTRY(name, entry)     \
  ...
  config.SetFlag(MATSYS_VIDCFG_FLAGS_WINDOWED,
                 registry->ReadInt("ScreenWindowed", 0) != 0);

  // Special case...
  const char *szMonitorGamma =
      registry->ReadString("ScreenMonitorGamma", "2.2");
  if (szMonitorGamma && strlen(szMonitorGamma) > 0) {
    float flMonitorGamma = atof(szMonitorGamma);
    // temp, to make sure people with gamma values saved in the old format don't
    // max out
  ...
  if (nValue != -1) {
    ...
    if (conVar.IsValid()) {
      ...
      config.m_bShadowDepthTexture =
          registry->ReadInt("ShadowDepthTexture", 0) != 0;
    }
  }
...
//-----------------------------------------------------------------------------
static void WriteMaterialSystemConfigToRegistry(
    const MaterialSystem_Config_t &config) {
#ifndef SWDS
  if (IsX360()) return;

#define WRITE_VIDCFG_ENTRY(name, entry) registry->WriteInt(name, entry)
  ...
  WRITE_VIDCFG_ENTRY("ShadowDepthTexture",
                     config.m_bShadowDepthTexture ? 1 : 0);
...
//-----------------------------------------------------------------------------
static void OverrideMaterialSystemConfigFromCommandLine(
    MaterialSystem_Config_t &config) {
  if (CommandLine()->FindParm("-dxlevel")) {
    config.dxSupportLevel =
        CommandLine()->ParmValue("-dxlevel", config.dxSupportLevel);
  ...
  if (CommandLine()->FindParm("-width") || CommandLine()->FindParm("-w")) {
    config.m_VideoMode.m_Width =
        CommandLine()->ParmValue("-width", config.m_VideoMode.m_Width);
    config.m_VideoMode.m_Width =
        CommandLine()->ParmValue("-w", config.m_VideoMode.m_Width);
    if (!(CommandLine()->FindParm("-height") ||
          CommandLine()->FindParm("-h"))) {
  ...
  if (CommandLine()->FindParm("-resizing")) {
    config.SetFlag(MATSYS_VIDCFG_FLAGS_RESIZING,
                   CommandLine()->CheckParm("-resizing") ? true : false);
  }
  ...
  config.m_nAASamples =
      CommandLine()->ParmValue("-mat_antialias", config.m_nAASamples);
  config.m_nAAQuality =
      CommandLine()->ParmValue("-mat_aaquality", config.m_nAAQuality);
...
void OverrideMaterialSystemConfig(MaterialSystem_Config_t &config) {
  ...
  if (mat_supportflashlight.GetInt() == -1) {
    ...
    if (!V_stricmp(gameName, "portal") || !V_stricmp(gameName, "tf")) {
...
void HandleServerAllowColorCorrection() {
  ...
  if (!sv_allow_color_correction.GetBool() && mat_colorcorrection.GetInt()) {
    Warning(
        "mat_colorcorrection being forced to 0 due to "
...
// auto config version to store in the rgeistry so we can force reconfigs if
// needed
...
void InitMaterialSystemConfig(bool bInEditMode) {
  ...
  uint32_t currentVendorID = registry->ReadInt("VendorID", -1);
  uint32_t currentDeviceID = registry->ReadInt("DeviceID", -1);

  uint32_t autoConfigVersion = registry->ReadInt("AutoConfigVersion", -1);

  if (autoConfigVersion != AUTOCONFIG_VERSION) {
    uint32_t max_dxlevel, recommended_dxlevel;
    materials->GetDXLevelDefaults(max_dxlevel, recommended_dxlevel);
    uint32_t currentDXLevel = registry->ReadInt("DXLevel_V1", -1);
    if ((max_dxlevel != recommended_dxlevel) &&
        (currentDXLevel != recommended_dxlevel)) {
      ConVarRef conVar("mat_dxlevel");
      if (conVar.IsValid()) {
...
//-----------------------------------------------------------------------------
// Purpose: Sets all the relevant keyvalue data to be uploaded as part of the
// benchmark data Input  : *dataToUpload - keyvalue set that will be uploaded
//-----------------------------------------------------------------------------
void GetMaterialSystemConfigForBenchmarkUpload(KeyValues *dataToUpload) {
  ...
  dataToUpload->SetInt("AnisoLevel",
                       g_pMaterialSystemConfig->m_nForceAnisotropicLevel);
  ...
  dataToUpload->SetInt("ShadowDepthTexture",
                       g_pMaterialSystemConfig->ShadowDepthTexture());
  dataToUpload->SetInt("MotionBlur", g_pMaterialSystemConfig->MotionBlur());
  dataToUpload->SetInt(
      "Windowed",
      (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_WINDOWED) ? 1
                                                                        : 0);
  dataToUpload->SetInt("Trilinear", (g_pMaterialSystemConfig->m_Flags &
                                     MATSYS_VIDCFG_FLAGS_FORCE_TRILINEAR)
                                        ? 1
                                        : 0);
  dataToUpload->SetInt("ForceHWSync", (g_pMaterialSystemConfig->m_Flags &
                                       MATSYS_VIDCFG_FLAGS_FORCE_HWSYNC)
                                          ? 1
                                          : 0);
  dataToUpload->SetInt("NoWaitForVSync", (g_pMaterialSystemConfig->m_Flags &
                                          MATSYS_VIDCFG_FLAGS_NO_WAIT_FOR_VSYNC)
                                             ? 1
                                             : 0);
  dataToUpload->SetInt("DisableSpecular", (g_pMaterialSystemConfig->m_Flags &
                                           MATSYS_VIDCFG_FLAGS_DISABLE_SPECULAR)
                                              ? 1
                                              : 0);
  dataToUpload->SetInt(
      "DisableBumpmapping",
      (g_pMaterialSystemConfig->m_Flags & MATSYS_VIDCFG_FLAGS_DISABLE_BUMPMAP)
          ? 1
          : 0);
  dataToUpload->SetInt("EnableParallaxMapping",
                       (g_pMaterialSystemConfig->m_Flags &
                        MATSYS_VIDCFG_FLAGS_ENABLE_PARALLAX_MAPPING)
                           ? 1
                           : 0);
  dataToUpload->SetInt("ZPrefill", (g_pMaterialSystemConfig->m_Flags &
                                    MATSYS_VIDCFG_FLAGS_USE_Z_PREFILL)
                                       ? 1
                                       : 0);
  dataToUpload->SetInt("ReduceFillRate", (g_pMaterialSystemConfig->m_Flags &
                                          MATSYS_VIDCFG_FLAGS_REDUCE_FILLRATE)
                                             ? 1
                                             : 0);
  dataToUpload->SetInt("RenderToTextureShadows",
                       r_shadowrendertotexture.GetInt() ? 1 : 0);
  dataToUpload->SetInt("FlashlightDepthTexture",
                       r_flashlightdepthtexture.GetInt() ? 1 : 0);
#ifndef _X360
  dataToUpload->SetInt("RealtimeWaterReflection",
                       r_waterforceexpensive.GetInt() ? 1 : 0);
#endif
  dataToUpload->SetInt("WaterReflectEntities",
                       r_waterforcereflectentities.GetInt() ? 1 : 0);
#endif
}
...
void PrintMaterialSystemConfig(const MaterialSystem_Config_t &config) {
  ...
  Warning("MATSYS_VIDCFG_FLAGS_FORCE_TRILINEAR: %s\n",
          (config.m_Flags & MATSYS_VIDCFG_FLAGS_FORCE_TRILINEAR) ? "true"
                                                                 : "false");
  Warning(
      "MATSYS_VIDCFG_FLAGS_FORCE_HWSYNC: %s\n",
      (config.m_Flags & MATSYS_VIDCFG_FLAGS_FORCE_HWSYNC) ? "true" : "false");
  Warning("MATSYS_VIDCFG_FLAGS_DISABLE_SPECULAR: %s\n",
          (config.m_Flags & MATSYS_VIDCFG_FLAGS_DISABLE_SPECULAR) ? "true"
                                                                  : "false");
  Warning("MATSYS_VIDCFG_FLAGS_ENABLE_PARALLAX_MAPPING: %s\n",
          (config.m_Flags & MATSYS_VIDCFG_FLAGS_ENABLE_PARALLAX_MAPPING)
              ? "true"
              : "false");
  Warning(
  ...
  Warning("MATSYS_VIDCFG_FLAGS_REDUCE_FILLRATE: %s\n",
          (config.m_Flags & MATSYS_VIDCFG_FLAGS_REDUCE_FILLRATE) ? "true"
                                                                 : "false");
...
CON_COMMAND(mat_savechanges,
            "saves current video configuration to the registry") {
  // if the user has got to the point where they can adjust and apply video
  // changes, then we can clear safe mode
...
//-----------------------------------------------------------------------------
CON_COMMAND_F(mat_debug, "Activates debugging spew for a specific material.",
              FCVAR_CHEAT) {
  if (args.ArgC() != 2) {
...
static ITexture *CreatePowerOfTwoFBTexture(void) {
  ...
  return materials->CreateNamedRenderTargetTextureEx2(
      "_rt_PowerOfTwoFB", 1024, 1024, RT_SIZE_DEFAULT,
...
static ITexture *CreateWaterReflectionTexture(void) {
  return materials->CreateNamedRenderTargetTextureEx2(
      "_rt_WaterReflection", 1024, 1024, RT_SIZE_PICMIP,
      materials->GetBackBufferFormat(), MATERIAL_RT_DEPTH_SHARED,
      TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT, CREATERENDERTARGETFLAGS_HDR);
}

static ITexture *CreateWaterRefractionTexture(void) {
  return materials->CreateNamedRenderTargetTextureEx2(
      "_rt_WaterRefraction", 1024, 1024, RT_SIZE_PICMIP,
      // This is different than reflection because it has to have alpha for fog
      // factor.
      IMAGE_FORMAT_RGBA8888, MATERIAL_RT_DEPTH_SHARED,
      TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT, CREATERENDERTARGETFLAGS_HDR);
}
...
static ITexture *CreateCameraTexture(void) {
  return materials->CreateNamedRenderTargetTextureEx2(
      "_rt_Camera", 256, 256, RT_SIZE_DEFAULT, materials->GetBackBufferFormat(),
      MATERIAL_RT_DEPTH_SHARED, 0, CREATERENDERTARGETFLAGS_HDR);
}

static ITexture *CreateBuildCubemaps16BitTexture(void) {
  return materials->CreateNamedRenderTargetTextureEx2(
      "_rt_BuildCubemaps16bit", 0, 0, RT_SIZE_FULL_FRAME_BUFFER,
      IMAGE_FORMAT_RGBA16161616, MATERIAL_RT_DEPTH_SHARED);
}

static ITexture *CreateQuarterSizedFBTexture(int n,
                                             unsigned int iRenderTargetFlags) {
  char nbuf[20];
  sprintf_s(nbuf, "_rt_SmallFB%d", n);
  ...
  return materials->CreateNamedRenderTargetTextureEx2(
      ...
      TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT, iRenderTargetFlags);
}

static ITexture *CreateTeenyFBTexture(int n) {
  char nbuf[20];
  sprintf_s(nbuf, "_rt_TeenyFB%d", n);
...
static ITexture *CreateFullFrameFBTexture(int textureIndex,
                                          int iExtraFlags = 0) {
  char textureName[256];
  if (textureIndex > 0) {
    sprintf_s(textureName, "_rt_FullFrameFB%d", textureIndex);
  } else {
  ...
  return materials->CreateNamedRenderTargetTextureEx2(
      textureName, 1, 1, RT_SIZE_FULL_FRAME_BUFFER,
      materials->GetBackBufferFormat(), MATERIAL_RT_DEPTH_SHARED,
      TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT, rtFlags);
}
...
void InitWellKnownRenderTargets(void) {
  ...
  // JasonM -
  // Do we put logic in here to determine which of these to create, based upon
  // DX level, HDR enable etc? YES! DX Level should gate these
  ...
  if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80) {
    ...
    if (IsX360())
      materials->AddTextureAlias(
          "_rt_SmallFB1",
          "_rt_SmallFB0");  // an alias is good enough on the 360 since we don't
                            // have a texture lock problem during post
                            // processing
    else
  ...
  if (g_pClientRenderTargets) {
    g_pClientRenderTargets->InitClientRenderTargets(
        materials, g_pMaterialSystemHardwareConfig);
  } else {
    // If this mod doesn't define the interface, fallback to initializing the
    // standard render textures NOTE: these should match up with the 'Get'
    // functions in cl_dll/rendertexture.h/cpp
  ...
  // End block in which all render targets should be allocated (kicking off an
  // Alt-Tab type behavior)
...
//-----------------------------------------------------------------------------
// Purpose: Shut down the render targets which mods rely upon to render
// correctly
//-----------------------------------------------------------------------------
void ShutdownWellKnownRenderTargets(void) {
  ...
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  pRenderContext->SetNonInteractiveTempFullscreenBuffer(
      NULL, MATERIAL_NON_INTERACTIVE_MODE_LEVEL_LOAD);
...
static void InitDebugMaterials(void) {
...
#ifndef SWDS
  g_materialWireframe =
      GL_LoadMaterial("debug/debugwireframe", TEXTURE_GROUP_OTHER);
  g_materialTranslucentSingleColor =
      GL_LoadMaterial("debug/debugtranslucentsinglecolor", TEXTURE_GROUP_OTHER);
  g_materialTranslucentVertexColor =
      GL_LoadMaterial("debug/debugtranslucentvertexcolor", TEXTURE_GROUP_OTHER);
  g_materialWorldWireframe =
      GL_LoadMaterial("debug/debugworldwireframe", TEXTURE_GROUP_OTHER);
  g_materialWorldWireframeZBuffer =
      GL_LoadMaterial("debug/debugworldwireframezbuffer", TEXTURE_GROUP_OTHER);

  g_materialBrushWireframe =
      GL_LoadMaterial("debug/debugbrushwireframe", TEXTURE_GROUP_OTHER);
  g_materialDecalWireframe =
      GL_LoadMaterial("debug/debugdecalwireframe", TEXTURE_GROUP_OTHER);
  g_materialDebugLightmap =
      GL_LoadMaterial("debug/debuglightmap", TEXTURE_GROUP_OTHER);
  g_materialDebugLightmapZBuffer =
      GL_LoadMaterial("debug/debuglightmapzbuffer", TEXTURE_GROUP_OTHER);
  g_materialDebugLuxels =
      GL_LoadMaterial("debug/debugluxels", TEXTURE_GROUP_OTHER);

  g_materialLeafVisWireframe =
      GL_LoadMaterial("debug/debugleafviswireframe", TEXTURE_GROUP_OTHER);
  g_pMaterialWireframeVertexColor =
      GL_LoadMaterial("debug/debugwireframevertexcolor", TEXTURE_GROUP_OTHER);
  g_pMaterialWireframeVertexColorIgnoreZ = GL_LoadMaterial(
      "debug/debugwireframevertexcolorignorez", TEXTURE_GROUP_OTHER);
  g_pMaterialLightSprite =
      GL_LoadMaterial("engine/lightsprite", TEXTURE_GROUP_OTHER);
  g_pMaterialShadowBuild =
      GL_LoadMaterial("engine/shadowbuild", TEXTURE_GROUP_OTHER);
  g_pMaterialMRMWireframe =
      GL_LoadMaterial("debug/debugmrmwireframe", TEXTURE_GROUP_OTHER);
  g_pMaterialDebugFlat =
      GL_LoadMaterial("debug/debugdrawflattriangles", TEXTURE_GROUP_OTHER);

  g_pMaterialAmbientCube =
      GL_LoadMaterial("debug/debugambientcube", TEXTURE_GROUP_OTHER);
  ...
  if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90) {
    ...
    g_pMaterialDepthWrite[0][0] = g_pMaterialSystem->FindProceduralMaterial(
        "__DepthWrite00", TEXTURE_GROUP_OTHER, pVMTKeyValues);

    pVMTKeyValues = new KeyValues("DepthWrite");
    pVMTKeyValues->SetInt("$no_fullbright", 1);
    pVMTKeyValues->SetInt("$alphatest", 0);
    pVMTKeyValues->SetInt("$nocull", 1);
    g_pMaterialDepthWrite[0][1] = g_pMaterialSystem->FindProceduralMaterial(
        "__DepthWrite01", TEXTURE_GROUP_OTHER, pVMTKeyValues);

    pVMTKeyValues = new KeyValues("DepthWrite");
    pVMTKeyValues->SetInt("$no_fullbright", 1);
    pVMTKeyValues->SetInt("$alphatest", 1);
    pVMTKeyValues->SetInt("$nocull", 0);
    g_pMaterialDepthWrite[1][0] = g_pMaterialSystem->FindProceduralMaterial(
        "__DepthWrite10", TEXTURE_GROUP_OTHER, pVMTKeyValues);

    pVMTKeyValues = new KeyValues("DepthWrite");
    pVMTKeyValues->SetInt("$no_fullbright", 1);
    pVMTKeyValues->SetInt("$alphatest", 1);
    pVMTKeyValues->SetInt("$nocull", 1);
    g_pMaterialDepthWrite[1][1] = g_pMaterialSystem->FindProceduralMaterial(
        "__DepthWrite11", TEXTURE_GROUP_OTHER, pVMTKeyValues);
  } else {
    g_pMaterialDepthWrite[0][0] = g_pMaterialDepthWrite[0][1] =
        g_pMaterialDepthWrite[1][0] = g_pMaterialDepthWrite[1][1] = NULL;
  }
#endif
}
...
void RestoreMaterialSystemObjects(int nChangeFlags) {
  ...
  if (host_state.worldmodel) {
    if (nChangeFlags & MATERIAL_RESTORE_VERTEX_FORMAT_CHANGED) {
#ifndef SWDS
      // Reload lightmaps, world meshes, etc. because we may have switched from
      // bumped to unbumped
...
bool TangentSpaceSurfaceSetup(SurfaceHandle_t surfID, Vector &tVect) {
  Vector sVect;
  VectorCopy(
      MSurf_TexInfo(surfID)->textureVecsTexelsPerWorldUnits[0].AsVector3D(),
      sVect);
  VectorCopy(
      MSurf_TexInfo(surfID)->textureVecsTexelsPerWorldUnits[1].AsVector3D(),
      tVect);
...
void TangentSpaceComputeBasis(Vector &tangentS, Vector &tangentT,
                              const Vector &normal, const Vector &tVect,
                              bool negateTangent) {
...
// The amount to blend between basetexture and basetexture2 used to sit in
// lightmap alpha, so we didn't care about the vertex color or vertex alpha. But
// now if they're using it, we have to make sure the vertex has the color and
// alpha specified correctly or it will look weird.
static inline bool CheckMSurfaceBaseTexture2(worldbrushdata_t *pBrushData,
    ...
    if (pMaterialName) {
      ...
      if (bShowIt) {
        ...
        for (int i = 0; i < MSurf_VertCount(surfID); i++) {
          int vertIndex =
              pBrushData->vertindices[MSurf_FirstVertIndex(surfID) + i];
        ...
        Warning(
            "Warning: using WorldTwoTextureBlend on a non-displacement "
            "surface.\n"
...
void BuildMSurfaceVertexArrays(worldbrushdata_t *pBrushData,
  ...
  uint8_t flatColor[4] = {255, 255, 255, 255};
  ...
  for (int i = 0; i < MSurf_VertCount(surfID); i++) {
    ...
    // garymct: normalized (within space of surface) lightmap texture
    // coordinates
    ...
    Vector &normal =
        pBrushData->vertnormals
            [pBrushData->vertnormalindices[MSurf_FirstVertNormal(surfID) + i]];
    ...
    // The amount to blend between basetexture and basetexture2 used to sit in
    // lightmap alpha, so we didn't care about the vertex color or vertex alpha.
    // But now if they're using it, we have to make sure the vertex has the
    // color and alpha specified correctly or it will look weird.
    if (!SurfaceHasDispInfo(surfID) &&
        (MSurf_TexInfo(surfID)->texinfoFlags & TEXINFO_USING_BASETEXTURE2)) {
      ...
      if (!bWarned) {
        ...
        Warning(
            ...
            "(material: %s). This wastes perf for no benefit.\n",
            pMaterialName);
      }
...
static int VertexCountForSurfaceList(const CMSurfaceSortList &list,
...
int FindOrAddMesh(IMaterial *pMaterial, int vertexCount) {
  ...
  for (int i = 0; i < g_Meshes.Count(); i++) {
    if (g_Meshes[i].vertexFormat != format) continue;

    if (g_Meshes[i].vertCount + vertexCount > nMaxVertices) continue;
...
void SetTexInfoBaseTexture2Flags() {
  for (int i = 0; i < host_state.worldbrush->numtexinfo; i++) {
    host_state.worldbrush->texinfo[i].texinfoFlags &=
        ~TEXINFO_USING_BASETEXTURE2;
  }
  ...
  for (int i = 0; i < host_state.worldbrush->numtexinfo; i++) {
    ...
    for (int i = 0; i < nParms; i++) {
      if (!pParms[i]->IsDefined()) continue;
...
void WorldStaticMeshCreate(void) {
  ...
  if (nSortIDs == 0) {
    // this is probably a bug in alt-tab.  It's calling this as a restore
    // function but the lightmaps haven't been allocated yet
    Assert(0);
    return;
  }
  ...
  memset(g_WorldStaticMeshes.Base(), 0,
         sizeof(g_WorldStaticMeshes[0]) * g_WorldStaticMeshes.Count());
  ...
  for (int surfaceIndex = 0; surfaceIndex < host_state.worldbrush->numsurfaces;
    ...
    if (bTools || (MSurf_TexInfo(surfID)->material->GetVertexFormat() &
                   VERTEX_TANGENT_SPACE)) {
      MSurf_Flags(surfID) |= SURFDRAW_TANGENTSPACE;
    }

    // don't create vertex buffers for nodraw faces, water faces, or faces with
    // dynamic data
    //		if ( (MSurf_Flags( surfID ) &
    //(SURFDRAW_NODRAW|SURFDRAW_WATERSURFACE|SURFDRAW_DYNAMIC))
  ...
  for (i = 0; i < g_WorldStaticMeshes.Count(); i++) {
    ...
    sortIndex[i] =
        surfID ? FindOrAddMesh(MSurf_TexInfo(surfID)->material, vertexCount)
               : -1;
  }
  ...
  for (i = 0; i < g_Meshes.Count(); i++) {
    ...
    g_Meshes[i].pVertexBuffer = pRenderContext->CreateStaticVertexBuffer(
        GetUncompressedFormat(g_Meshes[i].pMaterial), g_Meshes[i].vertCount,
  ...
  for (i = 0; i < g_Meshes.Count(); i++) {
    ...
    VertexFormat_t vertexFormat =
        ComputeWorldStaticMeshVertexFormat(g_Meshes[i].pMaterial);
    g_Meshes[i].pMesh = pRenderContext->CreateStaticMesh(
        vertexFormat, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_WORLD,
        g_Meshes[i].pMaterial);
    int vertBufferIndex = 0;
    // NOTE: Index count is zero because this will be a static vertex buffer!!!
    CMeshBuilder meshBuilder;
    meshBuilder.Begin(g_Meshes[i].pMesh, MATERIAL_TRIANGLES,
                      g_Meshes[i].vertCount, 0);
...
void SurfComputeTextureCoordinate(SurfaceCtx_t const &ctx,
                                  SurfaceHandle_t surfID, Vector const &vec,
                                  Vector2D &uv) {
  mtexinfo_t *pTexInfo = MSurf_TexInfo(surfID);

  // base texture coordinate
  uv.x = DotProduct(vec,
                    pTexInfo->textureVecsTexelsPerWorldUnits[0].AsVector3D()) +
         pTexInfo->textureVecsTexelsPerWorldUnits[0][3];
  uv.x /= pTexInfo->material->GetMappingWidth();

  uv.y = DotProduct(vec,
                    pTexInfo->textureVecsTexelsPerWorldUnits[1].AsVector3D()) +
         pTexInfo->textureVecsTexelsPerWorldUnits[1][3];
  uv.y /= pTexInfo->material->GetMappingHeight();
}

#if _DEBUG
void CheckTexCoord(float coord) { Assert(coord <= 1.0f); }
#endif

void SurfComputeLightmapCoordinate(SurfaceCtx_t const &ctx,
                                   SurfaceHandle_t surfID, Vector const &vec,
                                   Vector2D &uv) {
  if ((MSurf_Flags(surfID) & SURFDRAW_NOLIGHT)) {
    ...
    uv.x = DotProduct(
               vec, pTexInfo->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D()) +
           pTexInfo->lightmapVecsLuxelsPerWorldUnits[0][3];
    uv.x -= MSurf_LightmapMins(surfID)[0];
    uv.x += 0.5f;

    uv.y = DotProduct(
               vec, pTexInfo->lightmapVecsLuxelsPerWorldUnits[1].AsVector3D()) +
...
#if _DEBUG
  // This was here for check against displacements and they actually get
  // calculated later correctly.
//	CheckTexCoord( uv.x );
//	CheckTexCoord( uv.y );
#endif
...
void SurfSetupSurfaceContext(SurfaceCtx_t &ctx, SurfaceHandle_t surfID) {
  ...
  ctx.m_Offset.x =
      (float)MSurf_OffsetIntoLightmapPage(surfID)[0] * ctx.m_Scale.x;
  ctx.m_Offset.y =
      (float)MSurf_OffsetIntoLightmapPage(surfID)[1] * ctx.m_Scale.y;

  if (ctx.m_LightmapPageSize[0] != 0.0f) {
    ctx.m_BumpSTexCoordOffset =
        (float)ctx.m_LightmapSize[0] / (float)ctx.m_LightmapPageSize[0];
  } else {
