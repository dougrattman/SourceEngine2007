...
#define TIMINGS \
  256  // Number of values to track (must be power of 2) b/c of masking
...
void CVProfGraphPanel::Paint() {
  ...
  x += w / 2;
  for (int i = 3; --i >= 0;) {
    Q_snprintf(sz, sizeof(sz), "%07.3f ms (%s)", m_Samples[m_CurrentSample][i],
               pTitles[i]);
    y -= 10;
    g_pMatSystemSurface->DrawColoredText(m_hFont, x, y, color[i][0],
                                         color[i][1], color[i][2], 180, sz);
  }
}
...
void CVProfGraphPanel::GetNextSample() {
  ...
  m_Samples[m_CurrentSample][1] =
      m_CurrentNode->GetParent() ? m_CurrentNode->GetParent()->GetPrevTime()
                                 : m_CurrentNode->GetPrevTime();
  m_Samples[m_CurrentSample][2] =
