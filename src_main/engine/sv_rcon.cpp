...
CRConServer &RCONServer() { return g_RCONServer; }

static CRPTServer g_RPTServer;
CRConServer &RPTServer() { return g_RPTServer; }

static void RconPasswordChanged_f(IConVar *pConVar, const char *pOldString,
                                  float flOldValue) {
...
ConVar rcon_password("rcon_password", "",
                     FCVAR_SERVER_CANNOT_QUERY | FCVAR_DONTRECORD,
                     "remote console password.", RconPasswordChanged_f);
...
//-----------------------------------------------------------------------------
bool CRConServer::ConnectToListeningClient(const netadr_t &adr,
                                           bool bSingleSocket) {
...
//-----------------------------------------------------------------------------
bool CRConServer::ShouldAcceptSocket(SocketHandle_t hSocket,
                                     const netadr_t &netAdr) {
  return true;
}

void CRConServer::OnSocketAccepted(SocketHandle_t hSocket,
                                   const netadr_t &netAdr, void **ppData) {
  ConnectedRConSocket_t *pNewSocket = new ConnectedRConSocket_t;
  pNewSocket->lastRequestID = 0;
  pNewSocket->authed = false;
  pNewSocket->listenerID =
      g_ServerRemoteAccess.GetNextListenerID(true, &netAdr);
  *ppData = pNewSocket;
}

void CRConServer::OnSocketClosed(SocketHandle_t hSocket, const netadr_t &netAdr,
                                 void *pData) {
...
//-----------------------------------------------------------------------------
// Purpose: accept new connections and walk open sockets and handle any incoming
// data
//-----------------------------------------------------------------------------
void CRConServer::RunFrame() {
  ...
  for (int i = nCount - 1; i >= 0; --i) {
    ...
    while (pData->m_OutstandingSends.Count() > 0) {
      CUtlBuffer &packet =
          pData->m_OutstandingSends[pData->m_OutstandingSends.Head()];
      bool bSent = SendRCONResponse(i, packet.PeekGet(),
                                    packet.TellPut() - packet.TellGet(), true);
      if (bSent)  // all this packet was sent, remove it
      {
        pData->m_OutstandingSends.Remove(
            pData->m_OutstandingSends.Head());  // delete this entry no matter
                                                // what, SendRCONResponse() will
                                                // re-queue if needed
    ...
    if (sendLen > 0) {
      ...
      memcpy(pBuf, &sendLen,
             sizeof(sendLen));  // copy the size of the packet in
      g_ServerRemoteAccess.ReadDataResponse(pData->listenerID,
                                            pBuf + sizeof(int), sendLen);
    ...
    if (pendingLen == -1 && SocketWouldBlock()) continue;
    ...
    if (readLen > sizeof(int))  // we have a command to process
    {
      ...
      while (size > 0 && size <= response.TellPut() - response.TellGet()) {
        ...
        g_ServerRemoteAccess.WriteDataRequest(this, pData->listenerID,
                                              response.PeekGet(), size);
        SV_RedirectEnd();
        response.SeekGet(CUtlBuffer::SEEK_CURRENT,
                         size);  // eat up the buffer we just sent
      ...
      if (size > 0 || (response.TellPut() - response.TellGet() > 0)) {
        ...
        if (response.TellPut() - response.TellGet() > 0) {
          tmpBuf.Put(response.PeekGet(),
                     response.TellPut() - response.TellGet());
        }
        response.Purge();
        if (size > 0) {
...
void CRConServer::FinishRedirect(const char *msg, const netadr_t &adr) {
  ...
  for (int i = nCount - 1; i >= 0; --i) {
    const netadr_t &socketAdr = m_Socket.GetAcceptedSocketAddress(i);
    if (!adr.CompareAdr(socketAdr)) continue;
...
//-----------------------------------------------------------------------------
// Purpose: set the current outstanding request ID for this connection, used by
// the redirect flush above
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CRConServer::SendRCONResponse(int nIndex, const void *data, int len,
                                   bool fromQueue) {
  ...
  Assert(!(fromQueue &&
           data !=
               (pSocketData
                    ->m_OutstandingSends[pSocketData->m_OutstandingSends.Head()]
                    .Base())));

  int sendLen = 0;
  while (sendLen < len) {
    int ret = send(hSocket, (const char *)data + sendLen, len - sendLen,
                   MSG_NOSIGNAL);
    if (ret == -1) {
                       ...
                       // yet, add a new one
      {
        ...
        pSocketData->m_OutstandingSends[index].Put(
            (void *)((char *)data + sendLen), len - sendLen);
      } else  // update the existing queued item to show we
              // sent some of it (we only ever send the head of the list)
      {
        pSocketData->m_OutstandingSends[pSocketData->m_OutstandingSends.Head()]
            .SeekGet(CUtlBuffer::SEEK_CURRENT, sendLen);
      }
      return false;
    } else if (ret > 0) {
      sendLen += ret;
...
ConVar sv_rcon_banpenalty(
    "sv_rcon_banpenalty", "0", 0,
    "Number of minutes to ban users who fail rcon authentication", true, 0,
    false, 0);
ConVar sv_rcon_maxfailures("sv_rcon_maxfailures", "10", 0,
                           "Max number of times a user can fail rcon "
                           "authentication before being banned",
                           true, 1, true, 20);
ConVar sv_rcon_minfailures("sv_rcon_minfailures", "5", 0,
                           "Number of times a user can fail rcon "
                           "authentication in sv_rcon_minfailuretime before "
                           "being banned",
                           true, 1, true, 20);
ConVar sv_rcon_minfailuretime(
    "sv_rcon_minfailuretime", "30", 0,
    "Number of seconds to track failed rcon authentications", true, 1, false,
    0);

//-----------------------------------------------------------------------------
// Purpose: compares failed rcons based on most recent failure time
//-----------------------------------------------------------------------------
bool CRConServer::FailedRCon_t::operator<(
    const struct CRConServer::FailedRCon_t &rhs) const {
  int myTime = 0;
  int rhsTime = 0;
...
void CRConServer::HandleFailedRconAuth(const netadr_t &adr) {
  ...
  if (!failedRcon) {
    ...
    if (nCount >= 32) {
      ...
      for (i = 0; i < nCount; ++i) {
        if (indexToRemove < 0 ||
            m_failedRcons[i] < m_failedRcons[indexToRemove]) {
          indexToRemove = i;
        }
      }
      if (indexToRemove >= 0) {
  ...
  // remove old failure times (sv_rcon_maxfailures is limited to 20, so we won't
  // be hurting anything by pruning)
  ...
  //	ConMsg( "min=%d, max=%d, time=%.2f\n", sv_rcon_minfailures.GetInt(),
  // sv_rcon_maxfailures.GetInt(), sv_rcon_minfailuretime.GetFloat() );
  ...
  for (i = failedRcon->badPasswordTimes.Count() - 1; i >= 0; --i) {
    if (failedRcon->badPasswordTimes[i] + sv_rcon_minfailuretime.GetFloat() >=
        sv.GetTime()) {
      ++recentFailures;
    }
  }
