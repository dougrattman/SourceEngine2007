...
the_interface IMaster {
 public:
  ...
  // Called any time a master function might need to initialize the master
  // system
  ...
  // If parsing for server, etc. fails, always have at least one server around
  // to use.
  ...
  virtual void SendHeartbeat(netadr_t & from, int challangenr,
                             CBaseServer *pServer) = 0;
  ...
  virtual bool IsCorrectMasterChallenge(netadr_t * from, int challenge,
                                        CBaseServer *pServer) = 0;
  virtual void HandleUnknown(struct netpacket_s * packet, CBaseServer * pServer,
                             CGameServer * pGameServer) = 0;

  // returns true if we should issue a "quit" on level change (due to being out
  // of date)
...
extern bool IsUsingMasterLegacyMode();  // This is the on/off switch to enable
                                        // all this legacy stuff (controlled by
                                        // cvar sv_master_legacy_mode).
