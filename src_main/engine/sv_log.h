...
class CLog : public IGameEventListener2 {
 ...
 public:
  ...
  void SetLoggingState(
      bool state);  // set the logging state to true (on) or false (off)
 ...
 private:
  ...
  CUtlVector<netadr_t> m_LogAddresses;  // Server frag log stream is sent to the
                                        // address(es) in this list
