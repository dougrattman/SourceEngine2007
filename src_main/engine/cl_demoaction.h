...
enum DEMOACTION {
  ...
  DEMO_ACTION_ONSCREENFADE_FINISHED,  // Fire when screen fade of specified name
                                      // finishes
  DEMO_ACTION_ONTEXTMESSAGE_FINISHED,  // Fire when specified text message
                                       // finishes
...
typedef CBaseActionEditDialog *(*DEMOACTIONEDIT_FUNC)(CDemoEditorPanel *parent,
...
the_interface CBaseDemoAction {
 ...
 public:
  ...
  static CBaseActionEditDialog *CreateActionEditor(
      DEMOACTION actionType, CDemoEditorPanel * parent,
...
#define DECLARE_DEMOACTION(type, classname)                   \
  ...
  class CFactory##classname {                                 \
   public:                                                    \
    CFactory##classname() {                                   \
      CBaseDemoAction::AddFactory(type, FnCreate##classname); \
    }                                                         \
  };                                                          \
...
#define DECLARE_DEMOACTIONEDIT(type, classname)                               \
  static CBaseActionEditDialog *FnCreateEditor##classname(                    \
    ...
    CBaseActionEditDialog *editor = new classname(parent, action, newaction); \
  ...
  class CFactoryEditor##classname {                                           \
   public:                                                                    \
    CFactoryEditor##classname() {                                             \
      CBaseDemoAction::AddEditorFactory(type, FnCreateEditor##classname);     \
    }                                                                         \
  };                                                                          \
