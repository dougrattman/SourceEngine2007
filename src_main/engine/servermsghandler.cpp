...
static ConVar cl_allowupload("cl_allowupload", "1", FCVAR_ARCHIVE,
                             "Client uploads customization files");
static ConVar cl_voice_filter(
    "cl_voice_filter", "", 0,
    "Filter voice by name substring");  // filter incoming voice data
...
void CClientState::PacketEnd() {
  ...
  // Did we get any messages this tick (i.e., did we call
  ...
  // Msg( "%i/%i CL_PostReadMessages:  last ack %i most recent %i acked %i
  // commands\n", 	host_framecount, cl.tickcount, 	cl.last_command_ack,
...
void CClientState::Disconnect(bool bShowMainMenu) {
  ...
  if (bShowMainMenu && !engineClient->IsDrawingLoadingImage() &&
      (cl.demonum == -1)) {
...
bool CClientState::ProcessTick(NET_Tick *msg) {
  ...
  // Use the server tick while reading network data (used for interpolation
  // samples, etc).
  g_ClientGlobalVariables.tickcount = tick;
  g_ClientGlobalVariables.curtime = tick * host_state.interval_per_tick;
  g_ClientGlobalVariables.frametime =
      (tick - oldtickcount) *
      host_state
          .interval_per_tick;  // We used to call GetFrameTime() here, but
                               // 'insimulation' is always true so we have this
                               // code right in here to keep it simple.

  return true;
}

bool CClientState::ProcessStringCmd(NET_StringCmd *msg) {
  // If we're connect to an HLTV server and it's giving us a connect command,
  // then just do it. Otherwise, use the base class version, which performs
  // filtering.
...
bool CClientState::ProcessServerInfo(SVC_ServerInfo *msg) {
  ...
  // Don't verify the map and player .mdl crc's until after any missing
  // resources have been downloaded.  This will still occur before requesting
  // the rest of the signon.
...
bool CClientState::ProcessClassInfo(SVC_ClassInfo *msg) {
  ...
  if (!RecvTable_CreateDecoders(
          serverGameDLL
              ->GetStandardSendProxies()))  // create receive table decoders
  {
    Host_EndGame(true,
...
bool CClientState::ProcessVoiceData(SVC_VoiceData *msg) {
  ...
  if (Q_strlen(cl_voice_filter.GetString()) > 0 &&
      Q_strstr(playerinfo.name, cl_voice_filter.GetString()) == NULL)
    return true;

  // Data length can be zero when the server is just acking a client's voice
  // data.
  ...
  if (nChannel == VOICE_CHANNEL_ERROR) {
    // Create a channel in the voice engine and a channel in the sound engine
    // for this guy.
  ...
  // Give the voice engine the data (it in turn gives it to the mixer for the
  // sound engine).
...
bool CClientState::ProcessSounds(SVC_Sounds *msg) {
  ...
  for (int i = 0; i < msg->m_nNumSounds; i++) {
    ...
    // Add all received sounds to sorted queue (sounds may arrive in multiple
    // messages),
...
bool CClientState::ProcessBSPDecal(SVC_BSPDecal *msg) {
  ...
  if (msg->m_nEntityIndex) {
    ...
    if (!model) {
      Warning(
          "ProcessBSPDecal:  Trying to project on world before "
          "host_state.worldmodel is set!!!\n");
    }
  }
  ...
  if (model == NULL) {
    ...
    Warning(
        "Warning! Static BSP decal (%s), on NULL model index %i for entity "
        "index %i.\n",
        matname, msg->m_nModelIndex, msg->m_nEntityIndex);
    return true;
  }

  if (r_decals.GetInt()) {
    g_pEfx->DecalShoot(msg->m_nDecalTextureIndex, msg->m_nEntityIndex, model,
                       vec3_origin, vec3_angle, msg->m_Pos, NULL,
...
bool CClientState::ProcessUserMessage(SVC_UserMessage *msg) {
  ...
  uint8_t userdata[MAX_USER_MSG_DATA] = {0};
...
bool CClientState::ProcessEntityMessage(SVC_EntityMessage *msg) {
  ...
  IClientNetworkable *entity =
      entitylist->GetClientNetworkable(msg->m_nEntityIndex);

  if (!entity) {
    // message was send to use, even we don't have this entity TODO change that
    // on server side
    return true;
  }
  ...
  // buffer for incoming user message
  uint8_t entityData[MAX_ENTITY_MSG_DATA] = {0};
...
bool CClientState::ProcessPacketEntities(SVC_PacketEntities *msg) {
  ...
  if (!msg->m_bIsDelta) {
...
#endif
    // Tell prediction that we're recreating entities due to an uncompressed
    // packet arriving
    ...
    if (m_nDeltaTick == -1) {
      // we requested a full update but still got a delta compressed packet.
      // ignore it.
      return true;
    }
  }
  ...
  if (g_pLocalNetworkBackdoor) {
    ...
    // ignore message, all entities are transmitted using fast local memcopy
    // routines
    m_nDeltaTick = GetServerTickCount();
    return true;
  }

  if (!CL_ProcessPacketEntities(msg)) return false;
...
bool CClientState::ProcessTempEntities(SVC_TempEntities *msg) {
  ...
  for (int i = 0; i < msg->m_nNumEntries; i++) {
    ...
    if (buffer.ReadOneBit()) {
      ...
      RecvTable_MergeDeltas(pClientClass->m_pRecvTable, &fromBuf, &buffer,
                            &toBuf);
    }
