...
#define S2A_INFO_SRC \
  'I'  // + Address, hostname, map, gamedir, gamedescription, active players,
       // maxplayers, protocol
#define S2M_HEARTBEAT \
  'a'  // + challeange + sequence + active + #channels + channels
#define S2M_HEARTBEAT2 '0'  // New style heartbeat
#define S2M_SHUTDOWN 'b'    // no params
#define M2A_CHALLENGE 's'   // + challenge value
#define M2S_REQUESTRESTART \
  'O'  // HLMaster rejected a server's connection because the server needs to be
       // updated
#define A2S_RULES 'V'  // request rules list from server
#define S2A_RULES 'E'  // + number of rules + string key and string value pairs
#define A2S_INFO \
  'T'  // server info request - this must match the Goldsrc engine
...
bool Info_IsKeyImportant(const char *key) {
  ...
  if (!Q_strcmp(key, "name")) return true;
  if (!Q_strcmp(key, "model")) return true;
  if (!Q_strcmp(key, "rate")) return true;
  if (!Q_strcmp(key, "cl_updaterate")) return true;
  if (!Q_strcmp(key, "cl_lw")) return true;
  if (!Q_strcmp(key, "cl_lc")) return true;
  if (!Q_strcmp(key, "tracker")) return true;
...
void Info_SetValueForStarKey(char *s, const char *key, const char *value,
  ...
  if (!value || !Q_strlen(value)) return;
  ...
  if ((int)(Q_strlen(newArray) + Q_strlen(s)) >= maxsize) {
    ...
    if (Info_IsKeyImportant(key)) {
      ...
      } while (((int)(Q_strlen(newArray) + Q_strlen(s)) >= maxsize) &&
               *largekey != 0);
...
class CMaster : public IMaster {
 public:
  ...
  bool IsCorrectMasterChallenge(netadr_t *from, int challenge,
                                CBaseServer *pServer);
  ...
  virtual void HandleUnknown(struct netpacket_s *packet, CBaseServer *pServer,
 ...
 private:
  ...
  // If nomaster is true, the server will not send heartbeats to the master
  // server
  bool m_bNoMasters;
...
#define HEARTBEAT_SECONDS \
  300                  // default time between sending heartbeats to the masters
...
void CMaster::SendHeartbeat(netadr_t &adr, int challangenr,
                            CBaseServer *pServer) {
  ...
  Info_SetValueForKey(info, "protocol",
                      va("%i", MASTER_SERVER_PROTOCOL_VERSION), MAX_SINFO);
  Info_SetValueForKey(info, "challenge", va("%i", challangenr), MAX_SINFO);
  Info_SetValueForKey(info, "players", va("%i", players), MAX_SINFO);
  Info_SetValueForKey(info, "max", va("%i", pServer->GetMaxClients()),
                      MAX_SINFO);
  Info_SetValueForKey(info, "bots", va("%i", pServer->GetNumFakeClients()),
                      MAX_SINFO);
  ...
  Info_SetValueForKey(info, "lan", Steam3Server().BLanOnly() ? "1" : "0",
                      MAX_SINFO);
  ...
  if (pServer->IsHLTV()) {
    ...
    Info_SetValueForKey(info, "secure", "0",
                        MAX_SINFO);  // never requires a secure client
    Info_SetValueForKey(info, "version", "1.0.0.0", MAX_SINFO);
    Info_SetValueForKey(info, "product", "srctv", MAX_SINFO);

    // Info_SetValueForKey( info, "proxy", hltv->IsMasterProxy()?"1":"2",
    // MAX_SINFO );
  } else  // game server
  {
    Info_SetValueForKey(info, "type", pServer->IsDedicated() ? "d" : "l",
                        MAX_SINFO);
    Info_SetValueForKey(info, "secure", Steam3Server().BSecure() ? "1" : "0",
                        MAX_SINFO);
  ...
  NET_SendPacket(NULL, pServer->m_Socket, adr, (unsigned char *)message,
                 Q_strlen(message));
}

CHeartbeat *CMaster::GetHeartbeat(CMasterserver *pMasterServer,
                                  CBaseServer *pServer) {
...
void CMaster::CheckHeartbeat(CBaseServer *pServer) {
  ...
  if (m_bNoMasters ||  // We are ignoring heartbeats
      !pServer->IsActive() || !pServer->IsMultiplayer() ||
      (serverGameDLL && serverGameDLL->ShouldHideServer())) {
    return;
  }
  ...
  for (int i = 0; i < m_MasterServers.Count(); i++) {
    ...
    if (!pHeartbeat) continue;

    // Time for another try?
    if ((net_time - pHeartbeat->last_heartbeat) <
        m_iHeartBeatTimeout)  // not time to send yet
      continue;

    // Should we resend challenge request?
    if (pHeartbeat->challengetime > 0 &&
        (net_time - pHeartbeat->challengetime) < HB_TIMEOUT)
      continue;

    pHeartbeat->challengetime = net_time;
    pHeartbeat->last_heartbeat =
        net_time;  // Flag at start so we don't just keep trying for hb's when
...
//-----------------------------------------------------------------------------
// Purpose: Server is shutting down, unload master servers list, tell masters
// that we are closing the server
//-----------------------------------------------------------------------------
void CMaster::ShutdownConnection(CBaseServer *pServer) {
  ...
  if (m_bNoMasters ||               // We are ignoring heartbeats
      ...
      (serverGameDLL &&
       serverGameDLL->ShouldHideServer()))  // server doesn't want to be seen
  {
    return;
  }
  ...
  for (int i = 0; i < m_MasterServers.Count(); i++) {
    ...
    NET_SendPacket(NULL, pServer->m_Socket, m_MasterServers[i].addr,
                   (unsigned char *)message, Q_strlen(message) + 1);
...
bool CMaster::AddServer(netadr_t *adr) {
  if (IsMasterServerAddress(adr)) return false;  // already in list
...
#error "Define the steam.dll I should load!"
  ...
  typedef int (*PFSteamFindServersIterateServer)(
...
#define GetProcAddress dlsym
    ...
    pfnSteamFindServersNumServers =
        (PFSteamFindServersNumServers)GetProcAddress(
            m_hSteamDLL, "SteamFindServersNumServers");
    pfnSteamFindServersIterateServer =
        (PFSteamFindServersIterateServer)GetProcAddress(
            m_hSteamDLL, "SteamFindServersIterateServer");
  }

  if (!m_hSteamDLL || !pfnSteamFindServersNumServers ||
      !pfnSteamFindServersIterateServer) {
  ...
  if (numServers == eSteamFindSteamServersLibraryError) {
    ...
    for (unsigned int i = 0; i < numServers; i++) {
      ...
      if (VCRGetMode() != VCR_Playback) {
        pfnSteamFindServersIterateServer(eSteamHalfLife2MasterServer, i, szAdr,
                                         sizeof(szAdr));
      }
#if !defined(NO_VCR)
      ...
      if (!NET_StringToAdr(szAdr, &adr)) continue;
  ...
  if (NET_IsDedicated() && sv_region.GetInt() == -1) {
    ...
    // on the command line\n" );  ConMsg( "You must set sv_region in your
...
bool CMaster::IsCorrectMasterChallenge(netadr_t *from, int challenge,
                                       CBaseServer *pServer) {
  // check it came from a master server we heartbeat too and its the same
  // heartbeat challenge we use

  for (int i = 0; i < m_MasterServers.Count(); i++) {
    if (!m_MasterServers[i].addr.CompareAdr(*from)) continue;
...
void CMaster::HandleUnknown(netpacket_t *packet, CBaseServer *pServer,
                            CGameServer *pGameServer) {
  ...
  switch (c) {
    ...
    case M2S_REQUESTRESTART:
      ...
      if (IsMasterServerAddress(&packet->from)) {
        ...
        if (!msg.IsOverflowed() &&
            IsCorrectMasterChallenge(&packet->from, challenge, pServer)) {
...
#endif
          {
            Msg("Your server needs to be restarted in order to receive the "
                "latest update.\n");
            Log("Your server needs to be restarted in order to receive the "
                "latest update.\n");
          } else {
    ...
    case A2S_PLAYER: {
      ...
      if (pServer->ValidChallenge(packet->from, challengeNr)) {
        if (pGameServer) ReplyPlayers(packet->from, pGameServer);
      }
      break;
    }
...
void CMaster::ReplyInfo(netadr_t &adr, CBaseServer *pServer) {
  if (serverGameDLL && serverGameDLL->ShouldHideServer()) return;
  ...
  uint32_t appID = g_iSteamAppID;
  buf.WriteShort(appID);

  // this is a quick workaround from goldsrc for the admin mod reserved slots UI
  // problem
  int visibleClients = pServer->GetMaxClients();
  if (!pServer->IsHLTV() && sv_visiblemaxplayers.GetInt() > 0 &&
      sv_visiblemaxplayers.GetInt() < pServer->GetMaxClients()) {
...
#define S2A_EXTRA_DATA_HAS_GAME_PORT \
  0x80  // Next 2 bytes include the game port.
#define S2A_EXTRA_DATA_HAS_SPECTATOR_DATA \
  0x40  // Next 2 bytes include the spectator port, then the spectator server
        // name.
#define S2A_EXTRA_DATA_HAS_GAMETAG_DATA \
  0x20  // Next bytes are the game tag string
  ...
  NET_SendPacket(NULL, pServer->m_Socket, adr, buf.GetData(),
...
void CMaster::ReplyRules(netadr_t &from, CBaseServer *pServer) {
  ...
  NET_SendPacket(NULL, pServer->m_Socket, from, buf.GetData(),
                 buf.GetNumBytesWritten());
}
