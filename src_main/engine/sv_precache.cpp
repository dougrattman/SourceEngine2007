...
int SV_ModelIndex(const char *name) { return sv.LookupModelIndex(name); }
...
int SV_SoundIndex(const char *name) { return sv.LookupSoundIndex(name); }
...
int SV_GenericIndex(const char *name) { return sv.LookupGenericIndex(name); }
...
// Output : int
//-----------------------------------------------------------------------------
int SV_DecalIndex(const char *name) { return sv.LookupDecalIndex(name); }
...
//			&maxs -
//-----------------------------------------------------------------------------
void SV_ForceModelBounds(const char *name, const Vector &mins,
                         const Vector &maxs) {
...
// Output : int
//-----------------------------------------------------------------------------
int CGameServer::PrecacheModel(char const *name, int flags,
                               model_t *model /*=NULL*/) {
  if (!m_pModelPrecacheTable) return -1;
  ...
  CPrecacheUserData const *pExisting =
      (CPrecacheUserData const *)m_pModelPrecacheTable->GetStringUserData(idx,
                                                                          NULL);
  if (!pExisting) {
    p.flags = flags;
  } else {
  ...
  if (idx != 0) {
    if (bLoadNow) {
      slot->SetModel(modelloader->GetModelForName(
          name, IModelLoader::FMODELLOADER_SERVER));
#ifndef SWDS
...
model_t *CGameServer::GetModel(int index) {
  if (index <= 0 || !m_pModelPrecacheTable) return NULL;
  ...
  m = modelloader->GetModelForName(modelname,
...
int CGameServer::PrecacheSound(char const *name, int flags) {
  ...
  // mark the sound as being precached, but check first that reslist generation
  // is enabled to save on the va() call
  if (MapReslistGenerator().IsEnabled() && name[0]) {
    MapReslistGenerator().OnResourcePrecached(
        va("sound/%s", PSkipSoundChars(name)));
  }

  // first time, set file size & flags
  CPrecacheUserData p;
  CPrecacheUserData const *pExisting =
      (CPrecacheUserData const *)m_pSoundPrecacheTable->GetStringUserData(idx,
                                                                          NULL);
  if (!pExisting) {
    p.flags = flags;
  } else {
...
int CGameServer::LookupSoundIndex(char const *name) {
  if (!m_pSoundPrecacheTable) return 0;
...
int CGameServer::PrecacheGeneric(char const *name, int flags) {
  if (!m_pGenericPrecacheTable) return -1;
  ...
  // first time, set file size & flags
  CPrecacheUserData const *pExisting =
      (CPrecacheUserData const *)m_pGenericPrecacheTable->GetStringUserData(
          idx, NULL);
  if (!pExisting) {
    p.flags = flags;
...
char const *CGameServer::GetGeneric(int index) {
  // Bogus index
  if (index < 0 || !m_pGenericPrecacheTable) return "";
...
int CGameServer::LookupGenericIndex(char const *name) {
  if (!m_pGenericPrecacheTable) return 0;
...
int CGameServer::PrecacheDecal(char const *name, int flags) {
  if (!m_pDecalPrecacheTable) return -1;
  ...
  // first time, set file size & flags
  CPrecacheUserData const *pExisting =
      (CPrecacheUserData const *)m_pDecalPrecacheTable->GetStringUserData(idx,
                                                                          NULL);
  if (!pExisting) {
    p.flags = flags;
  } else {
...
void CGameServer::DumpPrecacheStats(INetworkStringTable *table) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    int testLength;
    const CPrecacheUserData *p =
        (const CPrecacheUserData *)table->GetStringUserData(i, &testLength);
    ErrorIfNot(testLength == sizeof(*p),
               ("CGameServer::DumpPrecacheStats: invalid CPrecacheUserData "
                "length (%d)",
                testLength));
