...
CON_COMMAND_EXTERN(Test_WaitForCheckPoint, Test_WaitForCheckPoint, "") {
  if (args.ArgC() < 2) {
    Error(
        "Test_WaitForCheckPoint <checkpoint name> [once]: requires checkpoint "
        "name.");
    return;
  }
...
CON_COMMAND_EXTERN(Test_StartLoop, Test_StartLoop,
                   "Test_StartLoop <loop name> - Denote the start of a loop. "
                   "Really just defines a named point you can jump to.") {
  if (args.ArgC() < 2) {
...
CON_COMMAND_EXTERN(Test_LoopCount, Test_LoopCount,
                   "Test_LoopCount <loop name> <count> - loop back to the "
...
CON_COMMAND_EXTERN(Test_Loop, Test_Loop,
                   "Test_Loop <loop name> - loop back to the specified loop "
...
CON_COMMAND_EXTERN(
    ...
    "Test_LoopForNumSeconds <loop name> <time> - loop back to the specified "
    "start point for the specified # of seconds.") {
  if (args.ArgC() < 3) {
    Error(
        "Test_LoopLoopForNumSeconds: requires a loop name and number of "
        "seconds to loop.");
    return;
  }
...
CON_COMMAND(
    Test_RandomChance,
    "Test_RandomChance <percent chance, 0-100> <token1> <token2...> - Roll the "
    "dice and maybe run the command following the percentage chance.") {
  if (args.ArgC() < 3) {
    Error(
        "Test_RandomChance: requires percentage chance parameter (0-100) "
        "followed by command to execute.");
  }
...
CON_COMMAND(Test_StartScript, "Start a test script running..") {
  if (args.ArgC() < 2) {
    Warning(
        "Test_StartScript: requires filename of script to start (file must be "
        "under testscripts directory).\n");
  }
...
bool CTestScriptMgr::StartTestScript(const char *pFilename) {
  ...
  if (m_hFile == FILESYSTEM_INVALID_HANDLE) return false;
...
void CTestScriptMgr::CheckPoint(const char *pName) {
  ...
  // Either the timer expired, or we hit the checkpoint we were waiting for. Run
  // some more commands.
  m_NextCheckPoint[0] = 0;
  RunCommands();
}
...
CLoopInfo *CTestScriptMgr::FindLoop(const char *pLoopName) {
  FOR_EACH_LL(m_Loops, i) {
    if (Q_stricmp(pLoopName, m_Loops[i]->m_Name) == 0) return m_Loops[i];
  }
  return NULL;
}
...
void CTestScriptMgr::SetWaitCheckPoint(const char *pCheckPointName,
  ...
  if (bOnce &&
      m_CheckPointsHit.Find(pCheckPointName) != m_CheckPointsHit.InvalidIndex())
    return;
