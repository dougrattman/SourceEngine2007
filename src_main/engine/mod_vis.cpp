...
//-----------------------------------------------------------------------------
// Purpose: For each cluster to be OR'd into vis, remember the origin, the last
// viewcluster
...
// I think this is enough.  We should have enough here to cover what we might
// have in a frame, including: 1) water reflection 2) camera/monitor (actually,
// this is merged with the regular world) 3) 3dskybox 4) regular world
...
static void VisMark_Cached(const VisCacheEntry &cache,
                           const worldbrushdata_t &worldbrush) {
  int count, visframe;
...
static void VisCache_Build(VisCacheEntry &cache,
                           const worldbrushdata_t &worldbrush) {
...
bool Map_AreAnyLeavesVisible(const worldbrushdata_t &worldbrush, int *leafList,
                             int nLeaves) {
  for (int i = 0; i < nLeaves; i++) {
    ...
    if (vis.rgCurrentVis[cluster >> 3] & (1 << (cluster & 7))) return true;
  }
  return false;
}
...
void Map_VisMark(bool forcenovis, model_t *worldmodel) {
  ...
  for (i = 0; i < vis.nClusters; i++) {
    if (vis.rgVisClusters[i].viewcluster !=
        vis.rgVisClusters[i].oldviewcluster) {
      break;
    }
  }

  // No changes
  if (i >= vis.nClusters && !forcenovis &&
      (vis.nClusters == vis.oldnClusters)) {
    return;
  }
  ...
  CM_Vis(vis.rgCurrentVis, sizeof(vis.rgCurrentVis),
         vis.rgVisClusters[0].viewcluster, DVIS_PVS);
  ...
  for (i = viscache.Head(); i != viscache.InvalidIndex();
       i = viscache.Next(i)) {
...
//			visorigincount - how many origins to merge together (
// usually 1, can be 0 if forcenovis is true ) 			origins[][3] -
// array of origins to
// merge together 			forcenovis - if set to true, ignore all
// origins and just mark everything as visible ( SLOW rendering!!! )
//-----------------------------------------------------------------------------
void Map_VisSetup(model_t *worldmodel, int visorigincount,
                  const Vector origins[], bool forcenovis /*=false*/,
