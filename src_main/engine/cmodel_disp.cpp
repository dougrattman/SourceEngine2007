...
void CM_PreStab(TraceInfo_t *pTraceInfo, cleaf_t *pLeaf, Vector &vStabDir,
                int collisionMask, int &contents);
void CM_Stab(TraceInfo_t *pTraceInfo, Vector const &start,
             Vector const &vStabDir, int contents);
...
class CDispLeafBuilder {
 public:
  CDispLeafBuilder(CCollisionBSPData *pBSPData) {
    ...
    // size both of these to the size of the array since there is exactly one
    // per element
  ...
  void BuildLeafListForDisplacement(int index) {
    ...
    if (!pDispTree || (pDispTree->GetPower() == 0)) return;
  ...
  void WriteLeafList(unsigned short *pLeafList) {
    ...
    // now iterate the references in disp order adding to each leaf's (now
    // compact) list for each displacement with leaves
    for (int i = 0; i < m_leafCount.Count(); i++) {
      ...
      for (int j = 0; j < count; j++) {
        ...
        int leafIndex =
            m_dispList[listIndex];  // this reference is for one leaf
        cleaf_t *pLeaf = &m_pBSPData->map_leafs[leafIndex];
        int outListIndex = pLeaf->dispListStart +
                           pLeaf->dispCount;  // output position for this leaf
...
void CM_DispTreeLeafnum(CCollisionBSPData *pBSPData) {
  // check to see if there are any displacement trees to push down the bsp
  // tree??
...
class CVirtualTerrain : public IVirtualMeshEvent {
 public:
  CVirtualTerrain() { m_pDispHullData = NULL; }
  ...
  // Query against the AABB tree to find the list of triangles for this patch in
  // a sphere
  ...
  void LevelInit(dphysdisp_t *pLump, int lumpSize) {
    ...
    uint8_t *pData = (uint8_t *)(pDataSize + pLump->numDisplacements);
    memcpy(m_pDispHullData, pData, totalHullData);
#if _DEBUG
    int offset = pData - ((uint8_t *)pLump);
...
// List of terrain collision models for the currently loaded level, indexed by
// terrain patch index
static CUtlVector<CPhysCollide *> g_TerrainList;

// Find or create virtual terrain collision model.  Note that these will be
// shared by client & server
CPhysCollide *CM_PhysCollideForDisp(int index) {
  if (index < 0 || index >= g_DispCollTreeCount) return NULL;
...
void CM_CreateDispPhysCollide(dphysdisp_t *pDispLump, int dispLumpSize) {
  ...
  for (int i = 0; i < g_DispCollTreeCount; i++) {
    // Don't create a physics collision model for displacements that have been
    // tagged as such.
    CDispCollTree *pDispTree = &g_pDispCollTrees[i];
    if (pDispTree &&
        pDispTree->CheckFlags(CCoreDispInfo::SURF_NOPHYSICS_COLL)) {
...
// New Collision!
//-----------------------------------------------------------------------------
void CM_TestInDispTree(TraceInfo_t *pTraceInfo, cleaf_t *pLeaf,
                       const Vector &traceStart, const Vector &boxMin,
                       const Vector &boxMax, int collisionMask,
                       trace_t *pTrace) {
  bool bIsBox =
      ((boxMin.x != 0.0f) || (boxMin.y != 0.0f) || (boxMin.z != 0.0f) ||
  ...
  if (bIsBox) {
    ...
    for (int i = 0; i < pLeaf->dispCount; i++) {
      ...
      if (IsBoxIntersectingBox(absMins, absMaxs, pDispBounds->mins,
                               pDispBounds->maxs)) {
...
void CM_PreStab(TraceInfo_t *pTraceInfo, cleaf_t *pLeaf, Vector &vStabDir,
  ...
  // if the point wasn't in the bounded area of any of the displacements -- stab
  // in any direction and set contents to "solid"
  ...
  for (int i = 0; i < pLeaf->dispCount; i++) {
    int leafdispIndex =
        pTraceInfo->m_pBSPData->map_dispList[pLeaf->dispListStart + i];
    pDispTree = &g_pDispCollTrees[leafdispIndex];

    // Respect trace contents
    if (!(pDispTree->GetContents() & collisionMask)) continue;

    if (pDispTree->PointInBounds(pTraceInfo->m_start, pTraceInfo->m_mins,
                                 pTraceInfo->m_maxs, pTraceInfo->m_ispoint)) {
      pDispTree->GetStabDirection(vStabDir);
...
// New Collision!
//-----------------------------------------------------------------------------
void CM_Stab(TraceInfo_t *pTraceInfo, const Vector &start,
             const Vector &vStabDir, int contents) {
  //
  ...
  // increment the checkcount -- so we can retest objects that may have been
  // tested previous to the stab
...
void CM_PostTraceToDispTree(TraceInfo_t *pTraceInfo) {
  ...
  if (!pTraceInfo->m_bDispHit) return;

  //
  // determine whether or not we are in solid
  //
  if (DotProduct(pTraceInfo->m_trace.plane.normal, pTraceInfo->m_delta) >
      0.0f) {
...
void FASTCALL CM_TraceToDispTree(TraceInfo_t *pTraceInfo,
  ...
  //	CM_TraceToDispTreeTest( pDispTree, traceStart, traceEnd, boxMin, boxMax,
