...
static ConVar bugreporter_includebsp(
    "bugreporter_includebsp", "1", 0,
    "Include .bsp for internal bug submissions.");
static ConVar bugreporter_uploadasync("bugreporter_uploadasync", "0",
                                      FCVAR_ARCHIVE,
...
CBugReportUploadProgressDialog::CBugReportUploadProgressDialog(
    Panel *parent, const char *name, const char *title, const char *message)
    : Frame(parent, name) {
  SetSize(300, 160);
  SetSizeable(false);
  MoveToFront();
...
class CBugReportFinishedDialog : public vgui::Frame {
 public:
  CBugReportFinishedDialog(vgui::Panel *parent, const char *name,
                           const char *title, const char *message);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBugReportFinishedDialog::CBugReportFinishedDialog(Panel *parent,
                                                   const char *name,
                                                   const char *title,
                                                   const char *message)
    : Frame(parent, name) {
...
class CBugUIPanel : public vgui::Frame {
 ...
 public:
  ...
  inline bool IsTakingSnapshot() const { return m_bTakingSnapshot; }
 ...
 protected:
  ...
  bool UploadBugSubmission(char const *levelname, int bugId,

                           char const *savefile, char const *screenshot,
                           char const *bsp, char const *vmf,
  ...
  enum AutoAddScreenshot {
    eAutoAddScreenshot_Detect = 0,
    eAutoAddScreenshot_Add = 1,
    eAutoAddScreenshot_DontAdd = 2
  };
  ...
  // If true, then once directory for vmfs is selected, we act like the Include
  // .vmf button got pressed, too
  bool m_bAddVMF;
...
CBugUIPanel::CBugUIPanel(bool bIsPublic, vgui::Panel *parent)
  ...
  m_pTitle->SetMaximumCharCount(60);  // Titles can be 80 chars, but we put the
                                      // mapname in front, so limit to 60
  ...
  m_pChooseVMFFolder =
      new vgui::Button(this, "BugChooseVMFFolder", "Choose folder");
  m_pIncludeFile = new vgui::Button(this, "BugIncludeFile", "Include file...");
  m_pClearIncludes =
      new vgui::Button(this, "BugClearIncludedFiles", "Clear files");
...
void CBugUIPanel::Init() {
  ...
  if (m_hBugReporter) {
    ...
    if (factory) {
      m_pBugReporter =
          (IBugReporter *)factory(INTERFACEVERSION_BUGREPORTER, NULL);
      if (m_pBugReporter) {
        ...
        ConColorMsg(clr, "Couldn't get interface '%s' from '%s'\n",
                    ...
                    static_cast<const char *>(m_sDllName));
      }
    } else {
  ...
  Q_snprintf(m_szVMFContentDirFullpath, sizeof(m_szVMFContentDirFullpath),
             "%s/maps", com_gamedir);
...
void CBugUIPanel::OnTick() {
  ...
  if (!IsVisible()) {
    ...
    if (host_framecount < m_nSnapShotFrame + 2) return;
    ;
  ...
  if (m_flPauseTime > 0.0f) {
    if (m_flPauseTime <= system()->GetFrameTime()) {
      ...
      if (!m_hFinishedDialog.Get()) {
        m_hFinishedDialog = new CBugReportFinishedDialog(
            NULL, "FinishDialog", "#Steam_FinishedBug_WorkingTitle",
...
void CBugUIPanel::OnTakeSnapshot() {
  GetDataFileBase(GetSubmitter(), m_szScreenShotName,
                  sizeof(m_szScreenShotName));
...
void CBugUIPanel::OnSaveGame() {
  ...
  if (m_pBugReporter && m_pBugReporter->IsPublicUI()) {
    // External users send us a "minisave" file which doesn't contain data from
    // other previously encoudntered/connected levels
...
void CBugUIPanel::OnSaveVMF() {
  if (m_pBugReporter && m_pBugReporter->IsPublicUI()) return;
  ...
  char localfile[512];
  Q_snprintf(localfile, sizeof(localfile), "%s/%s.vmf",
             m_szVMFContentDirFullpath, level);
  if (!g_pFileSystem->FileExists(localfile, NULL)) {
    if (!m_hDirectorySelectDialog.Get()) {
      m_hDirectorySelectDialog =
          new DirectorySelectDialog(this, "Choose .vmf folder");
    }
...
void CBugUIPanel::OnFileSelected(char const *fullpath) {
  ...
  if (!g_pFileSystem->FullPathToRelativePath(fullpath, relativepath,
                                             sizeof(relativepath))) {
    if (Q_stristr(fullpath, com_basedir)) {
      Q_snprintf(relativepath, sizeof(relativepath), "..%s",
                 fullpath + strlen(com_basedir));
      baseDirFile = true;
    } else {
  ...
  if (baseDirFile) {
    Q_snprintf(inc.fixedname, sizeof(inc.fixedname), "%s",
               inc.name + 3);  // strip the "..\"
  } else {
  ...
  for (int i = 0; i < m_IncludedFiles.Count(); ++i) {
    Q_strncat(concat, m_IncludedFiles[i].name, sizeof(concat),
              COPY_ALL_CHARACTERS);
...
void CBugUIPanel::OnIncludeFile() {
  ...
  if (!m_hFileOpenDialog.Get()) {
    m_hFileOpenDialog =
        new FileOpenDialog(this, "Choose file to include", true);
    if (m_hFileOpenDialog != NULL) {
...
//-----------------------------------------------------------------------------
// Purpose: method to make sure the email address is acceptable to be used by
// steam
//-----------------------------------------------------------------------------
bool CBugUIPanel::IsValidEmailAddress(char const *email) {
  ...
  // make sure it has letters, then the '@', then letters, then '.', then
  // letters
  const char *sz = email;
  if (!isalnum(*sz)) return false;
  sz = strchr(sz, '@');
  if (!sz) return false;
  sz++;
  if (!isalnum(*sz)) return false;
...
bool CBugUIPanel::IsValidSubmission(bool verbose) {
  if (!m_pBugReporter) return false;
  ...
  char owner[256];
  Q_strncpy(owner, m_pBugReporter->GetDisplayName(m_pAssignTo->GetActiveItem()),
            sizeof(owner));
  ...
  if (isPublic) {
    ...
    if (email[0] != 0 && !IsValidEmailAddress(email)) {
      return false;
    }
  }
...
bool CBugUIPanel::AddBugTextToZip(char const *textfilename, char const *text,
                                  int textlen) {
  if (!m_hZip) {
...
bool CBugUIPanel::AddFileToZip(char const *relative) {
  ...
  char fullpath[512];
  if (g_pFileSystem->RelativePathToFullPath(relative, "GAME", fullpath,
                                            sizeof(fullpath))) {
    char extension[32];
...
void CBugUIPanel::OnSubmit() {
  ...
  Q_snprintf(misc, sizeof(misc),
             "Convars:\n\tskill:  %i\n\tnet:  rate %i update %i cmd %i latency "
             "%i msec\n\thost_thread_mode:  %i\n\tsv_alternateticks:  "
             "%i\n\tai_strong_optimizations:  %i\n",
             skill.GetInt(), cl_rate->GetInt(), (int)cl_updaterate->GetFloat(),
             (int)cl_cmdrate->GetFloat(), latency, host_thread_mode.GetInt(),
             sv_alternateticks.GetInt(), ai_strong_optimizations.GetInt());

  if (cl.IsActive() && g_ServerGlobalVariables.mapversion != 0 &&
      host_state.worldmodel) {
    // Note, this won't work in multiplayer, oh well...
    extern CGlobalVars g_ServerGlobalVariables;
    char misc2[256];

    long mapfiletime = g_pFileSystem->GetFileTime(
        modelloader->GetName(host_state.worldmodel), "GAME");
    if (!isPublic && mapfiletime != 0L) {
      char filetimebuf[64];
      g_pFileSystem->FileTimeToString(filetimebuf, sizeof(filetimebuf),
                                      mapfiletime);

      Q_snprintf(misc2, sizeof(misc2), "Map version:  %i\nFile timestamp:  %s",
                 g_ServerGlobalVariables.mapversion, filetimebuf);
  ...
  if (isPublic) {
    ...
    m_pBugReporter->SetOwner(
        m_pBugReporter->GetUserNameForDisplayName(assignedto));
  ...
  if (m_pBugReporter->IsPublicUI()) {
    ...
    // Don't bother uploading any attachment if it's just the info.txt file,
    // either...
    ...
    if (m_szSaveGameName[0]) {
      Q_snprintf(fn, sizeof(fn), "%s/BugId/%s.sav", GetRepositoryURL(),
                 m_szSaveGameName);
      Q_FixSlashes(fn);
      m_pBugReporter->SetSaveGame(fn);
    }
    if (m_szScreenShotName[0]) {
      Q_snprintf(fn, sizeof(fn), "%s/BugId/%s.jpg", GetRepositoryURL(),
                 m_szScreenShotName);
      Q_FixSlashes(fn);
      m_pBugReporter->SetScreenShot(fn);
    }
    if (m_szBSPName[0]) {
      Q_snprintf(fn, sizeof(fn), "%s/BugId/%s.bsp", GetRepositoryURL(),
                 m_szBSPName);
      Q_FixSlashes(fn);
      m_pBugReporter->SetBSPName(fn);
    }
    if (m_szVMFName[0]) {
      Q_snprintf(fn, sizeof(fn), "%s/BugId/%s.vmf", GetRepositoryURL(),
                 m_szVMFName);
      Q_FixSlashes(fn);
    ...
    if (m_IncludedFiles.Count() > 0) {
      for (int i = 0; i < m_IncludedFiles.Count(); ++i) {
        Q_snprintf(fn, sizeof(fn), "%s/BugId/%s", GetRepositoryURL(),
                   m_IncludedFiles[i].fixedname);
        Q_FixSlashes(fn);
  ...
  if (m_pBugReporter->IsPublicUI()) {
    m_pProgressDialog = new CBugReportUploadProgressDialog(
        NULL, "ProgressDialog", "#Steam_SubmittingBug_WorkingTitle",
        "#Steam_SubmittingBug_WorkingText");
    m_pProgressDialog->Activate();
    vgui::input()->SetAppModalSurface(m_pProgressDialog->GetVPanel());
    m_flPauseTime =
        (float)system()->GetFrameTime() + PUBLIC_BUGREPORT_WAIT_TIME;
  } else {
    OnFinishBugReport();
  }
}
...
void CBugUIPanel::OnFinishBugReport() {
  ...
  if (success) {
    ...
    if (!m_pBugReporter->IsPublicUI()) {
      if (!UploadBugSubmission(m_szLevel, bugId, m_szSaveGameName,
                               m_szScreenShotName, m_szBSPName, m_szVMFName,
                               m_IncludedFiles)) {
        Warning(
            "Unable to upload saved game and screenshot to bug repository!\n");
        success = false;
      }
    }
  } else {
...
bool CBugUIPanel::UploadFile(char const *local, char const *remote,
  ...
  if (!g_pFileSystem->IsSteam()) {
    ...
    uint8_t *pCopyBuf = new byte[nCopyBufferSize];
    Assert(pCopyBuf);
    if (!pCopyBuf) {
      Warning(
          "CBugUIPanel::UploadFile:  Unable to allocate copy buffer of %d "
          "bytes\n",
          nCopyBufferSize);
      fclose(r);
      g_pFileSystem->Close(hLocal);
...
bool CBugUIPanel::UploadBugSubmission(char const *levelname, int bugId,
                                      char const *savefile,
                                      char const *screenshot, char const *bsp,
                                      char const *vmf,
  ...
  if (savefile && savefile[0]) {
    Q_snprintf(localfile, sizeof(localfile), "%s/save/%s.sav", com_gamedir,
               savefile);
    Q_snprintf(remotefile, sizeof(remotefile), "%s/%s.sav",
               GetSubmissionURL(bugId), savefile);
    Q_FixSlashes(localfile);
    Q_FixSlashes(remotefile);

    g_UploadQueue.QueueCall(this, &CBugUIPanel::UploadFile,
                            CUtlEnvelope<const char *>(localfile),
                            CUtlEnvelope<const char *>(remotefile), false);
  }

  if (screenshot && screenshot[0]) {
    Q_snprintf(localfile, sizeof(localfile), "%s/screenshots/%s.jpg",
               com_gamedir, screenshot);
    Q_snprintf(remotefile, sizeof(remotefile), "%s/%s.jpg",
               GetSubmissionURL(bugId), screenshot);
    Q_FixSlashes(localfile);
    Q_FixSlashes(remotefile);
  ...
  if (bsp && bsp[0]) {
    ...
    char *pszMapPath;
    FileHandle_t hBsp =
        g_pFileSystem->OpenEx(localfile, "rb", 0, 0, &pszMapPath);
    if (!hBsp) {
      Q_snprintf(localfile, sizeof(localfile), "%s/maps/%s.bsp", com_gamedir,
                 levelname);
    } else {
    ...
    Q_snprintf(remotefile, sizeof(remotefile), "%s/%s.bsp",
               GetSubmissionURL(bugId), bsp);
    Q_FixSlashes(localfile);
    Q_FixSlashes(remotefile);
  ...
  if (vmf && vmf[0]) {
    Q_snprintf(localfile, sizeof(localfile), "%s.vmf",
               m_szVMFContentDirFullpath, levelname);
    if (g_pFileSystem->FileExists(localfile, NULL)) {
      Q_snprintf(remotefile, sizeof(remotefile), "%s/%s.vmf",
                 GetSubmissionURL(bugId), vmf);
      Q_FixSlashes(localfile);
      Q_FixSlashes(remotefile);
  ...
  if (files.Count() > 0) {
    ...
    int c = files.Count();
    for (int i = 0; i < c; ++i) {
      Q_snprintf(localfile, sizeof(localfile), "%s/%s", com_gamedir,
                 files[i].name);
      Q_snprintf(remotefile, sizeof(remotefile), "%s/%s",
                 GetSubmissionURL(bugId), files[i].fixedname);
      Q_FixSlashes(localfile);
      Q_FixSlashes(remotefile);
...
void CBugUIPanel::DetermineSubmitterName() {
  ...
  if (m_pBugReporter->IsPublicUI()) {
    ...
    char fn[512];
    Q_snprintf(fn, sizeof(fn), "%s/%s", GetRepositoryURL(),
    ...
    if (fp) {
      ...
      Warning(
          "Unable to see '%s', check permissions and network connectivity\n",
          fn);
      m_bCanSubmit = false;
    }
...
void CBugUIPanel::PopulateControls() {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (!V_strcasecmp(name, "Triage")) defitem = i;
  ...
  for (i = 0; i < c; i++) {
    ...
    if (!V_strcasecmp(severity, "Zero")) defitem = i;
...
// Purpose:
//-----------------------------------------------------------------------------
void CBugUIPanel::DenySound() { Cbuf_AddText(va("play %s\n", DENY_SOUND)); }
...
void CBugUIPanel::ParseDefaultParams(void) {
  ...
  while (pfile) {
    ...
    if (!success) {
      ...
      if (!success) {
        Msg("Unable to determine where to set default bug parameter '%s', "
            "ignoring...\n",
            token);
      }
    }
...
void CBugUIPanel::ParseCommands(const CCommand &args) {
  ...
  for (int i = 1; i < args.ArgC(); i++) {
    ...
    if (!success) {
      ...
      if (!success) {
        Msg("Unable to determine where to set bug parameter '%s', "
            "ignoring...\n",
            args[i]);
      }
    }
...
bool CBugUIPanel::Compare(char const *value, char const *token, bool partial) {
  if (!partial) {
    if (!Q_stricmp(value, token)) return true;
  } else {
...
void CEngineBugReporter::Init() { m_ParentPanel = 0; }

void CEngineBugReporter::Shutdown() {
  if (g_pBugUI) {
    g_pBugUI->Shutdown();
    g_pBugUI = NULL;
  }
...
bool CEngineBugReporter::ShouldPause() const {
  return g_pBugUI && (g_pBugUI->IsVisible() || g_pBugUI->IsTakingSnapshot()) &&
         (cl.m_nMaxClients == 1);
}
...
int CBugUIPanel::GetArea() {
  ...
  for (int i = 0; i < m_pBugReporter->GetAreaMapCount(); i++) {
    ...
    if (pszAreaDir && pszAreaPrefix) {
      if (!Q_strcmp(szDirectory, gamedir) && Q_strstr(mapname, pszAreaPrefix)) {
        return i + 1;
      }
    } else if (pszAreaDir && !pszAreaPrefix) {
