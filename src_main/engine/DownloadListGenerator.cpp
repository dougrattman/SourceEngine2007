...
void CDownloadListGenerator::SetStringTable(INetworkStringTable *pStringTable) {
  ...
  // no customized resource file found
...
void CDownloadListGenerator::OnLevelLoadStart(const char *levelName) {
  ...
  // add a slash to the end of com_gamedir, so we can only deal with files for
  // this mod
...
//-----------------------------------------------------------------------------
void CDownloadListGenerator::OnModelPrecached(
...
//-----------------------------------------------------------------------------
void CDownloadListGenerator::OnSoundPrecached(
    const char *relativePathFileName) {
...
//-----------------------------------------------------------------------------
void CDownloadListGenerator::OnResourcePrecached(
    const char *relativePathFileName) {
  // ignore empty string
...
//-----------------------------------------------------------------------------
void CDownloadListGenerator::ForceExactFile(const char *relativePathFileName,
                                            ConsistencyType consistency) {
  if (!m_pStringTable) return;

  if (consistency != CONSISTENCY_EXACT &&
  ...
  if (error) {
    ...
    if (index != INVALID_STRING_INDEX) {
      m_pStringTable->SetStringUserData(index, sizeof(ExactFileUserData),
                                        &userData);
    } else {
      m_pStringTable->AddString(true, relativeFileName,
                                sizeof(ExactFileUserData), &userData);
    }
  }
}
...
//-----------------------------------------------------------------------------
void CDownloadListGenerator::ForceModelBounds(const char *relativePathFileName,
                                              const Vector &mins,
                                              const Vector &maxs) {
  if (!m_pStringTable) return;

  if (!relativePathFileName) relativePathFileName = "";
  ...
  int index = m_pStringTable->FindStringIndex(relativeFileName);
  if (index != INVALID_STRING_INDEX) {
    m_pStringTable->SetStringUserData(index, sizeof(ModelBoundsUserData),
                                      &userData);
  } else {
    m_pStringTable->AddString(true, relativeFileName,
                              sizeof(ModelBoundsUserData), &userData);
  }
}
...
//-----------------------------------------------------------------------------
void CDownloadListGenerator::OnResourcePrecachedFullPath(
