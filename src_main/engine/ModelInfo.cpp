...
//-----------------------------------------------------------------------------
static void R_StudioGetLightingCenter(IClientRenderable *pRenderable,
...
static int R_StudioBodyVariations(studiohdr_t *pstudiohdr) {
  ...
  // Each body part has nummodels variations so there are as many total
  // variations as there are in a matrix of each part by each other part
...
static int ModelFrameCount(model_t *model) {
  ...
  if (model->type == mod_sprite) {
    ...
    count =
        R_StudioBodyVariations((studiohdr_t *)modelloader->GetExtraData(model));
  }
...
class CModelInfo : public IVModelInfoClient {
 public:
  ...
  virtual void RecomputeTranslucency(
      const model_t *model, int nSkin, int nBody,
  ...
  virtual uint8_t *GetAnimBlock(const studiohdr_t *pStudioHdr,
                                int iBlock) const;

  uint8_t *LoadAnimBlock(model_t *model, const studiohdr_t *pStudioHdr,
                         int iBlock, cache_user_t *cache) const;

  // NOTE: These aren't in the server version, but putting them here makes this
  // code easier to write Sets/gets a map-specified fade range
  ...
  int GetAutoplayList(const studiohdr_t *pStudioHdr,
                      unsigned short **pAutoplayList) const;
  CPhysCollide *GetCollideForVirtualTerrain(int index);
  virtual int GetSurfacepropsForVirtualTerrain(int index) {
    return CM_SurfacepropsForDisp(index);
  }

  virtual bool IsUsingFBTexture(
      const model_t *model, int nSkin, int nBody,
      void /*IClientRenderable*/ *pClientRenderable) const;

  virtual MDLHandle_t GetCacheHandle(const model_t *model) const {
    return (model->type == mod_studio) ? model->studio : MDLHANDLE_INVALID;
  }
  ...
  virtual void GetBrushModelPlane(const model_t *model, int nIndex,
                                  cplane_t &plane, Vector *pOrigin) const;
};
...
const model_t *CModelInfo::FindOrLoadModel(const char *name) const {
  ...
  // load the model
  return modelloader->GetModelForName(name,
...
void CModelInfo::GetModelBounds(const model_t *model, Vector &mins,
                                Vector &maxs) const {
  VectorCopy(model->mins, mins);
  VectorCopy(model->maxs, maxs);
}

void CModelInfo::GetModelRenderBounds(const model_t *model, Vector &mins,
                                      Vector &maxs) const {
  if (!model) {
    mins.Init(0, 0, 0);
    maxs.Init(0, 0, 0);
  ...
  switch (model->type) {
    case mod_studio: {
      studiohdr_t *pStudioHdr =
          (studiohdr_t *)modelloader->GetExtraData((model_t *)model);
      Assert(pStudioHdr);
...
int CModelInfo::GetModelSpriteWidth(const model_t *model) const {
  ...
  if (model->type != mod_sprite) return 0;
...
//-----------------------------------------------------------------------------
const studiohdr_t *CModelInfo::FindModel(const studiohdr_t *pStudioHdr,
                                         void **cache,
                                         char const *modelname) const {
...
//-----------------------------------------------------------------------------
virtualmodel_t *CModelInfo::GetVirtualModel(
    const studiohdr_t *pStudioHdr) const {
...
// Purpose:
//-----------------------------------------------------------------------------
uint8_t *CModelInfo::GetAnimBlock(const studiohdr_t *pStudioHdr,
                                  int nBlock) const {
  MDLHandle_t handle = (MDLHandle_t)(uintptr_t)pStudioHdr->virtualModel;
  return g_pMDLCache->GetAnimBlock(handle, nBlock);
}

int CModelInfo::GetAutoplayList(const studiohdr_t *pStudioHdr,
                                unsigned short **pAutoplayList) const {
  MDLHandle_t handle = (MDLHandle_t)(uintptr_t)pStudioHdr->virtualModel;
...
virtualmodel_t *studiohdr_t::GetVirtualModel(void) const {
  ...
  return g_pMDLCache->GetVirtualModelFast(this,
...
int studiohdr_t::GetAutoplayList(unsigned short **pOut) const {
  return g_pMDLCache->GetAutoplayList((MDLHandle_t)(uintptr_t)virtualModel,
                                      pOut);
}
...
bool CModelInfo::IsUsingFBTexture(
    const model_t *model, int nSkin, int nBody,
    void /*IClientRenderable*/ *pClientRenderable) const {
  bool bMightUseFbTextureThisFrame =
      (model && (model->flags & MODELFLAG_STUDIOHDR_USES_FB_TEXTURE));
      ...
      case mod_brush: {
        for (int i = 0; i < model->brush.nummodelsurfaces; ++i) {
          SurfaceHandle_t surfID = SurfaceHandleFromIndex(
              model->brush.firstmodelsurface + i, model->brush.pShared);
          IMaterial *material =
              MSurf_TexInfo(surfID, model->brush.pShared)->material;
          if (material != NULL) {
...
void CModelInfo::RecomputeTranslucency(
    const model_t *model, int nSkin, int nBody,
    void /*IClientRenderable*/ *pClientRenderable) {
  if (model != NULL) {
    Mod_RecomputeTranslucency((model_t *)model, nSkin, nBody,
                              pClientRenderable);
  }
}
...
void CModelInfo::GetModelMaterials(const model_t *model, int count,
                                   IMaterial **ppMaterials) {
  if (model) Mod_GetModelMaterials((model_t *)model, count, ppMaterials);
}
...
void CModelInfo::GetIlluminationPoint(const model_t *model,
  ...
  if (pStudioHdr) {
    R_StudioGetLightingCenter(pRenderable, pStudioHdr, origin, angles,
                              pLightingOrigin);
  } else {
...
vcollide_t *CModelInfo::GetVCollide(int modelIndex) const {
  ...
  // Second model( index 1 ) is the world, then brushes/submodels, then players,
  // etc. So, we must subtract 1 from the model index to map modelindex to CM_
  // index in cmodels, 0 is the world, then brushes, etc.
...
const char *CModelInfo::GetModelKeyValueText(const model_t *model) {
  if (!model || model->type != mod_studio) return NULL;
...
bool CModelInfo::GetModelKeyValue(const model_t *model, CUtlBuffer &buf) {
  if (!model || model->type != mod_studio) return false;
...
int CModelInfo::GetBrushModelPlaneCount(const model_t *model) const {
  if (!model || model->type != mod_brush) return 0;

  return R_GetBrushModelPlaneCount(model);
}

void CModelInfo::GetBrushModelPlane(const model_t *model, int nIndex,
                                    cplane_t &plane, Vector *pOrigin) const {
...
class CModelInfoServer : public CModelInfo {
 public:
  ...
  virtual void GetModelMaterialColorAndLighting(
      const model_t *model, const Vector &origin, const QAngle &angles,
      trace_t *pTrace, Vector &lighting, Vector &matColor);
};
...
void CModelInfoServer::GetModelMaterialColorAndLighting(
    const model_t *model, const Vector &origin, const QAngle &angles,
    trace_t *pTrace, Vector &lighting, Vector &matColor) {
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CModelInfoServer, IVModelInfo,
...
class CModelInfoClient : public CModelInfo {
 ...
 private:
  ...
  // Sets/gets a map-specified fade range
  void SetScreenFadeRange(float flMinSize, float flMaxSize,
                          ScreenFadeInfo_t *pFade);
  unsigned char ComputeScreenFade(const Vector &vecAbsOrigin, float flRadius,
                                  float flFadeScale,
                                  const ScreenFadeInfo_t &fade) const;
...
// Sets/gets a map-specified fade range
//-----------------------------------------------------------------------------
void CModelInfoClient::SetScreenFadeRange(float flMinSize, float flMaxSize,
                                          ScreenFadeInfo_t *pFade) {
  ...
  if (pFade->m_flMaxScreenWidth != pFade->m_flMinScreenWidth) {
    pFade->m_flFalloffFactor =
        255.0f / (pFade->m_flMaxScreenWidth - pFade->m_flMinScreenWidth);
  } else {
    pFade->m_flFalloffFactor = 255.0f;
  }
}

void CModelInfoClient::SetLevelScreenFadeRange(float flMinSize,
                                               float flMaxSize) {
  SetScreenFadeRange(flMinSize, flMaxSize, &m_LevelFade);
}

void CModelInfoClient::GetLevelScreenFadeRange(float *pMinArea,
                                               float *pMaxArea) const {
...
// Sets/gets a map-specified per-view fade range
//-----------------------------------------------------------------------------
void CModelInfoClient::SetViewScreenFadeRange(float flMinSize,
                                              float flMaxSize) {
...
// Computes fade alpha based on distance fade + screen fade
//-----------------------------------------------------------------------------
inline unsigned char CModelInfoClient::ComputeScreenFade(
    const Vector &vecAbsOrigin, float flRadius, float flFadeScale,
    const ScreenFadeInfo_t &fade) const {
  if ((fade.m_flMinScreenWidth <= 0) || (flFadeScale <= 0.0f)) return 255;

  CMatRenderContextPtr pRenderContext(materials);

  float flPixelWidth =
      pRenderContext->ComputePixelWidthOfSphere(vecAbsOrigin, flRadius) /
      flFadeScale;

  unsigned char alpha = 0;
  if (flPixelWidth > fade.m_flMinScreenWidth) {
    if ((fade.m_flMaxScreenWidth >= 0) &&
...
unsigned char CModelInfoClient::ComputeLevelScreenFade(
    const Vector &vecAbsOrigin, float flRadius, float flFadeScale) const {
  return ComputeScreenFade(vecAbsOrigin, flRadius, flFadeScale, m_LevelFade);
}

unsigned char CModelInfoClient::ComputeViewScreenFade(
    const Vector &vecAbsOrigin, float flRadius, float flFadeScale) const {
...
IMaterial *BrushModel_GetLightingAndMaterial(const Vector &start,
  ...
  // TEMP initialize these values until we can find why R_LightVec is not
  // assigning values to them
  textureS = 0;
  textureT = 0;

  SurfaceHandle_t surfID =
      R_LightVec(start, end, true, diffuseLightColor, &textureS, &textureT);
  if (!IS_SURF_VALID(surfID) || !MSurf_TexInfo(surfID)) {
    ...
    // material->GetName(), diffuseLightColor[0], diffuseLightColor[1],
    // diffuseLightColor[2], baseColor[0], baseColor[1], baseColor[2] );
    return material;
  }
}

void CModelInfoClient::GetModelMaterialColorAndLighting(
    const model_t *model, const Vector &origin, const QAngle &angles,
    trace_t *pTrace, Vector &lighting, Vector &matColor) {
  switch (model->type) {
    case mod_brush: {
    ...
    case mod_studio: {
      ...
      LightcacheGetDynamic(pTrace->endpos, lightingState, stats,
      ...
      for (int i = 0; i < lightingState.numlights; ++i) {
        if (WorldLightToMaterialLight(lightingState.locallight[i],
                                      desc[count])) {
          ++count;
        }
      }
      ...
      g_pStudioRender->ComputeLighting(lightingState.r_boxcolor, count, desc,
                                       pTrace->endpos, pTrace->plane.normal,
                                       lighting);
      return;
    }
  }
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CModelInfoClient, IVModelInfoClient,
                                  VMODELINFO_CLIENT_INTERFACE_VERSION,
                                  g_ModelInfoClient);
