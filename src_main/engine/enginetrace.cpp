...
ConVar debugrayenable(
    "debugrayenable", "0", NULL,
    "Use this to enable ray testing.  To reset: bind \"F1\" "
    "\"clearalloverlays; debugrayreset  0; host_framerate 66.66666667\"");
ConVar debugrayreset("debugrayreset", "0");
ConVar debugraylimit(
    "debugraylimit", "500", NULL,
    "number of rays per frame that you have to hit before displaying them all");
...
abstract_class CEngineTrace : public IEngineTrace {
 public:
  ...
  virtual void ClipRayToEntity(const Ray_t &ray, unsigned int fMask,
                               IHandleEntity *pEntity, trace_t *pTrace);
  ...
  // abs start + abs end represents the collision origins you want to sweep the
  // collideable through vecAngles represents the collision angles of the
  ...
  virtual void ClipRayToCollideable(const Ray_t &ray, unsigned int fMask,
                                    ICollideable *pEntity, trace_t *pTrace);
  ...
  // retrieve brush planes and contents, returns true if data is being returned
  // in the output pointers, false if the brush doesn't exist
  virtual bool GetBrushInfo(int iBrush, CUtlVector<Vector4D> *pPlanesOut,
                            int *pContentsOut);

  virtual bool PointOutsideWorld(
      const Vector
          &ptTest);  // Tests a point to see if it's outside any playable area
 ...
 private:
  ...
  bool ClipRayToCustom(const Ray_t &ray, unsigned int fMask,
                       ICollideable *pCollideable, trace_t *pTrace);

  // Perform vphysics trace
  bool ClipRayToVPhysics(const Ray_t &ray, unsigned int fMask,
                         ICollideable *pCollideable, studiohdr_t *pStudioHdr,
                         trace_t *pTrace);

  // Perform hitbox trace
  bool ClipRayToHitboxes(const Ray_t &ray, unsigned int fMask,
                         ICollideable *pCollideable, trace_t *pTrace);

  // Perform bsp trace
  bool ClipRayToBSP(const Ray_t &ray, unsigned int fMask,
                    ICollideable *pCollideable, trace_t *pTrace);

  // bbox
  bool ClipRayToBBox(const Ray_t &ray, unsigned int fMask,
                     ICollideable *pCollideable, trace_t *pTrace);

  // OBB
  bool ClipRayToOBB(const Ray_t &ray, unsigned int fMask, ICollideable *pEntity,
                    trace_t *pTrace);
...
class CEngineTraceServer : public CEngineTrace {
 private:
  virtual void HandleEntityToCollideable(IHandleEntity *pEnt,
                                         ICollideable **ppCollide,
                                         const char **ppDebugName);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTraceServer, IEngineTrace,
                                  INTERFACEVERSION_ENGINETRACE_SERVER,
                                  s_EngineTraceServer);

#ifndef SWDS
static CEngineTraceClient s_EngineTraceClient;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineTraceClient, IEngineTrace,
                                  INTERFACEVERSION_ENGINETRACE_CLIENT,
                                  s_EngineTraceClient);
#endif
...
ICollideable *CEngineTraceClient::GetCollideable(IHandleEntity *pEntity) {
  ...
  IClientUnknown *pUnk =
      entitylist->GetClientUnknownFromHandle(pEntity->GetRefEHandle());
...
#ifndef SWDS
int CEngineTraceClient::SpatialPartitionTriggerMask() const { return 0; }
#endif
...
class CPointContentsEnum : public IPartitionEnumerator {
 public:
  CPointContentsEnum(CEngineTrace *pEngineTrace, const Vector &pos)
      : m_Contents{CONTENTS_EMPTY},
        m_pCollide{nullptr},
        m_pEngineTrace{pEngineTrace},
        m_Pos{pos} {}

  static inline bool TestEntity(CEngineTrace *pEngineTrace,
                                ICollideable *pCollide, const Vector &vPos,
                                int *pContents,
    ...
    if (pModel && pModel->type == mod_brush) {
      Assert(pCollide->GetCollisionModelIndex() < MAX_MODELS &&
             pCollide->GetCollisionModelIndex() >= 0);
      int nHeadNode = GetModelHeadNode(pCollide);
      int contents = CM_TransformedPointContents(
          vPos, nHeadNode, pCollide->GetCollisionOrigin(),
          pCollide->GetCollisionAngles());
  ...
  IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    const char *pDbgName;
    m_pEngineTrace->HandleEntityToCollideable(pHandleEntity, &pCollide,
                                              &pDbgName);
    if (!pCollide) return ITERATION_CONTINUE;

    if (CPointContentsEnum::TestEntity(m_pEngineTrace, pCollide, m_Pos,
                                       &m_Contents, &m_pCollide))
 ...
 private:
  static int GetModelHeadNode(ICollideable *pCollide) {
    ...
    if (modelindex >= MAX_MODELS || modelindex < 0) return -1;
...
// Returns the contents mask at a particular world-space position
//-----------------------------------------------------------------------------
int CEngineTrace::GetPointContents(const Vector &vecAbsPosition,
                                   IHandleEntity **ppEntity) {
  VPROF("CEngineTrace_GetPointContents");
  //	VPROF_BUDGET( "CEngineTrace_GetPointContents",
  ...
  if (nContents != CONTENTS_SOLID) {
    ...
    SpatialPartition()->EnumerateElementsAtPoint(
        SpatialPartitionMask(), vecAbsPosition, false, &contentsEnum);

    int nEntityContents = contentsEnum.m_Contents;
    if (nEntityContents & MASK_CURRENT) nContents = CONTENTS_WATER;
...
int CEngineTrace::GetPointContents_Collideable(ICollideable *pCollide,
                                               const Vector &vecAbsPosition) {
  int contents = CONTENTS_EMPTY;
  ICollideable *pDummy;
  CPointContentsEnum::TestEntity(this, pCollide, vecAbsPosition, &contents,
                                 &pDummy);
  return contents;
}
...
// Should we perform the custom raytest?
//-----------------------------------------------------------------------------
inline bool CEngineTrace::ShouldPerformCustomRayTest(
    const Ray_t &ray, ICollideable *pCollideable) const {
  // No model? The entity's got its own collision detector maybe
  // Does the entity force box or ray tests to go through its code?
  return (
      (pCollideable->GetSolid() == SOLID_CUSTOM) ||
...
// Performs the custom raycast
//-----------------------------------------------------------------------------
bool CEngineTrace::ClipRayToCustom(const Ray_t &ray, unsigned int fMask,
...
//-----------------------------------------------------------------------------
bool CEngineTrace::ClipRayToHitboxes(const Ray_t &ray, unsigned int fMask,
                                     ICollideable *pCollideable,
                                     trace_t *pTrace) {
  trace_t hitboxTrace;
  ...
  if (!pCollideable->TestHitboxes(ray, fMask, hitboxTrace)) return false;
  ...
  if (!hitboxTrace.DidHit()) {
    ...
    *pTrace = hitboxTrace;
    ...
    // Fill out the surfaceprop details from the hitbox. Use the physics bone
    // instead of the hitbox bone
...
int CEngineTrace::GetStatByIndex(int index, bool bClear) {
  if (index >= NUM_TRACE_STAT_COUNTER) return 0;
...
static void FASTCALL GetBrushesInAABB_ParseLeaf(
  ...
  for (unsigned int i = 0; i != pLeaf->numleafbrushes; ++i) {
    ...
    if (pCounters[iBrushNumber]) continue;

    pCounters[iBrushNumber] = 1;

    if ((pBrush->contents & iContentsMask) == 0) continue;

    if (pBrush->IsBox()) {
      cboxbrush_t *pBox = &pBSPData->map_boxbrushes[pBrush->GetBox()];
      if (IsBoxIntersectingBox(pBox->mins, pBox->maxs, pExtents[0],
                               pExtents[7])) {
      ...
      for (j = 0; j != pBrush->numsides; ++j) {
        cplane_t *pPlane =
            pBSPData->map_brushsides[pBrush->firstbrushside + j].plane;

        if ((pExtents[pPlane->signbits].Dot(pPlane->normal) - pPlane->dist) >
            0.0f)
          break;  // the bounding box extent that was most likely to be
                  // encapsulated by the plane is outside the halfspace, brush
                  // not in bbox
      }
...
void CEngineTrace::GetBrushesInAABB(const Vector &vMins, const Vector &vMaxs,
                                    CUtlVector<int> *pOutput,
                                    int iContentsMask) {
  ...
  for (int i = 0; i != iNumLeafs; ++i)
    GetBrushesInAABB_ParseLeaf(ptBBoxExtents, pBSPData,
                               &pBSPData->map_leafs[pLeafList[i]], pOutput,
                               iContentsMask, counters.Base());
}

//-----------------------------------------------------------------------------
// Purpose: Used to copy the collision information of all displacement surfaces
// in a specified box Input  : vMins - min vector of the AABB
//			vMaxs - max vector of the AABB
// Output : CPhysCollide* the collision mesh created from all the displacements
// partially contained in the specified box
//					Note: We're not clipping to the box.
// Collidable may be larger than the box provided.
//-----------------------------------------------------------------------------
CPhysCollide *CEngineTrace::GetCollidableFromDisplacementsInAABB(
    const Vector &vMins, const Vector &vMaxs) {
  CCollisionBSPData *pBSPData = GetCollisionBSPData();
  ...
  // Get all the triangles for displacement surfaces in this box, add them to a
  // polysoup
  CPhysPolysoup *pDispCollideSoup = physcollision->PolysoupCreate();

  // Count total triangles added to this poly soup- Can't support more than
  // 65435.
  int iTriCount = 0;
  ...
  // For each leaf in which the box lies, Get all displacements in that leaf and
  // use their triangles to create the mesh
  for (int i = 0; i < iLeafCount; ++i) {
    // Current leaf
    cleaf_t curLeaf = pBSPData->map_leafs[pLeafList[i]];

    // Test box against all displacements in the leaf.
    for (int j = 0; j < curLeaf.dispCount; j++) {
      int dispIndex = pBSPData->map_dispList[curLeaf.dispListStart + j];
      CDispCollTree *pDispTree = &g_pDispCollTrees[dispIndex];

      // make sure we only check this brush once per trace/stab
      if (!pTraceInfo->Visit(pDispTree->m_iCounter, count, pCounters)) continue;

      // If this displacement doesn't touch our test box, don't add it to the
      // list.
      if (!IsBoxIntersectingBox(vMins, vMaxs, pDispTree->m_mins,
                                pDispTree->m_maxs))
        continue;
      ...
      // TODO: Return a list of collidables? How often do we break 64k
      // triangles?
      iTriCount += meshTriList.triangleCount;
      if (iTriCount > 65535) {
        AssertMsg(0,
                  "Displacement surfaces have too many triangles to duplicate "
                  "in GetCollidableFromDisplacementsInBox.");
        EndTrace(pTraceInfo);
        return NULL;
      }

      for (int k = 0; k < meshTriList.indexCount; k += 3) {
        // Don't index past the index list
        Assert(k + 2 < meshTriList.indexCount);

        if (k + 2 >= meshTriList.indexCount) {
          EndTrace(pTraceInfo);
        ...
        Assert(i0 < meshTriList.vertexCount && i1 < meshTriList.vertexCount &&
               i2 < meshTriList.vertexCount);

        if (i0 >= meshTriList.vertexCount || i1 >= meshTriList.vertexCount ||
            i2 >= meshTriList.vertexCount) {
          EndTrace(pTraceInfo);
          physcollision->PolysoupDestroy(pDispCollideSoup);
          return NULL;
        ...
        // We don't need exact clipping to the box... Include any triangle that
        // has at least one vert on the inside
        if (IsPointInBox(v0, vMins, vMaxs) || IsPointInBox(v1, vMins, vMaxs) ||
            IsPointInBox(v2, vMins, vMaxs)) {
          // This is for collision only, so we don't need to worry about
          // blending-- Use the first surface prop.
          int nProp = pDispTree->GetSurfaceProps(0);
          physcollision->PolysoupAddTriangle(pDispCollideSoup, v0, v1, v2,
                                             nProp);
        }
  ...
  CPhysCollide *pCollide =
      physcollision->ConvertPolysoupToCollide(pDispCollideSoup, false);
...
bool CEngineTrace::GetBrushInfo(int iBrush, CUtlVector<Vector4D> *pPlanesOut,
                                int *pContentsOut) {
  CCollisionBSPData *pBSPData = GetCollisionBSPData();
  ...
  if (pPlanesOut) {
    ...
    if (pBrush->IsBox()) {
      ...
      for (int i = 0; i < 6; i++) {
        ...
        pPlanesOut->AddToTail(pt);
      }
    } else {
      cbrushside_t *stopside =
          &pBSPData->map_brushsides[pBrush->firstbrushside];
      // Note:  Don't do this in the [] since the final one on the last
      // brushside will be past the end of the array end by one index
      stopside += pBrush->numsides;
      for (cbrushside_t *side =
               &pBSPData->map_brushsides[pBrush->firstbrushside];
           side != stopside; ++side) {
        Vector4D pt(side->plane->normal.x, side->plane->normal.y,
                    side->plane->normal.z, side->plane->dist);
        pPlanesOut->AddToTail(pt);
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Expose to the game dll a method for finding the leaf which contains
// a given point Input  : &vPos - Returns the leaf which contains this point
...
class CStudioConvexInfo : public IConvexInfo {
 public:
  CStudioConvexInfo(studiohdr_t *pStudioHdr) { m_pStudioHdr = pStudioHdr; }
...
// Perform vphysics trace
//-----------------------------------------------------------------------------
bool CEngineTrace::ClipRayToVPhysics(const Ray_t &ray, unsigned int fMask,
                                     ICollideable *pEntity,
                                     studiohdr_t *pStudioHdr, trace_t *pTrace) {
  if (pEntity->GetSolid() != SOLID_VPHYSICS) return false;
  ...
  if (pStudioHdr) {
    ...
    if (pCollide && pCollide->solidCount) {
      physcollision->TraceBox(
          ray, fMask, &studioConvex,
          pCollide->solids[0],  // UNDONE: Support other solid indices?!?!?!?
                                // (forced zero)
          pEntity->GetCollisionOrigin(), pEntity->GetCollisionAngles(), pTrace);
      bTraced = true;
    }
  } else {
    ...
    if (!ray.m_IsRay || pModel->type != mod_brush) {
      ...
      // BUGBUG: This only works when the vcollide in question is the first
      // solid in the model
      vcollide_t *pCollide =
          CM_VCollideForModel(nModelIndex, (model_t *)pModel);

      if (pCollide && pCollide->solidCount) {
        CBrushConvexInfo brushConvex;

        IConvexInfo *pConvexInfo =
            (pModel->type) == mod_brush ? &brushConvex : NULL;
        physcollision->TraceBox(
            ray, fMask, pConvexInfo,
            pCollide->solids[0],  // UNDONE: Support other solid indices?!?!?!?
                                  // (forced zero)
            pEntity->GetCollisionOrigin(), pEntity->GetCollisionAngles(),
            pTrace);
        bTraced = true;
      }
    }
...
// Perform bsp trace
//-----------------------------------------------------------------------------
bool CEngineTrace::ClipRayToBSP(const Ray_t &ray, unsigned int fMask,
                                ICollideable *pEntity, trace_t *pTrace) {
  int nModelIndex = pEntity->GetCollisionModelIndex();
  cmodel_t *pCModel = CM_InlineModelNumber(nModelIndex - 1);
  int nHeadNode = pCModel->headnode;

  CM_TransformedBoxTrace(ray, nHeadNode, fMask, pEntity->GetCollisionOrigin(),
                         pEntity->GetCollisionAngles(), *pTrace);
  return true;
}

// NOTE: Switched over to SIMD ray/box test since there is a bug we haven't
// hunted down yet in the scalar version
bool CEngineTrace::ClipRayToBBox(const Ray_t &ray, unsigned int fMask,
  ...
  // NOTE: If m_pRootMoveParent is set, then the boxes should be rotated into
  // the root parent's space
  if (!ray.m_IsRay && m_pRootMoveParent) {
    ...
    VectorITransform(pEntity->GetCollisionOrigin(), *m_pRootMoveParent,
                     localEntityOrigin);
    ...
    if (pTrace->DidHit()) {
      ...
      if (pTrace->fraction == 1) {
        ...
        VectorMA(pTrace->startpos, pTrace->fraction, ray.m_Delta,
                 pTrace->endpos);
      }
...
bool CEngineTrace::ClipRayToOBB(const Ray_t &ray, unsigned int fMask,
                                ICollideable *pEntity, trace_t *pTrace) {
  if (pEntity->GetSolid() != SOLID_OBB) return false;

  // NOTE: This is busted because it doesn't compute fractionleftsolid, which at
  // the moment is required for the engine trace system.
  IntersectRayWithOBB(ray, pEntity->GetCollisionOrigin(),
                      pEntity->GetCollisionAngles(), pEntity->OBBMins(),
                      pEntity->OBBMaxs(), DIST_EPSILON, pTrace);
  return true;
}
...
//-----------------------------------------------------------------------------
#ifndef SWDS
void CEngineTraceClient::SetTraceEntity(ICollideable *pCollideable,
                                        trace_t *pTrace) {
...
void CEngineTraceServer::SetTraceEntity(ICollideable *pCollideable,
                                        trace_t *pTrace) {
  if (!pTrace->DidHit()) return;
...
// Traces a ray against a particular edict
//-----------------------------------------------------------------------------
void CEngineTrace::ClipRayToCollideable(const Ray_t &ray, unsigned int fMask,
                                        ICollideable *pEntity,
                                        trace_t *pTrace) {
  CM_ClearTrace(pTrace);
  VectorAdd(ray.m_Start, ray.m_StartOffset, pTrace->startpos);
  ...
  if (pModel && pModel->type == mod_studio) {
    ...
    if (((fMask & CONTENTS_HITBOX) == 0)) {
      if ((fMask & pStudioHdr->contents) == 0) return;
    }
  }
  ...
  if (bIsStudioModel && !bTracedHitboxes && pTrace->DidHit() &&
      (!bCustomPerformed || pTrace->surface.surfaceProps == 0)) {
    pTrace->contents = pStudioHdr->contents;
    // use the default surface properties
    pTrace->surface.name = "**studio**";
    pTrace->surface.flags = 0;
    pTrace->surface.surfaceProps =
        physprop->GetSurfaceIndex(pStudioHdr->pszSurfaceProp());
  }
...
// Main entry point for clipping rays to entities
//-----------------------------------------------------------------------------
void CEngineTrace::ClipRayToEntity(const Ray_t &ray, unsigned int fMask,
                                   IHandleEntity *pEntity, trace_t *pTrace) {
...
class CEntitiesAlongRay : public IPartitionEnumerator {
 public:
  ...
  void Reset() { m_EntityHandles.RemoveAll(); }
...
class CEntityListAlongRay : public IPartitionEnumerator {
 public:
  ...
  CEntityListAlongRay() { m_nCount = 0; }
...
bool CEngineTrace::ClipTraceToTrace(trace_t &clipTrace, trace_t *pFinalTrace) {
  if (clipTrace.allsolid || clipTrace.startsolid ||
      (clipTrace.fraction < pFinalTrace->fraction)) {
...
//-----------------------------------------------------------------------------
void CEngineTraceServer::HandleEntityToCollideable(IHandleEntity *pHandleEntity,
                                                   ICollideable **ppCollide,
                                                   const char **ppDebugName) {
...
#ifndef SWDS
void CEngineTraceClient::HandleEntityToCollideable(IHandleEntity *pHandleEntity,
                                                   ICollideable **ppCollide,
                                                   const char **ppDebugName) {
  *ppCollide = StaticPropMgr()->GetStaticProp(pHandleEntity);
  if (*ppCollide) {
...
void EngineTraceRenderRayCasts() {
  ...
  if (debugrayenable.GetBool() &&
      s_FrameRays.Count() > debugraylimit.GetInt() && !debugrayreset.GetInt()) {
    Warning("m_FrameRays.Count() == %d\n", s_FrameRays.Count());
    debugrayreset.SetValue(1);
    int i;
    for (i = 0; i < s_FrameRays.Count(); i++) {
      Ray_t &ray = s_FrameRays[i];
      if (ray.m_Extents.x != 0.0f || ray.m_Extents.y != 0.0f ||
          ray.m_Extents.z != 0.0f) {
        CDebugOverlay::AddLineOverlay(ray.m_Start, ray.m_Start + ray.m_Delta,
                                      255, 0, 0, 255, true, 3600.0f);
      } else {
        CDebugOverlay::AddLineOverlay(ray.m_Start, ray.m_Start + ray.m_Delta,
                                      255, 255, 0, 255, true, 3600.0f);
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CEngineTrace::SetupLeafAndEntityListRay(const Ray_t &ray,
                                             CTraceListData &traceData) {
  if (!ray.m_IsSwept) {
  ...
  CM_RayLeafnums(ray, traceData.m_aLeafList.Base(), traceData.LeafCountMax(),
                 traceData.m_nLeafCount);

  // Find all the entities in the voxels that intersect this ray.
  traceData.EntityCountReset();
  SpatialPartition()->EnumerateElementsAlongRay(SpatialPartitionMask(), ray,
                                                false, &traceData);
}
...
//-----------------------------------------------------------------------------
void CEngineTrace::SetupLeafAndEntityListBox(const Vector &vecBoxMin,
                                             const Vector &vecBoxMax,
                                             CTraceListData &traceData) {
  ...
  traceData.LeafCountReset();
  traceData.m_nLeafCount =
      CM_BoxLeafnums(vecBoxMin, vecBoxMax, traceData.m_aLeafList.Base(),
                     traceData.LeafCountMax(), &iTopNode);

  // Find all entities in the voxels that intersect this box.
  traceData.EntityCountReset();
  SpatialPartition()->EnumerateElementsInBox(SpatialPartitionMask(), vecBoxMin,
                                             vecBoxMax, false, &traceData);
}
...
//-----------------------------------------------------------------------------
void CEngineTrace::TraceRayAgainstLeafAndEntityList(const Ray_t &ray,
                                                    CTraceListData &traceData,
                                                    unsigned int fMask,
  ...
  if (pTraceFilter->GetTraceType() != TRACE_ENTITIES_ONLY) {
    ...
    CM_BoxTraceAgainstLeafList(ray, traceData.m_aLeafList.Base(),
                               traceData.LeafCount(), fMask, true, *pTrace);
    SetTraceEntity(pCollide, pTrace);

    // Blocked by the world or early out because we only are tracing against the
    // world.
    if ((pTrace->fraction == 0) ||
        (pTraceFilter->GetTraceType() == TRACE_WORLD_ONLY))
      return;
  } else {
    // Set initial start and endpos.  This is necessary if the world isn't
    // traced against, because we may not trace against anything below.
    VectorAdd(ray.m_Start, ray.m_StartOffset, pTrace->startpos);
    VectorAdd(pTrace->startpos, ray.m_Delta, pTrace->endpos);
  }
  ...
  // Create a ray that extends only until we hit the world and adjust the trace
  // accordingly
  Ray_t entityRay = ray;
  VectorScale(entityRay.m_Delta, pTrace->fraction, entityRay.m_Delta);

  // We know this is safe because if pTrace->fraction == 0, we would have exited
  // above.
  ...
  bool bFilterStaticProps =
      pTraceFilter->GetTraceType() == TRACE_EVERYTHING_FILTER_PROPS;
...
CON_COMMAND(ray_save, "Save the rays") {
  ...
  if (count) {
    ...
    if (hFile) {
      ...
      g_pFileSystem->Write(s_BenchmarkRays.Base(),
                           sizeof(s_BenchmarkRays[0]) * count, hFile);
...
CON_COMMAND(ray_load, "Load the rays") {
  ...
  if (hFile) {
    ...
    if (count) {
      ...
      g_pFileSystem->Read(s_BenchmarkRays.Base(),
                          sizeof(s_BenchmarkRays[0]) * count, hFile);
    }
    g_pFileSystem->Close(hFile);
  }
...
#endif
  {
    ...
    for (int i = 0; i < s_BenchmarkRays.Count(); i++) {
      ...
      if (0) {
        ...
        // Create a ray that extends only until we hit the world and adjust the
        // trace accordingly
        ...
        enumerator.Reset();
        SpatialPartition()->EnumerateElementsAlongRay(
            PARTITION_ENGINE_SOLID_EDICTS, entityRay, false, &enumerator);
        trace_t tr;
        ICollideable *pCollideable;
        ...
        if (0) {
          ...
          for (int i = 0; i < nCount; ++i) {
            ...
            if (!StaticPropMgr()->IsStaticProp(pHandleEntity)) continue;
            if (entityRay.m_IsRay)
              rayVsProp++;
            else
              boxVsProp++;
            s_EngineTraceServer.HandleEntityToCollideable(
                pHandleEntity, &pCollideable, &pDebugName);
            s_EngineTraceServer.ClipRayToCollideable(entityRay, MASK_SOLID,
                                                     pCollideable, &tr);
    ...
    Msg("RAY TEST: %d hits, %d misses, %.2fms   (%d rays, %d sweeps) (%d "
        "ray/prop, %d box/prop)\n",
...
// A version that simply accepts a ray (can work as a traceline or tracehull)
//-----------------------------------------------------------------------------
void CEngineTrace::TraceRay(const Ray_t &ray, unsigned int fMask,
                            ITraceFilter *pTraceFilter, trace_t *pTrace) {
#if defined _DEBUG && !defined SWDS
  if (debugrayenable.GetBool()) {
  ...
  if (pTraceFilter->GetTraceType() != TRACE_ENTITIES_ONLY) {
    ...
    if (pTrace->startsolid) return;

    // Early out if we only trace against the world
    if (pTraceFilter->GetTraceType() == TRACE_WORLD_ONLY) return;
  } else {
  ...
  if (pTrace->fraction == 0) {
    ...
    // Explicitly compute end so that this computation happens at the
    // quantization of the output (endpos).  That way we won't miss any
    // intersections we would get by feeding these results back in to the tracer
    // This is not the same as entityRay.m_Delta *= pTrace->fraction which
    // happens at a quantization that is more precise as m_Start moves away from
    // the origin
    Vector end;
  ...
  CEntityListAlongRay enumerator;
  enumerator.Reset();
  SpatialPartition()->EnumerateElementsAlongRay(SpatialPartitionMask(),
                                                entityRay, false, &enumerator);
  ...
  VectorMA(vecOffset, pTrace->fraction, ray.m_Delta, vecEndTest);
  Assert(VectorsAreEqual(vecEndTest, pTrace->endpos, 0.1f));
//	Assert( !ray.m_IsRay || pTrace->allsolid || pTrace->fraction >=
// pTrace->fractionleftsolid );
#endif
...
void CEngineTrace::EnumerateEntities(const Ray_t &ray, bool bTriggers,
  ...
  int fMask =
      !bTriggers ? SpatialPartitionMask() : SpatialPartitionTriggerMask();

  // NOTE: Triggers currently don't exist on the client
  if (fMask) {
    SpatialPartition()->EnumerateElementsAlongRay(fMask, ray, false,
                                                  &enumerator);
  }
}
...
//-----------------------------------------------------------------------------
void CEngineTrace::EnumerateEntities(const Vector &vecAbsMins,
                                     const Vector &vecAbsMaxs,
                                     IEntityEnumerator *pEnumerator) {
  m_traceStatCounters[TRACE_STAT_COUNTER_ENUMERATE]++;
  // FIXME: If we store CBaseHandles directly in the spatial partition, this
...
CON_COMMAND(test_sweepaabb, "method to test out sweeps") {
  ...
  g_pEngineTraceServer->EnumerateEntities(
      MainViewOrigin() - Vector(200, 200, 200),
      MainViewOrigin() + Vector(200, 200, 200), &list);

  if (!list.m_pClosest) return;

  // Visualize the intersection test
  ICollideable *pCollide = list.m_pClosest;
  if (pCollide->GetCollisionOrigin() == vec3_origin) return;

  QAngle test(0, 45, 0);
#ifndef SWDS
  CDebugOverlay::AddBoxOverlay(
      pCollide->GetCollisionOrigin(), pCollide->OBBMins(), pCollide->OBBMaxs(),
  ...
  bool bIntersect =
      IntersectRayWithOBB(ray, pCollide->GetCollisionOrigin(), test,
                          pCollide->OBBMins(), pCollide->OBBMaxs(), 0.0f, &tr);
  ...
  CDebugOverlay::AddSweptBoxOverlay(tr.startpos, tr.endpos,
                                    Vector(-10, -20, -10), Vector(30, 30, 20),
                                    vec3_angle, r, g, b, a, 5.0);
#endif
}
