...
class CSocketCreator {
 public:
  ...
  // This method is used to connect to/disconnect from an external listening
  // socket creator Returns accepted socket index, or -1 if it failed. Use
  // GetAcceptedSocket* methods to access this socket's data if bSingleSocket ==
  // true, all accepted sockets are closed before the new one is opened NOTE:
  // Closing an accepted socket will re-index all the sockets with higher
  // indices
