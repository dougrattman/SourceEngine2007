...
inline void CDispInfo::DecalProjectVert(Vector const &vPos,
                                        CDispDecalBase *pDecalBase,
                                        ShadowInfo_t const *pInfo,
                                        Vector &out) {
  if (!pInfo) {
    CDispDecal *pDispDecal = static_cast<CDispDecal *>(pDecalBase);
    out.x = vPos.Dot(pDispDecal->m_TextureSpaceBasis[0]) -
            pDispDecal->m_pDecal->dx + .5f;
    out.y = vPos.Dot(pDispDecal->m_TextureSpaceBasis[1]) -
            pDispDecal->m_pDecal->dy + .5f;
    out.z = 0;
  } else {
...
void CDispInfo::TestAddDecalTri(int iIndexStart, u16 decalHandle,
  ...
  if (vNormal.Dot(pDecal->position - v0) >= pDispDecal->m_flSize) return;
  ...
  if (outCount > 2) {
    ...
    CDispDecalFragment *pFragment =
        AllocateDispDecalFragment(decalHandle, outCount);
    ...
    for (iVert = 0; iVert < outCount; iVert++) {
      ...
      //				pFragment->m_tCoords[iVert] =
      // pClipped[iVert].m_tCoords;
...
void CDispInfo::TestAddDecalTri(int iIndexStart, u16 decalHandle,
  ...
  int count = g_pShadowMgr->ProjectAndClipVertices(pDecal->m_Shadow, 3,
                                                   ppPosition, &ppClipVertex);
  if (count < 3) return;
  ...
  // Allocate a new fragment...
  CDispShadowFragment *pFragment =
      AllocateShadowDecalFragment(decalHandle, count);

  // Copy the fragment data in place
  pFragment->m_nVerts = count;

  for (int i = 0; i < count; ++i) {
    VectorCopy(ppClipVertex[i]->m_Position,
               pFragment->m_ShadowVerts[i].m_Position);
    VectorCopy(ppClipVertex[i]->m_ShadowSpaceTexCoord,
               pFragment->m_ShadowVerts[i].m_ShadowSpaceTexCoord);

    // Make sure it's been clipped
    Assert(pFragment->m_ShadowVerts[i].m_ShadowSpaceTexCoord[0] >= -1e-3f);
    Assert(pFragment->m_ShadowVerts[i].m_ShadowSpaceTexCoord[0] - 1.0f <=
           1e-3f);
    Assert(pFragment->m_ShadowVerts[i].m_ShadowSpaceTexCoord[1] >= -1e-3f);
    Assert(pFragment->m_ShadowVerts[i].m_ShadowSpaceTexCoord[1] - 1.0f <=
           1e-3f);
  }
...
void CDispInfo::CullDecals(int iNodeBit, CDispDecal **decals, int nDecals,
                           CDispDecal **childDecals, int &nChildDecals) {
...
void CDispInfo::TesselateDisplacement() {
  ...
  helper.m_pDisp = this;
  helper.m_IndexMesh.BeginModify(m_pMesh->m_pMesh, 0, 0, m_iIndexOffset,
                                 nMaxIndices);
  helper.m_pActiveVerts = m_ActiveVerts.Base();
  helper.m_pPowerInfo = GetPowerInfo();

  // Generate the indices.
  ::TesselateDisplacement<CEngineTesselateHelper>(
      &helper);  // (implemented in disp_tesselate.h)
...
void CDispInfo::SpecifyWalkableDynamicMesh() {
  ...
  IMesh *pMesh = pRenderContext->GetDynamicMesh(
      false, NULL, NULL, g_materialTranslucentSingleColor);
...
void CDispInfo::InitializeActiveVerts() {
  ...
  for (int iSide = 0; iSide < 4; iSide++) {
    ...
    if ((pSide->m_SubNeighbors[0].IsValid() &&
         pSide->m_SubNeighbors[0].m_Span != CORNER_TO_CORNER) ||
        (pSide->m_SubNeighbors[1].IsValid() &&
         pSide->m_SubNeighbors[1].m_Span != CORNER_TO_CORNER)) {
      int iEdgeDim = g_EdgeDims[iSide];

      CVertIndex nodeIndex;
      nodeIndex[iEdgeDim] =
          g_EdgeSideLenMul[iSide] * m_pPowerInfo->m_SideLengthM1;
...
bool DispInfoRenderDebugModes() {
  if (ShouldDrawInWireFrameMode() || mat_luxels.GetInt() ||
      r_DispWalkable.GetInt() || r_DispBuildable.GetInt()
#if !defined(SWDS)
...
bool CDispInfo::Render(CGroupMesh *pGroup, bool bAllowDebugModes) {
  ...
  // Trivial reject?
  if (R_CullBox(m_BBoxMin, m_BBoxMax, g_Frustum)) return false;
  ...
  if (bAllowDebugModes) {
    ...
    if (mat_surfaceid.GetInt()) {
      ...
      int nInt = (mat_surfaceid.GetInt() != 2)
                     ? (int)m_ParentSurfID
...
typedef void ProcessLightmapSampleFunc_t(
    const ProcessLightmapSampleData_t &data, const Vector &vPos,
    const Vector &vNormal, const Vector &vTangentS, const Vector &vTangentT,
    int t, int s, int tmax, int smax);
...
static void ProcessLightmapSample(const ProcessLightmapSampleData_t &data,
                                  const Vector &vPos, const Vector &vNormal,
  ...
  if (distSqr < data.m_LightDistSqr) {
    ...
    int index = t * smax + s;
    VectorMA(blocklights[0][index].AsVector3D(), scale, data.m_Intensity,
             blocklights[0][index].AsVector3D());
  }
#endif
}

static void ProcessLightmapSampleBumped(const ProcessLightmapSampleData_t &data,
                                        const Vector &vPos,
...
//-----------------------------------------------------------------------------
static void ProcessLightmapSampleAlpha(const ProcessLightmapSampleData_t &data,
                                       const Vector &vPos,
                                       const Vector &vNormal,
                                       const Vector &vTangentS,
                                       const Vector &vTangentT, int t, int s,
...
// T::ProcessLightmapSample( Vector const &vPos, int t, int s, int tmax, int
// smax );
void IterateLightmapSamples(CDispInfo *pDisp,
  ...
  unsigned char *pCurSample =
      &g_DispLightmapSamplePositions[pDisp->m_iLightmapSamplePositionStart];
  ...
  for (int t = 0; t < tmax; t++) {
    for (int s = 0; s < smax; s++) {
      ...
      Vector vPos = pDisp->m_MeshReader.Position(pTri->m_Indices[0]) * a +
...
bool CDispInfo::SetupDecalNodeIntersect_R(CVertIndex const &nodeIndex,
                                          ...
                                          ShadowInfo_t const *pInfo, int iLevel,
  ...
  if (iLevel + 1 < m_Power) {
    ...
    for (int iChild = 0; iChild < 4; iChild++) {
      ...
      if (SetupDecalNodeIntersect_R(childNode, iChildNodeBit, pDispDecal, pInfo,
                                    iLevel + 1, pCache))
  ...
  if (!pCache->IsCached(iNodeIndex)) {
    DecalProjectVert(m_MeshReader.Position(iNodeIndex), pDispDecal, pInfo,
                     pCache->m_ProjectedVert[iNodeIndex]);
  ...
  for (int i = 0; i < 4; i++) {
    CVertIndex const &sideVert =
        m_pPowerInfo->m_pSideVerts[iNodeIndex].m_Verts[i];
    CVertIndex const &cornerVert =
        m_pPowerInfo->m_pSideVertCorners[iNodeIndex].m_Verts[i];

    int iSideIndex = VertIndex(sideVert);
    if (!pCache->IsCached(iSideIndex)) {
      DecalProjectVert(m_MeshReader.Position(iSideIndex), pDispDecal, pInfo,
                       pCache->m_ProjectedVert[iSideIndex]);
    ...
    if (!pCache->IsCached(iCornerIndex)) {
      DecalProjectVert(m_MeshReader.Position(iCornerIndex), pDispDecal, pInfo,
                       pCache->m_ProjectedVert[iCornerIndex]);
  ...
  // Now just see if our bbox intersects the [0,0] - [1,1] bbox, which is where
  // this decal sits.
  if (vMin.x <= 1 && vMax.x >= 0 && vMin.y <= 1 && vMax.y >= 0) {
    // Z cull for shadows...
    if (pInfo) {
      if ((vMax.z < 0) || (vMin.z > pInfo->m_MaxDist)) return false;
    }
...
void CDispInfo::SetupDecalNodeIntersect(CVertIndex const &nodeIndex,
                                        int iNodeBitIndex,
                                        CDispDecalBase *pDispDecal,
                                        ShadowInfo_t const *pInfo) {
...
Vector CDispInfo::GetFlatVert(int iVertex) {
  ...
  Vector endPts[2];
  VectorLerp(m_BaseSurfacePositions[0], m_BaseSurfacePositions[1], y * ooInt,
             endPts[0]);
  VectorLerp(m_BaseSurfacePositions[3], m_BaseSurfacePositions[2], y * ooInt,
             endPts[1]);
...
void CDispInfo::ComputeLightmapAndTextureCoordinate(
    RayDispOutput_t const &output, Vector2D *luv, Vector2D *tuv) {
#ifndef SWDS
  // lightmap coordinate
  if (luv) {
  ...
  if (tuv) {
    ...
    Vector2D basefaceUV;
    ComputePointFromBarycentric(baseUV[0], baseUV[1], baseUV[2], output.u,
                                output.v, basefaceUV);

    // Convert the base face uv to a texture uv based on the base face texture
    // coords
    TexCoordInQuadFromBarycentric(
        m_BaseSurfaceTexCoords[0], m_BaseSurfaceTexCoords[3],
        m_BaseSurfaceTexCoords[2], m_BaseSurfaceTexCoords[1], basefaceUV, *tuv);
  }
#endif
}
