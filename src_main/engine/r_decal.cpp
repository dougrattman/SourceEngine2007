...
ConVar r_decal_cullsize("r_decal_cullsize", "5", 0,
                        "Decals under this size in pixels are culled");
ConVar r_spray_lifetime("r_spray_lifetime", "2", 0,
                        "Number of rounds player sprays are visible");
ConVar r_queued_decals("r_queued_decals", "0", 0,
                       "Offloads a bit of decal rendering setup work to the "
                       "material system queue when enabled.");

// This makes sure all the decals got freed before the engine is shutdown.
class CDecalChecker {
 public:
  ~CDecalChecker() { Assert(g_nDynamicDecals == 0); }
} g_DecalChecker;
...
static ConVar r_drawbatchdecals("r_drawbatchdecals", "1", 0,
                                "Render decals batched.");

static void R_DecalCreate(decalinfo_t *pDecalInfo, SurfaceHandle_t surfID,
                          float x, float y, bool bForceForDisplacement);
...
float ComputeDecalLightmapOffset(SurfaceHandle_t surfID) {
  ...
  if (MSurf_Flags(surfID) & SURFDRAW_BUMPLIGHT) {
    ...
    materials->GetLightmapPageSize(
        SortInfoToLightmapPage(MSurf_MaterialSortID(surfID)), &nWidth,
        &nHeight);
...
//-----------------------------------------------------------------------------
void Shader_DecalDrawPoly(CDecalVert *v, IMaterial *pMaterial,
                          SurfaceHandle_t surfID, int vertCount,
                          decal_t *pdecal) {
#ifndef SWDS
  int vertexFormat = 0;
  ...
  uint8_t color[4] = {pdecal->color.r, pdecal->color.g, pdecal->color.b,
                      pdecal->color.a};
  ...
  for (int i = 0; i < vertCount; i++, v++) {
    ...
    if (pMaterial->InMaterialPage()) {
      ...
      meshBuilder.TexCoordSubRect2f(0, v->m_ctCoords.x, v->m_ctCoords.y,
...
void R_DecalGetMaterialAndSize(int decalIndex, IMaterial *&pDecalMaterial,
                               float &w, float &h) {
  pDecalMaterial = Draw_DecalMaterial(decalIndex);
  if (!pDecalMaterial) return;
...
static inline decal_t *MSurf_DecalPointer(SurfaceHandle_t surfID) {
  ...
  if (handle == WORLD_DECAL_HANDLE_INVALID) return NULL;

  return s_aDecalPool[handle];
}

static WorldDecalHandle_t DecalToHandle(decal_t *pDecal) {
  if (!pDecal) return WORLD_DECAL_HANDLE_INVALID;
...
void R_DecalInit(void) {
  ...
  for (i = 0; i < DECAL_CACHEENTRY; i++) gDecalCache[i].decalIndex = -1;

  R_DecalSortInit();
}

void R_DecalTerm(worldbrushdata_t *pBrushData, bool term_permanent_decals) {
  if (!pBrushData) return;
...
static void R_DecalCacheClear(decal_t *pdecal) {
  ...
  if (pCache->decalIndex ==
      index)  // If this is the decal that's cached here, clear it.
    pCache->decalIndex = -1;
}
...
void R_DecalUnlink(decal_t *pdecal, worldbrushdata_t *pData) {
  ...
  if (IS_SURF_VALID(pdecal->surfID)) {
    if (MSurf_DecalPointer(pdecal->surfID) == pdecal) {
      ...
      if (!tmp) Sys_Error("Bad decal list");
      while (tmp->pnext) {
        if (tmp->pnext == pdecal) {
    ...
    if (SurfaceHasDispInfo(pdecal->surfID)) {
      ...
      if (pDispInfo) pDispInfo->NotifyRemoveDecal(pdecal->m_DispDecal);
    }
  }
  ...
  if (spewdecals) {
    ...
    for (i = 0; i < g_nMaxDecals; ++i) {
      ...
      if (decal) {
        ...
        Msg("%i == %s on %i perm %i at %.2f %.2f %.2f on surf %i (%.2f %.2f "
            "%2.f)\n",
            i, decal->material->GetName(), (int)decal->entityIndex,
            permanent ? 1 : 0, decal->position.x, decal->position.y,
            decal->position.z, (int)decal->surfID, decal->dx, decal->dy,
            decal->scale);
      }
    }
  }
...
void R_DecalSurface(SurfaceHandle_t surfID, decalinfo_t *decalinfo,
  ...
  // Compute an effective width and height (axis aligned)	in the parent
  // texture space How does this work? decalBasis[0] represents the u-direction
  // (width) of the decal measured in world space, decalBasis[1] represents the
  ...
  // represents the texture space v direction. We want to find the dimensions
  // (w,h) of a square measured in texture space, axis aligned to that
  // coordinate system. All we need to do is to find the components of the decal
  // edge vectors (decalWidth * decalBasis[0], decalHeight * decalBasis[1]) in
  // texture coordinates:

  float w = fabs(decalinfo->m_decalWidth *
                 DotProduct(textureU.AsVector3D(), decalinfo->m_Basis[0])) +
            fabs(decalinfo->m_decalHeight *
                 DotProduct(textureU.AsVector3D(), decalinfo->m_Basis[1]));

  float h = fabs(decalinfo->m_decalWidth *
                 DotProduct(textureV.AsVector3D(), decalinfo->m_Basis[0])) +
            fabs(decalinfo->m_decalHeight *
                 DotProduct(textureV.AsVector3D(), decalinfo->m_Basis[1]));
  ...
  if (!bForceForDisplacement) {
    if (s <= -w || t <= -h || s > (MSurf_TextureExtents(surfID)[0] + w) ||
        t > (MSurf_TextureExtents(surfID)[1] + h)) {
      return;  // nope
    }
  }
...
void R_DecalLeaf(mleaf_t *pLeaf, decalinfo_t *decalinfo) {
  ...
  for (int i = 0; i < pLeaf->nummarksurfaces; i++) {
    ...
    float dist =
        fabs(DotProduct(decalinfo->m_Position, MSurf_Plane(surfID).normal) -
...
static void R_DecalNode(mnode_t *node, decalinfo_t *decalinfo) {
  ...
  dist =
      DotProduct(decalinfo->m_Position, splitplane->normal) - splitplane->dist;
...
static int __cdecl DecalDepthCompare(const decallist_t *elem1,
                                     const decallist_t *elem2) {
  if (elem1->depth > elem2->depth) return -1;
  if (elem1->depth < elem2->depth) return 1;
...
int DecalListCreate(decallist_t *pList) {
  ...
  if (host_state.worldmodel) {
    for (i = 0; i < g_nMaxDecals; i++) {
      ...
      if (!decal || !IS_SURF_VALID(decal->surfID) ||
      ...
      Q_strncpy(pList[total].name, pMaterial->GetName(),
...
static bool R_DecalUnProject(decal_t *pdecal, decallist_t *entry) {
  if (!pdecal || !IS_SURF_VALID(pdecal->surfID)) return false;
...
// Shoots a decal onto the surface of the BSP.  position is the center of the
// decal in world coords
static void R_DecalShoot_(IMaterial *pMaterial, int entity,
                          const model_t *model, const Vector &position,
                          const Vector *saxis, int flags,
                          const color32 &rgbaColor, void *userdata = 0) {
  ...
  if (!(flags & FDECAL_CUSTOM)) flags |= FDECAL_CLIPTEST;
  ...
  mnode_t *pnodes =
      decalInfo.m_pBrush->nodes + decalInfo.m_pModel->brush.firstnode;
  R_DecalNode(pnodes, &decalInfo);
}

// Shoots a decal onto the surface of the BSP.  position is the center of the
// decal in world coords This is called from cl_parse.cpp, cl_tent.cpp
void R_DecalShoot(int textureIndex, int entity, const model_t *model,
...
void R_PlayerDecalShoot(IMaterial *material, void *userdata, int entity,
                        const model_t *model, const Vector &position,
                        const Vector *saxis, int flags,
                        const color32 &rgbaColor) {
  ...
  for (i = 0; i < s_aDecalPool.Count(); i++) {
    ...
    if (decal && decal->flags & FDECAL_PLAYERSPRAY &&
  ...
  R_DecalShoot_(material, entity, model, position, saxis, flags, rgbaColor,
                userdata);
}

// Generate lighting coordinates at each vertex for decal vertices v[] on
// surface psurf
static void R_DecalVertsLight(CDecalVert *v, SurfaceHandle_t surfID,
                              int vertCount) {
  int j;
  float s, t;
  ...
  for (j = 0; j < vertCount; j++, v++) {
    s = DotProduct(v->m_vPos, MSurf_TexInfo(surfID)
                                  ->lightmapVecsLuxelsPerWorldUnits[0]
                                  .AsVector3D()) +
    ...
    t = DotProduct(v->m_vPos, MSurf_TexInfo(surfID)
                                  ->lightmapVecsLuxelsPerWorldUnits[1]
                                  .AsVector3D()) +
...
static CDecalVert *R_DecalVertsNoclip(decal_t *pdecal, SurfaceHandle_t surfID,
                                      IMaterial *pMaterial) {
  decalcache_t *pCache;
  int decalIndex;
  int outCount;
...
// decal basis is constant per plane, perhaps we should store it (unscaled) in
// the shared plane struct BRJ: Note, decal basis is not constant when decals
// need to specify an s direction but that certainly isn't the majority case
static decal_t *R_DecalIntersect(decalinfo_t *decalinfo, SurfaceHandle_t surfID,
                                 int *pcount) {
  decal_t *plast = NULL;
  ...
  *pcount = 0;
  ...
  int mapSize[2] = {pMaterial->GetMappingWidth(),
                    pMaterial->GetMappingHeight()};
  Vector decalExtents[2];
  decalExtents[0] =
      decalinfo->m_Basis[0] * (mapSize[0] / decalinfo->m_scale) * 0.5f;
  decalExtents[1] =
      decalinfo->m_Basis[1] * (mapSize[1] / decalinfo->m_scale) * 0.5f;
  ...
  while (pDecal) {
    ...
    if (!(pDecal->flags & FDECAL_PERMANENT) &&
      ...
      R_SetupDecalTextureSpaceBasis(pDecal, MSurf_Plane(surfID).normal,
                                    pMaterial, testBasis, testWorldScale);

      // Here, we project the min and max extents of the decal that got passed
      // in into this decal's (pDecal's) [0,0,1,1] clip space, just like we
      // would if we were clipping a triangle into pDecal's clip space.
      Vector2D vDecalMin(
          DotProduct(decalinfo->m_Position - decalExtents[0], testBasis[0]) -
              pDecal->dx + 0.5f,
          DotProduct(decalinfo->m_Position - decalExtents[1], testBasis[1]) -
              pDecal->dy + 0.5f);
      ...
      if (vUnionMin.x < 1 && vUnionMin.y < 1 && vUnionMax.x > 0 &&
        ...
        float flArea =
            (vUnionMax.x - vUnionMin.x) * (vUnionMax.y - vUnionMin.y);
...
// If the surface is a displacement, let the displacement precalculate data for
// the decal.
static void R_AddDecalToSurface(decal_t *pdecal, SurfaceHandle_t surfID,
                                decalinfo_t *decalinfo) {
  pdecal->pnext = NULL;
  ...
  if (SurfaceHasDispInfo(surfID)) {
    pdecal->m_DispDecal =
        MSurf_DispInfo(surfID)->NotifyAddDecal(pdecal, decalinfo->m_Size);
  }
...
static void R_DecalMaterialSort(decal_t *pDecal, SurfaceHandle_t surfID) {
  ...
  for (; iFormat < nFormatCount; ++iFormat) {
    if (g_aDecalFormats[iFormat].m_VertexFormat == vertexFormat) break;
  }
  ...
  if (iSort == -1) {
    int iBucket = g_aDecalSortTrees[iSortTree]
                      .m_aDecalSortBuckets[0][iTreeType]
                      .AddToTail();
    g_aDispDecalSortTrees[iSortTree]
        .m_aDecalSortBuckets[0][iTreeType]
        .AddToTail();

    g_aDecalSortTrees[iSortTree]
        .m_aDecalSortBuckets[0][iTreeType]
        .Element(iBucket)
        .m_nCheckCount = -1;
    g_aDispDecalSortTrees[iSortTree]
    ...
    for (int iGroup = 1; iGroup < (MAX_MAT_SORT_GROUPS + 1); ++iGroup) {
      g_aDecalSortTrees[iSortTree]
          .m_aDecalSortBuckets[iGroup][iTreeType]
          .AddToTail();
      g_aDispDecalSortTrees[iSortTree]
          .m_aDecalSortBuckets[iGroup][iTreeType]
          .AddToTail();
    ...
    pDecal->m_iSortTree = iSortTree;
    pDecal->m_iSortMaterial = g_aDecalSortTrees[iSortTree]
                                  .m_pTrees[iTreeType]
                                  ->Element(iSort)
                                  .m_iBucket;
  }
...
static void R_DecalCreate(decalinfo_t *decalinfo, SurfaceHandle_t surfID,
                          float x, float y, bool bForceForDisplacement) {
  decal_t *pdecal;
  int count, vertCount;
  ...
  IMaterialVar *decalVar =
      decalinfo->m_pMaterial->FindVar("$decalFadeDuration", &found, false);
  if (found) {
  ...
  decalVar =
      decalinfo->m_pMaterial->FindVar("$decalDynamicScale", &found, false);
  if (found) {
  ...
  if (found) pdecal->flags |= FDECAL_SECONDPASS;

  if (!bForceForDisplacement) {
    // Check to see if the decal actually intersects the surface
    // if not, then remove the decal
    R_DecalVertsClip(NULL, pdecal, surfID, decalinfo->m_pMaterial, &vertCount);
    if (!vertCount) {
      R_DecalUnlink(pdecal, host_state.worldbrush);
      return;
    }
...
CDecalVert *R_DecalSetupVerts(IMatRenderContext *pRenderContext,
                              decal_t *pDecal, SurfaceHandle_t surfID,
                              IMaterial *pMaterial, int &outCount,
  ...
  Vector worldSpacePosition =
      (pDecal->entityIndex != 0)
          ? g_BrushToWorldMatrix.VMul4x3(pDecal->position)
          : pDecal->position;
  float pixels = pRenderContext->ComputePixelWidthOfSphere(worldSpacePosition,
                                                           pDecal->m_Size);
  ...
  if (!(pDecal->flags & FDECAL_PLAYERSPRAY)) {
    if (pDecal->flags & FDECAL_DISTANCESCALE) {
      ...
      //
      if (fov != r_dscale_basefov.GetFloat() && fov > 0 &&
          r_dscale_basefov.GetFloat() > 0) {
        float fovScale = fov / r_dscale_basefov.GetFloat();
        nearScale *= fovScale;
        farScale *= fovScale;

        if (nearScale < 1.0f) nearScale = 1.0f;
        if (farScale < 1.0f) farScale = 1.0f;
      }
  ...
  CDecalVert *v;
  // Distance-scaled decals can't go through the noclip path or they can end up
  // scaled incorrectly
...
void DecalUpdateAndDrawSingle(IMatRenderContext *pRenderContext,
                              SurfaceHandle_t surfID, decal_t *pDecal,
                              const Vector &vModelOrg) {
  if (!pDecal->material) return;

  // Update dynamic decals
  bool retire = false;
  if (pDecal->flags & FDECAL_DYNAMIC) retire = DecalUpdate(pDecal);
  ...
  if (SurfaceHasDispInfo(surfID)) {
    ...
    int outCount;
    CDecalVert *v = R_DecalSetupVerts(pRenderContext, pDecal, surfID,
                                      pDecal->material, outCount, vModelOrg);
...
void DrawDecalsOnSingleSurface_NonQueued(IMatRenderContext *pRenderContext,
...
void DrawDecalsOnSingleSurface_QueueHelper(SurfaceHandle_t surfID,
                                           const Vector &vModelOrg) {
  CMatRenderContextPtr pRenderContext(materials);
  DrawDecalsOnSingleSurface_NonQueued(pRenderContext, surfID, vModelOrg);
}

void DrawDecalsOnSingleSurface(IMatRenderContext *pRenderContext,
                               SurfaceHandle_t surfID) {
  ICallQueue *pCallQueue;
  if (r_queued_decals.GetBool() &&
      (pCallQueue = pRenderContext->GetCallQueue()) != NULL) {
    // queue available and desired
    pCallQueue->QueueCall(DrawDecalsOnSingleSurface_QueueHelper, surfID,
                          modelorg);
  } else {
    // non-queued mode
...
void R_DrawDecalsAllImmediate_GatherDecals(IMatRenderContext *pRenderContext,
                                           int iGroup, int iTreeType,
  ...
  for (int iSortTree = 0; iSortTree < nSortTreeCount; ++iSortTree) {
    int nBucketCount = g_aDecalSortTrees[iSortTree]
                           .m_aDecalSortBuckets[iGroup][iTreeType]
                           .Count();
    for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
      if (g_aDecalSortTrees[iSortTree]
              .m_aDecalSortBuckets[iGroup][iTreeType]
              .Element(iBucket)
              .m_nCheckCount != nCheckCount)
        continue;

      int iHead = g_aDecalSortTrees[iSortTree]
                      .m_aDecalSortBuckets[iGroup][iTreeType]
                      .Element(iBucket)
                      .m_iHead;
...
void R_DrawDecalsAllImmediate_Gathered(IMatRenderContext *pRenderContext,
                                       decal_t **ppDecals, int iDecalCount,
                                       const Vector &vModelOrg) {
  for (int i = 0; i != iDecalCount; ++i) {
    ...
    int nCount;
    CDecalVert *pVerts =
        R_DecalSetupVerts(pRenderContext, pDecal, pDecal->surfID,
                          pDecal->material, nCount, vModelOrg);
    ...
    if (ShouldDrawInWireFrameMode()) {
      ...
      pRenderContext->BindLightmapPage(
          materialSortInfoArray[MSurf_MaterialSortID(pDecal->surfID)]
              .lightmapPageID);
    ...
    if (pDecal->flags & FDECAL_DYNAMIC) {
      ...
      if (pDecal->fadeDuration < 0) {
        flFadeValue =
            -(cl.GetTime() - pDecal->fadeStartTime) / pDecal->fadeDuration;
      } else {
        flFadeValue =
            1.0 - (cl.GetTime() - pDecal->fadeStartTime) / pDecal->fadeDuration;
      }
    ...
    Vector vecNormal(0.0f, 0.0f, 1.0f), vecTangentS(1.0f, 0.0f, 0.0f),
        vecTangentT(0.0f, 1.0f, 0.0f);
    if (vertexFormat & (VERTEX_NORMAL | VERTEX_TANGENT_SPACE)) {
      vecNormal = MSurf_Plane(pDecal->surfID).normal;
      if (vertexFormat & VERTEX_TANGENT_SPACE) {
        Vector tVect;
        bool bNegate = TangentSpaceSurfaceSetup(pDecal->surfID, tVect);
        TangentSpaceComputeBasis(vecTangentS, vecTangentT, vecNormal, tVect,
                                 bNegate);
      }
    }
    ...
    for (int iVert = 0; iVert < nCount; ++iVert, ++pVerts) {
      ...
      if (pDecal->material->InMaterialPage()) {
        ...
        meshBuilder.TexCoordSubRect2f(0, pVerts->m_ctCoords.x,
                                      pVerts->m_ctCoords.y, offset[0],
                                      offset[1], scale[0], scale[1]);
      } else {
...
//
//-----------------------------------------------------------------------------
void R_DrawDecalsAllImmediate(IMatRenderContext *pRenderContext, int iGroup,
                              int iTreeType, const Vector &vModelOrg,
                              int nCheckCount) {
  int nSortTreeCount = g_aDecalSortTrees.Count();
  for (int iSortTree = 0; iSortTree < nSortTreeCount; ++iSortTree) {
    int nBucketCount = g_aDecalSortTrees[iSortTree]
    ...
    for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
      ...
      while (iElement != g_aDecalSortPool.InvalidIndex()) {
        ...
        pMesh = pRenderContext->GetDynamicMesh();
        CMeshBuilder meshBuilder;
        meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, nCount,
                          ((nCount - 2) * 3));
        ...
        if (pDecal->flags & FDECAL_DYNAMIC) {
          ...
          if (pDecal->fadeDuration < 0) {
            ...
            flFadeValue = 1.0 - (cl.GetTime() - pDecal->fadeStartTime) /
                                    pDecal->fadeDuration;
          }
        ...
        for (int iVert = 0; iVert < nCount; ++iVert, ++pVerts) {
          ...
          if (pDecal->material->InMaterialPage()) {
            ...
            meshBuilder.TexCoord2f(0, pVerts->m_ctCoords.x,
                                   pVerts->m_ctCoords.y);
          }
          meshBuilder.TexCoord2f(1, pVerts->m_cLMCoords.x,
                                 pVerts->m_cLMCoords.y);
          meshBuilder.TexCoord1f(2, flOffset);
          if (vertexFormat & VERTEX_TANGENT_SPACE) {
            meshBuilder.TangentS3fv(vecTangentS.Base());
            meshBuilder.TangentT3fv(vecTangentT.Base());
...
//
//-----------------------------------------------------------------------------
void R_DrawDecalsAll_GatherDecals(IMatRenderContext *pRenderContext, int iGroup,
                                  int iTreeType,
                                  CUtlVector<decal_t *> &DrawDecals) {
  int nCheckCount = g_nDecalSortCheckCount;
  if (iGroup == MAX_MAT_SORT_GROUPS) {
  ...
  for (int iSortTree = 0; iSortTree < nSortTreeCount; ++iSortTree) {
    ...
    for (int iBucket = 0; iBucket < nBucketCount; ++iBucket) {
      DecalMaterialBucket_t &bucket =
          sortTree.m_aDecalSortBuckets[iGroup][iTreeType].Element(iBucket);
      if (bucket.m_nCheckCount != nCheckCount) continue;

      int iHead = bucket.m_iHead;
      if (!g_aDecalSortPool.IsValidIndex(iHead)) continue;

      decal_t *pDecalHead = g_aDecalSortPool.Element(iHead);
      Assert(pDecalHead->material);
      if (!pDecalHead->material) continue;

      // Vertex format.
      VertexFormat_t vertexFormat = GetUncompressedFormat(pDecalHead->material);
      if (vertexFormat == 0) continue;
...
void R_DrawDecalsAll_Gathered(IMatRenderContext *pRenderContext,
                              decal_t **ppDecals, int iDecalCount,
                              const Vector &vModelOrg) {
  ...
  for (int i = 0; i != iDecalCount; ++i) {
    ...
    if ((pDecal == DECALMARKERS_SWITCHSORTTREE) ||
        (pDecal == DECALMARKERS_SWITCHBUCKET)) {
      if (pBatch) {
      ...
      if (pDecal == DECALMARKERS_SWITCHBUCKET) {
        ...
        for (int j = i + 1; j != iDecalCount; ++j) {
          ...
          if ((pDecalHead != DECALMARKERS_SWITCHSORTTREE) &&
              (pDecalHead != DECALMARKERS_SWITCHBUCKET))
            break;
        }
    ...
    if (((nVertCount + nCount) > nDecalSortMaxVerts) ||
        (nIndexCount + (nCount - 2) > nDecalSortMaxIndices)) {
      // Finish this batch.
      if (pBatch) {
        pBatch->m_nIndexCount = (nIndexCount - pBatch->m_iStartIndex);
      }
    ...
    if (bMeshInit) {
      ...
      if (ShouldDrawInWireFrameMode()) {
        meshList.m_pMesh = pRenderContext->GetDynamicMesh(
            false, NULL, NULL, g_materialDecalWireframe);
      } else {
        meshList.m_pMesh = pRenderContext->GetDynamicMesh(false, NULL, NULL,
                                                          pDecalHead->material);
      }
      meshBuilder.Begin(meshList.m_pMesh, MATERIAL_TRIANGLES,
                        nDecalSortMaxVerts, nDecalSortMaxIndices);
    ...
    if (bBatchInit) {
      ...
      if (meshList.m_aBatches.Size() + 1 > meshList.m_aBatches.NumAllocated()) {
        Warning(
            "R_DrawDecalsAll: overflowing m_aBatches. Reduce # of decals in "
            "the scene.\n",
      ...
      if (ShouldDrawInWireFrameMode()) {
        ...
        pBatch->m_iLightmapPage =
            materialSortInfoArray[MSurf_MaterialSortID(pDecalHead->surfID)]
                .lightmapPageID;
      }
...
void R_DrawDecalsAll(IMatRenderContext *pRenderContext, int iGroup,
                     int iTreeType, const Vector &vModelOrg, int nCheckCount) {
  DecalMeshList_t meshList;
  CMeshBuilder meshBuilder;
      ...
      while (iElement != g_aDecalSortPool.InvalidIndex()) {
        ...
        if (bMeshInit) {
          ...
          if (ShouldDrawInWireFrameMode()) {
            ...
            meshList.m_pMesh = pRenderContext->GetDynamicMesh(
                false, NULL, NULL, pDecalHead->material);
          }
          meshBuilder.Begin(meshList.m_pMesh, MATERIAL_TRIANGLES,
                            nDecalSortMaxVerts, nDecalSortMaxIndices);
        ...
        if (bBatchInit) {
          // Create a batch for this bucket = material/lightmap pair.
          // Todo: we also could flush it right here and continue.
          if (meshList.m_aBatches.Size() + 1 >
              meshList.m_aBatches.NumAllocated()) {
            Warning(
                "R_DrawDecalsAll: overflowing m_aBatches. Reduce # of decals "
                "in the scene.\n",
                nDecalSortMaxVerts * meshList.m_aBatches.NumAllocated());
            meshBuilder.End();
            R_DrawDecalMeshList(meshList);
...
// Purpose:
//-----------------------------------------------------------------------------
void DecalSurfaceDraw_NonQueued(IMatRenderContext *pRenderContext,
                                int renderGroup, const Vector &vModelOrg,
                                int nCheckCount) {
  if (r_drawbatchdecals.GetBool()) {
    // Draw world decals.
    R_DrawDecalsAll(pRenderContext, renderGroup, PERMANENT_LIGHTMAP, vModelOrg,
                    nCheckCount);

    // Draw lightmapped non-world decals.
    R_DrawDecalsAll(pRenderContext, renderGroup, LIGHTMAP, vModelOrg,
                    nCheckCount);

    // Draw non-lit(mod2x) decals.
    R_DrawDecalsAll(pRenderContext, renderGroup, NONLIGHTMAP, vModelOrg,
                    nCheckCount);
  } else {
    // Draw world decals.
    R_DrawDecalsAllImmediate(pRenderContext, renderGroup, PERMANENT_LIGHTMAP,
                             vModelOrg, nCheckCount);

    // Draw lightmapped non-world decals.
    R_DrawDecalsAllImmediate(pRenderContext, renderGroup, LIGHTMAP, vModelOrg,
                             nCheckCount);

    // Draw non-lit(mod2x) decals.
    R_DrawDecalsAllImmediate(pRenderContext, renderGroup, NONLIGHTMAP,
                             vModelOrg, nCheckCount);
  }
}

void DecalSurfaceDraw_QueueHelper(bool bBatched, int renderGroup,
                                  const Vector &vModelOrg, int nCheckCount,
                                  decal_t **ppDecals, int iPermanentLightmap,
                                  int iLightmap, int iNonLightmap) {
  CMatRenderContextPtr pRenderContext(materials);

  if (bBatched) {
    R_DrawDecalsAll_Gathered(pRenderContext, ppDecals, iPermanentLightmap,
                             vModelOrg);
    ...
    R_DrawDecalsAllImmediate_Gathered(pRenderContext, ppDecals,
                                      iPermanentLightmap, vModelOrg);
    ppDecals += iPermanentLightmap;
    R_DrawDecalsAllImmediate_Gathered(pRenderContext, ppDecals, iLightmap,
                                      vModelOrg);
    ppDecals += iLightmap;
    R_DrawDecalsAllImmediate_Gathered(pRenderContext, ppDecals, iNonLightmap,
                                      vModelOrg);
  }
}
 ...
 public:
  ...
  ~CQueuedDecalMemoryManager(void) {
    ...
    for (int i = 0; i != 2; ++i) {
      for (int j = m_DeleteOnSwitch[i].Count(); --j >= 0;) {
        uint8_t *items = static_cast<uint8_t *>(m_DeleteOnSwitch[i].Element(j));
        delete[] items;
      }
  ...
  void SwitchStack(void) {
    ...
    for (int i = m_DeleteOnSwitch[m_nCurrentStack].Count(); --i >= 0;) {
      uint8_t *items =
          static_cast<uint8_t *>(m_DeleteOnSwitch[m_nCurrentStack].Element(i));
      delete[] items;
    }
  ...
  inline void *Alloc(size_t bytes) {
    ...
    if (pReturn == NULL) {
      ...
      Warning(
          "Overflowed decal queued rendering memory stack. Needed %d, have "
          "%d/%d\n",
          bytes, iMaxSize - m_QueuedDecalMemory[m_nCurrentStack].GetUsed(),
          iMaxSize);
...
void DecalBeginFrame(void) { s_QDMM.SwitchStack(); }
...
void DecalSurfaceDraw(IMatRenderContext *pRenderContext, int renderGroup) {
  ...
  if (r_queued_decals.GetBool() &&
    ...
    if (bBatched) {
      R_DrawDecalsAll_GatherDecals(pRenderContext, renderGroup,
                                   PERMANENT_LIGHTMAP, DrawDecals);
      iPermanentLightmap = DrawDecals.Count();
      R_DrawDecalsAll_GatherDecals(pRenderContext, renderGroup, LIGHTMAP,
                                   DrawDecals);
      iLightmap = DrawDecals.Count() - iPermanentLightmap;
      R_DrawDecalsAll_GatherDecals(pRenderContext, renderGroup, NONLIGHTMAP,
                                   DrawDecals);
      iNonLightmap = DrawDecals.Count() - (iPermanentLightmap + iLightmap);
    } else {
      R_DrawDecalsAllImmediate_GatherDecals(pRenderContext, renderGroup,
                                            PERMANENT_LIGHTMAP, DrawDecals);
      iPermanentLightmap = DrawDecals.Count();
      R_DrawDecalsAllImmediate_GatherDecals(pRenderContext, renderGroup,
                                            LIGHTMAP, DrawDecals);
      iLightmap = DrawDecals.Count() - iPermanentLightmap;
      R_DrawDecalsAllImmediate_GatherDecals(pRenderContext, renderGroup,
                                            NONLIGHTMAP, DrawDecals);
      iNonLightmap = DrawDecals.Count() - (iPermanentLightmap + iLightmap);
    }

    if (DrawDecals.Count()) {
      decal_t **ppDecals =
          (decal_t **)s_QDMM.Alloc(DrawDecals.Count() * sizeof(decal_t *));
      memcpy(ppDecals, DrawDecals.Base(),
             DrawDecals.Count() * sizeof(decal_t *));
      pCallQueue->QueueCall(DecalSurfaceDraw_QueueHelper, bBatched, renderGroup,
                            modelorg, nCheckCount, ppDecals, iPermanentLightmap,
                            iLightmap, iNonLightmap);

      DrawDecals.RemoveAll();
    }
  } else {
    // non-queued mode
    DecalSurfaceDraw_NonQueued(pRenderContext, renderGroup, modelorg,
                               nCheckCount);
  }
}
...
void DecalSurfaceAdd(SurfaceHandle_t surfID, int iGroup) {
  ...
  for (decal_t *pDecal = pDecalList; pDecal; pDecal = pNext) {
    ...
    if (iPool != g_aDecalSortPool.InvalidIndex()) {
      ...
      DecalMaterialBucket_t &bucket =
          sortTree.m_aDecalSortBuckets[iGroup][iTreeType].Element(
              pDecal->m_iSortMaterial);
