...
class DownloadCache {
 public:
  ...
  void PersistToDisk(
      const RequestContext *rc);  ///< Writes out a completed download to disk
  void PersistToCache(const RequestContext *rc);  ///< Writes out a partial
                                                  ///< download (lost
                                                  ///< connection, user abort,
                                                  ///< etc) to cache
...
//--------------------------------------------------------------------------------------------------------------
DownloadCache::DownloadCache() { m_cache = NULL; }
...
void DownloadCache::BuildKeyNames(const char *gamePath) {
  ...
  Q_snprintf(m_cachefileKey, sizeof(m_cachefileKey), "cachefile_%s",
             tmpGamePath);
  Q_snprintf(m_timestampKey, sizeof(m_timestampKey), "timestamp_%s",
             tmpGamePath);
...
void DownloadCache::GetCachedData(RequestContext *rc) {
  ...
  if (fp == FILESYSTEM_INVALID_HANDLE) return;
...
static bool DecompressBZipToDisk(const char *outFilename,
                                 const char *srcFilename, char *data,
                                 int bytesTotal) {
...
void DownloadCache::PersistToDisk(const RequestContext *rc) {
  ...
  if (rc && rc->data && rc->nBytesTotal) {
    ...
    if (!g_pFileSystem->FileExists(gamePath)) {
      ...
      bool success = false;
      if (rc->bIsBZ2) {
        success = DecompressBZipToDisk(gamePath, rc->gamePath,
                                       reinterpret_cast<char *>(rc->data),
                                       rc->nBytesTotal);
      } else {
...
void DownloadCache::GenerateCacheFilename(const RequestContext *rc,
  ...
  // ConDColorMsg( DownloadColor,"DownloadCache::GenerateCacheFilename() set %s
...
class DownloadManager {
 public:
  ...
  bool Update();  ///< Monitors download thread, starts new downloads, and
 ...
 private:
  ...
  void PruneCompletedRequests();  ///< Check download requests that have been
                                  ///< completed to see if their threads have
                                  ///< exited
  void
  CheckActiveDownload();  ///< Checks download status, and updates progress bar
  void
  StartNewDownload();  ///< Starts a new download if there are queued requests
  ...
  RequestContext *m_activeRequest;  ///< this is the active request being
                                    ///< downloaded in another thread
  RequestVector
      m_completedRequests;  ///< these are waiting for the thread to exit

  int m_lastPercent;  ///< last percent value the progress bar was updated with
                      ///< (to avoid spamming it)
  int m_totalRequests;  ///< Total number of requests (used to set the top
                        ///< progress bar)

  int m_RequestIDCounter;  ///< global increasing request ID counter

  typedef CUtlVector<char *> StrVector;
  StrVector m_downloadedMaps;  ///< List of maps for which we have already tried
                               ///< to download assets.
};
...
//--------------------------------------------------------------------------------------------------------------
bool DownloadManager::HasMapBeenDownloadedFromServer(
    const char *serverMapName) {
  if (!serverMapName) return false;
...
bool DownloadManager::FileDenied(const char *filename, unsigned int requestID) {
  if (!m_activeRequest) return false;

  if (m_activeRequest->nRequestID != requestID) return false;

  if (m_activeRequest->bAsHTTP) return false;
...
bool DownloadManager::FileReceived(const char *filename,
...
void DownloadManager::MarkMapAsDownloadedFromServer(const char *serverMapName) {
  if (!serverMapName) return;

  if (HasMapBeenDownloadedFromServer(serverMapName)) return;
...
void DownloadManager::Queue(const char *baseURL, const char *gamePath) {
...
#ifndef _DEBUG
  if (sv.IsActive()) {
    return;  // don't try to download things for the local server (in case a map
             // is missing sounds etc that aren't needed to play.
  }
#endif

  // only http downloads
  if (baseURL && (!Q_strnicmp(baseURL, "http://", 7) ||
                  !Q_strnicmp(baseURL, "https://", 8))) {
    bAsHTTP = true;
  }
  ...
  if (Q_strstr(gamePath, "lua/")) return;  // don't download into lua/ folder
  ...
  // Disallow .. in paths, but allow multiple periods otherwise.  This way we
  // can download blah.dx80.vtx etc.
  const char *backup = strstr(gamePath, "..");
  const char *extension = strrchr(gamePath, '.');

  if (backup || !extension) return;

  int baseLen = strlen(extension);
  if (baseLen > 4 || baseLen < 3) return;

  if (!Q_strcasecmp(extension, ".cfg")) return;

  if (!Q_strcasecmp(extension, ".lst")) return;

  if (!Q_strcasecmp(extension, ".exe")) return;

  if (!Q_strcasecmp(extension, ".vbs")) return;

  if (!Q_strcasecmp(extension, ".com")) return;

  if (!Q_strcasecmp(extension, ".bat")) return;

  if (!Q_strcasecmp(extension, ".dll")) return;

  if (!Q_strcasecmp(extension, ".ini")) return;

  if (!Q_strcasecmp(extension, ".log")) return;

  if (!Q_strcasecmp(extension, ".lua")) return;
  ...
  if (bAsHTTP && !g_pFileSystem->FileExists(va("%s.bz2", gamePath))) {
    ...
    Q_FixSlashes(rc->gamePath, '/');  // only matters for debug prints, which
                                      // are full URLS, so we want forward
                                      // slashes
    Q_strncpy(rc->serverURL, cl.m_NetChannel->GetRemoteAddress().ToString(),
              BufferSize);
  ...
  Q_strncpy(rc->basePath, com_gamedir, BufferSize);
  Q_strncpy(rc->gamePath, gamePath, BufferSize);
  Q_FixSlashes(rc->gamePath, '/');  // only matters for debug prints, which are
                                    // full URLS, so we want forward slashes
  Q_strncpy(rc->serverURL, cl.m_NetChannel->GetRemoteAddress().ToString(),
            BufferSize);
...
//--------------------------------------------------------------------------------------------------------------
// Check download requests that have been completed to see if their threads have
// exited
void DownloadManager::PruneCompletedRequests() {
  for (int i = m_completedRequests.Count() - 1; i >= 0; --i) {
    if (m_completedRequests[i]->threadDone ||
        !m_completedRequests[i]->bAsHTTP) {
...
void DownloadManager::CheckActiveDownload() {
  ...
  switch (m_activeRequest->status) {
    ...
    case HTTP_FETCH:
      ...
      // TODO: SetSecondaryProgressBarText( m_activeRequest->gamePath );
      if (m_activeRequest->nBytesTotal) {
        int percent = (m_activeRequest->nBytesCurrent * 100 /
                       m_activeRequest->nBytesTotal);
        if (percent != m_lastPercent) {
          /*
          ConDColorMsg( DownloadColor, "Downloading %s%s: %3.3d%% - %d of %d
          bytes\n", m_activeRequest->baseURL, m_activeRequest->gamePath,
                  percent, m_activeRequest->nBytesCurrent,
          m_activeRequest->nBytesTotal );
                  */
...
void DownloadManager::StartNewDownload() {
  if (m_activeRequest || !m_queuedRequests.Count()) return;
  ...
  if (m_activeRequest->bAsHTTP) {
    ...
    // TODO: ContinueLoadingProgressBar( "Http", m_totalRequests -
    // m_queuedRequests.Count(), 0.0f );
    ...
    ConDColorMsg(DownloadColor, "Downloading %s%s.\n", m_activeRequest->baseURL,
                 m_activeRequest->gamePath);
    m_lastPercent = 0;

    // Start the thread
    DWORD threadID;
    VCRHook_CreateThread(NULL, 0, DownloadThread, m_activeRequest, 0,
                         &threadID);
  } else {
    UpdateProgressBar();
    ...
    m_activeRequest->nRequestID =
        cl.m_NetChannel->RequestFile(m_activeRequest->gamePath);
  }
}
...
void DownloadManager::UpdateProgressBar() {
  ...
  if (m_activeRequest->bAsHTTP) {
    int overallPercent = (m_totalRequests - m_queuedRequests.Count() - 1) *
                         100 / m_totalRequests;
    int filePercent = 0;
    if (m_activeRequest->nBytesTotal > 0) {
      filePercent =
          (m_activeRequest->nBytesCurrent * 100 / m_activeRequest->nBytesTotal);
    }
