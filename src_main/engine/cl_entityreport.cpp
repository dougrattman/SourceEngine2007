...
void CL_RecordEntityBits(int entnum, int bitcount) {
  ...
  // Update average
  slot->average =
      (BITCOUNT_AVERAGE)*slot->average + (1.f - BITCOUNT_AVERAGE) * bitcount;
...
//			b -
//-----------------------------------------------------------------------------
void CEntityReportPanel::ApplyEffect(ENTITYBITS *entry, int &r, int &g,
                                     int &b) {
...
void CEntityReportPanel::Paint() {
  ...
  for (int i = start; i <= lastused; i++) {
    ...
    if (pNet && ((pClientClass = pNet->GetClientClass())) != NULL) {
      ...
      DrawColoredText(m_hFont, left + col * colwidth, top + row * rowheight, r,
                      g, b, a, unicode);
      ...
      if (inpvs) {
        ...
        vgui::surface()->DrawFilledRect(rcleft, rctop + rowheight / 2,
                                        rcleft + wide * fracs[0], rcbottom - 1);

        // Draw average a vertical bar
        vgui::surface()->DrawSetColor(192, 192, 192, 255);
        vgui::surface()->DrawFilledRect(
            rcleft + wide * fracs[2], rctop + rowheight / 2,
            rcleft + wide * fracs[2] + 1, rcbottom - 1);

        // Draw peak as a vertical red tick
        vgui::surface()->DrawSetColor(192, 0, 0, 255);
        vgui::surface()->DrawFilledRect(rcleft + wide * fracs[1], rctop + 1,
                                        rcleft + wide * fracs[1] + 1,
                                        rctop + rowheight / 2);
      }
    }
    /*else
    {
            ...
            wchar_t unicode[ 256 ];
            g_pVGuiLocalize->ConvertANSIToUnicode( ( effectactive &&
    entry->deletedclientclass ) ? entry->deletedclientclass->m_pNetworkName :
    "unused", unicode, sizeof( unicode ) );

            DrawColoredText( m_hFont, left + col * colwidth, top + row *
    rowheight, r, g, b, a, L"(%i) %s", i, unicode );
    }*/
    ...
    if (top + row * rowheight > videomode->GetModeHeight() - rowheight) {
      ...
      if (left + (col + 1) * 200 > videomode->GetModeWidth()) return;
    }
  }
}
