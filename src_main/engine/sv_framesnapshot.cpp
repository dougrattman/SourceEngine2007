...
static ConVar sv_creationtickcheck(
    "sv_creationtickcheck", "1", FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
...
CFrameSnapshotManager::CFrameSnapshotManager(void)
    : m_PackedEntitiesPool(MAX_EDICTS / 16, CMemoryPool::GROW_SLOW) {
  static_assert(INVALID_PACKED_ENTITY_HANDLE == 0);
...
CFrameSnapshot *CFrameSnapshotManager::NextSnapshot(
  ...
  if (next == m_FrameSnapshots.InvalidIndex()) return NULL;

  // return next element in list
  return m_FrameSnapshots[next];
}

CFrameSnapshot *CFrameSnapshotManager::CreateEmptySnapshot(int tickcount,
                                                           int maxEntities) {
...
CFrameSnapshot *CFrameSnapshotManager::TakeTickSnapshot(int tickcount) {
  ...
  for (int i = 0; i < sv.num_edicts; i++) {
    ...
    if (i > 0 && i <= maxclients) {
      ...
      if (!sv.GetClient(i - 1)->IsActive()) continue;
    }
  ...
  if (hltv && hltv->IsActive()) {
    ...
    Q_memset(snap->m_pHLTVEntityData, 0,
             snap->m_nValidEntities * sizeof(CHLTVEntityData));
  }
...
void CFrameSnapshotManager::DeleteFrameSnapshot(CFrameSnapshot *pSnapshot) {
  ...
  for (int i = 0; i < pSnapshot->m_nNumEntities; ++i) {
    if (pSnapshot->m_pEntities[i].m_pPackedData !=
...
void CFrameSnapshotManager::AddExplicitDelete(int iSlot) {
  ...
  if (m_iExplicitDeleteSlots.Find(iSlot) ==
      m_iExplicitDeleteSlots.InvalidIndex()) {
    m_iExplicitDeleteSlots.AddToTail(iSlot);
  }
}

//-----------------------------------------------------------------------------
// Purpose: Returns true if the "basis" for encoding m_flAnimTime,
// m_flSimulationTime has changed
//  since the time this entity was packed to the time we're trying to re-use the
//  packing.
//-----------------------------------------------------------------------------
bool CFrameSnapshotManager::ShouldForceRepack(CFrameSnapshot *pSnapshot,
                                              int entity,
  ...
  if (sv_creationtickcheck.GetBool()) {
    ...
    if (pe && pe->ShouldCheckCreationTick()) {
      int nCurrentNetworkBase = g_ServerGlobalVariables.GetNetworkBase(
          pSnapshot->m_nTickCount, entity);
      int nPackedEntityNetworkBase = g_ServerGlobalVariables.GetNetworkBase(
          pe->GetSnapshotCreationTick(), entity);
...
bool CFrameSnapshotManager::UsePreviouslySentPacket(CFrameSnapshot *pSnapshot,
  ...
  if (handle != INVALID_PACKED_ENTITY_HANDLE) {
    ...
    if (m_pSerialNumber[entity] == entSerialNumber) {
      // Check if we need to re-pack entity due to encoding against
      // gpGlobals->tickcount
...
PackedEntity *CFrameSnapshotManager::GetPreviouslySentPacket(
    int iEntity, int iSerialNumber) {
...
CThreadFastMutex &CFrameSnapshotManager::GetMutex() { return m_WriteMutex; }
...
PackedEntity *CFrameSnapshotManager::CreatePackedEntity(
    CFrameSnapshot *pSnapshot, int entity) {
  m_WriteMutex.Lock();
...
PackedEntity *CFrameSnapshotManager::GetPackedEntity(CFrameSnapshot *pSnapshot,
                                                     int entity) {
  if (!pSnapshot) return NULL;

  Assert(entity < pSnapshot->m_nNumEntities);

  PackedEntityHandle_t index = pSnapshot->m_pEntities[entity].m_pPackedData;

  if (index == INVALID_PACKED_ENTITY_HANDLE) return NULL;
...
// ------------------------------------------------------------------------------------------------
// // purpose: lookup cache if we have an uncompressed version of this packed
// entity
// ------------------------------------------------------------------------------------------------
// //
UnpackedDataCache_t *CFrameSnapshotManager::GetCachedUncompressedEntity(
  ...
  pdcOldest->bits = -1;  // important, this is the signal for the caller to fill
                         // this structure
...
CFrameSnapshot::CFrameSnapshot() {
  ...
  Assert(g_nAllocatedSnapshots <
         80000);  // this probably would indicate a memory leak.
#endif
}
