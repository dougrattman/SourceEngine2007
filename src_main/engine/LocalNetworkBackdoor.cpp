...
void CLocalNetworkBackdoor::InitFastCopy() {
  if (!cl.m_NetChannel->IsLoopback()) return;

  const CStandardSendProxies *pSendProxies = NULL;

  // If the game server is greater than v4, then it is using the new proxy
  // format.
  if (g_bServerGameDLLGreaterThanV4)  // check server version
  {
    pSendProxies = serverGameDLL->GetStandardSendProxies();
  } else {
    // If the game server is older than v4, it is using the old proxy; we set
    // the new proxy members to the engine's copy.
    static CStandardSendProxies compatSendProxy =
        *serverGameDLL->GetStandardSendProxies();

    compatSendProxy.m_DataTableToDataTable =
        g_StandardSendProxies.m_DataTableToDataTable;
    compatSendProxy.m_SendLocalDataTable =
        g_StandardSendProxies.m_SendLocalDataTable;
    compatSendProxy.m_ppNonModifiedPointerProxies =
        g_StandardSendProxies.m_ppNonModifiedPointerProxies;

    pSendProxies = &compatSendProxy;
  }

  const CStandardRecvProxies *pRecvProxies =
      g_ClientDLL->GetStandardRecvProxies();
  ...
  for (int iClass = 0; iClass < cl.m_nServerClasses; iClass++) {
    ...
    if (!pClientClass) Error("InitFastCopy - missing client class %d", iClass);
    ...
    LocalTransfer_InitFastCopy(pServerClass->m_pTable, pSendProxies,
                               pClientClass->m_pRecvTable, pRecvProxies,
                               nSlowCopyProps, nFastCopyProps);
  }

  int percentFast =
      (nFastCopyProps * 100) / (nSlowCopyProps + nFastCopyProps + 1);
  if (percentFast <= 55) {
    // This may not be a real problem, but at the time this code was added, 67%
    // of the properties were able to be copied without proxies. If percentFast
    // goes to 0 or some really low number suddenly, then something probably got
    // screwed up.
    Assert(false);
...
void CLocalNetworkBackdoor::EndEntityStateUpdate() {
  ...
  for (i = 0; i < m_nEntsChanged; i++) {
    ...
    m_CachedEntState[iEdict].m_pNetworkable->PostDataUpdate(
...
void CLocalNetworkBackdoor::EntityDormant(int iEnt, int iSerialNum) {
  ...
  if (pNet) {
    Assert(pCached->m_iSerialNumber ==
           pNet->GetIClientUnknown()->GetRefEHandle().GetSerialNumber());
...
void CLocalNetworkBackdoor::AddToPendingDormantEntityList(
    unsigned short iEdict) {
...
void CLocalNetworkBackdoor::EntState(int iEnt, int iSerialNum, int iClass,
                                     const SendTable *pSendTable,
                                     const void *pSourceEnt, bool bChanged,
  ...
  if (!bShouldTransmit) {
    if (pNet) {
      ...
      if (pCached->m_iSerialNumber == iSerialNum) {
        ...
        pNet = NULL;
        pCached->m_pNetworkable = NULL;
        // Since we set this above, need to clear it now to avoid assertion in
        // EndEntityStateUpdate()
  ...
  if (pNet) {
    ...
    // Tracker 73192:  ywb 8/1/07:  We used to get an assertion that the
    // pCached->m_bDormant was not equal to pNet->IsDormant() in
    // ProcessDormantEntities. This appears to be the case if when we get here,
    // the entity is set for Transmit still, but is a dormant entity on the
    // server. Seems safe to go ahead an fill in the cache with the correct
    // data.  Probably was just an oversight.
  ...
  if (bChanged || bCreated || bExistedAndWasDormant) {
    ...
    LocalTransfer_TransferEntity(
        &sv.edicts[iEnt], pSendTable, pSourceEnt, pClientClass->m_pRecvTable,
