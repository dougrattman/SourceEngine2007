...
// file is always stored on disk, so use g_pFileSystem
// when accessing it.
...
ConVar save_history_count(
    "save_history_count", "1", 0,
...
ConVar save_screenshot("save_screenshot", "1", 0,
...
// HACK HACK:  Some hacking to keep the .sav file backward compatible on the
// client!!!
...
inline void GetServerSaveCommentEx(char *comment, int maxlength,
                                   float flMinutes, float flSeconds) {
...
struct GAME_HEADER {
  ...
  char mapName[32];
  char comment[80];
  int mapCount;  // the number of map state files in the save file.  This is
                 // usually number of maps * 3 (.hl1, .hl2, .hl3 files)
...
class CSaveRestore : public ISaveRestore {
 public:
  ...
  int SaveGameSlot(const char *pSaveName, const char *pSaveComment,
                   bool onlyThisLevel, bool bSetMostRecent,
                   const char *pszDestMap = NULL,
                   const char *pszLandmark = NULL);
  bool SaveGameState(bool bTransition, CSaveRestoreData ** = NULL,
                     bool bOpenContainer = true,
                     bool bIsAutosaveOrDangerous = false);
 ...
 private:
  ...
  int SaveReadHeader(FileHandle_t pFile, GAME_HEADER *pHeader,
                     int readGlobalState);
  CSaveRestoreData *LoadSaveData(const char *level);
  void ParseSaveTables(CSaveRestoreData *pSaveData, SAVE_HEADER *pHeader,
                       int updateGlobals);
  int FileSize(FileHandle_t pFile);

  bool CalcSaveGameName(const char *pName, char *output,
  ...
  struct RestoreLookupTable {
    RestoreLookupTable() : m_vecLandMarkOffset(0, 0, 0) {}
  ...
  void ReapplyDecal(bool adjacent, RestoreLookupTable *table,
                    decallist_t *entry);
...
// The proper way to extend the file format (add a new data chunk) is to add
// a field here, and use it to determine whether your new data chunk is in
// the file or not.  If the file was not saved with your new field, the
// chunk won't be there either. Structure members can be added/deleted
// without any problems, new structures must be reflected in an existing
// struct and not read unless actually in the file.  New structure members
// will be zeroed out when reading 'old' files.
...
BEGIN_SIMPLE_DATADESC(SAVE_HEADER)
  ...
  DEFINE_FIELD(skillLevel, FIELD_INTEGER),
      ...
      DEFINE_FIELD(mapVersion, FIELD_INTEGER), DEFINE_FIELD(time, FIELD_TIME),
      DEFINE_ARRAY(mapName, FIELD_CHARACTER, 32),
...
BEGIN_SIMPLE_DATADESC(SAVELIGHTSTYLE)
  DEFINE_FIELD(index, FIELD_INTEGER), DEFINE_ARRAY(style, FIELD_CHARACTER, 64),
END_DATADESC()
...
const char *CSaveRestore::FindRecentSave(char *pNameBuf, int nameBufLen) {
  ...
  if (!m_szMostRecentSaveLoadGame[0]) return NULL;
...
//			this is so the current level will just restart if the
// player dies
//-----------------------------------------------------------------------------
void CSaveRestore::ForgetRecentSave() { m_szMostRecentSaveLoadGame[0] = 0; }

//-----------------------------------------------------------------------------
// Purpose: Returns the save game directory for the current player profile
//-----------------------------------------------------------------------------
char *CSaveRestore::GetSaveDir() {
  static char save_directory[MAX_OSPATH] = "SAVE/";
...
void CSaveRestore::AgeSaveFile(const char *pName, const char *ext, int count) {
  ...
  if (count == 1) {
    // quick.sav. DON'T FixSlashes on this, it needs to be //MOD
    sprintf_s(oldName, "//%s/%s%s.%s", MOD_DIR, GetSaveDir(), pName, ext);
  } else {
    // quick04.sav, etc. DON'T FixSlashes on this, it needs to be //MOD
    sprintf_s(oldName, "//%s/%s%s%02d.%s", MOD_DIR, GetSaveDir(), pName,
              count - 1, ext);
  }

  // DON'T FixSlashes on this, it needs to be //MOD
  sprintf_s(newName, "//%s/%s%s%02d.%s", MOD_DIR, GetSaveDir(), pName, count,
            ext);
...
int CSaveRestore::IsValidSave() {
  if (cmd_source != src_command) return 0;
  ...
  if (sv.GetClientCount() > 0 && sv.GetClient(0)->IsActive()) {
    ...
    CGameClient *gcl = sv.Client(0);
    CPlayerState *pl = serverGameClients->GetPlayerState(gcl->edict);
    if (!pl) {
...
int CSaveRestore::SaveGameSlot(const char *pSaveName, const char *pSaveComment,
  ...
  if (save_disable.GetBool()) return 0;
  ...
  strcpy_s(m_szSaveGameName, name);
  ...
  if (onlyThisLevel) {
    sprintf_s(hlPath, "%s%s*.HL?", GetSaveDir(), sv.GetMapName());
  } else {
    sprintf_s(hlPath, "%s*.HL?", GetSaveDir());
  }
  ...
  if (bIsQuick || bIsAutosave || bIsAutosaveDangerous) {
    ...
    g_AsyncSaveCallQueue.QueueCall(this, &CSaveRestore::AgeSaveList,
                                   CUtlEnvelope<const char *>(pSaveName),
                                   save_history_count.GetInt());
  }

  S_ExtraUpdate();
  if (!SaveGameState((pszDestMap != NULL), NULL, false,
                     (bIsAutosave || bIsAutosaveDangerous))) {
    m_szSaveGameName[0] = 0;
    return 0;
  }
  S_ExtraUpdate();
  ...
  if (pszDestMap && pszLandmark && *pszDestMap && *pszLandmark) {
    ...
    Q_strncpy(gameHeader.originMapName, sv.GetMapName(),
              sizeof(gameHeader.originMapName));
  ...
  gameHeader.mapCount = 0;  // No longer used. The map packer will place the map
                            // count at the head of the compound files (toml
                            // 7/18/2007)
  serverGameDLL->SaveWriteFields(pSaveData, "GameHeader", &gameHeader, NULL,
                                 GAME_HEADER::m_DataMap.dataDesc,
                                 GAME_HEADER::m_DataMap.dataNumFields);
  ...
  for (i = 0; i < pSaveData->SizeSymbolTable(); i++) {
    const char *pszToken =
        (pSaveData->StringFromSymbol(i)) ? pSaveData->StringFromSymbol(i) : "";
  ...
  // Write the header -- THIS SHOULD NEVER CHANGE STRUCTURE, USE SAVE_HEADER FOR
  // NEW HEADER INFORMATION THIS IS ONLY HERE TO IDENTIFY THE FILE AND GET IT'S
  // SIZE.
  tag = MAKEID('J', 'S', 'A', 'V');
  ...
  g_AsyncSaveCallQueue.QueueCall(
      g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
      CUtlEnvelope<const char *>(name), saveHeader.Base(), saveHeader.TellPut(),
      true, false, (FSAsyncControl_t *)NULL);
  g_AsyncSaveCallQueue.QueueCall(this, &CSaveRestore::DirectoryCopy,
                                 CUtlEnvelope<const char *>(hlPath),
                                 CUtlEnvelope<const char *>(name));

  // Finish all writes and close the save game container
  // @TODO: this async finish all writes has to go away, very expensive and will
  // make game hitchy. switch to a wait on the last async op
  g_AsyncSaveCallQueue.QueueCall(g_pFileSystem,
                                 &IFileSystem::AsyncFinishAllWrites);
  ...
  if (!IsX360() &&
      save_screenshot.GetBool())  // X360TBD: Faster savegame screenshots
  {
...
// Purpose:
//-----------------------------------------------------------------------------
int CSaveRestore::SaveReadHeader(FileHandle_t pFile, GAME_HEADER *pHeader,
                                 int readGlobalState) {
  ...
  if (g_pSaveRestoreFileSystem->Read(&tokenCount, sizeof(int), pFile) !=
      sizeof(int))
    return 0;

  if (g_pSaveRestoreFileSystem->Read(&tokenSize, sizeof(int), pFile) !=
      sizeof(int))
    return 0;

  // At this point we must clean this data up if we fail!
  void *pSaveMemory = SaveAllocMemory(
      sizeof(CSaveRestoreData) + tokenSize + size, sizeof(char));
  if (!pSaveMemory) {
    return 0;
  }
  ...
  if (tokenSize > 0) {
    if (g_pSaveRestoreFileSystem->Read(pszTokenList, tokenSize, pFile) !=
        tokenSize) {
      Finish(pSaveData);
      return 0;
    }
  ...
  if (g_pSaveRestoreFileSystem->Read(pSaveData->GetBuffer(), size, pFile) !=
      size) {
    Finish(pSaveData);
    return 0;
  }

  serverGameDLL->SaveReadFields(pSaveData, "GameHeader", pHeader, NULL,
                                GAME_HEADER::m_DataMap.dataDesc,
                                GAME_HEADER::m_DataMap.dataNumFields);
  ...
  if (pHeader->mapCount == 0) {
    if (g_pSaveRestoreFileSystem->Read(&pHeader->mapCount,
                                       sizeof(pHeader->mapCount),
                                       pFile) != sizeof(pHeader->mapCount))
      return 0;
  }
...
//-----------------------------------------------------------------------------
bool CSaveRestore::CalcSaveGameName(const char *pName, char *output,
                                    int outputStringLength) {
  if (!pName || !pName[0]) return false;

  sprintf_s(output, outputStringLength, "%s%s", GetSaveDir(), pName);
...
bool CSaveRestore::SaveFileExists(const char *pName) {
  ...
  char name[256];
  if (!CalcSaveGameName(pName, name, sizeof(name))) return false;

  return g_pFileSystem->FileExists(name);
}
...
bool CSaveRestore::LoadGame(const char *pName) {
  ...
  if (!CalcSaveGameName(pName, name, std::size(name))) {
  ...
  FileHandle_t save_file = g_pSaveRestoreFileSystem->Open(name, "rb", MOD_DIR);
  if (save_file) {
    char dummy_name[SOURCE_MAX_PATH], comment[SOURCE_MAX_PATH],
        elapsed_time[SOURCE_MAX_PATH];

    if (SaveReadNameAndComment(save_file, dummy_name, comment)) {
      // Elapsed time is the last 6 characters in comment. (mmm:ss)
      strcpy_s(elapsed_time, "??");
      usize i = strlen(comment);

      if (i >= 6) {
        strncpy_s(elapsed_time, (char *)&comment[i - 6], 7);
        elapsed_time[6] = '\0';

        // parse out
        iElapsedMinutes = atoi(elapsed_time);
        iElapsedSeconds = atoi(elapsed_time + 4);
      }
    } else {
      g_pSaveRestoreFileSystem->Close(save_file);
    ...
    g_pSaveRestoreFileSystem->Seek(save_file, 0, FILESYSTEM_SEEK_HEAD);

    if (SaveReadHeader(save_file, &gameHeader, 1)) {
      validload = DirectoryExtract(save_file, gameHeader.mapCount);
    }
  ...
  const bool is_transition_save{gameHeader.originMapName[0] != '\0'};
  const bool is_load_success{
      Host_NewGame(gameHeader.mapName, true, false,
                   is_transition_save ? gameHeader.originMapName : nullptr,
                   is_transition_save ? gameHeader.landmark : nullptr)};
...
void CSaveRestore::SetMostRecentSaveGame(const char *pSaveName) {
  if (pSaveName) {
    Q_strncpy(m_szMostRecentSaveLoadGame, pSaveName,
              sizeof(m_szMostRecentSaveLoadGame));
  } else {
    m_szMostRecentSaveLoadGame[0] = 0;
  }
...
//-----------------------------------------------------------------------------
int CSaveRestore::GetMostRecentElapsedMinutes() {
...
//-----------------------------------------------------------------------------
int CSaveRestore::GetMostRecentElapsedSeconds() {
  return m_MostRecentElapsedSeconds;
}

int CSaveRestore::GetMostRecentElapsedTimeSet() {
...
struct SaveFileHeaderTag_t {
  ...
  bool operator==(const SaveFileHeaderTag_t &rhs) const {
    return (memcmp(this, &rhs, sizeof(SaveFileHeaderTag_t)) == 0);
  }
  bool operator!=(const SaveFileHeaderTag_t &rhs) const {
    return !(*this == rhs);
  }
...
void CSaveRestore::SaveGameStateGlobals(CSaveRestoreData *pSaveData) {
  ...
  header.skillLevel =
      skill.GetInt();  // This is created from an int even though it's a float
  ...
  for (i = 0; i < MAX_LIGHTSTYLES; i++) {
    ...
    if (ligthStyle && ligthStyle[0]) header.lightStyleCount++;
  }

  pSaveData->levelInfo.time = 0;  // prohibits rebase of header.time (why not
                                  // just save time as a field_float and ditch
                                  // this hack?)
  serverGameDLL->SaveWriteFields(pSaveData, "Save Header", &header, NULL,
                                 SAVE_HEADER::m_DataMap.dataDesc,
                                 SAVE_HEADER::m_DataMap.dataNumFields);
  pSaveData->levelInfo.time = header.time;

  // Write adjacency list
  for (i = 0; i < pSaveData->levelInfo.connectionCount; i++)
    serverGameDLL->SaveWriteFields(
        pSaveData, "ADJACENCY", pSaveData->levelInfo.levelList + i, NULL,
        levellist_t::m_DataMap.dataDesc, levellist_t::m_DataMap.dataNumFields);
  ...
  for (i = 0; i < MAX_LIGHTSTYLES; i++) {
    ...
    if (ligthStyle && ligthStyle[0]) {
      ...
      serverGameDLL->SaveWriteFields(pSaveData, "LIGHTSTYLE", &light, NULL,
                                     SAVELIGHTSTYLE::m_DataMap.dataDesc,
                                     SAVELIGHTSTYLE::m_DataMap.dataNumFields);
    }
  }
}

CSaveRestoreData *CSaveRestore::SaveGameStateInit() {
  CSaveRestoreData *pSaveData = serverGameDLL->SaveInit(0);

  return pSaveData;
}

bool CSaveRestore::SaveGameState(bool bTransition,
                                 CSaveRestoreData **ppReturnSaveData,
                                 bool bOpenContainer,
  ...
  sectionsInfo.nBytesDataHeaders =
      pSaveData->AccessCurPos() - sections.pDataHeaders;
  ...
  int nBytesStateFile = sizeof(CURRENT_SAVEFILE_HEADER_TAG) +
                        sizeof(sectionsInfo) + sectionsInfo.nBytesSymbols +
                        sectionsInfo.nBytesDataHeaders +
  ...
  sprintf_s(name, "//%s/%s%s.HL1", MOD_DIR, GetSaveDir(),
            GetSaveGameMapName(sv.GetMapName()));
  SaveMsg("Queue COM_CreatePath\n");
  g_AsyncSaveCallQueue.QueueCall(&COM_CreatePath,
                                 CUtlEnvelope<const char *>(name));
  ...
  g_AsyncSaveCallQueue.QueueCall(
      g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
      CUtlEnvelope<const char *>(name), pBuffer, nBytesStateFile, true, false,
      (FSAsyncControl_t *)NULL);
  pBuffer = NULL;
  ...
  // DON'T FixSlashes on this, it needs to be //MOD
  sprintf_s(name, "//%s/%s%s.HL2", MOD_DIR, GetSaveDir(),
            GetSaveGameMapName(sv.GetMapName()));
  ...
  if (bTransition) FinishAsyncSave();
...
void CSaveRestore::Finish(CSaveRestoreData *save) {
  ...
  if (pTokens) SaveFreeMemory(pTokens);

  entitytable_t *pEntityTable = save->DetachEntityTable();
  if (pEntityTable) SaveFreeMemory(pEntityTable);
...
struct baseclientsectionsold_t {
  ...
  int SumBytes() { return entitysize + headersize + decalsize + symbolsize; }
};
...
int CSaveRestore::LookupRestoreSpotSaveIndex(RestoreLookupTable *table,
                                             int save) {
  int c = table->lookup.Count();
  for (int i = 0; i < c; i++) {
    SaveRestoreTranslate *slot = &table->lookup[i];
    if (slot->savedindex == save) return slot->restoredindex;
  }

  return -1;
}

void CSaveRestore::ReapplyDecal(bool adjacent, RestoreLookupTable *table,
                                decallist_t *entry) {
  ...
  // unlock sting tables to allow changes, helps to find unwanted changes (bebug
  // build only)
  bool oldlock = networkStringTableContainerServer->Lock(false);

  if (adjacent) {
    // These entities might not exist over transitions, so we'll use the saved
    // plane and do a traceline instead
    ...
    if (tr.fraction != 1.0f && !tr.allsolid) {
      ...
      if (dot >= 0.99) {
        // Hack, have to use server traceline stuff to get at an actuall index
        // here
        ...
        if (hit != NULL) {
          ...
          if (entityToHit >= 0) {
            IClientEntity *clientEntity =
                entitylist->GetClientEntity(entityToHit);
            if (!clientEntity) return;

            bool found = false;
            int decalIndex = Draw_DecalIndexFromName(entry->name, &found);
            if (!found) {
              // This is a serious HACK because we're grabbing the index that
              // the server hasn't networked down to us and forcing
              //  the decal name directly.  However, the message should
              //  eventually arrive and set the decal back to the same name on
              //  the server's index...we can live with that I suppose.
              decalIndex = sv.PrecacheDecal(entry->name, RES_FATALIFMISSING);
              Draw_DecalSetName(decalIndex, entry->name);
            }

            g_pEfx->DecalShoot(
                decalIndex, entityToHit, clientEntity->GetModel(),
                clientEntity->GetAbsOrigin(), clientEntity->GetAbsAngles(),
    ...
    int entityToHit = entry->entityIndex != 0 ? LookupRestoreSpotSaveIndex(
                                                    table, entry->entityIndex)
                                              : entry->entityIndex;
    if (entityToHit >= 0) {
      // NOTE: I re-initialized the origin and angles as the decals pos/angle
      // are saved in local space (ie. relative to the entity they are attached
      // to.
      ...
      if (clientEntity) {
        ...
        // This breaks client/server.  However, non-world entities are not in
        // your PVS potentially.
      ...
      if (pModel) {
        ...
        int decalIndex = Draw_DecalIndexFromName(entry->name, &found);
        if (!found) {
          // This is a serious HACK because we're grabbing the index that the
          // server hasn't networked down to us and forcing
          //  the decal name directly.  However, the message should eventually
          //  arrive and set the decal back to the same name on the server's
          //  index...we can live with that I suppose.
          decalIndex = sv.PrecacheDecal(entry->name, RES_FATALIFMISSING);
          Draw_DecalSetName(decalIndex, entry->name);
        }

        g_pEfx->DecalShoot(decalIndex, entityToHit, pModel, vecOrigin, vecAngle,
                           entry->position, 0, flags);
      }
    }
  }
...
void CSaveRestore::RestoreClientState(char const *fileName, bool adjacent) {
  FileHandle_t pFile = g_pSaveRestoreFileSystem->Open(fileName, "rb");
  if (!pFile) {
  ...
  if (magicnumber == SECTION_MAGIC_NUMBER) {
    g_pSaveRestoreFileSystem->Read(&sectionheaderversion,
                                   sizeof(sectionheaderversion), pFile);

    if (sectionheaderversion != SECTION_VERSION_NUMBER) {
      g_pSaveRestoreFileSystem->Close(pFile);
      return;
    }
    g_pSaveRestoreFileSystem->Read(&sections, sizeof(baseclientsections_t),
                                   pFile);
  } else {
    // Rewind
    ...
    g_pSaveRestoreFileSystem->Read(&oldsections,
                                   sizeof(baseclientsectionsold_t), pFile);
  ...
  void *pSaveMemory = SaveAllocMemory(
      sizeof(CSaveRestoreData) + sections.SumBytes(), sizeof(char));
  if (!pSaveMemory) {
    return;
  }
  ...
  // Needed?
  Q_strncpy(pSaveData->levelInfo.szCurrentMapName, fileName,
            sizeof(pSaveData->levelInfo.szCurrentMapName));

  g_pSaveRestoreFileSystem->Read((char *)(pSaveData + 1), sections.SumBytes(),
                                 pFile);
  g_pSaveRestoreFileSystem->Close(pFile);
  ...
  if (sections.symbolsize > 0) {
    void *symbol_save_memory =
        SaveAllocMemory(sections.symbolcount, sizeof(char *), true);
    if (!symbol_save_memory) {
      SaveFreeMemory(pSaveData);
      return;
    }

    pSaveData->InitSymbolTable((char **)symbol_save_memory,
                               sections.symbolcount);
  ...
  pSaveData->Init(
      (char *)(pszTokenList),
      size);  // The point pszTokenList was incremented to the end of the tokens
  ...
  for (int i = 0; i < c; i++) {
    ...
    entry->restoreentityindex =
        LookupRestoreSpotSaveIndex(table, entry->saveentityindex);

    // Adrian: This means we are a client entity with no index to restore and we
    // need our model precached.
    if (entry->restoreentityindex == -1 && entry->classname != NULL_STRING &&
        entry->modelname != NULL_STRING) {
      sv.PrecacheModel(STRING(entry->modelname),
                       RES_FATALIFMISSING | RES_PRELOAD);
      bFixTable = true;
    }
  }
  ...
  if (bFixTable == true) {
    ...
    while (iCount < sv.GetModelPrecacheTable()->GetNumStrings()) {
      string_t szString =
          MAKE_STRING(sv.GetModelPrecacheTable()->GetString(iCount));
  ...
  if (r_decals.GetInt()) {
    for (int i = 0; i < sections.decalcount; i++) {
      decallist_t entry;
      g_ClientDLL->SaveReadFields(pSaveData, "DECALLIST", &entry, NULL,
                                  decallist_t::m_DataMap.dataDesc,
                                  decallist_t::m_DataMap.dataNumFields);
  ...
  for (int i = 0; i < sections.musiccount; i++) {
    ...
    g_ClientDLL->SaveReadFields(pSaveData, "MUSICLIST", &song, NULL,
                                musicsave_t::m_DataMap.dataDesc,
                                musicsave_t::m_DataMap.dataNumFields);
...
bool CSaveRestore::SaveClientState(const char *name) {
  ...
  for (i = 0; i < sections.decalcount; i++) {
    ...
    g_ClientDLL->SaveWriteFields(pSaveData, "DECALLIST", entry, NULL,
                                 decallist_t::m_DataMap.dataDesc,
                                 decallist_t::m_DataMap.dataNumFields);
  }
  ...
  for (i = 0; i < sections.musiccount; ++i) {
    ...
    g_ClientDLL->SaveWriteFields(pSaveData, "MUSICLIST", song, NULL,
                                 musicsave_t::m_DataMap.dataDesc,
                                 musicsave_t::m_DataMap.dataNumFields);
  }
  ...
  unsigned nBytes = sizeof(CURRENT_SAVEFILE_HEADER_TAG) + sizeof(magicnumber) +
                    sizeof(sectionheaderversion) +
                    sizeof(baseclientsections_t) + sections.symbolsize +
                    sections.headersize + sections.entitysize +
                    sections.decalsize + sections.musicsize;
  ...
  g_AsyncSaveCallQueue.QueueCall(
      g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
      CUtlEnvelope<const char *>(name), pBuffer, nBytes, true, false, nullptr);
...
CSaveRestoreData *CSaveRestore::LoadSaveData(const char *level) {
  ...
  // DON'T FixSlashes on this, it needs to be //MOD
  sprintf_s(name, "//%s/%s%s.HL1", MOD_DIR, GetSaveDir(), level);
  ConMsg("Loading game from %s...\n", name);

  FileHandle_t pFile = g_pSaveRestoreFileSystem->Open(name, "rb");
  if (!pFile) {
    ConMsg("Game load error, couldn't open %s.\n", name);
    return NULL;
  }
  ...
  if (tag != CURRENT_SAVEFILE_HEADER_TAG) return NULL;

  // Read the sections info and the data
  SaveFileSectionsInfo_t sectionsInfo;

  if (g_pSaveRestoreFileSystem->Read(&sectionsInfo, sizeof(sectionsInfo),
                                     pFile) != sizeof(sectionsInfo))
    return NULL;

  void *pSaveMemory = SaveAllocMemory(
      sizeof(CSaveRestoreData) + sectionsInfo.SumBytes(), sizeof(char));
  if (!pSaveMemory) {
    return 0;
  }

  CSaveRestoreData *pSaveData = MakeSaveRestoreData(pSaveMemory);
  Q_strncpy(pSaveData->levelInfo.szCurrentMapName, level,
            sizeof(pSaveData->levelInfo.szCurrentMapName));

  if (g_pSaveRestoreFileSystem->Read((char *)(pSaveData + 1),
                                     sectionsInfo.SumBytes(),
                                     pFile) != sectionsInfo.SumBytes()) {
  ...
  char *pszTokenList =
      (char *)(pSaveData + 1);  // Skip past the CSaveRestoreData structure
...
void CSaveRestore::ParseSaveTables(CSaveRestoreData *pSaveData,
                                   SAVE_HEADER *pHeader, int updateGlobals) {
  int i;
  SAVELIGHTSTYLE light;
  INetworkStringTable *table = sv.GetLightStyleTable();
  ...
  pSaveData->Rebase();
  // Process SAVE_HEADER
  serverGameDLL->SaveReadFields(pSaveData, "Save Header", pHeader, NULL,
                                SAVE_HEADER::m_DataMap.dataDesc,
                                SAVE_HEADER::m_DataMap.dataNumFields);
  ...
  if (updateGlobals) {
    for (i = 0; i < MAX_LIGHTSTYLES; i++) table->SetStringUserData(i, 1, "");
  }

  for (i = 0; i < pHeader->lightStyleCount; i++) {
    serverGameDLL->SaveReadFields(pSaveData, "LIGHTSTYLE", &light, NULL,
                                  SAVELIGHTSTYLE::m_DataMap.dataDesc,
                                  SAVELIGHTSTYLE::m_DataMap.dataNumFields);
    if (updateGlobals) {
      table->SetStringUserData(light.index, Q_strlen(light.style) + 1,
                               light.style);
    }
  }
}

// Purpose: Write out the list of entities that are no longer in the save file
// for this level (they've been moved to another level)
void CSaveRestore::EntityPatchWrite(CSaveRestoreData *pSaveData,
                                    const char *level, bool bAsync) {
  char name[MAX_OSPATH];
  // DON'T FixSlashes on this, it needs to be //MOD
  sprintf_s(name, "//%s/%s%s.HL3", MOD_DIR, GetSaveDir(), level);

  int size = 0;
  for (int i = 0; i < pSaveData->NumEntities(); i++) {
    if (pSaveData->GetEntityInfo(i)->flags & FENTTABLE_REMOVED) size++;
  }
  ...
  if (!bAsync) {
    g_pSaveRestoreFileSystem->AsyncWrite(name, pBuffer, nBytesEntityPatch, true,
                                         false);
    g_pSaveRestoreFileSystem->AsyncFinishAllWrites();
  } else {
    SaveMsg("Queue AsyncWrite (%s)\n", name);
    g_AsyncSaveCallQueue.QueueCall(
        g_pSaveRestoreFileSystem, &ISaveRestoreFileSystem::AsyncWrite,
        CUtlEnvelope<const char *>(name), pBuffer, nBytesEntityPatch, true,
        false, (FSAsyncControl_t *)NULL);
  }
}

// Purpose: Read the list of entities that are no longer in the save file for
// this level (they've been moved to another level) and correct the table
void CSaveRestore::EntityPatchRead(CSaveRestoreData *pSaveData,
                                   const char *level) {
  char name[MAX_OSPATH];
  // DON'T FixSlashes on this, it needs to be //MOD
  sprintf_s(name, "//%s/%s%s.HL3", MOD_DIR, GetSaveDir(),
            GetSaveGameMapName(level));
...
CSaveRestore::RestoreLookupTable *CSaveRestore::FindOrAddRestoreLookupTable(
    char const *mapname) {
...
// Output : int
//-----------------------------------------------------------------------------
void CSaveRestore::BuildRestoredIndexTranslationTable(
    char const *mapname, CSaveRestoreData *pSaveData, bool verbose) {
  char name[256];
...
//			*pLandmarkName -
//-----------------------------------------------------------------------------
void LandmarkOrigin(CSaveRestoreData *pSaveData, Vector &output,
...
//			*pLandmarkName -
//-----------------------------------------------------------------------------
void CSaveRestore::LoadAdjacentEnts(const char *pOldLevel,
                                    const char *pLandmarkName) {
  FinishAsyncSave();
  ...
  for (i = 0; i < currentLevelData.levelInfo.connectionCount; i++) {
    ...
    // currentLevelData.levelInfo.levelList[i].mapName,
    // currentLevelData.levelInfo.levelList[i].landmarkName );
    pSaveData = LoadSaveData(
        GetSaveGameMapName(currentLevelData.levelInfo.levelList[i].mapName));
    ...
    if (pSaveData) {
      ...
      LandmarkOrigin(pSaveData, pSaveData->levelInfo.vecLandmarkOffset,
                     pLandmarkName);
      ...
      if (flags)
        movedCount =
            serverGameDLL->CreateEntityTransitionList(pSaveData, flags);
      ...
      BuildRestoredIndexTranslationTable(
          currentLevelData.levelInfo.levelList[i].mapName, pSaveData, true);
...
void CSaveRestore::ClearSaveDir() { m_bClearSaveDir = true; }
...
void CSaveRestore::DoClearSaveDir() {
  ...
  char save_directory_name[MAX_OSPATH];
  sprintf_s(save_directory_name, "%s", GetSaveDir());
  Q_FixSlashes(save_directory_name);
  // Create save directory if it doesn't exist
  Sys_mkdir(save_directory_name);

  strcat_s(save_directory_name, "*.HL?");
  DirectoryClear(save_directory_name);
}

void CSaveRestore::RequestClearSaveDir() { m_bClearSaveDir = true; }
...
void CSaveRestore::AutoSaveDangerousIsSafe() {
  ...
  if (!m_bWaitingForSafeDangerousSave) return;
  ...
  char szOldName[SOURCE_MAX_PATH], szNewName[SOURCE_MAX_PATH];

  // Rename the screenshot
  sprintf_s(szOldName, "//%s/%sautosavedangerous.tga", MOD_DIR, GetSaveDir());
  sprintf_s(szNewName, "//%s/%sautosave.tga", MOD_DIR, GetSaveDir());
  ...
  sprintf_s(szOldName, "//%s/%sautosavedangerous.sav", MOD_DIR, GetSaveDir());
  sprintf_s(szNewName, "//%s/%sautosave.sav", MOD_DIR, GetSaveDir());
...
static void SaveGame(const CCommand &args) {
  bool bFinishAsync = false, bSetMostRecent = true, bRenameMap = false;

  if (args.ArgC() > 2) {
    for (int i = 2; i < args.ArgC(); i++) {
      if (!_stricmp(args[i], "wait")) {
        bFinishAsync = true;
      } else if (!_stricmp(args[i], "notmostrecent")) {
        bSetMostRecent = false;
      } else if (!_stricmp(args[i], "copymap")) {
        bRenameMap = true;
      }
    }
  }
  ...
  if (bRenameMap) {
    ...
    strcpy_s(szMapName, sv.m_szMapname);
    strcpy_s(sv.m_szMapname, args[1]);
  }

  int iAdditionalSeconds = g_ServerGlobalVariables.curtime -
                           saverestore->GetMostRecentElapsedTimeSet();
  ...
  if (bRenameMap) {
    ...
    strcpy_s(sv.m_szMapname, szMapName);
  }
...
CON_COMMAND_F(save, "Saves current game.", FCVAR_DONTRECORD) {
  ...
  if (!saverestore->IsValidSave()) return;
  ...
  if (strstr(sv.m_szMapname, "background")) {
    ConDMsg(
        "\"background\" is a reserved map name and cannot be saved or "
        "loaded.\n");
    return;
  }
...
// Output : void Host_Savegame_f
//-----------------------------------------------------------------------------
CON_COMMAND_F(xsave, "Saves current game to a 360 storage device.",
              FCVAR_DONTRECORD) {
  // Can we save at this point?
  if (!saverestore->IsValidSave()) return;
...
// Output :
//-----------------------------------------------------------------------------
CON_COMMAND_F(minisave, "Saves game (for current level only!)",
              FCVAR_DONTRECORD) {
  // Can we save at this point?
  if (!saverestore->IsValidSave()) return;

  if (args.ArgC() != 2 || strstr(args[1], "..")) return;
...
static ConVar save_console(
    "save_console", "0", 0,
    "Autosave on the PC behaves like it does on the consoles.");
static ConVar save_huddelayframes(
    "save_huddelayframes", "1", 0,
...
CON_COMMAND(autosave, "Autosave") {
  // Can we save at this point?
  if (!saverestore->IsValidSave() || !sv_autosave.GetBool()) return;
...
CON_COMMAND(autosavedangerous, "AutoSaveDangerous") {
  ...
  // Don't print out "SAVED" unless we're running on an Xbox (in which case it
  // prints "CHECKPOINT").
  bool bConsole = save_console.GetBool();
  char const *pchSaving = "GAMESAVING";
...
CON_COMMAND_AUTOCOMPLETEFILE(load, Host_Loadgame_f, "Load a saved game.",
                             "save", sav);
...
CON_COMMAND(save_finish_async, "") { FinishAsyncSave(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CSaveRestore::Init() {
  int minplayers = 1;
  // serverGameClients should have been initialized by the CModAppSystemGroup
  // Create method (so it's before the Host_Init stuff which calls this)
  Assert(serverGameClients);
  ...
  if (!serverGameClients || (minplayers == 1)) {
    GetSaveMemory();
...
bool CSaveRestore::IsSaveInProgress() { return g_bSaveInProgress; }
