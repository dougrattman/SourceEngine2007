...
//-----------------------------------------------------------------------------
bool CToolFrameworkInternal::Connect(CreateInterfaceFn factory) { return true; }
...
//-----------------------------------------------------------------------------
// Purpose:  Here's where systems can access other interfaces implemented by
// this object
//  Returns NULL if it doesn't implement the requested interface
...
static bool CToolFrameworkInternal_QuitHandler(void *pvUserData) {
  CToolFrameworkInternal *tfm =
      reinterpret_cast<CToolFrameworkInternal *>(pvUserData);
  if (tfm) {
    return tfm->CanQuit();
  }
  return true;
...
void CToolFrameworkInternal::LoadToolsFromLibrary(const char *dllname) {
  ...
  if (!module) {
    Warning(
        "CToolFrameworkInternal::LoadToolsFromLibrary:  Unable to load '%s'\n",
        dllname);
    return;
  }
  ...
  if (!factory) {
    ...
    Warning(
        "CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' has no "
        "factory\n",
        dllname);
    return;
  }

  IToolDictionary *dictionary =
      (IToolDictionary *)factory(VTOOLDICTIONARY_INTERFACE_VERSION, NULL);
  if (!dictionary) {
    Sys_UnloadModule(module);
    Warning(
        "CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' doesn't "
        "support '%s'\n",
  ...
  if (!dictionary->Connect(g_AppSystemFactory)) {
    Sys_UnloadModule(module);
    Warning(
        "CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' connection "
        "phase failed.\n",
        dllname);
    return;
  }
  ...
  if (dictionary->Init() != INIT_OK) {
    Sys_UnloadModule(module);
    Warning(
        "CToolFrameworkInternal::LoadToolsFromLibrary:  Dll '%s' "
...
// Are we using tools?
//-----------------------------------------------------------------------------
bool CToolFrameworkInternal::InToolMode() { return m_bInToolMode; }
...
void CToolFrameworkInternal::LoadTools() {
  ...
  Assert(kv);
  if (kv &&
      kv->LoadFromFile(g_pFileSystem, "enginetools.txt", "EXECUTABLE_PATH")) {
    for (KeyValues *tool = kv->GetFirstSubKey(); tool != NULL;
...
//-----------------------------------------------------------------------------
const char *CToolFrameworkInternal::GetEntityData(
    const char *pActualEntityData) {
  if (m_nActiveToolIndex >= 0) {
    IToolSystem *tool = m_ToolSystems[m_nActiveToolIndex];
    Assert(tool);
...
//			height -
//-----------------------------------------------------------------------------
void CToolFrameworkInternal::AdjustEngineViewport(int &x, int &y, int &width,
                                                  int &height) {
  if (m_nActiveToolIndex >= 0) {
    IToolSystem *tool = m_ToolSystems[m_nActiveToolIndex];
    Assert(tool);
...
//-----------------------------------------------------------------------------
bool CToolFrameworkInternal::SetupEngineView(Vector &origin, QAngle &angles,
                                             float &fov) {
  if (m_nActiveToolIndex < 0) return false;
...
// Output : int
//-----------------------------------------------------------------------------
int CToolFrameworkInternal::GetToolCount() { return m_ToolSystems.Count(); }
...
void CToolFrameworkInternal::SwitchToTool(int index) {
  if ((m_ToolSystems.Count() < 1) || (index >= m_ToolSystems.Count())) return;
...
bool CToolFrameworkInternal::IsTopmostTool(const IToolSystem *sys) {
  if (m_ToolSystems.Count() <= 0 || (m_nActiveToolIndex < 0)) return false;
...
const IToolSystem *CToolFrameworkInternal::GetToolSystem(int index) const {
  if ((index < 0) || (index >= m_ToolSystems.Count())) return NULL;
...
bool CToolFrameworkInternal::GetSoundSpatialization(
    int iUserData, int guid, SpatializationInfo_t &info) {
  if (m_nActiveToolIndex >= 0) {
    IToolSystem *tool = m_ToolSystems[m_nActiveToolIndex];
    Assert(tool);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CToolFrameworkInternal,
                                  IToolFrameworkInternal,
                                  VTOOLFRAMEWORK_INTERFACE_VERSION,
...
EXPOSE_SINGLE_INTERFACE(CClientEngineTools, IClientEngineTools,
                        VCLIENTENGINETOOLS_INTERFACE_VERSION);
...
//			height -
//-----------------------------------------------------------------------------
void CClientEngineTools::AdjustEngineViewport(int &x, int &y, int &width,
                                              int &height) {
  g_ToolFrameworkInternal.AdjustEngineViewport(x, y, width, height);
}

bool CClientEngineTools::SetupEngineView(Vector &origin, QAngle &angles,
                                         float &fov) {
...
EXPOSE_SINGLE_INTERFACE(CServerEngineTools, IServerEngineTools,
                        VSERVERENGINETOOLS_INTERFACE_VERSION);
