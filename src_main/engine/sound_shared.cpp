...
#define SND_GAIN_COMP_EXP_MAX \
  2.5f  // Increasing SND_GAIN_COMP_EXP_MAX fits compression curve more closely
        // to original gain curve as it approaches 1.0.
#define SND_GAIN_COMP_EXP_MIN 0.8f

#define SND_GAIN_COMP_THRESH \
  0.5f  // gain value above which gain curve is rounded to approach 1.0
...
float SND_GetGainFromMult(float gain, float dist_mult, vec_t dist) {
  ...
  // if gain passess threshold, compress gain curve such that gain smoothly
  // approaches 1.0
  ...
  if (gain > SND_GAIN_COMP_THRESH) {
    ...
    if (sndlvl > SND_DB_MED) {
      ...
      snd_gain_comp_power =
          RemapVal((float)sndlvl, SND_DB_MED, SND_DB_MAX, SND_GAIN_COMP_EXP_MAX,
                   SND_GAIN_COMP_EXP_MIN);
    }

    // calculate crossover point

    Y = -1.0 / (pow(SND_GAIN_COMP_THRESH, snd_gain_comp_power) *
                (SND_GAIN_COMP_THRESH - 1));
  ...
  if (gain < snd_gain_min.GetFloat()) {
    // sounds less than snd_gain_min fall off to 0 in distance it took them to
    // fall to snd_gain_min
