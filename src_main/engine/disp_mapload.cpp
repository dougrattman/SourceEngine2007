...
// This makes sure that whoever is creating and deleting CDispInfos frees them
// all (and calls their destructors) before the module is gone.
...
static CDispGroup *FindCombo(CUtlVector<CDispGroup *> &combos, int idLMPage,
                             IMaterial *pMaterial) {
  for (int i = 0; i < combos.Size(); i++) {
    if (combos[i]->m_LightmapPageID == idLMPage &&
        combos[i]->m_pMaterial == pMaterial)
      return combos[i];
  }
  return NULL;
}

static CDispGroup *AddCombo(CUtlVector<CDispGroup *> &combos, int idLMPage,
                            IMaterial *pMaterial) {
...
static void BuildDispSurfInit(model_t *pWorld, CCoreDispInfo *pBuildDisp,
                              SurfaceHandle_t worldSurfID) {
  if (!IS_SURF_VALID(worldSurfID)) return;
  ...
  if (MSurf_VertCount(worldSurfID) != 4) return;

#ifndef SWDS
  BuildMSurfaceVerts(pWorld->brush.pShared, worldSurfID, surfPoints,
                     surfTexCoords, surfLightCoords);
#endif
  ...
  Vector vecS = MSurf_TexInfo(worldSurfID)
                    ->textureVecsTexelsPerWorldUnits[0]
                    .AsVector3D();
  Vector vecT = MSurf_TexInfo(worldSurfID)
                    ->textureVecsTexelsPerWorldUnits[1]
                    .AsVector3D();
  ...
  for (int ndxLuxel = 0; ndxLuxel < 4; ndxLuxel++) {
    switch (ndxLuxel) {
      ...
      case 1: {
        uv.Init(0.0f, (float)lightmapHeight);
        break;
      }
      case 2: {
        uv.Init((float)lightmapWidth, (float)lightmapHeight);
        break;
      }
      case 3: {
        uv.Init((float)lightmapWidth, 0.0f);
        break;
      }
    }
...
VertexFormat_t ComputeDisplacementStaticMeshVertexFormat(
    const IMaterial *pMaterial, const CDispGroup *pCombo,
...
void AddEmptyMesh(model_t *pWorld, CDispGroup *pCombo,
                  const ddispinfo_t *pMapDisps, int *pDispInfos, int nDisps,
                  int nTotalVerts, int nTotalIndices) {
  CMatRenderContextPtr pRenderContext(materials);

  CGroupMesh *pMesh = new CGroupMesh;
  pCombo->m_Meshes.AddToTail(pMesh);

  VertexFormat_t vertexFormat = ComputeDisplacementStaticMeshVertexFormat(
      pCombo->m_pMaterial, pCombo, pMapDisps);
  pMesh->m_pMesh = pRenderContext->CreateStaticMesh(
      vertexFormat, TEXTURE_GROUP_STATIC_VERTEX_BUFFER_DISP);
...
void FillStaticBuffer(CGroupMesh *pMesh, CDispInfo *pDisp,
                      const CCoreDispInfo *pCoreDisp, const CDispVert *pVerts,
                      int nLightmaps) {
#ifndef SWDS
  ...
  for (int i = 0; i < nVerts; i++) {
    // NOTE: position comes from our system-memory buffer so when you're
    // restoring
    //       static buffers (from alt+tab), it includes changes from terrain
    //       mods.
    ...
    if (nLightmaps > 1) {
      SurfComputeLightmapCoordinate(ctx, pDisp->GetParent(),
                                    pDisp->m_Verts[i].m_vPos, lightCoord);
...
void CDispInfo::CopyMapDispData(const ddispinfo_t *pBuildDisp) {
  ...
  // and multiply by 3!
  // These can be non-0 in the case of task switch restore
...
void DispInfo_CreateMaterialGroups(
    model_t *pWorld, const MaterialSystem_SortInfo_t *pSortInfos) {
  for (int iDisp = 0; iDisp < pWorld->brush.pShared->numDispInfos; iDisp++) {
    CDispInfo *pDisp = GetModelDisp(pWorld, iDisp);

    int idLMPage =
        pSortInfos[MSurf_MaterialSortID(pDisp->m_ParentSurfID)].lightmapPageID;

    CDispGroup *pCombo = FindCombo(
        g_DispGroups, idLMPage, MSurf_TexInfo(pDisp->m_ParentSurfID)->material);
    if (!pCombo)
      pCombo = AddCombo(g_DispGroups, idLMPage,
                        MSurf_TexInfo(pDisp->m_ParentSurfID)->material);
...
void DispInfo_LinkToParentFaces(model_t *pWorld, const ddispinfo_t *pMapDisps,
                                int nDisplacements) {
  for (int iDisp = 0; iDisp < nDisplacements; iDisp++) {
    ...
    Assert(pMapDisp->m_iMapFace >= 0 &&
           pMapDisp->m_iMapFace < pWorld->brush.pShared->numsurfaces);
...
void DispInfo_CreateEmptyStaticBuffers(model_t *pWorld,
                                       const ddispinfo_t *pMapDisps) {
  ...
  for (int i = 0; i < g_DispGroups.Size(); i++) {
    ...
    for (int iDisp = 0; iDisp < pCombo->m_DispInfos.Size(); iDisp++) {
      ...
      // If we're going to pass our vertex buffer limit, or we're at the last
      // one, make a static buffer and fill it up.
      if ((nTotalVerts + nVerts) > MAX_STATIC_BUFFER_VERTS ||
          (nTotalIndices + nIndices) > MAX_STATIC_BUFFER_INDICES) {
        AddEmptyMesh(pWorld, pCombo, pMapDisps, &pCombo->m_DispInfos[iStart],
                     iDisp - iStart, nTotalVerts, nTotalIndices);
        ...
        AddEmptyMesh(pWorld, pCombo, pMapDisps, &pCombo->m_DispInfos[iStart],
                     iDisp - iStart + 1, nTotalVerts + nVerts,
...
// Information: Setup the CCoreDispInfo using the ddispinfo_t and have it
// translate the data into a format we'll copy into the rendering structures.
// This roundaboutness is because of legacy code. It should all just be stored
// in the map file, but it's not a high priority right now.
//-----------------------------------------------------------------------------
bool DispInfo_CreateFromMapDisp(model_t *pWorld, int iDisp,
  ...
  pCoreDisp->InitDispInfo(pMapDisp->power, pMapDisp->minTess,
                          pMapDisp->smoothingAngle, pVerts, pTris);
  pCoreDisp->SetNeighborData(pMapDisp->m_EdgeNeighbors,
                             pMapDisp->m_CornerNeighbors);

  // Copy the allowed verts list.
  ErrorIfNot(pCoreDisp->GetAllowedVerts().GetNumDWords() ==
                 sizeof(pMapDisp->m_AllowedVerts) / 4,
             ("DispInfo_StoreMapData: size mismatch in 'allowed verts' list"));
  for (int iVert = 0; iVert < pCoreDisp->GetAllowedVerts().GetNumDWords();
       ++iVert) {
    pCoreDisp->GetAllowedVerts().SetDWord(iVert,
                                          pMapDisp->m_AllowedVerts[iVert]);
  }

  // Build the reset of the intermediate data from the initial map displacement
  // data.
  BuildDispSurfInit(pWorld, pCoreDisp, pDisp->GetParent());
  if (!pCoreDisp->Create()) return false;
  ...
  pDisp->m_iLightmapSamplePositionStart =
      pMapDisp->m_iLightmapSamplePositionStart;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
void DispInfo_CreateStaticBuffersAndTags(model_t *pWorld, int iDisp,
                                         CCoreDispInfo *pCoreDisp,
  ...
  FillStaticBuffer(pDisp->m_pMesh, pDisp, pCoreDisp, pVerts,
                   pDisp->NumLightMaps());
...
void SetupMeshReaders(model_t *pWorld, int nDisplacements) {
  for (int iDisp = 0; iDisp < nDisplacements; iDisp++) {
    ...
    pDisp->m_MeshReader.BeginRead_Direct(desc, pDisp->NumVerts(),
                                         pDisp->m_nIndices);
  }
}
...
bool DispInfo_LoadDisplacements(model_t *pWorld, bool bRestoring) {
  ...
  int nDisplacements =
      CMapLoadHelper::LumpSize(LUMP_DISPINFO) / sizeof(ddispinfo_t);
  int nLuxels = CMapLoadHelper::LumpSize(LUMP_DISP_LIGHTMAP_ALPHAS);
  int nSamplePositionBytes =
      CMapLoadHelper::LumpSize(LUMP_DISP_LIGHTMAP_SAMPLE_POSITIONS);
  ...
  if (bRestoring) {
    ...
    ErrorIfNot(g_DispLMAlpha.Count() == nLuxels,
               ("DispInfo_LoadDisplacements: lightmap alpha counts (%d and %d) "
                "don't match.",
                g_DispLMAlpha.Count(), nLuxels));
  } else {
    // Create the displacements.
    pWorld->brush.pShared->numDispInfos = nDisplacements;
    pWorld->brush.pShared->hDispInfos =
        DispInfo_CreateArray(pWorld->brush.pShared->numDispInfos);
    ...
    lhDispLMPositions.LoadLumpData(0, nSamplePositionBytes,
                                   g_DispLightmapSamplePositions.Base());
  }
  ...
  // using temporary storage that is not the stack for compatibility with
  // console stack
#ifndef _X360
...
#endif
  ErrorIfNot(nDisplacements <= MAX_MAP_DISPINFO,
             ("DispInfo_LoadDisplacements: nDisplacements (%d) > "
              "MAX_MAP_DISPINFO (%d)",
  ...
  // First, create "groups" (or "combos") which contain all the displacements
  // that use the same material and lightmap.
  ...
  for (iDisp = 0; iDisp < nDisplacements; ++iDisp) {
    ...
    if (!pMapDisp) continue;

    // Load the vertices from the file.
    int nVerts = NUM_DISP_POWER_VERTS(pMapDisp->power);
    ErrorIfNot(
        nVerts <= MAX_DISPVERTS,
        ("DispInfo_LoadDisplacements: invalid vertex count (%d)", nVerts));
    lhDispVerts.LoadLumpData(iCurVert * sizeof(CDispVert),
                             nVerts * sizeof(CDispVert), tempVerts);
    iCurVert += nVerts;

    // Load the triangle indices from the file.
    int nTris = NUM_DISP_POWER_TRIS(pMapDisp->power);
    ErrorIfNot(nTris <= MAX_DISPTRIS,
               ("DispInfo_LoadDisplacements: invalid tri count (%d)", nTris));
    lhDispTris.LoadLumpData(iCurTri * sizeof(CDispTri),
                            nTris * sizeof(CDispTri), tempTris);
    iCurTri += nTris;

    // Now create the CoreDispInfo and the base CDispInfo.
    if (!DispInfo_CreateFromMapDisp(pWorld, iDisp, pMapDisp, aCoreDisps[iDisp],
                                    tempVerts, tempTris, pSortInfos,
                                    bRestoring))
      return false;
  }
  ...
  for (iDisp = 0; iDisp < nDisplacements; ++iDisp) {
    DispInfo_CreateStaticBuffersAndTags(pWorld, iDisp, aCoreDisps[iDisp],
                                        tempVerts);

    // Copy over the now blended normals
    CDispInfo *pDisp = GetModelDisp(pWorld, iDisp);
    pDisp->CopyCoreDispVertData(aCoreDisps[iDisp],
                                pDisp->m_BumpSTexCoordOffset);
  }
...
void CDispInfo::CopyCoreDispVertData(const CCoreDispInfo *pCoreDisp,
...
bool CDispInfo::CopyCoreDispData(model_t *pWorld,
                                 const MaterialSystem_SortInfo_t *pSortInfos,
...
int CDispInfo::NumLightMaps() {
  return (MSurf_Flags(m_ParentSurfID) & SURFDRAW_BUMPLIGHT) ? NUM_BUMP_VECTS + 1
                                                            : 1;
}

//-----------------------------------------------------------------------------
// Purpose:
// NOTE: You cannot use the builddisp.cpp IsTriWalkable, IsTriBuildable
// functions
...
void BuildTagData(CCoreDispInfo *pCoreDisp, CDispInfo *pDisp) {
  ...
  for (iTri = 0; iTri < pCoreDisp->GetTriCount(); ++iTri) {
    if (pCoreDisp->IsTriTag(iTri, DISPTRI_TAG_WALKABLE)) {
      pCoreDisp->GetTriIndices(iTri, pDisp->m_pWalkIndices[nWalkCount],
    ...
    if (pCoreDisp->IsTriTag(iTri, DISPTRI_TAG_BUILDABLE)) {
      pCoreDisp->GetTriIndices(iTri, pDisp->m_pBuildIndices[nBuildCount],
...
int FindNeighborCornerVert(CCoreDispInfo *pDisp, const Vector &vecPoint) {
  ...
  for (int iCorner = 0; iCorner < 4; ++iCorner) {
    ...
    CVertIndex viCornerVert =
        pDispHelper->GetPowerInfo()->GetCornerPointIndex(iCorner);
...
void UpdateTangentSpace(CCoreDispInfo *pDisp, int iVert, const Vector &vNormal,
                        const Vector &vTanS) {
  Vector tanT;
...
void UpdateTangentSpace(CCoreDispInfo *pDisp, const CVertIndex &index,
                        const Vector &vNormal, const Vector &vTanS) {
...
void BlendSubNeighbors(CCoreDispInfo **ppListBase, int nListSize) {
  ...
  for (int iDisp = 0; iDisp < nListSize; ++iDisp) {
    ...
    for (int iEdge = 0; iEdge < 4; ++iEdge) {
      ...
      // Check to see if we have sub-neighbors - defines a t-junction in this
      // world.  If not, then the normal blend edges function will catch it all.
      if (!pEdge->m_SubNeighbors[0].IsValid() ||
          !pEdge->m_SubNeighbors[1].IsValid())
        continue;
      ...
      CCoreDispInfo *pNeighbor1 =
          ppListBase[pEdge->m_SubNeighbors[0].GetNeighborIndex()];
      CCoreDispInfo *pNeighbor2 =
          ppListBase[pEdge->m_SubNeighbors[1].GetNeighborIndex()];
      ...
      if (iCorners[0] != -1 && iCorners[1] != -1) {
        CVertIndex viCorners[2] = {
            pNeighbor1->GetCornerPointIndex(iCorners[0]),
            pNeighbor2->GetCornerPointIndex(iCorners[1])};

        // Accumulate the normals at the mid-point of the primary edge and
        // corners of the sub-neighbors.
      ...
      if (pEdge->m_SubNeighbors[i].IsValid())
        if (nNeighbors < 512)
          iNeighbors[nNeighbors++] =
              pEdge->m_SubNeighbors[i].GetNeighborIndex();
    }
  }
  ...
  for (int iDisp = 0; iDisp < nListSize; ++iDisp) {
    ...
    for (int iCorner = 0; iCorner < 4; iCorner++) {
      ...
      for (usize iNeighbor = 0; iNeighbor < nNeighbors; iNeighbor++) {
        ...
        CCoreDispInfo *pNeighbor = ppListBase[iNBListIndex];
        UpdateTangentSpace(pNeighbor, nbCornerVerts[iNeighbor], vAverage,
                           vAvgTanS);
      }
    }
  }
    ...
    for (int iEdge = 0; iEdge < 4; ++iEdge) {
      ...
      for (int iSubEdge = 0; iSubEdge < 2; ++iSubEdge) {
        ...
        while (it.Next()) {
          // Blend the two.
          if (!it.IsLastVert()) {
            Vector vecAverage = pDisp->GetNormal(it.GetVertIndex()) +
                                pNeighbor->GetNormal(it.GetNBVertIndex());
            Vector vAvgTanS = pDisp->GetTangentS(it.GetVertIndex()) +
                              pNeighbor->GetTangentS(it.GetNBVertIndex());
            VectorNormalize(vecAverage);
            VectorNormalize(vAvgTanS);
            UpdateTangentSpace(pDisp, it.GetVertIndex(), vecAverage, vAvgTanS);
            UpdateTangentSpace(pNeighbor, it.GetNBVertIndex(), vecAverage,
                               vAvgTanS);
          }
          ...
          for (int iTween = iPrevPos + 1; iTween < iCurPos; iTween++) {
            ...
            Vector vecNormal;
            VectorLerp(pDisp->GetNormal(viPrevPos),
                       pDisp->GetNormal(it.GetVertIndex()), flPercent,
                       vecNormal);
            VectorNormalize(vecNormal);
            Vector vAvgTanS;
            VectorLerp(pDisp->GetTangentS(viPrevPos),
                       pDisp->GetTangentS(it.GetVertIndex()), flPercent,
                       vAvgTanS);
            VectorNormalize(vAvgTanS);
