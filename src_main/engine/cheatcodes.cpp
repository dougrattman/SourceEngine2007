...
void ReadCheatCommandsFromFile(const char *pchFileName) {
  ...
  if (pCheatCodeKeys->LoadFromFile(g_pFullFileSystem, pchFileName, NULL)) {
    KeyValues *pKey = NULL;
    for (pKey = pCheatCodeKeys->GetFirstTrueSubKey(); pKey;
      ...
      Q_strncpy(pNewCheatCode->szName, pKey->GetName(),
                CHEAT_NAME_MAX_LEN);  // Get the name
      pNewCheatCode->bDevOnly =
          (pKey->GetInt("dev", 0) != 0);  // Get developer only flag
      pNewCheatCode->iCodeLength = 0;     // Start at zero code elements
      Q_strncpy(
          pNewCheatCode->szCommand,
          pKey->GetString("command", "echo \"Cheat code has no command!\""),
          CHEAT_COMMAND_MAX_LEN);

      KeyValues *pSubKey = NULL;
      for (pSubKey = pKey->GetFirstSubKey(); pSubKey;
           pSubKey = pSubKey->GetNextKey()) {
        const char *pchType = pSubKey->GetName();
        if (Q_strcmp(pchType, "code") == 0) {
          AssertMsg((pNewCheatCode->iCodeLength < CHEAT_NAME_MAX_LEN),
                    "Cheat code elements exceeded max!");

          pNewCheatCode->pButtonCodes[pNewCheatCode->iCodeLength] =
              g_pInputSystem->StringToButtonCode(pSubKey->GetString());
          ++pNewCheatCode->iCodeLength;
        }
      }

      if (pNewCheatCode->iCodeLength < CHEAT_NAME_MAX_LEN) {
        // If it's activation is a subsequence of another cheat, the longer
        // cheat can't be activated!
        DevWarning("Cheat code \"%s\" has less than %i code elements!",
                   pKey->GetName(), CHEAT_NAME_MAX_LEN);
      }
    }
  }
...
void CheckCheatCodes() {
  ...
  for (int iCheatCode = 0; iCheatCode < iNumCheatCodes; ++iCheatCode) {
    ...
    int iLogIndex =
        s_nKeyLogIndex -
        pCheatCode
            ->iCodeLength;  // Check code against the back chunk of the log

    if (iLogIndex < 0) continue;  // There's less codes in the log than we need

    int iCode = 0;

    while (iCode < pCheatCode->iCodeLength &&
           pCheatCode->pButtonCodes[iCode] == s_pKeyLog[iLogIndex]) {
      ++iCode;
      ++iLogIndex;
    }
