...
struct LightingState_t {
  ...
  bool HasAmbientColors() {
    for (int i = 0; i < 6; i++) {
      if (!r_boxcolor[i].IsZero(1e-4f)) return true;
    }
    return false;
  }

  void AddLocalLight(dworldlight_t* pLocalLight) {
    if (numlights >= MAXLOCALLIGHTS) return;
    for (int i = 0; i < numlights; i++) {
      if (locallight[i] == pLocalLight) return;
    }
...
// Called each frame to check for reinitializing the lightcache based on cvar
// changes.
void R_StudioCheckReinitLightingCache();

// static prop version
LightingState_t* LightcacheGetStatic(
    LightCacheHandle_t cache, ITexture** pEnvCubemap,
    unsigned int flags = (LIGHTCACHEFLAGS_STATIC | LIGHTCACHEFLAGS_DYNAMIC |
...
ITexture* LightcacheGetDynamic(
    const Vector& origin, LightingState_t& lightingState,
...
void InvalidateStaticLightingCache();
