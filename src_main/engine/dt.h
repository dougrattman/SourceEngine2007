...
class CDeltaBitsReader {
 public:
  ...
  // ReadNextPropIndex returns -1), call this so it won't assert in its
  // destructor.
  void ForceFinished();
...
class CSendNode {
 public:
  ...
  // Each datatable property (without SPROP_PROXY_ALWAYS_YES set) gets a unique
  // index here. The engine stores arrays of CSendProxyRecipients with the
  // results of the proxies and indexes the results with this index.
  //
...
class CFastLocalTransferInfo {
 public:
  ...
  CUtlVector<CFastLocalTransferPropInfo>
      m_OtherProps;  // Props that must be copied slowly (proxies and all).
};
...
class CSendTablePrecalc {
 ...
 public:
  ...
  CUtlVector<CProxyPathEntry>
      m_ProxyPathEntries;  // For each proxy index, this is all the DT proxies
                           // that generate it.
  ...
  // Each datatable in a SendTable's tree gets a proxy index, and its properties
  // reference that.
  ...
  // This is precalculated in single player to allow faster direct copying of
  // the entity data from the server entity to the client entity.
  CFastLocalTransferInfo m_FastLocalTransfer;

  // This tells how many data table properties there are without
  // SPROP_PROXY_ALWAYS_YES. Arrays allocated with this size can be indexed by
  // CSendNode::GetDataTableProxyIndex().
