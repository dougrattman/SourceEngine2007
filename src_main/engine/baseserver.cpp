...
// game server talks to Steam's master server updater (which is on the game
// server's machine, not the Steam servers).
#define MASTER_SERVER_UPDATE_INTERVAL 2.0

static void SvTagsChangeCallback(IConVar *pConVar, const char *pOldValue,
                                 float flOldValue) {
...
ConVar sv_region("sv_region", "-1", FCVAR_NONE,
                 "The region of the world to report this server in.");
static ConVar sv_instancebaselines(
    "sv_instancebaselines", "1", FCVAR_DEVELOPMENTONLY,
    "Enable instanced baselines. Saves network overhead.");
static ConVar sv_stats("sv_stats", "1", 0, "Collect CPU usage stats");
static ConVar sv_enableoldqueries(
    "sv_enableoldqueries", "0", 0,
    "Enable support for old style (HL1) server queries");
static ConVar sv_password("sv_password", "",
                          FCVAR_NOTIFY | FCVAR_PROTECTED | FCVAR_DONTRECORD,
                          "Server password for entry into multiplayer games");
ConVar sv_tags("sv_tags", "", FCVAR_NOTIFY,
...
ConVar sv_visiblemaxplayers(
    "sv_visiblemaxplayers", "-1", 0,
    "Overrides the max players reported to prospective clients");
ConVar sv_alternateticks(
    "sv_alternateticks", (IsX360()) ? "1" : "0", FCVAR_SPONLY,
    "If set, server only simulates entities on even numbered ticks.\n");
ConVar sv_allow_wait_command(
    "sv_allow_wait_command", "1", FCVAR_REPLICATED,
    "Allow or disallow the wait command on clients connected to this server.");
ConVar sv_allow_color_correction(
    "sv_allow_color_correction", "1", FCVAR_REPLICATED,
...
#ifndef NO_STEAM
static void SetMasterServerKeyValue(ISteamMasterServerUpdater *pUpdater,
                                    IConVar *pConVar) {
  ConVarRef var(pConVar);
...
static void ServerNotifyVarChangeCallback(IConVar *pConVar,
                                          const char *pOldValue,
                                          float flOldValue) {
  if (!pConVar->IsFlagSet(FCVAR_NOTIFY)) return;
#ifndef NO_STEAM
  ISteamMasterServerUpdater *pUpdater = SteamMasterServerUpdater();
  if (!pUpdater) {
    // This will force it to send all the rules whenever the master server
    // updater is there.
...
CBaseServer::CBaseServer()
    : m_ServerQueryChallenges(
          0, 1024)  // start with 1K of entries, and alloc in 1K chunks
{
  m_State = ss_dead;
...
bool CBaseServer::CheckChallengeNr(netadr_t &adr, int nChallengeValue) {
  ...
  for (int i = 0; i < m_ServerQueryChallenges.Count(); i++) {
    if (adr.CompareAdr(m_ServerQueryChallenges[i].adr, true))  // base adr only
    {
      ...
      if (net_time >
          (m_ServerQueryChallenges[i].time +
           CHALLENGE_LIFETIME))  // allow challenge values to last for 1 hour
      {
    ...
    if (net_time > (m_ServerQueryChallenges[i].time + CHALLENGE_LIFETIME)) {
      m_ServerQueryChallenges.FastRemove(i);
      i--;  // backup one as we just shifted the whole vector back by the
            // deleted element
    }
  }
...
#define MAX_REUSE_PER_IP \
  5  // 5 outstanding connect request within timeout window, to account for NATs
...
int CBaseServer::GetNextUserID() {
  ...
  for (int i = 0; i < m_Clients.Count() + 1; i++) {
    ...
    for (iClient = 0; iClient < m_Clients.Count(); iClient++) {
      if (m_Clients[iClient]->GetUserID() == nTestID) break;
    }

    // Ok, no client has this ID, so return it.
    if (iClient == m_Clients.Count()) return nTestID;
  }
...
================
*/
IClient *CBaseServer::ConnectClient(netadr_t &adr, int protocol, int challenge,
  ...
  if (!CheckChallengeType(
          client, nNextUserID, adr, authProtocol, hashedCDkey,
          cdKeyLen))  // we use the client pointer to track steam requests
  {
    return NULL;
  }
  ...
  INetChannel *netchan =
      NET_CreateNetChannel(m_Socket, &adr, adr.ToString(), client);
  ...
  // Will get reset from userinfo, but this value comes from sv_updaterate ( the
  // default )
...
Return true if this challenge number is correct for this host (for server
queries)
================
*/
...
bool CBaseServer::ValidInfoChallenge(netadr_t &adr, const char *nugget) {
  ...
  if (RequireValidChallenge(adr)) {
    if (Q_stricmp(nugget,
                  A2S_KEY_STRING))  // if the string isn't equal then fail out
    {
      return false;
    }
  }
...
bool CBaseServer::ProcessConnectionlessPacket(netpacket_t *packet) {
  ...
  switch (c) {
    case A2A_PING:
      NET_OutOfBandPrintf(packet->source, packet->from, "%c00000000000000",
                          A2A_ACK);
      break;
    ...
    case C2S_CONNECT: {
      ...
      if (authProtocol == PROTOCOL_STEAM) {
        ...
        ConnectClient(packet->from, protocol, challengeNr, authProtocol, name,
                      password, cdkey,
                      keyLen);  // cd key is actually a raw encrypted key
      } else {
        msg.ReadString(cdkey, sizeof(cdkey));
        ConnectClient(packet->from, protocol, challengeNr, authProtocol, name,
                      password, cdkey, strlen(cdkey));
      }

    } break;

    case A2A_CUSTOM:  // TODO fire game event with string and adr so 3rd party
                      // tool can get it
      break;
    ...
    default: {
      if (IsUsingMasterLegacyMode()) {
        ...
        if (!IsHLTV()) pThis = (CGameServer *)this;

        master->HandleUnknown(packet, this, pThis);
      }
#ifndef NO_STEAM
      // We don't understand it, let the master server updater at it.
      else if (SteamMasterServerUpdater() &&
               Steam3Server().IsMasterServerUpdaterSharingGameSocket()) {
        SteamMasterServerUpdater()->HandleIncomingPacket(
            packet->message.GetBasePointer(),
            packet->message.TotalBytesAvailable(),
            BigLong(packet->from.GetIP()), packet->from.GetPort());

        // This is where it will usually want to respond to something
        // immediately by sending some packets, so check for that immediately.
...
int CBaseServer::GetNumPlayers() {
  ...
  for (int i = 0; i < maxPlayers; i++) {
    const player_info_t *pi =
        (const player_info_t *)m_pUserInfoTable->GetStringUserData(i, NULL);

    if (!pi) continue;

    if (pi->fakeplayer) continue;  // don't count bots
...
bool CBaseServer::GetPlayerInfo(int nClientIndex, player_info_t *pinfo) {
  ...
  player_info_t *pi = (player_info_t *)GetUserInfoTable()->GetStringUserData(
      nClientIndex, NULL);
...
void CBaseServer::ReplyChallenge(netadr_t &adr) {
  ...
  if (authprotocol == PROTOCOL_STEAM) {
    ...
    if (!SteamGameServer()->GSGetSteam2GetEncryptionKeyToSendToNewClient(
            szEncryptionKey, &encryptionsize, sizeof(szEncryptionKey)) ||
        encryptionsize == 0) {
    ...
    if (encryptionsize > STEAM_KEYSIZE) {
      Warning(
          "SteamGetEncryptionKeyToSendToNewClient:  Key size too big (%i/%i)\n",
    ...
    uint64_t unSteamID = steamID.ConvertToUint64();
...
int CBaseServer::GetChallengeNr(netadr_t &adr) {
  ...
  // note the 0x0FFF of the top 16 bits, so that -1 will never be sent as a
  // challenge
  m_ServerQueryChallenges[newEntry].challenge =
      (RandomInt(0, 0x0FFF) << 16) | RandomInt(0, 0xFFFF);
...
void CBaseServer::CalculateCPUUsage(void) {
  ...
  // only do this every 1 second
  {
...
#else
    // linux CPU % code here :)
    static int32_t lastrunticks, lastcputicks;
    static float lastAvg = 0;
    ...
    int32_t ctime, stime, start_time;
    FILE *pFile;
    int32_t runticks, cputicks;
    ...
    fscanf(pFile,
           "%d %s %c %d %d %d %d %d %lu %lu \
			...
			%lu %lu %lu %lu %lu %lu",
           &dummy, statFile, &dummy, &dummy, &dummy, &dummy, &dummy, &dummy,
           &dummy, &dummy,  // end of first line
           &dummy, &dummy, &dummy, &ctime, &stime, &dummy, &dummy, &dummy,
           &dummy, &dummy,  // end of second
           &start_time, &dummy, &dummy, &dummy, &dummy, &dummy, &dummy, &dummy,
           &dummy, &dummy,  // end of third
           &dummy, &dummy, &dummy, &dummy, &dummy, &dummy);
    fclose(pFile);

    runticks =
        infos.uptime * HZ - start_time;  // time the process has been running
    ...
    if (lastrunticks == 0) {
      ...
      m_fCPUPercent =
          (float)(cputicks - lastcputicks) / (float)(runticks - lastrunticks);
    }
    ...
    // limit checking :)
    if (m_fCPUPercent > 0.999) m_fCPUPercent = 0.999;
    if (m_fCPUPercent < 0) m_fCPUPercent = 0;
...
void CBaseServer::InactivateClients(void) {
  for (int i = 0; i < m_Clients.Count(); i++) {
    ...
    if (cl->IsFakeClient() && !cl->IsHLTV()) {
      // If we don't do this, it'll have a bunch of extra steam IDs for
...
void CBaseServer::CheckTimeouts(void) {
  ...
  for (i = 0; i < m_Clients.Count(); i++) {
    ...
    if (cl->IsFakeClient() || !cl->IsConnected()) continue;
...
const char *CBaseServer::CompressPackedEntity(ServerClass *pServerClass,
                                              const char *data, int &bits) {
  ...
  SendTable_WriteAllDeltaProps(pServerClass->m_pTable, pBaselineData,
                               nBaselineBits, data, bits, -1, &writeBuf);
...
// uncompresses a
const char *CBaseServer::UncompressPackedEntity(PackedEntity *pPackedEntity,
                                                int &bits) {
  UnpackedDataCache_t *pdc =
      framesnapshotmanager->GetCachedUncompressedEntity(pPackedEntity);
  ...
  bf_read newBuf("UncompressPackedEntity2", pPackedEntity->GetData(),
                 Bits2Bytes(pPackedEntity->GetNumBits()));
  bf_write outBuf("UncompressPackedEntity3", pdc->data, MAX_PACKEDENTITY_DATA);

  Assert(pPackedEntity->m_pClientClass);

  RecvTable_MergeDeltas(pPackedEntity->m_pClientClass->m_pRecvTable, &oldBuf,
                        &newBuf, &outBuf);
...
bool CBaseServer::CheckProtocol(netadr_t &adr, int nProtocol) {
  if (nProtocol != PROTOCOL_VERSION) {
    ...
    if (nProtocol > PROTOCOL_VERSION) {
      RejectConnection(adr,
                       "This server is using an older protocol ( %i ) than "
                       "your client ( %i ).\n",
    ...
    {
      RejectConnection(adr,
                       "This server is using a newer protocol ( %i ) than your "
                       "client ( %i ).\n",
                       PROTOCOL_VERSION, nProtocol);
    }
    return false;
...
================
*/
bool CBaseServer::CheckChallengeType(CBaseClient *client, int nNewUserID,
                                     netadr_t &adr, int nAuthProtocol,
                                     const char *pchLogonCookie, int cbCookie) {
  if (AllowDebugDedicatedServerOutsideSteam()) return true;
  ...
  if ((nAuthProtocol == PROTOCOL_HASHEDCDKEY) &&
      (Q_strlen(pchLogonCookie) <= 0 || Q_strlen(pchLogonCookie) != 32)) {
  ...
  if (nAuthProtocol == PROTOCOL_STEAM) {
    ...
    Q_memset(&client->m_NetworkID.uid.steamid, 0x0,
             sizeof(client->m_NetworkID.uid.steamid));
...
#ifndef NO_STEAM
    if (!Steam3Server().NotifyClientConnect(client, nNewUserID, checkAdr,
                                            pchLogonCookie, cbCookie) &&
        !Steam3Server().BLanOnly())  // the userID isn't alloc'd yet so we need
    ...
    if (!Steam3Server().NotifyLocalClientConnect(client))  // the userID isn't
                                                           // alloc'd yet so we
                                                           // need to fill it in
                                                           // manually
    {
...
bool CBaseServer::CheckIPRestrictions(const netadr_t &adr, int nAuthProtocol) {
  ...
  if (Steam3Server().BLanOnly()) {
    ...
    if (adr.CompareClassBAdr(net_local_adr)) return true;

    // allow connection, if client has a private IP
    if (adr.IsReservedAdr()) return true;
...
bool CBaseServer::CheckPassword(netadr_t &adr, const char *password,
                                const char *name) {
  const char *server_password = GetPassword();

  if (!server_password) return true;  // no password set
...
float CBaseServer::GetTime() const { return m_nTickCount * m_flTickInterval; }

float CBaseServer::GetFinalTickTime() const {
  return (m_nTickCount + (host_frameticks - host_currentframetick)) *
         m_flTickInterval;
}
...
void CBaseServer::Clear(void) {
  ...
  // Use a different limit on the signon buffer, so we can save some memory in
  // SP (for xbox).
...
================
*/
void CBaseServer::RejectConnection(const netadr_t &adr, const char *fmt, ...) {
  va_list argptr;
  char text[1024];
...
bool CBaseServer::GetClassBaseline(ServerClass *pClass, void const **pData,
                                   int *pDatalen) {
  if (sv_instancebaselines.GetInt()) {
    ...
    *pData = GetInstanceBaselineTable()->GetStringUserData(
        pClass->m_InstanceBaselineIndex, pDatalen);
...
bool CBaseServer::ShouldUpdateMasterServer() {
  // If the game server itself is ever running, then it's the one who gets to
  // update the master server. (SourceTV will not update it in this case).
  return true;
}
...
void CBaseServer::CheckMasterServerRequestRestart() {
  ...
  if (!SteamMasterServerUpdater() ||
      !SteamMasterServerUpdater()->WasRestartRequested())
    return;
#endif
...
#endif
  {
    Msg("Your server needs to be restarted in order to receive the latest "
        "update.\n");
    Log("Your server needs to be restarted in order to receive the latest "
        "update.\n");
  } else {
...
void CBaseServer::UpdateMasterServer() {
#ifndef NO_STEAM
  if (!ShouldUpdateMasterServer()) return;
  ...
  if (!SteamMasterServerUpdater()) return;

  // Only update every so often.
  double flCurTime = Plat_FloatTime();
  if (flCurTime - m_flLastMasterServerUpdateTime <
  ...
  if (NET_IsDedicated() && sv_region.GetInt() == -1) {
    sv_region.SetValue(
        255);  // HACK!HACK! undo me once we want to enforce regions

    // Log_Printf( "You must set sv_region in your server.cfg or use +sv_region
    // on the command line\n" );  Con_Printf( "You must set sv_region in your
    // server.cfg or use +sv_region on the command line\n" );
    // Cbuf_AddText( "quit\n" );
    // return;
  }

  static bool bUpdateMasterServers = !CommandLine()->FindParm("-nomaster");
  if (!bUpdateMasterServers) return;

  bool bActive = IsActive() && IsMultiplayer() && g_bEnableMasterServerUpdater;
  if (serverGameDLL && serverGameDLL->ShouldHideServer()) bActive = false;
...
void CBaseServer::UpdateMasterServerRules() {
  ...
  // Need to respond with game directory, game name, and any server variables
  // that have been set that
  //  effect rules.  Also, probably need a hook into the .dll to respond with
  //  additional rule information.
  ConCommandBase *var;
  for (var = g_pCVar->GetCommands(); var; var = var->GetNext()) {
    if (!(var->IsFlagSet(FCVAR_NOTIFY))) continue;
  ...
  // Ok.. it's all updated, only send incremental updates now until we decide
  // they're all dirty.
...
void CBaseServer::UpdateMasterServerBasicData() {
  ...
  pUpdater->SetBasicServerData(PROTOCOL_VERSION, IsDedicated(),
                               sv_region.GetString(), gpszProductString,
                               nMaxReportedClients, (GetPassword() != NULL),
...
void CBaseServer::ForwardPacketsFromMasterServerUpdater() {
  ...
  while (1) {
    uint32_t netadrAddress;
    uint16_t netadrPort;
    unsigned char packetData[16 * 1024];
    int len = p->GetNextOutgoingPacket(packetData, sizeof(packetData),
                                       &netadrAddress, &netadrPort);
    if (len <= 0) break;
...
void CBaseServer::RunFrame(void) {
  ...
  SendPendingServerInfo();  // send outstanding signon packets after ALL user
...
CBaseClient *CBaseServer::CreateFakeClient(const char *name) {
  ...
  if (sv_stressbots.GetBool()) {
    // 0.0.0.0:0 signifies a bot. It'll plumb all the way
    // down to winsock calls but it won't make them.
    netadr_t bot_adr(0, 0);
    netchan = NET_CreateNetChannel(m_Socket, &bot_adr, bot_adr.ToString(),
                                   fakeclient, true);
  }
...
void CBaseServer::Shutdown(void) {
  ...
  for (int i = m_Clients.Count() - 1; i >= 0; i--) {
    ...
    if (cl->IsConnected()) {
      ...
      // free any memory do this out side here in case the reason the server is
      // shutting down
      //  is because the listen server client typed disconnect, in which case we
      //  won't call cl->DropClient, but the client might have some frame
      //  snapshot references left over, etc.
      cl->Clear();
    }
  ...
  if (SteamMasterServerUpdater()) SteamMasterServerUpdater()->NotifyShutdown();
...
void CBaseServer::BroadcastMessage(INetMessage &msg, bool onlyActive,
                                   bool reliable) {
  for (int i = 0; i < m_Clients.Count(); i++) {
    CBaseClient *cl = m_Clients[i];
    ...
    if (!cl->SendNetMsg(msg, reliable)) {
      if (msg.IsReliable() || reliable) {
        DevMsg(
            "BroadcastMessage: Reliable broadcast message overflow for client "
            "%s",
            cl->GetClientName());
      }
    }
...
void CBaseServer::BroadcastMessage(INetMessage &msg, IRecipientFilter &filter) {
  if (filter.IsInitMessage()) {
    // This really only applies to the first player to connect, but that works
    // in single player well enought
    if (IsActive()) {
      ConDMsg(
          "SV_BroadcastMessage: Init message being created after signon buffer "
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (index < 1 || index > m_Clients.Count()) {
        Msg("SV_BroadcastMessage:  Recipient Filter for message type %i "
            "(reliable: %s, init: %s) with bogus client index (%i) in list of "
            "%i clients\n",
            msg.GetType(), filter.IsReliable() ? "yes" : "no",
            filter.IsInitMessage() ? "yes" : "no", index, num);
      ...
      if (!cl->SendNetMsg(msg)) {
        if (msg.IsReliable()) {
          DevMsg(
              "BroadcastMessage: Reliable filter message overflow for client "
              "%s",
              cl->GetClientName());
        }
      }
...
//			*msg -
//-----------------------------------------------------------------------------
static ConVar sv_debugtempentities("sv_debugtempentities", "0", 0,
                                   "Show temp entity bandwidth usage.");

static bool CEventInfo_LessFunc(CEventInfo *const &lhs,
                                CEventInfo *const &rhs) {
  return lhs->classID < rhs->classID;
}

void CBaseServer::WriteTempEntities(CBaseClient *client,
                                    CFrameSnapshot *pCurrentSnapshot,
                                    CFrameSnapshot *pLastSnapshot,
                                    bf_write &buf, int ev_max) {
  ...
  // Build list of events sorted by send table classID (makes the delta work
  // better in cases with a lot of the same message type )
  while (pSnapshot && ((int)sorted.Count() < ev_max)) {
    for (int i = 0; i < pSnapshot->m_nTempEntities; ++i) {
      ...
      sorted.Insert(event);
      // More space still
      if ((int)sorted.Count() >= ev_max) break;
    }
  ...
  for (int i = sorted.FirstInorder(); i != sorted.InvalidIndex();
    ...
    if (pLastEvent && pLastEvent->classID == event->classID) {
      buffer.WriteOneBit(0);  // delta against last temp entity

      int startBit = bDebug ? buffer.GetNumBitsWritten() : 0;

      SendTable_WriteAllDeltaProps(event->pSendTable, pLastEvent->pData,
                                   pLastEvent->bits, event->pData, event->bits,
                                   -1, &buffer);
      ...
      if (IsMultiplayer()) {
        SendTable_WriteAllDeltaProps(event->pSendTable,
                                     ...
                                     0, event->pData, event->bits, -1, &buffer);
      } else {
...
// tags are recalculated and uploaded to the master server when the convar is
// changed.
...
void CBaseServer::AddTag(const char *pszTag) {
  ...
  for (int i = 0; i < TagList.Count(); i++) {
    ...
    if (!Q_stricmp(TagList[i], pszTag)) return;
  }
