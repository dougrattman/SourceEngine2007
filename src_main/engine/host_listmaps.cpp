...
// Input  : valid -
//-----------------------------------------------------------------------------
void CMapListItem::SetValid(int valid) { m_nValid = valid; }
...
// Input  : ts -
//-----------------------------------------------------------------------------
void CMapListItem::SetFileTimestamp(long ts) { m_lFileTimestamp = ts; }
...
// timestamp Input  : ts - Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
...
int CMapListItem::CheckFSHeaderVersion(char const *name) {
  ...
  return (header.version >= MINBSPVERSION && header.version <= BSPVERSION)
             ? VALID
             : INVALID;
}
...
void CMapListManager::RefreshList(void) {
  ...
  if (realtime < m_flLastRefreshTime + MIN_REFRESH_INTERVAL) return;
...
int CMapListManager::IsMapValid(int index) const {
  if (!m_Items.IsValidIndex(index)) return false;
...
char const *CMapListManager::GetMapName(int index) const {
  if (!m_Items.IsValidIndex(index)) return "Invalid!!!";
...
// Output : static void
//-----------------------------------------------------------------------------
static bool MapList_CheckPrintMap(const char *pakorfilesys, const char *mapname,
                                  int valid, bool showoutdated, bool verbose) {
...
// Output : static int
//-----------------------------------------------------------------------------
static int MapList_CountMaps(const char *pszSubString, bool listobsolete,
                             int &maxitemlength) {
...
// Input  : *pszSubString -
//-----------------------------------------------------------------------------
static int MapList_ListMaps(const char *pszSubString, bool listobsolete,
                            bool verbose, int maxcount, int maxitemlength,
                            char maplist[][64]) {
  int substringlength = 0;
  if (pszSubString && pszSubString[0]) {
    substringlength = strlen(pszSubString);
  ...
  for (showOutdated = listobsolete ? 1 : 0; showOutdated >= 0; showOutdated--) {
    ...
    for (int i = 0; i < g_MapListMgr.GetMapCount(); i++) {
      ...
      if (!substringlength ||
          !Q_strnicmp(&mapname[5], pszSubString, substringlength)) {
        if (MapList_CheckPrintMap("(fs)", &mapname[5], valid,
                                  showOutdated ? true : false, verbose)) {
          if (maxitemlength != 0) {
...
// Output : int
//-----------------------------------------------------------------------------
static int _Host_Map_f_CompletionFunc(
    char const *cmdname, char const *partial,
...
// Output : int
//-----------------------------------------------------------------------------
static int Host_Map_f_CompletionFunc(
...
// Output : int
//-----------------------------------------------------------------------------
static int Host_Map_Commentary_f_CompletionFunc(
    char const *partial, char commands[COMMAND_COMPLETION_MAXITEMS]
                                      [COMMAND_COMPLETION_ITEM_LENGTH]) {
...
// Output : int
//-----------------------------------------------------------------------------
static int Host_Changelevel_f_CompletionFunc(
    char const *partial, char commands[COMMAND_COMPLETION_MAXITEMS]
                                      [COMMAND_COMPLETION_ITEM_LENGTH]) {
...
// Output : int
//-----------------------------------------------------------------------------
static int Host_Changelevel2_f_CompletionFunc(
    char const *partial, char commands[COMMAND_COMPLETION_MAXITEMS]
                                      [COMMAND_COMPLETION_ITEM_LENGTH]) {
...
static void Host_Maps_f(const CCommand &args) {
  ...
  if (args.ArgC() == 2) {
    pszSubString = args[1];
    if (!pszSubString || !pszSubString[0]) return;
  }

  if (pszSubString && (pszSubString[0] == '*')) pszSubString = NULL;
...
static ConCommand map_background(
    "map_background", Host_Map_Background_f,
    "Runs a map as the background to the main menu.", FCVAR_DONTRECORD,
    Host_Map_f_CompletionFunc);
static ConCommand map_commentary(
    "map_commentary", Host_Map_Commentary_f,
    "Start playing, with commentary, on a specified map.", FCVAR_DONTRECORD,
    Host_Map_Commentary_f_CompletionFunc);
static ConCommand changelevel("changelevel", Host_Changelevel_f,
                              "Change server to the specified map",
                              FCVAR_DONTRECORD,
                              Host_Changelevel_f_CompletionFunc);
static ConCommand changelevel2(
