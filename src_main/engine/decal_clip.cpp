...
class CPlane_Top {
 public:
  static inline bool Inside(CDecalVert *pVert) {
    return pVert->m_ctCoords.y < 1;
  }
  static inline float Clip(CDecalVert *one, CDecalVert *two) {
    return (1 - one->m_ctCoords.y) / (two->m_ctCoords.y - one->m_ctCoords.y);
  }
};
...
class CPlane_Left {
 public:
  static inline bool Inside(CDecalVert *pVert) {
    return pVert->m_ctCoords.x > 0;
  }
  static inline float Clip(CDecalVert *one, CDecalVert *two) {
    return one->m_ctCoords.x / (one->m_ctCoords.x - two->m_ctCoords.x);
  }
};
...
class CPlane_Right {
 public:
  static inline bool Inside(CDecalVert *pVert) {
    return pVert->m_ctCoords.x < 1;
  }
  static inline float Clip(CDecalVert *one, CDecalVert *two) {
    return (1 - one->m_ctCoords.x) / (two->m_ctCoords.x - one->m_ctCoords.x);
  }
};
...
class CPlane_Bottom {
 public:
  static inline bool Inside(CDecalVert *pVert) {
    return pVert->m_ctCoords.y > 0;
  }
  static inline float Clip(CDecalVert *one, CDecalVert *two) {
    return one->m_ctCoords.y / (one->m_ctCoords.y - two->m_ctCoords.y);
  }
};
...
static inline void Intersect(Clipper &clip, CDecalVert *one, CDecalVert *two,
                             CDecalVert *out) {
...
template <class Clipper>
static inline int SHClip(CDecalVert *pDecalClipVerts, int vertCount,
                         CDecalVert *out, Clipper &clip) {
  int j, outCount;
  CDecalVert *s, *p;
...
CDecalVert *R_DoDecalSHClip(CDecalVert *pInVerts, CDecalVert *pOutVerts,
  ...
  if (pOutVerts == NULL) pOutVerts = &g_DecalClipVerts[0];
  ...
  outCount =
      SHClip(&g_DecalClipVerts2[0], outCount, &g_DecalClipVerts[0], left);
  outCount =
      SHClip(&g_DecalClipVerts[0], outCount, &g_DecalClipVerts2[0], right);
  ...
  if (outCount) {
    if (pDecal->flags & FDECAL_CLIPTEST) {
      ...
      // If there are exactly 4 verts and they are all 0,1 tex coords, then
      // we've got an unclipped decal A more precise test would be to calculate
      // the texture area and make sure it's one, but this should work as well.
      if (outCount == 4) {
        int clipped = 0;
        ...
        for (int j = 0; j < outCount && !clipped; j++, v++) {
          ...
          if (((fabs(s) > DECAL_CLIP_EPSILON) &&
               (fabs(1.0f - s) > DECAL_CLIP_EPSILON)) ||
              ((fabs(t) > DECAL_CLIP_EPSILON) &&
               (fabs(1.0f - t) > DECAL_CLIP_EPSILON))) {
            clipped = 1;
          }
        }

        // We didn't need to clip this decal, it's a quad covering the full
        // texture space, optimize subsequent frames.
  ...
  for (int i = 0; i < outCount; ++i) {
    VectorMA(pOutVerts[i].m_vPos, OVERLAY_AVOID_FLICKER_NORMAL_OFFSET,
             vecNormal, pOutVerts[i].m_vPos);
  }
...
// Build the initial list of vertices from the surface verts into the global
// array, 'verts'.
void R_SetupDecalVertsForMSurface(decal_t *pDecal, SurfaceHandle_t surfID,
                                  Vector textureSpaceBasis[3],
                                  CDecalVert *pVerts) {
  for (int j = 0; j < MSurf_VertCount(surfID); j++) {
    int vertIndex =
        host_state.worldbrush->vertindices[MSurf_FirstVertIndex(surfID) + j];
    ...
    pVerts[j].m_ctCoords.x =
        DotProduct(pVerts[j].m_vPos, textureSpaceBasis[0]) - pDecal->dx + .5f;
    pVerts[j].m_ctCoords.y =
        DotProduct(pVerts[j].m_vPos, textureSpaceBasis[1]) - pDecal->dy + .5f;
...
void R_DecalComputeBasis(Vector const &surfaceNormal, Vector const *pSAxis,
  ...
  if (pSAxis) {
    ...
    if (DotProduct(textureSpaceBasis[1], textureSpaceBasis[1]) > 1e-6) {
      // S = N cross T
      CrossProduct(textureSpaceBasis[2], textureSpaceBasis[1],
                   textureSpaceBasis[0]);
  ...
  if (fabs(surfaceNormal[2]) > SIN_45_DEGREES) {
    ...
    // T = S cross N
    CrossProduct(textureSpaceBasis[0], textureSpaceBasis[2],
                 textureSpaceBasis[1]);
...
void R_SetupDecalClip(CDecalVert *&pOutVerts, decal_t *pDecal,
                      Vector &vSurfNormal, IMaterial *pMaterial,
                      Vector textureSpaceBasis[3], float decalWorldScale[2]) {
  //	if ( pOutVerts == NULL )
  //		pOutVerts = &g_DecalClipVerts[0];

  R_SetupDecalTextureSpaceBasis(pDecal, vSurfNormal, pMaterial,
                                textureSpaceBasis, decalWorldScale);

  // Generate texture coordinates for each vertex in decal s,t space
  // probably should pre-generate this, store it and use it for decal-decal
  // collisions as in R_DecalsIntersect()
...
CDecalVert *R_DecalVertsClip(CDecalVert *pOutVerts, decal_t *pDecal,
  ...
  // Figure out where the decal maps onto the surface.
  R_SetupDecalClip(pOutVerts, pDecal, MSurf_Plane(surfID).normal, pMaterial,
                   textureSpaceBasis, decalWorldScale);

  // Build the initial list of vertices from the surface verts.
  R_SetupDecalVertsForMSurface(pDecal, surfID, textureSpaceBasis,
                               g_DecalClipVerts);

  return R_DoDecalSHClip(g_DecalClipVerts, pOutVerts, pDecal,
                         MSurf_VertCount(surfID), MSurf_Plane(surfID).normal,
                         pVertCount);
}
