...
void SV_ClearWorld(void) {
  ...
  for (int i = 0; i < 3; i++) {
    if (host_state.worldmodel->mins[i] < MIN_COORD_INTEGER ||
        host_state.worldmodel->maxs[i] > MAX_COORD_INTEGER) {
      Host_EndGame(
...
static void CM_GetCollideableTriggerTestBox(ICollideable *pCollide,
                                            Vector *pMins, Vector *pMaxs,
...
class CTouchLinks : public IPartitionEnumerator {
 public:
  ...
  IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    if (!pTriggerEntity) return ITERATION_CONTINUE;
    ...
    if (pTriggerCollideable->GetSolidFlags() & FSOLID_USE_TRIGGER_BOUNDS) {
      ...
      pTriggerCollideable->WorldSpaceTriggerBounds(&vecTriggerMins,
                                                   &vecTriggerMaxs);
      ...
      trace_t tr;
      g_pEngineTraceServer->ClipRayToCollideable(m_Ray, MASK_SOLID,
                                                 pTriggerCollideable, &tr);
 ...
 public:
  ...
  IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    if (m_pTrigger->GetSolidFlags() & FSOLID_USE_TRIGGER_BOUNDS) {
      ...
      trace_t tr;
      g_pEngineTraceServer->ClipRayToCollideable(ray, MASK_SOLID, m_pTrigger,
                                                 &tr);
      if (!(tr.contents & MASK_SOLID)) return ITERATION_CONTINUE;
    }
  ...
  void HandleTouchedEntities() {
    for (int i = 0; i < m_TouchedEntities.Count(); ++i) {
      serverGameEnts->MarkEntitiesAsTouching(m_TouchedEntities[i],
                                             m_pTriggerEntity);
    }
  }
...
void SV_SolidMoved(edict_t *pSolidEnt, ICollideable *pSolidCollide,
  ...
  if (!pPrevAbsOrigin) {
    ...
    SpatialPartition()->EnumerateElementsAlongRay(
        PARTITION_ENGINE_TRIGGER_EDICTS, touchEnumerator.m_Ray, false,
        &touchEnumerator);
