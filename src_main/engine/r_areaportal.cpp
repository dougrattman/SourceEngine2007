...
static inline bool GetPortalScreenExtents(dareaportal_t *pPortal,
  ...
  for (int j = 0; j < nPassCount; ++j) {
    ...
    for (i = 0; i < nStartVerts; i++) {
      clip->v0[i] =
          pBrushData->m_pClipPortalVerts[pPortal->m_FirstClipPortalVert + i];
...
static void R_FlowThroughArea(int area, const Vector &vecVisOrigin,
  ...
  for (int iAreaPortal = 0; iAreaPortal < pArea->numareaportals;
       iAreaPortal++) {
    Assert(pArea->firstareaportal + iAreaPortal < pBrushData->m_nAreaPortals);
    dareaportal_t *pAreaPortal =
        &pBrushData->m_pAreaPortals[pArea->firstareaportal + iAreaPortal];
    ...
    if (!R_TestBit(cl.m_chAreaPortalBits, pAreaPortal->m_PortalKey)) continue;

    // Make sure the viewer is on the right side of the portal to see through
    // it.
    cplane_t *pPlane = &pBrushData->planes[pAreaPortal->planenum];
    // Use the specified vis origin to test backface culling, or the main view
    // if none was specified
    float flDist = pPlane->normal.Dot(vecVisOrigin) - pPlane->dist;
    if (flDist < -0.1f) continue;

    // If the client doesn't want this area visible, don't try to flow into it.
    if (!R_TestBit(cl.m_chAreaBits, pAreaPortal->otherarea)) continue;
    ...
    float fDistTolerance =
        (pVisData) ? (pVisData->m_fDistToAreaPortalTolerance) : (0.1f);
    if (flDist > fDistTolerance) {
      portalVis = GetPortalScreenExtents(pAreaPortal, &clipTmp, portalRect,
    ...
    if (portalVis) {
      ...
      if (GetRectIntersection(&portalRect, pClipRect, &intersection)) {
        ...
        R_FlowThroughArea(pAreaPortal->otherarea, vecVisOrigin, &intersection,
                          pVisData, pReflectionWaterHeight);
      }
    }
  }
...
static void R_SetupVisibleAreaFrustums() {
  ...
  if (viewSetup.m_bOrtho) {
    ...
    // Assuming a view plane distance of 1, figure out the boundaries of a
    // window the view would project into given the FOV.
  ...
  // Now scale the portals as specified in the normalized view frustum
  // (-1,-1,1,1) into our view window and generate planes out of that.
  for (int i = 0; i < g_nVisibleAreas; i++) {
    CAreaCullInfo *pInfo = &g_AreaCullInfo[g_VisibleAreas[i]];

    CPortalRect portalWindow;
    portalWindow.left =
        RemapVal(pInfo->m_Rect.left, -1, 1, viewWindow.left, viewWindow.right);
    portalWindow.right =
        RemapVal(pInfo->m_Rect.right, -1, 1, viewWindow.left, viewWindow.right);
    portalWindow.top =
        RemapVal(pInfo->m_Rect.top, -1, 1, viewWindow.bottom, viewWindow.top);
    portalWindow.bottom = RemapVal(pInfo->m_Rect.bottom, -1, 1,
                                   viewWindow.bottom, viewWindow.top);

    if (viewSetup.m_bOrtho) {
      // Left and right planes...
      float orgOffset = DotProduct(CurrentViewOrigin(), CurrentViewRight());
      pInfo->m_Frustum.SetPlane(FRUSTUM_LEFT, PLANE_ANYZ, CurrentViewRight(),
                                portalWindow.left + orgOffset);
      pInfo->m_Frustum.SetPlane(FRUSTUM_RIGHT, PLANE_ANYZ, -CurrentViewRight(),
                                -portalWindow.right - orgOffset);

      // Top and bottom planes...
      orgOffset = DotProduct(CurrentViewOrigin(), CurrentViewUp());
      pInfo->m_Frustum.SetPlane(FRUSTUM_TOP, PLANE_ANYZ, CurrentViewUp(),
                                portalWindow.top + orgOffset);
      pInfo->m_Frustum.SetPlane(FRUSTUM_BOTTOM, PLANE_ANYZ, -CurrentViewUp(),
                                -portalWindow.bottom - orgOffset);
    } else {
      Vector normal;
      ...
      pInfo->m_Frustum.SetPlane(FRUSTUM_RIGHT, PLANE_ANYZ, normal,
                                DotProduct(normal, CurrentViewOrigin()));

      // left side
      normal = CurrentViewRight() - portalWindow.left * CurrentViewForward();
      VectorNormalize(normal);  // OPTIMIZE: This is unnecessary for culling
      pTest = pInfo->m_Frustum.GetPlane(FRUSTUM_LEFT);
      pInfo->m_Frustum.SetPlane(FRUSTUM_LEFT, PLANE_ANYZ, normal,
                                DotProduct(normal, CurrentViewOrigin()));
      ...
      pInfo->m_Frustum.SetPlane(FRUSTUM_TOP, PLANE_ANYZ, normal,
                                DotProduct(normal, CurrentViewOrigin()));
      ...
      pInfo->m_Frustum.SetPlane(FRUSTUM_BOTTOM, PLANE_ANYZ, normal,
                                DotProduct(normal, CurrentViewOrigin()));

      // farz
      pInfo->m_Frustum.SetPlane(
          FRUSTUM_FARZ, PLANE_ANYZ, -CurrentViewForward(),
          DotProduct(
              -CurrentViewForward(),
              CurrentViewOrigin() + CurrentViewForward() * viewSetup.zFar));
    }
    ...
    if (r_snapportal.GetInt() >= 0) {
      if (g_VisibleAreas[i] == r_snapportal.GetInt()) {
        pInfo->m_Frustum.SetPlane(
            FRUSTUM_NEARZ, PLANE_ANYZ, CurrentViewForward(),
            DotProduct(CurrentViewForward(), CurrentViewOrigin()));
        pInfo->m_Frustum.SetPlane(
            FRUSTUM_FARZ, PLANE_ANYZ, -CurrentViewForward(),
            DotProduct(-CurrentViewForward(),
                       CurrentViewOrigin() + CurrentViewForward() * 500));
...
inline bool R_CullNodeInternal(mnode_t *pNode, int &nClipMask,
  ...
  if (nClipMask & FRUSTUM_CLIP_RIGHT) {
    ...
    flCenterDotNormal =
        DotProduct(pNode->m_vecCenter, pPlane->normal) - pPlane->dist;
    flHalfDiagDotAbsNormal = DotProduct(pNode->m_vecHalfDiagonal,
                                        frustum.GetAbsNormal(FRUSTUM_RIGHT));
    if (flCenterDotNormal + flHalfDiagDotAbsNormal < 0.0f) return true;
  ...
  if (nClipMask & FRUSTUM_CLIP_LEFT) {
    ...
    flCenterDotNormal =
        DotProduct(pNode->m_vecCenter, pPlane->normal) - pPlane->dist;
    flHalfDiagDotAbsNormal = DotProduct(pNode->m_vecHalfDiagonal,
                                        frustum.GetAbsNormal(FRUSTUM_LEFT));
    if (flCenterDotNormal + flHalfDiagDotAbsNormal < 0.0f) return true;
    if (flCenterDotNormal - flHalfDiagDotAbsNormal < 0.0f)
  ...
  if (nClipMask & FRUSTUM_CLIP_TOP) {
    ...
    flCenterDotNormal =
        DotProduct(pNode->m_vecCenter, pPlane->normal) - pPlane->dist;
    flHalfDiagDotAbsNormal =
        DotProduct(pNode->m_vecHalfDiagonal, frustum.GetAbsNormal(FRUSTUM_TOP));
    if (flCenterDotNormal + flHalfDiagDotAbsNormal < 0.0f) return true;
    if (flCenterDotNormal - flHalfDiagDotAbsNormal < 0.0f)
  ...
  if (nClipMask & FRUSTUM_CLIP_BOTTOM) {
    ...
    flCenterDotNormal =
        DotProduct(pNode->m_vecCenter, pPlane->normal) - pPlane->dist;
    flHalfDiagDotAbsNormal = DotProduct(pNode->m_vecHalfDiagonal,
                                        frustum.GetAbsNormal(FRUSTUM_BOTTOM));
    if (flCenterDotNormal + flHalfDiagDotAbsNormal < 0.0f) return true;
    if (flCenterDotNormal - flHalfDiagDotAbsNormal < 0.0f)
...
bool R_CullNode(Frustum_t *pAreaFrustum, mnode_t *pNode, int &nClipMask) {
  ...
  if ((!g_bViewerInSolidSpace) && (pNode->area > 0)) {
    ...
    if (!R_IsAreaVisible(pNode->area)) return true;
...
void R_SetupAreaBits(int iForceViewLeaf /* = -1 */,
  ...
  if (host_state.worldbrush->leafs[leaf].contents & CONTENTS_SOLID ||
    ...
    // Draw everything if we're in solid space or if r_portalsopenall is true
    // (used for building cubemaps)
    g_bViewerInSolidSpace = true;

    if (r_ShowViewerArea.GetInt()) Con_NPrintf(3, "Viewer area: (solid space)");
  } else {
    int area = host_state.worldbrush->leafs[leaf].area;

    if (r_ShowViewerArea.GetInt()) Con_NPrintf(3, "Viewer area: %d", area);

    g_nVisibleAreas = 0;
    Vector vecVisOrigin = (pVisData) ? (pVisData->m_vecVisOrigin)
                                     : (g_EngineRenderer->ViewOrigin());
    R_FlowThroughArea(area, vecVisOrigin, &rect, pVisData,
