...
inline void SCR_ShowVCRPlaybackAmount() {
  ...
  Con_NXPrintf(&info, "VCR Playback: %.2f percent, frame %d",
               flCurPercent * 100.0, host_framecount);
  info.index++;

  Con_NXPrintf(&info, "'+' to speed up, '-' to slow down [current sleep: %d]",
...
void SCR_UpdateScreen(void) {
  ...
  if (scr_nextdrawtick != 0) {
    if (host_tickcount < scr_nextdrawtick) return;
  ...
  // This needs to happen before the client DLL is called because the client DLL
  // depends on some of the setup in FRAME_RENDER_START.
