...
static ConVar r_drawstaticprops("r_drawstaticprops", "1", FCVAR_CHEAT,
...
//-----------------------------------------------------------------------------
// All static props have these bits set (to differentiate them from edict
// indices)
//-----------------------------------------------------------------------------
enum {
...
static const objectparams_t g_PhysDefaultObjectParams = {
    ...
    "DEFAULT",
    NULL,  // game data
    0.f,   // volume (leave 0 if you don't have one or call
           // physcollision->CollideVolume() to compute it)
    1.0f,  // drag coefficient
    true,  // enable collisions?
};

// return true if the renderer should use the slow path that supports the
// various debug modes
inline bool IsUsingStaticPropDebugModes() {
  if (r_drawstaticprops.GetInt() != 1 ||
      r_DrawSpecificStaticProp.GetInt() >= 0 || r_colorstaticprops.GetBool() ||
      r_staticpropinfo.GetInt() || mat_fullbright.GetInt() ||
      r_drawmodellightorigin.GetBool() || r_drawmodelstatsoverlay.GetBool())
    return true;
  return false;
}
...
// A static prop
//-----------------------------------------------------------------------------
class CStaticProp : public IClientUnknown,
 ...
 public:
  ...
  // Returns the BRUSH model index if this is a brush model. Otherwise, returns
  // -1.
  ...
  virtual void WorldSpaceSurroundingBounds(Vector *pVecWorldMins,
 ...
 public:
  ...
  void CreateVPhysics(IPhysicsEnvironment *physenv,
                      IVPhysicsKeyHandler *pDefaults, void *pGameData);
 ...
 private:
  ...
  // bbox is the same for both GetBounds and GetRenderBounds since static props
  // never move. GetRenderBounds is interpolated data, and GetBounds is last
  // networked.
...
//-----------------------------------------------------------------------------
class CStaticPropMgr : public IStaticPropMgrEngine,
                       public IStaticPropMgrClient,
                       public IStaticPropMgrServer {
 public:
  ...
  virtual bool IsPropInPVS(IHandleEntity *pHandleEntity,
                           const uint8_t *pVis) const;
  virtual ICollideable *GetStaticProp(IHandleEntity *pHandleEntity);
  virtual void RecomputeStaticLighting();
  virtual LightCacheHandle_t GetLightCacheHandleForStaticProp(
  ...
  virtual void TraceRayAgainstStaticProp(const Ray_t &ray, int staticPropIndex,
                                         trace_t &tr);
  virtual void AddDecalToStaticProp(Vector const &rayStart,
                                    Vector const &rayEnd, int staticPropIndex,
  ...
  // Changes made specifically to support the Portal mod (smack Dave Kircher if
  // something breaks)
  //===================================================================
  virtual void GetAllStaticProps(CUtlVector<ICollideable *> *pOutput);
  virtual void GetAllStaticPropsInAABB(const Vector &vMins, const Vector &vMaxs,
                                       CUtlVector<ICollideable *> *pOutput);
  virtual void GetAllStaticPropsInOBB(const Vector &ptOrigin,
  ...
  void DrawStaticProps(IClientRenderable **pProps, int count, bool bShadowDepth,
                       bool drawVCollideWireframe);
  void DrawStaticProps_Slow(IClientRenderable **pProps, int count,
                            bool bShadowDepth, bool drawVCollideWireframe);
  void DrawStaticProps_Fast(IClientRenderable **pProps, int count,
                            bool bShadowDepth);
  void DrawStaticProps_FastPipeline(IClientRenderable **pProps, int count,
                                    bool bShadowDepth);
 ...
 private:
  ...
  unsigned char ComputeScreenFade(CStaticProp &prop, float flMinSize,
                                  float flMaxSize, float flFalloffFactor);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CStaticPropMgr, IStaticPropMgrClient,
                                  INTERFACEVERSION_STATICPROPMGR_CLIENT,
                                  s_StaticPropMgr);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CStaticPropMgr, IStaticPropMgrServer,
...
bool CStaticProp::Init(int index, StaticPropLump_t &lump, model_t *pModel) {
  ...
  if (pStudioHdr) {
    if (!(pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP)) {
      ...
      if (nBitchCount < 100) {
        Warning(
            "model %s used as a static prop, but not compiled as a static "
            "prop\n",
            pStudioHdr->pszName());
        nBitchCount++;
      }
  ...
  switch (m_nSolidType) {
    ...
    default: {
      ...
      Q_strncpy(szModel,
                ...
                sizeof(szModel));
      Warning(
          "CStaticProp::Init:  Map error, static_prop with bogus SOLID_ flag "
          "(%d)! (%s)\n",
  ...
  m_Flags =
      (lump.m_Flags & (STATIC_PROP_SCREEN_SPACE_FADE | STATIC_PROP_FLAG_FADES |
  ...
  bNoDraw =
      bNoDraw || (lump.m_nMaxDXLevel && lump.m_nMaxDXLevel < nCurrentDXLevel);
  if (bNoDraw) {
  ...
  if (lump.m_Flags & STATIC_PROP_USE_LIGHTING_ORIGIN) {
    ...
    modelinfo->GetIlluminationPoint(m_pModel, this, m_Origin, m_Angles,
                                    &m_LightingOrigin);
  }
...
void CStaticProp::WorldSpaceTriggerBounds(Vector *pVecWorldMins,
                                          Vector *pVecWorldMaxs) const {
...
// Surrounding box
//-----------------------------------------------------------------------------
void CStaticProp::WorldSpaceSurroundingBounds(Vector *pVecWorldMins,
                                              Vector *pVecWorldMaxs) {
...
bool CStaticProp::ShouldDraw() { return (m_Flags & STATIC_PROP_NO_DRAW) == 0; }

//-----------------------------------------------------------------------------
// Render setup
//-----------------------------------------------------------------------------
bool CStaticProp::SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones,
                             int boneMask, float currentTime) {
...
void CStaticProp::SetupWeights(const matrix3x4_t *pBoneToWorld,
                               int nFlexWeightCount, float *pFlexWeights,
                               float *pFlexDelayedWeights) {}
...
// state info
//-----------------------------------------------------------------------------
inline void CStaticProp::SetAlpha(unsigned char alpha) { m_Alpha = alpha; }

void CStaticProp::ComputeFxBlend() {
  s_StaticPropMgr.ComputePropOpacity(*this);
}

int CStaticProp::GetFxBlend() { return m_Alpha; }
...
// custom collision test
//-----------------------------------------------------------------------------
bool CStaticProp::TestCollision(const Ray_t &ray, unsigned int fContentsMask,
                                trace_t &tr) {
  Assert(0);
  return false;
}
...
// Perform hitbox test, returns true *if hitboxes were tested at all*!!
//-----------------------------------------------------------------------------
bool CStaticProp::TestHitboxes(const Ray_t &ray, unsigned int fContentsMask,
                               trace_t &tr) {
  return false;
}
...
#ifndef SWDS
  if (m_ModelInstance == MODEL_INSTANCE_INVALID) {
    LightCacheHandle_t lightCacheHandle = CreateStaticLightingCache(
        m_LightingOrigin, m_WorldRenderBBoxMin, m_WorldRenderBBoxMax);
...
void CStaticProp::DisplayStaticPropInfo(int nInfoType) {
  ...
  switch (nInfoType) {
    ...
    case 2:
      Q_snprintf(buf, sizeof(buf), "%d",
                 (m_EntHandle.ToInt() & (~STATICPROP_EHANDLE_MASK)));
      break;
    ...
    case 4: {
      ...
      float flPixelWidth = pRenderContext->ComputePixelWidthOfSphere(
          GetRenderOrigin(), Radius());
...
int CStaticProp::DrawModelSlow(int flags) {
#ifndef SWDS
  VPROF_BUDGET("CStaticProp::DrawModel",
               VPROF_BUDGETGROUP_STATICPROP_RENDERING);
  ...
  if (r_DrawSpecificStaticProp.GetInt() >= 0) {
    if ((m_EntHandle.ToInt() & (~STATICPROP_EHANDLE_MASK)) !=
        r_DrawSpecificStaticProp.GetInt())
      return 0;
  }
#endif
  ...
  //	CDebugOverlay::AddBoxOverlay( vec3_origin, m_WorldRenderBBoxMin,
  // m_WorldRenderBBoxMax, vec3_angle, 255, 0, 0, 32, 0.01 );
  //	CDebugOverlay::AddBoxOverlay( GetRenderOrigin(), m_RenderBBoxMin,
  // m_RenderBBoxMax, GetRenderAngles(), 0, 255, 0, 32, 0.01 );
  ...
  if (m_pModel && (flags & STUDIO_WIREFRAME_VCOLLIDE)) {
    if (m_nSolidType == SOLID_VPHYSICS) {
      ...
      if (pCollide && pCollide->solidCount == 1) {
        ...
        DebugDrawPhysCollide(pCollide->solids[0], NULL, m_ModelToWorld,
                             debugColor, false);
      }
    } else if (m_nSolidType == SOLID_BBOX) {
      static Color debugColor(0, 255, 255, 255);
      RenderWireframeBox(m_Origin, vec3_angle, m_pModel->mins, m_pModel->maxs,
                         debugColor, true);
    }
  }
...
void CStaticProp::InsertPropIntoKDTree() {
  ...
  if (m_nSolidType == SOLID_NONE) return;
  ...
  m_Partition = SpatialPartition()->CreateHandle(
      this,
...
// Create VPhysics representation
//-----------------------------------------------------------------------------
void CStaticProp::CreateVPhysics(IPhysicsEnvironment *pPhysEnv,
                                 IVPhysicsKeyHandler *pDefaults,
                                 void *pGameData) {
  if (m_nSolidType == SOLID_NONE) return;
  ...
  pPhysEnv->CreatePolyObjectStatic(pPhysCollide, surfaceData, m_Origin,
                                   m_Angles, &solid.params);
    ...
    if (lump.m_Flags & STATIC_PROP_FLAG_FADES) {
      ...
      if (fade.m_MaxDistSq != fade.m_MinDistSq) {
        if (lump.m_Flags & STATIC_PROP_SCREEN_SPACE_FADE) {
          fade.m_FalloffFactor =
              255.0f / (fade.m_MaxScreenWidth - fade.m_MinScreenWidth);
        } else {
  ...
  for (i = 0; i < m_StaticProps.Count(); i++) {
    ...
    for (bodyPart = 0; bodyPart < pStudioHdr->numbodyparts; bodyPart++) {
      ...
      for (model = 0; model < pBodyPart->nummodels; model++) {
        mstudiomodel_t *part_model = pBodyPart->pModel(model);
        totalVerts += part_model->numvertices;
      }
    }
  }
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (prop.LeafCount() > 0) {
      ...
      clientleafsystem->AddRenderableToLeaves(
          handle, prop.LeafCount(),
          (unsigned short *)&m_StaticPropLeaves[prop.FirstLeaf()]);
    } else {
      ...
      DevMsg(1, "Static prop in 0 leaves! %s, @ %.1f, %.1f, %.1f\n",
             modelloader->GetName(prop.GetModel()), origin.x, origin.y,
             origin.z);
    }
  }
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::CreateVPhysicsRepresentations(
    IPhysicsEnvironment *pPhysEnv, IVPhysicsKeyHandler *pDefaults,
    void *pGameData) {
...
// Handles to props
//-----------------------------------------------------------------------------
inline int CStaticPropMgr::HandleEntityToIndex(
    IHandleEntity *pHandleEntity) const {
...
ICollideable *CStaticPropMgr::GetStaticProp(IHandleEntity *pHandleEntity) {
  ...
  int nIndex =
      pHandleEntity ? pHandleEntity->GetRefEHandle().GetEntryIndex() : -1;
...
void CStaticPropMgr::GetAllStaticPropsInAABB(
...
void CStaticPropMgr::GetAllStaticPropsInOBB(
  ...
  for (int counter = 0; counter != 8; ++counter) {
    ...
    // expand AABB extents
    if (ptOBBExtents[counter].x < vAABBMins.x)
      vAABBMins.x = ptOBBExtents[counter].x;
    if (ptOBBExtents[counter].x > vAABBMaxs.x)
      vAABBMaxs.x = ptOBBExtents[counter].x;
    if (ptOBBExtents[counter].y < vAABBMins.y)
      vAABBMins.y = ptOBBExtents[counter].y;
    if (ptOBBExtents[counter].y > vAABBMaxs.y)
      vAABBMaxs.y = ptOBBExtents[counter].y;
    if (ptOBBExtents[counter].z < vAABBMins.z)
      vAABBMins.z = ptOBBExtents[counter].z;
    if (ptOBBExtents[counter].z > vAABBMaxs.z)
      vAABBMaxs.z = ptOBBExtents[counter].z;
  }
  ...
  for (int counter = 0; counter != iPropVectorSize; ++counter) {
    CStaticProp *pProp = &m_StaticProps[counter];
    ...
    AngleMatrix(pProp->GetCollisionAngles(), pProp->GetCollisionOrigin(),
                matPropWorld);

    int counter2, counter3;
    // generate prop extents, TODO: update these to handle props with OBB's
    // since it should be nearly trivial
    for (counter2 = 0; counter2 != 8; ++counter2) {
      Vector ptTemp{
          (counter2 & (1 << 0)) ? (vPropOBBMaxs.x) : (vPropOBBMins.x),
          (counter2 & (1 << 1)) ? (vPropOBBMaxs.y) : (vPropOBBMins.y),
          (counter2 & (1 << 2)) ? (vPropOBBMaxs.z) : (vPropOBBMins.z)};
    ...
    for (counter2 = 0; counter2 != 6; ++counter2)  // loop over OBB planes
    {
      ...
      {
        if ((ptPropExtents[counter3].Dot(vOBBPlaneNormals[counter2]) -
             fOBBPlaneDists[counter2]) < 0.0f) {
          // an extent of the prop is within the OBB halfspace, this halfspace
          // does not eliminate our prop, move to the next halfspace
          break;
        }
      }
      if (counter3 == 8)
        break;  // if all 8 extents lie outside the halfspace, then the prop is
                // not in the OBB
    }
    if (counter2 == 6) {
...
bool CStaticPropMgr::IsStaticProp(IHandleEntity *pHandleEntity) const {
  return (!pHandleEntity) ||
         ((pHandleEntity->GetRefEHandle().GetSerialNumber() ==
           (STATICPROP_EHANDLE_MASK >> NUM_ENT_ENTRY_BITS)) != 0);
}

bool CStaticPropMgr::IsStaticProp(CBaseHandle handle) const {
  return (handle.GetSerialNumber() ==
          (STATICPROP_EHANDLE_MASK >> NUM_ENT_ENTRY_BITS));
}
...
bool CStaticPropMgr::PropHasBakedLightingDisabled(
    IHandleEntity *pHandleEntity) const {
  // Strip off the bits
...
void CStaticPropMgr::PrecacheLighting() {
  ...
  while (--i >= 0) {
    MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
    if (!m_StaticProps[i].ShouldDraw()) continue;
...
//-----------------------------------------------------------------------------
bool CStaticPropMgr::IsPropInPVS(IHandleEntity *pHandleEntity,
                                 const uint8_t *pVis) const {
  // Strip off the bits
  int nIndex = HandleEntityToIndex(pHandleEntity);
...
void CStaticPropMgr::DrawStaticProps_Slow(IClientRenderable **pProps, int count,
                                          bool bShadowDepth,
                                          bool drawVCollideWireframe) {
  // slow mode
  MDLCACHE_CRITICAL_SECTION_(g_pMDLCache);
  ...
  if (drawVCollideWireframe) flags |= STUDIO_WIREFRAME_VCOLLIDE;
...
void CStaticPropMgr::DrawStaticProps_Fast(IClientRenderable **pProps, int count,
                                          bool bShadowDepth) {
  float color[3];
  color[0] = color[1] = color[2] = 1.0f;
...
// NOTE: This is a work in progress for a new static prop (eventually new model)
// rendering pipeline
void CStaticPropMgr::DrawStaticProps_FastPipeline(IClientRenderable **pProps,
                                                  int count,
                                                  bool bShadowDepth) {
  ...
  if (count > MAX_OBJECTS) {
    DrawStaticProps_FastPipeline(pProps + MAX_OBJECTS, count - MAX_OBJECTS,
                                 bShadowDepth);
  }
...
// NOTE: Set this to zero to revert to the previous static prop lighting
// behavior
ConVar pipeline_static_props("pipeline_static_props", "1");
void CStaticPropMgr::DrawStaticProps(IClientRenderable **pProps, int count,
                                     bool bShadowDepth,
                                     bool drawVCollideWireframe) {
  VPROF_BUDGET("CStaticPropMgr::DrawStaticProps",
               VPROF_BUDGETGROUP_STATICPROP_RENDERING);
...
//-----------------------------------------------------------------------------
LightCacheHandle_t CStaticPropMgr::GetLightCacheHandleForStaticProp(
    IHandleEntity *pHandleEntity) {
  int nIndex = HandleEntityToIndex(pHandleEntity);
  return modelrender->GetStaticLighting(
      m_StaticProps[nIndex].GetModelInstance());
}

//-----------------------------------------------------------------------------
// Computes fade from screen-space fading
//-----------------------------------------------------------------------------
unsigned char CStaticPropMgr::ComputeScreenFade(CStaticProp &prop,
                                                float flMinSize,
                                                float flMaxSize,
  ...
  float flPixelWidth = pRenderContext->ComputePixelWidthOfSphere(
      prop.GetRenderOrigin(), prop.Radius());
...
void CStaticPropMgr::ChangeRenderGroup(CStaticProp &prop) {
#ifndef SWDS
  static RenderGroup_t opaqueRenderGroup = (g_bClientLeafSystemV1)
  ...
  if (prop.GetFxBlend() == 0) {
    clientleafsystem->ChangeRenderableRenderGroup(renderHandle,
                                                  opaqueRenderGroup);
  } else if (prop.GetFxBlend() == 255) {
    RenderGroup_t nRenderGroup = prop.IsTransparent()
                                     ? RENDER_GROUP_TRANSLUCENT_ENTITY
                                     : opaqueRenderGroup;
    clientleafsystem->ChangeRenderableRenderGroup(renderHandle, nRenderGroup);
  } else {
    clientleafsystem->ChangeRenderableRenderGroup(
        renderHandle, RENDER_GROUP_TRANSLUCENT_ENTITY);
  }
#endif
}
...
void CStaticPropMgr::ComputePropOpacity(CStaticProp &prop) {
  ...
  if (modelinfoclient->ModelHasMaterialProxy(prop.GetModel())) {
    modelinfoclient->RecomputeTranslucency(prop.GetModel(), prop.GetSkin(),
                                           prop.GetBody(),
                                           prop.GetClientRenderable());
  }
#endif
  ...
  if ((prop.Flags() & STATIC_PROP_FLAG_FADES) != 0) {
    ...
    // Step over the list of all things that want to be faded out and recompute
    // alpha
    ...
    if ((prop.Flags() & STATIC_PROP_SCREEN_SPACE_FADE) == 0) {
      ...
      alpha = ComputeScreenFade(prop, fade.m_MinScreenWidth,
                                fade.m_MaxScreenWidth, fade.m_FalloffFactor);
    }
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::TraceRayAgainstStaticProp(const Ray_t &ray,
                                               int staticPropIndex,
                                               trace_t &tr) {
#ifndef SWDS
  // Get the prop
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::AddDecalToStaticProp(Vector const &rayStart,
                                          Vector const &rayEnd,
                                          int staticPropIndex, int decalIndex,
  ...
  if (doTrace && (prop.GetSolid() == SOLID_VPHYSICS) && !tr.startsolid &&
      !tr.allsolid) {
    Vector temp;
...
//-----------------------------------------------------------------------------
void CStaticPropMgr::AddShadowToStaticProp(unsigned short shadowHandle,
                                           IClientRenderable *pRenderable) {
#ifndef SWDS
...
void CStaticPropMgr::RemoveAllShadowsFromStaticProp(
    IClientRenderable *pRenderable) {
#ifndef SWDS
  Assert(dynamic_cast<CStaticProp *>(pRenderable) != 0);
  CStaticProp *pProp = static_cast<CStaticProp *>(pRenderable);
...
void CStaticPropMgr::GetStaticPropMaterialColorAndLighting(trace_t *pTrace,
  ...
  modelinfoclient->GetModelMaterialColorAndLighting(
      (model_t *)prop.GetModel(), prop.GetRenderOrigin(),
      prop.GetRenderAngles(), pTrace, lighting, matColor);
#endif
}
...
static ConCommand prop_crosshair("prop_crosshair", Cmd_PropCrosshair_f,
                                 "Shows name for prop looking at", FCVAR_CHEAT);
