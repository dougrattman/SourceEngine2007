...
void IncrementalUpdateTree_R(vgui::TreeView *pTree, int iCurTreeNode,
                             KeyValues *pValues, bool &bChanges,
    ...
    if (pSub) {
      ...
      if (pSubText) {
        if (iCurChild < nChildren) {
          ...
          if (fn(pTree, iChildItemId, pSub)) bChanges = true;
      ...
      // This means that the tree view has extra ones at the end. Get rid of
      // them.
      int iChildItemId = pTree->GetChild(iCurTreeNode, iCurChild);
      --nChildren;
      bChanges = true;

      // HACK: I put a hack in there so if you give a negative number for the
      // item, it'll delete the panels immediately. This gets around a bug in
      // the TreeView where the panels don't always get deleted when using
...
bool IncrementalUpdateTree(vgui::TreeView *pTree, KeyValues *pValues,
                           UpdateItemStateFn fn, int iRoot) {
  if (iRoot == -1) {
