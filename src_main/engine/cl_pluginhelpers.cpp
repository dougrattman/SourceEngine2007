...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CPluginMenu::CPluginMenu(vgui::Panel *parent)
    : EditablePanel(parent, "PluginMenu") {
...
void CPluginGameUIDialog::Show(DIALOG_TYPE type, KeyValues *kv) {
  ...
  switch (type) {
    ...
    case DIALOG_ENTRY:
      ...
      Q_strncpy(m_szEntryCommand, kv->GetString("command"),
                sizeof(m_szEntryCommand));
...
class CPluginHudMessage : public vgui::Frame {
 ...
 public:
  ...
  void ShowMessage(const wchar_t *message, int time, Color clr,
                   bool bHasExtraPanel);
  void StartHiding();
  void Hide();
...
CPluginHudMessage::CPluginHudMessage(vgui::VPANEL parent)
  ...
  m_pAnimationController->SetScriptFile(parent,
...
void CPluginHudMessage::ApplySchemeSettings(vgui::IScheme *pScheme) {
  m_pExtraPanelIcon->SetImage(
      vgui::scheme()->GetImage("plugin/message_waiting", true));
  BaseClass::ApplySchemeSettings(pScheme);
  SetBgColor(pScheme->GetColor(
      "Plugins.BgColor",
      pScheme->GetColor("TransparentBlack", Color(0, 0, 0, 192))));
...
//-----------------------------------------------------------------------------
// Purpose: run the anim controller and hide the message label if the anim var
// says to
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: start the shrinking anim for the control if it should be showed, the
// hide anim otherwsise
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CPluginHudMessage::ShowMessage(const wchar_t *text, int time, Color clr,
                                    bool bHasExtraPanel) {
  m_Message->SetVisible(true);
  m_Message->SetBounds(MESSAGE_X_INSET, 5, m_iTargetW - MESSAGE_X_INSET - 10,
                       m_iTargetH - 10);
...
CPluginUIManager::CPluginUIManager() : vgui::Panel(NULL, "PluginManager") {
  ...
  m_pHudMessage =
      new CPluginHudMessage(EngineVGui()->GetPanel(PANEL_CLIENTDLL));
...
void CPluginUIManager::Show(DIALOG_TYPE type, KeyValues *kv) {
  ...
  m_pHudMessage->ShowMessage(kv->GetWString("title"), 10, clr,
                             type != DIALOG_MSG);
}
...
void CPluginUIManager::GetHudMessagePosition(int &x, int &y, int &wide,
                                             int &tall) {
  if (m_pHudMessage) {
...
//
//=============================================================================
ConVar cl_showpluginmessages("cl_showpluginmessages", "1", FCVAR_ARCHIVE,
