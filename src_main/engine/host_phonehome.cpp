...
class CPhoneHome : public IPhoneHome {
 public:
  CPhoneHome() : m_bPhoneHome(false), m_uSessionID(0), m_pSocket(0) {
  ...
  virtual void Init() {
    ...
    Q_strncpy(build_identifier, "VLV_INTERNAL                    ",
              sizeof(build_identifier));
    size_t iBI = CommandLine()->FindParm("-bi");
    if (iBI > 0) {
    ...
    if (Q_strlen(build_identifier) >= 1 &&
        Q_strnicmp(build_identifier, "VLV_INTERNAL",
                   Q_strlen("VLV_INTERNAL"))) {
      ...
      if (NET_StringToAdr("207.173.177.12:27013", &m_cserIP)) {
        ...
        Q_strncpy(m_szBuildIdentifier, build_identifier,
                  sizeof(m_szBuildIdentifier));
  ...
  virtual void Message(uint8_t msgtype, char const *mapname) {
    if (!m_bPhoneHome || !m_pSocket) return;
 ...
 private:
  ...
  inline void Encrypt8ByteSequence(IceKey &cipher,
  ...
  void BuildMessage(bf_write &buf, uint8_t msgtype, char const *mapname,
    ...
    // Write the "build identifier"  -- unique to each person we give a build
    // to.
  ...
  void SendSessionMessage(uint8_t msgtype, char const *mapname) {
    ...
    m_pSocket->SendSocketMessage(sa, (const uint8_t *)buf.GetData(),
                                 buf.GetNumBytesWritten());

    // If we already have a sessionid, don't wait for the server to give us back
    // a new one...
    if (m_uSessionID != 0) {
      return;
    }
    ...
    if (m_pSocket->WaitForMessage(PHONE_HOME_TIMEOUT)) {
      ...
      uint32_t bytesReceived = m_pSocket->ReceiveSocketMessage(
          &replyaddress, (uint8_t *)readbuf, sizeof(readbuf));
      if (bytesReceived > 0) {
        // Fixup actual size
        replybuf.StartReading(readbuf, bytesReceived);

        // Parse out data
        uint8_t responseType = (uint8_t)replybuf.ReadByte();
  ...
  bool RequestSessionId(unsigned int &id) {
    ...
    for (int retries = 0; retries < PHONE_HOME_RETRIES; ++retries) {
      m_pSocket->SendSocketMessage(sa, (const uint8_t *)buf.GetData(),
                                   buf.GetNumBytesWritten());  // lint !e534
      if (m_pSocket->WaitForMessage(PHONE_HOME_TIMEOUT)) {
        uint8_t readbuf[128];
