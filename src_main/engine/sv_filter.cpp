...
//-----------------------------------------------------------------------------
// Purpose: Sends a message to prospective clients letting them know they're
// banned Input  : *adr -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Takes an IP address string and fills in an ipfilter_t mask and
// compare (raw address) Input  : *s -
//			*f -
...
static void Filter_Add_f(const CCommand &args) {
  ...
  ipfilter_t f;
  if (!Filter_ConvertString(args[2], &f)) return;
  ...
  // Time to unban.
  g_IPFilters[i].banEndTime =
      (banTime != 0.0) ? (realtime + 60.0 * banTime) : 0.0;
  ...
  if (bKick) {
    ...
    for (i = 0; i < sv.GetClientCount(); ++i) {
      client = sv.Client(i);
      if (!client || !client->IsActive() || !client->IsConnected() ||
          !client->IsSpawned())
        continue;
  ...
  if (event) {
    ...
    event->SetString(
        "by", (cmd_source == src_command) ? "Console" : host_client->m_Name);
...
static ConCommand addip("addip", Filter_Add_f,
                        "Add an IP address to the ban list.");
static ConCommand banip("banip", Filter_Add_f,
                        "Add an IP address to the ban list.");
...
CON_COMMAND(removeip, "Remove an IP address from the ban list.") {
    ...
    if (slot > 0 && slot <= g_IPFilters.Count()) {
      ...
      *(unsigned *)b = g_IPFilters[slot].compare;
      ...
      if (event) {
        ...
        event->SetString("by", (cmd_source == src_command)
                                   ? "Console"
                                   : host_client->m_Name);
  ...
  if (!Filter_ConvertString(args[1], &f)) return;
...
#ifndef _XBOX
  if (sv_filterban.GetInt() == 0) return false;
...
CON_COMMAND(
    writeid,
    "Writes a list of permanently-banned user IDs to " BANNED_USER_FILENAME
    ".") {
  FileHandle_t f;
  char name[MAX_OSPATH];
  int i;
...
CON_COMMAND(removeid, "Remove a user ID from the ban list.") {
  ...
  if (*pszArg1 < '0' || *pszArg1 > '9') {
    ...
    if (!Q_strnicmp(pszArg1, STEAM_PREFIX, Q_strlen(STEAM_PREFIX)) &&
        Q_strstr(args[2], ":")) {
...
CON_COMMAND(banid, "Add a user ID to the ban list.") {
  ...
  bKick =
      (args.ArgC() >= 4 && Q_strcasecmp(args[args.ArgC() - 1], "kick") == 0);

  // if the first letter is a charcter then
  // we're searching for a uniqueid ( e.g. STEAM_ )
  if (*pszArg2 < '0' || *pszArg2 > '9') {
    // SteamID (need to reassemble it)
    if (!Q_strnicmp(pszArg2, STEAM_PREFIX, strlen(STEAM_PREFIX)) &&
        Q_strstr(args[3], ":")) {
      Q_snprintf(szSearchString, sizeof(szSearchString), "%s:%s:%s", pszArg2,
                 args[4], args[6]);
    }
    // some other ID (e.g. "UNKNOWN", "STEAM_ID_PENDING", "STEAM_ID_LAN")
    // NOTE: assumed to be one argument
  ...
  for (i = 0; i < g_UserFilters.Count(); i++) {
    ...
    if (Steam3Server().CompareUserID(g_UserFilters[i].userid, *id)) break;
  }
