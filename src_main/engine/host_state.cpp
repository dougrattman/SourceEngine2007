...
static bool Host_ValidGame() {
  ...
  if (sv.IsMultiplayer()) {
    if (deathmatch.GetInt()) return true;
  } else {
    return true;
  }
...
static bool IsClientActive() {
  if (!sv.IsActive()) return cl.IsActive();

  for (int i = 0; i < sv.GetClientCount(); i++) {
    CGameClient *pClient = sv.Client(i);
    if (pClient->IsActive()) return true;
  }
...
static bool IsClientConnected() {
  if (!sv.IsActive()) return cl.IsConnected();
...
class CHostState {
 public:
  ...
  void FrameUpdate(float the_time) {
#if _DEBUG
    int loopCount = 0;
#endif
    ...
    while (true) {
      ...
      // execute the current state (and transition to the next state if not in
      // HS_RUN)
      ...
      // only do a single pass at HS_RUN per frame.  All other states loop until
      // they reach HS_RUN
      if (oldState == HS_RUN) break;

      // shutting down
      if (oldState == HS_SHUTDOWN || oldState == HS_RESTART) break;

        // Only HS_RUN is allowed to persist across loops!!!
        // Also, detect circular state graphs (more than 8 cycling changes is
        // probably a loop) NOTE: In the current graph, there are at most 2.
#if _DEBUG
  ...
  void SetNextState(HOSTSTATES next) {
  ...
  // The external API queues up state changes to happen when the state machine
  // is processed.
  void State_NewGame() {
    if (Host_ValidGame()) {
  ...
  void State_Run(float frameTime) {
    if (m_flShortFrameTime > 0) {
      if (IsClientActive()) {
        m_flShortFrameTime = (m_flShortFrameTime > frameTime)
                                 ? (m_flShortFrameTime - frameTime)
                                 : 0;
      }
      // Only clamp time if client is in process of connecting or is already
      // connected.
    ...
    switch (m_nextState) {
      ...
      case HS_RESTART:
        ...
        // This is done here instead of pathfinding through a state transition
        // graph. That would be equivalent as the only way to get from HS_RUN to
        // HS_LOAD_GAME is through HS_GAME_SHUTDOWN.
      case HS_GAME_SHUTDOWN:
  ...
  void RememberLocation() {
    ...
    IClientEntity *localPlayer =
        entitylist ? entitylist->GetClientEntity(cl.m_nPlayerSlot + 1) : NULL;
    if (localPlayer) {
      m_vecLocation = localPlayer->GetAbsOrigin();
    }

    m_vecLocation.z -=
        64.0f;  // subtract out a bit of Z to position the player lower
#endif
  }
  ...
  char m_saveName[256];
  float m_flShortFrameTime;  // run a few one-tick frames to avoid large
                             // timesteps while loading assets
...
void HostState_Init() { g_HostState.Init(); }

void HostState_Frame(float the_time) { g_HostState.FrameUpdate(the_time); }
...
void HostState_LoadGame(char const *pSaveFileName, bool remember_location) {
  ...
  Q_strncpy(g_HostState.m_saveName, pSaveFileName,
            sizeof(g_HostState.m_saveName));
...
void HostState_ChangeLevelSP(char const *pNewLevel, char const *pLandmarkName) {
  Q_strncpy(g_HostState.m_levelName, pNewLevel,
            sizeof(g_HostState.m_levelName));
  Q_strncpy(g_HostState.m_landmarkName, pLandmarkName,
...
void HostState_Shutdown() { g_HostState.SetNextState(HS_SHUTDOWN); }

// Purpose: Restart the engine

void HostState_Restart() { g_HostState.SetNextState(HS_RESTART); }

bool HostState_IsGameShuttingDown() { return g_HostState.IsGameShuttingDown(); }

void HostState_OnClientConnected() { g_HostState.OnClientConnected(); }

void HostState_OnClientDisconnected() { g_HostState.OnClientDisconnected(); }
