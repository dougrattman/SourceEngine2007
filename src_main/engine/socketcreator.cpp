...
bool CSocketCreator::CreateListenSocket(const netadr_t &netAdr) {
  ...
  int ret =
      bind(m_hListenSocket, (struct sockaddr *)&s, sizeof(struct sockaddr_in));
  if (ret == -1) {
...
int CSocketCreator::ConnectSocket(const netadr_t &netAdr, bool bSingleSocket) {
  ...
  if (ret == -1) {
    ...
    if (select(1, NULL, &writefds, NULL, &tv) <
        1)  // block for at most 1 second
    {
...
void CSocketCreator::CloseAcceptedSocket(int nIndex) {
  ...
  if (m_pListener) {
    m_pListener->OnSocketClosed(connected.m_hSocket, connected.m_Address,
                                connected.m_pData);
  }
