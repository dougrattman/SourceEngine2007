...
static ConVar sv_useexplicitdelete("sv_useexplicitdelete", "1",
                                   FCVAR_DEVELOPMENTONLY,
                                   "Explicitly delete dormant client entities "
                                   "caused by AllowImmediateReuse().");
...
edict_t *ED_Alloc(int iForceEdictIndex) {
  if (iForceEdictIndex >= 0) {
    if (iForceEdictIndex >= sv.num_edicts) {
      Warning("ED_Alloc( %d ) - invalid edict index specified.",
              iForceEdictIndex);
      return NULL;
    }
  ...
  for (int i = sv.GetMaxClients() + 1; i < sv.num_edicts; i++) {
    if (pEdict->IsFree() &&
        (pEdict->freetime < 2 ||
         sv.GetTime() - pEdict->freetime >= EDICT_FREETIME)) {
  ...
  if (sv.num_edicts >= sv.max_edicts) {
    ...
    if (sv.max_edicts == 0) Sys_Error("ED_Alloc: No edicts yet");
    Sys_Error("ED_Alloc: no free edicts");
  }

  // Do this before clearing since clear now needs to call back into the edict
  // to deduce the index so can get the changeinfo data in the parallel
  // structure
  sv.num_edicts++;
...
//);
  ...
  if ((ed - sv.edicts) >= 1 && (ed - sv.edicts) <= sv.GetMaxClients()) return;
  ...
  // Increment the serial number so it knows to send explicit deletes the
  // clients.
  ed->m_NetworkSerialNumber++;
}

//
// 	serverGameEnts->FreeContainingEntity( pEdict )  frees up memory
// associated with a DLL entity. InitializeEntityDLLFields clears out fields to
// NULL or UNKNOWN. Release is for terminating a DLL entity.  Initialize is for
// initializing one.
//
...
edict_t *EDICT_NUM(int n) {
  if (n < 0 || n >= sv.max_edicts) Sys_Error("EDICT_NUM: bad number %i", n);
  return &sv.edicts[n];
}
...
int NUM_FOR_EDICTINFO(const edict_t *e) {
  ...
  if (b < 0 || b >= sv.max_edicts) Sys_Error("NUM_FOR_EDICTINFO: bad pointer");
  return b;
}
