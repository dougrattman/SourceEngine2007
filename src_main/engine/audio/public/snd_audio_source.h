...
the_interface CAudioMixer {
 public:
  ...
  virtual int MixDataToDevice(IAudioDevice * pDevice, channel_t * pChannel,
                              ...
                              int outputOffset) = 0;
  virtual int SkipSamples(channel_t * pChannel, int sampleCount, int outputRate,
                          int outputOffset) = 0;
  ...
  // NOTE: Playback is optimized for linear streaming.  These calls will usually
  // cost performance It is currently optimal to call them before any playback
  // starts, but some audio sources may not guarantee this.  Also, some mixers
  // may choose to ignore these calls for internal reasons (none do currently).
  ...
  // How many samples to skip before commencing actual data reading ( to allow
  // sub-frametime sound
  //  offsets and avoid synchronizing sounds to various 100 msec clock intervals
  //  throughout the engine and game code)
  virtual void SetStartupDelaySamples(int delaySamples) = 0;
  virtual int GetMixSampleSize() = 0;

  // Certain async loaded sounds lazilly load into memory in the background, use
  // this to determine
...
class CAudioSourceCachedInfo : public IBaseCacheInfo {
 public:
  ...
  inline int Type() const { return info.m_Type; }
  void SetType(int type) { info.m_Type = type; }

  inline int Bits() const { return info.m_bits; }
  void SetBits(int bits) { info.m_bits = bits; }

  inline int Channels() const { return info.m_channels; }
  void SetChannels(int channels) { info.m_channels = channels; }

  inline int SampleSize() const { return info.m_sampleSize; }
  void SetSampleSize(int size) { info.m_sampleSize = size; }

  inline int Format() const { return info.m_format; }
  void SetFormat(int format) { info.m_format = format; }

  inline int SampleRate() const { return info.m_rate; }
  void SetSampleRate(int rate) { info.m_rate = rate; }
  ...
  void SetCachedData(const uint8_t *data) {
    m_pCachedData = (uint8_t *)data;
  ...
  void SetHeaderData(const uint8_t *data) {
    m_pHeader = (uint8_t *)data;
    flags.m_bHeader = (data != NULL) ? true : false;
  }

  inline int LoopStart() const { return m_loopStart; }
  void SetLoopStart(int start) { m_loopStart = start; }

  inline int SampleCount() const { return m_sampleCount; }

  void SetSampleCount(int count) { m_sampleCount = count; }
  inline int DataStart() const { return m_dataStart; }
  void SetDataStart(int start) { m_dataStart = start; }
  inline int DataSize() const { return m_dataSize; }
  void SetDataSize(int size) { m_dataSize = size; }
  inline CSentence *Sentence() const { return m_pSentence; }
...
class IAudioSourceCache {
 public:
  ...
  // that next time it's accessed. Used when you connect to a pure server.
...
enum { INVALID_BUFFER_HANDLE = (BufferHandle_t)~0 };

typedef unsigned int streamFlags_t;
enum {
  STREAMED_FROMDVD = 0x00000001,  // stream buffers are compliant to dvd sectors
  STREAMED_SINGLEPLAY = 0x00000002,  // non recurring data, buffers don't need
                                     // to persist and can be recycled
  STREAMED_QUEUEDLOAD =
...
struct CAudioSourceCachedInfoHandle_t {
  CAudioSourceCachedInfoHandle_t() : info(NULL), m_FlushCount(0) {}

  CAudioSourceCachedInfo *info;
  unsigned int m_FlushCount;

  inline CAudioSourceCachedInfo *Get(int audiosourcetype, bool soundisprecached,
                                     CSfxTable *sfx, int *pcacheddatasize) {
  ...
  inline bool IsValid() { return !!(m_FlushCount == s_nCurrentFlushCount); }
...
the_interface CAudioSource {
 public:
  ...
  // Provide samples for the mixer. You can point pData at your own data, or if
  ...
  // This is a best estimate. The compressed audio mixers understand this, but
  // in general do not assume that SampleSize() * SampleCount() = filesize or
  // even that SampleSize() is 100% accurate due to compression.
