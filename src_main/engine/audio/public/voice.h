...
#define VOICE_OUTPUT_SAMPLE_RATE \
  11025  // Sample rate that we feed to the mixer.

//! Returned on error from certain voice functions.
#define VOICE_CHANNEL_ERROR -1
#define VOICE_CHANNEL_IN_TWEAK_MODE \
...
//! This is called periodically by the engine when the server acks the local
//! player talking. This tells the client DLL that the local player is talking
//! and fades after about 200ms.
...
//! Get the most recent N bytes of compressed data. If nCount is less than the
...
//! \return A channel index for use with Voice_AddIncomingData or
//! VOICE_CHANNEL_ERROR on error.
int Voice_AssignChannel(int nEntity, bool bProximity);

//! Call this to get the channel index that the specified entity is talking
//! into. \return A channel index for use with Voice_AddIncomingData or
//! VOICE_CHANNEL_ERROR if the entity isn't talking.
