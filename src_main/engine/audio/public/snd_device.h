...
#define SOUND_MIX_WET \
  0  // mix only samples that don't have channel set to 'dry' or 'speaker'
     // (default)
#define SOUND_MIX_DRY \
  1  // mix only samples with channel set to 'dry' (ie: music)
#define SOUND_MIX_SPEAKER 2  // mix only samples with channel set to 'speaker'

#define SOUND_BUSS_ROOM \
  (1 << 0)  // mix samples using channel dspmix value (based on distance from
            // player)
#define SOUND_BUSS_FACING \
  (1 << 1)  // mix samples using channel dspface value (source facing)
#define SOUND_BUSS_FACINGAWAY (1 << 2)  // mix samples using 1-dspface
#define SOUND_BUSS_SPEAKER \
  (1 << 3)  // mix ch->bspeaker samples in mono to speaker buffer
#define SOUND_BUSS_DRY (1 << 4)  // mix ch->bdry samples into dry buffer

class Vector;
struct channel_t;

// UNDONE: Create a simulated audio device to replace the old -simsound
// functionality?
...
abstract_class IAudioDevice {
 public:
  ...
  // Audio_GetNullDevice which will create a "0" device that makes no sound. If
  // we can't create a real sound device, this will return a device of that
  // type. All of the interface functions can be called on the 0 device, but it
  // will not, of course, make sound.
  static IAudioDevice *AutoDetectInit(bool waveOnly);

  virtual ~IAudioDevice() = 0;

  // This is needed by some of the routines to avoid doing work when you've got
  // a 0 device
  ...
  // This should return 0 on all implementations that don't need a separate dry
  // mix
  virtual float MixDryVolume() = 0;
  // Should we mix sounds to a 3D (quadraphonic) sound buffer (front/rear both
  // stereo)
  virtual bool Should3DMix() = 0;

  // This is called when the application stops all sounds
  // NOTE: Stopping each channel and clearing the sound buffer are done
  // separately
  virtual void StopAllSounds() = 0;

  // Called before painting channels, must calculated the endtime and return it
  // (once per frame)
  ...
  virtual void SpatializeChannel(int volume[6], int master_vol,
  ...
  virtual int GetOutputPosition() = 0;
  ...
  // Called each time a new paint buffer is mixed (may be multiple times per
  // frame)
  ...
  // Reset a channel
  virtual void ChannelReset(int entnum, int channelIndex,
                            float distanceMod) = 0;
  ...
  virtual int DeviceDmaSpeed() = 0;     // Actual DMA speed
  virtual int DeviceSampleCount() = 0;  // Total samples in buffer

  virtual bool IsSurround() = 0;  // surround enabled, could be quad or 5.1
  virtual bool IsSurroundCenter() = 0;  // surround enabled as 5.1
  virtual bool IsHeadphone() = 0;
};

extern IAudioDevice *Audio_GetNullDevice();
