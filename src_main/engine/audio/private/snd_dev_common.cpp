...
//    this is used for the 2 speaker outpu case, which uses recombined 4 channel
//    front/rear mixing

static float XfadeSpeakerVolToMono(float scale, float xfade, float ispeaker,
                                   float cspeaker, bool fmix2channels) {
  float scale_out;
  float scale_target;
...
static float GetSpeakerVol(float yaw_source, float pitch_source, float mono,
  ...
  // mono goes from 0.0 to 1.0 as listener moves into 'mono' radius of sound
  // source. Also, as pitch_angle to sound source approaches 90 (sound
  // above/below listener), sounds become mono.
  ...
  if (pitch_angle > PITCH_ANGLE_THRESHOLD) {
    xfade = (pitch_angle - PITCH_ANGLE_THRESHOLD) /
  ...
  if (cspeaker == 4) {
    ...
    // scale ranges from 0.0 (at 90 degree difference between source and
    // speaker) to 1.0 (0 degree difference between source and speaker)
  ...
  // rear speakers are on 90 degree angles and return 0.0->1.0 range over +/- 90
  // degrees each center speaker is on 45 degree angle to left/right front
  // speaker center speaker has 0.0->1.0 range over 45 degrees
  ...
  switch (ispeaker) {
    ...
    case ISPEAKER_RIGHT_FRONT: {
      ...
      /*
                              if (yaw_source > yaw_speaker && yaw_source <
         (yaw_speaker + 90.0))
                              {
                                      // if sound source is between right front
         speaker and center speaker,
                                      // apply scaling over 45 degrees...
                                      if (adif > 45.0)
                                              goto GetVolExit;	// 0.0 scale

                                              scale = (1.0 - powf(adif/45.0,
         VOLCURVEPOWER));
                              }
      */
      else {
        // sound source is CW from right speaker, apply scaling over 90
        // degrees...
        scale = (1.0 - powf(adif / 90.0, VOLCURVEPOWER));
      }
    ...
    case ISPEAKER_LEFT_FRONT: {
      ...
      /*
         ...
         (yaw_speaker - 90.0))
                              {
                                      // if sound source is between left front
         speaker and center speaker,
                                      // apply scaling over 45 degrees...
                                      if (adif > 45.0)
                                              goto GetVolExit;	// 0.0 scale
...
GetVolExit:
  ...
  // crossfade speaker volumes towards mono with increased pitch angle of sound
  // source
...
void CAudioDeviceBase::SpatializeChannel(int volume[CCHANVOLUMES / 2],
                                         int master_vol,
  ...
  // linear crossfader for 2, 4 or 5 speakers, using polar coord. separation
  // angle as linear basis
  ...
  if (!m_bSurround) {
    ...
    if (m_bHeadphone) {
      ...
      rfscale =
          GetSpeakerVol(yaw, pitch, mono, 0.0, ISPEAKER_RIGHT_FRONT, 2, false);
      lfscale =
          GetSpeakerVol(yaw, pitch, mono, 180.0, ISPEAKER_LEFT_FRONT, 2, false);
    } else {
      // stereo speakers at 45 & 135 degrees: (mono sounds mix down to 2
      // channels)

      rfscale =
          GetSpeakerVol(yaw, pitch, mono, 45.0, ISPEAKER_RIGHT_FRONT, 4, true);
      lfscale =
          GetSpeakerVol(yaw, pitch, mono, 135.0, ISPEAKER_LEFT_FRONT, 4, true);
      rrscale =
          GetSpeakerVol(yaw, pitch, mono, 315.0, ISPEAKER_RIGHT_REAR, 4, true);
      lrscale =
          GetSpeakerVol(yaw, pitch, mono, 225.0, ISPEAKER_LEFT_REAR, 4, true);
      ...
      // DevMsg("lfscale=%f rfscale=%f lrscale=%f
      // rrscale=%f\n",lfscale,rfscale,lrscale,rrscale);  DevMsg("pitch=%f
      // yaw=%f \n",pitch, yaw);
    }
    goto SpatialExit;
  }
  ...
  if (m_bSurround && !m_bSurroundCenter) {
    ...
    rfscale =
        GetSpeakerVol(yaw, pitch, mono, 45.0, ISPEAKER_RIGHT_FRONT, 4, false);
    lfscale =
        GetSpeakerVol(yaw, pitch, mono, 135.0, ISPEAKER_LEFT_FRONT, 4, false);
    rrscale =
        GetSpeakerVol(yaw, pitch, mono, 315.0, ISPEAKER_RIGHT_REAR, 4, false);
    lrscale =
        GetSpeakerVol(yaw, pitch, mono, 225.0, ISPEAKER_LEFT_REAR, 4, false);
  ...
  if (m_bSurround && m_bSurroundCenter) {
    ...
    rfscale =
        GetSpeakerVol(yaw, pitch, mono, 45.0, ISPEAKER_RIGHT_FRONT, 5, false);
    fcscale =
        GetSpeakerVol(yaw, pitch, mono, 90.0, ISPEAKER_CENTER_FRONT, 5, false);
    lfscale =
        GetSpeakerVol(yaw, pitch, mono, 135.0, ISPEAKER_LEFT_FRONT, 5, false);
    rrscale =
        GetSpeakerVol(yaw, pitch, mono, 315.0, ISPEAKER_RIGHT_REAR, 5, false);
    lrscale =
        GetSpeakerVol(yaw, pitch, mono, 225.0, ISPEAKER_LEFT_REAR, 5, false);

    // DevMsg("lfscale=%f center= %f rfscale=%f lrscale=%f
    // rrscale=%f\n",lfscale,fcscale, rfscale,lrscale,rrscale); DevMsg("pitch=%f
    // yaw=%f \n",pitch, yaw);
...
void CAudioDeviceBase::ApplyDSPEffects(int idsp,
...
void CAudioDeviceBase::MixUpsample(int sampleCount, int filtertype) {
  ...
  S_MixBufferUpsample2x(sampleCount, pPaint->pbuf,
                        &(pPaint->fltmem[ifilter][0]), CPAINTFILTERMEM,
                        filtertype);

  if (pPaint->fsurround) {
    Assert(pPaint->pbufrear);
    S_MixBufferUpsample2x(sampleCount, pPaint->pbufrear,
                          &(pPaint->fltmemrear[ifilter][0]), CPAINTFILTERMEM,
                          filtertype);

    if (pPaint->fsurround_center) {
      Assert(pPaint->pbufcenter);
      S_MixBufferUpsample2x(sampleCount, pPaint->pbufcenter,
                            &(pPaint->fltmemcenter[ifilter][0]),
                            CPAINTFILTERMEM, filtertype);
    }
  }

  // make sure on next upsample pass for this paintbuffer, new filter memory is
  // used
  pPaint->ifilter++;
}
...
void CAudioDeviceBase::Mix8Mono(channel_t *pChannel, char *pData,
  ...
  if (FVolumeFrontNonZero(volume)) {
    Mix8MonoWavtype(pChannel, pPaint->pbuf + outputOffset, volume,
  ...
  if (pPaint->fsurround) {
    if (FVolumeRearNonZero(volume)) {
      Assert(pPaint->pbufrear);
      Mix8MonoWavtype(pChannel, pPaint->pbufrear + outputOffset,
                      &volume[IREAR_LEFT], (uint8_t *)pData, inputOffset,
                      rateScaleFix, outCount);
    }

    if (pPaint->fsurround_center && FVolumeCenterNonZero(volume)) {
      Assert(pPaint->pbufcenter);
      Mix8MonoWavtype(pChannel, pPaint->pbufcenter + outputOffset,
                      &volume[IFRONT_CENTER], (uint8_t *)pData, inputOffset,
                      rateScaleFix, outCount);
    }
  }
}
...
void CAudioDeviceBase::Mix8Stereo(channel_t *pChannel, char *pData,
  ...
  if (FVolumeFrontNonZero(volume)) {
    Mix8StereoWavtype(pChannel, pPaint->pbuf + outputOffset, volume,
                      (uint8_t *)pData, inputOffset, rateScaleFix, outCount);
  }
  ...
  if (pPaint->fsurround) {
    if (FVolumeRearNonZero(volume)) {
      Assert(pPaint->pbufrear);
      Mix8StereoWavtype(pChannel, pPaint->pbufrear + outputOffset,
                        &volume[IREAR_LEFT], (uint8_t *)pData, inputOffset,
                        rateScaleFix, outCount);
    }
    ...
    if (pPaint->fsurround_center && FVolumeCenterNonZero(volume)) {
      Assert(pPaint->pbufcenter);
      Mix8StereoWavtype(pChannel, pPaint->pbufcenter + outputOffset,
                        &volume[IFRONT_CENTER], (uint8_t *)pData, inputOffset,
                        rateScaleFix, outCount);
    }
  }
...
void CAudioDeviceBase::Mix16Mono(channel_t *pChannel, short *pData,
                                 int outputOffset, int inputOffset,
                                 fixedint rateScaleFix, int outCount,
                                 int timecompress) {
  int volume[CCHANVOLUMES];
  ...
  if (FVolumeFrontNonZero(volume)) {
    Mix16MonoWavtype(pChannel, pPaint->pbuf + outputOffset, volume, pData,
  ...
  if (pPaint->fsurround) {
    if (FVolumeRearNonZero(volume)) {
      Assert(pPaint->pbufrear);
      Mix16MonoWavtype(pChannel, pPaint->pbufrear + outputOffset,
                       &volume[IREAR_LEFT], pData, inputOffset, rateScaleFix,
                       outCount);
    }

    if (pPaint->fsurround_center && FVolumeCenterNonZero(volume)) {
      Assert(pPaint->pbufcenter);
      Mix16MonoWavtype(pChannel, pPaint->pbufcenter + outputOffset,
                       &volume[IFRONT_CENTER], pData, inputOffset, rateScaleFix,
                       outCount);
    }
  }
}

void CAudioDeviceBase::Mix16Stereo(channel_t *pChannel, short *pData,
                                   int outputOffset, int inputOffset,
                                   fixedint rateScaleFix, int outCount,
                                   int timecompress) {
  int volume[CCHANVOLUMES];
  ...
  if (FVolumeFrontNonZero(volume)) {
    Mix16StereoWavtype(pChannel, pPaint->pbuf + outputOffset, volume, pData,
                       inputOffset, rateScaleFix, outCount);
  }
  ...
  if (pPaint->fsurround) {
    if (FVolumeRearNonZero(volume)) {
      Assert(pPaint->pbufrear);
      Mix16StereoWavtype(pChannel, pPaint->pbufrear + outputOffset,
                         &volume[IREAR_LEFT], pData, inputOffset, rateScaleFix,
                         outCount);
    }
    ...
    if (pPaint->fsurround_center && FVolumeCenterNonZero(volume)) {
      Assert(pPaint->pbufcenter);
      Mix16StereoWavtype(pChannel, pPaint->pbufcenter + outputOffset,
                         &volume[IFRONT_CENTER], pData, inputOffset,
                         rateScaleFix, outCount);
    }
  }
}
