...
CAudioMixerWaveMP3::CAudioMixerWaveMP3(IWaveData *data)
  ...
  m_headerOffset = 0;
  m_pStream =
      vaudio->CreateMP3StreamDecoder(static_cast<IAudioStreamEvent *>(this));
  if (m_pStream) {
...
void CAudioMixerWaveMP3::GetID3HeaderOffset() {
  ...
  m_headerOffset = 0;
  if ((pData[0] == 0x49) && (pData[1] == 0x44) && (pData[2] == 0x33) &&
      (pData[3] < 0xff) && (pData[4] < 0xff) && (pData[6] < 0x80) &&
      (pData[7] < 0x80) && (pData[8] < 0x80) && (pData[9] < 0x80)) {
    // this is in id3 file
    // compute the size of the wrapper and skip it
    m_headerOffset =
        10 + (pData[9] | (pData[8] << 7) | (pData[7] << 14) | (pData[6] << 21));
  }
}

int CAudioMixerWaveMP3::StreamRequestData(void *pBuffer, int bytesRequested,
                                          int offset) {
  if (offset < 0) {
    offset = m_offset;
  } else {
  ...
  while (bytesRequested > 0) {
    ...
    void *pData = NULL;
    int bytesRead = m_pData->ReadSourceData(&pData, offset + totalBytesRead,
                                            bytesRequested, pOutputBuffer);
...
int CAudioMixerWaveMP3::GetOutputData(void **pData, int sampleCount,
  ...
  if (m_samplePosition < m_sampleCount) {
    ...
    *pData = (void *)(m_samples + m_samplePosition);
    ...
    if (available > bytesRequired) available = bytesRequired;
...
//			NOTE: In most cases, only call this once, and call it before
//playing 			any data.
...
CAudioMixer *CreateMP3Mixer(IWaveData *data, int *pSampleRate) {
  ...
  if (pMixer->IsValid()) {
    // pass the sample rate back just in time to save parsing the MP3 file twice
    // to get sample rate
    if (pSampleRate) {
