...
// console streaming buffer implementation, appropriate for high latency and low
// memory shift this many buffers through the wave
#define STREAM_BUFFER_COUNT 2
// duration of audio samples per buffer, 200ms is 2x the worst frame rate (10Hz)
// the engine then has at least 400ms to deliver a new buffer or pop (assuming 2
// buffers)
...
#define TF_XBOX_WAV_MEMORY_CACHE \
  (24 * 1024 * 1024)  // Team Fortress uses a larger cache

ConVar snd_async_spew_blocking(
    "snd_async_spew_blocking", "0", 0,
    "Spew message to console any time async sound loading blocks on file i/o.");
ConVar snd_async_fullyasync(
    "snd_async_fullyasync", "0", 0,
    "All playback is fully async (sound doesn't play until data arrives).");
ConVar snd_async_stream_spew(
    "snd_async_stream_spew", "0", 0,
...
struct asyncwaveparams_t {
  ...
  FileNameHandle_t
      hFilename;  // handle to sound item name (i.e. not with sound\ prefix)
  int datasize;
  int seekpos;
  int alignment;
  bool bPrefetch;
...
class CAsyncWaveData {
 public:
  ...
  static void AsyncCallback(const FileAsyncRequest_t &asyncRequest,
                            int numReadBytes, FSAsyncStatus_t err);
  ...
  void OnAsyncCompleted(const FileAsyncRequest_t *asyncFilePtr,
                        int numReadBytes, FSAsyncStatus_t err);
  bool BlockingCopyData(void *destbuffer, int destbufsize, int startoffset,
                        int count);
...
void CAsyncWaveData::DestroyResource() {
  if (m_hAsyncControl) {
    if (!m_bLoaded && !m_bMissing) {
      // NOTE:  We CANNOT call AsyncAbort since if the file is actually being
      // read we'll end
      //  up still getting a callback, but our this ptr (deleted below) will
      //  be feeefeee and we'll trash the heap pretty bad.  So we call
      //  AsyncFinish, which will do a blocking read and will definitely
      //  succeed
...
// Output : ch const
//-----------------------------------------------------------------------------
ch const *CAsyncWaveData::GetFileName() {
  static ch sz[SOURCE_MAX_PATH];
...
unsigned int CAsyncWaveData::Size() {
  unsigned int size = sizeof(*this) + m_nDataSize;
  return size;
}
...
// Output : CAsyncWaveData
//-----------------------------------------------------------------------------
CAsyncWaveData *CAsyncWaveData::CreateResource(
    const asyncwaveparams_t &params) {
...
unsigned int CAsyncWaveData::EstimatedSize(const asyncwaveparams_t &params) {
  unsigned int size = sizeof(CAsyncWaveData) + params.datasize;
  return size;
}

//-----------------------------------------------------------------------------
// Purpose: Static method, called by thread, don't call anything non-threadsafe
// from handler!!! Input  : asyncFilePtr -
//			numReadBytes -
//			err -
//-----------------------------------------------------------------------------
void CAsyncWaveData::AsyncCallback(const FileAsyncRequest_t &asyncRequest,
                                   int numReadBytes, FSAsyncStatus_t err) {
  CAsyncWaveData *pObject =
      reinterpret_cast<CAsyncWaveData *>(asyncRequest.pContext);
  Assert(pObject);
  if (pObject) {
...
// from handler!!!
//-----------------------------------------------------------------------------
void CAsyncWaveData::QueuedLoaderCallback(void *pContext, void *pContext2,
                                          const void *pData, int nSize,
                                          LoaderError_t loaderError) {
  CAsyncWaveData *pObject = reinterpret_cast<CAsyncWaveData *>(pContext);
  Assert(pObject);

  pObject->OnAsyncCompleted(
      NULL, nSize,
      loaderError == LOADERERROR_NONE ? FSASYNC_OK : FSASYNC_ERR_FILEOPEN);
}

//-----------------------------------------------------------------------------
// Purpose: NOTE: THIS IS CALLED FROM A THREAD SO YOU CAN'T CALL INTO ANYTHING
// NON-THREADSAFE
...
//			err -
//-----------------------------------------------------------------------------
void CAsyncWaveData::OnAsyncCompleted(const FileAsyncRequest_t *asyncFilePtr,
                                      int numReadBytes, FSAsyncStatus_t err) {
  // Take hold of pointer (we can just use delete[] across .dlls because we
  // are using a shared memory allocator...)
  if (err == FSASYNC_OK || err == FSASYNC_ERR_READING) {
    ...
    if (SndAlignReads()) {
      ...
      m_pvData = ((uint8_t *)m_pAlloc) + m_async.nOffset;
    ...
    // Tracker 22905, et al.
    // Because this api gets called from the other thread, don't spew warning
    // here as it can
    //  cause a crash in searching CUtlSymbolTables since they use a global
    //  var for a LessFunc context!!!
    m_bMissing = true;
  }
}
...
//-----------------------------------------------------------------------------
bool CAsyncWaveData::BlockingCopyData(void *destbuffer, int destbufsize,
                                      int startoffset, int count) {
  if (!m_bLoaded) {
    Assert(m_hAsyncControl);
    // Force it to finish
    // It could finish between the above line and here, but the AsyncFinish call
    // will just have a bogus id, not a big deal
    if (snd_async_spew_blocking.GetBool()) {
      ...
      DevMsg("%f BCD:  Async I/O Force %s (%8.2f msec / %8.2f msec total)\n",
             realtime, GetFileName(), 1000.0f * (float)(ed - st),
  ...
  if (!m_bLoaded) {
    ...
    DevMsg("%f Async I/O Read successful %s (%8.2f msec)\n", realtime,
           GetFileName(), 1000.0f * (float)(m_arrival - m_start));
    m_arrival = 0;
  }
  ...
  memcpy(destbuffer, (ch *)m_pvData + (startoffset - m_async.nOffset), count);
...
bool CAsyncWaveData::IsCurrentlyLoading() {
  ...
  if (status == FSASYNC_STATUS_INPROGRESS || status == FSASYNC_OK) return true;
  return false;
}
...
bool CAsyncWaveData::BlockingGetDataPointer(void **ppData) {
  ...
  if (!m_bLoaded) {
    ...
    if (snd_async_spew_blocking.GetBool()) {
      ...
      float ed = (float)Plat_FloatTime();
      DevMsg(
          "%f BlockingGetDataPointer:  Async I/O Force %s (%8.2f msec / %8.2f "
          "msec total )\n",
          realtime, GetFileName(), 1000.0f * (float)(ed - st),
          1000.0f * (float)(m_arrival - m_start));
    } else {
  ...
  if (m_async.priority != priority) {
    ...
    if (snd_async_spew_blocking.GetInt() >= 2) {
      DevMsg("%f Async I/O Bumped priority for %s (%8.2f msec)\n", realtime,
             GetFileName(), 1000.0f * (float)(Plat_FloatTime() - m_start));
    }
  }
}
...
void CAsyncWaveData::StartAsyncLoading(const asyncwaveparams_t &params) {
  ...
  int nPriority = 1;
  if (params.bPrefetch) {
    // lower the priority of prefetched sounds, so they don't block immediate
    // sounds from being loaded
    nPriority = 0;
  }
...
//-----------------------------------------------------------------------------
class CAsyncWavDataCache
    : public IAsyncWavDataCache,
 ...
 public:
  ...
  virtual memhandle_t AsyncLoadCache(ch const *filename, int datasize,
                                     int startpos, bool bIsPrefetch = false);
  virtual void PrefetchCache(ch const *filename, int datasize, int startpos);
  ...
  virtual bool CopyDataIntoMemory(memhandle_t &handle, ch const *filename,
                                  ...
                                  int bytestocopy, bool *pbPostProcessed);
  ...
  virtual void RestartDataLoad(memhandle_t *handle, ch const *filename,
                               int datasize, int startpos);
  virtual bool IsDataLoadInProgress(memhandle_t handle);

  // Xbox: alternate multi-buffer streaming implementation
  virtual StreamHandle_t OpenStreamedLoad(ch const *pFileName, int dataSize,
                                          int dataStart, int startPos,
                                          int loopPos, int bufferSize,
                                          int numBuffers, streamFlags_t flags);
  ...
  virtual int CopyStreamedDataIntoMemory(StreamHandle_t hStream, void *pBuffer,
                                         int bufferSize, int copyStartPos,
                                         int bytesToCopy);
  ...
  // Cache helpers
  bool GetItemName(DataCacheClientID_t clientId, const void *pItem, ch *pDest,
                   unsigned nMaxLen);
 ...
 private:
  ...
  static bool BufferHandleLessFunc(const BufferEntry_t &lhs,
                                   const BufferEntry_t &rhs) {
  ...
  static bool CacheHandleLessFunc(const CacheEntry_t &lhs,
                                  const CacheEntry_t &rhs) {
...
//-----------------------------------------------------------------------------
// Purpose: Creates initial cache object if it doesn't already exist, starts
// async loading the actual data
//  in any case.
// Input  : *filename -
//			datasize -
//			startpos -
// Output : memhandle_t
//-----------------------------------------------------------------------------
memhandle_t CAsyncWavDataCache::AsyncLoadCache(ch const *filename, int datasize,
                                               int startpos, bool bIsPrefetch) {
...
//-----------------------------------------------------------------------------
memhandle_t CAsyncWavDataCache::FindOrCreateBuffer(asyncwaveparams_t &params,
                                                   bool bFind) {
  ...
  if (bFind) {
    // look for an existing buffer that matches exactly (same file, offset, and
    // share)
  ...
  // a buffering stream may obsolete it's buffers by reducing the lock count,
  // allowing for lru
  CAsyncWaveData *pWaveData = CacheLock(search.m_hWaveData);
  if (!pWaveData) {
    ...
    BufferHandle_t hBuffer = m_BufferList.Insert(search);
...
//-----------------------------------------------------------------------------
// Purpose: Load data asynchronously via multi-buffers, returns specialized
// handle
//-----------------------------------------------------------------------------
StreamHandle_t CAsyncWavDataCache::OpenStreamedLoad(
    ch const *pFileName, int dataSize, int dataStart, int startPos, int loopPos,
    int bufferSize, int numBuffers, streamFlags_t flags) {
  ...
  // private and cannot be shared a non-single play stream wants persisting
  // buffers and attempts to reclaim a matching buffer
...
void CAsyncWavDataCache::CloseStreamedLoad(StreamHandle_t hStream) {
  ...
  for (int i = 0; i < streamedEntry.m_numBuffers; ++i) {
    // multiple streams could be using the same buffer, keeping the lock count
    // nonzero
    ...
    if (streamedEntry.m_bSinglePlay) {
      // a buffering single play stream has no reason to reuse its own buffers
      // and destroys them
      Assert(lockCount == 0);
...
//			startpos -
//-----------------------------------------------------------------------------
void CAsyncWavDataCache::PrefetchCache(ch const *filename, int datasize,
                                       int startpos) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAsyncWavDataCache::CopyDataIntoMemory(ch const *filename, int datasize,
                                            int startpos, void *buffer,
                                            int bufsize, int copystartpos,
                                            int bytestocopy,
                                            bool *pbPostProcessed) {
  ...
  return CopyDataIntoMemory(m_CacheHandles[idx].handle, filename, datasize,
                            startpos, buffer, bufsize, copystartpos,
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAsyncWavDataCache::CopyDataIntoMemory(memhandle_t &handle,
                                            ch const *filename, int datasize,
                                            int startpos, void *buffer,
                                            int bufsize, int copystartpos,
                                            int bytestocopy,
                                            bool *pbPostProcessed) {
  ...
  *pbPostProcessed = false;
  ...
  // Cache entry exists, but if filesize == 0 then the file itself wasn't on
  // disk...
...
//-----------------------------------------------------------------------------
int CAsyncWavDataCache::CopyStreamedDataIntoMemory(int hStream, void *pBuffer,
                                                   int bufferSize,
                                                   int copyStartPos,
                                                   int bytesToCopy) {
  ...
  while (1) {
    ...
    if (snd_async_stream_spew.GetInt() >= 1) {
      // interval is the audio block clock rate, the block must be available
      // within this interval a faster audio rate or smaller block size implies
      // a smaller interval latency is the actual block delivery time latency
      // must not exceed the delivery interval or stariving occurs and audio
      // pops
      ...
      DevMsg(
          "Stream:%2d interval:%5dms latency:%5dms offset:%d length:%d (%s)\n",
          hStream, interval, latency, pFront->m_async.nOffset,
          pFront->m_nReadSize, pFront->GetFileName());
    }

    if (bCompleted && pFront->m_hAsyncControl &&
        (pFront->m_bLoaded || pFront->m_bMissing)) {
    ...
    if (bCompleted && pFront->m_bLoaded) {
      if (bufferPos >= 0 && bufferPos < pFront->m_nReadSize) {
        ...
        memcpy(pBuffer, (ch *)pFront->m_pvData + bufferPos, count);
  ...
  if (streamedEntry.m_numBuffers > 1) {
    ...
    while (streamedEntry.m_Front < index) {
      if (!actualCopied && !bWaiting) {
        // couldn't return any data because the buffers aren't in the right
        // location oh no! caller must be skipping due to latency the next
        // buffer position has to start one full buffer ahead of the caller's
        // desired read location hopefully only 1 buffer will stutter
        nextStartPos = copyStartPos - streamedEntry.m_DataStart +
                       streamedEntry.m_BufferSize;
      ...
      nextStartPos = streamedEntry.m_SectorSize *
                     (nextStartPos / streamedEntry.m_SectorSize);
...
//-----------------------------------------------------------------------------
void *CAsyncWavDataCache::GetStreamedDataPointer(StreamHandle_t hStream,
                                                 bool bSync) {
  void *pData;
  CAsyncWaveData *pFront;
  int index;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAsyncWavDataCache::GetDataPointer(memhandle_t &handle, ch const *filename,
                                        int datasize, int startpos,
                                        void **pData, int copystartpos,
                                        bool *pbPostProcessed) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAsyncWavDataCache::IsDataLoadCompleted(memhandle_t handle,
                                             bool *pIsValid) {
...
void CAsyncWavDataCache::RestartDataLoad(memhandle_t *pHandle,
                                         const ch *pFilename, int dataSize,
                                         int startpos) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAsyncWavDataCache::GetItemName(DataCacheClientID_t clientId,
                                     const void *pItem, ch *pDest,
                                     unsigned nMaxLen) {
...
void PrefetchDataStream(const ch *pFileName, int dataOffset, int dataSize) {
...
//			The mixer doesn't know the file is streaming.  The
// IWaveData 			abstracts the data access.  The mixer abstracts
// data encoding/format
//-----------------------------------------------------------------------------
class CWaveDataStreamAsync : public IWaveData {
 public:
  CWaveDataStreamAsync(CAudioSource &source, IWaveStreamSource *pStreamSource,
                       const ch *pFileName, int fileStart, int fileSize,
                       CSfxTable *sfx, int startOffset);
  ~CWaveDataStreamAsync(void);

  // return the source pointer (mixer needs this to determine some things like
  // sampling rate)
  CAudioSource &Source() { return m_source; }

  // Read data from the source - this is the primary function of a IWaveData
  // subclass Get the data from the buffer (or reload from disk)
...
CWaveDataStreamAsync::CWaveDataStreamAsync(CAudioSource &source,
                                           IWaveStreamSource *pStreamSource,
                                           const ch *pFileName, int fileStart,
                                           int fileSize, CSfxTable *sfx,
                                           int startOffset)
    : m_source(source),
  ...
  m_buffer = new ch[SINGLE_BUFFER_SIZE];
  memset(m_buffer, 0, SINGLE_BUFFER_SIZE);
  ...
  m_hCache =
      wavedatacache->AsyncLoadCache(GetFileName(), m_dataSize, m_dataStart);
  ...
  m_AudioCacheHandle.Get(CAudioSource::AUDIO_SOURCE_WAV,
...
// Output : ch const
//-----------------------------------------------------------------------------
ch const *CWaveDataStreamAsync::GetFileName() {
  static ch fn[SOURCE_MAX_PATH];
...
bool CWaveDataStreamAsync::IsReadyToMix() {
  ...
  if (!bCacheValid) {
    wavedatacache->RestartDataLoad(&m_hCache, GetFileName(), m_dataSize,
                                   m_dataStart);
  }
  return bLoaded;
}
...
//-----------------------------------------------------------------------------
// Purpose: Read data from the source - this is the primary function of a
// IWaveData subclass
...
// Output : int
//-----------------------------------------------------------------------------
int CWaveDataStreamAsync::ReadSourceData(void **pData, int sampleIndex,
                                         int sampleCount,
                                         ch copyBuf[AUDIOSOURCE_COPYBUF_SIZE]) {
  ...
  // BUGBUG: Just play the beginning of the buffer until we get to a valid
  // linear position
  if (sampleIndex < m_sampleIndex) sampleIndex = m_sampleIndex;

  // calc sample position relative to the current buffer
  // m_sampleIndex is the sample position of the first uint8_t of the buffer
  ...
  if (sampleIndex >= m_bufferCount) {
    ...
    // if the remainder is greated than one buffer size, seek over it.
    // Otherwise, read the next chunk and leave the remainder as an offset.

    // number of buffers to "skip" (as in the case where we are starting a
    // streaming sound not at the beginning)
    ...
    if (IsPC()) {
      // See if we can load in the intial data right out of the cached data lump
      // instead.
      int cacheddatastartpos = (seekpos - m_dataStart);

      // FastGet doesn't call into IsPrecachedSound if the handle appears
      // valid...
      ...
      if (info && (m_nCachedDataSize > 0) &&
          (cacheddatastartpos < m_nCachedDataSize)) {
        // Get a ptr to the cached data
        const uint8_t *cacheddata = info->CachedData();
        if (cacheddata) {
          // See how many samples of cached data are available
          // (cacheddatastartpos is zero on the first read)
          int availSamples =
              (m_nCachedDataSize - cacheddatastartpos) / m_sampleSize;
          ...
          Q_memcpy(m_buffer, (ch *)cacheddata + cacheddatastartpos,
                   availSamples * m_sampleSize);

          startupCacheUsed = true;
        }
      }

      // Not in startup cache, grab data from async cache loader (will block if
      ...
      if (!startupCacheUsed) {
        ...
        if (!wavedatacache->CopyDataIntoMemory(
                m_hCache, GetFileName(), m_dataSize, m_dataStart, m_buffer,
                m_bufferSize * m_sampleSize, seekpos,
                m_bufferCount * m_sampleSize, &postprocessed)) {
          return 0;
        }

        // do any conversion the source needs (mixer will decode/decompress)
        if (!postprocessed) {
          // Note that we don't set the postprocessed flag on the underlying
          // data, since for streaming we're copying the
...
// Output : int
//-----------------------------------------------------------------------------
int CWaveDataMemoryAsync::ReadSourceData(void **pData, int sampleIndex,
                                         int sampleCount,
                                         ch copyBuf[AUDIOSOURCE_COPYBUF_SIZE]) {
...
// Output : IWaveData
//-----------------------------------------------------------------------------
IWaveData *CreateWaveDataStream(CAudioSource &source,
                                IWaveStreamSource *pStreamSource,
                                const ch *pFileName, int dataStart,
                                int dataSize, CSfxTable *pSfx,
                                int startOffset) {
  CWaveDataStreamAsync *pStream = new CWaveDataStreamAsync(
      source, pStreamSource, pFileName, dataStart, dataSize, pSfx, startOffset);
