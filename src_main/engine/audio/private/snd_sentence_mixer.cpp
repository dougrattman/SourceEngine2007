...
class CSentenceMixer : public CAudioMixer {
 public:
  ...
  // BUGBUG: These are only applied to the current word, not the whole
  // sentence!!!!
...
CSentenceMixer::CSentenceMixer(voxword_t *pWords) {
  ...
  for (int nWord = 0; nWord < m_nNumWords; nWord++) {
    // it is possible to get a 0 mixer (due to wav error, etc)
...
int CSentenceMixer::SkipSamples(channel_t *pChannel, int sampleCount,
...
// return number of samples mixed
int CSentenceMixer::MixDataToDevice(IAudioDevice *pDevice, channel_t *pChannel,
                                    int sampleCount, int outputRate,
                                    int outputOffset) {
  if (!m_pCurrentWordMixer) {
    return 0;
  ...
  while (sampleCount > 0 && m_pCurrentWordMixer) {
    int outputCount = m_pCurrentWordMixer->MixDataToDevice(
        pDevice, pChannel, sampleCount, outputRate, outputOffset);
