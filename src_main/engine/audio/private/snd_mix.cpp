...
void SND_PaintChannelFrom8(portable_samplepair_t *pOutput, int *volume,
                           uint8_t *pData8, int count);
...
// pointer to current paintbuffer (front and reare), used by all mixing,
// upsampling and dsp routines
...
// type 1 dsp, type 2 mixer, type 3 load sound, type 4 all sound
int g_snd_profile_type = 0;
...
bool MIX_InitAllPaintbuffers(void) {
  ...
  g_paintBuffers =
      (paintbuffer_t *)malloc(CPAINTBUFFERS * sizeof(paintbuffer_t));
  V_memset(g_paintBuffers, 0, CPAINTBUFFERS * sizeof(paintbuffer_t));

  g_temppaintbuffer = (portable_samplepair_t *)_aligned_malloc(
      TEMP_COPY_BUFFER_SIZE * sizeof(portable_samplepair_t), 16);
  V_memset(g_temppaintbuffer, 0,
           TEMP_COPY_BUFFER_SIZE * sizeof(portable_samplepair_t));

  for (i = 0; i < CPAINTBUFFERS; i++) {
    g_paintBuffers[i].pbuf = (portable_samplepair_t *)_aligned_malloc(
        PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t), 16);
    V_memset(g_paintBuffers[i].pbuf, 0,
             PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t));

    if (bSurround) {
      g_paintBuffers[i].pbufrear = (portable_samplepair_t *)_aligned_malloc(
          PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t), 16);
      V_memset(g_paintBuffers[i].pbufrear, 0,
               PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t));
    }
    if (bSurroundCenter) {
      g_paintBuffers[i].pbufcenter = (portable_samplepair_t *)_aligned_malloc(
          PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t), 16);
      V_memset(g_paintBuffers[i].pbufcenter, 0,
               PAINTBUFFER_MEM_SIZE * sizeof(portable_samplepair_t));
    }
  }
...
double MIX_GetMaxRate(double rate, int sampleCount) {
  ...
  double copybuf_bytes =
      (double)(TEMP_COPY_BUFFER_SIZE * sizeof(portable_samplepair_t));
...
// lpaintedtime - total number of 32 bit stereo samples previously output to
// hardware endtime - total number of 32 bit stereo samples currently mixed in
// paintbuffer
void S_TransferStereo16(void *pOutput, const portable_samplepair_t *pfront,
  ...
  while (lpaintedtime < endtime) {
    ...
    // lpos - samplepair index into dma buffer. First samplepair from
    // paintbuffer to be xfered here.
    lpos = lpaintedtime & sampleMask;

    // snd_out is L/R sample index into dma buffer.  First L sample from
    // paintbuffer goes here.
    snd_out = (short *)pOutput + (lpos << 1);

    // snd_linear_count is number of samplepairs between end of dma buffer and
    // xfer start index.
    ...
    if (snd_linear_count > endtime - lpaintedtime) {
      // endtime - lpaintedtime = number of premixed sample pairs ready for
      // xfer.
    ...
    SND_RecordBuffer();
    if (bShouldPlaySound) {
      // transfer 16bit samples from snd_p into snd_out, multiplying each sample
      // by volume.
...
void S_TransferPaintBuffer(void *pOutput, const portable_samplepair_t *pfront,
                           int lpaintedtime, int endtime) {
  ...
  // out_idx = ((paintedtime << HISPEED_DMA) * g_AudioDevice->DeviceChannels())
  // & out_mask;
  ...
  step =
      3 -
      g_AudioDevice
          ->DeviceChannels();  // mono output buffer - step 2, stereo - step 1
  int the_snd_vol = S_GetMasterVolume() * 256;
...
void S_FreeChannel(channel_t *ch) {
  ...
  if (ch->flags.m_bIsFreeingChannel) return;
  ch->flags.m_bIsFreeingChannel = true;

  SND_CloseMouth(ch);

  g_pSoundServices->OnSoundStopped(ch->guid, ch->soundsource, ch->entchannel,
                                   ch->sfx->getname());
...
// 'endtime' is reached. endtime: time in 44khz samples to mix rate: ignore
// samples which are not natively at this rate (for multipass mixing/filtering)
...
// outputRate: target mix rate for all samples.  Note, if outputRate =
// SOUND_DMA_SPEED, then
//		 this routine will fill the paintbuffer to endtime.  Otherwise,
// fewer samples are mixed. 		 if (endtime - paintedtime) is not
// aligned on boundaries of 4, 		 we'll miss data if outputRate <
// SOUND_DMA_SPEED!
void MIX_MixChannelsToPaintbuffer(CChannelList &list, int endtime, int flags,
                                  int rate, int outputRate) {
  ...
  Assert(
      !((endtime - g_paintedtime) & 0x3) ||
      (outputRate == SOUND_DMA_SPEED));  // make sure we're not discarding data
  ...
  for (i = list.Count(); --i >= 0;) {
    ...
    Assert(ch->sfx);
    // must never have a 'dry' and 'speaker' set - causes double mixing & double
    // data reading
    Assert(!(ch->flags.bdry && ch->flags.bSpeaker));

    // if mixing with SOUND_MIX_DRY flag, ignore (don't even load) all channels
    // not flagged as 'dry'
    if (flags == SOUND_MIX_DRY) {
      if (!ch->flags.bdry) continue;
    }

    // if mixing with SOUND_MIX_WET flag, ignore (don't even load) all channels
    // flagged as 'dry' or 'speaker'
    if (flags == SOUND_MIX_WET) {
      if (ch->flags.bdry || ch->flags.bSpeaker) continue;
    }

    // if mixing with SOUND_MIX_SPEAKER flag, ignore (don't even load) all
    // channels not flagged as 'speaker'
    if (flags == SOUND_MIX_SPEAKER) {
      if (!ch->flags.bSpeaker) continue;
    }
    ...
    switch (rate) {
      ...
      case SOUND_22k:
      case SOUND_44k:
        if (rate != ch->sfx->pSource->SampleRate()) continue;
        break;
      default:
    ...
    // Tracker 20771, if breen is speaking through the monitor, the client
    // doesn't have an entity
    //  for the "soundsource" but we still need the lipsync to pause if the game
    //  is paused.  Therefore I changed SND_IsMouth to look for any .wav on any
    //  channels which has sentence data
    bool bIsMouth = SND_IsMouth(ch);
    bool bShouldPause = IsX360() ? !ch->sfx->m_bIsUISound : bIsMouth;

    // Tracker 14637:  Pausing the game pauses voice sounds, but not other
    // sounds...
    ...
    if (bIsMouth) {
      if (entitylist->GetClientEntity(ch->soundsource) ||
          (ch->flags.bSpeaker &&
           entitylist->GetClientEntity(ch->speakerentity))) {
    ...
    // NOTE: must be called once per channel only - consecutive calls retrieve
    // additional data.
    if (list.IsQuashed(i)) {
      ...
      ch->pMixer->MixDataToDevice(g_AudioDevice, ch, sampleCount, outputRate,
                                  0);
    }
...
// pass in index -1...count+2, return pointer to source sample in either
// paintbuffer or delay buffer
inline portable_samplepair_t *S_GetNextpFilter(
    int i, portable_samplepair_t *pbuffer, portable_samplepair_t *pfiltermem) {
  // The delay buffer is assumed to precede the paintbuffer by 6 duplicated
  // samples
...
// prevfilter:  filter memory. NOTE: this must match the filtertype ie:
// filtercubic[] for FILTERTYPE_CUBIC
//				if NULL then perform no filtering. UNDONE:
// should have a filter memory array type
// count: how many samples to upsample. will become count*2 samples in buffer,
// in place.

void S_Interpolate2xCubic(portable_samplepair_t *pbuffer,
                          portable_samplepair_t *pfiltermem, int cfltmem,
                          int count) {
  // implement cubic interpolation on 2x upsampled buffer.   Effectively delays
  // buffer contents by 2 samples. pbuffer: contains samples at 0, 2, 4, 6...
  // temppaintbuffer is temp buffer, of same or larger size than a paintbuffer,
  // used to store processed values count: number of samples to process in
  // buffer ie: how many samples at 0, 2, 4, 6...
  ...
  for (i = 0; i < upCount; i++) pbuffer[i] = g_temppaintbuffer[i];
}

// pass forward over passed in buffer and linearly interpolate all odd samples
// pbuffer: buffer to filter (in place)
// prevfilter:  filter memory. NOTE: this must match the filtertype ie:
// filterlinear[] for FILTERTYPE_LINEAR
//				if NULL then perform no filtering.
// count: how many samples to upsample. will become count*2 samples in buffer,
// in place.

void S_Interpolate2xLinear(portable_samplepair_t *pbuffer,
                           portable_samplepair_t *pfiltermem, int cfltmem,
                           int count) {
  int i, upCount = count << 1;
...
void S_Interpolate2xLinear_2(int count, portable_samplepair_t *pbuffer,
                             portable_samplepair_t *pfiltermem, int cfltmem) {
  Assert(cfltmem >= 1);
...
// count: how many samples to upsample. will become count*2 samples in buffer,
// in place. pbuffer: buffer to upsample into (in place) pfiltermem:  filter
// memory. NOTE: this must match the filtertype ie: filterlinear[] for
// FILTERTYPE_LINEAR
//				if NULL then perform no filtering.
// cfltmem: max number of sample pairs filter can use
// filtertype: FILTERTYPE_NONE, _LINEAR, _CUBIC etc.  Must match prevfilter.
void S_MixBufferUpsample2x(int count, portable_samplepair_t *pbuffer,
                           portable_samplepair_t *pfiltermem, int cfltmem,
                           int filtertype) {
...
// The set paintbuffer is used by all subsequent mixing, upsampling and dsp
// routines. Also sets the rear paintbuffer if paintbuffer has fsurround true.
...
// returns 0 if fsurround is false;
...
// returns 0 if fsurround_center is false;
...
inline int MIX_GetIPaintFromPFront(portable_samplepair_t *pbuf) {
  ...
  for (i = 0; i < CPAINTBUFFERS; i++) {
    if (pbuf == g_paintBuffers[i].pbuf) return i;
  }
...
inline void MIX_ConvertBufferToSurround(int ipaintbuffer) {
  ...
  if (g_AudioDevice->IsSurround()) {
    ...
    if (g_AudioDevice->IsSurroundCenter())
      Q_memcpy(pcenter, pfront,
               sizeof(portable_samplepair_t) * PAINTBUFFER_SIZE);
  }
}
...
{
  int i;
  for (i = 0; i < CPAINTBUFFERS; i++) g_paintBuffers[i].ifilter = 0;
}
...
void MIX_ClearAllPaintBuffers(int SampleCount, bool clearFilters) {
  ...
  for (i = 0; i < CPAINTBUFFERS; i++) {
    if (g_paintBuffers[i].pbuf != NULL)
      Q_memset(g_paintBuffers[i].pbuf, 0,
               (count + 1) * sizeof(portable_samplepair_t));

    if (g_paintBuffers[i].pbufrear != NULL)
      Q_memset(g_paintBuffers[i].pbufrear, 0,
               (count + 1) * sizeof(portable_samplepair_t));

    if (g_paintBuffers[i].pbufcenter != NULL)
      Q_memset(g_paintBuffers[i].pbufcenter, 0,
               (count + 1) * sizeof(portable_samplepair_t));

    if (clearFilters) {
      Q_memset(g_paintBuffers[i].fltmem, 0, sizeof(g_paintBuffers[i].fltmem));
      Q_memset(g_paintBuffers[i].fltmemrear, 0,
               sizeof(g_paintBuffers[i].fltmemrear));
      Q_memset(g_paintBuffers[i].fltmemcenter, 0,
               sizeof(g_paintBuffers[i].fltmemcenter));
    }
  }
...
// if all buffers are 4 or 5 ch surround, mix rear & center channels into ibuf3
// as well.

// NOTE: for performance, conversion and mixing are done in a single pass
// instead of a two pass channel convert + mix scheme.

void MIX_MixPaintbuffers(int ibuf1, int ibuf2, int ibuf3, int count,
                         float fgain_out) {
  ...
  cchan1 = 2 + (g_paintBuffers[ibuf1].fsurround ? 2 : 0) +
           (g_paintBuffers[ibuf1].fsurround_center ? 1 : 0);
  cchan2 = 2 + (g_paintBuffers[ibuf2].fsurround ? 2 : 0) +
           (g_paintBuffers[ibuf2].fsurround_center ? 1 : 0);
  cchan3 = 2 + (g_paintBuffers[ibuf3].fsurround ? 2 : 0) +
           (g_paintBuffers[ibuf3].fsurround_center ? 1 : 0);
  ...
  if (cchan3 == 2) {
    ...
    if (cchan1 == 2 && cchan2 == 4) {
      ...
      for (i = 0; i < count; i++) {
        ...
        pbuf3[i].right =
            pbuf1[i].right + AVG(pbuf2[i].right, pbufrear2[i].right);
      }
      goto gain2ch;
    }
    ...
    if (cchan1 == 4 && cchan2 == 4) {
      ...
      for (i = 0; i < count; i++) {
        pbuf3[i].left = AVG(pbuf1[i].left, pbufrear1[i].left) +
                        AVG(pbuf2[i].left, pbufrear2[i].left);
        pbuf3[i].right = AVG(pbuf1[i].right, pbufrear1[i].right) +
                         AVG(pbuf2[i].right, pbufrear2[i].right);
      }
      goto gain2ch;
    }
    ...
    if (cchan1 == 4 && cchan2 == 5) {
      for (i = 0; i < count; i++) {
        ...
        pbuf3[i].left =
            AVG(pbuf1[i].left, pbufrear1[i].left) + AVG(l, pbufrear2[i].left);
        pbuf3[i].right = AVG(pbuf1[i].right, pbufrear1[i].right) +
                         AVG(r, pbufrear2[i].right);
      }
      goto gain2ch;
    }
    ...
    if (cchan1 == 5 && cchan2 == 5) {
      for (i = 0; i < count; i++) {
        ...
        pbuf3[i].right =
            AVG(r, pbufrear1[i].right) + AVG(r2, pbufrear2[i].right);
      }
      goto gain2ch;
    }
  }
  ...
  if (!g_paintBuffers[bufferIndex].fsurround) {
    ...
    if (g_paintBuffers[bufferIndex].fsurround_center) {
      for (i = 0; i < count; i++) {
        ...
        // pbufcenter[i].right = (pbufcenter[i].right * gain) >> 8; mono center
        // channel
      }
    }
  }
...
// DEBUG code - ibuf is buffer index, count is # samples to test, pppeakprev
// stores peak

void SDEBUG_GetAvgValue(int ibuf, int count, float *pav) {
#ifdef _SDEBUG
  if (snd_showstart.GetInt() != 4) return;

  float av = 0.0;

  for (int i = 0; i < count; i++)
    av += (float)(abs(g_paintBuffers[ibuf].pbuf->left) +
                  abs(g_paintBuffers[ibuf].pbuf->right)) /
          2.0;
...
void SDEBUG_GetAvgIn(int ibuf, int count) {
  ...
  sdebug_avg_in = ((av * count) + (sdebug_avg_in * sdebug_in_count)) /
                  (count + sdebug_in_count);
...
void SDEBUG_GetAvgOut(int ibuf, int count) {
  ...
  sdebug_avg_out = ((av * count) + (sdebug_avg_out * sdebug_out_count)) /
                   (count + sdebug_out_count);
  ...
  if (sdebug_in_count > SDEBUG_TOTAL_COUNT) {
    if ((int)sdebug_avg_in > 20.0 && (int)sdebug_avg_out > 20.0)
      DevMsg("dsp avg gain:%1.2f in:%1.2f out:%1.2f 1/gain:%1.2f\n",
             sdebug_avg_out / sdebug_avg_in, sdebug_avg_in, sdebug_avg_out,
...
// mix channels matching 'flags' (SOUND_MIX_DRY, SOUND_MIX_WET,
// SOUND_MIX_SPEAKER) into specified paintbuffer upsamples 11khz, 22khz channels
// to 44khz.

// NOTE: only call this on channels that will be mixed into only 1 paintbuffer
// and that will not be mixed until the next mix pass! otherwise,
// MIX_MixChannelsToPaintbuffer will advance any internal pointers on mixed
// channels; subsequent calls will be at incorrect offset.

void MIX_MixUpsampleBuffer(CChannelList &list, int ipaintbuffer, int end,
                           int count, int flags) {
  ...
  MIX_ActivatePaintbuffer(
      ipaintbuffer);  // operates on MIX_MixChannelsToPaintbuffer
  ...
  if (list.m_has11kChannels) {
    ...
    g_AudioDevice->MixUpsample(count / (SOUND_DMA_SPEED / SOUND_11k),
                               FILTERTYPE_LINEAR);
  }
  ...
  if (list.m_has22kChannels || list.m_has11kChannels) {
    ...
    g_AudioDevice->MixUpsample(count / (SOUND_DMA_SPEED / SOUND_22k),
                               FILTERTYPE_LINEAR);
#endif
  }
...
// upsample and mix sounds into final 44khz versions of the following
// paintbuffers: IROOMBUFFER, IFACINGBUFFER, IFACINGAWAY, IDRYBUFFER,
// ISPEAKERBUFFER dsp fx are then applied to these buffers by the caller. caller
// also remixes all into final IPAINTBUFFER output.

void MIX_UpsampleAllPaintbuffers(CChannelList &list, int end, int count) {
  VPROF("MixUpsampleAll");

  // 'dry' and 'speaker' channel sounds mix 100% into their corresponding
  // buffers

  // mix and upsample all 'dry' sounds (channels) to 44khz IDRYBUFFER
  // paintbuffer

  if (list.m_hasDryChannels)
    MIX_MixUpsampleBuffer(list, IDRYBUFFER, end, count, SOUND_MIX_DRY);

  // mix and upsample all 'speaker' sounds (channels) to 44khz ISPEAKERBUFFER
  // paintbuffer
  ...
  // 11khz sounds are mixed into 3 buffers based on distance from listener, and
  // facing direction These buffers are room, facing, facingaway These 3 mixed
  // buffers are then each upsampled to 22khz.

  // 22khz sounds are mixed into the 3 buffers based on distance from listener,
  // and facing direction These 3 mixed buffers are then each upsampled to
  // 44khz.

  // 44khz sounds are mixed into the 3 buffers based on distance from listener,
  ...
  if (!g_bDspOff) {
    ...
    MIX_ActivatePaintbuffer(
        IROOMBUFFER);  // operates on MIX_MixChannelsToPaintbuffer
  }
  ...
  MIX_MixChannelsToPaintbuffer(list, end, SOUND_MIX_WET, SOUND_44k,
                               SOUND_DMA_SPEED);
...
ConVar snd_cull_duplicates(
    "snd_cull_duplicates", "0", FCVAR_NONE,
    "If nonzero, aggressively cull duplicate sounds during mixing. The number "
    "specifies the number of duplicates allowed to be played.");

// Helper class for determining whether a given channel number should be culled
// from mixing, if snd_cull_duplicates is enabled (psychoacoustic quashing).
class CChannelCullList {
 public:
  // default constructor
  CChannelCullList() : m_numChans(0){};

  // call if you plan on culling channels - and not otherwise, it's a little
  // expensive (that's why it's not in the constructor)
  ...
  // TODO: there may be a way to do this faster on 360 (eg, pad to 128bit, use
  // SIMD)
  struct sChannelVolData {
    int m_channelNum;
    int m_vol;  // max volume of sound. -1 means "do not cull, ever, do not even
                // do the math"
...
// returns < 0 if a should come before b, > 0 if a should come after, 0
// otherwise
static int __cdecl ChannelVolComparator(const void *a, const void *b) {
  // greater numbers come first.
  return static_cast<const CChannelCullList::sChannelVolData *>(b)->m_vol -
         static_cast<const CChannelCullList::sChannelVolData *>(a)->m_vol;
}

void CChannelCullList::Initialize(CChannelList &list) {
  VPROF("CChannelCullList::Initialize");
  // First, build a sorted list of channels by decreasing volume, and by a hash
  // of their wavname.
  ...
  for (int i = m_numChans - 1; i >= 0; --i) {
    ...
    if (ch && ch->pMixer->IsReadyToMix()) {
      ...
      AssertMsg(m_channelInfo[i].m_vol >= 0,
                "Sound channel has a negative volume?");
  ...
  // Sort the list.
  qsort(m_channelInfo, MAX_CHANNELS, sizeof(sChannelVolData),
        ChannelVolComparator);

  // Then, determine if the given sound is less than the nth loudest of its
  // hash. If so, mark its flag for removal.
  ...
  for (int i = 0; i < m_numChans; ++i)  // i is index in original channel list
  {
    ...
    int howManyLouder = 0;
    for (int j = 0; m_channelInfo[j].m_channelNum != i &&
                    m_channelInfo[j].m_vol >= 0 && j < MAX_CHANNELS;
         ++j) {
...
void MIX_BuildChannelList(CChannelList &list) {
  ...
  for (int i = list.Count(); --i >= 0;) {
    channel_t *ch = list.GetChannel(i);
    bool bRemove = false;
    // Certain async loaded sounds lazily load into memory in the background,
    ...
    if (ch->pMixer->IsReadyToMix()) {
      ...
      // Don't mix sound data for sounds with 'zero' volume. If it's a
      // non-looping sound, just remove the sound when its volume goes to zero.
      // If it's a 'dry' channel sound (ie: music) then assume bZeroVolume is
      // fade in - don't restart

      // To be 'zero' volume, all target volume and current volume values must
      // all be less than 5

      bool bZeroVolume = BChannelLowVolume(ch, 1);

      if (!pSource ||
          (bZeroVolume && !pSource->IsLooped() && !ch->flags.bdry)) {
        // NOTE: Since we've loaded the sound, check to see if it's a sentence.
        // Play them at zero anyway to keep the character's lips moving and the
        // captions happening.
      ...
      if (!bRemove && snd_cull_duplicates.GetInt() > 0) {
        // We can't simply remove them, because then sounds will pile up waiting
        // to finish later. We need to flag them for not mixing.
    ...
    if (ch->flags.delayed_start && !SND_IsMouth(ch)) delayStart = true;
...
// A mix pass is performed, resulting in mixed sounds in IROOMBUFFER,
// IFACINGBUFFER, IFACINGAWAYBUFFER, IDRYBUFFER, ISPEAKERBUFFER:

// directional sounds are panned and mixed between IFACINGBUFFER and
// IFACINGAWAYBUFFER omnidirectional sounds are panned 100% into IFACINGBUFFER
// sound sources far from player (ie: near back of room ) are mixed in
// proportion to this distance into IROOMBUFFER sounds with ch->bSpeaker set are
// mixed in mono into ISPEAKERBUFFER

// dsp_facingaway fx (2 or 4ch filtering) are then applied to the
// IFACINGAWAYBUFFER dsp_speaker fx (1ch) are then applied to the ISPEAKERBUFFER
...
void MIX_PaintChannels(int endtime, bool bIsUnderwater) {
  ...
  if (g_AudioDevice->IsSurround()) {
    g_dsp_volume *=
        (g_AudioDevice->IsSurroundCenter() ? dsp_vol_5ch.GetFloat()
                                           : dsp_vol_4ch.GetFloat());
  } else {
  ...
  // get dsp preset gain values, update gain crossfaders, used when mixing dsp
  // processed buffers into paintbuffer
  SDEBUG_ShowAvgValue();
  ...
  while (g_paintedtime < endtime) {
    ...
    // apply appropriate dsp fx to each buffer, remix buffers into single quad
    // output buffer apply 2 or 4ch filtering to IFACINGAWAY buffer
    if (g_bdirectionalfx) {
      g_AudioDevice->ApplyDSPEffects(
          idsp_facingaway, MIX_GetPFrontFromIPaint(IFACINGAWAYBUFFER),
          MIX_GetPRearFromIPaint(IFACINGAWAYBUFFER),
          MIX_GetPCenterFromIPaint(IFACINGAWAYBUFFER), count);
    }

    if (!g_bDspOff && list.m_hasSpeakerChannels) {
      // apply 1ch filtering to ISPEAKERBUFFER
      g_AudioDevice->ApplyDSPEffects(
          idsp_speaker, MIX_GetPFrontFromIPaint(ISPEAKERBUFFER),
          MIX_GetPRearFromIPaint(ISPEAKERBUFFER),
    ...
    g_AudioDevice->ApplyDSPEffects(
        Get_idsp_room(), MIX_GetPFrontFromIPaint(IROOMBUFFER),
        MIX_GetPRearFromIPaint(IROOMBUFFER),
        MIX_GetPCenterFromIPaint(IROOMBUFFER), count);
    ...
    if (b_spatial_delays && !g_bDspOff && !DSP_RoomDSPIsOff()) {
      // upgrade mono room buffer to surround status so we can apply spatial
      // delays to all channels
      MIX_ConvertBufferToSurround(IROOMBUFFER);
      g_AudioDevice->ApplyDSPEffects(
          idsp_spatial, MIX_GetPFrontFromIPaint(IROOMBUFFER),
          MIX_GetPRearFromIPaint(IROOMBUFFER),
          MIX_GetPCenterFromIPaint(IROOMBUFFER), count);
    }
    ...
    if (g_bdirectionalfx)  // KDB: perf
    {
      ...
      MIX_MixPaintbuffers(IFACINGBUFFER, IFACINGAWAYBUFFER, IPAINTBUFFER, count,
                          DSP_NOROOM_MIX);

      // Add in dsp room fx to paintbuffer, mix at 75%
      MIX_MixPaintbuffers(IROOMBUFFER, IPAINTBUFFER, IPAINTBUFFER, count,
                          DSP_ROOM_MIX);
    } else {
      // Mix IFACING buffer with IROOMBUFFER
      // (IFACINGAWAYBUFFER contains no data, IFACINGBBUFFER has full dry mix
      // based on distance from listener) if dsp disabled, mix 100%
      // facingbuffer, otherwise, mix 75% facingbuffer + roombuffer
    ...
    if (bIsUnderwater) {
      // BUG: if out of water, previous delays will be heard. must clear dly
      // buffers.
      g_AudioDevice->ApplyDSPEffects(
          idsp_water, MIX_GetPFrontFromIPaint(IPAINTBUFFER),
          MIX_GetPRearFromIPaint(IPAINTBUFFER),
          MIX_GetPCenterFromIPaint(IPAINTBUFFER), count);
    }

    // find dsp gain
    SDEBUG_GetAvgIn(IPAINTBUFFER, count);

    // Apply player fx dsp_player (serial in-line) - does nothing if dsp fx are
    // disabled
    g_AudioDevice->ApplyDSPEffects(
        idsp_player, MIX_GetPFrontFromIPaint(IPAINTBUFFER),
        MIX_GetPRearFromIPaint(IPAINTBUFFER),
        MIX_GetPCenterFromIPaint(IPAINTBUFFER), count);
    ...
    /*
                    // apply left/center/right/lrear/rrear spatial delays to
       paint buffer

                    if ( b_spatial_delays )
                            g_AudioDevice->ApplyDSPEffects( idsp_spatial,
       MIX_GetPFrontFromIPaint(IPAINTBUFFER),
       MIX_GetPRearFromIPaint(IPAINTBUFFER),
       MIX_GetPCenterFromIPaint(IPAINTBUFFER), count );
    */
    // Add dry buffer, set output gain to water * player dsp gain (both 1.0 if
    // not active)

    MIX_MixPaintbuffers(IPAINTBUFFER, IDRYBUFFER, IPAINTBUFFER, count, 1.0);

    // clip all values > 16 bit down to 16 bit
    // NOTE: This is required - the hardware buffer transfer routines no longer
    // perform clipping.
...
// a) if another player in a multiplayer game is speaking, scale all volumes
// down. b) if mixing to IROOMBUFFER, scale all volumes by ch.dspmix and
// dsp_room gain c) if mixing to IFACINGAWAYBUFFER, scale all volumes by
// ch.dspface and dsp_facingaway gain d) If SURROUND_ON, but buffer is not
// surround, recombined front/rear volumes

// returns false if channel is to be entirely skipped.

bool MIX_ScaleChannelVolume(paintbuffer_t *ppaint, channel_t *pChannel,
  ...
  // if dsp is off, or room dsp is off, mix 0% to mono room buffer, 100% to
  // facing buffer

  if (g_bDspOff || DSP_RoomDSPIsOff()) dspmix = 0.0;
  ...
  // If mixing to the room buss, adjust volume based on channel's dspmix
  // setting. dspmix is DSP_MIX_MAX (~0.78) if sound is far from player,
  // DSP_MIX_MIN (~0.24) if sound is near player
  ...
  // If global dsp volume is less than 1, reduce dspmix (ie: increase dry
  // volume) If gloabl dsp volume is greater than 1,  do not reduce dspmix

  if (g_dsp_volume < 1.0) dspmix *= g_dsp_volume;

  // If mixing to facing/facingaway buss, adjust volume based on sound entity's
  // facing direction.

  // If sound directly faces player, ch->dspface = 1.0.  If facing directly
  // away, ch->dspface = -1.0. mix to lowpass buffer if facing away, to allpass
  // if facing
  ...
  // bias scale such that 1.0 to 'cone' is considered facing.  Facing cone
  // narrows as cone -> 1.0 and 'cone' -> 0.0 becomes 1.0 -> 0.0
  ...
  // if ( !g_bdirectionalfx || wavtype == CHAR_DOPPLER || wavtype == CHAR_OMNI
  // || (wavtype == CHAR_DIRECTIONAL && mixchans == 2) )
  if (!g_bdirectionalfx || wavtype != CHAR_DIRECTIONAL) {
    // if no directional fx mix 0% to facingaway buffer
    // if wavtype is DOPPLER, mix 0% to facingaway buffer - DOPPLER wavs have a
    // custom mixer if wavtype is OMNI, mix 0% to facingaway buffer - OMNI wavs
    // have no directionality if wavtype is DIRECTIONAL and stereo encoded, mix
    // 0% to facingaway buffer - DIRECTIONAL STEREO wavs have a custom mixer
...
#define psp_size 8
  ...
  __asm {
    ...
    mov		ecx,count
        ...
        // multiply so for 7 bits, the mask is 0xFE, 6 bits 0xFC, etc. The shift
        // must multiply by the table size.  There are 256 4-uint8_t values in
        // the table at each level. So each index must be shifted left by 10,
        // but since the bits we use are in the MSB rather than LSB they must be
...
// volume array contains appropriate spatialization volumes for doppler left
// (incoming sound)
void SW_Mix8StereoDopplerLeft(portable_samplepair_t *pOutput, int *volume,
                              uint8_t *pData, int inputOffset,
...
// volume array contains appropriate spatialization volumes for doppler right
// (outgoing sound)
void SW_Mix8StereoDopplerRight(portable_samplepair_t *pOutput, int *volume,
                               uint8_t *pData, int inputOffset,
                               fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDopplerLeft(portable_samplepair_t *pOutput, int *volume,
                               short *pData, int inputOffset,
                               fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDopplerRight(portable_samplepair_t *pOutput, int *volume,
                                short *pData, int inputOffset,
                                fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
...
// mix left wav (front facing) with right wav (rear facing) based on soundfacing
// direction
void SW_Mix8StereoDirectional(float soundfacing, portable_samplepair_t *pOutput,
                              int *volume, uint8_t *pData, int inputOffset,
                              fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
  int x;
  ...
  for (int i = 0; i < outCount; i++) {
    ...
    pOutput[i].left +=
        lscale[x & 0xFF];  // multiply by volume and convert to 16 bit
...
// mix left wav (front facing) with right wav (rear facing) based on soundfacing
// direction interpolating pitch shifter - sample(s) from preceding buffer are
// preloaded in pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8StereoDirectional_Interp(float soundfacing,
                                     portable_samplepair_t *pOutput,
                                     int *volume, uint8_t *pData,
                                     int inputOffset, fixedint rateScaleFix,
                                     int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
      rateScaleFix);  // convert 28 bit fixed point to 14 bit fixed point
  ...
  for (int i = 0; i < outCount; i++) {
    // interpolate between first & second sample (the samples bordering
    // sampleFrac12 fraction)
...
void SW_Mix16StereoDirectional(float soundfacing,
                               portable_samplepair_t *pOutput, int *volume,
                               short *pData, int inputOffset,
                               fixedint rateScaleFix, int outCount) {
  fixedint sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
...
void SW_Mix16StereoDirectional_Interp(float soundfacing,
                                      portable_samplepair_t *pOutput,
                                      int *volume, short *pData,
                                      int inputOffset, fixedint rateScaleFix,
                                      int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
...
void SW_Mix8StereoDistVar(float distmix, portable_samplepair_t *pOutput,
                          int *volume, uint8_t *pData, int inputOffset,
                          fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
...
// distance variant wav (left is close, right is far)
// interpolating pitch shifter - sample(s) from preceding buffer are preloaded
// in pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8StereoDistVar_Interp(float distmix, portable_samplepair_t *pOutput,
                                 int *volume, uint8_t *pData, int inputOffset,
                                 fixedint rateScaleFix, int outCount) {
  int x;
...
void SW_Mix16StereoDistVar(float distmix, portable_samplepair_t *pOutput,
                           int *volume, short *pData, int inputOffset,
                           fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
  int x;
...
void SW_Mix16StereoDistVar_Interp(float distmix, portable_samplepair_t *pOutput,
                                  int *volume, short *pData, int inputOffset,
                                  fixedint rateScaleFix, int outCount) {
  int x;
...
void SW_Mix8Mono(portable_samplepair_t *pOutput, int *volume, uint8_t *pData,
                 int inputOffset, fixedint rateScaleFix, int outCount) {
  // Not using pitch shift?
  if (rateScaleFix == FIX(1)) {
    // native code
    SND_PaintChannelFrom8(pOutput, volume, (uint8_t *)pData, outCount);
    return;
  }
...
// interpolating pitch shifter - sample(s) from preceding buffer are preloaded
// in pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8Mono_Interp(portable_samplepair_t *pOutput, int *volume,
                        uint8_t *pData, int inputOffset, fixedint rateScaleFix,
                        int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
      rateScaleFix);  // convert 28 bit fixed point to 14 bit fixed point
  ...
  for (int i = 0; i < outCount; i++) {
    ...
    pOutput[i].left +=
        lscale[interp & 0xFF];  // multiply by volume and convert to 16 bit
...
void SW_Mix8Stereo(portable_samplepair_t *pOutput, int *volume, uint8_t *pData,
                   int inputOffset, fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
  int *lscale, *rscale;
...
// interpolating pitch shifter - sample(s) from preceding buffer are preloaded
// in pData buffer, ensuring we can always provide 'outCount' samples.
void SW_Mix8Stereo_Interp(portable_samplepair_t *pOutput, int *volume,
                          uint8_t *pData, int inputOffset,
                          fixedint rateScaleFix, int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
  ...
  for (int i = 0; i < outCount; i++) {
    ...
    pOutput[i].left +=
        lscale[interpl & 0xFF];  // multiply by volume and convert to 16 bit
    pOutput[i].right += rscale[interpr & 0xFF];
...
void SW_Mix16Mono_Shift(portable_samplepair_t *pOutput, int *volume,
                        short *pData, int inputOffset, fixedint rateScaleFix,
                        int outCount) {
  int vol0 = volume[0];
  int vol1 = volume[1];
...
#else
  // in assembly, you can make this 32.32 instead of 4.28 and use the carry flag
  // instead of masking
...
void SW_Mix16Mono_NoShift(portable_samplepair_t *pOutput, int *volume,
                          short *pData, int outCount) {
  int vol0 = volume[0];
  int vol1 = volume[1];
#if !id386
  for (int i = 0; i < outCount; i++) {
...
void SW_Mix16Mono(portable_samplepair_t *pOutput, int *volume, short *pData,
                  int inputOffset, fixedint rateScaleFix, int outCount) {
  if (rateScaleFix == FIX(1)) {
    SW_Mix16Mono_NoShift(pOutput, volume, pData, outCount);
  } else {
    SW_Mix16Mono_Shift(pOutput, volume, pData, inputOffset, rateScaleFix,
                       outCount);
  }
}
...
void SW_Mix16Mono_Interp(portable_samplepair_t *pOutput, int *volume,
                         short *pData, int inputOffset, fixedint rateScaleFix,
                         int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
...
void SW_Mix16Stereo(portable_samplepair_t *pOutput, int *volume, short *pData,
                    int inputOffset, fixedint rateScaleFix, int outCount) {
  int sampleIndex = 0;
  fixedint sampleFrac = inputOffset;
...
void SW_Mix16Stereo_Interp(portable_samplepair_t *pOutput, int *volume,
                           short *pData, int inputOffset, fixedint rateScaleFix,
                           int outCount) {
  fixedint sampleIndex = 0;
  fixedint rateScaleFix14 = FIX_28TO14(
...
// return true if mixer should use high quality pitch interpolation for this
// sound

bool FUseHighQualityPitch(channel_t *pChannel) {
  // do not use interpolating pitch shifter if:
  // low quality flag set on sound (ie: wave name is prepended with
  // CHAR_FAST_PITCH) or pitch has no fractional part or snd_pitchquality is 0
  if (!snd_pitchquality.GetInt() || pChannel->flags.bfast_pitch) return false;
...
//===============================================================================
void Mix8MonoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput,
                     int *volume, uint8_t *pData, int inputOffset,
                     fixedint rateScaleFix, int outCount) {
  if (FUseHighQualityPitch(pChannel))
    SW_Mix8Mono_Interp(pOutput, volume, pData, inputOffset, rateScaleFix,
                       outCount);
  else
    SW_Mix8Mono(pOutput, volume, pData, inputOffset, rateScaleFix, outCount);
}

void Mix16MonoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput,
                      int *volume, short *pData, int inputOffset,
                      fixedint rateScaleFix, int outCount) {
  if (FUseHighQualityPitch(pChannel))
    SW_Mix16Mono_Interp(pOutput, volume, pData, inputOffset, rateScaleFix,
                        outCount);
  else
...
void Mix8StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput,
                       int *volume, uint8_t *pData, int inputOffset,
                       fixedint rateScaleFix, int outCount) {
  switch (pChannel->wavtype) {
    case CHAR_DOPPLER:
      SW_Mix8StereoDopplerLeft(pOutput, volume, pData, inputOffset,
                               rateScaleFix, outCount);
      SW_Mix8StereoDopplerRight(pOutput, &volume[IFRONT_LEFTD], pData,
                                inputOffset, rateScaleFix, outCount);
      break;

    case CHAR_DIRECTIONAL:
      if (FUseHighQualityPitch(pChannel))
        SW_Mix8StereoDirectional_Interp(pChannel->dspface, pOutput, volume,
                                        pData, inputOffset, rateScaleFix,
                                        outCount);
      else
        SW_Mix8StereoDirectional(pChannel->dspface, pOutput, volume, pData,
                                 inputOffset, rateScaleFix, outCount);
      break;

    case CHAR_DISTVARIANT:
      if (FUseHighQualityPitch(pChannel))
        SW_Mix8StereoDistVar_Interp(pChannel->distmix, pOutput, volume, pData,
                                    inputOffset, rateScaleFix, outCount);
      else
        SW_Mix8StereoDistVar(pChannel->distmix, pOutput, volume, pData,
                             inputOffset, rateScaleFix, outCount);
      break;
    ...
    case CHAR_OMNI:
      ...
      if (FUseHighQualityPitch(pChannel))
        SW_Mix8Stereo_Interp(pOutput, volume, pData, inputOffset, rateScaleFix,
                             outCount);
      else
        SW_Mix8Stereo(pOutput, volume, pData, inputOffset, rateScaleFix,
                      outCount);
      break;
...
void Mix16StereoWavtype(channel_t *pChannel, portable_samplepair_t *pOutput,
                        int *volume, short *pData, int inputOffset,
                        fixedint rateScaleFix, int outCount) {
  switch (pChannel->wavtype) {
    case CHAR_DOPPLER:
      SW_Mix16StereoDopplerLeft(pOutput, volume, pData, inputOffset,
                                rateScaleFix, outCount);
      SW_Mix16StereoDopplerRight(pOutput, &volume[IFRONT_LEFTD], pData,
                                 inputOffset, rateScaleFix, outCount);
      break;

    case CHAR_DIRECTIONAL:
      if (FUseHighQualityPitch(pChannel))
        SW_Mix16StereoDirectional_Interp(pChannel->dspface, pOutput, volume,
                                         pData, inputOffset, rateScaleFix,
                                         outCount);
      else
        SW_Mix16StereoDirectional(pChannel->dspface, pOutput, volume, pData,
                                  inputOffset, rateScaleFix, outCount);
      break;

    case CHAR_DISTVARIANT:
      if (FUseHighQualityPitch(pChannel))
        SW_Mix16StereoDistVar_Interp(pChannel->distmix, pOutput, volume, pData,
                                     inputOffset, rateScaleFix, outCount);
      else
        SW_Mix16StereoDistVar(pChannel->distmix, pOutput, volume, pData,
                              inputOffset, rateScaleFix, outCount);
      break;
    ...
    case CHAR_OMNI:
      ...
      if (FUseHighQualityPitch(pChannel))
        SW_Mix16Stereo_Interp(pOutput, volume, pData, inputOffset, rateScaleFix,
                              outCount);
      else
        SW_Mix16Stereo(pOutput, volume, pData, inputOffset, rateScaleFix,
                       outCount);
      break;
...
static CMouthInfo *GetMouthInfoForChannel(channel_t *pChannel) {
  int mouthentity = pChannel->speakerentity == -1 ? pChannel->soundsource
                                                  : pChannel->speakerentity;

  IClientEntity *pClientEntity = entitylist->GetClientEntity(mouthentity);

  if (!pClientEntity) return NULL;
  ...
  if (SND_IsMouth(pChannel)) {
    ...
    if (pMouth) {
      ...
      if (pChannel->sfx->pSource && pChannel->sfx->pSource->GetSentence()) {
        pMouth->AddSource(pChannel->sfx->pSource,
                          pChannel->flags.m_bIgnorePhonemes);
      }
    }
  }
}
...
void SND_MoveMouth8(channel_t *ch, CAudioSource *pSource, int count) {
  ...
  if (pSource->GetSentence()) {
    ...
    if (idx == UNKNOWN_VOICE_SOURCE) {
      ...
      if (vd) {
        ...
        float elapsed = (float)ch->pMixer->GetSamplePosition() /
                        (float)pSource->SampleRate();
  ...
  if (pMouth->NeedsEnvelope()) {
    int availableSamples = pSource->GetOutputData(
        (void **)&pdata, ch->pMixer->GetSamplePosition(), count, NULL);
...
bool SND_IsRecording() {
  if (cl_movieinfo.IsRecording() && !Con_IsVisible()) return true;
  return false;
}
