...
struct voxword_t {
  ...
  int fKeepCached;   // 1 if this word was already in cache before sentence
                     // referenced it
  int samplefrac;    // if pitch shifting, this is position into wav * 256
  int timecompress;  // % of wave to skip during playback (causes no pitch
                     // shift)
  CSfxTable *sfx;    // name and cache pointer
};

#define CVOXWORDMAX 32
#define CVOXZEROSCANMAX \
  255  // scan up to this many samples for next zero crossing
