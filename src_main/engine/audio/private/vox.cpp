...
//		...
//
//		There must be at least one space between the sentence name and
// the
// sentence. 		Sentences may be separated by one or more lines
// There may be tabs or
// spaces preceding the sentence name 		The sentence must end in a /n or
// /r 		Lines beginning with // are ignored as comments
//
//		Period or comma will insert a pause in the wave unless
//		the period or comma is the last character in the string.
//
//		If first 2 chars of a word are upper case, word volume increased
// by 25%
//
//		If last ch of a word is a number from 0 to 9
//		then word will be pitch-shifted up by 0 to 9, where 0 is a small
// shift 		and 9 is a very high pitch shift.
//
...
//
// When sound is played back in S_StartDynamicSound or s_startstaticsound, we
// detect the !name format and lookup the actual sentence in the sentences array
//
// To play, we parse each word in the sentence, chain the words, and play the
// sentence each word's data is loaded directy from disk and freed right after
// playback.
//===============================================================================
...
// Module Locals
static ch *rgpparseword[CVOXWORDMAX];  // array of pointers to parsed words
static ch voxperiod[] = "_period";     // vocal pause
static ch voxcomma[] = "_comma";       // vocal pause

#define CVOXMAPNAMESMAX 24
static ch *g_rgmapnames[CVOXMAPNAMESMAX];
static int g_cmapnames = 0;

// Sentence file list management
static void VOX_ListClear(void);
static int VOX_ListFileIsLoaded(const ch *psentenceFileName);
static void VOX_ListMarkFileLoaded(const ch *psentenceFileName);
static void VOX_InitAllEntnames(void);

void VOX_LookupMapnames(void);

static ConCommand vox_reload("vox_reload", VOX_Init,
                             "Reload sentences.txt file");

static CUtlVector<unsigned char> g_GroupLRU;
static CUtlVector<ch> g_SentenceFile;
...
struct sentencegroup_t {
 ...
 public:
  short lru;
  const ch *GroupName() const;
  CUtlSymbol GroupNameSymbol() const;
  void SetGroupName(const ch *pName);
  static CUtlSymbol GetSymbol(const ch *pName);
...
const ch *sentencegroup_t::GroupName() const {
  return s_SymbolTable.String(groupname);
}

void sentencegroup_t::SetGroupName(const ch *pName) {
  groupname = s_SymbolTable.AddString(pName);
}

CUtlSymbol sentencegroup_t::GroupNameSymbol() const { return groupname; }

CUtlSymbol sentencegroup_t::GetSymbol(const ch *pName) {
...
struct WordBuf {
  ...
  WordBuf(const WordBuf &src) { Q_strncpy(word, src.word, sizeof(word)); }
...
static void VOX_BuildVirtualNameList(ch *word, CUtlVector<WordBuf> &list);
...
// S_LoadSound
// S_FindName
// It also depends on vstdlib/RandomInt (all other random calls go through
// g_pSoundServices)
...
//			end of the string when it fails rather than NULL.  This
// is useful 			for parsing buffers containing multiple strings
// Input  : *string -
//			scan -
// Output : ch
//-----------------------------------------------------------------------------
ch *ScanForwardUntil(ch *string, ch scan) {
  while (string[0]) {
    if (string[0] == scan) return string;
...
ch **VOX_ParseString(ch *psz) {
  ...
  memset(rgpparseword, 0, sizeof(ch *) * CVOXWORDMAX);
  ...
  while (!fdone && i < CVOXWORDMAX) {
    ...
    c = *pszscan;
    while (c && !IN_CHARACTERSET(nextWord, c)) c = *(++pszscan);
    ...
    if (fdone || !c)
      ...
      if ((c == '.' || c == ',') && *(pszscan + 1) != '\n' &&
        ...
        if (i >= CVOXWORDMAX) break;
      }

      // 0 terminate substring
      *pszscan++ = 0;

      // skip whitespace
      c = *pszscan;
      while (c && IN_CHARACTERSET(skip, c)) c = *(++pszscan);
...
// return substring in szpath 0 terminated
// if '/' not found, return 'vox/'

ch *VOX_GetDirectory(ch *szpath, int maxpath, ch *psz) {
  ch c;
  int cb = 0;
  ch *pszscan = psz + Q_strlen(psz) - 1;
...
//  and 0 out trailing format characters
//  Format:
...
int VOX_ParseWordParams(ch *psz, voxword_t *pvoxword, int fFirst) {
  ch *pszsave = psz;
  ch c;
  ch ct;
  ch sznum[8];
  ...
  *pvoxword = voxwordDefault;
  ...
  // look at next to last ch to see if we have a
  // valid format:

  c = *(psz + strlen(psz) - 1);

  if (c != ')') return 1;  // no formatting, return

  // scan forward to first '('
  CharacterSetBuild(&delimitSet, "()");
  c = *psz;
  while (!IN_CHARACTERSET(delimitSet, c)) c = *(++psz);

  if (c == ')') return 0;  // bogus formatting
  ...
  *psz = 0;
  ...
  while (1) {
    ...
    while (ct && !IN_CHARACTERSET(commandSet, ct)) ct = *(++psz);
...
typedef struct _vox_entname {
  ...
  float timestampseq[CVOXGLOBMAX];  // latest update to this ent global
...
int VOX_LookupEntIndex(int type, SoundSource soundsource, bool fallocnew) {
  ...
  for (i = 0; i < CENTNAMESMAX; i++) {
    if ((g_entnames[i].type == type) &&
        (g_entnames[i].soundsource == soundsource)) {
      g_entnames[i].fdied = false;
      return i;
    }
  }

  if (!fallocnew) return -1;
...
// return static, 0 terminated string

ch *VOX_LookupRndVirtual(ch *pGroupName) {
  // get group index

  int isentenceg = VOX_GroupIndexFromName(pGroupName);

  if (isentenceg < 0) return NULL;

  ch szsentencename[32];

  // get pointer to sentence name within group, using lru

  int isentence =
      VOX_GroupPick(isentenceg, szsentencename, sizeof(szsentencename) - 1);

  if (isentence < 0) return NULL;

  // get pointer to sentence data

  ch *psz = VOX_LookupString(
      szsentencename[0] == '!' ? szsentencename + 1 : szsentencename, NULL);

  // strip trailing whitespace

  if (!psz) return NULL;

  ch *pend = Q_strstr(psz, " ");
  if (pend) *pend = 0;
...
ch *VOX_LookupSentenceByIndex(const ch *pGroupname, int ipick,
  ...
  int isentence = VOX_GroupPickSequential(
      isentenceg, szsentencename, sizeof(szsentencename) - 1, ipick, true);
  ...
  if (pipicknext) *pipicknext = isentence;
...
ch *VOX_LookupNumber(ch *pGroupName, int ipick) {
...
// lookup ent & type, return static, 0 terminated string
  ...
  *pcnew = cnew;
  ...
  switch (cnew) {
    ...
    case 1:  // 1..19,20,30,40,50,60,70,80,90,100,200,300
      *ppszNew =
          pones ? pones : (ptens ? ptens : (phundreds ? phundreds : NULL));
      return;
    case 2:
      if (ptens && pones) {
        *ppszNew = ptens;
...
ConVar snd_vox_globaltimeout(
    "snd_vox_globaltimeout",
    "300");  // n second timeout to reset global vox words
ConVar snd_vox_seqtimeout(
    "snd_vox_seqtimetout",
    "300");  // n second timeout to reset global sequential vox words
ConVar snd_vox_sectimeout("snd_vox_sectimetout",
                          "300");  // n second timeout to reset global sector id
ConVar snd_vox_captiontrace(
    "snd_vox_captiontrace", "0", 0,
...
ch *VOX_LookupSectorVirtual(ch *pGroupname) {
  ...
  if (curtime - g_vox_lastsectorupdate > snd_vox_sectimeout.GetInt()) {
    g_vox_isector++;
    if (g_vox_isector > VOXSECTORMAX) g_vox_isector = 1;
...
ch *VOX_LookupGlobalVirtual(int type, SoundSource soundsource, ch *pGroupName,
  ...
  for (i = 0; i < CENTNAMESMAX; i++) {
    if ((g_entnames[i].type == type)) {
      if (curtime - g_entnames[i].timestamp[iglobal] <=
          snd_vox_globaltimeout.GetInt()) {
  ...
  ch *psz = VOX_LookupRndVirtual(pGroupName);
...
ch *VOX_LookupGlobalSeqVirtual(int type, SoundSource soundsource,
  ...
  for (i = 0; i < CENTNAMESMAX; i++) {
    if ((g_entnames[i].type == type)) {
      if (curtime - g_entnames[i].timestampseq[iglobal] <=
          (snd_vox_seqtimeout.GetInt() / 2.0f)) {
...
Pick_next:
  ...
  ch *psz = VOX_LookupSentenceByIndex(pGroupName, ipick, &ipicknext);
...
void VOX_InsertWords(int ireplace, int cnew, ch *pszNew, ch *pszNew1,
                     ch *pszNew2) {
  if (cnew) {
    ...
    if (cnew == 2 || cnew == 3) rgpparseword[ireplace + 1] = pszNew1;

    if (cnew == 3) rgpparseword[ireplace + 2] = pszNew2;
  }
}
...
void VOX_DeleteWord(int iword) {
  if (iword < 0 || iword >= CVOXWORDMAX) return;
...
void VOX_LookupMapnames() {
  ...
  for (i = 0; i < CVOXMAPNAMESMAX; i++) {
    // step sequentially through group - return ptr to 1st word in each group
    // (map name)
...
int VOX_GetMapNameIndex(const ch *pszmapname) {
  for (int i = 0; i < g_cmapnames; i++) {
    if (Q_strstr(pszmapname, g_rgmapnames[i])) return i;
  }
  return 0;
}
...
inline bool IsVirtualName(const ch *pName) {
...
void VOX_ReplaceVirtualNames(channel_t *pchan) {
  ...
  const ch *pszmap = host_map.GetString();
    ...
    if (IsVirtualName(rgpparseword[i])) {
      ...
      if (iname >= 0) {
        ...
        if (Q_strstr(szparseword, "V_RND")) {
          ...
          VOX_LookupRangeHeadingOrGrid(0, szparseword, pchan, soundsource,
                                       &pszNew, &pszNew1, &pszNew2, &cnew,
                                       true);
        } else if (Q_strstr(szparseword, "V_DIR")) {
          // get heading from ent to player, return pointers to new words
          VOX_LookupRangeHeadingOrGrid(1, szparseword, pchan, soundsource,
                                       &pszNew, &pszNew1, &pszNew2, &cnew,
                                       false);
          ...
          if (ient < 0) {
            // if not found, allocate new ent, give him a name & number, mark as
            // dead
            ch szgroup1[32];
            ch szgroup2[32];
          ...
          VOX_LookupRangeHeadingOrGrid(2, szparseword, pchan, soundsource,
                                       &pszNew, &pszNew1, &pszNew2, &cnew,
                                       true);
        } else if (Q_strstr(szparseword, "V_GRIDY")) {
          // player y position in 10 meter increments
          VOX_LookupRangeHeadingOrGrid(3, szparseword, pchan, soundsource,
                                       &pszNew, &pszNew1, &pszNew2, &cnew,
                                       true);
          ...
          pszNew =
              VOX_LookupGlobalSeqVirtual(chtype, soundsource, szparseword, 0);
          if (pszNew) cnew = 1;
          ...
          pszNew =
              VOX_LookupGlobalSeqVirtual(chtype, soundsource, szparseword, 1);
          if (pszNew) cnew = 1;
          ...
          pszNew =
              VOX_LookupGlobalSeqVirtual(chtype, soundsource, szparseword, 2);
          if (pszNew) cnew = 1;
          ...
          pszNew =
              VOX_LookupGlobalSeqVirtual(chtype, soundsource, szparseword, 3);
          if (pszNew) cnew = 1;
        }
      }
...
void VOX_Precache(IEngineSound *pSoundSystem, int sentenceIndex,
                  const ch *pPathOverride = NULL) {
  voxword_t rgvoxword[CVOXWORDMAX];
  ch buffer[512];
  ch szpath[SOURCE_MAX_PATH];
  ch pathbuffer[SOURCE_MAX_PATH];
  ch *pWords[CVOXWORDMAX];  // array of pointers to parsed words
  ...
  // parse sentence (also inserts 0 terminators between words)
  ...
  for (i = 0; i < count; i++) {
    if (IsVirtualName(pWords[i])) {
      ...
      int c = list.Count();
      for (int j = 0; j < c; ++j) {
        Q_snprintf(pathbuffer, sizeof(pathbuffer), "%s%s.wav", szpath,
                   list[j].word);
...
void VOX_PrecacheSentenceGroup(IEngineSound *pSoundSystem, const ch *pGroupName,
  ...
  for (i = 0; i < g_Sentences.Count(); i++) {
    if (!g_Sentences[i].isPrecached &&
        !Q_strncasecmp(g_Sentences[i].pName, pGroupName, len)) {
...
void VOX_LoadSound(channel_t *pchan, const ch *pszin) {
#ifndef SWDS
  ch buffer[512];
  int i, cword;
  ch pathbuffer[SOURCE_MAX_PATH];
  ...
  while (rgpparseword[i]) {
    ...
    if (VOX_ParseWordParams(rgpparseword[i], &rgvoxword[cword], i == 0)) {
      ...
      // so we don't discard when word is done playing
      rgvoxword[cword].sfx =
          S_FindName(pathbuffer, &(rgvoxword[cword].fKeepCached));
      // JAY: HACKHACK: Keep all sentences cached for now
      rgvoxword[cword].fKeepCached = 1;

      ch captiontoken[128];
      Q_snprintf(captiontoken, sizeof(captiontoken), "S(%s%s) ", szpath,
                 rgpparseword[i]);

      Q_strncat(captionstream, captiontoken, sizeof(captionstream),
                COPY_ALL_CHARACTERS);
  ...
  if (cword) {
    ...
    // if no words, then pchan->pMixer is 0; chan will be released right away.

    pchan->pMixer = CreateSentenceMixer(rgvoxword);
    if (!pchan->pMixer) return;
...
void VOX_AddNumbers(ch *pGroupName, CUtlVector<WordBuf> &list) {
  // construct group name from V_NUMBERS + TYPE
  for (int i = 0; i <= 30; ++i) {
    ch sznumbers[16];
    int glen = Q_strlen(pGroupName);
...
void VOX_AddRndVirtual(ch *pGroupName, CUtlVector<WordBuf> &list) {
  // get group index

  int isentenceg = VOX_GroupIndexFromName(pGroupName);

  if (isentenceg < 0) return;

  ch szsentencename[32];

  ch const *szgroupname = g_SentenceGroups[isentenceg].GroupName();

  // get pointer to sentence name within group, using lru
  for (int snum = 0; snum < g_SentenceGroups[isentenceg].count; ++snum) {
    Q_snprintf(szsentencename, sizeof(szsentencename), "%s%d", szgroupname,
               snum);
...
void VOX_AddMyVirtualWords(int iname, ch *pGroupName, ch chtype,
                           CUtlVector<WordBuf> &list) {
  VOX_AddRndVirtual(pGroupName, list);
}

void VOX_BuildVirtualNameList(ch *word, CUtlVector<WordBuf> &list) {
  // for each word in the sentence, check for V_, if found
  // replace virtual word with saved word or rnd word
...
// Input  : *fn -
//-----------------------------------------------------------------------------
void VOX_Touch(ch const *fn, CUtlDict<int, int> &list) {
...
//			list -
//-----------------------------------------------------------------------------
void VOX_TouchSounds(CUtlDict<int, int> &list, CUtlRBTree<ccpair, int> &ccpairs,
                     bool spewsentences) {
  int i;
  for (i = list.First(); i != list.InvalidIndex(); i = list.Next(i)) {
    ch const *fn = list.GetElementName(i);
  ...
  if (spewsentences) {
    for (i = ccpairs.FirstInorder(); i != ccpairs.InvalidIndex();
         i = ccpairs.NextInorder(i)) {
      ccpair &pair = ccpairs[i];

      Msg("\"%s\"\t\"%s\"\n", pair.token.word, pair.value.word);
    }
...
// link all sounds in sentence, start playing first word.
// return number of words loaded
void VOX_TouchSound(const ch *pszin, CUtlDict<int, int> &filelist,
                    CUtlRBTree<ccpair, int> &ccpairs, bool spewsentences) {
#ifndef SWDS
  ch buffer[512];
  int i, cword;
  ch pathbuffer[SOURCE_MAX_PATH];
    ...
    if (VOX_ParseWordParams(rgpparseword[i], &rgvoxword[cword], i == 0)) {
      ...
      if (!Q_strnicmp(rgpparseword[i], "V_", 2)) {
        ...
        for (int j = 0; j < c; ++j) {
          ...
          ccpair pair;
          Q_snprintf(pair.token.word, sizeof(pair.token.word), "S(%s%s)",
                     szpath, name);
          pair.value.Set(name);

          Q_snprintf(pathbuffer, sizeof(pathbuffer), "%s/sound/%s%s.wav",
                     g_pSoundServices->GetGameDir(), szpath, name);
        ...
        ccpair pair;
        Q_snprintf(pair.token.word, sizeof(pair.token.word), "S(%s%s)", szpath,
                   rgpparseword[i]);
        pair.value.Set(rgpparseword[i]);

        Q_snprintf(pathbuffer, sizeof(pathbuffer), "%s/sound/%s%s.wav",
                   g_pSoundServices->GetGameDir(), szpath, rgpparseword[i]);
  ...
  if (spewsentences) {
    ...
    for (int j = 0; j < rep.Count(); ++j) {
      Q_strncat(outbuf, rep[j].word, sizeof(outbuf), COPY_ALL_CHARACTERS);
      if (j != rep.Count() - 1) {
...
//			{}.  The string is rewritten in place with those
// commands removed.
//
...
// that is 							parsed out
//-----------------------------------------------------------------------------
void VOX_ParseLineCommands(ch *pSentenceData, size_t sentence_data_size,
                           int sentenceIndex) {
  ch tempBuffer[512];
  ch *pNext, *pStart;
  int length, tempBufferPos = 0;

  if (!pSentenceData) return;
  ...
  while (*pSentenceData) {
      ...
      switch (tolower(*pSentenceData)) {
        case 'l':
          ...
          if (!Q_strnicmp(pSentenceData, "len", 3)) {
            ...
            while (*pSentenceData &&
                   (*pSentenceData != '}' && !(*pSentenceData <= 32)))
              pSentenceData++;
          }
          break;
        case 'c':
          ...
          if (!Q_strnicmp(pSentenceData, "closecaption", 12)) {
            ...
            pSentenceData = (ch *)COM_Parse(pSentenceData);
    ...
    while (*pSentenceData && *pSentenceData <= 32) pSentenceData++;
  }
...
//-----------------------------------------------------------------------------
int VOX_GroupAdd(const ch *pSentenceName) {
  int len = strlen(pSentenceName) - 1;

  // group members end in a number
  if (len <= 0 || !isdigit(pSentenceName[len])) return -1;
  ...
  ch *groupName = stack_alloc<ch>(len + 2);
...
//-----------------------------------------------------------------------------
void VOX_GroupClear() { g_SentenceGroups.RemoveAll(); }
#endif
...
void VOX_LRUInit(sentencegroup_t *pGroup) {
  ...
  if (pGroup->count) {
    ...
    for (i = 0; i < pGroup->count; i++) pLRU[i] = (unsigned char)i;
...
//-----------------------------------------------------------------------------
void VOX_AddSentenceWavesToResList() {
  ...
  bool spewsentences =
      CommandLine()->FindParm("-spewsentences") != 0 ? true : false;
  ...
  for (i = 0; i < sentencecount; i++) {
    ...
    if (spewsentences) {
      const ch *psz = VOX_LookupString(pSentence->pName, NULL);
      if (psz) {
        Msg("%s : %s\n", pSentence->pName, psz);
      }
    }
...
//-----------------------------------------------------------------------------
int VOX_GroupIndexFromName(const ch *pGroupName) {
  int i;
  ...
  if (pGroupName) {
    ...
    for (i = 0; i < g_SentenceGroups.Size(); i++) {
      if (symGroupName == g_SentenceGroups[i].GroupNameSymbol()) return i;
    }
  }
...
// Output : const ch * - name pointer
//-----------------------------------------------------------------------------
const ch *VOX_GroupNameFromIndex(int groupIndex) {
...
// ignore lru. pick next sentence from sentence group. Go in order until we hit
// the last sentence, then repeat list if freset is true.  If freset is false,
// then repeat last sentence. ipick is passed in as the requested sentence
// ordinal. ipick 'next' is returned. return of -1 indicates an error.

int VOX_GroupPickSequential(int isentenceg, ch *szfound, int szfoundLen,
                            int ipick, int freset) {
  const ch *szgroupname;
  unsigned char count;

  if (isentenceg < 0 || isentenceg > g_SentenceGroups.Size()) return -1;
...
int VOX_GroupPick(int isentenceg, ch *szfound, int strLen) {
  const ch *szgroupname;
  unsigned char *plru;
  unsigned char i;
  unsigned char count;
...
//-----------------------------------------------------------------------------
int VOX_ListFileIsLoaded(const ch *psentenceFileName) {
  filelist_t *pList = g_pSentenceFileList;
  while (pList) {
    if (!strcmp(psentenceFileName, pList->pFileName)) return true;
...
// Input  : *psentenceFileName -
//-----------------------------------------------------------------------------
void VOX_ListMarkFileLoaded(const ch *psentenceFileName) {
  size_t size{sizeof(filelist_t) + strlen(psentenceFileName) + 1};
  filelist_t *pEntry = (filelist_t *)malloc(size);

  if (pEntry) {
    ch *pName = (ch *)(pEntry + 1);
    Q_strcpy(pName, size - (pName - (ch *)pEntry), psentenceFileName);
...
// This creates a compact copy of the sentence file in memory with only the
// necessary data
void VOX_CompactSentenceFile() {
  ...
  for (i = 0; i < g_Sentences.Count(); i++) {
    ...
    const ch *pData = g_Sentences[i].pName + len;
  ...
  for (i = 0; i < g_Sentences.Count(); i++) {
    ...
    const ch *pData = g_Sentences[i].pName + len;
    int dataLen = Q_strlen(pData) + 1;
    ch *pDest = &g_SentenceFile[totalMem];
...
// Load sentence file into memory, insert 0 terminators to
// delimit sentence name/sentence pairs.  Keep pointer to each
// sentence name so we can search later.

void VOX_ReadSentenceFile(const ch *psentenceFileName) {
  ch c;
  ch *pchlast, *pSentenceData;
  characterset_t whitespace;

  // Have we already loaded this file?
  if (VOX_ListFileIsLoaded(psentenceFileName)) {
    // must touch any sentence wavs again to ensure the map's init path gets the
    // results
  ...
  g_pFileSystem->ReadEx(pFileData,
                        g_pFileSystem->GetOptimalReadSize(file, fileSize),
                        fileSize, file);
  g_pFileSystem->Close(file);
  ...
  while (pch < pchlast) {
    ...
    c = *pch;
    while (pch < pchlast && IN_CHARACTERSET(whitespace, c)) c = *(++pch);

    // YWB:  Fix possible crashes reading past end of file if the last line has
    // only whitespace on it...
    if (!*pch) break;

    // skip entire line if first ch is /
    if (*pch != '/') {
      ...
      // scan forward to first space, insert 0 terminator
      // after sentence name

      c = *pch;
      while (pch < pchlast && c != ' ') c = *(++pch);
    ...
    if (pSentenceData && pSentenceData < pchlast) {
      ...
      VOX_ParseLineCommands(
          pSentenceData, fileSize - (pSentenceData - (ch *)pFileData), index);
    }
  }
...
// Output : int
//-----------------------------------------------------------------------------
int VOX_SentenceCount() { return g_Sentences.Size(); }

float VOX_SentenceLength(int sentence_num) {
  if (sentence_num < 0 || sentence_num > g_Sentences.Size() - 1) return 0.0f;

  return g_Sentences[sentence_num].length;
}

// scan g_Sentences, looking for pszin sentence name
// return pointer to sentence data if found, 0 if not
...
ch *VOX_LookupString(const ch *pSentenceName, int *psentencenum,
  ...
  for (i = 0; i < c; i++) {
    ch const *name = g_Sentences[i].pName;
    ...
    if (!_stricmp(pSentenceName, name)) {
      ...
      return (ch *)(name + Q_strlen(name) + 1);
    }
  }
  return NULL;
}
...
const ch *VOX_SentenceNameFromIndex(int sentencenum) {
  if (sentencenum < g_Sentences.Size()) return g_Sentences[sentencenum].pName;
  return NULL;
}
