...
void EngineTool_OverrideSampleRate(unsigned long &rate);
...
#define TIME_PADDING \
  0.2f  // Time between receiving the first voice packet and actually starting
        // to play the sound. This accounts for frametime differences on the
        // clients and the server.

#define LOCALPLAYERTALKING_TIMEOUT \
  0.2f  // How long it takes for the client to decide the server isn't sending
        // acks of voice data back.

// If this is defined, then the data is converted to 8-bit and sent otherwise
// uncompressed. #define VOICE_SEND_RAW_TEST
...
unsigned long Voice_SamplesPerSec() {
  unsigned long rate = g_VoiceSampleFormat.nSamplesPerSec;
...
ConVar voice_fadeouttime("voice_fadeouttime", "0.1");  // It fades to no sound
                                                       // at the tail end of
                                                       // your voice data when
                                                       // you release the key.

// Debugging cvars.
ConVar voice_profile("voice_profile", "0");
ConVar voice_showchannels("voice_showchannels",
                          "0");  // 1 = list channels
                                 // 2 = show timing info, etc
ConVar voice_showincoming("voice_showincoming",
                          "0");  // show incoming voice data

ConVar voice_enable("voice_enable", "1",
                    FCVAR_ARCHIVE);  // Globally enable or disable voice.

extern ConVar sv_voicecodec;

// Have it force your mixer control settings so waveIn comes from the
// microphone. CD rippers change your waveIn to come from the CD drive
...
// While in tweak mode, you can't hear anything anyone else is saying, and your
// own voice data goes directly to the speakers.
...
extern IVoiceRecord *CreateVoiceRecord_DSound(unsigned long nSamplesPerSec);
...
class CVoiceChannel {
 public:
  ...
  // Called when someone speaks and a new voice channel is setup to hold the
  // data.
  void Init(int nEntity);

 public:
  int m_iEntity;  // Number of the entity speaking on this channel (index into
                  // cl_entities). This is -1 when the channel is unused.

  CSizedCircularBuffer<VOICE_RECEIVE_BUFFER_SIZE>
      m_Buffer;  // Circular buffer containing the voice data.

  // Used for upsampling..
  double m_LastFraction;  // Fraction between m_LastSample and the next sample.
  short m_LastSample;

  bool m_bStarved;  // Set to true when the channel runs out of data for the
                    // mixer. The channel is killed at that point.

  float m_TimePad;  // Set to TIME_PADDING when the first voice packet comes in
                    // from a sender. We add time padding (for frametime
                    // differences) by waiting a certain amount of time before
                    // starting to output the sound.

  IVoiceCodec *m_pVoiceCodec;  // Each channel gets is own IVoiceCodec instance
                               // so the codec can maintain state.
...
void CVoiceChannel::Init(int nEntity) {
  ...
  m_AutoGain.Reset(128, voice_maxgain.GetFloat(), voice_avggain.GetFloat(),
                   voice_scale.GetFloat());
}
...
static ConVar voice_writevoices(
    "voice_writevoices", "0", 0,
...
class CVoiceWriter {
 public:
  ...
  void Flush() {
    for (int i = m_VoiceWriter.FirstInorder();
         i != m_VoiceWriter.InvalidIndex(); i = m_VoiceWriter.NextInorder(i)) {
  ...
  void AddDecompressedData(CVoiceChannel *ch, const uint8_t *data,
                           size_t datalen) {
    if (!voice_writevoices.GetBool()) return;
...
inline void ApplyFadeToSamples(short *pSamples, int nSamples, int fadeOffset,
                               float fadeMul) {
  ...
  if (nSamplesGotten < sampleCount) {
    ...
    if (nSamplesGotten > 0) {
      ...
      if (nAdditionalNeeded > 0) {
        ...
        Assert((nAdditionalNeeded + nSamplesGotten + nSamplesToDuplicate) ==
               wantedSampleCount);
      }
    } else {
      Q_memset(&pCopyBuf[nSamplesGotten], 0,
               nAdditionalNeeded * BYTES_PER_SAMPLE);
    }
...
void Voice_OnAudioSourceShutdown(int iChannel) { Voice_EndChannel(iChannel); }
...
bool Voice_Init(const char *pCodecName) {
  ...
  if (!g_pVoiceRecord) {
    Msg("Unable to initialize DirectSoundCapture. You won't be able to speak "
        "to other players.");
  }
  ...
  if (!g_hVoiceCodecDLL ||
      (createCodecFn = Sys_GetFactory(g_hVoiceCodecDLL)) == NULL ||
      (g_pEncodeCodec = (IVoiceCodec *)createCodecFn(pCodecName, NULL)) ==
          NULL ||
      !g_pEncodeCodec->Init(quality)) {
  ...
  for (int i = 0; i < VOICE_NUM_CHANNELS; i++) {
    ...
    if ((pChannel->m_pVoiceCodec =
             (IVoiceCodec *)createCodecFn(pCodecName, NULL)) == NULL ||
  ...
  if (voice_forcemicrecord.GetInt()) {
    if (g_pMixerControls) g_pMixerControls->SelectMicrophoneForWaveInput();
  }
...
void Voice_Deinit() {
  // This call tends to be expensive and when voice is not enabled it will
  // continually call in here, so avoid the work if possible.
...
bool Voice_GetLoopback() { return !!voice_loopback.GetInt(); }
...
void Voice_UpdateVoiceTweakMode() {
  if (!g_bInTweakMode || !g_pVoiceRecord) return;
...
void Voice_Idle(float frametime) {
    ...
    if (pChannel->m_iEntity != -1) {
      if (pChannel->m_bStarved) {
        ...
        if (oldpad > 0 && pChannel->m_TimePad <= 0) {
          ...
          pChannel->m_nSoundGuid = VoiceSE_StartChannel(
              i, pChannel->m_iEntity, pChannel->m_bProximity,
  ...
  if (nActive == 0) VoiceSE_EndOverdrive();
  ...
  if (voice_showchannels.GetInt() >= 1) {
    for (int i = 0; i < VOICE_NUM_CHANNELS; i++) {
      ...
      if (pChannel->m_iEntity == -1) continue;
  ...
  // Show profiling data?
  if (voice_profile.GetInt()) {
    Msg("Voice - compress: %7.2fu, decompress: %7.2fu, gain: %7.2fu, upsample: "
        "%7.2fu, total: %7.2fu\n",
        g_CompressTime * 1000000.0, g_DecompressTime * 1000000.0,
        g_GainTime * 1000000.0, g_UpsampleTime * 1000000.0,
        (g_CompressTime + g_DecompressTime + g_GainTime + g_UpsampleTime) *
            1000000.0);
...
bool Voice_RecordStart(const char *pUncompressedFile,
  ...
  if (pMicInputFile) {
    int a, b, c;
    ReadWaveFile(pMicInputFile, g_pMicInputFileData, g_nMicInputFileBytes, a, b,
                 c);
...
bool Voice_RecordStop() {
  ...
  if (g_pUncompressedFileData) {
    WriteWaveFile(g_pUncompressedDataFilename, g_pUncompressedFileData,
                  g_nUncompressedDataBytes, g_VoiceSampleFormat.wBitsPerSample,
                  g_VoiceSampleFormat.nChannels, Voice_SamplesPerSec());
  ...
  if (g_pDecompressedFileData) {
    WriteWaveFile(g_pDecompressedDataFilename, g_pDecompressedFileData,
                  g_nDecompressedDataBytes, g_VoiceSampleFormat.wBitsPerSample,
                  g_VoiceSampleFormat.nChannels, Voice_SamplesPerSec());
...
//			already assigned to that entity or picking the least
// recently used 			channel. If the LRU channel is picked,
// it is flushed and all other 			channels are aged.
...
int Voice_AssignChannel(int nEntity, bool bProximity) {
  if (g_bInTweakMode) return VOICE_CHANNEL_IN_TWEAK_MODE;
...
int Voice_GetChannel(int nEntity) {
  for (int i = 0; i < VOICE_NUM_CHANNELS; i++)
    if (g_VoiceChannels[i].m_iEntity == nEntity) return i;

  return VOICE_CHANNEL_ERROR;
}

double UpsampleIntoBuffer(const short *pSrc, int nSrcSamples,
                          CCircularBuffer *pBuffer, double startFraction,
                          double rate) {
  double maxFraction = nSrcSamples - 1;

  while (1) {
    if (startFraction >= maxFraction) break;
...
int Voice_AddIncomingData(int nChannel, const char *pchData, int nCount,
  ...
  // If in tweak mode, we call this during Idle with -1 as the channel, so any
  // channel data from the network gets rejected.
  if (g_bInTweakMode) {
  ...
  if ((pChannel = GetVoiceChannel(nChannel)) == NULL ||
      !pChannel->m_pVoiceCodec) {
    return (0);
  }

  pChannel->m_bStarved = false;  // This only really matters if you call
                                 // Voice_AddIncomingData between the time the
                                 // mixer asks for data and Voice_Idle is
                                 // called.
  ...
  int nDecompressed = pChannel->m_pVoiceCodec->Decompress(
      pchData, nCount, decompressed, sizeof(decompressed));
  ...
  // Upsample into the dest buffer. We could do this in a mixer but it
  // complicates the mixer.
  pChannel->m_LastFraction = UpsampleIntoBuffer(
      (short *)decompressed, nDecompressed, &pChannel->m_Buffer,
  ...
  g_VoiceWriter.AddDecompressedData(pChannel, (const uint8_t *)decompressed,
                                    nDecompressed * 2);
...
int VoiceTweak_StartVoiceTweakMode() {
  ...
  if (g_bInTweakMode) {
    Assert(
        !"VoiceTweak_StartVoiceTweakMode called while already in tweak mode.");
    return 0;
  }
...
IVoiceTweak g_VoiceTweakAPI = {
    VoiceTweak_StartVoiceTweakMode, VoiceTweak_EndVoiceTweakMode,
