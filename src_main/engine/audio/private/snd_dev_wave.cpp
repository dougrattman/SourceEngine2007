...
extern void S_SpatializeChannel(int volume[6], int master_vol,
                                const Vector *psourceDir, float gain,
                                float mono);
...
int CAudioDeviceWave::PaintBegin(float mixAheadTime, int soundtime,
                                 int paintedtime) {
  //  soundtime - total samples that have been played out to hardware at
  //  dmaspeed paintedtime - total samples that have been mixed at speed endtime
  //  - target for samples in mixahead buffer at speed
...
void CAudioDeviceWave::PaintEnd(void) {
  ...
  while (((m_buffersSent - m_buffersCompleted) >> SAMPLE_16BIT_SHIFT) <
         cblocks) {
...
void CAudioDeviceWave::UpdateListener(const Vector &position,
...
void CAudioDeviceWave::ChannelReset(int entnum, int channelIndex,
                                    float distanceMod) {}
...
void CAudioDeviceWave::SpatializeChannel(int volume[CCHANVOLUMES / 2],
                                         int master_vol,
