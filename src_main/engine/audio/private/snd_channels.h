...
#define IFRONT_CENTER0 \
  5  // dummy slot - center channel is mono, but mixers reference volume[1] slot
...
#define IFRONT_CENTERD0 \
  11  // dummy slot - center channel is mono, but mixers reference volume[1]
      // slot
...
//-----------------------------------------------------------------------------
// NOTE: 128bytes.  These are memset to zero at some points.  Do not add
// virtuals without changing that pattern. UNDONE: now 300 bytes...
struct channel_t {
  int guid;      // incremented each time a channel is allocated (to match with
                 // channel free in tools, etc.)
  ...
  // NOTE: never access these fvolume[] elements directly! Use channel helpers
  // in snd_dma.cpp.
  ...
  float fvolume_inc[CCHANVOLUMES];     // volume increment, per frame, moves
                                       // volume[i] to vol_target[i] (per
                                       // spatialization)

  SoundSource soundsource;  // see iclientsound.h for description.
  int entchannel;           // sound channel (CHAN_STREAM, CHAN_VOICE, etc.)
  int speakerentity;  // if a sound is being played through a speaker entity
                      // (e.g., on a monitor,), this is the
                      //  entity upon which to show the lips moving, if the
                      //  sound has sentence data
  ...
  int mixgroups[8];  // sound belongs to these mixgroups: world, actor, player
                     // weapon, explosion etc.
  ...
  float dspmix;   // 0 - 1.0 proportion of dsp to mix with original sound, based
                  // on distance
  float dspface;  // -1.0 - 1.0 (1.0 = facing listener)
  float distmix;  // 0 - 1.0 proportion based on distance from listner (1.0 -
                  // 100% wav right - far)
  float dsp_mix_min;  // for dspmix calculation - set by current preset in
                      // SND_GetDspMix
  float dsp_mix_max;  // for dspmix calculation - set by current preset in
                      // SND_GetDspMix

  float radius;  // Radius of this sound effect (spatialization is different
                 // within the radius)

  float ob_gain;         // gain drop if sound source obscured from listener
  float ob_gain_target;  // target gain while crossfading between ob_gain &
                         // ob_gain_target
  ...
  union {
    ...
    struct {
      bool bUpdatePositions : 1;  // if true, assume sound source can move and
                                  // update according to entity
      bool isSentence : 1;        // true if playing linked sentence
      bool bdry : 1;  // if true, bypass all dsp processing for this sound (ie:
                      // music)
      bool bSpeaker : 1;    // true if sound is playing through in-game speaker
                            // entity.
      bool bstereowav : 1;  // if true, a stereo .wav file is the sample data
                            // source

      bool delayed_start : 1;  // If true, sound had a delay and so same sound
                               // on same channel won't channel steal from it
      bool fromserver : 1;     // for snd_show, networked sounds get colored
                               // differently than local sounds

      bool bfirstpass : 1;  // true if this is first time sound is spatialized
      bool bTraced : 1;  // true if channel was already checked this frame for
                         // obscuring
      bool bfast_pitch : 1;  // true if using low quality pitch (fast, but no
                             // interpolation)

      bool m_bIsFreeingChannel : 1;          // true when inside S_FreeChannel -
                                             // prevents reentrance
      bool m_bCompatibilityAttenuation : 1;  // True when we want to use goldsrc
                                             // compatibility mode for the
                                             // attenuation In that case,
                                             // dist_mul is set to a relatively
                                             // meaningful value in
                                             // StartDynamic/StartStaticSound,
                                             // but we interpret it totally
                                             // differently in SND_GetGain.
      bool m_bShouldPause : 1;  // if true, sound should pause when the game is
                                // paused
      bool m_bIgnorePhonemes : 1;  // if true, we don't want to drive animation
                                   // w/ phoneme data
    } flags;
  };
};
...
class CChannelList {
 public:
  ...
  bool m_quashed[MAX_CHANNELS];  // if true, the channel should be advanced, but
                                 // not mixed, because it's been heuristically
                                 // suppressed
...
inline int CChannelList::Count() { return m_count; }
