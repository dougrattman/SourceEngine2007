...
the_interface CAudioMixerWave : public CAudioMixer {
 public:
  ...
  int MixDataToDevice(IAudioDevice * pDevice, channel_t * pChannel,
                      int sampleCount, int outputRate, int outputOffset);
  int SkipSamples(channel_t * pChannel, int sampleCount, int outputRate,
                  int outputOffset);
  ...
  int GetSampleLoadRequest(double rate, int sampleCount,
