...
// ----------------------------------------------------------------------- //
// CAutoGain is fed samples and figures out a gain to apply to blocks of
// samples.

// Right now, this class applies gain one block behind. The assumption is that
// the blocks are small enough that gain settings for one block will usually be
// right for the next block.

// The ideal way to implement this class would be to have a delay the size of a
// block so it can apply the right gain to the actual block it was calculated
// for.
// ----------------------------------------------------------------------- //
class CAutoGain {
 public:
  ...
  // maxGain and avgToMaxVal are used to derive the gain amount for each block
  // of samples. All samples are scaled by scale.
 ...
 private:
  ...
  AGFixed
      m_GainMultiplier;  // (m_NextGain - m_CurrentGain) / (m_BlockSize - 1).
};
