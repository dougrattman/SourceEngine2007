...
bool CVBRHeader::IsVBRHeaderAvailable(CMPAFile* pMPAFile,
                                      VBRHeaderType& HeaderType,
                                      DWORD& dwOffset) {
  Assert(pMPAFile);

  // where does VBR header begin (XING)
  DWORD dwNewOffset = dwOffset +
                      m_dwXINGOffsets[pMPAFile->m_pMPAHeader->IsLSF()]
                                     [pMPAFile->m_pMPAHeader->IsMono()];
...
CVBRHeader::CVBRHeader(CMPAFile* pMPAFile, VBRHeaderType HeaderType,
                       DWORD dwOffset)
    : m_pMPAFile(pMPAFile),
      m_pnToc(NULL),
  ...
  switch (m_HeaderType) {
    case NoHeader:
      // no Header found
      throw CMPAException(CMPAException::NoVBRHeader, pMPAFile->GetFilename(),
                          NULL, false);
      break;
    case XINGHeader:
  ...
  if (m_dwBytes > 0 && m_dwFrames > 0) {
    ...
    m_dwBytesPerSec =
        m_pMPAFile->m_pMPAHeader->GetBytesPerSecond(m_dwFrames, m_dwBytes);
  } else  // incomplete header found
  {
    throw CMPAException(CMPAException::IncompleteVBRHeader,
                        pMPAFile->GetFilename(), NULL, false);
  }
}
...
bool CVBRHeader::CheckXING(CMPAFile* pMPAFile, DWORD& dwOffset) {
  // XING ID found?
  if (!CheckID(pMPAFile, 'X', 'i', 'n', 'g', dwOffset) &&
      !CheckID(pMPAFile, 'I', 'n', 'f', 'o', dwOffset))
    return false;
  return true;
}
...
bool CVBRHeader::CheckVBRI(CMPAFile* pMPAFile, DWORD& dwOffset) {
  // VBRI ID found?
  if (!CheckID(pMPAFile, 'V', 'B', 'R', 'I', dwOffset)) return false;
  return true;
}
...
bool CVBRHeader::ExtractLAMETag(DWORD dwOffset) {
  // LAME ID found?
  if (!CheckID(m_pMPAFile, 'L', 'A', 'M', 'E', dwOffset) &&
      !CheckID(m_pMPAFile, 'G', 'O', 'G', 'O', dwOffset))
    return false;
...
bool CVBRHeader::ExtractXINGHeader(DWORD dwOffset) {
  ...
  */
  if (!CheckXING(m_pMPAFile, dwOffset)) return false;
  ...
  if (dwFlags & FRAMES_FLAG) m_dwFrames = m_pMPAFile->ExtractBytes(dwOffset, 4);

  // extract total number of bytes in file
  if (dwFlags & BYTES_FLAG) m_dwBytes = m_pMPAFile->ExtractBytes(dwOffset, 4);
...
bool CVBRHeader::ExtractVBRIHeader(DWORD dwOffset) {
  ...
  */
  ...
  if (!CheckVBRI(m_pMPAFile, dwOffset)) return false;
...
DWORD CVBRHeader::SeekPointVBRI(float fPercent) const {
  return SeekPointByTimeVBRI(
      (fPercent / 100.0f) *
      m_pMPAFile->m_pMPAHeader->GetLengthSecond(m_dwFrames) * 1000.0f);
}
...
DWORD CVBRHeader::SeekPointByTimeVBRI(float fEntryTimeMS) const {
  ...
  fLengthMS =
      (float)m_pMPAFile->m_pMPAHeader->GetLengthSecond(m_dwFrames) * 1000.0f;
  fLengthMSPerTOCEntry = fLengthMS / (float)m_dwTableSize;

  if (fEntryTimeMS > fLengthMS) fEntryTimeMS = fLengthMS;
  ...
  fraction =
      ((int)((((fAccumulatedTimeMS - fEntryTimeMS) / fLengthMSPerTOCEntry) +
              (1.0f / (2.0f * (float)m_dwFramesPerEntry))) *
