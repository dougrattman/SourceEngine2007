...
CMPAHeader::CMPAHeader(CMPAFile* pMPAFile, DWORD dwExpectedOffset,
                       bool bSubsequentFrame, bool bReverse)
    : m_pMPAFile(pMPAFile),
      m_dwSyncOffset(dwExpectedOffset),
      m_dwRealFrameSize(0) {
  ...
  while (error != noError) {
    ...
    if (bSubsequentFrame) {
      if (nStep > m_dwTolerance) {
        ...
        throw CMPAException(CMPAException::NoFrameInTolerance,
                            pMPAFile->GetFilename());
      }
    ...
    if (nSyncOffset < 0 ||
        nSyncOffset > (int)((pMPAFile->m_dwEnd - pMPAFile->m_dwBegin) -
                            MPA_HEADER_SIZE) ||
...
// skips first 32kbit/s or lower bitrate frames to estimate bitrate (returns
// true if bitrate is variable)
bool CMPAHeader::SkipEmptyFrames() {
  if (m_dwBitrate > 32) return false;
  ...
  try {
    while (m_dwBitrate <= 32) {
      ...
      dwHeader =
          m_pMPAFile->ExtractBytes(m_dwSyncOffset, MPA_HEADER_SIZE, false);

      if (IsSync(dwHeader, false) != noError) return false;
    }
...
CMPAHeader::HeaderError CMPAHeader::DecodeHeader(DWORD dwHeader,
                                                 bool bSimpleDecode) {
  ...
  // get MPEG version
  m_Version =
      (MPAVersion)((dwHeader >> 19) & 0x03);  // mask only the rightmost 2 bits
  if (m_Version == MPEGReserved) return headerCorrupt;
  ...
  if (m_Layer == LayerReserved) return headerCorrupt;
  ...
  m_dwBitrate =
      m_dwBitrates[m_bLSF][m_Layer][bIndex] * 1000;  // convert from kbit to bit
  ...
  m_dwComputedFrameSize =
      (m_dwCoefficients[m_bLSF][m_Layer] * m_dwBitrate / m_dwSamplesPerSec) +
      m_dwPaddingSize;
  ...
  if (!bSimpleDecode) {
    ...
    if (m_Emphasis == EmphReserved) return headerCorrupt;
  }
  return noError;
}
  ...
  if ((dwHeader & 0xFFE00000) == 0xFFE00000) {
    ...
    if (error == noError) {
      ...
      if (bExtended) {
        ...
        DWORD next_dwOffset = m_dwSyncOffset + m_dwComputedFrameSize;
        try {
          CMPAHeader m_SubsequentFrame(m_pMPAFile, next_dwOffset, true);
          ...
          if (Exc.GetErrorID() == CMPAException::NoFrame) {
            if (next_dwOffset + m_pMPAFile->m_dwBegin > m_pMPAFile->m_dwEnd)
              m_dwRealFrameSize =
                  m_pMPAFile->m_dwEnd - m_pMPAFile->m_dwBegin - m_dwSyncOffset;
            else
