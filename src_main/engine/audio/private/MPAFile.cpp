...
LPCTSTR m_szErrors[] = {"Can't open the file.",
...
// 1KB is inital buffersize, each time the buffer needs to be increased it is
// doubled
...
bool CMPAFile::GetNextFrame() {
  DWORD dwOffset =
      m_pMPAHeader->m_dwSyncOffset + m_pMPAHeader->m_dwRealFrameSize;
  try {
    CMPAHeader* pFrame = new CMPAHeader(this, dwOffset, false);

    delete m_pMPAHeader;
    m_pMPAHeader = pFrame;
    if (m_dwFrameNo > 0) m_dwFrameNo++;
  } catch (...) {
    return false;
  }
  return true;
...
CMPAFile::~CMPAFile(void) {
  ...
  // close file
  if (m_bMustReleaseFile) g_pFullFileSystem->Close(m_hFile);
...
void CMPAFile::Open(LPCTSTR szFilename) {
  ...
  // mmioOpen)
  m_hFile = g_pFullFileSystem->Open(
      szFilename, "rb", "GAME");  //::CreateFile( szFilename, GENERIC_READ,
                                  // FILE_SHARE_READ, NULL, OPEN_EXISTING,
                                  // FILE_ATTRIBUTE_NORMAL, NULL );
  if (m_hFile == FILESYSTEM_INVALID_HANDLE) {
    // throw error
    throw CMPAException(CMPAException::ErrOpenFile, szFilename, "CreateFile",
                        true);
  }
}
...
DWORD CMPAFile::Read(LPVOID pData, DWORD dwSize, DWORD dwOffset) {
  ...
  //	throw CMPAException( CMPAException::ErrReadFile, m_szFile,
  //_T("ReadFile"), true );
...
// DWORD (32bit)
DWORD CMPAFile::ExtractBytes(DWORD& dwOffset, DWORD dwNumBytes,
                             bool bMoveOffset) {
  Assert(dwNumBytes > 0);
  ...
  // big endian extract (most significant uint8_t first) (will work on little
  // and big-endian computers)
  DWORD dwNumByteShifts = dwNumBytes - 1;

  for (DWORD n = dwOffset; n < dwOffset + dwNumBytes; n++) {
    dwResult |= m_pBuffer[n] << 8 * dwNumByteShifts--;  // the bit shift will do
                                                        // the correct uint8_t
                                                        // order for you
  }

  if (bMoveOffset) dwOffset += dwNumBytes;
...
void CMPAFile::FillBuffer(DWORD dwOffsetToRead) {
  ...
  // is it big enough?
  if (dwNewBufferSize < dwOffsetToRead) dwNewBufferSize = dwOffsetToRead;
  ...
  DWORD dwBytesRead = Read(m_pBuffer + m_dwBufferSize,
                           dwNewBufferSize - m_dwBufferSize, m_dwBufferSize);
...
float GetMP3Duration_Helper(char const* filename) {
  ...
  try {
    ...
    if (MPAFile.m_dwBytesPerSec != 0) {
      duration = (float)(MPAFile.m_dwEnd - MPAFile.m_dwBegin) /
                 (float)MPAFile.m_dwBytesPerSec;
    }
  } catch (...) {
  }
