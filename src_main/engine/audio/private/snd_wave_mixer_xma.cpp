...
// XMA can only peel a max of 1984 stereo samples per poll request (if
// available). Max is not achievable and degrades based on quality settings,
// stereo, etc, but using these numbers for for calcs. 1984 stereo samples
// should be decoded by xma in 31 msec. 1984 stereo samples at 44.1Khz dictates
// a request every 45 msec. GetOutputData() must be clocked faster than 45 msec
// or samples will not be available. However, the XMA decoder must be serviced
// much faster. It was designed for 5 msec. 15 msec seems to be fast enough for
// XMA to decode enough to keep the smaller buffer sizes satisfied, and have
// slop for +/- 5 msec swings.

// Need at least this amount of decoded pcm samples before mixing can commence.
// This needs to be able to cover the initial mix request, while a new decode
// cycle is in flight.
...
// for decoder input
// xma blocks are fetched from the datacache into one of these hw buffers for
// decoding must be in quantum units of XMA_BLOCK_SIZE
#define XMA_INPUT_BUFFER_SIZE (8 * XMA_BLOCK_SIZE)

// circular staging buffer to drain xma decoder and stage until mixer requests
// must be large enough to hold the slowest expected mixing frame worth of
// samples
...
#define MIX_DECODER_POLLING_LATENCY \
  5  // not faster than 5ms, or decoder will sputter

// diagnostic errors
#define ERROR_IO_DATA_TIMEOUT \
  -1  // async i/o taking too long to deliver xma blocks
#define ERROR_IO_TRUNCATED_BLOCK \
  -2                             // async i/o failed to deliver complete blocks
#define ERROR_IO_NO_XMA_DATA -3  // async i/o failed to deliver any block
#define ERROR_DECODER_TIMEOUT \
  -4                             // decoder taking too long to decode xma blocks
...
class CXMAAllocator {
 public:
  static void *Alloc(int bytes) {
    ...
    return XMemAlloc(
        bytes,
        MAKE_XALLOC_ATTRIBUTES(0, false, TRUE, FALSE, eXALLOCAllocatorId_XAUDIO,
                               XALLOC_PHYSICAL_ALIGNMENT_4K,
                               XALLOC_MEMPROTECT_WRITECOMBINE_LARGE_PAGES,
                               FALSE, XALLOC_MEMTYPE_PHYSICAL));
  }

  static void Free(void *p) {
    XMemFree(p, MAKE_XALLOC_ATTRIBUTES(
                    0, false, TRUE, FALSE, eXALLOCAllocatorId_XAUDIO,
                    XALLOC_PHYSICAL_ALIGNMENT_4K,
                    XALLOC_MEMPROTECT_WRITECOMBINE_LARGE_PAGES, FALSE,
                    XALLOC_MEMTYPE_PHYSICAL));
  }
};

// for XMA decoding, fixed size allocations aligned to 4K from a single physical
// heap
CAlignedMemPool<XMA_INPUT_BUFFER_SIZE, 4096, XMA_PHYSICAL_HEAP_SIZE,
                CXMAAllocator>
    g_XMAMemoryPool;
...
class CAudioMixerWaveXMA : public CAudioMixerWave {
 ...
 private:
  ...
  // output buffer, decoded pcm samples, a staging circular buffer, waiting for
  // mixer requests due to staging nature, contains decoded samples from
...
CON_COMMAND(snd_xma_info, "Spew XMA Info") {
  ...
  for (int hMixer = g_XMAMixerList.Head();
       hMixer != g_XMAMixerList.InvalidIndex();
       hMixer = g_XMAMixerList.Next(hMixer)) {
    CAudioMixerWaveXMA *pXMAMixer = g_XMAMixerList[hMixer];
    Msg("  rate:%5d ch:%1d '%s'\n", pXMAMixer->GetSource()->SampleRate(),
        pXMAMixer->GetSource()->IsStereoWav() ? 2 : 1,
        pXMAMixer->GetSource()->GetFileName());
  }
}

CAudioMixerWaveXMA::CAudioMixerWaveXMA(IWaveData *data,
                                       int initialStreamPosition)
    : CAudioMixerWave(data) {
  ...
  if (initialStreamPosition) {
    m_SamplePosition =
        m_pData->Source().StreamToSamplePosition(initialStreamPosition);
  ...
  if (snd_xma_spew_mixers.GetBool()) {
    Msg("XMA: 0x%8.8x (%2d), Mixer Alloc, '%s'\n", (unsigned int)this,
        g_XMAMixerList.Count(), m_pData->Source().GetFileName());
  }
}
...
void CAudioMixerWaveXMA::Mix(IAudioDevice *pDevice, channel_t *pChannel,
                             void *pData, int outputOffset, int inputOffset,
                             fixedint fracRate, int outCount,
                             int timecompress) {
  if (m_NumChannels == 1) {
...
//-----------------------------------------------------------------------------
// Looping is achieved in two passes to provide a circular view of the linear
// data. Pass1: Clamps a sample request to the end of data. Pass2: Snaps to the
// loop start, and returns the number of samples to discard, could be 0, up to
// the expected loop sample position. Returns the number of samples to discard,
// or 0.
//-----------------------------------------------------------------------------
int CAudioMixerWaveXMA::UpdatePositionForLooping(int *pNumRequestedSamples) {
  ...
  CAudioSourceWave &source =
      reinterpret_cast<CAudioSourceWave &>(m_pData->Source());
  int loopSampleStart =
      source.GetLoopingInfo(NULL, &numLeadingSamples, &numTrailingSamples);
  ...
  // want to split the straddle into two regions, due to loops possibly
  // requiring a trailer and leader of discarded samples
  if (numRemainingSamples > 0) {
    ...
    *pNumRequestedSamples = std::min(*pNumRequestedSamples, numRemainingSamples);
    ...
    // the position will be correct AFTER discarding decoded trailing and
    // leading samples
...
// An XMA file is thus treated as a series of fixed size large buffers (multiple
// xma blocks), which are streamed in sequentially. The XMA buffers may be
// delayed from the audio data cache due to async i/o latency. Returns < 0 if
// error, 0 if no decode started, 1 if decode submitted.
//-----------------------------------------------------------------------------
int CAudioMixerWaveXMA::GetXMABlocksAndSubmitToDecoder(bool bDecoderIsLocked) {
  ...
  if (!XMAPlaybackQueryReadyForMoreData(m_pXMAPlayback, 0) ||
      XMAPlaybackQueryInputDataPending(m_pXMAPlayback, 0, pXMABuffer)) {
  ...
  while (total < bufferSize) {
    int available = m_pData->ReadSourceData((void **)&pData, m_DataOffset,
                                            bufferSize - total, NULL);
    if (!available) break;
  ...
  // this is used as a cheap method for save/restore because an XMA seek table
  // is not available
  ...
  if (snd_xma_spew_decode.GetBool()) {
    Msg("XMA: 0x%8.8x, XMABuffer: 0x%8.8x, BufferSize: %d, NextDataOffset: %d, "
        "%s\n",
        (unsigned int)this, pXMABuffer, bufferSize, m_DataOffset,
...
int CAudioMixerWaveXMA::ServiceXMADecoder(bool bForceUpdate) {
  ...
  if (!bForceUpdate &&
      (Plat_MSTime() - m_LastPollTime <= MIX_DECODER_POLLING_LATENCY)) {
    return 0;
  }
  ...
  if (dwParseError) {
    if (snd_xma_spew_warnings.GetBool()) {
      Warning("XMA: 0x%8.8x, Decoder Error, Parse: %d, '%s'\n",
              (unsigned int)this, dwParseError,
              m_pData->Source().GetFileName());
    }
  ...
  if (snd_xma_spew_errors.GetBool()) {
    ...
    if (dwError) {
      Warning("XMA: 0x%8.8x, Playback Error: %d, '%s'\n", (unsigned int)this,
              dwError, m_pData->Source().GetFileName());
    }
  }
#endif
  ...
  while (numSamples) {
    ...
    int numSamplesDecoded = XMAPlaybackConsumeDecodedData(
        m_pXMAPlayback, 0, numSamples, (void **)&pPCMData);

    // put into staging buffer, ready for mixer to drain
    m_pPCMSamples->Write(pPCMData,
                         numSamplesDecoded * m_NumChannels * sizeof(short));
  ...
  // the decoder will always finish ahead of the mixer, submit nothing, and the
  // mixer will still be draining
  ...
  m_bFinished = (numNewSamples == 0) && (decodeStatus == 0) &&
                XMAPlaybackIsIdle(m_pXMAPlayback, 0);
...
// Query available number of samples (!numSamplesToCopy && !pData). Returns
// available.
//-----------------------------------------------------------------------------
int CAudioMixerWaveXMA::GetPCMSamples(int numSamplesToCopy, char *pData) {
  int numReadySamples =
      m_pPCMSamples->GetReadAvailable() / (m_NumChannels * sizeof(short));
  ...
  if (numRequestedSamples) {
    if (pData) {
      // copy to caller
      m_pPCMSamples->Read(pData,
                          numRequestedSamples * m_NumChannels * sizeof(short));
      pData += numRequestedSamples * m_NumChannels * sizeof(short);
    } else {
      // flush
      m_pPCMSamples->Advance(numRequestedSamples * m_NumChannels *
                             sizeof(short));
    }
  ...
  if (snd_xma_spew_drain.GetBool()) {
    ...
    Msg("XMA: 0x%8.8x, SamplePosition: %d, Ready: %d, Requested: %d, %s: %d, "
        "Elapsed: %d ms '%s'\n",
        (unsigned int)this, m_SamplePosition, numReadySamples, numSamplesToCopy,
        pOperation, numCopiedSamples, Plat_MSTime() - m_LastDrainTime,
        m_pData->Source().GetFileName());
  }
...
bool CAudioMixerWaveXMA::IsReadyToMix() {
  ...
  if (!m_pXMAPlayback) {
    ...
    if (g_XMAMemoryPool.BytesAllocated() !=
        numBuffers * g_XMAMemoryPool.ChunkSize()) {
      ...
      xmaPlaybackInit.outputBufferSizeInSamples =
          (m_NumChannels == 2) ? XMA_STEREO_OUTPUT_BUFFER_SAMPLES
                               : XMA_MONO_OUTPUT_BUFFER_SAMPLES;
      XMAPlaybackCreate(1, &xmaPlaybackInit, 0, &m_pXMAPlayback);

      int stagingSize = PCM_STAGING_BUFFER_TIME * m_SampleRate * m_NumChannels *
                        sizeof(short) * 0.001f;
  ...
  // can't mix until we have a minimum threshold of data or the decoder is
  // finished
  ...
  if (numReadySamples > minSamplesNeeded) {
    ...
    if (snd_xma_spew_startup.GetBool()) {
      Msg("XMA: 0x%8.8x, Startup Latency: %d ms, Samples Ready: %d, '%s'\n",
          (unsigned int)this, Plat_MSTime() - m_StartTime, numReadySamples,
          m_pData->Source().GetFileName());
    }
    return true;
  }
...
bool CAudioMixerWaveXMA::ShouldContinueMixing() {
  if (!IsRetail() && m_Error && snd_xma_spew_warnings.GetBool()) {
    ...
    Warning(
        "XMA: 0x%8.8x, Mixer Aborted: %s, SamplePosition: %d/%d, DataOffset: "
        "%d/%d, '%s'\n",
        (unsigned int)this, pErrorString, m_SamplePosition, m_SampleCount,
        m_DataOffset, m_TotalBytes, m_pData->Source().GetFileName());
  }
...
// If no samples can be fetched, returns 0, which hints the mixer to a pending
// shutdown state. This routines operates in large buffer quantums, and nothing
// smaller. XMA decode performance severly degrades if the lock is too frequent.
//-----------------------------------------------------------------------------
int CAudioMixerWaveXMA::GetOutputData(void **pData, int numSamplesToCopy,
                                      char copyBuf[AUDIOSOURCE_COPYBUF_SIZE]) {
  if (m_Error) {
  ...
  if (numCopiedSamples) {
    ...
    *pData = (void *)copyBuf;
...
#ifdef DEBUG_XMA
    if (snd_xma_record.GetBool()) {
      WaveAppendTmpFile("debug.wav", copyBuf, 16,
                        numCopiedSamples * m_NumChannels);
    ...
    if (!m_bFinished && numRequestedSamples) {
      // XMA latency error occurs when decoder not finished (not at EOF) and
      // caller wanted samples but can't get any
      if (snd_xma_spew_warnings.GetInt()) {
        Warning(
            "XMA: 0x%8.8x, No Decoded Data Ready: %d samples needed, '%s'\n",
            (unsigned int)this, numSamplesToCopy,
            m_pData->Source().GetFileName());
      }
      m_Error = ERROR_XMA_NO_PCM_DATA;
    }
...
//			NOTE: In most cases, only call this once, and call it
//before playing 			any data.
...
int CAudioMixerWaveXMA::GetPositionForSave() {
  if (m_bLooped) {
    // A looped sample cannot be saved/restored because the decoded sample
    // position, which is needed for loop calc, cannot ever be correctly
    // restored without the XMA seek table.
    return 0;
  }
