...
class CMPAHeader {
 public:
  CMPAHeader(CMPAFile* pMPAFile, DWORD dwExpectedOffset = 0,
             bool bSubsequentFrame = false, bool bReverse = false);
  ~CMPAHeader();
  ...
  DWORD GetLengthSecond(DWORD dwNumFrames) const {
    return dwNumFrames * m_dwSamplesPerFrame / m_dwSamplesPerSec;
  };
  DWORD GetBytesPerSecond(DWORD dwNumFrames, DWORD dwNumBytes) const {
 ...
 public:
  ...
  enum MPAVersion { MPEG25 = 0, MPEGReserved, MPEG2, MPEG1 } m_Version;

  enum MPALayer { Layer1, Layer2, Layer3, LayerReserved } m_Layer;
 ...
 private:
  enum HeaderError { noError, noSync, freeBitrate, headerCorrupt };
