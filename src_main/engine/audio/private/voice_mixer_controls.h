...
the_interface IMixerControls {
 ...
 public:
  enum Control {
    // Microphone boost is a boolean switch that sound cards support which
    // boosts the input signal by about +20dB. If this isn't on, the mic is
    // usually way too quiet.
    MicBoost = 0,

    // Volume values are 0-1.
    MicVolume,

    // Mic playback muting. You usually want this set to false, otherwise the
    // sound card echoes whatever you say into the mic.
    MicMute,
  ...
  // rather than the microphone. This should be called at startup to set it
  // back.
