...
CAudioMixerWaveADPCM::CAudioMixerWaveADPCM(IWaveData *data)
  ...
  if (m_pFormat) {
    ...
    m_pSamples =
        new short[m_pFormat->wSamplesPerBlock * m_pFormat->wfx.nChannels];

    // number of bytes for samples
    m_blockSize =
        ((m_pFormat->wSamplesPerBlock - 2) * m_pFormat->wfx.nChannels) / 2;
...
void CAudioMixerWaveADPCM::Mix(IAudioDevice *pDevice, channel_t *pChannel,
...
static int error_sign_lut[] = {0,  1,  2,  3,  4,  5,  6,  7,
                               -8, -7, -6, -5, -4, -3, -2, -1};
...
bool CAudioMixerWaveADPCM::DecodeBlock(void) {
  ...
  // get requested data
  int available =
      m_pData->ReadSourceData((void **)(&pData), m_offset, blockSize, NULL);
  if (available < blockSize) {
    ...
    while (available && total < blockSize) {
      ...
      total += available;
      available = m_pData->ReadSourceData((void **)(&pData), m_offset + total,
                                          blockSize - total, NULL);
    }
    pData = tmpBlock;
    available = total;
  }
  ...
  // short block?, fixup sample count (2 samples per byte, divided by number of
  // channels per sample set)
...
void CAudioMixerWaveADPCM::SetSampleStart(int newPosition) {
  ...
  if (blockOffset < m_sampleCount) blockOffset = m_sampleCount;
