 ...
 private:
  class CWaveDataVoice : public IWaveData {
   public:
    ...
    virtual bool IsReadyToMix() { return true; }
...
static ConVar voice_overdrivefadetime(
    "voice_overdrivefadetime",
    "0.4");  // How long it takes to fade in and out of the voice overdrive.
...
extern unsigned long Voice_SamplesPerSec();
...
CAudioMixer *CAudioSourceVoice::CreateMixer(int initialStreamPosition) {
  ...
  if (!pVoice) return NULL;

  CAudioMixer *pMixer =
      CreateWaveMixer(pVoice, WAVE_FORMAT_PCM, 1, BYTES_PER_SAMPLE * 8, 0);
  if (!pMixer) {
    delete pVoice;
    return NULL;
  }
...
int CAudioSourceVoice::GetOutputData(void **pData, int samplePosition,
  ...
  // If there weren't enough bytes in the received data channel, pad it with
  // zeros.
  if (nSamplesGotten < sampleCount) {
    memset(&copyBuf[nSamplesGotten], 0,
           (sampleCount - nSamplesGotten) * BYTES_PER_SAMPLE);
...
int CAudioSourceVoice::SampleRate() { return VOICE_OUTPUT_SAMPLE_RATE; }

int CAudioSourceVoice::SampleSize() { return BYTES_PER_SAMPLE; }

int CAudioSourceVoice::SampleCount() { return VOICE_OUTPUT_SAMPLE_RATE; }

void CAudioSourceVoice::ReferenceAdd(CAudioMixer *pMixer) { m_refCount++; }
...
bool CAudioSourceVoice::CanDelete() { return m_refCount == 0; }
...
bool VoiceSE_Init() {
  if (!snd_initialized) return false;
...
void VoiceSE_Idle(float frametime) {
  ...
  percent =
      (float)(-cos(percent * 3.1415926535) * 0.5 + 0.5);  // Smooth it out..
  float voiceOverdrive = 1 + (voice_overdrive.GetFloat() - 1) * percent;
  g_SND_VoiceOverdriveInt = (int)(256 / voiceOverdrive);
}

int VoiceSE_StartChannel(int iChannel,  //! Which channel to start.
                         int iEntity, bool bProximity, int nViewEntityIndex) {
...
void VoiceSE_EndChannel(int iChannel,  //! Which channel to stop.
                        int iEntity) {
  Assert(iChannel >= 0 && iChannel < VOICE_NUM_CHANNELS);
...
void VoiceSE_StartOverdrive() { g_bVoiceOverdriveOn = true; }

void VoiceSE_EndOverdrive() { g_bVoiceOverdriveOn = false; }
