...
#define SNDLVL_TO_DIST_MULT(sndlvl)                   \
  (sndlvl ? ((pow(10.0f, snd_refdb.GetFloat() / 20) / \
              pow(10.0f, (float)sndlvl / 20)) /       \
             snd_refdist.GetFloat())                  \
          : 0)
#define DIST_MULT_TO_SNDLVL(dist_mult)                                \
  (soundlevel_t)(int)(                                                \
      dist_mult ? (20 * log10(pow(10.0f, snd_refdb.GetFloat() / 20) / \
...
int MXR_GetMixGroupListFromDirName(const char *pDirname, uint8_t *pList,
                                   int listMax);
void MXR_GetMixGroupFromSoundsource(channel_t *pchan, SoundSource soundsource,
...
void ChannelSetVolTargets(channel_t *pch, int *pvolumes, int ivol_offset,
                          int cvol);
...
// Only does anything for voice tweak channel so if view entity changes it
// doesn't fade out to zero volume
...
// x360 override to fade out game music when the user is playing music through
// the dashboard
static float g_DashboardMusicMixValue = 1.0f;
static float g_DashboardMusicMixTarget = 1.0f;
const float g_DashboardMusicFadeRate = 0.5f;  // Fades one half full-scale
                                              // volume per second (two seconds
                                              // for complete fadeout)
...
// this is used to enable/disable music playback on x360 when the user selects
// his own soundtrack to play
...
void CActiveChannels::Remove(channel_t *pChannel) {
  if (pChannel->activeIndex == 0) return;
...
void CActiveChannels::Init() { m_count = 0; }
...
float S_ComputeDelayForSoundtime(float soundtime, clocksync_index_t syncIndex) {
  ...
  if (g_ClockSyncArray[syncIndex] == 0) {
    // Put the current time marker one tick back to impose a minimum delay on
    // the first sample this shifts the drift over so the sounds are more likely
    // to delay (rather than skip) over the burst NOTE: The first sound after a
    // sync MUST have a non-zero delay for the delay channel detection logic to
    // work (otherwise we keep resetting the clock)
  ...
  if (gameDeltaTime < 0 || fabs(delay) > 0.200f) {
    // Note that the equations assume a correlation between game time and real
    // time some kind of clock error.  This can happen with large host_timescale
    // or when the framerate hitches drastically (game time is a smaller clamped
    // value wrt real time). The current sync estimate has probably drifted due
    // to this or some other problem, recompute.
...
static CUtlMap<FileNameHandle_t, SfxDictEntry> s_Sounds(
...
ConVar snd_visualize("snd_visualize", "0", FCVAR_CHEAT,
                     "Show sounds location in world");
ConVar snd_pitchquality("snd_pitchquality", "1",
                        FCVAR_ARCHIVE);  // 1) use high quality pitch shifters

// master volume
static ConVar volume("volume", "1.0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
                     "Sound volume", true, 0.0f, true, 1.0f);
// user configurable music volume
ConVar snd_musicvolume("snd_musicvolume", "1.0",
                       FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX, "Music volume", true,
                       0.0f, true, 1.0f);

ConVar snd_mixahead("snd_mixahead", "0.1", FCVAR_ARCHIVE);
ConVar snd_mix_async("snd_mix_async", "0");
#ifdef _DEBUG
static ConCommand snd_mixvol("snd_mixvol", MXR_DebugSetMixGroupVolume,
...
class CResourcePreloadSound : public CResourcePreload {
 public:
  ...
  virtual void PurgeUnreferencedResources() {
    ...
    for (int i = s_Sounds.FirstInorder(); i != s_Sounds.InvalidIndex();
         i = s_Sounds.NextInorder(i)) {
      // the master sound table grows forever
      // remove sound sources from the master sound table that were not in the
      // preload list
...
void S_Startup() {
  ...
  if (!g_AudioDevice || g_AudioDevice == Audio_GetNullDevice()) {
    g_AudioDevice =
        IAudioDevice::AutoDetectInit(CommandLine()->CheckParm("-wavonly") != 0);
    if (!g_AudioDevice) {
...
static ConCommand play("play", S_Play, "Play a sound.",
...
static ConCommand stopsound("stopsound", S_StopAllSoundsC, 0,
                            FCVAR_CHEAT);  // Marked cheat because it gives an
                                           // advantage to players minimising
                                           // ambient noise.
...
void S_Init() {
  ...
  if (CommandLine()->CheckParm("-nosound")) {
    ...
    TRACEINIT(audiosourcecache->Init(host_parms.memsize >> 2),
...
// If not, allocate cache space for wave data, load wave file into temporary
// heap space, and dump/convert file data into cache.
//-----------------------------------------------------------------------------
  ...
  if (!pSfx->pSource) {
    ...
    if (bStream) {
      ...
      if (bUserVox) {
        if (!IsX360()) {
          pSfx->pSource =
              Voice_SetupAudioSource(ch->soundsource, ch->entchannel);
        } else {
          // not supporting
          Assert(0);
        }
...
void S_InternalReloadSound(CSfxTable *sfx) {
  ...
  int nSource = !Q_stricmp(pExt, "mp3") ? CAudioSource::AUDIO_SOURCE_MP3
                                        : CAudioSource::AUDIO_SOURCE_WAV;
  //	audiosourcecache->RebuildCacheEntry( nSource, sfx->IsPrecachedSound(),
  // sfx );
  audiosourcecache->GetInfo(
      nSource, sfx->IsPrecachedSound(),
      sfx);  // Do a size/date check and rebuild the cache entry if necessary.
}
...
// See comments on CL_HandlePureServerWhitelist for details of what we're doing
// here.
void S_ReloadFilesInList(IFileList *pFilesToReload) {
  ...
  S_StopAllSounds(true);
  wavedatacache->Flush();
  audiosourcecache->ForceRecheckDiskInfo();  // Force all cached audio data to
                                             // recheck size/date info next time
                                             // it's accessed.
  ...
  for (int i = s_Sounds.FirstInorder(); i != iLast;
    ...
    // If the file isn't cached in yet, then the filesystem hasn't touched its
...
void S_MarkUISound(CSfxTable *pSfx) { pSfx->m_bIsUISound = true; }

unsigned int RemainingSamples(channel_t *pChannel) {
  if (!pChannel || !pChannel->sfx || !pChannel->sfx->pSource) return 0;
...
Select a channel from the dynamic channel allocation area.  For the given
entity, override any other sound playing on the same channel (see code comments
below for exceptions).
=================
*/
channel_t *SND_StealDynamicChannel(SoundSource soundsource, int entchannel,
                                   const Vector &origin, CSfxTable *sfx) {
  ...
  // first pass to replace sounds on same ent/channel, and search for free or
  // stealable channels otherwise
  for (int ch_idx = 0; ch_idx < MAX_DYNAMIC_CHANNELS; ch_idx++) {
    ...
    if (ch->activeIndex) {
      ...
      if (entchannel != CHAN_AUTO) {
        ...
        if (!ch->flags.delayed_start && ch->soundsource == soundsource &&
            (soundsource != -1) && ch->entchannel == checkChannel)
          return ch;  // always override sound from same entity
      }

      // Never steal the channel of a streaming sound that is currently playing
      // or voice over IP data that is playing or any sound on CHAN_VOICE(
      // acting )
      if (ch->entchannel == CHAN_STREAM || ch->entchannel == CHAN_VOICE)
        continue;

      // don't let monster sounds override player sounds
      if (g_pSoundServices->IsPlayer(ch->soundsource) &&
      ...
      if (ch->sfx == sfx) {
        ...
        if (maxVolume < sameVol ||
            (maxVolume == sameVol && remaining < sameSoundRemaining)) {
  ...
  if (voice_steal.GetInt() > 1 && sameSoundIndex >= 0) {
    // if sounds of this type are normally delayed, then add an extra slot for
    // stealing NOTE: In HL2 these are usually NPC gunshot sounds - and stealing
    // too soon will cut them off early.  This is a safe heuristic to avoid that
    // problem.  There's probably a better long-term solution involving only
    // counting channels that are actually going to play (delay included) at the
    ...
    // don't play more than N copies of the same sound, steal the quietest &
    ...
    if (sameSoundCount >= maxSameSounds) {
      ...
      if (distSqr > 0.0f && ch->origin.DistToSqr(listener_origin) < distSqr &&
          entchannel != CHAN_WEAPON)
        return NULL;

      // Msg("Sound playing %d copies, stole %s (%d)\n", sameSoundCount,
      // ch->sfx->getname(), sameVol );
      return ch;
    }
  }

  // if there's a free channel, just take that one - don't steal
  if (availableChannel >= 0) return &channels[availableChannel];

  // Still haven't found a suitable channel, so choose the one with the least
...
channel_t *SND_PickDynamicChannel(SoundSource soundsource, int entchannel,
                                  const Vector &origin, CSfxTable *sfx) {
  channel_t *pChannel =
      SND_StealDynamicChannel(soundsource, entchannel, origin, sfx);
    ...
    if (pSource) {
      if (pSource->IsLooped()) {
        if (pChannel->soundsource == soundsource &&
            pChannel->entchannel == entchannel && pChannel->sfx == sfx) {
...
void S_SpatializeChannel(int volumes[CCHANVOLUMES / 2], int master_vol,
  ...
  // clear volumes
  for (int i = 0; i < CCHANVOLUMES / 2; i++) volumes[i] = 0;
  ...
  scale = gain * rscale / 2;
  volumes[IFRONT_RIGHT] = (int)(master_vol * scale);
...
bool S_IsMusic(channel_t *pChannel) {
  if (!pChannel->flags.bdry) return false;
  ...
  // Don't save restore looping sounds as you can end up with an entity
  // restarting them again and have
  //  them accumulate, etc.
  if (source->IsLooped()) return false;

  CAudioMixer *pMixer = pChannel->pMixer;
  if (!pMixer) return false;
...
void S_GetCurrentlyPlayingMusic(CUtlVector<musicsave_t> &musiclist) {
  ...
  for (int i = 0; i < list.Count(); i++) {
    ...
    if (!S_IsMusic(pChannel)) continue;
  ...
  if (pSound) {
    ...
    if (IsPC()) {
      ...
      for (int i = 0; i < total_channels; i++) {
        ...
        if (!ch->pMixer || !ch->pMixer->GetSource()) {
          continue;
        }
...
// calculate ammount of sound to be mixed to dsp, based on distance from
// listener

ConVar dsp_dist_min(
    "dsp_dist_min", "0.0",
    FCVAR_DEMO |
        FCVAR_CHEAT);  // range at which sounds are mixed at dsp_mix_min
ConVar dsp_dist_max(
    "dsp_dist_max", "1440.0",
    FCVAR_DEMO |
        FCVAR_CHEAT);  // range at which sounds are mixed at dsp_mix_max

ConVar dsp_mix_min("dsp_mix_min", "0.2",
                   FCVAR_DEMO);  // dsp mix at dsp_dist_min distance "near"
ConVar dsp_mix_max("dsp_mix_max", "0.8",
                   FCVAR_DEMO);  // dsp mix at dsp_dist_max distance "far"
ConVar dsp_db_min("dsp_db_min", "80", FCVAR_DEMO);  // sounds with sndlvl below
                                                    // this get dsp_db_mixdrop %
                                                    // less dsp mix
ConVar dsp_db_mixdrop("dsp_db_mixdrop", "0.5",
                      FCVAR_DEMO);  // sounds with sndlvl below dsp_db_min get
                                    // dsp_db_mixdrop % less mix

float DSP_ROOM_MIX =
    1.0;  // mix volume of dsp_room sounds when added back to 'dry' sounds
float DSP_NOROOM_MIX = 1.0;  // mix volume of facing + facing away sounds. added
                             // to dsp_room_mix sounds

extern ConVar dsp_off;

// returns 0-1.0 dsp mix value.  If sound source is at a range >= DSP_DIST_MAX,
// return a mix value of DSP_MIX_MAX.  This mix value is used later to determine
// wet/dry mix ratio of sounds.

// This ramp changes with db level of sound source,  and is set in the dsp room
// presets by room size empirical data: 0.78 is nominal mix for sound 100% at
// far end of room, 0.24 is mix for sound 25% into room
...
float SND_GetDspMix(channel_t *pchannel, int idist) {
  ...
  if (dsp_off.GetInt()) return 0.0;

  // doppler wavs are mixed dry

  if (pchannel->wavtype == CHAR_DOPPLER) return 0.0;
...
float SND_GetFacingDirection(channel_t *pChannel, const QAngle &source_angles) {
  ...
  if (pChannel->wavtype != CHAR_DIRECTIONAL) return 1.0;
  ...
  // dot source forward unit vector with source to listener unit vector to get
  // -1.0 - 1.0 facing. ie: projection of SF onto SL
...
// forward facing vector of the entity playing this sound points in exactly the
// direction of travel of the sound. ie: for bullets or tracers, forward vector
// must point in traceline direction. return true if sound is to be played,
// false if sound cannot be heard (shot away from player)

bool SND_GetClosestPoint(channel_t *pChannel, QAngle &source_angles,
                         Vector &vnearpoint) {
  ...
  if (pChannel->wavtype != CHAR_DOPPLER) return false;
  ...
  if (dSLSF <= 0 && !toolframework->IsToolRecording()) {
    ...
    // unless we're recording in the tool, since we may play back from in front
    // of the source
    return false;
  }
...
void SND_GetDopplerPoints(channel_t *pChannel, QAngle &source_angles,
                          Vector &vnearpoint, Vector &source_doppler_left,
  ...
  // source_doppler_left - location in space to play doppler left wav
  // (incomming) source_doppler_right	- location in space to play doppler
  // right wav (outgoing)
...
ConVar snd_showstart(
    "snd_showstart", "0",
    FCVAR_CHEAT);  // showstart always skips info on player footsteps!
...
float Gain_To_Amplitude(float gain) { return gain * 32768; }

float Amplitude_To_Gain(float amplitude) { return amplitude / 32768; }
...
//
// GAIN = 1/dist * snd_refdist * 10 ^ ( ( SNDLVL - snd_refdb - (dist *
// snd_foliage_db_loss / 1200)) / 20 )
//
//		for gain > SND_GAIN_THRESH, start curve smoothing with
//
// GAIN = 1 - 1 / (Y * GAIN ^ SND_GAIN_POWER)
//
//		 where Y = -1 / ( (SND_GAIN_THRESH ^ SND_GAIN_POWER) *
//(SND_GAIN_THRESH - 1) )
//
...
float SND_FadeToNewGain(channel_t *ch, float gain_new) {
  ...
  // if gain_new is close to existing gain, store new gain into gain & target,
  // return
...
#define SND_TRACE_UPDATE_MAX \
  2  // max of N channels may be checked for obscured source per frame

static int g_snd_trace_count =
    0;  // total tracelines for gain obscuring made this frame
...
bool SND_ChannelOkToTrace(channel_t *ch) {
  ...
  if (g_snd_trace_count >= SND_TRACE_UPDATE_MAX) return false;

  // ok to trace if this sound hasn't yet been traced in this round

  if (ch->flags.bTraced) return false;
...
void SND_ChannelTraceReset() {
  ...
  for (int i = 0; i < total_channels; i++) channels[i].flags.bTraced = false;
}

bool SND_IsLongWave(channel_t *pChannel) {
  CAudioSource *pSource = pChannel->sfx ? pChannel->sfx->pSource : nullptr;
  return pSource && pSource->IsStreaming();
}

ConVar snd_obscured_gain_db(
    "snd_obscured_gain_dB", "-2.70",
    FCVAR_CHEAT);  // dB loss due to obscured sound source
...
float SND_GetGainObscured(channel_t *ch, bool fplayersound, bool flooping,
  ...
  if (!bAttenuated) return 1.0f;

  if (fplayersound) return gain;
  ...
  if (!ch->flags.bfirstpass && !ch->flags.isSentence && !flooping &&
      !SND_IsLongWave(ch)) {
    gain = SND_FadeToNewGain(ch, -1.0);
    return gain;
  }
  ...
  // if long or looping sound, process N channels per frame - set 'processed'
  // flag, clear by cycling through all channels - this maintains a cap on
  // traces per frame
  ...
  if (tr.DidHit() && tr.fraction < 0.99) {
    ...
    for (i = 0; i < 4; i++) endpoints[i] = endpoint;
    ...
    // if src above listener, force 'up' vector to point down - create diagonals
    // up & down

    if (endpoint.z > listener_origin.z + (10 * 12)) vsrc_up.z = -vsrc_up.z;
    ...
    for (count = 0, i = 0; i < 4; i++) {
      // UNDONE: some endpoints are in walls - in this case, trace from the wall
      // hit location
      ...
      if (tr.DidHit() && tr.fraction < 0.99 && !tr.startsolid) {
        count++;  // skip first obscured point: at least 2 points + center
                  // should be obscured to hear db loss
        if (count > 1) gain = gain * dB_To_Gain(snd_gain_db);
      }
    }
  }
...
inline float dB_To_Radius(float db) {
  float radius = SND_RADIUS_MIN + (SND_RADIUS_MAX - SND_RADIUS_MIN) *
                                      (db - SND_DB_MIN) /
                                      (SND_DB_MAX - SND_DB_MIN);
...
float PercentDifference(float a, float b) {
  ...
  if (!(int)a && !(int)b) return 0.0;

  if (!(int)a || !(int)b) return 1.0;
...
void DSP_SetSpatialDelay(int chan, float v) {
  ...
  // assume first processor in dsp_spatial is the modulating delay unit for
  // DSP_ChangePresetValue
...
// use non-feedback delay to stereoize (or make quad, or quad + center) the mono
// dsp_room fx, This simulates the average sum of delays caused by reflections
...
void SND_SetSpatialDelays() {
  ...
  Vector v_dir, v_dir2;
  int chan_max =
      (g_AudioDevice->IsSurround() ? 4 : 2) +
      (g_AudioDevice->IsSurroundCenter() ? 1 : 0);  // 2, 4, 5 channels
  ...
  if (!dsp_spatial.GetInt()) return;

  // if listener has not been updated, do nothing

  if ((listener_origin == vec3_origin) && (listener_forward == vec3_origin) &&
      (listener_right == vec3_origin) && (listener_up == vec3_origin))
    return;

  if (!SND_IsInGame()) return;

  // get time

  double dtime = g_pSoundServices->GetHostTime();

  // compare to previous time - if starting new check - don't check for new room
  // until timer expires

  if (!g_ssp.chan && !g_ssp.cycle) {
    if (fabs(dtime - g_ssp.last_change) < SND_SPATIAL_WAIT) return;
  }

  // cycle through forward, left, rearward vectors, averaging to get left/right
  // delay count[chan][cycle] 0,1 0,2 0,3   1,1 1,2 1,3    2,1 2,2 2,3 ...
  ...
  if (g_ssp.cycle == 3) {
    ...
    // cycle through front left, front right, rear left, rear right, front
    // center delays

    g_ssp.chan++;

    if (g_ssp.chan >= chan_max) g_ssp.chan = 0;
  }
  ...
  switch (g_ssp.chan) {
    ...
    case 0:  // front left: trace max 100' 'cone' to player's left
      if (g_AudioDevice->IsSurround()) {
        ...
        v_dir = g_ssp.cycle ? (g_ssp.cycle == 1 ? -listener_right * 0.5
                                                : listener_forward2d * 0.5)
                            : v_dir;
      } else {
        // 2 speaker case - left
        v_dir = listener_right * -1.0;
        v_dir2 = g_ssp.cycle ? (g_ssp.cycle == 1 ? listener_forward2d * 0.5
                                                 : -listener_forward2d * 0.5)
                             : v_dir;
  ...
  // set new result in dsp_spatial delay params when all delay values have been
  // filled in
  ...
  if (!g_ssp.cycle && !g_ssp.chan) {
    ...
    for (int chan = 0; chan < chan_max; chan++) {
      ...
      v = (g_ssp.dist[chan][0] + g_ssp.dist[chan][1] + g_ssp.dist[chan][2]) /
          3.0;
...
//	a) enabled by setting dsp_room to DSP_AUTOMATIC.  Subsequently,
// dsp_automatic is the actual dsp value for dsp_room. 	b) disabled by setting
// dsp_room to anything else

//	c) while enabled, detection nodes are placed as player moves into a new
// space
//     i.	at each node, a new dsp setting is calculated and dsp_automatic
//     is set to an appropriate preset ii.	new nodes are set when player
//     moves out of sight of previous node iii. moving into line of sight of a
//     detection node causes closest node to player to set dsp_automatic
...
#define DAS_CWALLS \
  20  // # of wall traces to save for calculating room dimensions
#define DAS_ROOM_TRACE_LEN \
  (400.0 * 12.0)  // max size of trace to check for room dimensions

#define DAS_AUTO_WAIT \
  0.25  // wait min of n seconds between dsp_room changes and update checks

#define DAS_WIDTH_MIN \
...
struct das_room_t {
  int dist[DAS_CWALLS];  // distance in units from player to axis aligned and
                         // diagonal walls
  float reflect[DAS_CWALLS];  // acoustic reflectivity per wall
  float skyhits[DAS_CWALLS];  // every sky hit adds 0.1
  Vector hit[DAS_CWALLS];     // location of trace hit on wall - used for
                              // calculating average centers
  ...
  float diffusion;  // 0..1.0 check radius (avg of width_avg) for # of props -
                    // scale diffusion based on # found
  ...
  bool broomready;  // true if all distances are filled in and room is ready to
                    // check
  short
      lowceiling;  // if non-zero, ceiling directly above player if < 112 units
};
...
#define DAS_CNODES \
  40  // keep around last n nodes - must be same as DSP_CAUTO_PRESETS!!!
...
void DAS_DebugDrawTrace(trace_t *ptr, int r, int g, int b, float duration,
                        int imax);
...
void DAS_InitNodes() {
  ...
  for (int i = 0; i < DAS_CNODES; i++) DAS_InitAutoRoom(&(g_das_nodes[i].room));
  ...
  g_das_vec3[8].Init(
      vlu, vlu2,
      vlu / 2.0);  // front right up A x,y,z/2		(IVEC_DIAG_UP)
  g_das_vec3[9].Init(vlu, vlu2, vlu);  // front right up B x,y,z
  g_das_vec3[10].Init(vlu / 2.0, vlu2 / 2.0,
                      vlu);  // front right up C x/2,y/2,z

  g_das_vec3[11].Init(-vlu, -vlu2, vlu / 2.0);  // rear left up A -x,-y,z/2
  g_das_vec3[12].Init(-vlu, -vlu2, vlu);        // rear left up B -x,-y,z
  g_das_vec3[13].Init(-vlu / 2.0, -vlu2 / 2.0,
                      vlu);  // rear left up C -x/2,-y/2,z
...
bool DAS_NodeInRange(das_room_t *proom, das_node_t *pnode) {
  ...
  // player can still see previous room selection point, and it's less than n
  // feet away, then flag this node as visible
...
bool DAS_CheckNextNode(das_room_t *proom) {
  ...
  if (g_das_all_checked) return true;
  ...
  for (j = 0; j < DAS_CNODES; j++) {
    ...
    if (g_das_nodes[i].fused && DAS_NodeInRange(proom, &(g_das_nodes[i]))) {
      ...
      if (DAS_TraceNodeToPlayer(proom, &(g_das_nodes[i]))) goto checknode_exit;
    }
  }
...
checknode_exit:
  ...
  if (g_das_checked_count >= DAS_CNODES) g_das_all_checked = true;

  return g_das_all_checked;
}

int DAS_GetNextNodeIndex() { return g_das_store_next; }
...
void DAS_SetRoomBounds(das_room_t *proom, Vector &hit, bool bheight) {
  ...
  if (!bheight) {
    if (hit.x > maxs.x) maxs.x = hit.x;

    if (hit.x < mins.x) mins.x = hit.x;

    if (hit.z > maxs.z) maxs.z = hit.z;

    if (hit.z < mins.z) mins.z = hit.z;
  }

  if (bheight) {
    if (hit.y > maxs.y) maxs.y = hit.y;

    if (hit.y < mins.y) mins.y = hit.y;
  }
...
// when all walls are updated, calculate max length, width, height,
// reflectivity, sky hit%, room center returns true if room parameters are in
// good location to place a node returns false if room parameters are not in
// good location to place a node note: false occurs if up vector doesn't hit
// sky, but one or more up diagonal vectors do hit sky
...
bool DAS_CalcRoomProps(das_room_t *proom) {
  ...
  // standing just inside a window or doorway. keep looking for better node
  // location

  for (i = IVEC_DIAG_UP; i < IVEC_UP; i++) {
    if (proom->skyhits[i] > 0.0) b_diaghitsky = true;
  }

  if (b_diaghitsky && !(proom->skyhits[IVEC_UP] > 0.0)) return false;
  ...
  if (das_debug.GetInt() == 6) {
    ...
    CDebugOverlay::AddBoxOverlay(orig, absMax, absMin, vec3_angle, 255, 0, 255,
                                 0, 60.0f);
  }
  ...
  for (i = j * 2; i < 4 + (j * 2); i++) sky_pct += proom->skyhits[i];
  ...
  for (i = IVEC_DIAG_UP; i < IVEC_DOWN; i++) {
    if (proom->skyhits[i] > 0.0) proom->bskyabove = true;
  }
...
bool DAS_HitSolid(trace_t *ptr) {
  ...
  if (!ptr->DidHit()) return false;

  // if hit sky, return false (not solid)
  if (ptr->surface.flags & SURF_SKY) return false;
...
bool DAS_HitSky(trace_t *ptr) {
  if (ptr->DidHit() && (ptr->surface.flags & SURF_SKY)) return true;
  if (!ptr->DidHit()) {
    float dz = ptr->endpos.z - ptr->startpos.z;
    if (dz > 200 * 12.0f) return true;
  }
  return false;
}
...
// WARNING: the current settings are optimal for skipping floor and ceiling
// clutter, and for detecting rooms without 'looking' through doors or windows.
// Don't change these cvars for hl2!

void DAS_SetTraceHeight(das_room_t *proom, trace_t *ptrU, trace_t *ptrD) {
  // NOTE: when tracing down through player's box, endpos and startpos are
  // reversed and startsolid and allsolid are true.

  int zup =
      abs(ptrU->endpos.z - ptrU->startpos.z);  // height above player's head
  int zdown = abs(ptrD->endpos.z -
                  ptrD->startpos.z);  // distance to floor from player's head
  int h;
  h = zup + zdown;
  ...
  if (das_debug.GetInt() > 1) {
    ...
    CDebugOverlay::AddLineOverlay(ptrD->endpos, ptrU->endpos, 0, 255, 0, 255,
                                  false, 20);
    ...
    CDebugOverlay::AddBoxOverlay(proom->vplayer, mins, maxs, vec3_angle, 255, 0,
                                 0, 0, 20);

    CDebugOverlay::AddBoxOverlay(ptrU->endpos, mins, maxs, vec3_angle, 0, 255,
                                 0, 0, 20);
    CDebugOverlay::AddBoxOverlay(ptrD->endpos, mins, maxs, vec3_angle, 0, 255,
                                 0, 0, 20);
  }
}
...
bool DAS_StartTraceChecks(das_room_t *proom) {
  ...
  if (trD.DidHit() && abs(trD.endpos.z - trD.startpos.z) > 72) return false;
  ...
  // if down trace hits floor, set trace height, otherwise default is player eye
  // location

  if (DAS_HitSolid(&trD)) DAS_SetTraceHeight(proom, &trU, &trD);
...
void DAS_DebugDrawTrace(trace_t *ptr, int r, int g, int b, float duration,
  ...
  if (das_debug.GetInt() != imax) return;

  CDebugOverlay::AddLineOverlay(ptr->startpos, ptr->endpos, r, g, b, 255, false,
                                duration);
  ...
  CDebugOverlay::AddBoxOverlay(ptr->endpos, mins, maxs, vec3_angle, r, g, b, 0,
                               duration);
}
...
// if scanning for height, bounce a second trace off of ceiling and get dist to
// floor

void DAS_GetSurfaceData(das_room_t *proom, trace_t *ptr,
                        das_surfdata_t *psurfdata) {
  float dist;          // distance to player
  float reflectivity;  // acoustic reflectivity of material on surface
  Vector hit;          // trace hit location
  Vector norm;         // wall normal at hit location
  ...
  // ignore returned normal from 1st bounce, just search straight down from
  // trace hit location
...
// algorithm for detecting approximate size of space around player. Handles
// player in corner & non-axis aligned rooms. also handles player on catwalk or
// player under small bridge/overhang. The goal is to only change the dsp room
// description if the the player moves into a space which is SIGNIFICANTLY
// different from the previously set dsp space.

// save player position. find a point above player's head and trace out from
// here.
...
// a) trace out along front-up (or left-up, back-up, right-up), save hit
// locations b) trace down -z from hit locations c) save max height
...
bool DAS_UpdateRoomSize(das_room_t *proom) {
  ...
  if (proom->broomready) return true;
  ...
  // try less expensive world-only trace first (no props, no ents - just try to
  // hit walls)
  ...
  if (das_debug.GetInt() > 2) {
    ...
    if (DAS_HitSolid(&tr))
      DAS_DebugDrawTrace(&tr, 0, 255, 255, 10,
                         DAS_ScanningForHeight(proom) + 3);
    else
      DAS_DebugDrawTrace(
          &tr, 255, 0, 0, 10,
          DAS_ScanningForHeight(proom) + 3);  // red lines if sky hit or no hit
  }
  ...
  if (DAS_HitSolid(&tr)) DAS_GetSurfaceData(proom, &tr, &surfdata);
  ...
  if (proom->iwall == DAS_CWALLS) {
    ...
    proom->iwall = 0;
    proom->broomready =
        b_good_node_location;  // room ready to check if good node location
 ...
 public:
  ...
  IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    ICollideable *pCollideable =
        g_pEngineTraceClient->GetCollideable(pHandleEntity);

    if (!pCollideable) return ITERATION_CONTINUE;
    ...
    if (das_debug.GetInt() == 5) {
      ...
      CDebugOverlay::AddBoxOverlay(orig, mins, maxs,
                                   pCollideable->GetCollisionAngles(), 255, 0,
                                   255, 0, 60.0f);
    }
...
void DAS_SetDiffusion(das_room_t *proom) {
  // BRJ 7/12/05
  // This was commented out because the y component of proom->room_mins,
  // proom->room_maxs was never being computed, causing a bogus box to be sent
  // to the partition system. The results of this computation (namely the
  // diffusion + ent_count fields of das_room_t) were never being used.
...
void DAS_DisplayRoomDEBUG(das_room_t *proom, bool fnew, float preset) {
  ...
  if (fnew)
    DevMsg(
        "NEW DSP NODE: size:(%.0f,%.0f) height:(%.0f) dif %.4f : refl %.4f : "
        "cobj: %.0f : sky %.0f \n",
        dx, dy, dz, proom->diffusion, refl, count, fsky);

  if (!fnew && preset < 0.0) return;

  if (preset >= 0.0) {
    if (proom == NULL) return;

    DevMsg(
        "DSP PRESET: %.0f size:(%.0f,%.0f) height:(%.0f) dif %.4f : refl %.4f "
        ": cobj: %.0f : sky %.0f \n",
  ...
  CDebugOverlay::AddBoxOverlay(proom->vplayer, mins, maxs, vec3_angle, 0, 0,
                               255, 0, 1000.0f);
...
bool DAS_CheckNewRoom(das_room_t *proom) {
  ...
  // player can't see previous node, determine if this is a good place to lay
  // down a new node.  Get room at last seen node for comparison

  if (g_pdas_last_node) proom_prev = &(g_pdas_last_node->room);
  ...
  if (dist <= DAS_DIST_MIN) return false;
  ...
  if (proom_prev->width_max != 0)
    dw = (float)proom->width_max /
         (float)proom_prev->width_max;  // max width delta

  if (proom_prev->length_max != 0)
    dw2 = (float)proom->length_max /
          (float)proom_prev->length_max;  // max length delta

  if (proom_prev->height_max != 0)
    dh = (float)proom->height_max /
         (float)proom_prev->height_max;  // max height delta
  ...
  if (dw > 1.0) dw = 1.0 / dw;
  if (dw2 > 1.0) dw2 = 1.0 / dw2;
  if (dh > 1.0) dh = 1.0 / dh;
  if (dr > 1.0) dr = 1.0 / dr;

  if ((1.0 - dw) >= DAS_WIDTH_MIN) cchanged++;

  if ((1.0 - dw2) >= DAS_WIDTH_MIN) cchanged++;

  //	if ( (1.0 - dh) >= DAS_WIDTH_MIN )	// don't change room based on
  // height change 		cchanged++;

  // new room only if at least 1 changed

  if (cchanged >= 1) goto check_ret;

  //	if ( (1.0 - dr) >= DAS_REFL_MIN )	// don't change room based on
  // reflectivity change 		goto check_ret;
  ...
  if (proom->bskyabove != proom_prev->bskyabove) goto check_ret;
...
check_ret:
  ...
  if (bnewroom) {
    // if low ceiling detected < 112 units, and max height is > low ceiling
    // height by 20%, discard - no change this detects player in doorway, under
    // pipe or narrow bridge
...
extern int DSP_ConstructPreset(bool bskyabove, int width, int length,
                               int height, float fdiffusion,
                               float freflectivity, float *psurf_refl,
                               int inode, int cnodes);
...
int DAS_GetRoomDSP(das_room_t *proom, int inode) {
  ...
  // fill array of surface reflectivities - for
  // left,right,front,back,ceiling,floor

  for (int i = 0; i < 6; i++) surf_refl[i] = proom->refl_walls[i];

  return DSP_ConstructPreset(bskyabove, width, length, height, fdiffusion,
                             freflectivity, surf_refl, inode, DAS_CNODES);
}

// main entry point: call once per frame to update dsp_automatic
// for automatic room detection.  dsp_room must be set to DSP_AUTOMATIC to
// enable. NOTE: this routine accumulates traceline information over several
// frames - it never traces more than 3 times per call, and normally just once
// per call.
...
void DAS_CheckNewRoomDSP() {
  ...
  if (fabs(dtime - proom->last_dsp_change) < DAS_AUTO_WAIT) return;

  // first, update room size parameters, see if room is ready to check - if room
  // is updated, return true right away

  // 3 traces per frame while accumulating room size info

  for (int i = 0; i < 3; i++) bRoom_ready = DAS_UpdateRoomSize(proom);

  if (!bRoom_ready) return;

  if (!g_bdas_create_new_node) {
    // next, check all nodes for line of sight to player - if all checked,
    ...
    if (dsp_preset != -1) {
      ...
      if (dsp_preset != dsp_room_GetInt()) {
        ...
        if (g_pdas_last_node)
          DAS_DisplayRoomDEBUG(&(g_pdas_last_node->room), false,
                               (float)dsp_preset);
      }
...
// remap contents of volumes[] arrary if sound originates from player, or is
// music, and is 100% 'mono' ie: same volume in all channels

void RemapPlayerOrMusicVols(channel_t *ch, int volumes[CCHANVOLUMES / 2],
                            bool fplayersound, bool fmusicsound, float mono) {
  ...
  if (fplayersound)  // && (ch->bstereowav && ch->wavtype != CHAR_DIRECTIONAL &&
                     // ch->wavtype != CHAR_DISTVARIANT) )
  {
    // NOTE: player sounds also get n% overall volume boost.

    // float vol_dist5[]   = {0.29, 0.29, 0.09, 0.09, 0.63};	// FL, FR, RL,
    // RR, FC - 5 channel (mono source) volume distribution  float vol_dist5st[]
    // =
    ...
    if (ch->flags.bstereowav &&
        (ch->wavtype == CHAR_OMNI || ch->wavtype == CHAR_SPATIALSTEREO ||
         ch->wavtype == 0)) {
      pvol_dist =
          (g_AudioDevice->IsSurroundCenter() ? vol_dist5st : vol_dist4st);
    } else {
...
void SND_Spatialize(channel_t *ch) {
  ...
  if (ch->soundsource == g_pSoundServices->GetViewEntity() &&
    ...
    // sounds coming from listener actually come from a short distance directly
    // in front of listener in tool mode however, the view entity is
    // meaningless, since we're viewing from arbitrary locations in space
    fplayersound = true;
  }
  ...
  si.info.Set(ch->soundsource, ch->entchannel,
              ch->sfx ? ch->sfx->getname() : "", ch->origin, ch->direction,
              ch->master_vol, DIST_MULT_TO_SNDLVL(ch->dist_mult), looping,
              ch->pitch, listener_origin, ch->speakerentity);
  ...
  {
    VPROF_("SoundServices->GetSoundSpatializtion", 2,
           VPROF_BUDGETGROUP_OTHER_SOUND, false, BUDGETFLAG_OTHER);
    fvalidentity =
  ...
  if (fdopplerwav) {
    VPROF_("SND_Spatialize doppler", 2, VPROF_BUDGETGROUP_OTHER_SOUND, false,
           BUDGETFLAG_OTHER);
    ...
    // calculate point of closest approach for CHAR_DOPPLER wavs, replace
    // source_vec
    ...
    SND_GetDopplerPoints(ch, source_angles, vnearpoint, source_doppler_left,
                         source_doppler_right);
  ...
  // calculate dsp mix based on distance to listener & sound level (linear
  // approximation)
  ...
  // for sounds with a radius, spatialize left/right/front/rear evenly within
  // the radius

  if (ch->radius > 0 && dist < ch->radius && !fdopplerwav) {
    float interval = ch->radius * 0.5;
    mono = dist - interval;
    if (mono < 0.0) mono = 0.0;
    mono /= interval;
  ...
  // calculate gain based on distance, atmospheric attenuation, interposed
  // objects perform compression as gain approaches 1.0
  ...
  if (fdopplerwav) {
    ...
    g_AudioDevice->SpatializeChannel(volumes, ch->master_vol, source_vec_DL,
                                     gain, mono);
    ...
    g_AudioDevice->SpatializeChannel(volumes, ch->master_vol, source_vec_DR,
                                     gain, mono);
    ...
    g_AudioDevice->SpatializeChannel(volumes, ch->master_vol, source_vec, gain,
                                     mono);
  ...
  // prevent left/right/front/rear/center volumes from changing too quickly &
  // producing pops
...
void SND_SpatializeFirstFrameNoTrace(channel_t *pChannel) {
  if (snd_defer_trace.GetBool()) {
    ...
    // now reset tracing state to firstpass so the trace gets done on next
    // spatialize
...
int S_AlterChannel(int soundsource, int entchannel, CSfxTable *sfx, int vol,
                   int pitch, int flags) {
  THREAD_LOCK_SOUND();
  int ch_idx;
  ...
  if (TestSoundChar(sfx->getname(), CHAR_SENTENCE)) {
    ...
    for (int i = 0; i < list.Count(); i++) {
      ...
      if (channels[ch_idx].soundsource == soundsource &&
        ...
        if (flags & SND_CHANGE_PITCH) channels[ch_idx].basePitch = pitch;

        if (flags & SND_CHANGE_VOL) channels[ch_idx].master_vol = vol;
...
void S_SetChannelWavtype(channel_t *target_chan, CSfxTable *pSfx) {
  // if 1st or 2nd character of name is CHAR_DRYMIX, sound should be mixed dry
  // with no dsp (ie: music)
...
void S_SetChannelStereo(channel_t *target_chan, CAudioSource *pSource) {
  ...
  if (target_chan->soundsource == g_pSoundServices->GetViewEntity()) return;

  // default wavtype for stereo wavs is OMNI - except for drymix or sounds with
  // 0 attenuation

  if (target_chan->flags.bstereowav && !target_chan->wavtype &&
      !target_chan->flags.bdry && target_chan->dist_mult)
    // target_chan->wavtype = CHAR_DISTVARIANT;
    target_chan->wavtype = CHAR_OMNI;
}
...
// return true if all volumes and target volumes for channel are less/equal to
// 'vol'
...
bool BChannelLowVolume(channel_t *pch, int vol_min) {
  ...
  for (int i = 0; i < CCHANVOLUMES; i++) {
    ...
    if (vol_target > max_target) max_target = vol_target;
  }
...
int ChannelGetMaxVol(channel_t *pch) {
  ...
  for (int i = 0; i < CCHANVOLUMES; i++) {
    if (pch->fvolume[i] > max) max = pch->fvolume[i];
  }
...
// copy current channel volumes into target array, starting at ivol, copying
// cvol entries
...
// S_StartDynamicSound
// =======================================================================
// Start a sound effect for the given entity on the given channel (ie; voice,
// weapon etc). Try to grab a channel out of the 8 dynamic spots available.
// Currently used for looping sounds, streaming sounds, sentences, and regular
// entity sounds. NOTE: volume is 0.0 - 1.0 and attenuation is 0.0 - 1.0 when
// passed in. Pitch changes playback pitch of wave by % above or below 100.
// Ignored if pitch == 100

// NOTE: it's not a good idea to play looping sounds through StartDynamicSound,
// because if the looping sound starts out of range, or is bumped from the
// buffer by another sound it will never be restarted.  Use StartStaticSound
// (pass CHAN_STATIC to EMIT_SOUND or SV_StartSound.
...
int S_StartDynamicSound(StartSoundParams_t &params) {
  ...
  if (!g_AudioDevice || !g_AudioDevice->IsActive()) return 0;
  ...
  if (params.flags & (SND_STOP | SND_CHANGE_VOL | SND_CHANGE_PITCH)) {
    if (S_AlterChannel(params.soundsource, params.entchannel, params.pSfx, vol,
                       params.pitch, params.flags))
      return 0;
  ...
  target_chan = SND_PickDynamicChannel(params.soundsource, params.entchannel,
                                       params.origin, params.pSfx);
  if (!target_chan) return 0;

  int channelIndex = (int)(target_chan - channels);
  g_AudioDevice->ChannelReset(params.soundsource, channelIndex,
                              target_chan->dist_mult);

#ifdef DEBUG_CHANNELS
  {
    char szTmp[128];
    Q_snprintf(szTmp, sizeof(szTmp),
               "Sound %s playing on Dynamic game channel %d\n", sndname,
               IWavstreamOfCh(target_chan));
  ...
  target_chan->flags.bUpdatePositions =
      params.bUpdatePositions && (params.soundsource == 0 ? 0 : 1);

  // reference_dist / (reference_power_level / actual_power_level)
  target_chan->flags.m_bCompatibilityAttenuation =
      SNDLEVEL_IS_COMPATIBILITY_MODE(params.soundlevel);
  if (target_chan->flags.m_bCompatibilityAttenuation) {
    // Translate soundlevel from its 'encoded' value to a real soundlevel that
    // we can use in the sound system.
  ...
  if (bIsSentence) {
    ...
    if (!pSource && !params.pSfx->m_bIsLateLoad) {
      Warning(
          "Failed to load sound \"%s\", file probably missing from "
          "disk/repository\n",
          sndname);
    }
  }
  ...
  if (!target_chan->pMixer) {
    // couldn't load the sound's data, or sentence has 0 words (this is not an
    // error)
  ...
  MXR_GetMixGroupFromSoundsource(target_chan, params.soundsource,
                                 params.soundlevel);
  ...
  if (nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4) {
    ...
    DevMsg(
        "DynamicSound %s : src %d : channel %d : %d dB : vol %.2f : time "
        "%.3f\n",
        sndname, params.soundsource, params.entchannel, params.soundlevel,
        params.fvol, g_pSoundServices->GetHostTime());
    if (nSndShowStart == 2 || nSndShowStart == 5)
      DevMsg(
          "\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : cvol "
          "%1.2f : rvol %1.2f : rlvol %1.2f : rrvol %1.2f\n",
          ch->dspmix, ch->distmix, ch->dspface, ch->fvolume[IFRONT_LEFT],
          ch->fvolume[IFRONT_CENTER], ch->fvolume[IFRONT_RIGHT],
          ch->fvolume[IREAR_LEFT], ch->fvolume[IREAR_RIGHT]);
    if (nSndShowStart == 3)
  ...
  // If a client can't hear a sound when they FIRST receive the StartSound
  // message, the client will never be able to hear that sound. This is so that
  // out of range sounds don't fill the playback buffer.  For streaming sounds,
  // we bypass this optimization.

  if (BChannelLowVolume(target_chan, 0) && !toolframework->IsToolRecording()) {
    // Looping sounds don't use this optimization because they should stick
    // around until they're killed. Also bypass for speech (GetSentence)
    if (!params.pSfx->pSource || (!params.pSfx->pSource->IsLooped() &&
                                  !params.pSfx->pSource->GetSentence())) {
      // if this is long sound, play the whole thing.
      if (!SND_IsLongWave(target_chan)) {
        // DevMsg("S_StartDynamicSound: spatialized to 0 vol & ignored %s",
        // sndname);
        S_FreeChannel(target_chan);
  ...
  target_chan->flags.m_bIgnorePhonemes =
...
Pitch changes playback pitch of wave by % above or below 100.  Ignored if pitch
== 100
...
int S_StartStaticSound(StartSoundParams_t &params) {
  ...
  if (!g_AudioDevice->IsActive()) return 0;
  ...
  if ((params.flags & SND_STOP) || (params.flags & SND_CHANGE_VOL) ||
  ...
  */
  ...
  si.info.Set(params.soundsource, params.entchannel, params.pSfx ? sndname : "",
              params.origin, params.direction, vol, params.soundlevel, looping,
              params.pitch, listener_origin, params.speakerentity);
  ...
  ch = SND_PickStaticChannel(
      params.soundsource,
      params.pSfx);  // Autolooping sounds are always fixed origin(?)
  if (!ch) return 0;
...
#ifdef DEBUG_CHANNELS
  {
    char szTmp[128];
    Q_snprintf(szTmp, sizeof(szTmp),
               "Sound %s playing on Static game channel %d\n", sfxin->name,
               IWavstreamOfCh(ch));
    Plat_DebugString(szTmp);
  }
#endif
  ...
  // never update positions if source entity is 0
  ch->flags.bUpdatePositions =
      params.bUpdatePositions && (params.soundsource == 0 ? 0 : 1);

  ch->master_vol = vol;

  ch->flags.m_bCompatibilityAttenuation =
      SNDLEVEL_IS_COMPATIBILITY_MODE(params.soundlevel);
  if (ch->flags.m_bCompatibilityAttenuation) {
    // Translate soundlevel from its 'encoded' value to a real soundlevel that
    // we can use in the sound system.
  ...
  if (S_IsMusic(ch)) {
    // See if we have "music" of same name playing from "world" which means we
    // save/restored this sound already.  If so,
    ...
    for (int i = 0; i < list.Count(); i++) {
      ...
      if (ch == pChannel) continue;
      if (ch->sfx != pChannel->sfx) continue;
      if (pChannel->soundsource != SOUND_FROM_WORLD) continue;
      if (!S_IsMusic(pChannel)) continue;

      DevMsg(1, "Hooking duplicate restored song track %s\n", sndname);

      // the new channel will have an updated soundsource and probably
      // has an updated pitch or volume since we are receiving this sound
      // message after the sound has started playing (usually a volume change)
  ...
  if (nSndShowStart > 0 && nSndShowStart < 7 && nSndShowStart != 4) {
    DevMsg(
        "StaticSound %s : src %d : channel %d : %d dB : vol %.2f : radius %.0f "
        ": time %.3f\n",
        sndname, params.soundsource, params.entchannel, params.soundlevel,
        params.fvol, flSoundRadius, g_pSoundServices->GetHostTime());
    if (nSndShowStart == 2 || nSndShowStart == 5)
      DevMsg(
          "\t dspmix %1.2f : distmix %1.2f : dspface %1.2f : lvol %1.2f : cvol "
          "%1.2f : rvol %1.2f : rlvol %1.2f : rrvol %1.2f\n",
...
int S_StartSound(StartSoundParams_t &params) {
  if (params.staticsound) {
    ...
    VPROF_("StartDynamicSound", 0, VPROF_BUDGETGROUP_OTHER_SOUND, false,
           BUDGETFLAG_OTHER);
...
inline bool IsChannelLooped(int iChannel) {
  return (channels[iChannel].sfx && channels[iChannel].sfx->pSource &&
...
int S_GetCurrentStaticSounds(SoundInfo_t *pResult, int nSizeResult,
  ...
  for (int i = MAX_DYNAMIC_CHANNELS; i < total_channels && nSpaceRemaining;
       i++) {
    if (channels[i].entchannel == entchannel && channels[i].sfx) {
      pResult->Set(
          channels[i].soundsource, channels[i].entchannel,
          channels[i].sfx->getname(), channels[i].origin, channels[i].direction,
          ((float)channels[i].master_vol / 255.0),
          DIST_MULT_TO_SNDLVL(channels[i].dist_mult), IsChannelLooped(i),
          channels[i].basePitch, listener_origin, channels[i].speakerentity);
      pResult++;
      nSpaceRemaining--;
    }
  }
...
bool S_IsLoopingSoundByGuid(int guid) {
  channel_t *pChannel = S_FindChannelByGuid(guid);
  if (!pChannel || !pChannel->sfx) return false;

  return (pChannel->sfx->pSource->IsLooped());
}

//-----------------------------------------------------------------------------
// Purpose: Note that the guid is preincremented, so we can just return the
// current value as the "last sound" indicator Input  :  - Output : int
//-----------------------------------------------------------------------------
int S_GetGuidForLastSoundEmitted() { return s_nSoundGuid; }
...
float S_GetElapsedTimeByGuid(int guid) {
  ...
  float elapsed = mixer->GetSamplePosition() /
                  (mixer->GetSource()->SampleRate() * pChannel->pitch * 0.01f);
  return elapsed;
}
...
void S_UpdateSoundFade() {
  ...
  // Inside the hold time
    ...
    // Ramp down
    f = (elapsed - (soundfade.fadeouttime + soundfade.holdtime)) /
...
// Global Voice Ducker - enabled in vcd scripts, when characters deliver
// important dialog.  Overrides all other mixer ducking, and ducks all other
// sounds except dialog.
...
static void S_UpdateVoiceDuck(int voiceChannelCount, int voiceChannelMaxVolume,
                              float frametime) {
  ...
  if (voiceChannelCount > 0) {
    ...
    // KB: Change: ducker now active if any character is speaking above
    // threshold volume. KB: Active ducker drops all volumes to volumes *
    // snd_duckvolume

    if (voiceChannelMaxVolume > volume_threshold)
      duckTarget = volume_when_ducked;
  }
  float rate = (duckTarget < g_DuckScale) ? snd_duckerattacktime.GetFloat()
                                          : snd_duckerreleasetime.GetFloat();
  g_DuckScale = Approach(duckTarget, g_DuckScale,
                         frametime * ((1 - volume_when_ducked) / rate));
}
...
void ConvertListenerVectorTo2D(Vector *pvforward, Vector *pvright) {
  ...
  if (angles2d[YAW] >= 360) angles2d[YAW] -= 360;
...
ConVar snd_spatialize_roundrobin("snd_spatialize_roundrobin", "0", FCVAR_NONE,
                                 "Lowend optimization: if nonzero, spatialize "
                                 "only a fraction of sound channels each "
                                 "frame. 1/2^x of channels will be spatialized "
                                 "per frame.");
/*
============
S_Update
...
void S_Update(const AudioState_t *pAudioState) {
  ...
  if (pAudioState) {
    ...
    AngleVectors(pAudioState->m_Angles, &listener_forward, &listener_right,
                 &listener_up);
  ...
  g_AudioDevice->UpdateListener(listener_origin, listener_forward,
                                listener_right, listener_up);
          ...
          // frame.
  {
    ...
    for (int i = 0; i < list.Count(); ++i) {
      ...
      if (ch->flags.bfirstpass ||
          (robinmask & s_roundrobin) == (i & robinmask)) {
        SND_Spatialize(ch);  // respatialize channel
      }
  ...
  g_DashboardMusicMixValue =
      Approach(g_DashboardMusicMixTarget, g_DashboardMusicMixValue,
               g_DashboardMusicFadeRate * frametime);
  ...
  if (snd_show.GetInt()) {
    ...
    for (int i = 0; i < list.Count(); i++) {
      ...
      unsigned int sampleCount = RemainingSamples(ch);
      float timeleft =
          (float)sampleCount / (float)ch->sfx->pSource->SampleRate();
      bool bLooping = ch->sfx->pSource->IsLooped();

      if (snd_surround.GetInt() < 4) {
        Con_NXPrintf(&np,
                     "%02i l(%03d) r(%03d) vol(%03d) ent(%03d) pos(%6d %6d "
                     "%6d) timeleft(%f) looped(%d) %50s",
                     total + 1, (int)ch->fvolume[IFRONT_LEFT],
                     (int)ch->fvolume[IFRONT_RIGHT], ch->master_vol,
                     ch->soundsource, (int)ch->origin[0], (int)ch->origin[1],
                     (int)ch->origin[2], timeleft, bLooping,
                     ch->sfx->getname());
      } else {
        Con_NXPrintf(
            &np,
            "%02i l(%03d) c(%03d) r(%03d) rl(%03d) rr(%03d) vol(%03d) "
            "ent(%03d) pos(%6d %6d %6d) timeleft(%f) looped(%d) %50s",
            total + 1, (int)ch->fvolume[IFRONT_LEFT],
            (int)ch->fvolume[IFRONT_CENTER], (int)ch->fvolume[IFRONT_RIGHT],
            (int)ch->fvolume[IREAR_LEFT], (int)ch->fvolume[IREAR_RIGHT],
            ch->master_vol, ch->soundsource, (int)ch->origin[0],
            (int)ch->origin[1], (int)ch->origin[2], timeleft, bLooping,
            ch->sfx->getname());
      }
  ...
  if (delta > 0 && delta < snd_mix_minframetime.GetFloat()) return;
  // this is the last time we ran a sound frame
  g_LastSoundFrame = tNow;
  // this is the last time we did mixing (extraupdate also advances this if it
  // mixes)
  g_LastMixTime = tNow;
  // mix some sound
  // try to stay at least one frame + mixahead ahead in the mix.
  g_EstFrameTime =
      (g_EstFrameTime * 0.9f) + (g_pSoundServices->GetHostFrametime() * 0.1f);
...
CON_COMMAND(snd_dumpclientsounds, "Dump sounds to VXConsole") {
  ...
  for (int i = 0; i < list.Count(); i++) {
    ...
    Msg("%02i %s l(%03d) c(%03d) r(%03d) rl(%03d) rr(%03d) vol(%03d) pos(%6d "
        "%6d %6d) timeleft(%f) looped(%d) %50s chan:%d ent(%03d):%s\n",
        total + 1, ch->flags.fromserver ? "SERVER" : "CLIENT",
        (int)ch->fvolume[IFRONT_LEFT], (int)ch->fvolume[IFRONT_CENTER],
        (int)ch->fvolume[IFRONT_RIGHT], (int)ch->fvolume[IREAR_LEFT],
        (int)ch->fvolume[IREAR_RIGHT], ch->master_vol, (int)ch->origin[0],
        (int)ch->origin[1], (int)ch->origin[2], timeleft, bLooping,
        ch->sfx->getname(), ch->entchannel, ch->soundsource,
...
void GetSoundTime() {
  ...
  fullsamples =
      g_AudioDevice->DeviceSampleCount() / g_AudioDevice->DeviceChannels();

  // NOTE: it is possible to miscount buffers if it has wrapped twice between
  // calls to S_Update.  However, since the output buffer size is > 1 second of
  // sound, this should only occur for framerates lower than 1hz

  // sampleOutCount is counted in 16 bit *full* samples, of number of samples
  // output to hardware for current output buffer
  ...
  if (cl_movieinfo.IsRecording()) {
    ...
    if (s_lastsoundtime != t) {
      g_soundtime += g_pSoundServices->GetHostFrametime() *
                     g_AudioDevice->DeviceDmaSpeed();
      s_lastsoundtime = t;
    }
  } else {
...
void S_ExtraUpdate() {
  ...
  // listener at 'false' origin causes some sounds to incorrectly spatialize to
  // 0 volume, killing them before they can play.
  ...
  // we know we were at least snd_mixahead seconds ahead of the output the last
  // time we did mixing if we're not close to running out just exit to avoid
  // small mix batches
  if (delta > 0 && delta < (snd_mixahead.GetFloat() * 0.9f)) return;
  g_LastMixTime = tNow;
...
void S_Update_Guts(float mixAheadTime) {
  ...
  unsigned endtime =
      g_AudioDevice->PaintBegin(mixAheadTime, g_soundtime, g_paintedtime);
...
===============================================================================
*/
extern void DSP_DEBUGSetParams(int ipreset, int iproc, float *pvalues,
                               int cparams);
...
void S_SoundList() {
  ...
  for (int i = s_Sounds.FirstInorder(); i != s_Sounds.InvalidIndex();
    ...
    pSource = sfx->pSource;
    if (!pSource || !pSource->IsCached()) continue;
...
CON_COMMAND(vx_soundlist, "Dump sounds to VXConsole") {
  ...
  int i = 0;
  for (int iSrcSound = s_Sounds.FirstInorder();
       iSrcSound != s_Sounds.InvalidIndex();
       iSrcSound = s_Sounds.NextInorder(iSrcSound)) {
    dataSize = -1;
    sampleRate = -1;
    sampleBits = -1;
    pFormatStr = "???";
    ...
    V_strncpy(pSoundList[i].formatName, pFormatStr,
              sizeof(pSoundList[i].formatName));
...
void DEBUG_StartSoundMeasure(int type, int samplecount) {
  if (type != g_snd_profile_type) return;
...
// bring the average down.  If you want an average PER FRAME instead, it's
// generally higher.
...
static void S_Say(const CCommand &args) {
  ...
  if (!TestSoundChar(sound, CHAR_SENTENCE)) {
    ...
    // insert 0 terminator after sentence name
    sound[8] = 0;
...
// All conditions which are not 0 are ANDed together
#define CMXRCLASSMAX 16
#define CMXRNAMEMAX 32

struct classlistelem_t {
  char szclassname[CMXRNAMEMAX];  // name of entities' class, such as
                                  // CAI_BaseNPC or CHL2_Player
};
...
struct grouprule_t {
  ...
  int priority;        // 0..100 higher priority sound groups duck all lower pri
                       // groups if enabled
  int is_ducked;       // if 1, sound group is ducked by all higher priority
                       // 'causes_duck" sounds
  int causes_ducking;  // if 1, sound group ducks other 'is_ducked' sounds of
                       // lower priority
  float duck_target_pct;  // if sound group is ducked, target percent of
                          // original volume

  float total_vol;  // total volume of all sounds in this group, if group can
                    // cause ducking
  float ducker_threshold;  // ducking is caused by this group if total_vol >
                           // ducker_threshold and causes_ducking is enabled.
...
struct soundmixer_t {
  ...
  float mapMixgroupidToValue[CMXRGROUPMAX];  // sparse array of mix group values
                                             // for this soundmixer
};

int g_mapMixgroupidToGrouprulesid[CMXRGROUPMAX];  // map mixgroupid (one per
                                                  // unique group name) back to
                                                  // 1st entry of this name in
                                                  // g_grouprules
...
grouprule_t g_grouprules[CMXRGROUPRULESMAX];  // all rules for determining mix
                                              // group membership
...
ConVar snd_soundmixer("snd_soundmixer",
                      "Default_Mix");  // current soundmixer name
...
ConVar snd_showclassname(
    "snd_showclassname",
    "0");  // if 1, show classname of ent making sound
...
// use current soundmixer indicated with g_isoundmixer, and contents of
// g_rgpgrouprules. Algorithm:
//		1. all conditions in a row are AND conditions,
//		2. all rows sharing the same groupname are OR conditions.
// so - if a sound matches all conditions of a row, it is given that row's
// mixgroup id
//		if a sound doesn't match all conditions of a row, the next row
// is  checked.
...
void MXR_GetMixGroupFromSoundsource(channel_t *pchan, SoundSource soundsource,
  ...
  for (int i = 0; i < g_cgroupclass; i++) {
    classMatch[i] = false;
    if (pszclassname &&
        Q_stristr(pszclassname, g_groupclasslist[i].szclassname)) {
      classMatch[i] = true;
    }
  }
  ...
  // since this is a sorted list (in group rule order) we only need to test
  // against the next matching rule this avoids a search inside the loop
  ...
  for (i = 0; i < g_cgrouprules; i++) {
    ...
    if (prule->classId >= 0) {
      ...
      if (pszclassname &&
          Q_stristr(pszclassname,
                    g_groupclasslist[prule->classId].szclassname)) {
    ...
    if (fmatch) {
      ...
      cmixgroups++;
      if (cmixgroups >= 8) return;  // too many matches, stop looking
    }
...
void MXR_DebugGraphMixVolumes(debug_showvols_t *groupvols, int cgroups) {
  ...
  CDebugOverlay::AddScreenTextOverlay(flXpos, flYpos, duration, r, g, b, a,
                                      text);
  ...
  for (int i = 0; i < cgroups; i++) {
    ...
    Q_snprintf(text, 128, "%s: %1.2f (%1.2f)", groupvols[i].psz,
               groupvols[i].vol * g_DuckScale,
               groupvols[i].totalvol * g_DuckScale);
...
ConVar snd_disable_mixer_duck("snd_disable_mixer_duck",
                              "0");  // if 1, soundmixer ducking is disabled
...
float MXR_GetDuckVolume(int mixgroupid) {
  ...
  // return current duck value for this group, scaled by current fade in/out
  // ramp
...
void MXR_UpdateAllDuckerVolumes() {
  ...
  if (fabs(dtime - g_mxr_ducktime) < SND_DUCKER_UPDATETIME) return;

  g_mxr_ducktime = dtime;

  // clear out all total volume values for groups

  for (i = 0; i < g_cgrouprules; i++) g_grouprules[i].total_vol = 0.0;
  ...
  // if no sound group is causing ducking in this group, reset duck_target_vol
  // to 1.0
  ...
  for (i = 0; i < g_cgrouprules; i++) {
    if (g_grouprules[i].is_ducked) {
      ...
      // if global voice ducking is active, skip sound group ducking and just
      // set duck_volume target to 1.0
      ...
      if (g_DuckScale >= 1.0) {
        ...
        for (j = 0; j < g_cgrouprules; j++) {
          if (g_grouprules[j].priority > priority &&
            ...
            // UNDONE: to prevent edge condition caused by crossing threshold,
            // may need to have secondary UNDONE: timer which allows ducking at
            // 0.2 hz
  ...
  for (i = 0; i < g_cgrouprules; i++) {
    ...
    if (g_grouprules[i].is_ducked && (current != target)) {
      float ramptime = target < current ? snd_duckerattacktime.GetFloat()
                                        : snd_duckerreleasetime.GetFloat();
      ...
      if (current > target) delta = -delta;

      // update ramps

      current += delta;

      if (current < target && delta < 0) current = target;
      if (current > target && delta > 0) current = target;
...
ConVar snd_showmixer("snd_showmixer",
                     "0");  // set to 1 to show mixer every frame
  ...
  if (g_csoundmixers) {
    ...
    if (pmixer) {
      ...
      for (int i = 0; i < 8; i++) {
        ...
        // save lowest duck gain value for any of the mix groups this sound is
        // in
...
int MXR_GetMixgroupFromName(const char *pszgroupname) {
  ...
  if (!pszgroupname) return -1;

  if (Q_strlen(pszgroupname) == 0) return -1;
...
void MXR_AssignGroupIds() {
  ...
  for (int i = 0; i < g_cgrouprules; i++) {
    ...
    if (mixgroupid == -1) {
      ...
      // save reverse mapping, from mixgroupid to the first grouprules entry for
      // this name
...
int MXR_AddClassname(const char *pName) {
  ...
  for (int i = 0; i < g_cgroupclass; i++) {
    if (!Q_stricmp(szclassname, g_groupclasslist[i].szclassname)) return i;
  }
...
bool MXR_LoadAllSoundMixers() {
  ...
  pbuffer = (char *)COM_LoadFile(
      szFile, 5, NULL);  // Use malloc - free at end of this routine
  if (!pbuffer) {
  ...
  // get strings for: groupname, directory, classname, chan, sndlvl_min,
  // sndlvl_max convert chan to CHAN_ lookup convert sndlvl_min, sndl_max to
  // ints store all in g_grouprules, update g_cgrouprules;
  // get next line
  ...
  while (1) {
    ...
    if (strlen(com_token) <= 0) break;  // eof
  ...
  while (1) {
    ...
    // make sure all copied strings are 0 terminated
  ...
  while (1) {
    ...
    if (strlen(com_token) <= 0) break;  // eof //-V814
    ...
    while (1) {
      ...
      if (com_token[0] == CHAR_LEFT_PAREN) continue;  // skip {
...
float S_GetMono16Samples(const char *pszName, CUtlVector<short> &sampleList) {
  ...
  if (!pWave) return 0.0f;

  int nType = pWave->GetType();
  if (nType != CAudioSource::AUDIO_SOURCE_WAV) return 0.0f;

  CAudioMixer *pMixer = pWave->CreateMixer();
  if (!pMixer) return 0.0f;
