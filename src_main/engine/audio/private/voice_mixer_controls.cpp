...
class CMixerControls : public IMixerControls {
 ...
 private:
  ...
  bool GetControlOption_Bool(DWORD dwControlID, DWORD cMultipleItems,
                             bool &bValue);
  bool SetControlOption_Bool(DWORD dwControlID, DWORD cMultipleItems,
                             const bool bValue);

  bool GetControlOption_Unsigned(DWORD dwControlID, DWORD cMultipleItems,
                                 DWORD &value);
  bool SetControlOption_Unsigned(DWORD dwControlID, DWORD cMultipleItems,
                                 const DWORD value);
...
bool CMixerControls::Init() {
  ...
  for (UINT u = 0; u < mxcaps.cDestinations; u++) {
    ...
    mmr = mixerGetLineInfo((HMIXEROBJ)m_hMixer, &recordLine,
                           MIXER_GETLINEINFOF_DESTINATION);
    if (mmr != MMSYSERR_NOERROR) continue;

    // Go through the controls that aren't attached to a specific src
    // connection. We're looking for the checkbox that enables the user's
    // microphone for waveIn.
    ...
    // Now iterate over each connection (things like wave out, microphone,
    // speaker, CD audio), looking for Microphone.
    ...
    for (UINT v = 0; v < cConnections; v++) {
      ...
      mmr = mixerGetLineInfo((HMIXEROBJ)m_hMixer, &micLine,
                             MIXER_GETLINEINFOF_SOURCE);
      if (mmr != MMSYSERR_NOERROR) continue;

      // Now look at all the controls (volume, mute, boost, etc).
      MIXERCONTROL *controls =
          (MIXERCONTROL *)_alloca(sizeof(MIXERCONTROL) * micLine.cControls);
      ...
      for (UINT i = 0; i < micLine.cControls; i++) {
        ...
        if (micLine.dwComponentType == MIXERLINE_COMPONENTTYPE_SRC_MICROPHONE) {
          if (pControl->dwControlType == MIXERCONTROL_CONTROLTYPE_ONOFF &&
            ...
            m_ControlInfos[MicBoost].m_cMultipleItems =
                pControl->cMultipleItems;
          }

          if (recordLine.dwComponentType ==
          ...
          if (recordLine.dwComponentType ==
            ...
            m_ControlInfos[MicVolume].m_cMultipleItems =
                pControl->cMultipleItems;
          }
        }
      }
...
bool CMixerControls::GetValue_Float(Control iControl, float &value) {
  if (iControl < 0 || iControl >= NumControls ||
      !m_ControlInfos[iControl].m_bFound)
    return false;

  switch (iControl) {
    case IMixerControls::MicBoost:
    case IMixerControls::MicMute: {
      bool bValue = false;
      bool ret = GetControlOption_Bool(
          m_ControlInfos[iControl].m_dwControlID,
          m_ControlInfos[iControl].m_cMultipleItems, bValue);
      value = (float)bValue;
      return ret;
    }
    case IMixerControls::MicVolume: {
      DWORD dwValue = (DWORD)0;
      if (GetControlOption_Unsigned(m_ControlInfos[iControl].m_dwControlID,
                                    m_ControlInfos[iControl].m_cMultipleItems,
                                    dwValue)) {
        value = dwValue / 65535.0f;
        return true;
      }
    }
    case IMixerControls::NumControls:
    default:
      return false;
  }
}
...
bool CMixerControls::SetValue_Float(Control iControl, float value) {
  ...
  switch (iControl) {
    ...
    case IMixerControls::MicMute: {
      bool bValue = !!value;
      return SetControlOption_Bool(m_ControlInfos[iControl].m_dwControlID,
                                   m_ControlInfos[iControl].m_cMultipleItems,
                                   bValue);
    }
    case IMixerControls::MicVolume: {
      DWORD dwValue = (DWORD)(value * 65535.0f);
      return SetControlOption_Unsigned(
          m_ControlInfos[iControl].m_dwControlID,
          m_ControlInfos[iControl].m_cMultipleItems, dwValue);
    }
    case IMixerControls::NumControls:
    default:
      return false;
...
bool CMixerControls::SelectMicrophoneForWaveInput() {
  if (m_dwMicSelectControlID == 0xFFFFFFFF) return false;

  MIXERCONTROLDETAILS_BOOLEAN *pmxcdSelectValue =
      (MIXERCONTROLDETAILS_BOOLEAN *)_alloca(
          sizeof(MIXERCONTROLDETAILS_BOOLEAN) * m_dwMicSelectMultipleItems);
  ...
  if (mixerGetControlDetails(
          reinterpret_cast<HMIXEROBJ>(m_hMixer), &mxcd,
    ...
    if (m_dwMicSelectControlType == MIXERCONTROL_CONTROLTYPE_MUX) {
      ZeroMemory(pmxcdSelectValue, m_dwMicSelectMultipleItems *
...
void CMixerControls::Release() { delete this; }
...
bool CMixerControls::GetControlOption_Bool(DWORD dwControlID,
                                           DWORD cMultipleItems, bool &bValue) {
...
bool CMixerControls::SetControlOption_Bool(DWORD dwControlID,
...
bool CMixerControls::GetControlOption_Unsigned(DWORD dwControlID,
                                               DWORD cMultipleItems,
                                               DWORD &value) {
  MIXERCONTROLDETAILS details;
...
bool CMixerControls::SetControlOption_Unsigned(DWORD dwControlID,
                                               DWORD cMultipleItems,
                                               const DWORD value) {
  MIXERCONTROLDETAILS details;
  MIXERCONTROLDETAILS_UNSIGNED controlValue;
...
bool CMixerControls::GetLineControls(DWORD dwLineID, MIXERCONTROL *controls,
                                     DWORD nControls) {
  ...
  MMRESULT mmr = mixerGetLineControls((HMIXEROBJ)m_hMixer, &mxlc,
...
void CMixerControls::FindMicSelectControl(DWORD dwLineID, DWORD nControls) {
  ...
  MIXERCONTROL *recControls =
      (MIXERCONTROL *)_alloca(sizeof(MIXERCONTROL) * nControls);
  if (!GetLineControls(dwLineID, recControls, nControls)) return;

  for (UINT iRecControl = 0; iRecControl < nControls; iRecControl++) {
    if (recControls[iRecControl].dwControlType ==
            MIXERCONTROL_CONTROLTYPE_MIXER ||
        recControls[iRecControl].dwControlType ==
      ...
      MIXERCONTROLDETAILS_LISTTEXT *pmxcdSelectText =
          (MIXERCONTROLDETAILS_LISTTEXT *)_alloca(
              sizeof(MIXERCONTROLDETAILS_LISTTEXT) *
              m_dwMicSelectMultipleItems);
...
IMixerControls *GetMixerControls() { return new CMixerControls; }
