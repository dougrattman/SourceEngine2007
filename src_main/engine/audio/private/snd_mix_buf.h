...
// OPTIMIZE: note that making this larger will not increase performance
// (12/27/03)
...
#define CPAINTFILTERS \
  4  // maximum number of consecutive upsample passes per paintbuffer
...
struct paintbuffer_t {
  ...
  bool fsurround_center;  // if true, mix to front, rear and center paintbuffers
                          // using flags

  int flags;  // SOUND_BUSS_ROOM, SOUND_BUSS_FACING, SOUND_BUSS_FACINGAWAY,
              // SOUND_BUSS_SPEAKER, SOUND_BUSS_DRY

  portable_samplepair_t
      *pbuf;  // front stereo mix buffer, for 2 or 4 channel mixing
  ...
  portable_samplepair_t
      fltmem[CPAINTFILTERS]
            [CPAINTFILTERMEM];  // filter memory, for upsampling with linear or
                                // cubic interpolation
  portable_samplepair_t
      fltmemrear[CPAINTFILTERS]
                [CPAINTFILTERMEM];  // filter memory, for upsampling with linear
                                    // or cubic interpolation
  portable_samplepair_t
      fltmemcenter[CPAINTFILTERS]
                  [CPAINTFILTERMEM];  // filter memory, for upsampling with
...
extern bool MIX_InitAllPaintbuffers();
extern void MIX_FreeAllPaintbuffers();
