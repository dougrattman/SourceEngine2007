...
static ConVar net_showevents(
    "net_showevents", "0", 0,
    "Dump game events to console (1=client only, 2=all).");

// Expose CVEngineServer to the engine.

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CGameEventManager, IGameEventManager2,
...
CGameEvent::~CGameEvent() { m_pDataKeys->deleteThis(); }
...
const char *CGameEvent::GetString(const char *keyName,
...
const char *CGameEvent::GetName() const { return m_pDataKeys->GetName(); }

bool CGameEvent::IsLocal() const { return m_pDescriptor->local; }

bool CGameEvent::IsReliable() const { return m_pDescriptor->reliable; }

CGameEventManager::CGameEventManager() { Reset(); }
...
void CGameEventManager::Shutdown() { Reset(); }
...
bool CGameEventManager::HasClientListenersChanged(bool bReset /* = true  */) {
  if (!m_bClientListenersChanged) return false;

  if (bReset) m_bClientListenersChanged = false;
...
bool CGameEventManager::ParseEventList(SVC_GameEventList *msg) {
  ...
  for (i = 0; i < msg->m_nNumEvents; i++) {
    ...
    if (descriptor->keys) descriptor->keys->deleteThis();
...
void CGameEventManager::WriteListenEventList(CLC_ListenEvents *msg) {
  ...
  for (int i = 0; i < m_GameEvents.Count(); i++) {
    ...
    if (!bHasClientListener) continue;
...
bool CGameEventManager::FireEventIntern(IGameEvent *event, bool bServerOnly,
                                        bool bClientOnly) {
  if (event == NULL) return false;

  Assert(!(bServerOnly && bClientOnly));  // it can't be both

  VPROF_("CGameEventManager::FireEvent", 1, VPROF_BUDGETGROUP_OTHER_UNACCOUNTED,
         false,
         bClientOnly ? BUDGETFLAG_CLIENT
                     : (bServerOnly ? BUDGETFLAG_SERVER : BUDGETFLAG_OTHER));
  ...
  for (int i = 0; i < descriptor->listeners.Count(); i++) {
    ...
    // TODO optimized the serialize event for clients, call only once and not
    // per client
    ...
    if (listener->m_nListenerType == CLIENTSIDE_OLD ||
      ...
      IGameEventListener *pCallback =
          static_cast<IGameEventListener *>(listener->m_pCallback);
      CGameEvent *pEvent = static_cast<CGameEvent *>(event);

      pCallback->FireGameEvent(pEvent->m_pDataKeys);
    } else {
      // new system
      IGameEventListener2 *pCallback =
          static_cast<IGameEventListener2 *>(listener->m_pCallback);
...
bool CGameEventManager::SerializeEvent(IGameEvent *event, bf_write *buf) {
  ...
  // now iterate trough all fields described in gameevents.res and put them in
  // the buffer
  ...
  while (key) {
    ...
    // Assert( event->FindKey(keyName) && "GameEvent field not found in passed
    // KeyValues" );

    // see s_GameEnventTypeMap for index
    switch (type) {
      case TYPE_LOCAL:
        break;  // don't network this guy
      case TYPE_STRING:
        buf->WriteString(event->GetString(keyName, ""));
        break;
      case TYPE_FLOAT:
        buf->WriteFloat(event->GetFloat(keyName, 0.0f));
        break;
      case TYPE_LONG:
        buf->WriteLong(event->GetInt(keyName, 0));
        break;
      case TYPE_SHORT:
        buf->WriteShort(event->GetInt(keyName, 0));
        break;
      case TYPE_BYTE:
        buf->WriteByte(event->GetInt(keyName, 0));
        break;
      case TYPE_BOOL:
  ...
  while (key) {
    ...
    switch (type) {
      ...
      case TYPE_STRING:
        ...
        break;
      case TYPE_FLOAT:
        event->SetFloat(keyName, buf->ReadFloat());
        break;
      case TYPE_LONG:
        event->SetInt(keyName, buf->ReadLong());
        break;
      case TYPE_SHORT:
        event->SetInt(keyName, buf->ReadShort());
        break;
      case TYPE_BYTE:
        event->SetInt(keyName, buf->ReadByte());
        break;
      case TYPE_BOOL:
...
bool CGameEventManager::FindListener(IGameEventListener2 *listener,
                                     const char *name) {
  CGameEventDescriptor *pDescriptor = GetEventDescriptor(name);

  if (!pDescriptor) return false;  // event is unknown

  CGameEventCallback *pCallback = FindEventListener(listener);

  if (!pCallback) return false;  // listener is unknown

  // see if listener is in the list for this event
  return pDescriptor->listeners.IsValidIndex(
      pDescriptor->listeners.Find(pCallback));
}
...
int CGameEventManager::LoadEventsFromFile(const char *filename) {
  ...
  if (!key->LoadFromFile(g_pFileSystem, filename, "GAME")) return false;
...
bool CGameEventManager::AddListener(IGameEventListener2 *listener,
                                    const char *event, bool bServerSide) {
  ...
  return AddListener(listener, descriptor,
                     bServerSide ? SERVERSIDE : CLIENTSIDE);
}

bool CGameEventManager::AddListener(void *listener,
                                    CGameEventDescriptor *descriptor,
                                    int nListenerType) {
  if (!listener || !descriptor) return false;  // bahh
  ...
  if (descriptor->listeners.Find(pCallback) ==
      descriptor->listeners.InvalidIndex()) {
...
CGameEventDescriptor *CGameEventManager::GetEventDescriptor(
    int eventid)  // returns event name or NULL
{
  ...
  for (int i = 0; i < m_GameEvents.Count(); i++) {
    ...
    if (descriptor->eventid == eventid) return descriptor;
  }
...
CGameEventDescriptor *CGameEventManager::GetEventDescriptor(const char *name) {
  ...
  for (int i = 0; i < m_GameEvents.Count(); i++) {
    ...
    if (Q_strcmp(descriptor->name, name) == 0) return descriptor;
  }
...
bool CGameEventManager::AddListenerAll(void *listener, int nListenerType) {
  ...
  DevMsg(
      "Warning! Game event listener registerd for all events. Use newer game "
      "event interface.\n");
