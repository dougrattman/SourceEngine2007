...
// The function is split up into multiple smaller ones and they pass this
// structure around.
class CEntityWriteInfo : public CEntityInfo {
 public:
  ...
  // For each entity handled in the to packet, mark that's it has already been
  // deleted if that's the case
...
CChangeTrack *GetChangeTrack(const char *pName) {
  FOR_EACH_LL(g_Tracks, i) {
    ...
    if (_stricmp(pCur->m_pName, pName) == 0) return pCur;
  }
...
void PrintChangeTracks() {
  ...
  ConMsg(
      ...
      "\n");
  ConMsg(
      "CalcDelta MS / %% time / Encode MS / # Changed / # Unchanged / Class "
      "Name\n");
  ConMsg(
      "------------------------------------------------------------------------"
      "\n");
  ...
  FOR_EACH_LL(g_Tracks, j) {
    ...
    ConMsg("%6.2fms       %5.2f    %6.2fms    %4d        %4d          %s\n",
           ...
           pCur->m_EncodeCount.GetMillisecondsF(), pCur->m_nChanged,
           pCur->m_nUnchanged, pCur->m_pName);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Entity wasn't dealt with in packet, but it has been deleted, we'll
// flag
...
//-----------------------------------------------------------------------------
static inline bool SV_NeedsExplicitDestroy(int entnum, CFrameSnapshot *from,
                                           CFrameSnapshot *to) {
  // Never on uncompressed packet

  if (to->m_pEntities[entnum].m_pClass == NULL)  // doesn't exits in new
  {
    if (entnum >= from->m_nNumEntities) return false;  // didn't exist in old
...
//-----------------------------------------------------------------------------
static inline void SV_UpdateHeaderDelta(CEntityWriteInfo &u, int entnum) {
  // Profiling info
...
//
// Write the delta header. Also update the header delta info if
// bUpdateHeaderDelta is true.
//
// There are some cases where you want to tenatively write a header, then
// possibly back it out. In these cases:
// - pass in false for bUpdateHeaderDelta
// - store the return value from SV_WriteDeltaHeader
// - call SV_UpdateHeaderDelta ONLY if you want to keep the delta header it
// wrote
//
static inline void SV_WriteDeltaHeader(CEntityWriteInfo &u, int entnum,
                                       int flags) {
...
// NOTE: to optimize this, it could store the bit offsets of each property in
// the packed entity. It would only have to store the offsets for the entities
// for each frame, since it only reaches into the current frame's entities here.
static inline void SV_WritePropsFromPackedEntity(CEntityWriteInfo &u,
  ...
  if (g_bServerDTIEnabled && !u.m_pServer->IsHLTV()) {
    ...
    if (pEnt && pClientEnt) {
      float flDist =
          (pEnt->GetCollisionOrigin() - pClientEnt->GetCollisionOrigin())
              .Length();
  ...
  if (u.m_bCullProps) {
    ...
    nSendProps = SendTable_CullPropsFromProxies(
        pSendTable, pCheckProps, nCheckProps, u.m_nClientEntity - 1,

        pFrom->GetRecipients(), pFrom->GetNumRecipients(),

        pTo->GetRecipients(), pTo->GetNumRecipients(),

        pSendProps, std::size(pSendProps));
  } else {
    // this is a HLTV relay proxy
    bufStart = *u.m_pBuf;
  }

  SendTable_WritePropList(pSendTable, pToData, nToBits, u.m_pBuf,
                          pTo->m_nEntityIndex,

                          sendProps, nSendProps);
  ...
  if (!u.m_bCullProps && hltv) {
    ...
    hltv->m_DeltaCache.AddDeltaBits(
        pTo->m_nEntityIndex, u.m_pFromSnapshot->m_nTickCount, nBits, &bufStart);
  }
}

//-----------------------------------------------------------------------------
// Purpose: See if the entity needs a "hard" reset ( i.e., and explicit creation
// tag )
//  This should only occur if the entity slot deleted and re-created an entity
//  faster than the last two updates toa  player.  Should never or almost never
//  occur.  You never know though.
// Input  : type -
//			entnum -
//			*from -
//			*to -
...
static bool SV_NeedsExplicitCreate(CEntityWriteInfo &u) {
  ...
  // Server thinks the entity was continues, but the serial # changed, so we
  // might need to destroy and recreate it
  ...
  bool bNeedsExplicitCreate =
      (pFromEnt->m_pClass == NULL) ||
      pFromEnt->m_nSerialNumber != pToEnt->m_nSerialNumber;

#ifdef _DEBUG
  if (!bNeedsExplicitCreate) {
    // If it doesn't need explicit create, then the classnames should match.
    // This assert is analagous to the "Server / Client mismatch" one on the
    // client.
    static int nWhines = 0;
    if (pFromEnt->m_pClass->GetName() != pToEnt->m_pClass->GetName()) {
      if (++nWhines < 4) {
        Msg("ERROR in SV_NeedsExplicitCreate: ent %d from/to classname (%s/%s) "
            "mismatch.\n",
            u.m_nNewEntity, pFromEnt->m_pClass->GetName(),
            pToEnt->m_pClass->GetName());
      }
    }
  }
#endif
...
static inline void SV_DetermineUpdateType(CEntityWriteInfo &u) {
  ...
  // These should be the same! If they're not, then it should detect an explicit
  // create message.
  Assert(u.m_pOldPack->m_pServerClass == u.m_pNewPack->m_pServerClass);

  // We can early out with the delta bits if we are using the same pack
  // handles...
  ...
  if (!u.m_bCullProps && hltv) {
    int nBits;
    unsigned char *pBuffer = hltv->m_DeltaCache.FindDeltaBits(
        u.m_nNewEntity, u.m_pFromSnapshot->m_nTickCount, nBits);
  ...
  int nCheckProps = u.m_pNewPack->GetPropsChangedAfterTick(
      u.m_pFromSnapshot->m_nTickCount, checkProps, std::size(checkProps));

  if (nCheckProps == -1) {
    // check failed, we have to recalc delta props based on from & to snapshot
    // that should happen only in HLTV demo playback mode, this code is really
    // expensive
    ...
    nCheckProps = SendTable_CalcDelta(
        ...
        nNewBits, checkProps, std::size(checkProps), u.m_nNewEntity);
  }

#ifndef NO_VCR
  if (vcr_verbose.GetInt()) {
    VCRGenericValueVerify("checkProps", checkProps,
                          sizeof(checkProps[0]) * nCheckProps);
  }
#endif
  ...
  if (nCheckProps > 0) {
    ...
    TRACE_PACKET(("    Delta Bits (%d) = %d (%d bytes)\n", u.m_nNewEntity,
                  (endBit - startBit), ((endBit - startBit) + 7) / 8));
#endif
    // If the numbers are the same, then the entity was in the old and new
    // packet. Just delta compress the differences.
    u.m_UpdateType = DeltaEnt;
  } else {
#ifndef _X360
    if (!u.m_bCullProps && hltv) {
      // no bits changed, PreserveEnt
      hltv->m_DeltaCache.AddDeltaBits(u.m_nNewEntity,
                                      u.m_pFromSnapshot->m_nTickCount, 0, NULL);
    }
#endif
    u.m_UpdateType = PreserveEnt;
  }
...
static inline void SV_WriteEnterPVS(CEntityWriteInfo &u) {
  TRACE_PACKET(("  SV Enter PVS (%d) %s\n", u.m_nNewEntity,
                u.m_pNewPack->m_pServerClass->m_pNetworkName));
  ...
  u.m_pBuf->WriteUBitLong(entry->m_nSerialNumber,
                          NUM_NETWORKED_EHANDLE_SERIAL_NUMBER_BITS);

  // Get the baseline.
  // Since the ent is in the fullpack, then it must have either a static or an
  // instance baseline.
  PackedEntity *pBaseline =
      u.m_bAsDelta
          ? framesnapshotmanager->GetPackedEntity(u.m_pBaseline, u.m_nNewEntity)
          : NULL;
  const void *pFromData;
  int nFromBits;

  if (pBaseline &&
      (pBaseline->m_pServerClass == u.m_pNewPack->m_pServerClass)) {
    ...
    if (!u.m_pServer->GetClassBaseline(pClass, &pFromData, &nFromBytes)) {
      Error("SV_WriteEnterPVS: missing instance baseline for '%s'.",
            pClass->m_pNetworkName);
    }

    ErrorIfNot(pFromData, ("SV_WriteEnterPVS: missing pFromData for '%s'.",
                           pClass->m_pNetworkName));

    nFromBits = nFromBytes *
                8;  // NOTE: this isn't the EXACT number of bits but that's ok
                    // since it's only used to detect if we overran the buffer
                    // (and if we do, it's probably by more than 7 bits).
  }
  ...
  u.m_nFullProps += SendTable_WriteAllDeltaProps(
      pClass->m_pTable, pFromData, nFromBits, pToData, nToBits,
      u.m_pNewPack->m_nEntityIndex, u.m_pBuf);
...
static inline void SV_WriteLeavePVS(CEntityWriteInfo &u) {
  ...
  if (u.m_bAsDelta) {
    deleteentity = SV_NeedsExplicitDestroy(u.m_nOldEntity, u.m_pFromSnapshot,
                                           u.m_pToSnapshot);
  }
...
static inline void SV_WriteDeltaEnt(CEntityWriteInfo &u) {
  ...
  // NOTE: it was already written in DetermineUpdateType. By doing it this way,
  // we avoid an expensive (non-byte-aligned) copy of the data.
...
static inline void SV_PreserveEnt(CEntityWriteInfo &u) {
  ...
  // updateType is preserveEnt. The client will detect this because our next
  // entity will have a newnum that is greater than oldnum, in which case the
  // client just keeps the current entity alive.
  u.NextOldEntity();
  u.NextNewEntity();
}
...
static inline int SV_WriteDeletions(CEntityWriteInfo &u) {
  ...
  for (int i = 0; i < pSnapShot->m_nNumEntities; i++) {
    ...
    if (u.m_DeletionFlags.Get(i)) continue;

    // Looks like it should be gone
    bool bNeedsExplicitDelete =
        SV_NeedsExplicitDestroy(i, u.m_pFromSnapshot, pSnapShot);
    if (!bNeedsExplicitDelete && u.m_pTo) {
      bNeedsExplicitDelete = (pSnapShot->m_iExplicitDeleteSlots.Find(i) !=
                              pSnapShot->m_iExplicitDeleteSlots.InvalidIndex());
      if (bNeedsExplicitDelete) {
        const CFrameSnapshotEntry *pFromEnt =
            &u.m_pFromSnapshot->m_pEntities[i];
        const CFrameSnapshotEntry *pToEnt = &u.m_pToSnapshot->m_pEntities[i];

        if (pFromEnt && pToEnt) {
          bool bWillBeExplicitlyCreated =
              (pFromEnt->m_pClass == NULL) ||
              pFromEnt->m_nSerialNumber != pToEnt->m_nSerialNumber;
          if (bWillBeExplicitlyCreated && u.m_pTo->transmit_entity.Get(i)) {
            // Warning("Entity %d is being explicitly deleted, but it will be
            // explicitly created.\n", i );
...
void CBaseServer::WriteDeltaEntities(CBaseClient *client, CClientFrame *to,
  ...
  if (u.m_bAsDelta) {
    ...
    u.m_pBuf->WriteLong(
        u.m_pFrom
            ->tick_count);  // This is the sequence # that we are updating from.
  } else {
    u.m_pBuf->WriteOneBit(0);  // use baseline
  }

  u.m_pBuf->WriteUBitLong(client->m_nBaselineUsed,
                          1);  // tell client what baseline we are using
  ...
  if (!g_pLocalNetworkBackdoor) {
    ...
    while ((u.m_nOldEntity != ENTITY_SENTINEL) ||
           (u.m_nNewEntity != ENTITY_SENTINEL)) {
      u.m_pNewPack = (u.m_nNewEntity != ENTITY_SENTINEL)
                         ? framesnapshotmanager->GetPackedEntity(
                               u.m_pToSnapshot, u.m_nNewEntity)
                         : NULL;
      u.m_pOldPack = (u.m_nOldEntity != ENTITY_SENTINEL)
                         ? framesnapshotmanager->GetPackedEntity(
                               u.m_pFromSnapshot, u.m_nOldEntity)
                         : NULL;
      ...
      switch (u.m_UpdateType) {
        ...
        case PreserveEnt:
          break;
        case EnterPVS: {
          char const *eString = sv.edicts[u.m_pNewPack->m_nEntityIndex]
                                    .GetNetworkable()
                                    ->GetClassName();
          client->TraceNetworkData(pBuf, "enter [%s]", eString);
        } break;
        case LeavePVS: {
          // Note, can't use GetNetworkable() since the edict has been freed at
          // this point
          char const *eString = u.m_pOldPack->m_pServerClass->m_pNetworkName;
          client->TraceNetworkData(pBuf, "leave [%s]", eString);
        } break;
        case DeltaEnt: {
          char const *eString = sv.edicts[u.m_pOldPack->m_nEntityIndex]
                                    .GetNetworkable()
                                    ->GetClassName();
