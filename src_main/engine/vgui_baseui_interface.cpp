...
// vgui2 interface
// note that GameUI project uses ..\public\vgui and ..\public\vgui_controls, not
// ..\utils\vgui\include
#include <vgui/Cursor.h>
...
ConVar r_drawvgui("r_drawvgui", "1", FCVAR_CHEAT,
...
//-----------------------------------------------------------------------------
// Purpose: Console command to hide the gameUI, most commonly called from
// gameUI.dll
//-----------------------------------------------------------------------------
CON_COMMAND(gameui_hide, "Hides the game UI") { EngineVGui()->HideGameUI(); }

//-----------------------------------------------------------------------------
// Purpose: Console command to activate the gameUI, most commonly called from
// gameUI.dll
//-----------------------------------------------------------------------------
...
class CEnginePanel : public vgui::EditablePanel {
 ...
 public:
  CEnginePanel(vgui::Panel *pParent, const char *pName)
  ...
  /*	virtual vgui::VPANEL IsWithinTraverse(int x, int y, bool traversePopups)
          {
                  ...
                  vgui::VPANEL retval = BaseClass::IsWithinTraverse( x, y,
     traversePopups ); if ( retval == GetVPanel() ) return NULL; return retval;
          }*/
...
class CStaticPanel : public vgui::Panel {
 ...
 public:
  CStaticPanel(vgui::Panel *pParent, const char *pName)
...
class CEngineVGui : public IEngineVGuiInternal {
 public:
  ...
  CreateInterfaceFn GetGameUIFactory() { return m_GameUIFactory; }
  ...
  virtual void NotifyOfServerConnect(const char *game, int IP,
                                     int connectionPort, int queryPort);
  ...
  virtual void ShowNewGameDialog(
      int chapter = -1);  // -1 means just keep the currently select chapter
 ...
 private:
  ...
  CON_COMMAND_MEMBER_F(CEngineVGui, "debugsystemui", ToggleDebugSystemUI,
                       "Show/hide the debug system UI.", FCVAR_CHEAT);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CEngineVGui, IEngineVGui,
                                  VENGINE_VGUI_VERSION, g_EngineVGuiImp);
...
void CEngineVGui::Init() {
  ...
  staticGameUIFuncs =
      (IGameUI *)m_GameUIFactory(GAMEUI_INTERFACE_VERSION, NULL);
  ...
  staticGameConsole =
      (IGameConsole *)m_GameUIFactory(GAMECONSOLE_INTERFACE_VERSION, NULL);
  ...
  // The root panel for everything ( NULL parent makes it a child of the
  // embedded panel )
  ...
  staticPanel->SetBounds(0, 0, videomode->GetModeWidth(),
  ...
  staticClientDLLPanel->SetKeyBoardInputEnabled(
      false);  // popups in the client DLL can enable this.
  ...
  staticClientDLLToolsPanel->SetBounds(0, 0, videomode->GetModeWidth(),
                                       videomode->GetModeHeight());
  staticClientDLLToolsPanel->SetPaintBorderEnabled(false);
  staticClientDLLToolsPanel->SetPaintBackgroundEnabled(false);
  staticClientDLLToolsPanel->SetKeyBoardInputEnabled(
      false);  // popups in the client DLL can enable this.
  ...
  staticEngineToolsPanel->SetBounds(0, 0, videomode->GetModeWidth(),
                                    videomode->GetModeHeight());
  ...
  staticGameDLLPanel->SetBounds(0, 0, videomode->GetModeWidth(),
                                videomode->GetModeHeight());
  staticGameDLLPanel->SetPaintBorderEnabled(false);
  staticGameDLLPanel->SetPaintBackgroundEnabled(false);
  staticGameDLLPanel->SetKeyBoardInputEnabled(
      false);  // popups in the game DLL can enable this.
  ...
  staticFocusOverlayPanel =
      new CFocusOverlayPanel(staticPanel, "FocusOverlayPanel");
  staticFocusOverlayPanel->SetBounds(0, 0, videomode->GetModeWidth(),
                                     videomode->GetModeHeight());
  ...
  if (staticGameConsole && !CommandLine()->CheckParm("-forcestartupmenu") &&
      ...
      (CommandLine()->FindParm("-toconsole") ||
       CommandLine()->FindParm("-console") || CommandLine()->FindParm("-rpt") ||
...
void CEngineVGui::Connect() {
  m_pInputInternal = (vgui::IInputInternal *)g_GameSystemFactory(
      VGUI_INPUTINTERNAL_INTERFACE_VERSION, NULL);
...
void CEngineVGui::CreateVProfPanels(vgui::Panel *pParent) {
  ...
  m_pTextureBudgetPanel =
      new CTextureBudgetPanel(pParent, "TextureBudgetPanel");
#endif
}
...
void CEngineVGui::Shutdown() {
  ...
  // HACK HACK: There was a bug in the old versions of the viewport which would
  // crash in the case where the client .dll hadn't been fully unloaded, so
...
void CEngineVGui::ActivateGameUI() {
  if (m_bNotAllowedToShowGameUI) return;
...
bool CEngineVGui::HideGameUI() {
  if (m_bNotAllowedToHideGameUI) return false;

  const char *levelName = engineClient->GetLevelName();
  bool bInNonBgLevel =
      levelName && levelName[0] && !engineClient->IsLevelMainMenuBackground();
  if (bInNonBgLevel) {
    ...
    // Tracker 18820:  Pulling up options/console was perma-pausing the
    // background levels, now we
    //  unpause them when you hit the Esc key even though the UI remains...
    if (levelName && levelName[0] && (engineClient->GetMaxClients() <= 1) &&
...
bool CEngineVGui::IsConsoleVisible() {
  if (IsPC()) {
    return IsGameUIVisible() && staticGameConsole &&
           staticGameConsole->IsConsoleVisible();
  } else {
...
void CEngineVGui::OnLevelLoadingFinished() {
  ...
  staticGameUIFuncs->OnLevelLoadingFinished(
      gfExtendedError, gszDisconnectReason, gszExtendedDisconnectReason);
  m_eLastProgressPoint = PROGRESS_NONE;
...
void CEngineVGui::ShowErrorMessage() {
  if (!staticGameUIFuncs || !gfExtendedError) return;
...
void CEngineVGui::UpdateProgressBar(LevelLoadingProgress_e progress) {
  ...
  if (desc.nRepeat > 1 && m_nLastProgressPointRepeatCount) {
    ...
    flPerc += (flNextPerc - flPerc) *
              ((float)m_nLastProgressPointRepeatCount / desc.nRepeat);
  }
...
void CEngineVGui::Paint(PaintMode_t mode) {
  ...
  if (mode & PAINT_INGAMEPANELS) {
    ...
    // paint for the client .dll here NOTE: Disconnect each surface one at a
    // time so that we don't draw popups twice
...
void CEngineVGui::ToggleDebugSystemUI(const CCommand &args) {
  if (!staticDebugSystemPanel) return;
...
//-----------------------------------------------------------------------------
void CEngineVGui::NotifyOfServerConnect(const char *game_name, int IP,
                                        int connectionPort, int queryPort) {
  if (!staticGameUIFuncs) return;

  staticGameUIFuncs->OnConnectToServer2(game_name, IP, connectionPort,
                                        queryPort);
}
...
//-----------------------------------------------------------------------------
void CEngineVGui::ShowMessageDialog(const uint32_t nType, vgui::Panel *pOwner) {
  ActivateGameUI();
  staticGameUIFuncs->ShowMessageDialog(nType, pOwner);
}

void CEngineVGui::UpdatePlayerInfo(uint64_t nPlayerId, const char *pName,
                                   int nTeam, uint8_t cVoiceState,
                                   int nPlayersNeeded, bool bHost) {
  staticGameUIFuncs->UpdatePlayerInfo(nPlayerId, pName, nTeam, cVoiceState,
                                      nPlayersNeeded, bHost);
}

void CEngineVGui::SessionSearchResult(int searchIdx, void *pHostData,
...
ConVar vgui_drawfocus("vgui_drawfocus", "0", 0,
                      "Report which panel is under the mouse.");

CFocusOverlayPanel::CFocusOverlayPanel(vgui::Panel *pParent, const char *pName)
    : vgui::Panel(pParent, pName) {
...
void CFocusOverlayPanel::PostChildPaint() {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (!vgui::ipanel()->IsVisible(vpanel)) return;
...
static void VGui_RecursiveFindPanels(CUtlVector<vgui::VPANEL> &panelList,
                                     vgui::VPANEL check,
...
void VGui_FindNamedPanels(CUtlVector<vgui::VPANEL> &panelList,
                          char const *panelname) {
...
static void VGui_RecursePanel(CUtlVector<vgui::VPANEL> &panelList, int x, int y,
                              vgui::VPANEL check, bool include_hidden) {
...
void CEngineVGui::DrawMouseFocus() {
  ...
  if (vgui::surface()->IsCursorVisible() && vgui::surface()->IsWithin(x, y)) {
    ...
    for (int i = c - 1; i >= 0; i--) {
      ...
      if (!vgui::ipanel()->IsVisible(popup)) continue;
