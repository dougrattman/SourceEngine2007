...
// EMIT_SURFACE LIGHTS:
//
// Dim emit_surface lights go in the ambient cube because there are a ton of
// them and they are often so dim that they get filtered out by r_worldlightmin.
//
// (Dim) emit_surface lights only get calculated at runtime for static props
// because static props do the full calculation of ambient lighting at runtime
// instead of using vrad's per-leaf calculation. Vrad's calculation includes the
// emit_surfaces, so if we're NOT using it, then we want to include emit_surface
// lights here.
...
static uint8_t g_FrameMissCount = 0;
...
static ConVar r_lightcache_numambientsamples(
    "r_lightcache_numambientsamples", "162", FCVAR_CHEAT,
    "number of random directions to fire rays when computing ambient lighting",
    true, 1.0f, true, (float)NUMRANDOMNORMALS);

ConVar r_ambientlightingonly("r_ambientlightingonly", "0", FCVAR_CHEAT,
                             "Set this to 1 to light models with only ambient "
                             "lighting (and no static lighting).");

ConVar r_oldlightselection(
    "r_oldlightselection", "0", FCVAR_CHEAT,
...
//-----------------------------------------------------------------------------
// Flags to pass into LightIntensityAndDirectionAtPoint +
...
struct LightingStateInfo_t {
  ...
  LightingStateInfo_t() { memset(this, 0, sizeof(*this)); }
  void Clear() { memset(this, 0, sizeof(*this)); }
};

// This holds the shared data between lightcache_t and PropLightcache_t.
// This way, PropLightcache_t can be about half the size, since it doesn't need
// a bunch of data in lightcache_t.
class CBaseLightCache : public LightingStateInfo_t {
 public:
  ...
  bool HasLightStyle() {
    return (m_LightingFlags & (HACKLIGHTCACHEFLAGS_HASSWITCHABLELIGHTSTYLE |
 ...
 public:
  ...
  LightingState_t
      m_LightStyleLightingState;  // This includes m_StaticLightingState
  ...
  // TODO(d.rattman): could just use m_LightStyleWorldLights.Count() if we are a
  // static prop
  ...
  // for a dynamic prop, ideally the cache center if valid space, otherwise
  // initial origin for a static prop, the provided origin
...
class lightcache_t : public CBaseLightCache {
 public:
  lightcache_t() { m_LastFrameUpdated_DynamicLighting = -1; }
...
struct PropLightcache_t : public CBaseLightCache {
 public:
  ...
  int m_DLightMarkFrame;  // last frame in which a dlight was marked on this
                          // prop (helps detect lights that are marked but have
                          // moved away from this prop)
  CUtlVector<short> m_LightStyleWorldLights;  // This is a list of lights that
                                              // affect this static prop cache
                                              // entry.
  int m_SwitchableLightFrame;  // This is the last frame that switchable lights
                               // were calculated.
...
ConVar r_worldlights("r_worldlights", "4", 0,
                     "number of world lights to use per vertex");
ConVar r_radiosity("r_radiosity", "4", FCVAR_CHEAT,
                   "0: no radiosity\n1: radiosity with ambient cube (6 "
                   "samples)\n2: radiosity with 162 samples\n3: 162 samples "
                   "for static props, 6 samples for everything else");
...
static lightcache_t
    lightcache[MAX_CACHE_ENTRY + 2];  // the extra 2 are the head and tail
static unsigned short lightbuckets[MAX_CACHE_BUCKETS];

static CClassMemoryPool<PropLightcache_t> s_PropCache(
    256, CClassMemoryPool<lightcache_t>::GROW_SLOW);
...
void R_StudioInitLightingCache(void) {
  ...
  for (i = 0; i < SOURCE_ARRAYSIZE(lightcache); i++)
  ...
  if (g_pMaterialSystemHardwareConfig->MaxNumLights() <
  ...
  cached_r_lightcache_numambientsamples =
      r_lightcache_numambientsamples.GetInt();
...
//-----------------------------------------------------------------------------
// Purpose: Moves this cache entry to the end of the lru, i.e. marks it recently
// used Input  : *pcache -
//-----------------------------------------------------------------------------
static void LightcacheMark(lightcache_t *pcache) {
  // don't link in static lighting
  if (!pcache->lru_next && !pcache->lru_prev) return;
...
//-----------------------------------------------------------------------------
// Purpose: Quick & Dirty hashing function to bucket the cube in 4d parameter
// space
//-----------------------------------------------------------------------------
...
static lightcache_t *FindInCache(int bucket, int x, int y, int z, int leaf) {
  ...
  for (iCache = lightbuckets[bucket]; iCache != 0xFFFF;
       iCache = lightcache[iCache].next) {
    lightcache_t *pCache = &lightcache[iCache];

    // hit?
    if (pCache->x == x && pCache->y == y && pCache->z == z &&
        pCache->leaf == leaf) {
      return pCache;
    }
  }
...
//-----------------------------------------------------------------------------
void ComputeLightcacheBounds(const Vector &vecOrigin, Vector *pMins,
                             Vector *pMaxs) {
  ...
  Assert((pMins->x <= vecOrigin.x) && (pMins->y <= vecOrigin.y) &&
         (pMins->z <= vecOrigin.z));
  Assert((pMaxs->x >= vecOrigin.x) && (pMaxs->y >= vecOrigin.y) &&
         (pMaxs->z >= vecOrigin.z));
}
...
//-----------------------------------------------------------------------------
static inline void OriginToCacheOrigin(const Vector &origin, int &x, int &y,
                                       int &z) {
...
//-----------------------------------------------------------------------------
static void ComputeAmbientFromSurface(SurfaceHandle_t surfID,
...
static void ComputeAmbientFromSphericalSamples(const Vector &start,
  ...
  for (i = 0; i < cached_r_lightcache_numambientsamples; i++) {
    ...
    VectorMA(start, COORD_EXTENT * 1.74f, g_anorms[i], upend);
...
static void ComputeAmbientFromLeaf(const Vector &start, int leafID,
                                   Vector *lightBoxColor,
                                   bool *bAddedLeafAmbientCube) {
  if (leafID >= 0) {
    Mod_LeafAmbientColorAtPos(lightBoxColor, start, leafID);
    // The ambient lighting in the leaves has the emit_surface lights factored
    // in.
...
//-----------------------------------------------------------------------------
static void R_StudioGetAmbientLightForPoint(int leafID, const Vector &start,
  ...
  switch (r_radiosity.GetInt()) {
    ...
    case 4:
      if (bIsStaticProp)
        ...
        ComputeAmbientFromLeaf(start, leafID, pLightBoxColor,
...
class CTraceFilterWorldAndProps : public ITraceFilter {
 public:
  CTraceFilterWorldAndProps(IHandleEntity *pHandleEntity)
      : m_pIgnoreProp(pHandleEntity) {}

  bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    // We only bump against props + we ignore one particular prop
    if (!StaticPropMgr()->IsStaticProp(pHandleEntity)) return false;
...
static float LightIntensityAndDirectionAtPointOld(dworldlight_t *pLight,
                                                  const Vector &mid, int fFlags,
                                                  IHandleEntity *pIgnoreEnt,
                                                  Vector *pDirection) {
  ...
  switch (pLight->type) {
    case emit_skylight: {
      ...
      Vector end;
      VectorMA(mid, -COORD_EXTENT * 1.74f, pLight->normal,
               end);  // max_range * sqrt(3)
      ...
      if (!(tr.surface.flags & SURF_SKY)) return 0.0f;
  ...
  float ratio = Engine_WorldLightDistanceFalloff(
      pLight, *pDirection, (fFlags & LIGHT_NO_RADIUS_CHECK) != 0);
  ...
  // we don't test emit_surface lights here.
  if (pLight->type != emit_surface) {
    if (intensity * ratio < r_worldlightmin.GetFloat()) return 0.0f;
  }

  float dist = VectorNormalize(*pDirection);

  if (fFlags & LIGHT_NO_OCCLUSION_CHECK) return ratio;
...
//-----------------------------------------------------------------------------
static float LightIntensityAndDirectionAtPointNew(dworldlight_t *pLight,
  ...
  // check if we are so close to the light that we shouldn't use our coarse z
  // buf
  if (dist - (1 << HASH_GRID_SIZEZ) < 8 * SHADOW_ZBUF_RES) pZBuf = NULL;
  ...
  if (pZBuf) {
    ...
    if ((pSample->m_flHitDistance < pSample->m_flTraceDistance) ||
        (pSample->m_flTraceDistance >= dist)) {
      // hit!
      if (dist > pSample->m_flHitDistance + 8)  // shadow hit
      {
        if (r_drawlightcache.GetInt() == 2) {
          CDebugOverlay::AddLineOverlay(mid, pLight->origin, 0, 0, 0, 255, true,
                                        3);
        }
        return 0;
      } else {
        if (r_drawlightcache.GetInt() == 2) {
          CDebugOverlay::AddLineOverlay(mid, pLight->origin, 0, 255, 0, 255,
                                        true, 3);
        }
        return ratio;
      }
    ...
    // cache miss
    // trace a little further for better caching
    flTraceDistance = std::max(100.0, 2.0 * dist);
  ...
  float flHitDistance =
      (pm.startsolid) ? FLT_EPSILON : (pm.fraction) * flTraceDistance;
  ...
  if (dist > flHitDistance + 8) {
    if (r_drawlightcache.GetInt() == 2) {
      CDebugOverlay::AddLineOverlay(mid, pLight->origin, 255, 0, 0, 255, true,
                                    3);
    }
...
static float LightIntensityAndDirectionAtPoint(dworldlight_t *pLight,
                                               ...
                                               Vector *pDirection) {
  if (pZBuf)
    return LightIntensityAndDirectionAtPointNew(pLight, pZBuf, mid, fFlags,
                                                pIgnoreEnt, pDirection);
  else
    return LightIntensityAndDirectionAtPointOld(pLight, mid, fFlags, pIgnoreEnt,
                                                pDirection);
#if 0
...
// a particular point. PVS is used to speed up the task.
//-----------------------------------------------------------------------------
static float LightIntensityAndDirectionInBox(
    dworldlight_t *pLight, lightzbuffer_t *pZBuf, const Vector &mid,
    const Vector &mins, const Vector &maxs, int fFlags, Vector *pDirection) {
  ...
  if (!r_oldlightselection.GetBool()) {
    switch (pLight->type) {
      case emit_spotlight:  // directional & positional
      {
        ...
        if (dist > (sphereRadius + pLight->radius)) return 0;
        // PERFORMANCE: precalc this and store in the light?
        float angle = acos(pLight->stopdot2);
        float sinAngle = sin(angle);
        if (!IsSphereIntersectingCone(mid, sphereRadius, pLight->origin,
                                      pLight->normal, sinAngle,
                                      pLight->stopdot2))
          return 0;
      }
      ...
      case emit_point: {
        ...
        if (distSqr > pLight->radius * pLight->radius) return 0;
      }
      ...
      case emit_surface:  // directional & positional, fixed cone size
      {
        ...
        // PERFORMANCE: precalc this and store in the light?
        if (!IsSphereIntersectingCone(mid, sphereRadius, pLight->origin,
                                      pLight->normal, 1.0f, 0.0f))
          return 0;
      } break;
    }
  } else {
    // NOTE: Here, we do radius check to check to see if we should even care
    // about the light because we want to check the closest point in the box
    switch (pLight->type) {
      case emit_point:
      case emit_spotlight:  // directional & positional
      {
  ...
  return LightIntensityAndDirectionAtPoint(
      pLight, pZBuf, mid, fFlags | LIGHT_NO_RADIUS_CHECK, NULL, pDirection);
}

//-----------------------------------------------------------------------------
// Computes the static vertex lighting term from a large number of spherical
// samples
//-----------------------------------------------------------------------------
bool ComputeVertexLightingFromSphericalSamples(const Vector &vecVertex,
  ...
  for (i = 0; i < host_state.worldbrush->numworldlights; ++i) {
    ...
    // TODO(d.rattman): This is sort of a hack; only one skylight is allowed in
    // the lighting...
    if ((wl->type == emit_skylight) && bHasSkylight) continue;

    // only do it if the entity can see into the lights leaf
    if ((wl->cluster < 0) || (!BIT_SET(pVis, wl->cluster))) continue;

    float flRatio = LightIntensityAndDirectionAtPoint(
        wl, NULL, vecVertex, LIGHT_OCCLUDE_VS_PROPS, pIgnoreEnt, &vecDirection);

    // No light contribution? Get outta here!
    if (flRatio <= 0.0f) continue;

    // Check if we've got a skylight
    if (wl->type == emit_skylight) bHasSkylight = true;

    // Figure out spotlight attenuation
    float flAngularRatio =
        Engine_WorldLightAngle(wl, wl->normal, vecNormal, vecDirection);

    // Add in the direct lighting
    VectorMAInline(*pLinearColor, flAngularRatio * flRatio, wl->intensity,
                   *pLinearColor);
  }
...
static void AddWorldLightToLightCube(dworldlight_t *pWorldLight,
                                     Vector *pBoxColor, const Vector &direction,
                                     float ratio) {
  ...
  for (int j = g_pStudioRender->GetNumAmbientLightSamples(); --j >= 0;) {
    ...
    if (t > 0) {
      VectorMAInline(pBoxColor[j], ratio * t, pWorldLight->intensity,
                     pBoxColor[j]);
    }
  }
}
...
// Adds a world light to the ambient cube
//-----------------------------------------------------------------------------
void AddWorldLightToAmbientCube(dworldlight_t *pWorldLight,
                                const Vector &vecLightingOrigin,
                                AmbientCube_t &ambientCube) {
  Vector vecDirection;
  float ratio = LightIntensityAndDirectionAtPoint(
      pWorldLight, NULL, vecLightingOrigin, 0, NULL, &vecDirection);
  float angularRatio = Engine_WorldLightAngle(pWorldLight, pWorldLight->normal,
                                              vecDirection, vecDirection);
  AddWorldLightToLightCube(pWorldLight, ambientCube, vecDirection,
                           ratio * angularRatio);
}

static inline const byte *FastRejectLightSource(bool bIgnoreVis,
                                                const uint8_t *pVis,
                                                const Vector &bucketOrigin,
                                                int lightType, int lightCluster,
                                                bool &bReject) {
  bReject = false;
  if (!bIgnoreVis) {
    ...
    if (lightType == emit_skylight) {
      ...
      if ((lightCluster < 0) || (!BIT_SET(pVis, lightCluster))) bReject = true;
    }
  }
  return pVis;
}
...
//-----------------------------------------------------------------------------
static const uint8_t *AddWorldLightToLightingState(
    dworldlight_t *pWorldLight, lightzbuffer_t *pZBuf,
    LightingState_t &lightingState, LightingStateInfo_t &info,
    const Vector &bucketOrigin, const byte *pVis, bool dynamic = false,
    bool bIgnoreVis = false, bool bIgnoreVisTest = false) {
  ...
  if (!bIgnoreVisTest) {
    bool bReject;
    pVis =
        FastRejectLightSource(bIgnoreVis, pVis, bucketOrigin, pWorldLight->type,
                              pWorldLight->cluster, bReject);
    if (bReject) return pVis;
  }
  ...
  if (!dynamic && r_oldlightselection.GetBool()) {
    ratio = LightIntensityAndDirectionAtPoint(pWorldLight, pZBuf, bucketOrigin,
                                              0, NULL, &direction);
  } else {
    Vector mins, maxs;
    ComputeLightcacheBounds(bucketOrigin, &mins, &maxs);
    ratio = LightIntensityAndDirectionInBox(
        pWorldLight, pZBuf, bucketOrigin, mins, maxs,
        dynamic ? LIGHT_NO_OCCLUSION_CHECK : 0, &direction);
  }

  // No light contribution? Get outta here!
  if (ratio <= 0.0f) return pVis;
  ...
  float illum =
      ratio * DotProduct(pWorldLight->intensity,
                         s_Grayscale);  // Don't multiply by cone angle?
      ...
      illum >= r_worldlightmin.GetFloat())  // TODO(d.rattman): tune this value
  {
    ...
    if (minLightIndex != -1) {
      // TODO(d.rattman): We're sorting by ratio here instead of illum cause we
      // either have to store more memory or do more computations; I'm not
      ...
      // NOTE: We know the dot product can't be zero or illum would have been 0
      // to start with!
      ...
      if (pWorldLight->type == emit_skylight) {
        ...
        angularRatio = Engine_WorldLightAngle(pWorldLight, pWorldLight->normal,
                                              direction, direction);
      }
    }
  }
  ...
  AddWorldLightToLightCube(pWorldLight, lightingState.r_boxcolor, direction,
                           ratio * angularRatio);
  return pVis;
}
...
static void WorldLightFromDynamicLight(dlight_t const &dynamicLight,
  ...
  worldLight.intensity[0] =
      TexLightToLinear(dynamicLight.color.r, dynamicLight.color.exponent);
  worldLight.intensity[1] =
      TexLightToLinear(dynamicLight.color.g, dynamicLight.color.exponent);
  worldLight.intensity[2] =
      TexLightToLinear(dynamicLight.color.b, dynamicLight.color.exponent);
...
//-----------------------------------------------------------------------------
static const uint8_t *ComputeLightStyles(lightcache_t *pCache,
                                         LightingState_t &lightingState,
                                         const Vector &origin, int leaf,
                                         const byte *pVis) {
...
// Add in dynamic worldlights (lightstyles)
//-----------------------------------------------------------------------------
static void AddLightStylesForStaticProp(PropLightcache_t *pcache,
                                        LightingState_t &lightingState) {
  // Next, add each world light with a lightstyle into the lighting state,
  // ejecting less relevant local lights + folding them into the ambient cube
  for (int i = 0; i < pcache->m_LightStyleWorldLights.Count(); ++i) {
    Assert(pcache->m_LightStyleWorldLights[i] >= 0);
    Assert(pcache->m_LightStyleWorldLights[i] <
           host_state.worldbrush->numworldlights);
    dworldlight_t *wl =
        &host_state.worldbrush->worldlights[pcache->m_LightStyleWorldLights[i]];
    Assert(wl->style != 0);

    // Now add that world light into our list of worldlights
    AddWorldLightToLightingState(wl, NULL, lightingState, *pcache,
                                 pcache->m_LightingOrigin, NULL,
...
static const byte *AddDLights(LightingStateInfo_t &info,
                              LightingState_t &lightingState,
                              const Vector &origin, int leaf,
                              const byte *pVis) {
  if (!g_bActiveDlights) return pVis;
  ...
  for (int i = 0; i < MAX_DLIGHTS; ++i, ++dl) {
    ...
    if ((r_dlightactive & (1 << i)) == 0) continue;

    // If the light doesn't affect models, then continue
    if (dl->flags & (DLIGHT_NO_MODEL_ILLUMINATION | DLIGHT_DISPLACEMENT_MASK))
      continue;

    // Fast reject. If we can reject it here, then we don't have to call
    // WorldLightFromDynamicLight..
    bool bReject;
    int lightCluster =
        CM_LeafCluster(g_DLightLeafAccessors[i].GetLeaf(dl->origin));
    pVis = FastRejectLightSource(bIgnoreVis, pVis, origin, emit_point,
                                 lightCluster, bReject);
    if (bReject) continue;
    ...
    // Now add that world light into our list of worldlights
    pVis = AddWorldLightToLightingState(&s_pDynamicLight[i], NULL,
                                        lightingState, info, origin, pVis, true,
                                        bIgnoreVis, bIgnoreVisTest);
  }
  return pVis;
}

static const byte *AddELights(LightingStateInfo_t &info,
                              LightingState_t &lightingState,
                              const Vector &origin, int leaf,
                              const byte *pVis) {
  if (!g_bActiveElights) return pVis;
  ...
  for (int i = 0; i < MAX_ELIGHTS; ++i, ++dl) {
    ...
    bool bReject;
    int lightCluster =
        CM_LeafCluster(g_ELightLeafAccessors[i].GetLeaf(dl->origin));
    pVis = FastRejectLightSource(bIgnoreVis, pVis, origin, emit_point,
                                 lightCluster, bReject);
    if (bReject) continue;
    ...
    // Now add that world light into our list of worldlights
    pVis = AddWorldLightToLightingState(&s_pDynamicLight[i + MAX_DLIGHTS], NULL,
                                        lightingState, info, origin, pVis, true,
                                        bIgnoreVis, bIgnoreVisTest);
  }
  return pVis;
...
//-----------------------------------------------------------------------------
static const uint8_t *ComputeDynamicLighting(lightcache_t *pCache,
                                             LightingState_t &lightingState,
                                             const Vector &origin, int leaf,
                                             const byte *pVis = 0) {
  ...
  Assert(pCache->m_DynamicLightingState.numlights >= 0 &&
         pCache->m_DynamicLightingState.numlights <= MAXLOCALLIGHTS);
  memcpy(&lightingState, &pCache->m_DynamicLightingState,
         sizeof(LightingState_t));
  return pVis;
}
...
static void AddWorldLightToLightingStateForStaticProps(
  ...
  if (!dynamic) {
    ratio = LightIntensityAndDirectionAtPoint(
        pWorldLight, NULL, pCache->m_LightingOrigin, 0, NULL, &direction);
  } else {
    Vector mins, maxs;
    ComputeLightcacheBounds(pCache->m_LightingOrigin, &mins, &maxs);
    ratio = LightIntensityAndDirectionInBox(
        pWorldLight, NULL, pCache->m_LightingOrigin, pCache->mins, pCache->maxs,
        LIGHT_NO_OCCLUSION_CHECK, &direction);
  }
      ...
      illum >= r_worldlightmin.GetFloat())  // TODO(d.rattman): tune this value
  {
    ...
    if (minLightIndex != -1) {
      ...
      if (pWorldLight->type == emit_skylight) {
        ...
        VectorSubtract(pWorldLight->origin, pCache->m_LightingOrigin,
                       direction);
        VectorNormalize(direction);
...
static void AddDLightsForStaticProps(LightingStateInfo_t &info,
                                     LightingState_t &lightingState,
  ...
  for (int i = 0; i < MAX_DLIGHTS; ++i, ++dl) {
    ...
    AddWorldLightToLightingStateForStaticProps(
        &s_pDynamicLight[i], lightingState, info, pCache, true);
  }
}
...
static void AddStaticLighting(CBaseLightCache *pCache, const Vector &origin,
                              const byte *pVis, bool bStaticProp,
                              bool bAddedLeafAmbientCube) {
  ...
  // NOTE: for static props, we mark lightstyles elsewhere
  // (BuildStaticLightingCacheLightStyleInfo)
  if (!bStaticProp) {
    pCache->m_LightingFlags &=
        ~(HACKLIGHTCACHEFLAGS_HASSWITCHABLELIGHTSTYLE |
          HACKLIGHTCACHEFLAGS_HASNONSWITCHABLELIGHTSTYLE);
  ...
  for (i = 0; i < host_state.worldbrush->numworldlights; ++i) {
    ...
    // Don't add lights without lightstyles... we cache static lighting +
    // lightstyles separately from static lighting
    if (wl->style == 0) {
      // Now add that world light into our list of worldlights
      AddWorldLightToLightingState(wl, pZBuf, pCache->m_StaticLightingState,
                                   *pCache, origin, pVis, false, false);
    } else {
      ...
      if (!bStaticProp) {
        ...
        if (!(pCache->m_pLightstyles[byte] & (1 << bit))) {
          ...
          float ratio = LightIntensityAndDirectionInBox(
              wl, NULL, origin, mins, maxs,
              LIGHT_NO_OCCLUSION_CHECK | LIGHT_IGNORE_LIGHTSTYLE_VALUE,
              &dummyDirection);
...
//-----------------------------------------------------------------------------
static void DebugRenderLightcache(Vector &sampleOrigin,
                                  LightingState_t &lightingState,
                                  bool bDebugModel) {
  ...
  if (r_drawlightcache.GetInt() == 5) {
    if (bDebugModel) {
      CDebugOverlay::AddSphereOverlay(sampleOrigin, 2.5f, 32, 32, 255, 255, 255,
                                      255, 0.0f);  // 8 inch solid white sphere
      ...
      for (int j = 0; j < lightingState.numlights; ++j) {
        ...
        if (lightingState.locallight[j]->type == emit_skylight) {
          vLightPosition =
              sampleOrigin - lightingState.locallight[j]->normal * 10000.0f;
          r = 255;
          g = 50;
          b = 50;
        } else {
        ...
        CDebugOverlay::AddLineOverlay(sampleOrigin, vLightPosition, r, g, b,
                                      255, true, 0.0f);
      }
    }
  } else {
    // draw cache entry
    CDebugOverlay::AddBoxOverlay(cacheOrigin, cacheMins, cacheMaxs, vec3_angle,
                                 255, 255, 255, 0, 0.0f);
    ...
    if (lightingState.numlights > 0) {
      ...
      CDebugOverlay::AddBoxOverlay(sampleOrigin, lightMins, lightMaxs,
                                   vec3_angle, 100, 255, 100, 0, 0.0f);
    }
    ...
    for (int j = 0; j < lightingState.numlights; ++j) {
      ...
      CDebugOverlay::AddLineOverlay(sampleOrigin, vLightPosition, r, g, b, 255,
                                    true, 0.0f);
      CDebugOverlay::AddBoxOverlay(lightingState.locallight[j]->origin,
                                   lightMins, lightMaxs, vec3_angle, 255, 255,
                                   100, 0, 0.0f);
    }
  }
}
...
// Compute the cache...
//-----------------------------------------------------------------------------
static const byte *ComputeStaticLightingForCacheEntry(
    CBaseLightCache *pcache, const Vector &origin, int leaf,
    bool bStaticProp = false) {
  ...
  R_StudioGetAmbientLightForPoint(leaf, origin,
                                  pcache->m_StaticLightingState.r_boxcolor,
                                  bStaticProp, &bAddedLeafAmbientCube);
...
static void BuildStaticLightingCacheLightStyleInfo(PropLightcache_t *pcache,
  ...
  for (short i = 0; i < host_state.worldbrush->numworldlights; ++i) {
    ...
    // TODO(d.rattman): Could do better here if we had access to the list of
    // leaves that this static prop is in.  For now, we use the lighting origin.
    if (pVis[wl->cluster >> 3] & (1 << (wl->cluster & 7))) {
      ...
      Vector dummyDirection;
      float ratio = LightIntensityAndDirectionInBox(
          &tmpLight, NULL, pcache->m_LightingOrigin, mins, maxs,
          LIGHT_NO_OCCLUSION_CHECK | LIGHT_IGNORE_LIGHTSTYLE_VALUE,
          &dummyDirection);
      // See if this light has any contribution on this cache entry.
      if (ratio <= 0.0f) {
...
static ITexture *FindEnvCubemapForPoint(const Vector &origin) {
  worldbrushdata_t *brush_data = host_state.worldbrush;

  if (brush_data && brush_data->m_nCubemapSamples > 0) {
    int smallestIndex = 0;
    Vector v = origin - brush_data->m_pCubemapSamples[0].origin;
    float smallestDist = DotProduct(v, v);

    for (int i = 1; i < brush_data->m_nCubemapSamples; i++) {
      Vector vl = origin - brush_data->m_pCubemapSamples[i].origin;
      float dist = DotProduct(vl, vl);
    ...
    return brush_data->m_pCubemapSamples[smallestIndex].pTexture;
  }
...
LightCacheHandle_t CreateStaticLightingCache(const Vector &origin,
  ...
  pcache->m_Flags =
      0;  // must set this to zero so that this cache entry will be invalid.
    ...
    for (int i = 0; i < pcache->m_LightStyleWorldLights.Count(); ++i) {
      ...
      dworldlight_t *wl =
          &host_state.worldbrush
               ->worldlights[pcache->m_LightStyleWorldLights[i]];
      Assert(wl->style != 0);
      if (d_lightstylenumframes[wl->style] > 1) {
        return true;
      }
...
bool StaticLightCacheNeedsSwitchableLightUpdate(LightCacheHandle_t handle) {
  ...
  if (!pcache->HasSwitchableLightStyle()) {
    ...
    for (int i = 0; i < pcache->m_LightStyleWorldLights.Count(); ++i) {
      ...
      if (d_lightstylenumframes[wl->style] <= 1) {
        // Has it changed since the last time we updated our cached static VB
        // version?
...
void InvalidateStaticLightingCache(void) {
  for (PropLightcache_t *pCur = s_pAllStaticProps; pCur;
       pCur = pCur->m_pNextPropLightcache) {
...
//-----------------------------------------------------------------------------
LightingState_t *LightcacheGetStatic(LightCacheHandle_t cache,
  ...
  bool bRecalcLightStyles =
      (pcache->HasLightStyle() &&
       pcache->m_LastFrameUpdated_LightStyles != r_framecount) &&
      !IsCachedLightStylesValid(pcache);
  bool bRecalcDLights =
      pcache->HasDlights() &&
      pcache->m_LastFrameUpdated_DynamicLighting != r_framecount;

  if (flags != pcache->m_Flags) {
    // This should not happen often, but if the flags change, blow away all of
    // the lighting state. This cache entry's state must be regenerated.
  ...
  if (flags & LIGHTCACHEFLAGS_STATIC) {
    ...
    if (bRecalcStaticLighting && !(pcache->m_LightingFlags &
                                   HACKLIGHTCACHEFLAGS_HASDONESTATICLIGHTING)) {
      ComputeStaticLightingForCacheEntry(pcache, pcache->m_LightingOrigin,
                                         pcache->leaf, true);
  ...
  if (flags & LIGHTCACHEFLAGS_DYNAMIC) {
    if (bRecalcDLights) {
      ...
      AddDLightsForStaticProps(*(LightingStateInfo_t *)pcache, accumulatedState,
                               pcache);
...
inline const uint8_t *AddLightingState(LightingState_t &dst,
                                       const LightingState_t &src,
                                       LightingStateInfo_t &info,
                                       const Vector &bucketOrigin,
                                       const uint8_t *pVis, bool bDynamic,
                                       bool bIgnoreVis) {
  int i;
  for (i = 0; i < src.numlights; i++) {
    pVis =
        AddWorldLightToLightingState(src.locallight[i], NULL, dst, info,
                                     bucketOrigin, pVis, bDynamic, bIgnoreVis);
  }
  for (i = 0; i < 6; i++) {
...
const byte *PrecalcLightingState(lightcache_t *pCache, const uint8_t *pVis) {
  LightingState_t lightingState;
  lightingState.ZeroLightingState();
  ...
  for (i = 0; i < pCache->m_StaticLightingState.numlights; i++) {
    pVis = AddWorldLightToLightingState(
        pCache->m_StaticLightingState.locallight[i], NULL, lightingState,
        pCache->m_StaticPrecalc_LightingStateInfo, pCache->m_LightingOrigin,
        pVis,
        true,  // bDynamic
        false  // bIgnoreVis
    );
...
void CopyPrecalcedLightingState(lightcache_t *pCache,
                                LightingState_t &lightingState,
                                LightingStateInfo_t &info) {
...
void AdjustLightCacheOrigin(lightcache_t *pCache, const Vector &origin,
                            int originLeaf) {
  Vector cacheMins;
  Vector cacheMaxs;
  Vector center;
  ...
  if (centerLeaf != originLeaf) {
    ...
    if (CM_LeafContents(centerLeaf) & MASK_OPAQUE) {
      ...
      // ensure the desired center resides in the leaf that the provided origin
      // is in
  ...
  if (bTraceToCenter && tr.startsolid) {
    ...
    // ensure the desired center resides in the leaf that the provided origin is
    // in
...
bool AllowFullCacheMiss(int flags) {
  ...
  if (flags & LIGHTCACHEFLAGS_ALLOWFAST) return false;
...
ITexture *LightcacheGetDynamic(const Vector &origin,
                               LightingState_t &lightingState,
  ...
  */
  ...
  if (!pCache) {
    ...
    pCache->m_pEnvCubemapTexture =
        FindEnvCubemapForPoint(pCache->m_LightingOrigin);

    // Compute the static portion of the cache
    pVis = ComputeStaticLightingForCacheEntry(pCache, pCache->m_LightingOrigin,
                                              originLeaf);
    pVis = PrecalcLightingState(pCache, pVis);
  }

  // NOTE: On a cache miss, this has to be after
  // ComputeStaticLightingForCacheEntry since these flags are computed there.
  ...
  if (bComputeLightStyles) {
    pVis = ComputeLightStyles(pCache, pCache->m_LightStyleLightingState,
                              pCache->m_LightingOrigin, originLeaf, pVis);
  ...
  if (flags & LIGHTCACHEFLAGS_DYNAMIC) {
    pVis = ComputeDynamicLighting(pCache, pCache->m_DynamicLightingState,
                                  pCache->m_LightingOrigin, originLeaf, pVis);
  ...
  if (flags & LIGHTCACHEFLAGS_LIGHTSTYLE) {
    pVis = AddLightingState(lightingState, pCache->m_LightStyleLightingState,
                            info, pCache->m_LightingOrigin, pVis,
                            true /*bDynamic*/, false /*bIgnoreVis*/);
  }

  if (flags & LIGHTCACHEFLAGS_DYNAMIC) {
    pVis = AddLightingState(lightingState, pCache->m_DynamicLightingState, info,
                            pCache->m_LightingOrigin, pVis, true /*bDynamic*/,
                            false /*bIgnoreVis*/);
  }
...
//-----------------------------------------------------------------------------
void ComputeDynamicLighting(const Vector &pt, const Vector *pNormal,
                            Vector &color) {
  ...
  // Next, add each world light with a lightstyle into the lighting state,
  // ejecting less relevant local lights + folding them into the ambient cube
  static Vector ambientTerm[6] = {Vector(0, 0, 0), Vector(0, 0, 0),
  ...
  if (lightCount) {
    g_pStudioRender->ComputeLighting(ambientTerm, lightCount, pLightDesc, pt,
                                     *pNormal, color);
  } else {
    VectorFill(color, 0);
  }
...
//-----------------------------------------------------------------------------
void ComputeLighting(const Vector &pt, const Vector *pNormal, bool bClamp,
                     Vector &color, Vector *pBoxColors) {
  LightingState_t lightingState;
  LightcacheGetDynamic_Stats stats;
  LightcacheGetDynamic(pt, lightingState, stats,
                       LIGHTCACHEFLAGS_STATIC | LIGHTCACHEFLAGS_DYNAMIC |
                           LIGHTCACHEFLAGS_LIGHTSTYLE |
                           LIGHTCACHEFLAGS_ALLOWFAST);
  int i;
  if (pNormal) {
    LightDesc_t *pLightDesc = (LightDesc_t *)stackalloc(
        lightingState.numlights * sizeof(LightDesc_t));
    ...
    g_pStudioRender->ComputeLighting(lightingState.r_boxcolor,
                                     lightingState.numlights, pLightDesc, pt,
                                     *pNormal, color);
  } else {
    Vector direction;

    for (i = 0; i < lightingState.numlights; ++i) {
      if (IsDynamicLight(lightingState.locallight[i])) continue;

      float ratio = LightIntensityAndDirectionAtPoint(
          lightingState.locallight[i], NULL, pt, LIGHT_NO_OCCLUSION_CHECK, NULL,
          &direction);
      float angularRatio = Engine_WorldLightAngle(
          lightingState.locallight[i], lightingState.locallight[i]->normal,
          direction, direction);
      AddWorldLightToLightCube(lightingState.locallight[i],
                               lightingState.r_boxcolor, direction,
                               ratio * angularRatio);
    }
  ...
  if (pBoxColors) {
    memcpy(pBoxColors, lightingState.r_boxcolor,
           sizeof(lightingState.r_boxcolor));
  }
...
static const uint8_t *s_pDLightVis = NULL;
...
class MarkStaticPropLightsEmumerator : public IPartitionEnumerator {
 public:
  ...
  virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    PropLightcache_t *pCache =
        (PropLightcache_t *)StaticPropMgr()->GetLightCacheHandleForStaticProp(
            pHandleEntity);
    ...
    if (!s_pDLightVis) {
      s_pDLightVis = CM_ClusterPVS(
          CM_LeafCluster(CM_PointLeafnum(cl_dlights[m_nLightID].origin)));
    }
    ...
    if (r_drawlightcache.GetInt() == 4) {
      Vector mins(-5, -5, -5);
      Vector maxs(5, 5, 5);
      CDebugOverlay::AddLineOverlay(cl_dlights[m_nLightID].origin,
                                    pCache->m_LightingOrigin, 0, 0, 255, 255,
                                    true, 0.001f);
      CDebugOverlay::AddBoxOverlay(pCache->m_LightingOrigin, mins, maxs,
                                   vec3_angle, 255, 0, 0, 0, 0.001f);
    }
#endif
...
void MarkDLightsOnStaticProps(void) {
  ...
  for (int i = 0; i < MAX_DLIGHTS; i++, l++) {
    ...
    if (r_drawlightcache.GetInt() == 4) {
      Vector mins(-5, -5, -5);
      Vector maxs(5, 5, 5);
      CDebugOverlay::AddBoxOverlay(l->origin, mins, maxs, vec3_angle, 255, 255,
                                   255, 0, 0.001f);
    }
#endif
    s_pDLightVis = NULL;
    s_MarkStaticPropLightsEnumerator.SetLightID(i);
    SpatialPartition()->EnumerateElementsInSphere(
        PARTITION_ENGINE_STATIC_PROPS, l->origin, l->GetRadius(), true,
...
void InitDLightGlobals(int nMapVersion) {
  if (nMapVersion >= 20) {
    // The light level at which we are close enough to black to treat as black
    // for culling purposes.
    g_flMinLightingValue = 1.0f / 256.0f;
  } else {
    // This is the broken value from HL2.  It is supposed to be
    // the light level at which we are close enough to black to treat as black
    // for culling purposes.  We leave it at the broken value here for old bsp
    // files Since HL2 maps were compiled with this bsp version.
