...
//			flags -
//-----------------------------------------------------------------------------
void CVEfx::DecalShoot(int textureIndex, int entity, const model_t *model,
                       const Vector &model_origin, const QAngle &model_angles,
                       const Vector &position, const Vector *saxis, int flags) {
  color32 white = {255, 255, 255, 255};
  DecalColorShoot(textureIndex, entity, model, model_origin, model_angles,
                  position, saxis, flags, white);
}

void CVEfx::DecalColorShoot(int textureIndex, int entity, const model_t *model,
  ...
  ::R_DecalShoot(textureIndex, entity, model, localPosition, saxis, flags,
                 rgbaColor);
}
...
//			&rgbaColor -
//-----------------------------------------------------------------------------
void CVEfx::PlayerDecalShoot(IMaterial *material, void *userdata, int entity,
                             const model_t *model, const Vector &model_origin,
                             const QAngle &model_angles, const Vector &position,
                             const Vector *saxis, int flags,
                             const color32 &rgbaColor) {
  // Not world?
  ...
  R_PlayerDecalShoot(material, userdata, entity, model, position, saxis, flags,
                     rgbaColor);
}
...
// Output : dlight_t
//-----------------------------------------------------------------------------
dlight_t *CVEfx::CL_AllocDlight(int key) { return ::CL_AllocDlight(key); }
...
// Output : dlight_t
//-----------------------------------------------------------------------------
dlight_t *CVEfx::CL_AllocElight(int key) { return ::CL_AllocElight(key); }

// Given an elight key, find it. Does not search ordinary dlights. May return
// NULL.
