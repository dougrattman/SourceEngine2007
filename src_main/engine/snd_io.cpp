...
int COM_IOReadBinary::open(const ch *pFileName) {
  ch namebuffer[512];
  Q_strncpy(namebuffer, "sound", std::size(namebuffer));

  // the server is sending back sound names with slashes in front...
  if (pFileName[0] != '/' && pFileName[0] != '\\') {
    Q_strncat(namebuffer, "/", std::size(namebuffer), COPY_ALL_CHARACTERS);
  }

  Q_strncat(namebuffer, pFileName, std::size(namebuffer), COPY_ALL_CHARACTERS);
