...
static inline bool EncodeSpecialFloat(const SendProp *pProp, float fVal,
                                      bf_write *pOut) {
...
static inline void EncodeFloat(const SendProp *pProp, float fVal,
  ...
  if (fVal < pProp->m_fLowValue) {
    ...
    if (!(pProp->GetFlags() & SPROP_ROUNDUP)) {
      DataTable_Warning(
          "(class %s): Out-of-range value (%f) in SendPropFloat '%s', "
          "clamping.\n",
...
// anything.
static inline bool DecodeSpecialFloat(SendProp const *pProp, bf_read *pIn,
                                      float &fVal) {
  int flags = pProp->GetFlags();
...
static inline void DecodeVector(SendProp const *pProp, bf_read *pIn, float *v) {
  ...
  if ((pProp->GetFlags() & SPROP_NORMAL) == 0) {
    ...
    float v0v0v1v1 = v[0] * v[0] + v[1] * v[1];
    if (v0v0v1v1 < 1.0f)
...
static inline void DecodeQuaternion(SendProp const *pProp, bf_read *pIn,
                                    float *v) {
  v[0] = DecodeFloat(pProp, pIn);
  v[1] = DecodeFloat(pProp, pIn);
  v[2] = DecodeFloat(pProp, pIn);
...
int DecodeBits(DecodeInfo *pInfo, unsigned char *pOut) {
  ...
  // Read the property in (note: we don't return the bits from here because
  // Decode returns the decoded bits.. we're interested in getting the encoded
  // bits).
  temp = *pInfo->m_pIn;
...
// ----------------------------------------------------------------------------------------
// // Most of the prop types can use this generic FastCopy version. Arrays are a
// bit of a pain.
// ----------------------------------------------------------------------------------------
// //

inline void Generic_FastCopy(const SendProp *pSendProp,
                             const RecvProp *pRecvProp,
                             const unsigned char *pSendData,
                             unsigned char *pRecvData, int objectID) {
  // Get the data out of the ent.
  CRecvProxyData recvProxyData;

  pSendProp->GetProxyFn()(pSendProp, pSendData,
                          pSendData + pSendProp->GetOffset(),
                          &recvProxyData.m_Value, 0, objectID);
  ...
  pRecvProp->GetProxyFn()(&recvProxyData, pRecvData,
                          pRecvData + pRecvProp->GetOffset());
}
...
void Int_Encode(const unsigned char *pStruct, DVariant *pVar,
                const SendProp *pProp, bf_write *pOut, int objectID) {
  if (pProp->IsSigned()) {
...
bool Int_IsZero(const unsigned char *pStruct, DVariant *pVar,
...
void Float_Encode(const unsigned char *pStruct, DVariant *pVar,
                  const SendProp *pProp, bf_write *pOut, int objectID) {
...
int Float_CompareDeltas(const SendProp *pProp, bf_read *p1, bf_read *p2) {
  if (pProp->GetFlags() & SPROP_COORD) {
    ...
    return p1->ReadUBitLong(NORMAL_FRACTIONAL_BITS + 1) !=
           p2->ReadUBitLong(NORMAL_FRACTIONAL_BITS + 1);
  } else {
    return p1->ReadUBitLong(pProp->m_nBits) != p2->ReadUBitLong(pProp->m_nBits);
  }
}
...
bool Float_IsZero(const unsigned char *pStruct, DVariant *pVar,
                  const SendProp *pProp) {
...
void Float_SkipProp(const SendProp *pProp, bf_read *pIn) {
  ...
  if (pProp->GetFlags() & SPROP_COORD) {
    ...
    // this reads two bits, the first bit (bit0 in this word) indicates integer
    // part the second bit (bit1 in this word) indicates the fractional part
    ...
    if (val) {
      ...
      if (val & 1) seekDist += COORD_INTEGER_BITS;

      if (val & 2) seekDist += COORD_FRACTIONAL_BITS;
...
void Vector_Encode(const unsigned char *pStruct, DVariant *pVar,
                   const SendProp *pProp, bf_write *pOut, int objectID) {
...
bool Vector_IsZero(const unsigned char *pStruct, DVariant *pVar,
                   const SendProp *pProp) {
...
void String_Encode(const unsigned char *pStruct, DVariant *pVar,
                   const SendProp *pProp, bf_write *pOut, int objectID) {
...
void String_Decode(DecodeInfo *pInfo) {
  ...
  if (pInfo->m_pRecvProp && len >= DT_MAX_STRING_BUFFERSIZE) {
...
static inline int AreBitsDifferent(bf_read *pBuf1, bf_read *pBuf2, int nBits) {
  ...
  if (nRemainingBits > 0)
    diff |= pBuf1->ReadUBitLong(nRemainingBits) !=
            pBuf2->ReadUBitLong(nRemainingBits);
...
bool String_IsZero(const unsigned char *pStruct, DVariant *pVar,
                   const SendProp *pProp) {
...
int Array_GetLength(const unsigned char *pStruct, const SendProp *pProp,
                    int objectID) {
...
void Array_Encode(const unsigned char *pStruct, DVariant *pVar,
                  const SendProp *pProp, bf_write *pOut, int objectID) {
  SendProp *pArrayProp = pProp->GetArrayProp();
  AssertMsg(pArrayProp,
            ("Array_Encode: missing m_pArrayProp for SendProp '%s'.",
             pProp->m_pVarName));
  ...
  unsigned char *pCurStructOffset =
      (unsigned char *)pStruct + pArrayProp->GetOffset();
  for (int iElement = 0; iElement < nElements; iElement++) {
    DVariant var;

    // Call the proxy to get the value, then encode.
    pArrayProp->GetProxyFn()(pArrayProp, pStruct, pCurStructOffset, &var,
                             iElement, objectID);
    g_PropTypeFns[pArrayProp->GetType()].Encode(pStruct, &var, pArrayProp, pOut,
                                                objectID);
...
void Array_Decode(DecodeInfo *pInfo) {
  ...
  if (pInfo->m_pRecvProp) {
    ...
    // Note we get the OFFSET from the array element property and the STRIDE
    // from the array itself.
    subDecodeInfo.m_pData =
        (char *)pInfo->m_pData + pArrayRecvProp->GetOffset();
  ...
  int nElements =
      pInfo->m_pIn->ReadUBitLong(pInfo->m_pProp->GetNumArrayLengthBits());

  if (lengthProxy) lengthProxy(pInfo->m_pStruct, pInfo->m_ObjectID, nElements);

  for (subDecodeInfo.m_iElement = 0; subDecodeInfo.m_iElement < nElements;
       subDecodeInfo.m_iElement++) {
...
int Array_CompareDeltas(const SendProp *pProp, bf_read *p1, bf_read *p2) {
  SendProp *pArrayProp = pProp->GetArrayProp();
  AssertMsg(pArrayProp,
            ("Array_CompareDeltas: missing m_pArrayProp for SendProp '%s'.",
             pProp->m_pVarName));
...
void Array_FastCopy(const SendProp *pSendProp, const RecvProp *pRecvProp,
                    const unsigned char *pSendData, unsigned char *pRecvData,
                    int objectID) {
  ...
  if (lengthProxy) lengthProxy(pRecvData, objectID, nElements);
  ...
  for (recvProxyData.m_iElement = 0; recvProxyData.m_iElement < nElements;
       recvProxyData.m_iElement++) {
    // Get this array element out of the sender's data.
    pArraySendProp->GetProxyFn()(pArraySendProp, pSendData, pCurSendPos,
                                 &recvProxyData.m_Value,
                                 recvProxyData.m_iElement, objectID);
...
bool Array_IsZero(const unsigned char *pStruct, DVariant *pVar,
                  const SendProp *pProp) {
...
void Array_DecodeZero(DecodeInfo *pInfo) {
  ArrayLengthRecvProxyFn lengthProxy =
      pInfo->m_pRecvProp->GetArrayLengthProxy();
  if (lengthProxy) lengthProxy(pInfo->m_pStruct, pInfo->m_ObjectID, 0);
}

bool Array_IsEncodedZero(const SendProp *pProp, bf_read *pIn) {
  SendProp *pArrayProp = pProp->GetArrayProp();
  AssertMsg(pArrayProp,
            ("Array_IsEncodedZero: missing m_pArrayProp for a property."));
...
void Array_SkipProp(const SendProp *pProp, bf_read *pIn) {
  SendProp *pArrayProp = pProp->GetArrayProp();
  AssertMsg(pArrayProp,
            ("Array_SkipProp: missing m_pArrayProp for a property."));
