...
#else
  if (m_hDemoFile == FILESYSTEM_INVALID_HANDLE) return;
#endif
...
#ifdef DEMO_FILE_UTLBUFFER
  if (!m_Buffer.IsOpen()) return 0;
  if (bRead) return m_Buffer.TellGet();
  return m_Buffer.TellPut();
#else
  if (m_hDemoFile == FILESYSTEM_INVALID_HANDLE) return 0;
...
//-----------------------------------------------------------------------------
// Purpose: Rewind from the current spot by the time stamp, uint8_t code and
// frame counter offsets
//-----------------------------------------------------------------------------
...
#else
  if (m_hDemoFile != FILESYSTEM_INVALID_HANDLE) {
    if (g_pFileSystem)  // can get called during shutdown, with a NULL
                        // g_pFileSystem!
    {
