...
class CGameServer : public CBaseServer {
 ...
 public:
  ...
  inline CGameClient *Client(int i) {
    return static_cast<CGameClient *>(m_Clients[i]);
  };
 ...
 protected:
  ...
  bool FinishCertificateCheck(netadr_t &adr, int nAuthProtocol,
                              const char *szRawCertificate);
  void SendClientDatagrams(int clientCount, CGameClient **clients,
 ...
 public:
  ...
  IChangeInfoAccessor *edictchangeinfo;  // HACK to allow backward compat since
                                         // we can't change edict_t layout
  ...
  bool m_bIsLevelMainMenuBackground;  // true if the level running only as the
                                      // background to the main menu
...
extern int
    g_iServerGameClientsVersion;  // This matches the number at the end of the
                                  // interface name (so for
