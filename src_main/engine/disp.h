...
class CDispInfo : public IDispInfo, public CDispUtilsHelper {
 ...
 public:
  ...
  virtual bool ComputeShadowFragments(DispShadowHandle_t h, int &vertexCount,
                                      int &indexCount);
 ...
 public:
  ...
  CDispInfo *GetDispByIndex(int index) {
    return index == 0xFFFF ? 0 : &m_pDispArray->m_pDispInfos[index];
  }
 ...
 public:
  ...
  void SetNodeIntersectsDecal(CDispDecal *pDispDecal,
                              CVertIndex const &nodeIndex);
  int GetNodeIntersectsDecal(CDispDecal *pDispDecal,
                             CVertIndex const &nodeIndex);
 ...
 public:
  ...
  // This is called from CreateStaticBuffers_All after the CCoreDispInfo is
  // fully initialized. It just copies the data that it needs.
  bool CopyCoreDispData(model_t *pWorld,
                        const MaterialSystem_SortInfo_t *pSortInfos,
                        const CCoreDispInfo *pInfo, bool bRestoring);

  // called by CopyCoreDispData, just copies the vert data
  void CopyCoreDispVertData(const CCoreDispInfo *pInfo,
  ...
  // Rendering functions.
 public:
  // Set m_BBoxMin and m_BBoxMax. Uses g_TempDispVerts and assumes m_LODs have
  // been validated.
  ...
  void DecalProjectVert(Vector const &vPos, CDispDecalBase *pDispDecal,
                        ShadowInfo_t const *pInfo, Vector &out);

  void CullDecals(int iNodeBit, CDispDecal **decals, int nDecals,
 ...
 public:
  ...
  int m_nIndices;  // The actual # of indices being used (it can be less than
                   // m_Indices.Count() if
 ...
 public:
  ...
  // Copied from the ddispinfo. Speciifes where in g_DispLightmapSamplePositions
  // the (compressed) lightmap sample positions start.
...
inline int CDispInfo::VertIndex(CVertIndex const &vert) const {
  Assert(vert.x >= 0 && vert.x < GetSideLength() && vert.y >= 0 &&
         vert.y < GetSideLength());
