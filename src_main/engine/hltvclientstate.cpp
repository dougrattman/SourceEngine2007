...
static ConVar tv_timeout("tv_timeout", "30", 0,
                         "SourceTV connection timeout in seconds.");
ConVar tv_snapshotrate("tv_snapshotrate", "16", 0,
...
void CHLTVClientState::CopyNewEntity(CEntityReadInfo &u, int iClass,
  ...
  PackedEntity *baseline =
      u.m_bAsDelta ? GetEntityBaseline(u.m_nBaseline, ent) : NULL;
  ...
  if (baseline && baseline->m_pClientClass == pClientClass) {
    ...
    ErrorIfNot(GetClassBaseline(iClass, &pFromData, &nFromBits),
  ...
  PackedEntity *pPackedEntity =
      framesnapshotmanager->CreatePackedEntity(pSnapshot, ent);
  ...
  bf_read fromBuf("HLTV_ReadEnterPVS1", pFromData, Bits2Bytes(nFromBits),
                  nFromBits);
  ...
  int nChangedProps =
      RecvTable_MergeDeltas(pClientClass->m_pRecvTable, &fromBuf, u.m_pBuf,
                            &writeBuf, -1, false, changedProps);

  // update change tick in ChangeFrameList
  if (pChangeFrame) {
    pChangeFrame->SetChangeTick(changedProps, nChangedProps,
                                pSnapshot->m_nTickCount);
  }

  if (u.m_bUpdateBaselines) {
    SetEntityBaseline((u.m_nBaseline == 0) ? 1 : 0, pClientClass,
                      u.m_nNewEntity, packedData,
...
static inline void HLTV_CopyExitingEnt(CEntityReadInfo &u) {
  ...
  CFrameSnapshot *pFromSnapshot =
      u.m_pFrom->GetSnapshot();  // get from snapshot
  ...
  Assert(pSnapshot->m_pEntities[ent].m_pPackedData !=
         INVALID_PACKED_ENTITY_HANDLE);
...
bool CHLTVClientState::SetSignonState(int state, int count) {
  ...
  if (!CBaseClientState::SetSignonState(state, count)) return false;
  ...
  switch (m_nSignonState) {
    ...
    case SIGNONSTATE_SPAWN:
      m_pHLTV->SignonComplete();
      break;

    case SIGNONSTATE_FULL:
      m_NetChannel->SetTimeout(tv_timeout.GetFloat());
...
void CHLTVClientState::SendClientInfo(void) {
  ...
  for (int i = 0; i < MAX_CUSTOM_FILES; i++) info.m_nCustomFiles[i] = 0;
...
void CHLTVClientState::SendPacket() {
  ...
  if ((net_time < m_flNextCmdTime) || !m_NetChannel->CanPacket()) return;

  if (IsActive()) {
    NET_Tick tick(m_nDeltaTick, host_frametime_unbounded,
...
bool CHLTVClientState::ProcessSetConVar(NET_SetConVar *msg) {
  if (!CBaseClientState::ProcessSetConVar(msg)) return false;
...
bool CHLTVClientState::ProcessServerInfo(SVC_ServerInfo *msg) {
  ...
  Q_strncpy(m_pHLTV->m_szMapname, msg->m_szMapName,
            sizeof(m_pHLTV->m_szMapname));
  Q_strncpy(m_pHLTV->m_szSkyname, msg->m_szSkyName,
            sizeof(m_pHLTV->m_szSkyname));
...
bool CHLTVClientState::ProcessSetView(SVC_SetView *msg) {
  if (!CBaseClientState::ProcessSetView(msg)) return false;
...
bool CHLTVClientState::ProcessPacketEntities(SVC_PacketEntities *entmsg) {
...
#ifdef _HLTVTEST
  if (g_RecvDecoders.Count() == 0) return false;
#endif
  ...
  CFrameSnapshot *pSnapshot = framesnapshotmanager->CreateEmptySnapshot(
      GetServerTickCount(), entmsg->m_nMaxEntries);
...
void CHLTVClientState::ReadDeltaEnt(CEntityReadInfo &u) {
  ...
  PackedEntity *pToPackedEntity =
      framesnapshotmanager->CreatePackedEntity(pSnapshot, i);

  // WARNING! get pFromPackedEntity after new pPackedEntity has been created,
  // otherwise pointer may be wrong
  PackedEntity *pFromPackedEntity =
      framesnapshotmanager->GetPackedEntity(pFromSnapshot, i);

  pToPackedEntity->SetServerAndClientClass(pFromPackedEntity->m_pServerClass,
                                           pFromPackedEntity->m_pClientClass);
  ...
  int nChangedProps = RecvTable_MergeDeltas(
      pToPackedEntity->m_pClientClass->m_pRecvTable, &fromBuf, u.m_pBuf,
      &writeBuf, -1, false, changedProps);
  ...
  if (m_bSaveMemory) {
    ...
    const char *compressedData = m_pHLTV->CompressPackedEntity(
        pToPackedEntity->m_pServerClass, (char *)writeBuf.GetData(), bits);
...
void CHLTVClientState::UpdateStats() {
  ...
  Q_snprintf(acvar.value, sizeof(acvar.value), "%s:%u",
