...
void R_DrawSkyBox(float zFar, int nDrawFlags /*= 0x3F*/) {
  ...
  for (i = 0; i < 6; i++, nDrawFlags >>= 1) {
    ...
    // Normals are reversed so looking at face dots to 1.0, looking away from is
    // -1.0 Reject backfacing surfaces on the inside of the cube to avoid
    // binding their texture Assuming a 90 fov looking at face is 0 degrees, so
    // reject at 107
