...
static ConVar mod_forcetouchdata(
    "mod_forcetouchdata", "1", 0,
...
bool g_bBakedPropLightingNoSeparateHDR =
    false;  // Some maps only have HDR lighting on props, contained in the file
            // for non-hdr light data
...
class CModelLoader : public IModelLoader {
 ...
 public:
  CModelLoader()
      : m_ModelPool(sizeof(model_t), MAX_KNOWN_MODELS, CMemoryPool::GROW_FAST,
  ...
  // Finds the model, and loads it if it isn't already present.  Updates
  // reference flags
  ...
  // For any models with referencetype blank, frees all memory associated with
  // the model
  ...
  virtual bool LastLoadedMapHasHDRLighting();
  ...
  // Internal types
 private:
  // TODO, flag these and allow for UnloadUnreferencedModels to check for
  // allocation type
 ...
 private:
  ...
  // Byteswap
  int UpdateOrCreate(const char *pSourceName, char *pTargetName, int maxLen,
                     bool bForce);
...
// List of lump files found when map was loaded. Each entry is the lump file
// index for that lump id.
struct lumpfiles_t {
  FileHandle_t file;
  int lumpfileindex;
...
//-----------------------------------------------------------------------------
char *GetMapNameOnDisk(char *pDiskName, const char *pFullMapName,
                       unsigned int nDiskNameSize) {
  // pc names are as is
...
//-----------------------------------------------------------------------------
int CMapLoadHelper::GetRefCount() { return s_nMapLoadRecursion; }
...
void CMapLoadHelper::Init(model_t *pMapModel, const char *loadname) {
  ...
  if (s_MapHeader.version < MINBSPVERSION || s_MapHeader.version > BSPVERSION) {
    ...
    s_MapFileHandle = FILESYSTEM_INVALID_HANDLE;
    Host_Error(
        "CMapLoadHelper::Init, map %s has wrong version (%i when expecting "
        "%i)\n",
  ...
  if (IsPC()) {
    ...
    for (int iIndex = 0; iIndex < MAX_LUMPFILES; iIndex++) {
      ...
      if (lumpHeader.lumpID >= 0 && lumpHeader.lumpID < HEADER_LUMPS) {
        ...
        memcpy(&(s_MapLumpFiles[lumpHeader.lumpID].header), &lumpHeader,
               sizeof(lumpHeader));
      } else {
...
//-----------------------------------------------------------------------------
void CMapLoadHelper::InitFromMemory(model_t *pMapModel, const void *pData,
                                    int nDataSize) {
  // valid for 360 only
...
//-----------------------------------------------------------------------------
void CMapLoadHelper::LoadLumpElement(int nElemIndex, int nElemSize,
                                     void *pData) {
...
CMapLoadHelper::CMapLoadHelper(int lumpToLoad) {
  if (lumpToLoad < 0 || lumpToLoad >= HEADER_LUMPS) {
    Sys_Error("Can't load lump %i, range is 0 to %i!!!", lumpToLoad,
              HEADER_LUMPS - 1);
  }
  ...
  if (s_MapBuffer.Base()) {
    ...
    g_pFileSystem->GetOptimalIOConstraints(fileToUse, &nOffsetAlign,
                                           &nSizeAlign, &nBufferAlign);

    bool bTryOptimal =
        (m_nLumpOffset % 4 == 0);  // Don't return badly aligned data
    unsigned int alignedOffset = m_nLumpOffset;
    unsigned int alignedBytesToRead = ((m_nLumpSize) ? m_nLumpSize : 1);

    if (bTryOptimal) {
      alignedOffset =
          AlignValue((alignedOffset - nOffsetAlign) + 1, nOffsetAlign);
      alignedBytesToRead = AlignValue(
          (m_nLumpOffset - alignedOffset) + alignedBytesToRead, nSizeAlign);
    }

    m_pRawData = (uint8_t *)g_pFileSystem->AllocOptimalReadBuffer(
        fileToUse, alignedBytesToRead, alignedOffset);
    if (!m_pRawData && m_nLumpSize) {
      Sys_Error("Can't load lump %i, allocation of %i bytes failed!!!",
                lumpToLoad, m_nLumpSize + 1);
    }

    if (m_nLumpSize) {
      g_pFileSystem->Seek(fileToUse, alignedOffset, FILESYSTEM_SEEK_HEAD);
      g_pFileSystem->ReadEx(m_pRawData, alignedBytesToRead, alignedBytesToRead,
                            fileToUse);
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CMapLoadHelper::GetMapName(void) { return s_szMapName; }
...
char *CMapLoadHelper::GetDiskName(void) {
  return GetMapNameOnDisk(s_szMapNameOnDisk, GetLoadName(),
                          sizeof(s_szMapNameOnDisk));
}
...
// Output : byte
//-----------------------------------------------------------------------------
uint8_t *CMapLoadHelper::LumpBase(void) { return m_pData; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : int
//-----------------------------------------------------------------------------
int CMapLoadHelper::LumpSize() { return m_nLumpSize; }

int CMapLoadHelper::LumpOffset() { return m_nLumpOffset; }

int CMapLoadHelper::LumpVersion() const { return m_nLumpVersion; }

void EnableHDR(bool bEnable) {
  if (g_pMaterialSystemHardwareConfig->GetHDREnabled() == bEnable) return;
...
void Map_CheckFeatureFlags() {
  ...
  if (CMapLoadHelper::LumpSize(LUMP_MAP_FLAGS) > 0) {
    ...
    g_bLoadedMapHasBakedPropLighting =
        (flags_lump.m_LevelFlags &
         LVLFLAGS_BAKED_STATIC_PROP_LIGHTING_NONHDR) != 0 ||
        (flags_lump.m_LevelFlags & LVLFLAGS_BAKED_STATIC_PROP_LIGHTING_HDR) !=
            0;
...
bool Map_CheckForHDR(model_t *pModel, const char *pLoadName) {
  ...
  // might want to also consider the game lumps
  ...
  //			 Mod_GameLumpSize( GAMELUMP_DETAIL_PROP_LIGHTING_HDR ) >
  // 0
  ////  fixme
  if (s_MapHeader.version >= 20 &&
      CMapLoadHelper::LumpSize(LUMP_LEAF_AMBIENT_LIGHTING_HDR) == 0) {
    // This lump only exists in version 20 and greater, so don't bother checking
    // for it on earlier versions.
    bHasHDR = false;
  }
...
void Mod_LoadWorldlights(CMapLoadHelper &lh, bool bIsHDR) {
  ...
  lh.GetMap()->worldlights = (dworldlight_t *)Hunk_AllocName(
      lh.LumpSize(), va("%s [%s]", lh.GetLoadName(), "worldlights"));
  ...
  if (r_lightcache_zbuffercache.GetInt()) {
    ...
    lh.GetMap()->shadowzbuffers = (lightzbuffer_t *)Hunk_AllocName(
        zbufSize, va("%s [%s]", lh.GetLoadName(), "shadowzbuffers"));
  ...
  for (int i = 0; i < lh.GetMap()->numworldlights; i++) {
    dworldlight_t &world_light{lh.GetMap()->worldlights[i]};

    if (world_light.type == emit_spotlight) {
      if ((world_light.constant_attn == 0.0) &&
          (world_light.linear_attn == 0.0) &&
          (world_light.quadratic_attn == 0.0)) {
        world_light.quadratic_attn = 1.0;
      }

      if (world_light.exponent == 0.0) world_light.exponent = 1.0;
    } else if (world_light.type == emit_point) {
    ...
    if (world_light.radius < 1) {
      world_light.radius = ComputeLightRadius(&world_light, bIsHDR);
    }
  }
}
...
void Mod_LoadVertices(void) {
  ...
  out = (mvertex_t *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "vertexes"));
...
void Mod_LoadSubmodels(CUtlVector<mmodel_t> &submodelList) {
  ...
  for (i = 0; i < count; i++, in++) {
    ...
    submodelList[i].radius =
        RadiusFromBounds(submodelList[i].mins, submodelList[i].maxs);
  ...
  switch (lh.LumpVersion()) {
    case LUMP_OCCLUSION_VERSION: {
      ...
      if (b->numoccluders) {
        ...
        b->occluders =
            (doccluderdata_t *)Hunk_AllocName(nSize, "occluder data");
      ...
      if (b->numoccluderpolys) {
        ...
        b->occluderpolys =
            (doccluderpolydata_t *)Hunk_AllocName(nSize, "occluder poly data");
      ...
      if (b->numoccludervertindices) {
        ...
        b->occludervertindices =
            (int *)Hunk_AllocName(nSize, "occluder vertices");
...
void Mod_LoadTexdata(void) {
  // Don't bother loading these again; they're already stored in the collision
  // model which is guaranteed to be loaded at this point
...
void Mod_LoadTexinfo(void) {
  ...
  count = lh.LumpSize() / sizeof(*in);
  out = (mtexinfo_t *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "texinfo"));
  ...
  for (i = 0; i < count; ++i, ++in, ++out) {
    for (j = 0; j < 2; ++j) {
      for (int k = 0; k < 4; ++k) {
        out->textureVecsTexelsPerWorldUnits[j][k] =
            in->textureVecsTexelsPerWorldUnits[j][k];
        out->lightmapVecsLuxelsPerWorldUnits[j][k] =
            in->lightmapVecsLuxelsPerWorldUnits[j][k];
      }
    }

    // assume that the scale is the same on both s and t.
    out->luxelsPerWorldUnit =
        VectorLength(out->lightmapVecsLuxelsPerWorldUnits[0].AsVector3D());
    ...
    if (loadtextures) {
      if (in->texdata >= 0) {
        out->material = GL_LoadMaterial(lh.GetMap()->texdata[in->texdata].name,
                                        TEXTURE_GROUP_WORLD);
      } else {
...
static void CheckSurfaceLighting(SurfaceHandle_t surfID,
  ...
  if (!pLighting->m_pSamples) return;
  ...
  for (int maps = maxLightmapIndex; maps != 0; maps--) {
    ...
    if (color[0] <= minLightVal && color[1] <= minLightVal &&
        color[2] <= minLightVal) {
      // found a lightmap that is too dark, remove it and shift over the
      // subsequent maps/styles
      for (int i = maps; i < maxLightmapIndex; i++) {
        ColorRGBExp32 *pLightmapOverwrite =
            pLighting->m_pSamples + (i * offset);
        memcpy(pLightmapOverwrite, pLightmapOverwrite + offset,
               offset * sizeof(*pLightmapOverwrite));
...
static void CalcSurfaceExtents(CMapLoadHelper &lh, SurfaceHandle_t surfID) {
  ...
  for (i = 0; i < MSurf_VertCount(surfID); i++) {
    ...
    for (j = 0; j < 2; j++) {
      ...
      if (val < textureMins[j]) textureMins[j] = val;
      if (val > textureMaxs[j]) textureMaxs[j] = val;
    }
  }

  for (i = 0; i < 2; i++) {
    if (MSurf_LightmapExtents(surfID, pBrushData)[i] == 0 &&
        !MSurf_Samples(surfID, pBrushData)) {
    ...
    if (!(tex->flags & SURF_NOLIGHT) &&
        MSurf_LightmapExtents(surfID, pBrushData)[i] >
            MSurf_MaxLightmapSizeWithBorder(surfID)) {
...
void Mod_LoadVertNormals(void) {
  ...
  Vector *out = (Vector *)Hunk_AllocName(
      lh.LumpSize(), va("%s [%s]", lh.GetLoadName(), "vertnormals"));
...
void Mod_LoadVertNormalIndices(void) {
  ...
  unsigned short *out = (unsigned short *)Hunk_AllocName(
      lh.LumpSize(), va("%s [%s]", lh.GetLoadName(), "vertnormalindices"));
...
void Mod_LoadPrimitives(void) {
  ...
  count = lh.LumpSize() / sizeof(*in);
  out = (mprimitive_t *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "primitives"));
...
void Mod_LoadPrimVerts(void) {
  ...
  count = lh.LumpSize() / sizeof(*in);
  out = (mprimvert_t *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "primverts"));
...
void Mod_LoadPrimIndices(void) {
  ...
  count = lh.LumpSize() / sizeof(*in);
  out = (unsigned short *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "primindices"));
...
void Mod_LoadLump(model_t *loadmodel, int iLump, char *loadname,
                  int elementSize, void **ppData, int *nElements) {
...
//-----------------------------------------------------------------------------
bool Mod_LoadSurfaceLightingV1(msurfacelighting_t *pLighting, dface_t *in,
                               ColorRGBExp32 *pBaseLightData) {
  ...
  if ((i == -1) || (!pBaseLightData)) {
    ...
    pLighting->m_pSamples = (ColorRGBExp32 *)(((uint8_t *)pBaseLightData) + i);
  ...
  return ((pLighting->m_nStyles[0] != 0) && (pLighting->m_nStyles[0] != 255)) ||
         (pLighting->m_nStyles[1] != 255);
}

void *Hunk_AllocNameAlignedClear_(int size, int alignment,
                                  const char *pHunkName) {
...
// Allocates a block of T from the hunk.  Aligns as specified and clears the
// memory
template <typename T>
T *Hunk_AllocNameAlignedClear(int count, int alignment, const char *pHunkName) {
  return (T *)Hunk_AllocNameAlignedClear_(alignment + count * sizeof(T),
                                          alignment, pHunkName);
}
//-----------------------------------------------------------------------------
// Purpose:
...
void Mod_LoadFaces(void) {
  ...
  msurface2_t *out2 = Hunk_AllocNameAlignedClear<msurface2_t>(
      count, 32, va("%s [%s]", lh.GetLoadName(), "surface2"));

  msurfacelighting_t *pLighting =
      Hunk_AllocNameAlignedClear<msurfacelighting_t>(
          count, 32, va("%s [%s]", lh.GetLoadName(), "surfacelighting"));

  lh.GetMap()->surfaces1 = out1;
  lh.GetMap()->surfaces2 = out2;
  lh.GetMap()->surfacelighting = pLighting;
  lh.GetMap()->surfacenormals = Hunk_AllocNameAlignedClear<msurfacenormal_t>(
      count, 2, va("%s [%s]", lh.GetLoadName(), "surfacenormal"));
  lh.GetMap()->numsurfaces = count;

  worldbrushdata_t *pBrushData = lh.GetMap();

  for (surfnum = 0; surfnum < count;
       ++surfnum, ++in, ++out1, ++out2, ++pLighting) {
...
static void CheckSmallVolumeDifferences(mnode_t *pNode,
                                        const Vector &parentSize) {
  if (pNode->contents >= 0) return;
...
void Mod_LoadNodes(void) {
  ...
  count = lh.LumpSize() / sizeof(*in);
  out = (mnode_t *)Hunk_AllocName(count * sizeof(*out),
                                  va("%s [%s]", lh.GetLoadName(), "nodes"));
  ...
  for (i = 0; i < count; ++i, ++pNode) {
    if (pNode->contents == -1) {
      if ((pNode->m_vecHalfDiagonal.x <= 50) &&
          (pNode->m_vecHalfDiagonal.y <= 50) &&
        ...
        CheckSmallVolumeDifferences(pNode->children[0],
                                    pNode->m_vecHalfDiagonal);
        CheckSmallVolumeDifferences(pNode->children[1],
                                    pNode->m_vecHalfDiagonal);
      }
    }
  }
...
void Mod_LoadLeafs_Version_0(CMapLoadHelper &lh) {
  ...
  count = lh.LumpSize() / sizeof(*in);
  out = (mleaf_t *)Hunk_AllocName(count * sizeof(*out),
                                  va("%s [%s]", lh.GetLoadName(), "leafs"));

  lh.GetMap()->leafs = out;
  lh.GetMap()->numleafs = count;

  // one sample per leaf
  lh.GetMap()->m_pLeafAmbient = (mleafambientindex_t *)Hunk_AllocName(
      count * sizeof(*lh.GetMap()->m_pLeafAmbient), "LeafAmbient");
  lh.GetMap()->m_pAmbientSamples = (mleafambientlighting_t *)Hunk_AllocName(
  ...
  for (i = 0; i < count; i++, in++, out++) {
    ...
    pSamples[i].pad = 0;
    Q_memcpy(&pSamples[i].cube, &in->m_AmbientLighting,
             sizeof(pSamples[i].cube));
    ...
    out->flags = in->flags;
    /*
                    out->firstmarksurface = lh.GetMap()->marksurfaces +
       in->firstleafface;
    */
...
//			*loadname -
//-----------------------------------------------------------------------------
void Mod_LoadLeafs_Version_1(CMapLoadHelper &lh,
                             CMapLoadHelper &ambientLightingLump,
  ...
  if (ambientLightingLump.LumpVersion() != LUMP_LEAF_AMBIENT_LIGHTING_VERSION ||
      ambientLightingTable.LumpSize() == 0) {
    ...
    if (ambientLightingLump.LumpSize()) {
      ...
      Assert(ambientLightingLump.LumpSize() / sizeof(CompressedLightCube) ==
             lh.LumpSize() / sizeof(dleaf_t));
    }
    lh.GetMap()->m_pLeafAmbient = (mleafambientindex_t *)Hunk_AllocName(
        count * sizeof(*lh.GetMap()->m_pLeafAmbient), "LeafAmbient");
    lh.GetMap()->m_pAmbientSamples = (mleafambientlighting_t *)Hunk_AllocName(
    ...
    Assert(ambientLightingLump.LumpSize() % sizeof(dleafambientlighting_t) ==
           0);
    Assert(ambientLightingTable.LumpSize() % sizeof(dleafambientindex_t) == 0);
    Assert((ambientLightingTable.LumpSize() / sizeof(dleafambientindex_t)) ==
           (unsigned)count);  // should have one of these per leaf
    lh.GetMap()->m_pLeafAmbient = (mleafambientindex_t *)Hunk_AllocName(
        ambientLightingTable.LumpSize(), "LeafAmbient");
    lh.GetMap()->m_pAmbientSamples = (mleafambientlighting_t *)Hunk_AllocName(
        ambientLightingLump.LumpSize(), "LeafAmbientSamples");
    Q_memcpy(lh.GetMap()->m_pLeafAmbient, ambientLightingTable.LumpBase(),
             ambientLightingTable.LumpSize());
    Q_memcpy(lh.GetMap()->m_pAmbientSamples, ambientLightingLump.LumpBase(),
             ambientLightingLump.LumpSize());
  }
...
void Mod_LoadLeafWaterData(void) {
  ...
  if (lh.LumpSize() % sizeof(*in))
    Host_Error("Mod_LoadLeafs: funny lump size in %s", lh.GetMapName());
  count = lh.LumpSize() / sizeof(*in);
  out = (mleafwaterdata_t *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "leafwaterdata"));
...
void Mod_LoadCubemapSamples(void) {
  ...
  if (lh.LumpSize() % sizeof(*in))
    Host_Error("Mod_LoadCubemapSamples: funny lump size in %s",
               lh.GetMapName());
  count = lh.LumpSize() / sizeof(*in);
  out = (mcubemapsample_t *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "cubemapsample"));
  ...
  for (i = 0; i < count; i++, in++, out++) {
    out->origin.Init((float)in->origin[0], (float)in->origin[1],
                     (float)in->origin[2]);
    out->size = in->size;
    Q_snprintf(textureName, sizeof(textureName), "maps/%s/c%d_%d_%d%s",
               loadName, (int)in->origin[0], (int)in->origin[1],
               (int)in->origin[2], pHDRExtension);
    out->pTexture =
        materials->FindTexture(textureName, TEXTURE_GROUP_CUBE_MAP, true);
    if (IsErrorTexture(out->pTexture)) {
      if (g_pMaterialSystemHardwareConfig->GetHDRType() != HDR_TYPE_NONE) {
        Warning("Couldn't get HDR '%s' -- ", textureName);
        // try non hdr version
        Q_snprintf(textureName, sizeof(textureName), "maps/%s/c%d_%d_%d",
                   loadName, (int)in->origin[0], (int)in->origin[1],
                   (int)in->origin[2]);
      ...
      if (IsErrorTexture(out->pTexture)) {
        Q_snprintf(textureName, sizeof(textureName), "maps/%s/cubemapdefault",
                   loadName);
        out->pTexture =
            materials->FindTexture(textureName, TEXTURE_GROUP_CUBE_MAP, true);
        if (IsErrorTexture(out->pTexture)) {
          out->pTexture = materials->FindTexture("engine/defaultcubemap",
                                                 TEXTURE_GROUP_CUBE_MAP, true);
        }
  ...
  if (count) {
    pRenderContext->BindLocalCubemap(
        lh.GetMap()->m_pCubemapSamples[0].pTexture);
  } else {
    ...
    if (IsErrorTexture(pTexture)) {
      pTexture = materials->FindTexture("engine/defaultcubemap",
                                        TEXTURE_GROUP_CUBE_MAP, true);
    }
...
void Mod_LoadLeafMinDistToWater(void) {
  ...
  if (!foundOne || lh.LumpSize() == 0 || !g_pMaterialSystemHardwareConfig ||
    ...
    in = (unsigned short *)lh.LumpBase();
    if (lh.LumpSize() % sizeof(*in))
      Host_Error("Mod_LoadLeafMinDistToWater: funny lump size in %s",
                 lh.GetMapName());
    count = lh.LumpSize() / sizeof(*in);
    out = (unsigned short *)Hunk_AllocName(
        count * sizeof(*out),
        va("%s [%s]", lh.GetLoadName(), "leafmindisttowater"));
...
void Mod_LoadMarksurfaces(void) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!SurfaceHasDispInfo(surfID) &&
        !(MSurf_Flags(surfID) & SURFDRAW_NODRAW)) {
      realCount++;
    }
  }

  // now allocate the permanent list, and copy the non-terrain, non-nodraw surfs
  // into it
  SurfaceHandle_t *surfList = (SurfaceHandle_t *)Hunk_AllocName(
      realCount * sizeof(SurfaceHandle_t),
      va("%s [%s]", lh.GetLoadName(), "surfacehandle"));
  ...
  for (i = 0; i < pBrushData->numleafs; i++) {
    ...
    for (int j = 0; j < pLeaf[i].nummarksurfaces; j++) {
      // write a new copy of the mark surfaces for this leaf, strip out the
      // nodraw & terrain
...
void Mod_LoadSurfedges(medge_t *pedges) {
  ...
  if (count < 1 || count >= MAX_MAP_SURFEDGES)
    Host_Error("Mod_LoadSurfedges: bad surfedges count in %s: %i",
               lh.GetMapName(), count);
  out = (unsigned short *)Hunk_AllocName(
      count * sizeof(*out), va("%s [%s]", lh.GetLoadName(), "surfedges"));
...
bool Mod_LoadGameLump(int lumpId, void *pOutBuffer, int size) {
  ...
  if (bIsCompressed) {
    ...
    // compressed lump data length is determined from next dictionary entry
    // offset
  ...
  if (s_MapBuffer.Base()) {
    ...
    g_pFileSystem->Seek(fileHandle, g_GameLumpDict[i].fileofs,
...
void Mod_TouchAllData(model_t *pModel, int nServerCount) {
  ...
  if (pVirtualModel && nServerCount >= 1) {
    ...
    for (int i = 1; i < pVirtualModel->m_group.Count(); ++i) {
      MDLHandle_t childHandle =
          (MDLHandle_t)(uintptr_t)pVirtualModel->m_group[i].cache;
      model_t *pChildModel = (model_t *)g_pMDLCache->GetUserData(childHandle);
      if (pChildModel) {
        // child inherits parent reference
        pChildModel->nLoadFlags |=
            (pModel->nLoadFlags & IModelLoader::FMODELLOADER_REFERENCEMASK);
        pChildModel->nLoadFlags |= IModelLoader::FMODELLOADER_LOADED;
        pChildModel->nLoadFlags &=
            ~IModelLoader::FMODELLOADER_LOADED_BY_PRELOAD;
  ...
  if (!mod_forcetouchdata.GetBool()) return;
...
void CMDLCacheNotify::ComputeModelFlags(model_t *pModel, MDLHandle_t handle) {
  ...
  pModel->flags &=
      ~(MODELFLAG_TRANSLUCENT_TWOPASS | MODELFLAG_VERTEXLIT |
        MODELFLAG_TRANSLUCENT | MODELFLAG_MATERIALPROXY |
        MODELFLAG_FRAMEBUFFER_TEXTURE | MODELFLAG_STUDIOHDR_USES_FB_TEXTURE |
        MODELFLAG_STUDIOHDR_USES_BUMPMAPPING |
        MODELFLAG_STUDIOHDR_USES_ENV_CUBEMAP);
  ...
  for (int i = 0; i < materialCount; ++i) {
    ...
    if (pMaterial->NeedsPowerOfTwoFrameBufferTexture(false))  // The false
                                                              // checks if it
                                                              // will ever need
                                                              // the frame
                                                              // buffer, not
...
// Sets the bounds from the studiohdr
//-----------------------------------------------------------------------------
void CMDLCacheNotify::SetBoundsFromStudioHdr(model_t *pModel,
                                             MDLHandle_t handle) {
  studiohdr_t *pStudioHdr = g_pMDLCache->GetStudioHdr(handle);
...
// Callbacks to get called when various data is loaded or unloaded
//-----------------------------------------------------------------------------
void CMDLCacheNotify::OnDataLoaded(MDLCacheDataType_t type,
                                   MDLHandle_t handle) {
  ...
  switch (type) {
    ...
    case MDLCACHE_VCOLLIDE: {
      ...
      // Expand the model bounds to enclose the collision model (should be done
      // in studiomdl)
      vcollide_t *pCollide = g_pMDLCache->GetVCollide(handle);
      if (pCollide) {
        Vector mins, maxs;
        physcollision->CollideGetAABB(&mins, &maxs, pCollide->solids[0],
...
void CMDLCacheNotify::OnDataUnloaded(MDLCacheDataType_t type,
                                     MDLHandle_t handle) {}

//-----------------------------------------------------------------------------
// Hooks the cache notify into the MDL cache system
//-----------------------------------------------------------------------------
void ConnectMDLCacheNotify() { g_pMDLCache->SetCacheNotify(&s_MDLCacheNotify); }

void DisconnectMDLCacheNotify() { g_pMDLCache->SetCacheNotify(NULL); }
...
class CResourcePreloadModel : public CResourcePreload {
  static void QueuedLoaderMapCallback(void *pContext, void *pContext2,
  ...
  virtual bool CreateResource(const char *pName) {
    ...
    // each model type resource has entirely differnt schemes for
    // loading/creating
    if (modType == mod_brush) {
      ...
      if (pMapModel) {
        ...
        V_snprintf(szNameOnDisk, sizeof(szNameOnDisk), "maps/graphs/%s.360.ain",
                   szLoadName);
      ...
      if (pModel->nLoadFlags & (IModelLoader::FMODELLOADER_LOADED |
                                IModelLoader::FMODELLOADER_LOADED_BY_PRELOAD)) {
      ...
      // mark the model so that the normal studio load path can perform a final
      // fixup
      ...
      // if this hits, the mdlcache has the model, but the modelloader doesn't
      // think so if the refcounts go haywire, bad evil bugs will occur
  ...
  // memory to ensure i/o requests have enough memory to succeed. The models
  // that were touched by the CreateResource() are the ones to keep, all others
  // get purged.
  //-----------------------------------------------------------------------------
  virtual void PurgeUnreferencedResources() {
    ...
    for (;;) {
      ...
      if (pModel->type == mod_studio) {
        ...
        if (pModel->nLoadFlags &
            IModelLoader::FMODELLOADER_TOUCHED_BY_PRELOAD) {
          pModel->nLoadFlags &= ~IModelLoader::FMODELLOADER_TOUCHED_BY_PRELOAD;
        } else {
          if (bSpew) {
          ...
          if (g_pMDLCache->IsDataLoaded(pModel->studio,
...
int CModelLoader::GetCount(void) { return m_Models.Count(); }
...
model_t *CModelLoader::FindModel(const char *pName) {
  ...
  if (pName[0] == '*') {
    ...
    if (!IsWorldModelSet()) {
      Sys_Error("bad inline model number %i, worldmodel not yet setup",
                modelNum);
    }
  ...
  if (i == m_Models.InvalidIndex()) {
    ...
    // Various other subsystems fetch this 'object' name to do dictionary
    // lookups, which are usually case insensitive, but not to slashes or
    // dotslashes.
  ...
  // notify the reslist generator that this model may be referenced later in the
  // level (does nothing if reslist generation is not enabled)
...
//-----------------------------------------------------------------------------
// Purpose: Finds the model, and loads it if it isn't already present.  Updates
// reference flags Input  : *name -
//			referencetype -
// Output : model_t
//-----------------------------------------------------------------------------
model_t *CModelLoader::GetModelForName(const char *name,
...
//			referencetype -
//-----------------------------------------------------------------------------
model_t *CModelLoader::ReferenceModel(const char *name,
                                      REFERENCETYPE referencetype) {
  model_t *model = FindModel(name);
...
model_t *CModelLoader::LoadModel(model_t *mod, REFERENCETYPE *pReferencetype) {
  ...
  // at load end, models that have a mismatch count are considered candidates
  // for purge models that get marked, touch *all* their sub data to ensure the
  // cache is pre-populated and hitches less during gameplay
  ...
  // The model type will not be set for first time models that need to fall
  // through to the load path. A model that needs a post precache fixup will
  // fall through to the load path.
  if (mod->type == mod_studio &&
      !(mod->nLoadFlags & FMODELLOADER_LOADED_BY_PRELOAD)) {
    // in cache
  ...
  switch (mod->type) {
    ...
    case mod_brush: {
      ...
      // This is necessary on dedicated clients. On listen + dedicated servers,
      ...
      // need this before queued loader starts, various systems use this as a
      // cheap map changed state
...
static void BuildSpriteLoadName(const char *pName, char *pOut, int outLen,
  ...
  if ((bIsAVI || bIsBIK || bIsVMT) &&
      (strchr(pName, '/') || strchr(pName, '\\'))) {
...
//			referencetype -
//-----------------------------------------------------------------------------
void CModelLoader::UnreferenceModel(model_t *model,
                                    REFERENCETYPE referencetype) {
  model->nLoadFlags &= ~referencetype;
}
...
void CModelLoader::UnreferenceAllModels(REFERENCETYPE referencetype) {
  ...
  // UNDONE: If we ever free a studio model, write code to free the collision
  // data UNDONE: Reference count collision data?
...
void CModelLoader::ReloadFilesInList(IFileList *pFilesToReload) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (pModel->type != mod_studio) continue;

    if (!IsLoaded(pModel)) continue;
    ...
    if (pFilesToReload->IsFileInList(pModel->szName)) {
      ...
      g_pMDLCache->Flush(pModel->studio, (int)(MDLCACHE_FLUSH_ALL &
      ...
      if (g_pMDLCache->IsDataLoaded(pModel->studio, MDLCACHE_STUDIOHWDATA)) {
        ...
        if (pStudioHdr) {
          // Ok, we didn't have to do a full reload, but if any of our materials
          // changed, flush out the studiohwdata because the vertex format may
          // have changed.
...
//-----------------------------------------------------------------------------
// Purpose: For any models with referencetype blank (if checking), frees all
// memory associated with the model and frees up the models slot
//-----------------------------------------------------------------------------
void CModelLoader::UnloadAllModels(bool bCheckReference) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (model->nLoadFlags &
        (FMODELLOADER_LOADED | FMODELLOADER_LOADED_BY_PRELOAD)) {
      UnloadModel(model);
    }
  }
}
...
void CModelLoader::PurgeUnusedModels(void) {
  ...
  for (int i = 0; i < count; i++) {
    model_t *pModel = m_Models[i].modelpointer;
    if ((pModel->nLoadFlags & FMODELLOADER_LOADED) &&
        (pModel->nServerCount != nServerCount)) {
...
static void Mod_ComputeBrushModelFlags(model_t *mod) {
  ...
  mod->flags &= ~(MODELFLAG_MATERIALPROXY | MODELFLAG_TRANSLUCENT |
                  MODELFLAG_FRAMEBUFFER_TEXTURE);
  ...
  for (i = 0; i < scount; ++i) {
    SurfaceHandle_t surfID =
        SurfaceHandleFromIndex(mod->brush.firstmodelsurface + i, pBrushData);

    // Clear out flags we're going to set
    MSurf_Flags(surfID) &= ~(SURFDRAW_NOCULL | SURFDRAW_TRANS |
                             SURFDRAW_ALPHATEST | SURFDRAW_NODECALS);
    ...
    if ((pTex->flags & SURF_NODECALS) ||
        pMaterial->GetMaterialVarFlag(MATERIAL_VAR_SUPPRESS_DECALS) ||
...
//-----------------------------------------------------------------------------
void Mod_RecomputeTranslucency(model_t *mod, int nSkin, int nBody,
  ...
  switch (mod->type) {
    case mod_brush: {
      for (int i = 0; i < mod->brush.nummodelsurfaces; ++i) {
        SurfaceHandle_t surfID = SurfaceHandleFromIndex(
            mod->brush.firstmodelsurface + i, mod->brush.pShared);
        if (MSurf_Flags(surfID) & SURFDRAW_NODRAW) continue;

        IMaterial *material =
            MSurf_TexInfo(surfID, mod->brush.pShared)->material;
    ...
    case mod_studio: {
      ...
      if (pStudioHdr->flags & STUDIOHDR_FLAGS_FORCE_OPAQUE) return;
  ...
  switch (pModel->type) {
    case mod_brush: {
      for (i = 0; i < pModel->brush.nummodelsurfaces; ++i) {
        SurfaceHandle_t surfID = SurfaceHandleFromIndex(
            pModel->brush.firstmodelsurface + i, pModel->brush.pShared);
        if (MSurf_Flags(surfID) & SURFDRAW_NODRAW) continue;

        IMaterial *pMaterial =
            MSurf_TexInfo(surfID, pModel->brush.pShared)->material;

        // Try to find the material in the unique list of materials
        // if it's not there, then add it
        int j = found;
        while (--j >= 0) {
          if (ppMaterials[j] == pMaterial) break;
        }
        if (j < 0) ppMaterials[found++] = pMaterial;
...
static void MarkWaterSurfaces_ProcessLeafNode(mleaf_t *pLeaf) {
  ...
  int flags = (pLeaf->leafWaterDataID == -1) ? SURFDRAW_ABOVEWATER
                                             : SURFDRAW_UNDERWATER;
...
void MarkWaterSurfaces_r(mnode_t *node) {
  ...
  // if a leaf node, . .mark all the polys as to whether or not they are in
  // water.
  if (node->contents >= 0) {
...
static int SurfFlagsToSortGroup(SurfaceHandle_t surfID, int flags) {
  ...
  if (flags & SURFDRAW_WATERSURFACE) return MAT_SORT_GROUP_WATERSURFACE;

  if ((flags & (SURFDRAW_UNDERWATER | SURFDRAW_ABOVEWATER)) ==
      (SURFDRAW_UNDERWATER | SURFDRAW_ABOVEWATER))
    return MAT_SORT_GROUP_INTERSECTS_WATER_SURFACE;

  if (flags & SURFDRAW_UNDERWATER) return MAT_SORT_GROUP_STRICTLY_UNDERWATER;

  if (flags & SURFDRAW_ABOVEWATER) return MAT_SORT_GROUP_STRICTLY_ABOVEWATER;
  ...
  if (++warningcount < 10) {
    ...
    DevWarning("SurfFlagsToSortGroup:  unhandled flags (%X) (%s)!\n", flags,
               MSurf_TexInfo(surfID)->material->GetName());
    DevWarning(
        "- This implies you have a surface (usually a displacement) embedded "
        "in solid.\n");
    DevWarning("- Look near (%.1f, %.1f, %.1f)\n", vecCenter.x, vecCenter.y,
               vecCenter.z);
  }
  // Assert( 0 );
  return MAT_SORT_GROUP_STRICTLY_ABOVEWATER;
...
class CBrushBSPIterator : public ISpatialLeafEnumerator {
 public:
  ...
  bool EnumerateLeaf(int leaf, int) {
    ...
    int flags = (m_pShared->leafs[leaf].leafWaterDataID == -1)
                    ? SURFDRAW_ABOVEWATER
                    : SURFDRAW_UNDERWATER;
  ...
  void MarkModelSurfaces(int flags) {
    ...
    for (int i = 0; i < surfaceCount; ++i) {
      SurfaceHandle_t surfID = SurfaceHandleFromIndex(
          m_pBrush->brush.firstmodelsurface + i, m_pShared);
...
static void MarkBrushModelWaterSurfaces(model_t *world, Vector const &mins,
                                        Vector const &maxs, model_t *brush) {
  // HACK: This is a totally brutal hack dealing with initialization order
  // issues. I want to use the same box enumeration code so I don't have
  // multiple copies, but I want to use it from modelloader.
  // host_state.worldmodel isn't set up at that time however, so I have to fly
  // through these crazy hoops. Massive suckage.
...
void CModelLoader::RecomputeSurfaceFlags(model_t *mod) {
  for (int i = 0; i < mod->brush.pShared->numsubmodels; i++) {
    ...
    if (i != 0) {
      MarkBrushModelWaterSurfaces(mod, pSubModel->mins, pSubModel->maxs,
                                  pSubModel);
    }
  }
}
...
void CModelLoader::Map_UnloadModel(model_t *mod) {
  ...
  for (int texinfoID = 0; texinfoID < m_worldBrushData.numtexinfo;
       texinfoID++) {
...
//-----------------------------------------------------------------------------
static void GetSpriteInfo(const char *pName, bool bIsAVI, bool bIsBIK,
                          int &nWidth, int &nHeight, int &nFrameCount) {
  nFrameCount = 1;
  nWidth = nHeight = 1;
...
void CModelLoader::Sprite_LoadModel(model_t *mod) {
  ...
  uint8_t *pSprite = NULL;

#ifndef SWDS
  if (g_ClientDLL) {
    int nSize = g_ClientDLL->GetSpriteSize();
    if (nSize) {
      pSprite = (uint8_t *)new byte[nSize];
    }
  }
#endif
...
void CModelLoader::Sprite_UnloadModel(model_t *mod) {
  ...
  delete[](uint8_t *) mod->sprite.sprite;
  mod->sprite.sprite = 0;
...
void CModelLoader::Studio_ReloadModels(CModelLoader::ReloadType_t reloadType) {
#if !defined(SWDS)
  if (g_ClientDLL) g_ClientDLL->InvalidateMdlCache();
#endif  // SWDS
  if (serverGameDLL) serverGameDLL->InvalidateMdlCache();
  ...
  // has no way of indicating they refer to vcollides
  g_pMDLCache->Flush(
      (MDLCacheFlush_t)(MDLCACHE_FLUSH_ALL & (~MDLCACHE_FLUSH_VCOLLIDE)));
...
void CModelLoader::Studio_LoadModel(model_t *pModel, bool bTouchAllData) {
  ...
  if (pModel->nLoadFlags == FMODELLOADER_STATICPROP) {
    // this is the first call in loading as a static prop (load bit not set),
    // don't load physics yet the next call in causes the physics to load
    bLoadPhysics = false;
  }
  ...
  // this forces sync setup operations (materials/shaders) to build out now
  // during load and not at runtime
  double t1 = Plat_FloatTime();
  ...
  for (int i = 0; i < nMaterials; i++) {
    ...
    // meshes are not yet in the cache.
...
void CModelLoader::Studio_UnloadModel(model_t *pModel) {
  ...
  // leave these flags alone since we are going to return from alt-tab at some
  // point.
...
void CModelLoader::Print(void) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (pModel->type == mod_studio || pModel->type == mod_bad) {
      ...
      int refCount = (pModel->type == mod_studio)
                         ? g_pMDLCache->GetRef(pModel->studio)
                         : 0;
      ConMsg("%4d: Flags:0x%8.8x RefCount:%2d %s\n", i, pModel->nLoadFlags,
             refCount, pModel->szName);
    } else {
...
//-----------------------------------------------------------------------------
#if defined(_X360)
static bool BSPCreateCallback(const char *pSourceName, const char *pTargetName,
                              const char *pPathID, void *pExtraData) {
  ...
  bool bOk = true;
  if (!iBSPPack->SwapBSPFile(g_pFileSystem, pSourceName, pTargetName, IsX360(),
                             ConvertVTFTo360Format, NULL, NULL)) {
    bOk = false;
...
//-----------------------------------------------------------------------------
int CModelLoader::UpdateOrCreate(const char *pSourceName, char *pTargetName,
                                 int targetLen, bool bForce) {
#if defined(_X360)
  return ::UpdateOrCreate(pSourceName, pTargetName, targetLen, NULL,
                          BSPCreateCallback, bForce);
#else
  ...
  if (mapfile != FILESYSTEM_INVALID_HANDLE) {
    ...
    if (header.ident == IDBSPHEADER) {
      if (header.version >= MINBSPVERSION && header.version <= BSPVERSION) {
        ...
        Warning(
            "CModelLoader::Map_IsValid:  Map '%s' bsp version %i, expecting "
            "%i\n",
...
modtype_t CModelLoader::GetTypeFromName(const char *pModelName) {
  ...
  if (pExt) {
    if (!V_stricmp(pExt, "spr") || !V_stricmp(pExt, "vmt") ||
        !V_stricmp(pExt, "avi") || !V_stricmp(pExt, "bik")) {
      return mod_sprite;
...
const char *CModelLoader::GetActiveMapName(void) { return m_szActiveMapName; }
...
void Mod_LeafAmbientColorAtPos(Vector *pOut, const Vector &pos, int leafIndex) {
  ...
  mleafambientindex_t *pAmbient =
      &host_state.worldbrush->m_pLeafAmbient[leafIndex];
  if (!pAmbient->ambientSampleCount && pAmbient->firstAmbientSample) {
    // this leaf references another leaf, move there (this leaf is a solid leaf
    // so it borrows samples from a neighbor)
  ...
  if (count > 0) {
    int start =
        host_state.worldbrush->m_pLeafAmbient[leafIndex].firstAmbientSample;
    mleafambientlighting_t *pSamples =
        host_state.worldbrush->m_pAmbientSamples + start;
    ...
    for (int i = 0; i < count; i++) {
      ...
      samplePos.x +=
          float(pSamples[i].x) * pLeaf->m_vecHalfDiagonal.x * (2.0f / 255.0f);
      samplePos.y +=
          float(pSamples[i].y) * pLeaf->m_vecHalfDiagonal.y * (2.0f / 255.0f);
      samplePos.z +=
          float(pSamples[i].z) * pLeaf->m_vecHalfDiagonal.z * (2.0f / 255.0f);
