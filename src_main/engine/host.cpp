...
static ConVar violence_hgibs("violence_hgibs", "1", 0,
                             "Show human gib entities");
static ConVar violence_ablood("violence_ablood", "1", 0, "Draw alien blood");
static ConVar violence_agibs("violence_agibs", "1", 0,
                             "Show alien gib entities");

// Marked as FCVAR_USERINFO so that the server can cull CC messages before
// networking them down to us!!!
ConVar closecaption("closecaption", "0",
                    FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX | FCVAR_USERINFO,
...
void Snd_Restart_f() {
  ...
  // Flush soundscapes so they don't stop. We don't insert text in the buffer
  // here because cl_soundscape_flush is normally cheat-protected.
  ConCommand *pCommand = (ConCommand *)dynamic_cast<const ConCommand *>(
      g_pCVar->FindCommand("cl_soundscape_flush"));
  if (pCommand) {
...
static ConCommand snd_restart("snd_restart", Snd_Restart_f,
...
CON_COMMAND(mem_compact, "Compact heap.") { g_pMemAlloc->CompactHeap(); }
CON_COMMAND(mem_eat, "Allocate 1Mb of heap.") {
  g_pMemAlloc->Alloc(1024 * 1024);
}
CON_COMMAND(mem_test, "Check heap memory.") { g_pMemAlloc->CrtCheckMemory(); }

static ConVar mem_test_each_frame("mem_test_each_frame", "0", 0,
                                  "Run heap check at end of every frame\n");
static ConVar mem_test_every_n_seconds(
    "mem_test_every_n_seconds", "0", 0,
    "Run heap check at a specified interval\n");

static ConVar singlestep(
    "singlestep", "0", FCVAR_CHEAT,
...
ConVar host_showcachemiss(
    "host_showcachemiss", "0", 0,
    "Print a debug message when the client or server cache is missed.");
static ConVar mem_dumpstats("mem_dumpstats", "0", 0,
                            "Dump current and max heap usage info to console "
                            "at end of frame ( set to 2 for continuous output "
                            ")\n");
static ConVar host_ShowIPCCallCount("host_ShowIPCCallCount", "0", 0,
                                    "Print # of IPC calls this number of times "
                                    "per second. If set to -1, the # of IPC "
                                    "calls is shown every frame.");
...
ConVar host_thread_mode(
    "host_thread_mode", "0", 0,
    "Run the host in threaded mode, (0 == off, 1 == if multicore, 2 == force)");
extern ConVar threadpool_affinity;
void OnChangeThreadAffinity(IConVar *var, const char *pOldValue,
                            float flOldValue) {
...
ConVar threadpool_affinity("threadpool_affinity", "1", 0,
                           "Enable setting affinity", 0, 0, 0, 0,
                           &OnChangeThreadAffinity);
...
void ReserveThreads(int nToReserve) {
  ...
  Msg("%d threads being reserved\n", static_cast<int>(g_NumReservedThreads));
}

void OnChangeThreadReserve(IConVar *var, const char *pOldValue,
                           float flOldValue) {
...
CON_COMMAND(thread_test_tslist, "Run thread storage list tests.") {
  int nTests = (args.ArgC() == 1) ? 10000 : atoi(args.Arg(1));
  RunTSListTests(nTests);
}

CON_COMMAND(thread_test_tsqueue, "Run thread storage queue tests.") {
  int nTests = (args.ArgC() == 1) ? 10000 : atoi(args.Arg(1));
  RunTSQueueTests(nTests);
}

CON_COMMAND(threadpool_run_tests, "Run thread pool tests.") {
...
ConVar host_framerate("host_framerate", "0", 0,
                      "Set to lock per-frame time elapse.");
ConVar host_timescale("host_timescale", "1.0", FCVAR_REPLICATED,
                      "Prescale the clock by this amount.");
ConVar host_speeds(
    "host_speeds", "0", 0,
    "Show general system running times.");  // set for running times

ConVar host_flush_threshold("host_flush_threshold", "20", 0,
                            "Memory threshold below which the host should "
                            "flush caches between server instances");

#ifdef REL_TO_STAGING_MERGE_TODO
// Do this when merging the game DLLs so FCVAR_CHEAT can be set on them at the
// same time.
...
ConVar skill("skill", "1", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
             "Game skill level (1-3).", true, 1, true, 3);  // 1 - 3
ConVar deathmatch("deathmatch", "0", FCVAR_NOTIFY,
                  "Running a deathmatch server.");            // 0, 1, or 2
ConVar coop("coop", "0", FCVAR_NOTIFY, "Cooperative play.");  // 0 or 1
ConVar r_ForceRestore("r_ForceRestore", "0", 0);

ConVar vcr_verbose("vcr_verbose", "0", 0,
...
void CheckForFlushMemory(const char *pCurrentMapName,
                         const char *pDestMapName) {
  if (host_flush_threshold.GetInt() == 0) return;
}
...
void Host_WriteConfiguration(const char *dirname, const char *filename) {
  ...
  if (!sv.IsDedicated()) {
    ...
    if (g_pFileSystem->FileExists(szFileName, "MOD") &&
        !g_pFileSystem->IsFileWritable(szFileName, "MOD")) {
    ...
    g_pFileSystem->AsyncWrite(szFileName, tempBlock, configBuff.TellMaxPut(),
                              true);
...
bool XBX_SetProfileDefaultSettings() { return true; }
...
void Host_ReadConfiguration(const bool readDefault = false) {
  ...
  // check to see if we actually set any keys, if not, load defaults from
  // kb_def.lst so we at least have basics setup.
...
void Host_ReadPreStartupConfiguration() {
  ...
  static const char *s_PreStartupConfigConVars[] = {
      "sv_unlockedchapters",  // needed to display the startup graphic while
                              // loading
      "snd_legacy_surround",  // needed to init the sound system
      "gameui_xbox",          // needed to initialize the correct UI
      "save_in_memory"  // needed to preread data from the correct location in
                        // UI
  };
...
static ConCommand recompute_speed(
    "recompute_speed", Host_RecomputeSpeed_f,
    "Recomputes clock speed (for debugging purposes).", FCVAR_CHEAT);
...
static ConCommand dti_flush("dti_flush", DTI_Flush_f,
                            "Write out the datatable instrumentation files "
                            "(you must run with -dti for this to work).");

// This only happens at the end of a game, not between levels
void Host_ShutdownServer(void) {
  if (!sv.IsActive()) return;
  ...
  sv.Shutdown();  // sv.Shutdown() references some heap memory, so run it before
                  // Host_FreeToLowMark()
...
void Host_AccumulateTime(float dt) {
  ...
  // Adjust the client clock very slightly to keep it in line with the server
  // clock.
  float adj =
      cl.GetClockDriftMgr().AdjustFrameTime(host_frametime) - host_frametime;
  host_frametime += adj;
...
char const *Host_CleanupConVarStringValue(char const *invalue) {
  ...
  // Otherwise, if it appears numeric and has a decimal, try to strip all zeroes
  // after decimal
...
int Host_CountVariablesWithFlags(int flags, bool nonDefault) {
  ...
  for (ConCommandBase *var = g_pCVar->GetCommands(); var;
       var = var->GetNext()) {
    if (var->IsCommand()) continue;

    const ConVar *cvar_with_flags = (const ConVar *)var;

    if (!cvar_with_flags->IsFlagSet(flags)) continue;

    // It's == to the default value, don't count
    if (nonDefault && !Q_strcasecmp(cvar_with_flags->GetDefault(),
                                    cvar_with_flags->GetString()))
      continue;
...
void Host_BuildConVarUpdateMessage(NET_SetConVar *cvarMsg, int flags,
                                   bool nonDefault) {
  ...
  for (var = g_pCVar->GetCommands(); var; var = var->GetNext()) {
    ...
    const ConVar *updated_cvar = (const ConVar *)var;

    if (!updated_cvar->IsFlagSet(flags)) continue;

    // It's == to the default value, don't count
    if (nonDefault &&
        !Q_strcasecmp(updated_cvar->GetDefault(), updated_cvar->GetString()))
      continue;

    NET_SetConVar::cvar_t acvar;

    Q_strncpy(acvar.name, updated_cvar->GetName(), MAX_OSPATH);
    Q_strncpy(acvar.value,
              Host_CleanupConVarStringValue(updated_cvar->GetString()),
              MAX_OSPATH);
...
void CL_SendVoicePacket(bool bFinal) {
  ...
  voiceMsg.m_nLength =
      Voice_GetCompressedData(uchVoiceData, sizeof(uchVoiceData), bFinal) * 8;

  if (!voiceMsg.m_nLength) return;

  voiceMsg.m_DataOut.SeekToBit(
      voiceMsg.m_nLength);  // set correct writing position
...
void CFrameTimer::MarkFrame() {
  ...
  if (host_speeds.GetInt()) {
    ...
    for (i = 0; i < sv.num_edicts; i++) {
      if (!sv.edicts[i].IsFree()) ent_count++;
    }

    char sz[256];
    Q_snprintf(sz, sizeof(sz),
               "%3i fps -- inp(%3.1f) sv(%3.1f) cl(%3.1f) render(%3.1f) "
               "snd(%3.1f) cl_dll(%3.1f) exec(%3.1f) ents(%d) ticks(%d)",
               (int)fps, fs_input, fs_server, fs_client, fs_render, fs_sound,
               fs_cldll, fs_exec, ent_count, ticks);
...
void CFrameTimer::ComputeFrameVariability() {
  ...
  while ((nMaxSamples < FRAME_HISTORY_COUNT) &&
         (flTotalTime <= FRAME_TIME_FILTER_TIME)) {
    if (--i < 0) {
      i = FRAME_HISTORY_COUNT - 1;
    }
    if (m_pFrameTimeHistory[i] == 0.0f) break;
...
bool Host_ShouldRun(void) {
  ...
  if (next.GetInt()) {
    // Did we finally finish this frame ( Host_ShouldRun is called in 3 spots to
    // pause
    ...
    // Otherwise, keep running this one frame since we are still finishing this
    // frame
    return true;
  } else {
...
void _Host_RunFrame_Input(float accumulated_extra_samples, bool bFinalTick) {
  VPROF_BUDGET("_Host_RunFrame_Input", "Input");
  ...
  // Send any current movement commands to server and flush reliable buffer even
  // if not moving yet.
...
void _Host_RunFrame_Client(bool framefinished) {
  ...
  if (CL_IsHL2Demo() || CL_IsPortalDemo())  // don't need sv.IsDedicated()
                                            // because ded servers don't run
                                            // this
  {
...
bool CheckVarRange_Generic(ConVar *pVar, int minVal, int maxVal) {
  // Don't reenter (resetting the variable when we're checking the range might
  // cause us to reenter here).
  static bool bInFunction = false;
  if (bInFunction) return true;
  bInFunction = true;
...
void CheckSpecialCheatVars() {
  ...
  if (!mat_picmip) mat_picmip = g_pCVar->FindVar("mat_picmip");

  // In multiplayer, don't allow them to set mat_picmip > 2.
  if (mat_picmip) CheckVarRange_Generic(mat_picmip, -10, 2);
...
void CL_FindInterpolatedAddAngle(float t, float &frac, AddAngle **prev,
                                 AddAngle **angle_next) {
...
void CL_ApplyAddAngle() {
  ...
  AddAngle *prev = NULL, *angle_next = NULL;
  float frac = 0.0f;

  float addangletotal = 0.0f;

  CL_FindInterpolatedAddAngle(curtime, frac, &prev, &angle_next);

  if (prev && angle_next) {
    addangletotal = prev->total + frac * (angle_next->total - prev->total);
  } else {
...
float Host_GetSoundDuration(const char *pSample) {
  ...
  if (index >= 0) return AudioSource_GetSoundDuration(cl.GetSound(index));
#endif
  return SV_GetSoundDuration(pSample);
}

CON_COMMAND(host_runofftime,
...
void Host_ShowIPCCallCount() {
  ...
  if (host_ShowIPCCallCount.GetInt() == 0) return;
  ...
  if (flCurTime - s_flLastTime >= flInterval) {
    ...
    if (pSteamClient) {
      ...
      callCount = (uint32_t)SteamGameServer_GetIPCCallCount();
    }
#endif
    ...
    if (frameCount == 0 || tickCount == 0) return;

    Msg("host_ShowIPCCallCount: %d IPC calls in the past [%d frames, %d ticks] "
        " Avg: [%.2f/frame, %.2f/tick]\n",
        callCount, frameCount, tickCount, (float)callCount / frameCount,
        (float)callCount / tickCount);
...
#ifndef SWDS
  // Tracker 77931:  If the game is paused, then lock the client clock at the
  // previous tick boundary
  //  (otherwise we'll keep interpolating through the "remainder" time causing
  //  the paused characters to twitch like they have the shakes)
  // TODO:  Since this rounds down on the frame we paused, we could see a slight
  // backsliding.  We could remember the last "remainder" before pause and
  // re-use it and
  //  set insimulation == false to be mroe exact.  We'd still have to deal with
  //  the timing difference between when pause/unpause happens on the server
  //  versus the client
...
static ConVar host_Sleep(
    "host_sleep", "0", FCVAR_CHEAT,
...
void _Host_RunFrame(float time) {
  ...
  int numticks;
  {
    // Profile scope specific to the top of this function, protect from setjmp()
    // problems
    ...
    // When playing back a VCR file, don't do host_sleep. That way, if it was
    // recorded with host_sleep on, it'll play back way Faster.
...

  {
    ...
    // initialize networking for dedicated server after commandline &
    // autoexec.cfg have been parsed
    if (NET_IsDedicated() && !NET_IsMultiplayer()) NET_SetMutiplayer(true);

    g_HostTimes.EndFrameSegment(FRAME_SEGMENT_CMD_EXECUTE);

    // Msg( "Running %i ticks (%f remainder) for frametime %f total %f tick %f
    ...
    // We're just caching off the state here so that we have a consistent return
    // value for enginetool->IsInGame the entire frame
    g_pEngineToolInternal->SetIsInGame(cl.IsActive() &&
                                       (scr_nextdrawtick == 0));
#endif
    CJob *pGameJob = NULL;
...
// threaded path only supported in listen server
...
#endif
    {
      ...
      for (int tick = 0; tick < numticks; tick++) {
        ...
        // Only send updates on final tick so we don't re-encode network data
        // multiple times per frame unnecessarily
        bool bFinalTick = (tick == (numticks - 1));
        ...
        // Additional networking ops for SPLITPACKET stuff (99.9% of the time
        // this will be an empty list of work)
      ...
      // run HLTV if active
      if (hltv) hltv->RunFrame();

      if (hltvtest) hltvtest->RunFrame();
#ifndef SWDS
      // This is a hack to let timedemo pull messages from the queue faster than
      // every 15 msec Also when demoplayer is skipping packets to a certain
      // tick we should process the queue as quickly as we can.
      if (numticks == 0 &&
          (demoplayer->IsPlayingTimeDemo() || demoplayer->IsSkipping())) {
        _Host_RunFrame_Client(true);
      }
      if (!sv.IsDedicated()) {
        // This causes cl.gettime() to return the true clock being used for
        // rendering (tickcount * rate + remainder)
        ...
        //-------------------
        // If we haven't predicted/simulated the player (multiplayer with
        // prediction enabled and
        //  not a listen server with zero frame lag, then go ahead and predict
        //  now
        ...
        // but they have a cap applied by IN_SetSampleTime() so they are not
        // also simulated during input gathering
      ...
      if (!cl.IsPaused() || !sv.IsPaused()) {
        Msg("=============SIM: CLIENT %5d + %d, SERVER %5d + %d\t REM: %.2f\n",
            cl.GetClientTickCount(), numticks, sv.m_nTickCount, numticks,
            host_remainder * 1000.0f);
      }
#endif
    }
#ifndef SWDS
      ...
      static float host_remainder_last_frame = 0, prev_remainder_last_frame = 0,
                   last_frame_time = 0;
      ...
      pGameJob = new CFunctorJob(
          CreateFunctor(_Host_RunFrame_Server_Async, serverticks));

      g_pThreadPool->AddJob(pGameJob);
#if LOG_FRAME_OUTPUT
      if (!cl.IsPaused() || !sv.IsPaused()) {
        Msg("=============SIM: CLIENT %5d + %d, SERVER %5d + %d\t REM: %.2f\n",
            cl.GetClientTickCount(), clientticks, sv.m_nTickCount, serverticks,
            host_remainder * 1000.0f);
      }
#endif
    }
    ...
    if (shouldrender) {
      ...
      if (!cl.IsPaused() || !sv.IsPaused()) {
        ...
        Msg("RENDER AT: %6.4f: %.2fms [%.2fms implicit] frametime\n",
            ...
            g_ClientGlobalVariables.frametime * 1000.0f, frametime * 1000.0f);
    ...
    if (pGameJob) {
      {
        ...
        if (Host_IsSinglePlayerGame()) {
          // This should change to a YieldWait if the server starts wanting to
          // parallel process. If so, will need some route for the server to
          // queue up work it wants to execute outside its frame, otherwise some
          // of it would be performed during the yield. Right now need to wait
          // for server so we don't stall on queued AI operations (toml
          // 7/3/2007)
    ...
    {
      host_time =
          host_tickcount * host_state.interval_per_tick + cl.m_tickRemainder;
    }
...
inline bool IsThreadedEngine() {
  switch (host_thread_mode.GetInt()) {
    case HTM_DISABLED:
      return false;
    case HTM_DEFAULT:
      return g_pThreadPool->NumThreads() > 0;
    case HTM_FORCED:
      return true;
    default:
      return false;
...
void Host_RunFrame(float the_time) {
  ...
  if (!scr_drawloading && sv.IsActive() && cl.IsActive() && !sv.m_bLoadgame) {
    g_bThreadedEngine = IsThreadedEngine();
  } else
#endif
  {
  ...
  if (!host_profile.GetBool()) {
    _Host_RunFrame(the_time);
    return;
  }

  const double start_time = Plat_FloatTime();
  _Host_RunFrame(the_time);
  const double end_time = Plat_FloatTime();

  timetotal += end_time - start_time;  // time in seconds
  timecount++;

  if (timecount < 1000) return;

  const double fps = 1000 / (end_time - timestart);
...
#if defined(_WIN32)
// If "User Token 2" exists in
// HKEY_CURRENT_USER/Software/Valve/Half-Life/Settings then we disable gore.
...
bool IsLowViolence_Registry() {
  ...
  Q_snprintf(szSubKey, sizeof(szSubKey), "Software\\Valve\\%s\\Settings",
             appname);
  ...
  if (!bReducedGore) {
    Sys_GetRegKeyValue(szSubKey, "User Token 3", szBuffer, nBufferLen,
                       szBuffer);
  ...
  Q_snprintf(szSubKey, sizeof(szSubKey), "Software\\Valve\\%s\\%s\\Settings",
             appname, gamedir);
...
void Host_InitProcessor() {
  ...
  if (pi.m_bSSE) {
    if (MathLib_SSEEnabled())
      Q_strncat(szFeatureString, "SSE ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
    else
      Q_strncat(szFeatureString, "(SSE) ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
  ...
  if (pi.m_bSSE2) {
    if (MathLib_SSE2Enabled())
      Q_strncat(szFeatureString, "SSE2 ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
    else
      Q_strncat(szFeatureString, "(SSE2) ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
  ...
  if (pi.m_bMMX) {
    if (MathLib_MMXEnabled())
      Q_strncat(szFeatureString, "MMX ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
    else
      Q_strncat(szFeatureString, "(MMX) ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
  ...
  if (pi.m_b3DNow) {
    if (MathLib_3DNowEnabled())
      Q_strncat(szFeatureString, "3DNow ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
    else
      Q_strncat(szFeatureString, "(3DNow) ", sizeof(szFeatureString),
                COPY_ALL_CHARACTERS);
  ...
  if (pi.m_bRDTSC)
    Q_strncat(szFeatureString, "RDTSC ", sizeof(szFeatureString),
              COPY_ALL_CHARACTERS);
  if (pi.m_bCMOV)
    Q_strncat(szFeatureString, "CMOV ", sizeof(szFeatureString),
              COPY_ALL_CHARACTERS);
  if (pi.m_bFCMOV)
    Q_strncat(szFeatureString, "FCMOV ", sizeof(szFeatureString),
              COPY_ALL_CHARACTERS);
  ...
  if (pi.m_nLogicalProcessors == 1) {
    ConDMsg("1 CPU, Frequency: %.01f %s,  Features: %s\n", fFrequency,
            szFrequencyDenomination, szFeatureString);
  } else {
    char buffer[256] = "";
    if (pi.m_nPhysicalProcessors != pi.m_nLogicalProcessors) {
      Q_snprintf(buffer, sizeof(buffer), " (%i physical)",
                 (int)pi.m_nPhysicalProcessors);
    }

    ConDMsg("%i CPUs%s, Frequency: %.01f %s, Features: %s\n",
            (int)pi.m_nLogicalProcessors, buffer, fFrequency,
            szFrequencyDenomination, szFeatureString);
  }
}
...
int Host_GetServerCount() {
  ...
  // this is unfortunate, and happens, but the caller is too early in the
  // protocol or a demo cannot identify the correct server count return the same
  // count that demo will use
...
void Host_Init(bool bDedicated) {
  ...
  // Dedicated servers should not explicitly set the main thread's affinity so
  // that machines running multiple copies of the dedicated server can
  // load-balance properly. Machines running the client should set the main
  // thread affinity explicitly because we assume this when we assign work to
  // the other threads.
  if (!bDedicated) {
  ...
  if (g_pThreadPool) g_pThreadPool->Start(startParams);

  // From const.h, the loaded game .dll will give us the correct value which is
  ...
  TRACEINIT(g_pCVar->Init(),
            g_pCVar->Shutdown());  // So we can list cvars with "cvarlst"
  ...
  // Mark DLL as active
  //	eng->SetNextState( InEditMode() ? IEngine::DLL_PAUSED :
  // IEngine::DLL_ACTIVE );
  ...
  // if running outside of steam and NOT a dedicated server then phone home (or
  // if "-phonehome" is passed on the command line)
  if (!sv.IsDedicated() || CommandLine()->FindParm("-phonehome")) {
    // In debug, only run this check if -phonehome is on the command line (so a
    // debug build will "just work").
#ifdef _DEBUG
  ...
  if (!sv.IsDedicated() && S_IsInitted()) {
    if (!MapReslistGenerator().IsEnabled()) {
      // only build caches if we aren't' generating reslists (you need reslists
      // to make the caches)
  ...
  pRenderContext->SetNonInteractiveTempFullscreenBuffer(
...
//-----------------------------------------------------------------------------
void AddTransitionResources(CSaveRestoreData *pSaveData, const char *pLevelName,
...
void Host_Changelevel(bool loadfromsavedgame, const char *mapname,
  ...
  int iElapsedMinutes =
      saverestore->GetMostRecentElapsedMinutes() + (iElapsedSeconds / 60);
  ...
  if (bTransitionBySave) {
    char comment[80];
    // Pass in the total elapsed time so it gets added to the elapsed time for
    // this map.
    ...
    if (!saverestore->SaveGameSlot("_transition", comment, false, true, mapname,
                                   startspot)) {
    ...
    // Not going to load a save after the transition, so add this map's elapsed
    // time to the total elapsed time
    int totalSeconds = g_ServerGlobalVariables.curtime +
                       saverestore->GetMostRecentElapsedSeconds();
    saverestore->SetMostRecentElapsedMinutes(
        (int)(totalSeconds / 60.0f) +
        saverestore->GetMostRecentElapsedMinutes());
  ...
  if (loadfromsavedgame) {
    ...
    g_pServerPluginHandler->LevelInit(level, CM_EntityString(), oldlevel,
                                      startspot, true, false);
...
#endif
  {
    ...
    audiosourcecache->LevelInit(level);
#endif
    g_pServerPluginHandler->LevelInit(level, CM_EntityString(), NULL, NULL,
                                      false, false);
  }
  ...
  saverestore->SetMostRecentElapsedMinutes(
      saverestore->GetMostRecentElapsedMinutes() - minutes);
  saverestore->SetMostRecentElapsedSeconds(
      saverestore->GetMostRecentElapsedSeconds() - seconds);
#endif
...
void GetPlatformMapPath(const char *pMapPath, char *pPlatformMapPath,
                        int maxLength) {
...
bool Host_NewGame(char *mapName, bool loadGame, bool bBackgroundLevel,
  ...
  if (MapReslistGenerator().IsEnabled()) {
    // uncache all the materials, so their files get referenced again for the
    // reslists undone for now, since we're just trying to get a global reslist,
    // not per-map accurate
  ...
  if (!sv.SpawnServer(mapName, NULL)) return false;
  ...
  g_pServerPluginHandler->LevelInit(mapName, CM_EntityString(), pszOldMap,
                                    pszLandmark, loadGame, bBackgroundLevel);
...
#ifndef SWDS
  if (!loadGame) {
    // clear the most recent remember save, so the level will just restart if
    // the player dies
...
void Host_FreeStateAndWorld(bool server) {
  ...
  if (host_state.worldmodel) {
    modelloader->UnreferenceModel(host_state.worldmodel,
                                  IModelLoader::FMODELLOADER_SERVER);
    modelloader->UnreferenceModel(host_state.worldmodel,
                                  IModelLoader::FMODELLOADER_CLIENT);
...
void Host_Shutdown(void) {
  ...
  if (!sv.IsDedicated()) {
    if (MapReslistGenerator().ShouldRebuildCaches()) {
      ...
      // ****  this needs to be BEFORE S_Shutdown() as it uses sound
      // resources!!!
