...
ConVar mat_colcorrection_disableentities("mat_colcorrection_disableentities",
                                         "0");
...
class CPrecisionSlider : public vgui::Slider {
 ...
 public:
  CPrecisionSlider(Panel *parent, const ch *panelName);
  ~CPrecisionSlider();
...
// Constructor
//-----------------------------------------------------------------------------
CPrecisionSlider::CPrecisionSlider(Panel *parent, const ch *panelName)
...
// Destructor
//-----------------------------------------------------------------------------
CPrecisionSlider::~CPrecisionSlider() { delete m_pTextEntry; }
...
void CPrecisionSlider::OnSizeChanged(int wide, int tall) {
  ...
  m_pTextEntry->SetBounds(nSliderWidth + m_nSpacing, 0, nEditWidth,
                          nEditHeight);
...
enum { IMAGE_BUFFER_MAX_DIM = 128 };

class CColorCorrectionUIPanel;

// If you add a tool, add it to the string list and instance the panel in the
// constructor below
...
static const ch *s_pColorCorrectionToolNames[CC_TOOL_COUNT] = {
    "No Tool Active",    "Curves Tool", "Levels Tool",
    "Selected HSV Tool", "Lookup Tool", "Color Balance Tool",
};
...
//-----------------------------------------------------------------------------
static void ComputeSplinePoint(float flInColor, Vector *pControlPoints[4],
                               Vector &vecOut) {
  ...
  float flStart = 0.0f;
  float flEnd = 1.0f;
  float flMid = (flInColor - pControlPoints[1]->x) /
                (pControlPoints[2]->x - pControlPoints[1]->x);
  while (true) {
    Catmull_Rom_Spline(*pControlPoints[0], *pControlPoints[1],
                       *pControlPoints[2], *pControlPoints[3], flMid, vecOut);
    if (fabs(vecOut.x - flInColor) < 1e-5) return;
...
the_interface IColorOperation {
 public:
  ...
  virtual const ch *GetName() = 0;
  virtual void SetName(const ch *pName) = 0;
...
void CColorOperationList::DeleteOperation(int opIndex) {
  if (!m_OpList.IsValidIndex(opIndex)) return;
...
// Applys all operations in the list to the color
//-----------------------------------------------------------------------------
void CColorOperationList::Apply(color24 in, color24 &out,
...
// Queries for the number of operations in the list
//-----------------------------------------------------------------------------
int CColorOperationList::GetNumOperations() { return m_OpList.Count(); }

//-----------------------------------------------------------------------------
// Returns the operation at the specified index in the list
//-----------------------------------------------------------------------------
IColorOperation *CColorOperationList::GetOperation(int opIndex) {
  if (!m_OpList.IsValidIndex(opIndex)) return NULL;

  return m_OpList.Element(opIndex);
}

void CColorOperationList::BringForward(int opIndex) {
  if (!m_OpList.IsValidIndex(opIndex) || opIndex == 0) return;
...
class CColorCorrectionUIChildPanel : public vgui::Frame {
 ...
 public:
  CColorCorrectionUIChildPanel(vgui::Panel *parent, const ch *name)
  ...
  virtual void ReadUncorrectedImage(Rect_t *pSrcRect,
                                    unsigned char *pPreviewImage) {}
};
...
class CCurvesColorOperation : public IColorOperation {
 public:
  ...
  virtual const ch *GetName() { return m_pName; }
  virtual void SetName(const ch *pName) { Q_strcpy(m_pName, pName); }
  ...
  // Controls how much this op should take effect (1 = use 100% converted color,
  // 0 = use 100% input color)
  ...
  // Finds or adds a control point
  int FindOrAddControlPoint(float flInValue, float flTolerance,
                            float flOutValue);
...
//-----------------------------------------------------------------------------
// Controls how much this op should take effect (1 = use 100% converted color, 0
// = use 100% input color)
//-----------------------------------------------------------------------------
...
void CCurvesColorOperation::GetControlPoint(int nPoint, float *pInValue,
                                            float *pOutValue) {
...
// Finds or adds a control point
//-----------------------------------------------------------------------------
int CCurvesColorOperation::FindControlPoint(float flInValue,
                                            float flTolerance) {
  for (int i = m_ControlPoints.Count(); --i >= 0;) {
    if (fabs(m_ControlPoints[i].x - flInValue) < flTolerance) return i;
  }
  return -1;
}
...
// Finds or adds a control point
//-----------------------------------------------------------------------------
int CCurvesColorOperation::FindOrAddControlPoint(float flInValue,
                                                 float flTolerance,
                                                 float flOutValue) {
  int nPoint = FindControlPoint(flInValue, flTolerance);
  if (nPoint != -1) return nPoint;
...
// Modifies a control point
//-----------------------------------------------------------------------------
int CCurvesColorOperation::ModifyControlPoint(int nPoint, float flInValue,
                                              float flOutValue) {
...
void CCurvesColorOperation::RemoveControlPoint(int nPoint) {
  Assert((nPoint >= 0) && (nPoint < m_ControlPoints.Count()));
  if ((nPoint == 0) || (nPoint == m_ControlPoints.Count() - 1)) return;
...
float CCurvesColorOperation::ComputeActualCorrectedColor(float flInColor) {
  ...
  pControlPoints[3] =
      (i + 2 < nCount) ? &m_ControlPoints[i + 2] : &m_ControlPoints[nCount - 1];
...
IColorOperation *CCurvesColorOperation::Clone() {
  ...
  Q_memcpy(pClone->m_pName, m_pName, sizeof(ch) * 256);
...
class CColorCurvesEditPanel : public CCurveEditorPanel {
 ...
 public:
  // constructor
  CColorCurvesEditPanel(vgui::Panel *pParent, const ch *pName);
...
//-----------------------------------------------------------------------------
CColorCurvesEditPanel::CColorCurvesEditPanel(vgui::Panel *pParent,
                                             const ch *pName)
...
// Control points + values...
//-----------------------------------------------------------------------------
int CColorCurvesEditPanel::FindOrAddControlPoint(float flIn, float flTolerance,
                                                 float flOut) {
  Assert(m_pCurvesOp);
...
int CColorCurvesEditPanel::ModifyControlPoint(int nPoint, float flIn,
                                              float flOut) {
  Assert(m_pCurvesOp);
...
void CColorCurvesEditPanel::GetControlPoint(int nPoint, float *pIn,
                                            float *pOut) {
  Assert(m_pCurvesOp);
...
class CColorCurvesUIPanel : public CColorCorrectionUIChildPanel {
 ...
 public:
  ...
  virtual IColorOperation *GetOperation() {
    return (IColorOperation *)m_pColorOp;
  }
 ...
 private:
  ...
  static const ch *s_pColorMaskLabel[COLOR_MASK_TYPE_COUNT];
};

const ch *CColorCurvesUIPanel::s_pColorMaskLabel
    [CColorCurvesUIPanel::COLOR_MASK_TYPE_COUNT] = {"RGB", "Red", "Green",
                                                    "Blue"};

//-----------------------------------------------------------------------------
// constructor
//-----------------------------------------------------------------------------
CColorCurvesUIPanel::CColorCurvesUIPanel(vgui::Panel *pParent,
                                         CCurvesColorOperation *pOp)
    : BaseClass(pParent, "ColorCurvesUIPanel") {
...
CColorCurvesUIPanel::~CColorCurvesUIPanel() { delete m_pCurveEditor; }

//-----------------------------------------------------------------------------
// Command issued
//-----------------------------------------------------------------------------
void CColorCurvesUIPanel::OnMessage(const KeyValues *params,
                                    vgui::VPANEL fromPanel) {
  BaseClass::OnMessage(params, fromPanel);

  if (!Q_stricmp("SliderMoved", params->GetName())) {
    vgui::Panel *pPanel = reinterpret_cast<vgui::Panel *>(
        const_cast<KeyValues *>(params)->GetPtr("panel"));
    ...
    PostMessage(GetParent(),
                new KeyValues("command", "command", "BlendFactorUpdate"));
  }
}

void CColorCurvesUIPanel::OnCommand(const ch *command) {
...
// Sets input levels
//-----------------------------------------------------------------------------
void CLevelsColorOperation::SetInputLevels(float flMinValue, float flMidValue,
                                           float flMaxValue) {
  m_flMinInputLevel = std::clamp(flMinValue, 0.0f, 1.0f);
  m_flMidInputLevel = std::clamp(flMidValue, 0.0f, 1.0f);
  m_flMaxInputLevel = std::clamp(flMaxValue, 0.0f, 1.0f);
  UpdateOutputLevelArray();
  colorcorrectiontools->UpdateColorCorrection();
}
...
// Sets output levels
//-----------------------------------------------------------------------------
void CLevelsColorOperation::SetOutputLevels(float flMinValue,
                                            float flMaxValue) {
  m_flMinOutputLevel = std::clamp(flMinValue, 0.0f, 1.0f);
  m_flMaxOutputLevel = std::clamp(flMaxValue, 0.0f, 1.0f);
  UpdateOutputLevelArray();
  colorcorrectiontools->UpdateColorCorrection();
}
...
float CLevelsColorOperation::ComputeNormalizedInputLevel(float flInLevel) {
  if (flInLevel <= m_flMinInputLevel) return 0.0f;

  if (flInLevel >= m_flMaxInputLevel) return 1.0f;
...
class CColorHistogramPanel : public vgui::Panel {
 ...
 public:
  ...
  // constructor
  CColorHistogramPanel(vgui::Panel *pParent, const ch *pName,
  ...
  void ComputeHistogram(Rect_t &srcRect, unsigned char *pBits,
                        ImageFormat format, int nStride);
...
// constructor, destructor
//-----------------------------------------------------------------------------
CColorHistogramPanel::CColorHistogramPanel(vgui::Panel *pParent,
                                           const ch *pName,
                                           CLevelsColorOperation *pOp)
    : BaseClass(pParent, pName) {
  for (int i = 0; i < 256; ++i) {
...
void CColorHistogramPanel::ComputeHistogram(Rect_t &srcRect,
                                            unsigned char *pBits,
                                            ImageFormat format, int nStride) {
  for (int i = 0; i < 256; ++i) {
    m_pHistogram[i] = 0.0f;
  ...
  writer.SetPixelMemory(
      format, pBits + srcRect.y * nStride + srcRect.x * nSizeInBytes, nStride);
  for (int y = 0; y < srcRect.height; ++y) {
    ...
    for (int x = 0; x < srcRect.width; ++x) {
      ...
      color24 inColor, col;
      inColor.r = std::clamp(r, 0, 255);
      inColor.g = std::clamp(g, 0, 255);
      inColor.b = std::clamp(b, 0, 255);
...
//-----------------------------------------------------------------------------
void CColorHistogramPanel::PaintBackground() {
  int w, h;
  GetSize(w, h);
...
// Sets the color curves operation to edit
//-----------------------------------------------------------------------------
void CColorHistogramPanel::Paint() {
  int w, h;
  GetSize(w, h);
...
class CColorSlider : public vgui::Panel {
 ...
 public:
  // constructor
  CColorSlider(vgui::Panel *pParent, const ch *pName, int nKnobCount);
  ~CColorSlider();
  ...
  void SetRange(int min,
                int max);  // set to max and min range of rows to display
 ...
 private:
  ...
  void PaintKnob(float flPosition, unsigned char r, unsigned char g,
                 unsigned char b);
...
// constructor
//-----------------------------------------------------------------------------
CColorSlider::CColorSlider(vgui::Panel *pParent, const ch *pName,
                           int nKnobCount)
    : BaseClass(pParent, pName) {
  ...
  vgui::surface()->DrawSetTextureFile(m_nWhiteMaterial, "vgui/white", true,
                                      false);
...
void CColorSlider::SetNormalizedValue(int nKnobIndex, float flValue) {
  ...
  m_flKnobPosition[nKnobIndex] = std::clamp(flValue, 0.0f, 1.0f);
...
void CColorSlider::SetValue(int nKnobIndex, int value) {
  Assert(m_nKnobCount > nKnobIndex);
  SetNormalizedValue(
      nKnobIndex, (float)(value - m_nMinValue) / (m_nMaxValue - m_nMinValue));
}

int CColorSlider::GetValue(int nKnobIndex) {
  Assert(m_nKnobCount > nKnobIndex);
  return m_flKnobPosition[nKnobIndex] * (m_nMaxValue - m_nMinValue) +
         m_nMinValue;
}
...
void CColorSlider::OnMousePressed(vgui::MouseCode code) {
  ...
  if (code == MOUSE_LEFT) {
    ...
    for (int i = 1; i < m_nKnobCount; ++i) {
      if (fabs(flNormalizedVal - m_flKnobPosition[i]) <
          fabs(flNormalizedVal - m_flKnobPosition[m_nSelectedKnob])) {
        m_nSelectedKnob = i;
      }
    }
...
void CColorSlider::OnCursorMoved(int x, int y) {
  ...
  if (m_nSelectedKnob >= 0) {
    ...
    if (m_nSelectedKnob < 2 && m_nKnobCount == 3) {
      ...
      float fOldRelGrey = (m_flKnobPosition[2] - m_flKnobPosition[0]) /
                          (m_flKnobPosition[1] - m_flKnobPosition[0]);

      SetNormalizedValue(m_nSelectedKnob, flNormalizedVal);
      SetNormalizedValue(
          2, fOldRelGrey * (m_flKnobPosition[1] - m_flKnobPosition[0]) +
                 m_flKnobPosition[0]);
    } else {
      SetNormalizedValue(m_nSelectedKnob, flNormalizedVal);
    }
...
// Draws a single knob with a particular color
//-----------------------------------------------------------------------------
void CColorSlider::PaintKnob(float flPosition, unsigned char r, unsigned char g,
                             unsigned char b) {
  int w, h;
  GetSize(w, h);
...
class CColorLevelsUIPanel : public CColorCorrectionUIChildPanel {
 ...
 public:
  ...
  virtual IColorOperation *GetOperation() {
    return (IColorOperation *)m_pLevelsOp;
  }

 private:
  enum { HISTOGRAM_IMAGE_SIZE = 256 };
...
const ch *CColorLevelsUIPanel::s_pColorMaskLabel
    [CColorLevelsUIPanel::COLOR_MASK_TYPE_COUNT] = {"RGB", "Red", "Green",
                                                    "Blue"};

//-----------------------------------------------------------------------------
// constructor
//-----------------------------------------------------------------------------
CColorLevelsUIPanel::CColorLevelsUIPanel(vgui::Panel *pParent,
                                         CLevelsColorOperation *pOp)
    : BaseClass(pParent, "LevelsUIPanel") {
  m_pColorMask = new ComboBox(this, "ColorMask", COLOR_MASK_TYPE_COUNT, false);
  int i;
  for (i = 0; i < COLOR_MASK_TYPE_COUNT; i++) {
    m_pColorMask->AddItem(s_pColorMaskLabel[i], NULL);
...
// Reads the uncorrected image + generates a hisogram
//-----------------------------------------------------------------------------
void CColorLevelsUIPanel::ReadUncorrectedImage(Rect_t *pSrcRect,
                                               unsigned char *pPreviewImage) {
  Rect_t dstRect;
  dstRect.x = 0;
  dstRect.y = 0;
  ...
  m_pHistogramPanel->ComputeHistogram(dstRect, pPreviewImage,
                                      IMAGE_FORMAT_BGRX8888, dstRect.width * 4);
}

//-----------------------------------------------------------------------------
// Command issued
//-----------------------------------------------------------------------------
void CColorLevelsUIPanel::OnMessage(const KeyValues *params,
                                    vgui::VPANEL fromPanel) {
  BaseClass::OnMessage(params, fromPanel);
  ...
  if (!Q_stricmp("SliderMoved", params->GetName())) {
    ...
    if (pPanel == m_pInputLevelSlider) {
      m_pLevelsOp->SetInputLevels(m_pInputLevelSlider->GetValue(0) / 255.0f,
    ...
    if (pPanel == m_pOutputLevelSlider) {
      m_pLevelsOp->SetOutputLevels(m_pOutputLevelSlider->GetValue(0) / 255.0f,
...
void CColorLevelsUIPanel::OnCommand(const ch *command) {
  BaseClass::OnCommand(command);
...
class CSelectedHSVOperation : public IColorOperation {
 public:
  ...
  float GetSelectionAmount(unsigned char r, unsigned char g,
...
CSelectedHSVOperation::SelectionMethod_t
CSelectedHSVOperation::GetSelectionMethod() {
...
float FuzzyLessThan(float a, float b, float fuzziness) {
  if (fuzziness < 1.0f / 255.0f) return (a <= b) ? 1.0f : 0.0f;

  float min = b - fuzziness;
  float max = b + fuzziness;

  if (a < min) return 1.0f;
  if (a > max) return 0.0f;
...
float FuzzyGreaterThan(float a, float b, float fuzziness) {
  ...
  if (a > max) return 1.0f;
  if (a < min) return 0.0f;
...
float CSelectedHSVOperation::GetSelectionAmount(const Vector &rgb) const {
  ...
  for (int i = 0; i < m_SelectedRGBs.Count(); i++) {
    ...
    switch (m_SelectionMethod) {
      ...
      case SELECT_GREATER_RED:
        flCurSelAmount =
            FuzzyGreaterThan(rgb.x, m_SelectedRGBs[i].x, m_Fuzziness);
        break;
      ...
      case SELECT_GREATER_GREEN:
        flCurSelAmount =
            FuzzyGreaterThan(rgb.y, m_SelectedRGBs[i].y, m_Fuzziness);
        break;
      ...
      case SELECT_GREATER_BLUE:
        flCurSelAmount =
            FuzzyGreaterThan(rgb.z, m_SelectedRGBs[i].z, m_Fuzziness);
        break;
      case SELECT_LESSER_BLUE:
        flCurSelAmount = FuzzyLessThan(rgb.z, m_SelectedRGBs[i].z, m_Fuzziness);
        break;
      case SELECT_NEARBY_RGB:
        flCurSelAmount = FuzzyLessThan(rgb.DistTo(m_SelectedRGBs[i]),
                                       m_Tolerance, m_Fuzziness * m_Tolerance);
        break;
      case SELECT_GREATER_HUE:
        RGBtoHSV(rgb, hsv);
        flCurSelAmount =
            FuzzyGreaterThan(hsv.x, m_SelectedHSVs[i].x, m_Fuzziness);
        break;
      ...
      case SELECT_NEARBY_HUE:
        RGBtoHSV(rgb, hsv);
        flCurSelAmount =
            FuzzyLessThan(fabsf(hsv.x - m_SelectedHSVs[i].x) / 360.0f,
                          m_Tolerance, m_Fuzziness * m_Tolerance);
        break;
      case SELECT_GREATER_SATURATION:
        RGBtoHSV(rgb, hsv);
        flCurSelAmount =
            FuzzyGreaterThan(hsv.y, m_SelectedHSVs[i].y, m_Fuzziness);
        break;
      ...
      case SELECT_NEARBY_SATURATION:
        RGBtoHSV(rgb, hsv);
        flCurSelAmount = FuzzyLessThan(fabsf(hsv.y - m_SelectedHSVs[i].y),
                                       m_Tolerance, m_Fuzziness * m_Tolerance);
        break;
      case SELECT_GREATER_VALUE:
        RGBtoHSV(rgb, hsv);
        flCurSelAmount =
            FuzzyGreaterThan(hsv.z, m_SelectedHSVs[i].z, m_Fuzziness);
        break;
      ...
      case SELECT_NEARBY_VALUE:
        RGBtoHSV(rgb, hsv);
        flCurSelAmount = FuzzyLessThan(fabsf(hsv.z - m_SelectedHSVs[i].z),
                                       m_Tolerance, m_Fuzziness * m_Tolerance);
        break;
    }
  ...
  if (m_bInvertSelection) flSelAmount = 1.0f - flSelAmount;

  return flSelAmount;
}

float CSelectedHSVOperation::GetSelectionAmount(unsigned char r,
                                                unsigned char g,
                                                unsigned char b) const {
  Vector rgb(r, g, b);
  rgb /= 255.0f;
...
void CSelectedHSVOperation::AddSelectedColor(unsigned char r, unsigned char g,
                                             unsigned char b) {
  Vector color, hsv;
  color.x = r / 255.0f;
  color.y = g / 255.0f;
...
bool CSelectedHSVOperation::GetColorize() { return m_bColorize; }
...
bool CSelectedHSVOperation::GetInvertSelection() { return m_bInvertSelection; }

void CSelectedHSVOperation::SetTolerance(float flTolerance) {
  m_Tolerance = flTolerance;
}

float CSelectedHSVOperation::GetTolerance() { return m_Tolerance; }

void CSelectedHSVOperation::SetFuzziness(float flFuzziness) {
  m_Fuzziness = flFuzziness;
}

float CSelectedHSVOperation::GetFuzziness() { return m_Fuzziness; }
...
class CFullScreenSelectionPanel : public vgui::Panel {
 ...
 public:
  CFullScreenSelectionPanel(const ch *pName, CSelectedHSVOperation *pOp,
                            vgui::Panel *pParent);
...
CFullScreenSelectionPanel::CFullScreenSelectionPanel(const ch *pName,
...
void CFullScreenSelectionPanel::OnCursorMoved(int x, int y) {
  if (m_bMouseDown) {
    ...
    pRenderContext->ReadPixels(x, y, 1, 1, (unsigned char *)&pixelValue,
                               IMAGE_FORMAT_BGRX8888);

    bool bCTRLDown =
        (input()->IsKeyDown(KEY_LCONTROL) || input()->IsKeyDown(KEY_RCONTROL));

    if (!bCTRLDown) m_pOp->ClearSelectedColors();
...
class CUncorrectedImagePanel : public CProceduralTexturePanel {
 ...
 public:
  // constructor
  CUncorrectedImagePanel(vgui::Panel *pParent, const ch *pName);
...
// constructor
//-----------------------------------------------------------------------------
CUncorrectedImagePanel::CUncorrectedImagePanel(vgui::Panel *pParent,
                                               const ch *pName)
    : BaseClass(pParent, pName) {
  m_bMouseDown = false;
  SetMouseInputEnabled(true);
...
void CUncorrectedImagePanel::RegenerateTextureBits(ITexture *pTexture,
  ...
  for (int y = 0; y < nHeight; ++y) {
    ...
    for (int x = 0; x < nWidth; ++x, ++pTexel) {
      ...
      float flSelectionAmount =
          m_pHSVOp->GetSelectionAmount(col.r, col.g, col.b);
      flSelectionAmount *= 0.5f;

      // Blend between at most 50% (1,0,0) and the original texel based on
      // selection amount
...
void CUncorrectedImagePanel::OnCursorMoved(int x, int y) {
  ...
  int nSelectedX = std::min(x, m_nWidth);
  nSelectedX = std::max(0, nSelectedX);
  int nSelectedY = std::min(y, m_nHeight);
  nSelectedY = std::max(0, nSelectedY);
  BGRA8888_t *pTexel = &m_pImageBuffer[(nSelectedY * m_nWidth) + nSelectedX];

  if (!bCTRLDown) m_pHSVOp->ClearSelectedColors();
...
class CSelectedHSVUIPanel : public CColorCorrectionUIChildPanel {
 ...
 public:
  ...
  virtual IColorOperation *GetOperation() {
    return (IColorOperation *)m_pHSVOperation;
  }
 ...
 private:
  enum { DEFAULT_SELECTION_METHOD = CSelectedHSVOperation::SELECT_NONE };
  ...
  static const ch
      *s_pSelectionMethodNames[CSelectedHSVOperation::SELECTION_METHOD_COUNT];
};

//-----------------------------------------------------------------------------
// If you add a selection method, add it to the string list
//-----------------------------------------------------------------------------
const ch *CSelectedHSVUIPanel::s_pSelectionMethodNames
    [CSelectedHSVOperation::SELECTION_METHOD_COUNT] = {
        "Select None",
        "Select All",
...
//-----------------------------------------------------------------------------
CSelectedHSVUIPanel::CSelectedHSVUIPanel(vgui::Panel *parent,
                                         CSelectedHSVOperation *pOp)
    : BaseClass(parent, "SelectedHSVUIPanel") {
  ...
  m_pInvertSelectionButton =
      new vgui::CheckButton(this, "InvertSelectionButton", "Invert Selection");
  ...
  m_pFullScreenSelection =
      new CFullScreenSelectionPanel("SelectionPanel", pOp, this);
...
void CSelectedHSVUIPanel::Shutdown() { m_pUncorrectedImage->Shutdown(); }
...
void CSelectedHSVUIPanel::OnTextChanged(KeyValues *data) {
  ...
  if (pBox ==
      m_pSelectionMethod)  // don't change the text in the config setting combo
  {
...
void CSelectedHSVUIPanel::OnSelectionMethodSelected() {
  ...
  CSelectedHSVOperation::SelectionMethod_t method =
      (CSelectedHSVOperation::SelectionMethod_t)
          m_pSelectionMethod->GetActiveItem();
...
// Returns the image buffer
//-----------------------------------------------------------------------------
void CSelectedHSVUIPanel::ReadUncorrectedImage(Rect_t *pSrcRect,
                                               unsigned char *pPreviewImage) {
  Rect_t dstRect;
  dstRect.x = 0;
  dstRect.y = 0;
  ...
  for (int i = 0; i < g_nPreviewImageHeight; i++) {
    Q_memcpy(m_pUncorrectedImage->GetImageBuffer() + i * IMAGE_BUFFER_MAX_DIM,
             pPreviewImage + i * g_nPreviewImageWidth * 4,
             g_nPreviewImageWidth * 4);
  }
...
void CSelectedHSVUIPanel::OnCommand(const ch *command) {
  BaseClass::OnCommand(command);
...
class CColorLookupOperation : public IColorOperation {
 public:
  ...
  void LoadLookupTable(const ch *pFilename);
  ...
  const ch *GetFilename() { return m_pFilename; }
 ...
 private:
  ...
  ch m_pFilename[SOURCE_MAX_PATH];
...
// Destructor
//-----------------------------------------------------------------------------
CColorLookupOperation::~CColorLookupOperation() { DeleteLookupTableData(); }
...
void CColorLookupOperation::Apply(const Vector &inRGB, Vector &outRGB) {
  ...
  outRGB = (bottom * (1.0f - fb) + top * fb) * m_flBlendFactor + inRGB -
           inRGB * m_flBlendFactor;
}
...
void CColorLookupOperation::GetLookupValue(int r, int g, int b, Vector &out) {
  ...
  color24 out_color =
      m_LookupTable[r + g * m_Resolution + b * m_Resolution * m_Resolution];
...
//-----------------------------------------------------------------------------
void CColorLookupOperation::LoadLookupTable(const ch *pFilename) {
...
IColorOperation *CColorLookupOperation::Clone() {
  ...
  Q_memcpy(pClone->m_pFilename, m_pFilename, sizeof(ch) * SOURCE_MAX_PATH);
  ...
  pClone->m_LookupTable =
      new color24[m_Resolution * m_Resolution * m_Resolution];
  Q_memcpy(pClone->m_LookupTable, m_LookupTable,
           sizeof(color24) * m_Resolution * m_Resolution * m_Resolution);
...
class CColorLookupUIPanel : public CColorCorrectionUIChildPanel {
 ...
 public:
  ...
  virtual IColorOperation *GetOperation() {
    return (IColorOperation *)m_pLookupOp;
  }
...
// constructor
//-----------------------------------------------------------------------------
CColorLookupUIPanel::CColorLookupUIPanel(vgui::Panel *pParent,
                                         CColorLookupOperation *pOp)
    : BaseClass(pParent, "LookupUIPanel") {
  m_pLookupOp = pOp;
...
// Command issued
//-----------------------------------------------------------------------------
void CColorLookupUIPanel::OnMessage(const KeyValues *params,
                                    vgui::VPANEL fromPanel) {
  BaseClass::OnMessage(params, fromPanel);
...
void CColorLookupUIPanel::OnCommand(const ch *command) {
...
void CColorLookupUIPanel::OnFileSelected(const ch *filename) {
...
class CColorBalanceOperation : public IColorOperation {
 public:
  ...
  void SetPreserveLuminosity(bool bPreserveLum) {
    m_PreserveLuminosity = bPreserveLum;
    Update();
  }

  void SetCyanRedBalance(ColorBalanceMode_t mode, float value) {
    m_CyanRedBalance[(int)mode] = value;
    Update();
  }
  void SetMagentaGreenBalance(ColorBalanceMode_t mode, float value) {
    m_MagentaGreenBalance[(int)mode] = value;
    Update();
  }
  void SetYellowBlueBalance(ColorBalanceMode_t mode, float value) {
    m_YellowBlueBalance[(int)mode] = value;
    Update();
  }

  float GetCyanRedBalance(ColorBalanceMode_t mode) {
    return m_CyanRedBalance[(int)mode];
  }
  float GetMagentaGreenBalance(ColorBalanceMode_t mode) {
    return m_MagentaGreenBalance[(int)mode];
  }
  float GetYellowBlueBalance(ColorBalanceMode_t mode) {
    return m_YellowBlueBalance[(int)mode];
  }
...
CColorBalanceOperation::CColorBalanceOperation() {
  ...
  for (int i = 0; i < 256; i++) {
    m_HighlightsAddTransfer[i] = m_ShadowsSubTransfer[i] =
        (1.075f - 1.0f / ((float)i / 16.0f + 1.0f));
    //		m_HighlightsSubTransfer[i] = m_ShadowsAddTransfer[i] = ( 1.075f
    //- 1.0f
    /// ((float)(255-i)/16.0f + 1.0f) );

    float fi = ((float)i - 127.0f) / 127.0f;
    m_MidtonesAddTransfer[i] = m_MidtonesSubTransfer[i] =
        0.667f * (1.0f - fi * fi);
    m_ShadowsAddTransfer[i] = m_HighlightsSubTransfer[i] =
        0.667f * (1.0f - fi * fi);
  }
...
void CColorBalanceOperation::CreateLookupTables() {
  ...
  cyan_red_transfer[CC_BALANCE_MODE_SHADOWS] =
      (m_CyanRedBalance[CC_BALANCE_MODE_SHADOWS] > 0.0f) ? m_ShadowsAddTransfer
                                                         : m_ShadowsSubTransfer;
  cyan_red_transfer[CC_BALANCE_MODE_MIDTONES] =
      (m_CyanRedBalance[CC_BALANCE_MODE_MIDTONES] > 0.0f)
          ? m_MidtonesAddTransfer
          : m_MidtonesSubTransfer;
  cyan_red_transfer[CC_BALANCE_MODE_HIGHLIGHTS] =
      (m_CyanRedBalance[CC_BALANCE_MODE_HIGHLIGHTS] > 0.0f)
          ? m_HighlightsAddTransfer
          : m_HighlightsSubTransfer;

  magenta_green_transfer[CC_BALANCE_MODE_SHADOWS] =
      (m_MagentaGreenBalance[CC_BALANCE_MODE_SHADOWS] > 0.0f)
          ? m_ShadowsAddTransfer
          : m_ShadowsSubTransfer;
  magenta_green_transfer[CC_BALANCE_MODE_MIDTONES] =
      (m_MagentaGreenBalance[CC_BALANCE_MODE_MIDTONES] > 0.0f)
          ? m_MidtonesAddTransfer
          : m_MidtonesSubTransfer;
  magenta_green_transfer[CC_BALANCE_MODE_HIGHLIGHTS] =
      (m_MagentaGreenBalance[CC_BALANCE_MODE_HIGHLIGHTS] > 0.0f)
          ? m_HighlightsAddTransfer
          : m_HighlightsSubTransfer;

  yellow_blue_transfer[CC_BALANCE_MODE_SHADOWS] =
      (m_YellowBlueBalance[CC_BALANCE_MODE_SHADOWS] > 0.0f)
          ? m_ShadowsAddTransfer
          : m_ShadowsSubTransfer;
  yellow_blue_transfer[CC_BALANCE_MODE_MIDTONES] =
      (m_YellowBlueBalance[CC_BALANCE_MODE_MIDTONES] > 0.0f)
          ? m_MidtonesAddTransfer
          : m_MidtonesSubTransfer;
  yellow_blue_transfer[CC_BALANCE_MODE_HIGHLIGHTS] =
      (m_YellowBlueBalance[CC_BALANCE_MODE_HIGHLIGHTS] > 0.0f)
          ? m_HighlightsAddTransfer
          : m_HighlightsSubTransfer;
  ...
  for (int i = 0; i < 256; i++) {
    ...
    for (int mode = CC_BALANCE_MODE_SHADOWS; mode <= CC_BALANCE_MODE_HIGHLIGHTS;
         mode++) {
      redOut += m_CyanRedBalance[mode] * cyan_red_transfer[mode][redOut];
      greenOut +=
          m_MagentaGreenBalance[mode] * magenta_green_transfer[mode][greenOut];
      blueOut +=
          m_YellowBlueBalance[mode] * yellow_blue_transfer[mode][blueOut];

      redOut = std::clamp(redOut, 0, 255);
      greenOut = std::clamp(greenOut, 0, 255);
      blueOut = std::clamp(blueOut, 0, 255);
    }
...
IColorOperation *CColorBalanceOperation::Clone() {
  ...
  Q_memcpy(pClone->m_CyanRedBalance, m_CyanRedBalance,
           sizeof(float) * CC_BALANCE_MODE_COUNT);
  Q_memcpy(pClone->m_MagentaGreenBalance, m_MagentaGreenBalance,
           sizeof(float) * CC_BALANCE_MODE_COUNT);
  Q_memcpy(pClone->m_YellowBlueBalance, m_YellowBlueBalance,
           sizeof(float) * CC_BALANCE_MODE_COUNT);

  Q_memcpy(pClone->m_ShadowsSubTransfer, m_ShadowsSubTransfer,
           sizeof(float) * 256);
  Q_memcpy(pClone->m_MidtonesSubTransfer, m_ShadowsSubTransfer,
           sizeof(float) * 256);
  Q_memcpy(pClone->m_HighlightsSubTransfer, m_ShadowsSubTransfer,
           sizeof(float) * 256);
  Q_memcpy(pClone->m_ShadowsAddTransfer, m_ShadowsAddTransfer,
           sizeof(float) * 256);
  Q_memcpy(pClone->m_MidtonesAddTransfer, m_MidtonesAddTransfer,
           sizeof(float) * 256);
  Q_memcpy(pClone->m_HighlightsAddTransfer, m_HighlightsAddTransfer,
           sizeof(float) * 256);

  Q_memcpy(pClone->m_pRedLookup, m_pRedLookup, sizeof(ch) * 256);
  Q_memcpy(pClone->m_pGreenLookup, m_pGreenLookup, sizeof(ch) * 256);
  Q_memcpy(pClone->m_pBlueLookup, m_pBlueLookup, sizeof(ch) * 256);
...
class CColorBalanceUIPanel : public CColorCorrectionUIChildPanel {
 ...
 public:
  ...
  virtual IColorOperation *GetOperation() {
    return (IColorOperation *)m_pBalanceOp;
  }
...
// constructor
//-----------------------------------------------------------------------------
CColorBalanceUIPanel::CColorBalanceUIPanel(vgui::Panel *pParent,
                                           CColorBalanceOperation *pOp)
    : BaseClass(pParent, "BalanceUIPanel") {
  m_pPreserveLuminosityButton =
      new vgui::CheckButton(this, "PreserveLuminosity", "Preserve Luminosity");
  ...
  m_pHighlightModeButton =
      new vgui::RadioButton(this, "HighlightMode", "Highlights");
...
// Command issued
//-----------------------------------------------------------------------------
void CColorBalanceUIPanel::OnMessage(const KeyValues *params,
                                     vgui::VPANEL fromPanel) {
  BaseClass::OnMessage(params, fromPanel);
  ...
  if (!Q_stricmp("SliderMoved", params->GetName())) {
    ...
    if (pPanel == m_pBlendFactorSlider) {
      ...
      m_pBalanceOp->SetCyanRedBalance(GetCurrentMode(),
                                      m_pCyanRedSlider->GetValue() / 1.0f);
      return;
    } else if (pPanel == m_pMagentaGreenSlider) {
      m_pBalanceOp->SetMagentaGreenBalance(
          GetCurrentMode(), m_pMagentaGreenSlider->GetValue() / 1.0f);
      return;
    } else if (pPanel == m_pYellowBlueSlider) {
      m_pBalanceOp->SetYellowBlueBalance(
          GetCurrentMode(), m_pYellowBlueSlider->GetValue() / 1.0f);
      return;
    }
  } else if (!Q_stricmp("CheckButtonChecked", params->GetName())) {
    vgui::Panel *pPanel = reinterpret_cast<vgui::Panel *>(
    ...
    if (pPanel == m_pPreserveLuminosityButton) {
      m_pBalanceOp->SetPreserveLuminosity(
          m_pPreserveLuminosityButton->IsSelected());
      return;
    }
  }
}

void CColorBalanceUIPanel::OnCommand(const ch *command) {
  BaseClass::OnCommand(command);
...
void CColorBalanceUIPanel::OnRadioButtonHit() { ResetSliders(); }
...
void CColorBalanceUIPanel::ResetSliders() {
  if (!m_pBalanceOp) return;

  ColorBalanceMode_t mode = GetCurrentMode();

  m_pCyanRedSlider->SetValue((int)m_pBalanceOp->GetCyanRedBalance(mode), 0);
  m_pMagentaGreenSlider->SetValue(
      (int)m_pBalanceOp->GetMagentaGreenBalance(mode), 0);
  m_pYellowBlueSlider->SetValue((int)m_pBalanceOp->GetYellowBlueBalance(mode),
                                0);
}
...
class CLookupViewPanel : public CProceduralTexturePanel {
 ...
 public:
  ...
  void RegenerateTextureBits(ITexture *pTexture, IVTFTexture *pVTFTexture,
                             Rect_t *pRect);
...
CLookupViewPanel::CLookupViewPanel(vgui::Panel *parent,
                                   ColorCorrectionHandle_t CCHandle)
    : BaseClass(parent, "LookupViewPanel") {
  m_CCHandle = CCHandle;
}

CLookupViewPanel::~CLookupViewPanel() {}

void CLookupViewPanel::RegenerateTextureBits(ITexture *pTexture,
                                             IVTFTexture *pVTFTexture,
                                             Rect_t *pRect) {
  Assert(pVTFTexture->FrameCount() == 1);
  Assert(pVTFTexture->FaceCount() == 1);
...
CLookupViewWindow::CLookupViewWindow(vgui::Panel *parent,
                                     ColorCorrectionHandle_t CCHandle)
...
CLookupViewWindow::~CLookupViewWindow() { delete m_pLookupPanel; }
...
void CLookupViewWindow::Shutdown() { m_pLookupPanel->Shutdown(); }
...
CNewOperationDialog::CNewOperationDialog(vgui::Panel *parent,
                                         CColorOperationList *pOpList)
    : BaseClass(parent, "NewOperation") {
...
void CNewOperationDialog::OnCommand(const ch *command) {
  if (!Q_stricmp(command, "Create")) {
    ...
    switch ((ColorCorrectionTool_t)nSelectedItem + 1) {
      case CC_TOOL_BALANCE:
        newOp = new CColorBalanceOperation();
        break;
      case CC_TOOL_CURVES:
        newOp = new CCurvesColorOperation();
        break;
      case CC_TOOL_LOOKUP:
        newOp = new CColorLookupOperation();
        break;
      case CC_TOOL_LEVELS:
        newOp = new CLevelsColorOperation();
        ((CLevelsColorOperation *)newOp)->SetColorOpList(m_pOpList);
        break;
      case CC_TOOL_SELECTED_HSV:
        newOp = new CSelectedHSVOperation();
        ((CSelectedHSVOperation *)newOp)->SetColorOpList(m_pOpList);
        break;
    }
...
class COperationListPanel : public vgui::ListPanel {
 ...
 public:
  COperationListPanel(vgui::Panel *parent, const ch *pName);
  ~COperationListPanel();
...
COperationListPanel::COperationListPanel(vgui::Panel *parent, const ch *pName)
...
//-----------------------------------------------------------------------------
// CColorOperationListPanel - View window for operations in a
// CColorOperationList
//-----------------------------------------------------------------------------
class CColorOperationListPanel : public vgui::EditablePanel {
 ...
 public:
  CColorOperationListPanel(vgui::Panel *parent,
                           ColorCorrectionHandle_t CCHandle);
...
CColorOperationListPanel::CColorOperationListPanel(
    ...
    : BaseClass(parent, "ColorOperationListPanel") {
  m_pNewOperationButton =
      new vgui::Button(this, "NewOperation", "New", this, "NewOperation");
  m_pDeleteOperationButton = new vgui::Button(this, "DeleteOperation", "Delete",
                                              this, "DeleteOperation");
  m_pBringForwardButton =
      new vgui::Button(this, "BringForward", "Up", this, "BringForward");
  m_pPushBackButton =
      new vgui::Button(this, "PushBack", "Down", this, "PushBack");
  ...
  m_pEnableEntitiesButton =
      new vgui::CheckButton(this, "EnableEntities", "Enable Entities");
  ...
  m_pOperationListPanel->AddColumnHeader(0, "image", "", 24,
...
void CColorOperationListPanel::OnOpPanelClose(KeyValues *data) {
  CColorCorrectionUIChildPanel *pSender =
      (CColorCorrectionUIChildPanel *)data->GetPtr("panel", 0);
  if (pSender) {
...
void CColorOperationListPanel::OnSliderMoved(KeyValues *data) {
  vgui::Panel *pPanel = reinterpret_cast<vgui::Panel *>(
      const_cast<KeyValues *>(data)->GetPtr("panel"));
  if (pPanel == m_pBlendFactorSlider) {
    int nSelectedItem = m_pOperationListPanel->GetSelectedItem(0);
    if (nSelectedItem >= 0 &&
        nSelectedItem < m_pOperationListPanel->GetItemCount()) {
      IColorOperation *pOp =
          (IColorOperation *)m_pOperationListPanel->GetItemUserData(
              nSelectedItem);
...
void CColorOperationListPanel::OnCheckButtonChecked(KeyValues *data) {
  ...
  if (pPanel == m_pEnableButton) {
    if (m_pEnableButton->IsSelected()) {
      PostActionSignal(
          new KeyValues("Command", "Command", "EnableColorCorrection"));
      m_bEnable = true;
    } else {
      PostActionSignal(
          new KeyValues("Command", "Command", "DisableColorCorrection"));
      m_bEnable = false;
    }
...
void CColorOperationListPanel::OnCommand(const ch *command) {
  if (!Q_stricmp(command, "NewOperation")) {
    ...
    m_pOperationListPanel->SetSingleSelectedItem(
        m_pOperationListPanel->GetItemCount() - 1);
...
void CColorOperationListPanel::Init() { m_pLookupViewWindow->Init(); }
...
void CColorOperationListPanel::OnFileSelected(const ch *pFilename) {
...
void CColorOperationListPanel::LaunchOperationPanel(IColorOperation *pOp) {
  if (pOp) {
    ...
    switch (pOp->ToolID()) {
      case CC_TOOL_BALANCE:
        pOpPanel =
            new CColorBalanceUIPanel(this, (CColorBalanceOperation *)pOp);
        break;
      case CC_TOOL_CURVES:
        pOpPanel = new CColorCurvesUIPanel(this, (CCurvesColorOperation *)pOp);
        break;
      case CC_TOOL_LEVELS:
        pOpPanel = new CColorLevelsUIPanel(this, (CLevelsColorOperation *)pOp);
        break;
      case CC_TOOL_LOOKUP:
        pOpPanel = new CColorLookupUIPanel(this, (CColorLookupOperation *)pOp);
        break;
      case CC_TOOL_SELECTED_HSV:
        pOpPanel = new CSelectedHSVUIPanel(this, (CSelectedHSVOperation *)pOp);
        break;
    }
    ...
    int panelOffset = (m_OpPanelList.Count() + 1 < maxPanels)
                          ? m_OpPanelList.Count() + 1
                          : maxPanels;
...
void CColorOperationListPanel::ReadUncorrectedImage(
    Rect_t *pSrcRect, unsigned char *pPreviewImage) {
  for (int i = 0; i < m_OpPanelList.Count(); i++) {
...
// Purpose: Basic help dialog
//-----------------------------------------------------------------------------
CColorCorrectionUIPanel::CColorCorrectionUIPanel(vgui::Panel *parent)
    : BaseClass(parent, "ColorCorrectionUIPanel") {
  if (!colorcorrection) {
...
// Init, shutdown
//-----------------------------------------------------------------------------
void CColorCorrectionUIPanel::Init() { m_pOperationListPanel->Init(); }
...
//-----------------------------------------------------------------------------
void CColorCorrectionUIPanel::Activate() { BaseClass::Activate(); }

void CColorCorrectionUIPanel::OnCommand(ch const *command) {
  BaseClass::OnCommand(command);
...
void CColorCorrectionUIPanel::OnThink() {
  ...
  if (m_nCurrentRow != -1) {
    ...
    for (int r = m_nCurrentRow; r < 32 && r < (m_nCurrentRow + 32 / m_nRowStep);
         ++r, ++inColor.r) {
      inColor.g = 0;
      for (int g = 0; g < 32; ++g, ++inColor.g) {
        inColor.b = 0;
        for (int b = 0; b < 32; ++b, ++inColor.b) {
          ...
          if (m_bEnable) {
            m_pOperationListPanel->GetOperationList()->Apply(
                directColor, outColor, m_pFinalOperation);
          } else {
            outColor = directColor;
          }

          m_pLookupCache[inColor.r + inColor.g * 32 + inColor.b * 32 * 32] =
              outColor;
        }
      }
    }
...
//-----------------------------------------------------------------------------
void CColorCorrectionUIPanel::ReadUncorrectedImage(
    Rect_t *pSrcRect, unsigned char *pPreviewImage) {
...
void CColorCorrectionTools::Init() {
  if (g_pColorCorrectionUI) {
    g_pColorCorrectionUI->Init();
  }

  m_pPreviewImage =
      new BGRA8888_t[g_nPreviewImageWidth * g_nPreviewImageHeight];
}

void CColorCorrectionTools::Shutdown() {
  if (g_pColorCorrectionUI) {
...
void CColorCorrectionTools::InstallColorCorrectionUI(vgui::Panel *parent) {
  if (g_pColorCorrectionUI) return;

  g_pColorCorrectionUI = new CColorCorrectionUIPanel(parent);
  Assert(g_pColorCorrectionUI);
}

bool CColorCorrectionTools::ShouldPause() const { return false; }

void CColorCorrectionTools::GrabPreColorCorrectedFrame(int x, int y, int width,
                                                       int height) {
  if (!g_pColorCorrectionUI->IsVisible()) return;
  ...
  pRenderContext->ReadPixelsAndStretch(
      &srcRect, &dstRect, (unsigned char *)m_pPreviewImage,
      IMAGE_FORMAT_BGRX8888, g_nPreviewImageWidth * 4);

  g_pColorCorrectionUI->ReadUncorrectedImage(&srcRect,
                                             (unsigned char *)m_pPreviewImage);
}
...
static ConCommand colorcorrectionui("colorcorrectionui",
                                    ShowHideColorCorrectionUI,
...
static ConCommand print_colorcorrection(
    "print_colorcorrection", PrintColorCorrection,
    "Display the color correction layer information.", FCVAR_CHEAT);
