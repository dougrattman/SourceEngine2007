...
CSteam3Client &Steam3Client() { return s_Steam3Client; }
...
#if !defined(NO_STEAM)
  SteamAPI_Init();  // ignore failure, that will fall out later when they don't
                    // get a valid logon cookie
#endif
}
...
//-----------------------------------------------------------------------------
int CSteam3Client::InitiateConnection(void *pData, int cbMaxData, uint32_t unIP,
                                      uint16_t usPort, uint64_t unGSSteamID,
                                      bool bSecure,
  ...
  return SteamUser()->InitiateGameConnection(
      pData, cbMaxData, unGSSteamID, CGameID(g_iSteamAppID), ntohl(unIP),
      usPort, bSecure, pvSteam2GetEncryptionKey,
      cbSteam2GetEncryptionKey);  // port is already in host order
#else
  return 0;
#endif
}
...
//-----------------------------------------------------------------------------
void CSteam3Client::TerminateConnection(uint32_t unIP, uint16_t usPort) {
  m_bGSSecure = false;
  ...
  if (pClientGameServerDeny->m_uAppID == (uint32_t)g_iSteamAppID) {
    ...
    switch (pClientGameServerDeny->m_uReason) {
      ...
      case (k_EDenyNotLoggedOn):
        pszReason = "Not logged on";
        break;
      case (k_EDenyNoLicense):
        pszReason = "No license";
        break;
      case (k_EDenyCheater):
        pszReason = "VAC banned ";
        break;
      case (k_EDenyLoggedInElseWhere):
        pszReason = "Dropped from server";
        break;
      case (k_EDenyUnknownText):
        pszReason = "Unknown";
        break;
      case (k_EDenyIncompatibleAnticheat):
        pszReason = "Incompatible Anti Cheat";
        break;
      case (k_EDenyMemoryCorruption):
        pszReason = "Memory corruption";
        break;
      case (k_EDenyIncompatibleSoftware):
        pszReason = "Incompatible software";
        break;
      case (k_EDenySteamConnectionLost):
        pszReason = "Steam connection lost";
        break;
      case (k_EDenySteamConnectionError):
        pszReason = "Steam connection error";
        break;
      case (k_EDenySteamResponseTimedOut):
        pszReason = "Response timed out";
        break;
      case (k_EDenySteamValidationStalled):
        pszReason = "Verification failed";
        break;
    }
...
#ifndef SWDS
      if (!EngineVGui()->IsGameUIVisible() && !EngineVGui()->IsConsoleVisible())
#endif
      {
...
// Purpose:
//-----------------------------------------------------------------------------
void CSteam3Client::OnPersonaUpdated(
    PersonaStateChange_t *pPersonaStateChanged) {
