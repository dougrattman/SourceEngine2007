...
class CGameClient : public CBaseClient, public CClientFrameManager {
 public:
  ...
  // IClient interface
  void Connect(const char *szName, int nUserID, INetChannel *pNetChannel,
               bool bFakePlayer);
 ...
 public:
  ...
  CBitVec<ABSOLUTE_PLAYER_LIMIT>
      m_VoiceStreams;  // Which other clients does this guy's voice stream go
                       // to?
  CBitVec<ABSOLUTE_PLAYER_LIMIT>
      m_VoiceProximity;  // Should we use proximity for this guy?
