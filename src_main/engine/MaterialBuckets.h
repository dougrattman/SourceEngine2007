...
// buckets have been used since flush.

#ifndef MATERIALBUCKETS_H
#define MATERIALBUCKETS_H

// TODO: Make all of the buckets share the same m_Elements (ie. make
// m_Elements static)
...
class CMaterialsBuckets {
 public:
  ...
  CMaterialsBuckets() { m_FlushCount = -1; }

  // Set the number of buckets that are needed.  This should get called every
  ...
  //
  SortIDHandle_t GetFirstUsedSortID() { return m_UsedSortIDs.Head(); }

  SortIDHandle_t GetNextUsedSortID(SortIDHandle_t prevSortID) {
    return m_UsedSortIDs.Next(prevSortID);
  }

  int GetSortID(SortIDHandle_t handle) { return m_UsedSortIDs[handle]; }

  SortIDHandle_t InvalidSortIDHandle() { return m_UsedSortIDs.InvalidIndex(); }
  ...
  Element_t GetElement(ElementHandle_t h) { return m_Elements[h]; }

  ElementHandle_t InvalidElementHandle() { return m_Elements.InvalidIndex(); }
 ...
 private:
  ...
  // This is used in multilist mode to make elements that belong in the multiple
  // lists of
