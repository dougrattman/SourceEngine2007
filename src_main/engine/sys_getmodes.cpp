...
// Finds the video mode in the list of video modes
int CVideoMode_Common::FindVideoMode(int nDesiredWidth, int nDesiredHeight,
                                     bool bWindowed) {
...
//-----------------------------------------------------------------------------
void CVideoMode_Common::ResetCurrentModeForNewResolution(int nWidth,
                                                         int nHeight,
                                                         bool bWindowed) {
...
//-----------------------------------------------------------------------------
bool CVideoMode_Common::CreateGameWindow(int nWidth, int nHeight,
                                         bool bWindowed) {
...
//-----------------------------------------------------------------------------
IVTFTexture *CVideoMode_Common::LoadVTF(CUtlBuffer &temp,
...
//-----------------------------------------------------------------------------
void CVideoMode_Common::BlitGraphicToHDCWithAlpha(HDC hdc, uint8_t *rgba,
                                                  int imageWidth,
                                                  int imageHeight, int x0,
...
bool CVideoMode_MaterialSystem::Init() {
  ...
  for (int i = 0; i < nModeCount; i++) {
    ...
    // we've only added modes that the adapter can do and maybe the desktop rate
...
// Sets the video mode
//-----------------------------------------------------------------------------
bool CVideoMode_MaterialSystem::SetMode(int nWidth, int nHeight,
                                        bool bWindowed) {
...
//-----------------------------------------------------------------------------
// Called by the material system when mode changes after a call to
// OverrideConfig
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CVideoMode_MaterialSystem::ChangeDisplaySettingsToFullscreen(int nWidth,
                                                                  int nHeight,
                                                                  int nBPP) {
  if (IsWindowedMode()) return;
...
void CVideoMode_MaterialSystem::ReadScreenPixels(int x, int y, int w, int h,
                                                 void *pBuffer,
                                                 ImageFormat format) {
  if (!g_LostVideoMemory) {
    ...
    pRenderContext->ReadPixelsAndStretch(&rect, &rect, (unsigned char *)pBuffer,
                                         format,
...
void VideoMode_Destroy() {
  if (videomode) {
    CVideoMode_MaterialSystem *pVideoMode_MS =
        static_cast<CVideoMode_MaterialSystem *>(videomode);
