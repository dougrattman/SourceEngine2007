...
//-----------------------------------------------------------------------------
// Purpose: Individual entity data, did the entity exist and what was it's
// serial number
//-----------------------------------------------------------------------------
class CFrameSnapshotEntry {
 public:
  ...
  int m_nSerialNumber;
  // Keeps track of the fullpack info for this frame for all entities in any
  // pvs:
...
// the
//  snapshot is for.  Also tracks whether the snapshot is still referenced. When
//  no longer referenced, it's freed
//-----------------------------------------------------------------------------
class CFrameSnapshot {
 ...
 public:
  CInterlockedInt
      m_ListIndex;  // Index info CFrameSnapshotManager::m_FrameSnapshots.
  ...
  CHLTVEntityData* m_pHLTVEntityData;  // is NULL if not in HLTV mode or array
                                       // of m_pValidEntities entries
...
class CFrameSnapshotManager {
 ...
 public:
  ...
  // Note: the returned snapshot has a recount of 1 so you MUST call
  // ReleaseReference on it.
  ...
  // if we are copying a Packed Entity, we have to increase the reference
  // counter
  void AddEntityReference(PackedEntityHandle_t handle);

  // if we are removeing a Packed Entity, we have to decrease the reference
  // counter
  void RemoveEntityReference(PackedEntityHandle_t handle);

  // Uses a previously sent packet
  bool UsePreviouslySentPacket(CFrameSnapshot* pSnapshot, int entity,
                               int entSerialNumber);

  bool ShouldForceRepack(CFrameSnapshot* pSnapshot, int entity,
                         PackedEntityHandle_t handle);

  PackedEntity* GetPreviouslySentPacket(int iEntity, int iSerialNumber);

  // Return the entity sitting in iEntity's slot if iSerialNumber matches its
  // number.
 ...
 private:
  ...
  CUtlVector<UnpackedDataCache_t>
      m_PackedEntityCache;  // cache for uncompressed packed entities
