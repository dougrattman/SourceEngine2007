...
struct CUDPSocket::CImpl {
  struct sockaddr_in m_SocketIP;
};

CUDPSocket::CUDPSocket()
    : m_socketIP(), m_Socket(INVALID_SOCKET), m_Port(0), m_pImpl(new CImpl) {
  Q_memset(&m_socketIP, 0, sizeof(m_socketIP));
}

CUDPSocket::~CUDPSocket() { delete m_pImpl; }
...
bool CUDPSocket::RecvFrom(netadr_t &packet_from, CUtlBuffer &data) {
  ...
  int nBytesRead =
      recvfrom(m_Socket, pBuffer, nMaxBytesToRead, 0, &from, &nFromLen);
...
bool CUDPSocket::SendTo(const netadr_t &recipient, const byte *data,
                        size_t datalength) {
  sockaddr dest;
  ...
  // Send data
  int bytesSent =
      sendto(m_Socket, (const char *)data, (int)datalength, 0,
