...
bool GetCurrentPurchaseCount(void) {
  if (mysql->InitMySQL("gamestats_cstrike", pHostName, pUserName, pPassword)) {
    ...
    while (bFoundNext && iIndex < WEAPON_MAX) {
      ...
      g_iCurrentWeaponPurchases[iIndex] =
          mysql->GetColumnValue_Int(mysql->GetColumnIndex("Count"));
    ...
    g_iResetCounter =
        mysql->GetColumnValue_Int(mysql->GetColumnIndex("counter"));
    ...
    while (bFoundNext && iIndex < WEAPON_MAX) {
      ...
      int iWeaponID =
          mysql->GetColumnValue_Int(mysql->GetColumnIndex("WeaponID"));
      g_Weapons[iWeaponID].iCurrentPrice =
          mysql->GetColumnValue_Int(mysql->GetColumnIndex("current_price"));

      if (g_bWeeklyUpdate == true) {
        int iPreviousPurchases = mysql->GetColumnValue_Int(
            mysql->GetColumnIndex("purchases_thisweek"));
        g_Weapons[iWeaponID].iPurchaseCount = g_iPurchaseDelta[iWeaponID] =
            iPreviousPurchases;
      } else {
        int iPreviousPurchases =
            mysql->GetColumnValue_Int(mysql->GetColumnIndex("purchases"));
        int iPurchasesThisWeek = mysql->GetColumnValue_Int(
            mysql->GetColumnIndex("purchases_thisweek"));

        g_iPurchaseDelta[iWeaponID] =
            g_iCurrentWeaponPurchases[iWeaponID] - iPreviousPurchases;

        g_Weapons[iWeaponID].iPurchaseCount =
            g_iPurchaseDelta[iWeaponID] + iPurchasesThisWeek;
      }
    ...
    if (g_bWeeklyUpdate == true) {
      ...
      while (iIndex < WEAPON_MAX) {
        ...
        Q_snprintf(szQueryString, 512,
                   "select * from purchases_pricing where snapshot=\"%d_%d\"",
                   g_iCounter, iIndex);
...
void PrintNewPrices(void) {
  ...
  for (int j = 0; j < WEAPON_MAX; j++) {
    ...
    // g_Weapons[j].iDefaultPrice, g_iNewWeaponPrices[j] );
  ...
  for (int i = 0; i < iCount; i++) {
    Msg("#%d: %s \t\t| Purchases: %d \t| Current Price: %d \t| New Price: %d "
        "\t| Price Change: %d\n",
        i + 1, s_WeaponAliasInfo[g_PistolRanks[i].iID],
        g_Weapons[g_PistolRanks[i].iID].iPurchaseCount,
        g_PistolRanks[i].iCurrentPrice,
        g_iNewWeaponPrices[g_PistolRanks[i].iID],
        g_iPriceDelta[g_PistolRanks[i].iID]);
  }

  iCount = GetWeaponTypeCount(WEAPON_EVERYTHING);

  SortWeaponRanks(WEAPON_EVERYTHING, g_RifleAndEquipmentRanks,
                  SORT_PERCENTCHANGE);
  ...
  for (int i = 0; i < iCount; i++) {
    Msg("#%d: %s \t\t| Purchases: %d \t| Current Price: %d \t| New Price: %d "
        "\t| Price Change: %d\n",
        i + 1, s_WeaponAliasInfo[g_RifleAndEquipmentRanks[i].iID],
        g_Weapons[g_RifleAndEquipmentRanks[i].iID].iPurchaseCount,
        g_RifleAndEquipmentRanks[i].iCurrentPrice,
        g_iNewWeaponPrices[g_RifleAndEquipmentRanks[i].iID],
        g_iPriceDelta[g_RifleAndEquipmentRanks[i].iID]);
  }
...
void SortWeaponRanks(int iWeaponType, weapons_t *pWeaponsArray, int iSortType) {
  ...
  while (bSorted) {
    ...
    for (int i = 1; i < iCount; i++) {
      float flSize1 = 0;  // (pWeaponsArray[i].iPurchaseCount * PRICE_SCALE) *
                          // (pWeaponsArray[i].iCurrentPrice * PRICE_SCALE);
      float flSize2 = 0;  //(pWeaponsArray[i-1].iPurchaseCount * PRICE_SCALE) *
                          //(pWeaponsArray[i-1].iCurrentPrice * PRICE_SCALE);
      ...
      if (iSortType == SORT_PURCHASE) {
        ...
        flSize1 = (pWeaponsArray[i].iCurrentPrice /
                   g_iNewWeaponPrices[pWeaponsArray[i].iID]) -
                  1;
        flSize2 = (pWeaponsArray[i - 1].iCurrentPrice /
                   g_iNewWeaponPrices[pWeaponsArray[i - 1].iID]) -
                  1;
      }
      if (flSize1 > flSize2) {
...
void CalculateNewPrices(weapons_t *pWeaponsArray, int iWeaponType) {
  ...
  for (int i = 0; i < iCount; i++) {
    float flDollarAmount =
        ((float)pWeaponsArray[i].iCurrentPrice * PRICE_SCALE) *
        ((float)pWeaponsArray[i].iPurchaseCount * PRICE_SCALE);
    float flTest = flDollarAmount / (float)iTotalDollars;
    float flTest2 =
        (float)pWeaponsArray[i].iPurchaseCount / (float)iTotalPurchases;

    if (iSign == 1) {
      float flMod = (float)pWeaponsArray[i].iDefaultPrice /
                    (float)pWeaponsArray[i].iCurrentPrice;

      g_iNewWeaponPrices[pWeaponsArray[i].iID] =
          ceil(pWeaponsArray[i].iCurrentPrice +
               ((pWeaponsArray[i].iCurrentPrice * flTest2) * flMod));
      iTotalUp += iTotalDollars * flTest;
    } else {
      float flMod = (float)pWeaponsArray[i].iCurrentPrice /
                    (float)pWeaponsArray[i].iDefaultPrice;

      g_iNewWeaponPrices[pWeaponsArray[i].iID] =
          pWeaponsArray[i].iCurrentPrice - ((1.0f / flTest2) * flMod);
    ...
    g_iPriceDelta[pWeaponsArray[i].iID] =
        g_iNewWeaponPrices[pWeaponsArray[i].iID] -
        pWeaponsArray[i].iCurrentPrice;
...
void ProcessBlackMarket(void) {
  ...
  for (int i = 1; i < WEAPON_MAX; i++) {
    if (g_Weapons[i].iWeaponType == WEAPON_PISTOL) {
      g_flTotalDollarsPistols +=
          ((float)g_Weapons[i].iCurrentPrice * PRICE_SCALE) *
          ((float)g_Weapons[i].iPurchaseCount * PRICE_SCALE);
      g_flTotalWeaponPurchasesPistols += g_Weapons[i].iPurchaseCount;

      g_PistolRanks[iPistol] = g_Weapons[i];
      g_PistolRanks[iPistol].iPurchaseCount =
          g_Weapons[i].iPurchaseCount;  // g_iCurrentWeaponPurchases[i];
      iPistol++;
    } else if (g_Weapons[i].iWeaponType == WEAPON_EVERYTHING) {
      g_flTotalDollarsRifleEquipment +=
          ((float)g_Weapons[i].iCurrentPrice * PRICE_SCALE) *
          ((float)g_Weapons[i].iPurchaseCount * PRICE_SCALE);
      g_flTotalWeaponPurchasesRifleEquipment += g_Weapons[i].iPurchaseCount;

      g_RifleAndEquipmentRanks[iRifles] = g_Weapons[i];
      g_RifleAndEquipmentRanks[iRifles].iPurchaseCount =
          g_Weapons[i].iPurchaseCount;  // g_iCurrentWeaponPurchases[i];
      iRifles++;
    }
  }
...
void AddSnapshotToDatabase(void) {
  ...
  if (g_bWeeklyUpdate == true) {
    ...
    for (int i = 1; i < WEAPON_MAX; i++) {
      Q_snprintf(szSnapshot, sizeof(szSnapshot),
                 "update weapon_info set current_price=%d, "
                 "purchases_thisweek=0 where WeaponID=%d",
                 g_iNewWeaponPrices[i], i);
    ...
    Q_snprintf(szSnapshot, sizeof(szSnapshot),
               "update reset_counter set counter=counter+1");
  ...
  for (int i = 1; i < WEAPON_MAX; i++) {
    Q_snprintf(szSnapshot, sizeof(szSnapshot),
               "Insert into purchases_pricing ( snapshot, dt2, weapon_id, "
               "purchases, current_price, projected_price, reset_counter ) "
               "values ( \"%d_%d\", NOW(), %d, %d, %d, %d, %d );",
               g_iCounter, i, i, g_iPurchaseDelta[i],
               g_Weapons[i].iCurrentPrice, g_iNewWeaponPrices[i],
               g_iResetCounter);
  ...
  Q_snprintf(szSnapshot, sizeof(szSnapshot),
             "update snapshot_counter set counter=%d", g_iCounter);
  ...
  for (int i = 1; i < WEAPON_MAX; i++) {
    Q_snprintf(szSnapshot, sizeof(szSnapshot),
               "update weapon_info set purchases=%d, "
               "purchases_thisweek=purchases_thisweek+%d where WeaponID=%d",
               g_iCurrentWeaponPurchases[i], g_iPurchaseDelta[i], i);
...
int main(int argc, char *argv[]) {
  ...
  if (GetCurrentPurchaseCount() == false) exit(-1);
