...
CBugReporter::CBugReporter() {
  ...
  m_ReportType.AddToTail(
      m_BugStrings.AddString("Network / Connectivity Problems"));
  m_ReportType.AddToTail(
      m_BugStrings.AddString("Download / Installation Problems"));
  m_ReportType.AddToTail(m_BugStrings.AddString("In-game Crash"));
  m_ReportType.AddToTail(
      m_BugStrings.AddString("Game play / Strategy Problems"));
  m_ReportType.AddToTail(m_BugStrings.AddString("Steam Problems"));
  m_ReportType.AddToTail(m_BugStrings.AddString("Unlisted Bug"));
  m_ReportType.AddToTail(
      m_BugStrings.AddString("Feature Request / Suggestion"));
}
...
char const *CBugReporter::GetUserName() { return m_UserName.String(); }

char const *CBugReporter::GetUserName_Display() { return GetUserName(); }

int CBugReporter::GetNameCount() { return 1; }
...
int CBugReporter::GetDisplayNameCount() { return 1; }
...
int CBugReporter::GetPriorityCount() { return 0; }

char const *CBugReporter::GetPriority(int index) { return "<<Invalid>>"; }

int CBugReporter::GetAreaCount() { return 0; }

char const *CBugReporter::GetArea(int index) { return "<<Invalid>>"; }

int CBugReporter::GetAreaMapCount() { return 0; }

char const *CBugReporter::GetAreaMap(int index) { return "<<Invalid>>"; }

int CBugReporter::GetMapNumberCount() { return 0; }

char const *CBugReporter::GetMapNumber(int index) { return "<<Invalid>>"; }
...
bool CBugReporter::CommitBugReport(int &bugSubmissionId) {
  ...
  */
  ...
  int attachedfilesize =
      (m_pBug->zip[0] == 0) ? 0 : g_pFileSystem->Size(m_pBug->zip);

  if (!UploadBugReport(m_cserIP, m_SteamID, atoi(m_pBug->build), m_pBug->title,
                       (char const *)buf.Base(), m_pBug->exename,
                       m_pBug->gamedir, m_pBug->level, m_pBug->reporttype,
                       m_pBug->owner, m_pBug->submitter, m_pBug->ram,
                       m_pBug->cpu, m_pBug->processor, m_pBug->dxversionhigh,
                       m_pBug->dxversionlow, m_pBug->dxvendor, m_pBug->dxdevice,
                       m_pBug->osversion, m_pBug->zip, attachedfilesize)) {
...
void CBugReporter::SetSubmitter(char const *username /*= 0*/) {
  ...
  */
  ...
  Assert(m_pBug);
  Q_strncpy(m_pBug->submitter, username ? username : "",
            sizeof(m_pBug->submitter));
}
...
void CBugReporter::SetGameDirectory(char const *game_dir) {
  Assert(m_pBug);

  Q_FileBase(game_dir, m_pBug->gamedir, sizeof(m_pBug->gamedir));
}
...
void CBugReporter::SetDXVersion(unsigned int high, unsigned int low,
                                unsigned int vendor, unsigned int device) {
  Assert(m_pBug);
