...
typedef void (*BUGREPORTREPORTPROGRESSFUNC)(
    u32 uContext, const TBugReportProgress& rBugReportProgress);

static void BugUploadProgress(u32 uContext,
                              const TBugReportProgress& rBugReportProgress) {
...
//-----------------------------------------------------------------------------
inline void Encrypt8ByteSequence(IceKey& cipher, const unsigned char* plainText,
...
// Purpose:
//-----------------------------------------------------------------------------
void EncryptBuffer(IceKey& cipher, unsigned char* bufData,
                   uint32_t bufferSize) {
...
bool UploadBugReport(const netadr_t& cserIP, const TSteamGlobalUserID& userid,
  ...
  Q_strncpy(params.m_sExecutableName, exename,
            sizeof(params.m_sExecutableName));
  Q_strncpy(params.m_sGameDirectory, game_dir, sizeof(params.m_sGameDirectory));
  ...
  Q_strncpy(params.m_sAttachmentFile, attachedfile,
            sizeof(params.m_sAttachmentFile));
...
class CWin32UploadBugReport {
 public:
  explicit CWin32UploadBugReport(
      ...
      const TBugReportParameters& rBugReportParameters, u32 contextid);
 ...
 private:
  ...
  bool ConnectToHarvesterServer(EBugReportUploadStatus& status,
                                CUtlBuffer& buf);
  ...
  bool ReceiveFileUploadSuccess(EBugReportUploadStatus& status,
                                CUtlBuffer& buf);
  ...
  typedef bool (CWin32UploadBugReport::*pfnProtocolStateHandler)(
      EBugReportUploadStatus& status, CUtlBuffer& buf);
  struct FSMState_t {
...
CWin32UploadBugReport::CWin32UploadBugReport(
    const netadr_t& harvester, const TBugReportParameters& rBugReportParameters,
    u32 contextid)
    : m_States(),
  ...
  AddState(eConnectToHarvesterServer,
           &CWin32UploadBugReport::ConnectToHarvesterServer);
  ...
  AddState(eReceiveFileUploadSuccess,
           &CWin32UploadBugReport::ReceiveFileUploadSuccess);
...
//
//-----------------------------------------------------------------------------
bool CWin32UploadBugReport::DoBlockingReceive(uint32_t bytesExpected,
                                              CUtlBuffer& buf) {
...
void CWin32UploadBugReport::AddState(uint32_t StateIndex,
...
EBugReportUploadStatus CWin32UploadBugReport::Upload(CUtlBuffer& buf) {
  UpdateProgress(m_rBugReportParameters,
...
void CWin32UploadBugReport::SetNextState(uint32_t StateIndex) {
  Assert(StateIndex > m_uCurrentState);
  m_uCurrentState = StateIndex;
}

bool CWin32UploadBugReport::CreateTCPSocket(EBugReportUploadStatus& status,
                                            CUtlBuffer& /*buf*/) {
...
bool CWin32UploadBugReport::ConnectToHarvesterServer(
    EBugReportUploadStatus& status, CUtlBuffer& /*buf*/) {
  UpdateProgress(m_rBugReportParameters,
...
bool CWin32UploadBugReport::SendProtocolVersion(EBugReportUploadStatus& status,
                                                CUtlBuffer& buf) {
  UpdateProgress(m_rBugReportParameters,
...
bool CWin32UploadBugReport::ReceiveProtocolOkay(EBugReportUploadStatus& status,
                                                CUtlBuffer& buf) {
  UpdateProgress(m_rBugReportParameters,
...
bool CWin32UploadBugReport::SendUploadCommand(EBugReportUploadStatus& status,
                                              CUtlBuffer& buf) {
  UpdateProgress(m_rBugReportParameters,
...
bool CWin32UploadBugReport::ReceiveOKToSendFile(EBugReportUploadStatus& status,
                                                CUtlBuffer& buf) {
  UpdateProgress(m_rBugReportParameters,
...
bool CWin32UploadBugReport::SendWholeFile(EBugReportUploadStatus& status,
                                          CUtlBuffer& /*buf*/) {
  UpdateProgress(m_rBugReportParameters, "Uploading bug report data.");
  // Send to server
  char* filebuf = NULL;
  size_t sizeactual =
      g_pFileSystem->Size(m_rBugReportParameters.m_sAttachmentFile);
  if (sizeactual > 0) {
...
bool CWin32UploadBugReport::ReceiveFileUploadSuccess(
    EBugReportUploadStatus& status, CUtlBuffer& buf) {
  UpdateProgress(m_rBugReportParameters,
...
bool CWin32UploadBugReport::SendGracefulClose(EBugReportUploadStatus& status,
                                              CUtlBuffer& buf) {
...
bool CWin32UploadBugReport::CloseTCPSocket(EBugReportUploadStatus& status,
                                           CUtlBuffer& /*buf*/) {
...
EBugReportUploadStatus Win32UploadBugReportBlocking(
  ...
  encrypted.PutInt(
      rBugReportParameters.m_uEngineBuildNumber);  // build_identifier
  ...
  encrypted.PutInt(
      rBugReportParameters.m_uDXVersionHigh);  // driver version high part
  encrypted.PutInt(
      rBugReportParameters.m_uDXVersionLow);  // driver version low part
  ...
  {
    encrypted.Put(&rBugReportParameters.m_userid,
  ...
  if (bcs.WaitForMessage(2.0f)) {
    ...
    uint32_t bytesReceived =
        bcs.ReceiveSocketMessage(&replyaddress, (u8*)buf.Base(), 2048);
    if (bytesReceived > 0) {
      // Fixup actual size
      ...
      UpdateProgress(rBugReportParameters,
                     "Server requested bug report upload.");

      // Keep using the same scratch buffer for messaging
      CWin32UploadBugReport uploader(BugReportHarvesterFSMIPAddress,
                                     rBugReportParameters, dumpcontext);
