...
CSQLTable::CSQLTable(CSQLTable const &sqlTable) {
  ...
  for (int iSQLColumn = 0; iSQLColumn < sqlTable.m_VecSQLColumn.Count();
       iSQLColumn++) {
    m_VecSQLColumn.AddToTail(
        CSQLColumn(sqlTable.m_VecSQLColumn[iSQLColumn].PchColumnName(),
...
bool CSQLTableSet::Init(ISQLHelper *pSQLHelper) {
  ...
  for (int i = 0; i < m_VecSQLTable.Count(); i++) {
    ...
    char szQuery[512];
    Q_snprintf(szQuery, sizeof(szQuery), "select * from %s limit 1",
               sqlTable.PchName());
    MYSQL_RES *pMySQLRes = NULL;
...
// Purpose: constructor
//-----------------------------------------------------------------------------
CSQLRow::CSQLRow(MYSQL_ROW *pMySQLRow, const ISQLTable *pSQLTableDescription,
                 int cRowData) {
  ...
  for (int iRowData = 0; iRowData < cRowData; iRowData++) {
    ...
    const CSQLTable *tableDesc =
        static_cast<const CSQLTable *>(pSQLTableDescription);
    switch (tableDesc->GetESQLFieldType(iRowData)) {
      ...
      case FIELD_TYPE_YEAR:
      case FIELD_TYPE_NEWDATE:
        sqlRowData.data.ulTime = time(NULL);  // TODO(d.rattman):
...
//-----------------------------------------------------------------------------
// Purpose: returns the time value of this column (time_t equivalent, seconds
...
// Purpose: constructor
//-----------------------------------------------------------------------------
CResultSet::CResultSet() : m_SQLTableDescription("Table") {
  m_pSQLRow = NULL;
  m_MySQLRes = NULL;
}
...
bool CResultSet::Query(const char *pchQuery, ISQLHelper *pSQLHelper) {
  ...
  for (int iMySQLField = 0; iMySQLField < m_cSQLField; iMySQLField++) {
    m_SQLTableDescription.AddColumn(pMySQLField[iMySQLField].name,
                                    pMySQLField[iMySQLField].type);
  }
