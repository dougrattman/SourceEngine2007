...
bool CVCProjConvert::LoadProject(const char *project) {
  ...
  if (FAILED(hr)) {
    ...
    Msg("https://support.microsoft.com/en-us/help/269238/"
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (pNode) {
        ...
        if (toolName == "VCCLCompilerTool") {
          CUtlSymbol defines =
              GetXMLAttribValue(pElem, "PreprocessorDefinitions");
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (pNode) {
        ...
        if (fileName.IsValid()) {
          ...
          IterateFileConfigurations(pElem,
                                    fileName);  // now remove the excluded ones
        }
      }
    }  // for
  }
