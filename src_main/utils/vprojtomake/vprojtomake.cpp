...
class MyFileSystem : public IBaseFileSystem {
 public:
  ...
  bool Precache(const char *pFileName, const char *pPathID = 0) {
    return false;
  }
  ...
  bool SetFileWritable(char const *pFileName, bool writable,
                       const char *pPathID = 0) {
    return false;
  }
  long GetFileTime(const char *pFileName, const char *pPathID = 0) { return 0; }
  bool ReadFile(const char *pFileName, const char *pPath, CUtlBuffer &buf,
...
void OutputKeyValuesVersion(CVCProjConvert &proj) {
  ...
  for (int projIndex = 0; projIndex < proj.GetNumConfigurations();
       projIndex++) {
    ...
    for (int fileIndex = 0; fileIndex < config.GetNumFileNames(); fileIndex++) {
      if (config.GetFileType(fileIndex) ==
          CVCProjConvert::CConfiguration::FILE_SOURCE) {
        char num[20];
