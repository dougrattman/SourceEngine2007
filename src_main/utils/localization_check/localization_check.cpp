...
//
// Purpose: localization_check.cpp : Defines the entry point for the console
// application.
//
//
// What the tool does:
...
//
// 1)  For each sound emitted by a vcd not marked as CC_DISABLED, verify that
// there's an entry in the localization table 2)  For each combined sound in a
// vcd, make sure there's a valid entry in the localization table 3)  For each
// combined sound, verify that the english version combined .wav file has the
// proper checksum 4)  Note any files in the closecaption folder which are
// orphaned after parsing the above 5)  If hl2_french directories etc. exist,
// then compare combined .wav files with localized versions and
//   see if localized checksum tag differs from US one, or warn if tag missing,
//   but complain if .wav duration is different.
//
//	UNDONE:re-create combined .wav files in english to the extent that the
//checksums mismatch?
//
//===========================================================================//
#include <conio.h>
...
void BuildFileList_R(CUtlVector<CUtlSymbol> &files, char const *dir,
...
void BuildFileList(CUtlVector<CUtlSymbol> &files, char const *rootdir,
                   char const *extension) {
  files.RemoveAll();
...
char const *FacePoser_TranslateSoundNameGender(char const *soundname,
                                               gender_t gender) {
  if (Q_stristr(soundname, ".wav")) return PSkipSoundChars(soundname);

  return PSkipSoundChars(
      g_pSoundEmitterSystem->GetWavFileForSound(soundname, gender));
}
...
bool LoadSentenceFromWavFileUsingIO(char const *wavfile, CSentence &sentence,
                                    IFileReadBinary &io,
                                    void *formatbuffer = NULL,
                                    int *formatsize = NULL,
                                    int *datasize = NULL) {
  int insize = 0;
...
bool LoadSentenceFromWavFile(char const *wavfile, CSentence &sentence,
                             void *formatbuffer = NULL, int *formatsize = NULL,
                             int *dataSize = NULL) {
  return LoadSentenceFromWavFileUsingIO(wavfile, sentence, io_in, formatbuffer,
                                        formatsize, dataSize);
}
...
bool ValidateCombinedFileCheckSum(
    ...
    CUtlRBTree<CUtlSymbol, int> &referencedcaptionwaves) {
  CUtlVector<CombinerEntry> work;

  char actualfile[512];
  g_pSoundEmitterSystem->GenderExpandString(gender, outfilename, actualfile,
                                            sizeof(actualfile));
  ...
  bool valid = soundcombiner->IsCombinedFileChecksumValid(g_pFullFileSystem,
                                                          actualfile, work);
  if (!valid) {
    vprint(0,
           "combined file (%s) checksum mismatch for '%s'\n  event '%s' of "
           "scene '%s'\n",
           actualfile, cctoken, sorted[0]->GetName(),
           sorted[0]->GetScene()->GetFilename());
    ...
    if (regenerate) {
      ...
      if (dothisfile) {
        bool success = soundcombiner->CombineSoundFiles(g_pFullFileSystem,
                                                        actualfile, work);
    ...
    if (regenerate) {
      vprint(0,
             "combined file (%s) checksum still matches for %s, skipping "
             "rebuild...\n",
             actualfile, cctoken);
    }
  }
  ...
  if (referencedcaptionwaves.Find(sym) ==
      referencedcaptionwaves.InvalidIndex()) {
...
static bool SymbolLessFunc(const CUtlSymbol &p1, const CUtlSymbol &p2) {
  if (Q_stricmp(g_Analysis.symbols.String(p1), g_Analysis.symbols.String(p2)) <
      0)
    return true;
  return false;
}

bool ValidateCombinedSoundCheckSum(
    CChoreoEvent *e, CUtlRBTree<CUtlSymbol, int> &referencedcaptionwaves) {
...
struct PerMapVCDS {
  ...
  class CTree : public CUtlRBTree<CUtlSymbol> {
   public:
    CTree() : CUtlRBTree<CUtlSymbol>(0, 0, DefLessFunc(CUtlSymbol)) {}
...
void ParseVCDFilesFromResList(CUtlVector<CUtlSymbol> &vcdsinreslist,
                              char const *resfile) {
  char gd[256];
  ...
  if (FILESYSTEM_INVALID_HANDLE != resfilehandle) {
    ...
    if (length > 0) {
      ...
      if (pStart &&
          (length == g_pFullFileSystem->Read(pStart, length, resfilehandle))) {
        pStart[length] = 0;
        ...
        while (1) {
          ...
          if (strlen(token) > 0) {
            ...
            if (!Q_strnicmp(szFileName, gd, gdlen)) {
              ...
              // Ack
              // vprint( 1, "File %s not under game directory but in reslist,
              // skipping!!!\n", szFileName );
              pFileList = ParseFile(pFileList, token, NULL);
              continue;
            }
            ...
            if (idx == vcdsinreslist.InvalidIndex()) {
              ...
              // This is the first time this vcd was encountered, remember which
              // map we are in
              PerMapVCDS e;
              e.vcds.Insert(sym);
  ...
  //	vprint( 1, "\rFound %i new resources (%7i total) in %64s", filesFound,
  //resourcesConsidered, resfile );
}
...
void BuildVCDAndMapNameListsFromReslists(
  ...
  if (g_pFullFileSystem->FileExists(MAPLIST_FILE)) {
    ...
    if (FILESYSTEM_INVALID_HANDLE != resfilehandle) {
      ...
      if (length > 0) {
        char *pStart = (char *)new char[length + 1];
        if (pStart && (length == g_pFullFileSystem->Read(pStart, length,
                                                         resfilehandle))) {
          pStart[length] = 0;
  ...
  for (int i = 0; i < c; ++i) {
    ...
    ParseVCDFilesFromResList(vcdsinreslist,
                             g_Analysis.symbols.String(reslists[i]));
  }

  EndPacifier(true);
}

void CheckUnusedVcds(CUtlVector<CUtlSymbol> &vcdsinreslist,
...
void ParseUsedSoundsFromSndFile(CUtlRBTree<int, int> &usedsounds,
                                char const *sndfile) {
  char gd[256];
  Q_strncpy(gd, gamedir, sizeof(gd));
  Q_StripTrailingSlash(gd);
        ...
        while (1) {
          ...
          if (strlen(token) > 0) {
            ...
            if (!g_pSoundEmitterSystem->IsValidIndex(index)) {
              vprint(1,
                     "---> Sound %s doesn't exist in "
                     "g_pSoundEmitterSystemsystem!!!\n",
                     soundname);
              continue;
            }
  ...
  vprint(1, "Found %i new resources (%i total) in %s\n", addedStrings,
         resourcesConsidered, sndfile);
}

bool SplitCommand(wchar_t const **ppIn, wchar_t *cmd, wchar_t *args);

void SpewDuplicatedText(char const *lang, const char *entry,
                        const wchar_t *str) {
  ...
  *out = L'\0';
  ...
  // Now see how many characters from the first 50% of the text are also in the
  // second 50%
  int halflen = len / 2;
...
void CheckDuplcatedText(void) {
  ...
  for (int lang = 0; lang < CC_NUM_LANGUAGES; ++lang) {
    ...
    // Now check for closecaption_xxx.txt entries which are orphaned because
    // there isn't an existing sound script entry in use for them
...
void ExtractEnglish() {
  ...
  // Now check for closecaption_xxx.txt entries which are orphaned because there
  // isn't an existing sound script entry in use for them
  StringIndex_t str = g_pVGuiLocalize->GetFirstStringIndex();
  while (str != vgui::INVALID_STRING_INDEX) {
    char const *keyname = g_pVGuiLocalize->GetNameByIndex(str);
    if (keyname) {
...
void CheckUnusedSounds() {
  ...
  for (int i = 0; i < c; ++i) {
    ParseUsedSoundsFromSndFile(usedsounds,
                               g_Analysis.symbols.String(sndlists[i]));
  }
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (invalidindex == slot) {
      ...
      vprint(1, "  unref:  %s : %s\n", soundname,
             g_pSoundEmitterSystem->GetSourceFileForSound(i));
    } else {
      int scriptindex = g_pSoundEmitterSystem->FindSoundScript(
          g_pSoundEmitterSystem->GetSourceFileForSound(i));
      if (scriptindex != -1) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (usedscripts.InvalidIndex() == slot) {
      vprint(1,
             "  No sounds fron script %s are being used, should delete from "
             "manifest!!!\n",
             scriptname);
    }
  }
  ...
  while (str != vgui::INVALID_STRING_INDEX) {
    ...
    if (keyname) {
      ...
      if (soundindex == -1) {
        vprint(1,
               "  cc token %s not in current g_pSoundEmitterSystem scripts\n",
               keyname);
        ...
        if (usedsounds.InvalidIndex() == slot) {
          vprint(
              1,
              "  cc token %s exists, but the sound is not used by the game\n",
              keyname);

          logprint("cc_delete.txt", "\"%s\"\n", keyname);
        } else {
          // Now try to find a better bit of text
          CSoundParametersInternal *internal =
              g_pSoundEmitterSystem->InternalGetParametersForSound(soundindex);
          if (internal && internal->NumSoundNames() > 0) {
            ...
            if (wavname &&
                (Q_stristr(wavname, "vo/") || Q_stristr(wavname, "vo\\"))) {
              // See if 1) it's marked as !!! and try to figure out the text
              // from .wav files...
              if (!Q_strnicmp(ansi, "!!!", 3)) {
                CSentence sentence;
                if (LoadSentenceFromWavFile(
                        va("sound/%s", PSkipSoundChars(wavname)), sentence)) {
              ...
              for (int w = 0; w < internal->NumSoundNames(); ++w) {
                char const *wavname = g_pSoundEmitterSystem->GetWaveName(
                    internal->GetSoundNames()[w].symbol);
                logprint("todo.csv", "\"%s\",\"%s\",\"%s\"\n", keyname, ansi,
                         va("sound/%s", PSkipSoundChars(wavname)));
              }
            }
          } else {
            logprint("fixed.txt", "\t\"%s\"\t\t\"%s\"\n", keyname, ansi);
  ...
  // Now walk through all of the sounds that were used, but not in the
  // localization file and and those, too
  c = g_pSoundEmitterSystem->GetSoundCount();
  for (int i = 0; i < c; ++i) {
    int slot = usedsounds.Find(i);
    if (usedsounds.InvalidIndex() == slot) continue;
    ...
    if (text) {
      ...
      // Now try to find a better bit of text
      CSoundParametersInternal *internal =
          g_pSoundEmitterSystem->InternalGetParametersForSound(i);
      if (internal && internal->NumSoundNames() > 0) {
        CUtlSymbol &symwave = internal->GetSoundNames()[0].symbol;
        char const *wavname = g_pSoundEmitterSystem->GetWaveName(symwave);
        if (wavname &&
            (Q_stristr(wavname, "vo/") || Q_stristr(wavname, "vo\\"))) {
          CSentence sentence;
          if (LoadSentenceFromWavFile(va("sound/%s", PSkipSoundChars(wavname)),
                                      sentence)) {
            if (Q_strlen(sentence.GetText()) > 0) {
              Q_snprintf(ansi, sizeof(ansi), "%s", sentence.GetText());
              cleanquotes(ansi);
  ...
  for (int i = list.FirstInorder(); i != list.InvalidIndex();
    ...
    if (soundindex != -1) {
      ...
      if (internal && internal->NumSoundNames() > 0) {
        ...
        if (pname && (Q_stristr(pname, "vo/") || Q_stristr(pname, "vo\\") ||
                      Q_stristr(pname, "combined"))) {
          ...
          if (LoadSentenceFromWavFile(va("sound/%s", PSkipSoundChars(wavname)),
                                      sentence)) {
            if (Q_strlen(sentence.GetText()) > 0) {
              Q_snprintf(sentence_text, sizeof(sentence_text), "%s",
                         sentence.GetText());
    ...
    if (!str) {
      logprint("script.txt", "\t\tMissing token '%s' event '%s'\n\n", tok,
               e->GetName());
      Q_snprintf(ansi, sizeof(ansi), "missing '%s' for '%s'", tok,
                 e->GetName());
    } else {
    ...
    if (idx != g_FirstMapForVCD.InvalidIndex()) {
      Q_strncpy(mapname, g_Analysis.symbols.String(g_FirstMapForVCD[idx]),
                sizeof(mapname));
    }
    ...
    logprint("script.csv", "%u,%s,%s,%s,%6.3f,%s,%s,\"%s\",\"%s\"\n",
             sortindex++, mapname, vcdname, actorname, e->GetStartTime(), tok,
             wavname, ansi, sentence_text);
  }
}

void CheckLocalizationEntries(
    CUtlVector<CUtlSymbol> &vcdfiles,
    CUtlRBTree<CUtlSymbol, int> &referencedcaptionwaves) {
  int disabledcount = 0;
  int validcount = 0;
  ...
  for (int i = 0; i < c; ++i) {
    ...
    CUtlRBTree<CChoreoEvent *, int> sortedSpeakEvents(0, 0,
                                                      EventStartTimeLessFunc);

    // Load the .vcd
    char fullname[512];
    Q_snprintf(fullname, sizeof(fullname), "%s",
               g_Analysis.symbols.String(vcdname));

    LoadScriptFile(fullname);

    CChoreoScene *scene =
        ChoreoLoadScene(fullname, NULL, &g_TokenProcessor, Con_Printf);
    if (!scene) {
    ...
    for (int j = 0; j < numevents; j++) {
      ...
      for (int pass = 0; pass <= 1; ++pass) {
        ...
        if (!str) {
          ...
          // );
          Q_strncpy(fn, &g_Analysis.symbols.String(vcdname)[gamedirskip],
                    sizeof(fn));
          ...
          if (Q_stristr(tok, ".wav")) {
            if (verbose) {
              if (!regenerate_quiet) {
                vprint(0,
                       "(OBSOLETE???)missing cc token '%s' (!.wav file):  vcd "
                       "(%s), event (%s)\n",
                       tok, fn, e->GetName());
              }
            }
            ...
            if (verbose) {
              ...
              if (soundindex != -1) {
                // Now try to find a better bit of text
                CSoundParametersInternal *internal =
                    g_pSoundEmitterSystem->InternalGetParametersForSound(
                        soundindex);
                if (internal && internal->NumSoundNames() > 0) {
                  CUtlSymbol &symwave = internal->GetSoundNames()[0].symbol;
                  char const *wavname =
                      g_pSoundEmitterSystem->GetWaveName(symwave);
                  if (wavname && (Q_stristr(wavname, "vo/") ||
                                  Q_stristr(wavname, "vo\\"))) {
                    CSentence sentence;
                    if (LoadSentenceFromWavFile(
                            va("sound/%s", PSkipSoundChars(wavname)),
                            sentence)) {
                      if (Q_strlen(sentence.GetText()) > 0) {
                        Q_snprintf(suggested, sizeof(suggested), "%s",
                                   sentence.GetText());
  ...
  if (total != 0) {
    vprint(0,
           "\n%.2f %%%% invalid (%i valid, %i missing, %i wavfile(OBSOLETE), "
           "%i disabled - total %i)\n",
           100.0f * (float)missingcount / (float)total, validcount,
           missingcount, wavfile, disabledcount, total);
  }
}

void CheckForOrphanedCombinedWavs(CUtlVector<CUtlSymbol> &diskwaves,
                                  CUtlRBTree<CUtlSymbol, int> &captionsused) {
  ...
  if (orphans != 0) {
    vprint(0,
           "\n%.2f %%%% (%i/%i), orphaned combined .wav files in "
           "sound/combined/... folder\n",
...
void ValidateForeignLanguageWaves(char const *language,
  ...
  for (int i = 0; i < c; ++i) {
    ...
    Q_snprintf(localizedwavename, sizeof(localizedwavename), "%s%s", langdir,
               &wavname[skipchars]);
    ...
    if (sentence_english.GetText() && sentence_english.GetText()[0]) {
      if (!sentence_localized.GetText() || !sentence_localized.GetText()[0]) {
        vprint(0,
               "--> Localized combined file for '%s' doesn't have sentence "
               "data '%s'\n",
               language, localizedwavename);
      } else if (!Q_stricmp(sentence_english.GetText(),
                            sentence_localized.GetText())) {
        vprint(0,
               "--> Localized combined file for '%s' still using english "
               "phoneme and text data '%s'\n",
               language, localizedwavename);
      }
    }

    if (fabs(duration_localized - duration_english) >
        SOUND_DURATION_TOLERANCE) {
      ++outdated;
      vprint(0,
             "--> Mismatched localized file %s (english %.2f s./%s %.2f s.)\n",
             localizedwavename, duration_english, language, duration_localized);
      continue;
    }
  }
  ...
  if (c != 0) {
    vprint(0,
           "%.2f %%%% missing(%i)+outdated(%i)/total(%i), combined .wav files "
           "in %s closecaption/ folder\n",
           100.0f * (float)(missing + outdated) / (float)c, missing, outdated,
           c, language);
  }
}
...
char const *FindSoundEntry(CUtlDict<CUtlSymbol, int> &wavtosound,
                           char const *wavname) {
  char fixed[512];
...
void WavCheck(CUtlVector<CUtlSymbol> &wavfiles) {
  ...
  logprint("wavcheck.csv", "\"%s\",\"%s\",\"%s\",\"%s\"\n", "WaveName",
           "Sound Script Name", "Close Caption", "Phoneme Data String");
...
void BuildWavFileToFullPathLookup(CUtlVector<CUtlSymbol> &wavfile,
...
void MakeBatchFile(CUtlVector<CUtlSymbol> &wavfiles, char const *fromdir,
                   char const *todir) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    char fullname[512];
    Q_snprintf(
        fullname, sizeof(fullname), "%s/%s", todir,
        &g_Analysis.symbols.String(wavfiles[wavtofullpath[slot]])[gamedirskip]);
    Q_strlower(fullname);
...
void ExtractPhonemesForWave(IPhonemeExtractor *extractor, char const *wavname) {
  ...
  CSentence sentence;
  if (!LoadSentenceFromWavFile(wavname, sentence, formatbuffer, &formatsize,
                               &dataSize)) {
    vprint(0, "  skip '%s' missing\n", wavname);
    return;
  }

  if (!forceextract && sentence.m_Words.Count() > 0) {
  ...
  if (format == WAVE_FORMAT_ADPCM) {
    ...
    if (blockRem) {
      sampleCount +=
          pFormat->wSamplesPerBlock - (((blockSize - blockRem) * 2) / channels);
    }
  ...
  int result = extractor->Extract(
      filename,
      dataSize,  // (int)( m_pWaveFile->GetRunningLength() *
                 // m_pWaveFile->SampleRate() * m_pWaveFile->TrueSampleSize() ),
...
int LoadPhonemeExtractors() {
  ...
  const char *pFilename = g_pFullFileSystem->FindFirstEx(
      "phonemeextractors/*.dll", "EXECUTABLE_PATH", &findHandle);
  int useextractor = -1;
  while (pFilename) {
    char fullpath[512];
    ...
    e.extractor =
        (IPhonemeExtractor *)factory(VPHONEME_EXTRACTOR_INTERFACE, NULL);
    if (!e.extractor) {
...
//-----------------------------------------------------------------------------
int ConvertUnicodeToANSI(const wchar_t *unicode, char *ansi,
                         int ansiBufferSize) {
  int result = ::WideCharToMultiByte(CP_UTF8, 0, unicode, -1, ansi,
                                     ansiBufferSize, NULL, NULL);
...
void BuildOrderedCaptionList(CUtlVector<OrderedCaption_t> &list) {
  // parse out the file
  FileHandle_t file =
      g_pFullFileSystem->Open("resource/closecaption_english.txt", "rb");
  ...
  while (1) {
    ...
    if (!keytoken[0]) break;  // we've hit the null terminator
...
struct LookupData_t {
  LookupData_t() : unicode(0), caption(0) {}
...
//-----------------------------------------------------------------------------
// Purpose: importfile is a unicode file contains pairs of .wav names and
// caption strings
...
void ImportCaptions(char const *importfile) {
  ...
  // Now walk the import data, and try to figure out the caption name for each
  // one
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (idx != captionToUnicode.InvalidIndex()) {
      ...
      WriteAsciiStringAsUnicode(buf, "\t", false);
      WriteUnicodeString(
          buf, captionlist[i].english ? captionlist[i].english : L"???", true);
      WriteAsciiStringAsUnicode(buf, "\r\n", false);
    } else {
...
void SyncDucking(CUtlVector<CUtlSymbol> &english,
                 CUtlVector<CUtlSymbol> &localized) {
  int i, c;
  ...
  for (i = 0; i < c; ++i) {
    ...
    bool inenglishducked =
        englishducked.Find(croppedSym) != englishducked.InvalidIndex() ? true
                                                                       : false;
    bool inenglishunducked =
        englishunducked.Find(croppedSym) != englishunducked.InvalidIndex()
            ? true
            : false;
    ...
    if (ducked && inenglishducked) continue;
    if (!ducked && inenglishunducked) continue;

    if (!inenglishducked && !inenglishunducked) {
      vprint(0,
             "Warning:  %s is in localized tree, missing from english tree!!\n",
             fn);
      continue;
    }
...
int CLocalizationCheckApp::Main() {
      ...
      switch (pArg[1]) {
        ...
        case 'l':
          if (!Q_stricmp(&pArg[1], "loop")) {
            checkforloops = true;
            Q_strncpy(sounddir, CommandLine()->GetParm(i + 1),
                      sizeof(sounddir));
            i++;
          } else {
            uselogfile = true;
        ...
        case 'i': {
          importcaptions = true;
          Q_strncpy(importfile, CommandLine()->GetParm(i + 1),
                    sizeof(importfile));
          i++;
        } break;
        case 'm': {
  ...
  if (!forceducking && !checkforloops && !syncducking && !extractphonemes &&
      !importcaptions && language[0] != 0) {
  ...
  if (!checkforloops && !syncducking && !extractphonemes && !importcaptions &&
      language[0] != 0) {
    vprint(0, "\nLanguage:  %s\n", language);
  }
  ...
  if (forceducking) {
    ...
    if (extractenglish && !language[0]) {
      ...
      vprint(0,
             "syncducking:  building list of known .wav files in\n  %s\n  %s\n",
             fromdir, todir);
      ...
      // Check for missing localization data for all speak events not marked
      // CC_DISABLED
      CUtlRBTree<CUtlSymbol, int> referencedcaptionwaves(0, 0, SymbolLessFunc);

      vprint(0,
             "\nValidating close caption tokens and combined .wav file "
             "checksums\n\n");
      ...
      if (language[0] != 0) {
        vprint(0,
               "\nChecking for missing or out of date localized combined .wav "
               "files\n\n");
