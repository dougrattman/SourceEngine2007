...
bool CompareArrays(const CUtlVector<unsigned char> &a1,
                   const CUtlVector<unsigned char> &a2) {
...
void TestChannels(IChannel *pChannel1, IChannel *pChannel2, int maxPacketSize,
                  int nTests) {
  for (int iTest = 0; iTest < nTests; iTest++) {
    ...
    bool bReceived;
    if (!(bReceived = pChannel1->Recv(testVec, 15)) ||
        !CompareArrays(testVec, rnd2)) {
      assert(false);
    }

    if (!(bReceived = pChannel2->Recv(testVec, 15)) ||
        !CompareArrays(testVec, rnd1)) {
      assert(false);
    }
  }
}
...
void TestChannels(T *pSock[2]) {
  ...
  for (int iPort = 0; iPort < 2; iPort++) {
    ...
    for (int iTry = 0; iTry < nTries; iTry++) {
      ...
      if (pSock[iPort]->BindToAny(iPorts[iPort])) break;
    }
  }
  ...
  while (!pSock[0]->IsConnected() || !pSock[1]->IsConnected()) {
    CIPAddr remoteAddr;
    if (!pSock[0]->IsConnected()) pSock[0]->UpdateListen(&remoteAddr);

    if (!pSock[1]->IsConnected()) pSock[1]->UpdateConnect();
  }
  ...
  IChannel *pFrag[2] = {CreateFragmentLayer(pSock[0]),
                        CreateFragmentLayer(pSock[1])};
...
int main(int argc, char *argv[]) {
  ...
  for (int iChannelType = 0; iChannelType < 2; iChannelType++) {
    ...
    if (iChannelType == 0) {
      ...
      IReliableChannel *pReliableChannels[2] = {CreateReliableChannel(),
