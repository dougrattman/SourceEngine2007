  ...
  for (i = 0; i < numfaces; i++) {
    ...
    for (j = 0; j < f->numedges; j++) {
      if (g_numvertnormalindices == MAX_MAP_VERTNORMALINDICES) {
        Error("g_numvertnormalindices == MAX_MAP_VERTNORMALINDICES (%d)",
    ...
    // Note: this doesn't do an exhaustive vertex normal match because the vrad
    // does it. The result is that a little extra memory is wasted coming out of
    // vbsp, but it goes away after vrad.
    if (g_numvertnormals == MAX_MAP_VERTNORMALS) {
      Error("g_numvertnormals == MAX_MAP_VERTNORMALS (%d)",
            MAX_MAP_VERTNORMALS);
    }
