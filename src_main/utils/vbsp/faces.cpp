...
int GetVertexnum(Vector &v) {
  ...
  for (i = 0, dv = dvertexes; i < numvertexes; i++, dv++) {
    for (j = 0; j < 3; j++) {
      d = v[j] - dv->point[j];
      if (d > POINT_EPSILON || d < -POINT_EPSILON) break;
    }
    if (j == 3) return i;  // a match
  }
...
void EmitFaceVertexes(face_t **pListHead, face_t *f) {
  ...
  for (i = 0; i < w->numpoints; i++) {
    if (noweld) {  // make every point unique
      if (numvertexes == MAX_MAP_VERTS)
        Error(
            "Too many unique verts, max = %d (map has too much brush "
            "geometry)\n",
            MAX_MAP_VERTS);
...
void FindEdgeVerts(Vector &v1, Vector &v2) {
  ...
  num_edge_verts = numvertexes - 1;
  for (i = 0; i < num_edge_verts; i++) edge_verts[i] = i + 1;
}
#endif
...
void TestEdge(vec_t start, vec_t end, int p1, int p2, int startvert) {
  ...
  for (k = startvert; k < num_edge_verts; k++) {
    ...
    if (dist <= start || dist >= end) continue;  // off an end
    VectorMA(edge_start, dist, edge_dir, exact);
    VectorSubtract(p, exact, off);
    error = off.Length();

    if (error > OFF_EPSILON) continue;  // not on the edge
  ...
  if (numsuperverts >= MAX_SUPERVERTS)
    Error(
        "Edge with too many vertices due to t-junctions.  Max %d verts along "
        "an edge!\n",
        MAX_SUPERVERTS);
...
struct face_vert_table_t {
  ...
  bool HasEdge(int edge) const {
    if (edge >= 0) {
      if (edge0 == edge || edge1 == edge) return true;
    }
    return false;
  }
...
bool IsDiagonal(const face_vert_table_t &v0, const face_vert_table_t &v1) {
  if (v1.HasEdge(v0.edge0) || v1.HasEdge(v0.edge1)) return false;
...
void FixFaceEdges(face_t **pList, face_t *f) {
  ...
  if (f->badstartvert && entity_num == 0) {
    ...
    if (g_numprimitives > MAX_MAP_PRIMITIVES ||
      ...
      Error(
          "Too many t-junctions to fix up! (%d prims, max %d :: %d indices, "
          "max %d)\n",
...
void FixEdges_r(node_t *node) {
  ...
  for (f = node->faces; f; f = f->next) FixFaceEdges(&node->faces, f);

  for (i = 0; i < 2; i++) FixEdges_r(node->children[i]);
}
...
void GetEdge2_InitOptimizedList() {
  for (int i = 0; i < MAX_MAP_VERTS; i++) g_VertEdgeList[i].RemoveAll();
}
...
winding_t *TryMergeWinding(winding_t *f1, winding_t *f2, Vector &planenormal) {
  ...
  for (i = 0; i < f1->numpoints; i++) {
    ...
    for (j = 0; j < f2->numpoints; j++) {
      ...
      for (k = 0; k < 3; k++) {
        if (fabs((*p1)[k] - (*p4)[k]) > EQUAL_EPSILON) break;
        if (fabs((*p2)[k] - (*p3)[k]) > EQUAL_EPSILON) break;
      }
      if (k == 3) break;
    }
...
bool OverlaysAreEqual(face_t *f1, face_t *f2) {
  ...
  for (int iOverlay = 0; iOverlay < nOverlayCount; ++iOverlay) {
    ...
    if (f2->originalface->aOverlayIds.Find(nOverlayId) == -1) return false;
  }
...
bool FaceOnWaterBrush(face_t *face) {
  ...
  if (!pSide) return false;

  if (pSide->contents & (CONTENTS_WATER | CONTENTS_SLIME)) return true;
...
void MakeFaces_r(node_t *node) {
  ...
  if (node->planenum != PLANENUM_LEAF) {
    ...
    if (!nomerge) MergeFaceList(&node->faces);
    if (!nosubdiv) SubdivideFaceList(&node->faces);
