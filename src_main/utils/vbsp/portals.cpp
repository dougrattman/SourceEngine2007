...
portal_t *AllocPortal(void) {
  ...
  if (numthreads == 1) c_active_portals++;
  if (c_active_portals > c_peak_portals) c_peak_portals = c_active_portals;
...
bool Portal_AreaLeakFlood(portal_t *p, int s) {
  if (!Portal_EntityFlood(p, s)) return false;
...
// because of water areaportals support, the areaportal may not be the only
// brush on this node
...
void MakeTreePortals_r(node_t *node) {
  ...
  for (i = 0; i < 3; i++) {
    if (node->mins[i] < (MIN_COORD_INTEGER - SIDESPACE) ||
        node->maxs[i] > (MAX_COORD_INTEGER + SIDESPACE)) {
      ...
      Warning(
          "WARNING: BSP node with unbounded volume (material: %s, near %s)\n",
...
//			the number of hops from a node with an entity. If we
// ever mark 			the outside_node for this tree, we've leaked.
// Input  : node -
//			dist -
//-----------------------------------------------------------------------------
...
//
//			We then flood outward from that leaf to see if the
// entity leaks.
// Input  : headnode -
//			origin -
//			occupant -
...
bool FloodEntities(tree_t *tree) {
  ...
  for (i = 1; i < num_entities; i++) {
    ...
    gotit:;
      ...
      if (PlaceOccupant(headnode, origin, &entities[i])) inside = true;
    }
  }
...
void SetAreaPortalAreas_r(tree_t *tree, node_t *node) {
  ...
  if (IsAreaportalNode(node)) {
    if (node->area) return;  // already set
...
float AngleOffset(float flBaseAngle, float flTestAngle) {
  while (flTestAngle > flBaseAngle) flTestAngle -= 2 * M_PI;

  return fmod(flBaseAngle - flTestAngle, (float)(2 * M_PI));
}

int FindUniquePoints(const Vector2D *pPoints, int nPoints, int *indexMap,
                     int nMaxIndexMapPoints, float flTolerance) {
  ...
  for (int i = 0; i < nPoints; i++) {
    ...
    for (j = 0; j < nUniquePoints; j++) {
      if (pPoints[i].DistToSqr(pPoints[indexMap[j]]) < flToleranceSqr) break;
    }
    if (j == nUniquePoints) {
      if (nUniquePoints >= nMaxIndexMapPoints)
        Error("FindUniquePoints: overflowed unique point list (size %d).",
              nMaxIndexMapPoints);
...
int Convex2D(Vector2D const *pPoints, int nPoints, int *indices,
             int nMaxIndices) {
  int nIndices = 0;
  bool touched[512];
  int indexMap[512];
  ...
  for (i = 1; i < nPoints; i++) {
    if (pPoints[indexMap[i]].x < pPoints[indexMap[iBest]].x ||
        (pPoints[indexMap[i]].x == pPoints[indexMap[iBest]].x &&
         pPoints[indexMap[i]].y < pPoints[indexMap[iBest]].y)) {
      iBest = i;
    }
  }
  ...
  while (1) {
    ...
    if (iMinAngle == -1) {
      ...
      for (int jj = 0; jj < nIndices; jj++) {
        // if this assert hits, this routine is broken and is generating a
        // spiral rather than a closed polygon - basically an edge overlap of
        // some kind
...
void FindPortalsLeadingToArea_R(node_t *pHeadNode, int iSrcArea, int iDestArea,
                                plane_t *pPlane,
                                CUtlVector<portal_t *> &portals) {
  if (pHeadNode->planenum != PLANENUM_LEAF) {
    FindPortalsLeadingToArea_R(pHeadNode->children[0], iSrcArea, iDestArea,
                               pPlane, portals);
    FindPortalsLeadingToArea_R(pHeadNode->children[1], iSrcArea, iDestArea,
                               pPlane, portals);
    return;
  }
...
void EmitClipPortalGeometry(node_t *pHeadNode, portal_t *pPortal, int iSrcArea,
                            dareaportal_t *dp) {
  // Build a list of all the points in portals from the same original face.
  CUtlVector<portal_t *> portals;
  FindPortalsLeadingToArea_R(pHeadNode, iSrcArea, dp->otherarea,
                             &pPortal->plane, portals);
  ...
  if (dp->m_FirstClipPortalVert + dp->m_nClipPortalVerts >=
      MAX_MAP_PORTALVERTS) {
    Vector *p = pPortal->winding->p;
    Error(
        "MAX_MAP_PORTALVERTS (probably a broken areaportal near %.1f %.1f "
        "%.1f ",
        p->x, p->y, p->z);
  }
...
// Sets node_t::area for non-leaf nodes (this allows an optimization in the
// renderer).
...
void EmitAreaPortals(node_t *headnode) {
  ...
  if (c_areas > MAX_MAP_AREAS)
    Error(
        "Map is split into too many unique areas (max = %d)\nProbably too many "
        "areaportals",
        MAX_MAP_AREAS);
  numareas = c_areas + 1;
  ...
  for (int iSrcArea = 1; iSrcArea <= c_areas; iSrcArea++) {
    ...
    for (int j = 0; j < num_entities; j++) {
      ...
      if (e->portalareas[0] == iSrcArea || e->portalareas[1] == iSrcArea) {
        ...
        // One of the portals that leads into the CONTENTS_AREAPORTAL just
        // bounds the same two areas but the other bounds two different ones.
        ...
        if (pLeadingPortal) {
          Assert(pLeadingPortal->nodes[0]->area !=
                 pLeadingPortal->nodes[1]->area);
    ...
    dareas[iSrcArea].numareaportals =
        numareaportals - dareas[iSrcArea].firstareaportal;
  }
...
static float ComputeDistFromPlane(winding_t *pWinding, plane_t *pPlane,
                                  float maxdist) {
  float totaldist = 0.0f;
  for (int i = 0; i < pWinding->numpoints; ++i) {
    totaldist +=
        fabs(DotProduct(pPlane->normal, pWinding->p[i]) - pPlane->dist);
    if (totaldist > maxdist) return totaldist;
  }
  return totaldist;
}
...
static void DisplayPortalError(portal_t *p, int viscontents) {
  ...
  PrintBrushContentsToString(p->nodes[0]->contents, contents[0],
                             sizeof(contents[0]));
  PrintBrushContentsToString(p->nodes[1]->contents, contents[1],
                             sizeof(contents[1]));
  ...
  Warning(
      "\nFindPortalSide: Couldn't find a good match for which brush to assign "
      "to a portal near (%.1f %.1f %.1f)\n",
      center.x, center.y, center.z);
  Warning("Leaf 0 contents: %s\n", contents[0]);
  Warning("Leaf 1 contents: %s\n", contents[1]);
  Warning("viscontents (node 0 contents ^ node 1 contents): %s\n", contents[2]);
  Warning(
      "This means that none of the brushes in leaf 0 or 1 that touches the "
      "portal has %s\n",
      contents[2]);
  Warning(
      "Check for a huge brush enclosing the coordinates above that has "
      "contents %s\n",
      contents[2]);
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (bb = n->brushlist; bb; bb = bb->next) {
      ...
      for (i = 0; i < brush->numsides; i++) {
        ...
        if (side->texinfo == TEXINFO_NODE) continue;  // non-visible
...
gotit:
  if (!bestside) qprintf("WARNING: side not found for portal\n");
...
void MarkVisibleSides_r(node_t *node) {
  ...
  for (p = node->portals; p; p = p->next[!s]) {
    s = (p->nodes[0] == node);
    if (!p->onnode) continue;  // edge of world
    if (!p->sidefound) FindPortalSide(p);
    if (p->side) p->side->visible = true;
  }
}
  ...
  for (i = startbrush; i < endbrush; i++) {
    ...
    if (detailScreen != FULL_DETAIL) {
      ...
      if (onlyDetail ^ detail) {
        // both of these must have the same value or we're not interested in
        // this brush
        continue;
      }
    }
    ...
    for (j = 0; j < numsides; j++) mb->original_sides[j].visible = false;
  }
