...
/*
        ...
        While loading the map, it calls:
                *** Cubemap_SaveBrushSides
                        Builds a list of what cubemaps manually were assigned to
   what faces in s_EnvCubemapToBrushSides.

        Immediately after loading the map, it calls:
                *** Cubemap_FixupBrushSidesMaterials
                        Goes through s_EnvCubemapToBrushSides and does
   Cubemap_CreateTexInfo for each side referenced by an env_cubemap manually.

        Then it calls Cubemap_AttachDefaultCubemapToSpecularSides:
                *** Cubemap_InitCubemapSideData:
                        Setup s_aCubemapSideData.bHasEnvMapInMaterial and
   bManuallyPickedByAnEnvCubemap for each side. bHasEnvMapInMaterial is set if
   the side's material has $envmap. bManuallyPickedByAnEnvCubemap is true if the
   side was in s_EnvCubemapToBrushSides.

                Then, for each bHasEnvMapInMaterial and
   !bManuallyPickedByAnEnvCubemap (ie: every specular surface that wasn't
...
static bool CubemapLessFunc(const CubemapInfo_t &lhs,
                            const CubemapInfo_t &rhs) {
...
inline bool SideHasCubemapAndWasntManuallyReferenced(int iSide) {
  return s_aCubemapSideData[iSide].bHasEnvMapInMaterial &&
         !s_aCubemapSideData[iSide].bManuallyPickedByAnEnvCubemap;
}
...
// This is the list of materialvars which are used in our codebase to look up
// dependent materials
...
static const char *FindDependentMaterial(const char *pMaterialName,
  ...
  for (int i = 0; s_pDependentMaterialVar[i][0]; ++i) {
    if (!GetValueFromMaterial(pMaterialName, s_pDependentMaterialVar[i],
                              pDependentMaterialName, MAX_MATERIAL_NAME - 1))
      continue;

    if (!Q_stricmp(pDependentMaterialName, pMaterialName)) {
      Warning(
          "Material %s is depending on itself through materialvar %s! "
          "Ignoring...\n",
    ...
    while (s_pDependentMaterialVar[i][0]) {
      Assert(!GetValueFromMaterial(pMaterialName, s_pDependentMaterialVar[i],
                                   pDependentMaterialName2,
                                   MAX_MATERIAL_NAME - 1));
      ++i;
    }
#endif
...
static bool LoadSrcVTFFiles(IVTFTexture *pSrcVTFTextures[6],
                            const char *pSkyboxMaterialBaseName,
  ...
  for (i = 0; i < 6; i++) {
    ...
    // IMaterialVar *pSkyTextureVar = pSkyboxMaterial->FindVar( bHDR ?
    // "$hdrbasetexture" : "$basetexture", NULL ); //, bHDR ? false : true );
    IMaterialVar *pSkyTextureVar = pSkyboxMaterial->FindVar(
    ...
    *pUnionTextureFlags |= pSrcVTFTextures[i]->Flags();
    int flagsNoAlpha = pSrcVTFTextures[i]->Flags() &
                       ~(TEXTUREFLAGS_EIGHTBITALPHA | TEXTUREFLAGS_ONEBITALPHA);
    int flagsFirstNoAlpha =
        pSrcVTFTextures[0]->Flags() &
        ~(TEXTUREFLAGS_EIGHTBITALPHA | TEXTUREFLAGS_ONEBITALPHA);
...
void VTFNameToHDRVTFName(const char *pSrcName, char *pDest, int maxLen,
                         bool bHDR) {
...
void CreateDefaultCubemaps(bool bHDR) {
  ...
  if (!skyboxMaterialName) {
    if (s_DefaultCubemapNames.Count()) {
      Warning(
          "This map uses env_cubemap, and you don't have a skybox, so no "
          "default env_cubemaps will be generated.\n");
    }
    return;
  }
  ...
  pDstCubemap->Init(DEFAULT_CUBEMAP_SIZE, DEFAULT_CUBEMAP_SIZE, 1,
                    ...
                    unionTextureFlags | TEXTUREFLAGS_ENVMAP,
  ...
  AddBufferToPak(pak, dstVTFFileName, outputBuf.Base(), outputBuf.TellPut(),
                 false);
...
//-----------------------------------------------------------------------------
static void GeneratePatchedName(const char *pMaterialName,
                                const PatchInfo_t &info, bool bMaterialName,
                                char *pBuffer, int nMaxLen) {
  const char *pSeparator = bMaterialName ? "_" : "";
  int nLen = Q_snprintf(
      pBuffer, nMaxLen, "maps/%s/%s%s%d_%d_%d", info.m_pMapName, pMaterialName,
      pSeparator, info.m_pOrigin[0], info.m_pOrigin[1], info.m_pOrigin[2]);
...
//-----------------------------------------------------------------------------
// Patches the $envmap for a material and all its dependents, returns true if
// any patching happened
//-----------------------------------------------------------------------------
static bool PatchEnvmapForMaterialAndDependents(const char *pMaterialName,
  ...
  bool bShouldPatchEnvCubemap =
      DoesMaterialHaveKeyValuePair(pMaterialName, "$envmap", "env_cubemap");

  // See if we have a dependent material to patch
  bool bDependentMaterialPatched = false;
  const char *pDependentMaterialVar = NULL;
  const char *pDependentMaterial =
      FindDependentMaterial(pMaterialName, &pDependentMaterialVar);
  if (pDependentMaterial) {
    bDependentMaterialPatched = PatchEnvmapForMaterialAndDependents(
        pDependentMaterial, info, pCubemapTexture);
  }
  ...
  CreateMaterialPatch(pMaterialName, pPatchedMaterialName, nPatchCount,
                      pPatchInfo, PATCH_REPLACE);
...
//-----------------------------------------------------------------------------
// Create a VMT to override the specified texinfo which references the cubemap
// entity at the specified origin. Returns the index of the new (or preexisting)
// texinfo referencing that VMT.
//
// Also adds the new cubemap VTF filename to s_DefaultCubemapNames so it can
// copy the default (skybox) cubemap into this file so the cubemap doesn't have
// the pink checkerboard at runtime before they run buildcubemaps.
//-----------------------------------------------------------------------------
static int Cubemap_CreateTexInfo(int originalTexInfo, int origin[3]) {
  ...
  // Get out of here if the originalTexInfo is already a generated material for
  // this position.
  char pStringToSearchFor[512];
  Q_snprintf(pStringToSearchFor, 512, "_%d_%d_%d", origin[0], origin[1],
             origin[2]);
  if (Q_stristr(pMaterialName, pStringToSearchFor)) return originalTexInfo;
...
void Cubemap_FixupBrushSidesMaterials(void) {
  ...
  for (cubemapID = 0; cubemapID < g_nCubemapSamples; cubemapID++) {
    ...
    for (i = 0; i < brushSidesVector.Count(); i++) {
      ...
      pSide->texinfo = Cubemap_CreateTexInfo(
          pSide->texinfo, g_CubemapSamples[cubemapID].origin);
      if (pSide->pMapDisp) {
...
bool DoesMaterialOrDependentsUseEnvmap(const char *pPatchedMaterialName) {
  const char *pOriginalMaterialName =
      GetOriginalMaterialNameForPatchedMaterial(pPatchedMaterialName);
  if (DoesMaterialHaveKey(pOriginalMaterialName, "$envmap")) return true;

  const char *pDependentMaterial = FindDependentMaterial(pOriginalMaterialName);
  if (!pDependentMaterial) return false;
...
void Cubemap_InitCubemapSideData(void) {
  ...
  for (int iSide = 0; iSide < nummapbrushsides; ++iSide) {
    ...
    if (!pTexData) continue;

    CubemapInfo_t info;
    info.m_nTableId = pTexData->nameStringTableID;

    // Have we encountered this materal? If so, then copy the data we cached off
    // before
...
//-----------------------------------------------------------------------------
// For every specular surface that wasn't referenced by some env_cubemap, call
// Cubemap_CreateTexInfo.
//-----------------------------------------------------------------------------
void Cubemap_AttachDefaultCubemapToSpecularSides(void) {
  ...
  for (int iSide = 0; iSide < nummapbrushsides; ++iSide) {
    side_t *pSide = &brushsides[iSide];
    if (!SideHasCubemapAndWasntManuallyReferenced(iSide)) continue;

    int currentEntity = sideToEntityIndex[iSide];

    int iCubemap =
        Cubemap_FindClosestCubemap(entities[currentEntity].origin, pSide);
    if (iCubemap == -1) continue;
...
#endif
    pSide->texinfo = Cubemap_CreateTexInfo(pSide->texinfo,
                                           g_CubemapSamples[iCubemap].origin);
    if (pSide->pMapDisp) {
      pSide->pMapDisp->face.texinfo = pSide->texinfo;
    }
  }
