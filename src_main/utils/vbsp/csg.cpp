...
===============
*/
bspbrush_t *ClipBrushToBox(bspbrush_t *brush, const Vector &clipmins,
...
//-----------------------------------------------------------------------------
static bspbrush_t *CreateClippedBrush(mapbrush_t *mb, const Vector &clipmins,
                                      const Vector &clipmaxs) {
  int nNumSides = mb->numsides;
  if (!nNumSides) return NULL;
  ...
  for (int j = 0; j < nNumSides; j++) {
    ...
    // keep a pointer to the original map brush side -- use to create the
    // original face later!!
...
// Creates a clipped brush from a map brush
//-----------------------------------------------------------------------------
static void ComputeBoundingPlanes(const Vector &clipmins,
                                  const Vector &clipmaxs) {
  Vector normal;
  float dist;
  for (int i = 0; i < 2; i++) {
...
//-----------------------------------------------------------------------------
void CopyMatchingTexinfos(side_t *pDestSides, int numDestSides,
                          const bspbrush_t *pSource) {
  for (int i = 0; i < numDestSides; i++) {
    side_t *pSide = &pDestSides[i];
    plane_t *pPlane = &mapplanes[pSide->planenum];

    // We have to use the *original sides* because MapBSPBrushList could have
    // generated splits when cutting the original brush to the block being
    // processed. This will generate faces that use TEXINFO_NODE, which is
    // definitely *not* what we want. If we end up with faces using TEXINFO_NODE
    // here, the area portal will flood into the entire water volume
    // intersecting the areaportal.
    ...
    for (int j = 0; j < pSourceBrush->numsides; ++j, ++pSourceSide) {
      if (pSourceSide->texinfo == TEXINFO_NODE) continue;
...
// resulting space has the same properties as the water (normal areaportals
// assume "empty" surroundings)
void FixupAreaportalWaterBrushes(bspbrush_t *pList) {
  for (bspbrush_t *pAreaportal = pList; pAreaportal;
       pAreaportal = pAreaportal->next) {
    if (!(pAreaportal->original->contents & CONTENTS_AREAPORTAL)) continue;

    for (bspbrush_t *pWater = pList; pWater; pWater = pWater->next) {
      // avoid using areaportal/water combo brushes that have already been fixed
      // up
      if (pWater->original->contents & CONTENTS_AREAPORTAL) continue;

      if (!(pWater->original->contents & MASK_SPLITAREAPORTAL)) continue;

      if (BrushesDisjoint(pAreaportal, pWater)) continue;

      bspbrush_t *pIntersect = IntersectBrush(pAreaportal, pWater);
      if (!pIntersect) continue;
      ...
      // created from the map brush.  But since it hasn't been, retexture the
      // original map brush's sides
      CopyMatchingTexinfos(pAreaportal->sides, pAreaportal->numsides, pWater);
      CopyMatchingTexinfos(pAreaportal->original->original_sides,
                           pAreaportal->original->numsides, pWater);
    }
  }
}
...
// MakeBspBrushList
//-----------------------------------------------------------------------------
// UNDONE: Put detail brushes in a separate brush array and pass that instead of
// "onlyDetail" ?
...
bool BrushGE(bspbrush_t *b1, bspbrush_t *b2) {
  ...
  if ((b1->original->contents & CONTENTS_DETAIL) &&
    ...
    return false;
  if (b1->original->contents & CONTENTS_SOLID) return true;
  // Transparent brushes are not marked as detail anymore, so let them cut each
  // other.
  if ((b1->original->contents & TRANSPARENT_CONTENTS) &&
      (b2->original->contents & TRANSPARENT_CONTENTS))
    return true;
...
#if DEBUG_BRUSHMODEL
  if (entity_num == DEBUG_BRUSHMODEL) WriteBrushList("before.gl", head, false);
#endif
  keep = NULL;
  ...
  for (b1 = head; b1; b1 = next) {
    ...
    for (b2 = b1->next; b2; b2 = b2->next) {
      ...
      if (BrushGE(b2, b1)) {
        ...
        if (sub == b1) continue;  // didn't really intersect
      ...
      if (BrushGE(b1, b2)) {
        ...
        if (sub2 == b2) continue;  // didn't really intersect
      ...
      if (!sub && !sub2) continue;  // neither one can bite
      ...
      if (c1 > 1 && c2 > 1) {
        ...
        if (!((contents1 & contents2) & CONTENTS_DETAIL) &&
            !((contents1 | contents2) & CONTENTS_AREAPORTAL)) {
