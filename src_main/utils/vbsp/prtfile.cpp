  ...
  for (int clusterIndex = 0; clusterIndex < list.Count(); clusterIndex++) {
    for (int j = 0; j < list[clusterIndex].portals.Count(); j++) {
      ...
      if (DotProduct(p->plane.normal, normal) < 0.99) {  // backwards...
        ...
        fprintf(pFile, "%i %i %i ", w->numpoints, p->nodes[0]->cluster,
                p->nodes[1]->cluster);
      }
...
void AddVisCluster(entity_t *pFuncVisCluster) {
  ...
  bspbrush_t *pBSPBrush = MakeBspBrushList(
      pFuncVisCluster->firstbrush,
      pFuncVisCluster->firstbrush + pFuncVisCluster->numbrushes, clipMins,
      clipMaxs, NO_DETAIL);
...
// returns the total overlapping volume of intersection between the node and the
// brush list
float VolumeOfIntersection(bspbrush_t *pBrushList, node_t *pNode) {
  float volume = 0.0f;
  for (bspbrush_t *pBrush = pBrushList; pBrush; pBrush = pBrush->next) {
    if (IsBoxIntersectingBox(pNode->mins, pNode->maxs, pBrush->mins,
                             pBrush->maxs)) {
...
int GetVisCluster(node_t *pNode) {
  float maxVolume =
      BrushVolume(pNode->volume) *
      0.10f;  // needs to cover at least 10% of the volume to overlap
  int maxIndex = -1;
...
void NumberLeafs(const CUtlVector<node_t *> &leaves) {
  ...
  for (int i = 0; i < g_VisClusters.Count(); i++) {
    ...
    Msg("Writing %s\n", name);
    for (bspbrush_t *pBrush = g_VisClusters[i].pBrushes; pBrush;
         pBrush = pBrush->next) {
    ...
    for (int j = 0; j < leaves.Count(); j++) {
      if (leaves[j]->cluster == g_VisClusters[i].clusterIndex) {
        ...
        for (int k = 0; k < pBrush->numsides; k++)
          OutputWindingColor(pBrush->sides[k].winding, fp, 64 + (j & 31), 64,
                             64 - (j & 31));
      }
    }
...
int BuildPortalList(CUtlVector<cluster_portals_t> &portalList,
                    const CUtlVector<node_t *> &leaves) {
  int portalCount = 0;
  for (int i = 0; i < leaves.Count(); i++) {
    node_t *node = leaves[i];
    // count the portals
