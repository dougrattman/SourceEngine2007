...
static bool StaticLess(StaticPropLookup_t const& src1,
                       StaticPropLookup_t const& src2) {
  return src1.m_ModelName < src2.m_ModelName;
}

static CUtlRBTree<StaticPropLookup_t, unsigned short> s_StaticPropLookup(
    0, 32, StaticLess);
...
static void ParseDetailGroup(int detailId, KeyValues* pGroupKeyValues) {
  ...
  while (--i >= 0) {
    if (alpha > s_DetailObjectDict[detailId].m_Groups[i].m_Alpha) break;
  }
...
static int SelectGroup(const DetailObject_t& detail, float alpha) {
  ...
  for (start = 0; start < detail.m_Groups.Count() - 1; ++start) {
    if (alpha < detail.m_Groups[start + 1].m_Alpha) break;
  }

  end = start + 1;
  if (end >= detail.m_Groups.Count()) --end;
  ...
  float dist = 0.0f;
  float dAlpha =
      (detail.m_Groups[end].m_Alpha - detail.m_Groups[start].m_Alpha);
  if (dAlpha != 0.0f) {
...
static int SelectDetail(DetailObjectGroup_t const& group) {
  ...
  for (int i = 0; i < group.m_Models.Count(); ++i) {
    if (r <= group.m_Models[i].m_Amount) return i;
  }
...
static void AddDetailToLump(const char* pModelName, const Vector& pt,
                            const QAngle& angles, int nOrientation) {
...
//-----------------------------------------------------------------------------
static void AddDetailSpriteToLump(const Vector& vecOrigin,
                                  const QAngle& vecAngles, int nOrientation,
                                  const Vector2D* pPos, const Vector2D* pTex,
                                  float flScale, int iType, int iShapeAngle = 0,
                                  int iShapeSize = 0, int iSwayAmount = 0) {
  // Insert an element into the object dictionary if it aint there...
  int i = s_DetailObjectLump.AddToTail();
...
static void AddDetailSpriteToLump(const Vector& vecOrigin,
                                  ...
                                  DetailModel_t const& model, float flScale) {
  AddDetailSpriteToLump(vecOrigin, vecAngles, model.m_Orientation, model.m_Pos,
                        model.m_Tex, flScale, model.m_Type, model.m_ShapeAngle,
                        model.m_ShapeSize, model.m_SwayAmount);
}

//-----------------------------------------------------------------------------
// Got a detail! Place it on the surface...
//-----------------------------------------------------------------------------
// BUGBUG: When the global optimizer is on, "normal" gets trashed in this
// function (only when not in the debugger?) Printing the values of normal at
// the bottom of the function fixes it as does disabling global optimizations.
static void PlaceDetail(DetailModel_t const& model, const Vector& pt,
                        const Vector& normal) {
  ...
  if (model.m_Flags & MODELFLAG_UPRIGHT) {
    ...
    Vector xaxis(1, 0, 0);
    if (fabs(xaxis.Dot(zaxis)) - 1.0 > -1e-3) xaxis.Init(0, 1, 0);
    Vector yaxis;
  ...
  switch (model.m_Type) {
    case DETAIL_PROP_TYPE_MODEL:
      AddDetailToLump(model.m_ModelName.String(), pt, angles,
                      model.m_Orientation);
      break;
...
static void EmitDetailObjectsOnFace(dface_t* pFace, DetailObject_t& detail) {
  ...
  for (int i = 1; i < pFace->numedges - 1; ++i) {
    ...
    Vector e1, e2;
    VectorSubtract(dvertexes[pEdge->v[vertexIdx]].point, pFirstVertex->point,
                   e1);
    VectorSubtract(dvertexes[pEdge->v[1 - vertexIdx]].point,
                   pFirstVertex->point, e2);
...
static int SortFunc(const void* arg1, const void* arg2) {
  int nDelta =
      ((DetailObjectLump_t*)arg1)->m_Leaf - ((DetailObjectLump_t*)arg2)->m_Leaf;
...
static void SetLumpData() {
  ...
  qsort(s_DetailObjectLump.Base(), s_DetailObjectLump.Count(),
        sizeof(DetailObjectLump_t), SortFunc);
  ...
  int nDictSize =
      s_DetailObjectDictLump.Count() * sizeof(DetailObjectDictLump_t);
  int nSpriteDictSize =
      s_DetailSpriteDictLump.Count() * sizeof(DetailSpriteDictLump_t);
  int nObjSize = s_DetailObjectLump.Count() * sizeof(DetailObjectLump_t);
  int nSize = nDictSize + nSpriteDictSize + nObjSize + (3 * sizeof(int));

  handle = g_GameLumps.CreateGameLump(GAMELUMP_DETAIL_PROPS, nSize, 0,
                                      GAMELUMP_DETAIL_PROPS_VERSION);
...
void EmitDetailModels() {
  ...
  for (int j = 0; j < numfaces; ++j) {
    ...
    bool found;
    MaterialSystemMaterial_t handle = FindOriginalMaterial(
        TexDataStringTable_GetString(pTexData->nameStringTableID), &found,
        false);
    if (!found) continue;

    // See if its got any detail objects on it
    const char* pDetailType = GetMaterialVar(handle, "%detailtype");
    if (!pDetailType) continue;
    ...
    // Initialize the Random Number generators for detail prop placement based
    // on the hammer Face num.
