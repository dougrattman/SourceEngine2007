...
void OutputWindingColor(winding_t *w, FileHandle_t glview, int r, int g,
                        int b) {
  int i;
...
void WriteGLView(tree_t *tree, char *source) {
  ...
  c_glfaces = 0;
  sprintf_s(name, "%s%s.gl", outbase, source);
  Msg("Writing %s\n", name);

  glview = g_pFileSystem->Open(name, "w");
  if (!glview) Error("Couldn't open %s", name);
...
void WriteGLViewFaces(tree_t *tree, const char *pName) {
  ...
  c_glfaces = 0;
  sprintf_s(name, "%s%s.gl", outbase, pName);
  Msg("Writing %s\n", name);
