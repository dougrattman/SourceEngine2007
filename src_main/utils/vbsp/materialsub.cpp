...
// When the bsp is compiled, all materials listed in the file
// will be replaced by the second material in the pair.
...
void LoadMaterialReplacementKeys(const char *gamedir, const char *mapname) {
  ...
  if (allMapKeys) allMapKeys = 0;
  if (curMapKeys) curMapKeys = 0;
...
//-----------------------------------------------------------------------------
// Purpose: Replace the passed-in material name with a replacement name, if one
// exists
//-----------------------------------------------------------------------------
