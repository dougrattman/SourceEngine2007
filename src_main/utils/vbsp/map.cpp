...
#define BRUSH_CLIP_EPSILON \
  0.01f  // this should probably be the same
...
ChunkFileResult_t LoadDispDistancesCallback(CChunkFile *pFile,
                                            mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispDistancesKeyCallback(const char *szKey,
                                               const char *szValue,
                                               mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispInfoCallback(CChunkFile *pFile,
                                       mapdispinfo_t **ppMapDispInfo);
ChunkFileResult_t LoadDispInfoKeyCallback(const char *szKey,
                                          const char *szValue,
                                          mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispNormalsCallback(CChunkFile *pFile,
                                          mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispNormalsKeyCallback(const char *szKey,
                                             const char *szValue,
                                             mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispOffsetsCallback(CChunkFile *pFile,
                                          mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispOffsetsKeyCallback(const char *szKey,
                                             const char *szValue,
                                             mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispAlphasCallback(CChunkFile *pFile,
                                         mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispAlphasKeyCallback(const char *szKey,
                                            const char *szValue,
                                            mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispTriangleTagsCallback(CChunkFile *pFile,
                                               mapdispinfo_t *pMapDispInfo);
...
#ifdef VSVMFIO
ChunkFileResult_t LoadDispOffsetNormalsCallback(CChunkFile *pFile,
                                                mapdispinfo_t *pMapDispInfo);
ChunkFileResult_t LoadDispOffsetNormalsKeyCallback(const char *szKey,
                                                   const char *szValue,
                                                   mapdispinfo_t *pMapDispInfo);
#endif  // VSVMFIO
...
ChunkFileResult_t LoadConnectionsCallback(CChunkFile *pFile,
                                          LoadEntity_t *pLoadEntity);
...
//			Rounds dist to integer if it is within an epsilon of
// integer.
...
//-----------------------------------------------------------------------------
// Purpose: Snaps normal to axis-aligned if it is within an epsilon of axial.
//			Recalculates dist if the normal was snapped. Rounds dist
// to integer 			if it is within an epsilon of integer.
// Input  : normal - Plane normal vector (assumed to be unit length).
//			dist - Plane constant.
...
int FindFloatPlane(Vector &normal, f32 dist) {
  ...
  for (i = -1; i <= 1; i++) {
    ...
    for (p = planehash[h]; p; p = p->hash_chain) {
      if (PlaneEqual(p, normal, dist, RENDER_NORMAL_EPSILON,
                     RENDER_DIST_EPSILON))
        return p - mapplanes;
    }
  }
...
//			If the normal is nearly axial, it will be snapped to be
// axial. Looks 			up the plane in the unique planes.
...
int BrushContents(mapbrush_t *b) {
  ...
  // NOTE: we're making slime translucent	so that it doesn't block
  // lighting on things floating on its surface
  int transparentContents = unionContents & (CONTENTS_WINDOW | CONTENTS_GRATE |
...
bool IsAreaPortal(char const *pClassName) {
  // If the class name starts with "func_areaportal", then it's considered an
  // area portal.
  char const *pBaseName = "func_areaportal";
  char const *pCur = pBaseName;
  while (*pCur && *pClassName) {
    if (*pCur != *pClassName) break;
      ...
      for (axis = 0; axis < 3; axis++) {
        for (dir = -1; dir <= 1; dir += 2) {
          ...
          for (k = 0; k < b->numsides; k++) {
            ...
            // NOTE: Use a larger tolerance for collision planes than for
            // rendering planes
...
bool MakeBrushWindings(mapbrush_t *ob) {
  ...
  for (i = 0; i < ob->numsides; i++) {
    ...
    for (j = 0; j < ob->numsides && w; j++) {
      ...
      //			ChopWindingInPlace (&w, plane->normal,
      // plane->dist, 0); //CLIP_EPSILON);
...
//-----------------------------------------------------------------------------
// Purpose: Takes all of the brushes from the current entity and adds them to
// the
//			world's brush list. Used by func_detail and
// func_areaportal.
...
void MoveBrushesToWorld(entity_t *mapent) {
  ...
  for (i = 1; i < num_entities; i++) entities[i].firstbrush += newbrushes;
  free(temp);
...
//-----------------------------------------------------------------------------
// Purpose: Iterates the sides of brush and removed CONTENTS_DETAIL from each
// side Input  : *brush -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Iterates all brushes in an entity and removes CONTENTS_DETAIL from
// all brushes Input  : *mapent -
//-----------------------------------------------------------------------------
...
ChunkFileResult_t LoadDispDistancesCallback(CChunkFile *pFile,
                                            mapdispinfo_t *pMapDispInfo) {
  return (pFile->ReadChunk((KeyHandler_t)LoadDispDistancesKeyCallback,
                           pMapDispInfo));
}
...
ChunkFileResult_t LoadDispInfoCallback(CChunkFile *pFile,
  ...
  Handlers.AddHandler("normals", (ChunkHandler_t)LoadDispNormalsCallback,
                      pMapDispInfo);
  Handlers.AddHandler("distances", (ChunkHandler_t)LoadDispDistancesCallback,
                      pMapDispInfo);
  Handlers.AddHandler("offsets", (ChunkHandler_t)LoadDispOffsetsCallback,
                      pMapDispInfo);
  Handlers.AddHandler("alphas", (ChunkHandler_t)LoadDispAlphasCallback,
                      pMapDispInfo);
  Handlers.AddHandler("triangle_tags",
                      (ChunkHandler_t)LoadDispTriangleTagsCallback,
                      pMapDispInfo);

#ifdef VSVMFIO
  Handlers.AddHandler("offset_normals",
                      (ChunkHandler_t)LoadDispOffsetNormalsCallback,
                      pMapDispInfo);
#endif  // VSVMFIO
  ...
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadDispInfoKeyCallback, pMapDispInfo);
  pFile->PopHandlers();
...
ChunkFileResult_t LoadDispNormalsCallback(CChunkFile *pFile,
                                          mapdispinfo_t *pMapDispInfo) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadDispNormalsKeyCallback, pMapDispInfo));
}
...
ChunkFileResult_t LoadDispOffsetsCallback(CChunkFile *pFile,
                                          mapdispinfo_t *pMapDispInfo) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadDispOffsetsKeyCallback, pMapDispInfo));
}
...
ChunkFileResult_t LoadDispOffsetNormalsCallback(CChunkFile *pFile,
                                                mapdispinfo_t *pMapDispInfo) {
  return (pFile->ReadChunk((KeyHandler_t)LoadDispOffsetNormalsKeyCallback,
                           pMapDispInfo));
}
...
ChunkFileResult_t LoadDispAlphasCallback(CChunkFile *pFile,
                                         mapdispinfo_t *pMapDispInfo) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadDispAlphasKeyCallback, pMapDispInfo));
}
...
ChunkFileResult_t LoadDispTriangleTagsCallback(CChunkFile *pFile,
                                               mapdispinfo_t *pMapDispInfo) {
  return (pFile->ReadChunk((KeyHandler_t)LoadDispTriangleTagsKeyCallback,
                           pMapDispInfo));
}
...
// Add all the sides referenced by info_no_dynamic_shadows entities to
// g_NoDynamicShadowSides.
...
static ChunkFileResult_t LoadOverlayDataTransitionKeyCallback(
  ...
  if (!_stricmp(szKey, "material")) {
    ...
    if (strlen(pMaterialName) >= OVERLAY_MAP_STRLEN) {
      Error("Overlay Material Name (%s) > OVERLAY_MAP_STRLEN (%d)",
...
static ChunkFileResult_t LoadOverlayDataTransitionCallback(CChunkFile *pFile,
                                                           int nParam) {
  int iOverlay = g_aMapWaterOverlays.AddToTail();
  mapoverlay_t *pOverlay = &g_aMapWaterOverlays[iOverlay];
  if (!pOverlay) return ChunkFile_Fail;
  ...
  ChunkFileResult_t eResult = pFile->ReadChunk(
      (KeyHandler_t)LoadOverlayDataTransitionKeyCallback, pOverlay);
  return eResult;
}

static ChunkFileResult_t LoadOverlayTransitionCallback(CChunkFile *pFile,
                                                       int nParam) {
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("overlaydata",
...
//-----------------------------------------------------------------------------
// Purpose: Iterates all brushes in a ladder entity, generates its mins and
// maxs.
//          These are stored in the object, since the brushes are going to go
//          away.
// Input  : *mapent -
//-----------------------------------------------------------------------------
...
ChunkFileResult_t LoadEntityCallback(CChunkFile *pFile, int nParam) {
  ...
  Handlers.AddHandler("connections", (ChunkHandler_t)LoadConnectionsCallback,
                      &LoadEntity);
  Handlers.AddHandler("overlaytransition",
                      (ChunkHandler_t)LoadOverlayTransitionCallback, 0);
  ...
  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadEntityKeyCallback, &LoadEntity);
  pFile->PopHandlers();
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    if (mapent->origin[0] || mapent->origin[1] || mapent->origin[2]) {
      for (int i = 0; i < mapent->numbrushes; i++) {
        ...
        for (int j = 0; j < b->numsides; j++) {
          ...
          if (!onlyents) {
            s->texinfo = TexinfoForBrushTexture(
                &mapplanes[s->planenum], &side_brushtextures[s - brushsides],
                mapent->origin);
          }
        }
        MakeBrushWindings(b);
    ...
    //
    // func_detail brushes are moved into the world entity. The CONTENTS_DETAIL
    // flag was set by the loader.
    //
    ...
    //
    // func_ladder brushes are moved into the world entity.  We convert the
    // func_ladder to an info_ladder that holds the ladder's mins and maxs, and
    // leave the entity.  This helps the bots figure out ladders.
    //
    ...
    if (!Q_stricmp(pClassName, "light")) {
      CVmfImport::GetVmfImporter()->ImportLightCallback(
          ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"),
          ValueForKey(mapent, "_light"), ValueForKey(mapent, "_lightHDR"),
    ...
    if (!Q_stricmp(pClassName, "light_spot")) {
      CVmfImport::GetVmfImporter()->ImportLightSpotCallback(
          ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"),
          ValueForKey(mapent, "angles"), ValueForKey(mapent, "pitch"),
          ValueForKey(mapent, "_light"), ValueForKey(mapent, "_lightHDR"),
          ValueForKey(mapent, "_lightscaleHDR"),
          ValueForKey(mapent, "_quadratic_attn"),
          ValueForKey(mapent, "_inner_cone"), ValueForKey(mapent, "_cone"),
    ...
    if (!Q_stricmp(pClassName, "light_dynamic")) {
      CVmfImport::GetVmfImporter()->ImportLightDynamicCallback(
          ValueForKey(mapent, "hammerid"), ValueForKey(mapent, "origin"),
          ValueForKey(mapent, "angles"), ValueForKey(mapent, "pitch"),
          ValueForKey(mapent, "_light"), ValueForKey(mapent, "_quadratic_attn"),
          ValueForKey(mapent, "_inner_cone"), ValueForKey(mapent, "_cone"),
          ValueForKey(mapent, "brightness"), ValueForKey(mapent, "distance"),
    ...
    if (!Q_stricmp(pClassName, "light_environment")) {
      CVmfImport::GetVmfImporter()->ImportLightEnvironmentCallback(
          ...
          ValueForKey(mapent, "_light"), ValueForKey(mapent, "_lightHDR"),
          ValueForKey(mapent, "_lightscaleHDR"),
          ValueForKey(mapent, "_ambient"), ValueForKey(mapent, "_ambientHDR"),
    ...
    if (pModel && Q_strlen(pModel)) {
      CVmfImport::GetVmfImporter()->ImportModelCallback(
          pModel, ValueForKey(mapent, "hammerid"),
          ValueForKey(mapent, "angles"), ValueForKey(mapent, "origin"));
    }
#endif  // VSVMFIO

    // If it's not in the world at this point, unmark CONTENTS_DETAIL from all
    // sides...
...
void ForceFuncAreaPortalWindowContents() {
  ...
  for (int i = 0; i < num_entities; i++) {
    ...
    // Don't do this on "normal" func_areaportal entities.  Those are tied to
    // doors and should be opaque when closed.  But areaportal windows (and any
    // other distance-based areaportals) should be windows because they are
    // normally open/transparent
...
//			loader is used, otherwise the file is assumed to be in
// VMF format.
...
void LoadMapFile(const char *pszFileName) {
#ifdef VSVMFIO
  {
    ...
    // This seems to more or less handle it... not sure if this is all of it or
    // not
    entity_num = 0;
...
ChunkFileResult_t LoadSideCallback(CChunkFile *pFile, LoadSide_t *pSideInfo) {
  ...
  //
  CChunkHandlerMap Handlers;
  Handlers.AddHandler("dispinfo", (ChunkHandler_t)LoadDispInfoCallback,
                      &side->pMapDisp);
  ...
  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadSideKeyCallback, pSideInfo);
  pFile->PopHandlers();
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    if (!(side->contents & (ALL_VISIBLE_CONTENTS | CONTENTS_PLAYERCLIP |
      ...
      if (k == b->numsides) {
        ...
        if (CVmfImport::GetVmfImporter()) {
          CVmfImport::GetVmfImporter()->AddSideCallback(
              b, side, pSideInfo->td, pSideInfo->planepts[0],
              pSideInfo->planepts[1], pSideInfo->planepts[2]);
        }
#endif  // VSVMFIO
      }
    } else {
...
ChunkFileResult_t LoadSideKeyCallback(const char *szKey, const char *szValue,
  ...
  if (!_stricmp(szKey, "plane")) {
    ...
    pSideInfo->td.lightmapWorldUnitsPerLuxel =
        textureref[mt].lightmapWorldUnitsPerLuxel;
...
//			pLoadEntity - Structure to receive loaded entity
// information.
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t LoadConnectionsCallback(CChunkFile *pFile,
                                          LoadEntity_t *pLoadEntity) {
  return (
      pFile->ReadChunk((KeyHandler_t)LoadConnectionsKeyCallback, pLoadEntity));
}
...
//			szValue - Comma delimited fields in the following
// format: <target>,<input>,<parameter>,<delay>,<times
// to fire> 			pLoadEntity - Structure to receive loaded entity
// information.
// Output : ChunkFileResult_t
//-----------------------------------------------------------------------------
ChunkFileResult_t LoadConnectionsKeyCallback(const char *szKey,
...
ChunkFileResult_t LoadSolidCallback(CChunkFile *pFile,
  ...
  pFile->PushHandlers(&Handlers);
  ChunkFileResult_t eResult =
      pFile->ReadChunk((KeyHandler_t)LoadSolidKeyCallback, b);
  pFile->PopHandlers();
  ...
  if (eResult == ChunkFile_Ok) {
    ...
    //
    // find a map brushes with displacement surfaces and remove them from the
    // "world"
    //
    if (HasDispInfo(b)) {
...
void TestExpandBrushes(void) {
  ...
  fprintf(
      ...
      "\"mapversion\" \"220\"\n\"sounds\" \"1\"\n\"MaxRange\" "
      "\"4096\"\n\"mapversion\" \"220\"\n\"wad\" "
      "\"vert.wad;dev.wad;generic.wad;spire.wad;urb.wad;cit.wad;water.wad\"\n");
    ...
    for (i = 0; i < brush->numsides; i++) {
      ...
      fprintf(f, "%s [ 0 0 1 -512 ] [ 0 -1 0 -256 ] 0 1 1 \n",
              TexDataStringTable_GetString(
                  GetTexData(texinfo[s->texinfo].texdata)->nameStringTableID));
...
mapdispinfo_t *ParseDispInfoChunk(void) {
  ...
  vertCount =
      (((1 << pMapDispInfo->power) + 1) * ((1 << pMapDispInfo->power) + 1));
