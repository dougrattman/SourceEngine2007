...
int FindMiptex(const char *name) {
  ...
  // HANDLE ALL OF THE STUFF THAT ISN'T RENDERED WITH THE MATERIAL THAT IS ONE
  // IT.
  ...
  // handle nolight surfs (except water)
            ...
            StringIsTrue(propVal)) &&
           !((propVal2 = GetMaterialVar(matID, "%compileWater")) &&
             StringIsTrue(propVal2))) {
    ...
    if ((propVal = GetMaterialVar(matID, "%compilePassBullets")) &&
        StringIsTrue(propVal)) {
    ...
    if (g_BumpAll || GetMaterialShaderPropertyBool(
    ...
    if ((propVal = GetMaterialVar(matID, "%compileNoDraw")) &&
        StringIsTrue(propVal)) {
    ...
    if ((propVal = GetMaterialVar(matID, "%compileInvisible")) &&
        StringIsTrue(propVal)) {
      // change contents to grate, so bullets pass through
      // NOTE: This has effects on visibility too!
      textureref[i].contents &= ~CONTENTS_SOLID;
    ...
    // handle block LOS
    if ((propVal = GetMaterialVar(matID, "%compileBlockLOS")) &&
        StringIsTrue(propVal)) {
    ...
    bool bKeepLighting =
        ((propVal = GetMaterialVar(matID, "%compileKeepLight")) &&
         StringIsTrue(propVal));
    ...
    if ((propVal = GetMaterialVar(matID, "%compileWater")) &&
      ...
      // Set this so that we can check at the end of the process the presence of
      // a a WaterLODControl entity.
      g_bHasWater = true;
    }
    const char *pShaderName = GetMaterialShaderName(matID);
    if (!bKeepLighting && !Q_strncasecmp(pShaderName, "water", 5) ||
        !Q_strncasecmp(pShaderName, "UnlitGeneric", 12)) {
    ...
    if (checkWindow && opacity != UTILMATLIB_OPAQUE) {
      // transparent *and solid* brushes that aren't grates or water must be
      // windows
      ...
      // this affects engine primitive sorting, SURF_TRANS means sort as a
...
int GetSurfaceProperties2(MaterialSystemMaterial_t matID,
                          const char *pMatName) {
  const char *pPropString = NULL;
  int surfaceIndex = -1;
...
int FindAliasedTexData(const char *pName_, dtexdata_t *sourceTexture) {
  ...
  for (i = 0; i < numtexdata; i++) {
    if (!strcmp(pName,
                TexDataStringTable_GetString(GetTexData(i)->nameStringTableID)))
      return i;
  }
  ...
  // Get the width, height, view_width, view_height, and reflectivity from the
  // material system.
  matID = FindOriginalMaterial(
      TexDataStringTable_GetString(sourceTexture->nameStringTableID), &found,
      false);
...
int FindTexData(const char *pName) {
  ...
  for (int i = 0; i < numtexdata; i++) {
    char const *pTexDataName =
        TexDataStringTable_GetString(GetTexData(i)->nameStringTableID);
    if (!Q_stricmp(pTexDataName, pName)) return i;
  }
  return -1;
}
...
int AddCloneTexData(dtexdata_t *pExistingTexData,
                    char const *cloneTexDataName) {
...
int FindTexInfo(const texinfo_t &searchTexInfo) {
  for (int i = 0; i < texinfo.Count(); i++) {
    ...
    if (texinfo[i].texdata != searchTexInfo.texdata) continue;

    if (!memcmp(&texinfo[i], &searchTexInfo, sizeof(texinfo_t))) return i;
  }
...
int FindOrCreateTexInfo(const texinfo_t &searchTexInfo) {
  ...
  if (onlyents) {
    Error(
        "FindOrCreateTexInfo:  Tried to create new texinfo during -onlyents "
...
int TexinfoForBrushTexture(plane_t *plane, brush_texture_t *bt,
  ...
  if (!bt->textureWorldUnitsPerTexel[0]) bt->textureWorldUnitsPerTexel[0] = 1;
  if (!bt->textureWorldUnitsPerTexel[1]) bt->textureWorldUnitsPerTexel[1] = 1;
    ...
    for (i = 0; i < 2; i++) {
      for (j = 0; j < 3; j++) {
        tx.textureVecsTexelsPerWorldUnits[i][j] =
            vecs[i][j] / bt->textureWorldUnitsPerTexel[i];
        tx.lightmapVecsLuxelsPerWorldUnits[i][j] =
            tx.textureVecsTexelsPerWorldUnits[i][j] / 16.0f;
      }
    }
  } else {
    tx.textureVecsTexelsPerWorldUnits[0][0] =
        bt->UAxis[0] / bt->textureWorldUnitsPerTexel[0];
    tx.textureVecsTexelsPerWorldUnits[0][1] =
        bt->UAxis[1] / bt->textureWorldUnitsPerTexel[0];
    tx.textureVecsTexelsPerWorldUnits[0][2] =
        bt->UAxis[2] / bt->textureWorldUnitsPerTexel[0];

    tx.textureVecsTexelsPerWorldUnits[1][0] =
        bt->VAxis[0] / bt->textureWorldUnitsPerTexel[1];
    tx.textureVecsTexelsPerWorldUnits[1][1] =
        bt->VAxis[1] / bt->textureWorldUnitsPerTexel[1];
    tx.textureVecsTexelsPerWorldUnits[1][2] =
        bt->VAxis[2] / bt->textureWorldUnitsPerTexel[1];

    tx.lightmapVecsLuxelsPerWorldUnits[0][0] =
        bt->UAxis[0] / bt->lightmapWorldUnitsPerLuxel;
    tx.lightmapVecsLuxelsPerWorldUnits[0][1] =
        bt->UAxis[1] / bt->lightmapWorldUnitsPerLuxel;
    tx.lightmapVecsLuxelsPerWorldUnits[0][2] =
        bt->UAxis[2] / bt->lightmapWorldUnitsPerLuxel;

    tx.lightmapVecsLuxelsPerWorldUnits[1][0] =
        bt->VAxis[0] / bt->lightmapWorldUnitsPerLuxel;
    tx.lightmapVecsLuxelsPerWorldUnits[1][1] =
        bt->VAxis[1] / bt->lightmapWorldUnitsPerLuxel;
    tx.lightmapVecsLuxelsPerWorldUnits[1][2] =
        bt->VAxis[2] / bt->lightmapWorldUnitsPerLuxel;

    shiftScaleU =
        bt->textureWorldUnitsPerTexel[0] / bt->lightmapWorldUnitsPerLuxel;
    shiftScaleV =
        bt->textureWorldUnitsPerTexel[1] / bt->lightmapWorldUnitsPerLuxel;
  }

  tx.textureVecsTexelsPerWorldUnits[0][3] =
      bt->shift[0] + DOT_PRODUCT(origin, tx.textureVecsTexelsPerWorldUnits[0]);
  tx.textureVecsTexelsPerWorldUnits[1][3] =
      bt->shift[1] + DOT_PRODUCT(origin, tx.textureVecsTexelsPerWorldUnits[1]);

  tx.lightmapVecsLuxelsPerWorldUnits[0][3] =
      shiftScaleU * bt->shift[0] +
      DOT_PRODUCT(origin, tx.lightmapVecsLuxelsPerWorldUnits[0]);
  tx.lightmapVecsLuxelsPerWorldUnits[1][3] =
      shiftScaleV * bt->shift[1] +
...
void LoadSurfaceProperties(void) {
  ...
  if (!physicsFactory) return;

  physprops = (IPhysicsSurfaceProps *)physicsFactory(
      VPHYSICS_SURFACEPROPS_INTERFACE_VERSION, NULL);

  const char *SURFACEPROP_MANIFEST_FILE =
      "scripts/surfaceproperties_manifest.txt";
  KeyValues *manifest = new KeyValues(SURFACEPROP_MANIFEST_FILE);
  if (manifest->LoadFromFile(g_pFileSystem, SURFACEPROP_MANIFEST_FILE,
                             "GAME")) {
