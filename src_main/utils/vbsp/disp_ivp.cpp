...
void Disp_AddCollisionModels(CUtlVector<CPhysCollisionEntry *> &collisionList,
                             dmodel_t *pModel, int contentsMask) {
  int dispIndex;
  ...
  for (int grid = 0; grid < gDispGridList.Count(); grid++) {
    ...
    for (int listIndex = 0; listIndex < gDispGridList[grid].dispList.Count();
         listIndex++) {
      ...
      // Build a triangle list. This shares the tesselation code with the
      // engine.
      ...
      Assert(indices.Count() % 3 ==
             0);  // Make sure indices are a multiple of 3.
      ...
      if (triCount >= 65536) {
        ...
        if (pCollide) {
          collisionList.AddToTail(
              new CPhysCollisionEntryStaticMesh(pCollide, NULL));
        }
      ...
      for (int iTri = 0; iTri < nTriCount; ++iTri) {
        ...
        int nProp =
            g_SurfaceProperties[texinfo[pMapDisp->face.texinfo].texdata];
        ...
        physcollision->PolysoupAddTriangle(pTerrainPhysics, tmpVerts[0],
                                           tmpVerts[1], tmpVerts[2],
                                           nMaterialIndex);
      }
    }

    // convert the whole grid's polysoup to a collide and store in the collision
    // list
    CPhysCollide *pCollide =
        physcollision->ConvertPolysoupToCollide(pTerrainPhysics, false);
    if (pCollide) {
...
class CDispMeshEvent : public IVirtualMeshEvent {
 public:
  CDispMeshEvent(unsigned short *pIndices, int indexCount,
...
CDispMeshEvent::CDispMeshEvent(unsigned short *pIndices, int indexCount,
                               CCoreDispInfo *pDispInfo) {
  m_pIndices = pIndices;
...
void CDispMeshEvent::GetVirtualMesh(void *userData, virtualmeshlist_t *pList) {
  ...
  pList->pHull = NULL;
  int indexMax = SOURCE_ARRAYSIZE(pList->indices);
...
void CDispMeshEvent::GetWorldspaceBounds(void *userData, Vector *pMins,
                                         Vector *pMaxs) {
  Assert(userData == ((void *)this));
...
void CDispMeshEvent::GetTrianglesInSphere(void *userData, const Vector &center,
                                          float radius,
                                          virtualmeshtrianglelist_t *pList) {
  Assert(userData == ((void *)this));
  pList->triangleCount = m_indexCount / 3;
  int indexMax = SOURCE_ARRAYSIZE(pList->triangleIndices);
  int indexCount = std::min(m_indexCount, indexMax);
  Assert(m_indexCount < MAX_VIRTUAL_TRIANGLES * 3);
  Q_memcpy(pList->triangleIndices, m_pIndices,
           sizeof(*m_pIndices) * indexCount);
}
...
void Disp_BuildVirtualMesh(int contentsMask) {
  ...
  g_PhysDispSize = totalSize + sizeof(dphysdisp_t) +
                   (sizeof(unsigned short) * header.numDisplacements);
