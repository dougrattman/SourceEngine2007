...
void ComputeDispInfoBounds(int dispinfo, Vector &mins, Vector &maxs) {
  ...
  // Get a CCoreDispInfo. All we need is the triangles and lightmap texture
  // coordinates.
...
// Gets the barycentric coordinates of the position on the triangle where the
// lightmap coordinates are equal to lmCoords. This always generates the
// coordinates but it returns false if the point containing them does not lie
// inside the triangle.
bool GetBarycentricCoordsFromLightmapCoords(Vector2D tri[3],
...
bool FindTriIndexMapByUV(CCoreDispInfo *pCoreDisp, Vector2D const &lmCoords,
  ...
  for (int iTri = 0; iTri < nTriCount; ++iTri) {
    ...
    if (GetBarycentricCoordsFromLightmapCoords(vecUV, lmCoords,
                                               flBarycentric)) {
      iTriangle = iTri;
      return true;
    }
...
void CalculateLightmapSamplePositions(CCoreDispInfo *pCoreDispInfo,
                                      const dface_t *pFace,
...
void DispMapToCoreDispInfo(mapdispinfo_t *pMapDisp,
  ...
  Vector2D tCoords[4] = {Vector2D(0, 0), Vector2D(0, 1), Vector2D(1, 0),
                         Vector2D(1, 1)};
  if (pFace) {
    Assert(pFace->numedges == 4);

    Vector pt[4];
    for (int i = 0; i < 4; i++) pt[i] = pWinding->p[i];

    int zeroOffset[2] = {0, 0};
    CalcTextureCoordsAtPoints(pTexInfo->textureVecsTexelsPerWorldUnits,
                              zeroOffset, pt, 4, tCoords);
  }
  ...
  if (pFace) {
    ...
    if (bSwap) {
      if (pSwappedTexInfos[pMapDisp->face.texinfo] < 0) {
        ...
        // This fixes a lighting bug in d2_prison_08 where many non-displacement
        // surfaces were pitch black, in addition to bugs in other maps I bet.

        // NOTE: Copy here because adding a texinfo could realloc.
        texinfo_t temp = *pTexInfo;
        memcpy(temp.lightmapVecsLuxelsPerWorldUnits[0],
               pTexInfo->lightmapVecsLuxelsPerWorldUnits[1], 4 * sizeof(float));
        memcpy(temp.lightmapVecsLuxelsPerWorldUnits[1],
               pTexInfo->lightmapVecsLuxelsPerWorldUnits[0], 4 * sizeof(float));
    ...
    // NOTE: This is here to help future-proof code, since there are codepaths
    // where pTexInfo can be made invalid (texinfo.AddToTail above).
    pTexInfo = NULL;
  }
  ...
  pCoreDispInfo->InitDispInfo(pMapDisp->power, pMapDisp->minTess,
                              pMapDisp->smoothingAngle, pMapDisp->alphaValues,
                              vectorDisps, dispDists);
...
void ExportNeighborData(CCoreDispInfo **ppListBase, ddispinfo_t *pBSPDispInfos,
                        int listSize) {
...
void ExportCoreDispAllowedVertList(const CCoreDispInfo *pIn,
                                   ddispinfo_t *pOut) {
  ErrorIfNot(
...
void ExportAllowedVertLists(CCoreDispInfo **ppListBase,
                            ddispinfo_t *pBSPDispInfos, int listSize) {
...
bool FindEnclosingTri(const Vector2D &vert, CUtlVector<Vector2D> &vertCoords,
  ...
  for (int i = 0; i < indices.Count(); i += 3) {
    GetBarycentricCoords2D(vertCoords[indices[i + 0]],
                           ...
                           vertCoords[indices[i + 2]], vert, bcCoords);

    if (bcCoords[0] >= 0 && bcCoords[0] <= 1 && bcCoords[1] >= 0 &&
        bcCoords[1] <= 1 && bcCoords[2] >= 0 && bcCoords[2] <= 1) {
      *pStartVert = i;
      return true;
    }
  }
...
void SnapRemainingVertsToSurface(CCoreDispInfo *pCoreDisp,
                                 ddispinfo_t *pDispInfo) {
  ...
  // Generate 2D floating point coordinates for each vertex. We use these to
  // generate barycentric coordinates, and the scale doesn't matter.
  ...
  for (int y = 0; y < pCoreDisp->GetHeight(); y++) {
    for (int x = 0; x < pCoreDisp->GetWidth(); x++) {
      ...
      if (!(vertsTouched[index])) {
        float bcCoords[3];
        int iStartVert = -1;
        if (FindEnclosingTri(vertCoords[index], vertCoords, indices,
                             &iStartVert, bcCoords)) {
          ...
          // This is kind of cheesy, but it gets the job done. Since the
          // CDispVerts store the verts relative to some other offset, we'll
          // just offset their position instead of setting it directly.
          ...
          // Modify the CCoreDispInfo vert (although it probably won't be used
          // later).
          pCoreDisp->SetVert(index, vNewPos);
        } else {
          // This shouldn't happen because it would mean that the triangulation
          // that disp_tesselation.h produced was missing a chunk of the space
          // that the displacement covers. It also could indicate a
          // floating-point epsilon error.. check to see if FindEnclosingTri
          // finds a triangle that -almost- encloses the vert.
          Assert(false);
        }
      }
    }
...
void SnapRemainingVertsToSurface(CCoreDispInfo **ppListBase,
                                 ddispinfo_t *pBSPDispInfos, int listSize) {
...
void EmitDispLMAlphaAndNeighbors() {
  ...
  for (i = 0; i < nummapdispinfo; i++) {
    g_CoreDispInfos[i]->SetDispUtilsHelperInfo(g_CoreDispInfos.Base(),
                                               nummapdispinfo);
  }
  ...
  ExportAllowedVertLists(g_CoreDispInfos.Base(), g_dispinfo.Base(),
                         nummapdispinfo);

  // Now that we know which vertices are actually going to be around, snap the
  // ones that won't be around onto the slightly-reduced mesh. This is so the
  // engine's ray test code and overlay code works right.
  SnapRemainingVertsToSurface(g_CoreDispInfos.Base(), g_dispinfo.Base(),
                              nummapdispinfo);
  ...
  for (i = 0; i < nummapdispinfo; i++) {
    ...
    pDisp->m_iLightmapSamplePositionStart =
        g_DispLightmapSamplePositions.Count();

    CalculateLightmapSamplePositions(pCoreDispInfo, pFace,
...
void DispGetFaceInfo(mapbrush_t *pBrush) {
  ...
  if (pBrush->entitynum != 0) {
    const char *pszEntityName =
        ValueForKey(&entities[pBrush->entitynum], "classname");
    Error(
        "Error: displacement found on a(n) %s entity - not supported (entity "
        "%d, brush %d)\n",
  ...
  for (i = 0; i < pBrush->numsides; i++) {
    ...
    if (pSide->pMapDisp) {
      ...
      if (pSide->winding->numpoints != 4)
        Error(
            "Trying to create a non-quad displacement! (entity %d, brush %d)\n",
