...
void CreateBrushWindings(bspbrush_t *brush) {
  ...
  for (i = 0; i < brush->numsides; i++) {
    ...
    w = BaseWindingForPlane(plane->normal,
                            plane->dist + DotProduct(plane->normal, offset));
    for (j = 0; j < brush->numsides && w; j++) {
      if (i == j) continue;
      if (brush->sides[j].bevel) continue;
...
void FreeBrush(bspbrush_t *brushes) {
  ...
  for (i = 0; i < brushes->numsides; i++)
    if (brushes->sides[i].winding) FreeWinding(brushes->sides[i].winding);
  free(brushes);
  if (numthreads == 1) c_active_brushes--;
}
...
int BrushBspBoxOnPlaneSide(const Vector &mins, const Vector &maxs,
  ...
  side = 0;
  if (dist1 >= PLANESIDE_EPSILON) side = PSIDE_FRONT;
  if (dist2 < PLANESIDE_EPSILON) side |= PSIDE_BACK;
...
int TestBrushToPlanenum(bspbrush_t *brush, int planenum, int *numsplits,
                        bool *hintsplit, int *epsilonbrush) {
  int i, j, num;
  plane_t *plane;
  int s;
  winding_t *w;
  ...
  for (i = 0; i < brush->numsides; i++) {
    ...
    if (front && back) {
      if (!(brush->sides[i].surf & SURF_SKIP)) {
        (*numsplits)++;
        if (brush->sides[i].surf & SURF_HINT) *hintsplit = true;
      }
    }
  }
...
void RemoveAreaPortalBrushes_R(node_t *node) {
  if (node->planenum == PLANENUM_LEAF) {
    // Remove any CONTENTS_AREAPORTAL brushes we added. We don't want them in
    // the engine at runtime but we do want their flags in the leaves.
...
side_t *SelectSplitSide(bspbrush_t *brushes, node_t *node) {
  ...
  //
  for (brush = brushes; brush; brush = brush->next) {
    for (i = 0; i < brush->numsides; i++) brush->sides[i].tested = false;
  }
...
void SplitBrush(bspbrush_t *brush, int planenum, bspbrush_t **front,
  ...
  *front = *back = NULL;
  ...
  // This gives us much better floating point precision in the clipping
  // operations
  ...
  for (int i = 0; i < brush->numsides && w; i++) {
    ...
    ChopWindingInPlace(&w, plane2->normal,
                       plane2->dist + DotProduct(plane2->normal, offset),
                       0);  // PLANESIDE_EPSILON);
  }
  ...
  for (int i = 0; i < brush->numsides; i++) {
    ...
    // clip the winding
    ClipWindingEpsilon_Offset(w, plane->normal, plane->dist,
                              0 /*PLANESIDE_EPSILON*/, &cw[0], &cw[1], offset);
  ...
  for (int i = 0; i < 2; i++) {
    ...
    int j;
    for (j = 0; j < 3; j++) {
      if (b[i]->mins[j] < MIN_COORD_INTEGER ||
          b[i]->maxs[j] > MAX_COORD_INTEGER) {
...
tree_t *BrushBSP(bspbrush_t *brushlist, Vector &mins, Vector &maxs) {
  ...
  for (b = brushlist; b; b = b->next) {
    ...
    for (i = 0; i < b->numsides; i++) {
      if (b->sides[i].bevel) continue;
      if (!b->sides[i].winding) continue;
