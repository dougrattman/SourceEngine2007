...
// This radius is set to 0.25, but it's symmetric.  So shrinking potentially
// moveable brushes by 0.5 in every direction ensures that these brushes can be
// constructed touching the world, and constrained in place without collisions
// or friction UNDONE: Add a key to disable this shrinking if necessary
...
void CTextBuffer::WriteFloatArrayKey(const char *pKeyName,
                                     const float *outputData, int count) {
  char tmp[1024];
...
void DumpCollideToGlView(CPhysCollide *pCollide, const char *pFilename) {
  ...
  for (int i = 0; i < triCount; i++) {
    fprintf(fp, "3\n");
    fprintf(fp, "%6.3f %6.3f %6.3f 1 0 0\n", outVerts[vert].x, outVerts[vert].y,
            outVerts[vert].z);
    vert++;
...
class CPhysCollisionEntrySolid : public CPhysCollisionEntry {
 public:
  CPhysCollisionEntrySolid(CPhysCollide *pCollide, const char *pMaterialName,
                           float mass);

  virtual void WriteToTextBuffer(CTextBuffer *pTextBuffer, int modelIndex,
                                 int collideIndex);
  virtual void DumpCollide(CTextBuffer *pTextBuffer, int modelIndex,
                           int collideIndex);
...
CPhysCollisionEntrySolid::CPhysCollisionEntrySolid(CPhysCollide *pCollide,
...
void CPhysCollisionEntrySolid::DumpCollide(CTextBuffer *pTextBuffer,
                                           int modelIndex, int collideIndex) {
  DumpCollideFileName("collide", modelIndex, pTextBuffer);
}

void CPhysCollisionEntrySolid::WriteToTextBuffer(CTextBuffer *pTextBuffer,
                                                 int modelIndex,
                                                 int collideIndex) {
...
void CPhysCollisionEntryStaticSolid::DumpCollide(CTextBuffer *pTextBuffer,
                                                 int modelIndex,
                                                 int collideIndex) {
  char tmp[128];
...
void CPhysCollisionEntryStaticSolid::WriteToTextBuffer(CTextBuffer *pTextBuffer,
                                                       int modelIndex,
                                                       int collideIndex) {
...
void CPhysCollisionEntryStaticMesh::DumpCollide(CTextBuffer *pTextBuffer,
                                                int modelIndex,
                                                int collideIndex) {
  char tmp[128];
...
void CPhysCollisionEntryStaticMesh::WriteToTextBuffer(CTextBuffer *pTextBuffer,
                                                      int modelIndex,
                                                      int collideIndex) {
  pTextBuffer->WriteText("staticsolid {\n");
  pTextBuffer->WriteIntKey("index", collideIndex);
...
class CPhysCollisionEntryFluid : public CPhysCollisionEntry {
 public:
  ...
  CPhysCollisionEntryFluid(CPhysCollide *pCollide, const char *pSurfaceProp,
                           float damping, const Vector &normal, float dist,
                           int nContents);
...
void CPhysCollisionEntryFluid::WriteToTextBuffer(CTextBuffer *pTextBuffer,
                                                 int modelIndex,
                                                 int collideIndex) {
  ...
  pTextBuffer->WriteFloatArrayKey("surfaceplane", array,
                                  4);  // write out water surface plane
  pTextBuffer->WriteFloatArrayKey("currentvelocity", vec3_origin.Base(),
                                  3);  // write out water velocity
  pTextBuffer->WriteText("}\n");
}
...
static int PropIndex(CUtlVector<int> &propList, int propIndex) {
  for (int i = 0; i < propList.Count(); i++) {
    if (propList[i] == propIndex) return i + 1;
  }
...
static void AddListPlane(CUtlVector<listplane_t> *list, float x, float y,
                         float z, float d) {
  listplane_t plane;
  plane.normal[0] = x;
  plane.normal[1] = y;
...
class CPlaneList {
 ...
 private:
  CPhysConvex *BuildConvexForBrush(int brushnumber, float shrink,
...
void CPlaneList::ReferenceBrush(int brushnumber) {
  if (!(dbrushes[brushnumber].contents & m_contentsMask)) return;
...
CPhysConvex *CPlaneList::BuildConvexForBrush(int brushnumber, float shrink,
                                             CPhysCollide *pCollideTest,
                                             float shrinkMinimum) {
  ...
  for (int i = 0; i < dbrushes[brushnumber].numsides; i++) {
    ...
    if (i < mapbrushes[brushnumber].numsides) {
      if (!mapbrushes[brushnumber].original_sides[i].visible) {
        ...
        // this produces something closer to the ideal shrink than simply
        // shrinking all planes
        shrinkThisPlane = 0;
      }
    }
    ...
    if (pCollideTest && shrinkThisPlane != 0) {
      ...
      Vector end = physcollision->CollideGetExtent(pCollideTest, vec3_origin,
                                                   vec3_angle, -pplane->normal);
      float thick = DotProduct((end - start), pplane->normal);
      // NOTE: The object must be at least "shrinkMinimum" inches wide on each
      // axis
      if (fabs(thick) < shrinkMinimum) {
#if _DEBUG
        Warning("Can't shrink brush %d, plane %d (%.2f, %.2f, %.2f)\n",
                brushnumber, pside->planenum, pplane->normal[0],
  ...
  return physcollision->ConvexFromPlanes((float *)temp.Base(), temp.Count(),
                                         m_merge);
}
  ...
  for (int brushnumber = 0; brushnumber < numbrushes; brushnumber++) {
    if (IsBrushReferenced(brushnumber)) {
      ...
      if (m_shrink != 0) {
        ...
        pBrushConvex = BuildConvexForBrush(brushnumber, m_shrink,
                                           pUnshrunkCollide, m_shrink * 3);
...
// NOTE: ConvexFromPlanes will be doing a bunch of matrix inversions that can
// suffer if plane normals are too close to each other...
#if 0
...
bool CPlaneList::IsLeafReferenced(int leafIndex) {
  if (!m_leafList.Count()) return true;

  for (int i = 0; i < m_leafList.Count(); i++) {
    if (m_leafList[i] == leafIndex) return true;
  }
...
struct waterleaf_t {
  ...
  int surfaceTexInfo;    // if hasSurface == true, this is the texinfo index for
                         // the water material
  int outsideLeafIndex;  // this is the leaf on the other side of the water
                         // surface
  node_t *pNode;
};

// returns true if newleaf should appear before currentleaf in the list
static bool IsLowerLeaf(const waterleaf_t &newleaf,
                        const waterleaf_t &currentleaf) {
  if (newleaf.hasSurface && currentleaf.hasSurface) {
    ...
    if (fabs(currentleaf.surfaceNormal.z - newleaf.surfaceNormal.z) < 0.01) {
      if (newleaf.surfaceDist < currentleaf.surfaceDist) return true;
    }
    return true;
...
//-----------------------------------------------------------------------------
struct WaterTexInfo {
  // The mangled new .vmt name ( materials/levelename/oldmaterial_depth_xxx )
  // where xxx is
...
void EmitWaterMaterialFile(WaterTexInfo *wti) {
  ...
  GetWaterTextureName(mapbase, wti->m_MaterialName.String(),
                      (int)wti->m_nWaterDepth, waterTextureName);
...
//-----------------------------------------------------------------------------
// Purpose: Takes the texinfo_t referenced by the .vmt and the computed depth
// for the
//  surface and looks up or creates a texdata/texinfo for the mangled one-off
//  water .vmt file
// Input  : *pBaseInfo -
//			depth -
// Output : int
...
int FindOrCreateWaterTexInfo(texinfo_t *pBaseInfo, float depth) {
  ...
  char const *name = TexDataStringTable_GetString(
      GetTexData(pBaseInfo->texdata)->nameStringTableID);
  ...
  for (i = pModel->firstface; i < pModel->firstface + pModel->numfaces; i++) {
    ...
    if (pFaceNode) {
      if ((pTexInfo->flags & SURF_WARP) &&
          pFaceNode->planenum == PLANENUM_LEAF && pFaceNode->diskId >= 0) {
        pFace->surfaceFogVolumeID = dleafs[pFaceNode->diskId].leafWaterDataID;
        dleafwaterdata_t *pLeafWaterData =
            &dleafwaterdata[pFace->surfaceFogVolumeID];

        // HACKHACK: Should probably mark these faces as water bottom or
        // "bottommaterial" faces. HACKHACK: Use a heuristic, if it points up,
        // it's the water top.
...
static bool PortalCrossesWater(waterleaf_t &baseleaf, portal_t *portal) {
  if (baseleaf.hasSurface) {
    int side = WindingOnPlaneSide(portal->winding, baseleaf.surfaceNormal,
                                  baseleaf.surfaceDist);
    if (side == SIDE_CROSS || side == SIDE_FRONT) return true;
  }

  return false;
}

static int FindOrCreateLeafWaterData(float surfaceZ, float minZ,
...
// Enumerate all leaves under node with contents in contentsMask and add them to
// list
...
static void BuildWaterLeaf(node_t *pLeafIn, waterleaf_t &waterLeafOut) {
  ...
  for (portal_t *p = pLeafIn->portals; p; p = p->next[!oppositeNodeIndex]) {
    ...
    if (!(pOpposite->contents & MASK_WATER) &&
        !(pOpposite->contents & MASK_SOLID)) {
      ...
      if (waterLeafOut.hasSurface) {
        ...
        if (waterLeafOut.surfaceNormal.z > plane->normal.z) continue;
...
static void InsertSortWaterLeaf(CUtlVector<waterleaf_t> &list,
                                const waterleaf_t &leafInsert) {
  // insertion sort the leaf (lowest leaves go first)
  // leaves that aren't actually on the surface of the water will have
...
// Flood fill the tree, finding neighboring water volumes and connecting them to
// this list Cut groups that try to cross the surface. Mark leaves that are in a
// group as "visited" so they won't be chosen by subsequent fills
static void Flood_FindConnectedWaterVolumes_r(CUtlVector<node_t *> &list,
                                              node_t *pLeaf,
  ...
  for (portal_t *p = pLeaf->portals; p; p = p->next[!oppositeNodeIndex]) {
    ...
    Flood_FindConnectedWaterVolumes_r(list, p->nodes[oppositeNodeIndex],
                                      baseleaf, visited);
  }
}
...
void EmitWaterVolumesForBSP(dmodel_t *pModel, node_t *node) {
  ...
  for (i = 0; i < list.Count(); i++) {
    Flood_FindConnectedWaterVolumes_r(waterAreaList, list[i].pNode, list[i],
                                      visited);

    // did we find a list of leaves connected to this one?
    // remember the list is sorted, so this one may have been attached to a
    // previous leaf.  So it could have nothing hanging off of it.
    if (waterAreaList.Count()) {
      ...
      if (tmp.waterLeafData.surfaceTexInfo < 0) {
        // the map has probably leaked in this case, but output something
        // anyway.
      ...
      tmp.depthTexinfo = FindOrCreateWaterTexInfo(
          &texinfo[tmp.waterLeafData.surfaceTexInfo], waterDepth);
      tmp.fogVolumeIndex = FindOrCreateLeafWaterData(
          tmp.waterLeafData.surfaceDist, tmp.waterLeafData.minZ,
          tmp.waterLeafData.surfaceTexInfo);
...
static void ConvertWaterModelToPhysCollide(
    CUtlVector<CPhysCollisionEntry *> &collisionList, int modelIndex,
  ...
  for (int i = 0; i < g_WaterModels.Count(); i++) {
    ...
    if (waterModel.modelIndex != modelIndex) continue;
    ...
    // BUGBUG: NOTE: If your map has a brush that crosses the surface, it will
    // be added to two water volumes.  This only happens with connected water
    // volumes with multiple surface heights UNDONE: Right now map makers must
    // cut such brushes.  It could be automatically cut by adding the surface
    // plane to the list for each brush before calling ConvexFromPlanes()
    planes.AddBrushes();
    ...
    if (pCollide) {
      ...
      CPhysCollisionEntryFluid *pCollisionEntryFuild =
          new CPhysCollisionEntryFluid(pCollide, pSurfaceProp, damping,
                                       waterModel.waterLeafData.surfaceNormal,
...
// compute a normal for a triangle of the given three points (points are
// clockwise, normal points out)
static Vector TriangleNormal(const Vector &p0, const Vector &p1,
                             const Vector &p2) {
  Vector e0 = p1 - p0;
  Vector e1 = p2 - p0;
...
static void ConvertWorldBrushesToPhysCollide(
    CUtlVector<CPhysCollisionEntry *> &collisionList, float shrinkSize,
    float mergeTolerance, int contentsMask) {
  CPlaneList planes(shrinkSize, mergeTolerance);
  ...
  if (count) {
    ...
    collisionList.AddToTail(
        new CPhysCollisionEntryStaticSolid(pCollide, contentsMask));
  }
}
...
static void BuildWorldPhysModel(
  ...
  ConvertWorldBrushesToPhysCollide(collisionList, shrinkSize, mergeTolerance,
                                   MASK_SOLID);
  ConvertWorldBrushesToPhysCollide(collisionList, shrinkSize, mergeTolerance,
                                   CONTENTS_PLAYERCLIP);
  ConvertWorldBrushesToPhysCollide(collisionList, shrinkSize, mergeTolerance,
...
static void ConvertModelToPhysCollide(
    CUtlVector<CPhysCollisionEntry *> &collisionList, int modelIndex,
    int contents, float shrinkSize, float mergeTolerance) {
  int i;
  CPlaneList planes(shrinkSize, mergeTolerance);
  ...
  if (maxIndex >= 0) {
    ...
    // if this is a "shell" material (it is hollow and encloses some empty
    // space) compute the mass with a constant surface thickness
    if (thickness != 0) {
  ...
  collisionList.AddToTail(
      new CPhysCollisionEntrySolid(pCollide, pMaterial, mass));
}
...
void EmitPhysCollision() {
  ...
  for (i = 0; i < nummodels; i++) {
    ...
    pTextBuffer[i] = NULL;
    if (!collisionList[i].Count()) continue;

    // if we've got collision models, write their script for processing in the
    // game
      ...
      if (s_WorldPropList.Count()) {
        ...
        for (j = 0; j < s_WorldPropList.Count(); j++) {
          ...
          if (propIndex < 0) {
            ...
            pTextBuffer[i]->WriteIntKey(physprops->GetPropName(propIndex),
                                        j + 1);
          }
        }
  ...
  for (i = 0; i < nummodels; i++) {
    if (pTextBuffer[i]) {
      ...
      for (j = 0; j < model.solidCount; j++) {
        ...
        collisionList[i][j]->WriteCollisionBinary(
