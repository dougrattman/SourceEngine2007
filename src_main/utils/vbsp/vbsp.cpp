...
void ProcessBlock_Thread(int threadnum, int blocknum) {
  ...
  if (!nocsg) brushes = ChopBrushes(brushes);
...
void ProcessWorldModel(void) {
  ...
  for (optimize = 0; optimize <= 1; optimize++) {
    ...
    RunThreadsOnIndividual(
        (block_xh - block_xl + 1) * (block_yh - block_yl + 1), !verbose,
        ProcessBlock_Thread);
...
#if DEBUG_BRUSHMODEL
  if (entity_num == DEBUG_BRUSHMODEL) WriteGLView(tree, "tree_all");
#endif
...
static void GenerateOccluderFaceList(node_t *pOccluderNode,
  ...
  if (pOccluderNode->planenum == PLANENUM_LEAF) return;
  ...
  for (entity_num = 1; entity_num < num_entities; ++entity_num) {
    ...
    for (int i = faceList.Count(); --i >= 0;) {
      ...
      for (int j = sideList.Count(); --j >= 0;) {
        if (sideList[j] != f->originalface) continue;
  ...
  for (int j = 0; j < g_OccluderData[nOccluder].polycount; ++j) {
    ...
    for (int k = 0; k < pOccluderPoly->vertexcount; ++k) {
      ...
      for (pPortal = pNode->portals; pPortal;
        ...
        if (!pPortal->onnode) continue;  // edge of world
...
bool Is3DSkyboxArea(int area) {
  for (int i = g_SkyAreas.Count(); --i >= 0;) {
    if (g_SkyAreas[i] == area) return true;
  }
  return false;
}
...
int RunVBSP(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (!_stricmp(argv[i], "-threads")) {
      ...
      if (g_minLuxelScale < 1) g_minLuxelScale = 1;
      i++;
  ...
  GetPlatformMapPath(the_source, platformBSPFileName, g_nDXLevel, 1024);
