...
//			original face in any visible leaves that a clipped fragment
//falls 			into.
...
bool MergeFace_r(node_t *node, face_t *face, face_t *original) {
  ...
  if (node->planenum == PLANENUM_LEAF) {
    ...
    ClassifyWindingEpsilon_Offset(face->w, plane->normal, plane->dist, 0.001,
                                  &frontwinding, &backwinding, &onwinding,
                                  -offset);

    if (onwinding) {
      // face is in the split plane, go down the appropriate side according to
      // the facing direction
      assert(frontwinding == NULL);
      assert(backwinding == NULL);

      if (DotProduct(mapplanes[face->planenum].normal,
                     mapplanes[node->planenum].normal) > 0) {
...
//-----------------------------------------------------------------------------
// Purpose: Splits the face list into faces from the same plane and tries to
// merge
//			them if possible
// Input  : **pFaceList -
//-----------------------------------------------------------------------------
...
face_t *MergeDetailTree(tree_t *worldtree, int brush_start, int brush_end) {
  ...
  detailbrushes =
      MakeBspBrushList(brush_start, brush_end, map_mins, map_maxs, ONLY_DETAIL);
  if (detailbrushes) {
    start = Plat_FloatTime();
    Msg("Chop Details...");
    // if there are detail brushes, chop them against each other
    if (!nocsg) detailbrushes = ChopBrushes(detailbrushes);

    Msg("done (%d)\n", (int)(Plat_FloatTime() - start));
    // Now mark the visible sides so we can eliminate all detail brush sides
    // that are covered by other detail brush sides
    // NOTE: This still leaves detail brush sides that are covered by the world.
    // (these are removed in the merge operation)
...
bool BrushBoxOverlap(bspbrush_t *p1, bspbrush_t *p2) {
  ...
  for (int i = 0; i < 3; i++) {
    if (p1->mins[i] > p2->maxs[i] || p1->maxs[i] < p2->mins[i]) return false;
  }
...
//-----------------------------------------------------------------------------
// NOTE: This assumes the brushes have already been chopped so that no solid
// space is enclosed by more than one brush!!
bool ClipFaceToBrush(face_t *pFace, bspbrush_t *pbrush, face_t **pOutputList) {
  ...
  for (i = 0; i < pbrush->numsides && foundSide < 0; i++) {
    ...
    if (bplane == planenum) foundSide = i;
  }
  ...
  if (foundSide >= 0) {
    ...
    for (i = 0; i < pbrush->numsides; i++) {
      ...
      if (pbrush->sides[i].bevel) continue;
    ...
    for (i = 0; i < sortedSides.Size(); i++) {
      ...
      if (index == foundSide) continue;

      plane_t *plane = &mapplanes[pbrush->sides[index].planenum];
      winding_t *frontwinding, *backwinding;
      ClipWindingEpsilon_Offset(currentface->w, plane->normal, plane->dist,
                                0.001, &frontwinding, &backwinding, offset);

      // only clip if some part of this face is on the back side of all brush
      // sides
    ...
    // if we made it all the way through and didn't produce any fragments then
    // the whole face was clipped away
...
//			Brushes have already been chopped up so that they do not
//overlap, 			they merely touch.
...
side_t *FindOriginalSide(mapbrush_t *mb, side_t *pBspSide) {
  ...
  for (int i = 0; i < mb->numsides; i++) {
    ...
    if ((side->planenum & ~1) == (pBspSide->planenum & ~1)) {  // exact match
...
int GetListOfCutBrushes(CUtlVector<bspbrush_t *> &out, bspbrush_t *pSourceBrush,
  ...
  for (bspbrush_t *walk = pBrushList; walk; walk = walk->next) {
    ...
    if (!BrushBoxOverlap(pSourceBrush, walk)) continue;
...
static void ClipFaceToBrushList(face_t *f,
                                const CUtlVector<bspbrush_t *> &cutBrushes,
                                face_t **pOutputList) {
  *pOutputList = NULL;
  ...
  for (int i = 0; i < cutBrushes.Count(); i++) {
    ...
    for (face_t *pCutFace = pClipList; pCutFace; pCutFace = pCutFace->next) {
      ...
      if (ClipFaceToBrush(pCutFace, cut, &pClip)) {
        ...
        // insert face fragments at head of list (UNDONE: reverses order, do we
        // care?)
        while (pClip) {
...
face_t *ComputeVisibleBrushSides(bspbrush_t *list) {
  ...
  for (bspbrush_t *pbrush = list; pbrush; pbrush = pbrush->next) {
    ...
    for (int i = 0; i < pbrush->numsides; i++) {
      ...
      if (!(pbrush->sides[i].contents & ALL_VISIBLE_CONTENTS)) continue;
      ...
      for (face_t *f = pFaces; f; f = f->next) {
        ...
        if (pClip) {
          ...
          if (outCount <= 1) {
            ...
            f->split[0] = f;
            // insert face fragments at head of list (UNDONE: reverses order, do
            // we care?)
            while (pClip) {
              face_t *next = pClip->next;
              pClip->next = pFaces;
