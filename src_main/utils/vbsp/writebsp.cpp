    ...
    for (j = firstmodeledge; j < numedges; j++) {
      if ((eIndex[0] == dedges[j].v[1]) && (eIndex[1] == dedges[j].v[0]) &&
        ...
        if (numsurfedges >= MAX_MAP_SURFEDGES)
          Error(
              "Too much brush geometry in bsp, numsurfedges == "
              "MAX_MAP_SURFEDGES");
...
int FindOrigFace(face_t *f) {
  ...
  for (pSide = pOrigFaceSideList[f->planenum]; pSide; pSide = pSide->next) {
    if (pSide == f->originalface) return pSide->origIndex;
  }
...
int EmitDrawNode_r(node_t *node) {
  ...
  // emit a node
  if (numnodes == MAX_MAP_NODES) Error("MAX_MAP_NODES");
...
void SetLightStyles(void) {
  ...
  for (i = 1; i < num_entities; i++) {
    ...
    if (Q_strncasecmp(t, "light", 5)) continue;
    ...
    for (j = 0; j < stylenum; j++)
      if (!strcmp(lighttargets[j], t)) break;
    if (j == stylenum) {
...
void EmitBrushes(void) {
  ...
  for (bnum = 0; bnum < nummapbrushes; bnum++) {
    ...
    for (j = 0; j < b->numsides; j++) {
      if (numbrushsides == MAX_MAP_BRUSHSIDES) Error("MAX_MAP_BRUSHSIDES");
...
void DiscoverMacroTextures() {
  ...
  for (int iFace = 0; iFace < numfaces; iFace++) {
    ...
    if (pTexInfo->texdata < 0) continue;
...
void EnsurePresenceOfWaterLODControlEntity(void) {
  ...
  // None found, add one.
  Warning(
      "Water found with no water_lod_control entity, creating a default "
      "one.\n");
...
void EndBSPFile(void) {
  ...
  // phys collision needs dispinfo to operate (needs to generate phys collision
  // for displacement surfs)
  EmitPhysCollision();
  ...
  GetPlatformMapPath(the_source, targetPath, g_nDXLevel, 1024);
  Msg("Writing %s\n", targetPath);
...
static void AddNodeToBounds(int node, CUtlVector<int> &skipAreas, Vector &mins,
  ...
  if (node >= 0) {
    ...
    for (i = skipAreas.Count(); --i >= 0;) {
      if (dleafs[leaf].area == skipAreas[i]) return;
    }
    ...
    for (i = 0; i < dleafs[leaf].numleaffaces; ++i) {
      ...
      if (tex.flags & (SURF_SKY | SURF_NODRAW)) continue;
...
bool IsBoxInsideWorld(int node, CUtlVector<int> &skipAreas,
  ...
  while (1) {
    ...
    if (node < 0) {
      ...
      int i;
      for (i = skipAreas.Count(); --i >= 0;) {
        if (dleafs[leaf].area == skipAreas[i]) return false;
      }
