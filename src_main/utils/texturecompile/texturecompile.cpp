...
void Texture_ParseTextureInfoFromCompileCommands(int nCompileCommandID,
                                                 TextureInfo_t &shaderInfo);
...
CDispatchReg g_TextureHadErrorReg(TEXTUREHADERROR_PACKETID,
                                  TextureHadErrorDispatch);
...
void Master_ReceiveWorkUnitFn(uint64 iWorkUnit, MessageBuffer *pBuf,
                              int iWorker) {
  ...
  for (i = textureStart; i < textureEnd; i++) {
    ...
    if (!fp) Error("Can't open %s for writing.\n", g_CompileCommands[i]);
...
void Worker_ProcessWorkUnitFn(int iThread, uint64 iWorkUnit,
                              MessageBuffer *pBuf) {
  ...
  for (i = textureStart; i < textureEnd; i++) {
    ...
    sprintf(cmdline,
            "vtex -allowdebug -vproject \"%s%s\" -mkdir -nopause \"%s%s\"",
            g_WorkerTempPath, g_pGameDir + 3, g_WorkerTempPath,
            tganame + 3);  // hack hack
    DebugOut(cmdline);
    DebugOut("\n");
    //		MySystem( cmdline );
    ...
    if (fp) {
      ...
      //
      //			if( !m_FileAlreadyFailed.Defined(
      // g_CompileCommands[i] ) )
      //			{
      //				m_FileAlreadyFailed[g_CompileCommands[i]]
      //= true; 				CUtlVector<char> fileNameBuf;
      // fileNameBuf.AddToTail( ( char
      //)TEXTUREHADERROR_PACKETID ); 				int len =
      // strlen( g_CompileCommands[i] );
      // fileNameBuf.AddMultipleToTail( len + 1,
      // g_CompileCommands[i] ); 				VMPI_SendData(
      // fileNameBuf.Base(), fileNameBuf.Count(), VMPI_MASTER_ID );
      //			}
...
void Worker_ReadFilesToCopy(void) {
  ...
  while (CmdLib_FGets(buf, sizeof(buf), fp)) {
    ...
    *pStar = '\0';
    ...
    SourceTargetPair_t &pair =
        g_SourceTargetPairs[g_SourceTargetPairs.AddToTail()];
...
void Worker_GetFileFromMaster(const char *pFileName) {
  ...
  char path[1024];
  char filename[1024];
  sprintf(path, "%s%s", g_WorkerTempPath,
          pFileName + 3);  // dear lord . .skip the u:\ BUG BUG BUG
                           //		printf( "creating \"%s\"\n", path );
  Q_StripFilename(path);
  MakeDirHier(path);

  sprintf(filename, "%s%s", g_WorkerTempPath,
          pFileName + 3);  // dear lord . .skip the u:\ BUG BUG BUG
                           //	printf( "creating \"%s\"\n", pFileName );
...
void Worker_GetLocalCopyOfTextureSource(const char *pVtfName) {
  ...
  for (i = 0; i < g_SourceTargetPairs.Count(); i++) {
    ...
    // g_SourceTargetPairs[i].pTargetName );
...
void Shared_ParseListOfCompileCommands(void) {
  ...
  while (CmdLib_FGets(buf, 1023, fp)) {
    ...
    pNewString[strlen(pNewString) - 2] =
        '\0';  // This is some hacky shit right here.
...
int TextureCompile_Main(int argc, const char *argv[]) {
  InstallSpewFunction();
  ...
  if (!VMPI_Init(argc, argv, "dependency_info_texturecompile.txt",
  ...
  if (g_bMPIMaster) {
    ...
    TouchFile(path);
    sprintf(path, "%s%s\\bin\\client.dll", g_WorkerTempPath,
            g_pGameDir + 3);  // hack hack
    TouchFile(path);
...
int CTextureCompileDLL::main(int argc, const char **argv) {
  return TextureCompile_Main(argc, argv);
}

EXPOSE_SINGLE_INTERFACE(CTextureCompileDLL, ILaunchableDLL,
