...
#pragma comment(lib,                                                         \
                "deps/dxsdk_jun2010/Lib/" SOURCE_DX_PROXY_PLATFOM_SHORT_NAME \
                "/d3dx9.lib")
#include "deps/dxsdk_jun2010/Include/d3dx9shader.h"
...
static BOOL ExtractDependencies() {
  return TRUE;
  /*
          BOOL bResult = TRUE;
          ...
          if ( bResult )
          {
                  sprintf( chExtractPath + strlen( chExtractPath ), "%s",
  szDllVersion ); bResult = ::CreateDirectory( chExtractPath, NULL );
                  if ( bResult )
                  {
                          ...
                          for ( int k = 0; k < sizeof( arrNames ) / ( 2 *
  sizeof( arrNames[0] ) ); ++ k )
                          {
                                  ...
                                  sprintf( chCreateFileName, "%s%s",
  chExtractPath, szName );

                                  HANDLE hFile = CreateFile( chCreateFileName,
  FILE_ALL_ACCESS, FILE_SHARE_READ, NULL, CREATE_NEW, FILE_ATTRIBUTE_HIDDEN |
  FILE_FLAG_DELETE_ON_CLOSE, NULL ); #error "This is how you can create temp
  needed resources"
                          }
                  }
          }
...
BOOL WINAPI DllMain(HMODULE, DWORD call_reason, LPVOID) {
  switch (call_reason) {
...
// Obtain DLL version
SOURCE_API_EXPORT const char* WINAPI GetDllVersionLong(void) {
...
SOURCE_API_EXPORT const char* WINAPI GetDllVersion(void) {
#if defined(DX9_V00_PC) && defined(_DEBUG)
...
// Proxied routines
SOURCE_API_EXPORT HRESULT WINAPI Proxy_D3DXCompileShaderFromFile(
  ...
  if (!pInclude) pInclude = &s_incDxImpl;

  // Open the top-level file via our include interface
  LPCVOID lpcvData;
  UINT numBytes;
  HRESULT hr =
      pInclude->Open((D3DXINCLUDE_TYPE)0, pSrcFile, NULL, &lpcvData, &numBytes);
  if (FAILED(hr)) return hr;

  LPCSTR pShaderData = (LPCSTR)lpcvData;

#if defined(DX9_V00_PC) || defined(DX9_V30_PC)
  hr = D3DXCompileShader(pShaderData, numBytes, pDefines, pInclude,
                         pFunctionName, pProfile, Flags, ppShader, ppErrorMsgs,
                         ppConstantTable);
#endif

#if defined(DX10_V00_PC)
  hr = D3DX10CompileFromMemory(pShaderData, numBytes, pSrcFile, pDefines,
                               pInclude, pFunctionName, pProfile, Flags, 0,
                               NULL, ppShader, ppErrorMsgs, NULL);
#endif
