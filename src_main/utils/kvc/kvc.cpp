...
//============//
//
// Purpose: vcd_sound_check.cpp : Defines the entry point for the console
// application.
//
//=============================================================================//
#include <stdio.h>
...
void BuildFileListWildcard_R(CUtlVector<CUtlSymbol> &files, char const *dir,
                             char const *wildcard) {
...
void BuildFileListFromWildcard(CUtlVector<CUtlSymbol> &files,
                               char const *search) {
  char basepath[512];
  char wildcard[512];
...
void CompileKeyValuesFiles(CUtlVector<CUtlSymbol> &scriptFiles,
  ...
  for (i = 0; i < c; ++i) {
    ...
    char filename[512];
    Q_strncpy(filename, g_Analysis.symbols.String(scriptFiles[i]),
              sizeof(filename));
...
void ExamineKVFile(char const *filename) {
  ...
  */
  vprint(0,
         "loading of %d elements took %.3f msec, create %.3f in-place %.3f\n",
         results.Count(), (float)timer.GetDuration().GetMillisecondsF(),
  ...
  vprint(0, "parsing of %d elements took %.3f msec\n", results.Count(),
         (float)kvt.GetDuration().GetMillisecondsF());
}
