...
HTMLDemo2::HTMLDemo2(Panel *parent, const char *name) : DemoPage(parent, name) {
  ...
  // now open a URL
  m_pHTML1->OpenURL("https://www.valvesoftware.com/en/");
  m_pHTML2->OpenURL("http://www.valve-erc.com");
  // the URL can be any valid URL accepted by Internet Explorer, use
  // file:///c:/... for local filesystem files :)

  // this call causes the control to repaint itself every 1000msec or so, to
  // allow animated gifs to work
