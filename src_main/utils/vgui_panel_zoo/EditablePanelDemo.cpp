...
// Purpose: Constructor
//-----------------------------------------------------------------------------
EditablePanelDemo::EditablePanelDemo(Panel *parent, const char *name)
    : DemoPage(parent, name) {
  ...
  m_pInternetSpeed =
      (ComboBox *)(m_pEditablePanel->CreateControlByName("ComboBox"));
  // Set its parent to our editable panel.
  ...
  // I could have just created a panel and just created each child panel on my
  // own using code like this: memberLabel =  new Label(NULL, NULL, "Label"); I
  // could have even saved many lines of code by choosing nice constructor args.
