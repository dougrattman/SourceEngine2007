...
void MDLViewer::initRecentFiles() {
  for (int i = 0; i < 8; i++) {
    if (strlen(recentFiles[i])) {
      mb->modify(IDC_FILE_RECENTMODELS1 + i, IDC_FILE_RECENTMODELS1 + i,
                 recentFiles[i]);
    } else {
      mb->modify(IDC_FILE_RECENTMODELS1 + i, IDC_FILE_RECENTMODELS1 + i,
                 "(empty)");
...
AccelTableEntry_t accelTable[NUM_ACCELERATORS] = {
...
#ifdef WIN32
  // SetWindowLong ((HWND) d_MatSysWindow->getHandle (), GWL_EXSTYLE,
  // WS_EX_CLIENTEDGE);
#endif
...
void MDLViewer::LoadModelFile(const char *pszFile, int slot) {
  ...
  if (eLoaded != LoadModel_Success) {
    switch (eLoaded) {
      case LoadModel_LoadFail: {
        mxMessageBox(this, "Error loading model.", g_appTitle,
                     MX_MB_ERROR | MX_MB_OK);
        break;
      }

      case LoadModel_PostLoadFail: {
        mxMessageBox(this, "Error post-loading model.", g_appTitle,
                     MX_MB_ERROR | MX_MB_OK);
        break;
      }

      case LoadModel_NoModel: {
        mxMessageBox(this, "Error loading model. The model has no vertices.",
                     g_appTitle, MX_MB_ERROR | MX_MB_OK);
        break;
      }
    }
  ...
  if (slot == -1) {
    int i;
    for (i = 0; i < 8; i++) {
      if (!mx_strcasecmp(recentFiles[i], filename)) break;
    }
    ...
    case mxEvent::Action: {
      switch (event->action) {
        ...
        case IDC_OPTIONS_COLORAMBIENT: {
          float *cols[4] = {g_viewerSettings.bgColor, g_viewerSettings.gColor,
                            g_viewerSettings.lColor, g_viewerSettings.aColor};
        ...
        case IDC_OPTIONS_MAKESCREENSHOT: {
          ...
          if (ptr) {
            if (!strstr(ptr, ".tga")) strcat(ptr, ".tga");
        ...
        case IDC_HELP_GOTOHOMEPAGE:
          ShellExecute(0, "open",
                       "http://www.swissquake.ch/chumbalum-soft/index.html", 0,
                       0, SW_SHOW);
          break;
#endif
...
bool CHLModelViewerApp::Create() {
  ...
  g_pStudioRender =
      (IStudioRender *)FindSystem(STUDIO_RENDER_INTERFACE_VERSION);
  ...
  g_pSoundEmitterBase = (ISoundEmitterSystemBase *)FindSystem(
...
int CHLModelViewerApp::Main() {
  ...
  size_t nParmCount = CommandLine()->ParmCount();
  if (nParmCount > 1) {
...
static bool CHLModelViewerApp_SuggestGameInfoDirFn(
