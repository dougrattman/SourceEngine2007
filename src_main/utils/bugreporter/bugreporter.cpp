...
static TRKELookup g_Lookup[] = {TRKERROR(TRK_SUCCESS),
...
TRK_UINT CBugReporter::Login(TRK_HANDLE *pTrkHandle) {
  ...
  GetPrivateProfileStringA("login", "userid1",
                           ...
                           username, sizeof(username), "PVCSTRK.ini");

  if (!Q_stricmp(username, DEFAULT_USERNAME) ||
      !Q_stricmp(username, "BELMAPNTKY"))  // if userid1 didn't have a valid
                                           // name in it try userid0
  {
  ...
  if (g_pFileSystem) {
    ...
    if (kv) {  //-V668
      if (kv->LoadFromFile(g_pFileSystem, TRACKER_SETTINGS)) {
        Q_strncpy(dbms, kv->GetString("database_server", DEFAULT_DBMS),
                  sizeof(dbms));
        Q_strncpy(proj, kv->GetString("project_name", DEFAULT_PROJECT),
                  sizeof(proj));
      }
      ...
      if (g_pFileSystem->FileExists(TRACKER_LOGIN, "GAME")) {
        if (kv->LoadFromFile(g_pFileSystem, TRACKER_LOGIN)) {
          Q_strncpy(username, kv->GetString("username", username),
                    sizeof(username));
  ...
  TRK_UINT rc = TrkProjectLogin(*pTrkHandle, username, password, proj, NULL,
                                NULL, NULL, NULL, TRK_USE_INI_FILE_DBMS_LOGIN);

  if (rc != TRK_SUCCESS) {
    rc = TrkProjectLogin(*pTrkHandle, username, "", proj, NULL, NULL, NULL,
                         NULL, TRK_USE_INI_FILE_DBMS_LOGIN);
    if (rc != TRK_SUCCESS) {
      if (maybeNoPVCSInstalled) {
        ...
        Msg("Bug reporter init failed: Your tracker password must be your user "
            "name or blank.\n");
      }
      return rc;
    }
...
bool CBugReporter::Init(CreateInterfaceFn engineFactory) {
  if (engineFactory) {
    g_pFileSystem =
        (IFileSystem *)engineFactory(FILESYSTEM_INTERFACE_VERSION, NULL);
    if (!g_pFileSystem) {
  ...
  if (trkRecHandle) {
    ...
    if (rc != TRK_SUCCESS) {
      ReportError(rc, "TrkRecordHandleFree",
                  "Failed to Free Tracker Record Handle!");
    }
  }
...
int CBugReporter::GetNameCount() { return m_Names.Count(); }

char const *CBugReporter::GetName(int index) {
  if (index < 0 || index >= m_Names.Count()) return "<<Invalid>>";

  return m_BugStrings.String(m_Names[index]);
}

int CBugReporter::GetDisplayNameCount() { return m_SortedDisplayNames.Count(); }
...
char const *CBugReporter::GetUserNameForDisplayName(char const *display) {
  ...
  if (idx == m_InternalNameMapping.InvalidIndex()) return "<<Invalid>>";
...
int CBugReporter::GetSeverityCount() { return m_Severity.Count(); }

char const *CBugReporter::GetSeverity(int index) {
  if (index < 0 || index >= m_Severity.Count()) return "<<Invalid>>";

  return m_BugStrings.String(m_Severity[index]);
}

int CBugReporter::GetPriorityCount() { return m_Priority.Count(); }

char const *CBugReporter::GetPriority(int index) {
  if (index < 0 || index >= m_Priority.Count()) return "<<Invalid>>";

  return m_BugStrings.String(m_Priority[index]);
}

int CBugReporter::GetAreaCount() { return m_Area.Count(); }

char const *CBugReporter::GetArea(int index) {
  if (index < 0 || index >= m_Area.Count()) return "<<Invalid>>";

  return m_BugStrings.String(m_Area[index]);
}

int CBugReporter::GetAreaMapCount() { return m_AreaMap.Count(); }

char const *CBugReporter::GetAreaMap(int index) {
  if (index < 0 || index >= m_AreaMap.Count()) return "<<Invalid>>";

  return m_BugStrings.String(m_AreaMap[index]);
}

int CBugReporter::GetMapNumberCount() { return m_MapNumber.Count(); }

char const *CBugReporter::GetMapNumber(int index) {
  if (index < 0 || index >= m_MapNumber.Count()) return "<<Invalid>>";

  return m_BugStrings.String(m_MapNumber[index]);
}

int CBugReporter::GetReportTypeCount() { return m_ReportType.Count(); }

char const *CBugReporter::GetReportType(int index) {
  if (index < 0 || index >= m_ReportType.Count()) return "<<Invalid>>";
...
void CBugReporter::SubstituteBugId(int bugid, char *out, int outlen,
                                   CUtlBuffer &src) {
  out[0] = 0;
...
bool CBugReporter::CommitBugReport(int &bugSubmissionId) {
  ...
  // Populate fields
  rc = TrkSetStringFieldValue(trkRecHandle, "Title", m_pBug->title);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to add title!");
    return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Submitter", m_pBug->submitter);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to set submitter!");
    return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Owner", m_pBug->owner);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to set owner!");
    return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Severity", m_pBug->severity);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to set severity!");
    // return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Report Type", m_pBug->reporttype);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to set report type!");
    // return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Priority", m_pBug->priority);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to set priority!");
    // return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Area", m_pBug->area);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetStringFieldValue", "Failed to set area!");
    // return false;
  }
  rc = TrkSetStringFieldValue(trkRecHandle, "Map Number", m_pBug->mapnumber);
  if (rc != TRK_SUCCESS) {
  ...
  buf.Printf("level:  %s\nbuild:  %s\nposition:  setpos %s; setang %s\n",
             m_pBug->level, m_pBug->build, m_pBug->position,
             m_pBug->orientation);
  ...
  rc = TrkSetDescriptionData(trkRecHandle, buf.TellPut(),
                             (const char *)buf.Base(), 0);
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkSetDescriptionData", "Failed to set description data!");
    return false;
  }
  ...
  if (rc != TRK_SUCCESS) {
    ReportError(rc, "TrkUpdateRecordBegin", "Failed to open bug id for update");
    return false;
  } else {
    ...
    rc = TrkSetDescriptionData(trkRecHandle, Q_strlen(textbuf) + 1,
...
void CBugReporter::SetScreenShot(char const *screenshot_unc_address) {
  Assert(m_pBug);
  Q_strncpy(m_pBug->screenshot_unc, screenshot_unc_address,
            sizeof(m_pBug->screenshot_unc));
}

void CBugReporter::SetSaveGame(char const *savegame_unc_address) {
  Assert(m_pBug);
  Q_strncpy(m_pBug->savegame_unc, savegame_unc_address,
            sizeof(m_pBug->savegame_unc));
}
...
void CBugReporter::ResetIncludedFiles() { m_pBug->includedfiles.Purge(); }

bool CBugReporter::PopulateChoiceList(char const *listname,
...
EXPOSE_SINGLE_INTERFACE(CBugReporter, IBugReporter,
