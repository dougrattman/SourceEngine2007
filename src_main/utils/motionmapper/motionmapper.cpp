...
static s_source_t *FindCachedSource(char const *name, char const *xext) {
  ...
  if (xext[0]) {
    ...
    for (i = 0; i < g_numsources; i++) {
      if (stricmp(g_szFilename, g_source[i]->filename) == 0) return g_source[i];
    }
  } else {
...
s_source_t *Load_Source(char const *name, const char *ext, bool reverse,
  ...
  cddir[numdirs], pTempName ); strcpyn( g_source[g_numsources]->filename,
  g_szFilename );
...
// TODO(d.rattman):  since we don't us a .qc, we could have problems with
// scaling, etc.???
...
void Set_DefaultTemplate(s_template_t *pTemplate) {
  ...
  strcpy(pTemplate->planeConstraints[0]->jointNameString,
         "ValveBiped.Bip01_L_Toe0");
  pTemplate->planeConstraints[1] = New_planeConstraint(pTemplate->toeFloorZ);
  strcpy(pTemplate->planeConstraints[1]->jointNameString,
  ...
  // strcpy(pTemplate->ikSolves[3]->relativeLockNameString,
...
s_template_t *Load_Template(char *name) {
  ...
  while (fgets(g_szLine, sizeof(g_szLine), g_fpInput) != NULL) {
    ...
    if (checkCommand(cmd, "twoJointIKSolve", 1, numOptions)) {
      ...
      strcpy(pTemplate->ikSolves[pTemplate->numIKSolves]->jointNameString,
             sp[1]);
      ...
      strcpy(pTemplate->planeConstraints[pTemplate->numPlaneConstraints]
                 ->jointNameString,
             sp[1]);
      ...
      strcpy(pTemplate->ikSolves[pTemplate->numIKSolves - 1]
      ...
      strcpy(pTemplate->jointScales[pTemplate->numJointScales]->jointNameString,
             sp[1]);
...
s_source_t *MotionMap(s_source_t *pSource, s_source_t *pTarget,
  ...
  for (int t = 0; t < sourceNumFrames; t++) {
    ...
    for (int ii = 0; ii < pTemplate->numIKSolves; ii++) {
      ...
      if (!thisSolve->doRelativeLock) {
        ...
        float effectiveScaleFactor =
            ((thisLimbLengthFactor - 1.0) * thisSolve->extremityScale) + 1.0;
    ...
    float effectiveRootScale =
        ((rootScaleFactor - 1.0) * pTemplate->rootScaleAmount) + 1.0;
