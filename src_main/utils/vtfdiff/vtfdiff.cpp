...
void Usage(void) { printf("Usage: vtfdiff file1.vtf file2.vtf\n"); }
...
int main(int argc, char **argv) {
  ...
  if (!LoadFileIntoBuffer(argv[1], file1)) return 21;
  if (!LoadFileIntoBuffer(argv[2], file2)) return 22;
  ...
  if (pTexture1->Width() != pTexture2->Width() ||
    ...
    printf("%s dimensions differ: %dx%dx%d != %dx%dx%d\n", argv[1],
           ...
           (int)pTexture1->Depth(), (int)pTexture2->Width(),
           (int)pTexture2->Height(), (int)pTexture2->Depth());
    bMatch = false;
  }
  ...
  if (bMatch) {
    ...
    if (memcmp(pData1, pData2, iSize) != 0) {
      ...
      if ((pTexture1->Format() == IMAGE_FORMAT_DXT1) ||
          (pTexture1->Format() == IMAGE_FORMAT_DXT3) ||
          (pTexture1->Format() == IMAGE_FORMAT_DXT5) ||
          (pTexture1->Format() == IMAGE_FORMAT_ATI2N) ||
        ...
        for (int iFrame = 0; iFrame < pTexture1->FrameCount(); ++iFrame) {
          for (int iMipLevel = 0; iMipLevel < pTexture1->MipCount();
               ++iMipLevel) {
            int nMipWidth, nMipHeight, nMipDepth;
            pTexture1->ComputeMipLevelDimensions(iMipLevel, &nMipWidth,
                                                 &nMipHeight, &nMipDepth);

            for (int iCubeFace = 0; iCubeFace < pTexture1->FrameCount();
                 ++iCubeFace) {
              for (int z = 0; z < nMipDepth; ++z) {
                pData1 =
                    pTexture1->ImageData(iFrame, iCubeFace, iMipLevel, 0, 0, z);
                pData2 =
                    pTexture2->ImageData(iFrame, iCubeFace, iMipLevel, 0, 0, z);
                ...
                if (memcmp(pData1, pData2, nMipSize)) {
                  ...
                  for (int y = 0; y < nMipHeight; ++y) {
                    for (int x = 0; x < nMipWidth; ++x) {
                      unsigned char const *pData1a = pTexture1->ImageData(
                          iFrame, iCubeFace, iMipLevel, x, y, z);
                      unsigned char const *pData2a = pTexture2->ImageData(
                          iFrame, iCubeFace, iMipLevel, x, y, z);

                      if (memcmp(
                              pData1a, pData2a,
                              ImageLoader::SizeInBytes(pTexture1->Format()))) {
                        printf(
                            "Frame %d Mip level %d Face %d Z-slice %d texel "
                            "(%d,%d) different!\n",
  ...
  // Lowres data
  {
    ...
    if (bMatch) {
      if (memcmp(pTexture1->LowResImageData(), pTexture2->LowResImageData(),
                 iSize1) != 0) {
  ...
  // Check other resources
  {
    ...
    u32 *puiresbuffer = stack_alloc<u32>(numRes1 + numRes2);
    ...
    for (int itx = 0; itx < 2; ++itx) {
      ...
      while (arrNums[itx]-- > 0) {
        ...
        if (mapTypes.Find(uiResType) != mapTypes.InvalidIndex()) continue;

        mapTypes.Insert(uiResType, true);

        size_t numBytes1, numBytes2;
        void const *pvResData1 =
            pTexture1->GetResourceData(uiResType, &numBytes1);
        void const *pvResData2 =
            pTexture2->GetResourceData(uiResType, &numBytes2);
        ...
        if (!pvResData1 != !pvResData2) {
          ...
          printf("%s different resource %s size %s != %s\n", argv[1],
                 ResourceToString(uiResType), numBytes1, numBytes2);
          bMatch = false;
