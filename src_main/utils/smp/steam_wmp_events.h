...
#ifndef STEAM_WMP_EVENTS_H_
#define STEAM_WMP_EVENTS_H_

#include "atl_headers.h"

// if these files aren't found, set your path to include the wmp sdk include
// directory download the sdk from
// http://www.microsoft.com/windows/windowsmedia/mp10/sdk.aspx
#include <wmp.h>
#include <wmpids.h>

class SteamWMPEvents : public CComObjectRootEx<CComSingleThreadModel>,
                       public IWMPEvents,
                       public _WMPOCXEvents {
 public:
  BEGIN_COM_MAP(SteamWMPEvents)
  ...
  (REFIID riid, OLECHAR FAR *FAR *rgszNames, unsigned int cNames, LCID lcid,
   DISPID FAR *rgDispId) {
    return (E_NOTIMPL);
  }
...
using CComWMPEventDispatch = CComObject<SteamWMPEvents>;

#endif  // !STEAM_WMP_EVENTS_H_
