...
bool SetVideoScaleMode(int videoScaleMode) {
  ...
  if (bStretchToFit == IsStretchedToFit()) return true;
...
bool SetFullScreen(bool bWantToBeFullscreen) {
  ...
  if (hr != S_OK) {
    assert(spWMPPlayer2 == NULL);  // the MS documentation claims this shouldn't
                                   // happen, but it does...
    spWMPPlayer2 = NULL;
  }
  ...
  if (g_bWantToBeFullscreen) {
    g_spWMPPlayer->put_uiMode(bstr_t{L"none"});
    ...
    g_spWMPPlayer->put_uiMode(bstr_t{L"full"});

    if (spWMPPlayer2) {
      spWMPPlayer2->put_stretchToFit(
...
LRESULT CWMPHost::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam,
  ...
  if (FAILED(hr)) OutputDebugString("put_URL failed\n");
...
LRESULT CWMPHost::OnClose(UINT uMsg, WPARAM wParam, LPARAM lParam,
                          BOOL& bHandled) {
...
LRESULT CWMPHost::OnDestroy(UINT uMsg, WPARAM wParam, LPARAM lParam,
                            BOOL& bHandled) {
...
LRESULT CWMPHost::OnErase(UINT /* uMsg */, WPARAM /* wParam */,
...
LRESULT CWMPHost::OnSize(UINT /* uMsg */, WPARAM wParam, LPARAM lParam,
                         BOOL& /* lResult */) {
...
LRESULT CWMPHost::OnContextMenu(UINT /* uMsg */, WPARAM /* wParam */,
  ...
  TrackPopupMenu(m_hPopupMenu, TPM_LEFTALIGN | TPM_TOPALIGN, x, y, 0, m_hWnd,
                 NULL);

  return 0;
}

LRESULT CWMPHost::OnClick(UINT /* uMsg */, WPARAM /* wParam */,
                          LPARAM /* lParam */, BOOL& /* lResult */) {
  if (IsFullScreen()) {
    SetFullScreen(false);
  }
...
LRESULT CWMPHost::OnLeftDoubleClick(UINT /* uMsg */, WPARAM /* wParam */,
                                    LPARAM /* lParam */, BOOL& /* lResult */) {
  SetFullScreen(!IsFullScreen());
  return 1;
}

LRESULT CWMPHost::OnSysKeyDown(UINT /* uMsg */, WPARAM wParam,
                               LPARAM /* lParam */, BOOL& /* lResult */) {
  switch (wParam) {
    case VK_RETURN:
      SetFullScreen(!IsFullScreen());
...
LRESULT CWMPHost::OnKeyDown(UINT /* uMsg */, WPARAM wParam, LPARAM /* lParam */,
                            BOOL& /* lResult */) {
  switch (wParam) {
    case VK_SPACE:
...
LRESULT CWMPHost::OnNCActivate(UINT /* uMsg */, WPARAM wParam,
                               LPARAM /* lParam */, BOOL& /* lResult */) {
  if (wParam) {
    if (g_bWantToBeFullscreen) {
      SetFullScreen(true);
...
LRESULT CWMPHost::OnVideoScale(WORD wNotifyCode, WORD wID, HWND hWndCtl,
                               BOOL& bHandled) {
