...
int main(int argc, char **argv) {
  ...
  Msg("vtf reflectivity: %f %f %f\n", vecReflectivity[0], vecReflectivity[1],
      vecReflectivity[2]);
  Msg("transparency: ");
    ...
    for (int iMipLevel = 0; iMipLevel <= iLastMipLevel; ++iMipLevel) {
      ...
      for (int iCubeFace = 0; iCubeFace < iFaceCount; ++iCubeFace) {
        for (int z = 0; z < iDepth; ++z) {
          ...
          char *pTempNameBuf = stack_alloc<char>(iTGANameLen + 13);
          ...
          if (bIsCubeMap) {
            ...
            static const char *pCubeFaceName[7] = {"rt", "lf", "bk", "ft",
                                                   "up", "dn", "sph"};
          ...
          unsigned char *pSrcImage =
              pTex->ImageData(iFrame, iCubeFace, iMipLevel, 0, 0, z);
          ...
          if (srcFormat == IMAGE_FORMAT_RGBA16161616F) {
            ...
            if (ImageLoader::IsTransparent(srcFormat) ||
                (srcFormat == IMAGE_FORMAT_ATI1N) ||
                (srcFormat == IMAGE_FORMAT_ATI2N)) {
          ...
          unsigned char *pDstImage =
              new unsigned char[ImageLoader::GetMemRequired(iWidth, iHeight, 1,
                                                            dstFormat, false)];
          if (!ImageLoader::ConvertImageFormat(pSrcImage, srcFormat, pDstImage,
                                               dstFormat, iWidth, iHeight, 0,
                                               0)) {
          ...
          if (dstFormat != IMAGE_FORMAT_RGB323232F) {
            if (ImageLoader::IsTransparent(dstFormat) &&
                (dstFormat != IMAGE_FORMAT_RGBA8888)) {
              unsigned char *tmpImage = pDstImage;
              pDstImage = new unsigned char[ImageLoader::GetMemRequired(
                  iWidth, iHeight, 1, IMAGE_FORMAT_RGBA8888, false)];
              if (!ImageLoader::ConvertImageFormat(
                      tmpImage, dstFormat, pDstImage, IMAGE_FORMAT_RGBA8888,
                      iWidth, iHeight, 0, 0)) {
                Error("Error converting from %s to %s\n",
                      ImageLoader::GetName(dstFormat),
              ...
              dstFormat = IMAGE_FORMAT_RGBA8888;
            } else if (!ImageLoader::IsTransparent(dstFormat) &&
                       (dstFormat != IMAGE_FORMAT_RGB888)) {
              unsigned char *tmpImage = pDstImage;
              pDstImage = new unsigned char[ImageLoader::GetMemRequired(
                  iWidth, iHeight, 1, IMAGE_FORMAT_RGB888, false)];
              if (!ImageLoader::ConvertImageFormat(
                      tmpImage, dstFormat, pDstImage, IMAGE_FORMAT_RGB888,
                      iWidth, iHeight, 0, 0)) {
                Error("Error converting from %s to %s\n",
                      ImageLoader::GetName(dstFormat),
