...
void CheckStack(leaf_t *leaf, threaddata_t *thread) {
  ...
  for (p = thread->pstack_head.next; p; p = p->next) {
    //		Msg ("=");
    if (p->leaf == leaf) Error("CheckStack: leaf recursion");
    for (p2 = thread->pstack_head.next; p2 != p; p2 = p2->next)
      if (p2->leaf == p->leaf) Error("CheckStack: late leaf recursion");
  }
  //	Msg ("\n");
}
...
void FreeStackWinding(winding_t *w, pstack_t *stack) {
  ...
  if (stack->freewindings[i]) Error("FreeStackWinding: allready free");
...
winding_t *ClipToSeperators(winding_t *source, winding_t *pass,
  ...
  for (i = 0; i < source->numpoints; i++) {
    ...
    for (j = 0; j < pass->numpoints; j++) {
      ...
      if (length < ON_VIS_EPSILON) continue;
      ...
      for (k = 0; k < source->numpoints; k++) {
        ...
        if (d < -ON_VIS_EPSILON) {  // source is on the negative side, so we
                                    // want all
      ...
      if (!counts[0]) continue;  // planar with seperating plane
#else
      k = (j + 1) % pass->numpoints;
      d = DotProduct(pass->points[k], plane.normal) - plane.dist;
      if (d < -ON_VIS_EPSILON) continue;
...
void DumpPortalTrace(pstack_t *pStack) {
  ...
  if (g_PortalTrace.m_list.Count()) return;
...
void SimpleFlood(portal_t *srcportal, int leafnum) {
  ...
  for (i = 0; i < leaf->portals.Count(); i++) {
    p = leaf->portals[i];
    pnum = p - portals;
    if (!CheckBit(srcportal->portalfront, pnum)) continue;

    if (CheckBit(srcportal->portalflood, pnum)) continue;
...
void BasePortalVis(int iThread, int portalnum) {
  ...
  for (j = 0, tp = portals; j < g_numportals * 2; j++, tp++) {
    ...
    if (g_bUseRadius) {
      ...
      if (minDist2 > g_VisRadius) continue;
    }
...
void RecursiveLeafBitFlow(int leafnum, byte *mightsee, byte *cansee) {
  ...
  for (i = 0; i < leaf->portals.Count(); i++) {
    ...
    if (!CheckBit(mightsee, pnum)) continue;
