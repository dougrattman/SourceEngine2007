...
// Sub packet IDs.
#define VMPI_SUBPACKETID_DISCONNECT_NOTIFY \
  3  // We send ourselves this when there is a disconnect.
...
// This stuff is all for the multicast channel the master uses to send out the
// portal results.
...
bool VVIS_DispatchFn(MessageBuffer *pBuf, int iSource, int iPacketID) {
  switch (pBuf->data[1]) {
    ...
    case VMPI_SUBPACKETID_DISCONNECT_NOTIFY: {
      // This is just used to cause nonblocking dispatches to jump out so loops
      // like the one in AppBarrier can handle the fact that there are
      // disconnects.
      return true;
    }
    ...
    case VMPI_BASEPORTALVIS_RESULTS: {
      ...
      g_BasePortalVisResultsFilename.CopyArray(pFilename,
                                               strlen(pFilename) + 1);
      return true;
    }
...
CDispatchReg g_VVISDispatchReg(
    VMPI_VVIS_PACKET_ID,
    VVIS_DispatchFn);  // register to handle the messages we want
...
void VVIS_SetupMPI(int &argc, char **&argv) {
  ...
  if (!VMPI_Init(argc, argv, "dependency_info_vvis.txt", HandleMPIDisconnect,
                 mode)) {
...
void ProcessBasePortalVis(int iThread, u64 iPortal, MessageBuffer *pBuf) {
...
void ReceiveBasePortalVis(u64 iWorkUnit, MessageBuffer *pBuf, int iWorker) {
...
void RunMPIBasePortalVis() {
  ...
  if (g_bMPIMaster) StartPacifier("");

  VMPI_SetCurrentStage("RunMPIBasePortalVis");

  // Note: we're aiming for about 1500 portals in a map, so about 3000 work
  // units.
  g_CPUTime.Init();
  ...
  if (g_bMPIMaster) {
    if (!fastvis) {
      ...
      // Store all the portal results in a temp file and multicast that to the
      // workers.
      ...
      VMPI_FileSystem_CreateVirtualFile(pVirtualFilename, allPortalData.Base(),
                                        allPortalData.Count());
    ...
    // Open
    FileHandle_t fp = g_pFileSystem->Open(g_BasePortalVisResultsFilename.Base(),
                                          "rb", VMPI_VIRTUAL_FILES_PATH_ID);
    if (!fp)
      Error("Can't open '%s' to read portal info.",
            g_BasePortalVisResultsFilename.Base());
...
void ProcessPortalFlow(int iThread, u64 iPortal, MessageBuffer *pBuf) {
...
void ReceivePortalFlow(u64 iWorkUnit, MessageBuffer *pBuf, int iWorker) {
  ...
  if (p->status != stat_done) {
    ...
    if (g_pPortalMCSocket) {
      ...
      void *chunks[4] = {cPacketID, &g_PortalMCThreadUniqueID, &iWorkUnit,
                         p->portalvis};
      int chunkLengths[4] = {sizeof(cPacketID),
                             sizeof(g_PortalMCThreadUniqueID),
...
DWORD WINAPI PortalMCThreadFn(LPVOID p) {
  ...
  DWORD waitTime = 0;
  while (WaitForSingleObject(g_MCThreadExitEvent.GetEventHandle(), waitTime) !=
         WAIT_OBJECT_0) {
    CIPAddr ipFrom;
    ...
    if (len == -1) {
      ...
      if (len ==
        ...
        if (data[0] == VMPI_VVIS_PACKET_ID &&
            data[1] == VMPI_PORTALFLOW_RESULTS) {
...
// ---------------------------------------------------------------------------------
// // Cheesy hack to let them stop the job early and keep the results of what
        ...
        if (key == '1') {
          ...
          if (pPad) {
            ...
            for (int i = 0; i < g_numportals * 2; i++) {
              ...
              ScratchPad_DrawWinding(pPad, p->winding->numpoints,
                                     p->winding->points, Vector(1, 0, 0),
                                     Vector(.3, .3, .3));
            }
...
void RunMPIPortalFlow() {
  ...
  if (g_bMPIMaster) {
    ...
    g_PortalMCAddr.port =
        randomStream.RandomInt(22000, 25000);  // Pulled out of something else.
    ...
    VMPI_Send2Chunks(cPacketID, sizeof(cPacketID), &g_PortalMCAddr,
                     sizeof(g_PortalMCAddr), VMPI_PERSISTENT);
  } else {
    ...
    if (!g_hMCThread) {
      Error(
          "RunMPIPortalFlow: CreateThread failed for multicast receive "
          "thread.");
    }
  }
