...
//
//-----------------------------------------------------------------------------
static bool CHLSceneViewerApp_SuggestGameInfoDirFn(
...
bool CSceneViewerApp::Create() {
  if (CommandLine()->FindParm("-help")) {
    ...
    Msg("\n"
        ...
        " -nozoom . . . . . Stop sceneviewer zooming model viewer to occupy "
        "all client\n"
        "                   space when a dmx is specified on the command line\n"
        " -showasset  . . . Stop sceneviewer from hiding the asset builder "
        "when a dmx\n"
        ...
        " -fullscreen . . . Run application fullscreen rather than in a "
        "window\n"
        ...
        " -ref  . . . . . . Set MATERIAL_INIT_REFERENCE_RASTERIZER on "
        "adapter??\n"
        " -resizing . . . . Allow the window to be resized\n"
        " -mat_vsync  . . . Wait for VSYNC\n"
        " -mat_antialias  . Turn on Anti-Aliasing\n"
        " -mat_aaquality  . Antialiasing quality (set to zero unless you know "
        "what you're doing)\n"
        "\n");
...
bool CSceneViewerApp::PreInit() {
  ...
  if (!g_pFullFileSystem || !g_pMaterialSystem || !g_pDataModel ||
      !g_pDmElementFramework || !g_pStudioRender || !g_pDataCache ||
      !g_pMDLCache || !g_pVGuiSurface || !g_pVGui) {
    Warning(
        "CSceneViewerApp::PreInit: Unable to connect to necessary "
        "interface!\n");
    return false;
  }
...
void CSceneViewerApp::InitDefaultEnvCubemap() {
  ...
  ITexture *pCubemapTexture =
      g_pMaterialSystem->FindTexture("editor/cubemap", NULL, true);
...
int CSceneViewerApp::Main() {
  ...
  for (size_t i(CommandLine()->ParmCount() - 1); i > 0; --i) {
  ...
  while (g_pVGui->IsRunning()) {
    ...
    if ((nTime - nLastTime) < 16) continue;
    nLastTime = nTime;
  ...
  // HACK -  this is a bit of a hack, since in theory, there could be multiple
  // of these panels, or there could be elements allocated outside of these
  // panels, but since the filenames are hardcoded, I don't feel too bad about
  // unloading *all* files to make sure they're caught
