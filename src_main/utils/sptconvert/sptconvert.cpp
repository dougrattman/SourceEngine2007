...
//
// Purpose: A simple tool for converting SpeedTree .spt files into .smd files
// for use in Source.
...
void OutputTreeGeometry(CSpeedTreeRT &speedTree, CSpeedTreeRT::SGeometry &sGeom,
  ...
  char branchTextureName[SOURCE_MAX_PATH];
  ...
  for (int nStrip = 0; nStrip < sGeom.m_sFronds.m_usNumStrips; nStrip++) {
    ...
    for (int i = 0; i < nStripLength - 2; i++) {
      ...
      char frondTextureName[SOURCE_MAX_PATH];
  ...
  for (int i = 0; i < leaves.m_usLeafCount; i++) {
    ...
    char leafTextureName[SOURCE_MAX_PATH];
...
void OutputQCFile(char *treeName) {
  char smdFileName[SOURCE_MAX_PATH];
  sprintf(smdFileName, "%s.smd", treeName);

  char qcFileName[SOURCE_MAX_PATH];
  sprintf(qcFileName, "%s.qc", treeName);

  char smdAnimFileName[SOURCE_MAX_PATH];
  sprintf(smdAnimFileName, "%s_anim.smd", treeName);

  char mdlFileName[SOURCE_MAX_PATH];
