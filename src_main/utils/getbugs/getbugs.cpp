...
#define SQL_SETVALUE(fieldname, sqlcolumn)             \
  Q_strncpy(bug.m_pBug->fieldname, sqlcolumn.String(), \
            sizeof(bug.m_pBug->fieldname));
#define SQL_SETVALUESTRING(fieldname, str) \
  Q_strncpy(bug.m_pBug->fieldname, str, sizeof(bug.m_pBug->fieldname));
...
class CBugReporter {
 public:
  ...
  virtual void AddField(char const *fieldname, char const *value,
                        bool isdesc = false);
...
void CBugReporter::AddField(char const *fieldname, char const *value,
                            bool isdesc /*=false*/) {
  BugField_t fld;
...
TRK_UINT CBugReporter::Login(TRK_HANDLE *pTrkHandle, char const *projectname) {
  ...
  TRK_UINT rc =
      TrkProjectLogin(*pTrkHandle, username, password, projectname, NULL, NULL,
                      NULL, NULL, TRK_USE_INI_FILE_DBMS_LOGIN);

  if (rc != TRK_SUCCESS) {
    rc = TrkProjectLogin(*pTrkHandle, username, "", projectname, NULL, NULL,
                         NULL, NULL, TRK_USE_INI_FILE_DBMS_LOGIN);
    if (rc != TRK_SUCCESS) {
...
bool CBugReporter::CommitBugReport(int &bugSubmissionId) {
  ...
  for (int i = 0; i < fieldCount; ++i) {
    ...
    if (!fld.isdesc) {
      // Populate fields
      if (!fld.isnumeric) {
        rc = TrkSetStringFieldValue(trkRecHandle, fld.trkfield, fld.value);
      } else {
        rc = TrkSetNumericFieldValue(trkRecHandle, fld.trkfield, fld.numvalue);
      }
      if (rc != TRK_SUCCESS) {
        char es[256];
...
bool GetBugZip(int bugnum, char const *admin) {
  ...
  //	sprintf( commandline, "msdev engdll.dsw /MAKE \"quiver - Win32 GL
  // Debug\" /OUT log.txt" );
  ...
  if (!CreateProcess(NULL, commandline, NULL, NULL, TRUE, 0, NULL, directory,
                     &si, &pi)) {
    LPVOID lpMsgBuf;
    FormatMessage(
...
void GetBugInfo(CBugReporter &bug, char const *host, char const *database,
                char const *username, char const *password, char const *admin,
                int startbug, int endbug) {
  ...
  if (sql) {
    ...
    if (factory) {
      ...
      if (mysql) {
        if (mysql->InitMySQL(database, host, username, password)) {
          ...
          /*
          Q_snprintf( q, sizeof( q ), "insert into engineaccess
          (BuildIdentifier,AllowAccess) values (\"%s\",1);", argv[2] );
          ...
          */
          ...
          if (startbug != -1 && endbug != -1) {
            Q_snprintf(where, sizeof(where), "BugId>=%i and BugId<=%i and ",
                       startbug, endbug);
          }

          Q_snprintf(q, sizeof(q),
                     "select BugId, Time, BuildNumber, ExeName, GameDirectory, "
                     "MapName, Title, Description, IP,"
                     "BaseIP, RAM, CPU, ProcessorVendor, DXVersionHighPart, "
                     "DXVersionLowPart, DXVendorID, DXDeviceID, OSVersion, "
                     "BugReportFilePath, ReportType, EMail, AccountName, "
                     "SteamID, Processed, Important from bugreports where %s ( "
          ...
          if (retcode != 0) {
            ...
            if (rows && rows->NumFields() > 0) {
              while (rows->NextRow()) {
                ...
                CColumnValue GameDirectory =
                    rows->GetColumnValue("GameDirectory");
                ...
                CColumnValue ProcessorVendor =
                    rows->GetColumnValue("ProcessorVendor");
                CColumnValue DXVersionHighPart =
                    rows->GetColumnValue("DXVersionHighPart");
                CColumnValue DXVersionLowPart =
                    rows->GetColumnValue("DXVersionLowPart");
                ...
                CColumnValue BugReportFilePath =
                    rows->GetColumnValue("BugReportFilePath");
                ...
                Q_snprintf(dxversion, sizeof(dxversion), "%u.%u.%u.%u",
                           (vhigh >> 16), vhigh & 0xffff, (vlow >> 16),
                           vlow & 0xffff);
                ...
                char basepath[512];
                Q_FileBase(BugReportFilePath.String(), basepath,
                           sizeof(basepath));

                char desc[8192];

                if (BugReportFilePath.String()[0]) {
                  Q_snprintf(zipurl, sizeof(zipurl), BUG_REPOSITORY_FMT,
                             database, "BugId", basepath);
                ...
                if (success) {
                  ...
                  if (zipurl[0]) {
                    ...
                    Q_snprintf(zipurlfixed, sizeof(zipurlfixed),
                               BUG_REPOSITORY_FMT, database, id, basepath);
                    ...
                    Q_snprintf(zipurllocal, sizeof(zipurllocal), "%s\\%s.zip",
                               admindir, basepath);
              ...
              for (int i = 0; i < c; ++i) {
                Q_snprintf(q, sizeof(q),
                           "update bugreports set Processed=1 where BugId=%i;",
                           processed[i]);
      ...
      vprint(0,
             "Unable to get factory from mysql_wrapper.dll, not updating "
             "access mysql table!!!");
    }
    ...
    vprint(
        0,
        "Unable to load mysql_wrapper.dll, not updating access mysql table!!!");
  }
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 9) {
    GetBugInfo(bugreporter, argv[2], argv[3], argv[4], argv[5], argv[6],
               atoi(argv[7]), atoi(argv[8]));
  } else {
    GetBugInfo(bugreporter, argv[2], argv[3], argv[4], argv[5], argv[6], -1,
               -1);
  }
