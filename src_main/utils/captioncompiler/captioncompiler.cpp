...
//
// Purpose: Defines the entry point for the console
...
void CCompileCaptionsApp::CompileCaptionFile(char const *infile,
                                             char const *outfile) {
  ...
  for (StringIndex_t i = g_pVGuiLocalize->GetFirstStringIndex();
       i != INVALID_STRING_INDEX;
    ...
    if (len > blockSize) {
      Error(
          "Caption text file '%s' contains a single caption '%s' of %d bytes "
          "(%d is max), change MAX_BLOCK_SIZE in captioncompiler.h to fix!!!\n",
  ...
  if (maxindex != INVALID_STRING_INDEX) {
    vprint(0, "Longest string '%s' = (%i) bytes average(%.3f)\n%",
           ...
           (float)totalsize / (float)c);
  }

  vprint(0,
         "%d blocks (%d bytes each), %d bytes wasted (%.3f per block average), "
         "total bytes %d\n",
         curblock + 1, blockSize, freeSpace,
         (float)freeSpace / (float)(curblock + 1), totalsize);
...
void CCompileCaptionsApp::DescribeCaptions(char const *file) {
  ...
  directory.CopyArray((const CaptionLookup_t *)buf.PeekGet(),
  ...
  for (StringIndex_t idx = g_pVGuiLocalize->GetFirstStringIndex();
       idx != INVALID_STRING_INDEX;
...
int CCompileCaptionsApp::Main() {
  ...
  if (Q_stristr(worklist[worklist.Count() - 1].String(), gamedir)) {
    Q_strncpy(outfile,
              &worklist[worklist.Count() - 1].String()[Q_strlen(gamedir)],
              sizeof(outfile));
  } else {
    Q_snprintf(outfile, sizeof(outfile), "resource\\%s",
               worklist[worklist.Count() - 1].String());
  }
