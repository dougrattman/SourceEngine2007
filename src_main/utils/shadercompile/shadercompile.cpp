...
static inline uint32_t uint64_as_uint32(uint64_t x) {
  Assert(x < uint64_t(uint32_t(~0)));
  return uint32_t(x);
}
...
CfgProcessor::CfgEntryInfo const *GetEntryByStaticComboNum(
    uint64_t nStaticCombo, uint64_t *pnStaticCombo) {
...
void Shader_ParseShaderInfoFromCompileCommands(
    CfgProcessor::CfgEntryInfo const *pEntry, ShaderInfo_t &shaderInfo);
...
struct CStaticCombo  // all the data for one static combo
{
  ...
  uint64_t Key(void) const { return m_nStaticComboID; }

  CStaticCombo(uint64_t nComboID) { m_nStaticComboID = nComboID; }

  ~CStaticCombo(void) { m_DynamicCombos.PurgeAndDeleteElements(); }
...
// times  the  message was encountered
//			1 byte = \n			= newline delimiting the
// number
//
...
static void FlushCombos(size_t *pnTotalFlushedSize,
  ...
  uint8_t *pCompressedShader =
      LZMA_Compress(reinterpret_cast<uint8_t *>(pDynamicComboBuffer->Base()),
...
static void OutputDynamicCombo(size_t *pnTotalFlushedSize,
                               ...
                               int nComboSize, uint8_t *pComboCode) {
...
static void WriteShaderFiles(const char *pShaderName) {
  ...
  {
    StaticComboNodeHash_t *&rp =
        g_ShaderByteCode[pShaderName];  // Get a static combo pointer, reset it
                                        // as well
    pByteCodeArray = rp;
    rp = NULL;
  ...
  StaticComboHeaders.EnsureCapacity(
      1 + pByteCodeArray->Count());  // we know how much ram we need
    ...
    for (CStaticCombo *pStatic = pByteCodeArray->m_HashChains[nChain].m_pHead;
      ...
      if (pStatic->m_abPackedCode.GetLength()) {
        ...
        for (int i = 0; i < comboIndicesHashedByCRC32[nHashIdx].Count(); i++) {
          StaticComboAuxInfo_t const &check =
              StaticComboHeaders[comboIndicesHashedByCRC32[nHashIdx][i]];
  ...
  //
  CUtlStreamBuffer ShaderFile(
      szVCSfilename,
      NULL);  // Streaming buffer for vcs file (since this can blow memory)
  ...
  ShaderFile.Put(
      StaticComboHeaders.Base(),
      sizeof(StaticComboRecord_t) *
          StaticComboHeaders.Count());  // dummy write, 8 bytes per static combo
  ...
  //
  {
    ...
    if (!Handle) printf(" failed to re-open %s\n", szVCSfilename);
...
void Master_ReceiveWorkUnitFn(uint64_t iWorkUnit, MessageBuffer *pBuf,
  ...
  for (uint64_t iCombo = comboStart; iCombo++ < comboEnd;
    ...
    if (len <= 0) {
      ...
      for (uint64_t numSkips = -len - 1; numSkips > 0;) {
    ...
    // Read buffer
    uint8_t *pCodeBuffer =
        StaticComboFromDictAdd(chShaderName, nComboOfTheEntry)
...
void MySystem(char const *const pCommand, CmdSink::IResponse **ppResponse) {
  ...
  char process_name[9];
  strcpy_s(process_name, "temp.bat");
...
size_t AssembleWorkerReplyPackage(CfgProcessor::CfgEntryInfo const *pEntry,
  ...
  if (fabs(fCurTime - s_fLastInfoTime) > 1.f) {
    Msg("\rCompiling  %s  [ %2d remaining ] ...         \r", pEntry->m_szName,
        nComboOfEntry);
...
size_t CopyWorkerReplyPackage(CfgProcessor::CfgEntryInfo const *pEntry,
                              uint64_t nComboOfEntry, MessageBuffer *pBuf,
                              int nSkipsSoFar) {
  GLOBAL_DATA_MTX_LOCK();
  CStaticCombo *pStComboRec =
...
class CWorkerAccumState
  ...
  friend CParallelProcessorBase<
      CWorkerAccumState<TMutexType>>::ThisParallelProcessorBase_t;
 ...
 public:
  ...
  void HandleCommandResponse(CfgProcessor::ComboHandle hCombo,
                             CmdSink::IResponse *pResponse);

 public:
  using CParallelProcessorBase<
      CWorkerAccumState<TMutexType>>::ThisParallelProcessorBase_t::Run;
...
void CWorkerAccumState<TMutexType>::PrepareSubProcess(
    SubProcess **ppSp, SubProcessKernelObjects **ppCommObjs) {
  ...
  if (pSp) {
    ...
    sprintf(chBaseNameBuffer, "SHCMPL_SUB_%08X_%08llX_%08X", pSp->dwSvcThreadId,
            time(NULL), GetCurrentProcessId());
    pCommObjs = pSp->pCommObjs =
        new SubProcessKernelObjects_Create(chBaseNameBuffer);
    ...
    sprintf(chCommandLine, "\"%s\\shadercompile.exe\" -subprocess %s",
            g_WorkerTempPath, chBaseNameBuffer);
#ifdef _DEBUG
    V_strncat(chCommandLine, " -allowdebug", sizeof(chCommandLine));
#endif
    BOOL bCreateResult =
        CreateProcess(NULL, chCommandLine, NULL, NULL, FALSE, CREATE_NO_WINDOW,
                      NULL, g_WorkerTempPath, &si, &pSp->pi);
    (void)bCreateResult;
...
void CWorkerAccumState<TMutexType>::ExecuteCompileCommandThreaded(
    CfgProcessor::ComboHandle hCombo) {
...
void CWorkerAccumState<TMutexType>::HandleCommandResponse(
    CfgProcessor::ComboHandle hCombo, CmdSink::IResponse *pResponse) {
  ...
  if ((!g_bSuppressWarnings && szListing) || !pResponse->Succeeded()) {
    ...
    if (!szListing) {
      sprintf(chUnreportedListing,
              "(%s): error 0000: Compiler failed without error description, "
...
void CWorkerAccumState<TMutexType>::TryToPackageData(uint64_t iCommandNumber) {
  ...
  CfgProcessor::ComboHandle hChBegin =
      CfgProcessor::Combo_GetCombo(iLastFinished);
  CfgProcessor::ComboHandle hChEnd =
      CfgProcessor::Combo_GetCombo(iFinishedByNow);
  ...
  for (; pInfoBegin &&
    ...
    if (!nComboBegin--) {
      Combo_Free(hChBegin);
      if ((hChBegin = CfgProcessor::Combo_GetCombo(
...
void Worker_ProcessCommandRange_Singleton::ProcessCommandRange(
    uint64_t shaderStart, uint64_t shaderEnd) {
  if (m_MT.pThreadPool) {
...
void Worker_ProcessWorkUnitFn(int iThread, uint64_t iWorkUnit,
  ...
  // Failed shaders
  for (int k = 0, kEnd = g_Master_ShaderHadError.GetNumStrings(); k < kEnd;
       ++k) {
  ...
  // Compiler spew
  for (int k = 0, kEnd = g_Master_CompilerMsgInfo.GetNumStrings(); k < kEnd;
       ++k) {
...
void Shader_ParseShaderInfoFromCompileCommands(
    CfgProcessor::CfgEntryInfo const *pEntry, ShaderInfo_t &shaderInfo) {
  if (CfgProcessor::ComboHandle hCombo =
          CfgProcessor::Combo_GetCombo(pEntry->m_iCommandStart)) {
    char cmd[4096];
...

    {
      ...
      sscanf(pCentroidMask + strlen("/DCENTROIDMASK="), "%u",
...
void Worker_GetLocalCopyOfBinaries(void) {
  Worker_GetLocalCopyOfBinary("mysql_wrapper.dll");  // This is necessary so
                                                     // VMPI doesn't run in SDK
                                                     // mode.
...
void Shared_ParseListOfCompileCommands(void) {
  ...
  Msg("\rCompiling %s commands.         \r",
      PrettyPrintNumber(g_numCompileCommands), (tt_end - tt_start));
}
...
class CDistributeShaderCompileMaster : public IWorkUnitDistributorCallbacks {
 ...
 private:
  ...
  friend DWORD WINAPI ThreadProcAdapter(LPVOID pvArg);
...
void CDistributeShaderCompileMaster::OnWorkUnitsCompleted(
  ...
  CfgProcessor::CfgEntryInfo const *pEntry = GetEntryByStaticComboNum(
      g_numCompletedStaticCombos, &numStaticCombosOfTheEntry);
...
int ShaderCompile_Main(int argc, const char *argv[]) {
  ...
  SetupDebugFile();
  numthreads = 1;  // managed specifically in
  ...
  */
  ...
  if (g_bMPIMaster || !bShouldUseVMPI) {
    ...
    bool bValveVerboseComboErrors = (getenv("VALVE_VERBOSE_COMBO_ERRORS") &&
                                     atoi(getenv("VALVE_VERBOSE_COMBO_ERRORS")))
                                        ? true
                                        : false;
    ...
    for (int k = 0, kEnd = g_Master_CompilerMsgInfo.GetNumStrings(); k < kEnd;
      ...
      uint64_t iFirstCommand = _strtoui64(szFirstCmd, NULL, 10);
      CfgProcessor::ComboHandle hCombo = NULL;
      CfgProcessor::CfgEntryInfo const *pComboEntryInfo = NULL;
      if (CfgProcessor::Combo_GetNext(iFirstCommand, hCombo,
                                      g_numCompileCommands)) {
      ...
      if (bValveVerboseComboErrors) {
        ...
        while (pBegin && *pBegin && !isspace(*pBegin)) ++pBegin;
        while (pBegin && *pBegin && isspace(*pBegin)) ++pBegin;
        ...
        while (pBegin && *pBegin && (pBegin < pEnd)) {
          ...
          Msg("                  %.*s %.*s\n", (pSpace - pEqSign - 1),
              pEqSign + 1, (pEqSign - pDefine - 2), pDefine + 2);
        }
      }
      Msg("    %s\n", str);
    }
...
EXPOSE_SINGLE_INTERFACE(CShaderCompileDLL, IShaderCompileDLL,
