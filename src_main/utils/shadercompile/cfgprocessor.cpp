...
class QuickStrIdx : private std::unordered_map<std::string, int> {
 public:
  void Append(char const *szName, int idx) {
    this->insert(value_type(szName, idx));
  };
  int Size(void) const { return (int)this->size(); };
  int Get(char const *szName) const {
    const_iterator it = this->find(szName);
    if (end() != it)
      return it->second;
    else
      return -1;
...
#define BEGIN_EXPR_UNARY(className)     \
  class className : public CExprUnary { \
   public:                              \
    className(IExpression *x) : CExprUnary(x) {}
...
#define EXPR_BINARY_PRIORITY(nPriority) \
  virtual int Priority() const { return nPriority; };
...
IExpression *CComplexExpression::ParseInternal(char *&szExpression) {
  ...
  parsed_variable_name:
    int nSlot = m_pContext->GetVariableSlot(
...
class ComboGenerator : public IEvaluationContext {
 public:
  ...
  Define const *const GetDefinesEnd(void) {
 ...
 public:
  ...
  virtual char const *GetVariableName(int nSlot) {
    return m_arrDefines.Get(nSlot).Name();
  };
  virtual int GetVariableSlot(char const *szVariableName) {
...
void ReadConfiguration(FILE *fInputStream) {
  CAutoPushPop<FILE *> pushInputStream(
      ConfigurationProcessing::GetInputStream(fInputStream), fInputStream);
  ConfigurationProcessing::ProcessConfiguration();
}
...
void DescribeConfiguration(CArrayAutoPtr<CfgEntryInfo> &rarrEntries) {
  rarrEntries.Delete();
  rarrEntries.Attach(
      new CfgEntryInfo[ConfigurationProcessing::s_setEntries.size() + 1]);

  CfgEntryInfo *pInfo = rarrEntries.Get();
  uint64_t nCurrentCommand = 0;

  for (std::multiset<ConfigurationProcessing::CfgEntry>::reverse_iterator
           it = ConfigurationProcessing::s_setEntries.rbegin(),
...
ComboHandle Combo_GetCombo(uint64_t iCommandNumber) {
  ...
  CPCHI_t const &chiFound =
      ConfigurationProcessing::s_mapComboCommands.GetLessOrEq(iCommandFound,
                                                              emptyCPCHI);
...
ComboHandle Combo_GetNext(uint64_t &riCommandNumber, ComboHandle &rhCombo,
  ...
  for (;;) {
    ...
    CPCHI_t const &chiNext =
        ConfigurationProcessing::s_mapComboCommands.GetLessOrEq(iCommandLookup,
                                                                emptyCPCHI);
