...
SubProcessKernelObjects::SubProcessKernelObjects(void)
    : m_hMemorySection(NULL), m_hMutex(NULL) {
...
BOOL SubProcessKernelObjects::Create(char const *szBaseName) {
  ...
  m_hMemorySection =
      CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0,
                        4 * 1024 * 1024, chBufferName);  // 4Mb for a piece
  ...
  for (int k = 0; k < 2; ++k) {
    ...
    m_hEvent[k] = CreateEvent(NULL, FALSE, (k ? TRUE /* = master */ : FALSE),
                              chBufferName);
  }
      ...
      case WAIT_OBJECT_0: {
        ...
        if (*(const DWORD *)m_pLockData != m_pObjs->m_dwCookie) {
          ...
          *(DWORD *)m_pLockData = m_pObjs->m_dwCookie;
          m_pMemory = ((uint8_t *)m_pLockData) + 2 * sizeof(DWORD);
...
// length, 0 if failed
//				<bytes>		-	result buffer data, none
// if  result buffer length is 0
//				string		-	zero-terminated listing
// string
//
//////////////////////////////////////////////////////////////////////////
...
CSubProcessResponse::CSubProcessResponse(void const *pvMemory)
    : m_pvMemory(pvMemory) {
  uint8_t const *pBytes = (uint8_t const *)pvMemory;
...
void ShaderCompile_Subprocess_ExceptionHandler(unsigned long exceptionCode,
...
int ShaderCompile_Subprocess_Main(char const *szSubProcessData) {
  ...
  for (void *pvMemory = NULL; NULL != (pvMemory = shrmem.Lock());
       shrmem.Unlock()) {
    ...
    if (InterceptFxc::TryExecuteCommand(szCommand, &pResponse)) {
      uint8_t *pBytes = (uint8_t *)pvMemory;
