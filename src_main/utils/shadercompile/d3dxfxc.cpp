...
//		TODO: avoid writing "shader.o" and "output.txt" files to avoid
...
//
void FastShaderCompile(const char *pszFilename, const D3DXMACRO *pMacros,
                       const char *pszModel, CmdSink::IResponse **ppResponse) {
  ...
  HRESULT hr = s_dxModule.D3DXCompileShaderFromFile(
      pszFilename, pMacros, NULL /* LPD3DXINCLUDE */, "main", pszModel, 0,
      &pShader, &pErrorMessages,
...
/// DSHADER_MODEL_VS_2_0=1 /D_X360=1 /nologo /Foshader.o
...
void ExecuteCommand(const char *pCommand, CmdSink::IResponse **ppResponse) {
  ...
  for (char *pszFlag = pEditableCommand;
    ...
    const char *pszFlagName = pszFlag + 2;  // 2 = length of "/D"
  ...
  if (const char *pchCmdRedirect = strstr(pCommand, ">output.txt ")) {
    ...
    while (pszFilename > pEditableCommand && pszFilename[-1] &&
           ' ' != pszFilename[-1]) {
      --pszFilename;
    }
  }
  ...
  // Compile the stuff
  Private::FastShaderCompile(pszFilename, macros.Base(), chShaderModel,
                             ppResponse);
}
...
bool TryExecuteCommand(const char *pCommand, CmdSink::IResponse **ppResponse) {
  {
    ...
    static int s_dummy =
        (Msg(s_bNoIntercept ? "[shadercompile] Using old slow technique - runs "
                              "'fxc.exe'.\n"
                            : "[shadercompile] Using new faster Vitaliy's "
                              "implementation.\n"),
         1);
    if (!s_bNoIntercept && !strncmp(pCommand, InterceptFxc::s_pszCommand,
                                    InterceptFxc::s_uCommandLen)) {
