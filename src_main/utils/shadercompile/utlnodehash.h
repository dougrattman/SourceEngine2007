...
class CUtlNodeHash {
 ...
 public:
  ...
  void Purge(void) {
    m_nNumNodes = 0;
    // delete all nodes
    for (int i = 0; i < HASHSIZE; i++) m_HashChains[i].Purge();
  }

  int Count(void) const { return m_nNumNodes; }
