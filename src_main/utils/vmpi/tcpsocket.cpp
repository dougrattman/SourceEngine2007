...
#error \
    "I am TCPSocket and I suck. Use IThreadedTCPSocket or ThreadedTCPSocketEmu instead."
...
class COverlappedPlus : public WSAOVERLAPPED {
 public:
  COverlappedPlus() { memset(this, 0, sizeof(WSAOVERLAPPED)); }
...
// AddGlobalTCPSocket shouldn't be called until you're ready for SendKeepalive()
// to be called.
class CTCPSocket;
...
class CTCPSocket : public ITCPSocket {
 ...
 public:
  ...
  void Term() {
    ...
    if (m_Socket != SOCKET_ERROR && !m_bConnectionLost) {
      ...
      // Give the sends a second to complete. SO_LINGER is having trouble for
      // some reason.
  ...
  bool CheckConnectionLost() {
    ...
    // Have we timed out?
    if (g_bEnableTCPTimeout &&
        (Plat_FloatTime() - m_LastRecvTime > KEEPALIVE_TIMEOUT_SECONDS)) {
  ...
  // Called by any thread. All interface functions call CheckConnectionLost()
  // and return errors if it's lost.
  void SetConnectionLost(const char *pErrorString, int err = -1) {
    ...
    // Handle it right away if we're in the main thread. If we're in an IO
    // thread, it has to wait until the next interface function calls
    // CheckConnectionLost().
  ...
  virtual bool StartWaitingForSize(bool bFresh) {
    ...
    m_RecvStage = 0;
    m_RecvDataSize = -1;
    if (bFresh) m_nSizeBytesReceived = 0;

    DWORD dwNumBytesReceived = 0;
    WSABUF buf = {sizeof(&m_RecvDataSize) - m_nSizeBytesReceived,
                  ((char *)&m_RecvDataSize) + m_nSizeBytesReceived};
    DWORD dwFlags = 0;

    int status = WSARecv(m_Socket, &buf, 1, &dwNumBytesReceived, &dwFlags,
                         &m_RecvOverlapped, NULL);

    int err = -1;
    if (status == SOCKET_ERROR &&
        (err = WSAGetLastError()) != ERROR_IO_PENDING) {
  ...
  bool PostNextDataPart() {
    DWORD dwNumBytesReceived = 0;
    WSABUF buf = {m_RecvDataSize - m_AmountReceived,
                  (char *)m_pIncomingData->m_Data + m_AmountReceived};
    DWORD dwFlags = 0;
  ...
  bool StartWaitingForData() {
    ...
    ParanoidMemoryCheck();
    m_pIncomingData =
        (CRecvData *)malloc(sizeof(CRecvData) - 1 + m_RecvDataSize);
    if (!m_pIncomingData) {
      char str[512];
  ...
  virtual bool Recv(CUtlVector<unsigned char> &data, double flTimeout) {
    ...
    // Set the event again for the next time around, if there is more data
    // waiting.
  ...
  void HandleRecvCompletion(COverlappedPlus *pInfo, DWORD dwNumBytes) {
    ...
    if (m_RecvStage == 0) {
      ...
      if (m_nSizeBytesReceived == sizeof(m_RecvDataSize)) {
        ...
        // Handle the case where we only got some of the data (maybe one of the
      ...
      Assert(m_AmountReceived <= m_RecvDataSize);  // TODO: make this threadsafe
                                                   // for multiple IO threads.
#endif
  ...
  void SendDisconnectSentinel() {
    ...
    if (pBuf) {
      pBuf->m_DataLength = sizeof(int);
      *((int *)pBuf->m_Data) =
          SENTINEL_DISCONNECT;  // This signifies that we're exiting.
      InternalSendDataBuf(pBuf);
    }
  }
  ...
  virtual bool SendChunks(void const *const *pChunks, const int *pChunkLengths,
    ...
    ParanoidMemoryCheck();
    SendBuf_t *pBuf =
        (SendBuf_t *)malloc(sizeof(SendBuf_t) - 1 + totalLength + sizeof(int));
    if (!pBuf) {
      char str[512];
      _snprintf(str, sizeof(str),
                "malloc() in SendChunks() failed. totalLength = %d.",
                totalLength);
      SetConnectionLost(str);
      return false;
    }
  ...
  int InternalSendDataBuf(SendBuf_t *pBuf) {
    ...
    return WSASend(m_Socket, &buf, 1, &dwNumBytesSent, 0, &pBuf->m_Overlapped,
                   NULL);
  }
  ...
  bool CreateThreads() {
    ...
    m_hIOCP = CreateIoCompletionPort((HANDLE)m_Socket, m_hIOCP,
                                     (unsigned long)this, nThreads);

    for (int i = 0; i < nThreads; i++) {
      DWORD dwThreadID = 0;
      HANDLE hThread = CreateThread(NULL, 0, &CTCPSocket::StaticThreadFn, this,
                                    0, &dwThreadID);
  ...
  DWORD ThreadFn() {
    while (1) {
      ...
      if (GetQueuedCompletionStatus(
              ...
              &pOverlapped,      // the overlapped info that was passed into
                                 // AcceptEx, WSARecv, or WSASend.
              100  // listen for 100ms at a time so we can exit gracefully when
                   // the socket is deleted.
              )) {
 ...
 private:
  ...
  CUtlLinkedList<CRecvData *, int>
      m_RecvDatas;  // The head element is the next one to be picked up.
  ...
  // These reference the element at the tail of m_RecvData. It is the current
  // one getting
  volatile int
      m_nSizeBytesReceived;  // How much of m_RecvDataSize have we received yet?
  int m_RecvDataSize;        // this is received over the network
  int m_AmountReceived;      // How much we've received so far.

  // Last time we received anything from this connection. Used to determine if
...
DWORD WINAPI TCPKeepaliveThread(LPVOID pParameter) {
  while (1) {
    if (WaitForSingleObject(g_hKeepaliveThreadSignal, KEEPALIVE_INTERVAL_MS) ==
        WAIT_OBJECT_0)
      break;
...
void AddGlobalTCPSocket(CTCPSocket *pSocket) {
  ...
  if (g_TCPSockets.Count() == 1) {
    ...
    DWORD dwThreadID = 0;
    g_hKeepaliveThread =
        CreateThread(NULL, 0, TCPKeepaliveThread, NULL, 0, &dwThreadID);
  }
}
...
void RemoveGlobalTCPSocket(CTCPSocket *pSocket) {
  ...
  DWORD dwExitCode = 0;
  if (GetExitCodeThread(g_hKeepaliveThread, &dwExitCode) &&
