...
#define SERVICE_OFF_TIMEOUT \
  (20 * 1000)  // If we haven't heard from a service in this long,
               // then we assume the service is off.

#define SERVICES_PING_INTERVAL (3 * 1000)  // ping the services every so often

#define SERVICE_MAX_UPDATE_INTERVAL \
  (8 * 1000)  // Update each service in the listbox at least this often.
...
// ---------------------------------------------------------------------------------------------------------
// //
typedef int(CALLBACK *ServicesSortFn)(LPARAM iItem1, LPARAM iItem2,
                                      LPARAM lpParam);
...
static int CALLBACK SortByRunningTime(LPARAM iItem1, LPARAM iItem2,
                                      LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
  ...
  int v1 =
      pInfo1->m_LiveTimeMS /
      (1000 * 60);  // Sort on minutes so it doesn't constantly resort the list.
...
static int CALLBACK SortByWorkerAppRunningTime(LPARAM iItem1, LPARAM iItem2,
                                               LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;

  int v1 =
      pInfo1->m_WorkerAppTimeMS /
      (1000 * 60);  // Sort on minutes so it doesn't constantly resort the list.
...
static int CALLBACK SortByMasterName(LPARAM iItem1, LPARAM iItem2,
                                     LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
...
static int CALLBACK SortByProtocol(LPARAM iItem1, LPARAM iItem2,
                                   LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
...
static int CALLBACK SortByPassword(LPARAM iItem1, LPARAM iItem2,
                                   LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
...
static int CALLBACK SortByServiceVersion(LPARAM iItem1, LPARAM iItem2,
                                         LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
...
static int CALLBACK SortByCPUPercentage(LPARAM iItem1, LPARAM iItem2,
                                        LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
...
static int CALLBACK SortByMemUsage(LPARAM iItem1, LPARAM iItem2,
                                   LPARAM lpParam) {
  CServiceInfo *pInfo1 = (CServiceInfo *)iItem1;
  CServiceInfo *pInfo2 = (CServiceInfo *)iItem2;
...
static void PushSortColumn(int iColumn) {
  ...
  if (iPrev != g_SortColumns.InvalidIndex()) g_SortColumns.Remove(iPrev);
...
BOOL CServicesDlg::OnInitDialog() {
  ...
  // Setup the headers.
  for (int i = 0; i < std::size(g_ColumnInfos); i++) {
    m_ServicesList.InsertColumn(i, g_ColumnInfos[i].pText, LVCFMT_LEFT,
                                g_ColumnInfos[i].width, i);
  }
  ...
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_SERVICES_LABEL), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_SERVICES), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_DISABLED_SERVICES_LABEL),
                        ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_DISABLED_SERVICES),
                        ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WORKING_SERVICES_LABEL),
                        ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WORKING_SERVICES), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WAITING_SERVICES_LABEL),
                        ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_WAITING_SERVICES), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_OFF_SERVICES_LABEL),
                        ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_NUM_OFF_SERVICES), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_CURRENT_PASSWORD_LABEL),
                        ANCHOR_LEFT, ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_CURRENT_PASSWORD), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_PATCH_SERVICES), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_STOP_SERVICES), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_STOP_JOBS), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_FORCE_PASSWORD), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);
  m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_FILTER_BY_PASSWORD), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(ID_COPY_TO_CLIPBOARD), ANCHOR_LEFT,
                        ANCHOR_BOTTOM, ANCHOR_LEFT, ANCHOR_BOTTOM);

  m_AnchorMgr.AddAnchor(this, GetDlgItem(IDC_SERVICES_LIST), ANCHOR_LEFT,
                        ANCHOR_TOP, ANCHOR_RIGHT, ANCHOR_BOTTOM);

  // Unless they specify admin mode, hide all the controls that can mess with
  // the services.
  if (!FindArg("-Admin")) {
...
void CServicesDlg::BuildVMPIPingPacket(CUtlVector<char> &out, char cPacketID,
                                       ...
                                       bool bIgnorePassword) {
  out.Purge();
  out.AddToTail(protocolVersion);

  const char *pPassword = m_Password;
  if (pPassword[0] == 0 || bIgnorePassword) {
    // If they haven't set a password to filter by, then we want all services to
    // report in.
...
void CServicesDlg::UpdateServicesFromNetMessages() {
  while (1) {
    ...
    if (protocolVersion == 4 ||
        protocolVersion ==
            VMPI_PROTOCOL_VERSION)  // Protocol version 4 is almost the same.
    {
      ...
      if (packetID == VMPI_PING_RESPONSE) {
        ...
        if (buf.GetNumBitsLeft()) workerAppTimeMS = buf.ReadLong();
        ...
        int cpuPercentage = -1;
        if (buf.GetNumBytesLeft() >= 1) cpuPercentage = buf.ReadByte();
        ...
        short memUsage = -1;
        if (buf.GetNumBytesLeft() >= 2) memUsage = buf.ReadShort();
        ...
        if (!pInfo) {
          ...
          int iItem = m_ServicesList.InsertItem(COLUMN_COMPUTER_NAME,
                                                pInfo->m_ComputerName, NULL);
        ...
        V_strncpy(pInfo->m_ServiceVersion, serviceVersion,
                  sizeof(pInfo->m_ServiceVersion));
        pInfo->m_Addr = ipFrom;
...
void CServicesDlg::UpdateServicesFromNetView() {
  ...
  for (int i = 0; i < computerNames.Count(); i++) {
    ...
    if (!pInfo) {
      ...
      pInfo->m_LastPingTimeMS =
          Plat_MSTime() - SERVICE_OFF_TIMEOUT * 2;  // so it's marked as "off"
...
void CServicesDlg::OnIdle() {
  ...
  if (!m_pServicesPingSocket) return;
  ...
  if (curTime - m_dwLastServicesPing >= SERVICES_PING_INTERVAL) {
    ...
    for (int i = VMPI_SERVICE_PORT; i <= VMPI_LAST_SERVICE_PORT; i++) {
      ...
      // Also send out a version 4 one because we understand a version 4
      // response.
  ...
  // Issue a "net view" command, parse the output, and add any computers it
  // lists. This lets us figure out which PCs on the network are not running the
  // service.
...
CServiceInfo *CServicesDlg::FindServiceByComputerName(
    const char *pComputerName) {
...
void UpdateItemText(CListCtrl &ctrl, int iItem, int iColumn,
                    const char *pNewVal) {
  CString str = ctrl.GetItemText(iItem, iColumn);
  if (V_stricmp(str, pNewVal) != 0) ctrl.SetItemText(iItem, iColumn, pNewVal);
}
...
void CServicesDlg::UpdateServiceInListbox(CServiceInfo *pInfo) {
  ...
  if (iItem != -1) {
    UpdateItemText(m_ServicesList, iItem, COLUMN_COMPUTER_NAME,
    ...
    UpdateItemText(m_ServicesList, iItem, COLUMN_WORKER_APP_RUNNING_TIME,
                   timeStr);

    UpdateItemText(m_ServicesList, iItem, COLUMN_MASTER_NAME,
                   pInfo->m_MasterName);
    ...
    UpdateItemText(m_ServicesList, iItem, COLUMN_SERVICE_VERSION,
                   pInfo->m_ServiceVersion);
...
void CServicesDlg::UpdateServiceCountDisplay() {
  ...
  FOR_EACH_LL(m_Services, i) {
    if (m_Services[i]->IsOff()) {
      ++nOff;
    } else if (m_Services[i]->m_iState == VMPI_STATE_BUSY ||
               m_Services[i]->m_iState == VMPI_STATE_DOWNLOADING) {
      ++nWorking;
...
// This monstrosity is here because of the way they bundle string resources into
// groups in an exe file. See http://support.microsoft.com/kb/q196774/.
bool FindStringResourceEx(HINSTANCE hinst, UINT uId, UINT langId, char *pStr,
                          int outLen) {
...
int CheckServiceVersion(const char *pPatchDir, char *pServiceVersion,
                        int maxServiceVersionLen) {
  ...
  if (hInst) {
    bool bFound =
        FindStringResourceEx(hInst, VMPI_SERVICE_IDS_VERSION_STRING,
                             MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL),
                             pServiceVersion, maxServiceVersionLen);
    if (bFound) {
      ret = V_AfxMessageBox(MB_YESNOCANCEL,
                            "Service version in %s is %s.\n\nIs this correct?",
                            filename, pServiceVersion);
    } else {
      V_AfxMessageBox(MB_OK, "Can't get IDS_VERSION_STRING resource from %s.",
                      filename);
    }
...
TryAgain:;
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    V_ComposeFileName(dlg.m_VMPITransferDirectory, "vmpi_transfer.exe",
                      transferExe, sizeof(transferExe));
    ...
    int ret = CheckServiceVersion(dlg.m_PatchDirectory, strServiceVersion,
                                  sizeof(strServiceVersion));
    if (ret == IDCANCEL)
      return;
    else if (ret == IDNO)
      goto TryAgain;

    AppendStr(commandLine, sizeof(commandLine), "\"%s\" -PatchHost",
              transferExe);
    AppendStr(commandLine, sizeof(commandLine), " -mpi_PatchVersion %s",
              strServiceVersion);
    AppendStr(commandLine, sizeof(commandLine), " -mpi_PatchDirectory \"%s\"",
    ...
    AppendStr(commandLine, sizeof(commandLine), " -mpi_PatchWorkers %d",
              addrs.Count());
    for (int i = 0; i < addrs.Count(); i++) {
      AppendStr(commandLine, sizeof(commandLine), " %d.%d.%d.%d",
                addrs[i].ip[0], addrs[i].ip[1], addrs[i].ip[2], addrs[i].ip[3]);
    }
    ...
    if (CreateProcess(NULL, commandLine, NULL, NULL, false, 0, NULL,
                      (const char *)dlg.m_PatchDirectory, &si, &pi)) {
      CloseHandle(pi.hProcess);
      CloseHandle(pi.hThread);

      V_AfxMessageBox(MB_OK,
                      "Patch master successfully started.\nServices patching "
                      "now.\nClose the patch master console app when "
                      "finished.");
    } else {
      V_AfxMessageBox(MB_OK, "Error starting patch master: %s",
                      GetLastErrorString());
    }
  }
}
...
void CServicesDlg::OnStopServices() {
  if (MessageBox("Warning: if you stop these services, you won't be able to "
                 "control them from this application, and must restart them "
                 "manually. Contine?",
...
void CServicesDlg::OnForcePassword() {
  ...
  if (dlg.DoModal() == IDOK) {
    ...
    BuildVMPIPingPacket(data, VMPI_FORCE_PASSWORD_CHANGE, VMPI_PROTOCOL_VERSION,
                        true);
  ...
  if (pos) {
    ...
    if (iItem != -1) {
      ...
      if (pInfo) {
        ...
        char cmdLine[1024];
        Q_snprintf(cmdLine, sizeof(cmdLine), "vmpi_job_search -SelectWorker %s",
                   pInfo->m_ComputerName);
        ...
        if (!CreateProcess(NULL, (char *)(const char *)cmdLine,
                           NULL,  // security
                           NULL, TRUE,
                           0,     // flags
                           NULL,  // environment
...
BOOL CServicesDlg::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT *pResult) {
  ...
  if (pHdr->idFrom == IDC_SERVICES_LIST) {
    if (pHdr->code == LVN_COLUMNCLICK) {
      ...
      int iSortColumn = std::max(
          0, std::min(pListView->iSubItem, std::size(g_ColumnInfos) - 1));
...
void CServicesDlg::BuildClipboardText(CUtlVector<char> &clipboardText) {
  ...
  for (int i = 0; i < pHeader->GetItemCount(); i++) {
    ...
    if (!pHeader->GetItem(i, &item)) item.pszText = "<bug>";
