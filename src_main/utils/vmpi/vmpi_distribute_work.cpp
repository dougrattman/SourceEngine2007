...
static CUtlVector<uint64_t> g_wuCountByProcess;
static uint64_t g_totalWUCountByProcess[512];

static uint64_t g_nWUs;  // How many work units there were this time around.
static uint64_t g_nCompletedWUs;   // How many work units completed.
static uint64_t g_nDuplicatedWUs;  // How many times a worker sent results for a
...
int SortByWUCount(const void *elem1, const void *elem2) {
  uint64_t a = g_wuCountByProcess[*((const int *)elem1)];
  uint64_t b = g_wuCountByProcess[*((const int *)elem2)];
  if (a < b)
    return 1;
  else if (a == b)
    return 0;
...
void VMPI_WorkerThread(int iThread, void *pUserData) {
  ...
  uint64_t idxRunningWorkUnit = (uint64_t)iThread;
  {
