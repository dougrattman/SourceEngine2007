...
#define DW_SUBPACKETID_WU_ASSIGNMENT \
  (VMPI_DISTRIBUTE_WORK_EXTRA_SUBPACKET_BASE + 0)
...
class CWorkUnitsTracker {
 ...
 public:
  ...
  void PrepareForWorkUnits(uint64_t numUnits);
  ...
  //			because the received work unit filled the gap or was the
  // next pending work unit.
  // returns 0 to indicate that this work unit is a "faster processed future
  // work unit".
  uint64_t WorkUnitFinished(uint64_t iWorkUnit);

 public:
  enum WUInfo { kNone, kTrigger, kDone };
  CVisibleWindowVector<uint8_t> m_arrInfo;
};

void CWorkUnitsTracker::PrepareForWorkUnits(uint64_t numUnits) {
...
uint64_t CWorkUnitsTracker::WorkUnitFinished(uint64_t iWorkUnit) {
  uint64_t uiResult = uint64_t(0);

  if (iWorkUnit >= m_arrInfo.FirstPossibleIndex() &&
      iWorkUnit < m_arrInfo.PastPossibleIndex()) {
    // Need to access the element
    m_arrInfo.ExpandWindow(iWorkUnit + 1, kNone);

    // Set it done
    uint8_t &rchThere = m_arrInfo.Get(iWorkUnit), chThere = rchThere;
    rchThere = kDone;
...
static bool CompareSoonestWorkUnitSets(CPartitionInfo::PartitionWUs *const &x,
                                       CPartitionInfo::PartitionWUs *const &y) {
...
class CDistributor_DefaultMaster : public IWorkUnitDistributorMaster {
 public:
  ...
  virtual bool HandleWorkUnitResults(WUIndexType iWorkUnit) {
    ...
    if (iWorkUnit >= m_WULookup.FirstPossibleIndex() &&
        iWorkUnit < m_WULookup.PastVisibleIndex())
      pLookup = &m_WULookup.Get(iWorkUnit);

    if (!pLookup || pLookup->m_iWUInfo == -1) return false;
    ...
    if (iWorkUnit == m_WULookup.FirstPossibleIndex()) {
      ...
      for (WUIndexType kwuEnd = m_WULookup.PastVisibleIndex(); kwu < kwuEnd;
           ++kwu) {
        if (-1 != m_WULookup.Get(kwu).m_iWUInfo && kwu > iWorkUnit) break;
      }
    ...
    if (pPartition->m_WUs.Count() == 0) {
      ...
      // If there are any more WUs remaining, give the worker from this
      // partition some more of them.
      if (m_WULookup.FirstPossibleIndex() < m_WULookup.PastPossibleIndex()) {
        AssignWUsToWorker(iPartitionWorker);
      }
    }

    uint64_t iDoneWorkUnits =
        g_MasterWorkUnitsTracker.WorkUnitFinished(iWorkUnit);
  ...
  bool SplitWUsPartition(CPartitionInfo *pPartitionLarge,
                         ...
                         CPartitionInfo **ppSecondHalf, int iFirstHalfWorker,
    ...
    CPartitionInfo *arrNewParts[2] = {
        *ppFirstHalf ? *ppFirstHalf : *ppSecondHalf, *ppSecondHalf};
  ...
  void AssignWUsToWorker(int iWorker) {
    ...
    if (CommandLine()->FindParm("-mpi_NoScheduler")) {
      Warning(
          "\n\n-mpi_NoScheduler found: Warning - this should only be used for "
          "testing and with 1 worker!\n\n");
    ...
    // Any absolutely untouched partitions yet?
      ...
      if (numWusToDeal <= 0) {
        uint64_t uiFraction = vlkup.PastPossibleIndex() / g_nMaxWorkerCount;
        Assert(uiFraction < INT_MAX / 2);

        numWusToDeal = int(uiFraction);
        if (numWusToDeal <= 0) numWusToDeal = 8;
      }
      ...
      for (WUIndexType i = iBegin; i < iEnd; ++i) {
        ...
        CPartitionInfo *pPartition =
            arrPartitions[size_t((i - iBegin) % numPartitions)];
    ...
    // Split one of the last partitions to finish sooner
      ...
      if (iPartToSplit >= 0) {
        ...
        if (SplitWUsPartition(pPartition, &pOldHalf, &pNewHalf, iOldWorker,
                              iNewWorker)) {
          if (pOldHalf) SendPartitionToWorker(pOldHalf, iOldWorker);
          if (pNewHalf) SendPartitionToWorker(pNewHalf, iNewWorker);
  ...
  int FindSoonestPartition() {
    ...
    // Sorted partitions
    CUtlMap<CPartitionInfo::PartitionWUs *, int> sortedPartitions(
        CompareSoonestWorkUnitSets);
    sortedPartitions.EnsureCapacity(lst.Count());
    FOR_EACH_LL(lst, i) { sortedPartitions.Insert(&lst[i]->m_WUs, i); }
  ...
  int FindPartitionByWorker(int iWorker) {
    FOR_EACH_LL(m_Partitions, i) {
      if (m_Partitions[i]->m_iWorker == iWorker) return i;
    }
    return -1;
  }
 ...
 private:
  ...
  CVisibleWindowVector<CWULookupInfo>
      m_WULookup;  // Map work unit index to CWorkUnitInfo.
  CUtlLinkedList<CWorkUnitInfo, int>
      m_WUInfo;  // Sorted with most elegible WU at the head.
};
 ...
 public:
  ...
  virtual bool HandlePacket(MessageBuffer *pBuf, int iSource,
    ...
    if (pBuf->data[1] == DW_SUBPACKETID_WU_ASSIGNMENT) {
      // If the message wasn't even related to the current DistributeWork() call
      // we're on, ignore it.
  ...
  CUtlLinkedList<WUIndexType, int>
      m_WorkUnits;  // A list of work units assigned to this worker
};
