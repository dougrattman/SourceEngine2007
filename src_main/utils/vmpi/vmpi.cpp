...
#define VMPI_PARAM(paramName, paramFlags, helpText) \
  {paramName, paramFlags, "-" #paramName, helpText},
class CVMPIParam {
 public:
...
static CVMPIParam g_VMPIParams[] = {
    {k_eVMPICmdLineParam_FirstParam, 0, "k_eVMPICmdLineParam_FirstParam",
     "unused"},
...
class CVMPIConnection : public ITCPSocketHandler {
 ...
 public:
  ...
  const char *GetMachineName() { return m_MachineName.Base(); }
...
void ParseOptions(int argc, char **argv) {
  ...
  const char *pTransmitRate =
      VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_FileTransmitRate), "1");
  if (pTransmitRate) {
  ...
  const char *pVerbose =
      VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_Verbose), "1");
  if (pVerbose) {
    if (pVerbose[0] == '1')
...
void SetupDependencyFilename(CDependencyInfo *pInfo,
  ...
  if (pPatchDirectory) {
    V_strncpy(pInfo->m_DependencyFilesDir, pPatchDirectory,
              sizeof(pInfo->m_DependencyFilesDir));
  } else {
    V_strncpy(pInfo->m_DependencyFilesDir, baseExeFilename,
              sizeof(pInfo->m_DependencyFilesDir));
  ...
  V_strncpy(pInfo->m_OriginalExeFilename,
            V_UnqualifiedFileName(baseExeFilename),
            sizeof(pInfo->m_OriginalExeFilename));
}

bool ReadString(char *pOut, int maxLen, FILE *fp) {
  if (!fgets(pOut, maxLen, fp) || pOut[0] == 0) return false;
...
void ParseDependencyFile(CDependencyInfo *pInfo, const char *pDepFilename) {
  ...
  while (ReadString(tempStr, sizeof(tempStr), fp)) {
    ...
    if (strstr(tempStr, "optional ") == tempStr) {
      bOptional = true;
      Q_strncpy(pFile->m_Name, tempStr + strlen(pOptionalPrefix),
                sizeof(pFile->m_Name));
    } else {
...
void UpdateActiveConnectionsText() {
  if (!g_bMPIMaster || !g_pConsoleWndFn) return;
  ...
  if (g_bVMPISDKMode) {
    ...
    V_snprintf(str, sizeof(str), "VMPI - Workers: %d, Downloaders: %d",
               nRegularWorkers, nDownloaders);
  }
...
CDispatchReg g_VMPIInternalDispatchReg(
    VMPI_INTERNAL_PACKET_ID,
    VMPI_InternalDispatchFn);  // register to handle the messages we want
...
void VMPI_ReceiveCommandLine() {
  // For verification purposes, tell the master we're trying to get the command
  // line.
  ...
  if (V_stricmp(fileBase, "vmpi_transfer") != 0) {
    if (V_stricmp(fileBase, g_MasterExeName) != 0) {
      Error(
          ...
          " a job like vvis really fast and started a vrad immediately, and an "
...
// ----------------------------------------------------------------------------------------
// // CMasterBroadcaster This class broadcasts messages looking for workers. The
// app updates it as often as possible and it'll add workers as necessary.
// ----------------------------------------------------------------------------------------
// //
...
class CMasterBroadcaster {
 ...
 private:
  ...
  CUtlVector<CIPAddr>
      m_PatchWorkerIPs;  // If in patch mode, these are the IPs we send the job
                         // request to (instead of broadcasting).
  bool m_bPatching;
...
bool CMasterBroadcaster::Init(int argc, char **argv,
  ...
  const char *pPortStr =
      VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_Port));
  if (pPortStr) {
    ...
    for (int iTest = VMPI_MASTER_FIRST_PORT; iTest <= VMPI_MASTER_LAST_PORT;
         iTest++) {
      m_iListenPort = iTest;
      m_pListenSocket =
          ThreadedTCP_CreateListener(&m_ConnectionCreator, m_iListenPort);
    ...
    if (m_pListenSocket && !g_bVMPISDKMode) {
      for (int iTest = m_iListenPort + 1; iTest <= VMPI_MASTER_LAST_PORT;
           iTest++) {
  ...
  // Create a socket to broadcast from unless we're in the SDK in which case we
  // don't broadcast.
  m_bPatching = false;
  if (m_RunMode == VMPI_RUN_NETWORKED && !g_bVMPISDKMode) {
    ...
    // Add -mpi_filebase so it can use absolute paths with the filesystem so we
    // get the exact right set of files.
...
void CMasterBroadcaster::BuildBroadcastPacket(bf_write &buf) {
  ...
  // Write the machine name of the master into the command line. It's ignored by
  // the code, but it's useful if a job crashes the workers - by looking at the
  // command line in vmpi_service, you can see who ran the job.
...
void CMasterBroadcaster::SetPassword(const char *pPassword) {
  CCriticalSectionLock connectionsLock(&g_ConnectionsCS);
  connectionsLock.Lock();
  Q_strncpy(m_BroadcastInfo.m_Password, pPassword,
            sizeof(m_BroadcastInfo.m_Password));
}
...
void CMasterBroadcaster::SetNoTimeoutOption() {
  ...
  for (int i = 1; i < m_BroadcastInfo.m_Args.Count(); i++) {
    if (Q_stricmp(m_BroadcastInfo.m_Args[i],
                  VMPI_GetParamString(mpi_NoTimeout)) == 0)
      return;
  }

  m_BroadcastInfo.m_Args.InsertAfter(
      0, (char *)VMPI_GetParamString(mpi_NoTimeout));
}
...
bool MPI_Init_Worker(int &argc, char **&argv, const CIPAddr &masterAddr,
...
Retry:;
  ...
  for (iPort = iFirstPort; iPort <= iLastPort; iPort++) {
    pConnectSocket = ThreadedTCP_CreateConnector(
        masterAddr, CIPAddr(0, 0, 0, 0, iPort), &connectionCreator);
  ...
  if (VMPI_IsParamUsed(mpi_Retry)) {
    Msg("%s found. Retrying connection to %d.%d.%d.%d:%d (attempt %d).\n",
        VMPI_GetParamString(mpi_Retry), masterAddr.ip[0], masterAddr.ip[1],
        masterAddr.ip[2], masterAddr.ip[3], masterAddr.port, nAttempts++);
    goto Retry;
  }
...
bool SpawnLocalWorker(int argc, char **argv, int iListenPort,
  ...
  for (int i = 0; i < 9999999; i++) {
    ...
    if (i == 1) {
      Q_snprintf(argStr, sizeof(argStr), "-mpi_worker 127.0.0.1:%d ",
                 iListenPort);
      Q_strncat(commandLine, argStr, sizeof(commandLine), COPY_ALL_CHARACTERS);
      Q_strncat(commandLine, "-allowdebug ", sizeof(commandLine),
                COPY_ALL_CHARACTERS);
      ...
      // -mpi_AutoLocalWorker).
      if (VMPI_IsSDKMode() &&
          !VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_SDKMode), "")) {
        Q_strncat(commandLine, VMPI_GetParamString(mpi_SDKMode),
                  sizeof(commandLine), COPY_ALL_CHARACTERS);
      }
    }
  ...
  if (CreateProcess(
          ...
          (bShowConsoleWindow ? CREATE_NEW_CONSOLE : CREATE_NO_WINDOW) |
              IDLE_PRIORITY_CLASS,  // flags
          NULL,                     // environment
          workingDir,  // current directory (use c:\\ because we don't want it
                       // to accidentally share DLLs like vstdlib with us).
          &si, &pi)) {
    return true;
  } else {
    char errStr[1024];
...
bool InitMaster(int argc, char **argv, const char *pDependencyFilename,
                VMPIRunMode runMode, bool bPatchMode) {
  int nMaxWorkers = -1;
  const char *pProcCount =
      VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_WorkerCount));
  if (pProcCount) {
...
void VMPI_InitGlobals(int argc, char **argv, VMPIRunMode runMode) {
  ...
  if (g_hKernel32DLL) {
    g_pConsoleWndFn =
        (GetConsoleWndFn)GetProcAddress(g_hKernel32DLL, "GetConsoleWindow");
  }
...
bool IsValidSDKBinPath(CUtlVector<char *> &outStrings, int *pError) {
  *pError = 0;
  ...
  // Minimum must have
  // drive:/basedir/steamapps/name/sourcesdk/bin/[ep1|orangebox]/bin/exename
  ...
  for (int i = 1; i < outStrings.Count() - 5; i++) {
    ...
    V_strncat(sourcesdkContentPath, outStrings[i],
...
void VerifyValidSDKMode() {
  ...
  if (!IsValidSDKBinPath(outStrings, &err)) {
    ...
    if (err == 5)
      Error(
          "VMPI running in SDK mode but Steam hasn't been run recently. Please "
          "run Steam and retry.");
    else
      Error(
          "VMPI running in SDK mode but incorrect SDK install detected (error "
          "%d).",
          err);
  }
}
...
bool VMPI_HandleAutoRestart() {
  ...
  for (int i = 0; i < g_OriginalCommandLineParameters.Count(); i++) {
    char argStr[512];
    Q_snprintf(argStr, sizeof(argStr), "\"%s\" ",
               g_OriginalCommandLineParameters[i]);
    Q_strncat(commandLine, argStr, sizeof(commandLine), COPY_ALL_CHARACTERS);
  }
...
bool VMPI_Init(int &argc, char **&argv, const char *pDependencyFilename,
  ...
  const char *pMasterIP =
      VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_Worker), NULL);
  if (pMasterIP) {
    CIPAddr addr;
...
// If bWait is false, then this function returns false immediately if there are
// no messages waiting.
...
bool VMPI_DispatchUntil(MessageBuffer *pBuf, int *pSource, int packetID,
  ...
  while (1) {
    ...
    if (!VMPI_InternalDispatch(pBuf, *pSource)) {
      ...
      // barrier packet, and it'll get results. In that case, the packet should
...
inline bool VMPI_FilterPacketsForServiceDownloader(CVMPIConnection *pConnection,
                                                   void const *const *pChunks,
  ...
  if (pConnection->m_bIsAService) {
    ...
    for (int i = 0; i < nChunks; i++) {
      if (pChunkLengths[i] > 0) {
        ...
        if (cPacketID == VMPI_SHARED_PACKET_ID ||
...
bool VMPI_SendChunks(void const *const *pChunks, const int *pChunkLengths,
                     int nChunks, int iDest, int fVMPISendFlags) {
...
bool VMPI_Send2Chunks(const void *pChunk1, int chunk1Len, const void *pChunk2,
...
bool VMPI_Send3Chunks(const void *pChunk1, int chunk1Len, const void *pChunk2,
                      int chunk2Len, const void *pChunk3, int chunk3Len,
...
const char *VMPI_GetParamString(EVMPICmdLineParam eParam) {
  if (eParam <= k_eVMPICmdLineParam_FirstParam ||
      eParam >= k_eVMPICmdLineParam_LastParam) {
    Assert(false);
