...
#define VMPI_SERVICE_IDS_VERSION_STRING \
  102  // This matches IDS_VERSION_STRING in vmpi_service.exe.

// Known packet IDs in various systems.
#define VMPI_PACKETID_FILESYSTEM \
  0  // The file system reserves this packet ID.
...
#define VMPI_SERVICE_TO_UI_CONSOLE_TEXT \
  0  // Print some text to the UI's console.
#define VMPI_SERVICE_TO_UI_STATE \
  1  // Updates state reflecting whether it's idle, busy, etc.
#define VMPI_SERVICE_TO_UI_PATCHING \
  2  // Updates state reflecting whether it's idle, busy, etc.
#define VMPI_SERVICE_TO_UI_EXIT \
  3  // Updates state reflecting whether it's idle, busy, etc.
...
#define VMPI_SERVICE_EXIT \
  5  // User chose "exit" from the menu. Kill the service.
