...
typedef uint64_t WUIndexType;
class CDSInfo;
extern bool g_bVMPIEarlyExit;

// These classes are overridden to handle the details of communicating and
...
class IWorkUnitDistributorMaster {
 public:
  ...
  virtual bool HandlePacket(MessageBuffer *pBuf, int iSource,
                            bool bIgnoreContents) = 0;

  // Called when the results for a work unit arrive. This function must return
  // false if we've already received the results for this work unit.
...
class CVisibleWindowVectorT : protected CUtlVector<T> {
 ...
 public:
  inline void PostInitElement(uint64_t uiRealIdx,
                              T &newElement) { /* do nothing */
    return;
  }
  inline void PostInitElement(uint64_t uiRealIdx, T &newElement, T const &x) {
    newElement = x;
  }
 ...
 public:
  ...
  T &Get(uint64_t idx) {
    T *pElement = ((idx >= m_uiBase && idx < m_uiBase + BaseClass::Count())
                       ? (BaseClass::Base() + size_t(idx - m_uiBase))
                       : NULL);
    Assert(pElement);
    return *pElement;
  }
  ...
  void ExpandWindow(uint64_t idxAccessible) {
    ...
    if (idxAccessible >= m_uiBase + BaseClass::Count()) {
      ...
      Assert(iOldCount + iAddElements <=
             100000000 /* really need 100 Mb at once? */);
      BaseClass::AddMultipleToTail(iAddElements);
      for (int iNewCount = BaseClass::Count(); iOldCount < iNewCount;
           ++iOldCount)
        static_cast<Derived *>(this)->PostInitElement(
            m_uiBase + iOldCount, BaseClass::Element(iOldCount));
    }
  }

  // Expands the window and initializes the new elements to a given value
  void ExpandWindow(uint64_t idxAccessible, T const &x) {
    Assert(idxAccessible >= m_uiBase);
    if (idxAccessible >= m_uiBase + BaseClass::Count()) {
      int iOldCount = BaseClass::Count();
      int iAddElements = int(idxAccessible - m_uiBase) - iOldCount + 1;
      ...
      for (int iNewCount = BaseClass::Count(); iOldCount < iNewCount;
           ++iOldCount)
        static_cast<Derived *>(this)->PostInitElement(
            m_uiBase + iOldCount, BaseClass::Element(iOldCount), x);
    }
  }

  // Shrinks the window to drop some of the elements from the head
  void ShrinkWindow(uint64_t idxDrop) {
  ...
  uint64_t FirstPossibleIndex() const { return m_uiBase; }

  // Last possible index in this vector
  uint64_t PastPossibleIndex() const { return m_uiTotal; }
  // Past visible window index in this vector
  uint64_t PastVisibleIndex() const { return m_uiBase + BaseClass::Count(); }

 protected:
  uint64_t m_uiBase, m_uiTotal;
};

template <typename T>
class CVisibleWindowVector
    : public CVisibleWindowVectorT<T, CVisibleWindowVector<T> > {
 public:
...
class CWorkerInfo {
 public:
  ...
  CVisibleWindowVector<WUIndexType>
      m_WorkUnitsRunning;  // A list of work units currently running, index is
                           // the thread index
...
// Called to write the packet ID, the subpacket ID, and the ID of which
// DistributeWork() call we're at.
