...
CChunkWalker::CChunkWalker(void const *const *pChunks, const int *pChunkLengths,
                           int nChunks) {
  m_TotalLength = 0;
  for (int i = 0; i < nChunks; i++) m_TotalLength += pChunkLengths[i];
...
bool CWaitTimer::ShouldKeepWaiting() {
  if (m_WaitMS == 0) {
    ...
    return (SampleMilliseconds() - m_StartTime) <= m_WaitMS ||
           g_bForceWaitTimers;
  }
}
...
class CIPSocket : public ISocket {
 ...
 public:
  ...
  virtual bool ListenToMulticastStream(const CIPAddr &addr,
