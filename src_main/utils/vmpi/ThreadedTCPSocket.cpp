...
#define KEEPALIVE_SENTINEL \
  -12345  // When first 4 bytes of a packet = this, then it's just a keepalive.
...
// We get crashes at runtime if they don't link in the multithreaded runtime
// libraries, so raise a ruckus if they're using singlethreaded libraries.
#ifndef _MT
#pragma message( \
    "**** WARNING **** ThreadedTCPSocket requires multithreaded runtime libraries to be used.\n")
class MTChecker {
 public:
...
class CThreadedTCPSocket : public IThreadedTCPSocket {
 public:
  static IThreadedTCPSocket *Create(SOCKET iSocket, CIPAddr remoteAddr,
 ...
 public:
  ...
  virtual bool SendChunks(void const *const *pChunks, const int *pChunkLengths,
                          int nChunks) {
    if (CheckErrorSignal()) return false;
 ...
 private:
  ...
  bool Init(SOCKET iSocket, CIPAddr remoteAddr, ITCPSocketHandler *pHandler) {
    ...
    m_hSendThread =
        CreateThread(NULL, 0, &CThreadedTCPSocket::StaticSendThreadFn, this,
                     CREATE_SUSPENDED, &dwSendThreadID);
    m_hRecvThread =
        CreateThread(NULL, 0, &CThreadedTCPSocket::StaticRecvThreadFn, this,
                     CREATE_SUSPENDED, &dwRecvThreadID);
    ...
    // Make sure to init the handler before the threads actually run, so it
    // isn't handed data before initializing.
  ...
  void SetInitialSocketOptions() {
    ...
    BOOL val = TRUE;
    setsockopt(m_Socket, IPPROTO_TCP, TCP_NODELAY, (const char FAR *)&val,
               sizeof(BOOL));
    ...
    linger.l_onoff = 1;
    linger.l_linger = 3;
    setsockopt(m_Socket, SOL_SOCKET, SO_LINGER, (char *)&linger,
               sizeof(linger));
  }

  // Send thread functionality.
 private:
  // This function copies off the payload and adds a SendChunk_t to the list of
  // chunks to be sent. It also fires the ReadyToSend event so the thread will
  // pick it up.
  bool InternalSend(void const *const *pChunks, const int *pChunkLengths,
                    int nChunks, bool bPrependLength) {
    int totalLength = 0;
    for (int i = 0; i < nChunks; i++) totalLength += pChunkLengths[i];
    ...
    SendData_t *pSendData =
        (SendData_t *)malloc(sizeof(SendData_t) - 1 + totalLength);
    pSendData->m_Len = totalLength;

    char *pOut = pSendData->m_Payload;
    if (bPrependLength) {
      *((int *)pOut) = totalLength - 4;  // The length we prepend is the size of
                                         // the data, not data size + integer
                                         // for length.
      pOut += 4;
    }
    ...
    m_hReadyToSendEvent
        .SetEvent();  // Notify the thread that there is data to send.
  ...
  bool SendThread_HandleSendCompletionEvent() {
    ...
    if (!WSAGetOverlappedResult(m_Socket, &m_SendOverlapped, &cbTransfer, TRUE,
                                &flags)) {
    ...
    if (cbTransfer != m_nBytesToTransfer) {
      ...
      Q_snprintf(
          str, sizeof(str),
          "Invalid # bytes transferred (%d) in send thread (should be %d)",
          cbTransfer, m_nBytesToTransfer);
  ...
  bool SendThread_HandleReadyToSendEvent() {
    ...
    DWORD ret =
        WSASend(m_Socket, &buf, 1, &dwNumBytesSent, 0, &m_SendOverlapped, NULL);
    DWORD err = WSAGetLastError();
    if (ret == 0 || (ret == SOCKET_ERROR && err == WSA_IO_PENDING)) {
      // Either way, the operation completed successfully, and
      // m_hSendCompletionEvent is now set.
      return true;
    } else {
      HandleError(err);
      return false;
  ...
  DWORD SendThreadFn() {
    while (1) {
      ...
      DWORD waitValue = WaitForMultipleObjects(nHandles, handles, FALSE,
      ...
      switch (waitValue) {
        ...
        case WAIT_OBJECT_0 + 1: {
          if (!SendThread_HandleSendCompletionEvent()) return 1;
        } break;

        case WAIT_OBJECT_0 + 2: {
          if (!SendThread_HandleReadyToSendEvent()) return 1;
        } break;
        ...
        default: {
          ...
          Q_snprintf(str, sizeof(str),
                     "Unknown return value (%lu) from WaitForMultipleObjects",
                     waitValue);
          HandleError(ITCPSocketHandler::SocketError, str);
          return 0;
        }
 ...
 private:
  bool RecvThread_WaitToReceiveSize() {
    return RecvThread_InternalRecv(&m_NextPacketLen, sizeof(m_NextPacketLen),
                                   false, true);
  }
  ...
  bool RecvThread_InternalHandleRecvCompletion(DWORD dwTransfer) {
    ...
    if (cbTransfer > nBytesWanted) {
      char str[512];
      Q_snprintf(str, sizeof(str),
                 "Invalid # bytes received (%d) in recv thread (should be %d)",
                 cbTransfer, m_nBytesToReceive);
      HandleError(ITCPSocketHandler::SocketError, str);
      return false;
    } else if (cbTransfer < nBytesWanted) {
      // We have to reissue the receive command because it didn't receive all
      // the data.
      ...
      return RecvThread_InternalRecv(&pDest[m_nBytesReceivedSoFar],
                                     m_nBytesToReceive - m_nBytesReceivedSoFar,
                                     true);
    }
    ...
    if (m_bWaitingForSize) {
      ...
      if (m_NextPacketLen == KEEPALIVE_SENTINEL) {
        ...
        if (m_NextPacketLen < 1 || m_NextPacketLen > 1024 * 1024 * 75) {
          char str[512];
          Q_snprintf(str, sizeof(str),
                     "Invalid packet size in RecvThread (size = %d)",
                     m_NextPacketLen);
          HandleError(ITCPSocketHandler::SocketError, str);
          return false;
        } else {
          Assert(!m_pRecvBuffer);
          m_pRecvBuffer =
              (CTCPPacket *)malloc(sizeof(CTCPPacket) - 1 + m_NextPacketLen);
          m_pRecvBuffer->m_UserData = 0;
          m_pRecvBuffer->m_Len = m_NextPacketLen;

          return RecvThread_InternalRecv(m_pRecvBuffer->m_Data,
                                         m_pRecvBuffer->m_Len, false, false);
        }
      }
    } else {
  ...
  bool RecvThread_HandleRecvCompletionEvent() {
    // A send operation just completed. Now do the next one.
    DWORD cbTransfer, flags;
    if (!WSAGetOverlappedResult(m_Socket, &m_RecvOverlapped, &cbTransfer, TRUE,
                                &flags)) {
      HandleError(WSAGetLastError());
      return false;
    }
  ...
  bool RecvThread_InternalRecv(void *pDest, int destSize, bool bContinuation,
                               bool bWaitingForSize = false) {
    WSABUF buf = {destSize, (char *)pDest};

    if (!bContinuation) {
      // If this is not a continuation of whatever we were receiving before,
      // then
    ...
    DWORD ret = WSARecv(m_Socket, &buf, 1, &nBytesReceived, &dwFlags,
                        &m_RecvOverlapped, NULL);
    DWORD dwLastError = WSAGetLastError();
    if (ret == 0 || (ret == SOCKET_ERROR && dwLastError == WSA_IO_PENDING)) {
      // Note: m_hRecvEvent is in a signaled state, so the RecvThread will pick
      // up the results next time around.
      return true;
    } else {
  ...
  DWORD RecvThreadFn() {
    ...
    while (1) {
      ...
      switch (waitValue) {
        case WAIT_TIMEOUT: {
          if (g_bHandleTimeouts) {
            HandleError(ITCPSocketHandler::ConnectionTimedOut,
        ...
        case WAIT_OBJECT_0 + 1: {
          ...
          if (!RecvThread_HandleRecvCompletionEvent()) return 1;
        } break;
  ...
  // Error handling.
 private:
  // This checks to see if either thread has signaled an error. If so, it shuts
  // down the socket and returns true.
  bool CheckErrorSignal() { return m_bErrorSignal; }

  // This is called from any of the threads and signals that something went
  // awry. It shuts down the object and makes it return false from all of its
  // functions.
  void HandleError(DWORD errorValue) {
    ...
    // Windows likes to stick a carriage return in there and we don't want it so
    // get rid of it.
 ...
 private:
  ...
  CUtlLinkedList<SendData_t *, int>
      m_SendDatas;  // Added to the tail, popped off the head for sending.

  // Data for the recv thread.
  HANDLE m_hRecvThread;
  int m_nBytesToReceive;  // This stores how many bytes we want to receive for
                          // the next packet.
  int m_nBytesReceivedSoFar;  // This stores how many bytes we've received so
                              // far.
  bool m_bWaitingForSize;     // This tells if we're trying to receive the next
                              // packet length or the next packet's data.

  int m_NextPacketLen;  // Data is received INTO here before it receives each
                        // packet. This holds the length of each incoming
                        // packet.

  WSAOVERLAPPED m_RecvOverlapped;
  CEvent m_hRecvEvent;
  CTCPPacket *m_pRecvBuffer;  // This is allocated for each packet we're
                              // receiving and given to the app when the packet
...
class CTCPConnectSocket_Listener : public ITCPConnectSocket {
 public:
  ...
  static ITCPConnectSocket *Create(IHandlerCreator *pHandlerCreator,
    ...
    if (nQueueLength < 0) {
      Error(
          "CTCPConnectSocket_Listener::Create - SOMAXCONN not allowed - causes "
          "some XP SP2 systems to stop receiving any network data "
          "(systemwide).");
    }
    ...
    if (pRet->m_Socket == INVALID_SOCKET ||
        listen(pRet->m_Socket, nQueueLength == -1 ? SOMAXCONN : nQueueLength) !=
            0) {
      pRet->Release();
      return false;
    }
 ...
 public:
  ...
  virtual bool Update(IThreadedTCPSocket **pSocket,
    ...
    *pSocket = NULL;
    ...
    // We're still ok.. just wait until the socket becomes writable (is
    // connected) or we timeout.
    fd_set readSet;
    readSet.fd_count = 1;
    ...
    if (status > 0) {
      ...
      if (newSock == INVALID_SOCKET) {
        ...
        IThreadedTCPSocket *pRet = CThreadedTCPSocket::Create(
            newSock, connectedAddr, m_pHandler->CreateNewHandler());
        if (!pRet) {
          Assert(false);
          closesocket(m_Socket);
          m_Socket = INVALID_SOCKET;
...
ITCPConnectSocket *ThreadedTCP_CreateListener(IHandlerCreator *pHandlerCreator,
                                              const unsigned short port,
                                              int nQueueLength) {
  return CTCPConnectSocket_Listener::Create(pHandlerCreator, port,
                                            nQueueLength);
}
...
class CTCPConnectSocket_Connector : public ITCPConnectSocket {
 public:
  ...
  static ITCPConnectSocket *Create(const CIPAddr &connectAddr,
 ...
 public:
  ...
  virtual bool Update(IThreadedTCPSocket **pSocket,
    ...
    *pSocket = NULL;
    ...
    // If this condition holds, then we already returned a valid socket and
    // we're just waiting to be released.
    ...
    if (m_bConnected) {
      ...
      IThreadedTCPSocket *pRet = CThreadedTCPSocket::Create(
          m_Socket, m_RemoteAddr, m_pHandlerCreator->CreateNewHandler());
      if (pRet) {
        m_Socket = INVALID_SOCKET;
        *pSocket = pRet;
        return true;
...
ITCPConnectSocket *ThreadedTCP_CreateConnector(
    const CIPAddr &addr, const CIPAddr &localAddr,
    IHandlerCreator *pHandlerCreator) {
