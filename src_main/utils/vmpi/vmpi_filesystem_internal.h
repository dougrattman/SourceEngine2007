...
#define VMPI_FSPACKETID_CHUNK_RECEIVED \
  3  // Sent by workers to tell the master they received a chunk.
#define VMPI_FSPACKETID_FILE_RECEIVED \
  4  // Sent by workers to tell the master they received the whole file.
...
#define MAX_CHUNK_PAYLOAD_SIZE \
  std::max(TCP_CHUNK_PAYLOAD_SIZE, MULTICAST_CHUNK_PAYLOAD_SIZE)
