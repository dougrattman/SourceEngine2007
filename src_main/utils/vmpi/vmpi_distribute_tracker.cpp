...
class CWorkUnit {
 public:
  CWorkUnit() { m_iWorkerCompleted = -1; }

  int m_iWorkerCompleted;  // Which worker completed this work unit (-1 if not
                           // done yet).
...
static int CountActiveWorkUnits() {
  ...
  for (int i = 0; i < g_WorkUnits.Count(); i++) {
    if (g_WorkUnits[i].m_iWorkerCompleted == -1) ++nActive;
  }
  return nActive;
}
...
static COLORREF g_StateColors[] = {
    RGB(50, 50, 50), RGB(100, 0, 0), RGB(150, 0, 0), RGB(0, 155, 0),
    RGB(0, 255, 0),  RGB(0, 0, 150), RGB(0, 0, 250)};
...
static LRESULT CALLBACK TrackerWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                          LPARAM lParam) {
  switch (uMsg) {
    case WM_PAINT: {
      ...
      memcpy(wuStatus.Base(), g_WUStatus.Base(),
             wuStatus.Count() * sizeof(wuStatus[0]));
      ...
      for (int iState = 0; iState < SOURCE_ARRAYSIZE(hStateColors); iState++) {
        ...
        for (int iWU = 0; iWU < wuStatus.Count(); iWU++) {
          if (wuStatus[iWU].m_iState != iState) continue;
    ...
    case WM_SIZE: {
      ...
      // What is the max width of the grid elements so they will fit in the
      // width and height.
      int testSize;
...
static void CheckFlashTimers() {
  ...
  // Check timers for the events that just happened (we show them in a brighter
  // color if they just occurred).
...
static DWORD WINAPI ThreadProc(LPVOID lpParameter) {
  ...
  // Create the window.
  g_hWnd = CreateWindow(pClassName, "VMPI Tracker", WS_OVERLAPPEDWINDOW, 0, 0,
                        g_LastSizeX, g_LastSizeY, NULL, NULL,
  ...
  while (WaitForSingleObject(g_hDestroyWindowEvent, 200) != WAIT_OBJECT_0) {
    ...
    if (g_nChanges != g_nLastDrawnChanges) InvalidateRect(g_hWnd, NULL, FALSE);
  }
...
static void Graphical_Start() {
  ...
  for (int i = 0; i < g_WUStatus.Count(); i++) g_WUStatus[i].m_iState = 0;
...
static void Graphical_WorkUnitSentToWorker(int iWorkUnit) {
  ...
  if (s.m_iState != 3 && s.m_iState != 4 && s.m_iState != 5 &&
      s.m_iState != 6) {
    s.m_iState = 2;
...
static void Graphical_WorkUnitStarted(int iWorkUnit) {
  ...
  if (g_WUStatus[iWorkUnit].m_iState != 3 &&
...
void VMPITracker_Start(int nWorkUnits) {
  g_bTrackWorkUnitEvents =
      (VMPI_IsParamUsed(mpi_TrackEvents) || VMPI_IsParamUsed(mpi_Graphics));
...
bool VMPITracker_WriteDebugFile(const char *pFilename) {
  ...
  if (fopen_s(&fp, pFilename, "wt")) {
    ...
    for (int i = 0; i < g_WorkUnits.Count(); i++) {
      ...
      if (wu->m_iWorkerCompleted != -1) continue;
  ...
  if (iState == 0) {
    ...
    if (key == '1') {
      ...
      for (iFile; iFile < nMaxTries; iFile++) {
        Q_snprintf(filename, sizeof(filename), "c:\\vmpi_tracker_%d.txt",
                   iFile);
        if (_access(filename, 0) != 0) break;
      }
      if (iFile == nMaxTries) {
