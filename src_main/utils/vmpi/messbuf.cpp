...
//
// Purpose: MessageBuffer - handy for serializing/unserializing structures to be
// sent as messages dal - 9/2002
...
MessageBuffer::~MessageBuffer() { free(data); }

int MessageBuffer::getSize() { return size; }

int MessageBuffer::getLen() { return len; }
...
int MessageBuffer::getOffset() { return offset; }
...
int MessageBuffer::ReadString(char *pOut, int bufferLength) {
  ...
  while (1) {
    ...
    if (ch == 0 || nChars >= (bufferLength - 1)) break;
