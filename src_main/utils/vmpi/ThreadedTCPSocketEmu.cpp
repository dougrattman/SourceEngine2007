...
// ----------------------------------------------------------------------------------------
// // CThreadedTCPSocketEmu. This uses IThreadedTCPSocket to emulate the
// polling-type interface in ITCPSocket.
// ----------------------------------------------------------------------------------------
// //

// This class uses the IThreadedTCPSocket interface to emulate the old
// ITCPSocket.
class CThreadedTCPSocketEmu : public ITCPSocket,
                              public ITCPSocketHandler,
                              public IHandlerCreator {
 public:
 ...
 public:
  ...
  virtual bool BeginConnect(const CIPAddr &addr) {
    ...
    if (m_pConnectSocket) m_pConnectSocket->Release();

    m_pConnectSocket = ThreadedTCP_CreateConnector(
        addr, CIPAddr(0, 0, 0, 0, m_LocalPort), this);
  ...
  virtual bool UpdateConnect() {
    Assert(!m_pSocket);
    if (!m_pConnectSocket) return false;
  ...
  virtual bool SendChunks(void const *const *pChunks, const int *pChunkLengths,
                          int nChunks) {
    Assert(m_pSocket);
    if (!m_pSocket || !m_pSocket->IsValid()) return false;
  ...
  virtual bool Recv(CUtlVector<unsigned char> &data, double flTimeout) {
    // Use our m_RecvPacketsEvent event to determine if there is data to receive
    // yet.
    DWORD nMilliseconds = (DWORD)(flTimeout * 1000.0f);
    DWORD ret =
        WaitForSingleObject(m_RecvPacketsEvent.GetEventHandle(), nMilliseconds);
    if (ret == WAIT_OBJECT_0) {
      ...
      data.CopyArray((const unsigned char *)pPacket->GetData(),
                     pPacket->GetLen());
 ...
 private:
  ...
  bool m_bError;  // Set to true when there's an error. Next chance we get in
                  // the main thread, we'll close the socket.
};
...
class CThreadedTCPListenSocketEmu : public ITCPListenSocket,
                                    public IHandlerCreator {
 public:
  ...
  virtual ~CThreadedTCPListenSocketEmu() {
    if (m_pListener) m_pListener->Release();
  }

  bool StartListening(const unsigned short port, int nQueueLength) {
    m_pListener = ThreadedTCP_CreateListener(this, port, nQueueLength);
