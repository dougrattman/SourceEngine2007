...
class CMasterMulticastThread {
 public:
  ...
  bool Init(IBaseFileSystem *pPassThru, unsigned short localPort,
            const CIPAddr *pAddr, int maxFileSystemMemoryUsage);
  void Term();
 ...
 private:
  ...
  class CClientFileInfo {
   ...
   public:
    ...
    float m_flLastAckTime;   // Last time we heard an ack back from this client
...
bool CMasterMulticastThread::Init(IBaseFileSystem *pPassThru,
                                  ...
                                  const CIPAddr *pAddr, int maxMemoryUsage) {
  Term();
...
void CMasterMulticastThread::TCP_SendNextChunk(CMulticastFile *pFile,
  ...
  int chunkLengths[5] = {
      ...
      strlen(pFile->GetFilename()) + 1, iEndByte - iStartByte};
