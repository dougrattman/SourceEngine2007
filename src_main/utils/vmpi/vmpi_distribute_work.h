...
// Before calling DistributeWork, you can set this and it'll call your virtual
// functions.
...
// This is the function vrad and vvis use to divide the work units and send them
// out. It maintains a sliding window of work units so it can always keep the
// clients busy.
//
// The workers implement processFn to do the job work in a work unit.
// This function must send back a packet formatted with:
// cPacketID (char), cSubPacketID (char), iWorkUnit (int), (app-specific data
// for the results)
//
...
// VMPI calls this before shutting down because any threads that DistributeWork
// has running must stop, otherwise it can crash if a thread tries to send data
