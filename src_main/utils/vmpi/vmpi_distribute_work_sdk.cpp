...
#define DW_SUBPACKETID_REQUEST_SHUFFLE \
  (VMPI_DISTRIBUTE_WORK_EXTRA_SUBPACKET_BASE + 1)
#define DW_SUBPACKETID_WUS_COMPLETED_LIST \
  (VMPI_DISTRIBUTE_WORK_EXTRA_SUBPACKET_BASE + 2)
...
// starting at the back and continue on. At about this time, the master should
// reshuffle the remaining work units to evenly distribute them amongst the
// workers.
class CWorkUnitWalker {
 public:
  CWorkUnitWalker() { m_nWorkUnits = 0; }

  // This is all that's needed for it to start assigning work units.
  void Init(WUIndexType matrixWidth, WUIndexType matrixHeight,
            WUIndexType nWorkUnits) {
  ...
  bool GetNextWorkUnit(int iWorker, WUIndexType *pWUIndex,
    ...
    if (pWorker->m_iWorkUnitOffset >= m_nWorkUnits) return false;

    // If we've gone past the end of our work unit list, then we start at the
    // BACK of the other rows of work units in the hopes that we won't collide
    // with the guy working there. We also should tell the master to reshuffle.
 ...
 private:
  class CWorkerInfo {
   public:
    ...
    WUIndexType m_iWorkUnitOffset;  // Which work unit in my list of work units
                                    // am I working on?
  };
...
// In-between shuffles, you can call NoteWorkUnitCompleted when a work unit is
// completed and it'll avoid returning that work unit from GetNextWorkUnit
// again, but it WON'T
class CShuffledWorkUnitWalker {
 public:
  void Init(WUIndexType nWorkUnits, IShuffleRequester *pRequester) {
    ...
    for (WUIndexType iWU = 0; iWU < nWorkUnits; iWU++) {
      // Note: we're making an assumption here that if we add entries to a
      // CUtlLinkedList in ascending order, their indices will be ascending
      // 1-by-1 as well. If that assumption breaks, we can create an extra array
      // here to map WU indices to the linked list indices.
      WUIndexType index = m_WorkUnitsRemaining.AddToTail(iWU);
      if (index != iWU) {
        Error(
            "CShuffledWorkUnitWalker: assumption on CUtlLinkedList indexing "
            "failed.\n");
      }
    }
  }
  ...
  void Shuffle(int nWorkers) {
    ...
    // Here's the shuffle. The CWorkUnitWalker is going to walk each worker
    // through its own group from 0-W, and our job is to interleave it so when
    // worker 0 goes [0,1,2] and worker 1 goes [100,101,102], they're actually
    ...
    if ((m_WorkUnitsRemaining.Count() % matrixHeight) != 0) ++matrixWidth;
  ...
  bool Thread_IsWorkUnitCompleted(WUIndexType iWU) {
    ...
    uint8_t val = m_CompletedWUBits[iWU >> 3] & (1 << (iWU & 7));
    return (val != 0);
  }
  ...
  bool Thread_GetNextWorkUnit(int iWorker, WUIndexType *pWUIndex) {
    ...
    while (1) {
      ...
      bool bWorkerFinishedHisColumn;
      if (!m_Walker.GetNextWorkUnit(iWorker, &iUnmappedWorkUnit,
                                    &bWorkerFinishedHisColumn))
        return false;

      // If we've done all the work units assigned to us in the last shuffle,
      // then request a reshuffle.
      if (bWorkerFinishedHisColumn) HandleWorkerFinishedColumn();

      // Check the pending list.
      *pWUIndex = m_WorkUnitsMap[iUnmappedWorkUnit];
      uint8_t bIsCompleted =
          m_CompletedWUBits[*pWUIndex >> 3] & (1 << (*pWUIndex & 7));
      uint8_t bIsCompletedLocally =
          m_LocalCompletedWUBits[*pWUIndex >> 3] & (1 << (*pWUIndex & 7));
      if (!bIsCompleted && !bIsCompletedLocally) return true;
    }
  }
 ...
 private:
  // These are PENDING WU completions until we call Shuffle() again, at which
  // point we actually reorder the list based on the completed WUs.
  CUtlVector<uint8_t> m_CompletedWUBits;  // Bit vector of completed WUs.
  CUtlLinkedList<WUIndexType, WUIndexType> m_WorkUnitsRemaining;
  CUtlVector<WUIndexType> m_WorkUnitsMap;  // Maps the 0-N indices in the
                                           // CWorkUnitWalker to the list of
                                           // remaining work units.

  // Helps us avoid some duplicates that happen during shuffling if we've
  // completed some WUs and sent them to the master, but the master hasn't
  // included them in the DW_SUBPACKETID_WUS_COMPLETED_LIST yet.
  CUtlVector<uint8_t> m_LocalCompletedWUBits;  // Bit vector of completed WUs.

  // Used to control how frequently we request a reshuffle.
  unsigned int m_iCurShuffle;
  unsigned int m_iLastShuffleRequest;  // The index of the shuffle we last
                                       // requested a reshuffle on (don't
                                       // request a reshuffle on the same one).
...
class CDistributor_SDKMaster : public IWorkUnitDistributorMaster,
                               public IShuffleRequester {
 public:
  ...
  void Master_WorkerThread(int iThread) {
    while (m_WorkUnitWalker.Thread_NumWorkUnitsRemaining() > 0 &&
      ...
      // Do this work unit.
      m_WorkUnitWalker.Thread_NoteLocalWorkUnitCompleted(
          iWU);  // We do this before it's completed because otherwise if a
                 // Shuffle() occurs, the other thread might happen to pickup
                 // this work unit and we don't want that.
  ...
  virtual void DistributeWork_Master(CDSInfo *pInfo) {
    ...
    if (m_bUsingMasterLocalThreads)
      RunThreads_Start(Master_WorkerThread_Static, this,
                       k_eRunThreadsPriority_Idle);

    uint64_t lastShuffleTime = Plat_MSTime();
    while (m_WorkUnitWalker.Thread_NumWorkUnitsRemaining() > 0) {
  ...
  virtual void RequestShuffle() { m_bShuffleRequested = true; }

  bool CheckShuffleRequest() {
    if (m_bShuffleRequested) {
      double flCurTime = Plat_FloatTime();
      if (flCurTime - m_flLastShuffleRequestServiceTime >
          2.0f)  // Only handle shuffle requests every so often.
      {
  ...
  void Shuffle() {
...

    {
      ...
      for (int i = 0; i < pWorkersReady->m_WorkersReady.Count(); i++) {
        ...
        if (VMPI_IsProcConnected(iWorker)) whosWorking.AddToTail(iWorker);
      }
      m_WorkersReadyCS.Unlock();
    }

    // Before sending the shuffle command, tell any of these active workers
    // about the pending WUs completed.
    CWUsCompleted *pWUsCompleted = m_WUsCompletedCS.Lock();

    m_WUSCompletedMessageBuffer.setLen(0);
    if (BuildWUsCompletedMessage(pWUsCompleted->m_Pending,
                                 m_WUSCompletedMessageBuffer) > 0) {
      for (int i = m_bUsingMasterLocalThreads; i < whosWorking.Count(); i++) {
        VMPI_SendData(m_WUSCompletedMessageBuffer.data,
                      m_WUSCompletedMessageBuffer.getLen(), whosWorking[i]);
      }
    }
    pWUsCompleted->m_Completed.AddMultipleToTail(
        pWUsCompleted->m_Pending.Count(),
        pWUsCompleted->m_Pending
            .Base());  // Add the pending ones to the full list now.
    ...
    // Now for each worker, assign him an index in the shuffle and send the
    // shuffle command.
  ...
  int BuildWUsCompletedMessage(CUtlVector<WUIndexType> &wusCompleted,
                               MessageBuffer &mb) {
  ...
  virtual void OnWorkerReady(int iSource) {
    ...
    if (pWorkersReady->m_WorkersReady.Find(iSource) == -1) {
      ...
      // Get this guy up to speed on which WUs are done.
      {
        CWUsCompleted *pWUsCompleted = m_WUsCompletedCS.Lock();
        m_WUSCompletedMessageBuffer.setLen(0);
        BuildWUsCompletedMessage(pWUsCompleted->m_Completed,
                                 m_WUSCompletedMessageBuffer);
        m_WUsCompletedCS.Unlock();
      }

      VMPI_SendData(m_WUSCompletedMessageBuffer.data,
                    m_WUSCompletedMessageBuffer.getLen(), iSource);
  ...
  bool Thread_HandleWorkUnitResults(WUIndexType iWorkUnit) {
    if (m_WorkUnitWalker.Thread_IsWorkUnitCompleted(iWorkUnit)) {
      ...
      // We need the lock on here because our own worker threads can call into
      // here.
      CWUsCompleted *pWUsCompleted = m_WUsCompletedCS.Lock();
  ...
  virtual bool HandlePacket(MessageBuffer *pBuf, int iSource,
 ...
 public:
  ...
  class CWorkersReady {
   public:
    CUtlVector<int> m_WorkersReady;  // The list of workers who have said
  ...
  class CWUsCompleted {
   public:
    CUtlVector<WUIndexType>
        m_Completed;  // WUs completed that we have sent to workers.
    CUtlVector<WUIndexType>
        m_Pending;  // WUs completed that we haven't sent to workers.
  };
  CCriticalSectionData<CWUsCompleted> m_WUsCompletedCS;
  MessageBuffer
      m_WUSCompletedMessageBuffer;  // Used to send lists of completed WUs.
...
class CDistributor_SDKWorker : public IWorkUnitDistributorWorker,
                               public IShuffleRequester {
 public:
  ...
  virtual bool GetNextWorkUnit(WUIndexType *pWUIndex) {
    // If we don't have an ID yet, we haven't received a Shuffle() command, so
    // we're waiting for that before working.
    // TODO: we could do some random WUs here while we're waiting, although that
    // could suck if the WUs take forever to do
    //       and they're duplicates.
    if (m_iMyWorkUnitWalkerID == -1) return false;

    // Look in our current shuffled list of work units for the next one.
    return m_WorkUnitWalker.Thread_GetNextWorkUnit(m_iMyWorkUnitWalkerID,
                                                   pWUIndex);
  }
