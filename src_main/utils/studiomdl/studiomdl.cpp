...
//
// Generates a studio .mdl file from a .qc script
...
char g_path[1024];
Vector g_vecMinWorldspace =
    Vector(MIN_COORD_INTEGER, MIN_COORD_INTEGER, MIN_COORD_INTEGER);
Vector g_vecMaxWorldspace =
    Vector(MAX_COORD_INTEGER, MAX_COORD_INTEGER, MAX_COORD_INTEGER);
...
void EnsureDependencyFileCheckedIn(const char *pFileName) {
  ...
  char bufCanonicalPath[SOURCE_MAX_PATH] = {0};
  PathCanonicalize(bufCanonicalPath, pFullPath);
  CP4AutoAddFile p4_add_dep_file(bufCanonicalPath);
}

void StudioMdl_ScriptLoadedCallback(char const *pFilenameLoaded,
...
void CreateMakefile_OutputMakefile(void) {
  ...
  fprintf(fp, "\n");
  char mkdirpath[SOURCE_MAX_PATH];
...
void MdlError(const char *fmt, ...) {
  static char output[1024];
  static char *knownExtensions[] = {".mdl",      ".ani",      ".phy", ".sw.vtx",
                                    ".dx80.vtx", ".dx90.vtx", ".vvd"};
  ...
  if (g_bHasModelName) {
    ...
    for (int i = 0; i < SOURCE_ARRAYSIZE(knownExtensions); i++) {
...
// process immediately.
LONG SOURCE_STDCALL
MdlSecondExceptionFilter(struct _EXCEPTION_POINTERS *ExceptionInfo) {
...
#define ERR_RECORD(name) \
  ...
  int nErrors = sizeof(errors) / sizeof(errors[0]);
  {
    int i;
    for (i = 0; i < nErrors; i++) {
      if (errors[i].code == code) MdlHandleCrash(errors[i].pReason, true);
    }
...
// Key value block
//-----------------------------------------------------------------------------
static void AppendKeyValueText(CUtlVector<char> *pKeyValue,
                               const char *pString) {
...
s_sourceanim_t *FindSourceAnim(s_source_t *pSource, const char *pAnimName) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!Q_stricmp(pAnimName, pAnim->animationname)) return pAnim;
  }
  return NULL;
}

const s_sourceanim_t *FindSourceAnim(const s_source_t *pSource,
                                     const char *pAnimName) {
  if (!pAnimName[0]) return NULL;
...
s_sourceanim_t *FindOrAddSourceAnim(s_source_t *pSource,
                                    const char *pAnimName) {
  if (!pAnimName[0]) return NULL;
...
int LookupTexture(const char *pTextureName, bool bRelativePath) {
  char pTextureNoExt[SOURCE_MAX_PATH];
  char pTextureBase[SOURCE_MAX_PATH];
  char pTextureBase2[SOURCE_MAX_PATH];
  ...
  for (i = 0; i < g_numtextures; i++) {
    if (g_texture[i].flags == nFlags) {
      if (!Q_stricmp(pTextureNoExt, g_texture[i].name)) return i;
      continue;
    }

    // Comparing relative vs non-relative
    if (bRelativePath) {
      if (!Q_stricmp(pTextureBase, g_texture[i].name)) return i;
      continue;
    }

    // Comparing non-relative vs relative
    Q_FileBase(g_texture[i].name, pTextureBase2, sizeof(pTextureBase2));
    if (!Q_stricmp(pTextureNoExt, pTextureBase2)) return i;
  }
...
void SetSkinValues() {
  ...
  printf ("adjusted width: %i height: %i  top : %i  left: %i\n",
                  pmesh->skinwidth, pmesh->skinheight, pmesh->skintop,
  pmesh->skinleft );
  */
}
...
void Build_Reference(s_source_t *pSource, const char *pAnimName) {
  ...
  for (i = 0; i < pSource->numbones; i++) {
    ...
    if (parent == -1) {
      ...
      // TODO(d.rattman): Hey, it's orthogical so inv(A) == transpose(A)
      Assert(parent < i);
...
void Cmd_Eyeposition(void) {
  ...
  // axis
  //	TODO(d.rattman): these coords are bogus
  GetToken(false);
...
void Cmd_Illumposition(void) {
  ...
  if (TokenAvailable()) {
    ...
    Q_strncpy(g_attachment[g_numattachments].name, "__illumPosition",
              sizeof(g_attachment[g_numattachments].name));
    Q_strncpy(g_attachment[g_numattachments].bonename, token,
              sizeof(g_attachment[g_numattachments].bonename));
    AngleMatrix(QAngle(0, 0, 0), illumposition,
                g_attachment[g_numattachments].local);
...
void ProcessModelName(const char *pModelName) {
  ...
  char *pTmpBuf =
      reinterpret_cast<char *>(alloca((nModelNameLen + 1) * sizeof(char)));
  Q_strncpy(pTmpBuf, pModelName, nModelNameLen);
  Q_StripExtension(pTmpBuf, pTmpBuf, nModelNameLen);
  // write.cpp strips extension then adds .mdl and writes that into
  // studiohdr_t::name
  ...
  if (Q_strlen(pTmpBuf) + 4 >= (sizeof(shdr.name) / sizeof(shdr.name[0]))) {
    MdlError(
        "Model Name \"%s.mdl\" Too Big, %d Characters, Max %d Characters\n",
        pTmpBuf, Q_strlen(pTmpBuf) + 4,
...
//-----------------------------------------------------------------------------
void ProcessOptionStudio(s_model_t *pmodel, const char *pFullPath,
                         CDmeSourceSkin *pSkin) {
  ...
  // load source
  pmodel->source =
      Load_Source(pmodel->filename, "", pSkin->m_bFlipTriangles, true);

  // Reset currentscale to whatever global we currently have set
  // g_defaultscale gets set in Cmd_ScaleUp everytime the $scale command is
  // used.
...
void Option_Studio(s_model_t *pmodel) {
  ...
  if (ParseOptionStudio(pSourceSkin)) {
    ProcessOptionStudio(pmodel, pSourceSkin->GetRelativeFileName(),
                        pSourceSkin);
  }
...
int Option_Blank() {
  ...
  g_bodypart[g_numbodyparts].pmodel[g_bodypart[g_numbodyparts].nummodels] =
      g_model[g_nummodels];
...
void Cmd_Bodygroup() {
  ...
  if (g_numbodyparts == 0) {
    ...
    g_bodypart[g_numbodyparts].base = g_bodypart[g_numbodyparts - 1].base *
                                      g_bodypart[g_numbodyparts - 1].nummodels;
  }
...
void AddBodyFlexFetchRule(
    s_source_t *pSource, s_flexrule_t *pRule, int rawIndex,
  ...
  const int globalFlexControllerIndex =
      pRemapSourceIndexToGlobalFlexControllerIndex[remapSourceIndex];

  // Get the Remap record
  s_flexcontrollerremap_t &remap =
      pSource->m_FlexControllerRemaps[remapSourceIndex];
  switch (remap.m_RemapType) {
    ...
    case FLEXCONTROLLER_REMAP_EYELID:
      if (remapLocalIndex == 0) {
        ...
        pRule->op[pRule->numops].d.value =
            remap.m_EyesUpDownFlexController >= 0
                ? remap.m_EyesUpDownFlexController
                : -1;
        pRule->numops++;

        pRule->op[pRule->numops].op = STUDIO_CONST;
        pRule->op[pRule->numops].d.value =
            remap.m_BlinkController >= 0 ? remap.m_BlinkController : -1;
        pRule->numops++;

        pRule->op[pRule->numops].op = STUDIO_CONST;
        pRule->op[pRule->numops].d.value =
            globalFlexControllerIndex;  // CloseLid
        pRule->numops++;
    ...
    case FLEXCONTROLLER_REMAP_2WAY:
      // A little trickier... local index 0 is on the left, local index 1 is on
      // the right Left Equivalent RemapVal( -1.0, 0.0, 0.0, 1.0 ) Right
      // Equivalent RemapVal( 0.0, 1.0, 0.0, 1.0 )
      if (remapLocalIndex == 0) {
...
void AddBodyFlexRule(
    s_source_t *pSource, s_combinationrule_t &rule, int nFlexDesc,
    const CUtlVector<int> &pRawIndexToRemapSourceIndex,
    const CUtlVector<int> &pRawIndexToRemapLocalIndex,
    const CUtlVector<int> &pRemapSourceIndexToGlobalFlexControllerIndex) {
  if (g_numflexrules >= MAXSTUDIOFLEXRULES)
    MdlError("Line %d: Too many flex rules, max %d", g_iLinecount,
             MAXSTUDIOFLEXRULES);
  ...
  if (nCombinationCount) {
    for (int j = 0; j < nCombinationCount; ++j) {
      ...
      AddBodyFlexFetchRule(pSource, pRule, rule.m_Combination[j],
                           pRawIndexToRemapSourceIndex,
                           pRawIndexToRemapLocalIndex,
...
//-----------------------------------------------------------------------------
void AddFlexControllers(s_source_t *pSource) {
  ...
  for (int i = 0; i < nRemappedControlCount; ++i) {
    ...
    for (int j = 0; j < nRemappedRawControlCount; ++j) {
      for (int k = 0; k < nRawControlCount; ++k) {
        if (remapControl.m_RawControls[j] ==
            pSource->m_CombinationControls[k].name) {
          Assert(r2s[k] == -1);
          Assert(r2l[k] == -1);
    ...
    if (remapControl.m_bIsStereo) {
      // The controls have to be named 'right_' and 'left_' and right has to be
      // first for hlfaceposer to recognize them

      // See if we can add two more flex controllers
      if ((g_numflexcontrollers + 1) >= MAXSTUDIOFLEXCTRL)
        MdlError(
            "Line %d: Too many flex controllers, max %d, cannot add split "
            "control %s from source %s",
            g_iLinecount, MAXSTUDIOFLEXCTRL, remapControl.m_Name.Get(),
            pSource->filename);
      ...
      if (g_numflexcontrollers >= MAXSTUDIOFLEXCTRL)
        MdlError(
            "Line %d: Too many flex controllers, max %d, cannot add control %s "
            "from source %s",
            g_iLinecount, MAXSTUDIOFLEXCTRL, remapControl.m_Name.Get(),
            pSource->filename);
      ...
      l2i[i] = g_numflexcontrollers;
      s_flexcontroller_t *pController =
          &g_flexcontroller[g_numflexcontrollers++];
      Q_strncpy(pController->name, remapControl.m_Name.Get(),
                sizeof(pController->name));
      Q_strncpy(pController->type, remapControl.m_Name.Get(),
                sizeof(pController->type));
    ...
    if (remapControl.m_RemapType == FLEXCONTROLLER_REMAP_NWAY ||
      ...
      if (g_numflexcontrollers >= MAXSTUDIOFLEXCTRL)
        MdlError(
            "Line %d: Too many flex controllers, max %d, cannot add value "
            "control for nWay %s from source %s",
            g_iLinecount, MAXSTUDIOFLEXCTRL, remapControl.m_Name.Get(),
            pSource->filename);
...
void AddBodyFlexRules(s_source_t *pSource) {
  ...
  for (int i = 0; i < nRemapCount; ++i) {
    ...
    if (remap.m_RemapType == FLEXCONTROLLER_REMAP_EYELID &&
        !remap.m_EyesUpDownFlexName.IsEmpty()) {
      for (int j = 0; j < g_numflexcontrollers; ++j) {
        if (!Q_strcmp(g_flexcontroller[j].name,
                      remap.m_EyesUpDownFlexName.Get())) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    AddBodyFlexRule(pSource, rule, flexKey.flexdesc,
                    pSource->m_rawIndexToRemapSourceIndex,
                    pSource->m_rawIndexToRemapLocalIndex,
...
void AddBodyFlexData(s_source_t *pSource, int imodel) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (g_numflexkeys >= MAXSTUDIOFLEXKEYS)
      MdlError(
          "Line %d: Too many flex keys, max %d, cannot add flexKey %s from "
          "source %s",
          g_iLinecount, MAXSTUDIOFLEXKEYS, key.animationname,
          pSource->filename);
...
void AddBodyAttachments(s_source_t *pSource) {
  ...
  if (nCount > 0) {
    memcpy(&g_attachment[g_numattachments], pSource->m_Attachments.Base(),
           nCount * sizeof(s_attachment_t));
...
void ProcessCmdBody(const char *pFullPath, CDmeSourceSkin *pSkin) {
  ...
  Q_strncpy(g_bodypart[g_numbodyparts].name, pSkin->m_SkinName.Get(),
...
//-----------------------------------------------------------------------------
// Purpose: Set the default root rotation so that the Y axis is up instead of
// the Z axis (for Maya)
//-----------------------------------------------------------------------------
...
void ProcessSourceComment(s_source_t *psource, const char *pCommentString) {
  if (char const *szSceneComment =
          StringAfterPrefix(pCommentString, "// SCENE=")) {
    char szScene[1024];
    Q_strncpy(szScene, szSceneComment, SOURCE_ARRAYSIZE(szScene));
...
// Processes original content file "szOriginalContentFile" that was used to
// generate data file "szDataFile"
void ProcessOriginalContentFile(char const *szDataFile,
                                char const *szOriginalContentFile) {
  ...
  if (szContentDirRootEnd && szSceneName) {
    char chScenePath[SOURCE_MAX_PATH] = {0};
    ...
    char chScenePath[SOURCE_MAX_PATH] = {0};
    Q_snprintf(chScenePath, sizeof(chScenePath) - 1, "%.*s%s",
               std::max(strrchr(szDataFile, '\\'), strrchr(szDataFile, '/')) +
                   1 - szDataFile,
               szDataFile, szOriginalContentFile);
    EnsureDependencyFileCheckedIn(chScenePath);
  } else {
    MdlWarning(
        "ProcessOriginalContentFile for '%s' cannot detect scene source file "
        "from '%s'!\n",
        szDataFile, szOriginalContentFile);
  }
}
...
static s_source_t *FindCachedSource(const char *name, const char *xext) {
  ...
  if (xext[0]) {
    ...
    Q_snprintf(g_szFilename, sizeof(g_szFilename), "%s%s.%s", cddir[numdirs],
               name, xext);
    for (i = 0; i < g_numsources; i++) {
      if (!Q_stricmp(g_szFilename, g_source[i]->filename)) return g_source[i];
    }
  } else {
    // we don't know what extension to use, so look for all of 'em.
    Q_snprintf(g_szFilename, sizeof(g_szFilename), "%s%s.vrm", cddir[numdirs],
               name);
    for (i = 0; i < g_numsources; i++) {
      if (!Q_stricmp(g_szFilename, g_source[i]->filename)) return g_source[i];
    }
    ...
    Q_snprintf(g_szFilename, sizeof(g_szFilename), "%s%s.dmx", cddir[numdirs],
               name);
    for (i = 0; i < g_numsources; i++) {
      if (!Q_stricmp(g_szFilename, g_source[i]->filename)) return g_source[i];
    }
    Q_snprintf(g_szFilename, sizeof(g_szFilename), "%s%s.xml", cddir[numdirs],
               name);
    for (i = 0; i < g_numsources; i++) {
      if (!Q_stricmp(g_szFilename, g_source[i]->filename)) return g_source[i];
    }
    Q_snprintf(g_szFilename, sizeof(g_szFilename), "%s%s.obj", cddir[numdirs],
               name);
    for (i = 0; i < g_numsources; i++) {
      if (!Q_stricmp(g_szFilename, g_source[i]->filename)) return g_source[i];
    }
...
//-----------------------------------------------------------------------------
s_source_t *Load_Source(const char *name, const char *ext, bool reverse,
                        bool isActiveModel) {
  ...
  if (pSource) {
    if (isActiveModel) pSource->isActiveModel = true;
  ...
  char const *load_extensions[] = {"vrm", "smd", "sma", "phys",
                                   "vta", "obj", "dmx", "xml"};
  int (*load_procs[])(s_source_t *) = {Load_VRM, Load_SMD, Load_SMD, Load_SMD,
                                       Load_VTA, Load_OBJ, Load_DMX, Load_DMX};

  Assert(SOURCE_ARRAYSIZE(load_extensions) == SOURCE_ARRAYSIZE(load_procs));
  for (int kk = 0; kk < SOURCE_ARRAYSIZE(load_extensions); ++kk) {
    if ((!result && xext[0] == '\0') ||
        Q_stricmp(xext, load_extensions[kk]) == 0) {
      Q_snprintf(g_szFilename, sizeof(g_szFilename), "%s%s.%s", cddir[numdirs],
                 pTempName, load_extensions[kk]);
      strcpyn(g_source[g_numsources]->filename, g_szFilename);
  ...
  // copy over default settings of when the model was loaded (since there's no
  // actual animation for some of the systems)
...
s_sequence_t *LookupSequence(const char *name) {
  ...
  for (i = 0; i < g_sequence.Count(); ++i) {
    if (!Q_stricmp(g_sequence[i].name, name)) return &g_sequence[i];
  }
  return NULL;
}
...
s_animation_t *LookupAnimation(const char *name) {
  ...
  for (i = 0; i < g_numani; i++) {
    if (!Q_stricmp(g_panimation[i]->name, name)) return g_panimation[i];
  }
...
int ParseCmdlistToken(int &numcmds, s_animcmd_t *cmds) {
  ...
  } else if (stricmp("presubtract", token) ==
             0)  // TODO(d.rattman): rename this to something better
  {
    pcmd->cmd = CMD_SUBTRACT;
...
bool ParseAnimationToken(s_animation_t *panim) {
  ...
  if (!Q_stricmp("rotate", token)) {
    GetToken(false);
    // TODO(d.rattman): broken for Maya
  ...
  if (!Q_strnicmp("frame", token, 5)) {
    ...
    s_sourceanim_t *pSourceAnim =
        FindSourceAnim(panim->source, panim->animationname);
    if (pSourceAnim) {
  ...
  if (ParseCmdlistToken(panim->numcmds, panim->cmds)) return true;
  ...
  if (!Q_stricmp("cmdlist", token)) {
    ...
    if (i == g_numcmdlists) TokenError("unknown cmdlist %s\n", token);
...
//-----------------------------------------------------------------------------
// Purpose: create named order dependant s_animcmd_t blocks, used as replicated
// token list for $animations
//-----------------------------------------------------------------------------
...
void Cmd_Cmdlist() {
  ...
  while (1) {
    ...
    if (stricmp("{", token) == 0) {
      ...
      depth--;
    } else if (ParseCmdlistToken(g_cmdlist[g_numcmdlists].numcmds,
                                 g_cmdlist[g_numcmdlists].cmds)) {
    } else {
...
void Cmd_Animation() {
  ...
  if (panim->source->m_Animations.Count()) {
    ...
    panim->startframe = pSourceAnim->startframe;
    panim->endframe = pSourceAnim->endframe;
    Q_strncpy(panim->animationname, pSourceAnim->animationname,
              sizeof(panim->animationname));
  } else {
    panim->startframe = 0;
    panim->endframe = 0;
...
//-----------------------------------------------------------------------------
s_animation_t *Cmd_ImpliedAnimation(s_sequence_t *psequence,
                                    const char *filename) {
  // allocate animation entry
  g_panimation[g_numani] = (s_animation_t *)kalloc(1, sizeof(s_animation_t));
  g_panimation[g_numani]->index = g_numani;
  ...
  if (panim->source->m_Animations.Count()) {
    s_sourceanim_t *pSourceAnim = &panim->source->m_Animations[0];
    Q_strncpy(panim->animationname,
              panim->source->m_Animations[0].animationname,
              sizeof(panim->animationname));
    if (panim->startframe < pSourceAnim->startframe) {
      panim->startframe = pSourceAnim->startframe;
    }
...
//-----------------------------------------------------------------------------
// Purpose: copy globally reavent $animation options from one $animation to
// another
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void ProcessSequence(s_sequence_t *pseq, int numblends,
                     s_animation_t **animations, bool isAppend) {
  if (isAppend) return;
    ...
    } else if (stricmp("post", token) == 0)  // remove
    {
      ...
      while (TokenAvailable()) {
        ...
        if (stricmp("xfade", token) == 0) {
          ...
          GetToken(false);
          pseq->autolayer[pseq->numautolayers].pose =
              LookupPoseParameter(token);
...
void Cmd_Continue() {
  ...
  if (pseq) {
    GetToken(true);
    UnGetToken();
    if (token[0] != '$') ParseSequence(pseq, true);
    return;
  } else {
    s_animation_t *panim = LookupAnimation(token);
    ...
    if (panim) {
      GetToken(true);
      UnGetToken();
      if (token[0] != '$') ParseAnimation(panim, true);
      return;
    }
  }
...
void Option_Weightlist(s_weightlist_t *pweightlist) {
  ...
  while (1) {
    ...
    if (stricmp("{", token) == 0) {
      ...
      if (pweightlist->boneweight[i] == 0 &&
          pweightlist->boneposweight[i] > 0) {
        MdlError(
            "Non-zero Position weight with zero Rotation weight not allowed "
            "\'%s:%s %f %f\'\n",
            pweightlist->name, pweightlist->bonename[i],
            pweightlist->boneweight[i], pweightlist->boneposweight[i]);
      }
    } else {
...
void Option_Eyeball(s_model_t *pmodel) {
  ...
  for (i = 0; i < pmodel->source->nummeshes; i++) {
    j = pmodel->source
            ->meshindex[i];  // meshes are internally stored by material index

    if (j == mesh_material) {
      eyeball->mesh = i;  // TODO(d.rattman): should this be pre-adjusted?
      break;
    }
  }
  ...
  VectorITransform(tmp, pmodel->source->boneToPose[eyeball->bone],
                   eyeball->org);
  ...
  VectorIRotate(tmp, pmodel->source->boneToPose[eyeball->bone],
                eyeball->forward);
...
void Option_Flex(char *name, char *vtafile, int imodel, float pairsplit) {
  ...
  if (g_numflexkeys > 1) {
    if (g_flexkey[g_numflexkeys - 1].flexdesc ==
        g_flexkey[g_numflexkeys].flexdesc) {
      g_flexkey[g_numflexkeys - 1].target2 =
          g_flexkey[g_numflexkeys - 1].target1;
  ...
  if (pSource->m_Animations.Count()) {
    Q_strncpy(g_flexkey[g_numflexkeys].animationname,
              pSource->m_Animations[0].animationname,
...
int FindSourceFlexKey(s_source_t *pSource, const char *pName) {
  int nCount = pSource->m_FlexKeys.Count();
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(pSource->m_FlexKeys[i].animationname, pName)) return i;
  }
  return -1;
}
...
//-----------------------------------------------------------------------------
void AddFlexKey(s_source_t *pSource, CDmeCombinationOperator *pComboOp,
                const char *pFlexKeyName) {
  ...
  key.flexpair = pComboOp->IsDeltaStateStereo(
      pFlexKeyName);  // Signal used by AddBodyFlexData
}

//-----------------------------------------------------------------------------
// Adds combination data to the source
//-----------------------------------------------------------------------------
void AddCombination(s_source_t *pSource,
                    CDmeCombinationOperator *pCombination) {
  ...
  for (int i = 0; i < nControlCount; ++i) {
    ...
    Q_strncpy(control.name, pCombination->GetRawControlName(i),
              sizeof(control.name));
  }
  ...
  for (int i = 0; i < nControlCount; ++i) {
    ...
    if (pCombination->IsEyelidControl(i)) {
      ...
      remap.m_EyesUpDownFlexName =
          pEyesUpDownFlexName ? pEyesUpDownFlexName : "eyes_updown";
    } else {
      switch (nRemapCount) {
        case 0:
        case 1:
...
void Option_Eyelid(int imodel) {
  ...
  if (pSource->m_Animations.Count() > 0) {
    Q_strncpy(g_flexkey[g_numflexkeys + 0].animationname,
              pSource->m_Animations[0].animationname,
  ...
  Q_strncpy(g_flexkey[g_numflexkeys + 1].animationname,
            g_flexkey[g_numflexkeys + 0].animationname,
  ...
  Q_strncpy(g_flexkey[g_numflexkeys + 2].animationname,
            g_flexkey[g_numflexkeys + 0].animationname,
  ...
  for (int i = 0; i < pmodel->numeyeballs; i++) {
    ...
    if (szEyeball[0] != '\0') {
      if (stricmp(peyeball->name, szEyeball) != 0) continue;
    }
...
int Option_Mouth(s_model_t *pmodel) {
  ...
  if (index >= g_nummouths) g_nummouths = index + 1;
...
void Option_Flexrule(s_model_t *pmodel, char *name) {
  ...
  for (k = 0; k < i; k++) {
    ...
    switch (stream[k].op) {
      ...
      case STUDIO_MUL:
      case STUDIO_DIV:
        // pop all operators off of the stack that have equal or higher
        // precedence
        while (j > 0 &&
               precedence[stream[k].op] <= precedence[stack[j - 1].op]) {
          pRule->op[pRule->numops++] = stack[j - 1];
          j--;
        }
        // push operator onto the stack
...
void Cmd_Model() {
  ...
  while (1) {
    ...
    if (!Q_stricmp("{", token)) {
      ...
      Option_Flex(FAC, vtafile, g_nummodels,
                  0.0);  // TODO(d.rattman): this needs to point to a model
                         // used, not loaded!!!
      ...
      Option_Flex(FAC, vtafile, g_nummodels,
                  split);  // TODO(d.rattman): this needs to point to a model
                           // used, not loaded!!!
  ...
  // Actually connect up the expressions between the Dme Flex Controllers & Flex
  // Descriptors In case there was data added by some other eyeball command
  // (like eyelid)
  AddBodyFlexRules(g_model[g_nummodels]->source);
...
void Cmd_Controller(void) {
  if (GetToken(false)) {
    ...
    if (GetToken(false)) {
      ...
      GetToken(false);
      if ((g_bonecontroller[g_numbonecontrollers].type =
               lookupControl(token)) == -1) {
...
void Cmd_ScreenAlign(void) {
  if (GetToken(false)) {
    ...
    g_screenalignedbone[g_numscreenalignedbones].flags =
...
void Cmd_Hitbox() {
  ...
  memset(set->hitbox[set->numhitboxes].hitboxname, 0,
         sizeof(set->hitbox[set->numhitboxes].hitboxname));
...
char *GetSurfaceProp(const char *pJointName) {
  while (pJointName) {
    ...
    if (pSurfaceProp) return pSurfaceProp;
...
void ConsistencyCheckSurfaceProp() {
  for (int i = s_JointSurfaceProp.Count(); --i >= 0;) {
    ...
    if (j < 0) {
      MdlWarning(
          "You specified a joint surface property for joint\n"
...
int GetContents(const char *pJointName) {
  while (pJointName) {
    ...
    if (nContents != -1) return nContents;
...
void ConsistencyCheckContents() {
  for (int i = s_JointContents.Count(); --i >= 0;) {
    ...
    if (j < 0) {
      MdlWarning(
          "You specified a joint contents for joint\n"
          "    \"%s\" which either doesn't exist or was optimized out.\n",
          s_JointSurfaceProp[i].m_pJointName);
    }
  }
...
void Cmd_Attachment() {
  ...
  while (TokenAvailable()) {
    ...
    if (stricmp(token, "absolute") == 0) {
      ...
      TokenError("unknown attachment (%s) option: ",
                 g_attachment[g_numattachments].name, token);
    }
  }
...
static void Cmd_ReplaceModel(LodScriptData_t &lodData) {
  ...
  if (!lodData.IsStrippedFromModel()) {
    newReplacement.m_pSource =
        Load_Source(newReplacement.GetDstName(), "smd", reverse, false);
  } else if (!g_quiet) {
    printf("Stripped lod \"%s\" @ %.1f\n", newReplacement.GetDstName(),
           lodData.switchValue);
  }
}
...
//-----------------------------------------------------------------------------
// Parse replacebone command, causes a part of an LOD model to use a different
// bone
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Parse bonetreecollapse command, causes the entire subtree to use the same
// bone as the node
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Parse replacematerial command, causes a material to be used in place of
// another
//-----------------------------------------------------------------------------
...
void Cmd_LOD(const char *cmdname) {
  if (gflags & STUDIOHDR_FLAGS_HASSHADOWLOD) {
    MdlError(
        "Model can only have one $shadowlod and it must be the last lod in the "
        ".qc (%d) : %s\n",
  ...
  GetToken(true);
  if (stricmp("{", token) != 0) {
    MdlError("\"{\" expected while processing %s (%d) : %s", cmdname,
             g_iLinecount, g_szLine);
  }
  ...
  while (1) {
    ...
    if (stricmp("replacemodel", token) == 0) {
      ...
      MdlError("invalid input while processing %s (%d) : %s", cmdname,
               g_iLinecount, g_szLine);
    }
  }
...
void Cmd_ShadowLOD(void) {
  ...
  // Mark .mdl as having shadow lod (we also check above that we have only one
  // of these and that it's the last entered lod )
...
//-----------------------------------------------------------------------------
// Indicates the model be rendered with ambient boost heuristic (first used on
// Alyx in Episode 1)
//-----------------------------------------------------------------------------
void Cmd_AmbientBoost() { gflags |= STUDIOHDR_FLAGS_AMBIENT_BOOST; }

//-----------------------------------------------------------------------------
// Indicates the model should not cast shadows (useful for first-person models
// as used in L4D)
//-----------------------------------------------------------------------------
void Cmd_DoNotCastShadows() { gflags |= STUDIOHDR_FLAGS_DO_NOT_CAST_SHADOWS; }

//-----------------------------------------------------------------------------
// Indicates the model should cast texutre-based shadows in vrad (NOTE: only
// applicable to prop_static)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Indicates the model should not fade out even if the level or fallback
// settings say to
//-----------------------------------------------------------------------------
void Cmd_NoForcedFade() { gflags |= STUDIOHDR_FLAGS_NO_FORCED_FADE; }

//-----------------------------------------------------------------------------
// Indicates the model should not use the bone origin when calculating bboxes,
// sequence boxes, etc.
//-----------------------------------------------------------------------------
void Cmd_SkipBoneInBBox() { g_bUseBoneInBBox = false; }

//-----------------------------------------------------------------------------
// Indicates the model will lengthen the viseme check to always include two
// phonemes
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Indicates the model should keep pre-defined bone lengths regardless of
// animation changes
//-----------------------------------------------------------------------------
void Cmd_LockBoneLengths() { g_bLockBoneLengths = true; }

//-----------------------------------------------------------------------------
// Indicates the model should replace pre-defined bone lengths and default
// orientations
//-----------------------------------------------------------------------------
void Cmd_UnlockDefineBones() { g_bOverridePreDefinedBones = true; }
...
//-----------------------------------------------------------------------------
// The bones should be moved so that they center themselves on the verts they
// own.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: insert a virtual bone between a child and parent (currently
// unsupported)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: specify a bone to allow > 180 but < 360 rotation (forces a
// calculated "mid point" to rotation)
//-----------------------------------------------------------------------------
...
void Cmd_LimitRotation() {
  ...
  while (TokenAvailable()) {
    // sequence name
    GetToken(false);
    strcpyn(g_limitrotation[g_numlimitrotation]
                .sequencename[g_limitrotation[g_numlimitrotation].numseq++],
            token);
  }
...
void Grab_AxisInterpBones() {
  ...
  while (GetLineInput()) {
    ...
    int i = sscanf(g_szLine,
                   "%1023s \"%[^\"]\" \"%[^\"]\" \"%[^\"]\" \"%[^\"]\" %d", cmd,
                   pBone->bonename, tmp, pBone->controlname, tmp, &pBone->axis);
    if (i == 6 && stricmp(cmd, "bone") == 0) {
      // printf( "\"%s\" \"%s\" \"%s\" \"%s\"\n", cmd, pBone->bonename, tmp,
      // pBone->controlname );
      pAxis = pBone;
      ...
      i = sscanf(g_szLine, "basepos %f %f %f", &basepos.x, &basepos.y,
                 &basepos.z);
      // skip all type info
      ...
      QAngle rot;
      int j;
      i = sscanf(g_szLine, "axis %d %f %f %f %f %f %f", &j, &pos[0], &pos[1],
                 &pos[2], &rot[2], &rot[0], &rot[1]);
      if (i == 7) {
...
bool Grab_AimAtBones() {
  ...
  // Already know it's <aimconstraint> in the first string, otherwise wouldn't
  // be here
  if (sscanf(g_szLine, "%*s %127s %127s %127s", pAimAtBone->bonename,
             pAimAtBone->parentname, pAimAtBone->aimname) == 3) {
    g_numaimatbones++;
    ...
    while (GetLineInput()) {
      ...
      if (sscanf(g_szLine, "%1024s %f %f %f", cmd, &vector[0], &vector[1],
                 &vector[2]) != 4) {
        // Allow blank lines to be skipped without error
        bool allSpace(true);
        for (const char *pC(g_szLine); *pC != '\0' && pC < (g_szLine + 4096);
             ++pC) {
          if (!isspace(*pC)) {
            allSpace = false;
            break;
...
void Grab_QuatInterpBones() {
  ...
  while (GetLineInput()) {
    ...
    int i =
        sscanf(g_szLine, "%s %s %s %s %s", cmd, pBone->bonename,
               pBone->parentname, pBone->controlparentname, pBone->controlname);
    ...
    while (i == 4 && stricmp(cmd, "<aimconstraint>") == 0) {
      ...
      // Grab_AimAtBones will read input into g_szLine same as here until it
      // gets a line it doesn't understand, at which point it will exit leaving
      // that line in g_szLine, so check for the end and scan the current buffer
      // again and continue on with the normal QuatInterpBones process

      i = sscanf(g_szLine, "%s %s %s %s %s", cmd, pBone->bonename,
                 pBone->parentname, pBone->controlparentname,
                 pBone->controlname);
    }
    ...
    if (i == 5 && stricmp(cmd, "<helper>") == 0) {
      ...
      // There was a bug before which could cause the same command to be parsed
      // twice because if the sscanf above completely fails, it will return 0
      // and not change the contents of cmd, so i should be greater than 0 in
      // order for any of these checks to be valid... Still kind of buggy as
      // these checks do case insensitive stricmp but then the sscanf does case
      // sensitive matching afterwards... Should probably change those to
      ...
      if (stricmp(cmd, "<display>") == 0) {
        ...
        i = sscanf(g_szLine, "<display> %f %f %f %f", &size[0], &size[1],
                   &size[2], &distance);
        ...
        if (i == 4) {
          ...
          MdlError("Line %d: Unable to parse procedual <display> bone: %s",
                   g_iLinecount, g_szLine);
        }
      } else if (stricmp(cmd, "<basepos>") == 0) {
        i = sscanf(g_szLine, "<basepos> %f %f %f", &basepos.x, &basepos.y,
        ...
        i = sscanf(g_szLine, "%*s %f %f %f", &rotateaxis.x, &rotateaxis.y,
                   &rotateaxis.z);
        ...
        i = sscanf(g_szLine, "%*s %f %f %f", &jointorient.x, &jointorient.y,
                   &jointorient.z);
        ...
        i = sscanf(g_szLine, "<trigger> %f %f %f %f %f %f %f %f %f %f",
                   &tolerance, &trigger.x, &trigger.y, &trigger.z, &ang.x,
                   &ang.y, &ang.z, &pos.x, &pos.y, &pos.z);
        ...
        if (i == 10) {
          ...
          if (rotateaxis.x != 0.0 || rotateaxis.y != 0.0 ||
              rotateaxis.z != 0.0) {
            Quaternion q1;
            Quaternion q2;
          ...
          if (jointorient.x != 0.0 || jointorient.y != 0.0 ||
              jointorient.z != 0.0) {
            Quaternion q1;
            Quaternion q2;
          ...
          MdlError("Line %d: Unable to parse procedual <trigger> bone: %s",
                   g_iLinecount, g_szLine);
        }
      } else {
        MdlError("Line %d: Unable to parse procedual bone data: %s",
                 g_iLinecount, g_szLine);
      }
    } else {
      // Allow blank lines to be skipped without error
...
void Load_ProceduralBones() {
  ...
  if (!OpenGlobalFile(filename)) return;
...
void Cmd_CD() {
  if (cdset) MdlError("Two $cd in one model");
  cdset = true;
  GetToken(false);
...
void Cmd_MinLOD() {
  ...
  if (g_numAllowedRootLODs > 0 && g_numAllowedRootLODs < g_minLod) {
    MdlWarning(
        "$minlod %d overrides $allowrootlods %d, proceeding with "
        "$allowrootlods %d.\n",
...
void Cmd_AllowRootLODs() {
  ...
  if (g_numAllowedRootLODs > 0 && g_numAllowedRootLODs < g_minLod) {
    MdlWarning(
        "$allowrootlods %d is conflicting with $minlod %d, proceeding with "
        "$allowrootlods %d.\n",
...
void ParseScript(void) {
  while (1) {
    GetToken(true);
    if (endofscript) return;

    // Check all the commands we know about.
    int i;
    for (i = 0; i < SOURCE_ARRAYSIZE(g_Commands); i++) {
      if (!stricmp(g_Commands[i].m_pName, token)) {
        g_Commands[i].m_pCmd();
        break;
      }
    }
    if (i == SOURCE_ARRAYSIZE(g_Commands)) {
      if (!g_bCreateMakefile) {
...
bool GenerateModelName(CDmeMDLMakefile *pMDLMakeFile) {
  ...
  char pOutputFullPath[SOURCE_MAX_PATH];
  ...
  char pModelSubDir[SOURCE_MAX_PATH];
  GetModSubdirectory("models", pModelSubDir, sizeof(pModelSubDir));

  char pRelativePath[SOURCE_MAX_PATH];
  if (!Q_MakeRelativePath(pOutputFullPath, pModelSubDir, pRelativePath,
                          sizeof(pRelativePath))) {
...
bool GenerateAnimations(CDmeMDLMakefile *pMDLMakeFile) {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (!animationFiles[i]) continue;

    char pFullPath[SOURCE_MAX_PATH];
    pMDLMakeFile->GetSourceFullPath(animationFiles[i], pFullPath,
                                    sizeof(pFullPath));
    ...
    for (int j = 0; j < numblends; ++j) {
      s_sourceanim_t *pSourceAnim = FindSourceAnim(
      ...
      if (pSourceAnim) {
        ...
        if (!g_bCreateMakefile &&
            animations[j]->endframe < animations[j]->startframe) {
          TokenError("end frame before start frame in %s", animations[j]->name);
        }

        animations[j]->numframes =
            animations[j]->endframe - animations[j]->startframe + 1;
        Q_strncpy(animations[j]->animationname,
                  animationFiles[i]->m_SourceAnimationName,
                  sizeof(animations[j]->animationname));
      } else {
        MdlError("Requested unknown animation block name %s\n",
                 animationFiles[i]->m_SourceAnimationName);
      }
...
void ParseMDLMakeFile(CDmeMDLMakefile *pMDLMakeFile) {
  if (!GenerateModelName(pMDLMakeFile)) return;
  ...
  // Process bodies
  if (!GenerateSkin(pMDLMakeFile)) return;

  // Process animations
  if (!GenerateAnimations(pMDLMakeFile)) return;
}

// Used by the CheckSurfaceProps.py script.
// They specify the .mdl file and it prints out all the surface props that the
// model uses.
bool HandlePrintSurfaceProps(int &returnValue) {
  const char *pFilename =
      CommandLine()->ParmValue("-PrintSurfaceProps", (const char *)NULL);
  if (pFilename) {
    CUtlVector<char> buf;
...
bool HandleMdlReport(int &returnValue) {
  const char *pFilename =
      CommandLine()->ParmValue("-mdlreport", (const char *)NULL);
  if (pFilename) {
    CUtlVector<char> buf;
...
void UsageAndExit() {
  MdlError(
      ...
      "[-mdlreportspreadsheet] - report perf info as a comma-delimited "
      "spreadsheet\n"
      "[-striplods] - use only lod0\n"
      "[-overridedefinebones] - equivalent to specifying $unlockdefinebones in "
      ".qc file\n"
      "[-stripmodel] - process binary model files and strip extra lod data\n"
      "[-stripvhv] - strip hardware verts to match the stripped model\n"
      "[-vsi] - generate stripping information .vsi file - can be used on .mdl "
      "files too\n");
}
...
LONG SOURCE_STDCALL
VExceptionFilter(struct _EXCEPTION_POINTERS *ExceptionInfo) {
...
static bool CStudioMDLApp_SuggestGameInfoDirFn(
...
bool CStudioMDLApp::Create() {
  ...
  int nReturnValue;
  if (HandlePrintSurfaceProps(nReturnValue)) return false;
  ...
  // Add the P4 module separately so that if it is absent (say in the SDK) then
  // the other system will initialize properly
...
bool CStudioMDLApp::PreInit() {
  ...
  if (!g_pFullFileSystem || !g_pDataModel || !g_pMaterialSystem ||
      !g_pStudioRender) {
...
//-----------------------------------------------------------------------------
// Purpose: search through the "GamePath" key and create a mirrored version in
// the content path searches
//-----------------------------------------------------------------------------
...
int CStudioMDLApp::Main() {
  const bool bP4DLLExists =
      g_pFullFileSystem->FileExists("p4lib.dll", "EXECUTABLE_PATH");
  ...
  int nReturnValue;
  if (HandleMdlReport(nReturnValue)) return false;

  // Don't bother with undo here
  g_pDataModel->SetUndoEnabled(false);

  // look for the "content\hl2x" string in the qdir and add what should be the
  // correct path as an alternate
  // TODO(d.rattman): add these to an envvar if folks are using complicated
  // directory mappings instead of defaults
  ...
  // Look for the presence of a .mdl file (only -vsi is currently supported for
  // .mdl files)
  if (pExt && !Q_stricmp(pExt, "mdl")) {
    if (g_bMakeVsi) return Main_MakeVsi();
  ...
  if (pExt && !Q_stricmp(pExt, "dmx")) {
    CDmElement *pRoot;
    if (g_pDataModel->RestoreFromFile(g_path, NULL, NULL, &pRoot) !=
        DMFILEID_INVALID) {
  ...
  if (g_bCreateMakefile) {
    ...
    Q_snprintf(g_path, SOURCE_ARRAYSIZE(g_path), "%smodels/%s", gamedir,
               outname);
    Main_MakeVsi();
  }
...
//
// WriteFileToDisk
//	Equivalent to g_pFullFileSystem->WriteFile( pFileName, pPath, buf ), but
// works 	for relative paths.
//
bool WriteFileToDisk(const char *pFileName, const char *pPath,
                     CUtlBuffer &buf) {
...
//
bool WriteBufferToFile(CUtlBuffer &buf, const char *szFilebase,
                       const char *szExt) {
  char szFilename[1024];
  Q_snprintf(szFilename, SOURCE_ARRAYSIZE(szFilename), "%s%s", szFilebase,
             szExt);
...
//
bool LoadBufferFromFile(CUtlBuffer &buffer, char const *szFilebase,
                        char const *szExt, bool bError = true) {
  char szFilename[1024];
  Q_snprintf(szFilename, SOURCE_ARRAYSIZE(szFilename), "%s%s", szFilebase,
             szExt);
...
bool Load3ModelBuffers(CUtlBuffer &bufMDL, CUtlBuffer &bufVVD,
                       CUtlBuffer &bufVTX, char const *szFilebase) {
...
int CStudioMDLApp::Main_StripVhv() {
  ...
  *pExt = 0;
  ...
  CUtlBuffer bufVHV;
  if (!LoadBufferFromFile(bufVHV, g_path, ".vhv")) return 1;
...
int CStudioMDLApp::Main_MakeVsi() {
  ...
  *pExt = 0;
  ...
  CUtlBuffer bufVVD;
  CUtlBuffer bufVTX;
  if (!Load3ModelBuffers(bufMDL, bufVVD, bufVTX, g_path)) return 1;
