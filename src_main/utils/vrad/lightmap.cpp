...
void CalcFaceVectors(lightinfo_t *l) {
  ...
  if (fabs(det) < 1.0e-20) {
    Warning(
        " warning - face vectors parallel to face normal. bad lighting will be "
        "produced\n");
...
bool BuildFacesamples(lightinfo_t *pLightInfo, facelight_t *pFaceLight) {
  ...
  sample_t *samples = (sample_t *)
      sampleData;  // use a char array to speed up the debug version.
...
static void SetLightFalloffParams(entity_t *e, directlight_t *dl) {
  ...
  if (d50) {
    ...
    if (d0 < d50) {
      Warning(
          "light has _fifty_percent_distance of %f but _zero_percent_distance "
          "of %f\n",
          d50, d0);
      d0 = 2.0 * d50;
    }
...
static void ParseLightSpot(entity_t *e, directlight_t *dl) {
  ...
  if ((dl->light.stopdot == 180) && (dl->light.stopdot2 == 180)) {
    ...
    if (dl->light.stopdot > 90) {
      Warning(
          "WARNING: light_spot at (%i %i %i) has inner angle larger than 90 "
          "degrees! Clamping to 90...\n",
          (int)dl->light.origin[0], (int)dl->light.origin[1],
          (int)dl->light.origin[2]);
      dl->light.stopdot = 90;
    }
    ...
    if (dl->light.stopdot2 > 90) {
      Warning(
          "WARNING: light_spot at (%i %i %i) has outer angle larger than 90 "
          "degrees! Clamping to 90...\n",
          (int)dl->light.origin[0], (int)dl->light.origin[1],
          (int)dl->light.origin[2]);
...
bool CanLeafTraceToSky(int iLeaf) {
  ...
  for (int j = 0; j < NUMVERTEXNORMALS; j += 4) {
    ...
    delta.LoadAndSwizzle(g_anorms[j],
                         g_anorms[std::min(j + 1, NUMVERTEXNORMALS - 1)],
                         g_anorms[std::min(j + 2, NUMVERTEXNORMALS - 1)],
                         g_anorms[std::min(j + 3, NUMVERTEXNORMALS - 1)]);
...
static const char *ValueForKeyWithDefault(entity_t *ent, const char *key,
...
void CreateDirectLights(void) {
  ...
  for (i = 0; i < uiPatchCount; i++) {
    ...
    if (VectorAvg(p->baselight) >= dlight_threshold) {
      ...
      VectorScale(
          p->baselight,
          lightscale * p->area * p->scale[0] * p->scale[1] / p->basearea,
          dl->light.intensity);
...
void GatherSampleStandardLightSSE(SSE_sampleLightOutput_t &out,
  ...
  switch (dl->light.type) {
    ...
    case emit_surface:
      ...
      // move the endpoint away from the surface by epsilon to prevent hitting
      // the surface with the trace
    ...
    case emit_spotlight:
      ...
      // pow is fixed point, so this isn't the most accurate, but it doesn't
      // need to be
...
void GatherSampleLightSSE(SSE_sampleLightOutput_t &out, directlight_t *dl,
                          ...
                          float flEpsilon) {
  for (int b = 0; b < normalCount; b++) out.m_flDot[b] = Four_Zeros;
...
//-----------------------------------------------------------------------------
static void ResampleLightAt4Points(
    SSE_SampleInfo_t &info, int lightStyleIndex, int flags,
...
static int SupersampleLightAtPoint(lightinfo_t &l, SSE_SampleInfo_t &info,
  ...
  if (flags & NON_AMBIENT_ONLY) {
    ...
    for (int s = 0; s < 4; ++s) {
      ...
      // normalizing below use the number of samples used, not just numsamples
      // and some of them will be skipped if they are not inside of the winding
      ...
      // A winding should exist only if the sample wasn't a uniform luxel, or if
      // g_bDumpPatches is true.
      int invalidBits = 0;
      if (sample.w &&
...
static void BuildSupersampleFaceLights(lightinfo_t &l, SSE_SampleInfo_t &info,
  ...
  // discontinuities (large light intensity gradients) as a clue that we should
  // probably be supersampling in that area. Because the supersampling operation
  // will cause lighting changes, we've found that it's good to re-check the
  // gradients again and see if any other areas should be supersampled as a
  // result of the previous pass. Keep going until all the gradients are
  // reasonable or until we hit a max number of passes
  ...
  while (do_anotherpass && pass <= extrapasses) {
    ...
    for (int i = 0; i < info.m_pFaceLight->numsamples; ++i) {
      ...
      if (ambientSupersampleCount > 0 && directSupersampleCount > 0) {
        // Add the ambient + directional terms together, stick it back into the
        // lightmap
        for (int n = 0; n < info.m_NormalCount; ++n) {
...
static void InitSampleInfo(lightinfo_t const &l, int iThread,
  ...
  if (l.isflat) {
    ...
    // use facenormal along with the smooth normal to build the three bump map
    // vectors
...
void BuildFacelights(int iThread, int facenum) {
  ...
  if (g_pIncremental) {
    ...
    // thread A will load it in and thread B will increment it but its increment
    // will be overwritten by thread A when thread A writes it back.
    ThreadLock();
    ++g_iCurFace;
    ThreadUnlock();
  }
  ...
  for (int grp = 0; grp < numGroups; ++grp) {
    ...
    // Iterate over all the lights and add their contribution to this group of
    // spots
...
void PrecompLightmapOffsets() {
  ...
  // NOTE: We store avg face light data in this lump *before* the lightmap data
  // itself in *reverse order* of the way the lightstyles appear in the styles
  // array.
  ...
  // dlightdata since it only recomposites lighting for faces that have lights
  // that touch them.
...
void ConvertRGBExp32ToRGBA8888(const ColorRGBExp32 *pSrc, unsigned char *pDst) {
  ...
  // the colorspace version found in the engine, which *is* the same sequence
  // here
