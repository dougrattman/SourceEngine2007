...
class CBSPDispFaceListEnumerator : public ISpatialLeafEnumerator,
...
// RayEnumerator
class CBSPDispRayEnumerator : public ISpatialLeafEnumerator,
                              public IBSPTreeDataEnumerator {
 public:
  // ISpatialLeafEnumerator
  bool EnumerateLeaf(int ndxLeaf, int context);
...
class CVRadDispMgr : public IVRadDispMgr {
 public:
  ...
  // "CalcPoints"
  bool BuildDispSamples(lightinfo_t *pLightInfo, facelight_t *pFaceLight,
                        int ndxFace);
  bool BuildDispLuxels(lightinfo_t *pLightInfo, facelight_t *pFaceLight,
                       int ndxFace);
  bool BuildDispSamplesAndLuxels_DoFast(lightinfo_t *pLightInfo,
  ...
  bool SampleRadial(int ndxFace, radial_t *pRadial, Vector const &vPos,
                    int ndxLxl, LightingValue_t *pLightSample, int sampleCount,
                    bool bPatch);
  radial_t *BuildPatchRadial(int ndxFace, bool bBump);

  // utility
  void GetDispSurfNormal(int ndxFace, Vector &pt, Vector &ptNormal,
                         bool bInside);
  void GetDispSurf(int ndxFace, CVRADDispColl **ppDispTree);

  // bsp tree functions
  bool ClipRayToDisp(DispTested_t &dispTested, Ray_t const &ray);
  bool ClipRayToDispInLeaf(DispTested_t &dispTested, Ray_t const &ray,
                           int ndxLeaf);
  ...
  bool DispRayDistance_EnumerateElement(int userId,
                                        CBSPDispRayDistanceEnumerator *pEnum);
 ...
 private:
  ...
  void DispBuilderInit(CCoreDispInfo *pBuilderDisp, dface_t *pFace,
                       int ndxFace);

  //
  //  Sampling Helpers
  //
  void RadialLuxelBuild(CVRADDispColl *pDispTree, radial_t *pRadial,
                        int ndxStyle, bool bBump);
  void RadialLuxelAddSamples(int ndxFace, Vector const &luxelPt,
                             Vector const &luxelNormal, float radius,
                             radial_t *pRadial, int ndxRadial, bool bBump,
                             int lightStyle);

  void RadialPatchBuild(CVRADDispColl *pDispTree, radial_t *pRadial,
                        bool bBump);
  ...
  void GetInterestingPatchesForLuxels(int ndxFace,
                                      CUtlVector<CPatch *> &interestingPatches,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CVRadDispMgr::~CVRadDispMgr() { DestroyBSPTreeData(m_pBSPTreeData); }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRadDispMgr::DispBuilderInit(CCoreDispInfo *pBuilderDisp, dface_t *pFace,
                                   int ndxFace) {
  ...
  pBuilderDisp->SetNeighborData(pDisp->m_EdgeNeighbors,
                                pDisp->m_CornerNeighbors);
  ...
  //
  pBuilderDisp->InitDispInfo(pDisp->power, pDisp->minTess,
                             pDisp->smoothingAngle, pVerts, pTris);
}
...
void CVRadDispMgr::UnserializeDisps() {
  ...
  for (int iDisp = 0; iDisp < g_dispinfo.Count(); ++iDisp) {
    builderDisps[iDisp]->SetDispUtilsHelperInfo(builderDisps.Base(),
                                                g_dispinfo.Count());
  }
...
bool CVRadDispMgr::ClipRayToDisp(DispTested_t &dispTested, Ray_t const &ray) {
  ...
  return !m_pBSPTreeData->EnumerateLeavesAlongRay(ray, &m_EnumDispRay,
                                                  (int)&ctx);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVRadDispMgr::ClipRayToDispInLeaf(DispTested_t &dispTested,
                                       Ray_t const &ray, int ndxLeaf) {
  EnumContext_t ctx;
  ctx.m_pRay = &ray;
  ctx.m_pDispTested = &dispTested;

  return !m_pBSPTreeData->EnumerateElementsInLeaf(ndxLeaf, &m_EnumDispRay,
                                                  (int)&ctx);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRadDispMgr::ClipRayToDispInLeaf(DispTested_t &dispTested,
                                       Ray_t const &ray, int ndxLeaf,
                                       float &dist, dface_t *&pFace,
                                       Vector2D &luxelCoord) {
...
bool CVRadDispMgr::DispRay_EnumerateElement(int userId, int context) {
  ...
  CBaseTrace trace;
  trace.fraction = 1.0f;
  return (!dispTree.m_pDispTree->AABBTree_Ray(
      *pCtx->m_pRay, pCtx->m_pRay->InvDelta(), &trace, true));
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------

bool CVRadDispMgr::DispRayDistance_EnumerateElement(
    int userId, CBSPDispRayDistanceEnumerator *pCtx) {
  DispCollTree_t &dispTree = m_DispTrees[userId];
  ...
  if (dispTree.m_pDispTree->AABBTree_Ray(*pCtx->m_pRay, output)) {
    if (output.dist < pCtx->m_Distance) {
      ...
      ComputePointFromBarycentric(
          ...
          dispTree.m_pDispTree->GetLuxelCoord(output.ndxVerts[2]), output.u,
          output.v, pCtx->m_LuxelCoord);
...
bool CVRadDispMgr::DispFaceList_EnumerateLeaf(int ndxLeaf, int context) {
  ...
  for (int ndxFace = 0; ndxFace < pLeaf->numleaffaces; ndxFace++) {
    ...
    for (ndx = 0; ndx < size; ndx++) {
      if (m_EnumDispFaceList.m_FaceList[ndx] == ndxLeafFace) break;
    }
  ...
  return m_pBSPTreeData->EnumerateElementsInLeaf(ndxLeaf, &m_EnumDispFaceList,
                                                 context);
}
...
void AddSampleLightToRadial(Vector const &samplePos, Vector const &sampleNormal,
  ...
  if (bBumped) {
    if (bNeighborBumped) {
      for (int ndxBump = 0; ndxBump < (NUM_BUMP_VECTS + 1); ndxBump++) {
        pRadial->light[ndxBump][ndxRadial].AddWeighted(pSampleLight[ndxBump],
                                                       influence);
      }
      ...
      influence *= 0.05f;
      for (int ndxBump = 0; ndxBump < (NUM_BUMP_VECTS + 1); ndxBump++) {
        pRadial->light[ndxBump][ndxRadial].AddWeighted(pSampleLight[0],
                                                       influence);
      }
      pRadial->weight[ndxRadial] += influence;
    }
...
bool CVRadDispMgr::IsNeighbor(int iFace, int iNeighborFace) {
  if (iFace == iNeighborFace) return true;

  faceneighbor_t *pFaceNeighbor = &faceneighbor[iFace];
  for (int iNeighbor = 0; iNeighbor < pFaceNeighbor->numneighbors;
       iNeighbor++) {
    if (pFaceNeighbor->neighbor[iNeighbor] == iNeighborFace) return true;
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRadDispMgr::RadialLuxelAddSamples(int ndxFace, Vector const &luxelPt,
                                         Vector const &luxelNormal,
                                         float radius, radial_t *pRadial,
                                         int ndxRadial, bool bBump,
                                         int lightStyle) {
          ...
          for (int ndx = 0; ndx < count; ndx++) {
            ...
            if (pFaceLight && IsNeighbor(ndxFace, ndxFaceLight)) {
              ...
              if (pFace) {
                ...
                for (int ndxLightStyle = 0; ndxLightStyle < MAXLIGHTMAPS;
                     ndxLightStyle++) {
                ...
                if (ndxNeighborStyle == -1) continue;

                // is this surface bumped???
                bool bNeighborBump =
                    texinfo[pFace->texinfo].flags & SURF_BUMPLIGHT ? true
                                                                   : false;

                LightingValue_t sampleLight[NUM_BUMP_VECTS + 1];
                GetSampleLight(pFaceLight, ndxNeighborStyle, bNeighborBump,
                               ndxSample, sampleLight);
                AddSampleLightToRadial(pFaceLight->sample[ndxSample].pos,
                                       pFaceLight->sample[ndxSample].normal,
                                       sampleLight, radius * radius, luxelPt,
                                       luxelNormal, pRadial, ndxRadial, bBump,
                                       bNeighborBump);
              }
            }
          }
...
void CVRadDispMgr::RadialLuxelBuild(CVRADDispColl *pDispTree, radial_t *pRadial,
  ...
  for (int ndxRadial = 0; ndxRadial < radialSize; ndxRadial++) {
    RadialLuxelAddSamples(ndxFace, pFaceLight->luxel[ndxRadial],
                          pFaceLight->luxelNormals[ndxRadial], radius, pRadial,
                          ndxRadial, bBump, pFace->styles[ndxStyle]);
  }
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
radial_t *CVRadDispMgr::BuildLuxelRadial(int ndxFace, int ndxStyle,
                                         bool bBump) {
  // allocate the radial
  radial_t *pRadial = AllocateRadial(ndxFace);
  if (!pRadial) return NULL;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVRadDispMgr::SampleRadial(int ndxFace, radial_t *pRadial,
  ...
  for (int count = 0; count < sampleCount; count++) {
    ...
    if (pRadial->weight[ndxLxl] > 0.0f) {
      pLightSample[count].AddWeighted(pRadial->light[count][ndxLxl],
                                      (1.0f / pRadial->weight[ndxLxl]));
    } else {
      // error, luxel has no samples (not for patches)
      if (!bPatch) {
        // Yes, 2550 is correct!
        // pLightSample[count].Init( 2550.0f, 0.0f, 2550.0f );
        if (count == 0) bGoodSample = false;
      }
    }
  }
...
extern void PreGetBumpNormalsForDisp(texinfo_t *pTexinfo, Vector &vecU,
                                     Vector &vecV, Vector &vecNormal);
...
void AddPatchLightToRadial(Vector const &patchOrigin, Vector const &patchNormal,
  ...
  if (bBump) {
    ...
    if (bNeighborBump) {
      ...
      for (int ndxBump = 0; ndxBump < (NUM_BUMP_VECTS + 1); ndxBump++) {
        pRadial->light[ndxBump][ndxRadial].AddWeighted(pPatchLight[ndxBump],
                                                       flBumpInfluence);
      }
      ...
      for (int ndxBump = 0; ndxBump < (NUM_BUMP_VECTS + 1); ndxBump++) {
        pRadial->light[ndxBump][ndxRadial].AddWeighted(pPatchLight[0],
                                                       flBumpInfluence);
      }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRadDispMgr::RadialLuxelAddPatch(
    int ndxFace, Vector const &luxelPt, Vector const &luxelNormal, float radius,
    radial_t *pRadial, int ndxRadial, bool bBump,
    CUtlVector<CPatch *> &interestingPatches) {
  ...
  for (int i = 0; i < interestingPatches.Count(); i++) {
    ...
    bool bNeighborBump =
        texinfo[g_pFaces[pPatch->faceNumber].texinfo].flags & SURF_BUMPLIGHT
            ? true
            : false;

    Vector patchLight[NUM_BUMP_VECTS + 1];
    GetPatchLight(pPatch, bBump, patchLight);
    AddPatchLightToRadial(pPatch->origin, pPatch->normal, patchLight,
                          radius * radius, luxelPt, luxelNormal, pRadial,
                          ndxRadial, bBump, bNeighborBump);
  }
#else
  // calculate one over the voxel size
  float ooVoxelSize = 1.0f / SAMPLEHASH_VOXEL_SIZE;
    ...
    for (int ndxY = voxelMin[1]; ndxY < voxelMax[1] + 1; ndxY++) {
      for (int ndxX = voxelMin[0]; ndxX < voxelMax[0] + 1; ndxX++) {
        ...
        if (handle != g_PatchSampleHashTable.InvalidHandle()) {
          PatchSampleData_t *pPatchData =
              &g_PatchSampleHashTable.Element(handle);
          ...
          for (int ndx = 0; ndx < count; ndx++) {
            ...
            if (pPatch && pPatch->m_IterationKey != curIterationKey) {
              ...
              if (IsNeighbor(ndxFace, pPatch->faceNumber)) {
                bool bNeighborBump =
                    texinfo[g_pFaces[pPatch->faceNumber].texinfo].flags &
                            SURF_BUMPLIGHT
                        ? true
                        : false;
                ...
                Vector patchLight[NUM_BUMP_VECTS + 1];
                GetPatchLight(pPatch, bBump, patchLight);
                AddPatchLightToRadial(pPatch->origin, pPatch->normal,
                                      patchLight, radius * radius, luxelPt,
                                      luxelNormal, pRadial, ndxRadial, bBump,
                                      bNeighborBump);
              }
            }
...
void CVRadDispMgr::GetInterestingPatchesForLuxels(
    int ndxFace, CUtlVector<CPatch *> &interestingPatches,
    float patchSampleRadius) {
  facelight_t *pFaceLight = &facelight[ndxFace];
  ...
  for (int axis = 0; axis < 3; axis++) {
    allVoxelMin[axis] =
        (int)((vLuxelMin[axis] - patchSampleRadius) / SAMPLEHASH_VOXEL_SIZE);
    allVoxelMax[axis] =
        (int)((vLuxelMax[axis] + patchSampleRadius) / SAMPLEHASH_VOXEL_SIZE) +
        1;
  }
  int allVoxelSize[3] = {allVoxelMax[0] - allVoxelMin[0],
                         allVoxelMax[1] - allVoxelMin[1],
                         allVoxelMax[2] - allVoxelMin[2]};

  // Now figure out exactly which voxels these luxels touch.
  CUtlVector<unsigned char> voxelBits;
  voxelBits.SetSize(
      ((allVoxelSize[0] * allVoxelSize[1] * allVoxelSize[2]) + 7) / 8);
  memset(voxelBits.Base(), 0, voxelBits.Count());

  for (int i = 0; i < pFaceLight->numluxels; i++) {
    int voxelMin[3], voxelMax[3];
    for (int axis = 0; axis < 3; axis++) {
      voxelMin[axis] = (int)((pFaceLight->luxel[i][axis] - patchSampleRadius) /
                             SAMPLEHASH_VOXEL_SIZE);
      voxelMax[axis] = (int)((pFaceLight->luxel[i][axis] + patchSampleRadius) /
                             SAMPLEHASH_VOXEL_SIZE) +
                       1;
    }
    ...
    for (int x = voxelMin[0]; x < voxelMax[0]; x++) {
      for (int y = voxelMin[1]; y < voxelMax[1]; y++) {
        for (int z = voxelMin[2]; z < voxelMax[2]; z++) {
          int iBit =
              (z - allVoxelMin[2]) * (allVoxelSize[0] * allVoxelSize[1]) +
              (y - allVoxelMin[1]) * allVoxelSize[0] + (x - allVoxelMin[0]);
  ...
  for (int x = 0; x < allVoxelSize[0]; x++) {
    for (int y = 0; y < allVoxelSize[1]; y++) {
      for (int z = 0; z < allVoxelSize[2]; z++) {
        ...
        int iBit =
            z * (allVoxelSize[0] * allVoxelSize[1]) + y * allVoxelSize[0] + x;
        ...
        if (handle != g_PatchSampleHashTable.InvalidHandle()) {
          ...
          for (int ndx = 0; ndx < pPatchData->m_ndxPatches.Count(); ndx++) {
            ...
            // If we haven't touched the patch already and it's a valid
            // neighbor, then we want to use it.
            if (pPatch && pPatch->m_IterationKey != curIterationKey) {
              pPatch->m_IterationKey = curIterationKey;
...
void CVRadDispMgr::RadialPatchBuild(CVRADDispColl *pDispTree, radial_t *pRadial,
  ...
  int radialSize = pRadial->w * pRadial->h;
  for (int ndxRadial = 0; ndxRadial < radialSize; ndxRadial++) {
    RadialLuxelAddPatch(ndxFace, pFaceLight->luxel[ndxRadial],
                        pFaceLight->luxelNormals[ndxRadial], radius, pRadial,
                        ndxRadial, bBump, interestingPatches);
  }
}
...
void CVRadDispMgr::InsertSamplesDataIntoHashTable() {
  ...
  for (int ndxFace = 0; ndxFace < numfaces; ndxFace++) {
    dface_t *pFace = &g_pFaces[ndxFace];
    facelight_t *pFaceLight = &facelight[ndxFace];
    if (!pFace || !pFaceLight) continue;

    if (texinfo[pFace->texinfo].flags & TEX_SPECIAL) continue;
  ...
  for (int ndxFace = 0; ndxFace < numfaces; ndxFace++) {
    ...
    if (g_FacePatches.Element(ndxFace) != g_FacePatches.InvalidIndex()) {
      for (CPatch *pPatch = &g_Patches.Element(g_FacePatches.Element(ndxFace));
        ...
        if (pPatch->child1 != g_Patches.InvalidIndex()) continue;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVRadDispMgr::BuildDispSamples(lightinfo_t *pLightInfo,
                                    facelight_t *pFaceLight, int ndxFace) {
  ...
  //
  // build the samples points (based on s, t and sampleoffset (center of
  // samples); generates world space position and normal)
  //
  for (ndxV = 0; ndxV < height; ndxV++) {
    for (ndxU = 0; ndxU < width; ndxU++) {
      int ndxSample = (ndxV * width) + ndxU;
      ...
      pDispTree->DispUVToSurfPoint(pSamples[ndxSample].coord,
                                   pSamples[ndxSample].pos, 1.0f);
      pDispTree->DispUVToSurfNormal(pSamples[ndxSample].coord,
                                    pSamples[ndxSample].normal);
    }
  }
  ...
  if (!pFaceLight->sample) goto buildDispSamplesError;

  memcpy(pFaceLight->sample, pSamples,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVRadDispMgr::BuildDispLuxels(lightinfo_t *pLightInfo,
                                   facelight_t *pFaceLight, int ndxFace) {
  // get the tree assosciated with the face
  DispCollTree_t &dispTree = m_DispTrees[g_pFaces[ndxFace].dispinfo];
  CVRADDispColl *pDispTree = dispTree.m_pDispTree;
  ...
  pFaceLight->luxelNormals =
      (Vector *)calloc(pFaceLight->numluxels, sizeof(Vector));
  if (!pFaceLight->luxel || !pFaceLight->luxelNormals) return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVRadDispMgr::BuildDispSamplesAndLuxels_DoFast(lightinfo_t *pLightInfo,
  ...
  for (int ndxV = 0; ndxV < height; ndxV++) {
    for (int ndxU = 0; ndxU < width; ndxU++) {
      ...
      pDispTree->DispUVToSurfPoint(pFaceLight->sample[ndx].coord,
                                   pFaceLight->sample[ndx].pos, 1.0f);
      pDispTree->DispUVToSurfNormal(pFaceLight->sample[ndx].coord,
                                    pFaceLight->sample[ndx].normal);
