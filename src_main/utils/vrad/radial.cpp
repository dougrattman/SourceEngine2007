...
void WorldToLuxelSpace(lightinfo_t const *l, Vector const &world,
  ...
  coord[0] = DotProduct(pos, l->worldToLuxelSpace[0]) -
             l->face->m_LightmapTextureMinsInLuxels[0];
  coord[1] = DotProduct(pos, l->worldToLuxelSpace[1]) -
             l->face->m_LightmapTextureMinsInLuxels[1];
}
...
void WorldToLuxelSpace(lightinfo_t const *l, FourVectors const &world,
  ...
  coord.x =
      SubSIMD(coord.x, ReplicateX4(l->face->m_LightmapTextureMinsInLuxels[0]));
  coord.y = pos * l->worldToLuxelSpace[1];
  coord.y =
      SubSIMD(coord.y, ReplicateX4(l->face->m_LightmapTextureMinsInLuxels[1]));
  coord.z = Four_Zeros;
}
      ...
      if (area > EQUAL_EPSILON) {
        ...
        if (hasBumpmap) {
          if (neighborHasBumpmap) {
            for (bumpSample = 0; bumpSample < NUM_BUMP_VECTS + 1;
                 bumpSample++) {
              rad->light[bumpSample][i].AddWeighted(light[bumpSample], r);
            }
          } else {
            rad->light[0][i].AddWeighted(light[0], r);
            for (bumpSample = 1; bumpSample < NUM_BUMP_VECTS + 1;
                 bumpSample++) {
...
void AddBouncedToRadial(radial_t *rad, Vector const &pnt,
                        Vector2D const &coordmins, Vector2D const &coordmaxs,
...
void PatchLightmapCoordRange(radial_t *rad, int ndxPatch, Vector2D &mins,
                             Vector2D &maxs) {
  winding_t *w;
  int i;
  Vector2D coord;
...
radial_t *BuildPatchRadial(int facenum) {
  ...
  needsBumpmap =
      texinfo[g_pFaces[facenum].texinfo].flags & SURF_BUMPLIGHT ? true : false;
  ...
  if (g_FacePatches.Element(rad->facenum) != g_FacePatches.InvalidIndex()) {
    for (patch = &g_Patches.Element(g_FacePatches.Element(rad->facenum)); patch;
         patch = pNextPatch) {
      // next patch
      pNextPatch = NULL;
      ...
      //
      // displacement surface patch origin position and normal vectors have been
      // changed to represent the displacement surface position and normal --
      // for radial "blending" we need to get the base surface patch origin!
      //
      if (ValidDispFace(&g_pFaces[facenum])) {
        Vector patchOrigin;
        WindingCenter(patch->winding, patchOrigin);
        AddBouncedToRadial(rad, patchOrigin, mins, maxs,
                           patch->totallight.light, needsBumpmap, needsBumpmap);
      } else {
        AddBouncedToRadial(rad, patch->origin, mins, maxs,
                           patch->totallight.light, needsBumpmap, needsBumpmap);
      }
    }
  }
  ...
  for (j = 0; j < fn->numneighbors; j++) {
    if (g_FacePatches.Element(fn->neighbor[j]) !=
        g_FacePatches.InvalidIndex()) {
      for (patch = &g_Patches.Element(g_FacePatches.Element(fn->neighbor[j]));
        ...
        neighborNeedsBumpmap =
            texinfo[g_pFaces[facenum].texinfo].flags & SURF_BUMPLIGHT ? true
                                                                      : false;

        //
        // displacement surface patch origin position and normal vectors have
        // been changed to represent the displacement surface position and
        // normal -- for radial "blending" we need to get the base surface patch
        // origin!
        //
        if (ValidDispFace(&g_pFaces[fn->neighbor[j]])) {
          ...
          WindingCenter(patch->winding, patchOrigin);
          AddBouncedToRadial(rad, patchOrigin, mins, maxs,
                             patch->totallight.light, needsBumpmap,
                             needsBumpmap);
        } else {
          AddBouncedToRadial(rad, patch->origin, mins, maxs,
                             patch->totallight.light, needsBumpmap,
...
radial_t *BuildLuxelRadial(int facenum, int style) {
  ...
  for (int k = 0; k < fl->numsamples; k++) {
    ...
    AddDirectToRadial(rad, fl->sample[k].pos, fl->sample[k].mins,
                      fl->sample[k].maxs, light, needsBumpmap, needsBumpmap);
  }
  ...
  for (int j = 0; j < fn->numneighbors; j++) {
    ...
    if (g_pFaces[fn->neighbor[j]].styles[nstyle] !=
        g_pFaces[facenum].styles[style]) {
    ...
    for (int k = 0; k < fl->numsamples; k++) {
      if (neighborHasBumpmap) {
        for (int bumpSample = 0; bumpSample < NUM_BUMP_VECTS + 1;
             bumpSample++) {
      ...
      AddDirectToRadial(rad, fl->sample[k].pos, mins, maxs, light, needsBumpmap,
                        neighborHasBumpmap);
    }
  }
...
//-----------------------------------------------------------------------------
bool SampleRadial(radial_t *rad, Vector &pnt,
                  LightingValue_t light[NUM_BUMP_VECTS + 1],
                  int bumpSampleCount) {
  int bumpSample;
  Vector2D coord;
  ...
  for (bumpSample = 0; bumpSample < bumpSampleCount; bumpSample++) {
    ...
    if (rad->weight[i] > WEIGHT_EPS) {
      ...
      if (bumpSample == 0) baseSampleOk = false;
    }
  }
...
bool FloatLess(float const &src1, float const &src2) { return src1 < src2; }
...
void FinalLightFace(int iThread, int facenum) {
  ...
  bool needsBumpmap =
      (texinfo[f->texinfo].flags & SURF_BUMPLIGHT) ? true : false;
  ...
  for (k = 0; k < lightstyles; k++) {
    ...
    for (bumpSample = 0; bumpSample < bumpSampleCount; ++bumpSample) {
      pdata[bumpSample] =
          &(*pdlightdata)[f->lightofs + (k * bumpSampleCount + bumpSample) *
                                            fl->numluxels * 4];
    }
    ...
    for (j = 0; j < fl->numluxels; j++) {
      ...
      if (!do_fast) {
        if (!bDisp) {
          ...
          baseSampleOk = StaticDispMgr()->SampleRadial(
              facenum, rad, fl->luxel[j], j, lb, bumpSampleCount, false);
        }
      } else {
      ...
      if (prad) {
        ...
        if (!bDisp) {
          ...
          StaticDispMgr()->SampleRadial(facenum, prad, fl->luxel[j], j, v,
                                        bumpSampleCount, true);
        }
      ...
      for (bumpSample = 0; bumpSample < bumpSampleCount; bumpSample++) {
        ...
        VectorToColorRGBExp32(Vector(lb[bumpSample].m_vecLighting.x,
                                     lb[bumpSample].m_vecLighting.y,
    ...
    // Remember, the data goes *before* the specified light_ofs, in *reverse
    // order*
