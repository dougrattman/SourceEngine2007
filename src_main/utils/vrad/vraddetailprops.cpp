  ...
  switch (prop.m_Type) {
    case DETAIL_PROP_TYPE_MODEL:
      VectorMA(center, g_ModelCenterOffset[prop.m_DetailModel].x, forward,
               center);
      VectorMA(center, -g_ModelCenterOffset[prop.m_DetailModel].y, right,
               center);
      VectorMA(center, g_ModelCenterOffset[prop.m_DetailModel].z, normal,
               center);
      break;
 ...
 public:
  ...
  virtual bool EnumerateLeaf(int leaf, Ray_t const& ray, float start, float end,
    ...
    for (int i = 0; i < pLeaf->numleaffaces; ++i) {
      ...
      // Backface cull...
      if (DotProduct(pPlane->normal, ray.m_Delta) > 0) continue;
...
void CalcRayAmbientLighting(int iThread, const Vector& vStart,
  ...
  if (!surfEnum.FindIntersection(ray)) return;
...
void ComputeIndirectLightingAtPoint(Vector& position, Vector& normal,
  ...
  for (int j = 0; j < nSamples; j++) {
    ...
    if (!surfEnum.FindIntersection(ray)) continue;
    ...
    VectorMultiply(lightmapColor, dtexdata[pTex->texdata].reflectivity,
                   lightmapColor);
...
void UnserializeDetailPropLighting(
  ...
  if (g_GameLumps.GetGameLumpVersion(handle) != lumpVersion) return;
