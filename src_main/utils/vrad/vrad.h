...
struct CPatch {
  ...
  int ndxNextClusterChild;  // next terminal child index in cluster
                            //	struct		patch_s		*next;
                            //// next in
  // face 	struct		patch_s		*nextparent; // next in face
  // struct
  // patch_s		*nextclusterchild;		// next terminal child
  // in cluster
...
// normalCount, int iThread, int nLFlags = 0,
// int static_prop_to_skip=-1, float flEpsilon = 0.0 ); void
// GatherSampleAmbientSkySSE( SSE_sampleLightOutput_t &out, directlight_t *dl,
// int facenum, FourVectors const& pos, FourVectors
...
// *dl, int facenum,
// FourVectors const& pos, FourVectors *pNormals, int normalCount,
// int iThread, 						  int nLFlags =
// 0,
// //
// GATHERLFLAGS_xxx 						  int
// static_prop_to_skip=-1, float flEpsilon = 0.0 );
