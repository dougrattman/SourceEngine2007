...
static bool CompareLights(dworldlight_t *a, dworldlight_t *b) {
  ...
  bool a2 = VectorsAreEqual(a->intensity, b->intensity,
                            1.1f);  // intensities are huge numbers
...
void FileClose(long fp) {
  if (fp) g_pFileSystem->Close((FILE *)fp);
}
...
static inline void FileRead(long fp, void *pOut, int size) {
  if (g_bFileError ||
      g_pFileSystem->Read(pOut, size, (FileHandle_t)fp) != size) {
    g_bFileError = true;
    memset(pOut, 0, size);
  }
}
...
static inline void FileWrite(long fp, void const *pData, int size) {
  if (g_bFileError ||
      g_pFileSystem->Write(pData, size, (FileHandle_t)fp) != size) {
    g_bFileError = true;
  }
}
...
bool CIncremental::Init(char const *pBSPFilename,
                        char const *pIncrementalFilename) {
...
bool CIncremental::PrepareForLighting() {
  if (!m_pBSPFilename) return false;
  ...
  for (int i = m_Lights.Head(); i != m_Lights.InvalidIndex();
       i = m_Lights.Next(i))
    unmatched.AddToTail(i);

  // Match the light lists and get rid of lights that we already have all the
  // data for.
  directlight_t *pNext;
  ...
  for (directlight_t *dl = activelights; dl != NULL; dl = pNext) {
    ...
    for (iUnmatched = unmatched.Head(); iUnmatched != unmatched.InvalidIndex();
         iUnmatched = iNextUnmatched) {
  ...
  for (int iUnmatched = unmatched.Head();
       iUnmatched != unmatched.InvalidIndex();
       iUnmatched = unmatched.Next(iUnmatched)) {
    CIncLight *pLight = m_Lights[unmatched[iUnmatched]];

    // First tag faces that it touched so they get recomposited.
    for (unsigned short iFace = pLight->m_LightFaces.Head();
         iFace != pLight->m_LightFaces.InvalidIndex();
         iFace = pLight->m_LightFaces.Next(iFace)) {
...
bool CIncremental::ReadIncrementalHeader(long fp, CIncrementalHeader *pHeader) {
  int version;
  FileRead(fp, version);
  if (version != INCREMENTALFILE_VERSION) return false;

  int nFaces;
  FileRead(fp, nFaces);

  pHeader->m_FaceLightmapSizes.SetSize(nFaces);
  FileRead(fp, pHeader->m_FaceLightmapSizes.Base(),
           sizeof(CIncrementalHeader::CLMSize) * nFaces);
...
bool CIncremental::WriteIncrementalHeader(long fp) {
  ...
  for (int i = 0; i < nFaces; i++) {
    hdr.m_FaceLightmapSizes[i].m_Width =
        g_pFaces[i].m_LightmapTextureSizeInLuxels[0];
    hdr.m_FaceLightmapSizes[i].m_Height =
        g_pFaces[i].m_LightmapTextureSizeInLuxels[1];
  }

  FileWrite(fp, hdr.m_FaceLightmapSizes.Base(),
            sizeof(CIncrementalHeader::CLMSize) * nFaces);
  ...
  if (ReadIncrementalHeader(fp, &hdr)) {
    ...
    if (hdr.m_FaceLightmapSizes.Count() == numfaces) {
      ...
      for (i = 0; i < numfaces; i++) {
        if (hdr.m_FaceLightmapSizes[i].m_Width !=
                g_pFaces[i].m_LightmapTextureSizeInLuxels[0] ||
            hdr.m_FaceLightmapSizes[i].m_Height !=
                g_pFaces[i].m_LightmapTextureSizeInLuxels[1]) {
          break;
...
void CIncremental::AddLightToFace(IncrementalLightID lightID, int iFace,
                                  int iSample, int lmSize, float dot,
                                  int iThread) {
  ...
  if (pLight->m_pCachedFaces[iThread] &&
    ...
    if (bNew)
      m_TotalMemory +=
          pFace->m_LightValues.Count() * sizeof(pFace->m_LightValues[0]);
  }
...
void CompressLightData(CLightValue const *pValues, int nValues,
                       CUtlBuffer *pBuf) {
...
void MultiplyValues(CUtlVector<CLightValue> &values, float scale) {
  for (int i = 0; i < values.Count(); i++) values[i].m_Dot *= scale;
}

void CIncremental::FinishFace(IncrementalLightID lightID, int iFace,
                              int iThread) {
  CIncLight *pLight = m_Lights[lightID];
  ...
  if (pLight->m_pCachedFaces[iThread] &&
    ...
    CompressLightData(pFace->m_LightValues.Base(), pFace->m_LightValues.Count(),
...
bool CIncremental::Finalize() {
  ...
  Vector faceLight[(MAX_LIGHTMAP_DIM_WITHOUT_BORDER + 2) *
                   (MAX_LIGHTMAP_DIM_WITHOUT_BORDER + 2)];
  CUtlVector<CLightValue> faceLightValues;
  faceLightValues.SetSize((MAX_LIGHTMAP_DIM_WITHOUT_BORDER + 2) *
                          (MAX_LIGHTMAP_DIM_WITHOUT_BORDER + 2));

  // Only update the faces we've touched.
  for (int facenum = 0; facenum < numfaces; facenum++) {
    if (!m_FacesTouched[facenum] || !faceLights[facenum].Count()) continue;
    ...
    for (int iFace = 0; iFace < faceLights[facenum].Count(); iFace++) {
      ...
      for (int iSample = 0; iSample < nLuxels; iSample++) {
        ...
        if (flDot) {
          VectorMA(faceLight[iSample],
                   flDot / pFace->m_pLight->m_flMaxIntensity,
                   pFace->m_pLight->m_Light.intensity, faceLight[iSample]);
        }
      }
    }
...
bool CIncremental::Serialize() {
  if (!SaveIncrementalFile()) return false;
...
bool CIncremental::SaveIncrementalFile() {
  ...
  for (int iLight = m_Lights.Head(); iLight != m_Lights.InvalidIndex();
       iLight = m_Lights.Next(iLight)) {
    ...
    FileWrite(fp, nFaces);
    for (int iFace = pLight->m_LightFaces.Head();
         iFace != pLight->m_LightFaces.InvalidIndex();
         iFace = pLight->m_LightFaces.Next(iFace)) {
...
CLightFace *CIncLight::FindOrCreateLightFace(int iFace, int lmSize,
                                             bool *bNew) {
  if (bNew) *bNew = false;

  // Look for it.
  for (int i = m_LightFaces.Head(); i != m_LightFaces.InvalidIndex();
       i = m_LightFaces.Next(i)) {
