...
struct facelight_t {
  ...
  LightingValue_t *light[MAXLIGHTMAPS]
                        [NUM_BUMP_VECTS + 1];  // result of direct illumination,
                                               // indexed by sample
...
struct lightinfo_t {
  ...
  Vector luxelOrigin;
  Vector worldToLuxelSpace[2];  // s = (world - luxelOrigin) .
                                // worldToLuxelSpace[0], t = (world -
                                // luxelOrigin) . worldToLuxelSpace[1]
  Vector
      luxelToWorldSpace[2];  // world = luxelOrigin + s * luxelToWorldSpace[0] +
