...
#if defined(USE_SCRATCHPAD)
      ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(iCornerVert),
                                 pDisp->GetNormal(iCornerVert), Vector(0, 0, 1),
                                 25);
#endif
...
#if defined(USE_SCRATCHPAD)
          ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(iMidPoint),
                                     pDisp->GetNormal(iMidPoint),
                                     Vector(0, 1, 1), 25);
#endif
        }
      }
    ...
    for (int iEdge = 0; iEdge < 4; iEdge++) {
      ...
      for (int iSub = 0; iSub < 2; iSub++) {
        ...
        while (it.Next()) {
          // Blend the two.
          if (!it.IsLastVert()) {
            Vector vAverage = pDisp->GetNormal(it.GetVertIndex()) +
            ...
            ScratchPad_DrawArrowSimple(
                g_pPad, pDisp->GetVert(it.GetVertIndex()),
                pDisp->GetNormal(it.GetVertIndex()), Vector(1, 0, 0), 25);
#endif
          }
          ...
          for (int iTween = iPrevPos + 1; iTween < iCurPos; iTween++) {
            ...
            VectorLerp(pDisp->GetNormal(viPrevPos),
                       pDisp->GetNormal(it.GetVertIndex()), flPercent, vNormal);
...
#if defined(USE_SCRATCHPAD)
            ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(viTween),
                                       pDisp->GetNormal(viTween),
                                       Vector(1, 0.5, 0), 25);
#endif
          }
...
#if defined(USE_SCRATCHPAD)
void ScratchPad_DrawOriginalNormals(const CCoreDispInfo *pListBase,
                                    int listSize) {
  for (int i = 0; i < listSize; i++) {
    ...
    while (it.Next()) {
      ScratchPad_DrawArrowSimple(g_pPad, pDisp->GetVert(it.GetVertIndex()),
...
void SmoothNeighboringDispSurfNormals(CCoreDispInfo **ppListBase,
                                      int listSize) {
  //#if defined( USE_SCRATCHPAD )
