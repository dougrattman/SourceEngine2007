...
void BaseLightForFace(dface_t *f, Vector &light, float *parea,
  ...
  *parea = texdata->height * texdata->width;
  ...
  for (int i = 0; i < 3; i++) {
    if (reflectivity[i] > 0.99) reflectivity[i] = 0.99;
  }
}
...
bool IsSky(dface_t *f) {
  ...
  tx = &texinfo[f->texinfo];
  if (tx->flags & SURF_SKY) return true;
  return false;
}
...
entity_t *EntityForModel(int modnum) {
  ...
  for (int i = 0; i < num_entities; i++) {
    ...
    if (!strcmp(s, name)) return &entities[i];
  }
...
void SubdividePatches(void) {
  ...
  for (i = 0; i < uiPatchCount; i++) {
    ...
    patch->parent = -1;
    if (PreventSubdivision(patch)) continue;
...
float FormFactorPolyToDiff(CPatch *pPolygon, CPatch *pDifferential) {
  ...
  for (int iPoint = 0; iPoint < pWinding->numpoints; iPoint++) {
    ...
    if (flSinAlpha < -1.0f || flSinAlpha > 1.0f) return 0.0f;
...
void MakeScales(int ndxPatch, transfer_t *all_transfers) {
  ...
  if (ndxPatch == g_Patches.InvalidIndex()) return;
  CPatch *patch = &g_Patches.Element(ndxPatch);
  ...
  if (patch->numtransfers) {
    ...
    patch->transfers =
        (transfer_t *)calloc(1, patch->numtransfers * sizeof(transfer_t));
    if (!patch->transfers) Error("Memory allocation failure");
...
void WriteWorld(char *name, int iBump) {
  ...
  if (!out) Error("Couldn't open %s", name);
...
void WriteTrace(const char *pFileName, const FourRays &rays,
  ...
  if (!out) Error("Couldn't open %s", pFileName);
...
int CountClusters(void) {
  ...
  for (int i = 0; i < numleafs; i++) {
    if (dleafs[i].cluster > clusterCount) clusterCount = dleafs[i].cluster;
  }
...
void BuildFacesVisibleToLights(bool bAllVisible) {
  ...
  for (int i = 0; i < numfaces; i++) {
    if (g_FacesVisibleToLights[i >> 3] & (1 << (i & 7))) ++nFacesToProcess;
  }
}
...
void VRAD_LoadBSP(char const *pFilename) {
  ...
  if (!g_pFileSystem->FileExists(level_lights)) *level_lights = 0;
  ...
  for (i = 1; i < argc; i++) {
    if (!Q_stricmp(argv[i], "-StaticPropLighting")) {
      ...
      if (++i < argc) {
        ...
        if (luxeldensity > 1.0) luxeldensity = 1.0 / luxeldensity;
      } else {
