...
class CVRADDispColl : public CDispCollTree {
 public:
  ...
  // Patches.
  bool InitPatch(int iPatch, int iParentPatch, int iChild, Vector *pPoints,
                 int *pIndices, float &flArea);
  ...
  void DispUVToSurfPoint(Vector2D const &dispUV, Vector &vecPoint,
                         float flPushEps);
 ...
 protected:
  ...
  // Utility.
  void DispUVToSurf_TriTLToBR(Vector &vecPoint, float flPushEps, float flU,
                              float flV, int nSnapU, int nSnapV, int nWidth,
                              int nHeight);
  void DispUVToSurf_TriBLToTR(Vector &vecPoint, float flPushEps, float flU,
                              float flV, int nSnapU, int nSnapV, int nWidth,
                              int nHeight);
