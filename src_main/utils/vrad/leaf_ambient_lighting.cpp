...
void AddEmitSurfaceLights(const Vector &vStart, Vector lightBoxColor[6]) {
  ...
  for (int iLight = 0; iLight < *pNumworldlights; iLight++) {
    ...
    if (!(wl->flags & DWL_FLAGS_INAMBIENTCUBE)) continue;
    ...
    if (!TestSignSIMD(CmpGtSIMD(fractionVisible, Four_Zeros))) continue;
...
bool IsLeafAmbientSurfaceLight(dworldlight_t *wl) {
  ...
  if (wl->type != emit_surface) return false;
...
void AddSampleToList(CUtlVector<ambientsample_t> &list,
  ...
  if (list.Count() <= MAX_SAMPLES) return;
...
int NearestNeighborWithLight(int leafID) {
  ...
  for (int i = 0; i < leafList.m_list.Count(); i++) {
    ...
    if (!g_pLeafAmbientIndex->Element(testIndex).ambientSampleCount) continue;
...
void VMPI_ProcessLeafAmbient(int iThread, u64 iLeaf, MessageBuffer *pBuf) {
  CUtlVector<ambientsample_t> list;
...
//-----------------------------------------------------------------------------
void VMPI_ReceiveLeafAmbientResults(u64 leafID, MessageBuffer *pBuf,
                                    int iWorker) {
  // Decode the results.
  int nSamples;
...
void ComputePerLeafAmbientLighting() {
  ...
  for (int i = 0; i < *pNumworldlights; i++) {
    ...
    if (wl->type == emit_surface) ++nSurfaceLights;
  ...
  if (g_bUseMPI) {
    ...
    DistributeWork(numleafs, VMPI_DISTRIBUTEWORK_PACKETID,
                   VMPI_ProcessLeafAmbient, VMPI_ReceiveLeafAmbientResults);
  } else {
