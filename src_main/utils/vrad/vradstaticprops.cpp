...
class CVradStaticPropMgr : public IVradStaticPropMgr {
 ...
 private:
  // VMPI stuff.
  static void VMPI_ProcessStaticProp_Static(int iThread, u64 iStaticProp,
                                            MessageBuffer *pBuf);
  static void VMPI_ReceiveStaticPropResults_Static(u64 iStaticProp,
                                                   MessageBuffer *pBuf,
                                                   int iWorker);
 ...
 private:
  ...
  struct StaticPropDict_t {
    ...
    CUtlVector<int>
        m_triangleMaterialIndex;  // each triangle has an index if this model
                                  // casts texture shadows
  };
  ...
  void ApplyLightingToStaticProp(
      CStaticProp &prop, const CComputeStaticPropLightingResults *pResults);
...
bool IsStaticProp(studiohdr_t *pHdr) {
  if (!(pHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP)) return false;
...
//-----------------------------------------------------------------------------
inline static Vector *PositionFromIndex(
    const mstudio_meshvertexdata_t *vertData, mstudiomesh_t *pMesh,
...
class CShadowTextureList {
 public:
  ...
  unsigned char *LoadVTFRGB8888(const char *pName, int *pWidth, int *pHeight,
    ...
    CUtlBuffer buf;
    if (!LoadFileIntoBuffer(buf, szPath)) return NULL;
    IVTFTexture *pTex = CreateVTFTexture();
    if (!pTex->Unserialize(buf)) return NULL;
  ...
  void LoadAllTexturesForModel(studiohdr_t *pHdr, int *pTextureList) {
    for (int i = 0; i < pHdr->numtextures; i++) {
      ...
      for (int j = 0; j < pHdr->numcdtextures; j++) {
        ...
        Q_FixSlashes(szPath, CORRECT_PATH_SEPARATOR);
        if (FindOrLoadIfValid(szPath, &textureIndex)) break;
      }
  ...
  int SampleMaterial(int materialIndex, const Vector &coords, bool bBackface) {
    ...
    if (bBackface && !tex.allowBackface) return 0;
    Vector2D uv =
...
float ComputeCoverageFromTexture(float b0, float b1, float b2, i32 hitID) {
...
void CVradStaticPropMgr::UnserializeStaticProps() {
  ...
  if (g_GameLumps.GetGameLumpVersion(handle) != GAMELUMP_STATIC_PROPS_VERSION) {
    Error(
        "Cannot load the static props... encountered a stale map version. "
        "Re-vbsp the map.");
  }
...
void CVradStaticPropMgr::Init() {
  ...
  if (!physicsFactory) Error("Unable to load vphysics DLL.");
...
void CVradStaticPropMgr::ApplyLightingToStaticProp(
    CStaticProp &prop, const CComputeStaticPropLightingResults *pResults) {
  if (pResults->m_ColorVertsArrays.Count() == 0) return;
      ...
      for (int nLod = 0; nLod < pVtxHdr->numLODs; nLod++) {
        ...
        for (int nMesh = 0; nMesh < pStudioModel->nummeshes; ++nMesh) {
          ...
          for (int nGroup = 0; nGroup < pVtxMesh->numStripGroups; ++nGroup) {
            ...
            prop.m_MeshData[nMeshIdx].m_Verts.AddMultipleToTail(
            ...
            for (int nVertex = 0; nVertex < pStripGroup->numVerts; ++nVertex) {
              ...
              prop.m_MeshData[nMeshIdx].m_Verts[nVertex] =
...
void CVradStaticPropMgr::ComputeLighting(
  ...
  if (prop.m_Flags & STATIC_PROP_NO_PER_VERTEX_LIGHTING) return;
    ...
    for (int modelID = 0; modelID < pBodyPart->nummodels; ++modelID) {
      ...
      for (int meshID = 0; meshID < pStudioModel->nummeshes; ++meshID) {
        ...
        for (int vertexID = 0; vertexID < pStudioMesh->numvertices;
          ...
          VectorTransform(*vertData->Position(vertexID), matrix,
                          samplePosition);
          ...
          if (PositionInSolid(samplePosition)) {
            ...
            int skip_prop = -1;
            if (g_bDisablePropSelfShadowing ||
                (prop.m_Flags & STATIC_PROP_NO_SELF_SHADOWING)) {
              skip_prop = prop_index;
            }

            int nFlags = (prop.m_Flags & STATIC_PROP_IGNORE_NORMALS)
      ...
      if (badVerts.Count() &&
        ...
        for (int nBadVertex = 0; nBadVertex < badVerts.Count(); nBadVertex++) {
          ...
          while (--numIterations > 0) {
            ...
            if (PositionInSolid(midPosition)) break;
...
void CVradStaticPropMgr::VMPI_ReceiveStaticPropResults_Static(
    u64 iStaticProp, MessageBuffer *pBuf, int iWorker) {
...
void CVradStaticPropMgr::ComputeLighting(int iThread) {
  ...
  if (g_bUseMPI) {
    ...
    DistributeWork(count, VMPI_DISTRIBUTEWORK_PACKETID,
