...
CDispatchReg g_VRADDispatchReg(
    VMPI_VRAD_PACKET_ID,
    VRAD_DispatchFn);  // register to handle the messages we want
CDispatchReg g_DistributeWorkReg(VMPI_DISTRIBUTEWORK_PACKETID,
                                 DistributeWorkDispatch);
...
void VRAD_SetupMPI(int &argc, char **&argv) {
  ...
  if (!VMPI_FindArg(argc, argv, "-mpi", "") &&
      !VMPI_FindArg(argc, argv, VMPI_GetParamString(mpi_Worker), ""))
    return;
  ...
  if (!VMPI_Init(argc, argv, "dependency_info_vrad.txt", HandleMPIDisconnect,
                 mode)) {
...
void SerializeFace(MessageBuffer *pmb, int facenum) {
  ...
  if (fl->luxel) WriteValues(pmb, fl->luxel, fl->numluxels);

  if (fl->luxelNormals) WriteValues(pmb, fl->luxelNormals, fl->numluxels);
}
...
void UnSerializeFace(MessageBuffer *pmb, int facenum, int iSource) {
  ...
  if (pmb->read(f, sizeof(dface_t)) < 0)
    Error("UnSerializeFace - invalid dface_t from %s (mb len: %d, offset: %d)",
          VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());

  if (pmb->read(fl, sizeof(facelight_t)) < 0)
    Error(
        "UnSerializeFace - invalid facelight_t from %s (mb len: %d, offset: "
        "%d)",
        VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
  ...
  if (pmb->read(fl->sample, sizeof(sample_t) * fl->numsamples) < 0)
    Error(
        "UnSerializeFace - invalid sample_t from %s (mb len: %d, offset: %d, "
        "fl->numsamples: %d)",
        VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset(),
        fl->numsamples);
    ...
    for (n = 0; n < NUM_BUMP_VECTS + 1; ++n) {
      if (fl->light[i][n]) {
        ...
        if (ReadValues(pmb, fl->light[i][n], fl->numsamples) < 0)
          Error(
              "UnSerializeFace - invalid fl->light from %s (mb len: %d, "
              "offset: %d)",
              VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
      }
    }
  ...
  if (fl->luxel) {
    ...
    if (ReadValues(pmb, fl->luxel, fl->numluxels) < 0)
      Error(
          "UnSerializeFace - invalid fl->luxel from %s (mb len: %d, offset: "
          "%d)",
          VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
  }
  ...
  if (fl->luxelNormals) {
    ...
    if (ReadValues(pmb, fl->luxelNormals, fl->numluxels) < 0)
      Error(
          "UnSerializeFace - invalid fl->luxelNormals from %s (mb len: %d, "
          "offset: %d)",
          VMPI_GetMachineName(iSource), pmb->getLen(), pmb->getOffset());
  }
}

void MPI_ReceiveFaceResults(u64 iWorkUnit, MessageBuffer *pBuf, int iWorker) {
  UnSerializeFace(pBuf, iWorkUnit, iWorker);
}

void MPI_ProcessFaces(int iThread, u64 iWorkUnit, MessageBuffer *pBuf) {
...
void RunMPIBuildFacelights() {
  ...
  double elapsed = DistributeWork(numfaces, VMPI_DISTRIBUTEWORK_PACKETID,
                                  MPI_ProcessFaces, MPI_ReceiveFaceResults);
...
void MPI_ReceiveVisLeafsResults(u64 iWorkUnit, MessageBuffer *pBuf,
                                int iWorker) {
  ...
  for (int k = 0; k < patchesInCluster; ++k) {
    ...
    if (max_transfer < numtransfers) max_transfer = numtransfers;
  }
}

// Temporary variables used during callbacks. If we're going to be threadsafe,
// these should go in a structure and get passed around.
...
// The results are appended to g_VisLeafsMB and sent back to the master when all
// clusters are done.
void MPI_AddPatchData(int iThread, int patchnum, CPatch *patch) {
  ...
  if (pData->m_pVisLeafsMB) {
    ...
    pData->m_pVisLeafsMB->write(&patch->numtransfers,
                                sizeof(patch->numtransfers));
    pData->m_pVisLeafsMB->write(patch->transfers,
...
// list of clusters.
void MPI_ProcessVisLeafs(int iThread, u64 iWorkUnit, MessageBuffer *pBuf) {
  CTimeAdder adder(&g_CPUTime);
  ...
  if (pBuf) {
    ...
    pBuf->write(&pData->m_nPatchesInCluster,
                sizeof(pData->m_nPatchesInCluster));
  }

  // Collect the results in MPI_AddPatchData.
  BuildVisLeafs_Cluster(iThread, pData->m_pBuildVisLeafsTransfers, iCluster,
                        MPI_AddPatchData);

  // Now send the results back..
  if (pBuf) {
    pBuf->update(iSavePos, &pData->m_nPatchesInCluster,
                 sizeof(pData->m_nPatchesInCluster));
...
void RunMPIBuildVisLeafs() {
  ...
  if (!g_bMPIMaster ||
      VMPI_GetActiveWorkUnitDistributor() == k_eWorkUnitDistributor_SDK) {
  ...
  double elapsed =
      DistributeWork(dvis->numclusters, VMPI_DISTRIBUTEWORK_PACKETID,
                     MPI_ProcessVisLeafs, MPI_ReceiveVisLeafsResults);
...
void VMPI_DistributeLightData() {
  ...
  if (g_bMPIMaster) {
    ...
    lightFaceData.EnsureCapacity(pdlightdata->Count() +
                                 (numfaces * (MAXLIGHTMAPS + sizeof(int))));
    Q_memcpy(lightFaceData.PeekPut(), pdlightdata->Base(),
             pdlightdata->Count());
    ...
    VMPI_FileSystem_CreateVirtualFile(pVirtualFilename, lightFaceData.Base(),
                                      lightFaceData.TellMaxPut());

    char cPacketID[2] = {VMPI_VRAD_PACKET_ID,
                         VMPI_SUBPACKETID_PLIGHTDATA_RESULTS};
    VMPI_Send2Chunks(cPacketID, sizeof(cPacketID), pVirtualFilename,
                     strlen(pVirtualFilename) + 1, VMPI_PERSISTENT);
  } else {
    ...
    // Open
    FileHandle_t fp = g_pFileSystem->Open(g_LightResultsFilename.Base(), "rb",
                                          VMPI_VIRTUAL_FILES_PATH_ID);
    if (!fp)
      Error("Can't open '%s' to read lighting info.",
            g_LightResultsFilename.Base());
