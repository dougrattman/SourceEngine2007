...
//-----------------------------------------------------------------------------
void CVRADDispColl::GetMinorAxes(Vector const &vecNormal, int &nAxis0,
                                 int &nAxis1) {
  nAxis0 = 0;
  nAxis1 = 1;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRADDispColl::BaseFacePlaneToDispUV(Vector const &vecPlanePt,
                                          Vector2D &dispUV) {
  PointInQuadToBarycentric(m_vecSurfPoints[0], m_vecSurfPoints[3],
                           m_vecSurfPoints[2], m_vecSurfPoints[1], vecPlanePt,
                           dispUV);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRADDispColl::DispUVToSurfPoint(Vector2D const &dispUV, Vector &vecPoint,
                                      float flPushEps) {
  ...
  if (bOdd) {
    DispUVToSurf_TriTLToBR(vecPoint, flPushEps, flU, flV, nSnapU, nSnapV,
                           nWidth, nHeight);
  }
  // Bottom Left to Top Right
  else {
    DispUVToSurf_TriBLToTR(vecPoint, flPushEps, flU, flV, nSnapU, nSnapV,
                           nWidth, nHeight);
  }
}
...
void CVRADDispColl::DispUVToSurf_TriTLToBR(Vector &vecPoint, float flPushEps,
                                           float flU, float flV, int nSnapU,
                                           int nSnapV, int nWidth,
                                           int nHeight) {
  int nNextU = nSnapU + 1;
  int nNextV = nSnapV + 1;
  if (nNextU == nWidth) {
  ...
  if ((flFracU + flFracV) >= (1.0f + TRIEDGE_EPSILON)) {
    ...
    vecPoint = m_aVerts[nIndices[1]] + edgeU * (1.0f - flFracU) +
               edgeV * (1.0f - flFracV);
...
void CVRADDispColl::DispUVToSurf_TriBLToTR(Vector &vecPoint, float flPushEps,
  ...
  if (flFracU < flFracV) {
    ...
    vecPoint =
        m_aVerts[nIndices[1]] + edgeU * flFracU + edgeV * (1.0f - flFracV);
    ...
    vecPoint =
        m_aVerts[nIndices[2]] + edgeU * (1.0f - flFracU) + edgeV * flFracV;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVRADDispColl::DispUVToSurfNormal(Vector2D const &dispUV,
                                       Vector &vecNormal) {
  // Check to see that the point is on the surface.
  if (dispUV.x < 0.0f || dispUV.x > 1.0f || dispUV.y < 0.0f || dispUV.y > 1.0f)
    return;
...
float CVRADDispColl::CreateParentPatches(void) {
  ...
  vecPoints[1].Init(m_aVerts[(nInterval * (nInterval - 1))].x,
                    m_aVerts[(nInterval * (nInterval - 1))].y,
                    m_aVerts[(nInterval * (nInterval - 1))].z);
  vecPoints[2].Init(m_aVerts[((nInterval * nInterval) - 1)].x,
                    m_aVerts[((nInterval * nInterval) - 1)].y,
                    m_aVerts[((nInterval * nInterval) - 1)].z);
  vecPoints[3].Init(m_aVerts[(nInterval - 1)].x, m_aVerts[(nInterval - 1)].y,
                    m_aVerts[(nInterval - 1)].z);

  // Create and initialize the patch.
  int iPatch = g_Patches.AddToTail();
  if (iPatch == g_Patches.InvalidIndex()) return flTotalArea;
...
//			nLevel -
//-----------------------------------------------------------------------------
void CVRADDispColl::CreateChildPatchesFromRoot(int iParentPatch,
                                               int *pChildPatch) {
  ...
  if (flTestArea < flMinArea) return;
  ...
  if (pChildPatch[0] == g_Patches.InvalidIndex()) return;
...
void CVRADDispColl::CreateChildPatches(int iParentPatch, int nLevel) {
  ...
  if (pParentPatch->winding->numpoints == 4) {
    ...
    if (iChildPatch[0] != g_Patches.InvalidIndex() &&
        iChildPatch[1] != g_Patches.InvalidIndex()) {
  ...
  // Check to see if any more displacement verts exist - go to subdivision if
  // not.
  ...
  for (int iTri = 0; iTri < 2; ++iTri) {
    for (int iPoint = 0; iPoint < 3; ++iPoint) {
      VectorCopy(m_aVerts[nChildIndices[iTri][iPoint]],
                 vecChildPoints[iTri][iPoint]);
    }
  }
  ...
  for (int iChild = 0; iChild < 2; ++iChild) {
    ...
    float flArea = 0.0f;
    if (!InitPatch(iChildPatch[iChild], iParentPatch, iChild,
                   vecChildPoints[iChild], nChildIndices[iChild], flArea)) {
      if (iChild == 0) {
...
void CVRADDispColl::CreateChildPatchesSub(int iParentPatch) {
  ...
  switch (iLongEdge) {
    case 0: {
      ...
      vecChildPoints[0][1] =
          (pParentPatch->winding->p[0] + pParentPatch->winding->p[1]) * 0.5f;
    ...
    case 1: {
      ...
      vecChildPoints[0][2] =
          (pParentPatch->winding->p[1] + pParentPatch->winding->p[2]) * 0.5f;
    ...
    case 2: {
      ...
      vecChildPoints[0][1] = pParentPatch->winding->p[1];
      vecChildPoints[0][2] =
          (pParentPatch->winding->p[0] + pParentPatch->winding->p[2]) * 0.5f;
  ...
  for (int iChild = 0; iChild < 2; ++iChild) {
    ...
    float flArea = 0.0f;
    if (!InitPatch(iChildPatch[iChild], iParentPatch, iChild,
                   vecChildPoints[iChild], nChildIndices, flArea)) {
      if (iChild == 0) {
        pParentPatch->child1 = g_Patches.InvalidIndex();
        g_Patches.Remove(iChildPatch[iChild]);
        break;
...
//-----------------------------------------------------------------------------
bool CVRADDispColl::InitParentPatch(int iPatch, Vector *pPoints,
                                    float &flArea) {
  // Get the current patch.
  CPatch *pPatch = &g_Patches[iPatch];
  if (!pPatch) return false;
  ...
  BaseLightForFace(&g_pFaces[pPatch->faceNumber], pPatch->baselight,
                   &pPatch->basearea, pPatch->reflectivity);
...
//			flArea -
//-----------------------------------------------------------------------------
bool CVRADDispColl::InitPatch(int iPatch, int iParentPatch, int iChild,
                              Vector *pPoints, int *pIndices, float &flArea) {
  // Get the current patch.
  CPatch *pPatch = &g_Patches[iPatch];
  if (!pPatch) return false;
...
void CVRADDispColl::AddPolysForRayTrace(void) {
  ...
  for (int ndxTri = 0; ndxTri < m_aTris.Size(); ndxTri++) {
    ...
    int v[3];
    for (int ndxv = 0; ndxv < 3; ndxv++) v[ndxv] = tri->GetVert(ndxv);

    Vector fullCoverage;
    fullCoverage.x = 1.0f;
    g_RtEnv.AddTriangle(TRACE_ID_OPAQUE, m_aVerts[v[0]], m_aVerts[v[1]],
                        m_aVerts[v[2]], fullCoverage);
  }
}
