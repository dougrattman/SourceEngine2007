...
int CalcDatSize() {
  ...
  for (i = 0; i < count; i++) ret += g_Dats[i].size;
...
bool CVRadDLL::DoIncrementalLight(char const *pVMFFile) {
  char tempPath[SOURCE_MAX_PATH], tempFilename[SOURCE_MAX_PATH];
  ...
  if (!LoadEntsFromMapFile(tempFilename)) return false;
  ...
  if (RadWorld_Go()) {
    ...
    if (g_pIncremental) g_pIncremental->GetFacesTouched(g_FacesTouched);
...
bool CVRadDLL::Serialize() {
  if (!g_pIncremental) return false;
