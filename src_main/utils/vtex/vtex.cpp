...
class SmartIVtfTexture {
 ...
 public:
  IVTFTexture *Assign(IVTFTexture *pVtfNew) {
    IVTFTexture *pOld = m_p;
    m_p = pVtfNew;
    return pOld;
  }
...
static bool VTexErrorAborts() {
  if (CommandLine()->FindParm("-crcvalidate")) return false;
  ...
  if (!_stricmp(pKeyName, "skybox")) {
    ...
    if (iDecayChannel >= 0 && iDecayChannel < 4) {
      m_vtfProcOptions.flags0 |= (VtfProcessingOptions::OPT_DECAY_R |
                                  VtfProcessingOptions::OPT_DECAY_EXP_R)
                                 << iDecayChannel;
...
static ImageFormat ComputeDesiredImageFormat(IVTFTexture *pTexture,
  ...
  if (pTexture->Format() == IMAGE_FORMAT_RGB323232F) {
    ...
    if (g_bUsedAsLaunchableDLL && !(info.m_vtfProcOptions.flags0 &
                                    VtfProcessingOptions::OPT_NOCOMPRESS)) {
...
static void CreateLowResImage(IVTFTexture *pVTFTexture) {
  int iWidth, iHeight;
  ImageFormat imageFormat;
  VTFGetLowResImageInfo(pVTFTexture->Width(), pVTFTexture->Height(), &iWidth,
                        &iHeight, &imageFormat);
  ...
  if (!pVTFTexture->ConstructLowResImage()) {
    VTexError("Can't convert image from %s to %s in CalcLowResImage\n",
              ...
              ImageLoader::GetName(imageFormat));
  }
}
...
static void ComputeBufferHash(void const *pvBuffer, size_t numBytes,
                              CRC32_t &uiHashUpdate) {
...
static void InitializeSrcTexture_PSD(IVTFTexture *pTexture,
  ...
  if (!pTexture->Init(nWidth, nHeight, nDepth, IMAGE_FORMAT_DEFAULT,
                      info.m_nFlags, nFrameCount)) {
...
static void InitializeSrcTexture_PFM(IVTFTexture *pTexture,
  ...
  if (!pTexture->Init(nWidth, nHeight, nDepth, IMAGE_FORMAT_RGB323232F,
                      info.m_nFlags, nFrameCount)) {
    Error("Error initializing texture %s\n", pInputFileName);
  }
}
...
static void InitializeSrcTexture(IVTFTexture *pTexture,
  ...
  switch (g_eMode) {
    case eModePSD:
      InitializeSrcTexture_PSD(pTexture, pInputFileName, tgaBuffer, nDepth,
                               nFrameCount, info);
      break;
    case eModeTGA:
      InitializeSrcTexture_TGA(pTexture, pInputFileName, tgaBuffer, nDepth,
                               nFrameCount, info);
      break;
    case eModePFM:
      InitializeSrcTexture_PFM(pTexture, pInputFileName, tgaBuffer, nDepth,
                               nFrameCount, info);
      break;
  }
}
...
static bool LoadFaceFromPSD(IVTFTexture *pTexture, CUtlBuffer &psdBuffer, int z,
  ...
  if ((info.m_bAlphaToDistance) || (nWidth != pTexture->Width()) ||
    ...
    if (info.m_bAlphaToDistance) {
      ...
      ImageFormatInfo_t fmtInfo =
          ImageLoader::ImageFormatInfo(pTexture->Format());
      ...
      if (bWarnEdges) {
        VTexWarning(
            "%s: There are non-zero distance pixels along the image edge. You "
            "may need"
    ...
    ImageLoader::ConvertImageFormat(
        tmpDest.Base(), IMAGE_FORMAT_RGBA8888, pDestBits, pTexture->Format(),
        pTexture->Width(), pTexture->Height(), 0, 0);
    return true;
    ...
    ok = PSDReadFileRGBA8888(psdBuffer, bmPsdData);
    if (ok) {
      memcpy(pDestBits, bmPsdData.m_pBits,
             bmPsdData.m_nWidth * bmPsdData.m_nHeight *
                 ImageLoader::SizeInBytes(bmPsdData.m_ImageFormat));
    }
    return ok;
  }
}
...
static bool LoadFaceFromTGA(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z,
  ...
  if ((info.m_bAlphaToDistance) || (nWidth != pTexture->Width()) ||
    ...
    return TGALoader::Load(pDestBits, tgaBuffer, pTexture->Width(),
                           pTexture->Height(), pTexture->Format(), flGamma,
                           false);
  }
}
...
static bool LoadFaceFromX(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z,
  ...
  switch (g_eMode) {
    case eModePSD:
      return LoadFaceFromPSD(pTexture, tgaBuffer, z, nFrame, nFace, flGamma,
                             info);
      break;
    case eModeTGA:
      return LoadFaceFromTGA(pTexture, tgaBuffer, z, nFrame, nFace, flGamma,
                             info);
      break;
    case eModePFM:
      return LoadFaceFromPFM(pTexture, tgaBuffer, z, nFrame, nFace, flGamma,
                             info);
      break;
    default:
      return false;
...
static bool LoadFace(IVTFTexture *pTexture, CUtlBuffer &tgaBuffer, int z,
  ...
  if (info.m_numChannelsMax < 1 || info.m_numChannelsMax > 4) {
    VTexWarning(
        "%s: Invalid setting restricting number of channels to %d, "
        "discarded!\n",
        info.m_SrcName, info.m_numChannelsMax);
  } else if (info.m_numChannelsMax < 4) {
    if (4 != ImageLoader::SizeInBytes(pTexture->Format())) {
      VTexWarning(
          "%s: Channels restricted to %d, but cannot fill white"
          " because pixel format is %d (size in bytes %d)!"
          " Proceeding with unmodified channels.\n",
          info.m_SrcName, info.m_numChannelsMax, pTexture->Format(),
          ImageLoader::SizeInBytes(pTexture->Format()));
      Assert(0);
    } else {
    ...
    for (int iFace = 0; iFace < nFaceCount; ++iFace) {
      for (int z = 0; z < info.m_nVolumeTextureDepth; ++z) {
        ...
        MakeSrcFileName(pSrcName, info.m_nFlags, pFullNameWithoutExtension,
                        iFrame, iFace, bIsVolumeTexture ? z : -1, bIsCubeMap,
                        info.m_nStartFrame, info.m_nEndFrame,
                        info.m_bNormalToDuDv);
        ...
        if (!LoadFile(pSrcName, tgaBuffer, bFailOnError,
          ...
          // The only other way we can get here is if LoadFile tried to load a
          // spheremap and failed
          bGenerateSpheremaps = true;
          continue;
        }

        // Initialize the VTF Texture here if we haven't already....
        // Note that we have to do it here because we have to get the width +
        // height from the file
        if (!pTexture->ImageData()) {
          InitializeSrcTexture(pTexture, pSrcName, tgaBuffer,
                               info.m_nVolumeTextureDepth, nFrameCount, info);
...
void PreprocessSkyBox(char *pFullNameWithoutExtension, int *iSkyboxFace) {
  ...
  Error(
      "PreprocessSkyBox: filename %s doesn't have a proper extension (bk, dn, "
      "rt, etc..)\n",
      pFullNameWithoutExtension);
}

// Right now, we've got a full cubemap, and we want to return the one face of
// the skybox that we're supposed to be processing.
IVTFTexture *PostProcessSkyBox(IVTFTexture *pTexture, int iSkyboxFace) {
  int nFlags = pTexture->Flags();
  Assert(
      nFlags &
      TEXTUREFLAGS_ENVMAP);  // Should have been treated as an envmap till now.
  nFlags &= ~TEXTUREFLAGS_ENVMAP;  // But it ends now!

  IVTFTexture *pRet = CreateVTFTexture();
  if (!pRet->Init(pTexture->Width(), pTexture->Height(), 1, pTexture->Format(),
                  nFlags, pTexture->FrameCount()))
  ...
  // Note: there are a few things that don't get copied here, like alpha test
...
static void SetTextureLodData(IVTFTexture *pTexture,
                              VTexConfigInfo_t const &info) {
  if ((info.m_nMaxDimensionX > 0 &&
       info.m_nMaxDimensionX < pTexture->Width()) ||
      (info.m_nMaxDimensionY > 0 &&
       info.m_nMaxDimensionY < pTexture->Height()) ||
      (info.m_nMaxDimensionX_360 > 0 &&
       info.m_nMaxDimensionX_360 < pTexture->Width()) ||
      (info.m_nMaxDimensionY_360 > 0 &&
       info.m_nMaxDimensionY_360 < pTexture->Height())) {
    ...
    lodChunk.m_ResolutionClampX_360 =
        GetClampingValue(info.m_nMaxDimensionX_360);
    lodChunk.m_ResolutionClampY_360 =
        GetClampingValue(info.m_nMaxDimensionY_360);
    pTexture->SetResourceData(VTF_RSRC_TEXTURE_LOD_SETTINGS, &lodChunk,
                              sizeof(lodChunk));
  }
}
...
static void AttachShtFile(const char *pFullNameWithoutExtension,
  ...
  if (_stat(shtName, &statBuf) == -1) return;

  printf("Attaching .sht file %s.\n", shtName);

  // Ok, the file exists. Read it.
  CUtlBuffer buf;
  if (!LoadFile(shtName, buf, false, puiHash)) return;
...
bool ProcessFiles(const char *pFullNameWithoutExtension, const char *pOutputDir,
  ...
  bool bGenerateSpheremaps = false;
  bool bLoadedSourceImages = LoadSourceImages(
      pVTFTexture.Get(), pFullNameWithoutExtension, &bGenerateSpheremaps, info);
  ...
  AttachShtFile(pFullNameWithoutExtension, pVTFTexture.Get(),
                &info.m_uiInputHash);

  // No more file loads, finalize the sources hash
  CRC32_Final(&info.m_uiInputHash);
  pVTFTexture->SetResourceData(VTexConfigInfo_t::VTF_INPUTSRC_CRC,
                               &info.m_uiInputHash, sizeof(info.m_uiInputHash));
  ...
  if (CommandLine()->FindParm("-crcvalidate")) {
    ...
    size_t numDataBytes;
    void *pCrcData = spExistingVtf->GetResourceData(
        VTexConfigInfo_t::VTF_INPUTSRC_CRC, &numDataBytes);
    ...
    if (LoadFile(dstFileName, bufFile, false, NULL)) {
      ...
      if (spExistingVtf->Unserialize(bufFile)) {
        ...
        if (pCrcData && numDataBytes == sizeof(CRC32_t)) {
          CRC32_t crcFile = *reinterpret_cast<CRC32_t const *>(pCrcData);
          if (crcFile == crcWritten) {
            if (!g_Quiet) printf("%s is up-to-date\n", dstFileName);
  ...
  pVTFTexture->SetAlphaTestThreshholds(info.m_flAlphaThreshhold,
                                       info.m_flAlphaHiFreqThreshhold);
  ...
  bool bAllowFixCubemapOrientation =
      !info.m_bSkyBox;  // Don't let it rotate our pseudo-cubemap faces around
                        // if it's a skybox.
  pVTFTexture->SetPostProcessingSettings(&info.m_vtfProcOptions);
  pVTFTexture->PostProcess(bGenerateSpheremaps, info.m_LookDir,
                           bAllowFixCubemapOrientation);

  // Compute the preferred image format
  ImageFormat vtfImageFormat =
      ComputeDesiredImageFormat(pVTFTexture.Get(), info);

  // Set up the low-res image
  if (pVTFTexture->IsCubeMap()) {
    // "Stage 1" of matching cubemap borders. Sometimes, it has to store off the
    // original image.
  ...
  if (info.IsSettings0Valid()) {
    pVTFTexture->SetResourceData(VTF_RSRC_TEXTURE_SETTINGS_EX,
                                 &info.m_exSettings0,
                                 sizeof(info.m_exSettings0));
  }
...
const char *GetPossiblyQuotedWord(const char *pInBuf, char *pOutbuf) {
  ...
  if (pInBuf[0] == '"') {
    ...
    pWordEnd = strchr(pInBuf, ' ');
    if (!pWordEnd) pWordEnd = strchr(pInBuf, '\t');
    if (!pWordEnd) pWordEnd = pInBuf + strlen(pInBuf);
  }
  if ((!pWordEnd) || (pWordEnd == pInBuf)) return NULL;  // no word found
...
//		fills in "key" and "val" respectively and returns "true" if
...
static bool LoadConfigFile(const char *pFileBaseName, VTexConfigInfo_t &info,
  ...
  if (!bOK && !g_bNoPsd)  // If PSD mode was not disabled
  {
    ...
    if (bOK) {
      ...
      PSDResFileInfo resFileInfo(
          imgres.FindElement(PSDImageResources::eResFileInfo));
      PSDResFileInfo::ResFileInfoElement descr =
          resFileInfo.FindElement(PSDResFileInfo::eDescription);
      if (descr.m_pvData) {
    ...
    if (!bOK)
      ...
      printf(
          "Warning: psd file \"%s\" exists, but not used, delete tga and txt "
          "files to use psd file directly\n",
          pFileName);
  }
  ...
  if ((info.m_bNormalToDuDv || (info.m_vtfProcOptions.flags0 &
                                VtfProcessingOptions::OPT_NORMAL_DUDV)) &&
      !(info.m_vtfProcOptions.flags0 &
        VtfProcessingOptions::OPT_PREMULT_COLOR_ONEOVERMIP)) {
    printf(
        "Implicitly setting premultcolorbyoneovermiplevel since you are "
        "generating a dudv map\n");
    info.m_vtfProcOptions.flags0 |=
        VtfProcessingOptions::OPT_PREMULT_COLOR_ONEOVERMIP;
  }
  ...
  if ((info.m_bNormalToDuDv || (info.m_vtfProcOptions.flags0 &
                                VtfProcessingOptions::OPT_NORMAL_DUDV))) {
    printf(
        "Implicitly setting trilinear since you are generating a dudv map\n");
  ...
  if (Q_stristr(pFileBaseName, "_dudv")) {
    if (!info.m_bNormalToDuDv && !info.m_bDuDv) {
      if (!g_Quiet) {
        ...
        fprintf(stderr,
                "If you are trying to convert from a normal map to a dudv map, "
                "put \"normaltodudv\" \"1\" in description.\n");
      }
      info.m_bDuDv = true;
    }
  }

  // turn off nice filtering if we are a cube map (takes too long with
  // buildcubemaps) or if we are a normal map (looks like terd.)
...
void Usage(void) {
  VTexError(
      ...
      "-nomkdir          : don't create destination folder if it doesn't "
      "exist\n"
      "-vmtparam         : adds parameter and value to the .vmt file\n"
      "-outputdir <dir>  : write output to the specified dir regardless of "
      "source filename and vproject\n"
      "-deducepath       : deduce path of sources by target file names\n"
      "-quickconvert     : use with \"-nop4 -dontusegamedir -quickconvert\" to "
      "upgrade old .vmt files\n"
...
bool Process_File(char *pInputBaseName, int maxlen) {
  ...
  // Usage:
  //			vtex -nop4 -dontusegamedir -quickconvert
  // u:\data\game\tf\texture.vtf
  ...
  if (CommandLine()->FindParm("-quickconvert")) {
    ...
    if (!bRes) VTexError("Failed to read '%s'!\n", chFileNameConvert);
    ...
    if (!bRes) VTexError("Failed to write '%s'!\n", chFileNameConvert);
  ...
  if (!LoadConfigFile(pInputBaseName, info, &isCubeMap)) return FALSE;
  ...
  if ((info.m_nStartFrame == -1 && info.m_nEndFrame != -1) ||
    ...
    VTexError(
        "%s: If you use startframe, you must use endframe, and vice versa.\n",
        pInputBaseName);
    return FALSE;
  }
  ...
  bool bProcessedFilesOK =
      ProcessFiles(pInputBaseName, outputDir, pBaseName, isCubeMap, info);
  if (!bProcessedFilesOK) return FALSE;
...
class CVTex : public IVTex, public ILaunchableDLL {
 public:
  ...
  virtual int VTex(CreateInterfaceFn fsFactory, const char *pGameDir, int argc,
    ...
    if (!g_pFileSystem) {
      Error("IVTex3::VTex - fsFactory can't get '%s' interface.",
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVTex, ILaunchableDLL,
                                  LAUNCHABLE_DLL_INTERFACE_VERSION, g_VTex);
