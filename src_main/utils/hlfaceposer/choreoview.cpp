...
static void RescaleExpressionTimes(CChoreoEvent *event, float newstart,
                                   float newend) {
  if (!event || event->GetType() != CChoreoEvent::FLEXANIMATION) return;

  // Did it actually change
  if (newstart == event->GetStartTime() && newend == event->GetEndTime()) {
    return;
  }
  ...
  if (newstart != event->GetStartTime()) {
    // Otherwise, if the new start is later, then tags need to be shifted
    // backwards
...
StudioModel *FindAssociatedModel(CChoreoScene *scene, CChoreoActor *a) {
  ...
  if (DoesAnyActorHaveAssociatedModelLoaded(scene)) {
    // Then return NULL here so we don't override with the default an actor who
    // has a valid model going
    return NULL;
  }

  // Couldn't find it and nobody else has a loaded associated model, so just use
  // the default model
  if (!model) {
...
CChoreoView::CChoreoView(mxWindow *parent, int x, int y, int w, int h, int id)
    : IFacePoserToolWindow("CChoreoView", "Choreography"),
  ...
  m_pVertScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_CHOREOVSCROLL,
                                     mxScrollbar::Vertical);
  m_pHorzScrollBar = new mxScrollbar(this, 0, 0, 18, 100, IDC_CHOREOHSCROLL,
                                     mxScrollbar::Horizontal);

  m_bLayoutIsValid = false;
  m_flPixelsPerSecond = 150.0f;

  m_btnPlay = new mxBitmapButton(this, 2, 4, 16, 16, IDC_PLAYSCENE,
                                 "gfx/hlfaceposer/play.bmp");
  m_btnPause = new mxBitmapButton(this, 18, 4, 16, 16, IDC_PAUSESCENE,
                                  "gfx/hlfaceposer/pause.bmp");
  m_btnStop = new mxBitmapButton(this, 34, 4, 16, 16, IDC_STOPSCENE,
                                 "gfx/hlfaceposer/stop.bmp");
  ...
  if (workspacefiles->GetNumStoredFiles(IWorkspaceFiles::CHOREODATA) >= 1) {
    LoadSceneFromFile(
        workspacefiles->GetStoredFile(IWorkspaceFiles::CHOREODATA, 0));
  }
...
bool CChoreoView::Close(void) {
  if (m_pScene && m_bDirty) {
    int retval =
        mxMessageBox(NULL, va("Save changes to scene '%s'?", GetChoreoFile()),
                     g_appTitle, MX_MB_YESNOCANCEL);
    if (retval == 2) {
      return false;
    }
    if (retval == 0) {
...
//			right -
//-----------------------------------------------------------------------------
void CChoreoView::DrawTimeLine(CChoreoWidgetDrawHelper &drawHelper, RECT &rc,
  ...
  drawHelper.DrawColoredLine(COLOR_CHOREO_TIMELINE, PS_SOLID, 1, rc.left,
                             GetStartRow() - 1, rc.right, GetStartRow() - 1);
  ...
  while (f < right) {
    ...
    if (frac >= 0.0f && frac <= 1.0f) {
      rcLabel.left =
          GetLabelWidth() + (int)(frac * (rc.right - GetLabelWidth()));
      rcLabel.bottom = GetStartRow() - 1;
      rcLabel.top = rcLabel.bottom - 10;

      if (f != left) {
        drawHelper.DrawColoredLine(RGB(220, 220, 240), PS_DOT, 1, rcLabel.left,
                                   GetStartRow(), rcLabel.left, h2());
      }
      ...
      drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, COLOR_CHOREO_TEXT,
                                 rcLabel, sz);
    }
    f += granularity;
  }
...
//			rc -
//-----------------------------------------------------------------------------
void CChoreoView::DrawRelativeTagLines(CChoreoWidgetDrawHelper &drawHelper,
                                       RECT &rc) {
  if (!m_pScene) return;
    ...
    for (int j = 0; j < a->GetNumChannels(); j++) {
      ...
      for (int k = 0; k < c->GetNumEvents(); k++) {
        ...
        CEventRelativeTag *tag = m_pScene->FindTagByName(
            event->GetRelativeWavName(), event->GetRelativeTagName());
        ...
        drawHelper.DrawColoredLine(RGB(180, 180, 220), PS_SOLID, 1, pixel,
                                   rcClip.top, pixel, rcClip.bottom);
      }
    }
  }
...
//			rc -
//-----------------------------------------------------------------------------
void CChoreoView::DrawBackground(CChoreoWidgetDrawHelper &drawHelper,
                                 RECT &rc) {
  RECT rcClip;
  GetClientRect((HWND)getHandle(), &rcClip);
  rcClip.top = GetStartRow();
...
void CChoreoView::redraw() {
  ...
  if (!m_pScene) {
    ...
    drawHelper.DrawColoredText("Arial", pointsize, FW_NORMAL,
                               COLOR_CHOREO_LIGHTTEXT, rcText, sz);
  ...
  if (!clipped) {
    drawHelper.DrawColoredLine(COLOR_CHOREO_ENDTIME, PS_DOT, 1, finishx,
                               rc.top + GetStartRow(), finishx, rc.bottom);
  }
  ...
  if (m_UndoStack.Size() > 0) {
    int length = drawHelper.CalcTextWidth("Arial", 9, FW_NORMAL, "undo %i/%i",
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL, RGB(100, 180, 100),
                               rcText, "undo %i/%i", m_nUndoLevel,
                               m_UndoStack.Size());
  }
  ...
  drawHelper.DrawColoredText("Arial", fontsize, FW_NORMAL, RGB(50, 50, 50),
                             rcText, sz);
...
void CChoreoView::ShowContextMenu(int mx, int my) {
  ...
  GetObjectsUnderMouse(mx, my, &a, &c, &e, &ge, &ct, &at,
                       &clickedCloseCaptionButton);
  ...
  if (a && c) {
    if (!e) {
      ...
      pop->add(va("Edit Event '%s'...", e->GetEvent()->GetName()),
               IDC_EDITEVENT);
      switch (e->GetEvent()->GetType()) {
        default:
          break;
        case CChoreoEvent::FLEXANIMATION: {
          pop->add(va("Edit Event '%s' in expression tool",
                      e->GetEvent()->GetName()),
                   IDC_EXPRESSIONTOOL);
        } break;
        case CChoreoEvent::GESTURE: {
          pop->add(
              va("Edit Event '%s' in gesture tool", e->GetEvent()->GetName()),
              IDC_GESTURETOOL);
        } break;
      }
  ...
  if (a || c || e || ge) {
    ...
    {
      if (a) {
        editMenu->add(va("Actor '%s'...", a->GetActor()->GetName()),
                      IDC_EDITACTOR);
      }
      if (c) {
        editMenu->add(va("Channel '%s'...", c->GetChannel()->GetName()),
                      IDC_EDITCHANNEL);
      }
      if (ge) {
        switch (ge->GetEvent()->GetType()) {
          ...
          case CChoreoEvent::SECTION: {
            editMenu->add(
                va("Section Pause '%s'...", ge->GetEvent()->GetName()),
                IDC_EDITGLOBALEVENT);
          } break;
          case CChoreoEvent::LOOP: {
            editMenu->add(va("Loop Point '%s'...", ge->GetEvent()->GetName()),
                          IDC_EDITGLOBALEVENT);
          } break;
          case CChoreoEvent::STOPPOINT: {
            editMenu->add(
                va("Fire Completion '%s'...", ge->GetEvent()->GetName()),
                IDC_EDITGLOBALEVENT);
          } break;
        }
      }
  ...
  if (a || c) {
    ...
    if (a) {
      moveUpMenu->add(va("Move '%s' up", a->GetActor()->GetName()),
                      IDC_MOVEACTORUP);
      moveDownMenu->add(va("Move '%s' down", a->GetActor()->GetName()),
                        IDC_MOVEACTORDOWN);
    }
    if (c) {
      moveUpMenu->add(va("Move '%s' up", c->GetChannel()->GetName()),
                      IDC_MOVECHANNELUP);
      moveDownMenu->add(va("Move '%s' down", c->GetChannel()->GetName()),
                        IDC_MOVECHANNELDOWN);
    }
  ...
  if (a || c || e || ge || (ct != -1)) {
    ...
    if (a) {
      deleteMenu->add(va("Actor '%s'", a->GetActor()->GetName()),
                      IDC_DELETEACTOR);
    }
    if (c) {
      deleteMenu->add(va("Channel '%s'", c->GetChannel()->GetName()),
                      IDC_DELETECHANNEL);
    }
    if (e) {
      deleteMenu->add(va("Event '%s'", e->GetEvent()->GetName()),
                      IDC_DELETEEVENT);
    }
    if (ge) {
      switch (ge->GetEvent()->GetType()) {
        ...
        case CChoreoEvent::SECTION: {
          deleteMenu->add(
              va("Section Pause '%s'...", ge->GetEvent()->GetName()),
              IDC_DELETEGLOBALEVENT);
        } break;
        case CChoreoEvent::LOOP: {
          deleteMenu->add(va("Loop Point '%s'...", ge->GetEvent()->GetName()),
                          IDC_DELETEGLOBALEVENT);
        } break;
        case CChoreoEvent::STOPPOINT: {
          deleteMenu->add(
    ...
    if (e && ct != -1) {
      ...
      if (tag) {
        deleteMenu->add(va("Relative Tag '%s'...", tag->GetName()),
                        IDC_DELETERELATIVETAG);
      }
    }
  ...
  // Select
  {
    ...
    if (a || c) {
      ...
      if (a) {
        selectMenu->add(
            va("All events in actor '%s'", a->GetActor()->GetName()),
            IDC_CV_ALLEVENTS_ACTOR);
      }
      if (c) {
        selectMenu->add(
            va("All events in channel '%s'", c->GetChannel()->GetName()),
            IDC_CV_ALLEVENTS_CHANNEL);
      }
    }
  ...
  if (e) {
    ...
    switch (e->GetEvent()->GetType()) {
      ...
      case CChoreoEvent::FLEXANIMATION: {
        pop->add(
            va("Edit event '%s' in expression tool", e->GetEvent()->GetName()),
            IDC_EXPRESSIONTOOL);
      } break;
      case CChoreoEvent::GESTURE: {
        pop->add(
    ...
    pop->add(va("Move event '%s' to back", e->GetEvent()->GetName()),
             IDC_MOVETOBACK);
    if (CountSelectedEvents() > 1) {
      ...
      pop->add(va("Delete event '%s'", e->GetEvent()->GetName()),
               IDC_DELETEEVENT);
      pop->addSeparator();
      if (e->GetEvent()->GetActive()) {
        pop->add(va("Disable event '%s'", e->GetEvent()->GetName()),
                 IDC_CV_DISABLEEVENTS);
      } else {
        pop->add(va("Enable event '%s'", e->GetEvent()->GetName()),
                 IDC_CV_ENABLEEVENTS);
      }
    }
  }
  ...
  if (m_rgABPoints[0].active && m_rgABPoints[1].active) {
    ...
    timeMenu->add("Insert empty space between marks (shifts events right)",
                  IDC_INSERT_TIME);
    timeMenu->add("Delete events between marks (shifts remaining events left)",
                  IDC_DELETE_TIME);
  ...
  if (CanPaste() || e) {
    ...
    if (CountSelectedEvents() > 1) {
      ...
      pop->add(va("Copy event '%s' to clipboard", e->GetEvent()->GetName()),
               IDC_COPYEVENTS);
    }
  ...
  if (e) {
    ...
    if (CountSelectedEvents() > 1) {
      ...
      exportMenu->add(
          va("Export event '%s' to .vce...", e->GetEvent()->GetName()),
          IDC_EXPORTEVENTS);
    }
  ...
  bool bShowAlignLeft =
      (CountSelectedEvents() + CountSelectedGlobalEvents()) > 1 ? true : false;
  ...
  pop->add(va(m_bRampOnly ? "Ramp normal" : "Ramp only"),
           IDC_CV_TOGGLERAMPONLY);
  pop->setChecked(IDC_CV_PROCESSSEQUENCES, m_bProcessSequences);

  bool onmaster =
      (m_pClickedChannel && m_pClickedChannel->GetCaptionClickedEvent() &&
       m_pClickedChannel->GetCaptionClickedEvent()->GetCloseCaptionType() ==
           CChoreoEvent::CC_MASTER)
          ? true
          : false;
  bool ondisabled =
      (m_pClickedChannel && m_pClickedChannel->GetCaptionClickedEvent() &&
       m_pClickedChannel->GetCaptionClickedEvent()->GetCloseCaptionType() ==
           CChoreoEvent::CC_DISABLED)
          ? true
          : false;

  // The close captioning menu
  if (m_bShowCloseCaptionData &&
      (AreSelectedEventsCombinable() || AreSelectedEventsInSpeakGroup() ||
       onmaster || ondisabled)) {
    pop->addSeparator();
  ...
  if (m_pScene) {
    // Associate map file
    pop->addSeparator();
    pop->add(va("Associate .bsp (%s)", m_pScene->GetMapname()),
             IDC_ASSOCIATEBSP);
    if (a) {
      if (a->GetActor() && a->GetActor()->GetFacePoserModelName()[0]) {
        pop->add(va("Change .mdl for %s", a->GetActor()->GetName()),
                 IDC_ASSOCIATEMODEL);
      } else {
        pop->add(va("Associate .mdl with %s", a->GetActor()->GetName()),
                 IDC_ASSOCIATEMODEL);
      }
    }
  }
...
void CChoreoView::AssociateModel(void) {
  ...
  if (!ChoiceProperties(&params)) return;

  if (params.m_nSelected == oldsel) return;
...
void CChoreoView::AssociateBSP(void) {
  ...
  char mapname[512];
  if (!FacePoser_ShowOpenFileNameDialog(mapname, sizeof(mapname), "maps",
                                        "*.bsp")) {
    return;
  }
...
int CChoreoView::GetSelectedEventWidgets(
...
int CChoreoView::CountSelectedGlobalEvents(void) {
  ...
  for (int i = 0; i < m_SceneGlobalEvents.Size(); i++) {
    CChoreoGlobalEventWidget *event = m_SceneGlobalEvents[i];
    if (!event || !event->IsSelected()) continue;
...
bool CChoreoView::IsMouseOverEvent(CChoreoEventWidget *ew, int mx, int my) {
  ...
  if (event) {
    if (event->HasEndTime()) {
      int rightside =
          ew->GetDurationRightEdge() ? ew->GetDurationRightEdge() : ew->w();
...
bool CChoreoView::IsMouseOverEventEdge(CChoreoEventWidget *ew, bool bLeftEdge,
                                       int mx, int my) {
  int tolerance = DRAG_EVENT_EDGE_TOLERANCE;
...
int CChoreoView::GetEarliestEventIndex(
    CUtlVector<CChoreoEventWidget *> &events) {
  int best = -1;
...
int CChoreoView::ComputeEventDragType(int mx, int my) {
  ...
  if (IsMouseOverEventEdge(ew, true, mx, my)) {
    if (GetAsyncKeyState(VK_SHIFT)) return DRAGTYPE_EVENT_STARTTIME_RESCALE;
    return DRAGTYPE_EVENT_STARTTIME;
  }

  if (IsMouseOverEventEdge(ew, false, mx, my)) {
    if (GetAsyncKeyState(VK_SHIFT)) return DRAGTYPE_EVENT_ENDTIME_RESCALE;
      ...
      for (int k = 0; k < channel->GetNumEvents(); k++) {
        ...
        if (event == m_pClickedEvent &&
          ...
          if (!m_pClickedAbsoluteTag) {
            CEventRelativeTag *tag =
                event->GetEvent()->GetRelativeTag(m_nClickedTag);
            if (tag) {
              // Determine left edcge
              RECT bounds;
              bounds = event->getBounds();
              if (bounds.right - bounds.left > 0) {
                leftEdge = (int)(tag->GetPercentage() *
            ...
            bounds = event->getBounds();
            if (bounds.right - bounds.left > 0) {
              leftEdge = (int)(m_pClickedAbsoluteTag->GetPercentage() *
                                   (float)(bounds.right - bounds.left) +
                               0.5f);
            }
          }
...
void CChoreoView::MouseStartDrag(mxEvent *event, int mx, int my) {
  bool isrightbutton =
      event->buttons & mxEvent::MouseRightButton ? true : false;

  if (m_bDragging) {
    return;
  }

  GetObjectsUnderMouse(mx, my, &m_pClickedActor, &m_pClickedChannel,
                       &m_pClickedEvent, &m_pClickedGlobalEvent, &m_nClickedTag,
                       &m_pClickedAbsoluteTag,
                       &m_nClickedChannelCloseCaptionButton);
  ...
  if (m_pClickedEvent) {
    ...
    bool bIsEdgeRescale = (dtPreview == DRAGTYPE_EVENT_ENDTIME_RESCALE ||
                           dtPreview == DRAGTYPE_EVENT_STARTTIME_RESCALE);
    ...
    if (IsMouseOverScrubHandle(event)) {
      ...
      float maxoffset =
          0.5f * (float)SCRUBBER_HANDLE_WIDTH / GetPixelsPerSecond();
      m_flScrubberTimeOffset =
          std::clamp(m_flScrubberTimeOffset, -maxoffset, maxoffset);
    ...
    StartDraggingSceneEndTime(mx, my);
             ...
             m_nClickedChannelCloseCaptionButton !=
                 CChoreoChannelWidget::CLOSECAPTION_NONE &&
             m_nClickedChannelCloseCaptionButton !=
                 CChoreoChannelWidget::CLOSECAPTION_CAPTION) {
...
void CChoreoView::OnDoubleClicked() {
  if (m_pClickedChannel) {
    switch (m_nClickedChannelCloseCaptionButton) {
      ...
      case CChoreoChannelWidget::CLOSECAPTION_NONE: {
        ...
        m_pClickedChannel->GetChannel()->SetActive(
            !m_pClickedChannel->GetChannel()->GetActive());
  ...
  if (m_pClickedActor) {
    ...
    m_pClickedActor->GetActor()->SetActive(
        !m_pClickedActor->GetActor()->GetActive());
  ...
  for (int i = 0; i < m_FocusRects.Size(); i++) {
    ...
    switch (m_nDragType) {
      ...
      case DRAGTYPE_EVENTABSTAG_MOVE: {
        ...
        if (m_pClickedEvent) {
          bool shiftdown =
              (event->modifiers & mxEvent::KeyShift) ? true : false;
          ...
          if (m_nDragType == DRAGTYPE_EVENTTAG_MOVE ||
              m_nDragType == DRAGTYPE_EVENTABSTAG_MOVE) {
            dy = 0;
          }
    ...
    if (oldSize > 0) {
      ...
      for (int i = 0; i < c; ++i) {
        ...
        if (m_nDragType == DRAGTYPE_RESCALERIGHT) {
          f->m_rcFocus.left =
              m_nStart +
              (int)(rescale * (float)(f->m_rcFocus.left - m_nStart) + 0.5f);
          f->m_rcFocus.right =
              f->m_rcFocus.left + (int)(rescale * (float)w + 0.5f);
        } else {
          f->m_rcFocus.right =
              m_nEnd -
              (int)(rescale * (float)(m_nEnd - f->m_rcFocus.right) + 0.5f);
          f->m_rcFocus.left =
              f->m_rcFocus.right - (int)(rescale * (float)w + 0.5f);
        }
      }
    }
  }
...
bool CChoreoView::CheckGestureLength(CChoreoEvent *e, bool bCheckOnly) {
  ...
  return UpdateGestureLength(e, pStudioHdr, model->GetPoseParameters(),
                             bCheckOnly);
}
...
bool CChoreoView::AutoaddGestureKeys(CChoreoEvent *e, bool bCheckOnly) {
  ...
  return AutoAddGestureKeys(e, pStudioHdr, model->GetPoseParameters(),
                            bCheckOnly);
}
...
bool CChoreoView::CheckSequenceLength(CChoreoEvent *e, bool bCheckOnly) {
  ...
  return UpdateSequenceLength(e, pStudioHdr, model->GetPoseParameters(),
                              bCheckOnly, true);
}
  ...
  for (int i = 0; i < m_SceneActors.Size(); i++) {
    ...
    for (int j = 0; j < a->GetNumChannels(); j++) {
      ...
      for (k = channel->GetNumEvents() - 1; k >= 0; k--) {
        ...
        if (e->GetType() != CChoreoEvent::GESTURE) continue;
      ...
      for (k = channel->GetNumEvents() - 1; k >= 0; k--) {
        ...
        // No change to normal events that end earlier than new time (but do
        // change gestures)
        if (e->GetEndTime() < newendtime && e != finalGesture) {
          continue;
        }
        ...
        switch (e->GetType()) {
          default:
            break;
          case CChoreoEvent::GESTURE: {
            e->RescaleGestureTimes(e->GetStartTime(), e->GetEndTime() + dt,
                                   true);
          } break;
          case CChoreoEvent::FLEXANIMATION: {
...
void CChoreoView::FinishDraggingEvent(mxEvent *event, int mx, int my) {
  ...
  if (!dt) {
    if (m_pScene && m_pClickedEvent &&
        m_pClickedEvent->GetEvent()->GetType() == CChoreoEvent::SPEAK) {
      // Show phone wav in wav viewer
      char sndname[512];
      Q_strncpy(sndname,
                FacePoser_TranslateSoundName(m_pClickedEvent->GetEvent()),
                sizeof(sndname));
      if (sndname[0]) {
        SetCurrentWaveFile(va("sound/%s", sndname),
                           m_pClickedEvent->GetEvent());
      } else {
        Warning(
            "Unable to resolve sound name for '%s', check actor associations\n",
      ...
      for (int k = 0; k < channel->GetNumEvents(); k++) {
            ...
            case DRAGTYPE_EVENT_STARTTIME_RESCALE: {
              ...
              switch (e->GetType()) {
                default:
                  break;
                case CChoreoEvent::GESTURE: {
                  e->RescaleGestureTimes(
                      e->GetStartTime() + dt, e->GetEndTime(),
                      m_nDragType == DRAGTYPE_EVENT_STARTTIME);
                } break;
                case CChoreoEvent::FLEXANIMATION: {
                  RescaleExpressionTimes(e, e->GetStartTime() + dt,
                                         e->GetEndTime());
                } break;
              }
            ...
            case DRAGTYPE_EVENT_ENDTIME_RESCALE: {
              ...
              switch (e->GetType()) {
                default:
                  break;
                case CChoreoEvent::GESTURE: {
                  e->RescaleGestureTimes(e->GetStartTime(),
                                         e->GetEndTime() + dt,
                                         m_nDragType == DRAGTYPE_EVENT_ENDTIME);
                } break;
                case CChoreoEvent::FLEXANIMATION: {
                  RescaleExpressionTimes(e, e->GetStartTime(),
                                         e->GetEndTime() + dt);
                } break;
              }
              e->OffsetEndTime(dt);
              ...
              if (m_nClickedTag != -1) {
                ...
                if (tag) {
                  ...
                  if (bounds.right - bounds.left > 0) {
                    ...
                    float frac = (float)(left - bounds.left) /
                                 (float)(bounds.right - bounds.left);
        ...
        switch (e->GetType()) {
          ...
          case CChoreoEvent::SPEAK: {
            ...
            CAudioSource *wave = sound->LoadSound(
                va("sound/%s", FacePoser_TranslateSoundName(e)));
            if (wave) {
      ...
      for (i = 0; i < rescaleHelper.Count(); ++i) {
        ...
        switch (e->GetType()) {
          default:
            break;
          case CChoreoEvent::GESTURE: {
            e->RescaleGestureTimes(flNewStartTime,
                                   flNewStartTime + flNewDuration,
                                   m_nDragType == DRAGTYPE_EVENT_STARTTIME ||
                                       m_nDragType == DRAGTYPE_EVENT_ENDTIME);
          } break;
          case CChoreoEvent::FLEXANIMATION: {
            RescaleExpressionTimes(e, flNewStartTime,
                                   flNewStartTime + flNewDuration);
          } break;
        }
  ...
  for (int i = 0; i < m_SceneGlobalEvents.Count(); i++) {
    CChoreoGlobalEventWidget *gew = m_SceneGlobalEvents[i];
    if (!gew || !gew->IsSelected()) continue;
    ...
    case mxEvent::KeyDown: {
      ...
      switch (event->key) {
        ...
        case VK_NEXT:  // PgDown
        {
          ...
          m_flLeftOffset =
              std::min(m_flLeftOffset + (float)window, (float)pixels);
          MoveTimeSliderToPos((int)m_flLeftOffset);
        } break;
      }
    } break;
    case mxEvent::Action: {
      ...
      switch (event->action) {
        ...
        case IDC_CHANNELOPEN: {
          CActorBitmapButton *btn =
              static_cast<CActorBitmapButton *>(event->widget);
          if (btn) {
...
void CChoreoView::PlayScene(bool forward) {
  ...
  if (m_rgABPoints[0].active || m_rgABPoints[1].active) {
    if (m_rgABPoints[0].active && m_rgABPoints[1].active) {
      ...
      float startonly =
          m_rgABPoints[0].active ? m_rgABPoints[0].time : m_rgABPoints[1].time;
...
void SetupFlexControllerTracks(CStudioHdr *hdr, CChoreoEvent *event) {
  ...
  /*
  // TODO(d.rattman):  Brian hooked this stuff up for some took work, but at
  this point the .mdl files don't look like they've been updated to include the
  remapping data yet... int c = hdr->numflexcontrollerremaps(); for ( i = 0; i <
  c; ++i )
  {
          mstudioflexcontrollerremap_t *remap = hdr->pFlexcontrollerRemap( i );
          Msg( "remap %s\n", remap->pszName() );
          Msg( "  type %d\n", remap->remaptype );
          Msg( "  num remaps %d (stereo %s)\n", remap->numremaps, remap->stereo
  ? "true" : "false" ); for ( int j = 0 ; j < remap->numremaps; ++j )
          {
                  int index = remap->pRemapControlIndex( j );
                  Msg( "  %d:  maps to %d (%s) with %s\n", j, index,
  hdr->pFlexcontroller( index )->pszName(), remap->pRemapControl( j ) );
          }
  }
  */
  ...
  for (LocalFlexController_t i = LocalFlexController_t(0);
    ...
    if (combo) {
      track->SetFlexControllerIndex(
          LocalFlexController_t(i + 1),
          hdr->pFlexcontroller(LocalFlexController_t(i + 1))->localToGlobal, 1);
    ...
    // set range
    if (hdr->pFlexcontroller(i)->min == 0.0f ||
        hdr->pFlexcontroller(i)->max == 1.0f) {
      track->SetInverted(false);
      track->SetMin(hdr->pFlexcontroller(i)->min);
      track->SetMax(hdr->pFlexcontroller(i)->max);
    } else {
      // invert ranges for wide ranged, makes sense considering flexcontroller
      // names...
    ...
    if (track->GetNumSamples(0) > 0) {
      ...
      for (int i = 0; i < track->GetNumSamples(0); i++) {
        ...
        float rangedValue =
            orig_min * (1 - sample->value) + orig_max * sample->value;
        sample->value =
            std::clamp((rangedValue - track->GetMin()) / range, 0.0, 1.0);
      }
    }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
void CChoreoView::ProcessFlexAnimation(CChoreoScene *scene,
                                       CChoreoEvent *event) {
  ...
  bool bUpdateSliders = (eventwidget && eventwidget->IsSelected() &&
                         model == models->GetActiveStudioModel());
    ...
    if (track->IsComboType()) {
      for (int side = 0; side < 2; side++) {
        ...
        if (controller != -1) {
          ...
          flIntensity =
              current[controller] * (1 - weight) + flIntensity * weight;
...
void CChoreoView::ProcessLookat(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  if (!stricmp(event->GetParameters(), a->GetName()) ||
    ...
    if (event->GetPitch() != 0 || event->GetYaw() != 0) {
      QAngle angles(-(float)event->GetPitch(), (float)event->GetYaw(), 0);
      ...
      if (mapentities->LookupOrigin(a->GetName(), orgActor, anglesActor)) {
        if (mapentities->LookupOrigin(event->GetParameters(), orgTarget,
                                      anglesDummy)) {
          ...
          // Rotate around actor's placed forward direction since we look
          // straight down x in faceposer/hlmv
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CChoreoView::GetTarget(CChoreoScene *scene, CChoreoEvent *event,
                            Vector &vecTarget, QAngle &vecAngle) {
  if (!event->GetActor()) return false;
...
void CChoreoView::ProcessFace(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  //float idealYaw = info->m_flInitialYaw + (diff - m_goalBodyYaw * dir -
  m_goalSpineYaw * dir) * dir;
...
void CChoreoView::ProcessGesture(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  if (iSequence < 0) return;

  // Get spline intensity for controller
  float eventlocaltime = scene->GetTime() - event->GetStartTime();

  float referencetime = event->GetOriginalPercentageFromPlaybackPercentage(
                            eventlocaltime / event->GetDuration()) *
                        event->GetDuration();
  ...
  int iLayer =
      model->GetNewAnimationLayer(a->FindChannelIndex(event->GetChannel()));

  model->SetOverlaySequence(iLayer, iSequence,
                            event->GetIntensity(scene->GetTime()));
...
void CChoreoView::ProcessSequence(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  // TODO(d.rattman): shouldn't sequences always be lower priority than
  // gestures?
  int iLayer =
      model->GetNewAnimationLayer(a->FindChannelIndex(event->GetChannel()));
  model->SetOverlaySequence(iLayer, iSequence,
...
void CChoreoView::ProcessMoveto(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  // movement should always be higher priority than postures, but not
  // gestures....grrr, any way to tell them apart?
  int iLayer = model->GetNewAnimationLayer(
      0 /* a->FindChannelIndex( event->GetChannel() ) */);
  model->SetOverlaySequence(iLayer, iSequence, intensity);
  model->SetOverlayRate(iLayer, cycle, 0.0);
}

int CChoreoView::GetMovetoSequence(CChoreoScene *scene, CChoreoEvent *event,
                                   StudioModel *model) {
  // TODO(d.rattman): needs to pull from event (activity or sequence?)
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
void CChoreoView::ProcessEvent(float currenttime, CChoreoScene *scene,
                               CChoreoEvent *event) {
  if (!event || !event->GetActive()) return;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CChoreoView::CheckEvent(float currenttime, CChoreoScene *scene,
                             CChoreoEvent *event) {
  if (!event || !event->GetActive()) return true;
...
void CChoreoView::PauseThink(void) {
  // TODO(d.rattman):  Game code would check for conditions being met
  ...
  CChoreoWidgetDrawHelper drawHelper(this, rcPauseRect,
                                     COLOR_CHOREO_BACKGROUND);

  DrawSceneABTicks(drawHelper);

  if (m_flAutomationDelay > 0.0f && m_flAutomationTime < m_flAutomationDelay) {
    char sz[256];
    ...
    drawHelper.DrawColoredText("Arial", 9, FW_NORMAL,
                               COLOR_CHOREO_PLAYBACKTICKTEXT, rcText, sz);
...
void CChoreoView::SceneThink(float time) {
  ...
  if (!ShouldProcessSpeak()) {
    ...
    if (!anyscrub && !anyprocessing && autoprocess && !m_bForceProcess) {
      sound->StopAll();
...
void CChoreoView::LayoutScene(void) {
  ...
  for (i = 0; i < m_SceneGlobalEvents.Size(); i++) {
    ...
    float frac = (e->GetEvent()->GetStartTime() - m_flStartTime) /
                 (m_flEndTime - m_flStartTime);

    rcEvent.left = GetLabelWidth() + rcEvent.left +
                   (int)(frac * (m_rcTimeLine.right - m_rcTimeLine.left -
                                 GetLabelWidth()));
    rcEvent.left -= 4;
...
void CChoreoView::RepositionVSlider(void) {
  ...
  m_pVertScrollBar->setBounds(w2() - m_nScrollbarHeight, GetStartRow(),
                              m_nScrollbarHeight,
                              h2() - m_nScrollbarHeight - GetStartRow());
...
void CChoreoView::RepositionHSlider(void) {
  ...
  m_pHorzScrollBar->setBounds(0, h2() - m_nScrollbarHeight,
                              w - m_nScrollbarHeight, m_nScrollbarHeight);
...
void CChoreoView::New(void) {
  ...
  char scenefile[512];
  if (FacePoser_ShowSaveFileNameDialog(scenefile, sizeof(scenefile), "scenes",
                                       "*.vcd")) {
  ...
  if (!ActorProperties(&params)) return;
...
void CChoreoView::Load(void) {
  char scenefile[512];
  if (!FacePoser_ShowOpenFileNameDialog(scenefile, sizeof(scenefile), "scenes",
                                        "*.vcd")) {
    return;
  }
...
void CChoreoView::EditChannel(CChoreoChannel *channel) {
  ...
  if (!ChannelProperties(&params)) return;
...
// Input  : type -
//-----------------------------------------------------------------------------
void CChoreoView::AddEvent(int type, int subtype /*= 0*/,
                           char const *defaultparameters /*= NULL*/) {
  int mx, my;
  mx = m_nClickedX;
  my = m_nClickedY;
  CChoreoChannelWidget *channel = m_pClickedChannel;
  if (!channel || !channel->GetChannel()) {
    CChoreoActorWidget *actor = m_pClickedActor;
    if (actor) {
      if (actor->GetNumChannels() <= 0) return;

      channel = actor->GetChannel(0);
      if (!channel || !channel->GetChannel()) return;
    } else {
      return;
    }
  }
  ...
  if (defaultparameters) {
    Q_strncpy(params.m_szParameters, defaultparameters,
              sizeof(params.m_szParameters));
  }
  ...
  while (1) {
    ...
    if (Q_strlen(params.m_szName) <= 0) {
      mxMessageBox(this, va("Event must have a valid name"), "Edit Event",
    ...
    if (Q_strlen(params.m_szParameters) <= 0) {
      ...
      if (!shouldBreak) {
        mxMessageBox(this,
                     va("No parameters specified for %s\n", params.m_szName),
  ...
  if (event) {
    ...
    if (params.m_bHasEndTime && params.m_flEndTime != -1.0 &&
    ...
    switch (event->GetType()) {
      ...
      case CChoreoEvent::SPEAK: {
        // Try and load wav to get length
        CAudioSource *wave = sound->LoadSound(
            va("sound/%s", FacePoser_TranslateSoundName(event)));
        if (wave) {
          event->SetEndTime(params.m_flStartTime + wave->GetRunningLength());
          delete wave;
        }

        event->SetSuppressingCaptionAttenuation(
            params.m_bCloseCaptionNoAttenuate);
      } break;
    }
    event->SnapTimes();
...
void CChoreoView::AddGlobalEvent(CChoreoEvent::EVENTTYPE type) {
  ...
  if (!GlobalEventProperties(&params)) return;
...
void CChoreoView::EditGlobalEvent(CChoreoEvent *event) {
  ...
  switch (event->GetType()) {
    default: { event->SetParameters(params.m_szAction); } break;
    case CChoreoEvent::LOOP: {
      event->SetLoopCount(params.m_nLoopCount);
      event->SetParameters(va("%f", params.m_flLoopTime));
    } break;
...
void CChoreoView::EditEvent(CChoreoEvent *event) {
  ...
  switch (params.m_nType) {
    ...
    case CChoreoEvent::SUBSCENE:
      ...
      if (params.m_nType == CChoreoEvent::LOOKAT ||
          params.m_nType == CChoreoEvent::FACE) {
        if (event->GetPitch() != 0 || event->GetYaw() != 0) {
    ...
    default:
      Con_Printf(
          "Don't know how to edit event type %s\n",
  ...
  while (1) {
    ...
    if (Q_strlen(params.m_szName) <= 0) {
      mxMessageBox(this,
                   va("Event %s must have a valid name", event->GetName()),
                   "Edit Event", MX_MB_OK | MX_MB_ERROR);
      continue;
    }
  ...
  if (params.m_bHasEndTime && params.m_flEndTime != -1.0 &&
    ...
    switch (event->GetType()) {
      default:
        break;
      case CChoreoEvent::GESTURE: {
        event->RescaleGestureTimes(event->GetStartTime(),
                                   event->GetEndTime() + dt, true);
      } break;
      case CChoreoEvent::FLEXANIMATION: {
        RescaleExpressionTimes(event, event->GetStartTime(),
                               event->GetEndTime() + dt);
      } break;
    }
    event->SetEndTime(params.m_flEndTime);
  ...
  switch (event->GetType()) {
    ...
    case CChoreoEvent::SPEAK: {
      // Try and load wav to get length
      CAudioSource *wave =
          sound->LoadSound(va("sound/%s", FacePoser_TranslateSoundName(event)));
      if (wave) {
        event->SetEndTime(params.m_flStartTime + wave->GetRunningLength());
        delete wave;
      }
...
CChoreoScene *CChoreoView::LoadScene(char const *filename) {
  ...
  if (!Q_IsAbsolutePath(filename)) {
    filesystem->RelativePathToFullPath(filename, "GAME", pFullPathBuf,
                                       sizeof(pFullPathBuf));
  ...
  CChoreoScene *scene =
      ChoreoLoadScene(filename, this, tokenprocessor, Con_Printf);
  return scene;
}
...
//			*event -
//-----------------------------------------------------------------------------
void CChoreoView::StartEvent(float currenttime, CChoreoScene *scene,
                             CChoreoEvent *event) {
  if (!event || !event->GetActive()) return;
  ...
  switch (event->GetType()) {
      ...
      if (ShouldProcessSpeak()) {
        ...
        if (event->GetCloseCaptionType() == CChoreoEvent::CC_MASTER) {
          ...
          if (event->GetPlaybackCloseCaptionToken(tok, sizeof(tok))) {
            float duration =
                std::max(event->GetDuration(),
                         event->GetLastSlaveEndTime() - event->GetStartTime());

            closecaptionmanager->Process(tok, duration,
                                         GetCloseCaptionLanguageId());

            // Use the token as the sound name lookup, too.
            if (event->IsUsingCombinedFile() && (event->GetNumSlaves() > 0)) {
              Q_strncpy(soundname, tok, sizeof(soundname));
              actualEndTime =
                  std::max(actualEndTime, event->GetLastSlaveEndTime());
            }
          }
        }
    ...
    case CChoreoEvent::STOPPOINT: {
      // Nothing, this is a symbolic event for keeping the vcd alive for ramping
...
//			*event -
//-----------------------------------------------------------------------------
void CChoreoView::EndEvent(float currenttime, CChoreoScene *scene,
                           CChoreoEvent *event) {
  if (!event || !event->GetActive()) return;
...
//			my -
//-----------------------------------------------------------------------------
int CChoreoView::GetTagUnderCursorPos(CChoreoEventWidget *event, int mx,
                                      int my) {
  if (!event) {
    return -1;
  }
  ...
  for (int i = 0; i < event->GetEvent()->GetNumRelativeTags(); i++) {
    ...
    RECT bounds;
    bounds = event->getBounds();
    int left = bounds.left + (int)(tag->GetPercentage() *
                                       (float)(bounds.right - bounds.left) +
                                   0.5f);
...
//			my -
//-----------------------------------------------------------------------------
CEventAbsoluteTag *CChoreoView::GetAbsoluteTagUnderCursorPos(
    CChoreoEventWidget *event, int mx, int my) {
  if (!event) {
    return NULL;
  }

  for (int i = 0;
       i < event->GetEvent()->GetNumAbsoluteTags(CChoreoEvent::PLAYBACK); i++) {
    CEventAbsoluteTag *tag =
        event->GetEvent()->GetAbsoluteTag(CChoreoEvent::PLAYBACK, i);
    if (!tag) continue;
...
//			**event -
//-----------------------------------------------------------------------------
void CChoreoView::GetObjectsUnderMouse(
    int mx, int my, CChoreoActorWidget **actor, CChoreoChannelWidget **channel,
    CChoreoEventWidget **event, CChoreoGlobalEventWidget **globalevent,
    int *clickedTag, CEventAbsoluteTag **absolutetag, int *clickedCCArea) {
  if (actor) {
...
//-----------------------------------------------------------------------------
CChoreoGlobalEventWidget *CChoreoView::GetGlobalEventUnderCursorPos(int mx,
                                                                    int my) {
  POINT check;
  check.x = mx;
  check.y = my;
...
// Input  : *filename -
//-----------------------------------------------------------------------------
void CChoreoView::SetCurrentWaveFile(const char *filename,
                                     CChoreoEvent *event) {
...
//			*param1 -
//-----------------------------------------------------------------------------
void CChoreoView::SelectAllEvents(CChoreoWidget *widget,
                                  CChoreoWidget *param1) {
  CChoreoEventWidget *ew = dynamic_cast<CChoreoEventWidget *>(widget);
  CChoreoGlobalEventWidget *gew =
      dynamic_cast<CChoreoGlobalEventWidget *>(widget);
...
bool CChoreoView::CreateAnimationEvent(int mx, int my,
                                       char const *animationname) {
  if (!animationname || !animationname[0]) return false;
  ...
  GetObjectsUnderMouse(pt.x, pt.y, &m_pClickedActor, &m_pClickedChannel,
                       &m_pClickedEvent, &m_pClickedGlobalEvent, &m_nClickedTag,
                       &m_pClickedAbsoluteTag,
                       &m_nClickedChannelCloseCaptionButton);
  ...
  // At this point we need to ask the user what type of even to create (gesture
  // or sequence) and just show the approprite dialog
  CChoiceParams params;
  ...
  if (!ChoiceProperties(&params)) return false;

  if (params.m_nSelected < 0) return false;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CChoreoView::CreateExpressionEvent(int mx, int my, CExpClass *cl,
                                        CExpression *exp) {
...
void CChoreoView::UpdateCurrentSettings(void) {
  ...
  for (int i = 0; i < m_SceneActors.Size(); i++) {
    ...
    for (LocalFlexController_t j = LocalFlexController_t(0);
         j < hdr->numflexcontrollers(); j++) {
...
void CChoreoView::AddEventRelativeTag(void) {
  ...
  if (!InputProperties(&params)) return;
  ...
  float frac = 0.0f;
  if (bounds.right - bounds.left > 0) {
    frac = (float)(m_nClickedX - bounds.left) /
           (float)(bounds.right - bounds.left);
...
void CChoreoView::UpdateStatusArea(int mx, int my) {
  ...
  GetObjectsUnderMouse(mx, my, &fo.a, &fo.c, &fo.e, &fo.ge, &fo.tag, &fo.at,
                       &fo.ccbutton);
...
//			rcStatus -
//-----------------------------------------------------------------------------
void CChoreoView::RedrawStatusArea(CChoreoWidgetDrawHelper &drawHelper,
                                   RECT &rcStatus) {
  drawHelper.DrawFilledRect(COLOR_CHOREO_BACKGROUND, rcStatus);

  drawHelper.DrawColoredLine(COLOR_INFO_BORDER, PS_SOLID, 1, rcStatus.left,
                             rcStatus.top, rcStatus.right, rcStatus.top);
  ...
  if (m_Flyover.c && m_Flyover.ccbutton != -1) {
  ...
  if (m_pScene) {
    ...
    if (mapname) {
      ...
      drawHelper.DrawColoredText("Arial", fontsize, fontweight, COLOR_INFO_TEXT,
                                 rcText, sz);
  ...
  //	drawHelper.DrawColoredText( "Arial", 12, 500, RGB( 0, 0, 0 ), rcInfo,
  //m_Flyover.e ? m_Flyover.e->GetEvent()->GetName() : "" );
}
...
void CChoreoView::OnGestureTool(void) {
  if (m_pClickedEvent->GetEvent()->GetType() != CChoreoEvent::GESTURE) return;
...
void CChoreoView::ImportEvents(void) {
  ...
  char eventfile[512];
  if (!FacePoser_ShowOpenFileNameDialog(eventfile, sizeof(eventfile), "scenes",
                                        "*.vce"))
    return;
  ...
  if (!Q_IsAbsolutePath(eventfile)) {
    filesystem->RelativePathToFullPath(eventfile, "GAME", fullpathbuf,
                                       sizeof(fullpathbuf));
  ...
  m_pScene->ImportEvents(tokenprocessor, m_pClickedActor->GetActor(),
                         m_pClickedChannel->GetChannel());
...
void CChoreoView::ExportEvents(void) {
  ...
  if (!FacePoser_ShowSaveFileNameDialog(eventfilename, sizeof(eventfilename),
                                        "scenes", "*.vce"))
    return;
...
void CChoreoView::ImportVCDFile(char const *filename) {
  ...
  // Now walk through the "new" events and select everything that wasn't already
  // there (all of the stuff that was "added" during the merge)
  for (i = 0; i < m_SceneActors.Count(); ++i) {
    CChoreoActorWidget *actor = m_SceneActors[i];
    if (!actor) continue;
...
void CChoreoView::ProcessSpeak(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  if (!mixer || !sound->IsSoundPlaying(mixer)) {
    ...
    sound->PlaySound(
        model, volume,
        va("sound/%s", FacePoser_TranslateSoundName(soundname, model)), &mixer);
    event->SetMixer(mixer);
  }
  ...
  int samplelimit = mixer->GetSource()->SampleRate() *
                    0.02f;  // don't shift until samples are off by this much
  if (IsScrubbing()) {
    samplelimit = mixer->GetSource()->SampleRate() *
                  0.01f;  // make it shorter tolerance when scrubbing
  }
...
void CChoreoView::ProcessSubscene(CChoreoScene *scene, CChoreoEvent *event) {
  ...
  if (subscene->SimulationFinished()) return;
...
void CChoreoView::GetScrubHandleRect(RECT &rcHandle, bool clipped) {
  ...
  if (m_pScene) {
    ...
    if (clipped) {
      pixel = std::clamp(pixel, SCRUBBER_HANDLE_WIDTH / 2,
                         w2() - SCRUBBER_HANDLE_WIDTH / 2);
    }
  }
...
void CChoreoView::ScrubThink(float dt, bool scrubbing,
  ...
  if (thisframe == lastthinkframe) return;
...
void CChoreoView::ClampTimeToSelectionInterval(float &timeval) {
  // TODO(d.rattman): hook this up later
  return;
}
...
void CChoreoView::OnChangeScale(void) {
  ...
  Q_snprintf(params.m_szInputText, sizeof(params.m_szInputText), "%.2f",
             (float)GetTimeZoom(GetToolName()) / 100.0f);

  if (!InputProperties(&params)) return;

  SetTimeZoom(
      GetToolName(),
      std::clamp((int)(100.0f * atof(params.m_szInputText)), 1, MAX_TIME_ZOOM),
      false);
...
//			*event -
//-----------------------------------------------------------------------------
void CChoreoView::ProcessPermitResponses(CChoreoScene *scene,
                                         CChoreoEvent *event) {}

void CChoreoView::ApplyBounds(int &mx, int &my) {
  if (!m_bUseBounds) return;
...
// Output : float
//-----------------------------------------------------------------------------
float CChoreoView::FindNextEventTime(CChoreoEvent::EVENTTYPE type,
                                     CChoreoChannel *channel, CChoreoEvent *e,
                                     bool forward) {
  bool foundone = false;
  float bestTime = -1.0f;
...
void CChoreoView::CalcBounds(int movetype) {
  ...
  switch (movetype) {
    ...
    case DRAGTYPE_EVENT_ENDTIME_RESCALE: {
      ...
      // TODO(d.rattman): use this for finding adjacent gesture edges (kenb)
      if (m_bUseBounds) {
        ...
        int nextPixel =
            forwardTime != -1 ? GetPixelForTimeValue(forwardTime) : m_nMaxX;
        int prevPixel =
            reverseTime != -1 ? GetPixelForTimeValue(reverseTime) : m_nMinX;
...
bool CChoreoView::ShouldSelectEvent(SelectionParams_t &params,
                                    CChoreoEvent *event) {
  if (params.forward) {
    if (event->GetStartTime() >= params.time) return true;
  } else {
    float endtime =
        event->HasEndTime() ? event->GetEndTime() : event->GetStartTime();

    if (endtime <= params.time) return true;
  }
  return false;
}
...
void CChoreoView::SelectEvents(SelectionParams_t &params) {
  ...
  CChoreoChannel *channel =
      m_pClickedChannel ? m_pClickedChannel->GetChannel() : NULL;
...
int CChoreoView::GetTimeZoom(char const *tool) {
  if (!m_pScene) return 100;

  return m_pScene->GetTimeZoom(tool);
}

void CChoreoView::CheckInsertTime(CChoreoEvent *e, float dt, float starttime,
                                  float endtime) {
  // Not influenced
  ...
  if (e->GetStartTime() > starttime) {
    e->OffsetTime(dt);
    e->SnapTimes();
  } else if (!e->IsFixedLength() &&
             e->HasEndTime())  // the event starts before start, but ends after
                               // start time, need to insert space into the
                               // event, act like user dragged end time
  {
    float newduration = e->GetDuration() + dt;
    RescaleRamp(e, newduration);
    switch (e->GetType()) {
  ...
  switch (e->GetType()) {
    ...
    case CChoreoEvent::SPEAK: {
      // Try and load wav to get length
      CAudioSource *wave =
          sound->LoadSound(va("sound/%s", FacePoser_TranslateSoundName(e)));
      if (wave) {
        e->SetEndTime(e->GetStartTime() + wave->GetRunningLength());
        delete wave;
      }
...
void CChoreoView::CheckDeleteTime(CChoreoEvent *e, float dt, float starttime,
                                  float endtime, bool &deleteEvent) {
  deleteEvent = false;
    ...
    if (newduration <= 0.0f) {
      ...
      switch (e->GetType()) {
        default:
          break;
        case CChoreoEvent::GESTURE: {
          e->RescaleGestureTimes(e->GetStartTime(), e->GetEndTime() - maxoffset,
                                 true);
        } break;
        case CChoreoEvent::FLEXANIMATION: {
          RescaleExpressionTimes(e, e->GetStartTime(),
                                 e->GetEndTime() - maxoffset);
        } break;
      }
...
static bool EventStartTimeLessFunc(CChoreoEvent *const &p1,
                                   CChoreoEvent *const &p2) {
  CChoreoEvent *w1;
  CChoreoEvent *w2;
...
bool CChoreoView::GenerateCombinedFile(char const *outfilename,
                                       char const *cctoken, gender_t gender,
                                       CUtlRBTree<CChoreoEvent *> &sorted) {
  CUtlVector<CombinerEntry> work;

  char actualfile[512];
  soundemitter->GenderExpandString(gender, outfilename, actualfile,
                                   sizeof(actualfile));
  ...
  if (i != sorted.InvalidIndex()) {
    ...
    do {
      ...
      CombinerEntry ce;
      Q_snprintf(
          ce.wavefile, sizeof(ce.wavefile), "sound/%s",
          FacePoser_TranslateSoundNameGender(e->GetParameters(), gender));
...
bool CChoreoView::ValidateCombinedFileCheckSum(
    char const *outfilename, char const *cctoken, gender_t gender,
    CUtlRBTree<CChoreoEvent *> &sorted) {
  CUtlVector<CombinerEntry> work;
  ...
  return soundcombiner->IsCombinedFileChecksumValid(filesystem, actualfile,
                                                    work);
}

void SuggestCaption(char *dest, int destlen,
                    CUtlVector<CChoreoEvent *> &events) {
  // Walk through events and concatenate current captions, or raw wav data if
  // have any
  dest[0] = 0;
...
void CChoreoView::OnCombineSpeakEvents() {
  ...
  for (int i = c - 1; i >= 0; --i) {
    ...
    if (e->GetType() != CChoreoEvent::SPEAK) {
      Con_ErrorPrintf(
          "Can't combine events, all events must be SPEAK events.\n");
      return;
    }
    ...
    if (!firstChannel) {
      ...
      Con_ErrorPrintf(
          "Can't combine events, all events must reside in the same "
          "channel.\n");
      return;
    }
  }
  ...
  if (selected.Count() < 2) {
    Con_ErrorPrintf(
        "Can't combine events, must have at least two events selected.\n");
    return;
  }

  // Let the user pick a CC phrase
  CCloseCaptionLookupParams params;
  Q_strncpy(params.m_szDialogTitle, "Choose Close Caption Token",
            sizeof(params.m_szDialogTitle));
  ...
  if (!selected[0]->GetPlaybackCloseCaptionToken(playbacktoken,
                                                 sizeof(playbacktoken))) {
    return;
  }

  if (!Q_stristr(playbacktoken, "_cc")) {
    Q_strncpy(params.m_szCCToken, va("%s_cc", playbacktoken),
              sizeof(params.m_szCCToken));
  } else {
    Q_strncpy(params.m_szCCToken, va("%s", playbacktoken),
              sizeof(params.m_szCCToken));
  }

  // User hit okay and value actually changed?
  if (!CloseCaptionLookup(&params) && params.m_szCCToken[0] != 0) {
    return;
  }
  ...
  if (INVALID_STRING_INDEX == stringIndex) {
    ...
    Q_strncpy(ip.m_szDialogTitle, "Add Close Caption",
              sizeof(ip.m_szDialogTitle));
    Q_snprintf(ip.m_szPrompt, sizeof(ip.m_szPrompt),
               "Token (%s):", params.m_szCCToken);
    ...
    if (Q_strlen(ip.m_szInputText) == 0) {
      Q_snprintf(ip.m_szInputText, sizeof(ip.m_szInputText), "!!!%s",
                 params.m_szCCToken);
    }
  ...
  if (!e->ComputeCombinedBaseFileName(outfilename, sizeof(outfilename),
                                      genderwildcard)) {
  ...
  if (!scriptfile || !scriptfile[0]) {
    Con_ErrorPrintf(
        "Unable to find existing script to use for new combined sound "
        "entry.\n");
    return;
  }
  ...
  Q_strncpy(asp.m_szDialogTitle, "Add Combined Sound Entry",
            sizeof(asp.m_szDialogTitle));
  Q_strncpy(asp.m_szWaveFile, outfilename + Q_strlen("sound/"),
            sizeof(asp.m_szWaveFile));
  ...
  if (genderwildcard) {
    ...
    GenerateCombinedFile(outfilename, params.m_szCCToken, GENDER_FEMALE,
                         sorted);
  } else {
...
bool CChoreoView::ValidateCombinedSoundCheckSum(CChoreoEvent *e) {
  if (!e || e->GetType() != CChoreoEvent::SPEAK) return false;
  ...
  if (!e->GetChannel()->GetSortedCombinedEventList(e->GetCloseCaptionToken(),
                                                   eventList)) {
  ...
  if (genderwildcard) {
    checksumvalid = ValidateCombinedFileCheckSum(
        outfilename, e->GetCloseCaptionToken(), GENDER_MALE, eventList);
    checksumvalid &= ValidateCombinedFileCheckSum(
        outfilename, e->GetCloseCaptionToken(), GENDER_FEMALE, eventList);
  } else {
    checksumvalid = ValidateCombinedFileCheckSum(
        outfilename, e->GetCloseCaptionToken(), GENDER_NONE, eventList);
  }
...
void CChoreoView::OnRemoveSpeakEventFromGroup() {
  ...
  if (GetSelectedEvents(selected) > 0) {
    ...
    m_pClickedChannel->GetMasterAndSlaves(
        m_pClickedChannel->GetCaptionClickedEvent(), processlist);
  }
...
bool CChoreoView::AreSelectedEventsCombinable() {
  CUtlVector<CChoreoEvent *> events;
  if (GetSelectedEvents(events) <= 0) return false;
...
void CChoreoView::OnChangeCloseCaptionToken(CChoreoEvent *e) {
  CCloseCaptionLookupParams params;
  Q_strncpy(params.m_szDialogTitle, "Close Caption Token Lookup",
            sizeof(params.m_szDialogTitle));
  ...
  Q_strncpy(params.m_szCCToken, e->GetCloseCaptionToken(),
            sizeof(params.m_szCCToken));
...
void CChoreoView::OnToggleCloseCaptionsForEvent() {
  ...
  Con_Printf("Close Caption type for '%s' changed to '%s'\n", e->GetName(),
             CChoreoEvent::NameForCCType(newType));
}
...
void DeleteAllAndPurge(T &tree) {
  ...
  for (i = tree.FirstInorder(); i != T::InvalidIndex();
...
void CChoreoView::OnPlaceNextSpeakEvent() {
  ...
  if (nextIdx == m_SortedNames.InvalidIndex()) {
    Warning(
        "Can't place sound event, can't next sound after '%s' in sound list\n",
        ev->GetParameters());
    DeleteAllAndPurge(m_SortedNames);
    return;
  }
...
static int FindMetric(int type, CUtlVector<CChoreoEvent *> &list,
                      float &value) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (type != FM_LEFT && !e->HasEndTime()) continue;
...
void CChoreoView::OnMakeSameSize(bool smallest) {
  ...
  float value;
  int idx =
      FindMetric(smallest ? FM_SMALLESTWIDE : FM_LARGESTWIDE, list, value);
  if (idx == -1) {
    return;
  }
  ...
  char undotext[128];
  Q_snprintf(undotext, sizeof(undotext), "Size to %s",
...
void CChoreoView::SelectInChannel(CChoreoWidget *widget,
                                  CChoreoWidget *param1) {
  CChoreoEventWidget *ev = dynamic_cast<CChoreoEventWidget *>(widget);
  if (!ev) return;
