...
class CFacePoserVguiFrame : public Frame {
 ...
 public:
  ...
  MESSAGE_FUNC(OnButtonPressed, "OnButtonPressed") { Msg("OnButtonPressed\n"); }
...
class TestWindow : public CVGuiPanelWnd, public IFacePoserToolWindow {
 ...
 public:
  ...
  virtual int handleEvent(mxEvent *event) {
    if (HandleToolEvent(event)) return 1;
...
void CreatePath(const char *relative) {
  ...
  for (ofs = const_cast<char *>(path + 1); *ofs; ofs++) {
    c = *ofs;
    if (c == '/' || c == '\\') {
      // create the directory, but not if it's actually a filename with a dot in
      // it!!!
      *ofs = 0;
      if (!Q_stristr(path, ".")) {
        Q_mkdir(path);
...
void MDLViewer::initRecentFiles() {
  for (int i = 0; i < 8; i++) {
    if (strlen(recentFiles[i])) {
      mb->modify(IDC_FILE_RECENTFILES1 + i, IDC_FILE_RECENTFILES1 + i,
                 recentFiles[i]);
    } else {
      mb->modify(IDC_FILE_RECENTFILES1 + i, IDC_FILE_RECENTFILES1 + i,
                 "(empty)");
...
bool MDLViewer::CanClose() {
  ...
  if (m_bVCDSaved) {
    int retval = mxMessageBox(NULL, "Rebuild scenes.image?", g_appTitle,
                              MX_MB_YESNOCANCEL);
    if (retval == 2) {
      return false;
    }
  ...
  int c = soundemitter->GetNumSoundScripts();
  for (int i = 0; i < c; i++) {
    if (!soundemitter->IsSoundScriptDirty(i)) continue;
    ...
    int retval =
        mxMessageBox(NULL, va("Save changes to sound script '%s'?", scriptname),
...
bool MDLViewer::Closing(void) { return true; }
...
class CFlatButton : public mxButton {
 public:
  CFlatButton(mxWindow *parent, int id) : mxButton(parent, 0, 0, 0, 0, "", id) {
  ...
  void Init(void) {
    if (g_pChoreoView) {
      ...
      if (scene) {
        ...
        Q_snprintf(sz, sizeof(sz), "snap: %s",
...
class CMDLViewerModelTab : public CTabWindow {
 public:
  ...
  CMDLViewerModelTab(mxWindow *parent, int x, int y, int w, int h, int id = 0,
                     int style = 0)
    ...
    if (idx >= 0) {
      ...
      if (scene) {
        ...
        for (int i = 0; i < c; i++) {
          ...
          if (stricmp(a->GetFacePoserModelName(), filename)) continue;
          hasassoc = true;
          break;
        }

        if (hasassoc) {
          pop->add(va("Change associated actor for %s", current),
                   IDC_MODELTAB_ASSOCIATEACTOR);
        } else {
          pop->add(va("Associate actor to %s", current),
                   IDC_MODELTAB_ASSOCIATEACTOR);
        }
      }
      ...
      if (visible) {
        pop->add(va("Remove %s from 3D View", current),
                 IDC_MODELTAB_TOGGLE3DVIEW);
      } else {
        ...
        switch (event->action) {
          ...
          case IDC_MODELTAB_LOAD: {
            if (filesystem->IsSteam()) {
              ...
              char modelfile[512];
              if (FacePoser_ShowOpenFileNameDialog(modelfile, sizeof(modelfile),
                                                   "models", "*.mdl")) {
          ...
          case IDC_MODELTAB_ASSOCIATEACTOR: {
            ...
            if (idx >= 0) {
              ...
              if (scene) {
                ...
                if (ChoiceProperties(&params) && params.m_nSelected != oldsel) {
                  // Chose something new...
...
class CMDLViewerWindowTab : public CTabWindow {
 public:
  ...
  CMDLViewerWindowTab(mxWindow *parent, int x, int y, int w, int h, int id = 0,
                      int style = 0)
      : CTabWindow(parent, x, y, w, h, id, style) {
    SetInverted(true);
...
class CMDLViewerWorkspace : public mxWindow {
 public:
  CMDLViewerWorkspace(mxWindow *parent, int x, int y, int w, int h,
                      const char *label = 0, int style = 0)
...
void MDLViewer::LoadPosition(void) {
  ...
  FacePoser_LoadWindowPositions("MDLViewer", visible, x, y, w, h, locked,
                                zoomed);
...
void MDLViewer::SavePosition(void) {
  ...
  FacePoser_SaveWindowPositions("MDLViewer", visible, xpos, ypos, width, height,
                                false, zoomed);
}
...
MDLViewer::MDLViewer()
  ...
  g_pMatSysWindow =
      new MatSysWindow(workspace, 0, 0, 100, 100, "", mxWindow::Normal);
  ...
  g_pAnimationBrowserTool =
      new AnimationBrowser(workspace, IDC_ANIMATIONBROWSER);
...
void MDLViewer::UpdateWindowMenu(void) {
  ...
  for (int i = 0; i < c; i++) {
    IFacePoserToolWindow *tool = IFacePoserToolWindow::GetTool(i);
    menuWindow->setChecked(IDC_WINDOW_FIRSTTOOL + i,
                           tool->GetMxWindow()->isVisible());
  }
}
...
void MDLViewer::UpdateLanguageMenu(int currentLanguageId) {
  if (!menuCloseCaptionLanguages) return;

  for (int i = 0; i < CC_NUM_LANGUAGES; i++) {
    int id = IDC_OPTIONS_LANGUAGESTART + i;
    menuCloseCaptionLanguages->setChecked(
        id, i == currentLanguageId ? true : false);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void MDLViewer::InitModelTab(void) { modeltab->Init(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void MDLViewer::InitGridSettings(void) { gridsettings->Init(); }

//-----------------------------------------------------------------------------
// Purpose:
// Output : int
//-----------------------------------------------------------------------------
int MDLViewer::GetActiveModelTab(void) { return modeltab->getSelectedIndex(); }
...
void MDLViewer::Refresh(void) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (retval == 0) {
      soundemitter->SaveChangesToSoundScript(i);
      Con_ColorPrintf(RGB(50, 255, 100),
                      "  saving changes to script file '%s'\n", scriptname);
    }
  }
...
void MDLViewer::OnFileLoaded(char const *pszFile) {
  int i;
  for (i = 0; i < 8; i++) {
    if (!Q_stricmp(recentFiles[i], pszFile)) break;
  }
  ...
  // insert recent file
    ...
    for (i = 7; i > 0; i--) strcpy(recentFiles[i], recentFiles[i - 1]);
...
static bool WindowContainsPoint(mxWindow *wnd, int x, int y) {
  ...
  if (pt.x > wnd->w()) return false;
  if (pt.y > wnd->h()) return false;
...
int MDLViewer::handleEvent(mxEvent *event) {
  ...
  switch (event->event) {
    case mxEvent::Size: {
      ...
      gridsettings->setBounds(gridstart, height - rowheight + 1,
                              gridsettingswide, WINDOW_TAB_OFFSET - 2);
    ...
    case mxEvent::Action: {
      ...
      switch (event->action) {
        ...
        case IDC_FILE_LOADGROUNDTEX: {
          ...
          if (ptr) {
            if (0 /* g_pMatSysWindow->loadTexture (ptr, event->action - IDC_FILE_LOADBACKGROUNDTEX) */) {
              ...
              mxMessageBox(this, "Error loading texture.", g_appTitle,
        ...
        case IDC_OPTIONS_COLORLIGHT: {
          float *cols[3] = {g_viewerSettings.bgColor, g_viewerSettings.gColor,
                            g_viewerSettings.lColor};
        ...
        case IDC_HELP_GOTOHOMEPAGE:
          ShellExecute(
              0, "open",
              "http://developer.valvesoftware.com/wiki/Category:Choreography",
              0, 0, SW_SHOW);
          break;
#endif
        ...
        case IDC_HELP_ABOUT:
          mxMessageBox(
              ...
              "",
              "Valve Face Poser", MX_MB_OK | MX_MB_INFORMATION);
          break;
        ...
        case IDC_EXPRESSIONS_REDOBITMAPS: {
          ...
          if (active) {
            ...
            for (int i = 0; i < active->GetNumExpressions(); i++) {
              ...
              g_pProgressDialog->Update((float)i /
                                        (float)active->GetNumExpressions());
        ...
        case IDC_EXPRESSIONS_NEW: {
          char classfile[512];
          if (FacePoser_ShowSaveFileNameDialog(classfile, sizeof(classfile),
                                               "expressions", "*.txt")) {
        ...
        case IDC_EXPRESSIONS_LOAD: {
          char classfile[512];
          if (FacePoser_ShowOpenFileNameDialog(classfile, sizeof(classfile),
                                               "expressions", "*.txt")) {
        ...
        default: {
          ...
          if (tool_number >= 0 && tool_number <= max_tools &&
              tool_number < IFacePoserToolWindow::GetToolCount()) {
            iret = 1;
            IFacePoserToolWindow *tool =
                IFacePoserToolWindow::GetTool(tool_number);
            if (tool) {
              mxWindow *toolw = tool->GetMxWindow();
          ...
          if (lang_number >= 0 && lang_number < CC_NUM_LANGUAGES) {
            iret = 1;
...
int MDLViewer::GetCurrentFrame(void) { return m_nCurrentFrame; }
...
// Output : int
//-----------------------------------------------------------------------------
int MDLViewer::GetCurrentHitboxSet(void) { return 0; }
...
void MDLViewer::OnRebuildScenesImage() {
  ...
  const char *pFilename =
      bLittleEndian ? "scenes/scenes.image" : "scenes/scenes.360.image";

  CP4AutoEditAddFile checkout(pFilename);

  bool bSuccess = g_pSceneImage->CreateSceneImageFile(
      targetBuffer, gamedir, bLittleEndian, false, this);
  if (bSuccess) {
...
void MDLViewer::UpdateStatus(char const *pchSceneName, bool bQuiet, int nIndex,
                             int nCount) {
  g_pProgressDialog->UpdateText(pchSceneName);
  g_pProgressDialog->Update((float)nIndex / (float)nCount);
}

void MDLViewer::OnVCDSaved() { m_bVCDSaved = true; }
...
void MDLViewer::OnSaveSoundScriptChanges() {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (!filesystem->IsFileWritable(scriptname)) {
      mxMessageBox(
          NULL,
          va("Can't save changes to sound script '%s', file is READ-ONLY?",
             scriptname),
          g_appTitle, MX_MB_OK);
      continue;
    }
...
const char *GetGameDirectory() {
  // TODO: get rid of this and ONLY use the filesystem, so hlfaceposer works
  // nicely for mods that get the base game resources from the Steam filesystem.
  return gamedir;
}

char const *GetGameDirectorySimple() { return gamedirsimple; }
...
bool CHLFacePoserApp::PreInit() {
  ...
  g_pStudioDataCache =
      (IStudioDataCache *)FindSystem(STUDIO_DATA_CACHE_INTERFACE_VERSION);
  physcollision =
      (IPhysicsCollision *)FindSystem(VPHYSICS_COLLISION_INTERFACE_VERSION);
  physprop = (IPhysicsSurfaceProps *)FindSystem(
      VPHYSICS_SURFACEPROPS_INTERFACE_VERSION);
  g_pLocalize = (ILocalize *)FindSystem(VGUI_LOCALIZE_INTERFACE_VERSION);
  soundemitter = (ISoundEmitterSystemBase *)FindSystem(
      SOUNDEMITTERSYSTEM_INTERFACE_VERSION);

  if (!soundemitter || !g_pLocalize || !filesystem || !physprop ||
      !physcollision || !g_pMaterialSystem || !g_pStudioRender ||
      !g_pMDLCache || !g_pDataCache) {
...
int CHLFacePoserApp::Main() {
  ...
  for (int i = 0; i < files; i++) {
    expressions->LoadClass(
        workspacefiles->GetStoredFile(IWorkspaceFiles::EXPRESSION, i));
  }
...
static bool CHLFacePoserApp_SuggestGameInfoDirFn(
  ...
  for (int i = 1; i < CommandLine()->ParmCount(); i++) {
    if (Q_stristr(CommandLine()->GetParm(i), ".mdl")) {
      Q_MakeAbsolutePath(pchPathBuffer, nBufferLength,
                         CommandLine()->GetParm(i));
      return true;
    }
  }
