...
class CClassVariable {
 public:
  ...
  typedef enum { TPUBLIC = 0, TPROTECTED, TPRIVATE } VARTYPE;
...
class CClassMemberFunction {
 public:
  typedef enum { TPUBLIC = 0, TPROTECTED, TPRIVATE } MEMBERTYPE;
...
class CClass {
 public:
  ...
  bool CheckForMissingTypeDescriptionFields(int &missingcount,
                                            bool createtds = false);
  bool CheckForMissingPredictionFields(int &missingcount,
                                       bool createtds = false);
  bool CheckForPredictionFieldsInRecvTableNotMarkedAsSuchCorrectly(
      int &missingcount);
  void AddVariable(int protection, char *type, char *name, bool array,
                   char *std::size = 0);
  ...
  CTypeDescriptionField *AddTD(const char *name, const char *type,
  ...
  CTypeDescriptionField *AddPredTD(const char *name, const char *type,
                                   const char *definetype, bool incomments,
                                   bool inrecvtable);
  int m_nPredTDCount;
