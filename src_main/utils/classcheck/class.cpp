...
CTypeDescriptionField *CClass::FindTD(const char *name) {
  for (int i = 0; i < m_nTDCount; i++) {
    if (!strcmp(m_TDFields[i]->m_szVariableName, name)) return m_TDFields[i];
  }
  return NULL;
}
...
CClassVariable *CClass::FindVar(const char *name,
                                bool checkbaseclasses /*= false*/) {
  CClass *cl = this;
  while (cl) {
...
CClassMemberFunction *CClass::FindMember(const char *name) {
  for (int i = 0; i < m_nMemberCount; i++) {
    if (!strcmp(m_Members[i]->m_szName, name)) return m_Members[i];
  }
  return NULL;
}

CTypeDescriptionField *CClass::AddTD(const char *name, const char *type,
                                     const char *definetype, bool incomments) {
...
CTypeDescriptionField *CClass::AddPredTD(const char *name, const char *type,
                                         const char *definetype,
                                         bool incomments, bool inrecvtable) {
...
static bool IsType(char *input, char *test) {
  ...
  if ((pMatch[nLen] != 0) && (pMatch[nLen] != ' ')) return false;
...
void CClass::ReportTypeMismatches(CClassVariable *var,
                                  CTypeDescriptionField *td) {
  ...
  if (!strcmp(td->m_szType, "FIELD_TIME")) {
    if (!strcmp(t, "FIELD_FLOAT")) return;
  }

  if (!strcmp(td->m_szType, "FIELD_TICK")) {
    if (!strcmp(t, "FIELD_INTEGER")) return;
  }

  if (!strcmp(td->m_szType, "FIELD_MODELNAME") ||
      !strcmp(td->m_szType, "FIELD_SOUNDNAME")) {
    if (!strcmp(t, "FIELD_STRING")) return;
  }

  if (!strcmp(td->m_szType, "FIELD_MODELINDEX") ||
      !strcmp(td->m_szType, "FIELD_MATERIALINDEX")) {
    if (!strcmp(t, "FIELD_INTEGER")) return;
  }

  if (!strcmp(td->m_szType, "FIELD_POSITION_VECTOR")) {
    if (!strcmp(t, "FIELD_VECTOR")) return;
  }

  if (!strcmp(td->m_szType, "FIELD_VMATRIX_WORLDSPACE")) {
    if (!strcmp(t, "FIELD_VMATRIX")) return;
  }

  if (strcmp(t, td->m_szType)) {
    vprint(0,
           "class %s has an incorrect FIELD_ type for variable '%s (%s, %s)'\n",
           m_szName, var->m_szName, var->m_szType, td->m_szType);
  }
}

bool CClass::CheckForMissingTypeDescriptionFields(int &missingcount,
                                                  bool createtds) {
  bool bret = false;
  missingcount = 0;
  ...
  for (int i = 0; i < m_nVarCount; i++) {
    ...
    if (!createtds) {
      vprint(0,
             "class %s missing typedescription_t field for variable '%s %s'\n",
    ...
    vprint(0, "//\tClass %s:\n", m_szName);
    if (var->m_bIsArray &&
        (stricmp(var->m_szType, "char") || stricmp(t, "FIELD_STRING"))) {
      if (*t) {
...
bool CClass::CheckForPredictionFieldsInRecvTableNotMarkedAsSuchCorrectly(
    int &missingcount) {
  bool bret = false;
  missingcount = 0;
  // Didn't specify a TYPEDESCRIPTION at all
  ...
  for (int i = 0; i < m_nVarCount; i++) {
    ...
    if (!strcmp(td->m_szDefineType, "DEFINE_PRED_TYPEDESCRIPTION")) {
      ...
      if (cl2) {
        bret = cl2->CheckForPredictionFieldsInRecvTableNotMarkedAsSuchCorrectly(
            missingcount);
      }
      continue;
    }
    ...
    if (inreceivetable && !td->m_bRepresentedInRecvTable) {
      vprint(0,
             "%s::%s:  Missing FTYPEDESC_INSENDTABLE flag in prediction "
             "typedescription\n",
             m_szName, var->m_szName);
    } else {
      vprint(0,
             "%s::%s:  Field marked as FTYPEDESC_INSENDTABLE in prediction "
             "typedescription missing from RecvTable\n",
             m_szName, var->m_szName);
    }
  }
...
bool CClass::CheckForMissingPredictionFields(int &missingcount,
                                             bool createtds) {
  bool bret = false;
  missingcount = 0;
  // Didn't specify a TYPEDESCRIPTION at all
  ...
  for (int i = 0; i < m_nVarCount; i++) {
    ...
    // private and protected variables can't be referenced in data tables right
    // now
    ...
    if (!isstatic) {
      ...
      if (!td) {
        ...
        if (!createtds) {
          vprint(0,
                 "class %s missing prediction typedescription_t field for "
                 "variable '%s %s'\n",
                 m_szName, var->m_szType, var->m_szName);
        } else {
          char const *t = TranslateSimpleType(var);
          ...
          if (var->m_bIsArray &&
            ...
            if (*t) {
              ...
              vprint(0, "\t// DEFINE_ARRAY( %s, %s, %s ),\n", goodname,
                     var->m_szType, var->m_szArraySize);
            }
          } else {
            if (*t) {
              vprint(0, "\tDEFINE_FIELD( %s, %s ),\n", goodname, t);
...
bool IsMissingType(CClass *cl, CClassVariable *var) {
  ...
  if (lookup != missing_types.InvalidIndex()) return true;
...
void CClass::AddVariable(int protection, char *type, char *name, bool array,
                         char *size) {
...
bool CClass::ParseNetworkVar(char *&input, int protection) {
  ...
  if (!strcmp(com_token, "CNetworkHandle") ||
      !strcmp(com_token, "CNetworkHandleForDerived")) {
    input = CC_ParseToken(input);
    Assert(!strcmp(com_token, "("));
  ...
  if (!strcmp(com_token, "CNetworkArray") ||
      !strcmp(com_token, "CNetworkArrayForDerived")) {
    input = CC_ParseToken(input);
    Assert(!strcmp(com_token, "("));
...
bool CClass::ParseClassMember(char *&input, int protection) {
  ...
  do {
    ...
    // It's a templatized var
    else if ((com_token[strlen(com_token) - 1] == '<') &&
             strcmp(var.m_pName, "operator")) {
      do {
      ...
      if (!stricmp(var.m_pName, "typedef") || !stricmp(var.m_pName, "enum") ||
  ...
  */
  ...
  if (isfunction) {
    ...
    do {
      if (!skipvar) {
        AddVariable(protection, var.m_pType, var.m_pName, var.m_bArray,
                    var.m_pArraySize);
...
bool CClass::ParseNestedClass(char *&input) {
  if (stricmp(com_token, "struct") && stricmp(com_token, "class")) return false;
...
// if token is not one of those, then parse and concatenate all tokens up to the
// first
//   ; or (
//
...
static bool ShouldHungarianCheck(char const *name) {
  if (!Q_strncmp(name, "m_", 2) || !Q_strncmp(name, "g_", 2) ||
      !Q_strncmp(name, "s_", 2)) {
    return true;
  }
...
void CClass::CheckForHungarianErrors(int &warnings) {
  ...
  for (int i = 0; i < m_nVarCount; i++) {
    ...
    for (int j = 0; j < testcount; ++j) {
      ...
      bool match =
          !Q_strncmp(var->m_szName + 2, tst->prefix, Q_strlen(tst->prefix))
              ? true
              : false;
      if (!match) continue;
      ...
      bool typeFound =
          Q_stristr(var->m_szType, tst->mustinclude) ? true : false;
