...
-- MySQL code to create the databases, create the users, and set access
privileges.
...
-- This updates the "user" table, which is checked when someone tries to connect
to the database. grant select,insert,update on vrad.* to vrad_worker; grant
select on vrad.* to vmpi_browser; flush privileges;

/*

-- SQL code to (re)create the tables.

-- Master generates a unique job ID (in job_master_start) and sends it to
workers.
-- Each worker (and the master) make a job_worker_start, link it to the primary
job ID,
...
-- NOTE: do a "use vrad" or "use vvis" first, depending on the DB you want to
create.
...
AUTO_INCREMENT,	index id( JobID, MachineName(5) ), BSPFilename
TINYTEXT NOT NULL, StartTime				TIMESTAMP NOT NULL,
...
create table job_master_end (
        JobID					INTEGER UNSIGNED NOT NULL,
PRIMARY KEY ( JobID ), NumWorkersConnected		SMALLINT UNSIGNED NOT
NULL, NumWorkersDisconnected	SMALLINT UNSIGNED NOT NULL, ErrorText
TEXT NOT NULL
        );
...
-- Set to 1 if this "worker" is the master process. RunningTimeMS
INTEGER UNSIGNED NOT NULL default 0,
        MachineName				TEXT NOT NULL,
        WorkerState				SMALLINT UNSIGNED NOT NULL
default 0,	-- 0 = disconnected, 1 = connected NumWorkUnits
INTEGER UNSIGNED NOT NULL default 0,	-- how many work units this worker has
completed CurrentStage			TINYTEXT NOT NULL,
-- which compile stage is it on Thread0WU				INTEGER
NOT NULL default 0,		-- which WU thread 0 is on Thread1WU
INTEGER NOT NULL default 0,		-- which WU thread 1 is on Thread2WU
INTEGER NOT NULL default 0,		-- which WU thread 2 is on Thread3WU
INTEGER NOT NULL default 0		-- which WU thread 3 is on
        );

drop table text_messages;
create table text_messages (
        JobWorkerID				INTEGER UNSIGNED NOT NULL,
index id( JobWorkerID, MessageIndex ),
...
create table graph_entry (
        JobWorkerID				INTEGER UNSIGNED NOT NULL,
index id( JobWorkerID ), MSSinceJobStart			INTEGER UNSIGNED
NOT NULL, BytesSent				INTEGER UNSIGNED NOT NULL,
...
create table events (
        JobWorkerID				INTEGER UNSIGNED NOT NULL,
index id( JobWorkerID ), Text					TEXT NOT NULL
        );
*/
...
unsigned long g_JobPrimaryID =
    0;  // This represents this job, but doesn't link to a particular machine.
unsigned long g_JobWorkerID =
    0;  // A unique key in the DB that represents this machine in this job.
...
extern u64 g_ThreadWUs[4];

extern u64 VMPI_GetNumWorkUnitsCompleted(int iProc);
...
#define QUERYTEXT_GROWSIZE 1024
  ...
  while (1) {
    ...
    int ret =
        _vsnprintf(m_QueryText.Base(), m_QueryText.Count(), pFormat, marker);
    va_end(marker);
...
// ----------------------------------------------------------------------------------------------------
// // This inserts the necessary backslashes in front of backslashes or quote
// characters.
// ----------------------------------------------------------------------------------------------------
// //

char *FormatStringForSQL(const char *pText) {
  // First, count the quotes in the string. We need to put a backslash in front
  // of each one.
  int nChars = 0;
 ...
 public:
  virtual int RunCommand() {
    ...
    Q_snprintf(query, sizeof(query),
               "update job_master_start set NumWorkers=%d where JobID=%lu",
               nCurConnections, g_JobPrimaryID);
    ...
    for (int i = 1; i < nCurConnections; i++) {
      ...
      if (jobWorkerID != 0xFFFFFFFF) {
        Q_snprintf(query, sizeof(query),
                   "update "
                   "job_worker_start set WorkerState=%d, NumWorkUnits=%d where "
                   "JobWorkerID=%lu",
...
class CSQLDBCommand_JobMasterEnd : public CSQLDBCommandBase {
 public:
  virtual int RunCommand() {
    CMySQLQuery query;
    query.Format(
        "insert into job_master_end values ( %lu, %d, %d, \"no errors\" )",
        g_JobPrimaryID, g_nWorkersConnected, g_nWorkersDisconnected);
    query.Execute(g_pSQL);

    // Now set RunningTimeMS.
    unsigned long runningTimeMS = GetTickCount() - g_StatsStartTime;
    query.Format(
        "update job_master_start set RunningTimeMS=%lu where JobID=%lu",
        runningTimeMS, g_JobPrimaryID);
    query.Execute(g_pSQL);
    return 1;
  }
};
...
void UpdateJobWorkerRunningTime() {
  ...
  query.Format(
      ...
      "Thread0WU=%d, Thread1WU=%d, Thread2WU=%d, Thread3WU=%d where "
      "JobWorkerID=%lu",
      runningTimeMS, curStage, (int)g_ThreadWUs[0], (int)g_ThreadWUs[1],
      (int)g_ThreadWUs[2], (int)g_ThreadWUs[3], g_JobWorkerID);
  query.Execute(g_pSQL);
}

class CSQLDBCommand_GraphEntry : public CSQLDBCommandBase {
 public:
  CSQLDBCommand_GraphEntry(DWORD msTime, DWORD nBytesSent,
  ...
  virtual int RunCommand() {
    CMySQLQuery query;
    query.Format(
        "insert into graph_entry (JobWorkerID, MSSinceJobStart, BytesSent, "
        "BytesReceived) "
        "values ( %lu, %lu, %lu, %lu )",
        g_JobWorkerID, m_msTime, m_nBytesSent, m_nBytesReceived);
...
class CSQLDBCommand_TextMessage : public CSQLDBCommandBase {
 public:
  ...
  virtual int RunCommand() {
    CMySQLQuery query;
    query.Format(
        "insert into text_messages (JobWorkerID, MessageIndex, Text) values ( "
        "%lu, %lu, \"%s\" )",
...
void PerfThread_SendSpewText() {
  ...
  if (g_SpewText.Count() > 0) {
    ...
    if (g_bMPI_StatsTextOutput) {
      g_pDB->AddCommandToQueue(new CSQLDBCommand_TextMessage(g_SpewText.Base()),
                               NULL);
    } else {
      // Just show one message in the vmpi_job_watch window to let them know
      // that they need to use a command line option to get the output.
      static bool bFirst = true;
      if (bFirst) {
        char msg[512];
        V_snprintf(msg, sizeof(msg), "%s not enabled",
                   VMPI_GetParamString(mpi_Stats_TextOutput));
        bFirst = false;
...
void PerfThread_AddGraphEntry(DWORD startTicks, DWORD &lastSent,
                              DWORD &lastReceived) {
  ...
  g_pDB->AddCommandToQueue(new CSQLDBCommand_GraphEntry(
                               GetTickCount() - startTicks, curSent - lastSent,
...
DWORD WINAPI PerfThreadFn(LPVOID pParameter) {
  ...
  while (WaitForSingleObject(g_hPerfThreadExitEvent, 1000) != WAIT_OBJECT_0) {
    ...
    if (g_bMaster) {
      g_pDB->AddCommandToQueue(new CSQLDBCommand_WorkerStats, NULL);
    }
  }

  // Add the remaining text and one last graph entry (which will include the
  // current stage info).
  PerfThread_SendSpewText();
  PerfThread_AddGraphEntry(startTicks, lastSent, lastReceived);
...
bool LoadMySQLWrapper(const char *pHostName, const char *pDBName,
                      const char *pUserName) {
  UnloadMySQLWrapper();

  // Load the DLL and the interface.
  if (!Sys_LoadInterface("mysql_wrapper", MYSQL_WRAPPER_VERSION_NAME,
                         &g_hMySQLDLL, (void **)&g_pSQL))
    return false;
...
bool VMPI_Stats_Init_Master(const char *pHostName, const char *pDBName,
                            const char *pUserName, const char *pBSPFilename,
  ...
  if (!g_pDB || !g_pDB->Initialize() ||
      !LoadMySQLWrapper(pHostName, pDBName, pUserName)) {
    delete g_pDB;
    g_pDB = NULL;
    return false;
  }
  ...
  CMySQLQuery query;
  query.Format(
      "insert into job_master_start ( BSPFilename, StartTime, MachineName, "
      "RunningTimeMS ) values ( \"%s\", null, \"%s\", %lu )",
...
bool VMPI_Stats_Init_Worker(const char *pHostName, const char *pDBName,
                            const char *pUserName, unsigned long DBJobID) {
  g_StatsStartTime = GetTickCount();

  // If pDBServerName is null, then we're the master and we just want to make
  // the job_worker_start entry.
  if (pHostName) {
    Assert(!g_pDB);
  ...
  CMySQLQuery query;
  query.Format(
      "insert into job_worker_start ( JobID, CurrentStage, IsMaster, "
      "MachineName ) values ( %lu, \"none\", %d, \"%s\" )",
  ...
  g_hPerfThread = CreateThread(NULL, 0, PerfThreadFn, NULL, 0, &g_PerfThreadID);
...
void VMPI_Stats_Term() {
  ...
  while (GetTickCount() - startTime < 1000) {
    if (g_pDB->QueriesInOutQueue() == 0) break;
  }
...
static bool ReadStringFromFile(FILE *fp, char *pStr, int strSize) {
  int i = 0;
  for (i; i < strSize - 2; i++) {
    if (fread(&pStr[i], 1, 1, fp) != 1 || pStr[i] == '\n') {
      break;
    }
  }
...
void StatsDB_InitStatsDatabase(int argc, char **argv,
  ...
  if (g_bMPIMaster) {
    ...
    if (!VMPI_Stats_Init_Master(dbInfo.m_HostName, dbInfo.m_DBName,
                                dbInfo.m_UserName, argv[argc - 1],
                                &jobPrimaryID)) {
    ...
    char cmdLine[2048];
    Q_snprintf(cmdLine, sizeof(cmdLine), "vmpi_job_watch -JobID %d",
               jobPrimaryID);
