...
GameLumpHandle_t CGameLump::GetGameLumpHandle(GameLumpId_t id) {
  ...
  FOR_EACH_LL(m_GameLumps, i) {
    if (m_GameLumps[i].m_Id == id) return i;
  }
  ...
  switch (id) {
    ...
    case GAMELUMP_DETAIL_PROP_LIGHTING:
      ...
      g_Swap.SwapFieldsToTargetEndian((DetailPropLightstylesLump_t *)dest,
                                      (DetailPropLightstylesLump_t *)src,
                                      count);
      src += sizeof(DetailObjectDictLump_t) * count;
      dest += sizeof(DetailObjectDictLump_t) * count;
      break;
...
void CGameLump::ParseGameLump(dheader_t *pHeader) {
  ...
  if (length > 0) {
    ...
    for (int i = 0; i < pGameLumpHeader->lumpCount; ++i) {
      ...
      int file_length = pGameLump[i].filelen;
      GameLumpHandle_t lump =
          g_GameLumps.CreateGameLump(pGameLump[i].id, file_length,
                                     pGameLump[i].flags, pGameLump[i].version);
      if (g_bSwapOnLoad) {
        SwapGameLump(pGameLump[i].id, pGameLump[i].version,
                     (byte *)g_GameLumps.GetGameLump(lump),
                     (byte *)pHeader + pGameLump[i].fileofs, file_length);
      } else {
  ...
  do {
    ...
    if (pPhysModel->dataSize > 0) {
      ...
      for (int i = 0; i < pPhysModel->solidCount; ++i) {
        ...
        if (g_bSwapOnLoad) size = SwapLong(size);
        ...
        if (g_bSwapOnLoad) newsize = SwapLong(newsize);
...
int CopyLumpInternal(int fieldType, int lump, T *dest, int forceVersion) {
  ...
  ptrdiff_t length = g_pBSPHeader->lumps[lump].filelen;
  ptrdiff_t ofs = g_pBSPHeader->lumps[lump].fileofs;

  // count must be of the integral type
  ptrdiff_t count = length / sizeof(T);
...
bool GenerateNextLumpFileName(const char *bspfilename, char *lumpfilename,
  ...
  for (int i = 0; i < MAX_LUMPFILES; i++) {
    ...
    if (!g_pFileSystem->FileExists(lumpfilename)) return true;
  }
...
epair_t *ParseEpair(void) {
  ...
  if (strlen(token) >= MAX_KEY - 1) Error("ParseEpar: token too long");
  e->key = copystring(token);
  GetToken(false);
  if (strlen(token) >= MAX_VALUE - 1) Error("ParseEpar: token too long");
...
bool ParseEntity(void) {
  ...
  if (Q_stricmp(token, "{")) Error("ParseEntity: { not found");
  ...
  do {
    if (!GetToken(true)) Error("ParseEntity: EOF without closing brace");
...
static bool EnumerateNodesAlongRay_R(int node, Ray_t const &ray, float start,
  ...
  while (node >= 0) {
    ...
    if (front <= -TEST_EPSILON && back <= -TEST_EPSILON) {
      ...
      // Visit the node...
      if (!pEnum->EnumerateNode(node, ray, splitfrac, context)) return false;
...
bool SwapVHV(void *pDestBase, void *pSrcBase) {
  ...
  if (pHdr->m_nVersion != VHV_VERSION) return false;
...
int SwapLumpToDisk(int fieldType, int lumpnum) {
  if (g_pBSPHeader->lumps[lumpnum].filelen == 0) return 0;
  ...
  ptrdiff_t fieldSize =
      (fieldType == FIELD_VECTOR) ? sizeof(Vector) : sizeof(T);
  ptrdiff_t count = CopyLumpInternal<T>(fieldType, lumpnum, (T *)pBuffer,
  ...
  if (g_bSwapOnWrite) {
    ...
    switch (lumpnum) {
      ...
      default:
        g_Swap.SwapBufferToTargetEndian(
            (T *)pBuffer, (T *)pBuffer,
            g_pBSPHeader->lumps[lumpnum].filelen / sizeof(T));
        break;
    }
  }
...
bool CompressGameLump(dheader_t *pInBSPHeader, dheader_t *pOutBSPHeader,
  ...
  outputBuffer.Put(pInGameLump,
                   pInGameLumpHeader->lumpCount * sizeof(dgamelump_t));

  dgamelumpheader_t *pOutGameLumpHeader =
      (dgamelumpheader_t *)((byte *)outputBuffer.Base() + newOffset);
  ...
  byteSwap.SwapFieldsToTargetEndian(pOutGameLump,
                                    pOutGameLumpHeader->lumpCount);
...
bool CompressBSP(CUtlBuffer &inputBuffer, CUtlBuffer &outputBuffer,
  ...
  for (int i = 0; i < HEADER_LUMPS; ++i) {
    ...
    if (!pSortedLump->pLump->filelen) {
      ...
      *((unsigned int *)pOutBSPHeader->lumps[lumpNum].fourCC) = 0;
      ...
      CUtlBuffer bspBuffer;
      bspBuffer.SetExternalBuffer(
      ...
      if (lumpNum == LUMP_GAME_LUMP) {
        ...
        // compressed
        CompressGameLump(pInBSPHeader, pOutBSPHeader, outputBuffer,
                         pCompressFunc);
      } else if (lumpNum == LUMP_PAKFILE) {
        // add as is
        pOutBSPHeader->lumps[lumpNum].fileofs = newOffset;
        outputBuffer.Put(bspBuffer.Base(), bspBuffer.TellPut());
      } else {
        CUtlBuffer compressedBuffer;
        bool bCompressed = pCompressFunc(bspBuffer, compressedBuffer);
        if (bCompressed) {
          // placing the uncompressed size in the unused fourCC, will decode at
          // runtime
          *((unsigned int *)pOutBSPHeader->lumps[lumpNum].fourCC) =
              BigLong(bspBuffer.TellPut());
