...
static bool g_bReceivedDirectoryInfo = false;
...
void RecvQDirInfo() {
  while (!g_bReceivedDirectoryInfo) VMPI_DispatchNextMessage();
}
...
int VMPI_SendFileChunk(const void *pvChunkPrefix, int lenPrefix,
  ...
  hFile = CreateFileW(ptchFileName, GENERIC_READ, 0, NULL, OPEN_EXISTING,
...
void VMPI_HandleCrash(const char *pMessage, void *pvExceptionInfo,
  ...
  if (InterlockedIncrement(&crashHandlerCount) == 1) {
    ...
    VMPI_Send2Chunks(crashMsg, sizeof(crashMsg), pMessage, strlen(pMessage) + 1,
                     VMPI_MASTER_ID);
    ...
    if (pvExceptionInfo) {
      ...
      bool bSucceededWritingMinidump = WriteMiniDumpUsingExceptionInfo(
          ...
          // ( MINIDUMP_TYPE )( MiniDumpWithDataSegs | MiniDumpWithFullMemory |
...
#define ERR_RECORD(name) \
  ...
  for (i; (i < nErrors) && !pchReason; i++) {
    if (errors[i].code == uCode) pchReason = errors[i].pReason;
  }
