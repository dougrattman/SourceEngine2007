...
void RunThreadsOnIndividual(int workcnt, bool showpacifier,
                            ThreadWorkerFn func) {
...
void ThreadSetDefault(void) {
  ...
  if (numthreads == -1)  // not set manually
  {
    ...
    if (numthreads < 1 || numthreads > 32) numthreads = 1;
  }
...
void ThreadUnlock(void) {
  if (!threaded) return;
  if (!enter) Error("ThreadUnlock without lock\n");
  enter = 0;
...
void RunThreads_End() {
  ...
  for (int i = 0; i < numthreads; i++) CloseHandle(g_ThreadHandles[i]);
...
=============
*/
void RunThreadsOn(int workcnt, bool showpacifier, RunThreadsFn fn,
                  void *pUserData) {
  int start, end;
