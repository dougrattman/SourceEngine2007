...
//-----------------------------------------------------------------------------
// Purpose: Establishes connection to the database and sets up this object to
// handle db command Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CMySqlDatabase::Initialize() {
  // prepare critical sections
  //!! need to download SDK and replace these with
...
void CMySqlDatabase::RunThread() {
  ...
  while (m_bRunThread) {
    ...
    // check the size of the outqueue; if it's getting too big, sleep to let the
...
//-----------------------------------------------------------------------------
void CMySqlDatabase::AddCommandToQueue(ISQLDBCommand *cmd,
...
bool CMySqlDatabase::RunFrame() {
  ...
  while (m_OutQueue.Count() > 0) {
    ...
    // run result
    if (msg.replyTarget) {
      msg.replyTarget->SQLDBResponse(msg.cmd->GetID(), msg.returnState,
                                     msg.result, msg.cmd->GetReturnData());
...
//-----------------------------------------------------------------------------
// Purpose: load info - returns the number of sql db queries waiting to be
// processed
//-----------------------------------------------------------------------------
int CMySqlDatabase::QueriesInOutQueue() {
  // the queue names are from the DB point of view, not the server - thus the
  // reversal
