...
class CStripper {
 public:
  ...
  int CreateStrip(int tri, int vert, int maxlen, int *pswaps, bool flookahead,
                  bool fstartcw, int *pstriptris, int *pstripverts);
...
int CStripper::CreateStrip(int tri, int vert, int maxlen, int *pswaps,
                           bool flookahead, bool fstartcw, int *pstriptris,
                           int *pstripverts) {
  *pswaps = 0;

  // this guy has already been used?
  if (m_pused[tri]) return 0;
  ...
  for (stripcount = 3; stripcount < maxlen; stripcount++) {
    // dead end?
    if (nexttri == -1 || m_pused[nexttri]) break;
  ...
  for (int j = 2; j < stripcount; j++) m_pused[pstriptris[j]] = 0;
...
STRIPLIST::iterator FindBestCachedStrip(STRIPLIST *pstriplist,
  ...
  if (pstriplist->empty()) return pstriplist->end();
  ...
  for (STRIPLIST::iterator istriplist = pstriplist->begin();
       istriplist != pstriplist->end(); ++istriplist) {
    bool fFlip = false;
    ...
    if ((FIsStripCW(stripverts) == fstartcw) &&
      ...
      int numcachehits =
          vertcachenew.NumCacheHits() - vertcachestate.NumCacheHits();
...
int CStripper::CreateManyStrips(STRIPLIST *pstriplist, WORD **ppstripindices) {
  ...
  for (istriplist = pstriplist->begin(); istriplist != pstriplist->end();
       ++istriplist) {
  ...
  for (istriplist = pstriplist->begin(); !pstriplist->empty();
...
//=========================================================================
void CStripper::BuildStrips(STRIPLIST *pstriplist, int maxlen,
                            bool flookahead) {
  ...
  for (;;) {
    ...
    for (tri = 0; tri < m_numtris; tri++) {
      ...
      if (!curneightborcount) curneightborcount = 4;

      // if this guy has more neighbors than the current best - bail
      if (curneightborcount > bestneighborcount) continue;

      // try starting the strip with each of this tris verts
      for (int vert = 0; vert < 3; vert++) {
        int swaps;
        int len = CreateStrip(tri, vert, maxlen, &swaps, flookahead, fstartcw,
                              pstriptris, pstripverts);
        assert(len);
    ...
    // no strips found?
    if (bestneighborcount == INT_MAX) break;

    // recreate this strip
    int swaps;
    int len = CreateStrip(besttri, bestvert, maxlen, &swaps, flookahead,
                          fstartcw, pstriptris, pstripverts);
    assert(len);

    // mark the tris on the best strip as used
    for (tri = 0; tri < len; tri++) m_pused[pstriptris[tri]] = 1;
    ...
    if ((len & 0x1)) fstartcw = !fstartcw;
  }

#ifdef _DEBUG
  // make sure all tris are used
  for (int t = 0; t < m_numtris; t++) assert(m_pused[t]);
#endif
}
...
CStripper::CStripper(int numtris, TRIANGLELIST ptriangles) {
  ...
  for (itri = 0; itri < numtris; itri++) {
    for (int ivert = 0; ivert < 3; ivert++) {
      if (!(m_pused[itri] & (1 << ivert))) InitTriangleInfo(itri, ivert);
    }
  }
...
void EnableLeakChecking() {
  ...
  flCrtDbgFlags &= ~(_CRTDBG_LEAK_CHECK_DF | _CRTDBG_CHECK_ALWAYS_DF |
...
//=========================================================================
int Stripify(int numtris, WORD *ptriangles, int *pnumindices,
             WORD **ppstripindices) {
  if (!numtris || !ptriangles) return 0;
...
//    EnableLeakChecking();
  ...
  if (pnumindices) *pnumindices = numindices;
  return numindices;
}
...
void ComputeVertexPermutation(int numstripindices, WORD *pstripindices,
  ...
  for (i = 0; i < numstripindices; i++) {
    ...
    if (pSortTable[index].iFirstUsed == -1) pSortTable[index].iFirstUsed = i;
  }
