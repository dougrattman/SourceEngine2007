...
void FreeWinding(winding_t *w) {
  if (w->numpoints == 0xdeaddead) Error("FreeWinding: freed a freed winding");
...
void RemoveColinearPoints(winding_t *w) {
  ...
  if (numthreads == 1) c_removed += w->numpoints - nump;
  w->numpoints = nump;
...
void ClassifyWindingEpsilon_Offset(winding_t *in, const Vector &normal,
                                   ...
                                   const Vector &offset) {
  TranslateWinding(in, offset);
  ClassifyWindingEpsilon(in, normal, dist + DotProduct(offset, normal), epsilon,
                         front, back, on);
  TranslateWinding(in, -offset);
  if (front && *front) {
    TranslateWinding(*front, -offset);
...
void ChopWindingInPlace(winding_t **inout, const Vector &normal, f32 dist,
  ...
  if (f->numpoints > maxpts) Error("ClipWinding: points exceeded estimate");
  if (f->numpoints > MAX_POINTS_ON_WINDING)
    Error("ClipWinding: MAX_POINTS_ON_WINDING");
...
void CheckWinding(winding_t *w) {
  ...
  area = WindingArea(w);
  if (area < 1) Error("CheckWinding: %f area", area);
  ...
  for (i = 0; i < w->numpoints; i++) {
    ...
    if (VectorLength(dir) < ON_EPSILON) Error("CheckWinding: degenerate edge");
    ...
    for (j = 0; j < w->numpoints; j++) {
      ...
      if (d > edgedist) Error("CheckWinding: non-convex");
    }
  }
}
...
bool PointInWinding(const Vector &pt, winding_t *pWinding) {
  ...
  for (int ndxPt = 1; ndxPt < pWinding->numpoints; ndxPt++) {
    ...
    if (cross.Dot(testCross) < 0.0f) return false;
  }
