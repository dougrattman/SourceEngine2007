...
void LoadMaterialSystemInterface(CreateInterfaceFn fileSystemFactory) {
  ...
  if (clientFactory) {
    g_pMaterialSystem = (IMaterialSystem *)clientFactory(
...
MaterialSystemMaterial_t FindMaterial(const char *materialName, bool *pFound,
                                      bool bComplain) {
  IMaterial *pMat = g_pMaterialSystem->FindMaterial(
      materialName, TEXTURE_GROUP_OTHER, bComplain);
  MaterialSystemMaterial_t matHandle = pMat;

  if (pFound) {
    *pFound = true;
    if (IsErrorMaterial(pMat)) *pFound = false;
  }
...
void GetMaterialDimensions(MaterialSystemMaterial_t materialHandle, int *width,
  ...
  bool translucent;
  retVal = material->GetPreviewImageProperties(width, height, &dummyImageFormat,
                                               &translucent);
...
int GetMaterialShaderPropertyBool(MaterialSystemMaterial_t materialHandle,
  ...
  switch (propID) {
    case UTILMATLIB_NEEDS_BUMPED_LIGHTMAPS:
      return material->GetPropertyFlag(
...
int GetMaterialShaderPropertyInt(MaterialSystemMaterial_t materialHandle,
  ...
  switch (propID) {
    case UTILMATLIB_OPACITY:
      if (material->IsTranslucent()) return UTILMATLIB_TRANSLUCENT;
      if (material->IsAlphaTested()) return UTILMATLIB_ALPHATEST;
