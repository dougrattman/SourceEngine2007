...
// If pathMode is SCRIPT_USE_ABSOLUTE_PATH, then it uses ExpandPath() on the
// filename before trying to open it. Otherwise, it passes the filename straight
// into the filesystem (so you can leave it as a relative path).
...
//	pIncludedFromFileName	is the name of the parent file or NULL if
// loaded because of "LoadScriptFile" toplevel call. 	nIncludeLineNumber
// is the number of the line in the parent file with $include statement or 0 in
// case of "LoadScriptFile"
typedef void (*SCRIPT_LOADED_CALLBACK)(char const *pFilenameLoaded,
...
class IScriptLib {
 public:
  ...
  virtual int CompareFileTime(const char *pFilenameA,
                              const char *pFilenameB) = 0;
