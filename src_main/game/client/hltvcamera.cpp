...
ConVar spec_autodirector(
    "spec_autodirector", "1", FCVAR_CLIENTDLL | FCVAR_CLIENTCMD_CAN_EXECUTE,
...
void C_HLTVCamera::CalcChaseCamView(Vector& eyeOrigin, QAngle& eyeAngles,
                                    float& fov) {
  bool bManual =
      !spec_autodirector.GetBool();  // chase camera controlled manually
  ...
  if (target1->IsAlive() && target1->IsDormant()) return;
  ...
  UTIL_TraceHull(targetOrigin1, cameraOrigin, WALL_MIN, WALL_MAX, MASK_SOLID,
                 target1, COLLISION_GROUP_NONE, &trace);
  ...
  if (target2) {
    ...
    cameraAngles.x = std::clamp(cameraAngles.x, -60.0f, 60.0f);
...
void C_HLTVCamera::CalcInEyeCamView(Vector& eyeOrigin, QAngle& eyeAngles,
                                    float& fov) {
...
void C_HLTVCamera::CalcRoamingView(Vector& eyeOrigin, QAngle& eyeAngles,
                                   float& fov) {
  ...
  if (!IsPVSLocked()) {
    ...
    AngleVectors(m_LastCmd.viewangles, &forward, &right,
...
void C_HLTVCamera::CalcFixedView(Vector& eyeOrigin, QAngle& eyeAngles,
                                 float& fov) {
  eyeOrigin = m_vCamOrigin;
  eyeAngles = m_aCamAngle;
  fov = m_flFOV;
...
void C_HLTVCamera::FireGameEvent(IGameEvent* event) {
  if (!engine->IsHLTV()) return;  // not in HLTV mode
  ...
  if (Q_strcmp("hltv_title", type) == 0) {
    Q_strncpy(m_szTitleText, event->GetString("text", ""),
              sizeof(m_szTitleText));
    return;
  }
  ...
  if (Q_strcmp("hltv_chase", type) == 0) {
    ...
    bool bIsInChaseMode =
        (m_nCameraMode == OBS_MODE_IN_EYE) || (m_nCameraMode == OBS_MODE_CHASE);
...
void C_HLTVCamera::SmoothCameraAngle(QAngle& targetAngle) {
  if (m_flLastAngleUpdateTime > 0) {
    ...
    deltaTime = std::clamp(deltaTime * m_flInertia, 0.01f, 1.0f);
