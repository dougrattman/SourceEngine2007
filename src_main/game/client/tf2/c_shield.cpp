...
//
// Purpose: Client's shield entity
...
#define SHIELD_DAMAGE_CHANGE_TRANSITION_START_TIME \
  (SHIELD_DAMAGE_CHANGE_TIME - SHIELD_DAMAGE_CHANGE_TRANSITION_TIME)
#define SHIELD_DAMAGE_CHANGE_TOTAL_TIME         \
  (SHIELD_DAMAGE_CHANGE_TRANSITION_START_TIME + \
   SHIELD_DAMAGE_CHANGE_TRANSITION_TIME)
...
bool C_Shield::ShouldCollide(int collisionGroup, int contentsMask) const {
  return m_bCollisionsActive && ((collisionGroup == TFCOLLISION_GROUP_WEAPON) ||
                                 (collisionGroup == TFCOLLISION_GROUP_GRENADE));
}
...
//-----------------------------------------------------------------------------
bool C_Shield::TestCollision(const Ray_t& ray, unsigned int mask,
                             trace_t& trace) {
  // Can't block anything if we're EMPed, or we've got no power left to block
  if (m_bIsEMPed) return false;
  if (m_flPowerLevel <= 0) return false;
  ...
  for (int i = 0; i < h - 1; ++i) {
    for (int j = 0; j < w - 1; ++j) {
      ...
      t = IntersectRayWithTriangle(ray, GetPoint(j, i + 1),
                                   GetPoint(j + 1, i + 1), GetPoint(j, i),
                                   true);
      if ((t >= 0.0f) && (t < mint)) {
        mint = t;
        v1[0] = j;
      ...
      t = IntersectRayWithTriangle(ray, GetPoint(j + 1, i), GetPoint(j, i),
  ...
  float u, v;
  bool ok =
      ComputeIntersectionBarycentricCoordinates(ray, v1vec, v2vec, v3vec, u, v);
  Assert(ok);
  if (!ok) {
    return false;
  }
  ...
  UTIL_SetTrace(trace, ray, this, mint, hitgroup, CONTENTS_SOLID, normal,
                intercept);
  return true;
}
...
//-----------------------------------------------------------------------------
void C_Shield::RegisterDeflection(const Vector& vecDir, int bitsDamageType,
                                  trace_t* ptr) {
  Vector normalDir;
...
// Purpose:
//-----------------------------------------------------------------------------
void C_Shield::CreateShieldDeflection(int hitgroup, const Vector& dir,
                                      bool partialBlock) {
...
void C_Shield::DrawShieldPoints(Vector* pt, Vector* normal, float* opacity) {
  ...
  for (int i = 0; i < m_SubdivisionCount - 1; ++i) {
    ...
    for (int j = 0; j < m_SubdivisionCount - 1; ++j) {
      ...
      meshBuilder.Color4ub(color[0], color[1], color[2],
                           opacity[idx + m_SubdivisionCount]);
      ...
      meshBuilder.Color4ub(color[0], color[1], color[2],
                           opacity[idx + m_SubdivisionCount + 1]);
...
void C_Shield::DrawShieldDecals(Vector* pt, bool hitDecals) {
  ...
  for (int r = m_Decals.Size(); --r >= 0;) {
    ...
    if (!hitDecals)
      ...
      materials->Bind(passDecal ? m_pPassDecal2 : m_pHitDecal,
    ...
    if (i0 < 0) i0 = 0;
    if (i1 >= m_SubdivisionCount) i1 = m_SubdivisionCount - 1;
    if (j0 < 0) j0 = 0;
    if (j1 >= m_SubdivisionCount) j1 = m_SubdivisionCount - 1;
...
//-----------------------------------------------------------------------------
void C_Shield::ComputePoint(float s, float t, Vector& pt, Vector& normal,
                            float& opacity) {
  ...
  if (m_SplinePatch.m_fs == 0.0f)
    active =
        active || IsPanelActive(m_SplinePatch.m_is - 1, m_SplinePatch.m_it);
  if (m_SplinePatch.m_ft == 0.0f)
    active =
        active || IsPanelActive(m_SplinePatch.m_is, m_SplinePatch.m_it - 1);

  if (!active) {
    // If the panel's not active, it's transparent.
    opacity = 0.0f;
  } else {
    if ((s == 0.0f) || (t == 0.0f) || (s == (Width() - 1.0f)) ||
        (t == (Height() - 1.0f))) {
      ...
      if (m_ShieldOwnedByLocalPlayer) {
        ...
        if (dist != 0.0f) {
          ...
          if (angle < fov * .2f)
            ...
            angle -= fov * 0.2f;
            blendValue =
                1.0f - 0.9f * 0.5f * (cos(M_PI * angle / (fov * 0.2f)) + 1.0f);
          }
        }

        // Interpolate between 1 and the blend value based on the blend
        // factor...
    ...
    for (int i = 0; i < m_SubdivisionCount; ++i) {
      ...
      for (int j = 0; j < m_SubdivisionCount; ++j) {
        ...
        if (dr < m_Ripples[r].m_Radius) {
          ...
          float diff =
              amplitude * cos(0.5f * M_PI * dr / m_Ripples[r].m_Radius);
          ...
          if (impactopacity > opacity[idx]) opacity[idx] = impactopacity;
        }
      }
    }

    if (amplitude < 0.1) m_Ripples.Remove(r);
  }
}
...
int C_Shield::DrawModel(int flags) {
  ...
  if (m_FadeValue == 0.0f) return 1;

  // If I have no power, don't draw
  if (m_flPowerLevel <= 0) return 1;
  ...
  Vector const** pControlPoints = stack_alloc<Vector const*>(count);
  float* pControlOpacity = stack_alloc<float>(count);
  float* pControlBlend = stack_alloc<float>(count);
  ...
  Vector* pt = stack_alloc<Vector>(numSubdivisions);
  Vector* normal = stack_alloc<Vector>(numSubdivisions);
  float* opacity = stack_alloc<float>(numSubdivisions);
...
EXPOSE_INTERFACE(CShieldPowerLevelProxy, IMaterialProxy,
                 "ShieldPowerLevel" IMATERIAL_PROXY_INTERFACE_VERSION);
