...
class CHudDeathNotice : public CHudElement, public vgui::Panel {
 ...
 private:
  ...
  CPanelAnimationVar(Color, m_clrFriendlyText, "FriendlyTextColor",
                     "FriendlyTextColor");
  CPanelAnimationVar(Color, m_clrEnemyText, "EnemyTextColor", "EnemyTextColor");
  CPanelAnimationVar(Color, m_clrMyKillsText, "MyKillsTextColor",
                     "MyKillsTextColor");
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudDeathNotice::SetColorForNoticePlayer(C_BasePlayer *pPlayer,
                                              int iTeamNumber) {
  // Set the right color
...
void CHudDeathNotice::Paint() {
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    g_pVGuiLocalize->ConvertANSIToUnicode(m_DeathNotices[i].Assist.szName,
                                          assist, sizeof(assist));

    // Get the team numbers for the players involved
    C_BasePlayer *pKiller = static_cast<C_BasePlayer *>(
        cl_entitylist->GetEnt(m_DeathNotices[i].Killer.iEntIndex));
    int iKillerTeam = pKiller ? pKiller->GetTeamNumber() : 0;
    C_BasePlayer *pAssist = NULL;
    if (m_DeathNotices[i].Assist.iEntIndex) {
      pAssist = static_cast<C_BasePlayer *>(
          cl_entitylist->GetEnt(m_DeathNotices[i].Assist.iEntIndex));
    }
    C_BasePlayer *pVictim = static_cast<C_BasePlayer *>(
        cl_entitylist->GetEnt(m_DeathNotices[i].Victim.iEntIndex));
...
void CHudDeathNotice::MsgFunc_DeathMsg(bf_read &msg) {
  ...
  if (deathMsg.iSuicide) {
    ...
    Q_snprintf(sDeathMsg, sizeof(sDeathMsg), "%s teamkilled %s",
               deathMsg.Killer.szName, deathMsg.Victim.szName);
  } else {
    if (assist) {
      Q_snprintf(sDeathMsg, sizeof(sDeathMsg), "%s and %s killed %s",
                 deathMsg.Killer.szName, deathMsg.Assist.szName,
                 deathMsg.Victim.szName);
    } else {
  ...
  if (fullkilledwith && *fullkilledwith && (*fullkilledwith > 13) &&
      strcmp(fullkilledwith, "d_world") && !deathMsg.iTeamKill) {
