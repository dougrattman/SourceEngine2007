...
ConVar overview_health("overview_health", "1",
                       FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
                       "Show player's health in map overview.\n");
ConVar overview_names("overview_names", "1",
                      FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
                      "Show player's names in map overview.\n");
ConVar overview_tracks("overview_tracks", "1",
                       FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
                       "Show player's tracks in map overview.\n");
ConVar overview_locked("overview_locked", "1",
                       FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
                       "Locks map angle, doesn't follow view angle.\n");
ConVar overview_alpha("overview_alpha", "1.0",
                      FCVAR_ARCHIVE | FCVAR_CLIENTCMD_CAN_EXECUTE,
                      "Overview map translucency.\n");

IMapOverviewPanel *g_pMapOverview =
    NULL;  // we assume only one overview is created

CON_COMMAND(overview_zoom, "Sets overview map zoom: <zoom> [<time>] [rel]") {
  if (!g_pMapOverview || args.ArgC() < 2) return;

  float zoom = Q_atof(args[1]);

  float time = 0;

  if (args.ArgC() >= 3) time = Q_atof(args[2]);

  if (args.ArgC() == 4) zoom *= g_pMapOverview->GetZoom();

  // We are going to store their zoom pick as the resultant overview size that
  // it sees.  This way, the value will remain correct even on a different map
  // that has a different intrinsic zoom.
  float desiredViewSize = 0.0f;
  desiredViewSize = (zoom * OVERVIEW_MAP_SIZE * g_pMapOverview->GetFullZoom()) /
                    g_pMapOverview->GetMapScale();
  ...
  g_pClientMode->GetViewportAnimationController()->RunAnimationCommand(
      g_pMapOverview->GetAsPanel(), "zoom", zoom, 0.0, time,
...
CON_COMMAND(overview_mode, "Sets overview map mode off,small,large: <0|1|2>") {
  ...
  if (args.ArgC() < 2) {
    ...
    if (mode > CMapOverview::MAP_MODE_FULL) mode = CMapOverview::MAP_MODE_OFF;
  } else {
    // set specific mode
    mode = Q_atoi(args[1]);
  }
...
void CMapOverview::UpdatePlayerTrails() {
  if (m_fNextTrailUpdate > m_fWorldTime) return;
...
void CMapOverview::UpdateFollowEntity() {
  if (m_nFollowEntity != 0) {
    ...
    if (ent) {
      Vector position = MainViewOrigin();  // Use MainViewOrigin so SourceTV
                                           // works in 3rd person
...
void CMapOverview::SetPlayerPositions(int index, const Vector &position,
                                      const QAngle &angle) {
...
void CMapOverview::Update() {
  ...
  m_bFollowAngle =
      (GetMode() != MAP_MODE_RADAR && !overview_locked.GetBool()) ||
      (GetMode() == MAP_MODE_RADAR && !IsRadarLocked());
  m_fTrailUpdateInterval =
      overview_tracks.GetInt() && (GetMode() != MAP_MODE_RADAR);
...
void CMapOverview::DrawMapPlayerTrails() {
  if (m_fTrailUpdateInterval <= 0) return;  // turned off
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    for (int i = 0; i < (MAX_TRAIL_LENGTH - 1); i++) {
      if (player->trail[i + 1].x == 0 && player->trail[i + 1].y == 0) break;
      ...
      if (dist.LengthSqr() < (128 * 128)) {
        surface()->DrawSetColor(player->color[0], player->color[1],
...
bool CMapOverview::DrawIcon(MapObject_t *obj) {
  ...
  if (status >= 0.0f && status <= 1.0f && statusColor) {
    ...
    surface()->DrawFilledRect(pospanel.x - d, pospanel.y - 1, pospanel.x + d,
                              pospanel.y + 1);

    int length = (float)(d * 2) * status;
    surface()->DrawSetColor(statusColor->r(), statusColor->g(),
                            statusColor->b(), 255);
    surface()->DrawFilledRect(pospanel.x - d, pospanel.y - 1,
                              pospanel.x - d + length, pospanel.y + 1);
  ...
  if (text && textColor) {
    ...
    // draw name in color
    surface()->DrawSetTextColor(textColor->r(), textColor->g(), textColor->b(),
                                255);
...
int CMapOverview::GetPixelOffset(float height) {
  ...
  return (int)sqrt(
      (float)(a * a +
              b * b));  // number of panel pixels for "scale" units in world
}
...
void CMapOverview::SetMap(const char *levelname) {
  ...
  m_fNextTrailUpdate = 0;  // Set to 0 for immediate update.  Our WorldTime var
                           // hasn't been updated to 0 for the new map yet
  m_fWorldTime = 0;  // In release, we occasionally race and get this bug again
                     // if we gt a paint before an update.  Reset this before
                     // the old value gets in to the timer.
  ...
  Q_snprintf(tempfile, sizeof(tempfile), "resource/overviews/%s.txt",
             levelname);
  ...
  surface()->DrawSetTextureFile(
      m_nMapTextureID, m_MapKeyValues->GetString("material"), true, false);
...
void CMapOverview::DrawCamera() {
  ...
  surface()->DrawFilledRect(center.x - 2, center.y - 2, center.x + 2,
                            center.y + 2);
}
...
void CMapOverview::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp(type, "game_newmap") == 0) {
    ...
    Q_strncpy(player->name, event->GetString("name", "unknown"),
              sizeof(player->name));
...
void CMapOverview::UpdateSizeAndPosition() {
  if (g_pSpectatorGUI && g_pSpectatorGUI->IsVisible()) {
    ...
    SetBounds(x, y, w, std::min(h, iScreenTall));
  }
}
...
void CMapOverview::SetCenter(const Vector2D &mappos) {
  ...
  if (GetMode() != MAP_MODE_RADAR) {
    if (m_MapCenter.x < width) m_MapCenter.x = width;
...
CMapOverview::MapObject_t *CMapOverview::FindObjectByID(int objectID) {
  for (int i = 0; i < m_Objects.Count(); i++) {
    if (m_Objects[i].objectID == objectID) return &m_Objects[i];
  }
...
void CMapOverview::SetObjectPosition(int objectID, const Vector &position,
                                     const QAngle &angle) {
  MapObject_t *obj = FindObjectByID(objectID);
