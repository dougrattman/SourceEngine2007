...
class IMapOverviewPanel {
 public:
  ...
  virtual void SetPlayerPositions(int index, const Vector &position,
                                  const QAngle &angle) = 0;
...
typedef bool (*FnCustomMapOverviewObjectPaint)(int textureID, Vector pos,
                                               float scale, float angle,
                                               const char *text,
                                               Color *textColor, float status,
                                               Color *statusColor);

class CMapOverview : public CHudElement,
                     public vgui::Panel,
                     public IMapOverviewPanel {
 ...
 protected:                  // private structures & types
  float GetViewAngle(void);  // The angle that determines the viewport twist
                             // from map texture to panel drawing.
  ...
  typedef struct MapPlayer_s {
    ...
    Vector2D
        trail[MAX_TRAIL_LENGTH];  // save 1 footstep each second for 1 minute
  } MapPlayer_t;
...
#define MAP_OBJECT_ALIGN_TO_MAP (1 << 0)
 ...
 public:
  ...
  virtual int AddObject(
      const char *icon, int entity,
      float timeToLive);  // returns object ID, 0 = no entity, -1 = forever
  virtual void SetObjectIcon(int objectID, const char *icon,
                             float size);  // icon world size
  virtual void SetObjectText(int objectID, const char *text,
                             Color color);  // text under icon
  virtual void SetObjectStatus(int objectID, float value,
                               Color statusColor);  // status bar under icon
  virtual void SetObjectPosition(int objectID, const Vector &position,
                                 const QAngle &angle);  // world pos/angles
 ...
 protected:
  ...
  bool m_bRotateMap;   // if true roatate map around 90 degress, so it fits
                       // better to 4:3 screen ratio

  int m_nFollowEntity;  // entity number to follow, 0 = off
  CPanelAnimationVar(
      float, m_fZoom, "zoom",
      "1.0");  // current zoom n = overview panel shows 1/n^2 of whole map'
  float m_fFullZoom;      // best zoom factor for full map view (1.0 is map is a
                          // square)
  Vector2D m_ViewOrigin;  // map coordinates that are in the center of the
                          // pverview panel
  Vector2D m_MapCenter;   // map coordinates that are in the center of the
                          // pverview panel
