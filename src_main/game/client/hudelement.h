...
class CHudElement : public CGameEventListener {
 public:
  ...
  // called whenever the video mode changes, and whenever Init() would be
  // called, so the hud can vid init itself
  ...
  // called whenever the hud receives "reset" message, which is (usually) every
  // time the client respawns after getting killed
  ...
  // For now, CHUdElements declare a single priority value. They will only be
  // hidden by panels with a lower priority and will only lock out panels with a
  // lower priority
...
inline bool RenderGroupLessFunc(CHudElement *const &lhs,
                                CHudElement *const &rhs) {
