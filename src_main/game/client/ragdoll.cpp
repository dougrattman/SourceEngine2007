...
BEGIN_SIMPLE_DATADESC(CRagdoll)
  ...
  DEFINE_AUTO_ARRAY(m_ragdoll.boneIndex, FIELD_INTEGER),
      ...
      DEFINE_RAGDOLL_ELEMENT(0), DEFINE_RAGDOLL_ELEMENT(1),
      DEFINE_RAGDOLL_ELEMENT(2), DEFINE_RAGDOLL_ELEMENT(3),
      DEFINE_RAGDOLL_ELEMENT(4), DEFINE_RAGDOLL_ELEMENT(5),
      DEFINE_RAGDOLL_ELEMENT(6), DEFINE_RAGDOLL_ELEMENT(7),
      DEFINE_RAGDOLL_ELEMENT(8), DEFINE_RAGDOLL_ELEMENT(9),
      DEFINE_RAGDOLL_ELEMENT(10), DEFINE_RAGDOLL_ELEMENT(11),
      DEFINE_RAGDOLL_ELEMENT(12), DEFINE_RAGDOLL_ELEMENT(13),
      DEFINE_RAGDOLL_ELEMENT(14), DEFINE_RAGDOLL_ELEMENT(15),
      DEFINE_RAGDOLL_ELEMENT(16), DEFINE_RAGDOLL_ELEMENT(17),
      DEFINE_RAGDOLL_ELEMENT(18), DEFINE_RAGDOLL_ELEMENT(19),
      DEFINE_RAGDOLL_ELEMENT(20), DEFINE_RAGDOLL_ELEMENT(21),
      DEFINE_RAGDOLL_ELEMENT(22), DEFINE_RAGDOLL_ELEMENT(23),
...
void CRagdoll::Init(C_BaseEntity *ent, CStudioHdr *pstudiohdr,
                    const Vector &forceVector, int forceBone,
                    const matrix3x4_t *pDeltaBones0,
                    const matrix3x4_t *pDeltaBones1,
                    const matrix3x4_t *pCurrentBonePosition, float dt) {
  ...
  memcpy(m_savedBone1, &pDeltaBones0[0],
         sizeof(matrix3x4_t) * pstudiohdr->numbones());
  memcpy(m_savedBone2, &pDeltaBones1[0],
         sizeof(matrix3x4_t) * pstudiohdr->numbones());
  memcpy(m_savedBone3, &pCurrentBonePosition[0],
         sizeof(matrix3x4_t) * pstudiohdr->numbones());
#endif
}
...
void CRagdoll::RagdollBone(C_BaseEntity *ent, mstudiobone_t *pbones,
                           int boneCount, bool *boneSimulated,
...
void CRagdoll::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  if (m_lastUpdate == gpGlobals->curtime) return;
  ...
  if (m_allAsleep) {
    ...
    if (m_ragdoll.pGroup->IsInErrorState()) {
      C_BaseEntity *pEntity =
          static_cast<C_BaseEntity *>(m_ragdoll.list[0].pObject->GetGameData());
...
static ConVar ragdoll_sleepaftertime(
    "ragdoll_sleepaftertime", "5.0f", 0,
    "After this many seconds of being basically stationary, the ragdoll will "
    "go to sleep.");
...
void CRagdoll::DrawWireframe() {
  IMaterial *pWireframe = materials->FindMaterial(
      "shadertest/wireframevertexcolor", TEXTURE_GROUP_OTHER);
  ...
  for (i = 0; i < m_ragdoll.listCount; i++) {
    ...
    engine->DebugDrawPhysCollide(
        pCollide, pWireframe, m_savedBone1[m_ragdoll.boneIndex[i]], debugColor);
  }
  for (i = 0; i < m_ragdoll.listCount; i++) {
    ...
    const CPhysCollide *pCollide = m_ragdoll.list[i].pObject->GetCollide();
    engine->DebugDrawPhysCollide(
        pCollide, pWireframe, m_savedBone2[m_ragdoll.boneIndex[i]], debugColor);
  }
  ...
  for (i = 0; i < m_ragdoll.listCount; i++) {
    ...
    const CPhysCollide *pCollide = m_ragdoll.list[i].pObject->GetCollide();
    engine->DebugDrawPhysCollide(
        pCollide, pWireframe, m_savedBone3[m_ragdoll.boneIndex[i]], debugColor);
  }
#endif
}

CRagdoll *CreateRagdoll(C_BaseEntity *ent, CStudioHdr *pstudiohdr,
                        const Vector &forceVector, int forceBone,
                        const matrix3x4_t *pDeltaBones0,
                        const matrix3x4_t *pDeltaBones1,
                        const matrix3x4_t *pCurrentBonePosition, float dt) {
  CRagdoll *pRagdoll = new CRagdoll;
  pRagdoll->Init(ent, pstudiohdr, forceVector, forceBone, pDeltaBones0,
                 pDeltaBones1, pCurrentBonePosition, dt);
...
class C_ServerRagdoll : public C_BaseAnimating {
 public:
  ...
  virtual void AccumulateLayers(CStudioHdr *hdr, Vector pos[], Quaternion q[],
                                float poseparam[], float currentTime,
                                int boneMask);
...
int C_ServerRagdoll::InternalDrawModel(int flags) {
  ...
  if (vcollide_wireframe.GetBool()) {
    ...
    for (int i = 0; i < m_elementCount; i++) {
      ...
      engine->DebugDrawPhysCollide(pCollide->solids[i], pWireframe, matrix,
                                   debugColor);
    }
  }
  return ret;
...
CStudioHdr *C_ServerRagdoll::OnNewModel(void) {
  ...
  if (!m_elementCount) {
    ...
    if (!pCollide) {
      const char *pszName =
          modelinfo->GetModelName(modelinfo->GetModel(GetModelIndex()));
...
//-----------------------------------------------------------------------------
void C_ServerRagdoll::SetupWeights(const matrix3x4_t *pBoneToWorld,
                                   int nFlexWeightCount, float *pFlexWeights,
                                   float *pFlexDelayedWeights) {
...
void C_ServerRagdoll::AddEntity(void) {
  ...
  m_flBlendWeightCurrent = Approach(m_flBlendWeight, m_flBlendWeightCurrent,
                                    gpGlobals->frametime * 5.0f);
}

void C_ServerRagdoll::AccumulateLayers(CStudioHdr *hdr, Vector pos[],
  ...
  if (m_nOverlaySequence >= 0 && m_nOverlaySequence < hdr->GetNumSeq()) {
    AccumulatePose(hdr, m_pIk, pos, q, m_nOverlaySequence, GetCycle(),
                   poseparam, boneMask, m_flBlendWeightCurrent, currentTime);
  }
}

void C_ServerRagdoll::BuildTransformations(CStudioHdr *hdr, Vector *pos,
                                           Quaternion q[],
                                           const matrix3x4_t &cameraTransform,
                                           int boneMask,
  ...
  for (i = 0; i < hdr->numbones(); i++) {
    ...
    if (boneSimulated[i] || CalcProceduralBone(hdr, i, m_BoneAccessor)) {
      ...
      if (pbones[i].parent == -1) {
        ...
        ConcatTransforms(GetBone(pbones[i].parent), bonematrix,
                         GetBoneForWrite(i));
      }
    }
...
IPhysicsObject *C_ServerRagdoll::GetElement(int elementNum) { return NULL; }
...
void C_ServerRagdoll::UpdateOnRemove() {
  ...
  if (NULL != anim && anim->GetModel() && (anim->GetModel() == GetModel())) {
    // Need to tell C_BaseAnimating to blend out of the ragdoll data that we
    // received last
  ...
  bool SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones, int boneMask,
    ...
    if (GetMoveParent()) {
      ...
      if (!(boneFlags & boneMask)) {
        // BUGBUG: The attached bone is required and this call is going to skip
        // it, so force it HACKHACK: Assume the highest bit numbered bone flag
        // is the minimum bone set
        boneMask |= GetHighestBit(boneFlags);
      }
    }
    return BaseClass::SetupBones(pBoneToWorldOut, nMaxBones, boneMask,
                                 currentTime);
  }

  virtual void BuildTransformations(CStudioHdr *hdr, Vector *pos,
                                    Quaternion q[],
                                    const matrix3x4_t &cameraTransform,
                                    int boneMask, CBoneBitList &boneComputed) {
    VPROF_BUDGET("C_ServerRagdollAttached::SetupBones",
                 VPROF_BUDGETGROUP_CLIENT_ANIMATION);

    if (!hdr) return;

    float frac = RemapVal(gpGlobals->curtime, m_parentTime,
                          m_parentTime + ATTACH_INTERP_TIME, 0, 1);
    ...
    BaseClass::BuildTransformations(hdr, pos, q, cameraTransform, boneMask,
                                    boneComputed);
...
IMPLEMENT_CLIENTCLASS_DT(C_ServerRagdollAttached, DT_Ragdoll_Attached,
...
struct ragdoll_memory_list_t {
  ...
  bool IsInList(C_BaseEntity *pRagdoll) {
    for (int i = list.Count() - 1; i >= 0; --i) {
      if (list[i].ragdoll == pRagdoll) return true;
    }
