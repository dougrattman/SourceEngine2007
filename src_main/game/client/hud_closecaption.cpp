...
static ConVar cc_linger_time("cc_linger_time", "1.0", FCVAR_ARCHIVE,
                             "Close caption linger time.");
static ConVar cc_predisplay_time("cc_predisplay_time", "0.25", FCVAR_ARCHIVE,
                                 "Close caption delay before showing caption.");
static ConVar cc_captiontrace(
    "cc_captiontrace", "1", 0,
    "Show missing closecaptions (0 = no, 1 = devconsole, 2 = show in hud)");
static ConVar cc_subtitles(
    "cc_subtitles", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
    "If set, don't show sound effect captions, just voice overs (i.e., won't "
    "help hearing impaired players).");
ConVar english("english", "1", FCVAR_USERINFO,
               "If set to 1, running the english language set of assets.");
static ConVar cc_smallfontlength(
    "cc_smallfontlength", "300", 0,
    "If text stream is this long, force usage of small font size.");

#define MAX_CAPTION_CHARACTERS 4096

#define CAPTION_PAN_FADE_TIME \
  0.5  // The time it takes for a line to fade while panning over a large entry
#define CAPTION_PAN_SLIDE_TIME \
  0.5  // The time it takes for a line to slide on while panning over a large
       // entry
...
// Output : int
//-----------------------------------------------------------------------------
int ConvertANSIToUnicode(const char *ansi, wchar_t *unicode,
                         int unicodeBufferSize) {
  return g_pVGuiLocalize->ConvertANSIToUnicode(ansi, unicode,
                                               unicodeBufferSize);
}
...
// The width and height are stored so that layout can be quickly recomputed each
// frame
class CCloseCaptionWorkUnit {
 public:
  ...
  void SetFont(vgui::HFont fnt) { m_hFont = fnt; }
...
class CCloseCaptionItem {
 public:
  CCloseCaptionItem(const wchar_t *stream, float timetolive, float addedtime,
                    float predisplay, bool valid, bool fromplayer)
      : m_flTimeToLive(0.0f),
  ...
  void SetHeight(int h) { m_nTotalHeight = h; }
  int GetHeight() const { return m_nTotalHeight; }
  void SetWidth(int w) { m_nTotalWidth = w; }
  int GetWidth() const { return m_nTotalWidth; }
  ...
  float GetAlpha(float fadeintimehidden, float fadeintime, float fadeouttime) {
    ...
    if (totalfadeintime > 0.001f && time_since_start < totalfadeintime) {
    ...
    if (fadeouttime > 0.001f && time_until_end < fadeouttime) {
...
//-----------------------------------------------------------------------------
// Purpose: The only resource manager parameter we currently care about is the
// name
...
void CaptionAsyncLoaderCallback(const FileAsyncRequest_t &request,
                                int numReadBytes, FSAsyncStatus_t asyncStatus);
...
struct AsyncCaptionData_t {
  ...
  AsyncCaptionData_t *GetData() { return this; }
  unsigned int Size() { return sizeof(*this) + m_nBlockSize; }
  ...
  // -----------------------------------------------------------
  static AsyncCaptionData_t *CreateResource(
      const asynccaptionparams_t &params) {
...
//-----------------------------------------------------------------------------
// Purpose: This manages the instanced scene memory handles.  We essentially
// grow a handle list by scene filename where
//  the handle is a pointer to a AsyncCaptionData_t defined above.  If the
//  resource manager uncaches the handle, we reload the .vcd from disk.
//  Precaching a .vcd calls into FindOrAddBlock which moves the .vcd to the head
//  of the LRU if it's in memory or it reloads it from disk otherwise.
//-----------------------------------------------------------------------------
class CAsyncCaptionResourceManager
    : public CAutoGameSystem,
      public CManagedDataCacheClient<AsyncCaptionData_t, asynccaptionparams_t> {
 public:
  ...
  virtual bool Init() {
    CCacheClientBaseClass::Init(datacache, "Captions",
                                MAX_ASYNCCAPTION_MEMORY_CACHE);
    return true;
  }
  ...
  void SpewMemoryUsage() {
    ...
    DevMsg(
        "CAsyncCaptionResourceManager:  %i blocks total %s, %.2f %% of "
        "capacity\n",
  ...
  int ComputeBlockOffset(int fileIndex, int blockNum) {
    return m_Db[fileIndex].m_Header.dataoffset +
           blockNum * m_Db[fileIndex].m_Header.blocksize;
  }

  void GetBlockInfo(int fileIndex, int blockNum, bool &entry, bool &pending,
                    bool &loaded) {
    pending = false;
    loaded = false;
    ...
    CUtlRBTree<AsyncCaption_t::BlockInfo_t, unsigned short> &requested =
        m_Db[fileIndex].m_RequestedBlocks;
  ...
  // Either commences async loading or polls for async loading once per frame to
  // wait for it to complete...
  void PollForAsyncLoading(CHudCloseCaption *hudCloseCaption, int dbFileIndex,
                           int blockNum) {
    const char *dbname = m_Db[dbFileIndex].m_DataBaseFile.String();

    CUtlRBTree<AsyncCaption_t::BlockInfo_t, unsigned short> &requested =
        m_Db[dbFileIndex].m_RequestedBlocks;
    ...
    if (pCaptionData->m_bLoadCompleted) {
      ...
      if (hudCloseCaption) {
        hudCloseCaption->OnFinishAsyncLoad(
            requested[idx].fileindex, requested[idx].blocknum, pCaptionData);
      }
  ...
  //-----------------------------------------------------------------------------
  // Purpose: Touch the cache or load the scene into the cache for the first
  // time Input  : *filename -
  //-----------------------------------------------------------------------------
...
void CaptionAsyncLoaderCallback(const FileAsyncRequest_t &request,
                                int numReadBytes, FSAsyncStatus_t asyncStatus) {
  g_AsyncCaptionResourceManager.CaptionAsyncLoaderCallback(
      request, numReadBytes, asyncStatus);
}
...
static ConVar cc_minvisibleitems("cc_minvisibleitems", "1", 0,
...
void CHudCloseCaption::Paint(void) {
  ...
  if (m_bPaintDebugInfo) {
    ...
    for (int a = 0; a < m_AsyncCaptions.Count(); ++a) {
      ...
      for (int i = 0; i < c; ++i) {
        ...
        g_AsyncCaptionResourceManager.GetBlockInfo(a, i, entry, pending,
                                                   loaded);
  ...
  for (i = 0; i < c; i++) {
    ...
    while (itemheight <= avail_height && totalheight > avail_height &&
  ...
  totalheight = std::max(totalheight, m_nLineHeight);
  ...
  if (!visibleitems.Count() && m_nGoalHeight == m_nCurrentHeight &&
  ...
  // Continue growth?
  if (m_flGoalHeightFinishTime && m_flGoalHeightStartTime &&
    ...
    m_flCurrentAlpha = std::clamp(newAlpha, 0.0f, 1.0f);
  } else {
  ...
  DrawBox(rcText.left, std::max(rcText.top, 0), rcText.right - rcText.left,
          rcText.bottom - std::max(rcText.top, 0), bgColor, m_flCurrentAlpha);
  ...
  if (growingDown) {
    // If growing downward, keep the text locked to the bottom of the window
    // instead of anchored to the top
  ...
  for (i = 0; i < c; i++) {
    ...
    // If the oldest/top item was created with additional time, we can remove
    // that now
    if (i == 0) {
    ...
    if (height > avail_height) {
      ...
      int iLinesToMove =
          std::clamp(floor(flCurMove / flMoveDelta), 0.0f, (float)units);
      if (iLinesToMove) {
        ...
        if (flTimePostMove < CAPTION_PAN_FADE_TIME) {
          ...
          // It's time to fade out the top line. If it hasn't started fading
          // yet, start it.
          ...
          // Fade out quickly
          float flFadeTime =
              (gpGlobals->curtime - wu->GetFadeStart()) / CAPTION_PAN_FADE_TIME;
          flFadeLineAlpha = std::clamp(1.0f - flFadeTime, 0.0f, 1.0f);
        } else if (flTimePostMove <
                   (CAPTION_PAN_FADE_TIME + CAPTION_PAN_SLIDE_TIME)) {
          flTimePostMove -= CAPTION_PAN_FADE_TIME;
          float flSlideTime = std::clamp(flTimePostMove / 0.25f, 0.0f, 1.0f);
          iHeightToMove +=
              ceil((iCurrentLineHeight - iHeightToMove) * flSlideTime);
        } else {
          iHeightToMove = iCurrentLineHeight;
        }
      }
...
void CHudCloseCaption::OnTick(void) {
  ...
  for (i = 0; i < c; ++i) {
    ...
    // Skip the remainder of the items after we find the first/oldest active
    // item
...
bool CHudCloseCaption::SplitCommand(wchar_t const **ppIn, wchar_t *cmd,
                                    wchar_t *args) const {
...
//-----------------------------------------------------------------------------
bool CHudCloseCaption::GetFloatCommandValue(const wchar_t *stream,
...
bool CHudCloseCaption::StreamHasCommand(const wchar_t *stream,
                                        const wchar_t *findcmd) const {
  const wchar_t *curpos = stream;
...
void CHudCloseCaption::Process(const wchar_t *stream, float duration,
                               const char *tokenstream, bool fromplayer,
                               bool direct) {
  if (!direct) {
  ...
  if (m_Items.Count() > 0) {
    ...
    lifespan = std::max(lifespan, prevlife);
  }
    ...
    if (SplitCommand(&curpos, cmd, args)) {
      if (!wcscmp(cmd, L"delay")) {
        ...
        *out = L'\0';
        ...
        if (wcslen(phrase) > 0) {
          CCloseCaptionItem *item = new CCloseCaptionItem(
              phrase, lifespan, addedlife, delay, valid, fromplayer);
  ...
  *out = L'\0';
  if (wcslen(phrase) > 0) {
    CCloseCaptionItem *item = new CCloseCaptionItem(phrase, lifespan, addedlife,
                                                    delay, valid, fromplayer);
    m_Items.AddToTail(item);
...
void CHudCloseCaption::CreateFonts(void) {
  ...
  m_nLineHeight =
      std::max(6, vgui::surface()->GetFontTall(m_hFonts[CCFONT_NORMAL]));
}
...
void CHudCloseCaption::ComputeStreamWork(int available_width,
                                         CCloseCaptionItem *item) {
  ...
  for (; curpos && *curpos != L'\0'; ++curpos) {
    ...
    if (SplitCommand(&curpos, cmd, args)) {
      if (!wcscmp(cmd, L"cr")) {
        ...
        if (args[0] == 0 && colorStack.Count() >= 2) {
          ...
          Color newcolor;
          if (6 == swscanf(args, L"%i,%i,%i:%i,%i,%i", &pr, &pg, &pb, &nr, &ng,
                           &nb)) {
            newcolor = item->IsFromPlayer() ? Color(pr, pg, pb, 255)
                                            : Color(nr, ng, nb, 255);
            colorStack.AddToTail(newcolor);
            params.clr = colorStack[colorStack.Count() - 1];
          }
    ...
    if (IsPC()) {
      ...
      font = streamlen >= cc_smallfontlength.GetInt() ? CCFONT_SMALL
                                                      : CCFONT_NORMAL;
    }
...
void CHudCloseCaption::DrawStream(wrect_t &rcText, wrect_t &rcWindow,
                                  CCloseCaptionItem *item, int iFadeLine,
                                  float flFadeLineAlpha) {
  int c = item->GetNumWorkUnits();

  wrect_t rcOut;

  float alpha = item->GetAlpha(m_flItemHiddenTime, m_flItemFadeInTime,
                               m_flItemFadeOutTime);
  ...
  for (int i = 0; i < c; ++i) {
    ...
    // Used for single commentary entries that are too big to fit into the
    // element.
    ...
    if (!item->IsValid()) {
      ...
      vgui::surface()->DrawFilledRect(rcOut.left, rcOut.top, rcOut.right,
                                      rcOut.bottom);
    }
...
bool CHudCloseCaption::CaptionTokenLessFunc(const CaptionRepeat &lhs,
                                            const CaptionRepeat &rhs) {
  return (lhs.m_nTokenIndex < rhs.m_nTokenIndex);
}

static ConVar cc_sentencecaptionnorepeat("cc_sentencecaptionnorepeat", "4", 0,
...
class CAsyncCaption {
 public:
  CAsyncCaption()
      : m_flDuration(0.0f), m_bIsStream(false), m_bFromPlayer(false) {}
  ...
  void StartRequesting(CHudCloseCaption *hudCloseCaption,
    ...
    for (int i = 0; i < c; ++i) {
      ...
      CaptionLookup_t &entry =
          directories[caption->fileindex].m_CaptionDirectory[caption->dirindex];

      // Request this block, and if it's there, it'll call OnDataLoaded
      // immediately
      g_AsyncCaptionResourceManager.PollForAsyncLoading(
          hudCloseCaption, caption->fileindex, entry.blockNum);
    }
  }

  void OnDataArrived(CUtlVector<AsyncCaption_t> &directories, int nFileIndex,
                     int nBlockNum, AsyncCaptionData_t *pData) {
    int c = m_Tokens.Count();
    for (int i = 0; i < c; ++i) {
      caption_t *caption = m_Tokens[i];
      if (caption->stream != NULL) continue;

      // Lookup the data
      CaptionLookup_t &entry =
          directories[nFileIndex].m_CaptionDirectory[caption->dirindex];
      if (entry.blockNum != nBlockNum) continue;
  ...
  void ProcessAsyncWork(CHudCloseCaption *hudCloseCaption,
                        CUtlVector<AsyncCaption_t> &directories) {
    int c = m_Tokens.Count();
    for (int i = 0; i < c; ++i) {
      caption_t *caption = m_Tokens[i];
  ...
  bool AddToken(CUtlVector<AsyncCaption_t> &directories, const char *token) {
    ...
    for (i = 0; i < dc; ++i) {
      ...
      if (idx == directories[i].m_CaptionDirectory.InvalidIndex()) continue;

      break;
    }

    if (i >= dc || idx == -1) return false;
 ...
 private:
  ...
  struct caption_t {
    caption_t() : token(0), dirindex(-1), fileindex(-1), stream(0) {}
  ...
  for (i = m_AsyncWork.Head(); i != m_AsyncWork.InvalidIndex();) {
    ...
    if (stream[0] != L'\0') {
      ...
      if (item->IsStream()) {
        ...
        _ProcessCaption(stream, original, item->GetDuration(),
                        item->IsFromPlayer(), item->IsDirect());
      }
    }
...
void CHudCloseCaption::ProcessCaptionDirect(const char *tokenname,
                                            float duration,
...
bool CHudCloseCaption::AddAsyncWork(const char *tokenstream, bool bIsStream,
                                    float duration, bool fromplayer,
                                    bool direct /* = false */) {
  bool bret = true;
  ...
  if (!bIsStream) {
    bret = async->AddToken(m_AsyncCaptions, tokenstream);
  } else {
    ...
    // p points to reset of sentence tokens, build up a unicode string from
    // them...
  ...
  // Do this last as the block might be resident already and this will finish
  // immediately...
  async->StartRequesting(this, m_AsyncCaptions);
  return bret;
}
...
void CHudCloseCaption::ProcessSentenceCaptionStream(const char *tokenstream) {
  ...
  interval = std::clamp(interval, 0.1f, 60.0f);
...
void CHudCloseCaption::_ProcessSentenceCaptionStream(
    int wordCount, const char *tokenstream, const wchar_t *caption_full) {
  if (wcslen(caption_full) > 0) {
    Process(caption_full, (wordCount + 1) * 0.75f, tokenstream,
            false /*never from player!*/);
  }
}

bool CHudCloseCaption::ProcessCaption(const char *tokenname, float duration,
                                      bool fromplayer /* = false */,
                                      bool direct /* = false */) {
  return AddAsyncWork(tokenname, false, duration, fromplayer, direct);
}

void CHudCloseCaption::_ProcessCaption(const wchar_t *caption,
                                       const char *tokenname, float duration,
                                       bool fromplayer, bool direct) {
...
void CHudCloseCaption::MsgFunc_CloseCaption(bf_read &msg) {
  ...
  if (bIsMale || bIsFemale) {
    Q_snprintf(szTestName, sizeof(szTestName), "%s_%s", tokenname,
               bIsMale ? "male" : "female");
    // If the gender-ified version exists, use it, otherwise fall through and
    // pass the non-gender string to the cc system for processing
...
void CHudCloseCaption::InitCaptionDictionary(const char *dbfile) {
  if (m_CurrentLanguage.IsValid() &&
      !Q_stricmp(m_CurrentLanguage.String(), dbfile))
    return;
  ...
  for (char *path = strtok(searchPaths, ";"); path; path = strtok(NULL, ";")) {
    ...
    if (FILESYSTEM_INVALID_HANDLE != fh) {
      ...
      if (entry.m_Header.directorysize < 0 ||
          entry.m_Header.directorysize > 64 * 1024)
      ...
      int directoryBytes =
          entry.m_Header.directorysize * sizeof(CaptionLookup_t);
      ...
      entry.m_CaptionDirectory.CopyArray(
          (const CaptionLookup_t *)dirbuffer.PeekGet(),
          entry.m_Header.directorysize);
...
void CHudCloseCaption::OnFinishAsyncLoad(int nFileIndex, int nBlockNum,
...
static int EmitCaptionCompletion(
    const char *partial, char commands[COMMAND_COMPLETION_MAXITEMS]
                                      [COMMAND_COMPLETION_ITEM_LENGTH]) {
  int current = 0;
  if (!g_pVGuiLocalize || IsX360()) return current;
  ...
  while (i != vgui::INVALID_STRING_INDEX &&
    ...
    if (ccname) {
      if (!substring || !Q_strncasecmp(ccname, substring, substringLen)) {
        Q_snprintf(commands[current], sizeof(commands[current]), "%s %s",
                   cmdname, ccname);
        current++;
      }
    }
...
CON_COMMAND_F_COMPLETION(cc_emit, "Emits a closed caption", 0,
                         EmitCaptionCompletion) {
  if (args.ArgC() != 2) {
...
void OnCaptionLanguageChanged(IConVar *pConVar, const char *pOldString,
                              float flOldValue) {
...
void CHudCloseCaption::FindSound(char const *pchANSI) {
  ...
  for (int i = 0; i < m_AsyncCaptions.Count(); ++i) {
    ...
    for (int j = 0; j < dict.Count(); ++j) {
      ...
      params.blockoffset =
          data.m_Header.dataoffset + blockNum * data.m_Header.blocksize;
      params.fileindex = i;
      ...
      Q_memcpy((void *)stream, pIn, std::min((usize)lu.length, sizeof(stream)));

      // Now search for search text
      g_pVGuiLocalize->ConvertUnicodeToANSI(stream, streamANSI,
                                            sizeof(streamANSI));
      ...
      if (Q_stristr(streamANSI, pchANSI)) {
        ...
        for (int k = soundemitterbase->First();
             k != soundemitterbase->InvalidIndex();
             k = soundemitterbase->Next(k)) {
