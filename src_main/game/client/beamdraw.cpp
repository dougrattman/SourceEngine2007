...
//-----------------------------------------------------------------------------
CEngineSprite *Draw_SetSpriteTexture(const model_t *pSpriteModel, int frame,
                                     int rendermode) {
  ...
  if (ShouldDrawInWireFrameMode() || r_DrawBeams.GetInt() == 2) {
    if (!g_pBeamWireframeMaterial)
      g_pBeamWireframeMaterial = materials->FindMaterial(
...
//			color -
//-----------------------------------------------------------------------------
void DrawHalo(IMaterial *pMaterial, const Vector &source, float scale,
              float const *color, float flHDRColorScale) {
  static unsigned int nHDRColorScaleCache = 0;
  Vector point, screen;

  if (pMaterial) {
    IMaterialVar *pHDRColorScaleVar =
        pMaterial->FindVarFast("$hdrcolorscale", &nHDRColorScaleCache);
...
//-----------------------------------------------------------------------------
static void ComputeBeamPerpendicular(const Vector &vecBeamDelta,
                                     Vector *pPerp) {
...
void DrawSegs(int noise_divisions, float *prgNoise, const model_t *spritemodel,
              ...
              const Vector &delta, float startWidth, float endWidth,
              float scale, float freq, float speed, int segments, int flags,
              float *color, float fadeLength, float flHDRColorScale) {
  ...
  if (length * div < flMaxWidth * 1.414) {
    // Here, we have too many segments; we could get overlap... so lets have
    // less segments
  ...
  vLast = fmod(freq * speed, 1);  // Scroll speed 3.5 -- initial texture
                                  // position, scrolls 3.5/sec (1.0 is entire
                                  // texture)
  ...
  // Iterator to resample noise waveform (it needs to be generated in powers of
  // 2)
  ...
  for (i = 0; i < segments; i++) {
    ...
    if (endWidth == startWidth) {
      ...
      curSeg.m_flWidth =
          ((fraction * (endWidth - startWidth)) + startWidth) * 2;
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void CalcSegOrigin(Vector *vecOut, int iPoint, int noise_divisions,
                   float *prgNoise, const Vector &source, const Vector &delta,
                   const Vector &perp, int segments, float freq, float scale,
                   float fraction, int flags) {
  Assert(segments > 1);
  ...
  if (scale != 0) {
    ...
    if (flags & FBEAM_SINENOISE) {
      ...
      // Rotate the noise along the perpendicular axis a bit to keep the bolt
      // from looking diagonal
...
//			fadescale -
//-----------------------------------------------------------------------------
void DrawTeslaSegs(int noise_divisions, float *prgNoise,
                   const model_t *spritemodel, float frame, int rendermode,
                   const Vector &source, const Vector &delta, float startWidth,
                   float endWidth, float scale, float freq, float speed,
  ...
  brightness = 1.0;
  if (flags & FBEAM_SHADEIN) brightness = 0;
  ...
  for (i = 0; i < segments; i++) {
    ...
    CalcSegOrigin(&curSeg.m_vPos, i, noise_divisions, prgNoise, source, delta,
                  perp, segments, freq, scale, fraction, flags);
    ...
    if (i == (segments / 2)) {
      ...
      // Halve the width because the beam is breaking in two, and halve it again
      // because width is doubled above
  ...
  if (iBranches) {
    DrawTeslaSegs(noise_divisions, prgNoise, spritemodel, frame, rendermode,
                  vecStart, vecEnd, flWidth, flEndWidth, scale, freq, speed,
                  segments, flags, color, fadeLength, flHDRColorScale);
  }
}
...
void DrawSplineSegs(int noise_divisions, float *prgNoise,
                    const model_t *beammodel, const model_t *halomodel,
                    float flHaloScale, float frame, int rendermode,
                    int numAttachments, Vector *attachment, float startWidth,
                    float endWidth, float scale, float freq, float speed,
                    int segments, int flags, float *color, float fadeLength,
                    float flHDRColorScale) {
  ...
  CEngineSprite *pBeamSprite =
      Draw_SetSpriteTexture(beamsprite, frame, rendermode);
  if (!pBeamSprite) return;
  ...
  CMatRenderContextPtr pRenderContext(g_pMaterialSystem);
  CBeamSegDraw segDraw;
  segDraw.Start(pRenderContext, (segments - 1) * (numAttachments - 1),
                pBeamMaterial);

  CEngineSprite *pHaloSprite =
      (CEngineSprite *)modelinfo->GetModelExtraData(halosprite);
  ...
  float flBeamLength =
      (attachment[0] - attachment[numAttachments - 1]).Length();
  ...
  for (int j = 0; j < numAttachments - 1; j++) {
    ...
    // UNDONE: Expose this paramter as well(3.5)?  Texture scroll rate along
    // beam
    vLast = fmod(freq * speed, 1);  // Scroll speed 3.5 -- initial texture
                                    // position, scrolls 3.5/sec (1.0 is entire
                                    // texture)
    ...
    // -----------------------------------------------------------------------------
    // Iterator to resample noise waveform (it needs to be generated in powers
    // of 2)
    // -----------------------------------------------------------------------------
    noiseStep = (int)((float)(noise_divisions - 1) * div * 65536.0f);
    noiseIndex = noiseStep;

    if (flags & FBEAM_SINENOISE) noiseIndex = 0;
    ...
    for (int i = 1; i < segments; i++) {
      ...
      // Fade in our out beam to fadeLength
      // BUG BUG: should be based on total lengh of beam not this particular
      // fraction
      if (flags & FBEAM_SHADEIN) {
        brightness = fraction / fadeFraction;
        if (brightness > 1) {
      ...
      if (scale != 0) {
        ...
        if (flags & FBEAM_SINENOISE) {
          ...
          // Rotate the noise along the perpendicluar axis a bit to keep the
          // bolt from looking diagonal
      ...
      if (startWidth != endWidth)
        seg.m_flWidth =
            ((fraction * (endSegWidth - startSegWidth)) + startSegWidth) * 2;
      else
    ...
    if (false && pHaloMaterial) {
      ...
      if (fBestFraction > 0) {
        ...
        float curWidth =
            (fBestFraction * (endSegWidth - startSegWidth)) + startSegWidth;
        DrawHalo(pHaloMaterial, vHaloPos, flHaloScale * curWidth / endWidth,
                 haloColor, flHDRColorScale);
      }
    }
  }
...
void DrawDisk(int noise_divisions, float *prgNoise, const model_t *spritemodel,
              ...
              float speed, int segments, float *color, float flHDRColorScale) {
  int i;
...
//			*color -
//-----------------------------------------------------------------------------
void DrawCylinder(int noise_divisions, float *prgNoise,
                  const model_t *spritemodel, float frame, int rendermode,
                  const Vector &source, const Vector &delta, float width,
                  float scale, float freq, float speed, int segments,
...
//			(*pfnNoise -
//-----------------------------------------------------------------------------
void DrawRing(int noise_divisions, float *prgNoise,
              void (*pfnNoise)(float *noise, int divs, float scale),
              const model_t *spritemodel, float frame, int rendermode,
              const Vector &source, const Vector &delta, float width,
              float amplitude, float freq, float speed, int segments,
              float *color, float flHDRColorScale) {
  ...
  for (i = 0; i < segments + 1; i++) {
    ...
    if (i != 0) {
      ...
      tmp[2] = 0;
      VectorNormalize(tmp);
      VectorScale(CurrentViewUp(), tmp[0],
                  normal);  // Build point along noraml line (normal is -y, x)
...
//			*color -
//-----------------------------------------------------------------------------
void DrawBeamFollow(const model_t *spritemodel, BeamTrail_t *pHead, int frame,
                    int rendermode, Vector &delta, Vector &screen,
                    Vector &screenLast, float die, const Vector &source,
                    int flags, float width, float amplitude, float freq,
                    float *color, float flHDRColorScale) {
  float fraction;
  float div;
  float vLast = 0.0;
  ...
  // UNDONE: This won't work, screen and screenLast must be extrapolated here to
  // fix the first beam segment for this trail
...
*/
void DrawBeamQuadratic(const Vector &start, const Vector &control,
                       const Vector &end, float width, const Vector &color,
