...
static ConVar joy_forwardthreshold("joy_forwardthreshold", "0.15",
                                   FCVAR_ARCHIVE);
...
static ConVar joy_forwardsensitivity("joy_forwardsensitivity", "-1",
                                     FCVAR_ARCHIVE);
static ConVar joy_sidesensitivity("joy_sidesensitivity", "1", FCVAR_ARCHIVE);
static ConVar joy_pitchsensitivity("joy_pitchsensitivity", "1",
                                   FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX);
static ConVar joy_yawsensitivity("joy_yawsensitivity", "-1",
                                 FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX);

// Advanced sensitivity and response
static ConVar joy_response_move(
    "joy_response_move", "1", FCVAR_ARCHIVE,
    "'Movement' stick response mode: 0=Linear, 1=quadratic, 2=cubic, "
    "3=quadratic extreme, 4=power function(i.e., pow(x,1/sensitivity)), "
    "5=two-stage");
ConVar joy_response_move_vehicle("joy_response_move_vehicle", "6");
static ConVar joy_response_look(
...
static ConVar joy_autoaimdampenrange(
    "joy_autoaimdampenrange", "0", FCVAR_ARCHIVE,
    "The stick range where autoaim dampening is applied. 0 = off");
static ConVar joy_autoaimdampen("joy_autoaimdampen", "0", FCVAR_ARCHIVE,
                                "How much to scale user stick input when the "
                                "gun is pointing at a valid target.");
...
// Misc
static ConVar joy_diagonalpov(
    "joy_diagonalpov", "0", FCVAR_ARCHIVE,
...
ConVar joy_autosprint(
    "joy_autosprint", "0", 0,
    "Automatically sprint when moving with an analog joystick");

static ConVar joy_inverty(
    "joy_inverty", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
    "Whether to invert the Y axis of the joystick for looking.");

// XBox Defaults
static ConVar joy_yawsensitivity_default("joy_yawsensitivity_default", "-1.25",
                                         FCVAR_NONE);
static ConVar joy_pitchsensitivity_default("joy_pitchsensitivity_default",
                                           "-1.0", FCVAR_NONE);
static ConVar option_duck_method_default("option_duck_method_default", "1.0",
                                         FCVAR_NONE);
static ConVar joy_inverty_default(
    "joy_inverty_default", "0",
    FCVAR_ARCHIVE_XBOX);  // Extracted & saved from profile
static ConVar joy_movement_stick_default(
    "joy_movement_stick_default", "0",
    FCVAR_ARCHIVE_XBOX);  // Extracted & saved from profile
static ConVar sv_stickysprint_default("sv_stickysprint_default", "0",
                                      FCVAR_NONE);

void joy_movement_stick_Callback(IConVar *var, const char *pOldString,
                                 float flOldValue) {
  engine->ClientCmd("joyadvancedupdate");
}
static ConVar joy_movement_stick(
    "joy_movement_stick", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
    "Which stick controls movement (0 is left stick)",
    joy_movement_stick_Callback);

static ConVar joy_xcontroller_cfg_loaded(
    "joy_xcontroller_cfg_loaded", "0", FCVAR_ARCHIVE,
    "If 0, the 360controller.cfg file will be executed on startup & option "
    "changes.");
...
static float ResponseCurve(int curve, float x, int axis, float sensitivity) {
  switch (curve) {
    ...
    case 3: {
      ...
      if (x < 0) return -extreme * x * x * sensitivity;
    ...
    {
      if (axis == YAW) {
        // This code only wants to affect YAW axis (the left and right axis),
        // which is used for turning in the car. We fall-through and use a
        // linear curve on the PITCH axis, which is the vehicle's throttle.
        // REALLY, these are the 'forward' and 'side' axes, but we don't have
        // constants for those, so we re-use the same axis convention as the
        // look stick. (sjb)
        float sign = 1;
        ...
        if (x <= joy_vehicle_turn_lowend.GetFloat())
          x = RemapVal(x, 0.0f, joy_vehicle_turn_lowend.GetFloat(), 0.0f,
                       joy_vehicle_turn_lowmap.GetFloat());
        else
          x = RemapVal(x, joy_vehicle_turn_lowend.GetFloat(), 1.0f,
                       joy_vehicle_turn_lowmap.GetFloat(), 1.0f);

        return x * sensitivity * sign;
      }
      // else
      //	fall through and just return x*sensitivity below (as if using
      // default curve)
    }
  }
...
float AutoAimDampening(float x, int axis, float dist) {
  ...
  if (joy_autoaimdampen.GetFloat() > 0.0f &&
      fabs(x) < joy_autoaimdampenrange.GetFloat()) {
    // Get the HL2 player
    C_BaseHLPlayer *pLocalPlayer =
        (C_BaseHLPlayer *)C_BasePlayer::GetLocalPlayer();
...
ConVar joy_pegged(
    "joy_pegged",
    "0.75");  // Once the stick is pushed this far, it's assumed pegged.
ConVar joy_virtual_peg("joy_virtual_peg", "0");
static float ResponseCurveLookDefault(float x, int axis, float otherAxis,
                                      float dist, float frametime) {
  float input = x;

  bool bStickIsPhysicallyPegged = (dist >= joy_pegged.GetFloat());

  // Make X positive to make things easier, just remember whether we have to
  // flip it back!
  bool negative = false;
  if (x < 0.0f) {
    negative = true;
  ...
  if (axis == YAW && joy_virtual_peg.GetBool()) {
    ...
    if (controlEnvelope.peggedAxis[axis] == true) {
      ...
      if (bStickIsPhysicallyPegged &&
          negative == controlEnvelope.axisPeggedDir[axis]) {
        // If the user still has the stick physically pegged and hasn't changed
        // direction on this axis, keep pretending they have the stick pegged on
        // this axis.
        x = 1.0f;
      } else {
  ...
  if (x > joy_lowend.GetFloat()) {
    ...
    if (controlEnvelope.envelopeScale[axis] < 1.0f) {
      controlEnvelope.envelopeScale[axis] +=
          (frametime * joy_accelscale.GetFloat());
      if (controlEnvelope.envelopeScale[axis] > 1.0f) {
...
ConVar joy_accel_filter("joy_accel_filter",
                        "0.2");  // If the non-accelerated axis is pushed
                                 // farther than this, then accelerate it, too.
static float ResponseCurveLookAccelerated(float x, int axis, float otherAxis,
                                          float dist, float frametime) {
  float input = x;
  ...
  // Make X positive to make arithmetic easier for the rest of this function,
  // and remember whether we have to flip it back!
  bool negative = false;
  if (x < 0.0f) {
    negative = true;
    x *= -1;
...
//-----------------------------------------------
//-----------------------------------------------
static float ResponseCurveLook(int curve, float x, int axis, float otherAxis,
                               float dist, float frametime) {
  switch (curve) {
...
void CInput::Joystick_Advanced(void) {
  ...
  if (!joy_advanced.GetBool()) {
    ...
    dwTemp = (joy_movement_stick.GetBool()) ? (DWORD)joy_advaxisu.GetInt()
                                            : (DWORD)joy_advaxisx.GetInt();
    ...
    dwTemp = (joy_movement_stick.GetBool()) ? (DWORD)joy_advaxisr.GetInt()
                                            : (DWORD)joy_advaxisy.GetInt();
    ...
    dwTemp = (joy_movement_stick.GetBool()) ? (DWORD)joy_advaxisx.GetInt()
                                            : (DWORD)joy_advaxisu.GetInt();
...
void CInput::DescribeJoystickAxis(char const *axis, joy_axis_t *mapping) {
  if (!mapping->AxisMap) {
    ...
    Msg("%s:  mapped to %s (%s)\n", axis, DescribeAxis(mapping->AxisMap),
...
// Not currently used - controller button events are pumped through the
// windprocs. KWD
//-----------------------------------------------------------------------------
void CInput::ControllerCommands(void) {}

//-----------------------------------------------------------------------------
// Purpose: Scales the raw analog value to lie withing the axis range (full
// range - deadzone )
//-----------------------------------------------------------------------------
float CInput::ScaleAxisValue(const float axisValue, const float axisThreshold) {
  // Xbox scales the range of all axes in the inputsystem. PC can't do that
  // because each axis mapping has a (potentially) unique threshold value.  If
  // all axes were restricted to a single threshold as they are on the Xbox,
  // this function could move to inputsystem and be slightly more optimal.
  float result = 0.f;
...
void CInput::JoyStickMove(float frametime, CUserCmd *cmd) {
  // complete initialization if first time in ( needed as cvars are not
  ...
  if (!in_joystick.GetInt() || 0 == inputsystem->GetJoystickCount()) return;
  ...
  for (int i = 0; i < MAX_JOYSTICK_AXES; ++i) {
    if (GAME_AXIS_NONE == m_rgAxes[i].AxisMap) continue;

    float fAxisValue =
        inputsystem->GetAnalogValue((AnalogCode_t)JOYSTICK_AXIS(0, i));
  ...
  float forward =
      ScaleAxisValue(gameAxes[GAME_AXIS_FORWARD].value,
                     MAX_BUTTONSAMPLE * joy_forwardthreshold.GetFloat());
  float side = ScaleAxisValue(gameAxes[GAME_AXIS_SIDE].value,
                              MAX_BUTTONSAMPLE * joy_sidethreshold.GetFloat());
  float pitch =
      ScaleAxisValue(gameAxes[GAME_AXIS_PITCH].value,
                     MAX_BUTTONSAMPLE * joy_pitchthreshold.GetFloat());
  float yaw = ScaleAxisValue(gameAxes[GAME_AXIS_YAW].value,
                             MAX_BUTTONSAMPLE * joy_yawthreshold.GetFloat());
  ...
  if (CAM_IsThirdPerson() && thirdperson_platformer.GetInt()) {
    if (forward || side) {
      ...
      // factor in the camera offset, so that the move direction is relative to
      // the thirdperson camera
      viewangles[YAW] =
          RAD2DEG(atan2(-side, -forward)) + m_vecCameraOffset[YAW];
  ...
  if (pLocalPlayer && pLocalPlayer->IsInAVehicle()) {
    iResponseCurve =
        pLocalPlayer->GetVehicle()
            ? pLocalPlayer->GetVehicle()->GetJoystickResponseCurve()
            : joy_response_move_vehicle.GetInt();
  } else {
    iResponseCurve = joy_response_move.GetInt();
  }

  float val = ResponseCurve(iResponseCurve, forward, PITCH,
                            joy_forwardsensitivity.GetFloat());
  joyForwardMove += val * cl_forwardspeed.GetFloat();
  val =
      ResponseCurve(iResponseCurve, side, YAW, joy_sidesensitivity.GetFloat());
  ...
  if (JOY_ABSOLUTE_AXIS == gameAxes[GAME_AXIS_YAW].controlType) {
    float fAxisValue = ResponseCurveLook(joy_response_look.GetInt(), yaw, YAW,
                                         pitch, dist, frametime);
    angle = fAxisValue * joy_yawsensitivity.GetFloat() * aspeed *
            cl_yawspeed.GetFloat();
  } else {
  ...
  if (IsX360() || in_jlook.state & 1) {
    ...
    if (JOY_ABSOLUTE_AXIS == gameAxes[GAME_AXIS_PITCH].controlType) {
      float fAxisValue = ResponseCurveLook(joy_response_look.GetInt(), pitch,
                                           PITCH, yaw, dist, frametime);
      angle = fAxisValue * joy_pitchsensitivity.GetFloat() * aspeed *
              cl_pitchspeed.GetFloat();
    } else {
  ...
  if (CAM_IsThirdPerson() && thirdperson_screenspace.GetInt()) {
    ...
    float forward_movement =
        ideal_cos * joyForwardMove + ideal_sin * joySideMove;
  ...
  if (IsPC()) {
    CCommand tmp;
    if (FloatMakePositive(joyForwardMove) >= joy_autosprint.GetFloat() ||
