...
void CTextureAllocator::Init() {
  ...
  // 32bpp edram is only largest shadow fragment, but resolved to actual shadow
  // atlas because full-res 1024x1024 shadow buffer is too large for EDRAM
...
class CClientShadowMgr : public IClientShadowMgr {
 ...
 private:
  ...
  void BuildGeneralWorldToShadowMatrix(VMatrix& matWorldToShadow,
                                       const Vector& origin, const Vector& dir,
                                       const Vector& xvec, const Vector& yvec);

  void BuildWorldToShadowMatrix(VMatrix& matWorldToShadow, const Vector& origin,
  ...
  bool CullReceiver(ClientShadowHandle_t handle, IClientRenderable* pRenderable,
  ...
  bool BuildSetupListForRenderToTextureShadow(unsigned short clientShadowHandle,
                                              float flArea);
...
void CVisibleShadowList::EnumShadow(unsigned short clientShadowHandle) {
  ...
  if (shadowInfo.m_FalloffBias == 255) return;
  ...
  // shadow. That doesn't mean we shouldn't check it again though. Sucks that we
  // need to compute the sphere + bbox multiply times though.
...
bool CClientShadowMgr::Init() {
  ...
  m_nMaxDepthTextureShadows =
      bTools ? 4 : 1;  // Just one shadow depth texture in games, more in tools
...
void CClientShadowMgr::InitDepthTextureShadows() {
  ...
  if (!m_bDepthTextureActive) {
    ...
    ImageFormat nullFormat =
        materials->GetNullTextureFormat();  // Vendor-dependent 0 texture
                                            // format (takes as little memory as
                                            // possible)
#endif
    materials->BeginRenderTargetAllocation();

#if defined(_X360)
    // For the 360, we'll be rendering depth directly into the dummy depth and
    // Resolve()ing to the depth texture. only need the dummy surface, don't
    // care about color results
    m_DummyColorTexture.InitRenderTargetTexture(
        r_flashlightdepthres.GetInt(), r_flashlightdepthres.GetInt(),
        RT_SIZE_OFFSCREEN, IMAGE_FORMAT_BGR565, MATERIAL_RT_DEPTH_SHARED, false,
        "_rt_ShadowDummy");
    m_DummyColorTexture.InitRenderTargetSurface(r_flashlightdepthres.GetInt(),
                                                r_flashlightdepthres.GetInt(),
    ...
    for (int i = 0; i < m_nMaxDepthTextureShadows; i++) {
...
#if defined(_X360)
      // create a render target to use as a resolve target to get the shared
      // depth buffer surface is effectively never used
      ...
      depthTex.InitRenderTarget(m_nDepthTextureResolution,
                                m_nDepthTextureResolution, RT_SIZE_OFFSCREEN,
                                dstFormat, MATERIAL_RT_DEPTH_NONE, false,
                                strRTName);
#endif
...
void CClientShadowMgr::UpdateAllShadows() {
  for (ClientShadowHandle_t i = m_Shadows.Head(); i != m_Shadows.InvalidIndex();
       i = m_Shadows.Next(i)) {
    ClientShadow_t& shadow = m_Shadows[i];
...
const Vector& CClientShadowMgr::GetShadowDirection() const {
  ...
  if (!m_RenderToTextureActive) return s_vecDown;
...
ClientShadowHandle_t CClientShadowMgr::CreateShadow(ClientEntityHandle_t entity,
  ...
  // even if we have an entity because this entity hasn't set its shadow handle
  // yet
...
float CClientShadowMgr::ComputeLocalShadowOrigin(
  ...
  // the centroid of the caster. The direction of this line is the shadow cast
  // direction, and the point on that line corresponds to the endpoint of the
  // box that is furthest *back* along the shadow direction
  ...
  // product of that with the localShadowDir. lastly, we're subtracting out the
...
void CClientShadowMgr::BuildOrthoShadow(IClientRenderable* pRenderable,
  ...
  shadowmgr->ProjectShadow(m_Shadows[handle].m_ShadowHandle, worldOrigin,
                           vecShadowDir, matWorldToTexture, size, nCount,
                           pLeafList, maxHeight, falloffStart,
...
void CClientShadowMgr::BuildRenderToTextureShadow(
  ...
  for (int i = 0; i != 3; ++i) {
    ...
    test *=
        boxSize[i];  // doing after the projection to simplify projection math
  ...
  VMatrix matWorldToTexture;
  BuildOrthoWorldToShadowMatrix(m_Shadows[handle].m_WorldToShadow, worldOrigin,
                                vecShadowDir, xvec, yvec);
  BuildWorldToTextureMatrix(m_Shadows[handle].m_WorldToShadow, size,
                            matWorldToTexture);
  Vector2DCopy(size, m_Shadows[handle].m_WorldSize);
  ...
  // Compute the falloff attenuation
  // Area computation isn't exact since xvec is not perp to yvec, but close
  // enough Extra factor of 4 in the maxHeight due to the size being half as big
  //	float shadowArea = size.x * size.y;
...
void CClientShadowMgr::BuildFlashlight(ClientShadowHandle_t handle) {
  ...
  if (IsX360() || r_flashlight_version2.GetInt()) {
    // This will update the matrices, but not do work to add the flashlight to
    // surfaces
  ...
  while (pChild) {
    ...
    if (GetActualShadowCastType(pChild) != SHADOWS_NONE) {
      pChild->GetShadowRenderBounds(vecChildMins, vecChildMaxs,
                                    SHADOWS_RENDER_TO_TEXTURE);
      ConcatTransforms(matWorldToBBox, pChild->RenderableToWorldTransform(),
                       childToBBox);
      TransformAABB(childToBBox, vecChildMins, vecChildMaxs, vecNewChildMins,
                    vecNewChildMaxs);
...
bool CClientShadowMgr::ShouldUseParentShadow(IClientRenderable* pRenderable) {
  ...
  ShadowType_t shadowType = GetActualShadowCastType(pShadowParent);
  if (shadowType == SHADOWS_SIMPLE) return false;
...
void CClientShadowMgr::PreRender() {
...

  {
    ...
    // deallocation/(re)allocation
    if ((bDepthTextureActive != m_bDepthTextureActive) ||
        (nDepthTextureResolution != m_nDepthTextureResolution)) {
      // If shadow depth texturing remains on, but resolution changed, shut down
      // and reinitialize depth textures
      if ((bDepthTextureActive == true) && (m_bDepthTextureActive == true) &&
            ...
            !bDepthTextureActive)  // Turning off shadow depth texturing
        {
          ShutdownDepthTextureShadows();
                   ...
                   !m_bDepthTextureActive)  // Turning on shadow depth mapping
        {
          InitDepthTextureShadows();
        }
      }
...
void CClientShadowMgr::UpdateShadow(ClientShadowHandle_t handle, bool force) {
  ...
  // everything to be marked as dirty. So don't clear the flag until this point.
  pRenderable->MarkShadowDirty(false);
}
...
bool CClientShadowMgr::CullReceiver(ClientShadowHandle_t handle,
  ...
  if (foundSeparatingPlane) {
    ...
    if (shadowDot > 0.0f) {
      if (receiverDot <= sourceDot) {
        ...
        // pSourceRenderable->GetRenderOrigin(), dest, 255, 255, 0, true, 1.0f
        // );
        return true;
      } else {
        //				Vector dest;
        ...
        // pSourceRenderable->GetRenderOrigin(), 50, plane.normal, dest );
        //				debugoverlay->AddLineOverlay(
        // pSourceRenderable->GetRenderOrigin(), dest, 255, 0, 0, true, 1.0f );
      }
    } else {
      if (receiverDot >= sourceDot) {
        //				Vector dest;
  ...
  for ( i = 0; i < shadow.m_ClipPlaneCount; ++i)
  {
          ...
          plane.dist = shadow.m_ClipDist[i] - DotProduct( shadow.m_ClipPlane[i],
    ...
    case SHADOW_RECEIVER_STUDIO_MODEL:
      if (shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) {
        ...
        if ((!shadow.m_hTargetEntity) ||
          ...
          shadowmgr->AddShadowToModel(shadow.m_ShadowHandle,
...
bool CClientShadowMgr::BuildSetupShadowHierarchy(IClientRenderable* pRenderable,
  ...
  ShadowType_t shadowType = GetActualShadowCastType(pRenderable);
  if (pRenderable && shadowType == SHADOWS_SIMPLE) return false;
...
bool CClientShadowMgr::BuildSetupListForRenderToTextureShadow(
  ...
  if (bNeedsRedraw || bDirtyTexture) {
    ...
    if (BuildSetupShadowHierarchy(pRenderable, shadow)) return true;
  }
  return false;
}
...
bool CClientShadowMgr::DrawRenderToTextureShadow(
  ...
  if (bNeedsRedraw || bDirtyTexture) {
    ...
    if (DrawShadowHierarchy(pRenderable, shadow)) {
      ...
      // DrawShadowHierarchy but it's 2 days before 360 ship.. not going to
      // change this now.
      DevMsg(
          "Didn't draw shadow hierarchy.. bad shadow texcoords probably going "
          "to happen..grab Brian!\n");
    }
...
int CClientShadowMgr::BuildActiveShadowDepthList(
  ...
  for (ClientShadowHandle_t i = m_Shadows.Head(); i != m_Shadows.InvalidIndex();
    ...
    GeneratePerspectiveFrustum(viewSetup.origin, viewSetup.angles,
...
void CClientShadowMgr::ComputeShadowDepthTextures(const CViewSetup& viewSetup) {
  ...
  for (int j = 0; j < nActiveDepthShadowCount; ++j) {
    ...
    if (!bGotShadowDepthTexture) {
      // If we don't get one, that means we have too many this frame so bind no
      // depth texture
    ...
    QuaternionAngles(flashlightState.m_quatOrientation,
    ...
    // Can turn on all light frustum overlays or per light with flashlightState
    // parameter...
    ...
    // Associate the shadow depth texture and stencil bit with the flashlight
    // for use during scene rendering
...
void CClientShadowMgr::ComputeShadowTextures(const CViewSetup& view,
  ...
  if (m_bThreaded && g_pThreadPool->NumIdleThreads()) {
    ...
    ParallelProcess(s_NPCShadowBoneSetups.Base(), s_NPCShadowBoneSetups.Count(),
                    &SetupBonesOnBaseAnimating);
    ParallelProcess(s_NonNPCShadowBoneSetups.Base(),
                    s_NonNPCShadowBoneSetups.Count(),
                    &SetupBonesOnBaseAnimating);
...
void CClientShadowMgr::SetFlashlightTarget(ClientShadowHandle_t shadowHandle,
  ...
  if ((shadow.m_Flags & SHADOW_FLAGS_FLASHLIGHT) == 0) return;
...
bool CClientShadowMgr::IsFlashlightTarget(ClientShadowHandle_t shadowHandle,
  ...
  C_BaseEntity* pChild = shadow.m_hTargetEntity->FirstMoveChild();
  while (pChild) {
    if (pChild->GetClientRenderable() == pRenderable) return true;
...
void CShadowProxy::OnBind(void* pProxyData) {
  unsigned short clientShadowHandle = (unsigned short)(uintptr_t)pProxyData;
