...
class CCSMapOverview : public CMapOverview {
 ...
 public:
  enum { MAP_ICON_T = 0, MAP_ICON_CT, MAP_ICON_HOSTAGE, MAP_ICON_COUNT };
 ...
 protected:  // private structures & types
  ...
  typedef struct CSMapPlayer_s {
    ...
    bool isDead;  // Death latch, since client can be behind the times on health
                  // messages.
    float timeLastSeen;   // curtime that we last saw this guy.
    float timeFirstSeen;  // curtime that we started seeing this guy
    bool isHostage;  // Not a full player, a hostage.  Special icon, different
                     // death event
 ...
 private:
  bool DrawIconCS(int textureID, int offscreenTextureID, Vector pos,
                  float scale, float angle, int alpha,
                  bool allowRotation = true, const char *text = NULL,
                  Color *textColor = NULL, float status = -1,
                  Color *statusColor = NULL);

  int GetMasterAlpha(void);  // The main alpha that the map part should be,
                             // determined by using the mode to look at the
                             // right convar
  int GetBorderSize(void);   // How far in from the edge of the panel we draw,
                             // based on mode.  Let's the background fancy
                             // corners show.
  ...
  int m_nRadarMapTextureID;  // texture id for radar version of current overview
                             // image

  int m_playerPreferredMode;  // The mode the player wants to be in for when we
                              // aren't being the radar
};
