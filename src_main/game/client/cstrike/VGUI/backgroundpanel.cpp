...
static int coord[NumSegments + 1] = {0, 1, 2, 3, 4, 6, 9, 10};
...
//-----------------------------------------------------------------------------
// Purpose: transform a normalized value into one that is scaled based the
// minimum
...
int GetAlternateProportionalValueFromScaled(HScheme hScheme, int scaledValue) {
  return GetAlternateProportionalValueFromNormal(
      scheme()->GetProportionalNormalizedValueEx(hScheme, scaledValue));
}
...
#if DEBUG_WINDOW_RESIZING
  DevMsg(
      "Resizing '%s' from (%d,%d) %dx%d to (%d,%d) %dx%d -- initially was "
      "(%d,%d) %dx%d\n",
...
//-----------------------------------------------------------------------------
void ApplyBackgroundSchemeSettings(EditablePanel *pWindow,
                                   vgui::IScheme *pScheme) {
  Color bgColor =
      Color(255, 255, 255, pScheme->GetColor("BgColor", Color(0, 0, 0, 0))[3]);
  ...
  // corners --------------------------------------------
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("TopLeftPanel"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("TopRightPanel"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("BottomLeftPanel"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("BottomRightPanel"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  ...
  // background -----------------------------------------
  pBitmapPanel =
      dynamic_cast<CBitmapImagePanel *>(pWindow->FindChildByName("TopSolid"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("UpperMiddleSolid"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("LowerMiddleSolid"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("BottomSolid"));
  if (pBitmapPanel) {
    pBitmapPanel->setImageColor(bgColor);
  }
  ...
  // Logo -----------------------------------------------
  pBitmapPanel = dynamic_cast<CBitmapImagePanel *>(
      pWindow->FindChildByName("ExclamationPanel"));
  if (pBitmapPanel) {
...
//-----------------------------------------------------------------------------
static void FixupBackgroundPanels(EditablePanel *pWindow, int offsetX,
                                  int offsetY) {
  if (!pWindow) return;
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(screenWide - offsetX - inset - cornerSize,
                      offsetY + inset, cornerSize, cornerSize);
  }

  pPanel = pWindow->FindChildByName("BottomLeftPanel");
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(offsetX + inset,
                      screenTall - offsetY - inset - cornerSize, cornerSize,
                      cornerSize);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(offsetX + inset + cornerSize, offsetY + inset,
                      screenWide - 2 * offsetX - 2 * inset - 2 * cornerSize,
                      cornerSize);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(offsetX + inset, offsetY + inset + cornerSize,
                      screenWide - 2 * offsetX - 2 * inset, titleHeight);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(
        offsetX + inset + cornerSize, screenTall - offsetY - inset - cornerSize,
        screenWide - 2 * offsetX - 2 * inset - 2 * cornerSize, cornerSize);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(offsetX + inset,
                      screenTall - offsetY - inset - cornerSize - mainHeight,
                      screenWide - 2 * offsetX - 2 * inset, mainHeight);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(0, screenTall - offsetY - inset, screenWide,
                      offsetY + inset);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(0, offsetY + inset, offsetX + inset,
                      screenTall - 2 * offsetY - 2 * inset);
  }

  pPanel = pWindow->FindChildByName("RightClear");
  if (pPanel) {
    pPanel->SetZPos(-20);
    pPanel->SetBounds(screenWide - offsetX - inset, offsetY + inset,
                      offsetX + inset, screenTall - 2 * offsetY - 2 * inset);
  }
  ...
  if (pPanel) {
    ...
    pPanel->SetBounds(offsetX + inset + logoInset, offsetY + inset + logoInset,
                      logoSize, logoSize);
  }
  ...
  if (pPanel) {
    pPanel->SetZPos(-19);  // higher than the background
    pPanel->SetBounds(offsetX + captionInset /*inset + 2*logoInset + logoSize*/,
                      offsetY + inset + logoInset, screenWide, logoSize);
  }
}
...
void CreateBackground(EditablePanel *pWindow) {
  ...
  new CBitmapImagePanel(pWindow, "BottomRightPanel",
                        "gfx/vgui/round_corner_se");

  // background -----------------------------------------
  new CBitmapImagePanel(pWindow, "TopSolid", "gfx/vgui/solid_background");
  new CBitmapImagePanel(pWindow, "UpperMiddleSolid",
                        "gfx/vgui/solid_background");
  new CBitmapImagePanel(pWindow, "LowerMiddleSolid",
                        "gfx/vgui/solid_background");
  ...
  // Title caption --------------------------------------
  Panel *pPanel =
      dynamic_cast<Label *>(pWindow->FindChildByName("CaptionLabel"));
  if (!pPanel) new CaptionLabel(pWindow, "CaptionLabel", "");
}

void ResizeWindowControls(EditablePanel *pWindow, int tall, int wide,
                          int offsetX, int offsetY) {
  if (!pWindow || !pWindow->GetBuildGroup() ||
      !pWindow->GetBuildGroup()->GetPanelList())
    return;
  ...
  // Resize to account for 1.25 aspect ratio (1280x1024) screens
  {
    for (int i = 0; i < panelList->Size(); ++i) {
      ...
      for (int j = 0; j < resizedPanels.Size(); ++j) {
        if (panel == resizedPanels[j]) found = true;
      }
  ...
  for (int i = 0; i < panelList->Size(); ++i) {
    ...
    for (int j = 0; j < movedPanels.Size(); ++j) {
      if (panel == movedPanels[j]) found = true;
    }
    ...
    if (panel != pWindow) {
      ...
      DevMsg(
          "Repositioning '%s' from (%d,%d) to (%d,%d) -- a distance of "
          "(%d,%d)\n",
...
void LayoutBackgroundPanel(EditablePanel *pWindow) {
  ...
  // now that the panels are moved/resized, look for some bg panels, and
  // re-align them
