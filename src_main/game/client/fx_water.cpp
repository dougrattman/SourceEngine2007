...
// Each channel does not contribute to the luminosity equally, as represented
// here
...
//-----------------------------------------------------------------------------
void UTIL_GetNormalizedColorTintAndLuminosity(const Vector &color, Vector *tint,
                                              float *luminosity) {
...
inline void FX_GetSplashLighting(Vector position, Vector *color,
  ...
  if (luminosity != NULL) {
    *luminosity = std::min(1.0f, (*luminosity) * 4.0f);
...
//			scale -
//-----------------------------------------------------------------------------
void FX_WaterRipple(const Vector &origin, float scale, Vector *pColor,
                    float flLifetime, float flAlpha) {
  ...
  if (tr.fraction < 1.0f) {
    ...
    FX_AddQuad(tr.endpos + (tr.plane.normal * 0.5f), tr.plane.normal,
               16.0f * scale, 128.0f * scale, 0.7f,
               flAlpha,  // start alpha
               0.0f,     // end alpha
               0.25f, random->RandomFloat(0, 360),
               random->RandomFloat(-16.0f, 16.0f), color, flLifetime,
...
void FX_GunshotSplash(const Vector &origin, const Vector &normal, float scale) {
  ...
  sparkEmitter->GetBinding().SetBBox(origin - Vector(32, 32, 32),
                                     origin + Vector(32, 32, 32));
  ...
  for (int i = 0; i < 16; i++) {
    ...
    tParticle->m_vecVelocity =
        offDir * random->RandomFloat(SPLASH_MIN_SPEED * flScale * 3.0f,
                                     SPLASH_MAX_SPEED * flScale * 3.0f);
    ...
    tParticle->m_color.r = std::min(1.0f, color[0] * colorRamp) * 255;
    tParticle->m_color.g = std::min(1.0f, color[1] * colorRamp) * 255;
    tParticle->m_color.b = std::min(1.0f, color[2] * colorRamp) * 255;
  ...
  pSimple->GetBinding().SetBBox(origin - Vector(32, 32, 32),
                                origin + Vector(32, 32, 32));
...
//			*pColor -
//-----------------------------------------------------------------------------
void FX_GunshotSlimeSplash(const Vector &origin, const Vector &normal,
                           float scale) {
  if (cl_show_splashes.GetBool() == false) return;
  ...
  VPROF_BUDGET("FX_GunshotSlimeSplash", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
	...
	float	colorRamp;
	float	flScale = std::min( 1.0f, scale / 8.0f );
	...
	for ( int i = 0; i < 24; i++ )
	{
		...
		tParticle->m_color.r = std::min( 1.0f, color.x * colorRamp ) * 255;
		tParticle->m_color.g = std::min( 1.0f, color.y * colorRamp ) * 255;
		tParticle->m_color.b = std::min( 1.0f, color.z * colorRamp ) * 255;
...
void RippleCallback(const CEffectData &data) {
  ...
  // Get our lighting information
  FX_GetSplashLighting(data.m_vOrigin + (Vector(0, 0, 1) * 4.0f), &color,
                       &luminosity);
...
//			timeDelta -
//-----------------------------------------------------------------------------
void CSplashParticle::UpdateVelocity(SimpleParticle *pParticle,
                                     float timeDelta) {
  // Decellerate
  static float dtime;
  static float decay;
...
float CSplashParticle::UpdateAlpha(const SimpleParticle *pParticle) {
  if (m_bUseClipHeight) {
    ...
    return flAlpha *
           RemapValClamped(pParticle->m_Pos.z, m_flClipHeight,
                           m_flClipHeight - (UpdateScale(pParticle) * 0.5f),
                           1.0f, 0.0f);
  }

  return (pParticle->m_uchStartAlpha / 255.0f) +
         ((float)(pParticle->m_uchEndAlpha / 255.0f) -
