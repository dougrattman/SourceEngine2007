...
inline void UpdateRefractTexture(int x, int y, int w, int h,
                                 bool bForceUpdate = false) {
  Assert(!DrawingShadowDepthView());

  if (!IsRetail() && !r_updaterefracttexture.GetBool()) return;
...
inline void UpdateScreenEffectTexture(int textureIndex, int x, int y, int w,
                                      int h, bool bDestFullScreen = false,
                                      Rect_t *pActualRect = NULL) {
  Rect_t srcRect;
  srcRect.x = x;
  srcRect.y = y;
  srcRect.width = w;
  ...
  if (!bDestFullScreen &&
      (nSrcWidth > nDestWidth || nSrcHeight > nDestHeight)) {
    // the source and target sizes aren't necessarily the same (specifically in
    // dx7 where nonpow2 rendertargets aren't supported), so lets figure it out
    // here.
  ...
  pRenderContext->CopyRenderTargetToTextureEx(
      pTexture, 0, &srcRect, bDestFullScreen ? NULL : &destRect);
...
//-----------------------------------------------------------------------------
inline void DrawScreenEffectMaterial(IMaterial *pMaterial, int x, int y, int w,
                                     int h) {
  Rect_t actualRect;
  ...
  pRenderContext->DrawScreenSpaceRectangle(
      pMaterial, x, y, w, h, actualRect.x, actualRect.y,
      actualRect.x + actualRect.width - 1, actualRect.y + actualRect.height - 1,
      pTexture->GetActualWidth(), pTexture->GetActualHeight());
}

// intended for use by dynamic meshes to naively update front buffer textures
// needed by a material
inline void UpdateFrontBufferTexturesForMaterial(IMaterial *pMaterial,
                                                 bool bForce = false) {
  Assert(!DrawingShadowDepthView());
  ...
  if (pMaterial->NeedsPowerOfTwoFrameBufferTexture()) {
    ...
    UpdateScreenEffectTexture(0, pView->x, pView->y, pView->width,
                              pView->height);
  }
}
...
inline void UpdateScreenEffectTexture(void) {
  ...
  UpdateScreenEffectTexture(0, pViewSetup->x, pViewSetup->y, pViewSetup->width,
                            pViewSetup->height);
}
