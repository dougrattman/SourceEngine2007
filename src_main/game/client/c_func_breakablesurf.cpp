...
class C_BreakableSurface : public C_BaseEntity, public IBrushRenderer {
 public:
  ...
  // This is the texture we're going to use to multiply by the cracked base
  // texture
 ...
 public:
  void InitMaterial(WinEdge_t nEdgeType, int nEdgeStyle,
  ...
  bool RenderBrushModelSurface(IClientEntity* pBaseEntity,
 ...
 private:
  ...
  byte m_nPanelBits[MAX_NUM_PANELS]
                   [MAX_NUM_PANELS];  // UNDONE: allocate this dynamically?
  ...
  void DrawOneHighlight(IBrushSurface* pBrushSurface, IMesh* pMesh,
                        CMeshBuilder* pMeshBuilder, const Vector& vStartPos,
                        const Vector& vWStep, const Vector& vHstep,
                        WinSide_t nEdge);
  ...
  void AddToRenderList(int nWidth, int nHeight, WinSide_t nSide,
                       WinEdge_t nEdgeType, int forceStyle);
...
DEFINE_ARRAY(m_nPanelBits, FIELD_CHARACTER, MAX_NUM_PANELS* MAX_NUM_PANELS),
    ...
    // ), DEFINE_FIELD( m_pMaterialBox, CMaterialReference ), DEFINE_FIELD(
    // m_pSolid, EdgeTexture_t ), DEFINE_ARRAY( m_pEdge, EdgeTexture_t,
    // NUM_EDGE_TYPES][NUM_EDGE_STYLES ), DEFINE_FIELD( m_pCrackedMaterial,
    // CMaterialReference ), DEFINE_FIELD( m_pMaterialBoxTexture,
    // CTextureReference ),

    END_DATADESC()

        bool C_BreakableSurface::InLegalRange(int nWidth, int nHeight) {
  return (nWidth < m_nNumWide && nHeight < m_nNumHigh && nWidth >= 0 &&
          nHeight >= 0);
}
...
IMPLEMENT_CLIENTCLASS_DT(C_BreakableSurface, DT_BreakableSurface,
                         CBreakableSurface)
RecvPropInt(RECVINFO(m_nNumWide)), RecvPropInt(RECVINFO(m_nNumHigh)),
    RecvPropFloat(RECVINFO(m_flPanelWidth)),
    RecvPropFloat(RECVINFO(m_flPanelHeight)),
    RecvPropVector(RECVINFO(m_vNormal)), RecvPropVector(RECVINFO(m_vCorner)),
    RecvPropInt(RECVINFO(m_bIsBroken)), RecvPropInt(RECVINFO(m_nSurfaceType)),
    RecvPropArray3(RECVINFO_ARRAY(m_RawPanelBitVec),
                   RecvPropInt(RECVINFO(m_RawPanelBitVec[0]))),
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::InitMaterial(WinEdge_t nEdgeType, int nEdgeStyle,
                                      char const* pMaterialName) {
  m_pEdge[nEdgeType][nEdgeStyle].m_nRenderIndex = m_RenderList.InvalidIndex();
  m_pEdge[nEdgeType][nEdgeStyle].m_nStyle = nEdgeStyle;
  m_pEdge[nEdgeType][nEdgeStyle].m_pMaterialEdge.Init(
      pMaterialName, TEXTURE_GROUP_CLIENT_EFFECTS);
  m_pEdge[nEdgeType][nEdgeStyle].m_pMaterialEdgeTexture.Init(
      GetBaseTexture(m_pEdge[nEdgeType][nEdgeStyle].m_pMaterialEdge));
}
...
void C_BreakableSurface::OnDataChanged(DataUpdateType_t updateType) {
  ...
  Q_memcpy(m_PrevRawPanelBitVec, m_RawPanelBitVec.Base(),
           sizeof(m_PrevRawPanelBitVec));
}

bool C_BreakableSurface::IsTransparent(void) {
  // Not an identity brush if it's broken
  if (m_bIsBroken) return true;
...
bool C_BreakableSurface::RenderBrushModelSurface(IClientEntity* pBaseEntity,
                                                 IBrushSurface* pBrushSurface) {
...
void C_BreakableSurface::DrawRenderList(IBrushSurface* pBrushSurface) {
  ...
  for (unsigned short i = m_RenderList.Head(); i != m_RenderList.InvalidIndex();
       i = m_RenderList.Next(i)) {
    if (nCurStyle != m_RenderList[i].m_nStyle ||
        nCurEdgeType != m_RenderList[i].m_nEdgeType) {
      nCurStyle = m_RenderList[i].m_nStyle;
      nCurEdgeType = m_RenderList[i].m_nEdgeType;

      m_pCurrentDetailTexture =
          m_pEdge[nCurEdgeType][nCurStyle].m_pMaterialEdgeTexture;
      pRenderContext->Flush(false);
      pRenderContext->Bind(m_pCrackedMaterial, (IClientRenderable*)this);
      pMesh = pRenderContext->GetDynamicMesh();
    }

    Vector vRenderPos = m_vCorner + (m_RenderList[i].m_nWidth * vWidthStep) +
                        (m_RenderList[i].m_nHeight * vHeightStep);

    DrawOneEdge(pBrushSurface, pMesh, &pMeshBuilder, vRenderPos, vWidthStep,
                vHeightStep, (WinSide_t)m_RenderList[i].m_nSide);
  }
}
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::DrawRenderListHighlights(
    IBrushSurface* pBrushSurface) {
  // Get width and height steps
  QAngle vAngles;
  VectorAngles(-1 * m_vNormal, vAngles);
  ...
  for (unsigned short i = m_RenderList.Head(); i != m_RenderList.InvalidIndex();
    ...
    DrawOneHighlight(pBrushSurface, pMesh, &pMeshBuilder, vRenderPos,
                     vWidthStep, vHeightStep,
                     (WinSide_t)m_RenderList[i].m_nSide);
  }
}
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::UpdateEdgeType(int nWidth, int nHeight,
                                        int forceStyle /*=-1*/) {
...
//--------------------------------------------------------------------------------
// Purpose : Return index to panel in render list that meets these
// qualifications Input   : Output  :
//--------------------------------------------------------------------------------
int C_BreakableSurface::FindRenderPanel(int nWidth, int nHeight,
                                        WinSide_t nWinSide) {
  for (unsigned short i = m_RenderList.Head(); i != m_RenderList.InvalidIndex();
       i = m_RenderList.Next(i)) {
...
//----------------------------------------------------------------------------------
// Purpose : Returns first element in render list with the same edge type and
// style Input   : Output  :
//----------------------------------------------------------------------------------
int C_BreakableSurface::FindFirstRenderTexture(WinEdge_t nEdgeType,
                                               int nStyle) {
  for (unsigned short i = m_RenderList.Head(); i != m_RenderList.InvalidIndex();
       i = m_RenderList.Next(i)) {
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::AddToRenderList(int nWidth, int nHeight,
                                         WinSide_t nSide, WinEdge_t nEdgeType,
                                         int forceStyle) {
  // -----------------------------------------------------
  ...
  for (int width = 0; width < m_nNumWide; width++) {
    ...
    for (height = 0; height < m_nNumHigh; height++) {
      ...
      // Drow the strip and start counting again
        ...
        vCurPos =
            m_vCorner + vWidthStep * width + vHeightStep * (height - nHCount);
        DrawOneBlock(pBrushSurface, pMesh, &pMeshBuilder, vCurPos, vWidthStep,
                     vHeightStep * nHCount);
        nHCount = 0;
      }
    }
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::DrawOneBlock(IBrushSurface* pBrushSurface,
                                      IMesh* pMesh, CMeshBuilder* pMeshBuilder,
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::DrawOneHighlight(
    IBrushSurface* pBrushSurface, IMesh* pMesh, CMeshBuilder* pMeshBuilder,
    const Vector& vStartPos, const Vector& vWStep, const Vector& vHStep,
    WinSide_t nEdge) {
...
IMaterial* CBreakableSurfaceProxy::GetMaterial() {
  if (!m_BaseTextureVar) return NULL;

  return m_BaseTextureVar->GetOwningMaterial();
}

EXPOSE_INTERFACE(CBreakableSurfaceProxy, IMaterialProxy,
                 "BreakableSurface" IMATERIAL_PROXY_INTERFACE_VERSION);
