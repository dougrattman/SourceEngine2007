...
CTeamBitmapImage::~CTeamBitmapImage() {
  ...
  for (i = 0; i < BITMAP_COUNT; ++i) {
    if (m_ppImage[i]) delete m_ppImage[i];
  }
}
...
// initialization
//-----------------------------------------------------------------------------
bool CTeamBitmapImage::Init(vgui::Panel *pParent, KeyValues *pInitData,
  ...
  char **ppTeamNames =
      m_bRelativeTeams ? pRelativeTeamNames : pAbsoluteTeamNames;
  ...
  for (i = 0; i < BITMAP_COUNT; ++i) {
    ...
    if (!pClassImage || !pClassImage[0]) return false;

    // modulation color
    Color color;
    if (!ParseRGBA(pTeamKV, "color", color)) color.SetColor(255, 255, 255, 255);
...
void CTeamBitmapImage::Paint(float yaw /*= 0.0f*/) {
  ...
  if (m_bRelativeTeams) {
    ...
    if (GetEntity()) team = GetEntity()->GetTeamNumber();
  }
...
bool InitializeTeamImage(KeyValues *pInitData, const char *pSectionName,
                         vgui::Panel *pParent, C_BaseEntity *pEntity,
                         CTeamBitmapImage *pTeamImage) {
  KeyValues *pTeamImageSection = pInitData;
  if (pSectionName) {
    pTeamImageSection = pInitData->FindKey(pSectionName);
    if (!pTeamImageSection) return false;
  }
