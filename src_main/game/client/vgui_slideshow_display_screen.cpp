...
DECLARE_VGUI_SCREEN_FACTORY(CSlideshowDisplayScreen,
                            "slideshow_display_screen");

//-----------------------------------------------------------------------------
// Constructor:
//-----------------------------------------------------------------------------
CSlideshowDisplayScreen::CSlideshowDisplayScreen(vgui::Panel *parent,
...
void CSlideshowDisplayScreen::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_cDefault =
      pScheme->GetColor("CSlideshowDisplayScreen_Default", GetFgColor());
...
// Initialization
//-----------------------------------------------------------------------------
bool CSlideshowDisplayScreen::Init(KeyValues *pKeyValues,
                                   VGuiScreenInitData_t *pInitData) {
  // Make sure we get ticked...
  vgui::ivgui()->AddTickSignal(GetVPanel());

  if (!BaseClass::Init(pKeyValues, pInitData)) return false;
...
void CSlideshowDisplayScreen::OnTick() {
  ...
  if (g_SlideshowDisplays.Count() <= 0) return;

  C_SlideshowDisplay *pSlideshowDisplay = NULL;

  for (int iDisplayScreens = 0; iDisplayScreens < g_SlideshowDisplays.Count();
       ++iDisplayScreens) {
    C_SlideshowDisplay *pSlideshowDisplayTemp =
        g_SlideshowDisplays[iDisplayScreens];
...
void CSlideshowDisplayScreen::Update(C_SlideshowDisplay *pSlideshowDisplay) {
  char szBuff[256];
  pSlideshowDisplay->GetDisplayText(szBuff, SOURCE_ARRAYSIZE(szBuff));
  ...
  if (m_pSlideshowImages.Count() == 0) {
    ...
    for (int iSlide = 0; iSlide < pSlideshowDisplay->NumMaterials(); ++iSlide) {
      m_pSlideshowImages.AddToTail(
          SETUP_PANEL(new ImagePanel(this, "SlideshowImage")));
      ...
      if (iMatIndex > 0) {
        ...
        if (pMaterialName) {
          pMaterialName =
              Q_strnchr(pMaterialName, '/', Q_strlen(pMaterialName));
