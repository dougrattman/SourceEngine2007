...
// Purpose:
//-----------------------------------------------------------------------------
CAchievementNotificationPanel::CAchievementNotificationPanel(
    const char *pElementName)
    : CHudElement(pElementName),
      BaseClass(NULL, "AchievementNotificationPanel") {
...
void CAchievementNotificationPanel::PerformLayout(void) {
  ...
  // Set background color of various elements.  Need to do this in code, if we
  // do it in res file it gets slammed by the scheme.  (Incl. label background:
  // some products don't have label background colors set in their scheme and
  // helpfully slam it to white.)
...
void CAchievementNotificationPanel::FireGameEvent(IGameEvent *event) {
  ...
  if (0 == Q_strcmp(name, "achievement_event")) {
    ...
    if (IsPC()) {
#ifndef NO_STEAM
      // shouldn't ever get achievement progress if steam not running and user
      // logged in, but check just in case
      if (!SteamUserStats()) {
        Msg("Steam not running, achievement progress notification not "
            "displayed\n");
      }

      // use Steam to show achievement progress UI
      CGameID gameID(engine->GetAppID());
      SteamUserStats()->IndicateAchievementProgress(gameID, pchName, iCur,
                                                    iMax);
#endif
    } else {
      // on X360 we need to show our own achievement progress UI

      const wchar_t *pchLocalizedName =
          ACHIEVEMENT_LOCALIZED_NAME_FROM_STR(pchName);
      Assert(pchLocalizedName);
      if (!pchLocalizedName || !pchLocalizedName[0]) return;
      Q_wcsncpy(szLocalizedName, pchLocalizedName, sizeof(szLocalizedName));

      // this is achievement progress, compose the message of form: "<name>
      // (<#>/<max>)"
      wchar_t szFmt[128] = L"";
      ...
      if (!pchFmt || !pchFmt[0]) return;
      Q_wcsncpy(szFmt, pchFmt, sizeof(szFmt));

      g_pVGuiLocalize->ConstructString(szText, sizeof(szText), szFmt, 3,
                                       szLocalizedName, szNumFound, szNumTotal);
      AddNotification(pchName,
                      g_pVGuiLocalize->Find("#GameUI_Achievement_Progress"),
                      szText);
    }
  }
}
...
bool CAchievementNotificationPanel::ShouldDraw(void) {
  return ((m_flHideTime > 0) && (m_flHideTime > gpGlobals->curtime) &&
...
// Purpose:
//-----------------------------------------------------------------------------
void CAchievementNotificationPanel::AddNotification(const char *szIconBaseName,
                                                    const wchar_t *pHeading,
  ...
  // if we are not currently displaying a notification, go ahead and show this
  // one
  if (0 == m_flHideTime) {
    ShowNextNotification();
  }
...
void CAchievementNotificationPanel::ShowNextNotification() {
  ...
  Notification_t &notification =
      m_queueNotification[m_queueNotification.Head()];
  ...
  int iHeadingWidth =
      UTIL_ComputeStringWidth(hFontHeading, notification.szHeading);
  int iTitleWidth = UTIL_ComputeStringWidth(hFontTitle, notification.szTitle);
  // use the widest string
  int iTextWidth = std::max(iHeadingWidth, iTitleWidth);
  // don't let it be insanely wide
  iTextWidth = std::min(iTextWidth, (int)XRES(300));
...
// Purpose:
//-----------------------------------------------------------------------------
void CAchievementNotificationPanel::SetXAndWide(Panel *pPanel, int x,
                                                int wide) {
  int xCur, yCur;
...
CON_COMMAND_F(achievement_notification_test, "Test the hud notification UI",
              FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY) {
  static int iCount = 0;

  CAchievementNotificationPanel *pPanel =
      GET_HUDELEMENT(CAchievementNotificationPanel);
  if (pPanel) {
    pPanel->AddNotification(
        "HL2_KILL_ODESSAGUNSHIP", L"Achievement Progress",
        (0 == (iCount % 2) ? L"Test Notification Message A (1/10)"
                           : L"Test Message B"));
  }
