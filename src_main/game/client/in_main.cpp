...
// TODO(d.rattman): tie to entity state parsing for player!!!
int g_iAlive = 1;
...
kbutton_t in_graph;
kbutton_t in_joyspeed;  // auto-speed key from the joystick (only works for
                        // player movement, not vehicles)
...
===========
*/
void IN_Joystick_Advanced_f(void) { ::input->Joystick_Advanced(); }

/*
============
KB_ConvertString

Removes references to +use and replaces them with the keyname in the output
string.  If a binding is unfound, then the original text is retained. NOTE: Only
works for text with +word in it.
============
*/
int KB_ConvertString(char *in, char **ppout) {
  ...
  *ppout = NULL;
  ...
  while (*p) {
    if (*p == '+') {
      pEnd = binding;
      while (*p && (isalnum(*p) || (pEnd == binding)) &&
             ((pEnd - binding) < 63)) {
        *pEnd++ = *p++;
      }
...
void KeyUp(kbutton_t *b, const char *c) {
  ...
  if (b->down[0] || b->down[1]) {
    ...
    // b->down[1], c, b->down[0], b->down[1], c);
...
// Special handling
void IN_Cancel(const CCommand &args) { in_cancel = 1; }
...
void CInput::AdjustPitch(float speed, QAngle &viewangles) {
  ...
  if (UsingMouselook() == false) {
    ...
    if (in_klook.state & 1) {
      view->StopPitchDrift();
      viewangles[PITCH] -=
          speed * cl_pitchspeed.GetFloat() * KeyState(&in_forward);
      viewangles[PITCH] +=
          speed * cl_pitchspeed.GetFloat() * KeyState(&in_back);
    }
...
#ifndef PORTAL  // Don't constrain Roll in Portal because the player can be
                // upside down! -Jeep
...
void CInput::ComputeSideMove(CUserCmd *cmd) {
  ...
  // If strafing, check left and right keys and act like moveleft and moveright
  // keys
  if (in_strafe.state & 1) {
...
void CInput::ComputeForwardMove(CUserCmd *cmd) {
  ...
  if (CAM_IsThirdPerson() && thirdperson_platformer.GetInt()) {
    ...
    float movement = KeyState(&in_forward) || KeyState(&in_moveright) ||
                     KeyState(&in_back) || KeyState(&in_moveleft);
...
void CInput::ScaleMovements(CUserCmd *cmd) {
  ...
  // clip to maxspeed
  // TODO(d.rattman): This doesn't work
  return;
  ...
  float fmov = sqrt( (cmd->forwardmove*cmd->forwardmove) +
...
if active == 1 then we are 1) not playing back demos ( where our commands are
ignored ) and 2 ) we have finished signing on to server
================
*/
...
void CInput::ExtraMouseSample(float frametime, bool active) {
  ...
  if (active) {
    ...
    // Scale based on holding speed key or having too fast of a velocity based
    // on client maximum
    //  speed.
    ScaleMovements(cmd);
  ...
  // Retreive view angles from engine ( could have been set in IN_AdjustAngles
  // above )
  engine->GetViewAngles(viewangles);
...
void CInput::CreateMove(int sequence_number, float input_sample_frametime,
                        bool active) {
  ...
  if (active || sv_noclipduringpause.GetInt()) {
    ...
    // need to run and reset mouse input so that there is no view pop when
    // unpausing
    if (!m_fCameraInterceptingMouse && m_fMouseActive) {
      float mx, my;
...
//			to -
//-----------------------------------------------------------------------------
bool CInput::WriteUsercmdDeltaToBuffer(bf_write *buf, int from, int to,
                                       bool isnewcommand) {
  Assert(m_pCommands);
  ...
  if (buf->IsOverflowed()) {
    ...
    Msg("WARNING! User command buffer overflow(%i %i), last cmd was %i bits "
        "long\n",
...
// Output : static void
//-----------------------------------------------------------------------------
static void CalcButtonBits(int &bits, int in_button, int in_ignore,
                           kbutton_t *button, bool reset) {
  // Down or still down?
  if (button->state & 3) {
    bits |= in_button;
  }
...
int CInput::GetButtonBits(int bResetState) {
  ...
  CalcButtonBits(bits, IN_MOVELEFT, s_ClearInputState, &in_moveleft,
                 bResetState);
  CalcButtonBits(bits, IN_MOVERIGHT, s_ClearInputState, &in_moveright,
                 bResetState);
  ...
  CalcButtonBits(bits, IN_GRENADE1, s_ClearInputState, &in_grenade1,
                 bResetState);
  CalcButtonBits(bits, IN_GRENADE2, s_ClearInputState, &in_grenade2,
                 bResetState);
...
//-----------------------------------------------------------------------------
void CInput::ClearInputButton(int bits) { s_ClearInputState |= bits; }
...
==============================
*/
float CInput::GetLookSpring(void) { return lookspring.GetInt(); }

//-----------------------------------------------------------------------------
// Purpose:
// Output : float
//-----------------------------------------------------------------------------
float CInput::GetLastForwardMove(void) { return m_flLastForwardMove; }
...
void CInput::AddIKGroundContactInfo(int entindex, float minheight,
                                    float maxheight) {
...
static ConCommand startcommandermousemove("+commandermousemove",
                                          IN_CommanderMouseMoveDown);
static ConCommand endcommandermousemove("-commandermousemove",
                                        IN_CommanderMouseMoveUp);
