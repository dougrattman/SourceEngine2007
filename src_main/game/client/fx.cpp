...
extern void FX_TracerSound(const Vector &start, const Vector &end,
                           int iTracerType);
...
void FX_RicochetSound(const Vector &pos) {
  ...
  C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, "FX_RicochetSound.Ricochet",
                          &org);
}
...
//-----------------------------------------------------------------------------
bool FX_GetAttachmentTransform(ClientEntityHandle_t hEntity,
                               int attachmentIndex, Vector *origin,
                               QAngle *angles) {
  // Validate our input
...
//-----------------------------------------------------------------------------
bool FX_GetAttachmentTransform(ClientEntityHandle_t hEntity,
                               int attachmentIndex, matrix3x4_t &transform) {
  Vector origin;
  QAngle angles;
...
// Purpose:
//-----------------------------------------------------------------------------
void FX_MuzzleEffect(const Vector &origin, const QAngle &angles, float scale,
                     ClientEntityHandle_t hEntity, unsigned char *pFlashColor,
                     bool bOneFrame) {
  ...
  for (i = 1; i < 9; i++) {
    ...
    pParticle->m_uchStartSize =
        (random->RandomFloat(6.0f, 9.0f) * (12 - (i)) / 9) * flScale;
...
//			bOneFrame -
//-----------------------------------------------------------------------------
void FX_MuzzleEffectAttached(float scale, ClientEntityHandle_t hEntity,
                             int attachmentIndex, unsigned char *pFlashColor,
                             bool bOneFrame) {
  VPROF_BUDGET("FX_MuzzleEffect", VPROF_BUDGETGROUP_PARTICLE_RENDERING);

  CSmartPtr<CLocalSpaceEmitter> pSimple =
      CLocalSpaceEmitter::Create("MuzzleFlash", hEntity, attachmentIndex);
  Assert(pSimple);
  if (pSimple == NULL) return;

  // Lock our bounding box
  pSimple->GetBinding().SetBBox(-(Vector(16, 16, 16) * scale),
                                (Vector(16, 16, 16) * scale));
  ...
  for (i = 1; i < 9; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_SMG_Muzzleflash[random->RandomInt(0, 3)],
        offset);
  ...
  if (!clienttools->IsInRecordingMode()) return;
  ...
  KeyValues *pPosition =
      pInitializers->FindKey("DmeLinearAttachedPositionInitializer", true);
  ...
  KeyValues *pVelocity =
      pInitializers->FindKey("DmeConstantVelocityInitializer", true);
  ...
  KeyValues *pColor =
      pInitializers->FindKey("DmeRandomInterpolatedColorInitializer", true);
  Color color(pFlashColor ? pFlashColor[0] : 255,
              pFlashColor ? pFlashColor[1] : 255,
              pFlashColor ? pFlashColor[2] : 255, 255);
  ...
  // size = rand(6..9) * indexed(12/9..4/9) * flScale = rand(6..9) * ( 4f + f *
  // i )
  KeyValues *pSize =
      pInitializers->FindKey("DmeMuzzleFlashSizeInitializer", true);
...
void MuzzleFlashCallback(const CEffectData &data) {
  ...
  tempents->MuzzleFlash(
      vecOrigin, vecAngles, data.m_fFlags & (~MUZZLEFLASH_FIRSTPERSON),
      data.m_hEntity, (data.m_fFlags & MUZZLEFLASH_FIRSTPERSON) != 0);
}
...
//			flRollDelta -
//-----------------------------------------------------------------------------
CSmartPtr<CSimpleEmitter> FX_Smoke(const Vector &origin, const Vector &velocity,
                                   float scale, int numParticles,
                                   float flDietime, unsigned char *pColor,
                                   int iAlpha, const char *pMaterial,
                                   float flRoll, float flRollDelta) {
...
// Purpose: Smoke puffs
//-----------------------------------------------------------------------------
void FX_Smoke(const Vector &origin, const QAngle &angles, float scale,
              int numParticles, unsigned char *pColor, int iAlpha) {
  VPROF_BUDGET("FX_Smoke", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  Vector vecVelocity;
  Vector vecForward;
  ...
  for (int i = 0; i < numParticles; i++) {
    ...
    FX_Smoke(origin, vecVelocity, iSize, 1, random->RandomFloat(0.5f, 1.0f),
             particlecolor, alpha, "particle/particle_smokegrenade", flRoll,
             flRollDelta);
  }
}
...
//			This is an emitter that keeps spitting out particles for its
//lifetime 			It won't create particles if it's not in view, so it's best when
//short lived
//-----------------------------------------------------------------------------
class CSmokeEmitter : public CSimpleEmitter {
 ...
 public:
  CSmokeEmitter(ClientEntityHandle_t hEntity, int nAttachment,
  ...
  // Create
  static CSmokeEmitter *Create(ClientEntityHandle_t hEntity, int nAttachment,
                               const char *pDebugName = "smoke") {
  ...
  void SetSpawnRate(float flRate) { m_flSpawnRate = flRate; }

  void CreateSpurtParticles(void) {
    SimpleParticle *pParticle;
    // PMaterialHandle hMaterial = GetPMaterial(
    // "particle/particle_smokegrenade" );

    Vector vecOrigin = m_vSortOrigin;
    IClientRenderable *pRenderable =
        ClientEntityList().GetClientRenderableFromHandle(m_hEntity);
    ...
    for (int i = 0; i < numParticles; i++) {
      pParticle = (SimpleParticle *)AddParticle(sizeof(SimpleParticle),
                                                g_Mat_DustPuff[0], vecOrigin);
      ...
      int color[3][2];
      for (int i = 0; i < 3; ++i) {
        color[i][0] = std::max(0.0f, m_SpurtColor[i] - 64);
        color[i][1] = std::min(255.0f, m_SpurtColor[i] + 64);
      }
...
//-----------------------------------------------------------------------------
void FX_BuildSmoke(Vector &vecOrigin, QAngle &vecAngles,
                   ClientEntityHandle_t hEntity, int nAttachment,
                   float flLifeTime, const Vector4D &pColor) {
  CSmartPtr<CSmokeEmitter> pSimple =
      CSmokeEmitter::Create(hEntity, nAttachment, "FX_Smoke");
...
void SmokeCallback(const CEffectData &data) {
  ...
  FX_BuildSmoke(vecOrigin, vecAngles, data.m_hEntity, data.m_nAttachmentIndex,
                100.0, color);
}
...
//-----------------------------------------------------------------------------
void FX_GunshipImpact(const Vector &pos, const Vector &normal, float r, float g,
                      float b) {
  ...
  if (CImpactOverlay *pOverlay = new CImpactOverlay) {
    ...
    VectorMA(
        pos, 1.0f, normal,
        pOverlay
            ->m_vPos);  // Doesn't show up on terrain if you don't do this(sjb)
...
void CommandPointerCallback(const CEffectData &data) {
  ...
  for (int i = 0; i < size; i++) {
    if (commandercolors[i].index == data.m_nColor) {
      FX_GunshipImpact(data.m_vOrigin, Vector(0, 0, 1), commandercolors[i].r,
                       commandercolors[i].g, commandercolors[i].b);
      return;
    }
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void FX_GunshipMuzzleEffect(const Vector &origin, const QAngle &angles,
                            float scale, ClientEntityHandle_t hEntity,
  ...
  pParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), pSimple->GetPMaterial("effects/gunshipmuzzle"),
      offset);
...
void FX_GunshipTracer(Vector &start, Vector &end, int velocity, bool makeWhiz) {
  ...
  float life =
      (totalDist + length) /
      velocity;  // NOTENOTE: We want the tail to finish its run as well

  // Add it
  FX_AddDiscreetLine(start, shotDir, velocity, length, totalDist, 5.0f, life,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void FX_StriderMuzzleEffect(const Vector &origin, const QAngle &angles,
                            float scale, ClientEntityHandle_t hEntity,
                            unsigned char *pFlashColor) {
  Vector vecDir;
  ...
  for (int i = 0; i < 5; i++) {
    FX_AddDiscreetLine(origin, vecDir, speed, 32, speed * life, 5.0f, life,
                       "effects/bluespark");
    speed *= 1.5f;
  }
}
...
void FX_StriderTracer(Vector &start, Vector &end, int velocity, bool makeWhiz) {
  ...
  // Add it
  FX_AddDiscreetLine(start, shotDir, velocity, length, totalDist, 2.5f, life,
                     "effects/gunshiptracer");
...
void FX_HunterTracer(Vector &start, Vector &end, int velocity, bool makeWhiz) {
  ...
  // float flMinLength = std::min( totalDist, 128.0f );
  // float flMaxLength = std::min( totalDist, 128.0f );
  ...
  // Add it
  FX_AddDiscreetLine(start, shotDir, velocity * 0.5f, length, totalDist, 2.0f,
                     life, "effects/huntertracer");
...
void FX_GaussTracer(Vector &start, Vector &end, int velocity, bool makeWhiz) {
  ...
  // Add it
  FX_AddDiscreetLine(start, shotDir, velocity, length, totalDist,
                     random->RandomFloat(5.0f, 8.0f), life, "effects/spark");
}

//-----------------------------------------------------------------------------
// Purpose: Create a tesla effect between two points
//-----------------------------------------------------------------------------
void FX_BuildTesla(C_BaseEntity *pEntity, const Vector &vecOrigin,
                   const Vector &vecEnd, const char *pModelName,
                   float flBeamWidth, const Vector &vColor, int nFlags,
                   float flTimeVisible) {
  BeamInfo_t beamInfo;
...
void FX_Tesla(const CTeslaInfo &teslaInfo) {
  ...
  int iNumBeamsAround = (teslaInfo.m_nBeams * 2) /
                        3;  // (2/3 of the beams are placed around in a circle)
  ...
  for (int i = 0; i < iTotalBeams; i++) {
    ...
    do {
      ...
      UTIL_TraceLine(teslaInfo.m_vPos,
                     teslaInfo.m_vPos + (vecForward * teslaInfo.m_flRadius),
                     MASK_SHOT, pEntity, COLLISION_GROUP_NONE, &tr);
    ...
    if (tr.fraction < 1.0) {
      if (!EffectOccluded(tr.endpos, 0)) {
        ...
        pParticle = (SimpleParticle *)pSimple->AddParticle(
            sizeof(SimpleParticle),
            pSimple->GetPMaterial("effects/tesla_glow_noz"), vecFlash);
        if (pParticle != NULL) {
          pParticle->m_flLifetime = 0.0f;
          ...
          pParticle->m_uchColor[0] =
              std::min(1.0f, color[0] * colorRamp) * 255.0f;
          pParticle->m_uchColor[1] =
              std::min(1.0f, color[1] * colorRamp) * 255.0f;
          pParticle->m_uchColor[2] =
              std::min(1.0f, color[2] * colorRamp) * 255.0f;
    ...
    // Build the tesla
    FX_BuildTesla(pEntity, teslaInfo.m_vPos, tr.endpos,
                  teslaInfo.m_pszSpriteName, teslaInfo.m_flBeamWidth,
                  teslaInfo.m_vColor, FBEAM_ONLYNOISEONCE,
...
//-----------------------------------------------------------------------------
void FX_BuildTeslaHitbox(C_BaseEntity *pEntity, int nStartAttachment,
                         int nEndAttachment, float flBeamWidth,
                         const Vector &vColor, float flTimeVisible) {
  // One along the body
  BeamInfo_t beamInfo;
  beamInfo.m_nType = TE_BEAMTESLA;
  ...
  UTIL_TraceLine(pEntity->WorldSpaceCenter(),
                 pEntity->WorldSpaceCenter() + (randomDir * 100),
                 MASK_SOLID_BRUSHONLY, pEntity, COLLISION_GROUP_NONE, &tr);
  ...
  if (pEntity != NULL) {
    dlight_t *el =
        effects->CL_AllocElight(LIGHT_INDEX_TE_DYNAMIC + pEntity->entindex());
...
void FX_BuildTeslaHitbox(const CEffectData &data) {
  ...
  if (!pAnimating->HitboxToWorldTransforms(hitboxbones)) return;
  ...
  for (int i = 0; i < nBeamCount; ++i) {
    ...
    FX_BuildTeslaHitbox(pEntity, nStartHitBox, nEndHitBox, data.m_flScale,
                        vColor, random->RandomFloat(0.05f, 0.2f));
  }
}
