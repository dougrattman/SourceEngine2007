...
ConVar cl_chatfilters("cl_chatfilters", "31", FCVAR_CLIENTDLL | FCVAR_ARCHIVE,
...
// converts all '\r' characters to '\n', so that the engine can deal with the
// properly returns a pointer to str
...
//-----------------------------------------------------------------------------
// Purpose: Reads a string from the current message and checks if it is
// translatable
//-----------------------------------------------------------------------------
wchar_t *ReadLocalizedString(bf_read &msg, wchar_t *pOut, int outSize,
                             bool bStripNewline, char *originalString,
                             int originalSize) {
  char szString[2048];
  szString[0] = 0;
...
//-----------------------------------------------------------------------------
// Purpose: Reads a string from the current message, converts it to unicode, and
// strips out color codes
//-----------------------------------------------------------------------------
wchar_t *ReadChatTextString(bf_read &msg, wchar_t *pOut, int outSize) {
  ...
  // converts color control characters into control characters for the normal
  // color
...
void CBaseHudChatLine::PerformFadeout(void) {
  ...
  if (curtime >= m_flStartTime &&
      curtime < m_flStartTime + CHATLINE_FLASH_TIME) {
    ...
    InsertString(wbuf);
  } else if (curtime <= m_flExpireTime &&
             curtime > m_flExpireTime - CHATLINE_FADE_TIME) {
...
bool CBaseHudChatLine::IsReadyToExpire(void) {
  ...
  if (!engine->IsInGame() && !engine->IsConnected()) return true;

  if (gpGlobals->curtime >= m_flExpireTime) return true;
  return false;
}
...
void CBaseHudChatInputLine::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_pInput->SetFgColor(pScheme->GetColor(
      "Chat.TypingText",
      pScheme->GetColor("Panel.FgColor", Color(255, 255, 255, 255))));
...
void CBaseHudChatInputLine::GetMessageText(wchar_t *buffer,
                                           int buffersizebytes) {
...
CHudChatFilterCheckButton::CHudChatFilterCheckButton(vgui::Panel *pParent,
                                                     const char *pName,
                                                     const char *pText,
                                                     int iFlag)
    : BaseClass(pParent, pName, pText) {
...
CHudChatFilterPanel::CHudChatFilterPanel(vgui::Panel *pParent,
                                         const char *pName)
    : BaseClass(pParent, pName) {
  SetParent(pParent);

  new CHudChatFilterCheckButton(this, "joinleave_button", "Sky is blue?",
                                CHAT_FILTER_JOINLEAVE);
  new CHudChatFilterCheckButton(this, "namechange_button", "Sky is blue?",
                                CHAT_FILTER_NAMECHANGE);
  new CHudChatFilterCheckButton(this, "publicchat_button", "Sky is blue?",
                                CHAT_FILTER_PUBLICCHAT);
  new CHudChatFilterCheckButton(this, "servermsg_button", "Sky is blue?",
                                CHAT_FILTER_SERVERMSG);
  new CHudChatFilterCheckButton(this, "teamchange_button", "Sky is blue?",
                                CHAT_FILTER_TEAMCHANGE);
}
...
void CHudChatFilterPanel::OnFilterButtonChecked(vgui::Panel *panel) {
  CHudChatFilterCheckButton *pButton =
      dynamic_cast<CHudChatFilterCheckButton *>(panel);

  if (pButton && GetChatParent() && IsVisible()) {
    if (pButton->IsSelected()) {
      GetChatParent()->SetFilterFlag(GetChatParent()->GetFilterFlags() |
                                     pButton->GetFilterFlag());
    } else {
      GetChatParent()->SetFilterFlag(GetChatParent()->GetFilterFlags() &
                                     ~pButton->GetFilterFlag());
    }
...
void CHudChatFilterPanel::SetVisible(bool state) {
  if (state == true) {
    for (int i = 0; i < GetChildCount(); i++) {
      CHudChatFilterCheckButton *pButton =
          dynamic_cast<CHudChatFilterCheckButton *>(GetChild(i));
...
CHudChatHistory::CHudChatHistory(vgui::Panel *pParent, const char *panelName)
  ...
  vgui::HScheme scheme = vgui::scheme()->LoadSchemeFromFileEx(
      enginevgui->GetPanel(PANEL_CLIENTDLL), "resource/ChatScheme.res",
      "ChatScheme");
  SetScheme(scheme);
...
CBaseHudChat::CBaseHudChat(const char *pElementName)
  ...
  m_pFiltersButton =
      new CHudChatFilterButton(this, "ChatFiltersButton", "Filters");
...
void CBaseHudChat::MsgFunc_SayText2(bf_read &msg) {
  ...
  wchar_t *msg_text =
      ReadLocalizedString(msg, szBuf[0], sizeof(szBuf[0]), false,
                          untranslated_msg_text, sizeof(untranslated_msg_text));

  // keep reading strings and using C format strings for subsituting the strings
  // into the localised text string
  ...
  g_pVGuiLocalize->ConstructString(szBuf[5], sizeof(szBuf[5]), msg_text, 4,
                                   szBuf[1], szBuf[2], szBuf[3], szBuf[4]);
  ...
  if (bWantsToChat) {
    ...
    if (client > 0 &&
        (g_PR->GetTeam(client) != g_PR->GetTeam(GetLocalPlayerIndex()))) {
    ...
    // print raw chat text
    ChatPrintf(client, GetFilterForString(untranslated_msg_text), "%s",
               ansiString);
  }
}
...
// displays a string, looking them up from the titles.txt file, which can be
// localised parameters:
//   byte:   message direction  ( HUD_PRINTCONSOLE, HUD_PRINTNOTIFY,
//   HUD_PRINTCENTER, HUD_PRINTTALK ) string: message
// optional parameters:
//   string: message parameter 1
//   string: message parameter 2
//   string: message parameter 3
//   string: message parameter 4
// any string that starts with the character '#' is a message name, and is used
// to look up the real message in titles.txt the next (optional) one to four
// strings are parameters for that string (which can also be message names if
// they begin with '#')
//-----------------------------------------------------------------------------
void CBaseHudChat::MsgFunc_TextMsg(bf_read &msg) {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    if (pBuf) {
      ...
      if (i) {
        StripEndNewlineFromString(tmpStr);  // these strings are meant for
                                            // subsitution into the main
                                            // strings, so cull the automatic
                                            // end newlines
      }
  ...
  switch (msg_dest) {
    case HUD_PRINTCENTER:
      g_pVGuiLocalize->ConstructString(outputBuf, sizeof(outputBuf), szBuf[0],
                                       4, szBuf[1], szBuf[2], szBuf[3],
                                       szBuf[4]);
...
void CBaseHudChat::MsgFunc_VoiceSubtitle(bf_read &msg) {
  ...
  ChatPrintf(client, CHAT_FILTER_NONE, "%c(%s) %s%c: %s", COLOR_PLAYERNAME,
             szPrefix, GetDisplayedSubtitlePlayerName(client), COLOR_NORMAL,
...
void CBaseHudChat::OnTick(void) {
  ...
  if (line) {
    ...
    m_pChatInput->SetBounds(iInputX, iChatH - (m_iFontHeight * 1.75), iInputW,
                            m_iFontHeight);

    // Resize the History Panel so it fits more lines depending on the screen
    // resolution.
    int iChatHistoryX, iChatHistoryY, iChatHistoryW, iChatHistoryH;

    GetChatHistory()->GetBounds(iChatHistoryX, iChatHistoryY, iChatHistoryW,
                                iChatHistoryH);

    iChatHistoryH = (iChatH - (m_iFontHeight * 2.25)) - iChatHistoryY;

    GetChatHistory()->SetBounds(iChatHistoryX, iChatHistoryY, iChatHistoryW,
                                iChatHistoryH);
  }
  ...
  if (alpha >= 0) {
    if (GetChatHistory()) {
      if (IsMouseInputEnabled()) {
        SetAlpha(255);
        GetChatHistory()->SetBgColor(
            Color(0, 0, 0, CHAT_HISTORY_ALPHA - alpha));
        m_pChatInput->GetPrompt()->SetAlpha((CHAT_HISTORY_ALPHA * 2) - alpha);
        m_pChatInput->GetInputPanel()->SetAlpha((CHAT_HISTORY_ALPHA * 2) -
                                                alpha);
        SetBgColor(Color(GetBgColor().r(), GetBgColor().g(), GetBgColor().b(),
                         CHAT_HISTORY_ALPHA - alpha));
        m_pFiltersButton->SetAlpha((CHAT_HISTORY_ALPHA * 2) - alpha);
      } else {
        GetChatHistory()->SetBgColor(Color(0, 0, 0, alpha));
        SetBgColor(
            Color(GetBgColor().r(), GetBgColor().g(), GetBgColor().b(), alpha));
    ...
    while (txt && *txt) {
      ...
      switch (*txt) {
        ...
        case COLOR_NORMAL: {
          ...
          range.color =
              pChat->GetTextColorForClient((TextColor)(*txt), clientIndex);
          range.end = lineLen;
  ...
  for (int i = 0; i < m_textRanges.Count(); ++i) {
    ...
    if (len > 1) {
      ...
      if (pChat && pChat->GetChatHistory()) {
        ...
        pChat->GetChatHistory()->InsertFade(hud_saytext_time.GetFloat(),
...
void CBaseHudChat::Send(void) {
  ...
  /*
This is a very long string that I am going to attempt to paste into the cs hud
chat entry and we will see if it gets cropped or not.
  */
  ...
  if (len > 0) {
    ...
    Q_snprintf(szbuf, sizeof(szbuf), "%s \"%s\"",
               m_nMessageMode == MM_SAY ? "say" : "say_team", ansi);
...
// Input  : *newmap -
//-----------------------------------------------------------------------------
void CBaseHudChat::LevelInit(const char *newmap) { Clear(); }
...
void CBaseHudChat::ChatPrintf(int iPlayerIndex, int iFilter, const char *fmt,
  ...
  // Strip leading \n characters ( or notify/color signifiers ) for empty string
  // check
  char *pmsg = msg;
  ...
  if (iFilter != CHAT_FILTER_NONE) {
    if (!(iFilter & m_iFilterFlags)) return;
  }
  ...
  if (wbuf) {
    ...
    // find the player's name in the unicode string, in case there is no color
    // markup
