...
void CStunEffect::Init(void) {
  ...
  m_EffectMaterial.Init("__stuneffect", TEXTURE_GROUP_CLIENT_EFFECTS,
                        pVMTKeyValues);
...
void CStunEffect::Render(int x, int y, int w, int h) {
  ...
  if (m_bUpdateView) {
    ...
    srcRect.width = w;
    srcRect.height = h;
    pRenderContext->CopyRenderTargetToTextureEx(m_StunTexture, 0, &srcRect,
                                                NULL);
    m_bUpdateView = false;
  }
  ...
  float viewOffs = (flEffectPerc * 32.0f) * (cos(gpGlobals->curtime * 40.0f) *
                                             sin(gpGlobals->curtime * 17.0f));
  float vX = x + viewOffs;
  ...
  if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 80) {
    if (g_pMaterialSystemHardwareConfig->GetHDRType() == HDR_TYPE_NONE) {
      ...
      // This is a stupid fix, but I don't have time to do a cleaner
      // implementation. Since the introblur.vmt material uses unlit generic, it
      // will tone map, so I need to undo the tone mapping using color modulate.
      // The proper fix would be to use a different material type that supports
      // alpha blending but not tone mapping, which I don't think exists.
      // Whatever. This works when the tone mapping scalar is less than 1.0,
      // which it is in the cases it's used in game.
      float flUnTonemap = pow(
          1.0f / pRenderContext->GetToneMappingScaleLinear().x, 1.0f / 2.2f);
    ...
    pRenderContext->DrawScreenSpaceRectangle(
        m_EffectMaterial, 0, 0, w, h, vX, 0,
        (m_StunTexture->GetActualWidth() - 1) + vX,
        (m_StunTexture->GetActualHeight() - 1), m_StunTexture->GetActualWidth(),
...
void CEP1IntroEffect::Init(void) {
  ...
  KeyValues *pVMTKeyValues = new KeyValues("UnlitGeneric");
  pVMTKeyValues->SetString("$basetexture", STUN_TEXTURE);
  m_EffectMaterial.Init("__ep1introeffect", TEXTURE_GROUP_CLIENT_EFFECTS,
                        pVMTKeyValues);
  m_StunTexture.Init(STUN_TEXTURE, TEXTURE_GROUP_CLIENT_EFFECTS);
}
...
void CEP1IntroEffect::Render(int x, int y, int w, int h) {
  if ((m_flFinishTime == 0) || (IsEnabled() == false)) return;
  ...
  if (g_pMaterialSystemHardwareConfig->UsesSRGBCorrectBlending()) {
    // For DX10 cards, alpha blending happens in linear space, so try to adjust
    // by hacking alpha to 50%
    overlaycolor[3] *= 0.7f;
  }
  ...
  float vX =
      2.0f * -fabs(cosf(gpGlobals->curtime) * cosf(gpGlobals->curtime * 6.0));
  float vY = 2.0f * cosf(gpGlobals->curtime) * cosf(gpGlobals->curtime * 5.0);

  // Scale percentage
  float flScalePerc = 0.02f + (0.01f * cosf(gpGlobals->curtime * 2.0f) *
                               cosf(gpGlobals->curtime * 0.5f));
  ...
  pRenderContext->DrawScreenSpaceRectangle(
      m_EffectMaterial, vX, vY, w, h, flU1, flV1, flU2, flV2,
...
void CEP2StunEffect::Init(void) {
  ...
  KeyValues *pVMTKeyValues = new KeyValues("UnlitGeneric");
  pVMTKeyValues->SetString("$basetexture", STUN_TEXTURE);
  m_EffectMaterial.Init("__ep2stuneffect", TEXTURE_GROUP_CLIENT_EFFECTS,
                        pVMTKeyValues);
  m_StunTexture.Init(STUN_TEXTURE, TEXTURE_GROUP_CLIENT_EFFECTS);
}
...
void CEP2StunEffect::Render(int x, int y, int w, int h) {
  ...
  // Scale percentage
  float flScalePerc = flBaseScale + (0.01f * cosf(gpGlobals->curtime * 2.0f) *
                                     cosf(gpGlobals->curtime * 0.5f));
