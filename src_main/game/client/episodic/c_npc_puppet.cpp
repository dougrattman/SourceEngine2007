...
//-----------------------------------------------------------------------------
void C_NPC_Puppet::BuildTransformations(CStudioHdr *pStudioHdr, Vector *pos,
                                        Quaternion q[],
                                        const matrix3x4_t &cameraTransform,
                                        int boneMask,
  ...
  // Call the baseclass
  BaseClass::BuildTransformations(pStudioHdr, pos, q, cameraTransform, boneMask,
                                  boneComputed);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_NPC_Puppet::AccumulateLayers(CStudioHdr *hdr, Vector pos[],
                                    Quaternion q[], float poseparam[],
                                    float currentTime, int boneMask) {
  if (m_hAnimationTarget == NULL) return;

  C_BaseAnimatingOverlay *pTarget = dynamic_cast<C_BaseAnimatingOverlay *>(
      m_hAnimationTarget->GetBaseAnimating());
  if (pTarget == NULL) return;
    ...
    if (i < pTarget->m_AnimOverlay.Count()) {
      ...
      if (fWeight > 0) {
        const char *pSequenceName =
            pTarget->GetSequenceName(pTarget->m_AnimOverlay[i].m_nSequence);
        ...
        if (nSequence >= 0) {
          ...
          AccumulatePose(hdr, NULL, pos, q, nSequence, fCycle, poseparam,
                         boneMask, fWeight, currentTime);

#if _DEBUG
          if (Q_stristr(hdr->pszName(), r_sequence_debug.GetString()) != NULL) {
            DevMsgRT("%6.2f : %30s : %5.3f : %4.2f : %1d\n", currentTime,
                     hdr->pSeqdesc(nSequence).pszLabel(), fCycle, fWeight, i);
          }
#endif
        }
      }
