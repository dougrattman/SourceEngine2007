...
#define EXTRA_INTERPOLATION_HISTORY_STORED \
  0.05f  // It stores this much extra interpolation history,
...
class CInterpolationContext {
 public:
  ...
  static void EnableExtrapolation(bool state) { s_bAllowExtrapolation = state; }

  static bool IsThereAContext() { return s_pHead != NULL; }

  static bool IsExtrapolationAllowed() { return s_bAllowExtrapolation; }

  static void SetLastTimeStamp(float timestamp) {
    s_flLastTimeStamp = timestamp;
  }

  static float GetLastTimeStamp() { return s_flLastTimeStamp; }
...
inline Vector ExtrapolateInterpolatedVarType(const Vector &oldVal,
                                             const Vector &newVal,
                                             float divisor,
                                             float flExtrapolationAmount) {
...
inline QAngle ExtrapolateInterpolatedVarType(const QAngle &oldVal,
                                             const QAngle &newVal,
                                             float divisor,
                                             float flExtrapolationAmount) {
...
struct CInterpolatedVarEntryBase<Type, false> {
  ...
  void Init(int maxCount) { Assert(maxCount == 1); }
  Type *NewEntry(const Type *pValue, int maxCount, float time) {
    Assert(maxCount == 1);
    changetime = time;
    memcpy(&value, pValue, maxCount * sizeof(Type));
  ...
  void FastTransferFrom(CInterpolatedVarEntryBase &src) { *this = src; }
...
class CInterpolatedVarArrayBase : public IInterpolatedVar {
 ...
 public:
  ...
  int GetNext(int i) {
    int next = i + 1;
    if (!m_VarHistory.IsValidIndex(next)) return m_VarHistory.InvalidIndex();
    return next;
  }
...
template <typename Type, bool IS_ARRAY>
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::Setup(void *pValue,
                                                             int type) {
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::NoteChanged(
  ...
  // effects.
  RemoveEntriesPreviousTo(gpGlobals->curtime - interpolation_amount -
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::AddToHead(
  ...
  if (bFlushNewer) {
    ...
    for (int i = 1; i < m_VarHistory.Count(); i++) {
      if (m_VarHistory[i].changetime <= changeTime) break;
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::RemoveOldEntries(
  ...
  for (int i = m_VarHistory.Count(); --i > 2;) {
    if (m_VarHistory[i].changetime > oldesttime) break;
    newCount = i;
  }
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(
  ...
  for (int i = 0; i < varHistory.Count(); i++) {
    ...
    if (pInfo->newer == varHistory.InvalidIndex()) {
      ...
      if (pNoMoreChanges) *pNoMoreChanges = 1;
      return true;
    }
...
inline bool CInterpolatedVarArrayBase<Type, IS_ARRAY>::GetInterpolationInfo(
  ...
  if (pNewer) *pNewer = (int)info.newer;

  if (pOlder) *pOlder = (int)info.older;

  if (pOldest) *pOldest = (int)info.oldest;
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate(
  ...
  // Note that QAngle has a specialization that will do quaternion interpolation
  // here...
  for (int i = 0; i < m_nMaxCount; i++) {
    if (m_bLooping[i]) {
...
inline void CInterpolatedVarArrayBase<Type, IS_ARRAY>::_Interpolate_Hermite(
  ...
  for (int i = 0; i < m_nMaxCount; i++) {
    // Note that QAngle has a specialization that will do quaternion
    // interpolation here...
    if (m_bLooping[i]) {
