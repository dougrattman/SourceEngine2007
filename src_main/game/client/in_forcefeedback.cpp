...
static ConVar ff_autocenter("ff_autocenter", "1", FCVAR_ARCHIVE,
                            "Enable auto-centering of forcefeedback joystick.");
...
static void DescribeFFDevice(const DIDEVCAPS &caps) {
  ...
  ReportCap(caps.dwFlags, DIDC_POLLEDDATAFORMAT,
            "  device using polled data format");
  ReportCap(caps.dwFlags, DIDC_POLLEDDEVICE, "  device is polled");
  ReportCap(caps.dwFlags, DIDC_POSNEGCOEFFICIENTS,
            "  device supports two coefficient values for conditions");
...

{
  ...
  HRESULT hr;
  hr = ctx->device->CreateEffect(lpdife->GuidEffect, lpdife->lpDiEffect,
                                 &(*vecPtr)[idx], NULL);
...
static void LoadEffectFile(LPDIRECTINPUTDEVICE8 device, EffectMap_t &map) {
  ...
  HRESULT hr = device->EnumEffectsInFile(
      fullpath, EnumEffectsInFileProc, (LPVOID)&context, DIFEF_MODIFYIFNEEDED);
...
void CInput::Init_ForceFeedback() {
  ...
  hr =
      m_pFF->m_pIInput->CreateDevice(GUID_Joystick, &m_pFF->m_pIJoystick, NULL);
  if (FAILED(hr)) {
    return;
  }
  ...
  hr = m_pFF->m_pIJoystick->SetCooperativeLevel(
      mainWnd, DISCL_BACKGROUND | DISCL_EXCLUSIVE);
...
void CInput::ForceFeedback_Reaquire() {
  if (!m_pFF || !m_pFF->m_bForceFeedbackAvailable) return;
...
void CInput::ForceFeedback_Start(int effectnum, const FFBaseParams_t &params) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    if (!FAILED(hr)) {
      // If params.m_flDuration == 0.0f then that means use the duration in the
      // file
