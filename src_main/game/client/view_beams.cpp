...
// Input  : state -
//-----------------------------------------------------------------------------
void SetBeamCreationAllowed(bool state) { g_BeamCreationAllowed = state; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool BeamCreationAllowed(void) { return g_BeamCreationAllowed; }
...
class CViewRenderBeams : public IViewRenderBeams {
 ...
 public:
  ...
  virtual void CreateBeamEnts(int startEnt, int endEnt, int modelIndex,
                              int haloIndex, float haloScale, float life,
                              float width, float endWidth, float fadeLength,
                              float amplitude, float brightness, float speed,
                              int startFrame, float framerate, float r, float g,
                              float b, int type = -1);
  virtual void CreateBeamEntPoint(int nStartEntity, const Vector *pStart,
                                  int nEndEntity, const Vector *pEnd,
                                  int modelIndex, int haloIndex,
                                  float haloScale, float life, float width,
                                  float endWidth, float fadeLength,
                                  float amplitude, float brightness,
                                  float speed, int startFrame, float framerate,
                                  float r, float g, float b);
  virtual void CreateBeamPoints(Vector &start, Vector &end, int modelIndex,
                                int haloIndex, float haloScale, float life,
                                float width, float endWidth, float fadeLength,
                                float amplitude, float brightness, float speed,
                                int startFrame, float framerate, float r,
                                float g, float b);
  virtual void CreateBeamRing(int startEnt, int endEnt, int modelIndex,
                              int haloIndex, float haloScale, float life,
                              float width, float endWidth, float fadeLength,
                              float amplitude, float brightness, float speed,
                              int startFrame, float framerate, float r, float g,
                              float b, int flags);
  virtual void CreateBeamRingPoint(const Vector &center, float start_radius,
                                   float end_radius, int modelIndex,
                                   int haloIndex, float haloScale, float life,
                                   float width, float m_nEndWidth,
                                   float m_nFadeLength, float amplitude,
                                   float brightness, float speed,
                                   int startFrame, float framerate, float r,
                                   float g, float b, int flags);
  virtual void CreateBeamCirclePoints(
      int type, Vector &start, Vector &end, int modelIndex, int haloIndex,
      float haloScale, float life, float width, float endWidth,
      float fadeLength, float amplitude, float brightness, float speed,
      int startFrame, float framerate, float r, float g, float b);
  virtual void CreateBeamFollow(int startEnt, int modelIndex, int haloIndex,
                                float haloScale, float life, float width,
                                float endWidth, float fadeLength, float r,
                                float g, float b, float brightness);
 ...
 private:
  ...
  void DrawBeamWithHalo(Beam_t *pbeam, int frame, int rendermode, float *color,
                        float *srcColor, const model_t *sprite,
                        const model_t *halosprite, float flHDRColorScale);
  void DrawBeamFollow(const model_t *pSprite, Beam_t *pbeam, int frame,
                      int rendermode, float frametime, const float *color,
                      float flHDRColorScale = 1.0f);
  void DrawLaser(Beam_t *pBeam, int frame, int rendermode, float *color,
                 model_t const *sprite, model_t const *halosprite,
                 float flHDRColorScale = 1.0f);
  void DrawTesla(Beam_t *pBeam, int frame, int rendermode, float *color,
                 model_t const *sprite, float flHDRColorScale = 1.0f);
...
static void Noise(float *noise, int divs, float scale) {
  ...
  noise[div2] =
      (noise[0] + noise[divs]) * 0.5 + scale * beamRandom.RandomFloat(-1, 1);
  if (div2 > 1) {
...
bool ComputeBeamEntPosition(C_BaseEntity *pEnt, int nAttachment,
                            bool bInterpretAttachmentIndexAsHitboxIndex,
                            Vector &pt) {
  // NOTE: This will *leave* the pt at its current value, essential for
  // beam follow ents what want to stick around a little after their ent has
  // died
  if (!pEnt) return false;

  if (!bInterpretAttachmentIndexAsHitboxIndex) {
    QAngle angles;
    if (pEnt->GetAttachment(nAttachment, pt, angles)) return true;
  } else {
    C_BaseAnimating *pAnimating = pEnt->GetBaseAnimating();
    if (pAnimating) {
      studiohdr_t *pStudioHdr =
          modelinfo->GetStudiomodel(pAnimating->GetModel());
      if (pStudioHdr) {
        mstudiohitboxset_t *set =
            pStudioHdr->pHitboxSet(pAnimating->GetHitboxSet());
        if (set && (set->numhitboxes >= nAttachment) && (nAttachment > 0)) {
          ...
          if (pAnimating->HitboxToWorldTransforms(hitboxbones)) {
            ...
            VectorITransform(vecViewPt, *hitboxbones[pHitbox->bone],
                             vecLocalViewPt);

            Vector vecLocalClosestPt;
            CalcClosestPointOnAABB(pHitbox->bbmin, pHitbox->bbmax,
                                   vecLocalViewPt, vecLocalClosestPt);
...
const QAngle &Beam_t::GetRenderAngles(void) { return vec3_angle; }
...
bool Beam_t::ShouldDraw(void) { return true; }

bool Beam_t::IsTransparent(void) { return true; }

void Beam_t::ComputeFxBlend() {
  // Do nothing, they're always 255
}

int Beam_t::GetFxBlend() { return 255; }
...
CViewRenderBeams::~CViewRenderBeams(void) { ClearBeams(); }
...
void CViewRenderBeams::InitBeams(void) {
  ...
  if (p >= 0) {
    m_nNumBeamTrails = std::max(p, (int)MIN_PARTICLES);
  } else {
...
//-----------------------------------------------------------------------------
// Purpose: Iterates through active list and kills beams associated with
// deadEntity Input  : deadEntity -
//-----------------------------------------------------------------------------
...
void CViewRenderBeams::SetupBeam(Beam_t *pBeam, const BeamInfo_t &beamInfo) {
  ...
  if (beamInfo.m_nSegments == -1) {
    if (pBeam->amplitude >= 0.50) {
      pBeam->segments =
          VectorLength(pBeam->delta) * 0.25 + 3;  // one per 4 pixels
    } else {
      pBeam->segments =
          VectorLength(pBeam->delta) * 0.075 + 3;  // one per 16 pixels
    }
  } else {
...
//          beamInfo -
//-----------------------------------------------------------------------------
void CViewRenderBeams::SetBeamAttributes(Beam_t *pBeam,
...
int CViewRenderBeams::CullBeam(const Vector &start, const Vector &end,
                               int pvsOnly) {
  Vector mins, maxs;
  int i;
...
Beam_t *CViewRenderBeams::CreateGenericBeam(BeamInfo_t &beamInfo) {
  ...
  // Need a valid model.
  if (beamInfo.m_nModelIndex < 0) return NULL;
...
void CViewRenderBeams::CreateBeamEnts(int startEnt, int endEnt, int modelIndex,
                                      ...
                                      float life, float width, float endWidth,
                                      float fadeLength, float amplitude,
                                      float brightness, float speed,
                                      int startFrame, float framerate, float r,
                                      float g, float b, int type) {
  BeamInfo_t beamInfo;
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamEntPoint(
    int nStartEntity, const Vector *pStart, int nEndEntity, const Vector *pEnd,
    int modelIndex, int haloIndex, float haloScale, float life, float width,
    float endWidth, float fadeLength, float amplitude, float brightness,
  ...
  if (nStartEntity <= 0) {
    ...
    if (!beamInfo.m_pStartEnt) return;
  }
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamPoints(
    Vector &start, Vector &end, int modelIndex, int haloIndex, float haloScale,
...
Beam_t *CViewRenderBeams::CreateBeamPoints(BeamInfo_t &beamInfo) {
  // Don't start temporary beams out of the PVS
  if (beamInfo.m_flLife != 0 &&
      !CullBeam(beamInfo.m_vecStart, beamInfo.m_vecEnd, 1))
    return NULL;
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamCirclePoints(
    int type, Vector &start, Vector &end, int modelIndex, int haloIndex,
    float haloScale, float life, float width, float endWidth, float fadeLength,
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamFollow(int startEnt, int modelIndex,
                                        int haloIndex, float haloScale,
                                        float life, float width, float endWidth,
                                        float fadeLength, float r, float g,
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamRingPoint(
    const Vector &center, float start_radius, float end_radius, int modelIndex,
    int haloIndex, float haloScale, float life, float width, float endWidth,
    float fadeLength, float amplitude, float brightness, float speed,
    int startFrame, float framerate, float r, float g, float b, int nFlags) {
  BeamInfo_t beamInfo;
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamRing(int startEnt, int endEnt, int modelIndex,
                                      int haloIndex, float haloScale,
                                      float life, float width, float endWidth,
                                      float fadeLength, float amplitude,
                                      float brightness, float speed,
...
void CViewRenderBeams::UpdateBeam(Beam_t *pbeam, float frametime) {
  ...
  // if we are paused, force random numbers used by noise to generate the same
  // value every frame
  if (frametime == 0.0f) {
    beamRandom.SetSeed((int)gpGlobals->curtime);
  }

  // If FBEAM_ONLYNOISEONCE is set, we don't want to move once we've first
  // calculated noise
  ...
  if (pbeam->flags & (FBEAM_STARTENTITY | FBEAM_ENDENTITY)) {
    ...
    if (!RecomputeBeamEndpoints(pbeam)) return;
  ...
  switch (pbeam->type) {
    case TE_BEAMSPLINE: {
      ...
      for (int i = 1; i < pbeam->numAttachments; i++) {
        if (!ComputeBeamEntPosition(pbeam->entity[i], pbeam->attachmentIndex[i],
                                    (pbeam->flags & FBEAM_USE_HITBOXES) != 0,
                                    pbeam->attachment[i])) {
          // This should never happen, but if for some reason the attachment
          // doesn't exist, as a safety measure copy in the location of the
          // previous attachment point (rather than bailing)
    ...
    case TE_BEAMRINGPOINT: {
      ...
      if (dr != 0.0f) {
        ...
        VectorSubtract(pbeam->attachment[1], pbeam->attachment[0],
                       pbeam->delta);
        if (pbeam->amplitude >= 0.50)
          pbeam->segments =
              VectorLength(pbeam->delta) * 0.25 + 3;  // one per 4 pixels
    ...
    case TE_BEAMPOINTS:
      ...
      if (!CullBeam(pbeam->attachment[0], pbeam->attachment[1], 0)) return;
      break;
  }
...
void CViewRenderBeams::UpdateTempEntBeams(void) {
  VPROF_("UpdateTempEntBeams", 2, VPROF_BUDGETGROUP_CLIENT_SIM, false,
         BUDGETFLAG_CLIENT);
  if (!m_pActiveBeams) return;
  ...
  for (Beam_t *pBeam = m_pActiveBeams; pBeam; pBeam = pNext) {
    ...
    // Retire old beams
    if (!(pBeam->flags & FBEAM_FOREVER) && pBeam->die <= gpGlobals->curtime) {
      // Reset links
      if (pPrev) {
        pPrev->next = pNext;
      } else {
...
void CViewRenderBeams::DrawBeamFollow(const model_t *pSprite, Beam_t *pbeam,
  ...
  // Draw it
  ::DrawBeamFollow(pSprite, pbeam->trail, frame, rendermode, delta, screen,
                   screenLast, pbeam->die, pbeam->attachment[0], pbeam->flags,
                   pbeam->width, pbeam->amplitude, pbeam->freq, (float *)color);
...
// Output  :
//------------------------------------------------------------------------------
void CViewRenderBeams::DrawBeamWithHalo(Beam_t *pbeam, int frame,
                                        int rendermode, float *color,
                                        float *srcColor, const model_t *sprite,
  ...
  CalcClosestPointOnLine(CurrentViewOrigin(), pbeam->attachment[0],
                         pbeam->attachment[0] + (beamDir * 2), out,
                         &distToLine);
  ...
  if (distToLine < distThreshold) {
    ...
    dotScale = std::clamp(dotScale, 0.0f, 1.0f);
  }
  ...
  if (pbeam->flags & FBEAM_HALOBEAM) {
    DrawSegs(NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode,
             ...
             pbeam->flags, scaleColor, pbeam->fadeLength, flHDRColorScale);
  } else {
    // Draw primary beam just shy of its end so it doesn't clip
    DrawSegs(NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode,
             pbeam->attachment[0], pbeam->delta, pbeam->width, pbeam->width,
             pbeam->amplitude, pbeam->freq, pbeam->speed, 2, pbeam->flags,
             scaleColor, pbeam->fadeLength, flHDRColorScale);
  }
  ...
  float haloFractionVisible =
      PixelVisibility_FractionVisible(params, pbeam->m_queryHandleHalo);
  if (fade && haloFractionVisible > 0.0f) {
    // NOTENOTE: This is kinda funky when moving away and to the backside -- jdw
    float haloScale =
        RemapVal(distToLine, distThreshold, pbeam->width * 0.5f, 1.0f, 2.0f);

    haloScale = std::clamp(haloScale, 1.0f, 2.0f);

    haloScale *= pbeam->haloScale;

    float colorFade = fade * fade;
    colorFade = std::clamp(colorFade, 0.0f, 1.0f);
...
//------------------------------------------------------------------------------
void CViewRenderBeams::DrawLaser(Beam_t *pbeam, int frame, int rendermode,
                                 float *color, const model_t *sprite,
                                 const model_t *halosprite,
                                 float flHDRColorScale) {
  float color2[3];
  ...
  DrawSegs(NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode,
           pbeam->attachment[0], pbeam->delta, pbeam->width, pbeam->endWidth,
           pbeam->amplitude, pbeam->freq, pbeam->speed, pbeam->segments,
           pbeam->flags, color2, pbeam->fadeLength);
}

//------------------------------------------------------------------------------
// Purpose : Draw a fibrous tesla beam
//------------------------------------------------------------------------------
void CViewRenderBeams::DrawTesla(Beam_t *pbeam, int frame, int rendermode,
                                 float *color, const model_t *sprite,
                                 float flHDRColorScale) {
  DrawTeslaSegs(NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode,
                pbeam->attachment[0], pbeam->delta, pbeam->width,
                pbeam->endWidth, pbeam->amplitude, pbeam->freq, pbeam->speed,
                pbeam->segments, pbeam->flags, color, pbeam->fadeLength,
                flHDRColorScale);
}
...
void CViewRenderBeams::DrawBeam(Beam_t *pbeam) {
  ...
  int frame = ((int)(pbeam->frame + gpGlobals->curtime * pbeam->frameRate) %
               pbeam->frameCount);
  int rendermode =
      (pbeam->flags & FBEAM_SOLID) ? kRenderNormal : kRenderTransAdd;
  ...
  switch (pbeam->type) {
    ...
    case TE_BEAMPOINTS:
      if (halosprite) {
        DrawBeamWithHalo(pbeam, frame, rendermode, color, srcColor, sprite,
                         halosprite, pbeam->m_flHDRColorScale);
      } else {
        DrawSegs(NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode,
                 pbeam->attachment[0], pbeam->delta, pbeam->width,
                 pbeam->endWidth, pbeam->amplitude, pbeam->freq, pbeam->speed,
                 pbeam->segments, pbeam->flags, color, pbeam->fadeLength,
                 pbeam->m_flHDRColorScale);
      }
      break;

    case TE_BEAMFOLLOW:
      DrawBeamFollow(sprite, pbeam, frame, rendermode, gpGlobals->frametime,
                     color, pbeam->m_flHDRColorScale);
      break;

    case TE_BEAMRING:
    case TE_BEAMRINGPOINT:
      DrawRing(NOISE_DIVISIONS, pbeam->rgNoise, Noise, sprite, frame,
               rendermode, pbeam->attachment[0], pbeam->delta, pbeam->width,
               pbeam->amplitude, pbeam->freq, pbeam->speed, pbeam->segments,
               color, pbeam->m_flHDRColorScale);
      break;
    ...
    case TE_BEAMSPLINE:
      DrawSplineSegs(NOISE_DIVISIONS, pbeam->rgNoise, sprite, halosprite,
                     ...
                     pbeam->attachment, pbeam->width, pbeam->endWidth,
...
bool CViewRenderBeams::RecomputeBeamEndpoints(Beam_t *pbeam) {
  if (pbeam->flags & FBEAM_STARTENTITY) {
    if (ComputeBeamEntPosition(pbeam->entity[0], pbeam->attachmentIndex[0],
                               (pbeam->flags & FBEAM_USE_HITBOXES) != 0,
                               pbeam->attachment[0])) {
    ...
    if (!(pbeam->flags & FBEAM_STARTVISIBLE)) return false;
  }

  if (pbeam->flags & FBEAM_ENDENTITY) {
    if (ComputeBeamEntPosition(pbeam->entity[1], pbeam->attachmentIndex[1],
                               (pbeam->flags & FBEAM_USE_HITBOXES) != 0,
                               pbeam->attachment[1])) {
    ...
    if (!(pbeam->flags & FBEAM_ENDVISIBLE)) return false;
  }
...
void CViewRenderBeams::DrawBeam(C_Beam *pbeam,
  ...
  if (beamType == BEAM_ENTPOINT || beamType == BEAM_ENTS ||
      beamType == BEAM_LASER) {
    ComputeBeamEntPosition(pbeam->m_hAttachEntity[0], pbeam->m_nAttachIndex[0],
                           false, vRayStartPoint);
    ComputeBeamEntPosition(pbeam->m_hAttachEntity[1], pbeam->m_nAttachIndex[1],
                           false, vRayEndPoint);
  }
  ...
  CTraceFilterSkipClassname traceFilter(pStartEntity, "prop_energy_ball",
  ...
  C_Prop_Portal *pPortal = UTIL_Portal_TraceRay_Beam(
      rayBeam, MASK_SHOT, &traceFilterChain, &fEndFraction);
  ...
  if (pPortal) {
    ...
    if (iOldBrightness > 16) {
      // Remember the old values of the beam before changing it for the next
      // call
      ...
      // Get the transformed positions of the sub beam in the other portal's
      // space
      ...
      UTIL_Portal_PointTransform(
          matThisToLinked, rayBeam.m_Start + rayBeam.m_Delta, vTransformedEnd);
      ...
      if (bIsReversed) pbeam->SetEndWidth(pbeam->GetWidth());
      ...
      if (pOldStartEntity) pbeam->SetStartEntity(pOldStartEntity);
      if (pOldEndEntity) pbeam->SetEndEntity(pOldEndEntity);
      ...
      if (!bIsReversed) beamInfo.m_flEndWidth = beamInfo.m_flWidth;
    }
  }
  ...
  if (pbeam->m_nHaloIndex > 0) {
    ...
    float size =
        1.0f + (pbeam->m_fHaloScale * pbeam->m_fWidth / pbeam->m_fEndWidth);
    size = std::clamp(size, 1.0f, 8.0f);
  ...
  beam.flags |=
      pbeam->GetBeamFlags() & (FBEAM_SINENOISE | FBEAM_SOLID | FBEAM_SHADEIN |
                               FBEAM_SHADEOUT | FBEAM_NOTILE);

  if (beam.entity[0]) {
    // don't draw viewmodel effects in reflections
    if (CurrentViewID() == VIEW_REFLECTION) {
      int group = beam.entity[0]->GetRenderGroup();
      if (group == RENDER_GROUP_VIEW_MODEL_TRANSLUCENT ||
