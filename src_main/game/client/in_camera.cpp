...
#define CAM_HULL_OFFSET \
  9.0  // the size of the bounding hull used for collision checking
static Vector CAM_HULL_MIN(-CAM_HULL_OFFSET, -CAM_HULL_OFFSET,
                           -CAM_HULL_OFFSET);
static Vector CAM_HULL_MAX(CAM_HULL_OFFSET, CAM_HULL_OFFSET, CAM_HULL_OFFSET);

//-------------------------------------------------- Global Variables

static ConVar cam_command("cam_command", "0",
                          FCVAR_CHEAT);  // tells camera to go to thirdperson
static ConVar cam_snapto("cam_snapto", "0",
                         FCVAR_ARCHIVE);  // snap to thirdperson view
static ConVar cam_ideallag("cam_ideallag", "4.0", FCVAR_ARCHIVE,
                           "Amount of lag used when matching offset to ideal "
                           "angles in thirdperson view");
static ConVar cam_idealdelta("cam_idealdelta", "4.0", FCVAR_ARCHIVE,
                             "Controls the speed when matching offset to ideal "
                             "angles in thirdperson view");
ConVar cam_idealyaw("cam_idealyaw", "0", FCVAR_ARCHIVE);  // thirdperson yaw
ConVar cam_idealpitch("cam_idealpitch", "0",
                      FCVAR_ARCHIVE);  // thirperson pitch
static ConVar cam_idealdist("cam_idealdist", "150",
                            FCVAR_ARCHIVE);  // thirdperson distance
static ConVar cam_collision(
    "cam_collision", "1", FCVAR_ARCHIVE,
    "When in thirdperson and cam_collision is set to 1, an attempt is made to "
    "keep the camera from passing though walls.");
static ConVar cam_showangles(
    "cam_showangles", "0", FCVAR_CHEAT,
    "When in thirdperson, print viewangles/idealangles/cameraoffsets to the "
    "console.");
...
==============================
*/
void CAM_StartMouseMove(void) { input->CAM_StartMouseMove(); }
...
==============================
*/
void CAM_EndMouseMove(void) { input->CAM_EndMouseMove(); }
...
==============================
*/
void CAM_StartDistance(void) { input->CAM_StartDistance(); }
...
==============================
*/
void CAM_EndDistance(void) { input->CAM_EndDistance(); }
...
void CInput::CAM_Think(void) {
  ...
  if (Is_CAM_ThirdPerson_MayaMode()) {
    ...
    if (!m_fCameraMovingWithMouse) return;
...
#ifndef _XBOX
    if (/* Left+Middle Button Down */ vgui::input()->IsMouseDown(MOUSE_LEFT) &&
        vgui::input()->IsMouseDown(MOUSE_MIDDLE)) {
    ...
    } else if (/* Right Button Down */ vgui::input()->IsMouseDown(
        MOUSE_RIGHT)) {
      // Do only zoom in/out camera adjustment
      m_fCameraDistanceMove = true;
    } else {
  ...
  if (m_fCameraMovingWithMouse) {
    ...
    // don't do any movement of the cam using YAW/PITCH if we are zooming in/out
    // the camera
    if (!m_fCameraDistanceMove) {
      int x, y;
      GetWindowCenter(x, y);

      // keep the camera within certain limits around the player (ie avoid
      // certain bad viewing angles)
      if (m_nCameraX > x) {
  ...
  if (m_fCameraDistanceMove) {
    ...
    if (m_nCameraY > y) {
      ...
      if (idealAngles[DIST] > c_mindistance.GetFloat()) {
        idealAngles[DIST] -=
            (cam_idealdelta.GetFloat()) * ((y - m_nCameraY) / 2);
      }
      if (idealAngles[DIST] < c_mindistance.GetFloat()) {
  ...
  // This introduces a potential discontinuity when looking directly at model
  // face as camera yaw will be jumping from +180 to -180 and back, but when
  // working with the camera allows smooth rotational transitions from left to
  // right and back. Otherwise one of the transitions that has ">"-comparison
  // will be locked.
  // --
  ...
  idealAngles[PITCH] = std::clamp(idealAngles[PITCH], c_minpitch.GetFloat(),
                                  c_maxpitch.GetFloat());
  idealAngles[YAW] =
      std::clamp(idealAngles[YAW], c_minyaw.GetFloat(), c_maxyaw.GetFloat());
  idealAngles[DIST] = std::clamp(idealAngles[DIST], c_mindistance.GetFloat(),
                                 c_maxdistance.GetFloat());
  ...
  if (cam_snapto.GetInt()) {
    ...
    float lag = std::max(1.0f, 1 + cam_ideallag.GetFloat());

    if (camOffset[YAW] - viewangles[YAW] != cam_idealyaw.GetFloat())
      camOffset[YAW] = MoveToward(
          camOffset[YAW], cam_idealyaw.GetFloat() + viewangles[YAW], lag);

    if (camOffset[PITCH] - viewangles[PITCH] != cam_idealpitch.GetFloat())
      camOffset[PITCH] = MoveToward(
          camOffset[PITCH], cam_idealpitch.GetFloat() + viewangles[PITCH], lag);
  ...
  if (cam_collision.GetInt()) {
    ...
    if (localPlayer) {
      ...
      AngleVectors(QAngle(camOffset[PITCH], camOffset[YAW], 0), &camForward,
                   NULL, NULL);
      ...
      UTIL_TraceHull(origin, origin - (camForward * camOffset[DIST]),
                     CAM_HULL_MIN, CAM_HULL_MAX, MASK_SOLID, &traceFilter,
                     &trace);
      ...
      // For now, I'd rather see the insade of a player model than punch the
      // camera through a wall might try the fade out trick at some point
  ...
  if (cam_showangles.GetInt()) {
    engine->Con_NPrintf(4, "Pitch: %6.1f   Yaw: %6.1f %38s", viewangles[PITCH],
                        viewangles[YAW], "view angles");
    engine->Con_NPrintf(6, "Pitch: %6.1f   Yaw: %6.1f   Dist: %6.1f %19s",
                        cam_idealpitch.GetFloat(), cam_idealyaw.GetFloat(),
                        cam_idealdist.GetFloat(), "ideal angles");
    engine->Con_NPrintf(8, "Pitch: %6.1f   Yaw: %6.1f   Dist: %6.1f %16s",
                        m_vecCameraOffset[PITCH], m_vecCameraOffset[YAW],
                        m_vecCameraOffset[DIST], "camera offset");
  }
...
// Purpose:
//------------------------------------------------------------------------------
void CInput::CAM_SetCameraThirdData(CameraThirdData_t *pCameraData,
...
void CInput::CAM_CameraThirdThink(void) {
  ...
  // Move the CameraOffset "towards" the idealAngles, Note: CameraOffset =
  // viewangle + idealAngle
  Vector vecCamOffset;
  VectorCopy(m_vecCameraOffset, vecCamOffset);

  // Move the camera.
  float flLag = std::max(1.0f, 1 + m_pCameraThirdData->m_flLag);
  if (vecCamOffset[PITCH] - angView[PITCH] != m_pCameraThirdData->m_flPitch) {
    vecCamOffset[PITCH] =
        MoveToward(vecCamOffset[PITCH],
                   (m_pCameraThirdData->m_flPitch + angView[PITCH]), flLag);
  }
  if (vecCamOffset[YAW] - angView[YAW] != m_pCameraThirdData->m_flYaw) {
    vecCamOffset[YAW] = MoveToward(
        vecCamOffset[YAW], (m_pCameraThirdData->m_flYaw + angView[YAW]), flLag);
  }
  if (abs(vecCamOffset[DIST] - m_pCameraThirdData->m_flDist) < 2.0) {
    vecCamOffset[DIST] = m_pCameraThirdData->m_flDist;
  } else {
    vecCamOffset[DIST] +=
        (m_pCameraThirdData->m_flDist - vecCamOffset[DIST]) / flLag;
  }
  ...
  if (pLocalPlayer) {
    ...
    // Get the forward vector
    AngleVectors(QAngle(vecCamOffset[PITCH], vecCamOffset[YAW], 0), &vecForward,
                 NULL, NULL);

    // Collision trace and move the camera closer if we hit something.
    trace_t trace;
    UTIL_TraceHull(vecOrigin, vecOrigin - (vecForward * vecCamOffset[DIST]),
                   m_pCameraThirdData->m_vecHullMin,
...
==============================
*/
bool CInput::CAM_IsOrthographic(void) const { return m_CameraIsOrthographic; }
...
==============================
*/
int CInput::CAM_IsThirdPerson(void) { return m_fCameraInThirdPerson; }
...
==============================
*/
int CInput::CAM_InterceptingMouse(void) { return m_fCameraInterceptingMouse; }
...
static ConCommand thirdperson_mayamode(
    "thirdperson_mayamode", ::CAM_ToThirdPerson_MayaMode,
    "Switch to thirdperson Maya-like camera controls.", FCVAR_CHEAT);
static ConCommand firstperson("firstperson", ::CAM_ToFirstPerson,
...
==============================
*/
void CInput::Init_Camera(void) { m_CameraIsOrthographic = false; }
