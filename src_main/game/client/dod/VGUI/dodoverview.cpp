...
ConVar _overview_mode("_overview_mode", "1", FCVAR_ARCHIVE,
...
ConVar dod_overview_voice_icon_size("dod_overview_voice_icon_size", "64",
                                    FCVAR_ARCHIVE);

CDODMapOverview::CDODMapOverview(const char *pElementName)
...
void CDODMapOverview::UpdateCapturePoints() {
  if (!g_pObjectiveResource) return;
  ...
  for (int i = 0; i < g_pObjectiveResource->GetNumControlPoints(); i++) {
    ...
    if (iOwningIcon <= 0) continue;  // baah
    ...
    if (objID == 0) {
      ...
      SetObjectPosition(objID, g_pObjectiveResource->GetCPPosition(i),
                        vec3_angle);
    ...
    // Draw the number of cappers below the icon
      ...
      int numPlayers =
          g_pObjectiveResource->GetNumPlayersInArea(i, iCappingTeam);
      int requiredPlayers =
          g_pObjectiveResource->GetRequiredCappers(i, iCappingTeam);
    ...
    if (iCappingTeam != TEAM_UNASSIGNED) {
      SetObjectStatus(objID, g_pObjectiveResource->GetCPCapPercentage(i),
                      colorGreen);
...
void CDODMapOverview::InitTeamColorsAndIcons() {
  ...
  m_CameraIcons[TEAM_ALLIES] =
      AddIconTexture("sprites/minimap_icons/allies_camera");

  m_TeamColors[TEAM_AXIS] = COLOR_DOD_RED;
  m_TeamIcons[TEAM_AXIS] = AddIconTexture("sprites/minimap_icons/gplayer");
  m_CameraIcons[TEAM_AXIS] =
      AddIconTexture("sprites/minimap_icons/axis_camera");
...
bool CDODMapOverview::CanPlayerBeSeen(MapPlayer_t *player) {
  ...
  if (localPlayer->entindex() == (player->index + 1)) return false;
...
void CDODMapOverview::ShowLargeMap(void) {
  ...
  if (m_iLastMode == MAP_MODE_FULL) m_iLastMode = MAP_MODE_OFF;
...
void CDODMapOverview::ToggleZoom(void) {
  if (GetMode() != MAP_MODE_INSET) return;
...
void CDODMapOverview::UpdateSizeAndPosition() {
  ...
  if (!g_pSpectatorGUI ||
      (!g_pSpectatorGUI->IsVisible() && GetMode() == MAP_MODE_INSET)) {
    int x, y, w, h;
...
void CDODMapOverview::PlayerChat(int index) {
  m_flPlayerChatTime[index - 1] =
      gpGlobals->curtime + cl_overview_chat_time.GetFloat();
}
...
void CDODMapOverview::DrawVoiceIconForPlayer(int playerIndex) {
  ...
  // if they just sent a chat msg, or are using voice, or did a hand signal or
  // voice command draw a chat icon

  if (cl_voicetest.GetInt() ||
      GetClientVoiceMgr()->IsPlayerSpeaking(player->index + 1)) {
    MapObject_t obj;
    memset(&obj, 0, sizeof(MapObject_t));
...
void CDODMapOverview::DrawQuad(Vector pos, int scale, float angle,
                               int textureID, int alpha) {
  Vector offset;
  offset.z = 0;
  ...
  Vertex_t points[4] = {Vertex_t(MapToPanel(pos1), Vector2D(0, 0)),
...
bool CDODMapOverview::DrawCapturePoint(int iCP, MapObject_t *obj) {
  ...
  if (iBombsRequired) {
    ...
    if (iCappingTeam != TEAM_UNASSIGNED) {
      ...
      DrawHorizontalSwipe(pos, scale, AddIconTexture(textureName), flCapPercent,
                          bSwipeLeft);
    }

    // fixup for noone is capping, but someone is in the area
    int iNumAllies =
        g_pObjectiveResource->GetNumPlayersInArea(iCP, TEAM_ALLIES);
    ...
    if (iCappingTeam != TEAM_UNASSIGNED) {
      ...
      int requiredPlayers =
          g_pObjectiveResource->GetRequiredCappers(iCP, iCappingTeam);
    ...
    switch (iBombsRequired) {
      case 1: {
        ...
        switch (iBombsRemaining) {
          case 0:
            DrawQuad(bombPos, flBombIconScale, angle,
                     m_pExplodedIcon->textureId, 255);
            break;
          case 1:
            if (bBombPlanted) {
              ...
              DrawQuad(bombPos, flBombIconScale * 2, angle,
                       m_pC4PlantedBG->textureId, alpha);
            }
            DrawQuad(bombPos, flBombIconScale, angle, m_pC4Icon->textureId,
                     255);
            break;
        }
      } break;
      case 2: {
        ...
        switch (iBombsRemaining) {
          case 0:
            DrawQuad(bombPos1, flBombIconScale, angle,
                     m_pExplodedIcon->textureId, 255);
            DrawQuad(bombPos2, flBombIconScale, angle,
                     m_pExplodedIcon->textureId, 255);
            break;
          case 1:
            if (bBombPlanted) {
              ...
              int alpha =
                  (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
              DrawQuad(bombPos1, flBombIconScale * 2, angle,
                       m_pC4PlantedBG->textureId, alpha);
            }
            DrawQuad(bombPos1, flBombIconScale, angle, m_pC4Icon->textureId,
                     255);
            DrawQuad(bombPos2, flBombIconScale, angle,
                     m_pExplodedIcon->textureId, 255);
            break;
          case 2:
            if (bBombPlanted) {
              ...
              int alpha =
                  (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
              DrawQuad(bombPos2, flBombIconScale * 2, angle,
                       m_pC4PlantedBG->textureId, alpha);
            }
            DrawQuad(bombPos1, flBombIconScale, angle, m_pC4Icon->textureId,
                     255);
            DrawQuad(bombPos2, flBombIconScale, angle, m_pC4Icon->textureId,
                     255);
            break;
        }
      } break;
...
void CDODMapOverview::DrawBombTimerSwipeIcon(Vector pos, int scale,
                                             int textureID,
                                             float flPercentRemaining) {
  ...
  */
  ...
  for (j = 0; j <= 7; j++) {
    ...
    if (flAngle <= 0) {
      ...
      vec1.x = newPos.x + quadrants[j].vecTrailing.x * scale * 2 -
               xdir * tan(flAngle) * scale;
      vec1.y = newPos.y - quadrants[j].vecTrailing.y * scale * 2 +
               ydir * tan(flAngle) * scale;

      uv1.x = quadrants[j].vecTrailing.x -
              xdir *
                  abs(quadrants[j].vecLeading.x - quadrants[j].vecTrailing.x) *
                  tan(flAngle);
      uv1.y = quadrants[j].vecTrailing.y -
              ydir *
                  abs(quadrants[j].vecLeading.y - quadrants[j].vecTrailing.y) *
                  tan(flAngle);
...
void CDODMapOverview::DrawHorizontalSwipe(Vector pos, int scale, int textureID,
