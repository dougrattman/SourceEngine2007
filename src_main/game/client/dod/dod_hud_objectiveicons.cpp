...
class CHudObjectiveIcons : public CHudElement, public vgui::Panel {
 public:
  ...
  void DrawBackgroundBox(int xpos, int ypos, int nBoxWidth, int nBoxHeight,
                         bool bCutCorner);
 ...
 private:
  ...
  int m_iSecondsAdded;  // how many seconds were added in the last time_added
                        // event
  bool bInTimerWarningAnim;
  float m_flDrawTimeAddedUntil;

  CPanelAnimationVar(vgui::HFont, m_hTimerFont, "TimerFont", "Default");
  CPanelAnimationVar(vgui::HFont, m_hTimerFontSmall, "TimerFontSmall",
                     "DefaultSmall");

  CPanelAnimationVar(vgui::HFont, m_hTextFont, "ChatFont", "Default");

  CPanelAnimationVarAliasType(int, m_nIconSize, "iconsize", "24",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_nSeparatorWidth, "separator_width", "7",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_nCornerCutSize, "CornerCutSize", "5",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_nBackgroundOverlap, "BackgroundOverlap",
                              "5", "proportional_int");

  CPanelAnimationVarAliasType(int, m_iIconStartX, "icon_start_x", "10",
                              "proportional_int");
  CPanelAnimationVarAliasType(int, m_iIconStartY, "icon_start_y", "10",
                              "proportional_int");
  CPanelAnimationVarAliasType(float, m_flIconExpand, "icon_expand", "0",
                              "proportional_float");

  CPanelAnimationVar(Color, m_clrTimer, "TimerBG", "255 0 0 128");

  CPanelAnimationVarAliasType(int, m_nTimeAddedHeight, "time_added_height",
                              "12", "proportional_int");
  CPanelAnimationVar(float, m_flTimeAddedExpandPercent,
                     "time_added_height_anim", "0.0");
  CPanelAnimationVar(float, m_flTimeAddedAlpha, "time_added_alpha", "0");

  CPanelAnimationVar(float, m_flTimeAddedDuration, "time_added_duration",
                     "3.5");
};
...
// Purpose:
//-----------------------------------------------------------------------------
CHudObjectiveIcons::CHudObjectiveIcons(const char *pName)
    : vgui::Panel(NULL, "HudObjectiveIcons"), CHudElement(pName) {
  ...
  vgui::surface()->DrawSetTextureFile(m_iBackgroundTexture, "vgui/white", true,
                                      true);
...
void CHudObjectiveIcons::FireGameEvent(IGameEvent *event) {
  ...
  if (FStrEq("dod_timer_time_added", eventname)) {
    ...
    _snwprintf(wText, sizeof(wText) / sizeof(wchar_t), L"%s %d:%02d",
               bNegative ? L"-" : L"+", iSecondsToDraw / 60,
               iSecondsToDraw % 60);
...
// Purpose:
//-----------------------------------------------------------------------------
void CHudObjectiveIcons::DrawBackgroundBox(int xpos, int ypos, int nBoxWidth,
                                           int nBoxHeight, bool bCutCorner) {
  ...
  verts[2].Init(
      Vector2D(xpos + nBoxWidth + 1, ypos + nBoxHeight - nCornerCutSize + 1));
  verts[3].Init(
      Vector2D(xpos + nBoxWidth - nCornerCutSize + 1, ypos + nBoxHeight + 1));
  ...
  borderverts[2].Init(
      Vector2D(xpos + nBoxWidth, ypos + nBoxHeight - nCornerCutSize));
  borderverts[3].Init(
      Vector2D(xpos + nBoxWidth - nCornerCutSize, ypos + nBoxHeight));
...
void CHudObjectiveIcons::Paint() {
  ...
  if (m_flDrawTimeAddedUntil > 0 &&
      m_flDrawTimeAddedUntil < gpGlobals->curtime) {
    g_pClientMode->GetViewportAnimationController()->StartAnimationSequence(
        "HideTimeAdded");
  ...
  // let's count how many visible capture points we have (for cutting the corner
  // of the last visible one)
  for (k = 0; k < num; k++) {
  ...
  if (bShowTimer) {
    if (m_pTimer) {
      ...
      m_pTimeAdded->SetBounds(xpos, ypos + m_nIconSize, 2 * m_nIconSize,
                              m_nTimeAddedHeight + m_nBackgroundOverlap);
      ...
      int boxHeight =
          2 * m_nBackgroundOverlap + m_nIconSize +
          (int)((float)m_nTimeAddedHeight * m_flTimeAddedExpandPercent);

      DrawBackgroundBox(xpos - m_nBackgroundOverlap,
                        ypos - m_nBackgroundOverlap,
                        m_pTimer->GetWide() + 2 * m_nBackgroundOverlap,
                        boxHeight, (num <= 0) ? true : false);
      ...
      xpos +=
          (m_pTimer->GetWide() + m_nSeparatorWidth + m_nBackgroundOverlap * 2);
    }
  } else {
    if (m_pTimer && m_pTimer->IsVisible()) {
      m_pTimer->SetVisible(false);
  ...
  for (index = 0; index < num; index++) {
    if (g_pObjectiveResource->IsCPVisible(index)) {
      ...
      if (iBombsRequired > 0) {
        DrawBackgroundBox(
            flXPos - m_nBackgroundOverlap, flYPos - m_nBackgroundOverlap,
            flIconSize + 2 * m_nBackgroundOverlap,
            flIconSize + 2 * m_nBackgroundOverlap + m_nTimeAddedHeight,
            (index != iLastVisible) ? false : true);
      } else {
        DrawBackgroundBox(flXPos - m_nBackgroundOverlap,
                          flYPos - m_nBackgroundOverlap,
                          flIconSize + 2 * m_nBackgroundOverlap,
                          flIconSize + 2 * m_nBackgroundOverlap,
                          (index != iLastVisible) ? false : true);
      }
      ...
      if (g_pObjectiveResource->IsBombSetAtPoint(index)) {
        ...
        float flBombTime =
            g_pObjectiveResource->GetBombTimeForPoint(index);  // round up
        ...
        if (iBlankIcon == 0) {
          iBlankIcon =
              g_pObjectiveResource->GetIconForTeam(index, TEAM_UNASSIGNED);
        }

        const char *szMatName = GetMaterialNameFromIndex(iBlankIcon);

        vgui::surface()->DrawSetTextureFile(m_iCPTextures[index], szMatName,
                                            true, false);
        ...
        */
        ...
        for (j = 0; j <= 7; j++) {
          ...
          if (flAngle <= 0) {
            ...
            vec1.x = flXPos + quadrants[j].vecTrailing.x * flIconSize -
                     xdir * tan(flAngle) * flHalfWide;
            vec1.y = flYPos + quadrants[j].vecTrailing.y * flIconSize -
                     ydir * tan(flAngle) * flHalfTall;

            uv1.x = quadrants[j].vecTrailing.x -
                    xdir *
                        abs(quadrants[j].vecLeading.x -
                            quadrants[j].vecTrailing.x) *
                        tan(flAngle);
            uv1.y = quadrants[j].vecTrailing.y -
                    ydir *
                        abs(quadrants[j].vecLeading.y -
                            quadrants[j].vecTrailing.y) *
                        tan(flAngle);
            ...
            vert[2].Init(
                Vector2D(flXPos + quadrants[j].vecTrailing.x * flIconSize,
        ...
        if (iOwnerIcon != 0) {
          ...
          // todo: framerate
          IVguiMatInfo *pMat = vgui::surface()->DrawGetTextureMatInfoFactory(
          m_iCPTextures[index] );
          ...
          */
          ...
          vert[0].Init(Vector2D(flXPos, flYPos), uv11);
          vert[1].Init(Vector2D(flXPos + flIconSize, flYPos), uv21);
          vert[2].Init(Vector2D(flXPos + flIconSize, flYPos + flIconSize),
                       uv22);
          vert[3].Init(Vector2D(flXPos, flYPos + flIconSize), uv12);
      ...
      int iNumAxis =
          g_pObjectiveResource->GetNumPlayersInArea(index, TEAM_AXIS);
        ...
        switch (iBombsRequired) {
          case 1: {
            ...
            switch (iBombsRemaining) {
              case 0:
                m_pExplodedIcon->DrawSelf(xMid - iIconHalfWidth, yIcon,
                                          iIconWidth, iIconWidth, c);
                break;
              case 1:
                if (bBombPlanted) {
                  // draw the background behind 1
                  int alpha =
                      (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
                  m_pC4PlantedBG->DrawSelf(
                      xMid - iIconWidth, yIcon - iIconHalfWidth, iIconWidth * 2,
                      iIconWidth * 2, Color(255, 255, 255, alpha));
                }
                m_pC4Icon->DrawSelf(xMid - iIconHalfWidth, yIcon, iIconWidth,
                                    iIconWidth, c);
                break;
            }
          } break;
          case 2: {
            ...
            switch (iBombsRemaining) {
              case 0:
                m_pExplodedIcon->DrawSelf(xMid1 - iIconHalfWidth, yIcon,
                                          iIconWidth, iIconWidth, c);
                m_pExplodedIcon->DrawSelf(xMid2 - iIconHalfWidth, yIcon,
                                          iIconWidth, iIconWidth, c);
                break;
              case 1:
                if (bBombPlanted) {
                  ...
                  int alpha =
                      (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
                  m_pC4PlantedBG->DrawSelf(xMid1 - iIconWidth,
                                           yIcon - iIconHalfWidth,
                                           iIconWidth * 2, iIconWidth * 2,
                                           Color(255, 255, 255, alpha));
                }
                m_pC4Icon->DrawSelf(xMid1 - iIconHalfWidth, yIcon, iIconWidth,
                                    iIconWidth, c);
                m_pExplodedIcon->DrawSelf(xMid2 - iIconHalfWidth, yIcon,
                                          iIconWidth, iIconWidth, c);
                break;
              case 2:
                if (bBombPlanted) {
                  ...
                  int alpha =
                      (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
                  m_pC4PlantedBG->DrawSelf(xMid2 - iIconWidth,
                                           yIcon - iIconHalfWidth,
                                           iIconWidth * 2, iIconWidth * 2,
                                           Color(255, 255, 255, alpha));
                }
                m_pC4Icon->DrawSelf(xMid1 - iIconHalfWidth, yIcon, iIconWidth,
                                    iIconWidth, c);
                m_pC4Icon->DrawSelf(xMid2 - iIconHalfWidth, yIcon, iIconWidth,
                                    iIconWidth, c);
                break;
            }
          } break;
          default: {
            ...
            if (iBombsRemaining > 0) {
              ...
              if (bBombPlanted) {
                ...
                int alpha =
                    (float)(abs(sin(2 * gpGlobals->curtime)) * 205.0 + 50.0);
                m_pC4PlantedBG->DrawSelf(xpos - iIconHalfWidth,
                                         yIcon - iIconHalfWidth, iIconWidth * 2,
                                         iIconWidth * 2,
                                         Color(255, 255, 255, alpha));
              }
              ...
              {
                wchar_t wText[6];
                _snwprintf(wText, sizeof(wText) / sizeof(wchar_t), L"x %d",
                           iBombsRemaining);

                vgui::surface()->DrawSetTextColor(
                    g_PR->GetTeamColor(TEAM_SPECTATOR));

                // set pos centered under icon
                vgui::surface()->DrawSetTextPos(xpos + (flIconSize * 0.50f),
                                                yIcon + YRES(2));
      ...
      if (iCappingTeam == TEAM_UNASSIGNED) {
        ...
        if (iCappingTeam == TEAM_UNASSIGNED ||
            iCappingTeam == g_pObjectiveResource->GetOwningTeam(index)) {
      ...
      int numPlayers =
          g_pObjectiveResource->GetNumPlayersInArea(index, iCappingTeam);
      int requiredPlayers =
          g_pObjectiveResource->GetRequiredCappers(index, iCappingTeam);

      if (requiredPlayers > 1) {
        numPlayers = std::min(numPlayers, requiredPlayers);

        wchar_t wText[6];
        _snwprintf(wText, sizeof(wText) / sizeof(wchar_t), L"%d/%d", numPlayers,
                   requiredPlayers);
        ...
        vgui::surface()->DrawSetTextPos(
            xpos + (m_nIconSize / 2.0f) - (len / 2),
            ypos + flIconSize + m_nBackgroundOverlap +
                YRES(2));  // + 2 to account for the outlined box
      ...
      if (g_pObjectiveResource->GetCappingTeam(index) != TEAM_UNASSIGNED) {
        ...
        if (iOwnerIcon != iCapperIcon && iCapperIcon != 0) {
          ...
          bool bAxis =
              (g_pObjectiveResource->GetCappingTeam(index) == TEAM_AXIS)
                  ? true
                  : false;

          // swipe!
          float flCapPercentage =
              g_pObjectiveResource->GetCPCapPercentage(index);

          // reversing the direction of the swipe effect
          if (bAxis) {
            flCapPercentage =
                1.0f - g_pObjectiveResource->GetCPCapPercentage(index);
          }

          float width = (flIconSize * flCapPercentage);
          const char *szCappingMatName = GetMaterialNameFromIndex(iCapperIcon);

          vgui::surface()->DrawSetTextureFile(m_iCPCappingTextures[index],
                                              szCappingMatName, true, false);
