...
CDODWinPanel_Allies::CDODWinPanel_Allies(const char *pElementName)
    : CDODWinPanel("WinPanel_Allies", TEAM_ALLIES) {
...
CDODWinPanel_Axis::CDODWinPanel_Axis(const char *pElementName)
    : CDODWinPanel("WinPanel_Axis", TEAM_AXIS) {
...
void CDODWinPanel::FireGameEvent(IGameEvent *event) {
  ...
  if (Q_strcmp("dod_round_win", pEventName) == 0) {
    ...
    "final_event"		"byte"		// 0 - no event, 1 - bomb exploded, 2 -
    flag capped, 3 - timer expired

    "category_left"		"byte"		// 0-4: none, bombers, cappers,
    defenders, killers
    ...
    */
    ...
    int minutes = std::clamp(m_iTimerTime / 60, 0, 99);
    int seconds = std::clamp(m_iTimerTime % 60, 0, 59);
    ...
    if (m_bShowTimerDefend) {
      ...
      g_pVGuiLocalize->ConstructString(
          timerText, sizeof(timerText),
          g_pVGuiLocalize->Find("#winpanel_total_time"), 1, time);
      ...
      g_pVGuiLocalize->ConstructString(
          timerText, sizeof(timerText),
          g_pVGuiLocalize->Find("#winpanel_attack_time"), 1, time);
    ...
    m_pLeftCategoryHeader->SetText(
        g_pVGuiLocalize->Find(pszWinPanelCategoryHeaders[m_iLeftCategory]));
    m_pRightCategoryHeader->SetText(
        g_pVGuiLocalize->Find(pszWinPanelCategoryHeaders[m_iRightCategory]));
...
void CDODWinPanel::SetupAvatar(const char *pSide, int pos, int iPlayerIndex) {
  ...
  CAvatarImagePanel *pPlayerAvatar = dynamic_cast<CAvatarImagePanel *>(
      FindChildByName(CFmtStr("%s_%d_avatar", pSide, pos)));
...
void CDODWinPanel::SetFinalCaptureLabel(const char *szCappers,
                                        bool bShowAvatar) {
