...
CBaseEntity *C_BasePlayer::GetObserverTarget()
    const  // returns players targer or NULL
{
#ifndef _XBOX
  if (IsHLTV()) {
...
void C_BasePlayer::SetObserverTarget(EHANDLE hObserverTarget) {
  ...
  // equal, since it uses Get(), and compares NULL to NULL.  To combat this, we
  // need to check against GetEntryIndex() and GetSerialNumber().
  if (hObserverTarget.GetEntryIndex() != m_hObserverTarget.GetEntryIndex() ||
    ...
    // it's Get() has a chance to become non-NULL even if it currently resolves
    // to NULL.
...
bool C_BasePlayer::ViewModel_IsTransparent(void) { return IsTransparent(); }
...
surfacedata_t *C_BasePlayer::GetGroundSurface() {
  ...
  if (trace.fraction == 1.0f) return NULL;  // no ground
...
// Is the player dead?
//-----------------------------------------------------------------------------
bool C_BasePlayer::IsPlayerDead() { return pl.deadflag == true; }
...
void C_BasePlayer::SetVehicleRole(int nRole) {
  ...
  if (nRole > VEHICLE_ROLE_DRIVER) return;
...
void C_BasePlayer::PostDataUpdate(DataUpdateType_t updateType) {
  // This has to occur here as opposed to OnDataChanged so that EHandles to the
  // player created
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    ...
    if (g_nKillCamMode) iLocalPlayerIndex = g_nKillCamTarget1;
  ...
  if (IsLocalPlayer()) {
    ...
    if (!m_bWasFreezeFraming && GetObserverMode() == OBS_MODE_FREEZECAM) {
      ...
      if (pEvent) {
        pEvent->SetInt("killer", GetObserverTarget()
                                     ? GetObserverTarget()->entindex()
                                     : 0);
    ...
    for (int i = 0; i < MAX_AMMO_TYPES; i++) {
      if (GetAmmoCount(i) > m_iOldAmmo[i]) {
        ...
        if (!pWeaponData || !(pWeaponData->iFlags & ITEM_FLAG_NOAMMOPICKUPS)) {
          ...
          if (pHudHR) {
            pHudHR->AddToHistory(HISTSLOT_AMMO, i,
...
bool C_BasePlayer::CreateMove(float flInputSampleTime, CUserCmd *pCmd) {
  ...
  // If the frozen flag is set, prevent view movement (server prevents the rest
  // of the movement)
...
void C_BasePlayer::CreateWaterEffects(void) {
  ...
  if (m_bResampleWaterSurface) {
    ...
    m_flWaterSurfaceZ = UTIL_WaterLevel(
        WorldSpaceCenter(), WorldSpaceCenter().z, WorldSpaceCenter().z + 256);
    m_bResampleWaterSurface = false;
  }

  // Make sure the emitter is setup
  if (m_pWaterEmitter == NULL) {
    if ((m_pWaterEmitter = WaterDebrisEffect::Create("splish")) == NULL) return;
  }
  ...
  while (m_tWaterParticleTimer.NextEvent(curTime)) {
    offset =
        WorldSpaceCenter() + (vecVelocity * 128.0f) + RandomVector(-128, 128);
...
bool C_BasePlayer::ShouldInterpolate() {
  ...
  if (HLTVCamera()->GetCameraMan() == this) return true;
#endif
  return BaseClass::ShouldInterpolate();
}

bool C_BasePlayer::ShouldDraw() {
  return (!IsLocalPlayer() || C_BasePlayer::ShouldDrawLocalPlayer() ||
          (GetObserverMode() == OBS_MODE_DEATHCAM)) &&
         BaseClass::ShouldDraw();
}

int C_BasePlayer::DrawModel(int flags) {
  // if local player is spectating this player in first person mode, don't draw
  // it
...
Vector C_BasePlayer::GetChaseCamViewOffset(CBaseEntity *target) {
  ...
  if (player && player->IsAlive()) {
    if (player->GetFlags() & FL_DUCKING) return VEC_DUCK_VIEW;
...
void C_BasePlayer::CalcChaseCamView(Vector &eyeOrigin, QAngle &eyeAngles,
  ...
  m_flObserverChaseDistance =
      std::clamp(m_flObserverChaseDistance, 16.0f, CHASE_CAM_DISTANCE);
...
void C_BasePlayer::CalcFreezeCamView(Vector &eyeOrigin, QAngle &eyeAngles,
  ...
  float flBlendPerc =
      std::clamp(flCurTime / spec_freeze_traveltime.GetFloat(), 0.0f, 1.0f);
  ...
  if (trace.fraction < 1.0) {
    ...
    // To stop all close in views looking up at character's chins, move the view
    // up.
...
// Output : C_BasePlayer
//-----------------------------------------------------------------------------
C_BasePlayer *C_BasePlayer::GetLocalPlayer(void) { return s_pLocalPlayer; }
...
int C_BasePlayer::GetUserID(void) {
  ...
  if (!engine->GetPlayerInfo(entindex(), &pi)) return -1;
...
void C_BasePlayer::PlayPlayerJingle() {
  ...
  if (!cl_customsounds.GetBool()) return;

  // Doesn't have a jingle sound
  if (!info.customFiles[1]) return;
  ...
  if (!filesystem->FileExists(fullsoundname)) {
    ...
    if (!filesystem->FileExists(custname)) return;  // not downloaded yet

    // copy from download folder to materials/temp folder
    // this is done since material system can access only materials/*.vtf files

    if (!engine->CopyFile(custname, fullsoundname)) return;
  }
...
float C_BasePlayer::GetWaterJumpTime() const { return m_flWaterJumpTime; }

void C_BasePlayer::SetWaterJumpTime(float flWaterJumpTime) {
  m_flWaterJumpTime = flWaterJumpTime;
}

float C_BasePlayer::GetSwimSoundTime() const { return m_flSwimSoundTime; }
...
#if !defined(NO_ENTITY_PREDICTION)
      if (GetPredictable()) {
        // m_flFOVTime was set to a predicted time in the future, because the
        // FOV change was predicted.
...
void C_BasePlayer::ForceSetupBonesAtTimeFakeInterpolation(
  ...
  SetupBones(pBonesOut, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING,
...
void C_BasePlayer::GetRagdollInitBoneArrays(matrix3x4_t *pDeltaBones0,
                                            matrix3x4_t *pDeltaBones1,
                                            matrix3x4_t *pCurrentBones,
                                            float boneDt) {
  if (!IsLocalPlayer()) {
    BaseClass::GetRagdollInitBoneArrays(pDeltaBones0, pDeltaBones1,
  ...
  if (ragdollCreateTime != gpGlobals->curtime) {
    ForceSetupBonesAtTimeFakeInterpolation(
        pCurrentBones, ragdollCreateTime - gpGlobals->curtime);
  } else {
    SetupBones(pCurrentBones, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING,
               gpGlobals->curtime);
  }
}
...
void C_BasePlayer::GetPredictionErrorSmoothingVector(Vector &vOffset) {
  ...
  float errorAmount =
      (gpGlobals->curtime - m_flPredictionErrorTime) / cl_smoothtime.GetFloat();
...
IRagdoll *C_BasePlayer::GetRepresentativeRagdoll() const { return m_pRagdoll; }

IMaterial *C_BasePlayer::GetHeadLabelMaterial(void) {
  if (GetClientVoiceMgr() == NULL) return NULL;
...
bool IsInFreezeCam(void) {
  ...
  if (pPlayer && pPlayer->GetObserverMode() == OBS_MODE_FREEZECAM) return true;
...
void C_BasePlayer::FogControllerChanged(bool bSnap) {
  if (m_Local.m_PlayerFog.m_hCtrl) {
    ...
    /*
    Msg("Updating Fog Target: (%d,%d,%d) %.0f,%.0f -> (%d,%d,%d) %.0f,%.0f (%.2f
    ...
    // Update the fog player's local fog data with the fog controller's data if
    // need be.
    UpdateFogController();
  }
}
...
void C_BasePlayer::UpdateFogController(void) {
  if (m_Local.m_PlayerFog.m_hCtrl) {
    // Don't bother copying while we're transitioning, since it'll be stomped in
    // UpdateFogBlend();
