...
void CHUDAutoAim::VidInit(void) {
  ...
  vgui::surface()->DrawSetTextureFile(m_textureID_ActiveReticle,
                                      "vgui/hud/autoaim", true, false);

  m_textureID_FixedReticle = vgui::surface()->CreateNewTextureID();
  vgui::surface()->DrawSetTextureFile(m_textureID_FixedReticle,
                                      "vgui/hud/xbox_reticle", true, false);
}
...
bool CHUDAutoAim::ShouldDraw(void) {
  ...
  return (
      (hud_draw_fixed_reticle.GetBool() || hud_draw_active_reticle.GetBool()) &&
      CHudElement::ShouldDraw() && !engine->IsDrawingLoadingImage());
}
...
#define AUTOAIM_ONTARGET_CROSSHAIR_SPEED \
  (ScreenWidth() / 3)  // Can cross the whole screen in 3 seconds.
...
void CHUDAutoAim::OnThink() {
  ...
  if (pLocalPlayer->m_HL2Local.m_bZooming ||
      pLocalPlayer->m_HL2Local.m_bWeaponLowered) {
    flFixedAlphaGoal = 0.0f;
  }

  m_alphaFixed = Approach(flFixedAlphaGoal, m_alphaFixed,
                          (hud_alpha_speed.GetFloat() * gpGlobals->frametime));

  switch (hud_autoaim_method.GetInt()) {
    case AUTOAIM_METHOD_RETICLE: {
      if (pLocalPlayer->m_HL2Local.m_hAutoAimTarget.Get() &&
          pLocalPlayer->m_HL2Local.m_bStickyAutoAim) {
        if (!pLocalPlayer->IsInAVehicle()) {
          ...
          if (flSpeed >= 100 && fabs(flDot) <= 0.707f) {
            ...
            Vector vecDir = pLocalPlayer->m_HL2Local.m_vecAutoAimPoint -
...
void CHUDAutoAim::Paint() {
  if (hud_draw_active_reticle.GetBool()) {
    ...
    vgui::surface()->DrawGetTextureSize(m_textureID_ActiveReticle, width,
                                        height);
  ...
  if (hud_draw_fixed_reticle.GetBool()) {
    ...
    vgui::surface()->DrawGetTextureSize(m_textureID_FixedReticle, width,
                                        height);
