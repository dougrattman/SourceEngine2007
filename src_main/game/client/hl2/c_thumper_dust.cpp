...
void FX_ThumperDust(const CEffectData &data) {
  ...
  CSmartPtr<ThumperDustEmitter> pSimple =
      ThumperDustEmitter::Create("thumperdust");
  ...
  float flScale = std::min(data.m_flScale, 255.0f);
  ...
  for (i = 0; i < numPuffs; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_DustPuff[random->RandomInt(0, 1)],
        offset);
    if (pParticle != NULL) {
...
// Input  : &data -
//-----------------------------------------------------------------------------
void ThumperDustCallback(const CEffectData &data) { FX_ThumperDust(data); }
