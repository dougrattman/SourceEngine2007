...
static int IntersectSegmentWithSurfInfos(
    const Vector &vStart, const Vector &vEnd, SurfInfo *pSurfInfos,
    const int nSurfInfos, SurfInfo **pIntersections,
    Vector *pIntersectionPositions, int nMaxIntersections) {
  if (nMaxIntersections == 0) return 0;
  ...
  for (int i = 0; i < nSurfInfos; i++) {
    ...
    if ((dot1 > 0) != (dot2 > 0)) {
      ...
      // If the intersection is behind any edge plane, then it's not inside the
      // polygon.
      unsigned long iEdge;
      for (iEdge = 0; iEdge < pSurf->m_nVerts; iEdge++) {
        VPlane edgePlane;
        edgePlane.m_Normal = pSurf->m_Plane.m_Normal.Cross(
            pSurf->m_Verts[iEdge] -
            pSurf->m_Verts[(iEdge + 1) % pSurf->m_nVerts]);
        VectorNormalize(edgePlane.m_Normal);
        edgePlane.m_Dist = edgePlane.m_Normal.Dot(pSurf->m_Verts[iEdge]);

        if (edgePlane.DistTo(vIntersection) < 0.0f) break;
      }
      ...
      if (iEdge == pSurf->m_nVerts) {
        ...
        ++nIntersections;
        if (nIntersections >= nMaxIntersections) break;
      }
    }
  }
...
void C_AR2Explosion::Start(CParticleMgr *pParticleMgr,
                           IPrototypeArgAccess *pArgs) {
  m_pParticleMgr = pParticleMgr;
  if (!pParticleMgr->AddEffect(&m_ParticleEffect, this)) return;

  if (!m_szMaterialName[0]) {
    Q_strncpy(m_szMaterialName, "particle/particle_noisesphere",
              sizeof(m_szMaterialName));
  }
...
#define NUM_DUSTEMITTER_SURFINFOS 128
  ...
  // Center of explosion.
  Vector vCenter =
      GetAbsOrigin();  // HACKHACK.. when the engine bug is fixed, use origin.
...
#else
  {
    ...
    if (ent) {
      nSurfInfos = engine->GetIntersectingSurfaces(
          ent->GetModel(), vCenter, AR2_DUST_RADIUS, true, surfInfos,
  ...
  if (nSurfInfos > 0) {
    // For nParticles*N, generate a ray and cast it out. If it hits anything,
    // spawn a particle there.
...
#define MAX_SURFINFO_INTERSECTIONS 4
        ...
        nIntersections = IntersectSegmentWithSurfInfos(
            startPos, endPos, surfInfos, nSurfInfos, pIntersected,
            vIntersections, MAX_SURFINFO_INTERSECTIONS);

        if (nIntersections) {
          int iIntersection = rand() % nIntersections;

          Vector velocity;
          // velocity.Init(-1.0f + ((float)rand()/RAND_MAX) * 2.0f, -1.0f +
          // ((float)rand()/RAND_MAX) * 2.0f, -1.0f + ((float)rand()/RAND_MAX)
          // * 2.0f); velocity = velocity * FRand(m_MinSpeed, m_MaxSpeed);
          Vector direction = (vIntersections[iIntersection] - vCenter);
          float dist = VectorNormalize(direction);
          if (dist > AR2_DUST_RADIUS) dist = AR2_DUST_RADIUS;
          ...
          Vector reflection =
              ...
              2 *
                  DotProduct(direction,
                             pIntersected[iIntersection]->m_Plane.m_Normal) *
                  pIntersected[iIntersection]->m_Plane.m_Normal;
          VectorNormalize(reflection);

          velocity = reflection * AR2_DUST_SPEED * falloffMul;
          // velocity = velocity + (vIntersections[iIntersection] - vCenter) *
          // falloffMul;

          /*
          debugoverlay->AddLineOverlay( vIntersections[iIntersection],
                                                                    vIntersections[iIntersection]
          + reflection * 64, 128, 128, 255, false, 15.0 );
          */
#if 1
          AR2ExplosionParticle *pParticle =
              (AR2ExplosionParticle *)m_ParticleEffect.AddParticle(
                  sizeof(AR2ExplosionParticle), m_MaterialHandle);
          ...
          if (pParticle) {
            ...
            pParticle->m_Dwell =
                AR2_DUST_LIFETIME +
                random->RandomFloat(0, AR2_DUST_LIFETIME_DELTA);
            nParticles++;
            break;
          }
#endif
  ...
  for (int i = nParticles; i < iParticlesToSpawn; i++) {
    ...
    float dist = VectorNormalize(direction) +
                 random->RandomFloat(0, AR2_DUST_RADIUS / 4.0);
    if (dist > AR2_DUST_RADIUS) dist = AR2_DUST_RADIUS;
    ...
    if (pParticle) {
      ...
      pParticle->m_Dwell =
          0.5 *
          (AR2_DUST_LIFETIME + random->RandomFloat(0, AR2_DUST_LIFETIME_DELTA));
    }
  }
}
...
void C_AR2Explosion::SimulateParticles(CParticleSimulateIterator *pIterator) {
  ...
  AR2ExplosionParticle *pParticle =
      (AR2ExplosionParticle *)pIterator->GetFirst();
  while (pParticle) {
    if (dt > 0.05)
    ...
    if (pParticle->m_Lifetime > pParticle->m_Dwell) {
      ...
      if (pParticle->m_Lifetime >= 0.0f) {
        // Move it (this comes after rendering to make it clear that moving the
        // particle here won't change its rendering for this frame since
        // m_TransformedPos has already been set).
        pParticle->m_Pos = pParticle->m_Pos + pParticle->m_Velocity * dt;

        // keep track of distance traveled
        pParticle->m_Dist =
            pParticle->m_Dist + pParticle->m_Velocity.Length() * dt;
...
void C_AR2Explosion::RenderParticles(CParticleRenderIterator *pIterator) {
  const AR2ExplosionParticle *pParticle =
      (const AR2ExplosionParticle *)pIterator->GetFirst();
  while (pParticle) {
    float sortKey = 0;
    if (pParticle->m_Lifetime >= 0.0f) {
      // Draw.
      ...
      if (pParticle->m_Lifetime < AR2_DUST_FADE_IN_TIME) {
        alpha =
            AR2_DUST_ALPHA * (pParticle->m_Lifetime / AR2_DUST_FADE_IN_TIME);
      } else {
      ...
      RenderParticle_ColorSizeAngle(
          pIterator->GetParticleDraw(), tPos, color, alpha,
