...
class CHudWeaponSelection : public CBaseHudWeaponSelection, public vgui::Panel {
 ...
 private:
  ...
  CPanelAnimationVarAliasType(float, m_flSelectionNumberXPos,
                              "SelectionNumberXPos", "4", "proportional_float");
  CPanelAnimationVarAliasType(float, m_flSelectionNumberYPos,
                              "SelectionNumberYPos", "4", "proportional_float");
  ...
  CPanelAnimationVar(float, m_flHorizWeaponSelectOffsetPoint, "WeaponBoxOffset",
                     "0");
};
...
void CHudWeaponSelection::ActivateFastswitchWeaponDisplay(
  ...
  if (!m_pLastWeapon || m_iSelectedSlideDir == 0 ||
    ...
    // no previous weapon or weapon selected directly or selection during slide,
    ...
    for (int i = 0; i < cWeapons; i++) {
      ...
      if (start == m_iSelectedWeaponBox) break;
    ...
    if (m_iSelectedSlideDir < 0) flStart *= -1;
    flStop = 0;

    // shorten duration for scrolling when desired weapon is farther away
    // otherwise a large skip in the same duration causes the scroll to fly too
    // fast
    flTime = numIcons * 0.20f;
    if (numIcons > 1) flTime *= 0.5f;
  }
...
void CHudWeaponSelection::Paint() {
  ...
  if (!pSelectedWeapon) return;

  // interpolate the selected box size between the small box size and the large
  // box size interpolation has been removed since there is no weapon pickup
  // animation anymore, so it's all at the largest size
  float percentageDone = 1.0f;  // std::min(1.0f, (gpGlobals->curtime -
                                // m_flPickupStartTime) /
  ...
  switch (hud_fastswitch.GetInt()) {
    case HUDTYPE_CAROUSEL: {
      ...
      if (m_iSelectedWeaponBox == -1 || m_WeaponBoxes.Count() <= 1) {
        ...
        width = (m_WeaponBoxes.Count() - 1) * (m_flLargeBoxWide + m_flBoxGap) +
                m_flLargeBoxWide;
        ...
        for (int i = 0; i < m_WeaponBoxes.Count(); i++) {
          ...
          if (i == m_iSelectedWeaponBox) {
            ...
            DrawLargeWeaponBox(pWeapon, true, xpos, ypos, m_flLargeBoxWide,
                               m_flLargeBoxTall, selectedColor, alpha, -1);
          } else {
            DrawLargeWeaponBox(pWeapon, false, xpos, ypos, m_flLargeBoxWide,
        ...
        // draw the selected weapon in the center, as a continuous scrolling
        // carosuel draw at center the current selected and all items to its
        // right
        xpos =
            GetWide() / 2 + m_flHorizWeaponSelectOffsetPoint - largeBoxWide / 2;
        ...
        while (1) {
          ...
          if (i == m_iSelectedWeaponBox && !m_flHorizWeaponSelectOffsetPoint) {
            ...
            DrawLargeWeaponBox(pWeapon, true, xpos, ypos, largeBoxWide,
                               largeBoxTall, selectedColor, alpha, -1);
          } else {
            alpha = GetWeaponBoxAlpha(false);
            DrawLargeWeaponBox(pWeapon, false, xpos, ypos, largeBoxWide,
                               largeBoxTall / 1.5f, m_BoxColor, alpha, -1);
          }

          // advance until past edge
          xpos += (largeBoxWide + m_flBoxGap);
          if (xpos >= GetWide()) break;
    ...
    case HUDTYPE_PLUS: {
      ...
      int screenCenterY = GetTall() / 2 - 15;  // Height isn't quite screen
                                               // height, so adjust for center
                                               // alignement
      ...
      for (int i = 0; i < MAX_WEAPON_SLOTS; ++i) {
        // Set the top left corner so the first box would be centered in the
        // screen.
        ...
        for (int slotPos = 0; slotPos <= lastSlotPos; ++slotPos) {
          ...
          if (i == m_iSelectedSlot && slotPos == m_iSelectedBoxPosition) {
            // This is a bit of a misnomer... we really are asking "Is this the
            // selected slot"?
            selectedWeapon = true;
          }

          // Draw the box with the appropriate icon
          DrawLargeWeaponBox(pWeapon, selectedWeapon, x, y, boxWide, boxTall,
      ...
      for (int i = 0; i < MAX_WEAPON_SLOTS; i++) {
        if (i == iActiveSlot) {
          ...
          for (int slotpos = 0; slotpos <= iLastPos; slotpos++) {
            C_BaseCombatWeapon *pWeapon = GetWeaponInSlot(i, slotpos);
            if (!pWeapon) {
              if (!hud_showemptyweaponslots.GetBool()) continue;
              DrawBox(xpos, ypos, largeBoxWide, largeBoxTall, m_EmptyBoxColor,
                      m_flAlphaOverride, bDrawBucketNumber ? i + 1 : -1);
            } else {
              bool bSelected = (pWeapon == pSelectedWeapon);
              DrawLargeWeaponBox(
                  pWeapon, bSelected, xpos, ypos, largeBoxWide, largeBoxTall,
                  bSelected ? selectedColor : m_BoxColor,
                  GetWeaponBoxAlpha(bSelected), bDrawBucketNumber ? i + 1 : -1);
            }
          ...
          if (GetFirstPos(i)) {
            ...
            DrawBox(xpos, ypos, m_flSmallBoxSize, m_flSmallBoxSize, m_BoxColor,
                    m_flAlphaOverride, i + 1);
          } else {
            // draw empty slot
    ...
    case HUDTYPE_BUCKETS: {
      ...
      if (pWeapon->GetSpriteActive()) {
        ...
        if (!pWeapon->CanBeSelected()) {
          ...
          pWeapon->GetSpriteActive()->DrawSelf(xpos + x_offs, ypos + y_offs,
                                               col);
        }

        // draw the inactive version
        pWeapon->GetSpriteInactive()->DrawSelf(xpos + x_offs, ypos + y_offs,
                                               col);
      }
    } break;
  ...
  for (int i = 0; i < MAX_WEAPONS; i++) {
    ...
    if (CanBeSelectedInHUD(pWeapon)) {
      ...
      if (weaponSlot > iCurrentSlot ||
          (weaponSlot == iCurrentSlot && weaponPosition > iCurrentPosition)) {
...
void CHudWeaponSelection::CycleToPrevWeapon(void) {
  ...
  if (!pNextWeapon) {
    ...
    pNextWeapon =
        FindPrevWeaponInWeaponSelection(MAX_WEAPON_SLOTS, MAX_WEAPON_POSITIONS);
  }
...
void CHudWeaponSelection::PlusTypeFastWeaponSwitch(int iWeaponSlot) {
  ...
  if (-1 == m_iSelectedSlot || ((m_iSelectedSlot ^ iWeaponSlot) & 1)) {
    // Changing vertical/horizontal direction. Reset the selected box position
    // to zero.
    m_iSelectedBoxPosition = 0;
    ...
    if (m_iSelectedSlot != iWeaponSlot) {
      // Decrementing within the slot. If we're at the zero position in this
      // slot, jump to the zero position of the opposite slot. This also counts
      // as our increment.
      increment = -1;
