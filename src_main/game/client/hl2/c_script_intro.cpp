...
class C_ScriptIntro : public C_BaseEntity {
 ...
 private:
  ...
  float m_flFadeDuration;  // Time it should take to reach the server's new fade
                           // alpha
  float m_flFadeTimeStartedAt;   // Time at which we last recieved a new desired
                                 // fade alpha
  float m_flFadeAlphaStartedAt;  // Alpha at which we last received a new
                                 // desired fade alpha
};
...
IMPLEMENT_CLIENTCLASS_DT(C_ScriptIntro, DT_ScriptIntro, CScriptIntro)
  RecvPropVector(RECVINFO(m_vecCameraView)),
      ...
      // Fov & fov blends
      RecvPropInt(RECVINFO(m_iFOV)), RecvPropInt(RECVINFO(m_iNextFOV)),
...
void C_ScriptIntro::ClientThink(void) {
  ...
  if ((m_flNextBlendTime - m_flBlendStartTime) != 0) {
    flPerc = std::clamp((gpGlobals->curtime - m_flBlendStartTime) /
                            (m_flNextBlendTime - m_flBlendStartTime),
                        0.0f, 1.0f);
  }
...
extern float ScriptInfo_CalculateFOV(float flFOVBlendStartTime,
                                     float flNextFOVBlendTime, int nFOV,
                                     int nNextFOV, bool bSplineRamp);
...
void C_ScriptIntro::CalculateFOV(void) {
  ...
  if (m_flNextFOVBlendTime >= gpGlobals->curtime) {
    ...
    m_IntroData.m_playerViewFOV =
        ScriptInfo_CalculateFOV(m_flFOVBlendStartTime, m_flNextFOVBlendTime,
                                m_iStartFOV, m_iNextFOV, m_bAlternateFOV);
  }
}
...
void C_ScriptIntro::CalculateAlpha(void) {
  ...
  float flNewAlpha = RemapValClamped(gpGlobals->curtime, m_flFadeTimeStartedAt,
                                     m_flFadeTimeStartedAt + m_flFadeDuration,
                                     m_flFadeAlphaStartedAt, m_flFadeAlpha);
  /*
  if ( m_IntroData.m_flCurrentFadeColor[3] != flNewAlpha )
  {
          Msg("INTRO FADING: curtime %.2f    StartedAt %.2f    Duration:
  %.2f\n", gpGlobals->curtime, m_flFadeTimeStartedAt, m_flFadeDuration ); Msg("
  TimePassed %.2f    Alpha:    %.2f\n", RemapValClamped( gpGlobals->curtime,
  m_flFadeTimeStartedAt, m_flFadeTimeStartedAt + m_flFadeDuration, 0.0, 1.0 ),
  m_IntroData.m_flCurrentFadeColor[3] );
  }
  */
