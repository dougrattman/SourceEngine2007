...
ConVar cl_npc_speedmod_intime("cl_npc_speedmod_intime", "0.25",
                              FCVAR_CLIENTDLL | FCVAR_ARCHIVE);
ConVar cl_npc_speedmod_outtime("cl_npc_speedmod_outtime", "1.5",
                               FCVAR_CLIENTDLL | FCVAR_ARCHIVE);

IMPLEMENT_CLIENTCLASS_DT(C_BaseHLPlayer, DT_HL2_Player, CHL2_Player)
  RecvPropDataTable(RECVINFO_DT(m_HL2Local), 0,
                    &REFERENCE_RECV_TABLE(DT_HL2Local)),
...
static ConCommand dropprimary(
    "dropprimary", CC_DropPrimary,
...
C_BaseHLPlayer::C_BaseHLPlayer() {
  AddVar(&m_Local.m_vecPunchAngle, &m_Local.m_iv_vecPunchAngle,
         LATCH_SIMULATION_VAR);
  AddVar(&m_Local.m_vecPunchAngleVel, &m_Local.m_iv_vecPunchAngleVel,
         LATCH_SIMULATION_VAR);
...
float C_BaseHLPlayer::GetFOV() {
  ...
  // Clamp FOV in MP
  float min_fov = (gpGlobals->maxClients == 1) ? 5 : default_fov.GetInt();
...
float C_BaseHLPlayer::GetZoom(void) {
  ...
  if ((m_flZoomStart != m_flZoomEnd) && (m_flZoomRate > 0.0f)) {
    float deltaTime =
        (float)(gpGlobals->curtime - m_flZoomStartTime) / m_flZoomRate;

    if (deltaTime >= 1.0f) {
      // If we're past the zoom time, just take the new value and stop lerping
      fFOV = m_flZoomStart = m_flZoomEnd;
    } else {
      fFOV = SimpleSplineRemapVal(deltaTime, 0.0f, 1.0f, m_flZoomStart,
                                  m_flZoomEnd);
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Hack to zero out player's pitch, use value from poseparameter
// instead Input  : flags - Output : int
//-----------------------------------------------------------------------------
...
// Input:   pos - position to test move to, fVertDist - how far to trace
// downwards to see if the player would fall,
//			radius - how close the player can be to the object, objPos -
//position of the object to avoid, 			objDir - direction the object is travelling
//-----------------------------------------------------------------------------
bool C_BaseHLPlayer::TestMove(const Vector &pos, float fVertDist, float radius,
                              const Vector &objPos, const Vector &objDir) {
  trace_t trUp;
  trace_t trOver;
  trace_t trDown;
  float flHit1, flHit2;

  UTIL_TraceHull(GetAbsOrigin(), pos, GetPlayerMins(), GetPlayerMaxs(),
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &trOver);
  if (trOver.fraction < 1.0f) {
    // check if the endpos intersects with the direction the object is
    // travelling.  if it doesn't, this is a good direction to move.
    if (objDir.IsZero() ||
        (IntersectInfiniteRayWithSphere(objPos, objDir, trOver.endpos, radius,
                                        &flHit1, &flHit2) &&
      ...
      UTIL_TraceHull(GetAbsOrigin(),
                     GetAbsOrigin() + Vector(0, 0, m_Local.m_flStepSize),
                     GetPlayerMins(), GetPlayerMaxs(), MASK_SOLID_BRUSHONLY,
                     this, COLLISION_GROUP_NONE, &trUp);

      // do a trace from the stepped up height
      UTIL_TraceHull(trUp.endpos,
                     pos + Vector(0, 0, trUp.endpos.z - trUp.startpos.z),
                     GetPlayerMins(), GetPlayerMaxs(), MASK_SOLID_BRUSHONLY,
                     this, COLLISION_GROUP_NONE, &trOver);

      if (trOver.fraction < 1.0f) {
        // check if the endpos intersects with the direction the object is
        // travelling.  if it doesn't, this is a good direction to move.
        if (objDir.IsZero() ||
            IntersectInfiniteRayWithSphere(objPos, objDir, trOver.endpos,
                                           radius, &flHit1, &flHit2) &&
                ((flHit1 >= 0.0f) || (flHit2 >= 0.0f))) {
          return false;
        }
      }
  ...
  if (trDown.fraction == 1.0f) return false;
...
//-----------------------------------------------------------------------------
void C_BaseHLPlayer::PerformClientSideObstacleAvoidance(float flFrameTime,
                                                        CUserCmd *pCmd) {
  ...
  if (curspeed > 150.0f) {
    curspeed = std::min(2048.0f, curspeed);
  ...
  if (fabs(pCmd->forwardmove) > 0.0f || fabs(pCmd->sidemove) > 0.0f) {
    istryingtomove = true;
  ...
  if (istryingtomove == true) radius *= 1.3f;

  CPlayerAndObjectEnumerator avoid(radius);
  partition->EnumerateElementsInSphere(PARTITION_CLIENT_SOLID_EDICTS,
                                       GetAbsOrigin(), radius, false, &avoid);
  ...
  // "true" : "false", ismovingforward ? "true" : "false"  );
  ...
  for (int i = 0; i < c; i++) {
    ...
    float objectradius =
        0.5f * sqrt(objSize.x * objSize.x + objSize.y * objSize.y);

    // Don't run this code if the NPC is not moving UNLESS we are in stuck
    // inside of them.
    if (!obj->IsMoving() && flDist > objectradius) continue;
    ...
    if (!IntersectInfiniteRayWithSphere(GetAbsOrigin(), vRayDir,
                                        obj->GetAbsOrigin(), radius, &flHit1,
                                        &flHit2))
      continue;
    ...
    if (flNPCSpeed > 0.001f) {
      // This NPC is moving. First try deflecting the player left or right
      // relative to the NPC's velocity. Start with whatever side they're on
      // relative to the NPC's velocity.
      Vector vecNPCTrajectoryRight =
          CrossProduct(vecNPCVelocity, Vector(0, 0, 1));
      ...
      for (int nTries = 0; nTries < 2; nTries++) {
        ...
        if (TestMove(vTestPosition, size.z * 2, radius * 2, obj->GetAbsOrigin(),
                     vMoveDir)) {
    ...
    // If running, then do a lot more sideways veer since we're not going to do
    // anything to
    //  forward velocity
    if (istryingtomove) {
      sidescale = 6.0f;
  ...
  // Clamp the move to within legal limits, preserving direction. This is a
  // little complicated because we have different limits for forward, back, and
  // side
...
void C_BaseHLPlayer::PerformClientSideNPCSpeedModifiers(float flFrameTime,
                                                        CUserCmd *pCmd) {
  if (m_hClosestNPC == NULL) {
    if (m_flSpeedMod != cl_forwardspeed.GetFloat()) {
      float flDeltaTime = (m_flSpeedModTime - gpGlobals->curtime);
      m_flSpeedMod =
          RemapValClamped(flDeltaTime, cl_npc_speedmod_outtime.GetFloat(), 0,
                          m_flExitSpeedMod, cl_forwardspeed.GetFloat());
    }
  } else {
    ...
    if (pNPC) {
      ...
      if (flDist < pNPC->GetSpeedModifyRadius()) {
        // Now, only slowdown if we're facing & running parallel to the target's
        // movement Facing check first (in 2D)
      ...
      if (!bShouldModSpeed) {
        m_hClosestNPC = NULL;
        m_flSpeedModTime =
            gpGlobals->curtime + cl_npc_speedmod_outtime.GetFloat();
        m_flExitSpeedMod = m_flSpeedMod;
        return;
      } else {
        if (m_flSpeedMod != pNPC->GetSpeedModifySpeed()) {
          float flDeltaTime = (m_flSpeedModTime - gpGlobals->curtime);
          m_flSpeedMod = RemapValClamped(
              flDeltaTime, cl_npc_speedmod_intime.GetFloat(), 0,
              cl_forwardspeed.GetFloat(), pNPC->GetSpeedModifySpeed());
        }
      }
    }
  }
  ...
  if (pCmd->forwardmove > 0.0f) {
    pCmd->forwardmove =
        std::clamp(pCmd->forwardmove, -m_flSpeedMod, m_flSpeedMod);
  } else {
    pCmd->forwardmove =
        std::clamp(pCmd->forwardmove, -m_flSpeedMod, m_flSpeedMod);
  }
  pCmd->sidemove = std::clamp(pCmd->sidemove, -m_flSpeedMod, m_flSpeedMod);
