...
IMPLEMENT_CLIENTCLASS_DT(C_WeaponPhysCannon, DT_WeaponPhysCannon,
                         CWeaponPhysCannon)
...
bool C_WeaponPhysCannon::SetupEmitter(void) {
  if (!m_pLocalEmitter.IsValid()) {
    m_pLocalEmitter = CLocalSpaceEmitter::Create("physpowerup", GetRefEHandle(),
                                                 LookupAttachment("core"));

    if (m_pLocalEmitter.IsValid() == false) return false;
  }

  if (!m_pAttractor.IsValid()) {
    m_pAttractor = CParticleAttractor::Create(vec3_origin, "physpowerup_att");

    if (m_pAttractor.IsValid() == false) return false;
  }

  if (!m_pEmitter.IsValid()) {
    m_pEmitter = CSimpleEmitter::Create("physpowerup_glow");

    if (m_pEmitter.IsValid() == false) return false;
  }
...
int C_WeaponPhysCannon::DrawModel(int flags) {
  ...
  if (gpGlobals->frametime == 0) return BaseClass::DrawModel(flags);
  ...
  if (SetupEmitter()) {
    ...
    if (!pAnimating->HitboxToWorldTransforms(hitboxbones)) return 0;
    ...
    if (m_bIsCurrentlyUpgrading) {
      ...
      for (i = 0; i < set->numhitboxes; ++i) {
        ...
        int numParticles = std::clamp(4.0f * fadePerc, 1.0f, 3.0f);
        ...
        for (int j = 0; j < numParticles; j++) {
          ...
          sParticle = (SimpleParticle *)m_pEmitter->AddParticle(
              ...
              m_pEmitter->GetPMaterial("effects/combinemuzzle1"),
              vecAbsOrigin + offset);
  ...
  // Do the core effects
  for (int i = 0; i < 4; i++) {
    sParticle = (SimpleParticle *)m_pLocalEmitter->AddParticle(
        sizeof(SimpleParticle),
        m_pLocalEmitter->GetPMaterial("effects/strider_muzzle"), vec3_origin);
  ...
  if (m_bWasUpgraded && m_bIsCurrentlyUpgrading) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      sParticle = (SimpleParticle *)m_pAttractor->AddParticle(
          sizeof(SimpleParticle),
          m_pAttractor->GetPMaterial("effects/strider_muzzle"), offset);
...
void C_WeaponPhysCannon::ClientThink(void) {
  if (m_flTimeIgnoreForceView > gpGlobals->curtime) return;

  float flTime = (m_flTimeForceView - gpGlobals->curtime);

  if (flTime < 0.0f) return;
