...
class C_Strider : public C_AI_BaseNPC {
 ...
 public:
  ...
  virtual void CalculateIKLocks(float currentTime) {
    // NOTE: All strider IK is solved on the server, enable this to do it
    // client-side
...
// NOTE: Beam widths are half-widths or radii, so this is a beam that represents
// a cylinder with 2" radius
...
CStriderFXEnvelope::CStriderFXEnvelope() {
  ...
  AddKey(STRIDERFX_NARROW_BEAM_COLOR,
         CSimpleKeyInterp(1.25, KEY_ACCELERATE, 1.0));
  ...
  AddKey(STRIDERFX_WIDE_BEAM_LENGTH,
         CSimpleKeyInterp(1.5, KEY_ACCELERATE, 0.0));
...
void DrawSpriteTangentSpace(const Vector &vecOrigin, float flWidth,
                            float flHeight, color32 color) {
...
void Strider_DrawSprite(const Vector &vecOrigin, float size,
                        const color32 &color) {
  DrawSpriteTangentSpace(vecOrigin, size, size, color);
}

void Strider_DrawLine(const Vector &start, const Vector &end, float width,
                      IMaterial *pMaterial, const color32 &color) {
...
int C_StriderFX::DrawModel(int) {
  ...
  // This forces time to drive from the main clock instead of being integrated
  // per-draw below that way the effect moves on even when culled for visibility
  ...
  float gunFractionVisible =
      PixelVisibility_FractionVisible(gunParams, &m_queryHandleGun);
  bool gunVisible = gunFractionVisible > 0.0f ? true : false;

  // draw the narrow beam
  if (hasParam[STRIDERFX_NARROW_BEAM_COLOR] &&
      hasParam[STRIDERFX_NARROW_BEAM_SIZE]) {
    IMaterial *pMat = materials->FindMaterial("sprites/bluelaser1",
  ...
  // draw the wide beam
  if (hasParam[STRIDERFX_WIDE_BEAM_COLOR] &&
      hasParam[STRIDERFX_WIDE_BEAM_SIZE]) {
    IMaterial *pMat = materials->FindMaterial("effects/blueblacklargebeam",
                                              TEXTURE_GROUP_CLIENT_EFFECTS);
  ...
  if (hasParam[STRIDERFX_WARP_SCALE] && !hasParam[STRIDERFX_BUBBLE_SIZE] &&
    ...
    IMaterial *pMat = materials->FindMaterial("effects/strider_pinch_dudv",
                                              TEXTURE_GROUP_CLIENT_EFFECTS);
    float size = WARP_SIZE;
    float refract =
        params[STRIDERFX_WARP_SCALE].x * WARP_REFRACT * gunFractionVisible;
  ...
  // darkening sprite
  // glowy blue flare sprite
  if (hasParam[STRIDERFX_FLARE_COLOR] && hasParam[STRIDERFX_FLARE_SIZE] &&
      hasParam[STRIDERFX_DARKNESS] && gunVisible) {
    IMaterial *pMat = materials->FindMaterial("effects/blueblackflash",
                                              TEXTURE_GROUP_CLIENT_EFFECTS);
  ...
  if (hasParam[STRIDERFX_BUBBLE_SIZE]) {
    ...
    if (endVisible) {
      ...
      IMaterial *pMat = materials->FindMaterial("effects/strider_bulge_dudv",
                                                TEXTURE_GROUP_CLIENT_EFFECTS);
      float refract = endFractionVisible * WARP_BUBBLE_REFRACT *
                      params[STRIDERFX_BUBBLE_REFRACT].x;
...
void C_Strider::OnDataChanged(DataUpdateType_t updateType) {
  if (updateType == DATA_UPDATE_CREATED) {
    // We need to have our render bounds defined or shadow creation won't work
    // correctly
    ClientThink();
    ClientThinkList()->SetNextClientThink(GetClientHandle(),
                                          CLIENT_THINK_ALWAYS);
  }
...
void C_Strider::ClientThink() {
  ...
  // is because of IK. The code below recomputes bones so as to get at the
  // hitboxes, which causes IK to trigger, which causes raycasts against the
  // other entities to occur, which is illegal to do while in the Relink phase.
  ...
  // True argument because the origin may have stayed the same, but the size is
  // expected to always change
...
void MuzzleFlash_Strider(ClientEntityHandle_t hEntity, int attachmentIndex) {
  ...
  CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create(
      "MuzzleFlash_Strider", hEntity, attachmentIndex);
  ...
  for (int i = 1; i < FRONT_LENGTH; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        ...
        pSimple->GetPMaterial(
            VarArgs("effects/combinemuzzle%d", random->RandomInt(1, 2))),
        offset);
    ...
    pParticle->m_uchStartSize =
        ((random->RandomFloat(6.0f, 8.0f) * (FRONT_LENGTH - (i)) /
          (FRONT_LENGTH * 0.75f)) *
         flScale);
...
#define SIDE_LENGTH 8
  ...
  pParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), pSimple->GetPMaterial("effects/strider_muzzle"),
      vec3_origin);
  ...
  if (entityIndex >= 0) {
    dlight_t *el =
        effects->CL_AllocElight(LIGHT_INDEX_MUZZLEFLASH + entityIndex);
...
void StriderBlood(const Vector &origin, const Vector &normal, float scale) {
  ...
  luminosity = std::max(200.0f, luminosity * 255);
  ...
  for (i = 0; i < 64; i++) {
    ...
    tParticle->m_vecVelocity =
        offDir * random->RandomFloat(BLOOD_MIN_SPEED * flScale * 2.0f,
                                     BLOOD_MAX_SPEED * flScale * 2.0f);
    ...
    tParticle->m_flLength =
        random->RandomFloat(length * 0.5f, length) * flScale;
