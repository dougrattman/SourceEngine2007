...
void FX_AirboatGunImpact(const Vector &origin, const Vector &normal,
                         int scale) {
  // Normal metal spark
  FX_MetalSpark(origin, normal, normal, scale);

  // Add a quad to highlite the hit point
  FX_AddQuad(origin, normal, random->RandomFloat(16, 32),
             random->RandomFloat(32, 48), 0.75f, 1.0f, 0.0f, 0.4f,
             random->RandomInt(0, 360), 0, Vector(1.0f, 1.0f, 1.0f), 0.05f,
             "effects/combinemuzzle2_nocull",
             (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));
}

// Purpose: Handle weapon impacts from the airboat gun shooting (cheaper
// versions)
void ImpactAirboatGunCallback(const CEffectData &data) {
  VPROF_BUDGET("ImpactAirboatGunCallback",
  ...
  // If we hit, perform our custom effects and play the sound. Don't create
  // decals
  if (Impact(vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity, tr,
             IMPACT_NODECAL | IMPACT_REPORT_RAGDOLL_IMPACTS)) {
...
// Purpose: Handle weapon impacts from the helicopter shooting (cheaper
// versions)
void ImpactHelicopterCallback(const CEffectData &data) {
  VPROF_BUDGET("ImpactHelicopterCallback",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  ...
  if (Impact(vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity, tr,
    ...
    if ((iMaterial == CHAR_TEX_METAL) || (iMaterial == CHAR_TEX_COMPUTER)) {
      PerformCustomEffects(vecOrigin, tr, vecShotDir, iMaterial, 1.0,
                           FLAGS_CUSTIOM_EFFECTS_NOFLECKS);
    }
  }
