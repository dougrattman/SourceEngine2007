...
void CHudCredits::ReadParams(KeyValues *pKeyValue) {
  ...
  Q_strncpy(m_szLogo, pKeyValue->GetString("logo", "HALF-LIFE'"),
            sizeof(m_szLogo));
    ...
    if (i == m_CreditsList.Count() - 1) {
      if (m_bLastOneInPlace == false) {
        pCredit->flYPos -= gpGlobals->frametime *
                           ((float)g_iCreditsPixelHeight / m_flScrollTime);

        if ((int)pCredit->flYPos + (iFontTall / 2) <= iTall / 2) {
          m_bLastOneInPlace = true;

          // 360 certification requires that we not hold a static image too
          // long.
...
void CHudCredits::DrawLogo(void) {
  ...
  switch (m_iLogoState) {
    case LOGO_FADEIN: {
      ...
      m_Alpha =
          std::max(0.0f, RemapValClamped(flDeltaTime, 5.0f, 0, -128, 255));
...
// Purpose:
//-----------------------------------------------------------------------------
float CHudCredits::FadeBlend(float fadein, float fadeout, float hold,
                             float localTime) {
...
void CHudCredits::PrepareLine(vgui::HFont hFont, char const *pchLine) {
  ...
  if (pchLine[0] == '#') {
    g_pVGuiLocalize->ConstructString(unicode, sizeof(unicode),
                                     g_pVGuiLocalize->Find(pchLine), 0);
  } else {
