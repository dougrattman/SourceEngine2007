...
void GunshipTracerCallback(const CEffectData &data) {
  ...
  FX_GunshipTracer((Vector &)data.m_vStart, (Vector &)data.m_vOrigin,
                   flVelocity, bWhiz);
}
...
void StriderTracerCallback(const CEffectData &data) {
  float flVelocity = data.m_flScale;
  bool bWhiz = (data.m_fFlags & TRACER_FLAG_WHIZ);
  FX_StriderTracer((Vector &)data.m_vStart, (Vector &)data.m_vOrigin,
                   flVelocity, bWhiz);
}
...
void HunterTracerCallback(const CEffectData &data) {
  float flVelocity = data.m_flScale;
  bool bWhiz = (data.m_fFlags & TRACER_FLAG_WHIZ);
  FX_HunterTracer((Vector &)data.m_vStart, (Vector &)data.m_vOrigin, flVelocity,
                  bWhiz);
}
...
void GaussTracerCallback(const CEffectData &data) {
  float flVelocity = data.m_flScale;
  bool bWhiz = (data.m_fFlags & TRACER_FLAG_WHIZ);
  FX_GaussTracer((Vector &)data.m_vStart, (Vector &)data.m_vOrigin, flVelocity,
                 bWhiz);
}
...
void AirboatGunHeavyTracerCallback(const CEffectData &data) {
  ...
  if (flTotalDist <= 64) return;

  float flLength = random->RandomFloat(300.0f, 400.0f);
  float flLife =
      (flTotalDist + flLength) /
      flVelocity;  // NOTENOTE: We want the tail to finish its run as well

  // Add it
  FX_AddDiscreetLine(vecStart, vecShotDir, flVelocity, flLength, flTotalDist,
                     5.0f, flLife, "effects/gunshiptracer");
}
...
void AirboatGunTracerCallback(const CEffectData &data) {
  ...
  // Add it
  FX_AddDiscreetLine(vecStart, vecShotDir, flVelocity, flLength, flTotalDist,
                     2.0f, flLife, "effects/gunshiptracer");
}
...
void FX_PlayerAR2Tracer(const Vector &start, const Vector &end) {
  ...
  // Create the line
  CFXStaticLine *tracerLine = new CFXStaticLine(
      "Tracer", dStart, dEnd, random->RandomFloat(6.0f, 12.0f), 0.01f,
      "effects/gunshiptracer", 0);
  assert(tracerLine);
...
void FX_AR2Tracer(Vector &start, Vector &end, int velocity, bool makeWhiz) {
  ...
  FX_AddDiscreetLine(start, dir, velocity, length, dist,
                     random->RandomFloat(0.5f, 1.5f), life,
...
void AR2ExplosionCallback(const CEffectData &data) {
  ...
  // Ground splash
  FX_AddQuad(data.m_vOrigin, data.m_vNormal, data.m_flRadius,
             data.m_flRadius * 4.0f, 0.85f, 1.0f, 0.0f, 0.25f,
             random->RandomInt(0, 360), random->RandomFloat(-4, 4),
             Vector(1.0f, 1.0f, 1.0f), lifetime, "effects/combinemuzzle1",
  ...
  start = data.m_vOrigin;
        ...
        (data.m_vNormal * random->RandomFloat(radius * 2.0f, radius * 4.0f));
...
void AR2ImpactCallback(const CEffectData &data) {
  FX_AddQuad(data.m_vOrigin, data.m_vNormal, random->RandomFloat(24, 32), 0,
             0.75f, 1.0f, 0.0f, 0.4f, random->RandomInt(0, 360), 0,
             Vector(1.0f, 1.0f, 1.0f), 0.25f, "effects/combinemuzzle2_nocull",
             (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));
}
...
//-----------------------------------------------------------------------------
void CreateMuzzleflashELight(const Vector &origin, int exponent, int nMinRadius,
                             int nMaxRadius, ClientEntityHandle_t hEntity) {
...
void MuzzleFlash_Airboat(ClientEntityHandle_t hEntity, int attachmentIndex) {
  ...
  for (int i = 1; i < 7; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), pMuzzle[random->RandomInt(0, 1)], offset);
    ...
    pParticle->m_uchStartSize =
        ((random->RandomFloat(6.0f, 8.0f) * (9 - (i)) / 7) * flScale);
  ...
  // Tack on the smoke
  pParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), pSimple->GetPMaterial("sprites/ar2_muzzle1"),
      vec3_origin);
...
void MuzzleFlash_Chopper(ClientEntityHandle_t hEntity, int attachmentIndex) {
  ...
  for (int i = 1; i < 7; i++) {
    ...
    pParticle->m_uchStartSize =
        ((random->RandomFloat(6.0f, 8.0f) * (10 - (i)) / 7) * flScale);
    pParticle->m_uchEndSize = pParticle->m_uchStartSize;
    pParticle->m_flRoll = random->RandomInt(0, 360);
    pParticle->m_flRollDelta = 0.0f;
  }
...
void MuzzleFlash_Gunship(ClientEntityHandle_t hEntity, int attachmentIndex) {
  ...
  pParticle->m_uchStartSize =
      ((random->RandomFloat(6.0f, 8.0f) * 10.0 / 7.0) * flScale);
  pParticle->m_uchEndSize = pParticle->m_uchStartSize;
  pParticle->m_flRoll = random->RandomInt(0, 360);
  pParticle->m_flRollDelta = 0.0f;
