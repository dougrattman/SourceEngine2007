...
static void DrawSpriteTangentSpace(const Vector &vecOrigin, float flWidth,
...
void Gunship_DrawSprite(const Vector &vecOrigin, float size,
                        const color32 &color, bool glow) {
  if (glow) {
    pixelvis_queryparams_t params;
    params.Init(vecOrigin);
    if (PixelVisibility_FractionVisible(params, NULL) <= 0.0f) return;
  }
...
int C_GunshipFX::DrawModel(int) {
  ...
  if (hasParam[GUNSHIPFX_NARROW_BEAM_COLOR] &&
      hasParam[GUNSHIPFX_NARROW_BEAM_SIZE]) {
...
class C_CombineGunship : public C_BaseHelicopter {
 ...
 public:
  ...
  virtual ~C_CombineGunship(void) { m_cannonFX.EffectShutdown(); }
...
IMPLEMENT_CLIENTCLASS_DT(C_CombineGunship, DT_CombineGunship,
                         CNPC_CombineGunship)
...
void ImpactGunshipCallback(const CEffectData &data) {
  ...
  C_BaseEntity *pEntity =
      ParseImpactData(data, &vecOrigin, &vecStart, &vecShotDir, nSurfaceProp,
                      iMaterial, iDamageType, iHitbox);

  if (!pEntity) return;

  // If we hit, perform our custom effects and play the sound
  if (Impact(vecOrigin, vecStart, iMaterial, iDamageType, iHitbox, pEntity,
             tr)) {
