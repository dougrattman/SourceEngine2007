...
IMPLEMENT_CLIENTCLASS_DT(C_PropCombineBall, DT_PropCombineBall,
                         CPropCombineBall)
  RecvPropBool(RECVINFO(m_bEmit)), RecvPropFloat(RECVINFO(m_flRadius)),
      RecvPropBool(RECVINFO(m_bHeld)), RecvPropBool(RECVINFO(m_bLaunched)),
END_RECV_TABLE()
...
bool C_PropCombineBall::InitMaterials(void) {
  ...
  if (m_pBlurMaterial == NULL) {
    m_pBlurMaterial =
        materials->FindMaterial("effects/ar2_altfire1b", NULL, false);
  ...
  if (m_pBodyMaterial == NULL) {
    m_pBodyMaterial =
        materials->FindMaterial("effects/ar2_altfire1", NULL, false);
  ...
  if (m_pFlickerMaterial == NULL) {
    m_pFlickerMaterial =
        materials->FindMaterial("effects/combinemuzzle1", NULL, false);
...
void C_PropCombineBall::DrawMotionBlur(void) {
  ...
  float stepSize = std::min((speed * 0.5f), 4.0f);
...
//			color -
//-----------------------------------------------------------------------------
void DrawHaloOriented(const Vector &source, float scale, float const *color,
                      float roll) {
  Vector point, screen;
...
int C_PropCombineBall::DrawModel(int flags) {
  ...
  if (!InitMaterials()) {
    // NOTENOTE: This means that a material was not found for the combine ball,
    // so it may not render!
    AssertOnce(0);
    return 0;
  }
...
void CombineBallImpactCallback(const CEffectData &data) {
  // Quick flash
  FX_AddQuad(data.m_vOrigin, data.m_vNormal, data.m_flRadius * 10.0f, 0, 0.75f,
             1.0f, 0.0f, 0.4f, random->RandomInt(0, 360), 0,
             Vector(1.0f, 1.0f, 1.0f), 0.25f, "effects/combinemuzzle1_nocull",
             (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));

  // Lingering burn
  FX_AddQuad(data.m_vOrigin, data.m_vNormal, data.m_flRadius * 2.0f,
             data.m_flRadius * 4.0f, 0.75f, 1.0f, 0.0f, 0.4f,
             random->RandomInt(0, 360), 0, Vector(1.0f, 1.0f, 1.0f), 0.5f,
