...
int C_LocalTempEntity::DrawStudioModel(int flags) {
  VPROF_BUDGET("C_LocalTempEntity::DrawStudioModel",
  ...
  if (!GetModelPtr()) return drawn;

  if (m_pfnDrawHelper) {
    drawn = (*m_pfnDrawHelper)(this, flags);
  } else {
    drawn = modelrender->DrawModel(flags, this, MODEL_INSTANCE_INVALID, index,
                                   GetModel(), GetAbsOrigin(), GetAbsAngles(),
...
int C_LocalTempEntity::DrawModel(int flags) {
  ...
  switch (modelinfo->GetModelType(GetModel())) {
    case mod_sprite:
      drawn = DrawSprite(this, GetModel(), GetAbsOrigin(), GetAbsAngles(),
                         m_flFrame,  // sprite frame to render
                         m_nBody > 0 ? cl_entitylist->GetBaseEntity(m_nBody)
                                     : NULL,  // attach to
...
bool C_LocalTempEntity::Frame(float frametime, int framenumber) {
  ...
  m_vecTempEntVelocity =
      m_vecTempEntVelocity + (m_vecTempEntAcceleration * frametime);
  ...
  if (flags & FTENT_PLYRATTACHMENT) {
    ...
    SetLocalOrigin(Vector(
        x + sin(m_vecTempEntVelocity[2] +
                gpGlobals->curtime /* * anim.prevframe */) *
                (10 * m_flSpriteScale),
        y + sin(m_vecTempEntVelocity[2] + fastFreq + 0.7) *
                (8 * m_flSpriteScale),
    ...
    SetLocalOrigin(GetLocalOrigin() +
                   Vector(m_vecTempEntVelocity[0] * frametime +
                              8 * sin(gpGlobals->curtime * 20),
                          m_vecTempEntVelocity[1] * frametime +
                              4 * sin(gpGlobals->curtime * 30),
  ...
  if (flags & (FTENT_COLLIDEALL | FTENT_COLLIDEWORLD)) {
    ...
    if (flags & FTENT_COLLIDEALL) {
      ...
      if (cl_fasttempentcollision.GetInt() > 0 &&
          flags & FTENT_USEFASTCOLLISIONS) {
        if (m_iLastCollisionFrame + cl_fasttempentcollision.GetInt() >
      ...
      if (bShouldCollide == true) {
        // If the FTENT_COLLISIONGROUP flag is set, use the entity's collision
        // group
        ...
        UTIL_TraceLine(vPrevOrigin, GetLocalOrigin(), MASK_SOLID,
                       GetOwnerEntity(), collisionGroup, &trace);
      ...
      UTIL_TraceLine(m_vecPrevLocalOrigin, GetLocalOrigin(), MASK_SOLID,
                     &traceFilter, &trace);
      if (trace.fraction != 1) {
        traceFraction = trace.fraction;
        VectorCopy(trace.plane.normal, traceNormal);
    ...
    if (traceFraction != 1)  // Decent collision now, and damping works
    {
      ...
      if (flags & (FTENT_GRAVITY | FTENT_SLOWGRAVITY)) {
        ...
        {
          if (m_vecTempEntVelocity[2] <= 0 &&
              m_vecTempEntVelocity[2] >= gravity * 3) {
            damp = 0;  // Stop
            flags &= ~(FTENT_ROTATE | FTENT_GRAVITY | FTENT_SLOWGRAVITY |
                       FTENT_COLLIDEWORLD | FTENT_SMOKETRAIL);
      ...
      if (m_pszImpactEffect) {
        ...
        IClientNetworkable *pClient =
            cl_entitylist->GetClientEntity(clientIndex);
        ...
        if (trace.m_pEnt) {
          data.m_hEntity =
              ClientEntityList().EntIndexToHandle(trace.m_pEnt->entindex());
        }
      ...
      if (flags & FTENT_CLIENTSIDEPARTICLES) {
        // Stop the emission of particles on collision - removed from the
        // ClientEntityList on removal from the tempent pool.
      ...
      if (flags & FTENT_COLLIDEKILL) {
        ...
        if (damp != 0) {
          ...
          VectorMA(m_vecTempEntVelocity, -proj * 2, traceNormal,
                   m_vecTempEntVelocity);
...
void C_LocalTempEntity::AddParticleEffect(const char *pszParticleEffect) {
  ...
  if (!pszParticleEffect || (pszParticleEffect[0] == '\0')) return;

  // Check to see that we don't already have a particle effect.
  if ((flags & FTENT_CLIENTSIDEPARTICLES) != 0) return;
  ...
  // Set the particle flag on the temp entity and save the name of the particle
  // effect.
...
//-----------------------------------------------------------------------------
// Purpose: This helper keeps track of batches of "breakmodels" so that they can
// all share the lighting origin
//  of the first of the group (because the server sends down 15 chunks at a
//  time, and rebuilding 15 light cache entries for a map with a lot of
//  worldlights is really slow).
//-----------------------------------------------------------------------------
class CBreakableHelper {
 ...
 private:
  // A context is the first master until the next one, which starts a new
  // context
...
void CBreakableHelper::Remove(C_LocalTempEntity *entity) {
  for (unsigned short i = m_Breakables.Head();
       i != m_Breakables.InvalidIndex();) {
...
//-----------------------------------------------------------------------------
// Purpose: For a given breakable, find the "first" or head object and use it's
// current
...
//			speed -
//-----------------------------------------------------------------------------
void CTempEnts::Bubbles(const Vector &mins, const Vector &maxs, float height,
                        int modelIndex, int count, float speed) {
  C_LocalTempEntity *pTemp;
  const model_t *model;
  int i, frameCount;
  float sine, cosine;
  ...
  for (i = 0; i < count; i++) {
    ...
    pTemp->die =
        gpGlobals->curtime + ((height - (origin[2] - mins[2])) / zspeed) - 0.1;
    pTemp->m_flFrame = random->RandomInt(0, frameCount - 1);
...
//			speed -
//-----------------------------------------------------------------------------
void CTempEnts::BubbleTrail(const Vector &start, const Vector &end,
                            float flWaterZ, int modelIndex, int count,
                            float speed) {
  C_LocalTempEntity *pTemp;
  const model_t *model;
  int i, frameCount;
...
//-----------------------------------------------------------------------------
// Purpose: Only used by BreakModel temp ents for now.  Allows them to share a
// single
//  lighting origin amongst a group of objects.  If the master object goes away,
//  the next object in the group becomes the new lighting origin, etc.
// Input  : *entity -
//			flags -
// Output : int
//-----------------------------------------------------------------------------
int BreakModelDrawHelper(C_LocalTempEntity *entity, int flags) {
  ...
  // This is the main change, look up a lighting origin from the helper
  // singleton
...
//			flags -
//-----------------------------------------------------------------------------
void CTempEnts::BreakModel(const Vector &pos, const QAngle &angles,
                           const Vector &size, const Vector &dir,
                           float randRange, float life, int count,
  ...
  if (count == 0) {
    ...
    count = (size[0] * size[1] + size[1] * size[2] + size[2] * size[0]) /
            (3 * SHARD_VOLUME * SHARD_VOLUME);
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    pTemp->SetVelocity(
        Vector(dir[0] + random->RandomFloat(-randRange, randRange),
               dir[1] + random->RandomFloat(-randRange, randRange),
               dir[2] + random->RandomFloat(0, randRange)));

    pTemp->die = gpGlobals->curtime + life +
                 random->RandomFloat(0, 1);  // Add an extra 0-1 secs of life

    // We use a special rendering function because these objects will want to
    // share their lighting
    //  origin with the first/master object.  Prevents a huge spike in Light
    //  Cache building in maps with many worldlights.
    pTemp->SetDrawHelper(BreakModelDrawHelper);
    g_BreakableHelper.Insert(pTemp, isSlave);
  }
}

void CTempEnts::PhysicsProp(int modelindex, int skin, const Vector &pos,
...
//			*pOwner -
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::ClientProjectile(const Vector &vecOrigin,
                                               const Vector &vecVelocity,
                                               const Vector &vecAcceleration,
                                               int modelIndex, int lifetime,
                                               CBaseEntity *pOwner,
  ...
  if (pszParticleEffect) {
    ...
    // Set the particle flag on the temp entity and save the name of the
    // particle effect.
...
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::TempSprite(const Vector &pos, const Vector &dir,
                                         float scale, int modelIndex,
                                         int rendermode, int renderfx, float a,
                                         float life, int flags,
                                         const Vector &normal) {
  C_LocalTempEntity *pTemp;
  const model_t *model;
  int frameCount;
...
//			iRand -
//-----------------------------------------------------------------------------
void CTempEnts::Sprite_Spray(const Vector &pos, const Vector &dir,
                             int modelIndex, int count, int speed, int iRand) {
  C_LocalTempEntity *pTemp;
  const model_t *pModel;
  float noise;
  float znoise;
...
void CTempEnts::Sprite_Trail(const Vector &vecStart, const Vector &vecEnd,
                             int modelIndex, int nCount, float flLife,
                             float flSize, float flAmplitude, int nRenderamt,
                             float flSpeed) {
  C_LocalTempEntity *pTemp;
  const model_t *pModel;
  int flFrameCount;
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    pTemp->flags |=
        FTENT_COLLIDEWORLD | FTENT_SPRCYCLE | FTENT_FADEOUT | FTENT_SLOWGRAVITY;
...
//			life -
//-----------------------------------------------------------------------------
void CTempEnts::AttachTentToPlayer(int client, int modelIndex, float zoffset,
                                   float life) {
  C_LocalTempEntity *pTemp;
  const model_t *pModel;
  Vector position;
  ...
  FOR_EACH_LL(m_TempEnts, i) {
    ...
    if (pTemp->flags & FTENT_PLYRATTACHMENT) {
      ...
      if (pTemp->clientIndex == client) {
        pTemp->die =
            gpGlobals
                ->curtime;  // good enough, it will die on next tent update.
      }
    }
  }
}
...
//			scale -
//-----------------------------------------------------------------------------
void CTempEnts::RicochetSprite(const Vector &pos, model_t *pmodel,
...
//			size -
//-----------------------------------------------------------------------------
void CTempEnts::BloodSprite(const Vector &org, int r, int g, int b, int a,
                            int modelIndex, int modelIndex2, float size) {
  const model_t *model;
  ...
  if (modelIndex && (model = modelinfo->GetModel(modelIndex)) != NULL) {
    ...
    if ((pTemp = TempEntAllocHigh(org, (model_t *)model)) != NULL) {
      ...
      pTemp->die =
          gpGlobals->curtime +
          (frameCount / pTemp->m_flFrameRate);  // Play the whole thing Once
...
// Output : C_LocalTempEntity
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::DefaultSprite(const Vector &pos, int spriteIndex,
                                            float framerate) {
  C_LocalTempEntity *pTemp;
  int frameCount;
  ...
  if (!spriteIndex || !pSprite ||
      modelinfo->GetModelType(pSprite) != mod_sprite) {
...
void CTempEnts::Sprite_Smoke(C_LocalTempEntity *pTemp, float scale) {
  ...
  pTemp->SetRenderColor(iColor, iColor, iColor, 255);
...
//			type -
//-----------------------------------------------------------------------------
void CTempEnts::EjectBrass(const Vector &pos1, const QAngle &angles,
                           const QAngle &gunAngles, int type) {
  if (cl_ejectbrass.GetBool() == false) return;
  ...
  pTemp->die =
      ...
      random->RandomFloat(0.0f, 1.0f);  // Add an extra 0-1 secs of life
}

//-----------------------------------------------------------------------------
// Purpose: Create some simple physically simulated models
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::SpawnTempModel(model_t *pModel,
                                             const Vector &vecOrigin,
                                             const QAngle &vecAngles,
                                             const Vector &vecVelocity,
                                             float flLifeTime, int iFlags) {
  Assert(pModel);
...
//			firstPerson -
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash(int type, ClientEntityHandle_t hEntity,
                            int attachmentIndex, bool firstPerson) {
  switch (type) {
...
//			type -
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash(const Vector &pos1, const QAngle &angles, int type,
                            ClientEntityHandle_t hEntity, bool firstPerson) {
#ifdef CSTRIKE_DLL
...
//			flags -
//-----------------------------------------------------------------------------
void CTempEnts::Sprite_Explode(C_LocalTempEntity *pTemp, float scale,
                               int flags) {
  if (!pTemp) return;
...
C_LocalTempEntity *CTempEnts::TempEntAlloc(const Vector &org, model_t *model) {
  ...
  if (CommandLine()->CheckParm("-tools") != NULL) {
    ...
    if (first) {
      Msg("Currently not recording tempents, since recording them as entites "
          "causes them to be deleted as entities, even though they were "
          "allocated through the tempent pool. (crash)\n");
      first = false;
    }
#endif
    //		ClientEntityList().AddNonNetworkableEntity(	pTemp );
...
C_LocalTempEntity *CTempEnts::TempEntAlloc() {
  if (m_TempEnts.Count() >= MAX_TEMP_ENTITIES) return NULL;
...
void CTempEnts::TempEntFree(int index) {
  ...
  if (pTemp) {
    ...
    // Remove the tempent from the ClientEntityList before removing it from the
    // pool.
    if ((pTemp->flags & FTENT_CLIENTSIDEPARTICLES)) {
      // Stop the particle emission if this hasn't happened already - collision
      // or system timing out on its own.
...
//-----------------------------------------------------------------------------
// Purpose: Allocate a temp entity, if there are no slots, kick out a low
// priority
//  one if possible
// Input  : *org -
//			*model -
// Output : C_LocalTempEntity
//-----------------------------------------------------------------------------
C_LocalTempEntity *CTempEnts::TempEntAllocHigh(const Vector &org,
                                               model_t *model) {
  C_LocalTempEntity *pTemp;
...
void CTempEnts::PlaySound(C_LocalTempEntity *pTemp, float damp) {
  ...
  if (isshellcasing) {
    ...
    if (zvel < 200 && random->RandomInt(0, 3)) return;
  } else {
    if (random->RandomInt(0, 5)) return;
  }

  CSoundParameters params;
  if (!C_BaseEntity::GetParametersForSound(soundname, params, NULL)) return;
...
void CTempEnts::Update(void) {
  VPROF_("CTempEnts::Update", 1, VPROF_BUDGETGROUP_CLIENT_SIM, false,
         BUDGETFLAG_CLIENT);
  ...
  // in order to have tents collide with players, we have to run the player
  // prediction code so that the client has the player list. We run this code
  // once when we detect any COLLIDEALL tent, then set this BOOL to true so the
  // code doesn't get run again if there's more than one COLLIDEALL ent for this
  // update. (often are).
...
void CTempEnts::LevelInit() {
#ifndef TF_CLIENT_DLL
  m_pSpriteMuzzleFlash[0] =
      (model_t *)engine->LoadModel("sprites/ar2_muzzle1.vmt");
  m_pSpriteMuzzleFlash[1] =
      (model_t *)engine->LoadModel("sprites/muzzleflash4.vmt");
  m_pSpriteMuzzleFlash[2] =
      (model_t *)engine->LoadModel("sprites/muzzleflash4.vmt");

  m_pSpriteAR2Flash[0] =
      (model_t *)engine->LoadModel("sprites/ar2_muzzle1b.vmt");
  m_pSpriteAR2Flash[1] =
      (model_t *)engine->LoadModel("sprites/ar2_muzzle2b.vmt");
  m_pSpriteAR2Flash[2] =
      (model_t *)engine->LoadModel("sprites/ar2_muzzle3b.vmt");
  m_pSpriteAR2Flash[3] =
      (model_t *)engine->LoadModel("sprites/ar2_muzzle4b.vmt");

  m_pSpriteCombineFlash[0] =
      (model_t *)engine->LoadModel("effects/combinemuzzle1.vmt");
  m_pSpriteCombineFlash[1] =
      (model_t *)engine->LoadModel("effects/combinemuzzle2.vmt");
  ...
  m_pShells[2] =
      (model_t *)engine->LoadModel("models/weapons/shotgun_shell.mdl");
#endif
  ...
  m_pCS_12GaugeShell =
      (model_t *)engine->LoadModel("models/Shells/shell_12gauge.mdl");
  m_pCS_556Shell = (model_t *)engine->LoadModel("models/Shells/shell_556.mdl");
  m_pCS_762NATOShell =
      (model_t *)engine->LoadModel("models/Shells/shell_762nato.mdl");
  m_pCS_338MAGShell =
      (model_t *)engine->LoadModel("models/Shells/shell_338mag.mdl");
#endif
}
...
inline void CTempEnts::CacheMuzzleFlashes(void) {
  ...
  for (i = 0; i < 4; i++) {
    if (m_Material_MuzzleFlash_Player[i] == NULL) {
      m_Material_MuzzleFlash_Player[i] = ParticleMgr()->GetPMaterial(
          VarArgs("effects/muzzleflash%d_noz", i + 1));
    }
  }

  for (i = 0; i < 4; i++) {
    if (m_Material_MuzzleFlash_NPC[i] == NULL) {
      m_Material_MuzzleFlash_NPC[i] =
          ParticleMgr()->GetPMaterial(VarArgs("effects/muzzleflash%d", i + 1));
    }
  }

  for (i = 0; i < 2; i++) {
    if (m_Material_Combine_MuzzleFlash_Player[i] == NULL) {
      m_Material_Combine_MuzzleFlash_Player[i] = ParticleMgr()->GetPMaterial(
          VarArgs("effects/combinemuzzle%d_noz", i + 1));
    }
  }

  for (i = 0; i < 2; i++) {
    if (m_Material_Combine_MuzzleFlash_NPC[i] == NULL) {
      m_Material_Combine_MuzzleFlash_NPC[i] = ParticleMgr()->GetPMaterial(
          VarArgs("effects/combinemuzzle%d", i + 1));
    }
  }
}
...
//			attachmentIndex -
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash_Combine_Player(ClientEntityHandle_t hEntity,
                                           int attachmentIndex) {
  VPROF_BUDGET("MuzzleFlash_Combine_Player",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create(
      "MuzzleFlash", hEntity, attachmentIndex, FLE_VIEWMODEL);
  ...
  for (int i = 1; i < 6; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle),
        m_Material_Combine_MuzzleFlash_Player[random->RandomInt(0, 1)], offset);
    ...
    pParticle->m_uchStartSize =
        ((random->RandomFloat(6.0f, 8.0f) * (12 - (i)) / 12) * flScale);
  ...
  pParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle),
      m_Material_Combine_MuzzleFlash_Player[random->RandomInt(0, 1)],
      vec3_origin);
...
//			entityIndex -
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash_Combine_NPC(ClientEntityHandle_t hEntity,
                                        int attachmentIndex) {
...
#define FRONT_LENGTH 6
  ...
  for (int i = 1; i < FRONT_LENGTH; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle),
        g_Mat_Combine_Muzzleflash[random->RandomInt(0, 1)], offset);
    if (pParticle == NULL) return;
    ...
    pParticle->m_uchStartSize =
        ((random->RandomFloat(6.0f, 8.0f) * (FRONT_LENGTH * 1.25f - (i)) /
          (FRONT_LENGTH)) *
         flScale);
    pParticle->m_uchEndSize = pParticle->m_uchStartSize;
    pParticle->m_flRoll = random->RandomInt(0, 360);
...
#define SIDE_LENGTH 6
  ...
  for (int i = 1; i < SIDE_LENGTH; i++) {
    ...
    pParticle->m_uchStartSize = ((random->RandomFloat(2.0f, 4.0f) *
                                  (SIDE_LENGTH - (i)) / (SIDE_LENGTH * 0.5f)) *
                                 flScale);
    pParticle->m_uchEndSize = pParticle->m_uchStartSize;
    pParticle->m_flRoll = random->RandomInt(0, 360);
    pParticle->m_flRollDelta = 0.0f;
  ...
  pParticle = (SimpleParticle *)pSimple->AddParticle(
      sizeof(SimpleParticle), g_Mat_Combine_Muzzleflash[2], vec3_origin);
  if (pParticle == NULL) return;
  ...
  if (FX_GetAttachmentTransform(hEntity, attachmentIndex, matAttachment)) {
    ...
    // NOTENOTE: If you're here, you've specified an entity or an attachment
    // that is invalid
    Assert(0);
    return;
  }
  ...
  if (muzzleflash_light.GetBool()) {
    ...
    if (pEnt) {
      dlight_t *el =
          effects->CL_AllocElight(LIGHT_INDEX_MUZZLEFLASH + pEnt->entindex());
...
void CTempEnts::MuzzleFlash_AR2_NPC(const Vector &origin, const QAngle &angles,
...
// Purpose:
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash_SMG1_NPC(ClientEntityHandle_t hEntity,
                                     int attachmentIndex) {
  // Draw the cloud of fire
...
// Purpose:
//-----------------------------------------------------------------------------
void CTempEnts::MuzzleFlash_SMG1_Player(ClientEntityHandle_t hEntity,
                                        int attachmentIndex) {
  VPROF_BUDGET("MuzzleFlash_SMG1_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  CSmartPtr<CLocalSpaceEmitter> pSimple = CLocalSpaceEmitter::Create(
      "MuzzleFlash_SMG1_Player", hEntity, attachmentIndex, FLE_VIEWMODEL);
  ...
  for (int i = 1; i < 6; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle),
        m_Material_MuzzleFlash_Player[random->RandomInt(0, 3)], offset);
    ...
    pParticle->m_uchStartSize =
        ((random->RandomFloat(6.0f, 8.0f) * (8 - (i)) / 6) * flScale);
    pParticle->m_uchEndSize = pParticle->m_uchStartSize;
    pParticle->m_flRoll = random->RandomInt(0, 360);
    pParticle->m_flRollDelta = 0.0f;
  }
...
void CTempEnts::MuzzleFlash_Shotgun_Player(ClientEntityHandle_t hEntity,
                                           int attachmentIndex) {
  VPROF_BUDGET("MuzzleFlash_Shotgun_Player",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  CSmartPtr<CSimpleEmitter> pSimple =
      CSimpleEmitter::Create("MuzzleFlash_Shotgun_Player");
  ...
  pSimple->GetBinding().SetBBox(origin - Vector(4, 4, 4),
                                origin + Vector(4, 4, 4));
...
void CTempEnts::MuzzleFlash_Shotgun_NPC(ClientEntityHandle_t hEntity,
                                        int attachmentIndex) {
  // Draw the cloud of fire
  ...
  IClientRenderable *pRenderable =
      ClientEntityList().GetClientRenderableFromHandle(hEntity);
  ...
  if (random->RandomInt(0, 2) == 0) {
    ...
    for (int i = 0; i < numEmbers; i++) {
      pParticle = (SimpleParticle *)pEmbers->AddParticle(
          sizeof(SimpleParticle), g_Mat_SMG_Muzzleflash[0], origin);
  ...
  CSmartPtr<CTrailParticles> pTrails =
      CTrailParticles::Create("MuzzleFlash_Shotgun_NPC");
  ...
  for (i = 0; i < numEmbers; i++) {
    pTrailParticle = (TrailParticle *)pTrails->AddParticle(
        sizeof(TrailParticle), g_Mat_SMG_Muzzleflash[0], origin);

    if (pTrailParticle == NULL) return;
    ...
    pTrailParticle->m_vecVelocity *=
        (random->RandomFloat(256.0f, 1024.0f) * (1.0f - dot));
...
// Purpose:
//==================================================
void CTempEnts::MuzzleFlash_357_Player(ClientEntityHandle_t hEntity,
                                       int attachmentIndex) {
  VPROF_BUDGET("MuzzleFlash_357_Player", VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  CSmartPtr<CSimpleEmitter> pSimple =
      CSimpleEmitter::Create("MuzzleFlash_357_Player");
  ...
  pParticle = (SimpleParticle *)pSimple->AddParticle(sizeof(SimpleParticle),
                                                     g_Mat_DustPuff[0], offset);
...
void CTempEnts::MuzzleFlash_Pistol_Player(ClientEntityHandle_t hEntity,
                                          int attachmentIndex) {
  VPROF_BUDGET("MuzzleFlash_Pistol_Player",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  CSmartPtr<CSimpleEmitter> pSimple =
      CSimpleEmitter::Create("MuzzleFlash_Pistol_Player");
  pSimple->SetDrawBeforeViewModel(true);
  ...
  if (random->RandomInt(0, 3) != 0) {
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_DustPuff[0], offset);
...
void CTempEnts::MuzzleFlash_Pistol_NPC(ClientEntityHandle_t hEntity,
                                       int attachmentIndex) {
...
void CTempEnts::MuzzleFlash_RPG_NPC(ClientEntityHandle_t hEntity,
                                    int attachmentIndex) {
  // Draw the cloud of fire
...
void CTempEnts::HL1EjectBrass(const Vector &vecPosition,
                              const QAngle &angAngles,
                              const Vector &vecVelocity, int nType) {
...
void CTempEnts::CSEjectBrass(const Vector &vecPosition,
                             const QAngle &angVelocity, int nVelocity,
                             int shellType, CBasePlayer *pShooter) {
  const model_t *pModel = NULL;
  ...
  pTemp->m_nBody = 0;
  pTemp->flags = FTENT_FADEOUT | FTENT_GRAVITY | FTENT_COLLIDEALL |
                 FTENT_HITSOUND | FTENT_ROTATE | FTENT_CHANGERENDERONCOLLIDE;
