...
static ConVar fx_drawimpactdebris("fx_drawimpactdebris", "1",
...
void FX_CacheMaterialHandles(void) {
  ...
  g_Mat_Fleck_Antlion[0] =
      ParticleMgr()->GetPMaterial("effects/fleck_antlion1");
  g_Mat_Fleck_Antlion[1] =
      ParticleMgr()->GetPMaterial("effects/fleck_antlion2");
  ...
  g_Mat_DustPuff[0] =
      ParticleMgr()->GetPMaterial("particle/particle_smokegrenade");
  g_Mat_DustPuff[1] =
  ...
  g_Mat_SMG_Muzzleflash[0] =
      ParticleMgr()->GetPMaterial("effects/muzzleflash1");
  g_Mat_SMG_Muzzleflash[1] =
      ParticleMgr()->GetPMaterial("effects/muzzleflash2");
  g_Mat_SMG_Muzzleflash[2] =
      ParticleMgr()->GetPMaterial("effects/muzzleflash3");
  g_Mat_SMG_Muzzleflash[3] =
      ParticleMgr()->GetPMaterial("effects/muzzleflash4");

  g_Mat_Combine_Muzzleflash[0] =
      ParticleMgr()->GetPMaterial("effects/combinemuzzle1");
  g_Mat_Combine_Muzzleflash[1] =
      ParticleMgr()->GetPMaterial("effects/combinemuzzle2");
  g_Mat_Combine_Muzzleflash[2] =
      ParticleMgr()->GetPMaterial("effects/strider_muzzle");
#endif
}
...
void GetColorForSurface(trace_t *trace, Vector *color) {
  ...
  Vector end = trace->startpos +
               ((Vector)trace->endpos - (Vector)trace->startpos) * 1.1f;

  if (trace->DidHitWorld()) {
    if (trace->hitbox == 0) {
      // If we hit the world, then ask the world for the fleck color
      engine->TraceLineMaterialAndLighting(trace->startpos, end, diffuseColor,
                                           baseColor);
    } else {
      // In this case we hit a static prop.
      staticpropmgr->GetStaticPropMaterialColorAndLighting(
          trace, trace->hitbox - 1, diffuseColor, baseColor);
    }
  } else {
    ...
    modelinfo->GetModelMaterialColorAndLighting(
        pModel, pCollide->GetCollisionOrigin(), pCollide->GetCollisionAngles(),
        trace, diffuseColor, baseColor);
  }
...
static void CreateFleckParticles(const Vector &origin, const Vector &color,
                                 trace_t *trace, char materialType,
                                 int iScale) {
  Vector spawnOffset = trace->endpos + (trace->plane.normal * 1.0f);

  CSmartPtr<CFleckParticles> fleckEmitter =
      CFleckParticles::Create("FX_DebrisFlecks", spawnOffset, Vector(5, 5, 5));

  if (!fleckEmitter) return;

  // Handle increased scale
  float flMaxSpeed = FLECK_MAX_SPEED * iScale;
  float flAngularSpray =
      std::max(0.2f, FLECK_ANGULAR_SPRAY -
                         ((float)iScale *
                          0.2f));  // More power makes the spray more controlled
  // Setup our collision information
  fleckEmitter->m_ParticleCollision.Setup(
      spawnOffset, &trace->plane.normal, flAngularSpray, FLECK_MIN_SPEED,
      flMaxSpeed, FLECK_GRAVITY, FLECK_DAMPEN);
  ...
  for (i = 0; i < numFlecks; i++) {
    pFleckParticle = (FleckParticle *)fleckEmitter->AddParticle(
        sizeof(FleckParticle), hMaterial[random->RandomInt(0, 1)], spawnOffset);
    ...
    dir[0] = trace->plane.normal[0] +
             random->RandomFloat(-flAngularSpray, flAngularSpray);
    dir[1] = trace->plane.normal[1] +
             random->RandomFloat(-flAngularSpray, flAngularSpray);
    dir[2] = trace->plane.normal[2] +
             random->RandomFloat(-flAngularSpray, flAngularSpray);

    pFleckParticle->m_uchSize = random->RandomInt(1, 2);

    pFleckParticle->m_vecVelocity =
        dir * (random->RandomFloat(FLECK_MIN_SPEED, flMaxSpeed) *
               (3 - pFleckParticle->m_uchSize));
    ...
    pFleckParticle->m_uchColor[0] =
        std::min(1.0f, color[0] * colorRamp) * 255.0f;
    pFleckParticle->m_uchColor[1] =
        std::min(1.0f, color[1] * colorRamp) * 255.0f;
    pFleckParticle->m_uchColor[2] =
...
//-----------------------------------------------------------------------------
void FX_DebrisFlecks(const Vector &origin, trace_t *tr, char materialType,
                     int iScale, bool bNoFlecks) {
  ...
  // Lock the bbox
  pSimple->GetBinding().SetBBox(origin - (Vector(16, 16, 16) * iScale),
                                origin + (Vector(16, 16, 16) * iScale));
  ...
  for (i = 0; i < 4; i++) {
    if (i == 3) {
      pParticle = (SimpleParticle *)pSimple->AddParticle(
          sizeof(SimpleParticle), g_Mat_BloodPuff[0], origin);
    } else {
      pParticle = (SimpleParticle *)pSimple->AddParticle(
          sizeof(SimpleParticle), g_Mat_DustPuff[0], origin);
    }
    ...
    if (pParticle != NULL) {
      ...
      pParticle->m_vecVelocity +=
          (tr->plane.normal * random->RandomFloat(1.0f, 6.0f));
      ...
      pParticle->m_uchStartSize =
          (iScale * 0.5f) * random->RandomInt(3, 4) * (i + 1);
  ...
  for (i = 0; i < 2; i++) {
    ...
    newParticle.m_vecVelocity =
        dir * random->RandomFloat(2.0f, 24.0f) * (i + 1);
    ...
    newParticle.m_uchColor[0] = std::min(1.0f, color[0] * colorRamp) * 255.0f;
    newParticle.m_uchColor[1] = std::min(1.0f, color[1] * colorRamp) * 255.0f;
    newParticle.m_uchColor[2] = std::min(1.0f, color[2] * colorRamp) * 255.0f;
...
void FX_GlassImpact(const Vector &pos, const Vector &normal) {
  ...
  CSmartPtr<CSimple3DEmitter> pGlassEmitter =
      CSimple3DEmitter::Create("FX_GlassImpact");
  ...
  for (int i = 0; i < numShards; i++) {
    ...
    pParticle = (Particle3D *)pGlassEmitter->AddParticle(
        sizeof(Particle3D), g_Mat_Fleck_Glass[random->RandomInt(0, 1)], pos);

    if (pParticle) {
      pParticle->m_flLifeRemaining =
          random->RandomFloat(GLASS_SHARD_MIN_LIFE, GLASS_SHARD_MAX_LIFE);

      pParticle->m_vecVelocity[0] =
          (normal[0] + random->RandomFloat(-0.8f, 0.8f)) *
          random->RandomFloat(GLASS_SHARD_MIN_SPEED, GLASS_SHARD_MAX_SPEED);
      pParticle->m_vecVelocity[1] =
          (normal[1] + random->RandomFloat(-0.8f, 0.8f)) *
          random->RandomFloat(GLASS_SHARD_MIN_SPEED, GLASS_SHARD_MAX_SPEED);
      pParticle->m_vecVelocity[2] =
          (normal[2] + random->RandomFloat(-0.8f, 0.8f)) *
          random->RandomFloat(GLASS_SHARD_MIN_SPEED, GLASS_SHARD_MAX_SPEED);

      pParticle->m_uchSize =
          flShardSize +
          random->RandomFloat(-0.5 * flShardSize, 0.5 * flShardSize);
  ...
  pGlassEmitter->m_ParticleCollision.Setup(
      pos, &normal, GLASS_SHARD_NOISE, GLASS_SHARD_MIN_SPEED,
      GLASS_SHARD_MAX_SPEED, GLASS_SHARD_GRAVITY, GLASS_SHARD_DAMPING);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    newParticle.m_vecVelocity =
        dir * random->RandomFloat(8.0f, 16.0f) * (i + 1);
...
void FX_AntlionImpact(const Vector &pos, trace_t *trace) {
  ...
  CSmartPtr<CSimple3DEmitter> fleckEmitter =
      CSimple3DEmitter::Create("FX_DebrisFlecks");
  if (fleckEmitter == NULL) return;
  ...
  fleckEmitter->GetBinding().SetBBox(spawnOffset - Vector(32, 32, 32),
                                     spawnOffset + Vector(32, 32, 32), true);
  ...
  // Setup our collision information
  fleckEmitter->m_ParticleCollision.Setup(spawnOffset, &shotDir, flAngularSpray,
                                          8.0f, flMaxSpeed, FLECK_GRAVITY,
                                          FLECK_DAMPEN);
  ...
  int i;
  for (i = 0; i < numFlecks; i++) {
    pFleckParticle = (Particle3D *)fleckEmitter->AddParticle(
        sizeof(Particle3D), g_Mat_Fleck_Antlion[random->RandomInt(0, 1)],
        spawnOffset);
    if (pFleckParticle == NULL) break;
  ...
  CSmartPtr<CSimpleEmitter> dustEmitter =
      CSimpleEmitter::Create("FX_DebrisFlecks");
  if (!dustEmitter) return;

  Vector offset = trace->endpos + (shotDir * 4.0f);

  dustEmitter->SetSortOrigin(offset);
  dustEmitter->GetBinding().SetBBox(spawnOffset - Vector(32, 32, 32),
                                    spawnOffset + Vector(32, 32, 32), true);

  for (i = 0; i < 4; i++) {
    pParticle = (SimpleParticle *)dustEmitter->AddParticle(
  ...
  C_BaseEntity::EmitSound(filter, 0, "FX_AntlionImpact.ShellImpact",
                          &trace->endpos);
}
...
#endif
void FX_BugBlood(Vector &pos, Vector &dir, Vector &vWorldMins,
                 Vector &vWorldMaxs) {
  ...
  pSimple->GetBinding().SetBBox(pos - Vector(32, 32, 32),
                                pos + Vector(32, 32, 32), true);
  ...
  for (i = 0; i < NUM_BUG_BLOOD; i++) {
    SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_BloodPuff[0], pos);
  ...
  for (i = 0; i < NUM_BUG_BLOOD2; i++) {
    SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_BloodPuff[1], pos);
  ...
  for (i = 0; i < NUM_BUG_SPLATS; i++) {
    ...
    SimpleParticle *sParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), g_Mat_BloodPuff[1], offset);
...
void FX_DustImpact(const Vector &origin, trace_t *tr, int iScale) {
  if (!fx_drawimpactdust.GetBool()) return;
  ...
  CSmartPtr<CDustParticle> pSimple = CDustParticle::Create("dust");
  pSimple->SetSortOrigin(origin);
  pSimple->GetBinding().SetBBox(origin - (Vector(32, 32, 32) * iScale),
                                origin + (Vector(32, 32, 32) * iScale));
...
void FX_GaussExplosion(const Vector &pos, const Vector &dir, int type) {
  ...
  CSmartPtr<CTrailParticles> pSparkEmitter =
      CTrailParticles::Create("FX_GaussExplosion");
  ...
  pSparkEmitter->GetBinding().SetBBox(pos - Vector(32, 32, 32),
                                      pos + Vector(32, 32, 32));
  ...
  AddSimpleParticle(&particle,
                    ParticleMgr()->GetPMaterial("effects/yellowflare"));
  ...
  //
  CSmartPtr<CTrailParticles> pSparkEmitter =
      CTrailParticles::Create("FX_ElectricSpark");
  ...
  // Setup our collision information
  pSparkEmitter->m_ParticleCollision.Setup(pos, &vDir, 0.8f, 128, 512, 800,
                                           0.3f);
  ...
  for (i = 0; i < numSparks; i++) {
    pParticle = (TrailParticle *)pSparkEmitter->AddParticle(
        sizeof(TrailParticle), hMaterial, pos);
...
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEGaussExplosion, DT_TEGaussExplosion,
                               CTEGaussExplosion)
RecvPropInt(RECVINFO(m_nType)), RecvPropVector(RECVINFO(m_vecDirection)),
END_RECV_TABLE
()
...
//			type -
//-----------------------------------------------------------------------------
void TE_GaussExplosion(IRecipientFilter &filter, float delay, const Vector &pos,
                       const Vector &dir, int type) {
