...
CON_COMMAND(showmapinfo, "Show map info panel") {
  ...
  // don't let the player open the team menu themselves until they're a
  // spectator or they're on a regular team and have picked a class
  if (pPlayer) {
...
//-----------------------------------------------------------------------------
void TFViewport::Start(IGameUIFuncs *pGameUIFuncs,
...
void TFViewport::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  if (IsX360()) {
    ...
    vgui::surface()->PrecacheFontCharacters(
        pScheme->GetFont("ScoreboardTeamName"), pAllChars);
    vgui::surface()->PrecacheFontCharacters(
        pScheme->GetFont("ScoreboardMedium"), pAllChars);
    vgui::surface()->PrecacheFontCharacters(pScheme->GetFont("ScoreboardSmall"),
                                            pAllChars);
    vgui::surface()->PrecacheFontCharacters(
        pScheme->GetFont("ScoreboardVerySmall"), pAllChars);
    vgui::surface()->PrecacheFontCharacters(pScheme->GetFont("TFFontMedium"),
                                            pAllChars);
    vgui::surface()->PrecacheFontCharacters(pScheme->GetFont("TFFontSmall"),
                                            pAllChars);
    vgui::surface()->PrecacheFontCharacters(pScheme->GetFont("HudFontMedium"),
                                            pAllChars);
    vgui::surface()->PrecacheFontCharacters(
        pScheme->GetFont("HudFontMediumSmallSecondary"), pAllChars);
    vgui::surface()->PrecacheFontCharacters(pScheme->GetFont("HudFontSmall"),
                                            pAllChars);
    vgui::surface()->PrecacheFontCharacters(pScheme->GetFont("DefaultSmall"),
                                            pAllChars);

    vgui::surface()->PrecacheFontCharacters(
        pScheme->GetFont("ScoreboardTeamScore"), pNumbers);
  }
}

//
// This is the main function of the viewport. Right here is where we create our
// class menu, team menu, and anything else that we want to turn on and off in
// the UI.
//
...
void TFViewport::OnScreenSizeChanged(int iOldWide, int iOldTall) {
  ...
  // we've changed resolution, let's try to figure out if we need to show any of
  // our menus
  if (!gViewPortInterface) return;
  ...
  if (pPlayer) {
    ...
    if (pPlayer->GetTeamNumber() == TEAM_UNASSIGNED) {
      engine->ClientCmd("show_motd");
               ...
               (pPlayer->m_Shared.GetDesiredPlayerClassIndex() ==
                TF_CLASS_UNDEFINED)) {
      switch (pPlayer->GetTeamNumber()) {
        case TF_TEAM_RED:
          gViewPortInterface->ShowPanel(PANEL_CLASS_RED, true);
