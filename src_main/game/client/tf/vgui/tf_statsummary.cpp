...
void CTFStatsSummaryPanel::UpdateClassDetails()
{
	...
	int iRow = 0;
	for ( int i = 0; i < std::size( classDetails ); i++ )
	{
		...
		else
		{
			...
			swprintf_s( wzStatNum, std::size( wzStatNum ), L"%d", iMaxVal );
		}
...
void CTFStatsSummaryPanel::InitBarChartComboBox( ComboBox *pComboBox )
{
	...
	for ( int i=0; i < std::size( initData ); i++ )
	{
		KeyValues *pKeyValues = new KeyValues( "data" );
	...
	pComboBox->SetNumberOfEditLines( std::size( initData ) );
}
...
void CTFStatsSummaryPanel::SetValueAsClass( const char *pDialogVariable, int iValue, int iPlayerClass )
{
	if ( iValue > 0 )
	{
		...
		wchar_t wzVal[16];
		wchar_t wzMsg[128];
		_itow_s( iValue, wzVal, std::size( wzVal ), 10 );
...
const char *CTFStatsSummaryPanel::RenderValue( float flValue, TFStatType_t statType, StatDisplay_t statDisplay )
{
	...
	{	
		...
		Q_snprintf( szValue, std::size( szValue ), "%.2f", flValue );
	}
	else
	{
		// otherwise, render as an integer
		Q_snprintf( szValue, std::size( szValue ), "%d", (int) flValue );
	}
