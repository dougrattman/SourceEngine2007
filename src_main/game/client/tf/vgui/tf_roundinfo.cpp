...
class RoundInfoOverlay : public vgui::EditablePanel {
 public:
  ...
  RoundInfoOverlay(Panel *parent, const char *panelName)
    ...
    vgui::surface()->DrawSetTextureFile(m_iBlueTeamTexture,
                                        "overviews/blueteam", true, false);

    m_iRedTeamTexture = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(m_iRedTeamTexture, "overviews/redteam",
                                        true, false);

    m_iCapArrowTexture = vgui::surface()->CreateNewTextureID();
    vgui::surface()->DrawSetTextureFile(m_iCapArrowTexture,
                                        "overviews/caparrows", true, false);
 ...
 private:
  ...
  struct roundinfo_control_point_t {
    char m_szName[64];
    int m_iXPos;
    int m_iYPos;
    bool m_bHideIcon;
  ...
  int m_iMode;  // 0 - start, 1 - previous round victory anim, 2 - attack
                // directive, new round
...
void RoundInfoOverlay::Paint(void) {
  ...
  for (int i = 0; i < m_ControlPoints.Count(); i++) {
    if (m_ControlPoints[i].m_bHideIcon) continue;
    ...
    switch (m_iMode) {
      ...
      {
        bool bPointInContention =
            (m_iNextRoundPoints[0] == i || m_iNextRoundPoints[1] == i);

        bool bBlueTeam = (m_iCurrentState & (1 << i));
        DrawTeamIcon(
            x, y, bBlueTeam,
            bPointInContention ? 1.4 : 1.0);  // rescale? pop looks weird
      } break;
    }
  }
...
void RoundInfoOverlay::DrawTeamIcon(int x, int y, bool bBlueTeam,
                                    float flBloat /* = 1.0f */) {
  ...
  vgui::surface()->DrawSetColor(Color(255, 255, 255, 255));
  vgui::surface()->DrawSetTexture(bBlueTeam ? m_iBlueTeamTexture
                                            : m_iRedTeamTexture);
...
void RoundInfoOverlay::Update(const char *szMapName) {
  ...
  Q_strncpy(strFullpath, "resource/roundinfo/",
  ...
  Q_strncat(strFullpath, ".res",
            SOURCE_MAX_PATH);  // Assume we're a .res extension type
  ...
  if (g_pFullFileSystem->FileExists(strFullpath), "MOD") {
    ...
    if (kvCapPoints) {
      if (kvCapPoints->LoadFromFile(g_pFullFileSystem, strFullpath)) {
        ...
        for (KeyValues *pData = kvCapPoints->GetFirstSubKey(); pData != NULL;
          ...
          point.m_iXPos =
              (int)((float)pData->GetInt("x", 0) * ((float)wide / 560.0f));
          point.m_iYPos =
              (int)((float)pData->GetInt("y", 0) * ((float)tall / 280.0f));
...
void RoundInfoOverlay::SetState(int iPrevState, int iCurrentState,
                                int iNextBattles) {
...
void RoundInfoOverlay::OnTick(void) {
  ...
  if (tf_roundinfo_pause.GetBool() == false &&
      m_flModeChangeTime <= gpGlobals->curtime) {
    switch (m_iMode) {
      case 0: {
      ...
      case 1: {
        ...
        C_BaseEntity::EmitSound(filter, SOUND_FROM_LOCAL_PLAYER,
                                "Hud.EndRoundScored");
      } break;
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFRoundInfo::PerformLayout() { BaseClass::PerformLayout(); }
...
void CTFRoundInfo::ShowPanel(bool bShow) {
  ...
  if (bShow) {
    // look for the textures we want to use and don't show the roundinfo panel
    // if any are missing
    char temp[255];
    Q_snprintf(temp, sizeof(temp), "VGUI/%s", m_szMapImage);
    IMaterial *pMapMaterial =
        materials->FindMaterial(temp, TEXTURE_GROUP_VGUI, false);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFRoundInfo::UpdateImage(ImagePanel *pImagePanel,
  ...
  if (pImagePanel && (Q_strlen(pszImageName) > 0)) {
    ...
    IMaterial *pTemp =
        materials->FindMaterial(szTemp, TEXTURE_GROUP_VGUI, false);
...
void CTFRoundInfo::OnKeyCodePressed(KeyCode code) {
  if (code == KEY_SPACE || code == KEY_ENTER || code == KEY_XBUTTON_A ||
      code == KEY_XBUTTON_B) {
    OnCommand("continue");
  } else {
...
void CTFRoundInfo::SetData(KeyValues *data) {
  if (m_pOverlay) {
    m_pOverlay->SetState(data->GetInt("prev"), data->GetInt("cur"),
                         data->GetInt("round"));
  }
}
