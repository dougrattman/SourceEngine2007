...
void CTFHudDeathNotice::OnGameEvent( IGameEvent *event, DeathNoticeItem &msg )
{
	...
	if ( FStrEq( pszEventName, "player_death" ) || FStrEq( pszEventName, "object_destroyed" ) )
	{
		...
		if ( assister_name )
		{
			...
			Q_snprintf( szKillerBuf, std::size(szKillerBuf), "%s + %s", msg.Killer.szName, assister_name );
			Q_strncpy( msg.Killer.szName, szKillerBuf, std::size( msg.Killer.szName ) );
		...
		else
		{
			...
			if ( iObjectType >= 0 && iObjectType < OBJ_LAST )
			{
				...
				{
					g_pVGuiLocalize->ConvertUnicodeToANSI( wszLocalizedObjectName, szLocalizedObjectName, std::size( szLocalizedObjectName ) );
				}
				else
				{
				...
				if ( msg.Victim.szName[0] )
				{
					...
					Q_snprintf( szVictimBuf, std::size(szVictimBuf), "%s (%s)", szLocalizedObjectName, msg.Victim.szName );
					Q_strncpy( msg.Victim.szName, szVictimBuf, std::size( msg.Victim.szName ) );
				}
				else
				{
					Q_strncpy( msg.Victim.szName, szLocalizedObjectName, std::size( msg.Victim.szName ) );
				}
		...
		case TF_DMG_CUSTOM_BACKSTAB:
			Q_strncpy( msg.szIcon, "d_backstab", std::size( msg.szIcon ) );
			break;
		case TF_DMG_CUSTOM_HEADSHOT:
			Q_strncpy( msg.szIcon, "d_headshot", std::size( msg.szIcon ) );
			break;
		case TF_DMG_CUSTOM_BURNING:
			...
			Q_strncpy( msg.szIcon, "d_flamethrower", std::size( msg.szIcon ) );
			msg.wzInfoText[0] = 0;
			break;
		...
		FStrEq( "teamplay_flag_event", pszEventName ) )
	{
		...
		Assert( iIndex < std::size( szCaptureIcons ) );

		Q_strncpy( msg.szIcon, bDefense ? szDefenseIcons[iIndex] : szCaptureIcons[iIndex], std::size( msg.szIcon ) );
	}
}
...
void CTFHudDeathNotice::AddAdditionalMsg( int iKillerID, int iVictimID, const char *pMsgKey )
{
	...
	Q_strncpy( msg2.Killer.szName, g_PR->GetPlayerName( iKillerID ), std::size( msg2.Killer.szName ) );
	Q_strncpy( msg2.Victim.szName, g_PR->GetPlayerName( iVictimID ), std::size( msg2.Victim.szName ) );
