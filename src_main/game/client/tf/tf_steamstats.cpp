...
void CTFSteamStats::FireGameEvent( IGameEvent *event )
{
	...
	else if ( 0 == Q_strcmp( pEventName, "user_data_downloaded" ) )
	{
		...
		for ( int iClass = TF_FIRST_NORMAL_CLASS; iClass <= TF_LAST_NORMAL_CLASS; iClass++ )
		{
			...
			for ( int iStat = 0; iStat < std::size( g_SteamStats ); iStat++ )
			{
				char szStatName[256];
				int iData;

				Q_snprintf( szStatName, std::size( szStatName ), "%s.accum.%s", g_aPlayerClassNames_NonLocalized[iClass], g_SteamStats[iStat].pszName );
				...
				Q_snprintf( szStatName, std::size( szStatName ), "%s.max.%s", g_aPlayerClassNames_NonLocalized[iClass], g_SteamStats[iStat].pszName );
				if ( SteamUserStats()->GetStat( gameID, szStatName, &iData ) )
				{
					if ( pStatPanel->IsLocalFileTrusted() )
					{
...
{
	int statsTotals[std::size( g_SteamStats )];
	...
	for ( int i = 0; i < std::size( g_SteamStats ); ++i )
	{
