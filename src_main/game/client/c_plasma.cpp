...
//			objectID -
//-----------------------------------------------------------------------------
void RecvProxy_PlasmaScale(const CRecvProxyData *pData, void *pStruct,
                           void *pOut) {
...
//			objectID -
//-----------------------------------------------------------------------------
void RecvProxy_PlasmaScaleTime(const CRecvProxyData *pData, void *pStruct,
                               void *pOut) {
  C_Plasma *pPlasmaSmoke = (C_Plasma *)pStruct;
...
void C_Plasma::AddEntity(void) {
  ...
  if ((m_nFlags & bitsFIRESMOKE_ACTIVE) == false) return;
  ...
  m_entGlow.SetScale(
      std::max(0.0000001f, (m_flScaleRegister * 1.5f) + GetFlickerScale()));
  m_entGlow.SetLocalOriginDim(
      Z_INDEX, m_entGlow.GetLocalOriginDim(Z_INDEX) + (dScale * 32.0f));
}
...
void C_Plasma::AddFlames(void) {
  ...
  for (int i = 0; i < NUM_CHILD_FLAMES; i++) {
    if (m_entFlames[i].GetScale() > 0.0f) {
      m_entFlames[i].SetRenderColor((255.0f * alpha), (255.0f * alpha),
                                    (255.0f * alpha));
...
//-----------------------------------------------------------------------------
bool C_Plasma::ShouldDraw() { return true; }
...
void C_Plasma::UpdateAnimation(void) {
  ...
  for (int i = 0; i < NUM_CHILD_FLAMES; i++) {
    ...
    if (m_entFlames[i].m_flFrame >= numFrames) {
      m_entFlames[i].m_flFrame =
          m_entFlames[i].m_flFrame - (int)(m_entFlames[i].m_flFrame);
    }
  }
}
...
void C_Plasma::UpdateFlames(void) {
  for (int i = 0; i < NUM_CHILD_FLAMES; i++) {
    ...
    m_entFlames[i].SetScale(std::max(0.000001f, newScale));

    if (i != 0) {
      m_entFlames[i].SetLocalOrigin(
          offset + (m_entFlames[i].m_vecMoveDir *
                    ((m_entFlames[i].GetScale()) * CHILD_SPREAD)));
    }

    Assert(!m_entFlames[i].GetMoveParent());
    m_entFlames[i].SetLocalOriginDim(
        Z_INDEX, m_entFlames[i].GetLocalOriginDim(Z_INDEX) + (dScale * 64.0f));
  }
}
...
void C_Plasma::UpdateScale(void) {
  ...
  if (m_flScaleRegister != m_flScaleEnd) {
    ...
    if (time > m_flScaleTimeEnd) {
      ...
      float timeFraction = 1.0f - (m_flScaleTimeEnd - time) /
                                      (m_flScaleTimeEnd - m_flScaleTimeStart);
      float newScale =
          m_flScaleStart + ((m_flScaleEnd - m_flScaleStart) * timeFraction);
  ...
  if (m_flScaleRegister > 0.1) {
    ...
    while (m_tDecalSpawn.NextEvent(tempDelta)) {
      ...
      if (ent) {
        ...
        if (index >= 0) {
          effects->DecalShoot(index, 0, ent->GetModel(), ent->GetAbsOrigin(),
                              ent->GetAbsAngles(), GetAbsOrigin(), 0, 0);
        }
      }
    }
  }
