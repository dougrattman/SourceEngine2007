...
IMPLEMENT_CLIENTCLASS_DT(C_SlideshowDisplay, DT_SlideshowDisplay,
                         CSlideshowDisplay)
  RecvPropBool(RECVINFO(m_bEnabled)), RecvPropString(RECVINFO(m_szDisplayText)),
      RecvPropString(RECVINFO(m_szSlideshowDirectory)),
      RecvPropArray3(RECVINFO_ARRAY(m_chCurrentSlideLists),
                     RecvPropInt(RECVINFO(m_chCurrentSlideLists[0]))),
...
int C_SlideshowDisplay::GetMaterialIndex(int iSlideIndex) {
  if (!m_SlideMaterialLists[0]) return 0;
...
void C_SlideshowDisplay::ClientThink(void) {
  ...
  m_NextSlideTime =
      gpGlobals->curtime + RandomFloat(m_fMinSlideTime, m_fMaxSlideTime);
  ...
  switch (m_iCycleType) {
    case SLIDESHOW_CYCLE_RANDOM: {
      ...
      if (m_bNoListRepeats && iNumCurrentSlideLists > 1 &&
          m_iCurrentSlideList == iOldSlideList) {
        ++m_iCurrentSlideList;
  ...
  SlideMaterialList_t *pSlideMaterialList =
      m_SlideMaterialLists[m_chCurrentSlideLists[m_iCurrentSlideList]];

  // Cycle in the list
  switch (m_iCycleType) {
    case SLIDESHOW_CYCLE_RANDOM:
      m_iCurrentSlide =
          RandomInt(0, pSlideMaterialList->iSlideMaterials.Count() - 1);
      break;
  ...
  m_iCurrentMaterialIndex =
      pSlideMaterialList->iSlideMaterials[m_iCurrentSlide];
...
void C_SlideshowDisplay::BuildSlideShowImagesList(void) {
  ...
  while (szMatFileName[0]) {
    ...
    Q_snprintf(szFileName, sizeof(szFileName), "vgui/%s/%s",
               m_szSlideshowDirectory, szMatFileName);
    ...
    if (bLoaded) {
      ...
      while (pchKeyword[0] != '\0') {
        ...
        for (iList = 0; iList < m_SlideMaterialLists.Count(); ++iList) {
          if (Q_strcmp(m_SlideMaterialLists[iList]->szSlideKeyword,
                       pchKeyword) == 0)
            break;
        }
    ...
    int iList;
    for (iList = 0; iList < m_SlideMaterialLists.Count(); ++iList) {
      if (Q_strcmp(m_SlideMaterialLists[iList]->szSlideKeyword, "") == 0) break;
    }
