...
static ConVar cl_interp_threadmodeticks("cl_interp_threadmodeticks", "0", 0,
                                        "Additional interpolation ticks to use "
                                        "when interpolating with threaded "
                                        "engine mode set.");
  ...
  for (i = 0; i < count; i++) {
    for (j = i + 1; j < count; j++) {
      ...
      if (p1->entindex() != -1 && p2->entindex() != -1) {
        if (p1->entindex() < p2->entindex()) continue;
      }

      if (p2->entindex() == -1) continue;
...
// Output : int
//-----------------------------------------------------------------------------
int CPredictableList::GetPredictableCount() { return m_Predictables.Count(); }
...
static C_BaseEntity *FindPreviouslyCreatedEntity(CPredictableId &testId) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (!e || !e->IsClientCreated()) continue;
...
// Output : int
//-----------------------------------------------------------------------------
int CRecordingList::Count() { return m_Recording.Count(); }

// Should these be somewhere else?
#define PITCH 0

// HACK HACK:  3/28/02 ywb Had to proxy around this or interpolation is borked
// in multiplayer, not sure what
...
void RecvProxy_AnimTime(const CRecvProxyData *pData, void *pStruct,
  ...
  // Note, this needs to be encoded relative to packet timestamp, not raw client
  // clock
  tickbase =
...
bool C_BaseEntity::IsAbsQueriesValid() {
  if (!ThreadInMainThread()) return true;
...
int C_BaseEntity::GetTextureFrameIndex() { return m_iTextureFrameIndex; }
...
inline int C_BaseEntity::Interp_Interpolate(VarMapping_t *map,
  ...
  for (int i = 0; i < map->m_nInterpolatedEntries; i++) {
    ...
    if (!e->m_bNeedsToInterpolate) continue;
...
const CBaseHandle &C_BaseEntity::GetRefEHandle() const { return m_RefEHandle; }
...
// Only test this in tf2
  ...
  if (g_pClientMode && !g_pClientMode->ShouldDrawEntity(this)) return false;
#endif

  // Some rendermodes prevent rendering
  if (m_nRenderMode == kRenderNone) return false;
...
void *C_BaseEntity::GetDataTableBasePtr() { return this; }
...
bool C_BaseEntity::ShouldReceiveProjectedTextures(int flags) {
  ...
  if (IsEffectActive(EF_NORECEIVESHADOW)) return false;

  if (modelinfo->GetModelType(model) == mod_studio) return false;
...
// Output : Index
//-----------------------------------------------------------------------------
int C_BaseEntity::entindex(void) const { return index; }
...
//-----------------------------------------------------------------------------
const Vector &C_BaseEntity::GetRenderOrigin() { return GetAbsOrigin(); }

const QAngle &C_BaseEntity::GetRenderAngles() { return GetAbsAngles(); }

const matrix3x4_t &C_BaseEntity::RenderableToWorldTransform() {
  return EntityToWorldTransform();
}

IPVSNotify *C_BaseEntity::GetPVSNotifyInterface() { return NULL; }
  ...
  if (nModelType == mod_studio || nModelType == mod_brush) {
    ...
    if (GetRenderAngles() == CollisionProp()->GetCollisionAngles()) {
      ...
      if (IsPointSized()) {
        ...
        //				Assert(0);
        IRotateAABB(EntityToWorldTransform(), CollisionProp()->OBBMins(),
...
//-----------------------------------------------------------------------------
const model_t *C_BaseEntity::GetModel(void) const { return model; }

//-----------------------------------------------------------------------------
// Purpose: Get model index for this entity
// Output : int - model index
//-----------------------------------------------------------------------------
int C_BaseEntity::GetModelIndex(void) const { return m_nModelIndex; }
...
void C_BaseEntity::SetMoveCollide(MoveCollide_t val) { m_MoveCollide = val; }
...
bool C_BaseEntity::UsesPowerOfTwoFrameBufferTexture() { return false; }

bool C_BaseEntity::UsesFullFrameBufferTexture() { return false; }

//-----------------------------------------------------------------------------
// Purpose: Get pointer to CMouthInfo data
// Output : CMouthInfo
//-----------------------------------------------------------------------------
CMouthInfo *C_BaseEntity::GetMouth() { return NULL; }
...
int C_BaseEntity::DrawBrushModel(bool bSort, bool bShadowDepth) {
  ...
  if (bShadowDepth) {
    render->DrawBrushModelShadowDepth(this, (model_t *)model, GetAbsOrigin(),
                                      GetAbsAngles(), bSort);
  } else {
    render->DrawBrushModel(this, (model_t *)model, GetAbsOrigin(),
                           GetAbsAngles(), bSort);
  }
  ...
  switch (modelType) {
    case mod_brush:
      drawn = DrawBrushModel(flags & STUDIO_TRANSPARENCY ? true : false,
                             flags & STUDIO_SHADOWDEPTHTEXTURE ? true : false);
      break;
    case mod_studio:
      // All studio models must be derived from C_BaseAnimating.  Issue warning.
      Warning(
          "ERROR:  Can't draw studio model %s because %s is not derived from "
          "C_BaseAnimating\n",
          modelinfo->GetModelName(model),
          GetClientClass()->m_pNetworkName ? GetClientClass()->m_pNetworkName
                                           : "unknown");
      break;
    case mod_sprite:
      // drawn = DrawSprite();
      Warning(
          "ERROR:  Sprite model's not supported any more except in legacy temp "
          "ents\n");
      break;
    default:
      break;
  ...
  switch (state) {
    case SHOULDTRANSMIT_START: {
      ...
      if (m_PredictableID.IsActive()) {
        // Find corresponding client side predicted entity and remove it from
        // predictables
        ...
        if (otherEntity) {
          ...
          Assert(
              ClientEntityList().IsHandleValid(otherEntity->GetClientHandle()));
...
const Vector &C_BaseEntity::GetOldOrigin() { return m_vecOldOrigin; }
...
void C_BaseEntity::MarkAimEntsDirty() {
  ...
  // MoveToAimEnt and MoveToAimEnt calls SetAbsOrigin/SetAbsAngles, that is how
  // CalcAbsPosition will cause the aim-ent's (and all its children's) dirty
  // state to be correctly updated.
  //
...
void C_BaseEntity::AddToAimEntsList() {
  // Already in list
  if (m_AimEntsListHandle != INVALID_AIMENTS_LIST_HANDLE) return;
...
void C_BaseEntity::HierarchyUpdateMoveParent() {
  if (m_hNetworkMoveParent.ToInt() == m_pMoveParent.ToInt()) return;
...
void C_BaseEntity::HierarchySetParent(C_BaseEntity *pNewParent) {
  ...
  if (newParentHandle.ToInt() == m_pMoveParent.ToInt()) return;
...
void C_BaseEntity::UnlinkFromHierarchy() {
  ...
  // fine if a full entity update was coming (caused by certain factors like too
  // many entities entering the pvs at once).  but otherwise it would not detect
  // the change on the client (since the server and client shouldn't be out of
  // sync) and the var would not be updated like it should.  m_iParentAttachment
  // = 0;
  ...
  while (pChild) {
    if (pChild->m_pMoveParent != this) {
      Warning(
          "C_BaseEntity::UnlinkFromHierarchy(): Entity has a child with the "
          "wrong parent!\n");
      Assert(0);
...
//-----------------------------------------------------------------------------
void C_BaseEntity::ValidateModelIndex() { SetModelByIndex(m_nModelIndex); }
...
void C_BaseEntity::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  // For non-predicted and non-client only ents, we need to latch network values
  // into the interpolation histories
  ...
  if (IsPlayerSimulated() && (NULL != local) && (local == m_hOwnerEntity)) {
    // Make sure player is driving simulation (field is only ever sent to local
    // player)
  ...
  if (!IsClientCreated()) {
    if (Teleported() || IsEffectActive(EF_NOINTERP)) AddToTeleportList();
  }
...
void C_BaseEntity::CheckInitPredictable(const char *context) {
  ...
  if (IsIntermediateDataAllocated()) return;
...
bool C_BaseEntity::IsSelfAnimating() { return true; }

//-----------------------------------------------------------------------------
// EFlags..
//-----------------------------------------------------------------------------
int C_BaseEntity::GetEFlags() const { return m_iEFlags; }

void C_BaseEntity::SetEFlags(int iEFlags) { m_iEFlags = iEFlags; }
...
int CBaseEntity::BaseInterpolatePart1(float &currentTime, Vector &oldOrigin,
                                      QAngle &oldAngles, int &bNoMoreChanges) {
...
bool C_BaseEntity::Interpolate(float currentTime) {
  ...
  int bNoMoreChanges;
  int retVal =
      BaseInterpolatePart1(currentTime, oldOrigin, oldAngles, bNoMoreChanges);
  ...
  if (retVal == INTERPOLATE_STOP) return true;
...
CStudioHdr *C_BaseEntity::OnNewModel() { return NULL; }
...
void C_BaseEntity::CreateLightEffects() {
  ...
  if (index == render->GetViewEntity()) return;
...
void C_BaseEntity::ProcessTeleportList() {
  int iNext;
  for (int iCur = g_TeleportList.Head(); iCur != g_TeleportList.InvalidIndex();
       iCur = iNext) {
...
void C_BaseEntity::CheckInterpolatedVarParanoidMeasurement() {
  ...
  for (int i = 0; i <= iHighest; i++) {
    ...
    if (pEnt->entindex() == 1 && engine->Con_IsVisible()) continue;

    // View models tend to screw up this test unnecesarily because they modify
    // origin, angles, and
    if (dynamic_cast<C_BaseViewModel *>(pEnt)) continue;
...
//-----------------------------------------------------------------------------
float C_BaseEntity::GetTextureAnimationStartTime() { return m_flSpawnTime; }
...
void C_BaseEntity::AddVisibleEntities() {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (!pEnt->IsClientCreated()) continue;

    // Only draw until it's ack'd since that means a real entity has arrived
    if (pEnt->m_PredictableID.GetAcknowledged()) continue;

    // Don't draw if dormant
    if (pEnt->IsDormantPredictable()) continue;
...
ClientThinkHandle_t C_BaseEntity::GetThinkHandle() { return m_hThink; }
...
void C_BaseEntity::ComputeFxBlend() {
  ...
  if (m_nFXComputeFrame == gpGlobals->framecount) return;
...
CollideType_t C_BaseEntity::GetCollideType() {
  if (!m_nModelIndex || !model) return ENTITY_SHOULD_NOT_COLLIDE;

  if (!IsSolid()) return ENTITY_SHOULD_NOT_COLLIDE;
  ...
  if (m_nModelIndex != 1) {
    if (IsPointSized()) return ENTITY_SHOULD_NOT_COLLIDE;
  }
...
void C_BaseEntity::AddStudioDecal(const Ray_t &ray, int hitbox, int decalIndex,
  ...
  // Exit out after doing the trace so any other effects that want to happen can
  // happen.
  if (!r_drawmodeldecals.GetBool()) return;
  ...
  if (doTrace && (GetSolid() == SOLID_VPHYSICS) && !tr.startsolid &&
    ...
    modelrender->AddDecal(m_ModelInstance, ray, up, decalIndex, GetStudioBody(),
                          false, maxLODToDecal);
  }
}
...
void C_BaseEntity::AddBrushModelDecal(const Ray_t &ray,
  ...
  effects->DecalShoot(decalIndex, index, model, GetAbsOrigin(), GetAbsAngles(),
                      decalCenter, 0, 0);
}
...
// TEAM HANDLING
//========================================================================================
C_Team *C_BaseEntity::GetTeam() { return GetGlobalTeam(m_iTeamNum); }

//-----------------------------------------------------------------------------
// Purpose:
// Output : int
//-----------------------------------------------------------------------------
int C_BaseEntity::GetTeamNumber(void) const { return m_iTeamNum; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
int C_BaseEntity::GetRenderTeamNumber() { return GetTeamNumber(); }
...
//-----------------------------------------------------------------------------
bool C_BaseEntity::InLocalTeam() { return (GetTeam() == GetLocalTeam()); }
...
//side  entities 			can decide for themselves whether to
// become  dormant.
//-----------------------------------------------------------------------------
...
void C_BaseEntity::BoneMergeFastCullBloat(Vector &localMins, Vector &localMaxs,
  ...
  // could hang out of the parent bbox, it one corner were touching the edge of
  // the parent's box, and the whole diagonal stretched out.
...
void C_BaseEntity::CalcAbsolutePosition() {
  ...
  if ((m_iEFlags & EFL_DIRTY_ABSTRANSFORM) == 0)  // need second check in event
                                                  // another thread grabbed
                                                  // mutex and did the
                                                  // calculation
  {
    return;
  ...
  ConcatTransforms(GetParentToWorldTransform(scratchMatrix), matEntityToParent,
  ...
  // GetAbsOrigin(), and they'll use the moveparent's OLD bone transforms to get
  // their attachments since the moveparent is right in the middle of setting up
  // new transforms.
  //
  // So here, we keep our absorigin invalidated. It means we're returning an
  // origin that is a frame old to CalculateIKLocks, but we'll still render with
  // the right origin.
  if (m_iParentAttachment != 0 &&
      (m_pMoveParent->GetFlags() & EFL_SETTING_UP_BONES)) {
    m_iEFlags |= EFL_DIRTY_ABSTRANSFORM;
  }
}

void C_BaseEntity::CalcAbsoluteVelocity() {
  if ((m_iEFlags & EFL_DIRTY_ABSVELOCITY) == 0) return;

  AUTO_LOCK(m_CalcAbsoluteVelocityMutex);

  if ((m_iEFlags & EFL_DIRTY_ABSVELOCITY) == 0)  // need second check in event
                                                 // another thread grabbed mutex
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool C_BaseEntity::GetPredictable(void) const { return m_bPredictable; }
...
void C_BaseEntity::PreEntityPacketReceived(int commands_acknowledged) {
  ...
  if (copyintermediate) {
    ...
    RestoreData("PreEntityPacketReceived(no commands ack)", SLOT_ORIGINALDATA,
                PC_EVERYTHING);
  }

  // At this point the entity has original network data restored as of the last
  // time the networking was updated, and it has any intermediate predicted
  // values properly copied over Unpacked and OnDataChanged will fill in any
  // changed, networked fields.
...
bool C_BaseEntity::PostNetworkDataReceived(int commands_acknowledged) {
  ...
  if (errorcheck) {
    ...
    CPredictionCopy errorCheckHelper(PC_NETWORKED_ONLY, predicted_state_data,
                                     PC_DATA_PACKED, original_state_data,
                                     PC_DATA_PACKED, counterrors, reporterrors,
                                     copydata);
...
void C_BaseEntity::Remove() {
  ...
  if (GetPredictable() || IsClientCreated()) {
    ...
    AddEFlags(EFL_KILLME);  // Make sure to ignore further calls into here or
                            // UTIL_Remove.
  }
...
const char *C_BaseEntity::GetDebugName() { return GetClassname(); }
...
#if !defined(NO_ENTITY_PREDICTION)
  // Nothing right now, but in theory you could look at the error in origins and
  // set
...
//-----------------------------------------------------------------------------
void C_BaseEntity::ChangeTeam(int iTeamNum) { m_iTeamNum = iTeamNum; }

//-----------------------------------------------------------------------------
// Purpose:
// Input  : name -
//-----------------------------------------------------------------------------
void C_BaseEntity::SetModelName(string_t name) { m_ModelName = name; }

//-----------------------------------------------------------------------------
// Purpose:
// Output : string_t
//-----------------------------------------------------------------------------
string_t C_BaseEntity::GetModelName(void) const { return m_ModelName; }
...
void C_BaseEntity::ComputePackedOffsets() {
  ...
  if (map->packed_offsets_computed) return;
...
int C_BaseEntity::ComputePackedSize_R(datamap_t *map) {
  ...
  for (i = 0; i < c; i++) {
    ...
    if (field->fieldType != FIELD_EMBEDDED) {
      ...
      if (field->flags & FTYPEDESC_PRIVATE) continue;
    }
...
void C_BaseEntity::SUB_Remove() {
  if (m_iHealth > 0) {
    // this situation can screw up NPCs who can't tell their entity pointers are
    // invalid.
    m_iHealth = 0;
...
void C_BaseEntity::DrawBBoxVisualizations() {
  if (m_fBBoxVisFlags & VISUALIZE_COLLISION_BOUNDS) {
    debugoverlay->AddBoxOverlay(
        ...
        CollisionProp()->OBBMaxs(), CollisionProp()->GetCollisionAngles(), 190,
        190, 0, 0, 0.01);
  }
  ...
  if (m_fBBoxVisFlags & VISUALIZE_SURROUNDING_BOUNDS) {
    ...
    CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins,
                                                 &vecSurroundMaxs);
    debugoverlay->AddBoxOverlay(vec3_origin, vecSurroundMins, vecSurroundMaxs,
                                vec3_angle, 0, 255, 255, 0, 0.01);
  }
...
RenderGroup_t C_BaseEntity::GetRenderGroup() {
  ...
  if (m_nRenderMode == kRenderNone) return RENDER_GROUP_OPAQUE_ENTITY;
  ...
  if (nFXBlend == 0) return RENDER_GROUP_OPAQUE_ENTITY;
...
int C_BaseEntity::SaveData(const char *context, int slot, int type) {
  ...
  if (slot == SLOT_ORIGINALDATA) {
    ...
    // Remember high water mark so that we can detect below if we are reading
    // from a slot not yet predicted into...
...
int C_BaseEntity::RestoreData(const char *context, int slot, int type) {
  ...
  const void *src = (slot == SLOT_ORIGINALDATA) ? GetOriginalNetworkDataObject()
                                                : GetPredictedFrame(slot);
  Assert(src);
  char sz[64];
  if (slot == SLOT_ORIGINALDATA) {
...
void C_BaseEntity::OnPostRestoreData() {
  ...
  // If our model index has changed, then make sure it's reflected in our model
  // pointer.
...
static float AdjustInterpolationAmount(C_BaseEntity *pEntity,
  ...
  if (cl_interp_npcs.GetFloat() > 0) {
    ...
    if (minNPCInterpolation > baseInterpolation) {
      while (pEntity) {
        if (pEntity->IsNPC()) return minNPCInterpolation;
...
float C_BaseEntity::GetInterpolationAmount(int flags) {
  // If single player server is "skipping ticks" everything needs to interpolate
  // for a bit longer
  ...
  return AdjustInterpolationAmount(
      this, TICK_INTERVAL *
                (TIME_TO_TICKS(GetClientInterpAmount()) + serverTickMultiple));
}
...
// capabilities
//-----------------------------------------------------------------------------
int C_BaseEntity::ObjectCaps() { return 0; }
...
void C_BaseEntity::RecordToolMessage() {
  ...
  if (!IsToolRecording()) return;

  if (HasRecordedThisFrame()) return;
...
void C_BaseEntity::CheckCLInterpChanged() {
  ...
  if (flLastValue_Interp != flCurValue_Interp ||
    ...
    // Tell all the existing entities to update their interpolation amounts to
...
int C_BaseEntity::GetCreationTick() const { return m_nCreationTick; }
...
static ConCommand cl_find_ent("cl_find_ent", CC_CL_Find_Ent,
                              "Find and list all client entities with "
                              "classnames that contain the specified "
...
static ConCommand cl_find_ent_index("cl_find_ent_index", CC_CL_Find_Ent_Index,
                                    "Display data for clientside entity "
                                    "matching specified index.\nFormat: "
