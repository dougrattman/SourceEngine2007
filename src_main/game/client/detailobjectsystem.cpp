...
ConVar cl_detaildist("cl_detaildist", "1200", 0,
                     "Distance at which detail props are no longer visible");
ConVar cl_detailfade("cl_detailfade", "400", 0,
                     "Distance across which detail props fade in");
#if defined(USE_DETAIL_SHAPES)
ConVar cl_detail_max_sway("cl_detail_max_sway", "0", FCVAR_ARCHIVE,
                          "Amplitude of the detail prop sway");
ConVar cl_detail_avoid_radius("cl_detail_avoid_radius", "0", FCVAR_ARCHIVE,
                              "radius around detail sprite to avoid players");
ConVar cl_detail_avoid_force("cl_detail_avoid_force", "0", FCVAR_ARCHIVE,
                             "force with which to avoid players ( in units, "
                             "percentage of the width of the detail sprite )");
ConVar cl_detail_avoid_recover_speed(
    "cl_detail_avoid_recover_speed", "0", FCVAR_ARCHIVE,
...
class CDetailModel : public IClientUnknown, public IClientRenderable {
 ...
 public:
  ...
  bool Init(int index, const Vector &org, const QAngle &angles, model_t *pModel,
            ColorRGBExp32 lighting, int lightstyle,
            unsigned char lightstylecount, int orientation);

  bool InitSprite(int index, bool bFlipped, const Vector &org,
                  const QAngle &angles, unsigned short nSpriteIndex,
                  ColorRGBExp32 lighting, int lightstyle,
                  unsigned char lightstylecount, int orientation, float flScale,
                  unsigned char type, unsigned char shapeAngle,
                  unsigned char shapeSize, unsigned char swayAmount);
 ...
 public:
  ...
  void InitShapedSprite(unsigned char shapeAngle, unsigned char shapeSize,
                        unsigned char swayAmount);
  void InitShapeTri();
  void InitShapeCross();

  void DrawSwayingQuad(CMeshBuilder &meshBuilder, Vector vecOrigin,
                       Vector vecSway, Vector2D texul, Vector2D texlr,
                       unsigned char *color, Vector width, Vector height);
#endif
...
struct FastSpriteX4_t {
  // mess with this structure without care and you'll be in a world of trouble.
  // layout matters.
  FourVectors m_Pos;
  fltx4 m_HalfWidth;
  fltx4 m_Height;
  uint8_t m_RGBColor[4][4];
...
class CFastDetailLeafSpriteList : public CClientLeafSubSystemData {
  ...
  // simd pointers into larger array - don't free individually or you will be
  // sad
...
class CDetailObjectSystem : public IDetailObjectSystem,
 ...
 public:
  ...
  void RenderTranslucentDetailObjects(const Vector &viewOrigin,
                                      const Vector &viewForward,
                                      const Vector &viewRight,
                                      const Vector &viewUp, int nLeafCount,
                                      LeafIndex_t *pLeafList);

  // Renders all translucent detail objects in a particular leaf up to a
  // particular point
  void RenderTranslucentDetailObjectsInLeaf(const Vector &viewOrigin,
                                            const Vector &viewForward,
                                            const Vector &viewRight,
                                            const Vector &viewUp, int nLeaf,
                                            const Vector *pVecClosestPoint);
  void RenderFastTranslucentDetailObjectsInLeaf(const Vector &viewOrigin,
                                                const Vector &viewForward,
                                                const Vector &viewRight,
                                                const Vector &viewUp, int nLeaf,
                                                const Vector *pVecClosestPoint);
 ...
 private:
  ...
  int BuildOutSortedSprites(CFastDetailLeafSpriteList *pData,
                            Vector const &viewOrigin, Vector const &viewForward,
                            Vector const &viewRight, Vector const &viewUp);

  void RenderFastSprites(const Vector &viewOrigin, const Vector &viewForward,
                         const Vector &viewRight, const Vector &viewUp,
                         int nLeafCount, LeafIndex_t const *pLeafList);

  void UnserializeFastSprite(FastSpriteX4_t *pSpritex4, int nSubField,
                             DetailObjectLump_t const &lump, bool bFlipped,
                             Vector const &posOffset);
  ...
  int CountFastSpritesInLeafList(int nLeafCount, LeafIndex_t const *pLeafList,
                                 int *nMaxInLeaf) const;
  ...
  int SortSpritesBackToFront(int nLeaf, const Vector &viewOrigin,
                             const Vector &viewForward, SortInfo_t *pSortInfo);
...
IDetailObjectSystem *DetailObjectSystem() { return &s_DetailObjectSystem; }

//-----------------------------------------------------------------------------
// Initialization
//-----------------------------------------------------------------------------

CUtlMap<CDetailModel *, CDetailModel::LightStyleInfo_t>
    CDetailModel::gm_LightStylesMap(DefLessFunc(CDetailModel *));

bool CDetailModel::InitCommon(int index, const Vector &org,
                              const QAngle &angles) {
...
inline int CDetailModel::QuadsToDraw() const { return s_pQuadCount[m_Type]; }

//-----------------------------------------------------------------------------
// Data accessors
//-----------------------------------------------------------------------------
const Vector &CDetailModel::GetRenderOrigin(void) { return m_Origin; }

const QAngle &CDetailModel::GetRenderAngles(void) { return m_Angles; }
...
IPVSNotify *CDetailModel::GetPVSNotifyInterface() { return NULL; }

void CDetailModel::GetRenderBoundsWorldspace(Vector &mins, Vector &maxs) {
  DefaultRenderBoundsWorldspace(this, mins, maxs);
}

bool CDetailModel::ShouldReceiveProjectedTextures(int flags) { return false; }

bool CDetailModel::UsesPowerOfTwoFrameBufferTexture() { return false; }

bool CDetailModel::UsesFullFrameBufferTexture() { return false; }

void CDetailModel::GetShadowRenderBounds(Vector &mins, Vector &maxs,
                                         ShadowType_t shadowType) {
...
// Render setup
//-----------------------------------------------------------------------------
bool CDetailModel::SetupBones(matrix3x4_t *pBoneToWorldOut, int nMaxBones,
                              int boneMask, float currentTime) {
...
void CDetailModel::SetupWeights(const matrix3x4_t *pBoneToWorld,
                                int nFlexWeightCount, float *pFlexWeights,
                                float *pFlexDelayedWeights) {}
...
// Render baby!
//-----------------------------------------------------------------------------
const model_t *CDetailModel::GetModel() const { return m_pModel; }

int CDetailModel::DrawModel(int flags) {
  if ((m_Alpha == 0) || (!m_pModel)) return 0;

  int drawn = modelrender->DrawModel(flags, this, MODEL_INSTANCE_INVALID,
                                     -1,  // no entity index
...
int CDetailModel::GetFxBlend() { return m_Alpha; }
...
CDetailModel::~CDetailModel() {
  ...
  if (m_bHasLightStyle) gm_LightStylesMap.Remove(this);
}
...
bool CDetailModel::Init(int index, const Vector &org, const QAngle &angles,
                        model_t *pModel, ColorRGBExp32 lighting, int lightstyle,
                        unsigned char lightstylecount, int orientation) {
  m_Color = lighting;
...
bool CDetailModel::InitSprite(int index, bool bFlipped, const Vector &org,
                              const QAngle &angles, unsigned short nSpriteIndex,
                              ColorRGBExp32 lighting, int lightstyle,
                              unsigned char lightstylecount, int orientation,
                              float flScale, unsigned char type,
                              unsigned char shapeAngle, unsigned char shapeSize,
                              unsigned char swayAmount) {
  m_Color = lighting;
  if (lightstylecount > 0) {
    m_bHasLightStyle = 1;
  ...
  if (type == DETAIL_PROP_TYPE_SHAPE_TRI ||
      type == DETAIL_PROP_TYPE_SHAPE_CROSS || swayAmount > 0) {
    m_Angles = angles;
...
#ifdef USE_DETAIL_SHAPES
void CDetailModel::InitShapedSprite(unsigned char shapeAngle,
                                    unsigned char shapeSize,
                                    unsigned char swayAmount) {
...
void CDetailModel::InitShapeTri(void) {
  ...
  // calculate the vectors for the three sides so they can be used in the
  // sorting test as well as in drawing
  for (int i = 0; i < 3; i++) {
...
void CDetailModel::InitShapeCross(void) {
  AngleVectors(m_Angles, &m_pAdvInfo->m_vecAnglesForward[0],
               &m_pAdvInfo->m_vecAnglesRight[0], &m_pAdvInfo->m_vecAnglesUp[0]);
}
#endif
  ...
  if (m_bHasLightStyle) {
    ...
    if (iInfo != gm_LightStylesMap.InvalidIndex()) {
      ...
      for (int i = 0; i < nLightStyles; ++i) {
        DetailPropLightstylesLump_t &lighting =
            s_DetailObjectSystem.DetailLighting(iLightStyle + i);
        val = engine->LightStyleValue(lighting.m_Style);
        if (val != 0) {
          color[0] += val * TexLightToLinear(lighting.m_Lighting.r,
                                             lighting.m_Lighting.exponent);
          color[1] += val * TexLightToLinear(lighting.m_Lighting.g,
                                             lighting.m_Lighting.exponent);
          color[2] += val * TexLightToLinear(lighting.m_Lighting.b,
                                             lighting.m_Lighting.exponent);
        }
      }
    }
...
void CDetailModel::DrawTypeSprite(CMeshBuilder &meshBuilder) {
  ...
  DetailPropSpriteDict_t &dict =
      s_DetailObjectSystem.DetailSpriteDict(m_SpriteInfo.m_nSpriteIndex);
  ...
  if (m_pAdvInfo) {
    ...
    float flSwayAmplitude =
        m_pAdvInfo->m_flSwayAmount * cl_detail_max_sway.GetFloat();
    if (flSwayAmplitude > 0) {
      // sway based on time plus a random seed that is constant for this
...
void CDetailModel::DrawTypeShapeCross(CMeshBuilder &meshBuilder) {
  ...
  // What a shameless re-use of bits (m_pModel == 0 when it should be flipped
  // horizontally)
  if (!m_pModel) {
    texul.x = dict.m_TexLR.x;
    texlr.x = dict.m_TexUL.x;
  ...
  // sway based on time plus a random seed that is constant for this instance of
  // the sprite
  Vector vecSway = (m_pAdvInfo->m_vecCurrentAvoid * flSizeX * 2);
  float flSwayAmplitude =
      m_pAdvInfo->m_flSwayAmount * cl_detail_max_sway.GetFloat();
  if (flSwayAmplitude > 0) {
    vecSway += UTIL_YawToVector(m_pAdvInfo->m_flSwayYaw) *
               sin(gpGlobals->curtime + m_Origin.x) * flSwayAmplitude;
  }
  ...
  // do dot products with the forward and right vectors to determine the
  // quadrant the viewer is in assume forward points North , right points East
  /*
                     N
                     |
                  3  |  0
  ...
  */
  ...
  // debugoverlay->AddLineOverlay( m_Origin, m_Origin + right * 20, 255, 0, 0,
  // true, 0.01 );  debugoverlay->AddLineOverlay( m_Origin, m_Origin + forward *
  // 20, 0, 0, 255, true, 0.01 );

  int iDrawn = 0;
  while (iDrawn < 4) {
    switch (iBranch) {
      case 0:  // south
        DrawSwayingQuad(meshBuilder, vecOrigin, vecSway, texumid, texlr, color,
                        -forward, up);
        break;
      case 1:  // west
        DrawSwayingQuad(meshBuilder, vecOrigin, vecSway, texumid, texll, color,
                        -right, up);
        break;
      case 2:  // north
        DrawSwayingQuad(meshBuilder, vecOrigin, vecSway, texumid, texll, color,
...
void CDetailModel::DrawTypeShapeTri(CMeshBuilder &meshBuilder) {
  ...
  bool bOutsideA =
      DotProduct(m_pAdvInfo->m_vecAnglesForward[0], viewOffset) > 0;
  bool bOutsideB =
      DotProduct(m_pAdvInfo->m_vecAnglesForward[1], viewOffset) > 0;
  bool bOutsideC =
      DotProduct(m_pAdvInfo->m_vecAnglesForward[2], viewOffset) > 0;
  ...
  while (iDrawn < 3) {
    ...
    VectorMA(vecOrigin, m_pAdvInfo->m_flShapeSize * flWidth,
             m_pAdvInfo->m_vecAnglesForward[iBranch], vecOrigin);

    // sway is calculated per side so they don't sway exactly the same
    Vector vecSway = (m_pAdvInfo->m_vecCurrentAvoid * flWidth) +
                     vecSwayYaw *
                         sin(gpGlobals->curtime + m_Origin.x + iBranch) *
                         flSwayAmplitude;

    DrawSwayingQuad(meshBuilder, vecOrigin, vecSway, texul, texlr, color,
                    vecWidth, vecHeight);

    iDrawn++;
    iBranch++;
    if (iBranch > 2) iBranch = 0;
  }
}
#endif
...
void CDetailModel::UpdatePlayerAvoid(void) {
  ...
  if (flForce < 0.1) return;

  if (m_pAdvInfo == NULL) return;
  ...
  partition->EnumerateElementsInSphere(PARTITION_CLIENT_SOLID_EDICTS, m_Origin,
                                       flRadius, false, &avoid);
  ...
  m_pAdvInfo->m_vecCurrentAvoid[0] = Approach(
      vecMaxAvoid[0], m_pAdvInfo->m_vecCurrentAvoid[0], flRecoverSpeed);
  m_pAdvInfo->m_vecCurrentAvoid[1] = Approach(
      vecMaxAvoid[1], m_pAdvInfo->m_vecCurrentAvoid[1], flRecoverSpeed);
  m_pAdvInfo->m_vecCurrentAvoid[2] = Approach(
      vecMaxAvoid[2], m_pAdvInfo->m_vecCurrentAvoid[2], flRecoverSpeed);
}
#endif
...
//-----------------------------------------------------------------------------
#ifdef USE_DETAIL_SHAPES
void CDetailModel::DrawSwayingQuad(CMeshBuilder &meshBuilder, Vector vecOrigin,
                                   Vector vecSway, Vector2D texul,
                                   Vector2D texlr, unsigned char *color,
...
CDetailObjectSystem::CDetailObjectSystem()
    : m_DetailSpriteDict(0, 32),
      m_DetailObjectDict(0, 32),
      m_DetailSpriteDictFlipped(0, 32) {
...
void CDetailObjectSystem::LevelInitPreEntity() {
  ...
  if (m_DetailObjects.Count() || m_DetailSpriteDict.Count()) {
    ...
    float flRatio = pMat->GetMappingWidth() * 1.0f / pMat->GetMappingHeight();
    if (flRatio > 1.0) {
...
void CDetailObjectSystem::LevelInitPostEntity() {
  ...
  if (pWorld && pWorld->GetDetailSpriteMaterial() &&
      *(pWorld->GetDetailSpriteMaterial())) {
  ...
  if (GetDetailController()) {
    cl_detailfade.SetValue(
        std::min(m_flDefaultFadeStart,
                 static_cast<float>(GetDetailController()->m_flFadeStartDist)));
    cl_detaildist.SetValue(
        std::min(m_flDefaultFadeEnd,
                 static_cast<float>(GetDetailController()->m_flFadeEndDist)));
  } else {
...
void CDetailObjectSystem::UnserializeDetailSprites(CUtlBuffer &buf) {
  ...
  while (--count >= 0) {
    ...
    swap(m_DetailSpriteDictFlipped[flipi].m_TexUL.x,
         m_DetailSpriteDictFlipped[flipi].m_TexLR.x);
  }
}
...
ConVar cl_detail_multiplier("cl_detail_multiplier", "1", FCVAR_CHEAT,
                            "extra details to create");

#define SPRITE_MULTIPLIER (cl_detail_multiplier.GetInt())

ConVar cl_fastdetailsprites("cl_fastdetailsprites", "1", FCVAR_CHEAT,
                            "whether to use new detail sprite system");

static bool DetailObjectIsFastSprite(DetailObjectLump_t const &lump) {
  return ((cl_fastdetailsprites.GetInt()) &&
          (lump.m_Type == DETAIL_PROP_TYPE_SPRITE) &&
          (lump.m_LightStyleCount == 0) && (lump.m_Orientation == 2) &&
          (lump.m_ShapeAngle == 0) && (lump.m_ShapeSize == 0) &&
          (lump.m_SwayAmount == 0));
}
...
void CDetailObjectSystem::ScanForCounts(CUtlBuffer &buf,
  ...
  while (--count >= 0) {
    ...
    if (detailObjectLeaf != lump.m_Leaf) {
      ...
      nFast += (0 - nFast) & 3;
      nMaxFast = std::max(nMaxFast, nNumFastInLeaf);
      nMaxOld = std::max(nMaxOld, nNumOldInLeaf);
      nNumOldInLeaf = 0;
      nNumFastInLeaf = 0;
...
void CDetailObjectSystem::UnserializeModels(CUtlBuffer &buf) {
  ...
  int nMaxOldInLeaf;
  int nMaxFastInLeaf;
  ScanForCounts(buf, &nNumOldStyleObjects, &nNumFastSpritesToAllocate,
                &nMaxOldInLeaf, &nMaxFastInLeaf);

  FreeSortBuffers();

  if (nMaxOldInLeaf) {
    m_pSortInfo = reinterpret_cast<SortInfo_t *>(MemAlloc_AllocAligned(
        (3 + nMaxOldInLeaf) * sizeof(SortInfo_t), sizeof(fltx4)));
  }
  if (nMaxFastInLeaf) {
    m_pFastSortInfo = reinterpret_cast<SortInfo_t *>(MemAlloc_AllocAligned(
        (3 + nMaxFastInLeaf) * sizeof(SortInfo_t), sizeof(fltx4)));
  ...
  if (nNumFastSpritesToAllocate) {
    ...
    m_pFastSpriteData =
        reinterpret_cast<FastSpriteX4_t *>(MemAlloc_AllocAligned(
  ...
  while (--count >= 0) {
    ...
    if (detailObjectLeaf != lump.m_Leaf) {
      if (detailObjectLeaf != -1) {
        ...
        ClientLeafSystem()->SetDetailObjectsInLeaf(
            detailObjectLeaf, firstDetailObject, detailObjectCount);
      }
    ...
    if (DetailObjectIsFastSprite(lump)) {
      for (int i = 0; i < SPRITE_MULTIPLIER; i++) {
        ...
        if (nSubField == 0)
          pSpritex4->ReplicateFirstEntryToOthers();  // keep bad numbers out to
                                                     // prevent denormals, etc
        nCurFastObject++;
        ...
        case DETAIL_PROP_TYPE_SHAPE_TRI: {
          for (int i = 0; i < SPRITE_MULTIPLIER; i++) {
            ...
            m_DetailObjects[newObj].InitSprite(
                newObj, bFlipped, pos, lump.m_Angles, lump.m_DetailModel,
                lump.m_Lighting, lump.m_LightStyles, lump.m_LightStyleCount,
                lump.m_Orientation, lump.m_flScale, lump.m_Type,
                lump.m_ShapeAngle, lump.m_ShapeSize, lump.m_SwayAmount);
            ++detailObjectCount;
          }
        } break;
      }
...
Vector CDetailObjectSystem::GetSpriteMiddleBottomPosition(
    DetailObjectLump_t const &lump) const {
  DetailPropSpriteDict_t &dict =
      s_DetailObjectSystem.DetailSpriteDict(lump.m_DetailModel);
...
void CDetailObjectSystem::UnserializeFastSprite(FastSpriteX4_t *pSpritex4,
                                                int nSubField,
                                                DetailObjectLump_t const &lump,
                                                bool bFlipped,
                                                Vector const &posOffset) {
  ...
  SubFloat(pSpritex4->m_HalfWidth, nSubField) =
      0.5 * lump.m_flScale * (pSDef->m_LR.x - pSDef->m_UL.x);
  SubFloat(pSpritex4->m_Height, nSubField) =
      lump.m_flScale * (pSDef->m_LR.y - pSDef->m_UL.y);
  if (!bFlipped) {
...
// Count the number of detail sprites in the leaf list
//-----------------------------------------------------------------------------
int CDetailObjectSystem::CountSpritesInLeafList(int nLeafCount,
                                                LeafIndex_t *pLeafList) const {
  VPROF_BUDGET("CDetailObjectSystem::CountSpritesInLeafList",
...
int CDetailObjectSystem::CountFastSpritesInLeafList(
    int nLeafCount, LeafIndex_t const *pLeafList, int *nMaxFoundInLeaf) const {
  VPROF_BUDGET("CDetailObjectSystem::CountSpritesInLeafList",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);
  int nCount = 0;
  int nMax = 0;
  for (int i = 0; i < nLeafCount; ++i) {
    CFastDetailLeafSpriteList *pData =
        reinterpret_cast<CFastDetailLeafSpriteList *>(
            ClientLeafSystem()->GetSubSystemDataInLeaf(
                pLeafList[i], CLSUBSYSTEM_DETAILOBJECTS));
    if (pData) {
      nCount += pData->m_nNumSprites;
      nMax = std::max(nMax, pData->m_nNumSprites);
    }
  }
...
// Count the number of detail sprite quads in the leaf list
//-----------------------------------------------------------------------------
int CDetailObjectSystem::CountSpriteQuadsInLeafList(
    int nLeafCount, LeafIndex_t *pLeafList) const {
#ifdef USE_DETAIL_SHAPES
  VPROF_BUDGET("CDetailObjectSystem::CountSpritesInLeafList",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);
  int nQuadCount = 0;
...
#define TREATASINT(x) (*(((int32_t const *)(&(x)))))

//-----------------------------------------------------------------------------
// Sorts sprites in back-to-front order
//-----------------------------------------------------------------------------
inline bool CDetailObjectSystem::SortLessFunc(
...
int CDetailObjectSystem::SortSpritesBackToFront(int nLeaf,
                                                const Vector &viewOrigin,
                                                const Vector &viewForward,
                                                SortInfo_t *pSortInfo) {
  VPROF_BUDGET("CDetailObjectSystem::SortSpritesBackToFront",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);
  int nFirstDetailObject, nDetailObjectCount;
  ClientLeafSystem()->GetDetailObjectsInLeaf(nLeaf, nFirstDetailObject,
                                             nDetailObjectCount);
  ...
  for (int j = nFirstDetailObject; j < nDetailObjectCount; ++j) {
    ...
    if (flSqDist >= flMaxSqDist) continue;
...
int CDetailObjectSystem::BuildOutSortedSprites(CFastDetailLeafSpriteList *pData,
                                               ...
                                               Vector const &viewUp) {
  // part 1 - do all vertex math, fading, etc into a buffer, using as much simd
  // as we can
  ...
  fltx4 falloffFactor =
      ReplicateX4(1.0 / (m_flCurMaxSqDist - m_flCurFadeSqDist));
  ...
  do {
    ...
    nLastBfMask = TestSignSIMD(
        OrSIMD(ofsDotFwd, CmpGtSIMD(distanceSquared, maxsqdist)));  //  cull
    if (nLastBfMask != 0xf) {
      FourVectors dx1;
      dx1.x = fnegate(ofs.y);
      dx1.y = (ofs.x);
      ...
      alpha =
          SubSIMD(Four_Ones, MinSIMD(MaxSIMD(alpha, Four_Zeros), Four_Ones));

      pQuadBufferOut->m_Alpha =
          AddSIMD(Four_MagicNumbers, MulSIMD(Four_255s, alpha));
...
void CDetailObjectSystem::RenderFastSprites(const Vector &viewOrigin,
                                            const Vector &viewForward,
                                            const Vector &viewRight,
                                            const Vector &viewUp,
                                            int nLeafCount,
  ...
  int nQuadCount =
      CountFastSpritesInLeafList(nLeafCount, pLeafList, &nMaxInLeaf);
  if (nQuadCount == 0) return;
  if (r_DrawDetailProps.GetInt() == 0) return;
  ...
  int nQuadsToDraw = std::min(nQuadCount, nMaxQuadsToDraw);
  ...
  for (int i = 0; i < nLeafCount; ++i) {
    ...
    CFastDetailLeafSpriteList *pData =
        reinterpret_cast<CFastDetailLeafSpriteList *>(
            ClientLeafSystem()->GetSubSystemDataInLeaf(
                nLeaf, CLSUBSYSTEM_DETAILOBJECTS));

    if (pData) {
      Assert(pData->m_nNumSprites);  // ptr with no sprites?

      int nCount = BuildOutSortedSprites(pData, viewOrigin, viewForward,
                                         viewRight, viewUp);
      ...
      while (nCount) {
        ...
        int nToDraw = std::min(nCount, nQuadsRemaining);
        nCount -= nToDraw;
        ...
        while (nToDraw--) {
          ...
          FastSpriteQuadBuildoutBufferNonSIMDView_t const *pquad =
              pQuadBuffer + nSIMDIdx;

          // voodoo - since everything is in 4s, offset structure pointer by a
          // couple of floats to handle sub-index
          pquad = (FastSpriteQuadBuildoutBufferNonSIMDView_t const
                       *)(((int)(pquad)) + (nSubIdx << 2));
          uint8_t const *pColorsCasted =
              reinterpret_cast<uint8_t const *>(pquad->m_Alpha);
          ...
          meshBuilder.Position3f(pquad->m_flX0[0], pquad->m_flY0[0],
                                 pquad->m_flZ0[0]);
          meshBuilder.Color4ubv(color);
          meshBuilder.TexCoord2f(0, pDict->m_TexLR.x, pDict->m_TexLR.y);
          meshBuilder.AdvanceVertex();

          meshBuilder.Position3f(pquad->m_flX1[0], pquad->m_flY1[0],
                                 pquad->m_flZ1[0]);
          meshBuilder.Color4ubv(color);
          meshBuilder.TexCoord2f(0, pDict->m_TexLR.x, pDict->m_TexUL.y);
          meshBuilder.AdvanceVertex();

          meshBuilder.Position3f(pquad->m_flX2[0], pquad->m_flY2[0],
                                 pquad->m_flZ2[0]);
          meshBuilder.Color4ubv(color);
          meshBuilder.TexCoord2f(0, pDict->m_TexUL.x, pDict->m_TexUL.y);
          meshBuilder.AdvanceVertex();

          meshBuilder.Position3f(pquad->m_flX3[0], pquad->m_flY3[0],
                                 pquad->m_flZ3[0]);
          meshBuilder.Color4ubv(color);
...
// Renders all translucent detail objects in a particular set of leaves
//-----------------------------------------------------------------------------
void CDetailObjectSystem::RenderTranslucentDetailObjects(
    const Vector &viewOrigin, const Vector &viewForward,
    const Vector &viewRight, const Vector &viewUp, int nLeafCount,
    LeafIndex_t *pLeafList) {
  VPROF_BUDGET("CDetailObjectSystem::RenderTranslucentDetailObjects",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);
  ...
  for (int i = 0; i < nLeafCount; ++i) {
    ...
    int nCount =
        SortSpritesBackToFront(nLeaf, viewOrigin, viewForward, pSortInfo);
...
void CDetailObjectSystem::RenderFastTranslucentDetailObjectsInLeaf(
    const Vector &viewOrigin, const Vector &viewForward,
  ...
  if (m_nSortedFastLeaf != nLeaf) {
    ...
    pData->m_nNumPendingSprites = BuildOutSortedSprites(
        pData, viewOrigin, viewForward, viewRight, viewUp);
    pData->m_nStartSpriteIndex = 0;
  }
  if (pData->m_nNumPendingSprites == 0) return;
  ...
  int nQuadsToDraw = std::min(nCount, nMaxQuadsToDraw);
  int nQuadsRemaining = nQuadsToDraw;
  ...
  while (nCount && (pDraw->m_flDistance >= flMinDistance)) {
    ...
    while (nToDraw--) {
      ...
      // couple of floats to handle sub-index
      pquad =
          (FastSpriteQuadBuildoutBufferNonSIMDView_t const *)(((int)(pquad)) +
                                                              (nSubIdx << 2));
      uint8_t const *pColorsCasted =
          reinterpret_cast<uint8_t const *>(pquad->m_Alpha);
...
//-----------------------------------------------------------------------------
// Renders a subset of the detail objects in a particular leaf (for interleaving
// with other translucent entities)
//-----------------------------------------------------------------------------
void CDetailObjectSystem::RenderTranslucentDetailObjectsInLeaf(
    const Vector &viewOrigin, const Vector &viewForward,
    const Vector &viewRight, const Vector &viewUp, int nLeaf,
    const Vector *pVecClosestPoint) {
  VPROF_BUDGET("CDetailObjectSystem::RenderTranslucentDetailObjectsInLeaf",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);

  RenderFastTranslucentDetailObjectsInLeaf(viewOrigin, viewForward, viewRight,
                                           viewUp, nLeaf, pVecClosestPoint);
  ...
  if (m_nSortedLeaf != nLeaf) {
    ...
    if (nSpriteCount == 0) return;

    // Sort detail sprites in each leaf independently; then render them
    m_nSpriteCount =
        SortSpritesBackToFront(nLeaf, viewOrigin, viewForward, m_pSortInfo);
    Assert(m_nSpriteCount <= nSpriteCount);
  }

  // No more to draw? Bye!
  if (m_nSpriteCount == m_nFirstSprite) return;
  ...
  if (m_pSortInfo[m_nFirstSprite].m_flDistance < flMinDistance) return;
  ...
  int nQuadsDrawn = 0;
  while (m_nFirstSprite < m_nSpriteCount &&
         m_pSortInfo[m_nFirstSprite].m_flDistance >= flMinDistance) {
...
bool CDetailObjectSystem::EnumerateLeaf(int leaf, int context) {
  VPROF_BUDGET("CDetailObjectSystem::EnumerateLeaf",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);
  Vector v;
  int firstDetailObject, detailObjectCount;

  EnumContext_t *pCtx = (EnumContext_t *)context;
  ClientLeafSystem()->DrawDetailObjectsInLeaf(
      leaf, pCtx->m_BuildWorldListNumber, firstDetailObject, detailObjectCount);
...
void CDetailObjectSystem::BuildDetailObjectRenderLists(
  ...
  VPROF_BUDGET("CDetailObjectSystem::BuildDetailObjectRenderLists",
               VPROF_BUDGETGROUP_DETAILPROP_RENDERING);

  if (!g_pClientMode->ShouldDrawDetailObjects() ||
      (r_DrawDetailProps.GetInt() == 0))
    return;

  // Don't bother doing any of this if the level doesn't have detail props.
  if ((!m_pFastSpriteData) && (m_DetailObjects.Count() == 0)) return;
  ...
  for (int i = m_DetailObjectDict.Size(); --i >= 0;) {
    if (modelinfo->ModelHasMaterialProxy(m_DetailObjectDict[i].m_pModel)) {
      modelinfo->RecomputeTranslucency(m_DetailObjectDict[i].m_pModel, 0, 0,
                                       NULL);
    }
  }
  ...
  m_flCurFadeSqDist = std::min(m_flCurFadeSqDist, m_flCurMaxSqDist - 1);
  m_flCurFalloffFactor = 255.0f / (m_flCurMaxSqDist - m_flCurFadeSqDist);

  ISpatialQuery *pQuery = engine->GetBSPTreeQuery();
  pQuery->EnumerateLeavesInSphere(CurrentViewOrigin(), cl_detaildist.GetFloat(),
                                  this, (int)&ctx);
}
