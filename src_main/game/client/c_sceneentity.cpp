...
//			objectID -
//-----------------------------------------------------------------------------
void RecvProxy_ForcedClientTime(const CRecvProxyData *pData, void *pStruct,
                                void *pOut) {
...
IMPLEMENT_CLIENTCLASS_DT(C_SceneEntity, DT_SceneEntity, CSceneEntity)
  RecvPropInt(RECVINFO(m_nSceneStringIndex)),
      ...
      RecvPropBool(RECVINFO(m_bPaused)), RecvPropBool(RECVINFO(m_bMultiplayer)),
      RecvPropFloat(RECVINFO(m_flForceClientTime), 0,
                    RecvProxy_ForcedClientTime),
      RecvPropUtlVector(RECVINFO_UTLVECTOR(m_hActorList), MAX_ACTORS_IN_SCENE,
...
ConVar mp_usehwmvcds(
    "mp_usehwmvcds", "0", NULL,
    "Enable the use of the hw morph vcd(s). (-1 = never, 1 = always, 0 = based "
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_SceneEntity::GetHWMorphSceneFileName(const char *pFilename,
                                            char *pHWMFilename,
  ...
  if ((V_strstr(pFilename, "/high") != NULL) ||
      (V_strstr(pFilename, "\\high") != NULL)) {
  ...
  V_strcpy(pHWMFilename, hwm_file_name_size, szSceneHWM);
  return true;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_SceneEntity::SetupClientOnlyScene(const char *pszFilename,
  ...
  if (m_hOwner.Get()) {
    ...
    if (m_pScene) {
      ...
      // Reset the flex weights when we start a new scene.  This is normally
      // done on the player model, but since we don't have a player here yet -
      // we need to do this!
...
//-----------------------------------------------------------------------------
void C_SceneEntity::ProcessEvent(float currenttime, CChoreoScene *scene,
                                 CChoreoEvent *event) {
  return;
}
...
//-----------------------------------------------------------------------------
bool C_SceneEntity::CheckEvent(float currenttime, CChoreoScene *scene,
                               CChoreoEvent *event) {
  return true;
}
...
C_BaseFlex *C_SceneEntity::FindNamedActor(CChoreoActor *pChoreoActor) {
  ...
  if (idx < 0 || idx >= m_hActorList.Count()) return NULL;
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::StartEvent(float currenttime, CChoreoScene *scene,
                               CChoreoEvent *event) {
  Assert(event);

  if (!Q_stricmp(event->GetName(), "NULL")) {
    Scene_Printf("%s : %8.2f:  ignored %s\n", GetSceneFileName(), currenttime,
  ...
  if (actor) {
    ...
    if (NULL == pActor) {
      // This can occur if we haven't been networked an actor yet... we need to
      // queue it so that we can
      //  fire off the start event as soon as we have the actor resident on the
      //  client.
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchProcessLoop(CChoreoScene *scene,
                                        CChoreoEvent *event) {
...
//			*parameters -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchStartSpeak(CChoreoScene *scene, C_BaseFlex *actor,
                                       CChoreoEvent *event,
...
void C_SceneEntity::DispatchEndSpeak(CChoreoScene *scene, C_BaseFlex *actor,
                                     CChoreoEvent *event) {
  if (IsClientOnly()) {
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::EndEvent(float currenttime, CChoreoScene *scene,
                             CChoreoEvent *event) {
  Assert(event);
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchStartFlexAnimation(CChoreoScene *scene,
                                               C_BaseFlex *actor,
                                               CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event, NULL, IsClientOnly());
}
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchEndFlexAnimation(CChoreoScene *scene,
                                             C_BaseFlex *actor,
                                             CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, false);
}
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchStartExpression(CChoreoScene *scene,
                                            C_BaseFlex *actor,
                                            CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event, NULL, IsClientOnly());
}
...
//			*event -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchEndExpression(CChoreoScene *scene,
                                          C_BaseFlex *actor,
                                          CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, false);
}
...
//			*parameters -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchStartGesture(CChoreoScene *scene, C_BaseFlex *actor,
                                         CChoreoEvent *event) {
  // Ingore 0 gestures
...
// Input  : *actor -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchStartSequence(CChoreoScene *scene, CBaseFlex *actor,
                                          CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event, NULL, IsClientOnly());
}
...
// Input  : *actor -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchEndSequence(CChoreoScene *scene, CBaseFlex *actor,
                                        CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, false);
}
...
//			*parameters -
//-----------------------------------------------------------------------------
void C_SceneEntity::DispatchEndGesture(CChoreoScene *scene, C_BaseFlex *actor,
                                       CChoreoEvent *event) {
  // Ingore 0 gestures
  if (!Q_stricmp(event->GetName(), "NULL")) return;
...
void C_SceneEntity::QueueStartEvent(float starttime, CChoreoScene *scene,
                                    CChoreoEvent *event) {
  // Check for duplicates
  int c = m_QueuedEvents.Count();
  for (int i = 0; i < c; ++i) {
    const QueuedEvents_t &check = m_QueuedEvents[i];
    if (check.scene == scene && check.event == event) return;
  }
...
//			forceClientSync - unused for now, we may want to
...
void C_SceneEntity::PrefetchAnimBlocks(CChoreoScene *pScene) {
  ...
  if (!pScene || !m_bMultiplayer) return;
              ...
              if (pStudioHdr) {
                ...
                for (int k = 0; k < seqdesc.groupsize[0]; ++k) {
                  for (int j = 0; j < seqdesc.groupsize[1]; ++j) {
                    ...
                    int iBaseAnimation =
                        pStudioHdr->iRelativeAnim(iSequence, iAnimation);
                    mstudioanimdesc_t &animdesc =
                        pStudioHdr->pAnimdesc(iBaseAnimation);
