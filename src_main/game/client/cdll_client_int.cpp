...
void ProcessCacheUsedMaterials() {
  if (!g_bRequestCacheUsedMaterials) return;
...
class CClientDLLSharedAppSystems : public IClientDLLSharedAppSystems {
 public:
  CClientDLLSharedAppSystems() {
    AddAppSystem("soundemittersystem.dll",
...
class CHLVoiceStatusHelper : public IVoiceStatusHelper {
 public:
  ...
  virtual bool CanShowSpeakerLabels() { return true; }
};
...
void TrackBoneSetupEnt(C_BaseAnimating *pEnt) {
  ...
  if (!cl_ShowBoneSetupEnts.GetInt()) return;
...
class CHLClient : public IBaseClientDLL {
 public:
  ...
  virtual int Init(CreateInterfaceFn appSystemFactory,
                   CreateInterfaceFn physicsFactory, CGlobalVarsBase *pGlobals);
...
int CHLClient::Init(CreateInterfaceFn appSystemFactory,
  ...
  if ((modelinfo = (IVModelInfoClient *)appSystemFactory(
    ...
    return false;
  if ((enginevgui =
           (IEngineVGui *)appSystemFactory(VENGINE_VGUI_VERSION, NULL)) == NULL)
    return false;
  ...
  if ((g_pClientGameStatsUploader = (IUploadGameStats *)appSystemFactory(
  ...
  if (!g_pMaterialSystemHardwareConfig) return false;
  ...
  if (!Initializer::InitializeAllObjects()) return false;

  if (!ParticleMgr()->Init(MAX_TOTAL_PARTICLES, materials)) return false;

  if (!VGui_Startup(appSystemFactory)) return false;
  ...
  IGameSystem::Add(g_pColorCorrectionMgr);  // NOTE: This must happen prior to
                                            // ClientThinkList (color correction
                                            // is updated there)
  ...
  if (!PhysicsDLLInit(physicsFactory)) return false;
...
// initialized
//-----------------------------------------------------------------------------
void CHLClient::PostInit() { IGameSystem::PostInitAllSystems(); }
...
//-----------------------------------------------------------------------------
void CHLClient::HudText(const char *message) { DispatchHudText(message); }
...
uint32_t CHLClient::GetPresenceID(const char *pIDName) {
  return presence->GetPresenceID(pIDName);
}

const char *CHLClient::GetPropertyIdString(const uint32_t id) {
  return presence->GetPropertyIdString(id);
}

void CHLClient::GetPropertyDisplayString(uint32_t id, uint32_t value,
                                         char *pOutput, int nBytes) {
...
void CHLClient::View_Fade(ScreenFade_t *pSF) {
  if (pSF != NULL) vieweffects->Fade(*pSF);
}
...
void CHLClient::LevelShutdown(void) {
  ...
  // string tables are cleared on disconnect from a server, so reset our global
  // pointers to NULL
...
// Output : int
//-----------------------------------------------------------------------------
int CHLClient::GetSpriteSize(void) const { return sizeof(CEngineSprite); }
...
//			bTalking -
//-----------------------------------------------------------------------------
void CHLClient::VoiceStatus(int entindex, int bTalking) {
...
void OnVguiScreenTableChanged(void *object, INetworkStringTable *stringTable,
  ...
  if (pPanel) PanelMetaClassMgr()->DestroyPanelMetaClass(pPanel);
}
...
void CHLClient::InstallStringTableCallback(const char *tableName) {
  ...
  if (!Q_strcasecmp(tableName, "VguiScreen")) {
    ...
    g_StringTableVguiScreen->SetStringChangedCallback(NULL,
                                                      OnVguiScreenTableChanged);
...
void ClearDataChangedEvent(int iStoredEvent) {
  if (iStoredEvent != -1) g_DataChangedEvents.Remove(iStoredEvent);
}
...
void UpdateClientRenderableInPVSStatus() {
  ...
  FOR_EACH_LL(theList, i) {
    ...
    if (pInfo->m_InPVSStatus & INPVS_YES) {
      ...
      // We need to set the INPVS_YES_THISFRAME flag if it's in this frame at
      // all, so we don't tell the entity it's not in the PVS anymore at the end
      // of the frame.
      ...
      // This entity doesn't think it's in the PVS yet. If it is now in the PVS,
      // let it know.
      if (g_pClientLeafSystem->IsRenderableInPVS(pInfo->m_pRenderable)) {
        pInfo->m_InPVSStatus |=
...
void UpdatePVSNotifiers() {
  ...
  // have INPVS_THISFRAME set so we can tell the entities that aren't in the PVS
  // anymore so.
  CUtlLinkedList<CClientEntityList::CPVSNotifyInfo, unsigned short> &theList =
      ClientEntityList().GetPVSNotifiers();
  FOR_EACH_LL(theList, i) {
...
void OnRenderStart() {
  ...
  // used. NOTE: the only thing that happens in CAM_Think is thirdperson related
  // code.
  input->CAM_Think();
  ...
  // This will place all entities in the correct position in world space and in
  // the KD-tree
...
// Save/restore system hooks
CSaveRestoreData *CHLClient::SaveInit(int size) { return ::SaveInit(size); }
...
void CHLClient::EmitSentenceCloseCaption(char const *tokenstream) {
  ...
  if (!closecaption.GetBool()) return;
...
// NEW INTERFACES
// save game screenshot writing
void CHLClient::WriteSaveGameScreenshotOfSize(const char *pFilename, int width,
                                              int height) {
