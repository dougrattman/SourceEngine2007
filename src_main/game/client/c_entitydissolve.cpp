...
//------------------------------------------------------------------------------
IMotionEvent::simresult_e C_EntityDissolve::Simulate(
...
// Tesla effect
//-----------------------------------------------------------------------------
static void FX_BuildTesla(C_BaseEntity *pEntity, Vector &vecOrigin,
                          Vector &vecEnd) {
  BeamInfo_t beamInfo;
...
//-----------------------------------------------------------------------------
void C_EntityDissolve::BuildTeslaEffect(mstudiobbox_t *pHitBox,
                                        const matrix3x4_t &hitboxToWorld,
                                        bool bRandom, float flYawOffset) {
  Vector vecOrigin;
  QAngle vecAngles;
  ...
  do {
    ...
    UTIL_TraceLine(vecOrigin, vecOrigin + (vecForward * 192), MASK_SHOT,
                   pEntity, COLLISION_GROUP_NONE, &tr);
...
//-----------------------------------------------------------------------------
void C_EntityDissolve::ComputeRenderInfo(mstudiobbox_t *pHitBox,
                                         const matrix3x4_t &hitboxToWorld,
                                         Vector *pVecAbsOrigin, Vector *pXVec,
                                         Vector *pYVec) {
...
// Sparks!
//-----------------------------------------------------------------------------
void C_EntityDissolve::DoSparks(mstudiohitboxset_t *set,
                                matrix3x4_t *hitboxbones[MAXSTUDIOBONES]) {
  if (m_flNextSparkTime > gpGlobals->curtime) return;

  float dt = m_flStartTime + m_flFadeOutStart - gpGlobals->curtime;
  dt = std::clamp(dt, 0.0f, m_flFadeOutStart);

  float flNextTime;
  if (m_nDissolveType == ENTITY_DISSOLVE_ELECTRICAL) {
    flNextTime = SimpleSplineRemapVal(dt, 0.0f, m_flFadeOutStart,
                                      2.0f * TICK_INTERVAL, 0.4f);
  } else {
  ...
  for (int i = 0; i < iTotalBeams; i++) {
    ...
    BuildTeslaEffect(pBox, *hitboxbones[pBox->bone], bRandom,
                     flActualYawOffset);
  }
}
...
float C_EntityDissolve::GetFadeInPercentage(void) {
  ...
  if (dt > m_flFadeOutStart) return 1.0f;

  if (dt < m_flFadeInStart) return 0.0f;
...
float C_EntityDissolve::GetFadeOutPercentage(void) {
  ...
  if (dt < m_flFadeInStart) return 1.0f;

  if (dt > m_flFadeOutStart) {
    dt -= m_flFadeOutStart;

    if (dt > m_flFadeOutLength) return 0.0f;
...
float C_EntityDissolve::GetModelFadeOutPercentage(void) {
  ...
  if (dt < m_flFadeOutModelStart) return 1.0f;

  if (dt > m_flFadeOutModelStart) {
    dt -= m_flFadeOutModelStart;

    if (dt > m_flFadeOutModelLength) return 0.0f;
...
void C_EntityDissolve::ClientThink(void) {
  C_BaseAnimating *pAnimating =
      GetMoveParent() ? GetMoveParent()->GetBaseAnimating() : NULL;
  if (!pAnimating) return;

  // NOTE: IsRagdoll means *client-side* ragdoll. We shouldn't be trying to
  // fight the server ragdoll (or any server physics) on the client
  ...
  if (GetFadeOutPercentage() <= 0.0f) {
    // Do NOT remove from the client entity list. It'll confuse the local
    // network backdoor, and the entity will never get destroyed because when
    // the server says to destroy it, the client won't be able to find it.
...
int C_EntityDissolve::DrawModel(int flags) {
  ...
  if (gpGlobals->frametime == 0 || m_bReadyToDraw == false) return 0;
  ...
  matrix3x4_t *hitboxbones[MAXSTUDIOBONES];
  if (pAnimating->HitboxToWorldTransforms(hitboxbones) == false) return 0;
  ...
  if ((fadePerc < 0.99f) &&
      ((m_nDissolveType == ENTITY_DISSOLVE_ELECTRICAL) ||
       (m_nDissolveType == ENTITY_DISSOLVE_ELECTRICAL_LIGHT))) {
  ...
  float spriteScale =
      ((gpGlobals->curtime - m_flStartTime) / m_flFadeOutLength);
  ...
  for (int i = 0; i < set->numhitboxes; ++i) {
    ...
    ComputeRenderInfo(pBox, *hitboxbones[pBox->bone], &vecAbsOrigin, &xvec,
                      &yvec);
    ...
    int numParticles = std::clamp(3.0f * fadePerc, 0.0f, 3.0f);
    ...
    for (int j = 0; j < iTempParts; j++) {
      // Skew the origin
      offset = xDir * Helper_RandomFloat(-xScale * 0.5f, xScale * 0.5f) +
               yDir * Helper_RandomFloat(-yScale * 0.5f, yScale * 0.5f);
      offset += vecSkew;

      if (random->RandomInt(0, 2) != 0) continue;

      sParticle = (SimpleParticle *)m_pEmitter->AddParticle(
          sizeof(SimpleParticle), g_Material_Spark, vecAbsOrigin + offset);

      if (sParticle == NULL) return 1;

      sParticle->m_vecVelocity = Vector(Helper_RandomFloat(-4.0f, 4.0f),
                                        Helper_RandomFloat(-4.0f, 4.0f),
                                        Helper_RandomFloat(16.0f, 64.0f));
    ...
    for (int j = 0; j < numParticles; j++) {
      ...
      sParticle = (SimpleParticle *)m_pEmitter->AddParticle(
          sizeof(SimpleParticle), g_Material_AR2Glow, vecAbsOrigin + offset);

      if (sParticle == NULL) return 1;

      sParticle->m_vecVelocity = Vector(Helper_RandomFloat(-4.0f, 4.0f),
                                        Helper_RandomFloat(-4.0f, 4.0f),
                                        Helper_RandomFloat(-64.0f, 128.0f));
