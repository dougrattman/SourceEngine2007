...
static ConVar net_graphtext("net_graphtext", "1", FCVAR_ARCHIVE,
                            "Draw text fields");
static ConVar net_graphmsecs(
    "net_graphmsecs", "400", FCVAR_ARCHIVE,
    "The latency graph represents this many milliseconds.");
static ConVar net_graphshowlatency("net_graphshowlatency", "1", FCVAR_ARCHIVE,
                                   "Draw the ping/packet loss graph.");
static ConVar net_graphshowinterp("net_graphshowinterp", "1", FCVAR_ARCHIVE,
                                  "Draw the interpolation graph.");

void NetgraphFontChangeCallback(IConVar *var, const char *pOldValue,
                                float flOldValue);

static ConVar net_graph("net_graph", "0", FCVAR_ARCHIVE,
                        "Draw the network usage graph, = 2 draws data on "
                        "payload, = 3 draws payload legend.",
                        NetgraphFontChangeCallback);
static ConVar net_graphheight("net_graphheight", "64", FCVAR_ARCHIVE,
                              "Height of netgraph panel",
                              NetgraphFontChangeCallback);
static ConVar net_graphproportionalfont(
    "net_graphproportionalfont", "1", FCVAR_ARCHIVE,
    "Determines whether netgraph font is proportional or not",
    NetgraphFontChangeCallback);

#define TIMINGS \
  1024  // Number of values to track (must be power of 2) b/c of masking
...
class CNetGraphPanel : public Panel {
 ...
 public:
  ...
  inline void DrawLine(vrect_t *rect, unsigned char *color,
                       unsigned char alpha);
  inline void DrawLine2(vrect_t *rect, unsigned char *color,
                        unsigned char *color2, unsigned char alpha,
                        unsigned char alpha2);

  void ResetLineSegments();
  void DrawLineSegments();

  int DrawDataSegment(vrect_t *rcFill, int bytes, byte r, byte g, byte b,
                      byte alpha = 255);
  ...
  void DrawTimes(vrect_t vrect, cmdinfo_t *cmdinfo, int x, int w,
                 int graphtype);
  void DrawTextFields(int graphvalue, int x, int y, int w,
                      netbandwidthgraph_t *graph, cmdinfo_t *cmdinfo);
  void GraphGetXY(vrect_t *rect, int width, int *x, int *y);
  void GetCommandInfo(INetChannelInfo *netchannel, cmdinfo_t *cmdinfo);
  void GetFrameData(INetChannelInfo *netchannel, int *biggest_message,
                    float *avg_message, float *f95thpercentile);
  void ColorForHeight(packet_latency_t *packet, byte *color, int *ping,
                      byte *alpha);
 ...
 private:
  ...
  void DrawLargePacketSizes(int x, int w, int graphtype,
...
void CNetGraphPanel::OnFontChanged() {
  ...
  int textTall;
  g_pMatSystemSurface->GetTextSize(m_hFontProportional, ustr, m_EstimatedWidth,
                                   textTall);
...
void CNetGraphPanel::ComputeNetgraphHeight() {
  ...
  m_nNetGraphHeight = std::max(lines * tall, m_nNetGraphHeight);
}
...
//			*alpha -
//-----------------------------------------------------------------------------
void CNetGraphPanel::ColorForHeight(packet_latency_t *packet, byte *color,
                                    int *ping, byte *alpha) {
...
void CNetGraphPanel::DrawTimes(vrect_t vrect, cmdinfo_t *cmdinfo, int x, int w,
                               int graphtype) {
  if (!net_graphshowinterp.GetBool() || graphtype <= 1) return;
  ...
  for (a = 0; a < w; a++) {
    ...
    h = std::min((cmdinfo[i].cmd_lerp / 3.0f) * LERP_HEIGHT,
                 LERP_HEIGHT * 1.0f);
...
//-----------------------------------------------------------------------------
// Purpose: Compute frame database for rendering m_NetChannel computes choked,
// and lost packets, too.
...
//			1 -
//-----------------------------------------------------------------------------
void CNetGraphPanel::GetFrameData(INetChannelInfo *netchannel,
                                  int *biggest_message, float *avg_message,
  ...
  *f95thpercentile = 0.0f;
  ...
  for (int seqnr = m_IncomingSequence - m_UpdateWindowSize + 1;
       seqnr <= m_IncomingSequence; seqnr++) {
    ...
    netchannel->GetPacketResponseLatency(FLOW_INCOMING, seqnr, &lat->latency,
                                         &lat->choked);

    if (lat->latency < 9995) {
      lat->latency += flAdjust;
      lat->latency = std::max(lat->latency, 0);
    }
  ...
  if (msg_count >= 1) {
    *avg_message /= msg_count;
    ...
    for (int seqnr = m_IncomingSequence - m_UpdateWindowSize + 1;
         seqnr <= m_IncomingSequence; seqnr++) {
      int bytes =
          m_Graph[seqnr & (TIMINGS - 1)].msgbytes[INetChannelInfo::TOTAL] -
          (*avg_message);
...
// Input  : *cmdinfo -
//-----------------------------------------------------------------------------
void CNetGraphPanel::GetCommandInfo(INetChannelInfo *netchannel,
                                    cmdinfo_t *cmdinfo) {
  for (int seqnr = m_OutgoingSequence - m_UpdateWindowSize + 1;
       seqnr <= m_OutgoingSequence; seqnr++) {
    // Also set up the lerp point.
    cmdinfo_t *ci = &cmdinfo[seqnr & (TIMINGS - 1)];

    ci->cmd_lerp =
        netchannel->GetCommandInterpolationAmount(FLOW_OUTGOING, seqnr);
    ci->sent = netchannel->IsValidPacket(FLOW_OUTGOING, seqnr);
    ci->size = netchannel->GetPacketBytes(FLOW_OUTGOING, seqnr,
                                          INetChannelInfo::TOTAL);
  }
}

//-----------------------------------------------------------------------------
// Purpose: Draws overlay text fields showing framerate, latency, bandwidth
// breakdowns,
...
//			avg -
//-----------------------------------------------------------------------------
void CNetGraphPanel::DrawTextFields(int graphvalue, int x, int y, int w,
                                    netbandwidthgraph_t *graph,
                                    cmdinfo_t *cmdinfo) {
  if (!net_graphtext.GetBool()) return;
  ...
  m_Framerate = FRAMERATE_AVG_FRAC * m_Framerate +
                (1.0 - FRAMERATE_AVG_FRAC) * gpGlobals->absoluteframetime;
  ...
  Q_snprintf(sz, sizeof(sz), "fps:%4i   ping: %i ms", (int)(1.0f / m_Framerate),
             (int)(m_AvgLatency * 1000.0f));

  g_pMatSystemSurface->DrawColoredText(font, x, y, GRAPH_RED, GRAPH_GREEN,
                                       GRAPH_BLUE, 255, sz);
  ...
  int totalsize = graph[(m_IncomingSequence & (TIMINGS - 1))]
                      .msgbytes[INetChannelInfo::TOTAL];
  ...
  Q_snprintf(sz, sizeof(sz), "lerp: %5.1f ms",
             GetClientInterpAmount() * 1000.0f);
  ...
  g_pMatSystemSurface->DrawColoredText(font, x + textWidth, y, interpcolor[0],
                                       interpcolor[1], interpcolor[2], 255, sz);

  Q_snprintf(sz, sizeof(sz), "%3.1f/s", m_AvgPacketIn);
  textWidth = g_pMatSystemSurface->DrawTextLen(font, "%s", sz);

  g_pMatSystemSurface->DrawColoredText(font, x + w - textWidth - 1, y,
                                       GRAPH_RED, GRAPH_GREEN, GRAPH_BLUE, 255,
                                       sz);
  ...
  if (graphvalue > 2) {
    Q_snprintf(sz, sizeof(sz), "loss:%3i    choke: %2i ",
               (int)(m_AvgPacketLoss * 100.0f),
               (int)(m_AvgPacketChoke * 100.0f));
    ...
    if (graphvalue > 3) {
      Q_snprintf(sz, sizeof(sz), "sv  : %5.1f   var: %4.2f msec",
                 m_flServerFramerate,
      ...
      g_pMatSystemSurface->DrawColoredText(
          font, x, y, servercolor[0], servercolor[1], servercolor[2], 255, sz);
  ...
  if (graphvalue >= 3) {
    ...
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 0, 0, 255, 255,
                                         "localplayer");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 0, 255, 0, 255,
                                         "otherplayers");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 255, 0, 0, 255,
                                         "entities");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 255, 255, 0, 255,
                                         "sounds");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 0, 255, 255, 255,
                                         "events");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 128, 128, 0, 255,
                                         "usermessages");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 0, 128, 128, 255,
                                         "entmessages");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 128, 0, 0, 255,
                                         "stringcmds");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 0, 128, 0, 255,
                                         "stringtables");
    y -= textTall;
    g_pMatSystemSurface->DrawColoredText(m_hFontSmall, x, y, 0, 0, 128, 255,
                                         "voice");
    y -= textTall;
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Determine type of graph to show, or if +graph key is held down, use
// detailed graph Output : int
//-----------------------------------------------------------------------------
int CNetGraphPanel::GraphValue(void) {
  ...
  if (!graphtype && !(in_graph.state & 1)) return 0;
  ...
  switch (net_graphpos.GetInt()) {
    ...
    case 2:
      *x = rect->x + (rect->width - 10 - width) / 2;
      break;
    default:
      *x = rect->x + std::clamp((int)XRES(net_graphpos.GetInt()), 5,
                                rect->width - width - 5);
  }
...
//-----------------------------------------------------------------------------
// Purpose: drawing stream progess (file download etc) as green bars ( under
// in/out) Input  : x -
//			y -
//			maxmsgbytes -
//-----------------------------------------------------------------------------
...
void CNetGraphPanel::DrawStreamProgress(int x, int y, int width) {
  ...
  if (m_StreamTotal[FLOW_INCOMING] > 0) {
    ...
    rcLine.width =
        (m_StreamRecv[FLOW_INCOMING] * width) / m_StreamTotal[FLOW_INCOMING];
  ...
  if (m_StreamTotal[FLOW_OUTGOING] > 0) {
    ...
    rcLine.width =
        (m_StreamRecv[FLOW_OUTGOING] * width) / m_StreamTotal[FLOW_OUTGOING];
    DrawLine(&rcLine, color, 255);
  }
}

//-----------------------------------------------------------------------------
// Purpose: If showing bandwidth data, draw hatches big enough for largest
// message Input  : x -
//			y -
//			maxmsgbytes -
//-----------------------------------------------------------------------------
void CNetGraphPanel::DrawHatches(int x, int y, int maxmsgbytes) {
  ...
  for (starty = rcHatch.y;
       rcHatch.y > 0 &&
       ((starty - rcHatch.y) * net_scale.GetFloat() < (maxmsgbytes + 50));
       rcHatch.y -= ystep) {
...
void CNetGraphPanel::DrawUpdateRate(int xright, int y) {
  ...
  g_pMatSystemSurface->GetTextSize(GetNetgraphFont(), unicode, textWide,
                                   textTall);

  g_pMatSystemSurface->DrawColoredText(GetNetgraphFont(), xright - textWide - 1,
                                       y, GRAPH_RED, GRAPH_GREEN, GRAPH_BLUE,
                                       255, "%s", sz);
}
...
// Output : int
//-----------------------------------------------------------------------------
int CNetGraphPanel::DrawDataSegment(vrect_t *rcFill, int bytes, byte r, byte g,
                                    byte b, byte alpha) {
  int h;
  byte color[3];
...
// Purpose:
//-----------------------------------------------------------------------------
void CNetGraphPanel::OnTick(void) { SetVisible(ShouldDraw()); }

bool CNetGraphPanel::ShouldDraw(void) {
  if (GraphValue() != 0) return true;

  return false;
}

void CNetGraphPanel::DrawLargePacketSizes(int x, int w, int graphtype,
                                          float warning_threshold) {
  ...
  for (a = 0; a < w; a++) {
    ...
    if (warning_threshold != 0.0f &&
        nTotalBytes > std::max(300.0f, warning_threshold)) {
      char sz[32];
      ...
      texty = std::max(0, rcFill.y - 11);

      g_pMatSystemSurface->DrawColoredText(m_hFont, textx, texty, 255, 255, 255,
                                           255, sz);
    }
  }
}
...
void CNetGraphPanel::Paint() {
  ...
  w = std::min((int)TIMINGS, m_EstimatedWidth);
  if (vrect.width < w + 10) {
    w = vrect.width - 10;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CNetGraphPanel::PaintLineArt(int x, int y, int w, int graphtype,
                                  int maxmsgbytes) {
  ...
  for (int a = 0; a < w; a++) {
    ...
    if (!DrawDataSegment(&rcFill,
                         m_Graph[i].msgbytes[INetChannelInfo::LOCALPLAYER], 0,
                         0, 255))
      continue;

    if (!DrawDataSegment(&rcFill,
                         m_Graph[i].msgbytes[INetChannelInfo::OTHERPLAYERS], 0,
                         255, 0))
      continue;

    if (!DrawDataSegment(
            &rcFill, m_Graph[i].msgbytes[INetChannelInfo::ENTITIES], 255, 0, 0))
      continue;

    if (!DrawDataSegment(&rcFill, m_Graph[i].msgbytes[INetChannelInfo::SOUNDS],
                         255, 255, 0))
      continue;

    if (!DrawDataSegment(&rcFill, m_Graph[i].msgbytes[INetChannelInfo::EVENTS],
                         0, 255, 255))
      continue;

    if (!DrawDataSegment(&rcFill,
                         m_Graph[i].msgbytes[INetChannelInfo::USERMESSAGES],
                         128, 128, 0))
      continue;

    if (!DrawDataSegment(&rcFill,
                         m_Graph[i].msgbytes[INetChannelInfo::ENTMESSAGES], 0,
                         128, 128))
      continue;

    if (!DrawDataSegment(&rcFill,
                         m_Graph[i].msgbytes[INetChannelInfo::STRINGCMD], 128,
                         0, 0))
      continue;

    if (!DrawDataSegment(&rcFill,
                         m_Graph[i].msgbytes[INetChannelInfo::STRINGTABLE], 0,
                         128, 0))
      continue;

    if (!DrawDataSegment(&rcFill, m_Graph[i].msgbytes[INetChannelInfo::VOICE],
                         0, 0, 128))
      continue;
...
void CNetGraphPanel::DrawLineSegments() {
  ...
  IMesh *m_pMesh =
      pRenderContext->GetDynamicMesh(true, NULL, NULL, m_WhiteMaterial);
...
//			alpha -
//-----------------------------------------------------------------------------
void CNetGraphPanel::DrawLine(vrect_t *rect, unsigned char *color,
                              unsigned char alpha) {
...
//			alpha -
//-----------------------------------------------------------------------------
void CNetGraphPanel::DrawLine2(vrect_t *rect, unsigned char *color,
                               unsigned char *color2, unsigned char alpha,
                               unsigned char alpha2) {
...
class CNetGraphPanelInterface : public INetGraphPanel {
 ...
 public:
  CNetGraphPanelInterface(void) { netGraphPanel = NULL; }
  void Create(VPANEL parent) { netGraphPanel = new CNetGraphPanel(parent); }
  void Destroy(void) {
    if (netGraphPanel) {
