...
class C_BasePlayer : public C_BaseCombatCharacter {
 public:
  ...
  virtual void CalcViewModelView(const Vector &eyeOrigin,
  ...
  // This can be overridden to return something other than m_pRagdoll if the mod
  // uses separate entities for ragdolls.
  ...
  // Returns the view model if this is the local player. If you're in third
  // person or this is a remote player, it returns the active weapon (and its
  // appropriate left/right weapon if this is TF2).
  ...
  virtual bool Weapon_ShouldSetLast(C_BaseCombatWeapon *pOldWeapon,
  ...
  bool SetFOV(CBaseEntity *pRequester, int FOV, float zoomRate,
              int iZoomStart = 0);
  ...
  // Called when not in tactical mode. Allows view to be overriden for things
  // like driving a tank.
  ...
  // Get the command number associated with the current usercmd we're running
  // (if in predicted code).
  ...
  virtual void UpdateStepSound(surfacedata_t *psurface, const Vector &vecOrigin,
                               const Vector &vecVelocity);
  virtual void PlayStepSound(Vector &vecOrigin, surfacedata_t *psurface,
                             float fvol, bool force);
  virtual surfacedata_t *GetFootstepSurface(const Vector &origin,
                                            const char *surfaceName);
  virtual void GetStepSoundVelocities(float *velwalk, float *velrun);
  virtual void SetStepSoundTime(stepsoundtimes_t iStepSoundTime, bool bWalking);

  // Called by prediction when it detects a prediction correction.
  // vDelta is the line from where the client had predicted the player to at the
  // usercmd in question, to where the server says the client should be at said
  // usercmd.
 ...
 public:
  ...
  int m_iFOVStart;       // starting value of the FOV changing over time (client
                         // only)
  float m_flFOVTime;     // starting time of the FOV zoom
  int m_iDefaultFOV;     // default FOV if no other zooms are occurring
  EHANDLE m_hZoomOwner;  // This is a pointer to the entity currently
                         // controlling the player's zoom
 ...
 protected:
  ...
  void CalcVehicleView(IClientVehicle *pVehicle, Vector &eyeOrigin,
                       QAngle &eyeAngles, float &zNear, float &zFar,
                       float &fov);
  virtual void CalcObserverView(Vector &eyeOrigin, QAngle &eyeAngles,
                                float &fov);
  virtual Vector GetChaseCamViewOffset(CBaseEntity *target);
  void CalcChaseCamView(Vector &eyeOrigin, QAngle &eyeAngles, float &fov);
  void CalcInEyeCamView(Vector &eyeOrigin, QAngle &eyeAngles, float &fov);
  virtual void CalcDeathCamView(Vector &eyeOrigin, QAngle &eyeAngles,
                                float &fov);
 ...
 protected:
  ...
  float m_flFreezeFrameStartTime;  // Time at which we entered freeze frame
                                   // observer mode
 ...
 protected:
  ...
  void ForceSetupBonesAtTimeFakeInterpolation(matrix3x4_t *pBonesOut,
                                              float curtimeOffset);

  float m_flLaggedMovementValue;

  // These are used to smooth out prediction corrections. They're most useful
  // when colliding with vphysics objects. The server will be sending constant
  // prediction corrections, and these can help the errors not be so jerky.
  Vector m_vecPredictionError;
  float m_flPredictionErrorTime;

  Vector
      m_vecPreviouslyPredictedOrigin;  // Used to determine if non-gamemovement
                                       // game code has teleported, or tweaked
                                       // the player's origin
