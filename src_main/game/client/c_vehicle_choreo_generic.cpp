...
IMPLEMENT_CLIENTCLASS_DT(C_PropVehicleChoreoGeneric,
                         DT_PropVehicleChoreoGeneric, CPropVehicleChoreoGeneric)
  RecvPropEHandle(RECVINFO(m_hPlayer)), RecvPropBool(RECVINFO(m_bEnterAnimOn)),
...
void C_PropVehicleChoreoGeneric::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  if (!m_hPlayer && m_hPrevPlayer) {
    ...
    // There's no need to "smooth" the view when leaving the vehicle so just set
    // this here so the stair code doesn't get confused.
...
C_BaseCombatCharacter *C_PropVehicleChoreoGeneric::GetPassenger(int nRole) {
  if (nRole == VEHICLE_ROLE_DRIVER) return m_hPlayer.Get();
...
//-----------------------------------------------------------------------------
int C_PropVehicleChoreoGeneric::GetPassengerRole(
    C_BaseCombatCharacter *pPassenger) {
  if (m_hPlayer.Get() == pPassenger) return VEHICLE_ROLE_DRIVER;
...
//-----------------------------------------------------------------------------
void C_PropVehicleChoreoGeneric::GetVehicleViewPosition(int nRole,
                                                        Vector *pAbsOrigin,
                                                        QAngle *pAbsAngles,
                                                        float *pFOV /*=NULL*/) {
  SharedVehicleViewSmoothing(m_hPlayer, pAbsOrigin, pAbsAngles, m_bEnterAnimOn,
                             m_bExitAnimOn, m_vecEyeExitEndpoint,
                             &m_ViewSmoothingData, pFOV);
}
...
//			pCmd -
//-----------------------------------------------------------------------------
void C_PropVehicleChoreoGeneric::UpdateViewAngles(C_BasePlayer *pLocalPlayer,
                                                  CUserCmd *pCmd) {
  ...
  flAngleDiff =
      std::clamp(flAngleDiff, static_cast<float>(m_vehicleView.flYawMin),
                 static_cast<float>(m_vehicleView.flYawMax));
  ...
  flAngleDiff =
      std::clamp(flAngleDiff, static_cast<float>(m_vehicleView.flPitchMin),
                 static_cast<float>(m_vehicleView.flPitchMax));
...
//-----------------------------------------------------------------------------
void C_PropVehicleChoreoGeneric::GetVehicleClipPlanes(float &flZNear,
                                                      float &flZFar) const {
