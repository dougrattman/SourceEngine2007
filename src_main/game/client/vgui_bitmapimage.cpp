...
bool BitmapImage::Init(vgui::VPANEL pParent, KeyValues* pInitData) {
  ...
  if (!pMaterialName || !pMaterialName[0]) return false;

  // modulation color
  Color color;
  if (!ParseRGBA(pInitData, "color", color)) color.SetColor(255, 255, 255, 255);
...
vgui::VPANEL BitmapImage::GetRenderSizePanel(void) { return m_pPanelSize; }
...
void BitmapImage::DoPaint(int x, int y, int wide, int tall, float yaw,
  ...
  if (yaw == 0) {
    if (!m_bUseViewport) {
      ...
      vgui::surface()->DrawTexturedSubRect(x, y, x + wide, y + tall,
                                           m_rgViewport[0], m_rgViewport[1],
                                           m_rgViewport[2], m_rgViewport[3]);
    }
  } else {
...
void BitmapImage::DoPaint(vgui::VPANEL pPanel, float yaw,
                          float flAlphaModulate) {
  int wide, tall;
  if (pPanel) {
...
void BitmapImage::Paint() { DoPaint(m_pPanelSize); }

void BitmapImage::SetColor(Color& clr) { m_clr = clr; }

Color BitmapImage::GetColor() { return m_clr; }

void BitmapImage::GetColor(uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) {
  m_clr.GetColor(r, g, b, a);
}
...
bool InitializeImage(KeyValues* pInitData, const char* pSectionName,
  ...
  if (pSectionName) {
    ...
    if (!pBitmapImageSection) return false;
  } else {
...
//			bottom -
//-----------------------------------------------------------------------------
void BitmapImage::SetViewport(bool use, float left, float top, float right,
                              float bottom) {
  m_bUseViewport = use;
  m_rgViewport[0] = left;
  m_rgViewport[1] = top;
