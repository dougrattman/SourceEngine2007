...
void DrawSpriteModel(IClientEntity *baseentity, CEngineSprite *psprite,
                     ...
                     const Vector &up, float flHDRColorScale = 1.0f);
float StandardGlowBlend(const pixelvis_queryparams_t &params,
                        pixelvis_handle_t *queryHandle, int rendermode,
                        int renderfx, int alpha, float *pscale);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CClientTools, IClientTools,
...
bool CClientTools::IsInRecordingMode() const { return m_bInRecordingMode; }
...
void CClientTools::AddClientRenderable(IClientRenderable *pRenderable,
                                       int renderGroup) {
  Assert(pRenderable);
  ...
  if (INVALID_CLIENT_RENDER_HANDLE == handle) {
    ...
    ClientLeafSystem()->SetRenderGroup(pRenderable->RenderHandle(),
...
void CClientTools::RemoveClientRenderable(IClientRenderable *pRenderable) {
  ...
  cl_entitylist->RemoveEntity(
      pRenderable->GetIClientUnknown()->GetRefEHandle());
}
...
void CClientTools::SetRenderGroup(IClientRenderable *pRenderable,
                                  int renderGroup) {
  ClientRenderHandle_t handle = pRenderable->RenderHandle();
  if (INVALID_CLIENT_RENDER_HANDLE == handle) {
    // create new renderer handle
...
bool CClientTools::DrawSprite(IClientRenderable *pRenderable, float scale,
                              float frame, int rendermode, int renderfx,
                              const Color &color, float flProxyRadius,
                              int *pVisHandle) {
  ...
  CEngineSprite *psprite =
      (CEngineSprite *)modelinfo->GetModelExtraData(pRenderable->GetModel());
  if (!psprite) return false;

  // Get orthonormal bases for current view - re-align to current camera (vs.
  // recorded camera)
  Vector forward, right, up;
  C_SpriteRenderer::GetSpriteAxes(
      (C_SpriteRenderer::SPRITETYPE)psprite->GetOrientation(), origin, angles,
      forward, right, up);
  ...
  if (rendermode != kRenderNormal) {
    ...
    if ((rendermode == kRenderGlow) || (rendermode == kRenderWorldGlow)) {
      ...
      float blend =
          oldBlend * StandardGlowBlend(params, (pixelvis_handle_t *)pVisHandle,
                                       rendermode, renderfx, color.a(), &scale);
  ...
  DrawSpriteModel((IClientEntity *)pRenderable, psprite, origin, scale, frame,
                  rendermode, r, g, b, color.a(), forward, right, up);
...
HTOOLHANDLE CClientTools::AttachToEntity(EntitySearchResult entityToAttach) {
  C_BaseEntity *ent = reinterpret_cast<C_BaseEntity *>(entityToAttach);
  Assert(ent);
  if (!ent) return (HTOOLHANDLE)0;

  HTOOLHANDLE curHandle = ent->GetToolHandle();
  if (curHandle != 0) return curHandle;  // Already attaached
...
C_BaseEntity *CClientTools::LookupEntity(HTOOLHANDLE handle) {
  int idx = m_Handles.Find(HToolEntry_t(handle));
  if (idx == m_Handles.InvalidIndex()) return NULL;

  return m_Handles[idx].m_hEntity;
}

int CClientTools::GetNumRecordables() { return m_ActiveHandles.Count(); }
...
void CClientTools::SetEnabled(HTOOLHANDLE handle, bool enabled) {
  int idx = m_Handles.Find(HToolEntry_t(handle));
  if (idx == m_Handles.InvalidIndex()) return;
...
bool CClientTools::ShouldRecord(HTOOLHANDLE handle) {
  int idx = m_Handles.Find(HToolEntry_t(handle));
  if (idx == m_Handles.InvalidIndex()) return false;
...
void CClientTools::OnEntityDeleted(CBaseEntity *pEntity) {
  ...
  if (handle == (HTOOLHANDLE)0) return;
...
bool CClientTools::GetLocalPlayerEyePosition(Vector &org, QAngle &ang,
                                             float &fov) {
...
ClientShadowHandle_t CClientTools::CreateFlashlight(
    const FlashlightState_t &lightState) {
...
void CClientTools::UpdateFlashlightState(ClientShadowHandle_t h,
                                         const FlashlightState_t &lightState) {
...
Vector CClientTools::GetAbsOrigin(HTOOLHANDLE handle) {
  int idx = m_Handles.Find(HToolEntry_t(handle));
  if (idx == m_Handles.InvalidIndex()) return vec3_origin;
...
QAngle CClientTools::GetAbsAngles(HTOOLHANDLE handle) {
  int idx = m_Handles.Find(HToolEntry_t(handle));
  if (idx == m_Handles.InvalidIndex()) return vec3_angle;
...
void CClientTools::PostToolMessage(KeyValues *pKeyValues) {
  if (!Q_stricmp(pKeyValues->GetName(), "QueryParticleManifest")) {
    // NOTE: This cannot be done during particle system init because tools
    // aren't set up at that point
  ...
  if (!Q_stricmp(pKeyValues->GetName(), "query CPortalRenderer")) {
    pKeyValues->SetInt("IsRenderingPortal",
                       g_pPortalRender->IsRenderingPortal() ? 1 : 0);
    return;
  }
#endif
}
...
//-----------------------------------------------------------------------------
void CClientTools::ReloadParticleDefintions(const char *pFileName,
                                            const void *pBufData, int nLen) {
  // Remove all new effects, because we are going to free internal structures
  // they point to
