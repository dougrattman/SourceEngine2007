...
//-----------------------------------------------------------------------------
bool ParseRGBA(KeyValues* pValues, const char* pFieldName, uint8_t& r,
               uint8_t& g, uint8_t& b, uint8_t& a) {
  r = g = b = a = 255;
  const char* pColorString = pValues->GetString(pFieldName, "255 255 255 255");
  if (!pColorString || !pColorString[0]) return false;
...
bool ParseRGBA(KeyValues* pValues, const char* pFieldName, Color& c) {
  uint8_t r, g, b, a;
  if (!ParseRGBA(pValues, pFieldName, r, g, b, a)) return false;
...
bool ParseCoord(KeyValues* pValues, const char* pFieldName, int& x, int& y) {
  ...
  if (!pCoordString || !pCoordString[0]) return false;
...
bool ParseRect(KeyValues* pValues, const char* pFieldName, int& x, int& y,
               int& w, int& h) {
  x = y = w = h = 0;
  const char* pRectString = pValues->GetString(pFieldName, "0 0 0 0");
  if (!pRectString || !pRectString[0]) return false;
...
//-----------------------------------------------------------------------------
// Helper class to make meta class panels (for use in entities, so they
// autocleanup)
//-----------------------------------------------------------------------------
...
void CPanelWrapper::Activate(char const* pMetaClassName, vgui::Panel* pParent,
                             int sortorder, void* pVoidInitData) {
  if (m_pPanel) {
    Deactivate();
  }

  m_pPanel = PanelMetaClassMgr()->CreatePanelMetaClass(
      pMetaClassName, sortorder, pVoidInitData, pParent);
}
...
class CPanelMetaClassMgrImp : public IPanelMetaClassMgr {
 ...
 private:
  ...
  bool ParseMetaClassList(const char* pFileName, KeyValues* pKeyValues,
                          int keyValueIndex);
...
IPanelMetaClassMgr* PanelMetaClassMgr() {
  ...
  // even though it means an extra check every time PanelMetaClassMgr is
  // accessed
...
//-----------------------------------------------------------------------------
void CPanelMetaClassMgrImp::InstallPanelType(const char* pPanelName,
                                             IPanelFactory* pFactory) {
...
bool CPanelMetaClassMgrImp::ParseMetaClassList(const char* pFileName,
  ...
  while (pIter) {
    if (!ParseSingleMetaClass(pFileName, pIter->GetName(), pIter,
                              keyValueIdx)) {
      //	return false;
...
//-----------------------------------------------------------------------------
static void KeyValueChainRecursive(KeyValues* pKeyValues,
...
//-----------------------------------------------------------------------------
vgui::Panel* CPanelMetaClassMgrImp::CreatePanelMetaClass(
    const char* pMetaClassName, int sortorder, void* pInitData,
    vgui::Panel* pParent, const char* pChainName) {
  // Search for the metaclass name
  int i = m_MetaClassDict.Find(pMetaClassName);
  if (i == m_MetaClassDict.InvalidIndex()) return NULL;
  ...
  vgui::Panel* pPanel = pFactory->Create(pMetaClassName, metaClass.m_pKeyValues,
                                         pInitData, pParent);
  if (pPanel) {
