...
void ClearKeyValuesCache() {
  MEM_ALLOC_CREDIT();
  for (int i = g_KeyValuesCache.First(); i != g_KeyValuesCache.InvalidIndex();
       i = g_KeyValuesCache.Next(i)) {
...
//-----------------------------------------------------------------------------
void C_VGuiScreen::GetAimEntOrigin(IClientEntity *pAttachedTo, Vector *pOrigin,
                                   QAngle *pAngles) {
...
void C_VGuiScreen::DestroyVguiScreen() { m_PanelWrapper.Deactivate(); }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool C_VGuiScreen::AcceptsInput() const { return m_bAcceptsInput; }
...
RenderGroup_t C_VGuiScreen::GetRenderGroup() {
  if (IsAttachedToViewModel()) return RENDER_GROUP_VIEW_MODEL_TRANSLUCENT;
...
bool C_VGuiScreen::IsVisibleToTeam(int nTeam) {
  ...
  if (IsVisibleOnlyToTeammates() && (nTeam > 0)) {
    ...
    if (pOwner && (nTeam != pOwner->GetTeamNumber())) return false;
  }
...
void ScreenToWorld(int mousex, int mousey, float fov,
  ...
  float scaled_fov =
      ScaleFOVByWidthRatio(fov, engine->GetScreenAspectRatio() * 0.75f);
...
void C_VGuiScreen::ClientThink(void) {
  ...
  m_nButtonPressed =
      nButtonsChanged &
      m_nButtonState;  // The changed ones still down are "pressed"
  m_nButtonReleased =
      nButtonsChanged & (~m_nButtonState);  // The ones not down are "released"

  BaseClass::ClientThink();

  // TODO(d.rattman): We should really be taking bob, shake, and roll into
  // account but if we did, then all the inputs would be generated multiple
  // times if the world was rendered multiple times (for things like water,
  // etc.)
  ...
  if (!IntersectWithRay(lookDir, &u, &v, NULL)) return;
  ...
  if ((m_nButtonReleased & IN_ATTACK) ||
      m_bLoseThinkNextFrame)  // for a button release on loosing focus
  {
...
//-----------------------------------------------------------------------------
void C_VGuiScreen::ComputeEdges(Vector *pUpperLeft, Vector *pUpperRight,
                                Vector *pLowerLeft) {
  Vector vecOrigin = GetAbsOrigin();
  Vector xaxis, yaxis;
  AngleVectors(GetAbsAngles(), &xaxis, &yaxis, NULL);

  // NOTE: Have to multiply by -1 here because yaxis goes out the -y axis in
  // AngleVectors actually...
  yaxis *= -1.0f;
...
//-----------------------------------------------------------------------------
bool C_VGuiScreen::IntersectWithRay(const Ray_t &ray, float *u, float *v,
                                    float *t) {
  ...
  return ComputeIntersectionBarycentricCoordinates(ray, origin, upt, vpt, *u,
                                                   *v, t);
}
...
void C_VGuiScreen::DrawScreenOverlay() {
  ...
  IMesh *pMesh =
      pRenderContext->GetDynamicMesh(true, NULL, NULL, m_OverlayMaterial);
...
int C_VGuiScreen::DrawModel(int flags) {
  ...
  if (IsBackfacing(CurrentViewOrigin())) return 0;

  // Recompute the panel-to-world center
  // TODO(d.rattman): Can this be cached off?
  ComputePanelToWorld();
...
bool C_VGuiScreen::ShouldDraw(void) { return !IsEffectActive(EF_NODRAW); }
...
//-----------------------------------------------------------------------------
// Purpose: Sometimes we only want a specific player to be able to input to a
// panel
//-----------------------------------------------------------------------------
...
IterationRetval_t CVGuiScreenEnumerator::EnumElement(
...
C_BaseEntity *FindNearbyVguiScreen(const Vector &viewPosition,
                                   const QAngle &viewAngle, int nTeam) {
  if (IsX360()) {
  ...
  partition->EnumerateElementsInSphere(PARTITION_CLIENT_NON_STATIC_EDICTS,
                                       viewPosition, VGUI_SCREEN_MODE_RADIUS,
                                       false, &localScreens);
  ...
  for (int i = localScreens.GetScreenCount(); --i >= 0;) {
    ...
    if (pScreen->IsBackfacing(viewPosition)) continue;
    ...
    // TODO(d.rattman): Should this maybe go into a derived class of some sort?
    // Don't bother with screens on the wrong team
    if (!pScreen->IsVisibleToTeam(nTeam)) continue;

    if (!pScreen->AcceptsInput()) continue;

    if (pScreen->IsInputOnlyToOwner() &&
        pScreen->GetPlayerOwner() != pLocalPlayer)
      continue;
    ...
    if ((flPerpDist < 0) || (flPerpDist > VGUI_SCREEN_MODE_RADIUS)) continue;

    // Perform a raycast to see where in barycentric coordinates the ray hits
    // the viewscreen; if it doesn't hit it, you're not in the mode
    float u, v, t;
    if (!pScreen->IntersectWithRay(lookRay, &u, &v, &t)) continue;
...
bool CVGuiScreenPanel::Init(KeyValues *pKeyValues,
