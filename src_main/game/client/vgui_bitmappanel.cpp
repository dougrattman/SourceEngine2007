...
bool CBitmapPanel::Init(KeyValues *pInitData) {
  ...
  int x, y, w, h;
  if (!ParseRect(pInitData, "position", x, y, w, h)) return false;
...
void CBitmapPanel::ApplySettings(KeyValues *pInitData) {
  ...
  // modulation color. Can't use ParseRGBA since this uses a vgui::KeyValues
  // (feh)
  m_r = m_g = m_b = m_a = 255;
...
// Output : char const
//-----------------------------------------------------------------------------
const char *CBitmapPanel::GetMouseOverText(void) { return m_szMouseOverText; }

//-----------------------------------------------------------------------------
// Purpose: Sets up the panel
//			Used by panels that aren't created by the commander
//overlay  factory (i.e. aren't parsed from a keyvalues file)
//-----------------------------------------------------------------------------
