...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CHudDamageTiles::CHudDamageTiles(const char *pElementName)
    : CHudElement(pElementName), BaseClass(NULL, "HudDamageTiles") {
...
void CHudDamageTiles::Paint(void) {
  ...
  for (i = 0; i < NUM_DMG_TYPES; i++) {
    ...
    if (m_bitsDamage & pDmgTile->fFlags) {
      pDmgTile->flExpire =
          std::min(gpGlobals->curtime + DMG_IMAGE_LIFE, pDmgTile->flExpire);
          ...
          && a < 40)  // and the flash is at the low point of the cycle
      {
        ...
        for (int j = 0; j < NUM_DMG_TYPES; j++) {
          ...
          if ((pDmgTileIter->y) && (pDmgTileIter->y < y) &&
              (pDmgTileIter->icon))
...
void CHudDamageTiles::UpdateTiles(long bitsDamage) {
  ...
  for (int i = 0; i < NUM_DMG_TYPES; i++) {
    ...
    if (m_bitsDamage & pDmgTile->fFlags) {
      pDmgTile->flExpire =
          gpGlobals->curtime + DMG_IMAGE_LIFE;  // extend the duration
  ...
  // damage bits are only turned on here;  they are turned off when the draw
  // time has expired (in Paint())
