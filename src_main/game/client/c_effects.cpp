...
ConVar cl_windspeed("cl_windspeed", "0", FCVAR_CHEAT,
...
ConVar r_RainSideVel("r_RainSideVel", "130", FCVAR_CHEAT,
                     "How much sideways velocity rain gets.");

ConVar r_RainSimulate("r_RainSimulate", "1", FCVAR_CHEAT,
                      "Enable/disable rain simulation.");
ConVar r_DrawRain("r_DrawRain", "1", FCVAR_CHEAT,
                  "Enable/disable rain rendering.");
ConVar r_RainProfile("r_RainProfile", "0", FCVAR_CHEAT,
...
class CClient_Precipitation : public C_BaseEntity {
 ...
 private:
  CClient_Precipitation(
      const CClient_Precipitation &);  // not defined, not accessible
};

// Just receive the normal data table stuff
IMPLEMENT_CLIENTCLASS_DT(CClient_Precipitation, DT_Precipitation,
                         CPrecipitation)
RecvPropInt(RECVINFO(m_nPrecipType)) END_RECV_TABLE()
...
static ConVar r_SnowInsideRadius("r_SnowInsideRadius", "256", FCVAR_CHEAT,
                                 "Snow.");
static ConVar r_SnowOutsideRadius("r_SnowOutsideRadius", "1024", FCVAR_CHEAT,
                                  "Snow.");
...
static ConVar r_SnowFallSpeed("r_SnowFallSpeed", "1.5", FCVAR_CHEAT,
                              "Snow fall speed scale.");
static ConVar r_SnowWindScale("r_SnowWindScale", "0.0035", FCVAR_CHEAT,
                              "Snow.");
static ConVar r_SnowDebugBox("r_SnowDebugBox", "0", FCVAR_CHEAT,
                             "Snow Debug Boxes.");
static ConVar r_SnowZoomOffset("r_SnowZoomOffset", "384.0f", FCVAR_CHEAT,
                               "Snow.");
static ConVar r_SnowZoomRadius("r_SnowZoomRadius", "512.0f", FCVAR_CHEAT,
                               "Snow.");
...
static ConVar r_SnowRayLength("r_SnowRayLength", "8192.0f", FCVAR_CHEAT,
                              "Snow.");
...
inline bool CClient_Precipitation::SimulateRain(
  ...
  if (GetRemainingLifetime(pParticle) < 0.0f) return false;
  ...
  if (!IsInAir(pParticle->m_Pos)) {
    // Possibly make a splash if we hit a water surface and it's in front of the
    // view.
...
inline bool CClient_Precipitation::SimulateSnow(
    CPrecipitationParticle *pParticle, float dt) {
  if (IsInAir(pParticle->m_Pos)) {
    // Update position
    VectorMA(pParticle->m_Pos, dt, pParticle->m_Velocity, pParticle->m_Pos);
...
void CClient_Precipitation::Simulate(float dt) {
  ...
  if (r_RainHack.GetInt())
    m_Lifetime = (GetClientWorldEntity()->m_WorldMaxs[2] -
                  GetClientWorldEntity()->m_WorldMins[2]) /
                 m_Speed;
  else
  ...
  if (m_nPrecipType == PRECIPITATION_TYPE_RAIN) {
    for (int i = m_Particles.Head(); i != m_Particles.InvalidIndex();
         i = iNext) {
      iNext = m_Particles.Next(i);
      if (!SimulateRain(&m_Particles[i], dt)) m_Particles.Remove(i);
    }
  } else if (m_nPrecipType == PRECIPITATION_TYPE_SNOW) {
    for (int i = m_Particles.Head(); i != m_Particles.InvalidIndex();
         i = iNext) {
      iNext = m_Particles.Next(i);
      if (!SimulateSnow(&m_Particles[i], dt)) m_Particles.Remove(i);
    }
  }

  if (r_RainProfile.GetInt()) {
    timer.End();
    engine->Con_NPrintf(15, "Rain simulation: %du (%d tracers)",
                        timer.GetDuration().GetMicroseconds(),
                        m_Particles.Count());
  }
}
...
inline void CClient_Precipitation::RenderParticle(
    CPrecipitationParticle *pParticle, CMeshBuilder &mb) {
  float scale;
  Vector start, delta;
  ...
  if (flScreenSpaceWidth < MIN_SCREENSPACE_RAIN_WIDTH) {
    // Make the rain tracer at least the min size, but fade its alpha the
    // smaller it gets.
...
void CClient_Precipitation::CreateWaterSplashes() {
  for (int i = 0; i < m_Splashes.Count(); i++) {
    ...
    if (CurrentViewForward().Dot(vSplash - CurrentViewOrigin()) > 1) {
      FX_WaterRipple(vSplash, g_flSplashScale, &g_vSplashColor,
                     g_flSplashLifetime, g_flSplashAlpha);
    }
  }
  m_Splashes.Purge();
}
...
void CClient_Precipitation::Render() {
  ...
  if (r_RainProfile.GetInt()) {
    timer.End();
    engine->Con_NPrintf(16, "Rain render    : %du",
                        timer.GetDuration().GetMicroseconds());
  }
}
  ...
  if (!m_MatHandle) {
    ...
    switch (m_nPrecipType) {
      case PRECIPITATION_TYPE_SNOW:
        m_Speed = SNOW_SPEED;
        m_MatHandle = materials->FindMaterial("particle/snow",
      ...
      case PRECIPITATION_TYPE_RAIN:
        ...
        m_Speed = RAIN_SPEED;
        m_MatHandle = materials->FindMaterial("particle/rain",
                                              TEXTURE_GROUP_CLIENT_EFFECTS);
        m_InitialRamp = 1.0f;
...
inline float CClient_Precipitation::GetRemainingLifetime(
    CPrecipitationParticle *pParticle) const {
...
bool CClient_Precipitation::ComputeEmissionArea(Vector &origin,
  ...
  float emissionHeight = std::min(vMaxs[2], pPlayer->GetAbsOrigin()[2] + 512);
  ...
  origin[0] = std::max(vMins[0], lobound[0]);
  origin[1] = std::max(vMins[1], lobound[1]);
  origin[2] = emissionHeight;

  hibound[0] = std::min(vMaxs[0], hibound[0]);
  hibound[1] = std::min(vMaxs[1], hibound[1]);
...
float AshDebrisEffect::UpdateRoll(SimpleParticle *pParticle, float timeDelta) {
  ...
  if (pParticle->m_iFlags & ASH_PARTICLE_NOISE) {
    ...
    pParticle->m_Pos =
        (pParticle->m_Pos + Vector(vTempEntVel[0] * timeDelta * s,
...
void CClient_Precipitation::CreateAshParticle(void) {
  ...
  if (m_pAshEmitter == NULL) {
    if ((m_pAshEmitter = AshDebrisEffect::Create("ashtray")) == NULL) return;
  ...
  while (m_tAshParticleTraceTimer.NextEvent(curTime)) {
    ...
    UTIL_TraceLine(vTraceStart, vTraceEnd, MASK_SHOT_HULL & (~CONTENTS_GRATE),
    ...
    if (tr.fraction != 1.0f) {
      ...
      UTIL_TraceModel(vTraceStart, tr.endpos, Vector(-1, -1, -1),
                      Vector(1, 1, 1), this, COLLISION_GROUP_NONE, &tr2);
  ...
  if (m_bActiveAshEmitter == false) return;
  ...
  while (m_tAshParticleTimer.NextEvent(curTime)) {
    ...
    if (offset[0] > absmaxs[0] || offset[1] > absmaxs[1] ||
        offset[2] > absmaxs[2] || offset[0] < absmins[0] ||
    ...
    pParticle = (SimpleParticle *)m_pAshEmitter->AddParticle(
        sizeof(SimpleParticle), hMaterial[iRandom], offset);
    ...
    if (bEmberTime == true) {
      ...
      pParticle->m_vecVelocity =
          Vector(RandomFloat(-20.0f, 20.0f), RandomFloat(-20.0f, 20.0f),
                 RandomFloat(-10, -15));
    }
...
void CClient_Precipitation::CreateRainOrSnowParticle(Vector vSpawnPosition,
                                                     Vector vVelocity) {
  // Create the particle
  ...
  p->m_Velocity[0] +=
      random->RandomFloat(-r_RainSideVel.GetInt(), r_RainSideVel.GetInt());
  p->m_Velocity[1] +=
      random->RandomFloat(-r_RainSideVel.GetInt(), r_RainSideVel.GetInt());
...
void CClient_Precipitation::EmitParticles(float fTimeDelta) {
  ...
  if (!ComputeEmissionArea(org, size)) return;
  ...
  // Compute number of particles to emit based on precip density and emission
  // area and dt
  ...
  for (int i = 0; i < cParticles; i++) {
    ...
    // Figure out where the particle should lie in Z by tracing a line from the
    // player's height up to the desired height and making sure it doesn't hit a
    // wall.
    Vector vPlayerHeight = vParticlePos;
    vPlayerHeight.z = vPlayerCenter.z;

    trace_t trace;
    UTIL_TraceLine(vPlayerHeight, vParticlePos, MASK_SOLID_BRUSHONLY, NULL,
                   COLLISION_GROUP_NONE, &trace);
    if (trace.fraction < 1) {
...
void CClient_Precipitation::ComputeWindVector() {
  ...
  QAngle windangle(0, cl_winddir.GetFloat(),
                   0);  // used to turn wind yaw direction into a vector

  // Randomize the wind angle and speed slightly to get us a little variation
  windangle[1] = windangle[1] + random->RandomFloat(-10, 10);
  float windspeed =
      cl_windspeed.GetFloat() * (1.0 + random->RandomFloat(-0.2, 0.2));
 ...
 public:
  ...
  virtual void LevelInitPostEntity() {
    if (r_RainHack.GetInt()) {
      ...
      pPrecipHackEnt->InitializeAsClientEntity(NULL,
...
RecvPropInt(RECVINFO(m_iMinWind)), RecvPropInt(RECVINFO(m_iMaxWind)),
    RecvPropInt(RECVINFO(m_iMinGust)), RecvPropInt(RECVINFO(m_iMaxGust)),
    RecvPropFloat(RECVINFO(m_flMinGustDelay)),
    RecvPropFloat(RECVINFO(m_flMaxGustDelay)),
    RecvPropInt(RECVINFO(m_iGustDirChange)), RecvPropInt(RECVINFO(m_iWindSeed)),
    ...
    END_RECV_TABLE()
        ...
        IMPLEMENT_CLIENTCLASS_DT(C_EnvWind, DT_EnvWind, CEnvWind)
            RecvPropDataTable(RECVINFO_DT(m_EnvWindShared), 0,
                              &REFERENCE_RECV_TABLE(DT_EnvWindShared)),
    END_RECV_TABLE()
...
void C_EnvWind::OnDataChanged(DataUpdateType_t updateType) {
  ...
  m_EnvWindShared.Init(
      entindex(), m_EnvWindShared.m_iWindSeed, m_EnvWindShared.m_flStartTime,
...
RecvPropInt(RECVINFO(m_nDensity)), RecvPropInt(RECVINFO(m_nLifetime)),
    RecvPropInt(RECVINFO(m_nSpeed)), RecvPropInt(RECVINFO(m_bEmit)),
    END_RECV_TABLE()
...
void C_Embers::SpawnEmber(void) {
  ...
  // Spawn the particle
  SimpleParticle *sParticle = (SimpleParticle *)m_pEmitter->AddParticle(
      sizeof(SimpleParticle), m_hMaterial, offset);
  ...
  sParticle->m_vecVelocity[0] +=
      random->RandomFloat(-(m_nSpeed / 8), (m_nSpeed / 8));
  sParticle->m_vecVelocity[1] +=
      random->RandomFloat(-(m_nSpeed / 8), (m_nSpeed / 8));
  sParticle->m_vecVelocity[2] +=
      random->RandomFloat(-(m_nSpeed / 8), (m_nSpeed / 8));
...
RecvPropVector(RECVINFO(m_targetPosition)),
  ...
  return Vector(color.r * (1.0 / 255.0f), color.g * (1.0 / 255.0f),
                color.b * (1.0 / 255.0f));
}
...
int C_QuadraticBeam::DrawModel(int) {
  ...
  DrawBeamQuadratic(GetRenderOrigin(), m_controlPosition, m_targetPosition,
                    m_flWidth, color, gpGlobals->curtime * m_scrollRate);
  return 1;
}
...
class CSnowFallManager : public C_BaseEntity {
 ...
 private:
  ...
  void CreateSnowFallParticles(float flCurrentTime, float flRadius,
                               const Vector &vecEyePos,
                               const Vector &vecForward, float flZoomScale);
  void CreateOutsideVolumeSnowParticles(float flCurrentTime, float flRadius,
                                        float flZoomScale);
  void CreateInsideVolumeSnowParticles(float flCurrentTime, float flRadius,
                                       const Vector &vecEyePos,
                                       const Vector &vecForward,
                                       float flZoomScale);
  void CreateSnowParticlesSphere(float flRadius);
  void CreateSnowParticlesRay(float flRadius, const Vector &vecEyePos,
                              const Vector &vecForward);
  void CreateSnowFallParticle(const Vector &vecParticleSpawn, int iBBox);

  int StandingInSnowVolume(Vector &vecPoint);
  void FindSnowVolumes(Vector &vecCenter, float flRadius, Vector &vecEyePos,
                       Vector &vecForward);
...
void CSnowFallManager::ClientThink(void) {
  ...
  if (!m_pSnowFallEmitter) {
    if (!CreateSnowFallEmitter()) return;
  }
...
void CSnowFallManager::AddSnowFallEntity(CClient_Precipitation *pSnowEntity) {
  if (!pSnowEntity) return;

  int nSnowCount = m_aSnow.Count();
  int iSnow = 0;
  for (iSnow = 0; iSnow < nSnowCount; ++iSnow) {
    if (m_aSnow[iSnow].m_pEntity == pSnowEntity) break;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CSnowFallManager::UpdateBounds(const Vector &vecSnowMin,
                                    const Vector &vecSnowMax) {
  int iAxis = 0;
...
int CSnowFallManager::StandingInSnowVolume(Vector &vecPoint) {
  ...
  for (iSnow = 0; iSnow < nSnowCount; ++iSnow) {
    UTIL_TraceModel(vecPoint, vecPoint, vec3_origin, vec3_origin,
                    static_cast<C_BaseEntity *>(m_aSnow[iSnow].m_pEntity),
                    COLLISION_GROUP_NONE, &traceSnow);
    if (traceSnow.startsolid) return iSnow;
  }
...
//			flRadius -
//-----------------------------------------------------------------------------
void CSnowFallManager::FindSnowVolumes(Vector &vecCenter, float flRadius,
                                       Vector &vecEyePos, Vector &vecForward) {
  // Reset.
  m_nActiveSnowCount = 0;
  ...
  for (iSnow = 0; iSnow < nSnowCount; ++iSnow) {
    ...
    bool bInPVS = g_pClientLeafSystem->IsRenderableInPVS(
        m_aSnow[iSnow].m_pEntity->GetClientRenderable());
    if (!bInPVS) continue;

    // Check to see if a snow volume is inside the given radius.
    if (IsBoxIntersectingSphere(m_aSnow[iSnow].m_vecMin,
                                m_aSnow[iSnow].m_vecMax, vecCenter, flRadius)) {
    ...
    // Check to see if a snow volume is outside of the sphere radius, but is
    // along line-of-sight.
    else {
      CBaseTrace trace;
      ...
      IntersectRayWithBox(vecEyePos, vecNewForward, m_aSnow[iSnow].m_vecMin,
                          m_aSnow[iSnow].m_vecMax, 0.325f, &trace);
  ...
  if (r_SnowDebugBox.GetFloat() != 0.0f) {
    for (iSnow = 0; iSnow < m_nActiveSnowCount; ++iSnow) {
      ...
      debugoverlay->AddBoxOverlay(vecCenter, vecMin, vecMax, QAngle(0, 0, 0),
                                  200, 0, 0, 25, r_SnowDebugBox.GetFloat());
    }
  }
#endif
}
...
void CSnowFallManager::CreateSnowFall(void) {
  ...
  // Check to see if we have a local player before starting the snow around a
  // local player.
  C_BasePlayer *pPlayer = C_BasePlayer::GetLocalPlayer();
  if (pPlayer == NULL) return;
  ...
  m_vecSnowFallEmitOrigin +=
      (vecForward * (64.0f + (flSpeed * 0.4f * r_SnowPosScale.GetFloat())));
  m_vecSnowFallEmitOrigin +=
      (vecVelocity * (flSpeed * 1.25f * r_SnowSpeedScale.GetFloat()));
  ...
  CreateSnowFallParticles(flCurrentTime, m_flSnowRadius, pPlayer->EyePosition(),
                          vecForward, flZoomScale);
}
...
//			flZoomScale -
//-----------------------------------------------------------------------------
void CSnowFallManager::CreateSnowFallParticles(float flCurrentTime,
                                               float flRadius,
                                               const Vector &vecEyePos,
                                               const Vector &vecForward,
                                               float flZoomScale) {
  ...
  // Inside of a snow volume.
    ...
    CreateInsideVolumeSnowParticles(flCurrentTime, flRadius, vecEyePos,
                                    vecForward, flZoomScale);
  }
}
...
//			flZoomScale -
//-----------------------------------------------------------------------------
void CSnowFallManager::CreateOutsideVolumeSnowParticles(float flCurrentTime,
                                                        float flRadius,
                                                        float flZoomScale) {
  ...
  while (m_tSnowFallParticleTimer.NextEvent(flCurrentTime)) {
    ...
    vecParticleSpawn.x = RandomFloat(m_aSnow[m_aActiveSnow[iSnow]].m_vecMin.x,
                                     m_aSnow[m_aActiveSnow[iSnow]].m_vecMax.x);
    vecParticleSpawn.y = RandomFloat(m_aSnow[m_aActiveSnow[iSnow]].m_vecMin.y,
                                     m_aSnow[m_aActiveSnow[iSnow]].m_vecMax.y);
    vecParticleSpawn.z = RandomFloat(m_aSnow[m_aActiveSnow[iSnow]].m_vecMin.z,
                                     m_aSnow[m_aActiveSnow[iSnow]].m_vecMax.z);

    float flDistance2 =
        (m_vecSnowFallEmitOrigin - vecParticleSpawn).LengthSqr();
...
//			flZoomScale -
//-----------------------------------------------------------------------------
void CSnowFallManager::CreateInsideVolumeSnowParticles(float flCurrentTime,
                                                       float flRadius,
                                                       const Vector &vecEyePos,
                                                       const Vector &vecForward,
                                                       float flZoomScale) {
  ...
  if (bZoomed) {
    vecZoomEmitOrigin =
        m_vecSnowFallEmitOrigin +
        (vecForward * (r_SnowZoomOffset.GetFloat() * flZoomScale));
...
void CSnowFallManager::CreateSnowParticlesSphere(float flRadius) {
  ...
  vecParticleSpawn.x =
      m_vecSnowFallEmitOrigin.x + RandomFloat(-flRadius, flRadius);
  vecParticleSpawn.y =
      m_vecSnowFallEmitOrigin.y + RandomFloat(-flRadius, flRadius);
  vecParticleSpawn.z =
      m_vecSnowFallEmitOrigin.z + RandomFloat(-flRadius, flRadius);
...
//			&vecForward -
//-----------------------------------------------------------------------------
void CSnowFallManager::CreateSnowParticlesRay(float flRadius,
                                              const Vector &vecEyePos,
                                              const Vector &vecForward) {
  ...
  Vector vecNewForward;
  vecNewForward =
      vecForward * RandomFloat(flRadius, r_SnowRayLength.GetFloat());
...
void CSnowFallManager::CreateSnowFallParticle(const Vector &vecParticleSpawn,
                                              int iSnow) {
  SimpleParticle *pParticle = (SimpleParticle *)m_pSnowFallEmitter->AddParticle(
      sizeof(SimpleParticle), m_aSnow[iSnow].m_hMaterial, vecParticleSpawn);
  if (pParticle == NULL) return;

  pParticle->m_flLifetime = 0.0f;
  pParticle->m_vecVelocity =
      Vector(RandomFloat(-5.0f, 5.0f), RandomFloat(-5.0f, 5.0f),
             (RandomFloat(-25, -35) * r_SnowFallSpeed.GetFloat()));
  pParticle->m_flDieTime =
      fabs((vecParticleSpawn.z - m_aSnow[iSnow].m_vecMin.z) /
           (pParticle->m_vecVelocity.z - 0.1));
...
bool SnowFallManagerCreate(CClient_Precipitation *pSnowEntity) {
  if (!s_pSnowFallMgr) {
    ...
    if (!s_pSnowFallMgr) return false;
  }
