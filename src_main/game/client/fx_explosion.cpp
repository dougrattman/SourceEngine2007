...
class CExplosionParticle : public CSimpleEmitter {
 public:
  ...
  virtual void UpdateVelocity(SimpleParticle *pParticle, float timeDelta) {
    ...
    // Decellerate
    // pParticle->m_vecVelocity += pParticle->m_vecVelocity * ( timeDelta *
    // -20.0f );
    static float dtime;
    static float decay;
...
// Output : float
//-----------------------------------------------------------------------------
float C_BaseExplosionEffect::ScaleForceByDeviation(Vector &deviant,
                                                   Vector &source, float spread,
                                                   float *force) {
  if ((deviant == vec3_origin) || (source == vec3_origin)) return 1.0f;
...
// Output : virtual void
//-----------------------------------------------------------------------------
void C_BaseExplosionEffect::Create(const Vector &position, float force,
  ...
  debugoverlay->AddBoxOverlay(m_vecOrigin, -Vector(32, 32, 32),
                              Vector(32, 32, 32), vec3_angle, 255, 0, 0, 64,
                              5.0f);
  debugoverlay->AddLineOverlay(
      m_vecOrigin, m_vecOrigin + (m_vecDirection * force * m_flForce), 0, 0,
      255, false, 3);
#endif
...
void C_BaseExplosionEffect::CreateCore(void) {
  if (m_fFlags & TE_EXPLFLAG_NOFIREBALL) return;
  ...
  // Cap our force
  if (force < EXPLOSION_FORCE_MIN) force = EXPLOSION_FORCE_MIN;

  if (force > EXPLOSION_FORCE_MAX) force = EXPLOSION_FORCE_MAX;

  float spread = 1.0f - (0.15f * force);

  SimpleParticle *pParticle;

  CSmartPtr<CExplosionParticle> pSimple =
      CExplosionParticle::Create("exp_smoke");
  ...
  if ((m_fFlags & TE_EXPLFLAG_NOFIREBALLSMOKE) == 0) {
    ...
    for (i = 0; i < 4; i++) {
      pParticle = (SimpleParticle *)pSimple->AddParticle(
          sizeof(SimpleParticle), m_Material_Smoke, m_vecOrigin);
      ...
      if (pParticle != NULL) {
        ...
        pParticle->m_vecVelocity +=
            (m_vecDirection * random->RandomFloat(1.0f, 6.0f));
        ...
        ScaleForceByDeviation(pParticle->m_vecVelocity, m_vecDirection, spread,
                              &fForce);

        pParticle->m_vecVelocity *= fForce;

#if __EXPLOSION_DEBUG
        debugoverlay->AddLineOverlay(m_vecOrigin,
                                     m_vecOrigin + pParticle->m_vecVelocity,
                                     255, 0, 0, false, 3);
#endif
    ...
    for (i = 0; i < 8; i++) {
      ...
      pParticle = (SimpleParticle *)pSimple->AddParticle(
          sizeof(SimpleParticle), m_Material_Smoke, offset);
    ...
    for (i = 0; i < numRingSprites; i++) {
      ...
      offset = (RandomVector(-4.0f, 4.0f) + m_vecOrigin) +
               (forward * random->RandomFloat(8.0f, 16.0f));
      ...
      if (pParticle != NULL) {
        ...
        // Scale the force down as we fall away from our main direction
        ScaleForceByDeviation(pParticle->m_vecVelocity,
                              pParticle->m_vecVelocity, spread, &fForce);
  ...
  for (i = 0; i < 16; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), m_Material_Embers[random->RandomInt(0, 1)],
        offset);
    ...
    if (pParticle != NULL) {
      ...
      // Scale the force down as we fall away from our main direction
      float vDev = ScaleForceByDeviation(pParticle->m_vecVelocity,
                                         m_vecDirection, spread);

      pParticle->m_vecVelocity *= fForce * (16.0f * (vDev * vDev * 0.5f));

#if __EXPLOSION_DEBUG
      debugoverlay->AddLineOverlay(m_vecOrigin,
                                   m_vecOrigin + pParticle->m_vecVelocity, 255,
                                   0, 0, false, 3);
#endif

      int nColor = random->RandomInt(192, 255);
      pParticle->m_uchColor[0] = pParticle->m_uchColor[1] =
          pParticle->m_uchColor[2] = nColor;

      pParticle->m_uchStartSize = random->RandomInt(8, 16) * vDev;

      pParticle->m_uchStartSize =
          std::clamp(pParticle->m_uchStartSize, (u8)4, (u8)32);
  ...
  for (i = 0; i < numFireballs; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), m_Material_FireCloud, offset);
    ...
    if (pParticle != NULL) {
      ...
      pParticle->m_uchStartSize =
          std::clamp(pParticle->m_uchStartSize, (u8)32, (u8)85);
...
void C_BaseExplosionEffect::CreateDebris(void) {
  if (m_fFlags & TE_EXPLFLAG_NOPARTICLES) return;
  ...
  CSmartPtr<CTrailParticles> pSparkEmitter =
      CTrailParticles::Create("CreateDebris 1");
  ...
  pSparkEmitter->GetBinding().SetBBox(m_vecOrigin - Vector(128, 128, 128),
                                      m_vecOrigin + Vector(128, 128, 128));
  ...
  for (i = 0; i < numSparks; i++) {
    tParticle = (TrailParticle *)pSparkEmitter->AddParticle(
        sizeof(TrailParticle), m_Material_FireCloud, m_vecOrigin);
  ...
  Vector offset;
  CSmartPtr<CFleckParticles> fleckEmitter = CFleckParticles::Create(
      "CreateDebris 2", m_vecOrigin, Vector(128, 128, 128));
  if (!fleckEmitter) return;

  // Setup our collision information
  fleckEmitter->m_ParticleCollision.Setup(m_vecOrigin, &m_vecDirection, 0.9f,
                                          512, 1024, 800, 0.5f);
  ...
  for (i = 0; i < numFlecks; i++) {
    ...
    FleckParticle *pParticle = (FleckParticle *)fleckEmitter->AddParticle(
        sizeof(FleckParticle), g_Mat_Fleck_Cement[random->RandomInt(0, 1)],
        offset);
    ...
    pParticle->m_uchColor[0] = std::min(1.0f, 0.25f * colorRamp) * 255.0f;
    pParticle->m_uchColor[1] = std::min(1.0f, 0.25f * colorRamp) * 255.0f;
    pParticle->m_uchColor[2] = std::min(1.0f, 0.25f * colorRamp) * 255.0f;
  }
#endif  // !_XBOX
}
...
void C_BaseExplosionEffect::CreateDynamicLight(void) {
  if (m_fFlags & TE_EXPLFLAG_NODLIGHTS) return;
...
void C_BaseExplosionEffect::PlaySound(void) {
  if (m_fFlags & TE_EXPLFLAG_NOSOUND) return;

  CLocalPlayerFilter filter;
  C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, "BaseExplosionEffect.Sound",
                          &m_vecOrigin);
}
...
// Output : float
//-----------------------------------------------------------------------------
float C_BaseExplosionEffect::Probe(const Vector &origin, Vector *vecDirection,
                                   float strength) {
  // Press out
  ...
  trace_t tr;
  UTIL_TraceLine(origin, endpos, CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE,
                 &tr);
...
#if __EXPLOSION_DEBUG
  debugoverlay->AddLineOverlay(m_vecOrigin, endpos,
                               (255 * (1.0f - tr.fraction)),
                               (255 * tr.fraction), 0, false, 3);
#endif
...
//			&m_flForce -
//-----------------------------------------------------------------------------
void C_BaseExplosionEffect::GetForceDirection(const Vector &origin,
                                              float magnitude,
...
void ExplosionCallback(const CEffectData &data) {
  BaseExplosionEffect().Create(data.m_vOrigin, data.m_flMagnitude,
                               data.m_flScale, data.m_fFlags);
}
 ...
 public:
  ...
  virtual float UpdateRoll(SimpleParticle *pParticle, float timeDelta) {
    ...
    if (fabs(pParticle->m_flRollDelta) < 0.25f) {
      pParticle->m_flRollDelta =
          (pParticle->m_flRollDelta > 0.0f) ? 0.25f : -0.25f;
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void C_WaterExplosionEffect::Create(const Vector &position, float force,
                                    float scale, int flags) {
  m_vecOrigin = position;
  ...
  UTIL_TraceLine(m_vecOrigin, m_vecOrigin + vecTrace, MASK_WATER, NULL,
  ...
  if (tr.startsolid == false) {
    ...
    if (tr.fraction < 1.0f) {
      ...
      // NOTENOTE: We somehow got into a water explosion without being near
      // water?
      Assert(0);
  ...
  FX_GetSplashLighting(m_vecOrigin + Vector(0, 0, 32), &m_vecColor,
                       &m_flLuminosity);
...
void C_WaterExplosionEffect::CreateCore(void) {
  ...
  Vector color;
  float luminosity;
  FX_GetSplashLighting(m_vecWaterSurface + Vector(0, 0, 8), &color,
                       &luminosity);

  float lifetime = random->RandomFloat(0.8f, 1.0f);

  // Ground splash
  FX_AddQuad(m_vecWaterSurface + Vector(0, 0, 2), Vector(0, 0, 1), 64,
             64 * 4.0f, 0.85f, luminosity, 0.0f, 0.25f,
             random->RandomInt(0, 360), random->RandomFloat(-4, 4), color, 2.0f,
             "effects/splashwake1", (FXQUAD_BIAS_SCALE | FXQUAD_BIAS_ALPHA));
...
void C_WaterExplosionEffect::CreateDebris(void) {
  ...
  CSmartPtr<CWaterExplosionParticle> pSimple =
      CWaterExplosionParticle::Create("waterexp_bubbles");
  pSimple->SetSortOrigin(m_vecOrigin);
  pSimple->SetNearClip(64, 128);
  ...
  for (i = 0; i < 16; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), pMaterial[random->RandomInt(0, 1)], offset);
...
void C_WaterExplosionEffect::CreateMisc(void) {
  ...
  for (i = 0; i < numDrops; i++) {
    ...
    tParticle = (TrailParticle *)sparkEmitter->AddParticle(
        sizeof(TrailParticle), hMaterial, offset);
    ...
    tParticle->m_vecVelocity =
        offDir *
        random->RandomFloat(50.0f * flScale * 2.0f, 100.0f * flScale * 2.0f);
    tParticle->m_vecVelocity[2] += random->RandomFloat(32.0f, 128.0f) * flScale;

    tParticle->m_flWidth =
        std::clamp(random->RandomFloat(1.0f, 3.0f) * flScale, 0.1f, 4.0f);
    tParticle->m_flLength =
        random->RandomFloat(length * 0.25f, length) /* * flScale*/;

    colorRamp = random->RandomFloat(1.5f, 2.0f);

    FloatToColor32(tParticle->m_color,
                   std::min(1.0f, m_vecColor[0] * colorRamp),
                   std::min(1.0f, m_vecColor[1] * colorRamp),
                   std::min(1.0f, m_vecColor[2] * colorRamp), m_flLuminosity);
  }
  ...
  for (i = 0; i < 4; i++) {
    ...
    tParticle->m_vecVelocity =
        offDir * random->RandomFloat(50 * flScale * 3.0f, 100 * flScale * 3.0f);
    tParticle->m_vecVelocity[2] += random->RandomFloat(32.0f, 128.0f) * flScale;

    tParticle->m_flWidth =
        std::clamp(random->RandomFloat(2.0f, 3.0f) * flScale, 0.1f, 4.0f);
    tParticle->m_flLength =
        random->RandomFloat(length * 0.25f, length) /* * flScale*/;
  ...
  pSimple->GetBinding().SetBBox(
      m_vecWaterSurface - (Vector(32.0f, 32.0f, 32.0f) * flScale),
      m_vecWaterSurface + (Vector(32.0f, 32.0f, 32.0f) * flScale));
  ...
  for (i = 0; i < 16; i++) {
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        sizeof(SimpleParticle), hMaterial, m_vecWaterSurface);

    if (pParticle == NULL) break;

    pParticle->m_flLifetime = 0.0f;
    pParticle->m_flDieTime = 2.0f;  // NOTENOTE: We use a clip plane to
                                    // realistically control our lifespan
    ...
    pParticle->m_uchColor[0] =
        std::min(1.0f, m_vecColor[0] * colorRamp) * 255.0f;
    pParticle->m_uchColor[1] =
        std::min(1.0f, m_vecColor[1] * colorRamp) * 255.0f;
    pParticle->m_uchColor[2] =
        std::min(1.0f, m_vecColor[2] * colorRamp) * 255.0f;

    pParticle->m_uchStartSize =
        24 * flScale * RemapValClamped(i, 7, 0, 1, 0.5f);
    pParticle->m_uchEndSize = std::min(255, pParticle->m_uchStartSize * 2);

    pParticle->m_uchStartAlpha =
        RemapValClamped(i, 7, 0, 255, 32) * m_flLuminosity;
    pParticle->m_uchEndAlpha = 0;
...
void C_WaterExplosionEffect::PlaySound(void) {
  ...
  CLocalPlayerFilter filter;
  C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, "Physics.WaterSplash",
                          &m_vecWaterSurface);

  if (m_flDepth > 128) {
    C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD,
                            "WaterExplosionEffect.Sound", &m_vecOrigin);
  } else {
    C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD,
...
void WaterSurfaceExplosionCallback(const CEffectData &data) {
  WaterExplosionEffect().Create(data.m_vOrigin, data.m_flMagnitude,
                                data.m_flScale, data.m_fFlags);
}
