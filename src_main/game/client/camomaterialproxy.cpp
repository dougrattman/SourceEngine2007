...
class CCamoMaterialProxy : public CEntityMaterialProxy {
 ...
 private:
  void LoadCamoPattern();
  void GenerateRandomPointsInNormalizedCube();
...
void CCamoTextureRegen::RegenerateTextureBits(ITexture *pTexture,
                                              IVTFTexture *pVTFTexture,
                                              Rect_t *pSubRect) {
...
CCamoMaterialProxy::~CCamoMaterialProxy() {
  ...
  if (m_pCamoTextureVar) {
    ...
    if (pCamoTexture) pCamoTexture->SetTextureRegenerator(NULL);
  }
...
bool CCamoMaterialProxy::Init(IMaterial *pMaterial, KeyValues *pKeyValues) {
  return false;  // hack!  Need to make sure that the TGA loader has a valid
  ...
  ITexture *pCamoTexture = m_pCamoTextureVar->GetTextureValue();
  if (pCamoTexture) pCamoTexture->SetTextureRegenerator(&m_TextureRegen);

  // Need to get the palettized texture to create the procedural texture from
  // somewhere.
  m_pCamoPatternTextureVar =
      m_pMaterial->FindVar("$camoPatternTexture", &found);
  if (!found) {
    m_pCamoTextureVar = NULL;
    return false;
  }
  ...
  subBoundingBoxMinVar =
      m_pMaterial->FindVar("$camoBoundingBoxMin", &found, false);
  if (!found) {
  ...
  subBoundingBoxMaxVar =
      m_pMaterial->FindVar("$camoBoundingBoxMax", &found, false);
  if (!found) {
...
void CCamoMaterialProxy::GetColors(Vector &diffuseColor, Vector &baseColor,
                                   int index, const Vector &boxMin,
                                   const Vector &boxExtents,
                                   const Vector &forward, const Vector &right,
                                   const Vector &up,
                                   const Vector &entityPosition) {
  ...
  transformedPosition[0] =
      right[0] * position[0] + forward[0] * position[1] + up[0] * position[2];
  transformedPosition[1] =
      right[1] * position[0] + forward[1] * position[1] + up[1] * position[2];
  transformedPosition[2] =
      right[2] * position[0] + forward[2] * position[1] + up[2] * position[2];
  ...
  //	engine->TraceLineMaterialAndLighting( g_vecInstantaneousRenderOrigin,
  // endPoint, diffuseColor, baseColor );
  engine->TraceLineMaterialAndLighting(transformedPosition, endPoint,
                                       diffuseColor, baseColor);

  // hack - optimize! - convert from linear to gamma space - this should be
  // hidden
...
// Procedurally generates the camo texture...
//-----------------------------------------------------------------------------
void CCamoMaterialProxy::GenerateCamoTexture(ITexture *pTexture,
                                             IVTFTexture *pVTFTexture) {
  if (!m_pEnt) return;
  ...
  for (i = 0; i < m_CamoPatternNumColors; i++) {
    GetColors(diffuseColor[i], baseColor, i, mins, maxsMinusMins, forward,
              right, up, entityPosition);
#if 1
...
void CCamoMaterialProxy::LoadCamoPattern() {
#if 0
  ...
  if (!TGALoader::GetInfo(m_pCamoPatternTextureVar->GetStringValue(),
                          &m_CamoPatternWidth, &m_CamoPatternHeight,
                          &indexImageFormat, &dummyGamma)) {
    // Warning( "Can't get tga info for
    // hl2/materials/models/combine_elite/camo7paletted.tga for camo material\n"
    // );
    m_pCamoTextureVar = NULL;
    return;
  }
  ...
  if (indexImageFormat != IMAGE_FORMAT_I8) {
    //	Warning( "Camo material texture
    // hl2/materials/models/combine_elite/camo7paletted.tga must be 8-bit
    // greyscale\n" );
    m_pCamoTextureVar = NULL;
    return;
  }

  indexImageSize = ImageLoader::GetMemRequired(
      m_CamoPatternWidth, m_CamoPatternHeight, 1, indexImageFormat, false);
#if 0
...
#ifndef _XBOX
  if (!TGALoader::Load(m_pCamoPatternImage,
                       m_pCamoPatternTextureVar->GetStringValue(),
                       m_CamoPatternWidth, m_CamoPatternHeight, IMAGE_FORMAT_I8,
                       dummyGamma, false)) {
    //			Warning( "camo texture
    // hl2/materials/models/combine_elite/camo7paletted.tga must be grey-scale"
    //);
    m_pCamoTextureVar = NULL;
    return;
  }
...
void CCamoMaterialProxy::GenerateRandomPointsInNormalizedCube() {
  ...
  int i;
  for (i = 0; i < m_CamoPatternNumColors; i++) {
    m_pointsInNormalizedBox[i][0] =
        random->RandomFloat(m_SubBoundingBoxMin[0], m_SubBoundingBoxMax[0]);
    m_pointsInNormalizedBox[i][1] =
        random->RandomFloat(m_SubBoundingBoxMin[1], m_SubBoundingBoxMax[1]);
    m_pointsInNormalizedBox[i][2] =
        random->RandomFloat(m_SubBoundingBoxMin[2], m_SubBoundingBoxMax[2]);
  }
}

IMaterial *CCamoMaterialProxy::GetMaterial() { return m_pMaterial; }

EXPOSE_INTERFACE(CCamoMaterialProxy, IMaterialProxy,
