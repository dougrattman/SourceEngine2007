...
//			frac -
//-----------------------------------------------------------------------------
void InterpolateAngles(const QAngle &start, const QAngle &end, QAngle &output,
                       float frac) {
  Quaternion src, dest;
...
//			output -
//-----------------------------------------------------------------------------
void InterpolateVector(float frac, const Vector &src, const Vector &dest,
                       Vector &output) {
  int i;
...
//			 as flecks) for the given iDamageType and the trace's
// end position
// Input   :
// Output  :
//------------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
void UTIL_SetTrace(trace_t &trace, const Ray_t &ray, C_BaseEntity *ent,
...
//			Returns true if it's onscreen
//-----------------------------------------------------------------------------
bool GetTargetInScreenSpace(C_BaseEntity *pTargetEntity, int &iX, int &iY,
                            Vector *vecOffset) {
  return GetVectorInScreenSpace(pTargetEntity->WorldSpaceCenter(), iX, iY,
                                vecOffset);
}
...
//			*param4 -
//-----------------------------------------------------------------------------
void ClientPrint(C_BasePlayer *player, int msg_dest, const char *msg_name,
...
CFlaggedEntitiesEnum::CFlaggedEntitiesEnum(C_BaseEntity **pList, int listMax,
                                           int flagMask) {
  m_pList = pList;
  m_listMax = listMax;
  m_flagMask = flagMask;
...
IterationRetval_t CFlaggedEntitiesEnum::EnumElement(
  ...
  IClientEntity *pClientEntity =
      cl_entitylist->GetClientEntityFromHandle(pHandleEntity->GetRefEHandle());
...
//-----------------------------------------------------------------------------
// Purpose: Pass in an array of pointers and an array size, it fills the array
// and returns the number inserted Input  : **pList -
//			listMax -
//			&mins -
//			&maxs -
//			flagMask -
// Output : int
//-----------------------------------------------------------------------------
int UTIL_EntitiesInBox(C_BaseEntity **pList, int listMax, const Vector &mins,
                       const Vector &maxs, int flagMask, int partitionMask) {
...
// Output : int
//-----------------------------------------------------------------------------
int UTIL_EntitiesInSphere(C_BaseEntity **pList, int listMax,
                          const Vector &center, float radius, int flagMask,
                          int partitionMask) {
  CFlaggedEntitiesEnum sphereEnum(pList, listMax, flagMask);
  partition->EnumerateElementsInSphere(partitionMask, center, radius, false,
                                       &sphereEnum);
...
//			not skip over more than one separator at a time. This
// allows parsing 			strings where tokens between separators
...
// missing.
//			str - String to parse.
//			sep - Character to use as separator. UNDONE: allow
// multiple separator chars
...
//			%<commandname>% will get replaced with its bound
// control,
// e.g. %attack2% 			Input buffer sizes are in bytes rather
// than unicode character
// count 			for consistency with other APIs.  If
// inbufsizebytes is
// 0
// a
// NULL-terminated 			input buffer is assumed, or you can pass
// the size of the input buffer if 			not NULL-terminated.
//-----------------------------------------------------------------------------
void UTIL_ReplaceKeyBindings(const wchar_t *inbuf, int inbufsizebytes,
                             wchar_t *outbuf, int outbufsizebytes) {
  ...
  while (inbuf != inbufend && *inbuf != 0) {
    ...
    if (*inbuf == '%') {
      ...
      if (end && (end != inbuf))  // make sure we handle %% in the string, which
                                  // should be treated in the output as %
      {
        wchar_t token[64];
        ...
        const char *key =
            engine->Key_LookupBinding(*binding == '+' ? binding + 1 : binding);
        if (!key) {
...
byte *UTIL_LoadFileForMe(const char *filename, int *pLength) {
  ...
  if (!buffer) {
    Warning(
        "UTIL_LoadFileForMe:  Couldn't allocate buffer of size %i for file "
        "%s\n",
        size + 1, filename);
...
//-----------------------------------------------------------------------------
static unsigned char ComputeDistanceFade(C_BaseEntity *pEntity, float flMinDist,
                                         float flMaxDist) {
  if ((flMinDist <= 0) && (flMaxDist <= 0)) return 255;

  if (flMinDist > flMaxDist) {
    swap(flMinDist, flMaxDist);
  }

  // If a negative value is provided for the min fade distance, then base it off
  // the max.
  if (flMinDist < 0) {
  ...
  float flCurrentDistanceSq =
      CurrentViewOrigin().DistToSqr(pEntity->WorldSpaceCenter());
  ...
  return std::clamp(nAlpha, 0, 255);
}

//-----------------------------------------------------------------------------
// Compute fade amount
//-----------------------------------------------------------------------------
unsigned char UTIL_ComputeEntityFade(C_BaseEntity *pEntity, float flMinDist,
                                     float flMaxDist, float flFadeScale) {
...
#endif
  {
    ...
    // The center of the sphere is at the center of the OBB, which is not
    // necessarily at the render origin. But it should be close enough.
    ...
    unsigned char nGlobalAlpha =
        modelinfo->ComputeLevelScreenFade(vecAbsCenter, flRadius, flFadeScale);
    unsigned char nDistAlpha;

    if (!engine->IsLevelMainMenuBackground()) {
      nDistAlpha =
          modelinfo->ComputeViewScreenFade(vecAbsCenter, flRadius, flFadeScale);
    } else {
      nDistAlpha = 255;
    }
...
//-----------------------------------------------------------------------------
bool UTIL_GetMapLoadCountFileName(const char *pszFilePrependName,
                                  char *pszBuffer, int iBuflen) {
