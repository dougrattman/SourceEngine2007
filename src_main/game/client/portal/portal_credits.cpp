...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CHudPortalCredits::CHudPortalCredits(const char *pElementName)
    : CHudElement(pElementName), BaseClass(NULL, "HudCredits") {
  vgui::Panel *pParent = g_pClientMode->GetViewport();
  SetParent(pParent);
}

void CHudPortalCredits::LevelShutdown() { Clear(); }
...
void CHudPortalCredits::ReadNames(KeyValues *pKeyValue) {
  ...
  while (pKVNames) {
    ...
    Q_strcpy(Credits.szFontName,
             pKeyValue->GetString(Credits.szCreditName, "Default"));
...
void CHudPortalCredits::ReadLyrics(KeyValues *pKeyValue) {
  ...
  while (pKVNames) {
    ...
    vgui::HFont m_hTFont =
        vgui::scheme()->GetIScheme(scheme)->GetFont("CreditsOutroText", true);
    Q_strcpy(Credits.szCreditName, pKVNames->GetName());
    Q_strcpy(Credits.szFontName,
             pKeyValue->GetString(Credits.szCreditName, "Default"));
    ...
    if (Credits.szCreditName[0] == '#') {
      g_pVGuiLocalize->ConstructString(
          Credits.szLyricLine, sizeof(Credits.szLyricLine),
          g_pVGuiLocalize->Find(Credits.szCreditName), 0);
      if (bNextOnSameLine) {
        iXOffsetTemp += GetStringPixelWidth(Credits.szLyricLine, m_hTFont);
      }
    } else {
      g_pVGuiLocalize->ConvertANSIToUnicode(Credits.szCreditName,
                                            Credits.szLyricLine,
                                            sizeof(Credits.szLyricLine));
      if (bNextOnSameLine) {
        iXOffsetTemp += GetStringPixelWidth(Credits.szLyricLine, m_hTFont);
      }
    }
...
void CHudPortalCredits::ReadAscii(KeyValues *pKeyValue) {
  ...
  while (pKVNames) {
    portalcreditname_t Credits;
    // Q_strcpy( Credits.szCreditName, pKVNames->GetName());
    // Q_strcpy( Credits.szFontName, pKeyValue->GetString( Credits.szCreditName,
    // "Default" ) );

    Q_strcpy(Credits.szFontName, pKVNames->GetName());
    Q_strcpy(Credits.szCreditName,
             pKeyValue->GetString(Credits.szFontName, "Default"));
...
int CHudPortalCredits::GetStringPixelWidth(wchar_t *pString,
                                           vgui::HFont hFont) {
  int iLength = 0;
...
void CHudPortalCredits::DrawOutroCreditsName(void) {
  if (m_CreditsList.Count() == 0) return;
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    if (pCredit == NULL) continue;

    vgui::HScheme scheme = vgui::scheme()->GetScheme("ClientScheme");
    vgui::HFont m_hTFont =
        vgui::scheme()->GetIScheme(scheme)->GetFont(pCredit->szFontName, true);
    ...
    if (i == m_CreditsList.Count() - 1) {
      if (m_bLastOneInPlace == false) {
        pCredit->flYPos -=
            gpGlobals->frametime *
            ((float)g_iPortalCreditsPixelHeight / m_flScrollTime);
    ...
    if (pCredit->bActive == false) continue;
    ...
    if (pCredit->szCreditName[0] == '#') {
      g_pVGuiLocalize->ConstructString(
          ...
          g_pVGuiLocalize->Find(pCredit->szCreditName), 0);
    } else {
      g_pVGuiLocalize->ConvertANSIToUnicode(pCredit->szCreditName, unicode,
                                            sizeof(unicode));
    }

    int iStringWidth = GetStringPixelWidth(unicode, m_hTFont);

    surface()->DrawSetTextPos((iWidth / 2) - (iStringWidth / 2),
                              pCredit->flYPos);
...
void CHudPortalCredits::DrawPortalOutroCreditsName(void) {
  ...
  if (gpGlobals->curtime - m_flLastPaintTime > 0.05) {
    ...
    iCounter += 100;
    if (iCounter > (int)m_flScrollTime &&
        m_iYOffset < (m_CreditsList.Count() - 1)) {
      bCursor = !bCursor;
      iCounter = 0;
      m_iXOffset += 1;

      if (m_iXOffset > Q_strlen(m_CreditsList[m_iYOffset].szCreditName) &&
          m_iYOffset < (m_CreditsList.Count() - 1)) {
        m_iXOffset = 1;
        m_iYOffset += 1;

        vgui::HScheme scheme = vgui::scheme()->GetScheme("ClientScheme");
        vgui::HFont m_hTFont = vgui::scheme()->GetIScheme(scheme)->GetFont(
            m_CreditsList[m_iYOffset].szFontName, true);
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    if (pCredit->flYPos < -iFontTall || pCredit->flYPos > iTall) {
      ...
      if (i <= m_iYOffset) pCredit->bActive = true;
    }
    ...
    if (i == m_CreditsList.Count() - 1) {
      ...
      // (float)g_iPortalCreditsPixelHeight / m_flScrollTime )); m_iYOffset +=
      // (int) (gpGlobals->frametime * ( (float)g_iPortalCreditsPixelHeight /
      // 1000 )); iCounter += 1; pCredit->flYPos -= gpGlobals->frametime * (
      // (float)g_iPortalCreditsPixelHeight / m_flScrollTime );
    }
    ...
    if (m_iYOffset == i) {
      ...
      Q_strncpy(tmpstr, pCredit->szCreditName,
                Q_strlen(pCredit->szCreditName) + 1);
    }

    if (tmpstr[0] == '#') {
      g_pVGuiLocalize->ConstructString(unicode, sizeof(unicode),
                                       g_pVGuiLocalize->Find(tmpstr), 0);
    } else {
    ...
    surface()->DrawSetTextPos(20 + m_iScreenXOffset,
                              pCredit->flYPos + m_iScreenYOffset);
    surface()->DrawUnicodeString(unicode);
  }
}

void CHudPortalCredits::DrawPortalAsciiArt(void) {
  if (m_AsciiList.Count() == 0) return;

  if (m_iCurrentAsciiArt == 0) return;
  ...
  for (int i = 0; i < 20; i++) {
    portalcreditname_t *pCredit =
        &m_AsciiList[((m_iCurrentAsciiArt - 1) * 20) + i];
    ...
    // pCredit->szFontName, true );
    vgui::HFont m_hTFont =
        vgui::scheme()->GetIScheme(scheme)->GetFont(m_szAsciiArtFont, true);
    ...
    surface()->DrawSetTextPos(
        20 + (iWidth / 2) + m_iAAScreenXOffset,
        10 + (iTall / 2) + m_iAAScreenYOffset + (i * ift));
    surface()->DrawUnicodeString(unicode);
  }
}

void CHudPortalCredits::DrawPortalOutroCreditsLyrics(void) {
  if (m_LyricsList.Count() == 0) return;
  ...
  for (int i = 0; i < m_LyricsList.Count(); i++) {
    ...
    if (pCredit->flTimeStart <= flCurTime && pCredit->flTimeEnd > flCurTime) {
      ...
      m_iXOffset =
          (int)(((flCurTime - pCredit->flTimeStart) / pCredit->flTimeInit) *
                Q_wcslen(pCredit->szLyricLine)) +
          1;
      if (m_iXOffset < 1)
        m_iXOffset = 1;
    ...
    if (!bBorders) {
      ...
      g_pVGuiLocalize->ConvertANSIToUnicode(m_szBorderH, bunicode,
                                            sizeof(bunicode));
      g_pVGuiLocalize->ConvertANSIToUnicode("|", bVunicode, sizeof(bVunicode));
      g_pVGuiLocalize->ConvertANSIToUnicode("||", bVunicode2,
                                            sizeof(bVunicode2));
      ...
      surface()->DrawSetTextPos((iWidth / 2) + m_iScreenXOffset,
                                10 + m_iScreenYOffset);
      surface()->DrawUnicodeString(bunicode);
      surface()->DrawSetTextPos(5 + m_iScreenXOffset,
                                (iTall - (iFontTall * 2)) + m_iScreenYOffset);
      surface()->DrawUnicodeString(bunicode);
      surface()->DrawSetTextPos((iWidth / 2) + m_iScreenXOffset,
                                (iTall / 2) + m_iScreenYOffset);
      surface()->DrawUnicodeString(bunicode);

      int iBorderY = 10 + iFontTall;
      while (iBorderY < (iTall - (iFontTall * 3))) {
        surface()->DrawSetTextPos(5 + m_iScreenXOffset,
                                  iBorderY + m_iScreenYOffset);
        surface()->DrawUnicodeString(bVunicode);
        surface()->DrawSetTextPos(iBorderEndX + m_iScreenXOffset,
                                  iBorderY + m_iScreenYOffset);
        if (iBorderY < (iTall / 2)) {
          surface()->DrawUnicodeString(bVunicode2);
          surface()->DrawSetTextPos(
              (iWidth - iBorderPipeWidth) + m_iScreenXOffset,
              iBorderY + m_iScreenYOffset);
          surface()->DrawUnicodeString(bVunicode);
        } else {
          surface()->DrawUnicodeString(bVunicode);
    ...
    if (m_iYOffset == i) {
      ...
      if (bCursor) tmpstr[m_iXOffset - 1] = '_';

    } else {
      Q_wcsncpy(tmpstr, pCredit->szLyricLine,
                (Q_wcslen(pCredit->szLyricLine) + 1) * sizeof(wchar_t));
    }

    surface()->DrawSetTextColor(m_cColor[0], m_cColor[1], m_cColor[2],
                                cColor[3]);
    surface()->DrawSetTextPos(20 + pCredit->iXOffset + m_iScreenXOffset,
                              30 + pCredit->flYPos + m_iScreenYOffset);
  ...
  for (int i = iCreditsListCount - 1; i >= 0; i--) {
    ...
    if (pCredit->flTimeStart <= flCurTime && pCredit->flTimeEnd > flCurTime) {
      ...
      m_iXOffsetNames =
          (int)(((flCurTime - pCredit->flTimeStart) / pCredit->flTimeInit) *
                Q_strlen(pCredit->szCreditName)) +
          1;
      if (m_iXOffsetNames < 1)
        m_iXOffsetNames = 1;
      else if (m_iXOffsetNames > Q_strlen(pCredit->szCreditName))
        m_iXOffsetNames = Q_strlen(pCredit->szCreditName);
    }
    ...
    if (pCredit->flYPos < 20 || pCredit->flYPos > (iTall / 2)) {
      ...
      if (i <= m_iYOffsetNames) pCredit->bActive = true;
    }
    ...
    surface()->DrawSetTextColor(m_cColor[0], m_cColor[1], m_cColor[2],
                                cColor[3]);
    surface()->DrawSetTextPos((iWidth / 2) + 10 + m_iScreenXOffset,
                              pCredit->flYPos + m_iScreenYOffset);
    surface()->DrawUnicodeString(unicode);
  }
}
...
void CHudPortalCredits::DrawLogo(void) {
  ...
  vgui::HScheme scheme = vgui::scheme()->GetScheme("ClientScheme");
  vgui::HFont m_hTFont =
      vgui::scheme()->GetIScheme(scheme)->GetFont(szLogoFont);
  ...
  if (hl2_episodic.GetBool()) {
    g_pVGuiLocalize->ConvertANSIToUnicode("== episode one==", unicode,
                                          sizeof(unicode));
...
// Purpose:
//-----------------------------------------------------------------------------
float CHudPortalCredits::FadeBlend(float fadein, float fadeout, float hold,
                                   float localTime) {
...
void CHudPortalCredits::DrawIntroCreditsName(void) {
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    vgui::HScheme scheme = vgui::scheme()->GetScheme("ClientScheme");
    vgui::HFont m_hTFont =
        vgui::scheme()->GetIScheme(scheme)->GetFont(pCredit->szFontName);
    ...
    surface()->DrawSetTextColor(
        ...
        FadeBlend(m_flFadeInTime, m_flFadeOutTime,
                  m_flFadeHoldTime + pCredit->flTimeAdd, localTime) *
            m_cColor[3]);
    ...
    if (m_flLogoTime > gpGlobals->curtime) continue;
      ...
      if (m_CreditsList.IsValidIndex(i + 3)) {
        ...
        if (pNextCredits && pNextCredits->flTime == 0.0f) {
          ...
          if (i < 3) {
            ...
            pNextCredits->flTime = gpGlobals->curtime + m_flFadeInTime +
                                   m_flFadeOutTime + m_flFadeHoldTime +
                                   pNextCredits->flTimeAdd;
          } else {
...
void CHudPortalCredits::PreparePortalOutroCredits(void) {
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    if (iHeight == -1)
      iHeight =
          (iTall / 2) - (surface()->GetFontTall(m_hTFont) + m_flSeparation);
    else
      iHeight += surface()->GetFontTall(m_hTFont) + m_flSeparation;

    if (i == 0) {
      Q_strcpy(m_szBorderH, "-");
      while (GetPixelWidth(m_szBorderH, m_hTFont) <
             ((iWidth / 2) - GetPixelWidth("---", m_hTFont))) {
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    m_CreditsList[i].flTimeInit =
        Q_strlen(m_CreditsList[i].szCreditName) * m_flNameCharTime;
...
void CHudPortalCredits::PrepareIntroCredits(void) {
  ...
  for (int i = 0; i < m_CreditsList.Count(); i++) {
    ...
    if (i < 3) {
      pCredit->bActive = true;
      pCredit->iSlot = iSlot;
      pCredit->flTime = gpGlobals->curtime + m_flFadeInTime + m_flFadeOutTime +
                        m_flFadeHoldTime;
