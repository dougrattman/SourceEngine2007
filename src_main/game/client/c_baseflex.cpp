...
ConVar g_CV_PhonemeDelay("phonemedelay", "0", 0,
                         "Phoneme delay to account for sound system latency.");
ConVar g_CV_PhonemeFilter("phonemefilter", "0.08", 0,
                          "Time duration of box filter to pass over phonemes.");
ConVar g_CV_FlexRules("flex_rules", "1", 0,
                      "Allow flex animation rules to run.");
ConVar g_CV_BlinkDuration("blink_duration", "0.2", 0,
                          "How many seconds an eye blink will last.");
ConVar g_CV_FlexSmooth(
    "flex_smooth", "1", 0,
...
IMPLEMENT_CLIENTCLASS_DT(C_BaseFlex, DT_BaseFlex, CBaseFlex)
  RecvPropArray3(RECVINFO_ARRAY(m_flexWeight),
      ...
      RecvPropVector(RECVINFO(m_vecLean)), RecvPropVector(RECVINFO(m_vecShift)),
#endif
...
C_BaseFlex::C_BaseFlex()
  ...
  AddVar(&m_viewtarget, &m_iv_viewtarget,
         LATCH_ANIMATION_VAR | INTERPOLATE_LINEAR_ONLY);
...
void C_BaseFlex::SetupMappings(char const *pchFileRoot) {
  ...
  Q_snprintf(strong->classname, sizeof(strong->classname), "%s_strong",
             pchFileRoot);
}
...
void C_BaseFlex::StandardBlendingRules(CStudioHdr *hdr, Vector pos[],
  ...
  if (hdr->GetNumIKChains() != 0 &&
      (m_vecShift.x != 0.0 || m_vecShift.y != 0.0)) {
    // CIKContext auto_ik;
    // auto_ik.Init( hdr, GetRenderAngles(), GetRenderOrigin(), currentTime,
    // gpGlobals->framecount, boneMask ); auto_ik.AddAllLocks( pos, q );
...
void C_BaseFlex::RunFlexRules(CStudioHdr *hdr, float *dest) {
  if (!g_CV_FlexRules.GetInt()) return;
  ...
  /*
          ...
          if ( pszExpression )
          {
                  nFlexRulesToRun = atoi(pszExpression); // 0 will be returned
  if not a numeric string
          }
  //*/
 ...
 public:
  ...
  virtual bool Init() {
    ...
    char const *pTFClasses[] = {
        "scout", "sniper", "soldier", "demo",     "medic",
    ...
    for (int i = 0; i < ARRAYSIZE(pTFClasses); ++i) {
      ...
      Q_snprintf(fn, sizeof(fn), "player/%s/phonemes/phonemes_weak",
                 pTFClasses[i]);
      FindSceneFile(NULL, fn, true);
      Q_snprintf(fn, sizeof(fn), "player/%s/phonemes/phonemes_strong",
                 pTFClasses[i]);
      FindSceneFile(NULL, fn, true);

      if (!IsX360()) {
        Q_snprintf(fn, sizeof(fn), "player/hwm/%s/phonemes/phonemes",
                   pTFClasses[i]);
        FindSceneFile(NULL, fn, true);
        Q_snprintf(fn, sizeof(fn), "player/hwm/%s/phonemes/phonemes_weak",
                   pTFClasses[i]);
        FindSceneFile(NULL, fn, true);
        Q_snprintf(fn, sizeof(fn), "player/hwm/%s/phonemes/phonemes_strong",
                   pTFClasses[i]);
        FindSceneFile(NULL, fn, true);
      }
      ...
      FindSceneFile(NULL, fn, true);
      if (!IsX360()) {
        Q_snprintf(fn, sizeof(fn), "player/hwm/%s/emotion/emotion",
                   pTFClasses[i]);
        FindSceneFile(NULL, fn, true);
      }
    }
  ...
  // Tracker 14992:  We used to load 18K of .vfes for every C_BaseFlex who
  ...
  // Output : 	void
  //-----------------------------------------------------------------------------
  void EnsureTranslations(C_BaseFlex *instance,
  ...
  void *FindSceneFile(C_BaseFlex *instance, const char *filename,
    ...
    void *buffer = NULL;
    int len = filesystem->ReadFileEx(VarArgs("expressions/%s.vfe", szFilename),
                                     "GAME", &buffer);
    ...
    if (IsX360()) {
      ...
      // Flex Settings
      flexsetting_t *pFlexSetting =
          (flexsetting_t *)((byte *)pHdr + pHdr->flexsettingindex);
      for (int i = 0; i < pHdr->numflexsettings; ++i, ++pFlexSetting) {
        swap.SwapFieldsToTargetEndian(pFlexSetting);

        flexweight_t *pWeight = (flexweight_t *)(((byte *)pFlexSetting) +
                                                 pFlexSetting->settingindex);
      ...
      swap.SwapBufferToTargetEndian((int *)pData, (int *)pData,
                                    pHdr->numindexes);
...
Vector C_BaseFlex::SetViewTarget(CStudioHdr *pStudioHdr) {
  if (!pStudioHdr) return Vector(0, 0, 0);
  ...
  if (!m_bSearchedForEyeFlexes) {
    ...
    if (m_iEyeUpdown != -1) {
      pStudioHdr->pFlexcontroller(m_iEyeUpdown)->localToGlobal =
          AddGlobalFlexController("eyes_updown");
    }
    if (m_iEyeRightleft != -1) {
      pStudioHdr->pFlexcontroller(m_iEyeRightleft)->localToGlobal =
          AddGlobalFlexController("eyes_rightleft");
    }
  }
  ...
  if (m_iEyeAttachment > 0) {
    ...
    if (local.x < flMaxEyeDeflection) {
      ...
      // debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), 1,
      // 0, "%5.3f %5.3f %5.3f", local.x, local.y, local.z );
      local.x = 0;
      float d = local.LengthSqr();
      if (d > 0.0f) {
        d = sqrtf((1.0f - flMaxEyeDeflection * flMaxEyeDeflection) /
                  (local.y * local.y + local.z * local.z));
  ...
  /*
  debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), 0, 0, "%.2f
  %.2f %.2f  : %.2f %.2f %.2f", m_viewtarget.x, m_viewtarget.y, m_viewtarget.z,
...
// Crossfade starts at STRONG_CROSSFADE_START and is full at
// STRONG_CROSSFADE_END If there isn't a strong then the intensity of the
// underlying phoneme is fixed at 2 x STRONG_CROSSFADE_START
//  so we don't get huge numbers
// Input  : *classes -
//			emphasis_intensity -
//-----------------------------------------------------------------------------
void C_BaseFlex::ComputeBlendedSetting(Emphasized_Phoneme *classes,
  ...
  if (emphasis_intensity > STRONG_CROSSFADE_START) {
    if (has_strong) {
      ...
      classes[PHONEME_CLASS_NORMAL].amount =
          (frac)*2.0f * STRONG_CROSSFADE_START;
    ...
    if (has_weak) {
      ...
      classes[PHONEME_CLASS_NORMAL].amount =
          (1.0f - frac) * 2.0f * WEAK_CROSSFADE_START;
...
//			newexpression -
//-----------------------------------------------------------------------------
void C_BaseFlex::AddViseme(Emphasized_Phoneme *classes,
                           float emphasis_intensity, int phoneme, float scale,
                           bool newexpression) {
  int type;
  ...
  for (type = 0; type < NUM_PHONEME_CLASSES; type++) {
    ...
    const flexsetting_t *pSetting =
        actual_flexsetting_header->pIndexedSetting(phoneme);
    if (!pSetting) {
      continue;
    }
    ...
    int truecount =
        pSetting->psetting((byte *)actual_flexsetting_header, 0, &pWeights);
    if (pWeights) {
      for (int i = 0; i < truecount; i++) {
        // Translate to global controller number
        int j = FlexControllerLocalToGlobal(actual_flexsetting_header,
                                            pWeights->key);
...
ConVar g_CV_PhonemeSnap("phonemesnap", "2", 0,
                        "Lod at level at which visemes stops always "
                        "considering two phonemes, regardless of duration.");
void C_BaseFlex::AddVisemesForSentence(Emphasized_Phoneme *classes,
                                       float emphasis_intensity,
                                       CSentence *sentence, float t, float dt,
                                       bool juststarted) {
    ...
    if (t > phoneme->GetStartTime() && t < phoneme->GetEndTime()) {
      ...
      if (bCrossfade) {
        if (k < pcount - 1) {
          ...
          if (next) {
            dt = std::max(
                dt, std::min(next->GetEndTime() - t,
                             phoneme->GetEndTime() - phoneme->GetStartTime()));
          }
        }
      }
    }
...
void C_BaseFlex::ProcessVisemes(Emphasized_Phoneme *classes) {
  ...
  for (int source = 0; source < MouthInfo().GetNumVoiceSources(); source++) {
    ...
    if (!vd || vd->ShouldIgnorePhonemes()) continue;
    ...
    /*
    // Tracker 20534:  Probably not needed any more with the async sound stuff
    that
    //  we now have (we don't have a disk i/o hitch on startup which might have
    been
    ...
    */
    ...
    AddVisemesForSentence(classes, emphasis_intensity, sentence, t, dt,
                          juststarted);
  }
}
...
void C_BaseFlex::GetToolRecordingState(KeyValues *msg) {
  ...
  for (i = LocalFlexController_t(0); i < hdr->numflexcontrollers(); i++) {
    ...
    g_flexweight[pflex->localToGlobal] =
        g_flexweight[pflex->localToGlobal] * (pflex->max - pflex->min) +
        pflex->min;
  }
  ...
  if (m_iEyeUpdown != -1 && m_iEyeRightleft != -1) {
    ...
    mstudioflexcontroller_t *flexrightleft =
        hdr->pFlexcontroller(m_iEyeRightleft);
    ...
    if (flexupdown->localToGlobal != -1 && flexrightleft->localToGlobal != -1) {
      ...
      if (flexrightleft->min != flexrightleft->max) {
        rightleft = RemapVal(rightleft, flexrightleft->min, flexrightleft->max,
                             0.0f, 1.0f);
      }
  ...
  for (i = LocalFlexController_t(0); i < hdr->numflexcontrollers(); i++) {
    ...
    if (pflex->max != pflex->min) {
      g_flexweight[pflex->localToGlobal] =
          (g_flexweight[pflex->localToGlobal] - pflex->min) /
          (pflex->max - pflex->min);
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void C_BaseFlex::SetupWeights(const matrix3x4_t *pBoneToWorld,
  ...
  // get the networked flexweights and convert them from 0..1 to real dynamic
  // range
  for (i = LocalFlexController_t(0); i < hdr->numflexcontrollers(); i++) {
    mstudioflexcontroller_t *pflex = hdr->pFlexcontroller(i);
  ...
  float flOOBlinkDuration =
      (flBlinkDuration > 0) ? 1.0f / flBlinkDuration : 0.0f;
  ...
  if (t > 0) {
    ...
    if (t > 0.0f && t < 1.0f) {
      ...
      g_flexweight[m_iBlink] =
          std::clamp(g_flexweight[m_iBlink] + t, 0.0f, 1.0f);
    }
  }
  ...
  if (pFlexDelayedWeights) {
    ...
    for (i = LocalFlexController_t(0); i < hdr->numflexdesc(); i++) {
      m_flFlexDelayedWeight[i] =
          m_flFlexDelayedWeight[i] * d + pFlexWeights[i] * (1 - d);
      pFlexDelayedWeights[i] = m_flFlexDelayedWeight[i];
    }
    // debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ),
    // i-hdr->numflexcontrollers, 0, "%.3f", d );
  }
  ...
  {
          debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ),
  i-hdr->numflexcontrollers, 0, "%2d:%s : %3.2f", i, hdr->pFlexdesc( i
  )->pszFACS(), pFlexWeights[i] );
  }
  */
  ...
  for (i = 0; i < g_numflexcontrollers; i++)
  {
          ...
          debugoverlay->AddTextOverlay( GetAbsOrigin() + Vector( 0, 0, 64 ), -i,
  0, "%s %3.2f", g_flexcontroller[i], g_flexweight[j] );
  }
  */
}
...
const flexsetting_t *C_BaseFlex::FindNamedSetting(
    const flexsettinghdr_t *pSettinghdr, const char *expr) {
  int i;
...
bool C_BaseFlex::ClearSceneEvent(CSceneEventInfo *info, bool fastKill,
                                 bool canceled) {
  Assert(info);
  Assert(info->m_pScene);
  Assert(info->m_pEvent);
...
//			duration -
//-----------------------------------------------------------------------------
void C_BaseFlex::AddSceneEvent(CChoreoScene *scene, CChoreoEvent *event,
                               CBaseEntity *pTarget, bool bClientSide) {
  if (!scene || !event) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_BaseFlex::StartSceneEvent(CSceneEventInfo *info, CChoreoScene *scene,
  ...
  switch (event->GetType()) {
    ...
    case CChoreoEvent::SEQUENCE:
      if (info->m_bClientSide) {
        return RequestStartSequenceSceneEvent(info, scene, event, actor,
                                              pTarget);
      }
      break;
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_BaseFlex::RequestStartSequenceSceneEvent(CSceneEventInfo *info,
                                                CChoreoScene *scene,
                                                CChoreoEvent *event,
                                                CChoreoActor *actor,
                                                CBaseEntity *pTarget) {
...
//			expression -
//-----------------------------------------------------------------------------
void C_BaseFlex::RemoveSceneEvent(CChoreoScene *scene, CChoreoEvent *event,
                                  bool fastKill) {
  Assert(event);
...
//-----------------------------------------------------------------------------
bool C_BaseFlex::CheckSceneEvent(float currenttime, CChoreoScene *scene,
                                 CChoreoEvent *event) {
  for (int i = 0; i < m_SceneEvents.Count(); i++) {
    CSceneEventInfo *info = &m_SceneEvents[i];
...
bool C_BaseFlex::CheckSceneEventCompletion(CSceneEventInfo *info,
                                           float currenttime,
                                           CChoreoScene *scene,
                                           CChoreoEvent *event) {
  return true;
...
//-----------------------------------------------------------------------------
bool C_BaseFlex::ProcessFlexAnimationSceneEvent(CSceneEventInfo *info,
                                                CChoreoScene *scene,
                                                CChoreoEvent *event) {
...
bool C_BaseFlex::ProcessFlexSettingSceneEvent(CSceneEventInfo *info,
                                              CChoreoScene *scene,
                                              CChoreoEvent *event) {
  ...
  if (scenefile && name) {
    ...
    const flexsettinghdr_t *pExpHdr =
        (const flexsettinghdr_t *)g_FlexSceneFileManager.FindSceneFile(
            this, scenefile, true);
    if (pExpHdr) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool C_BaseFlex::FlexSettingLessFunc(const FS_LocalToGlobal_t &lhs,
...
// Output : int
//-----------------------------------------------------------------------------
int C_BaseFlex::FlexControllerLocalToGlobal(const flexsettinghdr_t *pSettinghdr,
                                            int key) {
  FS_LocalToGlobal_t entry(pSettinghdr);
...
void C_BaseFlex::AddFlexSetting(const char *expr, float scale,
  ...
  for (i = 0; i < truecount; i++, pWeights++) {
    ...
    g_flexweight[index] =
        g_flexweight[index] * (1.0f - s) + pWeights->weight * s;
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
bool C_BaseFlex::ProcessSceneEvent(bool bFlexEvents, CSceneEventInfo *info,
...
//			*parameters -
//-----------------------------------------------------------------------------
bool C_BaseFlex::ProcessSequenceSceneEvent(CSceneEventInfo *info,
                                           CChoreoScene *scene,
                                           CChoreoEvent *event) {
    ...
    for (int i = 0; i < event->GetNumFlexAnimationTracks(); i++) {
      ...
      if (track->IsComboType()) {
        ...
        track->SetFlexControllerIndex(
            std::max(FindFlexController(name), LocalFlexController_t(0)), 0, 0);
        ...
        track->SetFlexControllerIndex(
            std::max(FindFlexController(name), LocalFlexController_t(0)), 0, 1);
      } else {
        track->SetFlexControllerIndex(
            std::max(FindFlexController((char *)track->GetFlexControllerName()),
                     LocalFlexController_t(0)),
            0);
      }
    }
