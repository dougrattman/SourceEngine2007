...
ConVar r_DrawParticles("r_drawparticles", "1", FCVAR_CHEAT,
                       "Enable/disable particle rendering");
static ConVar particle_simulateoverflow("particle_simulateoverflow", "0",
                                        FCVAR_CHEAT,
                                        "Used for stress-testing particle "
                                        "systems. Randomly denies creation of "
                                        "particles.");
ConVar cl_particleeffect_aabb_buffer("cl_particleeffect_aabb_buffer", "2",
                                     FCVAR_CHEAT,
                                     "Add this amount to a particle effect's "
                                     "bbox in the leaf system so if it's "
                                     "growing slowly, it won't have to be "
                                     "reinserted as often.");
ConVar cl_particles_show_bbox("cl_particles_show_bbox", "0", FCVAR_CHEAT);

#define BUCKET_SORT_EVERY_N \
  8  // It does a bucket sort for each material approximately every N times.
#define BBOX_UPDATE_EVERY_N \
  8  // It does a full bbox update (checks all particles instead of every eighth
     // one).
...
CParticleSubTextureGroup::CParticleSubTextureGroup() { m_pPageMaterial = NULL; }
...
CParticleEffectBinding::~CParticleEffectBinding() {
  if (m_pParticleMgr) m_pParticleMgr->RemoveEffect(this);
...
bool CParticleEffectBinding::IsTransparent(void) { return true; }

inline void CParticleEffectBinding::StartDrawMaterialParticles(
    CEffectMaterial *pMaterial, float flTimeDelta, IMesh *&pMesh,
    CMeshBuilder &builder, ParticleDraw &particleDraw, bool bWireframe) {
  CMatRenderContextPtr pRenderContext(m_pParticleMgr->m_pMaterialSystem);

  // Setup the ParticleDraw and bind the material.
  if (bWireframe) {
    IMaterial *pMaterial = m_pParticleMgr->m_pMaterialSystem->FindMaterial(
        "debug/debugparticlewireframe", TEXTURE_GROUP_OTHER);
  ...
  particleDraw.Init(&builder, pMaterial->m_pGroup->m_pPageMaterial,
                    flTimeDelta);
}

void CParticleEffectBinding::BBoxCalcStart(Vector &bbMin, Vector &bbMax) {
  if (!GetAutoUpdateBBox()) return;
...
void CParticleEffectBinding::BBoxCalcEnd(bool bboxSet, Vector &bbMin,
                                         Vector &bbMax) {
  if (!GetAutoUpdateBBox()) return;
  ...
  if (m_bLocalSpaceTransformIdentity) {
    ...
    TransformAABB(m_LocalSpaceTransform.As3x4(), bbMin, bbMax, bbMinWorld,
                  bbMaxWorld);
  }

  // If there were ANY particles in the system, then we've got a valid bbox
  // here. Otherwise, we don't have anything, so leave m_Min and m_Max at the
  // sort origin.
  if (bboxSet) {
    m_Min = bbMinWorld;
    m_Max = bbMaxWorld;
...
int CParticleEffectBinding::DrawModel(int flags) {
  VPROF_BUDGET("CParticleEffectBinding::DrawModel",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);
#ifndef PARTICLEPROTOTYPE_APP
  if (!r_DrawParticles.GetInt()) return 0;
#endif
  ...
  if (!g_pClientMode->ShouldDrawParticles()) return 0;

  // Avoid drawing particles while building depth textures. Perf win.
  // At the very least, we absolutely should not do refraction updates below. So
  // if this gets removed, be sure to wrap the refract/screen texture updates.
  if (flags & STUDIO_SHADOWDEPTHTEXTURE) return 0;

  SetDrawn(true);

  // Don't do anything if there are no particles.
  if (!m_nActiveParticles) return 1;
  ...
  // This does an incremental bubble sort. It only does one pass every frame,
  // and it will shuffle unsorted particles one step towards where they should
  // be.
  bool bWireframe = false;
  FOR_EACH_LL(m_Materials, iMaterial) {
    CEffectMaterial *pMaterial = m_Materials[iMaterial];

    if (pMaterial->m_pGroup->m_pPageMaterial &&
        pMaterial->m_pGroup->m_pPageMaterial
            ->NeedsPowerOfTwoFrameBufferTexture()) {
      UpdateRefractTexture();
    }

    if (pMaterial->m_pGroup->m_pPageMaterial &&
        pMaterial->m_pGroup->m_pPageMaterial->NeedsFullFrameBufferTexture()) {
      UpdateScreenEffectTexture();
    }

    DrawMaterialParticles(bBucketSort, pMaterial, flFrameTime, bWireframe);
  }
...
PMaterialHandle CParticleEffectBinding::FindOrAddMaterial(
...
Particle *CParticleEffectBinding::AddParticle(int sizeInBytes,
                                              PMaterialHandle hMaterial) {
  m_pParticleMgr->RepairPMaterial(hMaterial);  // HACKHACK: Remove this when we
                                               // can stop leaking handles from
                                               // level to level.
...
void CParticleEffectBinding::SetBBox(const Vector &bbMin, const Vector &bbMax,
                                     bool bDisableAutoUpdate) {
  m_Min = bbMin;
  m_Max = bbMax;

  if (bDisableAutoUpdate) SetAutoUpdateBBox(false);
}
...
void CParticleEffectBinding::SetLocalSpaceTransform(
...
void CParticleEffectBinding::DetectChanges() {
  ...
  if (m_Min.x < m_LastMin.x || m_Min.y < m_LastMin.y || m_Min.z < m_LastMin.z ||

      m_Min.x > (m_LastMin.x + flExtraBuffer) ||
      m_Min.y > (m_LastMin.y + flExtraBuffer) ||
      m_Min.z > (m_LastMin.z + flExtraBuffer) ||

      m_Max.x > m_LastMax.x || m_Max.y > m_LastMax.y || m_Max.z > m_LastMax.z ||
...
void CParticleEffectBinding::GrowBBoxFromParticlePositions(
    CEffectMaterial *pMaterial, bool &bboxSet, Vector &bbMin, Vector &bbMax) {
  // If its bbox is manually set, don't bother updating it here.
  if (!GetAutoUpdateBBox()) return;

  for (Particle *pCur = pMaterial->m_Particles.m_pNext;
       pCur != &pMaterial->m_Particles; pCur = pCur->m_pNext) {
    // Update bounding box
...
void CParticleEffectBinding::SimulateParticles(float flTimeDelta) {
  if (!m_pSim->ShouldSimulate()) return;
  ...
  if (GetFlag(FLAGS_NEW_PARTICLE_SYSTEM)) {
    ...
    // slow the expensive update operation for particle systems that use
    // auto-update-bbox auto update the bbox after N frames then randomly 1/N or
    // after 2*N frames
    bool bFullBBoxUpdate = false;
    ++m_UpdateBBoxCounter;
    if ((m_UpdateBBoxCounter >= BBOX_UPDATE_EVERY_N &&
         random->RandomInt(0, BBOX_UPDATE_EVERY_N) == 0) ||
...
void CParticleEffectBinding::SetDrawThruLeafSystem(int bDraw) {
  if (bDraw) {
    // If SetDrawBeforeViewModel was called, then they shouldn't be telling it
    // to draw through the leaf system too.
...
void CParticleEffectBinding::SetDrawBeforeViewModel(int bDraw) {
  // Don't draw through the leaf system if they want it to specifically draw
  // before the view model.
  if (bDraw) m_Flags &= ~FLAGS_DRAW_THRU_LEAF_SYSTEM;
...
int CParticleEffectBinding::GetActiveParticleList(int nCount,
  ...
  FOR_EACH_LL(m_Materials, i) {
    ...
    for (; pParticle != &pMaterial->m_Particles;
         pParticle = pParticle->m_pNext) {
      ppParticleList[nCurrCount] = pParticle;
      if (++nCurrCount == nCount) return nCurrCount;
    }
  }
...
int CParticleEffectBinding::DrawMaterialParticles(bool bBucketSort,
  ...
  IMesh *pMesh = NULL;
  StartDrawMaterialParticles(pMaterial, flTimeDelta, pMesh, builder,
                             particleDraw, bWireframe);

  if (m_nActiveParticles > MAX_TOTAL_PARTICLES)
    Error(
        "CParticleEffectBinding::DrawMaterialParticles: too many particles (%d "
        "should be less than %d)",
        m_nActiveParticles, MAX_TOTAL_PARTICLES);
  ...
  if (bBucketSort) {
    DoBucketSort(pMaterial, renderIterator.m_zCoords, renderIterator.m_nZCoords,
                 renderIterator.m_MinZ, renderIterator.m_MaxZ);
  }
...
void CParticleEffectBinding::RenderStart(VMatrix &tempModel,
                                         VMatrix &tempView) {
  ...
  if (GetAutoApplyLocalTransform() && !m_bLocalSpaceTransformIdentity) {
    m_pParticleMgr->m_mModelView =
        m_pParticleMgr->m_mModelView * m_LocalSpaceTransform;
  }
...
void CParticleEffectBinding::DoBucketSort(CEffectMaterial *pMaterial,
                                          float *zCoords, int nZCoords,
...
#define NUM_BUCKETS 32
  ...
  int iCurParticle = 0;
  Particle *pNext, *pCur;
  for (pCur = pMaterial->m_Particles.m_pNext; pCur != &pMaterial->m_Particles;
       pCur = pNext) {
    pNext = pCur->m_pNext;
    if (iCurParticle >= nZCoords) break;
...
void CParticleEffectBinding::Term() {
  ...
  FOR_EACH_LL(m_Materials, iMaterial) {
    ...
    Particle *pNext = NULL;
    for (Particle *pCur = pMaterial->m_Particles.m_pNext;
         pCur != &pMaterial->m_Particles; pCur = pNext) {
      pNext = pCur->m_pNext;
...
CEffectMaterial *CParticleEffectBinding::GetEffectMaterial(
    CParticleSubTexture *pSubTexture) {
  // Hash the IMaterial pointer.
  unsigned long index =
      (((unsigned long)pSubTexture->m_pGroup) >> 6) % EFFECT_MATERIAL_HASH_SIZE;
  for (CEffectMaterial *pCur = m_EffectMaterialHash[index]; pCur;
       pCur = pCur->m_pHashedNext) {
    if (pCur->m_pGroup == pSubTexture->m_pGroup) return pCur;
  }
...
CParticleMgr::CParticleMgr() {
  ...
  m_DefaultInvalidSubTexture.m_pGroup =
      &m_DefaultInvalidSubTexture.m_DefaultGroup;
  m_DefaultInvalidSubTexture.m_pMaterial = NULL;
  m_DefaultInvalidSubTexture.m_tCoordMins[0] =
      m_DefaultInvalidSubTexture.m_tCoordMins[1] = 0;
  m_DefaultInvalidSubTexture.m_tCoordMaxs[0] =
      m_DefaultInvalidSubTexture.m_tCoordMaxs[1] = 1;
...
CParticleMgr::~CParticleMgr() { Term(); }
...
void CParticleMgr::Term() {
  ...
  for (int i = m_SubTextures.First(); i != m_SubTextures.InvalidIndex();
       i = m_SubTextures.Next(i)) {
...
Particle *CParticleMgr::AllocParticle(int size) {
  ...
  if (m_nCurrentParticlesAllocated >= MAX_TOTAL_PARTICLES) return NULL;

  Particle *pRet = (Particle *)malloc(size);
  if (pRet) ++m_nCurrentParticlesAllocated;
...
void CParticleMgr::FreeParticle(Particle *pParticle) {
  Assert(m_nCurrentParticlesAllocated > 0);
  if (pParticle) --m_nCurrentParticlesAllocated;
...
void CParticleMgr::RegisterEffect(const char *pEffectType,
                                  CreateParticleEffectFN func) {
#ifdef _DEBUG
...
void CParticleMgr::AddEffect(CNewParticleEffect *pEffect) {
  ...
  if (pEffect->IsValid() && pEffect->m_pDef->IsViewModelEffect()) {
    ClientLeafSystem()->SetRenderGroup(pEffect->RenderHandle(),
                                       RENDER_GROUP_VIEW_MODEL_TRANSLUCENT);
  }
}

bool CParticleMgr::AddEffect(CParticleEffectBinding *pEffect,
                             IParticleEffect *pSim) {
#ifdef _DEBUG
...
void CParticleMgr::RemoveEffect(CParticleEffectBinding *pEffect) {
  ...
  // destructor.
  if (pEffect->GetRemovalInProgressFlag()) return;
  pEffect->SetRemovalInProgressFlag();

  // Don't call RemoveEffect while inside an IParticleEffect's Update()
  // function. Return false from the Update function instead.
...
void CParticleMgr::RemoveAllNewEffects() {
  ...
  for (CNewParticleEffect *pNewEffect = m_NewEffects.m_pHead; pNewEffect;) {
    ...
    // see it any entitiy has a particle prop pointing at this one. this loop
    // through all entities shouldn't be important perf-wise because it only
    // happens on reload
...
void CParticleMgr::RemoveAllEffects() {
  ...
  // HACKHACK: commented out because we need to keep leaking handles until every
  // piece of code that grabs one ditches it at level end
  // m_SubTextures.PurgeAndDeleteElements();
...
void CParticleMgr::IncrementFrameCode() {
  ...
  if (m_FrameCode == 0) {
    ...
    FOR_EACH_LL(m_Effects, i) { m_Effects[i]->m_FrameCode = 0; }
...
int GetParticlePerformance() {
  if (!g_nNumUSSpentSimulatingParticles) return 0;
...
void CParticleMgr::PostRender() {
  ...
  // Simulate all effects that weren't drawn (if they have their 'always
  // simulate' flag set).
  FOR_EACH_LL(m_Effects, i) {
...
void CParticleMgr::DrawBeforeViewModelEffects() {
  FOR_EACH_LL(m_Effects, i) {
    ...
    if (pEffect->GetFlag(
            CParticleEffectBinding::FLAGS_DRAW_BEFORE_VIEW_MODEL)) {
...
void BeginSimulateParticles(void) { g_flStartSimTime = Plat_FloatTime(); }

static ConVar r_particle_sim_spike_threshold_ms(
...
void EndSimulateParticles(void) {
  ...
  g_pParticleSystemMgr->CommitProfileInformation(
      flETime > .001 * r_particle_sim_spike_threshold_ms.GetInt());
}
...
static void ProcessPSystem(CNewParticleEffect *&pNewEffect) {
  ...
  if (bFirstUpdate) {
    // If the effect already disabled auto-updating of the bbox, then it should
    // have set the bbox by now and we can ignore this responsibility here.
    if (!pNewEffect->GetAutoUpdateBBox() ||
        pNewEffect->RecalculateBoundingBox()) {
      pNewEffect->SetNeedsBBoxUpdate(false);
    }
  }

  // This flag will get set to true if the effect is drawn through the leaf
  // system.
...
int CParticleMgr::ComputeParticleDefScreenArea(
    int nInfoCount, RetireInfo_t *pInfo, float *pTotalArea,
    CParticleSystemDefinition *pDef, const CViewSetup &view,
    const VMatrix &worldToPixels, float flFocalDist) {
  int nCollection = 0;
  ...
  *pTotalArea = 0.0f;
  ...
  for (; pCollection;
    ...
    CNewParticleEffect *pEffect =
        static_cast<CNewParticleEffect *>(pCollection);
    if (!pEffect->ShouldPerformCullCheck()) continue;
    ...
    vecCenter =
        pCollection->GetControlPointAtCurrentTime(pDef->GetCullControlPoint());

    Vector3DMultiplyPositionProjective(worldToPixels, vecCenter,
                                       vecScreenCenter);
    float lSqr = vecCenter.DistToSqr(view.origin);

    float flProjRadius =
        (lSqr > flCullRadiusSqr)
            ? 0.5f * flFocalDist * flCullRadius / sqrt(lSqr - flCullRadiusSqr)
            : 1.0f;
    flProjRadius *= view.width;

    float flMinX = std::max((float)view.x, vecScreenCenter.x - flProjRadius);
    float flMaxX =
        std::min((float)view.x + view.width, vecScreenCenter.x + flProjRadius);

    float flMinY = std::max((float)view.y, vecScreenCenter.y - flProjRadius);
    float flMaxY =
        std::min((float)view.y + view.height, vecScreenCenter.y + flProjRadius);
...
bool CParticleMgr::RetireParticleCollections(CParticleSystemDefinition *pDef,
  ...
  if ((!pReplacementDef || !pReplacementDef[0]) && (nCount <= 1)) return false;

  // Quicksort the retirement info
  qsort(pInfo, nCount, sizeof(RetireInfo_t), RetireSort);

  for (int i = 0; i < nCount; ++i) {
    if (flScreenArea <= flMaxTotalArea) break;

    // We can only replace stuff that's being emitted this frame
    if (!pInfo[i].m_bFirstFrame) continue;

    CNewParticleEffect *pRetireEffect =
        static_cast<CNewParticleEffect *>(pInfo[i].m_pCollection);
    CNewParticleEffect *pNewEffect =
        pRetireEffect->ReplaceWith(pReplacementDef);
    if (pNewEffect) {
...
bool CParticleMgr::EarlyRetireParticleSystems(int nCount,
  ...
  if (pViewSetup->width == 0 || pViewSetup->height == 0) return false;

  float flMaxScreenArea = cl_particle_retire_cost.GetFloat() * 1000.0f;
  if (flMaxScreenArea == 0.0f) return false;

  int nDefCount = 0;
  CParticleSystemDefinition **ppDefs = (CParticleSystemDefinition **)stackalloc(
      nCount * sizeof(CParticleSystemDefinition *));
  for (int i = 0; i < nCount; ++i) {
    CParticleSystemDefinition *pDef = ppEffects[i]->m_pDef;

    // Skip stuff that doesn't have a cull radius set
    if (pDef->GetCullRadius() == 0.0f) continue;

    // Only perform the cull check on creation
    if (!ppEffects[i]->GetFirstFrameFlag()) continue;

    if (pDef->HasRetirementBeenChecked(gpGlobals->framecount)) continue;
  ...
  render->GetMatricesForView(*pViewSetup, &worldToView, &viewToProjection,
                             &worldToProjection, &worldToScreen);
  ...
  for (int i = 0; i < nDefCount; ++i) {
    ...
    int nActualCount =
        ComputeParticleDefScreenArea(nCount, pInfo, &flScreenArea, pDef,
                                     *pViewSetup, worldToScreen, flFocalDist);
    if (flScreenArea > flMaxScreenArea) {
      if (RetireParticleCollections(pDef, nActualCount, pInfo, flScreenArea,
                                    flMaxScreenArea)) {
...
void CParticleMgr::BuildParticleSimList(
    CUtlVector<CNewParticleEffect *> &list) {
  float flNow = g_pParticleSystemMgr->GetLastSimulationTime();
  for (CNewParticleEffect *pNewEffect = m_NewEffects.m_pHead; pNewEffect;
       pNewEffect = pNewEffect->m_pNext) {
    if (flNow >= pNewEffect->m_flNextSleepTime &&
        pNewEffect->m_nActiveParticles > 0)
      continue;
    if (pNewEffect->GetRemoveFlag()) continue;
    if (g_bMeasureParticlePerformance) {
...
static int CountChildParticleSystems(CParticleCollection *p) {
  int nCount = 1;
  for (CParticleCollection *pChild = p->m_Children.m_pHead; pChild;
...
void CParticleMgr::SpewActiveParticleSystems() {
  ...
  for (CNewParticleEffect *pNewEffect = m_NewEffects.m_pHead; pNewEffect;
    ...
    if (++histo[pNewEffect->GetName()].m_nCount == 1) {
      histo[pNewEffect->GetName()].m_nChildCount =
          CountChildParticleSystems(pNewEffect);
    }
  }
  ...
  for (int i = 0; i < nCount; ++i) {
    Msg("%30s\t\t%d\t\t%d\n", histo.String(i), histo[i].m_nCount,
        histo[i].m_nChildCount);
  }
}
...
void CParticleMgr::UpdateAllEffects(float flTimeDelta) {
  ...
  if (flTimeDelta > 0.1f) flTimeDelta = 0.1f;
  ...
  FOR_EACH_LL(m_Effects, iEffect) {
    ...
    if (pEffect->GetRemoveFlag()) continue;

    // If this is a new effect, then update its bbox so it goes in the
    // right leaves (if it has particles).
    int bFirstUpdate = pEffect->GetNeedsBBoxUpdate();
    if (bFirstUpdate) {
      // If the effect already disabled auto-updating of the bbox, then it
      // should have set the bbox by now and we can ignore this responsibility
      // here.
...
CParticleSubTextureGroup *CParticleMgr::FindOrAddSubTextureGroup(
...
PMaterialHandle CParticleMgr::GetPMaterial(const char *pMaterialName) {
  ...
  if (hMat == m_SubTextures.InvalidIndex()) {
    IMaterial *pIMaterial =
        m_pMaterialSystem->FindMaterial(pMaterialName, TEXTURE_GROUP_PARTICLE);
    if (pIMaterial) {
      pIMaterial->AddRef();
      ...
      if (pIMaterial->InMaterialPage() && pPageMaterial) {
        ...
        pSubTexture->m_tCoordMins[0] =
            (0 * flScale[0] + flOffset[0]) * pPageMaterial->GetMappingWidth();
        pSubTexture->m_tCoordMaxs[0] =
            (1 * flScale[0] + flOffset[0]) * pPageMaterial->GetMappingWidth();

        pSubTexture->m_tCoordMins[1] =
            (0 * flScale[1] + flOffset[1]) * pPageMaterial->GetMappingHeight();
        pSubTexture->m_tCoordMaxs[1] =
            (1 * flScale[1] + flOffset[1]) * pPageMaterial->GetMappingHeight();

        pSubTexture->m_pGroup = FindOrAddSubTextureGroup(pPageMaterial);
      } else {
        // Ok, this material isn't part of a group. Give it its own subtexture
        // group.
      ...
      pSubTexture->m_tCoordMins[0] = (pSubTexture->m_tCoordMins[0] + 0.5f) /
                                     pPageMaterial->GetMappingWidth();
      pSubTexture->m_tCoordMins[1] = (pSubTexture->m_tCoordMins[1] + 0.5f) /
                                     pPageMaterial->GetMappingHeight();
      pSubTexture->m_tCoordMaxs[0] = (pSubTexture->m_tCoordMaxs[0] - 0.5f) /
                                     pPageMaterial->GetMappingWidth();
      pSubTexture->m_tCoordMaxs[1] = (pSubTexture->m_tCoordMaxs[1] - 0.5f) /
                                     pPageMaterial->GetMappingHeight();
    ...
    RepairPMaterial(m_SubTextures[hMat]);  // HACKHACK: Remove this when we can
                                           // stop leaking handles from level to
                                           // level.
...
// HACKHACK: The old system would leak handles and materials until shutdown. The
// new system still needs to leak handles until every piece of code that grabs
// one ditches it at level end.  This function takes a leaked handle from a
// previous level and reacquires necessary materials.
void CParticleMgr::RepairPMaterial(PMaterialHandle hMaterial) {
  if (hMaterial->m_pMaterial != NULL) return;
...
IMaterial *CParticleMgr::PMaterialToIMaterial(PMaterialHandle hMaterial) {
  if (hMaterial) {
    RepairPMaterial(hMaterial);  // HACKHACK: Remove this when we can stop
                                 // leaking handles from level to level.
...
void CParticleMgr::SpewInfo(bool bDetail) {
  ...
  FOR_EACH_LL(m_Effects, i) {
    ...
    DevMsg("%3d: NumActive: %3d, AutoBBox: %3s \"%s\" \n", i,
           m_Effects[i]->m_nActiveParticles,
           m_Effects[i]->GetAutoUpdateBBox() ? "on" : "off", pEffectName);
  }
}
CON_COMMAND(cl_particles_dump_effects, "") { ParticleMgr()->SpewInfo(true); }
#endif
