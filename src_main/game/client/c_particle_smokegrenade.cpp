...
static Vector s_FadePlaneDirections[] = {Vector(1, 0, 0), Vector(-1, 0, 0),
                                         Vector(0, 1, 0), Vector(0, -1, 0),
                                         Vector(0, 0, 1), Vector(0, 0, -1)};
#define NUM_FADE_PLANES \
  (sizeof(s_FadePlaneDirections) / sizeof(s_FadePlaneDirections[0]))
...
class C_ParticleSmokeGrenade : public C_BaseParticleEntity,
 ...
 public:
  ...
  // Proxies.
 public:
  static void RecvProxy_CurrentStage(const CRecvProxyData *pData, void *pStruct,
                                     void *pOut);
 ...
 private:
  ...
  inline bool IsValidXYZCoords(int x, int y, int z) {
    return x >= 0 && y >= 0 && z >= 0 && x < m_xCount && y < m_yCount &&
           z < m_zCount;
  }

  inline Vector GetSmokeParticlePos(int x, int y, int z) {
    return m_SmokeBasePos +
           Vector(((float)x / (m_xCount - 1)) * m_SpacingRadius * 2 -
                      m_SpacingRadius,
                  ((float)y / (m_yCount - 1)) * m_SpacingRadius * 2 -
                      m_SpacingRadius,
                  ((float)z / (m_zCount - 1)) * m_SpacingRadius * 2 -
                      m_SpacingRadius);
  }
 ...
 private:
  ...
  SmokeParticleInfo m_SmokeParticleInfos[NUM_PARTICLES_PER_DIMENSION *
...
// Datatable..
IMPLEMENT_CLIENTCLASS_DT(C_ParticleSmokeGrenade, DT_ParticleSmokeGrenade,
                         ParticleSmokeGrenade)
  RecvPropTime(RECVINFO(m_flSpawnTime)),
      RecvPropFloat(RECVINFO(m_FadeStartTime)),
      RecvPropFloat(RECVINFO(m_FadeEndTime)),
      RecvPropInt(RECVINFO(m_CurrentStage), 0,
                  &C_ParticleSmokeGrenade::RecvProxy_CurrentStage),
END_RECV_TABLE()
...
static inline void InterpColor(unsigned char dest[4], unsigned char src1[4],
                               unsigned char src2[4], float percent) {
...
static inline void WorldTraceLine(const Vector &start, const Vector &end,
                                  int contentsMask, trace_t *trace) {
#if defined(PARTICLEPROTOTYPE_APP)
  trace->fraction = 1;
#else
    ...
    if (testDist < fadeEnd) {
      if (testDist < flCoreDistance) {
        ...
        EngineGetSmokeFogOverlayAlpha() +=
            (1 - (testDist - flCoreDistance) / (fadeEnd - flCoreDistance)) *
            m_FadeAlpha;
      }
    }
  }
...
inline void C_ParticleSmokeGrenade::UpdateParticleDuringTrade(
    int iParticle, float fTimeDelta) {
  ...
  if (pInfo < pOther) {
    ...
    Vector otherPos =
        GetSmokeParticlePos(otherX, otherY, otherZ) - m_SmokeBasePos;
    ...
    if (pInfo->m_TradeClock >= pInfo->m_TradeDuration) {
      ...
      // Ok, move them closer.
      float percent = (float)cos(pInfo->m_TradeClock * 2 * 1.57079632f /
                                 pInfo->m_TradeDuration);
      percent = percent * 0.5 + 0.5;

      pInfo->m_pParticle->m_FadeAlpha =
          pInfo->m_FadeAlpha +
          (pOther->m_FadeAlpha - pInfo->m_FadeAlpha) * (1 - percent);
      pOther->m_pParticle->m_FadeAlpha =
          pInfo->m_FadeAlpha +
          (pOther->m_FadeAlpha - pInfo->m_FadeAlpha) * percent;

      InterpColor(pInfo->m_pParticle->m_Color, pInfo->m_Color, pOther->m_Color,
                  1 - percent);
      InterpColor(pOther->m_pParticle->m_Color, pInfo->m_Color, pOther->m_Color,
                  percent);
...
void C_ParticleSmokeGrenade::UpdateParticleAndFindTrade(int iParticle,
                                                        float fTimeDelta) {
  SmokeParticleInfo *pInfo = &m_SmokeParticleInfos[iParticle];
      ...
      for (int zCount = 0; zCount < 3; zCount++) {
        ...
        if (IsValidXYZCoords(testX, testY, testZ)) {
          ...
          if (pOther->m_pParticle && pOther->m_TradeIndex == -1) {
            ...
            pInfo->m_TradeDuration =
                FRand(TRADE_DURATION_MIN, TRADE_DURATION_MAX);
...
void C_ParticleSmokeGrenade::Update(float fTimeDelta) {
  ...
  if (flLifetime < m_FadeStartTime) {
    ...
    float fadePercent =
        (flLifetime - m_FadeStartTime) / (m_FadeEndTime - m_FadeStartTime);
  ...
  // Update our bbox.
  Vector vMins = m_SmokeBasePos -
                 Vector(m_SpacingRadius, m_SpacingRadius, m_SpacingRadius);
  Vector vMaxs = m_SmokeBasePos +
                 Vector(m_SpacingRadius, m_SpacingRadius, m_SpacingRadius);
  ...
  if (m_CurrentStage == 1) {
    ...
    m_ExpandRadius =
        (m_SpacingRadius * 2) *
        (float)sin(m_ExpandTimeCounter * M_PI * 0.5 / SMOKESPHERE_EXPAND_TIME);
...
void C_ParticleSmokeGrenade::UpdateDynamicLightList(const Vector &vMins,
                                                    const Vector &vMaxs) {
  ...
  for (int i = 0; i < nLights; i++) {
    ...
    if (pIn->origin.x + pIn->radius <= vMins.x ||
      ...
      if ((pIn->color.r != 0 || pIn->color.g != 0 || pIn->color.b != 0) &&
          pIn->color.exponent != 0) {
        ColorRGBExp32ToVector(pIn->color, pOut->m_vColor);
        pOut->m_vColor /= 255.0f;
        pOut->m_flRadiusSqr = (pIn->radius + SMOKEPARTICLE_SIZE) *
                              (pIn->radius + SMOKEPARTICLE_SIZE);
...
inline void C_ParticleSmokeGrenade::ApplyDynamicLight(
    const Vector &vParticlePos, Vector &color) {
  if (m_nActiveLights) {
    for (int i = 0; i < m_nActiveLights; i++) {
      ...
      if (flDistSqr < pLight->m_flRadiusSqr) {
        color +=
            pLight->m_vColor * (1 - flDistSqr / pLight->m_flRadiusSqr) * 0.1f;
      }
    }

    // Rescale the color..
    float flMax = std::max(color.x, std::max(color.y, color.z));
    if (flMax > 1) {
      color /= flMax;
    }
  }
...
void C_ParticleSmokeGrenade::RenderParticles(
    CParticleRenderIterator *pIterator) {
  const SmokeGrenadeParticle *pParticle =
      (const SmokeGrenadeParticle *)pIterator->GetFirst();
  while (pParticle) {
    ...
    if (len > m_ExpandRadius) {
      ...
      // This smooths out the growing sphere. Rather than having particles
      // appear in one spot as the sphere expands, they stay at the borders.
      Vector renderPos;
      if (len > m_ExpandRadius * 0.5f) {
        renderPos =
            m_SmokeBasePos + (pParticle->m_Pos * (m_ExpandRadius * 0.5f)) / len;
      } else {
      ...
      Vector color = m_MinColor + (m_MaxColor - m_MinColor) *
                                      (pParticle->m_ColorInterp / 255.1f);
      ...
      RenderParticle_ColorSizeAngle(
          pIterator->GetParticleDraw(), tRenderPos, color,
...
void C_ParticleSmokeGrenade::SimulateParticles(
    CParticleSimulateIterator *pIterator) {
  SmokeGrenadeParticle *pParticle =
      (SmokeGrenadeParticle *)pIterator->GetFirst();
  while (pParticle) {
    pParticle->m_CurRotation +=
        pParticle->m_RotationSpeed * pIterator->GetTimeDelta();
...
void C_ParticleSmokeGrenade::GetParticlePosition(Particle *pParticle,
                                                 Vector &worldpos) {
  worldpos = pParticle->m_Pos + m_SmokeBasePos;
}

void C_ParticleSmokeGrenade::RecvProxy_CurrentStage(const CRecvProxyData *pData,
...
void C_ParticleSmokeGrenade::FillVolume() {
  ...
  m_SpacingRadius = (SMOKEGRENADE_PARTICLERADIUS - overlap) *
                    NUM_PARTICLES_PER_DIMENSION * 0.5f;
  ...
  for (int x = 0; x < m_xCount; x++) {
    vPos.x = m_SmokeBasePos.x +
             ((float)x * invNumPerDimX) * m_SpacingRadius * 2 - m_SpacingRadius;

    for (int y = 0; y < m_yCount; y++) {
      vPos.y = m_SmokeBasePos.y +
               ((float)y * invNumPerDimY) * m_SpacingRadius * 2 -
               m_SpacingRadius;

      for (int z = 0; z < m_zCount; z++) {
        vPos.z = m_SmokeBasePos.z +
                 ((float)z * invNumPerDimZ) * m_SpacingRadius * 2 -
                 m_SpacingRadius;
        ...
        if (SmokeParticleInfo *pInfo = GetSmokeParticleInfo(x, y, z)) {
          // MD 11/10/03: disabled this because we weren't getting coverage near
          // the ground. If we want it back in certain cases, we can make it a
          // flag.
          ...
          */
          {
            SmokeGrenadeParticle *pParticle =
                (SmokeGrenadeParticle *)m_ParticleEffect.AddParticle(
                    sizeof(SmokeGrenadeParticle),
                    m_MaterialHandles[rand() % NUM_MATERIAL_HANDLES]);

            if (pParticle) {
              pParticle->m_Pos =
                  vPos - m_SmokeBasePos;  // store its position in local space
              pParticle->m_ColorInterp =
                  (unsigned char)((rand() * 255) / RAND_MAX);
              pParticle->m_RotationSpeed =
                  FRand(-ROTATION_SPEED, ROTATION_SPEED);  // Rotation speed.
            ...
            // Cast some rays and if it's too close to anything, fade its alpha
            // down.
            pInfo->m_FadeAlpha = 1;
            ...
            {
                    trace_t trace;
                    WorldTraceLine(vPos, vPos + s_FadePlaneDirections[i] * 100,
            MASK_SOLID_BRUSHONLY, &trace); if(trace.fraction < 1.0f)
                    {
                            float dist = DotProduct(trace.plane.normal, vPos) -
            trace.plane.dist; if(dist < 0)
                            {
                                    pInfo->m_FadeAlpha = 0;
                            }
                            else if(dist < SMOKEPARTICLE_SIZE)
                            {
                                    float alphaScale = dist /
            SMOKEPARTICLE_SIZE; alphaScale *= alphaScale * alphaScale;
...
void C_ParticleSmokeGrenade::CleanupToolRecordingState(KeyValues *msg) {
  ...
  if (m_bVolumeFilled &&
    ...
    pEmitter->SetInt("count", NUM_PARTICLES_PER_DIMENSION *
                                  NUM_PARTICLES_PER_DIMENSION *
                                  NUM_PARTICLES_PER_DIMENSION);
    ...
    KeyValues *pPosition =
        pInitializers->FindKey("DmeVoxelPositionInitializer", true);
    ...
    Color c1(std::clamp(m_MinColor.x * 255.0f, 0.0f, 255.0f),
             std::clamp(m_MinColor.y * 255.0f, 0.0f, 255.0f),
             std::clamp(m_MinColor.z * 255.0f, 0.0f, 255.0f), 255.0f);
    Color c2(std::clamp(m_MaxColor.x * 255.0f, 0.0f, 255.0f),
             std::clamp(m_MaxColor.y * 255.0f, 0.0f, 255.0f),
             std::clamp(m_MaxColor.z * 255.0f, 0.0f, 255.0f), 255.0f);
    ...
    KeyValues *pAlphaCosineUpdater =
        pUpdaters->FindKey("DmeAlphaCosineUpdater", true);
    pAlphaCosineUpdater->SetFloat("duration", m_FadeEndTime - m_FadeStartTime);

    pUpdaters->FindKey("DmeColorDynamicLightUpdater", true);

    KeyValues *pSmokeGrenadeUpdater =
        pUpdaters->FindKey("DmeSmokeGrenadeUpdater", true);
