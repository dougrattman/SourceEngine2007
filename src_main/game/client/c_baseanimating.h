...
struct ClientModelRenderInfo_t : public ModelRenderInfo_t {
  // Added space for lighting origin override. Just allocated space, need to set
  // base pointer
  matrix3x4_t lightingOffset;

  // Added space for model to world matrix. Just allocated space, need to set
  // base pointer
...
#define INVALID_CLIENTSIDEANIMATION_LIST_HANDLE \
  (ClientSideAnimationListHandle_t) ~0
...
class C_BaseAnimating : public C_BaseEntity {
 public:
  ...
  void DoInternalDrawModel(ClientModelRenderInfo_t *pInfo,
                           DrawModelState_t *pState,
                           matrix3x4_t *pBoneToWorldArray = NULL);
  ...
  virtual void FireEvent(const Vector &origin, const QAngle &angles, int event,
                         const char *options);
  virtual void FireObsoleteEvent(const Vector &origin, const QAngle &angles,
  ...
  virtual void StandardBlendingRules(CStudioHdr *pStudioHdr, Vector pos[],
                                     Quaternion q[], float currentTime,
                                     int boneMask);
  void UnragdollBlend(CStudioHdr *hdr, Vector pos[], Quaternion q[],
                      float currentTime);

  void MaintainSequenceTransitions(CStudioHdr *hdr, float flCycle,
                                   float flPoseParameter[], Vector pos[],
  ...
  bool GetPoseParameterRange(int iPoseParameter, float &minValue,
                             float &maxValue);
  ...
  // bool solveIK(float a, float b, const Vector &Foot, const Vector &Knee1,
  // Vector &Knee2); void DebugIK( mstudioikchain_t *pikchain );
  ...
  // This can be used to force client side animation to be on. Only use if you
  // know what you're doing! Normally, the server entity should set this.
  ...
  bool InitAsClientRagdoll(const matrix3x4_t *pDeltaBones0,
  ...
  // All view model attachments origins are stretched so you can place entities
  // at them and they will match up with where the attachment winds up being
  // drawn on the view model, since the view models are drawn with a different
  // FOV.
  //
  // If you're drawing something inside of a view model's DrawModel() function,
  // then you want the original attachment origin instead of the adjusted one.
  // To get that, call this on the adjusted attachment origin.
  virtual void UncorrectViewModelAttachment(Vector &vOrigin) {}

  // Call this if SetupBones() has already been called this frame but you need
  // to move the entity and rerender.
  void InvalidateBoneCache();
  bool IsBoneCacheValid() const;  // Returns true if the bone cache is
                                  // considered good for this frame.
  ...
  // Use a distinct "tag" when pushing/popping - asserts when push/pop tags do
  // not match.
  ...
  static void PushAllowBoneAccess(bool bAllowForNormalModels,
  ...
  // Invalidate bone caches so all SetupBones() calls force bone transforms to
  // be regenerated.
  ...
  void DisableMuzzleFlash();  // Turn off the muzzle flash (ie: signal that we
                              // handled the server's event).
  virtual void DoMuzzleFlash();  // Force a muzzle flash event. Note: this only
                                 // QUEUES an event, so
  ...
  // Sometimes the server wants to update the client's cycle to get the two to
  // run in sync (for proper hit detection)
 ...
 protected:
  // View models scale their attachment positions to account for FOV. To get the
  // unmodified attachment position (like if you're rendering something else
  // during the view model's DrawModel call), use
  // TransformViewModelAttachmentToWorld.
 ...
 private:
  // This method should return true if the bones have changed + SetupBones needs
  // to be called
 ...
 private:
  ...
  bool m_bSequenceFinished;  // flag set when StudioAdvanceFrame moves across a
                             // frame boundry
 ...
 private:
  int m_nOldSequence;
  CBoneMergeCache *m_pBoneMergeCache;  // This caches the strcmp lookups that it
                                       // has to do when merg

  CUtlVector<matrix3x4_t>
      m_CachedBoneData;  // never access this directly. Use m_BoneAccessor.
  ...
  // These are compared against each other to determine if the entity should
  // muzzle flash.
...
inline float C_BaseAnimating::GetPlaybackRate() { return m_flPlaybackRate; }
...
#ifdef _DEBUG
  // GetModelPtr() is often called before OnNewModel() so go ahead and set it up
  // first chance.
...
// TODO(d.rattman): move these to somewhere that makes sense
