...
class C_EnvParticleScript : public C_BaseAnimating, public IParticleEffect {
 ...
 public:
  ...
  // But we have to return true, unlike other particle systems, for the
  // animation events to work
...
// Datatable
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_DT(C_EnvParticleScript, DT_EnvParticleScript,
                         CEnvParticleScript)
...
// Creates, destroys particles attached to an attachment
//-----------------------------------------------------------------------------
void C_EnvParticleScript::CreateParticle(const char *pAttachmentName,
  ...
  ParticleScriptParticle_t *pParticle =
      (ParticleScriptParticle_t *)m_ParticleEffect.AddParticle(
          sizeof(ParticleScriptParticle_t), hMat);
...
void C_EnvParticleScript::DestroyAllParticles(const char *pAttachmentName) {
  ...
  int nActualCount =
      m_ParticleEffect.GetActiveParticleList(nCount, ppParticles);
  Assert(nActualCount == nCount);

  for (int i = 0; i < nActualCount; ++i) {
    ParticleScriptParticle_t *pParticle =
        (ParticleScriptParticle_t *)ppParticles[i];
...
//-----------------------------------------------------------------------------
void C_EnvParticleScript::FireEvent(const Vector &origin, const QAngle &angles,
  ...
  switch (event) {
    case CL_EVENT_SPRITEGROUP_CREATE: {
      ...
      int nArgs = sscanf_s(options, "%255s %255s", pAttachmentName,
                           SOURCE_ARRAYSIZE(pAttachmentName), pSpriteName,
                           SOURCE_ARRAYSIZE(pSpriteName));
      if (nArgs == 2) {
    ...
    case CL_EVENT_SPRITEGROUP_DESTROY: {
      char pAttachmentName[256];
      int nArgs = sscanf_s(options, "%255s", pAttachmentName,
                           SOURCE_ARRAYSIZE(pAttachmentName));
      if (nArgs == 1) {
...
void C_EnvParticleScript::RenderParticles(CParticleRenderIterator *pIterator) {
  const ParticleScriptParticle_t *pParticle =
      (const ParticleScriptParticle_t *)pIterator->GetFirst();
  while (pParticle) {
    Vector vecRenderPos;
    TransformParticle(ParticleMgr()->GetModelView(), pParticle->m_Pos,
                      vecRenderPos);
    float sortKey = vecRenderPos.z;

    Vector color(1, 1, 1);
    RenderParticle_ColorSize(pIterator->GetParticleDraw(), vecRenderPos, color,
                             1.0f, pParticle->m_flSize);

    pParticle = (const ParticleScriptParticle_t *)pIterator->GetNext(sortKey);
  }
}

void C_EnvParticleScript::SimulateParticles(
    CParticleSimulateIterator *pIterator) {
  ParticleScriptParticle_t *pParticle =
      (ParticleScriptParticle_t *)pIterator->GetFirst();
  while (pParticle) {
