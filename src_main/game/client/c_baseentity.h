...
#define ENTCLIENTFLAG_GETTINGSHADOWRENDERBOUNDS \
  0x0001  // Tells us if we're getting the real ent render bounds or the shadow
          // render bounds.
...
class C_BaseEntity : public IClientEntity {
  ...
  friend void cc_cl_interp_all_changed(IConVar *pConVar, const char *pOldString,
                                       float flOldValue);
 ...
 public:
  ...
  // This works for client-only entities and returns the GetEntryIndex() of the
  // entity's handle, so the sound system can get an IClientEntity from it.
 ...
 private:
  ...
  // Called after restoring data into prediction slots. This function is used in
  // place of proxies on the variables, so if some variable like m_nModelIndex
  // needs to update other state (like the model pointer), it is done here.
 ...
 public:
  ...
  // NOTE: These use the collision OBB to compute a reasonable center point for
  // the entity
  ...
  virtual bool InSameTeam(C_BaseEntity *pEntity);  // Returns true if the
                                                   // specified entity is on the
                                                   // same team as this one
 ...
 public:
  ...
  // It dispatches events like OnDataChanged(), and calls the legacy function
  // AddEntity().
  virtual void Simulate();

  // This event is triggered during the simulation phase if an entity's data has
  // changed. It is better to hook this instead of PostDataUpdate() because in
  // PostDataUpdate(), server entity origins are incorrect and attachment points
  // can't be used.
  ...
  // but it doesn't set animtime. In that case, the client returns true here so
  // it copies the server time to animtime in OnDataChanged and the position
  // history is correct for interpolation.
  ...
  // Set the next think time. Pass in CLIENT_THINK_ALWAYS to have Think() called
  // each frame.
 ...
 public:
  ...
  // Sorry folks, here lies TF2-specific stuff that really has no other place to
  // go
 ...
 public:
  ...
  // Destroys the shadow; causes its type to be recomputed if the entity doesn't
  // go away immediately.
  void DestroyShadow();
 ...
 private:
  // Set by tools if this entity should route "info" to various tools listening
  // to HTOOLENTITIES
 ...
 public:
  ...
  bool m_bEnableRenderingClipPlane;  // true to use the custom clip plane when
                                     // drawing
  float *GetRenderClipPlane(void);  // Rendering clip plane, should be 4 floats,
                                    // return value of NULL indicates a disabled
                                    // render clip plane

 protected:
  void AddToInterpolationList();
  void RemoveFromInterpolationList();
  unsigned short m_InterpolationListEntry;  // Entry into g_InterpolationList
                                            // (or
                                            // g_InterpolationList.InvalidIndex
                                            // if not in the list).
...
inline void CBaseEntity::SetViewOffset(const Vector &v) { m_vecViewOffset = v; }
