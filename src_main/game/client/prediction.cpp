...
ConVar cl_predictweapons("cl_predictweapons", "1", FCVAR_USERINFO,
                         "Perform client side prediction of weapon effects.");
ConVar cl_lagcompensation(
    "cl_lagcompensation", "1", FCVAR_USERINFO,
...
static ConVar cl_predictionlist("cl_predictionlist", "0", FCVAR_CHEAT,
                                "Show which entities are predicting\n");

static ConVar cl_predictionentitydump("cl_pdump", "-1", FCVAR_CHEAT,
                                      "Dump info about this entity to screen.");
static ConVar cl_predictionentitydumpbyclass(
    "cl_pclass", "", FCVAR_CHEAT, "Dump entity by prediction classname.");
static ConVar cl_pred_optimize("cl_pred_optimize", "2", 0,
                               "Optimize for not copying data if didn't "
                               "receive a network update (1), and also for not "
                               "repredicting if there were no errors (2).");
...
void CPrediction::CheckError(int commands_acknowledged) {
  ...
  typedescription_t *td =
      FindFieldByName("m_vecNetworkOrigin", player->GetPredDescMap());
  Assert(td);
  if (!td) return;

  Vector predicted_origin;

  memcpy((Vector *)&predicted_origin,
         (Vector *)((byte *)slot + td->fieldOffset[PC_DATA_PACKED]),
         sizeof(Vector));
  ...
  if (len > MAX_PREDICTION_ERROR) {
    ...
    if (len > MIN_PREDICTION_EPSILON) {
      ...
      if (cl_showerror.GetInt() >= 1) {
        ...
        engine->Con_NXPrintf(&np, "pred error %6.3f units (%6.3f %6.3f %6.3f)",
                             len, delta.x, delta.y, delta.z);
      }
    }
  }
#endif
...
void CPrediction::ShutdownPredictables(void) {
  ...
  if ((release_count > 0) || (shutdown_count > 0)) {
...
void CPrediction::PreEntityPacketReceived(int commands_acknowledged,
  ...
  // Don't screw up memory of current player from history buffers if not filling
  // in history buffers
  //  during prediction!!!
...
bool CPrediction::ShouldDumpEntity(C_BaseEntity *ent) {
  ...
  if (dump_entity != -1) {
    ...
    if (cl_predictionentitydumpbyclass.GetString()[0] == 0) return false;
  ...
  if (cl_predict->GetInt()) {
    ...
    for (i = 0; i < c; i++) {
      ...
      if (showlist) {
        char sz[32];
        if (ent->entindex() == -1) {
          Q_snprintf(sz, sizeof(sz), "handle %u",
                     (unsigned int)ent->GetClientHandle().ToInt());
        } else {
        ...
        if (showlist >= 2) {
          ...
          int intermediate_size =
              ent->GetIntermediateDataSize() * (MULTIPLAYER_BACKUP + 1);

          engine->Con_NXPrintf(
              &np, "%15s %30s (%5i / %5i bytes): %15s", sz, ent->GetClassname(),
          ...
          engine->Con_NXPrintf(
              &np, "%15s %30s: %15s", sz, ent->GetClassname(),
              ent->GetPredictable() ? "predicted" : "client created");
        }
      }
#ifndef _XBOX
      if (error_check && !entityDumped && dump && ShouldDumpEntity(ent)) {
        entityDumped = true;
    ...
    if (showlist >= 2) {
      ...
      engine->Con_NXPrintf(&np, "%15s %27s (%s / %s)  %14s", "totals:", "", sz1,
                           sz2, "");
    }
...
void CPrediction::SetupMove(C_BasePlayer *player, CUserCmd *ucmd,
  ...
  if (!pMoveParent) {
    ...
    ConcatTransforms(pMoveParent->EntityToWorldTransform(), viewToParent,
                     viewToWorld);
...
//			*to -
//-----------------------------------------------------------------------------
void CPrediction::FinishMove(C_BasePlayer *player, CUserCmd *ucmd,
                             CMoveData *move) {
#if !defined(NO_ENTITY_PREDICTION)
  ...
  if (player->m_hConstraintEntity)
    Assert(move->m_vecConstraintCenter ==
           player->m_hConstraintEntity->GetAbsOrigin());
  else
...
//-----------------------------------------------------------------------------
// Purpose: Runs the PLAYER's thinking code if time.  There is some play in the
// exact time the think
...
void CPrediction::RunThink(C_BasePlayer *player, double frametime) {
  ...
  if (thinktick <= 0 || thinktick > player->m_nTickBase) return;
...
//			*u -
//-----------------------------------------------------------------------------
void CPrediction::RunCommand(C_BasePlayer *player, CUserCmd *ucmd,
  ...
  if (ucmd->weaponselect != 0) {
    C_BaseCombatWeapon *weapon = dynamic_cast<C_BaseCombatWeapon *>(
        CBaseEntity::Instance(ucmd->weaponselect));
    if (weapon) {
...
//-----------------------------------------------------------------------------
// Purpose: In the forward direction, creates rays straight down and determines
// the
//  height of the 'floor' hit for each forward test.  Then, if the samples show
//  that the player is about to enter an up/down slope, sets *idealpitch to look
//  up or down that slope as appropriate
//-----------------------------------------------------------------------------
void CPrediction::SetIdealPitch(C_BasePlayer *player, const Vector &origin,
                                const QAngle &angles,
  ...
  if (player->GetGroundEntity() == NULL) return;

  AngleVectors(angles, &forward);
  forward[2] = 0;

  // Now move forward by 36, 48, 60, etc. units from the eye position and drop
  // lines straight down
  ...
  for (i = 0; i < MAX_FORWARD; i++) {
    ...
    UTIL_TraceLine(top, bottom, MASK_SOLID, NULL,
...
//-----------------------------------------------------------------------------
// Purpose: Walk backward through predictables looking for ClientCreated
// entities
...
void CPrediction::RemoveStalePredictedEntities(int sequence_number) {
  ...
  for (i = c - 1; i >= 0; i--) {
    ...
    // What's left should be things like projectiles that are just waiting to be
    // "linked"
    //  to their server counterpart and deleted
    Assert(ent->IsClientCreated());
    if (!ent->IsClientCreated()) continue;
    ...
    if (ent->m_PredictableID.GetAcknowledged()) {
      ...
      if (!ent->IsEFlagSet(EFL_KILLME)) {
        if (cl_showerror.GetInt() != 0) {
          ...
          Msg("Removing unack'ed predicted entity:  %s created %s(%i) id == %s "
              ": %p\n",
              ent->GetClassname(), ctx->m_pszCreationModule,
              ctx->m_nCreationLineNumber, ent->m_PredictableID.Describe(), ent);
        }
      }
    ...
    // This will remove it from predictables list and will also free the entity,
    // etc.
    ent->Release();
  }
#endif
...
void CPrediction::RestoreOriginalEntityState(void) {
  ...
  for (p = 0; p < pc; p++) {
    ...
    if (ent->GetPredictable()) {
      ent->RestoreData("RestoreOriginalEntityState",
                       C_BaseEntity::SLOT_ORIGINALDATA, PC_EVERYTHING);
    }
  }
#endif
}
...
//			*localPlayer -
//-----------------------------------------------------------------------------
void CPrediction::RunSimulation(int current_command, float curtime,
                                CUserCmd *cmd, C_BasePlayer *localPlayer) {
#if !defined(NO_ENTITY_PREDICTION)
  ...
  // Don't used cached numpredictables since entities can be created
  // mid-prediction by the player
  for (i = 0; i < predictables->GetPredictableCount(); i++) {
    // Always reset
    ...
    // Local player simulates first, if this assert fires then the predictables
    // list isn't sorted
    ...
    if (AddDataChangeEvent(entity, DATA_UPDATE_DATATABLE_CHANGED,
                           &entity->m_DataChangeEventRef)) {
    ...
    entity->OnLatchInterpolatedVariables(
        LATCH_SIMULATION_VAR | LATCH_ANIMATION_VAR |
        INTERPOLATE_OMIT_UPDATE_LAST_NETWORKED);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void InvalidateEFlagsRecursive(C_BaseEntity *pEnt, int nDirtyFlags,
                               int nChildFlags = 0) {
  pEnt->AddEFlags(nDirtyFlags);
  nDirtyFlags |= nChildFlags;
  for (CBaseEntity *pChild = pEnt->FirstMoveChild(); pChild;
...
//			previous_last_slot -
//-----------------------------------------------------------------------------
void CPrediction::ShiftIntermediateDataForward(int slots_to_remove,
...
void CPrediction::RestoreEntityToPredictedFrame(int predicted_frame) {
  ...
  for (i = 0; i < c; i++) {
    ...
    ent->RestoreData("RestoreEntityToPredictedFrame", predicted_frame,
                     PC_EVERYTHING);
  }
#endif
}
...
//-----------------------------------------------------------------------------
// Purpose: Computes starting destination for intermediate prediction data
// results and
...
// Output : int
//-----------------------------------------------------------------------------
int CPrediction::ComputeFirstCommandToExecute(bool received_new_world_update,
  ...
  // If we didn't receive a new update ( or we received an update that didn't
  // ack any new CUserCmds --
  //  so for the player it should be just like receiving no update ), just jump
  //  right up to the very last command we created for this very frame since we
  //  probably wouldn't have had any errors without being notified by the server
  //  of such a case.
  // NOTE:  received_new_world_update only gets set to false if cl_pred_optimize
  // >= 1
  if (!received_new_world_update || !m_nServerCommandsAcknowledged) {
    ...
    // Don't start past the last predicted command, though, or we'll get
    // prediction errors
    skipahead = std::min(skipahead, m_nCommandsPredicted);

    // Always restore since otherwise we might start prediction using an
    // "interpolated" value instead of a purely predicted value
    ...
    //  acknowledged running one or more commands, then we can revert the entity
    //  to the predicted state from last frame, shift the # of commands worth of
    //  intermediate state off of front the intermediate state array, and only
    //  predict the usercmd from the latest render frame.
    if (cl_pred_optimize.GetInt() >= 2 && !m_bPreviousAckHadErrors &&
        m_nCommandsPredicted > 0 &&
        m_nServerCommandsAcknowledged <= m_nCommandsPredicted) {
      // Copy all of the previously predicted data back into entity so we can
      // skip repredicting it This is the final slot that we previously
      // predicted
      ...
      if (m_bPreviousAckHadErrors) {
        ...
        // If an entity gets a prediction error, then we want to clear out its
        // interpolated variables so we don't mix different samples at the same
        // timestamps. We subtract 1 tick interval here because if we don't,
        // we'll have 3 interpolation entries with the same timestamp as this
        // predicted frame, so we won't be able to interpolate (which leads to
        // jerky movement in the player when ANY entity like your gun gets a
        // prediction error).
...
//-----------------------------------------------------------------------------
bool CPrediction::PerformPrediction(bool received_new_world_update,
  ...
  // This makes sure , tahe we are allwoed to sample the world when it may not
  // be ready to be sampled
  ...
  int i = ComputeFirstCommandToExecute(received_new_world_update,
  ...
  Assert(i >= 1);
  while (true) {
    // Incoming_acknowledged is the last usercmd the server acknowledged having
    // acted upon
  ...
  //	Msg( "%i : predicted %i commands forward, %i ack'd last frame, had
  // errors %s\n", 		gpGlobals->tickcount,
  //		m_nCommandsPredicted,
  ...
  // Somehow we looped past the end of the list (severe lag), don't predict at
  // all
...
void CPrediction::Update(int startframe, bool validframe,
  ...
  // Still starting at same frame, so make sure we don't do extra prediction
  // ,etc.
  if ((m_nPreviousStartFrame == startframe) && cl_pred_optimize.GetBool() &&
      cl_predict->GetInt()) {
  ...
  _Update(received_new_world_update, validframe, incoming_acknowledged,
          outgoing_command);
...
void CPrediction::_Update(bool received_new_world_update, bool validframe,
  ...
  // If we are not doing prediction, copy authoritative value into velocity and
  // angle.
  if (!cl_predict->GetInt()) {
  ...
  // This is cheesy, but if we have entities that are parented to attachments on
  // other entities, then it'll wind up needing to get a bone transform.
  {
    C_BaseAnimating::InvalidateBoneCaches();
    C_BaseAnimating::AutoAllowBoneAccess boneaccess(true, true);

    // Remove any purely client predicted entities that were left "dangling"
    // because the
    //  server didn't acknowledge them or which can now safely be removed
    RemoveStalePredictedEntities(incoming_acknowledged);

    // Restore objects back to "pristine" state from last network/world state
    // update
    if (received_new_world_update) {
      RestoreOriginalEntityState();
    }

    if (!PerformPrediction(received_new_world_update, localPlayer,
                           incoming_acknowledged, outgoing_command))
      return;
  }
...
#if !defined(NO_ENTITY_PREDICTION)
//-----------------------------------------------------------------------------
// Purpose: For determining that predicted creation entities are un-acked and
// should
//  be deleted
// Output : int
//-----------------------------------------------------------------------------
