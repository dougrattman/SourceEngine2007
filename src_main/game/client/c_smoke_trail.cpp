 ...
 public:
  ...
  virtual float UpdateRoll(SimpleParticle *pParticle, float timeDelta) {
    ...
    if (fabs(pParticle->m_flRollDelta) < 0.5f) {
      pParticle->m_flRollDelta =
          (pParticle->m_flRollDelta > 0.0f) ? 0.5f : -0.5f;
    }
...
IMPLEMENT_CLIENTCLASS_DT(C_SmokeTrail, DT_SmokeTrail, SmokeTrail)
  RecvPropFloat(RECVINFO(m_SpawnRate)), RecvPropVector(RECVINFO(m_StartColor)),
      RecvPropVector(RECVINFO(m_EndColor)),
      RecvPropFloat(RECVINFO(m_ParticleLifetime)),
      RecvPropFloat(RECVINFO(m_StopEmitTime)),
      RecvPropFloat(RECVINFO(m_MinSpeed)), RecvPropFloat(RECVINFO(m_MaxSpeed)),
      RecvPropFloat(RECVINFO(m_MinDirectedSpeed)),
      RecvPropFloat(RECVINFO(m_MaxDirectedSpeed)),
      RecvPropFloat(RECVINFO(m_StartSize)), RecvPropFloat(RECVINFO(m_EndSize)),
      RecvPropFloat(RECVINFO(m_SpawnRadius)), RecvPropInt(RECVINFO(m_bEmit)),
      RecvPropInt(RECVINFO(m_nAttachment)), RecvPropFloat(RECVINFO(m_Opacity)),
END_RECV_TABLE()
...
C_SmokeTrail::~C_SmokeTrail() {
  if (ToolsEnabled() && clienttools->IsInRecordingMode() &&
      m_pSmokeEmitter.IsValid() &&
      m_pSmokeEmitter->GetToolParticleEffectId() !=
...
// Purpose:
//-----------------------------------------------------------------------------
void C_SmokeTrail::GetAimEntOrigin(IClientEntity *pAttachedTo,
...
// Input  : bEmit -
//-----------------------------------------------------------------------------
void C_SmokeTrail::SetEmit(bool bEmit) { m_bEmit = bEmit; }
...
void C_SmokeTrail::Start(CParticleMgr *pParticleMgr,
  ...
  if (!pParticleMgr->AddEffect(&m_ParticleEffect, this)) return;
...
void C_SmokeTrail::Update(float fTimeDelta) {
  ...
  if ((m_StopEmitTime != 0) && (m_StopEmitTime <= gpGlobals->curtime)) return;
  ...
  while (m_ParticleSpawn.NextEvent(tempDelta)) {
    ...
    pParticle = (SimpleParticle *)m_pSmokeEmitter->AddParticle(
        sizeof(SimpleParticle), m_MaterialHandle[random->RandomInt(0, 1)],
        offset);
    ...
    float flDirectedVel =
        random->RandomFloat(m_MinDirectedSpeed, m_MaxDirectedSpeed);
    VectorMA(pParticle->m_vecVelocity, flDirectedVel, vecForward,
...
void C_SmokeTrail::CleanupToolRecordingState(KeyValues *msg) {
  ...
  if (!clienttools->IsInRecordingMode() || !m_pSmokeEmitter.IsValid()) return;

  // For now, we can't record smoketrails that don't have a moveparent
  C_BaseEntity *pEnt = GetMoveParent();
  if (!pEnt) return;

  bool bEmitterActive = m_bEmit && ((m_StopEmitTime == 0) ||
                                    (m_StopEmitTime > gpGlobals->curtime));

  // NOTE: Particle system destruction message will be sent by the particle
  // effect itself.
  if (m_pSmokeEmitter->GetToolParticleEffectId() ==
      TOOLPARTICLESYSTEMID_INVALID) {
    ...
    KeyValues *pLifetime =
        pInitializers->FindKey("DmeRandomLifetimeInitializer", true);
    pLifetime->SetFloat("minLifetime", m_ParticleLifetime);
    pLifetime->SetFloat("maxLifetime", m_ParticleLifetime);

    KeyValues *pVelocity =
        pInitializers->FindKey("DmeAttachmentVelocityInitializer", true);
    ...
    KeyValues *pRollSpeed =
        pInitializers->FindKey("DmeRandomRollSpeedInitializer", true);
    pRollSpeed->SetFloat("minRollSpeed", -1.0f);
    pRollSpeed->SetFloat("maxRollSpeed", 1.0f);

    KeyValues *pColor =
        pInitializers->FindKey("DmeRandomValueColorInitializer", true);
    Color c(std::clamp(m_StartColor.x * 255.0f, 0.0f, 255.0f),
            std::clamp(m_StartColor.y * 255.0f, 0.0f, 255.0f),
            std::clamp(m_StartColor.z * 255.0f, 0.0f, 255.0f), 255.0f);
    ...
    KeyValues *pAlpha =
        pInitializers->FindKey("DmeRandomAlphaInitializer", true);
    ...
    pAlpha->SetInt("minEndAlpha", std::clamp(nMinAlpha, 0, 255));
    pAlpha->SetInt("maxEndAlpha", std::clamp(nMaxAlpha, 0, 255));
    ...
    KeyValues *pRollSpeedUpdater =
        pUpdaters->FindKey("DmeRollSpeedAttenuateUpdater", true);
...
// Purpose:
//-----------------------------------------------------------------------------
void C_RocketTrail::GetAimEntOrigin(IClientEntity *pAttachedTo,
                                    Vector *pAbsOrigin, QAngle *pAbsAngles) {
  C_BaseEntity *pEnt = pAttachedTo->GetBaseEntity();
  if (pEnt && (m_nAttachment > 0)) {
    pEnt->GetAttachment(m_nAttachment, *pAbsOrigin, *pAbsAngles);
...
// Input  : bEmit -
//-----------------------------------------------------------------------------
void C_RocketTrail::SetEmit(bool bEmit) { m_bEmit = bEmit; }
...
void C_RocketTrail::Update(float fTimeDelta) {
  if (!m_pRocketEmitter) return;

  if (gpGlobals->frametime == 0.0f) return;
  ...
  float flScale =
      random->RandomFloat(m_flFlareScale - 0.5f, m_flFlareScale + 0.5f);
  ...
  for (i = 1; i < 9; i++) {
    ...
    pParticle = (SimpleParticle *)pSimple->AddParticle(
        ...
        pSimple->GetPMaterial(
            VarArgs("effects/muzzleflash%d", random->RandomInt(1, 4))),
        offset);
    ...
    pParticle->m_uchStartSize =
        (random->RandomFloat(5.0f, 6.0f) * (12 - (i)) / 9) * flScale;
  ...
  if (m_bEmit) {
    ...
    for (i = 1; i < numPuffs + 1; i++) {
      ...
      // debugoverlay->AddBoxOverlay( offset, -Vector(2,2,2), Vector(2,2,2),
      // vec3_angle, i*4, i*4, i*4, true, 4.0f );

      pParticle = (SimpleParticle *)m_pRocketEmitter->AddParticle(
          sizeof(SimpleParticle), m_MaterialHandle[random->RandomInt(0, 1)],
          offset);
      ...
      if (pParticle != NULL) {
        ...
        pParticle->m_flDieTime =
            m_ParticleLifetime + random->RandomFloat(m_ParticleLifetime * 0.9f,
                                                     m_ParticleLifetime * 1.1f);
        ...
        pParticle->m_uchStartSize =
            m_StartSize * random->RandomFloat(0.75f, 1.25f);
  ...
  if (m_bDamaged) {
    ...
    CSmartPtr<CEmberEffect> pEmitter =
        CEmberEffect::Create("C_RocketTrail::damaged");

    pEmitter->SetSortOrigin(GetAbsOrigin());

    PMaterialHandle flameMaterial = m_pRocketEmitter->GetPMaterial(
        VarArgs("sprites/flamelet%d", random->RandomInt(1, 4)));

    // Flames from the rocket
    for (i = 0; i < 8; i++) {
      offset = RandomVector(-8, 8) + GetAbsOrigin();

      pParticle = (SimpleParticle *)pEmitter->AddParticle(
          sizeof(SimpleParticle), flameMaterial, offset);
...
Vector SporeEffect::UpdateColor(const SimpleParticle *pParticle) {
  Vector color;
  float ramp =
      ((float)pParticle->m_uchStartAlpha / 255.0f) *
      sin(M_PI * (pParticle->m_flLifetime /
                  pParticle->m_flDieTime));  // 1.0f - ( pParticle->m_flLifetime
                                             // / pParticle->m_flDieTime );
...
IMPLEMENT_CLIENTCLASS_DT(C_SporeExplosion, DT_SporeExplosion, SporeExplosion)
  RecvPropFloat(RECVINFO(m_flSpawnRate)),
      ...
      RecvPropFloat(RECVINFO(m_flSpawnRadius)), RecvPropBool(RECVINFO(m_bEmit)),
...
void C_SporeExplosion::Start(CParticleMgr *pParticleMgr,
  ...
  if (pParticleMgr->AddEffect(&m_ParticleEffect, this) == false) return;

  // Create our main effect
  m_pSporeEffect = SporeEffect::Create("C_SporeExplosion");

  if (m_pSporeEffect == NULL) return;
...
void C_SporeExplosion::AddParticles(void) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    sParticle = (SimpleParticle *)m_pSporeEffect->AddParticle(
        sizeof(SimpleParticle), m_hMaterial, GetAbsOrigin() + offset);
  ...
  sParticle = (SimpleParticle *)m_pSporeEffect->AddParticle(
      sizeof(SimpleParticle), g_Mat_DustPuff[1], GetAbsOrigin() + offset);
...
ConVar cl_sporeclipdistance("cl_sporeclipdistance", "512",
...
void RPGShotDownCallback(const CEffectData &data) {
  ...
  C_BaseEntity::EmitSound(filter, SOUND_FROM_WORLD, "Missile.ShotDown",
                          &data.m_vOrigin);
  ...
  if (CExplosionOverlay *pOverlay = new CExplosionOverlay) {
    ...
    pOverlay->m_Sprites[0].m_flVertSize =
        pOverlay->m_Sprites[0].m_flHorzSize * 0.5f;
...
class C_SporeTrail : public C_BaseParticleEntity {
 ...
 private:
  ...
  Vector m_vecPos;
  Vector m_vecLastPos;  // This is stored so we can spawn particles in between
                        // the previous and new position to eliminate holes in
                        // the trail.
...
IMPLEMENT_CLIENTCLASS_DT(C_SporeTrail, DT_SporeTrail, SporeTrail)
  RecvPropFloat(RECVINFO(m_flSpawnRate)),
      ...
      RecvPropFloat(RECVINFO(m_flStartSize)),
      RecvPropFloat(RECVINFO(m_flEndSize)),
      RecvPropFloat(RECVINFO(m_flSpawnRadius)), RecvPropInt(RECVINFO(m_bEmit)),
END_RECV_TABLE()
...
// Input  : bEmit -
//-----------------------------------------------------------------------------
void C_SporeTrail::SetEmit(bool bEmit) { m_bEmit = bEmit; }
...
void C_SporeTrail::AddParticles(void) {
  ...
  // Make a new particle
  SimpleParticle *sParticle = (SimpleParticle *)m_ParticleEffect.AddParticle(
      sizeof(SimpleParticle),
      m_hMaterial);  // m_pSmokeEffect->AddParticle( sizeof(SimpleParticle),
                     // m_hMaterial, GetAbsOrigin()+offset );
...
void C_SporeTrail::RenderParticles(CParticleRenderIterator *pIterator) {
  ...
  const SimpleParticle *pParticle =
      (const SimpleParticle *)pIterator->GetFirst();
  while (pParticle) {
    // Render
    Vector tPos;
    ...
    // Render it
    RenderParticle_ColorSize(pIterator->GetParticleDraw(), tPos, color, 1.0f,
                             4);
...
// Purpose:
//-----------------------------------------------------------------------------
void C_SporeTrail::GetAimEntOrigin(IClientEntity *pAttachedTo,
                                   Vector *pAbsOrigin, QAngle *pAbsAngles) {
  C_BaseEntity *pEnt = pAttachedTo->GetBaseEntity();
...
IMPLEMENT_CLIENTCLASS_DT(C_FireTrail, DT_FireTrail, CFireTrail)
  RecvPropInt(RECVINFO(m_nAttachment)), RecvPropFloat(RECVINFO(m_flLifetime)),
END_RECV_TABLE()
...
void C_FireTrail::Start(CParticleMgr *pParticleMgr,
  ...
  m_hMaterial[FTRAIL_FLAME1] =
      m_pTrailEmitter->GetPMaterial("sprites/flamelet1");
  m_hMaterial[FTRAIL_FLAME2] =
      m_pTrailEmitter->GetPMaterial("sprites/flamelet2");
  m_hMaterial[FTRAIL_FLAME3] =
      m_pTrailEmitter->GetPMaterial("sprites/flamelet3");
  m_hMaterial[FTRAIL_FLAME4] =
      m_pTrailEmitter->GetPMaterial("sprites/flamelet4");
  m_hMaterial[FTRAIL_FLAME5] =
      m_pTrailEmitter->GetPMaterial("sprites/flamelet5");
...
void C_FireTrail::Update(float fTimeDelta) {
  if (!m_pTrailEmitter) return;

  if ((m_flLifetime != 0) && (m_flLifetime <= gpGlobals->curtime)) return;
...
#define MAX_SPEED 64
  ...
  // if ( ShouldEmit() )
  {
    ...
    for (int i = 1; i < numPuffs + 1; i++) {
      offset =
          m_vecLastPosition + (moveDiff * step * i) + RandomVector(-4.0f, 4.0f);

      // debugoverlay->AddBoxOverlay( offset, -Vector(2,2,2), Vector(2,2,2),
      // vec3_angle, i*4, i*4, i*4, true, 1.0f );

      pParticle = (SimpleParticle *)m_pSmokeEmitter->AddParticle(
          sizeof(SimpleParticle),
          m_hMaterial[random->RandomInt(FTRAIL_FLAME1, FTRAIL_FLAME5)], offset);
    ...
    pParticle = (SimpleParticle *)m_pSmokeEmitter->AddParticle(
        sizeof(SimpleParticle),
        m_hMaterial[random->RandomInt(FTRAIL_SMOKE1, FTRAIL_SMOKE2)], offset);

    if (pParticle != NULL) {
      pParticle->m_flLifetime = 0.0f;
      pParticle->m_flDieTime = (PARTICLE_LIFETIME * 10.0f) +
                               random->RandomFloat(PARTICLE_LIFETIME * 0.75f,
                                                   PARTICLE_LIFETIME * 1.25f);
...
class CDustFollower : public CSimpleEmitter {
 public:
  ...
  virtual void UpdateVelocity(SimpleParticle *pParticle, float timeDelta) {
    pParticle->m_vecVelocity =
        pParticle->m_vecVelocity * ExponentialDecay(0.3, timeDelta);
  }
...
IMPLEMENT_CLIENTCLASS_DT(C_DustTrail, DT_DustTrail, DustTrail)
  RecvPropFloat(RECVINFO(m_SpawnRate)), RecvPropVector(RECVINFO(m_Color)),
      RecvPropFloat(RECVINFO(m_ParticleLifetime)),
      RecvPropFloat(RECVINFO(m_StopEmitTime)),
      RecvPropFloat(RECVINFO(m_MinSpeed)), RecvPropFloat(RECVINFO(m_MaxSpeed)),
      RecvPropFloat(RECVINFO(m_MinDirectedSpeed)),
...
C_DustTrail::~C_DustTrail() {
  if (ToolsEnabled() && clienttools->IsInRecordingMode() &&
      m_pDustEmitter.IsValid() &&
      m_pDustEmitter->GetToolParticleEffectId() !=
          TOOLPARTICLESYSTEMID_INVALID) {
    KeyValues *msg = new KeyValues("OldParticleSystem_ActivateEmitter");
...
// Input  : bEmit -
//-----------------------------------------------------------------------------
void C_DustTrail::SetEmit(bool bEmit) { m_bEmit = bEmit; }
...
void C_DustTrail::Start(CParticleMgr *pParticleMgr,
  ...
  for (int i = 0; i < DUSTTRAIL_MATERIALS; i++) {
    ...
    m_MaterialHandle[i] =
        m_pDustEmitter->GetPMaterial("particle/smokesprites_0001");
  }
...
void C_DustTrail::Update(float fTimeDelta) {
  ...
  while (m_ParticleSpawn.NextEvent(tempDelta)) {
    ...
    offsetColor = m_Color;
    float flMaxVal = std::max(m_Color[0], m_Color[1]);
    ...
    if (m_StopEmitTime != 0 && m_StopEmitTime > m_StartEmitTime) {
      alpha *= sqrt((m_StopEmitTime - gpGlobals->curtime) /
                    (m_StopEmitTime - m_StartEmitTime));
    }
...
void C_DustTrail::CleanupToolRecordingState(KeyValues *msg) {
  ...
  // to use it to send particle system messages because we
  // know the grenade has been recorded at this point
  if (!clienttools->IsInRecordingMode() || !m_pDustEmitter.IsValid()) return;
  ...
  // NOTE: Particle system destruction message will be sent by the particle
  // effect itself.
  if (m_pDustEmitter->GetToolParticleEffectId() ==
      TOOLPARTICLESYSTEMID_INVALID) {
    ...
    KeyValues *pVelocity =
        pInitializers->FindKey("DmeDecayVelocityInitializer", true);
    ...
    KeyValues *pColor =
        pInitializers->FindKey("DmeRandomValueColorInitializer", true);
    Color c(std::clamp(m_Color.x * 255.0f, 0.0f, 255.0f),
            std::clamp(m_Color.y * 255.0f, 0.0f, 255.0f),
            std::clamp(m_Color.z * 255.0f, 0.0f, 255.0f), 255.0f);
    pColor->SetColor("startColor", c);
    ...
    int nMinAlpha = 255 * m_Opacity * 0.75f;
    int nMaxAlpha = 255 * m_Opacity * 1.25f;
    pAlpha->SetInt("minStartAlpha", std::clamp(nMinAlpha, 0, 255));
    pAlpha->SetInt("maxStartAlpha", std::clamp(nMaxAlpha, 0, 255));
    pAlpha->SetInt("minEndAlpha", std::clamp(nMinAlpha, 0, 255));
    pAlpha->SetInt("maxEndAlpha", std::clamp(nMaxAlpha, 0, 255));
