...
void FX_AddStaticLine(const Vector &start, const Vector &end, float scale,
                      float life, const char *materialName,
                      unsigned char flags) {
  CFXStaticLine *t = new CFXStaticLine("StaticLine", start, end, scale, life,
                                       materialName, flags);
  assert(t);
...
void FX_AddDiscreetLine(const Vector &start, const Vector &direction,
                        float velocity, float length, float clipLength,
                        float scale, float life, const char *shader) {
  CFXDiscreetLine *t =
      new CFXDiscreetLine("Line", start, direction, velocity, length,
                          clipLength, scale, life, shader);
  assert(t);
...
//-----------------------------------------------------------------------------
void FX_AddQuad(const Vector &origin, const Vector &normal, float startSize,
                float endSize, float sizeBias, float startAlpha, float endAlpha,
                float alphaBias, float yaw, float deltaYaw, const Vector &color,
                float lifeTime, const char *shader, unsigned int flags) {
  FXQuadData_t data;
...
void FX_PlayerTracer(Vector &start, Vector &end) {
  ...
  VectorMA(start, TRACER_BASE_OFFSET + random->RandomFloat(-24.0f, 64.0f),
           shotDir, dStart);
  ...
  // materialName = ( random->RandomInt( 0, 1 ) ) ? "effects/tracer_middle" :
  // "effects/tracer_middle2";
  materialName = "effects/spark";

  t = new CFXStaticLine("Tracer", dStart, dEnd,
                        random->RandomFloat(0.5f, 0.75f), 0.01f, materialName,
                        0);
  assert(t);
...
void FX_TracerSound(const Vector &start, const Vector &end, int iTracerType) {
  ...
  if (vDist >= (flWhizDist * flWhizDist)) return;
  ...
  if (C_BaseEntity::GetParametersForSound(pszSoundName, params, NULL)) {
    ...
    enginesound->EmitSound(filter, SOUND_FROM_WORLD, CHAN_STATIC,
                           params.soundname, params.volume,
                           SNDLVL_TO_ATTN(params.soundlevel), 0, params.pitch,
                           &start, &shotDir, false);
  }
  ...
  g_BulletWhiz.m_nextWhizTime =
      gpGlobals->curtime + random->RandomFloat(flMinWhizTime, flMaxWhizTime);
}
...
void FX_Tracer(Vector &start, Vector &end, int velocity, bool makeWhiz) {
  ...
  if (dist >= 256) {
    ...
    FX_AddDiscreetLine(start, dir, velocity, length, dist,
                       random->RandomFloat(0.75f, 0.9f), life, "effects/spark");
  }
