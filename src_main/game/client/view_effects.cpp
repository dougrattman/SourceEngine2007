...
// Arbitrary limit so that bad entity logic on the server can't consume tons of
...
BEGIN_SIMPLE_DATADESC(screenfade_t)
  DEFINE_FIELD(Speed, FIELD_FLOAT), DEFINE_FIELD(End, FIELD_TIME),
      DEFINE_FIELD(Reset, FIELD_TIME), DEFINE_FIELD(r, FIELD_CHARACTER),
      DEFINE_FIELD(g, FIELD_CHARACTER), DEFINE_FIELD(b, FIELD_CHARACTER),
      DEFINE_FIELD(alpha, FIELD_CHARACTER), DEFINE_FIELD(Flags, FIELD_INTEGER),
END_DATADESC()
...
BEGIN_SIMPLE_DATADESC(screenshake_t)
  DEFINE_FIELD(endtime, FIELD_TIME), DEFINE_FIELD(duration, FIELD_FLOAT),
      DEFINE_FIELD(amplitude, FIELD_FLOAT),
      DEFINE_FIELD(frequency, FIELD_FLOAT), DEFINE_FIELD(nextShake, FIELD_TIME),
      DEFINE_FIELD(offset, FIELD_VECTOR), DEFINE_FIELD(angle, FIELD_FLOAT),
END_DATADESC()
...
static ConVar shake_show("shake_show", "0", 0,
                         "Displays a list of the active screen shakes.");
static ConCommand shake_stop("shake_stop", CC_Shake_Stop,
                             "Stops all active screen shakes.\n", FCVAR_CHEAT);

//-----------------------------------------------------------------------------
// Purpose: Stops all active screen shakes.
//-----------------------------------------------------------------------------
void CC_Shake_Stop() { g_ViewEffects.ClearAllShakes(); }

//-----------------------------------------------------------------------------
// Purpose: Apply noise to the eye position.
// UNDONE: Feedback a bit of this into the view model position.  It shakes too
// much
//-----------------------------------------------------------------------------
void CViewEffects::CalcShake(void) {
  float fraction, freq;

  // We'll accumulate the aggregate shake for this frame into these data
  // members.
  ...
  for (int nShake = nShakeCount - 1; nShake >= 0; nShake--) {
    ...
    if ((gpGlobals->curtime > pShake->endtime) || pShake->duration <= 0 ||
        pShake->amplitude <= 0 || pShake->frequency <= 0) {
      // Retire this shake.
    ...
    if (bShow) {
      ...
      engine->Con_NXPrintf(&np, "%02d: dur(%8.2f) amp(%8.2f) freq(%8.2f)",
                           nShake + 1, (double)pShake->duration,
                           (double)pShake->amplitude,
                           (double)pShake->frequency);
    }
    ...
    if (gpGlobals->curtime > pShake->nextShake) {
      ...
      for (int i = 0; i < 3; i++) {
        pShake->offset[i] =
            random->RandomFloat(-pShake->amplitude, pShake->amplitude);
      }

      pShake->angle = random->RandomFloat(-pShake->amplitude * 0.25,
                                          pShake->amplitude * 0.25);
    }

    // Ramp down amplitude over duration (fraction goes from 1 to 0 linearly
    // with slope 1/duration)
    ...
    if (pShake->command != SHAKE_START_RUMBLEONLY) {
      // As long as this isn't a RUMBLE ONLY effect, then accumulate screen
      // shake
    ...
    pShake->amplitude -=
        pShake->amplitude *
        (gpGlobals->frametime / (pShake->duration * pShake->frequency));
  }
...
//-----------------------------------------------------------------------------
// Purpose: Apply the current screen shake to this origin/angles.  Factor is the
// amount to apply
...
//			use anytime we get an amplitude or frequency command,
//because the 			most likely case is that we're modifying a shake with a long
//			duration rather than a brief shake caused by an explosion,
//etc.
//-----------------------------------------------------------------------------
screenshake_t *CViewEffects::FindLongestShake() {
  ...
  for (int i = 0; i < nShakeCount; i++) {
    ...
    if (pShake &&
        (!pLongestShake || (pShake->duration > pLongestShake->duration))) {
...
void CViewEffects::Shake(ScreenShake_t &data) {
  if ((data.command == SHAKE_START || data.command == SHAKE_START_RUMBLEONLY) &&
...
void CViewEffects::FadeCalculate(void) {
  ...
  for (i = iSize - 1; i >= 0; i--) {
    ...
    // All done?
    if ((gpGlobals->curtime > pFade->Reset) &&
        (gpGlobals->curtime > pFade->End)) {
  ...
  m_bModulate = false;
  m_FadeColorRGBA[0] = m_FadeColorRGBA[1] = m_FadeColorRGBA[2] =
      m_FadeColorRGBA[3] = 0;
  ...
  for (i = 0; i < m_FadeList.Size(); i++) {
    ...
    if (pFade->Flags & (FFADE_OUT | FFADE_IN)) {
      ...
      iFadeAlpha = std::min(iFadeAlpha, (int)pFade->alpha);
...
//			*blend -
//-----------------------------------------------------------------------------
void CViewEffects::GetFadeParams(byte *r, byte *g, byte *b, byte *a,
                                 bool *blend) {
...
class CViewEffectsSaveRestoreBlockHandler : public CDefSaveRestoreBlockHandler {
 ...
 public:
  ...
  virtual void Save(ISave *pSave) { vieweffects->Save(pSave); }
