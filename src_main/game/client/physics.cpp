...
ConVar cl_phys_timescale(
    "cl_phys_timescale", "1.0", FCVAR_CHEAT,
...
class CCollisionEvent : public IPhysicsCollisionEvent,
 ...
 private:
  ...
  void AddTouchEvent(C_BaseEntity *pEntity0, C_BaseEntity *pEntity1,
                     int touchType, const Vector &point, const Vector &normal);
...
bool PhysicsDLLInit(CreateInterfaceFn physicsFactory) {
  if ((physics = (IPhysics *)physicsFactory(VPHYSICS_INTERFACE_VERSION,
                                            NULL)) == NULL ||
      (physprops = (IPhysicsSurfaceProps *)physicsFactory(
...
void PhysicsLevelInit(void) {
  ...
  g_PhysWorldObject =
      PhysCreateWorld_Shared(GetClientWorldEntity(), modelinfo->GetVCollide(1),
...
#endif
{
  ...
  Assert(GameRules());
  if (GameRules()) {
    if (!GameRules()->ShouldCollide(pEntity0->GetCollisionGroup(),
                                    pEntity1->GetCollisionGroup()))
      return 0;
  }

  if ((pObj0->GetGameFlags() & FVPHYSICS_PART_OF_RAGDOLL) &&
      (pObj1->GetGameFlags() & FVPHYSICS_PART_OF_RAGDOLL)) {
    if (!cl_ragdoll_collide.GetBool()) return 0;
  }
    ...
    if (pObj0->GetGameFlags() & FVPHYSICS_PART_OF_RAGDOLL) {
      ...
      if (pAnim && pAnim->m_pRagdoll) {
        IPhysicsConstraintGroup *pGroup =
            pAnim->m_pRagdoll->GetConstraintGroup();
        if (pGroup) {
...
class CPhysicsSystem : public CAutoGameSystemPerFrame {
 public:
  ...
  // HACKHACK: PhysicsDLLInit() is called explicitly because it requires a
  // parameter
  virtual bool Init();
  ...
  void AddImpactSound(void *pGameData, IPhysicsObject *pObject,
                      int surfaceProps, int surfacePropsHit, float volume,
                      float speed);
...
IGameSystem *PhysicsGameSystem(void) { return &g_PhysicsSystem; }

// HACKHACK: PhysicsDLLInit() is called explicitly because it requires a
// parameter
bool CPhysicsSystem::Init() { return true; }
...
void CPhysicsSystem::LevelInitPostEntity(void) { PhysicsLevelInit(); }
...
void CPhysicsSystem::AddImpactSound(void *pGameData, IPhysicsObject *pObject,
                                    int surfaceProps, int surfacePropsHit,
                                    float volume, float speed) {
  physicssound::AddImpactSound(m_impactSounds, pGameData, SOUND_FROM_WORLD,
                               CHAN_STATIC, pObject, surfaceProps,
    ...
    if (activeCount) {
      ...
      for (int i = 0; i < activeCount; i++) {
        C_BaseEntity *pEntity =
            reinterpret_cast<C_BaseEntity *>(pActiveList[i]->GetGameData());
        if (pEntity) {
...
void PhysicsSimulate() { g_PhysicsSystem.PhysicsSimulate(); }
...
void CCollisionEvent::ObjectSound(int index, vcollisionevent_t *pEvent) {
  ...
  if (pGameData) {
    float volume =
        speed * (1.0f / (320.0f * 320.0f));  // max volume at 320 in/s

    if (volume > 1.0f) volume = 1.0f;

    if (surfaceProps >= 0) {
      g_PhysicsSystem.AddImpactSound(pGameData, pObject, surfaceProps,
                                     pEvent->surfaceProps[!index], volume,
                                     speed);
    }
  }
}
...
//			touchType -
//-----------------------------------------------------------------------------
void CCollisionEvent::AddTouchEvent(C_BaseEntity *pEntity0,
                                    C_BaseEntity *pEntity1, int touchType,
                                    const Vector &point, const Vector &normal) {
...
//			*pEntity1 -
//-----------------------------------------------------------------------------
void CCollisionEvent::DispatchStartTouch(C_BaseEntity *pEntity0,
...
//			*pEntity1 -
//-----------------------------------------------------------------------------
void CCollisionEvent::DispatchEndTouch(C_BaseEntity *pEntity0,
                                       C_BaseEntity *pEntity1) {
...
void CCollisionEvent::Friction(IPhysicsObject *pObject, float energy,
  ...
  CallbackContext callback(this);
  if (energy < 0.05f || surfaceProps < 0) return;
...
void PhysicsSplash(IPhysicsFluidController *pFluid, IPhysicsObject *pObject,
  ...
  // Now we have a basis tangent to the surface that matches the object's local
  // orientation as well as possible compute an OBB using this basis

  // Get object extents in basis
  Vector tanPts[2], binPts[2];
  tanPts[0] = physcollision->CollideGetExtent(
      pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
      -tangent);
  tanPts[1] = physcollision->CollideGetExtent(pObject->GetCollide(),
                                              pEntity->GetAbsOrigin(),
                                              pEntity->GetAbsAngles(), tangent);
  binPts[0] = physcollision->CollideGetExtent(
      pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
      -binormal);
  binPts[1] = physcollision->CollideGetExtent(
      pObject->GetCollide(), pEntity->GetAbsOrigin(), pEntity->GetAbsAngles(),
      binormal);
  ...
  Vector centerPoint =
      center[0] * tangent + center[1] * binormal + dist * normal;
  ...
  */
  ...
  for (int i = 0; i < splashes; i++) {
    ...
    if (impactSpeed > 150) {
      ...
      FX_WaterRipple(point, random->RandomFloat(0.25f, 0.5f), &color,
                     luminosity, random->RandomFloat(0.5f, 1.0f));
    }
  }
}
...
void PhysFrictionSound(CBaseEntity *pEntity, IPhysicsObject *pObject,
  ...
  if (flVolume > (1.0f / 128.0f)) {
    ...
    if (!pFriction->pObject) {
      ...
      // clientside created entites doesn't have a valid entindex, let 'world'
      // play the sound for them
      if (entindex < 0) entindex = 0;
...
float PhysGetNextSimTime() {
  return physenv->GetSimulationTime() +
         gpGlobals->frametime * cl_phys_timescale.GetFloat();
}
...
float PhysGetSyncCreateTime() {
  ...
  if (nextTime < simTime) {
    ...
    // so create physics objects at that time so that they will reach the
    // current position at curtime.  Otherwise the physics object will simulate
    // forward from curtime and pop into the future a bit at this point of
    // transition
