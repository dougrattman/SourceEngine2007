...
IMPLEMENT_CLIENTCLASS_DT_NOBASE(C_Sun, DT_Sun, CSun)
  ...
  RecvPropInt(RECVINFO(m_clrRender), 0, RecvProxy_IntToColor32),
      ...
      RecvPropVector(RECVINFO(m_vDirection)), RecvPropInt(RECVINFO(m_bOn)),
      RecvPropInt(RECVINFO(m_nSize)), RecvPropInt(RECVINFO(m_nOverlaySize)),
      RecvPropInt(RECVINFO(m_nMaterial)),
      RecvPropInt(RECVINFO(m_nOverlayMaterial)),
      RecvPropFloat("HDRColorScale", 0, SIZEOF_IGNORE, 0,
                    RecvProxy_HDRColorScale),
...
void C_Sun::OnDataChanged(DataUpdateType_t updateType) {
  ...
  // We have to do special setup on our colors because we're tinting an additive
  // material. If we don't have at least one component at full strength, the
  // luminosity of the material will change and that will cause the material to
  // become more translucent  This would be incorrect for the sun, which should
  // always be completely opaque at its core.  Here, we renormalize the
  // components to make sure only hue is altered.

  float maxComponent =
      std::max(m_clrRender->r, std::max(m_clrRender->g, m_clrRender->b));
  ...
  // If we're non-zero, use the value (otherwise use the value we calculated
  // above)
  ...
  const model_t *pModel =
      (m_nMaterial != 0) ? modelinfo->GetModel(m_nMaterial) : NULL;
  const char *pModelName = pModel ? modelinfo->GetModelName(pModel) : "";
  m_Overlay.m_Sprites[0].m_pMaterial =
      materials->FindMaterial(pModelName, TEXTURE_GROUP_OTHER);
  ...
  pModel = (m_nOverlayMaterial != 0) ? modelinfo->GetModel(m_nOverlayMaterial)
                                     : NULL;
  pModelName = pModel ? modelinfo->GetModelName(pModel) : "";
  m_GlowOverlay.m_Sprites[0].m_pMaterial =
      materials->FindMaterial(pModelName, TEXTURE_GROUP_OTHER);
