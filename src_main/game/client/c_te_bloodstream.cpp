...
// Networking
//-----------------------------------------------------------------------------
IMPLEMENT_CLIENTCLASS_EVENT_DT(C_TEBloodStream, DT_TEBloodStream,
                               CTEBloodStream)
RecvPropVector(RECVINFO(m_vecDirection)), RecvPropInt(RECVINFO(r)),
    RecvPropInt(RECVINFO(g)), RecvPropInt(RECVINFO(b)),
    RecvPropInt(RECVINFO(a)), RecvPropInt(RECVINFO(m_nAmount)),
    END_RECV_TABLE()
...
// Recording
//-----------------------------------------------------------------------------
static inline void RecordBloodStream(const Vector &start,
                                     const Vector &direction, int r, int g,
                                     int b, int a, int amount) {
...
void TE_BloodStream(IRecipientFilter &filter, float delay, const Vector *org,
                    const Vector *direction, int r, int g, int b, int a,
                    int amount) {
  RecordBloodStream(*org, *direction, r, g, b, a, amount);

  CSmartPtr<CTEParticleRenderer> pRen =
      CTEParticleRenderer::Create("TEBloodStream", *org);
  if (!pRen) return;
    ...
    if (p) {
      ...
      for (count2 = 0; count2 < 2; count2++) {
        ...
        if (p) {
          ...
          p->SetAlpha(a);
          p->m_Pos.Init((*org)[0] + random->RandomFloat(-1, 1),
...
void C_TEBloodStream::PostDataUpdate(DataUpdateType_t updateType) {
  ...
  TE_BloodStream(filter, 0.0f, &m_vecOrigin, &m_vecDirection, r, g, b, a,
                 m_nAmount);
}
...
void TE_BloodStream(IRecipientFilter &filter, float delay,
  ...
  TE_BloodStream(filter, 0.0f, &vecOrigin, &vecDirection, c.r(), c.g(), c.b(),
                 c.a(), nAmount);
}
