...
CPDumpPanel *GetPDumpPanel() { return g_pPDumpPanel; }
...
CPDumpPanel::~CPDumpPanel() { g_pPDumpPanel = NULL; }
...
bool CPDumpPanel::ShouldDraw() {
  if (m_DumpEntityInfo.Count() == 0) return false;

  return CHudElement::ShouldDraw();
}

void CPDumpPanel::DumpComparision(const char *classname, const char *fieldname,
  ...
  Q_snprintf(slot->fieldstring, sizeof(slot->fieldstring), "%s %s", fieldname,
             value);
...
// Output : static void
//-----------------------------------------------------------------------------
static void DumpComparision(const char *classname, const char *fieldname,
                            const char *fieldtype, bool networked,
                            bool noterrorchecked, bool differs,
                            bool withintolerance, const char *value) {
  if (!g_pPDumpPanel) return;

  g_pPDumpPanel->DumpComparision(classname, fieldname, fieldtype, networked,
                                 noterrorchecked, differs, withintolerance,
                                 value);
}
...
// Output : static void
//-----------------------------------------------------------------------------
void CPDumpPanel::PredictionDumpColor(bool networked, bool errorchecked,
                                      bool differs, bool withintolerance,
...
void CPDumpPanel::DumpEntity(C_BaseEntity *ent, int commands_acknowledged) {
  ...
  CPredictionCopy datacompare(PC_EVERYTHING, original_state_data,
                              data_type_original, predicted_state_data,
                              data_type_predicted,
                              true,   // counterrors
                              true,   // reporterrors
                              false,  // copy data
...
void CPDumpPanel::Paint() {
  ...
  Q_snprintf(sz, sizeof(sz), "entity # %i: %s%s%s", ent->entindex(),
             classprefix, classname, classextra);
  ...
  for (i = 0; i < c; i++) {
    ...
    PredictionDumpColor(slot->networked, !slot->noterrorchecked, slot->differs,
                        slot->withintolerance, r, g, b, a);
