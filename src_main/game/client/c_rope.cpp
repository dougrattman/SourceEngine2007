...
static ConVar rope_subdiv("rope_subdiv", "2", 0, "Rope subdivision amount",
                          true, 0, true, MAX_ROPE_SUBDIVS);
static ConVar rope_collide("rope_collide", "1", 0,
                           "Collide rope with the world");

static ConVar rope_smooth("rope_smooth", "1", 0,
                          "Do an antialiasing effect on ropes");
static ConVar rope_smooth_enlarge(
    "rope_smooth_enlarge", "1.4", 0,
    "How much to enlarge ropes in screen space for antialiasing effect");

static ConVar rope_smooth_minwidth(
    "rope_smooth_minwidth", "0.3", 0,
    "When using smoothing, this is the min screenspace width it lets a rope "
    "shrink to");
static ConVar rope_smooth_minalpha("rope_smooth_minalpha", "0.2", 0,
                                   "Alpha for rope antialiasing effect");

static ConVar rope_smooth_maxalphawidth("rope_smooth_maxalphawidth", "1.75");
static ConVar rope_smooth_maxalpha("rope_smooth_maxalpha", "0.5", 0,
                                   "Alpha for rope antialiasing effect");

static ConVar mat_fullbright("mat_fullbright", "0",
                             FCVAR_CHEAT);  // get it from the engine
static ConVar r_drawropes("r_drawropes", "1", FCVAR_CHEAT);
static ConVar r_ropetranslucent("r_ropetranslucent", "1");

static ConVar rope_wind_dist("rope_wind_dist", "1000", 0,
                             "Don't use CPU applying small wind gusts to ropes "
                             "when they're past this distance.");
static ConVar rope_averagelight(
    "rope_averagelight", "1", 0,
...
static Vector g_BarbedSubdivs[MAX_ROPE_SUBDIVS] = {
    Vector(1.5, 1.5 * 1.5, 1.5 * 1.5 * 1.5),
 ...
 public:
  ...
  inline void *Alloc(size_t bytes) {
    ...
    if (pReturn == NULL) {
      ...
      Warning(
          "Overflowed rope queued rendering memory stack. Needed %d, have "
          "%d/%d\n",
          bytes, iMaxSize - m_QueuedRopeMemory[m_nCurrentStack].GetUsed(),
          iMaxSize);
      pReturn = new uint8_t[bytes];
  ...
  int m_nCurrentStack;
  CUtlVector<void *>
      m_DeleteOnSwitch[2];  // when we overflow the stack, we do new/delete
};
...
struct RopeSegData_t {
  ...
  // If this is less than rope_solid_minwidth and rope_solid_minalpha is 0, then
  // we can avoid drawing..
...
class CRopeManager : public IRopeManager {
 public:
  ...
  void OnRenderStart(void) { m_QueuedModeMemory.SwitchStack(); }

 private:
  struct RopeRenderData_t;

 public:
  void DrawRenderCache_NonQueued(
      bool bShadowDepth, RopeRenderData_t *pRenderCache, int nRenderCacheCount,
      const Vector &vCurrentViewForward, const Vector &vCurrentViewOrigin,
  ...
  enum { MAX_ROPE_RENDERCACHE = 512 };

  void RemoveRopeFromQueuedRenderCaches(C_RopeKeyframe *pRope);

 private:
  void RenderNonSolidRopes(IMatRenderContext *pRenderContext,
                           IMaterial *pMaterial, int nVertCount,
                           int nIndexCount);
  void RenderSolidRopes(IMatRenderContext *pRenderContext, IMaterial *pMaterial,
                        int nVertCount, int nIndexCount, bool bRenderNonSolid);
 ...
 private:
  ...
  CThreadFastMutex
      m_RenderCacheMutex;  // there's only any contention during the switch from
                           // r_queued_ropes on to off
...
IRopeManager *RopeManager() { return &s_RopeManager; }

inline bool ShouldUseFakeAA(IMaterial *pBackMaterial) {
  return pBackMaterial && rope_smooth.GetInt() &&
         engine->GetDXSupportLevel() > 70 &&
         !g_pMaterialSystemHardwareConfig->IsAAEnabled();
}
...
void CRopeManager::AddToRenderCache(C_RopeKeyframe *pRope) {
  ...
  for (; iRenderCache < nRenderCacheCount; ++iRenderCache) {
    if ((pRope->GetSolidMaterial() ==
         m_aRenderCache[iRenderCache].m_pSolidMaterial) &&
        (pRope->GetBackMaterial() ==
         m_aRenderCache[iRenderCache].m_pBackMaterial))
      break;
  }
  ...
  m_aRenderCache[iRenderCache]
      .m_aCache[m_aRenderCache[iRenderCache].m_nCacheCount] = pRope;
  ++m_aRenderCache[iRenderCache].m_nCacheCount;
}

void CRopeManager::DrawRenderCache_NonQueued(
    bool bShadowDepth, RopeRenderData_t *pRenderCache, int nRenderCacheCount,
    const Vector &vCurrentViewForward, const Vector &vCurrentViewOrigin,
    C_RopeKeyframe::BuildRopeQueuedData_t *pBuildRopeQueuedData) {
  VPROF_BUDGET("CRopeManager::DrawRenderCache", VPROF_BUDGETGROUP_ROPES);
  AUTO_LOCK(m_RenderCacheMutex);  // contention cases: Toggling from queued mode
                                  // on to off. Rope deletion from the cache.

  // Check to see if we want to render the ropes.
  if (!r_drawropes.GetBool()) return;
  ...
  if (bShadowDepth && !m_pDepthWriteMaterial && g_pMaterialSystem) {
    ...
    m_pDepthWriteMaterial = g_pMaterialSystem->FindProceduralMaterial(
  ...
  for (int iRenderCache = 0; iRenderCache < nRenderCacheCount; ++iRenderCache) {
    ...
    for (int iCache = 0; iCache < nCacheCount; ++iCache) {
      ...
      if (pRope) {
        ...
        if (pBuildRopeQueuedData) {
          pRope->BuildRope(pRopeSegment, vCurrentViewForward,
                           vCurrentViewOrigin, pBuildRopeQueuedData);
          ...
          for (int i = 0; i != stackQueuedData.m_iNodeCount; ++i) {
            vStackPredictedPositions[i] =
                pRope->m_RopePhysics.GetNode(i)->m_vPredicted;
          }

          pRope->BuildRope(pRopeSegment, vCurrentViewForward,
                           vCurrentViewOrigin, &stackQueuedData);
        }
      } else {
        if (pBuildRopeQueuedData) {
          // we should only be here if a rope was in the queue and then deleted.
          // We still have it's relevant data (and need to skip over it).
          ++pBuildRopeQueuedData;
        }
      }
    }
    ...
    int nVertCount = 0;
    int nIndexCount = 0;
    for (int iSegmentCache = 0; iSegmentCache < m_nSegmentCacheCount;
         ++iSegmentCache) {
    ...
    bool bRenderNonSolid =
        !bShadowDepth &&
        ShouldUseFakeAA(pRenderCache[iRenderCache].m_pBackMaterial);
    if (bRenderNonSolid) {
      RenderNonSolidRopes(pRenderContext,
                          pRenderCache[iRenderCache].m_pBackMaterial,
                          nVertCount, nIndexCount);
    }

    // Render the solid portion of the ropes.
    if (rope_rendersolid.GetInt()) {
      if (bShadowDepth)
        RenderSolidRopes(pRenderContext, m_pDepthWriteMaterial, nVertCount,
                         nIndexCount, bRenderNonSolid);
      else
        RenderSolidRopes(pRenderContext,
                         pRenderCache[iRenderCache].m_pSolidMaterial,
                         nVertCount, nIndexCount, bRenderNonSolid);
    }
  }
  ResetSegmentCache(0);
...
void CRopeManager::DrawRenderCache(bool bShadowDepth) {
  ...
  if (iRenderCacheCount == 0) return;

  Vector vForward = CurrentViewForward();
  Vector vOrigin = CurrentViewOrigin();

  ICallQueue *pCallQueue;
  if (r_queued_ropes.GetBool() &&
      (pCallQueue = materials->GetRenderContext()->GetCallQueue()) != NULL) {
    ...
    if (iRopeCount == 0) return;  // nothing to draw

    size_t iMemoryNeeded =
        (iRenderCacheCount * sizeof(CRopeManager::RopeRenderData_t)) +
    ...
    CRopeManager::RopeRenderData_t *pRenderCachesStart =
        (CRopeManager::RopeRenderData_t *)pMemory;
    C_RopeKeyframe::BuildRopeQueuedData_t *pBuildRopeQueuedDataStart =
        (C_RopeKeyframe::BuildRopeQueuedData_t *)(pRenderCachesStart +
                                                  iRenderCacheCount);
    Vector *pVectorDataStart =
        (Vector *)(pBuildRopeQueuedDataStart + iRopeCount);

    // memcpy( pRenderCachesStart, m_aRenderCache.Base(), iRenderCacheCount *
    // sizeof( CRopeManager::RopeRenderData_t ) );
    ...
    C_RopeKeyframe::BuildRopeQueuedData_t *pWriteRopeQueuedData =
        pBuildRopeQueuedDataStart;
    Vector *pVectorWrite = (Vector *)pVectorDataStart;

    // Setup the rest of our data. This writes to two separate areas of memory
    // at the same time. One area for the C_RopeKeyframe::BuildRopeQueuedData_t
    // array, the other for mini-arrays of vector data
    for (int i = 0; i != iRenderCacheCount; ++i) {
      ...
      for (int j = 0; j != iCacheCount; ++j) {
        ...
        // make two arrays, one of predicted positions followed immediately by
        // light values
        ...
        pVectorWrite += iNodes;  // so we don't overwrite the light values with
                                 // the next rope's predicted positions
      }
    }
    Assert(((void *)pVectorWrite ==
            (void *)(((uint8_t *)pMemory) + iMemoryNeeded)) &&
           ((void *)pWriteRopeQueuedData == (void *)pVectorDataStart));
    pCallQueue->QueueCall(this, &CRopeManager::DrawRenderCache_NonQueued,
                          bShadowDepth, pRenderCachesStart, iRenderCacheCount,
                          vForward, vOrigin, pBuildRopeQueuedDataStart);
  } else {
    DrawRenderCache_NonQueued(bShadowDepth, m_aRenderCache.Base(),
                              iRenderCacheCount, vForward, vOrigin, NULL);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CRopeManager::RenderNonSolidRopes(IMatRenderContext *pRenderContext,
                                       IMaterial *pMaterial, int nVertCount,
                                       int nIndexCount) {
  // Render the solid portion of the ropes.
  ...
  for (int iSegmentCache = 0; iSegmentCache < m_nSegmentCacheCount;
    ...
    beamSegment.Start(pRenderContext, nSegmentCount, pMaterial, &meshBuilder,
                      nVerts);
...
// Purpose:
//-----------------------------------------------------------------------------
void CRopeManager::RenderSolidRopes(IMatRenderContext *pRenderContext,
                                    IMaterial *pMaterial, int nVertCount,
                                    int nIndexCount, bool bRenderNonSolid) {
  // Render the solid portion of the ropes.
  CMeshBuilder meshBuilder;
  IMesh *pMesh = pRenderContext->GetDynamicMesh(true, NULL, NULL, pMaterial);
  meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, nVertCount, nIndexCount);
  ...
  if (bRenderNonSolid) {
    ...
    for (int iSegmentCache = 0; iSegmentCache < m_nSegmentCacheCount;
      ...
      if (rope_solid_minalpha.GetFloat() == 0.0 &&
          pSegData->m_flMaxBackWidth <= rope_solid_minwidth.GetFloat())
        continue;
      ...
      for (int iSegment = 0; iSegment < nSegmentCount; ++iSegment) {
        ...
        // To avoid aliasing, the "solid" version of the rope on xbox is just
        // "more solid", and it has its own values controlling its alpha.
        pSeg->m_flAlpha = RemapVal(
            pSeg->m_flWidth, rope_solid_minwidth.GetFloat(),
...
void C_RopeKeyframe::CPhysicsDelegate::GetNodeForces(
    CSimplePhysics::CNode *pNodes, int iNode, Vector *pAccel) {
  // Gravity.
  if (!(m_pKeyframe->GetRopeFlags() & ROPE_NO_GRAVITY)) {
    pAccel->Init(ROPE_GRAVITY);
  }

  if (!m_pKeyframe->m_LinksTouchingSomething[iNode] &&
      m_pKeyframe->m_bApplyWind) {
    Vector vecWindVel;
    ...
    if (vecWindVel.LengthSqr() > 0) {
      ...
      if (m_pKeyframe->m_flCurrentGustTimer <
          m_pKeyframe->m_flCurrentGustLifetime) {
        float div = m_pKeyframe->m_flCurrentGustTimer /
                    m_pKeyframe->m_flCurrentGustLifetime;
...
void LockNodeDirection(CSimplePhysics::CNode *pNodes, int parity,
                       int nFalloffNodes, float flLockAmount,
                       float flLockFalloff, const Vector &vIdealDir) {
...
void C_RopeKeyframe::CPhysicsDelegate::ApplyConstraints(
    CSimplePhysics::CNode *pNodes, int nNodes) {
  VPROF("CPhysicsDelegate::ApplyConstraints");

  CTraceFilterWorldOnly traceFilter;

  // Collide with the world.
  if (((m_pKeyframe->m_RopeFlags & ROPE_COLLIDE) && rope_collide.GetInt()) ||
    ...
    for (int i = 0; i < nNodes; i++) {
      ...
      for (iIteration = 0; iIteration < nIterations; iIteration++) {
        trace_t trace;
        UTIL_TraceHull(pNode->m_vPrevPos, pNode->m_vPos, Vector(-2, -2, -2),
                       Vector(2, 2, 2), MASK_SOLID_BRUSHONLY, &traceFilter,
                       &trace);
        ...
        float distBehind =
            trace.plane.normal.Dot(pNode->m_vPos) - trace.plane.dist;
        pNode->m_vPos += trace.plane.normal * (-distBehind + 2.2);
        m_pKeyframe->m_LinksTouchingSomething[i] = true;
      }

      if (iIteration == nIterations) pNodes[i].m_vPos = pNodes[i].m_vPrevPos;
    }
  }
  ...
  if (m_pKeyframe->m_fLockedPoints & ROPE_LOCK_START_POINT) {
    ...
    if ((m_pKeyframe->m_fLockedPoints & ROPE_LOCK_START_DIRECTION) &&
        (nNodes > 3)) {
      Vector forward;
  ...
  if (m_pKeyframe->m_fLockedPoints & ROPE_LOCK_END_POINT) {
    ...
    if (m_pKeyframe->m_fLockedPoints & ROPE_LOCK_END_DIRECTION &&
        (nNodes > 3)) {
      Vector forward;
      AngleVectors(angles, &forward);
...
C_RopeKeyframe *C_RopeKeyframe::Create(C_BaseEntity *pStartEnt,
                                       C_BaseEntity *pEndEnt,
                                       int iStartAttachment, int iEndAttachment,
                                       float ropeWidth,
                                       const char *pMaterialName,
                                       int numSegments, int ropeFlags) {
...
C_RopeKeyframe *C_RopeKeyframe::CreateFromKeyValues(C_BaseAnimating *pEnt,
                                                    KeyValues *pValues) {
  C_RopeKeyframe *pRope = C_RopeKeyframe::Create(
      pEnt, pEnt, pEnt->LookupAttachment(pValues->GetString("StartAttachment")),
      pEnt->LookupAttachment(pValues->GetString("EndAttachment")),
      pValues->GetFloat("Width", 0.5), pValues->GetString("Material"),
      pValues->GetInt("NumSegments"), 0);
  ...
  if (pRope) {
    ...
    pRope->m_TextureScale =
        pValues->GetFloat("TextureScale", pRope->m_TextureScale);
    pRope->m_Slack = 0;
...
int C_RopeKeyframe::GetRopesIntersectingAABB(C_RopeKeyframe **pRopes,
                                             int nMaxRopes,
                                             const Vector &vAbsMin,
                                             const Vector &vAbsMax) {
  if (nMaxRopes == 0) return 0;
...
int C_RopeKeyframe::GetRopeFlags() const { return m_RopeFlags; }
...
void C_RopeKeyframe::SetupHangDistance(float flHangDist) {
  ...
  float flSlack, flLen;
  CalcRopeStartingConditions(v1, v2, ROPE_MAX_SEGMENTS, flHangDist, &flLen,
                             &flSlack);
...
void C_RopeKeyframe::SetEndEntity(C_BaseEntity *pEnt) { m_hEndPoint = pEnt; }

C_BaseEntity *C_RopeKeyframe::GetStartEntity() const { return m_hStartPoint; }

C_BaseEntity *C_RopeKeyframe::GetEndEntity() const { return m_hEndPoint; }

CSimplePhysics::IHelper *C_RopeKeyframe::HookPhysics(
    CSimplePhysics::IHelper *pHook) {
...
void C_RopeKeyframe::RecomputeSprings() {
  m_RopePhysics.ResetSpringLength(
      (m_RopeLength + m_Slack + ROPESLACK_FUDGEFACTOR) /
      (m_RopePhysics.NumNodes() - 1));
}

void C_RopeKeyframe::ShakeRope(const Vector &vCenter, float flRadius,
                               float flMagnitude) {
...
void C_RopeKeyframe::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType != DATA_UPDATE_CREATED) return;
...
void C_RopeKeyframe::FinishInit(const char *pMaterialName) {
  ...
  m_pBackMaterial =
      materials->FindMaterial(backName, TEXTURE_GROUP_OTHER, false);
  if (IsErrorMaterial(m_pBackMaterial)) m_pBackMaterial = NULL;
...
void C_RopeKeyframe::RunRopeSimulation(float flSeconds) {
  ...
  for (int i = 0; i < m_nSegments; i++) m_LinksTouchingSomething[i] = false;
  ...
  m_nLinksTouchingSomething = 0;
  for (int i = 0; i < m_nSegments; i++) {
    if (m_LinksTouchingSomething[i]) ++m_nLinksTouchingSomething;
  }
}

Vector C_RopeKeyframe::ConstrainNode(const Vector &vNormal,
                                     const Vector &vNodePosition,
                                     const Vector &vMidpiont,
                                     float fNormalLength) {
  ...
  float fMidpointToNodeProjectedLengh =
      VectorNormalize(vMidpointToNodeProjected);
  ...
  return vMidpiont + vMidpointToNode * fMidpointToNodeLengh *
                         (fNormalLength / fMidpointToNodeProjectedLengh);
}

void C_RopeKeyframe::ConstrainNodesBetweenEndpoints(void) {
  if (!m_bConstrainBetweenEndpoints) return;

  // Get midpoint and normals
  Vector vMidpiont =
      (m_vCachedEndPointAttachmentPos[0] + m_vCachedEndPointAttachmentPos[1]) /
      2.0f;
  Vector vNormal = vMidpiont - m_vCachedEndPointAttachmentPos[0];
  float fNormalLength = VectorNormalize(vNormal);

  // Loop through all the middle segments and ensure their positions are
  // constrained between the endpoints
  for (int i = 1; i < m_RopePhysics.NumNodes() - 1; ++i) {
    // Fix the current position
    m_RopePhysics.GetNode(i)->m_vPos = ConstrainNode(
        vNormal, m_RopePhysics.GetNode(i)->m_vPos, vMidpiont, fNormalLength);

    // Fix the predicted position
    m_RopePhysics.GetNode(i)->m_vPredicted =
        ConstrainNode(vNormal, m_RopePhysics.GetNode(i)->m_vPredicted,
                      vMidpiont, fNormalLength);
  }
}
...
int C_RopeKeyframe::DrawModel(int flags) {
  ...
  if (m_hStartPoint && m_hStartPoint->IsDormant() && m_hEndPoint &&
      m_hEndPoint->IsDormant()) {
...
bool C_RopeKeyframe::ShouldDraw() {
  if (!r_ropetranslucent.GetBool()) return false;

  if (!(m_RopeFlags & ROPE_SIMULATE)) return false;
...
bool C_RopeKeyframe::GetAttachment(int number, matrix3x4_t &matrix) {
  ...
  if (number == ROPE_ATTACHMENT_START_POINT) {
    ...
    vForward = m_RopePhysics.GetNode(0)->m_vPredicted -
               m_RopePhysics.GetNode(1)->m_vPredicted;
  } else {
    origin = m_RopePhysics.GetNode(nNodes - 1)->m_vPredicted;
    vForward = m_RopePhysics.GetNode(nNodes - 1)->m_vPredicted -
               m_RopePhysics.GetNode(nNodes - 2)->m_vPredicted;
  }
...
bool C_RopeKeyframe::GetAttachmentVelocity(int number, Vector &originVel,
                                           Quaternion &angleVel) {
  Assert(0);
  return false;
}
...
bool C_RopeKeyframe::AnyPointsMoved() {
  for (int i = 0; i < m_RopePhysics.NumNodes(); i++) {
    ...
    if (flMoveDistSqr > 0.03f) return true;
  }

  if (--m_iForcePointMoveCounter > 0) return true;
...
inline bool C_RopeKeyframe::DidEndPointMove(int iPt) {
  ...
  if (!(m_fLockedPoints & (1 << iPt))) return false;
  ...
  if (!bOld && !m_bPrevEndPointPos[iPt]) return true;

  // Register a change if the endpoint moves.
  if (!VectorsAreEqual(vOld, m_vPrevEndPointPos[iPt], 0.1)) return true;
...
bool C_RopeKeyframe::DetectRestingState(bool &bApplyWind) {
  ...
  if (m_fPrevLockedPoints != m_fLockedPoints) {
    // Force it to move the points for some number of frames when they get
    // detached or after we get new data. This allows them to accelerate from
    // gravity.
  ...
  if (DidEndPointMove(0) || DidEndPointMove(1)) return false;
...
// simple struct to precompute basis for catmull rom splines for faster
// evaluation
struct catmull_t {
  Vector t3;
  Vector t2;
...
void Catmull_Rom_Spline_Matrix(const Vector &p1, const Vector &p2,
                               const Vector &p3, const Vector &p4,
                               catmull_t &output) {
  output.t3 = 0.5f * ((-1 * p1) + (3 * p2) + (-3 * p3) +
                      p4);  // 0.5 t^3 * [ (-1*p1) + ( 3*p2) + (-3*p3) + p4 ]
  output.t2 = 0.5f * ((2 * p1) + (-5 * p2) + (4 * p3) -
                      p4);  // 0.5 t^2 * [ ( 2*p1) + (-5*p2) + ( 4*p3) - p4 ]
  output.t = 0.5f * ((-1 * p1) + p3);  // 0.5 t * [ (-1*p1) + p3 ]
  output.c = p2;                       // p2
}

// evaluate one point on the spline, t is a vector of (t, t^2, t^3)
inline void Catmull_Rom_Eval(const catmull_t &spline, const Vector &t,
                             Vector &output) {
...
// Purpose:
//-----------------------------------------------------------------------------
void C_RopeKeyframe::BuildRope(
    RopeSegData_t *pSegmentData, const Vector &vCurrentViewForward,
    const Vector &vCurrentViewOrigin,
    C_RopeKeyframe::BuildRopeQueuedData_t *pQueuedData) {
  if (!pSegmentData) return;

  // Get the lighting values.
  Vector *pLightValues = (mat_fullbright.GetInt() == 1)
                             ? g_FullBright_LightValues
                             : pQueuedData->m_pLightValues;
  ...
  for (int iNode = 0; iNode < nodeCount; ++iNode) {
    ...
    pSegmentData->m_Segments[nSegmentCount].m_vColor =
        pLightValues[iNode] * vColorMod;
    ++nSegmentCount;
    ...
    if (iNode < lastNode) {
      ...
      Vector vecColorInc =
          subdivScale *
          ((pLightValues[iNode + 1] - pLightValues[iNode]) * vColorMod);
      // precompute spline basis
      Catmull_Rom_Spline_Matrix(
          pPredictedPositions[iPrevNode], pPredictedPositions[iNode],
          pPredictedPositions[iNext], pPredictedPositions[iNextNext], spline);
      for (int iSubdiv = 0; iSubdiv < nSubdivCount; ++iSubdiv) {
        pSegmentData->m_Segments[nSegmentCount].m_vColor =
            pSegmentData->m_Segments[nSegmentCount - 1].m_vColor + vecColorInc;
        // simple eval using precomputed basis
        Catmull_Rom_Eval(spline, pSubdivVecList[iSubdiv],
                         pSegmentData->m_Segments[nSegmentCount].m_vPos);
  ...
  float flTotalTexCoord =
      flPixelsPerInch * (pQueuedData->m_RopeLength + pQueuedData->m_Slack +
                         ROPESLACK_FUDGEFACTOR);
  int nTotalPoints = (nodeCount - 1) * nSubdivCount + 1;
  float flActualInc = (flTotalTexCoord / nTotalPoints) / (float)m_TextureHeight;

  // First draw a translucent rope underneath the solid rope for an antialiasing
  // effect.
  if (ShouldUseFakeAA(m_pBackMaterial)) {
    ...
    for (int iSegment = 0; iSegment < nSegmentCount; ++iSegment) {
      ...
      // Right here, we need to specify a width that will be 1 pixel larger in
      // screen space.
      ...
      if (flScreenSpaceWidth < flMinScreenSpaceWidth) {
        ...
        pSegmentData->m_Segments[iSegment].m_flWidth =
            flMinScreenSpaceWidth * zCoord / flHalfScreenWidth;
        ...
        if (flScreenSpaceWidth > flMaxAlphaScreenSpaceWidth) {
          ...
          pSegmentData->m_Segments[iSegment].m_flAlpha =
              RemapVal(flScreenSpaceWidth, flMinScreenSpaceWidth,
                       flMaxAlphaScreenSpaceWidth, flMinAlpha, flMaxAlpha);
        }

        pSegmentData->m_Segments[iSegment].m_flWidth = m_Width;
        pSegmentData->m_BackWidths[iSegment] =
            m_Width - (zCoord * flExtraScreenSpaceWidth) / flScreenWidth;
...
bool C_RopeKeyframe::InitRopePhysics() {
  if (!(m_RopeFlags & ROPE_SIMULATE)) return 0;
  ...
  if (!m_bPrevEndPointPos[0]) return false;
  ...
  if (!m_bPrevEndPointPos[1]) m_vPrevEndPointPos[1] = m_vPrevEndPointPos[0];
...
bool C_RopeKeyframe::CalculateEndPointAttachment(C_BaseEntity *pEnt,
                                                 int iAttachment, Vector &vPos,
                                                 QAngle *pAngles) {
  VPROF_BUDGET("C_RopeKeyframe::CalculateEndPointAttachment",
               VPROF_BUDGETGROUP_ROPES);
  ...
  if (m_RopeFlags & ROPE_PLAYER_WPN_ATTACH) {
    ...
    if (pPlayer) {
      ...
      if (pAngles) return pModel->GetAttachment(iAttachment, vPos, *pAngles);
...
bool C_RopeKeyframe::GetEndPointPos(int iPt, Vector &vPos) {
  // By caching the results here, we avoid doing this a bunch of times per
  // frame.
  if (m_bEndPointAttachmentPositionsDirty) {
    CalculateEndPointAttachment(m_hStartPoint, m_iStartAttachment,
                                m_vCachedEndPointAttachmentPos[0], NULL);
    CalculateEndPointAttachment(m_hEndPoint, m_iEndAttachment,
                                m_vCachedEndPointAttachmentPos[1], NULL);
...
bool C_RopeKeyframe::GetEndPointAttachment(int iPt, Vector &vPos,
                                           QAngle &angle) {
  // By caching the results here, we avoid doing this a bunch of times per
  // frame.
  if (m_bEndPointAttachmentPositionsDirty || m_bEndPointAttachmentAnglesDirty) {
    CalculateEndPointAttachment(m_hStartPoint, m_iStartAttachment,
                                m_vCachedEndPointAttachmentPos[0],
                                &m_vCachedEndPointAttachmentAngle[0]);
    CalculateEndPointAttachment(m_hEndPoint, m_iEndAttachment,
                                m_vCachedEndPointAttachmentPos[1],
                                &m_vCachedEndPointAttachmentAngle[1]);
    m_bEndPointAttachmentPositionsDirty = false;
...
Vector *C_RopeKeyframe::GetRopeSubdivVectors(int *nSubdivs) {
  if (m_RopeFlags & ROPE_BARBED) {
    *nSubdivs = g_nBarbedSubdivs;
    ...
    if (subdiv >= MAX_ROPE_SUBDIVS) subdiv = MAX_ROPE_SUBDIVS - 1;
...
void C_RopeKeyframe::CalcLightValues() {
  ...
  for (int i = 0; i < m_RopePhysics.NumNodes(); i++) {
    ...
    if (!rope_averagelight.GetInt()) {
      // The engine averages the lighting across the 6 box faces, but we would
      // rather just get the MAX intensity since we do our own half-lambert
      // lighting in the rope shader to simulate directionality.
      //
      // So here, we take the average of all the incoming light, and scale it to
      // use the max intensity of all the box sides.
