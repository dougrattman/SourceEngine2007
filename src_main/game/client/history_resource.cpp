...
void CHudHistoryResource::AddToHistory(C_BaseCombatWeapon *weapon) {
  // don't draw exhaustable weapons (grenades) since they'll have an ammo pickup
  // icon as well
...
void CHudHistoryResource::AddToHistory(int iType, int iId, int iCount) {
  ...
  if (iType == HISTSLOT_AMMO) {
    ...
    // clear out any ammo pickup denied icons, since we can obviously pickup
    // again
    for (int i = 0; i < m_PickupHistory.Count(); i++) {
      if (m_PickupHistory[i].type == HISTSLOT_AMMODENIED &&
          m_PickupHistory[i].iId == iId) {
        // kill the old entry
...
//-----------------------------------------------------------------------------
void CHudHistoryResource::AddIconToHistory(int iType, int iId,
                                           C_BaseCombatWeapon *weapon,
                                           int iCount, CHudTexture *icon) {
  m_bNeedsDraw = true;

  // Check to see if the pic would have to be drawn too high. If so, start again
  // from the bottom
  ...
  if (iType == HISTSLOT_AMMODENIED) {
    freeslot->DisplayTime =
        gpGlobals->curtime + (hud_drawhistory_time.GetFloat() / 2.0f);
  } else {
    freeslot->DisplayTime =
        gpGlobals->curtime + hud_drawhistory_time.GetFloat();
  }
}
...
void CHudHistoryResource::MsgFunc_AmmoDenied(bf_read &msg) {
  ...
  for (int i = 0; i < m_PickupHistory.Count(); i++) {
    if (m_PickupHistory[i].type == HISTSLOT_AMMO &&
        m_PickupHistory[i].iId == iAmmo) {
...
void CHudHistoryResource::CheckClearHistory(void) {
  for (int i = 0; i < m_PickupHistory.Count(); i++) {
    if (m_PickupHistory[i].type) return;
  }
...
bool CHudHistoryResource::ShouldDraw(void) {
  ...
  return false;
#else
  return ((m_iCurrentHistorySlot > 0 || m_bNeedsDraw) &&
...
void CHudHistoryResource::Paint(void) {
  ...
  for (int i = 0; i < m_PickupHistory.Count(); i++) {
    if (m_PickupHistory[i].type) {
      m_PickupHistory[i].DisplayTime =
          std::min(m_PickupHistory[i].DisplayTime,
                   gpGlobals->curtime + hud_drawhistory_time.GetFloat());
      ...
      switch (m_PickupHistory[i].type) {
...
#ifndef HL2MP
          const FileWeaponInfo_t *pWpnInfo =
              gWR.GetWeaponFromAmmo(m_PickupHistory[i].iId);
          if (pWpnInfo &&
              (pWpnInfo->iMaxClip1 >= 0 || pWpnInfo->iMaxClip2 >= 0)) {
        ...
        case HISTSLOT_ITEM: {
          if (!m_PickupHistory[i].iId) continue;
      ...
      if (itemAmmoIcon) {
        itemAmmoIcon->DrawSelf(xpos - (itemAmmoIcon->Width() * 1.25f), ypos,
                               clr);
      }

      if (iAmount) {
        wchar_t text[16];
        _snwprintf(text, sizeof(text) / sizeof(wchar_t), L"%i",
                   m_PickupHistory[i].iCount);

        // offset the number to sit properly next to the icon
        ypos -=
            (surface()->GetFontTall(m_hNumberFont) - itemIcon->Height()) / 2;
