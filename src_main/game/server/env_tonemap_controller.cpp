...
ConVar mat_hdr_tonemapscale("mat_hdr_tonemapscale", "1.0", FCVAR_CHEAT,
                            ...
                            "eyes fully closed, 16 = eyes wide open.");
...
DEFINE_FIELD(m_flBlendTonemapStart, FIELD_FLOAT),
    ...
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetAutoExposureMin",
                     InputSetAutoExposureMin),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetAutoExposureMax",
                     InputSetAutoExposureMax),
    DEFINE_INPUTFUNC(FIELD_VOID, "UseDefaultAutoExposure",
                     InputUseDefaultAutoExposure),
    DEFINE_INPUTFUNC(FIELD_VOID, "UseDefaultBloomScale",
                     InputUseDefaultBloomScale),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetBloomScale", InputSetBloomScale),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetBloomScaleRange",
                     InputSetBloomScaleRange),
    END_DATADESC()

        IMPLEMENT_SERVERCLASS_ST(CEnvTonemapController, DT_EnvTonemapController)
            SendPropInt(SENDINFO(m_bUseCustomAutoExposureMin), 1,
                        SPROP_UNSIGNED),
...
void CEnvTonemapController::InputBlendTonemapScale(inputdata_t &inputdata) {
  ...
  if (!pszParam || !pszParam[0]) {
    Warning(
        "%s (%s) received BlendTonemapScale input without a target tonemap "
        "scale. Syntax: <target tonemap scale> <blend time>\n",
  ...
  if (!pszParam || !pszParam[0]) {
    Warning(
        "%s (%s) received BlendTonemapScale input without a blend time. "
        "Syntax: <target tonemap scale> <blend time>\n",
        GetClassname(), GetDebugName());
    return;
  }
...
void CEnvTonemapController::UpdateTonemapScaleBlend(void) {
  float flRemapped =
      RemapValClamped(gpGlobals->curtime, m_flBlendStartTime, m_flBlendEndTime,
                      m_flBlendTonemapStart, m_flBlendTonemapEnd);
  mat_hdr_tonemapscale.SetValue(flRemapped);

  // Msg("Setting tonemap scale to %f (curtime %f, %f -> %f)\n", flRemapped,
  // gpGlobals->curtime, m_flBlendStartTime, m_flBlendEndTime );

  // Stop when we're out of the blend range
  if (gpGlobals->curtime >= m_flBlendEndTime) return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CEnvTonemapController::InputUseDefaultAutoExposure(
