...
BEGIN_DATADESC(CEnvProjectedTexture)
  DEFINE_FIELD(m_hTargetEntity, FIELD_EHANDLE),
      ...
      DEFINE_INPUTFUNC(FIELD_BOOLEAN, "LightOnlyTarget",
                       InputSetLightOnlyTarget),
      DEFINE_INPUTFUNC(FIELD_BOOLEAN, "LightWorld", InputSetLightWorld),
      DEFINE_INPUTFUNC(FIELD_BOOLEAN, "EnableShadows", InputSetEnableShadows),
      // this is broken . . need to be able to set color and intensity like
      // light_dynamic
      //	DEFINE_INPUTFUNC( FIELD_COLOR32, "LightColor",
      // InputSetLightColor ),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "Ambient", InputSetAmbient),
      DEFINE_INPUTFUNC(FIELD_STRING, "SpotlightTexture",
                       InputSetSpotlightTexture),
      DEFINE_THINKFUNC(InitialThink),
END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CEnvProjectedTexture, DT_EnvProjectedTexture)
  SendPropEHandle(SENDINFO(m_hTargetEntity)), SendPropBool(SENDINFO(m_bState)),
      ...
      SendPropInt(SENDINFO(m_nShadowQuality), 1,
                  SPROP_UNSIGNED),  // Just one bit for now
END_SEND_TABLE()
...
CEnvProjectedTexture::CEnvProjectedTexture(void) {
  ...
  Q_strcpy(m_SpotlightTextureName.GetForModify(), m_SpotlightTextureName.Size(),
...
bool CEnvProjectedTexture::KeyValue(const char *szKeyName,
                                    const char *szValue) {
...
void CC_CreateFlashlight(const CCommand &args) {
  ...
  CEnvProjectedTexture *pFlashlight = dynamic_cast<CEnvProjectedTexture *>(
      CreateEntityByName("env_projectedtexture"));
  if (args.ArgC() > 1) {
...
static ConCommand create_flashlight("create_flashlight", CC_CreateFlashlight, 0,
                                    FCVAR_CHEAT);
