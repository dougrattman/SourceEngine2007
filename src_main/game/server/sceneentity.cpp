...
static ConVar scene_forcecombined(
    "scene_forcecombined", "0", 0,
    "When playing back, force use of combined .wav files even in english.");
static ConVar scene_maxcaptionradius(
    "scene_maxcaptionradius", "1200", 0,
    "Only show closed captions if recipient is within this many units of "
    "speaking actor (0==disabled).");

// Assume sound system is 100 msec lagged (only used if we can't find
// snd_mixahead cvar!)
...
static recentNPCSpeech_t speechListSounds[SPEECH_LIST_MAX_SOUNDS] = {
    {0, "", ""}, {0, "", ""}, {0, "", ""}, {0, "", ""}, {0, "", ""}};
...
//-----------------------------------------------------------------------------
// Purpose: This class exists solely to call think on all scene entities in a
// deterministic order
//-----------------------------------------------------------------------------
class CSceneManager : public CBaseEntity {
 ...
 public:
  ...
  void QueueRestoredSound(CBaseFlex *actor, char const *soundname,
                          soundlevel_t soundlevel, float time_in_past);

  void OnClientActive(CBasePlayer *player);

  void RemoveActorFromScenes(CBaseFlex *pActor, bool bInstancedOnly,
                             bool bNonIdleOnly, const char *pszThisSceneOnly);
  ...
  bool IsRunningScriptedSceneWithSpeechAndNotPaused(
      CBaseFlex *pActor, bool bIgnoreInstancedScenes);
...
BEGIN_DATADESC(CSceneManager)
  ...
  // DEFINE_FIELD( m_QueuedSceneSounds, CUtlVector < CRestoreSceneSound > ),  //
  // Don't save/restore this, it's created and used by OnRestore only
...
void LocalScene_Printf(const char *pFormat, ...) {
  ...
  Scene_Printf("%s", msg);
  ADD_DEBUG_HISTORY(HISTORY_SCENE_PRINT,
                    UTIL_VarArgs("(%0.2f) %s", gpGlobals->curtime, msg));
}
#endif
...
//-----------------------------------------------------------------------------
bool CopySceneFileIntoMemory(char const *pFilename, void **pBuffer,
                             int *pSize) {
...
// Purpose:
//-----------------------------------------------------------------------------
void FreeSceneFileMemory(void *buffer) { delete[] buffer; }
...
//-----------------------------------------------------------------------------
// Purpose: Singleton scene manager.  Created by first placed scene or recreated
// it it's deleted for some unknown reason Output : CSceneManager
//-----------------------------------------------------------------------------
...
class CSceneEntity : public CPointEntity, public IChoreoEventCallback {
 ...
 public:
  ...
  // If the scene is playing, finds an actor in the scene who can respond to the
  // specified concept token
  ...
  // Scene load/unload
  static CChoreoScene *LoadScene(const char *filename,
  ...
  static bool SpeakEventSoundLessFunc(const SpeakEventSound_t &lhs,
                                      const SpeakEventSound_t &rhs);

  bool GetSoundNameForPlayer(CChoreoEvent *event, CBasePlayer *player,
                             char *buf, size_t buflen);

  void BuildSortedSpeakEventSoundsPrefetchList(
      CChoreoScene *scene, CUtlSymbolTable &table,
      CUtlRBTree<SpeakEventSound_t> &soundnames, float timeOffset);
  void PrefetchSpeakEventSounds(CUtlSymbolTable &table,
                                CUtlRBTree<SpeakEventSound_t> &soundnames);

  // Event handlers
  virtual void DispatchStartExpression(CChoreoScene *scene, CBaseFlex *actor,
                                       CChoreoEvent *event);
  virtual void DispatchEndExpression(CChoreoScene *scene, CBaseFlex *actor,
                                     CChoreoEvent *event);
  virtual void DispatchStartFlexAnimation(CChoreoScene *scene, CBaseFlex *actor,
                                          CChoreoEvent *event);
  virtual void DispatchEndFlexAnimation(CChoreoScene *scene, CBaseFlex *actor,
                                        CChoreoEvent *event);
  virtual void DispatchStartGesture(CChoreoScene *scene, CBaseFlex *actor,
                                    CChoreoEvent *event);
  virtual void DispatchEndGesture(CChoreoScene *scene, CBaseFlex *actor,
                                  CChoreoEvent *event);
  virtual void DispatchStartLookAt(CChoreoScene *scene, CBaseFlex *actor,
                                   CBaseEntity *actor2, CChoreoEvent *event);
  virtual void DispatchEndLookAt(CChoreoScene *scene, CBaseFlex *actor,
                                 CChoreoEvent *event);
  virtual void DispatchStartMoveTo(CChoreoScene *scene, CBaseFlex *actor,
                                   CBaseEntity *actor2, CChoreoEvent *event);
  virtual void DispatchEndMoveTo(CChoreoScene *scene, CBaseFlex *actor,
                                 CChoreoEvent *event);
  virtual void DispatchStartSpeak(CChoreoScene *scene, CBaseFlex *actor,
                                  CChoreoEvent *event,
                                  soundlevel_t iSoundlevel);
  virtual void DispatchEndSpeak(CChoreoScene *scene, CBaseFlex *actor,
                                CChoreoEvent *event);
  virtual void DispatchStartFace(CChoreoScene *scene, CBaseFlex *actor,
                                 CBaseEntity *actor2, CChoreoEvent *event);
  virtual void DispatchEndFace(CChoreoScene *scene, CBaseFlex *actor,
                               CChoreoEvent *event);
  virtual void DispatchStartSequence(CChoreoScene *scene, CBaseFlex *actor,
                                     CChoreoEvent *event);
  virtual void DispatchEndSequence(CChoreoScene *scene, CBaseFlex *actor,
                                   CChoreoEvent *event);
  virtual void DispatchStartSubScene(CChoreoScene *scene, CBaseFlex *actor,
                                     CChoreoEvent *event);
  ...
  virtual void DispatchStartGeneric(CChoreoScene *scene, CBaseFlex *actor,
                                    CChoreoEvent *event);
  virtual void DispatchEndGeneric(CChoreoScene *scene, CBaseFlex *actor,
                                  CChoreoEvent *event);

  // NPC can play interstitial vcds (such as responding to the player doing
  // something during a scene)
  virtual void DispatchStartPermitResponses(CChoreoScene *scene,
                                            CBaseFlex *actor,
                                            CChoreoEvent *event);
  virtual void DispatchEndPermitResponses(CChoreoScene *scene, CBaseFlex *actor,
                                          CChoreoEvent *event);
  ...
  bool InvolvesActor(
      CBaseEntity *pActor);  // NOTE: returns false if scene hasn't loaded yet
 ...
 public:
  ...
  virtual CBaseEntity *FindNamedEntity(const char *name,
                                       ...
                                       bool bBaseFlexOnly = false,
                                       bool bUseClear = false);
  CBaseEntity *FindNamedTarget(string_t iszTarget, bool bBaseFlexOnly = false);
  virtual CBaseEntity *FindNamedEntityClosest(const char *name,
                                              CBaseEntity *pActor = NULL,
                                              bool bBaseFlexOnly = false,
                                              bool bUseClear = false,
                                              const char *pszSecondary = NULL);
 ...
 private:
  ...
  CChoreoScene *GenerateSceneForSound(CBaseFlex *pFlexActor,
  ...
  // Set if we tried to async the scene but the FS returned that the data was
  // not loadable
...
SendPropInt(SENDINFO(m_nSceneStringIndex), MAX_CHOREO_SCENES_STRING_BITS,
            SPROP_UNSIGNED),
    SendPropBool(SENDINFO(m_bIsPlayingBack)), SendPropBool(SENDINFO(m_bPaused)),
    ...
    DEFINE_FIELD(m_flAutomationTime,
                 FIELD_FLOAT),  // relative, not absolute time
    ...
    // DEFINE_FIELD( m_pRecipientFilter, IRecipientFilter* ),	// Multiplayer
    // only
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "CancelAtNextInterrupt",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "StopWaitingForActor",
...
// Output : CChoreoScene
//-----------------------------------------------------------------------------
CChoreoScene *CSceneEntity::GenerateSceneForSound(CBaseFlex *pFlexActor,
                                                  const char *soundname) {
  float duration = CBaseEntity::GetSoundDuration(
      soundname, pFlexActor ? STRING(pFlexActor->GetModelName()) : NULL);
  if (duration <= 0.0f) {
    Warning(
  ...
  if (!scene) {
    Warning(
        "CSceneEntity::GenerateSceneForSound:  Failed to allocated new "
        "scene!!!\n");
  } else {
    ...
    if (!actor || !channel || !event) {
      Warning(
          "CSceneEntity::GenerateSceneForSound:  Alloc of actor, channel, or "
          "event failed!!!\n");
      delete scene;
      return NULL;
    }
  ...
  for (int i = 0; i < scene->GetNumEvents(); i++) {
    ...
    switch (event->GetType()) {
      ...
      case CChoreoEvent::SPEAK: {
        ...
        // NOTE:  The script entries associated with .vcds are forced to preload
        // to avoid
...
//			*soundname -
//-----------------------------------------------------------------------------
void CSceneEntity::GenerateSoundScene(CBaseFlex *pActor,
                                      const char *soundname) {
  m_bGenerated = true;
...
bool CSceneEntity::HasUnplayedSpeech(void) {
  if (m_pScene) return m_pScene->HasUnplayedSpeech();
...
bool CSceneEntity::HasFlexAnimation(void) {
  if (m_pScene) return m_pScene->HasFlexAnimation();
...
bool CSceneEntity::IsBackground(void) {
  if (m_pScene) return m_pScene->IsBackground();
...
void CSceneEntity::OnRestore() {
  ...
  if (!m_pScene) {
    ...
    if (ShouldNetwork()) {
      m_nSceneStringIndex = g_pStringTableClientSideChoreoScenes->AddString(
          CBaseEntity::IsServer(), STRING(m_iszSceneFile));
    }
  ...
  m_pScene->ResetSimulation(true, m_flCurrentTime - SCENE_THINK_INTERVAL,
                            m_flCurrentTime);
  ...
  // All the variables are saved/restored properly, so we can safely leave them
  // alone.
  // ClearInterrupt();
...
void CSceneEntity::PauseThink(void) {
  ...
  // If entity I/O paused the scene, then it'll have to resume/cancel the
  // scene...
  ...
  // Otherwise, see if resume/cancel is automatic and act accordingly if enough
  // time
  //  has passed
...
// Purpose:
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchPauseScene(CChoreoScene *scene,
                                      const char *parameters) {
  // Don't pause during restore, since we'll be restoring the pause state
  // already
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchProcessLoop(CChoreoScene *scene,
                                       CChoreoEvent *event) {
  // Don't restore this event since it's implied in the current "state" of the
  // scene timer, etc.
  if (m_bRestoring) return;
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStopPoint(CChoreoScene *scene,
                                     const char *parameters) {
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartInterrupt(CChoreoScene *scene,
                                          CChoreoEvent *event) {
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndInterrupt(CChoreoScene *scene,
                                        CChoreoEvent *event) {
  // Don't re-interrupt during restore
  if (m_bRestoring) return;
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartExpression(CChoreoScene *scene,
                                           CBaseFlex *actor,
                                           CChoreoEvent *event) {
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndExpression(CChoreoScene *scene, CBaseFlex *actor,
                                         CChoreoEvent *event) {
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartFlexAnimation(CChoreoScene *scene,
                                              CBaseFlex *actor,
                                              CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event);
}
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndFlexAnimation(CChoreoScene *scene,
                                            CBaseFlex *actor,
                                            CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, false);
}
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartGesture(CChoreoScene *scene, CBaseFlex *actor,
                                        CChoreoEvent *event) {
  // Ingore 0 gestures
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndGesture(CChoreoScene *scene, CBaseFlex *actor,
                                      CChoreoEvent *event) {
  // Ingore 0 gestures
  if (!Q_stricmp(event->GetName(), "NULL")) return;
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartGeneric(CChoreoScene *scene, CBaseFlex *actor,
                                        CChoreoEvent *event) {
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndGeneric(CChoreoScene *scene, CBaseFlex *actor,
                                      CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, m_bRestoring);
}
...
//			*actor2 -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartLookAt(CChoreoScene *scene, CBaseFlex *actor,
                                       CBaseEntity *actor2,
                                       CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event, actor2);
}

void CSceneEntity::DispatchEndLookAt(CChoreoScene *scene, CBaseFlex *actor,
                                     CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, m_bRestoring);
}
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartMoveTo(CChoreoScene *scene, CBaseFlex *actor,
                                       CBaseEntity *actor2,
                                       CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event, actor2);
}

void CSceneEntity::DispatchEndMoveTo(CChoreoScene *scene, CBaseFlex *actor,
                                     CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, m_bRestoring);
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool AttenuateCaption(const char *token, const Vector &listener,
  ...
  float maxdistSqr =
      scene_maxcaptionradius.GetFloat() * scene_maxcaptionradius.GetFloat();
...
//-----------------------------------------------------------------------------
bool CSceneEntity::GetSoundNameForPlayer(CChoreoEvent *event,
                                         CBasePlayer *player, char *buf,
                                         size_t buflen) {
  Assert(event);
  Assert(player);
  Assert(buf);
  ...
  char const *cvarvalue =
      engine->GetClientConVarValue(player->entindex(), "english");
...
//			*parameters -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartSpeak(CChoreoScene *scene, CBaseFlex *actor,
                                      CChoreoEvent *event,
                                      soundlevel_t iSoundlevel) {
  // Emit sound
  if (actor) {
    ...
    if (m_bRestoring) {
      // Need to queue sounds on restore because the player has not yet
      // connected
      GetSceneManager()->QueueRestoredSound(actor, event->GetParameters(),
                                            iSoundlevel, time_in_past);

      return;
    }

    // Add padding to prevent any other talker talking right after I'm done,
    // because I might be continuing speaking with another scene.
    ...
    if (pBaseActor) {
      ...
      GetSpeechSemaphore(actor->MyNPCPointer())
          ->Acquire(flDuration + GetPostSpeakDelay(), actor);
    }
    ...
    for (int i = 0; i < c; ++i) {
      ...
      Q_strncpy(speechListSounds[speechListIndex].name, soundname,
                sizeof(speechListSounds[0].name));
      Q_strncpy(speechListSounds[speechListIndex].sceneName,
                (scene) ? scene->GetFilename() : "",
                sizeof(speechListSounds[0].sceneName));
    ...
    if (event->GetCloseCaptionType() == CChoreoEvent::CC_MASTER) {
      ...
      if (validtoken) {
        ...
        // Certain events are marked "don't attenuate", (breencast), skip those
        // here
        if (!event->IsSuppressingCaptionAttenuation() &&
          ...
          for (int i = c - 1; i >= 0; --i) {
            CBasePlayer *player =
                UTIL_PlayerByIndex(filter.GetRecipientIndex(i));
            if (!player) continue;

            Vector playerOrigin = player->GetAbsOrigin();

            if (AttenuateCaption(lowercase, playerOrigin,
                                 es.m_UtlVecSoundOrigin)) {
              // If the player has a view entity, measure the distance to that
              if (!player->GetViewEntity() ||
                  AttenuateCaption(lowercase,
                                   player->GetViewEntity()->GetAbsOrigin(),
                                   es.m_UtlVecSoundOrigin)) {
...
void CSceneEntity::DispatchEndSpeak(CChoreoScene *scene, CBaseFlex *actor,
                                    CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, m_bRestoring);
}
...
//			*actor2 -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartFace(CChoreoScene *scene, CBaseFlex *actor,
                                     CBaseEntity *actor2, CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event, actor2);
}
...
//			*actor2 -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndFace(CChoreoScene *scene, CBaseFlex *actor,
                                   CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, m_bRestoring);
}
...
// Input  : *actor -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartSequence(CChoreoScene *scene, CBaseFlex *actor,
                                         CChoreoEvent *event) {
  actor->AddSceneEvent(scene, event);
}
...
// Input  : *actor -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndSequence(CChoreoScene *scene, CBaseFlex *actor,
                                       CChoreoEvent *event) {
  actor->RemoveSceneEvent(scene, event, m_bRestoring);
}

//-----------------------------------------------------------------------------
// Purpose: NPC can play interstitial vcds (such as responding to the player
// doing something during a scene) Input  : *scene -
//			*actor -
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartPermitResponses(CChoreoScene *scene,
                                                CBaseFlex *actor,
                                                CChoreoEvent *event) {
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchEndPermitResponses(CChoreoScene *scene,
                                              CBaseFlex *actor,
                                              CChoreoEvent *event) {
...
bool CSceneEntity::InvolvesActor(CBaseEntity *pActor) {
  ...
  for (i = 0; i < m_pScene->GetNumActors(); i++) {
    ...
    if (pTestActor == pActor) return true;
  }
  return false;
}
...
void CSceneEntity::DoThink(float frametime) {
  ...
  // Msg( "SV:  %d, %f for %s\n", gpGlobals->tickcount, m_flCurrentTime,
  // m_pScene->GetFilename() );
...
void CSceneEntity::InputTriggerEvent(inputdata_t &inputdata) {
  CBaseEntity *pActivator =
      this;  // at some point, find this from the inputdata
...
void CSceneEntity::InputInterjectResponse(inputdata_t &inputdata) {
  ...
  if (!m_bIsPlayingBack) {
    ...
    char modifiers[512];
    Q_snprintf(modifiers, sizeof(modifiers), "scene:%s",
    ...
    for (int i = 0; i < c; i++) {
      ...
      AI_Response *response =
          npc->SpeakFindResponse(inputdata.value.String(), modifiers);
    ...
    if (rcount >= 1) {
      ...
      for (int i = 0; i < rcount; i++) {
        ...
        if (i == slot) {
          pInterjection->npc->SpeakDispatchResponse(inputdata.value.String(),
                                                    pInterjection->response);
        } else {
...
bool CSceneEntity::CheckActors() {
  ...
  for (i = 0; i < m_pScene->GetNumActors(); i++) {
    ...
    if (m_BusyActor == SCENE_BUSYACTOR_WAIT) {
      ...
      if (pActor) {
        ...
        if (hl2_episodic.GetBool()) {
          // Episodic waits until the NPC is fully finished with any .vcd with
          // speech in it
...
#ifdef HL2_EPISODIC
          // HACK: Alyx cannot play scenes when she's in the middle of
          // transitioning
          if (pActor->IsInAVehicle()) {
            CNPC_Alyx *pAlyx = dynamic_cast<CNPC_Alyx *>(pActor);
            if (pAlyx != NULL &&
                (pAlyx->GetPassengerState() == PASSENGER_STATE_ENTERING ||
    ...
    } else if (m_BusyActor == SCENE_BUSYACTOR_INTERRUPT ||
               m_BusyActor == SCENE_BUSYACTOR_INTERRUPT_CANCEL) {
...
#if !defined(_RETAIL)
static ConVar scene_async_prefetch_spew(
    "scene_async_prefetch_spew", "0", 0,
...
void CSceneEntity::PrefetchAnimBlocks(CChoreoScene *scene) {
  ...
  CUtlMap<CChoreoActor *, CBaseFlex *> actorMap(0, 0,
                                                DefLessFunc(CChoreoActor *));
              ...
              if (pStudioHdr) {
                ...
                for (int i = 0; i < seqdesc.groupsize[0]; ++i) {
                  for (int j = 0; j < seqdesc.groupsize[1]; ++j) {
                    ...
                    int baseanimation =
                        pStudioHdr->iRelativeAnim(seq, animation);
                    mstudioanimdesc_t &animdesc =
                        pStudioHdr->pAnimdesc(baseanimation);
...
void CSceneEntity::StartPlayback(void) {
  ...
  BuildSortedSpeakEventSoundsPrefetchList(m_pScene, prefetchSoundSymbolTable,
                                          soundnames, 0.0f);
...
//-----------------------------------------------------------------------------
bool CSceneEntity::SpeakEventSoundLessFunc(const SpeakEventSound_t &lhs,
                                           const SpeakEventSound_t &rhs) {
  return lhs.m_flStartTime < rhs.m_flStartTime;
}

//-----------------------------------------------------------------------------
// Purpose: Prefetches the list of sounds build by
// BuildSortedSpeakEventSoundsPrefetchList
//-----------------------------------------------------------------------------
void CSceneEntity::PrefetchSpeakEventSounds(
    CUtlSymbolTable &table, CUtlRBTree<SpeakEventSound_t> &soundnames) {
  for (int i = soundnames.FirstInorder(); i != soundnames.InvalidIndex();
       i = soundnames.NextInorder(i)) {
  ...
  for (int i = 0; i < scene->GetNumEvents(); i++) {
    ...
    switch (event->GetType()) {
      ...
      case CChoreoEvent::SPEAK: {
        ...
        // In single player, try to use the combined or regular .wav files as
        // needed
        if (gpGlobals->maxClients == 1) {
          CBasePlayer *player = UTIL_GetLocalPlayer();
          if (player && !GetSoundNameForPlayer(event, player, soundname,
                                               sizeof(soundname))) {
            // Skip to next event
            continue;
          }
        ...
        {
                // UNDONE:  Probably need some other solution in multiplayer...
        (not sure how to "prefetch" on certain players
                // with one sound, but not prefetch the same sound for
        others...)
        }
        */
      ...
      case CChoreoEvent::SUBSCENE: {
        ...
        if (!scene->IsSubScene()) {
          ...
          if (!subscene) {
            ...
            // Now precache it's resources, if any
            BuildSortedSpeakEventSoundsPrefetchList(subscene, table, soundnames,
                                                    event->GetStartTime());
          }
        }
      } break;
...
//-----------------------------------------------------------------------------
// Purpose: Start a resume scene, if we have one, and resume playing when it
// finishes
//-----------------------------------------------------------------------------
void CSceneEntity::QueueResumePlayback(void) {
  ...
  if (m_iszResumeSceneFile != NULL_STRING) {
    ...
    // If it has ".vcd" somewhere in the string, try using it as a scene file
    // first
    if (Q_stristr(STRING(m_iszResumeSceneFile), ".vcd")) {
      bStartedScene =
          InstancedScriptedScene(NULL, STRING(m_iszResumeSceneFile),
                                 &m_hWaitingForThisResumeScene, 0, false) != 0;
    }

    // HACKHACK: For now, get the first target, and see if we can find a
    // response for him
    if (!bStartedScene) {
      ...
      if (pActor) {
        ...
        if (pBaseActor) {
          AI_Response *result =
              pBaseActor->SpeakFindResponse(STRING(m_iszResumeSceneFile), NULL);
          if (result) {
            char response[256];
            result->GetResponse(response, sizeof(response));
            bStartedScene = InstancedScriptedScene(
                                NULL, response, &m_hWaitingForThisResumeScene,
                                0, false) != 0;
          }
        }
      }
...
//-----------------------------------------------------------------------------
// Purpose: Query whether the scene actually loaded. Only meaninful after
// Spawn()
//-----------------------------------------------------------------------------
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::DispatchStartSubScene(CChoreoScene *scene, CBaseFlex *pActor,
                                         CChoreoEvent *event) {
  if (!scene->IsSubScene()) {
    CChoreoScene *subscene = event->GetSubScene();
    if (!subscene) {
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::StartEvent(float currenttime, CChoreoScene *scene,
                              CChoreoEvent *event) {
  Assert(event);
  ...
  switch (event->GetType()) {
    ...
    case CChoreoEvent::SPEAK: {
      if (pActor) {
        ...
        if (event->GetParameters2()) {
          ...
          if (iSoundlevel == SNDLVL_NONE) iSoundlevel = SNDLVL_TALKING;
        }
    ...
    case CChoreoEvent::MOVETO: {
      ...
      if (pActor && !IsMultiplayer()) {
        ...
        if (event->GetParameters3() && strlen(event->GetParameters3()) > 0) {
          pActor2 =
              FindNamedEntityClosest(event->GetParameters(), pActor, false,
                                     true, event->GetParameters3());
        } else {
          pActor2 =
              FindNamedEntity(event->GetParameters(), pActor, false, true);
        }
        if (pActor2) {
    ...
    case CChoreoEvent::GENERIC: {
      // If the first token in the parameters is "debugtext", print the rest of
      // the text
      if (event->GetParameters() &&
          !Q_strncmp(event->GetParameters(), "debugtext", 9)) {
    ...
    case CChoreoEvent::FIRETRIGGER: {
      ...
      // Don't re-fire triggers during restore, the entities should already
      // reflect all such state...
      if (m_bRestoring) {
        break;
      }
...
//			*event -
//-----------------------------------------------------------------------------
void CSceneEntity::EndEvent(float currenttime, CChoreoScene *scene,
                            CChoreoEvent *event) {
  Assert(event);
  ...
  switch (event->GetType()) {
    ...
    case CChoreoEvent::GENERIC: {
      // If the first token in the parameters is "debugtext", we printed it and
      // we're done
      if (event->GetParameters() &&
          !Q_strncmp(event->GetParameters(), "debugtext", 9))
        break;
...
bool CSceneEntity::ShouldNetwork() const {
  if (m_bMultiplayer) {
    if (m_pScene && (m_pScene->HasEventsOfType(CChoreoEvent::FLEXANIMATION) ||
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
void CSceneEntity::ProcessEvent(float currenttime, CChoreoScene *scene,
                                CChoreoEvent *event) {
  switch (event->GetType()) {
    case CChoreoEvent::SUBSCENE: {
...
//-----------------------------------------------------------------------------
bool CSceneEntity::CheckEvent(float currenttime, CChoreoScene *scene,
                              CChoreoEvent *event) {
  switch (event->GetType()) {
    case CChoreoEvent::SUBSCENE: {
    } break;
...
CBaseFlex *CSceneEntity::FindNamedActor(int index) {
  ...
  if (!m_hActorList.IsValidIndex(index)) {
    DevWarning("Scene %s has %d actors, but scene entity only has %d actors\n",
               m_pScene->GetFilename(), m_pScene->GetNumActors(),
               m_hActorList.Size());
    return NULL;
  }
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CBaseEntity *CSceneEntity::FindNamedTarget(string_t iszTarget,
                                           bool bBaseFlexOnly) {
  if (!stricmp(STRING(iszTarget), "!activator")) return m_hActivator;

  // If we don't have a wildcard in the target, just return the first entity
  // found
  ...
  while ((pTarget = gEntList.FindEntityByName(pTarget, iszTarget)) != NULL) {
    if (bBaseFlexOnly) {
      // Make sure it can actually do facial animation, etc.
      if (dynamic_cast<CBaseFlex *>(pTarget)) return pTarget;
    } else {
      return pTarget;
    }
  }
...
class CSceneFindMarkFilter : public IEntityFindFilter {
 public:
  void SetActor(CBaseEntity *pActor) { m_hActor = pActor; }
  ...
  bool ShouldFindEntity(CBaseEntity *pEntity) {
    ...
    AI_TraceHull(vecOrigin, vecOrigin, m_hActor->WorldAlignMins(),
                 m_hActor->WorldAlignMaxs(), MASK_SOLID, m_hActor,
...
class CSceneFindNearestMarkFilter : public IEntityFindFilter {
 public:
  CSceneFindNearestMarkFilter(const CBaseEntity *pActor, const Vector &vecPos2,
                              float flMaxRadius = MAX_TRACE_LENGTH) {
    m_vecPos2 = vecPos2;
  ...
  bool ShouldFindEntity(CBaseEntity *pEntity) {
    ...
    if (!tr.startsolid || tr.m_pEnt == m_hActor) {
      ...
      NDebugOverlay::Text( pEntity->GetAbsOrigin() + Vector( 0, 0, 8 ), text,
      false, 5.0f );
      */
      ...
      // find that node that's closest to both, but the distance to it from the
      // actor isn't farther than the distance to the second node.  This should
      // keep the actor from walking past their point of interest
      if (dist1 <= m_flMaxSegmentDistance && dist2 <= m_flMaxSegmentDistance &&
          dist2 < m_flNearestToTarget) {
  ...
  CBaseEntity *GetFilterResult(void) {
    if (m_pNearestToTarget) return m_pNearestToTarget;
...
// Output : CBaseFlex
//-----------------------------------------------------------------------------
CBaseEntity *CSceneEntity::FindNamedEntity(const char *name,
                                           CBaseEntity *pActor,
                                           bool bBaseFlexOnly, bool bUseClear) {
  CBaseEntity *entity = NULL;

  if (!stricmp(name, "Player") || !stricmp(name, "!player")) {
    entity = (gpGlobals->maxClients == 1) ? (CBaseEntity *)UTIL_GetLocalPlayer()
                                          : NULL;
...
// Output : CBaseFlex
//-----------------------------------------------------------------------------
CBaseEntity *CSceneEntity::FindNamedEntityClosest(const char *name,
                                                  CBaseEntity *pActor,
                                                  bool bBaseFlexOnly,
                                                  bool bUseClear,
  ...
  if (pActor && pActor->MyNPCPointer()) {
    if (pszSecondary && strlen(pszSecondary) > 0) {
      CBaseEntity *pActor2 =
          FindNamedEntityClosest(pszSecondary, pActor, false, false, NULL);

      if (pActor2) {
        CSceneFindNearestMarkFilter *pFilter =
            new CSceneFindNearestMarkFilter(pActor, pActor2->GetAbsOrigin());
    ...
    for (iCount = 0; iCount < FINDNAMEDENTITY_MAX_ENTITIES; iCount++) {
      ...
      if (current) {
        if (RandomInt(0, iCount) == 0) entity = current;
      }
    }
...
//-----------------------------------------------------------------------------
// Purpose: If we are currently interruptable, pause this scene and wait for the
// other
//  scene to finish
// Input  : *otherScene -
//-----------------------------------------------------------------------------
...
void scene_interrupt( const CCommand &args )
{
        ...
        CSceneEntity *s1 = dynamic_cast< CSceneEntity * >(
gEntList.FindEntityByName( NULL, scene1 ) ); CSceneEntity *s2 = dynamic_cast<
CSceneEntity * >( gEntList.FindEntityByName( NULL, scene2 ) );
...
static ConCommand interruptscene( "int", scene_interrupt, "interrupt scene 1
with scene 2.", FCVAR_CHEAT );
*/
...
void CSceneEntity::CheckInterruptCompletion() {
  ...
  // If the interruptor goes away it's the same as having that scene finish
  // up...
  if (m_hInterruptScene != NULL && !m_bInterruptSceneFinished) {
    return;
  }
...
void CSceneEntity::RequestCompletionNotification(CSceneEntity *notify) {
  ...
  h = notify;
  // Only add it once
  if (m_hNotifySceneCompletion.Find(h) ==
      m_hNotifySceneCompletion.InvalidIndex()) {
    m_hNotifySceneCompletion.AddToTail(h);
  }
}

//-----------------------------------------------------------------------------
// Purpose: An interrupt scene has finished or been canceled, we can resume once
// we pick up this state in CheckInterruptCompletion Input  : *interruptor -
//-----------------------------------------------------------------------------
...
int CSceneEntity::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  ...
  if (m_pRecipientFilter && result != FL_EDICT_DONTSEND) {
    ...
    for (i = 0; i < m_pRecipientFilter->GetRecipientCount(); i++) {
      ...
      CBasePlayer *player =
          static_cast<CBasePlayer *>(CBaseEntity::Instance(iRecipient));
...
class CInstancedSceneEntity : public CSceneEntity {
 ...
 public:
  ...
  virtual void DispatchStartMoveTo(CChoreoScene *scene, CBaseFlex *actor,
                                   CBaseEntity *actor2, CChoreoEvent *event) {
    if (PassThrough(actor))
      BaseClass::DispatchStartMoveTo(scene, actor, actor2, event);
  };
  ...
  virtual void DispatchStartFace(CChoreoScene *scene, CBaseFlex *actor,
                                 CBaseEntity *actor2, CChoreoEvent *event) {
    if (PassThrough(actor))
      BaseClass::DispatchStartFace(scene, actor, actor2, event);
  };
  ...
  virtual void DispatchStartSequence(CChoreoScene *scene, CBaseFlex *actor,
                                     CChoreoEvent *event){/* suppress */};
  virtual void DispatchEndSequence(CChoreoScene *scene, CBaseFlex *actor,
                                   CChoreoEvent *event){/* suppress */};
  virtual void DispatchPauseScene(CChoreoScene *scene,
                                  const char *parameters){/* suppress */};
...
float InstancedScriptedScene(CBaseFlex *pActor, const char *pszScene,
                             EHANDLE *phSceneEnt, float flPostDelay,
                             bool bIsBackground, AI_Response *response,
                             bool bMultiplayer,
                             IRecipientFilter *filter /* = NULL */) {
  VPROF("InstancedScriptedScene");

  CInstancedSceneEntity *pScene =
      (CInstancedSceneEntity *)CBaseEntity::CreateNoSpawn(
          "instanced_scripted_scene", vec3_origin, vec3_angle);

  // This code expands any $gender tags into male or female tags based on the
  // gender of the actor (based on his/her .mdl)
  if (pActor) {
    pActor->GenderExpandString(pszScene, pScene->m_szInstanceFilename,
                               sizeof(pScene->m_szInstanceFilename));
  } else {
    Q_strncpy(pScene->m_szInstanceFilename, pszScene,
              sizeof(pScene->m_szInstanceFilename));
  }
...
float InstancedAutoGeneratedSoundScene(CBaseFlex *pActor, char const *soundname,
  ...
  if (!pActor) {
    Warning(
        "InstancedAutoGeneratedSoundScene:  Expecting non-NULL pActor for "
        "sound %s\n",
        soundname);
    return 0;
  }
  ...
  Q_strncpy(pScene->m_szInstanceFilename,
            UTIL_VarArgs("AutoGenerated(%s)", soundname),
            sizeof(pScene->m_szInstanceFilename));
  pScene->m_iszSceneFile = MAKE_STRING(pScene->m_szInstanceFilename);
...
void PrecacheInstancedScene(char const *pszScene) {
  ...
  if (!scenefilecache->GetSceneCachedData(pszScene, &sceneData)) {
    ...
    // A scene that is not in the pre-built cache image, but on disk, is a true
    // error.
    if (developer.GetInt() &&
        (IsX360() && (g_pFullFileSystem->GetDVDMode() != DVDMODE_STRICT) &&
         g_pFullFileSystem->FileExists(pszScene, "GAME"))) {
      Warning(
          "PrecacheInstancedScene: Missing scene '%s' from scene image "
    ...
    for (int i = 0; i < sceneData.numSounds; ++i) {
      short stringId =
          scenefilecache->GetSceneCachedSound(sceneData.sceneId, i);
      CBaseEntity::PrecacheScriptSound(
          scenefilecache->GetSceneString(stringId));
    }
  }

  g_pStringTableClientSideChoreoScenes->AddString(CBaseEntity::IsServer(),
                                                  pszScene);
}
...
//-----------------------------------------------------------------------------
// Purpose: Suppress certain events when it's instanced since they're can cause
// odd problems Input  : actor Output : true - the event should happen, false -
// it shouldn't
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
void CSceneManager::ClearAllScenes() { m_ActiveScenes.RemoveAll(); }
...
void CSceneManager::OnClientActive(CBasePlayer *player) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (fabs(1000.0f * sound->time_in_past) > MAX_SOUND_DELAY_MSEC) continue;
...
//-----------------------------------------------------------------------------
void CSceneManager::RemoveActorFromScenes(CBaseFlex *pActor,
                                          bool bInstancedOnly,
                                          bool bNonIdleOnly,
  ...
  for (int i = 0; i < c; i++) {
    ...
    // If only stopping instanced scenes, then skip it if it can't cast to an
    // instanced scene
    if (bInstancedOnly &&
    ...
    if (pScene->InvolvesActor(pActor)) {
      ...
      LocalScene_Printf("%s : removed for '%s'\n",
                        STRING(pScene->m_iszSceneFile),
                        pActor ? pActor->GetDebugName() : "NULL");
      pScene->CancelPlayback();
    }
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Return true if this Actor is only in scenes that are interruptable
// right now
//-----------------------------------------------------------------------------
bool CSceneManager::IsInInterruptableScenes(CBaseFlex *pActor) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (pScene->InvolvesActor(pActor) && pScene->IsPlayingBack()) {
      if (pScene->IsInterruptable() == false) return false;
    }
  }
...
//-----------------------------------------------------------------------------
void CSceneManager::QueueActorsScenesToResume(CBaseFlex *pActor,
                                              bool bInstancedOnly) {
  int c = m_ActiveScenes.Count();
  for (int i = 0; i < c; i++) {
    CSceneEntity *pScene = m_ActiveScenes[i].Get();
...
//-----------------------------------------------------------------------------
bool CSceneManager::IsRunningScriptedScene(CBaseFlex *pActor,
                                           bool bIgnoreInstancedScenes) {
  int c = m_ActiveScenes.Count();
  for (int i = 0; i < c; i++) {
    CSceneEntity *pScene = m_ActiveScenes[i].Get();
    if (!pScene || !pScene->IsPlayingBack() ||
        (bIgnoreInstancedScenes &&
         dynamic_cast<CInstancedSceneEntity *>(pScene) != NULL)) {
      continue;
    }
...
bool CSceneManager::IsRunningScriptedSceneAndNotPaused(
    CBaseFlex *pActor, bool bIgnoreInstancedScenes) {
  int c = m_ActiveScenes.Count();
  for (int i = 0; i < c; i++) {
    CSceneEntity *pScene = m_ActiveScenes[i].Get();
    if (!pScene || !pScene->IsPlayingBack() || pScene->IsPaused() ||
        (bIgnoreInstancedScenes &&
         dynamic_cast<CInstancedSceneEntity *>(pScene) != NULL)) {
      continue;
    }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSceneManager::IsRunningScriptedSceneWithSpeech(
    CBaseFlex *pActor, bool bIgnoreInstancedScenes) {
  int c = m_ActiveScenes.Count();
  for (int i = 0; i < c; i++) {
    CSceneEntity *pScene = m_ActiveScenes[i].Get();
    ...
    if (pScene->InvolvesActor(pActor)) {
      if (pScene->HasUnplayedSpeech()) return true;
    }
  }
  return false;
}
...
//			soundtime -
//-----------------------------------------------------------------------------
void CSceneManager::QueueRestoredSound(CBaseFlex *actor, char const *soundname,
                                       soundlevel_t soundlevel,
                                       float time_in_past) {
...
void RemoveActorFromScriptedScenes(CBaseFlex *pActor, bool instancedscenesonly,
  ...
  GetSceneManager()->RemoveActorFromScenes(pActor, instancedscenesonly,
                                           nonidlescenesonly, pszThisSceneOnly);
}
...
//-----------------------------------------------------------------------------
bool IsRunningScriptedScene(CBaseFlex *pActor, bool bIgnoreInstancedScenes) {
  return GetSceneManager()->IsRunningScriptedScene(pActor,
                                                   bIgnoreInstancedScenes);
}

bool IsRunningScriptedSceneAndNotPaused(CBaseFlex *pActor,
                                        bool bIgnoreInstancedScenes) {
  return GetSceneManager()->IsRunningScriptedSceneAndNotPaused(
...
bool IsRunningScriptedSceneWithSpeech(CBaseFlex *pActor,
                                      bool bIgnoreInstancedScenes) {
  return GetSceneManager()->IsRunningScriptedSceneWithSpeech(
      pActor, bIgnoreInstancedScenes);
}

bool IsRunningScriptedSceneWithSpeechAndNotPaused(CBaseFlex *pActor,
                                                  bool bIgnoreInstancedScenes) {
  return GetSceneManager()->IsRunningScriptedSceneWithSpeechAndNotPaused(
      pActor, bIgnoreInstancedScenes);
}
  ...
  if (gpGlobals->eLoadType != MapLoad_LoadGame) {
    for (int i = 0; i < SCENE_LIST_MANAGER_MAX_SCENES; i++) {
      if (m_iszScenes[i] != NULL_STRING) {
        ...
        if (m_hScenes[i]) {
          CSceneEntity *pScene =
              dynamic_cast<CSceneEntity *>(m_hScenes[i].Get());
          if (pScene) {
            pScene->AddListManager(this);
          } else {
            CSceneListManager *pList =
                dynamic_cast<CSceneListManager *>(m_hScenes[i].Get());
            if (pList) {
              pList->AddListManager(this);
            } else {
              Warning(
                  "%s(%s) found an entity that wasn't a "
                  "logic_choreographed_scene or logic_scene_list_manager in "
                  "slot %d, named %s\n",
          ...
          Warning("%s(%s) could not find scene %d, named %s\n", GetDebugName(),
                  GetClassname(), i, STRING(m_iszScenes[i]));
        }
      }
    }
  }
...
void CSceneListManager::SceneStarted(CBaseEntity *pSceneOrManager) {
  // Move backwards and call remove on all scenes / managers earlier in the list
  // to the fired one
  bool bFoundStart = false;
  for (int i = SCENE_LIST_MANAGER_MAX_SCENES - 1; i >= 0; i--) {
    if (!m_hScenes[i]) continue;
...
void CSceneListManager::RemoveScene(int iIndex) {
  ...
  CSceneListManager *pList =
      dynamic_cast<CSceneListManager *>(m_hScenes[iIndex].Get());
  if (pList) {
    pList->ShutdownList();
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Return a list of the last 5 lines of speech from NPCs for bug
// reports Input  : Output : speech - last 5 sound files played as speech
...
static void ListRecentNPCSpeech(void) {
  ...
  for (i = 0; i < num; i++) {
    Msg("   time: %6.3f   sound name: %s   scene: %s\n", speech[i].time,
        speech[i].name, speech[i].sceneName);
  }
  Msg("Current time: %6.3f\n", gpGlobals->curtime);
}

static ConCommand ListRecentNPCSpeechCmd(
    "listRecentNPCSpeech", ListRecentNPCSpeech,
    "Displays a list of the last 5 lines of speech from NPCs.",
    FCVAR_DONTRECORD | FCVAR_GAMEDLL);

CON_COMMAND(scene_flush,
