...
SendPropArray(SendPropString(SENDINFO_ARRAY(m_iszOverlayNames), 0,
                             SendProxy_String_tToString),
              m_iszOverlayNames),
    SendPropArray(SendPropFloat(SENDINFO_ARRAY(m_flOverlayTimes), 11,
                                SPROP_ROUNDDOWN, -1.0f, 63.0f),
                  m_flOverlayTimes),
...
// Purpose:
//-----------------------------------------------------------------------------
void CEnvScreenOverlay::Spawn(void) { Precache(); }
...
void CEnvScreenOverlay::Precache(void) {
  for (int i = 0; i < 10; i++) {
    if (m_iszOverlayNames[i] == NULL_STRING) continue;
...
void CEnvScreenOverlay::InputStartOverlay(inputdata_t &inputdata) {
  ...
  while ((pEnt = gEntList.FindEntityByClassname(pEnt, "env_screenoverlay")) !=
         NULL) {
    if (pEnt != this) {
