...
BEGIN_DATADESC(CAI_ScriptConditions)
  ...
  DEFINE_THINKFUNC(EvaluationThink),
...
#ifndef HL2_EPISODIC
      DEFINE_FIELD(m_hActor, FIELD_EHANDLE), DEFINE_EMBEDDED(m_Timer),
      ...
      DEFINE_KEYFIELD(m_flActorTargetProximity, FIELD_FLOAT,
                      "ActorTargetProximity"),
      DEFINE_EMBEDDED(m_ActorTargetProxTester),

      DEFINE_KEYFIELD(m_flPlayerTargetProximity, FIELD_FLOAT,
      ...
      DEFINE_KEYFIELD(m_bPlayerTargetFOVTrueCone, FIELD_BOOLEAN,
                      "PlayerTargetFOVTrueCone"),

      DEFINE_KEYFIELD(m_fPlayerTargetLOS, FIELD_INTEGER, "PlayerTargetLOS"),
      DEFINE_KEYFIELD(m_fPlayerBlockingActor, FIELD_INTEGER,
                      "PlayerBlockingActor"),
...
BEGIN_SIMPLE_DATADESC(CAI_ProxTester)
  DEFINE_FIELD(m_distSq, FIELD_FLOAT), DEFINE_FIELD(m_fInside, FIELD_BOOLEAN),
END_DATADESC()
...
CAI_ScriptConditions::EvaluatorInfo_t CAI_ScriptConditions::gm_Evaluators[] = {
    EVALUATOR(ActorSeePlayer),       EVALUATOR(State),
    EVALUATOR(PlayerActorProximity), EVALUATOR(PlayerTargetProximity),
    EVALUATOR(ActorTargetProximity), EVALUATOR(PlayerBlockingActor),
    EVALUATOR(PlayerActorLook),      EVALUATOR(PlayerTargetLook),
    EVALUATOR(ActorSeeTarget),       EVALUATOR(PlayerActorLOS),
    EVALUATOR(PlayerTargetLOS),

#ifdef HL2_EPISODIC
    EVALUATOR(ActorInPVS),           EVALUATOR(PlayerInVehicle),
...
void CAI_ScriptConditions::OnRestore(void) {
  ...
  if (m_ElementList.Count() == 0 && m_Actor == NULL_STRING &&
      m_fDisabled == false) {
    AddNewElement(NULL);
  }
#endif
...
bool CAI_ScriptConditions::EvalState(const EvalArgs_t &args) {
  ...
  if (valLow > valHigh) {
    DevMsg(
        "Script condition warning: Invalid setting for Maximum/Minimum "
        "state\n");
    Disable();
    return false;
  }
...
bool CAI_ScriptConditions::EvalActorSeePlayer(const EvalArgs_t &args) {
  ...
  bool fCanSeePlayer =
      args.pActor->MyNPCPointer()->HasCondition(COND_SEE_PLAYER);
...
#ifdef HL2_EPISODIC
    // This is the code we want to have written for HL2, but HL2 shipped without
    // the QuerySeeEntity() call. This #ifdef really wants to be something like
    // #ifndef HL2_RETAIL, since this change does want to be in any products
    // that are built henceforth. (sjb)
    bool fSee = pNPCActor->FInViewCone(args.pTarget) &&
                pNPCActor->FVisible(args.pTarget) &&
                pNPCActor->QuerySeeEntity(args.pTarget);
#else
    bool fSee = pNPCActor->FInViewCone(args.pTarget) &&
                pNPCActor->FVisible(args.pTarget);
#endif  // HL2_EPISODIC
...
bool CAI_ScriptConditions::EvalPlayerActorProximity(const EvalArgs_t &args) {
  return (!args.pActor ||
          m_PlayerActorProxTester.Check(args.pPlayer, args.pActor));
}
...
bool CAI_ScriptConditions::EvalPlayerActorLook(const EvalArgs_t &args) {
  return (!args.pActor || IsInFOV(args.pPlayer, args.pActor, m_flPlayerActorFOV,
...
bool CAI_ScriptConditions::EvalPlayerTargetLook(const EvalArgs_t &args) {
  return (!args.pTarget ||
          IsInFOV(args.pPlayer, args.pTarget, m_flPlayerTargetFOV,
...
bool CAI_ScriptConditions::EvalPlayerActorLOS(const EvalArgs_t &args) {
  ...
  return (!args.pActor || PlayerHasLineOfSight(args.pPlayer, args.pActor,
                                               m_fPlayerActorLOS == TRS_FALSE));
}
...
bool CAI_ScriptConditions::EvalPlayerTargetLOS(const EvalArgs_t &args) {
  ...
  return (!args.pTarget ||
          PlayerHasLineOfSight(args.pPlayer, args.pTarget,
                               m_fPlayerTargetLOS == TRS_FALSE));
}
...
bool CAI_ScriptConditions::EvalActorInPVS(const EvalArgs_t &args) {
  ...
  return (!args.pActor ||
          ActorInPlayersPVS(args.pActor, m_fActorInPVS == TRS_FALSE));
}

//-----------------------------------------------------------------------------

bool CAI_ScriptConditions::EvalPlayerBlockingActor(const EvalArgs_t &args) {
  if (m_fPlayerBlockingActor == TRS_NONE) return true;
  ...
  if (m_fPlayerBlockingActor == TRS_FALSE) return true;
...
bool CAI_ScriptConditions::EvalPlayerInVehicle(const EvalArgs_t &args) {
  // We don't care
  if (m_fPlayerInVehicle == TRS_NONE) return true;

  // Need a player to test
  if (args.pPlayer == NULL) return false;
...
void CAI_ScriptConditions::Spawn() {
  Assert((m_fMinState == NPC_STATE_IDLE || m_fMinState == NPC_STATE_COMBAT ||
          m_fMinState == NPC_STATE_ALERT) &&
         (m_fMaxState == NPC_STATE_IDLE || m_fMaxState == NPC_STATE_COMBAT ||
          m_fMaxState == NPC_STATE_ALERT));
...
void CAI_ScriptConditions::EvaluationThink() {
  ...
  if (AI_GetSinglePlayer()->GetFlags() & FL_NOTARGET) {
    ScrCondDbgMsg(
        ("%s WARNING: Player is NOTARGET. This will affect all LOS conditiosn "
  ...
  for (int i = 0; i < m_ElementList.Count();) {
    ...
    AssertMsg(!m_fDisabled, ("Violated invariant between CAI_ScriptConditions "
                             "disabled state and think func setting"));

    if (m_Actor != NULL_STRING && !pActor) {
      if (m_ElementList.Count() == 1) {
        DevMsg(
            "Warning: Active AI script conditions associated with an "
            "non-existant or destroyed NPC\n");
    ...
    if (m_flMinTimeout > 0 && pConditionElement->GetTimeOut()->Expired()) {
      ScrCondDbgMsg(("%s firing output OnConditionsTimeout (%f seconds)\n",
                     ...
                     pConditionElement->GetTimeOut()->GetInterval()));
    ...
    EvalArgs_t args = {pActor, GetPlayer(), m_hTarget.Get()};
    ...
    if (result) {
      ScrCondDbgMsg(("%s waiting... %f\n", GetDebugName(),
                     pConditionElement->GetTimer()->GetRemaining()));
    }
...
int CAI_ScriptConditions::AddNewElement(CBaseEntity *pActor) {
  ...
  if (m_flMaxTimeout > 0) {
    conditionelement.GetTimeOut()->Set(
        random->RandomFloat(m_flMinTimeout, m_flMaxTimeout), false);
  } else {
...
void CAI_ScriptConditions::Enable(void) {
  ...
  if (m_ElementList.Count() == 0) {
    if (m_Actor != NULL_STRING && pActor == NULL) {
      DevMsg(
          "Warning: Spawning AI script conditions (%s) associated with an "
          "non-existant NPC\n",
          GetDebugName());
      m_NoValidActors.FireOutput(this, this, 0);
      Disable();
...
bool CAI_ScriptConditions::IsInFOV(CBaseEntity *pViewer, CBaseEntity *pViewed,
                                   float fov, bool bTrueCone) {
  CBaseCombatCharacter *pCombatantViewer =
      (pViewer) ? pViewer->MyCombatCharacterPointer() : NULL;
...
bool CAI_ScriptConditions::PlayerHasLineOfSight(CBaseEntity *pViewer,
                                                CBaseEntity *pViewed,
                                                bool fNot) {
...
bool CAI_ScriptConditions::ActorInList(CBaseEntity *pActor) {
  for (int i = 0; i < m_ElementList.Count(); i++) {
    if (m_ElementList[i].GetActor() == pActor) return true;
  }
...
void CAI_ScriptConditions::OnEntitySpawned(CBaseEntity *pEntity) {
  if (m_fDisabled && m_bLeaveAsleep) {
    // Don't add elements if we're not currently running and don't want to
    // automatically wake up. Any spawning NPC's we miss during this time will
    // be found and added when manually Enabled().
    return;
  }

  if (pEntity->MyNPCPointer() == NULL) return;
