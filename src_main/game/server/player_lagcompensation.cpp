...
ConVar sv_maxunlag("sv_maxunlag", "1.0", FCVAR_DEVELOPMENTONLY,
                   "Maximum lag compensation in seconds", true, 0.0f, true,
                   1.0f);
ConVar sv_lagflushbonecache("sv_lagflushbonecache", "1", FCVAR_DEVELOPMENTONLY,
                            "Flushes entity bone cache on lag compensation");
ConVar sv_showlagcompensation(
    "sv_showlagcompensation", "0", FCVAR_CHEAT,
    "Show lag compensated hitboxes whenever a player is lag compensated.");

ConVar sv_unlag_fixstuck("sv_unlag_fixstuck", "0", FCVAR_DEVELOPMENTONLY,
                         "Disallow backtracking a player for lag compensation "
                         "if it will cause them to become stuck");
...
ConVar sv_unlag_debug("sv_unlag_debug", "0",
                      FCVAR_GAMEDLL | FCVAR_DEVELOPMENTONLY);
...
static void RestorePlayerTo(CBasePlayer *pPlayer, const Vector &vWantedPos) {
  ...
  UTIL_TraceEntity(pPlayer, vWantedPos, vWantedPos, MASK_PLAYERSOLID, pPlayer,
                   COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
  if (tr.startsolid || tr.allsolid) {
    if (sv_unlag_debug.GetBool()) {
      DevMsg(
          "RestorePlayerTo() could not restore player position for client "
          "\"%s\" ( %.1f %.1f %.1f )\n",
          pPlayer->GetPlayerName(), vWantedPos.x, vWantedPos.y, vWantedPos.z);
    }

    UTIL_TraceEntity(pPlayer, pPlayer->GetLocalOrigin(), vWantedPos,
                     MASK_PLAYERSOLID, pPlayer, COLLISION_GROUP_PLAYER_MOVEMENT,
                     &tr);
    if (tr.startsolid || tr.allsolid) {
      // In this case, the guy got stuck back wherever we lag compensated him
      // to. Nasty.
      ...
      Vector vPos;
      VectorLerp(pPlayer->GetLocalOrigin(), vWantedPos,
                 tr.fraction * g_flFractionScale, vPos);
...
class CLagCompensationManager : public CAutoGameSystemPerFrame,
 ...
 private:
  ...
  void ClearHistory() {
    for (int i = 0; i < MAX_PLAYERS; i++) m_PlayerTrack[i].Purge();
  }
  ...
  CBasePlayer
      *m_pCurrentPlayer;  // The player we are doing lag compensation for
};
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    for (int layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
      ...
      if (currentLayer) {
        ...
        record.m_layerRecords[layerIndex].m_sequence =
            currentLayer->m_nSequence;
...
void CLagCompensationManager::StartLagCompensation(CBasePlayer *player,
  ...
  if (fabs(deltaTime) > 0.2f) {
    // difference between cmd time and latency is too big > 200ms, use time
    // correction based on latency DevMsg("StartLagCompensation: delta too big
    // (%.3f)\n", deltaTime );
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    // Custom checks for if things should lag compensate (based on things like
    // what team the player is on).
    if (!player->WantsLagCompensationOnEntity(pPlayer, cmd,
                                              pEntityTransmitBits))
      continue;
...
void CLagCompensationManager::BacktrackPlayer(CBasePlayer *pPlayer,
                                              float flTargetTime) {
  ...
  float frac = 0.0f;
  if (prevRecord && (record->m_flSimulationTime < flTargetTime) &&
  ...
  if (sv_unlag_fixstuck.GetBool()) {
    ...
    if (tr.startsolid || tr.allsolid) {
      if (sv_unlag_debug.GetBool())
        DevMsg(
            "WARNING: BackupPlayer trying to back player into a bad position - "
            "client %s\n",
            pPlayer->GetPlayerName());
      ...
      UTIL_TraceEntity(pPlayer, pPlayer->GetLocalOrigin(), org,
                       MASK_PLAYERSOLID, &tr);
      ...
      if (tr.startsolid || tr.allsolid) {
        ...
        Vector vPos;
        VectorLerp(pPlayer->GetLocalOrigin(), org,
                   tr.fraction * g_flFractionScale, vPos);
  ...
  if (prevRecord &&
      (record->m_masterSequence == prevRecord->m_masterSequence)) {
    // If the master state changes, all layers will be invalid too, so don't
    // interp (ya know, interp barely ever happens anyway)
  ...
  if (frac > 0.0f && interpolationAllowed) {
    ...
    pPlayer->SetSequence(
        Lerp(frac, record->m_masterSequence, prevRecord->m_masterSequence));
    pPlayer->SetCycle(
        Lerp(frac, record->m_masterCycle, prevRecord->m_masterCycle));

    if (record->m_masterCycle > prevRecord->m_masterCycle) {
      // the older record is higher in frame than the newer, it must have
      // wrapped around from 1 back to 0 add one to the newer so it is lerping
      // from .9 to 1.1 instead of .9 to .1, for example.
      float newCycle =
          Lerp(frac, record->m_masterCycle, prevRecord->m_masterCycle + 1);
      pPlayer->SetCycle(
          newCycle < 1
              ? newCycle
              : newCycle - 1);  // and make sure .9 to 1.2 does not end up 1.05
    } else {
      pPlayer->SetCycle(
          Lerp(frac, record->m_masterCycle, prevRecord->m_masterCycle));
    }
  ...
  for (int layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
    ...
    if (currentLayer) {
      ...
      restore->m_layerRecords[layerIndex].m_sequence =
          currentLayer->m_nSequence;
      ...
      if ((frac > 0.0f) && interpolationAllowed) {
        ...
        LayerRecord &prevRecordsLayerRecord =
            prevRecord->m_layerRecords[layerIndex];
        if ((recordsLayerRecord.m_order == prevRecordsLayerRecord.m_order) &&
            (recordsLayerRecord.m_sequence ==
             prevRecordsLayerRecord.m_sequence)) {
          // We can't interpolate across a sequence or order change
          interpolated = true;
          if (recordsLayerRecord.m_cycle > prevRecordsLayerRecord.m_cycle) {
            // the older record is higher in frame than the newer, it must have
            // wrapped around from 1 back to 0 add one to the newer so it is
            // lerping from .9 to 1.1 instead of .9 to .1, for example.
            float newCycle = Lerp(frac, recordsLayerRecord.m_cycle,
                                  prevRecordsLayerRecord.m_cycle + 1);
            currentLayer->m_flCycle =
                newCycle < 1
                    ? newCycle
                    : newCycle -
                          1;  // and make sure .9 to 1.2 does not end up 1.05
          } else {
            currentLayer->m_flCycle = Lerp(frac, recordsLayerRecord.m_cycle,
                                           prevRecordsLayerRecord.m_cycle);
          }
          currentLayer->m_nOrder = recordsLayerRecord.m_order;
          currentLayer->m_nSequence = recordsLayerRecord.m_sequence;
          currentLayer->m_flWeight = Lerp(frac, recordsLayerRecord.m_weight,
                                          prevRecordsLayerRecord.m_weight);
        }
      }
      if (!interpolated) {
  ...
  if (!flags) return;  // we didn't change anything
...
void CLagCompensationManager::FinishLagCompensation(CBasePlayer *player) {
  ...
  if (!m_bNeedToRestore) return;  // no player was changed at all
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (restore->m_fFlags & LC_SIZE_CHANGED) {
      ...
      // see if simulation made any changes, if no, then do the restore,
      // otherwise,
      //  leave new values in
    ...
    if (restore->m_fFlags & LC_ANIMATION_CHANGED) {
      ...
      for (int layerIndex = 0; layerIndex < layerCount; ++layerIndex) {
        ...
        if (currentLayer) {
          ...
          currentLayer->m_nSequence =
              restore->m_layerRecords[layerIndex].m_sequence;
          currentLayer->m_flWeight =
              restore->m_layerRecords[layerIndex].m_weight;
        }
      }
    }
