...
#if defined( _DEBUG )
#define WALKED_AWAY_FROM_KEYBOARD_SECONDS 15.0f   // 15 seconds of movement ==
might be paused #else #define WALKED_AWAY_FROM_KEYBOARD_SECONDS 300.0f   // 5
minutes of no movement == might be paused #endif
*/
...
CBaseGameStats *gamestats =
    &s_GameStats_Singleton;  // start out pointing at the basic version which
...
class CBaseGameStats_Driver : public CAutoGameSystemPerFrame {
 public:
  ...
  bool m_bEnabled;  // false if incapable of uploading or the user doesn't want
                    // to enable stat tracking
...
void CBaseGameStats::StatsLog(char const *fmt, ...) {
  if (!m_bLogging && !m_bLoggingToFile) return;
  ...
  if (m_bLoggingToFile) {
    if (FILESYSTEM_INVALID_HANDLE == g_LogFileHandle) {
      g_LogFileHandle =
          filesystem->Open(GAMESTATS_LOG_FILE, "a", GAMESTATS_PATHID);
    }

    if (FILESYSTEM_INVALID_HANDLE != g_LogFileHandle) {
      filesystem->FPrintf(g_LogFileHandle, "[GS %s - %7.2f] %s", timeString,
                          gpGlobals->realtime, buf);
...
const char *CBaseGameStats::GetStatSaveFileName(void) {
  AssertMsg(s_szSaveFileName[0] != '\0',
            "Don't know what file to save stats to.");
  return s_szSaveFileName;
}

const char *CBaseGameStats::GetStatUploadRegistryKeyName(void) {
  AssertMsg(s_szStatUploadRegistryKeyName[0] != '\0',
            "Don't know the registry key to use to mark stats uploads.");
...
void CBaseGameStats::Event_MapChange(const char *szOldMapName,
...
void CBaseGameStats::Event_LevelInit(void) {
  ...
  BasicGameStatsRecord_t *map = gamestats->m_BasicStats.FindOrAddRecordForMap(
      CBGSDriver.m_PrevMapName.String());
  ++map->m_nCount;
  ...
  if (gpGlobals->maxClients == 1) {
    ...
    if (closecaption.IsValid()) SetCaptionsStatistic(closecaption.GetBool());
...
void CBaseGameStats::Event_LevelShutdown(float flElapsed) {
  BasicGameStatsRecord_t *map =
      m_BasicStats.FindOrAddRecordForMap(CBGSDriver.m_PrevMapName.String());
  Assert(map);
...
void CBaseGameStats::Event_PlayerKilled(CBasePlayer *pPlayer,
...
void CBaseGameStats::Event_Credits() {
  ...
  if (gamestats->m_BasicStats.m_nSecondsToCompleteGame == 0) {
    if (gamestats->UserPlayedAllTheMaps()) {
      gamestats->m_BasicStats.m_nSecondsToCompleteGame =
          elapsed + gamestats->m_BasicStats.m_Summary.m_nSeconds;
...
void CBaseGameStats::Event_PlayerTraveled(CBasePlayer *pBasePlayer,
                                          float distanceInInches,
                                          bool bInVehicle, bool bSprinting) {}

void CBaseGameStats::Event_FlippedVehicle(CBasePlayer *pDriver,
...
// Called before .sav file is actually loaded (player should still be in
// previous level, if any)
void CBaseGameStats::Event_PreSaveGameLoaded(char const *pSaveName,
                                             bool bInGame) {
  StatsLog("CBaseGameStats::Event_PreSaveGameLoaded [%s] %s\n", pSaveName,
           bInGame ? "in-game" : "at console");
}

bool CBaseGameStats::SaveToFileNOW(void) {
  if (!StatsTrackingIsFullyEnabled()) return false;
  ...
  if (filesystem->FileExists(GetStatSaveFileName(), GAMESTATS_PATHID)) {
    filesystem->RelativePathToFullPath(GetStatSaveFileName(), GAMESTATS_PATHID,
                                       fullpath, sizeof(fullpath));
  } else {
    // filename is local to game dir for Steam, so we need to prepend game dir
    // for regular file save
    char gamePath[256];
    engine->GetGameDir(gamePath, 256);
    Q_StripTrailingSlash(gamePath);
    Q_snprintf(fullpath, sizeof(fullpath), "%s/%s", gamePath,
               GetStatSaveFileName());
    Q_strlower(fullpath);
  ...
  if (CBGSDriver.m_bShuttingDown)  // write synchronously
  {
    ...
    // Write data async
    filesystem->AsyncWrite(fullpath, statsBuffer.Base(), statsBuffer.TellPut(),
                           true, false);
  }
...
void CBaseGameStats::Event_PlayerDamage(CBasePlayer *pBasePlayer,
                                        const CTakeDamageInfo &info) {
  // StatsLog( "CBaseGameStats::Event_PlayerDamage [%s] took %.2f damage\n",
  // pBasePlayer->GetPlayerName(), info.GetDamage() );
}

void CBaseGameStats::Event_PlayerKilledOther(CBasePlayer *pAttacker,
                                             CBaseEntity *pVictim,
                                             const CTakeDamageInfo &info) {
...
void CBaseGameStats::Event_WeaponFired(CBasePlayer *pShooter, bool bPrimary,
                                       char const *pchWeaponName) {
  StatsLog("CBaseGameStats::Event_WeaponFired [%s] %s weapon [%s]\n",
           pShooter->GetPlayerName(), bPrimary ? "primary" : "secondary",
           pchWeaponName);
}

void CBaseGameStats::Event_WeaponHit(CBasePlayer *pShooter, bool bPrimary,
                                     char const *pchWeaponName,
                                     const CTakeDamageInfo &info) {
  StatsLog("CBaseGameStats::Event_WeaponHit [%s] %s weapon [%s] damage [%f]\n",
           pShooter->GetPlayerName(), bPrimary ? "primary" : "secondary",
           pchWeaponName, info.GetDamage());
}
...
void CBaseGameStats::Event_DecrementPlayerEnteredNoClip(
    CBasePlayer *pBasePlayer) {
  StatsLog(
      "CBaseGameStats::Event_DecrementPlayerEnteredNoClip [%s] decrementing "
      "NOCLIPe\n",
      pBasePlayer->GetPlayerName());
}

void CBaseGameStats::Event_IncrementCountedStatistic(
    const Vector &vecAbsOrigin, char const *pchStatisticName,
    float flIncrementAmount) {
  StatsLog("Incrementing %s by %f at pos (%d, %d, %d)\n", pchStatisticName,
           flIncrementAmount, (int)vecAbsOrigin.x, (int)vecAbsOrigin.y,
           (int)vecAbsOrigin.z);
}
...
bool CBaseGameStats::UploadStatsFileNOW(void) {
  ...
  if (!filesystem->FileExists(gamestats->GetStatSaveFileName(),
                              GAMESTATS_PATHID)) {
    return false;
  }
...
bool CBaseGameStats::LoadFromFile(void) {
  if (filesystem->FileExists(gamestats->GetStatSaveFileName(),
                             GAMESTATS_PATHID)) {
    char fullpath[512];
    filesystem->RelativePathToFullPath(gamestats->GetStatSaveFileName(),
                                       GAMESTATS_PATHID, fullpath,
                                       sizeof(fullpath));
    StatsLog("Loading stats from '%s'\n", fullpath);
  }

  CUtlBuffer buf;
  if (filesystem->ReadFile(gamestats->GetStatSaveFileName(), GAMESTATS_PATHID,
                           buf)) {
    bool bRetVal = true;
    ...
    if (s_szPseudoUniqueID[0] != 0) {
      if (Q_stricmp(CBGSDriver.m_szLoadedUserID, s_szPseudoUniqueID)) {
        ...
        filesystem->RemoveFile(gamestats->GetStatSaveFileName(),
                               GAMESTATS_PATHID);
      ...
      if (version <= GAMESTATS_FILE_VERSION_OLD5) {
        ...
        // Peek ahead in buffer to see if we have the "no default stats" secret
        // flag set.
        int iCheckForStandardStatsInFile = *(int *)buf.PeekGet();
        bool bValid = true;

        if (iCheckForStandardStatsInFile != GAMESTATS_STANDARD_NOT_SAVED) {
          // the GAMESTATS_STANDARD_NOT_SAVED flag coincides with user
          // completion time, rewind so the gamestats parser can grab it
        ...
        if ((buf.TellPut() - buf.TellGet()) !=
            0)  // more data left, must be custom data
        {
...
bool CBaseGameStats_Driver::Init() {
  ...
  Q_strncpy(s_szStatUploadRegistryKeyName, "GameStatsUpload_",
            sizeof(s_szStatUploadRegistryKeyName));
  Q_strncat(s_szStatUploadRegistryKeyName, szLoweredGameDir,
            sizeof(s_szStatUploadRegistryKeyName));

  gamestats->m_bLoggingToFile =
      CommandLine()->FindParm("-gamestatsloggingtofile") ? true : false;
  if (gamestats->m_bLoggingToFile) {
    gamestats->m_bLogging = true;
  } else {
    gamestats->m_bLogging =
        CommandLine()->FindParm("-gamestatslogging") ? true : false;
  }
  ...
  if (gamestatsuploader) {
    ...
    if (m_bEnabled) {
      gamestatsuploader->GetPseudoUniqueId(s_szPseudoUniqueID,
                                           sizeof(s_szPseudoUniqueID));
    }
  }
  ...
  if (s_szPseudoUniqueID[0] != 0) {
    ...
    if (gamestats->AutoSave_OnInit()) gamestats->SaveToFileNOW();

    if (gamestats->AutoUpload_OnInit()) gamestats->UploadStatsFileNOW();
  } else {
    m_bEnabled =
        false;  // unable to generate a pseudo-unique ID, disable tracking
  }
...
void CBaseGameStats_Driver::Shutdown() {
  ...
  if (gamestats->AutoUpload_OnShutdown()) gamestats->UploadStatsFileNOW();
...
void CBaseGameStats_Driver::PossibleMapChange(void) {
  ...
  if (Q_stricmp(m_PrevMapName.String(), STRING(gpGlobals->mapname))) {
    ...
    gamestats->Event_MapChange(PrevMapBackup.String(),
                               STRING(gpGlobals->mapname));

    if (gamestats->AutoSave_OnMapChange()) gamestats->SaveToFileNOW();

    if (gamestats->AutoUpload_OnMapChange()) gamestats->UploadStatsFileNOW();
  }
}
...
void CBaseGameStats_Driver::LevelInitPreEntity() {
  ...
  if (gamestats->AutoSave_OnLevelInit()) gamestats->SaveToFileNOW();

  if (gamestats->AutoUpload_OnLevelInit()) gamestats->UploadStatsFileNOW();
}
...
void CBaseGameStats_Driver::LevelShutdownPreEntity() {
  ...
  if (flElapsed < 0.0f) {
    Assert(0);
    Warning(
        "EVENT_LEVELSHUTDOWN:  with negative elapsed time (rt %f starttime "
        "%f)\n",
  ...
  if (m_bInLevel && (gpGlobals->eLoadType != MapLoad_Background)) {
    ...
    if (gamestats->AutoSave_OnLevelShutdown()) gamestats->SaveToFileNOW();
...
bool UserCmdChanged(const CUserCmd &lhs, const CUserCmd &rhs) {
  if (lhs.viewangles != rhs.viewangles) return true;
  if (lhs.forwardmove != rhs.forwardmove) return true;
  if (lhs.sidemove != rhs.sidemove) return true;
  if (lhs.upmove != rhs.upmove) return true;
  if (lhs.buttons != rhs.buttons) return true;
  if (lhs.impulse != rhs.impulse) return true;
  if (lhs.weaponselect != rhs.weaponselect) return true;
  if (lhs.weaponsubtype != rhs.weaponsubtype) return true;
  if (lhs.mousedx != rhs.mousedx) return true;
  if (lhs.mousedy != rhs.mousedy) return true;
  return false;
}
...
void CBaseGameStats::Clear(void) { gamestats->m_BasicStats.Clear(); }
...
void CBaseGameStats::SetHL2UnlockedChapterStatistic(void) {
  // Now grab the hl2/cfg/config.cfg and suss out the sv_unlockedchapters cvar
...
static ConCommand resetGameStats(
    "_resetgamestats", CC_ResetGameStats,
...
void CPointGamestatsCounter::InputIncrement(inputdata_t &inputdata) {
  ...
  if (NULL_STRING == m_strStatisticName) {
    DevMsg(1,
           "CPointGamestatsCounter::InputIncrement:  No stat name specified "
           "for point_gamestats_counter @%f, %f, %f [ent index %d]\n",
           GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z, entindex());
    return;
  }

  gamestats->Event_IncrementCountedStatistic(
      GetAbsOrigin(), STRING(m_strStatisticName), inputdata.value.Float());
}
