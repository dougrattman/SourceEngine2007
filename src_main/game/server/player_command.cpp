...
extern CMoveData
    *g_pMoveData;  // This is a global because it is subclassed by each game.
...
//-----------------------------------------------------------------------------
// Purpose: We're about to run this usercmd for the specified player.  We can
// set up groupinfo and masking here, etc.
//  This is the time to examine the usercmd for anything extra.  This call
//  happens even if think does not.
// Input  : *player -
//			*cmd -
//-----------------------------------------------------------------------------
void CPlayerMove::StartCommand(CBasePlayer *player, CUserCmd *cmd) {
  ...
  for (i = 0; i < cmd->entitygroundcontact.Count(); i++) {
    ...
    CBaseEntity *pEntity =
        CBaseEntity::Instance(engine->PEntityOfEntIndex(entindex));
    if (pEntity) {
      CBaseAnimating *pAnimating = pEntity->GetBaseAnimating();
      if (pAnimating) {
        pAnimating->SetIKGroundContactInfo(
            cmd->entitygroundcontact[i].minheight,
            cmd->entitygroundcontact[i].maxheight);
      }
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Checks if the player is standing on a moving entity and adjusts
// velocity and
...
void CPlayerMove::CheckMovingGround(CBasePlayer *player, double frametime) {
  ...
  if (!(player->GetFlags() & FL_BASEVELOCITY)) {
    ...
    player->ApplyAbsVelocityImpulse((1.0 + (frametime * 0.5)) *
...
//			time -
//-----------------------------------------------------------------------------
void CPlayerMove::FinishMove(CBasePlayer *player, CUserCmd *ucmd,
                             CMoveData *move) {
...
//-----------------------------------------------------------------------------
void CPlayerMove::RunCommand(CBasePlayer *player, CUserCmd *ucmd,
  ...
  if (player->m_bGamePaused) {
    // If no clipping and cheats enabled and noclipduring game enabled, then
    // leave
    //  forwardmove and angles stuff in usercmd
    if (player->GetMoveType() == MOVETYPE_NOCLIP && sv_cheats->GetBool() &&
  ...
  /*
  // TODO:  We can check whether the player is sending more commands than
  elapsed real time cmdtimeremaining -= ucmd->msec; if ( cmdtimeremaining < 0 )
  {
  //	return;
  }
  */
  ...
  if (ucmd->weaponselect != 0) {
    CBaseCombatWeapon *weapon = dynamic_cast<CBaseCombatWeapon *>(
