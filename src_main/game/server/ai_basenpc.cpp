...
ConVar ai_debug_dyninteractions("ai_debug_dyninteractions", "0", FCVAR_NONE,
                                "Debug the NPC dynamic interaction system.");
ConVar ai_frametime_limit(
    "ai_frametime_limit", "50", FCVAR_NONE,
...
#ifndef _RETAIL
#define ShouldUseEfficiency() \
  (ai_use_think_optimizations.GetBool() && ai_use_efficiency.GetBool())
#define ShouldUseFrameThinkLimits() \
  (ai_use_think_optimizations.GetBool() && ai_use_frame_think_limits.GetBool())
#define ShouldRebalanceThinks() \
  (ai_use_think_optimizations.GetBool() && ai_rebalance_thinks.GetBool())
#define ShouldDefaultEfficient() \
  (ai_use_think_optimizations.GetBool() && ai_default_efficient.GetBool())
#else
...
ConVar ai_spread_defocused_cone_multiplier(
...
ConVar ai_strong_optimizations("ai_strong_optimizations",
                               (IsX360()) ? "1" : "0");
bool AIStrongOpt(void) { return ai_strong_optimizations.GetBool(); }
...
void CPostFrameNavigationHook::FrameUpdatePreEntityThink(void) {
  ...
  if (ai_post_frame_navigation.GetBool() == false) return;

  SetGrameFrameRunning(true);
}

//-----------------------------------------------------------------------------
// Purpose: Now that the game frame has collected all the navigation queries,
// service them
//-----------------------------------------------------------------------------
void CPostFrameNavigationHook::FrameUpdatePostEntityThink(void) {
  if (ai_post_frame_navigation.GetBool() == false) return;

  // The guts of the NPC will check against this to decide whether or not to
  // queue its navigation calls
  SetGrameFrameRunning(false);

  // Throw this off to a thread job
  g_pQueuedNavigationQueryJob = ThreadExecute(
      &ProcessNavigationQueries, m_Functors.Base(), m_Functors.Count());
}

//-----------------------------------------------------------------------------
// Purpose: Queue up our navigation call
//-----------------------------------------------------------------------------
void CPostFrameNavigationHook::EnqueueEntityNavigationQuery(
    CAI_BaseNPC *pNPC, CFunctor *pFunctor) {
  if (ai_post_frame_navigation.GetBool() == false) return;
...
Activity CAI_BaseNPC::GetFlinchActivity(bool bHeavyDamage, bool bGesture) {
  ...
  switch (LastHitGroup()) {
    ...
    case HITGROUP_STOMACH:
      flinchActivity =
          bGesture ? ACT_GESTURE_FLINCH_STOMACH : ACT_FLINCH_STOMACH;
      break;
    case HITGROUP_LEFTARM:
      flinchActivity =
          bGesture ? ACT_GESTURE_FLINCH_LEFTARM : ACT_FLINCH_LEFTARM;
      break;
    case HITGROUP_RIGHTARM:
      flinchActivity =
          bGesture ? ACT_GESTURE_FLINCH_RIGHTARM : ACT_FLINCH_RIGHTARM;
      break;
    case HITGROUP_LEFTLEG:
      flinchActivity =
          bGesture ? ACT_GESTURE_FLINCH_LEFTLEG : ACT_FLINCH_LEFTLEG;
      break;
    case HITGROUP_RIGHTLEG:
      flinchActivity =
          bGesture ? ACT_GESTURE_FLINCH_RIGHTLEG : ACT_FLINCH_RIGHTLEG;
      break;
    case HITGROUP_CHEST:
...
void CAI_BaseNPC::SelectDeathPose(const CTakeDamageInfo &info) {
  ...
  SelectDeathPoseActivityAndFrame(this, info, LastHitGroup(), aActivity,
                                  iDeathFrame);
...
void CAI_BaseNPC::Event_Killed(const CTakeDamageInfo &info) {
  ...
  // Make sure this condition is fired too (OnTakeDamage breaks out before this
  // happens on death)
  ...
  // Adrian - Only set this if we are going to become a ragdoll. We still want
  // to select SCHED_DIE or do something special when this NPC dies and we wont
  ...
  // else has some special reason for us to vanish instead of creating a
  // ragdoll. i.e. The barnacle does this because it's already got a ragdoll for
  // us.
...
void CAI_BaseNPC::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
...
bool CAI_BaseNPC::PassesDamageFilter(const CTakeDamageInfo &info) {
  ...
  if ((CapabilitiesGet() & bits_CAP_FRIENDLY_DMG_IMMUNE) &&
    ...
    CBaseCombatCharacter *npcEnemy =
        info.GetAttacker()->MyCombatCharacterPointer();
...
int CAI_BaseNPC::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  // See if we're running a dynamic interaction that should break when I am
  // damaged.
  ...
  if (m_iHealth <= 0 && HasInteractionCantDie() &&
      info.GetAttacker() != m_hInteractionPartner) {
    m_iHealth = 1;
  }
  ...
  if ((info.GetDamageType() & DMG_CRUSH) &&
      !(info.GetDamageType() & DMG_PHYSGUN) &&
      info.GetDamage() >= MIN_PHYSICS_FLINCH_DAMAGE) {
  ...
  if (info.GetAttacker()->GetFlags() & (FL_NPC | FL_CLIENT)) {
    ...
    if (!FInViewCone(info.GetAttacker()) || !FVisible(info.GetAttacker())) {
      ...
      // ----------------------------------------------------------------
      if (GetEnemy() != NULL && !GetEnemies()->HasMemory(info.GetAttacker()) &&
  ...
  // ---------------------------------------------------------------
  CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(), 1024, 0.5, this,
...
int CAI_BaseNPC::OnTakeDamage_Dead(const CTakeDamageInfo &info) {
  ...
  // grab the vector of the incoming attack. ( pretend that the inflictor is a
  // little lower than it really is, so the body will tend to fly upward a bit).
  vecDir = vec3_origin;
  if (info.GetInflictor()) {
    vecDir = info.GetInflictor()->WorldSpaceCenter() - Vector(0, 0, 10) -
             WorldSpaceCenter();
    ...
    for (int i = 0; i < g_AI_Manager.NumAIs(); i++) {
      ...
      if (pNpc && pNpc != this) {
        ...
        if (fabsf(originNpc.z - origin.z) < NEAR_Z) {
          if ((originNpc.AsVector2D() - origin.AsVector2D()).LengthSqr() <
              NEAR_XY_SQ) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseNPC::OnFriendDamaged(CBaseCombatCharacter *pSquadmate,
                                  CBaseEntity *pAttacker) {
  if (GetSleepState() != AISS_WAITING_FOR_INPUT) {
    float distSqToThreat =
        (GetAbsOrigin() - pAttacker->GetAbsOrigin()).LengthSqr();
...
bool CAI_BaseNPC::IsLightDamage(const CTakeDamageInfo &info) {
  // ALL nonzero damage is light damage! Mask off COND_LIGHT_DAMAGE if you want
  // to ignore light damage.
...
void CAI_BaseNPC::DoRadiusDamage(const CTakeDamageInfo &info,
                                 const Vector &vecSrc, int iClassIgnore,
                                 CBaseEntity *pEntityIgnore) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_BaseNPC::ImpactTrace(trace_t *pTrace, int iDamageType,
...
//---------------------------------------------------------
float CAI_BaseNPC::GetHitgroupDamageMultiplier(int iHitGroup,
...
// TraceAttack
//=========================================================
void CAI_BaseNPC::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                              trace_t *ptr) {
  m_fNoDamageDecal = false;
  ...
  if (subInfo.GetDamage() >= 1.0 && !(subInfo.GetDamageType() & DMG_SHOCK)) {
    if (!IsPlayer() || (IsPlayer() && g_pGameRules->IsMultiplayer())) {
      ...
      SpawnBlood(ptr->endpos, vecDir, BloodColor(),
                 subInfo.GetDamage());  // a little surface blood.
    }
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::PointInSpread(CBaseCombatCharacter *pCheckEntity,
                                const Vector &sourcePos,
                                const Vector &targetPos,
                                const Vector &testPoint, float flSpread,
                                float maxDistOffCenter) {
  float distOffLine = CalcDistanceToLine2D(
      testPoint.AsVector2D(), sourcePos.AsVector2D(), targetPos.AsVector2D());
    ...
    if (distTarget > distTest) {
      ...
      if (dotProduct > flSpread) {
        ...
        if (pCheckEntity) {
          ...
          // !!!BUGBUG - this 2d check will stop a citizen shooting at a gunship
          // or strider if another citizen is between them, even though the
          // strider or gunship may be high up in the air (sjb)
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::PlayerInSpread(const Vector &sourcePos,
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer && (!ignoreHatedPlayers || IRelationType(pPlayer) != D_HT)) {
      if (PointInSpread(pPlayer, sourcePos, targetPos,
                        pPlayer->WorldSpaceCenter(), flSpread,
                        maxDistOffCenter))
        return true;
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
CBaseEntity *CAI_BaseNPC::PlayerInRange(const Vector &vecLocation,
                                        float flDist) {
  // loop through all players
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    CBasePlayer *pPlayer = UTIL_PlayerByIndex(i);
    ...
    if (pPlayer &&
        (vecLocation - pPlayer->WorldSpaceCenter()).Length2D() <= flDist) {
      return pPlayer;
    }
  }
  return NULL;
...
void BulletWizz(Vector vecSrc, Vector vecEndPos, edict_t *pShooter,
                bool isTracer) {
  CBasePlayer *pPlayer;
...
class CTriggerTraceEnum : public IEntityEnumerator {
 public:
  CTriggerTraceEnum(Ray_t *pRay, const CTakeDamageInfo &info, const Vector &dir,
                    int contentsMask)
      : m_info(info),
        m_VecDir(dir),
...
void CBaseEntity::TraceAttackToTriggers(const CTakeDamageInfo &info,
...
//			iTracerType -
//-----------------------------------------------------------------------------
void CAI_BaseNPC::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr,
                             int iTracerType) {
  if (GetActiveWeapon()) {
...
#ifdef HL2_DLL
  // If we're shooting at a bullseye, become perfectly accurate if the bullseye
  // demands it
...
void CBaseEntity::HandleShotImpactingGlass(const FireBulletsInfo_t &info,
  ...
  UTIL_TraceLine(testPos, tr.endpos, MASK_SHOT, pTraceFilter,
                 &penetrationTrace);
...
void CBaseEntity::CreateBubbleTrailTracer(const Vector &vecShotSrc,
...
//=========================================================
//=========================================================
void CAI_BaseNPC::MakeDamageBloodDecal(int cCount, float flNoise, trace_t *ptr,
                                       Vector vecDir) {
  ...
  for (i = 0; i < cCount; i++) {
    ...
    AI_TraceLine(ptr->endpos, ptr->endpos + vecTraceDir * 172,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &Bloodtr);
...
#define InterruptFromCondition(iCondition)                                 \
  AI_RemapFromGlobal(                                                      \
      (AI_IdIsLocal(iCondition)                                            \
           ? GetClassScheduleIdSpace()->ConditionLocalToGlobal(iCondition) \
           : iCondition))
...
bool CAI_BaseNPC::HasCondition(int iCondition, bool bUseIgnoreConditions) {
  if (bUseIgnoreConditions) return HasCondition(iCondition);
...
bool CAI_BaseNPC::HasInterruptCondition(int iCondition) {
  ...
  return (m_Conditions.IsBitSet(interrupt) &&
          GetCurSchedule()->HasInterrupt(interrupt));
}
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CAI_BaseNPC::ConditionInterruptsSchedule(int localScheduleID,
                                              int iCondition) {
...
//-----------------------------------------------------------------------------
void CAI_BaseNPC::SetScriptedScheduleIgnoreConditions(
...
//-----------------------------------------------------------------------------
// Purpose: Clears a flag in the custom interrupt flags, translating the
// condition
//			to the proper global space, if necessary
//-----------------------------------------------------------------------------
...
bool CAI_BaseNPC::QueryHearSound(CSound *pSound) {
  if (pSound->SoundContext() & SOUND_CONTEXT_COMBINE_ONLY) return false;

  if (pSound->SoundContext() & SOUND_CONTEXT_ALLIES_ONLY) {
    if (!IsPlayerAlly()) return false;
  }

  if (pSound->IsSoundType(SOUND_PLAYER) && GetState() == NPC_STATE_IDLE &&
      !FVisible(pSound->GetSoundReactOrigin())) {
    // NPC's that are IDLE should disregard player movement sounds if they can't
    // see them. This does not affect them hearing the player's weapon.
    // !!!BUGBUG - this probably makes NPC's not hear doors opening, because
    // doors opening put SOUND_PLAYER in the world, but the door's model will
    // block the FVisible() trace and this code will then deduce that the sound
    // can not be heard
    return false;
  }
  ...
  if (pSound->IsSoundType(SOUND_COMBAT) &&
      pSound->SoundChannel() == SOUNDENT_CHANNEL_NPC_FOOTSTEP) {
...
bool CAI_BaseNPC::QuerySeeEntity(CBaseEntity *pEntity,
                                 bool bOnlyHateOrFearIfNPC) {
...
void CAI_BaseNPC::OnLooked(int iDistance) {
  ...
  static int conditionsToClear[] = {
      COND_SEE_HATE,    COND_SEE_DISLIKE,     COND_SEE_ENEMY,
      COND_SEE_FEAR,    COND_SEE_NEMESIS,     COND_SEE_PLAYER,
      COND_LOST_PLAYER, COND_ENEMY_WENT_NULL,
  };
  ...
  while (pSightEnt) {
    ...
    // don't check anything else about an entity that can't be seen, or an
    // entity that you don't care about.
    if (relation != D_NU) {
      if (pSightEnt == GetEnemy()) {
        // we know this ent is visible, so if it also happens to be our enemy,
        // store that now.
        SetCondition(COND_SEE_ENEMY);
      }

      // don't add the Enemy's relationship to the conditions. We only want to
      // worry about conditions when we see npcs other than the Enemy.
      switch (relation) {
        case D_HT: {
...
void CAI_BaseNPC::OnListened() {
  ...
  while (pCurrentSound) {
    ...
    if (pCurrentSound->FIsSound()) {
      ...
      // if not a sound, must be a smell - determine if it's just a scent, or if
      // it's a food scent
...
Activity CAI_BaseNPC::GetHintActivity(short sHintType, Activity HintsActivity) {
  if (HintsActivity != ACT_INVALID) return HintsActivity;
...
CBaseGrenade *CAI_BaseNPC::IncomingGrenade(void) {
  ...
  for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
       pEMemory != NULL; pEMemory = GetEnemies()->GetNext(&iter)) {
    CBaseGrenade *pBG =
        dynamic_cast<CBaseGrenade *>((CBaseEntity *)pEMemory->hEnemy);
    ...
    if (iDist <= NPC_GRENADE_FEAR_DIST) return pBG;
...
void CAI_BaseNPC::TryRestoreHull(void) {
  if (IsUsingSmallHull() && GetCurSchedule()) {
    ...
    AI_TraceHull(GetAbsOrigin(), vUpBit, GetHullMins(), GetHullMaxs(),
...
CSound *CAI_BaseNPC::GetBestSound(int validTypes) {
  if (m_pLockedBestSound->m_iType != SOUND_NONE) return m_pLockedBestSound;
...
CSound *CAI_BaseNPC::GetBestScent(void) {
  ...
  if (pResult == NULL) DevMsg("Warning: NULL Return from GetBestScent\n");
  return pResult;
}
...
void CAI_BaseNPC::LockBestSound() {
  ...
  if (pBestSound) *m_pLockedBestSound = *pBestSound;
}
...
//-----------------------------------------------------------------------------
// Purpose: Return true if the specified sound is visible. Handles sounds
// generated by entities correctly. Input  : *pSound -
//-----------------------------------------------------------------------------
...
void CAI_BaseNPC::SetHeadDirection(const Vector &vTargetPos, float flInterval) {
  ...
  NDebugOverlay::Line(vEyePos, vEyePos + (50 * vHeadDir), 255, 0, 0, false,
                      0.1);
  NDebugOverlay::Line(vEyePos, vEyePos + (50 * vBodyDir), 0, 255, 0, false,
                      0.1);
  ...
  //--------------------------------------
  float flDesiredYaw =
      VecToYaw(vTargetPos - GetLocalOrigin()) - GetLocalAngles().y;
  if (flDesiredYaw > 180) flDesiredYaw -= 360;
  if (flDesiredYaw < -180) flDesiredYaw += 360;
...
CBaseEntity *CAI_BaseNPC::EyeLookTarget(void) {
  if (m_flNextEyeLookTime < gpGlobals->curtime) {
    ...
    for (CEntitySphereQuery sphere(GetAbsOrigin(), 1024, 0);
...
bool CAI_BaseNPC::FInAimCone(const Vector &vecSpot) {
  ...
  if (flDot >
      0.994)  //!!!BUGBUG - magic number same as FacingIdeal(), what is this?
    return true;
...
void CAI_BaseNPC::SetAim(const Vector &aimDir) {
  ...
  if (GetEnemy()) {
    ...
    newPitch = curPitch +
               0.8 * UTIL_AngleDiff(UTIL_ApproachAngle(angDir.x, curPitch, 20),
                                    curPitch);
    ...
    newPitch = curPitch +
               0.6 * UTIL_AngleDiff(UTIL_ApproachAngle(angDir.x, curPitch, 20),
                                    curPitch);
...
void CAI_BaseNPC::MaintainLookTargets(float flInterval) {
  ...
  if (GetNavigator()->IsGoalActive() &&
      ValidEyeTarget(GetNavigator()->GetCurWaypointPos())) {
  ...
  if (m_flNextEyeLookTime < gpGlobals->curtime) {
    ...
    float	x			= random->RandomFloat(-0.5,0.5) +
    random->RandomFloat(-0.5,0.5); float	y			=
    random->RandomFloat(-0.5,0.5) + random->RandomFloat(-0.5,0.5);
    float	z			= random->RandomFloat(-0.5,0.5) +
    random->RandomFloat(-0.5,0.5);
    ...
    Vector vecDir		= vBodyDir + (x * lookSpread.x * forward) + (y *
    lookSpread.y * right) + (z * lookSpread.z * up);
  ...
  float timeToUse = flInterval;
  while (timeToUse > 0) {
    m_vCurEyeTarget = ((1 - m_flEyeIntegRate) * m_vCurEyeTarget +
                       m_flEyeIntegRate * m_vEyeLookTarget);
    timeToUse -= 0.1;
  }
...
void CAI_BaseNPC::PerformMovement() {
  ...
  float flInterval = (m_flTimeLastMovement != FLT_MAX)
                         ? gpGlobals->curtime - m_flTimeLastMovement
                         : 0.1;
...
bool CAI_BaseNPC::PreThink(void) {
  ...
  if (m_hOpeningDoor.Get() && AIIsDebuggingDoors(this)) {
    NDebugOverlay::Line(EyePosition(), m_hOpeningDoor->WorldSpaceCenter(), 255,
                        255, 255, false, .1);
  }
...
void CAI_BaseNPC::RunAnimation(void) {
  ...
  // This needs a better home -- switching animations over time should be
  // encapsulated on a per-activity basis perhaps MaintainActivity() or a
  // ShiftAnimationOverTime() or something.
  if (m_NPCState != NPC_STATE_SCRIPT && m_NPCState != NPC_STATE_DEAD &&
    ...
    if (SequenceLoops()) {
      ...
      // animation that just ended doesn't loop! That means we just finished a
      // fidget and should return to our heaviest weighted idle (the subtle one)
      iSequence = SelectHeaviestSequence(m_translatedActivity);
    }
    if (iSequence != ACTIVITY_NOT_AVAILABLE) {
      ResetSequence(iSequence);  // Set to new anim (if it's there)

      // Adrian: Basically everywhere else in the AI code this variable gets set
      // to whatever our sequence is.  But here it doesn't and not setting it
      // causes any animation set through here to be stomped by the  ideal
      // sequence before it has a chance of playing out (since there's code that
      // reselects the ideal  sequence if it doesn't match the current one).
...
void CAI_BaseNPC::PostRun(void) {
  ...
  if (!IsMoving()) {
    if (GetIdealActivity() == ACT_WALK || GetIdealActivity() == ACT_RUN ||
...
void CAI_BaseNPC::PostRunStopMoving() {
  ...
  if (!GetNavigator()->SetGoalFromStoppingPath())
    SetIdealActivity(
        GetStoppedActivity());  // This is to prevent running in place
}
...
bool CAI_BaseNPC::ShouldAlwaysThink() {
  ...
  // the PVS while navigating to the player. Perhaps should incorporate a
  // heuristic taking into account mode, enemy, last time saw player, player
  // range etc. For example, if enemy is player, and player is within 100 feet,
  // and saw the player within the past 15 seconds, keep running...
  return HasSpawnFlags(SF_NPC_ALWAYSTHINK);
}

//-----------------------------------------------------------------------------
// Purpose: Return true if the Player should be running the auto-move-out-of-way
//			avoidance code, which also means that the NPC shouldn't
//care  about running into the Player.
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::ShouldPlayerAvoid(void) {
  if (GetState() == NPC_STATE_SCRIPT) return true;
...
void CAI_BaseNPC::UpdateEfficiency(bool bInPVS) {
  ...
  m_bInChoreo = (GetState() == NPC_STATE_SCRIPT ||
  ...
  if (gpGlobals->framecount != iPrevFrame) {
    ...
    if (pPlayer) {
      pPlayer->EyePositionAndVectors(&vPlayerEyePosition, &vPlayerForward, NULL,
                                     NULL);
    }
  }
  ...
  if (pPlayer) {
    bPlayerFacing =
        (bClientPVSExpanded || (bInPVS && vPlayerForward.Dot(vToNPC) > 0));
  } else {
    playerDist = 0;
    bPlayerFacing = true;
  }
  ...
  bool bInVisibilityPVS =
      (bClientPVSExpanded && UTIL_FindClientInVisibilityPVS(edict()) != NULL);

  //---------------------------------

  if ((bInPVS && (bPlayerFacing || playerDist < 25 * 12)) ||
      bClientPVSExpanded) {
    SetMoveEfficiency(AIME_NORMAL);
  } else {
  ...
  if (!IsRetail() && ai_efficiency_override.GetInt() > AIE_NORMAL &&
      ai_efficiency_override.GetInt() <= AIE_DORMANT) {
  ...
  if (m_bForceConditionsGather ||
      ...
      ((bInPVS || bInVisibilityPVS) &&
       ((GetTask() && !TaskIsRunning()) || GetTaskInterrupt() > 0 ||
        m_bInChoreo))) {
  ...
  if (GetSoundInterests() & SOUND_DANGER) {
    ...
    while (iSound != SOUNDLIST_EMPTY) {
      ...
      if (pCurrentSound && (SOUND_DANGER & pCurrentSound->SoundType())) {
        ...
        if (pCurrentSound->GetSoundOrigin().DistToSqr(vEarPosition) <=
            flHearDistanceSq) {
  ...
  if (bInPVS) {
    ...
    range = (playerDist < 50 * 12)
                ? DIST_NEAR
                : (playerDist < 200 * 12) ? DIST_MID : DIST_FAR;
  } else {
    range = (playerDist < 25 * 12)
                ? DIST_NEAR
  ...
  int state = GetState();
  if (state ==
      NPC_STATE_SCRIPT)  // Treat script as alert. Already confirmed not in PVS
...
void CAI_BaseNPC::UpdateSleepState(bool bInPVS) {
  if (GetSleepState() > AISS_AWAKE) {
    ...
    if (m_flWakeRadius > .1 && !(pLocalPlayer->GetFlags() & FL_NOTARGET) &&
        (pLocalPlayer->GetAbsOrigin() - GetAbsOrigin()).LengthSqr() <=
            Square(m_flWakeRadius))
      Wake();
      ...
      if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE))
        ...
        if ((GetSoundInterests() & SOUND_DANGER) &&
            !(HasSpawnFlags(SF_NPC_WAIT_TILL_SEEN))) {
          int iSound = CSoundEnt::ActiveList();
    ...
    if (!IsInAScript() && m_NPCState != NPC_STATE_SCRIPT) {
      ...
      if (HasSleepFlags(AI_SLEEP_FLAG_AUTO_PVS_AFTER_PVS)) {
        if (HasCondition(COND_IN_PVS)) {
          // OK, we're in the player's PVS. Now switch to regular old AUTO_PVS
          // sleep rules.
...
int __cdecl ThinkRebalanceCompare(const AIRebalanceInfo_t *pLeft,
                                  const AIRebalanceInfo_t *pRight) {
  ...
  if (bDebugThinkTicks) {
    ...
    if (gpGlobals->tickcount != iPrevTick) {
      DevMsg("NPC per tick is %d [%d] (tick %d, frame %d)\n",
             nRebalanceableThinksInTick, nThinksInTick, iPrevTick,
    ...
    nThinksInTick++;
    if (CanThinkRebalance()) nRebalanceableThinksInTick++;
  }

  if (ShouldRebalanceThinks() &&
      gpGlobals->tickcount >= gm_iNextThinkRebalanceTick) {
    AI_PROFILE_SCOPE(AI_Think_Rebalance);

    static CUtlVector<AIRebalanceInfo_t> rebalanceCandidates(16, 64);
    gm_iNextThinkRebalanceTick =
        gpGlobals->tickcount + TIME_TO_TICKS(random->RandomFloat(3, 5));
    ...
    int iMinTickRebalance =
        gpGlobals->tickcount - 1;  // -1 needed for alternate ticks
    ...
    for (i = 0; i < g_AI_Manager.NumAIs(); i++) {
      ...
      if (pCandidate->CanThinkRebalance() &&
        ...
        rebalanceCandidates[iInfo].iNextThinkTick =
        ...
        if (pCandidate->IsFlaggedEfficient()) {
          ...
          rebalanceCandidates[iInfo].bInPVS =
              (UTIL_FindClientInPVS(pCandidate->edict()) != NULL);
          rebalanceCandidates[iInfo].distPlayer = VectorNormalize(vToCandidate);
          rebalanceCandidates[iInfo].dotPlayer =
              vPlayerForward.Dot(vToCandidate);
        } else {
    ...
    if (rebalanceCandidates.Count()) {
      ...
      int iMaxThinkersPerTick =
          ceil((float)(rebalanceCandidates.Count() + 1) /
      ...
      for (i = 0; i < rebalanceCandidates.Count(); i++) {
        if (iRemainingThinksToDistribute == 0 ||
            rebalanceCandidates[i].iNextThinkTick > iCurTickDistributing) {
        ...
        if (rebalanceCandidates[i].pNPC->GetNextThinkTick() !=
            iCurTickDistributing) {
          if (bDebugThinkTicks)
          ...
          rebalanceCandidates[i].pNPC->SetNextThink(
              TICKS_TO_TIME(iCurTickDistributing));
        } else if (bDebugThinkTicks) {
          DevMsg("      Leaving %d\n",
                 rebalanceCandidates[i].pNPC->GetNextThinkTick());
        }
    ...
    Assert(GetNextThinkTick() ==
           TICK_NEVER_THINK);  // never change this objects tick
  }
}
  ...
  if (bUseThinkLimits && VCRGetMode() == VCR_Disabled) {
    if (m_iFrameBlocked == gpGlobals->framecount) {
      ...
      if (g_NpcTimeThisFrame > NPC_THINK_LIMIT) {
        float timeSinceLastRealThink =
            gpGlobals->curtime - m_flLastRealThinkTime;
...
void CAI_BaseNPC::SetPlayerAvoidState(void) {
  ...
  if (m_bPerformAvoidance || (ShouldPlayerAvoid() && bIsMoving)) {
    ...
    if (pLocalPlayer) {
      bShouldPlayerAvoid = IsBoxIntersectingBox(
          GetAbsOrigin() + vMins, GetAbsOrigin() + vMaxs,
          pLocalPlayer->GetAbsOrigin() + pLocalPlayer->WorldAlignMins(),
          pLocalPlayer->GetAbsOrigin() + pLocalPlayer->WorldAlignMaxs());
    }
  ...
  if (GetCollisionGroup() == COLLISION_GROUP_NPC ||
      GetCollisionGroup() == COLLISION_GROUP_NPC_ACTOR) {
...
//-----------------------------------------------------------------------------
// Purpose: Enables player avoidance when the player's vphysics shadow
// penetrates our vphysics shadow.  This can happen when the player is hit by a
// combine ball, which pushes them into an adjacent npc.  Subclasses should
// override this if it causes problems, but in general this will solve cases of
// the player getting stuck in the NPC from being pushed.
//-----------------------------------------------------------------------------
...
bool CAI_BaseNPC::CheckPVSCondition() {
  bool bInPVS =
      (UTIL_FindClientInPVS(edict()) != NULL) ||
      (UTIL_ClientPVSIsExpanded() && UTIL_FindClientInVisibilityPVS(edict()));
...
void CAI_BaseNPC::NPCThink(void) {
  ...
  if (GetEfficiency() < AIE_DORMANT && GetSleepState() == AISS_AWAKE) {
    ...
    if (g_pAINetworkManager && g_pAINetworkManager->IsInitialized()) {
      ...
      AI_PROFILE_SCOPE_BEGIN_(
          GetClassScheduleIdSpace()->GetClassName());  // need to use a string
                                                       // stable from map load
                                                       // to map load
      ...
      if (PreThink()) {
        if (m_flNextDecisionTime <= gpGlobals->curtime) {
          ...
          if (m_ScheduleState.bTaskRanAutomovement) AutoMovement();
          if (m_ScheduleState.bTaskUpdatedYaw) GetMotor()->UpdateYaw();
        }
    ...
    if (thinkLimit > 0) {
      ...
      if (thinkTime > thinkLimit) {
        int color =
            (int)RemapVal(thinkTime, thinkLimit, thinkLimit * 3, 96.0, 255.0);
        if (color > 255)
          color = 255;
        else if (color < 96)
          color = 96;
        ...
        NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 64), color, 0, 0,
                            false, 1.0);
        NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 16) + right * 16,
                            color, 0, 0, false, 1.0);
        NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 16) - right * 16,
                            color, 0, 0, false, 1.0);
      }
  ...
  if (m_bUsingStandardThinkTime) {
    static const char *ppszEfficiencies[] = {
        ...
        "AIE_SUPER_EFFICIENT", "AIE_DORMANT",
    };
    ...
    if (ai_debug_efficiency.GetBool())
      DevMsg(this, "Eff: %s, Move: %s\n", ppszEfficiencies[GetEfficiency()],
             ppszMoveEfficiencies[GetMoveEfficiency()]);
    ...
    if (bRanDecision) {
      m_flNextDecisionTime =
          gpGlobals->curtime + g_DecisionIntervals[GetEfficiency()];
    }
...
// activated it.
//=========================================================
void CAI_BaseNPC::NPCUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
int CAI_BaseNPC::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  // Decent fix to keep folks from kicking/punching hornets and snarks is to
  // check the onground flag(sjb)
...
void CAI_BaseNPC::GatherAttackConditions(CBaseEntity *pTarget, float flDist) {
  ...
  // we know the enemy is in front now. We'll find which attacks the npc is
  // capable of by checking for corresponding Activities in the model file, then
  // do the simple checks to validate those attack types.
  ...
  // Try the eyes first, as likely to succeed (because can see or else wouldn't
  // be here) thus reducing the odds of the need for a second trace
  ...
  if ((capability & bits_CAP_WEAPON_RANGE_ATTACK2) && bWeaponIsReady &&
      (GetActiveWeapon()->CapabilitiesGet() & bits_CAP_WEAPON_RANGE_ATTACK2)) {
    AI_PROFILE_SCOPE(
        CAI_BaseNPC_GatherAttackConditions_WeaponRangeAttack2Condition);
  ...
  if ((capability & bits_CAP_WEAPON_MELEE_ATTACK1) && bWeaponIsReady) {
    AI_PROFILE_SCOPE(
        CAI_BaseNPC_GatherAttackConditions_WeaponMeleeAttack1Condition);
  ...
  if ((capability & bits_CAP_WEAPON_MELEE_ATTACK2) && bWeaponIsReady) {
    AI_PROFILE_SCOPE(
        CAI_BaseNPC_GatherAttackConditions_WeaponMeleeAttack2Condition);
...
void CAI_BaseNPC::Wake(bool bFireOutput) {
  if (GetSleepState() != AISS_AWAKE) {
    ...
    if (bFireOutput) m_OnWake.FireOutput(this, this);

    if (m_bWakeSquad && GetSquad()) {
      AISquadIter_t iter;
      for (CAI_BaseNPC *pSquadMember = GetSquad()->GetFirstMember(&iter);
           pSquadMember; pSquadMember = GetSquad()->GetNextMember(&iter)) {
...
void CAI_BaseNPC::CheckOnGround(void) {
  bool bScriptedWait = (IsCurSchedule(SCHED_WAIT_FOR_SCRIPT) ||
                        (m_hCine && m_scriptState == CAI_BaseNPC::SCRIPT_WAIT));
  if (!bScriptedWait && !HasCondition(COND_FLOATING_OFF_GROUND)) {
    ...
    if ((GetNavType() == NAV_GROUND) && (GetMoveType() != MOVETYPE_VPHYSICS) &&
      ...
      if (m_CheckOnGroundTimer.Expired()) {
        ...
        if (mins !=
            maxs)  // some NPCs have no hull, so mins == maxs == vec3_origin
        {
          ...
          trace_t trace;
          m_pMoveProbe->TraceHull(vecStart, vecDown, mins, maxs, MASK_NPCSOLID,
                                  &trace);
          ...
          if (trace.fraction == 1.0) {
            ...
            if (trace.startsolid &&
                trace.m_pEnt->GetMoveType() == MOVETYPE_VPHYSICS &&
                trace.m_pEnt->VPhysicsGetObject() &&
                trace.m_pEnt->VPhysicsGetObject()->GetMass() <
                    VPHYSICS_LARGE_OBJECT_MASS) {
    ...
    // parented objects are never floating
    if (bScriptedWait || GetMoveParent() != NULL ||
        (GetFlags() & FL_ONGROUND) || GetNavType() != NAV_GROUND) {
...
bool CAI_BaseNPC::CanFlinch(void) {
  if (IsCurSchedule(SCHED_BIG_FLINCH)) return false;
...
void CAI_BaseNPC::CheckFlinches(void) {
  ...
  if (HasCondition(COND_HEAVY_DAMAGE)) {
    ...
    if (HasMemory(bits_MEMORY_FLINCHED)) {
      ...
      // Prevents the player from stun-locking enemies, even though they don't
      // full flinch.
      ClearCondition(COND_HEAVY_DAMAGE);
    ...
    // Otherwise, do nothing. The heavy damage will interrupt our schedule and
    // we'll flinch.
  ...
  // If it's been a while since we did a full flinch, forget that we flinched so
  // we'll flinch fully again
  if (HasMemory(bits_MEMORY_FLINCHED) &&
      gpGlobals->curtime > m_flNextFlinchTime) {
...
void CAI_BaseNPC::GatherConditions(void) {
  ...
  if (m_NPCState != NPC_STATE_NONE && m_NPCState != NPC_STATE_DEAD) {
    if (FacingIdeal()) Forget(bits_MEMORY_TURNING);
    ...
    // Sample the environment. Do this unconditionally if there is a player in
    // this npc's PVS. NPCs in COMBAT state are allowed to simulate when there
    // is no player in the same PVS. This is so that any fights in progress will
    // continue even if the player leaves the PVS.
    if (!IsFlaggedEfficient() &&
        (bForcedGather || HasCondition(COND_IN_PVS) || ShouldAlwaysThink() ||
      ...
      if (GetCurSchedule() &&
          ...
          GetEnemy() && !HasCondition(COND_NEW_ENEMY) &&
          GetCurSchedule()->HasInterrupt(COND_NEW_ENEMY)) {
        // @Note (toml 05-05-04): There seems to be a case where an NPC can not
        // respond
        //						  to COND_NEW_ENEMY. Only
        //evidence  right now is save
        //						  games after the fact, so
        //for  now, just patching it up
        DevMsg(2, "Had to force COND_NEW_ENEMY\n");
        SetCondition(COND_NEW_ENEMY);
      }
    } else {
      // if not done, can have problems if leave PVS in same frame heard/saw
      // things, since only PerformSensing clears conditions
...
void CAI_BaseNPC::PrescheduleThink(void) {
  ...
  if ((CapabilitiesGet() & bits_CAP_USE_WEAPONS) &&
    ...
    if (IsAlive() && !IsInAScript()) {
      if (!IsCurSchedule(SCHED_MELEE_ATTACK1, false) &&
          !IsCurSchedule(SCHED_MELEE_ATTACK2, false) &&
          !IsCurSchedule(SCHED_RANGE_ATTACK1, false) &&
          !IsCurSchedule(SCHED_RANGE_ATTACK2, false)) {
...
void CAI_BaseNPC::RunAI(void) {
  ...
  if (ai_debug_squads.GetBool()) {
    if (IsInSquad() && GetSquad() && !CAI_Squad::IsSilentMember(this) &&
        (GetSquad()->IsLeader(this) || GetSquad()->NumMembers() == 1)) {
      AISquadIter_t iter;
      ...
      vecPoint = EyePosition() + Vector(0, 0, 12);
      GetVectors(NULL, &right, NULL);
      NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 64), 0, 255, 0,
                          false, 0.1);
      NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 32) + right * 32, 0,
                          255, 0, false, 0.1);
      NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 32) - right * 32, 0,
                          255, 0, false, 0.1);

      for (CAI_BaseNPC *pSquadMember = pSquad->GetFirstMember(&iter, false);
           pSquadMember; pSquadMember = pSquad->GetNextMember(&iter, false)) {
        if (pSquadMember != this)
          NDebugOverlay::Line(
              EyePosition(), pSquadMember->EyePosition(), 0,
              CAI_Squad::IsSilentMember(pSquadMember) ? 127 : 255, 0, false,
              0.1);
      }
    }
  }
  ...
  if (ai_debug_loners.GetBool() && !IsInSquad() && AI_IsSinglePlayer()) {
    ...
    NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 64), 255, 0, 0, false,
                        0.1);
    NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 32) + right * 32, 255,
                        0, 0, false, 0.1);
    NDebugOverlay::Line(vecPoint, vecPoint + Vector(0, 0, 32) - right * 32, 255,
                        0, 0, false, 0.1);
  }
...
void CAI_BaseNPC::ClearTransientConditions() {
  // if the npc didn't use these conditions during the above call to
  // MaintainSchedule() we throw them out cause we don't want them sitting
  // around through the lifespan of a schedule that doesn't use them.
  ClearCondition(COND_LIGHT_DAMAGE);
  ClearCondition(COND_HEAVY_DAMAGE);
...
NPC_STATE CAI_BaseNPC::SelectIdleIdealState() {
  ...
  if (HasCondition(COND_NEW_ENEMY) || HasCondition(COND_SEE_ENEMY)) {
  ...
  if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE) ||
      (!GetEnemy() &&
       gpGlobals->curtime - GetEnemies()->LastTimeSeen(AI_UNKNOWN_ENEMY) <
    ...
    if (GetEnemy()) {
      ...
      if (GetEnemies()->Find(AI_UNKNOWN_ENEMY)) {
        ...
        // Don't have an enemy, so face the direction the last attack came from
        // (don't face north)
...
NPC_STATE CAI_BaseNPC::SelectScriptIdealState() {
  if (HasCondition(COND_TASK_FAILED) || HasCondition(COND_LIGHT_DAMAGE) ||
...
NPC_STATE CAI_BaseNPC::SelectIdealState(void) {
  ...
  switch (m_NPCState) {
    case NPC_STATE_IDLE: {
      ...
      if (nState != NPC_STATE_INVALID) return nState;
    } break;
...
void CAI_BaseNPC::GiveWeapon(string_t iszWeaponName) {
  ...
  if (GetEntityName() != NULL_STRING) {
    pWeapon->SetName(
        AllocPooledString(UTIL_VarArgs("%s_weapon", GetEntityName())));
  }
...
bool CAI_BaseNPC::ShouldLookForBetterWeapon() {
  if (m_flNextWeaponSearchTime > gpGlobals->curtime) return false;
...
//			 For now check if there is a weapon and I don't have
//one.  In 			 the future
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::WeaponLOSCondition(const Vector &ownerPos,
                                     const Vector &targetPos,
                                     bool bSetConditions) {
#if 0
  ...
  if (GetActiveWeapon()) {
    bHaveLOS = GetActiveWeapon()->WeaponLOSCondition(ownerPos, targetPos,
                                                     bSetConditions);
  ...
  if (CapabilitiesGet() &
    ...
    if (bHaveLOS) {
      if ((CapabilitiesGet() & bits_CAP_NO_HIT_SQUADMATES) && m_pSquad &&
          GetEnemy()) {
...
//			If bSetConditions is true, LOS related conditions will
//also  be set
// Input  :
// Output :
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::InnateWeaponLOSCondition(const Vector &ownerPos,
                                           const Vector &targetPos,
                                           bool bSetConditions) {
  // --------------------
  // Check for occlusion
  ...
  trace_t tr;
  AI_TraceLine(barrelPos, targetPos, MASK_SHOT, this, COLLISION_GROUP_NONE,
               &tr);
  ...
  if (GetEnemy() != NULL) {
    ...
    if (pCCEnemy != NULL && pCCEnemy->IsInAVehicle()) {
      // Ok, player in vehicle, check if vehicle is target we're looking at,
      // fire if it is Also, check to see if the owner of the entity is the
      // vehicle, in which case it's valid too. This catches vehicles that use
      // bone followers.
      CBaseEntity *pVehicleEnt = pCCEnemy->GetVehicleEntity();
      if (pHitEntity == pVehicleEnt ||
          pHitEntity->GetOwnerEntity() == pVehicleEnt)
        return true;
    }
  }
...
float CAI_BaseNPC::EnemyDistance(CBaseEntity *pEnemy) {
  ...
  // pEnemy->CollisionProp()->WorldSpaceSurroundingBounds( &enemyMins,
  // &enemyMaxs ); float enemyHeight = enemyMaxs.z - enemyMins.z;
...
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::UpdateEnemyMemory(CBaseEntity *pEnemy, const Vector &position,
                                    CBaseEntity *pInformer) {
  ...
  if (GetEnemies()) {
    ...
    float reactionDelay =
        (!pInformer || pInformer == this) ? GetReactionDelay(pEnemy) : 0.0;
    bool result =
        GetEnemies()->UpdateMemory(GetNavigator()->GetNetwork(), pEnemy,
                                   position, reactionDelay, firstHand);

    if (!firstHand && pEnemy && result &&
        GetState() == NPC_STATE_IDLE)  // if it's a new potential enemy
      ForceDecisionThink();
...
//			COND_ENEMY_OCCLUDED or COND_WEAPON_SIGHT_OCCLUDED is
// set.
//-----------------------------------------------------------------------------
...
//			gets and stores data and conditions pertaining to a
// npc's 			enemy.
// 			@TODO (toml 07-27-03): this should become subservient to
// the senses. right 			now, it yields different result Input  :
// Output :
//-----------------------------------------------------------------------------
void CAI_BaseNPC::GatherEnemyConditions(CBaseEntity *pEnemy) {
  ...
  // ---------------------------
  if (HasCondition(COND_NEW_ENEMY) ||
      GetSenses()->GetTimeLastUpdate(GetEnemy()) == gpGlobals->curtime) {
    ...
    if (!bSensesDidSee &&
        ((EnemyDistance(pEnemy) >= GetSenses()->GetDistLook()) ||
         !FVisible(pEnemy, MASK_BLOCKLOS, &pBlocker))) {
      // No LOS to enemy
  ...
  if (HasCondition(COND_SEE_ENEMY)) {
    ...
    if (pEnemy->GetSmoothedVelocity() != vec3_origin) {
      Vector vTrailPos =
          pEnemy->GetAbsOrigin() -
          pEnemy->GetSmoothedVelocity() * random->RandomFloat(-0.05, 0);
    ...
    if (pEnemy->MyCombatCharacterPointer() &&
        pEnemy->MyCombatCharacterPointer()->FInViewCone(this)) {
      ...
      SetCondition(COND_BEHIND_ENEMY);
              ...
              !HasCondition(COND_SEE_ENEMY)) &&
             (flDistToEnemy <= 256)) {
    // if the enemy is not occluded, and unseen, that means it is behind or
    // beside the npc. if the enemy is near enough the npc, we go ahead and let
    // the npc know where the enemy is. Send the enemy in as the informer so
    // this knowledge will be regarded as secondhand so that the NPC doesn't
  ...
  // If my enemy has moved significantly, or if the enemy has changed update my
  // path
  UpdateEnemyPos();
...
//-----------------------------------------------------------------------------
float CAI_BaseNPC::GetGoalRepathTolerance(CBaseEntity *pGoalEnt,
                                          GoalType_t type,
  ...
  if (!pGoalEnt->IsPlayer()) result *= 1.20;
...
void CAI_BaseNPC::UpdateTargetPos() {
  ...
  if (GetNavigator()->GetGoalType() == GOALTYPE_TARGETENT) {
    if (m_hTargetEnt != GetNavigator()->GetGoalTarget()) {
      ...
      if (GetTarget() == NULL ||
          (GetNavigator()->GetGoalPos() - GetTarget()->GetAbsOrigin())
                  .Length() >
              GetGoalRepathTolerance(GetTarget(), GOALTYPE_TARGETENT,
                                     GetNavigator()->GetGoalPos(),
                                     GetTarget()->GetAbsOrigin())) {
        if (!GetNavigator()->RefindPathToGoal(false)) {
          TaskFail(FAIL_NO_ROUTE);
        }
      }
...
void CAI_BaseNPC::CheckTarget(CBaseEntity *pTarget) {
  ...
  //  Set visibility conditions
  // ---------------------------
  if ((EnemyDistance(pTarget) >= GetSenses()->GetDistLook()) ||
      !FVisible(pTarget)) {
    // No LOS to target
...
//			this bullseye!
//-----------------------------------------------------------------------------
CAI_BaseNPC *CAI_BaseNPC::CreateCustomTarget(const Vector &vecOrigin,
                                             float duration) {
#ifdef HL2_DLL
...
Activity CAI_BaseNPC::NPC_TranslateActivity(Activity eNewActivity) {
  ...
  //						which causes the character to pop up and down
  //in  their idle state of firing while crouched. -- jdw
...
Activity CAI_BaseNPC::TranslateActivity(Activity idealActivity,
  ...
  idealWeaponActivity =
      Weapon_TranslateActivity(idealActivity, &bIdealWeaponRequired);
  if (pIdealWeaponActivity) *pIdealWeaponActivity = idealWeaponActivity;
  ...
  while (count++ < MAX_TRIES) {
    ...
    if (current != last) baseTranslation = current;
  ...
  if (last == ACT_SCRIPT_CUSTOM_MOVE) return ACT_SCRIPT_CUSTOM_MOVE;

  if (HaveSequenceForActivity(weaponTranslation)) return weaponTranslation;
  ...
  if (baseTranslation != weaponTranslation &&
      HaveSequenceForActivity(baseTranslation))
    return baseTranslation;

  if (idealWeaponActivity != baseTranslation &&
      HaveSequenceForActivity(idealWeaponActivity))
    return idealActivity;

  if (idealActivity != idealWeaponActivity &&
      HaveSequenceForActivity(idealActivity))
    return idealActivity;
...
//			weaponActivity -
//-----------------------------------------------------------------------------
void CAI_BaseNPC::ResolveActivityToSequence(Activity NewActivity,
                                            int &iSequence,
  ...
  if ((NewActivity == ACT_SCRIPT_CUSTOM_MOVE)) {
    ...
    if (iSequence == ACTIVITY_NOT_AVAILABLE) {
      ...
      if ((pLastWarn != this && lastWarnActivity != translatedActivity) ||
          gpGlobals->curtime - timeLastWarn > 5.0) {
        DevWarning("%s:%s:%s has no sequence for act:%s\n", GetClassname(),
                   GetDebugName(), STRING(GetModelName()),
                   ActivityList_NameForIndex(translatedActivity));
        pLastWarn = this;
...
void CAI_BaseNPC::SetActivityAndSequence(Activity NewActivity, int iSequence,
                                         Activity translatedActivity,
                                         Activity weaponActivity) {
  m_translatedActivity = translatedActivity;

  if (ai_sequence_debug.GetBool() == true &&
      (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)) {
    DevMsg("SetActivityAndSequence : %s: %s:%s -> %s:%s / %s:%s\n",
           GetClassname(), GetActivityName(GetActivity()),
           GetSequenceName(GetSequence()), GetActivityName(NewActivity),
           GetSequenceName(iSequence), GetActivityName(translatedActivity),
           GetActivityName(weaponActivity));
  }
  ...
  // As far as the code is concerned, a translation is merely a new set of
  // sequences that should be regarded as the activity in question.

  // Go ahead and set this so it doesn't keep trying when the anim is not
  // present
...
void CAI_BaseNPC::SetActivity(Activity NewActivity) {
  ...
  if (NewActivity != ACT_RESET && m_Activity == ACT_TRANSITION &&
      m_IdealActivity != ACT_DO_NOT_DISTURB) {
    return;
  }
  ...
  if (ai_sequence_debug.GetBool() == true &&
    ...
    DevMsg("SetActivity : %s: %s -> %s\n", GetClassname(),
           GetActivityName(GetActivity()), GetActivityName(NewActivity));
  }
  ...
  ResolveActivityToSequence(m_IdealActivity, m_nIdealSequence,
                            m_IdealTranslatedActivity, m_IdealWeaponActivity);

  // DevMsg("%s: SLAM %s -> %s\n", GetClassname(),
  // GetSequenceName(GetSequence()), GetSequenceName(m_nIdealSequence));

  SetActivityAndSequence(m_IdealActivity, m_nIdealSequence,
                         m_IdealTranslatedActivity, m_IdealWeaponActivity);
}
...
void CAI_BaseNPC::SetIdealActivity(Activity NewActivity) {
  // ignore if it's an ACT_TRANSITION, it means somewhere we're setting
  // IdealActivity with a bogus intermediate value
  ...
  if (NewActivity == ACT_RESET) {
    // They probably meant to call SetActivity(ACT_RESET)... we'll fix it for
    // them.
  ...
  // Perform translation in case we need to change sequences within a single
  // activity, such as between a standing idle and a crouching idle.
  ResolveActivityToSequence(m_IdealActivity, m_nIdealSequence,
                            m_IdealTranslatedActivity, m_IdealWeaponActivity);
}
...
void CAI_BaseNPC::AdvanceToIdealActivity(void) {
  // If there is a transition sequence between the current sequence and the
  // ideal sequence...
  int nNextSequence =
      FindTransitionSequence(GetSequence(), m_nIdealSequence, NULL);
  if (nNextSequence != -1) {
    ...
    if (nNextSequence != m_nIdealSequence) {
      ...
      // GetSequenceName(nNextSequence), GetSequenceName(m_nIdealSequence));
      ...
      if (eTransitionActivity != ACT_INVALID) {
        int nDiscard;
        ResolveActivityToSequence(eTransitionActivity, nDiscard,
                                  eTranslatedActivity, eWeaponActivity);
      }

      // Set activity and sequence to the transition stuff. Set the activity to
      // ACT_TRANSITION so we know we're in a transition.
      SetActivityAndSequence(ACT_TRANSITION, nNextSequence, eTranslatedActivity,
                             eWeaponActivity);
    } else {
      // DevMsg("%s: IDEAL %s -> %s\n", GetClassname(),
      // GetSequenceName(GetSequence()), GetSequenceName(m_nIdealSequence));

      // Set activity and sequence to the ideal stuff that was set up in
      // MaintainActivity.
      SetActivityAndSequence(m_IdealActivity, m_nIdealSequence,
                             m_IdealTranslatedActivity, m_IdealWeaponActivity);
    }
...
void CAI_BaseNPC::MaintainActivity(void) {
  ...
  if ((GetState() == NPC_STATE_SCRIPT)) {
    // HACK: finish any transitions we might be playing before we yield control
    // to the script
  ...
  if ((GetActivity() != m_IdealActivity) ||
      (GetSequence() != m_nIdealSequence)) {
    if (ai_sequence_debug.GetBool() == true &&
        (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)) {
      DevMsg("MaintainActivity %s : %s:%s -> %s:%s\n", GetClassname(),
             GetActivityName(GetActivity()), GetSequenceName(GetSequence()),
             GetActivityName(m_IdealActivity),
             GetSequenceName(m_nIdealSequence));
    }

    bool bAdvance = false;

    // If we're in a transition activity, see if we are done with the
    // transition.
    if (GetActivity() == ACT_TRANSITION) {
    ...
    // Else get a specific sequence for the activity and try to transition to
    // that.
    else {
      // Save off a target sequence and translated activities to apply when we
      // finish playing all the transitions and finally arrive at our ideal
      // activity.
      ResolveActivityToSequence(m_IdealActivity, m_nIdealSequence,
...
//-----------------------------------------------------------------------------
// Purpose: Checks to see if the activity is one of the standard phase-matched
// movement activities Input  : activity
//-----------------------------------------------------------------------------
...
//#define _THROWDEBUG
float CAI_BaseNPC::ThrowLimit(const Vector &vecStart, const Vector &vecEnd,
                              float fGravity, float fArcSize,
                              const Vector &mins, const Vector &maxs,
                              CBaseEntity *pTarget, Vector *jumpVel,
  ...
  *jumpVel = rawJumpVel;
  ...
  for (float flTime = 0; flTime < jumpTime - 0.1; flTime += timeStep) {
    // Calculate my position after the time step (average velocity over this
    // time step)
    Vector nextPos =
        vecFrom + (rawJumpVel - 0.5 * gravity * timeStep) * timeStep;
    ...
    trace_t tr;
    AI_TraceHull(vecFrom, nextPos, mins, maxs, MASK_SOLID, this,
...
//-----------------------------------------------------------------------------
// Purpose: Called to initialize or re-initialize the vphysics hull when the
// size
//			of the NPC changes
//-----------------------------------------------------------------------------
...
void CAI_BaseNPC::CheckPhysicsContacts() {
  if (gpGlobals->frametime <= 0.0f || !ai_auto_contact_solver.GetBool()) return;
    ...
    while (pSnapshot->IsValid()) {
      ...
      if (pOtherEntity && pGroundEntity != pOtherEntity) {
        ...
        if (pOtherEntity->GetMoveType() == MOVETYPE_VPHYSICS &&
            pOther->IsMoveable() && otherMass < VPHYSICS_LARGE_OBJECT_MASS &&
            !pOtherEntity->GetServerVehicle()) {
          m_bCheckContacts = true;
          Vector vel, point;
...
bool CAI_BaseNPC::SetHullSizeSmall(bool force) {
  if (!m_fIsUsingSmallHull || force) {
    UTIL_SetSize(this, NAI_Hull::SmallMins(GetHullType()),
                 NAI_Hull::SmallMaxs(GetHullType()));
...
bool CAI_BaseNPC::IsNavHullValid() const {
  ...
  if (GetSolid() == SOLID_BBOX) {
    ...
    physcollision->CollideGetAABB(&vecMins, &vecMaxs, pPhysCollide,
...
void CAI_BaseNPC::NPCInit(void) {
  ...
  if (GetSolid() != SOLID_VPHYSICS && !IsSolidFlagSet(FSOLID_NOT_SOLID)) {
    if (!IsNavHullValid()) {
      Warning(
          "NPC Entity %s (%d) has a bounding box which extends outside its nav "
          "box!\n",
  ...
  SetIdealState(
      NPC_STATE_IDLE);  // Assume npc will be idle, until proven otherwise
  ...
  if (CapabilitiesGet() &
      bits_CAP_USE_WEAPONS) {  // Does this npc spawn with a weapon
    if (m_spawnEquipment != NULL_STRING &&
        strcmp(STRING(m_spawnEquipment), "0")) {
      ...
      if (pWeapon) {
        ...
        if (GetEffects() & EF_NOSHADOW) {
          // BUGBUG: if this NPC drops this weapon it will forevermore have no
          // shadow
  ...
  // Robin: Removed this, since it stomps the weapon's settings, and it's
  // stomped
  //		  by OnUpdateShotRegulator() as soon as they finish firing the
  //first  time.  GetShotRegulator()->SetParameters( 2, 6, 0.3f, 0.8f );
  ...
  // NOTE: Must do this before CreateVPhysics() so bone followers have the
  // correct initial positions.
  if (HasSpawnFlags(SF_NPC_WAIT_FOR_SCRIPT)) {
    const char *pStartSequence =
        CAI_ScriptedSequence::GetSpawnPreIdleSequenceForScript(this);
    if (pStartSequence) {
...
void CAI_BaseNPC::OnUpdateShotRegulator() {
  ...
  // Default values
  m_ShotRegulator.SetBurstInterval(pWeapon->GetFireRate(),
                                   pWeapon->GetFireRate());
  m_ShotRegulator.SetBurstShotCountRange(pWeapon->GetMinBurst(),
                                         pWeapon->GetMaxBurst());
  m_ShotRegulator.SetRestInterval(pWeapon->GetMinRestTime(),
                                  pWeapon->GetMaxRestTime());
...
// Set up the shot regulator based on the equipped weapon
//-----------------------------------------------------------------------------
void CAI_BaseNPC::OnChangeActiveWeapon(CBaseCombatWeapon *pOldWeapon,
...
//-----------------------------------------------------------------------------
// Purpose: Tests to see if NPC can holster their weapon (if animation exists to
// holster weapon) Output : true if holster weapon animation exists
//-----------------------------------------------------------------------------
...
int CAI_BaseNPC::HolsterWeapon(void) {
  if (IsWeaponHolstered()) return -1;

  int iHolsterGesture = FindGestureLayer(ACT_DISARM);
  if (iHolsterGesture != -1) return iHolsterGesture;
  ...
  for (int i = 0; i < WeaponCount(); i++) {
    if (GetWeapon(i)) {
      ...
      if (iLayer != -1) {
        // Prevent firing during the holster / unholster
        float flDuration = GetLayerDuration(iLayer);
        m_ShotRegulator.FireNoEarlierThan(gpGlobals->curtime + flDuration +
                                          0.5);
...
bool CAI_BaseNPC::IsWeaponHolstered(void) {
  if (!GetActiveWeapon()) return true;
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_BaseNPC::AddRelationship(const char *pszRelationship,
  ...
  while (entityString) {
    ...
    if (dispositionString) {
      ...
      Warning(
          "Can't parse relationship info (%s) - Expecting 'name [D_HT, D_FR, "
          "D_LI, D_NU] [1-99]'\n",
          pszRelationship);
      Assert(0);
      return;
    ...
    int priority =
        (priorityString) ? atoi(priorityString) : DEF_RELATIONSHIP_PRIORITY;
    ...
    if (!bFoundEntity) {
      ...
      if (!stricmp("player", entityString) ||
          !stricmp("!player", entityString)) {
        AddClassRelationship(CLASS_PLAYER, disposition, priority);
      }
      // Otherwise try to create one too see if a valid classname and get class
      // type
      else {
        // HACKHACK:
        CBaseEntity *pEntity = CanCreateEntityClass(entityString)
                                   ? CreateEntityByName(entityString)
                                   : NULL;
        if (pEntity) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseNPC::AddEntityRelationship(CBaseEntity *pEntity,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_BaseNPC::AddClassRelationship(Class_T nClass,
                                       Disposition_t nDisposition,
                                       int nPriority) {
#if 0
	ForceGatherConditions();
...
void CAI_BaseNPC::NPCInitThink(void) {
  ...
  if (GetSleepState() == AISS_AUTO_PVS) {
    ...
    // select this option in Hammer. So we set a sleep flag to indicate the
    // choice, and then set the sleep state to awake (normal)
    AddSleepFlags(AI_SLEEP_FLAG_AUTO_PVS);
    SetSleepState(AISS_AWAKE);
  }
...
void CAI_BaseNPC::StartNPC(void) {
  ...
  if ((GetMoveType() != MOVETYPE_FLY) &&
      (GetMoveType() != MOVETYPE_FLYGRAVITY) &&
      !(CapabilitiesGet() & bits_CAP_MOVE_FLY) &&
      !HasSpawnFlags(SF_NPC_FALL_TO_GROUND) && !IsWaitingToRappel() &&
      !GetMoveParent()) {
    Vector origin = GetLocalOrigin();

    if (!GetMoveProbe()->FloorPoint(origin + Vector(0, 0, 0.1), MASK_NPCSOLID,
                                    0, -2048, &origin)) {
  ...
  //---------------------------------
  //
  // Spread think times of simultaneously spawned NPCs so that they don't all
  ...
  // If this NPC is spawning late in the game, just push through the rest of the
  // initialization start thinking right now. Some spread is added to handle
  // triggered spawns that bring a bunch of NPCs into the level
  ...
  static const float nextThinkTimes[20] = {
      .0,   .150, .075, .225, .030, .180, .120, .270, .045, .210,
      .105, .255, .015, .165, .090, .240, .135, .060, .195, .285};
...
void CAI_BaseNPC::StartTargetHandling(CBaseEntity *pTargetEnt) {
  ...
  bool bIsFlying =
      (GetMoveType() == MOVETYPE_FLY) || (GetMoveType() == MOVETYPE_FLYGRAVITY);
  AI_NavGoal_t goal(GOALTYPE_PATHCORNER, pTargetEnt->GetAbsOrigin(),
                    bIsFlying ? ACT_FLY : ACT_WALK, AIN_DEF_TOLERANCE,
                    AIN_YAW_TO_DEST);
...
void CAI_BaseNPC::TaskFail(AI_TaskFailureCode_t code) {
  ...
  if (g_pDeveloper->GetInt()) {
    ...
    ADD_DEBUG_HISTORY(HISTORY_AI_DECISIONS,
                      UTIL_VarArgs("%s(%d):       TaskFail -> %s\n",
...
void CAI_BaseNPC::RememberUnreachable(CBaseEntity *pEntity, float duration) {
  ...
  for (int i = m_UnreachableEnts.Size() - 1; i >= 0; i--) {
    ...
    if (pEntity == m_UnreachableEnts[i].hUnreachableEnt) {
      m_UnreachableEnts[i].fExpireTime =
          gpGlobals->curtime + NPC_UNREACHABLE_TIMEOUT;
  ...
  m_UnreachableEnts[nNewIndex].fExpireTime =
      gpGlobals->curtime + NPC_UNREACHABLE_TIMEOUT;
  m_UnreachableEnts[nNewIndex].vLocationWhenUnreachable =
  ...
  for (int i = m_UnreachableEnts.Size() - 1; i >= 0; i--) {
    ...
    if (m_UnreachableEnts[i].hUnreachableEnt == NULL) {
      ...
      if (gpGlobals->curtime > m_UnreachableEnts[i].fExpireTime ||
          pEntity->GetAbsOrigin().DistToSqr(
              m_UnreachableEnts[i].vLocationWhenUnreachable) >
              UNREACHABLE_DIST_TOLERANCE_SQ) {
        m_UnreachableEnts.FastRemove(i);
        return false;
      }
      return true;
...
bool CAI_BaseNPC::IsValidEnemy(CBaseEntity *pEnemy) {
  ...
  if (pEnemyNPC && pEnemyNPC->CanBeAnEnemyOf(this) == false) return false;

  // Test our enemy filter
  if (m_hEnemyFilter.Get() != NULL &&
      m_hEnemyFilter->PassesFilter(this, pEnemy) == false)
    return false;
...
bool CAI_BaseNPC::CanBeAnEnemyOf(CBaseEntity *pEnemy) {
  if (GetSleepState() > AISS_WAITING_FOR_THREAT) return false;
...
//			Prefers reachable enemies over enemies that are
// unreachable,
//			regardless of priority.  For enemies that are both
//reachable
// or 			unreachable picks by priority.  If priority is the same,
// picks 			by distance.
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
CBaseEntity *CAI_BaseNPC::BestEnemy(void) {
  ...
  int iBestDistSq =
      MAX_COORD_RANGE *
      MAX_COORD_RANGE;  // so first visible entity will become the closest.
  ...
  for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
    ...
    if (m_bIgnoreUnseenEnemies) {
      ...
      if (pEMemory->timeLastSeen <
          gpGlobals->curtime - TIME_CONSIDER_ENEMY_UNSEEN) {
        DbgEnemyMsg(
            this,
    ...
    if (m_flAcceptableTimeSeenEnemy > 0.0 &&
        pEMemory->timeLastSeen < m_flAcceptableTimeSeenEnemy) {
    ...
    // Skip enemies I fear that I've never seen. (usually seen through an enemy
    // finder)
    if (relation == D_FR && !pEMemory->bUnforgettable &&
        pEMemory->timeFirstSeen == AI_INVALID_TIME) {
    ...
    // If best is reachable and current is unreachable, skip the unreachable
    // enemy regardless of priority
    ...
    //  If best is unreachable and current is reachable, always pick the current
    ...
    // If both are unreachable or both are reachable, choose enemy based on
    // priority and distance
      ...
      if (pBestEnemy)
        DbgEnemyMsg(this, "    (%s displaced due to priority, %d > %d )\n",
                    pBestEnemy->GetDebugName(), IRelationPriority(pEnemy),
                    iBestPriority);
      ...
      // If current is farther, and best has previously been confirmed as seen
      // or visible, move on
      if (!bCloser) {
      ...
      // If current is closer, and best has previously been confirmed as not
      // seen and not visible, take it
      if (bCloser) {
      ...
      if (!bAcceptCurrent) {
        ...
        if (!bCloser) {
          if (fBestSeen == TRS_NONE) {
            fBestSeen =
                (GetSenses()->DidSeeEntity(pBestEnemy)) ? TRS_TRUE : TRS_FALSE;
          }
        ...
        if (bCloser && fBestVisible == TRS_NONE) {
          ...
          fCurVisible = ((EnemyDistance(pEnemy) < GetSenses()->GetDistLook()) &&
                         FVisible(pEnemy))
                            ? TRS_TRUE
                            : TRS_FALSE;
        ...
        if (!bAcceptCurrent) {
          ...
          if (fBestVisible == TRS_NONE) {
            fBestVisible =
                ((EnemyDistance(pBestEnemy) < GetSenses()->GetDistLook()) &&
                 FVisible(pBestEnemy))
                    ? TRS_TRUE
                    : TRS_FALSE;
          }
          ...
          if (fCurSeen == TRS_NONE) {
            fCurSeen =
                (GetSenses()->DidSeeEntity(pEnemy)) ? TRS_TRUE : TRS_FALSE;
          }
          ...
          bool bBestSeenOrVisible =
              (fBestSeen == TRS_TRUE || fBestVisible == TRS_TRUE);
          bool bCurSeenOrVisible =
              (fCurSeen == TRS_TRUE || fCurVisible == TRS_TRUE);
...
Vector CAI_BaseNPC::EyeOffset(Activity nActivity) {
  if (CapabilitiesGet() & bits_CAP_DUCK) {
    if (IsCrouchedActivity(nActivity)) return GetCrouchEyeOffset();
  }
...
Vector CAI_BaseNPC::EyePosition(void) {
  if (IsCrouching()) return GetAbsOrigin() + GetCrouchEyeOffset();
...
void CAI_BaseNPC::HandleAnimEvent(animevent_t *pEvent) {
  ...
  switch (pEvent->event) {
    ...
    case SCRIPT_EVENT_NOT_DEAD:
      if (m_NPCState == NPC_STATE_SCRIPT) {
        m_lifeState = LIFE_ALIVE;
        // This is for life/death sequences where the player can determine
        // whether a character is dead or alive after the script
    ...
    case SCRIPT_EVENT_SENTENCE_RND1:  // Play a named sentence group 33% of the
                                      // time
      if (random->RandomInt(0, 2) == 0) break;
      // fall through...
    case SCRIPT_EVENT_SENTENCE:  // Play a named sentence group
      SENTENCEG_PlayRndSz(edict(), pEvent->options, 1.0, SNDLVL_TALKING, 0,
                          100);
      break;

    case SCRIPT_EVENT_FIREEVENT: {
      //
      // Fire a script event. The number of the script event to fire is in the
      // options string.
      //
      if (m_hCine != NULL) {
    ...
    case SCRIPT_EVENT_NOINTERRUPT:  // Can't be interrupted from now on
      if (m_hCine) m_hCine->AllowInterrupt(false);
      break;

    case SCRIPT_EVENT_CANINTERRUPT:  // OK to interrupt now
      if (m_hCine) m_hCine->AllowInterrupt(true);
      break;
    ...
    case NPC_EVENT_SWISHSOUND: {
      // NO NPC may use this anim event unless that npc's precache precaches
      // this sound!!!
    ...
    case NPC_EVENT_ITEM_PICKUP: {
      ...
      if (pEvent->options && strlen(pEvent->options) > 0) {
        ...
        pPickup = gEntList.FindEntityGenericNearest(pEvent->options,
                                                    GetAbsOrigin(), 256, this);
      } else {
        // Pick up the weapon or item that was found earlier and cached in our
        // target pointer.
        pPickup = GetTarget();
      }
    ...
    case NPC_EVENT_RIGHTFOOT:
      // For right now, do nothing. All functionality for this lives in
      // individual npcs.
      break;

    case NPC_EVENT_OPEN_DOOR: {
      CBasePropDoor *pDoor = (CBasePropDoor *)(CBaseEntity *)GetNavigator()
                                 ->GetPath()
                                 ->GetCurWaypoint()
                                 ->GetEHandleData();
      if (pDoor != NULL) {
    ...
    default:
      if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) &&
        ...
        if (pEvent->event == AE_NPC_HOLSTER) {
          ...
          // Force the NPC to recalculate it's arrival activity since it'll most
          // likely be wrong now that we don't have a weapon out.
          ...
          if (GetActiveWeapon()) {
            ...
            // Force the NPC to recalculate it's arrival activity since it'll
            // most likely be wrong now.
            GetNavigator()->SetArrivalSequence(ACT_INVALID);
          ...
          if ((pWeapon) && (pEvent->options)) {
            ...
            if (act == ACT_INVALID) {
              ...
              act = Weapon_TranslateActivity(
                  (Activity)CAI_BaseNPC::GetActivityID(pEvent->options), false);
            }
          ...
          if (m_hInteractionPartner) {
            ...
            if (pEvent->options) {
              ...
              Q_strncpy(szEventOptions, pEvent->options,
                        sizeof(szEventOptions));
            ...
            DevWarning(1,
                       "Bad %s format. Should be: { "
                       "AE_NPC_HURT_INTERACTION_PARTNER <frame number> "
                       "\"<ME/THEM> <Amount of damage done>\" }\n",
                       pName);
            return;
          }

          DevWarning(
              "%s received AE_NPC_HURT_INTERACTION_PARTNER anim event, but "
      ...
      if (pEvent->pSource != this ||
          (pEvent->type & AE_TYPE_NEWEVENTSYSTEM &&
           pEvent->type & AE_TYPE_WEAPON) ||
          (pEvent->event >= EVENT_WEAPON &&
           pEvent->event <= EVENT_WEAPON_LAST)) {
...
void CAI_BaseNPC::DrawDebugGeometryOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_NPC_KILL_BIT) {
    ...
    info.SetInflictor((AI_IsSinglePlayer())
                          ? (CBaseEntity *)AI_GetSinglePlayer()
                          : (CBaseEntity *)this);
  ...
  if ((m_debugOverlays & OVERLAY_NPC_ROUTE_BIT)) {
    ...
    if (IsMoving()) {
      ...
      NDebugOverlay::Line(WorldSpaceCenter(),
                          WorldSpaceCenter() + vecYaw * GetHullWidth() * .5,
                          255, 255, 255, true, 0.0);
    }
  }

  if (!(CAI_BaseNPC::m_nDebugBits & bits_debugDisableAI) &&
      (IsCurSchedule(SCHED_FORCED_GO) || IsCurSchedule(SCHED_FORCED_GO_RUN))) {
    NDebugOverlay::Box(m_vecLastPosition, Vector(-5, -5, -5), Vector(5, 5, 5),
                       255, 0, 255, 0, 0);
    NDebugOverlay::HorzArrow(GetAbsOrigin(), m_vecLastPosition, 16, 255, 0, 255,
                             64, true, 0);
  }
  ...
  if ((m_debugOverlays & OVERLAY_NPC_NEAREST_BIT)) {
    ...
    if (iNodeID != NO_NODE) {
      NDebugOverlay::Box(
          GetNavigator()->GetNetwork()->AccessNodes()[iNodeID]->GetPosition(
              GetHullType()),
          Vector(-10, -10, -10), Vector(10, 10, 10), 255, 255, 255, 0, 0);
    }
  }
  ...
  if ((m_debugOverlays & OVERLAY_NPC_VIEWCONE_BIT)) {
    ...
    // Visualize it
    NDebugOverlay::VertArrow(EyePosition(), EyePosition() + (vLeftDir * 200),
                             64, 255, 0, 0, 50, false, 0);
    NDebugOverlay::VertArrow(EyePosition(), EyePosition() + (vRightDir * 200),
                             64, 255, 0, 0, 50, false, 0);
    NDebugOverlay::VertArrow(EyePosition(), EyePosition() + (vEyeDir * 100), 8,
                             0, 255, 0, 50, false, 0);
    NDebugOverlay::Box(EyePosition(), -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255,
                       0, 128, 0);
  }
  ...
  if (m_debugOverlays & OVERLAY_NPC_RELATION_BIT) {
    ...
    // Rate all NPCs
    for (int i = 0; i < g_AI_Manager.NumAIs(); i++) {
      if (ppAIs[i] == NULL || ppAIs[i] == this) continue;
      ...
      // Draw an arrow
      NDebugOverlay::HorzArrow(GetAbsOrigin(), ppAIs[i]->GetAbsOrigin(), 16, r,
                               g, b, 64, true, 0.0f);
    }
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      // Draw an arrow
      NDebugOverlay::HorzArrow(GetAbsOrigin(), pPlayer->GetAbsOrigin(), 16, r,
                               g, b, 64, true, 0.0f);
    }
  }
  ...
  if ((m_debugOverlays & OVERLAY_NPC_ENEMIES_BIT)) {
    AIEnemiesIter_t iter;
    for (AI_EnemyInfo_t *eMemory = GetEnemies()->GetFirst(&iter);
         eMemory != NULL; eMemory = GetEnemies()->GetNext(&iter)) {
      if (eMemory->hEnemy) {
        CBaseCombatCharacter *npcEnemy =
            (eMemory->hEnemy)->MyCombatCharacterPointer();
        if (npcEnemy) {
          float r, g, b;
          char debugText[255];
          debugText[0] = NULL;
          ...
          if (npcEnemy == GetEnemy()) {
            Q_strncat(debugText, "Current Enemy", sizeof(debugText),
                      COPY_ALL_CHARACTERS);
          } else if (npcEnemy == GetTarget()) {
            Q_strncat(debugText, "Current Target", sizeof(debugText),
                      COPY_ALL_CHARACTERS);
          } else {
            Q_strncat(debugText, "Other Memory", sizeof(debugText),
                      COPY_ALL_CHARACTERS);
          }
          if (IsUnreachable(npcEnemy)) {
            Q_strncat(debugText, " (Unreachable)", sizeof(debugText),
                      COPY_ALL_CHARACTERS);
          }
          if (eMemory->bEludedMe) {
            Q_strncat(debugText, " (Eluded)", sizeof(debugText),
                      COPY_ALL_CHARACTERS);
          }
          ...
          // If has a line on the player draw cross slightly in front so player
          // can see
          if (npcEnemy->IsPlayer() &&
              (eMemory->vLastKnownLocation - npcEnemy->GetAbsOrigin())
                      .Length() < 10) {
            ...
            NDebugOverlay::Line(eyePos + sideVec + upVec,
                                eyePos - sideVec - upVec, r, g, b, false, 0);
            NDebugOverlay::Line(eyePos + sideVec - upVec,
                                eyePos - sideVec + upVec, r, g, b, false, 0);
            ...
            NDebugOverlay::Cross3D(
                drawPos, NAI_Hull::Mins(npcEnemy->GetHullType()),
                NAI_Hull::Maxs(npcEnemy->GetHullType()), r, g, b, false, 0);
          }
        }
      }
    }
  ...
  if ((m_debugOverlays & OVERLAY_NPC_FOCUS_BIT)) {
    if (GetEnemy() != NULL) {
      NDebugOverlay::Line(EyePosition(), GetEnemy()->EyePosition(), 255, 0, 0,
                          true, 0.0);
    }
    if (GetTarget() != NULL) {
      NDebugOverlay::Line(EyePosition(), GetTarget()->EyePosition(), 0, 0, 255,
                          true, 0.0);
    }
  }
...
int CAI_BaseNPC::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // Print Health
    // --------------
    Q_snprintf(tempstr, sizeof(tempstr), "Health: %i  (DACC:%1.2f)", m_iHealth,
    ...
    if (GetCurSchedule()) {
      ...
      if (pBehavior) {
        Q_snprintf(tempstr, sizeof(tempstr), "Behv: %s, ",
                   pBehavior->GetName());
        EntityText(text_offset, tempstr, 0);
        text_offset++;
      }
      ...
      if (m_debugOverlays & OVERLAY_NPC_TASK_BIT) {
        ...
        if (pTask) {
          Q_snprintf(tempstr, sizeof(tempstr), "Task: %s (#%d), ",
                     TaskName(pTask->iTask), GetScheduleCurTaskIndex());
        } else {
    ...
    if (m_Activity != ACT_INVALID && m_IdealActivity != ACT_INVALID &&
      ...
      Q_snprintf(tempstr, sizeof(tempstr), "Actv: %s (%s) [%s]\n", pszActivity,
                 pszIdealActivity, pszRootActivity);
    ...
    if (GetAbsVelocity() != vec3_origin ||
        GetLocalAngularVelocity() != vec3_angle) {
      char tmp[512];
      Q_snprintf(tmp, sizeof(tmp), "Vel %.1f %.1f %.1f   Ang: %.1f %.1f %.1f\n",
                 GetAbsVelocity().x, GetAbsVelocity().y, GetAbsVelocity().z,
                 GetLocalAngularVelocity().x, GetLocalAngularVelocity().y,
                 GetLocalAngularVelocity().z);
    ...
    if (m_LastShootAccuracy != -1 && ai_shot_stats.GetBool()) {
      CFmtStr msg;
      EntityText(text_offset,
                 msg.sprintf("Cur Accuracy: %.1f", m_LastShootAccuracy), 0);
      text_offset++;
      if (m_TotalShots) {
        EntityText(
            text_offset,
            msg.sprintf("Act Accuracy: %.1f",
                        ((float)m_TotalHits / (float)m_TotalShots) * 100.0),
            0);
        text_offset++;
      }
      ...
      if (GetActiveWeapon() && GetEnemy()) {
        ...
        EntityText(text_offset,
                   msg.sprintf("Cone %.1f, Bias %.2f", curCone, bias), 0);
        text_offset++;
      }
    }
    ...
    if (GetGoalEnt() && GetNavigator()->GetGoalType() == GOALTYPE_PATHCORNER) {
      ...
      if (GetGoalEnt()->GetEntityName() != NULL_STRING) {
        Q_strncat(tempstr, STRING(GetGoalEnt()->GetEntityName()),
                  sizeof(tempstr), COPY_ALL_CHARACTERS);
      } else {
        Q_strncat(tempstr, STRING(GetGoalEnt()->m_iClassname), sizeof(tempstr),
                  COPY_ALL_CHARACTERS);
      }
      EntityText(text_offset, tempstr, 0);
      text_offset++;
    ...
    if (VPhysicsGetObject()) {
      ...
      Q_snprintf(tempstr, sizeof(tempstr), "Stress: %.2f",
    ...
    if (m_pSquad) {
      ...
      Q_snprintf(tempstr, sizeof(tempstr), "SquadSlot:%s",
                 GetSquadSlotDebugName(GetMyStrategySlot()));
      EntityText(text_offset, tempstr, 0);
      text_offset++;
    }
  }
...
ConVar ai_report_task_timings_on_limit("ai_report_task_timings_on_limit", "0",
                                       FCVAR_ARCHIVE);
ConVar ai_think_limit_label("ai_think_limit_label", "0", FCVAR_ARCHIVE);

void CAI_BaseNPC::ReportOverThinkLimit(float time) {
  DevMsg(
      "%s thinking for %.02fms!!! (%s); r%.2f (c%.2f, pst%.2f, ms%.2f), "
      "p-r%.2f, m%.2f\n",
  ...
  if (ai_think_limit_label.GetBool()) {
    ...
    NDebugOverlay::Text(
        tmp,
        (char *)(const char *)CFmtStr("Slow %.1f, %s %.1f ", time, pszMax, max),
        false, 1);
  }
...
//			sequence that is trying to possess it. If DisregardState
//is  set, the npc 			will be sucked into the script no matter
//what state it is  in. ONLY 			Scripted AI ents should allow
// this.
// Input  : fDisregardNPCState -
//			interruptLevel -
//			eMode - If the function returns true, eMode will be one
//of  the following values: 				CAN_PLAY_NOW
//				CAN_PLAY_ENQUEUED
// Output :
//-----------------------------------------------------------------------------
CanPlaySequence_t CAI_BaseNPC::CanPlaySequence(bool fDisregardNPCState,
                                               int interruptLevel) {
  CanPlaySequence_t eReturn = CAN_PLAY_NOW;

  if (m_hCine) {
    if (!m_hCine->CanEnqueueAfter()) {
      // npc is already running one scripted sequence and has an important
      // script to play next
      return CANNOT_PLAY;
    }
  ...
  if (m_NPCState == NPC_STATE_NONE || m_NPCState == NPC_STATE_IDLE ||
...
Vector CAI_BaseNPC::GetShootEnemyDir(const Vector &shootOrigin, bool bNoisy) {
  ...
  if (pEnemy) {
    ...
    Vector vecEnemyOffset =
        pEnemy->BodyTarget(shootOrigin, bNoisy) - pEnemy->GetAbsOrigin();

#ifdef PORTAL
    // Translate the enemy's position across the portals if it's only seen in
    // the portal view cone
    if (!FInViewCone(vecEnemyLKP) || !FVisible(vecEnemyLKP)) {
      ...
      if (pPortal) {
        UTIL_Portal_VectorTransform(
            pPortal->m_hLinkedPortal->MatrixThisToLinked(), vecEnemyOffset,
            vecEnemyOffset);
        UTIL_Portal_PointTransform(
            pPortal->m_hLinkedPortal->MatrixThisToLinked(), vecEnemyLKP,
            vecEnemyLKP);
      }
    }
#endif
...
//-----------------------------------------------------------------------------
void CAI_BaseNPC::CollectShotStats(const Vector &vecShootOrigin,
  ...
  if (ai_shot_stats.GetBool() != 0 && GetEnemy()->IsPlayer()) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      manipulator.ApplySpread(GetAttackSpread(GetActiveWeapon(), GetEnemy()),
                              GetSpreadBias(GetActiveWeapon(), GetEnemy()));
      Vector shotDir = manipulator.GetResult();

      Vector vecEnd = vecShootOrigin + shotDir * 8192;

      trace_t tr;
      AI_TraceLine(vecShootOrigin, vecEnd, MASK_SHOT, this,
                   COLLISION_GROUP_NONE, &tr);
...
Vector CAI_BaseNPC::GetActualShootPosition(const Vector &shootOrigin) {
  ...
  Vector vecEnemyLKP = GetEnemyLKP();
  Vector vecEnemyOffset =
      GetEnemy()->BodyTarget(shootOrigin) - GetEnemy()->GetAbsOrigin();
  ...
  if (pPortal) {
    ...
    UTIL_Portal_VectorTransform(pPortal->m_hLinkedPortal->MatrixThisToLinked(),
                                vecEnemyOffset, vecEnemyOffsetTransformed);
    UTIL_Portal_PointTransform(pPortal->m_hLinkedPortal->MatrixThisToLinked(),
                               vecEnemyLKP, vecEnemyLKPTransformed);
    Vector vecTargetPositionTransformed =
        vecEnemyOffsetTransformed + vecEnemyLKPTransformed;

    // Get the distance to the target with and without portals
    float fDistanceToEnemyThroughPortalSqr =
        GetAbsOrigin().DistToSqr(vecTargetPositionTransformed);
    float fDistanceToEnemySqr = GetAbsOrigin().DistToSqr(vecTargetPosition);

    if (fDistanceToEnemyThroughPortalSqr < fDistanceToEnemySqr ||
        !FInViewCone(vecEnemyLKP) || !FVisible(vecEnemyLKP)) {
  ...
  return (vecTargetPosition +
          (GetEnemy()->GetSmoothedVelocity() * ai_lead_time.GetFloat()));
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
float CAI_BaseNPC::GetSpreadBias(CBaseCombatWeapon *pWeapon,
                                 CBaseEntity *pTarget) {
  float bias = BaseClass::GetSpreadBias(pWeapon, pTarget);
  AI_EnemyInfo_t *pEnemyInfo = m_pEnemies->Find(pTarget);
  if (ai_shot_bias.GetFloat() != 1.0) bias = ai_shot_bias.GetFloat();
  if (pEnemyInfo) {
    float timeToFocus = ai_spread_pattern_focus_time.GetFloat();
    if (timeToFocus > 0.0) {
      float timeSinceValidEnemy =
          gpGlobals->curtime - pEnemyInfo->timeValidEnemy;
      if (timeSinceValidEnemy < 0.0f) {
        timeSinceValidEnemy = 0.0f;
      }
      float timeSinceReacquire =
          gpGlobals->curtime - pEnemyInfo->timeLastReacquired;
      if (timeSinceValidEnemy < timeToFocus) {
        float scale = timeSinceValidEnemy / timeToFocus;
        Assert(scale >= 0.0 && scale <= 1.0);
        bias *= scale;
      } else if (timeSinceReacquire < timeToFocus)  // handled seperately as
                                                    // might be tuning
                                                    // seperately
      {
...
//-----------------------------------------------------------------------------
Vector CAI_BaseNPC::GetAttackSpread(CBaseCombatWeapon *pWeapon,
                                    CBaseEntity *pTarget) {
  ...
  if (pEnemyInfo) {
    ...
    if (timeToFocus > 0.0) {
      float timeSinceValidEnemy =
          gpGlobals->curtime - pEnemyInfo->timeValidEnemy;
      if (timeSinceValidEnemy < 0) timeSinceValidEnemy = 0;
      if (timeSinceValidEnemy < timeToFocus) {
...
// concealment, etc.
//
// Ultimately, this code results in the shooter aiming his weapon somewhere
// ahead of a moving target. Since bullet traces are instant, aiming ahead of a
// target will result in more misses than hits. This is designed to provide
// targets with a bonus when moving perpendicular to the shooter's line of
// sight.
//
// Do not confuse this with leading a target in an effort to more easily hit it.
//
// This code PENALIZES a target for moving directly along the shooter's line of
// sight.
//
// This code REWARDS a target for moving perpendicular to the shooter's line of
// sight.
//-----------------------------------------------------------------------------
Vector CAI_BaseNPC::GetActualShootTrajectory(const Vector &shootOrigin) {
  if (!GetEnemy()) return GetShootEnemyDir(shootOrigin);

  // If we're above water shooting at a player underwater, bias some of the
  // shots forward of the player so that they see the cool bubble trails in the
  // water ahead of them.
  if (GetEnemy()->IsPlayer() && (GetWaterLevel() != 3) &&
      (GetEnemy()->GetWaterLevel() == 3)) {
#if 1
    if (random->RandomInt(0, 4) < 3) {
      ...
      Vector vecAimPos = GetEnemy()->EyePosition() + (48.0f * vecEnemyForward) +
                         (flVelocityScale * GetEnemy()->GetSmoothedVelocity());
      // NDebugOverlay::Cross3D(vecAimPos, Vector(-16,-16,-16),
      // Vector(16,16,16), 255, 255, 0, true, 1.0f );

      // vecAimPos.z = UTIL_WaterLevel( vecAimPos, vecAimPos.z, vecAimPos.z +
      // 400.0f );  NDebugOverlay::Cross3D(vecAimPos, Vector(-32,-32,-32),
      // Vector(32,32,32), 255, 0, 0, true, 1.0f );
  ...
  if (pEnemy && pEnemy->ShouldShootMissTarget(this)) {
    ...
    AI_TraceLine(shootOrigin, vecEnd, MASK_SHOT, this, COLLISION_GROUP_NONE,
                 &tr);

    if (tr.fraction != 1.0 && tr.m_pEnt &&
        tr.m_pEnt->m_takedamage != DAMAGE_NO) {
...
bool CAI_BaseNPC::FacingIdeal(void) {
  if (fabs(GetMotor()->DeltaIdealYaw()) <=
      0.006)  //!!!BUGBUG - no magic numbers!!!
  {
    return true;
  }
...
void CAI_BaseNPC::AddFacingTarget(CBaseEntity *pTarget, float flImportance,
                                  float flDuration, float flRamp) {
  GetMotor()->AddFacingTarget(pTarget, flImportance, flDuration, flRamp);
}

void CAI_BaseNPC::AddFacingTarget(const Vector &vecPosition, float flImportance,
                                  float flDuration, float flRamp) {
  GetMotor()->AddFacingTarget(vecPosition, flImportance, flDuration, flRamp);
}

void CAI_BaseNPC::AddFacingTarget(CBaseEntity *pTarget,
                                  const Vector &vecPosition, float flImportance,
                                  float flDuration, float flRamp) {
  GetMotor()->AddFacingTarget(pTarget, vecPosition, flImportance, flDuration,
                              flRamp);
}
...
int CAI_BaseNPC::PlaySentence(const char *pszSentence, float delay,
  ...
  if (pszSentence && IsAlive()) {
    if (pszSentence[0] == '!') {
      ...
      CBaseEntity::EmitSentenceByIndex(filter, entindex(), CHAN_VOICE,
                                       sentenceIndex, volume, soundlevel, 0,
                                       PITCH_NORM);
    } else {
      sentenceIndex = SENTENCEG_PlayRndSz(edict(), pszSentence, volume,
                                          soundlevel, 0, PITCH_NORM);
    }
  }
...
int CAI_BaseNPC::PlayScriptedSentence(const char *pszSentence, float delay,
                                      float volume, soundlevel_t soundlevel,
                                      bool bConcurrent,
                                      CBaseEntity *pListener) {
...
CBaseEntity *CAI_BaseNPC::FindNamedEntity(const char *name,
  ...
  if (!stricmp(name, "!player")) {
    ...
    if (GetEnemy() != NULL) return GetEnemy();
    ...
    if (++playerwarningcount < 5) {
      DevMsg(
          "ERROR: \"player\" is no longer used, use \"!player\" in vcd "
          "instead!\n");
    }
    return (CBaseEntity *)AI_GetSinglePlayer();
  } else {
    ...
    for (iCount = 0; iCount < FINDNAMEDENTITY_MAX_ENTITIES; iCount++) {
      entity =
          gEntList.FindEntityByName(entity, name, NULL, NULL, NULL, pFilter);
      if (!entity) {
        break;
      }
...
bool CAI_BaseNPC::BBoxFlat(void) {
  ...
  AI_TraceLine(vecPoint, vecPoint - Vector(0, 0, 100), MASK_NPCSOLID_BRUSHONLY,
...
const Vector &CAI_BaseNPC::GetEnemyLKP() const {
  return (const_cast<CAI_BaseNPC *>(this))
      ->GetEnemies()
      ->LastKnownPosition(GetEnemy());
}

float CAI_BaseNPC::GetEnemyLastTimeSeen() const {
  return (const_cast<CAI_BaseNPC *>(this))
      ->GetEnemies()
      ->LastTimeSeen(GetEnemy());
}
...
bool CAI_BaseNPC::ShouldChooseNewEnemy() {
  ...
  if (pEnemy) {
    ...
    if (HasCondition(COND_SEE_HATE) || HasCondition(COND_SEE_DISLIKE) ||
        HasCondition(COND_SEE_NEMESIS) || HasCondition(COND_SEE_FEAR)) {
    ...
    if (m_FailChooseEnemyTimer.Expired()) {
      ...
      if (HasCondition(COND_TASK_FAILED) ||
          (pInfo && (pInfo->timeAtFirstHand == AI_INVALID_TIME ||
                     gpGlobals->curtime - pInfo->timeLastSeen > 10))) {
        return true;
      }
    }
...
bool CAI_BaseNPC::ChooseEnemy(void) {
  ...
  // Use memory bits in case enemy pointer altered outside this function, (e.g.,
  // ehandle goes NULL)
  ...
  bool fEnemyEluded =
      (fEnemyWentNull ||
       (pInitialEnemy && GetEnemies()->HasEludedMe(pInitialEnemy)));
  ...
  if (!m_ScheduleState.bScheduleWasInterrupted && GetCurSchedule() &&
      !FScheduleDone()) {
    Assert(InterruptFromCondition(COND_NEW_ENEMY) == COND_NEW_ENEMY &&
           InterruptFromCondition(COND_LOST_ENEMY) == COND_LOST_ENEMY);
    ...
    fHaveCondNewEnemy = true;  // not having a schedule is the same as being
                               // interruptable by any condition
    fHaveCondLostEnemy = true;
  }

  if (!fEnemyWentNull) {
    if (!fHaveCondNewEnemy && !(fHaveCondLostEnemy && fEnemyEluded)) {
      // DO NOT mess with the npc's enemy pointer unless the schedule the npc is
      // currently running will be interrupted by COND_NEW_ENEMY or
      // COND_LOST_ENEMY. This will eliminate the problem of npcs getting a new
      // enemy while they are in a schedule that doesn't care, and then not
      // realizing it by the time they get to a schedule that does. I don't feel
      // this is a good permanent fix.
  ...
  if (fChangingEnemy || fEnemyWentNull) {
    ...
    if (!pChosenEnemy) {
      // Don't break on enemies going 0 if they've been killed
      ...
      Remember((pChosenEnemy->IsPlayer()) ? bits_MEMORY_HAD_PLAYER
...
//=========================================================
CBaseEntity *CAI_BaseNPC::DropItem(char *pszItemName, Vector vecPos,
                                   QAngle vecAng) {
  if (!pszItemName) {
  ...
  if (pItem) {
    ...
    if (pPhys) {
      ...
      pItem->ApplyLocalAngularVelocityImpulse(
          AngularImpulse(0, random->RandomFloat(0, 100), 0));
    }
...
void CAI_BaseNPC::MakeAIFootstepSound(float volume, float duration) {
  CSoundEnt::InsertSound(SOUND_COMBAT, EyePosition(), volume, duration, this,
...
void CAI_BaseNPC::JustMadeSound(int soundPriority, float flSoundLength) {
  m_flSoundWaitTime =
      gpGlobals->curtime + flSoundLength + random->RandomFloat(1.5, 2.0);
  m_nSoundPriority = soundPriority;

  if (m_pSquad) {
    m_pSquad->JustMadeSound(soundPriority, gpGlobals->curtime + flSoundLength +
                                               random->RandomFloat(1.5, 2.0));
  }
}
...
//								m_pSchedule  (reacquired
//on  restore)
DEFINE_EMBEDDED(m_ScheduleState),
    DEFINE_FIELD(m_IdealSchedule, FIELD_INTEGER),  // handled specially but left
                                                   // in for "virtual" schedules
    DEFINE_FIELD(m_failSchedule, FIELD_INTEGER),   // handled specially but left
                                                   // in for "virtual" schedules
    ...
    //(custom  save)
    //								m_poseAim_Pitch (not saved; recomputed
    //on  restore)
    //								m_poseAim_Yaw (not saved; recomputed
    //on  restore)
    //								m_poseMove_Yaw (not saved; recomputed
    //on  restore)
    ...
    DEFINE_EMBEDDEDBYREF(m_pSenses), DEFINE_EMBEDDEDBYREF(m_pLockedBestSound),
    ...
    DEFINE_EMBEDDEDBYREF(m_pNavigator), DEFINE_EMBEDDEDBYREF(m_pLocalNavigator),
    DEFINE_EMBEDDEDBYREF(m_pPathfinder), DEFINE_EMBEDDEDBYREF(m_pMoveProbe),
    ...
    // 							m_pEnemies					Saved specially
    // in ai_saverestore.cpp
    ...
    //							m_fIsUsingSmallHull			TODO -- This needs
    //more  consideration than simple save/load
    // 							m_failText
    // DEBUG 							m_interruptText
    // DEBUG 							m_failedSchedule
    // DEBUG
    // 							m_interuptSchedule
    // DEBUG 							m_nDebugCurIndex
    // DEBUG

    // 							m_LastShootAccuracy
    // DEBUG
    // 							m_RecentShotAccuracy
    // DEBUG 							m_TotalShots
    // DEBUG 							m_TotalHits
    // DEBUG
    //							m_bSelected
    ...
    DEFINE_OUTPUT(m_OnWake, "OnWake"), DEFINE_OUTPUT(m_OnSleep, "OnSleep"),
    ...
    DEFINE_INPUTFUNC(FIELD_FLOAT, "IgnoreDangerSounds",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "ActivateSpeedModifier",
                     InputActivateSpeedModifier),
    DEFINE_INPUTFUNC(FIELD_VOID, "DisableSpeedModifier",
                     InputDisableSpeedModifier),
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetSpeedModRadius",
                     InputSetSpeedModifierRadius),
    DEFINE_INPUTFUNC(FIELD_INTEGER, "SetSpeedModSpeed",
                     InputSetSpeedModifierSpeed),
    DEFINE_INPUTFUNC(FIELD_VOID, "HolsterWeapon", InputHolsterWeapon),
    DEFINE_INPUTFUNC(FIELD_VOID, "HolsterAndDestroyWeapon",
                     InputHolsterAndDestroyWeapon),
    DEFINE_INPUTFUNC(FIELD_VOID, "UnholsterWeapon", InputUnholsterWeapon),
    DEFINE_INPUTFUNC(FIELD_STRING, "ForceInteractionWithNPC",
                     InputForceInteractionWithNPC),
    DEFINE_INPUTFUNC(FIELD_STRING, "UpdateEnemyMemory", InputUpdateEnemyMemory),

    // Function pointers
    DEFINE_USEFUNC(NPCUse), DEFINE_THINKFUNC(CallNPCThink),
    DEFINE_THINKFUNC(CorpseFallThink), DEFINE_THINKFUNC(NPCInitThink),
    ...
    SendPropBool(SENDINFO(m_bIsMoving)), SendPropBool(SENDINFO(m_bFadeCorpse)),
    ...
    DEFINE_FIELD(flDelay, FIELD_FLOAT), DEFINE_FIELD(flDistSqr, FIELD_FLOAT),
...
void CAI_BaseNPC::Activate(void) {
  ...
  if (m_iszEnemyFilterName != NULL_STRING) {
    CBaseEntity *pFilter =
        gEntList.FindEntityByName(NULL, m_iszEnemyFilterName);
    if (pFilter != NULL) {
...
void CAI_BaseNPC::Precache(void) {
  gm_iszPlayerSquad = AllocPooledString(
      PLAYER_SQUADNAME);  // cache for fast IsPlayerSquad calls
...
struct AIExtendedSaveHeader_t {
  AIExtendedSaveHeader_t()
      : version(AI_EXTENDED_SAVE_HEADER_VERSION), flags(0), scheduleCrc(0) {
    szSchedule[0] = 0;
    szIdealSchedule[0] = 0;
    szFailSchedule[0] = 0;
    szSequence[0] = 0;
...
DEFINE_FIELD(version, FIELD_SHORT), DEFINE_FIELD(flags, FIELD_INTEGER),
  ...
  if (GetEnemy()) saveHeader.flags |= AIESH_HAD_ENEMY;
  if (GetTarget()) saveHeader.flags |= AIESH_HAD_TARGET;
  if (GetNavigator()->IsGoalActive()) saveHeader.flags |= AIESH_HAD_NAVGOAL;

  if (m_pSchedule) {
    const char *pszSchedule = m_pSchedule->GetName();

    Assert(Q_strlen(pszSchedule) < sizeof(saveHeader.szSchedule) - 1);
    Q_strncpy(saveHeader.szSchedule, pszSchedule,
              sizeof(saveHeader.szSchedule));

    CRC32_Init(&saveHeader.scheduleCrc);
    CRC32_ProcessBuffer(&saveHeader.scheduleCrc,
                        (void *)m_pSchedule->GetTaskList(),
                        m_pSchedule->NumTasks() * sizeof(Task_t));
  ...
  if (idealSchedule != -1 && idealSchedule != AI_RemapToGlobal(SCHED_NONE) &&
      idealSchedule != AI_RemapToGlobal(SCHED_AISCRIPT)) {
    CAI_Schedule *pIdealSchedule = GetSchedule(m_IdealSchedule);
    if (pIdealSchedule) {
      const char *pszIdealSchedule = pIdealSchedule->GetName();
      Assert(Q_strlen(pszIdealSchedule) <
             sizeof(saveHeader.szIdealSchedule) - 1);
      Q_strncpy(saveHeader.szIdealSchedule, pszIdealSchedule,
                sizeof(saveHeader.szIdealSchedule));
    }
  }

  int failSchedule = GetGlobalScheduleId(m_failSchedule);
  if (failSchedule != -1 && failSchedule != AI_RemapToGlobal(SCHED_NONE) &&
      failSchedule != AI_RemapToGlobal(SCHED_AISCRIPT)) {
    ...
    if (pFailSchedule) {
      ...
      Q_strncpy(saveHeader.szFailSchedule, pszFailSchedule,
                sizeof(saveHeader.szFailSchedule));
    }
  }
  ...
  if (GetSequence() != ACT_INVALID && GetModelPtr()) {
    ...
    if (pszSequenceName && *pszSequenceName) {
      ...
      Q_strncpy(saveHeader.szSequence, pszSequenceName,
                sizeof(saveHeader.szSequence));
    }
  }
...
void CAI_BaseNPC::OnRestore() {
  ...
  if (m_bDoPostRestoreRefindPath && CAI_NetworkManager::NetworksLoaded()) {
    ...
    if (!GetNavigator()->RefindPathToGoal(false)) DiscardScheduleState();
  } else {
    GetNavigator()->ClearGoal();
  }
  BaseClass::OnRestore();
...
int CAI_BaseNPC::Restore(IRestore &restore) {
  ...
  // Do schedule fix-up
  if (saveHeader.version >=
      AI_EXTENDED_SAVE_HEADER_FIRST_VERSION_WITH_SCHEDULE_ID_FIXUP) {
    if (saveHeader.szIdealSchedule[0]) {
      CAI_Schedule *pIdealSchedule =
          g_AI_SchedulesManager.GetScheduleByName(saveHeader.szIdealSchedule);
      m_IdealSchedule = (pIdealSchedule) ? pIdealSchedule->GetId() : SCHED_NONE;
    }

    if (saveHeader.szFailSchedule[0]) {
      CAI_Schedule *pFailSchedule =
          g_AI_SchedulesManager.GetScheduleByName(saveHeader.szFailSchedule);
  ...
  bool bLostScript = (m_NPCState == NPC_STATE_SCRIPT && m_hCine == NULL);
  bool bDiscardScheduleState =
      (bLostScript || bLostSequence || saveHeader.szSchedule[0] == 0 ||
  ...
  if (m_ScheduleState.taskFailureCode >= NUM_FAIL_CODES)
    m_ScheduleState.taskFailureCode =
        FAIL_NO_TARGET;  // must have been a string, gotta punt

  if (!bDiscardScheduleState) {
    m_pSchedule =
        g_AI_SchedulesManager.GetScheduleByName(saveHeader.szSchedule);
    if (m_pSchedule) {
      CRC32_t scheduleCrc;
      CRC32_Init(&scheduleCrc);
      CRC32_ProcessBuffer(&scheduleCrc, (void *)m_pSchedule->GetTaskList(),
                          m_pSchedule->NumTasks() * sizeof(Task_t));
...
void CAI_BaseNPC::SaveConditions(ISave &save,
                                 const CAI_ScheduleBits &conditions) {
  for (int i = 0; i < MAX_CONDITIONS; i++) {
    if (conditions.IsBitSet(i)) {
      const char *pszConditionName = ConditionName(AI_RemapToGlobal(i));
      if (!pszConditionName) break;
...
void CAI_BaseNPC::RestoreConditions(IRestore &restore,
                                    CAI_ScheduleBits *pConditions) {
  pConditions->ClearAll();
  char szCondition[256];
  for (;;) {
    restore.ReadString(szCondition, sizeof(szCondition), 0);
    if (!szCondition[0]) break;
    int iCondition = GetSchedulingSymbols()->ConditionSymbolToId(szCondition);
    if (iCondition != -1) pConditions->Set(AI_RemapFromGlobal(iCondition));
  }
}
...
CAI_BaseNPC::CAI_BaseNPC(void)
    : m_UnreachableEnts(0, 4), m_bDeferredNavigation(false) {
  m_pMotor = NULL;
  m_pMoveProbe = NULL;
  m_pNavigator = NULL;
  m_pSenses = NULL;
  ...
  SetHullType(
      HULL_HUMAN);  // Give human hull by default, subclasses should override
...
void CAI_BaseNPC::InputSetEnemyFilter(inputdata_t &inputdata) {
  // Get a handle to my enemy filter entity if there is one.
  CBaseEntity *pFilter =
      gEntList.FindEntityByName(NULL, inputdata.value.StringID());
  m_hEnemyFilter = dynamic_cast<CBaseFilter *>(pFilter);
}
...
void CAI_BaseNPC::InputForgetEntity(inputdata_t &inputdata) {
  ...
  if (g_pDeveloper->GetInt() &&
      pszEntityToForget[strlen(pszEntityToForget) - 1] == '*')
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::HandleInteraction(int interactionType, void *data,
...
CAI_BaseNPC *CAI_BaseNPC::GetInteractionPartner(void) {
  if (m_hInteractionPartner == NULL) return NULL;
...
ConVar sv_test_scripted_sequences("sv_test_scripted_sequences", "0", FCVAR_NONE,
                                  "Tests for scripted sequences that are "
                                  "embedded in the world. Run through your map "
                                  "with this set to check for NPCs falling "
                                  "through the world.");
...
bool CAI_BaseNPC::CineCleanup() {
  ...
  if (IsRunningDynamicInteraction()) {
    ...
    if (m_hInteractionPartner) {
      ...
      // GetDebugName(), m_hInteractionPartner->GetClassName(),
      // m_hInteractionPartner->GetDebugName(), gpGlobals->curtime );
    }
    ...
    // Clear interaction partner, because we're not running a scripted sequence
    // anymore
  ...
  if (m_hCine) {
    ...
    // NOTE that this will have had EF_NODRAW removed in script.dll when it's
    // cached off
  ...
  if (m_lifeState == LIFE_DYING) {
    ...
    UTIL_SetSize(
        this, WorldAlignMins(),
        Vector(WorldAlignMaxs().x, WorldAlignMaxs().y, WorldAlignMins().z + 2));
    ...
    if (CanBecomeRagdoll() == false) {
      StopAnimation();
      AddEffects(EF_NOINTERP);  // Don't interpolate either, assume the corpse
                                // is positioned in its final resting place
    }
  ...
  if (pOldCine && pOldCine->m_iszPlay != NULL_STRING &&
      pOldCine->PlayedSequence()) {
    if (!pOldCine->HasSpawnFlags(SF_SCRIPT_DONT_TELEPORT_AT_END)) {
      ...
      // We can't really solve this problem because we can't query the movement
      // of the origin relative to the sequence.  We can get the root bone's
      // position as we do here, but there are cases where the root bone is in a
      // different relative position to the entity's origin before/after the
      // sequence plays.  So we are stuck doing this:

      // !!!HACKHACK: Float the origin up and drop to floor because some
      // sequences have irregular motion that can't be properly accounted for.
      ...
      // this really needs to be done with the AX,AY,etc. flags, but that aren't
      // consistantly being set, so animations that really do move won't be
      // caught.
    ...
    // We should have some animation to put these guys in, but for now it's
    // idle. Due to NOINTERP above, there won't be any blending between this
    // anim & the sequence
    m_Activity = ACT_RESET;
  }
...
void CAI_BaseNPC::Teleport(const Vector *newPosition, const QAngle *newAngles,
...
bool CAI_BaseNPC::FindSpotForNPCInRadius(Vector *pResult,
                                         const Vector &vStartPos,
                                         CAI_BaseNPC *pNPC, float radius,
                                         bool bOutOfPlayerViewcone) {
  CBasePlayer *pPlayer = AI_GetSinglePlayer();
  QAngle fan;
  ...
  for (fan.y = 0; fan.y < 360; fan.y += 18.0) {
    ...
    UTIL_TraceLine(vecTest, vecTest - Vector(0, 0, 8192), MASK_SHOT, pNPC,
                   COLLISION_GROUP_NONE, &tr);
    if (tr.fraction == 1.0) {
      continue;
    }

    UTIL_TraceHull(tr.endpos, tr.endpos + Vector(0, 0, 10), pNPC->GetHullMins(),
                   pNPC->GetHullMaxs(), MASK_NPCSOLID, pNPC,
                   COLLISION_GROUP_NONE, &tr);

    if (tr.fraction == 1.0 &&
        pNPC->GetMoveProbe()->CheckStandPosition(tr.endpos, MASK_NPCSOLID)) {
      *pResult = tr.endpos;
      return true;
    }
  }
...
bool CAI_BaseNPC::IsNavigationUrgent() {
  // return true if the navigation is for something that can't react well to
  // failure
...
bool CAI_BaseNPC::ShouldFailNav(bool bMovementFailed) {
  ...
  if (ai_vehicle_avoidance.GetBool()) {
    // Never be blocked this way by a vehicle (creates too many headaches around
    // the levels)
    CBaseEntity *pEntity = GetNavigator()->GetBlockingEntity();
    if (pEntity && pEntity->GetServerVehicle()) {
      // Vital allies never get stuck, and urgent moves cannot be blocked by a
      // vehicle
  ...
  // It's up to the schedule that requested movement to deal with failed
  // movement.  Currently, only a handfull of schedules are considered Urgent,
  // and they need to deal with what to do when there's no route, which by
  // inspection they'd don't.
...
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::MovementCost(int moveType, const Vector &vecStart,
                               const Vector &vecEnd, float *pCost) {
  // We have nothing to say on the matter, but derived classes might
  return false;
}

bool CAI_BaseNPC::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                     float flInterval) {
  return false;
}
...
bool CAI_BaseNPC::OnCalcBaseMove(AILocalMoveGoal_t *pMoveGoal, float distClear,
                                 AIMoveResult_t *pResult) {
  if (pMoveGoal->directTrace.pObstruction) {
    CBasePropDoor *pPropDoor =
        dynamic_cast<CBasePropDoor *>(pMoveGoal->directTrace.pObstruction);
    if (pPropDoor &&
        OnUpcomingPropDoor(pMoveGoal, pPropDoor, distClear, pResult)) {
      return true;
    }
  }
...
bool CAI_BaseNPC::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal,
  ...
  if (pMoveGoal->directTrace.pObstruction) {
    CBaseDoor *pDoor =
        dynamic_cast<CBaseDoor *>(pMoveGoal->directTrace.pObstruction);
...
bool CAI_BaseNPC::OnUpcomingPropDoor(AILocalMoveGoal_t *pMoveGoal,
  ...
  if ((CapabilitiesGet() & bits_CAP_DOORS_GROUP) && !pDoor->IsDoorLocked() &&
      (pDoor->IsDoorClosed() || pDoor->IsDoorClosing())) {
    ...
    pOpenDoorRoute = GetPathfinder()->BuildLocalRoute(
        ...
        bits_BUILD_GROUND | bits_BUILD_IGNORE_NPCS, 0.0);

    if (pOpenDoorRoute) {
      if (AIIsDebuggingDoors(this)) {
        NDebugOverlay::Cross3D(opendata.vecStandPos + Vector(0, 0, 1), 32, 255,
                               255, 255, false, 1.0);
        Msg("Opening door!\n");
      }
...
//			the door moving. This is called either in response to an
// anim event
//			or as a fallback when we don't have an appropriate open
// activity.
//-----------------------------------------------------------------------------
void CAI_BaseNPC::OpenPropDoorNow(CBasePropDoor *pDoor) {
  ...
  // Wait for the door to finish opening before trying to move through the
  // doorway.
...
//-----------------------------------------------------------------------------
// Purpose: Called when the door we were trying to open becomes blocked before
// opening. Input  : pDoor -
//-----------------------------------------------------------------------------
...
//			do not spawn, but their keyvalues are saved for use by a
// template 			spawner.
//-----------------------------------------------------------------------------
...
int CAI_BaseNPC::FlyMove(const Vector &pfPosition, unsigned int mask) {
  ...
  if (trace.fraction == 1) {
    if ((GetFlags() & FL_SWIM) &&
        enginetrace->GetPointContents(trace.endpos) == CONTENTS_EMPTY)
...
static void AIMsgGuts(CAI_BaseNPC *pAI, unsigned flags, const char *pszMsg) {
  ...
  DevMsg(pszFmt2, pszMsg, pAI->GetClassname(), pAI->entindex(),
         (pAI->GetEntityName() == NULL_STRING) ? "<unnamed>"
                                               : STRING(pAI->GetEntityName()),
         gpGlobals->tickcount);
}

void DevMsg(CAI_BaseNPC *pAI, unsigned flags, const char *pszFormat, ...) {
  if ((flags & AIMF_IGNORE_SELECTED) ||
      (pAI->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)) {
...
bool CAI_BaseNPC::IsInPlayerSquad() const {
  return (m_pSquad &&
          MAKE_STRING(m_pSquad->GetName()) == GetPlayerSquadName() &&
          !CAI_Squad::IsSilentMember(this));
}
...
bool CAI_BaseNPC::IsCoverPosition(const Vector &vecThreat,
  ...
  // If I'm trying to find cover from the player, and the player is in a
  // vehicle, ignore the vehicle for the purpose of determining line of sight.
  CBaseEntity *pEnemy = GetEnemy();
  if (pEnemy) {
    // Hack to see if our threat position is our enemy
    bool bThreatPosIsEnemy =
        ((vecThreat - GetEnemy()->EyePosition()).LengthSqr() < 0.1f);
...
float CAI_BaseNPC::SetWait(float minWait, float maxWait) {
  ...
  if (maxWait == 0.0) {
    ...
    m_flWaitFinished =
        gpGlobals->curtime + (0.1 * random->RandomInt(minThinks, maxThinks));
  }
...
void CAI_BaseNPC::TestPlayerPushing(CBaseEntity *pEntity) {
  if (HasSpawnFlags(SF_NPC_NO_PLAYER_PUSHAWAY)) return;
...
void CAI_BaseNPC::CascadePlayerPush(const Vector &push,
                                    const Vector &pushOrigin) {
  //
  ...
  for (int i = 0; i < g_AI_Manager.NumAIs(); i++) {
    ...
    if (pOther != this && pOther->IRelationType(this) == D_LI &&
        !pOther->HasCondition(COND_PLAYER_PUSHING)) {
      const Vector &friendOrigin = pOther->GetAbsOrigin();
      if (fabsf(friendOrigin.z - origin.z) < MIN_Z_TO_TRANSMIT &&
          (friendOrigin.AsVector2D() - origin.AsVector2D()).LengthSqr() <
              DIST_REQD_TO_TRANSMIT_PUSH_SQ) {
        if (pushTestPoint == vec2_invalid) {
          pushTestPoint = origin2D - pushOrigin.AsVector2D();
          // No normalize, since it wants to just be a big number and we can't
          // be less that a hull away
          pushTestPoint *= 2000;
          pushTestPoint += origin2D;
        }
        float t;
        float distSq = CalcDistanceSqrToLine2D(friendOrigin.AsVector2D(),
                                               origin2D, pushTestPoint, &t);
...
void CAI_BaseNPC::Break(CBaseEntity *pBreaker) {
  ...
  breakablepropparams_t params(GetAbsOrigin(), GetAbsAngles(), velocity,
                               angVelocity);
...
bool CAI_BaseNPC::FindNearestValidGoalPos(const Vector &vTestPoint,
                                          Vector *pResult) {
  ...
  if (vCandidate == vec3_invalid) {
    ...
    if (iNearestNode != NO_NODE) {
      GetMoveProbe()->MoveLimit(
          NAV_GROUND, g_pBigAINet->GetNodePosition(GetHullType(), iNearestNode),
          vTestPoint, MASK_SOLID_BRUSHONLY, NULL, 0, &moveTrace);
      if ((moveTrace.vEndPosition - vTestPoint).Length2DSqr() <
              Square(GetHullWidth() * 3.0) &&
          GetMoveProbe()->CheckStandPosition(moveTrace.vEndPosition,
  ...
  if (vCandidate != vec3_invalid) {
    AI_Waypoint_t *pPathToPoint = GetPathfinder()->BuildRoute(
        GetAbsOrigin(), vCandidate, AI_GetSinglePlayer(), 5 * 12, NAV_NONE,
        true);
    if (pPathToPoint) {
  ...
  if (vCandidate == vec3_invalid) {
    GetMoveProbe()->MoveLimit(NAV_GROUND, GetAbsOrigin(), vTestPoint,
                              MASK_SOLID_BRUSHONLY, NULL, 0, &moveTrace);
    vCandidate = moveTrace.vEndPosition;
  }
...
float CAI_BaseNPC::LineOfSightDist(const Vector &vecDir, float zEye) {
  ...
  if (zEye == FLT_MAX) zEye = EyePosition().z;
...
void AI_TraceLOS(const Vector &vecAbsStart, const Vector &vecAbsEnd,
  ...
  if (ai_LOS_mode.GetBool()) {
    ...
    UTIL_TraceLine(vecAbsStart, vecAbsEnd, MASK_BLOCKLOS, pLooker,
  ...
  if (!pFilter) pFilter = &traceFilter;
...
void CAI_BaseNPC::ParseScriptedNPCInteractions(void) {
  ...
  if (!modelinfo->GetModelKeyValue(GetModel(), buf)) return;

  if (modelKeyValues->LoadFromBuffer(modelinfo->GetModelName(GetModel()),
                                     buf)) {
    // Do we have a dynamic interactions section?
    KeyValues *pkvInteractions =
        modelKeyValues->FindKey("dynamic_interactions");
    if (pkvInteractions) {
      ...
      while (pkvNode) {
        ...
        if (pszTrigger) {
          ...
          while (pszParam) {
            if (!Q_strncmp(pszParam, "on_damage", 9)) {
              sInteraction.iLoopBreakTriggerMethod |=
                  SNPCINT_LOOPBREAK_ON_DAMAGE;
            }
            if (!Q_strncmp(pszParam, "on_flashlight_illum", 19)) {
              sInteraction.iLoopBreakTriggerMethod |=
                  SNPCINT_LOOPBREAK_ON_FLASHLIGHT_ILLUM;
            }
        ...
        if (pszVelocity) {
          ...
          UTIL_StringToVector(sInteraction.vecRelativeVelocity.Base(),
                              pszVelocity);
        }
        ...
        if (pszSequence) {
          sInteraction.sPhases[SNPCINT_ENTRY].iszSequence =
              AllocPooledString(pszSequence);
        }
        // Entry Activity
        const char *pszActivity = pkvNode->GetString("entry_activity", NULL);
        if (pszActivity) {
          sInteraction.sPhases[SNPCINT_ENTRY].iActivity =
              GetActivityID(pszActivity);
        }

        // Sequence
        pszSequence = pkvNode->GetString("sequence", NULL);
        if (pszSequence) {
          sInteraction.sPhases[SNPCINT_SEQUENCE].iszSequence =
              AllocPooledString(pszSequence);
        }
        // Activity
        pszActivity = pkvNode->GetString("activity", NULL);
        if (pszActivity) {
          sInteraction.sPhases[SNPCINT_SEQUENCE].iActivity =
              GetActivityID(pszActivity);
        }
        ...
        if (pszSequence) {
          sInteraction.sPhases[SNPCINT_EXIT].iszSequence =
              AllocPooledString(pszSequence);
        }
        // Exit Activity
        pszActivity = pkvNode->GetString("exit_activity", NULL);
        if (pszActivity) {
          sInteraction.sPhases[SNPCINT_EXIT].iActivity =
              GetActivityID(pszActivity);
        }
        ...
        // Delta
        sInteraction.flDistSqr = pkvNode->GetFloat(
            "origin_max_delta", (DSS_MAX_DIST * DSS_MAX_DIST));
        ...
        // Fixup position?
        const char *pszDontFixup =
            pkvNode->GetString("dont_teleport_at_end", NULL);
        if (pszDontFixup) {
          if (!Q_stricmp(pszDontFixup, "me") ||
              !Q_stricmp(pszDontFixup, "both")) {
            sInteraction.iFlags |= SCNPC_FLAG_DONT_TELEPORT_AT_END_ME;
          } else if (!Q_stricmp(pszDontFixup, "them") ||
                     !Q_stricmp(pszDontFixup, "both")) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_BaseNPC::AddScriptedNPCInteraction(
  ...
  m_ScriptedInteractions[nNewIndex].matDesiredLocalToWorld.SetupMatrixOrgAngles(
      pInteraction->vecRelativeOrigin, pInteraction->angRelativeAngles);
}
...
const char *CAI_BaseNPC::GetScriptedNPCInteractionSequence(
  ...
  if (pInteraction->sPhases[iPhase].iActivity != ACT_INVALID) {
    int iSequence = SelectWeightedSequence(
        (Activity)pInteraction->sPhases[iPhase].iActivity);
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_BaseNPC::StartRunningInteraction(CAI_BaseNPC *pOtherNPC,
                                          bool bActive) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_BaseNPC::StartScriptedNPCInteraction(
    CAI_BaseNPC *pOtherNPC, ScriptedNPCInteraction_t *pInteraction,
    Vector vecOtherOrigin, QAngle angOtherAngles) {
  variant_t emptyVariant;
  ...
  if (pOtherNPC) {
    ...
    // GetDebugName(), pOtherNPC->GetClassName(), pOtherNPC->GetDebugName(),
    // gpGlobals->curtime );
    PhysDisableEntityCollisions(this, pOtherNPC);
  }

  // Determine which sequences we're going to use
  const char *pszEntrySequence =
      GetScriptedNPCInteractionSequence(pInteraction, SNPCINT_ENTRY);
  const char *pszSequence =
      GetScriptedNPCInteractionSequence(pInteraction, SNPCINT_SEQUENCE);
  const char *pszExitSequence =
      GetScriptedNPCInteractionSequence(pInteraction, SNPCINT_EXIT);
  ...
  if (ai_debug_dyninteractions.GetBool()) {
    if (pOtherNPC) {
      Msg("%s(%s) starting dynamic interaction \"%s\" with %s(%s).\n",
          ...
          STRING(pInteraction->iszInteractionName), pOtherNPC->GetClassname(),
          pOtherNPC->GetDebugName());
      if (pszEntrySequence) {
  ...
  char szSSName[256];
  if (pOtherNPC) {
    Q_snprintf(szSSName, sizeof(szSSName), "dss_%s%d%s%d", GetDebugName(),
               entindex(), pOtherNPC->GetDebugName(), pOtherNPC->entindex());
  } else {
    Q_snprintf(szSSName, sizeof(szSSName), "dss_%s%d", GetDebugName(),
               entindex());
  }
  string_t iszSSName = AllocPooledString(szSSName);

  // Setup next attempt
  pInteraction->flNextAttemptTime =
      gpGlobals->curtime + pInteraction->flDelay + RandomFloat(-2, 2);

  // Spawn a scripted sequence for this NPC to play the interaction anim
  CAI_ScriptedSequence *pMySequence =
      (CAI_ScriptedSequence *)CreateEntityByName("scripted_sequence");
  ...
  pMySequence->AddSpawnFlags(SF_SCRIPT_NOINTERRUPT | SF_SCRIPT_HIGH_PRIORITY |
                             SF_SCRIPT_OVERRIDESTATE);
  if ((pInteraction->iFlags & SCNPC_FLAG_DONT_TELEPORT_AT_END_ME) != 0) {
    pMySequence->AddSpawnFlags(SF_SCRIPT_DONT_TELEPORT_AT_END);
  }
  pMySequence->SetLoopActionSequence(
      (pInteraction->iFlags & SCNPC_FLAG_LOOP_IN_ACTION) != 0);
  ...
  if (pOtherNPC) {
    ...
    pTheirSequence->AddSpawnFlags(SF_SCRIPT_NOINTERRUPT |
    ...
    pTheirSequence->SetLoopActionSequence(
        (pInteraction->iFlags & SCNPC_FLAG_LOOP_IN_ACTION) != 0);
    ...
    pTheirSequence->SetupInteractionPosition(
        this, pInteraction->matDesiredLocalToWorld);
  }
  ...
  // Setup the outputs for both sequences. The first kills them both when it
  // finishes
  pMySequence->KeyValue("OnCancelFailedSequence",
                        UTIL_VarArgs("%s,Kill,,0,-1", szSSName));
  if (pszExitSequence) {
    pMySequence->KeyValue("OnPostIdleEndSequence",
                          UTIL_VarArgs("%s,Kill,,0,-1", szSSName));
    if (pTheirSequence) {
      pTheirSequence->KeyValue("OnPostIdleEndSequence",
                               UTIL_VarArgs("%s,Kill,,0,-1", szSSName));
    }
  } else {
    pMySequence->KeyValue("OnEndSequence",
                          UTIL_VarArgs("%s,Kill,,0,-1", szSSName));
    if (pTheirSequence) {
      pTheirSequence->KeyValue("OnEndSequence",
                               UTIL_VarArgs("%s,Kill,,0,-1", szSSName));
    }
  }
  if (pTheirSequence) {
    pTheirSequence->KeyValue("OnCancelFailedSequence",
                             UTIL_VarArgs("%s,Kill,,0,-1", szSSName));
  }
...
bool CAI_BaseNPC::CanRunAScriptedNPCInteraction(bool bForced) {
  ...
  if (bForced) {
    if (!m_hForcedInteractionPartner) return false;
  } else {
    if (m_hForcedInteractionPartner || m_hInteractionPartner) return false;
    if (IsInAScript() || !HasCondition(COND_IN_PVS)) return false;
    if (HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_MOVE_AWAY))
      return false;

    // Default AI prevents interactions while melee attacking, but not ranged
    // attacking
...
void CAI_BaseNPC::CheckForScriptedNPCInteractions(void) {
  ...
  if (!CanRunAScriptedNPCInteraction()) return;
  if (pNPC && !pNPC->CanRunAScriptedNPCInteraction()) return;
...
//-----------------------------------------------------------------------------
// Purpose: Calculate all the valid dynamic interactions we can perform with our
// current enemy
//-----------------------------------------------------------------------------
void CAI_BaseNPC::CalculateValidEnemyInteractions(void) {
  ...
  bool bDebug = (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT &&
                 ai_debug_dyninteractions.GetBool());
  if (bDebug) {
    Msg("%s(%s): Computing valid interactions with %s(%s)\n", GetClassname(),
        GetDebugName(), pNPC->GetClassname(), pNPC->GetDebugName());
  }
  ...
  for (int i = 0; i < m_ScriptedInteractions.Count(); i++) {
    ...
    // If the trigger method of the interaction isn't the one we're after, we're
    // done
    ...
    CTakeDamageInfo tempinfo(this, this, vec3_origin, vec3_origin, 1.0,
                             DMG_BULLET);
    if (!pNPC->PassesDamageFilter(tempinfo)) continue;

    // Check the weapon requirements for the interaction
    if (pInteraction->iFlags & SCNPC_FLAG_NEEDS_WEAPON_ME) {
      if (!GetActiveWeapon()) continue;

      // Check the specific weapon type
      if (pInteraction->iszMyWeapon != NULL_STRING &&
          GetActiveWeapon()->m_iClassname != pInteraction->iszMyWeapon)
        continue;
    }
    if (pInteraction->iFlags & SCNPC_FLAG_NEEDS_WEAPON_THEM) {
      if (!pNPC->GetActiveWeapon()) continue;

      // Check the specific weapon type
      if (pInteraction->iszTheirWeapon != NULL_STRING &&
          pNPC->GetActiveWeapon()->m_iClassname != pInteraction->iszTheirWeapon)
        continue;
    }
    ...
    if (pInteraction->sPhases[SNPCINT_SEQUENCE].iActivity != ACT_INVALID) {
      ...
      const char *pszSequence =
          GetScriptedNPCInteractionSequence(pInteraction, SNPCINT_SEQUENCE);
      if (!pszSequence) continue;
      if (pNPC->LookupSequence(pszSequence) == -1) continue;
    } else {
      if (pNPC->LookupSequence(STRING(
              pInteraction->sPhases[SNPCINT_SEQUENCE].iszSequence)) == -1)
        continue;
    }
...
void CAI_BaseNPC::CheckForcedNPCInteractions(void) {
  // If we don't have an interaction, we're waiting for our partner to start it.
  // Do nothing.
  ...
  if (!CanRunAScriptedNPCInteraction(true) ||
      !pNPC->CanRunAScriptedNPCInteraction(true)) {
  ...
  ScriptedNPCInteraction_t *pInteraction =
      &m_ScriptedInteractions[m_iInteractionPlaying];

  if (!bAbort) {
    if (!InteractionCouldStart(pNPC, pInteraction, vecOrigin, angAngles)) {
      if ((gpGlobals->curtime > m_flForcedInteractionTimeout) &&
...
bool CanNPCsTradePlaces(CAI_BaseNPC *pNPC1, CAI_BaseNPC *pNPC2, bool bDebug) {
  ...
  if (bTest1At2) {
    AI_TraceHull(pNPC2->GetAbsOrigin(), pNPC2->GetAbsOrigin(),
                 pNPC1->GetHullMins(), pNPC1->GetHullMaxs(), MASK_SOLID,
                 &traceFilter, &tr);
    if (tr.startsolid) {
      if (bDebug) {
        NDebugOverlay::Box(pNPC2->GetAbsOrigin(), pNPC1->GetHullMins(),
                           pNPC1->GetHullMaxs(), 255, 0, 0, true, 1.0);
      }
      return false;
    }
  }
  ...
  if (bTest2At1) {
    AI_TraceHull(pNPC1->GetAbsOrigin(), pNPC1->GetAbsOrigin(),
                 pNPC2->GetHullMins(), pNPC2->GetHullMaxs(), MASK_SOLID,
                 &traceFilter, &tr);
    if (tr.startsolid) {
      if (bDebug) {
        NDebugOverlay::Box(pNPC1->GetAbsOrigin(), pNPC2->GetHullMins(),
                           pNPC2->GetHullMaxs(), 255, 0, 0, true, 1.0);
      }
      return false;
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_BaseNPC::InteractionCouldStart(CAI_BaseNPC *pOtherNPC,
                                        ScriptedNPCInteraction_t *pInteraction,
  ...
  MatrixMultiply(matMeToWorld, pInteraction->matDesiredLocalToWorld,
                 matLocalToWorld);
  ...
  if (flDistSqr > pInteraction->flDistSqr) {
    if (bDebug) {
      if (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT ||
          pOtherNPC->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) {
        if (ai_debug_dyninteractions.GetFloat() == 2) {
          Msg("   %s distsqr: %0.2f (%0.2f %0.2f %0.2f), desired: <%0.2f "
              "(%0.2f %0.2f %0.2f)\n",
              GetDebugName(), flDistSqr, pOtherNPC->GetAbsOrigin().x,
              pOtherNPC->GetAbsOrigin().y, pOtherNPC->GetAbsOrigin().z,
              pInteraction->flDistSqr, vecOrigin.x, vecOrigin.y, vecOrigin.z);
        }
      }
    }
    return false;
  ...
  if (bDebug) {
    Msg("DYNINT: (%s) testing interaction \"%s\"\n", GetDebugName(),
        STRING(pInteraction->iszInteractionName));
    Msg("   %s is at: %0.2f %0.2f %0.2f\n", GetDebugName(), GetAbsOrigin().x,
        GetAbsOrigin().y, GetAbsOrigin().z);
    Msg("   %s distsqr: %0.2f (%0.2f %0.2f %0.2f), desired: (%0.2f %0.2f "
        "%0.2f)\n",
        GetDebugName(), flDistSqr, pOtherNPC->GetAbsOrigin().x,
        pOtherNPC->GetAbsOrigin().y, pOtherNPC->GetAbsOrigin().z, vecOrigin.x,
        vecOrigin.y, vecOrigin.z);

    if (pOtherNPC) {
      float flOtherSpeed =
          pOtherNPC->GetSequenceGroundSpeed(pOtherNPC->GetSequence());
      Msg("   %s Speed: %.2f\n",
  ...
  if (pInteraction->iFlags & SCNPC_FLAG_TEST_OTHER_ANGLES) {
    ...
    if (bDebug) {
      Msg("   %s angle matched: (%0.2f %0.2f %0.2f), desired (%0.2f, %0.2f, "
          "%0.2f)\n",
          GetDebugName(), anglemod(angEnemyAngles.x),
          anglemod(angEnemyAngles.y), anglemod(angEnemyAngles.z),
          anglemod(angAngles.x), anglemod(angAngles.y), anglemod(angAngles.z));
    }
  }
  ...
  // This isn't a very good method of checking, but it's cheap and rules out the
  // problems we're seeing so far. If we start getting interactions that start a
  // fair distance apart, we're going to need to do more work here.
  trace_t tr;
  AI_TraceLine(EyePosition(), pOtherNPC->EyePosition(), MASK_NPCSOLID, this,
               COLLISION_GROUP_NONE, &tr);
  if (tr.fraction != 1.0 && tr.m_pEnt != pOtherNPC) {
    if (bDebug) {
      ...
      NDebugOverlay::Line(pOtherNPC->EyePosition(), tr.endpos, 255, 0, 0, true,
                          1.0);
    }
    return false;
  }
  ...
  pOtherNPC->CollisionProp()->NormalizedToWorldSpace(Vector(0, 0, 0.25f),
                                                     &vecOtherKnee);
  AI_TraceLine(vecMyKnee, vecOtherKnee, MASK_NPCSOLID, this,
               COLLISION_GROUP_NONE, &tr);
  if (tr.fraction != 1.0 && tr.m_pEnt != pOtherNPC) {
    if (bDebug) {
      Msg("   %s Interaction was blocked.\n", GetDebugName());
  ...
  CTraceFilterSkipTwoEntities traceFilter(pOtherNPC, this,
                                          COLLISION_GROUP_NONE);
  AI_TraceHull(vecOrigin, vecOrigin, pOtherNPC->GetHullMins(),
               pOtherNPC->GetHullMaxs(), MASK_SOLID, &traceFilter, &tr);
  if (tr.startsolid) {
    if (bDebug) {
      NDebugOverlay::Box(vecOrigin, pOtherNPC->GetHullMins(),
                         pOtherNPC->GetHullMaxs(), 255, 0, 0, true, 1.0);
    }
    return false;
  }

  // If the NPCs are swapping places during this interaction, make sure they can
  // fit at each others' origins before allowing the interaction.
...
void CAI_BaseNPC::InputForceInteractionWithNPC(inputdata_t &inputdata) {
  ...
  if (!pszParam || !pszParam[0]) {
    Warning(
        "%s(%s) received ForceInteractionWithNPC input with bad parameters: "
        "%s\nFormat should be: ForceInteractionWithNPC <target NPC> "
        "<interaction name>\n",
  ...
  if (!pTarget) {
    Warning(
        "%s(%s) received ForceInteractionWithNPC input, but couldn't find "
        "entity named: %s\n",
  ...
  if (!pNPC || !pNPC->GetModelPtr()) {
    Warning(
        "%s(%s) received ForceInteractionWithNPC input, but entity named %s "
        "cannot run dynamic interactions.\n",
        GetClassname(), GetDebugName(), pszParam);
    return;
  }
  ...
  // Find the interaction from the name, and ensure it's one that the target NPC
  // can play
  int iInteraction = -1;
  for (int i = 0; i < m_ScriptedInteractions.Count(); i++) {
    if (Q_strncmp(pszParam,
                  STRING(m_ScriptedInteractions[i].iszInteractionName),
                  strlen(pszParam)))
      continue;

    // Use sequence? or activity?
    if (m_ScriptedInteractions[i].sPhases[SNPCINT_SEQUENCE].iActivity !=
        ACT_INVALID) {
      if (!pNPC->HaveSequenceForActivity((Activity)m_ScriptedInteractions[i]
                                             .sPhases[SNPCINT_SEQUENCE]
                                             .iActivity)) {
        // Other NPC may have all the matching sequences, but just without the
        // activity specified. Lets find a single sequence for us, and ensure
        // they have a matching one.
        int iMySeq = SelectWeightedSequence((Activity)m_ScriptedInteractions[i]
                                                .sPhases[SNPCINT_SEQUENCE]
                                                .iActivity);
        if (pNPC->LookupSequence(GetSequenceName(iMySeq)) == -1) continue;
      }
    } else {
      if (pNPC->LookupSequence(STRING(m_ScriptedInteractions[i]
                                          .sPhases[SNPCINT_SEQUENCE]
                                          .iszSequence)) == -1)
        continue;
    }
  ...
  if (iInteraction == -1) {
    Warning(
        "%s(%s) received ForceInteractionWithNPC input, but couldn't find an "
        "interaction named %s that entity named %s could run.\n",
...
void CAI_BaseNPC::CalculateForcedInteractionPosition(void) {
  ...
  // Pretend I was facing the target, and extrapolate from that the position I
  // should be at
  Vector vecToTarget =
      m_hForcedInteractionPartner->GetAbsOrigin() - GetAbsOrigin();
  ...
  m_vecForcedWorldPosition =
      m_hForcedInteractionPartner->GetAbsOrigin() + vecOrigin;
...
void CAI_BaseNPC::PlayerHasIlluminatedNPC(CBasePlayer *pPlayer, float flDot) {
  ...
  if (IsActiveDynamicInteraction()) {
    ScriptedNPCInteraction_t *pInteraction = GetRunningDynamicInteraction();
    if (pInteraction->iLoopBreakTriggerMethod &
        SNPCINT_LOOPBREAK_ON_FLASHLIGHT_ILLUM) {
...
void CAI_BaseNPC::ModifyOrAppendCriteria(AI_CriteriaSet &set) {
  ...
  if (m_flLastSawPlayerTime) {
    set.AppendCriteria(
        "timesinceseenplayer",
        UTIL_VarArgs("%f", gpGlobals->curtime - m_flLastSawPlayerTime));
  } else {
  ...
  if (GetEnemy()) {
    set.AppendCriteria("distancetoenemy",
                       UTIL_VarArgs("%f", EnemyDistance(GetEnemy())));
  } else {
...
bool CAI_BaseNPC::Stand(void) {
  if (m_bForceCrouch) return false;
