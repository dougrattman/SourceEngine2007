...
DEFINE_FIELD(m_vMark, FIELD_POSITION_VECTOR),
    DEFINE_FIELD(m_flMarkTolerance, FIELD_FLOAT) END_DATADESC()
...
//-----------------------------------------------------------------------------
void CAI_ShotRegulator::SetParameters(int minShotsPerBurst,
                                      int maxShotsPerBurst, float minRestTime,
                                      float maxRestTime) {
...
//-----------------------------------------------------------------------------
void CAI_ShotRegulator::SetBurstShotCountRange(int minShotsPerBurst,
...
//-----------------------------------------------------------------------------
void CAI_ShotRegulator::SetRestInterval(float flMinRestInterval,
...
//-----------------------------------------------------------------------------
void CAI_ShotRegulator::SetBurstInterval(float flMinBurstInterval,
...
void CAI_ShotRegulator::Reset(bool bStartShooting) {
  ...
  m_nBurstShotsRemaining =
      random->RandomInt(m_nMinBurstShots, m_nMaxBurstShots);
  if (bStartShooting) {
    ...
    m_flNextShotTime =
        gpGlobals->curtime +
        random->RandomFloat(m_flMinRestInterval, m_flMaxRestInterval);
...
//-----------------------------------------------------------------------------
float CAI_ShotRegulator::NextShotTime() const { return m_flNextShotTime; }
...
void CAI_ShotRegulator::OnFiredWeapon() {
  ...
  if (m_nBurstShotsRemaining <= 0) {
    ...
    m_bInRestInterval = false;
    m_flNextShotTime +=
        random->RandomFloat(m_flMinBurstInterval, m_flMaxBurstInterval);
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ShotRegulator::EnableShooting(void) { m_bDisabled = false; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ShotRegulator::DisableShooting(void) { m_bDisabled = true; }
...
DEFINE_FIELD(m_velocity, FIELD_FLOAT), DEFINE_FIELD(m_maxVelocity, FIELD_FLOAT),
    DEFINE_FIELD(m_minVelocity, FIELD_FLOAT),
    DEFINE_FIELD(m_invDecay, FIELD_FLOAT),
    DEFINE_FIELD(m_decayTime, FIELD_FLOAT), DEFINE_FIELD(m_accel, FIELD_FLOAT),
    END_DATADESC()

        void CAI_AccelDecay::SetParameters(float minVelocity, float maxVelocity,
                                           float accelPercentPerTick,
                                           float decelPercentPerTick) {
...
float CAI_AccelDecay::Update(float flCurrent, float flTarget,
                             float flInterval) {
  ...
  if (delta > 0.01) {
    ...
    if (delta < m_velocity * deltaSign * m_decayTime) {
      ...
      m_velocity =
          m_velocity * (1.0f - m_accel) + m_maxVelocity * m_accel * deltaSign;
      if (delta < m_velocity * deltaSign * m_decayTime) {
...
DEFINE_KEYFIELD(timeToTrigger, FIELD_FLOAT, "freepass_timetotrigger"),
    ...
    DEFINE_EMBEDDED(m_FreePassMoveMonitor), DEFINE_EMBEDDED(m_Params),
  ...
  if (!pTarget || m_Params.duration < 0.1) return;
...
void CAI_FreePass::Update() {
  ...
  // This works with old data because need to do before base class so as to not
  // choose as enemy
  if (!HasPass()) {
    float timePlayerLastSeen =
        (pTargetInfo) ? pTargetInfo->timeLastSeen : AI_INVALID_TIME;
    float lastTimeDamagedBy = (pTargetInfo)
                                  ? pTargetInfo->timeLastReceivedDamageFrom
                                  : AI_INVALID_TIME;

    if (timePlayerLastSeen == AI_INVALID_TIME ||
        gpGlobals->curtime - timePlayerLastSeen >
            .15)  // If didn't see the player last think
    {
      trace_t tr;
      UTIL_TraceLine(pTarget->EyePosition(), GetOuter()->EyePosition(),
      ...
      if (tr.fraction != 1.0 && tr.m_pEnt != pTarget) {
        ...
        if (dist < m_Params.coverDist) {
          if ((timePlayerLastSeen == AI_INVALID_TIME ||
               gpGlobals->curtime - timePlayerLastSeen >
                   m_Params.timeToTrigger) &&
              (lastTimeDamagedBy == AI_INVALID_TIME ||
               gpGlobals->curtime - lastTimeDamagedBy >
                   m_Params.timeToTrigger)) {
            m_FreePassTimeRemaining = m_Params.duration;
            m_FreePassMoveMonitor.SetMark(pTarget, m_Params.moveTolerance);
          }
    ...
    bool bCanSee = (pSenses && pSenses->ShouldSeeEntity(pTarget) &&
                    pSenses->CanSeeEntity(pTarget));
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CAI_FreePass::HasPass() { return (m_FreePassTimeRemaining > 0); }
...
void CAI_FreePass::Revoke(bool bUpdateMemory) {
  ...
  if (bUpdateMemory && GetPassTarget()) {
    GetOuter()->UpdateEnemyMemory(GetPassTarget(),
                                  GetPassTarget()->GetAbsOrigin());
  }
}
...
bool CAI_FreePass::ShouldAllowFVisible(bool bBaseResult) {
  ...
  if (!bBaseResult || HasPass()) return false;
  ...
  if (m_Params.peekTime > 0.1) {
    ...
    if ((lastTimeSeen == AI_INVALID_TIME ||
         gpGlobals->curtime - lastTimeSeen > m_Params.peekTime) &&
        (lastTimeDamagedBy == AI_INVALID_TIME ||
         gpGlobals->curtime - lastTimeDamagedBy >
             m_Params.peekTimeAfterDamage)) {
      Vector vToTarget;

      VectorSubtract(pTarget->EyePosition(), GetOuter()->EyePosition(),
                     vToTarget);
      vToTarget.z = 0.0f;
      ...
      UTIL_TraceLine(
          GetOuter()->EyePosition(),
          pTarget->EyePosition() + (vecRight * m_Params.peekEyeDist -
                                    Vector(0, 0, m_Params.peekEyeDistZ)),
          MASK_BLOCKLOS, GetOuter(), COLLISION_GROUP_NONE, &tr);
      if (tr.fraction != 1.0 && tr.m_pEnt != pTarget) {
        if (free_pass_peek_debug.GetBool())
          NDebugOverlay::Line(tr.startpos, tr.endpos - Vector(0, 0, 2), 0, 255,
                              0, false, 0.1);
        bIsVisible = false;
      }
      ...
      if (bIsVisible) {
        ...
        if (tr.fraction != 1.0 && tr.m_pEnt != pTarget) {
          if (free_pass_peek_debug.GetBool())
            NDebugOverlay::Line(tr.startpos, tr.endpos - Vector(0, 0, 2), 0,
                                255, 0, false, 0.1);
          bIsVisible = false;
        }
      }
    ...
    if (bIsVisible && free_pass_peek_debug.GetBool())
      NDebugOverlay::Line(GetOuter()->EyePosition(),
                          pTarget->EyePosition() - Vector(0, 0, 2), 255, 0, 0,
                          false, 0.1);
  }
...
//-----------------------------------------------------------------------------
CTraceFilterNav::CTraceFilterNav(CAI_BaseNPC *pProber,
                                 bool bIgnoreTransientEntities,
                                 const IServerEntity *passedict,
                                 int collisionGroup, bool bAllowPlayerAvoid)
...
bool CTraceFilterNav::ShouldHitEntity(IHandleEntity *pHandleEntity,
  ...
  if ((contentsMask & CONTENTS_MOVEABLE) == 0) {
    if (pEntity->ClassMatches("func_pushable")) return false;
  }
#endif
  ...
  // Adrian - If I'm flagged as using the new collision method, then ignore the
  // player when trying  to check if I can get somewhere.
  if (m_bAllowPlayerAvoid && m_pProber->ShouldPlayerAvoid() &&
      pEntity->IsPlayer())
    return false;

  if (pEntity->IsNavIgnored()) return false;
