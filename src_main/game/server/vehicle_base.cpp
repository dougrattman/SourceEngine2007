...
void CPropVehicle::Spawn() {
  CFourWheelServerVehicle *pServerVehicle =
      dynamic_cast<CFourWheelServerVehicle *>(GetServerVehicle());
...
// this allows reloading the script variables from disk over an existing vehicle
// state This is useful for tuning vehicles or updating old saved game formats
CON_COMMAND(vehicle_flushscript, "Flush and reload all vehicle scripts") {
  PhysFlushVehicleScripts();
  for (CBaseEntity *pEnt = gEntList.FirstEnt(); pEnt != NULL;
...
//-----------------------------------------------------------------------------
// Purpose: Tell the vehicle physics system whenever we teleport, so it can
// fixup the wheels.
//-----------------------------------------------------------------------------
void CPropVehicle::Teleport(const Vector *newPosition, const QAngle *newAngles,
...
//-----------------------------------------------------------------------------
void CPropVehicle::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
...
void CPropVehicle::InputSteering(inputdata_t &inputdata) {
  m_VehiclePhysics.SetSteering(inputdata.value.Float(),
                               2 * gpGlobals->frametime);
}
...
void CPropVehicle::Think() {
  ...
  // Derived classes of CPropVehicle have their own code to determine how
  // frequently to think. But the prop_vehicle entity native to this class will
  // only think one time, so this flag was added to allow prop_vehicle to always
  // think without affecting the derived classes.
...
void CPropVehicle::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  ...
  m_vecSmoothedVelocity = m_vecSmoothedVelocity * SMOOTHING_FACTOR +
                          velocity * (1 - SMOOTHING_FACTOR);

  // must be a wheel
  if (!m_VehiclePhysics.VPhysicsUpdate(pPhysics)) return;
...
// Output : const Vector
//-----------------------------------------------------------------------------
Vector CPropVehicle::GetSmoothedVelocity(void) { return m_vecSmoothedVelocity; }

//=============================================================================
#ifdef HL2_EPISODIC

//-----------------------------------------------------------------------------
// Purpose: Add an entity to a list which receives physics callbacks from the
// vehicle
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
CPropVehicleDriveable::~CPropVehicleDriveable(void) { DestroyServerVehicle(); }
...
void CPropVehicleDriveable::Precache(void) {
  ...
  // This step is needed because if we're precaching from a templated instance,
  // we'll miss our vehicle script sounds unless we do the parse below.  This
  // instance of the vehicle will be nuked when we're actually created.
...
void CPropVehicleDriveable::Spawn(void) {
  // Has to be created before Spawn is called (since that causes Precache to be
  // called)
  ...
  if (m_pServerVehicle->Initialize(STRING(m_vehicleScript)) == false) {
    Warning(
        "Vehicle (%s) unable to properly initialize due to script error in "
        "(%s)!\n",
...
void CPropVehicleDriveable::OnRestore(void) {
  ...
  // NOTE: This is necessary to prevent overflow of datatables on level
  // transition since the last exit eyepoint in the last level will have been
  // fixed up based on the level landmarks, resulting in a position that lies
  // outside typical map coordinates. If we're not in the middle of an exit
  // anim, the eye exit endpoint field isn't being used at all.
  if (!m_bExitAnimOn) {
...
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::GetVectors(Vector *pForward, Vector *pRight,
                                       Vector *pUp) const {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
CBaseEntity *CPropVehicleDriveable::GetDriver(void) {
  if (m_hNPCDriver) return m_hNPCDriver;
...
void CPropVehicleDriveable::EnterVehicle(CBaseCombatCharacter *pPassenger) {
  ...
  if (pPlayer != NULL) {
    ...
    // Clear our state
    m_pServerVehicle->InitViewSmoothing(pPlayer->GetAbsOrigin() + vecViewOffset,
                                        pPlayer->EyeAngles());
...
void CPropVehicleDriveable::DriveVehicle(CBasePlayer *pPlayer, CUserCmd *ucmd) {
  ...
  if (pPlayer->IsAlive() == false) return;

  DriveVehicle(TICK_INTERVAL, ucmd, pPlayer->m_afButtonPressed,
               pPlayer->m_afButtonReleased);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::DriveVehicle(float flFrameTime, CUserCmd *ucmd,
                                         int iButtonsDown,
  ...
  m_nScannerDisabledWeapons =
      false;  // off for now, change once we have scanners
...
void CPropVehicleDriveable::Think() {
  ...
  if (m_flTurnOffKeepUpright) {
    ...
    if (m_hKeepUpright != NULL && m_flTurnOffKeepUpright < gpGlobals->curtime) {
      ...
      m_hKeepUpright->AcceptInput("TurnOff", this, this, emptyVariant,
                                  USE_TOGGLE);
      m_flTurnOffKeepUpright = 0;
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
                                      IMoveHelper *pHelper, CMoveData *move) {
  // If the engine's not active, prevent driving
  if (!IsEngineOn() || m_bEngineLocked) return;

  // If the player's entering/exiting the vehicle, prevent movement
  if (m_bEnterAnimOn || m_bExitAnimOn) return;
...
bool CPropVehicleDriveable::CanExitVehicle(CBaseEntity *pEntity) {
  ...
  return (!m_bEnterAnimOn && !m_bExitAnimOn && !m_bLocked &&
          (m_nSpeed <= m_flMinimumSpeedToEnterExit));
}
...
//-----------------------------------------------------------------------------
bool CPropVehicleDriveable::IsEngineOn(void) { return m_VehiclePhysics.IsOn(); }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::StopEngine(void) { m_VehiclePhysics.TurnOff(); }

//-----------------------------------------------------------------------------
// Purpose: // The player takes damage if he hits something going fast enough
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::VPhysicsCollision(int index,
...
//=============================================================================
  ...
  for (int i = 0; i < m_hPhysicsChildren.Count(); i++) {
    if (m_hPhysicsChildren[i] == NULL) continue;
  ...
  CBaseCombatCharacter *pDriver =
      GetDriver() ? GetDriver()->MyCombatCharacterPointer() : NULL;
  if (!pDriver) return;
  ...
  if (pHitEntity->GetOwnerEntity() && (pHitEntity->GetEffects() & EF_NODRAW)) {
    ...
    // this allows strider legs to damage the player on impact but not d0g for
    // example
    if (pDriver->IRelationType(pOwner) == D_LI) return;
  }
  ...
  if (pBreakable) {
    // ROBIN: Do we want to only do this on func_breakables that are about to
    // die?
  ...
  int damageType = 0;
  float flDamage = CalculatePhysicsImpactDamage(
      index, pEvent, gDefaultPlayerVehicleImpactDamageTable, 1.0, true,
      damageType);
  if (flDamage > 0 && m_flNoImpactDamageTime < gpGlobals->curtime) {
    ...
    CTakeDamageInfo info(this, GetDriver(), damageForce, damagePos, flDamage,
                         (damageType | DMG_VEHICLE));
    GetDriver()->TakeDamage(info);
  }
}

int CPropVehicleDriveable::VPhysicsGetObjectList(IPhysicsObject **pList,
                                                 int listMax) {
...
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (info.GetDamageType() & DMG_PHYSGUN) {
    ...
    if (flUprightStrength) {
      ...
      if (m_hKeepUpright) {
        ...
        m_hKeepUpright->AcceptInput("SetAngularLimit", this, this, limitVariant,
                                    USE_TOGGLE);
      } else {
        // If we don't have one, create an upright controller for us
        m_hKeepUpright = CreateKeepUpright(GetAbsOrigin(), vec3_angle, this,
                                           GetUprightStrength(), false);
      }

      Assert(m_hKeepUpright);
      variant_t emptyVariant;
      m_hKeepUpright->AcceptInput("TurnOn", this, this, emptyVariant,
                                  USE_TOGGLE);
    ...
    for (int i = 0; i < m_hPhysicsChildren.Count(); i++) {
      ...
      variant_t emptyVariant;
      m_hPhysicsChildren[i]->AcceptInput("VehiclePunted", info.GetAttacker(),
                                         this, emptyVariant, USE_TOGGLE);
    }
#endif  // HL2_EPISODIC
  }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPropVehicleDriveable::NPC_CanEnterVehicle(CAI_BaseNPC *pPassenger,
                                                bool bCompanion) {
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPropVehicleDriveable::NPC_CanExitVehicle(CAI_BaseNPC *pPassenger,
                                               bool bCompanion) {
  // Always allowed unless a leaf class says otherwise
  return true;
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CPropVehicleDriveable::NPC_AddPassenger(CAI_BaseNPC *pPassenger,
...
//			&info -
//-----------------------------------------------------------------------------
void CPropVehicleDriveable::Event_KilledOther(CBaseEntity *pVictim,
...
//-----------------------------------------------------------------------------
void CFourWheelServerVehicle::InitViewSmoothing(const Vector &vecOrigin,
...
//-----------------------------------------------------------------------------
void CFourWheelServerVehicle::GetVehicleViewPosition(int nRole,
                                                     Vector *pAbsOrigin,
                                                     QAngle *pAbsAngles,
                                                     float *pFOV /*= NULL*/) {
  ...
  if (pDriver && pDriver->IsPlayer()) {
    ...
    SharedVehicleViewSmoothing(
        pPlayerDriver, pAbsOrigin, pAbsAngles, pVehicle->IsEnterAnimOn(),
        pVehicle->IsExitAnimOn(), pVehicle->GetEyeExitEndpoint(),
        &m_ViewSmoothing, pFOV);
  } else {
...
// Purpose:
//-----------------------------------------------------------------------------
const vehicle_operatingparams_t *
CFourWheelServerVehicle::GetVehicleOperatingParams(void) {
...
// Purpose:
//-----------------------------------------------------------------------------
const vehicle_controlparams_t *CFourWheelServerVehicle::GetVehicleControlParams(
    void) {
...
// Purpose:
//-----------------------------------------------------------------------------
CFourWheelVehiclePhysics *CFourWheelServerVehicle::GetFourWheelVehiclePhysics(
    void) {
  CPropVehicleDriveable *pVehicle = GetFourWheelVehicle();
...
void CFourWheelServerVehicle::NPC_SetDriver(CNPC_VehicleDriver *pDriver) {
  if (pDriver) {
    ...
    // Set our owner entity to be the NPC, so it can path check without hitting
    // us
...
void CFourWheelServerVehicle::NPC_DriveVehicle(void) {
  ...
  if (g_debug_vehicledriver.GetInt()) {
    if (m_nNPCButtons) {
      ...
      if (m_nNPCButtons & IN_FORWARD) {
        NDebugOverlay::Line(
            GetFourWheelVehicle()->GetAbsOrigin(),
            GetFourWheelVehicle()->GetAbsOrigin() + vecForward * 200, 0, 255, 0,
            true, 0.1);
      }
      ...
      if (m_nNPCButtons & IN_MOVELEFT) {
        NDebugOverlay::Line(GetFourWheelVehicle()->GetAbsOrigin(),
                            GetFourWheelVehicle()->GetAbsOrigin() -
                                vecRight * 200 * -m_flTurnDegrees,
                            0, 255, 0, true, 0.1);
      }
      ...
      if (m_nNPCButtons & IN_JUMP) {
        NDebugOverlay::Box(GetFourWheelVehicle()->GetAbsOrigin(),
                           -Vector(20, 20, 20), Vector(20, 20, 20), 0, 255, 0,
                           true, 0.1);
      }
    }
  }
  ...
  int afButtonPressed =
      buttonsChanged &
      m_nNPCButtons;  // The changed ones still down are "pressed"
  int afButtonReleased =
      buttonsChanged & (~m_nNPCButtons);  // The ones not down are "released"
  CUserCmd fakeCmd;
  fakeCmd.Reset();
  ...
  GetFourWheelVehicle()->DriveVehicle(gpGlobals->frametime, &fakeCmd,
                                      afButtonPressed, afButtonReleased);
...
//			&vecPos -
//-----------------------------------------------------------------------------
bool CFourWheelServerVehicle::GetWheelContactPoint(int nWheelIndex,
                                                   Vector &vecPos) {
  // Dig through a couple layers to get to our data
  CFourWheelVehiclePhysics *pVehiclePhysics = GetFourWheelVehiclePhysics();
  if (pVehiclePhysics) {
    IPhysicsVehicleController *pVehicleController =
        pVehiclePhysics->GetVehicle();
    if (pVehicleController) {
      return pVehicleController->GetWheelContactPoint(nWheelIndex, &vecPos,
                                                      NULL);
    }
  }
  return false;
