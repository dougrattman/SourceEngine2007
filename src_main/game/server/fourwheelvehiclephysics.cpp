...
// 0 <= t < start :		f(t) = 0
// start <= t <= end :	f(t) = end * spline(( t-start) / (end-start) )  // s
// curve between clamped and linear end < t :			f(t) = t
float RemapAngleRange(float startInterval, float endInterval, float value) {
  ...
  // in spline range?
    ...
    float newAngle = SimpleSpline((absAngle - startInterval) /
...
inline float CFourWheelVehiclePhysics::SetPoseParameter(int iParameter,
                                                        float flValue) {
  Assert(IsFinite(flValue));
  return m_pOuter->SetPoseParameter(iParameter, flValue);
}

inline bool CFourWheelVehiclePhysics::GetAttachment(const char *szName,
                                                    Vector &origin,
                                                    QAngle &angles) {
...
void CFourWheelVehiclePhysics::InitializePoseParameters() {
  m_poseParameters[VEH_FL_WHEEL_HEIGHT] =
      LookupPoseParameter("vehicle_wheel_fl_height");
  m_poseParameters[VEH_FR_WHEEL_HEIGHT] =
      LookupPoseParameter("vehicle_wheel_fr_height");
  m_poseParameters[VEH_RL_WHEEL_HEIGHT] =
      LookupPoseParameter("vehicle_wheel_rl_height");
  m_poseParameters[VEH_RR_WHEEL_HEIGHT] =
      LookupPoseParameter("vehicle_wheel_rr_height");
  m_poseParameters[VEH_FL_WHEEL_SPIN] =
      LookupPoseParameter("vehicle_wheel_fl_spin");
  m_poseParameters[VEH_FR_WHEEL_SPIN] =
      LookupPoseParameter("vehicle_wheel_fr_spin");
  m_poseParameters[VEH_RL_WHEEL_SPIN] =
      LookupPoseParameter("vehicle_wheel_rl_spin");
  m_poseParameters[VEH_RR_WHEEL_SPIN] =
      LookupPoseParameter("vehicle_wheel_rr_spin");
...
//-----------------------------------------------------------------------------
bool CFourWheelVehiclePhysics::ParseVehicleScript(const char *pScriptName,
                                                  solid_t &solid,
                                                  vehicleparams_t &vehicle) {
  // Physics keeps a cache of these to share among spawns of vehicles or flush
  // for debugging
...
void CFourWheelVehiclePhysics::CalcWheelData(vehicleparams_t &vehicle) {
  const char *pWheelAttachments[4] = {"wheel_fl", "wheel_fr", "wheel_rl",
                                      "wheel_rr"};
  Vector left, right;
  QAngle dummy;
  SetPoseParameter(m_poseParameters[VEH_FL_WHEEL_HEIGHT], 0);
  ...
  SetPoseParameter(m_poseParameters[VEH_RR_WHEEL_HEIGHT], 0);
  m_pOuter->InvalidateBoneCache();
  if (GetAttachment("wheel_fl", left, dummy) &&
      GetAttachment("wheel_fr", right, dummy)) {
  ...
  if (GetAttachment("wheel_rl", left, dummy) &&
      GetAttachment("wheel_rr", right, dummy)) {
    VectorITransform(left, m_pOuter->EntityToWorldTransform(), left);
    VectorITransform(right, m_pOuter->EntityToWorldTransform(), right);
    Vector center = (left + right) * 0.5;
  ...
  if (GetAttachment("raytrace_fl", left, dummy) &&
      GetAttachment("raytrace_fr", right, dummy)) {
    VectorITransform(left, m_pOuter->EntityToWorldTransform(), left);
    VectorITransform(right, m_pOuter->EntityToWorldTransform(), right);
    Vector center = (left + right) * 0.5;
  ...
  if (GetAttachment("raytrace_rl", left, dummy) &&
      GetAttachment("raytrace_rr", right, dummy)) {
    VectorITransform(left, m_pOuter->EntityToWorldTransform(), left);
    VectorITransform(right, m_pOuter->EntityToWorldTransform(), right);
    Vector center = (left + right) * 0.5;
...
bool CFourWheelVehiclePhysics::Initialize(const char *pVehicleScript,
  ...
  // NOTE: this needs to be greater than your max framerate (so zero is still
  // instant)
  ...
  IPhysicsObject *pBody =
      m_pOuter->VPhysicsInitNormal(SOLID_VPHYSICS, 0, false, &solid);
  PhysSetGameFlags(pBody,
                   FVPHYSICS_NO_SELF_COLLISIONS | FVPHYSICS_MULTIOBJECT_ENTITY);
  m_pVehicle = physenv->CreateVehicleController(pBody, vehicle, nVehicleType,
                                                physgametrace);
...
// Purpose:
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::SetSteering(float flSteering,
                                           float flSteeringRate) {
  if (!flSteeringRate) {
    m_controls.steering = flSteering;
  } else {
    m_controls.steering =
        Approach(flSteering, m_controls.steering, flSteeringRate);
  }
}
...
void CFourWheelVehiclePhysics::Teleport(matrix3x4_t &relativeTransform) {
  ...
  if (m_pOuterServerVehicle && m_pOuterServerVehicle->GetFourWheelVehicle()) {
    IPhysicsObject *pObj =
        m_pOuterServerVehicle->GetFourWheelVehicle()->VPhysicsGetObject();
    if (pObj) {
      pObj->Wake();
    }
  }
...
void CFourWheelVehiclePhysics::DrawDebugGeometryOverlays() {
  for (int iWheel = 0; iWheel < m_wheelCount; iWheel++) {
    ...
    NDebugOverlay::Sphere(m_wheelPosition[iWheel], m_wheelRotation[iWheel],
                          radius, 255, 255, 0, 0, false, 0);
  }
  ...
  if (pBody) {
    ...
    NDebugOverlay::BoxAngles(vecBodyPosition, Vector(-5, -5, -5),
                             Vector(5, 5, 5), angBodyDirection, 255, 0, 0, 0,
                             0);
    ...
    NDebugOverlay::BoxAngles(vecAxlePositionsHL[0], Vector(-3, -3, -3),
                             Vector(3, 3, 3), angBodyDirection, 0, 255, 0, 0,
                             0);
    NDebugOverlay::BoxAngles(vecAxlePositionsHL[1], Vector(-3, -3, -3),
                             Vector(3, 3, 3), angBodyDirection, 0, 255, 0, 0,
                             0);
    ...
    for (int iWheel = 0; iWheel < 4; ++iWheel) {
      ...
      NDebugOverlay::BoxAngles(vecStart, Vector(-1, -1, -1), Vector(1, 1, 1),
                               angBodyDirection, 0, 255, 0, 0, 0);
      NDebugOverlay::Line(vecStart, vecEnd, 255, 255, 0, true, 0);
      NDebugOverlay::BoxAngles(vecEnd, Vector(-1, -1, -1), Vector(1, 1, 1),
                               angBodyDirection, 255, 0, 0, 0, 0);

      NDebugOverlay::BoxAngles(vecImpact, Vector(-0.5f, -0.5f, -0.5f),
                               Vector(0.5f, 0.5f, 0.5f), angBodyDirection, 0, 0,
                               255, 0, 0);
...
int CFourWheelVehiclePhysics::DrawDebugTextOverlays(int nOffset) {
  ...
  char tempstr[512];
  Q_snprintf(tempstr, sizeof(tempstr), "Speed %.1f  T/S/B (%.0f/%.0f/%.1f)",
             params.speed, m_controls.throttle, m_controls.steering,
             m_controls.brake);
  m_pOuter->EntityText(nOffset, tempstr, 0);
  nOffset++;
  Msg("%s", tempstr);

  Q_snprintf(tempstr, sizeof(tempstr), "Gear: %d, RPM %4d", params.gear,
             (int)params.engineRPM);
  m_pOuter->EntityText(nOffset, tempstr, 0);
  nOffset++;
  Msg(" %s\n", tempstr);
...
bool CFourWheelVehiclePhysics::Think() {
  ...
  m_nHasBoost =
      vehicleData.engine
          .boostDelay;  // if we have any boost delay, vehicle has boost ability
  ...
  if (m_pOuter->VPhysicsGetObject() &&
      !m_pOuter->VPhysicsGetObject()->GetShadowController()) {
    const float skidFactor = 0.15f;
    const float minSpeed = DEFAULT_SKID_THRESHOLD / skidFactor;
    // we have to slide at least 15% of our speed at higher speeds to make the
    // skid sound (otherwise it can be too frequent)
    float skidThreshold =
        m_bLastSkid ? DEFAULT_SKID_THRESHOLD : (carState.speed * 0.15f);
    if (skidThreshold < DEFAULT_SKID_THRESHOLD) {
      // otherwise, ramp in the skid threshold to avoid the sound at really low
      // speeds unless really skidding
      skidThreshold =
          RemapValClamped(fabs(carState.speed), 0, minSpeed,
                          DEFAULT_SKID_THRESHOLD * 8, DEFAULT_SKID_THRESHOLD);
    }
...
#define STEER_DAMPING 0.8
  ...
  float flPhysicsSteer =
      carState.steeringAngle / vehicleData.steering.degreesSlow;
  SetPoseParameter(
      m_poseParameters[VEH_STEER],
      (STEER_DAMPING * flSteer) + ((1 - STEER_DAMPING) * flPhysicsSteer));
...
bool CFourWheelVehiclePhysics::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  ...
  for (int i = 0; i < m_wheelCount; i++) {
    if (pPhysics == m_pWheels[i]) {
      ...
      VectorITransform(m_wheelPosition[i], m_pOuter->EntityToWorldTransform(),
                       tmp);
      SetPoseParameter(m_poseParameters[VEH_FL_WHEEL_HEIGHT + i],
                       (m_wheelBaseHeight[i] - tmp.z) / m_wheelTotalHeight[i]);
      SetPoseParameter(m_poseParameters[VEH_FL_WHEEL_SPIN + i],
                       -m_wheelRotation[i].z);
      return false;
    }
  }
...
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::GetVehicleViewPosition(
    const char *pViewAttachment, float flPitchFactor, Vector *pAbsOrigin,
    QAngle *pAbsAngles) {
  ...
  if (r_VehicleViewDampen.GetInt()) {
    m_pOuterServerVehicle->GetFourWheelVehicle()->DampenEyePosition(
        vehicleEyeOrigin, vehicleEyeAngles);
  }
#endif

  // Compute the relative rotation between the unperterbed eye attachment + the
  // eye angles
  ...
  vehicleEyeAngles.x = RemapAngleRange(PITCH_CURVE_ZERO * flPitchFactor,
                                       PITCH_CURVE_LINEAR, vehicleEyeAngles.x);
  vehicleEyeAngles.z = RemapAngleRange(ROLL_CURVE_ZERO * flPitchFactor,
                                       ROLL_CURVE_LINEAR, vehicleEyeAngles.z);
  AngleMatrix(vehicleEyeAngles, vehicleEyeOrigin, vehicleEyePosToWorld);

  // Now treat the relative eye angles as being relative to this new, perterbed
  // view position...
  matrix3x4_t newCameraToWorld;
  ConcatTransforms(vehicleEyePosToWorld, vehicleCameraToEyePos,
                   newCameraToWorld);

  // output new view abs angles
  MatrixAngles(newCameraToWorld, *pAbsAngles);

  // UNDONE: *pOrigin would already be correct in single player if the
  // HandleView() on the server ran after vphysics
...
// Purpose:
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::SteeringRest(
    float carSpeed, const vehicleparams_t &vehicleData) {
  float flSteeringRate = RemapValClamped(
      carSpeed, vehicleData.steering.speedSlow, vehicleData.steering.speedFast,
      vehicleData.steering.steeringRestRateSlow,
      vehicleData.steering.steeringRestRateFast);
  m_controls.steering =
      Approach(0, m_controls.steering, flSteeringRate * gpGlobals->frametime);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::SteeringTurn(float carSpeed,
                                            const vehicleparams_t &vehicleData,
                                            bool bTurnLeft, bool bBrake,
                                            bool bThrottle) {
  float flTargetSteering = bTurnLeft ? -1.0f : 1.0f;
  // steering speeds are stored in MPH
  float flSteeringRestRate = RemapValClamped(
      carSpeed, vehicleData.steering.speedSlow, vehicleData.steering.speedFast,
      vehicleData.steering.steeringRestRateSlow,
      vehicleData.steering.steeringRestRateFast);
  ...
  if (carSpeedIns > vehicleData.engine.maxSpeed) {
    flSteeringRestRate =
        RemapValClamped(carSpeedIns, vehicleData.engine.maxSpeed,
                        vehicleData.engine.boostMaxSpeed,
                        vehicleData.steering.steeringRestRateFast,
                        vehicleData.steering.steeringRestRateFast * 0.5f);
  }

  const vehicle_operatingparams_t &carState = m_pVehicle->GetOperatingParams();
  bool bIsBoosting = carState.isTorqueBoosting;

  // if you're recovering from a boost and still going faster than max, use the
  // boost steering values
  bool bIsBoostRecover =
      (carState.boostTimeLeft == 100 || carState.boostTimeLeft == 0) ? false
                                                                     : true;
  float boostMinSpeed =
  ...
  float flSteeringRate = RemapValClamped(
      ...
      vehicleData.steering.steeringRateFast);
  ...
  m_controls.steering =
      Approach(flTargetSteering, m_controls.steering, flSteeringRate);
  m_controls.bAnalogSteering = false;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::SteeringTurnAnalog(
    float carSpeed, const vehicleparams_t &vehicleData, float sidemove) {
  // OLD Code
#if 0
	float flSteeringRate = STEERING_BASE_RATE;

	float factor = std::clamp( fabs( sidemove ) / STICK_EXTENTS, 0.0f, 1.0f );
...
#else
  // This is tested with gamepads with analog sticks.  It gives full analog
  // control allowing the player to hold shallow turns.
  float steering = (sidemove / STICK_EXTENTS);

  float flSign = (steering > 0) ? 1.0f : -1.0f;
  float flSteerAdj = RemapValClamped(
      fabs(steering), xbox_steering_deadzone.GetFloat(), 1.0f, 0.0f, 1.0f);
...
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::UpdateDriverControls(CUserCmd *cmd,
                                                    float flFrameTime) {
  ...
  // cmd->sidemove, carState.steeringAngle);
  // If changing direction, use default "return to zero" speed to more quickly
  // transition.
  ...
  // (The inverse is true when driving in reverse and the stick is moved
  // slightly forward)
  //-------------------------------------------------------------------------
  ...
  if (pDriver && pDriver->IsPlayer()) {
    ...
    if (cmd->forwardmove == 0.0f && (fabs(cmd->sidemove) < 200.0f)) {
      // If the stick goes neutral, clear out the bias. When the bias is
      // neutral, it will begin biasing in whichever direction the user next
      // presses the analog stick.
      pPlayerDriver->SetVehicleAnalogControlBias(VEHICLE_ANALOG_BIAS_NONE);
    } else if (cmd->forwardmove > 0.0f) {
      if (pPlayerDriver->GetVehicleAnalogControlBias() ==
          VEHICLE_ANALOG_BIAS_REVERSE) {
        // Player is pushing forward, but the controller is currently biased for
        // reverse driving. Must pass a threshold to be accepted as forward
        // input. Otherwise we just spoof a reduced reverse input to keep the
        // car moving in the direction the player probably expects.
      ...
      } else if (pPlayerDriver->GetVehicleAnalogControlBias() ==
                 VEHICLE_ANALOG_BIAS_NONE) {
  ...
  if (cmd->forwardmove > 0.0f) {
    ...
    flAnalogThrottle = std::clamp(flAnalogThrottle, 0.25f, 1.0f);
    ...
    float flMaxThrottle = std::max(0.1f, m_maxThrottle);
    if (m_controls.steering != 0) {
      float flThrottleReduce = 0;

      // ramp this in, don't just start at the slow speed reduction (helps
      // accelerate from a stop)
      if (carSpeed < vehicleData.steering.speedSlow) {
        flThrottleReduce =
            RemapValClamped(carSpeed, 0, vehicleData.steering.speedSlow, 0,
                            vehicleData.steering.turnThrottleReduceSlow);
      } else {
        flThrottleReduce =
            RemapValClamped(carSpeed, vehicleData.steering.speedSlow,
                            vehicleData.steering.speedFast,
                            vehicleData.steering.turnThrottleReduceSlow,
                            vehicleData.steering.turnThrottleReduceFast);
      }

      float limit = 1.0f - (flThrottleReduce * fabs(m_controls.steering));
      if (limit < 0) limit = 0;
      flMaxThrottle = std::min(flMaxThrottle, limit);
    }

    m_controls.throttle =
        Approach(flMaxThrottle * flAnalogThrottle, m_controls.throttle,
                 flFrameTime * m_throttleRate);

    // Apply the brake.
    if ((flCarSign < 0.0f) && m_controls.bHasBrakePedal) {
      m_controls.brake = Approach(BRAKE_MAX_VALUE, m_controls.brake,
                                  flFrameTime * r_vehicleBrakeRate.GetFloat() *
                                      BRAKE_BACK_FORWARD_SCALAR);
    ...
    flAnalogBrake = std::clamp(flAnalogBrake, 0.25f, 1.0f);
    ...
    float flMaxThrottle = std::min(-0.1f, m_flMaxRevThrottle);
    m_controls.throttle =
        Approach(flMaxThrottle * flAnalogBrake, m_controls.throttle,
                 flFrameTime * m_throttleRate);
  ...
  // digital control
    ...
    m_controls.throttle = Approach(flMaxThrottle, m_controls.throttle,
                                   flFrameTime * m_throttleRate);
  ...
  if (bThrottle &&
      (m_nLastSpeed > m_nSpeed && (m_nLastSpeed - m_nSpeed > 10))) {
    m_bLastThrottle = false;
  }

  // throttle down now but not before??? (or we're braking)
  if (!m_controls.handbrake && !m_controls.brakepedal && bThrottle &&
      !m_bLastThrottle) {
    m_throttleStartTime =
        gpGlobals->curtime;  // need to track how long throttle is down
  ...
  float flSpeedPercentage = std::clamp(m_nSpeed / m_flMaxSpeed, 0.0f, 1.0f);
...
bool CFourWheelVehiclePhysics::IsBoosting(void) {
  ...
  const vehicle_operatingparams_t *pVehicleOperating =
      &m_pVehicle->GetOperatingParams();
  if (pVehicleParams && pVehicleOperating) {
    if ((pVehicleOperating->boostDelay - pVehicleParams->engine.boostDelay) >
        0.0f)
      return true;
  }
...
static int AddPhysToList(IPhysicsObject **pList, int listMax, int count,
...
int CFourWheelVehiclePhysics::VPhysicsGetObjectList(IPhysicsObject **pList,
                                                    int listMax) {
  int count = 0;
  // add the body
