...
AI_CriteriaSet::AI_CriteriaSet(const AI_CriteriaSet &src)
  ...
  m_Lookup.Purge();
  for (short i = src.m_Lookup.FirstInorder(); i != src.m_Lookup.InvalidIndex();
...
//			1.0f -
//-----------------------------------------------------------------------------
void AI_CriteriaSet::AppendCriteria(const char *criteria,
                                    const char *value /*= ""*/,
                                    float weight /*= 1.0f*/) {
...
// Output : int
//-----------------------------------------------------------------------------
int AI_CriteriaSet::GetCount() const { return m_Lookup.Count(); }
...
int AI_CriteriaSet::FindCriterionIndex(const char *name) const {
  ...
  if (idx == m_Lookup.InvalidIndex()) return -1;
...
const char *AI_CriteriaSet::GetName(int index) const {
  ...
  if (index < 0 || index >= (int)m_Lookup.Count()) return "";
...
float AI_CriteriaSet::GetWeight(int index) const {
  if (index < 0 || index >= (int)m_Lookup.Count()) return 1.0f;
...
void AI_CriteriaSet::Describe() {
  for (short i = m_Lookup.FirstInorder(); i != m_Lookup.InvalidIndex();
       i = m_Lookup.NextInorder(i)) {
    CritEntry_t *entry = &m_Lookup[i];

    if (entry->weight != 1.0f) {
      DevMsg("  %20s = '%s' (weight %f)\n", entry->criterianame.String(),
             entry->value ? entry->value : "", entry->weight);
    } else {
      DevMsg("  %20s = '%s'\n", entry->criterianame.String(),
...
BEGIN_SIMPLE_DATADESC(AI_ResponseParams)
  DEFINE_FIELD(flags, FIELD_SHORT), DEFINE_FIELD(odds, FIELD_SHORT),
      DEFINE_FIELD(soundlevel, FIELD_CHARACTER),
      DEFINE_FIELD(
          delay, FIELD_INTEGER),  // These are compressed down to two float16s,
                                  // so treat as an INT for saverestore
      DEFINE_FIELD(respeakdelay, FIELD_INTEGER),  //
END_DATADESC()

BEGIN_SIMPLE_DATADESC(AI_Response)
  DEFINE_FIELD(m_Type, FIELD_CHARACTER),
      DEFINE_ARRAY(m_szResponseName, FIELD_CHARACTER,
                   AI_Response::MAX_RESPONSE_NAME),
      DEFINE_ARRAY(m_szMatchingRule, FIELD_CHARACTER,
                   AI_Response::MAX_RULE_NAME),
      // DEFINE_FIELD( m_pCriteria, FIELD_??? ), // Don't need to save this
      // probably
...
//			*criteria -
//-----------------------------------------------------------------------------
void AI_Response::Init(ResponseType_t type, const char *responseName,
                       const AI_CriteriaSet &criteria,
                       const AI_ResponseParams &responseparams,
                       const char *ruleName, const char *applyContext,
  ...
  Q_strncpy(m_szMatchingRule, ruleName ? ruleName : "NULL",
            sizeof(m_szMatchingRule));
...
void AI_Response::Describe() {
  ...
  if (m_szContext) {
    DevMsg("Contexts to set '%s' on %s, ", m_szContext,
           m_bApplyContextToWorld ? "world" : "speaker");
  }

  DevMsg("response %s = '%s'\n", DescribeResponse((ResponseType_t)m_Type),
         m_szResponseName);
}
...
//-----------------------------------------------------------------------------
const AI_CriteriaSet *AI_Response::GetCriteria() { return m_pCriteria; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void AI_Response::Release() { delete this; }
...
// Output : static bool
//-----------------------------------------------------------------------------
const char *SplitContext(const char *raw, char *key, int keylen, char *value,
                         int valuelen, float *duration) {
  char *colon1 = Q_strstr(raw, ":");
  if (!colon1) {
    DevMsg(
        "SplitContext:  warning, ignoring context '%s', missing colon "
        "separator!\n",
        raw);
    *key = *value = 0;
    return NULL;
  ...
  int len = colon1 - raw;
  Q_strncpy(key, raw, std::min(len + 1, keylen));
  key[std::min(len, keylen - 1)] = 0;
  ...
  if (colon2 && (colon2 < end)) {
    if (duration) *duration = atof(colon2 + 1);

    len = std::min(colon2 - (colon1 + 1), valuelen - 1);
    Q_strncpy(value, colon1 + 1, len + 1);
    value[len] = 0;
  } else {
    if (duration) *duration = 0.0;

    len = std::min(end - (colon1 + 1), valuelen - 1);
    Q_strncpy(value, colon1 + 1, len + 1);
    value[len] = 0;
  }
