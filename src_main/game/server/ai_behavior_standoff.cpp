...
static AI_StandoffParams_t AI_DEFAULT_STANDOFF_PARAMS = {
    AIHCR_MOVE_ON_COVER, true, true, 1.5, 2.5, 1, 3, 25, 0};

#define MAKE_ACTMAP_KEY(posture, activity) \
  ((((unsigned)(posture)) << 16) | ((unsigned)(activity)))
...
class CAI_BattleLine : public CBaseEntity {
 ...
 public:
  ...
  bool Affects(CAI_BaseNPC *pNpc) {
    ...
    if (pNpc->NameMatches(pszNamedActor) || pNpc->ClassMatches(pszNamedActor) ||
        (pNpc->GetSquad() &&
         stricmp(pNpc->GetSquad()->GetName(), pszNamedActor) == 0)) {
      return true;
    }
    return false;
  }
...
BEGIN_DATADESC(CAI_StandoffBehavior)
  DEFINE_FIELD(m_fActive, FIELD_BOOLEAN),
      ...
      DEFINE_FIELD(m_posture, FIELD_INTEGER), DEFINE_EMBEDDED(m_params),
      ...
      // 											m_UpdateBattleLinesSemaphore
      // (not saved, only an in-think item) m_BattleLines
      // (not saved, rebuilt)
...
void CAI_StandoffBehavior::SetParameters(const AI_StandoffParams_t &params,
  ...
  if (GetOuter() && GetOuter()->GetShotRegulator()) {
    GetOuter()->GetShotRegulator()->SetBurstShotCountRange(m_params.minShots,
                                                           m_params.maxShots);
    GetOuter()->GetShotRegulator()->SetRestInterval(m_params.minTimeShots,
                                                    m_params.maxTimeShots);
  }
}
...
bool CAI_StandoffBehavior::CanSelectSchedule() {
  if (!m_bHasLowCoverActivity) m_fActive = false;

  if (!m_fActive) return false;

  return (GetNpcState() == NPC_STATE_COMBAT &&
          GetOuter()->GetActiveWeapon() != NULL);
}
...
void CAI_StandoffBehavior::PrescheduleThink() {
  VPROF_BUDGET("CAI_StandoffBehavior::PrescheduleThink",
  ...
  if (DrawBattleLines.GetInt() != 0) {
    ...
    while ((pEntity = gEntList.FindEntityByClassname(
      ...
      if (pLine->m_fActive) {
        ...
        NDebugOverlay::Line(pLine->GetAbsOrigin() - Vector(0, 0, 64),
                            pLine->GetAbsOrigin() + Vector(0, 0, 64), 0, 255, 0,
                            false, 0.1);
      }
    }
  }
...
void CAI_StandoffBehavior::GatherConditions() {
  ...
  if (GetEnemy()) {
    ...
    if (pEnemyInfo && m_params.flAbandonTimeLimit > 0 &&
        ((pEnemyInfo->timeAtFirstHand != AI_INVALID_TIME &&
          gpGlobals->curtime - pEnemyInfo->timeLastSeen >
              m_params.flAbandonTimeLimit) ||
         (pEnemyInfo->timeAtFirstHand == AI_INVALID_TIME &&
          gpGlobals->curtime - pEnemyInfo->timeFirstSeen >
              m_params.flAbandonTimeLimit * 2))) {
      SetCondition(COND_ABANDON_TIME_EXPIRED);

      bAbandonStandoff = true;

      if (pSquad) {
        for (CAI_BaseNPC *pSquadMate = pSquad->GetFirstMember(&iter);
             pSquadMate; pSquadMate = pSquad->GetNextMember(&iter)) {
  ...
  if (bAbandonStandoff) {
    if (pSquad) {
      for (CAI_BaseNPC *pSquadMate = pSquad->GetFirstMember(&iter); pSquadMate;
    ...
    if (DrawBattleLines.GetInt() != 0) {
      if (IsBehindBattleLines(GetAbsOrigin())) {
        NDebugOverlay::Box(GetOuter()->GetAbsOrigin(), -Vector(48, 48, 4),
                           Vector(48, 48, 4), 255, 0, 0, 8, 0.1);
      } else {
        NDebugOverlay::Box(GetOuter()->GetAbsOrigin(), -Vector(48, 48, 4),
                           Vector(48, 48, 4), 0, 255, 0, 8, 0.1);
      }
    }
  }
}
...
int CAI_StandoffBehavior::SelectScheduleCheckCover(void) {
  ...
  if (GetOuter()->GetShotRegulator()->IsInRestInterval()) {
    ...
    if (random->RandomInt(0, 99) < 80) SetReuseCurrentCover();
    return SCHED_TAKE_COVER_FROM_ENEMY;
  }
  ...
  if (GetPosture() == AIP_PEEKING || GetPosture() == AIP_STANDING) {
    if (!HasCondition(COND_CAN_RANGE_ATTACK1) &&
      ...
      if (GetOuter()->GetActiveWeapon() &&
          (GetOuter()->GetActiveWeapon()->CapabilitiesGet() &
...
Activity CAI_MappedActivityBehavior_Temporary::GetMappedActivity(
    AI_Posture_t posture, Activity activity) {
  if (posture != AIP_STANDING) {
    unsigned short iActivityTranslation =
        m_ActivityMap.Find(MAKE_ACTMAP_KEY(posture, activity));
...
Vector CAI_StandoffBehavior::GetStandoffGoalPosition() {
  if (m_vecStandoffGoalPosition != GOAL_POSITION_INVALID) {
    ...
    for (;;) {
      pBattleLine = (CAI_BattleLine *)gEntList.FindEntityByClassname(
          pBattleLine, "ai_battle_line");

      if (!pBattleLine) break;
...
void CAI_StandoffBehavior::UpdateBattleLines() {
  if (m_UpdateBattleLinesSemaphore.EnterThink()) {
    // @TODO (toml 06-19-03): This is the quick to code thing. Could use some
    // optimization/caching to not recalc everything (up to) each think
    m_BattleLines.RemoveAll();

    bool bHaveGoalPosition =
        (m_vecStandoffGoalPosition != GOAL_POSITION_INVALID);
    ...
    if (bHaveGoalPosition) {
      ...
      if (GetDirectionOfStandoff(&goalLine.normal)) {
        goalLine.point =
            GetStandoffGoalPosition() + goalLine.normal * DIST_GOAL_PLANE;
      ...
      if (m_params.fPlayerIsBattleline) {
        ...
        if (GetDirectionOfStandoff(&playerLine.normal)) {
          playerLine.point =
              pPlayer->GetAbsOrigin() + playerLine.normal * DIST_PLAYER_PLANE;
    ...
    for (;;) {
      ...
      if (pBattleLine->m_fActive &&
          (pBattleLine->m_fStrict || !bHaveGoalPosition) &&
          pBattleLine->Affects(GetOuter())) {
...
bool CAI_StandoffBehavior::IsBehindBattleLines(const Vector &point) {
  ...
  for (int i = 0; i < m_BattleLines.Count(); i++) {
    ...
    if (DotProduct(m_BattleLines[i].normal, vecToPoint) > 0) {
      if (DrawBattleLines.GetInt() != 0) {
        NDebugOverlay::Box(point, -Vector(48, 48, 4), Vector(48, 48, 4), 0, 255,
                           0, 8, 1);
        NDebugOverlay::Line(point, GetOuter()->GetAbsOrigin(), 0, 255, 0, true,
                            1);
      }
      return false;
    }
  ...
  if (DrawBattleLines.GetInt() != 0) {
    NDebugOverlay::Box(point, -Vector(48, 48, 4), Vector(48, 48, 4), 255, 0, 0,
                       8, 1);
...
bool CAI_StandoffBehavior::IsValidCover(const Vector &vecCoverLocation,
  ...
  if (!BaseClass::IsValidCover(vecCoverLocation, pHint)) return false;

  if (IsCurSchedule(SCHED_TAKE_COVER_FROM_BEST_SOUND)) return true;
...
bool CAI_StandoffBehavior::IsValidShootPosition(const Vector &vLocation,
                                                CAI_Node *pNode,
                                                const CAI_Hint *pHint) {
  if (!BaseClass::IsValidShootPosition(vLocation, pNode, pHint)) return false;
...
void CAI_StandoffBehavior::StartTask(const Task_t *pTask) {
  ...
  switch (pTask->iTask) {
    ...
    case TASK_FIND_COVER_FROM_ENEMY: {
      ...
      if (!m_params.fStayAtCover && (!m_TimeForceCoverHint.Expired() ||
                                     m_RandomCoverChangeTimer.Expired())) {
        m_TimeForceCoverHint.Force();
        m_RandomCoverChangeTimer.Set(8, 16, false);

        // @TODO (toml 03-24-03):  clean this up be tool-izing base tasks. Right
        // now, this is here to force to not use lateral cover search
        ...
        if (pLeader && m_vecStandoffGoalPosition == GOAL_POSITION_INVALID) {
          if (random->RandomInt(1, 100) <= 50) {
            // Half the time, if the player is leading, try to find a spot near
            // them
        ...
        if (bTryGoalPosFirst) {
          ...
          pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos, 0,
                                          15 * 12, &coverPos);

          if (coverPos == vec3_origin)
            pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos,
                                            15 * 12 - 0.1, 40 * 12, &coverPos);
        ...
        if (coverPos == vec3_origin) {
          ...
          if (!GetTacticalServices()->FindCoverPos(enemyPos, enemyEyePos, 0,
                                                   coverRadius, &coverPos)) {
            // Try local lateral cover
            if (!GetTacticalServices()->FindLateralCover(enemyEyePos, 0,
                                                         &coverPos)) {
              // At this point, try again ignoring front lines. Any cover
              // probably better than hanging out in the open
              m_fIgnoreFronts = true;
              if (!GetTacticalServices()->FindCoverPos(
                      enemyPos, enemyEyePos, 0, coverRadius, &coverPos)) {
                if (!GetTacticalServices()->FindLateralCover(enemyEyePos, 0,
                                                             &coverPos)) {
    ...
    default: { fCallBase = true; }
  }

  if (fCallBase) BaseClass::StartTask(pTask);
}

//-------------------------------------

void CAI_StandoffBehavior::OnChangeHintGroup(string_t oldGroup,
                                             string_t newGroup) {
  OnChangeTacticalConstraints();
}
...
void CAI_StandoffBehavior::OnChangeTacticalConstraints() {
  ...
  if (m_params.hintChangeReaction == AIHCR_MOVE_IMMEDIATE) m_fTakeCover = true;
}
...
CBaseEntity *CAI_StandoffBehavior::GetPlayerLeader() {
  CBaseEntity *pPlayer = AI_GetSinglePlayer();
  if (pPlayer && GetOuter()->IRelationType(pPlayer) == D_LI) return pPlayer;
  return NULL;
}
...
Hint_e CAI_StandoffBehavior::GetHintType() {
  ...
  if (pHintNode) return pHintNode->HintType();
  return HINT_NONE;
}
...
void CAI_StandoffBehavior::UnlockHintNode() {
  ...
  if (pHintNode) {
    ...
    if (pNode && pNode->IsLocked()) pNode->Unlock();
    ClearHintNode();
  }
}
...
void CAI_MappedActivityBehavior_Temporary::UpdateTranslateActivityMap() {
  AI_ActivityMapping_t
      mappings[] =  // This array cannot be static, as some activity values are
                    // set on a per-map-load basis
      {
          {
              AIP_CROUCHING,
              ACT_IDLE,
    ...
    if (!mappings[i].pszWeapon ||
      ...
      if (HaveSequenceForActivity(mappings[i].translation) ||
          HaveSequenceForActivity(
              GetOuter()->Weapon_TranslateActivity(mappings[i].translation))) {
        Assert(m_ActivityMap.Find(MAKE_ACTMAP_KEY(mappings[i].posture,
                                                  mappings[i].activity)) ==
               m_ActivityMap.InvalidIndex());
        m_ActivityMap.Insert(
            MAKE_ACTMAP_KEY(mappings[i].posture, mappings[i].activity),
            mappings[i].translation);
      }
    }
  }
}
...
void CAI_StandoffBehavior::UpdateTranslateActivityMap() {
  ...
  if (lowCoverActivity == ACT_INVALID) lowCoverActivity = ACT_COVER_LOW;

  m_bHasLowCoverActivity =
      ((CapabilitiesGet() & bits_CAP_DUCK) &&
       (GetOuter()->TranslateActivity(lowCoverActivity) != ACT_INVALID));

  CBaseCombatWeapon *pWeapon = GetOuter()->GetActiveWeapon();
  if (pWeapon &&
      (GetOuter()->TranslateActivity(lowCoverActivity) == ACT_INVALID))
    DevMsg(
        "Note: NPC class %s lacks ACT_COVER_LOW, therefore cannot participate "
        "in standoff\n",
...
void CAI_MappedActivityBehavior_Temporary::OnChangeActiveWeapon(
...
void CAI_StandoffBehavior::OnRestore() { UpdateTranslateActivityMap(); }
...
AI_StandoffParams_t g_StandoffParamsByAgression[] = {
    //	hintChangeReaction,		fCoverOnReload, PlayerBtlLn,
    // minTimeShots,
    // maxTimeShots, 	minShots, 	maxShots, 	oddsCover
    // flAbandonTimeLimit
    {AIHCR_MOVE_ON_COVER, true, true, 4.0, 8.0, 2, 4, 50, false,
     30},  // AGGR_VERY_LOW
    {AIHCR_MOVE_ON_COVER, true, true, 2.0, 5.0, 3, 5, 25, false,
     20},  // AGGR_LOW
    {AIHCR_MOVE_ON_COVER, true, true, 0.6, 2.5, 3, 6, 25, false,
     10},  // AGGR_MEDIUM
    {AIHCR_MOVE_ON_COVER, true, true, 0.2, 1.5, 5, 8, 10, false,
     10},  // AGGR_HIGH
    {AIHCR_MOVE_ON_COVER, false, true, 0, 0, 100, 100, 0, false,
     5},  // AGGR_VERY_HIGH
};
...
class CAI_StandoffGoal : public CAI_GoalEntity {
 ...
 public:
  ...
  void DisableGoal(CAI_BaseNPC *pAI) {
    // @TODO (toml 04-07-03): remove the no damage spawn flag once stable. The
  ...
  void InputSetAggressiveness(inputdata_t &inputdata) {
    ...
    for (int i = 0; i < actors.Count(); i++) {
      ...
      CAI_StandoffBehavior *pBehavior;
      if (!pAI->GetBehavior(&pBehavior)) continue;
      SetBehaviorParams(pBehavior);
    }
  }
  ...
  void SetBehaviorParams(CAI_StandoffBehavior *pBehavior) {
    ...
    if (!m_bAbandonIfEnemyHides) params.flAbandonTimeLimit = 0.0;
...
BEGIN_DATADESC(CAI_StandoffGoal)
  DEFINE_KEYFIELD(m_aggressiveness, FIELD_INTEGER, "Aggressiveness"),
      //								   m_customParams
      //(individually)
      DEFINE_KEYFIELD(m_HintChangeReaction, FIELD_INTEGER,
      ...
      DEFINE_KEYFIELD(m_bAbandonIfEnemyHides, FIELD_BOOLEAN,
                      "AbandonIfEnemyHides"),
      DEFINE_KEYFIELD(m_customParams.fCoverOnReload, FIELD_BOOLEAN,
                      "CustomCoverOnReload"),
      DEFINE_KEYFIELD(m_customParams.minTimeShots, FIELD_FLOAT,
                      "CustomMinTimeShots"),
      DEFINE_KEYFIELD(m_customParams.maxTimeShots, FIELD_FLOAT,
                      "CustomMaxTimeShots"),
      ...
      DEFINE_KEYFIELD(m_customParams.oddsCover, FIELD_INTEGER,
                      "CustomOddsCover"),

      // Inputs
      DEFINE_INPUTFUNC(FIELD_INTEGER, "SetAggressiveness",
