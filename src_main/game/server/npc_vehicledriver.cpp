...
BEGIN_DATADESC(CNPC_VehicleDriver)
  DEFINE_KEYFIELD(m_iszVehicleName, FIELD_STRING, "vehicle"),
      ...
      // Inputs
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetDriversMaxSpeed",
                       InputSetDriversMaxSpeed),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetDriversMinSpeed",
...
void CNPC_VehicleDriver::Activate(void) {
  ...
  m_hVehicleEntity =
      (gEntList.FindEntityByName(NULL, STRING(m_iszVehicleName)));
  if (!m_hVehicleEntity) {
...
int CNPC_VehicleDriver::SelectSchedule(void) {
  // Vehicle driver hangs in the air inside the vehicle, so we never need to
  // fall to ground
  ...
  switch (m_NPCState) {
    ...
    case NPC_STATE_COMBAT: {
      ...
      // We can see him, but can't shoot him. Just wait and hope he comes
      // closer.
...
int CNPC_VehicleDriver::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  if (flDist < flMinRange) return COND_TOO_CLOSE_TO_ATTACK;
  if (flDist > flMaxRange) return COND_TOO_FAR_TO_ATTACK;
  ...
  if (flForwardDot < 0 && fabs(flDot) < 0.5) return COND_NOT_FACING_ATTACK;
  ...
  switch (pTask->iTask) {
    ...
    case TASK_VEHICLEDRIVER_GET_PATH: {
      ...
      if (g_debug_vehicledriver.GetInt() & DRIVER_DEBUG_PATH) {
        NDebugOverlay::Box(GetGoalEnt()->GetAbsOrigin(), -Vector(50, 50, 50),
                           Vector(50, 50, 50), 255, 255, 255, true, 5);
      }
...
void CNPC_VehicleDriver::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_RANGE_ATTACK1: {
      ...
      if (pEnemy) {
        // TODO: Get a bodytarget from the firing point of the gun in the
        // vehicle
...
bool CNPC_VehicleDriver::OverrideMove(float flInterval) {
  if (!m_hVehicleEntity) return true;
  ...
  if (GetNavigator()->IsGoalActive()) {
    if (OverridePathMove(flInterval)) return true;
  }
...
bool CNPC_VehicleDriver::WaypointReached(void) {
    ...
    if (pEntity) {
      if (pEntity->m_flSpeed > 0) {
        if (pEntity->m_flSpeed <= 1.0) {
          ...
          Warning(
              "path_track %s tried to tell the npc_vehicledriver to set speed "
              "to %.3f. npc_vehicledriver only accepts values between 0 and "
              "1.\n",
  ...
  m_Waypoints[1] = new CVehicleWaypoint(m_vecPrevPoint, m_vecDesiredPosition,
                                        m_vecPostPoint, m_vecPostPostPoint);
  m_pNextWaypoint = m_Waypoints[1];

  // Drop the spline marker back
  m_flDistanceAlongSpline = std::max(0.0f, m_flDistanceAlongSpline - 1.0f);
...
bool CNPC_VehicleDriver::OverridePathMove(float flInterval) {
  ...
  if (!m_pCurrentWaypoint) {
    ...
    m_Waypoints[0] = new CVehicleWaypoint(m_vecPrevPrevPoint, m_vecPrevPoint,
                                          m_vecDesiredPosition, m_vecPostPoint);
    m_Waypoints[1] = new CVehicleWaypoint(m_vecPrevPoint, m_vecDesiredPosition,
                                          m_vecPostPoint, m_vecPostPostPoint);
  ...
  // Have we reached our target? See if we've passed the current waypoint's
  // plane.
  Vector vecAbsMins, vecAbsMaxs;
  CollisionProp()->WorldSpaceAABB(&vecAbsMins, &vecAbsMaxs);
  if (BoxOnPlaneSide(vecAbsMins, vecAbsMaxs,
                     &m_pCurrentWaypoint->planeWaypoint) == 3) {
    if (WaypointReached()) return true;
  }

  // Did we bypass it and reach the next one already?
  if (m_pNextWaypoint && BoxOnPlaneSide(vecAbsMins, vecAbsMaxs,
                                        &m_pNextWaypoint->planeWaypoint) == 3) {
    if (WaypointReached()) return true;
  }
  ...
  // Get our current speed, and check it against the length of the spline to
  // know how far to advance our marker
  ...
  float flIncrement =
      flIncTime * (flSpeed / pCurrentSplineBeingTraversed->GetLength());

  // Now advance our point along the spline
  m_flDistanceAlongSpline =
      std::clamp(m_flDistanceAlongSpline + flIncrement, 0.0f, 2.0f);
  if (m_flDistanceAlongSpline > 1) {
    // We crossed the spline boundary
    pCurrentSplineBeingTraversed = m_pNextWaypoint;
  }

  Vector vSplinePoint = pCurrentSplineBeingTraversed->GetPointAt(
      m_flDistanceAlongSpline > 1 ? m_flDistanceAlongSpline - 1
                                  : m_flDistanceAlongSpline);
  Vector vSplineTangent = pCurrentSplineBeingTraversed->GetTangentAt(
      m_flDistanceAlongSpline > 1 ? m_flDistanceAlongSpline - 1
                                  : m_flDistanceAlongSpline);

  // Now that we've got the target spline point & tangent, use it to decide what
  // our desired velocity is. If we're close to the tangent, just use the
  // tangent. Otherwise, Lerp towards it.
  ...
  if (flDistToDesired > (flTangentLength * 0.75)) {
    ...
    VectorLerp(vSplineTangent, vecToDesired * flTangentLength,
               (flDistToDesired / (flTangentLength * 0.5)),
  ...
  // Bunch o'debug
  if (g_debug_vehicledriver.GetInt() & DRIVER_DEBUG_PATH) {
    NDebugOverlay::Box(m_vecPrevPrevPoint, -Vector(15, 15, 15),
                       Vector(15, 15, 15), 192, 0, 0, true, 0.1);
    NDebugOverlay::Box(m_vecPrevPoint, -Vector(20, 20, 20), Vector(20, 20, 20),
                       255, 0, 0, true, 0.1);
    NDebugOverlay::Box(m_vecPostPoint, -Vector(20, 20, 20), Vector(20, 20, 20),
                       0, 192, 0, true, 0.1);
    NDebugOverlay::Box(m_vecPostPostPoint, -Vector(20, 20, 20),
                       Vector(20, 20, 20), 0, 128, 0, true, 0.1);
    NDebugOverlay::Box(vSplinePoint, -Vector(10, 10, 10), Vector(10, 10, 10), 0,
                       0, 255, true, 0.1);
    NDebugOverlay::Line(vSplinePoint, vSplinePoint + (vSplineTangent * 40), 0,
                        0, 255, true, 0.1);

    // NDebugOverlay::HorzArrow( pCurrentSplineBeingTraversed->splinePoints[0],
    // pCurrentSplineBeingTraversed->splinePoints[1], 30, 255,255,255,0, false,
    // 0.1f ); NDebugOverlay::HorzArrow(
    // pCurrentSplineBeingTraversed->splinePoints[1],
    // pCurrentSplineBeingTraversed->splinePoints[2], 20, 255,255,255,0, false,
    // 0.1f ); NDebugOverlay::HorzArrow(
    // pCurrentSplineBeingTraversed->splinePoints[2],
    // pCurrentSplineBeingTraversed->splinePoints[3], 10, 255,255,255,0, false,
    // 0.1f );

    // Draw the plane we're checking against for waypoint passing
    Vector vecPlaneRight;
    CrossProduct(m_pCurrentWaypoint->planeWaypoint.normal, Vector(0, 0, 1),
                 vecPlaneRight);
    Vector vecPlane = m_pCurrentWaypoint->splinePoints[2];
    NDebugOverlay::Line(vecPlane + (vecPlaneRight * -100),
                        vecPlane + (vecPlaneRight * 100), 255, 0, 0, true, 0.1);

    // Draw the next plane too
    CrossProduct(m_pNextWaypoint->planeWaypoint.normal, Vector(0, 0, 1),
                 vecPlaneRight);
    vecPlane = m_pNextWaypoint->splinePoints[2];
    NDebugOverlay::Line(vecPlane + (vecPlaneRight * -100),
                        vecPlane + (vecPlaneRight * 100), 192, 0, 0, true, 0.1);
  }
  ...
  if (g_debug_vehicledriver.GetInt() & DRIVER_DEBUG_PATH_SPLINE) {
    for (int i = 0; i < 10; i++) {
      ...
      NDebugOverlay::Box(vecTarget, -Vector(10, 10, 10), Vector(10, 10, 10),
                         255, 0, 0, true, 0.1);
      NDebugOverlay::Line(vecTarget, vecTarget + (vecTangent * 10), 255, 255, 0,
                          true, 0.1);
    }
  }
...
void CNPC_VehicleDriver::DriveVehicle(void) {
  ...
  if (g_debug_vehicledriver.GetInt() & DRIVER_DEBUG_PATH) {
    NDebugOverlay::Box(m_vecDesiredPosition, -Vector(20, 20, 20),
                       Vector(20, 20, 20), 0, 255, 0, true, 0.1);
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + m_vecDesiredVelocity,
                        0, 255, 0, true, 0.1);
  }
...
void CNPC_VehicleDriver::CheckForTeleport(void) {
  ...
  if (pTrack->HasSpawnFlags(SF_PATH_TELEPORT)) {
    ...
    if (pTrack->GetNext()) {
      vecToTarget =
          (pTrack->GetNext()->GetAbsOrigin() - pTrack->GetAbsOrigin());
      VectorNormalize(vecToTarget);
...
void CNPC_VehicleDriver::InputStartFiring(inputdata_t &inputdata) {
  ...
  if (m_pVehicleInterface->NPC_HasPrimaryWeapon()) {
    ...
    if (m_flDistTooFar < flMaxRange ||
        GetSenses()->GetDistLook() < flMaxRange) {
