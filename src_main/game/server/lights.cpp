...
void CLight::Spawn(void) {
  ...
  if (m_iStyle >= 32) {
    ...
    if (FBitSet(m_spawnflags.operator const int &(), SF_LIGHT_START_OFF))
...
void CLight::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  if (m_iStyle >= 32) {
    if (!ShouldToggle(useType, !FBitSet(m_spawnflags.operator const int &(),
                                        SF_LIGHT_START_OFF)))
      return;
...
void CLight::TurnOn(void) {
  ...
  CLEARBITS(m_spawnflags.operator const int &(), SF_LIGHT_START_OFF);
}
...
void CLight::TurnOff(void) {
  engine->LightStyle(m_iStyle, "a");
  SETBITS(m_spawnflags.operator const int &(), SF_LIGHT_START_OFF);
}
