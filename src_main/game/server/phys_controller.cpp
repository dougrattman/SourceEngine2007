...
void CConstantForceController::SetConstantForce(const Vector &linear,
                                                const AngularImpulse &angular) {
  m_linear = linear;
  m_angular = angular;
...
IMotionEvent::simresult_e CConstantForceController::Simulate(
...
the_interface CPhysForce : public CPointEntity {
 public:
  ...
  virtual void SetupForces(IPhysicsObject * pPhys, Vector & linear,
                           AngularImpulse & angular) = 0;
...
BEGIN_DATADESC(CPhysForce)
  ...
  DEFINE_PHYSPTR(m_pController),
      ...
      // DEFINE_FIELD( m_wasRestored, FIELD_BOOLEAN ), // NOTE: DO NOT save/load
      // this - it's used to detect loads
...
//			Calculate the force (and optional torque) that the engine
//would create
//-----------------------------------------------------------------------------
...
void CPhysThruster::OnActivate(void) {
  if (m_attachedObject != NULL) {
    ...
    ConcatTransforms(worldToAttached, EntityToWorldTransform(),
                     thrusterToAttached);
...
void CalculateVelocityOffsetLocal(IPhysicsObject *pPhys,
                                  const Vector &forceLocal,
                                  const Vector &positionLocal,
                                  Vector &outVelLocal,
                                  AngularImpulse &outAngular) {
...
void CPhysThruster::SetupForces(IPhysicsObject *pPhys, Vector &linear,
                                AngularImpulse &angular) {
  Vector thrustVector;
  ...
  if (m_spawnflags & SF_THRUST_LOCAL_ORIENTATION) {
    CalculateVelocityOffsetLocal(pPhys, thrustVector, m_localOrigin, linear,
                                 angular);
  } else {
    Vector position;
    VectorTransform(m_localOrigin, m_attachedObject->EntityToWorldTransform(),
                    position);
...
void CPhysTorque::SetupForces(IPhysicsObject *pPhys, Vector &linear,
                              AngularImpulse &angular) {
  // clear out force
  linear.Init();
...
BEGIN_SIMPLE_DATADESC(CMotorController)
  ...
  DEFINE_FIELD(m_speed, FIELD_FLOAT), DEFINE_FIELD(m_maxTorque, FIELD_FLOAT),
      DEFINE_KEYFIELD(m_axis, FIELD_VECTOR, "axis"),
...
IMotionEvent::simresult_e CMotorController::Simulate(
    IPhysicsMotionController *pController, IPhysicsObject *pObject,
    float deltaTime, Vector &linear, AngularImpulse &angular) {
  linear = vec3_origin;
  angular = vec3_origin;

  if (m_speed == 0) return SIM_NOTHING;
  ...
  if (fabsf(m_lastAcceleration) > 0) {
    ...
    if (deltaSpeed * accel > 0) {
      ...
      factor = 1 - std::clamp(factor, 0.0f, 1.0f);
...
#define SF_MOTOR_HINGE \
  0x0004  // motor also acts as a hinge constraining the object to this axis
// NOTE: THIS DOESN'T WORK YET
#define SF_MOTOR_LOCAL \
  0x0008  // Maintain local relationship with the attached object
...
BEGIN_DATADESC(CPhysMotor)
  ...
  DEFINE_KEYFIELD(m_nameAttach, FIELD_STRING, "attach1"),
      ...
      DEFINE_FIELD(m_lastTime, FIELD_TIME), DEFINE_PHYSPTR(m_pHinge),
      DEFINE_PHYSPTR(m_pController),
...
void CPhysMotor::InputSetTargetSpeed(inputdata_t &inputdata) {
  if (m_flSpeed == inputdata.value.Float()) return;
...
void CPhysMotor::Spawn(void) {
  ...
  // double check that the axis is at least a unit long. If not, warn and
  // self-destruct.
  if (axisLength > 1.0f) {
    UTIL_SnapDirectionToAxis(m_motor.m_axis);
  } else {
    Warning(
        "phys_motor %s does not have a valid axis helper, and "
        "self-destructed!\n",
        GetDebugName());
...
void CPhysMotor::TurnOn(void) {
  ...
  if (pPhys) {
    m_pController->WakeObjects();
    // If the current speed is zero, the objects can run a tick without getting
    // torque'd and go back to sleep so force a think now and have some
    // acceleration happen before the controller gets called.
...
void CPhysMotor::Activate(void) {
  ...
  if (m_attachedObject == NULL) {
    ...
    if (pAttach && pAttach->GetMoveType() == MOVETYPE_VPHYSICS) {
      ...
      m_motor.m_maxTorque = inertia * m_motor.m_inertiaFactor *
                            (m_angularAcceleration + m_additionalAcceleration);
  ...
  if (m_attachedObject) {
    ...
    if (m_spawnflags & SF_MOTOR_HINGE) {
      ...
      if (!m_pHinge) {
        ...
        m_pHinge = physenv->CreateHingeConstraint(g_PhysWorldObject, pPhys,
                                                  NULL, hingeParams);
    ...
    // NOTE: On restore, this path isn't run because m_pController will not be
    // NULL
    if (!m_pController) {
...
void CPhysMotor::Think(void) {
  // angular acceleration is always positive - it should be treated as a
  // magnitude - the controller will apply it in the proper direction
  Assert(m_angularAcceleration >= 0);

  m_motor.m_speed =
      UTIL_Approach(m_flSpeed, m_motor.m_speed,
                    m_angularAcceleration * (gpGlobals->curtime - m_lastTime));
  m_lastTime = gpGlobals->curtime;
...
class CKeepUpright : public CPointEntity, public IMotionEvent {
 ...
 public:
  ...
  // Inputs
  void InputTurnOn(inputdata_t &inputdata) { m_bActive = true; }
  void InputTurnOff(inputdata_t &inputdata) { m_bActive = false; }
 ...
 private:
  friend CBaseEntity *CreateKeepUpright(const Vector &vecOrigin,
                                        const QAngle &vecAngles,
                                        CBaseEntity *pOwner,
                                        float flAngularLimit, bool bActive);
...
void CKeepUpright::Activate() {
  ...
  if (!m_pController) {
    ...
    // HACKHACK: Due to changes in the vehicle simulator the keepupright
    // controller used in coast_01 is unstable force it to have perfect damping
    // to compensate. detect it using the hack of angular limit == 150, attached
    // to a vehicle Fixing it in the code is the simplest course of action
    // presently
#ifdef HL2_DLL
...
//-----------------------------------------------------------------------------
// Purpose: Use this to spawn a keepupright controller via code instead of
// map-placed
//-----------------------------------------------------------------------------
CBaseEntity *CreateKeepUpright(const Vector &vecOrigin, const QAngle &vecAngles,
                               CBaseEntity *pOwner, float flAngularLimit,
                               bool bActive) {
  CKeepUpright *pKeepUpright = (CKeepUpright *)CBaseEntity::Create(
      "phys_keepupright", vecOrigin, vecAngles, pOwner);
  if (pKeepUpright) {
...
IMotionEvent::simresult_e CKeepUpright::Simulate(
    IPhysicsMotionController *pController, IPhysicsObject *pObject,
    float deltaTime, Vector &linear, AngularImpulse &angular) {
  if (!m_bActive) return SIM_NOTHING;
  ...
  if (m_bDampAllRotation) {
    angular =
        ComputeRotSpeedToAlignAxes(m_localTestAxis, currentLocalTargetAxis,
                                   angVel, 0, invDeltaTime, m_angularLimit);
    angular -= angVel;
  ...
  angular =
      ComputeRotSpeedToAlignAxes(m_localTestAxis, currentLocalTargetAxis,
                                 angVel, 1.0, invDeltaTime, m_angularLimit);
  angular *= invDeltaTime;
