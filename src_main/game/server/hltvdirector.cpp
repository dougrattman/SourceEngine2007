...
static ConVar tv_delay("tv_delay", "30", 0,
                       "SourceTV broadcast delay in seconds", true, 0, true,
                       HLTV_MAX_DELAY);
static ConVar tv_allow_static_shots(
    "tv_allow_static_shots", "1", 0,
    "Auto director uses fixed level cameras for shots");
static ConVar tv_allow_camera_man(
    "tv_allow_camera_man", "1", 0,
...
static float WeightedAngle(Vector vec1, Vector vec2) {
  ...
  return a * a;  // vectors are facing opposite direction
    ...
    !defined(TF_DLL)  // add your mod here if you use your own director

static CHLTVDirector s_HLTVDirector;  // singleton

EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CHLTVDirector, IHLTVDirector,
...
void CHLTVDirector::FireGameEvent(IGameEvent *event) {
  if (!m_pHLTVServer) return;  // don't do anything

  CGameEvent gameevent;

  gameevent.m_Event = gameeventmanager->DuplicateEvent(event);
  gameevent.m_Priority = event->GetInt(
      "priority", -1);  // priorities are leveled between 0..10, -1 means ignore
  gameevent.m_Tick = gpGlobals->tickcount;

  m_EventHistory.Insert(gameevent);
}

IHLTVServer *CHLTVDirector::GetHLTVServer(void) { return m_pHLTVServer; }
...
bool CHLTVDirector::IsActive(void) { return (m_pHLTVServer != NULL); }
...
void CHLTVDirector::UpdateSettings() {
  ...
  m_nBroadcastTick = std::max(0, newBroadcastTick);
}
...
const char **CHLTVDirector::GetModEvents() {
  static const char *s_modevents[] = {
      ...
      "player_chat", "round_start", "round_end",      NULL};
...
void CHLTVDirector::BuildCameraList(void) {
  ...
  CBaseEntity *pCamera =
      gEntList.FindEntityByClassname(NULL, GetFixedCameraEntityName());

  while (pCamera && m_nNumFixedCameras < MAX_NUM_CAMERAS) {
    CBaseEntity *pTarget =
        gEntList.FindEntityByName(NULL, STRING(pCamera->m_target));
    ...
    m_nNumFixedCameras++;
    pCamera =
        gEntList.FindEntityByClassname(pCamera, GetFixedCameraEntityName());
  }
}
...
void CHLTVDirector::StartDelayMessage() {
  if (m_nNextShotTick > gpGlobals->tickcount) return;
...
void CHLTVDirector::StartBestPlayerCameraShot() {
  ...
  if (iBestCamera != -1) {
    ...
    StartChaseCameraShot(iBestCamera, iBestTarget, 112.0f, 20,
                         (RandomFloat() > 0.5) ? 20 : -20, false);
  } else {
    StartBestFixedCameraShot(true);
  }
}
...
void CHLTVDirector::StartFixedCameraShot(int iCamera, int iTarget) {
  ...
  m_iPVSEntity =
      0;  // don't use camera entity, since it may not been transmitted
...
void CHLTVDirector::StartChaseCameraShot(int iTarget1, int iTarget2,
                                         int distance, int phi, int theta,
                                         bool bInEye) {
...
void CHLTVDirector::StartRandomShot() {
  ...
  m_nNextShotTick = std::min(m_nNextShotTick, toTick);
...
void CHLTVDirector::CreateShotFromEvent(CGameEvent *event) {
  ...
  if (bPlayerHurt || bPlayerKilled) {
    ...
    CBaseEntity *attacker =
        UTIL_PlayerByUserId(event->m_Event->GetInt("attacker"));
    ...
    } else  // attacker != NULL
    {
      ...
      bool bInEye = (bPlayerKilled && RandomFloat(0, 1) > 0.33) ||
                    (bPlayerHurt && RandomFloat(0, 1) > 0.66);
      ...
      // view over shoulder, randomly left or right
      StartChaseCameraShot(victim->entindex(), attacker->entindex(), 96, -20,
                           (RandomFloat() > 0.5) ? 30 : -30, bInEye);
    }

    // shot 2 seconds after death/hurt
    m_nNextShotTick =
        std::min(m_nNextShotTick, (event->m_Tick + TIME_TO_TICKS(2.0)));
...
int CHLTVDirector::FindFirstEvent(int tick) {
  ...
  while (event->m_Tick < tick) {
    ...
    if (index == m_EventHistory.InvalidIndex()) break;
...
bool CHLTVDirector::SetCameraMan(int iPlayerIndex) {
  if (!tv_allow_camera_man.GetBool()) return false;
  ...
  if (iPlayerIndex > 0) {
    ...
    if (!pPlayer || pPlayer->GetTeamNumber() != TEAM_SPECTATOR) return false;
  }
  ...
  if (iPlayerIndex > 0) {
    ...
    UTIL_ClientPrintFilter(filter, HUD_PRINTTALK,
  ...
  while (index != m_EventHistory.InvalidIndex()) {
    ...
    if (Q_strcmp(dc.m_Event->GetName(), "hltv_cameraman") == 0) {
      if (dc.m_Event->GetInt("index") > 0) {
        ...
        m_nNextShotTick =
            m_nBroadcastTick + 1;  // check setting right on next frame
...
void CHLTVDirector::StartNewShot() {
  ...
  int smallestTick =
      std::max(0, gpGlobals->tickcount - TIME_TO_TICKS(HLTV_MAX_DELAY));
...
void CHLTVDirector::AnalyzeCameras() {
  ...
  for (int i = 0; i < m_nNumFixedCameras; i++) {
    ...
    for (int j = 0; j < m_nNumActivePlayers; j++) {
      ...
      // check visibility
      trace_t tr;
      UTIL_TraceLine(vCamPos, pPlayer->GetAbsOrigin(), MASK_SOLID, pPlayer,
...
void CHLTVDirector::BuildActivePlayerList() {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer->GetTeamNumber() <= TEAM_SPECTATOR) continue;
...
void CHLTVDirector::AnalyzePlayers() {
  ...
  for (int i = 0; i < m_nNumActivePlayers; i++) {
    ...
    for (int j = 0; j < m_nNumActivePlayers; j++) {
      if (iPlayerIndex == j) continue;  // don't check against itself
      ...
      // check visibility
      trace_t tr;
      UTIL_TraceLine(vCamPos, pOtherPlayer->GetAbsOrigin(), MASK_SOLID,
                     pOtherPlayer, COLLISION_GROUP_NONE, &tr);
    ...
    if (nCount > 0) {
      float flDistribution =
          VectorLength(vDistribution) / nCount;  // normalize distribution
