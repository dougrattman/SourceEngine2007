...
// Just add more items to the bottom of this array and they will automagically
// be supported This is done instead of just a classname in the FGD so we can
// control which entities can be spawned, and still remain fairly flexible
...
BEGIN_DATADESC(CBreakable)
  ...
  DEFINE_FIELD(m_Material, FIELD_INTEGER),
      ...
      // Function Pointers
      DEFINE_ENTITYFUNC(BreakTouch), DEFINE_THINKFUNC(Die),
...
bool CBreakable::KeyValue(const char *szKeyName, const char *szValue) {
  // UNDONE_WC: explicitly ignoring these fields, but they shouldn't be in the
  // map file!
...
void CBreakable::Spawn(void) {
  ...
  if (!IsBreakable() && m_nRenderMode != kRenderNormal) AddFlag(FL_WORLDBRUSH);
...
void CBreakable::ParsePropData(void) {
  ...
  if (!Q_strncmp(STRING(m_iszPropData), "None", 4)) return;
...
void CBreakable::MaterialSoundRandom(int entindex, Materials soundMaterial,
                                     float volume) {
  ...
  CPASAttenuationFilter filter(CBaseEntity::Instance(entindex),
                               params.soundlevel);
...
void CBreakable::Precache(void) {
  ...
  switch (m_Material) {
    ...
    default:
      Warning(
          "%s (%s) at (%.3f %.3f %.3f) using obsolete or unknown material "
          "type.\n",
          GetClassname(), GetDebugName(), GetAbsOrigin().x, GetAbsOrigin().y,
          GetAbsOrigin().z);
      pGibName = "WoodChunks";
      break;
  }
...
void CBreakable::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                             trace_t *ptr) {
...
void CBreakable::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  if (HasSpawnFlags(SF_BREAK_PHYSICS_BREAK_IMMEDIATELY)) {
    ...
    CTakeDamageInfo dmgInfo(pHitEntity, pHitEntity, damageForce, damagePos,
                            (m_iHealth + 1), DMG_CRUSH);
...
int CBreakable::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  // damage from them
  if (m_takedamage == DAMAGE_NO || info.GetDamage() < m_iMinHealthDmg) return 0;
  ...
  if (!IsBreakable()) return 0;

  float flPropDamage =
      GetBreakableDamage(subInfo, assert_cast<IBreakableWithPropData *>(this));
  ...
  if (!UpdateHealth(iNewHealth, info.GetAttacker())) return 1;

  // Make a shard noise each time func breakable is hit, if it's capable of
  // taking damage
...
//-----------------------------------------------------------------------------
// Purpose: Breaks the breakable. m_hBreaker is the entity that caused us to
// break.
//-----------------------------------------------------------------------------
void CBreakable::Die(void) {
  ...
  int iCount =
      (vSize[0] * vSize[1] + vSize[1] * vSize[2] + vSize[2] * vSize[0]) /
      (3 * 12 * 12);
  ...
  if (m_iszModelName != NULL_STRING) {
    for (int i = 0; i < iCount; i++) {
      ...
      if (strstr(modelName, ".mdl") != NULL) {
        ...
        iModelIndex = modelinfo->GetModelIndex(
      ...
      te->BreakModel(filter2, 0.0, vecSpot,
                     pCollisionProp->GetCollisionAngles(), vSize, vecVelocity,
  ...
  if (m_iszSpawnObject != NULL_STRING) {
    CBaseEntity::Create(STRING(m_iszSpawnObject), vecSpot,
                        pCollisionProp->GetCollisionAngles(), this);
  }

  if (Explodable()) {
    ExplosionCreate(vecSpot, pCollisionProp->GetCollisionAngles(), this,
                    GetExplosiveDamage(), GetExplosiveRadius(), true);
  }
}
...
char const *CBreakable::DamageDecal(int bitsDamageType, int gameMaterial) {
  if (m_Material == matGlass) return "GlassBreak";

  if (m_Material == matUnbreakableGlass) return "BulletProof";
...
void CBreakable::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                 PhysGunPickup_t reason) {
  m_hPhysicsAttacker = pPhysGunUser;
  m_flLastPhysicsInfluenceTime = gpGlobals->curtime;
}

void CBreakable::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
void CPushable::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                    USE_TYPE useType, float value) {
#ifdef HL1_DLL
  if (m_spawnflags & SF_PUSH_NO_USE) return;
...
int CPushable::OnTakeDamage(const CTakeDamageInfo &info) {
  if (m_spawnflags & SF_PUSH_BREAKABLE) return BaseClass::OnTakeDamage(info);
