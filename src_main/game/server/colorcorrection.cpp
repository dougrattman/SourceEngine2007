...
BEGIN_DATADESC(CColorCorrection)
  ...
  DEFINE_THINKFUNC(FadeInThink), DEFINE_THINKFUNC(FadeOutThink),
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetFadeInDuration",
                       InputSetFadeInDuration),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetFadeOutDuration",
...
SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_NOSCALE, 0.0f, HIGH_DEFAULT,
               SendProxy_Origin),
...
void CColorCorrection::Spawn(void) {
  ...
  SetContextThink(&CColorCorrection::FadeInThink, TICK_NEVER_THINK,
                  s_pFadeInContextThink);
  SetContextThink(&CColorCorrection::FadeOutThink, TICK_NEVER_THINK,
                  s_pFadeOutContextThink);
...
void CColorCorrection::FadeIn(void) {
  ...
  SetNextThink(gpGlobals->curtime + COLOR_CORRECTION_ENT_THINK_RATE,
               s_pFadeInContextThink);
}
...
void CColorCorrection::FadeInThink(void) {
  ...
  if (m_flStartFadeInWeight > 0.0f) {
    ...
    flWeightRatio = std::clamp(flWeightRatio, 0.0f, 0.99f);
  ...
  flFadeRatio = std::clamp(flFadeRatio, 0.0f, 1.0f);
  m_flStartFadeInWeight = std::clamp(m_flStartFadeInWeight, 0.0f, 1.0f);
...
void CColorCorrection::FadeOutThink(void) {
  ...
  if (m_flStartFadeOutWeight < m_flMaxWeight) {
    ...
    flWeightRatio = std::clamp(flWeightRatio, 0.01f, 1.0f);
    flTimeToFade = m_flFadeOutDuration * flWeightRatio;
  }

  Assert(flTimeToFade > 0.0f);
  float flFadeRatio =
      (gpGlobals->curtime - m_flTimeStartFadeOut) / flTimeToFade;
  flFadeRatio = std::clamp(flFadeRatio, 0.0f, 1.0f);
  m_flStartFadeOutWeight = std::clamp(m_flStartFadeOutWeight, 0.0f, 1.0f);
