...
// identifiers passed to functions that can operate on either list, to indicate
...
BEGIN_SIMPLE_DATADESC(CSound)
  ...
  DEFINE_FIELD(m_hOwner, FIELD_EHANDLE), DEFINE_FIELD(m_iVolume, FIELD_INTEGER),
...
bool CSoundEnt::InitSoundEnt() {
  ...
  g_pSoundEnt = (CSoundEnt *)CBaseEntity::Create("soundent", vec3_origin,
                                                 vec3_angle, GetWorldEntity());
  if (!g_pSoundEnt) {
...
void CSoundEnt::Think(void) {
  ...
  while (iSound != SOUNDLIST_EMPTY) {
    if ((m_SoundPool[iSound].m_flExpireTime <= gpGlobals->curtime &&
         (!m_SoundPool[iSound].m_bNoExpirationTime)) ||
        !m_SoundPool[iSound].ValidateOwner()) {
      ...
      if (displaysoundlist.GetInt() == 2 &&
          m_SoundPool[iSound].IsSoundType(SOUND_DANGER)) {
      ...
      if (displaysoundlist.GetBool()) {
        ...
        if (displaysoundlist.GetInt() == 1 ||
            (displaysoundlist.GetInt() == 2 &&
             pSound->IsSoundType(SOUND_DANGER))) {
          NDebugOverlay::Line(
              pSound->GetSoundOrigin(),
              pSound->GetSoundOrigin() + forward * pSound->Volume(), r, g, b,
              false, 0.1);
          NDebugOverlay::Line(
              pSound->GetSoundOrigin(),
              pSound->GetSoundOrigin() - forward * pSound->Volume(), r, g, b,
          ...
          NDebugOverlay::Line(
              pSound->GetSoundOrigin(),
              pSound->GetSoundOrigin() + right * pSound->Volume(), r, g, b,
              false, 0.1);
          NDebugOverlay::Line(
              pSound->GetSoundOrigin(),
              pSound->GetSoundOrigin() - right * pSound->Volume(), r, g, b,
          ...
          NDebugOverlay::Line(pSound->GetSoundOrigin(),
                              pSound->GetSoundOrigin() + up * pSound->Volume(),
                              r, g, b, false, 0.1);
          NDebugOverlay::Line(pSound->GetSoundOrigin(),
                              pSound->GetSoundOrigin() - up * pSound->Volume(),
                              r, g, b, false, 0.1);
          ...
          if (pSound->m_flOcclusionScale != 1.0) {
            ...
            NDebugOverlay::Line(
                pSound->GetSoundOrigin(),
                pSound->GetSoundOrigin() + forward * pSound->OccludedVolume(),
                r, g, b, false, 0.1);
            NDebugOverlay::Line(
                pSound->GetSoundOrigin(),
                pSound->GetSoundOrigin() - forward * pSound->OccludedVolume(),
            ...
            NDebugOverlay::Line(
                pSound->GetSoundOrigin(),
                pSound->GetSoundOrigin() + right * pSound->OccludedVolume(), r,
                g, b, false, 0.1);
            NDebugOverlay::Line(
                pSound->GetSoundOrigin(),
                pSound->GetSoundOrigin() - right * pSound->OccludedVolume(), r,
            ...
            NDebugOverlay::Line(
                pSound->GetSoundOrigin(),
                pSound->GetSoundOrigin() + up * pSound->OccludedVolume(), r, g,
                b, false, 0.1);
            NDebugOverlay::Line(
                pSound->GetSoundOrigin(),
                pSound->GetSoundOrigin() - up * pSound->OccludedVolume(), r, g,
...
void CSoundEnt::FreeSound(int iSound, int iPrevious) {
  ...
  if (iPrevious != SOUNDLIST_EMPTY) {
    ...
    g_pSoundEnt->m_SoundPool[iPrevious].m_iNext =
        g_pSoundEnt->m_SoundPool[iSound].m_iNext;
  } else {
...
int CSoundEnt::IAllocSound(void) {
  ...
  m_iFreeSound = m_SoundPool[m_iFreeSound]
                     .m_iNext;  // move the index down into the free list.

  m_SoundPool[iNewSound].m_iNext =
      m_iActiveSound;  // point the new sound at the top of the active list.

  m_iActiveSound = iNewSound;  // now make the new sound the top of the active
                               // list. You're done.
...
// sound info.
//=========================================================
void CSoundEnt::InsertSound(int iType, const Vector &vecOrigin, int iVolume,
                            float flDuration, CBaseEntity *pOwner,
                            int soundChannelIndex, CBaseEntity *pSoundTarget) {
  int iThisSound;

  if (!g_pSoundEnt) return;
  ...
  // Keep track of whether this sound had an owner when it was made. If the
  // sound has a long duration, the owner could disappear by the time someone
  // hears this sound, so we have to look at this boolean and throw out sounds
  // who have a NULL owner but this field set to true. (sjb) 12/2/2005
  if (pOwner) {
...
int CSoundEnt::FindOrAllocateSound(CBaseEntity *pOwner, int soundChannelIndex) {
  ...
  while (iSound != SOUNDLIST_EMPTY) {
    ...
    if (sound.m_ownerChannelIndex == soundChannelIndex &&
        sound.m_hOwner == pOwner) {
      return iSound;
    }
...
void CSoundEnt::Initialize(void) {
  ...
  if (gpGlobals->maxClients > 1)
    nTotalSoundsInPool =
        std::min((int)MAX_WORLD_SOUNDS_MP, gpGlobals->maxClients + 32);
...
//-----------------------------------------------------------------------------
CSound *CSoundEnt::GetLoudestSoundOfType(int iType,
                                         const Vector &vecEarPosition) {
  ...
  float flBestDist =
      MAX_COORD_RANGE *
      MAX_COORD_RANGE;  // so first nearby sound will become best so far.
  float flDist;
  CSound *pSound;
...
void CAISound::InputInsertSound(inputdata_t &inputdata) {
  ...
  if (m_iszProxyEntityName != NULL_STRING) {
    ...
    if (pProxy) {
      ...
      DevWarning(
          "Warning- ai_sound cannot find proxy entity named '%s'. Using "
          "self.\n",
          STRING(m_iszProxyEntityName));
    }
  }

  g_pSoundEnt->InsertSound(m_iSoundType, vecLocation, iVolume, m_flDuration,
                           this);
}
...
void CAISound::InputEmitAISound(inputdata_t &inputdata) {
  ...
  g_pSoundEnt->InsertSound(m_iSoundType | m_iSoundContext, vecLocation,
                           m_iVolume, m_flDuration, this);
}
