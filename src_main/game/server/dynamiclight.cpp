...
BEGIN_DATADESC(CDynamicLight)
  ...
  DEFINE_FIELD(m_ActualFlags, FIELD_CHARACTER),
      DEFINE_FIELD(m_Flags, FIELD_CHARACTER), DEFINE_FIELD(m_On, FIELD_BOOLEAN),
...
void CDynamicLight::Spawn(void) {
  ...
  int clampedExponent =
      std::clamp(static_cast<int>(m_Exponent), MIN_DL_EXPONENT_VALUE,
                 MAX_DL_EXPONENT_VALUE);
  if (m_Exponent != clampedExponent) {
    Warning(
        "light_dynamic at [%d %d %d] has invalid exponent value (%d must be "
        "between %d and %d).\n",
        (int)GetAbsOrigin().x, (int)GetAbsOrigin().x, (int)GetAbsOrigin().x,
        m_Exponent.Get(), MIN_DL_EXPONENT_VALUE, MAX_DL_EXPONENT_VALUE);
