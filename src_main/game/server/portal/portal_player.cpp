...
// Ragdoll Entity
//
class CPortalRagdoll : public CBaseAnimatingOverlay,
...
SendPropExclude("DT_BaseAnimating", "m_flPlaybackRate"),
    ...
    // portal_playeranimstate and clientside animation takes care of these on
    // the client
    ...
    END_DATADESC()
        ...
        ConVar sv_regeneration_wait_time("sv_regeneration_wait_time", "1.0",
                                         FCVAR_REPLICATED);
...
void CPortal_Player::CreateSounds() {
  if (!m_pWooshSound) {
    ...
    m_pWooshSound =
        controller.SoundCreate(filter, entindex(), "PortalPlayer.Woosh");
...
void CPortal_Player::GiveAllItems(void) {
  ...
  CWeaponPortalgun *pPortalGun =
      static_cast<CWeaponPortalgun *>(GiveNamedItem("weapon_portalgun"));
...
void CPortal_Player::NotifySystemEvent(
...
bool CPortal_Player::Weapon_Switch(CBaseCombatWeapon *pWeapon,
                                   int viewmodelindex) {
...
void CPortal_Player::UpdateExpression(void) {
  ...
  if (m_iszExpressionScene != NULL_STRING &&
      stricmp(STRING(m_iszExpressionScene), szScene) == 0)
    return;
  ...
  float flDuration = InstancedScriptedScene(
      this, szScene, &m_hExpressionSceneEnt, 0.0, true, NULL);
...
void CPortal_Player::PostThink(void) {
  ...
  if (IsAlive() && GetHealth() < GetMaxHealth()) {
    ...
    if (gpGlobals->curtime >
        m_fTimeLastHurt + sv_regeneration_wait_time.GetFloat()) {
      ...
      m_bIsRegenerating = false;
      UTIL_ScreenFade(this, hurtScreenOverlay, 1.0f, 0.1f,
                      FFADE_IN | FFADE_PURGE);
    }
  }
  ...
  if (IsAlive() && m_flExpressionLoopTime >= 0 &&
      gpGlobals->curtime > m_flExpressionLoopTime) {
  ...
  if (m_bStuckOnPortalCollisionObject) {
    Vector vForward =
        ((CProp_Portal *)m_hPortalEnvironment.Get())->m_vPrevForward;
    Vector vNewPos =
        GetAbsOrigin() + vForward * gpGlobals->frametime * -1000.0f;
...
void CPortal_Player::UpdateWooshSounds(void) {
  if (m_pWooshSound) {
    ...
    if (fWooshVolume > 1.0f) fWooshVolume = 1.0f;
...
bool CPortal_Player::WantsLagCompensationOnEntity(
...
void CPortal_Player::SetupBones(matrix3x4_t *pBoneToWorld, int boneMask) {
  ...
  if (pParent) {
    ...
    if (pParentCache) {
      BuildMatricesWithBoneMerge(
          pStudioHdr, m_PlayerAnimState->GetRenderAngles(), adjOrigin, pos, q,
  ...
  Studio_BuildMatrices(pStudioHdr, m_PlayerAnimState->GetRenderAngles(),
                       adjOrigin, pos, q, -1, pBoneToWorld, boneMask);
}
...
bool CPortal_Player::BumpWeapon(CBaseCombatWeapon *pWeapon) {
  ...
  CWeaponPortalgun *pPickupPortalgun =
      dynamic_cast<CWeaponPortalgun *>(pWeapon);
  ...
  if (bOwnsWeaponAlready == true) {
    ...
    if (pPickupPortalgun) {
      CWeaponPortalgun *pPortalGun = static_cast<CWeaponPortalgun *>(
...
const Vector &CPortal_Player::WorldSpaceCenter() const {
  ...
  m_vWorldSpaceCenterHolder.z +=
      ((IsDucked()) ? (VEC_DUCK_HULL_MAX.z) : (VEC_HULL_MAX.z)) * 0.5f;
  return m_vWorldSpaceCenterHolder;
}

void CPortal_Player::Teleport(const Vector *newPosition,
...
void CPortal_Player::VPhysicsShadowUpdate(IPhysicsObject *pPhysics) {
  ...
  // below is mostly a cut/paste of existing CBasePlayer::VPhysicsShadowUpdate
  // code with some minor tweaks to avoid getting stuck in stuff when in a portal
  // environment
  ...
  if (dist >= maxDistErrorSqr || deltaV >= maxVelErrorSqr ||
    ...
    if (m_touchedPhysObject || pPhysGround) {
      ...
      Ray_t ray;
      ray.Init(GetAbsOrigin(), GetAbsOrigin(), WorldAlignMins(),
               WorldAlignMaxs());

      CTraceFilterSimple OriginalTraceFilter(this,
                                             COLLISION_GROUP_PLAYER_MOVEMENT);
      CTraceFilterTranslateClones traceFilter(&OriginalTraceFilter);
      UTIL_Portal_TraceRay_With(m_hPortalEnvironment, ray, MASK_PLAYERSOLID,
                                &traceFilter, &trace);
      ...
      if (trace.allsolid || trace.startsolid) {
        ...
        if (trace.startsolid == false) {
          ...
          if (!FindClosestPassableSpace(this, newPosition - GetAbsOrigin(),
                                        MASK_PLAYERSOLID)) {
            // Try moving the player closer to the center of the portal
            CProp_Portal *pPortal = m_hPortalEnvironment.Get();
            newPosition +=
                (pPortal->GetAbsOrigin() - WorldSpaceCenter()) * 0.1f;
            SetAbsOrigin(newPosition);

            DevMsg("Hurting the player for FindClosestPassableSpaceFailure!");

            // Deal 1 damage per frame... this will kill a player very fast, but
            // allow for the above correction to fix some cases
            CTakeDamageInfo info(this, this, vec3_origin, vec3_origin, 1,
                                 DMG_CRUSH);
            OnTakeDamage(info);
          }
        }
...
// bool CPortal_Player::StartReplayMode( float fDelay, float fDuration, int
// iEntity )
//{
...
void CPortal_Player::PlayerUse(void) {
  ...
  if (pUseEntity) {
    ...
    // TODO: Removed because we no longer have ghost animatings. May need to
    // rework this code.
    //// If we found a ghost animating then it needs to be held across a portal
    // CGhostAnimating *pGhostAnimating = dynamic_cast<CGhostAnimating*>(
    // pUseEntity );  if ( pGhostAnimating )
    //{
    //	CProp_Portal *pPortal = NULL;

    //	CPortalSimulator *pPortalSimulator =
    //CPortalSimulator::GetSimulatorThatOwnsEntity(
    //pGhostAnimating->GetSourceEntity() );

    //	//HACKHACK: This assumes all portal simulators are a member of a
    //prop_portal 	pPortal = (CProp_Portal *)(((char *)pPortalSimulator) -
    //((int)&(((CProp_Portal *)0)->m_PortalSimulator))); 	Assert(
    //(&(pPortal->m_PortalSimulator)) == pPortalSimulator ); //doublechecking
    //the hack
    ...
    CProp_Portal *pPortal =
        UTIL_Portal_FirstAlongRay(rayPortalTest, fMustBeCloserThan);
    ...
    if (pUseEntity) {
      ...
      // Signal that we want to play the deny sound, unless the user is +USEing
      // on a ladder! The sound is emitted in ItemPostFrame, since that occurs
      // after GameMovement::ProcessMove which lets the ladder code unset this
      // flag.
...
void CPortal_Player::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper) {
  if (m_bFixEyeAnglesFromPortalling) {
    // the idea here is to handle the notion that the player has portalled, but
    // they sent us an angle update before receiving that message.  If we don't
    // handle this here, we end up sending back their old angles which makes them
    // hiccup their angles for a frame
    float fOldAngleDiff =
        fabs(AngleDistance(ucmd->viewangles.x, m_qPrePortalledViewAngles.x));
    fOldAngleDiff +=
        fabs(AngleDistance(ucmd->viewangles.y, m_qPrePortalledViewAngles.y));
    fOldAngleDiff +=
        fabs(AngleDistance(ucmd->viewangles.z, m_qPrePortalledViewAngles.z));

    float fCurrentAngleDiff =
        fabs(AngleDistance(ucmd->viewangles.x, pl.v_angle.x));
    ...
    if (fCurrentAngleDiff > fOldAngleDiff)
      ucmd->viewangles = TransformAnglesToWorldSpace(
          ucmd->viewangles, m_matLastPortalled.As3x4());
...
void CPortal_Player::CreateRagdollEntity(const CTakeDamageInfo &info) {
  ...
  CBaseEntity *pRagdoll = CreateServerRagdoll(
      this, m_nForceBone, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, true);
  pRagdoll->m_takedamage = DAMAGE_NO;
  m_hRagdoll = pRagdoll;

  /*
          // If we already have a ragdoll destroy it.
          CPortalRagdoll *pRagdoll = dynamic_cast<CPortalRagdoll*>(
     m_hRagdoll.Get() ); if( pRagdoll )
          {
                  UTIL_Remove( pRagdoll );
                  pRagdoll = NULL;
          }
          ...
          // Create a ragdoll.
          pRagdoll = dynamic_cast<CPortalRagdoll*>( CreateEntityByName(
     "portal_ragdoll" ) ); if ( pRagdoll )
          {
     ...
     pBoneToWorldNext, BONE_USED_BY_ANYTHING ); SetCycle( fPreviousCycle );
                  ...
                  pRagdoll->InitRagdoll( info.GetDamageForce(), m_nForceBone,
     info.GetDamagePosition(), pBoneToWorld, pBoneToWorldNext, 0.1f,
     COLLISION_GROUP_INTERACTIVE_DEBRIS, true ); pRagdoll->SetMoveType(
     MOVETYPE_VPHYSICS ); pRagdoll->SetSolid( SOLID_VPHYSICS ); if (
     IsDissolving() )
                  {
...
void CPortal_Player::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if ((info.GetDamageType() & DMG_DISSOLVE) &&
      !(m_hRagdoll.Get()->GetEFlags() & EFL_NO_DISSOLVE)) {
    if (m_hRagdoll) {
      m_hRagdoll->GetBaseAnimating()->Dissolve(NULL, gpGlobals->curtime, false,
...
int CPortal_Player::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (inputInfoCopy.GetAttacker() == this &&
      inputInfoCopy.GetDamageType() == DMG_BULLET) {
  ...
  } else if ((inputInfoCopy.GetDamageType() & DMG_SHOCK) ||
             (inputInfoCopy.GetDamageType() & DMG_BURN)) {
...
//-----------------------------------------------------------------------------
void CPortal_Player::PickupObject(CBaseEntity *pObject,
                                  bool bLimitMassAndSize) {
  // can't pick up what you're standing on
  if (GetGroundEntity() == pObject) return;

  if (bLimitMassAndSize == true) {
    if (CBasePlayer::CanPickupObject(pObject, PORTAL_PLAYER_MAX_LIFT_MASS,
                                     PORTAL_PLAYER_MAX_LIFT_SIZE) == false)
      return;
  }
...
void CPortal_Player::ForceDropOfCarriedPhysObjects(
...
void CPortal_Player::UpdateSecondsTaken(void) {
  float fSecondsSinceLastUpdate =
      (gpGlobals->curtime - m_fTimeLastNumSecondsUpdate);
...
//-----------------------------------------------------------------------------
// Purpose: Update the area bits variable which is networked down to the client
// to determine
  ...
  for (int i = 0; i < iPortalCount; ++i) {
    ...
    if (pLocalPortal && pLocalPortal->m_bActivated) {
      ...
      if (pRemotePortal && pRemotePortal->m_bActivated &&
        ...
        engine->GetAreaBits(portalArea[i], &portalTempBits[i * 32],
                            sizeof(unsigned char) * 32);
      }
    }
  }

  // Use the union of player-view area bits and the portal-view area bits of
  // each portal
  for (int i = 0; i < m_Local.m_chAreaBits.Count(); i++) {
    for (int j = 0; j < iPortalCount; ++j) {
      ...
      if (bUsePortalForVis[j]) {
        m_Local.m_chAreaBits.Set(
            i, m_Local.m_chAreaBits[i] | portalTempBits[(j * 32) + i]);
      }
    }
  }
}
...
// once for the setup of each portal. input - pPortal: the portal we are viewing
// 'out of' which needs it's corners added to the PVS
//////////////////////////////////////////////////////////////////////////
void AddPortalCornersToEnginePVS(CProp_Portal *pPortal) {
  ...
  // Distance offsets to the different edges of the portal... Used in the
  // placement checks
  ...
  engine->AddOriginToPVS(ptOrigin + vToBottomEdge + vToRightEdge +
                         vForwardBump);
}

void PortalSetupVisibility(CBaseEntity *pPlayer, int area, unsigned char *pvs,
                           int pvssize) {
  int iPortalCount = CProp_Portal_Shared::AllPortals.Count();
  if (iPortalCount == 0) return;
  ...
  for (int i = 0; i != iPortalCount; ++i) {
    ...
    if (pPortal && pPortal->m_bActivated) {
      if (pPortal->NetworkProp()->IsInPVS(pPlayer->edict(), pvs, pvssize)) {
        if (engine->CheckAreasConnected(area,
                                        pPortal->NetworkProp()->AreaNum())) {
          CProp_Portal *pLinkedPortal =
              static_cast<CProp_Portal *>(pPortal->m_hLinkedPortal.Get());
          if (pLinkedPortal) {
...
void CPortal_Player::SetupVisibility(CBaseEntity *pViewEntity,
  ...
  // At this point the EyePosition has been added as a view origin, but if we
  // are currently stuck in a portal, our EyePosition may return a point in
  // solid. Find the reflected eye position and use that as a vis origin
  // instead.
  if (m_hPortalEnvironment) {
    ...
    if (pPortal && pRemotePortal && pPortal->m_bActivated &&
        pRemotePortal->m_bActivated) {
      ...
      {
        // Move eye origin to it's transformed position on the other side of the
        // portal
        UTIL_Portal_PointTransform(pPortal->MatrixThisToLinked(), eyeOrigin,
                                   eyeOrigin);

        // Use this as our view origin (as this is where the client will be
        // displaying from)
...
CBaseEntity *CPortal_Player::EntSelectSpawnPoint(void) {
  ...
  // we haven't found a place to spawn yet,  so kill any guy at the first spawn
  // point and spawn there
  if (pSpot) {
...
CON_COMMAND(startadmiregloves, "Starts the admire gloves animation.") {
  ...
  if (pPlayer == NULL) pPlayer = GetPortalPlayer(1);  // last ditch effort

  if (pPlayer) pPlayer->StartAdmireGlovesAnimation();
}

CON_COMMAND(displayportalplayerstats,
            "Displays current level stats for portals placed, steps taken, and "
            "seconds taken.") {
  CPortal_Player *pPlayer = (CPortal_Player *)UTIL_GetCommandClient();
  if (pPlayer == NULL) pPlayer = GetPortalPlayer(1);  // last ditch effort
  ...
  if (pPlayer) {
    ...
    NDebugOverlay::ScreenText(
        0.5f, 0.5f,
        msg.sprintf("Portals Placed: %d\nSteps Taken: %d\nTime: %d:%d",
                    pPlayer->NumPortalsPlaced(), pPlayer->NumStepsTaken(),
                    iMinutes, iSeconds),
        255, 255, 255, 150, 5.0f);
  }
}
