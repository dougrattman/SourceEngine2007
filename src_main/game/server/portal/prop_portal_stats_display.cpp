...
void CPropPortalStatsDisplay::SetTransmit(CCheckTransmitInfo *pInfo,
                                          bool bAlways) {
...
void CPropPortalStatsDisplay::Spawn(void) {
  ...
  BonusMapChallengeNames(
      szChallengeFileName.GetForModify(), szChallengeFileName.Size(),
      szChallengeMapName.GetForModify(), szChallengeMapName.Size(),
      szChallengeName.GetForModify(), szChallengeName.Size());
...
void CPropPortalStatsDisplay::Enable(void) {
  ...
  if (pPlayer && pPlayer->GetBonusChallenge() == 0) return;
...
void CPropPortalStatsDisplay::InputEnable(inputdata_t &inputdata) { Enable(); }
...
void CPropPortalStatsDisplay::GetControlPanelInfo(int nPanelIndex,
                                                  const char *&pPanelName) {
  pPanelName = "portal_stats_display_screen";
}

void CPropPortalStatsDisplay::GetControlPanelClassName(
    int nPanelIndex, const char *&pPanelName) {
...
void CPropPortalStatsDisplay::SpawnControlPanels() {
  ...
  for (nPanel = 0; true; ++nPanel) {
    ...
    if (nURAttachmentIndex <= 0) {
      ...
      if (nURAttachmentIndex <= 0) return;
    }
    ...
    CVGuiScreen *pScreen =
        CreateVGuiScreen(pScreenClassname, pScreenName, pEntityToSpawnOn, this,
                         nLLAttachmentIndex);
