...
class CEntityFactoryDictionary : public IEntityFactoryDictionary {
 public:
  ...
  virtual void Destroy(const char *pClassName,
...
void DumpEntityFactories_f() {
  CEntityFactoryDictionary *dict =
      (CEntityFactoryDictionary *)EntityFactoryDictionary();
  if (dict) {
    for (int i = dict->m_Factories.First();
         i != dict->m_Factories.InvalidIndex(); i = dict->m_Factories.Next(i)) {
      Warning("%s\n", dict->m_Factories.GetElementName(i));
    }
  }
}

static ConCommand dumpentityfactories("dumpentityfactories",
...
IEntityFactory *CEntityFactoryDictionary::FindFactory(const char *pClassName) {
  ...
  if (nIndex == m_Factories.InvalidIndex()) return NULL;
...
//
//-----------------------------------------------------------------------------
const char *CEntityFactoryDictionary::GetCannonicalName(
    const char *pClassName) {
...
//-----------------------------------------------------------------------------
void CEntityFactoryDictionary::Destroy(const char *pClassName,
                                       IServerNetworkable *pNetworkable) {
  IEntityFactory *pFactory = FindFactory(pClassName);
  if (!pFactory) {
...
void CEntityFactoryDictionary::ReportEntitySizes() {
  for (int i = m_Factories.First(); i != m_Factories.InvalidIndex();
       i = m_Factories.Next(i)) {
    Msg(" %s: %d", m_Factories.GetElementName(i),
        m_Factories[i]->GetEntitySize());
  }
}
...
CFlaggedEntitiesEnum::CFlaggedEntitiesEnum(CBaseEntity **pList, int listMax,
                                           int flagMask) {
  m_pList = pList;
  m_listMax = listMax;
  m_flagMask = flagMask;
...
bool CFlaggedEntitiesEnum::AddToList(CBaseEntity *pEntity) {
  if (m_count >= m_listMax) {
    AssertMsgOnce(0,
                  "reached enumerated list limit.  Increase limit, decrease "
                  "radius, or make it so entity flags will work for you");
    return false;
  }
...
IterationRetval_t CFlaggedEntitiesEnum::EnumElement(
    IHandleEntity *pHandleEntity) {
  CBaseEntity *pEntity = gEntList.GetBaseEntity(pHandleEntity->GetRefEHandle());
  if (pEntity) {
    if (m_flagMask &&
        !(pEntity->GetFlags() & m_flagMask))  // Does it meet the criteria?
...
float UTIL_GetSimulationInterval() {
  if (CBaseEntity::IsSimulatingOnAlternateTicks()) return (TICK_INTERVAL * 2.0);
...
int UTIL_EntitiesInBox(const Vector &mins, const Vector &maxs,
                       CFlaggedEntitiesEnum *pEnum) {
  partition->EnumerateElementsInBox(PARTITION_ENGINE_NON_STATIC_EDICTS, mins,
                                    maxs, false, pEnum);
  return pEnum->GetCount();
}

int UTIL_EntitiesAlongRay(const Ray_t &ray, CFlaggedEntitiesEnum *pEnum) {
  partition->EnumerateElementsAlongRay(PARTITION_ENGINE_NON_STATIC_EDICTS, ray,
                                       false, pEnum);
  return pEnum->GetCount();
}

int UTIL_EntitiesInSphere(const Vector &center, float radius,
                          CFlaggedEntitiesEnum *pEnum) {
  partition->EnumerateElementsInSphere(PARTITION_ENGINE_NON_STATIC_EDICTS,
                                       center, radius, false, pEnum);
  return pEnum->GetCount();
}
...
CBaseEntity *CEntitySphereQuery::GetCurrentEntity() {
  if (m_listIndex < m_listCount) return m_pList[m_listIndex];
  return NULL;
}
...
int UTIL_DropToFloor(CBaseEntity *pEntity, unsigned int mask,
  ...
  trace_t trace;
  // HACK: is this really the only sure way to detect crossing a terrain
  // boundry?
  UTIL_TraceEntity(pEntity, pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin(),
                   mask, pIgnore, pEntity->GetCollisionGroup(), &trace);
  if (trace.fraction == 0.0) return -1;

  UTIL_TraceEntity(pEntity, pEntity->GetAbsOrigin(),
                   pEntity->GetAbsOrigin() - Vector(0, 0, 256), mask, pIgnore,
                   pEntity->GetCollisionGroup(), &trace);

  if (trace.allsolid) return -1;
...
// is not a staircase.
//-----------------------------------------------------------------------------
bool UTIL_CheckBottom(CBaseEntity *pEntity, ITraceFilter *pTraceFilter,
                      float flStepSize) {
...
//-----------------------------------------------------------------------------
// Purpose: Sets the entity up for deletion.  Entity will not actually be
// deleted
...
void UTIL_Remove(IServerNetworkable *oldObj) {
  ...
  if (!pProp || pProp->IsMarkedForDeletion()) return;

  if (PhysIsInCallback()) {
    // This assert means that someone is deleting an entity inside a callback.
    // That isn't supported so this code will defer the deletion of that object
    // until the end of the current physics simulation frame Since this is
    // hidden from the calling code it's preferred to call PhysCallbackRemove()
    // directly from the caller in case the deferred delete will have unwanted
    // results (like continuing to receive callbacks).  That will make it
    // obvious why the unwanted results are happening so the caller can handle
    // them appropriately. (some callbacks can be masked or the calling entity
    // can be flagged to filter them in most cases)
    Assert(0);
  ...
  if (pBaseEnt) {
#ifdef PORTAL  // make sure entities are in the primary physics environment for
               // the portal mod, this code should be safe even if the entity is
               // in neither extra environment
...
void UTIL_DisableRemoveImmediate() { s_RemoveImmediateSemaphore++; }
void UTIL_EnableRemoveImmediate() {
  s_RemoveImmediateSemaphore--;
  Assert(s_RemoveImmediateSemaphore >= 0);
}
//-----------------------------------------------------------------------------
// Purpose: deletes an entity, without any delay.  WARNING! Only use this when
// sure
...
void UTIL_RemoveImmediate(CBaseEntity *oldObj) {
  ...
  if (!oldObj || oldObj->IsEFlagSet(EFL_KILLME)) return;
  ...
  oldObj->AddEFlags(EFL_KILLME);  // Make sure to ignore further calls into here
                                  // or UTIL_Remove.
...
// returns a CBaseEntity pointer to a player by index.  Only returns if the
// player is spawned and connected otherwise returns NULL Index is 1 based
...
CBasePlayer *UTIL_GetListenServerHost(void) {
  ...
  if (engine->IsDedicatedServer()) {
    ...
    Warning(
        "UTIL_GetListenServerHost() called from a dedicated server or "
        "single-player game.\n");
    return NULL;
  }
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Returns true if the command was issued by the listenserver host, or by the
 * dedicated server, via rcon or the server console. This is valid during
 * ConCommand execution.
 */
bool UTIL_IsCommandIssuedByServerAdmin(void) {
  int issuingPlayerIndex = UTIL_GetCommandClientIndex();

  if (engine->IsDedicatedServer() && issuingPlayerIndex > 0) return false;

  if (issuingPlayerIndex > 1) return false;
...
//-----------------------------------------------------------------------------
inline float ComputeShakeAmplitude(const Vector &center, const Vector &shakePt,
                                   float amplitude, float radius) {
  if (radius <= 0) return amplitude;
...
//-----------------------------------------------------------------------------
inline void TransmitShakeEvent(CBasePlayer *pPlayer, float localAmplitude,
  ...
  if ((localAmplitude > 0) || (eCommand == SHAKE_STOP)) {
    ...
    WRITE_BYTE(
        eCommand);  // shake command (SHAKE_START, STOP, FREQUENCY, AMPLITUDE)
...
//				SHAKE_START - starts the screen shake for all players
//within the radius
//				SHAKE_STOP - stops the screen shake for all players
//within the radius
//				SHAKE_AMPLITUDE - modifies the amplitude of the screen
//shake
//									for all players within
//the radius
//				SHAKE_FREQUENCY - modifies the frequency of the screen
//shake
//									for all players within
//the radius
//			bAirShake - if this is false, then it will only shake
//players standing on the ground.
//-----------------------------------------------------------------------------
const float MAX_SHAKE_AMPLITUDE = 16.0f;
void UTIL_ScreenShake(const Vector &center, float amplitude, float frequency,
                      float duration, float radius, ShakeCommand_t eCommand,
                      bool bAirShake) {
  int i;
  ...
  for (i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    //
    // Only start shakes for players that are on the ground unless doing an air
    // shake.
    //
    if (!pPlayer || (!bAirShake && (eCommand == SHAKE_START) &&
                     !(pPlayer->GetFlags() & FL_ONGROUND))) {
      continue;
    }

    localAmplitude = ComputeShakeAmplitude(center, pPlayer->WorldSpaceCenter(),
                                           amplitude, radius);

    // This happens if the player is outside the radius, in which case we should
    // ignore all commands
    if (localAmplitude < 0) continue;

    TransmitShakeEvent((CBasePlayer *)pPlayer, localAmplitude, frequency,
                       duration, eCommand);
  }
}
...
//-----------------------------------------------------------------------------
void UTIL_ScreenShakeObject(CBaseEntity *pEnt, const Vector &center,
                            float amplitude, float frequency, float duration,
                            float radius, ShakeCommand_t eCommand,
                            bool bAirShake) {
  int i;
  float localAmplitude;
  ...
  for (i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    // Shake the object, or anything hierarchically attached to it at maximum
    // amplitude
    localAmplitude = 0;
    if (pHighestParent == pPlayer->GetRootMoveParent()) {
      localAmplitude = amplitude;
    } else if ((pPlayer->GetFlags() & FL_ONGROUND) &&
               (pPlayer->GetGroundEntity()->GetRootMoveParent() ==
                pHighestParent)) {
      ...
      localAmplitude = ComputeShakeAmplitude(
          center, pPlayer->WorldSpaceCenter(), amplitude, radius);
...
//			bInAir - if this is false, then it will only punch players
//standing on the ground.
//-----------------------------------------------------------------------------
void UTIL_ViewPunch(const Vector &center, QAngle angPunch, float radius,
                    bool bInAir) {
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    CBaseEntity *pPlayer = UTIL_PlayerByIndex(i);

    //
    // Only apply the punch to players that are on the ground unless doing an
    // air punch.
    //
...
void UTIL_ScreenFadeBuild(ScreenFade_t &fade, const color32 &color,
                          float fadeTime, float fadeHold, int flags) {
  fade.duration =
      FixedUnsigned16(fadeTime, 1 << SCREENFADE_FRACBITS);  // 7.9 fixed
  fade.holdTime =
      FixedUnsigned16(fadeHold, 1 << SCREENFADE_FRACBITS);  // 7.9 fixed
  fade.r = color.r;
  fade.g = color.g;
  fade.b = color.b;
  fade.a = color.a;
...
void UTIL_ScreenFadeWrite(const ScreenFade_t &fade, CBaseEntity *pEntity) {
  if (!pEntity || !pEntity->IsNetClient()) return;
...
void UTIL_ScreenFadeAll(const color32 &color, float fadeTime, float fadeHold,
                        int flags) {
  int i;
  ScreenFade_t fade;
...
void UTIL_ScreenFade(CBaseEntity *pEntity, const color32 &color, float fadeTime,
                     float fadeHold, int flags) {
  ScreenFade_t fade;
...
void UTIL_ClientPrintFilter(IRecipientFilter &filter, int msg_dest,
                            const char *msg_name, const char *param1,
...
void UTIL_ClientPrintAll(int msg_dest, const char *msg_name, const char *param1,
  ...
  UTIL_ClientPrintFilter(filter, msg_dest, msg_name, param1, param2, param3,
                         param4);
}
...
void ClientPrint(CBasePlayer *player, int msg_dest, const char *msg_name,
  ...
  UTIL_ClientPrintFilter(user, msg_dest, msg_name, param1, param2, param3,
                         param4);
}

void UTIL_SayTextFilter(IRecipientFilter &filter, const char *pText,
                        CBasePlayer *pPlayer, bool bChat) {
...
void UTIL_SayText2Filter(IRecipientFilter &filter, CBasePlayer *pEntity,
                         bool bChat, const char *msg_name, const char *param1,
                         const char *param2, const char *param3,
                         const char *param4) {
...
void UTIL_SayText(const char *pText, CBasePlayer *pToPlayer) {
  if (!pToPlayer->IsNetClient()) return;
...
void UTIL_SetTrace(trace_t &trace, const Ray_t &ray, edict_t *ent,
                   float fraction, int hitgroup, unsigned int contents,
                   const Vector &normal, float intercept) {
...
//-----------------------------------------------------------------------------
static void SetMinMaxSize(CBaseEntity *pEnt, const Vector &mins,
                          const Vector &maxs) {
  for (int i = 0; i < 3; i++) {
    if (mins[i] > maxs[i]) {
      Error("%s: backwards mins/maxs",
...
// Sets the model size
//-----------------------------------------------------------------------------
void UTIL_SetSize(CBaseEntity *pEnt, const Vector &vecMin,
                  const Vector &vecMax) {
...
void UTIL_SetOrigin(CBaseEntity *entity, const Vector &vecOrigin,
                    bool bFireTriggers) {
...
void UTIL_ParticleEffect(const Vector &vecOrigin, const Vector &vecDirection,
                         unsigned long ulColor, unsigned long ulCount) {
  Msg("UTIL_ParticleEffect:  Disabled\n");
}

void UTIL_Smoke(const Vector &origin, const float scale,
...
bool UTIL_IsMasterTriggered(string_t sMaster, CBaseEntity *pActivator) {
  if (sMaster != NULL_STRING) {
    CBaseEntity *pMaster =
        gEntList.FindEntityByName(NULL, sMaster, NULL, pActivator);

    if (pMaster && (pMaster->ObjectCaps() & FCAP_MASTER)) {
      return pMaster->IsTriggered(pActivator);
    }

    Warning("Master was 0 or not a master!\n");
  }
...
//			 as flecks) for the given iDamageType and the trace's end
//position
// Input   :
// Output  :
//------------------------------------------------------------------------------
void UTIL_ImpactTrace(trace_t *pTrace, int iDamageType,
  ...
  if (!pEntity || !UTIL_IsValidEntity(pEntity) ||
...
void UTIL_PlayerDecalTrace(trace_t *pTrace, int playernum) {
  ...
  te->PlayerDecal(filter, 0.0, &pTrace->endpos, playernum,
                  pTrace->m_pEnt->entindex());
}

bool UTIL_TeamsMatch(const char *pTeamName1, const char *pTeamName2) {
  // Everyone matches unless it's teamplay
  if (!g_pGameRules->IsTeamplay()) return true;
...
void UTIL_AxisStringToPointDir(Vector &start, Vector &dir,
                               const char *pString) {
  Vector end;
...
float UTIL_WaterLevel(const Vector &position, float minz, float maxz) {
  ...
  midUp.z = maxz;
  if (UTIL_PointContents(midUp) & MASK_WATER) return maxz;
...
class CWaterTraceFilter : public CTraceFilter {
 public:
  bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pCollide->GetSolidFlags() & FSOLID_VOLUME_CONTENTS) return true;
...
void UTIL_Bubbles(const Vector &mins, const Vector &maxs, int count) {
  ...
  te->Bubbles(filter, 0.0, &mins, &maxs, flHeight, g_sModelIndexBubbles, count,
              8.0);
}
...
void UTIL_BubbleTrail(const Vector &from, const Vector &to, int count) {
  ...
  if ((flStartHeight == 0) && (flEndHeight == 0)) return;
  ...
  CBroadcastRecipientFilter filter;
  te->BubbleTrail(filter, 0.0, pFrom, pTo, flWaterZ, g_sModelIndexBubbles,
                  count, 8.0);
}
...
void UTIL_Beam(Vector &Start, Vector &End, int nModelIndex, int nHaloIndex,
  ...
  te->BeamPoints(filter, 0.0, &Start, &End, nModelIndex, nHaloIndex, FrameStart,
                 FrameRate, Life, Width, EndWidth, FadeLength, Noise, Red,
                 Green, Blue, Brightness, Speed);
}
...
// UNDONE: Do we need this to avoid doing too much of this?  Measure startup
// times and see
...
void UTIL_PrecacheOther(const char *szClassname, const char *modelName) {
  ...
  if (!g_PrecacheOtherList.AddOrMarkPrecached(szClassname)) return;
#endif
...
//-----------------------------------------------------------------------------
// Purpose: Spawns an entity into the game, initializing it with the map ent
// data block Input  : *pEntity - the newly created entity
...
int DispatchSpawn(CBaseEntity *pEntity) {
  if (pEntity) {
    ...
    // Initialize these or entities who don't link to the world won't have
    // anything in here is this necessary?
    // pEntity->SetAbsMins( pEntity->GetOrigin() - Vector(1,1,1) );
    // pEntity->SetAbsMaxs( pEntity->GetOrigin() + Vector(1,1,1) );

#if defined(TRACK_ENTITY_MEMORY) && defined(USE_MEM_DEBUG)
    const char *pszClassname = NULL;
    int iClassname = ((CEntityFactoryDictionary *)EntityFactoryDictionary())
                         ->m_Factories.Find(pEntity->GetClassname());
    if (iClassname != ((CEntityFactoryDictionary *)EntityFactoryDictionary())
                          ->m_Factories.InvalidIndex())
      pszClassname = ((CEntityFactoryDictionary *)EntityFactoryDictionary())
                         ->m_Factories.GetElementName(iClassname);
    if (pszClassname) {
...
#endif
    // Try to get the pointer again, in case the spawn function deleted the
    // entity. UNDONE: Spawn() should really return a code to ask that the
    // entity be deleted, but that would touch too much code for me to do that
    // right now.

    if (pEntSafe == NULL || pEntity->IsMarkedForDeletion()) return -1;
    ...
    if (pEntity->m_iGlobalname != NULL_STRING) {
      ...
      if (globalIndex >= 0) {
        ...
        if (GlobalEntity_GetState(globalIndex) == GLOBAL_DEAD) {
          pEntity->Remove();
          return -1;
        } else if (!FStrEq(STRING(gpGlobals->mapname),
                           GlobalEntity_GetMap(globalIndex))) {
          pEntity->MakeDormant();  // Hasn't been moved to this level yet, wait
                                   // but stay alive
        }
...
// UNDONE: This could be a better test - can we run the absbox through the bsp
// and see if it contains any solid space?  or would that eliminate some
// entities we want to keep?
...
void EntityMatrix::InitFromEntity(CBaseEntity *pEntity, int iAttachment) {
  ...
  ((VMatrix *)this)
      ->SetupMatrixOrgAngles(pEntity->GetAbsOrigin(), pEntity->GetAbsAngles());
}
...
void EntityMatrix::InitFromEntityLocal(CBaseEntity *entity) {
  ...
  ((VMatrix *)this)
      ->SetupMatrixOrgAngles(entity->GetLocalOrigin(),
...
//			not skip over more than one separator at a time. This allows
//parsing
//			strings where tokens between separators may or may not be
//present:
//
//			Door01,,,0 would be parsed as "Door01"  ""  ""  "0"
//			Door01,Open,,0 would be parsed as "Door01"  "Open"  ""
//"0"
//
// Input  : token - Returns with a token, or zero length if the token was
// missing.
//			str - String to parse.
//			sep - Character to use as separator. UNDONE: allow multiple
//separator chars
...
static int UTIL_GetNewCheckClient(int check) {
  ...
  if (check < 1) check = 1;
  if (check > gpGlobals->maxClients) check = gpGlobals->maxClients;
  ...
  for (;; i++) {
    ...
    if (!ent->GetUnknown()) continue;

    CBaseEntity *entity = GetContainingEntity(ent);
    if (!entity) continue;

    if (entity->GetFlags() & FL_NOTARGET) continue;
  ...
  if (i != check) {
    memset(g_CheckClient.m_checkVisibilityPVS, 0,
           sizeof(g_CheckClient.m_checkVisibilityPVS));
  ...
  if (ent) {
    ...
    if (clusterIndex != g_CheckClient.m_checkCluster) {
      ...
      engine->GetPVSForCluster(clusterIndex, sizeof(g_CheckClient.m_checkPVS),
                               g_CheckClient.m_checkPVS);
    }
  }
...
static edict_t *UTIL_GetCurrentCheckClient() {
  ...
  if (delta >= 0.1 || delta < 0) {
    g_CheckClient.m_lastcheck =
        UTIL_GetNewCheckClient(g_CheckClient.m_lastcheck);
  ...
  // It's annoying to die and see monsters stop thinking because you're no
  // longer "in" their PVS
...
void UTIL_SetClientVisibilityPVS(edict_t *pClient, const unsigned char *pvs,
                                 int pvssize) {
  if (pClient == UTIL_GetCurrentCheckClient()) {
    ...
    for (i = 0; i < remainder; i++) {
      ((unsigned char *)&pTo[limit])[i] = ((unsigned char *)&pFrom[limit])[i] &
                                          !((unsigned char *)&pMask[limit])[i];
...
bool UTIL_ClientPVSIsExpanded() { return g_CheckClient.m_bClientPVSIsExpanded; }

//-----------------------------------------------------------------------------
// Purpose: Returns a client (or object that has a client enemy) that would be a
// valid target.
...
// Output : edict_t*
//-----------------------------------------------------------------------------
CBaseEntity *UTIL_FindClientInPVS(const Vector &vecBoxMins,
  ...
  if (!engine->CheckBoxInPVS(vecBoxMins, vecBoxMaxs, g_CheckClient.m_checkPVS,
                             sizeof(g_CheckClient.m_checkPVS))) {
    return NULL;
  }
...
// Output : edict_t*
//-----------------------------------------------------------------------------
ConVar sv_strict_notarget(
    "sv_strict_notarget", "0", 0,
    "If set, notarget will cause entities to never think they are in the pvs");

static edict_t *UTIL_FindClientInPVSGuts(edict_t *pEdict, unsigned char *pvs,
                                         unsigned pvssize) {
  Vector view;
  ...
  if ((!pPlayerEntity || (pPlayerEntity->GetFlags() & FL_NOTARGET)) &&
      sv_strict_notarget.GetBool()) {
    return NULL;
  }
...
edict_t *UTIL_FindClientInPVS(edict_t *pEdict) {
  return UTIL_FindClientInPVSGuts(pEdict, g_CheckClient.m_checkPVS,
                                  sizeof(g_CheckClient.m_checkPVS));
}

//-----------------------------------------------------------------------------
// Purpose: Returns a client that could see the entity, including through a
// camera
//-----------------------------------------------------------------------------
edict_t *UTIL_FindClientInVisibilityPVS(edict_t *pEdict) {
  return UTIL_FindClientInPVSGuts(pEdict, g_CheckClient.m_checkVisibilityPVS,
                                  sizeof(g_CheckClient.m_checkVisibilityPVS));
}

//-----------------------------------------------------------------------------
// Purpose: Returns a chain of entities within the PVS of another entity
// (client)
...
// Output : edict_t
//-----------------------------------------------------------------------------
CBaseEntity *UTIL_EntitiesInPVS(CBaseEntity *pPVSEntity,
                                CBaseEntity *pStartingEntity) {
  Vector org;
  static byte pvs[MAX_MAP_CLUSTERS / 8];
  static Vector lastOrg(0, 0, 0);
  static int lastCluster = -1;

  if (!pPVSEntity) return NULL;
  ...
  // So, we'll always get the PVS each time we start a new EntitiesInPVS
  // iteration. Given that weapon_binocs + leveltransition code is the only
  // current clients of this, this seems safe.
  if (!pStartingEntity) {
  ...
  for (CBaseEntity *pEntity = gEntList.NextEnt(pStartingEntity); pEntity;
       pEntity = gEntList.NextEnt(pEntity)) {
    // Only return attached ents.
    if (!pEntity->edict()) continue;

    CBaseEntity *pParent = pEntity->GetRootMoveParent();

    Vector vecSurroundMins, vecSurroundMaxs;
    pParent->CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins,
                                                          &vecSurroundMaxs);
    if (!engine->CheckBoxInPVS(vecSurroundMins, vecSurroundMaxs, pvs,
                               sizeof(pvs)))
      continue;
...
//-----------------------------------------------------------------------------
// Purpose: Get the predicted postion of an entity of a certain number of
// seconds
//			Use this function with caution, it has great potential for
//annoying the player, especially
...
void UTIL_PredictedPosition(CBaseEntity *pTarget, float flTimeDelta,
  ...
  if ((pTarget == NULL) || (vecPredictedPosition == NULL)) return;
  ...
  // Get the result
  (*vecPredictedPosition) =
      pTarget->GetAbsOrigin() + (vecPredictedVel * flTimeDelta);
}
...
// Input  : *pDest - entity to be pointed at the target
//			strTarget - name of entity to target (will only choose the
//first!)
//-----------------------------------------------------------------------------
...
bool UTIL_TransferPoseParameters(CBaseEntity *pSourceEntity,
  ...
  CBaseAnimating *pSourceBaseAnimating =
      dynamic_cast<CBaseAnimating *>(pSourceEntity);
  CBaseAnimating *pDestBaseAnimating =
      dynamic_cast<CBaseAnimating *>(pDestEntity);

  if (!pSourceBaseAnimating || !pDestBaseAnimating) return false;

  for (int iPose = 0; iPose < MAXSTUDIOPOSEPARAM; ++iPose) {
    pDestBaseAnimating->SetPoseParameter(
        iPose, pSourceBaseAnimating->GetPoseParameter(iPose));
  }
...
//-----------------------------------------------------------------------------
void UTIL_MuzzleFlash(const Vector &origin, const QAngle &angles, int scale,
                      int type) {
...
//			vPoint - point to find nearest point to on specified
//line
//			clampEnds - clamps returned points to being on the line
//segment specified
// Output : Vector - nearest point on the specified line
//-----------------------------------------------------------------------------
Vector UTIL_PointOnLineNearestPoint(const Vector &vStartPos,
...
AngularImpulse WorldToLocalRotation(const VMatrix &localToWorld,
  ...
  // since the matrix maps local to world, do a transpose rotation to get world
  // to local
...
// Input  : *buffer -
//-----------------------------------------------------------------------------
void UTIL_FreeFile(byte *buffer) { filesystem->FreeOptimalReadBuffer(buffer); }

//-----------------------------------------------------------------------------
// Purpose: Determines whether an entity is within a certain angular tolerance
// to viewer Input  : *pEntity - entity which is the "viewer"
...
bool UTIL_IsFacingWithinTolerance(CBaseEntity *pViewer,
  ...
  if (flDot >= flDotTolerance) return true;
...
bool UTIL_IsFacingWithinTolerance(CBaseEntity *pViewer, CBaseEntity *pTarget,
  ...
  return UTIL_IsFacingWithinTolerance(pViewer, pTarget->GetAbsOrigin(),
                                      flDotTolerance, pflDot);
}
...
void LoadAndSpawnEntities_ParseEntKVBlockHelper(CBaseEntity *pNode,
                                                KeyValues *pkvNode) {
  ...
  if (pkvFile->LoadFromFile(filesystem, pScriptFile, "MOD")) {
    ...
    while (pkvNode) {
      ...
      if (pNode) {
        ...
        Warning(
            "UTIL_LoadAndSpawnEntitiesFromScript: Failed to spawn entity, "
            "type: '%s'\n",
            pNodeName);
      }
...
//-----------------------------------------------------------------------------
// Purpose: Convert a vector an angle from worldspace to the entity's parent's
// local space Input  : *pEntity - Entity whose parent we're concerned with
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Convert a vector and quaternion from worldspace to the entity's
// parent's local space Input  : *pEntity - Entity whose parent we're concerned
// with
//-----------------------------------------------------------------------------
void UTIL_ParentToWorldSpace(CBaseEntity *pEntity, Vector &vecPosition,
                             Quaternion &quat) {
...
//-----------------------------------------------------------------------------
// Purpose: Given a vector, clamps the scalar axes to MAX_COORD_FLOAT ranges
// from worldsize.h Input  : *pVecPos -
//-----------------------------------------------------------------------------
...
void CC_KDTreeTest(const CCommand &args) {
  ...
  //	CBaseEntity *pSpot = gEntList.FindEntityByClassname( NULL,
  //"info_player_start" ); 	Vector vecStart = pSpot->GetAbsOrigin();
  ...
  switch (nTestType) {
    case 0: {
      ...
      for (int iTest = 0; iTest < NUM_KDTREE_TESTS; ++iTest) {
        UTIL_TraceLine(vecStart, vecTargets[iTest], MASK_SOLID_BRUSHONLY, NULL,
    ...
    case 1: {
      ...
      trace_t trace;
      for (int iTest = 0; iTest < NUM_KDTREE_TESTS; ++iTest) {
        UTIL_TraceHull(vecStart, vecTargets[iTest], VEC_HULL_MIN, VEC_HULL_MAX,
                       MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &trace);
      }
      break;
    }
    case 2: {
      ...
      for (iTest = 0; iTest < NUM_KDTREE_TESTS; ++iTest) {
        nCount +=
            UTIL_EntitiesInBox(pList, 1024, vecMins[iTest], vecMaxs[iTest], 0);
      }
    ...
    case 3: {
      ...
      for (iTest = 0; iTest < NUM_KDTREE_TESTS; ++iTest) {
        nCount +=
            UTIL_EntitiesInSphere(pList, 1024, vecStart, flRadius[iTest], 0);
      }
...
static ConCommand kdtree_test("kdtree_test", CC_KDTreeTest,
...
static ConCommand voxeltree_view("voxeltree_view", CC_VoxelTreeView,
...
void CC_VoxelTreePlayerView(void) {
  ...
  CBasePlayer *pPlayer = static_cast<CBasePlayer *>(UTIL_GetLocalPlayer());
  Vector vecStart = pPlayer->GetAbsOrigin();
  partition->RenderObjectsInPlayerLeafs(vecStart - VEC_HULL_MIN,
                                        vecStart + VEC_HULL_MAX, 3.0f);
}

static ConCommand voxeltree_playerview(
    "voxeltree_playerview", CC_VoxelTreePlayerView,
    "View entities in the voxel-tree at the player position.", FCVAR_CHEAT);
...
void CC_VoxelTreeBox(const CCommand &args) {
  ...
  debugoverlay->AddLineOverlay(vecPoints[0], vecPoints[1], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[1], vecPoints[2], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[2], vecPoints[3], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[3], vecPoints[0], 255, 0, 0, true,
                               flTime);

  debugoverlay->AddLineOverlay(vecPoints[4], vecPoints[5], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[5], vecPoints[6], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[6], vecPoints[7], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[7], vecPoints[4], 255, 0, 0, true,
                               flTime);

  debugoverlay->AddLineOverlay(vecPoints[0], vecPoints[4], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[3], vecPoints[7], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[1], vecPoints[5], 255, 0, 0, true,
                               flTime);
  debugoverlay->AddLineOverlay(vecPoints[2], vecPoints[6], 255, 0, 0, true,
                               flTime);
...
static ConCommand voxeltree_box(
    "voxeltree_box", CC_VoxelTreeBox,
...
void CC_VoxelTreeSphere(const CCommand &args) {
  ...
  Vector vecMin, vecMax;
  vecMin.Init(vecCenter.x - flRadius, vecCenter.y - flRadius,
              vecCenter.z - flRadius);
  vecMax.Init(vecCenter.x + flRadius, vecCenter.y + flRadius,
              vecCenter.z + flRadius);
...
static ConCommand voxeltree_sphere("voxeltree_sphere", CC_VoxelTreeSphere,
                                   "View entities in the voxel-tree inside "
...
void CC_CollisionTest(const CCommand &args) {
  ...
  for (int j = 0; j < 2; j++) {
    ...
    if (testType == 1) {
      trace_t tr;
      for (i = 0; i < NUM_COLLISION_TESTS; i++) {
        UTIL_TraceHull(start, targets[i], -size[1], size[1], nMask, NULL,
      ...
      for (i = 0; i < NUM_COLLISION_TESTS; i++) {
        ...
        UTIL_TraceLine(start, targets[i], nMask, NULL, COLLISION_GROUP_NONE,
                       &tr);
...
static ConCommand collision_test("collision_test", CC_CollisionTest,
                                 "Tests collision system", FCVAR_CHEAT);
