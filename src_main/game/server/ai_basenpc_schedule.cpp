...
void CAI_BaseNPC::ClearSchedule(const char *szReason) {
  ...
  if (szReason) {
    ADD_DEBUG_HISTORY(HISTORY_AI_DECISIONS,
                      UTIL_VarArgs("%s(%d):  Schedule cleared: %s\n",
...
bool CAI_BaseNPC::SetSchedule(int localScheduleID) {
  ...
  if (pNewSchedule) {
    // ken: I'm don't know of any remaining cases, but if you find one, hunt it
    // down as to why the schedule is getting slammed while they're in the
    // middle of script
    if (m_hCine != NULL) {
      if (!(localScheduleID == SCHED_SLEEP ||
            localScheduleID == SCHED_WAIT_FOR_SCRIPT ||
            localScheduleID == SCHED_SCRIPTED_WALK ||
            localScheduleID == SCHED_SCRIPTED_RUN ||
            localScheduleID == SCHED_SCRIPTED_CUSTOM_MOVE ||
            localScheduleID == SCHED_SCRIPTED_WAIT ||
            localScheduleID == SCHED_SCRIPTED_FACE)) {
        Assert(0);
...
void CAI_BaseNPC::SetSchedule(CAI_Schedule *pNewSchedule) {
  ...
  m_ScheduleState.timeCurTaskStarted = m_ScheduleState.timeStarted =
      gpGlobals->curtime;
  ...
  if (bCondInPVS) SetCondition(COND_IN_PVS);
  ...
  #endif
  */
  // this is very useful code if you can isolate a test case in a level with a
  // single NPC. It will notify you of every schedule selection the NPC makes.
  ...
  ADD_DEBUG_HISTORY(
      ...
      UTIL_VarArgs("%s(%d): Schedule: %s (time: %.2f)\n", GetDebugName(),
                   entindex(), pNewSchedule->GetName(), gpGlobals->curtime));
  ...
  if (m_bSelected) {
    ...
    if (m_ScheduleHistory.Count() == SCHEDULE_HISTORY_SIZE) {
      ...
      float flTimeSpan = m_ScheduleHistory.Head().m_flTimeSelected -
                         m_ScheduleHistory.Tail().m_flTimeSelected;
      ...
      if (flSelectionsPerSecond >= 8.0f) {
        ...
        for (int i = 0; i < m_ScheduleHistory.Count(); i++) {
          ...
          Msg("--%s  %f\n", choice.m_pScheduleSelected->GetName(),
              choice.m_flTimeSelected);
        }
...
bool CAI_BaseNPC::IsScheduleValid() {
  ...
  // - Don't modify interrupts for Schedules that set the
  // COND_NO_CUSTOM_INTERRUPTS bit.
    ...
    if (g_pDeveloper->GetInt()) {
      ...
      for (int i = 0; i < MAX_CONDITIONS; i++) {
        if (testBits.IsBitSet(i)) {
          ...
          ADD_DEBUG_HISTORY(
              HISTORY_AI_DECISIONS,
              UTIL_VarArgs("%s(%d):      Break condition -> %s\n",
...
bool CAI_BaseNPC::ShouldSelectIdealState(void) {
  /*
          ...
          if ( m_IdealNPCState != NPC_STATE_DEAD &&
                   (m_IdealNPCState != NPC_STATE_SCRIPT || m_IdealNPCState ==
     m_NPCState) )
          {
                  if (	(m_afConditions &&
     !HasConditions(bits_COND_SCHEDULE_DONE)) || (GetCurSchedule() &&
  ...
  */
  ...
  // If the NPC has any current conditions, and one of those conditions
  // indicates that the previous schedule completed successfully, then don't run
  // SelectIdealState(). Paths between states only exist for interrupted
  // schedules, or when a schedule contains a task that suggests that the NPC
  // change state.
  if (!HasCondition(COND_SCHEDULE_DONE)) return true;

  // This seems like some sort of hack...
  // Currently no schedule that I can see in the AI uses this feature, but if a
  // schedule interrupt mask contains bits_COND_SCHEDULE_DONE, then force a call
  // to SelectIdealState(). If we want to keep this feature, I suggest we create
  // a new condition with a name that indicates exactly what it does.
  if (GetCurSchedule() && GetCurSchedule()->HasInterrupt(COND_SCHEDULE_DONE))
    return true;

  // Don't call SelectIdealState if a NPC in combat state has a valid enemy
  // handle. Otherwise, we need to change state immediately because something
  // unexpected happened to the enemy entity (it was blown apart by someone
  // else, for example), and we need the NPC to change state. THE REST OF OUR
  // CODE should be robust enough that this can go away!!
...
static bool ShouldStopProcessingTasks(CAI_BaseNPC *pNPC, int taskTime,
                                      int timeLimit) {
#ifdef DEBUG
  if (ai_simulate_task_overtime.GetBool()) return true;
#endif

  // Always stop processing if we've queued up a navigation query on the last
  // task
  if (pNPC->IsNavigationDeferred()) return true;

  if (AIStrongOpt()) {
    bool bInScript = (pNPC->GetState() == NPC_STATE_SCRIPT ||
                      pNPC->IsCurSchedule(SCHED_SCENE_GENERIC, false));
  ...
  if (taskTime > timeLimit) {
    if (ShouldUseEfficiency() || pNPC->IsMoving() ||
        (pNPC->GetIdealActivity() != ACT_RUN &&
         pNPC->GetIdealActivity() != ACT_WALK)) {
      return true;
    }
  }
  return false;
...
void CAI_BaseNPC::MaintainSchedule(void) {
  ...
  bool bDebugTaskNames =
      (developer.GetBool() || (VProfAI() && g_VProfCurrentProfile.IsEnabled()));
#else
  ...
  // UNDONE: Tune/fix this MAX_TASKS_RUN... This is just here so infinite loops
  // are impossible
  bool bStopProcessing = false;
  for (i = 0; i < MAX_TASKS_RUN && !bStopProcessing; i++) {
    if (GetCurSchedule() != NULL && TaskIsComplete()) {
      // Schedule is valid, so advance to the next task if the current is
      // complete.
      NextScheduledTask();

      // If we finished the current schedule, clear our ignored conditions so
      // they aren't applied to the next schedule selection.
    ...
    if (!IsScheduleValid() || m_NPCState != m_IdealNPCState) {
      ...
      if (!HasCondition(COND_NPC_FREEZE) &&
          (!m_bConditionsGathered || m_bSkippedChooseEnemy)) {
      ...
      if (HasCondition(COND_TASK_FAILED) && m_NPCState == m_IdealNPCState) {
        // Get a fail schedule if the previous schedule failed during execution
        // and the NPC is still in its ideal state. Otherwise, the NPC would
        ...
        DevWarning(2, "(%s) Schedule (%s) Failed at %d!\n",
                   ...
                   GetCurSchedule() ? GetCurSchedule()->GetName()
                                    : "GetCurSchedule() == NULL",
                   GetScheduleCurTaskIndex());
        SetSchedule(pNewSchedule);
      } else {
        // If the NPC is supposed to change state, it doesn't matter if the
        // previous schedule failed or completed. Changing state means selecting
    ...
    AI_PROFILE_SCOPE_BEGIN_(
        CAI_BaseNPC::GetSchedulingSymbols()->ScheduleIdToSymbol(
            GetCurSchedule()->GetId()));

    if (GetTaskStatus() == TASKSTATUS_NEW) {
      if (GetScheduleCurTaskIndex() == 0) {
        int globalId = GetCurSchedule()->GetId();
        int localId = GetLocalScheduleId(
            globalId);  // if localId == -1, then it came from a behavior
      ...
      const char *pszTaskName =
          (bDebugTaskNames) ? TaskName(pTask->iTask) : "ai_task";
      Assert(pTask != NULL);
      ...
      ADD_DEBUG_HISTORY(HISTORY_AI_DECISIONS,
                        UTIL_VarArgs("%s(%d):  Task: %s\n", GetDebugName(),
                                     entindex(), pszTaskName));
      ...
      // m_pTaskSR->GetStringText( pTask->iTask ) );
    }
    ...
    if (!TaskIsComplete() && GetTaskStatus() != TASKSTATUS_NEW) {
      if (TaskIsRunning() && !HasCondition(COND_TASK_FAILED) && runTask) {
        ...
        for (j = 0; j < 8; j++) {
          ...
          if (GetTaskInterrupt() == 0 || TaskIsComplete() ||
              HasCondition(COND_TASK_FAILED))
            break;

          if (ShouldUseEfficiency() &&
              ShouldStopProcessingTasks(this, Plat_MSTime() - taskTime,
                                        timeLimit)) {
        ...
        if (TaskIsRunning() && !HasCondition(COND_TASK_FAILED)) {
          if (IsCurTaskContinuousMove()) Remember(bits_MEMORY_MOVED_FROM_SPAWN);
          RunTaskOverlay();
        }
    ...
    if (!bStopProcessing &&
        ShouldStopProcessingTasks(this, Plat_MSTime() - taskTime, timeLimit))
      bStopProcessing = true;
  }

  // UNDONE: We have to do this so that we have an animation set to blend to if
  // RunTask changes the animation RunTask() will always change animations at
  // the end of a script! Don't do this twice
  MaintainActivity();
...
bool CAI_BaseNPC::FindCoverPos(CBaseEntity *pEntity, Vector *pResult) {
  ...
  if (!GetTacticalServices()->FindLateralCover(pEntity->EyePosition(), 0,
                                               pResult)) {
    if (!GetTacticalServices()->FindCoverPos(pEntity->GetAbsOrigin(),
                                             pEntity->EyePosition(), 0,
                                             CoverRadius(), pResult)) {
      return false;
    }
  }
...
bool CAI_BaseNPC::FindCoverPosInRadius(CBaseEntity *pEntity,
  ...
  if ((!GetSquad() || GetSquad()->GetFirstMember() == this) &&
    ...
    coverPos = goalPos;
  } else if (!pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos, 0,
                                              coverRadius * 0.5, &coverPos)) {
    if (!pTacticalServices->FindLateralCover(goalPos, enemyEyePos, 0,
                                             coverRadius * 0.5, 3, &coverPos)) {
      if (!pTacticalServices->FindCoverPos(goalPos, enemyPos, enemyEyePos,
                                           coverRadius * 0.5 - 0.1, coverRadius,
                                           &coverPos)) {
        pTacticalServices->FindLateralCover(goalPos, enemyEyePos, 0,
                                            coverRadius, 5, &coverPos);
      }
    }
  }

  if (coverPos == vec3_invalid) return false;
  *pResult = coverPos;
  return true;
}
...
void CAI_BaseNPC::ClearHintNode(float reuseDelay) {
  if (m_pHintNode) {
    if (m_pHintNode->IsLockedBy(this)) m_pHintNode->Unlock(reuseDelay);
    m_pHintNode = NULL;
  }
}
...
bool CAI_BaseNPC::FindCoverFromEnemy(bool bNodesOnly, float flMinDistance,
                                     float flMaxDistance) {
  ...
  if (bNodesOnly) {
    if (flMaxDistance == FLT_MAX) flMaxDistance = CoverRadius();

    if (!GetTacticalServices()->FindCoverPos(
            pEntity->GetAbsOrigin(), pEntity->EyePosition(), flMinDistance,
            flMaxDistance, &coverPos))
      return false;
  } else {
    if (!FindCoverPos(pEntity, &coverPos)) return false;
  }
...
bool CAI_BaseNPC::FindCoverFromBestSound(Vector *pCoverPos) {
  ...
  if (pBestSound) {
    ...
    DevMsg(2,
           "Attempting to find cover from best sound, but best sound not "
           "founc.\n");
  }
...
float CAI_BaseNPC::CalcReasonableFacing(bool bIgnoreOriginalFacing) {
  ...
  if (!bIgnoreOriginalFacing && !HasMemory(bits_MEMORY_MOVED_FROM_SPAWN) &&
      !HasCondition(COND_SEE_ENEMY)) {
    flReasonableYaw = m_flOriginalYaw;
  } else {
    // If I'm facing a wall, change my original yaw and try to find a good
    // direction to face.
    trace_t tr;
    Vector forward;
    QAngle angles(0, 0, 0);
    ...
    float zEye =
        GetAbsOrigin().z +
        m_vDefaultEyeOffset
            .z;  // always use standing eye so as to not screw with crouch cover
    ...
    for (int i = 0; i <= SEARCH_MAX; i++) {
      ...
      for (int j = -1; j <= 1; j += 2) {
        ...
        float curTrace;
        if ((curTrace = LineOfSightDist(forward, zEye)) > longestTrace &&
            IsValidReasonableFacing(forward, curTrace)) {
          // Take this one.
        ...
        if (i == 0 || i == SEARCH_MAX)  // if trying forwards or backwards, skip
                                        // the check of the other side...
          break;
      }
...
float CAI_BaseNPC::GetReasonableFacingDist(void) {
  if (GetTask() && GetTask()->iTask == TASK_FACE_ENEMY) {
    ...
    if (GetEnemy()) {
      float distEnemy = (GetEnemy()->GetAbsOrigin().AsVector2D() -
...
//-----------------------------------------------------------------------------
// TASK_SCRIPT_RUN_TO_TARGET / TASK_SCRIPT_WALK_TO_TARGET /
...
void CAI_BaseNPC::StartScriptMoveToTargetTask(int task) {
  ...
  if (m_hTargetEnt == NULL) {
    ...
    if ((newActivity != ACT_SCRIPT_CUSTOM_MOVE) &&
        TranslateActivity(newActivity) == ACT_INVALID) {
      ...
      if (m_hTargetEnt == NULL) {
        ...
        if (GetState() == NPC_STATE_SCRIPT &&
          ...
          if (m_ScriptArrivalActivity != AIN_DEF_ACTIVITY) {
            ...
            goal.arrivalSequence =
                LookupSequence(m_strScriptArrivalSequence.ToCStr());
          }
        }

        if (!GetNavigator()->SetGoal(goal, AIN_DISCARD_IF_FAIL)) {
          if (GetNavigator()->GetNavFailCounter() == 0) {
            // no path was built, but OnNavFailed() did something so that next
            // time it may work
...
void CAI_BaseNPC::StartTask(const Task_t *pTask) {
  ...
  switch (pTask->iTask) {
    ...
    case TASK_FIND_LOCK_HINTNODE: {
      if (!GetHintNode()) {
        SetHintNode(CAI_HintManager::FindHint(this, HINT_NONE,
                                              pTask->flTaskData, 2000));
      }
      if (GetHintNode()) {
        TaskComplete();
      } else {
      ...
      if (task == TASK_FIND_HINTNODE) break;
    }
    ...
    case TASK_STOP_MOVING:
      if ((GetNavigator()->IsGoalSet() && GetNavigator()->IsGoalActive()) ||
          GetNavType() == NAV_JUMP) {
        ...
        if (pTask->flTaskData == 1 &&
            GetNavigator()->SetGoalFromStoppingPath()) {
          DbgNavMsg(this, "Start TASK_STOP_MOVING\n");
          DbgNavMsg(this, "Initiating stopping path\n");
        } else {
          GetNavigator()->ClearGoal();
    ...
    case TASK_PLAY_HINT_ACTIVITY:
      if (GetHintNode()->HintActivityName() != NULL_STRING) {
        Activity hintActivity = (Activity)CAI_BaseNPC::GetActivityID(
            STRING(GetHintNode()->HintActivityName()));
        if (hintActivity != ACT_INVALID) {
          SetIdealActivity(
              GetHintActivity(GetHintNode()->HintType(), hintActivity));
        } else {
          int iSequence =
              LookupSequence(STRING(GetHintNode()->HintActivityName()));
    ...
    case TASK_SET_SCHEDULE:
      if (!SetSchedule(pTask->flTaskData)) TaskFail(FAIL_SCHEDULE_NOT_FOUND);
      break;

    case TASK_FIND_BACKAWAY_FROM_SAVEPOSITION: {
      if (GetEnemy() != NULL) {
        Vector backPos;
        if (!GetTacticalServices()->FindBackAwayPos(m_vSavePosition,
                                                    &backPos)) {
    ...
    case TASK_FIND_COVER_FROM_ENEMY: {
      ...
      float flMinDistance = (task == TASK_FIND_FAR_NODE_COVER_FROM_ENEMY)
                                ? pTask->flTaskData
                                : 0.0;
      float flMaxDistance = (task == TASK_FIND_NEAR_NODE_COVER_FROM_ENEMY)
                                ? pTask->flTaskData
                                : FLT_MAX;
    ...
    case TASK_FIND_COVER_FROM_ORIGIN: {
      ...
      if (GetTacticalServices()->FindCoverPos(GetLocalOrigin(), EyePosition(),
                                              0, CoverRadius(), &coverPos)) {
    ...
    case TASK_FACE_HINTNODE:
      ...
      GetMotor()->SetIdealYaw(
          CalcReasonableFacing(true));  // CalcReasonableFacing() is based on
                                        // previously set ideal yaw
      if (FacingIdeal())
        TaskComplete();
      else
        SetTurnActivity();
    ...
    case TASK_FACE_PATH: {
      if (!GetNavigator()->IsGoalActive()) {
        ...
        const float NPC_TRIVIAL_TURN =
            15;  // (Degrees). Turns this small or smaller, don't bother with a
                 // transition.
    ...
    case TASK_GET_CHASE_PATH_TO_ENEMY: {
      ...
      if ((pEnemy->GetAbsOrigin() - GetEnemyLKP()).LengthSqr() <
          Square(pTask->flTaskData)) {
    ...
    case TASK_GET_PATH_TO_INTERACTION_PARTNER: {
      if (!m_hForcedInteractionPartner ||
          IsUnreachable(m_hForcedInteractionPartner)) {
        TaskFail(FAIL_NO_ROUTE);
        return;
      }
    ...
    case TASK_GET_PATH_TO_RANGE_ENEMY_LKP_LOS: {
      if (GetEnemy()) {
        ...
        if (dist <= flRange ||
            GetNavigator()->SetVectorGoalFromTarget(GetEnemy()->GetAbsOrigin(),
                                                    dist - flRange)) {
          TaskComplete();
          break;
        }
    ...
    case TASK_GET_PATH_TO_ENEMY_LKP_LOS: {
      ...
      Vector vecEnemy = (task != TASK_GET_PATH_TO_ENEMY_LKP)
      ...
      if (!found) {
        ...
        if (GetTacticalServices()->FindLos(
                vecEnemy, vecEnemyEye, flMinRange, flMaxRange, 1.0, eFlankType,
                vecFlankRefPos, flFlankParam, &posLos)) {
          found = true;
        }
      }
    ...
    case TASK_GET_PATH_TO_GOAL: {
      AI_NavGoal_t goal(m_nStoredPathType, AIN_DEF_ACTIVITY, AIN_HULL_TOLERANCE,
                        AIN_DEF_FLAGS, m_hStoredPathTarget);
      ...
      switch ((int)pTask->flTaskData) {
        ...
        case PATH_LOS:  // A path to get LOS to our goal
        {
          ...
          Vector eyePosition = (m_hStoredPathTarget != NULL)
                                   ? m_hStoredPathTarget->EyePosition()
                                   : m_vecStoredPathGoal;

          Vector posLos;

          // See if we've found it
          if (GetTacticalServices()->FindLos(m_vecStoredPathGoal, eyePosition,
                                             flMinRange, flMaxRange, 1.0f,
                                             &posLos)) {
            goal.dest = posLos;
            foundPath = GetNavigator()->SetGoal(goal);
          } else {
        ...
        {
          CBaseEntity *pEntity =
              (m_hStoredPathTarget == NULL) ? this : m_hStoredPathTarget;
          ...
          if (GetTacticalServices()->FindLateralCover(pEntity->EyePosition(), 0,
            ...
            if (GetTacticalServices()->FindCoverPos(pEntity->GetAbsOrigin(),
              ...
              AI_NavGoal_t coverGoal(GOALTYPE_COVER, coverPos, ACT_RUN,
                                     AIN_HULL_TOLERANCE, AIN_DEF_FLAGS,
                                     m_hStoredPathTarget);
    ...
    case TASK_GET_PATH_TO_SAVEPOSITION_LOS: {
      ...
      if (GetTacticalServices()->FindLos(m_vSavePosition, m_vSavePosition,
                                         flMinRange, flMaxRange, 1.0,
                                         &posLos)) {
        GetNavigator()->SetGoal(
            AI_NavGoal_t(posLos, ACT_RUN, AIN_HULL_TOLERANCE));
      } else {
        // no coverwhatsoever.
        TaskFail(FAIL_NO_SHOOT);
      }
    ...
    case TASK_GET_PATH_TO_TARGET_WEAPON: {
      ...
      if (m_hTargetEnt == NULL) {
        ...
        // Since this weapon MAY be on a table, we find the nearest node without
        // verifying line-of-sight, since weapons on the table will not be able
        // to see nodes very nearby.
        int node = GetNavigator()->GetNetwork()->NearestNodeToPoint(
            this, m_hTargetEnt->GetAbsOrigin(), false);
        ...
        if (flDistZ > Z_LENIENCY) {
          ...
          CBaseCombatWeapon *pWeapon =
              dynamic_cast<CBaseCombatWeapon *>(m_hTargetEnt.Get());
          if (pWeapon) {
        ...
        if (flDistZ >= 16.0) {
          // The gun is higher or lower, but it's within reach. (probably on a
          // table).
          float flDistXY =
              (vecNodePos - m_hTargetEnt->GetAbsOrigin()).Length2D();

          // This means we have to stand on the nearest node and still be able
          // to reach the gun.
          if (flDistXY > XY_LENIENCY) {
            ...
            CBaseCombatWeapon *pWeapon =
                dynamic_cast<CBaseCombatWeapon *>(m_hTargetEnt.Get());
            if (pWeapon) {
              // Lock this weapon for a long time so no one else tries to get
              // it.
              pWeapon->Lock(30.0f, pWeapon);
              break;
            }
    ...
    case TASK_GET_PATH_TO_TARGET: {
      if (m_hTargetEnt == NULL) {
        ...
        AI_NavGoal_t goal(
            static_cast<const Vector &>(m_hTargetEnt->EyePosition()));
        goal.pTarget = m_hTargetEnt;
        GetNavigator()->SetGoal(goal);
      }
      break;
    ...
    case TASK_MARK_COMMAND_GOAL_POS:
      // Start watching my position to detect whether another AI process has
      // moved me from my mark.
    ...
    case TASK_GET_PATH_TO_RANDOM_NODE:  // Task argument is lenth of path to
                                        // build
    {
    ...
    case TASK_RUN_PATH: {
      // UNDONE: This is in some default AI and some NPCs can't run? -- walk
      // instead?
    ...
    case TASK_WALK_PATH: {
      bool bIsFlying = (GetMoveType() == MOVETYPE_FLY) ||
    ...
    case TASK_STRAFE_PATH: {
      ...
      // to start strafing, we have to first figure out if the target is on the
      // left side or right side
      Vector right;
      AngleVectors(GetLocalAngles(), NULL, &right, NULL);

      vec2DirToPoint =
          (GetNavigator()->GetCurWaypointPos() - GetLocalOrigin()).AsVector2D();
    ...
    case TASK_SOUND_ANGRY: {
      // sounds are complete as soon as we get here, cause we've already played
      // them.
      DevMsg(2, "SOUND\n");
      TaskComplete();
      break;
    ...
    case TASK_PUSH_SCRIPT_ARRIVAL_ACTIVITY: {
      ...
      if (iszArrivalText != NULL_STRING) {
        m_ScriptArrivalActivity =
            (Activity)GetActivityID(STRING(iszArrivalText));
    ...
    case TASK_ENABLE_SCRIPT: {
      //
      // Start waiting to play a script. Play the script's pre idle animation if
      // one is specified, otherwise just go to our default idle activity.
      //
      if (m_hCine->m_iszPreIdle != NULL_STRING) {
        m_hCine->StartSequence((CAI_BaseNPC *)this, m_hCine->m_iszPreIdle,
                               false);
        ...
        //
#ifdef HL2_EPISODIC
        // dvs: Check current activity rather than ideal activity. Since
        // scripted NPCs early out in MaintainActivity,
        //      they'll never reach their ideal activity if it's different from
        //      their current activity.
        if (GetActivity() == ACT_WALK || GetActivity() == ACT_RUN ||
            GetActivity() == ACT_WALK_AIM || GetActivity() == ACT_RUN_AIM) {
          SetActivity(ACT_IDLE);
        }
#else
    ...
    case TASK_FACE_SCRIPT: {
      if (m_hTargetEnt != NULL) {
        GetMotor()->SetIdealYaw(
            UTIL_AngleMod(m_hTargetEnt->GetLocalAngles().y));
      }

      if (m_scriptState != SCRIPT_CUSTOM_MOVE_TO_MARK) {
        SetTurnActivity();

        // dvs: HACK: MaintainActivity won't do anything while scripted, so go
        // straight there.
    ...
    case TASK_WEAPON_PICKUP: {
      ...
      if (GetTarget()) {
        CBaseCombatWeapon *pWeapon =
            dynamic_cast<CBaseCombatWeapon *>(GetTarget());
        if (pWeapon) {
    ...
    case TASK_WEAPON_CREATE: {
      if (!GetActiveWeapon() && GetTarget()) {
        ...
        if (pTargetWeapon) {
          CBaseCombatWeapon *pWeapon =
              Weapon_Create(pTargetWeapon->GetClassname());
          if (pWeapon) {
...
void CAI_BaseNPC::RunDieTask() {
  ...
  if (IsActivityFinished() && GetCycle() >= 1.0f) {
    ...
    if (!BBoxFlat()) {
      // a bit of a hack. If a corpses' bbox is positioned such that being left
      // solid so that it can be attacked will block the player on a slope or
      // stairs, the corpse is made nonsolid.
      //					SetSolid( SOLID_NOT );
      UTIL_SetSize(this, Vector(-4, -4, 0), Vector(4, 4, 1));
    } else  // !!!HACKHACK - put NPC in a thin, wide bounding box until we fix
            // the solid type/bounding volume problem
      UTIL_SetSize(this, WorldAlignMins(),
                   Vector(WorldAlignMaxs().x, WorldAlignMaxs().y,
                          WorldAlignMins().z + 1));
  }
}
...
void CAI_BaseNPC::RunAttackTask(int task) {
  ...
  // If our enemy was killed, but I'm not done animating, the last known
  // position comes back as the origin and makes the me face the world origin if
  // my attack schedule doesn't break when my enemy dies. (sjb)
  if (vecEnemyLKP != vec3_origin) {
    if ((task == TASK_RANGE_ATTACK1 || task == TASK_RELOAD) &&
      ...
      GetMotor()->SetIdealYawAndUpdate(GetMotor()->GetIdealYaw(),
                                       AI_KEEP_YAW_SPEED);
    } else {
  ...
  switch (pTask->iTask) {
    ...
    case TASK_PLAY_SEQUENCE_FACE_TARGET: {
      ...
      if (pTask->iTask == TASK_PLAY_SEQUENCE_FACE_TARGET)
        ...
        pTarget = GetEnemy();
      if (pTarget) {
        GetMotor()->SetIdealYawAndUpdate(
            pTarget->GetAbsOrigin() - GetLocalOrigin(), AI_KEEP_YAW_SPEED);
      }
      ...
      if (pTask->flTaskData == 1) {
        ...
        if (GetNavType() == NAV_JUMP) {
          if (GetFlags() & FL_ONGROUND) {
            ...
            SetNavType(
                NAV_GROUND);  // this assumes that NAV_JUMP only happens with
                              // npcs that use NAV_GROUND as base movement
          } else if (GetSmoothedVelocity().Length() >
                     0.01)  // use an EPSILON damnit!!
          {
            // wait until you land
            break;
          } else {
    ...
    case TASK_FACE_PLAYER: {
      ...
      if (pPlayer) {
        GetMotor()->SetIdealYawToTargetAndUpdate(pPlayer->GetAbsOrigin(),
                                                 AI_KEEP_YAW_SPEED);
        SetTurnActivity();
    ...
    case TASK_WAIT_PVS: {
      if (ShouldAlwaysThink() || UTIL_FindClientInPVS(edict()) ||
          (GetState() == NPC_STATE_COMBAT && GetEnemy() &&
           gpGlobals->curtime - GetEnemies()->LastTimeSeen(GetEnemy()) < 15)) {
        TaskComplete();
      }
      break;
    }
    ...
    case TASK_MOVE_TO_GOAL_RANGE: {
      ...
      if (pTarget == NULL) {
        ...
        // If we're jumping, wait until we're finished to update our goal
        // position.
        if (GetNavigator()->GetNavType() != NAV_JUMP) {
          // Re-evaluate when you think your finished, or the target has moved
          // too far
          if ((distance < pTask->flTaskData) ||
              (vecGoalPos - pTarget->GetAbsOrigin()).Length() >
                  pTask->flTaskData * 0.5) {
        ...
        // BUGBUG: this is checking linear distance (ie. through walls) and not
        // path distance or even visibility
        if (distance < pTask->flTaskData) {
          TaskComplete();
#ifndef HL2_DLL
          ...
          if (bForceRun) {
            ...
            followActivity = (distance < 190 && m_NPCState != NPC_STATE_COMBAT)
                                 ? ACT_WALK
                                 : ACT_RUN;
          }
    ...
    case TASK_MOVE_AWAY_PATH: {
      ...
      switch (GetTaskInterrupt()) {
        case 0: {
          if (IsPlayerAlly()) {
            ...
            hintCriteria.AddIncludePosition(GetAbsOrigin(),
                                            (20.0f * 12.0f));  // 20 feet max
            hintCriteria.AddExcludePosition(
                GetAbsOrigin(),
                28.0f);  // don't plant on an hint that you start on
            ...
            if (pHint) {
              ...
              if (vecGoal.DistToSqr(GetAbsOrigin()) <
                  vecGoal.DistToSqr(pPlayer->GetAbsOrigin())) {
                if (GetNavigator()->SetGoal(vecGoal)) {
                  pHint->DisableForSeconds(
                      0.1f);  // Force others to find their own.
                  TaskComplete();
                  break;
                }
              }
          ...
          if (pBestSound && pBestSound->m_hOwner &&
              pBestSound->m_hOwner->GetServerVehicle()) {
        ...
        case 2: {
          ...
          if (GetTacticalServices()->FindCoverPos(GetLocalOrigin(),
                                                  EyePosition(), 0,
                                                  CoverRadius(), &coverPos) &&
              IsValidMoveAwayDest(GetNavigator()->GetGoalPos())) {
    ...
    case TASK_WAIT_FOR_MOVEMENT: {
      bool fTimeExpired =
          (pTask->flTaskData != 0 &&
           pTask->flTaskData < gpGlobals->curtime - GetTimeTaskStarted());
    ...
    case TASK_WAIT_FOR_SCRIPT: {
      //
      // Waiting to play a script. If the script is ready, start playing the
      // sequence.
      //
      if (m_hCine && m_hCine->IsTimeToStart()) {
        ...
        if (m_hCine->m_iszEntry != NULL_STRING) {
          m_hCine->StartSequence((CAI_BaseNPC *)this, m_hCine->m_iszEntry,
                                 true);
        } else {
          m_hCine->StartSequence((CAI_BaseNPC *)this, m_hCine->m_iszPlay, true);
        }

        // StartSequence() can call CineCleanup().  If that happened, just exit
        // schedule
        if (!m_hCine) {
    ...
    case TASK_PLAY_SCRIPT: {
      ...
      if (IsSequenceFinished()) {
        ...
        if (m_hCine->FinishedActionSequence(this)) {
          // dvs: This is done in FixScriptNPCSchedule -- doing it here is too
          // early because we still
          //      need to play our post-action idle sequence, which might also
          //      require FL_FLY.
          //
          // drop to ground if this guy is only marked "fly" because of the auto
          // movement
    ...
    case TASK_PLAY_SCRIPT_POST_IDLE: {
      ...
      //
      // Playing a scripted post idle sequence. Quit early if another sequence
      // has grabbed the NPC.
      //
    ...
    case TASK_WALK_PATH_TIMED:
    case TASK_RUN_PATH_TIMED: {
      if (IsWaitFinished() || GetNavigator()->GetGoalType() == GOALTYPE_NONE) {
        TaskComplete();
      }
    } break;
    ...
    case TASK_WEAPON_PICKUP: {
      if (IsActivityFinished()) {
        CBaseCombatWeapon *pWeapon =
            dynamic_cast<CBaseCombatWeapon *>((CBaseEntity *)m_hTargetEnt);
    ...
    case TASK_FALL_TO_GROUND:
      if (GetFlags() & FL_ONGROUND) {
        ...
        if (IsWaitFinished()) {
          // After 4 seconds of trying to fall to ground, Assume that we're in a
          // bad case where the NPC isn't actually falling, and make an attempt
          // to slam the ground entity to whatever's under the NPC.
...
void CAI_BaseNPC::SetTurnActivity(void) {
  ...
  */
  ...
  if (flYD <= -80 && flYD >= -100 &&
      SelectWeightedSequence(ACT_90_RIGHT) != ACTIVITY_NOT_AVAILABLE) {
    // 90 degree right.
    Remember(bits_MEMORY_TURNING);
  ...
  if (flYD >= 80 && flYD <= 100 &&
      SelectWeightedSequence(ACT_90_LEFT) != ACTIVITY_NOT_AVAILABLE) {
    // 90 degree left.
    Remember(bits_MEMORY_TURNING);
  ...
  if (fabs(flYD) >= 160 &&
      SelectWeightedSequence(ACT_180_LEFT) != ACTIVITY_NOT_AVAILABLE) {
    Remember(bits_MEMORY_TURNING);
    SetIdealActivity(ACT_180_LEFT);
    return;
  }

  if (flYD <= -45 && SelectWeightedSequence(ACT_TURN_RIGHT) !=
                         ACTIVITY_NOT_AVAILABLE) {  // big right turn
    SetIdealActivity(ACT_TURN_RIGHT);
    return;
  }
  if (flYD >= 45 && SelectWeightedSequence(ACT_TURN_LEFT) !=
                        ACTIVITY_NOT_AVAILABLE) {  // big left turn
...
int CAI_BaseNPC::GetScriptCustomMoveSequence(void) {
  ...
  if (m_hCine != NULL) {
    ...
    if (iSequence == ACTIVITY_NOT_AVAILABLE) {
      Warning(
          "SCRIPT_CUSTOM_MOVE: %s failed scripted custom move. Has no sequence "
          "called: %s\n",
...
const Task_t *CAI_BaseNPC::GetTask(void) {
  ...
  if (!GetCurSchedule() || iScheduleIndex < 0 ||
      iScheduleIndex >= GetCurSchedule()->NumTasks())
...
bool CAI_BaseNPC::IsInterruptable() {
  if (GetState() == NPC_STATE_SCRIPT) {
    if (m_hCine) {
      if (!m_hCine->CanInterrupt()) return false;
...
int CAI_BaseNPC::SelectInteractionSchedule(void) {
  ...
  // If we have an interaction, we're the initiator. Move to our interaction
  // point.
...
int CAI_BaseNPC::SelectIdleSchedule() {
  if (m_hForcedInteractionPartner) return SelectInteractionSchedule();

  int nSched = SelectFlinchSchedule();
  if (nSched != SCHED_NONE) return nSched;

  if (HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_COMBAT) ||
      HasCondition(COND_HEAR_WORLD) || HasCondition(COND_HEAR_BULLET_IMPACT) ||
...
int CAI_BaseNPC::SelectAlertSchedule() {
  ...
  if (HasCondition(COND_ENEMY_DEAD) &&
      SelectWeightedSequence(ACT_VICTORY_DANCE) != ACTIVITY_NOT_AVAILABLE)
  ...
  if (HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_PLAYER) ||
      HasCondition(COND_HEAR_WORLD) || HasCondition(COND_HEAR_BULLET_IMPACT) ||
      HasCondition(COND_HEAR_COMBAT)) {
    return SCHED_ALERT_FACE_BESTSOUND;
  }

  if (gpGlobals->curtime - GetEnemies()->LastTimeSeen(AI_UNKNOWN_ENEMY) <
...
int CAI_BaseNPC::SelectCombatSchedule() {
  ...
  if (HasCondition(COND_NEW_ENEMY) &&
      gpGlobals->curtime - GetEnemies()->FirstTimeSeen(GetEnemy()) < 2.0) {
  ...
  if (IRelationType(GetEnemy()) == D_FR) {
    ...
    // If I've seen the enemy recently, cower. Ignore the time for unforgettable
    // enemies.
  ...
  if (!HasCondition(COND_SEE_ENEMY)) {
    ...
    // turn to face enemy
    if (!HasCondition(COND_ENEMY_OCCLUDED)) return SCHED_COMBAT_FACE;
  ...
  if (HasCondition(COND_TOO_CLOSE_TO_ATTACK)) return SCHED_BACK_AWAY_FROM_ENEMY;
  ...
  if (GetShotRegulator()->IsInRestInterval()) {
    if (HasCondition(COND_CAN_RANGE_ATTACK1)) return SCHED_COMBAT_FACE;
  }

  // we can see the enemy
  if (HasCondition(COND_CAN_RANGE_ATTACK1)) {
    if (!UseAttackSquadSlots() ||
        OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2))
      return SCHED_RANGE_ATTACK1;
    return SCHED_COMBAT_FACE;
  }

  if (HasCondition(COND_CAN_RANGE_ATTACK2)) return SCHED_RANGE_ATTACK2;

  if (HasCondition(COND_CAN_MELEE_ATTACK1)) return SCHED_MELEE_ATTACK1;

  if (HasCondition(COND_CAN_MELEE_ATTACK2)) return SCHED_MELEE_ATTACK2;

  if (HasCondition(COND_NOT_FACING_ATTACK)) return SCHED_COMBAT_FACE;

  if (!HasCondition(COND_CAN_RANGE_ATTACK1) &&
      !HasCondition(COND_CAN_MELEE_ATTACK1)) {
    // if we can see enemy but can't use either attack type, we must need to get
    // closer to enemy
    if (GetActiveWeapon()) return SCHED_MOVE_TO_WEAPON_RANGE;

    // If we have an innate attack and we're too far (or occluded) then get line
    // of sight
...
int CAI_BaseNPC::SelectScriptSchedule() {
  Assert(m_hCine != NULL);
  if (m_hCine) return SCHED_AISCRIPT;
...
void CAI_BaseNPC::PlayFlinchGesture() {
  ...
  if (iFlinchActivity != ACT_INVALID) {
    // Get the duration of the flinch and delay the next one by that (plus a bit
    // more)
...
int CAI_BaseNPC::SelectFlinchSchedule() {
  if (!HasCondition(COND_HEAVY_DAMAGE)) return SCHED_NONE;

  // If we've flinched recently, don't do it again. A gesture flinch will be
  // played instead.
  ...
  if (HaveSequenceForActivity(iFlinchActivity)) return SCHED_BIG_FLINCH;

  /*
  // Not used anymore, because gesture flinches are played instead for heavy
  damage
...
int CAI_BaseNPC::SelectSchedule(void) {
  ...
  switch (m_NPCState) {
    ...
    case NPC_STATE_IDLE:
      AssertMsgOnce(GetEnemy() == NULL,
                    "NPC has enemy but is not in combat state?");
...
int CAI_BaseNPC::SelectFailSchedule(int failedSchedule, int failedTask,
