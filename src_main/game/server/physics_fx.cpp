...
void PhysicsSplash(IPhysicsFluidController *pFluid, IPhysicsObject *pObject,
  ...
  */
  ...
  // proportional to cross-sectional area times velocity squared (fluid
  // pressure)
  float speed = rawSpeed * rawSpeed * extents[0] * extents[1] *
                (1.0f / 2500000.0f) * pObject->GetMass() * (0.01f);

  speed = std::clamp(speed, 0.0f, 50.0f);
  ...
  // int	splashes = clamp ( radius / 128.0f, 1, 2 );	//One splash for every
  // three square feet of area
  ...
  for ( i = 0; i < splashes; i++ )
  {
          ...
          point += centerPoint + axes[0] * random->RandomFloat( -1, 1 ) +
  axes[1] * random->RandomFloat( -1, 1 );
