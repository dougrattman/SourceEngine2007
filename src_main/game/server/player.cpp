...
ConVar sv_noclipduringpause("sv_noclipduringpause", "0",
                            FCVAR_REPLICATED | FCVAR_CHEAT,
                            "If cheats are enabled, then you can noclip with "
...
ConVar player_debug_print_damage("player_debug_print_damage", "0", FCVAR_CHEAT,
                                 "When true, print amount and type of all "
    ...
    if (pWeapon) {
      if (pWeapon->UsesPrimaryAmmo()) {
        ...
        if (ammoIndex != -1) {
          ...
          pPlayer->GiveAmmo(giveAmount,
                            GetAmmoDef()->GetAmmoOfIndex(ammoIndex)->pName);
        }
      }
...
static ConCommand givecurrentammo(
    "givecurrentammo", CC_GiveCurrentAmmo,
    "Give a supply of ammo for current weapon..\n", FCVAR_CHEAT);

// pl
BEGIN_SIMPLE_DATADESC(CPlayerState)
  // DEFINE_FIELD( netname, FIELD_STRING ),  // Don't stomp player name with
  // what's in save/restore
  DEFINE_FIELD(v_angle, FIELD_VECTOR), DEFINE_FIELD(deadflag, FIELD_BOOLEAN),
...
BEGIN_DATADESC(CBasePlayer)
  ...
  DEFINE_EMBEDDED(m_Local),
      ...
      // DEFINE_FIELD( m_iPlayerSound, FIELD_INTEGER ),	// Don't restore, set in
      // Precache()
      ...
      // DEFINE_FIELD( m_flgeigerRange, FIELD_FLOAT ),	// Don't restore, reset
      // in Precache() DEFINE_FIELD( m_flgeigerDelay, FIELD_FLOAT ),	// Don't
      // restore, reset in Precache() DEFINE_FIELD( m_igeigerRangePrev,
      // FIELD_FLOAT ),	// Don't restore, reset in Precache() DEFINE_FIELD(
      // m_iStepLeft, FIELD_INTEGER ), // Don't need to restore DEFINE_FIELD(
      // m_chTextureType, FIELD_CHARACTER ), // Don't need to restore
      // DEFINE_FIELD( m_surfaceProps, FIELD_INTEGER ),	// don't need to
      // restore, reset by gamemovement
      ...
      // DEFINE_FIELD( m_fGameHUDInitialized, FIELD_BOOLEAN ), // only used in
      // multiplayer games DEFINE_FIELD( m_fWeapon, FIELD_BOOLEAN ),  // Don't
      // restore, client needs reset DEFINE_FIELD( m_iUpdateTime, FIELD_INTEGER
      // ), // Don't need to restore DEFINE_FIELD( m_iClientBattery,
  ...
  // > ),
      ...
      DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetHUDVisibility",
...
inline bool ShouldRunCommandsInContext(const CCommandContext *ctx) {
  // TODO: This should be enabled at some point. If usercmds can run while
  // paused, then they can create entities which will never die and it will fill
  // up the entity list.
...
CBasePlayer::~CBasePlayer() { VPhysicsDestroyObject(); }
...
//			**pas -
//-----------------------------------------------------------------------------
void CBasePlayer::SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs,
                                  int pvssize) {
  // If we have a viewentity, we don't add the player's origin.
  if (pViewEntity) return;
...
int CBasePlayer::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  ...
  // I.e., always update the recipient player data even if it's nodraw (first
  // person mode)
  ...
  // Transmit for a short time after death and our death anim finishes so
  // ragdolls can access reliable player data. Note that if m_flDeathAnimTime is
  // never set, as long as m_lifeState is set to LIFE_DEAD after dying, this
  // test will act as if the death anim is finished.
  if (IsEffectActive(EF_NODRAW) ||
      (IsObserver() && (gpGlobals->curtime - m_flDeathTime > 0.5) &&
       (m_lifeState == LIFE_DEAD) &&
       (gpGlobals->curtime - m_flDeathAnimTime > 0.5))) {
...
bool CBasePlayer::WantsLagCompensationOnEntity(
  ...
  // multiply by 1.5 to to avoid "dead zones"  (sqrt(2) would be the exact
  // value)
  float maxDistance = 1.5 * pPlayer->MaxSpeed() * sv_maxunlag.GetFloat();

  // If the player is within this distance, lag compensate them in case they're
  // running past us.
...
void CBasePlayer::SetBonusProgress(int iBonusProgress) {
  if (!m_bPauseBonusProgress) m_iBonusProgress = iBonusProgress;
}
...
//-----------------------------------------------------------------------------
// Purpose: Draw all overlays (should be implemented in cascade by subclass to
// add
...
void CBasePlayer::DrawDebugGeometryOverlays(void) {
  ...
  if ((m_debugOverlays & OVERLAY_BUDDHA_MODE) && m_iHealth == 1) {
    ...
    NDebugOverlay::Line(eyePos + vSide + vUp, eyePos - vSide - vUp, 255, 0, 0,
                        false, 0);
    NDebugOverlay::Line(eyePos + vSide - vUp, eyePos - vSide + vUp, 255, 0, 0,
                        false, 0);
  }
...
// TraceAttack
//=========================================================
void CBasePlayer::TraceAttack(const CTakeDamageInfo &inputInfo,
  ...
  if (m_takedamage) {
    ...
    if (info.GetAttacker()) {
      ...
      if (pNPC && (pNPC->CapabilitiesGet() & bits_CAP_NO_HIT_PLAYER) &&
...
#endif
    {
      SpawnBlood(ptr->endpos, vecDir, BloodColor(),
                 info.GetDamage());  // a little surface blood.
...
void CBasePlayer::DamageEffect(float flDamage, int fDamageType) {
  if (fDamageType & DMG_CRUSH) {
    ...
    ViewPunch(QAngle(random->RandomInt(-0.1, 0.1), random->RandomInt(-0.1, 0.1),
                     random->RandomInt(-0.1, 0.1)));
...
/*
        Take some damage.
        NOTE: each call to OnTakeDamage with bitsDamageType set to a time-based
   damage type will cause the damage time countdown to be reset.  Thus the
   ongoing effects of poison, radiation etc are implemented with subsequent
   calls to OnTakeDamage using DMG_GENERIC.
*/
...
#ifdef PORTAL
  if (inputInfo.GetDamageType() & DMG_ACID) return true;
#endif

  // In commentary, ignore all damage except for falling and leeches
  if (!(inputInfo.GetDamageType() &
        (DMG_BURN | DMG_PLASMA | DMG_FALL | DMG_CRUSH)) &&
      inputInfo.GetDamageType() != DMG_GENERIC)
    return false;

  // We let DMG_CRUSH pass the check above so that we can check here for stress
  // damage. Deny the CRUSH damage if there is no attacker, or if the attacker
  // isn't a BSP model. Therefore, we're allowing any CRUSH damage done by a BSP
  // model.
  if ((inputInfo.GetDamageType() & DMG_CRUSH) &&
      (inputInfo.GetAttacker() == NULL ||
       !inputInfo.GetAttacker()->IsBSPModel()))
    return false;
...
int CBasePlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (pVehicle) {
    ...
    if (pVehicle->PassengerShouldReceiveDamage(info) == false) return 0;
  }
  ...
  {
    char dmgtype[64];
    CTakeDamageInfo::DebugGetDamageTypeString(info.GetDamageType(), dmgtype,
                                              512);
    char outputString[256];
    Q_snprintf(outputString, 256,
               "%f: Player %s at [%0.2f %0.2f %0.2f] took %f damage from %s, "
               "type %s\n",
    ...
    if (player_debug_print_damage
            .GetBool())  // if we're not in here just for the debug history
#endif
    {
      Msg(outputString);
    }
  ...
  // Armor.
  if (m_ArmorValue && !(info.GetDamageType() &
                        (DMG_FALL | DMG_DROWN | DMG_POISON |
                         DMG_RADIATION)))  // armor doesn't protect against fall
                                           // or drown damage!
  {
  ...
  // this cast to INT is critical!!! If a player ends up with 0.5 health, the
  // engine will get that as an int (zero) and think the player is dead! (this
  // will incite a clientside screentilt, etc)

  // NOTENOTE: jdw - We are now capable of retaining the mantissa of this damage
  // value and deferring its application
  ...
  // Reset damage time countdown for each type of time based damage player just
  // sustained
  for (int i = 0; i < CDMG_TIMEBASED; i++) {
    ...
    if ((info.GetDamageType() & iDamage) &&
        g_pGameRules->Damage_IsTimeBased(iDamage)) {
  ...
  // UNDONE: still need to record damage and heal messages for the following
  // types
  ...
  m_bitsDamageType |=
      bitsDamage;        // Save this so we can report it to the client
  ...
  while (fTookDamage &&
    ...
    if (bitsDamage & DMG_CLUB) {
      if (fmajor)
        SetSuitUpdate("!HEV_DMG4", false,
                      SUIT_NEXT_IN_30SEC);  // minor fracture
      bitsDamage &= ~DMG_CLUB;
      ffound = true;
    }
    if (bitsDamage & (DMG_FALL | DMG_CRUSH)) {
      if (fmajor)
        SetSuitUpdate("!HEV_DMG5", false,
                      SUIT_NEXT_IN_30SEC);  // major fracture
      else
        SetSuitUpdate("!HEV_DMG4", false,
                      SUIT_NEXT_IN_30SEC);  // minor fracture
    ...
    if (bitsDamage & DMG_SLASH) {
      if (fmajor)
        SetSuitUpdate("!HEV_DMG1", false,
                      SUIT_NEXT_IN_30SEC);  // major laceration
      else
        SetSuitUpdate("!HEV_DMG0", false,
                      SUIT_NEXT_IN_30SEC);  // minor laceration
    ...
    if (bitsDamage & DMG_SONIC) {
      if (fmajor)
        SetSuitUpdate("!HEV_DMG2", false,
                      SUIT_NEXT_IN_1MIN);  // internal bleeding
    ...
    if (bitsDamage & (DMG_POISON | DMG_PARALYZE)) {
      ...
      SetSuitUpdate("!HEV_DMG3", false,
                    SUIT_NEXT_IN_1MIN);  // blood toxins detected
      bitsDamage &= ~(DMG_POISON | DMG_PARALYZE);
      ffound = true;
    }

    if (bitsDamage & DMG_ACID) {
      SetSuitUpdate("!HEV_DET1", false,
                    SUIT_NEXT_IN_1MIN);  // hazardous chemicals detected
      bitsDamage &= ~DMG_ACID;
      ffound = true;
    }

    if (bitsDamage & DMG_NERVEGAS) {
      SetSuitUpdate("!HEV_DET0", false,
                    SUIT_NEXT_IN_1MIN);  // biohazard detected
      bitsDamage &= ~DMG_NERVEGAS;
      ffound = true;
    }

    if (bitsDamage & DMG_RADIATION) {
      SetSuitUpdate("!HEV_DET2", false,
                    SUIT_NEXT_IN_1MIN);  // radiation detected
  ...
  if (hl2_episodic.GetBool() && info.GetAttacker() &&
      !FInViewCone(info.GetAttacker())) {
  ...
  if (fTookDamage && !ftrivial && fcritical && flHealthPrev < 75) {
    ...
    if (m_iHealth < 6)
      ...
      SetSuitUpdate("!HEV_HLTH2", false,
                    SUIT_NEXT_IN_10MIN);  // health critical

    // give critical health warnings
    if (!random->RandomInt(0, 3) && flHealthPrev < 50)
      SetSuitUpdate("!HEV_DMG7", false,
                    SUIT_NEXT_IN_5MIN);  // seek medical attention
  }

  // if we're taking time based damage, warn about its continuing effects
  if (fTookDamage && g_pGameRules->Damage_IsTimeBased(info.GetDamageType()) &&
      flHealthPrev < 75) {
    if (flHealthPrev < 50) {
      ...
      SetSuitUpdate("!HEV_HLTH1", false,
                    SUIT_NEXT_IN_10MIN);  // health dropping
  }
...
void CBasePlayer::OnDamagedByExplosion(const CTakeDamageInfo &info) {
  ...
  bool ear_ringing =
      distanceFromPlayer < MIN_EAR_RINGING_DISTANCE ? true : false;
  bool shock = lastDamage >= MIN_SHOCK_AND_CONFUSION_DAMAGE;

  if (!shock && !ear_ringing) return;
...
void CBasePlayer::PackDeadPlayerItems(void) {
  ...
  int i;
  CBaseCombatWeapon
      *rgpPackWeapons[20];  // 20 hardcoded for now. How to determine exactly
  ...
  if (iWeaponRules == GR_PLR_DROP_GUN_NO && iAmmoRules == GR_PLR_DROP_AMMO_NO) {
    // nothing to pack. Remove the weapons and return. Don't call create on the
    // box!
    RemoveAllItems(true);
    return;
  }
      ...
      if (GetAmmoCount(i) > 0) {
        ...
        switch (iAmmoRules) {
          ...
          case GR_PLR_DROP_AMMO_ACTIVE:
            ...
            else if ( GetActiveWeapon() && i ==
            GetActiveWeapon()->m_iSecondaryAmmoType )
            {
  ...
  RemoveAllItems(
      true);  // now strip off everything that wasn't handled by the code above.
}
...
bool CBasePlayer::IsDead() const { return m_lifeState == LIFE_DEAD; }
...
int CBasePlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (info.GetInflictor() && (GetMoveType() == MOVETYPE_WALK) &&
    ...
    Vector force =
        vecDir * -DamageForce(WorldAlignSize(), info.GetBaseDamage());
    if (force.z > 250.0f) {
      force.z = 250.0f;
    }
  ...
  if (attacker->IsNPC()) {
    CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(), 512, 0.5,
                           this);  //<<TODO>>//magic number
  }
...
void CBasePlayer::Event_Killed(const CTakeDamageInfo &info) {
  ...
  // until they respawn
  pSound =
      CSoundEnt::SoundPointerForIndex(CSoundEnt::ClientSoundIndex(edict()));
  {
    if (pSound) {
      pSound->Reset();
    }
...
void CBasePlayer::SetAnimation(PLAYER_ANIM playerAnim) {
  ...
  // This could stand to be redone. Why is playerAnim abstracted from activity?
  // (sjb)
  if (playerAnim == PLAYER_JUMP) {
    ...
    if (m_Activity == ACT_HOVER || m_Activity == ACT_SWIM ||
        m_Activity == ACT_HOP || m_Activity == ACT_LEAP ||
  ...
  if (idealActivity == ACT_RANGE_ATTACK1) {
    ...
    // Tracker 24588:  In single player when firing own weapon this causes eye
    // and punchangle to jitter
    // if (!SequenceLoops())
    //{
    ...
    if (GetActivity() == idealActivity) return;
  ...
  if (GetWaterLevel() != WL_Eyes || CanBreatheUnderwater()) {
        ...
        !(GetFlags() & FL_GODMODE))  // drown!
    {
      if (m_PainFinished < gpGlobals->curtime) {
        ...
        OnTakeDamage(CTakeDamageInfo(GetContainingEntity(INDEXENT(0)),
                                     GetContainingEntity(INDEXENT(0)),
                                     m_nDrownDmgRate, DMG_DROWN));
...
float CBasePlayer::GetWaterJumpTime() const { return m_flWaterJumpTime; }

void CBasePlayer::SetWaterJumpTime(float flWaterJumpTime) {
  m_flWaterJumpTime = flWaterJumpTime;
}

float CBasePlayer::GetSwimSoundTime(void) const { return m_flSwimSoundTime; }

void CBasePlayer::SetSwimSoundTime(float flSwimSoundTime) {
  m_flSwimSoundTime = flSwimSoundTime;
}

void CBasePlayer::ShowViewPortPanel(const char *name, bool bShow,
                                    KeyValues *data) {
...
void CBasePlayer::PlayerDeathThink(void) {
  ...
  if (HasWeapons()) {
    // we drop the guns here because weapons that have an area effect and can
    // kill their user will sometimes crash coming back from
    // CBasePlayer::Killed() if they kill their owner because the player class
    // sometimes is freed. It's safer to manipulate the weapons once we know we
    // aren't calling into any of their code anymore through the player pointer.
    PackDeadPlayerItems();
  }
  ...
  // if the player has been dead for one second longer than allowed by
  // forcerespawn, forcerespawn isn't on. Send the player off to an intermission
  // camera until they choose to respawn.
  ...
  // wait for any button down,  or mp_forcerespawn is set and the respawn time
  // is up
  if (!fAnyButtonDown &&
      !(g_pGameRules->IsMultiplayer() && forcerespawn.GetInt() > 0 &&
        (gpGlobals->curtime > (m_flDeathTime + 5))))
    return;
...
at their corpse trace_t tr;

                CreateCorpse();

                UTIL_TraceLine( GetAbsOrigin(), GetAbsOrigin() + Vector( 0, 0,
128 ), MASK_PLAYERSOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr ); QAngle
angles; VectorAngles( GetAbsOrigin() - tr.endpos, angles ); StartObserverMode(
tr.endpos, angles ); return;
        }
} */
...
void CBasePlayer::StopObserverMode() {
  ...
  if (m_iObserverMode == OBS_MODE_NONE) return;
...
bool CBasePlayer::StartObserverMode(int mode) {
  ...
  if (GetActiveWeapon()) GetActiveWeapon()->Holster();
  ...
  m_iHealth = 1;
  m_lifeState =
      LIFE_DEAD;  // Can't be dead, otherwise movement doesn't work right.
  m_flDeathAnimTime = gpGlobals->curtime;
  pl.deadflag = true;
...
bool CBasePlayer::SetObserverMode(int mode) {
  if (mode < OBS_MODE_NONE || mode >= NUM_OBSERVER_MODES) return false;
...
int CBasePlayer::GetObserverMode() { return m_iObserverMode; }

void CBasePlayer::ForceObserverMode(int mode) {
  int tempMode = OBS_MODE_ROAMING;

  if (m_iObserverMode == mode) return;
...
void CBasePlayer::CheckObserverSettings() {
  ...
  if (m_iObserverMode == OBS_MODE_IN_EYE || m_iObserverMode == OBS_MODE_CHASE) {
    ...
    // Update the fog.
    if (target) {
      if (target->m_Local.m_PlayerFog.m_hCtrl.Get() !=
          m_Local.m_PlayerFog.m_hCtrl.Get()) {
        m_Local.m_PlayerFog.m_hCtrl.Set(
            target->m_Local.m_PlayerFog.m_hCtrl.Get());
      }
    }
  }
}
...
bool CBasePlayer::StartReplayMode(float fDelay, float fDuration, int iEntity) {
  if ((sv_maxreplay == NULL) || (sv_maxreplay->GetFloat() <= 0)) return false;
...
int CBasePlayer::GetDelayTicks() {
  if (m_fReplayEnd > gpGlobals->curtime) {
    ...
    if (m_fDelay > 0.0f) StopReplayMode();
...
void CBasePlayer::ObserverUse(bool bIsPressed) {
#ifndef _XBOX
  if (!HLTVDirector()->IsActive()) return;
  ...
  trace_t	tr;
  UTIL_TraceLine( start, end, MASK_PLAYERSOLID, this,
  ...
  UTIL_TraceRay( ray, MASK_PLAYERSOLID, this, COLLISION_GROUP_PLAYER_MOVEMENT,
  &tr );
...
bool CBasePlayer::SetObserverTarget(CBaseEntity *target) {
  if (!IsValidObserverTarget(target)) return false;
  ...
  if (m_iObserverMode == OBS_MODE_ROAMING) {
    ...
    trace_t tr;
    UTIL_TraceRay(ray, MASK_PLAYERSOLID, target,
                  COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
...
bool CBasePlayer::IsValidObserverTarget(CBaseEntity *target) {
  ...
  if (player == this) return false;  // We can't observe ourselves.

  if (player->IsEffectActive(EF_NODRAW))  // don't watch invisible players
    return false;

  if (player->m_lifeState ==
      LIFE_RESPAWNABLE)  // target is dead, waiting for respawn
    return false;

  if (player->m_lifeState == LIFE_DEAD || player->m_lifeState == LIFE_DYING) {
    if ((player->m_flDeathTime + DEATH_ANIMATION_TIME) < gpGlobals->curtime) {
      return false;  // allow watching until 3 seconds after death to see death
                     // animation
    }
  }
  ...
  if (GetTeamNumber() != TEAM_SPECTATOR) {
    switch (mp_forcecamera.GetInt()) {
      case OBS_ALLOW_ALL:
        break;
      case OBS_ALLOW_TEAM:
      case OBS_ALLOW_TEAM_ALL:
        if (GetTeamNumber() != target->GetTeamNumber()) return false;
        break;
      case OBS_ALLOW_NONE:
        return false;
    }
...
//-----------------------------------------------------------------------------
bool CBasePlayer::IsUseableEntity(CBaseEntity *pEntity,
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBasePlayer::CanPickupObject(CBaseEntity *pObject, float massLimit,
                                  float sizeLimit) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (pList[i]->GetGameFlags() & FVPHYSICS_NO_PLAYER_PICKUP) return false;
    if (pList[i]->IsHinged()) return false;
  }
...
// ID's player as such.
//
Class_T CBasePlayer::Classify(void) { return CLASS_PLAYER; }
...
CCommandContext *CBasePlayer::GetCommandContext(int index) {
  if (index < 0 || index >= m_CommandContext.Count()) return NULL;
...
// Purpose:
//-----------------------------------------------------------------------------
void CBasePlayer::RemoveAllCommandContexts() { m_CommandContext.RemoveAll(); }

//-----------------------------------------------------------------------------
// Purpose: Removes all existing contexts, but leaves the last one around ( or
// creates it if it doesn't exist -- which would be a bug )
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Replaces the first nCommands CUserCmds in the context with the ones
// passed in -- this is used to help meter out CUserCmds over the number of
// simulation ticks on the server
//-----------------------------------------------------------------------------
void CBasePlayer::ReplaceContextCommands(CCommandContext *ctx,
...
int CBasePlayer::DetermineSimulationTicks(void) {
  ...
  // clock as needed
  for (context_number = 0; context_number < command_context_count;
       context_number++) {
...
static ConVar sv_clockcorrection_msecs(
    "sv_clockcorrection_msecs", "60", 0,
    "The server tries to keep each player's m_nTickBase withing this many "
    "msecs of the server absolute tickcount");
static ConVar sv_playerperfhistorycount(
    "sv_playerperfhistorycount", "20", 0,
    "Number of samples to maintain in player perf history", true, 1.0f, true,
    128.0);

//-----------------------------------------------------------------------------
// Purpose: Based upon amount of time in simulation time, adjust m_nTickBase so
// that
//  we just end at the end of the current frame (so the player is basically on
...
void CBasePlayer::AdjustPlayerTimeBase(int simulation_ticks) {
  ...
  // Start in the past so that we get to the sv.time that we'll hit at the end
  // of the
  //  frame, just as we process the final command

  if (gpGlobals->maxClients == 1) {
    // set TickBase so that player simulation tick matches gpGlobals->tickcount
    // after all commands have been executed
    m_nTickBase =
        gpGlobals->tickcount - simulation_ticks + gpGlobals->simTicksThisFrame;
  } else  // multiplayer
  {
    ...
    if (nEstimatedFinalTick > too_fast_limit ||
      ...
      int nCorrectedTick =
          nIdealFinalTick - simulation_ticks + gpGlobals->simTicksThisFrame;
  ...
  if (pi) {
    pi->m_flFinalSimulationTime = TICKS_TO_TIME(m_nTickBase + simulation_ticks +
...
void CBasePlayer::PhysicsSimulate(void) {
  ...
  // If some time will elapse, make sure our clock (m_nTickBase) starts at the
  // correct time
  ...
  for (int context_number = 0; context_number < command_context_count;
       context_number++) {
    // Get oldest ( newer are added to tail )
    CCommandContext *ctx = GetCommandContext(context_number);
    if (!ShouldRunCommandsInContext(ctx)) continue;

    if (!ctx->cmds.Count()) continue;
    ...
    // Now run any new command(s).  Go backward because the most recent command
    // is at index 0.
  ...
  // gpGlobals->simTicksThisFrame == number of ticks remaining to be run, so we
  ...
  if (gpGlobals->simTicksThisFrame >= commandLimit &&
    ...
    if (commandsToRollOver > 0) {
      ...
      ReplaceContextCommands(ctx, &vecAvailCommands[commandsToRun],
                             commandsToRollOver);
    } else {
      // Clear all contexts
      RemoveAllCommandContexts();
  ...
  if (commandsToRun > 0) {
    ...
    for (int i = 0; i < commandsToRun; ++i) {
      ...
      if (m_pPhysicsController) {
        ...
        // If simulating at 2 * TICK_INTERVAL, add an extra TICK_INTERVAL to
        // position arrival computation
        UpdateVPhysicsPosition(m_vNewVPhysicsPosition, m_vNewVPhysicsVelocity,
                               vphysicsArrivalTime);
...
//-----------------------------------------------------------------------------
// Purpose: This will force usercmd processing to actually consume commands even
// if the global tick counter isn't incrementing
//-----------------------------------------------------------------------------
void CBasePlayer::ForceSimulation() { m_nSimulationTick = -1; }
...
void CBasePlayer::ProcessUsercmds(CUserCmd *cmds, int numcmds, int totalcmds,
  ...
  ctx->numcmds = numcmds;
  ctx->totalcmds = totalcmds, ctx->dropped_packets = dropped_packets;
  ctx->paused = paused;
  ...
  if (ctx->paused) {
    ...
    // If no clipping and cheats enabled and sv_noclipduringpause enabled, then
    // don't zero out movement part of CUserCmd
    if (GetMoveType() == MOVETYPE_NOCLIP && sv_cheats->GetBool() &&
...
void CBasePlayer::DumpPerfToRecipient(CBasePlayer *pRecipient,
                                      int nMaxRecords) {
  ...
  for (int i = m_vecPlayerSimInfo.Tail();
       i != m_vecPlayerSimInfo.InvalidIndex();
       i = m_vecPlayerSimInfo.Previous(i)) {
    ...
    char line[128];
    int len =
        Q_snprintf(line, sizeof(line), "%.3f %d %d %.3f %.3f %.3f [vel %.2f]\n",
                   pi->m_flTime, pi->m_nNumCmds, pi->m_nTicksCorrected,
                   pi->m_flFinalSimulationTime, pi->m_flGameSimulationTime,
                   pi->m_flServerFrameTime, vel);
    ...
    ++nDumped;
    if (nMaxRecords != -1 && nDumped >= nMaxRecords) break;
  ...
  for (int i = m_vecPlayerCmdInfo.Tail();
       i != m_vecPlayerCmdInfo.InvalidIndex();
       i = m_vecPlayerCmdInfo.Previous(i)) {
    const CPlayerCmdInfo *pi = &m_vecPlayerCmdInfo[i];

    char line[128];
    int len = Q_snprintf(line, sizeof(line), "%.3f %d %d\n", pi->m_flTime,
                         pi->m_nNumCmds, pi->m_nDroppedPackets);
...
void CBasePlayer::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper) {
  ...
  // Handle FL_FROZEN.
  // Prevent player moving for some seconds after New Game, so that they pick up
  // everything
  if (GetFlags() & FL_FROZEN ||
      (developer.GetInt() == 0 && gpGlobals->eLoadType == MapLoad_NewGame &&
    ...
    if (GetToggledDuckState()) {
      // If this is set, we've altered our menu options and need to debounce the
      // duck
...
// Purpose: Strips off IN_xxx flags from the player's input
//-----------------------------------------------------------------------------
void CBasePlayer::ForceButtons(int nButtons) { m_afButtonForced |= nButtons; }
...
void CBasePlayer::HandleFuncTrain(void) {
  ...
  if (pTrain) {
    if (!(pTrain->ObjectCaps() & FCAP_DIRECTIONAL_USE)) pTrain = NULL;
  }
  ...
  if (!pTrain) {
    if (GetActiveWeapon()->ObjectCaps() & FCAP_DIRECTIONAL_USE) {
      ...
      UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -38),
                     MASK_PLAYERSOLID_BRUSHONLY, this, COLLISION_GROUP_NONE,
                     &trainTrace);
  ...
  if (vel) {
    m_iTrain = TrainSpeed(pTrain->m_flSpeed,
...
void CBasePlayer::PreThink(void) {
  if (g_fGameOver || m_iPlayerLocked) return;  // intermission or finale
  ...
  // checks if new client data (for HUD and view control) needs to be sent to
  // the client
  UpdateClientData();
  ...
  if ((m_nButtons & IN_DUCK) || (GetFlags() & FL_DUCKING) ||
  ...
  if (area && area != m_lastNavArea) {
    ...
    if (area->GetPlace() != UNDEFINED_PLACE) {
      ...
      if (placeName && *placeName) {
        Q_strncpy(m_szLastPlaceName.GetForModify(), placeName,
  ...
  // StudioFrameAdvance( );//!!!HACKHACK!!! Can't be hit by traceline when not
  // animating?
}
...
/* Time based Damage works as follows:
        1) There are several types of timebased damage:
                ...
                #define DMG_PARALYZE		(1 << 14)	// slows
   affected creature down #define DMG_NERVEGAS		(1 << 15)	// nerve
   toxins, very bad #define DMG_POISON			(1 << 16)	// blood
   poisioning #define DMG_RADIATION		(1 << 17)	// radiation
   exposure #define DMG_DROWNRECOVER	(1 << 18)	// drown recovery
                #define DMG_ACID			(1 << 19)	// toxic
   chemicals or acid burns #define DMG_SLOWBURN		(1 << 20)	// in an
   oven

        2) A new hit inflicting tbd restarts the tbd counter - each NPC has an
   8bit counter, per damage type. The counter is decremented every second, so
   the maximum time an effect will last is 255/60 = 4.25 minutes.  Of course,
   staying within the radius of a damaging effect like fire, nervegas, radiation
   will continually reset the counter to max.

        3) Every second that a tbd counter is running, the player takes damage.
   The damage is determined by the type of tdb. Paralyze		- 1/2
   movement rate, 30 second duration.
                        Nervegas		- 5 points per second, 16 second
   duration = 80 points max dose.
                        Poison			- 2 points per second, 25 second
   duration = 50 points max dose. Radiation		- 1 point per second, 50
   second duration = 50 points max dose. Drown			- 5 points per
   second, 2 second duration. Acid/Chemical	- 5 points per second, 10 second
   duration = 50 points max.
                        Burn			- 10 points per second, 2 second
   duration.
                        Freeze			- 3 points per second, 10 second
   duration = 30 points max.

        4) Certain actions or countermeasures counteract the damaging effects of
   tbds:

                Armor/Heater/Cooler - Chemical(acid),burn, freeze all do damage
   to armor power, then to body
                                                        - recharged by suit
   recharger
                Air In Lungs		- drowning damage is done to air in
   lungs first, then to body
                                                        - recharged by poking
   head out of water
                                                        - 10 seconds if swiming
   fast Air In SCUBA		- drowning damage is done to air in tanks first,
   then to body
                                                        - 2 minutes in tanks.
   Need new tank once empty. Radiation Syringe	- Each syringe full provides
   protection vs one radiation dosage Antitoxin Syringe	- Each syringe full
   provides protection vs one poisoning (nervegas or poison).
...
void CBasePlayer::CheckTimeBasedDamage() {
  ...
  if (abs(gpGlobals->curtime - m_tbdPrev) < 2.0) return;
  ...
  for (i = 0; i < CDMG_TIMEBASED; i++) {
    ...
    // This is kind of hacky but necessary until we setup DamageType as an enum.
    int iDamage = (DMG_PARALYZE << i);
    if (!g_pGameRules->Damage_IsTimeBased(iDamage)) continue;
...
The player gets the suit shortly after getting off the train in C1A0 and it
stays with him for the entire game.

Protection

        Heat/Cold
                When the player enters a hot/cold area, the heating/cooling
indicator on the suit will come on and the battery will drain while the player
stays in the area. After the battery is dead, the player starts to take damage.
                This feature is built into the suit and is automatically
engaged. Radiation Syringe This will cause the player to be immune from the
effects of radiation for N seconds. Single use item. Anti-Toxin Syringe This
will cure the player from being poisoned. Single use item. Health Small (1st aid
kits, food, etc.) Large (boxes on walls) Armor The armor works using energy to
create a protective field that deflects a percentage of damage projectile and
explosive attacks. After the armor has been deployed, it will attempt to
recharge itself to full capacity with the energy reserves from the battery. It
takes the armor N seconds to fully charge.
...
x	Geiger counter
                ...
                alerts player to dangerous levels of radiation. This is not
visible when radiation levels are normal. x	Poison Armor Displays the
current level of armor.

Augmentation

        Reanimation (w/adrenaline)
                Causes the player to come back to life after he has been dead
for 3 seconds. Will not work if player was gibbed. Single use. Long Jump Used by
...
void CBasePlayer::UpdateGeigerCounter(void) {
  ...
  if (gpGlobals->curtime < m_flgeigerDelay) return;
...
void CBasePlayer::CheckSuitUpdate() {
  ...
  if (gpGlobals->curtime >= m_flSuitUpdate && m_flSuitUpdate > 0) {
    ...
    for (i = 0; i < CSUITPLAYLIST; i++) {
      if ((isentence = m_rgSuitPlayList[isearch]) != 0) break;

      if (++isearch == CSUITPLAYLIST) isearch = 0;
    }
    ...
    if (isentence) {
      ...
      if (isentence > 0) {
        ...
        char sentence[512];
        Q_snprintf(sentence, sizeof(sentence), "!%s",
                   engine->SentenceNameFromIndex(isentence));
...
void CBasePlayer::SetSuitUpdate(char *name, int fgroup, int iNoRepeatTime) {
  ...
  if (g_pGameRules->IsMultiplayer()) {
    // due to static channel design, etc. We don't play HEV sounds in
    // multiplayer right now.
    return;
  }
  ...
  if (!name) {
    for (i = 0; i < CSUITPLAYLIST; i++) m_rgSuitPlayList[i] = 0;
    return;
  }
  // get sentence or group number
  if (!fgroup) {
    isentence =
        SENTENCEG_Lookup(name);  // Lookup sentence index (not group) by name
    if (isentence < 0) return;
  } else
  ...
  for (i = 0; i < CSUITNOREPEAT; i++) {
    ...
    if (!m_rgiSuitNoRepeat[i]) iempty = i;
  }
  ...
  if (iNoRepeatTime) {
    if (iempty < 0)
      iempty = random->RandomInt(
          0, CSUITNOREPEAT - 1);  // pick random slot to take over
  ...
  if (m_iSuitPlayNext == CSUITPLAYLIST) m_iSuitPlayNext = 0;
...
void CBasePlayer::UpdatePlayerSound(void) {
  ...
  if (GetFlags() & FL_ONGROUND) {
    ...
    // NOTE: 512 units is a pretty large radius for a sound made by the player's
    // body. then again, I think some materials are pretty loud.
    if (iBodyVolume > 512) {
      iBodyVolume = 512;
    }
  } else {
  ...
  // if target volume is greater than the player sound's current volume, we
  // paste the new volume in immediately. If target is less than the current
  // volume, current volume is not set immediately to the lower volume, rather
  // works itself towards target volume over time. This gives NPCs a much better
  // chance to hear a sound, especially if they don't listen every frame.
  ...
  // Below are a couple of useful little bits that make it easier to visualize
  // just how much noise the player is making.
  // Vector forward = UTIL_YawToVector( pl.v_angle.y );
  // UTIL_Sparks( GetAbsOrigin() + forward * iVolume );
  // Msg( "%d/%d\n", iVolume, m_iTargetVolume );
}

// This is a glorious hack to find free space when you've crouched into some
// solid space Our crouching collisions do not work correctly for some reason
// and this is easier than fixing the problem :(
void FixPlayerCrouchStuck(CBasePlayer *pPlayer) {
  ...
  for (i = 0; i < 18; i++) {
    UTIL_TraceHull(pPlayer->GetAbsOrigin(), pPlayer->GetAbsOrigin(),
                   ...
                   pPlayer, COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
    if (trace.startsolid) {
...
// UNDONE: Look and see if the ground entity is in hierarchy with a
// MOVETYPE_VPHYSICS? Behavior in that case is not as good currently when the
// parent is rideable
bool CBasePlayer::IsRideablePhysics(IPhysicsObject *pPhysics) {
  if (pPhysics) {
    if (pPhysics->GetMass() > (VPhysicsGetObject()->GetMass() * 2)) return true;
  }
...
IPhysicsObject *CBasePlayer::GetGroundVPhysics() {
  ...
  if (pGroundEntity && pGroundEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
    ...
    if (pPhysGround && pPhysGround->IsMoveable()) return pPhysGround;
  }
  return NULL;
}
...
void CBasePlayer::PostThink() {
  m_vecSmoothedVelocity = m_vecSmoothedVelocity * SMOOTHING_FACTOR +
                          GetAbsVelocity() * (1 - SMOOTHING_FACTOR);
  ...
  if (!g_fGameOver && !m_iPlayerLocked) {
    if (IsAlive()) {
      ...
      if (m_hUseEntity != NULL) {
        // if they've moved too far from the gun, or deployed another weapon,
        // unuse the gun
        if (m_hUseEntity->OnControls(this) &&
            (!GetActiveWeapon() ||
             GetActiveWeapon()->IsEffectActive(EF_NODRAW) ||
      ...
      if (GetFlags() & FL_ONGROUND) {
        if (m_Local.m_flFallVelocity > 64 && !g_pGameRules->IsMultiplayer()) {
          CSoundEnt::InsertSound(SOUND_PLAYER, GetAbsOrigin(),
                                 m_Local.m_flFallVelocity, 0.2, this);
...
void CBasePlayer::Touch(CBaseEntity *pOther) {
  if (pOther == GetGroundEntity()) return;

  if (pOther->GetMoveType() != MOVETYPE_VPHYSICS ||
      pOther->GetSolid() != SOLID_VPHYSICS ||
      (pOther->GetSolidFlags() & FSOLID_TRIGGER))
    return;

  IPhysicsObject *pPhys = pOther->VPhysicsGetObject();
  if (!pPhys || !pPhys->IsMoveable()) return;
...
void CBasePlayer::PostThinkVPhysics(void) {
  ...
  if (!m_pPhysicsController) return;

  Vector newPosition = GetAbsOrigin();
  float frametime = gpGlobals->frametime;
  if (frametime <= 0 || frametime > 0.1f) frametime = 0.1f;
  ...
  if (!(TouchedPhysics() || pPhysGround)) {
    float maxSpeed =
        m_flMaxspeed > 0.0f ? m_flMaxspeed : sv_maxspeed.GetFloat();
    g_pMoveData->m_outWishVel.Init(maxSpeed, maxSpeed, maxSpeed);
  }

  // teleport the physics object up by stepheight (game code does this - reflect
  // in the physics)
  if (g_pMoveData->m_outStepHeight > 0.1f) {
    if (g_pMoveData->m_outStepHeight > 4.0f) {
      ...
      trace_t trace;
      UTIL_TraceEntity(this, position, end, MASK_PLAYERSOLID, this,
...
void CBasePlayer::UpdateVPhysicsPosition(const Vector &position,
                                         const Vector &velocity,
                                         float secondsToArrival) {
  bool onground = (GetFlags() & FL_ONGROUND) ? true : false;
  IPhysicsObject *pPhysGround = GetGroundVPhysics();

  // if the object is much heavier than the player, treat it as a local
  // coordinate system the player controller will solve movement differently in
  // this case.
  if (!IsRideablePhysics(pPhysGround)) {
    pPhysGround = NULL;
  }

  m_pPhysicsController->Update(position, velocity, secondsToArrival, onground,
                               pPhysGround);
}
      ...
      if (pSpot) {
        ...
        if (g_pGameRules->IsSpawnPointValid(pSpot, this)) {
          if (pSpot->GetLocalOrigin() == vec3_origin) {
            pSpot =
                gEntList.FindEntityByClassname(pSpot, "info_player_deathmatch");
            continue;
          }
    ...
    if (pSpot) {
      ...
      for (CEntitySphereQuery sphere(pSpot->GetAbsOrigin(), 128);
        ...
        if (ent->IsPlayer() && !(ent->edict() == player))
          ent->TakeDamage(CTakeDamageInfo(GetContainingEntity(INDEXENT(0)),
                                          GetContainingEntity(INDEXENT(0)), 300,
                                          DMG_GENERIC));
      }
      goto ReturnSpot;
    }
...
void CBasePlayer::Spawn(void) {
  ...
  m_flgeigerDelay = gpGlobals->curtime +
                    2.0;  // wait a few seconds until user-defined message
                          // registrations are recieved by all clients

  m_flFieldOfView = 0.766;  // some NPCs use this to determine whether or not
                            // the player is looking at them.

  m_vecAdditionalPVSOrigin = vec3_origin;
  m_vecCameraPVSOrigin = vec3_origin;

  if (!m_fGameHUDInitialized) g_pGameRules->SetDefaultPlayerTeam(this);

  g_pGameRules->GetPlayerSpawnSpot(this);

  m_Local.m_bDucked = false;  // This will persist over round restart if you
                              // hold duck otherwise.
...
int CBasePlayer::Save(ISave &save) {
  if (!BaseClass::Save(save)) return 0;
...
class CPlayerRestoreHelper {
 public:
  const Vector &GetAbsOrigin(CBaseEntity *pent) { return pent->m_vecAbsOrigin; }
...
int CBasePlayer::Restore(IRestore &restore) {
  ...
  // recalculated by a call to GetAbsOrigin because hierarchy isn't fully
  // restored yet, so we use this backdoor to get at the private data in
  // CBaseEntity.
...
void CBasePlayer::SetArmorValue(int value) { m_ArmorValue = value; }

void CBasePlayer::IncrementArmorValue(int nCount, int nMaxValue) {
  m_ArmorValue += nCount;
  if (nMaxValue > 0) {
    if (m_ArmorValue > nMaxValue) m_ArmorValue = nMaxValue;
  }
}
...
void CBasePlayer::NotifyNearbyRadiationSource(float flRange) {
  ...
  if (m_flgeigerRange >= flRange) m_flgeigerRange = flRange;
}
...
void CBasePlayer::CommitSuicide(bool bExplode /*= false*/,
  ...
  if (m_fNextSuicideTime > gpGlobals->curtime && !bForce) return;

  // don't let them suicide for 5 seconds after suiciding
  m_fNextSuicideTime = gpGlobals->curtime + 5;

  int fDamage = DMG_PREVENT_PHYSICS_FORCE |
                (bExplode ? (DMG_BLAST | DMG_ALWAYSGIB) : DMG_NEVERGIB);

  // have the player kill themself
  m_iHealth = 0;
  Event_Killed(
      CTakeDamageInfo(this, this, 0, fDamage, m_iSuicideCustomKillFlags));
  Event_Dying();
  m_iSuicideCustomKillFlags = 0;
}

// Suicide with style...
void CBasePlayer::CommitSuicide(const Vector &vecForce,
                                bool bExplode /*= false*/,
                                bool bForce /*= false*/) {
  MDLCACHE_CRITICAL_SECTION();
...
bool CBasePlayer::CanEnterVehicle(IServerVehicle *pVehicle, int nRole) {
  ...
  if (pVehicle->GetPassenger(nRole)) return false;
  ...
  if (pVehicle->IsPassengerUsingStandardWeapons(nRole) == false) {
    ...
    if ((pWeapon != NULL) && (pWeapon->CanHolster() == false)) return false;
  }
...
bool CBasePlayer::GetInVehicle(IServerVehicle *pVehicle, int nRole) {
  ...
  if (CanEnterVehicle(pVehicle, nRole) == false) return false;
  ...
  // This is a hack to fixup the player's stats since they really didn't "cheat"
  // and enter noclip from the console
...
//-----------------------------------------------------------------------------
void CBasePlayer::LeaveVehicle(const Vector &vecExitPoint,
                               const QAngle &vecExitAngles) {
  if (NULL == m_hVehicle.Get()) return;
...
CBaseEntity *CBasePlayer::GiveNamedItem(const char *pszName, int iSubType) {
  ...
  CBaseCombatWeapon *pWeapon =
      dynamic_cast<CBaseCombatWeapon *>((CBaseEntity *)pent);
  if (pWeapon) {
...
CBaseEntity *FindEntityClassForward(CBasePlayer *pMe, char *classname) {
  ...
  UTIL_TraceLine(pMe->EyePosition(),
                 pMe->EyePosition() + forward * MAX_COORD_RANGE, MASK_SOLID,
                 pMe, COLLISION_GROUP_NONE, &tr);
...
CBaseEntity *FindEntityForward(CBasePlayer *pMe, bool fHull) {
  if (pMe) {
    ...
    pMe->EyeVectors(&forward);
    UTIL_TraceLine(pMe->EyePosition(),
                   pMe->EyePosition() + forward * MAX_COORD_RANGE, mask, pMe,
...
CBaseEntity *FindPickerEntityClass(CBasePlayer *pPlayer, char *classname) {
  ...
  if (!pEntity) {
    ...
    pEntity =
        gEntList.FindEntityClassNearestFacing(origin, forward, 0.95, classname);
  }
  return pEntity;
}
...
CAI_Node *FindPickerAINode(CBasePlayer *pPlayer, NodeType_e nNodeType) {
  ...
  return g_pAINetworkManager->GetEditOps()->FindAINodeNearestFacing(
      origin, forward, 0.90, nNodeType);
}
...
CAI_Link *FindPickerAILink(CBasePlayer *pPlayer) {
  ...
  pPlayer->EyeVectors(&forward);
  origin = pPlayer->EyePosition();
  return g_pAINetworkManager->GetEditOps()->FindAILinkNearestFacing(
      origin, forward, 0.90);
}
...
When recording a demo, we need to have the server tell us the entire client
state so that the client side .dll can behave correctly. Reset stuff so that the
  ...
  switch (iImpulse) {
    ...
    case 201:  // paint decal
...

      {
        Vector forward;
        EyeVectors(&forward);
        UTIL_TraceLine(EyePosition(), EyePosition() + forward * 128,
...
static void CreateJalopy(CBasePlayer *pPlayer) {
  ...
  if (pJeep) {
    Vector vecOrigin =
        pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0, 0, 64);
...
static ConCommand ch_createjalopy("ch_createjalopy", CC_CH_CreateJalopy,
...
static ConCommand ch_createjeep("ch_createjeep", CC_CH_CreateJeep,
...
static void CreateAirboat(CBasePlayer *pPlayer) {
  ...
  AngleVectors(pPlayer->EyeAngles(), &vecForward);
  CBaseEntity *pJeep =
      (CBaseEntity *)CreateEntityByName("prop_vehicle_airboat");
  if (pJeep) {
    Vector vecOrigin =
        pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0, 0, 64);
    QAngle vecAngles(0, pPlayer->GetAbsAngles().y - 90, 0);
...
static ConCommand ch_createairboat("ch_createairboat", CC_CH_CreateAirboat,
  ...
  switch (iImpulse) {
    case 76: {
      if (!giPrecacheGrunt) {
        ...
        Create("NPC_human_grunt", GetLocalOrigin() + forward * 128,
               GetLocalAngles());
      }
      break;
    }
    ...
    case 107: {
      ...
      UTIL_TraceLine(start, end, MASK_SOLID_BRUSHONLY, this,
                     COLLISION_GROUP_NONE, &tr);
      if (tr.m_pEnt) pWorld = tr.m_pEnt->edict();

      const char *pTextureName = tr.surface.name;

      if (pTextureName) Msg("Texture: %s\n", pTextureName);
    } break;
...
bool CBasePlayer::ClientCommand(const CCommand &args) {
  ...
  if (stricmp(cmd, "test_SmokeGrenade") == 0) {
    if (sv_cheats && sv_cheats->GetBool()) {
      ParticleSmokeGrenade *pSmoke = dynamic_cast<ParticleSmokeGrenade *>(
      ...
      if (pSmoke) {
        ...
        pSmoke->SetFadeTime(25,
                            30);  // Fade out between 25 seconds and 30 seconds.
        pSmoke->Activate();
  ...
  } else if (stricmp(cmd, "spectate") ==
             0)  // join spectator team & start observer mode
  {
    if (GetTeamNumber() == TEAM_SPECTATOR) return true;
...
extern bool UTIL_ItemCanBeTouchedByPlayer(CBaseEntity *pItem,
                                          CBasePlayer *pPlayer);
...
bool CBasePlayer::BumpWeapon(CBaseCombatWeapon *pWeapon) {
  ...
  if (hl2_episodic.GetBool()) {
    ...
    if (pWeapon->FVisible(this, MASK_SOLID) == false &&
        !(GetFlags() & FL_NOTARGET))
      return false;
  }
  ...
  // -------------------------
    ...
    if (IsInAVehicle()) {
      ...
      if (!PlayerHasMegaPhysCannon()) {
        // If it uses clips, load it full. (this is the first time you've picked
        // up this type of weapon)
...
// Purpose:
//-----------------------------------------------------------------------------
QAngle CBasePlayer::BodyAngles() { return EyeAngles(); }
...
void CBasePlayer::UpdateClientData(void) {
  ...
  if (m_fInitHUD) {
    ...
    if (!m_fGameHUDInitialized) {
      ...
      if (g_pGameRules->IsMultiplayer()) {
        variant_t value;
        g_EventQueue.AddEvent("game_player_manager", "OnPlayerJoin", value, 0,
                              this, this);
      }
    }

    variant_t value;
    g_EventQueue.AddEvent("game_player_manager", "OnPlayerSpawn", value, 0,
                          this, this);
  }
  ...
  m_Local.m_bPoisoned = (m_bitsDamageType & DMG_POISON) &&
                        (m_nPoisonDmg > m_nPoisonRestored) && (m_iHealth < 100);

  // Check if the bonus progress HUD element should be displayed
  if (m_iBonusChallenge == 0 && m_iBonusProgress == 0 &&
...
void CBasePlayer::RumbleEffect(unsigned char index, unsigned char rumbleData,
...
bool CBasePlayer::ShouldAutoaim(void) {
  ...
  if (gpGlobals->maxClients > 1) return false;
...
void CBasePlayer::GetAutoaimVector(autoaim_params_t &params) {
  ...
  if ((ShouldAutoaim() == false) ||
      (params.m_fScale == AUTOAIM_SCALE_DIRECT_ONLY)) {
    Vector forward;
  ...
  if (!g_pGameRules->AllowAutoTargetCrosshair()) m_fOnTarget = false;
...
//-----------------------------------------------------------------------------
float CBasePlayer::GetAutoaimScore(const Vector &eyePosition,
                                   const Vector &viewDir,
                                   const Vector &vecTarget,
                                   CBaseEntity *pTarget, float fScale,
  ...
  Vector vecNearestPoint = PointOnLineNearestPoint(
      eyePosition, eyePosition + viewDir * 8192, vecTarget);
...
// Output : Vector
//-----------------------------------------------------------------------------
QAngle CBasePlayer::AutoaimDeflection(Vector &vecSrc,
                                      autoaim_params_t &params) {
  float bestscore;
  float score;
  QAngle eyeAngles;
  ...
  eyeAngles = EyeAngles();
  AngleVectors(eyeAngles + m_Local.m_vecPunchAngle + m_vecAutoAim, &v_forward,
               &v_right, &v_up);
  ...
  UTIL_TraceLine(vecSrc, vecSrc + bestdir * MAX_COORD_FLOAT, MASK_SHOT,
                 &traceFilter, &tr);
  ...
  if (pEntHit && pEntHit->m_takedamage != DAMAGE_NO &&
    ...
    if (!((GetWaterLevel() != 3 && pEntHit->GetWaterLevel() == 3) ||
          (GetWaterLevel() == 3 && pEntHit->GetWaterLevel() == 0))) {
      if (pEntHit->ShouldAttractAutoAim(this)) {
        bool bAimAtThis = true;

        if (pEntHit->IsNPC() && g_pGameRules->GetAutoAimMode() > AUTOAIM_NONE) {
          int iRelationType =
              GetDefaultRelationshipDisposition(pEntHit->Classify());
  ...
  if (count) {
    CBaseEntity **pList =
        (CBaseEntity **)stackalloc(sizeof(CBaseEntity *) * count);
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!pEntity->ShouldAttractAutoAim(this)) continue;
      ...
      if (!g_pGameRules->ShouldAutoAim(this, pEntity->edict())) continue;
      ...
      // Don't autoaim at the noisy bodytarget, this makes the autoaim crosshair
      // wobble.
      ...
      if (dist > params.m_fMaxDist) continue;
      ...
      if (!(pEntity->GetFlags() & FL_FLY)) {
        ...
        if (GetActiveWeapon() != NULL &&
            dot < GetActiveWeapon()->GetMaxAutoAimDeflection()) {
          // Be lenient if the player is looking down, though. 30 degrees
          // through 90 degrees of pitch. (90 degrees is looking down at
          // player's own 'feet'. Looking straight ahead is 0 degrees pitch.
          // This was done for XBox to make it easier to fight headcrabs around
          // the player's feet.
          if (eyeAngles.x < 30.0f || eyeAngles.x > 90.0f ||
              g_pGameRules->GetAutoAimMode() != AUTOAIM_ON_CONSOLE) {
            continue;
          }
        }
      }

      score = GetAutoaimScore(vecSrc, v_forward, pEntity->GetAutoAimCenter(),
                              pEntity, params.m_fScale, GetActiveWeapon());
      ...
      if (tr.fraction != 1.0 && tr.m_pEnt != pEntity) {
        // Msg( "hit %s, can't see %s\n", STRING( tr.u.ent->classname ), STRING(
        // pEdict->classname ) );
        continue;
      }
...
//-----------------------------------------------------------------------------
bool CBasePlayer::Weapon_CanUse(CBaseCombatWeapon *pWeapon) { return true; }

//-----------------------------------------------------------------------------
// Purpose: Override to clear dropped weapon from the hud
//-----------------------------------------------------------------------------
void CBasePlayer::Weapon_Drop(CBaseCombatWeapon *pWeapon,
...
#ifdef HL2_DLL
  if (bShouldSwitch == false &&
      PhysCannonGetHeldEntity(GetActiveWeapon()) == pWeapon &&
...
BEGIN_DATADESC(CStripWeapons)
  DEFINE_INPUTFUNC(FIELD_VOID, "Strip", InputStripWeapons),
      DEFINE_INPUTFUNC(FIELD_VOID, "StripWeaponsAndSuit",
...
BEGIN_DATADESC(CRevertSaved)
  ...
  DEFINE_KEYFIELD(m_iszMessage, FIELD_STRING, "message"),
      DEFINE_KEYFIELD(m_messageTime, FIELD_FLOAT,
                      "messagetime"),  // These are not actual times, but
                                       // durations, so save as floats
...
CBaseEntity *CreatePlayerLoadSave(Vector vOrigin, float flDuration,
                                  float flHoldTime, float flLoadTime) {
  CRevertSaved *pRevertSaved =
      (CRevertSaved *)CreateEntityByName("player_loadsaved");

  if (pRevertSaved == NULL) return NULL;
...
void CRevertSaved::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
()
    ...
    BEGIN_SEND_TABLE_NOBASE(CBasePlayer, DT_LocalPlayerExclusive)
        ...
        SendPropDataTable(SENDINFO_DT(m_Local),
                          &REFERENCE_SEND_TABLE(DT_Local)),
// If HL2_DLL is defined, then baseflex.cpp already sends these.
#ifndef HL2_DLL
    SendPropFloat(SENDINFO_VECTORELEM(m_vecViewOffset, 0), 8, SPROP_ROUNDDOWN,
                  -32.0, 32.0f),
    SendPropFloat(SENDINFO_VECTORELEM(m_vecViewOffset, 1), 8, SPROP_ROUNDDOWN,
                  -32.0, 32.0f),
    SendPropFloat(SENDINFO_VECTORELEM(m_vecViewOffset, 2), 10,
                  SPROP_CHANGES_OFTEN, 0.0f, 128.0f),
#endif
    ...
    SendPropFloat(SENDINFO_VECTORELEM(m_vecVelocity, 0), 32,
                  SPROP_NOSCALE | SPROP_CHANGES_OFTEN),
    SendPropFloat(SENDINFO_VECTORELEM(m_vecVelocity, 1), 32,
                  SPROP_NOSCALE | SPROP_CHANGES_OFTEN),
    SendPropFloat(SENDINFO_VECTORELEM(m_vecVelocity, 2), 32,
                  SPROP_NOSCALE | SPROP_CHANGES_OFTEN),
...
()
    ...
    SendPropInt(SENDINFO(m_ubEFNoInterpParity), NOINTERP_PARITY_MAX_BITS,
                SPROP_UNSIGNED),
...
()
    ...
    void CBasePlayer::SetupVPhysicsShadow(const Vector &vecAbsOrigin,
  ...
  m_pShadowStand =
      PhysModelCreateCustom(this, pStandModel, GetLocalOrigin(),
                            GetLocalAngles(), pStandHullName, false, &solid);
  m_pShadowStand->SetCallbackFlags(CALLBACK_GLOBAL_COLLISION |
                                   CALLBACK_SHADOW_COLLISION);

  // create crouchig hull
  m_pShadowCrouch =
      PhysModelCreateCustom(this, pCrouchModel, GetLocalOrigin(),
                            GetLocalAngles(), pCrouchHullName, false, &solid);
  m_pShadowCrouch->SetCallbackFlags(CALLBACK_GLOBAL_COLLISION |
                                    CALLBACK_SHADOW_COLLISION);
...
void CBasePlayer::VPhysicsShadowUpdate(IPhysicsObject *pPhysics) {
  ...
  bool physicsUpdated =
      m_pPhysicsController->GetShadowPosition(&newPosition, NULL) > 0 ? true
                                                                      : false;

  // UNDONE: If the player is penetrating, but the player's game collisions are
  // not stuck, teleport the physics shadow to the game position
  ...
  if (physicsshadowupdate_render.GetBool()) {
    NDebugOverlay::Box(GetAbsOrigin(), WorldAlignMins(), WorldAlignMaxs(), 255,
                       0, 0, 24, 15.0f);
  ...
  // player's physics was frozen, try moving to the game's simulated position if
  // possible
  if (m_pPhysicsController->WasFrozen()) {
    ...
    trace_t trace;
    UTIL_TraceEntity(this, GetAbsOrigin(), GetAbsOrigin(), MASK_PLAYERSOLID,
                     this, COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
    if (!trace.startsolid) return;

    // The physics shadow position is probably not in solid, try to move from
    // there to the desired position
    UTIL_TraceEntity(this, newPosition, GetAbsOrigin(), MASK_PLAYERSOLID, this,
                     COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
    if (!trace.startsolid) {
  ...
  if (dist >= maxDistErrorSqr || deltaV >= maxVelErrorSqr ||
      (pPhysGround && !m_touchedPhysObject)) {
    if (m_touchedPhysObject || pPhysGround) {
      ...
      trace_t trace;
      UTIL_TraceEntity(this, newPosition, newPosition, MASK_PLAYERSOLID, this,
                       COLLISION_GROUP_PLAYER_MOVEMENT, &trace);
    ...
    if (m_touchedPhysObject) {
      ...
      if (trace.allsolid || trace.startsolid) {
        // no use the final stuck check to move back to old if this stuck fix
        // didn't work
        bCheckStuck = true;
...
bool CBasePlayer::ShouldSavePhysics() { return false; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CBasePlayer::InitVCollision(const Vector &vecAbsOrigin,
                                 const Vector &vecAbsVelocity) {
  ...
  CPhysCollide *pCrouchModel =
      PhysCreateBbox(VEC_DUCK_HULL_MIN, VEC_DUCK_HULL_MAX);

  SetupVPhysicsShadow(vecAbsOrigin, vecAbsVelocity, pModel, "player_stand",
                      pCrouchModel, "player_crouch");
}

void CBasePlayer::VPhysicsDestroyObject() {
  // Since CBasePlayer aliases its pointer to the physics object, tell
  // CBaseEntity to clear out its physics object pointer so we don't wind up
  // deleting one of the aliased objects twice.
...
// Purpose:
//-----------------------------------------------------------------------------
void CBasePlayer::SetVCollisionState(const Vector &vecAbsOrigin,
                                     const Vector &vecAbsVelocity,
                                     int collisionState) {
...
int CBasePlayer::GetFOV(void) {
  ...
  if (GetVehicle()) {
    CacheVehicleView();
    nDefaultFOV =
        (m_flVehicleViewFOV == 0) ? GetDefaultFOV() : (int)m_flVehicleViewFOV;
  } else {
  ...
  if (deltaTime >= 1.0f) {
    ...
    fFOV =
        SimpleSplineRemapValClamped(deltaTime, 0.0f, 1.0f, m_iFOVStart, fFOV);
  }
...
float CBasePlayer::GetFOVDistanceAdjustFactorForNetworking() {
  ...
  // If FOV is lower, then we're "zoomed" in and this will give a factor < 1 so
  // apparent LOD distances can be
  //  shorted accordingly
...
void CBasePlayer::ModifyOrAppendPlayerCriteria(AI_CriteriaSet &set) {
  ...
  set.AppendCriteria("playeractivity",
                     CAI_BaseNPC::GetActivityName(GetActivity()));

  set.AppendCriteria("playerspeed",
                     UTIL_VarArgs("%.3f", GetAbsVelocity().Length()));
...
void CBasePlayer::SetPunchAngle(const QAngle &punchAngle) {
  ...
  if (IsAlive()) {
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      if (pPlayer && i != index && pPlayer->GetObserverTarget() == this &&
...
//-----------------------------------------------------------------------------
void CBasePlayer::ActivateMovementConstraint(CBaseEntity *pEntity,
...
//-----------------------------------------------------------------------------
CBaseEntity *CBasePlayer::DoubleCheckUseNPC(CBaseEntity *pNPC,
                                            const Vector &vecSrc,
                                            const Vector &vecDir) {
  trace_t tr;

  UTIL_TraceLine(vecSrc, vecSrc + vecDir * 1024, MASK_SHOT, this,
                 COLLISION_GROUP_NONE, &tr);
...
bool CBasePlayer::IsBot() const { return (GetFlags() & FL_FAKECLIENT) != 0; }
...
void CBasePlayer::RemoveSuit(void) { m_Local.m_bWearingSuit = false; }
...
void CBasePlayer::InputSetFogController(inputdata_t &inputdata) {
  ...
  CFogController *pFogController = dynamic_cast<CFogController *>(
      gEntList.FindEntityByName(NULL, inputdata.value.String()));
  if (pFogController) {
...
//-----------------------------------------------------------------------------
// Purpose: Looks at the player's reserve ammo and also all his weapons for any
// ammo
...
bool CBasePlayer::HasAnyAmmoOfType(int nAmmoIndex) {
  ...
  if (nAmmoIndex < 0) return false;

  // If we have some in reserve, we're already done
  if (GetAmmoCount(nAmmoIndex)) return true;
  ...
  for (int i = 0; i < MAX_WEAPONS; i++) {
    ...
    if (pWeapon->UsesClipsForAmmo1() &&
        pWeapon->GetPrimaryAmmoType() == nAmmoIndex) {
      // If we have any ammo, we're done
      if (pWeapon->HasPrimaryAmmo()) return true;
    }

    // We'll check both clips for the same ammo type, just in case
    if (pWeapon->UsesClipsForAmmo2() &&
        pWeapon->GetSecondaryAmmoType() == nAmmoIndex) {
      if (pWeapon->HasSecondaryAmmo()) return true;
    }
  }
...
const char *CBasePlayer::GetNetworkIDString() {
  Q_strncpy(m_szNetworkIDString, engine->GetPlayerNetworkIDString(edict()),
            sizeof(m_szNetworkIDString));
...
class DisableAutokick {
 public:
  DisableAutokick(int userID) { m_userID = userID; }
...
//-----------------------------------------------------------------------------
//  CPlayerInfo functions (simple passthroughts to get around the CBasePlayer
//  multiple inheritence limitation)
//-----------------------------------------------------------------------------
...
// Notify that I've killed some other entity. (called from Victim's
// Event_Killed).
void CBasePlayer::Event_KilledOther(CBaseEntity *pVictim,
