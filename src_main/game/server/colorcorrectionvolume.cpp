...
class CColorCorrectionVolume : public CBaseTrigger {
 ...
 private:
  ...
  CNetworkString(m_lookupFilename, SOURCE_MAX_PATH);
...
IMPLEMENT_SERVERCLASS_ST_NOBASE(CColorCorrectionVolume,
                                DT_ColorCorrectionVolume)
SendPropFloat(SENDINFO(m_Weight)), SendPropString(SENDINFO(m_lookupFilename)),
    END_SEND_TABLE()
...
bool CColorCorrectionVolume::KeyValue(const char *szKeyName,
                                      const char *szValue) {
  if (FStrEq(szKeyName, "filename")) {
    Q_strncpy(m_lookupFilename.GetForModify(), szValue, SOURCE_MAX_PATH);
...
bool CColorCorrectionVolume::PassesTriggerFilters(CBaseEntity *pEntity) {
  if (pEntity == UTIL_GetLocalPlayer()) return true;
  ...
  if (!m_bEnabled) {
    ...
    if (m_LastEnterTime > m_LastExitTime) {
      ...
      if (m_Weight < 1.0f) {
        ...
        float weight = m_LastEnterWeight +
                       dt / ((1.0f - m_LastEnterWeight) * m_FadeDuration);
      ...
      if (m_Weight > 0.0f) {
        ...
        float weight = (1.0f - m_LastExitWeight) +
                       dt / (m_LastExitWeight * m_FadeDuration);
        if (weight > 1.0f) weight = 1.0f;
