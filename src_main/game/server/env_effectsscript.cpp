...
class CEnvEffectsScript : public CBaseAnimating {
 public:
  ...
  const char *GetScriptFile(void) { return STRING(m_iszScriptName); }
 ...
 private:
  ...
  bool IsRootCommand(void) {
    if (!Q_stricmp(token, "effect")) return true;
...
inline void Unget() { g_bUnget = true; }
...
void CEnvEffectsScript::TrailEffectEvent(CEffectScriptElement *pEffect) {
  if (pEffect->IsActive() == false) {
    ...
    if (pEffect->m_pTrail == NULL) {
      pEffect->m_pTrail = CSpriteTrail::SpriteTrailCreate(pEffect->m_szMaterial,
                                                          GetAbsOrigin(), true);
      pEffect->m_pTrail->FollowEntity(this);
      pEffect->m_pTrail->SetTransparency(pEffect->m_iRenderType, pEffect->m_iR,
                                         pEffect->m_iG, pEffect->m_iB,
                                         pEffect->m_iA, kRenderFxNone);
      pEffect->m_pTrail->SetStartWidth(pEffect->m_flScale);
      if (pEffect->m_flTextureRes < 0.0f) {
        pEffect->m_pTrail->SetTextureResolution(1.0f /
                                                (16.0f * pEffect->m_flScale));
      } else {
      ...
      pEffect->m_pTrail->SetAttachment(
          this, LookupAttachment(pEffect->m_szAttachment));
...
void CEnvEffectsScript::SpriteEffectEvent(CEffectScriptElement *pEffect) {
  if (pEffect->IsActive() == false) {
    ...
    if (pEffect->m_pSprite == NULL) {
      pEffect->m_pSprite =
          CSprite::SpriteCreate(pEffect->m_szMaterial, GetAbsOrigin(), true);
      pEffect->m_pSprite->FollowEntity(this);
      pEffect->m_pSprite->SetTransparency(pEffect->m_iRenderType, pEffect->m_iR,
                                          pEffect->m_iG, pEffect->m_iB,
                                          pEffect->m_iA, kRenderFxNone);
      pEffect->m_pSprite->SetScale(pEffect->m_flScale);
      pEffect->m_pSprite->TurnOn();
      pEffect->m_pSprite->SetAttachment(
          this, LookupAttachment(pEffect->m_szAttachment));
...
void CEnvEffectsScript::ParseScriptFile(void) {
  ...
  unsigned char *buffer =
      (unsigned char *)UTIL_LoadFileForMe(pScriptName, &length);
  if (length <= 0 || !buffer) {
...
void CEnvEffectsScript::LoadFromBuffer(const char *scriptfile,
                                       const char *buffer) {
  while (1) {
    ParseToken();
...
void CEnvEffectsScript::ParseNewEffect(void) {
  ...
  // Effect Group Name
  ParseToken();
  Q_strncpy(NewElement.m_szEffectName, token,
            sizeof(NewElement.m_szEffectName));
    ...
    if (!Q_stricmp(token, "{")) {
      while (1) {
        ...
        if (!Q_stricmp(token, "material")) {
          ParseToken();
          Q_strncpy(NewElement.m_szMaterial, token,
                    sizeof(NewElement.m_szMaterial));
        ...
        if (!Q_stricmp(token, "attachment")) {
          ParseToken();
          Q_strncpy(NewElement.m_szAttachment, token,
                    sizeof(NewElement.m_szAttachment));
        ...
        if (!Q_stricmp(token, "texturescale")) {
          ...
          NewElement.m_flTextureRes =
              (flTextureScale > 0.0f) ? 1.0f / flTextureScale : 0.0f;
          continue;
        }
...
CEffectScriptElement *CEnvEffectsScript::GetScriptElementByName(
    const char *pName) {
