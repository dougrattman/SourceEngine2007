...
ConVar sv_turbophysics("sv_turbophysics", "0", FCVAR_REPLICATED,
                       "Turns on turbo physics");

#ifdef HL2_EPISODIC
#define PROP_FLARE_LIFETIME 30.0f
#define PROP_FLARE_IGNITE_SUBSTRACT 5.0f
CBaseEntity *CreateFlare(Vector vOrigin, QAngle Angles, CBaseEntity *pOwner,
                         float flDuration);
void KillFlare(CBaseEntity *pOwnerEntity, CBaseEntity *pEntity,
               float flKillTime);
#endif
...
void CBaseProp::Spawn(void) {
  ...
  if (!OverridePropdata()) {
    if (iResult == PARSE_FAILED_BAD_DATA) {
      DevWarning(
          "%s at %.0f %.0f %0.f uses model %s, which has an invalid prop_data "
          "type. DELETED.\n",
      ...
      if (FClassnameIs(this, "prop_physics")) {
        DevWarning(
            "%s at %.0f %.0f %0.f uses model %s, which has no propdata which "
            "means it must be used on a prop_static. DELETED.\n",
            GetClassname(), GetAbsOrigin().x, GetAbsOrigin().y,
            GetAbsOrigin().z, szModel);
        UTIL_Remove(this);
        return;
      }
      ...
      if (!dynamic_cast<CPhysicsProp *>(this)) {
        DevWarning(
            "%s at %.0f %.0f %0.f uses model %s, which has propdata which "
            "means that it be used on a prop_physics. DELETED.\n",
            GetClassname(), GetAbsOrigin().x, GetAbsOrigin().y,
            GetAbsOrigin().z, szModel);
        UTIL_Remove(this);
        return;
...
void CBaseProp::Activate(void) {
  ...
  if (m_takedamage == DAMAGE_NO && m_iHealth != 0) {
    Warning(
        "%s has a health specified in model '%s'. Use prop_physics or "
        "prop_dynamic instead.\n",
...
int CBaseProp::ParsePropData(void) {
  ...
  if (!modelKeyValues->LoadFromBuffer(
  ...
  int iResult =
      g_PropDataSystem.ParsePropFromKV(this, pkvPropData, modelKeyValues);
  modelKeyValues->deleteThis();
  return iResult;
}
...
void CBreakableProp::SetEnableMotionPosition(const Vector &position,
                                             const QAngle &angles) {
  ClearEnableMotionPosition();
  CBaseEntity *pFixup =
      CBaseEntity::Create("point_enable_motion_fixup", position, angles, this);
  if (pFixup) {
...
CBaseEntity *CBreakableProp::FindEnableMotionFixup() {
  ...
  for (int i = list.Count() - 1; i >= 0; --i) {
    if (FClassnameIs(list[i], "point_enable_motion_fixup")) return list[i];
  }

  return NULL;
}

bool CBreakableProp::GetEnableMotionPosition(Vector *pPosition,
                                             QAngle *pAngles) {
  CBaseEntity *pFixup = FindEnableMotionFixup();
  if (!pFixup) return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBreakableProp::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
                            bool bCalledByLevelDesigner) {
  if (IsOnFire()) return;

  if (!HasInteraction(PROPINTER_FIRE_FLAMMABLE)) return;
  ...
  CSoundEnt::InsertSound(SOUND_DANGER, GetAbsOrigin(), 128.0f, 1.0f, this,
                         SOUNDENT_CHANNEL_REPEATED_DANGER);
}

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBreakableProp::HandleFirstCollisionInteractions(
  ...
  if (HasInteraction(PROPINTER_PHYSGUN_FIRST_PAINT)) {
    ...
    trace_t tr;
    UTIL_TraceLine(vecPos, vecPos + (vecVelocity * 64), MASK_SHOT, this,
    ...
    if (tr.m_pEnt) {
      ...
      if (iClassify != CLASS_PLAYER_ALLY_VITAL &&
          iClassify != CLASS_PLAYER_ALLY &&
          iClassify != CLASS_CITIZEN_PASSIVE &&
          iClassify != CLASS_CITIZEN_REBEL)
#endif
      {
        switch (entindex() % 3) {
          case 0:
  ...
  if (HasInteraction(PROPINTER_PHYSGUN_NOTIFY_CHILDREN)) {
    ...
    for (int i = 0; i < children.Count(); i++) {
      ...
      IParentPropInteraction *pPropInter =
          dynamic_cast<IParentPropInteraction *>(pent);
      if (pPropInter) {
        pPropInter->OnParentCollisionInteraction(
            COLLISIONINTER_PARENT_FIRST_IMPACT, index, pEvent);
      }
    }
  }
}
...
//			*pEvent -
//-----------------------------------------------------------------------------
void CPhysicsProp::HandleAnyCollisionInteractions(
    int index, gamevcollisionevent_t *pEvent) {
  ...
  if (HasInteraction(PROPINTER_PHYSGUN_FIRST_IMPALE)) {
    ...
    if (pHitEntity->IsWorld()) {
      ...
      if ((sign * dot) < DOT_45DEGREE) return;
      ...
      trace_t tr;
      UTIL_TraceLine(vecPos, vecPos + pEvent->preVelocity[index] * 1.5,
      ...
      CBaseEntity *pRagdoll = CreateServerRagdollAttached(
          pNPC, vec3_origin, -1, COLLISION_GROUP_INTERACTIVE_DEBRIS, pObj, this,
          0, vecImpalePos, iBone, vec3_origin);
      if (pRagdoll) {
...
void CBreakableProp::StickAtPosition(const Vector &stickPosition,
                                     const Vector &savePosition,
                                     const QAngle &saveAngles) {
  if (!VPhysicsGetObject()->IsMotionEnabled()) return;

  EmitSound("Metal.SawbladeStick");
  Teleport(&stickPosition, NULL, NULL);
  SetEnableMotionPosition(
      savePosition,
      saveAngles);  // this uses hierarchy, so it must be set after teleport
...
//			*pEvent -
//-----------------------------------------------------------------------------
void CBreakableProp::HandleInteractionStick(int index,
  ...
  if (speed > 1000.0f) {
    ...
    if (flDot > 0.3) {
      // Finally, inhibit sticking in metal, grates, sky, or anything else that
      ...
      if (psurf->game.material != CHAR_TEX_METAL &&
        ...
        position += vecEmbed;
        g_PostSimulationQueue.QueueCall(this, &CBreakableProp::StickAtPosition,
                                        position, savePosition, angles);
      }
    }
  }
}
...
static ConCommand prop_debug(
    "prop_debug", CC_Prop_Debug,
    "Toggle prop debug mode. If on, props will show colorcoded bounding boxes. "
    "Red means ignore all damage. White means respond physically to damage but "
    "never break. Green maps health in the range of 100 down to 1.",
    FCVAR_CHEAT);
...
BEGIN_DATADESC(CBreakableProp)
  ...
  DEFINE_KEYFIELD(m_explodeDamage, FIELD_FLOAT, "ExplodeDamage"),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnablePhyscannonPickup",
                       InputEnablePhyscannonPickup),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisablePhyscannonPickup",
      ...
      DEFINE_OUTPUT(m_OnPhysCannonAnimatePreStarted,
                    "OnPhysCannonAnimatePreStarted"),
      DEFINE_OUTPUT(m_OnPhysCannonAnimatePullStarted,
                    "OnPhysCannonAnimatePullStarted"),
      DEFINE_OUTPUT(m_OnPhysCannonAnimatePostStarted,
                    "OnPhysCannonAnimatePostStarted"),
      DEFINE_OUTPUT(m_OnPhysCannonPullAnimFinished,
                    "OnPhysCannonPullAnimFinished"),

      // Function Pointers
      DEFINE_THINKFUNC(BreakThink), DEFINE_THINKFUNC(AnimateThink),
...
CBreakableProp::CBreakableProp() {
  ...
  // This defaults to on. Most times mapmakers won't specify a punt sound to
  // play.
...
void CBreakableProp::Spawn() {
  ...
  // jmd: I am guessing that the call to Spawn will set any flags that should be
  // set anyway; this clears flags we don't want (specifically the FL_ONFIRE for
  // explosive barrels in HL2MP)]
#ifdef HL2MP
  ClearFlags();
#endif
  ...
  // If we have no custom breakable chunks, see if we're breaking into generic
  // ones
  if (!m_iNumBreakableChunks) {
    IBreakableWithPropData *pBreakableInterface =
        assert_cast<IBreakableWithPropData *>(this);
    if (pBreakableInterface->GetBreakableModel() != NULL_STRING &&
        pBreakableInterface->GetBreakableCount()) {
      m_iNumBreakableChunks = pBreakableInterface->GetBreakableCount();
    }
  }

  // Setup takedamage based upon the health we parsed earlier, and our
  // interactions
  if ((m_iHealth == 0) || (!m_iNumBreakableChunks &&
  ...
  // The presence of this activity causes us to have to detach it before it can
  // be grabbed.
  if (SelectWeightedSequence(ACT_PHYSCANNON_ANIMATE) !=
      ACTIVITY_NOT_AVAILABLE) {
    m_nPhysgunState = PHYSGUN_ANIMATE_ON_PULL;
  } else if (SelectWeightedSequence(ACT_PHYSCANNON_DETACH) !=
             ACTIVITY_NOT_AVAILABLE) {
...
void CBreakableProp::CopyFadeFrom(CBreakableProp *pSource) {
  ...
  if (m_flFadeScale != m_flDefaultFadeScale) {
    ...
    SetContextThink(&CBreakableProp::RampToDefaultFadeScale, flNextThink,
                    s_pFadeScaleThink);
  }
}
  ...
  if (HasSpawnFlags(SF_PHYSPROP_TOUCH)) {
    ...
    if (flDamage >= m_iHealth) {
      ...
      CalculateMeleeDamageForce(
          &info, (pOther->GetAbsOrigin() - GetAbsOrigin()), GetAbsOrigin());
      pOther->TakeDamage(info);
    }
  }

  if (HasSpawnFlags(SF_PHYSPROP_PRESSURE) &&
      pOther->GetGroundEntity() == this) {
...
//-----------------------------------------------------------------------------
// UNDONE: Time stamp the object's creation so that an explosion or something
// doesn't break the parent object and then break the children who spawn
// afterward ? Explosions should use entities in box before they start to do
// damage.  Make sure nothing traverses the list in a way that would hose this.
int CBreakableProp::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  CTakeDamageInfo info = inputInfo;

  // If attacker can't do at least the min required damage to us, don't take any
  // damage from them
  ...
  float flPropDamage =
      GetBreakableDamage(info, assert_cast<IBreakableWithPropData *>(this));
  ...
  if (IsOnFire() && (inputInfo.GetDamageType() & DMG_BURN) &&
      !(inputInfo.GetDamageType() & DMG_DIRECT)) {
    return 0;
  }
  ...
  if (bDeadly && (info.GetDamageType() & DMG_BLAST) &&
      HasInteraction(PROPINTER_FIRE_EXPLOSIVE_RESIST) && info.GetInflictor()) {
    // This explosion would kill me, but I have a special interaction with
    // explosions.

    float flDist =
        (WorldSpaceCenter() - info.GetInflictor()->WorldSpaceCenter()).Length();
    ...
    if (flDist >= PROP_EXPLOSION_IGNITE_RADIUS) {
      ...
      // Just clamp distance.
      if (flDist > MAX_BLAST_DIST) flDist = MAX_BLAST_DIST;
  ...
  if (!bDeadly && (info.GetDamageType() & DMG_BLAST)) {
    ...
    // Ignite if burned, and flammable (the Ignite() function takes care of all
    // of this).
    Ignite(random->RandomFloat(10, 15), false);
    ...
    if (HasInteraction(PROPINTER_FIRE_IGNITE_HALFHEALTH)) {
      if ((m_iHealth - info.GetDamage()) <= m_iMaxHealth / 2 && !IsOnFire()) {
        // Bump back up to full health so it burns longer. Magically getting
        // health back isn't a big problem because if this item takes damage
        // again whilst burning, it will break.
...
//-----------------------------------------------------------------------------
// Purpose: Choke point for changes to breakable health. Ensures outputs are
// fired. Input  : iNewHealth -
//			pActivator -
...
//-----------------------------------------------------------------------------
bool CBreakableProp::OnAttemptPhysGunPickup(CBasePlayer *pPhysGunUser,
                                            PhysGunPickup_t reason) {
  if (m_nPhysgunState == PHYSGUN_CAN_BE_GRABBED) return true;
  if (m_nPhysgunState == PHYSGUN_ANIMATE_FINISHED) return false;
  ...
  if (m_nPhysgunState == PHYSGUN_ANIMATE_ON_PULL) {
    ...
    if (reason == PUNTED_BY_CANNON) return false;
    ...
    if (iSequence != ACTIVITY_NOT_AVAILABLE) {
      ...
      SetContextThink(&CBreakableProp::AnimateThink, gpGlobals->curtime + 0.1,
                      s_pPropAnimateThink);
  ...
  if (m_nPhysgunState == PHYSGUN_ANIMATE_IS_ANIMATING) {
    ...
    // Here, we're grabbing it. If we try to punt it, advance frames by quite a
    // bit.
    StudioFrameAdvanceManual(
        (reason == PICKED_UP_BY_CANNON) ? gpGlobals->frametime : 0.5f);
    ResetClientsideFrame();
    DispatchAnimEvents(this);
...
void CBreakableProp::RampToDefaultFadeScale() {
  ...
  if (m_flFadeScale >= m_flDefaultFadeScale) {
    ...
    SetContextThink(&CBreakableProp::RampToDefaultFadeScale,
                    gpGlobals->curtime + TICK_INTERVAL, s_pFadeScaleThink);
  }
}
...
//-----------------------------------------------------------------------------
void CBreakableProp::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                     PhysGunPickup_t reason) {
  ...
  if (reason == PICKED_UP_BY_CANNON) {
    ...
    SetContextThink(&CBreakableProp::RampToDefaultFadeScale,
                    gpGlobals->curtime + 2.0f, s_pFadeScaleThink);
  }
...
void CBreakableProp::CreateFlare(float flLifetime) {
  // Create the flare
  CBaseEntity *pFlare =
      ::CreateFlare(GetAbsOrigin(), GetAbsAngles(), this, flLifetime);
  if (pFlare) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CBreakableProp::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
AngularImpulse CBreakableProp::PhysGunLaunchAngularImpulse() {
  if (HasInteraction(PROPINTER_PHYSGUN_LAUNCH_SPIN_NONE) ||
      HasInteraction(PROPINTER_PHYSGUN_LAUNCH_SPIN_Z)) {
    // Don't add in random angular impulse if this object is supposed to spin in
    // a specific way.
...
void CBreakableProp::PlayPuntSound() {
  if (!m_bUsePuntSound) return;

  if (m_iszPuntSound == NULL_STRING) return;
...
void CBreakableProp::Break(CBaseEntity *pBreaker, const CTakeDamageInfo &info) {
  ...
  if (pModelName && Q_stristr(pModelName, "crate")) {
    ...
    if (pBreaker && pBreaker->IsPlayer()) {
      ...
      CPropVehicleDriveable *veh =
          static_cast<CPropVehicleDriveable *>(pBreaker);
  ...
  if (m_hLastAttacker) {
    ...
    // This way the player allies can get immunity from barrels exploded by the
    // player.
    pAttacker = m_hLastAttacker;
  } else if (m_hPhysicsAttacker) {
    // If I have a physics attacker and was influenced in the last 2 seconds,
    // Make the attacker my physics attacker. This helps protect citizens from
    // dying in the explosion of a physics object that was thrown by the
    // player's physgun and exploded on impact.
  ...
  if (m_explodeDamage > 0 || m_explodeRadius > 0) {
    if (HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE)) {
      ExplosionCreate(WorldSpaceCenter(), angles, pAttacker, m_explodeDamage,
                      m_explodeRadius,
                      SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
                          SF_ENVEXPLOSION_NOSMOKE |
                          SF_ENVEXPLOSION_SURFACEONLY | SF_ENVEXPLOSION_NOSOUND,
                      0.0f, this);
      ...
      ExplosionCreate(WorldSpaceCenter(), angles, pAttacker, m_explodeDamage,
                      m_explodeRadius,
                      SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
                          SF_ENVEXPLOSION_NOSMOKE | SF_ENVEXPLOSION_SURFACEONLY,
                      0.0f, this);
    }
  ...
  if (gpGlobals->maxClients > 1 && breakable_multiplayer.GetBool()) {
    ...
    if (pPhysics) pPhysics->GetVelocity(&velocity, NULL);
    ...
    switch (GetMultiplayerBreakMode()) {
      ...
      case MULTIPLAYER_BREAK_CLIENTSIDE:
        te->PhysicsProp(filter, -1, GetModelIndex(), m_nSkin, GetAbsOrigin(),
                        GetAbsAngles(), velocity, true, GetEffects());
        break;
      case MULTIPLAYER_BREAK_SERVERSIDE:  // server-side break
        if (m_PerformanceMode != PM_NO_GIBS ||
            breakable_disable_gib_limit.GetBool()) {
          PropBreakableCreateAll(GetModelIndex(), pPhysics, params, this, -1,
                                 (m_PerformanceMode == PM_FULL_GIBS), false);
        }
        break;
  ...
  // no damage/damage force? set a burst of 100 for some movement
           ...
           breakable_disable_gib_limit.GetBool()) {
    PropBreakableCreateAll(GetModelIndex(), pPhysics, params, this, -1,
                           (m_PerformanceMode == PM_FULL_GIBS));
  }
  ...
  if (HasInteraction(PROPINTER_PHYSGUN_BREAK_EXPLODE)) {
    ...
    for (CEntitySphereQuery sphere(origin, m_explodeRadius);
...
BEGIN_DATADESC(CDynamicProp)
  ...
  DEFINE_KEYFIELD(m_iszDefaultAnim, FIELD_STRING, "DefaultAnim"),
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "SetDefaultAnimation",
...
SendPropBool(SENDINFO(m_bUseHitboxesForRenderBox)), END_SEND_TABLE()
...
void CDynamicProp::Spawn() {
  ...
  if (m_bRandomAnimator || (m_iszDefaultAnim != NULL_STRING)) {
    ...
    if (m_bRandomAnimator) {
      ...
      m_flNextRandAnim =
          gpGlobals->curtime +
          random->RandomFloat(m_flMinRandAnimTime, m_flMaxRandAnimTime);
  ...
  m_bUseHitboxesForRenderBox =
      HasSpawnFlags(SF_DYNAMICPROP_USEHITBOX_FOR_RENDERBOX);
...
// Call this when creating bone followers or changing hierarchy to make sure the
// bone followers get updated when hierarchy changes
void CDynamicProp::BoneFollowerHierarchyChanged() {
  // If we have bone followers and we're parented to something, we need to
  // constantly update our bone followers
...
bool CDynamicProp::CreateVPhysics(void) {
  if (GetSolid() == SOLID_NONE || ((GetSolidFlags() & FSOLID_NOT_SOLID) &&
                                   HasSpawnFlags(SF_DYNAMICPROP_NO_VPHYSICS)))
    return true;
  ...
  if (m_BoneFollowerManager.GetNumBoneFollowers()) {
    if (GetSolidFlags() & FSOLID_NOT_SOLID) {
      ...
      for (int i = 0; i < m_BoneFollowerManager.GetNumBoneFollowers(); i++) {
        CBaseEntity *pFollower =
            m_BoneFollowerManager.GetBoneFollower(i)->hFollower;
        if (pFollower) {
    ...
    AddSolidFlags(FSOLID_NOT_SOLID);
    // add these for the client, FSOLID_NOT_SOLID should keep it out of the
    // testCollision code except in the case of TraceEntity() which the client
    // does for impact effects
...
void CDynamicProp::CreateBoneFollowers() {
  ...
  if (m_BoneFollowerManager.GetNumBoneFollowers()) return;
  ...
  // if we got here, we don't have a bone follower section, but if we have a
  // ragdoll go ahead and create default bone followers for it
...
bool CDynamicProp::TestCollision(const Ray_t &ray, unsigned int mask,
                                 trace_t &trace) {
  if (IsSolidFlagSet(FSOLID_NOT_SOLID)) {
    // if this entity is marked non-solid and custom test it must have bone
    // followers
    if (IsSolidFlagSet(FSOLID_CUSTOMBOXTEST) &&
        IsSolidFlagSet(FSOLID_CUSTOMRAYTEST)) {
      for (int i = 0; i < m_BoneFollowerManager.GetNumBoneFollowers(); i++) {
        CBaseEntity *pEntity =
            m_BoneFollowerManager.GetBoneFollower(i)->hFollower;
        if (pEntity && pEntity->TestCollision(ray, mask, trace)) return true;
      }
    }
  }
  return false;
...
void CDynamicProp::AnimThink(void) {
  ...
  if (((m_iTransitionDirection > 0 && GetCycle() >= 0.999f) ||
       (m_iTransitionDirection < 0 && GetCycle() <= 0.0f)) &&
      !SequenceLoops()) {
...
void CDynamicProp::PropSetSequence(int nSequence) {
  ...
  if (GotoSequence(GetSequence(), GetCycle(), GetPlaybackRate(),
                   m_iGoalSequence, nNextSequence, nextCycle,
...
// NOTE: To avoid risk, currently these do nothing about collisions, only
// visually on/off
...
class COrnamentProp : public CDynamicProp {
 ...
 public:
  ...
  void Spawn();
  void Activate();
  void AttachTo(const char *pAttachEntity, CBaseEntity *pActivator = NULL,
...
void COrnamentProp::AttachTo(const char *pAttachName, CBaseEntity *pActivator,
  ...
  CBaseEntity *pAttach =
      gEntList.FindEntityByName(NULL, pAttachName, NULL, pActivator, pCaller);
  if (pAttach) {
    RemoveEffects(EF_NODRAW);
...
void COrnamentProp::InputDetach(inputdata_t &inputdata) { DetachFromOwner(); }
...
BEGIN_DATADESC(CPhysicsProp)
  ...
  DEFINE_INPUTFUNC(FIELD_VOID, "EnableMotion", InputEnableMotion),
      ...
      DEFINE_KEYFIELD(m_damageToEnableMotion, FIELD_INTEGER,
                      "damagetoenablemotion"),
      DEFINE_KEYFIELD(m_flForceToEnableMotion, FIELD_FLOAT,
                      "forcetoenablemotion"),
...
SendPropBool(SENDINFO(m_bAwake)), END_SEND_TABLE()
...
void CPhysicsProp::Spawn() {
  ...
  if (HasSpawnFlags(SF_PHYSPROP_DEBRIS) ||
      HasInteraction(PROPINTER_PHYSGUN_CREATE_FLARE)) {
    SetCollisionGroup(HasSpawnFlags(SF_PHYSPROP_FORCE_TOUCH_TRIGGERS)
  ...
  // Episode 1 change:
  // Hi, since we're trying to ship this game we'll just go ahead and make all
  // these doors not fade out instead of changing all the levels.
  if (Q_strcmp(STRING(GetModelName()), "models/props_c17/door01_left.mdl") ==
      0) {
    SetFadeDistance(-1, 0);
    DisableAutoFade();
  }
...
bool CPhysicsProp::CreateVPhysics() {
  ...
  if (m_inertiaScale > 0) {
    ...
    if (tmpSolid.params.inertia < 0.5) tmpSolid.params.inertia = 0.5;
  }

  PhysGetMassCenterOverride(this, modelinfo->GetVCollide(GetModelIndex()),
                            tmpSolid);
  ...
  IPhysicsObject *pPhysicsObject =
      VPhysicsInitNormal(SOLID_VPHYSICS, 0, asleep, &tmpSolid);
  ...
  if (!pPhysicsObject) {
    ...
    if (HasSpawnFlags(SF_PHYSPROP_MOTIONDISABLED) ||
...
bool CPhysicsProp::CanBePickedUpByPhyscannon(void) {
  if (HasSpawnFlags(SF_PHYSPROP_PREVENT_PICKUP)) return false;

  IPhysicsObject *pPhysicsObject = VPhysicsGetObject();
  if (pPhysicsObject && pPhysicsObject->IsMoveable() == false) {
    if (HasSpawnFlags(SF_PHYSPROP_ENABLE_ON_PHYSCANNON) == false) return false;
  }
...
//-----------------------------------------------------------------------------
void CPhysicsProp::InputEnableMotion(inputdata_t &inputdata) { EnableMotion(); }
...
void CPhysicsProp::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (pPhysicsObject && !pPhysicsObject->IsMoveable()) {
    if (!HasSpawnFlags(SF_PHYSPROP_ENABLE_ON_PHYSCANNON)) return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysicsProp::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
                                 PhysGunDrop_t Reason) {
...
//-----------------------------------------------------------------------------
// Purpose: Get the specified key's angles for this prop from the QC's
...
//			value -
//-----------------------------------------------------------------------------
void CPhysicsProp::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
//-----------------------------------------------------------------------------
void CPhysicsProp::ComputeEnablingImpulse(int index,
                                          gamevcollisionevent_t *pEvent) {
  ...
  CrossProduct(vecAngularVelocity, vecLocalContactPoint,
               vecLocalContactVelocity);
  pImpactObject->LocalToWorldVector(&vecContactVelocity,
                                    vecLocalContactVelocity);
  ...
  pEvent->pObjects[index]->CalculateForceOffset(
      vecContactVelocity, vecContactPoint, &vecForce, &vecTorque);
...
void CPhysicsProp::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  if (m_flForceToEnableMotion) {
    ...
    if ((pOther && pOther->IsPlayer() &&
         HasSpawnFlags(SF_PHYSPROP_PREVENT_PLAYER_TOUCH_ENABLE)) == false) {
  ...
  if (!HasSpawnFlags(SF_PHYSPROP_DONT_TAKE_PHYSICS_DAMAGE)) {
    ...
    IBreakableWithPropData *pBreakableInterface =
        assert_cast<IBreakableWithPropData *>(this);
    float damage = CalculateDefaultPhysicsDamage(
        index, pEvent, m_impactEnergyScale, true, damageType,
        pBreakableInterface->GetPhysicsDamageTable());
    if (damage > 0) {
  ...
  if (m_bThrownByPlayer || m_bFirstCollisionAfterLaunch) {
    ...
    if (pHitEntity && pHitEntity->MyNPCPointer()) {
      pHitEntity->MyNPCPointer()->DispatchInteraction(
          g_interactionHitByPlayerThrownPhysObj, this, NULL);
      m_bThrownByPlayer = false;
    }
  }
  ...
  if (m_bFirstCollisionAfterLaunch) {
    ...
    SetContextThink(&CPhysicsProp::ClearFlagsThink, gpGlobals->curtime,
                    "PROP_CLEARFLAGS");
  }
}
...
int CPhysicsProp::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (IsOnFire()) {
    if ((info.GetDamageType() & DMG_BURN) &&
      ...
      if (VPhysicsGetObject()) {
        ...
        if (hl2_episodic.GetBool()) {
          // In Episodic, burning items (such as destroyed APCs) are making very
          // large danger sounds which frighten NPCs. This danger sound was
          // designed to frighten NPCs away from burning objects that are about
          // to explode (barrels, etc). So if this item has no more health (ie,
          // has died but hasn't exploded), make a smaller danger sound, just to
          // keep NPCs away from the flames. I suspect this problem didn't
          // appear in HL2 simply because we didn't have NPCs in such close
          // proximity to destroyed NPCs. (sjb)
          if (GetHealth() < 1) {
            // This item has no health, but still exists. That means that it may
            // keep burning, but isn't likely to explode, so don't frighten over
            // such a large radius.
            dangerRadius = 120;
          }
        }
        ...
        trace_t tr;
        UTIL_TraceLine(WorldSpaceCenter(), WorldSpaceCenter() + vel, MASK_SHOT,
                       this, COLLISION_GROUP_NONE, &tr);
        CSoundEnt::InsertSound(SOUND_DANGER, tr.endpos, dangerRadius, 1.0, this,
                               SOUNDENT_CHANNEL_REPEATED_DANGER);
      }
    }
  }

  // If we have a force to enable motion, and we're still disabled, check to see
  // if this should enable us
  if (m_flForceToEnableMotion) {
    // Large enough to enable motion?
  ...
  if (m_damageToEnableMotion > 0 && GetHealth() < m_damageToEnableMotion) {
    ...
    // The damage that enables motion may have been enough damage to kill me if
    // I'm breakable in which case my physics object is gone.
...
int CPhysicsProp::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    if (VPhysicsGetObject()) {
      char tempstr[512];
      Q_snprintf(tempstr, sizeof(tempstr), "Mass: %.2f kg / %.2f lb (%s)",
                 VPhysicsGetObject()->GetMass(),
                 kg2lbs(VPhysicsGetObject()->GetMass()),
                 GetMassEquivalent(VPhysicsGetObject()->GetMass()));
...

      {
        ...
        Q_snprintf(tempstr, sizeof(tempstr), "Stress: %.2f (%.2f / %.2f)",
                   stress, stressOut.exertedStress, stressOut.receivedStress);
        EntityText(text_offset, tempstr, 0);
        text_offset++;
      }
      ...
      if (m_iszBasePropData != NULL_STRING) {
        Q_snprintf(tempstr, sizeof(tempstr), "Base PropData: %s",
                   STRING(m_iszBasePropData));
        EntityText(text_offset, tempstr, 0);
        text_offset++;
      }
      ...
      if (m_iNumBreakableChunks != 0) {
        ...
        Q_snprintf(tempstr, sizeof(tempstr),
                   "Breakable Chunks: %d (Max Size %d)", m_iNumBreakableChunks,
                   pBreakableInterface->GetMaxBreakableSize());
        EntityText(text_offset, tempstr, 0);
        text_offset++;
      }
      ...
      Q_snprintf(tempstr, sizeof(tempstr), "Health: %d, collision group %d",
                 GetHealth(), GetCollisionGroup());
      EntityText(text_offset, tempstr, 0);
      text_offset++;
    }
  }
...
static CBreakableProp *BreakModelCreate_Prop(
    CBaseEntity *pOwner, breakmodel_t *pModel, const Vector &position,
    const QAngle &angles, const breakablepropparams_t &params) {
  CBreakableProp *pEntity = (CBreakableProp *)CBaseEntity::CreateNoSpawn(
      "prop_physics", position, angles, pOwner);
  if (pEntity) {
    ...
    pEntity->m_impactEnergyScale =
        params.impactEnergyScale;  // assume the same material
    ...
    if (pAnimating && pAnimating->IsOnFire()) {
      CEntityFlame *pOwnerFlame =
          dynamic_cast<CEntityFlame *>(pAnimating->GetEffectEntity());
...
static CBaseAnimating *BreakModelCreate_Ragdoll(CBaseEntity *pOwner,
                                                breakmodel_t *pModel,
                                                const Vector &position,
                                                const QAngle &angles) {
  CBaseAnimating *pAnimating = CreateServerRagdollSubmodel(
      dynamic_cast<CBaseAnimating *>(pOwner), pModel->modelName, position,
      angles, pModel->collisionGroup);
  return pAnimating;
}

CBaseEntity *BreakModelCreateSingle(CBaseEntity *pOwner, breakmodel_t *pModel,
                                    const Vector &position,
                                    const QAngle &angles,
  ...
  if (pEntity) {
    ...
    if (pModel->fadeTime) {
      ...
      if (pProp && !pProp->GetNumBreakableChunks() &&
...
class CBreakModelsPrecached : public CAutoGameSystem {
 public:
  ...
  static bool BreakLessFunc(breakable_precache_t const &lhs,
                            breakable_precache_t const &rhs) {
  ...
  void LevelShutdownPostEntity() { m_modelList.RemoveAll(); }
...
bool PropBreakableCapEdictsOnCreateAll(int modelindex, IPhysicsObject *pPhysics,
                                       const breakablepropparams_t &params,
                                       CBaseEntity *pEntity,
                                       int iPrecomputedBreakableCount = -1) {
  // @Note (toml 10-07-03): this is stop-gap to prevent this function from
  // crashing the engine
  const int BREATHING_ROOM = 64;

  CUtlVector<breakmodel_t> list;
  BreakModelList(list, modelindex, params.defBurstScale,
                 params.defCollisionGroup);
  ...
  if (iPrecomputedBreakableCount != -1) {
    ...
    // Then see if the propdata specifies any breakable pieces
      ...
      IBreakableWithPropData *pBreakableInterface =
          dynamic_cast<IBreakableWithPropData *>(pEntity);
      if (pBreakableInterface &&
          pBreakableInterface->GetBreakableModel() != NULL_STRING &&
          pBreakableInterface->GetBreakableCount()) {
  ...
  return (
      !numToCreate ||
      (engine->GetEntityCount() + numToCreate + BREATHING_ROOM < MAX_EDICTS));
}
...
BEGIN_DATADESC(CBasePropDoor)
  ...
  DEFINE_KEYFIELD(m_nHardwareType, FIELD_INTEGER, "hardware"),
      ...
      DEFINE_KEYFIELD(m_ls.sLockedSound, FIELD_SOUNDNAME,
                      "soundlockedoverride"),
      DEFINE_KEYFIELD(m_ls.sUnlockedSound, FIELD_SOUNDNAME,
      ...
      DEFINE_OUTPUT(m_OnClose, "OnClose"), DEFINE_OUTPUT(m_OnOpen, "OnOpen"),
      DEFINE_OUTPUT(m_OnLockedUse, "OnLockedUse"), DEFINE_EMBEDDED(m_ls),

      // Function Pointers
      DEFINE_THINKFUNC(DoorOpenMoveDone), DEFINE_THINKFUNC(DoorCloseMoveDone),
...
CBasePropDoor::CBasePropDoor(void) { m_hMaster = NULL; }
...
void CBasePropDoor::Spawn() {
  ...
  if ((m_nHardwareType == 0) && (!HasSpawnFlags(SF_DOOR_LOCKED))) {
    ...
    DevWarning(1,
               "Unlocked prop_door '%s' at (%.0f %.0f %.0f) has no hardware. "
               "All openable doors must have hardware!\n",
               GetDebugName(), GetAbsOrigin().x, GetAbsOrigin().y,
               GetAbsOrigin().z);
  }
...
int CBasePropDoor::ObjectCaps() {
  return BaseClass::ObjectCaps() |
         (HasSpawnFlags(SF_DOOR_IGNORE_USE)
              ? 0
...
void CBasePropDoor::Activate(void) {
  ...
  if (GetEntityName() != NULL_STRING) {
    ...
    // Otherwise, see if there are any other doors that match our name
    ...
    while ((pTarget = gEntList.FindEntityByName(pTarget, iszSearchName)) !=
           NULL) {
      if (pTarget != this) {
...
void CBasePropDoor::CalcDoorSounds() {
  ...
  // Otherwise, use the sounds specified by the model keyvalues. These are
  // looked up based on skin and hardware.
  KeyValues *modelKeyValues = new KeyValues("");
  if (modelKeyValues->LoadFromBuffer(
          modelinfo->GetModelName(GetModel()),
          modelinfo->GetModelKeyValueText(GetModel()))) {
    ...
    if (pkvDoorSounds) {
      ...
      if (pkvSkinData) {
        ...
        if (pSurfaceprop && VPhysicsGetObject()) {
          bFoundSkin = true;
          VPhysicsGetObject()->SetMaterialIndex(
              physprops->GetSurfaceIndex(pSurfaceprop));
        }
      }
      ...
      if (pkvHardwareData) {
        strSoundLocked =
            AllocPooledString(pkvHardwareData->GetString("locked"));
        strSoundUnlocked =
            AllocPooledString(pkvHardwareData->GetString("unlocked"));
      }

      // If any sounds were missing, try the "defaults" block.
      if ((strSoundOpen == NULL_STRING) || (strSoundClose == NULL_STRING) ||
          (strSoundMoving == NULL_STRING) || (strSoundLocked == NULL_STRING) ||
          (strSoundUnlocked == NULL_STRING)) {
        KeyValues *pkvDefaults = pkvDoorSounds->FindKey("defaults");
        if (pkvDefaults) {
          ASSIGN_STRING_IF_NULL(
              strSoundOpen, AllocPooledString(pkvDefaults->GetString("open")));
          ASSIGN_STRING_IF_NULL(
              strSoundClose,
              AllocPooledString(pkvDefaults->GetString("close")));
          ASSIGN_STRING_IF_NULL(
              strSoundMoving,
              AllocPooledString(pkvDefaults->GetString("move")));
          ASSIGN_STRING_IF_NULL(
              strSoundLocked,
              AllocPooledString(pkvDefaults->GetString("locked")));
          ASSIGN_STRING_IF_NULL(
              strSoundUnlocked,
              AllocPooledString(pkvDefaults->GetString("unlocked")));
  ...
  if (!bFoundSkin && VPhysicsGetObject()) {
    Warning(
        "%s has Door model (%s) with no door_options! Verify that SKIN is "
        "valid, and has a corresponding options block in the model QC file\n",
        GetDebugName(), modelinfo->GetModelName(GetModel()));
    VPhysicsGetObject()->SetMaterialIndex(physprops->GetSurfaceIndex("wood"));
  }

  // Any sound data members that are already filled out were specified as level
  // designer overrides, so they should not be overwritten.
...
void CBasePropDoor::UpdateAreaPortals(bool isOpen) {
  ...
  while ((pPortal = gEntList.FindEntityByClassname(
              pPortal, "func_areaportal")) != NULL) {
...
//			value -
//-----------------------------------------------------------------------------
void CBasePropDoor::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                        USE_TYPE useType, float value) {
  if (GetMaster() != NULL) {
...
//			value -
//-----------------------------------------------------------------------------
void CBasePropDoor::OnUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                          USE_TYPE useType, float value) {
...
//-----------------------------------------------------------------------------
void CBasePropDoor::InputLock(inputdata_t &inputdata) { Lock(); }
...
//-----------------------------------------------------------------------------
// Purpose: Opens the door away from a specified entity if it is not already
// open.
//-----------------------------------------------------------------------------
void CBasePropDoor::InputOpenAwayFrom(inputdata_t &inputdata) {
  CBaseEntity *pOpenAwayFrom =
      gEntList.FindEntityByName(NULL, inputdata.value.String(), NULL,
                                inputdata.pActivator, inputdata.pCaller);
...
// Input  : *pOpenAwayFrom -
//-----------------------------------------------------------------------------
void CBasePropDoor::OpenIfUnlocked(CBaseEntity *pActivator,
                                   CBaseEntity *pOpenAwayFrom) {
...
//-----------------------------------------------------------------------------
void CBasePropDoor::InputUnlock(inputdata_t &inputdata) { Unlock(); }

//-----------------------------------------------------------------------------
// Purpose: Locks the door so that it cannot be opened.
//-----------------------------------------------------------------------------
void CBasePropDoor::Lock(void) { m_bLocked = true; }
...
void CBasePropDoor::Unlock(void) {
  if (!m_nHardwareType) {
    // Doors with no hardware must always be locked.
    DevWarning(1,
               "Unlocking prop_door '%s' at (%.0f %.0f %.0f) with no hardware. "
               "All openable doors must have hardware!\n",
               GetDebugName(), GetAbsOrigin().x, GetAbsOrigin().y,
               GetAbsOrigin().z);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Causes the door to "do its thing", i.e. start moving, and cascade
// activation.
//-----------------------------------------------------------------------------
...
void CBasePropDoor::DoorOpen(CBaseEntity *pOpenAwayFrom) {
  ...
  if (IsDoorOpen() || IsDoorOpening()) return;
  ...
  // Emit door moving and stop sounds on CHAN_STATIC so that the multicast
  // doesn't filter them out and leave a client stuck with looping door sounds!
  if (!HasSpawnFlags(SF_DOOR_SILENT)) {
    EmitSound(STRING(m_SoundMoving));

    if (m_hActivator && m_hActivator->IsPlayer() &&
        !HasSpawnFlags(SF_DOOR_SILENT_TO_NPCS)) {
      CSoundEnt::InsertSound(SOUND_PLAYER, GetAbsOrigin(), 512, 0.5,
  ...
  // Virtual function that starts the door moving for whatever type of door this
  // is.
...
void CBasePropDoor::DoorAutoCloseThink(void) {
  // When autoclosing, we check both sides so that we don't close in the
  // player's face, or in an NPC's face for that matter, because they might be
...
void CBasePropDoor::DoorClose(void) {
  // Don't bother if we're already doing this
  if (IsDoorClosed() || IsDoorClosing()) return;
...
//-----------------------------------------------------------------------------
// Purpose: Called the first frame that the door is blocked while opening or
// closing. Input  : pOther - The blocking entity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Called every frame when the door is blocked while opening or
// closing. Input  : pOther - The blocking entity.
//-----------------------------------------------------------------------------
void CBasePropDoor::Blocked(CBaseEntity *pOther) {
  ...
  if (m_bForceClosed && (pOther->GetMoveType() == MOVETYPE_VPHYSICS) &&
      (pOther->m_takedamage == DAMAGE_NO ||
       pOther->m_takedamage == DAMAGE_EVENTS_ONLY)) {
    EntityPhysics_CreateSolver(this, pOther, true, 4.0f);
  } else if (m_bForceClosed && (pOther->GetMoveType() == MOVETYPE_VPHYSICS) &&
             (pOther->m_takedamage == DAMAGE_YES)) {
    pOther->TakeDamage(
        CTakeDamageInfo(this, this, pOther->GetHealth(), DMG_CRUSH));
  }

  // If we're set to force ourselves closed, keep going
  if (m_bForceClosed) return;
...
//-----------------------------------------------------------------------------
// Purpose: Called the first frame that the door is unblocked while opening or
// closing.
//-----------------------------------------------------------------------------
...
bool CBasePropDoor::TestCollision(const Ray_t &ray, unsigned int mask,
                                  trace_t &trace) {
  if (!VPhysicsGetObject()) return false;

  CStudioHdr *pStudioHdr = GetModelPtr();
  if (!pStudioHdr) return false;

  if (!(pStudioHdr->contents() & mask)) return false;

  physcollision->TraceBox(ray, VPhysicsGetObject()->GetCollide(),
                          GetAbsOrigin(), GetAbsAngles(), &trace);
...
// Will only test against entities and rejects physics objects below a mass
// threshold
//-----------------------------------------------------------------------------
...
class CTraceFilterDoor : public CTraceFilterEntitiesOnly {
 public:
  ...
  CTraceFilterDoor(const IHandleEntity *pDoor, const IHandleEntity *passentity,
                   int collisionGroup)
      : m_pDoor(pDoor),
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (!PassServerEntityFilter(pHandleEntity, m_pDoor)) return false;
    ...
    if (pEntity) {
      ...
      if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
        ...
        if (pPhysics->IsMoveable() && pPhysics->GetMass() < 32) return false;
      }
    }
...
inline void TraceHull_Door(const CBasePropDoor *pDoor,
                           const Vector &vecAbsStart, const Vector &vecAbsEnd,
                           const Vector &hullMin, const Vector &hullMax,
                           unsigned int mask, const CBaseEntity *ignore,
...
class CPropDoorRotating : public CBasePropDoor {
 ...
 private:
  ...
  void CalculateDoorVolume(QAngle closedAngles, QAngle openAngles,
  ...
  void InputSetRotationDistance(
      inputdata_t
          &inputdata);  // Set the degree difference between open and closed

  void CalcOpenAngles(void);  // Subroutine to setup the m_angRotation QAngles
                              // based on the m_flDistance variable
  ...
  QAngle m_angRotationOpenForward;  // Our angles when we are fully open towards
                                    // our forward vector.
  QAngle m_angRotationOpenBack;  // Our angles when we are fully open away from
                                 // our forward vector.
...
BEGIN_DATADESC(CPropDoorRotating)
  DEFINE_KEYFIELD(m_eSpawnPosition, FIELD_INTEGER, "spawnpos"),
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetRotationDistance",
...
//			*destMaxs -
//-----------------------------------------------------------------------------
void UTIL_ComputeAABBForBounds(const Vector &mins1, const Vector &maxs1,
...
void CPropDoorRotating::Spawn() {
  ...
  // NOTE: If you change this, be sure to change IsHingeOnLeft to account for
  // it!
  ...
  CalculateDoorVolume(GetLocalAngles(), m_angRotationOpenForward,
                      &m_vecForwardBoundsMin, &m_vecForwardBoundsMax);
  CalculateDoorVolume(GetLocalAngles(), m_angRotationOpenBack,
                      &m_vecBackBoundsMin, &m_vecBackBoundsMax);
}

//-----------------------------------------------------------------------------
// Purpose: Setup the m_angRotationOpenForward and m_angRotationOpenBack
// variables based on
//			the m_flDistance variable. Also restricts m_flDistance >
// 0.
//-----------------------------------------------------------------------------
void CPropDoorRotating::CalcOpenAngles() {
  ...
  m_angRotationOpenForward.x =
      m_angRotationClosed.x - (vecMoveDir.x * m_flDistance);
  m_angRotationOpenForward.y =
      m_angRotationClosed.y - (vecMoveDir.y * m_flDistance);
  m_angRotationOpenForward.z =
      m_angRotationClosed.z - (vecMoveDir.z * m_flDistance);

  m_angRotationOpenBack.x =
      m_angRotationClosed.x + (vecMoveDir.x * m_flDistance);
  m_angRotationOpenBack.y =
      m_angRotationClosed.y + (vecMoveDir.y * m_flDistance);
  m_angRotationOpenBack.z =
      m_angRotationClosed.z + (vecMoveDir.z * m_flDistance);
}
...
doorCheck_e CPropDoorRotating::GetOpenState(void) {
  return (m_angGoal == m_angRotationOpenForward) ? DOOR_CHECK_FORWARD
                                                 : DOOR_CHECK_BACKWARD;
}
...
void CPropDoorRotating::OnDoorOpened(void) {
  if (m_hDoorBlocker != NULL) {
    ...
    if (g_debug_doors.GetBool()) {
      NDebugOverlay::Box(
          GetAbsOrigin(), m_hDoorBlocker->CollisionProp()->OBBMins(),
          m_hDoorBlocker->CollisionProp()->OBBMaxs(), 0, 255, 0, true, 1.0f);
    }
  }
}
...
bool CPropDoorRotating::DoorCanClose(bool bAutoClose) {
  if (GetMaster() != NULL) return GetMaster()->DoorCanClose(bAutoClose);
  ...
  if (HasSlaves()) {
    ...
    for (int i = 0; i < numDoors; i++) {
      pLinkedDoor =
          dynamic_cast<CPropDoorRotating *>((CBasePropDoor *)m_hDoorList[i]);

      if (pLinkedDoor != NULL) {
        if (!pLinkedDoor->CheckDoorClear(
                bAutoClose ? DOOR_CHECK_FULL : pLinkedDoor->GetOpenState()))
          return false;
      }
    }
  }
...
//			*destMaxs -
//-----------------------------------------------------------------------------
void CPropDoorRotating::CalculateDoorVolume(QAngle closedAngles,
                                            QAngle openAngles, Vector *destMins,
                                            Vector *destMaxs) {
  ...
  // Find the minimum extents
  UTIL_ComputeAABBForBounds(closedMins, closedMaxs, openMins, openMaxs,
                            destMins, destMaxs);
...
bool CPropDoorRotating::CheckDoorClear(doorCheck_e state) {
  ...
  switch (state) {
    ...
    case DOOR_CHECK_FULL:
      UTIL_ComputeAABBForBounds(m_vecForwardBoundsMin, m_vecForwardBoundsMax,
                                m_vecBackBoundsMin, m_vecBackBoundsMax,
                                &moveMins, &moveMaxs);
      break;
  }

  // Look for blocking entities, ignoring ourselves and the entity that opened
  // us.
  trace_t tr;
  TraceHull_Door(this, GetAbsOrigin(), GetAbsOrigin(), moveMins, moveMaxs,
                 MASK_SOLID, GetActivator(), COLLISION_GROUP_NONE, &tr);
  if (tr.allsolid || tr.startsolid) {
    if (g_debug_doors.GetBool()) {
      NDebugOverlay::Box(GetAbsOrigin(), moveMins, moveMaxs, 255, 0, 0, true,
                         10.0f);

      if (tr.m_pEnt) {
        NDebugOverlay::Box(
            tr.m_pEnt->GetAbsOrigin(), tr.m_pEnt->CollisionProp()->OBBMins(),
            tr.m_pEnt->CollisionProp()->OBBMaxs(), 220, 220, 0, true, 10.0f);
      }
    }
  ...
  if (g_debug_doors.GetBool()) {
    NDebugOverlay::Box(GetAbsOrigin(), moveMins, moveMaxs, 0, 255, 0, true,
                       10.0f);
  }
...
//-----------------------------------------------------------------------------
// Purpose: After rotating, set angle to exact final angle, call "move done"
// function.
//-----------------------------------------------------------------------------
...
void CPropDoorRotating::BeginOpening(CBaseEntity *pOpenAwayFrom) {
  ...
  } else  // Can open either direction, test to see which is appropriate
  {
    if (pOpenAwayFrom != NULL) {
      ...
      if (vecForwardDoor.Dot(pOpenAwayFrom->GetAbsOrigin()) >
          vecForwardDoor.Dot(GetAbsOrigin())) {
        angOpen = m_angRotationOpenBack;
        eDirCheck = DOOR_CHECK_BACKWARD;
      }
    }
    ...
    if (IsPlayerOpening() && (pOpenAwayFrom && pOpenAwayFrom->IsPlayer()) &&
        !CheckDoorClear(eDirCheck)) {
  ...
  m_hDoorBlocker =
      CEntityBlocker::Create(GetAbsOrigin(), mins, maxs, pOpenAwayFrom, false);
  ...
  // Debug
  if (g_debug_doors.GetBool()) {
    NDebugOverlay::Cross3D(volumeCenter,
                           -Vector(volumeRadius, volumeRadius, volumeRadius),
                           Vector(volumeRadius, volumeRadius, volumeRadius),
                           255, 0, 0, true, 1.0f);
  }
  ...
  if (!HasSpawnFlags(SF_DOOR_SILENT_TO_NPCS)) {
    CSoundEnt::InsertSound(SOUND_MOVE_AWAY, volumeCenter, volumeRadius, 0.5f,
                           pOpenAwayFrom);
  }
  ...
  if (m_hDoorBlocker != NULL) {
    ...
    if (g_debug_doors.GetBool()) {
      NDebugOverlay::Box(
          GetAbsOrigin(), m_hDoorBlocker->CollisionProp()->OBBMins(),
          m_hDoorBlocker->CollisionProp()->OBBMaxs(), 255, 0, 0, true, 1.0f);
    }
  }
...
//			opendata -
//-----------------------------------------------------------------------------
void CPropDoorRotating::GetNPCOpenData(CAI_BaseNPC *pNPC,
                                       opendata_t &opendata) {
...
int CPropDoorRotating::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "Avelocity: %.2f %.2f %.2f",
 ...
 public:
  ...
  bool CreateVPhysics() {
    ...
    IPhysicsObject *pPhysicsObject = physenv->CreateSphereObject(
        12, 0, GetAbsOrigin(), GetAbsAngles(), &params, false);
    if (pPhysicsObject) {
...
void CPropDoorRotating::InputSetSpeed(inputdata_t &inputdata) {
  AssertMsg1(inputdata.value.Float() > 0.0f,
             "InputSetSpeed on %s called with negative parameter!",
             GetDebugName());
...
class CPhysBoxMultiplayer : public CPhysBox, public IMultiplayerPhysics {
 public:
  ...
  virtual bool IsAsleep() { return VPhysicsGetObject()->IsAsleep(); }
...
SendPropInt(SENDINFO(m_iPhysicsMode), 1, SPROP_UNSIGNED),
  ...
  virtual void Spawn(void) {
    ...
    // VPhysicsGetObject() is NULL on the client, which prevents the client from
    // finding a decent AABB surrounding the collision bounds.  If we've got a
    // VPhysicsGetObject()->GetCollide(), we'll grab it's unrotated bounds and
    // use it to calculate our collision surrounding bounds.  This can end up
    // larger than the CollisionProp() would have calculated on its own, but
    // it'll be identical on the client and the server.
    m_usingCustomCollisionBounds = false;
    if ((GetSolid() == SOLID_VPHYSICS) &&
      ...
      if (pPhysics && pPhysics->GetCollide()) {
        physcollision->CollideGetAABB(
            &m_collisionMins.GetForModify(), &m_collisionMaxs.GetForModify(),
            pPhysics->GetCollide(), vec3_origin, vec3_angle);
  ...
  virtual void ComputeWorldSpaceSurroundingBox(Vector *mins, Vector *maxs) {
    ...
    TransformAABB(EntityToWorldTransform(), m_collisionMins, m_collisionMaxs,
                  *mins, *maxs);
  }
...
void CPhysicsPropRespawnable::Materialize(void) {
  trace_t tr;
  UTIL_TraceHull(m_vOriginalSpawnOrigin, m_vOriginalSpawnOrigin,
                 m_vOriginalMins, m_vOriginalMaxs, MASK_SOLID, this,
                 COLLISION_GROUP_NONE, &tr);
...
void CC_Prop_Dynamic_Create(const CCommand &args) {
  ...
  CDynamicProp *pProp =
      dynamic_cast<CDynamicProp *>(CreateEntityByName("dynamic_prop"));
  if (pProp) {
    char buf[512];
    // Pass in standard key values
    Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", tr.endpos.x, tr.endpos.y,
               tr.endpos.z);
    pProp->KeyValue("origin", buf);
    Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", angles.x, angles.y,
               angles.z);
...
static ConCommand prop_dynamic_create(
    "prop_dynamic_create", CC_Prop_Dynamic_Create,
    "Creates a dynamic prop with a specific .mdl aimed away from where the "
    "player is looking.\n\tArguments: {.mdl name}",
    FCVAR_CHEAT);
...
void CC_Prop_Physics_Create(const CCommand &args) {
  ...
  // Must have vphysics to place as a physics prop
  if (!mdlcache->GetVCollide(h)) return;
  ...
  UTIL_TraceHull(pPlayer->EyePosition(),
                 pPlayer->EyePosition() + forward * MAX_TRACE_LENGTH,
                 vecSweepMins, vecSweepMaxs, MASK_NPCSOLID, pPlayer,
                 COLLISION_GROUP_NONE, &tr);

  // No hit? We're done.
  if (tr.fraction == 1.0 || tr.allsolid) return;
  ...
  // Try to create entity
  CPhysicsProp *pProp =
      dynamic_cast<CPhysicsProp *>(CreateEntityByName("physics_prop"));
  if (pProp) {
    char buf[512];
    // Pass in standard key values
    Q_snprintf(buf, sizeof(buf), "%.10f %.10f %.10f", tr.endpos.x, tr.endpos.y,
...
static ConCommand prop_physics_create(
    "prop_physics_create", CC_Prop_Physics_Create,
    "Creates a physics prop with a specific .mdl aimed away from where the "
    "player is looking.\n\tArguments: {.mdl name}",
    FCVAR_CHEAT);
...
static ConCommand ent_rotate("ent_rotate", CC_Ent_Rotate,
