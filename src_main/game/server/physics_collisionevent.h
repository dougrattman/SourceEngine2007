...
class CCollisionEvent : public IPhysicsCollisionEvent,
 ...
 public:
  ...
  virtual void AddDamageEvent(CBaseEntity *pEntity, const CTakeDamageInfo &info,
                              IPhysicsObject *pInflictorPhysics,
  ...
  int AdditionalCollisionChecksThisTick(int currentChecksDone) {
    ...
    if (currentChecksDone < 2400) {
      DevMsg(1,
             "%s: VPhysics Collision detection getting expensive, "
             "check for too many convex pieces!\n",
             ModuleName());
      return 2400 - currentChecksDone;
    }
    DevMsg(1,
