...
//-----------------------------------------------------------------------------
// Purpose: Default test implementation. Should only be called by derived
// classes Input  : *current_origin -
//			*current_angles -
//-----------------------------------------------------------------------------
void CBaseTempEntity::Test(const Vector &current_origin,
...
//-----------------------------------------------------------------------------
// Purpose: Called at startup to allow temp entities to precache any
// models/sounds that they need
//-----------------------------------------------------------------------------
...
void CBaseTempEntity::Create(IRecipientFilter &filter, float delay) {
  // temp entities can't be reliable or part of the signon message, use real
  // entities instead
  Assert(!filter.IsInitMessage() && !filter.IsInitMessage());
  Assert(delay >= -1 && delay <= 1);  // 1 second max delay

  engine->PlaybackTempEntity(filter, delay, (void *)this,
