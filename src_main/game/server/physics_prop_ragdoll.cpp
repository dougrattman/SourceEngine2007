...
SendPropArray(SendPropQAngles(SENDINFO_ARRAY(m_ragAngles), 13, 0), m_ragAngles),
    SendPropArray(SendPropVector(SENDINFO_ARRAY(m_ragPos), -1, SPROP_COORD),
                  m_ragPos),
...
#define DEFINE_RAGDOLL_ELEMENT(i)                                  \
  DEFINE_FIELD(m_ragdoll.list[i].originParentSpace, FIELD_VECTOR), \
    ...
    // think functions
    DEFINE_THINKFUNC(SetDebrisThink), DEFINE_THINKFUNC(ClearFlagsThink),
    ...
    DEFINE_RAGDOLL_ELEMENT(1), DEFINE_RAGDOLL_ELEMENT(2),
    DEFINE_RAGDOLL_ELEMENT(3), DEFINE_RAGDOLL_ELEMENT(4),
    DEFINE_RAGDOLL_ELEMENT(5), DEFINE_RAGDOLL_ELEMENT(6),
    DEFINE_RAGDOLL_ELEMENT(7), DEFINE_RAGDOLL_ELEMENT(8),
    DEFINE_RAGDOLL_ELEMENT(9), DEFINE_RAGDOLL_ELEMENT(10),
    DEFINE_RAGDOLL_ELEMENT(11), DEFINE_RAGDOLL_ELEMENT(12),
    DEFINE_RAGDOLL_ELEMENT(13), DEFINE_RAGDOLL_ELEMENT(14),
    DEFINE_RAGDOLL_ELEMENT(15), DEFINE_RAGDOLL_ELEMENT(16),
    DEFINE_RAGDOLL_ELEMENT(17), DEFINE_RAGDOLL_ELEMENT(18),
    DEFINE_RAGDOLL_ELEMENT(19), DEFINE_RAGDOLL_ELEMENT(20),
    DEFINE_RAGDOLL_ELEMENT(21), DEFINE_RAGDOLL_ELEMENT(22),
...
void CRagdollProp::Spawn(void) {
  ...
  // NOTE: If this fires, then the assert or the datadesc is wrong!  (see
  // DEFINE_RAGDOLL_ELEMENT above)
  ...
  InitRagdoll(vec3_origin, 0, vec3_origin, pBoneToWorld, pBoneToWorld, 0,
              collisionGroup, true, bWake);
...
void CRagdollProp::OnSave(IEntitySaveUtils *pUtils) {
  ...
  VPhysicsSetObject(
      m_ragdoll.list[0]
          .pObject);  // make sure object zero is saved by CBaseEntity
...
void CRagdollProp::OnRestore() {
  ...
  RagdollSetupCollisions(m_ragdoll, modelinfo->GetVCollide(GetModelIndex()),
                         GetModelIndex());
...
// Purpose:
//-----------------------------------------------------------------------------
void CRagdollProp::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (HasPhysgunInteraction("onpickup", "boogie")) {
    if (reason == PUNTED_BY_CANNON) {
      CRagdollBoogie::Create(this, 150, gpGlobals->curtime, 3.0f,
                             SF_RAGDOLL_BOOGIE_ELECTRICAL);
    } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void CRagdollProp::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
  ...
  if (GetCollisionGroup() == COLLISION_GROUP_INTERACTIVE_DEBRIS) {
    SetContextThink(&CRagdollProp::SetDebrisThink, gpGlobals->curtime + 5,
                    s_pDebrisContext);
  }

  if (Reason != LAUNCHED_BY_CANNON) return;
...
void CRagdollProp::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  if (m_takedamage != DAMAGE_NO) {
    int damageType = 0;
    float damage =
        CalculateDefaultPhysicsDamage(index, pEvent, 1.0f, true, damageType);
    if (damage > 0) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CRagdollProp::HasPhysgunInteraction(const char *pszKeyName,
                                         const char *pszValue) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CRagdollProp::HandleFirstCollisionInteractions(
  ...
  if (HasPhysgunInteraction("onfirstimpact", "paintsplat")) {
    ...
    trace_t tr;
    UTIL_TraceLine(vecPos, vecPos + pEvent->preVelocity[0] * 1.5, MASK_SHOT,
  ...
  bool bAlienBloodSplat =
      HasPhysgunInteraction("onfirstimpact", "alienbloodsplat");
  if (bAlienBloodSplat ||
      HasPhysgunInteraction("onfirstimpact", "bloodsplat")) {
    IPhysicsObject *pObj = VPhysicsGetObject();
    ...
    UTIL_BloodDecalTrace(
        &tr, bAlienBloodSplat ? BLOOD_COLOR_GREEN : BLOOD_COLOR_RED);
  }
}
...
void CRagdollProp::InitRagdoll(const Vector &forceVector, int forceBone,
                               const Vector &forcePos, matrix3x4_t *pPrevBones,
                               matrix3x4_t *pBoneToWorld, float dt,
                               int collisionGroup, bool activateRagdoll,
                               bool bWakeRagdoll) {
  ...
  if (m_anglesOverrideString != NULL_STRING &&
      Q_strlen(m_anglesOverrideString.ToCStr()) > 0) {
    char szToken[2048];
    const char *pStr = nexttoken(szToken, STRING(m_anglesOverrideString), ',');
    // anglesOverride is index,angles,index,angles (e.g. "1, 22.5 123.0 0.0, 2,
    // 0 0 0, 3, 0 0 180.0")
    while (szToken[0] != 0) {
      ...
      if (objectIndex >= m_ragdoll.listCount) {
        Warning(
            ...
            GetDebugName(), GetModelName().ToCStr(),
        ...
        if (element.parentIndex >= 0) {
          ...
          VectorTransform(element.originParentSpace,
                          pBoneToWorld[parentBoneIndex], out);
        } else {
          out = GetAbsOrigin();
        }
  ...
  for (int i = 0; i < m_ragdoll.listCount; i++) {
    ...
    g_pPhysSaveRestoreManager->AssociateModel(m_ragdoll.list[i].pObject,
                                              GetModelIndex());
    physcollision->CollideGetAABB(&m_ragdollMins[i], &m_ragdollMaxs[i],
                                  m_ragdoll.list[i].pObject->GetCollide(),
...
//-----------------------------------------------------------------------------
// Purpose: Force all the ragdoll's bone's physics objects to recheck their
// collision filters
//-----------------------------------------------------------------------------
...
void CRagdollProp::TraceAttack(const CTakeDamageInfo &info, const Vector &dir,
                               trace_t *ptr) {
...
void CRagdollProp::SetupBones(matrix3x4_t *pBoneToWorld, int boneMask) {
  ...
  for (i = 0; i < m_ragdoll.listCount; i++) {
    ...
    if (!m_ragdoll.list[i].pObject) continue;
  ...
  for (i = 0; i < pStudioHdr->numbones(); i++) {
    ...
    ConcatTransforms(pBoneToWorld[pbones[i].parent], matBoneLocal,
                     pBoneToWorld[i]);
  }
}

bool CRagdollProp::TestCollision(const Ray_t &ray, unsigned int mask,
                                 trace_t &trace) {
#if 0
  ...
  for (int i = 0; i < m_ragdoll.listCount; i++) {
    ...
    if (m_ragdoll.list[i].pObject) {
      ...
      physcollision->TraceBox(ray, m_ragdoll.list[i].pObject->GetCollide(),
                              position, angles, &tr);
...
void CRagdollProp::Teleport(const Vector *newPosition, const QAngle *newAngles,
  ...
  // so set object 0 up and compute the origin/angles for its new position (base
...
void CRagdollProp::VPhysicsUpdate(IPhysicsObject *pPhysics) {
  if (m_lastUpdateTickCount == (unsigned int)gpGlobals->tickcount) return;
  ...
  if (m_allAsleep &&
      GetCollisionGroup() == COLLISION_GROUP_INTERACTIVE_DEBRIS) {
    SetCollisionGroup(COLLISION_GROUP_DEBRIS);
    RecheckCollisionFilter();
...
void CRagdollProp::UpdateNetworkDataFromVPhysics(IPhysicsObject *pPhysics,
                                                 int index) {
...
void CRagdollProp::FadeOut(float flDelay, float fadeTime) {
  ...
  m_flFadeScale = 0;
  SetContextThink(&CRagdollProp::FadeOutThink,
                  gpGlobals->curtime + flDelay + 0.01f, s_pFadeOutContext);
}
...
void CRagdollProp::FadeOutThink(void) {
  ...
  if (dt < 0) {
    SetContextThink(&CRagdollProp::FadeOutThink, gpGlobals->curtime + 0.1,
                    s_pFadeOutContext);
    ...
    NetworkStateChanged();
    SetContextThink(&CRagdollProp::FadeOutThink,
                    gpGlobals->curtime + TICK_INTERVAL, s_pFadeOutContext);
  } else {
...
int CRagdollProp::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    if (m_ragdoll.listCount) {
      ...
      char tempstr[512];
      Q_snprintf(tempstr, sizeof(tempstr), "Mass: %.2f kg / %.2f lb (%s)", mass,
                 kg2lbs(mass), GetMassEquivalent(mass));
  ...
  if (m_debugOverlays & OVERLAY_PIVOT_BIT) {
    for (int i = 0; i < m_ragdoll.listCount; i++) {
      if (m_ragdoll.list[i].pObject) {
        ...
        NDebugOverlay::EntityTextAtPosition(pos, 0, str.Access(), 0, 0, 255, 0,
                                            255);
      }
    }
  }
...
class CRagdollPropAttached : public CRagdollProp {
 ...
 public:
  CRagdollPropAttached() { m_bShouldDetach = false; }
  ...
  void InitRagdollAttached(IPhysicsObject *pAttached, const Vector &forceVector,
                           int forceBone, matrix3x4_t *pPrevBones,
                           matrix3x4_t *pBoneToWorld, float dt,
                           int collisionGroup, CBaseAnimating *pFollow,
                           int boneIndexRoot, const Vector &boneLocalOrigin,
...
SendPropInt(SENDINFO(m_boneIndexAttached), MAXSTUDIOBONEBITS, SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_ragdollAttachedObjectIndex), RAGDOLL_INDEX_BITS,
                SPROP_UNSIGNED),
...
CBaseAnimating *CreateServerRagdollSubmodel(CBaseAnimating *pOwner,
                                            const char *pModelName,
                                            const Vector &position,
                                            const QAngle &angles,
                                            int collisionGroup) {
  CRagdollProp *pRagdoll = (CRagdollProp *)CBaseEntity::CreateNoSpawn(
      "prop_ragdoll", position, angles, pOwner);
  ...
  pRagdoll->InitRagdoll(vec3_origin, -1, vec3_origin, pBoneToWorld,
                        pBoneToWorldNext, 0.1, collisionGroup, true);
  return pRagdoll;
}

CBaseEntity *CreateServerRagdoll(CBaseAnimating *pAnimating, int forceBone,
                                 const CTakeDamageInfo &info,
                                 int collisionGroup, bool bUseLRURetirement) {
  if (info.GetDamageType() & (DMG_VEHICLE | DMG_CRUSH)) {
    // if the entity was killed by physics or a vehicle, move to the vphysics
    // shadow position before creating the ragdoll.
    SyncAnimatingWithPhysics(pAnimating);
  }
  CRagdollProp *pRagdoll = (CRagdollProp *)CBaseEntity::CreateNoSpawn(
      "prop_ragdoll", pAnimating->GetAbsOrigin(), vec3_angle, NULL);
  ...
  // NPC_STATE_DEAD npc's will have their COND_IN_PVS cleared, so this needs to
  // force SetupBones to happen
  unsigned short fPrevFlags = pAnimating->GetBoneCacheFlags();
  pAnimating->SetBoneCacheFlags(BCF_NO_ANIMATION_SKIP);

  // UNDONE: Extract velocity from bones via animation (like we do on the
  // client) UNDONE: For now, just move each bone by the total entity velocity
  // if set. Get Bones positions before Store current cycle
  float fSequenceDuration =
      pAnimating->SequenceDuration(pAnimating->GetSequence());
  ...
  if ((vel.Length() == 0) && (dt > 0)) {
    ...
    if (pstudiohdr) {
      Vector deltaPos;
      QAngle deltaAngles;
      if (Studio_SeqMovement(pstudiohdr, pAnimating->GetSequence(),
                             fPreviousCycle, pAnimating->GetCycle(),
                             pAnimating->GetPoseParameterArray(), deltaPos,
                             deltaAngles)) {
  ...
  pAnimating->DrawRawSkeleton(pBoneToWorld, BONE_USED_BY_ANYTHING, true, 20,
                              false);
  pAnimating->DrawRawSkeleton(pBoneToWorldNext, BONE_USED_BY_ANYTHING, true, 20,
                              true);
#endif
  ...
  if ((info.GetDamageType() & DMG_VEHICLE) && pAnimating->MyNPCPointer()) {
    ...
    for (int i = 0; i < count; i++) {
      int physBone =
          pAnimating->GetPhysicsBone(pAnimating->GetHitboxBone(boxList[i]));
      IPhysicsObject *pPhysics = pRagInfo->list[physBone].pObject;
      pPhysics->ApplyForceCenter(info.GetDamageForce() * pPhysics->GetMass() *
                                 massScale);
    }
  } else {
    pRagdoll->InitRagdoll(info.GetDamageForce(), forceBone,
                          info.GetDamagePosition(), pBoneToWorld,
                          pBoneToWorldNext, dt, collisionGroup, true);
  }
  ...
  // Tracker 22598:  If we don't set the OBB mins/maxs to something valid here,
  // then the client will have a zero sized hull
  //  for the ragdoll for one frame until Vphysics updates the real obb bounds
  //  after the first simulation frame.  Having a zero sized hull makes the
  //  ragdoll think it should be faded/alpha'd to zero for a frame, so you get a
  //  blink where the ragdoll doesn't draw initially.
  Vector mins, maxs;
...
void CRagdollPropAttached::InitRagdollAttached(
    IPhysicsObject *pAttached, const Vector &forceVector, int forceBone,
    matrix3x4_t *pPrevBones, matrix3x4_t *pBoneToWorld, float dt,
    int collisionGroup, CBaseAnimating *pFollow, int boneIndexRoot,
    const Vector &boneLocalOrigin, int parentBoneAttach,
    const Vector &worldAttachOrigin) {
  ...
  InitRagdoll(forceVector, forceBone, vec3_origin, pPrevBones, pBoneToWorld, dt,
  ...
  ConcatTransforms(worldToReference, constraintToWorld,
                   constraint.constraintToReference);
  ConcatTransforms(worldToAttached, constraintToWorld,
                   constraint.constraintToAttached);

  // for now, just slam this to be the passed in value
  MatrixSetColumn(attachmentPointRagdollSpace, 3,
                  constraint.constraintToReference);

  PhysDisableEntityCollisions(pAttached, m_ragdoll.list[0].pObject);
  m_pAttachConstraint = physenv->CreateRagdollConstraint(
      pRefObject, pAttached, m_ragdoll.pGroup, constraint);

  SetParent(pFollow);
  SetOwnerEntity(pFollow);

  RagdollActivate(m_ragdoll, modelinfo->GetVCollide(GetModelIndex()),
                  GetModelIndex());
...
CRagdollProp *CreateServerRagdollAttached(
    CBaseAnimating *pAnimating, const Vector &vecForce, int forceBone,
    int collisionGroup, IPhysicsObject *pAttached,
  ...
  CRagdollPropAttached *pRagdoll =
      (CRagdollPropAttached *)CBaseEntity::CreateNoSpawn(
          "prop_ragdoll_attached", pAnimating->GetAbsOrigin(), vec3_angle,
          NULL);
  pRagdoll->CopyAnimationDataFrom(pAnimating);
  ...
  matrix3x4_t pBoneToWorld[MAXSTUDIOBONES];
  pAnimating->SetupBones(pBoneToWorld, BONE_USED_BY_ANYTHING);
  pRagdoll->InitRagdollAttached(pAttached, vecForce, forceBone, pBoneToWorld,
                                pBoneToWorld, 0.1, collisionGroup,
                                pParentEntity, boneAttach, boneOrigin,
                                parentBoneAttach, originAttached);
...
void DetachAttachedRagdoll(CBaseEntity *pRagdollIn) {
  CRagdollPropAttached *pRagdoll =
      dynamic_cast<CRagdollPropAttached *>(pRagdollIn);
...
void CRagdollProp::GetAngleOverrideFromCurrentState(char *pOut, int size) {
  ...
  for (int i = 0; i < m_ragdoll.listCount; i++) {
    ...
    CFmtStr str("%d,%.2f %.2f %.2f", i, m_ragAngles[i].x, m_ragAngles[i].y,
                m_ragAngles[i].z);
...
void CRagdollProp::InputFadeAndRemove(inputdata_t &inputdata) {
  ...
  if (flFadeDuration == 0.0f) flFadeDuration = 1.0f;

  FadeOut(0.0f, flFadeDuration);
}

void Ragdoll_GetAngleOverrideString(char *pOut, int size,
                                    CBaseEntity *pEntity) {
