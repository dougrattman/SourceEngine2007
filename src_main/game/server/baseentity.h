...
class CBaseEntity : public IServerEntity {
 ...
 protected:
  ...
  static int
      m_nDebugSteps;  // Number of entity outputs to fire before pausing again.

  static bool sm_bDisableTouchFuncs;  // Disables PhysicsTouch and
                                      // PhysicsStartTouch function calls
 public:
  static bool sm_bAccurateTriggerBboxChecks;  // SOLID_BBOX entities do a fully
                                              // accurate trigger vs bbox check
                                              // when this is set
 ...
 public:
  ...
  // attachment, this calculates the attachment's transform and gives you that.
  //
  // You must pass in tempMatrix for scratch space - it may need to fill that in
  // and return it instead of pointing you right at a variable in your parent.
 ...
 public:
  ...
  // Called once per frame after the server frame loop has finished and after
  // all messages being
  //  sent to clients have been sent.
  // NOTE: This will not be called unless the entity requests it via
  // gEntList.AddPostClientMessageEntity
  ...
  // Set the movement parent. Your local origin and angles will become relative
  // to this parent. If iAttachment is a valid attachment on the parent, then
  // your local origin and angles are relative to the attachment on this entity.
  // If iAttachment == -1, it'll preserve the current m_iParentAttachment.
 ...
 public:
  ...
  void RemoveDeferred(void);  // Sets the entity invisible, and makes it remove
...
// Verifies that the data description is valid in debug builds.
  ...
  virtual bool AcceptInput(const char *szInputName, CBaseEntity *pActivator,
                           CBaseEntity *pCaller, variant_t Value, int outputID);
  ...
  // tries to read a field from the entities data description - result is placed
  // in variant_t
  ...
  const char *
  GetDebugName();  // do not make this virtual -- designed to handle NULL this
 ...
 public:
  ...
  string_t
      m_iGlobalname;  // identifier for carrying entity across level transitions
  string_t m_iParent;  // the name of the entities parent; linked into m_pParent
                       // during Activate()
 ...
 public:
  ...
  CNetworkVar(float, m_flAnimTime);  // this is the point in time that the
                                     // client will interpolate to
                                     // position,angle,frame,etc.
  CNetworkVar(float, m_flSimulationTime);

  int m_nLastThinkTick;

#if !defined(NO_ENTITY_PREDICTION)
  // Certain entities (projectiles) can be created on the client and thus need a
  // matching id number
 ...
 public:
  ...
  // Classify - returns the type of group (i.e, "houndeye", or "human military"
  // so that NPCs with different classnames still realize that they are
  // teammates. (overridden for NPCs that form groups)
  ...
  // Call this to do a TraceAttack on an entity, performs filtering. Don't call
  // TraceAttack() directly except when chaining up to base class
 ...
 public:
  ...
  bool InSameTeam(CBaseEntity *pEntity) const;  // Returns true if the specified
                                                // entity is on the same team as
                                                // this one
  ...
  void (CBaseEntity ::*m_pfnUse)(CBaseEntity *pActivator, CBaseEntity *pCaller,
 ...
 public:
  ...
  void FireBullets(int cShots, const Vector &vecSrc,
                   ...
                   bool bFirstShotAccurate = false, bool bPrimaryAttack = true);
 ...
 public:
  ...
  string_t m_target;
  CNetworkVarForDerived(int, m_iMaxHealth);  // CBaseEntity doesn't care about
                                             // changes to this variable, but
                                             // there are derived classes that
                                             // do.
  ...
  static CBaseEntity *CreateNoSpawn(const char *szName, const Vector &vecOrigin,
                                    const QAngle &vecAngles,
                                    CBaseEntity *pOwner = NULL);
  ...
  const Vector &GetViewOffset() const {
    return const_cast<CBaseEntity *>(this)->GetViewOffset();
  }
  ...
  virtual bool FVisible(const Vector &vecTarget, int traceMask = MASK_BLOCKLOS,
                        CBaseEntity **ppBlocker = NULL);
  ...
  // These methods return a *world-aligned* box relative to the absorigin of the
  // entity. This is used for collision purposes and is *not* guaranteed to
  // surround the entire entity's visual representation NOTE: It is illegal to
  // ask for the world-aligned bounds for SOLID_BSP objects
  ...
  // of the entity. NOTE: The world space center *may* move when the entity
  // rotates.
  float BoundingRadius() const;
  bool IsPointSized() const;

  // NOTE: Setting the abs origin or angles will cause the local origin + angles
  // to be set also
  void SetAbsOrigin(const Vector &origin);
  void SetAbsAngles(const QAngle &angles);

  // Origin and angles in local space ( relative to parent )
  // NOTE: Setting the local origin or angles will cause the abs origin + angles
  // to be set also
  ...
  // For now, we're hacking around it by moving the sound emission origin up on
  // certain objects like vehicles.
  //
  ...
  void EmitSound(const char *soundname, float soundtime = 0.0f,
                 float *duration = NULL);  // Override for doing the general
                                           // case of CPASAttenuationFilter
                                           // filter( this ), and EmitSound(
                                           // filter, entindex(), etc. );
  void EmitSound(const char *soundname, HSOUNDSCRIPTHANDLE &handle,
  ...
  static void EmitSound(IRecipientFilter &filter, int iEntIndex,
                        const EmitSound_t &params, HSOUNDSCRIPTHANDLE &handle);
  ...
  // For each client who appears to be a valid recipient, checks the client has
  // disabled CC and if so, removes them from
  //  the recipient list.
  ...
  static void EmitCloseCaption(IRecipientFilter &filter, int entindex,
                               char const *token,
                               CUtlVector<Vector> &soundorigins, float duration,
 ...
 public:
  ...
  // This creates a normal vphysics simulated object - physics determines where
  // it goes (gravity, friction, etc) and the entity receives updates from
  // vphysics.  SetAbsOrigin(), etc do not affect the object!
  IPhysicsObject *VPhysicsInitNormal(SolidType_t solidType, int nSolidFlags,
                                     bool createAsleep, solid_t *pSolid = NULL);

  // This creates a vphysics object with a shadow controller that follows the AI
  // Move the object to where it should be and call
  // UpdatePhysicsShadowToCurrentPosition()
  IPhysicsObject *VPhysicsInitShadow(bool allowPhysicsMovement,
                                     bool allowPhysicsRotation,
                                     solid_t *pSolid = NULL);

  // Force a non-solid (ie. solid_trigger) physics object to collide with other
  // entities.
 ...
 public:
  ...
  // update the shadow so it will coincide with the current AI position at some
  // time in the future (or 0 for now)
 ...
 private:
  ...
  // Run regular think and latch off angle/origin changes so we can interpolate
  // them on the server to fake simulation
 ...
 private:
  ...
  friend void SendProxy_Origin(const SendProp *pProp, const void *pStruct,
                               const void *pData, DVariant *pOut, int iElement,
                               int objectID);
  friend void SendProxy_Angles(const SendProp *pProp, const void *pStruct,
                               const void *pData, DVariant *pOut, int iElement,
                               int objectID);
 ...
 private:
  ...
  CNetworkVar(unsigned char, m_iParentAttachment);  // 0 if we're relative to
                                                    // the parent's absorigin
                                                    // and absangles.
  ...
  CNetworkHandle(
      CBaseEntity,
      m_hOwnerEntity);  // only used to point to an edict it won't collide with
 ...
 private:
  ...
  // prediction code
  //  randon number generators to spit out the same random numbers on both sides
  //  for a particular usercmd input.
 ...
 public:
  ...
  // Call this when hierarchy is not completely set up (such as during Restore)
  // to throw asserts when people call GetAbsAnything.
...
#define SetUse(a)                                                              \
  UseSet(static_cast<void (CBaseEntity::*)(CBaseEntity * pActivator,           \
                                           CBaseEntity * pCaller,              \
