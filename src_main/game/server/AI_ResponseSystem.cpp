...
ConVar rr_debugresponses(
    "rr_debugresponses", "0", FCVAR_NONE,
    "Show verbose matching output (1 for simple, 2 for rule scoring). If set "
    "to 3, it will only show response success/failure for npc_selected NPCs.");
ConVar rr_debugrule(
    "rr_debugrule", "", FCVAR_NONE,
    "If set to the name of the rule, that rule's score will be shown whenever "
    "a concept is passed into the response rules system.");
ConVar rr_dumpresponses(
    "rr_dumpresponses", "0", FCVAR_NONE,
...
class Matcher {
 public:
  ...
  void SetToken(char const *s) { token = g_RS.AddString(s); }
  ...
  void SetRaw(char const *raw) { rawtoken = g_RS.AddString(raw); }
  char const *GetRaw() {
    if (rawtoken.IsValid()) {
...
struct ResponseGroup {
  ...
  bool HasUndepletedChoices() const {
    if (!m_bDepleteBeforeRepeat) return true;

    int c = group.Count();
    for (int i = 0; i < c; i++) {
      if (group[i].depletioncount != m_nDepletionCount) return true;
    }
  ...
  void MarkResponseUsed(int idx) {
    if (!m_bDepleteBeforeRepeat) return;
  ...
  bool HasUndepletedFirst(int &index) {
    ...
    if (!m_bDepleteBeforeRepeat) return false;
...
abstract_class CResponseSystem : public IResponseSystem {
 public:
  ...
  virtual void PrecacheResponses(bool bEnable) { m_bPrecache = bEnable; }
 ...
 protected:
  ...
  float RecursiveLookForCriteria(const AI_CriteriaSet &criteriaSet,
                                 Criteria *pParent);

 public:
  void CopyRuleFrom(Rule * pSrcRule, int iRule, CResponseSystem *pCustomSystem);
  void CopyCriteriaFrom(Rule * pSrcRule, Rule * pDstRule,
                        CResponseSystem * pCustomSystem);
  void CopyResponsesFrom(Rule * pSrcRule, Rule * pDstRule,
                         CResponseSystem * pCustomSystem);
  ...
  inline bool ParseToken(void) {
    ...
    m_ScriptStack[0].currenttoken =
        engine->ParseFile(m_ScriptStack[0].currenttoken, token, sizeof(token));
    m_ScriptStack[0].tokencount++;
    return m_ScriptStack[0].currenttoken != NULL ? true : false;
  }

  inline void Unget() { m_bUnget = true; }
  ...
  inline bool TokenWaiting(void) {
    ...
    while (*p && *p != '\n') {
      ...
      if (!isspace(*p) || isalnum(*p)) return true;
  ...
  bool CompareUsingMatcher(const char *setValue, Matcher &m,
                           bool verbose = false);
  void ComputeMatcher(Criteria * c, Matcher & matcher);
  void ResolveToken(Matcher & matcher, char *token, size_t bufsize,
                    char const *rawtoken);
  float LookupEnumeration(const char *name, bool &found);

  int FindBestMatchingRule(const AI_CriteriaSet &set, bool verbose);

  float ScoreCriteriaAgainstRule(const AI_CriteriaSet &set, int irule,
                                 bool verbose = false);
  float RecursiveScoreSubcriteriaAgainstRule(const AI_CriteriaSet &set,
                                             Criteria *parent, bool &exclude,
                                             bool verbose /*=false*/);
  float ScoreCriteriaAgainstRuleCriteria(const AI_CriteriaSet &set,
                                         int icriterion, bool &exclude,
                                         bool verbose = false);
  bool GetBestResponse(ResponseSearchResult & result, Rule * rule,
                       bool verbose = false, IResponseFilter *pFilter = NULL);
  bool ResolveResponse(ResponseSearchResult & result, int depth,
                       const char *name, bool verbose = false,
                       IResponseFilter *pFilter = NULL);
  int SelectWeightedResponseFromResponseGroup(ResponseGroup * g,
                                              IResponseFilter * pFilter);
...
void CResponseSystem::GetCurrentScript(char *buf, size_t buflen) {
  Assert(buf);
  buf[0] = 0;
  if (m_ScriptStack.Count() <= 0) return;
...
void CResponseSystem::PushScript(const char *scriptfile,
                                 unsigned char *buffer) {
  ScriptEntry e;
...
// Input  : matcher -
//-----------------------------------------------------------------------------
void CResponseSystem::ResolveToken(Matcher &matcher, char *token,
                                   size_t bufsize, char const *rawtoken) {
  if (rawtoken[0] != '[') {
...
static bool AppearsToBeANumber(char const *token) {
  if (atof(token) != 0.0f) return true;
  ...
  while (!done) {
    switch (*in) {
      ...
      case '\0': {
        ...
        // Convert raw token to real token in case token is an enumerated type
        // specifier
...
bool CResponseSystem::CompareUsingMatcher(const char *setValue, Matcher &m,
                                          bool verbose /*=false*/) {
  ...
  if (m.notequal) {
    if (m.isnumeric) {
      if (v == (float)atof(m.GetToken())) return false;
    } else {
      if (!Q_stricmp(setValue, m.GetToken())) return false;
    }
  ...
  if (m.isnumeric) {
    ...
    if (!setValue || !setValue[0]) return false;
...
bool CResponseSystem::Compare(const char *setValue, Criteria *c,
                              bool verbose /*= false*/) {
  Assert(c);
  Assert(setValue);
...
float CResponseSystem::RecursiveScoreSubcriteriaAgainstRule(
    const AI_CriteriaSet &set, Criteria *parent, bool &exclude,
    bool verbose /*=false*/) {
  float score = 0.0f;
  ...
  for (int i = 0; i < subcount; i++) {
    ...
    score += ScoreCriteriaAgainstRuleCriteria(set, icriterion, excludesubrule,
                                              verbose);
  }
...
float CResponseSystem::RecursiveLookForCriteria(
    const AI_CriteriaSet &criteriaSet, Criteria *pParent) {
  float flScore = 0.0f;
...
float CResponseSystem::LookForCriteria(const AI_CriteriaSet &criteriaSet,
                                       int iCriteria) {
  ...
  if (iIndex == -1) return 0.0f;

  Assert(criteriaSet.GetValue(iIndex));
  if (Q_stricmp(criteriaSet.GetValue(iIndex), pCriteria->value)) return 0.0f;

  return 1.0f;
}

float CResponseSystem::ScoreCriteriaAgainstRuleCriteria(
    const AI_CriteriaSet &set, int icriterion, bool &exclude,
    bool verbose /*=false*/) {
  ...
  if (verbose) {
    DevMsg("  criterion '%25s':'%15s' ", m_Criteria.GetElementName(icriterion),
           c->name);
  }
  ...
  if (Compare(actualValue, c, verbose)) {
    ...
    if (verbose) {
      DevMsg("matched, weight %4.2f (s %4.2f x c %4.2f)", score, w, c->weight);
    }
  } else {
    if (c->required) {
      exclude = true;
...
float CResponseSystem::ScoreCriteriaAgainstRule(const AI_CriteriaSet &set,
                                                int irule,
                                                bool verbose /*=false*/) {
  ...
  if (pszText && pszText[0] &&
      !Q_stricmp(pszText, m_Rules.GetElementName(irule))) {
    bBeingWatched = true;
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    bool exclude = false;
    score +=
        ScoreCriteriaAgainstRuleCriteria(set, icriterion, exclude, verbose);
...
// Output : int
//-----------------------------------------------------------------------------
int CResponseSystem::SelectWeightedResponseFromResponseGroup(
    ResponseGroup *g, IResponseFilter *pFilter) {
  int c = g->group.Count();
  if (!c) {
  ...
  if (pFilter && g->ShouldCheckRepeats()) {
    for (i = 0; i < c; i++) {
      ...
      if (r->depletioncount != g->GetDepletionCount() &&
          !pFilter->IsValidResponse(r->GetType(), r->value)) {
  ...
  if (!g->HasUndepletedChoices()) {
    ...
    if (!g->HasUndepletedChoices()) return -1;
  ...
  if (checkrepeats) {
    ...
    if (slot == -1 && g->HasUndepletedLast(check) && check != -1) {
      ...
      for (i = 0; i < c; i++) {
        Response *r = &g->group[i];
        if (checkrepeats && (r->depletioncount == depletioncount)) {
          continue;
        }
  ...
  if (slot == -1) {
    for (i = 0; i < c; i++) {
      ...
      if (checkrepeats && r->last) continue;
      ...
      if (!totalweight ||
          random->RandomFloat(0, totalweight) < r->weight.GetFloat()) {
        slot = i;
      }

      if (!checkrepeats && slot != prevSlot && pFilter &&
          !pFilter->IsValidResponse(r->GetType(), r->value)) {
        slot = prevSlot;
        totalweight -= r->weight.GetFloat();
      }
  ...
  if (slot != -1) g->MarkResponseUsed(slot);
...
//-----------------------------------------------------------------------------
bool CResponseSystem::ResolveResponse(ResponseSearchResult &searchResult,
                                      int depth, const char *name,
                                      bool verbose /*= false*/,
                                      IResponseFilter *pFilter) {
  int responseIndex = m_Responses.Find(name);
  if (responseIndex == m_Responses.InvalidIndex()) return false;
  ...
  if (g->IsSequential()) {
    ...
    do {
      ...
      if (!pFilter || pFilter->IsValidResponse(g->group[idx].GetType(),
                                               g->group[idx].value)) {
        bFoundValid = true;
        break;
      }
  ...
  if (result->type == RESPONSE_RESPONSE) {
    // Recurse
    bret = ResolveResponse(searchResult, depth + 1, result->value, verbose,
                           pFilter);
  } else {
...
//			depth -
//-----------------------------------------------------------------------------
void CResponseSystem::DescribeResponseGroup(ResponseGroup *group, int selected,
                                            int depth) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    DebugPrint(
        depth + 1, "%s%20s : %40s %5.3f\n", i == selected ? "-> " : "   ",
        AI_Response::DescribeResponse(r->GetType()), r->value, r->weight);
  }
}
...
//-----------------------------------------------------------------------------
bool CResponseSystem::GetBestResponse(ResponseSearchResult &searchResult,
                                      Rule *rule, bool verbose /*=false*/,
                                      IResponseFilter *pFilter) {
  ...
  if (g->IsSequential()) {
    ...
    do {
      ...
      if (!pFilter ||
          pFilter->IsValidResponse(g->group[responseIndex].GetType(),
                                   g->group[responseIndex].value)) {
        bFoundValid = true;
        break;
      }
...
// Output : int
//-----------------------------------------------------------------------------
int CResponseSystem::FindBestMatchingRule(const AI_CriteriaSet &set,
                                          bool verbose) {
  ...
  if (bestCount == 1) return bestrules[0];
...
// Output : AI_Response
//-----------------------------------------------------------------------------
bool CResponseSystem::FindBestResponse(const AI_CriteriaSet &set,
                                       AI_Response &response,
                                       IResponseFilter *pFilter) {
  bool valid = false;
  ...
  // Look for match. verbose mode used to be at level 2, but disabled because
  // the writers don't actually care for that info.
  ...
  for (int i = 0; i < (int)m_Responses.Count(); i++) {
    ...
    for (int j = 0; j < group.group.Count(); j++) {
      ...
      if (response.type != RESPONSE_RESPONSE) {
        ...
        pResponse->Init(response.GetType(), response.value, AI_CriteriaSet(),
                        group.rp, NULL, NULL, false);
        ...
        case RESPONSE_SCENE: {
          ...
          if (gender) {
            ...
            *gender = 0;
            ...
            Q_snprintf(genderFile, sizeof(genderFile), "%sfemale%s", file,
                       postGender);
...
void CResponseSystem::LoadFromBuffer(const char *scriptfile, const char *buffer,
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "#include")) {
      ...
      int byteoffset =
          m_ScriptStack[0].currenttoken - (const char *)m_ScriptStack[0].buffer;

      Error(
          "CResponseSystem::LoadFromBuffer:  Unknown entry type '%s', "
          "expecting 'response', 'criterion', 'enumeration' or 'rules' in file "
          "%s(offset:%i)\n",
...
void CResponseSystem::LoadRuleSet(const char *basescript) {
  int length = 0;
  unsigned char *buffer =
      (unsigned char *)UTIL_LoadFileForMe(basescript, &length);
  if (length <= 0 || !buffer) {
...
void CResponseSystem::ParseOneResponse(const char *responseGroupName,
                                       ResponseGroup &group) {
...
bool CResponseSystem::IsRootCommand() {
  if (!Q_stricmp(token, "#include")) return true;
  if (!Q_stricmp(token, "response")) return true;
  if (!Q_stricmp(token, "enumeration")) return true;
  if (!Q_stricmp(token, "criteria")) return true;
  if (!Q_stricmp(token, "criterion")) return true;
  if (!Q_stricmp(token, "rule")) return true;
  return false;
}
...
void CResponseSystem::ParseRule(void) {
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "criteria") || !Q_stricmp(token, "criterion")) {
      // Read them until we run out.
      while (TokenWaiting()) {
        ParseToken();
...
int CResponseSystem::GetCurrentToken() const {
  if (m_ScriptStack.Count() <= 0) return -1;
...
// Purpose:
//-----------------------------------------------------------------------------
void CResponseSystem::CopyCriteriaFrom(Rule *pSrcRule, Rule *pDstRule,
                                       CResponseSystem *pCustomSystem) {
  ...
  for (int iCriteria = 0; iCriteria < nCriteriaCount; ++iCriteria) {
    ...
    if (pSrcCriteria) {
      int iIndex =
          pCustomSystem->m_Criteria.Find(m_Criteria.GetElementName(iSrcIndex));
      ...
      for (int iSubCriteria = 0; iSubCriteria < nSubCriteriaCount;
           ++iSubCriteria) {
        int iSrcSubIndex = pSrcCriteria->subcriteria[iSubCriteria];
        Criteria *pSrcSubCriteria = &m_Criteria[iSrcSubIndex];
        if (pSrcCriteria) {
          int iSubIndex =
              pCustomSystem->m_Criteria.Find(pSrcSubCriteria->value);
          if (iSubIndex != pCustomSystem->m_Criteria.InvalidIndex()) continue;
          ...
          int iSubInsertIndex = pCustomSystem->m_Criteria.Insert(
              pSrcSubCriteria->value, dstSubCriteria);
          dstCriteria.subcriteria.AddToTail(iSubInsertIndex);
        }
      }

      int iInsertIndex = pCustomSystem->m_Criteria.Insert(
          m_Criteria.GetElementName(iSrcIndex), dstCriteria);
...
// Purpose:
//-----------------------------------------------------------------------------
void CResponseSystem::CopyResponsesFrom(Rule *pSrcRule, Rule *pDstRule,
                                        CResponseSystem *pCustomSystem) {
  // Add responses from this rule to global list in custom response system.
  int nResponseGroupCount = pSrcRule->m_Responses.Count();
  for (int iResponseGroup = 0; iResponseGroup < nResponseGroupCount;
       ++iResponseGroup) {
    ...
    if (pSrcResponseGroup) {
      ...
      dstResponseGroup.m_bDepleteBeforeRepeat =
          pSrcResponseGroup->m_bDepleteBeforeRepeat;
      ...
      int iInsertIndex = pCustomSystem->m_Responses.Insert(
          m_Responses.GetElementName(iSrcResponseGroup), dstResponseGroup);
...
void CResponseSystem::CopyEnumerationsFrom(CResponseSystem *pCustomSystem) {
  ...
  for (int iEnumeration = 0; iEnumeration < nEnumerationCount; ++iEnumeration) {
    ...
    if (pSrcEnumeration) {
      ...
      pCustomSystem->m_Enumerations.Insert(
          m_Enumerations.GetElementName(iEnumeration), dstEnumeration);
    }
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CResponseSystem::CopyRuleFrom(Rule *pSrcRule, int iRule,
                                   CResponseSystem *pCustomSystem) {
  // Verify data.
  Assert(pSrcRule);
  Assert(pCustomSystem);
  if (!pSrcRule || !pCustomSystem) return;
...
class CInstancedResponseSystem : public CResponseSystem {
 ...
 public:
  CInstancedResponseSystem(const char *scriptfile) : m_pszScriptFile(0) {
    Assert(scriptfile);
  ...
  ~CInstancedResponseSystem() { delete[] m_pszScriptFile; }
...
class CDefaultResponseSystem : public CResponseSystem, public CAutoGameSystem {
 ...
 public:
  ...
  void AddInstancedResponseSystem(const char *scriptfile,
                                  CInstancedResponseSystem *sys) {
  ...
  CInstancedResponseSystem *FindResponseSystem(const char *scriptfile) {
    ...
    if (idx == m_InstancedSystems.InvalidIndex()) return NULL;
    return m_InstancedSystems[idx];
  }

  IResponseSystem *PrecacheCustomResponseSystem(const char *scriptfile) {
    CInstancedResponseSystem *sys =
        (CInstancedResponseSystem *)FindResponseSystem(scriptfile);
    if (!sys) {
      ...
      if (!sys->Init()) {
        Error(
            "CInstancedResponseSystem:  Failed to init response system from "
            "%s!",
            scriptfile);
      }
...
IResponseSystem *CDefaultResponseSystem::BuildCustomResponseSystemGivenCriteria(
  ...
  CInstancedResponseSystem *pCustomSystem =
      new CInstancedResponseSystem(pszCustomName);
  if (!pCustomSystem) {
    Error(
        "BuildCustomResponseSystemGivenCriterea: Failed to create custom "
        "response system %s!",
        pszCustomName);
  }
...
CON_COMMAND(rr_reloadresponsesystems, "Reload all response system scripts.") {
  ...
  if (g_pGameRules->IsMultiplayer()) {
    CMultiplayRules *pMultiplayRules =
        static_cast<CMultiplayRules *>(g_pGameRules);
...
// note:  this won't save/restore settings from instanced response systems.
// Could add that with a CDefSaveRestoreOps implementation if needed
//
class CDefaultResponseSystemSaveRestoreBlockHandler
    : public CDefSaveRestoreBlockHandler {
 public:
  const char *GetBlockName() { return "ResponseSystem"; }
  ...
  void ReadRestoreHeaders(IRestore *pRestore) {
    // No reason why any future version shouldn't try to retain backward
    // compatability. The default here is to not do so.
    short version;
...
//
// BUGBUG:  This will save the same response system to file multiple times for
// "shared" response systems and
//  therefore it'll restore the same data onto the same pointer N times on
//  reload (probably benign for now, but we could write code to save/restore the
//  instanced ones by filename in the block handler above maybe?
//-----------------------------------------------------------------------------
...
class CResponseSystemSaveRestoreOps : public CDefSaveRestoreOps {
 public:
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    ...
    if (!pRS || pRS == &defaultresponsesytem) return;
...
IResponseSystem *BuildCustomResponseSystemGivenCriteria(
    const char *pszBaseFile, const char *pszCustomName,
    AI_CriteriaSet &criteriaSet, float flCriteriaScore) {
  return defaultresponsesytem.BuildCustomResponseSystemGivenCriteria(
      pszBaseFile, pszCustomName, criteriaSet, flCriteriaScore);
}
  ...
  for (int iRule = 0; iRule < nRuleCount; ++iRule) {
    ...
    for (int iResponseGroup = 0; iResponseGroup < nResponseGroupCount;
      ...
      Msg("		ResponseGroup %d: %s\n", iResponseGroup,
          m_Responses.GetElementName(iRuleResponse));
