...
class CAimTargetManager : public IEntityListener {
 public:
  ...
  void ForceRepopulateList() {
    ...
    while (pEnt) {
      if (ShouldAddEntity(pEnt)) AddEntity(pEnt);
  ...
  virtual void OnEntityDeleted(CBaseEntity *pEntity) {
    if (!(pEntity->GetFlags() & FL_AIMTARGET)) return;
 ...
 public:
  ...
  int ListCopy(CBaseEntity *pList[], int listMax) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (m_simThinkList[i].nextThinkTick <= gpGlobals->tickcount) {
        ...
        Assert(m_simThinkList[i].nextThinkTick == 0 ||
               pList[out]->GetFirstThinkTick() ==
                   m_simThinkList[i].nextThinkTick);
  ...
  void EntityChanged(CBaseEntity *pEntity) {
    ...
    if (pEntity->IsEFlagSet(EFL_NO_THINK_FUNCTION) &&
      ...
      // already in the list? (had think or sim last time, now has both - or had
      // both last time, now just one)
      if (m_entinfoIndex[index] == 0xFFFF) {
        ...
        if (pEntity->IsEFlagSet(EFL_NO_GAME_PHYSICS_SIMULATION)) {
          m_simThinkList[m_entinfoIndex[index]].nextThinkTick =
              pEntity->GetFirstThinkTick();
...
// This manages a list of entities queued up to receive PostClientMessages
// callbacks
class CPostClientMessageManager {
 public:
  void LevelShutdownPostEntity() { m_list.Purge(); }
  void AddEntity(CBaseEntity *pEntity) {
    MEM_ALLOC_CREDIT();
  ...
  void PostClientMessagesSent() {
    for (int i = m_list.Count() - 1; i >= 0; --i) {
      CBaseEntity *pEntity =
          (CBaseEntity *)gEntList.GetEntInfoPtrByIndex(m_list[i])->m_pEntity;
...
void CGlobalEntityList::Clear(void) {
  ...
  // Add all remaining entities in the game to the delete list and call
  // appropriate UpdateOnRemove
...
CBaseEntity *CGlobalEntityList::NextEnt(CBaseEntity *pCurrentEnt) {
  ...
  if (pList) pList = NextEntInfo(pList);
...
void CGlobalEntityList::ReportEntityFlagsChanged(CBaseEntity *pEntity,
...
bool CGlobalEntityList::IsEntityPtr(void *pTest) {
  if (pTest) {
    ...
    for (; pInfo; pInfo = pInfo->m_pNext) {
      if (pTest == (void *)pInfo->m_pEntity) return true;
    }
  }
...
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByClassname(CBaseEntity *pStartEntity,
                                                      const char *szName) {
  const CEntInfo *pInfo =
      pStartEntity ? GetEntInfoPtr(pStartEntity->GetRefEHandle())->m_pNext
                   : FirstEntInfo();
...
//			pSearchingEntity -
//			pActivator - The activator entity if this was called from an
//input 				or Use handler.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityProcedural(
    const char *szName, CBaseEntity *pSearchingEntity, CBaseEntity *pActivator,
    CBaseEntity *pCaller) {
  //
...
//			pActivator - Activator entity if this was called from an
//input 				handler or Use handler.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByName(CBaseEntity *pStartEntity,
                                                 const char *szName,
  ...
  if (szName[0] == '!') {
    ...
    if (pStartEntity == NULL)
      return FindEntityProcedural(szName, pSearchingEntity, pActivator,
                                  pCaller);
...
//			szModelName -
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByModel(CBaseEntity *pStartEntity,
...
//			flRadius -
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityInSphere(CBaseEntity *pStartEntity,
  ...
  for (; pInfo; pInfo = pInfo->m_pNext) {
    ...
    if (!IsBoxIntersectingSphere(ent->CollisionProp()->OBBMins(),
                                 ent->CollisionProp()->OBBMaxs(),
...
//			flRadius - Search radius for classname search, 0 to search
//everywhere. 			pSearchingEntity - The entity that is doing the search. 			pActivator
//- The activator entity if this was called from an input 				or Use handler, NULL
//otherwise.
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByNameNearest(
  ...
  while ((pSearch = gEntList.FindEntityByName(pSearch, szName, pSearchingEntity,
                                              pActivator, pCaller)) != NULL) {
    if (!pSearch->edict()) continue;
...
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByNameWithin(
    CBaseEntity *pStartEntity, const char *szName, const Vector &vecSrc,
    float flRadius, CBaseEntity *pSearchingEntity, CBaseEntity *pActivator,
    CBaseEntity *pCaller) {
  //
  // Check for matching class names within the search radius.
  //
  ...
  float flMaxDist2 = flRadius * flRadius;
  if (flMaxDist2 == 0) {
    return gEntList.FindEntityByName(pEntity, szName, pSearchingEntity,
                                     pActivator, pCaller);
  }

  while ((pEntity = gEntList.FindEntityByName(pEntity, szName, pSearchingEntity,
                                              pActivator, pCaller)) != NULL) {
...
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByClassnameNearest(
    const char *szName, const Vector &vecSrc, float flRadius) {
  CBaseEntity *pEntity = NULL;
...
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityByClassnameWithin(
    CBaseEntity *pStartEntity, const char *szName, const Vector &vecSrc,
    float flRadius) {
  //
  // Check for matching class names within the search radius.
...
// Input  : pStartEntity - The entity to start from when doing the search.
//			szName - Entity name to search for. Treated as a target name
//first, 				then as an entity class name, ie "info_target". 			vecSrc - Center of
//search radius. 			flRadius - Search radius for classname search, 0 to search
//everywhere. 			pSearchingEntity - The entity that is doing the search. 			pActivator
//- The activator entity if this was called from an input 				or Use handler, NULL
//otherwise.
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityGeneric(CBaseEntity *pStartEntity,
                                                  const char *szName,
                                                  CBaseEntity *pSearchingEntity,
                                                  CBaseEntity *pActivator,
                                                  CBaseEntity *pCaller) {
  ...
  pEntity = gEntList.FindEntityByName(pStartEntity, szName, pSearchingEntity,
                                      pActivator, pCaller);
  if (!pEntity) {
...
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityGenericWithin(
    CBaseEntity *pStartEntity, const char *szName, const Vector &vecSrc,
    float flRadius, CBaseEntity *pSearchingEntity, CBaseEntity *pActivator,
    CBaseEntity *pCaller) {
  CBaseEntity *pEntity = NULL;
  ...
  pEntity =
      gEntList.FindEntityByNameWithin(pStartEntity, szName, vecSrc, flRadius,
                                      pSearchingEntity, pActivator, pCaller);
  if (!pEntity) {
    pEntity = gEntList.FindEntityByClassnameWithin(pStartEntity, szName, vecSrc,
                                                   flRadius);
  }
...
//-----------------------------------------------------------------------------
// Purpose: Finds the nearest entity by target name or class name within a
// radius. Input  : pStartEntity - The entity to start from when doing the
// search.
//			szName - Entity name to search for. Treated as a target name
//first, 				then as an entity class name, ie "info_target". 			vecSrc - Center of
//search radius. 			flRadius - Search radius for classname search, 0 to search
...
// Output : Returns a pointer to the found entity, NULL if none.
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityGenericNearest(
    const char *szName, const Vector &vecSrc, float flRadius,
    CBaseEntity *pSearchingEntity, CBaseEntity *pActivator,
    CBaseEntity *pCaller) {
  CBaseEntity *pEntity = NULL;

  pEntity = gEntList.FindEntityByNameNearest(
      szName, vecSrc, flRadius, pSearchingEntity, pActivator, pCaller);
  if (!pEntity) {
...
//-----------------------------------------------------------------------------
// Purpose: Find the nearest entity along the facing direction from the given
// origin
//			within the angular threshold (ignores worldspawn) with
//the 			given classname.
// Input  : origin -
//			facing -
//			threshold -
//			classname -
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityClassNearestFacing(
    const Vector &origin, const Vector &facing, float threshold,
    char *classname) {
  ...
  for (; pInfo; pInfo = pInfo->m_pNext) {
    ...
    if (dot > bestDot) {
      if (FClassnameIs(ent, classname)) {
        ...
        if (!FClassnameIs(ent, "worldspawn") &&
            !FClassnameIs(ent, "soundent")) {
          bestDot = dot;
          best_ent = ent;
        }
      }
...
//			threshold -
//-----------------------------------------------------------------------------
CBaseEntity *CGlobalEntityList::FindEntityNearestFacing(const Vector &origin,
                                                        const Vector &facing,
                                                        float threshold) {
  float bestDot = threshold;
  CBaseEntity *best_ent = NULL;
  ...
  for (; pInfo; pInfo = pInfo->m_pNext) {
    ...
    // Ignore if worldspawn
    if (!FStrEq(STRING(ent->m_iClassname), "worldspawn") &&
        !FStrEq(STRING(ent->m_iClassname), "soundent")) {
      bestDot = dot;
      best_ent = ent;
    }
  }
...
void CGlobalEntityList::OnRemoveEntity(IHandleEntity *pEnt,
                                       CBaseHandle handle) {
#ifdef DEBUG
  if (!g_fInCleanupDelete) {
    ...
    for (i = 0; i < g_DeleteList.Count(); i++) {
      if (g_DeleteList[i]->GetEntityHandle() == pEnt) {
        ...
        Msg("ERROR: Entity being destroyed but previously threaded on "
            "g_DeleteList\n");
        break;
      }
    }
...
class CNotifyList : public INotify, public IEntityListener {
 public:
  ...
  void ReportSystemEvent(CBaseEntity *pEntity, notify_system_event_t eventType,
...
void CNotifyList::RemoveEntity(CBaseEntity *pNotify, CBaseEntity *pWatched) {
  for (int i = m_notifyList.Count(); --i >= 0;) {
    if (m_notifyList[i].pNotify == pNotify &&
        m_notifyList[i].pWatched == pWatched) {
      m_notifyList.FastRemove(i);
    }
  }
}

void CNotifyList::ReportNamedEvent(CBaseEntity *pEntity,
  ...
  for (int i = 0; i < m_notifyList.Count(); i++) {
    if (m_notifyList[i].pWatched == pEntity) {
      m_notifyList[i].pNotify->AcceptInput(pInputName, pEntity, pEntity,
                                           emptyVariant, 0);
    }
  }
}
...
void CNotifyList::ClearEntity(CBaseEntity *pNotify) {
  for (int i = m_notifyList.Count(); --i >= 0;) {
    if (m_notifyList[i].pNotify == pNotify ||
        m_notifyList[i].pWatched == pNotify) {
      m_notifyList.FastRemove(i);
    }
  }
}

void CNotifyList::ReportSystemEvent(
...
class CEntityTouchManager : public IEntityListener {
 public:
  ...
  void Clear() { m_updateList.Purge(); }

  // IEntityListener
  virtual void OnEntityCreated(CBaseEntity *pEntity) {}
  virtual void OnEntityDeleted(CBaseEntity *pEntity) {
    if (!pEntity->GetCheckUntouch()) return;
...
// Since most of the little list managers in here only need one or two of the
// game system callbacks, this hook is a game system that passes them the
...
class CEntityListSystem : public CAutoGameSystemPerFrame {
 public:
  ...
  void LevelShutdownPreEntity() { g_NotifyList.LevelShutdownPreEntity(); }
  void LevelShutdownPostEntity() {
  ...
  void FrameUpdatePostEntityThink() {
    ...
    if (m_bRespawnAllEntities) {
      ...
      // Allows us to immediately re-use the edict indices we just freed to
      // avoid edict overflow
...
static ConCommand restart_entities("respawn_entities", RespawnEntities,
  ...
  class CEntityReportLess {
   public:
    bool Less(const ENTITYPTR &src1, const ENTITYPTR &src2, void *pCtx) {
      if (stricmp(src1->GetClassname(), src2->GetClassname()) < 0) return true;
