...
SendPropEHandle(SENDINFO(m_hLightingLandmark)), END_SEND_TABLE()
  ...
  if (m_strLightingLandmark == NULL_STRING) {
    ...
    m_hLightingLandmark =
        gEntList.FindEntityByName(NULL, m_strLightingLandmark);
    if (!m_hLightingLandmark) {
      DevWarning("%s: Could not find lighting landmark '%s'!\n", GetClassname(),
                 STRING(m_strLightingLandmark));
    } else {
...
//-----------------------------------------------------------------------------
void CInfoLightingRelative::SetTransmit(CCheckTransmitInfo *pInfo,
                                        bool bAlways) {
...
BEGIN_DATADESC(CBaseAnimating)
  ...
  DEFINE_FIELD(m_flGroundSpeed, FIELD_FLOAT),
      ...
      DEFINE_ARRAY(m_flPoseParameter, FIELD_FLOAT,
                   CBaseAnimating::NUM_POSEPAREMETERS),
      DEFINE_ARRAY(m_flEncodedController, FIELD_FLOAT,
                   CBaseAnimating::NUM_BONECTRLS),
      ...
      DEFINE_KEYFIELD(m_iszLightingOriginRelative, FIELD_STRING,
                      "LightingOriginHack"),
      ...
      DEFINE_INPUTFUNC(FIELD_INTEGER, "IgniteNumHitboxFires",
                       InputIgniteNumHitboxFires),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "IgniteHitboxFireScale",
                       InputIgniteHitboxFireScale),
      DEFINE_INPUTFUNC(FIELD_VOID, "BecomeRagdoll", InputBecomeRagdoll),
      DEFINE_INPUTFUNC(FIELD_STRING, "SetLightingOriginHack",
                       InputSetLightingOriginRelative),
      DEFINE_INPUTFUNC(FIELD_STRING, "SetLightingOrigin",
...
#if !defined(TF_DLL)
  // Anything derived from this class can potentially burn - true, but do we
  // want it to!
...
float CBaseAnimating::GetAnimTimeInterval(void) const {
  ...
  if (m_flAnimTime < gpGlobals->curtime) {
    ...
    flInterval = std::clamp(gpGlobals->curtime - m_flAnimTime, 0.0f,
                            MAX_ANIMTIME_INTERVAL);
  } else {
    // report actual
    flInterval = std::clamp(m_flAnimTime - m_flPrevAnimTime, 0.0f,
                            MAX_ANIMTIME_INTERVAL);
  }
  return flInterval;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseAnimating::StudioFrameAdvanceInternal(CStudioHdr *pStudioHdr,
                                                float flCycleDelta) {
...
void CBaseAnimating::StudioFrameAdvanceManual(float flInterval) {
  ...
  float flCycleRate =
      GetSequenceCycleRate(pStudioHdr, GetSequence()) * m_flPlaybackRate;
...
void CBaseAnimating::StudioFrameAdvance() {
  ...
  flInterval = std::clamp(flInterval, 0.0f, MAX_ANIMTIME_INTERVAL);

  // Msg( "%i %s interval %f\n", entindex(), GetClassname(), flInterval );
  if (flInterval <= 0.001) {
    // Msg("%s : %s : %5.3f (skip)\n", GetClassname(), GetSequenceName(
  ...
  if (ai_sequence_debug.GetBool() == true &&
    ...
    Msg("%5.2f : %s : %s : %5.3f\n", gpGlobals->curtime, GetClassname(),
        GetSequenceName(GetSequence()), GetCycle());
  }
}
...
void CBaseAnimating::SetLightingOrigin(string_t strLightingOrigin) {
  if (strLightingOrigin == NULL_STRING) {
    ...
    CBaseEntity *pLightingOrigin =
        gEntList.FindEntityByName(NULL, strLightingOrigin);
    if (!pLightingOrigin) {
...
KeyValues *CBaseAnimating::GetSequenceKeyValues(int iSequence) {
  ...
  if (szText) {
    ...
    if (seqKeyValues->LoadFromBuffer(modelinfo->GetModelName(GetModel()),
                                     szText)) {
      return seqKeyValues;
    }
...
float CBaseAnimating::GetSequenceMoveYaw(int iSequence) {
  ...
  Assert(GetModelPtr());
  ::GetSequenceLinearMotion(GetModelPtr(), iSequence, GetPoseParameterArray(),
                            &vecReturn);
...
// Output : float
//-----------------------------------------------------------------------------
float CBaseAnimating::GetSequenceMoveDist(CStudioHdr *pStudioHdr,
                                          int iSequence) {
  Vector vecReturn;

  ::GetSequenceLinearMotion(pStudioHdr, iSequence, GetPoseParameterArray(),
                            &vecReturn);
...
bool CBaseAnimating::BecomeRagdollOnClient(const Vector &force) {
  // If this character has a ragdoll animation, turn it over to the physics
  // system
  if (CanBecomeRagdoll()) {
...
bool CBaseAnimating::CanBecomeRagdoll(void) {
  ...
  if (ragdollSequence == ACTIVITY_NOT_AVAILABLE) return false;

  if (GetFlags() & FL_TRANSRAGDOLL) return false;
...
void CBaseAnimating::ResetSequenceInfo() {
  if (GetSequence() == -1) {
    // This shouldn't happen.  Setting m_nSequence blindly is a horrible coding
    // practice.
    SetSequence(0);
  }
...
void CBaseAnimating::SetSequence(int nSequence) {
  Assert(GetModelPtr() && (nSequence < GetModelPtr()->GetNumSeq()) &&
         (GetModelPtr()->GetNumSeq() < (1 << ANIMATION_SEQUENCE_BITS)));
...
float CBaseAnimating::SequenceDuration(CStudioHdr *pStudioHdr, int iSequence) {
  if (!pStudioHdr) {
    DevWarning(
        2, "CBaseAnimating::SequenceDuration( %d ) NULL pstudiohdr on %s!\n",
...
float CBaseAnimating::GetSequenceCycleRate(CStudioHdr *pStudioHdr,
                                           int iSequence) {
...
float CBaseAnimating::GetLastVisibleCycle(CStudioHdr *pStudioHdr,
                                          int iSequence) {
  if (!pStudioHdr) {
    DevWarning(
        2, "CBaseAnimating::LastVisibleCycle( %d ) NULL pstudiohdr on %s!\n",
        iSequence, GetClassname());
    return 1.0;
  }

  if (!(GetSequenceFlags(pStudioHdr, iSequence) & STUDIO_LOOPING)) {
    return 1.0f - (pStudioHdr->pSeqdesc(iSequence).fadeouttime) *
                      GetSequenceCycleRate(iSequence) * m_flPlaybackRate;
  } else {
    return 1.0;
  }
}

float CBaseAnimating::GetSequenceGroundSpeed(CStudioHdr *pStudioHdr,
                                             int iSequence) {
  float t = SequenceDuration(pStudioHdr, iSequence);
...
bool CBaseAnimating::HasAnimEvent(int nSequence, int nEvent) {
  ...
  int index = 0;
  while ((index = GetAnimationEvent(pstudiohdr, nSequence, &event, 0.0f, 1.0f,
                                    index)) != 0) {
...
void CBaseAnimating::DispatchAnimEvents(CBaseAnimating *eventHandler) {
  ...
  pstudiohdr->pSeqdesc( GetSequence() ).pszLabel(), flStart, flEnd,
  m_bSequenceFinished );
  }
  */
  ...
  while ((index = GetAnimationEvent(pstudiohdr, GetSequence(), &event, flStart,
    ...
    if (flCycleRate > 0.0) {
      ...
      event.eventtime = m_flAnimTime + (flCycle - GetCycle()) / flCycleRate +
                        GetAnimTimeInterval();
    }
    ...
    */
    ...
    if (pNowStudioHdr != pstudiohdr) {
      AssertMsg2(false,
                 "%s has changed its model while processing AnimEvents on "
                 "sequence %d. Aborting dispatch.\n",
                 GetDebugName(), GetSequence());
      Warning(
          "%s has changed its model while processing AnimEvents on sequence "
          "%d. Aborting dispatch.\n",
          GetDebugName(), GetSequence());
      break;
    }
    ...
    if (pEvent->event == AE_SV_PLAYSOUND) {
      ...
      if (sscanf(pEvent->options, "%s %f %f", szAttachment, &flDuration,
        ...
        if (hDustTrail) {
          ...
          hDustTrail->m_ParticleLifetime =
              1.5;  // Lifetime of each particle, In seconds
          hDustTrail->m_Color.Init(0.5f, 0.46f, 0.44f);
          hDustTrail->m_StartSize = flSize;
          hDustTrail->m_EndSize = hDustTrail->m_StartSize * 8;
          hDustTrail->m_SpawnRadius = 3;  // Each particle randomly offset from
                                          // the center up to this many units
          hDustTrail->m_MinSpeed = 4;     // u/sec
          hDustTrail->m_MaxSpeed = 10;    // u/sec
          hDustTrail->m_Opacity = 0.5f;
          hDustTrail->SetLifetime(
              flDuration);  // Lifetime of the spawner, in seconds
...
//=========================================================
//=========================================================
float CBaseAnimating::SetPoseParameter(CStudioHdr *pStudioHdr,
                                       const char *szName, float flValue) {
  int poseParam = LookupPoseParameter(pStudioHdr, szName);
  AssertMsg2(poseParam >= 0,
             "SetPoseParameter called with invalid argument %s by %s", szName,
             GetDebugName());
  return SetPoseParameter(pStudioHdr, poseParam, flValue);
}

float CBaseAnimating::SetPoseParameter(CStudioHdr *pStudioHdr, int iParameter,
                                       float flValue) {
  if (!pStudioHdr) {
    return flValue;
  }
...
float CBaseAnimating::GetPoseParameter(int iParameter) {
  ...
  if (iParameter >= 0) {
    return Studio_GetPoseParameter(pstudiohdr, iParameter,
                                   m_flPoseParameter[iParameter]);
  }

  return 0.0;
}

bool CBaseAnimating::GetPoseParameterRange(int index, float &minValue,
                                           float &maxValue) {
  CStudioHdr *pStudioHdr = GetModelPtr();
...
//=========================================================
//=========================================================
int CBaseAnimating::LookupPoseParameter(CStudioHdr *pStudioHdr,
                                        const char *szName) {
...
bool CBaseAnimating::HasPoseParameter(int iSequence, int iParameter) {
  ...
  if (pstudiohdr->GetSharedPoseParameter(iSequence, seqdesc.paramindex[0]) ==
          iParameter ||
      pstudiohdr->GetSharedPoseParameter(iSequence, seqdesc.paramindex[1]) ==
          iParameter) {
    return true;
  }
  return false;
...
//=========================================================
// Purpose: from input of 75% to 200% of maximum range, rescale smoothly from
// 75% to 100%
//=========================================================
float CBaseAnimating::EdgeLimitPoseParameter(int iParameter, float flValue,
                                             float flBase) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr) {
    return flValue;
...
//=========================================================
//=========================================================
void CBaseAnimating::GetBonePosition(int iBone, Vector &origin,
                                     QAngle &angles) {
  CStudioHdr *pStudioHdr = GetModelPtr();
  if (!pStudioHdr) {
...
//-----------------------------------------------------------------------------
// Purpose: Interpolates client IK floor position and drops entity down so that
// the feet will reach
//-----------------------------------------------------------------------------

ConVar npc_height_adjust("npc_height_adjust", "1", FCVAR_ARCHIVE,
...
void CBaseAnimating::UpdateStepOrigin() {
  ...
  */
  ...
  if (m_flIKGroundContactTime > 0.2 &&
    ...
    if ((GetFlags() & (FL_FLY | FL_SWIM)) == 0 && GetMoveParent() == NULL &&
        GetGroundEntity() != NULL && !GetGroundEntity()->IsMoving()) {
      ...
      if (toAbs.z == 0.0) {
        ...
        float bias =
            std::clamp((m_flIKGroundMaxHeight - m_flIKGroundMinHeight) - height,
                       0.0f, height);
        // save off reasonable offset
        m_flEstIkOffset =
            std::clamp(m_flEstIkFloor - GetAbsOrigin().z, -height + bias, 0.0f);
        return;
      }
    }
  }
    ...
    for (int i = 0; i < m_pIk->m_target.Count(); i++) {
      ...
      switch (pTarget->type) {
        case IK_GROUND: {
          ...
          /*
          debugoverlay->AddBoxOverlay( p1, Vector(-r,-r,0), Vector(r,r,1),
          QAngle( 0, 0, 0 ), 255, 0, 0, 0, 1.0f ); debugoverlay->AddBoxOverlay(
          trace.endpos, Vector(-r,-r,0), Vector(r,r,1), QAngle( 0, 0, 0 ), 255,
          0, 0, 0, 1.0f ); debugoverlay->AddLineOverlay( p1, trace.endpos, 255,
          0, 0, 0, 1.0f );
          */

          if (trace.startsolid) {
            ray.Init(pTarget->trace.hip, pTarget->est.pos, Vector(-r, -r, 0),
                     Vector(r, r, 1));
...
void CBaseAnimating::Teleport(const Vector *newPosition,
...
//-----------------------------------------------------------------------------
// Purpose: build matrices first from the parent, then from the passed in arrays
// if the bone doesn't exist on the parent
//-----------------------------------------------------------------------------

void CBaseAnimating::BuildMatricesWithBoneMerge(
    const CStudioHdr *pStudioHdr, const QAngle &angles, const Vector &origin,
    const Vector pos[MAXSTUDIOBONES], const Quaternion q[MAXSTUDIOBONES],
    matrix3x4_t bonetoworld[MAXSTUDIOBONES], CBaseAnimating *pParent,
  ...
  for (int i = 0; i < pStudioHdr->numbones(); i++) {
    ...
    if (!merged) {
      ...
      if (pbones[i].parent == -1) {
        ...
        ConcatTransforms(bonetoworld[pbones[i].parent], bonematrix,
                         bonetoworld[i]);
      }
    }
  }
...
ConVar sv_pvsskipanimation("sv_pvsskipanimation", "1", FCVAR_ARCHIVE,
                           "Skips SetupBones when npc's are outside the PVS");
ConVar ai_setupbones_debug("ai_setupbones_debug", "0", 0,
                           "Shows that bones that are setup every think");

inline bool CBaseAnimating::CanSkipAnimation(void) {
  if (!sv_pvsskipanimation.GetBool()) return false;

  CAI_BaseNPC *pNPC = MyNPCPointer();
  if (pNPC && !pNPC->HasCondition(COND_IN_PVS) &&
      (m_fBoneCacheFlags & (BCF_NO_ANIMATION_SKIP | BCF_IS_IN_SPAWN)) ==
          false) {
    // If we have a player as a child, then we better setup our bones. If we
    // don't, the PVS will be screwy.
...
void CBaseAnimating::SetupBones(matrix3x4_t *pBoneToWorld, int boneMask) {
  ...
  if (pParent) {
    ...
    if (pParentCache) {
      BuildMatricesWithBoneMerge(pStudioHdr, GetAbsAngles(), adjOrigin, pos, q,
                                 pBoneToWorld, pParent, pParentCache);
  ...
  Studio_BuildMatrices(pStudioHdr, GetAbsAngles(), adjOrigin, pos, q, -1,
                       pBoneToWorld, boneMask);

  if (ai_setupbones_debug.GetBool()) {
    // Msg("%s:%s:%s (%x)\n", GetClassname(), GetDebugName(),
    // STRING(GetModelName()), boneMask );
    DrawRawSkeleton(pBoneToWorld, boneMask, true, 0.11);
  }
  RemoveEFlags(EFL_SETTING_UP_BONES);
}
...
int CBaseAnimating::LookupAttachment(const char *szName) {
  ...
  // The +1 is to make attachment indices be 1-based (namely 0 == invalid or
  // unused attachment)
...
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetAttachment(const char *szName, Vector &absOrigin,
                                   QAngle &absAngles) {
...
// Output :	location and angles
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetAttachment(int iAttachment, Vector &absOrigin,
                                   QAngle &absAngles) {
...
// Output :	location and angles
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetAttachment(int iAttachment,
                                   matrix3x4_t &attachmentToWorld) {
  CStudioHdr *pStudioHdr = GetModelPtr();
  if (!pStudioHdr) {
  ...
  const mstudioattachment_t &pattachment =
      pStudioHdr->pAttachment(iAttachment - 1);
...
int CBaseAnimating::GetAttachmentBone(int iAttachment) {
  CStudioHdr *pStudioHdr = GetModelPtr();
  if (!pStudioHdr || iAttachment < 1 ||
      iAttachment > pStudioHdr->GetNumAttachments()) {
...
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetAttachmentLocal(const char *szName, Vector &origin,
                                        QAngle &angles) {
  return GetAttachmentLocal(LookupAttachment(szName), origin, angles);
}

bool CBaseAnimating::GetAttachmentLocal(int iAttachment, Vector &origin,
                                        QAngle &angles) {
...
bool CBaseAnimating::GetAttachmentLocal(int iAttachment,
...
//=========================================================
//=========================================================
int CBaseAnimating::FindTransitionSequence(int iCurrentSequence,
                                           int iGoalSequence, int *piDir) {
  Assert(GetModelPtr());

  if (piDir == NULL) {
    int iDir = 1;
    int sequence = ::FindTransitionSequence(GetModelPtr(), iCurrentSequence,
                                            iGoalSequence, &iDir);
    if (iDir != 1)
      return -1;
    else
  ...
  return ::FindTransitionSequence(GetModelPtr(), iCurrentSequence,
                                  iGoalSequence, piDir);
}

bool CBaseAnimating::GotoSequence(int iCurrentSequence, float flCurrentCycle,
                                  float flCurrentRate, int iGoalSequence,
                                  int &nNextSequence, float &flNextCycle,
                                  int &iNextDir) {
  return ::GotoSequence(GetModelPtr(), iCurrentSequence, flCurrentCycle,
                        flCurrentRate, iGoalSequence, nNextSequence,
                        flNextCycle, iNextDir);
}
...
const char *CBaseAnimating::GetFlexControllerName(
...
const char *CBaseAnimating::GetFlexControllerType(
    LocalFlexController_t iFlexController) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr) return 0;
...
//-----------------------------------------------------------------------------
// Purpose: Converts the ground speed of the animating entity into a true
// velocity Output : Vector - velocity of the character at its current
// m_flGroundSpeed
//-----------------------------------------------------------------------------
Vector CBaseAnimating::GetGroundSpeedVelocity(void) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr) return vec3_origin;
...
float CBaseAnimating::GetInstantaneousVelocity(float flInterval) {
  ...
  Vector vecVelocity;
  Studio_SeqVelocity(pstudiohdr, GetSequence(), flNextCycle,
                     GetPoseParameterArray(), vecVelocity);
...
float CBaseAnimating::GetEntryVelocity(int iSequence) {
  ...
  Vector vecVelocity;
  Studio_SeqVelocity(pstudiohdr, iSequence, 0.0, GetPoseParameterArray(),
                     vecVelocity);
...
float CBaseAnimating::GetExitVelocity(int iSequence) {
  ...
  Vector vecVelocity;
  Studio_SeqVelocity(pstudiohdr, iSequence, 1.0, GetPoseParameterArray(),
                     vecVelocity);
...
// Output :
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetIntervalMovement(float flIntervalUsed,
                                         bool &bMoveSeqFinished,
                                         Vector &newPosition,
                                         QAngle &newAngles) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr || !pstudiohdr->SequencesAvailable()) return false;

  float flComputedCycleRate = GetSequenceCycleRate(GetSequence());

  float flNextCycle =
      GetCycle() + flIntervalUsed * flComputedCycleRate * m_flPlaybackRate;
  ...
  if (Studio_SeqMovement(pstudiohdr, GetSequence(), GetCycle(), flNextCycle,
                         GetPoseParameterArray(), deltaPos, deltaAngles)) {
...
// Output :
//-----------------------------------------------------------------------------
bool CBaseAnimating::GetSequenceMovement(int nSequence, float fromCycle,
                                         float toCycle, Vector &deltaPosition,
                                         QAngle &deltaAngles) {
  CStudioHdr *pstudiohdr = GetModelPtr();
  if (!pstudiohdr) return false;

  return Studio_SeqMovement(pstudiohdr, nSequence, fromCycle, toCycle,
                            GetPoseParameterArray(), deltaPosition,
                            deltaAngles);
}
...
float CBaseAnimating::GetMovementFrame(float flDist) {
  ...
  float t = Studio_FindSeqDistance(pstudiohdr, GetSequence(),
                                   GetPoseParameterArray(), flDist);
...
CBoneCache *CBaseAnimating::GetBoneCache(void) {
  ...
  if (pcache) {
    if (pcache->IsValid(gpGlobals->curtime) &&
        (pcache->m_boneMask & boneMask) == boneMask &&
        pcache->m_timeValid <= gpGlobals->curtime) {
      // Msg("%s:%s:%s (%x:%x:%8.4f) cache\n", GetClassname(), GetDebugName(),
      // STRING(GetModelName()), boneMask, pcache->m_boneMask,
      // pcache->m_timeValid ); in memory and still valid, use it!
      return pcache;
    }
  ...
  if (pcache) {
    ...
    pcache->UpdateBones(bonetoworld, pStudioHdr->numbones(),
                        gpGlobals->curtime);
  } else {
...
bool CBaseAnimating::TestCollision(const Ray_t &ray, unsigned int fContentsMask,
                                   trace_t &tr) {
...
bool CBaseAnimating::TestHitboxes(const Ray_t &ray, unsigned int fContentsMask,
                                  trace_t &tr) {
  CStudioHdr *pStudioHdr = GetModelPtr();
  if (!pStudioHdr) {
    Assert(!"CBaseAnimating::GetBonePosition: model missing");
...
float CBaseAnimating::GetBoneController(int iController) {
  ...
  return Studio_GetController(pmodel, iController,
                              m_flEncodedController[iController]);
}
...
// Output  :
//------------------------------------------------------------------------------
void CBaseAnimating::GetVelocity(Vector *vVelocity,
...
void CBaseAnimating::GetSkeleton(CStudioHdr *pStudioHdr, Vector pos[],
  ...
  AccumulatePose(pStudioHdr, m_pIk, pos, q, GetSequence(), GetCycle(),
                 GetPoseParameterArray(), boneMask, 1.0, gpGlobals->curtime);

  if (m_pIk) {
    CIKContext auto_ik;
    auto_ik.Init(pStudioHdr, GetAbsAngles(), GetAbsOrigin(), gpGlobals->curtime,
                 0, boneMask);
    CalcAutoplaySequences(pStudioHdr, &auto_ik, pos, q, GetPoseParameterArray(),
                          boneMask, gpGlobals->curtime);
  } else {
    CalcAutoplaySequences(pStudioHdr, NULL, pos, q, GetPoseParameterArray(),
                          boneMask, gpGlobals->curtime);
  }
...
int CBaseAnimating::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // ----------------
    char tempstr[1024];
    Q_snprintf(tempstr, sizeof(tempstr), "Sequence: (%3d) %s", GetSequence(),
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Cycle: %.5f (%.5f)",
               (float)GetCycle(), m_flAnimTime.Get());
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
  ...
  if (m_debugOverlays & OVERLAY_ATTACHMENTS_BIT) {
    ...
    if (pStudioHdr) {
      ...
      for (int i = 1; i <= pStudioHdr->GetNumAttachments(); i++) {
        ...
        NDebugOverlay::Line(vecPos, vecPos + (vecForward * 4.0f), 255, 0, 0,
                            true, 0.05f);
        NDebugOverlay::Line(vecPos, vecPos + (vecRight * 4.0f), 0, 255, 0, true,
                            0.05f);
        NDebugOverlay::Line(vecPos, vecPos + (vecUp * 4.0f), 0, 0, 255, true,
                            0.05f);

        Q_snprintf(tempstr, sizeof(tempstr), " < %s (%d)",
                   pStudioHdr->pAttachment(i - 1).pszName(), i);
...
//-----------------------------------------------------------------------------
void CBaseAnimating::GetInputDispatchEffectPosition(const char *sInputString,
                                                    Vector &pOrigin,
                                                    QAngle &pAngles) {
...
//-----------------------------------------------------------------------------
// Purpose: Send the current hitboxes for this model to the client ( to compare
// with
...
void CBaseAnimating::DrawServerHitboxes(float duration /*= 0.0f*/,
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    NDebugOverlay::BoxAngles(position, pbox->bbmin, pbox->bbmax, angles, r, g,
                             b, 0, duration);
  }
}

void CBaseAnimating::DrawRawSkeleton(matrix3x4_t boneToWorld[], int boneMask,
...
//-----------------------------------------------------------------------------
bool CBaseAnimating::ComputeHitboxSurroundingBox(Vector *pVecWorldMins,
  ...
  // which causes IK to trigger, which causes raycasts against the other
  // entities to occur, which is illegal to do while in the Relink phase.
  ...
  for (int i = 0; i < set->numhitboxes; i++) {
    ...
    if (pMatrix) {
      TransformAABB(*pMatrix, pbox->bbmin, pbox->bbmax, vecBoxAbsMins,
                    vecBoxAbsMaxs);
...
//-----------------------------------------------------------------------------
bool CBaseAnimating::ComputeEntitySpaceHitboxSurroundingBox(
...
bool CBaseAnimating::LookupHitbox(const char *szName, int &outSet,
                                  int &outBox) {
...
int CBaseAnimating::GetHitboxesFrontside(int *boxList, int boxMax,
...
// Input  : scale -
//-----------------------------------------------------------------------------
void CBaseAnimating::SetModelWidthScale(float scale,
                                        float change_duration /*= 0.0f*/) {
  if (change_duration > 0.0f) {
    ...
    mvs->m_flModelWidthScaleFinishTime =
        mvs->m_flModelWidthScaleStartTime + change_duration;
  } else {
...
void CBaseAnimating::UpdateModelWidthScale() {
  ...
  float dt =
      mvs->m_flModelWidthScaleFinishTime - mvs->m_flModelWidthScaleStartTime;
  Assert(dt > 0.0f);
  ...
  if (gpGlobals->curtime >= mvs->m_flModelWidthScaleFinishTime) {
    ...
    m_flModelWidthScale =
        Lerp(frac, mvs->m_flModelWidthScaleStart, mvs->m_flModelWidthScaleGoal);
  }
}
...
// Output : float
//-----------------------------------------------------------------------------
float CBaseAnimating::GetModelWidthScale() const { return m_flModelWidthScale; }
...
void CBaseAnimating::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
  ...
  // Right now this prevents stuff we don't want to catch on fire from catching
  // on fire.
...
void CBaseAnimating::IgniteLifetime(float flFlameLifetime) {
  if (!IsOnFire()) Ignite(30, false, 0.0f, true);
...
// Fades out!
//-----------------------------------------------------------------------------
bool CBaseAnimating::Dissolve(const char *pMaterialName, float flStartTime,
                              bool bNPCOnly, int nDissolveType,
                              Vector vDissolverOrigin, int iMagnitude) {
  // Right now this prevents stuff we don't want to catch on fire from catching
  // on fire.
  ...
  CEntityDissolve *pDissolve = CEntityDissolve::Create(
      this, pMaterialName, flStartTime, nDissolveType, &bRagdollCreated);
  if (pDissolve) {
...
void CBaseAnimating::ResetSequence(int nSequence) {
  if (ai_sequence_debug.GetBool() == true &&
    ...
    DevMsg("ResetSequence : %s: %s -> %s\n", GetClassname(),
           GetSequenceName(GetSequence()), GetSequenceName(nSequence));
  }
  ...
  // Tracker 17868:  If the sequence number didn't actually change, but you call
  // resetsequence info, it changes
  //  the newsequenceparity bit which causes the client to call
  //  m_flCycle.Reset() which causes a very slight discontinuity in looping
  //  animations as they reset around to cycle 0.0.  This was causing the
  //  parentattached helmet on barney to hitch every time barney's idle cycled
...
//-----------------------------------------------------------------------------
// Purpose: Async prefetches all anim data used by a particular sequence.
// Returns true if all of the required data is memory resident Input  :
// iSequence - Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseAnimating::PrefetchSequence(int iSequence) {
  CStudioHdr *pStudioHdr = GetModelPtr();
  if (!pStudioHdr) return true;
