...
BEGIN_DATADESC_NO_BASE(vehicle_gear_t)
  ...
  DEFINE_FIELD(flMinSpeed, FIELD_FLOAT), DEFINE_FIELD(flMaxSpeed, FIELD_FLOAT),
...
BEGIN_SIMPLE_DATADESC(CBaseServerVehicle)
  ...
  DEFINE_FIELD(m_nNPCButtons, FIELD_INTEGER),
      ...
      DEFINE_SOUNDPATCH(m_pStateSound), DEFINE_SOUNDPATCH(m_pStateSoundFade),
...
void CBaseServerVehicle::Precache(void) {
  ...
  for (i = 0; i < m_vehicleSounds.crashSounds.Count(); i++) {
    if (m_vehicleSounds.crashSounds[i].iszCrashSound != NULL_STRING) {
      CBaseEntity::PrecacheScriptSound(
          STRING(m_vehicleSounds.crashSounds[i].iszCrashSound));
    }
  }
  ...
  for (i = 0; i < SS_NUM_STATES; i++) {
    if (m_vehicleSounds.iszStateSounds[i] != NULL_STRING) {
      CBaseEntity::PrecacheScriptSound(
          STRING(m_vehicleSounds.iszStateSounds[i]));
    }
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Returns the driver. Unlike GetPassenger(VEHICLE_ROLE_DRIVER), it
// will return
...
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::NPC_AddPassenger(CBaseCombatCharacter *pPassenger,
                                          string_t strRoleName, int nSeat) {
  // Players cannot yet use this code! - jdw
  Assert(pPassenger != NULL && pPassenger->IsPlayer() == false);
  if (pPassenger == NULL || pPassenger->IsPlayer()) return false;

  // Find our role
  int nRole = FindRoleIndexByName(strRoleName);
  if (nRole == -1) return false;

  // Cannot evict a passenger already in this position
  CBaseCombatCharacter *pCurrentPassenger =
      NPC_GetPassengerInSeat(nRole, nSeat);
  if (pCurrentPassenger == pPassenger) return true;
  ...
  for (int i = 0; i < m_PassengerInfo.Count(); i++) {
    if (m_PassengerInfo[i].GetSeat() == nSeat &&
        m_PassengerInfo[i].GetRole() == nRole) {
...
//-----------------------------------------------------------------------------
int CBaseServerVehicle::NPC_GetPassengerSeatAttachment(
...
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::NPC_GetPassengerSeatPosition(
...
// Input  : *pPassenger - Passenger's seat to use
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::NPC_GetPassengerSeatPositionLocal(
    CBaseCombatCharacter *pPassenger, Vector *vecResultPos,
    QAngle *vecResultAngles) {
  // Get our attachment point
  int nSeatAttachment = NPC_GetPassengerSeatAttachment(pPassenger);
  ...
  Vector vecPos;
  QAngle vecAngles;
  pAnimating->InvalidateBoneCache();  // NOTE: We're moving with velocity, so
                                      // we're almost always out of date
...
//-----------------------------------------------------------------------------
// Purpose: Retrieves a list of animations used to enter/exit the seat occupied
// by the passenger Input  : *pPassenger - Passenger who's seat anims to
// retrieve
//			nType - which set of animations to retrieve
//-----------------------------------------------------------------------------
const PassengerSeatAnims_t *CBaseServerVehicle::NPC_GetPassengerSeatAnims(
    CBaseCombatCharacter *pPassenger, PassengerSeatAnimType_t nType) {
  // Get the role and seat the the supplied passenger
  for (int i = 0; i < m_PassengerInfo.Count(); i++) {
    if (m_PassengerInfo[i].m_hPassenger == pPassenger) {
      int nSeat = m_PassengerInfo[i].GetSeat();
      ...
      switch (nType) {
        case PASSENGER_SEAT_ENTRY:
          return &m_PassengerRoles[nRole]
                      .m_PassengerSeats[nSeat]
                      .m_EntryTransitions;
          break;
...
//-----------------------------------------------------------------------------
// Purpose: Get and set the current driver. Use PassengerRole_t enum in
// shareddefs.h for adding passengers
//-----------------------------------------------------------------------------
void CBaseServerVehicle::SetPassenger(int nRole,
                                      CBaseCombatCharacter *pPassenger) {
  // Baseclass only handles vehicles with a single passenger
  Assert(nRole == VEHICLE_ROLE_DRIVER);

  if (pPassenger != NULL && pPassenger->IsPlayer() == false) {
    // Use NPC_AddPassenger() for NPCs at the moment, these will all be
    // collapsed into one system -- jdw
    Assert(0);
    return;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Get a position in *world space* inside the vehicle for the player to
// start at
//-----------------------------------------------------------------------------
void CBaseServerVehicle::GetPassengerSeatPoint(int nRole, Vector *pPoint,
                                               QAngle *pAngles) {
  Assert(nRole == VEHICLE_ROLE_DRIVER);
  ...
  if (pAnimating) {
    ...
    Q_snprintf(pAttachmentName, sizeof(pAttachmentName),
               "vehicle_feet_passenger%d", nRole);
...
//		  distance from origin to drop player (allows for different shaped
//vehicles
// Output: returns true if valid location, pEndPoint
//         updated with actual exit point
//---------------------------------------------------------------------------------
bool CBaseServerVehicle::CheckExitPoint(float yaw, int distance,
                                        Vector *pEndPoint) {
  ...
  *pEndPoint = vecStart + vecDir * distance;
  ...
  trace_t tr;
  UTIL_TraceHull(vecStart, *pEndPoint, VEC_HULL_MIN, VEC_HULL_MAX,
                 MASK_PLAYERSOLID, m_pVehicle, COLLISION_GROUP_NONE, &tr);
...
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::GetPassengerExitPoint(int nRole, Vector *pExitPoint,
                                               QAngle *pAngles) {
  Assert(nRole == VEHICLE_ROLE_DRIVER);
  ...
  if (pAnimating) {
    ...
    if (pAnimating->GetAttachment("vehicle_driver_exit", vehicleExitOrigin,
                                  vehicleExitAngles)) {
      // Make sure it's clear
      trace_t tr;
      UTIL_TraceHull(vehicleExitOrigin + Vector(0, 0, 12), vehicleExitOrigin,
                     VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, m_pVehicle,
  ...
  // left side
  if (CheckExitPoint(90, 90, pExitPoint))  // angle from car, distance from
                                           // origin, actual exit point
    return true;
  ...
  // Make sure it's clear
  trace_t tr;
  UTIL_TraceHull(m_pVehicle->CollisionProp()->WorldSpaceCenter(), *pExitPoint,
                 VEC_HULL_MIN, VEC_HULL_MAX, MASK_PLAYERSOLID, m_pVehicle,
                 COLLISION_GROUP_NONE, &tr);
  if (!tr.startsolid) {
    return true;
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::ParseExitAnim(KeyValues *pkvExitList,
                                       bool bEscapeExit) {
  ...
  while (pkvExitAnim) {
    ...
    Q_strncpy(m_ExitAnimations[iIndex].szAnimName, pkvExitAnim->GetName(),
              sizeof(m_ExitAnimations[iIndex].szAnimName));
...
//-----------------------------------------------------------------------------
void CBaseServerVehicle::ParseNPCSeatTransition(
    KeyValues *pTransitionKeyValues, CPassengerSeatTransition *pTransition) {
  // Store it
...
int __cdecl SeatPrioritySort(const SortSeatPriorityType *s1,
                             const SortSeatPriorityType *s2) {
...
//-----------------------------------------------------------------------------
void CBaseServerVehicle::ParseNPCPassengerSeat(KeyValues *pSetKeyValues,
                                               CPassengerSeat *pSeat) {
  CBaseAnimating *pAnimating = (CBaseAnimating *)m_pVehicle;

  // Get our attachment name
  const char *lpszAttachmentName =
      pSetKeyValues->GetString("target_attachment");
...
//		  : *nIndex - the index into the CUtlBuffer where this role
//resides
// Output : CPassengerRole * - Role found or created
//-----------------------------------------------------------------------------
CPassengerRole *CBaseServerVehicle::FindOrCreatePassengerRole(string_t strName,
                                                              int *nIndex) {
...
void CBaseServerVehicle::ParseNPCRoles(KeyValues *pkvPassengerList) {
  ...
  while (pkvPassengerKey != NULL) {
    ...
    ParseNPCPassengerSeat(pkvPassengerKey,
                          &pRole->m_PassengerSeats[nSeatIndex]);
    ...
    m_PassengerInfo[nPassengerIndex].m_strSeatName =
        pRole->m_PassengerSeats[nSeatIndex].m_strSeatName;
  ...
  if (g_debug_npc_vehicle_roles.GetBool()) {
    ...
    for (int i = 0; i < m_PassengerRoles.Count(); i++) {
      Msg("\tPassenger Role:\t%s (%d seats)\n", m_PassengerRoles[i].m_strName,
          m_PassengerRoles[i].m_PassengerSeats.Count());
...
//-----------------------------------------------------------------------------
// Purpose: Get an attachment point at a specified time in its cycle (note: not
// exactly a speedy query, use sparingly!) Input  : nSequence - sequence to test
//			nAttachmentIndex - attachment to test
//			flCyclePoint - 0.0 - 1.0
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::GetLocalAttachmentAtTime(int nQuerySequence,
  ...
  // TODO: It's annoying to stomp and restore this off each time when we're just
  // going to stomp it again later, but the function
  //		 should really leave the car in an acceptable state to run this
  //query -- jdw
...
//-----------------------------------------------------------------------------
// Purpose: Get an attachment point at a specified time in its cycle (note: not
// exactly a speedy query, use sparingly!) Input  : lpszAnimName - name of the
// sequence to test
//			nAttachmentIndex - attachment to test
//			flCyclePoint - 0.0 - 1.0
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseServerVehicle::GetLocalAttachmentAtTime(const char *lpszAnimName,
                                                  int nAttachmentIndex,
                                                  float flCyclePoint,
                                                  Vector *vecOriginOut,
                                                  QAngle *vecAnglesOut) {
  ...
  if (nQuerySequence < 0) return false;

  return GetLocalAttachmentAtTime(nQuerySequence, nAttachmentIndex,
                                  flCyclePoint, vecOriginOut, vecAnglesOut);
}
...
void CBaseServerVehicle::CacheEntryExitPoints(void) {
  ...
  for (int i = 0; i < m_ExitAnimations.Count(); i++) {
    if (GetLocalAttachmentAtTime(m_ExitAnimations[i].szAnimName, nAttachment,
                                 1.0f, &m_ExitAnimations[i].vecExitPointLocal,
                                 &m_ExitAnimations[i].vecExitAnglesLocal) ==
        false) {
    ...
    if (g_debug_vehicleexit.GetBool()) {
      ...
      NDebugOverlay::Box(vecExitPoint, -Vector(8, 8, 8), Vector(8, 8, 8), 0,
                         255, 0, 0, 20.0f);
...
void CBaseServerVehicle::ParseEntryExitAnims(void) {
  ...
  if (modelKeyValues->LoadFromBuffer(
          modelinfo->GetModelName(m_pVehicle->GetModel()),
    ...
    if (pkvEntryList) {
      ...
      while (pkvEntryAnim) {
        ...
        Q_strncpy(m_EntryAnimations[iIndex].szAnimName, pkvEntryAnim->GetName(),
                  sizeof(m_EntryAnimations[iIndex].szAnimName));
    ...
    KeyValues *pkvPassengerList =
        modelKeyValues->FindKey("vehicle_npc_passengers");
    if (pkvPassengerList) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::HandlePassengerEntry(CBaseCombatCharacter *pPassenger,
  ...
  if (pPlayer != NULL) {
    ...
    if (iEntryAnim == ACTIVITY_NOT_AVAILABLE) {
      ...
      // We failed to find a valid entry anim, but we've got to get back in
      // because the player's got stuck exiting the vehicle. For now, just use
      // the first get in anim UNDONE: We need a better solution for this.
    ...
    if (GetDrivableVehicle()->CanEnterVehicle(pPlayer)) {
      ...
      if (pPlayer->CanEnterVehicle(this, VEHICLE_ROLE_DRIVER)) {
        // Setup the "enter" vehicle sequence and skip the animation if it isn't
        // present.
        ...
        pAnimating->InvalidateBoneCache();  // This is necessary because we need
                                            // to query attachment points this
                                            // frame for blending!
...
bool CBaseServerVehicle::HandlePassengerExit(CBaseCombatCharacter *pPassenger) {
  ...
  if (pPlayer != NULL) {
    ...
    if ((bAllPointsBlocked) || (iSequence == ACTIVITY_NOT_AVAILABLE)) {
      // Animation-driven exit points are all blocked, or we have none. Fall
      // back to the more simple static exit points.
      if (!GetPassengerExitPoint(nRole, &vecNewPos, &angNewAngles) &&
          !GetDrivableVehicle()->AllowBlockedExit(pPlayer, nRole))
        return false;

      // At this point, the player has exited the vehicle but did so without
      // playing an animation.  We need to give the vehicle a chance to do any
      // post-animation clean-up it may need to perform.
      HandleEntryExitFinish(false, true);
    }

    // Now we either have an exit sequence to play, a valid static exit
    // position, or we don't care whether we're blocked or not. We're getting
    ...
    if (iSequence > ACTIVITY_NOT_AVAILABLE) {
      ...
      if (pAnimating) {
        ...
        // To prevent anything moving into the volume the player's going to
        // occupy at the end of the exit NOTE: Set the player as the blocker's
        // owner so the player is allowed to intersect it
        Vector vecExitFeetPoint = vecExitPoint - VEC_VIEW;
        m_hExitBlocker = CEntityBlocker::Create(vecExitFeetPoint, VEC_HULL_MIN,
                                                VEC_HULL_MAX, pPlayer, true);

        // We may as well stand where we're going to get out at and stop being
        // parented
...
int CBaseServerVehicle::GetEntryAnimForPoint(const Vector &vecEyePoint) {
  ...
  if (!set || !set->numhitboxes) return 0;
    ...
    if (IsPointInBox(localEyePoint, pbox->bbmin, pbox->bbmax)) {
      ...
      for (int entry = 0; entry < iCount; entry++) {
        if (m_EntryAnimations[entry].iHitboxGroup == pbox->group) {
          ...
          return pAnimating->LookupSequence(
              m_EntryAnimations[entry].szAnimName);
        }
      }
    }
  }
...
// Input  : vecEyeExitEndpoint - Returns with the final eye position after
// exiting.
//			bAllPointsBlocked - Returns whether all exit points were
//found to be blocked.
// Output :
//-----------------------------------------------------------------------------
int CBaseServerVehicle::GetExitAnimToUse(Vector &vecEyeExitEndpoint,
                                         bool &bAllPointsBlocked) {
  ...
  // Figure out which entrypoint hitbox the player is in
  CBaseAnimating *pAnimating = dynamic_cast<CBaseAnimating *>(m_pVehicle);
  if (!pAnimating) return ACTIVITY_NOT_AVAILABLE;

  CStudioHdr *pStudioHdr = pAnimating->GetModelPtr();
  if (!pStudioHdr) return ACTIVITY_NOT_AVAILABLE;

  bool bUpright = IsVehicleUpright();

  // Loop through the exit animations and find one that ends in a clear position
  // Also attempt to choose the animation which brings you closest to your view
  // direction.
  CBasePlayer *pPlayer = ToBasePlayer(GetDriver());
  if (pPlayer == NULL) return ACTIVITY_NOT_AVAILABLE;
  ...
  for (int i = 0; i < iCount; i++) {
    if (m_ExitAnimations[i].bUpright != bUpright) continue;

    // Don't use an escape point if we found a non-escape point already
    if (!bBestExitIsEscapePoint && m_ExitAnimations[i].bEscapeExit) continue;
    ...
    //		 the player would exit to.  This was rendered unnecessary in Ep2, but
    //the choreo vehicles of these older products 		 did not have proper exit
    //animations and relied on the exact queries that were happening before. For
    //choreo vehicles, 		 we now just allow them to perform those older queries to
    //keep those products happy.  - jdw

    // Get the position we think we're going to end up at
    if (m_bUseLegacyExitChecks) {
      pAnimating->GetAttachment(m_ExitAnimations[i].szAnimName,
    ...
    // But always check non-escape exits if our current best exit is an escape
    // exit.
    if (!bBestExitIsEscapePoint || m_ExitAnimations[i].bEscapeExit) {
      if (flCosAngleDelta < flMaxCosAngleDelta) continue;
    }

    // The attachment points are where the driver's eyes will end up, so we
    // subtract the view offset to get the actual exit position.
    ...
    // NOTE: The hull has no vertical span because we want to test the lateral
    // constraints against the ground, not height (yet)
    trace_t tr;
    UTIL_TraceHull(vecStart, vecEnd, VEC_HULL_MIN,
                   Vector(VEC_HULL_MAX.x, VEC_HULL_MAX.y, VEC_HULL_MIN.z),
                   MASK_PLAYERSOLID, NULL, COLLISION_GROUP_NONE, &tr);

    if (g_debug_vehicleexit.GetBool()) {
      NDebugOverlay::SweptBox(
          vecStart, vecEnd, VEC_HULL_MIN,
          Vector(VEC_HULL_MAX.x, VEC_HULL_MAX.y, VEC_HULL_MIN.y), vec3_angle,
          255, 255, 255, 8.0f, 20.0f);
    }

    if (tr.fraction < 1.0f) {
      // If we hit the ground, try to now "stand up" at that point to see if
      // we'll fit
      ...
      if (tr.startsolid) {
        if (g_debug_vehicleexit.GetBool()) {
          NDebugOverlay::Box(tr.endpos, VEC_HULL_MIN, VEC_HULL_MAX, 255, 0, 0,
                             8.0f, 20.0f);
        }
        continue;
      }
      ...
      if (g_debug_vehicleexit.GetBool()) {
        NDebugOverlay::Box(tr.endpos, VEC_HULL_MIN, VEC_HULL_MAX, 0, 255, 0,
                           8.0f, 20.0f);
      }
    } else if (tr.allsolid ||
               ((tr.fraction == 1.0) &&
                !GetDrivableVehicle()->AllowMidairExit(pPlayer, nRole))) {
      if (g_debug_vehicleexit.GetBool()) {
        NDebugOverlay::Box(tr.endpos, VEC_HULL_MIN, VEC_HULL_MAX, 255, 0, 0, 64,
                           10);
      }
      continue;
    }
    ...
    UTIL_TraceLine(vecViewOrigin, vecExitEndPoint, MASK_PLAYERSOLID, pAnimating,
                   COLLISION_GROUP_NONE, &tr);
    ...
    if (tr.fraction != 1.0) {
      ...
      {
        if (g_debug_vehicleexit.GetBool()) {
          NDebugOverlay::Line(vecViewOrigin, vecExitEndPoint, 255, 0, 0, true,
                              10);
        }
        continue;
      }
  ...
  if (nBestExitAnim >= 0) {
    ...
    if (g_debug_vehicleexit.GetBool()) {
      ...
      NDebugOverlay::Box(m_vecCurrentExitEndPoint, VEC_HULL_MIN, VEC_HULL_MAX,
                         255, 255, 255, 8, 10);
    }
    ...
    return pAnimating->LookupSequence(
        m_ExitAnimations[m_iCurrentExitAnim].szAnimName);
  }
  ...
  if (bExitAnimOn) {
    ...
    if (pPlayer != NULL) {
      ...
      if (m_iCurrentExitAnim >= 0 &&
          m_iCurrentExitAnim < m_ExitAnimations.Count()) {
      ...
      if (g_debug_vehicleexit.GetBool()) {
        NDebugOverlay::Box(vecEyes, -Vector(2, 2, 2), Vector(2, 2, 2), 255, 0,
                           0, 64, 10.0);
      }
      ...
      /*
      trace_t tr;
      UTIL_TraceHull( vecEyes, vecEyes, VEC_HULL_MIN, VEC_HULL_MAX, MASK_SOLID,
      NULL, COLLISION_GROUP_NONE, &tr ); if ( tr.startsolid && tr.fraction < 1.0
      )
      {
...
//-----------------------------------------------------------------------------
void CBaseServerVehicle::GetVehicleViewPosition(int nRole, Vector *pAbsOrigin,
                                                QAngle *pAbsAngles,
                                                float *pFOV /*= NULL*/) {
  Assert(nRole == VEHICLE_ROLE_DRIVER);
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::ProcessMovement(CBasePlayer *pPlayer,
                                         CMoveData *pMoveData) {
  GetDrivableVehicle()->ProcessMovement(pPlayer, pMoveData);

  trace_t tr;
  UTIL_TraceLine(pPlayer->GetAbsOrigin(),
                 pPlayer->GetAbsOrigin() - Vector(0, 0, 256), MASK_PLAYERSOLID,
                 GetVehicleEnt(), COLLISION_GROUP_NONE, &tr);

  // If our gamematerial has changed, tell any player surface triggers that are
  // watching
  IPhysicsSurfaceProps *physprops = MoveHelper()->GetSurfaceProps();
  const surfacedata_t *pSurfaceProp =
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::FinishMove(CBasePlayer *player, CUserCmd *ucmd,
                                    CMoveData *move) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::Weapon_PrimaryRanges(float *flMinRange,
                                              float *flMaxRange) {
  *flMinRange = 64;
  *flMaxRange = 1024;
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseServerVehicle::Weapon_SecondaryRanges(float *flMinRange,
                                                float *flMaxRange) {
  *flMinRange = 64;
  *flMaxRange = 1024;
}
...
//-----------------------------------------------------------------------------
// Purpose: Return the time at which this vehicle's primary weapon can fire
// again
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Return the time at which this vehicle's secondary weapon can fire
// again
//-----------------------------------------------------------------------------
...
void CBaseServerVehicle::PlayLoopingSound(const char *pSoundName) {
  ...
  if (pSoundName && pSoundName[0]) {
    pNewSound = controller.SoundCreate(filter, m_pVehicle->entindex(),
                                       CHAN_STATIC, pSoundName, ATTN_NORM);
  }

  if (m_pStateSound && pNewSound &&
      controller.SoundGetName(pNewSound) ==
    ...
    const char *pStopSound =
        m_pStateSound ? controller.SoundGetName(m_pStateSound).ToCStr()
                      : "NULL";
    const char *pStartSound =
        pNewSound ? controller.SoundGetName(pNewSound).ToCStr() : "NULL";
...
bool CBaseServerVehicle::PlayCrashSound(float speed) {
  ...
  if (rumble > 5.0f) {
    if (GetDriver()) {
      UTIL_ScreenShake(GetDriver()->GetAbsOrigin(), rumble, 150.0f, 1.0f,
                       240.0f, SHAKE_START_RUMBLEONLY, true);
    }
  }

  for (i = 0; i < m_vehicleSounds.crashSounds.Count(); i++) {
    const vehicle_crashsound_t &crash = m_vehicleSounds.crashSounds[i];
    if (!crash.gearLimit) continue;
...
sound_states CBaseServerVehicle::SoundState_ChooseState(
    vbs_sound_update_t &params) {
  float timeInState = gpGlobals->curtime - m_soundStateStartTime;
  bool bInStateForMinTime =
      timeInState > m_vehicleSounds.minStateTime[m_soundState] ? true : false;
  ...
  switch (m_soundState) {
    ...
    case SS_START_WATER:
    case SS_SHUTDOWN_WATER:
      if (!params.bVehicleInWater) return SS_START_IDLE;
      break;

    case SS_TURBO:
      if (params.bVehicleInWater) return SS_SHUTDOWN_WATER;
      if (CheckCrash(params)) return SS_IDLE;
      break;
  ...
  switch (m_soundState) {
    case SS_START_IDLE:
      if (bInStateForMinTime || params.bThrottleDown) return SS_IDLE;
      break;
    case SS_IDLE:
      if (bInStateForMinTime && params.bThrottleDown) {
        if (params.bTurbo) return SS_TURBO;
        return params.bReverse ? SS_REVERSE : SS_GEAR_0;
      }
      break;
    ...
    case SS_GEAR_1:
      if (bInStateForMinTime) {
        if (!params.bThrottleDown) return SS_SLOWDOWN;
      }
      if (m_iSoundGear != 1) return MapGearToState(params, m_iSoundGear);
      break;
...
// vehicle is starting up disabled, but in some cases you still want to play a
// sound HACK: handle those here.
...
void CBaseServerVehicle::SoundShutdown(float flFadeTime) {
  ...
  // Stop any looping sounds that may be running, as the following stop sound
  // may not exist and thus leave a looping sound playing after the user gets
  // out.
...
void CBaseServerVehicle::SoundUpdate(vbs_sound_update_t &params) {
  if (g_debug_vehiclesound.GetInt() > 1) {
    Msg("Throttle: %s, Reverse: %s\n", params.bThrottleDown ? "on" : "off",
        params.bReverse ? "on" : "off");
  }
...
void CBaseServerVehicle::StopSound(vehiclesound iSound) {
  if (m_vehicleSounds.iszSound[iSound] != NULL_STRING) {
    CBaseEntity::StopSound(m_pVehicle->entindex(), CHAN_VOICE,
                           STRING(m_vehicleSounds.iszSound[iSound]));
  }
}

//-----------------------------------------------------------------------------
// Purpose: Calculate the gear we should be in based upon the vehicle's current
// speed
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
CBaseCombatCharacter *CBaseServerVehicle::NPC_GetPassengerInSeat(int nRoleID,
                                                                 int nSeatID) {
  // Search all passengers in the vehicle
  for (int i = 0; i < m_PassengerInfo.Count(); i++) {
    // If the seat ID matches, return the entity in that seat
    if (m_PassengerInfo[i].GetSeat() == nSeatID &&
        m_PassengerInfo[i].GetRole() == nRoleID)
...
//-----------------------------------------------------------------------------
int CBaseServerVehicle::NPC_GetAvailableSeat_Any(
    CBaseCombatCharacter *pPassenger, int nRoleID) {
  ...
  for (int i = 0; i < m_PassengerRoles[nRoleID].m_PassengerSeats.Count(); i++) {
    ...
    CBaseCombatCharacter *pCurrentPassenger =
        NPC_GetPassengerInSeat(nRoleID, i);
    if (pCurrentPassenger != NULL && pCurrentPassenger != pPassenger) continue;
...
// Output : int - Seat by index
//-----------------------------------------------------------------------------
int CBaseServerVehicle::NPC_GetAvailableSeat_Nearest(
    CBaseCombatCharacter *pPassenger, int nRoleID) {
  // Not yet implemented
  Assert(0);
  return -1;
...
//			nQueryType -  Method for choosing the best seat (if
//multiple)
// Output : int - Seat by unique ID
//-----------------------------------------------------------------------------
int CBaseServerVehicle::NPC_GetAvailableSeat(CBaseCombatCharacter *pPassenger,
  ...
  // Get the role index
  int nRole = FindRoleIndexByName(strRoleName);
  if (m_PassengerRoles.IsValidIndex(nRole) == false) return -1;
...
//-----------------------------------------------------------------------------
int CBaseServerVehicle::FindSeatIndexByName(int nRoleIndex,
                                            string_t strSeatName) {
  // Role must be valid
  ...
  for (int i = 0; i < m_PassengerRoles[nRoleIndex].m_PassengerSeats.Count();
       i++) {
    // Return that index if found
    if (m_PassengerRoles[nRoleIndex].m_PassengerSeats[i].GetAttachmentID() ==
        nAttachmentID)
      return i;
  }
...
void CBaseServerVehicle::RestorePassengerInfo(void) {
  ...
  for (int i = 0; i < m_PassengerInfo.Count(); i++) {
    ...
    if (m_PassengerRoles.IsValidIndex(nRoleIndex)) {
      ...
      int nSeatIndex =
          FindSeatIndexByName(nRoleIndex, m_PassengerInfo[i].m_strSeatName);
      if (m_PassengerRoles[nRoleIndex].m_PassengerSeats.IsValidIndex(
              nSeatIndex)) {
        // New seat index
        m_PassengerInfo[i].m_nSeat = nSeatIndex;
      } else {
        // The seat attachment was not found.  This most likely means that the
        // seat attachment name has changed in the target vehicle and the NPC
        // passenger is now stranded!
        Assert(0);
      }
    } else {
      // The role was not found.  This most likely means that the role names
      // have changed in the target vehicle and the NPC passenger is now
      // stranded!
      Assert(0);
    }
  }
...
void CBaseServerVehicle::ReloadScript() {
  if (m_pDrivableVehicle) {
    ...
    vehicleparams_t *pVehicleParams =
        pController ? &(pController->GetVehicleParamsForChange()) : NULL;
    PhysFindOrAddVehicleScript(script.ToCStr(), pVehicleParams,
                               &m_vehicleSounds);
    if (pController) {
...
//-----------------------------------------------------------------------------
// Purpose: Passes this call down into the server vehicle where the tests are
// done
//-----------------------------------------------------------------------------
...
// These are sounds that are to be automatically stopped whenever the vehicle's
// driver leaves it
...
CVehicleSoundsParser::CVehicleSoundsParser(void) {
  // UNDONE: Revisit this pattern - move sub-block processing ideas into the
  // parser architecture
  m_iCurrentGear = -1;
  m_iCurrentState = -1;
...
// Purpose:
//-----------------------------------------------------------------------------
void CVehicleSoundsParser::ParseKeyValue(void *pData, const char *pKey,
                                         const char *pValue) {
  ...
  if (!strcmpi(pKey, "gear")) {
    ...
    if (iNewGear == 0) {
      ...
      pSounds->pGears[iNewGear].flMinSpeed =
          pSounds->pGears[iNewGear - 1].flMaxSpeed;
    }
    ...
    if (m_iCurrentCrashSound >= 0) {
      if (!strcmpi(pKey, "min_speed")) {
        ...
        pSounds->crashSounds[m_iCurrentCrashSound].iszCrashSound =
            AllocPooledString(pValue);
        return;
      } else if (!strcmpi(pKey, "min_speed_change")) {
        pSounds->crashSounds[m_iCurrentCrashSound].flMinDeltaSpeed =
            atof(pValue);
        return;
