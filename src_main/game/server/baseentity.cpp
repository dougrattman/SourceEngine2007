...
bool CBaseEntity::m_bInDebugSelect =
    false;                             // Used for selection in debug overlays
int CBaseEntity::m_nDebugPlayer = -1;  // Player doing the selection

// This can be set before creating an entity to force it to use a particular
// edict.
edict_t *g_pForceAttachEdict = NULL;

bool CBaseEntity::m_bDebugPause = false;  // Whether entity i/o is paused.
int CBaseEntity::m_nDebugSteps =
    1;  // Number of entity outputs to fire before pausing again.
bool CBaseEntity::sm_bDisableTouchFuncs =
    false;  // Disables PhysicsTouch and PhysicsStartTouch function calls
bool CBaseEntity::sm_bAccurateTriggerBboxChecks =
    true;  // set to false for legacy behavior in ep1
...
// When this is false, throw an assert in debug when GetAbsAnything is called.
// Used when hierachy is incomplete/invalid.
bool CBaseEntity::s_bAbsQueriesValid = true;

ConVar sv_netvisdist("sv_netvisdist", "10000",
...
void SendProxy_AnimTime(const SendProp *pProp, const void *pStruct,
  ...
  int addt = 0;
  // If it's within the last tick interval through the current one, then we can
  // encode it
...
// NOTE:  Animtime must be sent before origin and angles ( from pev ) because it
// has a
//  proxy on the client that stores off the old values before writing in the new
//  values and if it is sent after the new values, then it will only have the
//  new origin and studio model, etc. interpolation will be busted
SendPropInt(SENDINFO(m_flAnimTime), 8,
...
SendPropDataTable("AnimTimeMustBeFirst", 0,
                  &REFERENCE_SEND_TABLE(DT_AnimTimeMustBeFirst),
    ...
    SendPropVector(SENDINFO(m_vecOrigin), -1,
                   SPROP_NOSCALE | SPROP_CHANGES_OFTEN, 0.0f, HIGH_DEFAULT,
                   SendProxy_Origin),
#else
    ...
    SendPropDataTable(SENDINFO_DT(m_Collision),
                      &REFERENCE_SEND_TABLE(DT_CollisionProperty)),
    ...
    SendPropInt(SENDINFO(m_iParentAttachment), NUM_PARENTATTACHMENT_BITS,
                SPROP_UNSIGNED),

    SendPropInt(SENDINFO_NAME(m_MoveType, movetype), MOVETYPE_MAX_BITS,
                SPROP_UNSIGNED),
    SendPropInt(SENDINFO_NAME(m_MoveCollide, movecollide), MOVECOLLIDE_MAX_BITS,
                SPROP_UNSIGNED),
#if PREDICTION_ERROR_CHECK_LEVEL > 1
    SendPropVector(SENDINFO(m_angRotation), -1,
                   SPROP_NOSCALE | SPROP_CHANGES_OFTEN, 0, HIGH_DEFAULT,
                   SendProxy_Angles),
#else
    SendPropQAngles(SENDINFO(m_angRotation), 13, SPROP_CHANGES_OFTEN,
                    SendProxy_Angles),
#endif
...
#if !defined(NO_ENTITY_PREDICTION)
    SendPropDataTable("predictable_id", 0,
                      &REFERENCE_SEND_TABLE(DT_PredictableId),
...
CBaseEntity::~CBaseEntity() {
  ...
  // EHANDLE accessors will check, in debug, for access to entities during
  // destruction of
  //  another entity.
  // That kind of operation should only occur in UpdateOnRemove calls
  // Deletion should only occur via UTIL_Remove(Immediate) calls, not via naked
  // delete calls
...
void CBaseEntity::PostConstructor(const char *szClassname) {
  ...
  if (IsEFlagSet(EFL_SERVER_ONLY)) {
    ...
    // Some ents like the player override the AttachEdict function and do it at
    // a different time. While precaching, they don't ever have an edict, so we
    // don't need to add them to the entity list in that case.
    if (edict()) {
      gEntList.AddNetworkableEntity(this, entindex());

      // Cache our IServerNetworkable pointer for the engine for fast access.
      if (edict()) edict()->m_pNetworkable = NetworkProp();
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Verifies that this entity's data description is valid in debug
// builds.
//-----------------------------------------------------------------------------
#ifdef _DEBUG
typedef CUtlVector<const char *> KeyValueNameList_t;

static void AddDataMapFieldNamesToList(KeyValueNameList_t &list,
                                       datamap_t *pDataMap) {
  while (pDataMap != NULL) {
...
void CBaseEntity::ValidateDataDescription(void) {
  ...
  if ((pDataMap == NULL) || pDataMap->bValidityChecked) return;
...
CBaseEntity *CBaseEntity::GetFollowedEntity() {
  if (!IsFollowingEntity()) return NULL;
...
Vector CBaseEntity::HeadTarget(const Vector &posSrc) { return EyePosition(); }
...
void CBaseEntity::DrawBBoxOverlay(float flDuration) {
  if (edict()) {
    ...
    if (CollisionProp()->IsSolidFlagSet(FSOLID_USE_TRIGGER_BOUNDS)) {
      ...
      CollisionProp()->WorldSpaceTriggerBounds(&vecTriggerMins,
                                               &vecTriggerMaxs);
...
// Output  :
//------------------------------------------------------------------------------
void CBaseEntity::EntityText(int text_offset, const char *text, float duration,
  ...
  if ((m_Collision.GetCollisionAngles() == vec3_angle) ||
      (vecLocalCenter == vec3_origin)) {
    VectorAdd(vecLocalCenter, m_Collision.GetCollisionOrigin(), origin);
  } else {
    VectorTransform(vecLocalCenter, m_Collision.CollisionToWorldTransform(),
                    origin);
  }

  NDebugOverlay::EntityTextAtPosition(origin, text_offset, text, duration, r, g,
                                      b, a);
}
...
void CBaseEntity::DrawTimedOverlays(void) {
  ...
  while (pTO) {
    ...
    if ((!CBaseEntity::Debug_IsPaused() &&
         gpGlobals->curtime > pTO->msgEndTime) ||
        (nCount > 10)) {
      if (pLastTO) {
      ...
      if (!CBaseEntity::Debug_IsPaused()) {
        nAlpha = 255 * ((gpGlobals->curtime - pTO->msgStartTime) /
                        (pTO->msgEndTime - pTO->msgStartTime));
      }
      int r = 185;
      int g = 145;
      int b = 145;
...
void CBaseEntity::DrawDebugGeometryOverlays(void) {
    ...
    if (VPhysicsGetObject()) {
      ...
      if (GetMoveType() != MOVETYPE_VPHYSICS) {
        ...
        if (dist > 2 || fabsf(deltaAngle) > 2) {
          Vector mins, maxs;
          physcollision->CollideGetAABB(&mins, &maxs,
                                        VPhysicsGetObject()->GetCollide(),
  ...
  if (m_debugOverlays & OVERLAY_AUTOAIM_BIT && (GetFlags() & FL_AIMTARGET) &&
      AI_GetSinglePlayer() != NULL) {
    ...
    NDebugOverlay::Line(vecCenter, vecCenter + vecRight * radius, r, g, b, true,
                        0.1);
    NDebugOverlay::Line(vecCenter, vecCenter - vecRight * radius, r, g, b, true,
                        0.1);
    NDebugOverlay::Line(vecCenter, vecCenter + vecUp * radius, r, g, b, true,
                        0.1);
    NDebugOverlay::Line(vecCenter, vecCenter - vecUp * radius, r, g, b, true,
                        0.1);
    ...
    NDebugOverlay::Line(vecCenter - vecDiag * radius,
                        vecCenter + vecDiag * radius, r, g, b, true, 0.1);
...
int CBaseEntity::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "(%d) Name: %s (%s)", entindex(),
    ...
    if (m_hDamageFilter.Get() != NULL) {
      Q_snprintf(tempstr, sizeof(tempstr), "DAMAGE FILTER:%s",
                 m_hDamageFilter->GetDebugName());
      EntityText(offset, tempstr, 0);
      offset++;
    }
  }
...
void CBaseEntity::SetParent(string_t newParent, CBaseEntity *pActivator,
  ...
  CBaseEntity *pParent =
      gEntList.FindEntityByName(NULL, newParent, NULL, pActivator);

  // debug check
  if (newParent != NULL_STRING && pParent == NULL) {
    Msg("Entity %s(%s) has bad parent %s\n", STRING(m_iClassname),
        GetDebugName(), STRING(newParent));
  } else {
...
void CBaseEntity::TransformStepData_ParentToWorld(CBaseEntity *pParent) {
  ...
  if (step != NULL) {
    ...
    UTIL_ParentToWorldSpace(pParent, step->m_Previous2.vecOrigin,
                            step->m_Previous2.qRotation);
    UTIL_ParentToWorldSpace(pParent, step->m_Previous.vecOrigin,
                            step->m_Previous.qRotation);
  }
}
...
//-----------------------------------------------------------------------------
void CBaseEntity::TransformStepData_ParentToParent(CBaseEntity *pOldParent,
  ...
  if (step != NULL) {
    // Convert our positions
    UTIL_ParentToWorldSpace(pOldParent, step->m_Previous2.vecOrigin,
                            step->m_Previous2.qRotation);
    UTIL_WorldToParentSpace(pNewParent, step->m_Previous2.vecOrigin,
                            step->m_Previous2.qRotation);

    UTIL_ParentToWorldSpace(pOldParent, step->m_Previous.vecOrigin,
                            step->m_Previous.qRotation);
    UTIL_WorldToParentSpace(pNewParent, step->m_Previous.vecOrigin,
                            step->m_Previous.qRotation);
  }
}
...
void CBaseEntity::TransformStepData_WorldToParent(CBaseEntity *pParent) {
  ...
  if (step != NULL) {
    // Convert our positions
    UTIL_WorldToParentSpace(pParent, step->m_Previous2.vecOrigin,
                            step->m_Previous2.qRotation);
    UTIL_WorldToParentSpace(pParent, step->m_Previous.vecOrigin,
                            step->m_Previous.qRotation);
  }
}
...
void CBaseEntity::SetParent(CBaseEntity *pParentEntity, int iAttachment) {
  ...
  if (pParentEntity) {
    if (const_cast<CBaseEntity *>(pParentEntity)
            ->GetRootMoveParent()
            ->GetSolid() == SOLID_BSP) {
  ...
  if (edict()) {
    ...
    matrix.InitFromEntity(const_cast<CBaseEntity *>(pParentEntity),
                          m_iParentAttachment);  // parent->world
    ...
    // result is that the child is now in parent space, but still oriented the
    // same way
  ...
  if (VPhysicsGetObject()) {
    if (VPhysicsGetObject()->IsStatic()) {
      if (VPhysicsGetObject()->IsAttachedToConstraint(false)) {
        Warning(
            "SetParent on static object, all constraints attached to %s "
            "(%s)will now be broken!\n",
            GetDebugName(), GetClassname());
      }
      VPhysicsDestroyObject();
...
void CBaseEntity::ValidateEntityConnections() {
  if (m_target == NULL_STRING) return;

  if (ClassMatches("scripted_*") || ClassMatches("trigger_relay") ||
      ClassMatches("trigger_auto") || ClassMatches("path_*") ||
      ClassMatches("monster_*") || ClassMatches("trigger_teleport") ||
      ClassMatches("func_train") || ClassMatches("func_tracktrain") ||
      ClassMatches("func_plat*") || ClassMatches("npc_*") ||
      ClassMatches("info_big*") || ClassMatches("env_texturetoggle") ||
      ClassMatches("env_render") || ClassMatches("func_areaportalwindow") ||
      ClassMatches("point_view*") || ClassMatches("func_traincontrols") ||
      ClassMatches("multisource") || ClassMatches("xen_plant*"))
    return;
    ...
    for (int i = 0; i < fields; i++) {
      ...
      if ((dataDesc->fieldType == FIELD_CUSTOM) &&
        ...
        CBaseEntityOutput *pOutput =
            (CBaseEntityOutput *)((int)this + (int)dataDesc->fieldOffset[0]);
        if (pOutput->NumberOfElements()) return;
      }
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseEntity::FireNamedOutput(const char *pszOutput, variant_t variant,
                                  CBaseEntity *pActivator, CBaseEntity *pCaller,
                                  float flDelay) {
  if (pszOutput == NULL) return;
...
int CBaseEntity::TakeHealth(float flHealth, int bitsDamageType) {
  if (!edict() || m_takedamage < DAMAGE_YES) return 0;

  // heal
  if (m_iHealth >= m_iMaxHealth) return 0;
...
// inflict damage on this entity.  bitsDamageType indicates type of damage
// inflicted, ie: DMG_CRUSH

int CBaseEntity::OnTakeDamage(const CTakeDamageInfo &info) {
  Vector vecTemp;

  if (!edict() || !m_takedamage) return 0;
  ...
  // this global is still used for glass and other non-NPC killables, along with
  // decals.
  ...
  // physics objects have their own calcs for this: (don't let fire move things
  // around!)
    ...
    if ((GetMoveType() == MOVETYPE_VPHYSICS)) {
      ...
      if (info.GetInflictor() &&
        ...
        float flForce =
            info.GetDamage() *
            ((32 * 32 * 72.0) /
             (WorldAlignSize().x * WorldAlignSize().y * WorldAlignSize().z)) *
            5;
...
void CBaseEntity::TakeDamage(const CTakeDamageInfo &inputInfo) {
  if (!g_pGameRules) return;

  bool bHasPhysicsForceDamage =
      !g_pGameRules->Damage_NoPhysicsForce(inputInfo.GetDamageType());
  if (bHasPhysicsForceDamage && inputInfo.GetDamageType() != DMG_GENERIC) {
    // If you hit this assert, you've called TakeDamage with a damage type that
    // requires a physics damage force & position without specifying one or both
    // of them. Decide whether your damage that's causing this is something you
    // believe should impart physics force on the receiver. If it is, you need
    // to setup the damage force & position inside the CTakeDamageInfo (Utility
    // functions for this are in takedamageinfo.cpp. If you think the damage
    // shouldn't cause force (unlikely!) then you can set the damage type to
    // DMG_GENERIC, or | DMG_CRUSH if you need to preserve the damage type for
    // purposes of HUD display.

    if (inputInfo.GetDamageForce() == vec3_origin ||
        inputInfo.GetDamagePosition() == vec3_origin) {
      static int warningCount = 0;
      if (++warningCount < 10) {
        if (inputInfo.GetDamageForce() == vec3_origin) {
          DevWarning(
              "CBaseEntity::TakeDamage:  with inputInfo.GetDamageForce() == "
              "vec3_origin\n");
        }
        if (inputInfo.GetDamagePosition() == vec3_origin) {
          DevWarning(
              "CBaseEntity::TakeDamage:  with inputInfo.GetDamagePosition() == "
              "vec3_origin\n");
        }
      }
    }
...
int CBaseEntity::VPhysicsTakeDamage(const CTakeDamageInfo &info) {
  ...
  bool bNoPhysicsForceDamage =
      g_pGameRules->Damage_NoPhysicsForce(info.GetDamageType());
  if (bNoPhysicsForceDamage || info.GetDamageType() == DMG_GENERIC) return 1;
  ...
  if (VPhysicsGetObject()) {
    ...
    if (gameFlags & FVPHYSICS_PLAYER_HELD) {
      // if the player is holding the object, use it's real mass (player holding
      // reduced the mass)
...
class CThinkContextsSaveDataOps : public CDefSaveRestoreOps {
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    AssertMsg(fieldInfo.pTypeDesc->fieldSize == 1,
              "CThinkContextsSaveDataOps does not support arrays");

    // Write out the vector
    CUtlVector<thinkfunc_t> *pUtlVector =
        (CUtlVector<thinkfunc_t> *)fieldInfo.pField;
    ...
    for (int i = 0; i < pUtlVector->Size(); i++) {
      ...
      if (bHasFunc) {
        pSave->WriteFunction(pOwner->GetDataDescMap(), "m_pfnThink",
                             (int *)(char *)ppV, 1);
      }
    }
    pSave->EndBlock();
...
BEGIN_SIMPLE_DATADESC(ResponseContext_t)
  ...
  DEFINE_FIELD(m_iszName, FIELD_STRING), DEFINE_FIELD(m_iszValue, FIELD_STRING),
...
BEGIN_DATADESC_NO_BASE(CBaseEntity)
  ...
  DEFINE_KEYFIELD(m_iClassname, FIELD_STRING, "classname"),
      ...
      DEFINE_KEYFIELD(m_iHammerID, FIELD_INTEGER,
                      "hammerid"),  // save ID numbers so that entities can be
                                    // tracked between save/restore and vmf
  ...
  // DEFINE_FIELD( m_PredictableID, CPredictableId ),
      ...
      //(not saved, debug field only, and think transient to boot)
      ...
      DEFINE_FIELD(m_iName, FIELD_STRING), DEFINE_EMBEDDED(m_Collision),
      ...
      // Local time is local to each object.  It doesn't need to be re-based if
      // the clock changes.  Therefore it is saved as a FIELD_FLOAT, not a
      // FIELD_TIME
      ...
      DEFINE_FIELD(
          m_vecOrigin,
          FIELD_VECTOR),  // NOTE: MUST BE IN LOCAL SPACE, NOT
                          // POSITION_VECTOR!!! (see CBaseEntity::Restore)
...
//	DEFINE_FIELD( m_hPlayerSimulationOwner, FIELD_EHANDLE ),
      ...
      // read it in, because we can only set it after the team entity has been
      // read in, which may or may not actually occur before the entity is
      // parsed. Therefore, we set the TeamNum from the InitialTeamNum in
      // Activate
      ...
      DEFINE_INPUTFUNC(FIELD_BOOLEAN, "AlternativeSorting",
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "SetParentAttachment",
                       InputSetParentAttachment),
      DEFINE_INPUTFUNC(FIELD_STRING, "SetParentAttachmentMaintainOffset",
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableDamageForces",
                       InputEnableDamageForces),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableDamageForces",
      ...
      DEFINE_OUTPUT(m_OnUser1, "OnUser1"), DEFINE_OUTPUT(m_OnUser2, "OnUser2"),
      DEFINE_OUTPUT(m_OnUser3, "OnUser3"), DEFINE_OUTPUT(m_OnUser4, "OnUser4"),

      // Function Pointers
      DEFINE_FUNCTION(SUB_Remove), DEFINE_FUNCTION(SUB_DoNothing),
      DEFINE_FUNCTION(SUB_StartFadeOut),
      DEFINE_FUNCTION(SUB_StartFadeOutInstant), DEFINE_FUNCTION(SUB_FadeOut),
      DEFINE_FUNCTION(SUB_Vanish), DEFINE_FUNCTION(SUB_CallUseToggle),
...
//  Entities that need to unlink themselves from other entities should do the
//  unlinking here rather than in their destructor.  The reason why is that when
//  the global entity list is told to Clear(), it first takes a pass through all
//  active entities and calls UTIL_Remove on each such entity.  Then it calls
//  the delete function on each deleted entity in the list.
// In the old code, the objects were simply destroyed in order and there was no
// guarantee that the
//  destructor of one object would not try to access another object that might
//  already have been destructed (especially since the entity list order is more
//  or less random!).
// NOTE:  You should never call delete directly on an entity (there's an assert
// now), see note
//  at CBaseEntity::~CBaseEntity for more information.
//
// NOTE:  You should chain to BaseClass::UpdateOnRemove after doing your own
// cleanup code, e.g.:
//
...
// }
//
// In general, this function updates global tables that need to know about
...
void CBaseEntity::UpdateOnRemove(void) {
  ...
  if (edict()) {
    ...
    if (GetFlags() & FL_GRAPHED) {
      /*	<<TODO>>
      // this entity was a LinkEnt in the world node graph, so we must remove it
      from
      ...
      for ( int i = 0 ; i < WorldGraph.m_cLinks ; i++ )
      {
              ...
              {
                      // if this link has a link ent which is the same ent that
      is removing itself, remove it! WorldGraph.m_pLinkPool [ i ].m_pLinkEnt =
      NULL;
              }
      }
      */
...
int CBaseEntity::ObjectCaps(void) {
  ...
  // We inherit our parent's use capabilities so that we can forward use
  // commands to our parent.
  CBaseEntity *pParent = GetParent();
  if (pParent) {
    int caps = pParent->ObjectCaps();

    if (!bIsBrush)
      caps &= (FCAP_ACROSS_TRANSITION | FCAP_IMPULSE_USE | FCAP_CONTINUOUS_USE |
               FCAP_ONOFF_USE | FCAP_DIRECTIONAL_USE);
    else
      caps &= (FCAP_IMPULSE_USE | FCAP_CONTINUOUS_USE | FCAP_ONOFF_USE |
               FCAP_DIRECTIONAL_USE);

    if (pParent->IsPlayer()) caps |= FCAP_ACROSS_TRANSITION;
  ...
  if (GetParent()) {
    ...
    parentCaps &= (FCAP_IMPULSE_USE | FCAP_CONTINUOUS_USE | FCAP_ONOFF_USE |
                   FCAP_DIRECTIONAL_USE);
  }

  model_t *pModel = GetModel();
  if (pModel && modelinfo->GetModelType(pModel) == mod_brush) return parentCaps;
...
void CBaseEntity::StartTouch(CBaseEntity *pOther) {
  // notify parent
  if (m_pParent != NULL) m_pParent->StartTouch(pOther);
}

void CBaseEntity::Touch(CBaseEntity *pOther) {
  if (m_pfnTouch) (this->*m_pfnTouch)(pOther);

  // notify parent of touch
  if (m_pParent != NULL) m_pParent->Touch(pOther);
}
...
//-----------------------------------------------------------------------------
// Purpose: Dispatches blocked events to this entity's blocked handler, set via
// SetBlocked. Input  : pOther - The entity that is blocking us.
//-----------------------------------------------------------------------------
...
//			value -
//-----------------------------------------------------------------------------
void CBaseEntity::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
static CBaseEntity *FindPhysicsBlocker(IPhysicsObject *pPhysics,
  ...
  while (pSnapshot->IsValid()) {
    ...
    CBaseEntity *pOtherEntity =
        static_cast<CBaseEntity *>(pOther->GetGameData());
    bool inList = false;
...
static void ComputePushStartMatrix(matrix3x4_t &start, CBaseEntity *pEntity,
                                   const pushblock_t &params) {
  Vector localOrigin;
  QAngle localAngles;
  if (params.pList) {
    ...
    localOrigin = params.pRootParent->GetAbsOrigin() -
                  params.pRootParent->GetAbsVelocity() * params.movetime;
    localAngles =
        params.pRootParent->GetAbsAngles() -
        params.pRootParent->GetLocalAngularVelocity() * params.movetime;
  }
...
static void CheckPushedEntity(CBaseEntity *pEntity, pushblock_t &params) {
  ...
  if (pEntity == params.pRootParent) {
    if (pEntity->GetLocalAngularVelocity() == vec3_angle) checkrot = false;
    if (pEntity->GetLocalVelocity() == vec3_origin) checkmove = false;
  } else {
#if DEBUG_PUSH_MESSAGES
  ...
  if (checkmove) {
    ...
    // if target and entity origin are not in sync, then the position of the
    // entity was updated by something outside of push physics
      ...
      if (dist > 1) {
        ...
        if (pEntity == params.pRootParent) {
          if (params.pList) {
            ...
            float localTargetAmt =
                DotProduct(pEntity->GetLocalOrigin(), localVel);
            movementAmount = targetAmount +
                             DotProduct(params.pList->localOrigin, localVel) -
                             localTargetAmt;
          }
        } else {
          matrix3x4_t start;
  ...
  if (checkrot) {
    ...
    if (fabsf(deltaAngle) > 0.5f) {
      ...
      if (fabsf(deltaTargetAngle) > 0.01f) {
        ...
        if (pEntity == params.pRootParent) {
          expectedDist =
              pEntity->GetLocalAngularVelocity().Length() * params.movetime;
        } else {
          matrix3x4_t start;
          ComputePushStartMatrix(start, pEntity, params);
          Vector startAxis;
          ...
          RotationDeltaAxisAngle(startAngles, pEntity->GetAbsAngles(),
                                 startAxis, startAngle);
...
void CBaseEntity::VPhysicsUpdatePusher(IPhysicsObject *pPhysics) {
  ...
  if (GetSolid() != SOLID_BSP &&
    ...
    // NDebugOverlay::BoxAngles( origin, CollisionProp()->OBBMins(),
    // CollisionProp()->OBBMaxs(), angles, 255,0,0,0, gpGlobals->frametime);
    ...
    if (params.pBlockedEntity) {
      ...
      if (moveback > 0) {
        ...
        if (physLocalTime <= (m_flVPhysicsUpdateLocalTime + movetime * 0.99f)) {
          ...
          if (params.pBlockedEntity->VPhysicsGetObject()->GetContactPoint(
                  NULL, &pOther)) {
          ...
          if (pList) {
            ...
            CBaseEntity *pPhysicsBlocker = FindPhysicsBlocker(
                VPhysicsGetObject(), *pList, pList->pushVec[0]);
            if (pPhysicsBlocker) {
...
void CBaseEntity::PhysicsRelinkChildren(float dt) {
  ...
  for (child = FirstMoveChild(); child != NULL; child = child->NextMovePeer()) {
    ...
    if (child->GetMoveType() != MOVETYPE_VPHYSICS) {
      ...
      // the only case where this is valid is if this entity is an attached
      // ragdoll. So assert here to catch the non-ragdoll case.
      Assert(0);
    }
...
void CBaseEntity::PhysicsTouchTriggers(const Vector *pPrevAbsOrigin) {
  ...
  if (pEdict && !IsWorld()) {
    ...
    bool isSolidCheckTriggers =
        IsSolid() &&
        !isTriggerCheckSolids;  // NOTE: Moving triggers (items, ammo etc) are
                                // not checked against other triggers ot reduce
                                // the number of touchlinks created
    if (!(isSolidCheckTriggers || isTriggerCheckSolids)) return;
...
void CBaseEntity::VPhysicsShadowCollision(int index,
...
void CBaseEntity::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  // Don't make sounds / effects if neither entity is MOVETYPE_VPHYSICS.  The
  // game physics should have done so.
  if (GetMoveType() != MOVETYPE_VPHYSICS &&
      pHitEntity->GetMoveType() != MOVETYPE_VPHYSICS)
    return;

  if (pEvent->deltaCollisionTime < 0.5 && (pHitEntity == this)) return;

  // don't make noise for hidden/invisible/sky materials
  surfacedata_t *phit =
      physprops->GetSurfaceData(pEvent->surfaceProps[otherIndex]);
  const surfacedata_t *pprops =
      physprops->GetSurfaceData(pEvent->surfaceProps[index]);
  if (phit->game.material == 'X' || pprops->game.material == 'X') return;

  if (pHitEntity == this) {
    PhysCollisionSound(this, pEvent->pObjects[index], CHAN_BODY,
                       pEvent->surfaceProps[index],
                       pEvent->surfaceProps[otherIndex],
                       pEvent->deltaCollisionTime, pEvent->collisionSpeed);
  } else {
    PhysCollisionSound(this, pEvent->pObjects[index], CHAN_STATIC,
                       pEvent->surfaceProps[index],
                       pEvent->surfaceProps[otherIndex],
                       pEvent->deltaCollisionTime, pEvent->collisionSpeed);
  }
...
void CBaseEntity::VPhysicsFriction(IPhysicsObject *pObject, float energy,
...
bool CBaseEntity::Intersects(CBaseEntity *pOther) {
  if (!edict() || !pOther->edict()) return false;

  CCollisionProperty *pMyProp = CollisionProp();
  CCollisionProperty *pOtherProp = pOther->CollisionProp();

  return IsOBBIntersectingOBB(
      pMyProp->GetCollisionOrigin(), pMyProp->GetCollisionAngles(),
      pMyProp->OBBMins(), pMyProp->OBBMaxs(), pOtherProp->GetCollisionOrigin(),
      pOtherProp->GetCollisionAngles(), pOtherProp->OBBMins(),
      pOtherProp->OBBMaxs());
}
...
//=========================================================
bool CBaseEntity::FVisible(CBaseEntity *pEntity, int traceMask,
  ...
  if (ai_LOS_mode.GetBool()) {
    UTIL_TraceLine(vecLookerOrigin, vecTargetOrigin, traceMask, this,
    ...
    UTIL_TraceLine(vecLookerOrigin, vecTargetOrigin, traceMask, &traceFilter,
                   &tr);
  }
  ...
  if (tr.fraction != 1.0 || tr.startsolid) {
    ...
    if (pEntity && pEntity->IsPlayer()) {
      ...
      if (tr.m_pEnt == pPlayer->GetVehicleEntity()) return true;
    }
...
//=========================================================
bool CBaseEntity::FVisible(const Vector &vecTarget, int traceMask,
                           CBaseEntity **ppBlocker) {
#if HL1_DLL
  ...
  if (ai_LOS_mode.GetBool()) {
    UTIL_TraceLine(vecLookerOrigin, vecTarget, traceMask, this,
                   COLLISION_GROUP_NONE, &tr);
  } else {
    // If we're doing an LOS search, include NPCs.
    if (traceMask == MASK_BLOCKLOS) {
...
ConVar ai_debug_los(
    "ai_debug_los", "0", FCVAR_CHEAT,
    "NPC Line-Of-Sight debug mode. If 1, solid entities that block NPC LOC "
    "will be highlighted with white bounding boxes. If 2, it'll show non-solid "
    "entities that would do it if they were solid.",
    CC_AI_LOS_Debug);

Class_T CBaseEntity::Classify(void) { return CLASS_NONE; }
...
void CBaseEntity::SetShadowCastDistance(float flDesiredDistance,
                                        float flDelay) {
  m_flDesiredShadowCastDistance = flDesiredDistance;
  if (m_flDesiredShadowCastDistance != m_flShadowCastDistance) {
    SetContextThink(&CBaseEntity::ShadowCastDistThink,
                    gpGlobals->curtime + flDelay, "ShadowCastDistThink");
  }
}
...
FORCEINLINE bool NamesMatch(const char *pszQuery, string_t nameToMatch) {
  ...
  while (*pszNameToMatch && *pszQuery) {
    ...
    if (cName != cQuery &&
        tolower(cName) != tolower(cQuery))  // people almost always use
                                            // lowercase, so assume that first
      break;
    ++pszNameToMatch;
    ++pszQuery;
  }

  if (*pszQuery == 0 && *pszNameToMatch == 0) return true;

  // @TODO (toml 03-18-03): Perhaps support real wildcards. Right now, only
  // thing supported is trailing *
  if (*pszQuery == '*') return true;

  return false;
}

bool CBaseEntity::NameMatchesComplex(const char *pszNameOrWildcard) {
  if (!Q_stricmp("!player", pszNameOrWildcard)) return IsPlayer();
...
int CBaseEntity::IsDormant(void) { return IsEFlagSet(EFL_DORMANT); }

bool CBaseEntity::IsInWorld(void) const {
  if (!edict()) return true;
...
int CBaseEntity::ShouldToggle(USE_TYPE useType, int currentState) {
  if (useType != USE_TOGGLE && useType != USE_SET) {
    if ((currentState && useType == USE_ON) ||
        (!currentState && useType == USE_OFF))
      return 0;
  }
  return 1;
}

// NOTE: szName must be a pointer to constant memory, e.g. "NPC_class" because
// the entity will keep a pointer to it after this call.
CBaseEntity *CBaseEntity::Create(const char *szName, const Vector &vecOrigin,
...
// NOTE: szName must be a pointer to constant memory, e.g. "NPC_class" because
// the entity will keep a pointer to it after this call.
CBaseEntity *CBaseEntity::CreateNoSpawn(const char *szName,
...
int CBaseEntity::Restore(IRestore &restore) {
  ...
  // loops through the data description list, restoring each data desc block in
  // order
  ...
  // Also, twiddling with the flags here ensures it gets added to the KD tree
  // dirty list (We don't want to use the saved version of this flag)
  RemoveEFlags(EFL_DIRTY_SPATIAL_PARTITION);
  CollisionProp()->MarkSurroundingBoundsDirty();

  if (edict() && GetModelIndex() != 0 && GetModelName() != NULL_STRING &&
      restore.GetPrecacheMode()) {
    PrecacheModel(STRING(GetModelName()));

    // Adrian: We should only need to do this after we precache. No point in
  ...
  // Tracker 22129
  // This is a hack to make sure that the entity is added to the
  // AddPostClientMessageEntity
  //  list so that EF_NOINTERP can be cleared at the end of the frame.
  //  Otherwise, a restored entity with this flag will not interpolate until the
  //  next time the flag is set.  ywb
...
void CBaseEntity::OnRestore() {
  ...
  // NOTE: We don't do this on transitions, because we'd miss the OnStartTouch
  // call!
  ...
  // Adrian: If I'm restoring with these fields it means I've become a client
  // side ragdoll. Don't create another one, just wait until is my time of being
  // removed.
...
#endif
      // We only need to be back in the parent's list because we're already in
      // the right place and with the right data
      LinkChild(m_pParent, this);
    }
  }

  // We're not save/loading the PVS dirty state. Assume everything is dirty
  // after a restore
  NetworkProp()->MarkPVSInformationDirty();
}
...
bool CBaseEntity::ShouldSavePhysics() { return true; }
...
void *CBaseEntity::operator new(size_t stAllocateBlock, int nBlockUse,
...
bool CBaseEntity::TestCollision(const Ray_t &ray, unsigned int mask,
                                trace_t &trace) {
  return false;
}
...
//-----------------------------------------------------------------------------
bool CBaseEntity::TestHitboxes(const Ray_t &ray, unsigned int fContentsMask,
                               trace_t &tr) {
  return false;
}
...
void CBaseEntity::SetMoveType(MoveType_t val, MoveCollide_t moveCollide) {
  ...
  if (m_MoveType == MOVETYPE_VPHYSICS && val != m_MoveType) {
    if (VPhysicsGetObject() && val != MOVETYPE_NONE) {
      ...
      // You probably meant to call VPhysicsInitShadow() instead of
      // VPhysicsInitNormal()!
  ...
  switch (m_MoveType) {
    ...
    case MOVETYPE_STEP: {
      // This will probably go away once I remove the cvar that controls the
      // test code
    ...
    case MOVETYPE_FLYGRAVITY: {
      // Initialize our water state, because these movetypes care about
      // transitions in/out of water
      UpdateWaterState();
    } break;
    default: {
      SetSimulatedEveryTick(true);
  ...
  // This will probably go away or be handled in a better way once I remove the
  // cvar that controls the test code
...
int CBaseEntity::UpdateTransmitState() {
  ...
  // If an object is the moveparent of something else, don't skip it just
  // because it's marked EF_NODRAW or else
  //  the client won't have a proper origin for the child since the hierarchy
  //  won't be correctly transmitted down
  if (IsEffectActive(EF_NODRAW) && !m_hMoveChild.Get()) {
...
int CBaseEntity::DispatchUpdateTransmitState() {
  edict_t *ed = edict();
  if (m_nTransmitStateOwnedCounter != 0) return ed ? ed->m_fStateFlags : 0;
...
//-----------------------------------------------------------------------------
// Purpose: Note, an entity can override the send table ( e.g., to send less
// data or to send minimal data for
...
int CBaseEntity::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  ...
  /*#ifdef INVASION_DLL
          // Check test network vis distance stuff. Eventually network LOD will
  do this. float flTestDistSqr = pRecipientEntity->GetAbsOrigin().DistToSqr(
  WorldSpaceCenter() ); if ( flTestDistSqr > sv_netvisdist.GetFloat() *
  sv_netvisdist.GetFloat() )
...
void CBaseEntity::SetTransmit(CCheckTransmitInfo *pInfo, bool bAlways) {
  ...
  if (pInfo->m_pTransmitEdict->Get(index)) return;
...
CSkyCamera *CBaseEntity::GetEntitySkybox() {
  ...
  while (pCur) {
    if (engine->CheckAreasConnected(area, pCur->m_skyboxData.area)) return pCur;
...
//------------------------------------------------------------------------------
void CBaseEntity::ComputeWorldSpaceSurroundingBox(Vector *pMins,
                                                  Vector *pMaxs) {
...
void CBaseEntity::DrawInputOverlay(const char *szInputName,
  ...
  if (Value.FieldType() == FIELD_INTEGER) {
    DevMsg(2, "input: (%s,%d) -> (%s,%s), from (%s)\n", szInputName,
           Value.Int(), STRING(m_iClassname), GetDebugName(),
           pCaller ? pCaller->GetDebugName() : NULL);
  } else if (Value.FieldType() == FIELD_STRING) {
    DevMsg(2, "input: (%s,%s) -> (%s,%s), from (%s)\n", szInputName,
           Value.String(), STRING(m_iClassname), GetDebugName(),
           pCaller ? pCaller->GetDebugName() : NULL);
  } else
    DevMsg(2, "input: (%s) -> (%s,%s), from (%s)\n", szInputName,
           STRING(m_iClassname), GetDebugName(),
           pCaller ? pCaller->GetDebugName() : NULL);
}
...
void CBaseEntity::DrawOutputOverlay(CEventAction *ev) {
  ...
  if (ev->m_flDelay) {
    Q_snprintf(bigstring, sizeof(bigstring), "%3.1f  (%s) --> (%s),%.1f) \n",
               gpGlobals->curtime, STRING(ev->m_iTargetInput),
               STRING(ev->m_iTarget), ev->m_flDelay);
  } else {
    Q_snprintf(bigstring, sizeof(bigstring), "%3.1f  (%s) --> (%s)\n",
               gpGlobals->curtime, STRING(ev->m_iTargetInput),
               STRING(ev->m_iTarget));
  ...
  if (ev->m_flDelay) {
    DevMsg(2, "output: (%s,%s) -> (%s,%s,%.1f)\n", STRING(m_iClassname),
           GetDebugName(), STRING(ev->m_iTarget), STRING(ev->m_iTargetInput),
           ev->m_flDelay);
  } else {
...
void CBaseEntity::OnEntityEvent(EntityEvent_t event, void *pEventData) {
  ...
  if (GetMoveType() != MOVETYPE_VPHYSICS) return;
...
ConVar ent_messages_draw("ent_messages_draw", "0", FCVAR_CHEAT,
                         "Visualizes all entity input/output activity.");

//-----------------------------------------------------------------------------
// Purpose: calls the appropriate message mapped function in the entity
// according
//			to the fired action.
...
bool CBaseEntity::AcceptInput(const char *szInputName, CBaseEntity *pActivator,
  ...
  if (ent_messages_draw.GetBool()) {
    if (pCaller != NULL) {
      NDebugOverlay::Line(pCaller->GetAbsOrigin(), GetAbsOrigin(), 255, 255,
                          255, false, 3);
      NDebugOverlay::Box(pCaller->GetAbsOrigin(), Vector(-4, -4, -4),
                         Vector(4, 4, 4), 255, 0, 0, 0, 3);
    }

    NDebugOverlay::Text(GetAbsOrigin(), szInputName, false, 3);
    NDebugOverlay::Box(GetAbsOrigin(), Vector(-4, -4, -4), Vector(4, 4, 4), 0,
                       255, 0, 0, 3);
  }

  // loop through the data description list, restoring each data desc block
  for (datamap_t *dmap = GetDataDescMap(); dmap != NULL; dmap = dmap->baseMap) {
    // search through all the actions in the data description, looking for a
    // match
        ...
        if (!Q_stricmp(dmap->dataDesc[i].externalName, szInputName)) {
          ...
          if (pCaller != NULL) {
            Q_snprintf(szBuffer, sizeof(szBuffer),
                       "(%0.2f) input %s: %s.%s(%s)\n", gpGlobals->curtime,
                       STRING(pCaller->m_iName), GetDebugName(), szInputName,
                       Value.String());
          } else {
            Q_snprintf(szBuffer, sizeof(szBuffer),
          ...
          if (Value.FieldType() != dmap->dataDesc[i].fieldType) {
            if (!(Value.FieldType() == FIELD_VOID &&
                  dmap->dataDesc[i].fieldType ==
                      FIELD_STRING))  // allow empty strings
            {
          ...
          if (pfnInput) {
            ...
            Value.SetOther(((char *)this) +
                           dmap->dataDesc[i].fieldOffset[TD_OFFSET_NORMAL]);

            // TODO: if this becomes evil and causes too many full entity
            // updates, then we should make a macro like this:
            //
            // define MAKE_INPUTVAR(x) void Note##x##Modified() {
            // x.GetForModify(); }
            //
            // Then the datadesc points at that function and we call it here.
            // The only pain is to add that function for all the DEFINE_INPUT
            // calls.
            NetworkStateChanged();
          }
...
//-----------------------------------------------------------------------------
// Purpose: Input handler for the entity color. Ignores alpha since that is
// handled
...
//-----------------------------------------------------------------------------
// Purpose: Called whenever the entity is 'Used'.  This can be when a player
// hits
//			use, or when an entity targets it without an output name
//(legacy entities)
//-----------------------------------------------------------------------------
void CBaseEntity::InputUse(inputdata_t &inputdata) {
  Use(inputdata.pActivator, inputdata.pCaller, (USE_TYPE)inputdata.nOutputID,
      0);
}
...
bool CBaseEntity::ReadKeyField(const char *varName, variant_t *var) {
  ...
  // loop through the data description list, restoring each data desc block
  for (datamap_t *dmap = GetDataDescMap(); dmap != NULL; dmap = dmap->baseMap) {
    // search through all the readable fields in the data description, looking
    // for a match
    for (int i = 0; i < dmap->dataNumFields; i++) {
      if (dmap->dataDesc[i].flags & (FTYPEDESC_OUTPUT | FTYPEDESC_KEY)) {
        if (!Q_stricmp(dmap->dataDesc[i].externalName, varName)) {
          var->Set(
              dmap->dataDesc[i].fieldType,
              ((char *)this) + dmap->dataDesc[i].fieldOffset[TD_OFFSET_NORMAL]);
          return true;
        }
      }
    }
...
//-----------------------------------------------------------------------------
void CBaseEntity::GetInputDispatchEffectPosition(const char *sInputString,
                                                 Vector &pOrigin,
                                                 QAngle &pAngles) {
...
void CBaseEntity::InputKill(inputdata_t &inputdata) {
  // tell owner ( if any ) that we're dead.This is mostly for NPCMaker
  // functionality.
...
// Purpose:
//------------------------------------------------------------------------------
void CBaseEntity::SetParentAttachment(const char *szInputName,
  ...
  if (!iAttachment) {
    Warning(
        "ERROR: Tried to %s for entity %s (%s), but it has no attachment named "
        "%s.\n",
...
//-----------------------------------------------------------------------------
// Purpose: Input handler for changing this entity's movement parent's
// attachment point
//-----------------------------------------------------------------------------
...
// attachment point
//-----------------------------------------------------------------------------
void CBaseEntity::InputSetParentAttachmentMaintainOffset(
    inputdata_t &inputdata) {
  SetParentAttachment("SetParentAttachmentMaintainOffset",
                      inputdata.value.String(), true);
}

//------------------------------------------------------------------------------
// Purpose: Input handler for clearing this entity's movement parent.
//------------------------------------------------------------------------------
void CBaseEntity::InputClearParent(inputdata_t &inputdata) { SetParent(NULL); }
...
//-----------------------------------------------------------------------------
void CBaseEntity::GetVectors(Vector *pForward, Vector *pRight,
                             Vector *pUp) const {
...
//-----------------------------------------------------------------------------
// Purpose: Called once per frame after the server frame loop has finished and
// after all messages being
//  sent to clients have been sent.  NOTE: Only called if scheduled via
//  AddPostClientMessageEntity() !
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CBaseEntity::ChangeTeam(int iTeamNum) { m_iTeamNum = iTeamNum; }

//-----------------------------------------------------------------------------
// Get the Team this entity is on
//-----------------------------------------------------------------------------
CTeam *CBaseEntity::GetTeam(void) const { return GetGlobalTeam(m_iTeamNum); }
...
//-----------------------------------------------------------------------------
bool CBaseEntity::IsInTeam(CTeam *pTeam) const { return (GetTeam() == pTeam); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
int CBaseEntity::GetTeamNumber(void) const { return m_iTeamNum; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseEntity::IsInAnyTeam(void) const { return (GetTeam() != NULL); }

//-----------------------------------------------------------------------------
// Purpose: Returns the type of damage that this entity inflicts.
//-----------------------------------------------------------------------------
int CBaseEntity::GetDamageType() const { return DMG_GENERIC; }

//-----------------------------------------------------------------------------
// process notification
//-----------------------------------------------------------------------------

void CBaseEntity::NotifySystemEvent(
    CBaseEntity *pNotify, notify_system_event_t eventType,
...
static void TeleportEntity(CBaseEntity *pSourceEntity,
  ...
  g_pNotify->ReportTeleportEvent(pTeleport, prevOrigin, prevAngles,
                                 rotatePhysics);
...
void CBaseEntity::Teleport(const Vector *newPosition, const QAngle *newAngles,
                           const Vector *newVelocity) {
  if (g_TeleportStack.Find(this) >= 0) return;
  ...
  if (idx != -1) {
    ...
    if (mdl) {
      CStudioHdr *studioHdr =
          new CStudioHdr(modelinfo->GetStudiomodel(mdl), mdlcache);
...
static CUtlCachedFileData<CModelSoundsCache> g_ModelSoundsCache(
    "modelsounds.cache", MODELSOUNDSCACHE_VERSION, 0,
    UTL_CACHED_FILE_USE_FILESIZE, false);
 ...
 public:
  ...
  virtual void LevelShutdownPostEntity() {
    if (IsX360()) {
      ...
      // It is the common case that PrecacheModel() still gets called (and needs
      // this table), after LevelInitPostEntity, as PrecacheModel() redundantly
      // precaches.
...
class CWatchForModelAccess : public CAutoGameSystem {
 public:
  ...
  virtual void Shutdown() { filesystem->RemoveLoggingFunc(&ModelLogFunc); }
};
...
// HACK:  This must match the #define in cl_animevent.h in the client .dll
// code!!!
...
void CBaseEntity::PrecacheModelComponents(int nModelIndex) {
  ...
  // particles
  {
    ...
    if (pModelKeyValues->LoadFromBuffer(
            modelinfo->GetModelName(pModel),
            modelinfo->GetModelKeyValueText(pModel))) {
      KeyValues *pParticleEffects = pModelKeyValues->FindKey("Particles");
      if (pParticleEffects) {
        // Start grabbing the sounds and slotting them in
        for (KeyValues *pSingleEffect = pParticleEffects->GetFirstSubKey();
             pSingleEffect; pSingleEffect = pSingleEffect->GetNextKey()) {
          const char *pParticleEffectName =
              pSingleEffect->GetString("name", "");
    ...
    if (studioHdr.IsValid()) {
      ...
      for (int i = 0; i < nSeqCount; ++i) {
        ...
        for (int j = 0; j < nEventCount; ++j) {
          ...
          // 360 precaches the model sounds now at init time, the cost is now
          // ~250 msecs worst case. The disk based solution was not needed. Now
          // at runtime partly due to already crawling the sequences for the
          // particles and the expensive part was redundant
          // PrecacheScriptSound(), which is now prevented by a local symbol
          // table.
          if (IsX360()) {
            switch (pEvent->event) {
              default: {
                if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) &&
                    (pEvent->event == AE_SV_PLAYSOUND)) {
              ...
              case CL_EVENT_FOOTSTEP_RIGHT: {
                ...
                Q_snprintf(soundname, sizeof(soundname), "%s.RunFootstepLeft",
                           options);
                PrecacheSoundHelper(soundname);
                Q_snprintf(soundname, sizeof(soundname), "%s.RunFootstepRight",
                           options);
                PrecacheSoundHelper(soundname);
                Q_snprintf(soundname, sizeof(soundname), "%s.FootstepLeft",
                           options);
                PrecacheSoundHelper(soundname);
                Q_snprintf(soundname, sizeof(soundname), "%s.FootstepRight",
                           options);
                PrecacheSoundHelper(soundname);
              } break;
              case AE_CL_PLAYSOUND: {
                if (!(pEvent->type & AE_TYPE_CLIENT)) break;

                if (pEvent->pszOptions()[0]) {
                  PrecacheSoundHelper(pEvent->pszOptions());
                } else {
                  Warning(
                      "-- Error --:  empty soundname, .qc error on "
                      "AE_CL_PLAYSOUND in model %s, sequence %s, animevent # "
                      "%i\n",
                      studioHdr.GetRenderHdr()->pszName(), seq.pszLabel(),
                      j + 1);
                }
              } break;
              case CL_EVENT_SOUND:
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseEntity::Remove() { UTIL_Remove(this); }

//   Entity degugging console commands
extern CBaseEntity *FindPickerEntity(CBasePlayer *pPlayer);
extern void SetDebugBits(CBasePlayer *pPlayer, const char *name, int bit);
extern CBaseEntity *GetNextCommandEntity(CBasePlayer *pPlayer, const char *name,
                                         CBaseEntity *ent);
...
static ConCommand ent_text(
    "ent_text", CC_Ent_Text,
    "Displays text debugging information about the given entity(ies) on top of "
    "the entity (See Overlay Text)\n\tArguments:   	{entity_name} / "
...
static ConCommand ent_bbox(
    "ent_bbox", CC_Ent_BBox,
    "Displays the movement bounding box for the given entity(ies) in orange.  "
    "Some entites will also display entity specific overlays.\n\tArguments:   "
    "	{entity_name} / {class_name} / no argument picks what player is "
    "looking at ",
    FCVAR_CHEAT);
...
static ConCommand ent_absbox(
    "ent_absbox", CC_Ent_AbsBox,
    "Displays the total bounding box for the given entity(s) in green.  Some "
...
static ConCommand ent_rbox(
    "ent_rbox", CC_Ent_RBox,
    "Displays the total bounding box for the given entity(s) in green.  Some "
    "entites will also display entity specific overlays.\n\tArguments:   "
    "	{entity_name} / {class_name} / no argument picks what player is "
    "looking at ",
...
static ConCommand ent_attachments(
    "ent_attachments", CC_Ent_AttachmentPoints,
...
static ConCommand ent_viewoffset(
    "ent_viewoffset", CC_Ent_ViewOffset,
    ...
    if (index) {
      ...
      while ((ent = gEntList.NextEnt(ent)) != NULL) {
        if ((ent->GetEntityName() != NULL_STRING &&
             FStrEq(args[1], STRING(ent->GetEntityName()))) ||
            (ent->m_iClassname != NULL_STRING &&
             FStrEq(args[1], STRING(ent->m_iClassname))) ||
            (ent->GetClassname() != NULL &&
             FStrEq(args[1], ent->GetClassname()))) {
          pEntity = ent;
          break;
        }
      }
...
static ConCommand ent_remove(
    "ent_remove", CC_Ent_Remove,
...
static ConCommand ent_remove_all(
    "ent_remove_all", CC_Ent_RemoveAll,
...
static ConCommand ent_setname(
    "ent_setname", CC_Ent_SetName,
    "Sets the targetname of the given entity(s)\n\tArguments:   	{new "
    "entity name} {entity_name} / {class_name} / no argument picks what player "
    "is looking at ",
    FCVAR_CHEAT);
...
void CC_Find_Ent(const CCommand &args) {
  ...
  Msg("Searching for entities with class/target name containing substring: "
      "'%s'\n",
      pszSubString);
  ...
  while ((ent = gEntList.NextEnt(ent)) != NULL) {
    ...
    if (bMatches) {
      iCount++;
      Msg("   '%s' : '%s' (entindex %d) \n", ent->GetClassname(),
          ent->GetEntityName().ToCStr(), ent->entindex());
    }
  }

  Msg("Found %d matches.\n", iCount);
}
static ConCommand find_ent(
    "find_ent", CC_Find_Ent,
    "Find and list all entities with classnames or targetnames that contain "
...
void CC_Find_Ent_Index(const CCommand &args) {
  ...
  if (pEnt) {
    Msg("   '%s' : '%s' (entindex %d) \n", pEnt->GetClassname(),
        pEnt->GetEntityName().ToCStr(), iIndex);
  } else {
    Msg("Found no entity at %d.\n", iIndex);
  }
}
static ConCommand find_ent_index("find_ent_index", CC_Find_Ent_Index,
                                 "Display data for entity matching specified "
...
void CC_Ent_Dump(const CCommand &args) {
  ...
  if (args.ArgC() < 2) {
    ...
    while ((ent = gEntList.FindEntityByName(ent, args[1])) != NULL) {
      bFound = true;
      for (datamap_t *dmap = ent->GetDataDescMap(); dmap != NULL;
           dmap = dmap->baseMap) {
        // search through all the actions in the data description, printing out
        // details
        for (int i = 0; i < dmap->dataNumFields; i++) {
          variant_t var;
          if (ent->ReadKeyField(dmap->dataDesc[i].externalName, &var)) {
            ...
            switch (var.FieldType()) {
              case FIELD_STRING:
                ...
                break;
              case FIELD_INTEGER:
                if (var.Int()) Q_snprintf(buf, sizeof(buf), "%d", var.Int());
                break;
              case FIELD_FLOAT:
                if (var.Float())
              ...
              case FIELD_EHANDLE: {
                ...
                if (var.Entity()) {
                  Q_snprintf(buf, sizeof(buf), "%s",
                             STRING(var.Entity()->GetEntityName()));
                }
              } break;
            }
...
static ConCommand ent_dump("ent_dump", CC_Ent_Dump,
                           "Usage:\n   ent_dump <entity name>\n", FCVAR_CHEAT);
...
class CEntFireAutoCompletionFunctor : public ICommandCallback,
 ...
 public:
  virtual void CommandCallback(const CCommand &command) {
    ...
    if (command.ArgC() < 2) {
      ...
      // Don't allow them to run anything on a point_servercommand unless
      // they're the host player. Otherwise they can ent_fire and run any
      // command on the server. Admittedly, they can only do the ent_fire if
      // sv_cheats is on, but people complained about users resetting the rcon
      // password if the server briefly turned on cheats like this:
      //    give point_servercommand
      //    ent_fire point_servercommand command "rcon_password mynewpassword"
      if (gpGlobals->maxClients > 1 &&
          V_stricmp(target, "point_servercommand") == 0) {
        if (engine->IsDedicatedServer()) return;

        CBasePlayer *pHostPlayer = UTIL_GetListenServerHost();
        if (pPlayer != pHostPlayer) return;
      }
  ...
  virtual int CommandCompletionCallback(const char *partial,
    ...
    while ((pos = gEntList.NextEnt(pos)) != NULL) {
      ...
      if (pos->GetEntityName() == NULL_STRING) continue;
    ...
    for (int i = symbols.FirstInorder(); i != symbols.InvalidIndex();
 ...
 private:
  int EntFire_AutoCompleteInput(const char *partial,
                                CUtlVector<CUtlString> &commands) {
    const char *cmdname = "ent_fire";
    ...
    for (datamap_t *dmap = target->GetDataDescMap(); dmap != NULL;
         dmap = dmap->baseMap) {
      ...
      for (int i = 0; i < c; i++) {
        ...
        // Only want inputs
        if (!(field->flags & FTYPEDESC_INPUT)) continue;

        // Only want input functions
        if (field->flags & FTYPEDESC_SAVE) continue;
...
static ConCommand ent_fire(
    "ent_fire", &g_EntFireAutoComplete,
    "Usage:\n   ent_fire <target> [action] [value] [delay]\n", FCVAR_CHEAT,
...
static ConCommand ent_cancelpendingentfires(
    "ent_cancelpendingentfires", CC_Ent_CancelPendingEntFires,
    "Cancels all ent_fire created outputs that are currently waiting for their "
    "delay to expire.");
...
void CC_Ent_Info(const CCommand &args) {
  ...
  if (args.ArgC() < 2) {
    ...
    if (ent) {
      ...
      ClientPrint(pPlayer, HUD_PRINTCONSOLE,
                  UTIL_VarArgs("no such entity %s\n", args[1]));
    }
  }
}
static ConCommand ent_info("ent_info", CC_Ent_Info,
                           "Usage:\n   ent_info <class name>\n", FCVAR_CHEAT);
...
static ConCommand ent_messages(
    "ent_messages", CC_Ent_Messages,
    "Toggles input/output message display for the selected entity(ies).  The "
    "name of the entity will be displayed as well as any messages that it "
...
static ConCommand ent_pause(
    "ent_pause", CC_Ent_Pause,
    "Toggles pausing of input/output message processing for entities.  When "
    "turned on processing of all message will stop.  Any messages displayed "
    "with 'ent_messages' will stop fading and be displayed indefinitely. To "
    "step through the messages one by one use 'ent_step'.",
    FCVAR_CHEAT);
...
static ConCommand picker(
    "picker", CC_Ent_Picker,
    "Toggles 'picker' mode.  When picker is on, the bounding box, pivot and "
    "debugging text is displayed for whatever entity the player is looking "
...
static ConCommand ent_pivot(
    "ent_pivot", CC_Ent_Pivot,
    "Displays the pivot for the given entity(ies).\n\t(y=up=green, "
    "z=forward=blue, x=left=red). \n\tArguments:   	{entity_name} / "
    "{class_name} / no argument picks what player is looking at ",
    FCVAR_CHEAT);
...
static ConCommand ent_step("ent_step", CC_Ent_Step,
                           "When 'ent_pause' is set this will step through one "
...
void CBaseEntity::CalcAbsolutePosition(void) {
  ...
  ConcatTransforms(GetParentToWorldTransform(scratchSpace),
                   m_rgflCoordinateFrame, tmpMatrix);
...
void CBaseEntity::CalcAbsoluteVelocity() {
  if (!IsEFlagSet(EFL_DIRTY_ABSVELOCITY)) return;
...
//-----------------------------------------------------------------------------
void CBaseEntity::ComputeAbsPosition(const Vector &vecLocalPosition,
                                     Vector *pAbsPosition) {
  CBaseEntity *pMoveParent = GetMoveParent();
  if (!pMoveParent) {
...
// Computes the abs position of a point specified in local space
//-----------------------------------------------------------------------------
void CBaseEntity::ComputeAbsDirection(const Vector &vecLocalDirection,
...
matrix3x4_t &CBaseEntity::GetParentToWorldTransform(matrix3x4_t &tempMatrix) {
  ...
  if (m_iParentAttachment != 0) {
    ...
    if (pAnimating &&
        pAnimating->GetAttachment(m_iParentAttachment, tempMatrix)) {
      return tempMatrix;
    }
  }

  // If we fall through to here, then just use the move parent's abs origin and
  // angles.
...
void CBaseEntity::SetAbsOrigin(const Vector &absOrigin) {
  ...
  if (m_vecAbsOrigin == absOrigin) return;
...
void CBaseEntity::SetAbsAngles(const QAngle &absAngles) {
  ...
  if (m_angAbsRotation == absAngles) return;
...
void CBaseEntity::SetAbsVelocity(const Vector &vecAbsVelocity) {
  if (m_vecAbsVelocity == vecAbsVelocity) return;
...
void CBaseEntity::SetAbsAngularVelocity( const QAngle &vecAbsAngVelocity )
{
        ...
        // Moveparent case: transform the abs relative angular vel into local
space matrix3x4_t worldToParent, localMatrix; MatrixInvert(
pMoveParent->EntityToWorldTransform(), worldToParent ); ConcatTransforms(
worldToParent, entityToWorld, localMatrix ); MatrixAngles( localMatrix,
m_vecAngVelocity );
}
*/

//-----------------------------------------------------------------------------
// Methods that modify local physics state, and let us know to compute abs state
// later
//-----------------------------------------------------------------------------
...
void CBaseEntity::SetLocalAngles(const QAngle &angles) {
  ...
  //        handling things like +/-180 degrees properly. This should be
  //        revisited.
  // QAngle angleNormalize( AngleNormalize( angles.x ), AngleNormalize( angles.y
  // ), AngleNormalize( angles.z ) );
...
bool CBaseEntity::IsFloating() {
  if (!IsEFlagSet(EFL_TOUCHING_FLUID)) return false;
...
//-----------------------------------------------------------------------------
// Purpose: Created predictable and sets up Id.  Note that persist is ignored on
// the server. Input  : *classname -
//			*module -
//			line -
//			persist -
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CBaseEntity *CBaseEntity::CreatePredictedEntityByName(
...
void CBaseEntity::ModifyOrAppendCriteria(AI_CriteriaSet &set) {
  ...
  CWorld *world = dynamic_cast<CWorld *>(
      CBaseEntity::Instance(engine->PEntityOfEntIndex(0)));
  if (world) {
...
//			"" -
//-----------------------------------------------------------------------------
void CBaseEntity::AppendContextToCriteria(AI_CriteriaSet &set,
                                          const char *prefix /*= ""*/) {
...
// Output : int
//-----------------------------------------------------------------------------
int CBaseEntity::GetContextCount() const { return m_ResponseContexts.Count(); }
...
int CBaseEntity::FindContextByName(const char *name) const {
  ...
  for (int i = 0; i < c; i++) {
    if (FStrEq(name, GetContextName(i))) return i;
  }
...
//-----------------------------------------------------------------------------
IResponseSystem *CBaseEntity::GetResponseSystem() { return NULL; }
...
void CBaseEntity::InputAddOutput(inputdata_t &inputdata) {
  ...
  if (sChar) {
    *sChar = '\0';
    ...
    // Has to be done this way because Hammer doesn't allow ,'s inside
    // parameters.
    ...
    Warning(
        "AddOutput input fired with bad string. Format: <output name> "
        "<targetname>,<inputname>,<parameter>,<delay>,<max times to fire (-1 "
        "== infinite)>\n");
  }
}
...
void CBaseEntity::DispatchResponse(const char *conceptName) {
  ...
  CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
  if (pPlayer) pPlayer->ModifyOrAppendPlayerCriteria(set);
...
void CC_Ent_Show_Response_Criteria(const CCommand &args) {
  CBaseEntity *pEntity = NULL;
  while ((pEntity = GetNextCommandEntity(UTIL_GetCommandClient(), args[1],
                                         pEntity)) != NULL) {
    pEntity->DumpResponseCriteria();
  }
}
static ConCommand ent_show_response_criteria(
    "ent_show_response_criteria", CC_Ent_Show_Response_Criteria,
    "Print, to the console, an entity's current criteria set used to select "
...
static ConCommand ent_autoaim(
    "ent_autoaim", CC_Ent_Autoaim,
...
CAI_BaseNPC *CBaseEntity::MyNPCPointer(void) {
  if (IsNPC()) return assert_cast<CAI_BaseNPC *>(this);
...
bool CBaseEntity::ComputeStepSimulationNetworkOrigin(StepSimulationData *step) {
  ...
  if (step->m_Previous2.nTickCount == 0 ||
      step->m_Previous2.nTickCount >= step->m_Previous.nTickCount) {
    ...
    Hermite_Spline(step->m_Previous2.vecOrigin, step->m_Previous.vecOrigin,
                   step->m_Next.vecOrigin, frac, step->m_vecNetworkOrigin);
  }
...
bool CBaseEntity::ComputeStepSimulationNetworkAngles(StepSimulationData *step) {
  ...
  if (step->m_Previous2.nTickCount == 0 ||
    ...
    QuaternionBlend(step->m_Previous.qRotation, step->m_Next.qRotation, frac,
                    outangles);
...
bool CBaseEntity::AddStepDiscontinuity(float flTime, const Vector &vecOrigin,
                                       const QAngle &vecAngles) {
...
Vector CBaseEntity::GetStepOrigin(void) const { return GetLocalOrigin(); }

QAngle CBaseEntity::GetStepAngles(void) const { return GetLocalAngles(); }

//-----------------------------------------------------------------------------
// Purpose: For each client who appears to be a valid recipient, checks the
// client has disabled CC and if so, removes them from
//  the recipient list.
// Input  : filter -
//-----------------------------------------------------------------------------
void CBaseEntity::RemoveRecipientsIfNotCloseCaptioning(
  ...
  for (int i = c - 1; i >= 0; --i) {
    ...
    CBasePlayer *player =
        static_cast<CBasePlayer *>(CBaseEntity::Instance(playerIndex));
    if (!player) continue;
    const char *cvarvalue =
        engine->GetClientConVarValue(playerIndex, "closecaption");
    Assert(cvarvalue);
    if (!cvarvalue[0]) continue;
...
//-----------------------------------------------------------------------------
// Purpose: Wrapper to emit a sentence and also a close caption token for the
// sentence as appropriate. Input  : filter -
//			iEntIndex -
//			iChannel -
//			iSentenceIndex -
//			flVolume -
...
void CBaseEntity::EmitSentenceByIndex(
    IRecipientFilter &filter, int iEntIndex, int iChannel, int iSentenceIndex,
    float flVolume, soundlevel_t iSoundlevel, int iFlags /*= 0*/,
    int iPitch /*=PITCH_NORM*/, const Vector *pOrigin /*=NULL*/,
    const Vector *pDirection /*=NULL*/, bool bUpdatePositions /*=true*/,
    float soundtime /*=0.0f*/) {
  CUtlVector<Vector> dummy;
  enginesound->EmitSentenceByIndex(
      filter, iEntIndex, iChannel, iSentenceIndex, flVolume, iSoundlevel,
      iFlags, iPitch, pOrigin, pDirection, &dummy, bUpdatePositions, soundtime);
}
...
bool CServerOnlyPointEntity::KeyValue(const char *szKeyName,
                                      const char *szValue) {
  if (FStrEq(szKeyName, "mins") || FStrEq(szKeyName, "maxs")) {
    Warning("Warning! Can't specify mins/maxs for point entities! (%s)\n",
            GetClassname());
...
//-----------------------------------------------------------------------------
// Purpose: Sets the entity invisible, and makes it remove itself on the next
// frame
//-----------------------------------------------------------------------------
...
void CBaseEntity::SUB_StartFadeOutInstant() { SUB_StartFadeOut(0, true); }
...
void CBaseEntity::SUB_Vanish(void) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if ((pPlayer = UTIL_PlayerByIndex(i)) != NULL) {
      ...
      if (flDistSqr < (MIN_CORPSE_FADE_DIST * MIN_CORPSE_FADE_DIST)) return;
...
bool CBaseEntity::SUB_AllowedToFade(void) {
  ...
  if (pPlayer && pPlayer->FInViewCone(this)) return false;
...
inline bool AnyPlayersInHierarchy_R(CBaseEntity *pEnt) {
  if (pEnt->IsPlayer()) return true;

  for (CBaseEntity *pCur = pEnt->FirstMoveChild(); pCur;
       pCur = pCur->NextMovePeer()) {
    if (AnyPlayersInHierarchy_R(pCur)) return true;
  }
...
void CC_Ent_Create(const CCommand &args) {
  ...
  CBaseEntity *entity =
      dynamic_cast<CBaseEntity *>(CreateEntityByName(args[1]));
  if (entity) {
    entity->Precache();
    ...
    if (tr.fraction != 1.0) {
      // Raise the end position a little up off the floor, place the npc and
      // drop him down
      tr.endpos.z += 12;
...
//------------------------------------------------------------------------------
bool CC_GetCommandEnt(const CCommand &args, CBaseEntity **ent,
                      Vector *vecTargetPoint, QAngle *vecPlayerAngle) {
  // Find the entity
  *ent = NULL;
...
static ConCommand ent_teleport(
    "ent_teleport", CC_Ent_Teleport,
...
static ConCommand ent_orient(
    "ent_orient", CC_Ent_Orient,
    "Orient the specified entity to match the player's angles. By default, "
    "only orients target entity's YAW. Use the 'allangles' option to orient on "
