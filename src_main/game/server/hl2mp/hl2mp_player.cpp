...
Activity CHL2MP_Player::TranslateTeamActivity(Activity ActToTranslate) {
  if (m_iModelType == TEAM_COMBINE) return ActToTranslate;

  if (ActToTranslate == ACT_RUN) return ACT_RUN_AIM_AGITATED;
  ...
  /*	if ( GetNextTeamChangeTime() >= gpGlobals->curtime )
          {
                  ...
                  Q_snprintf( szReturnString, sizeof( szReturnString ), "Please
...
void CHL2MP_Player::DeathSound(const CTakeDamageInfo &info) {
  if (m_hRagdoll && m_hRagdoll->GetBaseAnimating()->IsDissolving()) return;
  ...
  if (GetParametersForSound(szStepSound, params, pModelName) == false) return;
...
CON_COMMAND(timeleft, "prints the time remaining in the match") {
  ...
  if (iTimeRemaining == 0) {
    ...
    if (pPlayer) {
      ClientPrint(pPlayer, HUD_PRINTTALK, "Time left in map: %s1:%s2", minutes,
                  seconds);
    } else {
