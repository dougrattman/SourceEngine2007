...
#define SF_BUTTON_JIGGLE_ON_USE_LOCKED \
  8192  // whether to jiggle if someone uses us when we're locked
...
BEGIN_DATADESC(CBaseButton)
  ...
  DEFINE_KEYFIELD(m_vecMoveDir, FIELD_VECTOR, "movedir"),
      ...
      //	DEFINE_FIELD( m_ls, FIELD_SOUNDNAME ),   // This is restored in
      // Precache()
      //  DEFINE_FIELD( m_nState, FIELD_INTEGER ),

      // Function Pointers
      DEFINE_FUNCTION(ButtonTouch), DEFINE_FUNCTION(ButtonSpark),
      DEFINE_FUNCTION(TriggerAndWait), DEFINE_FUNCTION(ButtonReturn),
      DEFINE_FUNCTION(ButtonBackHome), DEFINE_FUNCTION(ButtonUse),
      ...
      DEFINE_OUTPUT(m_OnIn, "OnIn"), DEFINE_OUTPUT(m_OnOut, "OnOut"),
...
void CBaseButton::Press(CBaseEntity *pActivator, BUTTON_CODE eCode) {
  if ((eCode == BUTTON_PRESS) &&
      (m_toggle_state == TS_GOING_UP || m_toggle_state == TS_GOING_DOWN)) {
    return;
  }

  if ((eCode == BUTTON_ACTIVATE) &&
      (m_toggle_state == TS_GOING_UP || m_toggle_state == TS_AT_TOP)) {
    return;
  }

  if ((eCode == BUTTON_RETURN) &&
      (m_toggle_state == TS_GOING_DOWN || m_toggle_state == TS_AT_BOTTOM)) {
    return;
  }
  ...
  if ((eCode == BUTTON_PRESS) && (m_toggle_state == TS_AT_TOP) ||
    ...
    ButtonReturn();
  } else if ((eCode == BUTTON_PRESS) || ((eCode == BUTTON_ACTIVATE) &&
...
void CBaseButton::Spawn() {
  ...
  if (HasSpawnFlags(
          SF_BUTTON_SPARK_IF_OFF))  // this button should spark in OFF state
  {
    SetThink(&CBaseButton::ButtonSpark);
    SetNextThink(gpGlobals->curtime +
                 0.5f);  // no hurry, make sure everything else spawns
  }
  ...
  m_vecPosition2 =
      m_vecPosition1 +
      (m_vecMoveDir * (DotProductAbs(m_vecMoveDir, vecButtonOBB) - m_flLip));

  // Is this a non-moving button?
  if (((m_vecPosition2 - m_vecPosition1).Length() < 1) ||
      HasSpawnFlags(SF_BUTTON_DONTMOVE)) {
...
//			Also used by CBaseDoor to get 'touched' door lock/unlock
// sounds
...
void CBaseButton::ButtonSpark(void) {
  ...
  SetNextThink(gpGlobals->curtime + 0.1 +
               random->RandomFloat(0, 1.5));  // spark again at random interval
...
//			value -
//-----------------------------------------------------------------------------
void CBaseButton::ButtonUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                            USE_TYPE useType, float value) {
  // Ignore touches if button is moving, or pushed-in and waiting to
  // auto-come-out. UNDONE: Should this use ButtonResponseToTouch() too?
...
//-----------------------------------------------------------------------------
// Purpose: Returns a code indicating how the button should respond to being
// touched. Output : Returns one of the following:
...
CBaseButton::BUTTON_CODE CBaseButton::ButtonResponseToTouch(void) {
  ...
  if (m_toggle_state == TS_GOING_UP || m_toggle_state == TS_GOING_DOWN ||
      (m_toggle_state == TS_AT_TOP && !m_fStayPushed &&
       !HasSpawnFlags(SF_BUTTON_TOGGLE)))
...
int CBaseButton::ObjectCaps(void) {
  return ((BaseClass::ObjectCaps() & ~FCAP_ACROSS_TRANSITION) |
          (HasSpawnFlags(SF_BUTTON_USE_ACTIVATES)
...
int CBaseButton::DrawDebugTextOverlays() {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    static char *pszStates[] = {
        "Pressed",       "Unpressed",       "Pressing...",
        "Unpressing...", "<UNKNOWN STATE>",
    };
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "%s",
...
BEGIN_DATADESC(CMomentaryRotButton)
  ...
  DEFINE_FIELD(m_lastUsed, FIELD_INTEGER), DEFINE_FIELD(m_start, FIELD_VECTOR),
      DEFINE_FIELD(m_end, FIELD_VECTOR), DEFINE_FIELD(m_IdealYaw, FIELD_FLOAT),
      DEFINE_FIELD(m_sNoise, FIELD_SOUNDNAME),
      ...
      // Function Pointers
      DEFINE_FUNCTION(UseMoveDone), DEFINE_FUNCTION(ReturnMoveDone),
      DEFINE_FUNCTION(SetPositionMoveDone), DEFINE_FUNCTION(UpdateThink),

      // Inputs
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetPosition", InputSetPosition),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetPositionImmediately",
                       InputSetPositionImmediately),
      DEFINE_INPUTFUNC(FIELD_VOID, "_DisableUpdateTarget",
                       InputDisableUpdateTarget),
      DEFINE_INPUTFUNC(FIELD_VOID, "_EnableUpdateTarget",
...
void CMomentaryRotButton::Spawn(void) {
  ...
  m_start =
      GetLocalAngles() - m_vecMoveAng * m_flMoveDistance * m_flStartPosition;
  m_end = GetLocalAngles() +
          m_vecMoveAng * m_flMoveDistance * (1 - m_flStartPosition);
...
void CMomentaryRotButton::PlaySound(void) {
  if (m_sNoise == NULL_STRING) return;
...
//-----------------------------------------------------------------------------
// Purpose: Returns a given angular position as a value along our motion from 0
// to 1. Input  : vecAngles -
//-----------------------------------------------------------------------------
...
void CMomentaryRotButton::InputSetPosition(inputdata_t &inputdata) {
  ...
  //
  // Think again in 0.1 seconds or the time that it will take us to reach our
  // movement goal, whichever is the shorter interval. This prevents us from
  // overshooting and stuttering when we are told to change position in very
  // small increments.
  //
  QAngle vecNewAngles =
      m_start + m_vecMoveAng * (m_IdealYaw * m_flMoveDistance);
  float flAngleDelta =
      fabs(AxisDelta(m_spawnflags, vecNewAngles, GetLocalAngles()));
...
//			value -
//-----------------------------------------------------------------------------
void CMomentaryRotButton::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                              USE_TYPE useType, float value) {
  if (m_bDisabled == true) return;

  if (m_bLocked) {
    if (OnUseLocked(pActivator) &&
        HasSpawnFlags(SF_BUTTON_JIGGLE_ON_USE_LOCKED)) {
      // Jiggle two degrees.
...
//			and updating our avelocity while being used by the
// player.
...
void CMomentaryRotButton::UpdateSelf(float value, bool bPlaySound) {
  //
  // Set our move clock to 0.1 seconds in the future so we stop spinning unless
  // we are used again before then.
  //
  SetMoveDoneTime(0.1);
  ...
  //
  // If we returned to the start, zero our avelocity and snap to the start
  // angles.
  //
...
int CMomentaryRotButton::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "QAngle: %.2f %.2f %.2f",
               GetLocalAngles()[0], GetLocalAngles()[1], GetLocalAngles()[2]);
    EntityText(text_offset, tempstr, 0);
    text_offset++;

    Q_snprintf(tempstr, sizeof(tempstr), "AVelocity: %.2f %.2f %.2f",
               GetLocalAngularVelocity()[0], GetLocalAngularVelocity()[1],
               GetLocalAngularVelocity()[2]);
    EntityText(text_offset, tempstr, 0);
    text_offset++;
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Direction: %s",
               (m_direction == 1) ? "Forward" : "Backward");
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
  return text_offset;
