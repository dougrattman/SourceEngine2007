...
//
// This entity listens as though it is an NPC, meaning it will only hear sounds
// that were emitted using the CSound::InsertSound function.
//
...
void CEnvMicrophone::Spawn(void) {
  ...
  m_flSmoothFactor = std::clamp(m_flSmoothFactor, 0.0f, 0.9f);
...
//			Finds the reference point at which to measure sound
// level.
//-----------------------------------------------------------------------------
void CEnvMicrophone::Activate(void) {
  ...
  if (m_iszListenFilter != NULL_STRING) {
    m_hListenFilter = dynamic_cast<CBaseFilter *>(
        gEntList.FindEntityByName(NULL, m_iszListenFilter));
  }
  ...
  if (m_target != NULL_STRING) {
    ...
    if ((m_hMeasureTarget == NULL) || (m_hMeasureTarget->edict() == NULL)) {
      // We've decided to disable this warning since this seems to be the 90%
      // case.
      // Warning( "EnvMicrophone - Measure target not found or measure target
      // with no origin. Using Self.!\n");
...
//-----------------------------------------------------------------------------
// Purpose: If we've got a speaker, add ourselves to the list of microphones
// that want to listen
//-----------------------------------------------------------------------------
void CEnvMicrophone::ActivateSpeaker(void) {
  ...
  if (m_iszSpeakerName != NULL_STRING) {
    // We've got a speaker to play heard sounds through. To do this, we need to
    // add ourselves to the list of microphones who want to be told whenever a
    // sound is played.
...
//			flVolume - Returns with the relative sound volume from 0
//- 1.
// Output : Returns true if the sound could be heard at the sample point, false
// if not.
//-----------------------------------------------------------------------------
bool CEnvMicrophone::CanHearSound(CSound *pSound, float &flVolume) {
  ...
  float flDistance =
      (pSound->GetSoundOrigin() - m_hMeasureTarget->GetAbsOrigin()).Length();
...
//-----------------------------------------------------------------------------
bool CEnvMicrophone::CanHearSound(int entindex, soundlevel_t soundlevel,
                                  float &flVolume, const Vector *pOrigin) {
  if (m_bDisabled) {
    flVolume = 0;
    return false;
  }

  if ((m_spawnflags & SF_MICROPHONE_IGNORE_NONATTENUATED) &&
  ...
  if (pOrigin) {
    ...
    flDistance =
        pEntity->WorldSpaceCenter().DistTo(m_hMeasureTarget->GetAbsOrigin());
  }
...
void CEnvMicrophone::SetSpeakerName(string_t iszSpeakerName) {
  ...
  // Set the speaker to 0. This will force it to find the speaker next time a
  // sound is routed.
  m_hSpeaker = NULL;
  ActivateSpeaker();
}
...
void CEnvMicrophone::Think(void) {
  ...
  if (flMaxVolume != m_SoundLevel.Get()) {
    //
    // Don't smooth if we are within an epsilon. This allows the output to stop
    ...
    if (fabs(flMaxVolume - m_SoundLevel.Get()) < MICROPHONE_SETTLE_EPSILON) {
      ...
      m_SoundLevel.Set(flMaxVolume * (1 - m_flSmoothFactor) +
                           m_SoundLevel.Get() * m_flSmoothFactor,
                       this, this);
    }
  }
...
//-----------------------------------------------------------------------------
MicrophoneResult_t CEnvMicrophone::SoundPlayed(
    int entindex, const char *soundname, soundlevel_t soundlevel,
    float flVolume, int iFlags, int iPitch, const Vector *pOrigin,
    float soundtime, CUtlVector<Vector> &soundorigins) {
  if (m_bAvoidFeedback) return MicrophoneResult_Ok;

  // Don't hear sounds that have already been heard by a microphone to avoid
  // feedback!
  ...
  // We've heard it. Play it out our speaker. If our speaker's gone away, we're
  // done.
  if (!m_hSpeaker) {
    // First time, find our speaker. Done here, because finding it in Activate()
    // wouldn't find players, and we need to be able to specify !player for a
    // speaker.
    if (m_iszSpeakerName != NULL_STRING) {
...
//			active microphones can have a chance to pick up the
// sound.
// Output : Returns whether or not the sound was swallowed by the microphone.
//			Swallowed sounds should not be played by the sound
// system.
//-----------------------------------------------------------------------------
bool CEnvMicrophone::OnSoundPlayed(int entindex, const char *soundname,
                                   soundlevel_t soundlevel, float flVolume,
                                   int iFlags, int iPitch,
                                   const Vector *pOrigin, float soundtime,
    ...
    for (int i = iCount - 1; i >= 0; i--) {
      if (s_Microphones[i]) {
        MicrophoneResult_t eResult = s_Microphones[i]->SoundPlayed(
            entindex, soundname, soundlevel, flVolume, iFlags, iPitch, pOrigin,
            soundtime, soundorigins);
