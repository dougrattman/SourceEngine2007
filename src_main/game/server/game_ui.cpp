...
BEGIN_DATADESC(CGameUI)
  ...
  DEFINE_KEYFIELD(m_flFieldOfView, FIELD_FLOAT, "FieldOfView"),
      ...
      DEFINE_OUTPUT(m_xaxis, "XAxis"), DEFINE_OUTPUT(m_yaxis, "YAxis"),
...
void CGameUI::Deactivate(CBaseEntity *pActivator) {
  ...
  if (pPlayer) {
    ...
    if (FBitSet(m_spawnflags.operator const int &(), SF_GAMEUI_FREEZE_PLAYER)) {
      m_player->RemoveFlag(FL_ATCONTROLS);
    }

    // Restore weapons
    if (FBitSet(m_spawnflags.operator const int &(), SF_GAMEUI_HIDE_WEAPON)) {
...
void CGameUI::InputActivate(inputdata_t &inputdata) {
  ...
  if (inputdata.value.StringID() != NULL_STRING) {
    ...
    if (inputdata.pActivator == NULL ||
        inputdata.pActivator->IsPlayer() == false) {
  ...
  if (m_player.Get() != NULL && pPlayer != m_player.Get()) {
    // TODO: We could allow this by calling Deactivate() at this point and
    // continuing on -- jdw
    return;
  }
...
void CGameUI::Think(void) {
  ...
  if (((pPlayer->m_afButtonPressed & IN_USE) &&
      ...
      ((pPlayer->m_afButtonPressed & IN_JUMP) &&
       (m_spawnflags & SF_GAMEUI_JUMP_DEACTIVATES))) {
    Deactivate(pPlayer);
    return;
  }
