...
BEGIN_DATADESC(CLogicCompareInteger)
  ...
  DEFINE_OUTPUT(m_OnEqual, "OnEqual"),
      ...
      DEFINE_KEYFIELD(m_iShouldCompareToValue, FIELD_INTEGER,
...
void CLogicCompareInteger::InputValue(inputdata_t &inputdata) {
  ...
  if (!inputdata.value.Convert(FIELD_INTEGER)) return;

  // update the value list with the new value
  m_AllIntCompares.AddValue(inputdata.value, inputdata.nOutputID);

  // if we haven't already this frame, send a message to ourself to update and
  // fire
  if (!m_AllIntCompares.m_bUpdatedThisFrame) {
    // TODO: need to add this event with a lower priority, so it gets called
    // after all inputs have arrived
    g_EventQueue.AddEvent(this, "CompareValues", 0, inputdata.pActivator, this,
                          inputdata.nOutputID);
...
BEGIN_DATADESC(CTimerEntity)
  ...
  DEFINE_KEYFIELD(m_iDisabled, FIELD_INTEGER, "StartDisabled"),
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SubtractFromTimer",
...
// Purpose:
//-----------------------------------------------------------------------------
void CTimerEntity::Think(void) { FireTimer(); }

//-----------------------------------------------------------------------------
// Purpose: Sets the time the timerentity will next fire
//-----------------------------------------------------------------------------
void CTimerEntity::ResetTimer(void) {
  if (m_iDisabled) return;

  if (m_iUseRandomTime) {
    m_flRefireTime =
        random->RandomFloat(m_flLowerRandomBound, m_flUpperRandomBound);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CTimerEntity::InputEnable(inputdata_t &inputdata) { Enable(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CTimerEntity::InputDisable(inputdata_t &inputdata) { Disable(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CTimerEntity::InputToggle(inputdata_t &inputdata) { Toggle(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CTimerEntity::InputFireTimer(inputdata_t &inputdata) { FireTimer(); }
...
int CTimerEntity::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // print refire time
    Q_snprintf(tempstr, sizeof(tempstr), "refire interval: %.2f sec",
               m_flRefireTime);
    ...
    if (!m_iDisabled) {
      float flNextThink = GetNextThink();
      Q_snprintf(tempstr, sizeof(tempstr), "      firing in: %.2f sec",
                 flNextThink - gpGlobals->curtime);
      EntityText(text_offset, tempstr, 0);
      text_offset++;
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CMathRemap::InputEnable(inputdata_t &inputdata) { m_bEnabled = true; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CMathRemap::InputDisable(inputdata_t &inputdata) { m_bEnabled = false; }
...
void CMathRemap::InputValue(inputdata_t &inputdata) {
  ...
  if ((flClampValue == flValue) ||
    ...
    //
    float flRemappedValue =
        m_flOut1 + (((flValue - m_flInMin) * (m_flOut2 - m_flOut1)) /
                    (m_flInMax - m_flInMin));
...
void CMathColorBlend::InputValue(inputdata_t &inputdata) {
  ...
  if ((flClampValue == flValue) ||
    ...
    //
    color32 Color;
    Color.r = m_OutColor1.r +
              (((flClampValue - m_flInMin) * (m_OutColor2.r - m_OutColor1.r)) /
               (m_flInMax - m_flInMin));
    Color.g = m_OutColor1.g +
              (((flClampValue - m_flInMin) * (m_OutColor2.g - m_OutColor1.g)) /
               (m_flInMax - m_flInMin));
    Color.b = m_OutColor1.b +
              (((flClampValue - m_flInMin) * (m_OutColor2.b - m_OutColor1.b)) /
               (m_flInMax - m_flInMin));
    Color.a = m_OutColor1.a +
              (((flClampValue - m_flInMin) * (m_OutColor2.a - m_OutColor1.a)) /
               (m_flInMax - m_flInMin));
...
void CC_Global_Set(const CCommand &args) {
  ...
  if (szGlobal == NULL || szState == NULL) {
    Msg("Usage: global_set <globalname> <state>: Sets the state of the given "
        "env_global (0 = OFF, 1 = ON, 2 = DEAD).\n");
    return;
  }
  ...
  if (nIndex >= 0) {
    ...
    GlobalEntity_Add(szGlobal, STRING(gpGlobals->mapname),
                     (GLOBALESTATE)nState);
  }
}

static ConCommand global_set(
    "global_set", CC_Global_Set,
    "global_set <globalname> <state>: Sets the state of the given env_global "
    "(0 = OFF, 1 = ON, 2 = DEAD).",
    FCVAR_CHEAT);
...
#ifdef HL2_EPISODIC
  // if we modify the state of the physics cannon, make sure we precache the
  // ragdoll boogie zap sound
  if ((m_globalstate != NULL_STRING) &&
      (stricmp(STRING(m_globalstate), "super_phys_gun") == 0)) {
...
void CEnvGlobal::InputGetCounter(inputdata_t &inputdata) {
  ...
  m_outCounter.Set(GlobalEntity_GetCounter(m_globalstate), inputdata.pActivator,
                   this);
}
...
bool CMultiSource::KeyValue(const char *szKeyName, const char *szValue) {
  if (FStrEq(szKeyName, "style") || FStrEq(szKeyName, "height") ||
      FStrEq(szKeyName, "killtarget") || FStrEq(szKeyName, "value1") ||
      FStrEq(szKeyName, "value2") || FStrEq(szKeyName, "value3")) {
  } else {
...
//			value -
//-----------------------------------------------------------------------------
void CMultiSource::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                       USE_TYPE useType, float value) {
  int i = 0;

  // Find the entity in our list
  while (i < m_iTotal)
    if (m_rgEntities[i++] == pCaller) break;
  ...
  // CONSIDER: a Use input to the multisource always toggles.  Could check
  // useType for ON/OFF/TOGGLE
  ...
  if (IsTriggered(pActivator)) {
    ...
    if (m_globalstate != NULL_STRING) useType = USE_ON;
...
bool CMultiSource::IsTriggered(CBaseEntity *) {
  ...
  // Still initializing?
  if (m_spawnflags & SF_MULTI_INIT) return 0;
...
void CMultiSource::Register(void) {
  ...
  while (pTarget && (m_iTotal < MS_MAX_TARGETS)) {
    if (pTarget) m_rgEntities[m_iTotal++] = pTarget;
...
class CMathCounter : public CLogicalEntity {
 ...
 private:
  float m_flMin;   // Minimum clamp value. If min and max are BOTH zero, no
                   // clamping is done.
  float m_flMax;   // Maximum clamp value.
  bool m_bHitMin;  // Set when we reach or go below our minimum value, cleared
                   // if we go above it again.
  bool m_bHitMax;  // Set when we reach or exceed our maximum value, cleared if
                   // we fall below it again.
...
void CMathCounter::Spawn(void) {
  ...
  if ((m_flMin != 0) || (m_flMax != 0)) {
    float flStartValue = std::clamp(m_OutValue.Get(), m_flMin, m_flMax);
...
void CMathCounter::InputSetValueNoFire(inputdata_t &inputdata) {
  ...
  if ((m_flMin != 0) || (m_flMax != 0)) {
    flNewValue = std::clamp(flNewValue, m_flMin, m_flMax);
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMathCounter::InputEnable(inputdata_t &inputdata) { m_bDisabled = false; }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CMathCounter::InputDisable(inputdata_t &inputdata) { m_bDisabled = true; }

//-----------------------------------------------------------------------------
// Purpose: Sets the value to the new value, clamping and firing the output
// value. Input  : fNewValue - Value to set.
//-----------------------------------------------------------------------------
void CMathCounter::UpdateOutValue(CBaseEntity *pActivator, float fNewValue) {
  if ((m_flMin != 0) || (m_flMax != 0)) {
    ...
    fNewValue = std::clamp(fNewValue, m_flMin, m_flMax);
  }
...
class CLogicCase : public CLogicalEntity {
 ...
 private:
  ...
  // Outputs
  COutputEvent m_OnCase[MAX_LOGIC_CASES];  // Fired when the input value matches
...
// Purpose: Called before spawning, after key values have been set.
//-----------------------------------------------------------------------------
void CLogicCase::Spawn(void) { m_nLastShuffleCase = -1; }
...
void CLogicCase::InputPickRandomShuffle(inputdata_t &inputdata) {
  ...
  //
  // Choose a random case from the ones that were set up by the level designer.
  // Never repeat a case within a shuffle batch, nor consecutively across
  // batches.
  //
  if (nCaseCount > 0) {
...
class CLogicCompare : public CLogicalEntity {
 ...
 private:
  ...
  // Outputs
  COutputFloat m_OnLessThan;  // Fired when the input value is less than the
                              // compare value.
  COutputFloat
      m_OnEqualTo;  // Fired when the input value is equal to the compare value.
  COutputFloat m_OnNotEqualTo;   // Fired when the input value is not equal to
                                 // the compare value.
  COutputFloat m_OnGreaterThan;  // Fired when the input value is greater than
                                 // the compare value.
...
//-----------------------------------------------------------------------------
// Purpose: Compares the input value to the compare value, firing the
// appropriate
...
int CLogicCompare::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // print hold time
    Q_snprintf(tempstr, sizeof(tempstr), "    Compare Value: %f",
               m_flCompareValue);
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
...
class CLogicBranch : public CLogicalEntity {
 ...
 private:
  ...
  void UpdateValue(bool bNewValue, CBaseEntity *pActivator,
                   LogicBranchFire_t eFire);

  bool m_bInValue;  // Place to hold the last input value for a future test.

  CUtlVector<EHANDLE>
      m_Listeners;  // A list of logic_branch_listeners that are monitoring us.
...
BEGIN_DATADESC(CLogicBranch)
  ...
  DEFINE_KEYFIELD(m_bInValue, FIELD_BOOLEAN, "InitialValue"),
      ...
      // Outputs
      DEFINE_OUTPUT(m_OnTrue, "OnTrue"), DEFINE_OUTPUT(m_OnFalse, "OnFalse"),
...
void CLogicBranch::InputSetValue(inputdata_t &inputdata) {
  UpdateValue(inputdata.value.Bool(), inputdata.pActivator,
              LOGIC_BRANCH_NO_FIRE);
}
...
// Input  : bInValue -
//-----------------------------------------------------------------------------
void CLogicBranch::UpdateValue(bool bNewValue, CBaseEntity *pActivator,
                               LogicBranchFire_t eFire) {
...
//-----------------------------------------------------------------------------
// Purpose: Accessor for logic_branchlist to test the value of the branch on
// demand.
//-----------------------------------------------------------------------------
bool CLogicBranch::GetLogicBranchState() { return m_bInValue; }
...
int CLogicBranch::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // print refire time
    Q_snprintf(tempstr, sizeof(tempstr), "Branch value: %s",
               (m_bInValue) ? "TRUE" : "FALSE");
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
...
BEGIN_DATADESC(CLogicAutosave)
  DEFINE_KEYFIELD(m_bForceNewLevelUnit, FIELD_BOOLEAN, "NewLevelUnit"),
      ...
      DEFINE_KEYFIELD(m_minHitPointsToCommit, FIELD_INTEGER,
      ...
      DEFINE_INPUTFUNC(FIELD_INTEGER, "SetMinHitpointsThreshold",
...
void CLogicAutosave::InputSaveDangerous(inputdata_t &inputdata) {
  ...
  if (pPlayer->GetHealth() >= m_minHitPoints) {
    ...
    g_ServerGameDLL.m_fAutoSaveDangerousTime =
        gpGlobals->curtime + inputdata.value.Float();

    // Player must have this much health when we go to commit, or we don't
    // commit.
    g_ServerGameDLL.m_fAutoSaveDangerousMinHealthToCommit =
...
class CLogicActiveAutosave : public CLogicAutosave {
  ...
  void InputDisable(inputdata_t &inputdata) { SetThink(NULL); }
    ...
    if (pPlayer) {
      if (m_flStartTime < 0) {
        ...
        if (pPlayer->GetHealth() >= m_TriggerHitPoints) {
          ...
          m_flStartTime = -1;
        } else if (m_flTimeToTrigger > 0 &&
                   gpGlobals->curtime - m_flStartTime > m_flTimeToTrigger) {
          m_flStartTime = -1;
        }
      }
    }
...
BEGIN_DATADESC(CLogicActiveAutosave)
  DEFINE_KEYFIELD(m_TriggerHitPoints, FIELD_INTEGER, "TriggerHitPoints"),
      ...
      DEFINE_FIELD(m_flStartTime, FIELD_TIME), DEFINE_THINKFUNC(SaveThink),
      DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
      DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),
END_DATADESC()
...
void CLogicAutosave::InputSetMinHitpointsThreshold(inputdata_t &inputdata) {
  ...
  AssertMsg1(setTo >= 0 && setTo <= 100,
             "Tried to set autosave MinHitpointsThreshold to %d!\n", setTo);
...
IPhysicsObject *FindPhysicsObjectByNameOrWorld(string_t name,
                                               CBaseEntity *pErrorEntity) {
  if (!name) return g_PhysWorldObject;

  IPhysicsObject *pPhysics =
      FindPhysicsObjectByName(name.ToCStr(), pErrorEntity);
  if (!pPhysics) {
    DevWarning("%s: can't find %s\n", pErrorEntity->GetClassname(),
               name.ToCStr());
  }
  return pPhysics;
}
...
class CLogicCollisionPair : public CLogicalEntity {
 ...
 public:
  void EnableCollisions(bool bEnable) {
    IPhysicsObject *pPhysics0 =
        FindPhysicsObjectByNameOrWorld(m_nameAttach1, this);
    IPhysicsObject *pPhysics1 =
        FindPhysicsObjectByNameOrWorld(m_nameAttach2, this);
  ...
  void InputDisableCollisions(inputdata_t &inputdata) {
    if (m_succeeded && m_disabled) return;
    EnableCollisions(false);
  }
...
class CLogicBranchList : public CLogicalEntity {
 ...
 private:
  ...
  // Outputs
  COutputEvent
      m_OnAllTrue;  // Fired when all the registered logic_branches become true.
  COutputEvent m_OnAllFalse;  // Fired when all the registered logic_branches
                              // become false.
  COutputEvent m_OnMixed;     // Fired when one of the registered logic branches
                              // changes, but not all are true or false.
...
BEGIN_DATADESC(CLogicBranchList)
  ...
  DEFINE_KEYFIELD(m_nLogicBranchNames[0], FIELD_STRING, "Branch01"),
      ...
      DEFINE_INPUTFUNC(FIELD_INPUT, "_OnLogicBranchChanged",
                       Input_OnLogicBranchChanged),
      DEFINE_INPUTFUNC(FIELD_INPUT, "_OnLogicBranchRemoved",
...
//-----------------------------------------------------------------------------
// Finds all the logic_branches that we are monitoring and register ourselves
// with them.
//-----------------------------------------------------------------------------
void CLogicBranchList::Activate(void) {
  for (int i = 0; i < MAX_LOGIC_BRANCH_NAMES; i++) {
    CBaseEntity *pEntity = NULL;
    while ((pEntity = gEntList.FindEntityGeneric(
                pEntity, STRING(m_nLogicBranchNames[i]), this)) != NULL) {
      if (FClassnameIs(pEntity, "logic_branch")) {
        ...
        DevWarning(
            "logic_branchlist %s refers to entity %s, which is not a "
            "logic_branch\n",
...
int CLogicBranchList::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    for (int i = 0; i < m_LogicBranchList.Count(); i++) {
      CLogicBranch *pBranch =
          (CLogicBranch *)m_LogicBranchList.Element(i).Get();
      if (pBranch) {
        Q_snprintf(tempstr, sizeof(tempstr), "Branch (%s): %s",
                   pBranch->GetEntityName(),
                   (pBranch->GetLogicBranchState()) ? "TRUE" : "FALSE");
        EntityText(text_offset, tempstr, 0);
        text_offset++;
      }
    }
