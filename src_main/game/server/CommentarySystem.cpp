...
//
// Purpose: The system for handling director's commentary style production info
// in-game.
...
ConVar commentary_available("commentary_available", "0", FCVAR_NONE,
                            "Automatically set by the game when a commentary "
...
bool IsInCommentaryMode(void) { return g_bInCommentaryMode; }
...
class CPointCommentaryNode : public CBaseAnimating {
 ...
 public:
  ...
  // Called to tell the node when it's moved under/not-under the player's
  // crosshair
 ...
 private:
  ...
  EHANDLE m_hViewTargetAngles;  // Entity used to blend view angles to look at
                                // the target
  string_t m_iszViewPosition;
  CNetworkVar(EHANDLE, m_hViewPosition);
  EHANDLE m_hViewPositionMover;  // Entity used to blend the view to the
                                 // viewposition entity
...
BEGIN_DATADESC(CPointCommentaryNode)
  DEFINE_KEYFIELD(m_iszPreCommands, FIELD_STRING, "precommands"),
      ...
      DEFINE_KEYFIELD(m_iszCommentaryFileNoHDR, FIELD_STRING,
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StartUnstoppableCommentary",
                       InputStartUnstoppableCommentary),
      DEFINE_INPUTFUNC(FIELD_VOID, "Enable", InputEnable),
      DEFINE_INPUTFUNC(FIELD_VOID, "Disable", InputDisable),

      // Functions
      DEFINE_THINKFUNC(SpinThink), DEFINE_THINKFUNC(UpdateViewThink),
...
class CCommentaryViewPosition : public CSprite {
 ...
 public:
  ...
  virtual void Precache(void) { PrecacheModel("sprites/redglow1.vmt"); }
};
...
//===========================================================================================================
void CV_GlobalChange_Commentary(IConVar *var, const char *pOldString,
                                float flOldValue);
...
class CCommentarySystem : public CAutoGameSystemPerFrame {
 public:
  ...
  void CalculateCommentaryState(void) {
    ...
    char szFullName[512];
    Q_snprintf(szFullName, sizeof(szFullName), "maps/%s_commentary.txt",
  ...
  virtual void LevelShutdownPreEntity() { ShutDownCommentary(); }
...
#define COMMENTARY_STRING_LENGTH_MAX 1024
        ...
        if (Q_strnchr(pszValue, '^', COMMENTARY_STRING_LENGTH_MAX)) {
          // We want to support quotes in our strings so that we can specify
          // multiple parameters in an output inside our commentary files. We
          // convert ^s to "s here.
  ...
  CPointCommentaryNode *GetNodeUnderCrosshair() {
    ...
    UTIL_TraceLine(vecSrc, vecSrc + vecForward * MAX_TRACE_LENGTH, MASK_SOLID,
  ...
  void PrePlayerRunCommand(CBasePlayer *pPlayer, CUserCmd *pUserCmds) {
    ...
    if (pPlayer) {
      ...
      if (GetActiveNode() && GetActiveNode()->PreventsMovement()) {
        pUserCmds->buttons &= ~(IN_FORWARD | IN_BACK | IN_MOVELEFT |
                                IN_MOVERIGHT | IN_JUMP | IN_DUCK);
        pUserCmds->upmove = 0;
        pUserCmds->sidemove = 0;
        pUserCmds->forwardmove = 0;
      }

      // When we swallow button down events, we have to keep clearing that
      // button until the player releases the button. Otherwise, the frame after
      // we swallow it, the code detects the button down and goes ahead as
      // normal.
    ...
    if (m_iTeleportStage != TELEPORT_NONE) {
      if (m_flNextTeleportTime <= gpGlobals->curtime) {
        if (m_iTeleportStage == TELEPORT_FADEOUT) {
          ...
          UTIL_ScreenFade(pPlayer, clr, 0.3, 0,
  ...
  CPointCommentaryNode *GetActiveNode(void) { return m_hActiveCommentaryNode; }
  ...
  int GetCommentaryNodeCount(void) { return m_iCommentaryNodeCount; }

  bool CommentaryConvarsChanging(void) { return m_bCommentaryConvarsChanging; }

  void SetCommentaryConvarsChanging(bool bChanging) {
    m_bCommentaryConvarsChanging = bChanging;
  }

  void ConvarChanged(IConVar *pConVar, const char *pOldString,
                     float flOldValue) {
    ...
    for (int i = 0; i < m_ModifiedConvars.Count(); i++) {
      ...
      if (!Q_strncmp(var.GetName(), m_ModifiedConvars[i].pszConvar,
                     MAX_MODIFIED_CONVAR_STRING)) {
        Q_strncpy(m_ModifiedConvars[i].pszCurrentValue, var.GetString(),
                  MAX_MODIFIED_CONVAR_STRING);
        // Msg("    Updating Convar %s: value %s (org %s)\n",
        // m_ModifiedConvars[i].pszConvar, m_ModifiedConvars[i].pszCurrentValue,
        // m_ModifiedConvars[i].pszOrgValue );
        return;
      }
    }
    ...
    Q_strncpy(newConvar.pszCurrentValue, var.GetString(),
              MAX_MODIFIED_CONVAR_STRING);
    ...
    var->GetString(), pOldString ); Msg(" Convars stored: %d\n",
    m_ModifiedConvars.Count() ); for ( int i = 0; i < m_ModifiedConvars.Count();
    i++ )
    {
  ...
  void InitCommentary(void) {
    ...
    // If we find the commentary semaphore, the commentary entities already
    // exist. This occurs when you transition back to a map that has saved
    // commentary nodes in it.
    if (gEntList.FindEntityByName(NULL, COMMENTARY_SPAWNED_SEMAPHORE)) return;
    ...
    if (pkvFile->LoadFromFile(filesystem, szFullName, "MOD")) {
      ...
      while (pkvNode) {
        ...
        if (pNode) {
          ...
          CPointCommentaryNode *pCommNode =
              dynamic_cast<CPointCommentaryNode *>(pNode);
          if (pCommNode) {
  ...
  void ShutDownCommentary(void) {
    ...
    CBaseEntity *pSemaphore =
        gEntList.FindEntityByName(NULL, COMMENTARY_SPAWNED_SEMAPHORE);
    if (pSemaphore) {
  ...
  void OnRestore(void) {
    ...
    // Set any convars that have already been changed by the commentary before
    // the save
    for (int i = 0; i < m_ModifiedConvars.Count(); i++) {
      ConVar *pConVar = (ConVar *)cvar->FindVar(m_ModifiedConvars[i].pszConvar);
      if (pConVar) {
  ...
  void JumpToNextNode(CBasePlayer *pPlayer) {
    ...
    while ((pEnt = gEntList.FindEntityByClassname(
                pEnt, "point_commentary_node")) != m_hLastCommentaryNode) {
...
BEGIN_SIMPLE_DATADESC(modifiedconvars_t)
  DEFINE_ARRAY(pszConvar, FIELD_CHARACTER, MAX_MODIFIED_CONVAR_STRING),
      DEFINE_ARRAY(pszCurrentValue, FIELD_CHARACTER,
                   MAX_MODIFIED_CONVAR_STRING),
...
// Purpose:
//-----------------------------------------------------------------------------
void CC_CommentaryChanged(IConVar *pConVar, const char *pOldString,
                          float flOldValue) {
  ConVarRef var(pConVar);
...
ConVar commentary("commentary", "0", FCVAR_ARCHIVE | FCVAR_ARCHIVE_XBOX,
                  "Desired commentary mode state.", CC_CommentaryChanged);

//-----------------------------------------------------------------------------
// Purpose: We need to revert back any convar changes that are made by the
//			commentary system during commentary. This code stores
//convar
// changes 			made by the commentary system, and reverts them when
// finished.
//-----------------------------------------------------------------------------
void CV_GlobalChange_Commentary(IConVar *var, const char *pOldString,
                                float flOldValue) {
...
static ConCommand commentary_cvarsnotchanging("commentary_cvarsnotchanging",
                                              CC_CommentaryNotChanging, 0);
  ...
  if (m_iszViewTarget != NULL_STRING) {
    ...
    if (!m_hViewTarget) {
      Warning("%s: %s could not find viewtarget %s.\n", GetClassname(),
              GetDebugName(), STRING(m_iszViewTarget));
    }
  }
  ...
  if (m_iszViewPosition != NULL_STRING) {
    ...
    if (!m_hViewPosition.Get()) {
      Warning("%s: %s could not find viewposition %s.\n", GetClassname(),
              GetDebugName(), STRING(m_iszViewPosition));
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Called to tell the node when it's moved under/not-under the player's
// crosshair
//-----------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// Purpose: Prevent collisions of everything except the trace from the
// commentary system
//------------------------------------------------------------------------------
bool CPointCommentaryNode::TestCollision(const Ray_t &ray, unsigned int mask,
                                         trace_t &trace) {
  if (!g_bTracingVsCommentaryNodes) return false;
...
//			Used when players shut down commentary while we're in
//the
// middle 			of playing a node, so we can't smoothly blend out
// (since the 			commentary entities need to be removed).
//-----------------------------------------------------------------------------
void CPointCommentaryNode::AbortPlaying(void) {
  if (m_bActive) {
    FinishCommentary(false);
  } else if (m_bPreventChangesWhileMoving) {
    // We're a node that's not active, but is in the process of transitioning
    // the view. Finish movement.
...
void CPointCommentaryNode::TeleportTo(CBasePlayer *pPlayer) {
  ...
  trace_t trace;
  UTIL_TraceHull(vecTarget, vecTarget + Vector(0, 0, -500),
                 pPlayer->WorldAlignMins(), pPlayer->WorldAlignMaxs(),
...
void CPointCommentaryNode::StartCommentary(void) {
  ...
  if (m_hViewTarget || m_hViewPosition.Get()) {
    ...
    SetContextThink(&CPointCommentaryNode::UpdateViewThink, gpGlobals->curtime,
                    s_pCommentaryUpdateViewThink);
  }

  // SetContextThink( &CPointCommentaryNode::FinishCommentary,
  // gpGlobals->curtime + flDuration, s_pFinishCommentaryThink );
}
...
static ConCommand commentary_finishnode("commentary_finishnode",
                                        CC_CommentaryFinishNode, 0);
...
void CPointCommentaryNode::UpdateViewThink(void) {
  ...
  if (m_hViewTarget) {
    ...
    if (m_hViewPositionMover) {
      ...
      VectorAngles(m_hViewTarget->WorldSpaceCenter() -
                       m_hViewPositionMover->GetAbsOrigin(),
                   angGoal);
    } else if (m_hViewTargetAngles) {
      angCurrent = m_hViewTargetAngles->GetAbsAngles();
      m_hViewTargetAngles->SetAbsOrigin(pPlayer->EyePosition());
      VectorAngles(m_hViewTarget->WorldSpaceCenter() -
                       m_hViewTargetAngles->GetAbsOrigin(),
                   angGoal);
    } else {
      angCurrent = pPlayer->EyeAngles();
      VectorAngles(m_hViewTarget->WorldSpaceCenter() - pPlayer->EyePosition(),
                   angGoal);
    }
  ...
  if (m_hViewPosition.Get()) {
    ...
    float flBlendPerc = std::clamp(flCurTime / 2.0f, 0.0f, 1.0f);

    // Figure out the current view position
    Vector vecCurEye;
    VectorLerp(pPlayer->EyePosition(), m_hViewPosition.Get()->GetAbsOrigin(),
               flBlendPerc, vecCurEye);
...
void CPointCommentaryNode::UpdateViewPostThink(void) {
  ...
  if (m_hViewPosition.Get() && m_hViewPositionMover) {
    ...
    float flTimeToBlend = std::min(2.0f, m_flFinishedTime - m_flStartTime);
    float flBlendPerc = 1.0 - std::clamp(flCurTime / flTimeToBlend, 0.0f, 1.0f);

    // Msg("OUT: CurTime %.2f, BlendTime: %.2f, Blend: %.3f\n", flCurTime,
    // flTimeToBlend, flBlendPerc );
    ...
    if (flBlendPerc > 0) {
      ...
      if (m_hViewTarget) {
        ...
        QuaternionSlerp(quatFinish, quatOriginal, 1.0 - flBlendPerc,
                        quatCurrent);
        QAngle angCurrent;
...
void CPointCommentaryNode::FinishCommentary(bool bBlendOut) {
  ...
  if (bBlendOut && m_hViewPositionMover) {
    ...
    // We've moved away from the player's position. Move back to it before
    // ending
    SetContextThink(&CPointCommentaryNode::UpdateViewPostThink,
                    gpGlobals->curtime, s_pCommentaryUpdateViewThink);
    return;
  }
...
void CPointCommentaryNode::CleanupPostCommentary(void) {
  ...
  if (m_hViewPositionMover &&
      pPlayer->GetViewEntity() == m_hViewPositionMover) {
    pPlayer->SetViewEntity(NULL);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPointCommentaryNode::InputStartUnstoppableCommentary(
...
// Purpose:
//-----------------------------------------------------------------------------
void CPointCommentaryNode::SetTransmit(CCheckTransmitInfo *pInfo,
                                       bool bAlways) {
...
bool CPointCommentaryNode::PreventsMovement(void) {
  ...
  if (m_hViewPosition.Get()) return true;
...
class CCommentary_SaveRestoreBlockHandler : public CDefSaveRestoreBlockHandler {
 public:
  const char *GetBlockName() { return "Commentary"; }
  ...
  void Restore(IRestore *pRestore, bool createPlayers) {
    if (m_fDoLoad) {
      ...
      if (g_bInCommentaryMode) {
        pRestore->ReadAll(&g_CommentarySystem,
                          g_CommentarySystem.GetDataDescMap());
...
//			Fires outputs based upon how commentary mode has been
// activated.
//-----------------------------------------------------------------------------
...
BEGIN_DATADESC(CCommentaryAuto)
  ...
  DEFINE_INPUTFUNC(FIELD_VOID, "MultiplayerSpawned", InputMultiplayerSpawned),
      ...
      DEFINE_OUTPUT(m_OnCommentaryMultiplayerSpawn,
...
bool IsInCommentaryMode(void) { return false; }
