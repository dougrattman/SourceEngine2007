...
static impactentry_t playerLinearTable[] = {
    {150 * 150, 5},  {250 * 250, 10},  {450 * 450, 20},
    {550 * 550, 50}, {700 * 700, 100}, {1000 * 1000, 500},
};
...
static impactentry_t playerVehicleLinearTable[] = {
    {450 * 450, 5},    {600 * 600, 10},    {700 * 700, 25},
    {1000 * 1000, 50}, {1500 * 1500, 100}, {2000 * 2000, 500},
};
...
static impactentry_t glassLinearTable[] = {
    {25 * 25, 10},   {50 * 50, 20},    {100 * 100, 50},
    {200 * 200, 75}, {500 * 500, 100}, {250 * 250, 500},
};
...
impactdamagetable_t gGlassImpactDamageTable = {
    ...
    50,  // large mass in kg
    4,   // large mass scale (anything over 50kg does 4X as much energy to read
         // from damage table)
    0.0f,  // min vel
};
...
float ReadDamageTable(impactentry_t *pTable, int tableCount, float impulse,
  ...
  if (pTable) {
    ...
    for (i = 0; i < tableCount; i++) {
      if (impulse < pTable[i].impulse) break;
    }
    if (i > 0) {
      i--;
      if (bDebug) {
...
// Purpose:
//-----------------------------------------------------------------------------
float CalculatePhysicsImpactDamage(int index, gamevcollisionevent_t *pEvent,
                                   const impactdamagetable_t &table,
                                   float energyScale, bool allowStaticDamage,
                                   int &damageType,
                                   bool bDamageFromHeldObjects) {
  damageType = DMG_CRUSH;
  int otherIndex = !index;

  // UNDONE: Expose a flag for self-inflicted damage?  Can't think of a valid
  // case so far.
  if (pEvent->pEntities[0] == pEvent->pEntities[1]) return 0;

  if (pEvent->pObjects[otherIndex]->GetGameFlags() &
      FVPHYSICS_NO_NPC_IMPACT_DMG) {
    if (pEvent->pEntities[index]->IsNPC() ||
        pEvent->pEntities[index]->IsPlayer()) {
      return 0;
    }
  }

  // use implicit velocities on ragdolls since they may have high constraint
  // velocities that aren't actually executed, just pushed through contacts
  if ((pEvent->pObjects[otherIndex]->GetGameFlags() &
       FVPHYSICS_PART_OF_RAGDOLL) &&
      pEvent->pEntities[index]->IsPlayer()) {
    pEvent->pObjects[otherIndex]->GetImplicitVelocity(
        &pEvent->preVelocity[otherIndex],
        &pEvent->preAngularVelocity[otherIndex]);
  }
  ...
  if (energyScale <= 0.0f) return 0;

  const int gameFlagsNoDamage =
      FVPHYSICS_CONSTRAINT_STATIC | FVPHYSICS_NO_IMPACT_DMG;

  // NOTE: Crushing damage is handled by stress calcs in vphysics update
  // functions, this is ONLY impact damage this is a non-moving object due to a
  // constraint - no damage
  if (pEvent->pObjects[otherIndex]->GetGameFlags() & gameFlagsNoDamage)
    return 0;

  // If it doesn't take damage from held objects and the object is being held -
  // no damage
  if (!bDamageFromHeldObjects &&
      (pEvent->pObjects[otherIndex]->GetGameFlags() & FVPHYSICS_PLAYER_HELD)) {
    // If it doesn't take damage from held objects - no damage
    if (!bDamageFromHeldObjects) return 0;
  }
  ...
  if (pEvent->pObjects[index]->GetGameFlags() & FVPHYSICS_PLAYER_HELD) {
    ...
    if (pEvent->pEntities[otherIndex]->IsPlayer()) return 0;
  ...
  if (pEvent->pEntities[otherIndex] &&
      (pEvent->pObjects[otherIndex]->GetGameFlags() &
       FVPHYSICS_MULTIOBJECT_ENTITY)) {
  ...
  if (!allowStaticDamage) {
    if (otherMass < table.minMass) return 0;
    // check to see if the object is small
    if (otherMass < table.smallMassMax &&
        otherSpeedSqr < table.smallMassMinSpeedSqr)
      return 0;
  ...
  if (pEvent->pEntities[index]->IsFloating() &&
      !pEvent->pEntities[otherIndex]->IsWorld()) {
    if (energyScale < 3.0f) {
      energyScale = 3.0f;
    }
  }
  ...
  if (otherAngSqr > table.minRotSpeedSqr) {
    ...
    float angularMom =
        DotProductAbs(otherInertia, pEvent->preAngularVelocity[otherIndex]);
    damage = ReadDamageTable(table.angularTable, table.angularCount,
  ...
  float deltaV = pEvent->preVelocity[index].Length() -
                 pEvent->postVelocity[index].Length();
  float mass = pEvent->pObjects[index]->GetMass();

  // If I lost speed, and I lost less than min velocity, then filter out this
  // energy
  ...
  deltaV = pEvent->preVelocity[otherIndex].Length() -
           pEvent->postVelocity[otherIndex].Length();
  ...
  if (otherMass >= table.largeMassMin) {
    ...
    float dz =
        pEvent->preVelocity[otherIndex].z - pEvent->postVelocity[otherIndex].z;

    if (deltaV > 0 && dz < 0 && pEvent->preVelocity[otherIndex].z < 0) {
      float factor = fabs(dz / deltaV);
      otherEliminatedEnergy *=
          (1 + factor * (table.largeMassFallingScale - 1.0f));
    }
  }
  ...
  damage += ReadDamageTable(table.linearTable, table.linearCount,
                            eliminatedEnergy, bDebug);
...
// Purpose:
//-----------------------------------------------------------------------------
float CalculateDefaultPhysicsDamage(int index, gamevcollisionevent_t *pEvent,
                                    float energyScale, bool allowStaticDamage,
                                    int &damageType,
    ...
    for (int i = 0; i < ARRAYSIZE(gDamageTableRegistry); i++) {
      if (!Q_strcmp(gDamageTableRegistry[i].pszTableName,
        ...
        return CalculatePhysicsImpactDamage(
            index, pEvent, *(gDamageTableRegistry[i].pTable), energyScale,
            allowStaticDamage, damageType, bDamageFromHeldObjects);
    }
  ...
  return CalculatePhysicsImpactDamage(
      index, pEvent, gDefaultNPCImpactDamageTable, energyScale,
      allowStaticDamage, damageType, bDamageFromHeldObjects);
}

static bool IsPhysicallyControlled(CBaseEntity *pEntity,
...
float CalculateObjectStress(IPhysicsObject *pObject,
  ...
  while (pSnapshot->IsValid()) {
    ...
    if (force > 0.0f) {
      ...
      if (!pOtherEntity) {
        // object was just deleted, but we still have a contact point this
        // frame... just assume it came from the world.
      ...
      else if (pBCC && (pBCC->IRelationType(pOtherOwner) == D_LI ||
                        pOtherEntity->IsNavIgnored())) {
        outIndex = 1;
      }
      // player held objects do no stress
        ...
        // aggregate contacts over each object to avoid greater stress in
        // multiple contact cases NOTE: Contacts should be in order, so this
        // shouldn't ever search, but just in case
      ...
      if (outIndex != 0 &&
          pInputOwnerEntity->GetMoveType() != MOVETYPE_VPHYSICS &&
          !IsPhysicallyControlled(pOtherEntity, pOther)) {
        // UNDONE: Test this!  This is to remove any shadow/shadow stress.  The
        // game should handle this with blocked/damage
        force = 0.0f;
      }
  ...
  for (int objectIndex = pObjectList.Count() - 1; objectIndex >= 0;
       --objectIndex) {
  ...
  // "external" stress is two way (something pushes on the object and something
  // else pushes back) so the set of minimum values per component are the
  // projections of the two-way force "internal" stress is one way (the object
  // is pushing against something OR something pushing back) the momentum must
  // have come from inside the object (gravity, controller, etc)
  ...
  if (pInputOwnerEntity->GetMoveType() != MOVETYPE_VPHYSICS &&
      pObject->IsMoveable()) {
    Vector lastVel;
    lastVel.Init();
    if (pObject->GetShadowController()) {
      ...
      if ((pObject->GetCallbackFlags() & CALLBACK_IS_PLAYER_CONTROLLER)) {
        ...
        IPhysicsPlayerController *pController =
            pPlayer ? pPlayer->GetPhysicsController() : NULL;
        if (pController) {
    ...
    // Peek into the controller for this object.  Look at the input velocity and
    // make sure it's all accounted for in the computed stress.  If not,
    // redistribute external to internal as it's probably being reflected in a
    // way we can't measure here.
    float inputLen =
        lastVel.Length() * (1.0f / physenv->GetSimulationTimestep()) * objMass;
    if (inputLen > 0.0f) {
  ...
  if (!pObject->IsMoveable()) {
    // the above algorithm will see almost all force as internal if the object
    // is not moveable (it doesn't push on anything else, so nothing is
    // reciprocated) exceptions for friction of a single other object with
    // multiple contact points on this object

    // But the game wants to see it all as external because obviously the object
    // can't move, so it can't have internal stress
