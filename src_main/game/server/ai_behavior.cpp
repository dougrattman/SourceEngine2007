...
int CAI_BehaviorBase::SelectFailSchedule(int failedSchedule, int failedTask,
...
bool CAI_BehaviorBase::IsValidShootPosition(const Vector &vLocation,
                                            CAI_Node *pNode,
                                            CAI_Hint const *pHint) {
  Assert(m_pBackBridge != NULL);

  return m_pBackBridge->BackBridge_IsValidShootPosition(vLocation, pNode,
                                                        pHint);
}
...
void CAI_BehaviorBase::OnFriendDamaged(CBaseCombatCharacter *pSquadmate,
...
bool CAI_BehaviorBase::OnCalcBaseMove(AILocalMoveGoal_t *pMoveGoal,
                                      float distClear,
                                      AIMoveResult_t *pResult) {
  Assert(m_pBackBridge != NULL);

  return m_pBackBridge->BackBridge_OnCalcBaseMove(pMoveGoal, distClear,
                                                  pResult);
}
...
void CAI_BehaviorBase::Teleport(const Vector *newPosition,
                                const QAngle *newAngles,
                                const Vector *newVelocity) {
  Assert(m_pBackBridge != NULL);

  return m_pBackBridge->BackBridge_Teleport(newPosition, newAngles,
                                            newVelocity);
}
...
bool CAI_BehaviorBase::NotifyChangeBehaviorStatus(bool fCanFinishSchedule) {
  bool fInterrupt =
      GetOuter()->OnBehaviorChangeStatus(this, fCanFinishSchedule);
  ...
  if (fInterrupt) {
    if (GetOuter()->m_hCine) {
      if (GetOuter()->m_hCine->PlayedSequence()) {
        ...
        DevWarning(
            "NPC: %s canceled script %s without playing, due to behavior "
            "change\n",
            GetOuter()->GetDebugName(), GetOuter()->m_hCine->GetDebugName());
      }
    ...
    //!!!HACKHACK
    // this is dirty, but it forces NPC to pick a new schedule next time
    // through.
...
void CAI_BehaviorBase::SaveBehaviors(ISave &save,
  ...
  for (int i = 0; i < nBehaviors; i++) {
    if (strcmp(ppBehavior[i]->GetDataDescMap()->dataClassName,
...
int CAI_BehaviorBase::RestoreBehaviors(IRestore &restore,
                                       CAI_BehaviorBase **ppBehavior,
                                       int nBehaviors) {
  int iCurrent = -1;
    ...
    if (version == BEHAVIOR_SAVE_VERSION) {
      ...
      for (int i = 0; i < nToRestore; i++) {
        ...
        for (int j = 0; j < nBehaviors; j++) {
          if (strcmp(ppBehavior[j]->GetDataDescMap()->dataClassName,
                     szClassNameCurrent) == 0) {
            if (bIsCurrent) iCurrent = j;
