...
// The handle to the debug history singleton. Created on first access via
// GetDebugHistory.
...
void CDebugHistory::Spawn() {
  ...
  if (g_pGameRules && g_pGameRules->IsMultiplayer()) {
    ...
    Warning(
        "DEBUG HISTORY IS ENABLED. Disable before release (in "
        "env_debughistory.h).\n");
  }
#endif
...
void CDebugHistory::AddDebugHistoryLine(int iCategory, const char *szLine) {
  if (iCategory < 0 || iCategory >= MAX_HISTORY_CATEGORIES) {
    Warning(
        "Attempted to add a debughistory line to category %d. Valid categories "
        "are %d to %d.\n",
  ...
  int iCharsToWrite =
      strlen(pszRemaining) + 1;  // Add 1 so that we copy the 0 terminator

  // Clip the line if it's too long. Wasteful doing it this way, but keeps code
  // below nice & simple.
  ...
  while (iCharsToWrite) {
    int iCharsLeftBeforeLoop =
        sizeof(m_DebugLines[iCategory]) -
...
void CDebugHistory::DumpDebugHistory(int iCategory) {
  if (iCategory < 0 || iCategory >= MAX_HISTORY_CATEGORIES) {
    Warning(
        "Attempted to dump a history for category %d. Valid categories are %d "
        "to %d.\n",
        iCategory, 0, (MAX_HISTORY_CATEGORIES - 1));
    return;
  }
  ...
  // Are we at the start of a line? If there's a 0 terminator before us, then
  // we're good to go.
  while ((!(pszLine == m_DebugLines[iCategory] &&
            *(m_DebugLines[iCategory] + sizeof(m_DebugLines[iCategory]) - 1) ==
                '\0') &&
    ...
    // Have we looped?
    if ((pszLine - m_DebugLines[iCategory]) >=
        sizeof(m_DebugLines[iCategory])) {
      pszLine = m_DebugLines[iCategory];
    }
...
CDebugHistory *GetDebugHistory() {
  ...
  if (s_DebugHistory == NULL) {
    CBaseEntity *pEnt =
        gEntList.FindEntityByClassname(NULL, "env_debughistory");
    if (pEnt) {
...
static ConCommand dbghist_addline(
    "dbghist_addline", CC_DebugHistory_AddLine,
...
static ConCommand dbghist_dump(
    "dbghist_dump", CC_DebugHistory_Dump,
