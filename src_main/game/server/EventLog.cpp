...
bool CEventLog::PrintGameEvent(IGameEvent *event) {
  //	const char * name = event->GetName() + Q_strlen("game_"); // remove
  //prefix
...
bool CEventLog::PrintPlayerEvent(IGameEvent *event) {
  ...
  if (!Q_strncmp(eventName, "player_connect",
                 Q_strlen("player_connect")))  // player connect is before the
                                               // CBasePlayer pointer is setup
  {
    ...
    UTIL_LogPrintf("\"%s<%i><%s><>\" connected, address \"%s\"\n", name, userid,
                   networkid, address);
    return true;
  } else if (!Q_strncmp(eventName, "player_disconnect",
                        Q_strlen("player_disconnect"))) {
    ...
    UTIL_LogPrintf("\"%s<%i><%s><%s>\" disconnected (reason \"%s\")\n", name,
                   userid, networkid, team ? team->GetName() : "", reason);
    return true;
  }
  ...
  if (!pPlayer) {
    DevMsg(
        "CEventLog::PrintPlayerEvent: Failed to find player (userid: %i, "
        "event: %s)\n",
        userid, eventName);
    return false;
  }
  ...
  if (!Q_strncmp(eventName, "player_team", Q_strlen("player_team"))) {
    ...
    if (!bDisconnecting) {
      ...
      UTIL_LogPrintf("\"%s<%i><%s><%s>\" joined team \"%s\"\n",
                     ...
                     pPlayer->GetNetworkIDString(), oldteam->GetName(),
                     team->GetName());
    }
    ...
    if (pPlayer == pAttacker && pPlayer) {
      ...
      UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"%s\"\n",
                     pPlayer->GetPlayerName(), userid,
                     pPlayer->GetNetworkIDString(), team ? team->GetName() : "",
                     weapon);
#else
      UTIL_LogPrintf("\"%s<%i><%s><%s>\" committed suicide with \"%s\"\n",
                     pPlayer->GetPlayerName(), userid,
      ...
      UTIL_LogPrintf(
          ...
          pAttacker->GetPlayerName(), attackerid,
          pAttacker->GetNetworkIDString(),
          attackerTeam ? attackerTeam->GetName() : "", pPlayer->GetPlayerName(),
          userid, pPlayer->GetNetworkIDString(), team ? team->GetName() : "",
          weapon);
#else
      UTIL_LogPrintf(
    ...
    return true;
  } else if (!Q_strncmp(eventName, "player_activate",
                        Q_strlen("player_activate"))) {
    UTIL_LogPrintf("\"%s<%i><%s><>\" entered the game\n",
                   pPlayer->GetPlayerName(), userid,
                   pPlayer->GetNetworkIDString());

    return true;
  } else if (!Q_strncmp(eventName, "player_changename",
                        Q_strlen("player_changename"))) {
    const char *newName = event->GetString("newname");
    const char *oldName = event->GetString("oldname");
    CTeam *team = pPlayer->GetTeam();
    UTIL_LogPrintf("\"%s<%i><%s><%s>\" changed name to \"%s\"\n", oldName,
                   userid, pPlayer->GetNetworkIDString(),
                   team ? team->GetName() : "", newName);
    return true;
  }
...
bool CEventLog::PrintTeamEvent(IGameEvent *event) {
  //	const char * name = event->GetName() + Q_strlen("team_"); // remove
  //prefix

  return false;
}

bool CEventLog::PrintOtherEvent(IGameEvent *event) { return false; }
