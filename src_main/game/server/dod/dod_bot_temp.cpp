...
ConVar bot_forceattackon("bot_forceattackon", "0", 0,
...
static ConVar bot_mimic_yaw_offset("bot_mimic_yaw_offset", "0", 0,
                                   "Offsets the bot yaw.");
ConVar bot_attack("bot_attack", "0", 0, "Shoot!");

ConVar bot_sendcmd("bot_sendcmd", "", 0,
...
CBasePlayer *BotPutInServer(bool bFrozen, int iTeam, int iClass) {
  ...
  // This is an evil hack, but we use it to prevent sv_autojointeam from kicking
  // in.
  ...
  g_BotData[pPlayer->entindex() - 1].m_flJoinTeamTime =
      gpGlobals->curtime + 0.3;
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove(CDODPlayer *fakeclient, const QAngle &viewangles,
...
void Bot_Think(CDODPlayer *pBot) {
  ...
  if (pBot->GetTeamNumber() == TEAM_UNASSIGNED &&
    ...
    pBot->HandleCommand_JoinTeam(botdata->m_WantedTeam);
             ...
             pBot->m_Shared.PlayerClass() == PLAYERCLASS_UNDEFINED) {
    ...
    // If bots are being forced to fire a weapon, see if I have it
      ...
      CBaseCombatWeapon *pWeapon =
          pBot->Weapon_OwnsThisType(bot_forcefireweapon.GetString());
      if (pWeapon) {
    ...
    if (bot_flipout.GetInt()) {
      ...
      if (RandomFloat(0.0, 1.0) > 0.9) buttons |= IN_RELOAD;
    }
  ...
  if (bot_flipout.GetInt() >= 2) {
    ...
    for ( int i = 0 ; i < 2; i++ )
    {
            if ( fabs( botdata->lastAngles[ i ] - botdata->forwardAngle[ i ] )
    > 15.0f )
            {
                    if ( botdata->lastAngles[ i ] > botdata->forwardAngle[ i ] )
                    {
                            botdata->lastAngles[ i ] = botdata->forwardAngle[ i
    ] + 15;
                    }
                    else
                    {
  ...
  RunPlayerMove(pBot, pBot->GetLocalAngles(), forwardmove, sidemove, upmove,
                buttons, impulse, frametime);
}
