...
// Redefine some things for the stat reader so it doesn't have to include
// weapon_dodbase.h
#ifndef GAME_DLL
...
struct dod_gamestats_weapon_distance_t {
  short iNumAttacks;  // times fired
  short iNumHits;     // times hit

  // distance buckets - distances are defined per-weapon ( 0 is closest,
  // buckets-1 farthest )
  short iDistanceBuckets[DOD_NUM_WEAPON_DISTANCE_BUCKETS];
};

// Stats for non-bullet weapons
struct dod_gamestats_weapon_nodist_t {
  short iNumAttacks;  // times fired
  short iNumHits;     // times hit
};

struct dod_gamestats_t {
  ...
  // Weapon Data
  dod_gamestats_weapon_distance_t weaponStatsDistance
      [DOD_NUM_DISTANCE_STAT_WEAPONS];  // 14 * 22 = 308 bytes
  dod_gamestats_weapon_nodist_t
      weaponStats[DOD_NUM_NODIST_STAT_WEAPONS];  // 4 * 14 = 56 bytes
