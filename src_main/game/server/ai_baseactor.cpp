...
ConVar ai_debug_expressions("ai_debug_expressions", "0", FCVAR_NONE,
                            "Show random expression decisions for NPCs.");
static ConVar scene_showfaceto(
    "scene_showfaceto", "0", FCVAR_ARCHIVE,
...
BEGIN_DATADESC(CAI_BaseActor)
  ...
  DEFINE_FIELD(m_fLatchedPositions, FIELD_INTEGER),
      ...
      DEFINE_KEYFIELD(m_bDontUseSemaphore, FIELD_BOOLEAN,
                      "DontUseSpeechSemaphore"),

      DEFINE_KEYFIELD(m_iszExpressionOverride, FIELD_STRING,
                      "ExpressionOverride"),

      DEFINE_EMBEDDEDBYREF(m_pExpresser),

      DEFINE_INPUTFUNC(FIELD_STRING, "SetExpressionOverride",
                       InputSetExpressionOverride),

END_DATADESC()

BEGIN_SIMPLE_DATADESC(CAI_InterestTarget_t)
  DEFINE_FIELD(m_eType, FIELD_INTEGER), DEFINE_FIELD(m_hTarget, FIELD_EHANDLE),
...
bool CAI_BaseActor::StartSceneEvent(CSceneEventInfo *info, CChoreoScene *scene,
...
bool CAI_BaseActor::ProcessSceneEvent(CSceneEventInfo *info,
                                      CChoreoScene *scene,
                                      CChoreoEvent *event) {
  Assert(info);
  Assert(info->m_pScene);
  ...
  switch (info->m_pEvent->GetType()) {
    case CChoreoEvent::FACE: {
      ...
      // lockbodyfacing is designed to run on top of both normal AI and on top
      // of scripted_sequences.  By allowing torso turns during post-idles,
      // pre-idles, act-busy's, scripted_sequences, normal AI movements, etc.,
      // it increases the functionality of those AI features without breaking
      // their assuptions that the entity won't be made to "turn" by something
      // outside of those AI's control. lockbody facing is also usefull when
      // npcs are moving and you want them to turn towards something but still
      ...
      m_goalBodyYaw =
          m_goalBodyYaw * (1.0 - intensity) + intensity * flBodyYaw * dir;

      /*
      NDebugOverlay::YawArrow( GetAbsOrigin(), GetLocalAngles().y, 64, 16, 255,
      255, 255, 0, true, 0.1 ); NDebugOverlay::YawArrow( GetAbsOrigin() +
      Vector( 0, 0, 8 ), GetLocalAngles().y + m_goalBodyYaw, 64, 16, 255, 128,
      128, 0, true, 0.1 ); NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0,
      0, 16 ), GetLocalAngles().y + m_goalSpineYaw, 64, 16, 128, 255, 128, 0,
      true, 0.1 ); NDebugOverlay::YawArrow( GetAbsOrigin() + Vector( 0, 0, 24 ),
      info->m_flTargetYaw, 64, 16, 128, 128, 255, 0, true, 0.1 );
      */
      ...
      if (developer.GetInt() > 0 && scene_showfaceto.GetBool()) {
        NDebugOverlay::YawArrow(GetAbsOrigin() + Vector(0, 0, 1), goalYaw,
                                8 + 32 * intensity, 8, 255, 255, 255, 0, true,
                                0.12);
      }
      ...
      if (!event->IsLockBodyFacing()) {
        AddFacingTarget(info->m_hTarget, intensity,
                        0.2);  // facing targets are lagged by one frame
      }
      return true;
    }
    case CChoreoEvent::GENERIC: {
      switch (info->m_nType) {
        ...
        case SCENE_AI_RANDOMLOOK: {
          if (info->m_flNext < gpGlobals->curtime) {
            info->m_flNext =
                gpGlobals->curtime + PickLookTarget(m_syntheticLookQueue) - 0.4;
...
bool CAI_BaseActor::RandomFaceFlex(CSceneEventInfo *info, CChoreoScene *scene,
                                   CChoreoEvent *event) {
  if (info->m_flNext < gpGlobals->curtime) {
    const flexsettinghdr_t *pSettinghdr =
        (const flexsettinghdr_t *)FindSceneFile(event->GetParameters2());
    if (pSettinghdr == NULL) {
    ...
    if (pSettinghdr) {
      info->m_flNext =
          gpGlobals->curtime +
          random->RandomFloat(0.3, 0.5) * (30.0 / pSettinghdr->numflexsettings);

      flexsetting_t const *pSetting = NULL;
      pSetting = pSettinghdr->pSetting(
          random->RandomInt(0, pSettinghdr->numflexsettings - 1));

      flexweight_t *pWeights = NULL;
      int truecount = pSetting->psetting((byte *)pSettinghdr, 0, &pWeights);
      if (!pWeights) return false;

      int i;
      for (i = 0; i < truecount; i++, pWeights++) {
        // Translate to local flex controller
        // this is translating from the settings's local index to the models
        // local index
  ...
  // adjust intensity if this is a background scene and there's other flex
  // animations playing
  float intensity =
      info->UpdateWeight(this) * event->GetIntensity(scene->GetTime());
...
bool CAI_BaseActor::ClearSceneEvent(CSceneEventInfo *info, bool fastKill,
                                    bool canceled) {
  Assert(info);
  Assert(info->m_pScene);
  Assert(info->m_pEvent);
...
bool CAI_BaseActor::CheckSceneEventCompletion(CSceneEventInfo *info,
                                              float currenttime,
                                              CChoreoScene *scene,
                                              CChoreoEvent *event) {
  Assert(info);
    ...
    case CChoreoEvent::GENERIC: {
      switch (info->m_nType) {
        ...
        case SCENE_AI_UNHOLSTER: {
          ...
          float t = (1.0 - GetLayerCycle(info->m_iLayer)) *
                    SequenceDuration(GetLayerSequence(info->m_iLayer));
...
void CAI_BaseActor::UpdateLatchedValues() {
  if (!(m_fLatchedPositions & HUMANOID_LATCHED_HEAD)) {
    ...
    // origin.z, angles.x, angles.y, angles.z );
  }
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if target is in legal range of eye movement for the
...
bool CAI_BaseActor::ValidEyeTarget(const Vector &lookTargetPos) {
  ...
  // lookTargetPos.y, lookTargetPos.z, dotPr );
...
bool CAI_BaseActor::ValidHeadTarget(const Vector &lookTargetPos) {
  ...
  if (dotPr > 0 &&
      fabs(lookTargetDir.z) < 0.7)  // +- 90 degrees side to side, +- 45 up/down
  {
    return true;
  }
  return false;
...
float CAI_BaseActor::HeadTargetValidity(const Vector &lookTargetPos) {
  ...
  if (flDist < MAX_FULL_LOOK_TARGET_DIST) {
    flInterest = flInterest * (flDist - MIN_LOOK_TARGET_DIST) /
                 (MAX_FULL_LOOK_TARGET_DIST - MIN_LOOK_TARGET_DIST);
  }
...
//-----------------------------------------------------------------------------
void CAI_BaseActor::SetHeadDirection(const Vector &vTargetPos,
                                     float flInterval) {
  Assert(0);  // Actors shouldn't be calling this, it doesn't do anything
}

float CAI_BaseActor::ClampWithBias(PoseParameter_t index, float value,
                                   float base) {
...
//-----------------------------------------------------------------------------
// Purpose: match actors "forward" attachment to point in direction of
// vHeadTarget
//-----------------------------------------------------------------------------
...
static ConVar scene_clamplookat(
    "scene_clamplookat", "1", FCVAR_NONE,
    "Clamp head turns to a max of 20 degrees per think.");

void CAI_BaseActor::UpdateHeadControl(const Vector &vHeadTarget,
  ...
  // calc current animation head bias, movement needs to clamp accumulated with
  // this
  QAngle angBias;
  ...
  if (iEyes <= 0 || iForward <= 0) {
    ...
    // Most likely this is a cheaple or a generic_actor set to a model that
    // doesn't support head/eye turning. DevWarning( "%s using model \"%s\" that
    // doesn't support head turning\n", GetClassname(), STRING( GetModelName() )
    // );
  ...
  if (iChest > 0) {
    ...
    if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
    {
            ...
            // NDebugOverlay::Line( EyePosition(), pEntity->EyePosition(),
    0,0,255, false, 0.5);
    }
    */
  } else {
  ...
  m_goalHeadCorrection.x =
      UTIL_Approach(m_goalHeadCorrection.x * s0 + vTargetAngles.x * s1,
                    m_goalHeadCorrection.x, 10.0);
  m_goalHeadCorrection.y =
      UTIL_Approach(m_goalHeadCorrection.y * s0 + vTargetAngles.y * s1,
                    m_goalHeadCorrection.y, 30.0);
  m_goalHeadCorrection.z =
      UTIL_Approach(m_goalHeadCorrection.z * s0 + vTargetAngles.z * s1,
                    m_goalHeadCorrection.z, 10.0);

  /*
  if ( (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) )
  {
          // Msg( "yaw %.1f (%f) pitch %.1f (%.1f)\n", m_goalHeadCorrection.y,
  vTargetAngles.y, vTargetAngles.x, m_goalHeadCorrection.x );
          // Msg( "yaw %.2f (goal %.2f) (influence %.2f) (flex %.2f)\n",
  flLimit, m_goalHeadCorrection.y, flHeadInfluence, Get(
  ...
  */
  ...
  m_goalHeadCorrection.y, Get( m_FlexweightHeadRightLeft ), angBias.y, Get(
  m_ParameterHeadYaw ) );
  }
  */
  ...
  m_goalHeadCorrection.x, Get( m_FlexweightHeadUpDown ), angBias.x, Get(
  ...
  */
  ...
  m_goalHeadCorrection.z, Get( m_FlexweightHeadTilt ), angBias.z, Get(
  m_ParameterHeadRoll ) );
  }
  */
...
//-----------------------------------------------------------------------------
float CAI_BaseActor::PickLookTarget(bool bExcludePlayers, float minTime,
                                    float maxTime) {
  return PickLookTarget(m_randomLookQueue, bExcludePlayers, minTime, maxTime);
}

float CAI_BaseActor::PickLookTarget(CAI_InterestTarget &queue,
                                    bool bExcludePlayers, float minTime,
                                    float maxTime) {
  ...
  // See if derived NPCs want to do anything with this look target before I use
  // it
...
bool CAI_BaseActor::PickTacticalLookTarget(AILookTargetArgs_t *pArgs) {
  ...
  if (pEnemy != NULL) {
    if ((FVisible(pEnemy) || random->RandomInt(0, 3) == 0) &&
        ValidHeadTarget(pEnemy->EyePosition())) {
...
bool CAI_BaseActor::PickRandomLookTarget(AILookTargetArgs_t *pArgs) {
  bool bIsNavigating =
      (GetNavigator()->IsGoalActive() && GetNavigator()->IsGoalSet());

  if (bIsNavigating && random->RandomInt(1, 10) <= 3) {
    Vector navLookPoint;
    Vector delta;
    if (GetNavigator()->GetPointAlongPath(&navLookPoint, 12 * 12) &&
        (delta = navLookPoint - GetAbsOrigin()).Length() > 8.0 * 12.0) {
  ...
  int iConsidered = 0;
  for (CEntitySphereQuery sphere(GetAbsOrigin(), 30 * 12, 0);
    ...
    if (bObjectParentedToPlayer) continue;
    ...
    if ((pEntity->GetFlags() & FL_CLIENT) &&
        (pEntity->IsMoving() || random->RandomInt(0, 2) == 0)) {
...
#else
    // No, for now, give all targets random priority (as long as they're in
    // front)
    ...
    if (iImportance > iHighestImportance) {
      if (FVisible(pEntity) && ValidHeadTarget(pEntity->EyePosition())) {
        ...
        pBestEntity = pEntity;
        // NDebugOverlay::Line( EyePosition(), pEntity->EyePosition(), 0,0,255,
        // false, 0.5);
      }
    }
  }
  ...
  if (pBestEntity) {
    ...
    // NDebugOverlay::Line( EyePosition(), pBestEntity->WorldSpaceCenter(), 255,
    // 0, 0, false, 5 );
...
//-----------------------------------------------------------------------------
void CAI_BaseActor::MakeRandomLookTarget(AILookTargetArgs_t *pArgs,
  ...
  // For now, just look farther afield while driving in the vehicle.  Without
  // this we look around wildly!
#ifdef HL2_EPISODIC
  if (MyCombatCharacterPointer() &&
      MyCombatCharacterPointer()->IsInAVehicle()) {
    pArgs->vTarget = EyePosition() + forward * 2048 +
                     right * random->RandomFloat(-650, 650) +
                     up * random->RandomFloat(-32, 32);
  } else
#endif  // HL2_EPISODIC
  {
    pArgs->vTarget = EyePosition() + forward * 128 +
                     right * random->RandomFloat(-32, 32) +
                     up * random->RandomFloat(-16, 16);
  }
...
//-----------------------------------------------------------------------------
void CAI_BaseActor::AddLookTarget(CBaseEntity *pTarget, float flImportance,
                                  float flDuration, float flRamp) {
  m_lookQueue.Add(pTarget, flImportance, flDuration, flRamp);
}

void CAI_BaseActor::AddLookTarget(const Vector &vecPosition, float flImportance,
                                  float flDuration, float flRamp) {
...
void CAI_BaseActor::MaintainLookTargets(float flInterval) {
  ...
  if (m_iszExpressionScene != NULL_STRING && m_hExpressionSceneEnt == NULL) {
    InstancedScriptedScene(this, STRING(m_iszExpressionScene),
                           &m_hExpressionSceneEnt, 0.0, true);
  }
  ...
  // initialize goal head direction to be current direction - this frames
  // animation layering/pose parameters - but with the head controlls removed.
  Vector vHead = HeadDirection3D();
  float flHeadInfluence = 0.0;

  // NDebugOverlay::Line( vEyePosition, vEyePosition + vHead * 16, 0,0,255,
  // false, 0.1);
  ...
  if (m_lookQueue.Count() != 0 || m_syntheticLookQueue.Count() != 0) {
    for (i = 0; i < m_randomLookQueue.Count(); i++) {
      if (gpGlobals->curtime < m_randomLookQueue[i].m_flEndTime -
                                   m_randomLookQueue[i].m_flRamp - 0.2) {
        m_randomLookQueue[i].m_flEndTime =
            gpGlobals->curtime + m_randomLookQueue[i].m_flRamp + 0.2;
      }
    }
    m_flNextRandomLookTime = gpGlobals->curtime + 1.0;
  } else if (gpGlobals->curtime >= m_flNextRandomLookTime &&
             GetState() != NPC_STATE_SCRIPT) {
    // Look at whatever!
    m_flNextRandomLookTime =
        gpGlobals->curtime + PickLookTarget(m_randomLookQueue) - 0.4;
  }
  ...
  if (m_flNextRandomExpressionTime &&
      gpGlobals->curtime > m_flNextRandomExpressionTime) {
  ...
  for (i = 0; i < active.Count(); i++) {
    ...
    active[i]->m_hTarget->GetClassname(), active[i]->GetPosition().x,
    active[i]->GetPosition().y, active[i]->GetPosition().z );
    }
    */
    ...
    if (flInterest > 0.0) {
      ...
      // NDebugOverlay::Line( vEyePosition, vEyePosition + dir * 64, 0,255,0,
      // false, 0.1);
    } else {
      // NDebugOverlay::Line( vEyePosition, active[i]->GetPosition(), 255,0,0,
      // false, 0.1);
    }
  }
  ...
  if (bValidHeadTarget) {
    ...
    // 100, 255,0,0, false, 0.1);
  }

  // DevMsg( "%.1f %.1f ", GetPoseParameter( "head_pitch" ), GetPoseParameter(
  // "head_roll" ) );

  // figure out eye target
  // eyes need to look directly at a target, even if the head doesn't quite aim
  // there yet.
  ...
  for (i = active.Count() - 1; i >= 0; i--) {
    if (active[i]->IsThis(this)) {
      // DevMsg( "eyes (%d) %s\n", i, STRING(
      // active[i]->m_hTarget->GetEntityName().ToCStr() ) );
      bFoundTarget = true;
      hTarget = this;
      SetViewtarget(vEyePosition + HeadDirection3D() * 100);
      // NDebugOverlay::Line( vEyePosition, vEyePosition + HeadDirection3D() *
      // 100, 255,0,0, false, 0.1);
      break;
    } else {
      // E3 Hack
  ...
  if (m_hLookTarget != hTarget) {
    ...
    if ((m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) &&
        ai_debug_looktargets.GetInt() == 2 && m_hLookTarget.Get()) {
      if (m_hLookTarget != this) {
        Vector vecEyePos = m_hLookTarget->EyePosition();
        NDebugOverlay::Box(vecEyePos, -Vector(5, 5, 5), Vector(5, 5, 5), 0, 255,
                           0, 255, 20);
        NDebugOverlay::Line(EyePosition(), vecEyePos, 0, 255, 0, true, 20);
        NDebugOverlay::Text(
            vecEyePos,
            UTIL_VarArgs("%s (%s)", m_hLookTarget->GetClassname(),
                         m_hLookTarget->GetDebugName()),
            false, 20);
      }
    }
  ...
  // NDebugOverlay::Triangle( vEyePosition, GetViewtarget(), GetAbsOrigin(),
  // 255, 255, 255, 10, false, 0.1 );

  // DevMsg("pitch %.1f yaw %.1f\n", GetFlexWeight( "eyes_updown" ),
  // GetFlexWeight( "eyes_rightleft" ) );
  ...
  if (ai_debug_looktargets.GetInt() == 1 &&
      (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)) {
    NDebugOverlay::Box(GetViewtarget(), -Vector(2, 2, 2), Vector(2, 2, 2), 0,
                       255, 0, 0, 20);
...
#ifdef HL2_EPISODIC
  // If we've just switched states, ensure we stop any scenes that asked to be
  // stopped
...
float CAI_BaseActor::SetExpression(const char *pszExpressionScene) {
  ...
  if (m_iszExpressionScene != NULL_STRING &&
      stricmp(STRING(m_iszExpressionScene), pszExpressionScene) == 0) {
    return 0;
  }
  ...
  if (pszExpressionScene) {
    float flDuration = InstancedScriptedScene(
        this, pszExpressionScene, &m_hExpressionSceneEnt, 0.0, true);
...
bool CAI_BaseActor::UseSemaphore(void) {
  if (m_bDontUseSemaphore) return false;
