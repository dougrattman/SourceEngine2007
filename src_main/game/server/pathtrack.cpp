...
BEGIN_DATADESC(CPathTrack)
  ...
  DEFINE_FIELD(m_pnext, FIELD_CLASSPTR),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableAlternatePath",
                       InputEnableAlternatePath),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableAlternatePath",
                       InputDisableAlternatePath),
      DEFINE_INPUTFUNC(FIELD_VOID, "ToggleAlternatePath",
...
void CPathTrack::ToggleAlternatePath(void) {
  ...
  if (m_paltpath != NULL) {
    if (FBitSet(m_spawnflags.operator const int &(), SF_PATH_ALTERNATE) ==
        false) {
      EnableAlternatePath();
    } else {
...
void CPathTrack::EnableAlternatePath(void) {
  if (m_paltpath != NULL) {
    SETBITS(m_spawnflags.operator const int &(), SF_PATH_ALTERNATE);
  }
}
...
void CPathTrack::DisableAlternatePath(void) {
  if (m_paltpath != NULL) {
    CLEARBITS(m_spawnflags.operator const int &(), SF_PATH_ALTERNATE);
  }
}
...
void CPathTrack::TogglePath(void) {
  // Use toggles between two paths
  if (FBitSet(m_spawnflags.operator const int &(), SF_PATH_DISABLED)) {
    EnablePath();
  } else {
    DisablePath();
  }
...
void CPathTrack::EnablePath(void) {
  CLEARBITS(m_spawnflags.operator const int &(), SF_PATH_DISABLED);
}
...
void CPathTrack::DisablePath(void) {
  SETBITS(m_spawnflags.operator const int &(), SF_PATH_DISABLED);
}
...
void CPathTrack::DrawDebugGeometryOverlays() {
  ...
  if (m_debugOverlays & (OVERLAY_BBOX_BIT | OVERLAY_ABSBOX_BIT)) {
    if (m_pnext) {
      NDebugOverlay::Line(GetAbsOrigin(), m_pnext->GetAbsOrigin(), 255, 100,
                          100, true, 0.0);
    }
  }
...
CPathTrack *CPathTrack::ValidPath(CPathTrack *ppath, int testFlag) {
  ...
  if (testFlag &&
      FBitSet(ppath->m_spawnflags.operator const int &(), SF_PATH_DISABLED))
    return NULL;

  return ppath;
}

void CPathTrack::Project(CPathTrack *pstart, CPathTrack *pend, Vector &origin,
                         float dist) {
  if (pstart && pend) {
...
CPathTrack *CPathTrack::GetNext(void) {
  if (m_paltpath &&
      FBitSet(m_spawnflags.operator const int &(), SF_PATH_ALTERNATE) &&
      !FBitSet(m_spawnflags.operator const int &(), SF_PATH_ALTREVERSE))
    return m_paltpath;
...
//			dist - distance to look ahead, negative values look
// backward 			move -
...
CPathTrack *CPathTrack::LookAhead(Vector &origin, float dist, int move,
  ...
  while (dist > 0) {
    ...
    Vector dir =
        pcurrent->GetNextInDir(bForward)->GetLocalOrigin() - currentPos;
    float length = dir.Length();

    // If we are at the next node and there isn't one beyond it, return the next
    // node.
    if (!length &&
        !ValidPath(pcurrent->GetNextInDir(bForward)->GetNextInDir(bForward),
                   move)) {
      if (pNextNext) {
        *pNextNext = NULL;
      }
    ...
    // If we don't hit the next path track within the distance remaining, we're
    // done.
    if (length > dist) {
...
CPathTrack *CPathTrack::Instance(edict_t *pent) {
  ...
  if (FClassnameIs(pEntity, "path_track")) return (CPathTrack *)pEntity;
  return NULL;
}
