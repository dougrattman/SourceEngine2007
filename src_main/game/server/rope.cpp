...
SendPropEHandle(SENDINFO(m_hStartPoint)),
    ...
    SendPropInt(SENDINFO(m_Slack), 12), SendPropInt(SENDINFO(m_RopeLength), 15),
...
()
  ...
  m_RopeLength = 20;
  m_fLockedPoints =
      (int)(ROPE_LOCK_START_POINT |
            ROPE_LOCK_END_POINT);  // by default, both points are locked
  m_flScrollSpeed = 0;
...
void CRopeKeyframe::SetAttachmentPoint(CBaseHandle &hOutEnt,
                                       short &iOutAttachment, CBaseEntity *pEnt,
                                       int iAttachment) {
...
void CRopeKeyframe::SetStartPoint(CBaseEntity *pStartPoint, int attachment) {
  SetAttachmentPoint(m_hStartPoint.GetForModify(),
                     m_iStartAttachment.GetForModify(), pStartPoint,
                     attachment);
}

void CRopeKeyframe::SetEndPoint(CBaseEntity *pEndPoint, int attachment) {
  SetAttachmentPoint(m_hEndPoint.GetForModify(),
                     m_iEndAttachment.GetForModify(), pEndPoint, attachment);
}
...
CRopeKeyframe *CRopeKeyframe::Create(CBaseEntity *pStartEnt,
                                     CBaseEntity *pEndEnt, int iStartAttachment,
                                     int iEndAttachment, int ropeWidth,
  ...
  pRet->m_nSegments = std::clamp(numSegments, 2, ROPE_MAX_SEGMENTS);

  return pRet;
}

CRopeKeyframe *CRopeKeyframe::CreateWithSecondPointDetached(
    CBaseEntity *pStartEnt, int iStartAttachment, int ropeLength, int ropeWidth,
    const char *pMaterialName, int numSegments, bool bInitialHang) {
  CRopeKeyframe *pRet = (CRopeKeyframe *)CreateEntityByName("keyframe_rope");
  if (!pRet) return NULL;
  ...
  pRet->m_bCreatedFromMapFile = false;
  pRet->m_fLockedPoints.Set(
      ROPE_LOCK_START_POINT);  // Only attach the first point.
...
void CRopeKeyframe::ShakeRopes(const Vector &vCenter, float flRadius,
                               float flMagnitude) {
...
void CRopeKeyframe::Init() {
  ...
  m_nSegments = std::clamp(static_cast<int>(m_nSegments), 2, ROPE_MAX_SEGMENTS);
...
void CRopeKeyframe::Activate() {
  ...
  if (!m_bCreatedFromMapFile) return;
  ...
  if (pEnt && pEnt->edict()) {
    ...
    if (m_spawnflags & SF_ROPE_RESIZE) m_RopeFlags |= ROPE_RESIZE;
  } else {
  ...
  // If we don't do this here, then when we save/load, we won't "own" the
  // transmit state of our parent, so the client might get our entity without
  // our parent entity.
...
void CRopeKeyframe::RecalculateLength(void) {
  ...
  if (m_hEndPoint.Get()) {
    ...
    // Set the length
    m_RopeLength =
        (int)(pStartEnt->GetAbsOrigin() - pEndEnt->GetAbsOrigin()).Length();
  } else {
    m_RopeLength = 0;
  }
}
...
//			Right now only the client knows when it reaches a
//resting state, so 			for now it just removes itself after a
// short time.
//-----------------------------------------------------------------------------
...
void CRopeKeyframe::SetTransmit(CCheckTransmitInfo *pInfo, bool bAlways) {
  if (!pInfo->m_pTransmitEdict->Get(entindex())) {
    ...
    if (pEnt) pEnt->SetTransmit(pInfo, bAlways);
...
bool CRopeKeyframe::GetEndPointPos2(CBaseEntity *pAttached, int iAttachment,
                                    Vector &vPos) {
  if (!pAttached) return false;

  if (iAttachment > 0) {
    CBaseAnimating *pAnim = pAttached->GetBaseAnimating();
    if (pAnim) {
      if (!pAnim->GetAttachment(iAttachment, vPos)) return false;
    } else {
      return false;
    }
  } else {
...
// Output  :
//------------------------------------------------------------------------------
void CRopeKeyframe::PropagateForce(CBaseEntity *pActivator,
                                   CBaseEntity *pCaller,
                                   CBaseEntity *pFirstLink, float x, float y,
                                   float z) {
  ...
  CRopeKeyframe *pNextLink =
      dynamic_cast<CRopeKeyframe *>((CBaseEntity *)m_hEndPoint);
...
void CRopeKeyframe::InputSetForce(inputdata_t &inputdata) {
  ...
  PropagateForce(inputdata.pActivator, inputdata.pCaller, this, vecForce.x,
                 vecForce.y, vecForce.z);
}
...
void CRopeKeyframe::NotifyPositionChanged(CBaseEntity *pEntity) {
  ...
  if ((m_RopeFlags & ROPE_RESIZE) && ents[0] && ents[0]->edict() && ents[1] &&
      ents[1]->edict()) {
    int len =
        (int)(ents[0]->GetAbsOrigin() - ents[1]->GetAbsOrigin()).Length() +
        m_Slack;
    if (len != m_RopeLength) {
      m_RopeLength = len;
    }
  ...
  // Figure out if our attachment points have gone away and make sure to update
...
int CRopeKeyframe::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (!(m_RopeFlags & ROPE_BREAKABLE)) return false;
...
bool CRopeKeyframe::KeyValue(const char *szKeyName, const char *szValue) {
  if (stricmp(szKeyName, "Breakable") == 0) {
    if (atoi(szValue) == 1) m_RopeFlags |= ROPE_BREAKABLE;
  } else if (stricmp(szKeyName, "Collide") == 0) {
    if (atoi(szValue) == 1) m_RopeFlags |= ROPE_COLLIDE;
  } else if (stricmp(szKeyName, "Barbed") == 0) {
    if (atoi(szValue) == 1) m_RopeFlags |= ROPE_BARBED;
...
int CRopeKeyframe::UpdateTransmitState() {
  // Certain entities like sprites and ropes are strewn throughout the level and
  // they rarely change. For these entities, it's more efficient to transmit
  // them once and then always leave them on the client. Otherwise, the server
  // will have to send big bursts of data with the entity states as they come in
  // and out of the PVS.
