...
class CEnvBeam : public CBeam {
 public:
  ...
  inline bool ServerSide(void) {
    if (m_life == 0 && !HasSpawnFlags(SF_BEAM_RING)) return true;
...
BEGIN_DATADESC(CEnvBeam)
  ...
  DEFINE_FIELD(m_active, FIELD_INTEGER),
      ...
      // Function Pointers
      DEFINE_FUNCTION(StrikeThink), DEFINE_FUNCTION(UpdateThink),
...
void CEnvBeam::Spawn(void) {
  ...
  m_noiseAmplitude = std::min(MAX_BEAM_NOISEAMPLITUDE, m_noiseAmplitude);
  ...
  if (ServerSide()) {
    ...
    m_active = 0;
    if (!GetEntityName() ||
        FBitSet(m_spawnflags.operator const int &(), SF_BEAM_STARTON)) {
...
void CEnvBeam::Precache(void) {
  if (!Q_stristr(STRING(m_iszSpriteName), ".vmt")) {
    // HACK/YWB:  This was almost always the laserbeam.spr, so alloc'ing the
    // name a second time with the proper extension isn't going to
    //  kill us on memrory.
    // Warning( "Level Design Error:  %s (%i:%s) Sprite name (%s) missing .vmt
    // extension!\n", 	STRING( m_iClassname ), entindex(), GetEntityName(),
    //STRING(m_iszSpriteName) );
...
//			any interval refiring that might be going on. If the
//lifetime is set 			to zero (infinite) it will turn on and stay on.
//-----------------------------------------------------------------------------
void CEnvBeam::InputStrikeOnce(inputdata_t &inputdata) { Strike(); }

//-----------------------------------------------------------------------------
// Purpose: Turns the lightning on. If it is set for interval refiring, it will
//			begin doing so. If it is set to be continually on, it will
//do so.
//-----------------------------------------------------------------------------
...
void CEnvBeam::StrikeThink(void) {
  if (m_life != 0) {
    if (m_spawnflags & SF_BEAM_RANDOM)
      SetNextThink(gpGlobals->curtime + m_life +
                   random->RandomFloat(0, m_restrike));
    else
...
void CEnvBeam::Strike(void) {
  ...
  m_speed = std::clamp(m_speed, 0, MAX_BEAM_SCROLLSPEED);
  ...
  if (pointStart || pointEnd) {
    ...
    te->BeamEntPoint(filter, 0.0, pointStart ? 0 : pStart->entindex(),
                     pointStart ? &pStart->GetAbsOrigin() : NULL,
                     pointEnd ? 0 : pEnd->entindex(),
                     pointEnd ? &pEnd->GetAbsOrigin() : NULL, m_spriteTexture,
                     0,  // No halo
                     m_frameStart, (int)m_flFrameRate, m_life, m_boltWidth,
                     m_boltWidth,  // End width
                     0,            // No fade
                     m_noiseAmplitude, m_clrRender->r, m_clrRender->g,
                     m_clrRender->b, m_clrRender->a, m_speed);
  } else {
    if (m_spawnflags & SF_BEAM_RING) {
      te->BeamRing(filter, 0.0, pStart->entindex(), pEnd->entindex(),
                   m_spriteTexture,
                   0,  // No halo
                   m_frameStart, (int)m_flFrameRate, m_life, m_boltWidth,
                   0,  // No spread
      ...
      te->BeamEnts(filter, 0.0, pStart->entindex(), pEnd->entindex(),
                   m_spriteTexture,
                   0,  // No halo
                   m_frameStart, (int)m_flFrameRate, m_life, m_boltWidth,
                   m_boltWidth,  // End width
  ...
  if (m_flDamage > 0) {
    trace_t tr;
    UTIL_TraceLine(pStart->GetAbsOrigin(), pEnd->GetAbsOrigin(), MASK_SOLID,
...
class CTraceFilterPlayersNPCs : public ITraceFilter {
 public:
  bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    ...
    if (pEntity) {
      if (pEntity->IsPlayer() || pEntity->MyNPCPointer()) return true;
    }
  ...
  bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    ...
    if (pEntity) {
      if (pEntity->IsPlayer() || pEntity->MyNPCPointer() ||
          pEntity->m_iClassname == g_iszPhysicsPropClassname)
        return true;
    }
...
void CEnvBeam::UpdateThink(void) {
  ...
  if ((m_flDamage > 0) && (gpGlobals->curtime >= m_flFireTime + 0.1)) {
    trace_t tr;
    UTIL_TraceLine(GetAbsStartPos(), GetAbsEndPos(), MASK_SOLID, NULL,
...
void CEnvBeam::RandomArea(void) {
  ...
  for (iLoops = 0; iLoops < 10; iLoops++) {
    ...
    Vector vecDir1 =
        Vector(random->RandomFloat(-1.0, 1.0), random->RandomFloat(-1.0, 1.0),
               random->RandomFloat(-1.0, 1.0));
    VectorNormalize(vecDir1);
    trace_t tr1;
    UTIL_TraceLine(vecSrc, vecSrc + vecDir1 * m_radius, MASK_SOLID_BRUSHONLY,
                   this, COLLISION_GROUP_NONE, &tr1);

    if (tr1.fraction == 1.0) continue;
    ...
    trace_t tr2;
    UTIL_TraceLine(vecSrc, vecSrc + vecDir2 * m_radius, MASK_SOLID_BRUSHONLY,
                   this, COLLISION_GROUP_NONE, &tr2);

    if (tr2.fraction == 1.0) continue;

    if ((tr1.endpos - tr2.endpos).Length() < m_radius * 0.1) continue;

    UTIL_TraceLine(tr1.endpos, tr2.endpos, MASK_SOLID_BRUSHONLY, this,
...
void CEnvBeam::RandomPoint(const Vector &vecSrc) {
  ...
  for (iLoops = 0; iLoops < 10; iLoops++) {
    ...
    if ((tr1.endpos - vecSrc).Length() < m_radius * 0.1) continue;
...
void CEnvBeam::BeamUpdateVars(void) {
  ...
  SetWidth(std::min(MAX_BEAM_WIDTH, m_boltWidth));
  SetNoise(std::min(MAX_BEAM_NOISEAMPLITUDE, m_noiseAmplitude));
