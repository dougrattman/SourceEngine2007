...
BEGIN_DATADESC(CSlideshowDisplay)
  DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
      ...
      DEFINE_AUTO_ARRAY_KEYFIELD(m_szDisplayText, FIELD_CHARACTER,
                                 "displaytext"),
      ...
      DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetNoListRepeats",
...
bool CSlideshowDisplay::KeyValue(const char *szKeyName, const char *szValue) {
  ...
  if (FStrEq(szKeyName, "angles")) {
    ...
    // Because the vgui screen basis is strange (z is front, y is up, x is
    // right) we need to rotate the typical basis before applying it
...
void CSlideshowDisplay::InputDisable(inputdata_t &inputdata) { Disable(); }
...
void CSlideshowDisplay::InputAddSlides(inputdata_t &inputdata) {
  ...
  for (iList = 0; iList < m_SlideKeywordList.Count(); ++iList) {
    if (Q_strcmp(m_SlideKeywordList[iList]->szSlideKeyword,
                 inputdata.value.String()) == 0)
      break;
  }
  ...
  if (iList < m_SlideKeywordList.Count()) {
    ...
    for (iNumCurrentSlideLists = 0; iNumCurrentSlideLists < 16;
         ++iNumCurrentSlideLists) {
      if (m_chCurrentSlideLists[iNumCurrentSlideLists] == (unsigned char)-1)
        break;
    }

    if (iNumCurrentSlideLists >= 16) return;
...
void CSlideshowDisplay::GetControlPanelInfo(int nPanelIndex,
                                            const char *&pPanelName) {
  pPanelName = "slideshow_display_screen";
}

void CSlideshowDisplay::GetControlPanelClassName(int nPanelIndex,
                                                 const char *&pPanelName) {
...
void CSlideshowDisplay::SpawnControlPanels() {
  ...
  for (nPanel = 0; true; ++nPanel) {
    ...
    if (!pScreenName) continue;

    const char *pScreenClassname;
    GetControlPanelClassName(nPanel, pScreenClassname);
    if (!pScreenClassname) continue;

    float flWidth = m_iScreenWidth;
    float flHeight = m_iScreenHeight;

    CVGuiScreen *pScreen =
        CreateVGuiScreen(pScreenClassname, pScreenName, this, this, -1);
...
void CSlideshowDisplay::RestoreControlPanels(void) {
  ...
  for (nPanel = 0; true; ++nPanel) {
    ...
    CVGuiScreen *pScreen =
        (CVGuiScreen *)gEntList.FindEntityByClassname(NULL, pScreenClassname);

    while ((pScreen && pScreen->GetOwnerEntity() != this) ||
           Q_strcmp(pScreen->GetPanelName(), pScreenName) != 0) {
      pScreen = (CVGuiScreen *)gEntList.FindEntityByClassname(pScreen,
                                                              pScreenClassname);
    }
...
void CSlideshowDisplay::BuildSlideShowImagesList(void) {
  ...
  if (IsX360()) {
    Q_snprintf(szDirectory, sizeof(szDirectory), "materials/vgui/%s/slides.txt",
               m_szSlideshowDirectory.Get());
    ...
    Q_snprintf(szDirectory, sizeof(szDirectory), "materials/vgui/%s/*.vmt",
               m_szSlideshowDirectory.Get());
    const char *pMatFileName =
        g_pFullFileSystem->FindFirst(szDirectory, &matHandle);
  ...
  while (szMatFileName[0]) {
    ...
    bool bLoaded =
        pMaterialKeys->LoadFromFile(g_pFullFileSystem, szFullFileName, NULL);
    ...
    if (bLoaded) {
      ...
      while (pchKeyword[0] != '\0') {
        ...
        if (pNextKeyword[0] != '\0') {
          ...
          while (pNextKeyword[0] != '\0' &&
                 (pNextKeyword[0] == ',' || pNextKeyword[0] == ' '))
            ++pNextKeyword;
        }
        ...
        int iList;
        for (iList = 0; iList < m_SlideKeywordList.Count(); ++iList) {
          if (Q_strcmp(m_SlideKeywordList[iList]->szSlideKeyword, pchKeyword) ==
              0)
            break;
        }
    ...
    int iList;
    for (iList = 0; iList < m_SlideKeywordList.Count(); ++iList) {
      if (Q_strcmp(m_SlideKeywordList[iList]->szSlideKeyword, "") == 0) break;
    }
