...
BEGIN_DATADESC(CBaseNPCMaker)
  ...
  DEFINE_KEYFIELD(m_nMaxNumNPCs, FIELD_INTEGER, "MaxNPCCount"),
      ...
      DEFINE_INPUTFUNC(FIELD_INTEGER, "SetMaxLiveChildren",
                       InputSetMaxLiveChildren),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetSpawnFrequency",
...
bool CBaseNPCMaker::HumanHullFits(const Vector &vecLocation) {
  trace_t tr;
  UTIL_TraceHull(vecLocation, vecLocation + Vector(0, 0, 1),
                 NAI_Hull::Mins(HULL_HUMAN), NAI_Hull::Maxs(HULL_HUMAN),
                 MASK_NPCSOLID, m_hIgnoreEntity, COLLISION_GROUP_NONE, &tr);
...
bool CBaseNPCMaker::CanMakeNPC(bool bIgnoreSolidEntities) {
  if (ai_inhibit_spawners.GetBool()) return false;

  if (m_nMaxLiveChildren > 0 &&
      m_nLiveChildren >=
          m_nMaxLiveChildren) {  // not allowed to make a new one yet. Too many
                                 // live ones out right now.
    return false;
  }
    ...
    if (count) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if ((pList[i]->GetSolidFlags() & FSOLID_NOT_SOLID) == false) {
          // Since the outer method doesn't work well around striders on account
          // of their huge bounding box. Find the ground under me and see if a
          // human hull would fit there.
          trace_t tr;
          UTIL_TraceHull(GetAbsOrigin() + Vector(0, 0, 2),
                         GetAbsOrigin() - Vector(0, 0, 8192),
                         NAI_Hull::Mins(HULL_HUMAN), NAI_Hull::Maxs(HULL_HUMAN),
                         MASK_NPCSOLID, m_hIgnoreEntity, COLLISION_GROUP_NONE,
                         &tr);
...
bool CBaseNPCMaker::IsDepleted() {
  if ((m_spawnflags & SF_NPCMAKER_INF_CHILD) || m_nMaxNumNPCs > 0) return false;
...
//-----------------------------------------------------------------------------
void CBaseNPCMaker::InputEnable(inputdata_t &inputdata) { Enable(); }

//-----------------------------------------------------------------------------
// Purpose: Input hander that stops the spawner
//-----------------------------------------------------------------------------
void CBaseNPCMaker::InputDisable(inputdata_t &inputdata) { Disable(); }

//-----------------------------------------------------------------------------
// Purpose: Input hander that toggles the spawner
//-----------------------------------------------------------------------------
void CBaseNPCMaker::InputToggle(inputdata_t &inputdata) { Toggle(); }
...
// Constructor
//-----------------------------------------------------------------------------
CNPCMaker::CNPCMaker(void) { m_spawnEquipment = NULL_STRING; }
...
void CNPCMaker::MakeNPC(void) {
  if (!CanMakeNPC()) return;

  CAI_BaseNPC *pent =
      (CAI_BaseNPC *)CreateEntityByName(STRING(m_iszNPCClassname));
  ...
  // Strip pitch and roll from the spawner's angles. Pass only yaw to the
  // spawned NPC.
  ...
  if (m_ChildTargetName != NULL_STRING) {
    // if I have a netname (overloaded), give the child NPC that name as a
    // targetname
...
void CBaseNPCMaker::ChildPostSpawn(CAI_BaseNPC *pChild) {
  ...
  while (bFound) {
    trace_t tr;
    UTIL_TraceHull(pChild->GetAbsOrigin(), pChild->GetAbsOrigin(),
                   pChild->WorldAlignMins(), pChild->WorldAlignMaxs(),
                   MASK_NPCSOLID, pChild, COLLISION_GROUP_NONE, &tr);
    // NDebugOverlay::Box( pChild->GetAbsOrigin(), pChild->WorldAlignMins(),
    // pChild->WorldAlignMaxs(), 0, 255, 0, 32, 5.0 );
...
void CBaseNPCMaker::DeathNotice(CBaseEntity *pVictim) {
  // ok, we've gotten the deathnotice from our child, now clear out its owner if
  // we don't want it to fade.
  m_nLiveChildren--;

  // If we're here, we're getting erroneous death messages from children we
  // haven't created
  AssertMsg(
      m_nLiveChildren >= 0,
      "npc_maker receiving child death notice but thinks has no children\n");
...
//-----------------------------------------------------------------------------
// Purpose: Creates new NPCs from a template NPC. The template NPC must be
// marked
...
BEGIN_DATADESC(CTemplateNPCMaker)
  ...
  DEFINE_KEYFIELD(m_iszTemplateName, FIELD_STRING, "TemplateName"),
      ...
      DEFINE_KEYFIELD(m_CriterionVisibility, FIELD_INTEGER,
                      "CriterionVisibility"),
      ...
      DEFINE_INPUTFUNC(FIELD_INTEGER, "SetMinimumSpawnDistance",
...
void CTemplateNPCMaker::Precache() {
  ...
  // If the mapper marked this as "preload", then instance the entity preache
  // stuff and delete the entity
...
void CTemplateNPCMaker::MakeNPC(void) {
  ...
  if (!CanMakeNPC((m_iszDestinationGroup != NULL_STRING))) return;
  ...
  if (m_iszDestinationGroup != NULL_STRING) {
    ...
    if (!pDestination) {
      DevMsg(2,
             "%s '%s' failed to find a valid spawnpoint in destination group: "
             "'%s'\n",
             GetClassname(), STRING(GetEntityName()),
             STRING(m_iszDestinationGroup));
      return;
    }
  }
...
bool CTemplateNPCMaker::PlaceNPCInLine(CAI_BaseNPC *pNPC) {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 8192), MASK_SHOT,
  ...
  for (int i = 0; i < 10; i++) {
    UTIL_TraceHull(vecPlace, vecPlace + Vector(0, 0, 10), pNPC->GetHullMins(),
                   pNPC->GetHullMaxs(), MASK_SHOT, pNPC, COLLISION_GROUP_NONE,
                   &tr);
...
bool CTemplateNPCMaker::PlaceNPCInRadius(CAI_BaseNPC *pNPC) {
  ...
  if (CAI_BaseNPC::FindSpotForNPCInRadius(&vPos, GetAbsOrigin(), pNPC,
                                          m_flRadius)) {
