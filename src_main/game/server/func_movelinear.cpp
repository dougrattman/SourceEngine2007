...
LINK_ENTITY_TO_CLASS(momentary_door,
                     CFuncMoveLinear);  // For backward compatibility

//
// func_water_analog is implemented as a linear mover so we can raise/lower the
// water level.
//
...
void CFuncMoveLinear::Spawn(void) {
  ...
  m_vecPosition1 =
      GetAbsOrigin() - (m_vecMoveDir * m_flMoveDistance * m_flStartPosition);
  ...
  if (!FClassnameIs(this, "func_water_analog") &&
      FBitSet(m_spawnflags.operator const int &(), SF_MOVELINEAR_NOTSOLID)) {
...
// Purpose:
//------------------------------------------------------------------------------
void CFuncMoveLinear::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CFuncMoveLinear::SetPosition(float flPosition) {
  Vector vTargetPos =
      m_vecPosition1 + (flPosition * (m_vecPosition2 - m_vecPosition1));
...
void CFuncMoveLinear::InputSetSpeed(inputdata_t &inputdata) {
  ...
  if (flDistToGoalSqr > Square(FLT_EPSILON)) {
    // NOTE: We do NOT want to call sound functions here, just vanilla position
    // changes
...
int CFuncMoveLinear::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Current Pos: %3.3f",
               flCurDist / flTravelDist);
    EntityText(text_offset, tempstr, 0);
    text_offset++;

    float flTargetDist = (m_vecPosition1 - m_vecFinalDest).Length();
    Q_snprintf(tempstr, sizeof(tempstr), "Target Pos: %3.3f",
               flTargetDist / flTravelDist);
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
  return text_offset;
