...
void CAI_Motor::Init(IAI_MovementSink *pMovementServices) {
  ...
  m_pMoveProbe = GetOuter()->GetMoveProbe();  // @TODO (toml 03-30-03): this is
                                              // a "bad" way to grab this
                                              // pointer. Components should have
                                              // an explcit "init" phase.
}

//-----------------------------------------------------------------------------
// Step iteratively toward a destination position
//-----------------------------------------------------------------------------
AIMotorMoveResult_t CAI_Motor::MoveGroundStep(const Vector &newPos,
                                              CBaseEntity *pMoveTarget,
                                              float yaw, bool bAsFarAsCan,
                                              bool bTestZ,
  ...
  if (!bTestZ) testFlags |= AITGM_2D;

#ifdef DEBUG
  if (ai_draw_motor_movement.GetBool()) testFlags |= AITGM_DRAW_RESULTS;
#endif

  GetMoveProbe()->TestGroundMove(GetLocalOrigin(), newPos, MASK_NPCSOLID,
                                 testFlags, &moveTrace);
  if (pTraceResult) {
  ...
  bool bHitTarget =
      (moveTrace.pObstruction && (pMoveTarget == moveTrace.pObstruction));
  ...
  if (!bIsBlocked || bAsFarAsCan || bHitTarget) {
#ifdef DEBUG
    if (GetMoveProbe()->CheckStandPosition(GetLocalOrigin(), MASK_NPCSOLID) &&
    ...
    // NOTE: This is to detect changes in ground entity as the movement code has
    // optimized out ground checks.  So now we have to do a simple recheck to
    // make sure we detect when we've stepped onto a new entity.
    ...
    if (moveTrace.flStepUpDistance > 0.1f) {
      ...
      if (pPhysicsObject) {
        IPhysicsShadowController *pShadow =
            pPhysicsObject->GetShadowController();
        if (pShadow) {
    ...
    if (bHitTarget) return AIM_PARTIAL_HIT_TARGET;

    if (!bIsBlocked) return AIM_SUCCESS;

    if (moveTrace.fStatus == AIMR_BLOCKED_NPC) return AIM_PARTIAL_HIT_NPC;
...
void CAI_Motor::MoveClimbStart(const Vector &climbDest, const Vector &climbDir,
...
AIMoveResult_t CAI_Motor::MoveClimbExecute(const Vector &climbDest,
  ...
  if (fabsf(climbDir.z) > .1) {
    if (GetActivity() != ACT_CLIMB_DISMOUNT) {
      Activity desiredActivity =
          (climbDir.z > -0.01) ? ACT_CLIMB_UP : ACT_CLIMB_DOWN;
    ...
    if (m_nDismountSequence != ACT_INVALID) {
      if (GetActivity() == ACT_CLIMB_UP) {
        if (climbNodesLeft <= 2 && climbDist < fabs(m_vecDismount.z)) {
          ...
          GetOuter()->SetCycle(
              GetOuter()->GetMovementFrame(m_vecDismount.z - climbDist));
        }
      }
    }
  }
...
int CAI_Motor::MoveJumpExecute() {
  ...
  if (GetOuter()->GetActivity() == ACT_JUMP &&
      GetOuter()->IsActivityFinished()) {
...
float CAI_Motor::GetIdealSpeed() const { return GetOuter()->GetIdealSpeed(); }

float CAI_Motor::GetIdealAccel() const { return GetOuter()->GetIdealAccel(); }
...
float CAI_Motor::MinStoppingDist(float flMinResult) {
  ...
  if (flDecelRate > 0.0) {
    ...
    if (flDist > flMinResult) return flDist;
    return flMinResult;
  }
  return flMinResult;
}
...
AIMotorMoveResult_t CAI_Motor::MoveGroundExecute(const AILocalMoveGoal_t &move,
                                                 AIMoveTrace_t *pTraceResult) {
  // --------------------------------------------
  ...
  // --------------------------------------------
  return MoveGroundExecuteWalk(move, GetIdealSpeed(), CalcIntervalMove(),
                               pTraceResult);
}

AIMotorMoveResult_t CAI_Motor::MoveGroundExecuteWalk(
    const AILocalMoveGoal_t &move, float speed, float dist,
    AIMoveTrace_t *pTraceResult) {
  ...
  if (dist > 0.0) {
    ...
    result = MoveGroundStep(vecTo, move.pMoveTarget, -1, true,
                            bReachingLocalGoal, pTraceResult);

    if (result == AIM_FAILED) MoveStop();
...
void CAI_Motor::MoveFacing(const AILocalMoveGoal_t &move) {
  if (GetOuter()->OverrideMoveFacing(move, GetMoveInterval())) return;
...
void CAI_Motor::RecalculateYawSpeed() { SetYawSpeed(CalcYawSpeed()); }
...
void CAI_Motor::UpdateYaw(int yawSpeed) {
  ...
  if (yawSpeed == -1) yawSpeed = GetYawSpeed();

  // NOTE: GetIdealYaw() will never exactly be reached because UTIL_AngleMod
  // also truncates the angle to 16 bits of resolution. So lets truncate it
  // here.
...
void CAI_Motor::SetIdealYawToTargetAndUpdate(const Vector &target,
                                             float yawSpeed) {
...
//-----------------------------------------------------------------------------
void CAI_Motor::AddFacingTarget(CBaseEntity *pTarget, float flImportance,
                                float flDuration, float flRamp) {
  m_facingQueue.Add(pTarget, flImportance, flDuration, flRamp);
}

void CAI_Motor::AddFacingTarget(const Vector &vecPosition, float flImportance,
                                float flDuration, float flRamp) {
  m_facingQueue.Add(vecPosition, flImportance, flDuration, flRamp);
}

void CAI_Motor::AddFacingTarget(CBaseEntity *pTarget, const Vector &vecPosition,
...
float CAI_Motor::GetFacingDirection(Vector &vecDir) {
  ...
  for (i = 0; i < m_facingQueue.Count(); i++) {
    ...
    // NDebugOverlay::Line( m_facingQueue[i].GetPosition(), GetAbsOrigin(), 255,
    // 0, 0, false, 0.1 );
...
AIMoveResult_t CAI_Motor::MoveNormalExecute(const AILocalMoveGoal_t &move) {
  ...
  if (result != AIMR_OK) {
    ...
    SetMoveInterval(0);  // always consume interval on failure, even if
                         // overridden by OnMoveExecuteFailed()
  }
...
float CAI_Motor::StepHeight() const { return GetOuter()->StepHeight(); }
...
float CAI_Motor::GetPlaybackRate() { return GetOuter()->GetPlaybackRate(); }
