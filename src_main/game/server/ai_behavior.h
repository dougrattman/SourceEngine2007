...
// selecting schedules and running tasks
//
...
class CAI_BehaviorHost : public BASE_NPC, private IBehaviorBackBridge {
 public:
  ...
  virtual void ForceSelectedGoRandom();
 ...
 private:
  ...
  CBaseEntity *BackBridge_BestEnemy();
  bool BackBridge_IsValidCover(const Vector &vLocation, CAI_Hint const *pHint);
  bool BackBridge_IsValidShootPosition(const Vector &vLocation, CAI_Node *pNode,
                                       CAI_Hint const *pHint);
  ...
  bool BackBridge_OnCalcBaseMove(AILocalMoveGoal_t *pMoveGoal, float distClear,
                                 AIMoveResult_t *pResult);
...
inline int CAI_BehaviorHost<BASE_NPC>::SelectFailSchedule(
  ...
  m_bCalledBehaviorSelectSchedule = true;
  int result = 0;
  if (m_pCurBehavior && m_pCurBehavior->BridgeSelectFailSchedule(
                            failedSchedule, failedTask, taskFailCode, &result))
    return result;
...
inline float CAI_BehaviorHost<BASE_NPC>::GetDefaultNavGoalTolerance() {
  if (m_pCurBehavior) return m_pCurBehavior->BridgeGetDefaultNavGoalTolerance();
...
inline bool CAI_BehaviorHost<BASE_NPC>::IsCrouchedActivity(Activity activity) {
  if (m_pCurBehavior) return m_pCurBehavior->BridgeIsCrouchedActivity(activity);
