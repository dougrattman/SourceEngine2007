...
void Pickup_OnPhysGunDrop(CBaseEntity *pDroppedObject, CBasePlayer *pPlayer,
  ...
  IPlayerPickupVPhysics *pPickup =
      dynamic_cast<IPlayerPickupVPhysics *>(pDroppedObject);
  if (pPickup) {
...
void Pickup_OnPhysGunPickup(CBaseEntity *pPickedUpObject, CBasePlayer *pPlayer,
  ...
  IPlayerPickupVPhysics *pPickup =
      dynamic_cast<IPlayerPickupVPhysics *>(pPickedUpObject);
  if (pPickup) {
...
bool Pickup_OnAttemptPhysGunPickup(CBaseEntity *pPickedUpObject,
                                   CBasePlayer *pPlayer,
                                   PhysGunPickup_t reason) {
  IPlayerPickupVPhysics *pPickup =
      dynamic_cast<IPlayerPickupVPhysics *>(pPickedUpObject);
...
bool Pickup_GetPreferredCarryAngles(CBaseEntity *pObject, CBasePlayer *pPlayer,
  ...
  IPlayerPickupVPhysics *pPickup =
      dynamic_cast<IPlayerPickupVPhysics *>(pObject);
  if (pPickup) {
    if (pPickup->HasPreferredCarryAnglesForPlayer(pPlayer)) {
      outputAnglesWorldSpace = TransformAnglesToWorldSpace(
          pPickup->PreferredCarryAngles(), localToWorld);
      return true;
    }
  }
  return false;
...
Vector Pickup_DefaultPhysGunLaunchVelocity(const Vector &vecForward,
  ...
  float mass = flMass;
  if (mass > 100) {
    mass = std::min(mass, 1000.0f);
    float flForceMin = physcannon_minforce.GetFloat();
    flForce =
        SimpleSplineRemapValClamped(mass, 100, 600, flForceMax, flForceMin);
  }
...
Vector Pickup_PhysGunLaunchVelocity(CBaseEntity *pObject,
  ...
  IPlayerPickupVPhysics *pPickup =
      dynamic_cast<IPlayerPickupVPhysics *>(pObject);
  if (pPickup != NULL && pPickup->ShouldPuntUseLaunchForces(reason))
    return pPickup->PhysGunLaunchVelocity(vecForward,
                                          pPhysicsObject->GetMass());

  // Do our default behavior
  return Pickup_DefaultPhysGunLaunchVelocity(vecForward,
                                             pPhysicsObject->GetMass());
}
