...
#define SF_BRUSH_ACCDCC \
  16  // brush should accelerate and decelerate when toggled
#define SF_BRUSH_HURT \
  32  // rotating brush that inflicts pain based on rotation speed
#define SF_ROTATING_NOT_SOLID \
  64  // some special rotating objects are not solid.
...
void CFuncWall::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CFuncWallToggle::Spawn(void) {
  BaseClass::Spawn();
  if (HasSpawnFlags(SF_WALL_START_OFF)) TurnOff();
...
bool CFuncWallToggle::IsOn(void) {
  if (IsSolidFlagSet(FSOLID_NOT_SOLID)) return false;
  return true;
}
...
void CFuncWallToggle::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                          USE_TYPE useType, float value) {
  int status = IsOn();
...
void CFuncConveyor::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                        USE_TYPE useType, float value) {
  m_flSpeed = -m_flSpeed;
  UpdateSpeed(m_flSpeed);
}
...
// =================== FUNC_ILLUSIONARY
// ============================================== A simple entity that looks
// solid but lets you walk through it.
...
//
//			It will rotate around the Z axis by default. Spawnflags can
//be set 			to make it rotate around the X or Y axes.
//
//			The direction of rotation is also controlled by a
//spawnflag.
//-----------------------------------------------------------------------------
class CFuncRotating : public CBaseEntity {
 ...
 public:
  ...
  void RotatingUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                   USE_TYPE useType, float value);
 ...
 protected:
  ...
  float m_flVolume;
  float m_flTargetSpeed;  // Target value for m_flSpeed, used for spinning up
                          // and down.
  float m_flMaxSpeed;     // Maximum value for m_flSpeed, used for ramping sound
                          // effects.
...
BEGIN_DATADESC(CFuncRotating)
  ...
  DEFINE_FIELD(m_vecMoveAng, FIELD_VECTOR),
      ...
      // Function Pointers
      DEFINE_FUNCTION(SpinUpMove), DEFINE_FUNCTION(SpinDownMove),
      DEFINE_FUNCTION(HurtTouch), DEFINE_FUNCTION(RotatingUse),
      DEFINE_FUNCTION(RotateMove), DEFINE_FUNCTION(ReverseMove),
...
extern void SendProxy_Angles(const SendProp *pProp, const void *pStruct,
                             const void *pData, DVariant *pOut, int iElement,
                             int objectID);
void SendProxy_FuncRotatingAngles(const SendProp *pProp, const void *pStruct,
                                  const void *pData, DVariant *pOut,
                                  int iElement, int objectID) {
  CFuncRotating *entity = (CFuncRotating *)pStruct;
  Assert(entity);
...
extern void SendProxy_SimulationTime(const SendProp *pProp, const void *pStruct,
                                     const void *pVarData, DVariant *pOut,
                                     int iElement, int objectID);
void SendProxy_FuncRotatingSimulationTime(const SendProp *pProp,
                                          const void *pStruct,
                                          const void *pVarData, DVariant *pOut,
                                          int iElement, int objectID) {
  CFuncRotating *entity = (CFuncRotating *)pStruct;
...
SendPropExclude("DT_BaseEntity", "m_angRotation"),
    ...
    SendPropVector(SENDINFO(m_vecOrigin), -1, SPROP_COORD | SPROP_CHANGES_OFTEN,
                   0.0f, HIGH_DEFAULT, SendProxy_FuncRotatingOrigin),
    SendPropQAngles(SENDINFO(m_angRotation), 13, SPROP_CHANGES_OFTEN,
                    SendProxy_FuncRotatingAngles),
    SendPropInt(SENDINFO(m_flSimulationTime), SIMULATION_TIME_WINDOW_BITS,
                ...
                SendProxy_FuncRotatingSimulationTime),
    END_SEND_TABLE()
  ...
  if (FStrEq(szKeyName, "fanfriction")) {
    ...
    m_flVolume = std::clamp(m_flVolume, 0.0f, 1.0f);
  } else {
...
void CFuncRotating::Spawn() {
  ...
  if (HasSpawnFlags(SF_BRUSH_ROTATE_START_ON)) {
    ...
    SetNextThink(gpGlobals->curtime +
                 .2);  // leave a magic delay for client to start up
  }
...
void CFuncRotating::RampPitchVol(void) {
  ...
  float fvol = std::clamp(m_flVolume * fpct, 0.0f,
                          1.0f);  // slowdown volume ramps down to 0

  float fpitch = FANPITCHMIN + (FANPITCHMAX - FANPITCHMIN) * fpct;

  int pitch = std::clamp(fpitch, 0.0f, 255.0f);
  if (pitch == PITCH_NORM) {
    pitch = PITCH_NORM - 1;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Sets the current speed to the given value and manages the sound
// effects. Input  : flNewSpeed - New speed in degrees per second.
//-----------------------------------------------------------------------------
void CFuncRotating::UpdateSpeed(float flNewSpeed) {
  float flOldSpeed = m_flSpeed;
  m_flSpeed = std::clamp(flNewSpeed, -m_flMaxSpeed, m_flMaxSpeed);
  ...
  if (m_bStopAtStartPos) {
    ...
    float angDelta =
        anglemod(GetLocalAngles()[checkAxis] - m_angStart[checkAxis]);
    if (angDelta > 180.0f) {
      angDelta -= 360.0f;
    }
...
//-----------------------------------------------------------------------------
// Purpose: Think function. Accelerates a func_rotating to a higher angular
// velocity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Think function. Decelerates a func_rotating to a lower angular
// velocity.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Think function. Called while rotating at a constant angular
// velocity.
//-----------------------------------------------------------------------------
void CFuncRotating::RotateMove(void) {
  ...
  if (m_bStopAtStartPos) {
    ...
    float angDelta =
        anglemod(GetLocalAngles()[checkAxis] - m_angStart[checkAxis]);
    if (angDelta > 180.0f) angDelta -= 360.0f;
...
//-----------------------------------------------------------------------------
// Purpose: Used for debug output. Returns the given speed considering our
// current
//			direction of rotation, so that positive values are forward
...
//			value -
//-----------------------------------------------------------------------------
void CFuncRotating::RotatingUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                                USE_TYPE useType, float value) {
  //
...
void CFuncRotating::InputSetSpeed(inputdata_t &inputdata) {
  ...
  flSpeed = fabs(flSpeed);
  SetTargetSpeed(std::clamp(flSpeed, 0.0f, 1.0f) * m_flMaxSpeed);
}
...
int CFuncRotating::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "Speed cur (target): %3.2f (%3.2f)",
               GetMoveSpeed(m_flSpeed), GetMoveSpeed(m_flTargetSpeed));
...
bool CFuncVPhysicsClip::EntityPassesFilter(CBaseEntity *pOther) {
  ...
  if (pFilter) return pFilter->PassesFilter(this, pOther);
