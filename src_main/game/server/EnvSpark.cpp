...
//			location -
//-----------------------------------------------------------------------------
void DoSpark(CBaseEntity *ent, const Vector &location, int nMagnitude,
             int nTrailLength, bool bPlaySound, const Vector &vecDir) {
...
void CEnvSpark::Spawn(void) {
  ...
  if (FBitSet(m_spawnflags.operator const int &(), SF_SPARK_START_ON)) {
...
void CEnvSpark::SparkThink(void) {
  ...
  if (FBitSet(m_spawnflags.operator const int &(), SF_SPARK_DIRECTIONAL)) {
    AngleVectors(GetAbsAngles(), &vecDir);
  }

  DoSpark(this, WorldSpaceCenter(), m_nMagnitude, m_nTrailLength,
          !(m_spawnflags.operator const int &() & SF_SPARK_SILENT), vecDir);

  m_OnSpark.FireOutput(this, this);

  if (FBitSet(m_spawnflags.operator const int &(), SF_SPARK_GLOW)) {
    CPVSFilter filter(GetAbsOrigin());
    te->GlowSprite(filter, 0.0, &GetAbsOrigin(), m_nGlowSpriteIndex, 0.2, 1.5,
                   25);
  }
}
...
// Purpose: Input handler for starting the sparks.
//-----------------------------------------------------------------------------
void CEnvSpark::InputStopSpark(inputdata_t &inputdata) { SetThink(NULL); }
