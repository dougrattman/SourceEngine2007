...
#define CREATE_ENTITY(newClass, className) \
  _CreateEntity((newClass *)NULL, className)
#define CREATE_UNSAVED_ENTITY(newClass, className) \
...
abstract_class IEntityFactoryDictionary {
 public:
  ...
  virtual void Destroy(const char *pClassName,
                       IServerNetworkable *pNetworkable) = 0;
...
inline bool CanCreateEntityClass(const char *pszClassname) {
  return (EntityFactoryDictionary() != NULL &&
...
class CEntityFactory : public IEntityFactory {
 public:
  ...
  virtual size_t GetEntitySize() { return sizeof(T); }
};
...
//
inline int ENTINDEX(edict_t *pEdict) { return engine->IndexOfEdict(pEdict); }
...
#define VIEW_FIELD_WIDE \
  (float)-0.7  // +-135 degrees 0.1 // +-85 degrees, used for full FOV checks
#define VIEW_FIELD_NARROW \
  (float)0.7  // +-45 degrees, more narrow check used to set up ranged attacks
#define VIEW_FIELD_ULTRA_NARROW \
  (float)0.9  // +-25 degrees, more narrow check used to set up ranged attacks
...
void UTIL_SetTrace(trace_t &tr, const Ray_t &ray, edict_t *edict,
...
// NOTENOTE: Use UTIL_GetLocalPlayer instead of UTIL_PlayerByIndex IF you're in
// single player and you want the player.
...
// Pass in an array of pointers and an array size, it fills the array and
...
inline int UTIL_EntitiesInBox(CBaseEntity **pList, int listMax,
...
inline int UTIL_EntitiesAlongRay(CBaseEntity **pList, int listMax,
                                 const Ray_t &ray, int flagMask) {
  CFlaggedEntitiesEnum rayEnum(pList, listMax, flagMask);
  return UTIL_EntitiesAlongRay(ray, &rayEnum);
}

inline int UTIL_EntitiesInSphere(CBaseEntity **pList, int listMax,
...
// deletes an entity, without any delay.  Only use this when sure no pointers
// rely on this entity.
...
void UTIL_ScreenFadeAll(const color32 &color, float fadeTime, float holdTime,
                        int flags);
...
#define ASSERT(f) \
  DBG_AssertFunction((bool)((f) != 0), #f, __FILE__, __LINE__, NULL)
#define ASSERTSZ(f, sz) \
  DBG_AssertFunction((bool)((f) != 0), #f, __FILE__, __LINE__, sz)
#else  // !DEBUG
#define ASSERT(f)
#define ASSERTSZ(f, sz)
#endif  // !DEBUG
...
class EntityMatrix : public VMatrix {
 public:
  ...
  inline Vector LocalToWorld(const Vector &vVec) const { return VMul4x3(vVec); }
