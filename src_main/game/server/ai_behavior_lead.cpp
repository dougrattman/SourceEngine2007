...
BEGIN_SIMPLE_DATADESC(AI_LeadArgs_t)
  ...
  DEFINE_FIELD(flags, FIELD_INTEGER),
      ...
      // DEFINE_FIELD(		bDontSpeakStart,
      // FIELD_BOOLEAN ), DEFINE_FIELD(		bGagLeader,
      // FIELD_BOOLEAN ),
...
BEGIN_DATADESC(CAI_LeadBehavior)
  DEFINE_EMBEDDED(m_args),
      ...
      DEFINE_EMBEDDED(m_MoveMonitor), DEFINE_EMBEDDED(m_LostTimer),
...
int CAI_LeadBehavior::DrawDebugTextOverlays(int text_offset) {
  ...
  if (GetOuter()->m_debugOverlays & OVERLAY_TEXT_BIT) {
    if (HasGoal()) {
      Q_snprintf(tempstr, sizeof(tempstr), "Goal: %s %s", m_args.pszGoal,
                 VecToString(m_goal));
...
void CAI_LeadBehavior::LeadPlayer(const AI_LeadArgs_t &leadArgs,
                                  CAI_LeadBehaviorHandler *pSink) {
#ifndef CSTRIKE_DLL
...
bool CAI_LeadBehavior::CanSelectSchedule() {
  if (!AI_GetSinglePlayer() || AI_GetSinglePlayer()->IsDead()) return false;

  bool fAttacked =
      (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE));
  bool fNonCombat =
      (GetNpcState() == NPC_STATE_IDLE || GetNpcState() == NPC_STATE_ALERT);
...
void CAI_LeadBehavior::BeginScheduleSelection() {
  ...
  if (pExpresser) pExpresser->ClearSpokeConcept(TLK_LEAD_ARRIVAL);
}
...
bool CAI_LeadBehavior::SetGoal(const AI_LeadArgs_t &args) {
  ...
  if (!pGoalEnt) return false;

  m_args = args;  // @Q (toml 08-13-02): need to copy string?
  m_goal = pGoalEnt->GetLocalOrigin();
  m_goalyaw =
      (args.flags & AILF_USE_GOAL_FACING) ? pGoalEnt->GetLocalAngles().y : -1;
  m_waitpoint = vec3_origin;
  m_waitdistance = args.flWaitDistance;
  m_leaddistance = args.flLeadDistance ? args.flLeadDistance : 64;
  m_retrievedistance = args.flRetrieveDistance
                           ? args.flRetrieveDistance
                           : (m_leaddistance + LEAD_MIN_RETRIEVEDIST_OFFSET);
  ...
  if (args.pszWaitPoint && args.pszWaitPoint[0]) {
    CBaseEntity *pWaitPoint =
        gEntList.FindEntityByName(NULL, args.pszWaitPoint);
    if (pWaitPoint) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_LeadBehavior::GetClosestPointOnRoute(const Vector &targetPos,
                                              Vector *pVecClosestPoint) {
  AI_Waypoint_t *waypoint =
      GetOuter()->GetNavigator()->GetPath()->GetCurWaypoint();
  ...
  if (!waypoint) {
    ...
    builtwaypoints = GetOuter()->GetPathfinder()->BuildRoute(
        GetOuter()->GetAbsOrigin(), m_goal, NULL,
        GetOuter()->GetDefaultNavGoalTolerance(), GetOuter()->GetNavType(),
        true);
    if (!builtwaypoints) return false;
  ...
  for (; (waypoint != NULL); waypoint = waypoint->GetNext()) {
    ...
    Vector vecClosest;
    CalcClosestPointOnLineSegment(targetPos, vecPrevPos, waypoint->GetPos(),
                                  vecClosest);
    /*
    if ( builtwaypoints )
    {
            NDebugOverlay::Line( vecPrevPos, waypoint->GetPos(),
    0,0,255,true, 10.0 );
    }
    */
    ...
    if (flPathDist2D > flNearestDist2D) continue;
...
//-----------------------------------------------------------------------------
// Purpose: Return true if the player is further ahead on the lead route than I
// am
//-----------------------------------------------------------------------------
bool CAI_LeadBehavior::PlayerIsAheadOfMe(bool bForce) {
  // Find the nearest point on our route to the player, and see if that's
  // further ahead of us than our nearest point.

  // If we're not leading, our route doesn't lead to the goal, so we can't use
  // it. If we just started leading, go ahead and test, and we'll build a temp
  // route.
  if (!m_bInitialAheadTest && !IsCurSchedule(SCHED_LEAD_PLAYER, false) &&
      !bForce)
    return false;
  ...
  if (GetClosestPointOnRoute(AI_GetSinglePlayer()->GetAbsOrigin(),
                             &vecClosestPoint)) {
...
void CAI_LeadBehavior::GatherConditions(void) {
  ...
  if (HasGoal()) {
    ...
    if ((WorldSpaceCenter() - m_goal).LengthSqr() > (64 * 64) &&
        IsCurSchedule(SCHED_LEAD_AWAIT_SUCCESS, false)) {
    ...
    if (pFollower) {
      ...
      float flFollowerDist =
          (WorldSpaceCenter() - pFollower->WorldSpaceCenter()).Length();
      ...
      // To be considered not lagging, the follower must be visible, and within
      // the lead distance
      if (GetOuter()->FVisible(pFollower) &&
          GetOuter()->GetSenses()->ShouldSeeEntity(pFollower)) {
        SetCondition(COND_LEAD_HAVE_FOLLOWER_LOS);
        m_LostLOSTimer.Stop();
      } else {
        ClearCondition(COND_LEAD_HAVE_FOLLOWER_LOS);

        // We don't have a LOS. But if we did have LOS, don't clear it until the
        // timer is up.
      ...
      // Now we want to see if the follower is lost. Being lost means being (far
      // away || out of LOS )
      // && some time has passed. Also, lagging players are considered lost if
      // the NPC's never delivered the start speech, because it means the NPC
      // should run to the player to start the lead.
      ...
      // Success right now means being stationary, close to the goal, and having
      // the player close by
      if (!(m_args.flags & AILF_NO_DEF_SUCCESS)) {
        ...
        if (bWithinZ && (GetLocalOrigin() - m_goal).Length2D() <= 64) {
          if (HasCondition(COND_LEAD_FOLLOWER_VERY_CLOSE)) {
            ...
            float flDistSqr =
                (pFollower->GetAbsOrigin() - GetLocalOrigin()).Length2DSqr();
  ...
  if (HasGoal()) {
    ...
    if (m_weaponname != NULL_STRING) {
      ...
      if (pFollower && !pFollower->Weapon_OwnsThisType(STRING(m_weaponname))) {
        ...
        if (!m_flWeaponSafetyTimeOut ||
            (m_flWeaponSafetyTimeOut > gpGlobals->curtime))
    ...
    if (HasWaitPoint() && !PlayerIsAheadOfMe(true)) {
      ...
      // If we have no wait distance, trigger as soon as the player comes in
      // view
      if (!m_waitdistance) {
      ...
      if (bKeepWaiting) return SCHED_LEAD_WAITFORPLAYER;
    ...
    if (!m_hasspokenstart) {
      if (HasCondition(COND_LEAD_HAVE_FOLLOWER_LOS) &&
          HasCondition(COND_LEAD_FOLLOWER_VERY_CLOSE))
    ...
    if (HasCondition(COND_LEAD_FOLLOWER_LAGGING)) {
      ...
      if (IsCurSchedule(SCHED_LEAD_RETRIEVE, false) ||
          IsCurSchedule(SCHED_LEAD_WAITFORPLAYERIDLE, false)) {
        Speak(TLK_LEAD_RETRIEVE);

        // If we spoke something, wait for it to finish, if the mapmakers wants
        // us to
...
int CAI_LeadBehavior::TranslateSchedule(int scheduleType) {
  bool bInCombat =
      (m_args.bLeadDuringCombat && GetOuter()->GetState() == NPC_STATE_COMBAT);
  switch (scheduleType) {
    case SCHED_LEAD_PAUSE:
      if (bInCombat) return SCHED_LEAD_PAUSE_COMBAT;
      break;
  }
...
bool CAI_LeadBehavior::IsCurTaskContinuousMove() {
  ...
  if (pCurTask && pCurTask->iTask == TASK_LEAD_MOVE_TO_RANGE) return true;
...
void CAI_LeadBehavior::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_LEAD_WALK_PATH: {
      ...
      if (m_run && (IsCurSchedule(SCHED_LEAD_WAITFORPLAYER, false) ||
                    IsCurSchedule(SCHED_LEAD_PLAYER, false) ||
                    IsCurSchedule(SCHED_LEAD_SPEAK_THEN_LEAD_PLAYER, false) ||
                    IsCurSchedule(SCHED_LEAD_RETRIEVE, false))) {
    ...
    case TASK_LEAD_PLAYER_NEEDS_WEAPON: {
      float flAvailableTime =
          GetOuter()->GetExpresser()->GetSemaphoreAvailableTime(GetOuter());
    ...
    case TASK_WAIT_FOR_SPEAK_FINISH: {
      ...
      // If we're not supposed to wait for the player, don't wait for speech to
      // finish. Instead, just wait a wee tad, and then start moving. NPC will
      // speak on the go.
  ...
  switch (pTask->iTask) {
      ...
      if (!m_hasspokenstart) {
        ...
        if (!TaskIsComplete()) {
          ...
          if (distance > m_retrievedistance) {
            ...
            if (GetOuter()->GetState() == NPC_STATE_COMBAT ||
                (!bWithinZ || distance < (m_retrievedistance * 4)) &&
                    GetOuter()->GetState() != NPC_STATE_COMBAT) {
              followActivity = ACT_RUN;
            }

            // Don't confuse move and shoot by resetting the activity every
            // think
    ...
    case TASK_LEAD_WALK_PATH: {
      ...
      if (TaskIsRunning() && IsCurSchedule(SCHED_LEAD_PLAYER, false)) {
        // If we're not speaking, and we haven't tried for a while, try to speak
        // lead idle
        if (m_flNextLeadIdle < gpGlobals->curtime && !IsSpeaking()) {
          m_flNextLeadIdle = gpGlobals->curtime + RandomFloat(10, 15);

          if (!m_args.iRetrievePlayer &&
              HasCondition(COND_LEAD_FOLLOWER_LOST) &&
...
bool CAI_LeadBehavior::Speak(AIConcept_t concept) {
  CAI_Expresser *pExpresser = GetOuter()->GetExpresser();
  if (!pExpresser) return false;

  // If the leader is gagged, don't speak any lead speech
  if (m_gagleader) return false;

  // If we haven't said the start speech, don't nag
  bool bNag =
      (FStrEq(concept, TLK_LEAD_COMINGBACK) ||
       FStrEq(concept, TLK_LEAD_CATCHUP) || FStrEq(concept, TLK_LEAD_RETRIEVE));
  if (!m_hasspokenstart && bNag) return false;

  if (hl2_episodic.GetBool()) {
    // If we're a player ally, only speak the concept if we're allowed to.
    // This allows the response rules to control it better (i.e. handles
    // respeakdelay) We ignore nag timers for this, because the response rules
    // will control refire rates.
...
bool CAI_LeadBehavior::Connect(CAI_LeadBehaviorHandler *pSink) {
  ...
  if (m_hSinkImplementor == NULL)
    DevMsg(2,
           "Note: CAI_LeadBehaviorHandler connected to a sink that isn't an "
           "entity. Manual fixup on load will be necessary\n");
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE				"
    "SCHEDULE:SCHED_LEAD_RETRIEVE_WAIT"
    "		"
    "	Interrupts"
...
DEFINE_SCHEDULE(SCHED_LEAD_SPEAK_THEN_RETRIEVE_PLAYER,

                "	Tasks"
                "		TASK_WAIT_FOR_SPEAK_FINISH		1"
                "		TASK_SET_SCHEDULE			"
                "	SCHEDULE:SCHED_LEAD_RETRIEVE"
                "		"
                "	Interrupts"
                "		COND_LEAD_FOLLOWER_VERY_CLOSE"
                "		COND_LEAD_FOLLOWER_MOVING_TOWARDS_ME"
...
DEFINE_SCHEDULE(SCHED_LEAD_RETRIEVE_WAIT,
...
DEFINE_SCHEDULE(SCHED_LEAD_PLAYER,
...
DEFINE_SCHEDULE(SCHED_LEAD_AWAIT_SUCCESS,
...
DEFINE_SCHEDULE(SCHED_LEAD_SUCCEED,
...
DEFINE_SCHEDULE(SCHED_LEAD_SPEAK_START,
...
DEFINE_SCHEDULE(SCHED_LEAD_SPEAK_THEN_LEAD_PLAYER,

                "	Tasks"
                "		TASK_STOP_MOVING			0"
                "		TASK_WAIT_FOR_SPEAK_FINISH	1"
                "		TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_LEAD_PLAYER"
                ""
                "	Interrupts"
                "		COND_LEAD_FOLLOWER_LOST"
                "		COND_LEAD_FOLLOWER_LAGGING"
...
class CAI_LeadGoal : public CAI_GoalEntity, public CAI_LeadBehaviorHandler {
 ...
 private:
  ...
  bool
      m_fArrived;  // @TODO (toml 08-16-02): move arrived tracking onto behavior
...
BEGIN_DATADESC(CAI_LeadGoal)
  ...
  DEFINE_FIELD(m_fArrived, FIELD_BOOLEAN),
      ...
      DEFINE_KEYFIELD(m_iRetrieveWaitForSpeak, FIELD_INTEGER,
                      "RetrieveWaitForSpeak"),
      DEFINE_KEYFIELD(m_iComingBackWaitForSpeak, FIELD_INTEGER,
      ...
      DEFINE_KEYFIELD(m_iszStartConceptModifier, FIELD_STRING,
                      "StartConceptModifier"),
      DEFINE_KEYFIELD(m_iszAttractPlayerConceptModifier, FIELD_STRING,
                      "AttractPlayerConceptModifier"),
      DEFINE_KEYFIELD(m_iszWaitOverConceptModifier, FIELD_STRING,
                      "WaitOverConceptModifier"),
      DEFINE_KEYFIELD(m_iszArrivalConceptModifier, FIELD_STRING,
                      "ArrivalConceptModifier"),
      DEFINE_KEYFIELD(m_iszPostArrivalConceptModifier, FIELD_STRING,
                      "PostArrivalConceptModifier"),
      DEFINE_KEYFIELD(m_iszSuccessConceptModifier, FIELD_STRING,
                      "SuccessConceptModifier"),
      DEFINE_KEYFIELD(m_iszFailureConceptModifier, FIELD_STRING,
                      "FailureConceptModifier"),
      DEFINE_KEYFIELD(m_iszRetrieveConceptModifier, FIELD_STRING,
                      "RetrieveConceptModifier"),
      DEFINE_KEYFIELD(m_iszComingBackConceptModifier, FIELD_STRING,
...
void CAI_LeadGoal::InputActivate(inputdata_t &inputdata) {
  ...
  if (!pBehavior) {
    DevMsg(
        "Lead goal entity activated for an NPC that doesn't have the lead "
        "behavior\n");
    return;
  }
#ifdef HL2_EPISODIC
  if ((m_flLeadDistance * 4) < m_flRetrieveDistance) {
    Warning(
        "ai_goal_lead '%s': lead distance (%.2f) * 4 is < retrieve distance "
        "(%.2f). This will make the NPC act stupid. Either reduce the retrieve "
        "distance, or increase the lead distance.\n",
        GetDebugName(), m_flLeadDistance, m_flRetrieveDistance);
  }
#endif

  if (m_flRetrieveDistance <
      (m_flLeadDistance + LEAD_MIN_RETRIEVEDIST_OFFSET)) {
#ifdef HL2_EPISODIC
    Warning(
        "ai_goal_lead '%s': retrieve distance (%.2f) < lead distance (%.2f) + "
        "%d. Retrieve distance should be at least %d greater than the lead "
        "distance, or NPC will ping-pong while retrieving.\n",
        GetDebugName(), m_flRetrieveDistance, m_flLeadDistance,
        LEAD_MIN_RETRIEVEDIST_OFFSET, LEAD_MIN_RETRIEVEDIST_OFFSET);
#endif
...
void CAI_LeadGoal::OnEvent(int event) {
  ...
  if (event == LBE_ARRIVAL) m_fArrived = true;

  if (pOutputEvent) pOutputEvent->FireOutput(this, this);
}

//-----------------------------------------------------------------------------

const char *CAI_LeadGoal::GetConceptModifiers(const char *pszConcept) {
  if (m_iszStartConceptModifier != NULL_STRING &&
      *STRING(m_iszStartConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_START) == 0)
    return STRING(m_iszStartConceptModifier);

  if (m_iszAttractPlayerConceptModifier != NULL_STRING &&
      *STRING(m_iszAttractPlayerConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_ATTRACTPLAYER) == 0)
    return STRING(m_iszAttractPlayerConceptModifier);

  if (m_iszWaitOverConceptModifier != NULL_STRING &&
      *STRING(m_iszWaitOverConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_WAITOVER) == 0)
    return STRING(m_iszWaitOverConceptModifier);

  if (m_iszArrivalConceptModifier != NULL_STRING &&
      *STRING(m_iszArrivalConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_ARRIVAL) == 0)
    return STRING(m_iszArrivalConceptModifier);

  if (m_iszSuccessConceptModifier != NULL_STRING &&
      *STRING(m_iszSuccessConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_SUCCESS) == 0)
    return STRING(m_iszSuccessConceptModifier);

  if (m_iszFailureConceptModifier != NULL_STRING &&
      *STRING(m_iszFailureConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_FAILURE) == 0)
    return STRING(m_iszFailureConceptModifier);

  if (m_iszRetrieveConceptModifier != NULL_STRING &&
      *STRING(m_iszRetrieveConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_RETRIEVE) == 0)
    return STRING(m_iszRetrieveConceptModifier);

  if (m_iszComingBackConceptModifier != NULL_STRING &&
      *STRING(m_iszComingBackConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_COMINGBACK) == 0)
    return STRING(m_iszComingBackConceptModifier);

  if (m_fArrived && m_iszPostArrivalConceptModifier != NULL_STRING &&
      *STRING(m_iszPostArrivalConceptModifier))
...
BEGIN_DATADESC(CAI_LeadGoal_Weapon)
  ...
  DEFINE_KEYFIELD(m_iszWeaponName, FIELD_STRING, "WeaponName"),
      DEFINE_KEYFIELD(m_iszMissingWeaponConceptModifier, FIELD_STRING,
...
const char *CAI_LeadGoal_Weapon::GetConceptModifiers(const char *pszConcept) {
  if (m_iszMissingWeaponConceptModifier != NULL_STRING &&
      *STRING(m_iszMissingWeaponConceptModifier) &&
      strcmp(pszConcept, TLK_LEAD_MISSINGWEAPON) == 0)
