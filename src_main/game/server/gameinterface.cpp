...
void DrawAllDebugOverlays(void) {
  ...
  if (sv_massreport.GetInt()) {
    ...
    for (; pInfo; pInfo = pInfo->m_pNext) {
      CBaseEntity *ent = (CBaseEntity *)pInfo->m_pEntity;
      if (!ent->VPhysicsGetObject()) continue;
...
bool CServerGameDLL::DLLInit(CreateInterfaceFn appSystemFactory,
  ...
  if (!soundemitterbase->Connect(appSystemFactory)) return false;
...
// Ignoring this for now, server ops are abusing it
  ...
  if (CommandLine()->CheckParm("-tickrate")) {
    ...
    if (tickrate > 10) tickinterval = 1.0f / tickrate;
  }
#endif
...
void CServerGameDLL::PreClientUpdate(bool simulating) {
  ...
  */
  ...
  if (sv_showhitboxes.GetInt() == -1) return;
...
// descriptions fieldCount - the size of the array (number of field
// descriptions)
//-----------------------------------------------------------------------------
...
bool CServerGameDLL::GetUserMessageInfo(int msg_type, char *name,
  ...
  if (!usermessages->IsValidIndex(msg_type)) return false;
...
bool CServerGameDLL::ShouldHideServer(void) {
  if (g_pcv_commentary && g_pcv_commentary->GetBool()) return true;

  if (gpGlobals->eLoadType == MapLoad_Background) return true;
...
void CServerGameDLL::LoadMessageOfTheDay() {
  ...
  if (hFile == FILESYSTEM_INVALID_HANDLE) return;
  ...
  g_pStringTableInfoPanel->AddString(CBaseEntity::IsServer(), "motd",
                                     length + 1, data);
#endif
}
...
void UpdateRichPresence(void) {
  ...
  uint32_t iGameID, iChapterIndex, iChapterID, iGamePresenceID;
  ...
  if (Q_stristr(modDir, "hl2")) {
    ...
    Warning(
        "UpdateRichPresence failed in GameInterface. Didn't recognize -game "
        "parameter.");
  }

  if (iChapterID < 0) iChapterID = 0;
}
...
int GetParticleSystemIndex(const char *pParticleSystemName) {
  if (pParticleSystemName) {
    ...
    if (nIndex != INVALID_STRING_INDEX) return nIndex;
...
#define QUICKGETCVARVALUE(v) \
  ...
  // Unfortunately, we have to duplicate the code in cdll_bounded_cvars.cpp here
  // because the client doesn't send the virtualized value up (because it has no
  // way to know when the virtualized value changes). Possible todo: put the
  // responsibility on the bounded cvar to notify the engine when its
...
void CServerGameClients::ClientSetupVisibility(edict_t *pViewEntity,
  ...
  for (unsigned short i = g_AreaPortals.Head();
    ...
    if (pCur->m_iPortalVersion == 0) bIsOpenOnClient = true;
...
#ifdef PORTAL
  // *After* the player's view has updated its area bits, add on any other areas
  // seen by portals
...
CPlayerState *CServerGameClients::GetPlayerState(edict_t *player) {
  // Is the client spawned yet?
  if (!player || !player->GetUnknown()) return NULL;

  CBasePlayer *pBasePlayer = (CBasePlayer *)CBaseEntity::Instance(player);
  if (!pBasePlayer) return NULL;
  ...
  if (gpGlobals->maxClients == 1) {
    ...
    if (ent) {
      Q_snprintf(buf, buflen, "Picker %i/%s - ent %s model %s\n",
                 ...
                 STRING(ent->GetEntityName()), ent->GetModelName());
    }
...
void MessageWriteByte(int iValue) {
  if (!g_pMsgBuffer) Error("WRITE_BYTE called with no active message\n");

  g_pMsgBuffer->WriteByte(iValue);
}

void MessageWriteChar(int iValue) {
  if (!g_pMsgBuffer) Error("WRITE_CHAR called with no active message\n");

  g_pMsgBuffer->WriteChar(iValue);
}

void MessageWriteShort(int iValue) {
  if (!g_pMsgBuffer) Error("WRITE_SHORT called with no active message\n");

  g_pMsgBuffer->WriteShort(iValue);
}

void MessageWriteWord(int iValue) {
  if (!g_pMsgBuffer) Error("WRITE_WORD called with no active message\n");

  g_pMsgBuffer->WriteWord(iValue);
}

void MessageWriteLong(int iValue) {
  if (!g_pMsgBuffer) Error("WriteLong called with no active message\n");

  g_pMsgBuffer->WriteLong(iValue);
}

void MessageWriteFloat(float flValue) {
  if (!g_pMsgBuffer) Error("WriteFloat called with no active message\n");

  g_pMsgBuffer->WriteFloat(flValue);
}

void MessageWriteAngle(float flValue) {
  if (!g_pMsgBuffer) Error("WriteAngle called with no active message\n");

  g_pMsgBuffer->WriteBitAngle(flValue, 8);
}

void MessageWriteCoord(float flValue) {
  if (!g_pMsgBuffer) Error("WriteCoord called with no active message\n");

  g_pMsgBuffer->WriteBitCoord(flValue);
}

void MessageWriteVec3Coord(const Vector &rgflValue) {
  if (!g_pMsgBuffer) Error("WriteVec3Coord called with no active message\n");

  g_pMsgBuffer->WriteBitVec3Coord(rgflValue);
}

void MessageWriteVec3Normal(const Vector &rgflValue) {
  if (!g_pMsgBuffer) Error("WriteVec3Normal called with no active message\n");
...
void MessageWriteString(const char *sz) {
  if (!g_pMsgBuffer) Error("WriteString called with no active message\n");

  g_pMsgBuffer->WriteString(sz);
}

void MessageWriteEntity(int iValue) {
  if (!g_pMsgBuffer) Error("WriteEntity called with no active message\n");

  g_pMsgBuffer->WriteShort(iValue);
}

void MessageWriteEHandle(CBaseEntity *pEntity) {
  if (!g_pMsgBuffer) Error("WriteEHandle called with no active message\n");
