...
ConVar bot_forcefireweapon("bot_forcefireweapon", "", 0,
                           "Force bots with the specified weapon to fire.");
ConVar bot_forceattack2("bot_forceattack2", "0", 0,
                        "When firing, use attack2.");
ConVar bot_forceattackon("bot_forceattackon", "1", 0,
                         "When firing, don't tap fire, hold it down.");
ConVar bot_flipout("bot_flipout", "0", 0, "When on, all bots fire their guns.");
ConVar bot_defend("bot_defend", "0", 0,
                  "Set to a team number, and that team will all keep their "
                  "combat shields raised.");
ConVar bot_changeclass("bot_changeclass", "0", 0,
...
static ConVar bot_mimic("bot_mimic", "0", 0,
                        "Bot uses usercmd of player by index.");
static ConVar bot_mimic_yaw_offset("bot_mimic_yaw_offset", "180", 0,
                                   "Offsets the bot yaw.");
...
CBasePlayer *BotPutInServer(bool bFrozen, int iTeam, int iClass,
  ...
  if (pszCustomName && pszCustomName[0]) {
    ...
    switch (RandomInt(0, 5)) {
      case 0:
        Q_snprintf(botname, sizeof(botname), "Bot", BotNumber);
        break;
      case 1:
        Q_snprintf(botname, sizeof(botname), "This is a medium Bot", BotNumber);
        break;
      case 2:
        Q_snprintf(botname, sizeof(botname),
                   "This is a super long bot name that is too long for the "
                   "game to allow",
                   BotNumber);
        break;
      case 3:
        Q_snprintf(botname, sizeof(botname), "Another bot", BotNumber);
        break;
      case 4:
        ...
        break;
      default:
        Q_snprintf(botname, sizeof(botname), "B", BotNumber);
        break;
    }
  } else {
...
CON_COMMAND_F(bot, "Add a bot.", FCVAR_CHEAT) {
  ...
  while (--count >= 0) {
    ...
    if (args.FindArg("-all")) iClass = 9 - count;
...
bool RunMimicCommand(CUserCmd &cmd) {
  if (bot_mimic.GetInt() <= 0) return false;
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove(CTFPlayer *fakeclient, const QAngle &viewangles,
...
void Bot_Think(CTFPlayer *pBot) {
  ...
  if (pBot->GetTeamNumber() == TEAM_UNASSIGNED &&
      gpGlobals->curtime > botdata->m_flJoinTeamTime) {
  ...
  } else if (pBot->GetTeamNumber() != TEAM_UNASSIGNED &&
             pBot->GetPlayerClass()->IsClass(TF_CLASS_UNDEFINED)) {
    // If they're on a team but haven't picked a class, choose a random class..
    pBot->HandleCommand_JoinClass(
        GetPlayerClassData(botdata->m_WantedClass)->m_szClassName);
  ...
  if (bot_flipout.GetInt() == 2) {
    ...
    botdata->lastAngles.y =
        AngleNormalize((gpGlobals->curtime * 1.7 + pBot->entindex()) * 45);
    botdata->lastAngles.z = 0.0;

    float speed =
        300;  // sin( gpGlobals->curtime / 1.7 + pBot->entindex() ) * 600;
  ...
  RunPlayerMove(pBot, vecViewAngles, forwardmove, sidemove, upmove, buttons,
                impulse, frametime);
}
...
void cc_bot_sendcommand(const CCommand &args) {
  if (args.ArgC() < 3) {
    Msg("Too few parameters.  Usage: bot_command <bot name> <command "
        "string...>\n");
    return;
  }
...
static ConCommand bot_sendcommand("bot_command", cc_bot_sendcommand,
                                  "<bot id> <command string...>.  Sends "
                                  "specified command on behalf of specified "
                                  "bot",
                                  FCVAR_CHEAT);
...
static ConCommand bot_kill("bot_kill", cc_bot_kill, "<bot id>.  Kills bot.",
                           FCVAR_CHEAT);
...
CON_COMMAND_F(bot_whack, "Deliver lethal damage from player to specified bot",
              FCVAR_CHEAT) {
  if (args.ArgC() < 2) {
...
CON_COMMAND_F(bot_teleport,
              "Teleport the specified bot to the specified position & "
              "angles.\n\tFormat: bot_teleport <bot name> <X> <Y> <Z> <Pitch> "
              "<Yaw> <Roll>",
              FCVAR_CHEAT) {
  if (args.ArgC() < 8) {
    Msg("Too few parameters.  bot_teleport <bot name> <X> <Y> <Z> <Pitch> "
        "<Yaw> <Roll>\n");
    return;
  }
