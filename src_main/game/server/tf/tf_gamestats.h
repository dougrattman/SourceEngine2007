...
class CTFGameStats : public CBaseGameStats, public CAutoGameSystemPerFrame {
 public:
  ...
  void Event_PlayerLeachedHealth(CTFPlayer *pPlayer, bool bDispenserHeal,
                                 float amount);
  ...
  void Event_PlayerDestroyedBuilding(CTFPlayer *pPlayer,
  ...
  void Event_PlayerUsedTeleport(CTFPlayer *pTeleportOwner,
                                CTFPlayer *pTeleportingPlayer);
  void Event_PlayerFiredWeapon(CTFPlayer *pPlayer, bool bCritical);
  void Event_PlayerDamage(CBasePlayer *pPlayer, const CTakeDamageInfo &info,
                          int iDamageTaken);
  void Event_PlayerKilledOther(CBasePlayer *pAttacker, CBaseEntity *pVictim,
                               const CTakeDamageInfo &info);
 ...
 public:
  TFReportedStats_t
      m_reportedStats;  // Stats which are uploaded from TF server to Steam
 protected:
  PlayerStats_t m_aPlayerStats[MAX_PLAYERS + 1];  // List of stats for each
                                                  // player for current life -
                                                  // reset after each death or
                                                  // class change
};
