...
class CPassengerInfo {
 public:
  CPassengerInfo(void)
      : m_nRole(-1),
        m_nSeat(-1),
        m_strRoleName(NULL_STRING),
        m_strSeatName(NULL_STRING) {}
...
class CBaseServerVehicle : public IServerVehicle {
 ...
 public:
  ...
  virtual void HandlePassengerEntry(CBaseCombatCharacter *pPassenger,
                                    bool bAllowEntryOutsideZone = false);
  virtual bool HandlePassengerExit(CBaseCombatCharacter *pPassenger);

  virtual void GetPassengerSeatPoint(int nRole, Vector *pPoint,
                                     QAngle *pAngles);
  virtual bool GetPassengerExitPoint(int nRole, Vector *pPoint,
                                     QAngle *pAngles);
  ...
  virtual float Weapon_PrimaryCanFireAt(
      void);  // Return the time at which this vehicle's primary weapon can fire
              // again
  virtual float Weapon_SecondaryCanFireAt(
 ...
 public:
  bool NPC_AddPassenger(CBaseCombatCharacter *pPassenger, string_t strRoleName,
                        int nSeat);
  bool NPC_RemovePassenger(CBaseCombatCharacter *pPassenger);
  virtual bool NPC_GetPassengerSeatPosition(CBaseCombatCharacter *pPassenger,
                                            Vector *vecResultPos,
                                            QAngle *vecResultAngle);
  virtual bool NPC_GetPassengerSeatPositionLocal(
  ...
  virtual int NPC_GetAvailableSeat(CBaseCombatCharacter *pPassenger,
                                   string_t strRoleName,
                                   VehicleSeatQuery_e nQueryType);
  bool NPC_HasAvailableSeat(string_t strRoleName);

  virtual const PassengerSeatAnims_t *NPC_GetPassengerSeatAnims(
      CBaseCombatCharacter *pPassenger, PassengerSeatAnimType_t nType);
  virtual CBaseCombatCharacter *NPC_GetPassengerInSeat(int nRoleID,
                                                       int nSeatID);
 ...
 private:
  ...
  void ParseNPCSeatTransition(KeyValues *pTransitionKeyValues,
                              CPassengerSeatTransition *pTransition);
 ...
 protected:
  ...
  int NPC_GetAvailableSeat_Nearest(CBaseCombatCharacter *pPassenger,
                                   int nRoleID);

  CPassengerRole *FindOrCreatePassengerRole(string_t strName,
                                            int *nIndex = NULL);
 ...
 public:
  ...
  virtual int GetExitAnimToUse(Vector &vecEyeExitEndpoint,
  ...
  void SetVehicleVolume(float flVolume) {
    m_flVehicleVolume = std::clamp(flVolume, 0.0f, 1.0f);
  }
 ...
 public:
  ...
  struct exitanim_t {
    ...
    bool bEscapeExit;
    char szAnimName[128];
    Vector vecExitPointLocal;  // Point the animation leaves the player at when
                               // finished
  ...
  bool m_bUseLegacyExitChecks;  // HACK: Choreo vehicles use non-sensical setups
                                // to move the player, we need to poll their
  ...
  CHandle<CEntityBlocker>
      m_hExitBlocker;  // Entity to prevent other entities blocking the player's
                       // exit point during the exit animation
 ...
 private:
  ...
  bool GetLocalAttachmentAtTime(int nQuerySequence, int nAttachmentIndex,
                                float flCyclePoint, Vector *vecOriginOut,
                                QAngle *vecAnglesOut);
  bool GetLocalAttachmentAtTime(const char *lpszAnimName, int nAttachmentIndex,
                                float flCyclePoint, Vector *vecOriginOut,
                                QAngle *vecAnglesOut);
};
