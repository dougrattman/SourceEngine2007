...
SOURCE_DECLARE_POINTER_HANDLE(AI_PathNode_t);
...
struct AI_ProgressFlyPathParams_t {
  AI_ProgressFlyPathParams_t(unsigned collisionMask,
                             ...
                             float waypointTolerance = 100,
                             float goalTolerance = 12,
  ...
  float blockTolerance;  // @TODO (toml 07-03-02): rename "blockTolerance". This
                         // is specifically the "simplify" block tolerance. See
                         // SimplifyFlyPath()
  float waypointTolerance;
  float goalTolerance;  // @TODO (toml 07-03-02): goalTolerance appears to have
                        // come into existence because
  // noone had set a good tolerance in the path itself. It is therefore
  // redundant, and more than likely should be excised
  AI_NpcBlockHandling_t
      blockHandling;  // @TODO (toml 07-03-02): rename "blockHandling". This is
                      // specifically the "simplify" block handling. See
                      // SimplifyFlyPath()
...
class CAI_Navigator : public CAI_Component, public CAI_DefMovementSink {
 ...
 private:
  ...
  float m_timePathRebuildMax;  // How long to try rebuilding path before failing
                               // task
  float m_timePathRebuildDelay;  // How long to wait before trying to rebuild
                                 // again

  float m_timePathRebuildFail;  // Current global time when should fail building
                                // path
