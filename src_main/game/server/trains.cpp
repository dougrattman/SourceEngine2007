...
class CBasePlatTrain : public CBaseToggle {
 ...
 public:
  ...
  // This is done to fix spawn flag collisions between this class and a derived
  // class
...
void CBasePlatTrain::PlayMovingSound() {
  ...
  if (m_NoiseMoving != NULL_STRING) {
    ...
    m_pMovementSound = controller.SoundCreate(filter, entindex(), CHAN_STATIC,
...
class CFuncPlat : public CBasePlatTrain {
 ...
 public:
  ...
  void PlatUse(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType,
               float value);
...
BEGIN_DATADESC(CFuncPlat)
  ...
  DEFINE_FIELD(m_sNoise, FIELD_STRING),
      ...
      // Function Pointers
      DEFINE_FUNCTION(PlatUse), DEFINE_FUNCTION(CallGoDown),
      DEFINE_FUNCTION(CallHitTop), DEFINE_FUNCTION(CallHitBottom),
...
void CFuncPlat::Spawn() {
  ...
  // If this platform is the target of some button, it starts at the TOP
  // position, and is brought down by that button.  Otherwise, it starts at
  // BOTTOM.
...
static void PlatSpawnInsideTrigger(edict_t *pevPlatform) {
  // old code: //GetClassPtr( (CPlatTrigger *)NULL)->SpawnInsideTrigger(
  // GetClassPtr( (CFuncPlat *)pevPlatform ) );
...
void CPlatTrigger::SpawnInsideTrigger(CFuncPlat *pPlatform) {
  ...
  // Create trigger entity, "point" it at the owning platform, give it a touch
  // method
  SetSolid(SOLID_BSP);
  ...
  vecTMin.z = vecTMax.z - (m_pPlatform->m_vecPosition1.z -
                           m_pPlatform->m_vecPosition2.z + 8);
...
void CFuncPlat::InputGoUp(inputdata_t &data) {
  if (m_toggle_state == TS_AT_BOTTOM) GoUp();
}
...
//			value -
//-----------------------------------------------------------------------------
void CFuncPlat::PlatUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CFuncTrain::Blocked(CBaseEntity *pOther) {
  if (gpGlobals->curtime < m_flNextBlockTime) return;
...
void CFuncTrain::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                     USE_TYPE useType, float value) {
...
BEGIN_DATADESC(CFuncTrackTrain)
  ...
  DEFINE_KEYFIELD(m_length, FIELD_FLOAT, "wheels"),
      ...
      DEFINE_FIELD(m_ppath, FIELD_CLASSPTR), DEFINE_FIELD(m_dir, FIELD_FLOAT),
      DEFINE_FIELD(m_controlMins, FIELD_VECTOR),
      DEFINE_FIELD(m_controlMaxs, FIELD_VECTOR),
      DEFINE_FIELD(m_flVolume, FIELD_FLOAT),
      DEFINE_FIELD(m_oldSpeed, FIELD_FLOAT),
      // DEFINE_FIELD( m_lastBlockPos, FIELD_POSITION_VECTOR ), // temp values
      // for blocking, don't save DEFINE_FIELD( m_lastBlockTick, FIELD_INTEGER
      // ),
      ...
      // Function Pointers
      DEFINE_FUNCTION(Next), DEFINE_FUNCTION(Find),
      DEFINE_FUNCTION(NearestPath), DEFINE_FUNCTION(DeadEnd),
...
int CFuncTrackTrain::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "angles: %g %g %g",
               (double)GetLocalAngles()[PITCH], (double)GetLocalAngles()[YAW],
               (double)GetLocalAngles()[ROLL]);
    EntityText(nOffset, tempstr, 0);
    nOffset++;

    float flCurSpeed = GetLocalVelocity().Length();
    Q_snprintf(tempstr, sizeof(tempstr), "current speed (goal): %g (%g)",
               (double)flCurSpeed, (double)m_flSpeed);
    EntityText(nOffset, tempstr, 0);
    nOffset++;
...
void CFuncTrackTrain::DrawDebugGeometryOverlays() {
  ...
  if (m_debugOverlays & OVERLAY_BBOX_BIT) {
    NDebugOverlay::Box(GetAbsOrigin(), -Vector(4, 4, 4), Vector(4, 4, 4), 255,
                       0, 255, 0, 0);
    Vector out;
    VectorTransform(Vector(m_length, 0, 0), EntityToWorldTransform(), out);
    NDebugOverlay::Box(out, -Vector(4, 4, 4), Vector(4, 4, 4), 255, 0, 255, 0,
                       0);
  }
}
...
//-----------------------------------------------------------------------------
void CFuncTrackTrain::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                          USE_TYPE useType, float value) {
  // player +USE
  if (useType == USE_SET) {
    float delta = value;
...
void CFuncTrackTrain::InputSetSpeedReal(inputdata_t &inputdata) {
  SetSpeed(std::clamp(inputdata.value.Float(), 0.0f, m_maxSpeed));
}
...
void CFuncTrackTrain::InputSetSpeed(inputdata_t &inputdata) {
  float flScale = std::clamp(inputdata.value.Float(), 0.0f, 1.0f);
...
void CFuncTrackTrain::InputSetSpeedDir(inputdata_t &inputdata) {
  ...
  float flScale = std::clamp(newSpeed, 0.0f, 1.0f);
  SetSpeed(m_maxSpeed * flScale);
}
    ...
    if (pPhysics) {
      ...
      while (pSnapshot->IsValid()) {
        ...
        if (pOtherEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
          ...
          float dot = DotProduct(pParentEntity->GetAbsVelocity(),
                                 pSnapshot->GetNormalForce() * normal);
...
void CFuncTrackTrain::Blocked(CBaseEntity *pOther) {
  ...
  if (HasSpawnFlags(SF_TRACKTRAIN_UNBLOCKABLE_BY_PLAYER)) {
    ...
    if (pPhysicsBlocker) {
      ...
      // The heuristic here is to keep instantaneous blocks from invoking the
      // somewhat heavy-handed solver (which will disable collisions until we're
      // clear) in cases where physics can solve it easily enough.
      int ticksBlocked = gpGlobals->tickcount - m_lastBlockTick;
      float dist = 0.0f;
      // wait at least 10 ticks and make sure the train isn't actually moving
      // before really blocking
      const int MIN_BLOCKED_TICKS = 10;
      if (ticksBlocked > MIN_BLOCKED_TICKS) {
        dist = (GetAbsOrigin() - m_lastBlockPos).Length();
        // must have moved at least 10% of normal velocity over the blocking
        // interval, or we're being blocked
        float minLength = GetAbsVelocity().Length() * TICK_INTERVAL *
                          MIN_BLOCKED_TICKS * 0.10f;
        if (dist < minLength) {
          // been stuck for more than one tick without moving much?
          // yes, disable collisions with the physics object most likely to be
          // blocking us
  ...
  DevWarning(2, "TRAIN(%s): Blocked by %s (dmg:%.2f)\n", GetDebugName(),
             pOther->GetClassname(), m_flBlockDamage);
  if (m_flBlockDamage <= 0) return;
...
//			NOTE: when train goes through transition,
// m_bSoundPlaying
// should become 			false, which will cause the looped sound
// to restart.
//-----------------------------------------------------------------------------
void CFuncTrackTrain::SoundUpdate(void) {
  ...
  if (HasSpawnFlags(SF_TRACKTRAIN_USE_MAXSPEED_FOR_PITCH)) {
    flSpeedRatio = std::clamp(fabs(m_flSpeed) / m_maxSpeed, 0.0f, 1.0f);
  } else {
    flSpeedRatio = std::clamp(fabs(m_flSpeed) / TRAIN_MAXSPEED, 0.0f, 1.0f);
  }

  float flpitch =
      RemapVal(flSpeedRatio, 0, 1, m_nMoveSoundMinPitch, m_nMoveSoundMaxPitch);
  ...
  if (!m_bSoundPlaying) {
    ...
    m_flNextMoveSoundTime =
        gpGlobals->curtime + RemapVal(flSpeedRatio, 0, 1, m_flMoveSoundMaxTime,
    ...
    if ((m_iszSoundMovePing != NULL_STRING) &&
        (gpGlobals->curtime > m_flNextMoveSoundTime)) {
      EmitSound(STRING(m_iszSoundMovePing));
      m_flNextMoveSoundTime = gpGlobals->curtime +
                              RemapVal(flSpeedRatio, 0, 1, m_flMoveSoundMaxTime,
                                       m_flMoveSoundMinTime);
    }
  }
}
...
void CFuncTrackTrain::ArriveAtNode(CPathTrack *pNode) {
  // BUGBUG: This is wrong.  We need to fire all targets between the one we've
  // passed and the one we've switched to.
  ...
  if (m_spawnflags & SF_TRACKTRAIN_NOCONTROL) {
    ...
    if (pNode->m_flSpeed != 0) {
      ...
      DevMsg(2, "TrackTrain %s arrived at %s, speed to %4.2f\n", GetDebugName(),
             pNode->GetDebugName(), pNode->m_flSpeed);
    }
  }
}
...
// Input  : *pnext -
//-----------------------------------------------------------------------------
void CFuncTrackTrain::UpdateTrainVelocity(CPathTrack *pPrev, CPathTrack *pNext,
...
// Input  : pnext -
//-----------------------------------------------------------------------------
void CFuncTrackTrain::UpdateTrainOrientation(CPathTrack *pPrev,
                                             CPathTrack *pNext,
                                             const Vector &nextPos,
                                             float flInterval) {
  ...
  // share the same move parent as the train.
  ...
  switch (GetTrainOrientationType()) {
    ...
    case TrainOrientation_LinearBlend: {
      UpdateOrientationBlend(GetTrainOrientationType(), pPrev, pNext, nextPos,
                             flInterval);
      break;
    }
  }
...
void CFuncTrackTrain::UpdateOrientationAtPathTracks(CPathTrack *pPrev,
  ...
  // !!!  All of this crap has to be done to make the angles not wrap around,
  // revisit this.
  FixupAngles(angles);
...
void CFuncTrackTrain::UpdateOrientationBlend(
    TrainOrientationType_t eOrientationType, CPathTrack *pPrev,
    CPathTrack *pNext, const Vector &nextPos, float flInterval) {
...
//-----------------------------------------------------------------------------
// Purpose: Sets our angular velocity to approach the target angles over the
// given interval.
//-----------------------------------------------------------------------------
void CFuncTrackTrain::DoUpdateOrientation(const QAngle &curAngles,
                                          const QAngle &angles,
                                          float flInterval) {
  float vy, vx;
  if (!(m_spawnflags & SF_TRACKTRAIN_NOPITCH)) {
  ...
  // HACKHACK: Clamp really small angular deltas to avoid rotating movement on
  ...
  QAngle vecAngVel(vx / flInterval, vy / flInterval,
                   GetLocalAngularVelocity().z);

  if (m_flBank != 0) {
    if (vecAngVel.y < -5) {
      vecAngVel.z = UTIL_AngleDistance(
          UTIL_ApproachAngle(-m_flBank, curAngles.z, m_flBank * 2),
          curAngles.z);
    } else if (vecAngVel.y > 5) {
      vecAngVel.z = UTIL_AngleDistance(
          UTIL_ApproachAngle(m_flBank, curAngles.z, m_flBank * 2), curAngles.z);
    } else {
      vecAngVel.z =
          UTIL_AngleDistance(UTIL_ApproachAngle(0, curAngles.z, m_flBank * 4),
                             curAngles.z) *
          4;
    }
  }
...
void CFuncTrackTrain::Next(void) {
  ...
  if (m_debugOverlays & OVERLAY_BBOX_BIT) {
    if (pNext != NULL) {
      NDebugOverlay::Line(GetAbsOrigin(), pNext->GetAbsOrigin(), 255, 0, 0,
                          true, 0.1);
      NDebugOverlay::Line(pNext->GetAbsOrigin(),
                          pNext->GetAbsOrigin() + Vector(0, 0, 32), 255, 0, 0,
                          true, 0.1);
      NDebugOverlay::Box(pNext->GetAbsOrigin(), Vector(-8, -8, -8),
                         Vector(8, 8, 8), 255, 0, 0, 0, 0.1);
    }

    if (pNextNext != NULL) {
      NDebugOverlay::Line(GetAbsOrigin(), pNextNext->GetAbsOrigin(), 0, 255, 0,
                          true, 0.1);
      NDebugOverlay::Line(pNextNext->GetAbsOrigin(),
                          pNextNext->GetAbsOrigin() + Vector(0, 0, 32), 0, 255,
                          0, true, 0.1);
      NDebugOverlay::Box(pNextNext->GetAbsOrigin(), Vector(-8, -8, -8),
                         Vector(8, 8, 8), 0, 255, 0, 0, 0.1);
    }
  }
...
void CFuncTrackTrain::FirePassInputs(CPathTrack *pStart, CPathTrack *pEnd,
                                     bool forward) {
...
void CFuncTrackTrain::DeadEnd(void) {
  ...
  // different node depending on it's speed so we have to traverse the list to
  // it's end.
  if (pTrack) {
    if (m_oldSpeed < 0) {
      do {
        pNext = pTrack->ValidPath(pTrack->GetPrevious(), true);
        if (pNext) pTrack = pNext;
      } while (pNext);
    } else {
      do {
...
#ifndef PORTAL  // env_portal_path_track is a child of path_track and would like
                // to get found
...
void CFuncTrackTrain::NearestPath(void) {
  ...
  for (CEntitySphereQuery sphere(GetAbsOrigin(), 1024);
       (pTrack = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity()) {
    // filter out non-tracks
    if (!(pTrack->GetFlags() & (FL_CLIENT | FL_NPC)) &&
        FClassnameIs(pTrack, "path_track")) {
  ...
  DevMsg(2, "TRAIN: %s, Nearest track is %s\n", GetDebugName(),
         pNearest->GetDebugName());
  // If I'm closer to the next path_track on this path, then it's my real path
  pTrack = ((CPathTrack *)pNearest)->GetNext();
  if (pTrack) {
    if ((GetLocalOrigin() - pTrack->GetLocalOrigin()).Length() <
        (GetLocalOrigin() - pNearest->GetLocalOrigin()).Length())
      pNearest = pTrack;
  }
...
#ifdef HL1_DLL
  // BUGBUG: For now, just force this for testing.  Remove if we want to tag all
  // of the trains in the levels
  SetSolid(SOLID_BSP);
#else
...
void CFuncTrainControls::Spawn(void) {
  ...
  Assert(GetParent() &&
         "func_traincontrols needs parent to properly align to train");
...
//-----------------------------------------------------------------------------
// This entity is a rotating/moving platform that will carry a train to a new
// track. It must be larger in X-Y planar area than the train, since it must
// contain the train within these dimensions in order to operate when the train
// is near it.
//-----------------------------------------------------------------------------
...
void CFuncTrackChange::Find(void) {
  ...
  if (target) {
    ...
    if (target) {
      ...
      if (target) {
        m_train =
            (CFuncTrackTrain *)gEntList.FindEntityByName(NULL, m_trainName);
        if (!m_train) {
...
TRAIN_CODE CFuncTrackChange::EvaluateTrain(CPathTrack *pcurrent) {
  ...
  if (m_train->m_ppath == pcurrent ||
    ...
    if (m_train->m_flSpeed != 0) return TRAIN_BLOCKING;
...
void CFuncTrackChange::UpdateTrain(QAngle &dest) {
  ...
  // Attempt at getting the train to rotate properly around the origin of the
  // trackchange
  if (time <= 0) return;
...
void CFuncTrackChange::GoDown(void) {
  if (m_code == TRAIN_BLOCKING) return;
...
void CFuncTrackChange::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                           USE_TYPE useType, float value) {
  if (m_toggle_state != TS_AT_TOP && m_toggle_state != TS_AT_BOTTOM) return;
...
void CFuncTrackAuto::TriggerTrackChange(inputdata_t &inputdata) {
  ...
  if (inputdata.pActivator &&
      FClassnameIs(inputdata.pActivator, "func_tracktrain")) {
    ...
    if (pTarget) pTarget = pTarget->GetNext();
    if (pTarget && m_train->m_ppath != pTarget &&
        ShouldToggle(USE_TOGGLE, m_targetState)) {
      if (m_targetState == TS_AT_TOP)
        m_targetState = TS_AT_BOTTOM;
      else
        m_targetState = TS_AT_TOP;
...
void CFuncTrackAuto::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                         USE_TYPE useType, float value) {
  CPathTrack *pTarget;
  ...
  if (FClassnameIs(pActivator, "func_tracktrain")) {
    ...
    if (pTarget) pTarget = pTarget->GetNext();
    if (pTarget && m_train->m_ppath != pTarget &&
        ShouldToggle(useType, m_targetState)) {
      if (m_targetState == TS_AT_TOP)
        m_targetState = TS_AT_BOTTOM;
      else
        m_targetState = TS_AT_TOP;
