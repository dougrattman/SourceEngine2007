...
void W_Precache(void) {
  ...
  g_sModelIndexWExplosion =
      CBaseEntity::PrecacheModel("sprites/WXplo1.vmt");  // underwater fireball
  g_sModelIndexBloodSpray =
      CBaseEntity::PrecacheModel("sprites/bloodspray.vmt");  // initial blood
  g_sModelIndexBloodDrop =
      CBaseEntity::PrecacheModel("sprites/blood.vmt");  // splattered blood
  g_sModelIndexLaserDot = CBaseEntity::PrecacheModel("sprites/laserdot.vmt");
#endif  // HL1_DLL

#ifndef TF_DLL
  g_sModelIndexFireball =
      CBaseEntity::PrecacheModel("sprites/zerogxplode.vmt");  // fireball

  g_sModelIndexSmoke =
      CBaseEntity::PrecacheModel("sprites/steam1.vmt");  // smoke
  g_sModelIndexBubbles =
      CBaseEntity::PrecacheModel("sprites/bubble.vmt");  // bubbles
...
int CBaseCombatWeapon::UpdateTransmitState(void) {
  // If the weapon is being carried by a CBaseCombatCharacter, let the combat
  // character do the logic about whether or not to transmit it.
  if (GetOwner()) {
    return SetTransmitState(FL_EDICT_PVSCHECK);
  } else {
    // If it's just lying around, then use CBaseEntity's visibility test to see
...
void CBaseCombatWeapon::Operator_HandleAnimEvent(
  ...
  DevWarning(2, "Unhandled animation event %d from %s --> %s\n", pEvent->event,
             pOperator->GetClassname(), GetClassname());
}

// NOTE: This should never be called when a character is operating the weapon.
// Animation events should be routed through the character, and then back into
// CharacterAnimEvent()
...
CBaseEntity *CBaseCombatWeapon::Respawn(void) {
  ...
  // (no touch function). The weapon spawn/respawn code will decide when to make
  // the weapon visible and touchable.
  CBaseEntity *pNewWeapon = CBaseEntity::Create(
      GetClassname(), g_pGameRules->VecWeaponRespawnSpot(this),
      GetLocalAngles(), GetOwnerEntity());
  ...
  if (pNewWeapon) {
    ...
    // not a typo! We want to know when the weapon the player just picked up
    // should respawn! This new entity we created is the replacement, but when
    // it should respawn is based on conditions belonging to the weapon that was
    // taken.
    pNewWeapon->SetNextThink(gpGlobals->curtime +
                             g_pGameRules->FlWeaponRespawnTime(this));
  } else {
...
class CWeaponLOSFilter : public CTraceFilterSkipTwoEntities {
 ...
 public:
  CWeaponLOSFilter::CWeaponLOSFilter(IHandleEntity *pHandleEntity,
    ...
    if (pHandleEntity != NULL) {
      CBaseCombatCharacter *pBCC =
          ((CBaseEntity *)pHandleEntity)->MyCombatCharacterPointer();
      if (pBCC != NULL) {
...
//also be set
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::WeaponLOSCondition(const Vector &ownerPos,
  ...
  VectorSubtract(npcOwner->Weapon_ShootPosition(), npcOwner->GetAbsOrigin(),
  ...
  CWeaponLOSFilter traceFilter(m_hOwner.Get(), npcOwner->GetEnemy(),
  ...
  // is player in a vehicle? if so, verify vehicle is target and return if so
  // (so npc shoots at vehicle)
  if (pEnemyPlayer && pEnemyPlayer->IsInAVehicle()) {
    // Ok, player in vehicle, check if vehicle is target we're looking at, fire
    // if it is Also, check to see if the owner of the entity is the vehicle, in
    // which case it's valid too. This catches vehicles that use bone followers.
  ...
  // If a vehicle is blocking the view, grab its driver and use that as the
  // combat character
  CBaseCombatCharacter *pBCC;
  ...
  if (pBCC) {
    if (npcOwner->IRelationType(pBCC) == D_HT) return true;
...
int CBaseCombatWeapon::WeaponRangeAttack1Condition(float flDot, float flDist) {
  ...
  } else if (flDot < 0.5)  // UNDONE: Why check this here? Isn't the AI checking
                           // this already?
  {
...
//			they hit the ground. Once we're sure that the object is
// grounded, 			we change its solid type to trigger and set it in a
// large box that 			helps the player get it.
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::FallThink(void) {
  ...
  if (shouldMaterialize) {
    ...
    // don't clatter if the gun is waiting to respawn (if it's waiting, it is
    // invisible!)
    if (GetOwnerEntity()) {
...
class CWeaponList : public CAutoGameSystem {
 public:
  ...
  virtual void LevelShutdownPostEntity() { m_list.Purge(); }

  void AddWeapon(CBaseCombatWeapon *pWeapon) { m_list.AddToTail(pWeapon); }
...
int CBaseCombatWeapon::GetAvailableWeaponsInBox(CBaseCombatWeapon **pList,
                                                int listMax, const Vector &mins,
                                                const Vector &maxs) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  if (pPlayer) {
    ...
    //
    // Bump the weapon to try equipping it before picking it up physically. This
    // is important in a few spots in the game where the player could
    // potentially +use pickup and then THROW AWAY a vital weapon, rendering
    // them unable to continue the game.
    //
