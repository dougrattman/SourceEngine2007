...
ConVar g_debug_transitions("g_debug_transitions", "0", FCVAR_NONE,
                           "Set to 1 and restart the map to be warned if the "
                           "map has no trigger_transition volumes. Set to 2 to "
                           "see a dump of all entities & associated results "
                           "during a transition.");
...
void Cmd_ShowtriggersToggle_f(const CCommand &args) {
  // Loop through the entities in the game and make visible anything derived
  // from CBaseTrigger
...
static ConCommand showtriggers_toggle("showtriggers_toggle",
                                      Cmd_ShowtriggersToggle_f,
                                      "Toggle show triggers", FCVAR_CHEAT);
...
//------------------------------------------------------------------------------
void CBaseTrigger::InputEnable(inputdata_t &inputdata) { Enable(); }

//------------------------------------------------------------------------------
// Purpose: Input handler to turn off this trigger.
//------------------------------------------------------------------------------
void CBaseTrigger::InputDisable(inputdata_t &inputdata) { Disable(); }

void CBaseTrigger::InputTouchTest(inputdata_t &inputdata) { TouchTest(); }

//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void CBaseTrigger::Spawn() {
  if (HasSpawnFlags(SF_TRIGGER_ONLY_PLAYER_ALLY_NPCS) ||
      HasSpawnFlags(SF_TRIGGER_ONLY_NPCS_IN_VEHICLES)) {
...
void CBaseTrigger::Activate(void) {
  ...
  if (m_iFilterName != NULL_STRING) {
    m_hFilter = dynamic_cast<CBaseFilter *>(
        gEntList.FindEntityByName(NULL, m_iFilterName));
  }
...
bool CBaseTrigger::PassesTriggerFilters(CBaseEntity *pOther) {
  ...
  if (HasSpawnFlags(SF_TRIGGER_ALLOW_ALL) ||
      (HasSpawnFlags(SF_TRIGGER_ALLOW_CLIENTS) &&
       (pOther->GetFlags() & FL_CLIENT)) ||
      (HasSpawnFlags(SF_TRIGGER_ALLOW_NPCS) && (pOther->GetFlags() & FL_NPC)) ||
      (HasSpawnFlags(SF_TRIGGER_ALLOW_PUSHABLES) &&
       FClassnameIs(pOther, "func_pushable")) ||
      (HasSpawnFlags(SF_TRIGGER_ALLOW_PHYSICS) &&
    ...
    if (pOther->GetFlags() & FL_NPC) {
      ...
      if (HasSpawnFlags(SF_TRIGGER_ONLY_NPCS_IN_VEHICLES)) {
        if (!pNPC || !pNPC->IsInAVehicle()) return false;
      }
    }
    ...
    if (HasSpawnFlags(SF_TRIGGER_ONLY_CLIENTS_IN_VEHICLES) && bOtherIsPlayer) {
      if (!((CBasePlayer *)pOther)->IsInAVehicle()) return false;

      // Make sure we're also not exiting the vehicle at the moment
      IServerVehicle *pVehicleServer = ((CBasePlayer *)pOther)->GetVehicle();
      if (pVehicleServer == NULL) return false;

      if (pVehicleServer->IsPassengerExiting()) return false;
    }
...
void CBaseTrigger::StartTouch(CBaseEntity *pOther) {
  if (PassesTriggerFilters(pOther)) {
    ...
    if (m_hTouchingEntities.Find(hOther) ==
        m_hTouchingEntities.InvalidIndex()) {
...
void CBaseTrigger::EndTouch(CBaseEntity *pOther) {
  if (IsTouching(pOther)) {
    ...
    // If there are no more entities touching this trigger, fire the lost all
    // touches Loop through the touching entities backwards. Clean out old ones,
    // and look for existing
...
bool CBaseTrigger::IsTouching(CBaseEntity *pOther) {
  EHANDLE hOther;
  hOther = pOther;
  return (m_hTouchingEntities.Find(hOther) !=
          m_hTouchingEntities.InvalidIndex());
}

//-----------------------------------------------------------------------------
// Purpose: Return a pointer to the first entity of the specified type being
...
CBaseEntity *CBaseTrigger::GetTouchedEntityOfType(const char *sClassName) {
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    if (FClassnameIs(pEntity, sClassName)) return pEntity;
  }
...
//			the player's geiger counter level according to distance
// from  center 			of trigger.
//-----------------------------------------------------------------------------
...
void CTriggerHurt::RadiationThink(void) {
  ...
  CBasePlayer *pPlayer = static_cast<CBasePlayer *>(
      UTIL_FindClientInPVS(vecSurroundMins, vecSurroundMaxs));

  if (pPlayer) {
    // get range to player;
    float flRange =
        CollisionProp()->CalcDistanceFromPoint(pPlayer->WorldSpaceCenter());
    flRange *= 3.0f;
...
//-----------------------------------------------------------------------------
// Purpose: When touched, a hurt trigger does m_flDamage points of damage each
// half-second. Input  : pOther - The entity that is touching us.
//-----------------------------------------------------------------------------
bool CTriggerHurt::HurtEntity(CBaseEntity *pOther, float damage) {
  if (!pOther->m_takedamage || !PassesTriggerFilters(pOther)) return false;
  ...
  // NDebugOverlay::Box( pOther->GetAbsOrigin(), pOther->WorldAlignMins(),
  // pOther->WorldAlignMaxs(), 255,0,0,0,0.5 );
  return true;
}
...
//			This function applies damage to any entities currently
...
int CTriggerHurt::HurtAllTouchers(float dt) {
  ...
  if (root) {
    for (touchlink_t *link = root->nextLink; link != root;
         link = link->nextLink) {
  ...
  if (m_damageModel == DAMAGEMODEL_DOUBLE_FORGIVENESS) {
    if (hurtCount == 0) {
      if (gpGlobals->curtime > m_flDmgResetTime) {
        // Didn't hurt anyone. Reset the damage if it's time. (hence, the
        // forgiveness)
      ...
      // Now, put the damage reset time into the future. The forgive time is how
      // long the trigger must go without harming anyone in order that its
      // accumulated damage be reset to the amount set by the level designer.
      // This is a stop-gap for an exploit where players could hop through slime
      // and barely take any damage because the trigger would reset damage
      // anytime there was no one in the trigger when this function was called.
      // (sjb)
...
//-----------------------------------------------------------------------------
void CTriggerMultiple::MultiWaitOver(void) { SetThink(NULL); }
...
class CTriggerLook : public CTriggerOnce {
 ...
 public:
  ...
  float m_flTimeoutDuration;  // Number of seconds after start touch to fire
                              // anyway
  bool m_bTimeoutFired;  // True if the OnTimeout output fired since the last
                         // StartTouch.
...
// Purpose:
//-----------------------------------------------------------------------------
void CTriggerLook::TimeoutThink(void) { Trigger(m_hActivator, true); }
    ...
    if (HasSpawnFlags(SF_TRIGGERLOOK_USEVELOCITY)) {
      ...
      if (vLookDir == vec3_origin) {
        ...
        if (pPlayer->IsInAVehicle()) {
          vLookDir =
              pPlayer->GetVehicle()->GetVehicleEnt()->GetSmoothedVelocity();
        }
      }
...
//	>> TriggerVolume
// ##################################################################################
class CTriggerVolume : public CPointEntity  // Derive from point entity so this
                                            // doesn't move across levels
{
 public:
...
class CChangeLevel : public CBaseTrigger {
 ...
 private:
  ...
  static int AddTransitionToList(levellist_t *pLevelList, int listCount,
                                 const char *pMapName,
  ...
  static int BuildEntityTransitionList(CBaseEntity *pLandmarkEntity,
                                       const char *pLandmarkName,
                                       CBaseEntity **ppEntList,
                                       int *pEntityFlags, int nMaxList);

  // Adds a single entity to the transition list, if appropriate. Returns the
  // new count
  static int AddEntityToTransitionList(CBaseEntity *pEntity, int flags,
                                       int nCount, CBaseEntity **ppEntList,
                                       int *pEntityFlags);

  // Adds in all entities depended on by entities near the transition
  static int AddDependentEntities(int nCount, CBaseEntity **ppEntList,
                                  int *pEntityFlags, int nMaxList);
 ...
 private:
  ...
  char m_szLandmarkName[cchMapNameMost];  // trigger_changelevel only:  landmark
                                          // on next map
  bool m_bTouched;
...
void CChangeLevel::InputChangeLevel(inputdata_t &inputdata) {
  // Ignore changelevel transitions if the player's dead or attempting a
  // challenge
...
bool CChangeLevel::IsEntityInTransition(CBaseEntity *pEntity) {
  ...
  Vector vecSurroundMins, vecSurroundMaxs;
  pEntity->CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins,
                                                        &vecSurroundMaxs);

  return engine->CheckBoxInPVS(vecSurroundMins, vecSurroundMaxs, pvs,
                               sizeof(pvs));
}
...
void CChangeLevel::NotifyEntitiesOutOfTransition() {
  ...
  while (pEnt) {
    ...
    if (pEnt->ObjectCaps() & FCAP_NOTIFY_ON_TRANSITION) {
      ...
      if (!(pEnt->ObjectCaps() &
            (FCAP_ACROSS_TRANSITION | FCAP_FORCE_TRANSITION)) ||
          !IsEntityInTransition(pEnt)) {
...
//------------------------------------------------------------------------------
// Purpose : Checks all spawned AIs and prints a warning if any are actively
// leading Input   : Output  :
//------------------------------------------------------------------------------
...
void CChangeLevel::ChangeLevelNow(CBaseEntity *pActivator) {
  ...
  CBaseEntity *pPlayer = (pActivator && pActivator->IsPlayer())
                             ? pActivator
                             : UTIL_GetLocalPlayer();
  ...
  if (transitionState == TRANSITION_VOLUME_NOT_FOUND) {
    ...
    if (pPlayer) {
      Vector vecSurroundMins, vecSurroundMaxs;
      pPlayer->CollisionProp()->WorldSpaceSurroundingBounds(&vecSurroundMins,
                                                            &vecSurroundMaxs);
      bool playerInPVS = engine->CheckBoxInPVS(vecSurroundMins, vecSurroundMaxs,
                                               pvs, sizeof(pvs));
  ...
  // This object will get removed in the call to engine->ChangeLevel, copy the
  // params into "safe" memory
...
// (a designer may have placed multiple trigger_changelevels with the same
// landmark)
int CChangeLevel::AddTransitionToList(levellist_t *pLevelList, int listCount,
                                      const char *pMapName,
                                      const char *pLandmarkName,
                                      edict_t *pentLandmark) {
  int i;
  ...
  if (stricmp(pMapName, STRING(gpGlobals->mapname)) == 0) return 0;

  for (i = 0; i < listCount; i++) {
    if (pLevelList[i].pentLandmark == pentLandmark &&
        stricmp(pLevelList[i].mapName, pMapName) == 0)
      return 0;
  }
  Q_strncpy(pLevelList[listCount].mapName, pMapName,
            sizeof(pLevelList[listCount].mapName));
  Q_strncpy(pLevelList[listCount].landmarkName, pLandmarkName,
            sizeof(pLevelList[listCount].landmarkName));
...
// NOTE: This routine is relatively slow.  If you need to use it for per-frame
// work, consider that fact. UNDONE: Expand this to the full matrix of solid
// types on each side and move into enginetrace
static bool TestEntityTriggerIntersection_Accurate(CBaseEntity *pTrigger,
                                                   CBaseEntity *pEntity) {
  ...
  if (pTrigger->Intersects(pEntity))  // It touches one, it's in the volume
  {
    switch (pEntity->GetSolid()) {
      case SOLID_BBOX: {
        ...
        Ray_t ray;
        trace_t tr;
        ray.Init(pEntity->GetAbsOrigin(), pEntity->GetAbsOrigin(),
                 pEntity->WorldAlignMins(), pEntity->WorldAlignMaxs());
        enginetrace->ClipRayToCollideable(ray, MASK_ALL, pCollide, &tr);

        if (tr.startsolid) return true;
      } break;
      case SOLID_BSP:
      case SOLID_VPHYSICS: {
        CPhysCollide *pTriggerCollide =
            modelinfo->GetVCollide(pTrigger->GetModelIndex())->solids[0];
        ...
        if (physicsCount) {
          ...
          vcollide_t *pVCollide =
              modelinfo->GetVCollide(pEntity->GetModelIndex());
        ...
        for (int i = collideList.Count() - 1; i >= 0; --i) {
          ...
          trace_t tr;
          physcollision->TraceCollide(element.origin, element.origin,
                                      element.pCollide, element.angles,
                                      pTriggerCollide, pTrigger->GetAbsOrigin(),
                                      pTrigger->GetAbsAngles(), &tr);
          if (tr.startsolid) return true;
        }
      } break;
...
int CChangeLevel::InTransitionVolume(CBaseEntity *pEntity,
                                     const char *pVolumeName) {
  CBaseEntity *pVolume;

  if (pEntity->ObjectCaps() & FCAP_FORCE_TRANSITION)
    return TRANSITION_VOLUME_PASSED;

  // If you're following another entity, follow it through the transition
  // (weapons follow the player)
  pEntity = pEntity->GetRootMoveParent();

  int inVolume = TRANSITION_VOLUME_NOT_FOUND;  // Unless we find a
                                               // trigger_transition, everything
                                               // is in the volume
  ...
  while (pVolume) {
    if (pVolume && FClassnameIs(pVolume, "trigger_transition")) {
      if (TestEntityTriggerIntersection_Accurate(
              pVolume, pEntity))  // It touches one, it's in the volume
        return TRANSITION_VOLUME_PASSED;

      inVolume = TRANSITION_VOLUME_SCREENED_OUT;  // Found a trigger_transition,
                                                  // but I don't intersect it --
                                                  // if I don't find another,
                                                  // don't go!
    }
...
int CChangeLevel::BuildChangeLevelList(levellist_t *pLevelList, int maxList) {
  ...
  CBaseEntity *pentChangelevel =
      gEntList.FindEntityByClassname(NULL, "trigger_changelevel");
  while (pentChangelevel) {
    ...
    if (pTrigger) {
      ...
      if (pentLandmark) {
        ...
        if (AddTransitionToList(pLevelList, nCount, pTrigger->m_szMapName,
    ...
    pentChangelevel =
        gEntList.FindEntityByClassname(pentChangelevel, "trigger_changelevel");
  }
...
//------------------------------------------------------------------------------
// Adds a single entity to the transition list, if appropriate. Returns the new
// count
//------------------------------------------------------------------------------
...
// count
//------------------------------------------------------------------------------
inline int CChangeLevel::AddEntityToTransitionList(CBaseEntity *pEntity,
                                                   int flags, int nCount,
                                                   CBaseEntity **ppEntList,
                                                   int *pEntityFlags) {
...
// Builds the list of entities to bring across a particular transition
//------------------------------------------------------------------------------
int CChangeLevel::BuildEntityTransitionList(CBaseEntity *pLandmarkEntity,
                                            const char *pLandmarkName,
                                            CBaseEntity **ppEntList,
                                            int *pEntityFlags, int nMaxList) {
  int iEntity = 0;
  ...
  if (g_debug_transitions.GetInt() &&
      pLandmarkEntity->NameMatches(st_szNextSpot)) {
    g_iDebuggingTransition = g_debug_transitions.GetInt();

    // Show us where the landmark entity is
    pLandmarkEntity->m_debugOverlays |=
        (OVERLAY_PIVOT_BIT | OVERLAY_BBOX_BIT | OVERLAY_NAME_BIT);
  } else {
    g_iDebuggingTransition = 0;
  }
  ...
  while ((pEntity = UTIL_EntitiesInPVS(pLandmarkEntity, pEntity)) != NULL) {
    ...
    iEntity = AddEntityToTransitionList(pEntity, flags, iEntity, ppEntList,
                                        pEntityFlags);
  }
...
int CChangeLevel::AddDependentEntities(int nCount, CBaseEntity **ppEntList,
                                       int *pEntityFlags, int nMaxList) {
  ...
  // NOTE: nCount will change value during this loop in
  // AddEntityToTransitionList
  for (i = 0; i < nCount; ++i) {
    CBaseEntity *pEntity = ppEntList[i];

    // Find dependencies in the hash.
    int nDepCount = pSaveUtils->GetEntityDependencyCount(pEntity);
    if (!nDepCount) continue;

    CBaseEntity **ppDependentEntities =
        (CBaseEntity **)stackalloc(nDepCount * sizeof(CBaseEntity *));
    ...
    for (int j = 0; j < nDepCount; ++j) {
      ...
      if (!pDependent) continue;

      // NOTE: Must use GetEntryIndex because we're saving non-networked
      // entities
      int nEntIndex = pDependent->GetRefEHandle().GetEntryIndex();

      // Don't re-add it if it's already in the list
      if (IsBitSet(pEntitiesSaved, nEntIndex)) continue;
      ...
      if (flags) {
        ...
        nCount = AddEntityToTransitionList(pEntity, flags, nCount, ppEntList,
                                           pEntityFlags);
      } else {
        Warning(
            "Warning!! Save dependency is linked to an entity that doesn't "
            "want to be saved!\n");
      }
    }
  }
...
int CChangeLevel::ChangeList(levellist_t *pLevelList, int maxList) {
  ...
  if (!gpGlobals->pSaveData ||
      (static_cast<CSaveRestoreData *>(gpGlobals->pSaveData)->NumEntities() ==
       0))
    return count;
  ...
  for (i = 0; i < count; i++) {
    ...
    CBaseEntity *pLandmarkEntity =
        CBaseEntity::Instance(pLevelList[i].pentLandmark);
    int iEntity =
...
class CTriggerPush : public CBaseTrigger {
 public:
  ...
  float m_flAlternateTicksFix;  // Scale factor to apply to the push speed when
                                // running with alternate ticks
  float m_flPushSpeed;
};
...
void CTriggerPush::Touch(CBaseEntity *pOther) {
  if (!pOther->IsSolid() || (pOther->GetMoveType() == MOVETYPE_PUSH ||
                             pOther->GetMoveType() == MOVETYPE_NONE))
    return;
  ...
  switch (pOther->GetMoveType()) {
    ...
    case MOVETYPE_VPHYSICS: {
      ...
      if (pPhys) {
        ...
        pPhys->ApplyForceCenter(m_flPushSpeed * vecAbsDir * 100.0f *
                                gpGlobals->frametime);
        return;
      }
    } break;
    ...
    default: {
#if defined(HL2_DLL)
      // HACK HACK  HL2 players on ladders will only be disengaged if the sf is
      // set, otherwise no push occurs.
      if (pOther->IsPlayer() && pOther->GetMoveType() == MOVETYPE_LADDER) {
      ...
      // apply x, y as a base velocity so we travel at constant speed on
      // conveyors
      vecPush.z = 0;
#endif
...
// Teleport trigger
//-----------------------------------------------------------------------------
const int SF_TELEPORT_PRESERVE_ANGLES =
    0x20;  // Preserve angles even when a local landmark is not specified
...
//			setting the toucher's angles to our target's angles if
// they  are a 			player.
//
//			If a landmark was specified, the toucher is offset from
// the  target 			by their initial offset from the landmark and
// their angles are 			left alone.
//
...
void CTriggerTeleport::Touch(CBaseEntity *pOther) {
  ...
  pentTarget =
      gEntList.FindEntityByName(pentTarget, m_target, NULL, pOther, pOther);
  if (!pentTarget) {
    return;
  }
  ...
  if (m_iLandmark != NULL_STRING) {
    // The activator and caller are the same
    pentLandmark = gEntList.FindEntityByName(pentLandmark, m_iLandmark, NULL,
                                             pOther, pOther);
    if (pentLandmark) {
  ...
  if (!pentLandmark && pOther->IsPlayer()) {
    // make origin adjustments in case the teleportee is a player. (origin in
    // center, not at feet)
    tmp.z -= pOther->WorldAlignMins().z;
  }

  //
  // Only modify the toucher's angles and zero their velocity if no landmark was
  // specified.
  //
...
#else
    pVelocity =
        NULL;  // BUGBUG - This does not set the player's velocity to zero!!!
#endif
  }
...
//-----------------------------------------------------------------------------
// Purpose: Saves the game when the player touches the trigger. Can be enabled
// or disabled
//-----------------------------------------------------------------------------
...
void CTriggerSave::Touch(CBaseEntity *pOther) {
  ...
  if (m_fDangerousTimer != 0.0f) {
    if (g_ServerGameDLL.m_fAutoSaveDangerousTime != 0.0f &&
  ...
  // this is a one-way transition - there is no way to return to the previous
  // map.
  ...
  if (m_fDangerousTimer != 0.0f) {
    ...
    if (pPlayer && pPlayer->GetHealth() >= m_minHitPoints) {
      ...
      g_ServerGameDLL.m_fAutoSaveDangerousTime =
          gpGlobals->curtime + m_fDangerousTimer;
    }
  } else {
    engine->ServerCommand("autosave\n");
  }
...
class CAI_ChangeHintGroup : public CBaseEntity {
 ...
 private:
  CAI_BaseNPC *FindQualifiedNPC(CAI_BaseNPC *pPrev, CBaseEntity *pActivator,
                                CBaseEntity *pCaller);
...
DEFINE_KEYFIELD(m_iSearchType, FIELD_INTEGER, "SearchType"),
  ...
  while (!pResult) {
    ...
    switch (m_iSearchType) {
      case 0: {
        pEntity = gEntList.FindEntityByNameWithin(pEntity, pszSearchName,
                                                  GetLocalOrigin(), m_flRadius,
                                                  NULL, pActivator, pCaller);
        break;
      }

      case 1: {
        pEntity = gEntList.FindEntityByClassnameWithin(
            pEntity, pszSearchName, GetLocalOrigin(), m_flRadius);
        break;
      }

      case 2: {
        pEntity = gEntList.FindEntityInSphere(
            pEntity, GetLocalOrigin(),
            (m_flRadius != 0.0) ? m_flRadius : FLT_MAX);
        break;
      }
    }
    ...
    if (pResult && m_iSearchType == 2 &&
        (!FStrEq(STRING(pResult->GetHintGroup()), pszSearchName))) {
      pResult = NULL;
    }
  }
...
void CAI_ChangeHintGroup::InputActivate(inputdata_t &inputdata) {
  ...
  while ((pTarget = FindQualifiedNPC(pTarget, inputdata.pActivator,
                                     inputdata.pCaller)) != NULL) {
...
DEFINE_FIELD(m_hPlayer, FIELD_EHANDLE), DEFINE_FIELD(m_hTarget, FIELD_EHANDLE),
  ...
  SetRenderColorA(0);    // The engine won't draw this model if this is set to 0
                         // and blending is on
  ...
  if (m_acceleration == 0) m_acceleration = 500;

  if (m_deceleration == 0) m_deceleration = 500;
...
// Purpose: Input handler to turn off this trigger.
//------------------------------------------------------------------------------
void CTriggerCamera::InputDisable(inputdata_t &inputdata) { Disable(); }
...
void CTriggerCamera::Enable(void) {
  ...
  // if the player was already under control of a similar trigger, disable the
  // previous trigger.
  {
    CBaseEntity *pPrevViewControl = pPlayer->GetViewEntity();
    if (pPrevViewControl && pPrevViewControl != pPlayer) {
      CTriggerCamera *pOtherCamera =
          dynamic_cast<CTriggerCamera *>(pPrevViewControl);
      if (pOtherCamera) {
  ...
  // Make the player invulnerable while under control of the camera.  This will
  // prevent situations where the player dies while under camera control but
  // cannot restart their game due to disabled player inputs.
  ...
  if (m_hTarget) {
    ...
    if (m_iszTargetAttachment != NULL_STRING) {
      if (!m_hTarget->GetBaseAnimating()) {
        Warning(
            "%s tried to target an attachment (%s) on target %s, which has no "
            "model.\n",
            GetClassname(), STRING(m_iszTargetAttachment),
            STRING(m_hTarget->GetEntityName()));
      } else {
        m_iAttachmentIndex = m_hTarget->GetBaseAnimating()->LookupAttachment(
            STRING(m_iszTargetAttachment));
  ...
  if (m_pPath) {
    if (m_pPath->m_flSpeed != 0) m_targetSpeed = m_pPath->m_flSpeed;
...
void CTriggerCamera::Disable(void) {
  ...
  m_OnEndFollow.FireOutput(
      this, this);  // dvsents2: what is the best name for this output?
...
// Purpose:
//-----------------------------------------------------------------------------
void CTriggerCamera::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
void CTriggerCamera::FollowTarget() {
  ...
  if (!HasSpawnFlags(SF_CAMERA_PLAYER_INFINITE_WAIT) &&
      (!m_hTarget || m_flReturnTime < gpGlobals->curtime)) {
    Disable();
    return;
  }
  ...
  if (m_bSnapToGoal) {
    ...
    QAngle vecAngVel;
    vecAngVel.Init(dx * 40 * gpGlobals->frametime,
                   dy * 40 * gpGlobals->frametime, GetLocalAngularVelocity().z);
...
void CTriggerCamera::Move() {
  ...
  // episodic we add the capacity for interpolation to the start point.
#if HL2_EPISODIC
  if (m_pPath)
#else
...
#endif
  {
    ...
    if (m_flStopTime > gpGlobals->curtime)
      m_flSpeed =
          UTIL_Approach(0, m_flSpeed, m_deceleration * gpGlobals->frametime);
    else
      m_flSpeed = UTIL_Approach(m_targetSpeed, m_flSpeed,
                                m_acceleration * gpGlobals->frametime);

    float fraction = 2 * gpGlobals->frametime;
    SetAbsVelocity(((m_vecMoveDir * m_flSpeed) * fraction) +
                   (GetAbsVelocity() * (1 - fraction)));
  }
#if HL2_EPISODIC
    ...
    if (tt >= 1.0f) {
      ...
      Vector nextPos =
          ((m_vEndPos - m_vStartPos) * SimpleSpline(tt)) + m_vStartPos;
      // rather than stomping origin, set the velocity so that we get there in
      // the proper time
      Vector desiredVel =
          (nextPos - GetAbsOrigin()) * (1.0f / gpGlobals->frametime);
...
void CTriggerCDAudio::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                          USE_TYPE useType, float value) {
  PlayTrack();
}
...
//			the trigger, provided they are within a given radius of
// the  specified 			entity. The nearest entity distance is
// output as a number  from [0 - 1].
//-----------------------------------------------------------------------------
class CTriggerProximity : public CBaseTrigger {
 ...
 protected:
  ...
  float m_fRadius;  // The radius around the measure target that we measure
                    // within.
...
void CTriggerProximity::Activate(void) {
  ...
  if ((m_hMeasureTarget == NULL) || (m_hMeasureTarget->edict() == NULL)) {
    Warning(
        "TriggerProximity - Missing measure target or measure target with no "
        "origin!\n");
  }
}

//-----------------------------------------------------------------------------
// Purpose: Decrements the touch count and cancels the think if the count
// reaches
//			zero.
// Input  : pOther -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Think function called every frame as long as we have entities
// touching
//			us that we care about. Finds the closest entity to the
// measure 			target and outputs the distance as a normalized
// value from [0..1].
//-----------------------------------------------------------------------------
  ...
  if (root) {
    ...
    while (pLink != root) {
      ...
      if ((pEntity != NULL) && PassesTriggerFilters(pEntity)) {
        float flDistance =
            (pEntity->GetLocalOrigin() - m_hMeasureTarget->GetLocalOrigin())
                .Length();
...
class CPhysicsWind : public IMotionEvent {
 ...
 public:
  simresult_e Simulate(IPhysicsMotionController *pController,
                       IPhysicsObject *pObject, float deltaTime, Vector &linear,
                       AngularImpulse &angular) {
    // If we have no windspeed, we're not doing anything
    if (!m_flWindSpeed) return IMotionEvent::SIM_NOTHING;
    ...
    vWindAngle[1] =
        m_nWindYaw + (30 * cos(nNoiseMod * gpGlobals->curtime + nNoiseMod));
    Vector vWind;
...
DEFINE_FIELD(m_nSpeedCurrent, FIELD_INTEGER),
  ...
  SetContextThink(&CTriggerWind::WindThink, gpGlobals->curtime,
                  WIND_THINK_CONTEXT);
}
...
void CTriggerWind::InputEnable(inputdata_t &inputdata) {
  ...
  SetContextThink(&CTriggerWind::WindThink, gpGlobals->curtime + 0.1f,
                  WIND_THINK_CONTEXT);
}
...
void CTriggerWind::WindThink(void) {
  // By default...
  SetContextThink(&CTriggerWind::WindThink, gpGlobals->curtime + 0.1,
                  WIND_THINK_CONTEXT);
  ...
  if (m_bSwitch) {
    ...
    m_nSpeedTarget =
        m_nSpeedBase + random->RandomInt(-m_nSpeedNoise, m_nSpeedNoise);
    m_nDirTarget = UTIL_AngleMod(m_nDirBase +
                                 random->RandomInt(-m_nDirNoise, m_nDirNoise));
  } else {
    bool bDone = true;
    // either ramp up, or sleep till change
    if (abs(m_nSpeedTarget - m_nSpeedCurrent) > MAX_WIND_CHANGE) {
      m_nSpeedCurrent += (m_nSpeedTarget > m_nSpeedCurrent) ? MAX_WIND_CHANGE
                                                            : -MAX_WIND_CHANGE;
      bDone = false;
    }
    ...
    if (abs(m_nDirTarget - m_nDirCurrent) > MAX_WIND_CHANGE) {
      m_nDirCurrent =
          UTIL_ApproachAngle(m_nDirTarget, m_nDirCurrent, MAX_WIND_CHANGE);
      bDone = false;
    }
    ...
    if (bDone) {
      ...
      SetContextThink(
          ...
          m_nHoldBase + random->RandomFloat(-m_nHoldNoise, m_nHoldNoise),
          WIND_THINK_CONTEXT);
      m_bSwitch = true;
    }
  }
...
int CTriggerWind::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // --------------
    char tempstr[255];
    Q_snprintf(tempstr, sizeof(tempstr), "Dir: %i (%i)", m_nDirCurrent,
               m_nDirTarget);
    EntityText(text_offset, tempstr, 0);
    text_offset++;

    Q_snprintf(tempstr, sizeof(tempstr), "Speed: %i (%i)", m_nSpeedCurrent,
               m_nSpeedTarget);
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
...
class CTriggerImpact : public CTriggerMultiple {
 ...
 public:
  ...
  // Outputs
  COutputVector
      m_pOutputForce;  // Output force in case anyone else wants to use it
...
void CTriggerImpact::InputImpact(inputdata_t &inputdata) {
  ...
  m_pOutputForce.Set(m_flMagnitude * vDir, inputdata.pActivator,
                     inputdata.pCaller);
...
void CTriggerImpact::StartTouch(CBaseEntity *pOther) {
  ...
  if (pOther->IsPlayer() && fabs(m_flMagnitude) > 0) {
    ...
    float flPunch =
        -m_flViewkick * m_flMagnitude * TRIGGERIMPACT_VIEWKICK_SCALE;
...
void CTriggerPlayerMovement::Spawn(void) {
  if (HasSpawnFlags(SF_TRIGGER_ONLY_PLAYER_ALLY_NPCS)) {
    // @Note (toml 01-07-04): fix up spawn flag collision coding error. Remove
    // at some point once all maps fixed up please!
    DevMsg(
        "*** trigger_playermovement using obsolete spawnflag. Remove and reset "
        "with new value for \"Disable auto player movement\"\n");
...
//=====================================================================================================================
//-----------------------------------------------------------------------------
// Purpose: VPhysics trigger that changes the motion of vphysics objects that
// touch it
//-----------------------------------------------------------------------------
class CTriggerVPhysicsMotion : public CBaseVPhysicsTrigger,
                               public IMotionEvent {
 ...
 public:
  ...
  // UNDONE: Pass trigger event in or change Start/EndTouch.  Add
  // ITriggerVPhysics perhaps? BUGBUG: If a player touches two of these, his
  // movement will screw up. BUGBUG: If a player uses crouch/uncrouch it will
  // generate touch events and clear the motioncontroller flag
  void StartTouch(CBaseEntity *pOther);
  void EndTouch(CBaseEntity *pOther);
...
float CTriggerVPhysicsMotion::LinearLimit() {
  if (m_linearLimitTime == 0.0f) return m_linearLimit;
...
// Start/End Touch
//------------------------------------------------------------------------------
// UNDONE: Pass trigger event in or change Start/EndTouch.  Add ITriggerVPhysics
// perhaps? BUGBUG: If a player touches two of these, his movement will screw
// up. BUGBUG: If a player uses crouch/uncrouch it will generate touch events
// and clear the motioncontroller flag
...
//------------------------------------------------------------------------------
IMotionEvent::simresult_e CTriggerVPhysicsMotion::Simulate(
    IPhysicsMotionController *pController, IPhysicsObject *pObject,
    float deltaTime, Vector &linear, AngularImpulse &angular) {
  if (m_bDisabled) return SIM_NOTHING;

  linear.Init();
  angular.Init();

  if (HasGravityScale()) {
    // assume object already has 1.0 gravities applied to it, so apply the
    // additional amount
  ...
  if (HasAirDensity() || HasLinearLimit() || HasLinearScale() ||
      HasAngularLimit() || HasAngularScale()) {
    Vector vel;
    ...
    if (HasAirDensity()) {
      float linearDrag = -0.5 * m_addAirDensity *
                         pObject->CalculateLinearDrag(unitVel) * deltaTime;
      if (linearDrag < -1) {
        linearDrag = -1;
      }
      speedScale += linearDrag / deltaTime;
      float angDrag = -0.5 * m_addAirDensity *
                      pObject->CalculateAngularDrag(unitAngVel) * deltaTime;
      if (angDrag < -1) {
        angDrag = -1;
      }
...
bool IsTriggerClass(CBaseEntity *pEntity) {
  if (NULL != dynamic_cast<CBaseTrigger *>(pEntity)) return true;

  if (NULL != dynamic_cast<CTriggerVPhysicsMotion *>(pEntity)) return true;

  if (NULL != dynamic_cast<CTriggerVolume *>(pEntity)) return true;
