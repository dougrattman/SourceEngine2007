...
ConVar fish_dormant("fish_dormant", "0", FCVAR_REPLICATED | FCVAR_CHEAT,
                    "Turns off interactive fish behavior. Fish become immobile "
                    "and unresponsive.");
...
BEGIN_DATADESC(CFish)
  DEFINE_FIELD(m_pool, FIELD_EHANDLE), DEFINE_FIELD(m_id, FIELD_INTEGER),
      DEFINE_FIELD(m_angle, FIELD_FLOAT),
      DEFINE_FIELD(m_angleChange, FIELD_FLOAT),
      DEFINE_FIELD(m_forward, FIELD_VECTOR), DEFINE_FIELD(m_perp, FIELD_VECTOR),
 ...
 */
void SendProxy_FishOriginX(const SendProp *pProp, const void *pStruct,
...
void SendProxy_FishOriginY(const SendProp *pProp, const void *pStruct,
                           const void *pData, DVariant *pOut, int iElement,
                           int objectID) {
  CFish *fish = (CFish *)pStruct;
  Assert(fish);
...
SendPropVector(SENDINFO(m_poolOrigin), -1, SPROP_COORD, 0.0f,
               HIGH_DEFAULT),  // only sent once

    SendPropFloat(SENDINFO(m_angle), 7, 0 /*SPROP_CHANGES_OFTEN*/, 0.0f, 360.0f,
                  SendProxy_FishAngle),
 ...
 * 'amount' ranges from zero to one, representing the amount of flocking
 * influence allowed If 'other' is NULL, flock to the center of the pool.
 */
void CFish::FlockTo(CFish *other, float amount) {
  // allow fish to disperse a bit at round start
  if (!m_disperseTimer.IsElapsed()) return;

  const float maxRange = (other) ? 100.0f : 300.0f;

  Vector to = (other) ? (other->GetAbsOrigin() - GetAbsOrigin())
                      : (m_pool->GetAbsOrigin() - GetAbsOrigin());
...
float CFish::Avoid(void) {
  ...
  UTIL_TraceLine(
      GetAbsOrigin(),
      GetAbsOrigin() + m_avoidRange * (m_forward + sideOffset * m_perp),
      MASK_PLAYERSOLID, this, COLLISION_GROUP_NONE, &result);
...
void CFish::Update(float deltaT) {
  ...
  // pass relative position to the client
  // clamp them here to cover the rare cases where a fish's high velocity skirts
  // the range limit
  m_x = std::clamp(deltaPos.x, -255.0f, 255.0f);
  m_y = std::clamp(deltaPos.y, -255.0f, 255.0f);
  m_z = m_poolOrigin->z;
  ...
  const float maxAngleChange =
      (25.0f + 175.0f * (m_speed / m_panicSpeed)) * deltaT;
 ...
 */
void CFish::ResetVisible(void) { m_visible.RemoveAll(); }

//-------------------------------------------------------------------------------------------------------------
/**
 * Add this fish to our visible vector
 */
void CFish::AddVisible(CFish *fish) { m_visible.AddToTail(fish); }

//-------------------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------------------
/**
 * A CFishPool manages a collection of CFish, and defines where the "pool" is in
 * the world.
 */
...
void CFishPool::Spawn() {
  ...
  m_waterLevel = UTIL_WaterLevel(GetAbsOrigin(), GetAbsOrigin().z,
                                 GetAbsOrigin().z + 1000.0f);
...
void CFishPool::FireGameEvent(IGameEvent *event) {
  ...
  float range =
      (Q_strcmp("player_footstep", event->GetName())) ? loudRange : quietRange;

  for (int i = 0; i < m_fishes.Count(); ++i) {
    // if player is NULL, assume a game-wide event
    if (player && (player->GetAbsOrigin() - m_fishes[i]->GetAbsOrigin())
                      .IsLengthGreaterThan(range)) {
...
void CFishPool::Update(void) {
  ...
  if (m_visTimer.IsElapsed()) {
    ...
    for (i = 0; i < m_fishes.Count(); ++i) {
      if (!m_fishes[i]->IsAlive()) continue;

      for (j = i + 1; j < m_fishes.Count(); ++j) {
        if (!m_fishes[j]->IsAlive()) continue;

        UTIL_TraceLine(m_fishes[i]->GetAbsOrigin(), m_fishes[j]->GetAbsOrigin(),
                       MASK_PLAYERSOLID, m_fishes[i], COLLISION_GROUP_NONE,
                       &result);
        if (result.fraction >= 1.0f) {
