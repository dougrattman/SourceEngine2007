...
enum PlayerPhysFlag_e {
  ...
  PFLAG_GAMEPHYSICS_ROTPUSH = (1 << 5),  // game physics did a rotating push
                                         // that we may want to override with
                                         // vphysics
...
class CBasePlayer : public CBaseCombatCharacter {
 ...
 public:
  ...
  // Networking is about to update this entity, let it override and specify it's
  // own pvs
  ...
  // this player runs usercmds. Saves a lot of overhead on the server if we can
  // cull out entities that don't need to lag compensate (like team members,
  // entities out of our PVS, etc).
  ...
  virtual bool IsPlayer(void) const {
    return true;
  }  // Spectators return TRUE for this, use IsObserver to seperate cases
  ...
  void CalcView(Vector &eyeOrigin, QAngle &eyeAngles, float &zNear, float &zFar,
                float &fov);
  ...
  virtual void ValidateCurrentObserverTarget(void);  // Checks the current
                                                     // observer target, and
                                                     // moves on if it's not
                                                     // valid anymore
  ...
  // Run a user command. The default implementation calls ::PlayerRunCommand. In
  // TF, this controls a vehicle if the player is in one.
 ...
 public:
  ...
  void SetDefaultFOV(
      int FOV);  // Sets the base FOV if nothing else is affecting it by zooming
  ...
  void ActivateMovementConstraint(CBaseEntity *pEntity, const Vector &vecCenter,
                                  float flRadius, float flConstraintWidth,
                                  float flSpeedFactor);
 ...
 protected:
  ...
  CNetworkHandle(CBaseEntity, m_hUseEntity);  // the player is currently
                                              // controlling this entity because
                                              // of +USE latched, NULL if no
                                              // entity
  ...
  float m_flDeathAnimTime;  // the time at which the player finished their death
                            // anim (used in PlayerDeathThink() and
                            // ShouldTransmit())
  ...
  bool m_bForcedObserverMode;  // true, player was forced by invalid targets to
                               // switch mode

  CNetworkHandle(CBaseEntity, m_hZoomOwner);  // This is a pointer to the entity
  ...
  float m_fNextSuicideTime;  // the time after which the player can next use the
                             // suicide command
 ...
 protected:  // used to be private, but need access for portal mod (Dave
             // Kircher)
 ...
 private:
  ...
  int m_iUpdateTime;     // stores the number of frame ticks before sending HUD
                         // update messages
  int m_iClientBattery;  // the Battery currently known by the client.  If this
                         // changes, send a new
 ...
 protected:
  ...
  Vector m_vecPreviouslyPredictedOrigin;  // Used to determine if
                                          ...
                                          // teleported, or tweaked the player's
                                          // origin
