...
//
// Purpose: A planar textured surface that breaks into increasingly smaller
// fragments
//			as it takes damage. Undamaged pieces remain attached to
// the world 			until they are damaged. Used for window panes.
...
BEGIN_DATADESC(CWindowPane)
  ...
  // Function Pointers
  DEFINE_FUNCTION(Die), DEFINE_FUNCTION(PaneTouch),
...
void CWindowPane::Die(void) {
  ...
  te->ShatterSurface(filter, 0.0, &GetAbsOrigin(), &GetAbsAngles(),
                     &GetAbsVelocity(), &GetAbsOrigin(), WINDOW_PANEL_SIZE,
                     WINDOW_PANEL_SIZE, WINDOW_SMALL_SHARD_SIZE,
                     SHATTERSURFACE_GLASS, 255, 255, 255, 255, 255, 255);
...
// Output  :
//------------------------------------------------------------------------------
CWindowPane *CWindowPane::CreateWindowPane(const Vector &vecOrigin,
...
BEGIN_DATADESC(CBreakableSurface)
  ...
  DEFINE_KEYFIELD(m_nSurfaceType, FIELD_INTEGER, "surfacetype"),
      ...
      //			We should use client-side save/load to fix this
      // problem.
      DEFINE_AUTO_ARRAY2D(m_flSupport, FIELD_FLOAT),
      DEFINE_ARRAY(m_RawPanelBitVec, FIELD_BOOLEAN,
                   MAX_NUM_PANELS * MAX_NUM_PANELS),

      // Function Pointers
      DEFINE_THINKFUNC(BreakThink), DEFINE_ENTITYFUNC(SurfaceTouch),

      DEFINE_INPUTFUNC(FIELD_VECTOR, "Shatter", InputShatter),

  // DEFINE_FIELD( m_ForceUpdateClientData, CBitVec < MAX_PLAYERS > ),  // No
  // need to save/restore this, it's just a temporary flag field
END_DATADESC()
...
SendPropInt(SENDINFO(m_nNumWide), 8, SPROP_UNSIGNED),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_RawPanelBitVec),
                   SendPropInt(SENDINFO_ARRAY(m_RawPanelBitVec), 1,
                               SPROP_UNSIGNED)),
    END_SEND_TABLE()
...
void CBreakableSurface::SurfaceTouch(CBaseEntity *pOther) {
  ...
  int nMinWidth = Floor2Int(std::max(0.0f, std::min(flMinsWidth, flMaxsWidth)));
  int nMaxWidth = Ceil2Int(std::min(static_cast<float>(m_nNumWide),
                                    std::max(flMinsWidth, flMaxsWidth)));

  int nMinHeight =
      Floor2Int(std::max(0.0f, std::min(flMinsHeight, flMaxsHeight)));
  int nMaxHeight = Ceil2Int(std::min(static_cast<float>(m_nNumHigh),
                                     std::max(flMinsHeight, flMaxsHeight)));
...
//------------------------------------------------------------------------------
void CBreakableSurface::TraceAttack(const CTakeDamageInfo &info,
    ...
    if (m_nSurfaceType == SHATTERSURFACE_TILE) {
      ...
      for (int width = nWidth - 4; width < nWidth + 4; width++) {
        for (int height = nHeight - 4; height < nHeight + 4; height++) {
          if ((abs(nWidth - width) + abs(nHeight - height)) <
              random->RandomInt(2, 5)) {
    ...
    // ----------------------------------------
      ...
      // ---------------------------------------------------------------
        ...
        CreateShards(m_vCorner, vAngles, vBlastDir, ptr->endpos,
                     m_nNumWide * m_flPanelWidth, m_nNumHigh * m_flPanelHeight,
        ...
        for (int width = 0; width < m_nNumWide; width++) {
          ...
          for (height = 0; height < m_nNumHigh; height++) {
            ...
            // Shatter the strip and start counting again
              ...
              Vector vBreakPos =
                  m_vCorner + (width * vWidthDir * m_flPanelWidth) +
                  ((height - nHCount) * vHeightDir * m_flPanelHeight);

              CreateShards(vBreakPos, vAngles, vBlastDir, ptr->endpos,
...
void CBreakableSurface::InputShatter(inputdata_t &inputdata) {
  ...
  Vector2D vecActualCenter(flCenterX * m_flPanelWidth,
                           flCenterY * m_flPanelHeight);
  for (int width = nMinX; width < nMaxX; width++) {
    for (int height = nMinY; height < nMaxY; height++) {
      Vector2D pt((width + 0.5f) * m_flPanelWidth,
                  (height + 0.5f) * m_flPanelWidth);
      if (pt.DistToSqr(vecActualCenter) <=
          vecShatterInfo.z * vecShatterInfo.z) {
        Vector vBreakPos = m_vCorner + (width * vWidthDir * m_flPanelWidth) +
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableSurface::Event_Killed(CBaseEntity *pInflictor,
                                     CBaseEntity *pAttacker, float flDamage,
                                     int bitsDamageType) {
  return;
}
...
float CBreakableSurface::GetSupport(int nWidth, int nHeight) {
  return std::max(0.0f, m_flSupport[nWidth][nHeight]);
}

//------------------------------------------------------------------------------
// Purpose : Return the structural support for this pane.  Assumes window
//			 is upright.  Still works for windows parallel to the
// ground 			 but simulation isn't quite as good
// Input   :
// Output  :
//------------------------------------------------------------------------------
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableSurface::PanePos(const Vector &vPos, float *flWidth,
                                float *flHeight) {
...
void CBreakableSurface::DropPane(int nWidth, int nHeight) {
  ...
  if (!IsBroken(nWidth, nHeight)) {
    ...
    Vector vWidthDir, vHeightDir;
    AngleVectors(vAngles, NULL, &vWidthDir, &vHeightDir);
    Vector vBreakPos = m_vCorner + (nWidth * vWidthDir * m_flPanelWidth) +
                       (nHeight * vHeightDir * m_flPanelHeight);

    CreateShards(vBreakPos, vAngles, vec3_origin, vec3_origin,
                 WINDOW_PANEL_SIZE, WINDOW_PANEL_SIZE, WINDOW_SMALL_SHARD_SIZE);
...
void CBreakableSurface::CreateShards(const Vector &vBreakPos,
                                     const QAngle &vAngles,
                                     const Vector &vForce,
                                     const Vector &vForcePos, float flWidth,
                                     float flHeight, int nShardSize) {
  ...
  te->ShatterSurface(filter, 0.0, &vAdjustedBreakPos, &vAngles, &vAdjustedForce,
                     &vForcePos, flWidth, flHeight, WINDOW_SMALL_SHARD_SIZE,
                     m_nSurfaceType, front_r, front_g, front_b, back_r, back_g,
                     back_b);  // 4);
}
...
// Output  :
//------------------------------------------------------------------------------
bool CBreakableSurface::ShatterPane(int nWidth, int nHeight,
                                    const Vector &vForce,
  ...
  if (IsBroken(nWidth, nHeight)) return false;
  ...
  CreateShards(vBreakPos, vAngles, vForce, vForcePos, m_flPanelWidth,
               m_flPanelHeight, WINDOW_SMALL_SHARD_SIZE);
...
void CBreakableSurface::Spawn(void) {
  ...
  if (m_nQuadError == QUAD_ERR_MULT_FACES) {
    Warning(
        "Rejecting func_breakablesurf.  Has multiple faces that aren't "
        "NODRAW.\n");
    UTIL_Remove(this);
  ...
  int materialCount =
      modelinfo->GetModelMaterialCount(const_cast<model_t *>(GetModel()));
  if (materialCount != 1) {
    Warning(
        "Encountered func_breakablesurf that has a material applied to more "
        "than one surface!\n");
    UTIL_Remove(this);
  }
  ...
  IMaterial *pMaterial;
  modelinfo->GetModelMaterials(const_cast<model_t *>(GetModel()), 1,
                               &pMaterial);

  // The material should point to a cracked version of itself
  bool foundVar;
  IMaterialVar *pCrackName =
      pMaterial->FindVar("$crackmaterial", &foundVar, false);
  if (foundVar) {
...
void CBreakableSurface::VPhysicsCollision(int index,
                                          gamevcollisionevent_t *pEvent) {
  if (!m_bIsBroken) {
    int damageType = 0;
    string_t iszDamageTable =
        ((m_nSurfaceType == SHATTERSURFACE_GLASS) ? ("glass") : (NULL_STRING));
    bool bDamageFromHeldObjects =
        ((m_spawnflags & SF_BREAKABLESURF_DAMAGE_FROM_HELD_OBJECTS) != 0);
    float damage =
        CalculateDefaultPhysicsDamage(index, pEvent, 1.0, false, damageType,
                                      iszDamageTable, bDamageFromHeldObjects);

    if (damage > 10) {
      // HACKHACK: Reset mass to get correct collision response for the object
      // breaking this
      ...
      CTakeDamageInfo info(pInflictor, pInflictor, normal, damagePos, damage,
                           damageType);
      ...
      if (m_spawnflags & SF_BREAKABLESURF_CRACK_DECALS) {
        ...
        trace_t tr;
        UTIL_TraceLine(damagePos - normal, damagePos + normal,
