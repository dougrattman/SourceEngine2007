...
BEGIN_DATADESC(CNPCSimpleTalker)
  DEFINE_FIELD(m_useTime, FIELD_TIME),
      ...
      // 							m_FollowBehavior (auto
      // saved by AI) Function Pointers
...
void CNPCSimpleTalker::BuildScheduleTestBits(void) {
  ...
  if (ConditionInterruptsCurSchedule(COND_PLAYER_PUSHING) ||
      ConditionInterruptsCurSchedule(COND_PROVOKED)) {
...
bool CNPCSimpleTalker::ShouldSuspendMonolog(void) {
  ...
  flDist = ((assert_cast<CNPCSimpleTalkerExpresser *>(GetExpresser()))
                ->GetMonologueTarget()
                ->GetAbsOrigin() -
            GetAbsOrigin())
               .Length();
...
void CNPCSimpleTalker::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_TALKER_WAIT_FOR_SEMAPHORE:
      if (GetExpresser()->SemaphoreIsAvailable(this)) TaskComplete();
      break;
  ...
  switch (pTask->iTask) {
    ...
    case TASK_TALKER_LOOK_AT_CLIENT:
      ...
      if (pTask->iTask == TASK_TALKER_CLIENT_STARE && AI_IsSinglePlayer()) {
        ...
        if ((pPlayer->GetAbsOrigin() - GetAbsOrigin()).Length2D() >
            TALKER_STARE_DIST) {
          // player moved away.
        ...
        if (UTIL_DotPoints(pPlayer->GetAbsOrigin(), GetAbsOrigin(), forward) <
            m_flFieldOfView) {
          // player looked away
...
// Purpose:
//-----------------------------------------------------------------------------
CBaseEntity *CNPCSimpleTalker::EnumFriends(CBaseEntity *pPrevious,
                                           int listNumber, bool bTrace) {
  ...
  while (pszFriend != NULL && ((pFriend = gEntList.FindEntityByClassname(
                                    pFriend, pszFriend)) != NULL)) {
    ...
    if (bTrace) {
      Vector vecCheck;
      pFriend->CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 1.0f),
                                                       &vecCheck);
      UTIL_TraceLine(GetAbsOrigin(), vecCheck, MASK_SOLID_BRUSHONLY, this,
      ...
      if (pNPC->IsAlive()) {
        ...
        if (pKiller->GetFlags() & FL_CLIENT) {
          ...
          if (IRelationType(pKiller) == D_HT) {
            ...
            if (pAlly &&
                pAlly->GetExpresser()->CanSpeakConcept(TLK_ALLY_KILLED)) {
...
// UNDONE: Keep a follow time in each follower, make a list of followers in this
// function and do LRU UNDONE: Check this in Restore to keep restored NPCs from
// joining a full list of followers
      ...
      if (pNPC) {
        if (pNPC->GetTarget() == pPlayer) {
          ...
          if (count > maxFollowers &&
              (pTalker = dynamic_cast<CNPCSimpleTalker *>(pNPC)) != NULL)
...
void CNPCSimpleTalker::HandleAnimEvent(animevent_t *pEvent) {
  switch (pEvent->event) {
    case SCRIPT_EVENT_SENTENCE_RND1:  // Play a named sentence group 25% of the
                                      // time
      if (random->RandomInt(0, 99) < 75) break;
      // fall through...
...
//-----------------------------------------------------------------------------
// Purpose: Scan for nearest, visible friend. If fPlayer is true, look for
// nearest player
//-----------------------------------------------------------------------------
...
void CNPCSimpleTalker::IdleRespond(void) {
  if (!IsOkToSpeak()) return;

  // play response
  SpeakAnswerFriend(GetSpeechTarget());

  DeferAllIdleSpeech(random->RandomFloat(TALKER_DEFER_IDLE_SPEAK_MIN,
                                         TALKER_DEFER_IDLE_SPEAK_MAX));
}

bool CNPCSimpleTalker::IsOkToSpeak(void) {
  if (m_flNextIdleSpeechTime > gpGlobals->curtime) return false;
...
int CNPCSimpleTalker::FIdleSpeak(void) {
  ...
  if (pTarget != NULL) {
    if (pTarget->IsPlayer()) {
      if (pTarget->IsAlive()) {
        ...
        //!!!KELLY - here's a cool spot to have the talkNPC talk about the dead
        //!player if we want.
  ...
  /*
  // if there is a friend nearby to speak to, play sentence, set friend's
  response time, return CBaseEntity *pFriend = FindNearestFriend(false);
  ...
  if (pFriend && !(pFriend->IsMoving()) && random->RandomInt( 0, 3 ) != 0 )
  {
          if ( SpeakQuestionFriend( pFriend ) )
          {
                  ...
                  CAI_PlayerAlly *pTalkNPC = dynamic_cast<CAI_PlayerAlly
  *>(pFriend); if (pTalkNPC && !pTalkNPC->HasSpawnFlags(SF_NPC_GAG) &&
  !pTalkNPC->IsInAScript() )
                  {
                          SetSpeechTarget( pFriend );
                          pTalkNPC->SetAnswerQuestion( this );
                          pTalkNPC->GetExpresser()->BlockSpeechUntil(
  GetExpresser()->GetTimeSpeechComplete() );

                          m_nSpeak++;
                  }

                  // Don't let anyone else butt in.
                  DeferAllIdleSpeech( random->RandomFloat(
  TALKER_DEFER_IDLE_SPEAK_MIN, TALKER_DEFER_IDLE_SPEAK_MAX ), pTalkNPC ); return
  true;
          }
  }
  */

  // Otherwise, play an idle statement, try to face client when making a
  // statement.
  CBaseEntity *pFriend = FindNearestFriend(true);
  if (pFriend) {
    SetSpeechTarget(pFriend);

    // If we're about to talk to the player, and we've never said hello, say
    // hello first
...
void CNPCSimpleTalker::FIdleSpeakWhileMoving(void) {
  if (GetExpresser()->CanSpeak()) {
    if (!GetExpresser()->IsSpeaking() || GetSpeechTarget() == NULL) {
      ...
      if (ShouldSpeakRandom(
              m_nSpeak * 20,
              GetSpeechFilter() ? GetSpeechFilter()->GetIdleModifier() : 1.0)) {
        FIdleSpeak();
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPCSimpleTalker::PlayScriptedSentence(const char *pszSentence, float delay,
                                           float volume,
                                           soundlevel_t soundlevel,
                                           bool bConcurrent,
                                           CBaseEntity *pListener) {
  if (!bConcurrent) ShutUpFriends();

  int sentenceIndex = BaseClass::PlayScriptedSentence(
      pszSentence, delay, volume, soundlevel, bConcurrent, pListener);
...
int CNPCSimpleTalker::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (subInfo.GetAttacker() &&
      (subInfo.GetAttacker()->GetFlags() & FL_CLIENT) &&
      subInfo.GetDamage() < GetHealth()) {
...
int CNPCSimpleTalker::SelectNonCombatSpeechSchedule() {
  if (!IsOkToSpeak()) return SCHED_NONE;

  // talk about world
  if (ShouldSpeakRandom(m_nSpeak * 2, GetSpeechFilter()
                                          ? GetSpeechFilter()->GetIdleModifier()
                                          : 1.0)) {
  ...
  if (AI_IsSinglePlayer() && GetExpresser()->CanSpeak() &&
      HasCondition(COND_SEE_PLAYER) && random->RandomInt(0, 6) == 0) {
    CBasePlayer *pPlayer = UTIL_GetLocalPlayer();
    Assert(pPlayer);
    ...
    if (pPlayer) {
      ...
      if ((pPlayer->GetAbsOrigin() - GetAbsOrigin()).Length2D() <
              TALKER_STARE_DIST &&
          UTIL_DotPoints(pPlayer->GetAbsOrigin(), GetAbsOrigin(), forward) >=
              m_flFieldOfView) {
        // go into the special STARE schedule if the player is close, and
        // looking at me too.
...
#ifndef HL1_DLL
  if (Classify() == CLASS_PLAYER_ALLY_VITAL) return false;
#endif
...
void CNPCSimpleTalker::OnStartingFollow(CBaseEntity *pTarget) {
  GetExpresser()->SetSpokeConcept(
      TLK_HELLO, NULL);  // Don't say hi after you've started following
  if (IsOkToSpeak())     // don't speak if idle talk is blocked. player
                         // commanded/use follow will always speak
...
void CNPCSimpleTalker::FollowerUse(CBaseEntity *pActivator,
...
int CNPCSimpleTalkerExpresser::SpeakRawSentence(const char *pszSentence,
  ...
  return CAI_Expresser::SpeakRawSentence(pszSentence, delay, volume, soundlevel,
                                         pListener);
}

//-------------------------------------

void CNPCSimpleTalkerExpresser::BeginMonolog(char *pszSentenceName,
                                             CBaseEntity *pListener) {
  if (pListener) {
  ...
  // change the "AI_SP_START_MONOLOG" to an "AI_SP_MONOLOG_LINE".
  // m_sMonologSentence is now the string we'll tack numbers onto to play
  // sentences from this group in sequential order.
...
void CNPCSimpleTalkerExpresser::SpeakMonolog(void) {
  ...
  if (CanSpeak()) {
    ...
    Q_snprintf(szSentence, sizeof(szSentence), "%s%d", m_szMonologSentence,
               m_iMonologIndex);
    m_iMonologIndex++;
...
void CNPCSimpleTalker::OnChangeRunningBehavior(CAI_BehaviorBase *pOldBehavior,
                                               CAI_BehaviorBase *pNewBehavior) {
  BaseClass::OnChangeRunningBehavior(pOldBehavior, pNewBehavior);

  CAI_FollowBehavior *pFollowBehavior;
  if ((pFollowBehavior = dynamic_cast<CAI_FollowBehavior *>(pNewBehavior)) !=
      NULL) {
    OnStartingFollow(pFollowBehavior->GetFollowTarget());
  } else if ((pFollowBehavior =
                  dynamic_cast<CAI_FollowBehavior *>(pOldBehavior)) != NULL) {
    OnStoppingFollow(pFollowBehavior->GetFollowTarget());
  }
}

bool CNPCSimpleTalker::OnBehaviorChangeStatus(CAI_BehaviorBase *pBehavior,
                                              bool fCanFinishSchedule) {
  bool interrupt =
      BaseClass::OnBehaviorChangeStatus(pBehavior, fCanFinishSchedule);
  if (!interrupt) {
    interrupt =
        (dynamic_cast<CAI_FollowBehavior *>(pBehavior) != NULL &&
         (m_NPCState == NPC_STATE_IDLE || m_NPCState == NPC_STATE_ALERT));
  }
  return interrupt;
}
//-----------------------------------------------------------------------------
// Purpose: Return true if I should speak based on the chance & the speech
// filter's modifier
//-----------------------------------------------------------------------------
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_ACTIVITY				"
    "ACTIVITY:ACT_IDLE"  // Stop and listen
    "		TASK_WAIT						0.5"  // Wait until sure it's me they are talking to
    "		TASK_TALKER_IDEALYAW			0"  // look at who I'm
                                                            // talking to
    "		TASK_FACE_IDEAL					0"
    "		TASK_TALKER_EYECONTACT			0"  // Wait until
                                                            // speaker is done
    "		TASK_TALKER_WAIT_FOR_SEMAPHORE	0"
    "		TASK_TALKER_EYECONTACT			0"  // Wait until
                                                            // speaker is done
    "		TASK_TALKER_RESPOND				0"  // Wait and
                                                                    // then say
                                                                    // my
                                                                    // response
    "		TASK_TALKER_IDEALYAW			0"  // look at who I'm
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_TALKER_SPEAK			0"  // question or
                                                            // remark
    "		TASK_TALKER_IDEALYAW		0"  // look at who I'm talking
                                                    // to
    "		TASK_FACE_IDEAL				0"
...
DEFINE_SCHEDULE(SCHED_TALKER_IDLE_HELLO,
                ...
                "	Tasks"
                "		 TASK_SET_ACTIVITY			"
                "	ACTIVITY:ACT_SIGNAL3"  // Stop and talk
                "		 TASK_TALKER_HELLO			"
                "	0"  // Try to say hello to player
                "		 TASK_TALKER_EYECONTACT			0"
                "		 TASK_WAIT				"
                "		0.5"  // wait a bit
                "		 TASK_TALKER_HELLO			"
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		 TASK_TALKER_STOPSHOOTING	0"  // tell player to stop
                                                    // shooting friend
    ""
    "	Interrupts"
    "		COND_NEW_ENEMY"
...
DEFINE_SCHEDULE(
    ...
    "		COND_HEAVY_DAMAGE"
    "		COND_PROVOKED"
    "		COND_HEAR_COMBAT"  // sound flags - change these and you'll
                                   // break the talking code.
    "		COND_HEAR_DANGER"
    "		COND_SMELL"
    "		COND_PLAYER_PUSHING"
