...
void CPointSpotlight::Spawn(void) {
  ...
  if (m_flSpotlightGoalWidth > MAX_BEAM_WIDTH) {
    DevMsg("%s (%s) has an invalid spotlight width %.1f (max %.1f).\n",
           GetClassname(), GetDebugName(), m_flSpotlightGoalWidth,
           MAX_BEAM_WIDTH);
...
void CPointSpotlight::ComputeRenderInfo() {
  ...
  float flNewWidth = m_flSpotlightGoalWidth *
                     (m_flSpotlightCurLength / m_flSpotlightMaxLength);
  flNewWidth = std::clamp(flNewWidth, 0.0f, MAX_BEAM_WIDTH);
...
void CPointSpotlight::CreateEfficientSpotlight() {
  if (m_hSpotlightTarget.Get() != NULL) return;
  ...
  VectorSubtract(m_hSpotlightTarget->GetAbsOrigin(),
                 m_hSpotlightTarget->m_vSpotlightOrg,
                 m_hSpotlightTarget->m_vSpotlightDir);
...
//-------------------------------------------------------------------------------------
// Purpose : Send even though we don't have a model so spotlight gets proper
// position Input   : Output  :
//-------------------------------------------------------------------------------------
int CPointSpotlight::UpdateTransmitState() {
  if (m_bEfficientSpotlight) return SetTransmitState(FL_EDICT_DONTSEND);
...
void CPointSpotlight::SpotlightCreate(void) {
  ...
  UTIL_TraceLine(GetAbsOrigin(),
                 GetAbsOrigin() + m_vSpotlightDir * m_flSpotlightMaxLength,
  ...
  // m_hSpotlight = CBeam::BeamCreate( "sprites/spotlight.vmt",
  // m_flSpotlightGoalWidth );
  m_hSpotlight =
      CBeam::BeamCreate("sprites/glow_test02.vmt", m_flSpotlightGoalWidth);
  ...
  if (m_bEfficientSpotlight) {
    m_hSpotlight->PointsInit(GetAbsOrigin(),
                             m_hSpotlightTarget->GetAbsOrigin());
  } else {
...
void CPointSpotlight::SpotlightUpdate(void) {
  ...
  Vector vTargetDir;
  VectorSubtract(m_vSpotlightCurrentPos, m_hSpotlightTarget->GetAbsOrigin(),
                 vTargetDir);
  ...
  // If a large move is requested, just jump to final spot as we probably hit a
  // discontinuity
  ...
  m_flSpotlightCurLength =
      (0.60 * m_flSpotlightCurLength) + (0.4 * flBeamLength);
