...
//-----------------------------------------------------------------------------
static void Physics_TraceEntity(CBaseEntity *pBaseEntity,
  ...
  if (pBaseEntity->GetDamageType() != DMG_GENERIC) {
    GameRules()->WeaponTraceEntity(pBaseEntity, vecAbsStart, vecAbsEnd, mask,
                                   ptr);
  } else {
...
// Output : trace_t
//-----------------------------------------------------------------------------
static void PhysicsCheckSweep(CBaseEntity *pEntity, const Vector &vecAbsStart,
                              const Vector &vecAbsDelta, trace_t *pTrace) {
...
class CPhysicsPushedEntities {
 public:
  ...
  // Purpose: Tries to linearly push an entity hierarchy, returns the blocker if
  // any
 ...
 private:
  ...
  // Speculatively checks to see if all entities in this list can be pushed
  bool SpeculativelyCheckRotPush(const RotatingPushMove_t &rotPushMove,
                                 CBaseEntity *pRoot);
  ...
  void FinishRotPushedEntity(CBaseEntity *pPushedEntity,
                             const RotatingPushMove_t &rotPushMove);
...
void CPhysicsPushedEntities::UnlinkPusherList(int *pPusherHandles) {
  for (int i = m_rgPusher.Count(); --i >= 0;) {
    pPusherHandles[i] = partition->HideElement(
        m_rgPusher[i].m_pEntity->CollisionProp()->GetPartitionHandle());
  }
}
...
// Compute the direction to move the rotation blocker
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::ComputeRotationalPushDirection(
    CBaseEntity *pBlocker, const RotatingPushMove_t &rotPushMove, Vector *pMove,
    CBaseEntity *pRoot) {
  ...
  if (pRoot->GetSolid() == SOLID_VPHYSICS) {
    // HACKHACK: Use move dir to guess which corner of the box determines
    // contact and rotate the box so that corner remains in the same local
    // position. BUGBUG: This will break, but not as badly as the previous
    // solution!!!
    ...
    if (pPlayer) {
      // notify the player physics code so it can use vphysics to keep players
      // from getting stuck
...
bool CPhysicsPushedEntities::IsPushedPositionValid(CBaseEntity *pBlocker) {
  ...
  trace_t trace;
  UTIL_TraceEntity(pBlocker, pBlocker->GetAbsOrigin(), pBlocker->GetAbsOrigin(),
                   pBlocker->PhysicsSolidMaskForEntity(), &pushFilter, &trace);
...
// Speculatively checks to see if all entities in this list can be pushed
//-----------------------------------------------------------------------------
bool CPhysicsPushedEntities::SpeculativelyCheckPush(PhysicsPushedInfo_t &info,
                                                    const Vector &vecAbsPush,
                                                    bool bRotationalPush) {
  CBaseEntity *pBlocker = info.m_pEntity;

  // See if it's possible to move the entity, but disable all pushers in the
  // hierarchy first
  ...
  UTIL_TraceEntity(pBlocker, pBlocker->GetAbsOrigin(), pushDestPosition,
                   pBlocker->PhysicsSolidMaskForEntity(), &pushFilter,
                   &info.m_Trace);

  RelinkPusherList(pPusherHandles);
  info.m_bPusherIsGround = false;
  if (pBlocker->GetGroundEntity() &&
      pBlocker->GetGroundEntity()->GetRootMoveParent() ==
          m_rgPusher[0].m_pEntity) {
    info.m_bPusherIsGround = true;
  }

  bool bIsUnblockable = (m_bIsUnblockableByPlayer &&
                         (pBlocker->IsPlayer() || pBlocker->MyNPCPointer()))
                            ? true
                            : false;
  if (bIsUnblockable) {
  ...
  if (bIsUnblockable) {
    ...
    for (int checkCount = 0; checkCount < 4; checkCount++) {
      Vector move;
      MatrixGetColumn(m_rgPusher[0].m_pEntity->EntityToWorldTransform(),
                      checkCount >> 1, move);
...
// Speculatively checks to see if all entities in this list can be pushed
//-----------------------------------------------------------------------------
bool CPhysicsPushedEntities::SpeculativelyCheckRotPush(
    const RotatingPushMove_t &rotPushMove, CBaseEntity *pRoot) {
  Vector vecAbsPush;
  m_nBlocker = -1;
  for (int i = m_rgMoved.Count(); --i >= 0;) {
    ComputeRotationalPushDirection(m_rgMoved[i].m_pEntity, rotPushMove,
                                   &vecAbsPush, pRoot);
...
// Speculatively checks to see if all entities in this list can be pushed
//-----------------------------------------------------------------------------
bool CPhysicsPushedEntities::SpeculativelyCheckLinearPush(
    const Vector &vecAbsPush) {
  m_nBlocker = -1;
  for (int i = m_rgMoved.Count(); --i >= 0;) {
...
// Causes all entities in the list to touch triggers from their prev position
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::FinishRotPushedEntity(
    CBaseEntity *pPushedEntity, const RotatingPushMove_t &rotPushMove) {
  ...
  if (pPushedEntity->IsPlayer()) {
    ...
    // Because we can run multiple ticks per server frame, accumulate a total
    // offset here instead of straight
    //  setting it.  The engine will reset anglechange to 0 when the message is
    //  actually sent to the client
    player->pl.anglechange += rotPushMove.amove;
  } else {
    QAngle angles = pPushedEntity->GetAbsAngles();

    // only rotate YAW with pushing.  Freely rotateable entities should either
    // use VPHYSICS or be set up as children
...
// Causes all entities in the list to touch triggers from their prev position
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::FinishPush(
    bool bIsRotPush, const RotatingPushMove_t *pRotPushMove) {
  FinishPushers();
...
// store off a list of what has changed - so vphysicsUpdate can undo this if the
// object gets blocked
void CPhysicsPushedEntities::StoreMovedEntities(physicspushlist_t &list) {
  ...
  for (int i = 0; i < list.pushedCount; i++) {
    ...
    list.pushVec[i] = m_rgMoved[i].m_pEntity->GetAbsOrigin() -
                      m_rgMoved[i].m_vecStartAbsOrigin;
  }
}
...
// Input  : fromrotation - if the move is from a rotation, then angular move
 ...
 public:
  virtual bool ShouldHitEntity(IHandleEntity *pEntity, int contentsMask) {
    for (int i = m_entityList.Count() - 1; i >= 0; --i) {
      if (m_entityList[i] == pEntity) return true;
    }
...
class CPushBlockerEnum : public IPartitionEnumerator {
 public:
  CPushBlockerEnum(CPhysicsPushedEntities *pPushedEntities)
    ...
    m_pRootHighestParent =
        m_pPushedEntities->m_rgPusher[0].m_pEntity->GetRootMoveParent();
    ++s_nEnumCount;
    ...
    for (int i = m_pPushedEntities->m_rgPusher.Count(); --i >= 0;) {
      ...
      int collisionGroup =
          m_pPushedEntities->m_rgPusher[i].m_pEntity->GetCollisionGroup();
  ...
  virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    if (!pCheck) return ITERATION_CONTINUE;
 ...
 private:
  ...
  bool IntersectsPushers(CBaseEntity *pTest) {
    ...
    enginetrace->SweepCollideable(
        pCollision, pTest->GetAbsOrigin(), pTest->GetAbsOrigin(),
        pCollision->GetCollisionAngles(), pTest->PhysicsSolidMaskForEntity(),
        &m_pushersOnly, &tr);
  ...
  CBaseEntity *GetPushableEntity(IHandleEntity *pHandleEntity) {
    ...
    bool bCollide = false;
    for (int i = 0; i < m_collisionGroupCount; i++) {
      if (g_pGameRules->ShouldCollide(pCheck->GetCollisionGroup(),
                                      m_collisionGroups[i])) {
        bCollide = true;
        break;
      }
    }
    if (!bCollide) return NULL;
    // We're not pushing stuff we're hierarchically attached to
    CBaseEntity *pCheckHighestParent = pCheck->GetRootMoveParent();
    if (pCheckHighestParent == m_pRootHighestParent) return NULL;
    ...
    if (!IsStandingOnPusher(pCheck)) {
      // Our surrounding boxes are touching. But we may well not be
      // colliding.... see if the ent's bbox is inside the pusher's final
      // position
...
void CPhysicsPushedEntities::GenerateBlockingEntityList() {
  ...
  for (int i = m_rgPusher.Count(); --i >= 0;) {
    ...
    if (!pPusher->IsSolid() ||
        pPusher->IsSolidFlagSet(FSOLID_VOLUME_CONTENTS)) {
      continue;
    }
...
// Generates a list of potential blocking entities
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::GenerateBlockingEntityListAddBox(
...
void CPhysicsPushedEntities::SetupAllInHierarchy(CBaseEntity *pParent) {
  ...
  CBaseEntity *pChild;
  for (pChild = pParent->FirstMoveChild(); pChild != NULL;
...
// Purpose: Rotates the root entity, fills in the pushmove structure
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::RotateRootEntity(CBaseEntity *pRoot,
                                              float movetime,
                                              RotatingPushMove_t &rotation) {
...
// Purpose: Tries to rotate an entity hierarchy, returns the blocker if any
//-----------------------------------------------------------------------------
CBaseEntity *CPhysicsPushedEntities::PerformRotatePush(CBaseEntity *pRoot,
                                                       float movetime) {
  VPROF("CPhysicsPushedEntities::PerformRotatePush");

  m_bIsUnblockableByPlayer =
      (pRoot->GetFlags() & FL_UNBLOCKABLE_BY_PLAYER) ? true : false;
  // Build a list of this entity + all its children because we're going to try
  // to move them all This will also make sure each entity is linked in the
  // appropriate place with correct absboxes
  m_rgPusher.RemoveAll();
  ...
  // Next generate a list of all entities that could potentially be intersecting
  // with any of the children in their new locations...
  GenerateBlockingEntityList();

  // Now we have a unique list of things that could potentially block our push
  // and need to be pushed out of the way. Lets try to push them all out of the
  // way. If we fail, undo it all
...
// Purpose: Linearly moves the root entity
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::LinearlyMoveRootEntity(CBaseEntity *pRoot,
                                                    float movetime,
                                                    Vector *pAbsPushVector) {
...
// any
//-----------------------------------------------------------------------------
CBaseEntity *CPhysicsPushedEntities::PerformLinearPush(CBaseEntity *pRoot,
                                                       float movetime) {
...
//			All other entity thinking is done during worldspawn's
// think
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsDispatchThink(BASEPTR thinkFunc) {
  VPROF_ENTER_SCOPE(
      (!vprof_scope_entity_thinks.GetBool())
...
// Output : trace_t
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsCheckSweep(const Vector &vecAbsStart,
...
int CBaseEntity::PhysicsTryMove(float flTime, trace_t *steptrace) {
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    if (vecAbsVelocity == vec3_origin) break;
    ...
    if (trace.plane.normal[2] > 0.7) {
      ...
      if (CanStandOn(trace.m_pEnt)) {
        ...
        if (GetGroundEntity() != trace.m_pEnt) {
          SetGroundChangeTime(gpGlobals->curtime +
                              (flTime - (1 - trace.fraction) * time_left));
        }
    ...
    if (IsMarkedForDeletion() || IsEdictFree()) break;
    ...
    if (GetMoveType() == MOVETYPE_WALK &&
        (!(GetFlags() & FL_ONGROUND) ||
         GetFriction() != 1))  // relfect player velocity
    {
      for (i = 0; i < numplanes; i++) {
        if (planes[i][2] > 0.7) {  // floor or slope
          ...
          PhysicsClipVelocity(original_velocity, planes[i], new_velocity,
                              1.0 + sv_bounce.GetFloat() * (1 - GetFriction()));
        }
      }
...
void CBaseEntity::PhysicsPushEntity(const Vector &push, trace_t *pTrace) {
  ...
  // to test the swept ray from previous to current location for trigger
  // intersections
...
//-----------------------------------------------------------------------------
// Purpose:  See if entity is inside another entity, if so, returns true if so,
// fills in *ppEntity if ppEntity is not NULL Input  : **ppEntity - optional
// return pointer to entity we are inside of Output : Returns true on success,
// false on failure.
//-----------------------------------------------------------------------------
...
void CBaseEntity::PerformPush(float movetime) {
  ...
  if (movetime > 0) {
    ...
    // For MOVETYPE_PUSH objects with VPhysics shadow objects, the move done
    // time is handled by CBaseEntity::VPhyicsUpdatePusher, which only gets
    // called if the physics system thinks the entity is awake. That will happen
    // if the shadow gets updated, but the push code above doesn't update unless
    // the move is successful or non-zero. So we must make sure it's awake
  ...
  if (VPhysicsGetObject()) {
    ...
    // if you actually did an unblocked push that moved entities, and you're
    // using physics (which may block later)
    if (movetime > 0 && !m_pBlocker && GetSolid() == SOLID_VPHYSICS &&
        s_PushedEntities.CountMovedEntities() > 0) {
      ...
      physicspushlist_t *pList =
          (physicspushlist_t *)CreateDataObject(PHYSICSPUSHLIST);
      if (pList) {
...
//-----------------------------------------------------------------------------
// Purpose: UNDONE: This is only different from PhysicsParent because of the
// callback to PhysicsVelocity() Can we support that callback in push objects as
// well?
//-----------------------------------------------------------------------------
...
void CBaseEntity::PerformCustomPhysics(Vector *pNewPosition,
...
void CBaseEntity::PhysicsCustom() {
  ...
  // Moving upward, off the ground, or  resting on a client/monster, remove
  // FL_ONGROUND
  if (m_vecVelocity[2] > 0 || !GetGroundEntity() ||
      !GetGroundEntity()->IsStandable()) {
    SetGroundEntity(NULL);
  }

  // NOTE: The entity must set the position, angles, velocity in its custom
  // movement
  ...
  PerformCustomPhysics(&vecNewPosition, &vecNewVelocity, &angNewAngles,
                       &angNewAngVelocity);
...
//-----------------------------------------------------------------------------
// Purpose: Run regular think and latch off angle/origin changes so we can
// interpolate them on the server to fake simulation Input  : *step -
//-----------------------------------------------------------------------------
void CBaseEntity::StepSimulationThink(float dt) {
  ...
  if (!step) {
    ...
    // Latch new values to see if external code modifies our
...
// This allows the client-side interpolation to interpolate animation and
// simulation data at the same time. UNDONE:	Remove all other cases from this
// loop - only use MOVETYPE_STEP to simulate
...
void CBaseEntity::PhysicsStep() {
  ...
  // The underlying values don't actually change, but we need the network
  // sendproxy on origin/angles
  //  to get triggered, and that only happens if NetworkStateChanged() appears
  //  to have occured.
  // Getting them for modify marks them as changed automagically.
  m_vecOrigin.GetForModify();
  m_angRotation.GetForModify();

  // HACK:  Make sure that the client latches the networked origin/orientation
  // changes with the current server tick count
  //  so that we don't get jittery interpolation.  All of this is necessary to
  //  mimic actual continuous simulation of the underlying variables.
  ...
  // UNDONE: Remove this and assert?  Force MOVETYPE_STEP objs to become
  // MOVETYPE_TOSS when they aren't thinking? UNDONE: this happens as the first
  // frame for a bunch of things like dynamically created ents. can't remove
  // until initial conditions are resolved
  ...
  if (HasDataObjectType(VPHYSICSUPDATEAI)) {
    vphysicsupdateai_t *pUpdate =
        static_cast<vphysicsupdateai_t *>(GetDataObject(VPHYSICSUPDATEAI));
    if (pUpdate->stopUpdateTime > gpGlobals->curtime) {
      updateFromVPhysics = true;
    } else {
      float maxAngular;
      VPhysicsGetObject()->GetShadowController()->GetMaxSpeed(NULL,
                                                              &maxAngular);
      VPhysicsGetObject()->GetShadowController()->MaxSpeed(
          pUpdate->savedShadowControllerMaxSpeed, maxAngular);
  ...
  if (updateFromVPhysics && VPhysicsGetObject() && !GetParent()) {
    ...
    if (delta < 1) {
      ...
      trace_t tr;
      Physics_TraceEntity(this, GetAbsOrigin(), GetAbsOrigin(),
                          PhysicsSolidMaskForEntity(), &tr);
    ...
    // NDebugOverlay::Box( position, WorldAlignMins(), WorldAlignMaxs(), 255,
    // 255, 0, 0, 0.0 );
  }

  // not going to think, don't run game physics either
  if (thinktick > gpGlobals->tickcount) return;
  ...
  if (VPhysicsGetObject()) {
    if (!VectorCompare(oldOrigin, GetAbsOrigin())) {
      VPhysicsGetObject()->UpdateShadow(
          GetAbsOrigin(), vec3_angle, (GetFlags() & FL_FLY) ? true : false, dt);
    }
  }
  PhysicsRelinkChildren(dt);
}

void UTIL_TraceLineFilterEntity(CBaseEntity *pEntity, const Vector &vecAbsStart,
                                const Vector &vecAbsEnd, unsigned int mask,
                                const int nCollisionGroup, trace_t *ptr);
...
void CBaseEntity::PhysicsStepRecheckGround() {
  ...
  for (x = 0; x <= 1; x++) {
    for (y = 0; y <= 1; y++) {
      ...
      if (pCollision && IsNPC()) {
        UTIL_TraceLineFilterEntity(this, point, point, mask,
...
// After this long, if a player isn't updating, then return it's projectiles to
// server control
...
void Physics_SimulateEntity(CBaseEntity *pEntity) {
  VPROF((!vprof_scope_entity_gamephys.GetBool())
            ...
            : EntityFactoryDictionary()->GetCannonicalName(
  ...
  if (pEntity->edict()) {
    ...
    if (pEntity->IsPlayerSimulated()) {
      ...
      if (simulatingPlayer &&
          (simulatingPlayer->GetTimeBase() >
           gpGlobals->curtime -
               PLAYER_PACKETS_STOPPED_SO_RETURN_TO_PHYSICS_TIME)) {
...
#if !defined(NO_ENTITY_PREDICTION)
    // If an object was at one point player simulated, but had that status
    // revoked (as just
    //  above when no packets have arrived in a while ), then we still will
    //  assume that the owner/player will be predicting the entity locally (even
    //  if the game is playing like butt) and so we won't spam that player with
    //  additional network data such as effects/sounds that are theoretically
    //  being predicted by the player anyway.
    if (pEntity->m_PredictableID->IsActive()) {
      CBasePlayer *playerowner = ToBasePlayer(pEntity->GetOwnerEntity());
      if (playerowner) {
        CBasePlayer *pl = ToBasePlayer(
            UTIL_PlayerByIndex(pEntity->m_PredictableID->GetPlayer() + 1));
...
//-----------------------------------------------------------------------------
// Purpose: Runs the main physics simulation loop against all entities ( except
// players )
//-----------------------------------------------------------------------------
  ...
  if (!simulating) {
    ...
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      ...
      if (pPlayer) {
        ...
        // Force usercmd processing even though gpGlobals->tickcount isn't
        // incrementing
    ...
    // UNDONE: This has problems with UTIL_RemoveImmediate() (now disabled
    // during this loop). Do we really need UTIL_RemoveImmediate()?
