...
class CFire : public CBaseEntity {
 public:
  ...
  void Precache(void);
  void Init(const Vector &position, float scale, float attackTime, float fuel,
            int flags, int fireType);
  bool GoOut();
 ...
 protected:
  ...
  float m_flHeatLevel;   // Used as a "health" for the fire.  > 0 means the fire
                         // is burning
  float m_flHeatAbsorb;  // This much heat must be "absorbed" before it gets
                         // transferred to the flame size
  float m_flDamageScale;

  float m_flMaxHeat;
  float m_flLastHeatLevel;

  // NOTENOTE: Lifetime is an expression of the sum total of these amounts plus
  // the global time when started
...
class CFireSphere : public IPartitionEnumerator {
 public:
  CFireSphere(CFire **pList, int listMax, bool onlyActiveFires,
...
CFireSphere::CFireSphere(CFire **pList, int listMax, bool onlyActiveFires,
                         const Vector &origin, float radius) {
  m_pList = pList;
  m_listMax = listMax;
  m_count = 0;
...
bool CFireSphere::AddToList(CFire *pFire) {
  if (m_count >= m_listMax) return false;
...
IterationRetval_t CFireSphere::EnumElement(IHandleEntity *pHandleEntity) {
  ...
  if (pEntity) {
    ...
    if (!FClassnameIs(pEntity, "env_fire")) return ITERATION_CONTINUE;
    ...
    if (pFire) {
      if (!m_onlyActiveFires || pFire->IsBurning()) {
        if ((m_origin - pFire->GetAbsOrigin()).LengthSqr() < m_radiusSqr) {
          if (!AddToList(pFire)) return ITERATION_STOP;
        }
      }
    }
  }
...
int FireSystem_GetFiresInSphere(CFire **pList, int listMax,
                                bool onlyActiveFires, const Vector &origin,
                                float radius) {
  ...
  partition->EnumerateElementsInSphere(PARTITION_ENGINE_NON_STATIC_EDICTS,
                                       origin, radius, false, &sphereEnum);
...
bool FireSystem_IsFireInWall(Vector &position, fireType_e type) {
  ...
  if (type == FIRE_NATURAL) return false;

  trace_t tr;
  UTIL_TraceHull(position, position + Vector(0, 0, 0.1), FIRE_MINS, FIRE_MAXS,
...
bool FireSystem_CanAddFire(Vector *position, float separationRadius,
  ...
  if (FireSystem_IsFireInWall(*position, type)) {
    ...
    vTestPos.x += 10;
    if (FireSystem_IsValidFirePosition(vTestPos, separationRadius) &&
        !FireSystem_IsFireInWall(vTestPos, type)) {
      *position = vTestPos;
      return true;
    }
    vTestPos.y += 10;
...
//-----------------------------------------------------------------------------
bool FireSystem_StartFire(const Vector &position, float fireHeight,
                          float attack, float fuel, int flags,
                          CBaseEntity *owner, fireType_e type) {
  ...
  // Spawn the fire
  // Fires not placed by a designer should be cleaned up automatically (not
  // catch fire again)
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool FireSystem_StartFire(CBaseAnimating *pEntity, float fireHeight,
                          float attack, float fuel, int flags,
                          CBaseEntity *owner, fireType_e type) {
...
void FireSystem_ExtinguishInRadius(const Vector &origin, float radius,
                                   float rate) {
...
//			heat -
//-----------------------------------------------------------------------------
void FireSystem_AddHeatInRadius(const Vector &origin, float radius,
                                float heat) {
...
bool FireSystem_GetFireDamageDimensions(CBaseEntity *pEntity, Vector *pFireMins,
                                        Vector *pFireMaxs) {
  CFire *pFire = dynamic_cast<CFire *>(pEntity);
...
BEGIN_DATADESC(CFire)
  ...
  DEFINE_FIELD(m_hEffect, FIELD_EHANDLE), DEFINE_FIELD(m_hOwner, FIELD_EHANDLE),
...
void CFire::InputEnable(inputdata_t &inputdata) { m_bEnabled = true; }

void CFire::InputDisable(inputdata_t &inputdata) { Disable(); }
...
void CFire::StartFire(void) {
  ...
  if (m_spawnflags & SF_FIRE_DONT_DROP) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 1024),
                   MASK_FIRE_SOLID, this, COLLISION_GROUP_NONE, &tr);
    vFirePos = tr.endpos;
  }

  int spawnflags = m_spawnflags;
  m_spawnflags |= SF_FIRE_START_ON;
  Init(vFirePos, m_flFireSize, m_flAttackTime, GetHealth(), m_spawnflags,
...
void CFire::Spawn(void) {
  ...
  m_flHeatLevel = 0;
  Init(GetAbsOrigin(), m_flFireSize, m_flAttackTime, m_flFuel, m_spawnflags,
       m_nFireType);
...
int CFire::UpdateTransmitState() {
  // Don't want to be FL_EDICT_DONTSEND because our fire entity may make us
  // transmit.
...
void CFire::SpawnEffect(fireType_e type, float scale) {
  ...
  switch (type) {
    ...
    case FIRE_NATURAL: {
      ...
      fireSmoke->EnableVisibleFromAbove(
          (m_spawnflags & SF_FIRE_VISIBLE_FROM_ABOVE) != false);
...
//			lifetime -
//-----------------------------------------------------------------------------
void CFire::Init(const Vector &position, float scale, float attackTime,
                 float fuel, int flags, int fireType) {
...
void CFire::Start() {
  ...
  UTIL_SetSize(this, Vector(-boxWidth, -boxWidth, 0),
               Vector(boxWidth, boxWidth, m_flFireSize));
...
bool CFire::IsBurning(void) const {
  if (m_flHeatLevel > 0) return true;
...
void CFire::Update(float simTime) {
  ...
  float addedHeat =
      (m_flAttackTime > 0) ? m_flMaxHeat / m_flAttackTime : m_flMaxHeat;
  ...
  // NDebugOverlay::Box( GetAbsOrigin(), fireMins, fireMaxs, 255, 255, 255, 0,
  // fire_dmginterval.GetFloat() );
  ...
  if (m_flDamageTime <= gpGlobals->curtime) {
    ...
    outputDamage = (fire_dmgbase.GetFloat() +
                    outputHeat * fire_dmgscale.GetFloat() * m_flDamageScale) *
                   fire_dmginterval.GetFloat();
    if (outputDamage) {
      damage = true;
    }
  ...
  for (i = 0; i < nearbyCount; i++) {
    ...
    if (pOther == this) {
      ...
      if (FIRE_SPREAD_DAMAGE_MULTIPLIER != 1.0 &&
          !pOther
               ->IsPlayer())  // if set to 1.0, optimizer will remove this code
      {
        Vector otherMins, otherMaxs;
        pOther->CollisionProp()->WorldSpaceAABB(&otherMins, &otherMaxs);
        bDoDamage = IsBoxIntersectingBox(
            otherMins, otherMaxs, fireEntityDamageMins, fireEntityDamageMaxs);
      ...
      if (bDoDamage) {
        ...
        trace_t tr;
        UTIL_TraceLine(this->WorldSpaceCenter(), pOther->WorldSpaceCenter(),
                       MASK_FIRE_SOLID, pOther, COLLISION_GROUP_NONE, &tr);

        if (tr.fraction == 1.0 && !tr.startsolid) {
          pOther->TakeDamage(
              CTakeDamageInfo(this, this, outputDamage, damageFlags));
        }
      }
    }
  }
...
// Output  :
//------------------------------------------------------------------------------
int CFire::OnTakeDamage(const CTakeDamageInfo &info) { return 0; }
...
void CFire::AddHeat(float heat, bool selfHeat) {
  if (m_bEnabled) {
    ...
    if (m_flHeatLevel > m_flMaxHeat) m_flHeatLevel = m_flMaxHeat;
  }
}
...
void CFire::Extinguish(float heat) {
  ...
  float averageAttackTime =
      m_flMaxHeat * (FIRE_NORMAL_ATTACK_TIME / FIRE_MAX_HEAT_LEVEL);
  m_flAttackTime =
      Approach(averageAttackTime, m_flAttackTime, 2 * gpGlobals->frametime);
...
bool CFire::GoOut() {
  ...
  DestroyEffect();
  m_flHeatLevel -= 20;
  if (m_flHeatLevel > 0) m_flHeatLevel = 0;
...
void CEnvFireSource::InputEnable(inputdata_t &inputdata) { TurnOn(); }
void CEnvFireSource::InputDisable(inputdata_t &inputdata) { TurnOff(); }
...
void CEnvFireSensor::InputEnable(inputdata_t &inputdata) { TurnOn(); }
void CEnvFireSensor::InputDisable(inputdata_t &inputdata) { TurnOff(); }
