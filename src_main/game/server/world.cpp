...
//
// Purpose: Precaches and defs for entities and other data that must always be
// available.
...
class CDecal : public CPointEntity {
 public:
  ...
  // Need to apply static decals here to get them into the signon buffer for the
  // server appropriately
  virtual void Activate();

  void TriggerDecal(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
BEGIN_DATADESC(CDecal)
  ...
  DEFINE_FIELD(m_nTexture, FIELD_INTEGER),
      DEFINE_KEYFIELD(m_bLowPriority, FIELD_BOOLEAN,
                      "LowPriority"),  // Don't mark as FDECAL_PERMANENT so not
                                       // save/restored and will be reused on
                                       // the client preferentially

      // Function pointers
      DEFINE_FUNCTION(StaticDecal), DEFINE_FUNCTION(TriggerDecal),
...
void CDecal::Activate() {
  ...
  if (!GetEntityName()) {
    ...
    // if there IS a targetname, the decal sprays itself on when it is
    // triggered.
...
void CDecal::TriggerDecal(CBaseEntity *pActivator, CBaseEntity *pCaller,
                          USE_TYPE useType, float value) {
  // this is set up as a USE function for info_decals that have targetnames, so
  // that the decal doesn't get applied until it is fired. (usually by a
  // scripted sequence)
  trace_t trace;
  int entityIndex;

  UTIL_TraceLine(GetAbsOrigin() - Vector(5, 5, 5),
                 GetAbsOrigin() + Vector(5, 5, 5), MASK_SOLID_BRUSHONLY, this,
...
void CDecal::StaticDecal(void) {
  class CTraceFilterValidForDecal : public CTraceFilterSimple {
   public:
    CTraceFilterValidForDecal(const IHandleEntity *passentity,
                              int collisionGroup)
    ...
    virtual bool ShouldHitEntity(IHandleEntity *pServerEntity,
      ...
      static const char *ppszIgnoredClasses[] = {
          "weapon_*",     "item_*",      "prop_ragdoll",
          "prop_dynamic", "prop_static", "prop_physics",
          "npc_bullseye",  // Tracker 15335
      };

      CBaseEntity *pEntity = EntityFromEntityHandle(pServerEntity);

      // Tracker 15335:  Never impact decals against entities which are not
      // rendering, either.
  ...
  UTIL_TraceLine(position - Vector(5, 5, 5), position + Vector(5, 5, 5),
  ...
  if (entityIndex) {
    ...
    if (ent) {
      ...
      if (!canDraw) {
        Warning(
            "Suppressed StaticDecal which would have hit entity %i (class:%s, "
            "name:%s) with modelindex = 0\n",
            ent->entindex(), ent->GetClassname(), STRING(ent->GetEntityName()));
      }
    }
  }
...
void CProjectedDecal::ProjectDecal(CRecipientFilter &filter) {
  te->ProjectDecal(filter, 0.0, &GetAbsOrigin(), &GetAbsAngles(), m_flDistance,
                   m_nTexture);
}

void CProjectedDecal::TriggerDecal(CBaseEntity *pActivator,
...
BEGIN_DATADESC(CWorld)
  ...
  DEFINE_FIELD(m_flWaveHeight, FIELD_FLOAT),
      ...
      DEFINE_KEYFIELD(m_flMaxPropScreenSpaceWidth, FIELD_FLOAT,
                      "maxpropscreenwidth"),
      DEFINE_KEYFIELD(m_flMinPropScreenSpaceWidth, FIELD_FLOAT,
                      "minpropscreenwidth"),
      DEFINE_KEYFIELD(m_iszDetailSpriteMaterial, FIELD_STRING,
                      "detailmaterial"),
...
void CWorld::DecalTrace(trace_t *pTrace, char const *decalName) {
  ...
  CBroadcastRecipientFilter filter;
  if (pTrace->hitbox != 0) {
    te->Decal(filter, 0.0f, &pTrace->endpos, &pTrace->startpos, 0,
              pTrace->hitbox, index);
  } else {
...
void CWorld::Spawn(void) {
  ...
  g_EventQueue.Init();
  Precache();
  GlobalEntity_Add("is_console", STRING(gpGlobals->mapname),
                   (IsConsole()) ? GLOBAL_ON : GLOBAL_OFF);
  GlobalEntity_Add("is_pc", STRING(gpGlobals->mapname),
                   (!IsConsole()) ? GLOBAL_ON : GLOBAL_OFF);
}
...
void CWorld::Precache(void) {
  ...
  g_Language.SetValue(
      LANGUAGE_ENGLISH);  // TODO use VGUI to get current language
  ...
  if (m_iszChapterTitle != NULL_STRING) {
    ...
    CMessage *pMessage = (CMessage *)CBaseEntity::Create(
        "env_message", vec3_origin, vec3_angle, NULL);
    if (pMessage) {
...
// Output : float
//-----------------------------------------------------------------------------
float GetRealTime() { return engine->Time(); }

bool CWorld::GetDisplayTitle() const { return m_bDisplayTitle; }

bool CWorld::GetStartDark() const { return m_bStartDark; }

void CWorld::SetDisplayTitle(bool display) { m_bDisplayTitle = display; }

void CWorld::SetStartDark(bool startdark) { m_bStartDark = startdark; }

bool CWorld::IsColdWorld(void) { return m_bColdWorld; }
