...
BEGIN_DATADESC(CBaseDoor)
  ...
  DEFINE_KEYFIELD(m_vecMoveDir, FIELD_VECTOR, "movedir"),
      ...
      // Function Pointers
      DEFINE_FUNCTION(DoorTouch), DEFINE_FUNCTION(DoorGoUp),
      DEFINE_FUNCTION(DoorGoDown), DEFINE_FUNCTION(DoorHitTop),
      DEFINE_FUNCTION(DoorHitBottom), DEFINE_THINKFUNC(MovingSoundThink),
...
//
// func_water is implemented as a linear door so we can raise/lower the water
// level.
//
...
#define BUTTON_SOUNDWAIT 0.5
    ...
    // Input  : pEdict -
    //			pls -
    //			flocked - if true, play 'door is locked' sound, otherwise
    //play
    //'door 				is unlocked' sound. 			fbutton
    //-
    //-----------------------------------------------------------------------------
  ...
  if (flocked) {
    int fplaysound = (pls->sLockedSound != NULL_STRING &&
                      gpGlobals->curtime > pls->flwaitSound);
    int fplaysentence =
        (pls->sLockedSentence != NULL_STRING && !pls->bEOFLocked &&
    ...
    // if there is a sentence, we've not played all in list, and we've
    // debounced, play sound
    if (fplaysentence) {
      // play next 'door locked' sentence in group
      int iprev = pls->iLockedSentence;

      pls->iLockedSentence = SENTENCEG_PlaySequentialSz(
          pEdict->edict(), STRING(pls->sLockedSentence), 0.85f, SNDLVL_NORM, 0,
          100, pls->iLockedSentence, FALSE);
    ...
    int fplaysound = (pls->sUnlockedSound != NULL_STRING &&
                      gpGlobals->curtime > pls->flwaitSound);
    int fplaysentence =
        (pls->sUnlockedSentence != NULL_STRING && !pls->bEOFUnlocked &&
         gpGlobals->curtime > pls->flwaitSentence);
    float fvol;
    ...
    if (fplaysentence) {
      ...
      pls->iUnlockedSentence = SENTENCEG_PlaySequentialSz(
          pEdict->edict(), STRING(pls->sUnlockedSentence), 0.85, SNDLVL_NORM, 0,
          100, pls->iUnlockedSentence, FALSE);
...
void CBaseDoor::Spawn() {
  ...
  // Subtract 2 from size because the engine expands bboxes by 1 in all
  // directions making the size too big
  Vector vecOBB = CollisionProp()->OBBSize();
  vecOBB -= Vector(2, 2, 2);
  m_vecPosition2 =
      m_vecPosition1 +
      (m_vecMoveDir * (DotProductAbs(m_vecMoveDir, vecOBB) - m_flLip));

  if (!IsRotatingDoor()) {
    if ((m_eSpawnPosition == FUNC_DOOR_SPAWN_OPEN) ||
        HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE)) {  // swap pos1 and pos2,
                                                       // put door at pos2
  ...
  if (!FClassnameIs(this, "func_water")) {
    ...
    if (HasSpawnFlags(SF_DOOR_NONSOLID_TO_PLAYER)) {
      ...
      // HACKHACK: Set this hoping that any children of the door that get
      // blocked by the player will get fixed up by vphysics NOTE: We could
      // decouple this as a separate behavior, but managing player collisions is
      // already complex enough. NOTE: This is necessary to prevent the player
      // from blocking the wrecked train car in ep2_outland_01
    ...
    if (m_bIgnoreDebris) {
      ...
      if (HasSpawnFlags(SF_DOOR_NONSOLID_TO_PLAYER)) {
        Warning(
            "Door %s with conflicting collision settings, removing "
            "ignoredebris\n",
            GetDebugName());
      } else {
  ...
  if ((m_eSpawnPosition == FUNC_DOOR_SPAWN_OPEN) &&
      HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE)) {
    Warning(
        "Door %s using obsolete 'Start Open' spawnflag with 'Spawn Position' "
        "set to 'Open'. Reverting to old behavior.\n",
        GetDebugName());
  }
...
void CBaseDoor::MovingSoundThink(void) {
  ...
  if (m_NoiseMovingClosed == NULL_STRING || m_toggle_state == TS_GOING_DOWN ||
      m_toggle_state == TS_AT_BOTTOM) {
  ...
  if (ShouldLoopMoveSound()) {
    ...
    SetContextThink(&CBaseDoor::MovingSoundThink, gpGlobals->curtime + duration,
                    "MovingSound");
  }
}
...
void CBaseDoor::StopMovingSound(void) {
  ...
  char *pSoundName;
  if (m_NoiseMovingClosed == NULL_STRING || m_toggle_state == TS_GOING_UP ||
      m_toggle_state == TS_AT_TOP) {
...
bool CBaseDoor::CreateVPhysics() {
  if (!FClassnameIs(this, "func_water")) {
    ...
    SETBITS(m_spawnflags.operator const int &(),
            SF_DOOR_SILENT);  // water is silent for now
    ...
    fluid.surfacePlane[3] = CollisionProp()->GetCollisionOrigin().z +
                            CollisionProp()->OBBMaxs().z - 1;
...
void CBaseDoor::Activate(void) {
  ...
  for (int i = 0; i < doorCount; i++) {
    if (pDoorList[i]->m_vecMoveDir == m_vecMoveDir) {
      ...
      if (pDoorList[i]->IsRotatingDoor()) {
        error =
            (pDoorList[i]->GetLocalAngles() != GetLocalAngles()) ? true : false;
      } else {
        error =
            (pDoorList[i]->GetLocalOrigin() != GetLocalOrigin()) ? true : false;
      }
      if (error) {
  ...
  if (m_iBlockFilterName != NULL_STRING) {
    m_hBlockFilter = dynamic_cast<CBaseFilter *>(
        gEntList.FindEntityByName(NULL, m_iBlockFilterName, NULL));
  }
#endif
}
...
#ifdef HL1_DLL
  if (m_ls.sLockedSound != NULL_STRING &&
      strlen((char *)STRING(m_ls.sLockedSound)) < 4) {
    // Too short to be ANYTHING ".wav", so it must be an old index into a
    // long-lost array of sound choices. slam it to a known "deny" sound. We
    // lose the designer's original selection, but we don't get unresponsive
    // doors.
...
//-----------------------------------------------------------------------------
// Purpose: Doors not tied to anything (e.g. button, another door) can be
// touched,
//			to make them activate.
// Input  : *pOther -
//-----------------------------------------------------------------------------
void CBaseDoor::DoorTouch(CBaseEntity *pOther) {
  if (m_ChainTarget != NULL_STRING) ChainTouch(pOther);
...
//-----------------------------------------------------------------------------
// Purpose: Delays turning off area portals when closing doors to prevent visual
// artifacts
//-----------------------------------------------------------------------------
...
void CBaseDoor::UpdateAreaPortals(bool isOpen) {
  ...
  if (IsRotatingDoor() &&
      HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE))  // logic inverted when using
                                                   // rot doors that start open
    isOpen = !isOpen;
...
//			value -
//-----------------------------------------------------------------------------
void CBaseDoor::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                    USE_TYPE useType, float value) {
  m_hActivator = pActivator;

  if (m_ChainTarget != NULL_STRING) ChainUse();

  // We can't +use this if it can't be +used
  if (m_hActivator != NULL && m_hActivator->IsPlayer() &&
      HasSpawnFlags(SF_DOOR_PUSE) == false) {
    PlayLockSounds(this, &m_ls, TRUE, FALSE);
    return;
  }
  ...
  if (HasSpawnFlags(SF_DOOR_NEW_USE_RULES)) {
    ...
    if ((m_toggle_state == TS_AT_BOTTOM || m_toggle_state == TS_GOING_DOWN) ||
        (HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN) &&
         (m_toggle_state == TS_AT_TOP || m_toggle_state == TS_GOING_UP)))
      bAllowUse = true;
  } else {
    // Legacy behavior:
    if (m_toggle_state == TS_AT_BOTTOM ||
        (HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN) && m_toggle_state == TS_AT_TOP))
      bAllowUse = true;
  }
...
void CBaseDoor::ChainUse(void) {
  ...
  while ((ent = gEntList.FindEntityByName(ent, m_ChainTarget, NULL)) != NULL) {
    ...
    if (door) {
      ...
      door->Use(m_hActivator, NULL, USE_TOGGLE,
                0.0f);  // only the first param is used
...
//-----------------------------------------------------------------------------
void CBaseDoor::InputLock(inputdata_t &inputdata) { Lock(); }
...
//-----------------------------------------------------------------------------
void CBaseDoor::InputUnlock(inputdata_t &inputdata) { Unlock(); }
...
//-----------------------------------------------------------------------------
void CBaseDoor::Lock(void) { m_bLocked = true; }

//-----------------------------------------------------------------------------
// Purpose: Unlocks the door so that it can be opened.
//-----------------------------------------------------------------------------
void CBaseDoor::Unlock(void) { m_bLocked = false; }

//-----------------------------------------------------------------------------
// Purpose: Causes the door to "do its thing", i.e. start moving, and cascade
// activation. Output : int
//-----------------------------------------------------------------------------
int CBaseDoor::DoorActivate() {
  if (!UTIL_IsMasterTriggered(m_sMaster, m_hActivator)) return 0;
...
//-----------------------------------------------------------------------------
// Purpose: Starts the door going to its "up" position (simply
// ToggleData->vecPosition2).
//-----------------------------------------------------------------------------
  ...
  if (IsRotatingDoor())  // !!! BUGBUG Triggered doors don't work with this yet
  {
    ...
    if (m_hActivator != NULL) {
      ...
      if (!HasSpawnFlags(SF_DOOR_ONEWAY) &&
          m_vecMoveAng.y)  // Y axis rotation, move away from the player
      {
        // Positive is CCW, negative is CW, so make 'sign' 1 or -1 based on
        // which way we want to open. Important note:  All doors face East at
        // all times, and twist their local angle to open.
        //					So you can't look at the door's
        //facing to determine which way to open.

        Vector nearestPoint;
        CollisionProp()->CalcNearestPoint(m_hActivator->GetAbsOrigin(),
                                          &nearestPoint);
        Vector activatorToNearestPoint =
            nearestPoint - m_hActivator->GetAbsOrigin();
        activatorToNearestPoint.z = 0;

        Vector activatorToOrigin =
            GetAbsOrigin() - m_hActivator->GetAbsOrigin();
        activatorToOrigin.z = 0;

        // Point right hand at door hinge, curl hand towards closest spot on
        // door, if thumb is up, open door CW.  -- Department of Basic Cross
        // Product Understanding for Noobs
...
void CBaseDoor::DoorHitTop(void) {
  ...
  if (HasSpawnFlags(SF_DOOR_NO_AUTO_RETURN)) {
    ...
    // In flWait seconds, DoorGoDown will fire, unless wait is -1, then door
    // stays open
...
//-----------------------------------------------------------------------------
// Purpose: Starts the door going to its "down" position (simply
// ToggleData->vecPosition1).
//-----------------------------------------------------------------------------
...
void CBaseDoor::DoorHitBottom(void) {
  ...
  // Close the area portals just after the door closes, to prevent visual
  // artifacts in multiplayer games
  SetContextThink(&CBaseDoor::CloseAreaPortalsThink, gpGlobals->curtime + 0.5f,
                  CLOSE_AREAPORTAL_THINK_CONTEXT);
}
...
void CBaseDoor::Blocked(CBaseEntity *pOther) {
  ...
  if (m_flBlockDamage) {
    // if the door is marked "force closed" or it has a negative wait, then
    // there's nothing to do but push/damage the object. If block damage is set,
    // but this object is a physics prop that can't be damaged, just give up and
    // disable collisions
    if ((m_bForceClosed || m_flWait < 0) &&
      ...
      pOther->TakeDamage(
          CTakeDamageInfo(this, this, m_flBlockDamage, DMG_CRUSH));
    }
  }
  ...
  if (GetEntityName() != NULL_STRING) {
    ...
    for (int i = 0; i < doorCount; i++) {
      ...
      if (pDoor->m_flWait >= 0) {
        if (m_bDoorGroup && pDoor->m_vecMoveDir == m_vecMoveDir &&
            pDoor->GetAbsVelocity() == GetAbsVelocity() &&
            pDoor->GetLocalAngularVelocity() == GetLocalAngularVelocity()) {
          pDoor->m_nSimulationTick =
              m_nSimulationTick;  // don't run simulation this frame if you
                                  // haven't run yet

          // this is the most hacked, evil, bastardized thing I've ever seen.
          // kjb
          if (!pDoor->IsRotatingDoor()) {  // set origin to realign normal doors
            pDoor->SetLocalOrigin(GetLocalOrigin());
            pDoor->SetAbsVelocity(vec3_origin);  // stop!

          } else {  // set angles to realign rotating doors
...
void CRotDoor::Spawn(void) {
  ...
  ASSERTSZ(m_vecAngle1 != m_vecAngle2,
  ...
  if (HasSpawnFlags(SF_DOOR_START_OPEN_OBSOLETE)) {
    ...
    // We've already had our physics setup in BaseClass::Spawn, so teleport to
    // our current position. If we don't do this, our vphysics shadow will not
    // update.
