...
ConVar ai_strong_optimizations_no_checkstand(
...
CON_COMMAND(ai_set_move_height_epsilon,
...
BEGIN_SIMPLE_DATADESC(CAI_MoveProbe)
  //					m_pTraceListData (not saved, a cached
  // item)
...
bool CAI_MoveProbe::ShouldBrushBeIgnored(CBaseEntity *pEntity) {
  if (pEntity->m_iClassname == g_iszFuncBrushClassname) {
    ...
    // this is true if my class or entity name matches the exclusion name on the
    // func brush
#if HL2_EPISODIC
    bool nameMatches =
        (pFuncBrush->m_iszExcludedClass == GetOuter()->m_iClassname) ||
        GetOuter()->NameMatches(pFuncBrush->m_iszExcludedClass);
#else  // do not match against entity name in base HL2 (just in case there is
       // some case somewhere that might be broken by this)
    bool nameMatches =
        (pFuncBrush->m_iszExcludedClass == GetOuter()->m_iClassname);
#endif

    // return true (ignore brush) if the name matches, or, if exclusion is
    // inverted, if the name does not match
...
void CAI_MoveProbe::TraceLine(const Vector &vecStart, const Vector &vecEnd,
                              unsigned int mask, bool bUseCollisionGroup,
                              trace_t *pResult) const {
  int collisionGroup =
      (bUseCollisionGroup) ? GetCollisionGroup() : COLLISION_GROUP_NONE;

  CTraceFilterNav traceFilter(const_cast<CAI_BaseNPC *>(GetOuter()),
                              m_bIgnoreTransientEntities, GetOuter(),
                              collisionGroup);

  AI_TraceLine(vecStart, vecEnd, mask, &traceFilter, pResult);

#ifdef _DEBUG
  // Just to make sure; I'm not sure that this is always the case but it should
  // be
  if (pResult->allsolid) {
...
CAI_MoveProbe::~CAI_MoveProbe() { delete m_pTraceListData; }
...
void CAI_MoveProbe::TraceHull(const Vector &vecStart, const Vector &vecEnd,
  ...
  if (!m_pTraceListData || m_pTraceListData->IsEmpty())
    ...
    enginetrace->TraceRayAgainstLeafAndEntityList(
        ray, *(const_cast<CAI_MoveProbe *>(this)->m_pTraceListData), mask,
        &traceFilter, pResult);
#if 0
  ...
  // NDebugOverlay::SweptBox( vecStart, vecEnd, hullMin, hullMax, vec3_angle,
  // 255, 255, 0, 0, 10 );
  // Just to make sure; I'm not sure that this is always the case but it should
  // be
...
void CAI_MoveProbe::TraceHull(const Vector &vecStart, const Vector &vecEnd,
                              unsigned int mask, trace_t *pResult) const {
  TraceHull(vecStart, vecEnd, WorldAlignMins(), WorldAlignMaxs(), mask,
            pResult);
}

//-----------------------------------------------------------------------------

void CAI_MoveProbe::SetupCheckStepTraceListData(
    const CheckStepArgs_t &args) const {
  if (ai_moveprobe_usetracelist.GetBool()) {
    ...
    if (args.groundTest != STEP_DONT_CHECK_GROUND) hullMin.z -= args.stepHeight;
    ...
    Vector vecEnd;
    Vector2DMA(args.vecStart.AsVector2D(), args.stepSize,
               args.vecStepDir.AsVector2D(), vecEnd.AsVector2D());
    ...
    enginetrace->SetupLeafAndEntityListRay(
        ray, *(const_cast<CAI_MoveProbe *>(this)->m_pTraceListData));
  }
}
...
bool CAI_MoveProbe::CheckStep(const CheckStepArgs_t &args,
  ...
  Vector stepStart(args.vecStart.x, args.vecStart.y,
                   args.vecStart.z + MOVE_HEIGHT_EPSILON);
  ...
  if (trace.startsolid || (trace.fraction < 1)) {
    ...
    if (!trace.startsolid) {
      if (g_bAIDebugStep)
        NDebugOverlay::Box(trace.endpos, WorldAlignMins(), WorldAlignMaxs(), 64,
                           64, 64, 0, 5);
      ...
      if (g_bAIDebugStep)
        NDebugOverlay::Box(stepTrace.endpos, WorldAlignMins(), WorldAlignMaxs(),
                           96, 96, 96, 0, 5);
    ...
    if (stepTrace.startsolid || stepTrace.fraction <= 0.01) {
      ...
      // If didn't step forward enough to qualify as a step, try as if stepped
      // forward to confirm there's potentially enough space to "land"
      float landingDistSq =
          (stepEnd.AsVector2D() - stepStart.AsVector2D()).LengthSqr();
      ...
      if (landingDistSq < requiredLandingDistSq) {
        ...
        VectorMA(stepStart, args.minStepLanding, args.vecStepDir,
                 stepEndWithLanding);
        TraceHull(stepStart, stepEndWithLanding, collisionMask, &landingTrace);
        if (landingTrace.fraction < 1) {
          if (g_bAIDebugStep)
            NDebugOverlay::Box(
                landingTrace.endpos, WorldAlignMins() + Vector(0, 0, 0.1),
                WorldAlignMaxs() + Vector(0, 0, 0.1), 255, 0, 0, 0, 5);

          bRejectStep = true;
          if (landingTrace.m_pEnt) pResult->pBlocker = landingTrace.m_pEnt;
        }
      } else if ((stepTrace.endpos.AsVector2D() - stepStart.AsVector2D())
                     .LengthSqr() < requiredLandingDistSq) {
        if (g_bAIDebugStep)
          NDebugOverlay::Box(
              stepTrace.endpos, WorldAlignMins() + Vector(0, 0, 0.1),
              WorldAlignMaxs() + Vector(0, 0, 0.1), 255, 0, 0, 0, 5);
    ...
    if (!bRejectStep) trace = stepTrace;

    if (trace.fraction < 1.0) {
      if (!pResult->pBlocker) pResult->pBlocker = trace.m_pEnt;
      pResult->hitNormal = trace.plane.normal;
    }
  ...
  // The ground is only valid if it's within a step height of the original
  // position
  Assert(VectorsAreEqual(trace.endpos, stepEnd, 1e-3));
  stepStart = stepEnd;
  stepEnd.z = args.vecStart.z - args.stepHeight * args.stepDownMultiplier -
              MOVE_HEIGHT_EPSILON;
  ...
  if (trace.fraction == 1.0f) {
    if (g_bAIDebugStep)
      NDebugOverlay::Box(trace.endpos, WorldAlignMins(), WorldAlignMaxs(), 255,
                         0, 0, 0, 5);

    Assert(pResult->endPoint == args.vecStart);
    if (const_cast<CAI_MoveProbe *>(this)->GetOuter()->GetGroundEntity()) {
      pResult->pBlocker =
          const_cast<CAI_MoveProbe *>(this)->GetOuter()->GetGroundEntity();
    } else {
  ...
  if (g_bAIDebugStep)
    NDebugOverlay::Box(trace.endpos, WorldAlignMins(), WorldAlignMaxs(), 160,
                       160, 160, 0, 5);
  ...
  pResult->endPoint.z +=
      MOVE_HEIGHT_EPSILON;  // always safe because always stepped down at least
                            // by epsilon
  ...
  return (
      pResult->pBlocker ==
      NULL);  // totally clear if pBlocker is NULL, partial blockage otherwise
}
...
// is 2 1/2D, and we may end up on a ledge above or below the actual desired
// endpoint.
//-----------------------------------------------------------------------------
bool CAI_MoveProbe::TestGroundMove(const Vector &vecActualStart,
  ...
  if (!pMoveTrace) pMoveTrace = &ignored;
  ...
  Vector vecMoveDir;
  pMoveTrace->flTotalDist = ComputePathDirection(NAV_GROUND, vecActualStart,
                                                 vecDesiredEnd, &vecMoveDir);
  ...
  if ((flags & AITGM_IGNORE_FLOOR) || pctToCheckStandPositions < 0.001) {
    ...
    if (pctToCheckStandPositions > 99.999) pctToCheckStandPositions = 100;

    if ((flags & AITGM_IGNORE_INITIAL_STAND_POS) ||
        CheckStandPosition(vecActualStart, collisionMask))
  ...
  float distStartToIgnoreGround =
      (pctToCheckStandPositions == 100)
          ? pMoveTrace->flTotalDist
          : pMoveTrace->flTotalDist * (pctToCheckStandPositions * 0.01);
  bool bTryNavIgnore =
      ((vecActualStart - GetLocalOrigin()).Length2DSqr() < 0.1 &&
       fabsf(vecActualStart.z - GetLocalOrigin().z) <
           checkStepArgs.stepHeight * 0.5);

  CUtlVector<CBaseEntity *> ignoredEntities;

  for (;;) {
    float flStepSize =
        std::min(LOCAL_STEP_SIZE, pMoveTrace->flTotalDist - distClear);
    if (flStepSize < 0.001) break;
    ...
    for (i = 0; i < 16; i++) {
      ...
      if (!bTryNavIgnore || !checkStepResult.pBlocker ||
          !checkStepResult.fStartSolid)
        break;

      if (checkStepResult.pBlocker->GetMoveType() != MOVETYPE_VPHYSICS &&
          !checkStepResult.pBlocker->IsNPC())
        break;
      ...
      if (checkStepResult.pBlocker->GetMoveType() == MOVETYPE_VPHYSICS) {
        ...
        Vector vObstacleDir = (checkStepResult.pBlocker->WorldSpaceCenter() -
                               GetOuter()->WorldSpaceCenter());
        VectorNormalize(vObstacleDir);

        if (vMoveDir.Dot(vObstacleDir) >= 0) break;
      }

      if ((flags & AITGM_DRAW_RESULTS) && checkStepResult.fStartSolid &&
          checkStepResult.pBlocker->IsNPC()) {
    ...
    if (flags & AITGM_DRAW_RESULTS) {
      if (!CheckStandPosition(checkStepResult.endPoint, collisionMask)) {
        NDebugOverlay::Box(checkStepResult.endPoint, WorldAlignMins(),
                           WorldAlignMaxs(), 255, 0, 0, 0, 0.1);
        NDebugOverlay::Cross3D(checkStepResult.endPoint, 16, 255, 0, 0, true,
                               0.1);
      } else {
        NDebugOverlay::Box(checkStepResult.endPoint, WorldAlignMins(),
                           WorldAlignMaxs(), 0, 255, 0, 0, 0.1);
        NDebugOverlay::Cross3D(checkStepResult.endPoint, 16, 0, 255, 0, true,
                               0.1);
      }
    }
    ...
    if (checkStepResult.pBlocker) {
      distClear +=
          (checkStepResult.endPoint - checkStepArgs.vecStart).Length2D();
      break;
    }
  ...
  if (!(flags & AITGM_2D)) {
    ...
    if (fabs(pMoveTrace->vEndPosition.z - vecDesiredEnd.z) > threshold) {
      ...
      pMoveTrace->flDistObstructed = ComputePathDistance(
          NAV_GROUND, pMoveTrace->vEndPosition, vecDesiredEnd);
      return false;
    }
  }
...
//-----------------------------------------------------------------------------
void CAI_MoveProbe::GroundMoveLimit(
    const Vector &vecStart, const Vector &vecEnd, unsigned int collisionMask,
    const CBaseEntity *pTarget, unsigned testGroundMoveFlags,
    float pctToCheckStandPositions, AIMoveTrace_t *pTrace) const {
  ...
  if (!IterativeFloorPoint(vecStart, collisionMask, &vecActualStart)) {
    ...
    // DevMsg( "Warning: attempting to path from/to a point that is in solid
    // space or is too high\n" );
    return;
  }
  ...
  TestGroundMove(vecActualStart, vecDesiredEnd, collisionMask,
                 pctToCheckStandPositions, testGroundMoveFlags, pTrace);

  // Check to see if the target is in a vehicle and the vehicle is blocking our
  // way
  bool bVehicleMatchesObstruction = false;

  if (pTarget != NULL) {
    CBaseCombatCharacter *pCCTarget =
        ((CBaseEntity *)pTarget)->MyCombatCharacterPointer();
...
//
//			if the move fails, returns the distance remaining to
// vecEnd
//-----------------------------------------------------------------------------
void CAI_MoveProbe::FlyMoveLimit(const Vector &vecStart, const Vector &vecEnd,
  ...
  if (tr.fraction < 1) {
    ...
    pMoveTrace->flDistObstructed =
        ComputePathDistance(NAV_FLY, tr.endpos, vecEnd);
    pMoveTrace->vEndPosition = tr.endpos;
    return;
  }
...
void CAI_MoveProbe::JumpMoveLimit(const Vector &vecStart, const Vector &vecEnd,
  ...
  Vector gravity =
      Vector(0, 0, sv_gravity.GetFloat() * GetOuter()->GetJumpGravity());
  ...
  Vector vecApex;
  Vector rawJumpVel = CalcJumpLaunchVelocity(
  ...
  do {
    rawJumpVel = CalcJumpLaunchVelocity(vecFrom, vecTo, gravity.z,
    ...
    if (!IsJumpLegal(vecFrom, vecApex, vecTo)) {
      ...
      for (float flTime = 0; flTime < jumpTime - 0.01; flTime += timeStep) {
        ...
        // time step)
        Vector nextPos =
            vecTest + (rawJumpVel - 0.5 * gravity * timeStep) * timeStep;
                       ...
                       // after Jay resolves collision eplisons
        {
          ...
          // WorldAlignMaxs(), 255, 255, 0, 0, 10.0 );
  ...
  } while (minJumpHeight > baselineJumpHeight && minJumpHeight <= 1024.0 &&
           minJumpStep >= 16.0);
  ...
  if (minSuccessfulJumpHeight != 1024.0) {
    ...
    pMoveTrace->vJumpVelocity =
        CalcJumpLaunchVelocity(vecFrom, vecTo, gravity.z,
                               &minSuccessfulJumpHeight, maxHorzVel, &vecApex);
  } else {
    // ----------------------------------------------------------
    // If blocked by an npc remember
    // ----------------------------------------------------------
...
void CAI_MoveProbe::ClimbMoveLimit(const Vector &vecStart, const Vector &vecEnd,
  ...
  if (tr.fraction < 1.0) {
    ...
    if (pEntity == pTarget) {
      ...
      float flDist = (1.0 - tr.fraction) *
                     ComputePathDistance(NAV_CLIMB, vecStart, vecEnd);
      if (flDist <= 0.001) {
        flDist = 0.001;
      }
      pMoveTrace->flDistObstructed = flDist;
...
bool CAI_MoveProbe::MoveLimit(Navigation_t navType, const Vector &vecStart,
  ...
  switch (navType) {
    case NAV_GROUND: {
      ...
      if (flags & AIMLF_2D) testGroundMoveFlags |= AITGM_2D;
      if (flags & AIMLF_DRAW_RESULTS) testGroundMoveFlags |= AITGM_DRAW_RESULTS;
      ...
      if (flags & AIMLF_QUICK_REJECT) {
        ...
        trace_t tr;
        TraceLine(const_cast<CAI_MoveProbe *>(this)->GetOuter()->EyePosition(),
                  vecEnd, collisionMask, true, &tr);
        bDoIt = (tr.fraction > 0.99);
      }

      if (bDoIt)
        GroundMoveLimit(vecStart, vecEnd, collisionMask, pTarget,
                        testGroundMoveFlags, pctToCheckStandPositions, pTrace);
      else {
        pTrace->pObstruction = GetContainingEntity(INDEXENT(0));
        pTrace->vHitNormal = vec3_origin;
        pTrace->fStatus = AIMR_BLOCKED_WORLD;
        pTrace->flDistObstructed =
            ComputePathDistance(NAV_GROUND, vecStart, vecEnd);
      }
  ...
  if (IsMoveBlocked(pTrace->fStatus) && pTrace->pObstruction &&
      !pTrace->pObstruction->IsWorld()) {
...
// Output :
//-----------------------------------------------------------------------------
Vector CAI_MoveProbe::CalcJumpLaunchVelocity(const Vector &startPos,
                                             const Vector &endPos,
                                             float flGravity, float *pminHeight,
                                             float maxHorzVelocity,
                                             Vector *pvecApex) const {
  ...
  float minHorzHeight =
      0.5 * flGravity * (minHorzTime * 0.5) * (minHorzTime * 0.5);
  ...
  *pminHeight = std::max(*pminHeight, stepHeight);
  ...
  if (pvecApex) {
    *pvecApex =
        startPos + targetDir2D * velHorz * t0 + Vector(0, 0, *pminHeight);
  }
...
bool CAI_MoveProbe::CheckStandPosition(const Vector &vecStart,
  ...
  if (ai_strong_optimizations_no_checkstand.GetBool()) return true;
  ...
  Vector vecDown =
      Vector(vecStart.x, vecStart.y,
             vecStart.z - StepHeight() * GetOuter()->GetStepDownMultiplier());
  ...
  if (!GetOuter()->IsFlaggedEfficient()) {
    ...
    TraceHull(vecUp, vecDown, contactMin, vHullBottomCenter, collisionMask,
              &trace1);
    if (trace1.fraction != 1.0 && CanStandOn(trace1.m_pEnt)) {
      TraceHull(vecUp, vecDown, vHullBottomCenter, contactMax, collisionMask,
                &trace2);
      if (trace2.fraction != 1.0 &&
          (trace1.m_pEnt == trace2.m_pEnt || CanStandOn(trace2.m_pEnt))) {
        return true;
      }
    }
    ...
    if (trace1.fraction != 1.0 && CanStandOn(trace1.m_pEnt)) return true;
  }
...
bool CAI_MoveProbe::OldCheckStandPosition(const Vector &vecStart,
                                          unsigned int collisionMask) const {
  AI_PROFILE_SCOPE(CAI_Motor_CheckStandPosition);
  ...
  if (trace.fraction == 1.0 || !CanStandOn(trace.m_pEnt)) return false;
  ...
  if (!GetOuter()->IsFlaggedEfficient()) {
    ...
    for (int x = 0; x <= 1; x++) {
      for (int y = 0; y <= 1; y++) {
        ...
        TraceHull(vecUp, vecDown, contactMin, contactMax, collisionMask,
                  &trace);
        ...
        if (trace.fraction == 1.0 || !CanStandOn(trace.m_pEnt)) {
          ...
          if (sumFraction > 2.0) return false;
        }
      }
    }
  }
...
//-----------------------------------------------------------------------------
bool CAI_MoveProbe::FloorPoint(const Vector &vecStart,
                               unsigned int collisionMask, float flStartZ,
                               float flEndZ, Vector *pVecResult) const {
  ...
  Vector vecUp(vecStart.x, vecStart.y,
               vecStart.z + flStartZ + MOVE_HEIGHT_EPSILON);
  ...
  if (trace.fraction == 1 || trace.allsolid ||
      (fStartedInObject && trace.startsolid)) {
    // set result to start position if it doesn't work
    *pVecResult = vecStart;
    if (fStartedInObject)
      return true;  // in this case, probably got intruded on by a physics
                    // object. Try ignoring it...
    return false;
  }
...
//-----------------------------------------------------------------------------
bool CAI_MoveProbe::IterativeFloorPoint(const Vector &vecStart,
                                        unsigned int collisionMask,
...
bool CAI_MoveProbe::IterativeFloorPoint(const Vector &vecStart,
  ...
  return FloorPoint(
      vecStart, collisionMask,
      StepHeight() * GetOuter()->GetStepDownMultiplier() + flAddedStep,
      -(12 * 60), pVecResult);
}

//-----------------------------------------------------------------------------

float CAI_MoveProbe::StepHeight() const { return GetOuter()->StepHeight(); }
...
bool CAI_MoveProbe::IsJumpLegal(const Vector &startPos, const Vector &apex,
