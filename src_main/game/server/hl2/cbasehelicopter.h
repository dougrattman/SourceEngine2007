...
#define BASECHOPPER_WASH_RAMP_TIME \
  1.0f  // Time it takes to ramp from the initial to the max force on an object
...
class CBaseHelicopter : public CAI_TrackPather {
 ...
 protected:
  ...
  // Override the desired position if your derived helicopter is doing something
  // special
 ...
 protected:
  CSoundPatch *m_pRotorSound;  // Rotor loop played when the player can see the
                               // helicopter
...
class CAvoidSphere : public CBaseEntity {
 ...
 public:
  ...
  static void ComputeAvoidanceForces(CBaseEntity *pEntity, float flEntityRadius,
