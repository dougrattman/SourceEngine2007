...
int CNPC_CraneDriver::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  Vector2D vecEnemy2D(GetEnemy()->GetAbsOrigin().x,
                      GetEnemy()->GetAbsOrigin().y);
...
int CNPC_CraneDriver::SelectSchedule(void) {
  ...
  if (m_bForcedPickup) {
    if (m_hPickupTarget) return SCHED_CRANE_PICKUP_OBJECT;
  ...
  switch (m_NPCState) {
    ...
    case NPC_STATE_COMBAT:
      ...
      // We can't attack him, so if we don't have anything on the crane, grab
      // something
      if (m_hCrane->GetTotalMassOnCrane() == 0)
        return SCHED_CRANE_FIND_LARGE_OBJECT;
  }
...
void CNPC_CraneDriver::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_CRANE_TURN_MAGNET_OFF: {
      // If we picked up something, and we were being forced to pick something
      // up, fire our output
    ...
    case TASK_CRANE_FIND_OBJECT_TO_PICKUP: {
      Vector2D vecOrigin2D(m_hCrane->GetAbsOrigin().x,
                           m_hCrane->GetAbsOrigin().y);
      ...
      int count =
          UTIL_EntitiesInBox(pList, 1024, m_hCrane->GetAbsOrigin() - delta,
                             m_hCrane->GetAbsOrigin() + delta, 0);
      for (int i = 0; i < count; i++) {
        if (!pList[i]) continue;
        // Ignore the crane & the magnet
        if (pList[i] == m_hCrane || pList[i] == m_hCrane->GetMagnet()) continue;
        if (m_PreviouslyPickedUpObjects.Find(pList[i]) !=
            m_PreviouslyPickedUpObjects.InvalidIndex())
          continue;
        ...
        if (pPhysics && pList[i]->GetMoveType() == MOVETYPE_VPHYSICS) {
          ...
          if (flMass > flLargestMass && (flMass < MAXIMUM_CRANE_PICKUP_MASS) &&
              (flMass > MINIMUM_CRANE_PICKUP_MASS)) {
            ...
            Vector2D vecOrigin2D(m_hCrane->GetAbsOrigin().x,
                                 m_hCrane->GetAbsOrigin().y);
            Vector2D vecEnemy2D(pList[i]->GetAbsOrigin().x,
                                pList[i]->GetAbsOrigin().y);
            float flDist = (vecOrigin2D - vecEnemy2D).Length();
            // Maximum & Minimum size of the crane's reach
            if (flDist > MAX_CRANE_FLAT_REACH) continue;
            if (flDist < MIN_CRANE_FLAT_REACH) continue;
...
void CNPC_CraneDriver::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_WAIT_FOR_MOVEMENT: {
      ...
      if (m_flDistanceToTarget < flDistance && m_hCrane->GetTurnRate() < 0.1 &&
          vecVelocity.Length() < flVelocity) {
        TaskComplete();
      }
    } break;
    ...
    case TASK_CRANE_TURN_MAGNET_OFF: {
      // We're waiting for the pause length before dropping whatever's on our
      // magnet
...
void CNPC_CraneDriver::DriveVehicle(void) {
  ...
  if (g_debug_vehicledriver.GetInt()) {
    NDebugOverlay::Box(vecTarget, -Vector(50, 50, 50), Vector(50, 50, 50), 0,
                       255, 0, true, 0.1);
    NDebugOverlay::Box(vecCraneTip, -Vector(2, 2, 5000), Vector(2, 2, 5), 0,
                       255, 0, true, 0.1);
    NDebugOverlay::Box(vecTarget, -Vector(2, 2, 5), Vector(2, 2, 5000), 0, 255,
                       0, true, 0.1);
  }
  ...
  float flMinDelta =
      50 + (50 * fabs(m_hCrane->GetExtensionRate() / CRANE_EXTENSION_RATE_MAX));
  ...
  flSpeed = std::min(m_hCrane->GetMaxTurnRate(), flSpeed);
...
void CNPC_CraneDriver::InputForcePickup(inputdata_t &inputdata) {
  ...
  if (iszPickupName != NULL_STRING) {
    // Turn the magnet off now to drop anything we might have already on the
    // magnet
    m_hCrane->TurnMagnetOff();
    m_hPickupTarget = gEntList.FindEntityByName(
        NULL, iszPickupName, NULL, inputdata.pActivator, inputdata.pCaller);
...
void CNPC_CraneDriver::InputForceDrop(inputdata_t &inputdata) {
  ...
  if (iszDropName != NULL_STRING) {
    CBaseEntity *pEntity = gEntList.FindEntityByName(
        NULL, iszDropName, NULL, inputdata.pActivator, inputdata.pCaller);
    if (!pEntity) {
