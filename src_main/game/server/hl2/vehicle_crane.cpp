...
#define CRANE_SPRING_CONSTANT_INITIAL_RAISING \
  (CRANE_SPRING_CONSTANT_HANGING * 0.5)
...
BEGIN_DATADESC(CPropCrane)
  ...
  DEFINE_INPUTFUNC(FIELD_VOID, "Lock", InputLock),
      ...
      // Keys
      DEFINE_EMBEDDED(m_ServerVehicle), DEFINE_EMBEDDED(m_BoneFollowerManager),
      ...
      DEFINE_FIELD(m_hRope, FIELD_EHANDLE), DEFINE_PHYSPTR(m_pConstraintGroup),
      DEFINE_KEYFIELD(m_vehicleScript, FIELD_STRING, "vehiclescript"),

END_DATADESC()

IMPLEMENT_SERVERCLASS_ST(CPropCrane, DT_PropCrane)
SendPropEHandle(SENDINFO(m_hPlayer)), SendPropBool(SENDINFO(m_bMagnetOn)),
...
void CPropCrane::Activate(void) {
  ...
  m_hCraneMagnet = dynamic_cast<CPhysMagnet *>(
      gEntList.FindEntityByName(NULL, STRING(m_iszMagnetName)));
  if (!m_hCraneMagnet) {
  ...
  m_hCraneTip = CCraneTip::Create(m_hCraneMagnet, m_pConstraintGroup, vecOrigin,
                                  vecAngles);
  if (!m_hCraneTip) {
    UTIL_Remove(this);
    return;
  ...
  if (m_hRope) {
    ...
    m_hRope->m_RopeLength =
        (m_hCraneMagnet->GetAbsOrigin() - m_hCraneTip->GetAbsOrigin())
            .Length() *
        1.1;
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCrane::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                             trace_t *ptr) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCrane::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
bool CPropCrane::CanExitVehicle(CBaseEntity *pEntity) {
  ...
  return (!m_bLocked && (GetLocalAngularVelocity() == vec3_angle) &&
          m_bExitAnimOn == false && m_bEnterAnimOn == false);
}
...
void CPropCrane::DrawDebugGeometryOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_BBOX_BIT) {
    ...
    NDebugOverlay::Line(m_hCraneTip->GetAbsOrigin(), vecPoint, 255, 255, 255,
                        true, 0.1);
  }
...
//-----------------------------------------------------------------------------
void CPropCrane::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
...
//-----------------------------------------------------------------------------
void CPropCrane::DriveCrane(int iDriverButtons, int iButtonsPressed,
                            float flNPCSteering) {
  ...
  if (iDriverButtons & IN_MOVELEFT) {
    ...
    if (flNPCSteering) {
      ...
      if (m_flTurn < 0) {
        flTurnAdd = std::max(flTurnAdd, m_flTurnDecel);
      }

      m_flTurn = UTIL_Approach(m_flMaxTurnSpeed, m_flTurn,
                               flTurnAdd * gpGlobals->frametime);
    }
  ...
  if (m_hPlayer) {
    ...
    m_hPlayer->RumbleEffect(RUMBLE_FLAT_BOTH, (int)(rumble * 100),
  ...
  if (iDriverButtons & IN_FORWARD) {
    m_flExtensionRate =
        UTIL_Approach(m_flMaxExtensionSpeed, m_flExtensionRate,
                      m_flExtensionAccel * gpGlobals->frametime);
    m_bExtending = true;
    ...
    m_flExtensionRate = UTIL_Approach(
        0, m_flExtensionRate, m_flExtensionDecel * gpGlobals->frametime);
    m_bExtending = false;
  }
  ...
  float flSpeedPercentage =
      std::clamp(fabs(m_flTurn) / m_flMaxTurnSpeed, 0.0f, 1.0f);
...
void CPropCrane::RecalculateCraneTip(void) {
  ...
  if (m_hCraneTip->VPhysicsGetObject()) {
    m_hCraneTip->VPhysicsGetObject()->UpdateShadow(vecOrigin, vec3_angle, true,
                                                   TICK_INTERVAL * 2.0f);
  }
}
...
void CPropCrane::RunCraneMovement(float flTime) {
  if (m_flExtensionRate) {
    ...
    m_flExtension = std::clamp(
        m_flExtension + (m_flExtensionRate * 10 * flTime), 0.0f, 2.0f);
    SetPoseParameter("armextensionpose", m_flExtension);
    StudioFrameAdvance();
  }
  ...
  if (m_bDropping) {
    ...
    if (m_hCraneMagnet->HasHitSomething()) {
      ...
      m_hCraneTip->m_pSpring->SetSpringConstant(
          CRANE_SPRING_CONSTANT_INITIAL_RAISING);
      m_bDropping = false;
    ...
    flDelta = std::clamp(flDelta, 0.0f, CRANE_SLOWRAISE_TIME);
    float flCurrentSpringConstant = RemapVal(
        flDelta, 0, CRANE_SLOWRAISE_TIME, CRANE_SPRING_CONSTANT_INITIAL_RAISING,
        CRANE_SPRING_CONSTANT_HANGING);
    m_hCraneTip->m_pSpring->SetSpringConstant(flCurrentSpringConstant);
  }

  // If we've moved in any way, update the tip
  if (m_bDropping || m_flExtensionRate ||
      GetLocalAngularVelocity() != vec3_angle) {
    RecalculateCraneTip();
  }

  // Make danger sounds underneath the magnet if we have something attached to
  // it
  /*
  if ( (m_flNextDangerSoundTime < gpGlobals->curtime) &&
  (m_hCraneMagnet->GetTotalMassAttachedObjects() > 0) )
  {
          // Trace down from the magnet and make a danger sound on the ground
          trace_t tr;
          Vector vecSource = m_hCraneMagnet->GetAbsOrigin();
          UTIL_TraceLine( vecSource, vecSource - Vector(0,0,2048),
  MASK_SOLID_BRUSHONLY, m_hCraneMagnet, 0, &tr );

          if ( tr.fraction < 1.0 )
          {
                  // Make the volume proportional to the amount of mass on the
  magnet float flVolume = std::clamp(
  (m_hCraneMagnet->GetTotalMassAttachedObjects() * 0.5), 100.f, 600.f );
                  CSoundEnt::InsertSound( SOUND_DANGER, tr.endpos, flVolume,
  0.2, this );
  ...
  m_hCraneMagnet->GetTotalMassAttachedObjects(), flVolume );
                  ...
                  //NDebugOverlay::Box( tr.endpos, -vecVolume, vecVolume,
  255,0,0, false, 0.3 );
                  //NDebugOverlay::Cross3D( tr.endpos, -Vector(10,10,10),
  Vector(10,10,10), 255,0,0, false, 0.3 );
          }
  ...
  */
  ...
  if ((m_flNextCreakSound < gpGlobals->curtime) &&
      (m_hCraneMagnet->GetTotalMassAttachedObjects() > 100)) {
    // Randomly play creaks from the magnet, and increase the chance based on
    // the turning speed
    float flSpeedPercentage =
        std::clamp(fabs(m_flTurn) / m_flMaxTurnSpeed, 0.0f, 1.0f);
    if (RandomFloat(0, 1) > (0.95 - (0.1 * flSpeedPercentage))) {
      if (m_ServerVehicle.m_vehicleSounds.iszSound[VS_MISC4] != NULL_STRING) {
        ...
        ep.m_pSoundName =
            STRING(m_ServerVehicle.m_vehicleSounds.iszSound[VS_MISC4]);
        ep.m_flVolume = 1.0f;
...
//-----------------------------------------------------------------------------
void CPropCrane::GetVectors(Vector *pForward, Vector *pRight,
                            Vector *pUp) const {
...
//-----------------------------------------------------------------------------
void CPropCrane::InputLock(inputdata_t &inputdata) { m_bLocked = true; }

//-----------------------------------------------------------------------------
// Purpose: Allow the player to enter / exit the vehicle
//-----------------------------------------------------------------------------
void CPropCrane::InputUnlock(inputdata_t &inputdata) { m_bLocked = false; }
...
void CPropCrane::SetNPCDriver(CNPC_VehicleDriver *pDriver) {
  ...
  if (pDriver) {
    ...
    m_flExtensionDecel = CRANE_EXTENSION_DECEL *
                         20;  // Npcs stop quickly to make them more accurate
    m_flTurnAccel = CRANE_TURN_ACCEL * 2;
    m_flTurnDecel =
        CRANE_DECEL * 10;  // Npcs stop quickly to make them more accurate
...
void CCraneServerVehicle::GetVehicleViewPosition(int nRole, Vector *pAbsOrigin,
  ...
  *pAbsAngles = pPlayer->EyeAngles();  // yuck. this is an in/out parameter.
  ...
  GetCrane()->GetAttachment("vehicle_driver_eyes", vehicleEyeOrigin,
                            vehicleEyeAngles);
  ...
  if (g_debug_vehicledriver.GetInt()) {
    if (m_nNPCButtons) {
      ...
      if (m_nNPCButtons & IN_FORWARD) {
        NDebugOverlay::Line(GetCrane()->GetAbsOrigin(),
                            GetCrane()->GetAbsOrigin() + vecForward * 200, 0,
                            255, 0, true, 0.1);
      }
      ...
      if (m_nNPCButtons & IN_MOVELEFT) {
        NDebugOverlay::Line(GetCrane()->GetAbsOrigin(),
                            GetCrane()->GetAbsOrigin() - vecRight * 200, 0, 255,
                            0, true, 0.1);
      }
      ...
      if (m_nNPCButtons & IN_JUMP) {
        NDebugOverlay::Box(GetCrane()->GetAbsOrigin(), -Vector(20, 20, 20),
                           Vector(20, 20, 20), 0, 255, 0, true, 0.1);
      }
    }
  }
...
//-----------------------------------------------------------------------------
bool CCraneTip::CreateConstraint(CBaseAnimating *pCraneMagnet,
  ...
  if (!pCraneMagnetPhysObject) {
    Msg(" Error: Tried to create a crane_tip with a crane magnet that has no "
        "physics model.\n");
    return false;
  }
  Assert(pPhysObject);
  ...
  length.Defaults();
  length.InitWorldspace( pPhysObject, pCraneMagnetPhysObject, GetAbsOrigin(),
  vecPoint ); length.constraint.Defaults(); m_pConstraint =
  physenv->CreateLengthConstraint( pPhysObject, pCraneMagnetPhysObject, pGroup,
  length );
  */
  ...
  m_pSpring =
      physenv->CreateSpring(pPhysObject, pCraneMagnetPhysObject, &spring);
...
//-----------------------------------------------------------------------------
CCraneTip *CCraneTip::Create(CBaseAnimating *pCraneMagnet,
                             IPhysicsConstraintGroup *pGroup,
                             const Vector &vecOrigin, const QAngle &vecAngles) {
  CCraneTip *pCraneTip =
      (CCraneTip *)CBaseEntity::Create("crane_tip", vecOrigin, vecAngles);
  if (!pCraneTip) return NULL;

  if (!pCraneTip->CreateConstraint(pCraneMagnet, pGroup)) return NULL;
