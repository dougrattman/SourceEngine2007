...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_Spotlight::Init(CAI_BaseNPC *pOuter, int nFlags,
                         float flConstraintAngle, float flMaxLength) {
  SetOuter(pOuter);
  m_nFlags = nFlags;
...
void CAI_Spotlight::ComputeEndpoint(const Vector &vecStartPoint,
  ...
  // Create the endpoint
  trace_t tr;
  AI_TraceLine(vecStartPoint,
               vecStartPoint + m_vSpotlightDir * 2 * m_flSpotlightMaxLength,
...
// Purpose:
//------------------------------------------------------------------------------
void CAI_Spotlight::SpotlightCreate(int nAttachment,
                                    const Vector &vecInitialDir) {
...
// Purpose:
//------------------------------------------------------------------------------
void CAI_Spotlight::SetSpotlightTargetDirection(
    const Vector &vSpotlightTargetDir) {
  if (!m_hSpotlight) {
    CreateSpotlightEntities();
  }

  VectorMA(m_hSpotlight->GetAbsStartPos(), 1000.0f, vSpotlightTargetDir,
...
bool CAI_Spotlight::ConstrainToCone(Vector *pDirection) {
  ...
  if (flDot >= cos(DEG2RAD(m_flConstraintAngle))) return false;
...
void CAI_Spotlight::UpdateSpotlightDirection(void) {
  ...
  Vector vTargetDir;
  VectorSubtract(m_vSpotlightTargetPos, m_hSpotlight->GetAbsStartPos(),
                 vTargetDir);
  ...
  float flClampedAngle = std::clamp(flAngle, 0.0f, 45.0f);
  float flBeamTurnRate =
      SimpleSplineRemapVal(flClampedAngle, 0.0f, 45.0f, 10.0f, 45.0f);
  ...
  QuaternionSlerp(m_vAngularVelocity, desiredQuat, QUAT_BLEND_FACTOR,
                  resultQuat);
  m_vAngularVelocity = resultQuat;

  // If we're really close, and we're not moving very quickly, slam.
  float flActualRotation =
      AngleNormalize(RAD2DEG(2 * acos(m_vAngularVelocity.w)));
...
void CAI_Spotlight::UpdateSpotlightEndpoint(void) {
  ...
  flNewWidth = std::min(100.0f, flNewWidth);
