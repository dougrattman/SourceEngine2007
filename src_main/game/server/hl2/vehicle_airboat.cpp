...
#define AIRBOAT_LOCK_SPEED \
  10  // Airboat must be going slower than this for player to enter or exit, in
      // in/sec
...
static ConVar hud_airboathint_numentries("hud_airboathint_numentries", "10",
                                         FCVAR_NONE);
static ConVar airboat_fatal_stress(
    "airboat_fatal_stress", "5000", FCVAR_NONE,
...
class CPropAirboat : public CPropVehicleDriveable {
 ...
 public:
  ...
  virtual void DriveVehicle(float flFrameTime, CUserCmd *ucmd, int iButtonsDown,
  ...
  void ComputePDControllerCoefficients(float *pCoefficientsOut,
                                       float flFrequency, float flDampening,
                                       float flDeltaTime);
  void DampenForwardMotion(Vector &vecVehicleEyePos,
                           QAngle &vecVehicleEyeAngles, float flFrameTime);
  void DampenUpMotion(Vector &vecVehicleEyePos, QAngle &vecVehicleEyeAngles,
                      float flFrameTime);
  ...
  // Airboat passengers do not directly receive damage from blasts or radiation
  // damage
  virtual bool PassengerShouldReceiveDamage(CTakeDamageInfo &info) {
    if (info.GetDamageType() & DMG_VEHICLE) return true;

    return (info.GetDamageType() & (DMG_RADIATION | DMG_BLAST | DMG_CRUSH)) ==
           0;
  }
 ...
 private:
  ...
  void UpdateEngineSound(CSoundEnvelopeController &controller,
                         float speedRatio);
 ...
 private:
  ...
  float m_flPrevThrottle;  // Throttle during last think. Used for detecting
                           // state changes.
  float m_flSpinRate;  // Current rate of spin of propeller: 0 = min, 1.0 = max
  float m_flTargetSpinRate;  // Target rate of spin of propeller: 0 = min, 1.0 =
                             // max
  ...
  float m_flEngineDuckTime;  // Time to reduce the volume of the engine's idle
                             // sound.

  bool m_bFadeOutFan;  // Fade out fan sound after cruising at max speed for a
                       // while.

  int m_nPrevWaterLevel;  // Used for detecting transitions into/out of water.
  float m_flWaterStoppedPitchTime;  // Time to pitch shift the water stopped
                                    // sound.

  float m_flLastImpactEffectTime;
  int m_iNumberOfEntries;

  IPhysicsConstraint *m_pAntiFlipConstraint;  // A ragdoll constraint that
                                              // prevents us from flipping.
...
SendPropBool(SENDINFO(m_bHeadlightIsOn)),
    ...
    SendPropVector(SENDINFO(m_vecPhysVelocity)), END_SEND_TABLE();
...
BEGIN_DATADESC(CPropAirboat)
  DEFINE_FIELD(m_vecLastEyePos, FIELD_POSITION_VECTOR),
      ...
      DEFINE_SOUNDPATCH(m_pFanSound), DEFINE_SOUNDPATCH(m_pFanMaxSpeedSound),
      DEFINE_SOUNDPATCH(m_pEngineSound), DEFINE_SOUNDPATCH(m_pWaterFastSound),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StartRotorWashForces",
                       InputStartRotorWashForces),
      DEFINE_INPUTFUNC(FIELD_VOID, "StopRotorWashForces",
...
void CPropAirboat::CreateAntiFlipConstraint() {
  ...
  BuildObjectRelativeXform(g_PhysWorldObject, VPhysicsGetObject(),
                           ragdoll.constraintToReference);
  ...
  m_pAntiFlipConstraint = physenv->CreateRagdollConstraint(
      g_PhysWorldObject, VPhysicsGetObject(), NULL, ragdoll);

  // NDebugOverlay::Cross3DOriented( ragdoll.constraintToReference, 128, 255,
  // true, 100 );
}
...
void CPropAirboat::Activate() {
  ...
  CBaseServerVehicle *pServerVehicle =
      dynamic_cast<CBaseServerVehicle *>(GetServerVehicle());
  if (pServerVehicle) {
    if (pServerVehicle->GetPassenger()) {
      // If a boat comes back from a save game with a driver, make sure the
      // engine rumble starts up.
...
void CPropAirboat::CreatePlayerBlocker() {
  ...
  m_hPlayerBlocker = CEntityBlocker::Create(GetAbsOrigin(), Vector(-84, -32, 0),
                                            Vector(54, 32, 84), this, false);
...
Vector CPropAirboat::GetSmoothedVelocity(void) {
  ...
  if (flSpeed >= SMOOTHED_MAX_VELOCITY) return vecSmoothedVelocity;
  ...
  float flBlend = SimpleSplineRemapVal(flSpeed, SMOOTHED_MIN_VELOCITY,
                                       SMOOTHED_MAX_VELOCITY, 0.0f, 1.0f);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::TraceAttack(const CTakeDamageInfo &inputInfo,
...
int CPropAirboat::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (m_hPlayer != NULL) {
    ...
    // Mark that we're passing it to the player so the base player accepts the
    // damage
...
// Scraping noises for the various things we drive on.
//-----------------------------------------------------------------------------
void CPropAirboat::VPhysicsFriction(IPhysicsObject *pObject, float energy,
                                    int surfaceProps, int surfacePropsHit) {
  ...
  PhysFrictionSound(this, pObject, pSoundName, psurf->soundhandles.scrapeRough,
                    flVolume);
}
...
void CPropAirboat::RechargeAmmo(void) {
  ...
  if (m_nAmmoCount == nMaxAmmo) return;
...
void CPropAirboat::ComputeAimPoint(Vector *pVecAimPoint) {
  ...
  if (g_pGameRules->GetAutoAimMode() == AUTOAIM_ON_CONSOLE) {
    ...
    params.m_fScale =
        AUTOAIM_SCALE_DEFAULT * sv_vehicle_autoaim_scale.GetFloat();
  ...
  Vector vecEndPos;
  VectorMA(m_hPlayer->EyePosition(), MAX_TRACE_LENGTH, vecEyeDirection,
           vecEndPos);
  trace_t trace;
  UTIL_TraceLine(m_hPlayer->EyePosition(), vecEndPos, MASK_SHOT, this,
...
void CPropAirboat::Think(void) {
  ...
  trace_t tr;
  UTIL_TraceHull(startPos, endPos, vecMins, vecMaxs,
...
void CPropAirboat::UpdatePropeller() {
  if ((m_bExitAnimOn) || (m_bEnterAnimOn)) return;
...
#define SPIN_RATE_HIGH 0.6
  ...
  if (fabs(m_flSpinRate) > SPIN_RATE_HIGH) {
    ...
    if ((fabs(flPrevSpinRate) <= SPIN_RATE_MED) ||
        (fabs(flPrevSpinRate) > SPIN_RATE_HIGH)) {
      SetBodygroup(AIRBOAT_BODYGROUP_PROP, true);
      SetBodygroup(AIRBOAT_BODYGROUP_BLUR, true);
      SetSequence(LookupSequence("propeller_spin1"));
    }
...
void CPropAirboat::CreateSounds() {
  ...
  if (!m_pEngineSound) {
    m_pEngineSound =
        controller.SoundCreate(filter, entindex(), "Airboat_engine_idle");
    controller.Play(m_pEngineSound, 0, 100);
  }

  if (!m_pFanSound) {
    m_pFanSound =
        controller.SoundCreate(filter, entindex(), "Airboat_fan_idle");
    controller.Play(m_pFanSound, 0, 100);
  }

  if (!m_pFanMaxSpeedSound) {
    m_pFanMaxSpeedSound =
        controller.SoundCreate(filter, entindex(), "Airboat_fan_fullthrottle");
    controller.Play(m_pFanMaxSpeedSound, 0, 100);
  }

  if (!m_pWaterStoppedSound) {
    m_pWaterStoppedSound =
        controller.SoundCreate(filter, entindex(), "Airboat_water_stopped");
    controller.Play(m_pWaterStoppedSound, 0, 100);
  }

  if (!m_pWaterFastSound) {
    m_pWaterFastSound =
        controller.SoundCreate(filter, entindex(), "Airboat_water_fast");
    controller.Play(m_pWaterFastSound, 0, 100);
  }

  if (!m_pGunFiringSound) {
    m_pGunFiringSound =
        controller.SoundCreate(filter, entindex(), "Airboat.FireGunLoop");
...
//-----------------------------------------------------------------------------
void CPropAirboat::UpdateEngineSound(CSoundEnvelopeController &controller,
                                     float speedRatio) {
...
#define ENGINE_DUCK_TIME 4.0
  ...
  if (controller.SoundGetVolume(m_pEngineSound) == 0) {
    if (gpGlobals->curtime > m_flEngineIdleTime) {
      // If we've finished playing the engine start sound, start playing the
      // idle sound.
      controller.Play(m_pEngineSound, ENGINE_MAX_VOLUME, 100);

      // Ramp down the engine idle sound over time so that we can ramp it back
      // up again based on speed.
      controller.SoundChangeVolume(m_pEngineSound, ENGINE_MIN_VOLUME,
                                   ENGINE_DUCK_TIME);
      controller.SoundChangePitch(m_pEngineSound, ENGINE_MIN_PITCH,
                                  ENGINE_DUCK_TIME);

      // Reduce the volume of the engine idle sound after our ears get 'used' to
      // it.
    ...
    controller.SoundChangeVolume(
        m_pEngineSound,
        RemapValClamped(speedRatio, 0, 1.0, ENGINE_MIN_VOLUME,
                        ENGINE_MAX_VOLUME),
        0.0);
    controller.SoundChangePitch(
        m_pEngineSound,
        RemapValClamped(speedRatio, 0, 1.0, ENGINE_MIN_PITCH, ENGINE_MAX_PITCH),
        0);
  }
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::UpdateFanSound(CSoundEnvelopeController &controller,
                                  float speedRatio) {
...
#define FAN_DUCK_TIME 2.0           // seconds over which to duck the fan sound
  ...
  if (speedRatio >= 0.8) {
    ...
    controller.SoundChangeVolume(
        m_pFanSound,
        RemapValClamped(speedRatio, 0.8, 1.0, FAN_MAX_VOLUME, FAN_MIN_VOLUME),
        FAN_CHANGE_VOLUME_TIME);
    controller.SoundChangeVolume(
        m_pFanMaxSpeedSound,
        RemapValClamped(speedRatio, 0.8, 1.0, FAN_MIN_VOLUME, FAN_MAX_VOLUME),
        FAN_CHANGE_VOLUME_TIME);
    ...
    controller.SoundChangeVolume(
        m_pFanSound,
        RemapValClamped(fabs(m_flThrottle), 0, 1.0, FAN_MIN_VOLUME,
                        FAN_MAX_VOLUME),
        0.25);
    controller.SoundChangeVolume(m_pFanMaxSpeedSound, 0.0, 0.0);
  }

  controller.SoundChangePitch(m_pFanSound, 100 * (fabs(m_flThrottle) + 0.2),
                              0.25);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::UpdateWaterSound(CSoundEnvelopeController &controller,
                                    float speedRatio) {
  int nWaterLevel = GetWaterLevel();

  // Manage the state of the water stopped sound (gentle lapping at the
  // pontoons).
  if (nWaterLevel == 0) {
    controller.SoundChangeVolume(m_pWaterStoppedSound, 0.0, 0.0);
  } else {
    if (m_nPrevWaterLevel == 0) {
      ...
      if ((fabs(vecVelocityWorld.x) > 400) ||
          (fabs(vecVelocityWorld.y) > 400) ||
          (fabs(vecVelocityWorld.z) > 400)) {
    ...
    if (speedRatio <= 0.1) {
      if (!controller.SoundGetVolume(m_pWaterStoppedSound)) {
        ...
        m_flWaterStoppedPitchTime =
            gpGlobals->curtime + random->RandomFloat(1.0, 3.0);
      } else if (gpGlobals->curtime > m_flWaterStoppedPitchTime) {
        controller.SoundChangeVolume(m_pWaterStoppedSound,
                                     random->RandomFloat(0.2, 1.0),
                                     random->RandomFloat(1.0, 3.0));
        controller.SoundChangePitch(m_pWaterStoppedSound,
                                    random->RandomFloat(90, 110),
                                    random->RandomFloat(1.0, 3.0));
        m_flWaterStoppedPitchTime =
            gpGlobals->curtime + random->RandomFloat(2.0, 4.0);
      }
    } else {
      if (controller.SoundGetVolume(m_pWaterStoppedSound)) {
  ...
  // Manage the state of the water fast sound (water hissing under the
  // pontoons).
  if (nWaterLevel == 0) {
    controller.SoundChangeVolume(m_pWaterFastSound, 0.0, 0.0);
  } else {
...
void CPropAirboat::FireGun() {
  ...
  */
  ...
  if (pDriver && pDriver->IsPlayer()) {
    pPlayerDriver = dynamic_cast<CBasePlayer *>(pDriver);
    if (pPlayerDriver) {
      pPlayerDriver->RumbleEffect(RUMBLE_AIRBOAT_GUN, 0,
                                  RUMBLE_FLAG_LOOP | RUMBLE_FLAG_ONLYONE);
    }
  }
  ...
  if (gpGlobals->curtime >= m_flNextGunShakeTime) {
    UTIL_ScreenShakeObject(this, WorldSpaceCenter(), 0.2, 250.0,
                           CANNON_SHAKE_INTERVAL, 250, SHAKE_START);
    m_flNextGunShakeTime = gpGlobals->curtime + 0.5 * CANNON_SHAKE_INTERVAL;
  }

  // Specifically kill APC missiles in the cone. But we're going to totally
  // cheat because it's hard to hit them when they are close. Use the player's
  // eye position as the center of the cone.
  if (!m_hPlayer) return;
  ...
  CAPCMissile *pEnt =
      FindAPCMissileInCone(vecEyePosition, vecEyeDirection, 2.5f);
  if (pEnt && (pEnt->GetHealth() > 0)) {
    CTakeDamageInfo info(this, this, 1, DMG_AIRBOAT);
    CalculateBulletDamageForce(&info, ammoType, vecRay,
                               pEnt->WorldSpaceCenter());
    pEnt->TakeDamage(info);

    Vector vecVelocity = pEnt->GetAbsVelocity();

    // Pick a vector perpendicular to the vecRay which will push it away from
    // the airboat
    Vector vecPerp;
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::DriveVehicle(float flFrameTime, CUserCmd *ucmd,
...
void CPropAirboat::ProcessMovement(CBasePlayer *pPlayer, CMoveData *pMoveData) {
  ...
  if (gpGlobals->frametime != 0) {
    ...
    if (m_VehiclePhysics.GetThrottle() > 0) {
      CSoundEnt::InsertSound(SOUND_PLAYER_VEHICLE, pPlayer->GetAbsOrigin(),
                             3500, 0.1f, pPlayer,
...
void CPropAirboat::CreateDangerSounds(void) {
  ...
  if (fabs(speed) > 120) {
    ...
    CSoundEnt::InsertSound(SOUND_DANGER, vecSpot, radius, soundDuration, this,
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::DampenEyePosition(Vector &vecVehicleEyePos,
                                     QAngle &vecVehicleEyeAngles) {
  // Get the frametime. (Check to see if enough time has passed to warrent
  // dampening).
...
// speed += ( pCoefficientsOut[0] * ( targetPos - currentPos ) +
// pCoefficientsOut[1] * ( targetSpeed - currentSpeed ) ) * flDeltaTime;
//-----------------------------------------------------------------------------
void CPropAirboat::ComputePDControllerCoefficients(float *pCoefficientsOut,
  ...
  float flScale =
      1.0f / (1.0f + flKd * flDeltaTime + flKs * flDeltaTime * flDeltaTime);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::DampenForwardMotion(Vector &vecVehicleEyePos,
                                       QAngle &vecVehicleEyeAngles,
                                       float flFrameTime) {
  // Get forward vector.
  Vector vecForward;
  ...
  // Calculate target speed based on the current vehicle eye position and the
  // last vehicle eye position and frametime.
  Vector vecVehicleEyeSpeed =
      (vecVehicleEyePos - m_vecLastEyeTarget) / flFrameTime;
  ...
  // Generate an updated (dampening) speed for use in next frames position
  // extrapolation.
  float flCoefficients[2];
  ComputePDControllerCoefficients(
      flCoefficients, r_AirboatViewDampenFreq.GetFloat(),
      r_AirboatViewDampenDamp.GetFloat(), flFrameTime);
  m_vecEyeSpeed +=
      ((flCoefficients[0] * vecDeltaPos + flCoefficients[1] * vecDeltaSpeed) *
       flFrameTime);
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropAirboat::DampenUpMotion(Vector &vecVehicleEyePos,
                                  QAngle &vecVehicleEyeAngles,
                                  float flFrameTime) {
  // Get up vector.
  Vector vecUp;
...
void CPropAirboat::CreateSplash(int nSplashType) {
  ...
  switch (nSplashType) {
    ...
    case AIRBOAT_SPLASH_RIPPLE: {
      ...
      vecSplashDir = vecUp;
      data.m_vNormal = vecSplashDir;
      data.m_flScale = AIRBOAT_SPLASH_RIPPLE_SIZE + random->RandomFloat( 0,
      AIRBOAT_SPLASH_RIPPLE_SIZE * 0.25 ); DispatchEffect( "waterripple", data
      );
      */
    }
    default: { return; }
...
//-----------------------------------------------------------------------------
float CPropAirboat::CalculatePhysicsStressDamage(
  ...
  if (stressOut.hasNonStaticStress &&
      (stressOut.receivedStress > airboat_fatal_stress.GetFloat())) {
