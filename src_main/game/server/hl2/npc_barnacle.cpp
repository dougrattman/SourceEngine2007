...
static ConVar npc_barnacle_swallow("npc_barnacle_swallow", "0", 0,
                                   "Use prototype swallow code.");

const char *CNPC_Barnacle::m_szGibNames[NUM_BARNACLE_GIBS] = {
    "models/gibs/hgibs.mdl", "models/gibs/hgibs_scapula.mdl",
    "models/gibs/hgibs_rib.mdl", "models/gibs/hgibs_spine.mdl"};
...
BEGIN_DATADESC(CNPC_Barnacle)
  ...
  DEFINE_FIELD(m_flAltitude, FIELD_FLOAT),
      DEFINE_FIELD(m_cGibs, FIELD_INTEGER),  // barnacle loads up on gibs each
                                             // time it kills something.
      ...
      DEFINE_FIELD(m_flLastPull, FIELD_FLOAT), DEFINE_EMBEDDED(m_StuckTimer),

      DEFINE_INPUTFUNC(FIELD_VOID, "DropTongue", InputDropTongue),
      DEFINE_INPUTFUNC(FIELD_INTEGER, "SetDropTongueSpeed",
      ...
      // Function pointers
      DEFINE_THINKFUNC(BarnacleThink), DEFINE_THINKFUNC(WaitTillDead),
...
//-----------------------------------------------------------------------------
void CNPC_Barnacle::ComputeWorldSpaceSurroundingBox(Vector *pVecWorldMins,
...
void CNPC_Barnacle::Spawn() {
  ...
  m_flFieldOfView = 0.5;  // indicates the width of this monster's forward view
...
void CNPC_Barnacle::SetAltitude(float flAltitude) {
  if (HasSpawnFlags(SF_BARNACLE_AMBUSH)) return;

  m_flAltitude = flAltitude;
}

void CNPC_Barnacle::DropTongue(void) {
  if (m_hTongueRoot) return;

  m_hTongueRoot =
      CBarnacleTongueTip::CreateTongueRoot(m_vecRoot, QAngle(90, 0, 0));
  m_hTongueTip = CBarnacleTongueTip::CreateTongueTip(this, m_hTongueRoot,
                                                     m_vecTip, QAngle(0, 0, 0));
...
void CNPC_Barnacle::Activate(void) {
  ...
  if (!m_hTongueRoot) {
    ...
    m_pConstraint =
        physenv->CreateFixedConstraint(pTonguePhys, pPlayerPhys, NULL, fixed);
  }
}
...
int CNPC_Barnacle::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (hl2_episodic.GetBool() && info.GetAttacker() &&
      info.GetAttacker()->Classify() == CLASS_PLAYER_ALLY_VITAL) {
...
void CNPC_Barnacle::PlayerHasIlluminatedNPC(CBasePlayer *pPlayer, float flDot) {
  ...
  if (IsAlive()) {
    CSoundEnt::InsertSound(SOUND_MOVE_AWAY | SOUND_CONTEXT_ALLIES_ONLY,
                           m_vecTip, 60.0f, FLASHLIGHT_NPC_CHECK_INTERVAL);
  }
}
...
//-----------------------------------------------------------------------------
// Purpose:
// TODO: The LostPrey(true) at the top of 	if ( m_hRagdoll ) isnt' quite
// right: it will make the barnacle drop anything that's shot on the way up.
// This is a quick fix for the antlions which crashed otherwise (they have
// somewhat anomalous ragdoll behaivor) but should be revisted.
//-----------------------------------------------------------------------------
void CNPC_Barnacle::BarnacleThink(void) {
  ...
  if (m_hRagdoll) {
    ...
    // Stay bloated as we digest
      ...
      if (m_flDigestFinish > gpGlobals->curtime) {
        ...
        // Finished digesting
#if HL2_EPISODIC
        // have to save this off because LostPrey() resets it (and if we take
        // damage before hitting that, then the dead thing will go flying)
    ...
    // barnacle has no prey right now, so just idle and check to see if anything
    // is touching the tongue.

    // If idle and no nearby client, don't think so often
    // NOTE: Use the surrounding bounds so that we'll think often event if the
    // tongue tip is in the PVS but the body isn't
    ...
    if (!UTIL_FindClientInPVS(vecSurroundMins, vecSurroundMaxs)) {
      SetNextThink(
          gpGlobals->curtime +
          random->RandomFloat(1, 1.5));  // Stagger a bit to keep barnacles from
                                         // thinking on the same frame
    }
    ...
    if (m_flAltitude >= flLength) {
      ...
      if ((pTouchEnt != NULL) && (pTouchEnt != m_hLastSpitEnemy.Get())) {
        ...
        CBaseCombatCharacter *pBCC =
            dynamic_cast<CBaseCombatCharacter *>(pTouchEnt);

        if (CanPickup(pBCC)) {
          Vector vecGrabPos = pTouchEnt->EyePosition();
          if (!pBCC ||
              pBCC->DispatchInteraction(g_interactionBarnacleVictimGrab,
                                        &vecGrabPos, this)) {
            EmitSound("NPC_Barnacle.BreakNeck");
            AttachTongueToTarget(pTouchEnt, vecGrabPos);

            // Set the local timer to 60 seconds, which starts the lifting phase
            // on the upshot of the sine wave which right away makes it more
            // obvious that the player is being lifted.
      ...
      if (!bGrabbedTarget) {
        ...
        if (m_hTongueTip) {
          m_hTongueTip->m_pSpring->SetSpringConstant(
  ...
  // NDebugOverlay::Box( GetAbsOrigin() - Vector( 0, 0, m_flAltitude ), Vector(
  // -2, -2, -2 ), Vector( 2, 2, 2 ), 255,255,255, 0, 0.1 );
...
bool CNPC_Barnacle::CanPickup(CBaseCombatCharacter *pBCC) {
  ...
  // in TongueTouchEnt. It just isn't an NPC or player and doesn't need further
  // inspection.
  if (!pBCC) return true;

  // Don't pickup turrets
  if (FClassnameIs(pBCC, "npc_turret_floor")) return false;

  // Don't pick up a dead player or NPC
  if (!pBCC->IsAlive()) return false;
  ...
  if (pBCC->IsPlayer()) {
    ...
    if (pPlayer->HasPhysicsFlag(PFLAG_ONBARNACLE)) return false;
...
bool CNPC_Barnacle::WaitForRagdollToSettle(float flBiteZOffset) {
  ...
  // NDebugOverlay::Box( vecBitePoint, -Vector(10,10,10), Vector(10,10,10),
  // 0,255,0, 0, 0.1 ); NDebugOverlay::Line( vecBitePoint, vecCheckPos, 0, 255,
  // 0, true, 0.1 );

  if ((vecBitePoint.x - vecCheckPos.x) > flDelta ||
      (vecBitePoint.y - vecCheckPos.y) > flDelta) {
    // I can't bite this critter because it's not lined up with me on the X/Y
    // plane. If it is as close to my mouth as I can get it, I should drop it.
  ...
  // Get the velocity of the bone we've grabbed, and only bite when it's not
  // moving much
...
//-----------------------------------------------------------------------------
// Purpose: Make a horrific noise before we pull the prey stuck to our tongue up
// towards our mouth
//-----------------------------------------------------------------------------
...
void CNPC_Barnacle::PullEnemyTorwardsMouth(bool bAdjustEnemyOrigin) {
  ...
  if (bAdjustEnemyOrigin) {
    if (m_flLastPull > 1.0) {
      if ((pEnemy->GetAbsOrigin() - m_vLastEnemyPos).LengthSqr() <
          Square(m_flLastPull - 1.0)) {
...
void CNPC_Barnacle::UpdatePlayerConstraint(void) {
  ...
  if (bStanding == m_bPlayerWasStanding) return;
...
void CNPC_Barnacle::LiftPlayer(float flBiteZOffset) {
  ...
  // Figure out when the prey has reached our bite range use eye position to
  // avoid clipping into the barnacle body
...
void CNPC_Barnacle::LiftRagdoll(float flBiteZOffset) {
  ...
  if (GetAbsOrigin().z - m_vecTip.Get().z < flBiteZOffset) {
    ...
    if (!WaitForRagdollToSettle(flBiteZOffset)) return;
    ...
    // Apply forces to the attached ragdoll based upon the animations of the
    // enemy, if the enemy is still alive.
    if (GetEnemy()->IsAlive()) {
      ...
      CalcPoseSingle( pStudioHdr, pos, q, pAnimating->GetSequence(),
      pAnimating->m_flCycle, pAnimating->GetPoseParameterArray(),
      BONE_USED_BY_ANYTHING ); Studio_BuildMatrices( pStudioHdr, vec3_angle,
      vec3_origin, pos, q, -1, pBoneToWorld, BONE_USED_BY_ANYTHING );
      ...
      */
      ...
      // Apply the forces to the ragdoll
      RagdollApplyAnimationAsVelocity(*(m_hRagdoll->GetRagdoll()),
                                      m_pRagdollBones, pBoneToWorld, 0.2);
...
void CNPC_Barnacle::LiftPhysicsObject(float flBiteZOffset) {
  ...
  // NDebugOverlay::Box( vecCheckPos, -Vector(10,10,10), Vector(10,10,10),
  // 255,255,255, 0, 0.1 );
  ...
  // Figure out when the prey has reached our bite range
  if (GetAbsOrigin().z - m_vecTip.Get().z <
      flBiteZOffset)  // then yes, let's chomp
  {
    if (m_hTongueTip) {
      m_hTongueTip->m_pSpring->SetSpringConstant(
          BARNACLE_TONGUE_SPRING_CONSTANT_HANGING);
    ...
    if (!WaitForPhysicsObjectToSettle(flBiteZOffset)) return;
...
#ifdef HL2_EPISODIC
    // if the object is a combatclass, send it a chomp interaction in case it
    // wants to respond to that in some nonstandard way.
...
void CNPC_Barnacle::LiftPrey(void) {
  ...
  trace_t tr;
  AI_TraceLine(WorldSpaceCenter(), pVictim->WorldSpaceCenter(), MASK_SOLID,
               this, COLLISION_GROUP_NONE, &tr);
  bool bEnemyIsNPC = IsEnemyAnNPC() && !IsEnemyARagdoll();
  if ((bEnemyIsNPC && !pVictim->IsAlive()) ||
      (tr.fraction < 1.0 && tr.m_pEnt != pVictim && tr.m_pEnt != m_hRagdoll)) {
  ...
  if (m_hRagdoll) {
    ...
    Vector centerDelta =
        m_hRagdoll->WorldSpaceCenter() - GetEnemy()->WorldSpaceCenter();
...
//-----------------------------------------------------------------------------
// Purpose: Attach a serverside ragdoll prop for the specified entity to our
// tongue
//-----------------------------------------------------------------------------
CRagdollProp *CNPC_Barnacle::AttachRagdollToTongue(CBaseAnimating *pAnimating) {
  ...
  // HACK: Until we have correctly assigned hitgroups on our models, lookup the
  // bones
  ...
  // NDebugOverlay::Box( vecBonePos, -Vector(5,5,5), Vector(5,5,5), 255,255,255,
  // 0, 10.0 );

  // Create the ragdoll attached to tongue
  IPhysicsObject *pTonguePhysObject = m_hTongueTip->VPhysicsGetObject();
  CRagdollProp *pRagdoll = CreateServerRagdollAttached(
      pAnimating, vec3_origin, -1, COLLISION_GROUP_NONE, pTonguePhysObject,
      m_hTongueTip, 0, vecBonePos, m_iGrabbedBoneIndex, vec3_origin);
  if (pRagdoll) {
#if HL2_EPISODIC
...
void CNPC_Barnacle::InputDropTongue(inputdata_t &inputdata) { DropTongue(); }

//-----------------------------------------------------------------------------
// Purpose: Grab the specified target with our tongue
//-----------------------------------------------------------------------------
void CNPC_Barnacle::AttachTongueToTarget(CBaseEntity *pTouchEnt,
                                         Vector vecGrabPos) {
#if HL2_EPISODIC
  m_OnGrab.Set(pTouchEnt, this, this);
#endif

  // Reset this valricue each time we attach prey. If it needs to be reduced,
  // code below will do so.
  m_flBarnaclePullSpeed = BARNACLE_PULL_SPEED;
  ...
  if (pTouchEnt->IsPlayer()) {
    ...
    if (pPlayer->IsInAVehicle()) {
      ...
      // The player could have warped through the tongue while on a high-speed
      // vehicle. Move him back under the barnacle.
      Vector vecDelta;
  ...
  // teleporting the player in this way is illegitimate -- try it in third
  // person to see the problem
  if (/* pTouchEnt->IsPlayer() || */ pTouchEnt->MyNPCPointer()) {
    Vector origin = GetAbsOrigin();
    origin.z = pTouchEnt->GetAbsOrigin().z;

    CTraceFilterSkipTwoEntities traceFilter(this, pTouchEnt,
                                            COLLISION_GROUP_NONE);
    trace_t placementTrace;
    UTIL_TraceHull(origin, origin, pTouchEnt->WorldAlignMins(),
                   pTouchEnt->WorldAlignMaxs(), MASK_NPCSOLID, &traceFilter,
                   &placementTrace);
    if (placementTrace.startsolid) {
      UTIL_TraceHull(origin + Vector(0, 0, 24), origin,
                     pTouchEnt->WorldAlignMins(), pTouchEnt->WorldAlignMaxs(),
                     MASK_NPCSOLID, &traceFilter, &placementTrace);
      if (!placementTrace.startsolid) {
  ...
  if (IsEnemyAPlayer() || IsEnemyAPhysicsObject()) {
    // The player (and phys objects) doesn't ragdoll, so just grab him and pull
    // him up manually
    ...
    if (pTouchEnt->IsPlayer()) {
...
#if BARNACLE_USE_TONGUE_OFFSET
      VectorRotate(m_svPlayerHeldTipOffset, pTouchEnt->EntityToWorldTransform(),
                   m_vecTipDrawOffset.GetForModify());
      ...
      vecGrabPos = physcollision->CollideGetExtent(
          pPlayerPhys->GetCollide(), pTouchEnt->GetAbsOrigin(),
          pTouchEnt->GetAbsAngles(), vecGrabPos);
#if BARNACLE_USE_TONGUE_OFFSET
    ...
    /*
You can use this stanza to try to counterplace the constraint on the player's
head so he gets hauled sideways to the right place on the barnacle, but it is
better to just move the tongue before attachment. if ( IsEnemyAPlayer() )
    {
            Vector2D vToCenter = GetAbsOrigin().AsVector2D() -
pTouchEnt->EyePosition().AsVector2D(); fixed.attachedRefXform[0][3] -=
vToCenter.x ; fixed.attachedRefXform[1][3] -= vToCenter.y ;
    }
    */
    ...
    m_hTongueTip->m_pSpring->SetSpringConstant(
        BARNACLE_TONGUE_SPRING_CONSTANT_LIFTING);
    UpdateTongue();
  ...
  // barnacle might let go if ragdoll is separated - so increase the separation
  // checking a bit
...
void CNPC_Barnacle::BitePrey(void) {
  ...
  if (pVictim == NULL) {
    if (GetEnemy()) {
      ...
      if (pBounce) {
        // Stop the ragdoll moving and start to pull the sucker up into our
        // mouth
        ...
        pTonguePhys->UpdateShadow(m_hTongueTip->GetAbsOrigin(),
                                  m_hTongueTip->GetAbsAngles(), false, 0);
  ...
  pVictim->TakeDamage(
      CTakeDamageInfo(this, this, nDamage, iDamageType | DMG_CRUSH));
  ...
  if (enemyClass == CLASS_ZOMBIE) {
    ...
    Vector vecBloodPos;
    CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.0f),
                                            &vecBloodPos);
    UTIL_BloodSpray(vecBloodPos, Vector(0, 0, -1), GetEnemy()->BloodColor(), 8,
                    FX_BLOODSPRAY_ALL);
  ...
  // in episodic, where barnacles can eat antlions, vanish the ragdoll because
  // the gibs will spray everywhere and hide it.
  if (enemyClass == CLASS_ANTLION) {
    ...
    if (m_bSwallowingPoison) {  // hurt me
      TakeDamage(CTakeDamageInfo(this, this, m_iHealth, DMG_ACID));
    }
  ...
  PhysDisableEntityCollisions(m_hRagdoll->VPhysicsGetObject(),
                              g_PhysWorldObject);
  ...
  if (!npc_barnacle_swallow.GetBool()) return;

  // Because the victim is dead, remember the blood color
  m_flNextBloodTime = 0.0f;

  // NOTE: This was too confusing to people with the more recognizable blood --
  // jdw
#ifndef _XBOX
  m_nBloodColor = pVictim->BloodColor();
#endif
...
void CNPC_Barnacle::SprayBlood() {
  if (gpGlobals->curtime < m_flNextBloodTime) return;
...
#ifndef _XBOX
  UTIL_BloodSpray(
      m_vecBloodPos + jitterPos, Vector(0, 0, -1), m_nBloodColor,
      RandomInt(4, 8),
      RandomInt(0, 2) == 0 ? FX_BLOODSPRAY_ALL : FX_BLOODSPRAY_CLOUD);
#else
  UTIL_BloodSpray(
      m_vecBloodPos + jitterPos, Vector(0, 0, -1), BLOOD_COLOR_YELLOW,
      RandomInt(4, 8),
      RandomInt(0, 2) == 0 ? FX_BLOODSPRAY_ALL : FX_BLOODSPRAY_CLOUD);
#endif
}
...
void CNPC_Barnacle::SwallowPrey(void) {
  ...
  // NDebugOverlay::Box( vecSwallowPos, -Vector(5,5,5), Vector(5,5,5),
  // 255,255,255, 0, 0.1 );
...
#if HL2_EPISODIC
    // digest poisonous things for just a moment before being killed by them (it
    // looks wierd if it's instant)
...
void CNPC_Barnacle::RemoveRagdoll(bool bDestroyRagdoll) {
  ...
  if (m_hRagdoll) {
    ...
    m_hTongueTip->m_pSpring->SetSpringConstant(
        BARNACLE_TONGUE_SPRING_CONSTANT_LOWERING);
...
//-----------------------------------------------------------------------------
// Purpose: For some reason (he was killed, etc) we lost the prey we were
// dragging towards our mouth.
//-----------------------------------------------------------------------------
void CNPC_Barnacle::LostPrey(bool bRemoveRagdoll) {
  ...
  if (pEnemy) {
    ...
    CBaseCombatCharacter *pVictim = GetEnemyCombatCharacterPointer();
    if (pVictim) {
      pVictim->DispatchInteraction(g_interactionBarnacleVictimReleased, NULL,
                                   this);
      pVictim->RemoveEFlags(EFL_IS_BEING_LIFTED_BY_BARNACLE);
      ...
      if (m_hRagdoll) {
        ...
        Vector centerDelta =
            m_hRagdoll->WorldSpaceCenter() - pEnemy->WorldSpaceCenter();
  ...
  if (m_hTongueTip) {
    // Remove our tongue's shadow object, in case we just finished swallowing
    // something
...
void CNPC_Barnacle::Event_Killed(const CTakeDamageInfo &info) {
  ...
  bool spawnGibs =
      (!HasSpawnFlags(SF_BARNACLE_CHEAP_DEATH) || random->RandomInt(0, 1));
  ...
  // Puke blood
#ifdef _XBOX
  UTIL_BloodSpray(GetAbsOrigin(), Vector(0, 0, -1), BLOOD_COLOR_YELLOW, 8,
                  FX_BLOODSPRAY_ALL);
#else
  UTIL_BloodSpray(GetAbsOrigin(), Vector(0, 0, -1), BLOOD_COLOR_RED, 8,
                  FX_BLOODSPRAY_ALL);
#endif

  // Put blood on the ground if near enough
  trace_t bloodTrace;
  AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 256),
               MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &bloodTrace);
...
void CNPC_Barnacle::WaitTillDead(void) {
  ...
  trace_t tr;
  AI_TraceLine(m_vecRoot.Get(), m_vecRoot.Get() - Vector(0, 0, 256),
  ...
  if (fabs(flDist - goalAltitude) > 20.0f) {
    ...
    if (vecVelocity.LengthSqr() < 1.0f) {
      ...
      // to avoid strange looking rest conditions (when the tongue is really
      // bent from picking up a barrel, it looks strange to switch to the
      // hanging constant)
      m_hTongueTip->m_pSpring->SetSpringConstant(
          BARNACLE_TONGUE_SPRING_CONSTANT_HANGING);
...
bool CNPC_Barnacle::IsPoisonous(CBaseEntity *pVictim) {
  ...
  if (FClassnameIs(pVictim, "npc_headcrab_poison")) return true;

  if (FClassnameIs(pVictim, "npc_headcrab_black")) return true;
...
// Barnacle has custom impact damage tables, so it can take grave damage from
// sawblades.
static impactentry_t barnacleLinearTable[] = {
    {150 * 150, 5},   {250 * 250, 10},    {350 * 350, 50},
    {500 * 500, 100}, {1000 * 1000, 500},
};
...
IterationRetval_t CTongueEntitiesEnum::EnumElement(
...
class CBarnacleTongueFilter : public CTraceFilterSimple {
 ...
 public:
  CBarnacleTongueFilter(CBaseEntity *pLastEnemy, const IHandleEntity *passedict,
                        int collisionGroup)
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pServerEntity, int contentsMask) {
    if (pServerEntity == m_pLastEnemy) return true;
...
CBaseEntity *CNPC_Barnacle::TongueTouchEnt(float *pflLength) {
  ...
  // trace once to hit architecture and see if the tongue needs to change
  // position.
  CBarnacleTongueFilter tongueFilter(m_hLastSpitEnemy, this,
                                     COLLISION_GROUP_NONE);
  AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 2048), iMask,
               &tongueFilter, &tr);
  ...
  partition->EnumerateElementsInBox(PARTITION_ENGINE_SOLID_EDICTS, mins, maxs,
                                    false, &tongueEnum);
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    // Necessary for good +use interactions
    if (pTest->IsEFlagSet(EFL_IS_BEING_LIFTED_BY_BARNACLE)) continue;

    // Vehicles can drive so fast that players can warp through the barnacle
    // tongue. Therefore, we have to do a check to ensure that doesn't happen.
    if (pTest->GetServerVehicle()) {
      ...
      if (pDriver) {
        ...
        VectorMA(pDriver->GetAbsOrigin(), -0.1f, vecPrevDriverPos,
                 vecPrevDriverPos);

        Ray_t sweptDriver;
        sweptDriver.Init(vecPrevDriverPos, pDriver->GetAbsOrigin(),
                         pDriver->WorldAlignMins(), pDriver->WorldAlignMaxs());
    ...
    if (pTest->GetMoveType() == MOVETYPE_VPHYSICS) {
      ...
      if (pObject && pObject->GetMass() <= BARNACLE_TONGUE_MAX_LIFT_MASS) {
        ...
        if (pTest->GetSolidFlags() & FSOLID_TRIGGER) {
          if (UTIL_DistApprox2D(WorldSpaceCenter(), pTest->WorldSpaceCenter()) >
              16) {
            continue;
          }
        }
    ...
    if (pTest != this && IRelationType(pTest) == D_HT &&
...
BEGIN_DATADESC(CBarnacleTongueTip)
  ...
  DEFINE_FIELD(m_hBarnacle, FIELD_EHANDLE), DEFINE_PHYSPTR(m_pSpring),
...
void CBarnacleTongueTip::Spawn(void) {
  ...
  // We don't want this to be solid, because we don't want it to collide with
  // the barnacle.
...
// Purpose:
//-----------------------------------------------------------------------------
void CBarnacleTongueTip::Precache(void) { BaseClass::Precache(); }
...
bool CBarnacleTongueTip::CreateSpring(CBaseAnimating *pTongueRoot) {
  ...
  spring.naturalLength =
      (GetAbsOrigin() - pTongueRoot->GetAbsOrigin()).Length();
...
//-----------------------------------------------------------------------------
CBarnacleTongueTip *CBarnacleTongueTip::CreateTongueTip(
    CNPC_Barnacle *pBarnacle, CBaseAnimating *pTongueRoot,
    const Vector &vecOrigin, const QAngle &vecAngles) {
  CBarnacleTongueTip *pTip = (CBarnacleTongueTip *)CBaseEntity::Create(
      "npc_barnacle_tongue_tip", vecOrigin, vecAngles);
  if (!pTip) return NULL;

  pTip->VPhysicsInitNormal(pTip->GetSolid(), pTip->GetSolidFlags(), false);
  if (!pTip->CreateSpring(pTongueRoot)) return NULL;
  ...
  pTipPhys->SetCallbackFlags(pTipPhys->GetCallbackFlags() &
...
//-----------------------------------------------------------------------------
CBarnacleTongueTip *CBarnacleTongueTip::CreateTongueRoot(
    const Vector &vecOrigin, const QAngle &vecAngles) {
  CBarnacleTongueTip *pTip = (CBarnacleTongueTip *)CBaseEntity::Create(
      "npc_barnacle_tongue_tip", vecOrigin, vecAngles);
  if (!pTip) return NULL;
...
// Activities
DECLARE_ACTIVITY(
    ACT_BARNACLE_SLURP)  // Pulling the tongue up with prey on the end
...
DECLARE_ACTIVITY(
    ACT_BARNACLE_TONGUE_WRAP)  // Wrapping the tongue around a target
DECLARE_ACTIVITY(ACT_BARNACLE_TASTE_SPIT)  // Yuck! Me no like that!
DECLARE_ACTIVITY(
    ACT_BARNACLE_BITE_SMALL_THINGS)  // Biting small things, like a headcrab
DECLARE_ACTIVITY(
    ACT_BARNACLE_CHEW_SMALL_THINGS)  // Chewing small things, like a headcrab
