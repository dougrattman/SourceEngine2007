...
//-----------------------------------------------------------------------------
IMotionEvent::simresult_e CRollerController::Simulate(
...
#define ROLLERMINE_SEE_VEHICLESONLY_BEYOND_IDLE \
  300  // See every other than vehicles upto this distance (i.e. old idle see
       // dist)
#define ROLLERMINE_SEE_VEHICLESONLY_BEYOND_NORMAL \
  800  // See every other than vehicles upto this distance (i.e. old normal see
       // dist)
...
#define ROLLERMINE_OPEN_THRESHOLD 256.0f
...
// sound, and the sound will not be allowed to play until the flag is then
// cleared.
...
//=========================================================
//=========================================================
class CNPC_RollerMine : public CNPCBaseInteractive<CAI_BaseNPC>,
 ...
 public:
  ...
  Class_T Classify() {
    if (!m_bTurnedOn) return CLASS_NONE;

    // About to blow up after being hacked so do damage to the player.
    if (m_bHackedByAlyx && (m_flPowerDownDetonateTime > 0.0f &&
                            m_flPowerDownDetonateTime <= gpGlobals->curtime))
      return CLASS_COMBINE;

    return (m_bHeld || m_bHackedByAlyx) ? CLASS_HACKED_ROLLERMINE
                                        : CLASS_COMBINE;
  }
 ...
 protected:
  ...
  CNetworkVar(float, m_flActiveTime);  // If later than the current time, this
                                       // will force the mine to be active
...
BEGIN_DATADESC(CNPC_RollerMine)
  ...
  DEFINE_SOUNDPATCH(m_pRollSound), DEFINE_SOUNDPATCH(m_pPingSound),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "RespondToExplodeChirp",
      ...
      // Function Pointers
      DEFINE_ENTITYFUNC(SpikeTouch), DEFINE_ENTITYFUNC(ShockTouch),
      DEFINE_ENTITYFUNC(CloseTouch), DEFINE_ENTITYFUNC(EmbedTouch),
      DEFINE_THINKFUNC(Explode), DEFINE_THINKFUNC(PreDetonate),
...
void CNPC_RollerMine::Precache(void) {
  ...
  gm_iszDropshipClassname =
      AllocPooledString("npc_combinedropship");  // For fast string compares.
#ifdef HL2_EPISODIC
...
unsigned int CNPC_RollerMine::PhysicsSolidMaskForEntity(void) const {
  if (HasSpawnFlags(SF_ROLLERMINE_PROP_COLLISION)) return MASK_SOLID;
...
void CNPC_RollerMine::Bury(trace_t *tr) {
  AI_TraceHull(GetAbsOrigin() + Vector(0, 0, 64),
               GetAbsOrigin() - Vector(0, 0, MAX_TRACE_LENGTH),
               Vector(-16, -16, -16), Vector(16, 16, 16), MASK_NPCSOLID, this,
               GetCollisionGroup(), tr);

  // NDebugOverlay::Box( tr->startpos, Vector(-16,-16,-16), Vector(16,16,16),
  // 255, 0, 0, 64, 10.0 ); NDebugOverlay::Box( tr->endpos, Vector(-16,-16,-16),
  // Vector(16,16,16), 0, 255, 0, 64, 10.0 );
...
void CNPC_RollerMine::OnRestore() {
  ...
  // If we're stuck to a vehicle over a level transition, restart our jolt
  // inputs
    ...
    if (!m_bHeld && vecVelocity.Length() > 64.0) {
      if (m_bHackedByAlyx) {
        ...
        CSoundEnt::InsertSound(
            (SOUND_DANGER | SOUND_CONTEXT_COMBINE_ONLY |
             SOUND_CONTEXT_REACT_TO_SOURCE | SOUND_CONTEXT_DANGER_APPROACH),
            WorldSpaceCenter() + Vector(0, 0, 32) + vecVelocity * 0.5f, 120.0f,
            0.2f, this, SOUNDENT_CHANNEL_REPEATED_DANGER);
      } else {
        // Scare player allies
        CSoundEnt::InsertSound(
            (SOUND_DANGER | SOUND_CONTEXT_EXCLUDE_COMBINE |
             SOUND_CONTEXT_REACT_TO_SOURCE | SOUND_CONTEXT_DANGER_APPROACH),
            WorldSpaceCenter() + Vector(0, 0, 32) + vecVelocity * 0.5f, 120.0f,
            0.2f, this, SOUNDENT_CHANNEL_REPEATED_DANGER);
      }
...
int CNPC_RollerMine::RangeAttack1Conditions(float flDot, float flDist) {
  if (HasCondition(COND_SEE_ENEMY) == false) return COND_NONE;

  if (EnemyInVehicle()) return COND_CAN_RANGE_ATTACK1;

  if (flDist > ROLLERMINE_MAX_ATTACK_DIST) return COND_TOO_FAR_TO_ATTACK;

  if (flDist < ROLLERMINE_MIN_ATTACK_DIST) return COND_TOO_CLOSE_TO_ATTACK;
...
int CNPC_RollerMine::SelectSchedule(void) {
  if (m_bPowerDown) return SCHED_ROLLERMINE_POWERDOWN;
...
int CNPC_RollerMine::GetHackedIdleSchedule(void) {
  ...
  if (!pPlayer) return SCHED_NONE;

  if (!HasCondition(COND_SEE_PLAYER)) return SCHED_ROLLERMINE_PATH_TO_PLAYER;

  if (GetAbsOrigin().DistToSqr(pPlayer->GetAbsOrigin()) >
      ROLLERMINE_RETURN_TO_PLAYER_DIST)
...
int CNPC_RollerMine::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    case SCHED_IDLE_STAND: {
      ...
      if (iSched != SCHED_NONE) return iSched;
    ...
    case SCHED_ROLLERMINE_RANGE_ATTACK1:
      if (HasCondition(COND_ENEMY_OCCLUDED)) {
        // Because of an unfortunate arrangement of cascading failing schedules,
        // the rollermine could end up here with instructions to drive towards
        // the target, although the target is not in sight. Nudge around
        // randomly until we're back on the nodegraph.
...
bool CNPC_RollerMine::QuerySeeEntity(CBaseEntity *pSightEnt,
  ...
  if (IRelationType(pSightEnt) == D_FR) {
    ...
    float flDist =
        (WorldSpaceCenter() - pSightEnt->WorldSpaceCenter()).LengthSqr();
    if (flDist > ROLLERMINE_FEAR_DISTANCE) return false;
  }
...
void CNPC_RollerMine::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_FACE_IDEAL:
      // This only applies to NPCs that aren't spheres with omnidirectional
      // eyesight.
      TaskComplete();
      break;
...

    {
      ...
      trace_t tr;
      AI_TraceLine(GetAbsOrigin() + Vector(0, 0, 1),
                   GetAbsOrigin() - Vector(0, 0, 64), MASK_SOLID_BRUSHONLY,
    ...
    case TASK_WAIT_FOR_MOVEMENT: {
      // TASK_RUN_PATH and TASK_WALK_PATH work different on the rollermine and
      // run until movement is done, so movement is already complete when
      // entering this task.
      TaskComplete();
    } break;
    ...
    case TASK_ROLLERMINE_GET_PATH_TO_FLEE: {
      ...
      for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
        ...
        if (IRelationType(pEnemy) != D_FR) continue;

        float flDist =
            (WorldSpaceCenter() - pEnemy->WorldSpaceCenter()).LengthSqr();
        if (flDist < flNearest) {
          flNearest = flDist;
    ...
    case TASK_ROLLERMINE_NUDGE_TOWARDS_NODES: {
      ...
      if (pPhysicsObject) {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          UTIL_TraceLine(WorldSpaceCenter(),
                         WorldSpaceCenter() + vecNudge * 24.0f, MASK_SHOT, this,
...
void CNPC_RollerMine::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_ROLLERMINE_UNBURROW: {
      ...
      // Robin: HACK: Bloat the rollermine check to catch the model switch
      // (roller.mdl->roller_spikes.mdl)
      trace_t tr;
      AI_TraceHull(vCenter, vCenter, Vector(-16, -16, -16), Vector(16, 16, 16),
    ...
    case TASK_WALK_PATH:
      ...
      if (IsCurSchedule(SCHED_ROLLERMINE_FLEE) &&
      ...
      // Start turning early
      if ((GetLocalOrigin() - GetNavigator()->GetCurWaypointPos()).Length() <=
          64) {
        if (GetNavigator()->CurWaypointIsGoal()) {
          // Hit the brakes a bit.
          float yaw = UTIL_VecToYaw(GetNavigator()->GetCurWaypointPos() -
                                    GetLocalOrigin());
          Vector vecRight;
          AngleVectors(QAngle(0, yaw, 0), NULL, &vecRight, NULL);

          m_RollerController.m_vecAngular +=
              WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()),
                                   vecRight, -m_flForwardSpeed * 5);
...

      {
        ...
        // figure out if the roller is turning. If so, cut the throttle a
        // little.
        float flDot;
        Vector vecVelocity;
        ...
        if (flDot > 0.25 && flDot < 0.7) {
          // Feed a little torque backwards into the axis perpendicular to the
          // velocity. This will help get rid of momentum that would otherwise
          // make us overshoot our goal.
          ...
          m_RollerController.m_vecAngular =
              WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()),
                                   vecCompensate, m_flForwardSpeed * -0.75);
        }
        ...
        if (m_bHackedByAlyx) {
          ...
          m_RollerController.m_vecAngular +=
              WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()),
                                   vecRight, m_flForwardSpeed * 2.0f);
        } else {
          m_RollerController.m_vecAngular += WorldToLocalRotation(
              SetupMatrixAngles(GetLocalAngles()), vecRight, m_flForwardSpeed);
        }
      }
      break;
    ...
    case TASK_ROLLERMINE_CHARGE_ENEMY: {
      ...
      if (EnemyInVehicle()) {
        ...
        // Project it's velocity and find our closest point on that line. Do it
        // all in 2d space.
        Vector vecVehicleVelocity =
            pCCEnemy->GetVehicleEntity()->GetSmoothedVelocity();
        ...
        CalcClosestPointOnLine2D(vecOrigin2D, vecTargetPosition2D,
                                 vecProjected2D, vecIntercept2D, &flT);
        Vector vecIntercept(vecIntercept2D.x, vecIntercept2D.y,
                            GetAbsOrigin().z);

        // NDebugOverlay::Line( vecTargetPosition, vecProjected, 0,255,0, true,
        // 0.1 );

        // If we're ahead of the line somewhere, try to intercept
        if (flT > 0) {
          // If it's beyond the end of the intercept line, just move towards the
          // end of the line
          if (flT > 1) {
          ...
          // If we're closer to the intercept point than to the vehicle, move
          // towards the intercept
          if ((GetAbsOrigin() - vecTargetPosition).LengthSqr() >
              (GetAbsOrigin() - vecIntercept).LengthSqr()) {
            // NDebugOverlay::Box( vecIntercept, -Vector(20,20,20),
            // Vector(20,20,20), 255,0,0, 0.1, 0.1 );
        ...
        // NDebugOverlay::Box( vecTargetPosition, -Vector(20,20,20),
        // Vector(20,20,20), 255,255,255, 0.1, 0.1 );
      }
      ...
      // NDebugOverlay::Line( GetLocalOrigin(), GetLocalOrigin() +
      // (GetEnemy()->GetLocalOrigin() - GetLocalOrigin()), 0,255,0, true, 0.1
      // );
      ...
      m_RollerController.m_vecAngular =
          WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()),
                               vecCompensate, m_flForwardSpeed * -0.75);
      m_RollerController.m_vecAngular +=
          WorldToLocalRotation(SetupMatrixAngles(GetLocalAngles()), vecRight,
                               m_flForwardSpeed * flTorqueFactor);

      // Taunt when I get closer
      if (!(m_iSoundEventFlags & ROLLERMINE_SE_TAUNT) &&
          UTIL_DistApprox(GetLocalOrigin(), vecTargetPosition) <= 400) {
      ...
      if ((m_bIsOpen == false) &&
        ...
        if (flDistance >= flThreshold) {
          ...
          Close();
        } else if (EnemyInVehicle() &&
                   flDistance < ROLLERMINE_VEHICLE_HOP_THRESHOLD) {
          // Keep trying to hop when we're ramming a vehicle, so we're visible
          // to the player
          if (vecVelocity.x != 0 && vecVelocity.y != 0 && flTorqueFactor > 3 &&
              flDot > 0.0) {
            Hop(300);
          }
        }
      ...
      if (!EnemyInVehicle()) {
        if (vecVelocity.x != 0 && vecVelocity.y != 0) {
          if (gpGlobals->curtime - m_flChargeTime > 1.0 && flTorqueFactor > 1 &&
              flDot < 0.0) {
            if (m_bIsOpen) {
              Close();
            }
    ...
    case TASK_ROLLERMINE_RETURN_TO_PLAYER: {
      ...
      if (GetAbsOrigin().DistToSqr(vecTargetPosition) <
          (ROLLERMINE_RETURN_TO_PLAYER_DIST * 2.0)) {
        TaskComplete();
      }
    } break;
    ...
    case TASK_ROLLERMINE_POWERDOWN: {
      ...
      // No TaskComplete() here, because the task will never complete. The
      // rollermine will explode.
    } break;
    ...
    if (!m_pConstraint) {
      if (EnemyInVehicle()) {
        Hop(256);
                 ...
                 GetEnemy()->Classify() !=
                     CLASS_BULLSEYE)  // Don't hop when attacking bullseyes
      {
        Hop(128);
      }
    }
...
void CNPC_RollerMine::SpikeTouch(CBaseEntity *pOther) {
  ...
  // If we just hit a breakable glass object, don't explode. We want to blow
  through it. CBreakable *pBreakable = dynamic_cast<CBreakable*>(pOther); if (
  pBreakable && pBreakable->GetMaterialType() == matGlass ) return;
...
void CNPC_RollerMine::CloseTouch(CBaseEntity *pOther) {
  ...
  bool bOtherIsDead =
      (pOther->MyNPCPointer() && !pOther->MyNPCPointer()->IsAlive());
...
float CNPC_RollerMine::GetAttackDamageScale(CBaseEntity *pVictim) {
  // If we're friendly, don't damage players or player-friendly NPCs, even with
  // collisions
  if (HasSpawnFlags(SF_ROLLERMINE_FRIENDLY)) {
...
void CNPC_RollerMine::ShockTouch(CBaseEntity *pOther) {
  ...
  CTakeDamageInfo info(this, this, vecForce, out,
                       sk_rollermine_shock.GetFloat(), DMG_SHOCK);

  if (FClassnameIs(pOther, "npc_combine_s")) {
    if (pOther->GetHealth() <= (pOther->GetMaxHealth() / 2)) {
      // Instant special death for a combine soldier who has less than half
      // health.
      ...
      pOther->MyCombatCharacterPointer()->BecomeRagdollBoogie(
          this, vecDamageForce, 5.0f, SF_RAGDOLL_BOOGIE_ELECTRICAL);
      return;
    } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_RollerMine::VPhysicsCollision(int index,
                                        gamevcollisionevent_t *pEvent) {
  // Make sure we don't keep hitting the same entity
  int otherIndex = !index;
  CBaseEntity *pOther = pEvent->pEntities[otherIndex];
  if (pEvent->deltaCollisionTime < 0.5 && (pOther == this)) return;
  ...
  if (m_bHeld || m_hVehicleStuckTo || !IsPlayerVehicle(pOther)) {
    ...
    if (m_bEmbedOnGroundImpact) {
      ...
      g_PostSimulationQueue.QueueCall(this, &CNPC_RollerMine::EmbedTouch,
                                      pOther);
    }
    return;
  }
...
void CNPC_RollerMine::StickToVehicle(CBaseEntity *pOther) {
  ...
  m_pConstraint =
      physenv->CreateFixedConstraint(pOtherPhysics, pPhysics, NULL, fixed);
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPC_RollerMine::CountRollersOnMyVehicle(
    CUtlVector<CNPC_RollerMine *> *pRollerList) {
  CBaseEntity *entityList[64];
...
void CNPC_RollerMine::AnnounceArrivalToOthers(CBaseEntity *pOther) {
  ...
  // Stop all rollers on the vehicle falling off due to the force of the
  // arriving one
  ...
  if ( iRollers >= ROLLERMINE_REQUIRED_TO_EXPLODE_VEHICLE )
  {
          ...
          for ( int i = 0; i < iRollers; i++ )
          {
                  ...
                  g_EventQueue.AddEvent( aRollersOnVehicle[i],
  "RespondToExplodeChirp", RandomFloat(2,5), NULL, NULL );
          }
  }
  else
  {
  */
  ...
  if (iRollers > 1) {
    ...
    for (int i = 1; i < iRollers; i++) {
      variant_t emptyVariant;
      g_EventQueue.AddEvent(aRollersOnVehicle[i], "RespondToChirp",
                            RandomFloat(2, 3), NULL, NULL);
    }
  }
  //	}
}
...
void CNPC_RollerMine::InputJoltVehicle(inputdata_t &inputdata) {
  ...
  Vector vecForce =
      impulse * ImpulseScale(pVehiclePhysics->GetMass(), RandomFloat(150, 250));
  ...
  if ( pBeam )
  {
          ...
          CBaseAnimating *pAnimating = dynamic_cast<CBaseAnimating *>(
  GetVehicleStuckTo() ); if ( pAnimating )
          {
...
// Input  : *pPhysGunUser -
//-----------------------------------------------------------------------------
void CNPC_RollerMine::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
int CNPC_RollerMine::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & (DMG_BURN | DMG_BLAST)) {
    if (info.GetAttacker() &&
        info.GetAttacker()->m_iClassname != m_iClassname) {
      SetThink(&CNPC_RollerMine::PreDetonate);
...
void CNPC_RollerMine::Hop(float height) {
  if (m_flNextHop > gpGlobals->curtime) return;
...
void CNPC_RollerMine::Explode(void) {
  ...
  if (UTIL_PointContents(GetAbsOrigin()) & MASK_WATER) {
    ...
    data.m_flScale = 128;
    data.m_fFlags = (SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
                     SF_ENVEXPLOSION_NOSMOKE);
    DispatchEffect("WaterSurfaceExplosion", data);
  } else {
    ExplosionCreate(WorldSpaceCenter(), GetLocalAngles(), this, expDamage, 128,
                    true);
  }
  ...
  // Remove myself a frame from now to avoid doing it in the middle of running
  // AI
...
float CNPC_RollerMine::RollingSpeed() {
  ...
  if (!m_hVehicleStuckTo && !m_bHeld && pPhysics && !pPhysics->IsAsleep()) {
    ...
    rollingSpeed = std::clamp(rollingSpeed, 1.0f, MAX_ROLLING_SPEED);
...
//-----------------------------------------------------------------------------
// Purpose: We've been dropped by a dropship. Embed in the ground if we land on
// it.
//-----------------------------------------------------------------------------
...
void CNPC_RollerMine::UpdateRollingSound() {
  if (m_rollingSoundState == ROLL_SOUND_NOT_READY) return;
  ...
  switch (soundState) {
    ...
    case ROLL_SOUND_OPEN:
      CBaseEntity::GetParametersForSound("NPC_RollerMine.RollWithSpikes",
                                         params, NULL);
      break;
  ...
  if (m_rollingSoundState != soundState) {
    ...
    m_pRollSound = controller.SoundCreate(filter, entindex(), params.channel,
                                          params.soundname, params.soundlevel);
  ...
  if (m_pRollSound) {
    // for tuning
    // DevMsg("SOUND: %s, VOL: %.1f\n", m_rollingSoundState == ROLL_SOUND_CLOSED
    // ? "CLOSED" : "OPEN ", rollingSpeed );
    controller.SoundChangePitch(
        m_pRollSound,
        params.pitchlow + (params.pitchhigh - params.pitchlow) * rollingSpeed,
        0.1);
    controller.SoundChangeVolume(m_pRollSound, params.volume * rollingSpeed,
                                 0.1);
  }
}
...
void CNPC_RollerMine::UpdatePingSound() {
  ...
  if (m_bIsOpen && !IsShocking() && !m_bHeld) {
    ...
    if (pEnemy) {
      ...
      pingSpeed = std::clamp(pingSpeed, 1.0f, ROLLERMINE_OPEN_THRESHOLD);
  ...
  if (pingSpeed > 0) {
    ...
    if (!m_pPingSound) {
      CPASAttenuationFilter filter(this);
      m_pPingSound =
          controller.SoundCreate(filter, entindex(), params.channel,
                                 params.soundname, params.soundlevel);
    ...
    controller.SoundChangePitch(
        m_pPingSound,
        params.pitchlow + (params.pitchhigh - params.pitchlow) * pingSpeed,
        0.1);
...
bool CNPC_RollerMine::IsValidEnemy(CBaseEntity *pEnemy) {
  // If the enemy's over the vehicle detection range, and it's not a player in a
  // vehicle, ignore it
  if (pEnemy) {
    ...
    if (flDistance >= m_flSeeVehiclesOnlyBeyond) {
      ...
      if (pCCEnemy != NULL && pCCEnemy->IsInAVehicle()) {
        // If we're buried, we only care when they're heading directly towards
        // us
        if (m_bBuried)
          return (VehicleHeading(pCCEnemy->GetVehicle()->GetVehicleEnt()) >
                  DOT_20DEGREE);

        // If we're not buried, chase him as long as he's not heading away from
        // us
...
float CNPC_RollerMine::VehicleHeading(CBaseEntity *pVehicle) {
  ...
  // This will enable already active rollers to chase the vehicle if it's
  // stationary.
...
//			*ptr -
//-----------------------------------------------------------------------------
void CNPC_RollerMine::TraceAttack(const CTakeDamageInfo &info,
...
DEFINE_SCHEDULE(SCHED_ROLLERMINE_RANGE_ATTACK1,
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_ROLLERMINE_RANGE_ATTACK1"
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_ROLLERMINE_ALERT_STAND"
