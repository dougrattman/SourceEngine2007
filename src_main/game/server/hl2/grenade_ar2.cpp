...
ConVar g_CV_SmokeTrail("smoke_trail", "1",
                       0);  // temporary dust explosion switch
...
void CGrenadeAR2::Spawn(void) {
  ...
  SetGravity(UTIL_ScaleForGravity(
      400));  // use a lower gravity for grenades to make them easier to see
  SetFriction(0.8);
  SetSequence(0);
...
//			 person firing it can bounce it off a nearby wall.
//However if it 			 hits another character it blows up
// immediately
// Input  :
// Output :
//-----------------------------------------------------------------------------
void CGrenadeAR2::GrenadeAR2Think(void) {
  ...
  if (m_bIsLive) {
    if (GetAbsVelocity().Length() == 0.0 || GetGroundEntity() != NULL) {
      Detonate();
    }
  }

  // The old way of making danger sounds would scare the crap out of EVERYONE
  // between you and where the grenade was going to hit. The radius of the
  // danger sound now 'blossoms' over the grenade's lifetime, making it seem
  ...
  CSoundEnt::InsertSound(SOUND_DANGER, GetAbsOrigin() + GetAbsVelocity() * 0.5,
                         m_fDangerRadius, 0.2, this,
