...
#define JEEP_SEAGULL_THINK_INTERVAL \
  10.0  // Interval between checks for seagull perches
#define JEEP_SEAGULL_POOP_INTERVAL \
  45.0  // Interval between checks for seagull poopage
#define JEEP_SEAGULL_HIDDEN_TIME \
  15.0  // Time for which the player must be hidden from the jeep for a seagull
        // to perch
#define JEEP_SEAGULL_MAX_TIME \
  60.0  // Time at which a seagull will definately perch on the jeep
...
BEGIN_DATADESC(CPropJeep)
  DEFINE_FIELD(m_bGunHasBeenCutOff, FIELD_BOOLEAN),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StartRemoveTauCannon",
                       InputStartRemoveTauCannon),
      DEFINE_INPUTFUNC(FIELD_VOID, "FinishRemoveTauCannon",
...
SendPropBool(SENDINFO(m_bHeadlightIsOn)), END_SEND_TABLE();
...
void CPropJeep::Activate() {
  ...
  if (pServerVehicle) {
    if (pServerVehicle->GetPassenger()) {
      // If a jeep comes back from a save game with a driver, make sure the
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeep::TraceAttack(const CTakeDamageInfo &inputInfo,
...
float CPropJeep::PassengerDamageModifier(const CTakeDamageInfo &info) {
  if (info.GetInflictor() &&
      FClassnameIs(info.GetInflictor(), "hunter_flechette"))
    return 0.1f;
...
int CPropJeep::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  // HACKHACK: Scale up grenades until we get a better explosion/pressure damage
  // system
  ...
  // small amounts of shock damage disrupt the car, but aren't transferred to
  // the player
  ...
  if (GetDriver()) {
    ...
    // Scale the damage and mark that we're passing it in so the base player
    // accepts the damage
...
void CPropJeep::AimGunAt(Vector *endPos, float flInterval) {
  ...
  float newTargetYaw =
      std::clamp(targetYaw, -CANNON_MAX_LEFT_YAW, CANNON_MAX_RIGHT_YAW);
  float newTargetPitch =
      std::clamp(targetPitch, -CANNON_MAX_DOWN_PITCH, CANNON_MAX_UP_PITCH);

  // If the angles have been clamped, we're looking outside of our valid range
  if (fabs(newTargetYaw - targetYaw) > 1e-4 ||
      fabs(newTargetPitch - targetPitch) > 1e-4) {
    m_bUnableToFire = true;
  }
  ...
  trace_t tr;
  UTIL_TraceLine(vecMuzzle, vecMuzzle + (vecMuzzleDir * MAX_TRACE_LENGTH),
                 MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);

  // see if we hit something, if so, adjust endPos to hit location
  if (tr.fraction < 1.0) {
    m_vecGunCrosshair =
        vecMuzzle + (vecMuzzleDir * MAX_TRACE_LENGTH * tr.fraction);
  }
}
...
void CPropJeep::HandleWater(void) {
  ...
  if (CheckWater()) {
    for (int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel) {
      ...
      if (m_WaterData.m_bWheelInWater[iWheel] !=
          m_WaterData.m_bWheelWasInWater[iWheel]) {
      ...
      if (m_WaterData.m_bWheelInWater[iWheel] &&
        ...
        if (m_WaterData.m_flNextRippleTime[iWheel] < gpGlobals->curtime) {
          ...
          m_WaterData.m_flNextRippleTime[iWheel] =
              gpGlobals->curtime + RandomFloat(0.1, 0.3);
          CreateRipple(m_WaterData.m_vecWheelContactPoints[iWheel]);
        }
      }
    }
  ...
  for (int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel) {
    m_WaterData.m_bWheelWasInWater[iWheel] =
        m_WaterData.m_bWheelInWater[iWheel];
  }
}
...
bool CPropJeep::CheckWater(void) {
  ...
  for (int iWheel = 0; iWheel < JEEP_WHEEL_COUNT; ++iWheel) {
    ...
    if (pWheel->GetContactPoint(&m_WaterData.m_vecWheelContactPoints[iWheel],
                                NULL)) {
      m_WaterData.m_bWheelInWater[iWheel] =
          (UTIL_PointContents(m_WaterData.m_vecWheelContactPoints[iWheel]) &
           MASK_WATER)
              ? true
              : false;
      if (m_WaterData.m_bWheelInWater[iWheel]) {
  ...
  m_WaterData.m_bBodyInWater =
      (UTIL_PointContents(vecEnginePoint) & MASK_WATER) ? true : false;
...
void CPropJeep::CheckWaterLevel(void) {
  ...
  if (pEntity && pEntity->IsPlayer()) {
    ...
    bool bEyes =
        (UTIL_PointContents(vecAttachPoint) & MASK_WATER) ? true : false;
    if (bEyes) {
...
void CPropJeep::Think(void) {
  ...
  if (m_bHasGun && m_hPlayer && !m_bExitAnimOn && !m_bEnterAnimOn) {
    ...
    trace_t trace;
    UTIL_TraceLine(vecEyePos, vecEndPos, MASK_SHOT, this, COLLISION_GROUP_NONE,
                   &trace);
  ...
  if (IsSequenceFinished() && (m_bExitAnimOn || m_bEnterAnimOn)) {
    if (m_bEnterAnimOn) {
      ...
      // HACKHACK: This forces the jeep to play a sound when it gets entered
      // underwater
      if (m_VehiclePhysics.IsEngineDisabled()) {
        CBaseServerVehicle *pServerVehicle =
            dynamic_cast<CBaseServerVehicle *>(GetServerVehicle());
    ...
    // If we're exiting and have had the tau cannon removed, we don't want to
    // reset the animation
    if (hl2_episodic.GetBool()) {
      // Reset on exit anim
      GetServerVehicle()->HandleEntryExitFinish(m_bExitAnimOn, m_bExitAnimOn);
    } else {
      GetServerVehicle()->HandleEntryExitFinish(
          m_bExitAnimOn, !(m_bExitAnimOn && TauCannonHasBeenCutOff()));
    }
  }

  // See if the ammo crate needs to close
  if ((m_flAmmoCrateCloseTime < gpGlobals->curtime) &&
      (GetSequence() == LookupSequence("ammo_open"))) {
    m_flAnimTime = gpGlobals->curtime;
    m_flPlaybackRate = 0.0;
    SetCycle(0);
    ResetSequence(LookupSequence("ammo_close"));
  } else if ((GetSequence() == LookupSequence("ammo_close")) &&
             IsSequenceFinished()) {
    m_flAnimTime = gpGlobals->curtime;
    m_flPlaybackRate = 0.0;
    SetCycle(0);
...
//			useMuzzle -
//-----------------------------------------------------------------------------
void CPropJeep::DrawBeam(const Vector &startPos, const Vector &endPos,
                         float width) {
  // Tracer down the middle
  UTIL_Tracer(startPos, endPos, 0, TRACER_DONT_USE_ATTACHMENT, 6500, false,
              "GaussTracer");
...
void CPropJeep::FireCannon(void) {
  ...
  FireBulletsInfo_t info(1, m_vecGunOrigin, aimDir, VECTOR_CONE_1DEGREES,
  ...
  m_nSpinPos += JEEP_GUN_SPIN_RATE;
  // SetPoseParameter( JEEP_GUN_SPIN, m_nSpinPos );	//TODO(d.rattman): Don't bother
  // with this for E3, won't look right
}
...
void CPropJeep::FireChargedCannon(void) {
  ...
  trace_t tr;
  UTIL_TraceLine(m_vecGunOrigin, endPos, MASK_SHOT, this, COLLISION_GROUP_NONE,
                 &tr);
  ...
  float flChargeAmount =
      (gpGlobals->curtime - m_flCannonChargeStartTime) / MAX_GAUSS_CHARGE_TIME;
  ...
  if (tr.DidHitWorld() && !(tr.surface.flags & SURF_SKY)) {
    ...
    UTIL_TraceLine(testPos, tr.endpos, MASK_SHOT, GetDriver(),
    ...
    CalculateBulletDamageForce(&dmgInfo, GetAmmoDef()->Index("GaussEnergy"),
                               aimDir, tr.endpos, 1.0f + flChargeAmount * 4.0f);
  ...
  if (penetrated == true) {
    RadiusDamage(CTakeDamageInfo(this, this, flDamage, DMG_SHOCK), tr.endpos,
                 200.0f, CLASS_NONE, NULL);
  }
}
...
void CPropJeep::ChargeCannon(void) {
  ...
  if (m_bCannonCharging == false) {
    ...
    m_sndCannonCharge = (CSoundEnvelopeController::GetController())
                            .SoundCreate(filter, entindex(), CHAN_STATIC,
                                         "Jeep.GaussCharge", ATTN_NORM);

    if (m_hPlayer) {
      m_hPlayer->RumbleEffect(
          RUMBLE_FLAT_LEFT, (int)(0.1 * 100),
          RUMBLE_FLAG_RESTART | RUMBLE_FLAG_LOOP | RUMBLE_FLAG_INITIAL_SCALE);
    }

    assert(m_sndCannonCharge != NULL);
    if (m_sndCannonCharge != NULL) {
      (CSoundEnvelopeController::GetController())
          .Play(m_sndCannonCharge, 1.0f, 50);
      (CSoundEnvelopeController::GetController())
          .SoundChangePitch(m_sndCannonCharge, 250, 3.0f);
    }

    return;
  } else {
    float flChargeAmount = (gpGlobals->curtime - m_flCannonChargeStartTime) /
                           MAX_GAUSS_CHARGE_TIME;
    if (flChargeAmount > 1.0f) {
      flChargeAmount = 1.0f;
    }
    ...
    if (m_hPlayer) {
      m_hPlayer->RumbleEffect(RUMBLE_FLAT_LEFT, (int)(rumble * 100),
  ...
  // TODO: Check for overcharge and have the weapon simply fire or instead
  // "decharge"?
}
...
void CPropJeep::StopChargeSound(void) {
  if (m_sndCannonCharge != NULL) {
    (CSoundEnvelopeController::GetController())
        .SoundFadeOut(m_sndCannonCharge, 0.1f);
  }
...
//-----------------------------------------------------------------------------
// Purpose: If the player uses the jeep while at the back, he gets ammo from the
// crate instead
//-----------------------------------------------------------------------------
void CPropJeep::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  UTIL_TraceLine(vecStart, vecStart + vecForward * 1024,
                 MASK_SOLID | CONTENTS_DEBRIS | CONTENTS_HITBOX, pPlayer,
                 COLLISION_GROUP_NONE, &tr);
  ...
  if (tr.m_pEnt == this && tr.hitgroup == JEEP_AMMOCRATE_HITGROUP) {
    ...
    if ((GetSequence() != LookupSequence("ammo_open")) &&
        (GetSequence() != LookupSequence("ammo_close"))) {
      // Open the crate
      m_flAnimTime = gpGlobals->curtime;
      m_flPlaybackRate = 0.0;
      SetCycle(0);
...
bool CPropJeep::CanExitVehicle(CBaseEntity *pEntity) {
  return (!m_bEnterAnimOn && !m_bExitAnimOn && !m_bLocked &&
          (m_nSpeed <= g_jeepexitspeed.GetFloat()));
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeep::DampenEyePosition(Vector &vecVehicleEyePos,
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeep::DampenForwardMotion(Vector &vecVehicleEyePos,
                                    QAngle &vecVehicleEyeAngles,
                                    float flFrameTime) {
  // Get forward vector.
  Vector vecForward;
  ...
  ComputePDControllerCoefficients(flCoefficients,
                                  r_JeepViewDampenFreq.GetFloat(),
                                  r_JeepViewDampenDamp.GetFloat(), flFrameTime);
  m_vecEyeSpeed +=
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeep::DampenUpMotion(Vector &vecVehicleEyePos,
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeep::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
                          IMoveHelper *pHelper, CMoveData *move) {
  // If we are overturned and hit any key - leave the vehicle (IN_USE is already
  // handled!).
  if (m_flOverturnedTime > OVERTURNED_EXIT_WAITTIME) {
    if ((ucmd->buttons & (IN_FORWARD | IN_BACK | IN_MOVELEFT | IN_MOVERIGHT |
                          IN_SPEED | IN_JUMP | IN_ATTACK | IN_ATTACK2)) &&
        !m_bExitAnimOn) {
      // Can't exit yet? We're probably still moving. Swallow the keys.
      if (!CanExitVehicle(player)) return;

      if (!GetServerVehicle()->HandlePassengerExit(m_hPlayer) &&
          (m_hPlayer != NULL)) {
  ...
  // If the throttle is disabled or we're upside-down, don't allow throttling
  // (including turbo)
  CUserCmd tmp;
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropJeep::DriveVehicle(float flFrameTime, CUserCmd *ucmd,
...
void CPropJeep::CreateDangerSounds(void) {
  ...
  if (fabs(speed) > 120) {
    ...
    CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_PLAYER_VEHICLE, vecSpot,
                           radius, soundDuration, this, 0);
    CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER | SOUND_CONTEXT_PLAYER_VEHICLE,
                           vecSpot, radius, soundDuration, this, 1);
  ...
  CSoundEnt::InsertSound(SOUND_PLAYER | SOUND_CONTEXT_PLAYER_VEHICLE,
                         GetAbsOrigin(), 800, soundDuration, this, 0);
...
void CPropJeep::ExitVehicle(int nRole) {
  ...
  if (GlobalEntity_GetState("no_seagulls_on_jeep") == GLOBAL_OFF) {
    ...
    if (pHint) {
      ...
      SetContextThink(&CPropJeep::JeepSeagullThink,
                      gpGlobals->curtime + JEEP_SEAGULL_THINK_INTERVAL,
                      g_pJeepThinkContext);
    }
  }
}
...
void CPropJeep::JeepSeagullThink(void) {
  if (!m_hLastPlayerInVehicle) return;
  ...
  if (m_hSeagull) {
    ...
    if (pSeagull) {
      ...
      if (pSeagull->m_bOnJeep == true) {
        ...
        SetContextThink(&CPropJeep::JeepSeagullThink,
                        gpGlobals->curtime + JEEP_SEAGULL_POOP_INTERVAL,
                        g_pJeepThinkContext);
      } else {
  ...
  SetContextThink(&CPropJeep::JeepSeagullThink, gpGlobals->curtime + 0.2,
                  g_pJeepThinkContext);

  // Not taken enough time yet?
  float flHiddenTime = (gpGlobals->curtime - m_flLastSawPlayerAt);
  if (flHiddenTime < JEEP_SEAGULL_HIDDEN_TIME) return;

  // Random chance based upon the time it's taken
  float flChance = std::clamp(flHiddenTime / JEEP_SEAGULL_MAX_TIME, 0.0, 1.0);
...
void CPropJeep::SpawnPerchedSeagull(void) {
  ...
  CNPC_Seagull *pSeagull = (CNPC_Seagull *)CBaseEntity::Create(
      "npc_seagull", vecOrigin, vecAngles, NULL);
...
void CPropJeep::AddSeagullPoop(const Vector &vecOrigin) {
  ...
  for (int i = 0; i < iDecals; i++) {
    ...
    trace_t tr;
    UTIL_TraceLine(vecPoop, vecPoop - Vector(0, 0, 512), MASK_SHOT, m_hSeagull,
                   COLLISION_GROUP_NONE, &tr);
...
// Output : int
//-----------------------------------------------------------------------------
int CJeepFourWheelServerVehicle::GetExitAnimToUse(Vector &vecEyeExitEndpoint,
  ...
  // If we don't have the gun anymore, we want to get out using the "gun-less"
  // animation
  if (pAnimating && ((CPropJeep *)m_pVehicle)->TauCannonHasBeenCutOff()) {
    ...
    UTIL_TraceHull(vecStart, vecEnd, VEC_HULL_MIN, VEC_HULL_MAX, MASK_SOLID,
