...
// Changing this classname avoids bugs where ai_relationship entities that
// change the strider's relationship with bullseyes would affect its
...
ConVar strider_show_weapon_los_condition("strider_show_weapon_los_condition",
                                         "0");
ConVar strider_idle_test("strider_idle_test", "0");
ConVar strider_always_use_procedural_height(
    "strider_always_use_procedural_height", "0");
ConVar strider_test_height("strider_test_height", "0");
ConVar strider_pct_height_no_crouch_move("strider_pct_height_no_crouch_move",
                                         "90");
...
ConVar strider_free_pass_move_tolerance("strider_free_pass_move_tolerance",
                                        "320");
ConVar strider_free_knowledge("strider_free_knowledge", "0.5");

ConVar strider_free_pass_after_escorts_dead(
    "strider_free_pass_after_escorts_dead", "2.5");
ConVar strider_free_pass_tolerance_after_escorts_dead(
...
ConVar npc_strider_shake_ropes_magnitude("npc_strider_shake_ropes_magnitude",
                                         "150");
...
extern void CreateConcussiveBlast(const Vector &origin,
...
#define STRIDER_DEFAULT_SHOOT_DURATION \
  2.5  // spend this much time stitching to each target.
#define STRIDER_SHOOT_ON_TARGET_TIME \
  0.5  // How much of DEFAULT_SHOOT_DURATION is spent on-target (vs. stitching
       // up to a target)
...
#define STRIDER_SUBSEQUENT_TARGET_DURATION \
  1.5  // Spend this much time stitching to targets chosen by distributed fire.
...
#define STRIDER_NO_TRACK_NAME "0"
...
static void MoveToGround(Vector *position, CBaseEntity *ignore,
...
SendPropVector(SENDINFO(m_vecHitPos), -1, SPROP_COORD),
    SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecIKTarget, 0), -1,
                   SPROP_COORD),
    SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecIKTarget, 1), -1,
                   SPROP_COORD),
    SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecIKTarget, 2), -1,
                   SPROP_COORD),
    SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecIKTarget, 3), -1,
                   SPROP_COORD),
    SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecIKTarget, 4), -1,
                   SPROP_COORD),
    SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecIKTarget, 5), -1,
                   SPROP_COORD),
    END_SEND_TABLE()
...
void CNPC_Strider::Spawn() {
  ...
  CapabilitiesAdd(bits_CAP_MOVE_FLY | bits_CAP_INNATE_RANGE_ATTACK2 |
                  bits_CAP_INNATE_MELEE_ATTACK1 |
                  bits_CAP_INNATE_MELEE_ATTACK2 | bits_CAP_SQUAD);

  // Don't allow us to skip animation setup because our attachments are critical
  // to us!
  ...
  origin.z +=
      GetMaxHeight();  //(GetAbsOrigin().z - vecSurroundMins.z) + mins.z;
...
void CNPC_Strider::Activate() {
  ...
  if (m_BodyTargetBone == -1) {
    DevMsg(
        "Couldn't find npc_strider bone %s, which is used as target for "
        "others\n",
        pszBodyTargetBone);
  }
  ...
  if (gm_zCannonDist == 0) {
    ...
    Assert(pStrider);
    pStrider->m_bDisableBoneFollowers =
        true;  // don't create these since we're just going to destroy him
    ...
    VectorITransform(position, pStrider->EntityToWorldTransform(),
                     gm_vLocalRelativePositionCannon);
...
Class_T CNPC_Strider::Classify() {
  if (CarriedByDropship()) return CLASS_NONE;
...
#ifdef HL2_EPISODIC
  if (m_hAttachedBusters.Count() > 0) return false;
#endif  // HL2_EPISODIC
...
int CNPC_Strider::DrawDebugTextOverlays() {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    EntityText(text_offset,
               CFmtStr("Ideal Height: %.1f; Height: %.1f", GetIdealHeight(),
                       GetHeight()),
               0);
    text_offset++;
    if (m_PlayerFreePass.HasPass()) {
      EntityText(
          text_offset,
          CFmtStr("Free pass: %.1f", m_PlayerFreePass.GetTimeRemaining()), 0);
      text_offset++;
    }

    CBaseEntity *pPlayer = UTIL_PlayerByIndex(1);
    if (pPlayer) {
      if (GetSenses()->ShouldSeeEntity(pPlayer) &&
          GetSenses()->CanSeeEntity(pPlayer)) {
    ...
    if (m_flTargetSpeedScale != 1.0) {
      EntityText(text_offset, CFmtStr("Speed scaled to %.1f", m_flGroundSpeed),
                 0);
      text_offset++;
    }
  }
...
void CNPC_Strider::PrescheduleThink() {
  if (IsUsingAggressiveBehavior() && GetEnemy() && GetEnemy()->IsPlayer()) {
    AddFacingTarget(GetEnemy(), GetEnemies()->LastKnownPosition(GetEnemy()),
                    1.0, 2.0);
  }
  ...
  if (strider_show_focus.GetBool()) {
    ...
    NDebugOverlay::Cross3DOriented(GetFocus()->GetAbsOrigin(), angles, 24, 255,
                                   255, 0, false, 0.1);
  }
}
...
void CNPC_Strider::GatherConditions() {
  if (AIGetNumFollowers(this, m_iszHunterClassname) == 0) {
    // This works with old data because need to do before base class so as to
    // not choose as enemy
    if (m_PlayerFreePass.HasPass() ||
        (!m_pMinigun->IsShooting() ||
         GetEnemy() != m_PlayerFreePass.GetPassTarget()))  // no free pass when
                                                           // in midst of
                                                           // shooting at target
      m_PlayerFreePass.Update();
  } else {
    m_PlayerFreePass.Reset(
        strider_free_pass_after_escorts_dead.GetFloat(),
        strider_free_pass_tolerance_after_escorts_dead.GetFloat());
  }
  ...
  if (IsUsingAggressiveBehavior()) {
    if (GetEnemy()) {
      if (HasCondition(COND_SEE_ENEMY)) {
        // Keep setting up to play my hunt sound at some random time after
        // losing sight of my enemy.
        m_flTimeNextHuntSound = gpGlobals->curtime + 1.0f;
      } else {
        if (gpGlobals->curtime >= m_flTimeNextHuntSound &&
            !m_pMinigun->IsShooting()) {
          HuntSound();
        }
      }
    ...
    if (m_hPlayersMissile) {
      if (!m_pMinigun->IsShooting() && GetEnemy() && GetEnemy()->IsPlayer()) {
        // If the missile is closer to the player than I am, stay suppressed.
        // This is essentially allowing the missile to strike me if it was fired
        // off before I started shooting. If the missile passes me or goes way
        // off course, I can shoot.
        float flPlayerMissileDist;
        float flPlayerStriderDist;

        flPlayerMissileDist = GetEnemy()->GetAbsOrigin().DistTo(
            m_hPlayersMissile->GetAbsOrigin());
        flPlayerStriderDist = GetEnemy()->GetAbsOrigin().DistTo(EyePosition());
        float flDiff = flPlayerMissileDist - flPlayerStriderDist;

        // Figure out how long it's been since I've fired my cannon because of a
        // player's missile.
        float flTimeSuppressed =
            gpGlobals->curtime - m_flTimePlayerMissileDetected;

        if (flDiff < strider_missile_suppress_dist.GetFloat() &&
            flTimeSuppressed < strider_missile_suppress_time.GetFloat()) {
  ...
  if (GetCannonTarget()) {
    ...
    if (strider_show_cannonlos.GetBool()) {
      trace_t tr;
      UTIL_TraceLine(CannonPosition(), GetCannonTarget()->WorldSpaceCenter(),
                     MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
      NDebugOverlay::Line(tr.startpos, tr.endpos, 0, 255, 0, false, 0.1);

      if (tr.fraction != 1.0)
        NDebugOverlay::Line(tr.endpos, GetCannonTarget()->WorldSpaceCenter(),
                            255, 0, 0, false, 0.1);
    }
  } else {
  ...
  if (!m_bCrouchLocked && !m_bDontCrouch) {
    if (m_hCannonTarget != NULL) {
      if (!IsStriderCrouching() && !IsStriderStanding()) {
        if (WeaponLOSCondition(GetAdjustedOrigin(),
                               m_hCannonTarget->GetAbsOrigin(), false)) {
      ...
      if (strider_distributed_fire.GetBool() && !IsUsingAggressiveBehavior()) {
        ...
        for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
          ...
          if (IRelationType(pEMemory->hEnemy) != D_NU &&
              IRelationType(pEMemory->hEnemy) != D_LI) {
        ...
        // If I'm on target and see more targets than just this one, move on to
        // another target for a bit! Because the Mingun's state will stay "on
        // target" until the minigun gets a chance to think, and this function
        // may be called several times per strider think, don't call this code
        // anymore in the same think when a new enemy is chosen.
        //
        // Don't switch targets if shooting at a bullseye! Level designers
        // depend on bullseyes.
        if (GetEnemy() && m_pMinigun->IsShooting() &&
            GetTimeEnemyAcquired() != gpGlobals->curtime) {
          if (m_pMinigun->IsOnTarget(3) &&
              !FClassnameIs(GetEnemy(), "npc_bullseye")) {
            if (m_iVisibleEnemies > 1) {
              // Time to ignore this guy for a little while and switch targets.
              GetEnemies()->SetTimeValidEnemy(
                  GetEnemy(),
                  gpGlobals->curtime +
                      (STRIDER_IGNORE_TARGET_DURATION * m_iVisibleEnemies));
              SetEnemy(NULL, false);
              ChooseEnemy();
            } else if (GetEnemy()->IsPlayer() &&
                       GetEnemy() == m_pMinigun->GetTarget()) {
              // Give the poor target a break.
              m_pMinigun->StopShootingForSeconds(this, GetEnemy(),
                                                 GetMinigunShootDowntime());
            }
          }
        }
      }

      if (GetEnemy())  // Can go 0 above
      {
        if (!IsStriderCrouching() && !IsStriderStanding() &&
            (!HasCondition(COND_SEE_ENEMY) ||
             !WeaponLOSCondition(GetAdjustedOrigin(),
                                 GetEnemy()->BodyTarget(GetAdjustedOrigin()),
                                 false))) {
#if 0
    ...
    if (m_hCannonTarget != NULL &&
        CurrentWeaponLOSCondition(m_hCannonTarget->GetAbsOrigin(), false))
      SetCondition(COND_CAN_RANGE_ATTACK2);
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::GatherHeightConditions(const Vector &vTestPos,
                                          CBaseEntity *pEntity) {
  if (HasCondition(COND_STRIDER_SHOULD_CROUCH) &&
  ...
  if (FInViewCone(pEntity) &&
      GetWeaponLosZ(vTestPos, minZ, maxZ, GetHeightRange() * .1, pEntity,
                    &newHeight)) {
    bool bDoProceduralHeightChange = true;
    newHeight = GetMaxHeightModel() - (GetAbsOrigin().z - newHeight);

    if (m_LowZCorrectionTimer.Expired()) {
      // Hack to handle discrepency between ideal gun pos and actual pos due to
      // strider head tilt in crouch pos
      if (pEntity && fabs(newHeight - GetHeight()) < 12 &&
          newHeight < GetMinHeight() + GetHeightRange() * .33) {
        Vector muzzlePos;
        ...
        trace_t tr;
        AI_TraceLine(muzzlePos, targetPos, MASK_SHOT, this,
                     COLLISION_GROUP_NONE, &tr);

        if ((tr.m_pEnt != pEntity) && tr.fraction != 1.0 &&
            !CanShootThrough(tr, targetPos)) {
          if (!GetWeaponLosZ(vTestPos, minZ + GetHeightRange() * .33, maxZ,
                             GetHeightRange() * .1, pEntity, &newHeight)) {
            return;
          }
          newHeight = GetMaxHeightModel() - (GetAbsOrigin().z - newHeight);
        } else {
    ...
    if (!strider_always_use_procedural_height.GetBool()) {
      ...
      if (fabsf(GetMaxHeight() - GetHeight()) < 0.1 &&
          fabsf(GetMinHeight() - newHeight) < 0.1) {
        if (random->RandomInt(1, 10) <= 6) {
          SetCondition(COND_STRIDER_SHOULD_CROUCH);
          bDoProceduralHeightChange = false;
        }
      } else if (fabsf(GetMinHeight() - GetHeight()) < 0.1 &&
                 fabsf(GetMaxHeight() - newHeight) < 0.1) {
        if (random->RandomInt(1, 10) <= 6) {
      ...
      // Otherwise, if going from near max or near min to the other, use
      // animations based on time
      if (bDoProceduralHeightChange && m_PostureAnimationTimer.Expired()) {
        if (GetHeight() - GetMinHeight() > GetHeightRange() * .85 &&
            newHeight - GetMinHeight() < GetHeightRange() * .15) {
          m_PostureAnimationTimer.Reset();
          SetCondition(COND_STRIDER_SHOULD_CROUCH);
          bDoProceduralHeightChange = false;
        } else if (newHeight - GetMinHeight() > GetHeightRange() * .85 &&
                   GetHeight() - GetMinHeight() < GetHeightRange() * .15) {
          m_PostureAnimationTimer.Reset();
          SetCondition(COND_STRIDER_SHOULD_STAND);
          bDoProceduralHeightChange = false;
        }
...
void CNPC_Strider::BuildScheduleTestBits() {
  ...
  if (GetCurSchedule()->HasInterrupt(COND_CAN_RANGE_ATTACK1) &&
      HasCondition(COND_STRIDER_HAS_CANNON_TARGET)) {
    SetCustomInterruptCondition(COND_STRIDER_HAS_CANNON_TARGET);
  }

  if (IsCurSchedule(SCHED_IDLE_WALK) ||
      IsCurSchedule(SCHED_IDLE_STAND) && hl2_episodic.GetBool()) {
...
int CNPC_Strider::SelectSchedule() {
  ...
  */
  ...
  if (HasCondition(COND_STRIDER_SHOULD_CROUCH) &&
      GetHeight() - GetMinHeight() > GetHeightRange() * .5) {
    return SCHED_STRIDER_CROUCH;
  }
  if (HasCondition(COND_STRIDER_SHOULD_STAND) && !m_bCrouchLocked &&
      GetHeight() - GetMinHeight() < GetHeightRange() * .5) {
    return SCHED_STRIDER_STAND;
  }
  ...
  } else if (m_AttemptCannonLOSTimer.Expired() &&
             HasCondition(COND_STRIDER_HAS_CANNON_TARGET)) {
  ...
  if (m_NPCState == NPC_STATE_COMBAT) {
    if (!HasCondition(COND_NEW_ENEMY)) {
      if (m_hRagdoll.Get() && (gpGlobals->curtime > m_ragdollTime ||
                               HasCondition(COND_STRIDER_DO_FLICK))) {
    ...
    if (!m_EnemyUpdatedTimer.Expired()) {
      ...
      if (baseResult != SCHED_COMBAT_FACE || !GetGoalEnt()) return baseResult;
    }
  ...
  switch (scheduleType) {
    ...
    case SCHED_ESTABLISH_LINE_OF_FIRE_FALLBACK:
      ...
      if (!m_bCrouchLocked) {
        ...
        } else if (HasCondition(COND_STRIDER_HAS_LOS_Z) &&
                   HasCondition(COND_SEE_ENEMY)) {
          return SCHED_STRIDER_COMBAT_FACE;
        }
      }
...
void CNPC_Strider::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_STRIDER_AIM: {
      ...
      // te->StreakSphere( filter, 0, 0, 150, 100, entindex(),
      // gm_CannonAttachment );
    } break;
    ...
    case TASK_STRIDER_STOMP: {
      m_nextStompTime = gpGlobals->curtime + 5;
      Activity stompAct = (Activity)(
          pTask->flTaskData > 0 ? ACT_STRIDER_STOMPR : ACT_STRIDER_STOMPL);
    ...
    case TASK_STRIDER_REFRESH_HUNT_PATH:
      ...
      if (GetGoalEnt()) {
        AI_NavGoal_t goal(GOALTYPE_PATHCORNER, GetGoalEnt()->GetLocalOrigin(),
                          ACT_WALK, AIN_DEF_TOLERANCE, AIN_YAW_TO_DEST);

        TranslateNavGoal(GetGoalEnt(), goal.dest);

        if ((m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) &&
            (m_debugOverlays & OVERLAY_NPC_ROUTE_BIT)) {
          NDebugOverlay::Line(GetAbsOrigin() + Vector(0, 0, 4), goal.dest, 255,
                              0, 255, true, 3);
        }
    ...
    case TASK_STRIDER_GET_PATH_TO_CANNON_LOS: {
      ...
      if (!found &&
          GetTacticalServices()->FindLos(vecEnemy, vecEnemy, flMinRange,
                                         flMaxRange, 1.0, &posLos)) {
        found = true;
      }
      ...
      if (found) {
        ...
        AddFacingTarget(GetCannonTarget(),
                        GetCannonTarget()->WorldSpaceCenter(), 5.0, 5.0);
      } else {
...
void CNPC_Strider::Explode(void) {
  ...
  // Break into pieces
  breakablepropparams_t params(EyePosition(), GetAbsAngles(), velocity,
                               angVelocity);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Strider::HandleInteraction(int interactionType, void *data,
...
void CNPC_Strider::HandleAnimEvent(animevent_t *pEvent) {
  ...
  switch (pEvent->event) {
    ...
    case STRIDER_AE_SHOOTMINIGUN: {
      CBaseEntity *pTarget =
          gEntList.FindEntityGeneric(NULL, pEvent->options, this, this);
      if (pTarget) {
...
Disposition_t CNPC_Strider::IRelationType(CBaseEntity *pTarget) {
  if (IsCannonTarget(pTarget)) return D_HT;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::AddEntityRelationship(CBaseEntity *pEntity,
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::ScheduledMoveToGoalEntity(int scheduleType,
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::ScheduledFollowPath(int scheduleType,
...
void CNPC_Strider::InputSetCannonTarget(inputdata_t &inputdata) {
  ...
  if (pTarget) {
    if (m_hCannonTarget == pTarget) return;
...
void CNPC_Strider::InputDisableCollisionWith(inputdata_t &inputdata) {
  IPhysicsObject *pIgnorePhys =
      FindPhysicsObjectByName(inputdata.value.String(), this);
  if (!pIgnorePhys) return;

  // CBoneFollower *pFol;

  for (int idx = m_BoneFollowerManager.GetNumBoneFollowers() - 1; idx >= 0;
       --idx)  // stop when the function starts returning 0 (idx is no longer
               // good)
  {
    IPhysicsObject *pFollowPhys =
        GetBoneFollowerByIndex(idx)->VPhysicsGetObject();
    Assert(pFollowPhys);
...
void CNPC_Strider::InputEnableCollisionWith(inputdata_t &inputdata) {
  ...
  for (idx = m_BoneFollowerManager.GetNumBoneFollowers() - 1; idx >= 0;
       --idx)  // stop when the function starts returning 0 (idx is no longer
               // good)
  {
    /*
...
void CNPC_Strider::InputClearTargetPath(inputdata_t &inputdata) {
  if (GetNavigator()->IsGoalActive() &&
      GetNavigator()->GetGoalType() == GOALTYPE_PATHCORNER) {
...
void CNPC_Strider::InputEnableAggressiveBehavior(inputdata_t &inputdata) {
  ...
  GetEnemies()->SetEnemyDiscardTime(
      601.0f);  // Make the assert in SetFreeKnowledgeDuration() happy.
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::FVisible(CBaseEntity *pEntity, int traceMask,
...
bool CNPC_Strider::IsValidEnemy(CBaseEntity *pTarget) {
  ...
  if (pTarget->IsPlayer()) {
    if (AIGetNumFollowers(this, m_iszHunterClassname) > 0) return false;
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::UpdateEnemyMemory(CBaseEntity *pEnemy,
  ...
  if (pInformer && FClassnameIs(pInformer, "npc_cscanner")) {
...
#else
    trace_t tr;
    AI_TraceLine(EyePosition(), position + Vector(0, 0, 32), MASK_SHOT, this,
                 COLLISION_GROUP_NONE, &tr);
    ...
    BaseClass::UpdateEnemyMemory(GetFocus(), GetFocus()->GetAbsOrigin(),
                                 pInformer);

    // Change the informer to myself so that information provided by a scanner
    // is as good as firsthand knowledge insofar as enemy memory is concerned.
    pInformer = this;
  }
...
float CNPC_Strider::StriderEnemyDistance(CBaseEntity *pEnemy) {
  ...
  CollisionProp()->WorldSpaceSurroundingBounds(&striderSurroundMins,
                                               &striderSurroundMaxs);
...
int CNPC_Strider::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  // recompute this because the base class function does not work for the
  // strider
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::WeaponLOSCondition(const Vector &ownerPos,
  ...
  trace_t tr;
  AI_TraceLine(ownerPos + vRootOffset, targetPos, MASK_SHOT, this,
               COLLISION_GROUP_NONE, &tr);

  // Hit the enemy, or hit nothing (traced all the way to a nonsolid enemy like
  // a bullseye)
  if ((pTargetEnt && tr.m_pEnt == pTargetEnt) || tr.fraction == 1.0 ||
    ...
    AI_TraceLine(vBarrelPos, targetPos, MASK_SHOT, this, COLLISION_GROUP_NONE,
                 &tr);
    if ((pTargetEnt && tr.m_pEnt == pTargetEnt) || tr.fraction == 1.0 ||
        CanShootThrough(tr, targetPos)) {
      if (strider_show_weapon_los_condition.GetBool()) {
        NDebugOverlay::Line(ownerPos + vRootOffset, targetPos, 255, 0, 255,
                            false, 0.1);
    ...
    if (strider_show_weapon_los_condition.GetBool()) {
      NDebugOverlay::Line(ownerPos + vRootOffset, tr.endpos, 255, 0, 0, false,
                          0.1);
    }
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::CurrentWeaponLOSCondition(const Vector &targetPos,
                                             bool bSetConditions) {
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::IsValidShootPosition(const Vector &vecCoverLocation,
                                        CAI_Node *pNode,
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::TestShootPosition(const Vector &vecShootPos,
                                     const Vector &targetPos) {
  ...
  // What if I crouched?
  float zDelta =
      ((vecShootPos.z - targetPos.z) + gm_vLocalRelativePositionMinigun.z) - 12;
  if (zDelta > 0) {
    if (zDelta > GetHeightRange()) zDelta = GetHeightRange();
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr,
                              int iTracerType) {
  float flTracerDist;
  Vector vecDir;
  Vector vecEndPos;
  ...
  UTIL_Tracer(vecTracerSrc, tr.endpos, 0, TRACER_DONT_USE_ATTACHMENT, 5000,
              true, "StriderTracer");
}
...
void CNPC_Strider::DoImpactEffect(trace_t &tr, int nDamageType) {
  ...
  if (tr.m_pEnt && !tr.m_pEnt->MyNPCPointer()) {
    ...
    if (UTIL_PointContents(vecReTrace) == CONTENTS_EMPTY) {
      AI_TraceLine(vecReTrace, vecReTrace - vecDir * 24, MASK_SHOT, NULL,
                   COLLISION_GROUP_NONE, &retrace);
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::GetWeaponLosZ(const Vector &vOrigin, float minZ, float maxZ,
                                 float increment, CBaseEntity *pTarget,
                                 float *pResult) {
  Vector vTestPos;
  ...
  while (vTestPos.z <= maxZ &&
         !WeaponLOSCondition(vTestPos, vTargetPos, false)) {
    vTestPos += vIncrement;
  }
...
void CNPC_Strider::HuntSound() {
  if (m_PlayerFreePass.HasPass()) {
    ...
    m_flTimeNextHuntSound =
        gpGlobals->curtime + random->RandomFloat(8.0f, 12.0f);
  }
}

//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::TraceAttack(const CTakeDamageInfo &inputInfo,
  ...
  for (int i = 0; i < m_hAttachedBusters.Count(); i++) {
    if (m_hAttachedBusters[i] == NULL) continue;

    flDistSqr = (m_hAttachedBusters[i]->WorldSpaceCenter() -
                 inputInfo.GetDamagePosition())
                    .LengthSqr();
...
int CNPC_Strider::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (IsUsingAggressiveBehavior()) {
    // Any damage the player inflicts gets my attention, even if it doesn't
    // actually harm me.
    if (info.GetAttacker()->IsPlayer()) {
  ...
  if ((info.GetDamageType() & DMG_BLAST) && info.GetMaxDamage() > 50) {
    ...
    float dist = CalcDistanceToAABB(WorldAlignMins(), WorldAlignMaxs(),
                                    info.GetDamagePosition() - headPos);
    // close enough to do damage?
    if (dist < 200) {
      bool bPlayer = info.GetAttacker()->IsPlayer();
      if (bPlayer) {
        m_PlayerFreePass.Revoke();
        AddFacingTarget(info.GetAttacker(), info.GetAttacker()->GetAbsOrigin(),
                        1.0, 2.0);

        UpdateEnemyMemory(info.GetAttacker(),
                          info.GetAttacker()->GetAbsOrigin());
      } else
        AddFacingTarget(info.GetAttacker(), info.GetAttacker()->GetAbsOrigin(),
                        0.5, 2.0);
...
// NOTE: Currently radius damage doesn't even call this because it uses the
// origin, not the box for distance
#if 0
...
int CNPC_Strider::TakeDamageFromCombineBall(const CTakeDamageInfo &info) {
  ...
  AddFacingTarget(info.GetInflictor(), info.GetInflictor()->GetAbsOrigin(), 0.5,
                  2.0);
  if (!UTIL_IsAR2CombineBall(info.GetInflictor()))
...
void CNPC_Strider::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (info.GetInflictor() && UTIL_IsCombineBall(info.GetInflictor())) {
    ...
    // Tracker 23610:  Strider playing death sounds twice (AI_BaseNPC calls it
    // in Event_Killed, too) DeathSound( info );
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::RagdollDeathEffect(CRagdollProp *pRagdoll,
                                      float flDuration) {
...
bool CNPC_Strider::ShouldExplodeFromDamage(const CTakeDamageInfo &info) {
  ...
  if (pInflictor == NULL) return false;
...
bool CNPC_Strider::BecomeRagdoll(const CTakeDamageInfo &info,
    ...
    if (pPlayer && mat_dxlevel.GetInt() > 0) {
      ...
      if (maxRagdolls > 0) {
        ...
        while ((pRagdoll = gEntList.NextEntByClass(pRagdoll)) != NULL) {
          if (pRagdoll->GetModelName() == GetModelName() &&
              !pRagdoll->IsFading()) {
        ...
        if (striderRagdolls.Count() >= maxRagdolls) {
          ...
          for (int i = 0; i < striderRagdolls.Count(); i++) {
            float distSqrCur = CalcSqrDistanceToAABB(
                striderRagdolls[i]->WorldAlignMins(),
                striderRagdolls[i]->WorldAlignMaxs(), pPlayer->GetAbsOrigin());
      ...
      pRagdoll = assert_cast<CRagdollProp *>(CreateServerRagdoll(
          this, m_nForceBone, info, HL2COLLISION_GROUP_STRIDER));
...
bool CNPC_Strider::IsStriderCrouching() {
  if (IsCurSchedule(SCHED_STRIDER_CROUCH, false)) return true;
...
bool CNPC_Strider::IsStriderStanding() {
  if (IsCurSchedule(SCHED_STRIDER_STAND, false)) return true;
...
bool CNPC_Strider::OverrideMove(float flInterval) {
  if (GetCannonTarget()) {
    AddFacingTarget(GetCannonTarget(), GetCannonTarget()->GetAbsOrigin(), 1.0,
                    0.5);
  } else if (GetEnemy()) {
    ...
    if (!m_bCrouchLocked && !m_hCannonTarget &&
        GetIdealHeight() < GetMaxHeight() &&
        timeSinceSeenEnemy > TIME_CARE_ENEMY)
      SetIdealHeight(GetMaxHeight());
  ...
  // If we're not supposed to crouch walk and we're under the threshold for what
  // we consider crouch walking, stand back up!
  if (!m_bCrouchLocked && m_bNoCrouchWalk && IsMoving() &&
      (GetIdealHeight() <
       (GetMinHeight() +
        GetHeightRange() *
            (strider_pct_height_no_crouch_move.GetFloat() / 100.0)))) {
...
#define HEIGHTACCEL 0.5  // accel toward maxVelocity by X percent each cycle
    ...
    if (heightMove < m_HeightVelocity * heightMoveSign * HEIGHTDECAYTIME) {
      ...
      m_HeightVelocity = m_HeightVelocity * (1.0f - HEIGHTACCEL) +
                         maxVelocity * HEIGHTACCEL * heightMoveSign;
      if (heightMove < m_HeightVelocity * heightMoveSign * HEIGHTDECAYTIME) {
...
void CNPC_Strider::MaintainTurnActivity(void) {
  ...
  if (!IsMoving() && m_doTurn > 180.0f &&
      m_flNextTurnAct < gpGlobals->curtime) {
...
bool CNPC_Strider::IsUnusableNode(int iNodeID, CAI_Hint *pHint) {
  if (!BaseClass::IsUnusableNode(iNodeID, pHint)) {
    if (pHint && pHint->HintType() == HINT_STRIDER_NODE) return false;
  }
  return true;
}

//---------------------------------------------------------
// Purpose: Compute the nav position for the strider's origin relative to this
// enemy.
//---------------------------------------------------------
void CNPC_Strider::TranslateNavGoal(CBaseEntity *pEnemy,
                                    Vector &chasePosition) {
  if (pEnemy) {
    if (!(pEnemy->GetFlags() & FL_ONGROUND)) {
      MoveToGround(&chasePosition, pEnemy, pEnemy->WorldAlignMins(),
                   pEnemy->WorldAlignMaxs());
    }
...
bool CNPC_Strider::HasPendingTargetPath() {
  bool bReturn =
      (!FStrEq(STRING(m_strTrackName), STRIDER_NO_TRACK_NAME) && !GetGoalEnt());
...
void CNPC_Strider::OnMovementComplete() {
  if (GetGoalEnt() &&
      (IsCurSchedule(SCHED_IDLE_WALK) || IsCurSchedule(SCHED_ALERT_WALK) ||
       IsCurSchedule(SCHED_COMBAT_WALK) || IsCurSchedule(SCHED_STRIDER_HUNT))) {
    m_strTrackName = MAKE_STRING(STRIDER_NO_TRACK_NAME);
    SetGoalEnt(NULL);
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
float CNPC_Strider::GetSequenceGroundSpeed(CStudioHdr *pStudioHdr,
                                           int iSequence) {
...
float CNPC_Strider::MaxYawSpeed() {
  switch (GetActivity()) {
    ...
    default:
      return 10;  // should be zero, but this makes it easy to see when he's
                  // turning with default AI code
      break;
  }
}
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::ShootMinigun(const Vector *pTarget, float aimError,
  ...
  if (pTarget) {
    ...
    if (m_bMinigunUseDirectFire) {
      ...
      FireBullets(1, muzzlePos, vecShootDir, vecSpread, 8192,
                  m_miniGunDirectAmmo, 1);
    } else {
      // exactly on target w/tracer
      FireBullets(1, muzzlePos, vecShootDir, vecSpread, 8192, m_miniGunAmmo, 1);
    }

    // g_pEffects->MuzzleFlash( muzzlePos, muzzleAng, random->RandomFloat(
    // 2.0f, 4.0f ) , MUZZLEFLASH_TYPE_STRIDER );
    DoMuzzleFlash();
...
float CNPC_Strider::GetMinigunShootDuration() {
  if (IsUsingAggressiveBehavior()) return STRIDER_EP1_SHOOT_DURATION;
...
float CNPC_Strider::GetMinigunShootDowntime() {
  if (IsUsingAggressiveBehavior()) return STRIDER_EP1_SHOOT_DOWNTIME;
...
float CNPC_Strider::GetMinigunShootVariation() {
  if (IsUsingAggressiveBehavior()) {
    if (m_bMinigunUseDirectFire) return 0.0f;
...
bool CNPC_Strider::AimCannonAt(CBaseEntity *pEntity, float flInterval) {
  ...
  Vector unitAngles =
      Vector(localEnemyAngles.x, localEnemyAngles.y, localEnemyAngles.z);
  ...
  // as long as the velocity is less than the delta between the limit and 180,
  // this is fine.
  m_aimPitch = GetPoseParameter(gm_PitchControl);
  m_aimYaw = GetPoseParameter(gm_YawControl);

  // UNDONE: Zero out any movement past the limit and go ahead and fire if the
  // strider hit its target except for clamping.  Need to clamp targets to
  // limits and compare?
...
void CNPC_Strider::FireCannon() {
  ...
  AI_TraceLine(vecShootPos, vecShootPos + vecShootDir * flDist, MASK_SHOT, this,
               COLLISION_GROUP_NONE, &tr);
  ...
  SetContextThink(&CNPC_Strider::CannonHitThink, gpGlobals->curtime + 0.2f,
                  "CANNON_HIT");
}
...
void CNPC_Strider::CannonHitThink() {
  ...
  if (pCannonTarget) {
    ...
    if (fAlive && pCannonTarget->IsAlive()) {
      DevWarning("* * * * * * * * * * * * * * *\n");
      DevWarning(
          "NASTYGRAM: STRIDER failed to kill its cannon target. Killing "
          "directly...\n");
      DevWarning("* * * * * * * * * * * * * * *\n");
...
//---------------------------------------------------------
//---------------------------------------------------------
void CNPC_Strider::VPhysicsShadowCollision(int index,
  ...
  if (pOther && UTIL_IsCombineBall(pOther)) {
    ...
    CTakeDamageInfo dmgInfo(pOther, pOther, vec3_origin, damagePos,
                            (m_iMaxHealth / 3) + 1,
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::TestCollision(const Ray_t &ray, unsigned int mask,
                                 trace_t &trace) {
  ...
  if (IntersectRayWithBox(ray, WorldAlignMins() + GetAbsOrigin(),
                          WorldAlignMaxs() + GetAbsOrigin(), DIST_EPSILON,
                          &trace)) {
    trace.hitbox = 0;
...
Vector CNPC_Strider::LeftFootHit(float eventtime) {
  ...
  if (hl2_episodic.GetBool()) {
    ...
    EmitSound(filter, 0, "NPC_Strider.FootstepEverywhere", &footPosition,
              eventtime);
  } else {
...
//---------------------------------------------------------
//---------------------------------------------------------
static Vector GetAttachmentPositionInSpaceOfBone(CStudioHdr *pStudioHdr,
  ...
  Studio_CalcBoneToBoneTransform(pStudioHdr, iBone, outputBoneIndex,
                                 inputToOutputBone);
...
void CNPC_Strider::StompHit(int followerBoneIndex) {
  ...
  physfollower_t *bone =
      m_BoneFollowerManager.GetBoneFollower(followerBoneIndex);
  if (!bone) return;

  const char *pBoneNames[] = {"left skewer", "right skewer"};
  int nameIndex = followerBoneIndex == STRIDER_LEFT_LEG_FOLLOWER_INDEX ? 0 : 1;
  Vector localHit = GetAttachmentPositionInSpaceOfBone(
      pStudioHdr, pBoneNames[nameIndex], bone->boneIndex);
  IPhysicsObject *pLegPhys = bone->hFollower->VPhysicsGetObject();

  // now transform into the worldspace of the current position of the leg's
  // physics
  ...
  // NDebugOverlay::Box( hitPosition, Vector(-16,-16,-16), Vector(16,16,16), 0,
  // 255, 0, 255, 1.0 );
  CBaseEntity *pEnemy = GetEnemy();
  ...
  if (delta.LengthSqr() > (STRIDER_STOMP_RANGE * STRIDER_STOMP_RANGE)) return;

  // DVS E3 HACK: Assume we stab our victim midway between their eyes and their
  // center.
  Vector vecStabPos =
      (pEnemy->WorldSpaceCenter() + pEnemy->EyePosition()) * 0.5f;
  ...
  UTIL_BloodSpray(vecStabPos + vecBloodDelta * 4, vecBloodDelta,
                  BLOOD_COLOR_RED, 8, FX_BLOODSPRAY_ALL);
  UTIL_BloodSpray(vecStabPos + vecBloodDelta * 4, vecBloodDelta,
                  BLOOD_COLOR_RED, 11, FX_BLOODSPRAY_DROPS);
  CBaseEntity *pRagdoll = CreateServerRagdollAttached(
      pNPC, vec3_origin, -1, COLLISION_GROUP_DEBRIS, pLegPhys, this,
      bone->boneIndex, vecStabPos, -1, localHit);
  if (pRagdoll) {
...
void CNPC_Strider::FootFX(const Vector &origin) {
  trace_t tr;
  AI_TraceLine(origin + Vector(0, 0, 48), origin - Vector(0, 0, 100),
               MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
  float yaw = random->RandomInt(0, 120);

  if (UTIL_PointContents(tr.endpos + Vector(0, 0, 1)) & MASK_WATER) {
    float flWaterZ =
        UTIL_FindWaterSurface(tr.endpos, tr.endpos.z, tr.endpos.z + 100.0f);
  ...
  if (npc_strider_shake_ropes_radius.GetInt()) {
    CRopeKeyframe::ShakeRopes(tr.endpos,
                              npc_strider_shake_ropes_radius.GetFloat(),
                              npc_strider_shake_ropes_magnitude.GetFloat());
  }

  //
  // My feet are scary things! NOTE: We might want to make danger sounds as the
  // feet move through the air. Then soldiers could run from the feet, which
  // would look cool.
  //
  CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_EXCLUDE_COMBINE,
                         tr.endpos, 512, 1.0f, this);
}
...
static void MoveToGround(Vector *position, CBaseEntity *ignore,
  ...
  floor.z -= 1024;
  AI_TraceHull(*position, floor, mins, maxs, MASK_NPCSOLID_BRUSHONLY, ignore,
               COLLISION_GROUP_NONE, &tr);
  if (tr.fraction < 1) {
...
void AdjustStriderNodePosition(CAI_Network *pNetwork, CAI_Node *pNode) {
  if (pNode->GetHint() && pNode->GetHint()->HintType() == HINT_STRIDER_NODE) {
    CNPC_Strider *pStrider =
        (CNPC_Strider *)gEntList.FindEntityByClassname(NULL, "npc_strider");
    bool bCreated = false;
    if (!pStrider) {
    ...
    if (pStrider) {
      ...
      if (bCreated) UTIL_Remove(pStrider);
    }
  }
}

//---------------------------------------------------------
// Implement a navigator so that the strider can get closer to the ground when
// he's walking on a slope This gives him enough reach for his legs to IK to
// lower ground points
//---------------------------------------------------------
void LookaheadPath(const Vector &current, AI_Waypoint_t *pWaypoint, float dist,
                   Vector &nextPos) {
  if (!pWaypoint) return;
...
void CNPC_Strider::CNavigator::MoveCalcBaseGoal(AILocalMoveGoal_t *pMoveGoal) {
  ...
  LookaheadPath(GetAbsOrigin(), pMoveGoal->pPath->GetCurWaypoint(),
                CNPC_Strider::gm_strideLength, dest);

  // NDebugOverlay::Box( dest, Vector(-16,-16,-16), Vector(16,16,16), 0, 255, 0,
  // 255, 0.1 );
...
bool CNPC_Strider::CNavigator::MoveUpdateWaypoint(AIMoveResult_t *pResult) {
  // Note that goal & waypoint tolerances are handled in progress blockage cases
  // (e.g., OnObstructionPreSteer)
  ...
  // The strider is following paths most of the time, but he needs a bunch of
  // freedom to adjust himself to make his feet look good.
...
bool CNPC_Strider::CNavigator::DoFindPathToPos() {
  ...
  if (BaseClass::DoFindPathToPos()) {
    ...
    if (pFirstWaypoint->IsReducible() && pFirstWaypoint->GetNext() &&
      ...
      Vector vClosest;
      CalcClosestPointOnLineSegment(vOrigin, pFirstWaypoint->GetPos(),
      ...
      // If we're seemingly beyond the waypoint and our hull is near enough to
      // the path segment, advance to the next node
      const float EPS = 0.1f;
      bool bPastStartNode =
          ((pFirstWaypoint->GetPos() - vClosest).Length() > EPS);
      bool bNearPathSegment =
          ((vOrigin - vClosest).Length() < GetHullWidth() * 0.5f);
    ...
    if (pLast->iNodeID == NO_NODE) {
      ...
      if (pLastNodeWaypoint) {
        ...
        if (pLastNodeWaypoint->iNodeID != NO_NODE) {
          CAI_Node *pLastNode =
              GetNetwork()->GetNode(pLastNodeWaypoint->iNodeID);
          float bestDistSq =
              (pLast->vecLocation - pLastNodeWaypoint->vecLocation).LengthSqr();
          ...
          for (int link = 0; link < pLastNode->NumLinks(); link++) {
            ...
            if (!(pLink->m_LinkInfo & bits_LINK_OFF) &&
                (pLink->m_iAcceptedMoveTypes[GetHullType()] &
                 bits_CAP_MOVE_FLY)) {
              CAI_Node *pTestNode = GetNetwork()->GetNode(
                  pLink->DestNodeID(pLastNodeWaypoint->iNodeID));
              if (pTestNode->GetHint() &&
                  pTestNode->GetHint()->HintType() == HINT_STRIDER_NODE) {
                Vector vClosest;
                CalcClosestPointOnLineSegment(
                    pLast->vecLocation, pLastNodeWaypoint->vecLocation,
                    pTestNode->GetPosition(GetHullType()), vClosest);
          ...
          if (vNewEnd == vec3_invalid) {
            ...
            delete pLast;
          } else if (!pLastNodeWaypoint->GetPrev() ||
                     pLastNodeWaypoint->GetPrev()->iNodeID !=
                         segmentDestClosest) {
            DbgNavMsg(GetOuter(),
          ...
          GetPath()->ResetGoalPosition(
              GetPath()->GetGoalWaypoint()->vecLocation);
        }
      }
    } else
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::CNavigator::ShouldOptimizeInitialPathSegment(
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_Strider::CNavigator::GetStoppingPath(
...
bool CNPC_Strider::IsLegBoneFollower(CBoneFollower *pFollower) {
  ...
  if (nFollowerIndex == -1) return false;
...
// Input  : nIndex - index our bone follower manager will use to retrieve the
// follower Output : NULL if not found, otherwise the bone follower we were
// seeking
//-----------------------------------------------------------------------------
CBoneFollower *CNPC_Strider::GetBoneFollowerByIndex(int nIndex) {
  ...
  if (pFollower != NULL) return pFollower->hFollower;
...
bool CNPC_Strider::ShouldProbeCollideAgainstEntity(CBaseEntity *pEntity) {
  if (pEntity->m_iClassname == m_iszStriderBusterName) return false;
...
float CStriderMinigun::GetAimError() {
  return fabs(m_yaw.target - m_yaw.current) +
         fabs(m_pitch.target - m_pitch.current);
}

//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::AimAtPoint(IStriderMinigunHost *pHost,
                                 const Vector &vecPoint, bool bSnap) {
  matrix3x4_t gunMatrix;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::AimAtTarget(IStriderMinigunHost *pHost,
                                  CBaseEntity *pTarget, bool bSnap) {
  if (pTarget && !(CAI_BaseNPC::m_nDebugBits & bits_debugDisableAI)) {
    Vector vecTargetPos =
        pTarget->BodyTarget(pHost->GetEntity()->EyePosition());
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::ShootAtTarget(IStriderMinigunHost *pHost,
                                    CBaseEntity *pTarget, float shootTime) {
  if (!pTarget && !(CAI_BaseNPC::m_nDebugBits & bits_debugDisableAI)) return;

  variant_t emptyVariant;
  pTarget->AcceptInput("InputTargeted", pHost->GetEntity(), pHost->GetEntity(),
                       emptyVariant, 0);
  Enable(NULL, true);
  if (shootTime <= 0) {
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::StartShooting(IStriderMinigunHost *pHost,
  ...
  if (pTarget->IsPlayer()) {
    ...
    if (!pPlayer->FInViewCone(pHost->GetEntity())) {
      ...
      if (flDot < 0.95) {
        // If the player is looking sufficiently to a side, start 30 feet out in
        // that direction.
        m_vecAnchor += pPlayer->EyeDirection3D() * 320;
        bHasSetAnchor = true;
      } else {
        // Start over their head, cause firing the direction they are looking
        // will drill them. in the back!
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::StopShootingForSeconds(IStriderMinigunHost *pHost,
                                             CBaseEntity *pTarget,
                                             float duration) {
  if (IsShooting()) {
    SetState(MINIGUN_OFF);
...
//---------------------------------------------------------
//---------------------------------------------------------
void CStriderMinigun::SetTarget(IStriderMinigunHost *pHost,
                                CBaseEntity *pTarget, bool bOverrideEnemy) {
  m_hTarget = pTarget;
...
//---------------------------------------------------------
bool CStriderMinigun::CanStartShooting(IStriderMinigunHost *pHost,
                                       CBaseEntity *pTargetEnt) {
  if (CAI_BaseNPC::m_nDebugBits & bits_debugDisableAI) return false;

  if (!pTargetEnt) return false;

  if (gpGlobals->curtime < m_burstTime) return false;

  CNPC_Strider *pStrider = dynamic_cast<CNPC_Strider *>(pHost->GetEntity());
  if (pTargetEnt->IsPlayer() && pStrider->HasPass()) return false;
  ...
  if (!m_bOverrideEnemy) {
    ...
    if (!pHost->GetEntity()->HasCondition(COND_SEE_ENEMY)) {
      ...
      if (!pStrider->WeaponLOSCondition(pStrider->GetAdjustedOrigin(),
                                        pTargetEnt->WorldSpaceCenter(),
                                        false)) {
        if (pStrider->IsUsingAggressiveBehavior() && pTargetEnt->IsPlayer() &&
            !pStrider->HasPass()) {
          // I can shoot the player's cover until he hides long enough to earn a
          // free pass.
          float flTimeSinceLastSeen =
              gpGlobals->curtime -
              pStrider->GetEnemies()->LastTimeSeen(pTargetEnt);
  ...
  Vector los =
      (pTargetEnt->WorldSpaceCenter() - pHost->GetEntity()->EyePosition());
...
void CStriderMinigun::Think(IStriderMinigunHost *pHost, float dt) {
  ...
  if (ShouldFindTarget(pHost)) {
    ...
    if (IsShooting()) {
      ...
      ExtendShooting(STRIDER_SUBSEQUENT_TARGET_DURATION +
                     random->RandomFloat(0, 0.5));
    }
  ...
  if (IsShooting()) {
    if (gpGlobals->curtime > m_burstTime || !pTargetEnt) {
      ...
      StopShootingForSeconds(pHost, pTargetEnt,
                             pHost->GetMinigunShootDowntime());
    }
  } else {
    if (CanStartShooting(pHost, pTargetEnt)) {
      StartShooting(
          pHost, pTargetEnt,
          pHost->GetMinigunShootDuration() +
              random->RandomFloat(0, pHost->GetMinigunShootVariation()));
    }
  }
  ...
  if (m_nextBulletTime <= gpGlobals->curtime && !IsPegged()) {
    if (pTargetEnt && pTargetEnt == pHost->GetEntity()->GetEnemy()) {
      ...
      // time. This guarantees that the minigun will strike the target a few
      // times.
      float flFactor =
      ...
      const Vector *pTargetPoint =
          pTargetEnt ? &pTargetEnt->GetAbsOrigin() : NULL;
      pHost->ShootMinigun(pTargetPoint, GetAimError());
    }

    m_nextBulletTime =
        gpGlobals->curtime + (1.0f / pHost->GetMinigunRateOfFire());
  }
}
...
void CSparkTrail::Spawn() {
  ...
  if (random->RandomInt(0, 1) == 0) vecVelocity.x *= -1;
