...
// This switches between the single primary weapon, and multiple weapons with
// buckets approach (jdw)
...
ConVar player_showpredictedposition_timestep(
    "player_showpredictedposition_timestep", "1.0");

ConVar player_squad_transient_commands("player_squad_transient_commands", "1",
                                       FCVAR_REPLICATED);
...
ConVar sv_stickysprint("sv_stickysprint", "0",
...
static impactentry_t cappedPlayerLinearTable[] = {
    {150 * 150, 5}, {250 * 250, 10}, {450 * 450, 20}, {550 * 550, 30},
    //{ 700*700, 100 },
    //{ 1000*1000, 500 },
};

static impactentry_t cappedPlayerAngularTable[] = {
    {100 * 100, 10}, {150 * 150, 20}, {200 * 200, 30},
    //{ 300*300, 500 },
};
...
bool Flashlight_UseLegacyVersion(void) {
  ...
  if (g_bCacheLegacyFlashlightStatus) {
    ...
    g_bUseLegacyFlashlight =
        (!Q_strcmp(modDir, "hl2") || !Q_strcmp(modDir, "episodic"));
...
//-----------------------------------------------------------------------------
// Purpose: Used to relay outputs/inputs from the player to the world and
// viceversa
//-----------------------------------------------------------------------------
class CLogicPlayerProxy : public CLogicalEntity {
 ...
 public:
  ...
  COutputEvent m_PlayerMissedAR2AltFire;  // Player fired a combine ball which
                                          // did not dissolve any enemies.
...
static ConCommand toggle_zoom("toggle_zoom", CC_ToggleZoom,
                              "Toggles zoom display");

// ConVar cl_forwardspeed( "cl_forwardspeed", "400", FCVAR_CHEAT ); // Links us
// to the client's version
ConVar xc_crouch_range("xc_crouch_range", "0.85", FCVAR_ARCHIVE,
                       "Percentarge [1..0] of joystick range to allow ducking "
                       "within");  // Only 1/2 of the range is used
ConVar xc_use_crouch_limiter("xc_use_crouch_limiter", "0", FCVAR_ARCHIVE,
...
void CC_ToggleDuck(void) {
  ...
  // Cannot be frozen
  if (pPlayer->GetFlags() & FL_FROZEN) return;
  ...
  if (xc_use_crouch_limiter.GetBool()) {
    if (pPlayer->GetToggledDuckState() == false) {
      ...
      flForwardSpeed = std::max(1.0f, flForwardSpeed);

      // Make sure we're not in the blindspot on the crouch detection
      float flStickDistPerc =
          (pPlayer->GetStickDist() / flForwardSpeed);  // Speed is the magnitude
      if (flStickDistPerc > xc_crouch_range.GetFloat()) return;
    }
  }
...
BEGIN_DATADESC(CHL2_Player)
  ...
  DEFINE_FIELD(m_nControlClass, FIELD_INTEGER), DEFINE_EMBEDDED(m_HL2Local),
      ...
      */
      ...
      DEFINE_EMBEDDED(m_LowerWeaponTimer), DEFINE_EMBEDDED(m_AutoaimTimer),

      DEFINE_INPUTFUNC(FIELD_FLOAT, "IgnoreFallDamage", InputIgnoreFallDamage),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "IgnoreFallDamageWithoutReset",
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "ForceDropPhysObjects",
                       InputForceDropPhysObjects),

      DEFINE_SOUNDPATCH(m_sndLeeches), DEFINE_SOUNDPATCH(m_sndWaterSplashes),
...
#ifdef HL2MP
CSuitPowerDevice SuitDeviceSprint(bits_SUIT_DEVICE_SPRINT,
                                  25.0f);  // 100 units in 4 seconds
#else
CSuitPowerDevice SuitDeviceSprint(bits_SUIT_DEVICE_SPRINT,
                                  12.5f);  // 100 units in 8 seconds
#endif

#ifdef HL2_EPISODIC
CSuitPowerDevice SuitDeviceFlashlight(bits_SUIT_DEVICE_FLASHLIGHT,
                                      1.111);  // 100 units in 90 second
#else
CSuitPowerDevice SuitDeviceFlashlight(bits_SUIT_DEVICE_FLASHLIGHT,
                                      2.222);  // 100 units in 45 second
#endif
CSuitPowerDevice SuitDeviceBreather(
    bits_SUIT_DEVICE_BREATHER,
    6.7f);  // 100 units in 15 seconds (plus three padded seconds)

IMPLEMENT_SERVERCLASS_ST(CHL2_Player, DT_HL2_Player)
SendPropDataTable(SENDINFO_DT(m_HL2Local), &REFERENCE_SEND_TABLE(DT_HL2Local),
...
void CHL2_Player::HandleSpeedChanges(void) {
  ...
  bool bWantSprint =
      (bCanSprint && IsSuitEquipped() && (m_nButtons & IN_SPEED));
  if (bIsSprinting != bWantSprint && (buttonsChanged & IN_SPEED)) {
    // If someone wants to sprint, make sure they've pressed the button to do
    // so. We want to prevent the case where a player can hold down the sprint
    // key and burn tiny bursts of sprint as the suit recharges We want a full
    // debounce of the key to resume sprinting after the suit is completely
    // drained
    if (bWantSprint) {
  ...
  if (IsSuitEquipped()) {
    bWantWalking =
        (m_nButtons & IN_WALK) && !IsSprinting() && !(m_nButtons & IN_DUCK);
  } else {
    bWantWalking = true;
  }
...
void CHL2_Player::HandleArmorReduction(void) {
  if (m_flArmorReductionTime < gpGlobals->curtime) return;

  if (ArmorValue() <= 0) return;

  float flPercent =
      1.0f - ((m_flArmorReductionTime - gpGlobals->curtime) / ARMOR_DECAY_TIME);
...
void CHL2_Player::PreThink(void) {
  if (player_showpredictedposition.GetBool()) {
    ...
    UTIL_PredictedPosition(
        this, player_showpredictedposition_timestep.GetFloat(), &predPos);

    NDebugOverlay::Box(predPos, NAI_Hull::Mins(GetHullType()),
                       NAI_Hull::Maxs(GetHullType()), 0, 255, 0, 0, 0.01f);
    NDebugOverlay::Line(GetAbsOrigin(), predPos, 0, 255, 0, 0, 0.01f);
  }

#ifdef HL2_EPISODIC
  if (m_hLocatorTargetEntity != NULL) {
    // Keep track of the entity here, the client will pick up the rest of the
    // work
    m_HL2Local.m_vecLocatorOrigin = m_hLocatorTargetEntity->WorldSpaceCenter();
  } else {
    m_HL2Local.m_vecLocatorOrigin =
        vec3_invalid;  // This tells the client we have no locator target.
  }
#endif  // HL2_EPISODIC
  ...
  if (IsInAVehicle()) {
    ...
    // make sure we update the client, check for timed damage and update suit
    // even if we are in a vehicle
    UpdateClientData();
  ...
  if ((GetFlags() & FL_ONGROUND) && sv_autojump.GetFloat() != 0) {
    ...
    if (flVelocity > 200) {
      ...
      UTIL_TraceHull(WorldSpaceCenter() - Vector(0, 0, 16), vecCheckDir,
                     NAI_Hull::Mins(HULL_TINY_CENTERED),
                     NAI_Hull::Maxs(HULL_TINY_CENTERED), MASK_PLAYERSOLID, this,
      ...
      if (tr.fraction == 1.0 && !tr.startsolid) {
        // Now trace down!
        UTIL_TraceLine(vecCheckDir, vecCheckDir - Vector(0, 0, 64),
  ...
  if (sv_stickysprint.GetBool() && m_bIsAutoSprinting) {
    ...
    // Stop sprinting if the player lets off the stick for a moment.
      ...
      // Stop sprinting one half second after the player stops inputting with
      // the move stick.
  ...
  if (g_fGameOver || IsPlayerLockedInPlace()) return;  // finale
  ...
  if (m_afPhysicsFlags & PFLAG_DIROVERRIDE) {
    ...
    if (!pTrain) {
      if (GetActiveWeapon() &&
          (GetActiveWeapon()->ObjectCaps() & FCAP_DIRECTIONAL_USE)) {
  ...
  if (vm && !GetActiveWeapon()) {
    ...
    if (idealSequence >= 0) {
      ...
      m_flAdmireGlovesAnimTime =
          gpGlobals->curtime + vm->SequenceDuration(idealSequence);
    }
  }
}
...
void CHL2_Player::Activate(void) {
  ...
  if (GetActiveWeapon()) {
    float flRemaining =
        GetActiveWeapon()->m_flNextPrimaryAttack - gpGlobals->curtime;

    if (flRemaining < HL2PLAYER_RELOADGAME_ATTACK_DELAY) {
      GetActiveWeapon()->m_flNextPrimaryAttack =
    ...
    if (flRemaining < HL2PLAYER_RELOADGAME_ATTACK_DELAY) {
      GetActiveWeapon()->m_flNextSecondaryAttack =
          gpGlobals->curtime + HL2PLAYER_RELOADGAME_ATTACK_DELAY;
    }
  }
...
//-----------------------------------------------------------------------------
bool CHL2_Player::HandleInteraction(int interactionType, void *data,
                                    CBaseCombatCharacter *sourceEnt) {
  if (interactionType == g_interactionBarnacleVictimDangle) return false;
...
void CHL2_Player::Spawn(void) {
  ...
  if (!IsSuitEquipped()) StartWalking();
  ...
  m_pPlayerAISquad =
      g_AI_SquadManager.FindCreateSquad(AllocPooledString(PLAYER_SQUADNAME));
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHL2_Player::InitSprinting(void) { StopSprinting(); }
...
bool CHL2_Player::CanSprint() {
  return (
      m_bSprintEnabled &&  // Only if sprint is enabled
      !IsWalking() &&      // Not if we're walking
      !(m_Local.m_bDucked && !m_Local.m_bDucking) &&  // Nor if we're ducking
      (GetWaterLevel() != 3) &&  // Certainly not underwater
      (GlobalEntity_GetState("suit_no_sprint") !=
       GLOBAL_ON));  // Out of the question without the sprint module
}
...
void CHL2_Player::StartSprinting(void) {
  ...
  if (!SuitPower_AddDevice(SuitDeviceSprint)) return;
...
bool CHL2_Player::IsZooming(void) {
  if (m_hZoomOwner != NULL) return true;
...
// Purpose:
//-----------------------------------------------------------------------------
void CHL2_Player::InitVCollision(const Vector &vecAbsOrigin,
...
bool CHL2_Player::CommanderFindGoal(commandgoal_t *pGoal) {
  ...
  //---------------------------------
  // MASK_SHOT on purpose! So that you don't hit the invisible hulls of the
  // NPCs.
  CTraceFilterSkipTwoEntities filter(this,
                                     PhysCannonGetHeldEntity(GetActiveWeapon()),
                                     COLLISION_GROUP_INTERACTIVE_DEBRIS);

  UTIL_TraceLine(EyePosition(), EyePosition() + forward * MAX_COORD_RANGE,
                 MASK_SHOT, &filter, &tr);

  if (!tr.DidHitWorld()) {
    CUtlVector<CAI_BaseNPC *> Allies;
    AISquadIter_t iter;
    for (pAllyNpc = m_pPlayerAISquad->GetFirstMember(&iter); pAllyNpc;
         pAllyNpc = m_pPlayerAISquad->GetNextMember(&iter)) {
      if (pAllyNpc->IsCommandable()) Allies.AddToTail(pAllyNpc);
    }
  ...
  if (tr.m_pEnt->IsNPC() && ((CAI_BaseNPC *)(tr.m_pEnt))->IsCommandable()) {
    ...
    // Now trace down.
    // UTIL_TraceLine( vecTarget, vecTarget - Vector( 0, 0, 8192 ), MASK_SOLID,
    // this, COLLISION_GROUP_NONE, &tr );
    UTIL_TraceHull(vecTarget + tr.plane.normal * 24,
                   vecTarget - Vector(0, 0, 8192), mins, maxs,
...
int CHL2_Player::GetNumSquadCommandables() {
  AISquadIter_t iter;
  int c = 0;
  for (CAI_BaseNPC *pAllyNpc = m_pPlayerAISquad->GetFirstMember(&iter);
       pAllyNpc; pAllyNpc = m_pPlayerAISquad->GetNextMember(&iter)) {
    if (pAllyNpc->IsCommandable()) c++;
  }
  return c;
}
...
int CHL2_Player::GetNumSquadCommandableMedics() {
  ...
  for (CAI_BaseNPC *pAllyNpc = m_pPlayerAISquad->GetFirstMember(&iter);
       pAllyNpc; pAllyNpc = m_pPlayerAISquad->GetNextMember(&iter)) {
    if (pAllyNpc->IsCommandable() && pAllyNpc->IsMedic()) c++;
  }
  return c;
}
...
void CHL2_Player::CommanderUpdate() {
  ...
  if (pCommandRepresentative) {
    ...
    NDebugOverlay::ScreenText(
            0.932, 0.919,
            CFmtStr( "%d|%c%s", GetNumSquadCommandables(), ( bFollowMode ) ? 'F'
    : 'S', pszMoving ), 255, 128, 0, 128, 0 );
    */
  ...
  if (m_QueuedCommand != CC_NONE &&
      (m_QueuedCommand == CC_FOLLOW ||
       gpGlobals->realtime - m_RealTimeLastSquadCommand >=
           player_squad_double_tap_time.GetFloat())) {
    CommanderExecute(m_QueuedCommand);
    m_QueuedCommand = CC_NONE;
  } else if (!bFollowMode && pCommandRepresentative &&
             m_CommanderUpdateTimer.Expired() &&
             player_squad_transient_commands.GetBool()) {
    m_CommanderUpdateTimer.Set(2.5);

    if (pCommandRepresentative->ShouldAutoSummon()) CommanderExecute(CC_FOLLOW);
  }
}
...
//-----------------------------------------------------------------------------
bool CHL2_Player::CommanderExecuteOne(CAI_BaseNPC *pNpc,
                                      const commandgoal_t &goal,
...
void CHL2_Player::CommanderExecute(CommanderCommand_t command) {
  ...
  if (command == CC_TOGGLE) {
    ...
    if (command == CC_FOLLOW &&
        pPlayerSquadLeader->GetCommandGoal() == vec3_invalid)
      return;
  }
...
#endif
  CAI_BaseNPC *pTargetNpc =
      (goal.m_pGoalEntity) ? goal.m_pGoalEntity->MyNPCPointer() : NULL;

  bool bHandled = false;
  if (pTargetNpc) {
    bHandled =
        !CommanderExecuteOne(pTargetNpc, goal, Allies.Base(), Allies.Count());
  }

  for (i = 0; !bHandled && i < Allies.Count(); i++) {
    if (Allies[i] != pTargetNpc && Allies[i]->IsPlayerAlly()) {
      bHandled =
          !CommanderExecuteOne(Allies[i], goal, Allies.Base(), Allies.Count());
    }
    Assert(nAIs ==
           g_AI_Manager.NumAIs());  // not coded to support mutating set of NPCs
  }
}
...
void CHL2_Player::CommanderMode() {
  ...
  if (commandInterval < player_squad_double_tap_time.GetFloat()) {
    ...
    m_QueuedCommand =
        (player_squad_transient_commands.GetBool()) ? CC_SEND : CC_TOGGLE;
  }
}
...
void CHL2_Player::CheatImpulseCommands(int iImpulse) {
  switch (iImpulse) {
    ...
    case 51: {
      ...
      CBaseEntity *pItem =
          (CBaseEntity *)CreateEntityByName("item_dynamic_resupply");
      if (pItem) {
    ...
    case 52: {
      ...
      UTIL_TraceLine(EyePosition(),
                     EyePosition() + EyeDirection3D() * MAX_COORD_RANGE,
      ...
      if (tr.fraction != 1.0) {
        ...
        DevMsg(1, "\nStartPos: %.4f %.4f %.4f --- EndPos: %.4f %.4f %.4f\n",
               tr.startpos.x, tr.startpos.y, tr.startpos.z, tr.endpos.x,
               tr.endpos.y, tr.endpos.z);
        DevMsg(1,
               "3D Distance: %.4f units  (%.2f feet) --- 2D Distance: %.4f "
               "units  (%.2f feet)\n",
...
// Purpose:
//-----------------------------------------------------------------------------
void CHL2_Player::SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs,
                                  int pvssize) {
...
void CHL2_Player::SuitPower_Update(void) {
  if (SuitPower_ShouldRecharge()) {
    ...
    // Since stickysprint quickly shuts off sprint if it isn't being used, this
    // isn't an issue.
    if (!sv_stickysprint.GetBool()) {
    ...
    if (SuitPower_IsDeviceActive(SuitDeviceFlashlight)) {
      ...
      flPowerLoad -=
          (SuitDeviceFlashlight.GetDeviceDrainRate() * (1.0f - factor));
    }
    ...
    if (Flashlight_UseLegacyVersion()) {
      // turn off flashlight a little bit after it hits below one aux power
      // notch (5%)
...
// Input:	Amount of charge to remove (expressed as percentage of full
// charge) Output:	Returns TRUE if successful, FALSE if not enough power
// available.
//-----------------------------------------------------------------------------
bool CHL2_Player::SuitPower_Drain(float flPower) {
  // Suitpower cheat on?
  if (sv_infinite_aux_power.GetBool()) return true;
...
bool CHL2_Player::SuitPower_AddDevice(const CSuitPowerDevice &device) {
  ...
  if (m_HL2Local.m_bitsActiveDevices & device.GetDeviceID()) return false;

  if (!IsSuitEquipped()) return false;
...
bool CHL2_Player::SuitPower_RemoveDevice(const CSuitPowerDevice &device) {
  ...
  // Take a little bit of suit power when you disable a device. If the device is
  // shutting off because the battery is drained, no harm done, the battery
  // charge cannot go below 0. This code in combination with the delay before
  // the suit can start recharging are a defense against exploits where the
  // player could rapidly tap sprint and never run out of power.
  ...
  if (m_HL2Local.m_bitsActiveDevices == 0x00000000) {
    // With this device turned off, we can set this timer which tells us when
    // the suit power system entered a no-load state.
...
bool CHL2_Player::SuitPower_ShouldRecharge(void) {
  ...
  if (m_HL2Local.m_bitsActiveDevices != 0x00000000) return false;

  // Is the system fully charged?
  if (m_HL2Local.m_flSuitPower >= 100.0f) return false;

  // Has the system been in a no-load state for long enough
  // to begin recharging?
  if (gpGlobals->curtime <
      m_flTimeAllSuitDevicesOff + SUITPOWER_BEGIN_RECHARGE_DELAY)
    return false;
...
bool CHL2_Player::ApplyBattery(float powerMultiplier) {
  ...
  if ((ArmorValue() < MAX_NORMAL_BATTERY) && IsSuitEquipped()) {
    ...
    IncrementArmorValue(sk_battery.GetFloat() * powerMultiplier,
                        MAX_NORMAL_BATTERY);
    ...
    pct =
        (int)((float)(ArmorValue() * 100.0) * (1.0 / MAX_NORMAL_BATTERY) + 0.5);
    pct = (pct / 5);
    if (pct > 0) pct--;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CHL2_Player::FlashlightIsOn(void) { return IsEffectActive(EF_DIMLIGHT); }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHL2_Player::FlashlightTurnOn(void) {
  if (m_bFlashlightDisabled) return;

  if (Flashlight_UseLegacyVersion()) {
    if (!SuitPower_AddDevice(SuitDeviceFlashlight)) return;
  }
#ifdef HL2_DLL
...
void CHL2_Player::FlashlightTurnOff(void) {
  if (Flashlight_UseLegacyVersion()) {
    if (!SuitPower_RemoveDevice(SuitDeviceFlashlight)) return;
  }
...
bool CHL2_Player::IsIlluminatedByFlashlight(CBaseEntity *pEntity,
                                            float *flReturnDot) {
  if (!FlashlightIsOn()) return false;

  if (pEntity->Classify() == CLASS_BARNACLE && pEntity->GetEnemy() == this) {
    // As long as my flashlight is on, the barnacle that's pulling me in is
    // considered illuminated. This is because players often shine their
    // flashlights at Alyx when they are in a barnacle's grasp, and wonder why
    // Alyx isn't helping. Alyx isn't helping because the light isn't pointed at
    // the barnacle. This will allow Alyx to see the barnacle no matter which
    // way the light is pointed.
    return true;
  }

  // Within 50 feet?
  float flDistSqr = GetAbsOrigin().DistToSqr(pEntity->GetAbsOrigin());
  if (flDistSqr > FLASHLIGHT_RANGE) return false;
  ...
  if (flDot < 0.92387f) return false;

  if (!FVisible(pEntity)) return false;
...
void CHL2_Player::CheckFlashlight(void) {
  if (!FlashlightIsOn()) return;

  if (m_flNextFlashlightCheckTime > gpGlobals->curtime) return;
  m_flNextFlashlightCheckTime =
      gpGlobals->curtime + FLASHLIGHT_NPC_CHECK_INTERVAL;
...
bool CHL2_Player::PassesDamageFilter(const CTakeDamageInfo &info) {
  ...
  if (pAttacker && pAttacker->MyNPCPointer() &&
      pAttacker->MyNPCPointer()->IsPlayerAlly()) {
    return false;
  }
...
void CHL2_Player::InputIgnoreFallDamage(inputdata_t &inputdata) {
  ...
  if (timeToIgnore <= 0.0) timeToIgnore = TIME_IGNORE_FALL_DAMAGE;
...
//-----------------------------------------------------------------------------
// Purpose: Absolutely prevent the player from taking fall damage for [n]
// seconds.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Notification of a player's npc ally in the players squad being
// killed
//-----------------------------------------------------------------------------
...
int CHL2_Player::OnTakeDamage(const CTakeDamageInfo &info) {
  if (GlobalEntity_GetState("gordon_invulnerable") == GLOBAL_ON) return 0;

  // ignore fall damage if instructed to do so by input
  if ((info.GetDamageType() & DMG_FALL) &&
      m_flTimeIgnoreFallDamage > gpGlobals->curtime) {
    // usually, we will reset the input flag after the first impact. However
    // there is another input that prevents this behavior.
  ...
  if (info.GetDamage() > 0.0f) {
    ...
    if (info.GetAttacker()) NotifyFriendsOfDamage(info.GetAttacker());
  }
  ...
  if (info.GetDamageType() == DMG_GENERIC && info.GetAttacker() == this &&
      info.GetInflictor() == this) {
    // Only do a skill level adjustment if the player isn't his own attacker AND
    // inflictor. This prevents damage from SetHealth() inputs from being
    // adjusted for skill level.
    bAdjustForSkillLevel = false;
  }

  if (GetVehicleEntity() != NULL &&
      GlobalEntity_GetState("gordon_protect_driver") == GLOBAL_ON) {
    if (playerDamage.GetDamage() > test_massive_dmg.GetFloat() &&
        playerDamage.GetInflictor() == GetVehicleEntity() &&
        (playerDamage.GetDamageType() & DMG_CRUSH)) {
      playerDamage.ScaleDamage(test_massive_dmg_clip.GetFloat() /
...
int CHL2_Player::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if ((info.GetDamageType() & DMG_SLASH) && hl2_episodic.GetBool()) {
    if (m_afPhysicsFlags & PFLAG_USING) {
      // Stop the player using a rotating button for a short time if hit by a
      // creature's melee attack. This is for the antlion burrow-corking
      // training in EP1 (sjb).
      SuspendUse(0.5f);
    }
  }
...
void CHL2_Player::OnDamagedByExplosion(const CTakeDamageInfo &info) {
  if (info.GetInflictor() && info.GetInflictor()->ClassMatches("mortarshell")) {
    ...
    UTIL_ScreenShake(info.GetInflictor()->GetAbsOrigin(), 4.0, 1.0, 0.5, 1000,
                     SHAKE_START, false);
    return;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Notifies Alyx that player has put a combine ball into a socket so
// she can comment on it. Input  : pCombineBall - ball the was socketed
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHL2_Player::Event_KilledOther(CBaseEntity *pVictim,
  ...
  if (IsX360()) {
    if (IsInAVehicle()) {
      if (m_hLockedAutoAimEntity && m_hLockedAutoAimEntity->IsAlive() &&
          ShouldKeepLockedAutoaimTarget(m_hLockedAutoAimEntity)) {
        ...
        params.m_hAutoAimEntity = m_hLockedAutoAimEntity;
        Vector vecTarget =
            m_hLockedAutoAimEntity->BodyTarget(EyePosition(), false);
    ...
    // If the player manually gets his crosshair onto a target, make that target
    // sticky
...
bool CHL2_Player::ShouldKeepLockedAutoaimTarget(EHANDLE hLockedTarget) {
  ...
  if (flDist > autoaim_max_dist.GetFloat()) return false;
  ...
  if (flDot < autoaim_unlock_target.GetFloat()) return false;
...
int CHL2_Player::GiveAmmo(int nCount, int nAmmoIndex, bool bSuppressSound) {
  ...
  if (bCheckAutoSwitch) {
    CBaseCombatWeapon *pWeapon =
        g_pGameRules->GetNextBestWeapon(this, GetActiveWeapon());
...
bool CHL2_Player::Weapon_CanUse(CBaseCombatWeapon *pWeapon) {
  ...
  if (pWeapon->ClassMatches("weapon_stunstick")) {
    if (ApplyBattery(0.5)) UTIL_Remove(pWeapon);
    return false;
  }
#endif
...
bool CHL2_Player::BumpWeapon(CBaseCombatWeapon *pWeapon) {
  ...
  if (Weapon_OwnsThisType(pWeapon->GetClassname(), pWeapon->GetSubType())) {
    ...
    if (Weapon_EquipAmmoOnly(pWeapon) == false) return false;

    // Only remove me if I have no ammo left
    // Can't just check HasAnyAmmo because if I don't use clips, I want to be
    // removed,
  ...
  // -------------------------
    ...
    if (Weapon_SlotOccupied(pWeapon)) {
      ...
      if (pActiveWeapon != NULL && pActiveWeapon->HasAnyAmmo() == false &&
          Weapon_CanSwitchTo(pWeapon)) {
...
void CHL2_Player::PlayerUse(void) {
  ...
  if (m_afButtonPressed & IN_USE) {
    ...
    if (ClearUseEntity()) {
      ...
      if (m_afPhysicsFlags & PFLAG_DIROVERRIDE) {
        ...
        CBaseEntity *pTrain = GetGroundEntity();
        if (pTrain && !(m_nButtons & IN_JUMP) && (GetFlags() & FL_ONGROUND) &&
  ...
  if (pUseEntity) {
    ...
    if (m_afButtonPressed & IN_USE) {
      // Robin: Don't play sounds for NPCs, because NPCs will allow respond with
      // speech.
    ...
    if (((m_nButtons & IN_USE) && (caps & FCAP_CONTINUOUS_USE)) ||
      ...
      if (caps & FCAP_CONTINUOUS_USE) m_afPhysicsFlags |= PFLAG_USING;

      pUseEntity->AcceptInput("Use", this, this, emptyVariant, USE_TOGGLE);

      usedSomething = true;
    }
    // UNDONE: Send different USE codes for ON/OFF.  Cache last ONOFF_USE object
    // to send 'off' if you turn away
    ...
    if (m_afButtonPressed & IN_USE) {
      CBaseCombatWeapon *pWeapon =
          dynamic_cast<CBaseCombatWeapon *>(pUseEntity);
    ...
    // Signal that we want to play the deny sound, unless the user is +USEing on
    // a ladder! The sound is emitted in ItemPostFrame, since that occurs after
    // GameMovement::ProcessMove which lets the ladder code unset this flag.
...
void CHL2_Player::UpdateWeaponPosture(void) {
  CBaseCombatWeapon *pWeapon =
      dynamic_cast<CBaseCombatWeapon *>(GetActiveWeapon());
  ...
  if (pWeapon && m_LowerWeaponTimer.Expired() && pWeapon->CanLower()) {
    ...
    trace_t tr;
    UTIL_TraceLine(EyePosition(), EyePosition() + vecAim * CHECK_FRIENDLY_RANGE,
                   MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);
    ...
    if (aimTarget && !tr.DidHitWorld()) {
      if (!aimTarget->IsNPC() ||
          aimTarget->MyNPCPointer()->GetState() != NPC_STATE_COMBAT) {
        ...
        if (sv_show_crosshair_target.GetBool()) {
          ...
          // Draw the text
          NDebugOverlay::EntityText(aimTarget->entindex(), text_offset, tempstr,
                                    0);
        }
  ...
  if (g_pGameRules->GetAutoAimMode() != AUTOAIM_NONE) {
    ...
    if (!m_AutoaimTimer.Expired()) return;

    m_AutoaimTimer.Set(.1);

    VPROF("hl2_x360_aiming");

    // Call the autoaim code to update the local player data, which allows the
    // client to update.
    ...
    m_HL2Local.m_bAutoAimTarget =
        (params.m_bAutoAimAssisting || params.m_bOnTargetNatural);
    return;
  } else {
    // Make sure there's no residual autoaim target if the user changes the
    // xbox_aiming convar on the fly.
...
bool CHL2_Player::Weapon_Lower(void) {
  ...
  // Already lowered?
  if (m_HL2Local.m_bWeaponLowered) return true;
...
bool CHL2_Player::Weapon_Ready(void) {
  ...
  // Already ready?
  if (m_HL2Local.m_bWeaponLowered == false) return true;
...
#endif
  if (pVehicle && !pPlayer->UsingStandardWeaponsInVehicle()) return false;
...
void CHL2_Player::PickupObject(CBaseEntity *pObject, bool bLimitMassAndSize) {
  ...
  if (bLimitMassAndSize == true) {
    if (CBasePlayer::CanPickupObject(pObject, 35, 128) == false) return;
  }
...
//-----------------------------------------------------------------------------
void CHL2_Player::ForceDropOfCarriedPhysObjects(
    CBaseEntity *pOnlyIfHoldingThis) {
  if (PhysIsInCallback()) {
    variant_t value;
    g_EventQueue.AddEvent(this, "ForceDropPhysObjects", value, 0.01f,
                          pOnlyIfHoldingThis, this);
    return;
  }
  ...
  // Then force the physcannon to drop anything it's holding, if it's our active
  // weapon
...
void CHL2_Player::UpdateClientData(void) {
  if (m_DmgTake || m_DmgSave || m_bitsHUDDamage != m_bitsDamageType) {
    ...
    m_DmgTake = std::clamp(m_DmgTake, 0.0f, 255.0f);
    m_DmgSave = std::clamp(m_DmgSave, 0.0f, 255.0f);
    ...
    // recovering from a poison bite would register as poisonous as well and
    // flash the whole screen! -- jdw
    ...
    WRITE_FLOAT(
        damageOrigin.x);  // BUG: Should be fixed point (to hud) not floats
    WRITE_FLOAT(damageOrigin.y);  // BUG: However, the HUD does _not_ implement
                                  // bitfield messages (yet)
    WRITE_FLOAT(
        damageOrigin.z);  // BUG: We use WRITE_VEC3COORD for everything else
    MessageEnd();
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CHL2_Player::Weapon_Switch(CBaseCombatWeapon *pWeapon,
                                int viewmodelindex) {
  MDLCACHE_CRITICAL_SECTION();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CHL2_Player::CalcWeaponProficiency(
    CBaseCombatWeapon *pWeapon) {
  ...
  if (weapon_showproficiency.GetBool() != 0) {
    Msg("Player switched to %s, proficiency is %s\n", pWeapon->GetClassname(),
        GetWeaponProficiencyName(proficiency));
  }
...
bool LineCircleIntersection(const Vector2D &center, const float radius,
  ...
  // If (b^2 - 4ac) is = 0 there is one solution (a case this function doesn't
  // support). If (b^2 - 4ac) is > 0 there are two solutions.
  Vector2D P;
  ...
  insideSqr = b * b - 4 * a * c;
  if (insideSqr <= 0.000001f) return false;
...
bool IntersectRayWithAACylinder(const Ray_t &ray, const Vector &center,
  ...
  if (!LineCircleIntersection(vec3_origin.AsVector2D(), radius,
                              vStart.AsVector2D(), ray.m_Delta.AsVector2D(),
                              &flCircleEnterFrac, &flCircleLeaveFrac)) {
    return false;  // no hit
  }
  ...
  if (flEnterFrac < flCircleEnterFrac) flEnterFrac = flCircleEnterFrac;
  if (flLeaveFrac > flCircleLeaveFrac) flLeaveFrac = flCircleLeaveFrac;
  ...
  // Calculate the point on our center line where we're nearest the intersection
  // point
  Vector collisionCenter;
  CalcClosestPointOnLineSegment(
      pTrace->endpos, center + Vector(0, 0, halfHeight),
      center - Vector(0, 0, halfHeight), collisionCenter);

  // Our normal is the direction from that center point to the intersection
  // point
...
bool CHL2_Player::TestHitboxes(const Ray_t &ray, unsigned int fContentsMask,
                               trace_t &tr) {
  if (g_pGameRules->IsMultiplayer()) {
    ...
    if (IntersectRayWithAACylinder(ray, WorldSpaceCenter(),
...
void CHL2_Player::ExitLadder() {
  if (MOVETYPE_LADDER != GetMoveType()) return;
...
//-----------------------------------------------------------------------------
void CHL2_Player::PlayUseDenySound() { m_bPlayUseDenySound = true; }
...
void CHL2_Player::StartWaterDeathSounds(void) {
  ...
  if (m_sndLeeches == NULL) {
    m_sndLeeches = (CSoundEnvelopeController::GetController())
                       .SoundCreate(filter, entindex(), CHAN_STATIC,
                                    "coast.leech_bites_loop", ATTN_NORM);
  }
  ...
  if (m_sndWaterSplashes == NULL) {
    m_sndWaterSplashes =
        (CSoundEnvelopeController::GetController())
            .SoundCreate(filter, entindex(), CHAN_STATIC,
                         "coast.leech_water_churn_loop", ATTN_NORM);
  }

  if (m_sndWaterSplashes) {
    (CSoundEnvelopeController::GetController())
        .Play(m_sndWaterSplashes, 1.0f, 100);
  }
}
...
void CHL2_Player::StopWaterDeathSounds(void) {
  if (m_sndLeeches) {
    (CSoundEnvelopeController::GetController())
        .SoundFadeOut(m_sndLeeches, 0.5f, true);
    m_sndLeeches = NULL;
  }

  if (m_sndWaterSplashes) {
    (CSoundEnvelopeController::GetController())
        .SoundFadeOut(m_sndWaterSplashes, 0.5f, true);
...
void CHL2_Player::StopLoopingSounds(void) {
  ...
  if (m_sndWaterSplashes != NULL) {
    (CSoundEnvelopeController::GetController())
        .SoundDestroy(m_sndWaterSplashes);
    m_sndWaterSplashes = NULL;
  }
...
const impactdamagetable_t &CHL2_Player::GetPhysicsImpactDamageTable() {
  if (m_bUseCappedPhysicsDamageTable) return gCappedPlayerImpactDamageTable;
...
CLogicPlayerProxy *CHL2_Player::GetPlayerProxy(void) {
  CLogicPlayerProxy *pProxy =
      dynamic_cast<CLogicPlayerProxy *>(m_hPlayerProxy.Get());

  if (pProxy == NULL) {
    pProxy = (CLogicPlayerProxy *)gEntList.FindEntityByClassname(
        NULL, "logic_playerproxy");
...
void CHL2_Player::FirePlayerProxyOutput(const char *pszOutputName,
                                        variant_t variant,
                                        CBaseEntity *pActivator,
                                        CBaseEntity *pCaller) {
  if (GetPlayerProxy() == NULL) return;

  GetPlayerProxy()->FireNamedOutput(pszOutputName, variant, pActivator,
                                    pCaller);
}
...
BEGIN_DATADESC(CLogicPlayerProxy)
  DEFINE_OUTPUT(m_OnFlashlightOn, "OnFlashlightOn"),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "RequestPlayerHealth",
                       InputRequestPlayerHealth),
      DEFINE_INPUTFUNC(FIELD_VOID, "SetFlashlightSlowDrain",
                       InputSetFlashlightSlowDrain),
      DEFINE_INPUTFUNC(FIELD_VOID, "SetFlashlightNormalDrain",
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableCappedPhysicsDamage",
                       InputEnableCappedPhysicsDamage),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableCappedPhysicsDamage",
                       InputDisableCappedPhysicsDamage),
      DEFINE_INPUTFUNC(FIELD_STRING, "SetLocatorTargetEntity",
...
void CLogicPlayerProxy::InputRequestPlayerHealth(inputdata_t &inputdata) {
  ...
  m_RequestedPlayerHealth.Set(m_hPlayer->GetHealth(), inputdata.pActivator,
                              inputdata.pCaller);
}
...
void CLogicPlayerProxy::InputSetFlashlightSlowDrain(inputdata_t &inputdata) {
  ...
  if (pPlayer)
    pPlayer->SetFlashlightPowerDrainScale(
        hl2_darkness_flashlight_factor.GetFloat());
}
...
void CLogicPlayerProxy::InputSetFlashlightNormalDrain(inputdata_t &inputdata) {
  ...
  if (pPlayer) pPlayer->SetFlashlightPowerDrainScale(1.0f);
}
  ...
  for (int i = 0; i < pPlayer->WeaponCount(); ++i) {
    ...
    if (pCheck) {
      if (pCheck->HasAnyAmmo() &&
          (pCheck->UsesPrimaryAmmo() || pCheck->UsesSecondaryAmmo())) {
...
void CLogicPlayerProxy::InputDisableCappedPhysicsDamage(
