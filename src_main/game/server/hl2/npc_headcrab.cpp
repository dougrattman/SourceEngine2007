...
enum {
  ...
  SCHED_HEADCRAB_HOP_RANDOMLY,  // get off something you're not supposed to be
                                // on.
...
BEGIN_DATADESC(CBaseHeadcrab)
  ...
  DEFINE_FIELD(m_bHidden, FIELD_BOOLEAN),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StartHangingFromCeiling",
                       InputStartHangingFromCeiling),
      DEFINE_INPUTFUNC(FIELD_VOID, "DropFromCeiling", InputDropFromCeiling),

      // Function Pointers
      DEFINE_THINKFUNC(EliminateRollAndPitch), DEFINE_THINKFUNC(ThrowThink),
...
//-----------------------------------------------------------------------------
// Because the AI code does a tracehull to find the ground under an NPC,
// headcrabs can often be seen standing with one edge of their box perched on a
// ledge and 80% or more of their body hanging out over nothing. This is often a
// case where a headcrab will be unable to pathfind out of its location. This
// heuristic very crudely tries to determine if this is the case by casting a
// simple ray down from the center of the headcrab.
//-----------------------------------------------------------------------------
...
bool CBaseHeadcrab::IsFirmlyOnGround() {
  ...
  UTIL_TraceLine(GetAbsOrigin(),
                 GetAbsOrigin() - Vector(0, 0, HEADCRAB_MAX_LEDGE_HEIGHT),
...
void CBaseHeadcrab::JumpToBurrowHint(CAI_Hint *pHint) {
  Vector vecVel = VecCheckToss(this, GetAbsOrigin(), pHint->GetAbsOrigin(),
                               0.5f, 1.0f, false, NULL, NULL);

  // Undershoot by a little because it looks bad if we overshoot and turn around
  // to burrow.
  vecVel *= 0.9f;
  Leap(vecVel);
...
void CBaseHeadcrab::Leap(const Vector &vecVel) {
  ...
  m_flIgnoreWorldCollisionTime =
      gpGlobals->curtime + HEADCRAB_IGNORE_WORLD_COLLISION_TIME;
...
void CBaseHeadcrab::JumpAttack(bool bRandomJump, const Vector &vecPos,
  ...
  if (!bRandomJump) {
    ...
    // How fast does the headcrab need to travel to reach the position given
    // gravity?
    ...
    // NOTE: This calculation jumps at a position INSIDE the box of the enemy
    // (player) so if you make the additional height too high, the crab can land
    // on top of the enemy's head.  If we want to jump high, we'll need to move
    // vecPos to the surface/outside of the enemy's box.
  ...
  switch (pTask->iTask) {
    ...
    case TASK_HEADCRAB_WAIT_FOR_BARNACLE_KILL:
      if (m_flNextFlinchTime < gpGlobals->curtime) {
        m_flNextFlinchTime =
            gpGlobals->curtime + random->RandomFloat(1.0f, 2.0f);
    ...
    case TASK_HEADCRAB_HOP_OFF_NPC:
      if (GetFlags() & FL_ONGROUND) {
        ...
        GetMotor()->SetIdealYawToTargetAndUpdate(GetAbsOrigin() +
                                                 GetAbsVelocity());
      }
      break;
    ...
    case TASK_HEADCRAB_CEILING_FALL: {
      ...
      UTIL_TraceHull(vecPrPos, vecPrPos, GetHullMins(), GetHullMaxs(),
    ...
    case TASK_HEADCRAB_CEILING_LAND: {
      if (IsActivityFinished()) {
        RemoveSolidFlags(
            FSOLID_NOT_SOLID);  // double-dog verify that we're solid.
        TaskComplete();
        m_bHangingFromCeiling = false;
      }
    } break;
...
bool CBaseHeadcrab::HasHeadroom() {
  trace_t tr;
  UTIL_TraceEntity(this, GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 1),
                   MASK_NPCSOLID, this, GetCollisionGroup(), &tr);
...
//-----------------------------------------------------------------------------
// Purpose: LeapTouch - this is the headcrab's touch function when it is in the
// air. Input  : *pOther -
//-----------------------------------------------------------------------------
  ...
  if (IRelationType(pOther) == D_HT) {
    ...
    if (!(GetFlags() & FL_ONGROUND)) {
      if (pOther->m_takedamage != DAMAGE_NO) {
        ...
        // attack succeeded, so don't delay our next attack if we previously
        // thought we failed
        m_bAttackFailed = false;
      } else {
        ImpactSound();
    ...
    // just ran into something solid, so the attack probably failed.  make a
    // note of it so that when the attack is done, we'll delay attacking for a
    // while so we don't just repeatedly leap at the enemy from a bad location.
    ...
    if (gpGlobals->curtime < m_flIgnoreWorldCollisionTime) {
      ...
      // this touch function, which can cause them to hit the player and not
      // bite. A timer probably isn't the best way to fix this, but it's one of
      // our safer options at this point (sjb).
      return;
    }
  }
...
void CBaseHeadcrab::GatherConditions(void) {
  ...
  if (m_bHidden) {
    ...
    trace_t tr;
    AI_TraceHull(GetAbsOrigin(), GetAbsOrigin(), GetHullMins(), GetHullMaxs(),
  ...
  if ((GetFlags() & FL_ONGROUND) && ground && !ground->IsWorld()) {
    if (IsHangingFromCeiling() == false) {
      ...
      } else if (ground->VPhysicsGetObject() &&
                 (ground->VPhysicsGetObject()->GetGameFlags() &
...
void CBaseHeadcrab::PrescheduleThink(void) {
  ...
  if (m_bBurrowed && (eActivity != ACT_HEADCRAB_BURROW_IDLE) &&
...
void CBaseHeadcrab::EliminateRollAndPitch() {
  ...
  if ((angles.x == 0.0f) && (angles.z == 0.0f)) return;
  ...
  SetContextThink(&CBaseHeadcrab::EliminateRollAndPitch,
                  gpGlobals->curtime + TICK_INTERVAL, s_pPitchContext);
}
...
void CBaseHeadcrab::BeginClimbFromCanister() {
  ...
  CAI_Hint *pHint = CAI_HintManager::FindHint(
      this, HINT_HEADCRAB_BURROW_POINT, 0, HEADCRAB_BURROW_POINT_SEARCH_RADIUS,
      &vecSearchCenter);

  if (!pHint && hl2_episodic.GetBool()) {
    // Look for exit points within 10 feet.
    pHint = CAI_HintManager::FindHint(this, HINT_HEADCRAB_EXIT_POD_POINT, 0,
                                      120.0f, &vecSearchCenter);
  }
  ...
  if (pHint && (!pHint->IsLocked())) {
    ...
    if (flAngle >= 0.707f) {
      ...
      // All we care about is the sign of the z component. If it's +, the hint
      // is on the left. If it's -, then the hint is on the right.
...
void CBaseHeadcrab::JumpFromCanister() {
  ...
  if (pEnemy) {
    ...
    if ((flDist < HEADCRAB_ATTACK_PLAYER_FROM_CANISTER_DIST) &&
        (DotProduct(vecDirToEnemy, vecForward) >=
         HEADCRAB_ATTACK_PLAYER_FROM_CANISTER_COSANGLE)) {
      GrabHintNode(NULL);
...
#ifdef HL2_EPISODIC
  if (IsHangingFromCeiling()) return false;
#endif
...
void CBaseHeadcrab::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_HEADCRAB_CEILING_POSITION: {
      trace_t tr;
      UTIL_TraceHull(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 512),
                     NAI_Hull::Mins(GetHullType()),
                     NAI_Hull::Maxs(GetHullType()), MASK_SOLID_BRUSHONLY, this,
    ...
    case TASK_HEADCRAB_HOP_OFF_NPC: {
      CBaseEntity *ground = GetGroundEntity();
      if (ground) {
        // If jumping off of a physics object that the player is holding, create
        // a solver to prevent the headcrab from colliding with that object for
        // a short time.
        if (ground && ground->VPhysicsGetObject()) {
          if (ground->VPhysicsGetObject()->GetGameFlags() &
              FVPHYSICS_PLAYER_HELD) {
        ...
        // Jump in some random direction. This way if the person I'm standing on
        // is against a wall, I'll eventually get away.
    ...
    case TASK_RANGE_ATTACK1: {
      ...
      EmitSound(filter, entindex(), CHAN_WEAPON, pAttackSounds[0],
                GetSoundVolume(), ATTN_IDLE, 0, GetVoicePitch());
#endif
      SetIdealActivity(ACT_RANGE_ATTACK1);
      break;
    }
    ...
    case TASK_HEADCRAB_FIND_BURROW_IN_POINT: {
      if (FindBurrow(GetAbsOrigin(), pTask->flTaskData, true) == false) {
        TaskFail(
            "TASK_HEADCRAB_FIND_BURROW_IN_POINT: Unable to find burrow in "
            "position\n");
      } else {
        TaskComplete();
      }
...
int CBaseHeadcrab::RangeAttack1Conditions(float flDot, float flDist) {
  if (gpGlobals->curtime < m_flNextAttack) return 0;

  if ((GetFlags() & FL_ONGROUND) == false) return 0;

  // When we're burrowed ignore facing, because when we unburrow we'll cheat and
  // face our enemy.
  ...
  if (pEnemy) {
    ...
    trace_t tr;
    AI_TraceLine(EyePosition(), pEnemy->EyePosition(), MASK_SOLID, this,
                 COLLISION_GROUP_NONE, &tr);

    if (tr.m_pEnt != GetEnemy()) {
      if (!bEnemyIsBullseye || tr.m_pEnt != NULL) return COND_NONE;
    }

    if (GetEnemy()->EyePosition().z - 36.0f > GetAbsOrigin().z) {
      // Only run this test if trying to jump at a player who is higher up than
      // me, else this code will always prevent a headcrab from jumping down at
      // an enemy, and sometimes prevent it jumping just slightly up at an
      // enemy.
      ...
      AI_TraceHull(vStartHullTrace, vEndHullTrace, GetHullMins(), GetHullMaxs(),
                   MASK_NPCSOLID, this, GetCollisionGroup(), &tr);
  ...
  if (m_NPCState == NPC_STATE_DEAD) {
    if (GetAbsVelocity().Length() > 250) {
      ...
      if ((tr.fraction != 1.0) && (dotPr < -0.8)) {
        CTakeDamageInfo info(GetWorldEntity(), GetWorldEntity(), 100.0f,
                             DMG_CRUSH);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseHeadcrab::ClampRagdollForce(const Vector &vecForceIn,
                                      Vector *vecForceOut) {
...
void CBaseHeadcrab::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & (DMG_GENERIC | DMG_PREVENT_PHYSICS_FORCE)) {
    ...
    AI_TraceLine(GetAbsOrigin() + Vector(0, 0, 1),
                 GetAbsOrigin() - Vector(0, 0, 64), MASK_SOLID_BRUSHONLY, this,
                 COLLISION_GROUP_NONE, &tr);
...
int CBaseHeadcrab::SelectSchedule(void) {
  ...
  if (IsHangingFromCeiling()) {
    if (HL2GameRules()->IsAlyxInDarknessMode() == false &&
        (HasCondition(COND_CAN_RANGE_ATTACK1) || HasCondition(COND_NEW_ENEMY)))
  ...
  if (m_bBurrowed) {
    if (HasCondition(COND_CAN_RANGE_ATTACK1)) return SCHED_HEADCRAB_BURROW_OUT;
    ...
    case NPC_STATE_ALERT: {
      if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE)) {
        if (fabs(GetMotor()->DeltaIdealYaw()) <
            (1.0 - m_flFieldOfView) * 60)  // roughly in the correct direction
        {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CBaseHeadcrab::SelectFailSchedule(int failedSchedule, int failedTask,
                                      AI_TaskFailureCode_t taskFailCode) {
  if (failedSchedule == SCHED_BACK_AWAY_FROM_ENEMY &&
      failedTask == TASK_FIND_BACKAWAY_FROM_SAVEPOSITION) {
  ...
  if (failedSchedule == SCHED_BACK_AWAY_FROM_ENEMY ||
      failedSchedule == SCHED_PATROL_WALK ||
      failedSchedule == SCHED_COMBAT_PATROL) {
    if (!IsFirmlyOnGround()) {
      return SCHED_HEADCRAB_HOP_RANDOMLY;
    }
  }
...
//			*ptr -
//-----------------------------------------------------------------------------
void CBaseHeadcrab::TraceAttack(const CTakeDamageInfo &info,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CBaseHeadcrab::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
...
//-----------------------------------------------------------------------------
bool CBaseHeadcrab::HandleInteraction(int interactionType, void *data,
  ...
  if (interactionType == g_interactionBarnacleVictimDangle) {
    ...
    OnTakeDamage(CTakeDamageInfo(sourceEnt, sourceEnt, m_iHealth,
                                 DMG_CRUSH | DMG_ALWAYSGIB));

    // Create dead headcrab in its place
    CBaseHeadcrab *pEntity =
        (CBaseHeadcrab *)CreateEntityByName("npc_headcrab");
    pEntity->Spawn();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseHeadcrab::FindBurrow(const Vector &origin, float distance,
                               bool excludeNear) {
...
Activity CHeadcrab::NPC_TranslateActivity(Activity eNewActivity) {
  if (eNewActivity == ACT_WALK) return ACT_RUN;
    ...
    case TASK_RANGE_ATTACK2:
      ...
      if (GetEnemy())
        ...
        GetMotor()->SetIdealYawAndUpdate(
            GetEnemy()->GetAbsOrigin() - GetAbsOrigin(), AI_KEEP_YAW_SPEED);
...
void CFastHeadcrab::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_HEADCRAB_HOP_ASIDE: {
      ...
      // This could be a problem. Since I'm adjusting the headcrab's gravity for
      // flight, this check actually checks farther ahead than the crab will
      // actually jump. (sjb)
      AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + vecDir, GetHullMins(),
                   GetHullMaxs(), MASK_SHOT, this, GetCollisionGroup(), &tr);
      ...
      if (tr.fraction == 1.0) {
        ...
        if (!IsMoveBlocked(moveTrace)) {
          SetAbsVelocity(
              m_vecJumpVel);  // + 0.5f * Vector(0,0,sv_gravity.GetFloat()) *
                              // flInterval;
          ...
          if (fJumpIsLeft) {
            ...
            GetNavigator()->SetMovementActivity(
                (Activity)ACT_HEADCRAB_HOP_LEFT);
          } else {
            SetIdealActivity((Activity)ACT_HEADCRAB_HOP_RIGHT);
            GetNavigator()->SetMovementActivity(
                (Activity)ACT_HEADCRAB_HOP_RIGHT);
          }
        } else {
...
void CBlackHeadcrab::BuildScheduleTestBits(void) {
  ...
  if (IsCurSchedule(SCHED_HEADCRAB_CRAWL_FROM_CANISTER) ||
  ...
  // If we're committed to jump, carry on even if our enemy hides behind a
  // crate. Or a barrel.
  ...
  if (!IsHangingFromCeiling()) {
    ...
    if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE)) {
      if ((gpGlobals->curtime >= m_flNextHopTime) &&
          SelectWeightedSequence(ACT_SMALL_FLINCH) != -1) {
  ...
  if (pOther->m_iHealth > 1) {
    ...
    if (pOther->IsAlive() && pOther->m_iHealth > 1) {
      ...
      if (hl2_episodic.GetBool()) {
        if (pOther->IsPlayer()) {
          // That didn't finish them. Take them down to one point with poison
          // damage. It'll heal.
          pOther->TakeDamage(
              CTakeDamageInfo(this, this, pOther->m_iHealth - 1, DMG_POISON));
        } else {
          // Just take some amount of slash damage instead
          pOther->TakeDamage(CTakeDamageInfo(
              this, this, sk_headcrab_poison_npc_damage.GetFloat(), DMG_SLASH));
        }
      } else {
        // That didn't finish them. Take them down to one point with poison
        // damage. It'll heal.
...
void CBlackHeadcrab::Eject(const QAngle &vecAngles, float flVelocityScale,
  ...
  SetAbsVelocity(flVelocityScale * random->RandomInt(20, 50) *
                 Vector(random->RandomFloat(-1.0, 1.0),
                        random->RandomFloat(-1.0, 1.0),
...
bool CBlackHeadcrab::FInViewCone(CBaseEntity *pEntity) {
  if (IsCurSchedule(SCHED_HEADCRAB_AMBUSH) &&
      ((pEntity->IsNPC() || pEntity->IsPlayer()) &&
       pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
           CRAB_360_VIEW_DIST_SQR)) {
    // Only see players and NPC's with 360 cone
    // For instance, DON'T tell the eyeball/head tracking code that you can see
    // an object that is behind you!
    return true;
  } else {
...
void CBlackHeadcrab::JumpFlinch(const Vector *pvecDir) {
  ...
  if (pvecDir) {
    SetAbsVelocity(Vector(pvecDir->x * 4, pvecDir->y * 4, up.z) *
                   random->RandomFloat(40, 80));
  } else {
    SetAbsVelocity(
        Vector(random->RandomFloat(-4, 4), random->RandomFloat(-4, 4), up.z) *
        random->RandomFloat(40, 80));
  }
}
...
void CBlackHeadcrab::HandleAnimEvent(animevent_t *pEvent) {
  if (pEvent->event == AE_POISONHEADCRAB_FOOTSTEP) {
    bool walk =
        (GetActivity() == ACT_WALK);  // ? 1.0 : 0.6; !!cgreen! old code had bug
  ...
  if (pEvent->event == AE_POISONHEADCRAB_FLINCH_HOP) {
    ...
    // to hide, jump forward -- hopefully that will take us closer to a hiding
    // spot.
    //
    if (m_bPanicState) {
      Vector vecForward;
...
bool CBlackHeadcrab::IsHeavyDamage(const CTakeDamageInfo &info) {
  if (!HasMemory(bits_MEMORY_FLINCHED) && info.GetDamage() > 1.0f) {
    // If I haven't flinched lately, any amount of damage is interpreted as
    // heavy.
    return true;
  }
...
DEFINE_SCHEDULE(
    ...
    "ACTIVITY:ACT_HEADCRAB_THREAT_DISPLAY"
    ""
    "	Interrupts")
...
DEFINE_SCHEDULE(SCHED_HEADCRAB_DROWN,
                ...
                "SCHEDULE:SCHED_HEADCRAB_FAIL_DROWN"
                "		TASK_SET_ACTIVITY			"
                "ACTIVITY:ACT_HEADCRAB_DROWN"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ROUTE_SEARCH_TIME		2"  // Spend 2 seconds
                                                            // trying to build a
                                                            // path if stuck
...
DEFINE_SCHEDULE(SCHED_HEADCRAB_CRAWL_FROM_CANISTER,
                "	Tasks"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_PLAY_SEQUENCE					"
    "ACTIVITY:ACT_HEADCRAB_BURROW_IN"
    "		TASK_PLAY_SEQUENCE					"
    "ACTIVITY:ACT_HEADCRAB_BURROW_IDLE"
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_HEADCRAB_BURROW_WAIT"
    ""
    "	Interrupts"
    "		COND_TASK_FAILED")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_WAIT_FOR_MOVEMENT				0"
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_HEADCRAB_BURROW_IN"
    ""
    "	Interrupts"
    "		COND_TASK_FAILED"
    "		COND_GIVE_WAY"
...
DEFINE_SCHEDULE(
    ...
    "	Interrupts"
    "		COND_TASK_FAILED"
    "		COND_NEW_ENEMY"  // HACK: We don't actually choose a new
                                 // schedule on new enemy, but we need this
                                 // interrupt so that the headcrab actually
                                 // acquires new enemies while burrowed. (look
                                 // in ai_basenpc.cpp for "DO NOT mess")
    "		COND_CAN_RANGE_ATTACK1")
...
DEFINE_SCHEDULE(SCHED_HEADCRAB_BURROW_OUT,
                ...
                "		TASK_PLAY_SEQUENCE			"
                "	ACTIVITY:ACT_HEADCRAB_BURROW_OUT"
                ""
                "	Interrupts"
                "		COND_TASK_FAILED")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_HEADCRAB_BURROW_OUT"
    ""
    "	Interrupts"
    "		COND_TASK_FAILED")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ACTIVITY					"
    "ACTIVITY:ACT_HEADCRAB_CEILING_IDLE"
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE				"
    "SCHEDULE:SCHED_HEADCRAB_CEILING_WAIT"
