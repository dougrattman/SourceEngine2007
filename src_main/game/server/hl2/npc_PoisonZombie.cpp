...
//
//			Anyway, this guy has two range attacks: at short range,
// headcrabs 			will leap from the nest to attack. At long range he
// will wrench a 			headcrab from his back to throw it at his enemy.
...
//
// Controls how soon he throws the first headcrab after seeing his enemy (also
// when the first headcrab leaps off)
//
...
#define ZOMBIE_ENEMY_BREATHE_DIST \
  300  // How close we must be to our enemy before we start breathing hard.
...
// NOTE: If you change this value you must also change the lookup table in
// Spawn!
//-----------------------------------------------------------------------------
#define MAX_CRABS 3
...
class CNPC_PoisonZombie : public CAI_BlendingHost<CNPC_BaseZombie> {
 ...
 public:
  ...
  virtual int SelectFailSchedule(int nFailedSchedule, int nFailedTask,
                                 AI_TaskFailureCode_t eTaskFailCode);
 ...
 private:
  ...
  float m_flNextCrabThrowTime;  // The next time we are allowed to throw a
                                // headcrab.
...
BEGIN_DATADESC(CNPC_PoisonZombie)
  ...
  DEFINE_SOUNDPATCH(m_pFastBreathSound), DEFINE_SOUNDPATCH(m_pSlowBreathSound),
...
void CNPC_PoisonZombie::Spawn(void) {
  ...
  m_pFastBreathSound = ENVELOPE_CONTROLLER.SoundCreate(
      filter, entindex(), CHAN_ITEM, "NPC_PoisonZombie.FastBreath", ATTN_IDLE);
  ENVELOPE_CONTROLLER.Play(m_pFastBreathSound, 0.0f, 100);

  CPASAttenuationFilter filter2(this);
  m_pSlowBreathSound = ENVELOPE_CONTROLLER.SoundCreate(
      filter2, entindex(), CHAN_ITEM, "NPC_PoisonZombie.Moan1", ATTN_NORM);
...
void CNPC_PoisonZombie::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (!m_fIsTorso) {
    EvacuateNest(info.GetDamageType() == DMG_BLAST, info.GetDamage(),
                 info.GetAttacker());
  }
...
//-----------------------------------------------------------------------------
// Purpose: Checks conditions for letting a headcrab leap off our back at our
// enemy.
//-----------------------------------------------------------------------------
...
void CNPC_PoisonZombie::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_ZOMBIE_POISON_THROW_CRAB) {
    ...
    CBlackHeadcrab *pCrab = (CBlackHeadcrab *)CreateNoSpawn(
        GetHeadcrabClassname(), EyePosition(), vec3_angle, this);
    ...
    m_flNextCrabThrowTime =
        gpGlobals->curtime +
        random->RandomInt(ZOMBIE_THROW_MIN_DELAY, ZOMBIE_THROW_MAX_DELAY);
    return;
  }
  ...
  if (pEvent->event == AE_ZOMBIE_POISON_SPIT) {
    ...
    forward = forward * 200;
    ClawAttack(GetClawAttackRange(), sk_zombie_poison_dmg_spit.GetFloat(),
               qaPunch, forward, ZOMBIE_BLOOD_BITE);
    return;
  }
...
// Input  : bExplosion - We were evicted by an explosion so we should go
// a-flying.
//			flDamage - The damage that was done to cause the
// evacuation.
//-----------------------------------------------------------------------------
void CNPC_PoisonZombie::EvacuateNest(bool bExplosion, float flDamage,
  ...
  for (int i = 0; i < MAX_CRABS; i++) {
    if (m_bCrabs[i]) {
      ...
      // Now slam the angles because the attachment point will have pitch and
      // roll, which we can't use.
      vecAngles = QAngle(0, random->RandomFloat(0, 360), 0);

      CBlackHeadcrab *pCrab = (CBlackHeadcrab *)CreateNoSpawn(
          GetHeadcrabClassname(), vecPosition, vecAngles, this);
      pCrab->Spawn();
...
void CNPC_PoisonZombie::PrescheduleThink(void) {
  if (HasCondition(COND_NEW_ENEMY)) {
    m_flNextCrabThrowTime =
        gpGlobals->curtime + random->RandomInt(ZOMBIE_THROW_FIRST_MIN_DELAY,
                                               ZOMBIE_THROW_FIRST_MAX_DELAY);
  }
  ...
  if (bNearEnemy) {
    if (!m_bNearEnemy) {
      ...
      ENVELOPE_CONTROLLER.SoundChangeVolume(m_pFastBreathSound, BREATH_VOL_MAX,
                                            duration);
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pFastBreathSound,
                                           random->RandomInt(100, 120),
                                           random->RandomFloat(1.0f, 2.0f));
    ...
    ENVELOPE_CONTROLLER.SoundChangeVolume(m_pFastBreathSound, BREATH_VOL_MAX,
                                          duration);
    ENVELOPE_CONTROLLER.SoundChangeVolume(m_pSlowBreathSound, 0.0f, duration);
    //		ENVELOPE_CONTROLLER.SoundChangePitch( m_pBreathSound,
    // random->RandomInt( 80, 100 ), duration );
...
void CNPC_PoisonZombie::BuildScheduleTestBits(void) {
  ...
  if (IsCurSchedule(SCHED_CHASE_ENEMY)) {
    ...
    SetCustomInterruptCondition(COND_HEAVY_DAMAGE);
             ...
             IsCurSchedule(SCHED_RANGE_ATTACK2)) {
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPC_PoisonZombie::SelectFailSchedule(int nFailedSchedule, int nFailedTask,
                                          AI_TaskFailureCode_t eTaskFailCode) {
  int nSchedule = BaseClass::SelectFailSchedule(nFailedSchedule, nFailedTask,
                                                eTaskFailCode);
...
int CNPC_PoisonZombie::TranslateSchedule(int scheduleType) {
  ...
  if (scheduleType == SCHED_MOVE_TO_WEAPON_RANGE) return SCHED_CHASE_ENEMY;
...
//-----------------------------------------------------------------------------
// Purpose: If we don't have any headcrabs to throw, we must close to attack our
// enemy.
//-----------------------------------------------------------------------------
...
//			come through.
//-----------------------------------------------------------------------------
void CNPC_PoisonZombie::MoanSound(envelopePoint_t *pEnvelope,
                                  int iEnvelopeSize) {
  if (!m_pMoanSound) {
    ...
    // pszSound, ATTN_NORM );
    m_pMoanSound = ENVELOPE_CONTROLLER.SoundCreate(filter, entindex(),
                                                   CHAN_STATIC, pszSound, 1.5);
  ...
  ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound,
                                       m_flMoanPitch + flPitchShift, 0.3);

  m_flNextMoanSound = gpGlobals->curtime + duration + 9999;
}

//-----------------------------------------------------------------------------
// Purpose: Overloaded so that explosions don't split the poison zombie in
// twain.
//-----------------------------------------------------------------------------
bool CNPC_PoisonZombie::ShouldBecomeTorso(const CTakeDamageInfo &info,
...
DEFINE_SCHEDULE(SCHED_ZOMBIE_POISON_RANGE_ATTACK2,
                ...
                "ACTIVITY:ACT_ZOMBIE_POISON_THREAT"
                "		TASK_FACE_IDEAL				"
                "			0"
                "		TASK_RANGE_ATTACK2			"
                "			0"
