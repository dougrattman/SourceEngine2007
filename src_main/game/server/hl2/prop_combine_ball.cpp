...
ConVar sk_combineball_guidefactor("sk_combineball_guidefactor", "0.5",
                                  FCVAR_REPLICATED);
ConVar sk_combine_ball_search_radius("sk_combine_ball_search_radius", "512",
                                     FCVAR_REPLICATED);
ConVar sk_combineball_seek_angle("sk_combineball_seek_angle", "15.0",
                                 FCVAR_REPLICATED);
ConVar sk_combineball_seek_kill("sk_combineball_seek_kill", "0",
                                FCVAR_REPLICATED);
...
// Output : CBaseEntity
//-----------------------------------------------------------------------------
CBaseEntity *CreateCombineBall(const Vector &origin, const Vector &velocity,
                               float radius, float mass, float lifetime,
                               CBaseEntity *pOwner) {
  CPropCombineBall *pBall =
      static_cast<CPropCombineBall *>(CreateEntityByName("prop_combine_ball"));
...
CBasePlayer *CPropCombineBall::HasPhysicsAttacker(float dt) {
  // Must have an owner
  if (GetOwnerEntity() == NULL) return false;

  // Must be a player
  if (GetOwnerEntity()->IsPlayer() == false) return false;
...
//			the physcannon because any object held by the physcannon
// is 			COLLISIONGROUP_DEBRIS.
//-----------------------------------------------------------------------------
...
bool UTIL_IsAR2CombineBall(CBaseEntity *pEntity) {
  ...
  if (pBall && pBall->WasWeaponLaunched()) return true;
...
BEGIN_DATADESC(CPropCombineBall)
  ...
  DEFINE_FIELD(m_flLastBounceTime, FIELD_TIME),
      ...
      DEFINE_THINKFUNC(ExplodeThink), DEFINE_THINKFUNC(WhizSoundThink),
      DEFINE_THINKFUNC(DieThink), DEFINE_THINKFUNC(DissolveThink),
      DEFINE_THINKFUNC(DissolveRampSoundThink), DEFINE_THINKFUNC(AnimThink),
...
SendPropBool(SENDINFO(m_bEmit)),
    ...
    SendPropBool(SENDINFO(m_bHeld)), SendPropBool(SENDINFO(m_bLaunched)),
    END_SEND_TABLE()
...
// Spherical vphysics
//-----------------------------------------------------------------------------
bool CPropCombineBall::OverridePropdata() { return true; }
...
bool CPropCombineBall::CreateVPhysics() {
  ...
  if (!pPhysicsObject) return false;
  ...
  if (WasFiredByNPC()) {
    // Don't do impact damage. Just touch them and do your dissolve damage and
    // move on.
    PhysSetGameFlags(pPhysicsObject, FVPHYSICS_NO_NPC_IMPACT_DMG);
  } else {
    PhysSetGameFlags(pPhysicsObject,
                     FVPHYSICS_DMG_DISSOLVE | FVPHYSICS_HEAVY_OBJECT);
  }
...
void CPropCombineBall::Spawn(void) {
  ...
  m_pGlowTrail = CSpriteTrail::SpriteTrailCreate(PROP_COMBINE_BALL_SPRITE_TRAIL,
                                                 GetAbsOrigin(), false);
...
void CPropCombineBall::StartAnimating(void) {
  // Start our animation cycle. Use the random to avoid everything thinking the
  // same frame
  SetContextThink(&CPropCombineBall::AnimThink,
                  gpGlobals->curtime + random->RandomFloat(0.0f, 0.1f),
                  s_pAnimThinkContext);
...
void CPropCombineBall::CaptureBySpawner() {
  ...
  if (flSpeed > 25.0f) {
    ...
    SetContextThink(&CPropCombineBall::CaptureBySpawner,
                    gpGlobals->curtime + 0.01f, s_pCaptureContext);
    return;
  }
...
float CPropCombineBall::LastCaptureTime() const {
  if (IsInField() || IsBeingCaptured()) return gpGlobals->curtime;
...
void CPropCombineBall::StartLifetime(float flDuration) {
  SetContextThink(&CPropCombineBall::ExplodeThink,
                  gpGlobals->curtime + flDuration, s_pExplodeTimerContext);
}
...
void CPropCombineBall::UpdateOnRemove() {
  ...
  // Sigh... this is the only place where I can get a message after the ball is
  // done dissolving.
  if (hl2_episodic.GetBool()) {
    if (IsDissolving()) {
      if (GetSpawner()) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCombineBall::ExplodeThink(void) { DoExplosion(); }
...
void CPropCombineBall::StartWhizSoundThink(void) {
  SetContextThink(&CPropCombineBall::WhizSoundThink,
                  gpGlobals->curtime + 2.0f * TICK_INTERVAL,
                  s_pWhizThinkContext);
}
...
void CPropCombineBall::WhizSoundThink() {
  ...
  if (pPlayer) {
    ...
    if (DotProduct(vecDelta, vecVelocity) > 0.5f) {
      ...
      float flDist = CalcDistanceToLineSegment(pPlayer->GetAbsOrigin(),
                                               vecPosition, vecEndPoint);
      if (flDist < 200.0f) {
        ...
        SetContextThink(&CPropCombineBall::WhizSoundThink,
                        gpGlobals->curtime + 0.5f, s_pWhizThinkContext);
        return;
      }
    }
  }
...
//-----------------------------------------------------------------------------
void CPropCombineBall::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (reason != PUNTED_BY_CANNON) {
    ...
    // Let the ball know is not being captured by one of those ball fields
    // anymore.
    //
    m_bCaptureInProgress = false;

    SetContextThink(&CPropCombineBall::DissolveRampSoundThink,
                    gpGlobals->curtime + GetBallHoldSoundRampTime(),
                    s_pHoldDissolveContext);
...
void CPropCombineBall::SetPlayerLaunched(CBasePlayer *pOwner) {
  ...
  if (VPhysicsGetObject()) {
    ...
    PhysSetGameFlags(VPhysicsGetObject(),
                     FVPHYSICS_DMG_DISSOLVE | FVPHYSICS_HEAVY_OBJECT);
  }
}
...
// Activate death-spin!
//-----------------------------------------------------------------------------
void CPropCombineBall::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
  ...
  if (Reason != LAUNCHED_BY_CANNON) {
    ...
    // ball is launched directly down the crosshair even if the player is
    // moving.
...
void CPropCombineBall::DissolveRampSoundThink() {
  ...
  SetContextThink(&CPropCombineBall::DissolveThink, gpGlobals->curtime + dt,
                  s_pHoldDissolveContext);
}
...
// Pow!
//------------------------------------------------------------------------------
void CPropCombineBall::DissolveThink() { DoExplosion(); }
...
void CPropCombineBall::DoExplosion() {
  // don't do this twice
  if (GetMoveType() == MOVETYPE_NONE) return;
  ...
  if (hl2_episodic.GetBool()) {
    CSoundEnt::InsertSound(SOUND_COMBAT | SOUND_CONTEXT_EXPLOSION,
                           WorldSpaceCenter(), 180.0f, 0.25, this);
  }
  ...
  if (!m_bStruckEntity && hl2_episodic.GetBool() && GetOwnerEntity() != NULL) {
    // Notify the player proxy that this combine ball missed so that it can fire
    // an output.
  ...
  SetContextThink(&CPropCombineBall::SUB_Remove, gpGlobals->curtime + 0.5f,
                  s_pRemoveContext);
  StopLoopingSounds();
}
...
// Enable/disable
//-----------------------------------------------------------------------------
void CPropCombineBall::InputExplode(inputdata_t &inputdata) { DoExplosion(); }
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCombineBall::CollisionEventToTrace(int index,
...
bool CPropCombineBall::DissolveEntity(CBaseEntity *pEntity) {
  if (pEntity->IsEFlagSet(EFL_NO_DISSOLVE)) return false;
  ...
  pEntity->GetBaseAnimating()->Dissolve("", gpGlobals->curtime, false,
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCombineBall::OnHitEntity(CBaseEntity *pHitEntity, float flSpeed,
  ...
  if (FClassnameIs(pHitEntity, "npc_strider") ||
      (pHitEntity->GetOwnerEntity() &&
       FClassnameIs(pHitEntity->GetOwnerEntity(), "npc_strider"))) {
    DoExplosion();
    return;
  }

  CTakeDamageInfo info(this, GetOwnerEntity(), GetAbsVelocity(), GetAbsOrigin(),
                       sk_npc_dmg_combineball.GetFloat(), DMG_DISSOLVE);

  bool bIsDissolving = (pHitEntity->GetFlags() & FL_DISSOLVING) != 0;
  bool bShouldHit = pHitEntity->PassesDamageFilter(info);

  // One more check
  // Combine soldiers are not allowed to hurt their friends with combine balls
  // (they can still shoot and hurt each other with grenades).
  ...
  if (!bIsDissolving && bShouldHit == true) {
    if (pHitEntity->PassesDamageFilter(info)) {
      if (WasFiredByNPC() || m_nMaxBounces == -1) {
        // Since Combine balls fired by NPCs do a metered dose of damage per
        // impact, we have to ignore touches for a little while after we hit
        // someone, or the ball will immediately touch them again and do more
        // damage.
        if (gpGlobals->curtime >= m_flNextDamageTime) {
          EmitSound("NPC_CombineBall.KillImpact");

          if (pHitEntity->IsNPC() &&
              pHitEntity->Classify() != CLASS_PLAYER_ALLY_VITAL &&
              hl2_episodic.GetBool() == true) {
            if (pHitEntity->Classify() != CLASS_PLAYER_ALLY ||
                pHitEntity->Classify() == CLASS_PLAYER_ALLY &&
                    m_bStruckEntity == false) {
        ...
        if ((m_nState == STATE_THROWN) &&
            (pHitEntity->IsNPC() || dynamic_cast<CRagdollProp *>(pHitEntity))) {
          EmitSound("NPC_CombineBall.KillImpact");
        }
        if ((m_nState != STATE_HOLDING)) {
          CBasePlayer *pPlayer = ToBasePlayer(GetOwnerEntity());
          if (pPlayer && UTIL_IsAR2CombineBall(this) &&
              ToBaseCombatCharacter(pHitEntity)) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCombineBall::DoImpactEffect(const Vector &preVelocity, int index,
                                      gamevcollisionevent_t *pEvent) {
  ...
  UTIL_TraceLine(tr.startpos - preVelocity * 2.0f,
                 tr.startpos + preVelocity * 2.0f, MASK_SOLID, pTraceEntity,
...
//-----------------------------------------------------------------------------
// Tells whether this combine ball should consider deflecting towards this
// entity.
//-----------------------------------------------------------------------------
bool CPropCombineBall::IsAttractiveTarget(CBaseEntity *pEntity) {
  if (!pEntity->IsAlive()) return false;

  if (pEntity->GetFlags() & EF_NODRAW) return false;

  // Don't guide toward striders
  if (FClassnameIs(pEntity, "npc_strider")) return false;
  ...
  if (WasFiredByNPC()) {
    ...
    if (pEntity->m_iClassname == GetOwnerEntity()->m_iClassname) return false;
  } else {
#ifndef HL2MP
    if (GetOwnerEntity()) {
    ...
    if (pEntity->Classify() == CLASS_BULLSEYE) return false;

#else
    if (pEntity->IsPlayer() == false) return false;

    if (pEntity == GetOwnerEntity()) return false;

    // No tracking teammates in teammode!
    if (g_pGameRules->IsTeamplay()) {
      if (g_pGameRules->PlayerRelationship(GetOwnerEntity(), pEntity) ==
          GR_TEAMMATE)
        return false;
    }
#endif
    ...
    trace_t tr;
    UTIL_TraceLine(WorldSpaceCenter(), pEntity->BodyTarget(WorldSpaceCenter()),
                   MASK_SOLID, this, COLLISION_GROUP_NONE, &tr);

    if (tr.fraction < 1.0f && tr.m_pEnt != pEntity) return false;
  }
...
//-----------------------------------------------------------------------------
void CPropCombineBall::DeflectTowardEnemy(float flSpeed, int index,
                                          gamevcollisionevent_t *pEvent) {
  // Bounce toward a particular enemy; choose one that's closest to my new
  // velocity.
  ...
  bool bSeekKill = m_bStruckEntity &&
                   (WasWeaponLaunched() || sk_combineball_seek_kill.GetInt());

  if (bSeekKill) {
    int nCount = UTIL_EntitiesInSphere(list, 1024, GetAbsOrigin(),
                                       sk_combine_ball_search_radius.GetFloat(),
                                       FL_NPC | FL_CLIENT);

    for (int i = 0; i < nCount; i++) {
      if (!IsAttractiveTarget(list[i])) continue;
...
//-----------------------------------------------------------------------------
void CPropCombineBall::BounceInSpawner(float flSpeed, int index,
                                       gamevcollisionevent_t *pEvent) {
...
bool CPropCombineBall::IsHittableEntity(CBaseEntity *pHitEntity) {
  if (pHitEntity->IsWorld()) return false;

  if (pHitEntity->GetMoveType() == MOVETYPE_PUSH) {
    if (pHitEntity->GetOwnerEntity() &&
        FClassnameIs(pHitEntity->GetOwnerEntity(), "npc_strider")) {
      // The Strider's Bone Followers are MOVETYPE_PUSH, and we want the combine
      // ball to hit these.
      return true;
    }

    // If the entity we hit can take damage, we're good
    if (pHitEntity->m_takedamage == DAMAGE_YES) return true;
...
void CPropCombineBall::VPhysicsCollision(int index,
  ...
  if (m_nMaxBounces == -1) {
    ...
    if (pHit->game.material != CHAR_TEX_FLESH || !hl2_episodic.GetBool()) {
      ...
      // Remove self without affecting the object that was hit. (Unless it was
      // flesh)
      NotifySpawnerOfRemoval();
      PhysCallbackRemove(this->NetworkProp());

      // disable dissolve damage so we don't kill off the player when he's the
      // one we hit
  ...
  if (m_nState == STATE_HOLDING) return;
  ...
  if (gpGlobals->curtime - m_flLastBounceTime < 0.25f) return;

  // Save off our last bounce time
  m_flLastBounceTime = gpGlobals->curtime;

  // Reset the sound timer
  SetContextThink(&CPropCombineBall::WhizSoundThink, gpGlobals->curtime + 0.01,
                  s_pWhizThinkContext);
...
void CPropCombineBall::AnimThink(void) {
  StudioFrameAdvance();
  SetContextThink(&CPropCombineBall::AnimThink, gpGlobals->curtime + 0.1f,
                  s_pAnimThinkContext);
}
...
BEGIN_DATADESC(CFuncCombineBallSpawner)
  ...
  DEFINE_KEYFIELD(m_nBallCount, FIELD_INTEGER, "ballcount"),
      ...
      DEFINE_THINKFUNC(BallThink), DEFINE_ENTITYFUNC(GrabBallTouch),
...
void CFuncCombineBallSpawner::Spawn() {
  ...
  if (!(m_flMinSpeed == 0 && m_flMaxSpeed == 0)) {
    flDeltaT = (CollisionProp()->OBBSize().z - 2 * m_flBallRadius) /
               ((m_flMinSpeed + m_flMaxSpeed) * 0.5f);
...
void CFuncCombineBallSpawner::ChoosePointInBox(Vector *pVecPoint) {
  float flXBoundary = (CollisionProp()->OBBSize().x != 0)
                          ? m_flBallRadius / CollisionProp()->OBBSize().x
                          : 0.0f;
  float flYBoundary = (CollisionProp()->OBBSize().y != 0)
                          ? m_flBallRadius / CollisionProp()->OBBSize().y
                          : 0.0f;
  if (flXBoundary > 0.5f) {
    flXBoundary = 0.5f;
  }
  ...
  CollisionProp()->RandomPointInBounds(
      Vector(flXBoundary, flYBoundary, 0.0f),
      Vector(1.0f - flXBoundary, 1.0f - flYBoundary, 0.0f), pVecPoint);
}
...
void CFuncCombineBallSpawner::ChoosePointInCylinder(Vector *pVecPoint) {
  ...
  CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.0f),
                                          &vecEndPoint1);
  CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 1.0f),
                                          &vecEndPoint2);
  ...
  do {
    CollisionProp()->RandomPointInBounds(
        ...
        Vector(0.5f + flXRange, 0.5f + flYRange, 0.0f), pVecPoint);
...
//-----------------------------------------------------------------------------
void CFuncCombineBallSpawner::RegisterReflection(CPropCombineBall *pBall,
                                                 bool bForward) {
  if (bForward) {
...
//-----------------------------------------------------------------------------
void CFuncCombineBallSpawner::GetTargetEndpoint(bool bForward,
                                                Vector *pVecEndPoint) {
...
void CFuncCombineBallSpawner::BallGrabbed(CBaseEntity *pCombineBall) {
  ...
  // Stop the ball thinking in case it was in the middle of being captured
  // (which could re-add incorrectly)
...
void CFuncCombineBallSpawner::GrabBallTouch(CBaseEntity *pOther) {
  ...
  if (m_nBallsRemainingInField >= m_nBallCount) return;

  if (pOther->GetCollisionGroup() != HL2COLLISION_GROUP_COMBINE_BALL) return;

  CPropCombineBall *pBall = dynamic_cast<CPropCombineBall *>(pOther);
  Assert(pBall);

  // Don't grab AR2 alt-fire
  if (pBall->WasWeaponLaunched() || !pBall->VPhysicsGetObject()) return;

  // Don't grab balls that are already in the field..
  if (pBall->IsInField()) return;

  // Don't grab fading out balls...
  if (!pBall->IsSolid()) return;

  // Don't capture balls that were very recently in the field (breaks punting)
  if (gpGlobals->curtime - pBall->LastCaptureTime() < 0.5f) return;
...
void CFuncCombineBallSpawner::RespawnBall(float flRespawnTime) {
  ...
  m_BallRespawnTime.AddToTail(gpGlobals->curtime + flRespawnTime -
                              m_flDisableTime);
}
...
void CFuncCombineBallSpawner::RespawnBallPostExplosion(void) {
  if (m_flBallRespawnTime < 0) return;

  if (m_flBallRespawnTime == 0.0f) {
    m_BallRespawnTime.AddToTail(gpGlobals->curtime + 4.0f - m_flDisableTime);
  } else {
    m_BallRespawnTime.AddToTail(gpGlobals->curtime + m_flBallRespawnTime -
                                m_flDisableTime);
  }
}
...
void CPointCombineBallLauncher::SpawnBall() {
  ...
  qAngle = qAngle +
           QAngle(random->RandomFloat(-m_flConeDegrees, m_flConeDegrees),
                  random->RandomFloat(-m_flConeDegrees, m_flConeDegrees), 0);
  ...
  if (GetSpawnFlags() & SF_COMBINE_BALL_LAUNCHER_ATTACH_BULLSEYE) {
    CNPC_Bullseye *pBullseye =
        static_cast<CNPC_Bullseye *>(CreateEntityByName("npc_bullseye"));
