...
void CNPC_MissileDefense::AimGun( void )
{
	...
	if (angles.x > m_vGunAng.x)
		m_vGunAng.x = std::min( angles.x, m_vGunAng.x + MD_PITCH_SPEED );
	if (angles.x < m_vGunAng.x)
		m_vGunAng.x = std::max( angles.x, m_vGunAng.x - MD_PITCH_SPEED );
	if (angles.y > m_vGunAng.y)
		m_vGunAng.y = std::min( angles.y, m_vGunAng.y + MD_YAW_SPEED );
	if (angles.y < m_vGunAng.y)
		m_vGunAng.y = std::max( angles.y, m_vGunAng.y - MD_YAW_SPEED );
