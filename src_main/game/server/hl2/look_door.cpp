...
void CLookDoor::Spawn(void) {
  ...
  CLookDoorThinker *pLookThinker =
      (CLookDoorThinker *)CreateEntityByName("lookdoorthinker");
  if (pLookThinker) {
...
void CLookDoor::MoveThink(void) {
  ...
  if (FBitSet(m_spawnflags.operator const int &(), SF_LDOOR_FROM_OPEN)) {
  ...
  if (m_flProximityDistance > 0) {
    float flDist =
        (m_hLooker->GetAbsOrigin() - vOrigin).Length() - m_flProximityOffset;
    if (flDist < 0) flDist = 0;

    if (flDist < m_flProximityDistance) {
      if (FBitSet(m_spawnflags.operator const int &(), SF_LDOOR_THRESHOLD)) {
        flProxMove = 1.0;
      } else {
  ...
  if (FBitSet(m_spawnflags.operator const int &(), SF_LDOOR_INVERT)) {
    flMove = 1 - flMove;
  }
