...
class CNPC_Strider : public CAI_BlendingHost<CAI_BaseNPC>,
 ...
 private:
  ...
  bool m_bNoMoveToLOS;
  bool m_bFastCrouch;
  bool m_bMinigunEnabled;  // If false, minigun disabled by level designer until
                           // further notice.
...
class CStriderMinigun {
 public:
  ...
  void StopShootingForSeconds(IStriderMinigunHost *pHost, CBaseEntity *pTarget,
                              float duration);
 ...
 private:
  ...
  int m_minigunState;
  float
      m_nextBulletTime;  // Minigun is shooting, when can I fire my next bullet?
  float m_burstTime;  // If firing, how long till done? If not, how long till I
                      // can?
