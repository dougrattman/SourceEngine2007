...
class CTraceFilterPhyscannon : public CTraceFilterSimple {
 public:
  ...
  // For this test, we only test against entities (then world brushes
  // afterwards)
  ...
  bool HasContentsGrate(CBaseEntity *pEntity) {
    ...
    if (nModelType == mod_studio) {
      ...
      if (contents & CONTENTS_GRATE) return true;
    }
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pHandleEntity == m_pTraceOwner) return false;
    ...
    if (HasContentsGrate(pEntity)) {
      ...
      if (pPhysProp != NULL) return pPhysProp->CanBePickedUpByPhyscannon();
      ...
      if (FClassnameIs(pEntity, "prop_physics")) {
        ...
        if (pPhysBox) return pPhysBox->CanBePickedUpByPhyscannon();
...
//-----------------------------------------------------------------------------
void UTIL_PhyscannonTraceLine(const Vector &vecAbsStart,
                              const Vector &vecAbsEnd, CBaseEntity *pTraceOwner,
                              trace_t *pTrace) {
  // Default to HL2 vanilla
  if (hl2_episodic.GetBool() == false) {
    CTraceFilterNoOwnerTest filter(pTraceOwner, COLLISION_GROUP_NONE);
    UTIL_TraceLine(vecAbsStart, vecAbsEnd, (MASK_SHOT | CONTENTS_GRATE),
                   &filter, pTrace);
    return;
  }
  ...
  UTIL_TraceLine(vecAbsStart, vecAbsEnd, (MASK_SHOT | CONTENTS_GRATE), &filter,
                 pTrace);
  ...
  if (pTrace->m_pEnt != NULL) {
    ...
    UTIL_TraceLine(pTrace->startpos, pTrace->endpos, MASK_SHOT, &brushFilter,
                   &testTrace);

    // If we hit a brush, replace the trace with that result
    if (testTrace.fraction < 1.0f || testTrace.startsolid ||
        testTrace.allsolid) {
      *pTrace = testTrace;
    }
  }
...
//-----------------------------------------------------------------------------
void UTIL_PhyscannonTraceHull(const Vector &vecAbsStart,
                              const Vector &vecAbsEnd, const Vector &vecAbsMins,
                              const Vector &vecAbsMaxs,
                              CBaseEntity *pTraceOwner, trace_t *pTrace) {
  // Default to HL2 vanilla
  if (hl2_episodic.GetBool() == false) {
    CTraceFilterNoOwnerTest filter(pTraceOwner, COLLISION_GROUP_NONE);
    UTIL_TraceHull(vecAbsStart, vecAbsEnd, vecAbsMins, vecAbsMaxs,
                   (MASK_SHOT | CONTENTS_GRATE), &filter, pTrace);
    return;
  }
  ...
  if (pTrace->m_pEnt != NULL) {
    trace_t testTrace;
    CTraceFilterOnlyBrushes brushFilter(COLLISION_GROUP_NONE);
    UTIL_TraceHull(pTrace->startpos, pTrace->endpos, vecAbsMins, vecAbsMaxs,
                   MASK_SHOT, &brushFilter, &testTrace);
...
//-----------------------------------------------------------------------------
IPhysicsObject *GetRagdollChildAtPosition(CBaseEntity *pTarget,
                                          const Vector &position) {
  // Check for a ragdoll
  if (dynamic_cast<CRagdollProp *>(pTarget) == NULL) return NULL;
...
static void ComputePlayerMatrix(CBasePlayer *pPlayer, matrix3x4_t &out) {
  ...
  if (angles.x < 0) {
    ...
    zoffset =
        RemapVal(angles.x, 0, PLAYER_LOOK_PITCH_RANGE, PLAYER_HOLD_LEVEL_EYES,
...
class CGrabController : public IMotionEvent {
 ...
 private:
  ...
  bool m_bAllowObjectOverhead;  // Can the player hold this object directly
                                // overhead? (Default is NO)
...
void CGrabController::AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity,
  ...
  int playerMaterial = pPlayer->VPhysicsGetObject()
                           ? pPlayer->VPhysicsGetObject()->GetMaterialIndex()
                           : hitMaterial;
  PhysicsImpactSound(pPlayer, pPhys, CHAN_STATIC, hitMaterial, playerMaterial,
                     1.0, 64);
  Vector position;
  QAngle angles;
  ...
  // If we haven't been killed by a grab, we allow the gun to grab the nearest
  // part of a ragdoll
  if (bUseGrabPosition) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    // reduce the mass to prevent the player from adding crazy amounts of energy
    // to the system
  ...
  m_errorTime = bIsMegaPhysCannon
                    ? -1.5f
                    : -1.0f;  // 1 seconds until error starts accumulating
  m_error = 0;
  m_contactAmount = 0;
...
IMotionEvent::simresult_e CGrabController::Simulate(
  ...
  shadowParams.maxAngular =
      m_shadow.maxAngular * m_contactAmount * m_contactAmount * m_contactAmount;
  m_timeToArrive =
...
float CGrabController::GetSavedMass(IPhysicsObject *pObject) {
  ...
  if (pHeld) {
    if (pObject->GetGameData() == (void *)pHeld) {
      ...
      for (int i = 0; i < count; i++) {
        if (pList[i] == pObject) return m_savedMass[i];
      }
    }
  }
  return 0.0f;
...
bool CGrabController::IsObjectAllowedOverhead(CBaseEntity *pEntity) {
  ...
  if (UTIL_IsCombineBallDefinite(pEntity)) return true;

  // Allow props that are specifically flagged as such
  CPhysicsProp *pPhysProp = dynamic_cast<CPhysicsProp *>(pEntity);
  if (pPhysProp != NULL &&
      pPhysProp->HasInteraction(PROPINTER_PHYSGUN_ALLOW_OVERHEAD))
    return true;

  // String checks are fine here, we only run this code one time- when the
  // object is picked up.
  if (pEntity->ClassMatches("grenade_helicopter")) return true;

  if (pEntity->ClassMatches("weapon_striderbuster")) return true;
...
class CPlayerPickupController : public CBaseEntity {
 ...
 public:
  ...
  void Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType,
           float value);
  void OnRestore() { m_grabController.OnRestore(); }
...
void CPlayerPickupController::Init(CBasePlayer *pPlayer, CBaseEntity *pObject) {
  ...
  m_grabController.AttachEntity(pPlayer, pObject, pPhysics, false, vec3_origin,
                                false);
  ...
  if (m_pPlayer) {
    ...
    if (m_pPlayer->GetActiveWeapon()) {
      if (!m_pPlayer->GetActiveWeapon()->Deploy()) {
        ...
        // This usually happens when they're holding an empty weapon that
        // doesn't autoswitch away when out of ammo. Switch to next best weapon.
...
void CPlayerPickupController::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  if (ToBasePlayer(pActivator) == m_pPlayer) {
    ...
    // UNDONE: Must fix case of forcing objects into the ground you're standing
    // on (causes stress) before that will work
...
BEGIN_SIMPLE_DATADESC(thrown_objects_t)
  DEFINE_FIELD(fTimeThrown, FIELD_TIME), DEFINE_FIELD(hEntity, FIELD_EHANDLE),
END_DATADESC()
...
class CWeaponPhysCannon : public CBaseHLCombatWeapon {
 public:
  ...
  void ForceDrop(void);
  bool DropIfEntityHeld(CBaseEntity *pTarget);  // Drops its held entity if it
                                                // matches the entity passed in
  CGrabController &GetGrabController() { return m_grabController; }
 ...
 protected:
  ...
  void ApplyVelocityBasedForce(CBaseEntity *pEntity, const Vector &forward,
                               const Vector &vecHitPos, PhysGunForce_t reason);
  ...
  inline bool IsMegaPhysCannon() { return PlayerHasMegaPhysCannon(); }
...
BEGIN_DATADESC(CWeaponPhysCannon)
  ...
  DEFINE_FIELD(m_bOpen, FIELD_BOOLEAN), DEFINE_FIELD(m_bActive, FIELD_BOOLEAN),
  ...
  if (m_bActive) {
    if (!pSwitchingTo ||
        (m_grabController.GetAttached() == pSwitchingTo &&
         GetOwner()->Weapon_OwnsThisType(pSwitchingTo->GetClassname(),
                                         pSwitchingTo->GetSubType()))) {
    } else {
      return false;
    }
  }
  ...
  if (hl2_episodic.GetBool() && mass > 250.0f) {
    ...
    for (int i = 0; i < nAIs; i++) {
      if (ppAIs[i]->Classify() == CLASS_PLAYER_ALLY_VITAL) {
        ppAIs[i]->DispatchInteraction(g_interactionPlayerPuntedHeavyObject,
                                      pEntity, pOwner);
      }
    }
  }
...
void CWeaponPhysCannon::PuntVPhysics(CBaseEntity *pEntity,
  ...
  if (Pickup_OnAttemptPhysGunPickup(pEntity, pOwner, PUNTED_BY_CANNON)) {
    ...
    if (forward.z < 0) {
      // reflect, but flatten the trajectory out a bit so it's easier to hit
      // standing targets
      forward.z *= -0.65f;
    }
    ...
    if (!IsMegaPhysCannon() &&
        !Pickup_ShouldPuntUseLaunchForces(pEntity, PHYSGUN_FORCE_PUNTED)) {
      int i;
      ...
      ApplyVelocityBasedForce(pEntity, vecForward, tr.endpos,
                              PHYSGUN_FORCE_PUNTED);
    }
  }
...
void CWeaponPhysCannon::ApplyVelocityBasedForce(CBaseEntity *pEntity,
  ...
  if (pRagdoll == NULL) {
    ...
    if (reason == PHYSGUN_FORCE_PUNTED && pEntity->GetServerVehicle()) {
      // We want the point to emanate low on the vehicle to move it along the
      // ground, not to twist it
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::PuntRagdoll(CBaseEntity *pEntity,
                                    const Vector &vecForward, trace_t &tr) {
  CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
//-----------------------------------------------------------------------------
// If there's any special rejection code you need to do per entity then do it
// here This is kinda nasty but I'd hate to move more physcannon related stuff
// into CBaseEntity
//-----------------------------------------------------------------------------
...
void CWeaponPhysCannon::PrimaryAttack(void) {
  ...
  if (m_bActive) {
    ...
    if (pHeld != NULL) {
      float heldDist = pHeld->CollisionProp()->CalcDistanceFromPoint(
  ...
  trace_t tr;
  UTIL_PhyscannonTraceHull(start, end, -Vector(8, 8, 8), Vector(8, 8, 8),
                           pOwner, &tr);
  bool bValid = true;
  ...
  if (!bValid) {
    ...
    if (tr.fraction == 1 || !tr.m_pEnt ||
      ...
      if (hl2_episodic.GetBool()) {
        ...
        CBaseEntity *pObject =
            FindObjectInCone(start, forward, physcannon_punt_cone.GetFloat());

        if (pObject) {
          // Trace to the object.
          UTIL_PhyscannonTraceLine(start, pObject->WorldSpaceCenter(), pOwner,
                                   &tr);

          if (tr.m_pEnt && tr.m_pEnt == pObject &&
  ...
  if (pEntity->GetMoveType() != MOVETYPE_VPHYSICS) {
    ...
    if (GetOwner()->IsPlayer() && !IsMegaPhysCannon()) {
      ...
      // headcrabs.
      if (pEntity->IsNPC() && pEntity->Classify() != CLASS_HEADCRAB &&
          !FClassnameIs(pEntity, "npc_antlion")) {
        DryFire();
        return;
      }
    }
    ...
    if (IsMegaPhysCannon()) {
      if (pEntity->IsNPC() &&
          !pEntity->IsEFlagSet(EFL_NO_MEGAPHYSCANNON_RAGDOLL) &&
          pEntity->MyNPCPointer()->CanBecomeRagdoll()) {
        CTakeDamageInfo info(pOwner, pOwner, 1.0f, DMG_GENERIC);
        CBaseEntity *pRagdoll =
            CreateServerRagdoll(pEntity->MyNPCPointer(), 0, info,
                                COLLISION_GROUP_INTERACTIVE_DEBRIS, true);
        PhysSetEntityGameFlags(pRagdoll, FVPHYSICS_NO_SELF_COLLISIONS);
        pRagdoll->SetCollisionBounds(pEntity->CollisionProp()->OBBMins(),
                                     pEntity->CollisionProp()->OBBMaxs());

        // Necessary to cause it to do the appropriate death cleanup
        CTakeDamageInfo ragdollInfo(pOwner, pOwner, 10000.0,
                                    DMG_PHYSGUN | DMG_REMOVENORAGDOLL);
...
bool CWeaponPhysCannon::AttachObject(CBaseEntity *pObject,
  ...
  if (bIsMegaPhysCannon) {
    if (pObject->IsNPC() &&
        !pObject->IsEFlagSet(EFL_NO_MEGAPHYSCANNON_RAGDOLL)) {
      Assert(pObject->MyNPCPointer()->CanBecomeRagdoll());
      CTakeDamageInfo info(GetOwner(), GetOwner(), 1.0f, DMG_GENERIC);
      CBaseEntity *pRagdoll =
          CreateServerRagdoll(pObject->MyNPCPointer(), 0, info,
                              COLLISION_GROUP_INTERACTIVE_DEBRIS, true);
      PhysSetEntityGameFlags(pRagdoll, FVPHYSICS_NO_SELF_COLLISIONS);

      pRagdoll->SetCollisionBounds(pObject->CollisionProp()->OBBMins(),
                                   pObject->CollisionProp()->OBBMaxs());

      // Necessary to cause it to do the appropriate death cleanup
      CTakeDamageInfo ragdollInfo(GetOwner(), GetOwner(), 10000.0,
                                  DMG_PHYSGUN | DMG_REMOVENORAGDOLL);
  ...
  // mass
  m_grabController.AttachEntity(pOwner, pObject, pPhysics, bIsMegaPhysCannon,
                                vPosition, (!bKilledByGrab));
  ...
  if (pOwner) {
    ...
    float maxSpeed =
        hl2_walkspeed.GetFloat() +
        ((hl2_normspeed.GetFloat() - hl2_walkspeed.GetFloat()) * loadWeight);

    // Msg( "Load perc: %f -- Movement speed: %f/%f\n", loadWeight, maxSpeed,
    // hl2_normspeed.GetFloat() );
    pOwner->SetMaxSpeed(maxSpeed);
  }

  // Don't drop again for a slight delay, in case they were pulling objects near
  // them
...
#if defined(HL2_DLL)
  if (physcannon_right_turrets.GetBool() &&
    ...
    if (flDot < 0.5f) {
      // The turret is NOT upright, so have the client help us by raising up the
      // player's view.
...
void CWeaponPhysCannon::FindObjectTrace(CBasePlayer *pPlayer,
                                        trace_t *pTraceResult) {
  Vector forward;
  ...
  if (!pTraceResult->DidHitNonWorldEntity()) {
    UTIL_PhyscannonTraceHull(start, end, -Vector(4, 4, 4), Vector(4, 4, 4),
                             pPlayer, pTraceResult);
  }
}
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject(void) {
  ...
  Assert(pPlayer);
  if (pPlayer == NULL) return OBJECT_NOT_FOUND;
  ...
  if (!IsMegaPhysCannon()) {
    ...
    pConeEntity = MegaPhysCannonFindObjectInCone(
        start, forward, physcannon_cone.GetFloat(),
        physcannon_ball_cone.GetFloat(), bAttach || bPull);
  }
  ...
  pullDir *= IsMegaPhysCannon() ? physcannon_mega_pullforce.GetFloat()
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponPhysCannon::MegaPhysCannonFindObjectInCone(
    const Vector &vecOrigin, const Vector &vecDir, float flCone,
    float flCombineBallCone, bool bOnlyCombineBalls) {
  ...
  for (int i = 0; i < count; i++) {
    if (!list[i]->VPhysicsGetObject()) continue;

    bool bIsCombineBall = FClassnameIs(list[i], "prop_combine_ball");
    if (!bIsCombineBall && bNearestIsCombineBall) continue;
    ...
    if (!bIsCombineBall || bNearestIsCombineBall) {
      ...
      // NOW: If it's either closer than nearest dist or within the ball cone,
      // use it!
      if ((flDist > flNearestDist) &&
          (DotProduct(los, vecDir) <= flCombineBallCone))
        continue;
    }

    // Make sure it isn't occluded!
    trace_t tr;
    UTIL_PhyscannonTraceLine(vecOrigin, list[i]->WorldSpaceCenter(), GetOwner(),
                             &tr);
    if (tr.m_pEnt == list[i]) {
...
CBaseEntity *CWeaponPhysCannon::FindObjectInCone(const Vector &vecOrigin,
  ...
  float flNearestDist =
      physcannon_tracelength.GetFloat() + 1.0;  // Use regular distance.
  Vector mins = vecOrigin - Vector(flNearestDist, flNearestDist, flNearestDist);
  Vector maxs = vecOrigin + Vector(flNearestDist, flNearestDist, flNearestDist);
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError) {
  CBaseEntity *pEntity = GetAttached();
  if (!pEntity || ComputeError() > flError ||
  ...
  if (HL2GameRules()->MegaPhyscannonActive()) {
    Vector los =
        (pEntity->WorldSpaceCenter() - pPlayer->Weapon_ShootPosition());
    VectorNormalize(los);

    float flDot = DotProduct(los, forward);

    // Let go of the item if we turn around too fast.
    if (flDot <= 0.35f) return false;
  }
  ...
  if (g_debug_physcannon.GetBool()) {
    NDebugOverlay::Box(end, -Vector(2, 2, 2), Vector(2, 2, 2), 0, 255, 0, true,
                       0);

    NDebugOverlay::Box(GetAttached()->WorldSpaceCenter(),
                       -Vector(radius, radius, radius),
                       Vector(radius, radius, radius), 255, 0, 0, true, 0.0f);
  }
...
void CWeaponPhysCannon::DetachObject(bool playSound, bool wasLaunched) {
  ...
  if (pObject != NULL) {
    Pickup_OnPhysGunDrop(pObject, pOwner,
...
void CWeaponPhysCannon::ItemPreFrame() {
  ...
  m_flElementPosition =
      UTIL_Approach(m_flElementDestination, m_flElementPosition, 0.1f);

  CBaseViewModel *vm = pOwner->GetViewModel();

  if (vm != NULL) {
    // This has to happen here because of how the SetModel interacts with the
    // caching at startup
...
void CWeaponPhysCannon::CheckForTarget(void) {
  ...
  if ((tr.fraction != 1.0f) && (tr.m_pEnt != NULL)) {
    ...
    if (dist <= TraceLength()) {
      // TODO(d.rattman): Try just having the elements always open when pointed at a
      // physics object
...
void CWeaponPhysCannon::BeginUpgrade() {
  if (IsMegaPhysCannon()) return;

  if (m_bIsCurrentlyUpgrading) return;
  ...
  SetContextThink(&CWeaponPhysCannon::WaitForUpgradeThink,
                  gpGlobals->curtime + 6.0f, s_pWaitForUpgradeContext);
...
void CWeaponPhysCannon::WaitForUpgradeThink() {
  ...
  if (!GlobalEntity_IsInTable("super_phys_gun")) {
    GlobalEntity_Add(MAKE_STRING("super_phys_gun"), gpGlobals->mapname,
                     GLOBAL_ON);
  } else {
  ...
  CBaseEntity *pEnt =
      gEntList.FindEntityByName(NULL, "script_physcannon_upgrade");
  if (pEnt) {
...
void CWeaponPhysCannon::DoEffectIdle(void) {
  ...
  if ((m_hEndSprites[0] != NULL) && (m_hEndSprites[1] != NULL)) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      m_hEndSprites[i]->SetScale(random->RandomFloat(0.1, 0.15) *
                                 flScaleFactor);
    }
  }
  ...
  for (int i = 0; i < NUM_SPRITES; i++) {
    if (m_hGlowSprites[i] == NULL) continue;

    if (IsMegaPhysCannon()) {
      m_hGlowSprites[i]->SetBrightness(random->RandomInt(32, 48));
      m_hGlowSprites[i]->SetScale(random->RandomFloat(0.15, 0.2) *
                                  flScaleFactor);
    } else {
      m_hGlowSprites[i]->SetBrightness(random->RandomInt(16, 24));
      m_hGlowSprites[i]->SetScale(random->RandomFloat(0.3, 0.35) *
                                  flScaleFactor);
    }
  }
  ...
  if (IsMegaPhysCannon()) {
    ...
    if (m_hCenterSprite != NULL) {
      if (m_EffectState == EFFECT_HOLDING) {
        ...
        m_hCenterSprite->SetScale(random->RandomFloat(0.2, 0.25) *
                                  flScaleFactor);
      } else {
        m_hCenterSprite->SetBrightness(random->RandomInt(32, 64));
        m_hCenterSprite->SetScale(random->RandomFloat(0.125, 0.15) *
                                  flScaleFactor);
      }
    }
    ...
    if (m_hBlastSprite != NULL) {
      if (m_EffectState == EFFECT_HOLDING) {
        ...
        m_hBlastSprite->SetScale(random->RandomFloat(0.125, 0.15) *
                                 flScaleFactor);
      } else {
        m_hBlastSprite->SetBrightness(random->RandomInt(32, 64));
        m_hBlastSprite->SetScale(random->RandomFloat(0.075, 0.15) *
                                 flScaleFactor);
      }
    }
  }
...
void CWeaponPhysCannon::LaunchObject(const Vector &vecDir, float flForce) {
  ...
  if (m_grabController.GetAttached()) {
    ...
    // Trace ahead a bit and make a chain of danger sounds ahead of the phys
    // object to scare potential targets
    trace_t tr;
    ...
    UTIL_TraceLine(vecStart, vecStart + vecDir * flForce, MASK_SHOT, pObject,
                   COLLISION_GROUP_NONE, &tr);
    iLength = (tr.startpos - tr.endpos).Length();
    vecSpot = vecStart + vecDir * PHYSCANNON_DANGER_SOUND_RADIUS;

    for (i = PHYSCANNON_DANGER_SOUND_RADIUS; i < iLength;
         i += PHYSCANNON_DANGER_SOUND_RADIUS) {
      CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER, vecSpot,
                             PHYSCANNON_DANGER_SOUND_RADIUS, 0.5, pObject);
...
bool CWeaponPhysCannon::CanPickupObject(CBaseEntity *pTarget) {
  ...
  if (pTarget->HasSpawnFlags(SF_PHYSBOX_ALWAYS_PICK_UP) ||
      pTarget->HasSpawnFlags(SF_PHYSBOX_NEVER_PICK_UP)) {
    // It may seem strange to check this spawnflag before we know the class of
    // this object, since the spawnflag only applies to func_physbox, but it can
    // act as a filter of sorts to reduce the number of irrelevant entities that
    // fall through to this next casting check, which is slower.
  ...
  if (pTarget->HasSpawnFlags(SF_PHYSPROP_ALWAYS_PICK_UP)) {
    ...
    if (pPhysProp != NULL) return true;
  }

  if (pTarget->IsEFlagSet(EFL_NO_PHYSCANNON_INTERACTION)) return false;

  CBasePlayer *pOwner = ToBasePlayer(GetOwner());
  if (pOwner && pOwner->GetGroundEntity() == pTarget) return false;
  ...
  if (dynamic_cast<CRagdollProp *>(pTarget)) return true;
...
void CWeaponPhysCannon::OpenElements(void) {
  ...
  if (m_flElementPosition < 0.0f) m_flElementPosition = 0.0f;
...
void CWeaponPhysCannon::CloseElements(void) {
  ...
  if (m_flElementPosition > 1.0f) m_flElementPosition = 1.0f;
      ...
      m_sndMotor =
          (CSoundEnvelopeController::GetController())
              .SoundCreate(filter, entindex(), CHAN_STATIC,
                           "Weapon_MegaPhysCannon.HoldSound", ATTN_NORM);
    } else {
...
void CWeaponPhysCannon::StartEffects(void) {
  ...
  for (i = 0; i < NUM_BEAMS; i++) {
    if (m_hBeams[i]) continue;

    const char *beamAttachNames[] = {
        "fork1t", "fork2t", "fork1t", "fork2t", "fork1t", "fork2t",
    };
  ...
  for (i = 0; i < NUM_SPRITES; i++) {
    if (m_hGlowSprites[i]) continue;

    const char *attachNames[] = {"fork1b", "fork1m", "fork1t",
                                 "fork2b", "fork2m", "fork2t"};
    ...
    m_hGlowSprites[i]->SetAttachment(pOwner->GetViewModel(),
                                     LookupAttachment(attachNames[i]));

    if (bIsMegaCannon) {
      m_hGlowSprites[i]->SetTransparency(kRenderTransAdd, 255, 255, 255, 128,
                                         kRenderFxNone);
    } else {
      m_hGlowSprites[i]->SetTransparency(kRenderTransAdd, 255, 128, 0, 64,
  ...
  for (i = 0; i < 2; i++) {
    if (m_hEndSprites[i] == NULL) {
      const char *attachNames[] = {"fork1t", "fork2t"};
      ...
      m_hEndSprites[i]->SetAttachment(pOwner->GetViewModel(),
                                      LookupAttachment(attachNames[i]));
      m_hEndSprites[i]->SetTransparency(kRenderTransAdd, 255, 255, 255, 255,
                                        kRenderFxNoDissipation);
  ...
  if (m_hCenterSprite == NULL) {
    ...
    m_hCenterSprite->SetTransparency(kRenderTransAdd, 255, 255, 255, 255,
                                     kRenderFxNone);
  ...
  if (m_hBlastSprite == NULL) {
    ...
    m_hBlastSprite->SetTransparency(kRenderTransAdd, 255, 255, 255, 255,
                                    kRenderFxNone);
...
void CWeaponPhysCannon::DoEffectLaunch(Vector *pos) {
  ...
  CBeam *pBeam = CBeam::BeamCreate(
      IsMegaPhysCannon() ? MEGACANNON_BEAM_SPRITE : PHYSCANNON_BEAM_SPRITE, 8);
  ...
  te->GaussExplosion(filter, 0.0f, endpos - (shotDir * 4.0f),
                     RandomVector(-1.0f, 1.0f), 0);
...
void CWeaponPhysCannon::DoMegaEffectLaunch(Vector *pos) {
  ...
  CBeam *pBeam = CBeam::BeamCreate(
      IsMegaPhysCannon() ? MEGACANNON_BEAM_SPRITE : PHYSCANNON_BEAM_SPRITE,
      0.8);
...
void CWeaponPhysCannon::RecordThrownObject(CBaseEntity *pObject) {
  ...
  for (int i = 0; i < count; i++) {
    if (m_ThrownEntities[i].hEntity == pObject) {
      ...
      // Msg("++UPDATING: %s (%d)\n",
      // m_ThrownEntities[i].hEntity->GetClassname(),
      // m_ThrownEntities[i].hEntity->entindex() );
  ...
  do {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!m_ThrownEntities[i].hEntity.Get()) {
        bRemove = true;
      } else if (gpGlobals->curtime > (m_ThrownEntities[i].fTimeThrown +
                                       PHYSCANNON_THROWN_LIST_TIMEOUT)) {
        bRemove = true;
      } else {
        IPhysicsObject *pObject =
            m_ThrownEntities[i].hEntity->VPhysicsGetObject();
...
bool CWeaponPhysCannon::IsAccountableForObject(CBaseEntity *pObject) {
  ...
  // Now if this object is in the list, the player is accountable for it
  // striking something.
  int count = m_ThrownEntities.Count();
...
void PhysCannonBeginUpgrade(CBaseAnimating *pAnim) {
  CWeaponPhysCannon *pWeaponPhyscannon =
      assert_cast<CWeaponPhysCannon *>(pAnim);
...
CBaseEntity *GetPlayerHeldEntity(CBasePlayer *pPlayer) {
  ...
  CPlayerPickupController *pPlayerPickupController =
      (CPlayerPickupController *)(pPlayer->GetUseEntity());
