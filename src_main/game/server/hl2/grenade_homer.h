...
class CGrenadeHomer : public CBaseGrenade {
 public:
  ...
  static CGrenadeHomer *CreateGrenadeHomer(string_t nModelName,
                                           string_t sFlySound,
  ...
  void Spawn(void);
  void Launch(CBaseEntity *pOwner, CBaseEntity *pTarget,
              const Vector &vInitVelocity, float m_flHomingSpeed,
              float fFallSpeed, int nRocketTrailType);
  void SetSpin(float flSpinMagnitude, float flSpinSpeed);
  void SetHoming(float flStrength, float flDelay, float flRampUp,
