...
int g_fCombineQuestion;  // true if an idle grunt asked a question. Cleared when
                         // someone answers. YUCK old global from grunt code
...
#define COMBINE_GRENADE_FLUSH_TIME \
  3.0  // Don't try to flush an enemy who has been out of sight for longer than
       // this.
#define COMBINE_GRENADE_FLUSH_DIST \
  256.0  // Don't try to flush an enemy who has moved farther than this distance
         // from the last place I saw him.
...
#define COMBINE_AE_CAUGHT_ENEMY \
  (10)  // grunt established sight with an enemy (player only) that had
        // previously eluded the squad.
...
enum TacticalVariant_T {
  ...
  TACTICAL_VARIANT_PRESSURE_ENEMY_UNTIL_CLOSE,  // Act like
                                                // VARIANT_PRESSURE_ENEMY, but
                                                // go to VARIANT_DEFAULT once
                                                // within 30 feet
};
...
BEGIN_DATADESC(CNPC_Combine)
  ...
  DEFINE_FIELD(m_nKickDamage, FIELD_INTEGER),
      ...
      //							m_AssaultBehavior
      //(auto saved by AI)
      //m_StandoffBehavior (auto saved by AI) 							m_FollowBehavior (auto saved by
      //AI)
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "ThrowGrenadeAtTarget",
      ...
      DEFINE_KEYFIELD(m_iPathfindingVariant, FIELD_INTEGER,
                      "pathfindingvariant"),
...
void CNPC_Combine::InputAssault(inputdata_t &inputdata) {
  m_AssaultBehavior.SetParameters(AllocPooledString(inputdata.value.String()),
                                  CUE_DONT_WAIT, RALLY_POINT_SELECT_DEFAULT);
}
...
//			If I'm a combine elite, fire my combine ball at the
//target instead.
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CNPC_Combine::InputThrowGrenadeAtTarget(inputdata_t &inputdata) {
  ...
  if (!pEntity) {
    DevMsg(
        "%s (%s) received ThrowGrenadeAtTarget input, but couldn't find target "
        "entity '%s'\n",
...
void CNPC_Combine::Spawn(void) {
  ...
  // bits_CAP_MOVE_JUMP | bits_CAP_MOVE_CLIMB);
  ...
  m_bFirstEncounter = true;  // this is true when the grunt spawns, because he
                             // hasn't encountered an enemy yet.
...
void CNPC_Combine::GatherConditions() {
  ...
  if (GetState() == NPC_STATE_COMBAT) {
    if (IsCurSchedule(SCHED_COMBINE_WAIT_IN_COVER, false)) {
      // Soldiers that are standing around doing nothing poll for attack slots
      // so that they can respond quickly when one comes available. If they can
      // occupy a vacant attack slot, they do so. This holds the slot until
      // their schedule breaks and schedule selection runs again, essentially
      // reserving this slot. If they do not select an attack schedule, then
      // they'll release the slot.
    ...
    if (IsUsingTacticalVariant(TACTICAL_VARIANT_PRESSURE_ENEMY_UNTIL_CLOSE)) {
      if (GetEnemy() != NULL && !HasCondition(COND_ENEMY_OCCLUDED)) {
        ...
        if (GetAbsOrigin().DistToSqr(GetEnemy()->GetAbsOrigin()) <
            Square(30.0f * 12.0f))
...
void CNPC_Combine::PrescheduleThink() {
  ...
  if (m_flGroundSpeed > 0 && GetState() == NPC_STATE_COMBAT &&
      m_MoveAndShootOverlay.IsSuspended()) {
    // Return to move and shoot when near my goal so that I 'tuck into' the
    // location facing my enemy.
    if (GetNavigator()->GetPathTimeToGoal() <= 1.0f) {
...
bool CNPC_Combine::ShouldMoveAndShoot() {
  ...
  // "It's OK to move and shoot until gpGlobals->curtime ==
  // m_flStopMoveShootTime"
  m_flStopMoveShootTime = FLT_MAX;

  if (IsCurSchedule(SCHED_COMBINE_HIDE_AND_RELOAD, false))
    m_flStopMoveShootTime =
        gpGlobals->curtime + random->RandomFloat(0.4f, 0.6f);
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Combine::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                      float flInterval) {
...
void CNPC_Combine::StartTaskChaseEnemyContinuously(const Task_t *pTask) {
  ...
  if (WorldSpaceCenter().DistToSqr(pEnemy->WorldSpaceCenter()) <=
      pTask->flTaskData * pTask->flTaskData) {
    TaskComplete();
    return;
  }
...
void CNPC_Combine::RunTaskChaseEnemyContinuously(const Task_t *pTask) {
  ...
  if (m_vSavePosition.DistToSqr(pEnemy->WorldSpaceCenter()) <
      (pTask->flTaskData * pTask->flTaskData))
    return;
...
void CNPC_Combine::StartTask(const Task_t *pTask) {
  // NOTE: This reset is required because we change it in
  ...
  switch (pTask->iTask) {
    ...
    case TASK_COMBINE_PLAY_SEQUENCE_FACE_ALTFIRE_TARGET:
      ...
      GetMotor()->SetIdealYawToTargetAndUpdate(m_vecAltFireTarget,
                                               AI_KEEP_YAW_SPEED);
      break;
    ...
    case TASK_COMBINE_GET_PATH_TO_FORCED_GREN_LOS: {
      ...
      if (!found &&
          GetTacticalServices()->FindLos(vecEnemy, vecEnemyEye, flMinRange,
    ...
    case TASK_COMBINE_IGNORE_ATTACKS:
      ...
      if (m_pSquad && m_pSquad->NumMembers() > 2) {
        ...
        if (GetEnemy() &&
            (GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter()).Length() >
                512.0) {
    ...
    case TASK_COMBINE_DEFER_SQUAD_GRENADES: {
      if (m_pSquad) {
        // iterate my squad and stop everyone from throwing grenades for a
        // little while.
        AISquadIter_t iter;

        CAI_BaseNPC *pSquadmate =
            m_pSquad ? m_pSquad->GetFirstMember(&iter) : NULL;
        while (pSquadmate) {
          CNPC_Combine *pCombine = dynamic_cast<CNPC_Combine *>(pSquadmate);
    ...
    case TASK_FACE_ENEMY: {
      if (pTask->iTask == TASK_FACE_ENEMY &&
    ...
    case TASK_FIND_COVER_FROM_ENEMY: {
      if (GetHintGroup() == NULL_STRING) {
        ...
        if (pEntity) {
          ...
          if (!HasMemory(bits_MEMORY_PLAYER_HURT) && pEntity->IsPlayer() &&
              pEntity->GetHealth() <= 20) {
            if (m_pSquad) {
          ...
          if (pEntity->MyNPCPointer()) {
            if (!(pEntity->MyNPCPointer()->CapabilitiesGet() &
                  bits_CAP_WEAPON_RANGE_ATTACK1) &&
                !(pEntity->MyNPCPointer()->CapabilitiesGet() &
  ...
  switch (pTask->iTask) {
    ...
    case TASK_COMBINE_FACE_TOSS_DIR: {
      ...
      GetMotor()->SetIdealYawToTargetAndUpdate(
          GetLocalOrigin() + m_vecTossVelocity * 64, AI_KEEP_YAW_SPEED);
...
Vector CNPC_Combine::BodyTarget(const Vector &posSrc, bool bNoisy) {
  ...
  // @TODO (toml 02-02-04): this seems wrong. Isn't this already be accounted
  // for with the eye position used in the base BodyTarget()
...
// Purpose:
//------------------------------------------------------------------------------
bool CNPC_Combine::FVisible(CBaseEntity *pEntity, int traceMask,
...
void CNPC_Combine::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (GetActivity() == ACT_RANGE_ATTACK2) {
    if (m_iLastAnimEventHandled != COMBINE_AE_GREN_TOSS) {
      ...
      CBaseEntity *pItem =
          DropItem("weapon_frag", vecStart, RandomAngle(0, 360));
      ...
      if (pItem) {
        ...
        if (PlayerHasMegaPhysCannon()) {
          ...
          pWeapon->Dissolve(NULL, gpGlobals->curtime, false,
...
//-----------------------------------------------------------------------------
bool CNPC_Combine::UpdateEnemyMemory(CBaseEntity *pEnemy,
...
Activity CNPC_Combine::NPC_TranslateActivity(Activity eNewActivity) {
  // Slaming this back to ACT_COMBINE_BUGBAIT since we don't want ANYTHING to
  // change our activity while we burn.
  ...
  if (eNewActivity == ACT_RANGE_ATTACK2) {
    ...
    // grenade or fired grenade, we must determine which and pick proper
    // sequence
    ...
    if (!IsCrouching() &&
        (m_NPCState == NPC_STATE_COMBAT || m_NPCState == NPC_STATE_ALERT)) {
...
bool CNPC_Combine::QueryHearSound(CSound *pSound) {
  if (pSound->SoundContext() & SOUND_CONTEXT_COMBINE_ONLY) return true;
...
void CNPC_Combine::AnnounceAssault(void) {
  if (random->RandomInt(0, 5) > 1) return;
...
int CNPC_Combine::SelectCombatSchedule() {
  ...
  if (HasCondition(COND_NEW_ENEMY)) {
    ...
    float flTimeSinceFirstSeen =
        gpGlobals->curtime - GetEnemies()->FirstTimeSeen(pEnemy);
    ...
    if (m_pSquad && pEnemy) {
      ...
      if (HasCondition(COND_CAN_RANGE_ATTACK1) &&
          OccupyStrategySlot(SQUAD_SLOT_ATTACK1)) {
        // Start suppressing if someone isn't firing already (SLOT_ATTACK1).
        // This means I'm the guy who spotted the enemy, I should react
        // immediately.
      ...
      if (m_pSquad->IsLeader(this) ||
          (m_pSquad->GetLeader() &&
           m_pSquad->GetLeader()->GetEnemy() != pEnemy)) {
        // I'm the leader, but I didn't get the job suppressing the enemy. We
        // know this because This code only runs if the code above didn't assign
        // me SCHED_COMBINE_SUPPRESS.
        if (HasCondition(COND_CAN_RANGE_ATTACK1) &&
        ...
        if (HasCondition(COND_WEAPON_HAS_LOS) &&
            IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1,
                                        SQUAD_SLOT_ATTACK2)) {
          // If everyone else is attacking and I have line of fire, wait for a
          // chance to cover someone.
        ...
        if (m_pSquad->GetLeader() && FOkToMakeSound(SENTENCE_PRIORITY_MEDIUM)) {
          JustMadeSound(
              SENTENCE_PRIORITY_MEDIUM);  // squelch anything that isn't high
                                          // priority so the leader can speak
        }
  ...
  if (HasCondition(COND_LIGHT_DAMAGE)) {
    if (GetEnemy() != NULL) {
      ...
      if (!IsCrouching()) {
        if (GetEnemy() && random->RandomFloat(0, 100) < 50 &&
            CouldShootIfCrouching(GetEnemy())) {
          Crouch();
        } else {
  ...
  if (IRelationType(GetEnemy()) == D_FR) {
    if (HasCondition(COND_SEE_ENEMY) || HasCondition(COND_SEE_FEAR) ||
  ...
  if (attackSchedule != SCHED_NONE) return attackSchedule;
  ...
  if (HasCondition(COND_ENEMY_OCCLUDED)) {
    ...
    // If I'm a long, long way away, establish a LOF anyway. Once I get there
    // I'll start respecting the squad slots again.
    float flDistSq =
        GetEnemy()->WorldSpaceCenter().DistToSqr(WorldSpaceCenter());
    if (flDistSq > Square(3000)) return SCHED_ESTABLISH_LINE_OF_FIRE;
  ...
  if (HasCondition(COND_SEE_ENEMY) && !HasCondition(COND_CAN_RANGE_ATTACK1)) {
    if ((HasCondition(COND_TOO_FAR_TO_ATTACK) ||
         IsUsingTacticalVariant(TACTICAL_VARIANT_PRESSURE_ENEMY)) &&
        OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2)) {
      return SCHED_COMBINE_PRESS_ATTACK;
    }
...
int CNPC_Combine::SelectSchedule(void) {
  ...
  if (m_hForcedGrenadeTarget) {
    ...
    // Can't throw at the target, so lets try moving to somewhere where I can
    // see it
    if (!FVisible(m_hForcedGrenadeTarget)) {
  ...
  if (m_NPCState != NPC_STATE_SCRIPT) {
    ...
    // We've been told to move away from a target to make room for a grenade to
    // be thrown at it
    ...
    if (m_NPCState != NPC_STATE_DEAD && m_NPCState != NPC_STATE_PRONE) {
      ...
      if (HasCondition(COND_HEAR_DANGER)) {
        ...
        if (pSound) {
          if (pSound->m_iType & SOUND_DANGER) {
            ...
            if (pSoundOwner) {
              CBaseGrenade *pGrenade =
                  dynamic_cast<CBaseGrenade *>(pSoundOwner);
            ...
            m_Sentences.Speak(pSentenceName, SENTENCE_PRIORITY_NORMAL,
                              SENTENCE_CRITERIA_NORMAL);

            // If the sound is approaching danger, I have no enemy, and I don't
            // see it, turn to face.
            if (!GetEnemy() &&
                pSound->IsSoundType(SOUND_CONTEXT_DANGER_APPROACH) &&
                pSound->m_hOwner &&
                !FInViewCone(pSound->GetSoundReactOrigin())) {
          ...
          if (!HasCondition(COND_SEE_ENEMY) &&
              (pSound->m_iType & (SOUND_PLAYER | SOUND_COMBAT))) {
            GetMotor()->SetIdealYawToTarget(pSound->GetSoundReactOrigin());
          }
        }
      }
  ...
  switch (m_NPCState) {
    case NPC_STATE_IDLE: {
      if (m_bShouldPatrol) return SCHED_COMBINE_PATROL;
    }
      // NOTE: Fall through!
    ...
    case NPC_STATE_ALERT: {
      if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE)) {
        ...
        if (pDanger && FInViewCone(pDanger->vLastKnownLocation) &&
            !BaseClass::FVisible(pDanger->vLastKnownLocation)) {
          // I've been hurt, I'm facing the danger, but I don't see it, so move
          // from this position.
      ...
      if (HasCondition(COND_HEAR_COMBAT)) {
        ...
        if (pSound && pSound->IsSoundType(SOUND_COMBAT)) {
          if (m_pSquad &&
              m_pSquad->GetSquadMemberNearestTo(
                  pSound->GetSoundReactOrigin()) == this &&
      ...
      // Don't patrol if I'm in the middle of an assault, because I'll never
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Combine::SelectFailSchedule(int failedSchedule, int failedTask,
  ...
  if (failedSchedule == SCHED_COMBINE_TAKE_COVER1) {
    if (IsInSquad() &&
      ...
      // This eases the effects of an unfortunate bug that usually plagues
      // shotgunners. Since their rate of fire is low, they spend relatively
      // long periods of time without an attack squad slot. If you corner a
      // shotgunner, usually the other memebers of the squad will hog all of the
      // attack slots and pick schedules to move to establish line of fire.
      // During this time, the shotgunner is prevented from attacking. If he
      // also cannot find cover (the fallback case) he will stand around like an
      // idiot, right in front of you. Instead of this, we have him run up to
      // you for a melee attack.
...
bool CNPC_Combine::ShouldChargePlayer() {
  return GetEnemy() && GetEnemy()->IsPlayer() && PlayerHasMegaPhysCannon() &&
         !IsLimitingHintGroups();
}
...
#define COMBINE_MEGA_PHYSCANNON_ATTACK_DISTANCE_SQ \
  (COMBINE_MEGA_PHYSCANNON_ATTACK_DISTANCE *       \
   COMBINE_MEGA_PHYSCANNON_ATTACK_DISTANCE)
...
int CNPC_Combine::SelectScheduleAttack() {
  ...
  // If I'm fighting a combine turret (it's been hacked to attack me), I can't
  // really hurt it with bullets, so become grenade happy.
  if (GetEnemy() && GetEnemy()->Classify() == CLASS_COMBINE &&
      FClassnameIs(GetEnemy(), "npc_turret_floor")) {
    ...
    // If we're not in the viewcone of the turret, run up and hit it. Do this a
    // bit later to give other squadmembers a chance to throw a grenade before I
    // run in.
  ...
  if (GetEnemy() && !HasCondition(COND_SEE_ENEMY)) {
    ...
    flDist = (GetEnemy()->GetAbsOrigin() -
              GetEnemies()->LastSeenPosition(GetEnemy()))
                 .Length();

    // Msg("Time: %f   Dist: %f\n", flTime, flDist );
    if (flTime <= COMBINE_GRENADE_FLUSH_TIME &&
        flDist <= COMBINE_GRENADE_FLUSH_DIST && CanGrenadeEnemy(false) &&
        OccupyStrategySlot(SQUAD_SLOT_GRENADE1)) {
      return SCHED_RANGE_ATTACK2;
    }
  }
  ...
  if (HasCondition(COND_WEAPON_SIGHT_OCCLUDED)) {
    ...
    if (pBlocker && pBlocker->GetHealth() > 0 &&
        OccupyStrategySlot(SQUAD_SLOT_ATTACK_OCCLUDER)) {
...
int CNPC_Combine::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    case SCHED_TAKE_COVER_FROM_ENEMY: {
      if (m_pSquad) {
        // Have to explicitly check innate range attack condition as may have
        // weapon with range attack 2
    ...
    case SCHED_ESTABLISH_LINE_OF_FIRE: {
      ...
      if (CanAltFireEnemy(true) &&
        ...
        // If an elite in the squad could fire a combine ball at the player's
        // last known position, do so!
        return SCHED_COMBINE_AR2_ALTFIRE;
      }
    ...
    case SCHED_HIDE_AND_RELOAD: {
      ...
      // DesireStand();
      if (CanGrenadeEnemy() && OccupyStrategySlot(SQUAD_SLOT_GRENADE1) &&
          random->RandomInt(0, 100) < 20) {
        // If I COULD throw a grenade and I need to reload, 20% chance I'll
        // throw a grenade before I hide to reload.
    ...
    case SCHED_RANGE_ATTACK1: {
      ...
      if (CanAltFireEnemy(true) &&
          OccupyStrategySlot(SQUAD_SLOT_SPECIAL_ATTACK)) {
        // Since I'm holding this squadslot, no one else can try right now. If I
        // die before the shot goes off, I won't have affected anyone else's
        // ability to use this attack at their nearest convenience.
        return SCHED_COMBINE_AR2_ALTFIRE;
      }

      if (IsCrouching() ||
          (CrouchIsDesired() && !HasCondition(COND_HEAVY_DAMAGE))) {
        // See if we can crouch and shoot
        if (GetEnemy() != NULL) {
          float dist =
              (GetLocalOrigin() - GetEnemy()->GetLocalOrigin()).Length();
          ...
          if (dist > COMBINE_MIN_CROUCH_DISTANCE) {
            ...
            Vector targetPos =
                GetEnemy()->BodyTarget(GetActiveWeapon()->GetLocalOrigin());
    ...
    case SCHED_RANGE_ATTACK2: {
      ...
      if (GetActiveWeapon() && GetActiveWeapon()->CapabilitiesGet() &
...
#define MIN_SIGNAL_DIST 256
      if (GetEnemy() != NULL && GetEnemy()->IsPlayer() && m_bFirstEncounter) {
        ...
        if (flDistToEnemy >= MIN_SIGNAL_DIST) {
          m_bFirstEncounter =
              false;  // after first encounter, leader won't issue handsigns
                      // anymore when he has a new enemy
  ...
  if (pEvent->type & AE_TYPE_NEWEVENTSYSTEM) {
    if (pEvent->event == COMBINE_AE_BEGIN_ALTFIRE) {
      ...
      if (IsElite()) {
        ...
        // I'm disabling this decrementor. At the time of this change, the
        // elites don't bother to check if they have grenades anyway. This means
        // that all elites have infinite combine balls, even if the designer
        // marks the elite as having 0 grenades. By disabling this decrementor,
        // yet enabling the code that makes sure the elite has grenades in order
        // to fire a combine ball, we preserve the legacy behavior while making
        // it possible for a designer to prevent elites from shooting combine
        // balls by setting grenades to '0' in hammer. (sjb) EP2_OUTLAND_10
        // m_iNumGrenades--;
      }
    ...
    switch (pEvent->event) {
      ...
      case COMBINE_AE_GREN_TOSS: {
        ...
        if (m_NPCState == NPC_STATE_SCRIPT) {
          ...
          // Grenades thrown from a script do not count against grenades
          // remaining for the AI to use.
          Vector forward, up, vecThrow;

          GetVectors(&forward, NULL, &up);
          vecThrow = forward * 750 + up * 175;
          Fraggrenade_Create(vecStart, vec3_angle, vecThrow, vecSpin, this,
                             COMBINE_GRENADE_TIMER, true);
        } else {
          // Use the Velocity that AI gave us.
          Fraggrenade_Create(vecStart, vec3_angle, m_vecTossVelocity, vecSpin,
                             this, COMBINE_GRENADE_TIMER, true);
          m_iNumGrenades--;
        }

        // wait six seconds before even looking again to see if a grenade can be
        // thrown.
      ...
      case COMBINE_AE_GREN_LAUNCH: {
        ...
        CBaseEntity *pGrenade = CreateNoSpawn(
            "npc_contactgrenade", Weapon_ShootPosition(), vec3_angle, this);
        ...
        if (g_pGameRules->IsSkillLevel(SKILL_HARD))
          m_flNextGrenadeCheck =
              gpGlobals->curtime +
              random->RandomFloat(
                  2, 5);  // wait a random amount of time before shooting again
        else
          m_flNextGrenadeCheck = gpGlobals->curtime +
                                 6;  // wait six seconds before even looking
                                     // again to see if a grenade can be thrown.
      }
        handledEvent = true;
        break;
      ...
      case COMBINE_AE_GREN_DROP: {
        ...
        Fraggrenade_Create(vecStart, vec3_angle, m_vecTossVelocity, vec3_origin,
                           this, COMBINE_GRENADE_TIMER, true);
        m_iNumGrenades--;
      }
        handledEvent = true;
      ...
      case COMBINE_AE_KICK: {
        // Does no damage, because damage is applied based upon whether the
        // target can handle the interaction
        CBaseEntity *pHurt = CheckTraceHullAttack(
            70, -Vector(16, 16, 18), Vector(16, 16, 18), 0, DMG_CLUB);
        CBaseCombatCharacter *pBCC = ToBaseCombatCharacter(pHurt);
        if (pBCC) {
          Vector forward, up;
          AngleVectors(GetLocalAngles(), &forward, NULL, &up);

          if (!pBCC->DispatchInteraction(g_interactionCombineBash, NULL,
                                         this)) {
            if (pBCC->IsPlayer()) {
...
// BUGBUG: It looks like this is never played because combine don't do
// SCHED_WAKE_ANGRY or anything else that does a TASK_SOUND_WAKE
...
void CNPC_Combine::NotifyDeadFriend(CBaseEntity *pFriend) {
  if (GetSquad()->NumMembers() < 2) {
    m_Sentences.Speak("COMBINE_LAST_OF_SQUAD", SENTENCE_PRIORITY_INVALID,
                      SENTENCE_CRITERIA_NORMAL);
    JustMadeSound();
    return;
  }
...
void CNPC_Combine::DeathSound(void) {
  ...
  m_Sentences.Speak("COMBINE_DIE", SENTENCE_PRIORITY_INVALID,
                    SENTENCE_CRITERIA_ALWAYS);
}
...
// Purpose:
//
//			This is for Grenade attacks.  As the test for grenade
// attacks 			is expensive we don't want to do it every frame.
// Return true 			if we meet minimum set of requirements and then
// test for actual 			throw later if we actually decide to do
// a grenade attack.
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
bool CNPC_Combine::CanThrowGrenade(const Vector &vecTarget) {
  ...
  // ---------------------------------------------------------------------
  if (m_pSquad) {
    if (m_pSquad->SquadMemberInRange(vecTarget,
      ...
      CSoundEnt::InsertSound(SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY,
                             vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST, 0.1);
      return false;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the combine can throw a grenade at the specified
// target point Input  : &vecTarget - Output : Returns true on success, false on
// failure.
//-----------------------------------------------------------------------------
bool CNPC_Combine::CheckCanThrowGrenade(const Vector &vecTarget) {
  ...
  if (FInViewCone(vecTarget) && CBaseEntity::FVisible(vecTarget)) {
    vecToss =
        VecCheckThrow(this, EyePosition(), vecTarget,
                      COMBINE_GRENADE_THROW_SPEED, 1.0, &vecMins, &vecMaxs);
  } else {
    // Have to try a high toss. Do I have enough room?
    trace_t tr;
    AI_TraceLine(EyePosition(), EyePosition() + Vector(0, 0, 64), MASK_SHOT,
                 this, COLLISION_GROUP_NONE, &tr);
    if (tr.fraction != 1.0) {
      return false;
    }

    vecToss = VecCheckToss(this, EyePosition(), vecTarget, -1, 1.0, true,
                           &vecMins, &vecMaxs);
  }
...
bool CNPC_Combine::CanAltFireEnemy(bool bUseFreeKnowledge) {
  ...
  // See Steve Bond if you plan on changing this next piece of code!! (SJB)
  // EP2_OUTLAND_10
  ...
  if (!pEnemy->IsPlayer() &&
      (!pEnemy->IsNPC() || !pEnemy->MyNPCPointer()->IsPlayerAlly()))
    return false;
  ...
  if (bUseFreeKnowledge) {
    vecTarget = GetEnemies()->LastKnownPosition(pEnemy) +
                (pEnemy->GetViewOffset() * 0.75);  // approximates the chest
  } else {
    vecTarget = GetEnemies()->LastSeenPosition(pEnemy) +
                (pEnemy->GetViewOffset() * 0.75);  // approximates the chest
  }

  // Trace a hull about the size of the combine ball (don't shoot through
  // grates!)
  trace_t tr;
  ...
  UTIL_TraceHull(vShootPosition, vecTarget, mins, maxs, MASK_SHOT, this,
  ...
  // If the ball can travel at least 65% of the distance to the player then let
  // the NPC shoot it.
...
Vector CNPC_Combine::EyeOffset(Activity nActivity) {
  if (CapabilitiesGet() & bits_CAP_DUCK) {
    if (IsCrouchedActivity(nActivity)) return COMBINE_EYE_CROUCHING_POSITION;
  }
...
bool CNPC_Combine::OnBeginMoveAndShoot() {
  if (BaseClass::OnBeginMoveAndShoot()) {
    ...
    if (!HasStrategySlotRange(SQUAD_SLOT_GRENADE1,
                              SQUAD_SLOT_ATTACK_OCCLUDER) &&
        OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2))
      return true;
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CNPC_Combine::CalcWeaponProficiency(
...
bool CNPC_Combine::HasShotgun() {
  if (GetActiveWeapon() &&
      GetActiveWeapon()->m_iClassname == s_iszShotgunClassname) {
    return true;
  }
...
//-----------------------------------------------------------------------------
bool CNPC_Combine::HandleInteraction(int interactionType, void *data,
...
bool CNPC_Combine::IsUsingTacticalVariant(int variant) {
  if (variant == TACTICAL_VARIANT_PRESSURE_ENEMY &&
...
bool CNPC_Combine::IsRunningApproachEnemySchedule() {
  if (IsCurSchedule(SCHED_CHASE_ENEMY)) return true;
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_COMBINE_RUN_AWAY_FROM_BEST_SOUND"
    "		 TASK_STOP_MOVING					0"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_FACE_ENEMY				0"
    "		TASK_SET_SCHEDULE			"
    "SCHEDULE:SCHED_COMBINE_OVERWATCH"
    ""
    "	Interrupts"
    "		COND_HEAR_DANGER"
    "		COND_NEW_ENEMY")
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE		"
    "SCHEDULE:SCHED_COMBINE_TAKECOVER_FAILED"
    "		TASK_STOP_MOVING				0"
    "		TASK_WAIT					0.2"
    "		TASK_FIND_COVER_FROM_ENEMY	0"
    "		TASK_RUN_PATH				0"
    ...
    "		TASK_REMEMBER				MEMORY:INCOVER"
    "		TASK_SET_SCHEDULE			"
    "SCHEDULE:SCHED_COMBINE_WAIT_IN_COVER"
    ""
    "	Interrupts")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_STOP_MOVING				0"
    "		TASK_FACE_ENEMY					0"
    "		TASK_ANNOUNCE_ATTACK			1"  // 1 = primary
                                                            // attack
    ...
    //								comibine will fire where player
    //was
    // after 								he has moved for a little while.  Good
    // effect!!
    // WEAPON_SIGHT_OCCLUDED		Don't block on this! Looks better for
    // railings, etc.
)
...
//
//	secondary range attack. Overriden because base class stops attacking
// when the enemy is occluded. 	combines's grenade toss requires the enemy be
// occluded.
//=========================================================
DEFINE_SCHEDULE(
    SCHED_COMBINE_RANGE_ATTACK2,
    ...
    "		TASK_COMBINE_DEFER_SQUAD_GRENADES	0"
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_COMBINE_WAIT_IN_COVER"  // don't run immediately after
                                            // throwing grenade.
    ""
    "	Interrupts")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_COMBINE_DEFER_SQUAD_GRENADES	0"
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_HIDE_AND_RELOAD"  // don't run immediately after throwing
                                      // grenade.
    ""
    "	Interrupts")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_WAIT_RANDOM				3"
    "		TASK_SET_SCHEDULE				"
    "SCHEDULE:SCHED_COMBINE_PATROL"  // keep doing it
    ""
    "	Interrupts"
    "		COND_ENEMY_DEAD"
    "		COND_LIGHT_DAMAGE"
