...
BEGIN_DATADESC(CBaseHelicopter)
  ...
  DEFINE_THINKFUNC(HelicopterThink), DEFINE_THINKFUNC(RotorWashThink),
      DEFINE_THINKFUNC(CallDyingThink), DEFINE_THINKFUNC(DelayedKillThink),
      DEFINE_ENTITYFUNC(CrashTouch), DEFINE_ENTITYFUNC(FlyTouch),
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "MoveSpecifiedSpeed",
...
SendPropTime(SENDINFO(m_flStartupTime)), END_SEND_TABLE()
...
void CBaseHelicopter::Spawn(void) {
  ...
  // motor
  //******
  // All of this stuff is specific to the individual type of aircraft. Handle it
  // yourself.
  //******
  ...
  CollisionProp()->SetSurroundingBoundsType(USE_SPECIFIED_BOUNDS,
                                            &m_cullBoxMins, &m_cullBoxMaxs);
...
void CBaseHelicopter::HelicopterThink(void) {
  ...
  if (GetEnemy() != NULL &&
      (!GetEnemy()->IsAlive() || GetEnemy()->GetFlags() & FL_NOTARGET ||
       IRelationType(GetEnemy()) == D_NU)) {
    SetEnemy(NULL);
  }
...
void CBaseHelicopter::RotorWashThink(void) {
  if (m_lifeState == LIFE_ALIVE || m_lifeState == LIFE_DYING) {
    ...
    SetContextThink(&CBaseHelicopter::RotorWashThink,
                    gpGlobals->curtime + HELICOPTER_ROTORWASH_THINK_INTERVAL,
                    s_pRotorWashThinkContext);
  } else {
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseHelicopter::DrawRotorWash(float flAltitude,
...
void CBaseHelicopter::DoWashPushOnAirboat(CBaseEntity *pAirboat,
  ...
  if (vecUp.z < MAX_AIRBOAT_ROLL_COSANGLE) return;

  // Compute roll direction so that we get pushed down on the side where the
  // rotor wash is.
  ...
  // Compute a vector which is the max direction we can roll given the roll
  // constraint
  Vector vecExtremeUp;
  VMatrix rot;
  ...
  pAirboat->VPhysicsTakeDamage(CTakeDamageInfo(
      this, this, vecForce, vecWashOrigin, flWashAmount, DMG_BLAST));
}

//-----------------------------------------------------------------------------
// Purpose: Push a physics object in our wash. Return false if it's now out of
// our wash
//-----------------------------------------------------------------------------
bool CBaseHelicopter::DoWashPush(washentity_t *pWash,
                                 const Vector &vecWashOrigin) {
  if (!pWash || !pWash->hEntity.Get()) return false;
  ...
  if (flDist > BASECHOPPER_WASH_RADIUS) return false;
  ...
  if (pShooter) {
    Vector vecForce =
        (0.015f / 0.1f) * flWashAmount * vecToSpot * phys_pushscale.GetFloat();
  ...
  pEntity->VPhysicsTakeDamage(CTakeDamageInfo(
      this, this, vecForce, vecWashOrigin, flWashAmount, DMG_BLAST));

  // Debug
  if (g_debug_basehelicopter.GetInt() == BASECHOPPER_DEBUG_WASH) {
    NDebugOverlay::Cross3D(pEntity->GetAbsOrigin(), -Vector(4, 4, 4),
                           Vector(4, 4, 4), 255, 0, 0, true, 0.1f);
    NDebugOverlay::Line(pEntity->GetAbsOrigin(),
                        pEntity->GetAbsOrigin() + vecForce, 255, 255, 0, true,
                        0.1f);

    IPhysicsObject *pPhysObject = pEntity->VPhysicsGetObject();
    Msg("Pushed %s (index %d) (mass %f) with force %f (min %.2f max %.2f) at "
        "time %.2f\n",
        pEntity->GetClassname(), pEntity->entindex(), pPhysObject->GetMass(),
        flWashAmount, BASECHOPPER_WASH_PUSH_MIN * flMass,
        BASECHOPPER_WASH_PUSH_MAX * flMass, gpGlobals->curtime);
  }

  // If we've pushed this thing for some time, remove it to give us a chance to
  // find lighter things nearby
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseHelicopter::DoRotorPhysicsPush(const Vector &vecRotorOrigin,
                                         float flAltitude) {
  CBaseEntity *pEntity = NULL;
  trace_t tr;

  // First, trace down and find out where the was is hitting the ground
  UTIL_TraceLine(vecRotorOrigin, vecRotorOrigin + Vector(0, 0, -flAltitude),
                 (MASK_SOLID_BRUSHONLY | CONTENTS_WATER), NULL,
  ...
  // Debug
  if (g_debug_basehelicopter.GetInt() == BASECHOPPER_DEBUG_WASH) {
    NDebugOverlay::Cross3D(vecPhysicsOrigin, -Vector(16, 16, 16),
                           Vector(16, 16, 16), 0, 255, 255, true, 0.1f);
  }
  ...
  if (iCount >= BASECHOPPER_WASH_MAX_OBJECTS) return;

  // Find the lightest physics entity below us and add it to our list to push
  // around
  CBaseEntity *pLightestEntity = NULL;
  float flLightestMass = 9999;
  while ((pEntity = gEntList.FindEntityInSphere(
              pEntity, vecPhysicsOrigin, BASECHOPPER_WASH_RADIUS)) != NULL) {
    IRotorWashShooter *pShooter = GetRotorWashShooter(pEntity);

    if (pEntity->IsEFlagSet(EFL_NO_ROTORWASH_PUSH)) continue;

    if (pShooter || pEntity->GetMoveType() == MOVETYPE_VPHYSICS ||
        (pEntity->VPhysicsGetObject() && !pEntity->IsPlayer())) {
      ...
      if (pShooter) {
        ...
        if (pPhysObject) {
          ...
          if (flMass > BASECHOPPER_WASH_MAX_MASS) continue;
        }
      }
      ...
      // Don't push things too far below our starting point (helps reduce
      // through-roof cases w/o doing a trace)
      if (fabs(vecSpot.z - vecPhysicsOrigin.z) > 96) continue;

      Vector vecToSpot = (vecSpot - vecPhysicsOrigin);
      vecToSpot.z = 0;
      float flDist = VectorNormalize(vecToSpot);
      if (flDist > BASECHOPPER_WASH_RADIUS) continue;

      // Try to cast to the helicopter; if we can't, then we can't be hit.
      if (pEntity->GetServerVehicle()) {
        UTIL_TraceLine(vecSpot, vecPhysicsOrigin, MASK_SOLID_BRUSHONLY, pEntity,
                       COLLISION_GROUP_NONE, &tr);
      ...
      iCount = m_hEntitiesPushedByWash.Count();
      if (iCount >= BASECHOPPER_WASH_MAX_OBJECTS) break;
    }
  }
...
void CBaseHelicopter::UpdateFacingDirection() {
  if (1) {
    ...
    if (!IsCrashing() &&
        m_flLastSeen + 5 >
            gpGlobals->curtime)  //&& DotProduct( targetDir, desiredDir) > 0.25)
    {
...
void CBaseHelicopter::FireWeapons() {
  ...
  if (m_fHelicopterFlags & BITS_HELICOPTER_GUN_ON) {
    // if ( (m_flLastSeen + 1 > gpGlobals->curtime) && (m_flPrevSeen + 2 <
    // gpGlobals->curtime) )
    {
      if (FireGun()) {
...
void CBaseHelicopter::UpdatePlayerDopplerShift() {
  ...
  if (m_iSoundState == 0) {
    ...
    if (pPlayer) {
      ...
      int iPitch =
          100 * ((1 - velReceiver / 13049) / (1 + velTransmitter / 13049));
#else
      // This is a bogus doppler shift, but I like it better
      float relV =
          DotProduct(GetAbsVelocity() - pPlayer->GetAbsVelocity(), dir);
...
//-----------------------------------------------------------------------------
void CBaseHelicopter::ComputeActualTargetPosition(float flSpeed, float flTime,
                                                  float flPerpDist,
                                                  Vector *pDest,
                                                  bool bApplyNoise) {
...
void CBaseHelicopter::Flight(void) {
  ...
  // NDebugOverlay::Line(GetAbsOrigin(), m_vecDesiredPosition, 0,0,255, true,
  // 0.1);
  ...
  AngleVectors(GetLocalAngles() + GetLocalAngularVelocity() * 2 + vecAdj,
               &forward, &right, &up);

  // Vector vecEst1 = GetLocalOrigin() + GetAbsVelocity() + up * m_flForce -
  // Vector( 0, 0, 384 ); float flSide = DotProduct( m_vecDesiredPosition -
  // vecEst1, right );
  ...
  Vector vecEst = GetAbsOrigin() + GetAbsVelocity() * 2.0 +
                  up * m_flForce * 20 - Vector(0, 0, 384 * 2);
  ...
  float flDir = DotProduct(Vector(forward.x, forward.y, 0),
                           Vector(GetAbsVelocity().x, GetAbsVelocity().y, 0));
  if (flDir < 0) {
    flSpeed = -flSpeed;
  }
  ...
  // These functions contain code Ken wrote that used to be right here as part
  // of the flight model, but we want different helicopter vehicles to have
  // different drag characteristics, so I made them virtual functions (sjb)
...
#define FORCE_NEGDELTA 8
  ...
  if (m_flForce < MAX_FORCE && vecEst.z < GetDesiredPosition().z) {
    ...
    if (vecEst.z > GetDesiredPosition().z) m_flForce -= FORCE_NEGDELTA;
  }
  ...
  if (flDist > 0 && flSpeed < m_flGoalSpeed /* && flSpeed < flDist */ &&
      GetLocalAngles().x + angVel.x < 40) {
    // ALERT( at_console, "F " );
    // lean forward
    angVel.x += 12.0;
  } else if (flDist < 0 && flSpeed > -50 &&
             GetLocalAngles().x + angVel.x > -20) {
    // ALERT( at_console, "B " );
    // lean backward
    angVel.x -= 12.0;
  ...
  // GetAbsOrigin().x, GetAbsVelocity().x, flDist, flSpeed, GetLocalAngles().x,
  // m_vecAngVelocity.x, m_flForce ); ALERT( at_console, "%.0f %.0f : %.0f %0.f
  // : %.0f\n", GetAbsOrigin().z, GetAbsVelocity().z, vecEst.z,
  // m_vecDesiredPosition.z, m_flForce );
}
...
void CBaseHelicopter::InputKill(inputdata_t &inputdata) {
  ...
  m_bSuppressSound = true;
  SetContextThink(&CBaseHelicopter::DelayedKillThink, gpGlobals->curtime + 3.0f,
                  s_pDelayedKillThinkContext);
}
...
void CBaseHelicopter::InitializeRotorSound(void) {
  ...
  if (m_pRotorBlast) {
    // Start the blast sound and then immediately drop it to 0 (starting it at 0
    // wouldn't start it)
...
void CBaseHelicopter::DrawDebugGeometryOverlays(void) {
  if (m_pfnThink != NULL) {
    ...
    if (m_debugOverlays & OVERLAY_NPC_ROUTE_BIT) {
      NDebugOverlay::Line(GetAbsOrigin(), GetDesiredPosition(), 0, 0, 255, true,
                          0);
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
void CBaseHelicopter::TraceAttack(const CTakeDamageInfo &info,
...
void CBaseHelicopter::Startup(void) {
  ...
  if (!(m_spawnflags & SF_NOROTORWASH)) {
    m_hRotorWash = CreateRotorWashEmitter(GetAbsOrigin(), GetAbsAngles(), this,
  ...
  SetContextThink(&CBaseHelicopter::RotorWashThink, gpGlobals->curtime,
                  s_pRotorWashThinkContext);
}
