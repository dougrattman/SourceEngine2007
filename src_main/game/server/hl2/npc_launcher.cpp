...
//  Input Functions
// ===================
void CNPC_Launcher::InputTurnOn(inputdata_t &inputdata) { LauncherTurnOn(); }

void CNPC_Launcher::InputTurnOff(inputdata_t &inputdata) { SetThink(NULL); }
...
void CNPC_Launcher::InputClearEnemy(inputdata_t &inputdata) { SetEnemy(NULL); }
...
// Purpose:
//-----------------------------------------------------------------------------
Class_T CNPC_Launcher::Classify(void) { return CLASS_NONE; }
...
void CNPC_Launcher::LaunchGrenade(CBaseEntity *pEnemy) {
  ...
  if (m_sPathCornerName != NULL_STRING) {
    CGrenadePathfollower *pGrenade =
        CGrenadePathfollower::CreateGrenadePathfollower(
            m_sMissileModel, m_sFlySound, GetAbsOrigin(), vec3_angle, edict());
    ...
    pGrenade->SetHoming((0.01 * m_nHomingStrength), m_flHomingDelay,
                        m_flHomingRampUp, m_flHomingDuration,
                        m_flHomingRampDown);
    pGrenade->SetDamage(m_flDamage);
    pGrenade->SetDamageRadius(m_flDamageRadius);
    pGrenade->Launch(this, pEnemy, vLaunchVelocity, m_flHomingSpeed,
                     GetGravity(), m_nSmokeTrail);
  }
  ...
  if (m_bSmokeLaunch) {
    UTIL_Smoke(GetAbsOrigin(), random->RandomInt(20, 30),
...
//------------------------------------------------------------------------------
bool CNPC_Launcher::FInViewCone(CBaseEntity *pEntity) { return true; }
...
bool CNPC_Launcher::IsValidEnemy(CBaseEntity *pTarget) {
  ...
  if (!FBitSet(m_spawnflags.operator const int &(), SF_LAUNCHER_CHECK_LOS)) {
    return true;
  }
  // ------------------------------------------------------
  ...
  AI_TraceLine(vStartPos, vEndPos, MASK_SHOT, pTarget, COLLISION_GROUP_NONE,
               &tr);
...
int CNPC_Launcher::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "State: %s",
               (m_pfnThink) ? "On" : "Off");
    EntityText(text_offset, tempstr, 0);
    text_offset++;

    Q_snprintf(
        tempstr, sizeof(tempstr), "LOS: %s",
        (FBitSet(m_spawnflags.operator const int &(), SF_LAUNCHER_CHECK_LOS))
            ? "On"
            : "Off");
    EntityText(text_offset, tempstr, 0);
    text_offset++;
