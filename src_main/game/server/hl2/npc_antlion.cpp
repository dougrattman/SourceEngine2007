...
ConVar sk_antlion_worker_burst_damage(
    "sk_antlion_worker_burst_damage", "50", FCVAR_NONE,
    "How much damage is inflicted by an antlion worker's death explosion.");
ConVar sk_antlion_worker_health(
    "sk_antlion_worker_health", "0", FCVAR_NONE,
    "Hitpoints of an antlion worker. If 0, will use base antlion hitpoints.");
ConVar sk_antlion_worker_spit_speed(
    "sk_antlion_worker_spit_speed", "0", FCVAR_NONE,
    "Speed at which an antlion spit grenade travels.");

// This must agree with the AntlionWorkerBurstRadius() function!
ConVar sk_antlion_worker_burst_radius(
    "sk_antlion_worker_burst_radius", "160", FCVAR_NONE,
...
BEGIN_DATADESC(CNPC_Antlion)
  ...
  DEFINE_KEYFIELD(m_bStartBurrowed, FIELD_BOOLEAN, "startburrowed"),
      ...
      DEFINE_KEYFIELD(m_bSuppressUnburrowEffects, FIELD_BOOLEAN,
                      "unburroweffects"),
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "StopFightToPosition",
      ...
      // Function Pointers
      DEFINE_ENTITYFUNC(Touch), DEFINE_USEFUNC(BurrowUse),
...
#if HL2_EPISODIC
  m_iHealth = (IsWorker()) ? sk_antlion_worker_health.GetFloat()
                           : sk_antlion_health.GetFloat();
#else
...
void CNPC_Antlion::UpdateHead(void) {
  ...
  if (pTarget != NULL) {
    ...
    SetPoseParameter(m_poseHead_Pitch,
                     UTIL_Approach(pitch + pitchDiff, pitch, 50));
  } else {
    SetPoseParameter(m_poseHead_Yaw, UTIL_Approach(0, yaw, 10));
    SetPoseParameter(m_poseHead_Pitch, UTIL_Approach(0, pitch, 10));
  }
...
bool CNPC_Antlion::FInViewCone(CBaseEntity *pEntity) {
  m_flFieldOfView =
      (GetEnemy() != NULL) ? ANTLION_VIEW_FIELD_NARROW : VIEW_FIELD_WIDE;
...
bool CNPC_Antlion::CanBecomeRagdoll() {
  // This prevents us from dying in the regular way. It forces a schedule
  // selection that will select SCHED_DIE, where we can do our poison burst
  // thing.
#ifdef HL2_EPISODIC
  if (IsWorker() && ANTLION_WORKERS_BURST()) {
    // If we're in a script, we're allowed to ragdoll. This lets the vort's
    // dynamic interaction ragdoll us.
...
void CNPC_Antlion::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (IsCurSchedule(SCHED_ANTLION_BURROW_IN) ||
      IsCurSchedule(SCHED_ANTLION_BURROW_OUT)) {
    AddEFlags(EF_NOSHADOW);
  }

  if (info.GetDamageType() & DMG_CRUSH) {
    CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER, GetAbsOrigin(), 256, 0.5f,
                           this);
  }
  ...
  if (pAttacker && pAttacker->GetServerVehicle() && ShouldGib(info) == true) {
    trace_t tr;
    UTIL_TraceLine(GetAbsOrigin() + Vector(0, 0, 64), pAttacker->GetAbsOrigin(),
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Antlion::MeleeAttack(float distance, float damage, QAngle &viewPunch,
                               Vector &shove) {
  Vector vecForceDir;
  ...
  CBaseEntity *pHurt =
      CheckTraceHullAttack(distance, -Vector(16, 16, 32), Vector(16, 16, 32),
                           damage, DMG_SLASH, 5.0f);
  ...
  if (pHurt) {
    ...
    if (pPlayer != NULL) {
      ...
      if (!(pPlayer->GetFlags() & FL_GODMODE) &&
          pPlayer->GetMoveType() != MOVETYPE_NOCLIP) {
...
bool CNPC_Antlion::FindChasePosition(const Vector &targetPos, Vector &result) {
  ...
  // TODO: If we're making multiple inquiries to this, make sure it's evenly
  // spread

  if (g_debug_antlion.GetInt() == 1) {
    NDebugOverlay::Cross3D(result, -Vector(32, 32, 32), Vector(32, 32, 32), 255,
                           255, 0, true, 2.0f);
  }
...
bool CNPC_Antlion::GetGroundPosition(const Vector &testPos, Vector &result) {
  ...
  trace_t tr;
  AI_TraceHull(testPos, testPos + Vector(0, 0, 64),
               NAI_Hull::Mins(GetHullType()), NAI_Hull::Maxs(GetHullType()),
  ...
  if (tr.allsolid) {
    if (g_debug_antlion.GetInt() == 3) {
      NDebugOverlay::BoxDirection(
          testPos, NAI_Hull::Mins(GetHullType()),
          NAI_Hull::Maxs(GetHullType()) + Vector(0, 0, 128), Vector(0, 0, 1),
          255, 0, 0, true, 2.0f);
    }
  ...
  if (g_debug_antlion.GetInt() == 3) {
    NDebugOverlay::BoxDirection(
        testPos, NAI_Hull::Mins(GetHullType()),
        NAI_Hull::Maxs(GetHullType()) + Vector(0, 0, 128), Vector(0, 0, 1), 0,
        255, 0, true, 2.0f);
  }

  // Trace down to find the ground
  AI_TraceHull(tr.endpos, tr.endpos - Vector(0, 0, 128),
               NAI_Hull::Mins(GetHullType()), NAI_Hull::Maxs(GetHullType()),
               MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);

  if (g_debug_antlion.GetInt() == 3) {
    NDebugOverlay::BoxDirection(
        tr.endpos, NAI_Hull::Mins(GetHullType()) - Vector(0, 0, 256),
        NAI_Hull::Maxs(GetHullType()), Vector(0, 0, 1), 255, 255, 0, true,
        2.0f);
  }
  ...
  if (tr.fraction < 1.0f) {
    if (g_debug_antlion.GetInt() == 3) {
      NDebugOverlay::Cross3D(tr.endpos, NAI_Hull::Mins(GetHullType()),
                             NAI_Hull::Maxs(GetHullType()), 255, 0, 0, true,
                             2.0f);
    }
...
void CNPC_Antlion::ManageFleeCapabilities(bool bEnable) {
  if (bEnable == false) {
    ...
    if (m_bDisableJump == false) CapabilitiesAdd(bits_CAP_MOVE_JUMP);
...
bool CNPC_Antlion::GetPathToSoundFleePoint(int soundType) {
  ...
  if (pSound == NULL) {
    // NOTENOTE: If you're here, there's a disparity between Listen() and
    // GetLoudestSoundOfType() - jdw
  ...
  if (pHint != NULL) {
    ...
    AI_NavGoal_t goal(vecFleeGoal, (Activity)ACT_ANTLION_RUN_AGITATED, 128,
                      AIN_DEF_FLAGS);
    ...
    if (GetNavigator()->SetGoal(goal)) {
      ...
      GetNavigator()->SetArrivalActivity(
          (Activity)ACT_ANTLION_DISTRACT_ARRIVED);
  ...
  vecFleeGoal =
      vecSoundPos + vecFleeDir * (pSound->Volume() + random->RandomInt(32, 64));

  // Find a route to that position
  AI_NavGoal_t goal(vecFleeGoal + Vector(0, 0, 8),
                    (Activity)ACT_ANTLION_RUN_AGITATED, 512, AIN_DEF_FLAGS);
...
//-----------------------------------------------------------------------------
// Purpose: Returns whether the enemy has been seen within the time period
// supplied Input  : flTime - Timespan we consider Output : Returns true on
// success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::SeenEnemyWithinTime(float flTime) {
  float flLastSeenTime = GetEnemies()->LastTimeSeen(GetEnemy());
  return (flLastSeenTime != 0.0f &&
          (gpGlobals->curtime - flLastSeenTime) < flTime);
}

//-----------------------------------------------------------------------------
// Purpose: Test whether this antlion can hit the target
//-----------------------------------------------------------------------------
bool CNPC_Antlion::InnateWeaponLOSCondition(const Vector &ownerPos,
                                            const Vector &targetPos,
                                            bool bSetConditions) {
  if (GetNextAttack() > gpGlobals->curtime) return false;

  // If we can see the enemy, or we've seen them in the last few seconds just
  // try to lob in there
  ...
  return BaseClass::InnateWeaponLOSCondition(ownerPos, targetPos,
                                             bSetConditions);
}
...
Vector VecCheckThrowTolerance(CBaseEntity *pEdict, const Vector &vecSpot1,
                              Vector vecSpot2, float flSpeed,
                              float flTolerance) {
  flSpeed = std::max(1.0f, flSpeed);
  ...
  trace_t tr;
  UTIL_TraceLine(vecSpot1, vecApex, MASK_SOLID, pEdict, COLLISION_GROUP_NONE,
                 &tr);
  if (tr.fraction != 1.0) {
    // fail!
  ...
  UTIL_TraceLine(vecApex, vecSpot2, MASK_SOLID_BRUSHONLY, pEdict,
                 COLLISION_GROUP_NONE, &tr);
  if (tr.fraction != 1.0) {
    bool bFail = true;

    // Didn't make it all the way there, but check if we're within our tolerance
    // range
    if (flTolerance > 0.0f) {
      float flNearness = (tr.endpos - vecSpot2).LengthSqr();
      if (flNearness < Square(flTolerance)) {
        if (g_debug_antlion_worker.GetBool()) {
          NDebugOverlay::Sphere(tr.endpos, vec3_angle, flTolerance, 0, 255, 0,
                                0, true, 5.0);
        }
    ...
    if (bFail) {
      if (g_debug_antlion_worker.GetBool()) {
        ...
        NDebugOverlay::Sphere(tr.endpos, vec3_angle, flTolerance, 255, 0, 0, 0,
                              true, 5.0);
      }
      return vec3_origin;
    }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::GetSpitVector(const Vector &vecStartPos,
                                 const Vector &vecTarget, Vector *vecOut) {
  // antlion workers exist only in episodic.
#if HL2_EPISODIC
  // Try the most direct route
  Vector vecToss = VecCheckThrowTolerance(
      this, vecStartPos, vecTarget, sk_antlion_worker_spit_speed.GetFloat(),
      (10.0f * 12.0f));

  // If this failed then try a little faster (flattens the arc)
  if (vecToss == vec3_origin) {
    vecToss = VecCheckThrowTolerance(
        this, vecStartPos, vecTarget,
        sk_antlion_worker_spit_speed.GetFloat() * 1.5f, (10.0f * 12.0f));
    if (vecToss == vec3_origin) return false;
  }
...
void CNPC_Antlion::DelaySquadAttack(float flDuration) {
  if (GetSquad()) {
    // Reduce the duration by as much as 50% of the total time to make this less
    // robotic
    float flAdjDuration =
        flDuration - random->RandomFloat(0.0f, (flDuration * 0.5f));
    GetSquad()->BroadcastInteraction(g_interactionAntlionFiredAtTarget,
                                     (void *)&flAdjDuration, this);
  }
}
...
void CNPC_Antlion::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_ANTLION_WORKER_SPIT) {
    if (GetEnemy()) {
      ...
      if (HasCondition(COND_ENEMY_FACING_ME) &&
          UTIL_DistApprox(GetAbsOrigin(), GetEnemy()->GetAbsOrigin()) >
              (40 * 12)) {
      ...
      if (GetSpitVector(vSpitPos, vTarget, &vecToss) == false) {
        ...
        if (GetSpitVector(vSpitPos, m_vSavePosition, &vecToss) == false) {
          // Failing that, just shoot with the old velocity we calculated
          // initially!
          vecToss = m_vecSaveSpitVelocity;
        }
      }

      // Find what our vertical theta is to estimate the time we'll impact the
      // ground
      ...
      float flTime =
          (vSpitPos - vTarget).Length2D() / (flVelocity * flCosTheta);

      // Emit a sound where this is going to hit so that targets get a chance to
      // act correctly
      CSoundEnt::InsertSound(SOUND_DANGER, vTarget, (15 * 12), flTime, this);

      // Don't fire again until this volley would have hit the ground (with some
      // lag behind it)
      SetNextAttack(gpGlobals->curtime + flTime +
                    random->RandomFloat(0.5f, 2.0f));

      // Tell any squadmates not to fire for some portion of the time this
      // volley will be in the air (except on hard)
      if (g_pGameRules->IsSkillLevel(SKILL_HARD) == false)
        DelaySquadAttack(flTime);

      for (int i = 0; i < 6; i++) {
        CGrenadeSpit *pGrenade =
            (CGrenadeSpit *)CreateEntityByName("grenade_spit");
        ...
        if (i == 0) {
          ...
          pGrenade->SetAbsVelocity((vecToss + RandomVector(-0.035f, 0.035f)) *
                                   flVelocity);
          pGrenade->SetSpitSize(random->RandomInt(SPIT_SMALL, SPIT_MEDIUM));
        }

        // Tumble through the air
        pGrenade->SetLocalAngularVelocity(QAngle(
            random->RandomFloat(-250, -500), random->RandomFloat(-250, -500),
            random->RandomFloat(-250, -500)));
      }

      for (int i = 0; i < 8; i++) {
        DispatchParticleEffect("blood_impact_yellow_01",
                               vSpitPos + RandomVector(-12.0f, 12.0f),
                               RandomAngle(0, 360));
      }
  ...
  if (pEvent->event == AE_ANTLION_MELEE_HIT1) {
    MeleeAttack(ANTLION_MELEE1_RANGE, sk_antlion_swipe_damage.GetFloat(),
  ...
  if (pEvent->event == AE_ANTLION_MELEE_HIT2) {
    MeleeAttack(ANTLION_MELEE1_RANGE, sk_antlion_swipe_damage.GetFloat(),
                QAngle(20.0f, 0.0f, 0.0f), Vector(-350.0f, 1.0f, 1.0f));
    return;
  }

  if (pEvent->event == AE_ANTLION_MELEE_POUNCE) {
    MeleeAttack(ANTLION_MELEE2_RANGE, sk_antlion_swipe_damage.GetFloat(),
                QAngle(4.0f, 0.0f, 0.0f), Vector(-250.0f, 1.0f, 1.0f));
    return;
  }
  ...
  if (pEvent->event == AE_ANTLION_WORKER_EXPLODE_WARN) {
    CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER, GetAbsOrigin(),
                           sk_antlion_worker_burst_radius.GetFloat(), 0.5f,
                           this);
    return;
  }
  ...
  if (pEvent->event == AE_ANTLION_WORKER_EXPLODE) {
    CTakeDamageInfo info(
        this, this, sk_antlion_worker_burst_damage.GetFloat(),
        DMG_BLAST_SURFACE |
            (ANTLION_WORKER_BURST_IS_POISONOUS() ? DMG_POISON : DMG_ACID));
    Event_Gibbed(info);
    return;
  }
#endif
...
bool CNPC_Antlion::IsUnusableNode(int iNodeID, CAI_Hint *pHint) {
  ...
  if (g_test_new_antlion_jump.GetBool() == 0) return iBaseReturn;

  CAI_Node *pNode = GetNavigator()->GetNetwork()->GetNode(iNodeID);

  if (pNode) {
    if (pNode->IsLocked()) return true;
  }
...
void CNPC_Antlion::LockJumpNode(void) {
  if (HasSpawnFlags(SF_ANTLION_USE_GROUNDCHECKS) == false) return;

  if (GetNavigator()->GetPath() == NULL) return;

  if (g_test_new_antlion_jump.GetBool() == false) return;

  AI_Waypoint_t *pWaypoint = GetNavigator()->GetPath()->GetCurWaypoint();

  while (pWaypoint) {
    AI_Waypoint_t *pNextWaypoint = pWaypoint->GetNext();
    if (pNextWaypoint && pNextWaypoint->NavType() == NAV_JUMP &&
        pWaypoint->iNodeID != NO_NODE) {
      CAI_Node *pNode =
          GetNavigator()->GetNetwork()->GetNode(pWaypoint->iNodeID);

      if (pNode) {
        // NDebugOverlay::Box( pNode->GetOrigin(), Vector( -16, -16, -16 ),
        // Vector( 16, 16, 16 ), 255, 0, 0, 0, 2 );
        pNode->Lock(0.5f);
        break;
      }
    } else {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Antlion::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal,
                                         float distClear,
                                         AIMoveResult_t *pResult) {
  bool iBaseReturn =
      BaseClass::OnObstructionPreSteer(pMoveGoal, distClear, pResult);

  if (g_test_new_antlion_jump.GetBool() == false) return iBaseReturn;

  if (HasSpawnFlags(SF_ANTLION_USE_GROUNDCHECKS) == false) return iBaseReturn;
  ...
  if (pBlocker && pBlocker->Classify() == CLASS_ANTLION) {
    ...
    if (pAntlion) {
      if (pAntlion->AllowedToBePushed() == true && GetEnemy() == NULL) {
        // NDebugOverlay::Box( pAntlion->GetAbsOrigin(), GetHullMins(),
        // GetHullMaxs(), 0, 255, 0, 0, 2 );
...
class CTraceFilterAntlion : public CTraceFilterEntitiesOnly {
 public:
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pEntity->IsNPC() == false) return false;

    if (NPC_Antlion_IsAntlion(pEntity)) return true;
...
void CNPC_Antlion::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_ANTLION_FIND_COVER_FROM_SAVEPOSITION: {
      ...
      if (GetTacticalServices()->FindCoverPos(m_vSavePosition, EyePosition(), 0,
                                              CoverRadius(), &coverPos)) {
    ...
    case TASK_ANTLION_DISMOUNT_NPC: {
      ...
      if (pGroundEnt != NULL) {
        ...
        AI_TraceHull(GetAbsOrigin(), GetAbsOrigin(), WorldAlignMins(),
                     WorldAlignMaxs(), MASK_SOLID, &traceFilter, &trace);
        ...
        // Doing ACT_RESET first assures they play the animation, even when in
        // transition
        ResetActivity();
    ...
    case TASK_ANTLION_GET_PATH_TO_BUGBAIT: {
      ...
      if (FindChasePosition(m_vecHeardSound, goalPos)) {
        AI_NavGoal_t goal(goalPos, (Activity)ACT_ANTLION_RUN_AGITATED,
        ...
        if (GetNavigator()->SetGoal(goal, AIN_DISCARD_IF_FAIL) == false) {
          ...
          AI_NavGoal_t nearGoal(GOALTYPE_LOCATION_NEAREST_NODE, goalPos,
                                (Activity)ACT_ANTLION_RUN_AGITATED,
                                ANTLION_BUGBAIT_NAV_TOLERANCE);
    ...
    case TASK_ANTLION_FIND_BURROW_IN_POINT:
      ...
      if (FindBurrow(GetAbsOrigin(), pTask->flTaskData, ANTLION_BURROW_IN) ==
          false) {
        TaskFail(
            "TASK_ANTLION_FIND_BURROW_IN_POINT: Unable to find burrow in "
            "position\n");
      } else {
        TaskComplete();
      }
    ...
    case TASK_ANTLION_FIND_BURROW_OUT_POINT:
      ...
      if (FindBurrow(GetAbsOrigin(), pTask->flTaskData, ANTLION_BURROW_OUT) ==
          false) {
        TaskFail(
            "TASK_ANTLION_FIND_BURROW_OUT_POINT: Unable to find burrow out "
            "position\n");
      } else {
        TaskComplete();
      }
    ...
    case TASK_ANTLION_VANISH:
      ...
      if (pTask->flTaskData) {
        ...
        // NOTE: We can't UTIL_Remove here, because we're in the middle of
        // running our AI, and
        //		 we'll crash later in the bowels of the AI. Remove
        // ourselves next frame.
  ...
  switch (pTask->iTask) {
    ...
    case TASK_ANTLION_DROWN: {
      if (gpGlobals->curtime > m_flTimeDrownSplash) {
        float flWaterZ = UTIL_FindWaterSurface(
            GetAbsOrigin(), GetAbsOrigin().z,
            GetAbsOrigin().z + NAI_Hull::Maxs(GetHullType()).z);
    ...
    case TASK_ANTLION_DISMOUNT_NPC:
      ...
      if (GetFlags() & FL_ONGROUND) {
        ...
        if ((pGroundEnt != NULL) &&
            ((pGroundEnt->MyNPCPointer() != NULL) ||
             pGroundEnt->GetSolidFlags() & FSOLID_NOT_STANDABLE)) {
          // Jump behind the other NPC so I don't block their path.
          Vector vecJumpDir;
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Antlion::IsJumpLegal(const Vector &startPos, const Vector &apex,
  ...
  if (CAntlionRepellant::IsPositionRepellantFree(endPos) == false) return false;

  // Adrian: Don't try to jump if my destination is right next to me.
  if ((endPos - GetAbsOrigin()).Length() < MIN_JUMP_DISTANCE) return false;

  if (HasSpawnFlags(SF_ANTLION_USE_GROUNDCHECKS) &&
      g_test_new_antlion_jump.GetBool() == true) {
    trace_t tr;
    AI_TraceHull(endPos, endPos, GetHullMins(), GetHullMaxs(), MASK_NPCSOLID,
  ...
  return BaseClass::IsJumpLegal(startPos, apex, endPos, MAX_JUMP_RISE,
...
bool CNPC_Antlion::IsFirmlyOnGround(void) {
  ...
  //	NDebugOverlay::Line( vOrigin, vOrigin - Vector( 0, 0, flHeight * 0.5  ),
  // 255, 0, 0, true, 5 );
  UTIL_TraceLine(vOrigin, vOrigin - Vector(0, 0, flHeight * 0.5), MASK_NPCSOLID,
...
// Purpose:
//-----------------------------------------------------------------------------
int CNPC_Antlion::SelectFailSchedule(int failedSchedule, int failedTask,
...
bool CNPC_Antlion::ShouldJump(void) {
  ...
  if (m_flJumpTime > gpGlobals->curtime) return false;

  // only jump if you're on the ground
  if (!(GetFlags() & FL_ONGROUND) || GetNavType() == NAV_JUMP) return false;

  // Don't jump if I'm not allowed
  if ((CapabilitiesGet() & bits_CAP_MOVE_JUMP) == false) return false;
  ...
  if (flDist < ANTLION_JUMP_MIN) return false;
  ...
  GetMoveProbe()->MoveLimit(NAV_JUMP, GetAbsOrigin(), targetPos, MASK_NPCSOLID,
                            GetNavTargetEntity(), &moveTrace);

  // See if it succeeded
  if (IsMoveBlocked(moveTrace.fStatus)) {
    if (g_debug_antlion.GetInt() == 2) {
      NDebugOverlay::Box(targetPos, GetHullMins(), GetHullMaxs(), 255, 0, 0, 0,
                         5);
  ...
  if (g_debug_antlion.GetInt() == 2) {
    NDebugOverlay::Box(targetPos, GetHullMins(), GetHullMaxs(), 0, 255, 0, 0,
                       5);
...
int CNPC_Antlion::TranslateSchedule(int scheduleType) {
  if ((m_hFollowTarget != NULL) || IsAllied()) {
    if ((scheduleType == SCHED_IDLE_STAND) ||
        (scheduleType == SCHED_ALERT_STAND))
...
int CNPC_Antlion::ChooseMoveSchedule(void) {
  ...
  if (ShouldResumeFollow()) {
    ...
    CAntlionTemplateMaker *pMaker =
        dynamic_cast<CAntlionTemplateMaker *>(GetOwnerEntity());
  ...
  switch (m_MoveState) {
    ...
    case ANTLION_MOVE_FIGHT_TO_GOAL: {
      if (m_hFightGoalTarget) {
        float targetDist = UTIL_DistApprox(WorldSpaceCenter(),
                                           m_hFightGoalTarget->GetAbsOrigin());

        if (targetDist > 256) {
          Vector testPos;
          Vector targetPos = (m_hFightGoalTarget)
                                 ? m_hFightGoalTarget->GetAbsOrigin()
                                 : m_vSavePosition;
    ...
    case ANTLION_MOVE_FOLLOW: {
      if (m_FollowBehavior.CanSelectSchedule()) {
        ...
        if (ShouldAbandonFollow()) return SCHED_ANTLION_BURROW_AWAY;
...
void CNPC_Antlion::ZapThink(void) {
  ...
  if (m_flZapDuration > gpGlobals->curtime) {
    SetContextThink(&CNPC_Antlion::ZapThink,
                    gpGlobals->curtime + random->RandomFloat(0.05f, 0.25f),
                    "ZapThink");
  } else {
    SetContextThink(NULL, gpGlobals->curtime, "ZapThink");
  }
...
int CNPC_Antlion::SelectSchedule(void) {
  ...
  if (m_bStartBurrowed) return SCHED_ANTLION_WAIT_FOR_UNBORROW_TRIGGER;

  // See if a friendly player is pushing us away
  if (HasCondition(COND_PLAYER_PUSHING)) return SCHED_MOVE_AWAY;
  ...
  if ((m_spawnflags & SF_ANTLION_BURROW_ON_ELUDED) &&
      (HasCondition(COND_ENEMY_UNREACHABLE) ||
       HasCondition(COND_ENEMY_TOO_FAR)))
  ...
  if (HasCondition(COND_HEAR_BUGBAIT) && (m_bIgnoreBugbait == false)) {
    ...
    if (m_bHasHeardSound) {
      ...
      while ((pTarget = gEntList.FindEntityInSphere(
                  pTarget, soundOrg, bugbait_radius.GetInt())) != NULL) {
        CAI_BaseNPC *pNPC = pTarget->MyNPCPointer();

        if (pNPC == NULL) continue;

        if (pNPC->CanBeAnEnemyOf(this) == false) continue;
  ...
  switch (m_NPCState) {
    case NPC_STATE_COMBAT: {
      ...
      if (hl2_episodic.GetBool() && IsWorker()) {
        ...
        if (HasCondition(COND_CAN_MELEE_ATTACK2)) {
          ...
          if (m_bLeapAttack == true) return SCHED_ANTLION_POUNCE_MOVING;
        ...
        // Try to jump
        if (HasCondition(COND_ANTLION_CAN_JUMP)) return SCHED_ANTLION_JUMP;
      }
    } break;
    ...
    default: {
      ...
      if (moveSched != SCHED_NONE) return moveSched;
...
void CNPC_Antlion::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
...
int CNPC_Antlion::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (hl2_episodic.GetBool() && antlion_easycrush.GetBool()) {
    if (newInfo.GetDamageType() & DMG_CRUSH) {
      if (newInfo.GetInflictor() &&
          newInfo.GetInflictor()->VPhysicsGetObject()) {
  ...
  // If we're being hoisted by a barnacle, we only take damage from that
  // barnacle (otherwise we can die too early!)
...
void CNPC_Antlion::CascadePush(const Vector &vecForce) {
  ...
  if (hl2_episodic.GetBool() ==
      false /*|| g_antlion_cascade_push.GetBool() == false*/)
    return;
  ...
  for (int i = 0; i < nNumEnemies; i++) {
    ...
    if (pEnemySearch[i] == NULL ||
        pEnemySearch[i]->Classify() != CLASS_ANTLION || pEnemySearch[i] == this)
      continue;
...
inline bool CNPC_Antlion::IsFlipped(void) {
  return (GetActivity() == ACT_ANTLION_FLIP ||
          GetActivity() == ACT_ANTLION_ZAP_FLIP);
}

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Antlion::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (IsFlipped()) {
    ...
    if (newInfo.GetDamageType() & (DMG_CRUSH | DMG_PHYSGUN)) {
      PainSound(newInfo);
      Vector vecForce = (vecShoveDir * random->RandomInt(500.0f, 1000.0f)) +
                        Vector(0, 0, 64.0f);
    ...
    newInfo.ScaleDamage(4.0f);
             ...
             (newInfo.GetDamageType() & (DMG_BLAST | DMG_CRUSH) &&
              newInfo.GetDamage() >= 25.0f)) {
      ...
      if (hl2_episodic.GetBool()) {
        ...
        if (GetFlags() & FL_ONGROUND) {
          ...
          ApplyAbsVelocityImpulse(
              (vecShoveDir * random->RandomInt(500.0f, 1000.0f)) +
              Vector(0, 0, 64.0f));
          SetGroundEntity(NULL);
        }
      }
...
// Output :
//-----------------------------------------------------------------------------
float CNPC_Antlion::GetIdealAccel(void) const { return GetIdealSpeed() * 2.0; }
...
bool CNPC_Antlion::ShouldPlayIdleSound(void) {
  ...
  if (m_flIdleDelay > gpGlobals->curtime) return false;

  // Randomize it a bit
  if (random->RandomInt(0, 20)) return false;
...
bool CNPC_Antlion::FCanCheckAttacks(void) {
  if (IsWorker()) {
    // Only do this if we've seen our target recently and our schedule can be
    // interrupted
    if (SeenEnemyWithinTime(3.0f) &&
        ConditionInterruptsCurSchedule(COND_CAN_RANGE_ATTACK1))
...
int CNPC_Antlion::RangeAttack1Conditions(float flDot, float flDist) {
  if (GetNextAttack() > gpGlobals->curtime) return COND_NOT_FACING_ATTACK;

  if (flDot < DOT_10DEGREE) return COND_NOT_FACING_ATTACK;

  if (flDist > (150 * 12)) return COND_TOO_FAR_TO_ATTACK;

  if (flDist < (20 * 12)) return COND_TOO_CLOSE_TO_ATTACK;
...
int CNPC_Antlion::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  if (flPrDist > Square(ANTLION_MELEE1_RANGE)) return COND_TOO_FAR_TO_ATTACK;
  ...
  if (flDist > flAdjustedDist) return COND_TOO_FAR_TO_ATTACK;
...
int CNPC_Antlion::MeleeAttack2Conditions(float flDot, float flDist) {
  ...
  if (m_flPounceTime > gpGlobals->curtime) return 0;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::HandleInteraction(int interactionType, void *data,
                                     CBaseCombatCharacter *sender) {
  ...
  // fixed for episodic: allow interactions to fall through in the base class.
  // ifdefed away for mainline in case anything depends on this bug.
#ifdef HL2_EPISODIC
...
bool CNPC_Antlion::Alone(void) {
  if (m_pSquad == NULL) return true;

  if (m_pSquad->NumMembers() <= 1) return true;
...
void CNPC_Antlion::StartJump(void) {
  if (m_bForcedStuckJump == false) {
    ...
    if ((GetFlags() & FL_ONGROUND) == false) return;
  }
...
bool CNPC_Antlion::FValidateHintType(CAI_Hint *pHint) {
  switch (m_iContext) {
    case ANTLION_BURROW_OUT: {
      ...
      if (ValidBurrowPoint(vHintPos) == false) return false;
    } break;
  }
...
void CNPC_Antlion::ClearBurrowPoint(const Vector &origin) {
  ...
  for (CEntitySphereQuery sphere(origin, 128);
    ...
    if (pEntity->m_takedamage != DAMAGE_NO &&
        pEntity->Classify() != CLASS_PLAYER && pEntity->VPhysicsGetObject()) {
      ...
      CollisionProp()->RandomPointInBounds(
          vec3_origin, Vector(1.0f, 1.0f, 1.0f), &vecCenter);

      if (flDist <= 128.0f) {
        pEntity->VPhysicsGetObject()->Wake();
        pEntity->VPhysicsGetObject()->ApplyForceOffset(vecForce * 250.0f,
                                                       vecCenter);
      }
    }
  }
...
class CTraceFilterSimpleNPCExclude : public CTraceFilterSimple {
 public:
  ...
  CTraceFilterSimpleNPCExclude(const IHandleEntity *passentity,
                               int collisionGroup)
  ...
  bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (GetPassEntity()) {
      CBaseEntity *pEnt =
          gEntList.GetBaseEntity(GetPassEntity()->GetRefEHandle());

      if (pEnt->IsNPC()) {
        if (NPC_CheckBrushExclude(pEnt, pTestEntity) == true) return false;
      }
    }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Antlion::FindBurrow(const Vector &origin, float distance, int type,
                              bool excludeNear) {
  // Burrowing in?
...
void CNPC_Antlion::BurrowUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
bool CNPC_Antlion::CheckLanding(void) {
  ...
  testPos[2] -=
      (0.5 * sv_gravity.GetFloat() * GetGravity() * timeStep * timeStep);

  if (g_debug_antlion.GetInt() == 2) {
    NDebugOverlay::Line(GetAbsOrigin(), testPos, 255, 0, 0, 0, 0.5f);
    NDebugOverlay::Cross3D(m_vecSavedJump, -Vector(2, 2, 2), Vector(2, 2, 2), 0,
                           255, 0, true, 0.5f);
  }

  // Look below
  AI_TraceHull(GetAbsOrigin(), testPos, NAI_Hull::Mins(GetHullType()),
               NAI_Hull::Maxs(GetHullType()), MASK_NPCSOLID, this,
               COLLISION_GROUP_NONE, &tr);
...
void CNPC_Antlion::CreateDust(bool placeDecal) {
  trace_t tr;
  AI_TraceLine(
      GetAbsOrigin() + Vector(0, 0, 1), GetAbsOrigin() - Vector(0, 0, 64),
      MASK_SOLID_BRUSHONLY | CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP, this,
      COLLISION_GROUP_NONE, &tr);

  if (tr.fraction < 1.0f) {
    const surfacedata_t *pdata =
        physprops->GetSurfaceData(tr.surface.surfaceProps);
...
bool CNPC_Antlion::QueryHearSound(CSound *pSound) {
  ...
  if (pSound->m_iType == SOUND_BUGBAIT) {
    ...
    if (pSound->SoundExpirationTime() <= m_flIgnoreSoundTime) return false;
...
void CNPC_Antlion::BuildScheduleTestBits(void) {
  ...
  if (m_NPCState == NPC_STATE_SCRIPT) return;
  ...
  /*
  if ( GetActivity() == ACT_JUMP || GetActivity() == ACT_GLIDE || GetActivity()
  == ACT_LAND )
  {
  ...
  */
  ...
  // Interrupt any schedule unless already fleeing, burrowing, burrowed, or
  // unburrowing.
...
bool CNPC_Antlion::IsValidEnemy(CBaseEntity *pEnemy) {
  ...
  if (IsAllied() && pEnemy->IsPlayer()) return false;

  if (pEnemy->IsWorld()) return false;

  // If we're chasing bugbait, close to within a certain radius before picking
  // up enemies
  if (IsCurSchedule(GetGlobalScheduleId(SCHED_ANTLION_CHASE_BUGBAIT)) &&
      (GetNavigator() != NULL)) {
    // If the enemy is without the target radius, then don't allow them
    if ((GetNavigator()->IsGoalActive()) &&
        (GetNavigator()->GetGoalPos() - pEnemy->GetAbsOrigin()).Length() >
            bugbait_radius.GetFloat())
      return false;
  }
  ...
  if (m_FollowBehavior.GetFollowTarget() != NULL) {
    ...
    if (m_flObeyFollowTime > gpGlobals->curtime) {
      ...
      if (enemyDist > (128 * 128)) return false;
    } else {
      // Otherwise don't follow if the target is far
      if (enemyDist > (2000 * 2000)) return false;
    }
  }
...
void CNPC_Antlion::GatherConditions(void) {
  ...
  if (((pGroundEnt != NULL) &&
       (pGroundEnt->GetSolidFlags() & FSOLID_NOT_STANDABLE)) &&
      (GetFlags() & FL_ONGROUND) &&
      (!IsEffectActive(EF_NODRAW) && !pGroundEnt->IsEffectActive(EF_NODRAW))) {
  ...
  if (IsCurSchedule(SCHED_ANTLION_FLIP)) ClearCondition(COND_PLAYER_PUSHING);
}
...
void CNPC_Antlion::PrescheduleThink(void) {
  ...
  } else if ((eActivity != ACT_ANTLION_RUN_AGITATED) &&
             (m_bAgitatedSound == true)) {
    // Stop sound
    StopSound("NPC_Antlion.LoopingAgitated");
    m_bAgitatedSound = false;
  ...
  if ((m_bWingsOpen) && (eActivity != ACT_ANTLION_JUMP_START) &&
      (eActivity != ACT_JUMP) && (eActivity != ACT_GLIDE) &&
      (eActivity != ACT_ANTLION_LAND) && (eActivity != ACT_ANTLION_DISTRACT)) {
    SetWings(false);
  }

  // Make sure we've turned off our burrow state if we're not in it
  if (IsEffectActive(EF_NODRAW) && (eActivity != ACT_ANTLION_BURROW_IDLE) &&
  ...
  // See if we should jump because of desirables conditions, or a scripted
  // request
  if (ShouldJump()) {
...
bool CNPC_Antlion::IsLightDamage(const CTakeDamageInfo &info) {
  if ((random->RandomInt(0, 1)) && (info.GetDamage() > 3)) return true;
...
bool CNPC_Antlion::ShouldResumeFollow(void) {
  if (IsAllied() == false) return false;

  if (m_MoveState == ANTLION_MOVE_FOLLOW || m_hFollowTarget == NULL)
    return false;

  if (m_flSuppressFollowTime > gpGlobals->curtime) return false;
  ...
  // TODO: See if the follow target has wandered off too far from where we last
  // followed them to
...
bool CNPC_Antlion::ShouldAbandonFollow(void) {
  ...
  if (m_FollowBehavior.FollowTargetVisible()) return false;

  // Never give up if we're too close
  float flDistance =
      UTIL_DistApprox2D(m_FollowBehavior.GetFollowTarget()->WorldSpaceCenter(),
                        WorldSpaceCenter());

  if (flDistance < 1500) return false;

  if (flDistance > 1500 * 2.0f) return true;

  // If we've failed too many times, give up
  if (m_FollowBehavior.GetNumFailedFollowAttempts()) return true;

  // If the target simply isn't reachable to us, give up
  if (m_FollowBehavior.TargetIsUnreachable()) return true;
...
void CNPC_Antlion::GatherEnemyConditions(CBaseEntity *pEnemy) {
  ...
  if ((m_spawnflags & SF_ANTLION_BURROW_ON_ELUDED) == false) return;

  // If we're not already too far away, check again
  // TODO: Check to make sure we don't already have a condition set that removes
  // the need for this
...
bool CNPC_Antlion::ShouldGib(const CTakeDamageInfo &info) {
  // If we're being hoisted, we only want to gib when the barnacle hurts us with
  // his bite!
  if (IsEFlagSet(EFL_IS_BEING_LIFTED_BY_BARNACLE)) {
    if (info.GetAttacker() && info.GetAttacker()->Classify() != CLASS_BARNACLE)
      return false;
  ...
  if (info.GetDamageType() & (DMG_NEVERGIB | DMG_DISSOLVE)) return false;

#ifdef HL2_EPISODIC
  if (IsWorker() && ANTLION_WORKERS_BURST()) return !m_bDontExplode;
#endif

  if (info.GetDamageType() & (DMG_ALWAYSGIB | DMG_BLAST)) return true;

  if (m_iHealth < -20) return true;
...
#endif  // HL2_EPISODIC
  {
    // Use the bone position to handle being moved by an animation (like a
    // dynamic scripted sequence)
...
void CNPC_Antlion::Touch(CBaseEntity *pOther) {
  ...
  if (pPlayer && pPlayer->IsInAVehicle()) {
    ...
    if (pVehicleEnt == pOther) {
      CPropVehicleDriveable *pDrivableVehicle =
          dynamic_cast<CPropVehicleDriveable *>(pVehicleEnt);
      ...
      if (pDrivableVehicle != NULL) {
        ...
        if (((pDrivableVehicle->m_nRPM > 75) &&
             DotProduct(vecShoveDir, vecTargetDir) <= 0) ||
            bBurrowingOut == true) {
          if (IsFlipped() || bBurrowingOut == true) {
            float flDamage = m_iHealth;

            if (random->RandomInt(0, 10) > 4) flDamage += 25;

            CTakeDamageInfo dmgInfo(pVehicleEnt, pPlayer, flDamage,
                                    DMG_VEHICLE);

            CalculateMeleeDamageForce(&dmgInfo, vecShoveDir,
                                      pOther->GetAbsOrigin());
            TakeDamage(dmgInfo);
          } else {
            // We're being shoved
            ...
            ApplyAbsVelocityImpulse((vecTargetDir * 250.0f) +
                                    Vector(0, 0, 64.0f));
            SetGroundEntity(NULL);

            CSoundEnt::InsertSound(SOUND_PHYSICS_DANGER, GetAbsOrigin(), 256,
                                   0.5f, this);
          }
        }
      }
  ...
  // in episodic, an antlion colliding with the player in midair does him
  // damage. pursuant bugs 58590, 56960, this happens only once per glide.
#ifdef HL2_EPISODIC
  if (GetActivity() == ACT_GLIDE && IsValidEnemy(pOther) &&
      !m_bHasDoneAirAttack) {
    CTakeDamageInfo dmgInfo(this, this, sk_antlion_air_attack_dmg.GetInt(),
                            DMG_SLASH);
    ...
    if (bIsPlayer && !(pOther->GetFlags() & FL_GODMODE) &&
        pOther->GetMoveType() != MOVETYPE_NOCLIP) {
  ...
  if (pOther->IsNPC()) {
    if (pOther->Classify() == CLASS_COMBINE_GUNSHIP) {
      ...
      GuessDamageForce(&dmgInfo, (pOther->GetAbsOrigin() - GetAbsOrigin()),
                       pOther->GetAbsOrigin());
      TakeDamage(dmgInfo);
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_Antlion::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                      float flInterval) {
  if (hl2_episodic.GetBool()) {
...
Vector CNPC_Antlion::BodyTarget(const Vector &posSrc, bool bNoisy /*= true*/) {
  ...
  if (bNoisy) return vecResult + RandomVector(-8, 8);
...
void CNPC_Antlion::Flip(bool bZapped /*= false*/) {
  ...
  if (IsRunningDynamicInteraction()) return;

  SetCondition(COND_ANTLION_FLIPPED);

  if (bZapped) {
    m_flZapDuration = gpGlobals->curtime +
                      SequenceDuration(SelectWeightedSequence(
                          (Activity)ACT_ANTLION_ZAP_FLIP)) +
                      0.1f;
...
void CNPC_Antlion::InputJumpAtTarget(inputdata_t &inputdata) {
  CBaseEntity *pJumpTarget =
      gEntList.FindEntityByName(NULL, inputdata.value.String(), this,
                                inputdata.pActivator, inputdata.pCaller);
  if (pJumpTarget == NULL) {
  ...
  Vector vecApex;
  Vector rawJumpVel = GetMoveProbe()->CalcJumpLaunchVelocity(
      GetAbsOrigin(), targetPos, sv_gravity.GetFloat() * GetJumpGravity(),
      &minJumpHeight, maxHorzVel, &vecApex);
...
void CNPC_Antlion::DoPoisonBurst() {
  if (GetWaterLevel() < 2) {
    ...
    RadiusDamage(info, GetAbsOrigin(),
                 sk_antlion_worker_burst_radius.GetFloat(), CLASS_NONE, this);

    DispatchParticleEffect("antlion_gib_02", WorldSpaceCenter(),
                           GetAbsAngles());
  } else {
    CEffectData data;
    ...
    data.m_flScale = 128;
    data.m_fFlags = (SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS |
...
bool CNPC_Antlion::IsHeavyDamage(const CTakeDamageInfo &info) {
  if (hl2_episodic.GetBool() && IsWorker()) {
    if (m_nSustainedDamage + info.GetDamage() > 6) return true;
  }
...
float CAntlionRepellant::GetRadius(void) {
  if (m_bEnabled == false) return 0.0f;
  ...
  for (int i = 0; i < m_hRepellantList.Count(); i++) {
    if (m_hRepellantList[i]) {
      ...
      if (pRep) {
        ...
        if (flDist <= pRep->GetRadius()) return false;
      }
    }
  }
...
DEFINE_SCHEDULE(
    ...
    "		TASK_PLAY_SEQUENCE				"
    "ACTIVITY:ACT_ANTLION_JUMP_START"
    "		TASK_ANTLION_JUMP				0"
    ""
    "	Interrupts"
    "		COND_TASK_FAILED")
...
DEFINE_SCHEDULE(SCHED_ANTLION_WAIT_UNBORROW,

                "	Tasks"
                "		TASK_ANTLION_BURROW_WAIT		0"
                "		TASK_SET_SCHEDULE			"
                "	SCHEDULE:SCHED_ANTLION_WAIT_FOR_CLEAR_UNBORROW"
                ""
                "	Interrupts"
                "		COND_TASK_FAILED")
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_ANTLION_BURROW_WAIT"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_PLAY_SEQUENCE					"
    "ACTIVITY:ACT_ANTLION_BURROW_IN"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_ANTLION_BURROW_IN"
    ""
    "	Interrupts"
    "		COND_TASK_FAILED"
    "		COND_GIVE_WAY"
...
DEFINE_SCHEDULE(SCHED_ANTLION_BURROW_OUT,
                ...
                "		TASK_PLAY_SEQUENCE			"
                "	ACTIVITY:ACT_ANTLION_BURROW_OUT"
                ""
                "	Interrupts"
                "		COND_TASK_FAILED")
...
DEFINE_SCHEDULE(SCHED_ANTLION_WAIT_FOR_CLEAR_UNBORROW,
                ...
                "		TASK_SET_SCHEDULE			"
                "		SCHEDULE:SCHED_ANTLION_BURROW_OUT"
                ""
                "	Interrupts"
                "		COND_TASK_FAILED")
...
//=========================================================
DEFINE_SCHEDULE(SCHED_ANTLION_DISMOUNT_NPC,
...
DEFINE_SCHEDULE(
    ...
    "		COND_TASK_FAILED")
// Pounce forward at our enemy
DEFINE_SCHEDULE(SCHED_ANTLION_POUNCE_MOVING,
                ...
                "		TASK_RESET_ACTIVITY		0"
                "		TASK_PLAY_SEQUENCE		"
                "ACTIVITY:ACT_ANTLION_POUNCE_MOVING"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_ANTLION_DROWN,

                "	Tasks"
                "		TASK_SET_ACTIVITY			"
                "ACTIVITY:ACT_ANTLION_DROWN"
...
DEFINE_SCHEDULE(SCHED_ANTLION_WORKER_FLANK_RANDOM,
                ...
                "		1"  // Spend 1 second trying to build a path if
                                    // stuck
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ROUTE_SEARCH_TIME		1"  // Spend 1 second
                                                            // trying to build a
                                                            // path if stuck
...
bool IsAntlionWorker(CBaseEntity *pEntity) {
  ...
  return (pEntity != NULL && pEntity->Classify() == CLASS_ANTLION &&
          pEntity->HasSpawnFlags(SF_ANTLION_WORKER) &&
          dynamic_cast<CNPC_Antlion *>(pEntity) !=
              NULL);  // Save this as the last step
}
...
#ifdef HL2_EPISODIC
//-----------------------------------------------------------------------------
// Purpose: Used by other entities to judge the antlion worker's radius of
// damage
//-----------------------------------------------------------------------------
