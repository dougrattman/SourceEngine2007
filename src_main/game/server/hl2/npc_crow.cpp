...
void CNPC_Crow::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_CROW_HOP) {
    ...
    //
    // How fast does the crow need to travel to reach the hop goal given
    // gravity?
    //
...
void CNPC_Crow::InputFlyAway(inputdata_t &inputdata) {
  ...
  if (sTarget != NULL_STRING)  // this npc has a target
  {
    ...
    if (pEnt) {
      trace_t tr;
      AI_TraceLine(EyePosition(), pEnt->GetAbsOrigin(), MASK_NPCSOLID, this,
...
bool CNPC_Crow::OverrideMove(float flInterval) {
  if (GetNavigator()->GetPath()->CurWaypointNavType() == NAV_FLY &&
      GetNavigator()->GetNavType() != NAV_FLY) {
    SetNavType(NAV_FLY);
  }
      ...
      if (m_flLastStuckCheck <= gpGlobals->curtime) {
        if (m_vLastStoredOrigin == GetAbsOrigin()) {
          if (GetAbsVelocity() == vec3_origin) {
            ...
            GuessDamageForce(&dmgInfo, vec3_origin - Vector(0, 0, 0.1),
                             GetAbsOrigin());
            TakeDamage(dmgInfo);
...
Activity CNPC_Crow::NPC_TranslateActivity(Activity eNewActivity) {
  ...
  if (eNewActivity == ACT_FLY) {
    if (m_flSoarTime < gpGlobals->curtime) {
      ...
      if (random->RandomInt(0, 100) <= 50 && m_bSoar == false &&
          GetAbsVelocity().z < 0) {
        m_bSoar = true;
...
void CNPC_Crow::MoveCrowFly(float flInterval) {
  ...
  if (GetNavigator()->IsGoalActive()) {
    ...
    if (GetNavigator()->CurWaypointIsGoal() == false) {
      ...
      GetNavigator()->ProgressFlyPath(
          params);  // ignore result, crow handles completion directly
  ...
  if (GetHintNode()) {
    ...
    GetMoveProbe()->MoveLimit(NAV_FLY, GetAbsOrigin(),
  ...
  if (GetAbsVelocity().Length() > 0 && GetNavigator()->CurWaypointIsGoal() &&
      flDistance < CROW_AIRSPEED) {
    SetIdealActivity((Activity)ACT_CROW_LAND);
  }
  ...
  flRoll = std::clamp(flRoll, -45.0f, 45.0f);
...
//-----------------------------------------------------------------------------
bool CNPC_Crow::Probe(const Vector &vecMoveDir, float flSpeed,
                      Vector &vecDeflect) {
  //
  // Look 1/2 second ahead.
  //
  trace_t tr;
  AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() + vecMoveDir * flSpeed,
               GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this,
               HL2COLLISION_GROUP_CROW, &tr);
  if (tr.fraction < 1.0f) {
    //
...
void CNPC_Crow::SetFlyingState(FlyState_t eState) {
  if (eState == FlyState_Flying) {
    ...
    m_flGroundIdleMoveTime =
        gpGlobals->curtime + random->RandomFloat(5.0f, 10.0f);
...
void CNPC_Crow::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                            trace_t *ptr) {
...
void CNPC_Crow::StartTargetHandling(CBaseEntity *pTargetEnt) {
  AI_NavGoal_t goal(GOALTYPE_PATHCORNER, pTargetEnt->GetAbsOrigin(), ACT_FLY,
  ...
  switch (pTask->iTask) {
    ...
    case TASK_CROW_PICK_EVADE_GOAL: {
      if (GetEnemy() != NULL) {
        ...
        m_vSavePosition =
            GetAbsOrigin() + m_vSavePosition * (32 + random->RandomInt(0, 32));
    ...
    case TASK_FIND_HINTNODE: {
      ...
      if (!GetHintNode()) {
        SetHintNode(CAI_HintManager::FindHint(
            this, HINT_CROW_FLYTO_POINT,
            bits_HINT_NODE_NEAREST | bits_HINT_NODE_USE_GROUP, 10000));
      }
    ...
    case TASK_GET_PATH_TO_HINTNODE: {
      ...
      if (GetHintNode()) {
        ...
        SetNavType(NAV_FLY);
        CapabilitiesAdd(bits_CAP_MOVE_FLY);
        // @HACKHACK: Force allow triangulation. Too many HL2 maps were relying
        // on this feature WRT fly nodes (toml 8/1/2007)
    ...
    //
    // We have failed to fly normally. Pick a random "up" direction and fly that
    // way.
    //
    case TASK_CROW_FLY: {
      float flYaw = UTIL_AngleMod(random->RandomInt(-180, 180));

      Vector vecNewVelocity(cos(DEG2RAD(flYaw)), sin(DEG2RAD(flYaw)),
                            random->RandomFloat(0.1f, 0.5f));
    ...
    case TASK_CROW_PICK_RANDOM_GOAL: {
      m_vSavePosition =
          GetLocalOrigin() + Vector(random->RandomFloat(-48.0f, 48.0f),
                                    random->RandomFloat(-48.0f, 48.0f), 0);
      TaskComplete();
      break;
    }
  ...
  switch (pTask->iTask) {
    case TASK_CROW_TAKEOFF: {
      if (GetNavigator()->IsGoalActive()) {
        GetMotor()->SetIdealYawToTargetAndUpdate(
            GetAbsOrigin() + GetNavigator()->GetCurWaypointPos(),
            AI_KEEP_YAW_SPEED);
      } else
        TaskFail(FAIL_NO_ROUTE);
    ...
    case TASK_CROW_HOP: {
      ...
      if ((GetAbsOrigin().z < m_flHopStartZ) && (!(GetFlags() & FL_ONGROUND))) {
        ...
        //
        trace_t tr;
        AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -32),
                     MASK_SOLID, this, HL2COLLISION_GROUP_CROW, &tr);
        if (tr.fraction == 1.0f) {
          //
          // We're falling! Better fly away. SelectSchedule will check ONGROUND
          // and do the right thing.
          //
          TaskComplete();
        } else {
          //
          // We'll be okay. Don't check again unless what we're hopping onto
    ...
    case TASK_CROW_FLY: {
      GetMotor()->SetIdealYawToTargetAndUpdate(
          GetAbsOrigin() + GetAbsVelocity(), AI_KEEP_YAW_SPEED);
    ...
    case TASK_CROW_WAIT_FOR_BARNACLE_KILL: {
      if (m_flNextFlinchTime < gpGlobals->curtime) {
        m_flNextFlinchTime =
            gpGlobals->curtime + random->RandomFloat(0.5f, 2.0f);
        // dvs: TODO: squirm
...
bool CNPC_Crow::CorpseGib(const CTakeDamageInfo &info) {
  ...
  // TODO: crow gibs?
  // CGib::SpawnSpecificGibs( this, CROW_GIB_COUNT, 300, 400,
  // "models/gibs/crow_gibs.mdl");
...
//-----------------------------------------------------------------------------
#define CROW_RAGDOLL_SPEED_LIMIT \
  1000.0f  // Crow ragdoll speed limit in inches per second.
bool CNPC_Crow::BecomeRagdollOnClient(const Vector &force) {
  ...
  if (VPhysicsGetObject()) {
    ...
    speed = std::min(speed, (CROW_RAGDOLL_SPEED_LIMIT * flMass));
    newForce *= speed;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Returns the best new schedule for this NPC based on current
// conditions.
//-----------------------------------------------------------------------------
int CNPC_Crow::SelectSchedule(void) {
  ...
  switch (m_NPCState) {
    ...
    case NPC_STATE_COMBAT: {
      if (!IsFlying()) {
        if (m_bOnJeep == true) return SCHED_IDLE_STAND;

        //
        // If we are hanging out on the ground, see if it is time to pick a new
        // place to walk to.
        //
        if (gpGlobals->curtime > m_flGroundIdleMoveTime) {
          m_flGroundIdleMoveTime =
              gpGlobals->curtime + random->RandomFloat(10.0f, 20.0f);
...
void CNPC_Crow::IdleSound(void) {
  if (m_iBirdType != BIRDTYPE_CROW) return;
...
//-----------------------------------------------------------------------------
bool CNPC_Crow::HandleInteraction(int interactionType, void *data,
...
int CNPC_Crow::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    if (GetEnemy() != NULL) {
      Q_snprintf(tempstr, sizeof(tempstr), "enemy (dist): %s (%g)",
                 GetEnemy()->GetClassname(), (double)m_flEnemyDist);
      EntityText(nOffset, tempstr, 0);
      nOffset++;
    }
  }
