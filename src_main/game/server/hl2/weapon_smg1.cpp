...
class CWeaponSMG1 : public CHLSelectFireMachineGun {
 ...
 public:
  ...
  void FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator,
                            Vector &vecShootOrigin, Vector &vecShootDir);
...
void CWeaponSMG1::FireNPCPrimaryAttack(CBaseCombatCharacter *pOperator,
  ...
  CSoundEnt::InsertSound(SOUND_COMBAT | SOUND_CONTEXT_GUNFIRE,
                         pOperator->GetAbsOrigin(), SOUNDENT_VOLUME_MACHINEGUN,
                         0.2, pOperator, SOUNDENT_CHANNEL_WEAPON,
                         pOperator->GetEnemy());
  pOperator->FireBullets(1, vecShootOrigin, vecShootDir,
                         VECTOR_CONE_PRECALCULATED, MAX_TRACE_LENGTH,
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponSMG1::Operator_ForceNPCFire(CBaseCombatCharacter *pOperator,
                                        bool bSecondary) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponSMG1::Operator_HandleAnimEvent(animevent_t *pEvent,
                                           CBaseCombatCharacter *pOperator) {
  switch (pEvent->event) {
    case EVENT_WEAPON_SMG1: {
      ...
      if ((pEvent->options == NULL) || (pEvent->options[0] == '\0') ||
          (!pOperator->GetAttachment(pEvent->options, vecShootOrigin,
                                     angDiscard))) {
      ...
      vecShootOrigin, vec3_angle, npc ); pGrenade->SetAbsVelocity( vecThrow );
...
void CWeaponSMG1::SecondaryAttack(void) {
  ...
  CGrenadeAR2 *pGrenade =
      (CGrenadeAR2 *)Create("grenade_ar2", vecSrc, angles, pPlayer);
...
int CWeaponSMG1::WeaponRangeAttack2Condition(float flDot, float flDist) {
  ...
  */
  ...
  if (!(pEnemy->GetFlags() & FL_ONGROUND) && pEnemy->GetWaterLevel() == 0 &&
    ...
    //!Players who jump a lot are unlikely to
    // be grenaded.
  ...
  // pEnemy->GetLocalOrigin()); estimate position vecTarget = vecTarget +
  // pEnemy->m_vecVelocity * 2;
  ...
  while ((pTarget = gEntList.FindEntityInSphere(
              pTarget, vecTarget, COMBINE_MIN_GRENADE_CLEAR_DIST)) != NULL) {
    // Check to see if the default relationship is hatred, and if so intensify
    // that
