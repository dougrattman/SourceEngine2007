...
enum {
  // NOTE: Exact #s are important, since they are referred to by number in
  // schedules below
...
ConVar sk_metropolice_stitch_tight_hitcount(
    "sk_metropolice_stitch_tight_hitcount", "2");
ConVar sk_metropolice_stitch_at_hitcount("sk_metropolice_stitch_at_hitcount",
                                         "1");
ConVar sk_metropolice_stitch_behind_hitcount(
    "sk_metropolice_stitch_behind_hitcount", "3");
ConVar sk_metropolice_stitch_along_hitcount(
...
BEGIN_DATADESC(CNPC_MetroPolice)
  ...
  DEFINE_EMBEDDED(m_BatonSwingTimer), DEFINE_EMBEDDED(m_NextChargeTimer),
      ...
      DEFINE_EMBEDDED(m_TimeYieldShootSlot), DEFINE_EMBEDDED(m_Sentences),
...
// Purpose
//------------------------------------------------------------------------------
CBaseEntity *CNPC_MetroPolice::CheckTraceHullAttack(
    float flDist, const Vector &mins, const Vector &maxs, int iDamage,
    int iDmgType, float forceScale, bool bDamageAnyNPC) {
  // If only a length is given assume we want to trace in our facing direction
  Vector forward;
  AngleVectors(GetAbsAngles(), &forward);
  Vector vStart = GetAbsOrigin();

  // The ideal place to start the trace is in the center of the attacker's
  // bounding box. however, we need to make sure there's enough clearance. Some
  // of the smaller monsters aren't as big as the hull we try to trace with.
  // (SJB)
  float flVerticalOffset = WorldAlignSize().z * 0.5;

  if (flVerticalOffset < maxs.z) {
    // There isn't enough room to trace this hull, it's going to drag the
    // ground. so make the vertical offset just enough to clear the ground.
...
class CTraceFilterMetroPolice : public CTraceFilterEntitiesOnly {
 public:
  ...
  CTraceFilterMetroPolice(const IHandleEntity *passentity, int collisionGroup,
  ...
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    ...
    if (pEntity) {
      ...
      Vector attackDir = pEntity->WorldSpaceCenter() -
                         m_dmgInfo->GetAttacker()->WorldSpaceCenter();
      VectorNormalize(attackDir);

      CTakeDamageInfo info = (*m_dmgInfo);
      CalculateMeleeDamageForce(&info, attackDir,
                                info.GetAttacker()->WorldSpaceCenter(),
                                m_flForceScale);
      ...
      if (pBCC && pVictimBCC) {
        ...
        if (m_bDamageAnyNPC || pBCC->IRelationType(pEntity) == D_HT ||
          ...
          if (info.GetDamage()) {
            ...
            if (!pEntity->IsPlayer() ||
                GlobalEntity_GetState("gordon_precriminal") == GLOBAL_OFF) {
              if (pEntity->IsPlayer() &&
                  ((CBasePlayer *)pEntity)->IsSuitEquipped()) {
                info.ScaleDamage(.25);
...
CBaseEntity *CNPC_MetroPolice::CheckTraceHullAttack(
    ...
    const Vector &maxs, int iDamage, int iDmgType, float flForceScale,
    bool bDamageAnyNPC) {
  CTakeDamageInfo dmgInfo(this, this, iDamage, DMG_SLASH);

  CTraceFilterMetroPolice traceFilter(this, COLLISION_GROUP_NONE, &dmgInfo,
                                      flForceScale, bDamageAnyNPC);
  ...
  if (pEntity == NULL) {
    // See if perhaps I'm trying to claw/bash someone who is standing on my
    // head.
    Vector vecTopCenter;
    Vector vecEnd;
...
void CNPC_MetroPolice::NotifyDeadFriend(CBaseEntity *pFriend) {
  ...
  if (pFriend == m_hManhack) {
    m_Sentences.Speak("METROPOLICE_MANHACK_KILLED", SENTENCE_PRIORITY_NORMAL,
  ...
  if (FClassnameIs(pFriend, "npc_manhack")) return;
  ...
  if (GetSquad()->NumMembers() < 2) {
    m_Sentences.Speak("METROPOLICE_LAST_OF_SQUAD", SENTENCE_PRIORITY_MEDIUM,
                      SENTENCE_CRITERIA_NORMAL);
    return;
  }
...
void CNPC_MetroPolice::PrescheduleThink(void) {
  ...
  if (PlayerIsCriminal() == false) {
    ...
    if (pPlayer &&
        (pPlayer->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr() <
            (128 * 128)) {
      m_bPlayerIsNear = true;
      AddLookTarget(pPlayer, 0.75f, 5.0f);

      if ((m_PolicingBehavior.IsEnabled() == false) &&
          (m_nNumWarnings >= METROPOLICE_MAX_WARNINGS)) {
        m_flBatonDebounceTime =
            gpGlobals->curtime + random->RandomFloat(2.5f, 4.0f);
        SetTarget(pPlayer);
        SetBatonState(true);
      }
    } else {
      if (m_PolicingBehavior.IsEnabled() == false &&
          gpGlobals->curtime > m_flBatonDebounceTime) {
        SetBatonState(false);
      }
...
//-----------------------------------------------------------------------------
bool CNPC_MetroPolice::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                          float flInterval) {
  // Don't do this if we're scripted
  if (IsInAScript()) return BaseClass::OverrideMoveFacing(move, flInterval);
  ...
  if ( m_PolicingBehavior.IsEnabled() )
  {
          ...
          if ( pTarget )
          {
                  AddFacingTarget( pTarget, pTarget->WorldSpaceCenter(), 1.0f,
  0.2f );
          }
  }
  */
...
void CNPC_MetroPolice::Spawn(void) {
  ...
  m_flFieldOfView = -0.2;  // indicates the width of this NPC's forward view
                           // cone ( as a dotproduct result )
  ...
  if (HasSpawnFlags(SF_METROPOLICE_ALWAYS_STITCH)) {
    if (!Weapon_OwnsThisType("weapon_smg1")) {
      Warning(
          "Warning! Metrocop is trying to use the stitch behavior but he has "
          "no smg1!\n");
...
void CNPC_MetroPolice::SpeakStandoffSentence(int nSentenceType) {
  switch (nSentenceType) {
    case STANDOFF_SENTENCE_BEGIN_STANDOFF:
      m_Sentences.Speak("METROPOLICE_SO_BEGIN", SENTENCE_PRIORITY_HIGH,
                        SENTENCE_CRITERIA_SQUAD_LEADER);
      break;

    case STANDOFF_SENTENCE_END_STANDOFF:
      m_Sentences.Speak("METROPOLICE_SO_END", SENTENCE_PRIORITY_HIGH,
                        SENTENCE_CRITERIA_SQUAD_LEADER);
      break;
    ...
    case STANDOFF_SENTENCE_STAND_CHECK_TARGET:
      if (gm_flTimeLastSpokePeek != 0 &&
          gpGlobals->curtime - gm_flTimeLastSpokePeek > 20) {
...
void CNPC_MetroPolice::SpeakAssaultSentence(int nSentenceType) {
  switch (nSentenceType) {
    case ASSAULT_SENTENCE_HIT_RALLY_POINT:
      m_Sentences.SpeakQueued("METROPOLICE_AS_HIT_RALLY",
                              SENTENCE_PRIORITY_NORMAL);
      break;

    case ASSAULT_SENTENCE_HIT_ASSAULT_POINT:
      m_Sentences.SpeakQueued("METROPOLICE_AS_HIT_ASSAULT",
                              SENTENCE_PRIORITY_NORMAL);
      break;

    case ASSAULT_SENTENCE_SQUAD_ADVANCE_TO_RALLY:
      if (m_Sentences.Speak("METROPOLICE_AS_ADV_RALLY",
                            SENTENCE_PRIORITY_MEDIUM,
                            SENTENCE_CRITERIA_SQUAD_LEADER) >= 0) {
        GetSquad()->BroadcastInteraction(
            g_interactionMetrocopClearSentenceQueues, NULL);
      }
      break;

    case ASSAULT_SENTENCE_SQUAD_ADVANCE_TO_ASSAULT:
      if (m_Sentences.Speak("METROPOLICE_AS_ADV_ASSAULT",
                            SENTENCE_PRIORITY_MEDIUM,
                            SENTENCE_CRITERIA_SQUAD_LEADER) >= 0) {
        GetSquad()->BroadcastInteraction(
            g_interactionMetrocopClearSentenceQueues, NULL);
...
void CNPC_MetroPolice::SpeakSentence(int nSentenceType) {
  ...
  switch (nSentenceType) {
    case METROPOLICE_SENTENCE_FREEZE:
      m_Sentences.Speak("METROPOLICE_FREEZE", SENTENCE_PRIORITY_MEDIUM,
                        SENTENCE_CRITERIA_NORMAL);
      break;

    case METROPOLICE_SENTENCE_HES_OVER_HERE:
      m_Sentences.Speak("METROPOLICE_OVER_HERE", SENTENCE_PRIORITY_MEDIUM,
                        SENTENCE_CRITERIA_NORMAL);
      break;

    case METROPOLICE_SENTENCE_HES_RUNNING:
      m_Sentences.Speak("METROPOLICE_HES_RUNNING", SENTENCE_PRIORITY_HIGH,
                        SENTENCE_CRITERIA_NORMAL);
      break;

    case METROPOLICE_SENTENCE_TAKE_HIM_DOWN:
      m_Sentences.Speak("METROPOLICE_TAKE_HIM_DOWN", SENTENCE_PRIORITY_HIGH,
                        SENTENCE_CRITERIA_NORMAL);
      break;

    case METROPOLICE_SENTENCE_ARREST_IN_POSITION:
      m_Sentences.Speak("METROPOLICE_ARREST_IN_POS", SENTENCE_PRIORITY_MEDIUM,
                        SENTENCE_CRITERIA_NORMAL);
      break;
      ...
      if (pEntity && !HasSpawnFlags(SF_METROPOLICE_ARREST_ENEMY)) {
        if (pEntity->IsPlayer() && (pEntity->GetHealth() <= 20)) {
          if (!HasMemory(bits_MEMORY_PLAYER_HURT)) {
            if (m_Sentences.Speak("METROPOLICE_PLAYERHIT",
                                  SENTENCE_PRIORITY_HIGH) >= 0) {
    ...
    case METROPOLICE_SENTENCE_HEARD_SOMETHING:
      if ((GetState() == NPC_STATE_ALERT) || (GetState() == NPC_STATE_IDLE)) {
        m_Sentences.Speak("METROPOLICE_HEARD_SOMETHING",
                          SENTENCE_PRIORITY_MEDIUM);
      }
      break;
  }
  ...
  if (pSoundOwner) {
    ...
    if (pGrenade) {
      if (IRelationType(pGrenade->GetThrower()) != D_LI) {
        ...
        m_Sentences.Speak("METROPOLICE_DANGER_GREN", SENTENCE_PRIORITY_HIGH,
                          SENTENCE_CRITERIA_NORMAL);
      }
      return;
    }
    ...
    if (pSoundOwner->GetServerVehicle()) {
      m_Sentences.Speak("METROPOLICE_DANGER_VEHICLE", SENTENCE_PRIORITY_HIGH,
                        SENTENCE_CRITERIA_NORMAL);
      return;
    }
    ...
    if (FClassnameIs(pSoundOwner, "npc_manhack")) {
      if (pSoundOwner->HasPhysicsAttacker(1.0f)) {
        m_Sentences.Speak("METROPOLICE_DANGER_MANHACK", SENTENCE_PRIORITY_HIGH,
                          SENTENCE_CRITERIA_NORMAL);
      }
      return;
    }
...
bool CNPC_MetroPolice::IsEnemyInAnAirboat() const {
  ...
  CBaseEntity *pVehicle =
      static_cast<CBasePlayer *>(GetEnemy())->GetVehicleEntity();
...
CBaseEntity *CNPC_MetroPolice::GetShootTarget() {
  ...
  if (!pEnemy || !pEnemy->IsPlayer()) return pEnemy;

  CBaseEntity *pVehicle =
      static_cast<CBasePlayer *>(pEnemy)->GetVehicleEntity();
  ...
  if (Weapon_OwnsThisType("weapon_pistol")) {
    if (m_nBurstMode == BURST_NOT_ACTIVE) {
      if (GetEnemy()) {
        ...
        int nMinBurst =
            MIN_MIN_PISTOL_BURST +
            (MAX_MIN_PISTOL_BURST - MIN_MIN_PISTOL_BURST) * (1.0 - factor);
        int nMaxBurst =
            MIN_MAX_PISTOL_BURST +
            (MAX_MAX_PISTOL_BURST - MIN_MAX_PISTOL_BURST) * (1.0 - factor);
        float flMinRestInterval =
            MIN_MIN_PISTOL_REST_INTERVAL +
            (MAX_MIN_PISTOL_REST_INTERVAL - MIN_MIN_PISTOL_REST_INTERVAL) *
                factor;
        float flMaxRestInterval =
            MIN_MAX_PISTOL_REST_INTERVAL +
            (MAX_MAX_PISTOL_REST_INTERVAL - MIN_MAX_PISTOL_REST_INTERVAL) *
                factor;

        GetShotRegulator()->SetRestInterval(flMinRestInterval,
                                            flMaxRestInterval);
        GetShotRegulator()->SetBurstShotCountRange(nMinBurst, nMaxBurst);
      } else {
        GetShotRegulator()->SetBurstShotCountRange(
            GetActiveWeapon()->GetMinBurst(), GetActiveWeapon()->GetMaxBurst());
...
void CNPC_MetroPolice::SetBurstMode(bool bEnable) {
  ...
  if (m_nBurstMode != nOldBurstMode) {
    ...
    if (m_nBurstMode == BURST_NOT_ACTIVE) {
      ...
      GetShotRegulator()->GetBurstShotCountRange(&nMinBurstCount,
                                                 &nMaxBurstCount);
...
bool CNPC_MetroPolice::ShouldAttemptToStitch() {
  if (IsEnemyInAnAirboat()) return true;

  if (!GetShootTarget()) return false;

  if (HasSpawnFlags(SF_METROPOLICE_ALWAYS_STITCH)) {
    // Don't stitch if the player is at the same level or higher
    if (GetEnemy()->GetAbsOrigin().z - GetAbsOrigin().z > -36) return false;
...
Vector CNPC_MetroPolice::StitchAimTarget(const Vector &posSrc, bool bNoisy) {
  ...
  if (!IsEnemyInAnAirboat()) {
    ...
    if ((GetEnemy()->GetWaterLevel() == 0) &&
        (GetEnemy()->GetFlags() & FL_ONGROUND)) {
      GetEnemy()->CollisionProp()->NormalizedToWorldSpace(
          Vector(0.5f, 0.5f, 0.08f), &vecBodyTarget);
    ...
    trace_t trace;
    GetEnemy()->CollisionProp()->NormalizedToWorldSpace(
        Vector(0.5f, 0.5f, 1.0f), &vecBodyTarget);
    float flHeight = GetEnemy()->WorldAlignSize().z;
    UTIL_TraceLine(vecBodyTarget, vecBodyTarget + Vector(0, 0, -flHeight - 80),
                   (MASK_SOLID_BRUSHONLY | MASK_WATER), NULL,
                   COLLISION_GROUP_NONE, &trace);
    return trace.endpos;
  }

  // NOTE: HACK! Ths 0.08 is where the water level happens to be.
  // We probably want to find the exact water level and use that as the z
  // position.
  Vector vecBodyTarget;
  if (!bNoisy) {
    GetShootTarget()->CollisionProp()->NormalizedToWorldSpace(
        Vector(0.5f, 0.5f, 0.08f), &vecBodyTarget);
  } else {
    GetShootTarget()->CollisionProp()->RandomPointInBounds(
        Vector(0.25f, 0.25f, 0.08f), Vector(0.75f, 0.75f, 0.08f),
        &vecBodyTarget);
  }
...
// Burst mode!
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::AimBurstRandomly(int nMinCount, int nMaxCount,
                                        float flMinDelay, float flMaxDelay) {
  if (!IsCurrentlyFiringBurst()) return;

  GetShotRegulator()->SetParameters(nMinCount, nMaxCount, flMinDelay,
                                    flMaxDelay);
  ...
  Vector vecDelta =
      StitchAimTarget(GetAbsOrigin(), true) - Weapon_ShootPosition();
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::RandomDirectionBetweenVectors(const Vector &vecStart,
                                                     const Vector &vecEnd,
                                                     Vector *pResult) {
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::PredictShootTargetPosition(float flDeltaTime,
                                                  float flMinLeadDist,
                                                  float flAddVelocity,
                                                  Vector *pVecTarget,
                                                  Vector *pVecTargetVelocity) {
  ...
  *pVecTarget = StitchAimTarget(GetAbsOrigin(), true);
  ...
  flAngVel -= 30.0f;
  if (flAngVel > 0.0f) {
    MatrixBuildRotationAboutAxis(rotationMatrix, angImpulse,
                                 flAngVel * flDeltaTime * 0.333f);
  ...
  *pVecTargetVelocity = vecLeadVector;
  ...
  if ((vecLeadVector.LengthSqr() * flDeltaTime * flDeltaTime) <
      flMinLeadDist * flMinLeadDist) {
...
//-----------------------------------------------------------------------------
// Compute a predicted velocity n seconds into the future (given a known
// acceleration rate)
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::PredictShootTargetVelocity(float flDeltaTime,
                                                  Vector *pVecTargetVel) {
...
// Visualize stitch
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::VisualizeStitch(const Vector &vecStart,
                                       const Vector &vecEnd) {
  NDebugOverlay::Cross3D(vecStart, -Vector(32, 32, 32), Vector(32, 32, 32), 255,
                         0, 0, false, 5.0f);
  NDebugOverlay::Cross3D(vecEnd, -Vector(32, 32, 32), Vector(32, 32, 32), 0,
                         255, 0, false, 5.0f);
...
void CNPC_MetroPolice::VisualizeLineOfDeath() {
  ...
  NDebugOverlay::Line(m_vecBurstLineOfDeathOrigin,
                      m_vecBurstLineOfDeathOrigin + m_vecBurstLineOfDeathDelta,
                      255, 255, 0, false, 5.0f);
  VectorMA(m_vecBurstLineOfDeathOrigin, m_flBurstSteerDistance, vecAcross,
           vecStart);
  NDebugOverlay::Line(vecStart, vecStart + m_vecBurstLineOfDeathDelta, 255, 0,
                      0, false, 5.0f);
  VectorMA(m_vecBurstLineOfDeathOrigin, -m_flBurstSteerDistance, vecAcross,
           vecStart);
  NDebugOverlay::Line(vecStart, vecStart + m_vecBurstLineOfDeathDelta, 255, 0,
...
#define AIM_AT_NEAR_DISTANCE_DELTA \
  (AIM_AT_NEAR_DISTANCE_MAX - AIM_AT_NEAR_DISTANCE_MIN)
...
#define AIM_AT_FAR_DISTANCE_BONUS \
  200.0f  // Add this much bonus after each BONUS_DISTANCE

//-----------------------------------------------------------------------------
// Modify the stitch length
//-----------------------------------------------------------------------------
float CNPC_MetroPolice::ComputeDistanceStitchModifier(
    float flDistanceToTarget) const {
  ...
  if (flDistanceToTarget < AIM_AT_NEAR_DISTANCE_MAX) {
    float flFraction = 1.0f - ((flDistanceToTarget - AIM_AT_NEAR_DISTANCE_MIN) /
                               AIM_AT_NEAR_DISTANCE_DELTA);
    return flFraction * AIM_AT_NEAR_DISTANCE_BONUS;
  }

  if (flDistanceToTarget > AIM_AT_FAR_DISTANCE_MIN) {
    float flFactor = (flDistanceToTarget - AIM_AT_FAR_DISTANCE_MIN) /
                     AIM_AT_FAR_DISTANCE_BONUS_DISTANCE;
...
int CNPC_MetroPolice::SetupBurstShotRegulator(float flReactionTime) {
  ...
  GetShotRegulator()->SetBurstShotCountRange(nDesiredShotCount,
                                             nDesiredShotCount);
...
void CNPC_MetroPolice::AimBurstTightGrouping(float flShotTime) {
  ...
  float flDistToTargetSqr =
      GetShootTarget()->WorldSpaceCenter().DistToSqr(Weapon_ShootPosition());
  ...
  if ((flDistToTargetSqr > TIGHT_GROUP_MIN_DIST * TIGHT_GROUP_MIN_DIST) ||
      (vecTargetVel.LengthSqr() >
       TIGHT_GROUP_MIN_SPEED * TIGHT_GROUP_MIN_SPEED)) {
...
static float s_pReactionFraction[AIM_AT_TIME_DIST_COUNT]
...
float CNPC_MetroPolice::AimBurstAtReactionTime(float flReactionTime,
  ...
  if (IsEnemyInAnAirboat()) {
    ...
    int nu = (int)u;
    int nv = (int)v;
    if ((nu < AIM_AT_TIME_SPEED_COUNT - 1) &&
        (nv < AIM_AT_TIME_DIST_COUNT - 1)) {
      float fu = u - nu;
      float fv = v - nv;
      float flReactionFactor =
          s_pReactionFraction[nv][nu] * (1.0f - fu) * (1.0f - fv);
...
int CNPC_MetroPolice::AimBurstAtSetupHitCount(float flDistToTarget,
  ...
  if (IsEnemyInAnAirboat()) {
    ...
    int nu = (int)u;
    int nv = (int)v;
    if ((nu < AIM_AT_SHOT_SPEED_COUNT - 1) &&
        (nv < AIM_AT_SHOT_DIST_COUNT - 1)) {
      float fu = u - nu;
      float fv = v - nv;
      float flShotFactor =
          s_pShotCountFraction[nv][nu] * (1.0f - fu) * (1.0f - fv);
...
void CNPC_MetroPolice::AimBurstAtEnemy(float flReactionTime) {
  ...
  float flDistToTargetSqr =
      GetShootTarget()->WorldSpaceCenter().AsVector2D().DistToSqr(
          Weapon_ShootPosition().AsVector2D());
  float flDistToTarget = sqrt(flDistToTargetSqr);

  flReactionTime =
      AimBurstAtReactionTime(flReactionTime, flDistToTarget, flCurrentSpeed);
  ...
  float flExtraTime = GetTimeForShots(nExtraShots) +
                      (1.0f - AIM_AT_REACTION_TIME_FRACTION) * flReactionTime;
  ...
  PredictShootTargetPosition(flFirstHitTime, 0.0f, 0.0f, &vecShootAt,
                             &vecShootAtVel);
  ...
  // The max stitch distance here is used to guarantee the cop doesn't try to
  // lead the airboat so much that he ends up shooting behind himself
  ...
  if (bIsInVehicle) {
    ...
    VectorMA(vecShootAt, random->RandomFloat(flMinAdd, 100.0f), vecAcross,
             vecShootAt);
  }

  // Compute the distance along the stitch direction to the cop. we don't want
  // to cross that line
  ...
  trace_t trace;
  UTIL_TraceLine(vecStitchStart, vecStitchStart + Vector(0, 0, -512),
                 (MASK_SOLID_BRUSHONLY | MASK_WATER), NULL,
                 COLLISION_GROUP_NONE, &trace);
...
void CNPC_MetroPolice::AimBurstInFrontOfEnemy(float flReactionTime) {
  ...
  // The goal here is to slow him down. Choose a target position such that we
  // predict where he'd be in he accelerated by N over the reaction time.
  // Prevent him from getting there.
  Vector vecShootAt, vecShootAtVel, vecAcross;
  PredictShootTargetPosition(flReactionTime * AIM_IN_FRONT_REACTION_FRACTION,
                             AIM_IN_FRONT_OF_MINIMUM_DISTANCE, 0.0f,
                             &vecShootAt, &vecShootAtVel);
  ...
  RandomDirectionBetweenVectors(vecShootAtVel, vecTargetToGun,
                                &vecExtraDistance);
  ...
  VectorSubtract(Weapon_ShootPosition(), StitchAimTarget(GetAbsOrigin(), false),
                 vecTargetToGun);
  float flSign = (DotProduct(vecAcross, vecTargetToGun) >= 0.0f) ? 1.0f : -1.0f;
  VectorMA(vecShootAt, flSign * flStitchLength * AIM_IN_FRONT_REACTION_FRACTION,
           vecAcross, vecEndPoint1);
  VectorMA(vecShootAt,
           -flSign * flStitchLength * (1.0f - AIM_IN_FRONT_REACTION_FRACTION),
           vecAcross, vecEndPoint2);
  ...
  // Make the visual representation of the line of death lie closest to the
  // boat.
  VectorMA(m_vecBurstTargetPos, -AIM_IN_FRONT_STEER_DISTANCE, vecShootAtVel,
           m_vecBurstTargetPos);
  m_vecBurstDelta /= (nShotCount - 1);
...
void CNPC_MetroPolice::AimBurstBehindEnemy(float flShotTime) {
  ...
  // Make the visual representation of the line of death lie closest to the
  // boat.
  VectorMA(m_vecBurstTargetPos, -AIM_BEHIND_STEER_DISTANCE, vecDelta,
           m_vecBurstTargetPos);
  m_vecBurstDelta /= (nShotCount - 1);
...
void CNPC_MetroPolice::AimBurstAlongSideOfEnemy(float flFollowTime) {
  ...
  Vector vecShootAt, vecShootAtVel, vecAcross;
  PredictShootTargetPosition(AIM_ALONG_SIDE_LINE_OF_DEATH_LEAD_TIME, 225.0f,
  ...
  float flDist =
      AIM_ALONG_SIDE_LINE_OF_DEATH_DISTANCE + random->RandomFloat(0.0f, 50.0f);
  ...
  VectorMA(m_vecBurstLineOfDeathDelta, flSign * flExtraDist, vecAcross,
  ...
  // Make the visual representation of the line of death lie closest to the
  // boat.
  VectorMA(m_vecBurstTargetPos, -flSign * AIM_ALONG_SIDE_STEER_DISTANCE,
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::SteerBurstTowardTargetUseSpeedOnly(
    const Vector &vecShootAt, const Vector &vecShootAtVelocity,
    float flPredictTime, int nShotsTillPredict) {
  // Only account for changes in *speed*; ignore all changes in velocity
  // direction, etc. This one only hits the player if there is *no* steering,
  // just acceleration or decceleration
...
void CNPC_MetroPolice::SteerBurstTowardTargetUseVelocity(
    const Vector &vecShootAt, const Vector &vecShootAtVelocity,
...
void CNPC_MetroPolice::SteerBurstTowardTargetUsePosition(
    const Vector &vecShootAt, const Vector &vecShootAtVelocity,
    int nShotsTillPredict) {
...
void CNPC_MetroPolice::SteerBurstTowardPredictedPoint(
    const Vector &vecShootAt, const Vector &vecShootAtVelocity,
    int nShotsTillPredict) {
  // Account for velocity + position changes, but only within a constrained
  // cylinder
  Vector vecConstrainedShootPosition;
  CalcClosestPointOnLine(
      vecShootAt, m_vecBurstLineOfDeathOrigin,
      m_vecBurstLineOfDeathOrigin + m_vecBurstLineOfDeathDelta,
  ...
  if (vecDelta.LengthSqr() <= m_flBurstSteerDistance * m_flBurstSteerDistance) {
    ...
    VectorNormalize(vecDelta);
    VectorMA(vecConstrainedShootPosition, m_flBurstSteerDistance, vecDelta,
             vecConstrainedShootPosition);
  }

  // This method *always* hits if the entity is within the cylinder
  VectorSubtract(vecConstrainedShootPosition, m_vecBurstTargetPos,
                 m_vecBurstDelta);
...
void CNPC_MetroPolice::SteerBurstWithinLineOfDeath() {
  ...
  CalcClosestPointOnLine(
      m_vecBurstTargetPos, m_vecBurstLineOfDeathOrigin,
      m_vecBurstLineOfDeathOrigin + m_vecBurstLineOfDeathDelta,
      vecPointOnLineOfDeath);

  Vector vecDelta;
  VectorSubtract(vecShootAt, vecPointOnLineOfDeath, vecDelta);
  if (vecDelta.LengthSqr() <= m_flBurstSteerDistance * m_flBurstSteerDistance) {
    VectorSubtract(vecShootAt, m_vecBurstTargetPos, m_vecBurstDelta);
    if (m_vecBurstDelta.LengthSqr() >
        (STEER_LINE_OF_DEATH_MAX_DISTANCE * STEER_LINE_OF_DEATH_MAX_DISTANCE)) {
    ...
    float t;
    CalcClosestPointOnLine(
        vecNext, m_vecBurstLineOfDeathOrigin,
        m_vecBurstLineOfDeathOrigin + m_vecBurstLineOfDeathDelta,
        vecPointOnLineOfDeath, &t);
    if ((t < -0.1f) || (t > 1.1f)) {
...
void CNPC_MetroPolice::SteerBurstTowardTarget() {
  switch (m_nBurstSteerMode) {
    ...
    case BURST_STEER_ADJUST_FOR_SPEED_CHANGES: {
      // Predict the airboat position at the point where we were expecting to
      // hit them
      if (m_flBurstPredictTime <= gpGlobals->curtime) return;

      float flPredictTime = m_flBurstPredictTime - gpGlobals->curtime;
      int nShotsTillPredict = CountShotsInTime(flPredictTime);
      if (nShotsTillPredict <= 1) return;

      Vector vecShootAt, vecShootAtVelocity;
      PredictShootTargetPosition(flPredictTime, 0.0f, 0.0f, &vecShootAt,
                                 &vecShootAtVelocity);
      SteerBurstTowardTargetUseSpeedOnly(vecShootAt, vecShootAtVelocity,
                                         flPredictTime, nShotsTillPredict);
    } break;
...
//-----------------------------------------------------------------------------
Vector CNPC_MetroPolice::ComputeBurstLockOnTrajectory(
...
Vector CNPC_MetroPolice::ComputeBurstDeliberatelyMissTrajectory(
    const Vector &shootOrigin) {
...
//-----------------------------------------------------------------------------
Vector CNPC_MetroPolice::AimCloseToTargetButMiss(CBaseEntity *pTarget,
                                                 const Vector &shootOrigin) {
  ...
  VectorMA(vecRandomDir, -DotProduct(vecNormalizedSpace, vecRandomDir),
           vecNormalizedSpace, vecRandomDir);
  ...
  Vector vecBodyTarget;
  pTarget->CollisionProp()->NormalizedToWorldSpace(vecRandomDir,
                                                   &vecBodyTarget);
...
Vector CNPC_MetroPolice::ComputeTightBurstTrajectory(
    const Vector &shootOrigin) {
  CBaseEntity *pEnemy = GetEnemy();
  if (!pEnemy) {
  ...
  if (flDist > MIN_TIGHT_BURST_DIST) {
    ...
    flMax = SimpleSplineRemapVal(flDist, MIN_TIGHT_BURST_DIST,
                                 MAX_TIGHT_BURST_DIST, 1.2f, 1.7f);
  }

  // Aim randomly at the player. Since body target uses the vehicle body target,
  // we instead are going to not use it
  Vector vecBodyTarget;
  pEnemy->CollisionProp()->RandomPointInBounds(
      Vector(flMin, flMin, flMin), Vector(flMax, flMax, flMax * 0.75f),
      &vecBodyTarget);
  vecBodyTarget -= shootOrigin;
  return vecBodyTarget;
}
...
Vector CNPC_MetroPolice::GetActualShootTrajectory(const Vector &shootOrigin) {
  switch (m_nBurstMode) {
    ...
    case BURST_LOCKED_ON:
      ...
      GetShootTarget()->CollisionProp()->NormalizedToWorldSpace(
          Vector(0.5f, 0.5f, 1.0f), &m_vecBurstTargetPos);
    ...
    case BURST_LOCK_ON_AFTER_HIT:
      ...
      if (GetShootTarget()->CollisionProp()->IsPointInBounds(
              m_vecBurstTargetPos)) {
        // Now raytrace against only the world + (good for cops on bridges)
        trace_t tr;
        CTraceFilterWorldOnly traceFilter;
        UTIL_TraceLine(Weapon_ShootPosition(), m_vecBurstTargetPos, MASK_SOLID,
                       &traceFilter, &tr);
        if (tr.fraction == 1.0f) {
...
void CNPC_MetroPolice::DeathSound(const CTakeDamageInfo &info) {
  ...
  m_Sentences.Speak("METROPOLICE_DIE", SENTENCE_PRIORITY_INVALID,
...
void CNPC_MetroPolice::LostEnemySound(void) {
  ...
  if (gpGlobals->curtime <= m_flNextLostSoundTime) return;

  const char *pSentence;
  if (!(CBaseEntity *)GetEnemy() ||
      gpGlobals->curtime - GetEnemyLastTimeSeen() > 10) {
  ...
  switch (m_nIdleChatterType) {
    ...
    case METROPOLICE_CHATTER_ASK_QUESTION: {
      if (m_bPlayerIsNear && !HasMemory(bits_MEMORY_PLAYER_HARASSED)) {
        if (m_Sentences.Speak("METROPOLICE_IDLE_HARASS_PLAYER",
                              SENTENCE_PRIORITY_NORMAL,
                              SENTENCE_CRITERIA_NORMAL) >= 0) {
      ...
      if (!random->RandomInt(0, 1)) break;

      int nQuestionType =
          random->RandomInt(0, METROPOLICE_CHATTER_RESPONSE_TYPE_COUNT);
      if (!IsInSquad() ||
          (nQuestionType == METROPOLICE_CHATTER_RESPONSE_TYPE_COUNT)) {
        m_Sentences.Speak(bIsCriminal ? "METROPOLICE_IDLE_CR"
                                      : "METROPOLICE_IDLE");
        break;
      }
      ...
      if (m_Sentences.Speak(pQuestion[bIsCriminal][nQuestionType]) >= 0) {
        GetSquad()->BroadcastInteraction(
            g_interactionMetrocopIdleChatter,
            (void *)(METROPOLICE_CHATTER_RESPONSE + nQuestionType), this);
    ...
    default: {
      ...
      if (m_Sentences.Speak(pResponse[bIsCriminal][nResponseType]) >= 0) {
        GetSquad()->BroadcastInteraction(
            g_interactionMetrocopIdleChatter,
            (void *)(METROPOLICE_CHATTER_ASK_QUESTION), this);
        m_nIdleChatterType = METROPOLICE_CHATTER_ASK_QUESTION;
      }
    } break;
  }
...
void CNPC_MetroPolice::PainSound(const CTakeDamageInfo &info) {
  if (gpGlobals->curtime < m_flNextPainSoundTime) return;

  // Don't make pain sounds if I'm on fire. The looping sound will take care of
  // that for us.
  if (IsOnFire()) return;
  ...
  if (healthRatio > 0.0f) {
    ...
    // This causes it to speak it no matter what; doesn't bother with setting
    // sounds.
...
int CNPC_MetroPolice::GetSoundInterests(void) {
  return SOUND_WORLD | SOUND_COMBAT | SOUND_PLAYER | SOUND_PLAYER_VEHICLE |
         SOUND_DANGER | SOUND_PHYSICS_DANGER | SOUND_BULLET_IMPACT |
         SOUND_MOVE_AWAY;
}
...
//
//-----------------------------------------------------------------------------
Class_T CNPC_MetroPolice::Classify(void) { return CLASS_METROPOLICE; }
...
bool CNPC_MetroPolice::PlayerIsCriminal(void) {
  ...
  if (GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON) return false;
...
void CNPC_MetroPolice::OnAnimEventStartDeployManhack(void) {
  ...
  pManhack->AddSpawnFlags(
      (SF_MANHACK_PACKED_UP | SF_MANHACK_CARRIED | SF_NPC_WAIT_FOR_SCRIPT));
...
void CNPC_MetroPolice::OnAnimEventShove(void) {
  CBaseEntity *pHurt = CheckTraceHullAttack(
      16, Vector(-16, -16, -16), Vector(16, 16, 16), 15, DMG_CLUB, 1.0f, false);
  ...
  if (pHurt) {
    ...
    if (pPlayer != NULL) {
      ...
      if (!(pHurt->GetFlags() & FL_ONGROUND)) forward.z = 0.0f;
...
void CNPC_MetroPolice::OnAnimEventBatonOn(void) {
  ...
  CWeaponStunStick *pStick =
      dynamic_cast<CWeaponStunStick *>(GetActiveWeapon());
...
//-----------------------------------------------------------------------------
bool CNPC_MetroPolice::HandleInteraction(int interactionType, void *data,
  ...
  if (interactionType == g_interactionMetrocopStartedStitch) {
    ...
    m_flValidStitchTime =
        gpGlobals->curtime +
        random->RandomFloat(METROPOLICE_SQUAD_STITCH_MIN_INTERVAL,
                            METROPOLICE_SQUAD_STITCH_MAX_INTERVAL);
    return true;
  }
  ...
  if (interactionType == g_interactionHitByPlayerThrownPhysObj) {
    ...
    // See if the object is the cupcop can. If so, fire the output (for x360
    // achievement)
...
Activity CNPC_MetroPolice::NPC_TranslateActivity(Activity newActivity) {
  ...
  if (newActivity == ACT_PUSH_PLAYER) {
    if (m_nNumWarnings >= METROPOLICE_MAX_WARNINGS) return ACT_MELEE_ATTACK1;
  }
  ...
  if (m_fWeaponDrawn && newActivity == ACT_IDLE &&
      (GetState() == NPC_STATE_COMBAT || BatonActive())) {
...
void CNPC_MetroPolice::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (pPlayer != NULL) {
    ...
    if (pHL2GameRules->NPC_ShouldDropHealth(pPlayer)) {
      DropItem("item_healthvial", WorldSpaceCenter() + RandomVector(-4, 4),
               RandomAngle(0, 360));
...
int CNPC_MetroPolice::SelectRangeAttackSchedule() {
  if (HasSpawnFlags(SF_METROPOLICE_ALWAYS_STITCH)) {
    ...
    if (nSched != SCHED_NONE) return nSched;
  }

  // Range attack if we're able
  if (TryToEnterPistolSlot(SQUAD_SLOT_ATTACK1) ||
  ...
  if (CanDeployManhack() &&
      OccupyStrategySlot(SQUAD_SLOT_POLICE_DEPLOY_MANHACK)) {
...
int CNPC_MetroPolice::SelectScheduleArrestEnemy() {
  ...
  if (!HasCondition(COND_SEE_ENEMY)) return SCHED_NONE;

  if (!m_fWeaponDrawn) return SCHED_METROPOLICE_DRAW_PISTOL;
...
int CNPC_MetroPolice::SelectScheduleNewEnemy() {
  ...
  if (HasBaton() && BatonActive() == false &&
      IsCurSchedule(SCHED_METROPOLICE_ACTIVATE_BATON) == false) {
    SetTarget(GetEnemy());
    SetBatonState(true);
    m_flBatonDebounceTime =
        gpGlobals->curtime + random->RandomFloat(2.5f, 4.0f);
...
// Purpose:
//-----------------------------------------------------------------------------
bool CNPC_MetroPolice::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal,
                                             float distClear,
...
int CNPC_MetroPolice::SelectScheduleNoDirectEnemy() {
  ...
  // If you can't attack, but you have a baton & there's a physics object in
  // front of you, swat it
...
int CNPC_MetroPolice::SelectCombatSchedule() {
  ...
  if (nResult != SCHED_NONE) return nResult;

  if (!m_fWeaponDrawn) {
    return SCHED_METROPOLICE_DRAW_PISTOL;
  }

  if (!HasBaton() && ((float)m_nRecentDamage / (float)GetMaxHealth()) >
                         RECENT_DAMAGE_THRESHOLD) {
    m_nRecentDamage = 0;
    m_flRecentDamageTime = 0;
    m_Sentences.Speak("METROPOLICE_COVER_HEAVY_DAMAGE",
  ...
  if (HasCondition(COND_WEAPON_SIGHT_OCCLUDED) && !HasBaton()) {
    // If they are hiding behind something that we can destroy, start shooting
    // at it.
    CBaseEntity *pBlocker = GetEnemyOccluder();
    if (pBlocker && pBlocker->GetHealth() > 0 &&
        OccupyStrategySlotRange(SQUAD_SLOT_POLICE_ATTACK_OCCLUDER1,
                                SQUAD_SLOT_POLICE_ATTACK_OCCLUDER2)) {
...
bool CNPC_MetroPolice::ShouldKnockOutTarget(CBaseEntity *pTarget) {
  if (m_PolicingBehavior.IsEnabled() &&
      m_PolicingBehavior.ShouldKnockOutTarget(pTarget))
    return true;
...
float CNPC_MetroPolice::StitchAtWeight(float flDist, float flSpeed, float flDot,
                                       float flReactionTime,
                                       const Vector &vecTargetToGun) {
  // Can't do an 'attacking' stitch if it's too soon
  if (m_flValidStitchTime > gpGlobals->curtime) return 0.0f;
  ...
  if (flDist < STITCH_MIN_DISTANCE) return 0.0f;

  // Don't do it if the player is farther but really slow
  if ((flDist < STITCH_MIN_DISTANCE_SLOW) && (flSpeed < 150.0f)) return 0.0f;

  // Does the predicted stitch position cross the plane from me to the target's
  // initial position? If so, it'll look really dumb. Disallow that.
  Vector vecGunToPredictedTarget, vecShootAtVel;
  PredictShootTargetPosition(flReactionTime, 0.0f, 0.0f,
                             &vecGunToPredictedTarget, &vecShootAtVel);
  ...
  if (DotProduct2D(vecGunToTarget, vecGunToPredictedTarget.AsVector2D()) <=
      STITCH_AT_COS_MIN_SPIN_ANGLE)
    return 0.0f;

  // If the cop is in the view cone, then up the cone in which the stitch will
  // occur
...
float CNPC_MetroPolice::StitchAcrossWeight(float flDist, float flSpeed,
                                           float flDot, float flReactionTime) {
  return 0.0f;
  ...
  // Don't do it if the player doesn't have enough time to react
  if (flDist < flSpeed * flReactionTime) return 0.0f;

  // We want to stitch across if we're within the stitch across cone
  if (flDot < STITCH_ACROSS_CONE) return 0.0f;
...
float CNPC_MetroPolice::StitchAlongSideWeight(float flDist, float flSpeed,
                                              float flDot) {
  // No squad slots? no way.
  if (IsStrategySlotRangeOccupied(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2) &&
      IsStrategySlotRangeOccupied(SQUAD_SLOT_POLICE_COVERING_FIRE1,
                                  SQUAD_SLOT_POLICE_COVERING_FIRE2))
    return 0.0f;

  if (flDist < (AIM_ALONG_SIDE_LINE_OF_DEATH_DISTANCE +
                AIM_ALONG_SIDE_STEER_DISTANCE + 100.0f))
    return 0.0f;

  if (flSpeed < STITCH_ALONG_MIN_SPEED) return 0.0f;
  ...
  if ((flDot > flMinConeAngle) || (flDot < STITCH_ALONG_MAX_CONE)) return 0.0f;
...
float CNPC_MetroPolice::StitchBehindWeight(float flDist, float flSpeed,
                                           float flDot) {
  return 0.0f;
  ...
  if (flDist < AIM_BEHIND_MINIMUM_DISTANCE) return 0.0f;

  // We want to stitch across if we're within the stitch across cone
  if (flDot > STITCH_BEHIND_MIN_CONE) return 0.0f;
...
float CNPC_MetroPolice::StitchTightWeight(float flDist, float flSpeed,
                                          const Vector &vecTargetToGun,
  ...
  if (DotProduct(vecTargetToGunDir, vecVelocityDir) > 0.95f) return 8.0f;
...
int CNPC_MetroPolice::SelectStitchSchedule() {
  ...
  if (HasSpawnFlags(SF_METROPOLICE_NO_FAR_STITCH)) {
    if (flDist > 6000.0f) return SCHED_NONE;
  }

  float flReactionTime =
      STITCH_REACTION_TIME * sk_metropolice_stitch_reaction.GetFloat();
  Vector vecVelocity;
  ...
  flWeight[0] =
      StitchAtWeight(flDist, flSpeed, flDot, flReactionTime, vecTargetToGun);
  flWeight[1] =
      flWeight[0] + StitchAcrossWeight(flDist, flSpeed, flDot, flReactionTime);
  flWeight[2] = flWeight[1] +
                StitchTightWeight(flDist, flSpeed, vecTargetToGun, vecVelocity);
  ...
  if (flWeight[STITCH_SCHEDULE_COUNT - 1] == 0.0f) return SCHED_NONE;
  ...
  for (i = 0; i < STITCH_SCHEDULE_COUNT; ++i) {
    if (flRand <= flWeight[i]) break;
  }

  // If we're basically a covering activity, take up that slot
  if (i >= 3) {
    if (OccupyStrategySlotRange(SQUAD_SLOT_POLICE_COVERING_FIRE1,
                                SQUAD_SLOT_POLICE_COVERING_FIRE2)) {
      return pSched[i];
    }
  }
  ...
  if (OccupyStrategySlotRange(SQUAD_SLOT_ATTACK1, SQUAD_SLOT_ATTACK2)) {
    if (IsInSquad() && (i < 2)) {
      GetSquad()->BroadcastInteraction(g_interactionMetrocopStartedStitch,
                                       NULL);
    }
...
int CNPC_MetroPolice::SelectMoveToLedgeSchedule() {
  ...
  if (m_flNextLedgeCheckTime > gpGlobals->curtime) return SCHED_NONE;
  ...
  if ((GetAbsOrigin().z - GetShootTarget()->GetAbsOrigin().z) >= 150.0f) {
    ...
    Vector vecDelta;
    VectorSubtract(GetShootTarget()->WorldSpaceCenter(), Weapon_ShootPosition(),
                   vecDelta);
    vecDelta.z = 0.0f;
    VectorNormalize(vecDelta);
    ...
    trace_t tr;
    CTraceFilterWorldOnly traceFilter;
    UTIL_TraceLine(Weapon_ShootPosition(), Weapon_ShootPosition() + vecDelta,
                   MASK_SOLID, &traceFilter, &tr);
...
bool CNPC_MetroPolice::IsHeavyDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_BULLET) return true;
...
// TraceAttack
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::TraceAttack(const CTakeDamageInfo &info,
...
void CNPC_MetroPolice::PlayFlinchGesture(void) {
  ...
  // To ensure old playtested difficulty stays the same, stop cops shooting for
  // a bit after gesture flinches
...
void CNPC_MetroPolice::AdministerJustice(void) {
  if (!AI_IsSinglePlayer()) return;

  // If we're allowed to chase the player, do so. Otherwise, just threaten.
  if (!IsInAScript() && (m_NPCState != NPC_STATE_SCRIPT) &&
      HasSpawnFlags(SF_METROPOLICE_ALLOWED_TO_RESPOND)) {
    ...
    for (int i = 0; i < nAIs; i++) {
      if (ppAIs[i] == this) continue;

      if (ppAIs[i]->Classify() == CLASS_METROPOLICE &&
          FClassnameIs(ppAIs[i], "npc_metropolice")) {
        ...
        if (pNPC->HasSpawnFlags(SF_METROPOLICE_ALLOWED_TO_RESPOND)) {
          ...
          if (FVisible(pNPC) && pNPC->FVisible(UTIL_PlayerByIndex(1)) &&
              UTIL_DistApprox(WorldSpaceCenter(), pNPC->WorldSpaceCenter()) <
                  512) {
...
int CNPC_MetroPolice::SelectSchedule(void) {
  if (!GetEnemy() && HasCondition(COND_IN_PVS) && AI_GetSinglePlayer() &&
      !AI_GetSinglePlayer()->IsAlive()) {
    return SCHED_PATROL_WALK;
  }

  if (HasCondition(COND_METROPOLICE_ON_FIRE)) {
    m_Sentences.Speak("METROPOLICE_ON_FIRE", SENTENCE_PRIORITY_INVALID,
                      SENTENCE_CRITERIA_ALWAYS);
  ...
  if (HasCondition(COND_METROPOLICE_PHYSOBJECT_ASSAULT)) {
    ...
    if (PlayerIsCriminal() == false) {
      m_Sentences.Speak("METROPOLICE_HIT_BY_PHYSOBJECT",
                        SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS);
      m_nNumWarnings = METROPOLICE_MAX_WARNINGS;
      AdministerJustice();
    } else if (GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON) {
      // We're not allowed to respond, but warn them
      m_Sentences.Speak("METROPOLICE_IDLE_HARASS_PLAYER",
                        SENTENCE_PRIORITY_INVALID, SENTENCE_CRITERIA_ALWAYS);
    }
  }
  ...
  if (HasBaton()) {
    ...
    if (m_bShouldActivateBaton && BatonActive() == false &&
        IsCurSchedule(SCHED_METROPOLICE_ACTIVATE_BATON) == false)
      return SCHED_METROPOLICE_ACTIVATE_BATON;

    if (m_bShouldActivateBaton == false && BatonActive() &&
        IsCurSchedule(SCHED_METROPOLICE_DEACTIVATE_BATON) == false)
  ...
  if (PlayerIsCriminal() == false) {
    if (!IsInAScript() && (HasCondition(COND_METROPOLICE_PLAYER_TOO_CLOSE) ||
                           m_bPlayerTooClose)) {
      // Don't hit the player too many times in a row, unless he's trying to
      // push a cop who hasn't moved
      ...
      // If we're not in combat, and we've got a pre-chase origin, move back to
      // it
  ...
  if (HasCondition(COND_HEAR_DANGER)) {
    ...
    if (pSound) {
      ...
      if (!HasCondition(COND_SEE_ENEMY) &&
          (pSound->m_iType &
           (SOUND_PLAYER | SOUND_PLAYER_VEHICLE | SOUND_COMBAT))) {
  ...
  if (HasCondition(COND_NO_PRIMARY_AMMO)) {
    if (bHighHealth) return SCHED_RELOAD;
  ...
  if (!BehaviorSelectSchedule()) {
    ...
    if (m_bKeepFacingPlayer && !PlayerIsCriminal()) return SCHED_TARGET_FACE;
...
// Output : int
//-----------------------------------------------------------------------------
int CNPC_MetroPolice::SelectFailSchedule(int failedSchedule, int failedTask,
  ...
  switch (scheduleType) {
    ...
    case SCHED_METROPOLICE_ADVANCE:
      if (m_NextChargeTimer.Expired() && metropolice_charge.GetBool()) {
        if (Weapon_OwnsThisType("weapon_pistol")) {
          if (GetEnemy() && GetEnemy()->GetAbsOrigin().DistToSqr(
                                GetAbsOrigin()) > 300 * 300) {
...
bool CNPC_MetroPolice::ShouldMoveAndShoot() {
  if (HasSpawnFlags(SF_METROPOLICE_ARREST_ENEMY)) return false;

  if (ShouldAttemptToStitch()) return false;
...
// Only move and shoot when attacking
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::OnEndMoveAndShoot() { VacateStrategySlot(); }
  ...
  switch (pTask->iTask) {
    ...
    case TASK_METROPOLICE_ACTIVATE_BATON: {
      ...
      if (activate) {
        ...
        m_Sentences.Speak("METROPOLICE_ACTIVATE_BATON",
        ...
        m_Sentences.Speak("METROPOLICE_DEACTIVATE_BATON",
                          SENTENCE_PRIORITY_NORMAL, SENTENCE_CRITERIA_NORMAL);
    ...
    case TASK_METROPOLICE_RELOAD_FOR_BURST: {
      if (GetActiveWeapon()) {
        ...
        // Do our fake reload to simulate a bigger clip without having to change
        // the SMG1
...
void CNPC_MetroPolice::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_METROPOLICE_BURST_ATTACK: {
      ...
      Vector vecAimPoint;
      GetMotor()->SetIdealYawToTargetAndUpdate(m_vecBurstTargetPos,
                                               AI_KEEP_YAW_SPEED);
    ...
    case TASK_METROPOLICE_ARREST_ENEMY: {
      ...
      if (pTask->iTask == TASK_METROPOLICE_LEAD_ARREST_ENEMY) {
        ...
        if (m_vSavePosition.DistToSqr(GetEnemy()->GetAbsOrigin()) >
            FLEEING_DISTANCE_SQR) {
      ...
      if (GetEnemy() && FacingIdeal()) {
        ...
        if (fabs(UTIL_AngleDiff(GetMotor()->GetIdealYaw(), flNewIdealYaw)) >=
            45.0f) {
      ...
      switch (GetTaskInterrupt()) {
        case 0: {
          ...
          if (!pSound) {
            ...
            if (GetTacticalServices()->FindLos(
                    pSound->GetSoundReactOrigin(), losTarget, flMinRange,
                    flMaxRange, 1.0, &m_vInterruptSavePosition)) {
              TaskInterrupt();
            } else {
        ...
        case 1: {
          AI_NavGoal_t goal(m_vInterruptSavePosition, ACT_RUN,
                            AIN_HULL_TOLERANCE);
...
bool CNPC_MetroPolice::CanDeployManhack(void) {
  if (HasSpawnFlags(SF_METROPOLICE_NO_MANHACK_DEPLOY)) return false;

  // Nope, already have one out.
  if (m_hManhack != NULL) return false;

  // Nope, don't have any!
  if (m_iManhacks < 1) return false;
...
void CNPC_MetroPolice::BuildScheduleTestBits(void) {
  ...
  if (!IsCurSchedule(SCHED_METROPOLICE_BURNING_RUN) &&
      !IsCurSchedule(SCHED_METROPOLICE_BURNING_STAND) && !IsMoving()) {
  ...
  } else if (HasBaton() && IsCurSchedule(SCHED_COMBAT_FACE) &&
             !m_BatonSwingTimer.Expired()) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
WeaponProficiency_t CNPC_MetroPolice::CalcWeaponProficiency(
    CBaseCombatWeapon *pWeapon) {
  if (FClassnameIs(pWeapon, "weapon_pistol")) {
...
void CNPC_MetroPolice::GatherConditions(void) {
  ...
  float distToPlayerSqr =
      (pPlayer->GetAbsOrigin() - GetAbsOrigin()).LengthSqr();
  ...
  if (pPlayer->GetGroundEntity() == this) {
    ...
    // Don't clear out the player hit count for a few seconds after we last hit
    // him This avoids states where two metropolice have the player pinned
    // between them.
...
bool CNPC_MetroPolice::HasBaton(void) {
  ...
  if (pWeapon) return FClassnameIs(pWeapon, "weapon_stunstick");
...
bool CNPC_MetroPolice::BatonActive(void) {
  ...
  if (pStick) return pStick->GetStunState();
#endif
...
bool CNPC_MetroPolice::QueryHearSound(CSound *pSound) {
  ...
  if (PlayerIsCriminal() == false) {
    ...
    if (pSound->IsSoundType(SOUND_DANGER) && pSound->m_hOwner &&
        IRelationType(pSound->m_hOwner) == D_NU)
      return false;
  }
...
//			*pEvent -
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::VPhysicsCollision(int index,
...
//-----------------------------------------------------------------------------
void CNPC_MetroPolice::PrecriminalUse(CBaseEntity *pActivator,
                                      CBaseEntity *pCaller, USE_TYPE useType,
                                      float value) {
  if (IsInAScript()) return;
  // Don't respond if I'm busy hating the player
  if (IRelationType(pActivator) == D_HT ||
      ((GetState() != NPC_STATE_ALERT) && (GetState() != NPC_STATE_IDLE)))
    return;
  if (PlayerIsCriminal()) return;
...
DEFINE_SCHEDULE(
    ...
    "ACTIVITY:ACT_METROPOLICE_DRAW_PISTOL"
...
// > ChaseEnemy
//=========================================================
DEFINE_SCHEDULE(SCHED_METROPOLICE_CHASE_ENEMY,

                "	Tasks"
                "		TASK_STOP_MOVING			"
                "	0"
                "		TASK_SET_FAIL_SCHEDULE			"
                "SCHEDULE:SCHED_METROPOLICE_ESTABLISH_LINE_OF_FIRE"
                "		TASK_SET_TOLERANCE_DISTANCE		24"
                "		TASK_GET_CHASE_PATH_TO_ENEMY	300"
                "		TASK_SPEAK_SENTENCE			"
                "	6"  // METROPOLICE_SENTENCE_MOVE_INTO_POSITION
                "		TASK_RUN_PATH				"
                "	0"
...
DEFINE_SCHEDULE(SCHED_METROPOLICE_ESTABLISH_LINE_OF_FIRE,

                "	Tasks "
                "		TASK_SET_FAIL_SCHEDULE			"
                "SCHEDULE:SCHED_FAIL_ESTABLISH_LINE_OF_FIRE"
                "		TASK_FACE_ENEMY				"
                "	0"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_METROPOLICE_DEPLOY_MANHACK,

                "	Tasks"
                "		TASK_SPEAK_SENTENCE			"
                "		5"  // METROPOLICE_SENTENCE_DEPLOY_MANHACK
                "		TASK_PLAY_SEQUENCE			"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_FACE_ENEMY						0"
    "		TASK_WAIT_FACE_ENEMY				1"  // give the
                                                                    // guy some
                                                                    // time to
                                                                    // come out
                                                                    // on his
...
DEFINE_SCHEDULE(
    ...
    "		TASK_STOP_MOVING				0"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_METROPOLICE_ADVANCE"
    //	"		TASK_SET_TOLERANCE_DISTANCE		24"
    "		TASK_STORE_LASTPOSITION			0"
    "		TASK_GET_CHASE_PATH_TO_ENEMY	300"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE		"
    "SCHEDULE:SCHED_ALERT_FACE_BESTSOUND"
    ""
    "	Interrupts"
    "")
...
//===============================================
//===============================================
DEFINE_SCHEDULE(SCHED_METROPOLICE_WARN_AND_ARREST_ENEMY,
