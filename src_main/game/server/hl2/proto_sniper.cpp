...
extern Vector PointOnLineNearestPoint(const Vector &vStartPos,
...
// Moved to HL2_SharedGameRules because these are referenced by shared AmmoDef
// functions
...
#define SF_SNIPER_VIEWCONE \
  (1 << 17)  ///< when set, sniper only sees in a small cone around the laser.
#define SF_SNIPER_NOCORPSE (1 << 18)  ///< when set, no corpse
#define SF_SNIPER_STARTDISABLED (1 << 19)
#define SF_SNIPER_FAST \
  (1 << 20)  ///< This is faster-shooting sniper. Paint time is decreased 25%.
             ///< Bullet speed increases 150%.
#define SF_SNIPER_NOSWEEP \
  (1 << 21)  ///< This sniper doesn't sweep to the target or use decoys.
...
// These numbers determine the interval between shots. They used to be
// constants, but are now keyfields. HL2 backwards compatibility was maintained
// by supplying default values in the constructor.
#if 0
...
// Target protection
#define SNIPER_PROTECTION_MINDIST \
  (1024.0 * 1024.0)  // Distance around protect target that sniper does priority
                     // modification in
#define SNIPER_PROTECTION_PRIORITYCAP \
  100.0  // Max addition to priority of an enemy right next to the protect
         // target, falls to 0 at SNIPER_PROTECTION_MINDIST.
...
class CSniperBullet : public CBaseEntity {
 public:
  ...
  bool Start(const Vector &vecOrigin, const Vector &vecTarget,
             CBaseEntity *pOwner, bool bDirectShot);
  void Stop(void);
...
class CProtoSniper : public CAI_BaseNPC {
 ...
 private:
  ...
  void GetPaintAim(const Vector &vecStart, const Vector &vecGoal,
                   float flParameter, Vector *pProgress);
 ...
 private:
  ...
  Vector m_vecPaintStart;  // used to track where a sweep starts for the purpose
                           // of interpolating.
  ...
  float m_flDangerEnemyDistance;  // Distance to the enemy nearest the protect
                                  // target
  ...
  bool m_bShootZombiesInChest;  ///< if true, do not try to shoot zombies in the
                                ///< headcrab
...
BEGIN_DATADESC(CProtoSniper)
  ...
  DEFINE_FIELD(m_fWeaponLoaded, FIELD_BOOLEAN),
      ...
      DEFINE_KEYFIELD(m_bShootZombiesInChest, FIELD_BOOLEAN,
                      "shootZombiesInChest"),

      DEFINE_KEYFIELD(m_flKeyfieldPaintTime, FIELD_FLOAT, "PaintInterval"),
      DEFINE_KEYFIELD(m_flKeyfieldPaintTimeNoise, FIELD_FLOAT,
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "SweepTargetHighestPriority",
                       InputSweepTargetHighestPriority),
      DEFINE_INPUTFUNC(FIELD_STRING, "SweepGroupRandomly",
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetPaintIntervalVariance",
...
bool CProtoSniper::QuerySeeEntity(CBaseEntity *pEntity,
  ...
  if (!FInViewCone(pEntity)) {
    // Yes, this does call FInViewCone twice a frame for all entities checked
    // for visibility, but doing this allows us to cut out a bunch of traces
    // that would be done by VerifyShot for entities that aren't even in our
    // viewcone.
    return false;
  }
...
bool CProtoSniper::FInViewCone(CBaseEntity *pEntity) {
  if (pEntity->GetFlags() & FL_CLIENT) {
    ...
    if (m_spawnflags & SF_SNIPER_VIEWCONE) {
      ...
      vecNearestPoint = PointOnLineNearestPoint(
          EyePosition(), EyePosition() + vecLOS * 8192, pPlayer->EyePosition());
...
void CProtoSniper::LaserOn(const Vector &vecTarget, const Vector &vecDeviance) {
  ...
  // so that the beam can be tapered to very thin where it emits from the
  // sniper.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CProtoSniper::GetPaintAim(const Vector &vecStart, const Vector &vecGoal,
                               float flParameter, Vector *pProgress) {
#if 0
	Vector vecDelta;
...
void CProtoSniper::PaintTarget(const Vector &vecTarget, float flPaintTime) {
  ...
  if (P > 0.25f && GetEnemy() && GetEnemy()->IsNPC() &&
      HasCondition(COND_SEE_ENEMY) && !m_bWarnedTargetEntity) {
    m_bWarnedTargetEntity = true;

    if (GetEnemy()->Classify() == CLASS_PLAYER_ALLY_VITAL &&
        GetEnemy()->MyNPCPointer()->FVisible(this)) {
      CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE,
                             GetEnemy()->EarPosition(), 16, 1.0f, this);
    }
  }
...
#define THRESHOLD 0.8f
  ...
  // mult by P
  vecCurrentDir.x +=
      flNoiseScale * (sin(3 * M_PI * gpGlobals->curtime) * 0.0006);
  vecCurrentDir.y +=
      flNoiseScale * (sin(2 * M_PI * gpGlobals->curtime + 0.5 * M_PI) * 0.0006);
  vecCurrentDir.z +=
      flNoiseScale * (sin(1.5 * M_PI * gpGlobals->curtime + M_PI) * 0.0006);
#endif

  trace_t tr;

  UTIL_TraceLine(vecStart, vecStart + vecCurrentDir * 8192, MASK_SHOT, this,
...
void CProtoSniper::Spawn(void) {
  ...
  // m_hBullet = (CSniperBullet *)Create( "sniperbullet", GetBulletOrigin(),
  // GetLocalAngles(), NULL );
...
void CProtoSniper::SetSweepTarget(const char *pszTarget) {
  ...
  // In case the sniper was sweeping a random set of targets when asked to sweep
  // a normal chain.
  ClearTargetGroup();
...
void CProtoSniper::InputSweepTargetHighestPriority(inputdata_t &inputdata) {
  ...
  if (GetCurSchedule() &&
      stricmp(GetCurSchedule()->GetName(), "SCHED_PSNIPER_RELOAD")) {
...
int CProtoSniper::IRelationPriority(CBaseEntity *pTarget) {
  ...
  if (m_hProtectTarget) {
    float flDistance =
        (pTarget->GetAbsOrigin() - m_hProtectTarget->GetAbsOrigin())
            .LengthSqr();
    if (flDistance <= SNIPER_PROTECTION_MINDIST) {
      float flBonus = (1.0 - (flDistance / SNIPER_PROTECTION_MINDIST)) *
                      SNIPER_PROTECTION_PRIORITYCAP;
      priority += flBonus;

      if (m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) {
        NDebugOverlay::Text(pTarget->GetAbsOrigin() + Vector(0, 0, 16),
                            UTIL_VarArgs("P: %d (b %f)!", priority, flBonus),
                            false, 0.1);
      }
    }
  }
...
// already fired at in this list. When they've been pushed off the list by
// others, then they are valid targets again.
//-----------------------------------------------------------------------------
...
int CProtoSniper::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (flDist > SNIPER_MAX_INFLICTOR_DIST) {
    // Sniper only takes damage from explosives that are nearby. This makes a
    // sniper susceptible to a grenade that lands in his nest, but not to a
    // large explosion that goes off elsewhere and just happens to be able to
    // trace into the sniper's nest.
    return 0;
  }
...
void CProtoSniper::Event_Killed(const CTakeDamageInfo &info) {
  if (!(m_spawnflags & SF_SNIPER_NOCORPSE)) {
    ...
    pGib = CreateRagGib(
        "models/combine_soldier.mdl", GetLocalOrigin(), GetLocalAngles(),
        (vecForward * flForce) + Vector(0, 0, 600), flFadeTime, bShouldIgnite);
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
void CProtoSniper::Event_KilledOther(CBaseEntity *pVictim,
...
int CProtoSniper::SelectSchedule(void) {
  ...
  if (HasCondition(COND_HEAR_DANGER)) {
    ...
    if (pSound && pSound->IsSoundType(SOUND_DANGER) &&
        BaseClass::FVisible(pSound->GetSoundReactOrigin())) {
      // The sniper will scream if the sound of a grenade about to detonate is
      // heard. If this COND_HEAR_DANGER is due to the sound really being
      // SOUND_DANGER_SNIPERONLY, the sniper keeps quiet, because the player's
      // grenade might miss the mark.

      // Make sure the sound is visible, otherwise the sniper will scream at a
      // grenade that probably won't harm him.
  ...
  if (HasCondition(COND_SNIPER_SWEEP_TARGET)) {
    ...
    if (m_hSweepTarget &&
            m_hSweepTarget->HasSpawnFlags(SF_SNIPERTARGET_NOINTERRUPT) ||
...
#define SEARCH_DEPTH 50
  ...
  count = UTIL_EntitiesInBox(pList, SEARCH_DEPTH, vecTarget - vecDelta,
                             vecTarget + vecDelta, 0);
  ...
  for (i = 0; i < count; i++) {
    ...
    if (FClassnameIs(pCurrent, "func_breakable") ||
        FClassnameIs(pCurrent, "prop_physics") ||
        FClassnameIs(pCurrent, "func_physbox")) {
      if (!pCurrent->VPhysicsGetObject()) continue;
      ...
      if (pCurrent->VPhysicsGetObject()->GetGameFlags() &
  ...
  for (i = 0; i < 4; i++) {
    ...
    pProspect->CollisionProp()->RandomPointInBounds(
        Vector(.1, .1, .1), Vector(.6, .6, .6), &vecDecoyTarget);

    // When trying to trace to an object using its absmin + some fraction of its
    // size, it's best to lengthen the trace a little beyond the object's
    // bounding box in case it's a more complex object, or not axially aligned.
    vecDirToDecoy = vecDecoyTarget - vecBulletOrigin;
    VectorNormalize(vecDirToDecoy);

    // Right now, tracing with MASK_BLOCKLOS and checking the fraction as well
    // as the object the trace has hit makes it possible for the decoy behavior
    ...
    if (tr.m_pEnt == pProspect || tr.fraction == 1.0) {
      ...
      Vector vecNoise;
      pProspect->CollisionProp()->RandomPointInBounds(
          Vector(0.25, 0.25, 0.25), Vector(0.75, 0.75, 0.75), &vecNoise);
...
bool CProtoSniper::ShouldSnapShot(void) {
  if (GetEnemy()->IsPlayer()) {
    if (GetEnemy()->GetSmoothedVelocity().Length() >=
        SNIPER_SNAP_SHOT_VELOCITY) {
      return true;
    } else {
      return false;
    }
...
bool CProtoSniper::VerifyShot(CBaseEntity *pTarget) {
  ...
  UTIL_TraceLine(GetBulletOrigin(), vecTarget, MASK_SHOT, pTarget,
                 COLLISION_GROUP_NONE, &tr);

  if (tr.fraction != 1.0) {
    if (pTarget->IsPlayer()) {
      // if the target is the player, do another trace to see if we can shoot
      // his eyeposition. This should help improve sniper responsiveness in
      // cases where the player is hiding his chest from the sniper with his
      // head in full view.
      UTIL_TraceLine(GetBulletOrigin(), pTarget->EyePosition(), MASK_SHOT,
                     pTarget, COLLISION_GROUP_NONE, &tr);
...
int CProtoSniper::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  if (fFrustration >= 2 && !m_fIsPatient) {
    if (!(m_spawnflags & SF_SNIPER_NOSWEEP) && !m_hDecoyObject &&
        FindDecoyObject()) {
...
bool CProtoSniper::FireBullet(const Vector &vecTarget, bool bDirectShot) {
  ...
  pBullet = (CSniperBullet *)Create("sniperbullet", GetBulletOrigin(),
                                    GetLocalAngles(), NULL);
...
void CProtoSniper::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_SNIPER_PAINT_ENEMY:
      ...
      // If the sniper has a sweep target, clear it, unless it's flagged to
      // resume
      ...
      if (m_spawnflags & SF_SNIPER_VIEWCONE) {
        ...
        if (GetEnemy()->IsPlayer()) {
          ...
          if (gpGlobals->curtime - m_flTimeLastAttackedPlayer <=
              SNIPER_FASTER_ATTACK_PERIOD) {
          ...
          m_flPaintTime =
              m_flKeyfieldPaintTimeNoise > 0
                  ? m_flKeyfieldPaintTime +
                        random->RandomFloat(0, m_flKeyfieldPaintTimeNoise)
                  : m_flKeyfieldPaintTime;
    ...
    case TASK_SNIPER_PAINT_FRUSTRATED:
      m_flPaintTime = SNIPER_PAINT_FRUSTRATED_TIME +
                      random->RandomFloat(0, SNIPER_PAINT_FRUSTRATED_TIME);
      SetWait(m_flPaintTime);
      if (FindFrustratedShot(pTask->flTaskData)) {
        LaserOff();
        LaserOn(m_vecFrustratedTarget, vec3_origin);
    ...
    case TASK_SNIPER_PAINT_SWEEP_TARGET:
      ...
      if (IsWaitFinished()) {
        ...
        if (pNext || IsSweepingRandomly()) {
          ...
          // This is done regardless of whether we just fired at the current
          // target.
          ...
          if (IsSweepingRandomly()) {
            ...
            // Pick another target in the group. Don't shoot at the one we just
            // shot at.
            if (m_iNumGroupTargets > 1) {
              do {
                m_hSweepTarget = m_pGroupTarget[random->RandomInt(
                    0, m_iNumGroupTargets - 1)];
...
void CProtoSniper::PrescheduleThink(void) {
  ...
  if (m_hSweepTarget != NULL) {
    if (m_bSweepHighestPriority || (!HasCondition(COND_CAN_RANGE_ATTACK1) &&
                                    !HasCondition(COND_SNIPER_NO_SHOT))) {
...
Vector CProtoSniper::DesiredBodyTarget(CBaseEntity *pTarget) {
  ...
  if (pTarget->GetFlags() & FL_CLIENT) {
    ...
    if (pTarget->Classify() == CLASS_HEADCRAB) {
      ...
      // Shoot about a few inches above the origin. This makes it easy to hit
      // antlions even if they are on their backs.
      vecTarget = pTarget->GetAbsOrigin();
      vecTarget.z += 18.0f;
  ...
  */
  if (sniperLines.GetBool()) {
    Msg("Sniper %s is targeting %s\n", GetDebugName(),
        pTarget ? pTarget->GetDebugName() : "nobody");
  }
  ...
  if (pTarget->IsPlayer() || pTarget->Classify() == CLASS_MISSILE) {
    ...
    if (pTarget->MyNPCPointer() &&
        pTarget->MyNPCPointer()->GetNavType() == NAV_FLY) {
      // Take a flying monster's velocity directly.
      vecVelocity = pTarget->GetAbsVelocity();
    } else {
      // Have to build a velocity vector using the character's current
      // groundspeed.
  ...
  // if the adjusted shot falls well short of the target, take the straight
  // shot. it's not very interesting for the bullet to hit something far away
  // from the target. (for instance, if a sign or ledge or something is between
  // the player and the sniper, and the sniper would hit this object if he tries
  // to lead the player)
  ...
  /*
          ...
          /////////////////////////////////////////////
          // the shot taken should hit within 10% of the sniper's distance to
     projected target.
          // else, shoot straight. (there's some object in the way of the
     adjusted shot)
          /////////////////////////////////////////////
...
CBaseEntity *CProtoSniper::PickDeadPlayerTarget() {
  ...
  int iNumEntities = UTIL_EntitiesInSphere(
      pEntities, iSearchSize, AI_GetSinglePlayer()->GetAbsOrigin(), 180.0f, 0);

  // Not very robust, but doesn't need to be. Randomly select a nearby object in
  // the list that isn't an NPC.
  if (iNumEntities > 0) {
    int i;
    ...
    for (i = 0; i < 10; i++) {
      CBaseEntity *pCandidate =
          pEntities[random->RandomInt(0, iNumEntities - 1)];
...
#define MAX_TRIES 15
  for (int i = 0; i < MAX_TRIES; i++) {
    ...
    if (GetEnemy()->FVisible(vecSpot) || i == MAX_TRIES - 1) {
      trace_t tr;
      AI_TraceLine(vecSrc, vecSrc + vecDir * 8192, MASK_SHOT, this,
                   COLLISION_GROUP_NONE, &tr);
...
bool CProtoSniper::FVisible(CBaseEntity *pEntity, int traceMask,
  ...
  if (fabs(GetAbsOrigin().z - pEntity->WorldSpaceCenter().z) <= 120.f) {
    ...
    // Otherwise, look at a spot below his eyes. This allows the player to back
    // away from his cover a bit and have a peek at the sniper without being
    // detected.
  ...
  UTIL_TraceLine(EyePosition(), pEntity->EyePosition() + vecEye, MASK_BLOCKLOS,
  ...
  if (!fCheckFailed) {
    vecEye = -vecRight * SNIPER_EYE_DIST - vecVerticalOffset;
    UTIL_TraceLine(EyePosition(), pEntity->EyePosition() + vecEye,
                   MASK_BLOCKLOS, this, COLLISION_GROUP_NONE, &tr);
  ...
  if ((pPlayer->GetFlags() & FL_DUCKING) &&
      pPlayer->MuzzleFlashTime() > gpGlobals->curtime) {
    vecEye = pPlayer->EyePosition() + Vector(0, 0, 32);
    UTIL_TraceLine(EyePosition(), vecEye, MASK_BLOCKLOS, this,
                   COLLISION_GROUP_NONE, &tr);
...
int CProtoSniper::DrawDebugTextOverlays() {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Sweep group (count): %s (%d)",
               pTarget != NULL ? STRING(pTarget->m_iszGroupName) : "<None>",
               m_iNumGroupTargets);
    ...
    for (int i = 0; i < m_iNumGroupTargets; i++) {
      if (m_pGroupTarget[i] != NULL) {
        NDebugOverlay::VertArrow(EyePosition(),
                                 m_pGroupTarget[i]->GetAbsOrigin(), 8, 0, 255,
                                 0, 0, true, 0);
      }
    }
  }
...
void CProtoSniper::NotifyShotMissedTarget() {
  ...
  // miss zombie and zombines over and over because of the large amount of head
  // movement in these NPCs' walk and run animations.
}
...
void CSniperBullet::BulletThink(void) {
  ...
  if (gpGlobals->curtime >= m_SoundTime) {
    ...
    if (GetOwnerEntity()) {
      ...
      if (pSniper && pSniper->GetEnemy()) {
        ...
        if (pEnemyNPC && GetOwnerEntity() &&
            pEnemyNPC->FVisible(GetOwnerEntity()->WorldSpaceCenter())) {
          CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_FROM_SNIPER,
                                 pSniper->GetEnemy()->EarPosition(), 16, 1.0f,
                                 GetOwnerEntity());
        }
      }
    }
  ...
  if (tr.fraction != 1.0) {
    ...
    GetOwnerEntity()->FireBullets(1, vecStart, m_vecDir, vec3_origin, flDist,
                                  m_AmmoType, 0);
    m_iImpacts++;
...
#endif  // HL2_EPISODIC
    {
      // Bullet stops when it hits an NPC, or when it has penetrated enough
      // times.

      if (tr.m_pEnt && tr.m_pEnt->VPhysicsGetObject()) {
        if (tr.m_pEnt->VPhysicsGetObject()->GetGameFlags() &
            FVPHYSICS_PLAYER_HELD) {
...
#define NUM_STEPS 6
      ...
      for (int i = 0; i < NUM_STEPS; i++) {
        ...
        if (UTIL_PointContents(vecCursor) != CONTENTS_SOLID) {
          ...
          // Fire another tracer.
          AI_TraceLine(vecCursor, vecCursor + m_vecDir * 8192, MASK_SHOT, this,
                       COLLISION_GROUP_NONE, &tr);
          UTIL_Tracer(vecCursor, tr.endpos, 0, TRACER_DONT_USE_ATTACHMENT,
                      m_Speed, true, "StriderTracer");
          return;
        }
      }

      // Bullet also stops when it fails to exit material after penetrating this
      // far.
      // Msg("#\n");
      if (m_bDirectShot) {
...
bool CSniperBullet::Start(const Vector &vecOrigin, const Vector &vecTarget,
  ...
  if (m_AmmoType == -1) {
    ...
    // This is the bullet that is used for all subsequent FireBullets() calls
    // after the first call penetrates a surface and keeps going.
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + m_vecDir * 8192,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
  UTIL_Tracer(vecOrigin, tr.endpos, 0, TRACER_DONT_USE_ATTACHMENT, m_Speed,
              true, "StriderTracer");
  ...
  /*
          ...
          // Try to find all of the things the bullet can go through along the
  way.
          //-------------------------------
          //-------------------------------
          m_vecDir = vecTarget - vecOrigin;
  ...
  simulation. float flElapsedTime = 0;
          ...
          {
                  // Trace to the target.
                  UTIL_TraceLine( GetAbsOrigin(), vecTarget, MASK_SHOT, this,
  COLLISION_GROUP_NONE, &tr );

                  flShotDist = (tr.endpos - GetAbsOrigin()).Length();

                  // Record the two endpoints of the segment and the time at
  which this bullet hits,
                  // and the time at which it's supposed to hit its mark.
                  m_ImpactTime[ i ] = flElapsedTime + ( flShotDist /
  GetBulletSpeed() ); m_vecStart[ i ] = tr.startpos; m_vecEnd[ i ] = tr.endpos;

                  // The elapsed time is now pushed forward by how long it takes
  the bullet
                  ...
                  if( !pEnt
  || pEnt->MyNPCPointer()
  || UTIL_DistApprox2D( tr.endpos, vecTarget ) <= 4		|| FClassnameIs(
  pEnt, "prop_physics" ) )
                  {
                          // If we're close to the target, assume the shot is
  going to hit
                  ...
                  // We're going to try to penetrate whatever the bullet has
  hit.

                  // Push through the object by the penetration distance, then
  trace back. Vector vecCursor;

                  vecCursor = tr.endpos;
                  vecCursor += m_vecDir * PENETRATION_THICKNESS;

                  UTIL_TraceLine( vecCursor, vecCursor + m_vecDir * -2,
  MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );

  #if 1
                  if( tr.startsolid )
                  {
                          // The cursor is inside the solid. Solid is too thick
