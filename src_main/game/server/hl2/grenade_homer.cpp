...
extern short g_sModelIndexFireball;  // (in combatweapon.cpp) holds the index
                                     // for the smoke cloud
...
// Output  :
//------------------------------------------------------------------------------
CGrenadeHomer *CGrenadeHomer::CreateGrenadeHomer(string_t sModelName,
                                                 string_t sFlySound,
                                                 const Vector &vecOrigin,
                                                 const QAngle &vecAngles,
                                                 edict_t *pentOwner) {
  CGrenadeHomer *pGrenade =
      (CGrenadeHomer *)CreateEntityByName("grenade_homer");
  if (!pGrenade) {
...
// Output  :
//------------------------------------------------------------------------------
void CGrenadeHomer::SetHoming(float flStrength, float flDelay, float flRampUp,
                              float flDuration, float flRampDown) {
...
// Output  :
//------------------------------------------------------------------------------
void CGrenadeHomer::Launch(CBaseEntity *pOwner, CBaseEntity *pTarget,
                           const Vector &vInitVelocity, float flHomingSpeed,
                           float flGravity, int nRocketTrailType) {
  ...
  // Smoke trail.
  // -------------
  if ((m_nRocketTrailType == HOMER_SMOKE_TRAIL_ON) ||
  ...
  if (pTarget) {
    ...
    float flTime = std::max(0.5f, flDist / GetAbsVelocity().Length());

    CSoundEnt::InsertSound(SOUND_DANGER, m_hTarget->GetAbsOrigin(), 300, flTime,
                           pOwner);
  }
}
...
void CGrenadeHomer::GrenadeHomerTouch(CBaseEntity *pOther) {
  ...
  // ----------------------------------
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + GetAbsVelocity(),
...
void CGrenadeHomer::Detonate(void) {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + 60 * vecForward,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
  ...
  RadiusDamage(CTakeDamageInfo(this, GetOwnerEntity(), m_flDamage, DMG_BLAST),
...
void CGrenadeHomer::AimThink(void) {
  ...
  // has been reached
  if (m_flDetonateTime != 0 && gpGlobals->curtime > m_flDetonateTime) {
    Detonate();
    return;
  }
  ...
  if (m_hTarget != NULL) {
    ...
    if ((vTargetPos - GetAbsOrigin()).Length() > 200) {
      ...
      trace_t tr;
      UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vTravelDir, MASK_SHOT,
                     m_hTarget, COLLISION_GROUP_NONE, &tr);
      if (tr.fraction != 1.0) {
        // Head off in normal
        ...
        VectorNormalize(vTargetDir);
        // DEBUG TOOL
        // NDebugOverlay::Line(GetOrigin(), GetOrigin()+vTravelDir, 255,0,0,
        // true, 20); NDebugOverlay::Line(GetOrigin(),
        // GetOrigin()+(12*tr.plane.normal), 0,0,255, true, 20);
        // NDebugOverlay::Line(GetOrigin(), GetOrigin()+(vTargetDir), 0,255,0,
        // true, 20);
      }
    }
    ...
    float flHomingRampDownStartTime =
        flHomingSustainStartTime + m_flHomingDuration;
    float flHomingEndHomingTime =
    ...
    // -----------
      ...
      flCurHomingStrength =
          m_flHomingStrength * (1 - (flAge / m_flHomingRampDown));
      flTargetSpeed = m_flHomingSpeed;
    }
    // ---------------
    //  Set Homing
    ...
    if (flCurHomingStrength > 0) {
      ...
      flTargetSpeed = std::max(flTargetSpeed, flCurSpeed);

      // Add in homing direction
      Vector vecNewVelocity = GetAbsVelocity();
      float flTimeToUse = gpGlobals->frametime;
      while (flTimeToUse > 0) {
        vecNewVelocity = (flCurHomingStrength * vTargetDir) +
                         ((1 - flCurHomingStrength) * vCurDir);
        flTimeToUse = -0.1;
      }
  ...
  if (m_flSpinMagnitude > 0) {
    vecImpulse.x += m_flSpinMagnitude *
                    sin(m_flSpinSpeed * gpGlobals->curtime + m_flSpinOffset);
    vecImpulse.y += m_flSpinMagnitude *
                    cos(m_flSpinSpeed * gpGlobals->curtime + m_flSpinOffset);
    vecImpulse.z -= m_flSpinMagnitude *
                    cos(m_flSpinSpeed * gpGlobals->curtime + m_flSpinOffset);
  }
