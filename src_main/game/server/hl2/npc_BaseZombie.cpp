...
//
// In Torso state, the zombie has been blasted or cut in half, and the Torso
// will drag itself along the ground with its arms. It will try to claw the
// player.
//
...
//
// After taking damage, ignore further damage for n seconds. This keeps the
// zombie from being interrupted while.
//
...
BEGIN_DATADESC(CNPC_BaseZombie)
  ...
  DEFINE_SOUNDPATCH(m_pMoanSound), DEFINE_FIELD(m_fIsTorso, FIELD_BOOLEAN),
...
bool CNPC_BaseZombie::FindNearestPhysicsObject(int iMaxMass) {
  ...
  class CZombieSwatEntitiesEnum : public CFlaggedEntitiesEnum {
   public:
    CZombieSwatEntitiesEnum(CBaseEntity **pList, int listMax, int iMaxMass)
        : CFlaggedEntitiesEnum(pList, listMax, 0), m_iMaxMass(iMaxMass) {}
  ...
  CZombieSwatEntitiesEnum swatEnum(pList, ZOMBIE_PHYSICS_SEARCH_DEPTH,
                                   iMaxMass);

  int count = UTIL_EntitiesInBox(GetAbsOrigin() - vecDelta,
                                 GetAbsOrigin() + vecDelta, &swatEnum);

  // magically know where they are
  Vector vecZombieKnees;
  CollisionProp()->NormalizedToWorldSpace(Vector(0.5f, 0.5f, 0.25f),
                                          &vecZombieKnees);
  ...
  for (i = 0; i < count; i++) {
    ...
    physcollision->CollideGetAABB(&objMins, &objMaxs, pCollide->solids[0],
                                  pList[i]->GetAbsOrigin(),
                                  pList[i]->GetAbsAngles());
    ...
    if (hl2_episodic.GetBool()) {
      // Skip things that the enemy can't see. Do we want this as a general
      // thing? The case for this feature is that zombies who are pursuing the
      // player will stop along the way to swat objects at the player who is
      // around the corner or otherwise not in a place that the object has a
      // hope of hitting. This diversion makes the zombies very late (in a
      // random fashion) getting where they are going. (sjb 1/2/06)
      if (!GetEnemy()->FVisible(pList[i])) continue;
    }

    // Make this the last check, since it makes a string.
    // Don't swat server ragdolls!
    if (FClassnameIs(pList[i], "physics_prop_ragdoll")) continue;

    if (FClassnameIs(pList[i], "prop_ragdoll")) continue;
...
Class_T CNPC_BaseZombie::Classify(void) {
  if (IsSlumped()) return CLASS_NONE;
...
//-----------------------------------------------------------------------------
// Purpose: Returns the maximum yaw speed based on the monster's current
// activity.
//-----------------------------------------------------------------------------
...
// Output :
//-----------------------------------------------------------------------------
bool CNPC_BaseZombie::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                         float flInterval) {
  ...
  if (GetEnemy()) {
    float flEDist =
        UTIL_DistApprox2D(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter());

    if (flEDist < 256.0) {
      float flEYaw =
          UTIL_VecToYaw(GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter());

      if (flEDist < 128.0) {
        idealYaw = flEYaw;
      } else {
        idealYaw = flMoveYaw +
                   UTIL_AngleDiff(flEYaw, flMoveYaw) * (2 - flEDist / 128.0);
      }
  ...
  float flDiff =
      UTIL_AngleDiff(flMoveYaw, GetLocalAngles().y + fSequenceMoveYaw);
...
int CNPC_BaseZombie::MeleeAttack1Conditions(float flDot, float flDist) {
    ...
    if (GetEnemy() != NULL) {
      ...
      if (GetEnemy()->IsPlayer()) {
        ...
        if (pObject) {
          ...
          if (flDist <= GetClawAttackRange()) return COND_CAN_MELEE_ATTACK1;
        }
      }
#endif
    }
  ...
  AI_TraceHull(WorldSpaceCenter(),
               WorldSpaceCenter() + forward * GetClawAttackRange(), vecMins,
               vecMaxs, MASK_NPCSOLID, &traceFilter, &tr);
  ...
  if (tr.fraction == 1.0 || !tr.m_pEnt) {
    ...
    // This attack would miss completely. Trick the zombie into moving around
    // some more.
    return COND_TOO_FAR_TO_ATTACK;
  }
  ...
  if (tr.m_pEnt == GetEnemy() || tr.m_pEnt->IsNPC() ||
    ...
    // -Also let him swipe at NPC's that happen to be between the zombie and the
    // enemy.
    //  This makes mobs of zombies seem more rowdy since it doesn't leave guys
    //  in the back row standing around.
    // -Also let him swipe at things that takedamage, under the assumptions that
    // they can be broken.
    return COND_CAN_MELEE_ATTACK1;
  }
  ...
  if (GetEnemy() && GetEnemy()->MyCombatCharacterPointer() &&
      tr.m_pEnt ==
          static_cast<CBaseCombatCharacter *>(GetEnemy())->GetVehicleEntity()) {
  ...
  if (tr.m_pEnt->IsBSPModel()) {
    // The trace hit something solid, but it's not the enemy. If this item is
    // closer to the zombie than the enemy is, treat this as an obstruction.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define ZOMBIE_BUCKSHOT_TRIPLE_DAMAGE_DIST \
  96.0f  // Triple damage from buckshot at 8 feet (headshot only)
float CNPC_BaseZombie::GetHitgroupDamageMultiplier(
    int iHitGroup, const CTakeDamageInfo &info) {
  switch (iHitGroup) {
    case HITGROUP_HEAD: {
      if (info.GetDamageType() & DMG_BUCKSHOT) {
        ...
        if (info.GetAttacker()) {
          flDist =
              (GetAbsOrigin() - info.GetAttacker()->GetAbsOrigin()).Length();
        }
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::TraceAttack(const CTakeDamageInfo &info,
                                  const Vector &vecDir, trace_t *ptr) {
  CTakeDamageInfo infoCopy = info;

  // Keep track of headshots so we can determine whether to pop off our
  // headcrab.
  ...
  if (infoCopy.GetDamageType() & DMG_BUCKSHOT) {
    // Zombie gets across-the-board damage reduction for buckshot. This
    // compensates for the recent changes which make the shotgun much more
    // powerful, and returns the zombies to a level that has been playtested
    // extensively.(sjb) This normalizes the buckshot damage to what it used to
    // be on normal (5 dmg per pellet. Now it's 8 dmg per pellet).
...
//-----------------------------------------------------------------------------
bool CNPC_BaseZombie::ShouldBecomeTorso(const CTakeDamageInfo &info,
...
//-----------------------------------------------------------------------------
// Purpose: A zombie has taken damage. Determine whether he release his
// headcrab. Output : YES, IMMEDIATE, or SCHEDULED (see HeadcrabRelease_t)
//-----------------------------------------------------------------------------
HeadcrabRelease_t CNPC_BaseZombie::ShouldReleaseHeadcrab(
    const CTakeDamageInfo &info, float flDamageThreshold) {
  if (m_iHealth <= 0) {
    if (info.GetDamageType() & DMG_REMOVENORAGDOLL) return RELEASE_NO;
...
int CNPC_BaseZombie::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (inputInfo.GetDamageType() & DMG_BURN) {
    // If a zombie is on fire it only takes damage from the fire that's attached
    // to it. (DMG_DIRECT) This is to stop zombies from burning to death 10x
    // faster when they're standing around 10 fire entities.
  ...
  // Take some percentage of damage from bullets (unless hit in the crab).
  // Always take full buckshot & sniper damage
  if (!m_bHeadShot && (info.GetDamageType() & DMG_BULLET) &&
      !(info.GetDamageType() & (DMG_BUCKSHOT | DMG_SNIPER))) {
  ...
  if (!m_fIsTorso && (bChopped || bSquashed) && !bKilledByVehicle &&
    ...
    switch (release) {
      ...
      case RELEASE_RAGDOLL:
        ...
        ReleaseHeadcrab(EyePosition(), inputInfo.GetDamageForce() * 0.25, true,
                        false, true);
        break;
    ...
    if (ShouldBecomeTorso(info, flDamageThreshold)) {
      bool bHitByCombineCannon = (inputInfo.GetAmmoType() ==
                                  GetAmmoDef()->Index("CombineHeavyCannon"));
  ...
  // IMPORTANT: always clear the headshot flag after applying damage. No early
  // outs!
  m_bHeadShot = false;
...
bool CNPC_BaseZombie::CanPlayMoanSound() {
  if (HasSpawnFlags(SF_NPC_GAG)) return false;

  // Burning zombies play their moan loop at full volume for as long as they're
  // burning. Don't let a moan envelope play cause it will turn the volume down
  // when done.
  ...
  // This serves to limit the number of zombies that can moan at one time when
  // there are a lot.
  if (random->RandomInt(1, zombie_moanfreq.GetInt() * (s_iAngryZombies / 2)) ==
      1) {
    return true;
  }
...
void CNPC_BaseZombie::MoanSound(envelopePoint_t *pEnvelope, int iEnvelopeSize) {
  ...
  if (!m_pMoanSound) {
    ...
    m_flMoanPitch =
        random->RandomInt(zombie_basemin.GetInt(), zombie_basemax.GetInt());

    // m_pMoanSound = ENVELOPE_CONTROLLER.SoundCreate( entindex(), CHAN_STATIC,
    // pszSound, ATTN_NORM );
    CPASAttenuationFilter filter(this);
    m_pMoanSound = ENVELOPE_CONTROLLER.SoundCreate(
        filter, entindex(), CHAN_STATIC, pszSound, ATTN_NORM);
  ...
  float duration = ENVELOPE_CONTROLLER.SoundPlayEnvelope(
      m_pMoanSound, SOUNDCTRL_CHANGE_VOLUME, pEnvelope, iEnvelopeSize);

  float flPitch = random->RandomInt(m_flMoanPitch + zombie_changemin.GetInt(),
                                    m_flMoanPitch + zombie_changemax.GetInt());
...
void CNPC_BaseZombie::DieChopped(const CTakeDamageInfo &info) {
  ...
  CBaseEntity *pLegGib =
      CreateRagGib(GetLegsModel(), GetAbsOrigin(), GetAbsAngles(), vecLegsForce,
                   flFadeTime, ShouldIgniteZombieGib());
  if (pLegGib) {
  ...
  TorsoAngles.x -= 90.0f;
  CBaseEntity *pTorsoGib = CreateRagGib(
      GetTorsoModel(), GetAbsOrigin() + Vector(0, 0, 64), TorsoAngles,
      forceVector, flFadeTime, ShouldIgniteZombieGib());
  if (pTorsoGib) {
...
bool CNPC_BaseZombie::ShouldIgnite(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_BURN) {
    //
    // If we take more than ten percent of our health in burn damage within a
    // five second interval, we should catch on fire.
    //
...
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::Ignite(float flFlameLifetime, bool bNPCOnly, float flSize,
                             bool bCalledByLevelDesigner) {
  BaseClass::Ignite(flFlameLifetime, bNPCOnly, flSize, bCalledByLevelDesigner);

#ifdef HL2_EPISODIC
  if (HL2GameRules()->IsAlyxInDarknessMode() == true &&
      GetEffectEntity() != NULL) {
  ...
  if (!m_ActBusyBehavior.IsActive()) {
    ...
    if (HaveSequenceForActivity(burningActivity)) {
      // Make sure we have a sequence for this activity (torsos don't have any,
      // for instance) to prevent the baseNPC & baseAnimating code from throwing
      // red level errors.
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_BaseZombie::ClawAttack(float flDist, int iDamage,
                                         QAngle &qaViewPunch,
  ...
  if (GetEnemy()) {
    ...
    // CheckTraceHullAttack() can damage player in vehicle as side effect of
    // melee attack damaging physics objects, which the car forwards to the
    // player need to detect this to get correct damage effects
    CBaseCombatCharacter *pCCEnemy =
        (GetEnemy() != NULL) ? GetEnemy()->MyCombatCharacterPointer() : NULL;
    CBaseEntity *pVehicleEntity;
    if (pCCEnemy != NULL &&
        (pVehicleEntity = pCCEnemy->GetVehicleEntity()) != NULL) {
      if (pVehicleEntity->GetServerVehicle() &&
          dynamic_cast<CPropVehicleDriveable *>(pVehicleEntity)) {
        pDriver =
            static_cast<CPropVehicleDriveable *>(pVehicleEntity)->GetDriver();
  ...
  if (GetEnemy() && GetEnemy()->Classify() == CLASS_BULLSEYE) {
    ...
    pHurt = GetEnemy();
    CTakeDamageInfo info(this, this, vec3_origin, GetAbsOrigin(), iDamage,
                         DMG_SLASH);
  ...
  if (!pHurt && m_hPhysicsEnt != NULL &&
      IsCurSchedule(SCHED_ZOMBIE_ATTACKITEM)) {
    pHurt = m_hPhysicsEnt;
    ...
    CTakeDamageInfo info(this, this, vForce, GetAbsOrigin(), iDamage,
                         DMG_SLASH);
    pHurt->TakeDamage(info);
...
//-----------------------------------------------------------------------------
// Purpose: The zombie is frustrated and pounding walls/doors. Make an
// appropriate noise Input  :
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::PoundSound() {
  ...
  AI_TraceLine(EyePosition(), EyePosition() + forward * 128, MASK_SOLID, this,
               COLLISION_GROUP_NONE, &tr);
...
void CNPC_BaseZombie::HandleAnimEvent(animevent_t *pEvent) {
  if (pEvent->event == AE_NPC_ATTACK_BROADCAST) {
    if (GetEnemy() && GetEnemy()->IsNPC()) {
      if (HasCondition(COND_CAN_MELEE_ATTACK1)) {
        // This animation is sometimes played by code that doesn't intend to
        // attack the enemy (For instance, code that makes a zombie take a
        // frustrated swipe at an obstacle). Try not to trigger a reaction from
        // our enemy unless we're really attacking.
        GetEnemy()->MyNPCPointer()->DispatchInteraction(
            g_interactionZombieMeleeWarning, NULL, this);
      }
    }
    return;
  }
  ...
  if (pEvent->event == AE_ZOMBIE_GET_UP) {
    ...
    if (!IsOnFire()) {
      // If you let this code run while a zombie is burning, it will stop
      // wailing.
  ...
  if (pEvent->event == AE_ZOMBIE_SWATITEM) {
    ...
    if (pEnemy) {
      ...
      PhysicsImpactSound(pEnemy, pPhysObj, CHAN_BODY,
                         pPhysObj->GetMaterialIndex(),
                         physprops->GetSurfaceIndex("flesh"), 0.5, 800);

      Vector physicsCenter = pPhysicsEntity->WorldSpaceCenter();
      v = pEnemy->WorldSpaceCenter() - physicsCenter;
      VectorNormalize(v);

      // Send the object at 800 in/sec toward the enemy.  Add 200 in/sec up
      // velocity to keep it in the air for a second or so.
      v = v * 800;
      v.z += 200;

      // add some spin so the object doesn't appear to just fly in a straight
      // line Also this spin will move the object slightly as it will press on
      // whatever the object is resting on.
      AngularImpulse angVelocity(random->RandomFloat(-180, 180), 20,
                                 random->RandomFloat(-360, 360));
  ...
  if (pEvent->event == AE_ZOMBIE_ATTACK_RIGHT) {
    ...
    ClawAttack(GetClawAttackRange(), sk_zombie_dmg_one_slash.GetFloat(),
               QAngle(-15, -20, -10), right + forward, ZOMBIE_BLOOD_RIGHT_HAND);
    return;
  }
  ...
  if (pEvent->event == AE_ZOMBIE_ATTACK_LEFT) {
    ...
    ClawAttack(GetClawAttackRange(), sk_zombie_dmg_one_slash.GetFloat(),
               QAngle(-15, 20, -10), right + forward, ZOMBIE_BLOOD_LEFT_HAND);
    return;
  }
  ...
  if (pEvent->event == AE_ZOMBIE_POPHEADCRAB) {
    if (GetInteractionPartner() == NULL) return;
    ...
    if (boneIndex == -1) {
      Warning(
          "AE_ZOMBIE_POPHEADCRAB event using invalid bone name! Usage: event "
          "AE_ZOMBIE_POPHEADCRAB \"<BoneName> <Speed>\" \n");
      return;
    }
    ...
    if (!token) {
      Warning(
          "AE_ZOMBIE_POPHEADCRAB event format missing velocity parameter! "
          "Usage: event AE_ZOMBIE_POPHEADCRAB \"<BoneName> <Speed>\" \n");
      return;
    }
    ...
    GetInteractionPartner()->GetBonePosition(boneIndex, vecBonePosition,
                                             angles);
    ...
    CTakeDamageInfo dmgInfo(this, GetInteractionPartner(), m_iHealth,
                            DMG_DIRECT);
...
int CNPC_BaseZombie::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    case SCHED_CHASE_ENEMY:
      if (HasCondition(COND_ZOMBIE_LOCAL_MELEE_OBSTRUCTION) &&
          !HasCondition(COND_TASK_FAILED) &&
          IsCurSchedule(SCHED_ZOMBIE_CHASE_ENEMY, false)) {
    ...
    case SCHED_ZOMBIE_SWATITEM:
      // If the object is far away, move and swat it. If it's close, just swat
      // it.
...
void CNPC_BaseZombie::OnScheduleChange(void) {
  //
  // If we took damage and changed schedules, ignore further damage for a few
  // seconds.
  //
...
//---------------------------------------------------------
//---------------------------------------------------------
int CNPC_BaseZombie::SelectFailSchedule(int failedSchedule, int failedTask,
  ...
  switch (m_NPCState) {
    case NPC_STATE_COMBAT:
      ...
      if (HasCondition(COND_LOST_ENEMY) ||
          (HasCondition(COND_ENEMY_UNREACHABLE) && MustCloseToAttack())) {
...
bool CNPC_BaseZombie::IsSlumped(void) {
  if (hl2_episodic.GetBool()) {
    if (m_ActBusyBehavior.IsInsideActBusy() &&
        !m_ActBusyBehavior.IsStopBusying()) {
      return true;
    }
  } else {
...
void CNPC_BaseZombie::GatherConditions(void) {
  ...
  if (m_NPCState == NPC_STATE_COMBAT && !m_fIsTorso) {
    ...
    // eliminates the zombie picking a better physics object if one happens to
    // fall between him and the object he's heading for already.
  ...
  if ((m_hPhysicsEnt != NULL) && gpGlobals->curtime >= m_flNextSwat &&
      HasCondition(COND_SEE_ENEMY) && !HasCondition(COND_ZOMBIE_RELEASECRAB)) {
...
void CNPC_BaseZombie::PrescheduleThink(void) {
  ...
  //
  if ((m_flBurnDamageResetTime) &&
      (gpGlobals->curtime >= m_flBurnDamageResetTime)) {
    m_flBurnDamage = 0;
  }
}
  ...
  switch (pTask->iTask) {
    ...
    case TASK_ZOMBIE_SWAT_ITEM: {
      if (m_hPhysicsEnt == NULL) {
        ...
        // Physics ent is no longer in range! Probably another zombie swatted it
        // or it moved for some other reason.
...
// torso!
//---------------------------------------------------------
void CNPC_BaseZombie::BecomeTorso(const Vector &vecTorsoForce,
  ...
  if (m_fIsTorso == true) {
    // -40 on Z to make up for the +40 on Z that we did above. This stops legs
    // spawning above the head.
    CBaseEntity *pGib =
        CreateRagGib(GetLegsModel(), GetAbsOrigin() - Vector(0, 0, 40),
                     GetAbsAngles(), vecLegsForce, flFadeTime);
...
void CNPC_BaseZombie::Event_Killed(const CTakeDamageInfo &info) {
  if (info.GetDamageType() & DMG_VEHICLE) {
    ...
    // Big blood splat
    UTIL_BloodSpray(WorldSpaceCenter(), vecDamageDir, BLOOD_COLOR_YELLOW, 8,
                    FX_BLOODSPRAY_CLOUD);
  }
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_BaseZombie::BecomeRagdoll(const CTakeDamageInfo &info,
...
#define ZOMBIE_CRAB_INHERITED_SPAWNFLAGS \
  (SF_NPC_GAG | SF_NPC_LONG_RANGE | SF_NPC_FADE_CORPSE | SF_NPC_ALWAYSTHINK)
...
bool CNPC_BaseZombie::HeadcrabFits(CBaseAnimating *pCrab) {
  ...
  trace_t tr;
  AI_TraceHull(vecSpawnLoc, vecSpawnLoc - Vector(0, 0, 1),
               NAI_Hull::Mins(HULL_TINY) * CRAB_HULL_EXPAND,
               NAI_Hull::Maxs(HULL_TINY) * CRAB_HULL_EXPAND, MASK_NPCSOLID,
               &traceFilter, &tr);
  ...
  // NDebugOverlay::Box( vecSpawnLoc, NAI_Hull::Mins(HULL_TINY) *
  // CRAB_HULL_EXPAND, NAI_Hull::Maxs(HULL_TINY) * CRAB_HULL_EXPAND, 0, 255, 0,
  // 100, 10.0 );
  return true;
}
...
//			fRagdollBody -
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::ReleaseHeadcrab(const Vector &vecOrigin,
  ...
  if (fRagdollCrab) {
    ...
    CBaseEntity *pGib =
        CreateRagGib(GetHeadcrabModel(), vecOrigin, GetLocalAngles(),
    ...
    if (pGib) {
      ...
      if (UTIL_ShouldShowBlood(BLOOD_COLOR_YELLOW)) {
        UTIL_BloodImpact(pGib->WorldSpaceCenter(), Vector(0, 0, 1),
...
void CNPC_BaseZombie::SetHeadcrabSpawnLocation(int iCrabAttachment,
                                               CBaseAnimating *pCrab) {
  ...
  // find spawn location needed for rootLocal transform to match
  // attachmentToWorld
...
//-----------------------------------------------------------------------------
// Purpose: Refines a base activity into something more specific to our internal
// state.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_BaseZombie::OnInsufficientStopDist(AILocalMoveGoal_t *pMoveGoal,
                                             float distClear,
                                             AIMoveResult_t *pResult) {
  if (pMoveGoal->directTrace.fStatus == AIMR_BLOCKED_ENTITY &&
      gpGlobals->curtime >= m_flNextSwat) {
...
//			&chasePosition -
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::TranslateNavGoal(CBaseEntity *pEnemy,
                                       Vector &chasePosition) {
  // If our enemy is in a vehicle, we need them to tell us where to navigate to
  // them
  ...
  if (pBCC && pBCC->IsInAVehicle()) {
    ...
    chasePosition =
        pBCC->WorldSpaceCenter() + (vecForward * 24.0f) + (vecRight * 48.0f);
    return;
  }
...
DEFINE_SCHEDULE(SCHED_ZOMBIE_MOVE_TO_AMBUSH,
                ...
                "	Tasks"
                "		TASK_WAIT				"
                "		1.0"  // don't react as soon as you see the
                                      // player.
                ...
                "	180"
                "		TASK_SET_SCHEDULE			"
                "	SCHEDULE:SCHED_ZOMBIE_WAIT_AMBUSH"
                "	"
                "	Interrupts"
                "		COND_TASK_FAILED"
                "		COND_NEW_ENEMY")
...
DEFINE_SCHEDULE(SCHED_ZOMBIE_WANDER_MEDIUM,
                ...
                "	0"
                "		TASK_WAIT_PVS				"
                "	0"  // if the player left my PVS, just wait.
                "		TASK_SET_SCHEDULE			"
                "	SCHEDULE:SCHED_ZOMBIE_WANDER_MEDIUM"  // keep doing it
                "	"
                "	Interrupts"
                "		COND_NEW_ENEMY"
                "		COND_SEE_ENEMY"
...
DEFINE_SCHEDULE(SCHED_ZOMBIE_WANDER_FAIL,
                ...
                "		TASK_WAIT				1"
                "		TASK_SET_SCHEDULE		"
                "SCHEDULE:SCHED_ZOMBIE_WANDER_MEDIUM"
                "	Interrupts"
                "		COND_NEW_ENEMY"
                "		COND_LIGHT_DAMAGE"
                "		COND_HEAVY_DAMAGE"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_MELEE_ATTACK1		0"
    "		TASK_SET_SCHEDULE		"
    "SCHEDULE:SCHED_ZOMBIE_POST_MELEE_WAIT"
    ""
    "	Interrupts"
    "		COND_LIGHT_DAMAGE"
    "		COND_HEAVY_DAMAGE")
