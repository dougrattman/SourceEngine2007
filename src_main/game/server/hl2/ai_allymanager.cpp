...
void CAI_AllyManager::CountAllies(int *pTotal, int *pMedics) {
  ...
  for (int i = 0; i < nAIs; i++) {
    if (ppAIs[i]->IsAlive() && ppAIs[i]->IsPlayerAlly()) {
      ...
      if (ppAIs[i]->Classify() == CLASS_PLAYER_ALLY_VITAL) continue;

      // They only count if I can use them.
      if (ppAIs[i]->HasSpawnFlags(SF_CITIZEN_NOT_COMMANDABLE)) continue;

      // They only count if I can use them.
      if (ppAIs[i]->IRelationType(UTIL_GetLocalPlayer()) != D_LI) continue;
      ...
      if (FClassnameIs(ppAIs[i], "npc_citizen")) {
        ...
        if (ppAIs[i]->HasSpawnFlags(SF_CITIZEN_MEDIC)) (*pMedics)++;
      }
