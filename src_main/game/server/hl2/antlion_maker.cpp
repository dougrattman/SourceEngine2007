...
void CAntlionMakerManager::GatherMakers(void) {
  ...
  // Find these all once
  while ((pSearch = gEntList.FindEntityByClassname(
              pSearch, "npc_antlion_template_maker")) != NULL) {
...
BEGIN_DATADESC(CAntlionTemplateMaker)
  ...
  DEFINE_KEYFIELD(m_strSpawnGroup, FIELD_STRING, "spawngroup"),
      ...
      DEFINE_INPUTFUNC(FIELD_INTEGER, "SetPoolRegenAmount",
                       InputSetPoolRegenAmount),
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetPoolRegenTime", InputSetPoolRegenTime),
      DEFINE_INPUTFUNC(FIELD_STRING, "ChangeDestinationGroup",
...
void CAntlionTemplateMaker::FixupOrphans(void) {
  ...
  while ((pSearch = gEntList.FindEntityByClassname(pSearch, "npc_antlion")) !=
         NULL) {
    ...
    if (pAntlion && pAntlion->GetOwnerEntity() == NULL && pAntlion->IsAlive()) {
      ...
      if (stricmp(pAntlion->GetParentSpawnerName(), STRING(GetEntityName())) ==
          0) {
        // Relink us to this antlion, he's come through a transition and was
        // orphaned
        AddChild(pAntlion);
      }
    }
...
void CAntlionTemplateMaker::Activate(void) {
  ...
  if (m_iMaxPool) {
    ...
    // Start up our think cycle unless we're reloading this map (which would
    // reset it)
    if (m_bDisabled == false && gpGlobals->eLoadType != MapLoad_LoadGame) {
      // Start our pool regeneration cycle
      SetContextThink(&CAntlionTemplateMaker::PoolRegenThink,
                      gpGlobals->curtime + m_flPoolRegenTime,
                      s_pPoolThinkContext);

      // Start our blocked effects cycle
      if (hl2_episodic.GetBool() == true &&
          HasSpawnFlags(SF_ANTLIONMAKER_DO_BLOCKEDEFFECTS)) {
        SetContextThink(&CAntlionTemplateMaker::FindNodesCloseToPlayer,
                        gpGlobals->curtime + 1.0f,
                        s_pBlockedEffectsThinkContext);
      }
    }
  }
...
void CAntlionTemplateMaker::ActivateSpore(const char *sporename,
                                          Vector vOrigin) {
  if (m_bCreateSpores == false) return;

  char szName[64];
  Q_snprintf(szName, sizeof(szName), "%s_spore", sporename);

  SporeExplosion *pSpore =
      (SporeExplosion *)gEntList.FindEntityByName(NULL, szName);
  ...
  for (int i = 0; i < hintList.Count(); i++) {
    ...
    if (pTestHint) {
      ...
      if (!AllHintsFromClusterBlocked(pTestHint, bBlank)) {
        ActivateSpore(STRING(pTestHint->GetEntityName()),
                      pTestHint->GetAbsOrigin());
      }
    }
  }
}
...
CBaseEntity *CAntlionTemplateMaker::GetFightTarget(void) {
  if (m_hFightTarget != NULL) return m_hFightTarget;
...
void CAntlionTemplateMaker::UpdateChildren(void) {
  ...
  for (; i < m_Children.Count(); i++) {
    ...
    if (pAntlion->m_lifeState != LIFE_ALIVE) continue;
...
void CAntlionTemplateMaker::SetFightTarget(string_t strTarget,
  ...
  if (HasSpawnFlags(SF_ANTLIONMAKER_RANDOM_FIGHT_TARGET)) {
    ...
    for (int i = random->RandomInt(1, 5); i > 0; i--)
      pSearch = gEntList.FindEntityByName(pSearch, strTarget, this, pActivator,
                                          pCaller);

    if (pSearch != NULL) {
      SetFightTarget(pSearch);
    } else {
      SetFightTarget(gEntList.FindEntityByName(NULL, strTarget, this,
                                               pActivator, pCaller));
    }
  } else {
    SetFightTarget(
        gEntList.FindEntityByName(NULL, strTarget, this, pActivator, pCaller));
  }
}
...
void CAntlionTemplateMaker::SetFollowTarget(string_t strTarget,
                                            ...
                                            CBaseEntity *pCaller) {
  CBaseEntity *pSearch =
      gEntList.FindEntityByName(NULL, strTarget, NULL, pActivator, pCaller);
...
bool CAntlionTemplateMaker::CanMakeNPC(bool bIgnoreSolidEntities) {
  if (m_nMaxLiveChildren == 0) return false;
  ...
  if (m_iMaxPool && !m_iPool) return false;
...
void CAntlionTemplateMaker::Enable(void) {
  ...
  if (hl2_episodic.GetBool() == true &&
      HasSpawnFlags(SF_ANTLIONMAKER_DO_BLOCKEDEFFECTS)) {
    SetContextThink(&CAntlionTemplateMaker::FindNodesCloseToPlayer,
                    gpGlobals->curtime + 1.0f, s_pBlockedEffectsThinkContext);
  }
...
void CAntlionTemplateMaker::ChildPreSpawn(CAI_BaseNPC *pChild) {
  ...
  if ((m_flWorkerSpawnRate > 0) &&
      (random->RandomFloat(0, 1) < m_flWorkerSpawnRate)) {
...
void CAntlionTemplateMaker::MakeNPC(void) {
  ...
  if (CanMakeNPC(true) == false) return;
  ...
  CBaseEntity *pTarget =
      gEntList.FindEntityByName(NULL, m_strSpawnTarget, this);
  ...
  if (HasSpawnFlags(SF_ANTLIONMAKER_SPAWN_CLOSE_TO_TARGET)) {
    if (FindNearTargetSpawnPosition(spawnOrigin, m_flSpawnRadius, pTarget) ==
        false)
      return;
  } else {
    // If we can't find a spawn position, then we can't spawn this time
    if (FindHintSpawnPosition(targetOrigin, m_flSpawnRadius, m_strSpawnGroup,
                              &pNode, bRandom) == false)
      return;
    ...
    if (m_iMaxPool) {
      ...
      if (g_debug_antlionmaker.GetInt() == 2) {
        Msg("SPAWNED: Pool: %d (max %d) (Regenerating %d every %f)\n", m_iPool,
            m_iMaxPool, m_iPoolRegenAmount, m_flPoolRegenTime);
      }
    } else {
      m_nMaxNumNPCs--;
    }
...
bool CAntlionTemplateMaker::FindPositionOnFoot(Vector &origin, float radius,
                                               CBaseEntity *pTarget) {
  int iMaxTries = 10;
...
bool CAntlionTemplateMaker::FindPositionOnVehicle(Vector &origin, float radius,
                                                  CBaseEntity *pTarget) {
  int iMaxTries = 10;
  Vector vSpawnOrigin = pTarget->GetAbsOrigin();
  vSpawnOrigin.z += 96;
  ...
  while (iMaxTries > 0) {
    ...
    float flSpeed =
        (pTarget->GetSmoothedVelocity().Length() * m_flVehicleSpawnDistance) *
        random->RandomFloat(1.0f, 1.5f);

    vSpawnOrigin = vSpawnOrigin + (vForward * flSpeed) +
                   vRight * random->RandomFloat(-radius, radius);
...
bool CAntlionTemplateMaker::ValidateSpawnPosition(Vector &vOrigin,
                                                  CBaseEntity *pTarget) {
  trace_t tr;
  UTIL_TraceLine(vOrigin, vOrigin - Vector(0, 0, 1024),
                 MASK_BLOCKLOS | CONTENTS_WATER, NULL, COLLISION_GROUP_NONE,
                 &tr);
  ...
  if (tr.fraction != 1.0) {
    ...
    if (psurf) {
      ...
      if (psurf->game.material != CHAR_TEX_SAND) return false;
    }
    ...
    trace_t trCheck;
    UTIL_TraceHull(tr.endpos, tr.endpos + Vector(0, 0, 5),
                   NAI_Hull::Mins(HULL_MEDIUM), NAI_Hull::Maxs(HULL_MEDIUM),
                   MASK_NPCSOLID, NULL, COLLISION_GROUP_NONE, &trCheck);

    if (trCheck.DidHit() == false) {
      if (g_debug_antlionmaker.GetInt() == 1)
        NDebugOverlay::Box(tr.endpos + Vector(0, 0, 5),
                           NAI_Hull::Mins(HULL_MEDIUM),
                           NAI_Hull::Maxs(HULL_MEDIUM), 0, 255, 0, 128, 5);
      ...
      if (pTarget) {
        if (pTarget->IsPlayer()) {
          ...
          CTraceFilterSkipTwoEntities traceFilter(pPlayer, pVehicle,
                                                  COLLISION_GROUP_NONE);

          trace_t trVerify;

          Vector vVerifyOrigin =
              pPlayer->GetAbsOrigin() + pPlayer->GetViewOffset();
          float flZOffset = NAI_Hull::Maxs(HULL_MEDIUM).z;
          UTIL_TraceLine(vVerifyOrigin, tr.endpos + Vector(0, 0, flZOffset),
                         MASK_BLOCKLOS | CONTENTS_WATER, &traceFilter,
                         &trVerify);

          if (trVerify.fraction != 1.0f) {
            const surfacedata_t *psurf =
                physprops->GetSurfaceData(trVerify.surface.surfaceProps);

            if (psurf) {
              if (psurf->game.material == CHAR_TEX_DIRT) {
                if (g_debug_antlionmaker.GetInt() == 1) {
                  NDebugOverlay::Line(vVerifyOrigin, trVerify.endpos, 255, 0, 0,
                                      false, 5);
                }
          ...
          if (g_debug_antlionmaker.GetInt() == 1) {
            NDebugOverlay::Line(vVerifyOrigin, trVerify.endpos, 0, 255, 0,
                                false, 5);
          }
        }
      }
      ...
      if (g_debug_antlionmaker.GetInt() == 1)
        NDebugOverlay::Box(tr.endpos + Vector(0, 0, 5),
                           NAI_Hull::Mins(HULL_MEDIUM),
                           NAI_Hull::Maxs(HULL_MEDIUM), 255, 0, 0, 128, 5);
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAntlionTemplateMaker::FindNearTargetSpawnPosition(Vector &origin,
                                                        float radius,
                                                        CBaseEntity *pTarget) {
  if (pTarget) {
    CBaseEntity *pVehicle = NULL;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAntlionTemplateMaker::FindHintSpawnPosition(const Vector &origin,
                                                  float radius,
                                                  string_t hintGroupName,
                                                  CAI_Hint **pHint,
                                                  bool bRandom) {
  ...
  if (pChosenHint != NULL) {
    ...
    if (AllHintsFromClusterBlocked(pChosenHint, bChosenHintBlocked)) {
      if ((GetIndexForThinkContext(s_pBlockedCheckContext) ==
           NO_THINK_CONTEXT) ||
          (GetNextThinkTick(s_pBlockedCheckContext) == TICK_NEVER_THINK)) {
        SetContextThink(&CAntlionTemplateMaker::BlockedCheckFunc,
                        gpGlobals->curtime + 2.0f, s_pBlockedCheckContext);
      }
...
void CAntlionTemplateMaker::DoBlockedEffects(CBaseEntity *pBlocker,
                                             Vector vOrigin) {
  ...
  if (pBlocker) {
    ...
    if (pPhysObj && pPhysObj->IsAsleep()) {
      ...
      m_flBlockedBumpTime =
          gpGlobals->curtime + random->RandomFloat(1.75, 2.75);
    }
  }
}

CBaseEntity *CAntlionTemplateMaker::AllHintsFromClusterBlocked(
    CAI_Hint *pNode, bool &bChosenHintBlocked) {
  // Only do this for episodic content!
  if (hl2_episodic.GetBool() == false) return NULL;
      ...
      if (pTestHint) {
        if (pTestHint->NameMatches(pNode->GetEntityName())) {
          ...
          int count = UTIL_EntitiesInBox(
              pList, 20, spawnOrigin + NAI_Hull::Mins(HULL_MEDIUM),
              spawnOrigin + NAI_Hull::Maxs(HULL_MEDIUM), 0);

          // Iterate over all the possible targets
          for (int i = 0; i < count; i++) {
            if (pList[i]->GetMoveType() != MOVETYPE_VPHYSICS) continue;
          ...
          if (g_debug_antlionmaker.GetInt() == 1) {
            if (bBlocked) {
              NDebugOverlay::Box(
                  spawnOrigin + Vector(0, 0, 5), NAI_Hull::Mins(HULL_MEDIUM),
                  NAI_Hull::Maxs(HULL_MEDIUM), 255, 0, 0, 128, 0.25f);
            } else {
              NDebugOverlay::Box(
                  spawnOrigin + Vector(0, 0, 5), NAI_Hull::Mins(HULL_MEDIUM),
                  NAI_Hull::Maxs(HULL_MEDIUM), 0, 255, 0, 128, 0.25f);
            }
          }
        }
      }
...
void CAntlionTemplateMaker::FindNodesCloseToPlayer(void) {
  SetContextThink(&CAntlionTemplateMaker::FindNodesCloseToPlayer,
                  gpGlobals->curtime + random->RandomFloat(0.75, 1.75),
                  s_pBlockedEffectsThinkContext);
  ...
  hintCriteria.SetGroup(m_strSpawnGroup);
  hintCriteria.SetHintType(HINT_ANTLION_BURROW_POINT);
  hintCriteria.AddIncludePosition(pPlayer->GetAbsOrigin(),
                                  ANTLION_MAKER_PLAYER_DETECT_RADIUS);
  ...
  //----
  // What we do here is find all hints of the same name (cluster name) and
  // figure out if all of them are blocked. If they are then we only need to play
  // the blocked effects once
  //---
  for (int i = 0; i < hintList.Count(); i++) {
    CAI_Hint *pNode = hintList[i];

    if (pNode && pNode->HintMatchesCriteria(
                     NULL, hintCriteria, pPlayer->GetAbsOrigin(), &flRadius)) {
      bool bClusterAlreadyBlocked = false;

      // Have one of the nodes from this cluster been checked for blockage? If
      // so then there's no need to do block checks again for this cluster.
      for (int iStringCount = 0; iStringCount < m_BlockedNames.Count();
           iStringCount++) {
      ...
      if (bClusterAlreadyBlocked == true) continue;
...
void CAntlionTemplateMaker::BlockedCheckFunc(void) {
  SetContextThink(&CAntlionTemplateMaker::BlockedCheckFunc, -1,
                  s_pBlockedCheckContext);

  if (m_bBlocked == true) return;
  ...
  if (CAI_HintManager::FindAllHints(vec3_origin, hintCriteria, &hintList) > 0) {
    for (int i = 0; i < hintList.Count(); i++) {
      ...
      if (pNode) {
        ...
        CBaseEntity *pList[20];
        int count = UTIL_EntitiesInBox(
            pList, 20, vHintPos + NAI_Hull::Mins(HULL_MEDIUM),
            vHintPos + NAI_Hull::Maxs(HULL_MEDIUM), 0);
...
void CAntlionTemplateMaker::PoolRegenThink(void) {
  ...
  SetContextThink(&CAntlionTemplateMaker::PoolRegenThink,
                  gpGlobals->curtime + m_flPoolRegenTime, s_pPoolThinkContext);
}
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    if (m_iMaxPool) {
      ...
      float flTimeRemaining =
          GetNextThink(s_pPoolThinkContext) - gpGlobals->curtime;
...
void CAntlionTemplateMaker::DrawDebugGeometryOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    // Draw ourself
    NDebugOverlay::Box(GetAbsOrigin(), -Vector(8, 8, 8), Vector(8, 8, 8), r, g,
                       b, true, 0.05f);
    ...
    if (m_strSpawnGroup != NULL_STRING) {
      ...
      while (pHint != NULL) {
        ...
        NDebugOverlay::VertArrow(GetAbsOrigin(),
                                 pHint->GetAbsOrigin() + Vector(0, 0, 32), 8.0f,
                                 r, g, b, 0, true, 0.05f);

        // Draw a box to represent where it's sitting
        Vector vecForward;
        AngleVectors(pHint->GetAbsAngles(), &vecForward);
        NDebugOverlay::BoxDirection(pHint->GetAbsOrigin(), -Vector(32, 32, 0),
                                    Vector(32, 32, 16), vecForward, r, g, b,
                                    true, 0.05f);
    ...
    if (m_strSpawnTarget != NULL_STRING) {
      ...
      if (pTarget != NULL) {
        NDebugOverlay::VertArrow(GetAbsOrigin(), pTarget->WorldSpaceCenter(),
                                 4.0f, 255, 255, 255, 0, true, 0.05f);
      }
    }
