...
ConVar g_cannon_reloadtime("g_cannon_reloadtime", "3",
                           FCVAR_CHEAT | FCVAR_GAMEDLL);
ConVar g_cannon_max_traveltime("g_cannon_max_traveltime", "1.5",
                               FCVAR_CHEAT | FCVAR_GAMEDLL);
ConVar g_cannon_debug("g_cannon_debug", "0", FCVAR_CHEAT | FCVAR_GAMEDLL);
ConVar g_cannon_damageandradius("g_cannon_damageandradius", "512",
                                FCVAR_CHEAT | FCVAR_GAMEDLL);
...
SendPropEHandle(SENDINFO(m_hPlayer)), SendPropBool(SENDINFO(m_bEnterAnimOn)),
...
//-----------------------------------------------------------------------------
void CPropCannon::SetupMove(CBasePlayer *player, CUserCmd *ucmd,
                            IMoveHelper *pHelper, CMoveData *move) {
...
void CPropCannon::ProjectileExplosion(void) {
  ...
  // do damage
  CTakeDamageInfo info(this, this, g_cannon_damageandradius.GetInt(),
                       DMG_BLAST);

  info.SetDamagePosition(m_vCrashPoint);
  RadiusDamage(info, m_vCrashPoint, g_cannon_damageandradius.GetInt(),
               CLASS_NONE, NULL);
}
...
void CPropCannon::Think(void) {
  ...
  if (m_flFlyTime > 0.0f) {
    if (m_flFlyTime - 1.0f <= gpGlobals->curtime &&
        m_flFlyTime - 0.8f > gpGlobals->curtime) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPropCannon::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                      USE_TYPE useType, float value) {
...
void CPropCannon::LaunchProjectile(void) {
  // ADRIANTODO: Come back to this once we get the right model and remove all
  // the fix ups caused by temp content.
  ...
  while (bCollided == false && iFailSafe < 100000) {
    ...
    trace_t pm;
    UTIL_TraceLine(vOldOrigin, vOrigin, MASK_SOLID, this, COLLISION_GROUP_NONE,
                   &pm);
    ...
    if (pm.fraction != 1.0f && bInSky == false) {
      ...
      if (g_cannon_debug.GetBool() == true) {
        NDebugOverlay::Box(vOrigin, Vector(256, 256, 256),
                           Vector(-256, -256, -256), 255, 0, 0, 0, 5);
      }
    } else {
      vVelocity[2] += gravity;
    }
  ...
  UTIL_ScreenShake(GetDriver()->GetAbsOrigin(), 50.0, 150.0, 1.0, 750,
                   SHAKE_START, true);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CCannonServerVehicle::GetVehicleViewPosition(int nRole, Vector *pAbsOrigin,
                                                  QAngle *pAbsAngles,
                                                  float *pFOV /*= NULL*/) {
  ...
  *pAbsAngles = pPlayer->EyeAngles();  // yuck. this is an in/out parameter.
  ...
  GetCannon()->GetAttachment("vehicle_driver_eyes", vehicleEyeOrigin,
                             vehicleEyeAngles);
