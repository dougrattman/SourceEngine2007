...
BEGIN_DATADESC(CNPC_BaseScanner)
  DEFINE_EMBEDDED(m_KilledInfo), DEFINE_SOUNDPATCH(m_pEngineSound),
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetDistanceOverride",
...
void CNPC_BaseScanner::Spawn(void) {
  ...
  CapabilitiesAdd(bits_CAP_MOVE_FLY | bits_CAP_SQUAD | bits_CAP_TURN_HEAD |
...
void CNPC_BaseScanner::UpdateOnRemove(void) {
  // Stop combat loops if I'm alive. If I'm dead, the die sound will already
  // have stopped it.
...
int CNPC_BaseScanner::SelectSchedule(void) {
  ...
  if (GetEnemy() != NULL && GetEnemy()->IsAlive()) {
    ...
    if (gpGlobals->curtime >= m_flNextAttack) {
      if (HasCondition(COND_CAN_MELEE_ATTACK1)) return SCHED_SCANNER_ATTACK;
    }
...
int CNPC_BaseScanner::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  float vEnemyDist2D =
      (GetEnemy()->GetLocalOrigin() - GetLocalOrigin()).Length2D();
    ...
    case TASK_SCANNER_SET_FLY_DIVE: {
      ...
      if (GetEnemy() != NULL) {
        ...
        m_vecDiveBombDirection =
            forward + Vector(random->RandomFloat(-10, 10),
                             random->RandomFloat(-10, 10),
                             random->RandomFloat(-20, -10));
      }
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::TraceAttack(const CTakeDamageInfo &info,
...
//-----------------------------------------------------------------------------
#define SCANNER_SMASH_SPEED \
  250.0  // How fast a scanner must slam into something to take full damage
...
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::TakeDamageFromPhysicsImpact(
...
// Physics impact
//------------------------------------------------------------------------------
#define SCANNER_SMASH_TIME \
  0.75  // How long after being thrown from a physcannon that a manhack is
        // eligible to die from impact
void CNPC_BaseScanner::VPhysicsCollision(int index,
...
void CNPC_BaseScanner::Gib(void) {
  ...
  te->DynamicLight(filter, 0.0, &WorldSpaceCenter(), 255, 180, 100, 0, 100, 0.1,
                   0);
...
//			bPunting -
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
...
// Input  : *pPhysGunUser -
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
void CNPC_BaseScanner::AttackDivebombCollide(float flInterval) {
  ...
  AI_TraceHull(GetAbsOrigin(), checkPos, GetHullMins(), GetHullMaxs(),
    ...
    if (pHitEntity && pHitEntity->Classify() != CLASS_SCANNER) {
      if (!pHitEntity->ClassMatches("item_battery")) {
        if (!pHitEntity->IsWorld()) {
          CTakeDamageInfo info(this, this, sk_scanner_dmg_dive.GetFloat(),
                               DMG_CLUB);
          CalculateMeleeDamageForce(&info, (tr.endpos - tr.startpos),
                                    tr.endpos);
  ...
  if (tr.fraction != 1.0) {
    ...
    if (flInterval > 0) {
      float moveLen =
          (1.0 - tr.fraction) * (GetAbsOrigin() - checkPos).Length();
    ...
    if (pBCC) {
      ...
      SpawnBlood(tr.endpos, g_vecAttackDir, pBCC->BloodColor(),
                 sk_scanner_dmg_dive.GetFloat());
    } else {
      ...
      AI_TraceLine(GetAbsOrigin(), checkPos, MASK_SOLID_BRUSHONLY, this,
                   COLLISION_GROUP_NONE, &tr);
      if (tr.fraction != 1.0) {
        g_pEffects->Sparks(tr.endpos);

        CBroadcastRecipientFilter filter;
        te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 255, 180, 100, 0, 50,
                         0.1, 0);
      }
    }
  }
...
void CNPC_BaseScanner::PlayFlySound(void) {
  ...
  if (m_pEngineSound == NULL) {
    ...
    m_pEngineSound = controller.SoundCreate(filter, entindex(), CHAN_STATIC,
                                            GetEngineSound(), ATTN_NORM);
  ...
  int iPitch = std::min(255.0f, 80 + (20 * (speed / GetMaxSpeed())));
...
void CNPC_BaseScanner::MoveExecute_Alive(float flInterval) {
  ...
  if (m_nFlyMode != SCANNER_FLY_DIVE) {
    SetCurrentVelocity(GetCurrentVelocity() +
                       VelocityToAvoidObstacles(flInterval));
  }
  ...
  float tailPerc = std::clamp(GetCurrentVelocity().z, -150.0f, 250.0f);
...
// Input  : flInterval -
//-----------------------------------------------------------------------------
bool CNPC_BaseScanner::OverridePathMove(CBaseEntity *pMoveTarget,
                                        float flInterval) {
  // Save our last patrolling direction
  Vector lastPatrolDir = GetNavigator()->GetCurWaypointPos() - GetAbsOrigin();

  // Continue on our path
  if (ProgressFlyPath(flInterval, pMoveTarget, (MASK_NPCSOLID | CONTENTS_WATER),
                      false, 64) == AINPP_COMPLETE) {
...
// Output : Vector
//-----------------------------------------------------------------------------
Vector CNPC_BaseScanner::IdealGoalForMovement(const Vector &goalPos,
                                              const Vector &startPos,
                                              float idealRange,
                                              float idealHeightDiff) {
  Vector vMoveDir;
  ...
  Vector vIdealPos =
      goalPos + Vector(0, 0, idealHeightDiff) + (vMoveDir * -idealRange);

  // Trace down and make sure we can fit here
  trace_t tr;
  AI_TraceHull(vIdealPos, vIdealPos - Vector(0, 0, MinGroundDist()),
               GetHullMins(), GetHullMaxs(), MASK_NPCSOLID, this,
               COLLISION_GROUP_NONE, &tr);
  ...
  // Debug tools
  if (g_debug_basescanner.GetBool()) {
    NDebugOverlay::Cross3D(goalPos, -Vector(8, 8, 8), Vector(8, 8, 8), 255, 255,
                           0, true, 0.1f);
    NDebugOverlay::Cross3D(startPos, -Vector(8, 8, 8), Vector(8, 8, 8), 255, 0,
                           255, true, 0.1f);
    NDebugOverlay::Cross3D(vIdealPos, -Vector(8, 8, 8), Vector(8, 8, 8), 255,
                           255, 255, true, 0.1f);
    NDebugOverlay::Line(startPos, goalPos, 0, 255, 0, true, 0.1f);

    NDebugOverlay::Cross3D(goalPos + (vMoveDir * -idealRange), -Vector(8, 8, 8),
                           Vector(8, 8, 8), 255, 255, 255, true, 0.1f);
    NDebugOverlay::Line(goalPos, goalPos + (vMoveDir * -idealRange), 255, 255,
                        0, true, 0.1f);
    NDebugOverlay::Line(goalPos + (vMoveDir * -idealRange), vIdealPos, 255, 255,
                        0, true, 0.1f);
  }
...
void CNPC_BaseScanner::MoveToAttack(float flInterval) {
  ...
  // float flDesiredDist = m_flAttackNearDist + ( ( m_flAttackFarDist -
  // m_flAttackNearDist ) / 2 );

  Vector idealPos = IdealGoalForMovement(vTargetPos, GetAbsOrigin(),
                                         GetGoalDistance(), m_flAttackNearDist);
...
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::MoveToTarget(float flInterval,
  ...
  if (pPlayer) {
    ...
    if (DotProduct(vecDelta, vecVelocity) > 0.5f) {
      ...
      if (flDist < 200.0f) {
        ...
        SetContextThink(&CNPC_BaseScanner::DiveBombSoundThink,
                        gpGlobals->curtime + 0.5f,
                        s_pDiveBombSoundThinkContext);
        return;
      }
    }
  }
...
void CNPC_BaseScanner::MoveToDivebomb(float flInterval) {
  ...
  // Set net velocity
  MoveInDirection(flInterval, m_vecDiveBombDirection, myAccel, myAccel,
                  myDecay);
...
float CNPC_BaseScanner::GetGoalDistance(void) {
  ...
  switch (m_nFlyMode) {
    case SCANNER_FLY_ATTACK: {
      float goalDist =
          (m_flAttackNearDist + ((m_flAttackFarDist - m_flAttackNearDist) / 2));
...
bool CNPC_BaseScanner::GetGoalDirection(Vector *vOut) {
  ...
  if (FClassnameIs(pTarget, "info_hint_air") ||
      FClassnameIs(pTarget, "info_target")) {
...
int CNPC_BaseScanner::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "speed (max): %.2f (%.2f)",
               vel.Length(), m_flSpeed);
...
void CNPC_BaseScanner::UpdateHead(float flInterval) {
  ...
  if (pTarget != NULL) {
    ...
    SetPoseParameter(m_nPoseFaceVert,
...
//			&angular -
//-----------------------------------------------------------------------------
void CNPC_BaseScanner::ClampMotorForces(Vector &linear,
                                        AngularImpulse &angular) {
  // limit reaction forces
  if (m_nFlyMode != SCANNER_FLY_DIVE) {
    linear.x = std::clamp(linear.x, -500.0f, 500.0f);
    linear.y = std::clamp(linear.y, -500.0f, 500.0f);
    linear.z = std::clamp(linear.z, -500.0f, 500.0f);
  }
  ...
  angular.z = std::clamp(angular.z, -GetHeadTurnRate(), GetHeadTurnRate());
  if (m_nFlyMode == SCANNER_FLY_DIVE) {
...
void CNPC_BaseScanner::IdleSound(void) {
  if (m_NPCState == NPC_STATE_COMBAT) {
    // dvs: the combat sounds should be related to what is happening, rather
    // than random
