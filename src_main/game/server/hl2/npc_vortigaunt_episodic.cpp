...
#define VORTIGAUNT_SENTENCE_VOLUME \
  (float)0.35                          // volume of vortigaunt sentences
...
#define VORTIGAUNT_CURE_LIFESPAN \
  8.0  // cure tokens only live this long (so they don't get stuck on geometry)

#define VORTIGAUNT_BLUE_FADE_TIME \
  2.25f  // takes this long to fade from green to blue or back

#define VORTIGAUNT_FEAR_ZOMBIE_DIST_SQR \
  Square(60)  // back away from zombies inside this radius

#define PLAYER_CRITICAL_HEALTH_PERC 0.15f

#define TLK_SQUISHED_GRUB \
  "TLK_SQUISHED_GRUB"  // response rule for vortigaunts when they step on a grub
...
ConVar sk_vortigaunt_armor_charge_per_token(
...
ConVar sk_vortigaunt_zap_range("sk_vortigaunt_zap_range", "100", FCVAR_NONE,
                               "Range of vortigaunt's ranged attack (feet)");
ConVar sk_vortigaunt_vital_antlion_worker_dmg(
    "sk_vortigaunt_vital_antlion_worker_dmg", "0.2", FCVAR_NONE,
    "Vital-ally vortigaunts scale damage taken from antlion workers by this "
    "amount.");
...
int AE_VORTIGAUNT_STOP_HURT_GLOW;   // Turn off the hurt handglow: 0=left,
                                    // 1=right
...
BEGIN_DATADESC(CNPC_Vortigaunt)
  ...
  DEFINE_FIELD(m_eHealState, FIELD_INTEGER),
      ...
      DEFINE_KEYFIELD(m_bArmorRechargeEnabled, FIELD_BOOLEAN,
      ...
      DEFINE_KEYFIELD(m_bRegenerateHealth, FIELD_BOOLEAN,
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableArmorRecharge",
                       InputEnableArmorRecharge),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableArmorRecharge",
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableHealthRegeneration",
                       InputEnableHealthRegeneration),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableHealthRegeneration",
      ...
      // Outputs
      DEFINE_OUTPUT(m_OnFinishedExtractingBugbait,
...
SendPropTime(SENDINFO(m_flBlueEndFadeTime)), SendPropBool(SENDINFO(m_bIsBlue)),
...
CNPC_Vortigaunt::CNPC_Vortigaunt(void)
    : m_bPlayerRequestedHeal(false),
      m_flNextHealTime(
          3.0f),  // Let the player settle before we decide to do this
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt::PlayerBelowHealthPercentage(CBasePlayer *pPlayer,
                                                  float flPerc) {
  if (pPlayer == NULL) return false;

  if (pPlayer->ArmorValue()) return false;

  float flMaxHealth = pPlayer->GetMaxHealth();
  if (flMaxHealth == 0.0f) return false;

  float flHealthPerc =
      (flMaxHealth != 0) ? (float)pPlayer->GetHealth() / flMaxHealth : 0.0f;
...
void CNPC_Vortigaunt::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_VORTIGAUNT_HEAL: {
      ...
      float flArmorDelta =
          (float)sk_vortigaunt_armor_charge.GetInt() - pPlayer->ArmorValue();
      m_nNumTokensToSpawn =
          ceil(flArmorDelta / sk_vortigaunt_armor_charge_per_token.GetInt());
...
void CNPC_Vortigaunt::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_RANGE_ATTACK1: {
      // If our enemy is gone, dead or out of sight, pick a new one (only if
      // we're not delaying this behavior)
      if ((HasCondition(COND_ENEMY_OCCLUDED) || GetEnemy() == NULL ||
    ...
    case TASK_VORTIGAUNT_WAIT_FOR_PLAYER: {
      ...
      if (pPlayer != NULL) {
        ...
        if (GetMotor()->DeltaIdealYaw() < 10) {
          ...
          if ((GetAbsOrigin() - pPlayer->GetAbsOrigin()).LengthSqr() <
              Square(32 * 12)) {
            TaskComplete();
          }
        }
...
//-----------------------------------------------------------------------------
// Purpose: Allows each sequence to have a different turn rate associated with
// it. Output : float CNPC_Vortigaunt::MaxYawSpeed
//-----------------------------------------------------------------------------
...
//			long swing arm, this causes stability issues when an npc is
//trying to 			face a vort that's also turning their head
// Output :
//-----------------------------------------------------------------------------
Vector CNPC_Vortigaunt::FacingPosition(void) { return WorldSpaceCenter(); }

//-----------------------------------------------------------------------------
// Purpose: Normal body target is the mid-point between the center and the eyes,
// but
//			the vort's eyes are so far offset, that this is usually in
...
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt::InnateWeaponLOSCondition(const Vector &ownerPos,
                                               const Vector &targetPos,
                                               bool bSetConditions) {
  // Try and figure out a rough idea of where we'll be after a certain time
  // delta and base our conditions on that instead.  This is necessary because
  // the vortigaunt takes a long time to deliver his attack and looks very
  // strange if he starts to attack when he'd never be able to hit due to
  // movement.
  ...
  // Debug data
  /*
  NDebugOverlay::Box( GetEnemy()->BodyTarget( vecNewOwnerPos ), -Vector(4,4,4),
  Vector(4,4,4), 255, 0, 0, 0, 3.0f ); NDebugOverlay::Box( vecFinalTargetPos,
  -Vector(4,4,4), Vector(4,4,4), 255, 255, 0, 0, 3.0f );
  NDebugOverlay::HorzArrow( GetEnemy()->BodyTarget( vecNewOwnerPos ),
  vecFinalTargetPos, 12.0f, 255, 0, 0, 16.0f, true, 3.0f );
  NDebugOverlay::HorzArrow( vecNewOwnerPos, GetEnemy()->BodyTarget(
  vecNewOwnerPos ), 8.0f, 255, 255, 0, 32.0f, true, 3.0f );
  */

  return BaseClass::InnateWeaponLOSCondition(vecNewOwnerPos, vecFinalTargetPos,
                                             bSetConditions);
}
...
int CNPC_Vortigaunt::RangeAttack1Conditions(float flDot, float flDist) {
  if (GetEnemy() == NULL) return COND_NONE;

  if (gpGlobals->curtime < m_flNextAttack) return COND_NONE;

  // Don't do shooting while playing a scene
  if (IsCurSchedule(SCHED_SCENE_GENERIC)) return COND_NONE;
...
#else
  if (flDist < 32.0f) return COND_TOO_CLOSE_TO_ATTACK;
#endif  // HL2_EPISODIC
  ...
  if (IsAntlionWorker(GetEnemy())) {
    ...
    if ((GetAbsOrigin() - GetEnemy()->GetAbsOrigin()).LengthSqr() <
        Square(AntlionWorkerBurstRadius()))
      return COND_TOO_CLOSE_TO_ATTACK;

    CBasePlayer *pPlayer = AI_GetSinglePlayer();
    if (pPlayer &&
        (pPlayer->GetAbsOrigin() - GetEnemy()->GetAbsOrigin()).LengthSqr() <
            Square(AntlionWorkerBurstRadius())) {
...
int CNPC_Vortigaunt::MeleeAttack1Conditions(float flDot, float flDist) {
  if (m_flDispelTestTime > gpGlobals->curtime) return COND_NONE;
...
void CNPC_Vortigaunt::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_VORTIGAUNT_ZAP_POWERUP) {
    if (m_fGlowChangeTime > gpGlobals->curtime) return;
  ...
  if (pEvent->event == AE_VORTIGAUNT_ZAP_SHOOT) {
    ...
    if (m_bExtractingBugbait) {
      ...
      if (pWeapon) {
        // Starting above the body, spawn closer and closer to the vort until
        // it's clear
        Vector vecSpawnOrigin =
            GetTarget()->WorldSpaceCenter() + Vector(0, 0, 32);
        int iNumAttempts = 4;
        ...
        for (; i < iNumAttempts; i++) {
          trace_t tr;
          CTraceFilterSkipTwoEntities traceFilter(GetTarget(), this,
                                                  COLLISION_GROUP_NONE);
          AI_TraceLine(vecSpawnOrigin, vecSpawnOrigin, MASK_SHOT, &traceFilter,
                       &tr);

          if (tr.fraction == 1.0 && !tr.m_pEnt) {
            // Make sure it can fit there
            AI_TraceHull(vecSpawnOrigin, vecSpawnOrigin, -Vector(16, 16, 16),
                         Vector(16, 16, 48), MASK_SHOT, &traceFilter, &tr);
            if (tr.fraction == 1.0 && !tr.m_pEnt) break;
          }

          // NDebugOverlay::Box( vecSpawnOrigin, pWeapon->WorldAlignMins(),
          // pWeapon->WorldAlignMins(), 255,0,0, 64, 100 );
        ...
        // NDebugOverlay::Box( vecSpawnOrigin, -Vector(20,20,20),
        // Vector(20,20,20), 0,255,0, 64, 100 );
...
Activity CNPC_Vortigaunt::NPC_TranslateActivity(Activity eNewActivity) {
  ...
  if (IsCarryingNPC()) {
    if (eNewActivity == ACT_IDLE) return ACT_IDLE_CARRY;

    if (eNewActivity == ACT_WALK || eNewActivity == ACT_WALK_AIM ||
        eNewActivity == ACT_RUN || eNewActivity == ACT_RUN_AIM)
      return ACT_WALK_CARRY;
  }

  // NOTE: This is a stand-in until the readiness system can handle non-weapon
  // holding NPC's
  if (eNewActivity == ACT_IDLE) {
    // More than relaxed means we're stimulated
    if (GetReadinessLevel() >= AIRL_STIMULATED) return ACT_IDLE_STIMULATED;
  }

  if (eNewActivity == ACT_RANGE_ATTACK2) return (Activity)ACT_VORTIGAUNT_DISPEL;
...
void CNPC_Vortigaunt::Spawn(void) {
  ...
  SetViewOffset(
      Vector(0, 0, 64));  // position of the eyes relative to monster's origin.
...
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::TraceAttack(const CTakeDamageInfo &inputInfo,
                                  const Vector &vecDir, trace_t *ptr) {
  CTakeDamageInfo info = inputInfo;

  if ((info.GetDamageType() & DMG_SHOCK) &&
      FClassnameIs(info.GetAttacker(), GetClassname())) {
...
int CNPC_Vortigaunt::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    ...
    case SCHED_TAKE_COVER_FROM_BEST_SOUND:
      ...
      // Stand still if we're in the middle of an attack.  Failing to do so can
      // make us miss our shot!
      if (IsPlayingGesture(ACT_GESTURE_RANGE_ATTACK1)) return SCHED_COMBAT_FACE;
    ...
    case SCHED_RANGE_ATTACK1:
      ...
      // If we're told to fire when we're already firing, just face our target.
      // If we don't do this, we get a bizarre double-shot
      if (IsPlayingGesture(ACT_GESTURE_RANGE_ATTACK1)) return SCHED_COMBAT_FACE;
      ...
      decisions when this creature
              // uses an innate ability.
              if ( ( GetNextAttack() > gpGlobals->curtime ) && HasCondition(
      COND_ENEMY_TOO_FAR ) == false ) return SCHED_COMBAT_FACE;
...
//-----------------------------------------------------------------------------
// Purpose: Sets the heal target for the vort and preps him for completing the
// action Input  : *pTarget - Target we're after
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::SetHealTarget(CBaseEntity *pTarget,
                                    bool bPlayerRequested) {
...
CBaseEntity *CNPC_Vortigaunt::FindHealTarget(void) {
  ...
  if (ShouldHealTarget(pEntity) == false) return NULL;
...
bool CNPC_Vortigaunt::HealBehaviorAvailable(void) {
  ...
  if (m_eHealState != HEAL_STATE_NONE) return false;

  // Must be allowed to do this behavior
  if (m_bArmorRechargeEnabled == false) return false;

  // Don't interrupt a script
  if (IsInAScript() || m_NPCState == NPC_STATE_SCRIPT) return false;

  // Cannot interrupt bugbait extraction
  if (IsCurSchedule(SCHED_VORTIGAUNT_EXTRACT_BUGBAIT)) return false;
  ...
  if (IsStrategySlotRangeOccupied(SQUAD_SLOT_HEAL_PLAYER,
...
bool CNPC_Vortigaunt::ShouldHealTarget(CBaseEntity *pTarget) {
  ...
  if (IsInAScript() || HasSpawnFlags(SF_NPC_WAIT_FOR_SCRIPT)) return false;
  ...
  if (pPlayer->IsSuitEquipped() == false) return false;

  // Don't heal a target we can't see..?
  if (pPlayer->GetFlags() & FL_NOTARGET) return false;
  ...
  // Must be alive!
  if (pPlayer->IsAlive() == false) return false;

  // Only consider things in here if the player is NOT at critical health or the
  // heal is a passive one (not requested)
  if (PlayerBelowHealthPercentage(pPlayer, PLAYER_CRITICAL_HEALTH_PERC) ==
          false ||
      m_bPlayerRequestedHeal) {
    // Don't heal when fighting
    if (m_NPCState == NPC_STATE_COMBAT) return false;
...
int CNPC_Vortigaunt::SelectHealSchedule(void) {
  ...
  if (m_LeadBehavior.HasGoal()) return SCHED_NONE;
...
//-----------------------------------------------------------------------------
// Purpose: Watch this function path for a route around our normal schedule
// changing callbacks
//-----------------------------------------------------------------------------
...
void CNPC_Vortigaunt::OnScheduleChange(void) {
  ...
  if (m_eHealState != HEAL_STATE_NONE) return;

  // If we're changing sequences, always clear
  EndHandGlow(VORTIGAUNT_BEAM_ALL);
  m_fGlowChangeTime =
      gpGlobals->curtime + 0.1f;  // No more glows for this amount of time!
}
...
int CNPC_Vortigaunt::SelectSchedule(void) {
...
#else
  if (IsInAScript()) return BaseClass::SelectSchedule();
#endif

  // If we're currently supposed to be doing something scripted, do it
  // immediately.
  if (m_bExtractingBugbait) return SCHED_VORTIGAUNT_EXTRACT_BUGBAIT;
  ...
  if (HasCondition(COND_VORTIGAUNT_CAN_HEAL)) return SCHED_VORTIGAUNT_HEAL;
...
//
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt::SelectFailSchedule(int failedSchedule, int failedTask,
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::DeclineFollowing(void) { Speak(VORT_POK); }
...
bool CNPC_Vortigaunt::CanBeUsedAsAFriend(void) {
  ...
  if (IsCurSchedule(SCHED_VORTIGAUNT_HEAL)) return false;
...
bool CNPC_Vortigaunt::FInViewCone(CBaseEntity *pEntity) {
  ...
  if ((pEntity->IsNPC() || pEntity->IsPlayer()) &&
      pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
          VORT_360_VIEW_DIST_SQR)
    return true;
...
void CNPC_Vortigaunt::StopHealing(bool bInterrupt) {
  ...
  // Give us time to stop our animation before we start attacking (otherwise we
  // get weird collisions)
...
void CNPC_Vortigaunt::MaintainHealSchedule(void) {
  ...
  if (m_AssaultBehavior.GetOuter() != NULL) {
    ...
    if (m_AssaultBehavior.IsRunning() &&
        (m_AssaultBehavior.IsUrgent() || m_AssaultBehavior.OnStrictAssault())) {
      StopHealing(true);
      return;
    }
  }
    ...
    if (HasCondition(COND_VORTIGAUNT_HEAL_VALID)) {
      if (m_flNextHealTokenTime < gpGlobals->curtime) {
        ...
        if (m_nNumTokensToSpawn <= 0 ||
            (m_bForceArmorRecharge == false &&
             (pPlayer &&
              pPlayer->ArmorValue() >= sk_vortigaunt_armor_charge.GetInt()))) {
          m_flHealHinderedTime = 0.0f;
          m_nNumTokensToSpawn = 0;
        ...
        CVortigauntChargeToken::CreateChargeToken(vecHandPos, this,
                                                  m_hHealTarget);
        m_flNextHealTokenTime =
            gpGlobals->curtime + random->RandomFloat(0.5f, 1.0f);
        m_nNumTokensToSpawn--;
      ...
      /*
      // NOTENOTE: It's better if the vort give up than ignore things around him
      to try and continue -- jdw
...
inline bool CNPC_Vortigaunt::InAttackSequence(void) {
  if (m_MoveAndShootOverlay.IsMovingAndShooting()) return true;

  if (GetActivity() == ACT_RANGE_ATTACK1) return true;

  if (GetActivity() == ACT_VORTIGAUNT_DISPEL) return true;

  if (IsPlayingGesture(ACT_GESTURE_RANGE_ATTACK1)) return true;
...
void CNPC_Vortigaunt::MaintainGlows(void) {
  // Verify that if we're not in an attack gesture, that we're not doing an
  // attack glow
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                         float flInterval) {
...
void CNPC_Vortigaunt::ArmBeam(int beamType, int nHand) {
  ...
  for (int i = 0; i < 3; i++) {
    Vector vecAim = forward * random->RandomFloat(-1, 1) +
                    right * side * random->RandomFloat(0, 1) +
                    up * random->RandomFloat(-1, 1);
    trace_t tr1;
    AI_TraceLine(vecSrc, vecSrc + vecAim * (10 * 12), MASK_SOLID, this,
                 COLLISION_GROUP_NONE, &tr1);

    // Don't hit the sky
    if (tr1.surface.flags & SURF_SKY) continue;
  ...
  unsigned char uchAttachment =
      (nHand == HAND_LEFT) ? m_iLeftHandAttachment : m_iRightHandAttachment;
...
void CNPC_Vortigaunt::StartHandGlow(int beamType, int nHand) {
  // We need this because there's a rare case where a scene can interrupt and
  // turn off our hand glows, but are then turned back on in the same frame due
  // to how animations are applied and anim events are executed after the AI
  // frame.
  if (m_fGlowChangeTime > gpGlobals->curtime) return;
  ...
  switch (beamType) {
    ...
    case VORTIGAUNT_BEAM_ZAP: {
      ...
      if (m_hHandEffect[nHand] == NULL) {
        ...
        m_hHandEffect[nHand] = CVortigauntEffectDispel::CreateEffectDispel(
            GetAbsOrigin(), this, NULL);
        if (m_hHandEffect[nHand] == NULL) return;
      }

      // Stomp our settings
      m_hHandEffect[nHand]->SetParent(this, (nHand == HAND_LEFT)
...
bool CNPC_Vortigaunt::IsValidEnemy(CBaseEntity *pEnemy) {
  if (IsRoller(pEnemy)) {
    ...
    if (pNPC && pNPC->GetEnemy() != NULL) return true;
    return false;
  }

  // Wait until our animation is finished
  if (GetEnemy() == NULL && m_flAimDelay > gpGlobals->curtime) return false;
...
void CNPC_Vortigaunt::CreateBeamBlast(const Vector &vecOrigin) {
  CSprite *pBlastSprite =
      CSprite::SpriteCreate("sprites/vortring1.vmt", vecOrigin, true);
  if (pBlastSprite != NULL) {
    pBlastSprite->SetTransparency(kRenderTransAddFrameBlend, 255, 255, 255, 255,
                                  kRenderFxNone);
...
void CNPC_Vortigaunt::ZapBeam(int nHand) {
  ...
  Vector vecAim =
      GetShootEnemyDir(vecSrc, false);  // We want a clear shot to their core
  ...
  if (m_bExtractingBugbait == true) {
    ...
    if (pTest) {
      ...
      if (m_ragdoll) {
        ...
        AI_TraceLine(vecSrc, vOrigin, MASK_SHOT, this, COLLISION_GROUP_NONE,
                     &tr);
      }
    ...
    AI_TraceLine(vecSrc, vecSrc + (vecAim * InnateRange1MaxRange()), MASK_SHOT,
  ...
  if (pEntity != NULL && m_takedamage) {
    if (g_debug_vortigaunt_aim.GetBool()) {
      NDebugOverlay::Box(tr.endpos, -Vector(2, 2, 2), Vector(2, 2, 2), 255, 0,
                         0, 8, 10.0f);
    }

    CTakeDamageInfo dmgInfo(this, this, sk_vortigaunt_dmg_zap.GetFloat(),
                            DMG_SHOCK);
...
void CNPC_Vortigaunt::InputChargeTarget(inputdata_t &data) {
  CBaseEntity *pTarget = gEntList.FindEntityByName(
      NULL, data.value.String(), NULL, data.pActivator, data.pCaller);
  ...
  int playerArmor =
      (pTarget->IsPlayer()) ? ((CBasePlayer *)pTarget)->ArmorValue() : 0;
...
int CNPC_Vortigaunt::IRelationPriority(CBaseEntity *pTarget) {
  ...
  if (pTarget == NULL) return priority;

  CBaseEntity *pEnemy = GetEnemy();

  // Handle antlion cases
  if (pEnemy != NULL && pEnemy != pTarget) {
    // I have an enemy that is not this thing. If that enemy is near, I
    // shouldn't become distracted.
  ...
  if (m_FollowBehavior.GetFollowTarget() &&
      m_FollowBehavior.GetFollowTarget()->GetAbsOrigin().DistToSqr(
          pTarget->GetAbsOrigin()) < Square(25 * 12)) {
    priority++;
  }

  // Flipped antlions are of lower priority
  CAI_BaseNPC *pNPC = pTarget->MyNPCPointer();
  if (pNPC && pNPC->Classify() == CLASS_ANTLION &&
      pNPC->GetActivity() == ACT_ANTLION_ZAP_FLIP)
    priority--;
...
Disposition_t CNPC_Vortigaunt::IRelationType(CBaseEntity *pTarget) {
  if (pTarget == NULL) return D_NU;

  Disposition_t disposition = BaseClass::IRelationType(pTarget);

  if (pTarget->Classify() == CLASS_ZOMBIE && disposition == D_HT) {
    if (GetAbsOrigin().DistToSqr(pTarget->GetAbsOrigin()) <
        VORTIGAUNT_FEAR_ZOMBIE_DIST_SQR) {
      // Be afraid of a zombie that's near if I'm not allowed to dodge. This
      // will make Alyx back away.
      return D_FR;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Determines whether the heal gesture can successfully reach the
// player Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_Vortigaunt::HealGestureHasLOS(void) {
  ...
  if (pTargetEnt == NULL) return false;
  ...
  UTIL_TraceLine(vecHandPos, pTargetEnt->WorldSpaceCenter(), MASK_SHOT, &filter,
                 &tr);

  // Must be clear
  if (tr.fraction < 1.0f || tr.startsolid || tr.allsolid) return false;
...
void CNPC_Vortigaunt::GatherHealConditions(void) {
  ...
  if (m_bArmorRechargeEnabled == false || HasCondition(COND_NEW_ENEMY) ||
      HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAVY_DAMAGE)) {
  ...
  if (m_bForceArmorRecharge) return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::DispelAntlions(const Vector &vecOrigin, float flRadius,
                                     bool bDispel /*= true*/) {
  // More effects
  if (bDispel) {
  ...
  for (int i = 0; i < nNumEnemies; i++) {
    // We only care about antlions
    if (IsAntlion(pEnemySearch[i]) == false) continue;
    ...
    if (pAntlion->IsWorker() == false) {
      ...
      UTIL_TraceLine(vecOrigin, pAntlion->BodyTarget(vecOrigin),
                     MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
      if (tr.fraction < 1.0f && tr.m_pEnt != pAntlion) continue;
      ...
      float flFalloff =
          RemapValClamped(flDist, 0, flRadius * 0.75f, 1.0f, 0.1f);
      ...
      if (flDist < 128 && bDispel) {
        ...
        CTakeDamageInfo dmgInfo(this, this, vecDir, pAntlion->GetAbsOrigin(),
                                100, DMG_SHOCK);
...
bool CNPC_Vortigaunt::CanRunAScriptedNPCInteraction(bool bForced /*= false*/) {
  ...
  if (InAttackSequence()) return false;
...
// Input  : interrupt -
//-----------------------------------------------------------------------------
void CNPC_Vortigaunt::SetScriptedScheduleIgnoreConditions(
  ...
  static int g_VortConditions[] = {COND_VORTIGAUNT_CAN_HEAL,
...
//-----------------------------------------------------------------------------
// !!!HACKHACK - EP2 - Stop vortigaunt taking all physics damage to prevent it
// dying in freak accidents resembling spontaneous stress damage death (which
// are now impossible) Also stop it taking damage from flames: Fixes it being
// burnt to death from entity flames attached to random debris chunks while
// inside scripted sequences.
//-----------------------------------------------------------------------------
int CNPC_Vortigaunt::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  if (info.GetDamageType() & (DMG_CRUSH | DMG_BURN)) return 0;

  // vital vortigaunts (eg the vortigoth in ep2) take less damage from
  // explosions so that zombines don't blow them up disappointingly. They take
  // less damage still from antlion workers.
...
bool CNPC_Vortigaunt::ShouldMoveAndShoot(void) {
  if (m_FollowBehavior.IsActive()) return true;
...
void CNPC_Vortigaunt::AimGun(void) {
  ...
  if (m_flAimDelay >= gpGlobals->curtime) return;
  ...
  if (GetEnemy()) {
    ...
    if (g_debug_vortigaunt_aim.GetBool()) {
      NDebugOverlay::Line(WorldSpaceCenter(),
                          WorldSpaceCenter() + vecShootDir * 256.0f, 255, 0, 0,
                          true, 0.1f);
    }
...
bool CNPC_Vortigaunt::CanFlinch(void) {
  if (IsActiveDynamicInteraction()) return false;

  if (IsPlayingGesture(ACT_GESTURE_RANGE_ATTACK1)) return false;

  if (IsCurSchedule(SCHED_VORTIGAUNT_DISPEL_ANTLIONS) ||
...
DEFINE_SCHEDULE(SCHED_VORTIGAUNT_RANGE_ATTACK,
                ...
                "	0"
                "		TASK_WAIT				"
                "		0.2"  // Wait a sec before killing beams
                ""
                "	Interrupts"
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_VORTIGAUNT_STAND"
    "		TASK_STOP_MOVING				0"
...
DEFINE_SCHEDULE(
    ...
    "ACTIVITY:ACT_IDLE"
    "		TASK_WAIT						"
    "	2"  // repick IDLESTAND every two seconds."
    ""
    "	Interrupts"
    "		COND_NEW_ENEMY"
    "		COND_LIGHT_DAMAGE"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_VORTIGAUNT_EXTRACT_BUGBAIT,
                ...
                "	128"  // Move within 128 of target ent (client)
                "		TASK_STOP_MOVING			"
                "		0"
                "		TASK_VORTIGAUNT_WAIT_FOR_PLAYER		0"
                "		TASK_SPEAK_SENTENCE			"
                "		500"  // Start extracting sentence
                ...
                "		TASK_SPEAK_SENTENCE			"
                "		501"  // Finish extracting sentence
                "		TASK_WAIT_FOR_SPEAK_FINISH		"
                "	1"
                "		TASK_WAIT				"
                "			2"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_VORTIGAUNT_FACE_PLAYER,
...
//=========================================================
DEFINE_SCHEDULE(SCHED_VORTIGAUNT_DISPEL_ANTLIONS,
...
BEGIN_DATADESC(CVortigauntChargeToken)
  DEFINE_FIELD(m_hTarget, FIELD_EHANDLE),
      ...
      DEFINE_ENTITYFUNC(SeekThink), DEFINE_ENTITYFUNC(SeekTouch),
END_DATADESC()
...
//-----------------------------------------------------------------------------
CVortigauntChargeToken *CVortigauntChargeToken::CreateChargeToken(
    const Vector &vecOrigin, CBaseEntity *pOwner, CBaseEntity *pTarget) {
  CVortigauntChargeToken *pToken =
      (CVortigauntChargeToken *)CreateEntityByName("vort_charge_token");
...
//-----------------------------------------------------------------------------
// Purpose: Creates an influence vector which causes the token to move away from
// obstructions
//-----------------------------------------------------------------------------
Vector CVortigauntChargeToken::GetSteerVector(const Vector &vecForward) {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecProbe, MASK_SHOT,
                 &filterSkip, &tr);
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vecProbe, MASK_SHOT, this,
...
void CVortigauntChargeToken::SeekThink(void) {
  ...
  vecOffset +=
      vecUp * cos(gpGlobals->curtime * 20.0f) * 200.0f * gpGlobals->frametime;
  vecOffset += vecRight * sin(gpGlobals->curtime * 15.0f) * 200.0f *
               gpGlobals->frametime;
...
void CVortigauntChargeToken::SeekTouch(CBaseEntity *pOther) {
  ...
  if (pPlayer->ArmorValue() < sk_vortigaunt_armor_charge.GetInt()) {
    pPlayer->IncrementArmorValue(sk_vortigaunt_armor_charge_per_token.GetInt() +
                                     random->RandomInt(-1, 1),
                                 sk_vortigaunt_armor_charge.GetInt());
  }
...
//			*pTarget - Who we're seeking towards
//-----------------------------------------------------------------------------
CVortigauntEffectDispel *CVortigauntEffectDispel::CreateEffectDispel(
    const Vector &vecOrigin, CBaseEntity *pOwner, CBaseEntity *pTarget) {
  CVortigauntEffectDispel *pToken =
      (CVortigauntEffectDispel *)CreateEntityByName("vort_effect_dispel");
  if (pToken == NULL) return NULL;
