...
#define ACTBUSY_COMBAT_PLAYER_MAX_DIST \
  720.0f  // NPCs in combat actbusy should try to stay within this distance of
          // the player.
...
class CActBusyAnimData : public CAutoGameSystem {
 public:
  ...
  // Purpose: Returns the index of the busyanim data for the specified activity
  // or sequence
...
// Inherited from IAutoServerSystem
//-----------------------------------------------------------------------------
void CActBusyAnimData::LevelInitPostEntity(void) { ParseAnimDataFile(); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void CActBusyAnimData::LevelShutdownPostEntity(void) { m_ActBusyAnims.Purge(); }
...
//-----------------------------------------------------------------------------
bool CActBusyAnimData::ParseActBusyFromKV(busyanim_t *pAnim,
                                          KeyValues *pSection) {
  pAnim->iszName = AllocPooledString(pSection->GetName());

  // Activities
  pAnim->iActivities[BA_BUSY] = (Activity)CAI_BaseNPC::GetActivityID(
      pSection->GetString("busy_anim", "ACT_INVALID"));
  pAnim->iActivities[BA_ENTRY] = (Activity)CAI_BaseNPC::GetActivityID(
      pSection->GetString("entry_anim", "ACT_INVALID"));
  pAnim->iActivities[BA_EXIT] = (Activity)CAI_BaseNPC::GetActivityID(
      pSection->GetString("exit_anim", "ACT_INVALID"));

  // Sequences
  pAnim->iszSequences[BA_BUSY] =
      AllocPooledString(pSection->GetString("busy_sequence", NULL));
  pAnim->iszSequences[BA_ENTRY] =
      AllocPooledString(pSection->GetString("entry_sequence", NULL));
  pAnim->iszSequences[BA_EXIT] =
      AllocPooledString(pSection->GetString("exit_sequence", NULL));

  // Sounds
  pAnim->iszSounds[BA_BUSY] =
      AllocPooledString(pSection->GetString("busy_sound", NULL));
  pAnim->iszSounds[BA_ENTRY] =
      AllocPooledString(pSection->GetString("entry_sound", NULL));
  pAnim->iszSounds[BA_EXIT] =
      AllocPooledString(pSection->GetString("exit_sound", NULL));
...
int CActBusyAnimData::FindBusyAnim(Activity iActivity, const char *pSequence) {
  ...
  for (int i = 0; i < iCount; i++) {
    ...
    if (pSequence && pBusyAnim->iszName != NULL_STRING &&
        !Q_stricmp(STRING(pBusyAnim->iszName), pSequence))
      return i;

    if (iActivity != ACT_INVALID &&
        pBusyAnim->iActivities[BA_BUSY] == iActivity)
      return i;
  }

  if (pSequence) {
    Warning(
        "Specified '%s' as a busy anim name, and it's not in the act busy anim "
        "list.\n",
        pSequence);
  } else if (iActivity != ACT_INVALID) {
    Warning(
        "Tried to use Activity %d as a busy anim, and it's not in the act busy "
        "anim list.\n",
        iActivity);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::Enable(CAI_ActBusyGoal *pGoal, float flRange,
                                 bool bVisibleOnly) {
  NotifyBusyEnding();
  ...
  m_flNextBusySearchTime =
      gpGlobals->curtime + ai_actbusy_search_time.GetFloat();
  m_flEndBusyAt = 0;
  m_bVisibleOnly = bVisibleOnly;
  m_bInQueue =
      dynamic_cast<CAI_ActBusyQueueGoal *>(m_hActBusyGoal.Get()) != NULL;
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::ForceActBusy(CAI_ActBusyGoal *pGoal,
                                       CAI_Hint *pHintNode, float flMaxTime,
                                       bool bVisibleOnly, bool bTeleportToBusy,
                                       bool bUseNearestBusy,
                                       CBaseEntity *pSeeEntity,
                                       Activity activity) {
  Assert(!m_bLeaving);

  if (m_bNeedsToPlayExitAnim) {
    // If we hit this, the mapmaker's told this NPC to actbusy somewhere while
    // it's still in an actbusy. Right now, we don't support this. We could
    // support it with a bit of work.
    if (HasAnimForActBusy(m_iCurrentBusyAnim, BA_EXIT)) {
      Warning(
          "ACTBUSY: %s(%s) was told to actbusy while inside an actbusy that "
          "needs to exit first. IGNORING.\n",
  ...
  if (ai_debug_actbusy.GetInt() == 4) {
    Msg("ACTBUSY: ForceActBusy on NPC %s (%s): ", GetOuter()->GetClassname(),
...
//-----------------------------------------------------------------------------
// Purpose: Break the NPC out of the current busy state, but don't disable
// busying
//-----------------------------------------------------------------------------
...
CAI_Hint *CAI_ActBusyBehavior::FindActBusyHintNode() {
  ...
  if (ai_debug_actbusy.GetInt() == 3 &&
      GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) {
  ...
  CAI_Hint *pNode = CAI_HintManager::FindHint(
      GetOuter(), HINT_WORLD_WORK_POSITION, iBits, m_flBusySearchRange);
...
CAI_Hint *CAI_ActBusyBehavior::FindCombatActBusyHintNode() {
  ...
  if (m_bVisibleOnly) iBits |= bits_HINT_NODE_VISIBLE;
  ...
  criteria.AddIncludePosition(pPlayer->GetAbsOrigin(),
                              ACTBUSY_COMBAT_PLAYER_MAX_DIST);
...
CAI_Hint *CAI_ActBusyBehavior::FindCombatActBusyTeleportHintNode() {
  ...
  criteria.SetFlag(iBits);
  criteria.AddIncludePosition(pPlayer->GetAbsOrigin(),
                              ACTBUSY_COMBAT_PLAYER_MAX_DIST * 1.1f);
...
bool CAI_ActBusyBehavior::FValidateHintType(CAI_Hint *pHint) {
  if (pHint->HintType() != HINT_WORLD_WORK_POSITION &&
      pHint->HintType() != HINT_NPC_EXIT_POINT)
    return false;
  ...
  // Check for clearance
  trace_t tr;
  AI_TraceHull(pHint->GetAbsOrigin(), pHint->GetAbsOrigin(),
               GetOuter()->WorldAlignMins(), GetOuter()->WorldAlignMaxs(),
               MASK_SOLID, GetOuter(), COLLISION_GROUP_NONE, &tr);
  ...
  if (ai_debug_actbusy.GetInt() == 3 &&
    ...
    NDebugOverlay::Box(pHint->GetAbsOrigin(), GetOuter()->WorldAlignMins(),
                       GetOuter()->WorldAlignMaxs(), 255, 0, 0, 8, 2.0);
  }
...
bool CAI_ActBusyBehavior::CanSelectSchedule(void) {
  ...
  if (m_bBusy || m_bForceActBusy || m_bNeedsToPlayExitAnim) return true;

  if (!m_bEnabled) return false;

  if (m_flDeferUntil > gpGlobals->curtime) return false;
  ...
  if (!IsCurScheduleOverridable()) return false;
...
//-----------------------------------------------------------------------------
// Purpose: Return true if the current schedule is one that ActBusy is allowed
// to override
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::IsCurScheduleOverridable(void) {
  if (IsCombatActBusy()) {
    // The whole point of a combat actbusy is that it can run in any state
    // (including combat) the only exception is SCRIPT (sjb)
    return (GetOuter()->GetState() != NPC_STATE_SCRIPT);
  }

  // Act busies are not valid inside of a vehicle
  if (GetOuter()->IsInAVehicle()) return false;
...
bool CAI_ActBusyBehavior::ShouldIgnoreSound(CSound *pSound) {
  // If we're busy in an actbusy anim that's an ambush, stay mum as long as we
  // can
  if (m_bBusy) {
    busyanim_t *pBusyAnim =
        g_ActBusyAnimDataSystem.GetBusyAnim(m_iCurrentBusyAnim);
    ...
    if (pBusyAnim && (pBusyAnim->iBusyInterruptType == BA_INT_AMBUSH) ||
        (pBusyAnim->iBusyInterruptType == BA_INT_COMBAT)) {
      /*
      ...
      */
      ...
      if (!GetOuter()->FVisible(pSound->GetSoundReactOrigin())) return true;
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::OnFriendDamaged(CBaseCombatCharacter *pSquadmate,
                                          CBaseEntity *pAttacker) {
  if (IsCombatActBusy() && pSquadmate->IsPlayer() && IsInSafeZone(pAttacker)) {
    SetCondition(
        COND_ACTBUSY_AWARE_OF_ENEMY_IN_SAFE_ZONE);  // Break the actbusy, if
                                                    // we're running it.
    m_flDeferUntil = gpGlobals->curtime +
                     4.0f;  // Stop actbusying and go deal with that enemy!!
  }
...
int CAI_ActBusyBehavior::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  if (IsCombatActBusy() && info.GetAttacker() &&
...
void CAI_ActBusyBehavior::GatherConditions(void) {
  // Clear this condition before we call up, since the look sensing code will
  // run and set this condition if it is relevant.
  ...
  if (m_hActBusyGoal &&
      m_hActBusyGoal->m_iSightMethod == ACTBUSY_SIGHT_METHOD_LOS_ONLY) {
    bCheckFOV = false;
  }

  // If we have a see entity, make sure we can still see it
  if (m_hSeeEntity && m_bBusy) {
    if ((!bCheckFOV || GetOuter()->FInViewCone(m_hSeeEntity)) &&
        GetOuter()->QuerySeeEntity(m_hSeeEntity) &&
        (!bCheckLOS || GetOuter()->FVisible(m_hSeeEntity))) {
      ...
      if (fDelta >= m_hActBusyGoal->m_flSeeEntityTimeout) {
        ...
        if (IsCombatActBusy() &&
            (GetOuter()->Classify() == CLASS_PLAYER_ALLY_VITAL &&
             m_hSeeEntity->IsPlayer())) {
          // Defer any actbusying for several seconds. This serves as a
          // heuristic for waiting for the player to settle after moving out of
          // the room. This helps Alyx pick a more pertinent Actbusy near the
  ...
  if (ai_debug_actbusy.GetInt() == 5) {
    ...
    for (int i = 0; i < m_SafeZones.Count(); i++) {
      ...
      NDebugOverlay::Box(vecBoxOrigin, vecBoxMins, vecBoxMaxs, 255, 0, 255, 64,
                         0.2f);
    }
  }
}
...
void CAI_ActBusyBehavior::CheckAndCleanupOnExit(void) {
  ...
  // If we're supposed to use render bounds while inside the busy anim, restore
  // normal now
...
void CAI_ActBusyBehavior::BuildScheduleTestBits(void) {
  ...
  if (IsCurSchedule(SCHED_ACTBUSY_START_BUSYING)) {
    if (GetOuter()->GetTask()->iTask == TASK_ACTBUSY_PLAY_ENTRY) return;

    GetOuter()->SetCustomInterruptCondition(COND_PROVOKED);

    if (IsCombatActBusy()) {
      GetOuter()->SetCustomInterruptCondition(
          GetClassScheduleIdSpace()->ConditionLocalToGlobal(
              COND_ACTBUSY_ENEMY_TOO_CLOSE));
    }
  }

  // If we're in a queue, or leaving, we have no extra conditions
  if (m_bInQueue || IsCurSchedule(SCHED_ACTBUSY_LEAVE)) return;

  // If we're not busy, or we're exiting a busy, we have no extra conditions
  if (!m_bBusy || IsCurSchedule(SCHED_ACTBUSY_STOP_BUSYING)) return;
...
int CAI_ActBusyBehavior::SelectScheduleForLeaving(void) {
    ...
    if (GetHintNode()->HintType() == HINT_NPC_EXIT_POINT) {
      ...
      if (UTIL_DistApprox(GetHintNode()->GetAbsOrigin(), GetAbsOrigin()) < 64) {
        if (!GetOuter()->IsMarkedForDeletion()) {
          ...
          GetOuter()->SetThink(
              &CBaseEntity::SUB_Remove);  // SUB_Remove) ;
                                          // //GetOuter()->SUB_Remove );
  ...
  hintCriteria.SetFlag(bits_HINT_NODE_RANDOM | bits_HINT_NODE_CLEAR |
                       bits_HINT_NODE_USE_GROUP);
  CAI_Hint *pNode = CAI_HintManager::FindHintRandom(
      GetOuter(), GetOuter()->GetAbsOrigin(), hintCriteria);
  if (pNode) {
    SetHintNode(pNode);
    return SCHED_ACTBUSY_LEAVE;
  }
...
int CAI_ActBusyBehavior::SelectScheduleWhileNotBusy(int iBase) {
  // Randomly act busy (unless we're being forced, in which case we should
  // search immediately)
  if (m_bForceActBusy || m_flNextBusySearchTime < gpGlobals->curtime) {
    ...
    if (m_bForceActBusy || IsCombatActBusy()) {
      ...
      m_flNextBusySearchTime =
          gpGlobals->curtime +
          RandomFloat(ai_actbusy_search_time.GetFloat(),
                      ai_actbusy_search_time.GetFloat() * 2);
    }
    ...
    if (!pNode) {
      if (IsCombatActBusy()) {
        if (m_hActBusyGoal->IsCombatActBusyTeleportAllowed() &&
            m_iNumConsecutivePathFailures >= 2 &&
            !AI_GetSinglePlayer()->FInViewCone(GetOuter())) {
          // Looks like I've tried several times to find a path to a valid hint
          // node and haven't been able to. This means I'm on a patch of node
          // graph that simply does not connect to any hint nodes that match my
          // criteria. So try to find a node that's safe to teleport to. (sjb)
          // ep2_outland_10 (Alyx) (Also, I must not be in the player's
          // viewcone)
    ...
    if (pNode) {
      ...
      if (cSpace) {
        ...
        Q_strncpy(sActOrSeqName, pSequenceOrActivity,
                  (cSpace - pSequenceOrActivity) + 1);
        iNodeActivity = (Activity)CAI_BaseNPC::GetActivityID(sActOrSeqName);
        iBusyAnim =
            g_ActBusyAnimDataSystem.FindBusyAnim(iNodeActivity, sActOrSeqName);
      } else {
        iNodeActivity =
            (Activity)CAI_BaseNPC::GetActivityID(pSequenceOrActivity);
        iBusyAnim = g_ActBusyAnimDataSystem.FindBusyAnim(iNodeActivity,
                                                         pSequenceOrActivity);
      }
      ...
      if (HasAnimForActBusy(iBusyAnim, BA_BUSY)) {
        if (HasCondition(COND_ACTBUSY_LOST_SEE_ENTITY)) {
          ...
          if (m_bForceActBusy) {
            // We were being told to act busy, which we can't do now that we've
            // lost the see entity. Abort, and assume that the mapmaker will
            // make us retry.
            StopBusying();
          }
          return iBase;
        ...
        if (m_iCurrentBusyAnim == -1) return iBase;
        ...
        if (GetHintNode() && GetHintNode()->Lock(GetOuter())) {
          if (ai_debug_actbusy.GetInt() == 2) {
            ...
            NDebugOverlay::Line(GetOuter()->WorldSpaceCenter(),
                                pNode->GetAbsOrigin(), 0, 255, 0, true, 5.0);
            NDebugOverlay::Box(
                pNode->GetAbsOrigin(), GetOuter()->WorldAlignMins(),
                GetOuter()->WorldAlignMaxs(), 0, 255, 0, 64, 5.0);
          }
          ...
          if (m_hActBusyGoal &&
              m_hActBusyGoal->m_iszSeeEntityName != NULL_STRING) {
            // Set the see entity Handle if we have one.
            m_hSeeEntity.Set(gEntList.FindEntityByName(
                NULL, m_hActBusyGoal->m_iszSeeEntityName));
          }
          ...
          if (m_bTeleportToBusy) {
            ...
            // We found a place to go, so teleport there and forget that we ever
            // had trouble.
            m_iNumConsecutivePathFailures = 0;
            return SCHED_ACTBUSY_TELEPORT_TO_BUSY;
          }
...
int CAI_ActBusyBehavior::SelectScheduleWhileBusy(void) {
  // Are we supposed to stop on our current actbusy, but stay in the actbusy
  // state?
  if (!ActBusyNodeStillActive() ||
      (m_flEndBusyAt && gpGlobals->curtime >= m_flEndBusyAt)) {
    if (ai_debug_actbusy.GetInt() == 4) {
  ...
  if (IsCombatActBusy() &&
      (HasCondition(COND_ACTBUSY_AWARE_OF_ENEMY_IN_SAFE_ZONE) ||
       HasCondition(COND_ACTBUSY_ENEMY_TOO_CLOSE))) {
    return SCHED_ACTBUSY_STOP_BUSYING;
  }
...
int CAI_ActBusyBehavior::SelectSchedule() {
  ...
  if (!IsCombatActBusy() && !m_bForceActBusy && iBase != SCHED_IDLE_STAND) {
    ...
    if (m_bBusy) return SCHED_ACTBUSY_STOP_BUSYING;
  ...
  if (m_bLeaving) return SelectScheduleForLeaving();

  // NPCs should not be busy if the actbusy behaviour has been disabled, or if
  // they've received player squad commands
  bool bShouldNotBeBusy =
  ...
  if (bShouldNotBeBusy) {
    ...
    if (m_bBusy) return SelectScheduleWhileBusy();
...
bool CAI_ActBusyBehavior::CanFlinch(void) {
  if (m_bNeedsToPlayExitAnim) return false;
...
bool CAI_ActBusyBehavior::QueryHearSound(CSound *pSound) {
  ...
  if (pSound->IsSoundType(SOUND_COMBAT) ||
      pSound->IsSoundType(SOUND_BULLET_IMPACT)) {
    if (GetOuter()->IRelationType(pSound->m_hOwner) == D_LI) return false;
  }
...
void CAI_ActBusyBehavior::OnSeeEntity(CBaseEntity *pEntity) {
  ...
  if (IsCombatActBusy() && GetOuter()->IRelationType(pEntity) < D_LI) {
    if (pEntity->GetAbsOrigin().DistToSqr(GetAbsOrigin()) <=
        ACTBUSY_ENEMY_TOO_CLOSE_DIST_SQR) {
...
bool CAI_ActBusyBehavior::ShouldPlayerAvoid(void) {
  if (IsCombatActBusy()) {
    ...
    // animation. She isn't allowed to shove you around while she's running
    // around
    if (IsCurSchedule(SCHED_ACTBUSY_START_BUSYING)) {
    ...
    if (IsCurSchedule(SCHED_ACTBUSY_START_BUSYING)) {
      if (GetCurTask() && GetCurTask()->iTask == TASK_WAIT_FOR_MOVEMENT ||
          GetOuter()->GetTask()->iTask == TASK_ACTBUSY_PLAY_ENTRY)
        return true;
    } else if (IsCurSchedule(SCHED_ACTBUSY_STOP_BUSYING)) {
      if (GetCurTask() && GetCurTask()->iTask == TASK_ACTBUSY_PLAY_EXIT)
        return true;
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the current NPC is acting busy, or moving to an
// actbusy
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::CollectSafeZoneVolumes(
    CAI_ActBusyGoal *pActBusyGoal) {
  ...
  if (pActBusyGoal->m_iszSafeZoneVolume != NULL_STRING) {
    CBaseEntity *pVolume =
        gEntList.FindEntityByName(NULL, pActBusyGoal->m_iszSafeZoneVolume);

    while (pVolume != NULL) {
      busysafezone_t newSafeZone;
      pVolume->CollisionProp()->WorldSpaceAABB(&newSafeZone.vecMins,
                                               &newSafeZone.vecMaxs);
      m_SafeZones.AddToTail(newSafeZone);
      pVolume =
          gEntList.FindEntityByName(pVolume, pActBusyGoal->m_iszSafeZoneVolume);
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Return true if this NPC has the anims required to use the specified
// actbusy hint
//-----------------------------------------------------------------------------
bool CAI_ActBusyBehavior::HasAnimForActBusy(int iActBusy,
                                            busyanimparts_t AnimPart) {
  if (iActBusy == -1) return false;

  busyanim_t *pBusyAnim = g_ActBusyAnimDataSystem.GetBusyAnim(iActBusy);
  if (!pBusyAnim) return false;

  // Try and play the sequence first
  if (pBusyAnim->iszSequences[AnimPart] != NULL_STRING)
    return (GetOuter()->LookupSequence((char *)STRING(
                pBusyAnim->iszSequences[AnimPart])) != ACTIVITY_NOT_AVAILABLE);

  // Try and play the activity second
  if (pBusyAnim->iActivities[AnimPart] != ACT_INVALID)
    return GetOuter()->HaveSequenceForActivity(
        pBusyAnim->iActivities[AnimPart]);

  return false;
}

//-----------------------------------------------------------------------------
// Purpose: Play the sound associated with the specified part of the current
// actbusy, if any
//-----------------------------------------------------------------------------
void CAI_ActBusyBehavior::PlaySoundForActBusy(busyanimparts_t AnimPart) {
  ...
  if (pBusyAnim->iszSounds[AnimPart] != NULL_STRING) {
    ...
    if (GetOuter()->GetParametersForSound(
            STRING(pBusyAnim->iszSounds[AnimPart]), params,
            STRING(GetOuter()->GetModelName()))) {
...
bool CAI_ActBusyBehavior::PlayAnimForActBusy(busyanimparts_t AnimPart) {
  ...
  // Try and play the sequence first
  if (pBusyAnim->iszSequences[AnimPart] != NULL_STRING) {
    GetOuter()->SetSequenceByName(
        (char *)STRING(pBusyAnim->iszSequences[AnimPart]));
...
void CAI_ActBusyBehavior::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_ACTBUSY_PLAY_BUSY_ANIM: {
      // If we're not enabled here, it's due to the actbusy being deactivated
      // during the NPC's entry animation. We can't abort in the middle of the
      // entry, so we arrive here with a disabled actbusy behaviour. Exit
      // gracefully.
      if (!m_bEnabled) {
        TaskComplete();
        return;
      ...
      // Set the flag to remind the code to recompute the NPC's box from render
      // bounds. This is used to delay the process so that we don't get a box
      // built from render bounds when a character is still interpolating to
      // their busy pose.
      ...
      // be true if there was no entry animation for this busy. We do it this
      // way because the entry code contains this same check, and so we assume
      // we're valid even if we're off now, because some entry animations move
      // the character off the node.
      if (m_bMovingToBusy) {
        if (UTIL_DistApprox(GetHintNode()->GetAbsOrigin(), GetAbsOrigin()) >
                16 ||
            !GetOuter()->FacingIdeal()) {
      ...
      // If we're supposed to use render bounds while inside the busy anim, do
      // so
      if (m_bUseRenderBoundsForCollision) {
    ...
    case TASK_ACTBUSY_PLAY_ENTRY: {
      ...
      // Fail if we're not on the node & facing the correct way
      if (UTIL_DistApprox(GetHintNode()->GetAbsOrigin(), GetAbsOrigin()) > 16 ||
          !GetOuter()->FacingIdeal()) {
        m_bBusy = false;
        TaskFail("Not correctly on hintnode");
        return;
      ...
      // Play the entry animation. If it fails, we don't have an entry anim, so
    ...
    case TASK_ACTBUSY_VERIFY_EXIT: {
      // NPC's that changed their bounding box must ensure that they can restore
      // their regular box before they exit their actbusy. This task is designed
      // to delay until that time if necessary.
      if (!m_bUseRenderBoundsForCollision) {
    ...
    case TASK_ACTBUSY_PLAY_EXIT: {
      // If we're supposed to use render bounds while inside the busy anim,
      // restore normal now
      if (m_bUseRenderBoundsForCollision) {
        GetOuter()->SetHullSizeNormal(true);
      }
      ...
      // Play the exit animation. If it fails, we don't have an entry anim, so
      // complete immediately.
    ...
    case TASK_ACTBUSY_GET_PATH_TO_ACTBUSY: {
      ...
      if (!HasCondition(COND_TASK_FAILED)) {
        ...
        // Set the arrival sequence for the actbusy to be the busy sequence, if
        // we don't have an entry animation
        busyanim_t *pBusyAnim =
            g_ActBusyAnimDataSystem.GetBusyAnim(m_iCurrentBusyAnim);
        if (pBusyAnim && pBusyAnim->iszSequences[BA_ENTRY] == NULL_STRING &&
            pBusyAnim->iActivities[BA_ENTRY] == ACT_INVALID) {
          // Try and play the sequence first
          if (pBusyAnim->iszSequences[BA_BUSY] != NULL_STRING) {
            GetNavigator()->SetArrivalSequence(GetOuter()->LookupSequence(
                STRING(pBusyAnim->iszSequences[BA_BUSY])));
          } else if (pBusyAnim->iActivities[BA_BUSY] != ACT_INVALID) {
            // Try and play the activity second
            GetNavigator()->SetArrivalActivity(pBusyAnim->iActivities[BA_BUSY]);
          }
        } else {
          // Robin: Set the arrival sequence / activity to be the entry
          // animation.
          if (pBusyAnim->iszSequences[BA_ENTRY] != NULL_STRING) {
            GetNavigator()->SetArrivalSequence(GetOuter()->LookupSequence(
                STRING(pBusyAnim->iszSequences[BA_ENTRY])));
          } else if (pBusyAnim->iActivities[BA_ENTRY] != ACT_INVALID) {
            // Try and play the activity second
            GetNavigator()->SetArrivalActivity(
                pBusyAnim->iActivities[BA_ENTRY]);
          }
        }
      } else {
        ...
        if (ai_debug_actbusy.GetInt() == 1) {
          if (GetHintNode()) {
            // Show which actbusy we're moving towards
            NDebugOverlay::Line(GetOuter()->WorldSpaceCenter(),
                                GetHintNode()->GetAbsOrigin(), 255, 0, 0, true,
                                1.0);
          }
        }
      }
  ...
  switch (pTask->iTask) {
    ...
    case TASK_ACTBUSY_PLAY_BUSY_ANIM: {
      ...
      if (IsCombatActBusy()) {
        if (GetEnemy() != NULL && !HasCondition(COND_ENEMY_OCCLUDED)) {
          ...
          // I'll probably go to hell for not doing this with conditions like I
          // should. (sjb)
          float flDistSqr =
              GetAbsOrigin().DistToSqr(GetEnemy()->GetAbsOrigin());
    ...
    case TASK_ACTBUSY_VERIFY_EXIT: {
      if (GetOuter()->IsWaitFinished()) {
        // Trace my normal hull over this spot to see if I'm able to stand up
        // right now.
        trace_t tr;
        CTraceFilterOnlyNPCsAndPlayer filter(GetOuter(), COLLISION_GROUP_NONE);
        UTIL_TraceHull(GetOuter()->GetAbsOrigin(), GetOuter()->GetAbsOrigin(),
                       NAI_Hull::Mins(HULL_HUMAN), NAI_Hull::Maxs(HULL_HUMAN),
        ...
        if (tr.startsolid) {
          ...
          // Put an entity blocker here for a moment until I get into my
          // bounding box.
          CBaseEntity *pBlocker = CEntityBlocker::Create(
              GetOuter()->GetAbsOrigin(), NAI_Hull::Mins(HULL_HUMAN),
              NAI_Hull::Maxs(HULL_HUMAN), GetOuter(), true);
...
void CAI_ActBusyBehavior::NotifyBusyEnding(void) {
  ...
  if (GetHintNode()) {
    ...
    if (IsCombatActBusy()) {
      // Don't allow anyone to use this node for a bit. This is so the tactical
      // position doesn't get re-occupied the moment I leave it.
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_ACTBUSY_BUSY"
    ""
    "	Interrupts"
    "		COND_ACTBUSY_LOST_SEE_ENTITY")

DEFINE_SCHEDULE(SCHED_ACTBUSY_BUSY,
...
DEFINE_SCHEDULE(SCHED_ACTBUSY_TELEPORT_TO_BUSY,
...
BEGIN_DATADESC(CAI_ActBusyGoal)
  DEFINE_KEYFIELD(m_flBusySearchRange, FIELD_FLOAT, "busysearchrange"),
      ...
      DEFINE_KEYFIELD(m_bAllowCombatActBusyTeleport, FIELD_BOOLEAN,
                      "allowteleport"),
      ...
      // Inputs
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetBusySearchRange",
                       InputSetBusySearchRange),
      DEFINE_INPUTFUNC(FIELD_STRING, "ForceNPCToActBusy",
                       InputForceNPCToActBusy),
      DEFINE_INPUTFUNC(FIELD_EHANDLE, "ForceThisNPCToActBusy",
                       InputForceThisNPCToActBusy),
      DEFINE_INPUTFUNC(FIELD_EHANDLE, "ForceThisNPCToLeave",
...
// Purpose:
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CAI_ActBusyGoal::GetBusyBehaviorForNPC(
    CBaseEntity *pEntity, const char *sInputName) {
  CAI_BaseNPC *pActor = dynamic_cast<CAI_BaseNPC *>(pEntity);
  if (!pActor) {
    Msg("ai_goal_actbusy input %s fired targeting an entity that isn't an "
        "NPC.\n",
        sInputName);
    return NULL;
  }
  ...
  if (!pActor->GetBehavior(&pBehavior)) {
    Msg("ai_goal_actbusy input %s fired on an NPC that doesn't support ActBusy "
        "behavior.\n",
        sInputName);
    return NULL;
  }
...
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CAI_ActBusyGoal::GetBusyBehaviorForNPC(
    const char *pszActorName, CBaseEntity *pActivator, CBaseEntity *pCaller,
    const char *sInputName) {
  CBaseEntity *pEntity = gEntList.FindEntityByName(
      NULL, MAKE_STRING(pszActorName), NULL, pActivator, pCaller);
  if (!pEntity) {
    Msg("ai_goal_actbusy input %s fired targeting a non-existant entity "
        "(%s).\n",
...
void CAI_ActBusyGoal::EnableGoal(CAI_BaseNPC *pAI) {
  ...
  if (pAI->GetBehavior(&pBehavior)) {
    ...
    DevMsg(
        "ActBusy goal entity activated for an NPC (%s) that doesn't have the "
        "ActBusy behavior\n",
        pAI->GetDebugName());
    return;
  }
  ...
  for (int i = 0; i < NumActors(); i++) {
    ...
    if (pActor) {
      ...
      if (pActor->GetBehavior(&pBehavior)) {
        ...
        DevMsg(
            "ActBusy goal entity deactivated for an NPC that doesn't have the "
            "ActBusy behavior\n");
        return;
      }
    }
...
void CAI_ActBusyGoal::InputForceNPCToActBusy(inputdata_t &inputdata) {
  ...
  CAI_ActBusyBehavior *pBehavior =
      GetBusyBehaviorForNPC(pszParam, inputdata.pActivator, inputdata.pCaller,
                            "InputForceNPCToActBusy");
  if (!pBehavior) return;
  ...
  if (pszParam) {
    ...
    CBaseEntity *pEntity = gEntList.FindEntityByName(
        NULL, pszParam, NULL, inputdata.pActivator, inputdata.pCaller);
    if (pEntity) {
      pHintNode = dynamic_cast<CAI_Hint *>(pEntity);
      if (!pHintNode) {
        Msg("ai_goal_actbusy input ForceNPCToActBusy fired targeting an entity "
            "that isn't a hintnode.\n",
            pszParam);
        return;
      }
  ...
  while (pszParam) {
    ...
    if (!Q_strncmp(pszParam, "teleport", 8)) {
      ...
      if (activity == ACT_INVALID) {
        ...
        pBehavior->GetOuter()->m_iszSceneCustomMoveSeq =
            AllocPooledString(pAnimName);
  ...
  if (ai_debug_actbusy.GetInt() == 4) {
    Msg("ACTBUSY: Actbusy goal %s (%s) ForceNPCToActBusy input with data: "
        "%s.\n",
  ...
  pBehavior->SetBusySearchRange(m_flBusySearchRange);
  pBehavior->ForceActBusy(this, pHintNode, flMaxTime, m_bVisibleOnly, bTeleport,
                          bUseNearestBusy, pSeeEntity, activity);
}
...
void CAI_ActBusyGoal::InputForceThisNPCToActBusy(inputdata_t &inputdata) {
  CAI_ActBusyBehavior *pBehavior = GetBusyBehaviorForNPC(
      inputdata.value.Entity(), "InputForceThisNPCToActBusy");
  if (!pBehavior) return;
...
void CAI_ActBusyGoal::InputForceThisNPCToLeave(inputdata_t &inputdata) {
  CAI_ActBusyBehavior *pBehavior = GetBusyBehaviorForNPC(
      inputdata.value.Entity(), "InputForceThisNPCToLeave");
  if (!pBehavior) return;
...
BEGIN_DATADESC(CAI_ActBusyQueueGoal)
  ...
  DEFINE_FIELD(m_iCurrentQueueCount, FIELD_INTEGER),
      ...
      // DEFINE_ARRAY( m_iszNodes, FIELD_STRING, MAX_QUEUE_NODES ), // Silence
      // Classcheck!
      ...
      // Inputs
      DEFINE_INPUTFUNC(FIELD_INTEGER, "PlayerStartedBlocking",
                       InputPlayerStartedBlocking),
      DEFINE_INPUTFUNC(FIELD_INTEGER, "PlayerStoppedBlocking",
      ...
      DEFINE_THINKFUNC(QueueThink), DEFINE_THINKFUNC(MoveQueueUpThink),
...
void CAI_ActBusyQueueGoal::InputActivate(inputdata_t &inputdata) {
  if (!IsActive()) {
    ...
    SetContextThink(&CAI_ActBusyQueueGoal::QueueThink, gpGlobals->curtime + 5,
                    QUEUE_THINK_CONTEXT);
  }
...
void CAI_ActBusyQueueGoal::RecalculateQueueCount(void) {
  ...
  if (iCount == m_iCurrentQueueCount) return;
  ...
  for (int i = 0; i < MAX_QUEUE_NODES; i++) {
    if (m_hNodes[i]) {
      ...
      if (i > iCount) {
        ...
        // To prevent NPCs outside the queue moving directly to nodes within the
        // queue, only have the entry node be a valid actbusy node.
        if (i == iCount) {
...
void CAI_ActBusyQueueGoal::InputPlayerStartedBlocking(inputdata_t &inputdata) {
  ...
  /*
  // First, find all NPCs heading to points in front of the player's blocked
  spot for ( int i = 0; i < iNode; i++ )
  {
          ...
          {
                  // We may be ahead of the player in the queue, which means we
  can safely
                  // be left alone to reach the node. Make sure we're not closer
  to it than the player is float flPlayerDistToNode =
  (inputdata.pActivator->GetAbsOrigin() -
  m_hNodes[i]->GetAbsOrigin()).LengthSqr(); if ( (pNPC->GetAbsOrigin() -
  m_hNodes[i]->GetAbsOrigin()).LengthSqr() < flPlayerDistToNode ) continue;

                  // We're an NPC heading to a node past the player, and yet the
  player's in our way. pBehavior->StopBusying();
          }
  }
  */
...
//-----------------------------------------------------------------------------
// Purpose: Find a node back in the queue to move to, and push all NPCs beyond
// that backwards
//-----------------------------------------------------------------------------
void CAI_ActBusyQueueGoal::PushNPCBackInQueue(CAI_BaseNPC *pNPC,
                                              int iStartingNode) {
  // Push this guy back, and tell everyone behind him to move back too, until we
  // find a gap
  while (pNPC) {
    CAI_ActBusyBehavior *pBehavior = GetQueueBehaviorForNPC(pNPC);
    ...
    for (int iNext = iStartingNode + 1; iNext < MAX_QUEUE_NODES; iNext++) {
      if (!m_bPlayerBlockedNodes[iNext]) {
        ...
        if (pTargetNPC) {
          CAI_ActBusyBehavior *pTargetBehavior =
              GetQueueBehaviorForNPC(pTargetNPC);
...
void CAI_ActBusyQueueGoal::InputMoveQueueUp(inputdata_t &inputdata) {
  ...
  for (int i = 0; i < MAX_QUEUE_NODES; i++) {
    ...
    if (pNPC) {
      ...
      if (!pBehavior->IsMovingToBusy() || !m_bForceReachFront) break;
...
void CAI_ActBusyQueueGoal::MoveQueueUpThink(void) {
  ...
  for (int iEmptyNode = 0; iEmptyNode < (MAX_QUEUE_NODES - 1); iEmptyNode++) {
    if (!NodeIsOccupied(iEmptyNode) && !m_bPlayerBlockedNodes[iEmptyNode]) {
      // Look for NPCs farther down the queue, but not on the other side of a
      // player
      for (int iNext = iEmptyNode + 1; iNext < MAX_QUEUE_NODES; iNext++) {
        // Is the player blocking this node? If so, we're done
        if (m_bPlayerBlockedNodes[iNext]) break;
...
void CAI_ActBusyQueueGoal::RemoveNPCFromQueue(CAI_BaseNPC *pNPC) {
  ...
  // Find the node the NPC was heading to, and tell the guy behind him to move
  // forward
  MoveQueueUp();
}
...
//-----------------------------------------------------------------------------
CAI_ActBusyBehavior *CAI_ActBusyQueueGoal::GetQueueBehaviorForNPC(
    CAI_BaseNPC *pNPC) {
  CAI_ActBusyBehavior *pBehavior;
