...
int CAI_FuncTankBehavior::SelectSchedule() {
  ...
  if (HasCondition(COND_FUNCTANK_DISMOUNT) ||
      m_hFuncTank->GetAmmoCount() == 0) {
    if (m_bMounted) {
      Dismount();
    }
  ...
  if (GetEnemy()) {
    // Tell the func tank whenever we see the player for the first time since
    // not seeing him for a while
    if (HasCondition(COND_NEW_ENEMY) && GetEnemy()->IsPlayer() &&
...
Activity CAI_FuncTankBehavior::NPC_TranslateActivity(Activity activity) {
  ...
  if (m_bMounted) return ACT_IDLE_MANNEDGUN;
...
void CAI_FuncTankBehavior::Dismount(void) {
  ...
  if (m_hFuncTank) {
    ...
    Assert(m_hFuncTank->IsMarkedForDeletion() ||
           m_hFuncTank->GetController() == GetOuter());

    m_hFuncTank->NPC_SetInRoute(false);
    if (m_hFuncTank->GetController() == GetOuter()) m_hFuncTank->StopControl();
    SetFuncTank(NULL);
  }
...
void CAI_FuncTankBehavior::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_FACE_FUNCTANK: {
      ...
      if (fabs(vecDelta.x) > 16 || fabs(vecDelta.y) > 16 ||
          fabs(vecDelta.z) > 48) {
    ...
    case TASK_HOLSTER_WEAPON: {
      ...
      if (GetOuter()->IsWeaponHolstered() || !GetOuter()->CanHolsterWeapon()) {
        ...
        // We are at the correct position and facing for the func_tank, mount
        // it.
    ...
    case TASK_SCAN_LEFT_FUNCTANK: {
      ...
      QAngle vecTargetAngles( flCenterPitch + flPitch, UTIL_AngleMod(
      flCenterYaw + flScanAmount ), 0 );
      */
  ...
  switch (pTask->iTask) {
    ...
    case TASK_FIRE_FUNCTANK: {
      ...
      if (m_hFuncTank->NPC_HasEnemy()) {
        ...
        // The NPC may have decided to stop using the func_tank, because it's
        // out of ammo.
        if (!m_hFuncTank) {
          TaskComplete();
          break;
...
CBaseEntity *CAI_FuncTankBehavior::BestEnemy(void) {
  ...
  if (!m_hFuncTank || !IsMounted()) return BaseClass::BestEnemy();
  ...
  for (AI_EnemyInfo_t *pEMemory = GetEnemies()->GetFirst(&iter);
    ...
    if ((pEnemy->GetFlags() & FL_NOTARGET) ||
        (pNPC->IRelationType(pEnemy) != D_HT &&
         pNPC->IRelationType(pEnemy) != D_FR) ||
        !IsValidEnemy(pEnemy))
      continue;

    if (pEMemory->timeLastSeen < pNPC->GetAcceptableTimeSeenEnemy()) continue;

    if (pEMemory->timeValidEnemy > gpGlobals->curtime) continue;

    // Skip enemies that have eluded me to prevent infinite loops
    if (GetEnemies()->HasEludedMe(pEnemy)) continue;
    ...
    if (!bBestUnreachable && bUnreachable) continue;

    //  If best is unreachable and current is reachable, always pick the current
    //  regardless of priority
    if (bBestUnreachable && !bUnreachable) {
      bBestSeen =
          (pNPC->GetSenses()->DidSeeEntity(pEnemy) ||
           pNPC->FVisible(pEnemy));  // @TODO (toml 04-02-03): Need to optimize
                                     // CanSeeEntity() so multiple calls in
                                     // frame do not recalculate, rather cache
    ...
    // If both are unreachable or both are reachable, chose enemy based on
    // priority and distance
      ...
      if (bCloser || !bBestSeen) {
        // @TODO (toml 04-02-03): Need to optimize FVisible() so multiple calls
        // in frame do not recalculate, rather cache
        bool fSeen =
            (pNPC->GetSenses()->DidSeeEntity(pEnemy) || pNPC->FVisible(pEnemy));
        if ((bCloser && (fSeen || !bBestSeen)) ||
            (!bCloser && !bBestSeen && fSeen)) {
          bBestSeen = fSeen;
          iBestDistSq = iDistSq;
          iBestPriority = pNPC->IRelationPriority(pEnemy);
          pBestEnemy = pEnemy;
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE		SCHEDULE: "
...
DEFINE_SCHEDULE(SCHED_FAIL_MOVE_TO_FUNCTANK,
