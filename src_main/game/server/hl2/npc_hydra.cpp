...
enum {
  ...
  SCHED_HYDRA_TIGHTEN_SLACK,  // snagged on something, tighten slack up to
                              // obstacle and try again from there
  SCHED_HYDRA_RETREAT,
  SCHED_HYDRA_THROW,
...
SendPropVector(SENDINFO_NETWORKARRAYELEM(m_vecChain, 0), -1, SPROP_COORD),
    ...
    SendPropFloat(SENDINFO(m_flRelaxedLength), 12, 0, 0.0,
                  HYDRA_MAX_LENGTH * 1.5),
    END_SEND_TABLE()
...
()
    ...
    DEFINE_FIELD(bStuck, FIELD_BOOLEAN), DEFINE_FIELD(bOnFire, FIELD_BOOLEAN),
...
()
    ...
    static ConVar
    sv_hydraLength("hydra_length", "100", FCVAR_ARCHIVE, "Hydra Length");
static ConVar sv_hydraSlack("hydra_slack", "200", FCVAR_ARCHIVE, "Hydra Slack");

static ConVar sv_hydraSegmentLength("hydra_segment_length", "30", FCVAR_ARCHIVE,
                                    "Hydra Slack");

static ConVar sv_hydraTest("hydra_test", "1", FCVAR_ARCHIVE, "Hydra Slack");

static ConVar sv_hydraBendTension("hydra_bend_tension", "0.4", FCVAR_ARCHIVE,
                                  "Hydra Slack");
static ConVar sv_hydraBendDelta("hydra_bend_delta", "50", FCVAR_ARCHIVE,
                                "Hydra Slack");

static ConVar sv_hydraGoalTension("hydra_goal_tension", "0.5", FCVAR_ARCHIVE,
                                  "Hydra Slack");
static ConVar sv_hydraGoalDelta("hydra_goal_delta", "400", FCVAR_ARCHIVE,
                                "Hydra Slack");

static ConVar sv_hydraMomentum("hydra_momentum", "0.5", FCVAR_ARCHIVE,
                               "Hydra Slack");

static ConVar sv_hydraTestSpike("sv_hydraTestSpike", "1", 0,
                                "Hydra Test impaling code");
...
void CNPC_Hydra::Activate(void) {
  ...
  m_pExtendTentacleSound =
      controller.SoundCreate(filter, entindex(), "NPC_Hydra.ExtendTentacle");
...
//-----------------------------------------------------------------------------
Class_T CNPC_Hydra::Classify(void) { return CLASS_BARNACLE; }
...
void CNPC_Hydra::Spawn() {
  ...
  ClearEffects();
  m_iHealth = 20;
  m_flFieldOfView = -1.0;  // indicates the width of this NPC's forward view
...
void CNPC_Hydra::RunAI(void) {
  ...
  // NDebugOverlay::Box(m_body[i].vecPos, Vector( -2, -2, -2 ), Vector( 2, 2, 2
  // ), 0, 255, 0, 20, .1); NDebugOverlay::Box( m_vecHeadGoal, Vector( -2, -2,
  // -2
  // ), Vector( 2, 2, 2 ), 255, 255, 0, 20, .1);
  for (; i < CHAIN_LINKS; i++) {
    m_vecChain.Set(i, m_vecChain[i - 1]);
  }
}
...
Vector CNPC_Hydra::TestPosition(float t) {
  // return GetAbsOrigin( ) + Vector( sin( (m_seed + t) * 2.3 ) * 15, cos(
  // (m_seed + t) * 2.4 ) * 150, sin( ( m_seed + t ) * 1.8 ) * 50 ) +
  // m_vecOutward * sv_hydraLength.GetFloat();;
  t = (int)(t * 0.2);
#if 1
  Vector tmp =
      Vector(sin((m_seed + t) * 0.8) * 15, cos((m_seed + t) * 0.9) * 150,
             sin((m_seed + t) * 0.4) * 50);
  tmp += Vector(sin((m_seed + t) * 1.0) * 4, cos((m_seed + t) * 0.9) * 4,
                sin((m_seed + t) * 1.1) * 6);
  ...
  return tmp;
#endif
  // m_vecHeadGoal = GetAbsOrigin( ) + Vector( sin( gpGlobals->curtime * 0.3 ) *
  // 15, cos( gpGlobals->curtime * 0.4 ) * 150, sin( gpGlobals->curtime * 0.2 )
  // * 50 + dt );
}

//-----------------------------------------------------------------------------
// Purpose: Calculate the bone forces based on goal positions, bending rules,
// stretching rules, etc. Input  : Output :
//-----------------------------------------------------------------------------
...
void CNPC_Hydra::CalcGoalForces() {
  ...
  m_body[iLast].vecGoalPos =
      m_vecHeadGoal;  // + m_vecHeadDir * m_body[iLast-1].flActualLength;
  m_body[iLast].flGoalInfluence = m_flHeadGoalInfluence;

  m_body[iLast - 1].vecGoalPos =
      m_vecHeadGoal - m_vecHeadDir * m_idealSegmentLength;
  ...
  float flGoalSegmentLength =
      m_idealSegmentLength * (m_idealLength / m_flCurrentLength);
  ...
  for (i = iFirst; i <= iLast; i++) {
    ...
    if (flInfluence > 0) {
      ...
      // NDebugOverlay::Box(m_body[i].vecGoalPos, Vector( -2, -2, -2 ), Vector(
      // 2, 2, 2 ), 255, 255, 0, flInfluence * 255, .1);
    }
  }
#endif
  ...
  for (i = iFirst - 1; i <= iLast - 1; i++) {
    ...
    // NDebugOverlay::Line( m_body[i].vecPos + v3 * flGoalSegmentLength,
    // m_body[i].vecPos - v3 * flGoalSegmentLength, 255, 0, 0, true, .1);

    if (i + 1 <= iLast) {
      // towards head
      delta =
          (m_body[i].vecPos + v3 * flGoalSegmentLength - m_body[i + 1].vecPos) *
          sv_hydraBendTension.GetFloat();
      length = delta.Length();
      if (length > sv_hydraBendDelta.GetFloat()) {
        delta = delta * (sv_hydraBendDelta.GetFloat() / length);
      }
      m_body[i + 1].vecDelta += delta;
      // NDebugOverlay::Line( m_body[i+1].vecPos, m_body[i+1].vecPos + delta,
      // 255, 0, 0, true, .1);
    }
  ...
  while (i > iFirst && maxChecks > 0) {
    ...
    Vector stretch = (m_body[i].vecPos + m_body[i].vecDelta) -
                     (m_body[i - 1].vecPos + m_body[i - 1].vecDelta);
...
void CNPC_Hydra::MoveBody() {
  ...
  for (i = iFirst; i <= iLast; i++) {
    ...
    AI_TraceHull(m_body[i].vecPos, m_body[i].vecPos + m_body[i].vecDelta,
                 Vector(-2, -2, -2), Vector(2, 2, 2), MASK_NPCSOLID, this,
    ...
    if (tr.fraction != 1.0) {
      ...
      // NDebugOverlay::Line( m_body[i].vecPos, tr.endpos, 255, 255, 0, true,
      // 1);
    ...
    AI_TraceHull(direct, m_body[i - 1].vecPos, Vector(-2, -2, -2),
                 Vector(2, 2, 2), MASK_NPCSOLID, this, COLLISION_GROUP_NONE,
                 &tr);
    ...
    if (tr.fraction == 1.0) {
      ...
      if (tr.fraction == 1.0) {
        ...
        // TODO(d.rattman): compute nudge force
        m_body[i].bStuck = true;
...
void CNPC_Hydra::Nudge(CBaseEntity *pOther, const Vector &vecContact,
                       const Vector &vecSpeed) {
...
void CNPC_Hydra::Stab(CBaseEntity *pOther, const Vector &vecSpeed,
                      trace_t &tr) {
...
void CNPC_Hydra::Kick(CBaseEntity *pHitEntity, const Vector &vecContact,
                      const Vector &vecSpeed) {}
...
void CNPC_Hydra::CheckLength() {
  ...
  for (i = 1; i < m_body.Count() - 1; i++) {
    ...
    if (length > m_idealSegmentLength * 3.0 &&
        (m_body[i].bStuck || m_body[i + 1].bStuck)) {
      // NDebugOverlay::Line( m_body[i].vecPos, m_body[i+1].vecPos, 255, 0, 0,
      // true, 1.0);
  ...
  if (m_flCurrentLength > HYDRA_MAX_LENGTH) {
...
void CNPC_Hydra::AdjustLength() {
  ...
  // m_idealLength, m_idealSegmentLength * (m_body.Count() - 3) );
  ...
  if (m_flCurrentLength < m_idealLength) {
    ...
    if (m_flRelaxedLength - m_idealSegmentLength * 0.5 > m_idealLength ||
        HasCondition(COND_HYDRA_SNAGGED)) {
      bShouldAdjust = true;
      if (!ContractFromRoot()) {
  ...
  if (!bAdjustFailed) {
    ...
    // m_pExtendTentacleSound, 0.0, 0.3 );
  }
...
bool CNPC_Hydra::ContractFromHead() {
  ...
  if (m_body[iNode].bStuck &&
      m_body[iNode - 1].flActualLength > m_idealSegmentLength * 2.0) {
    AddNodeBefore(iNode);
...
//-----------------------------------------------------------------------------
// Purpose: Starting at the first stuck node back from the head, find a node to
// remove
//			between it and the actual root who is part of a chain
//that isn't too long.
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
bool CNPC_Hydra::ContractBetweenStuckSegments() {
  ...
  if (iShortest = -1) return false;

  // TODO(d.rattman): check for tunneling
...
bool CNPC_Hydra::ContractFromRoot() {
  ...
  if (m_body[2].flActualLength > m_idealSegmentLength * 2.0) return false;

  if (!IsValidConnection(1, 3)) return false;
...
bool CNPC_Hydra::AddNodeBefore(int iNode) {
  ...
  /*
  // TODO(d.rattman): can't do this, may be embedded in the world
  int i0 = (iNode>2)?iNode-2:0;
  int i1 = (iNode>1)?iNode-1:0;
  int i2 = iNode;
  int i3 = (iNode<m_body.Count()-1)?iNode+1:m_body.Count()-1;
  Catmull_Rom_Spline( m_body[i0].vecPos, m_body[i1].vecPos, m_body[i2].vecPos,
  m_body[i3].vecPos, 0.5, bone.vecPos );
  */
...
bool CNPC_Hydra::GrowFromVirtualRoot() {
  if (m_body[1].flActualLength < m_idealSegmentLength * 0.5) return false;
...
void CNPC_Hydra::CalcRelaxedLength() {
  m_flRelaxedLength =
      m_idealSegmentLength * (m_body.Count() - 2) + HYDRA_OUTWARD_BIAS;
}

bool CNPC_Hydra::IsValidConnection(int iNode0, int iNode1) {
  trace_t tr;
  // check to make sure new connection is valid
  AI_TraceHull(m_body[iNode0].vecPos, m_body[iNode1].vecPos, Vector(-2, -2, -2),
               Vector(2, 2, 2), MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
...
void CNPC_Hydra::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_HYDRA_PREP_STAB: {
      ...
      if (m_idealLength > HYDRA_MAX_LENGTH) m_idealLength = HYDRA_MAX_LENGTH;

      if (distToTarget < 100.0) {
        m_vecTargetDir = (m_vecTarget - m_vecHeadGoal);
        VectorNormalize(m_vecTargetDir);
        m_vecHeadGoal =
            m_vecHeadGoal - m_vecTargetDir * (100 - distToTarget) * 0.5;
      } else if (distToTarget > 200.0) {
        m_vecTargetDir = (m_vecTarget - m_vecHeadGoal);
        VectorNormalize(m_vecTargetDir);
        m_vecHeadGoal =
            m_vecHeadGoal - m_vecTargetDir * (200.0 - distToTarget) * 0.5;
      }
      ...
      float influence =
          1.0 - (m_flTaskEndTime - gpGlobals->curtime) / pTask->flTaskData;
      if (influence > 1) influence = 1.0;
      ...
      i = m_body.Count() - 2;
      m_body[i].vecGoalPos =
          m_vecHeadGoal - m_vecHeadDir * m_body[i].flActualLength;
      ...
      for (i = i - 1; i > 1 && distBackFromHead < distToTarget; i--) {
        ...
        // spiral
        Vector p0 = m_vecHeadGoal - m_vecHeadDir * distBackFromHead * 0.5 +
        ...
        m_body[i].flGoalInfluence =
            influence * (1.0 - (distBackFromHead / distToTarget));

        /*
        if ( (pEnemy->EyePosition( ) - m_body[i].vecPos).Length() <
        distBackFromHead)
        {
      ...
      for (i = i; i < m_body.Count() - 1; i++) {
        if (m_body[i].bStuck) {
          Vector delta = DotProduct(m_body[i].vecGoalPos - m_body[i].vecPos,
                                    m_vecHeadDir) *
                         m_vecHeadDir;
    ...
    case TASK_HYDRA_STAB: {
      ...
      // face enemy
      // m_vecHeadDir = (pEnemy->EyePosition( ) -
      // m_body[m_body.Count()-1].vecPos); VectorNormalize(
      // m_vecHeadDir.GetForModify() );
      ...
      */
      ...
      for (i = i - 1; i > 1 && distBackFromHead < distToTarget; i--) {
        ...
        if ((m_vecTarget - m_body[i].vecPos).Length() >
            distToTarget + distBackFromHead) {
          m_body[i].flGoalInfluence = 1.0 - (distBackFromHead / distToTarget);
        } else {
          m_body[i].vecGoalPos =
              EyePosition() - m_vecHeadDir * distBackFromHead;
          m_body[i].flGoalInfluence = 1.0 - (distBackFromHead / distToTarget);
        }
...
const QAngle &CNPC_Hydra::EyeAngles() { return GetAbsAngles(); }
...
Vector CNPC_Hydra::BodyTarget(const Vector &posSrc, bool bNoisy) {
  ...
  // NDebugOverlay::Box(m_body[iShortest].vecPos, Vector( -2, -2, -2 ), Vector(
  // 2, 2, 2 ), 0, 0, 255, 20, .1);
...
//			between that entity and the target ragdoll, and then
// updating then 			entity to follow the hydra.
//-----------------------------------------------------------------------------
...
class CHydraImpale : public CBaseAnimating {
 ...
 public:
  ...
  IPhysicsConstraint *CreateConstraint(CNPC_Hydra *pHydra,
                                       IPhysicsObject *pTargetPhys,
...
BEGIN_DATADESC(CHydraImpale)
  DEFINE_PHYSPTR(m_pConstraint), DEFINE_FIELD(m_hHydra, FIELD_EHANDLE),
...
//-----------------------------------------------------------------------------
// Purpose: To by usable by the constraint system, this needs to have a phys
// model.
//-----------------------------------------------------------------------------
...
void CHydraImpale::ImpaleThink(void) {
  ...
  // NDebugOverlay::Cross3D( vecOrigin, Vector( -5, -5, -5 ), Vector( 5, 5, 5 ),
  // 255, 0, 0, 20, .1);
...
//-----------------------------------------------------------------------------
IPhysicsConstraint *CHydraImpale::CreateConstraint(
    CNPC_Hydra *pHydra, IPhysicsObject *pTargetPhys,
    IPhysicsConstraintGroup *pGroup) {
  m_hHydra = pHydra;
  ...
  m_pConstraint = physenv->CreateFixedConstraint(pImpalePhysObject, pTargetPhys,
                                                 pGroup, fixed);
  if (m_pConstraint) {
...
CHydraImpale *CHydraImpale::Create(CNPC_Hydra *pHydra, CBaseEntity *pTarget) {
  ...
  CHydraImpale *pImpale =
      (CHydraImpale *)CBaseEntity::Create("hydra_impale", vecOrigin, vecAngles);
  if (!pImpale) return NULL;

  IPhysicsObject *pTargetPhysObject = pTarget->VPhysicsGetObject();
  if (!pTargetPhysObject) {
    DevMsg(
        " Error: Tried to hydra_impale an entity without a physics model.\n");
    return NULL;
  }
...
void CNPC_Hydra::AttachStabbedEntity(CBaseAnimating *pAnimating,
                                     Vector vecForce, trace_t &tr) {
  CTakeDamageInfo info(this, this, pAnimating->m_iHealth + 25, DMG_SLASH);
  info.SetDamageForce(vecForce);
  info.SetDamagePosition(tr.endpos);

  CBaseEntity *pRagdoll = CreateServerRagdoll(
      pAnimating, 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS);
...
void CNPC_Hydra::GetDesiredImpaledPosition(Vector *vecOrigin,
                                           QAngle *vecAngles) {
