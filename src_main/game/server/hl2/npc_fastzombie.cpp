...
// If flying at an enemy, and this close or closer, start playing the maul
// animation!!
...
int AE_FASTZOMBIE_VEHICLE_SS_DIE;  // Killed while doing scripted sequence on
                                   // vehicle
...
enum {
  SCHED_FASTZOMBIE_RANGE_ATTACK1 =
      ...
      100,  // hack to get past the base zombie's schedules
...
class CFastZombie : public CNPC_BaseZombie {
 ...
 public:
  ...
  bool MovementCost(int moveType, const Vector &vecStart, const Vector &vecEnd,
                    float *pCost);
...
//=============================================================================
 ...
 private:
  ...
  CSoundPatch
      *m_pLayer2;  // used for climbing ladders, and when jumping (pre apex)
...
BEGIN_DATADESC(CFastZombie)
  ...
  DEFINE_FIELD(m_flDistFactor, FIELD_FLOAT),
      ...
      // Function Pointers
      DEFINE_ENTITYFUNC(LeapAttackTouch), DEFINE_ENTITYFUNC(ClimbTouch),
...
void CFastZombie::PrescheduleThink(void) {
  ...
  if (m_pMoanSound && gpGlobals->curtime > m_flTimeUpdateSound) {
    ...
    if (flDistNoBBox >= FASTZOMBIE_EXCITE_DIST && m_flDistFactor != 1.0) {
      ...
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, FASTZOMBIE_IDLE_PITCH,
                                           FASTZOMBIE_SOUND_UPDATE_FREQ);
    } else if (flDistNoBBox < FASTZOMBIE_EXCITE_DIST) {
      // Zombie is close! Recalculate pitch.
      int iPitch;

      m_flDistFactor = std::min(1.0, 1 - flDistNoBBox / FASTZOMBIE_EXCITE_DIST);
      iPitch = FASTZOMBIE_MIN_PITCH +
               ((FASTZOMBIE_MAX_PITCH - FASTZOMBIE_MIN_PITCH) * m_flDistFactor);
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, iPitch,
                                           FASTZOMBIE_SOUND_UPDATE_FREQ);
    }
...
void CFastZombie::SoundInit(void) {
  ...
  if (!m_pLayer2) {
    // Set up layer2
    m_pLayer2 = ENVELOPE_CONTROLLER.SoundCreate(
        filter, entindex(), CHAN_VOICE, "NPC_FastZombie.Gurgle", ATTN_NORM);
...
void CFastZombie::Spawn(void) {
  ...
  CapabilitiesClear();
  CapabilitiesAdd(
      bits_CAP_MOVE_CLIMB | bits_CAP_MOVE_JUMP | bits_CAP_MOVE_GROUND |
      bits_CAP_INNATE_RANGE_ATTACK1 /* | bits_CAP_INNATE_MELEE_ATTACK1 */);
...
//-----------------------------------------------------------------------------
// Purpose: Returns the classname (ie "npc_headcrab") to spawn when our headcrab
// bails.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns the model to use for our legs ragdoll when we are blown in
// twain.
//-----------------------------------------------------------------------------
const char *CFastZombie::GetLegsModel(void) { return s_pLegsModel; }
...
int CFastZombie::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  // @TODO (toml 07-21-04): follow up with Steve to find out why fz was
  // explicitly not using these conditions
  if (baseResult == COND_TOO_FAR_TO_ATTACK ||
      baseResult == COND_NOT_FACING_ATTACK) {
    return COND_NONE;
  }
...
// Purpose: Play a random attack sound.
//-----------------------------------------------------------------------------
void CFastZombie::AttackSound(void) { EmitSound("NPC_FastZombie.Attack"); }
...
void CFastZombie::AlertSound(void) {
  ...
  if (pPlayer) {
    // Measure how far the player is, and play the appropriate type of alert
    // sound. Doesn't matter if I'm getting mad at a different character, the
    // player is the one that hears the sound.
    float flDist;
...
float CFastZombie::InnateRange1MaxRange(void) { return FASTZOMBIE_MAXLEAP; }
...
int CFastZombie::RangeAttack1Conditions(float flDot, float flDist) {
  ...
  AI_TraceHull(WorldSpaceCenter(), WorldSpaceCenter() + vecDirToEnemy,
               vecHullMin, vecHullMax, MASK_NPCSOLID, this,
...
void CFastZombie::HandleAnimEvent(animevent_t *pEvent) {
  if (pEvent->event == AE_FASTZOMBIE_CLIMB_LEFT ||
      pEvent->event == AE_FASTZOMBIE_CLIMB_RIGHT) {
    if (++m_iClimbCount % 3 == 0) {
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pLayer2,
  ...
  if (pEvent->event == AE_ZOMBIE_ATTACK_RIGHT) {
    ...
    ClawAttack(GetClawAttackRange(), 3, QAngle(-3, -5, -3), right,
               ZOMBIE_BLOOD_RIGHT_HAND);
    return;
  }
...
//=============================================================================
  ...
  if (pEvent->event == AE_FASTZOMBIE_VEHICLE_SS_DIE) {
    if (IsInAVehicle()) {
      ...
      vecVelocity +=
          (vecForward * -2500.0f) + (vecRight * 200.0f) + (vecUp * 300);

      // Always kill
      float flDamage = GetMaxHealth() + 10;

      // Take the damage and die
      CTakeDamageInfo info(this, this, vecVelocity * 25.0f, WorldSpaceCenter(),
                           flDamage, (DMG_CRUSH | DMG_VEHICLE));
      TakeDamage(info);
    }
    return;
  }
...
void CFastZombie::LeapAttack(void) {
  ...
  if (pEnemy) {
    ...
    //
    // How fast does the zombie need to travel to reach my enemy's eyes given
    // gravity?
    //
...
void CFastZombie::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_FASTZOMBIE_UNSTICK_JUMP: {
      ...
      // Call begin attack jump. A little bit later if we fail to pathfind, we
      // check this value to see if we just jumped. If so, we assume we've
      // jumped to someplace that's not pathing friendly, and so must jump again
      // to get out.
      BeginAttackJump();
    ...
    case TASK_WAIT_FOR_MOVEMENT:
      // If we're waiting for movement, that means that pathfinding succeeded,
      // and we're about to be moving. So we aren't stuck. So clear this flag.
      m_fJustJumped = false;
    ...
    case TASK_FACE_ENEMY: {
      // We don't use the base class implementation of this, because
      // GetTurnActivity stomps our landing scrabble animations (sjb)
  ...
  switch (pTask->iTask) {
    ...
    case TASK_RANGE_ATTACK1:
      if ((GetFlags() & FL_ONGROUND) || (m_pfnTouch == NULL)) {
        // All done when you touch the ground, or if our touch function has
        // somehow cleared.
        TaskComplete();
...
int CFastZombie::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    ...
    case SCHED_FASTZOMBIE_UNSTICK_JUMP:
      if (GetActivity() == ACT_CLIMB_UP || GetActivity() == ACT_CLIMB_DOWN ||
          GetActivity() == ACT_CLIMB_DISMOUNT) {
...
Activity CFastZombie::NPC_TranslateActivity(Activity baseAct) {
  if (baseAct == ACT_CLIMB_DOWN) return ACT_CLIMB_UP;
...
void CFastZombie::LeapAttackTouch(CBaseEntity *pOther) {
  ...
  ClawAttack(GetClawAttackRange(), 5, qaPunch, forward * 500,
             ZOMBIE_BLOOD_BOTH_HANDS);
  ...
  if (pOther->IsPlayer()) {
    ...
    if (GetActivity() != ACT_CLIMB_DISMOUNT ||
        (pOther->GetGroundEntity() == NULL && GetNavigator()->IsGoalActive() &&
         pOther->GetAbsOrigin().z - GetNavigator()->GetCurWaypointPos().z <
             -1.0)) {
...
//-----------------------------------------------------------------------------
void CFastZombie::BecomeTorso(const Vector &vecTorsoForce,
...
// Output :
//-----------------------------------------------------------------------------
bool CFastZombie::IsJumpLegal(const Vector &startPos, const Vector &apex,
                              const Vector &endPos) const {
...
bool CFastZombie::MovementCost(int moveType, const Vector &vecStart,
...
bool CFastZombie::ShouldFailNav(bool bMovementFailed) {
  if (!BaseClass::ShouldFailNav(bMovementFailed)) {
    DevMsg(2,
           "Fast zombie in scripted sequence probably hit bad node "
           "configuration at %s\n",
           VecToString(GetAbsOrigin()));

    if (GetNavigator()->GetPath()->CurWaypointNavType() == NAV_JUMP &&
        GetNavigator()->RefindPathToGoal(false)) {
      return false;
    }
...
void CFastZombie::OnChangeActivity(Activity NewActivity) {
  ...
  if (NewActivity == ACT_GLIDE) {
    ...
    if (m_pMoanSound)
      ENVELOPE_CONTROLLER.SoundChangePitch(m_pMoanSound, FASTZOMBIE_MIN_PITCH,
                                           0.3);
  }
  ...
  } else if (GetActivity() == ACT_CLIMB_DISMOUNT ||
             (GetActivity() == ACT_CLIMB_UP &&
...
//
//=========================================================
int CFastZombie::SelectFailSchedule(int failedSchedule, int failedTask,
                                    AI_TaskFailureCode_t taskFailCode) {
  if (m_fJustJumped) {
...
void CFastZombie::BeginAttackJump(void) {
  ...
  // to someplace that's not pathing friendly, and so must jump again to get
  // out.
  m_fJustJumped = true;
...
void CFastZombie::BuildScheduleTestBits(void) {
  ...
  if (GetRunningBehavior()) GetRunningBehavior()->BridgeBuildScheduleTestBits();
...
void CFastZombie::OnStateChange(NPC_STATE OldState, NPC_STATE NewState) {
  if (NewState == NPC_STATE_COMBAT) {
    SetAngrySoundState();
  } else if ((m_pMoanSound) &&
             (NewState == NPC_STATE_IDLE ||
              NewState == NPC_STATE_ALERT))  ///!!!HACKHACK - sjb
  {
    // Don't make this sound while we're slumped
    if (IsSlumped() == false) {
      // Set it up so that if the zombie goes into combat state sometime down
      // the road that he'll be able to scream.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CFastZombie::ShouldBecomeTorso(const CTakeDamageInfo &info,
...
void CFastZombie::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  if (m_PassengerBehavior.CanSelectSchedule() &&
    ...
    float flDamage = CalculatePhysicsImpactDamage(
        index, pEvent, gZombiePassengerImpactDamageTable, 1.0, true,
        damageType);
    ...
    if (flDamage > 0) {
      ...
      CTakeDamageInfo info(this, this, damageForce, damagePos, flDamage,
...
void CFastZombie::VehicleLeapAttack(void) {
  ...
  Vector vecJumpDir = VecCheckToss(this, GetAbsOrigin(), vecEnemyPos, 0.1f,
                                   1.0f, false, &vecMins, &vecMaxs);
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ACTIVITY				"
    "ACTIVITY:ACT_FASTZOMBIE_LEAP_STRIKE"
    "		TASK_RANGE_ATTACK1				0"
    "		TASK_WAIT						0.1"
    "		TASK_FASTZOMBIE_LAND_RECOVER	0"  // essentially just figure
...
//=========================================================
DEFINE_SCHEDULE(SCHED_FASTZOMBIE_UNSTICK_JUMP,
...
//=========================================================
//=========================================================
DEFINE_SCHEDULE(SCHED_FASTZOMBIE_CLIMBING_UNSTICK_JUMP,
