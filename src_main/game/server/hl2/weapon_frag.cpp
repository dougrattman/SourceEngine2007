...
CWeaponFrag::CWeaponFrag() : CBaseHLCombatWeapon(), m_bRedraw(false) { NULL; }
...
#define RETHROW_DELAY 0.5
  if (fThrewGrenade) {
    ...
    if (pOwner) {
      ...
      UTIL_TraceLine(vecSrc, vecSrc + vecDir * 1024, MASK_SOLID_BRUSHONLY,
                     pOwner, COLLISION_GROUP_NONE, &tr);

      CSoundEnt::InsertSound(SOUND_DANGER_SNIPERONLY, tr.endpos, 384, 0.2,
                             pOwner);
    }
  }
}
...
void CWeaponFrag::RollGrenade(CBasePlayer *pPlayer) {
  ...
  Fraggrenade_Create(vecSrc, orientation, vecThrow, rotSpeed, pPlayer,
                     GRENADE_TIMER, false);
