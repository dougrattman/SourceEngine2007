...
//-----------------------------------------------------------------------------
// Weapon-dissolve trigger; all weapons in this field (sans the physcannon) are
// destroyed!
//-----------------------------------------------------------------------------
class CTriggerWeaponDissolve : public CTriggerMultiple {
 ...
 public:
  ...
  void CreateBeam(const Vector &vecSource, CBaseEntity *pDest,
                  float flLifetime);
...
void CTriggerWeaponDissolve::Activate(void) {
  ...
  while ((pEntity = gEntList.FindEntityByName(pEntity, m_strEmitterName)) !=
         NULL) {
    m_pConduitPoints.AddToTail(pEntity);
  }

  SetContextThink(&CTriggerWeaponDissolve::DissolveThink,
                  gpGlobals->curtime + 0.1f, s_pDissolveThinkContext);
}
...
bool CTriggerWeaponDissolve::HasWeapon(CBaseCombatWeapon *pWeapon) {
  if (m_pWeapons.Find(pWeapon) == m_pWeapons.InvalidIndex()) return false;
...
void CTriggerWeaponDissolve::StartTouch(CBaseEntity *pOther) {
  ...
  if (PassesTriggerFilters(pOther) == false) return;
...
//-----------------------------------------------------------------------------
void CTriggerWeaponDissolve::CreateBeam(const Vector &vecSource,
                                        CBaseEntity *pDest, float flLifetime) {
  CBroadcastRecipientFilter filter;

  te->BeamEntPoint(filter, 0.0, 0, &vecSource, pDest->entindex(),
                   &(pDest->WorldSpaceCenter()), m_spriteTexture,
                   0,  // No halo
                   1,  // Frame
                   30, flLifetime,
                   16.0f,  // Start width
...
Vector CTriggerWeaponDissolve::GetConduitPoint(CBaseEntity *pTarget) {
  ...
  for (int i = 0; i < m_pConduitPoints.Count(); i++) {
    testDist = (m_pConduitPoints[i]->GetAbsOrigin() - pTarget->GetAbsOrigin())
                   .LengthSqr();
...
void CTriggerWeaponDissolve::DissolveThink(void) {
  ...
  for (int i = 0; i < numWeapons; i++) {
    ...
    m_pWeapons.Remove(i);
    SetContextThink(&CTriggerWeaponDissolve::DissolveThink,
                    gpGlobals->curtime + random->RandomFloat(0.5f, 1.5f),
                    s_pDissolveThinkContext);
    return;
  }
...
void CTriggerWeaponStrip::StartTouch(CBaseEntity *pOther) {
  ...
  if (pCharacter && pCharacter->IsAllowedToPickupWeapons()) {
    CBaseCombatWeapon *pBugbait =
        pCharacter->Weapon_OwnsThisType("weapon_bugbait");
    if (pBugbait) {
...
BEGIN_DATADESC(CWateryDeathLeech)
  DEFINE_THINKFUNC(LeechThink), DEFINE_FIELD(m_iFadeState, FIELD_INTEGER),
END_DATADESC()

void CWateryDeathLeech::Precache(void) {
  // Ugh this is temporary until Jakob finishes the animations and doesn't need
  // the command anymore.
...
class CTriggerWateryDeath : public CBaseTrigger {
 ...
 public:
  ...
  virtual bool PassesTriggerFilters(CBaseEntity *pOther) {
    if (!BaseClass::PassesTriggerFilters(pOther)) return false;
...
void CTriggerWateryDeath::SpawnLeeches(CBaseEntity *pOther) {
  ...
  if (m_hLeeches.Count() > 0) return;

  int iMaxLeeches = 12;

  for (int i = 0; i < iMaxLeeches; i++) {
    CWateryDeathLeech *pLeech =
        (CWateryDeathLeech *)CreateEntityByName("ent_watery_leech");
...
void CTriggerWateryDeath::Touch(CBaseEntity *pOther) {
  ...
  if (iIndex == m_hTouchingEntities.InvalidIndex()) return;

  float flKillTime = m_flEntityKillTimes[iIndex];

  // Time to kill it?
  if (gpGlobals->curtime > flKillTime) {
    // EmitSound( filter, entindex(), "WateryDeath.Bite",
    // &pOther->GetAbsOrigin() );
    // Kill it
    if (pOther->IsPlayer()) {
    ...
    // This ensures that if the target is the player, the damage isn't modified
    // by skill
    CTakeDamageInfo info =
        CTakeDamageInfo(pOther, pOther, m_flPainValue, DMG_GENERIC);

    GuessDamageForce(&info, (pOther->GetAbsOrigin() - GetAbsOrigin()),
                     pOther->GetAbsOrigin());
...
void CTriggerWateryDeath::EndTouch(CBaseEntity *pOther) {
  ...
  if (pOther->IsPlayer()) {
    for (int i = 0; i < m_hLeeches.Count(); i++) {
      CWateryDeathLeech *pLeech =
          dynamic_cast<CWateryDeathLeech *>(m_hLeeches[i].Get());
    ...
    if (m_hLeeches.Count() > 0) m_hLeeches.Purge();
