...
extern void SpawnBlood(Vector vecSpot, const Vector &vAttackDir, int bloodColor,
                       float flDamage);
...
enum ManhackConditions {
  COND_MANHACK_START_ATTACK =
      LAST_SHARED_CONDITION,  // We are able to do a bombing run on the current
                              // enemy.
};
...
BEGIN_DATADESC(CNPC_Manhack)
  ...
  DEFINE_FIELD(m_vForceVelocity, FIELD_VECTOR),
      ...
      DEFINE_FIELD(m_bGib, FIELD_BOOLEAN), DEFINE_FIELD(m_bHeld, FIELD_BOOLEAN),
...
SendPropIntWithMinusOneFlag(SENDINFO(m_nEnginePitch1), 8),
    ...
    SendPropIntWithMinusOneFlag(SENDINFO(m_nEnginePitch2), 8) END_SEND_TABLE()
...
void CNPC_Manhack::PrescheduleThink(void) {
  ...
  if (GetWaterLevel() > 0) {
    ...
    if (IsAlive()) {
      // If I've been out of water for 2 seconds or more, I'm eligible to be
      // stuck in water again.
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Manhack::TraceAttack(const CTakeDamageInfo &info,
...
//-----------------------------------------------------------------------------
bool CNPC_Manhack::ShouldGib(const CTakeDamageInfo &info) { return (m_bGib); }
...
void CNPC_Manhack::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (m_nEnginePitch1 < 0) {
    // Probably this manhack was killed immediately after spawning. Turn the
    // sound on right now so that we can pitch it up for the crash!
    SoundInit();
  }
...
//-----------------------------------------------------------------------------
#define MANHACK_SMASH_SPEED \
  500.0  // How fast a manhack must slam into something to take full damage
void CNPC_Manhack::TakeDamageFromPhyscannon(CBasePlayer *pPlayer) {
  ...
  flFactor = std::clamp(flFactor, 0.0f, 2.0f);
...
//-----------------------------------------------------------------------------
void CNPC_Manhack::TakeDamageFromVehicle(int index,
  ...
  Vector damageForce =
      2.0f * pEvent->postVelocity[index] * pEvent->pObjects[index]->GetMass();
  if (damageForce == vec3_origin) {
    // This can happen if this entity is a func_breakable, and can't move.
    // Use the velocity of the entity that hit us instead.
    damageForce = 2.0f * pEvent->postVelocity[!index] *
                  pEvent->pObjects[!index]->GetMass();
  }
  Assert(damageForce != vec3_origin);
  CTakeDamageInfo dmgInfo(pOther, pOther, damageForce, damagePos, flDamage,
                          DMG_CRUSH);
  TakeDamage(dmgInfo);
}
...
//-----------------------------------------------------------------------------
void CNPC_Manhack::TakeDamageFromPhysicsImpact(int index,
                                               gamevcollisionevent_t *pEvent) {
  ...
  int damageType = 0;
  float damage = CalculateDefaultPhysicsDamage(index, pEvent, flDamageScale,
                                               true, damageType);
  if (damage == 0) return;
...
// Purpose:
//-----------------------------------------------------------------------------
#define MANHACK_SMASH_TIME \
  0.35  // How long after being thrown from a physcannon that a manhack is
        // eligible to die from impact
void CNPC_Manhack::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
  ...
  if (pPlayer) {
    ...
    if (!pHitEntity->IsNPC() &&
        (!pRagdollProp || pRagdollProp->GetKiller() != this)) {
      TakeDamageFromPhyscannon(pPlayer);
      StopBurst(true);
      return;
    }
...
void CNPC_Manhack::VPhysicsShadowCollision(int index,
                                           gamevcollisionevent_t *pEvent) {
  int otherIndex = !index;
  CBaseEntity *pOther = pEvent->pEntities[otherIndex];
...
//-----------------------------------------------------------------------------
// Purpose: Manhack is out of control! (dying) Just explode as soon as you touch
// anything! Input  : Output :
//-----------------------------------------------------------------------------
...
void CNPC_Manhack::CreateSmokeTrail() {
  if (HasSpawnFlags(SF_MANHACK_NO_DAMAGE_EFFECTS)) return;

  if (m_hSmokeTrail != NULL) return;
...
int CNPC_Manhack::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (tdInfo.GetAmmoType() == GetAmmoDef()->Index("SniperRound")) {
    // Unfortunately, this is the easiest way to stop the sniper killing
    // manhacks in one shot.
  ...
  if (info.GetDamageType() & DMG_PHYSGUN) {
    ...
    //		 if fudging this vector a little bit could help me slam into a physics
    //object, 		 then make that adjustment. This is a simple heuristic. The manhack
    //will be 		 directed towards the physics object that is closest to
    //g_vecAttackDir
    //

    //		-Take 150% damage from club attacks. This makes crowbar duels take two
    //hits.
...
#define MANHACK_PHYSICS_SEARCH_RADIUS 128
    ...
    Vector testCenter =
        GetAbsOrigin() + (attackDir * MANHACK_PHYSICS_SEARCH_RADIUS);
    Vector vecDelta(MANHACK_PHYSICS_SEARCH_RADIUS,
                    MANHACK_PHYSICS_SEARCH_RADIUS,
                    MANHACK_PHYSICS_SEARCH_RADIUS);

    int count =
        UTIL_EntitiesInBox(pList, MANHACK_PHYS_SEARCH_SIZE,
                           testCenter - vecDelta, testCenter + vecDelta, 0);
...
bool CNPC_Manhack::CorpseGib(const CTakeDamageInfo &info) {
  ...
  PropBreakableCreateAll(GetModelIndex(), NULL, GetAbsOrigin(), GetAbsAngles(),
                         vecGibVelocity, vecGibAVelocity, 1.0, 60,
...
void CNPC_Manhack::OnStateChange(NPC_STATE OldState, NPC_STATE NewState) {
  if (m_vNoiseMod.z == MANHACK_NOISEMOD_HIDE &&
      !(m_spawnflags & SF_NPC_WAIT_FOR_SCRIPT) &&
  ...
  if (NewState != NPC_STATE_IDLE && (m_spawnflags & SF_NPC_GAG) &&
      (m_nEnginePitch1 < 0)) {
...
bool CNPC_Manhack::IsFlyingActivity(Activity baseAct) {
  return ((baseAct == ACT_FLY || baseAct == ACT_IDLE || baseAct == ACT_RUN ||
           baseAct == ACT_WALK) &&
          m_bBladesActive);
}
...
int CNPC_Manhack::TranslateSchedule(int scheduleType) {
  ...
  if (m_spawnflags & SF_MANHACK_PACKED_UP) {
    if (scheduleType != SCHED_WAIT_FOR_SCRIPT) return SCHED_MANHACK_DEPLOY;
  }
...
void CNPC_Manhack::Loiter() {
  // NDebugOverlay::Line( GetAbsOrigin(), m_vecLoiterPosition, 255, 255, 255,
  // false, 0.1 );
...
void CNPC_Manhack::MaintainGroundHeight(void) {
  ...
  if (zSpeed > 32.0f) return;

  const float minGroundHeight = 52.0f;

  trace_t tr;
  AI_TraceHull(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, minGroundHeight),
               GetHullMins(), GetHullMaxs(), (MASK_NPCSOLID_BRUSHONLY), this,
...
bool CNPC_Manhack::OverrideMove(float flInterval) {
  ...
  if (HasSpawnFlags(SF_MANHACK_PACKED_UP | SF_MANHACK_CARRIED)) return true;
  ...
  // ------------------------------------------------------------------
    ...
    bool bReducible =
        GetNavigator()->GetPath()->GetCurWaypoint()->IsReducible();
    const float strictTolerance = 64.0;
    // NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 10 ),
    // 255, 0, 0, true, 0.1);
    if (ProgressFlyPath(flInterval, GetEnemy(), MoveCollisionMask(), bReducible,
                        strictTolerance) == AINPP_COMPLETE)
      return true;
  }
  // -----------------------------------------------------------------
...
void CNPC_Manhack::MoveToTarget(float flInterval, const Vector &vMoveTarget) {
  ...
  // -----------------------------------------
  if (gpGlobals->curtime < m_flEngineStallTime || m_iHealth <= 0) return;
  ...
  if (m_flBurstDuration > gpGlobals->curtime) {
    ...
    /*
    NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() + ( targetDir * 64.0f ),
    255, 0, 0, true, 2.1f ); NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin()
    + ( steerDir * 64.0f ), 0, 255, 0, true, 2.1f ); NDebugOverlay::Line(
    GetAbsOrigin(), GetAbsOrigin() + ( m_vecBurstDirection * 64.0f ), 0, 0, 255,
    true, 2.1f ); NDebugOverlay::Cross3D( GetAbsOrigin() , -Vector(8,8,8),
    Vector(8,8,8), 255, 0, 0, true, 2.1f );
    */
    ...
    if (flDot > 0.25) {
      ...
      // Have a harder time correcting my course if I'm currently flying away
      // from my target.
      myAccel = 200;
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
int CNPC_Manhack::MoveCollisionMask(void) { return MASK_NPCSOLID; }
...
void CNPC_Manhack::Splash(const Vector &vecSplashPos) {
  ...
  if (!(contents & (CONTENTS_SLIME | CONTENTS_WATER))) {
    ...
    trace_t tr;
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() - Vector(0, 0, 256),
                   (CONTENTS_WATER | CONTENTS_SLIME), this,
                   COLLISION_GROUP_NONE, &tr);
...
//-----------------------------------------------------------------------------
void CNPC_Manhack::ComputeSliceBounceVelocity(CBaseEntity *pHitEntity,
                                              trace_t &tr) {
  if (pHitEntity->IsAlive() &&
      FClassnameIs(pHitEntity, "func_breakable_surf")) {
    // We want to see if the manhack hits a breakable pane of glass. To keep
    // from checking The classname of the HitEntity on each impact, we only do
    // this check if we hit something that's alive. Anyway, prevent the manhack
    // bouncing off the pane of glass, since this impact will shatter the glass
    // and let the manhack through.
    return;
  }
...
bool CNPC_Manhack::IsHeldByPhyscannon() {
  return VPhysicsGetObject() &&
         (VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD);
}
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Manhack::Slice(CBaseEntity *pHitEntity, float flInterval,
                         trace_t &tr) {
  // Don't hurt the player if I'm in water
  if (GetWaterLevel() > 0 && pHitEntity->IsPlayer()) return;

  // Can't slice players holding it with the phys cannon
  if (IsHeldByPhyscannon()) {
    if (pHitEntity && (pHitEntity == HasPhysicsAttacker(FLT_MAX))) return;
  }

  if (pHitEntity->m_takedamage == DAMAGE_NO) return;
  ...
  if (IsHeldByPhyscannon()) {
    ...
    flDamage = sk_combine_guard_health
                   .GetFloat();  // the highest healthed fleshy enemy
  } else if (dynamic_cast<CBaseProp *>(pHitEntity) ||
             dynamic_cast<CBreakable *>(pHitEntity)) {
    // If we hit a prop, we want it to break immediately
    flDamage = pHitEntity->GetHealth();
  } else if (pHitEntity->IsNPC() && IRelationType(pHitEntity) == D_HT &&
             FClassnameIs(pHitEntity, "npc_combine_s")) {
    flDamage *= 6.0f;
  }
  ...
  if (pHitEntity->BloodColor() == DONT_BLEED) {
    ...
    // TODO: What we really want to do is get a material reference and emit the
    // proper sprayage! - jdw
  } else {
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_Manhack::Bump(CBaseEntity *pHitEntity, float flInterval,
                        trace_t &tr) {
  if (!VPhysicsGetObject()) return;

  // Surpressing this behavior
  if (m_flBumpSuppressTime > gpGlobals->curtime) return;

  if (pHitEntity->GetMoveType() == MOVETYPE_VPHYSICS &&
  ...
  if (flInterval > 0) {
    float moveLen =
        ((GetCurrentVelocity() * flInterval) * (1.0 - tr.fraction)).Length();
    ...
    if (fabs(DotProduct(myUp, tr.plane.normal)) < 0.25) {
      ...
      te->DynamicLight(filter, 0.0, &GetAbsOrigin(), 255, 180, 100, 0, 50, 0.3,
                       150);
      ...
      // For decals and sparks we must trace a line in the direction of the
      // surface norm that we hit.
      trace_t decalTrace;
      AI_TraceLine(GetAbsOrigin(), GetAbsOrigin() - (tr.plane.normal * 24),
                   MASK_SOLID, this, COLLISION_GROUP_NONE, &decalTrace);

      if (decalTrace.fraction != 1.0) {
        // Leave decal only if colliding horizontally
        if ((DotProduct(Vector(0, 0, 1), decalTrace.plane.normal) < 0.5) &&
            (DotProduct(Vector(0, 0, -1), decalTrace.plane.normal) < 0.5)) {
    ...
    if (tr.allsolid || tr.startsolid) {
      // Build a fake reflection back along our current velocity because we
      // can't know how to reflect off a non-existant surface! -- jdw
  ...
  if (GetNavigator()->IsGoalActive() &&
      !(GetNavigator()->GetPath()->CurWaypointFlags() &
        bits_WP_TO_PATHCORNER)) {
    AIMoveTrace_t moveTrace;
    GetMoveProbe()->MoveLimit(NAV_GROUND, GetAbsOrigin(),
                              GetNavigator()->GetCurWaypointPos(),
...
void CNPC_Manhack::CheckCollisions(float flInterval) {
  ...
  if (IsHeldByPhyscannon()) {
    ...
    if (pCarrier) {
      if (pCarrier->CollisionProp()->CalcDistanceFromPoint(WorldSpaceCenter()) <
          30) {
  ...
  AI_TraceHull(GetAbsOrigin(), vecCheckPos, GetHullMins(), GetHullMaxs(),
               MoveCollisionMask(), this, COLLISION_GROUP_NONE, &tr);

  if ((tr.fraction != 1.0 || tr.startsolid) && tr.m_pEnt) {
    PhysicsMarkEntitiesAsTouching(tr.m_pEnt, tr);
    pHitEntity = tr.m_pEnt;

    if (m_bHeld && tr.m_pEnt->MyNPCPointer() &&
        tr.m_pEnt->MyNPCPointer()->IsPlayerAlly()) {
      // Don't slice Alyx when she approaches to hack. We need a better solution
      // for this!!
      // Msg("Ignoring!\n");
      return;
    }

    if (pHitEntity != NULL && pHitEntity->m_takedamage == DAMAGE_YES &&
        pHitEntity->Classify() != CLASS_MANHACK &&
...
void CNPC_Manhack::PlayFlySound(void) {
  ...
  // Spin sound based on distance from enemy (unless we're crashing)
    ...
    if (flEnemyDist < MANHACK_PITCH_DIST1) {
      ...
      flDistFactor = std::min(1.0f, 1 - flEnemyDist / MANHACK_PITCH_DIST1);
      iPitch1 = MANHACK_MIN_PITCH1 +
                ((MANHACK_MAX_PITCH1 - MANHACK_MIN_PITCH1) * flDistFactor);

      // NOTE: MANHACK_PITCH_DIST2 must be < MANHACK_PITCH_DIST1
      flDistFactor = std::min(1.0f, 1 - flEnemyDist / MANHACK_PITCH_DIST2);
      iPitch2 = MANHACK_MIN_PITCH2 +
                ((MANHACK_MAX_PITCH2 - MANHACK_MIN_PITCH2) * flDistFactor);
  ...
  if (gpGlobals->curtime > m_flNextEngineSoundTime && flEnemyDist < 48) {
    m_flNextEngineSoundTime =
        gpGlobals->curtime + random->RandomFloat(3.0, 10.0);
...
void CNPC_Manhack::MoveExecute_Alive(float flInterval) {
  ...
  // ----------------------------------------------------------------------------------------
  // Add time-coherent noise to the current velocity so that it never looks
  // bolted in place.
  // ----------------------------------------------------------------------------------------
  ...
  if ((CBaseEntity *)GetEnemy()) {
    ...
    if (IsInEffectiveTargetZone(GetEnemy()) &&
        flDist < MANHACK_CHARGE_MIN_DIST &&
        gpGlobals->curtime > m_flNextBurstTime) {
      Vector vecCurrentDir = GetCurrentVelocity();
      VectorNormalize(vecCurrentDir);
  ...
  if (m_flWaterSuspendTime > gpGlobals->curtime) {
    if (UTIL_PointContents(GetAbsOrigin()) &
      ...
      // Ooops, we're submerged somehow. Move upwards until our origin is out of
      // the water.
    ...
    m_vCurrentVelocity.z = std::max(m_vCurrentVelocity.z, 0.0f);
  }

  CheckCollisions(flInterval);

  // Blend out desired velocity when launched by the physcannon
  if (HasPhysicsAttacker(MANHACK_SMASH_TIME) && (!IsHeldByPhyscannon()) &&
      VPhysicsGetObject()) {
    Vector vecCurrentVelocity;
    VPhysicsGetObject()->GetVelocity(&vecCurrentVelocity, NULL);
    float flLerpFactor = (gpGlobals->curtime - m_flLastPhysicsInfluenceTime) /
                         MANHACK_SMASH_TIME;
    flLerpFactor = std::clamp(flLerpFactor, 0.0f, 1.0f);
    flLerpFactor = SimpleSplineRemapVal(flLerpFactor, 0.0f, 1.0f, 0.0f, 1.0f);
    VectorLerp(vecCurrentVelocity, m_vCurrentVelocity, flLerpFactor,
               m_vCurrentVelocity);
  }
  ...
  if (gpGlobals->curtime < m_flEngineStallTime) {
    ...
    */
    ...
    while (timeToUse > 0) {
      m_vCurrentBanking.x =
          (iRate * m_vCurrentBanking.x) + (1 - iRate) * (m_vTargetBanking.x);
      m_vCurrentBanking.z =
          (iRate * m_vCurrentBanking.z) + (1 - iRate) * (m_vTargetBanking.z);
      timeToUse = -0.1;
    }
...
void CNPC_Manhack::GatherEnemyConditions(CBaseEntity *pEnemy) {
  ...
  if (GetEnemy()->IsPlayer() &&
      assert_cast<CBasePlayer *>(GetEnemy())->IsInAVehicle()) {
    flZDist = 24.0f;
  }
...
int CNPC_Manhack::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  if (m_flNextBurstTime > gpGlobals->curtime) return COND_NONE;

  float flMaxDist = 45;
  float flMinDist = 24;
  bool bEnemyInVehicle = GetEnemy()->IsPlayer() &&
                         assert_cast<CBasePlayer *>(GetEnemy())->IsInAVehicle();
  if (GetEnemy()->IsPlayer() &&
      assert_cast<CBasePlayer *>(GetEnemy())->IsInAVehicle()) {
    flMinDist = 0;
  ...
  // Check our current velocity and speed, if it's too far off, we need to
  // settle
...
void CNPC_Manhack::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_WAIT_FOR_MOVEMENT: {
      ...
      if (pEnemy && (GetCurSchedule()->GetId() == SCHED_CHASE_ENEMY) &&
          GetNavigator()->IsGoalActive()) {
        Vector vecEnemyPosition;
        vecEnemyPosition = pEnemy->EyePosition();
        if (GetNavigator()->GetGoalPos().DistToSqr(vecEnemyPosition) >
            40 * 40) {
    ...
    case TASK_MANHACK_MOVEAT_SAVEPOSITION: {
      ...
      //m_vSavePosition, 0, 255, 0, true, 0.1);
...
void CNPC_Manhack::Spawn(void) {
  ...
  SetViewOffset(
      Vector(0, 0, 10));  // Position of the eyes relative to NPC's origin.
  ...
  // Set the noise mod to huge numbers right now, in case this manhack starts
  // out waiting for a script for instance, we don't want him to bob whilst he's
  // waiting for a script. This allows designers to 'hide' manhacks in small
  // places. (sjb)
  SetNoiseMod(MANHACK_NOISEMOD_HIDE, MANHACK_NOISEMOD_HIDE,
              MANHACK_NOISEMOD_HIDE);
  ...
  // Manhacks are designed to slam into things, so don't take much damage from
  // it!
...
void CNPC_Manhack::StartEye(void) {
  ...
  if (m_pEyeGlow == NULL) {
    m_pEyeGlow =
        CSprite::SpriteCreate(MANHACK_GLOW_SPRITE, GetLocalOrigin(), false);
    m_pEyeGlow->SetAttachment(this, LookupAttachment("Eye"));

    if (m_bHackedByAlyx) {
      m_pEyeGlow->SetTransparency(kRenderTransAdd, 0, 255, 0, 128,
                                  kRenderFxNoDissipation);
      m_pEyeGlow->SetColor(0, 255, 0);
    } else {
      m_pEyeGlow->SetTransparency(kRenderTransAdd, 255, 0, 0, 128,
                                  kRenderFxNoDissipation);
  ...
  if (m_pLightGlow == NULL) {
    ...
    if (m_bHackedByAlyx) {
      m_pLightGlow->SetTransparency(kRenderTransAdd, 0, 255, 0, 128,
                                    kRenderFxNoDissipation);
      m_pLightGlow->SetColor(0, 255, 0);
    } else {
      m_pLightGlow->SetTransparency(kRenderTransAdd, 255, 0, 0, 128,
                                    kRenderFxNoDissipation);
...
//-----------------------------------------------------------------------------
// Purpose: Get the engine sound started. Unless we're not supposed to have it
// on yet!
//-----------------------------------------------------------------------------
void CNPC_Manhack::PostNPCInit(void) {
  // SetAbsVelocity( vec3_origin );
  m_bBladesActive = (m_spawnflags & (SF_MANHACK_PACKED_UP | SF_MANHACK_CARRIED))
                        ? false
                        : true;
  BladesInit();
}
...
void CNPC_Manhack::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_MANHACK_FIND_SQUAD_CENTER: {
      ...
      AISquadIter_t iter;
      for (CAI_BaseNPC *pSquadMember = m_pSquad->GetFirstMember(&iter);
           pSquadMember; pSquadMember = m_pSquad->GetNextMember(&iter)) {
        if (pSquadMember->HasStrategySlotRange(SQUAD_SLOT_ATTACK1,
                                               SQUAD_SLOT_ATTACK2)) {
    ...
    case TASK_MANHACK_FIND_SQUAD_MEMBER: {
      if (m_pSquad) {
        ...
        AISquadIter_t iter;
        for (pSquadMember = m_pSquad->GetFirstMember(&iter); pSquadMember;
    ...
    case TASK_MANHACK_MOVEAT_SAVEPOSITION: {
      trace_t tr;
      AI_TraceLine(GetAbsOrigin(), m_vSavePosition, MASK_NPCWORLDSTATIC, this,
                   COLLISION_GROUP_NONE, &tr);
      if (tr.DidHitWorld()) {
        TaskFail(FAIL_NO_ROUTE);
      } else {
        m_fSwarmMoveTime =
            gpGlobals->curtime +
            RandomFloat(pTask->flTaskData * 0.8, pTask->flTaskData * 1.2);
      }
    } break;
...
//-----------------------------------------------------------------------------
bool CNPC_Manhack::HandleInteraction(int interactionType, void *data,
...
void CNPC_Manhack::ClampMotorForces(Vector &linear, AngularImpulse &angular) {
  ...
  if (m_flEngineStallTime > gpGlobals->curtime) {
    linear.x = 0.0f;
    linear.y = 0.0f;
    linear.z = std::clamp(linear.z, -fscale, fscale < 1200 ? 1200 : fscale);
  } else {
    // limit reaction forces
    linear.x = std::clamp(linear.x, -fscale, fscale);
    linear.y = std::clamp(linear.y, -fscale, fscale);
    linear.z = std::clamp(linear.z, -fscale, fscale < 1200 ? 1200 : fscale);
  }
...
bool CNPC_Manhack::IsInEffectiveTargetZone(CBaseEntity *pTarget) {
  ...
  if (pTarget && pTarget->IsPlayer() &&
      assert_cast<CBasePlayer *>(pTarget)->IsInAVehicle()) {
    CBaseEntity *pVehicle =
        assert_cast<CBasePlayer *>(pTarget)->GetVehicleEntity();
    pVehicle->CollisionProp()->NormalizedToWorldSpace(Vector(0.0f, 0.0f, 1.0f),
                                                      &vecMaxPos);
    pVehicle->CollisionProp()->NormalizedToWorldSpace(Vector(0.0f, 0.0f, 0.0f),
                                                      &vecMinPos);

    if (ourHeight > vecMinPos.z && ourHeight < vecMaxPos.z) return true;

    return false;
  }

  // Get the enemies top and bottom point
  pTarget->CollisionProp()->NormalizedToWorldSpace(Vector(0.0f, 0.0f, 1.0f),
                                                   &vecMaxPos);
#ifdef _XBOX
  pTarget->CollisionProp()->NormalizedToWorldSpace(
      Vector(0.0f, 0.0f, 0.5f), &vecMinPos);  // Only half the body is valid
#else
  pTarget->CollisionProp()->NormalizedToWorldSpace(Vector(0.0f, 0.0f, 0.0f),
                                                   &vecMinPos);
#endif  // _XBOX
...
//			&tolerance -
//-----------------------------------------------------------------------------
void CNPC_Manhack::TranslateNavGoal(CBaseEntity *pEnemy,
                                    Vector &chasePosition) {
  if (pEnemy && pEnemy->IsPlayer() &&
      assert_cast<CBasePlayer *>(pEnemy)->IsInAVehicle()) {
    Vector vecNewPos;
    CBaseEntity *pVehicle =
        assert_cast<CBasePlayer *>(pEnemy)->GetVehicleEntity();
    pVehicle->CollisionProp()->NormalizedToWorldSpace(Vector(0.5, 0.5, 0.5f),
                                                      &vecNewPos);
    chasePosition.z = vecNewPos.z;
  } else {
    Vector vecTarget;
    pEnemy->CollisionProp()->NormalizedToCollisionSpace(Vector(0, 0, 0.75f),
                                                        &vecTarget);
    chasePosition.z += vecTarget.z;
  }
}

float CNPC_Manhack::GetDefaultNavGoalTolerance() { return GetHullWidth(); }
...
void CNPC_Manhack::InputUnpack(inputdata_t &inputdata) {
  if (HasSpawnFlags(SF_MANHACK_PACKED_UP) == false) return;
...
//			reason -
//-----------------------------------------------------------------------------
void CNPC_Manhack::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
  ...
  if (reason == PUNTED_BY_CANNON) {
    ...
    // Suppress collisions between the manhack and the player; we're currently
    // bumping almost certainly because it's not purely a physics object.
...
//			Reason -
//-----------------------------------------------------------------------------
void CNPC_Manhack::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
CBasePlayer *CNPC_Manhack::HasPhysicsAttacker(float dt) {
  ...
  if (IsHeldByPhyscannon() ||
      (gpGlobals->curtime - dt <= m_flLastPhysicsInfluenceTime)) {
...
void CNPC_Manhack::ShowHostile(bool hostile /*= true*/) {
  if (m_bShowingHostile == hostile) return;
...
void CNPC_Manhack::StartBurst(const Vector &vecDirection) {
  if (m_flBurstDuration > gpGlobals->curtime) return;
...
void CNPC_Manhack::SetEyeState(int state) {
  ...
  switch (state) {
    ...
    default:
      if (m_pEyeGlow) m_pEyeGlow->m_nRenderFX = kRenderFxNone;
      break;
  }
}
...
bool CNPC_Manhack::CreateVPhysics(void) {
  if (HasSpawnFlags(SF_MANHACK_CARRIED)) return false;
...
DEFINE_SCHEDULE(
    ...
    "	SCHEDULE:SCHED_MANHACK_SWARM_FAILURE"
