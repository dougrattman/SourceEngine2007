...
#define SCANNER_CIT_INSPECT_GROUND_DIST \
  500  // How far to look for citizens to inspect
#define SCANNER_CIT_INSPECT_FLY_DIST \
  1500  // How far to look for citizens to inspect
...
BEGIN_DATADESC(CNPC_CScanner)
  ...
  DEFINE_SOUNDPATCH(m_pEngineSound),
      ...
      DEFINE_KEYFIELD(m_bOnlyInspectPlayers, FIELD_BOOLEAN,
                      "OnlyInspectPlayers"),
      DEFINE_KEYFIELD(m_bNeverInspectPlayers, FIELD_BOOLEAN,
                      "NeverInspectPlayers"),
      DEFINE_FIELD(m_fNextPhotographTime, FIELD_TIME),
      //	DEFINE_FIELD( m_pEyeFlash,
      // FIELD_CLASSPTR ),
      ...
      //	DEFINE_FIELD( m_hSpotlight,
      // FIELD_EHANDLE ), 	DEFINE_FIELD( m_hSpotlightTarget,
      // FIELD_EHANDLE ),
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "InspectTargetPhoto",
                       InputInspectTargetPhoto),
      DEFINE_INPUTFUNC(FIELD_STRING, "InspectTargetSpotlight",
...
void CNPC_CScanner::Spawn(void) {
  ...
  if (m_flSpotlightMaxLength <= 0) {
    DevMsg(
        "CNPC_CScanner::Spawn: Invalid spotlight length <= 0, setting to "
        "500\n");
...
void CNPC_CScanner::Activate() {
  ...
  m_pEyeFlash =
      CSprite::SpriteCreate("sprites/blueflare1.vmt", GetLocalOrigin(), FALSE);
  m_pEyeFlash->SetTransparency(kRenderGlow, 255, 255, 255, 0,
...
void CNPC_CScanner::Gib(void) {
  ...
  // Spawn all gibs
  if (m_bIsClawScanner) {
    CGib::SpawnSpecificGibs(this, 1, 500, 250,
  ...
  if (!HasSpawnFlags(SF_NPC_NO_WEAPON_DROP) &&
      random->RandomFloat(0.0f, 1.0f) < 0.3f) {
...
void CNPC_CScanner::Event_Killed(const CTakeDamageInfo &info) {
  ...
  // up into the base class's Event_Killed() until we gib. (gibbing is ultimate
  // death)
  m_KilledInfo = info;
  ...
  if (!m_bIsClawScanner && GetEnemy() != NULL &&
      (info.GetDamageType() & DMG_DISSOLVE) == false) {
    Vector vecDelta = GetLocalOrigin() - GetEnemy()->GetLocalOrigin();
    if ((vecDelta.z > 120) && (vecDelta.Length() > 360)) {
      // If I'm divebombing, don't take any more damage. It will make
      // Event_Killed() be called again. This is especially bad if someone
      // machineguns the divebombing scanner.
      AttackDivebomb();
      return;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Tells use whether or not the NPC cares about a given type of hint
// node. Input  : sHint - Output : TRUE if the NPC is interested in this hint
// type, FALSE if not.
//-----------------------------------------------------------------------------
...
char *CNPC_CScanner::GetEngineSound(void) {
  if (m_bIsClawScanner) return "NPC_SScanner.FlyLoop";
  ...
  if (m_pSquad) {
    ...
    for (CAI_BaseNPC *pSquadMember = m_pSquad->GetFirstMember(&iter);
      ...
      if (pSquadMember != this) {
        if (GetTarget()) {
          pSquadMember->DispatchInteraction(
              g_interactionScannerSupportEntity,
              ((void *)((CBaseEntity *)GetTarget())), this);
        } else {
          pSquadMember->DispatchInteraction(g_interactionScannerSupportPosition,
                                            ((void *)m_vInspectPos.Base()),
                                            this);
        }
      }
    }
...
bool CNPC_CScanner::IsValidInspectTarget(CBaseEntity *pEntity) {
  ...
  if (pEntity->Classify() == CLASS_CITIZEN_PASSIVE) {
    if (((CNPC_Citizen *)pEntity)->GetNextScannerInspectTime() >
        gpGlobals->curtime) {
      return false;
    }
  }
...
CBaseEntity *CNPC_CScanner::BestInspectTarget(void) {
  if (!m_bShouldInspect) return NULL;
  ...
  if (m_bOnlyInspectPlayers) {
    ...
    return WorldSpaceCenter().DistToSqr(pPlayer->EyePosition()) <=
                   (fSearchDist * fSearchDist)
               ? pPlayer
               : NULL;
  }
  ...
  for (i = 0; i < g_AI_Manager.NumAIs(); i++) {
    if (ppAIs[i] != this &&
        vSearchOrigin.DistToSqr(ppAIs[i]->GetAbsOrigin()) < fSearchDistSq) {
...
//			 the given entity and set the durection of the
// inspection
//------------------------------------------------------------------------------
void CNPC_CScanner::SetInspectTargetToEnt(CBaseEntity *pEntity,
...
//------------------------------------------------------------------------------
// Purpose: Clears any previous inspect target and set inspect target to
//			 the given hint node and set the durection of the
// inspection
//------------------------------------------------------------------------------
void CNPC_CScanner::SetInspectTargetToHint(CAI_Hint *pHint,
                                           float fInspectDuration) {
  ClearInspectTarget();
  ...
  trace_t tr;
  AI_TraceLine(vHintOrigin, vHintEnd, MASK_BLOCKLOS, this, COLLISION_GROUP_NONE,
               &tr);

  if (g_debug_cscanner.GetBool()) {
    NDebugOverlay::Line(vHintOrigin, tr.endpos, 255, 0, 0, true, 4.0f);
    NDebugOverlay::Cross3D(tr.endpos, -Vector(8, 8, 8), Vector(8, 8, 8), 255, 0,
                           0, true, 4.0f);
  }
...
//------------------------------------------------------------------------------
// Purpose: Clears any previous inspect target and set inspect target to
//			 the given position and set the durection of the
// inspection
// Input   :
// Output  :
//------------------------------------------------------------------------------
void CNPC_CScanner::SetInspectTargetToPos(const Vector &vInspectPos,
                                          float fInspectDuration) {
  ClearInspectTarget();
...
void CNPC_CScanner::InputShouldInspect(inputdata_t &inputdata) {
  ...
  if (!m_bShouldInspect) {
    if (GetEnemy() == GetTarget()) SetEnemy(NULL);
    ClearInspectTarget();
    SetTarget(NULL);
    SpotlightDestroy();
  }
...
// Input  : String name or classname of the entity to inspect.
//-----------------------------------------------------------------------------
void CNPC_CScanner::InspectTarget(inputdata_t &inputdata,
                                  ScannerFlyMode_t eFlyMode) {
  CBaseEntity *pEnt = gEntList.FindEntityGeneric(NULL, inputdata.value.String(),
                                                 this, inputdata.pActivator);
...
bool CNPC_CScanner::MovingToInspectTarget(void) {
  // If we're flying to a photograph target and the photo isn't yet taken, we're
  // still moving to it
...
void CNPC_CScanner::GatherConditions(void) {
  ...
  if (HaveInspectTarget() && gpGlobals->curtime > m_fInspectEndTime &&
      m_nFlyMode != SCANNER_FLY_FOLLOW) {
  ...
  if ((HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_DANGER)) &&
    ...
    if (pSound) {
      ...
      if (pSound->m_hOwner != NULL) {
        ...
        if (IRelationType(pSound->m_hOwner) != D_LI) {
          ...
          if (FVisible(pSound->m_hOwner)) {
            SetInspectTargetToEnt(pSound->m_hOwner,
                                  SCANNER_SOUND_INSPECT_LENGTH);
          }
        }
      } else {
      ...
      m_nFlyMode =
          (random->RandomInt(0, 2) == 0) ? SCANNER_FLY_SPOT : SCANNER_FLY_PHOTO;
    }
  }
  ...
  if (gpGlobals->curtime > m_fCheckCitizenTime &&
      HaveInspectTarget() == false) {
    CBaseEntity *pBestEntity = BestInspectTarget();

    if (pBestEntity != NULL) {
      SetInspectTargetToEnt(pBestEntity, SCANNER_CIT_INSPECT_LENGTH);
      m_nFlyMode =
          (random->RandomInt(0, 3) == 0) ? SCANNER_FLY_SPOT : SCANNER_FLY_PHOTO;
      SetCondition(COND_CSCANNER_HAVE_INSPECT_TARGET);
    }
  }
  ...
  if (gpGlobals->curtime > m_fCheckHintTime && HaveInspectTarget() == false) {
    SetHintNode(CAI_HintManager::FindHint(this, HINT_WORLD_WINDOW, 0,
                                          SCANNER_CIT_INSPECT_FLY_DIST));
  ...
  if (m_hSpotlightTarget != NULL && HaveInspectTarget() &&
      m_hSpotlightTarget->GetSmoothedVelocity().Length() < 25) {
    // If I have a target entity, check my spotlight against the
    // actual position of the entity
    if (GetTarget()) {
      float fInspectDist =
          (m_vSpotlightTargetPos - m_vSpotlightCurrentPos).Length();
      if (fInspectDist < 100) {
  ...
  if (m_nFlyMode == SCANNER_FLY_PHOTO) {
    // Make sure I have something to photograph and I'm ready to photograph and
    // I'm not moving to fast
    if (gpGlobals->curtime > m_fNextPhotographTime && HaveInspectTarget() &&
        GetCurrentVelocity().LengthSqr() < (64 * 64)) {
      // Check that I'm in the right distance range
      float fInspectDist =
          (InspectTargetPosition() - GetAbsOrigin()).Length2D();
      ...
      if (fInspectDist > SCANNER_PHOTO_NEAR_DIST &&
        ...
        if (UTIL_AngleDiff(GetAbsAngles().y, VecToYaw(InspectTargetPosition() -
                                                      GetAbsOrigin())) < 4.0f) {
          trace_t tr;
          AI_TraceLine(GetAbsOrigin(), InspectTargetPosition(), MASK_BLOCKLOS,
...
Disposition_t CNPC_CScanner::IRelationType(CBaseEntity *pTarget) {
  ...
  if (pTarget->Classify() == CLASS_PLAYER) {
    if (GlobalEntity_GetState("gordon_precriminal") == GLOBAL_ON) return D_NU;
  }
...
int CNPC_CScanner::SelectSchedule(void) {
  ...
  if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE)) {
    if (IsHeldByPhyscannon()) return SCHED_SMALL_FLINCH;
    ...
    if (m_NPCState == NPC_STATE_ALERT) {
      ...
      if (random->RandomInt(0, 10) < 4) return SCHED_SMALL_FLINCH;
    }
  }
  ...
  if (GetEnemy() != NULL && GetEnemy()->IsAlive() && m_bShouldInspect) {
    ...
    if (HasCondition(COND_CAN_MELEE_ATTACK1)) {
      if (random->RandomInt(0, 1)) return SCHED_CSCANNER_ATTACK_FLASH;
  ...
  if (HaveInspectTarget()) {
    ...
    if (m_nFlyMode == SCANNER_FLY_CHASE) {
      m_nFlyMode =
          (random->RandomInt(0, 1) == 0) ? SCANNER_FLY_SPOT : SCANNER_FLY_PHOTO;
    }
...
void CNPC_CScanner::SpotlightCreate(void) {
  ...
  // YWB:  Because the scanner only moves the target during think, make sure we
  // interpolate over 0.1 sec instead of every tick!!!
  ...
  // Set the temporary spawnflag on the beam so it doesn't save (we'll recreate
  // it on restore)
...
Vector CNPC_CScanner::SpotlightTargetPos(void) {
  ...
  // ----------------------------------------------
    ...
    m_vSpotlightTargetPos.x +=
        noiseScale * sin(noiseMod.x * gpGlobals->curtime + noiseMod.x);
    m_vSpotlightTargetPos.y +=
        noiseScale * cos(noiseMod.y * gpGlobals->curtime + noiseMod.y);
    m_vSpotlightTargetPos.z -=
        fabs(noiseScale * cos(noiseMod.z * gpGlobals->curtime + noiseMod.z));
...
Vector CNPC_CScanner::SpotlightCurrentPos(void) {
  ...
  // -------------------------------------------------
  float fBeamDist =
      (m_hSpotlightTarget->GetLocalOrigin() - GetLocalOrigin()).Length();
  ...
  m_vSpotlightAngVelocity =
      (myDecay * m_vSpotlightAngVelocity + (1 - myDecay) * vNewAngVelocity);
  ...
  AI_TraceLine(GetAbsOrigin(), vTraceEnd, MASK_OPAQUE, this,
...
void CNPC_CScanner::SpotlightUpdate(void) {
  ...
  if ((m_nFlyMode != SCANNER_FLY_SPOT) && (m_nFlyMode != SCANNER_FLY_PATROL) &&
  ...
  // --------------------------------------------------------------
  Vector vTargetDir =
      (m_vSpotlightCurrentPos - m_hSpotlightTarget->GetLocalOrigin());
  ...
  m_hSpotlightTarget->m_vSpotlightDir = m_hSpotlightTarget->GetLocalOrigin() -
                                        m_hSpotlightTarget->m_vSpotlightOrg;
  float flBeamLength = VectorNormalize(m_hSpotlightTarget->m_vSpotlightDir);
  m_flSpotlightCurLength =
      (0.80 * m_flSpotlightCurLength) + (0.2 * flBeamLength);
  ...
  if (m_flSpotlightCurLength > 2 * m_flSpotlightMaxLength) {
    ...
    m_hSpotlightTarget->SetRenderColorA(
        (1 - ((m_flSpotlightCurLength - m_flSpotlightMaxLength) /
...
void CNPC_CScanner::TakePhoto(void) {
  ...
  if (GetTarget()) {
    ...
    if (GetTarget()->MyNPCPointer() != NULL) {
      ...
      GetTarget()->MyNPCPointer()->DispatchInteraction(
          g_interactionScannerInspectBegin, NULL, this);
    }
  }
...
void CNPC_CScanner::BlindFlashTarget(CBaseEntity *pTarget) {
  ...
  if (dotPr > 0.5f) {
    ...
    trace_t tr;
    AI_TraceLine(GetAbsOrigin(), pTarget->EyePosition(), MASK_OPAQUE, this,
                 COLLISION_GROUP_NONE, &tr);
...
void CNPC_CScanner::AttackFlashBlind(void) {
  ...
  float fAttackDelay =
      random->RandomFloat(SCANNER_ATTACK_MIN_DELAY, SCANNER_ATTACK_MAX_DELAY);
    ...
    case TASK_CSCANNER_SPOT_INSPECT_ON: {
      if (GetTarget() == NULL) {
        ...
        if (!pNPC) {
          ...
          pNPC->DispatchInteraction(g_interactionScannerInspectBegin, NULL,
                                    this);
    ...
    case TASK_CSCANNER_SPOT_INSPECT_WAIT: {
      if (GetTarget() == NULL) {
        ...
        if (!pNPC) {
          ...
          pNPC->DispatchInteraction(g_interactionScannerInspectHandsUp, NULL,
                                    this);
        }
        TaskComplete();
      }
    ...
    case TASK_CSCANNER_SPOT_INSPECT_OFF: {
      if (GetTarget() == NULL) {
        ...
        if (!pNPC) {
          ...
          pNPC->DispatchInteraction(g_interactionScannerInspectDone, NULL,
                                    this);
...
char *CNPC_CScanner::GetScannerSoundPrefix(void) {
  if (m_bIsClawScanner) return "NPC_SScanner";
...
void CNPC_CScanner::AdjustScannerVelocity(void) {
  if (m_bIsClawScanner) {
    m_vCurrentVelocity *=
        (1 + sin((gpGlobals->curtime + m_flFlyNoiseBase) * 2.5f) * .1);
  }
}
...
bool CNPC_CScanner::OverrideMove(float flInterval) {
  ...
  if (m_nFlyMode == SCANNER_FLY_DIVE) {
    ...
    if (!GetNavigator()->IsGoalActive() ||
        (GetNavigator()->GetCurWaypointFlags() | bits_WP_TO_PATHCORNER)) {
      // Select move target
      if (GetTarget() != NULL) {
    ...
    if (pMoveTarget || HaveInspectTarget()) {
      trace_t tr;
      AI_TraceHull(GetAbsOrigin(), vMoveTargetPos, GetHullMins(), GetHullMaxs(),
                   MASK_NPCSOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);

      float fTargetDist =
          (1.0f - tr.fraction) * (GetAbsOrigin() - vMoveTargetPos).Length();

      if ((tr.m_pEnt == pMoveTarget) || (fTargetDist < 50)) {
        if (g_debug_cscanner.GetBool()) {
          NDebugOverlay::Line(GetLocalOrigin(), vMoveTargetPos, 0, 255, 0, true,
                              0);
          NDebugOverlay::Cross3D(tr.endpos, Vector(-5, -5, -5), Vector(5, 5, 5),
                                 0, 255, 0, true, 0.1);
        }
        ...
        // HANDY DEBUG TOOL
        if (g_debug_cscanner.GetBool()) {
          NDebugOverlay::Line(GetLocalOrigin(), vMoveTargetPos, 255, 0, 0, true,
                              0);
          NDebugOverlay::Cross3D(tr.endpos, Vector(-5, -5, -5), Vector(5, 5, 5),
                                 255, 0, 0, true, 0.1);
        }
...
//-----------------------------------------------------------------------------
void CNPC_CScanner::MoveToTarget(float flInterval,
                                 const Vector &vecMoveTarget) {
  ...
  // Get the relationship between my current velocity and the way I want to be
  // going.
  ...
  if (flDot > 0.25) {
    ...
    // Have a harder time correcting my course if I'm currently flying away from
    // my target.
    myAccel = 128;
  }
...
void CNPC_CScanner::MoveToSpotlight(float flInterval) {
  ...
  // float flDesiredDist = SCANNER_SPOTLIGHT_NEAR_DIST + ( (
  // SCANNER_SPOTLIGHT_FAR_DIST - SCANNER_SPOTLIGHT_NEAR_DIST ) / 2 );
  ...
  Vector idealPos = IdealGoalForMovement(vTargetPos, GetAbsOrigin(),
                                         GetGoalDistance(), flIdealHeightDiff);
          ...
          gpGlobals->curtime > m_fInspectEndTime)
  {
          ...
          {
                  Vector			vNearestDir =
  (pNearest->GetLocalOrigin() - GetLocalOrigin()); if (vNearestDir.Length() <
  SCANNER_SQUAD_FLY_DIST)
                  {
                          vNearestDir		= pNearest->GetLocalOrigin() -
  GetLocalOrigin(); VectorNormalize(vNearestDir); vFlyDirection  -=
  0.5*vNearestDir;
                  }
          }
  }
...
float CNPC_CScanner::GetGoalDistance(void) {
  if (m_flGoalOverrideDistance != 0.0f) return m_flGoalOverrideDistance;

  switch (m_nFlyMode) {
    case SCANNER_FLY_PHOTO:
      return (SCANNER_PHOTO_NEAR_DIST +
              ((SCANNER_PHOTO_FAR_DIST - SCANNER_PHOTO_NEAR_DIST) / 2));
      break;

    case SCANNER_FLY_SPOT: {
      float goalDist =
          (SCANNER_SPOTLIGHT_NEAR_DIST +
           ((SCANNER_SPOTLIGHT_FAR_DIST - SCANNER_SPOTLIGHT_NEAR_DIST) / 2));
      if (IsEnemyPlayerInSuit()) {
        goalDist *= 0.5;
      }
...
void CNPC_CScanner::MoveToPhotograph(float flInterval) {
  if (HaveInspectTarget() == false) return;

  // float flDesiredDist = SCANNER_PHOTO_NEAR_DIST + ( ( SCANNER_PHOTO_FAR_DIST
  // - SCANNER_PHOTO_NEAR_DIST ) / 2 );

  Vector idealPos = IdealGoalForMovement(
      InspectTargetPosition(), GetAbsOrigin(), GetGoalDistance(), 32.0f);
...
//-----------------------------------------------------------------------------
bool CNPC_CScanner::HandleInteraction(int interactionType, void *data,
                                      CBaseCombatCharacter *pSourceEnt) {
  //	TODO:: - doing this by just an interrupt contition would be a lot
  // better!
  if (interactionType == g_interactionScannerSupportEntity) {
    ...
    if (GetEnemy() == NULL && !HaveInspectTarget()) {
      ...
      float fTargetDist =
          (pTarget->GetLocalOrigin() - GetLocalOrigin()).Length();

      if (fTargetDist < SCANNER_SQUAD_HELP_DIST) {
        float fInspectTime = (((CNPC_CScanner *)pSourceEnt)->m_fInspectEndTime -
                              gpGlobals->curtime);
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ROUTE_SEARCH_TIME			5"  // Spend 5
                                                                    // seconds
                                                                    // trying to
                                                                    // build a
                                                                    // path if
...
DEFINE_SCHEDULE(SCHED_CSCANNER_SPOTLIGHT_INSPECT_CIT,
                ...
                "			2"
                "		TASK_SPEAK_SENTENCE			"
                "		1"  // Hands on head or Show Armband!
                "		TASK_WAIT				"
                "			1"
