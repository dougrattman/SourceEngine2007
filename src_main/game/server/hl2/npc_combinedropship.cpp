...
#define DROPSHIP_LANDING_HOVER_TIME \
  5  // Time to spend on the ground if we have no troops to drop
...
#define DROPSHIP_MIN_CHASE_DIST_DIFF \
...
ConVar sk_npc_dmg_dropship("sk_npc_dmg_dropship", "5", FCVAR_NONE,
...
Activity ACT_DROPSHIP_DEPLOY_IDLE;  // idle on the ground with door open. Troops
                                    // are leaving.
...
enum LandingState_t {
  ...
  // Hovering, which we're saying is a type of landing since there's so much
  // landing code to leverage
  LANDING_START_HOVER,
...
class CNPC_CombineDropship : public CBaseHelicopter {
 ...
 public:
  ...
  void DropMine(void);
  void UpdateContainerGunFacing(Vector &vecMuzzle, Vector &vecToTarget,
                                Vector &vecAimDir, float *flTargetRange);
  ...
  Vector GetDropoffFinishPosition(Vector vecOrigin, CAI_BaseNPC *pNPC,
 ...
 private:
  ...
  void UpdateRotorWashVolume(CSoundPatch *pRotorSound, float flVolume,
                             float flDeltaTime);
 ...
 private:
  ...
  int m_totalMinesToDrop;  // total # of mines to drop as a group (based upon
                           // triggered input)
 ...
 protected:
  ...
  // again.
  static int m_poseBody_Accel, m_poseBody_Sway, m_poseCargo_Body_Accel,
      m_poseCargo_Body_Sway, m_poseWeapon_Pitch, m_poseWeapon_Yaw;
...
// Should we trigger a damage effect?
//-----------------------------------------------------------------------------
inline bool CCombineDropshipContainer::ShouldTriggerDamageEffect(
...
void CCombineDropshipContainer::CreateCorpse() {
  ...
  CollisionProp()->RandomPointInBounds(vecNormalizedMins, vecNormalizedMaxs,
                                       &vecAbsPoint);
  te->Explosion(filter, 0.0f, &vecAbsPoint, g_sModelIndexFireball,
                random->RandomInt(4, 10), random->RandomInt(8, 15),
                TE_EXPLFLAG_NOPARTICLES, 100, 0);

  // Break into chunks
  Vector angVelocity;
  QAngleToAngularImpulse(GetLocalAngularVelocity(), angVelocity);
  PropBreakableCreateAll(GetModelIndex(), VPhysicsGetObject(), GetAbsOrigin(),
                         GetAbsAngles(), GetAbsVelocity(), angVelocity, 1.0,
                         250, COLLISION_GROUP_NPC, this);
...
void CCombineDropshipContainer::Event_Killed(const CTakeDamageInfo &info) {
  if (GetOwnerEntity()) {
    CNPC_CombineDropship *pDropship =
        assert_cast<CNPC_CombineDropship *>(GetOwnerEntity());
...
int CCombineDropshipContainer::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() & DMG_BLAST) {
    ...
    if ((info.GetInflictor() != m_hLastInflictor) ||
        (gpGlobals->curtime != m_flLastHitTime)) {
  ...
  if (nPrevHealth != GetHealth()) {
    ...
    if (ShouldTriggerDamageEffect(nPrevHealth, MAX_EXPLOSIONS)) {
      ExplosionCreate(dmgInfo.GetDamagePosition(), vec3_angle, this, 1000,
                      500.0f,
                      SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS |
                          SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE,
                      0);
      UTIL_ScreenShake(dmgInfo.GetDamagePosition(), 25.0, 150.0, 1.0, 750.0f,
                       SHAKE_START);
...
void CCombineDropshipContainer::AddSmokeTrail(const Vector &vecPos) {
  ...
  ExplosionCreate(vecPos, vec3_angle, this, 1000, 500.0f,
                  SF_ENVEXPLOSION_NODAMAGE | SF_ENVEXPLOSION_NOSPARKS |
                      SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NOSMOKE,
                  0);
...
BEGIN_DATADESC(CNPC_CombineDropship)
  ...
  DEFINE_FIELD(m_flTimeTakeOff, FIELD_TIME),
      ...
      DEFINE_KEYFIELD(m_sRollermineTemplate, FIELD_STRING,
                      "RollermineTemplate"),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StopWaitingForDropoff",
      ...
      DEFINE_OUTPUT(m_OnContainerShotDownBeforeDropoff,
                    "OnCrateShotDownBeforeDropoff"),
      DEFINE_OUTPUT(m_OnContainerShotDownAfterDropoff,
...
void CNPC_CombineDropship::Spawn(void) {
  ...
  InitPathingData(DROPSHIP_ARRIVE_DIST, DROPSHIP_MIN_CHASE_DIST_DIFF,
                  DROPSHIP_AVOID_DIST);
  ...
  switch (m_iCrateType) {
    ...
    case CRATE_SOLDIER:
      m_hContainer =
          (CBaseAnimating *)CreateEntityByName("prop_dropship_container");
      if (m_hContainer) {
        ...
        if (pPhysicsObject) {
          ...
          pPhysicsObject->UpdateShadow(m_hContainer->GetAbsOrigin(),
                                       m_hContainer->GetAbsAngles(), false, 0);
        }
        ...
        m_iAttachmentTroopDeploy =
            m_hContainer->LookupAttachment("deploy_landpoint");
        m_iAttachmentDeployStart =
            m_hContainer->LookupAttachment("Deploy_Start");
        m_iMuzzleAttachment = m_hContainer->LookupAttachment("muzzle");
        m_iMachineGunBaseAttachment =
            m_hContainer->LookupAttachment("gun_base");
        // NOTE: gun_ref must have the same position as gun_base, but rotates
        // with the gun
    ...
    case CRATE_APC: {
      m_soldiersToDrop = 0;
      m_hContainer = (CBaseAnimating *)gEntList.FindEntityByName(
          NULL, m_iszAPCVehicleName);
      if (!m_hContainer) {
    ...
    case CRATE_JEEP:
      m_hContainer =
          (CBaseAnimating *)CreateEntityByName("prop_dynamic_override");
      if (m_hContainer) {
  ...
  //!!!HACKHACK
  // This tricks the AI code that constantly complains that the vehicle has no
  // schedule.
  ...
  if (!m_sRollermineTemplateData) {
    ...
    if (m_sRollermineTemplate != NULL_STRING) {
      ...
      m_sRollermineTemplateData =
          Templates_FindByTargetName(STRING(m_sRollermineTemplate));
      ...
      for (int i = 0; i < DROPSHIP_MAX_SOLDIERS; i++) {
        if (m_sNPCTemplate[i] != NULL_STRING) {
          if (m_sNPCTemplateData[i] == NULL_STRING) {
            m_sNPCTemplateData[i] =
                Templates_FindByTargetName(STRING(m_sNPCTemplate[i]));
          }
          if (m_sNPCTemplateData[i] != NULL_STRING) {
...
void CNPC_CombineDropship::Flight(void) {
  ...
  // We assume CRATE_NONE means the dropship just picked up some random phys
  // object.
  if (m_hContainer != NULL &&
      (m_iCrateType == CRATE_SOLDIER || m_iCrateType == CRATE_NONE)) {
  ...
  if (bRunFlight) {
    ...
    // NDebugOverlay::Line( GetAbsOrigin(), GetAbsOrigin() + deltaPos, 0, 255,
    // 0, true, 0.1f );
    ...
    accel.z = 2.0 *
              (deltaPos.z - GetAbsVelocity().z * dt + 0.5 * 384 * dt * dt) /
              (dt * dt);

    float flDistFromPath = 0.0f;
    Vector vecPoint, vecDelta;
    if (IsOnPathTrack() && GetLandingState() == LANDING_NO) {
      // Also, add in a little force to get us closer to our current line
      // segment if we can
    ...
    QAngle goalAngAccel;
    goalAngAccel.x = 2.0 *
                     (AngleDiff(goalPitch, AngleNormalize(GetLocalAngles().x)) -
                      GetLocalAngularVelocity().x * dt) /
                     (dt * dt);
    goalAngAccel.y = 2.0 *
                     (AngleDiff(goalYaw, AngleNormalize(GetLocalAngles().y)) -
                      GetLocalAngularVelocity().y * dt) /
                     (dt * dt);
    goalAngAccel.z = 2.0 *
                     (AngleDiff(goalRoll, AngleNormalize(GetLocalAngles().z)) -
    ...
    m_flForce =
        m_flForce * 0.8 +
        (accel.z + fabs(accel.x) * 0.1 + fabs(accel.y) * 0.1) * 0.1 * 0.2;
  ...
  if (GetLandingState() == LANDING_DESCEND ||
      GetLandingState() == LANDING_TOUCHDOWN ||
      GetLandingState() == LANDING_UNLOADING ||
      GetLandingState() == LANDING_UNLOADED || IsHovering()) {
    finspeed = -60;
  }
  ...
  SetPoseParameter(poseBodyAccel,
                   EdgeLimitPoseParameter(poseBodyAccel, curFinAccel));
  ...
  SetPoseParameter(poseBodySway,
                   EdgeLimitPoseParameter(poseBodySway, curFinSway));
...
void CNPC_CombineDropship::InitializeRotorSound(void) {
  ...
  m_pNearRotorSound = controller.SoundCreate(
      filter, entindex(), "NPC_CombineDropship.NearRotorLoop");
  m_pRotorOnGroundSound = controller.SoundCreate(
      filter, entindex(), "NPC_CombineDropship.OnGroundRotorLoop");
  m_pDescendingWarningSound = controller.SoundCreate(
      filter, entindex(), "NPC_CombineDropship.DescendingWarningLoop");
  m_pCannonSound = controller.SoundCreate(filter, entindex(),
...
//------------------------------------------------------------------------------
void CNPC_CombineDropship::UpdateRotorWashVolume(CSoundPatch *pRotorSound,
                                                 float flVolume,
                                                 float flDeltaTime) {
  if (!pRotorSound) return;
...
void CNPC_CombineDropship::UpdateRotorWashVolume() {
  ...
  if (m_pRotorSound) {
    UpdateRotorWashVolume(
        m_pRotorSound,
        m_engineThrust * GetRotorVolume() * (1.0f - flNearFactor), 3.0f);
  }

  if (m_pNearRotorSound) {
    UpdateRotorWashVolume(m_pNearRotorSound,
                          m_engineThrust * GetRotorVolume() * flNearFactor,
                          3.0f);
  }
}
...
void CNPC_CombineDropship::UpdateRotorSoundPitch(int iPitch) {
  ...
  if (m_pRotorOnGroundSound) {
    controller.SoundChangePitch(m_pRotorOnGroundSound, iPitch + rotorPitch,
                                0.1);
  }
...
//------------------------------------------------------------------------------
// Purpose :
// Input   : bHover - If true, means we're landing on a hover point, not the
// ground Output  :
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// Purpose : Drop mine inputs... done this way so generic path_corners can be
// used Input   : Output  :
//------------------------------------------------------------------------------
...
void CNPC_CombineDropship::InputDropStrider(inputdata_t &inputdata) {
  if (!m_hContainer || !FClassnameIs(m_hContainer, "npc_strider")) {
    Warning(
        "npc_combinedropship %s was told to drop Strider, but isn't carrying "
        "one!\n",
...
void CNPC_CombineDropship::InputDropAPC(inputdata_t &inputdata) {
  if (!m_hContainer || !FClassnameIs(m_hContainer, "prop_vehicle_apc")) {
    Warning(
        "npc_combinedropship %s was told to drop APC, but isn't carrying "
        "one!\n",
        STRING(GetEntityName()));
    return;
  }
...
void CNPC_CombineDropship::DropSoldierContainer() {
  ...
  if (m_hLastTroopToLeave) {
    CTakeDamageInfo dmgInfo(this, this, vec3_origin,
                            m_hContainer->GetAbsOrigin(),
                            m_hLastTroopToLeave->GetMaxHealth(), DMG_GENERIC);
...
void CNPC_CombineDropship::InputPickup(inputdata_t &inputdata) {
  ...
  if (m_hContainer) {
    Warning(
        "npc_combinedropship %s was told to pickup, but is already carrying "
        "something.\n",
        STRING(GetEntityName()));
    return;
  }
  ...
  if (iszTargetName == NULL_STRING) {
    Warning(
        "npc_combinedropship %s tried to pickup with no specified pickup "
        "target.\n",
        STRING(GetEntityName()));
    return;
  }
...
void CNPC_CombineDropship::SetLandingState(LandingState_t landingState) {
  if (landingState == m_iLandState) return;

  if (m_pDescendingWarningSound) {
    CSoundEnvelopeController &controller =
        CSoundEnvelopeController::GetController();
    if ((landingState == LANDING_DESCEND) ||
        (landingState == LANDING_TOUCHDOWN) ||
        (landingState == LANDING_UNLOADING) ||
        (landingState == LANDING_UNLOADED) ||
        (landingState == LANDING_HOVER_DESCEND)) {
      controller.SoundChangeVolume(m_pDescendingWarningSound,
                                   m_bSuppressSound ? 0.0f : 1.0f, 0.3f);
    } else {
...
bool CNPC_CombineDropship::IsHovering() {
  ...
  if (GetLandingState() > LANDING_START_HOVER &&
      GetLandingState() < LANDING_END_HOVER) {
    bIsHovering = true;
  }
...
void CNPC_CombineDropship::UpdateGroundRotorWashSound(float flAltitude) {
  float flVolume =
      RemapValClamped(flAltitude, DROPSHIP_GROUND_WASH_MIN_ALTITUDE,
                      DROPSHIP_GROUND_WASH_MAX_ALTITUDE, 1.0f, 0.0f);
  UpdateRotorWashVolume(m_pRotorOnGroundSound, flVolume * GetRotorVolume(),
                        0.5f);
}
...
void CNPC_CombineDropship::PrescheduleThink(void) {
  ...
  if (m_debugOverlays & OVERLAY_NPC_KILL_BIT) {
    if (m_hContainer) {
      CTakeDamageInfo dmgInfo(this, this, vec3_origin, vec3_origin, 1000,
                              DMG_BLAST);
  ...
  switch (GetLandingState()) {
    case LANDING_NO: {
      if (IsActivityFinished() &&
          (GetActivity() != ACT_DROPSHIP_FLY_IDLE_EXAGG &&
           GetActivity() != ACT_DROPSHIP_FLY_IDLE_CARGO)) {
        if (m_hContainer) {
          SetIdealActivity((Activity)ACT_DROPSHIP_FLY_IDLE_CARGO);
        } else {
          SetIdealActivity((Activity)ACT_DROPSHIP_FLY_IDLE_EXAGG);
...
#define ALTITUDE_CAP 512.0f
      ...
      if (flAltitude < 72) {
        ...
        QAngle angles = GetLocalAngles();
        angles.x = m_existPitch +
                   (sin(gpGlobals->curtime * 3.5f) * DROPSHIP_MAX_LAND_TILT);
        angles.z = m_existRoll +
                   (sin(gpGlobals->curtime * 3.75f) * DROPSHIP_MAX_LAND_TILT);
        SetLocalAngles(angles);
      }

      DoRotorWash();

      // place danger sounds 1 foot above ground to get troops to scatter if
      // they are below dropship
      ...
      AI_TraceHull(vecBottom, pEndPoint, vecBBoxMin, vecBBoxMax, MASK_SOLID,
      ...
      if (!bStillOrienting &&
          ((flAltitude <= 0.5f) || (m_iCrateType == CRATE_APC))) {
        if (IsHovering()) {
    ...
    case LANDING_HOVER_TOUCHDOWN: {
      ...
      */
      ...
      if (m_soldiersToDrop && m_hContainer) {
        ...
        float flHoverTime =
            (m_iCrateType >= 0) ? DROPSHIP_LANDING_HOVER_TIME : 0.5f;
        m_flTimeTakeOff = gpGlobals->curtime + flHoverTime;
      }
    } break;

    case LANDING_UNLOADING: {
      // If we've got no specified takeoff time, we're still waiting for troops
      // to exit. Idle.
      if (!m_flTimeTakeOff) {
        float idleVolume = 0.2f;
        m_engineThrust = UTIL_Approach(idleVolume, m_engineThrust, 0.04f);
        if (m_engineThrust > idleVolume) {
          // Make sure we're kicking up dust/water as long as engine thrust is
          // up
          DoRotorWash();
        }

        // If we've lost the last troop who was leaving, he probably got killed
        // during dustoff.
          ...
          */
          {
            ...
            if (pPhysicsObject) {
              pPhysicsObject->EnableMotion(true);
              pPhysicsObject->SetShadow(1e4, 1e4, false, false);
              pPhysicsObject->UpdateShadow(GetAbsOrigin(), GetAbsAngles(),
                                           false, 0);
            }
...
void CNPC_CombineDropship::SpawnTroop(void) {
  ...
  // Are we fully unloaded? If so, take off. Otherwise, tell the next troop to
  // exit.
  if (m_iCurrentTroopExiting >= m_soldiersToDrop ||
      m_sNPCTemplateData[m_iCurrentTroopExiting] == NULL_STRING) {
    // We're done, take off.
    m_flTimeTakeOff = gpGlobals->curtime + 0.5;
    return;
  }
  ...
  // HACK: This is a nasty piece of work. We want to make sure the deploy end is
  // clear, and has enough room with our deploying NPC, but we don't want to
  // create the NPC unless it's clear, and we don't know how much room he needs
  // without spawning him. So, because we know that we only ever spawn combine
  // soldiers at the moment, we'll just use their hull. HACK: Add some bloat
  // because the endpoint isn't perfectly aligned with NPC end origin
  ...
  m_hContainer->GetAttachment(m_iAttachmentTroopDeploy, vecDeployEndPoint,
                              vecDeployEndAngles);
  vecDeployEndPoint =
      GetDropoffFinishPosition(vecDeployEndPoint, NULL, vecNPCMins, vecNPCMaxs);
  ...
  AI_TraceHull(vecDeployEndPoint, vecDeployEndPoint, vecNPCMins, vecNPCMaxs,
               MASK_SOLID, &filter, &tr);
  if (tr.m_pEnt) {
    if (g_debug_dropship.GetInt() == 2) {
      NDebugOverlay::Box(vecDeployEndPoint, vecNPCMins, vecNPCMaxs, 255, 0, 0,
                         64, 0.5);
    }
  ...
  if (g_debug_dropship.GetInt() == 2) {
    NDebugOverlay::Box(vecDeployEndPoint, vecNPCMins, vecNPCMaxs, 0, 255, 0, 64,
                       0.5);
  }
  ...
  QAngle vecSpawnAngles;
  m_hContainer->GetAttachment(m_iAttachmentDeployStart, vecSpawnOrigin,
                              vecSpawnAngles);

  // Spawn the templated NPC
  CBaseEntity *pEntity = NULL;
  MapEntity_ParseEntity(
      pEntity, STRING(m_sNPCTemplateData[m_iCurrentTroopExiting]), NULL);
  ...
  CBaseEntity *pBlocker = CEntityBlocker::Create(vecDeployEndPoint, vecNPCMins,
                                                 vecNPCMaxs, pNPC, true);
  g_EventQueue.AddEvent(pBlocker, "Kill", 2.5, this, this);
  if (g_debug_dropship.GetInt() == 2) {
    NDebugOverlay::Box(vecDeployEndPoint, vecNPCMins, vecNPCMaxs, 255, 255, 255,
                       64, 2.5);
  }
  ...
  CAI_ScriptedSequence *pSequence =
  ...
  pSequence->KeyValue(
      "OnEndSequence",
      UTIL_VarArgs("%s,NPCFinishDustoff,%s,0,-1", STRING(GetEntityName()),
                   STRING(pNPC->GetEntityName())));
  pSequence->SetAbsOrigin(vecSpawnOrigin);
  pSequence->SetAbsAngles(vecSpawnAngles);
  pSequence->AddSpawnFlags(SF_SCRIPT_NOINTERRUPT | SF_SCRIPT_HIGH_PRIORITY |
...
//-----------------------------------------------------------------------------
// Purpose: Returns a safe position above/below the specified origin for the NPC
// to finish it's dropoff on Input  : vecOrigin -
//-----------------------------------------------------------------------------
Vector CNPC_CombineDropship::GetDropoffFinishPosition(Vector vecOrigin,
                                                      CAI_BaseNPC *pNPC,
                                                      Vector vecMins,
                                                      Vector vecMaxs) {
  ...
  trace_t tr;
  AI_TraceHull(vecOrigin + Vector(0, 0, 32), vecOrigin, vecMins, vecMaxs,
  ...
  if (tr.fraction < 1.0) {
    ...
    AI_TraceHull(vecOrigin + Vector(0, 0, 32), vecOrigin, vecMins, vecMaxs,
                 MASK_SOLID, pNPC, COLLISION_GROUP_NONE, &tr);
    if (!tr.startsolid) return (tr.endpos + Vector(0, 0, 1));
...
void CNPC_CombineDropship::InputNPCFinishDustoff(inputdata_t &inputdata) {
  CBaseEntity *pEnt =
      gEntList.FindEntityByName(NULL, inputdata.value.StringID(), NULL,
  ...
  Vector vecOrigin = GetDropoffFinishPosition(pNPC->GetAbsOrigin(), pNPC,
  ...
  if (m_sDustoffPoints[m_iCurrentTroopExiting - 1] != NULL_STRING) {
    pDustoff = gEntList.FindEntityByName(
        NULL, m_sDustoffPoints[m_iCurrentTroopExiting - 1]);
    if (!pDustoff) {
      Warning("npc_combinedropship %s couldn't find dustoff target named %s\n",
              STRING(GetEntityName()),
              STRING(m_sDustoffPoints[m_iCurrentTroopExiting - 1]));
    }
  }
  ...
  if (!pDustoff) {
    ...
    CSoundEnt::InsertSound(SOUND_MOVE_AWAY | SOUND_CONTEXT_COMBINE_ONLY,
                           pNPC->GetAbsOrigin(), 128, 0.1);
  } else {
    if (g_debug_dropship.GetInt() == 1) {
      NDebugOverlay::Box(pDustoff->GetAbsOrigin(), -Vector(10, 10, 10),
                         Vector(10, 10, 10), 0, 255, 0, 8, 5.0);
    }
...
float CNPC_CombineDropship::GetAltitude(void) {
  ...
  AI_TraceEntity(this, vecBottom, vecBottom - Vector(0, 0, 4096),
                 MASK_SOLID_BRUSHONLY, &tr);
...
void CNPC_CombineDropship::DropMine(void) {
  NPC_Rollermine_DropFromPoint(GetAbsOrigin(), this,
                               STRING(m_sRollermineTemplateData));
}
...
void CNPC_CombineDropship::UpdatePickupNavigation(void) {
  ...
  // NDebugOverlay::Cross3D( GetDesiredPosition(), -Vector(32,32,32),
  // Vector(32,32,32), 0, 255, 255, true, 0.1f );
}
...
void CNPC_CombineDropship::Hunt(void) {
  ...
  if (GetLandingState() == LANDING_DESCEND ||
      GetLandingState() == LANDING_LEVEL_OUT || IsHovering()) {
    if (m_hLandTarget) {
      // We've got a land target, so match it's orientation
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::UpdateContainerGunFacing(Vector &vecMuzzle,
                                                    Vector &vecToTarget,
                                                    Vector &vecAimDir,
                                                    float *flTargetRange) {
  Assert(m_hContainer);
  ...
  if (quadTarget > vecBarrelPos.LengthSqr()) {
    ...
    // The rotation we need is the opposite of the rotation that moves the
    // target along the surface of that sphere to intersect with the gun's
    // shooting direction To calculate that rotation, we simply calculate the
    // intersection of the ray coming out of the barrel with the target sphere
    // (that's the new target position) and use atan2() to get angles

    // angles from target pos to center
    float targetToCenterYaw = atan2(target.y, target.x);
    float centerToGunYaw = atan2(
        vecBarrelPos.y, sqrt(quadTarget - (vecBarrelPos.y * vecBarrelPos.y)));

    float targetToCenterPitch = atan2(target.z, sqrt(quadTargetXY));
    float centerToGunPitch = atan2(
        -vecBarrelPos.z, sqrt(quadTarget - (vecBarrelPos.z * vecBarrelPos.z)));

    QAngle angles;
    angles.Init(RAD2DEG(targetToCenterPitch + centerToGunPitch),
                RAD2DEG(targetToCenterYaw + centerToGunYaw), 0);

    float flNewAngle = AngleNormalize(UTIL_ApproachAngle(
        angles.x, m_hContainer->GetPoseParameter(m_poseWeapon_Pitch),
        DROPSHIP_GUN_SPEED));
    m_hContainer->SetPoseParameter(m_poseWeapon_Pitch, flNewAngle);

    flNewAngle = AngleNormalize(UTIL_ApproachAngle(
        angles.y, m_hContainer->GetPoseParameter(m_poseWeapon_Yaw),
        DROPSHIP_GUN_SPEED));
...
bool CNPC_CombineDropship::FireCannonRound(void) {
  ...
  // Update the container gun, and get the vector to the enemy, and the gun's
  ...
  g_pEffects->MuzzleFlash(vecMuzzle, vecAimAngles,
                          random->RandomFloat(5.0f, 7.0f),
                          MUZZLEFLASH_TYPE_GUNSHIP);
  ...
  FireBullets(1, vecMuzzle, vecAimDir, VECTOR_CONE_2DEGREES, 8192, ammoType, 1,
              -1, -1, sk_npc_dmg_dropship.GetInt());
...
//-----------------------------------------------------------------------------
void CNPC_CombineDropship::MakeTracer(const Vector &vecTracerSrc,
                                      const trace_t &tr, int iTracerType) {
  switch (iTracerType) {
    case TRACER_LINE: {
      float flTracerDist;
      Vector vecDir;
      ...
      UTIL_Tracer(vecTracerSrc, tr.endpos, 0, TRACER_DONT_USE_ATTACHMENT, 16000,
                  true, "GunshipTracer");
    } break;
