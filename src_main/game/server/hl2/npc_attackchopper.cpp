...
#define HELICOPTER_CHUNK_COCKPIT \
  "models/gibs/helicopter_brokenpiece_04_cockpit.mdl"
#define HELICOPTER_CHUNK_TAIL \
...
#define CHOPPER_MIN_CHASE_DIST_DIFF \
  128.0f  // Distance threshold used to determine when a target has moved enough
          // to update our navigation to it
...
#define CHOPPER_BULLRUSH_ENEMY_BOMB_DISTANCE \
  g_helicopter_bullrush_bomb_enemy_distance.GetFloat()
#define CHOPPER_BULLRUSH_ENEMY_BOMB_TIME \
  g_helicopter_bullrush_bomb_time.GetFloat()
#define CHOPPER_BULLRUSH_ENEMY_BOMB_SPEED \
  g_helicopter_bullrush_bomb_speed.GetFloat()
#define CHOPPER_BULLRUSH_SHOOTING_VERTICAL_OFFSET \
  g_helicopter_bullrush_shoot_height.GetFloat()
...
#define CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_SQ \
  (CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED * CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED)

#define CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_2 450
#define CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_2_SQ \
  (CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_2 * CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_2)

// CVars
ConVar sk_helicopter_health("sk_helicopter_health", "5600");
ConVar sk_helicopter_firingcone(
    "sk_helicopter_firingcone", "20.0", 0,
    "The angle in degrees of the cone in which the shots will be fired");
ConVar sk_helicopter_burstcount(
    "sk_helicopter_burstcount", "12", 0,
    "How many shot bursts to fire after charging up. The bigger the number, "
    "the longer the firing is");
ConVar sk_helicopter_roundsperburst("sk_helicopter_roundsperburst", "5", 0,
                                    "How many shots to fire in a single burst");

ConVar sk_helicopter_grenadedamage(
    "sk_helicopter_grenadedamage", "25.0", 0,
    "The amount of damage the helicopter grenade deals.");
ConVar sk_helicopter_grenaderadius(
    "sk_helicopter_grenaderadius", "275.0", 0,
    "The damage radius of the helicopter grenade.");
ConVar sk_helicopter_grenadeforce(
    "sk_helicopter_grenadeforce", "55000.0", 0,
    "The physics force that the helicopter grenade exerts.");
ConVar sk_helicopter_grenade_puntscale(
    "sk_helicopter_grenade_puntscale", "1.5", 0,
...
ConVar sk_npc_dmg_helicopter_to_plr(
    "sk_npc_dmg_helicopter_to_plr", "3", 0,
    "Damage helicopter shots deal to the player");
ConVar sk_npc_dmg_helicopter(
    "sk_npc_dmg_helicopter", "6", 0,
    "Damage helicopter shots deal to everything but the player");

ConVar sk_helicopter_drone_speed("sk_helicopter_drone_speed", "450.0", 0,
                                 "How fast does the zapper drone move?");

ConVar g_helicopter_chargetime(
    "g_helicopter_chargetime", "2.0", 0,
    "How much time we have to wait (on average) between the time we start "
    "hearing the charging sound + the chopper fires");
ConVar g_helicopter_bullrush_distance("g_helicopter_bullrush_distance", "5000");
ConVar g_helicopter_bullrush_bomb_enemy_distance(
    "g_helicopter_bullrush_bomb_enemy_distance", "0");
ConVar g_helicopter_bullrush_bomb_time("g_helicopter_bullrush_bomb_time", "10");
ConVar g_helicopter_idletime("g_helicopter_idletime", "3.0", 0,
                             "How much time we have to wait (on average) after "
                             "we fire before we can charge up again");
ConVar g_helicopter_maxfiringdist("g_helicopter_maxfiringdist", "2500.0", 0,
                                  "The maximum distance the player can be from "
                                  "the chopper before it stops firing");
ConVar g_helicopter_bullrush_bomb_speed(
    "g_helicopter_bullrush_bomb_speed", "850.0", 0,
    "The maximum distance the player can be from the chopper before it stops "
    "firing");
ConVar g_helicopter_bullrush_shoot_height(
    "g_helicopter_bullrush_shoot_height", "650.0", 0,
    "The maximum distance the player can be from the chopper before it stops "
    "firing");
ConVar g_helicopter_bullrush_mega_bomb_health(
    "g_helicopter_bullrush_mega_bomb_health", "0.25", 0,
    "Fraction of the health of the chopper before it mega-bombs");

ConVar g_helicopter_bomb_danger_radius("g_helicopter_bomb_danger_radius",
                                       "120");
...
#define BOMB_LIFETIME \
  2.5f  // Don't access this directly. Call GetBombLifetime();
...
#define GRENADE_HELICOPTER_MODEL \
...
// Purpose :
//------------------------------------------------------------------------------
static inline float ClampSplineRemapVal(float flValue, float flMinValue,
...
#ifdef HL2_EPISODIC
  float m_flLifetime;
  EHANDLE m_hCollisionObject;  // Pointer to object we re-enable collisions with
                               // when picked up
  bool m_bPickedUp;
...
class CHelicopterChunk : public CBaseAnimating {
 ...
 public:
  ...
  static CHelicopterChunk *CreateHelicopterChunk(const Vector &vecPos,
...
class CNPC_AttackHelicopter : public CBaseHelicopter {
 public:
  ...
  void InputDropBombAtTarget(inputdata_t &inputdata);
  void InputDropBombAtTargetAlways(inputdata_t &inputdata);
  void InputDropBombAtTargetInternal(inputdata_t &inputdata,
                                     bool bCheckFairness);
  void InputDropBombDelay(inputdata_t &inputdata);
 ...
 private:
  ...
  CBaseEntity *FindTrackBlocker(const Vector &vecViewPoint,
  ...
  void ShootAtFacingDirection(const Vector &vBasePos, const Vector &vGunDir,
  ...
  bool DoGunCharging();
  bool DoGunFiring(const Vector &vBasePos, const Vector &vGunDir,
                   const Vector &vecFireAtPosition);
  void FireElectricityGun();

  // Chooses a point within the circle of death to fire in
  void PickDirectionToCircleOfDeath(const Vector &vBasePos,
                                    const Vector &vecFireAtPosition,
                                    Vector *pResult);
  ...
  void CreateBomb(bool bCheckForFairness = true, Vector *pVecVelocity = NULL,
                  bool bMegaBomb = false);
  CGrenadeHelicopter *SpawnBombEntity(
      const Vector &vecPos,
      const Vector &vecVelocity);  // Spawns the bomb entity and sets it up

  // Deliberately aims as close as possible w/o hitting
  void AimCloseToTargetButMiss(CBaseEntity *pTarget, float flMinDist,
                               float flMaxDist, const Vector &shootOrigin,
                               Vector *pResult);

  // Pops a shot inside the circle of death using the burst rules
  void ShootInsideCircleOfDeath(const Vector &vBasePos,
                                const Vector &vecFireAtPosition);
  ...
  // Purpose :
  float CreepTowardEnemy(float flSpeed, float flMinSpeed, float flMaxSpeed,
  ...
  void ComputeAngularVelocity(const Vector &vecGoalUp,
                              const Vector &vecFacingDirection);
  void ComputeVelocity(const Vector &deltaPos, float flAdditionalHeight,
                       float flMinDistFromSegment, float flMaxDistFromSegment,
                       Vector *pVecAccel);
  void FlightDirectlyOverhead(void);

  // Methods related to computing leading distance
  float ComputeBombingLeadingDistance(float flSpeed, float flSpeedAlongPath,
                                      bool bEnemyInVehicle);
  float ComputeBullrushLeadingDistance(float flSpeed, float flSpeedAlongPath,
                                       bool bEnemyInVehicle);
...
#ifdef HL2_EPISODIC
static const char *pFollowerBoneNames[] = {"Chopper.Body"};
#endif  // HL2_EPISODIC
...
BEGIN_DATADESC(CNPC_AttackHelicopter)
  ...
  DEFINE_ENTITYFUNC(FlyTouch),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableAlwaysTransition",
                       InputEnableAlwaysTransition),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableAlwaysTransition",
                       InputDisableAlwaysTransition),
      DEFINE_INPUTFUNC(FIELD_VOID, "OutsideTransition", InputOutsideTransition),
      DEFINE_INPUTFUNC(FIELD_STRING, "SetTransitionTarget",
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StartAlwaysLeadingVehicle",
                       InputStartAlwaysLeadingVehicle),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartBombingVehicle",
                       InputStartBombingVehicle),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartTrailingVehicle",
                       InputStartTrailingVehicle),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartDefaultBehavior",
                       InputStartDefaultBehavior),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartBullrushBehavior",
                       InputStartBullrushBehavior),

      DEFINE_INPUTFUNC(FIELD_VOID, "DropBomb", InputDropBomb),
      DEFINE_INPUTFUNC(FIELD_VOID, "DropBombStraightDown",
                       InputDropBombStraightDown),
      DEFINE_INPUTFUNC(FIELD_STRING, "DropBombAtTargetAlways",
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "StartCarpetBombing",
                       InputStartCarpetBombing),
      DEFINE_INPUTFUNC(FIELD_VOID, "StopCarpetBombing", InputStopCarpetBombing),
      DEFINE_INPUTFUNC(FIELD_VOID, "BecomeIndestructible",
                       InputBecomeIndestructible),
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableDeadlyShooting",
                       InputEnableDeadlyShooting),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableDeadlyShooting",
                       InputDisableDeadlyShooting),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartNormalShooting",
                       InputStartNormalShooting),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartLongCycleShooting",
                       InputStartLongCycleShooting),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartContinuousShooting",
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetHealthFraction",
                       InputSetHealthFraction),
      DEFINE_INPUTFUNC(FIELD_VOID, "StartBombExplodeOnContact",
                       InputStartBombExplodeOnContact),
      DEFINE_INPUTFUNC(FIELD_VOID, "StopBombExplodeOnContact",
                       InputStopBombExplodeOnContact),

      DEFINE_INPUTFUNC(FIELD_VOID, "DisablePathVisibilityTests",
                       InputDisablePathVisibilityTests),
      DEFINE_INPUTFUNC(FIELD_VOID, "EnablePathVisibilityTests",
...
void CNPC_AttackHelicopter::Precache(void) {
  ...
  // If we're never going to engage in combat, we don't need to load these
  // assets!
...
void CNPC_AttackHelicopter::InputSetOutsideTransitionTarget(
...
bool CNPC_AttackHelicopter::CreateComponents() {
  if (!BaseClass::CreateComponents()) return false;
...
void CNPC_AttackHelicopter::Spawn(void) {
  ...
  float flChaseDist = HasSpawnFlags(SF_HELICOPTER_AGGRESSIVE)
                          ? CHOPPER_MIN_AGGRESSIVE_CHASE_DIST_DIFF
                          : CHOPPER_MIN_CHASE_DIST_DIFF;
  ...
  m_flNextMegaBombHealth =
      m_iMaxHealth -
      m_iMaxHealth * g_helicopter_bullrush_mega_bomb_health.GetFloat();
  ...
  m_hSensor = static_cast<CBombDropSensor *>(
      CreateEntityByName("npc_helicoptersensor"));
  m_hSensor->Spawn();
...
void CNPC_AttackHelicopter::Startup() {
  ...
  if (HasSpawnFlags(SF_HELICOPTER_LIGHTS)) {
    for (int i = 0; i < MAX_HELICOPTER_LIGHTS; ++i) {
      ...
      m_hLights[i] =
          CSprite::SpriteCreate(CHOPPER_RED_LIGHT_SPRITE, vec3_origin, false);
      if (!m_hLights[i]) continue;
      ...
      m_hLights[i]->SetTransparency(kRenderTransAdd, 255, 255, 255, 200,
                                    kRenderFxNone);
      m_hLights[i]->SetScale(1.0f);
      m_hLights[i]->TurnOn();
    }

    SetContextThink(&CNPC_AttackHelicopter::BlinkLightsThink,
                    gpGlobals->curtime + CHOPPER_LIGHT_BLINK_TIME_SHORT,
                    s_pBlinkLightThinkContext);
  }
}
...
void CNPC_AttackHelicopter::BlinkLightsThink() {
  ...
  if (bIsOn) {
    ...
    flTime = m_bShortBlink ? CHOPPER_LIGHT_BLINK_TIME_SHORT
                           : CHOPPER_LIGHT_BLINK_TIME;
    m_bShortBlink = !m_bShortBlink;
  }

  SetContextThink(&CNPC_AttackHelicopter::BlinkLightsThink,
                  gpGlobals->curtime + flTime, s_pBlinkLightThinkContext);
}
...
void CNPC_AttackHelicopter::SpotlightStartup() {
  if (!HasSpawnFlags(SF_HELICOPTER_LIGHTS)) return;
...
void CNPC_AttackHelicopter::SpotlightThink() {
  ...
  m_Spotlight.Update();
  SetContextThink(&CNPC_AttackHelicopter::SpotlightThink,
                  gpGlobals->curtime + TICK_INTERVAL, s_pSpotlightThinkContext);
}

//-----------------------------------------------------------------------------
// Purpose: Always transition along with the player
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputEnableAlwaysTransition(
    inputdata_t &inputdata) {
...
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputDisableAlwaysTransition(
    inputdata_t &inputdata) {
...
void CNPC_AttackHelicopter::InitializeRotorSound(void) {
  if (!m_pRotorSound) {
    ...
    if (HasSpawnFlags(SF_HELICOPTER_LOUD_ROTOR_SOUND)) {
      m_pRotorSound = controller.SoundCreate(filter, entindex(),
    ...
    m_pRotorBlast = controller.SoundCreate(filter, entindex(),
...
float CNPC_AttackHelicopter::GetMaxSpeed() {
  if (HasSpawnFlags(SF_HELICOPTER_ELECTRICAL_DRONE)) return DRONE_SPEED;

  if ((m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) &&
      IsInSecondaryMode(BULLRUSH_MODE_DROP_BOMBS_FIXED_SPEED))
...
float CNPC_AttackHelicopter::GetMaxFiringDistance() {
  if (!GetEnemyVehicle()) return CHOPPER_GUN_MAX_FIRING_DIST;
...
void CNPC_AttackHelicopter::InputStartAlwaysLeadingVehicle(
    inputdata_t &inputdata) {
...
void CNPC_AttackHelicopter::InputStartLongCycleShooting(
    inputdata_t &inputdata) {
  m_nShootingMode = SHOOT_MODE_LONG_CYCLE;
}

void CNPC_AttackHelicopter::InputStartContinuousShooting(
    inputdata_t &inputdata) {
...
bool CNPC_AttackHelicopter::IsDeadlyShooting() {
  if (m_bDeadlyShooting) return true;

  if ((m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) &&
      IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER)) {
...
int CNPC_AttackHelicopter::GetShootingMode() {
  if (IsDeadlyShooting()) return SHOOT_MODE_LONG_CYCLE;
...
// Visibility tests
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputDisablePathVisibilityTests(
    inputdata_t &inputdata) {
  m_bIgnorePathVisibilityTests = true;
  GetEnemies()->SetUnforgettable(GetEnemy(), true);
}

void CNPC_AttackHelicopter::InputEnablePathVisibilityTests(
    inputdata_t &inputdata) {
  m_bIgnorePathVisibilityTests = false;
...
void CNPC_AttackHelicopter::InputSelfDestruct(inputdata_t &inputdata) {
  ...
  CTakeDamageInfo info(this, this, Vector(0, 0, 1), WorldSpaceCenter(),
                       GetMaxHealth(), CLASS_MISSILE);
  TakeDamage(info);
}
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputStartBombExplodeOnContact(
    inputdata_t &inputdata) {
...
// Input  : &inputdata -
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputStopBombExplodeOnContact(
    inputdata_t &inputdata) {
  m_bBombsExplodeOnContact = false;
}
...
CTraceFilterChopper::CTraceFilterChopper(const IHandleEntity *passentity,
                                         int collisionGroup)
    : CTraceFilterSimple(passentity, collisionGroup) {}

bool CTraceFilterChopper::ShouldHitEntity(IHandleEntity *pServerEntity,
                                          int contentsMask) {
  CBaseEntity *pEnt =
      static_cast<IServerUnknown *>(pServerEntity)->GetBaseEntity();
  if (pEnt) {
...
// Enemy visibility check
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_AttackHelicopter::FindTrackBlocker(
    const Vector &vecViewPoint, const Vector &vecTargetPos) {
  if (m_bIgnorePathVisibilityTests) return NULL;

  CTraceFilterChopper chopperFilter(this, COLLISION_GROUP_NONE);

  trace_t tr;
  AI_TraceHull(vecViewPoint, vecTargetPos, -Vector(4, 4, 4), Vector(4, 4, 4),
               MASK_SHOT, &chopperFilter, &tr);
...
// More Enemy visibility check
//-----------------------------------------------------------------------------
bool CNPC_AttackHelicopter::FVisible(CBaseEntity *pEntity, int traceMask,
  ...
  trace_t tr;
  UTIL_TraceLine(vecLookerOrigin, vecTargetOrigin, traceMask, &chopperFilter,
                 &tr);
...
Vector CNPC_AttackHelicopter::GetAttackSpread(CBaseCombatWeapon *pWeapon,
                                              CBaseEntity *pTarget) {
  float flSinConeDegrees = sin(sk_helicopter_firingcone.GetFloat() *
                               PLAYER_TIGHTEN_FACTOR * 0.5f * (3.14f / 180.0f));
...
// Find interesting nearby things to shoot
//------------------------------------------------------------------------------
int CNPC_AttackHelicopter::BuildMissTargetList(int nCount,
                                               CBaseEntity **ppMissCandidates) {
  ...
  int numEnts =
      UTIL_EntitiesInBox(pEnts, 256, vecSource - radius, vecSource + radius, 0);

  for (int i = 0; i < numEnts; i++) {
    if (pEnts[i] == NULL) continue;
...
// Purpose :
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ShootAtPlayer(const Vector &vBasePos,
                                          const Vector &vGunDir) {
  ...
  float flSinConeDegrees =
      sin(sk_helicopter_firingcone.GetFloat() * 0.5f * (3.14f / 180.0f));
  Vector vecSpread(flSinConeDegrees, flSinConeDegrees, flSinConeDegrees);

  // How many times should we hit the player this time?
  int nDesiredHitCount = (int)(((float)(m_nMaxBurstHits - m_nBurstHits) /
                                (float)m_nRemainingBursts) +
                               0.5f);
  ...
  for (i = sk_helicopter_roundsperburst.GetInt() - 1; --i >= 0;) {
    ...
    if (GetEnemy() && (nPlayerShotCount < nDesiredHitCount)) {
      GetEnemy()->CollisionProp()->RandomPointInBounds(
          Vector(0, 0, 0), Vector(1, 1, 1), &info.m_vecDirShooting);
      info.m_vecDirShooting -= vBasePos;
      VectorNormalize(info.m_vecDirShooting);
      info.m_vecSpread = VECTOR_CONE_PRECALCULATED;
      info.m_flDistance = MAX_COORD_RANGE;
...
// Chooses a point within the circle of death to fire in
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::PickDirectionToCircleOfDeath(
    const Vector &vBasePos, const Vector &vecFireAtPosition, Vector *pResult) {
...
// Deliberately aims as close as possible w/o hitting
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::AimCloseToTargetButMiss(CBaseEntity *pTarget,
                                                    float flMinDist,
                                                    float flMaxDist,
  ...
  float flRadius =
      pTarget->BoundingRadius() + random->RandomFloat(flMinDist, flMaxDist);

  float flMinRadius = flRadius;
  if (flDist > flRadius) {
    flMinRadius =
        flDist * flRadius / sqrt(flDist * flDist - flRadius * flRadius);
  }

  // Choose random points in a plane perpendicular to the shoot origin.
  Vector vecRandomDir;
  vecRandomDir.Random(-1.0f, 1.0f);
  VectorMA(vecRandomDir, -DotProduct(vecDirection, vecRandomDir), vecDirection,
           vecRandomDir);
...
void CNPC_AttackHelicopter::FireBullets(const FireBulletsInfo_t &info) {
  ...
  if ((pPlayer->GetHealth() < nPrevHealth) ||
      (pPlayer->ArmorValue() < nPrevArmor)) {
    ++m_nBurstHits;
  }
}
...
// Purpose :
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ShootInsideCircleOfDeath(
    const Vector &vBasePos, const Vector &vecFireAtPosition) {
  Vector vecFireDirection;
  if (m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) {
    PickDirectionToCircleOfDeath(vBasePos, vecFireAtPosition,
                                 &vecFireDirection);
    ...
    if ((m_nBurstHits < m_nMaxBurstHits) || !GetEnemy()) {
      ...
      m_nNearShots += 6;
      AimCloseToTargetButMiss(GetEnemy(), 20.0f, 50.0f, vBasePos,
                              &vecFireDirection);
    }
  } else {
    AimCloseToTargetButMiss(GetEnemyVehicle(), 10.0f, 80.0f, vBasePos,
                            &vecFireDirection);
  }

  FireBulletsInfo_t info(1, vBasePos, vecFireDirection,
...
void CNPC_AttackHelicopter::ShootAtVehicle(const Vector &vBasePos,
                                           const Vector &vecFireAtPosition) {
  ...
  if (hl2_episodic.GetBool()) {
    ...
    for (int i = 0; i < nShotsRemaining; i++) {
      ...
      if (flRange <= 0.1f) {
        ...
        AimCloseToTargetButMiss(GetEnemy(), (3 * 12) * flRange,
                                (10 * 12) * flRange, vBasePos, &vecShotDir);
      }

      FireBulletsInfo_t info(1, vBasePos, vecShotDir, VECTOR_CONE_PRECALCULATED,
                             MAX_COORD_RANGE, m_iAmmoType);
      info.m_iTracerFreq = 1;
      FireBullets(info);
    }
  ...
  if (m_nBurstHits < m_nMaxBurstHits) {
    ...
    FireBulletsInfo_t info(1, vBasePos, vecDir, VECTOR_CONE_PRECALCULATED,
                           MAX_COORD_RANGE, m_iAmmoType);
    info.m_iTracerFreq = 1;
    FireBullets(info);
    --nShotsRemaining;
  ...
  // Fire half of the bullets within the circle of death, the other half at
  // interesting things
  int i;
  ...
  for (i = nShotsRemaining; --i >= 0;) {
    ...
    if (nActualTargets > i) {
      Vector vecFireDirection;
      ppNearbyTargets[i]->CollisionProp()->RandomPointInBounds(
          Vector(.25, .25, .25), Vector(.75, .75, .75), &vecFireDirection);
...
// Various states of the helicopter firing...
//------------------------------------------------------------------------------
bool CNPC_AttackHelicopter::PoseGunTowardTargetDirection(
    const Vector &vTargetDir) {
  Vector vecOut;
...
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ComputeFireAtPosition(
...
// Compute the enemy position (non-vehicle case)
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ComputeVehicleFireAtPosition(
    Vector *pVecActualTargetPosition) {
...
//------------------------------------------------------------------------------
bool CNPC_AttackHelicopter::DoGunIdle(const Vector &vGunDir,
                                      const Vector &vTargetDir) {
  // When bullrushing, skip the idle
  if ((m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) &&
      (IsInSecondaryMode(BULLRUSH_MODE_SHOOT_GUN) ||
       IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER))) {
  ...
  float flDistSqr =
      WorldSpaceCenter().DistToSqr(GetEnemy()->WorldSpaceCenter());
  if (flDistSqr > flMaxDistSqr) return false;
  ...
  if (fDotPr < flCosConeDegrees) return false;
  ...
  if (m_nShootingMode == SHOOT_MODE_FAST) {
    ...
    m_flChargeTime =
        gpGlobals->curtime + random->RandomFloat(flChargeTime - flVariance,
                                                 flChargeTime + flVariance);
    m_nGunState = GUN_STATE_CHARGING;
    m_flAvoidMetric = 0.0f;
    m_vecLastAngVelocity.Init(0, 0, 0);
  }
...
bool CNPC_AttackHelicopter::DoGunCharging() {
  // Update the target hittability, which will indicate how many hits we'll
  // accept.
  ...
  switch (GetShootingMode()) {
    ...
    case SHOOT_MODE_CONTINUOUS:
      if (m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) {
        // We're relying on the special aiming behavior for bullrushing to just
        // randomly deal damage
  ...
  if (!GetEnemyVehicle()) {
    ...
    m_flCircleOfDeathRadius = SimpleSplineRemapVal(
        flTotal, 0.0f, 1.0f, CHOPPER_MIN_CIRCLE_OF_DEATH_RADIUS,
        CHOPPER_MAX_CIRCLE_OF_DEATH_RADIUS);
  }
...
// Shoot where we're facing
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ShootAtFacingDirection(const Vector &vBasePos,
                                                   const Vector &vGunDir,
                                                   bool bFirstShotAccurate) {
  // Just shoot where we're facing
  float flSinConeDegrees =
    ...
    if (GetEnemy()) {
      ...
      if (t > 0.0f) {
        ...
        if (flSinAngle <= flSinConeDegrees) {
          FireBulletsInfo_t info(1, vBasePos, vecDelta,
                                 VECTOR_CONE_PRECALCULATED, 8192, m_iAmmoType);
          info.m_iTracerFreq = 1;
          FireBullets(info);
          --nShotCount;
        }
...
#ifdef HL2_EPISODIC
  if (GetEnemy() != NULL) {
    CSoundEnt::InsertSound(SOUND_DANGER, GetEnemy()->WorldSpaceCenter(), 180.0f,
                           0.5f, this, SOUNDENT_CHANNEL_REPEATED_DANGER);
  }
#endif  // HL2_EPISODIC

  DoMuzzleFlash();

  FireBulletsInfo_t info(nShotCount, vBasePos, vGunDir, vecSpread, 8192,
                         m_iAmmoType);
  info.m_iTracerFreq = 1;
  FireBullets(info);
}
...
void CNPC_AttackHelicopter::FireElectricityGun() {
  ...
  if (GetEnemy()) {
    Vector vecDelta;
    Vector2DSubtract(GetEnemy()->WorldSpaceCenter().AsVector2D(),
                     WorldSpaceCenter().AsVector2D(), vecDelta.AsVector2D());
    vecDelta.z = 0.0f;

    flDist = VectorNormalize(vecDelta);
    Vector vecPerp(-vecDelta.y, vecDelta.x, 0.0f);
    int nBoltCount =
        (int)(ClampSplineRemapVal(flDist, 256.0f, 1024.0f, 8, 0) + 0.5f);

    for (i = 0; i < nBoltCount; ++i) {
      Vector vecTargetPt = GetEnemy()->WorldSpaceCenter();
      VectorMA(vecTargetPt, random->RandomFloat(flDist + 100, flDist + 500),
               vecDelta, vecTargetPt);
      VectorMA(vecTargetPt, random->RandomFloat(-500, 500), vecPerp,
               vecTargetPt);
  ...
  for (i = 0; i < nBoltCount; ++i) {
    if ((nCandidateCount > 0) && random->RandomFloat(0.0f, 1.0f) < 0.6f) {
      ...
      Vector vecTarget;
      ppCandidates[nCandidateCount]->CollisionProp()->RandomPointInBounds(
          vec3_origin, Vector(1, 1, 1), &vecTarget);
      CreateZapBeam(vecTarget);
      CreateEntityZapEffect(ppCandidates[nCandidateCount]);
    } else {
      // Select random point *on* sphere
      Vector vecTargetPt;
      float flEffectRadius =
          random->RandomFloat(flRadius * 1.2, flRadius * 1.5f);
  ...
  if (random->RandomFloat(0.0f, 1.0f) < flHitRatio) {
    if (GetEnemyVehicle()) {
      Vector vecTarget;
      GetEnemyVehicle()->CollisionProp()->RandomPointInBounds(
          vec3_origin, Vector(1, 1, 1), &vecTarget);
      CreateZapBeam(vecTarget);
...
bool CNPC_AttackHelicopter::DoGunFiring(const Vector &vBasePos,
                                        const Vector &vGunDir,
                                        const Vector &vecFireAtPosition) {
  CSoundEnvelopeController &controller =
      CSoundEnvelopeController::GetController();
  ...
  --m_nRemainingBursts;
  if (m_nRemainingBursts > 0) return true;
  ...
  m_flNextAttack =
      gpGlobals->curtime + m_flIdleTimeDelay +
      random->RandomFloat(flIdleTime - flVariance, flIdleTime + flVariance);
  m_nGunState = GUN_STATE_IDLE;
  SetPauseState(PAUSE_NO_PAUSE);
  return true;
}
...
bool CNPC_AttackHelicopter::IsBombDropFair(const Vector &vecBombStartPos,
                                           const Vector &vecBombVelocity) {
  if ((m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) &&
      IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER))
    return true;
  ...
  if (GetEnemy()->GetSmoothedVelocity().LengthSqr() <
      (CHOPPER_SLOW_BOMB_SPEED * CHOPPER_SLOW_BOMB_SPEED))
    return true;
  ...
  if (GetEnemy() && GetEnemy()->IsPlayer()) {
    ...
    if (vecBomb.AsVector2D().DistToSqr(vecTarget.AsVector2D()) <
        (flDistFactorSq * MIN_BOMB_DISTANCE_SQR))
      return false;
  }
...
//-----------------------------------------------------------------------------
CGrenadeHelicopter *CNPC_AttackHelicopter::SpawnBombEntity(
    const Vector &vecPos, const Vector &vecVelocity) {
  // Create the grenade and set it up
  CGrenadeHelicopter *pGrenade = static_cast<CGrenadeHelicopter *>(
      CreateEntityByName("grenade_helicopter"));
...
// Actually drops the bomb
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::CreateBomb(bool bCheckForFairness,
                                       Vector *pVecVelocity, bool bMegaBomb) {
  if (m_bBombingSuppressed) return;

  Vector vTipPos;
  GetAttachment(m_nBombAttachment, vTipPos);

  if (!CBombSuppressor::CanBomb(vTipPos)) return;
  ...
  if (bCheckForFairness) {
    if (!IsBombDropFair(vTipPos, vecActualVelocity)) return;
  }
...
void CNPC_AttackHelicopter::InputDropBomb(inputdata_t &inputdata) {
  if (m_flInputDropBombTime > gpGlobals->curtime) return;
  ...
  // If we're in the middle of a bomb dropping schedule, wait to drop another
  // bomb.
  if (ShouldDropBombs()) {
    m_flNextAttack =
        gpGlobals->curtime + 0.5f + random->RandomFloat(0.3f, 0.6f);
  }
}
...
// Drop a bomb at a particular location
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputDropBombAtTargetInternal(
    inputdata_t &inputdata, bool bCheckFairness) {
  if (m_flInputDropBombTime > gpGlobals->curtime) return;
  ...
  if (bCheckFairness) {
    if (!IsBombDropFair(vTipPos, vecVelocity)) return;
  }
  ...
  if (ShouldDropBombs()) {
    m_flNextAttack =
        gpGlobals->curtime + 1.5f + random->RandomFloat(0.1f, 0.2f);
  }
}
...
// Drop a bomb at a particular location
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::InputDropBombAtTargetAlways(
    inputdata_t &inputdata) {
...
void CNPC_AttackHelicopter::DropBombs() {
  ...
  if (m_nAttackMode != ATTACK_MODE_BULLRUSH_VEHICLE) {
    if (GetEnemy() && GetEnemy()->IsPlayer()) {
      if (GetEnemy()->GetSmoothedVelocity().LengthSqr() >
          (CHOPPER_SLOW_BOMB_SPEED * CHOPPER_SLOW_BOMB_SPEED)) {
        // Don't drop bombs if you are behind the player, unless the player is
        // moving slowly
        ...
        if (vecPoint.AsVector2D().DistToSqr(GetDesiredPosition().AsVector2D()) >
            flLeadingDistSq)
          return;
      }
    }
    ...
    if (GetEnemy()) {
      if (GetEnemy()->GetAbsOrigin().AsVector2D().DistToSqr(
              GetAbsOrigin().AsVector2D()) > MAX_BULLRUSH_BOMB_DISTANCE_SQR)
        return;
    }
  }
...
bool CNPC_AttackHelicopter::ShouldDropBombs(void) {
  if (IsCarpetBombing()) return true;

  if (m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) {
    // Distance determines whether or not we should do this
    if ((m_nSecondaryMode == BULLRUSH_MODE_SHOOT_IDLE_PLAYER) &&
        (SecondaryModeTime() >= BULLRUSH_IDLE_PLAYER_FIRE_TIME))
      return ShouldBombIdlePlayer();

    return ((m_nSecondaryMode == BULLRUSH_MODE_DROP_BOMBS_FIXED_SPEED) ||
            (m_nSecondaryMode == BULLRUSH_MODE_DROP_BOMBS_FOLLOW_PLAYER));
  }
  ...
  // This is for bombing. If you get hit, give a grace period to get back to
  // speed
...
void CNPC_AttackHelicopter::BullrushBombs() {
  if (gpGlobals->curtime < m_flNextBullrushBombTime) return;
...
bool CNPC_AttackHelicopter::FireGun(void) {
  ...
  // HACK: CBaseHelicopter ignores this, and fire forever at the last place it
  // saw the player. Why?
  if ((m_nGunState == GUN_STATE_IDLE) &&
      (m_nAttackMode != ATTACK_MODE_BULLRUSH_VEHICLE) && !IsCarpetBombing()) {
    if ((m_flLastSeen + 1 <= gpGlobals->curtime) ||
        (m_flPrevSeen + m_flGracePeriod > gpGlobals->curtime))
      return false;
  }
  ...
  if (m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) {
    ...
    if (!IsInSecondaryMode(BULLRUSH_MODE_SHOOT_GUN) &&
        !IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER))
      return false;

    // If we're in the grace period on this mode, then don't fire
    if (IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER) &&
        (SecondaryModeTime() < BULLRUSH_IDLE_PLAYER_FIRE_TIME)) {
      // Stop our gun sound
      if (m_nGunState != GUN_STATE_IDLE) {
...
// Should we trigger a damage effect?
//-----------------------------------------------------------------------------
inline bool CNPC_AttackHelicopter::ShouldTriggerDamageEffect(
    int nPrevHealth, int nEffectCount) const {
  int nPrevRange =
      (int)(((float)nPrevHealth / (float)GetMaxHealth()) * nEffectCount);
  int nRange =
      (int)(((float)GetHealth() / (float)GetMaxHealth()) * nEffectCount);
...
void CNPC_AttackHelicopter::AddSmokeTrail(const Vector &vecPos) {
  ...
  // See if there's an attachment for this smoke trail
  int nAttachment =
      LookupAttachment(UTIL_VarArgs("damage%d", m_nSmokeTrailCount));
  ...
  if (m_nSmokeTrailCount == 0 || m_nSmokeTrailCount % 2) {
    ...
    pFireTrail->FollowEntity(this,
                             UTIL_VarArgs("damage%d", m_nSmokeTrailCount));
...
// Input  : &vecChunkPos -
//-----------------------------------------------------------------------------
void Chopper_CreateChunk(CBaseEntity *pChopper, const Vector &vecChunkPos,
                         const QAngle &vecChunkAngles, const char *pszChunkName,
                         bool bSmall) {
  ...
  if (bSmall == false) {
    IPhysicsObject *pPhysicsObject = pChunk->VPhysicsInitNormal(
        SOLID_VPHYSICS, pChunk->GetSolidFlags(), false);
...
// Pow!
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::ExplodeAndThrowChunk(
    const Vector &vecExplosionPos) {
  CEffectData data;
  ...
  if (random->RandomInt(0, 4)) {
    for (int i = 0; i < 2; i++) {
      Chopper_CreateChunk(this, vecExplosionPos, RandomAngle(0, 360),
                          g_PropDataSystem.GetRandomChunkModel("MetalChunks"),
                          true);
    }
  } else {
    Chopper_CreateChunk(
        this, vecExplosionPos, RandomAngle(0, 360),
        s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_SMALL_CHUNKS - 1)],
        false);
  }
}
...
void CNPC_AttackHelicopter::DropCorpse(int nDamage) {
  ...
  CBaseEntity *pGib = CreateRagGib("models/combine_soldier.mdl", GetAbsOrigin(),
                                   GetAbsAngles(), vecForceVector);
  if (pGib) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::TraceAttack(const CTakeDamageInfo &info,
                                        const Vector &vecDir, trace_t *ptr) {
  // Take no damage from trace attacks unless it's blast damage. RadiusDamage()
  // sometimes calls TraceAttack() as a means for delivering blast damage.
  // Usually when the explosive penetrates the target. (RPG missiles do this
  // sometimes).
...
int CNPC_AttackHelicopter::OnTakeDamage(const CTakeDamageInfo &info) {
  // We don't take blast damage from anything but the airboat or missiles (or
  // myself!)
  ...
  if (m_bIndestructible) {
    if (GetHealth() < info.GetDamage()) return 0;
  }

  // helicopter takes extra damage from its own grenades
  CGrenadeHelicopter *pGren =
      dynamic_cast<CGrenadeHelicopter *>(info.GetInflictor());
...
int CNPC_AttackHelicopter::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if ((info.GetInflictor() != NULL) &&
      (info.GetInflictor()->GetOwnerEntity() != NULL) &&
      (info.GetInflictor()->GetOwnerEntity() == this)) {
    // Don't take damage from my own bombs. (Unless the player grabbed them and
    // threw them back)
    return 0;
  }
  ...
  if (nPrevHealth != GetHealth()) {
    ...
    if (m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE) {
      if ((nPrevHealth > m_flNextMegaBombHealth) &&
          (GetHealth() <= m_flNextMegaBombHealth)) {
...
void Chopper_BecomeChunks(CBaseEntity *pChopper) {
  ...
  if (pAttackHelicopter != NULL) {
    // New for EP2, we may be tailspinning, (crashing) and playing an animation
    // that is spinning our root bone, which means our model is not facing the
    // way our entity is facing. So we have to do some attachment point math to
    // get the proper angles to use for computing the relative positions of the
    // gibs. The attachment points called DAMAGE0 is properly oriented and
    // attached to the chopper body so we can use its angles.
  ...
  // Body
  CHelicopterChunk *pBodyChunk = CHelicopterChunk::CreateHelicopterChunk(
      vecChunkPos, vecChunkAngles, pChopper->GetAbsVelocity(),
      HELICOPTER_CHUNK_BODY, CHUNK_BODY);
  Chopper_CreateChunk(
      pChopper, vecChunkPos, RandomAngle(0, 360),
      s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_CHUNKS - 1)], false);

  vecChunkPos =
      pChopper->GetAbsOrigin() + (vecForward * 100.0f) + (vecUp * -38.0f);

  // Cockpit
  CHelicopterChunk *pCockpitChunk = CHelicopterChunk::CreateHelicopterChunk(
      vecChunkPos, vecChunkAngles,
      pChopper->GetAbsVelocity() + vecRight * -800.0f, HELICOPTER_CHUNK_COCKPIT,
      CHUNK_COCKPIT);
  Chopper_CreateChunk(
      pChopper, vecChunkPos, RandomAngle(0, 360),
      s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_CHUNKS - 1)], false);
  ...
  // Tail
  CHelicopterChunk *pTailChunk = CHelicopterChunk::CreateHelicopterChunk(
      vecChunkPos, vecChunkAngles,
      pChopper->GetAbsVelocity() + vecRight * 800.0f, HELICOPTER_CHUNK_TAIL,
      CHUNK_TAIL);
  Chopper_CreateChunk(
      pChopper, vecChunkPos, RandomAngle(0, 360),
      s_pChunkModelName[random->RandomInt(0, CHOPPER_MAX_CHUNKS - 1)], false);
  ...
  pBodyChunk->m_pTailConstraint =
      physenv->CreateFixedConstraint(pBodyObject, pTailObject, pGroup, fixed);

  fixed.Defaults();
  fixed.InitWithCurrentObjectState(pBodyObject, pCockpitObject);
  fixed.constraint.Defaults();

  pBodyChunk->m_pCockpitConstraint = physenv->CreateFixedConstraint(
      pBodyObject, pCockpitObject, pGroup, fixed);
}
...
void CNPC_AttackHelicopter::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (GetCrashPoint() == NULL) {
    CBaseEntity *pCrashPoint =
        gEntList.FindEntityByClassname(NULL, "info_target_helicopter_crash");
    if (pCrashPoint != NULL) {
      ...
      CPASAttenuationFilter filter(this);
      m_pRotorSound = controller.SoundCreate(
          filter, entindex(), "NPC_AttackHelicopter.EngineFailure");
    ...
    case LIFE_DYING: {
      ...
      if (random->RandomInt(0, 4) == 0) {
        ...
        CollisionProp()->RandomPointInBounds(
            Vector(0.25, 0.25, 0.25), Vector(0.75, 0.75, 0.75), &explodePoint);
...
float CNPC_AttackHelicopter::UpdatePerpPathDistance(float flMaxPathOffset) {
  ...
  if (ShouldDropBombs()) {
    ...
    flNewPathOffset +=
        flSinHeight * sin(2.0f * M_PI * (gpGlobals->curtime / 6.0f));
  }
...
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::GetMaxSpeedAndAccel(float *pMaxSpeed,
                                                float *pAccelRate) {
...
void CNPC_AttackHelicopter::ComputeVelocity(const Vector &vecTargetPosition,
  ...
  pVecAccel->z = 2.0f * (deltaPos.z - GetAbsVelocity().z * dt) / (dt * dt) +
                 HELICOPTER_GRAVITY;

  float flDistFromPath = 0.0f;
  Vector vecPoint, vecDelta;
  if (flMaxDistFromSegment != 0.0f) {
    // Also, add in a little force to get us closer to our current line segment
    // if we can
    ClosestPointToCurrentPath(&vecPoint);
    ...
    if (flAdditionalHeight != 0.0f) {
      ...
      CalcClosestPointOnLineSegment(GetAbsOrigin(), vecPoint, vecEndPoint,
                                    vecClosest);
      vecPoint = vecClosest;
    }
  ...
  float flHorizLiftFactor =
      fabs(pVecAccel->x) * 0.10f + fabs(pVecAccel->y) * 0.10f;
  ...
  float flDampening = (flNewHorizLiftFactor != 0.0f)
                          ? (flNewHorizLiftFactor / flHorizLiftFactor)
                          : 1.0f;
  if (flDampening < 1.0f) {
  ...
  float flForceBlend = GetEnemyVehicle() ? HELICOPTER_FORCE_BLEND_VEHICLE
                                         : HELICOPTER_FORCE_BLEND;
  m_flForce *= flForceBlend;

  // Now add force based on our acceleration factors
  m_flForce += (pVecAccel->z + flHorizLiftFactor) * HELICOPTER_DT *
               (1.0f - flForceBlend);

  // The force is always *locally* upward based; we pitch + roll the chopper to
  // get movement
  Vector vecImpulse;
  VectorMultiply(up, m_flForce, vecImpulse);

  // NOTE: These have to be done *before* the additional path distance drag
  // forces are applied below
  ApplySidewaysDrag(right);
  ApplyGeneralDrag();

  // If LIFE_DYING, maintain control as long as we're flying to a crash point.
  if (m_lifeState != LIFE_DYING ||
      (m_lifeState == LIFE_DYING && GetCrashPoint() != NULL)) {
    vecImpulse.z += -HELICOPTER_GRAVITY * HELICOPTER_DT;

    if (flMinDistFromSegment != 0.0f &&
        (flDistFromPath > flMinDistFromSegment)) {
    ...
    // Damp the horizontal impulses; we should pretty much be falling
    // ballistically
    vecImpulse.x *= 0.1f;
    vecImpulse.y *= 0.1f;
  }
...
// Computes the max speed + acceleration:
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::ComputeAngularVelocity(
    const Vector &vecGoalUp, const Vector &vecFacingDirection) {
  QAngle goalAngAccel;
  if (m_lifeState != LIFE_DYING ||
      (m_lifeState == LIFE_DYING && GetCrashPoint() != NULL)) {
    Vector forward, right, up;
    ...
    float dt = 0.6;
    goalAngAccel.x = 2.0 *
                     (AngleDiff(goalPitch, AngleNormalize(GetAbsAngles().x)) -
                      GetLocalAngularVelocity().x * dt) /
                     (dt * dt);
    goalAngAccel.y = 2.0 *
                     (AngleDiff(goalYaw, AngleNormalize(GetAbsAngles().y)) -
                      GetLocalAngularVelocity().y * dt) /
                     (dt * dt);
    goalAngAccel.z = 2.0 *
                     (AngleDiff(goalRoll, AngleNormalize(GetAbsAngles().z)) -
  ...
  // GetLocalAngles().z, m_vecAngVelocity.z ); DevMsg( "%6.1f %6.1f %6.1f  :  ",
  // goalAngAccel.x, goalAngAccel.y, goalAngAccel.z ); DevMsg( "%6.0f %6.0f
...
void CNPC_AttackHelicopter::FlightDirectlyOverhead(void) {
  ...
  if (HasEnemy() && FVisible(pEnemy)) {
    if (GetEnemy()->IsPlayer()) {
      CBaseEntity *pEnemyVehicle =
          assert_cast<CBasePlayer *>(GetEnemy())->GetVehicleEntity();
      if (pEnemyVehicle) {
        Vector vecEnemyVel = pEnemyVehicle->GetSmoothedVelocity();
        Vector vecRelativePosition;
        VectorSubtract(GetAbsOrigin(), pEnemyVehicle->GetAbsOrigin(),
                       vecRelativePosition);
  ...
  NDebugOverlay::Cross3D(vecTargetPosition, -Vector(32, 32, 32),
                         Vector(32, 32, 32), 0, 0, 255, true, 0.1f);
...
void CNPC_AttackHelicopter::Flight(void) {
  ...
  Vector accel;
  UpdateFacingDirection(vecTargetPosition);
  ComputeVelocity(vecTargetPosition, flAdditionalHeight, flMinDistFromSegment,
                  flMaxDistFromSegment, &accel);
  ComputeAngularVelocity(accel, m_vecDesiredFaceDir);
}
...
// Updates the facing direction
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::UpdateFacingDirection(
    const Vector &vecActualDesiredPosition) {
  bool bIsBullrushing = (m_nAttackMode == ATTACK_MODE_BULLRUSH_VEHICLE);

  bool bSeenTargetRecently = HasSpawnFlags(SF_HELICOPTER_AGGRESSIVE) ||
                             (m_flLastSeen + 5 > gpGlobals->curtime);
  if (GetEnemy() && !bIsBullrushing) {
    ...
    float flDistSqr = vecActualDesiredPosition.AsVector2D().DistToSqr(
        GetAbsOrigin().AsVector2D());
    if (flDistSqr <= 50 * 50) {
      if ((flDistSqr > 1 * 1) && bSeenTargetRecently &&
          IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER)) {
        m_vecDesiredFaceDir = m_vecTargetPosition - GetAbsOrigin();
...
float CNPC_AttackHelicopter::CreepTowardEnemy(float flSpeed, float flMinSpeed,
                                              float flMaxSpeed, float flMinDist,
                                              float flMaxDist) {
  float dt = gpGlobals->curtime - GetLastThink();
  float flEnemyCreepDist = ENEMY_CREEP_RATE * dt;

  // When the player is slow, creep toward him within a second or two
  float flLeadingDist = ClampSplineRemapVal(flSpeed, flMinSpeed, flMaxSpeed,
                                            flMinDist, flMaxDist);
...
//------------------------------------------------------------------------------
float CNPC_AttackHelicopter::ComputeBombingLeadingDistance(
    float flSpeed, float flSpeedAlongPath, bool bEnemyInVehicle) {
  if ((flSpeed <= MIN_ENEMY_SPEED) && bEnemyInVehicle) {
    return CreepTowardEnemy(flSpeed, 0.0f, MIN_ENEMY_SPEED, 0.0f, 1000.0f);
  }

  return ClampSplineRemapVal(flSpeedAlongPath, 200.0f, 600.0f, 1000.0f,
                             2000.0f);
}
...
//------------------------------------------------------------------------------
float CNPC_AttackHelicopter::ComputeBullrushLeadingDistance(
    float flSpeed, float flSpeedAlongPath, bool bEnemyInVehicle) {
  switch (m_nSecondaryMode) {
    ...
    case BULLRUSH_MODE_GET_DISTANCE:
      return m_bRushForward ? -CHOPPER_BULLRUSH_MODE_DISTANCE
                            : CHOPPER_BULLRUSH_MODE_DISTANCE;

    case BULLRUSH_MODE_DROP_BOMBS_FOLLOW_PLAYER:
      //		return m_bRushForward ? 1500.0f : -1500.0f;
      return ComputeBombingLeadingDistance(flSpeed, flSpeedAlongPath,
                                           bEnemyInVehicle);
...
// Secondary mode
//------------------------------------------------------------------------------
inline void CNPC_AttackHelicopter::SetSecondaryMode(int nMode,
                                                    bool bRetainTime) {
...
bool CNPC_AttackHelicopter::ShouldShootIdlePlayerInBullrush() {
  // Once he starts shooting, then don't stop until the player is moving pretty
  // fast
  float flSpeedSqr = IsInSecondaryMode(BULLRUSH_MODE_SHOOT_IDLE_PLAYER)
                         ? CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_2_SQ
                         : CHOPPER_BULLRUSH_SLOW_SHOOT_SPEED_SQ;
  return (GetEnemy() &&
          GetEnemy()->GetSmoothedVelocity().LengthSqr() <= flSpeedSqr);
}
...
bool CNPC_AttackHelicopter::ShouldBombIdlePlayer(void) {
  ...
  float flDistToTargetSqr =
      (GetEnemy()->WorldSpaceCenter() - GetAbsOrigin()).Length2DSqr();

  if (flDistToTargetSqr < Square(HELICOPTER_MIN_IDLE_BOMBING_DIST)) return true;
...
void CNPC_AttackHelicopter::UpdateBullrushState(void) {
  ...
  switch (m_nSecondaryMode) {
    case BULLRUSH_MODE_WAIT_FOR_ENEMY: {
      ...
      if (GetEnemy() && !IsInForcedMove()) {
        ...
        if (SecondaryModeTime() > 0.3f) {
          ...
          bool bMovingForward = DotProduct2D(GetAbsVelocity().AsVector2D(),
                                             vecPathDir.AsVector2D()) >= 0.0f;
    ...
    case BULLRUSH_MODE_GET_DISTANCE: {
      ...
      float flDistanceToGoal = ComputeDistanceToTargetPosition();
      if (m_bRushForward) {
        if (flDistanceToGoal < (CHOPPER_BULLRUSH_MODE_DISTANCE - 1000)) break;
      } else {
        if (flDistanceToGoal > -(CHOPPER_BULLRUSH_MODE_DISTANCE - 1000)) break;
      }
      ...
      if (GetHealth() <= m_flNextMegaBombHealth) {
        m_flNextMegaBombHealth -=
            GetMaxHealth() * g_helicopter_bullrush_mega_bomb_health.GetFloat();
    ...
    case BULLRUSH_MODE_SHOOT_GUN: {
      ...
      if (m_bRushForward) {
        m_flBullrushAdditionalHeight = ClampSplineRemapVal(
            flDistanceToGoal, flSwitchToBombDist,
            flSwitchToBombDist + flDropDownDist, 0.0f, flShootingHeight);
        if (flDistanceToGoal > flSwitchToBombDist) break;
      } else {
        m_flBullrushAdditionalHeight = ClampSplineRemapVal(
            flDistanceToGoal, -flSwitchToBombDist - flDropDownDist,
            -flSwitchToBombDist, flShootingHeight, 0.0f);
        if (flDistanceToGoal < -flSwitchToBombDist) break;
      }
    ...
    case BULLRUSH_MODE_SHOOT_IDLE_PLAYER: {
      ...
      //			m_nBurstHits = 0;
      m_flCircleOfDeathRadius = ClampSplineRemapVal(
          SecondaryModeTime(), BULLRUSH_IDLE_PLAYER_FIRE_TIME,
          BULLRUSH_IDLE_PLAYER_FIRE_TIME + 5.0f, 256.0f, 64.0f);
      m_flBullrushAdditionalHeight = CHOPPER_BULLRUSH_SHOOTING_VERTICAL_OFFSET;
      if (!ShouldShootIdlePlayerInBullrush()) {
        ShutdownGunDuringBullrush();
        SetSecondaryMode(BULLRUSH_MODE_DROP_BOMBS_FIXED_SPEED);
    ...
    case BULLRUSH_MODE_DROP_BOMBS_FIXED_SPEED: {
      ...
      m_flBullrushAdditionalHeight = 0.0f;
      if ((SecondaryModeTime() >= CHOPPER_BULLRUSH_ENEMY_BOMB_TIME) ||
          (flDistanceToGoal > BULLRUSH_BOMB_MAX_DISTANCE)) {
        m_bRushForward = !m_bRushForward;
        SetSecondaryMode(BULLRUSH_MODE_GET_DISTANCE);
      }
    } break;
...
void CNPC_AttackHelicopter::UpdateEnemyLeading(void) {
  ...
  if (HasSpawnFlags(SF_HELICOPTER_ELECTRICAL_DRONE)) {
    if (flSpeedAlongPath < 200.0f) {
      flLeadingDist =
          ClampSplineRemapVal(flSpeedAlongPath, 0.0f, 200.0f, 100.0f, -200.0f);
    } else {
      flLeadingDist = ClampSplineRemapVal(flSpeedAlongPath, 200.0f, 600.0f,
                                          -200.0f, -500.0f);
    }
  ...
  switch (m_nAttackMode) {
    case ATTACK_MODE_BULLRUSH_VEHICLE:
      flLeadingDist = ComputeBullrushLeadingDistance(flSpeed, flSpeedAlongPath,
                                                     bEnemyInVehicle);
      break;
    ...
    case ATTACK_MODE_ALWAYS_LEAD_VEHICLE:
      if ((flSpeed <= MIN_ENEMY_SPEED) && (bEnemyInVehicle)) {
        flLeadingDist =
            CreepTowardEnemy(flSpeed, 0.0f, MIN_ENEMY_SPEED, 0.0f, 1000.0f);
      } else {
        if (flSpeedAlongPath > 0.0f) {
          flLeadingDist = ClampSplineRemapVal(flSpeedAlongPath, 200.0f, 600.0f,
                                              1000.0f, 2000.0f);
        } else {
          flLeadingDist = ClampSplineRemapVal(flSpeedAlongPath, -600.0f,
                                              -200.0f, -2000.0f, -1000.0f);
        }
      }
      break;

    case ATTACK_MODE_BOMB_VEHICLE:
      flLeadingDist = ComputeBombingLeadingDistance(flSpeed, flSpeedAlongPath,
                                                    bEnemyInVehicle);
      break;
    ...
    case ATTACK_MODE_TRAIL_VEHICLE:
      if ((flSpeed <= MIN_ENEMY_SPEED) && (bEnemyInVehicle)) {
        flLeadingDist = CreepTowardEnemy(flSpeed, 150.0f, MIN_ENEMY_SPEED,
                                         500.0f, -1000.0f);
      } else {
        flLeadingDist = ClampSplineRemapVal(flSpeedAlongPath, -600.0f, -200.0f,
                                            -2500.0f, -1000.0f);
      }
      break;
  }
...
//			bAlways -
//-----------------------------------------------------------------------------
void CNPC_AttackHelicopter::SetTransmit(CCheckTransmitInfo *pInfo,
                                        bool bAlways) {
...
void CNPC_AttackHelicopter::Hunt(void) {
  ...
  if (!(m_fHelicopterFlags & BITS_HELICOPTER_GUN_ON)) {
    ...
    // doesn't get called by the above call to FireWeapons() if the gun is
    // turned off) Adding this little exception here lets me avoid going into
    // the CBaseHelicopter and making some functions virtual that don't want to
    // be virtual.
    if (IsCarpetBombing()) {
      BullrushBombs();
    }
...
void CBombDropSensor::InputDropBomb(inputdata_t &inputdata) {
  ...
  assert_cast<CNPC_AttackHelicopter *>(GetOwnerEntity())
      ->InputDropBomb(myVersion);
}
...
void CBombDropSensor::InputDropBombStraightDown(inputdata_t &inputdata) {
  ...
  myVersion.pActivator = this;
  assert_cast<CNPC_AttackHelicopter *>(GetOwnerEntity())
      ->InputDropBombStraightDown(myVersion);
}
...
void CBombDropSensor::InputDropBombAtTarget(inputdata_t &inputdata) {
  ...
  myVersion.pActivator = this;
  assert_cast<CNPC_AttackHelicopter *>(GetOwnerEntity())
      ->InputDropBombAtTarget(myVersion);
}
...
void CBombDropSensor::InputDropBombAtTargetAlways(inputdata_t &inputdata) {
  ...
  myVersion.pActivator = this;
  assert_cast<CNPC_AttackHelicopter *>(GetOwnerEntity())
      ->InputDropBombAtTargetAlways(myVersion);
}
...
BEGIN_DATADESC(CGrenadeHelicopter)
  ...
  DEFINE_FIELD(m_bActivated, FIELD_BOOLEAN),
      ...
      DEFINE_THINKFUNC(ExplodeThink), DEFINE_THINKFUNC(AnimateThink),
      DEFINE_THINKFUNC(RampSoundThink), DEFINE_THINKFUNC(WarningBlinkerThink),
      DEFINE_ENTITYFUNC(ExplodeConcussion),

END_DATADESC()

#define SF_HELICOPTER_GRENADE_DUD \
  (1 << 16)  // Will not become active on impact, only when launched via
             // physcannon
...
void CGrenadeHelicopter::Spawn(void) {
  ...
  if (hl2_episodic.GetBool()) {
    SetContextThink(&CGrenadeHelicopter::AnimateThink, gpGlobals->curtime,
                    s_pAnimateThinkContext);
  }
}
...
void CGrenadeHelicopter::BecomeActive() {
  ...
  if (!bMegaBomb) {
    SetContextThink(
        ...
        gpGlobals->curtime + GetBombLifetime() - BOMB_RAMP_SOUND_TIME,
        s_pRampSoundContext);

    CSoundEnvelopeController &controller =
        CSoundEnvelopeController::GetController();
    CReliableBroadcastRecipientFilter filter;
    m_pWarnSound = controller.SoundCreate(filter, entindex(),
                                          "NPC_AttackHelicopterGrenade.Ping");
    controller.Play(m_pWarnSound, 1.0, PITCH_NORM);
  }

  SetContextThink(&CGrenadeHelicopter::WarningBlinkerThink,
                  gpGlobals->curtime + (GetBombLifetime() - 2.0f),
                  s_pWarningBlinkerContext);
...
void CGrenadeHelicopter::WarningBlinkerThink() {
  ...
  /*
          if( !m_hWarningSprite.Get() )
          {
                  ...
                  m_hWarningSprite = CSprite::SpriteCreate(
     "sprites/redglow1.vmt", GetAbsOrigin() + up * 10.0f, false ); CSprite
     *pSprite = (CSprite *)m_hWarningSprite.Get();

                  if( pSprite != NULL )
                  {
                          pSprite->SetParent( this, LookupAttachment("top") );
                          pSprite->SetTransparency( kRenderTransAdd, 255, 255,
     255, 255, kRenderFxNone ); pSprite->SetScale( 0.35, 0.0 );
                  }
  ...
  */
  {
    ...
    if (m_bBlinkerAtTop) {
      // m_hWarningSprite->SetParentAttachment( "SetParentAttachment", "bottom",
      // false );
      m_nSkin = SKIN_REGULAR;
      m_bBlinkerAtTop = false;
    } else {
      // m_hWarningSprite->SetParentAttachment( "SetParentAttachment", "top",
      // false );
      m_nSkin = SKIN_DUD;
      m_bBlinkerAtTop = true;
    }
  ...
  // Frighten people
  CSoundEnt::InsertSound(SOUND_DANGER, WorldSpaceCenter(),
                         g_helicopter_bomb_danger_radius.GetFloat(), 0.2f, this,
                         SOUNDENT_CHANNEL_REPEATED_DANGER);

#ifdef HL2_EPISODIC
  if (gpGlobals->curtime >= m_flBlinkFastTime) {
    SetContextThink(&CGrenadeHelicopter::WarningBlinkerThink,
                    gpGlobals->curtime + 0.1f, s_pWarningBlinkerContext);
  } else {
    SetContextThink(&CGrenadeHelicopter::WarningBlinkerThink,
                    gpGlobals->curtime + 0.2f, s_pWarningBlinkerContext);
  }
#endif  // HL2_EPISODIC
}
...
void CGrenadeHelicopter::AnimateThink() {
  StudioFrameAdvance();
  SetContextThink(&CGrenadeHelicopter::AnimateThink, gpGlobals->curtime + 0.1f,
                  s_pAnimateThinkContext);
}
...
//------------------------------------------------------------------------------
void CGrenadeHelicopter::VPhysicsCollision(int index,
...
//------------------------------------------------------------------------------
Vector CGrenadeHelicopter::PhysGunLaunchVelocity(const Vector &forward,
                                                 float flMass) {
  // return ( striderbuster_shot_velocity.GetFloat() * forward );

  return BaseClass::PhysGunLaunchVelocity(forward, flMass) *
...
// Pow!
//------------------------------------------------------------------------------
void CGrenadeHelicopter::DoExplosion(const Vector &vecOrigin,
                                     const Vector &vecVelocity) {
  ExplosionCreate(GetAbsOrigin(), GetAbsAngles(),
                  ...
                  (SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODLIGHTS |
                   SF_ENVEXPLOSION_NODECAL | SF_ENVEXPLOSION_NOFIREBALL |
                   SF_ENVEXPLOSION_NOPARTICLES),
                  sk_helicopter_grenadeforce.GetFloat(), this);

  if (GetShakeAmplitude()) {
    UTIL_ScreenShake(GetAbsOrigin(), GetShakeAmplitude(), 150.0, 1.0,
                     GetShakeRadius(), SHAKE_START);
  }
...
#ifdef HL2_EPISODIC
  // remember if we were thrown by player, we can only determine this prior to
  // explosion
  bool bIsThrownByPlayer = IsThrownByPlayer();
  int iHealthBefore = 0;
  // get the health of the helicopter we came from prior to our explosion
  CNPC_AttackHelicopter *pOwner =
      dynamic_cast<CNPC_AttackHelicopter *>(GetOriginalThrower());
  if (pOwner) {
...
#ifdef HL2_EPISODIC
  // if we were thrown by player, look at health of helicopter after explosion
  // and determine if we damaged it
  if (bIsThrownByPlayer && pOwner && (iHealthBefore > 0)) {
    int iHealthAfter = pOwner->GetHealth();
    if (iHealthAfter == iHealthBefore) {
      // The player threw us, we exploded due to timer, and we did not damage
      // the helicopter that fired us.  Send a miss event
      SendMissEvent();
    }
  }
#endif  // HL2_EPISODIC
...
// I think I Pow!
//------------------------------------------------------------------------------
void CGrenadeHelicopter::ResolveFlyCollisionCustom(trace_t &trace,
                                                   Vector &vecVelocity) {
...
void CGrenadeHelicopter::ExplodeConcussion(CBaseEntity *pOther) {
  ...
  if (pEntityHit->ClassMatches("phys_bone_follower") &&
      pEntityHit->GetOwnerEntity()) {
    pEntityHit = pEntityHit->GetOwnerEntity();
  }
  if ((CLASS_COMBINE_GUNSHIP != pEntityHit->Classify()) ||
      !pEntityHit->ClassMatches("npc_helicopter")) {
    // We hit something other than a helicopter.  If the player threw us, send a
    // miss event
    if (IsThrownByPlayer()) {
      SendMissEvent();
    }
...
//-----------------------------------------------------------------------------
void CGrenadeHelicopter::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
                                         PhysGunPickup_t reason) {
  if (reason == PICKED_UP_BY_CANNON) {
    if (!m_bPickedUp) {
      ...
      CReliableBroadcastRecipientFilter filter;
      m_pWarnSound = controller.SoundCreate(
          filter, entindex(), "NPC_AttackHelicopterGrenade.PingCaptured");
      controller.Play(m_pWarnSound, 1.0, PITCH_NORM);
...
// Purpose:
//-----------------------------------------------------------------------------
void CGrenadeHelicopter::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
bool CGrenadeHelicopter::IsThrownByPlayer() {
  // if player is the owner and we're set to explode on contact, then the player
  // threw this grenade.
...
void CGrenadeHelicopter::SendMissEvent() {
  // send a miss event
  IGameEvent *event =
      gameeventmanager->CreateEvent("helicopter_grenade_punt_miss");
  if (event) {
...
//-----------------------------------------------------------------------------
//
// This entity is used to create little force spheres that the helicopters
// should avoid.
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
CBaseEntity *CreateHelicopterAvoidanceSphere(CBaseEntity *pParent,
                                             int nAttachment, float flRadius,
                                             bool bAvoidBelow) {
  CAvoidSphere *pSphere =
      static_cast<CAvoidSphere *>(CreateEntityByName("npc_heli_avoidsphere"));
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidSphere::ComputeAvoidanceForces(CBaseEntity *pEntity,
                                          float flEntityRadius,
                                          float flAvoidTime,
  ...
  for (int i = s_AvoidSpheres.Count(); --i >= 0;) {
    ...
    float t1, t2;
    if (!IntersectRayWithSphere(vecEntityCenter, vecEntityDelta, vecAvoidCenter,
                                flTotalRadius, &t1, &t2)) {
      continue;
    }
    ...
    if (flDist < 0.01f) {
      ...
      if (flZDist < 0.0f &&
          !pSphere->HasSpawnFlags(SF_AVOIDSPHERE_AVOID_BELOW)) {
        Vector vecExitPoint;
        vecDir.z = -vecDir.z;
...
// Where are how should we avoid?
//-----------------------------------------------------------------------------
void CAvoidBox::ComputeAvoidanceForces(CBaseEntity *pEntity,
  ...
  for (int i = s_AvoidBoxes.Count(); --i >= 0;) {
    ...
    if (!IntersectInfiniteRayWithSphere(vecEntityCenter, vecEntityDelta,
                                        vecAvoidCenter, flTotalRadius, &t1,
                                        &t2)) {
      continue;
    }
    ...
    // Unlike the avoid spheres, we also need to make sure the ray intersects
    // the box
    Vector vecLocalCenter, vecLocalDelta;
    pBox->CollisionProp()->WorldToCollisionSpace(vecEntityCenter,
                                                 &vecLocalCenter);
    pBox->CollisionProp()->WorldDirectionToCollisionSpace(vecEntityDelta,
                                                          &vecLocalDelta);
    ...
    trace_t tr;
    if (!IntersectRayWithBox(vecLocalCenter, vecLocalDelta, vecBoxMin,
                             vecBoxMax, 0.0f, &tr))
      continue;
...
//
//-----------------------------------------------------------------------------
CUtlVector<CBombSuppressor::BombSuppressorHandle_t>
...
bool CBombSuppressor::CanBomb(const Vector &vecPosition) {
  for (int i = s_BombSuppressors.Count(); --i >= 0;) {
    ...
    if (pBox->CollisionProp()->IsPointInBounds(vecPosition)) return false;
  }
...
BEGIN_DATADESC(CHelicopterChunk)
  ...
  DEFINE_THINKFUNC(FallThink),
      ...
      DEFINE_PHYSPTR(m_pTailConstraint), DEFINE_PHYSPTR(m_pCockpitConstraint),
...
//			*pEvent -
//-----------------------------------------------------------------------------
void CHelicopterChunk::VPhysicsCollision(int index,
                                         gamevcollisionevent_t *pEvent) {
  BaseClass::VPhysicsCollision(index, pEvent);
...
//-----------------------------------------------------------------------------
CHelicopterChunk *CHelicopterChunk::CreateHelicopterChunk(
    const Vector &vecPos, const QAngle &vecAngles, const Vector &vecVelocity,
    const char *pszModelName, int chunkID) {
  // Drop a flaming, smoking chunk.
  CHelicopterChunk *pChunk =
      CREATE_ENTITY(CHelicopterChunk, "helicopter_chunk");
