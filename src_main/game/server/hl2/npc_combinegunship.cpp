...
Wedge's notes:
  ...
  Gunship should move its head according to flight model when the target is
behind the gunship, or when the target is too far away to shoot at. Otherwise,
the head should aim at the target.
...
#define GUNSHIP_HOVER_SPEED \
  300.0f  // play hover animation if moving slower than this.
...
#define GUNSHIP_MIN_CHASE_DIST_DIFF \
  128.0f  // Distance threshold used to determine when a target has moved enough
          // to update our navigation to it

#define MIN_GROUND_ATTACK_DIST \
  500.0f  // Minimum distance a target has to be for the gunship to consider
          // using the ground attack weapon
#define MIN_GROUND_ATTACK_HEIGHT_DIFF \
  128.0f  // Target's position and hit position must be within this threshold
          // vertically
...
#define GUNSHIP_BELLYBLAST_TARGET_HEIGHT \
  512.0  // Height above targets that the gunship wants to be when bellyblasting
...
class CTargetGunshipCrash : public CPointEntity {
 ...
 public:
  ...
  void GunshipCrashedOnTarget(void) { m_OnCrashed.FireOutput(this, this); }
...
class CNPC_CombineGunship : public CBaseHelicopter {
 public:
  ...
  // NOTENOTE: I'm rather queasy about adding these, as they can lead to nasty
  // bugs...
 ...
 protected:
  ...
  // again.
  static int m_poseFlex_Horz, m_poseFlex_Vert, m_posePitch, m_poseYaw,
      m_poseFin_Accel, m_poseFin_Sway;
...
SendPropVector(SENDINFO(m_vecHitPos), -1, SPROP_COORD),
    ...
    DEFINE_SOUNDPATCH(m_pAirExhaustSound), DEFINE_SOUNDPATCH(m_pAirBlastSound),
    ...
    DEFINE_FIELD(m_angGun, FIELD_VECTOR), DEFINE_PHYSPTR(m_pCrashingController),
    DEFINE_EMBEDDED(m_crashCallback), DEFINE_FIELD(m_hRagdoll, FIELD_EHANDLE),
    ...
    DEFINE_ARRAY(m_bDamageOutputsFired, FIELD_BOOLEAN,
                 GUNSHIP_NUM_DAMAGE_OUTPUTS),

    // Function pointers
    DEFINE_INPUTFUNC(FIELD_VOID, "OmniscientOn", InputOmniscientOn),
    DEFINE_INPUTFUNC(FIELD_VOID, "OmniscientOff", InputOmniscientOff),
    DEFINE_INPUTFUNC(FIELD_FLOAT, "SetPenetrationDepth",
    ...
    DEFINE_INPUTFUNC(FIELD_VOID, "DisableGroundAttack",
...
void CNPC_CombineGunship::CreateBellyBlastEnergyCore(void) {
  CCitadelEnergyCore *pCore = static_cast<CCitadelEnergyCore *>(
      CreateEntityByName("env_citadel_energy_core"));
...
void CNPC_CombineGunship::Spawn(void) {
  ...
  ExtractBbox(SelectHeaviestSequence(ACT_GUNSHIP_PATROL), m_cullBoxMins,
              m_cullBoxMaxs);
  BaseClass::Spawn();

  InitPathingData(GUNSHIP_ARRIVE_DIST, GUNSHIP_MIN_CHASE_DIST_DIFF,
                  sk_gunship_burst_min.GetFloat());
  ...
  //!!!HACKHACK
  // This tricks the AI code that constantly complains that the gunship has no
  // schedule.
  ...
  m_bCanGroundAttack =
      (m_spawnflags & SF_GUNSHIP_NO_GROUND_ATTACK) ? false : true;
...
void CNPC_CombineGunship::Ping(void) {
  ...
  if (GetEnemy() != NULL) {
    if (!HasCondition(COND_SEE_ENEMY) &&
        gpGlobals->curtime > m_flTimeNextPing) {
...
void CNPC_CombineGunship::PlayPatrolLoop(void) {
  ...
  CSoundEnvelopeController::GetController(); controller.SoundChangeVolume(
  m_pPatrolSound, 1.0, 1.0 ); controller.SoundChangeVolume( m_pAngrySound,
  0.0, 1.0 );
  */
}
...
void CNPC_CombineGunship::PlayAngryLoop(void) {
  ...
  CSoundEnvelopeController &controller =
  CSoundEnvelopeController::GetController(); controller.SoundChangeVolume(
  m_pPatrolSound, 0.0, 1.0 ); controller.SoundChangeVolume( m_pAngrySound,
  1.0, 1.0 );
  */
}
...
Vector CNPC_CombineGunship::GetGroundAttackHitPosition(void) {
  ...
  AI_TraceLine(vecShootPos, vecShootPos + Vector(0, 0, -MAX_TRACE_LENGTH),
               MASK_SHOT, this, COLLISION_GROUP_NONE, &tr);

  if (m_hGroundAttackTarget) {
    return Vector(tr.endpos.x, tr.endpos.y,
                  m_hGroundAttackTarget->WorldSpaceCenter().z);
  }
  return tr.endpos;
}
...
bool CNPC_CombineGunship::CheckGroundAttack(void) {
  if (m_bCanGroundAttack == false) return false;
...
void CNPC_CombineGunship::ManageWarningBeam(void) {
  ...
  trace_t tr;
  CTraceFilterSkipTwoEntities filter(m_hGroundAttackTarget, this,
  ...
  while (tr.endpos != m_vecHitPos) {
    ...
    if (tr.fraction != 1.0) {
      ...
      if (tr.startsolid) {
        ...
        Vector vEndPunch =
            vStartPunch + vDir * (flLength * tr.fractionleftsolid);

        UTIL_TraceLine(vEndPunch, m_vecHitPos, MASK_SOLID, &filter, &tr);

        trace_t tr2;
        UTIL_TraceLine(vEndPunch, vEndPunch - vDir * 2, MASK_SOLID, &filter,
                       &tr2);

        if ((m_flGroundAttackTime - gpGlobals->curtime) <= 2.0f) {
          g_pEffects->EnergySplash(tr2.endpos + vDir * 8, tr2.plane.normal,
                                   true);
        }

        g_pEffects->Sparks(tr2.endpos,
                           3.0f - (m_flGroundAttackTime - gpGlobals->curtime),
                           3.5f - (m_flGroundAttackTime - gpGlobals->curtime),
                           &tr2.plane.normal);
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_CombineGunship::DoBellyBlastDamage(trace_t &tr, Vector vMins,
                                             Vector vMaxs) {
  ...
  int count =
      UTIL_EntitiesInBox(pList, 100, tr.endpos + vMins, tr.endpos + vMaxs, 0);
  ...
  for (int i = 0; i < count; i++) {
    ...
    trace_t groundTrace;
    UTIL_TraceLine(pEntity->GetAbsOrigin(),
                   pEntity->GetAbsOrigin() - Vector(0, 0, 256), MASK_SOLID,
                   pEntity, COLLISION_GROUP_NONE, &groundTrace);
...
void CNPC_CombineGunship::DoGroundAttackExplosion(void) {
  ...
  trace_t tr;
  UTIL_TraceLine(vecSrc, impactPoint, MASK_SHOT, this, COLLISION_GROUP_NONE,
                 &tr);
  ...
  Vector vBlastMins =
      Vector(-GUNSHIP_BELLY_BLAST_RADIUS, -GUNSHIP_BELLY_BLAST_RADIUS, 0);
  Vector vBlastMaxs =
      Vector(GUNSHIP_BELLY_BLAST_RADIUS, GUNSHIP_BELLY_BLAST_RADIUS, 96);
...
void CNPC_CombineGunship::StopGroundAttack(bool bDoAttack) {
  if (!m_bIsGroundAttacking) return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CNPC_CombineGunship::DrawRotorWash(float flAltitude,
                                        const Vector &vecRotorOrigin) {
...
//------------------------------------------------------------------------------
// Purpose : Override the desired position if your derived helicopter is doing
// something special
//------------------------------------------------------------------------------
void CNPC_CombineGunship::UpdateDesiredPosition(void) {
  if (m_hCrashTarget) {
    ...
    SetDesiredPosition(m_hGroundAttackTarget->GetAbsOrigin() +
                       Vector(0, 0, GUNSHIP_BELLYBLAST_TARGET_HEIGHT));
  }
}
...
void CNPC_CombineGunship::DoCombat(void) {
    ...
    if (HasCondition(COND_NEW_ENEMY)) {
      ...
      if (IsTargettingMissile()) {
        ...
        if ((m_flTimeNextAttack > gpGlobals->curtime) &&
            ((m_flTimeNextAttack - gpGlobals->curtime) >
             GUNSHIP_MISSILE_MAX_RESPONSE_TIME)) {
          m_flTimeNextAttack =
              gpGlobals->curtime + GUNSHIP_MISSILE_MAX_RESPONSE_TIME;
  ...
  if (m_hGroundAttackTarget && !m_bIsGroundAttacking) {
    // If we're over it, blast. Can't use GetDesiredPosition() because it's not
    // updated yet.
    Vector vecTarget = m_hGroundAttackTarget->GetAbsOrigin() +
                       Vector(0, 0, GUNSHIP_BELLYBLAST_TARGET_HEIGHT);
  ...
  if (GetEnemy() != NULL && GetEnemy()->Classify() == CLASS_FLARE &&
      flDeltaSeen > GUNSHIP_FLARE_IGNORE_TIME) {
...
bool CNPC_CombineGunship::ChooseEnemy(void) {
  ...
  // stopping a burst before he's really fired at all, which makes him look
  // indecisive.
...
void CNPC_CombineGunship::MoveHead(void) {
  ...
  /*
          This head-turning code will cause the head to POP when switching from
     looking at the enemy to looking according to the flight model. I will fix
     this later. Right now I'm turning the code over to Ken for some aiming
     fixups. (sjb)
  */
  ...
  while (1) {
    ...
    flYaw = UTIL_Approach(GetLocalAngularVelocity().y, flYaw,
                          2.0 * 10 * m_flDeltaT);
    flPitch = UTIL_Approach(GetLocalAngularVelocity().x, flPitch,
                            2.0 * 10 * m_flDeltaT);
...
void CNPC_CombineGunship::PrescheduleThink(void) {
  ...
  if (m_lifeState == LIFE_ALIVE) {
    ...
    if (bCreateExplosion) {
      Vector explodePoint;
      if (m_hRagdoll) {
        m_hRagdoll->CollisionProp()->RandomPointInBounds(
    ...
    if (m_flNextGunshipCrashFind && !m_hRagdoll) {
      ...
      // closer to a different point than the one we were near when we first
      // died.
  ...
  SetPoseParameter(m_posePitch, random->RandomFloat(GUNSHIP_HEAD_MAX_LEFT,
                                                    GUNSHIP_HEAD_MAX_RIGHT));
  SetPoseParameter(m_poseYaw, random->RandomFloat(GUNSHIP_HEAD_MAX_UP,
                                                  GUNSHIP_HEAD_MAX_DOWN));
#endif
}
...
bool CNPC_CombineGunship::FireGun(void) {
  ...
  if (GetEnemy() && !m_bIsFiring && gpGlobals->curtime > m_flTimeNextAttack) {
    ...
    if (!bTargetingMissile &&
        GroundDistToPosition(GetEnemy()->GetAbsOrigin()) < GUNSHIP_STITCH_MIN)
      return false;
...
void CNPC_CombineGunship::FireCannonRound(void) {
  ...
  if (g_debug_gunship.GetInt() == GUNSHIP_DEBUG_STITCHING) {
    ...
    NDebugOverlay::Line(vecMuzzle, vecMuzzle + (vForward * 64.0f), 255, 0, 0,
                        true, 1.0f);
    NDebugOverlay::Line(vecMuzzle, vecMuzzle + (vRight * 32.0f), 0, 255, 0,
                        true, 1.0f);
    NDebugOverlay::Line(vecMuzzle, vecMuzzle + (vUp * 32.0f), 0, 0, 255, true,
                        1.0f);
  }
  ...
  if (IsTargettingMissile()) {
    // Fire a fake shot
    FireBullets(1, vecMuzzle, vecToEnemy, VECTOR_CONE_5DEGREES, 8192,
                m_iAmmoType, 1);
    ...
    if ((DotProduct(threatDir, missileDir) > 0.95f) && (threatDist < 1024.0f)) {
      if (random->RandomInt(0, 1) == 0) {
        ...
        CalculateBulletDamageForce(&info, m_iAmmoType, -threatDir,
                                   WorldSpaceCenter());
    ...
    FireBulletsInfo_t info(1, vecMuzzle, vecToEnemy, vec3_origin,
                           MAX_COORD_RANGE, m_iAmmoType);
    info.m_iTracerFreq = 1;
    CAmmoDef *pAmmoDef = GetAmmoDef();
    info.m_iPlayerDamage = pAmmoDef->PlrDamage(m_iAmmoType);

    // If we've already hit the player, do 0 damage. This ensures we don't hit
    // the player multiple times during a single burst.
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CNPC_CombineGunship::FVisible(CBaseEntity *pEntity, int traceMask,
...
//-----------------------------------------------------------------------------
// Purpose: Allows the gunship to fire at an unseen enemy. The gunship is
// relying
//			on hitting the target with bullets that will punch
// through the 			cover that the enemy is hiding behind. (Such as
...
void CNPC_CombineGunship::Event_Killed(const CTakeDamageInfo &info) {
  ...
  // BUGBUG: Isn't this sound just going to get stomped when the base class
  // calls StopLoopingSounds() ??
  CPASAttenuationFilter filter2(this);
  m_pRotorSound = controller.SoundCreate(filter2, entindex(),
...
bool CNPC_CombineGunship::FindNearestGunshipCrash(void) {
  // Find the nearest crash point. If we find one, we'll try to fly to it and
  // die. If we can't find one, we'll die right here.
  bool bFoundAnyCrashTargets = false;
  float flNearest = MAX_TRACE_LENGTH * MAX_TRACE_LENGTH;
  CTargetGunshipCrash *pNearest = NULL;
  CBaseEntity *pEnt = NULL;
  while ((pEnt = gEntList.FindEntityByClassname(
              pEnt, "info_target_gunshipcrash")) != NULL) {
    CTargetGunshipCrash *pCrashTarget =
        assert_cast<CTargetGunshipCrash *>(pEnt);
    if (pCrashTarget->IsDisabled()) continue;
    ...
    if (flDist < flNearest) {
      trace_t tr;
      UTIL_TraceLine(WorldSpaceCenter(), pEnt->WorldSpaceCenter(),
  ...
  if (!pNearest) {
    // If we found a gunship crash, but none near enough, claim we did find one,
    // so that we don't blow up yet. This will give us 3 seconds to attempt to
    // find one before dying.
  ...
  if (g_debug_gunship.GetInt()) {
    NDebugOverlay::Line(GetAbsOrigin(), m_hCrashTarget->GetAbsOrigin(), 0, 255,
                        0, true, 0.5);
    NDebugOverlay::Box(m_hCrashTarget->GetAbsOrigin(), -Vector(200, 200, 200),
                       Vector(200, 200, 200), 0, 255, 0, 128, 0.5);
  }
...
void CNPC_CombineGunship::Flight(void) {
  ...
  if (g_debug_gunship.GetInt() == GUNSHIP_DEBUG_PATH) {
    NDebugOverlay::Line(GetLocalOrigin(), GetDesiredPosition(), 0, 0, 255, true,
                        0.1);
  }
  ...
  accel.z = 2.0 * (deltaPos.z - GetAbsVelocity().z * dt + 0.5 * 384 * dt * dt) /
            (dt * dt);
  ...
  if (!m_hCrashTarget && m_lifeState == LIFE_DYING && !hl2_episodic.GetBool()) {
    ...
    // EXCEPT In episodic, where forcing it to the ground means it crashes where
    // the player can't see (attic showdown) (sjb)
    vecImpulse.z = -10;
  } else {
  ...
  float speed = m_flForce *
                DotProduct(vecVelDir,
                           vecAccelDir);  // * DotProduct( forward, vecVelDir );
  ...
  if (g_debug_gunship.GetInt() == GUNSHIP_DEBUG_PATH) {
    NDebugOverlay::Line(GetLocalOrigin(), GetLocalOrigin() + vecImpulse, 255, 0,
                        0, true, 0.1);
  }
...
void CNPC_CombineGunship::InitializeRotorSound(void) {
  ...
  m_pAirExhaustSound = controller.SoundCreate(
      filter, entindex(), "NPC_CombineGunship.ExhaustSound");
  m_pAirBlastSound = controller.SoundCreate(
      filter, entindex(), "NPC_CombineGunship.RotorBlastSound");
...
void CNPC_CombineGunship::UpdateRotorSoundPitch(int iPitch) {
  ...
  if (pPlayer) {
    ...
    float flVolume = std::clamp(
        RemapVal(flDistance, (900 * 900), (1800 * 1800), 1, 0), 0.0f, 1.0f);
    controller.SoundChangeVolume(m_pAirExhaustSound,
                                 flVolume * GetRotorVolume(), 0.1);

    // Fade in the blast when it's close to the player (in 2D)
    flVolume = std::clamp(RemapVal(flDistance, (600 * 600), (700 * 700), 1, 0),
                          0.0f, 1.0f);
    controller.SoundChangeVolume(m_pAirBlastSound, flVolume * GetRotorVolume(),
                                 0.1);
  }
...
void CNPC_CombineGunship::SelfDestruct(void) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    ExplosionCreate(vecOrigin + vecDelta, QAngle(-90, 0, 0), this, 10, 10,
                    false);
  }
  ...
  if (!m_hCrashTarget) {
    ...
    PropBreakableCreateAll(pBreakEnt->GetModelIndex(),
                           pBreakEnt->VPhysicsGetObject(),
                           pBreakEnt->GetAbsOrigin(), pBreakEnt->GetAbsAngles(),
                           pBreakEnt->GetAbsVelocity(), angVelocity, 1.0, 800,
                           COLLISION_GROUP_NPC, pBreakEnt);

    // Throw out some small chunks too
    CPVSFilter filter(GetAbsOrigin());
    for (int i = 0; i < 20; i++) {
      Vector gibVelocity = RandomVector(-100, 100) * 10;
      int iModelIndex = modelinfo->GetModelIndex(
          g_PropDataSystem.GetRandomChunkModel("MetalChunks"));
      te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle,
                     Vector(40, 40, 40), gibVelocity, iModelIndex, 400, 1, 2.5,
                     BREAK_METAL);
    }
...
#ifdef HL2_EPISODIC
  NPCEventResponse()->TriggerEvent("TLK_CITIZEN_RESPONSE_KILLED_GUNSHIP", false,
                                   false);
#endif
}
...
void CNPC_CombineGunship::UpdateEnemyTarget(void) {
  ...
  if (g_debug_gunship.GetInt() == GUNSHIP_DEBUG_STITCHING) {
    // Final position
    NDebugOverlay::Cross3D(m_vecAttackPosition, -Vector(2, 2, 2),
                           Vector(2, 2, 2), 0, 0, 255, true, 4.0f);
  }

  float yawDiff = UTIL_AngleDiff(lastChaseAngles[YAW], chaseAngles[YAW]);
  float maxYaw = bTargettingPlayer ? 4.0f : 20.0f;

  yawDiff = std::clamp(yawDiff, -maxYaw, maxYaw);

  chaseAngles[PITCH] = 0.0f;
  chaseAngles[ROLL] = 0.0f;

  bool bMaxHits = (m_iBurstHits >= GUNSHIP_MAX_HITS_PER_BURST ||
                   (GetEnemy() && !GetEnemy()->IsAlive()));
  ...
  // If we've hit the target already, or we're not close enough to it, then just
  // stitch along
  if (bMaxHits || (m_vecAttackPosition - enemyPos).LengthSqr() > (64 * 64)) {
    ...
    if (g_debug_gunship.GetInt() == GUNSHIP_DEBUG_STITCHING) {
      NDebugOverlay::Line(m_vecAttackPosition,
                          m_vecAttackPosition + (m_vecAttackVelocity * 0.1),
                          255, 0, 0, true, 4.0f);
    }
...
//------------------------------------------------------------------------------
bool CNPC_CombineGunship::PoseGunTowardTargetDirection(
...
//			 this is often modified by m_flAttackOffset to provide
// the
//'stitching' 			 behavior that's so popular with the kids these
// days (sjb)
//
...
//			iTracerType -
//-----------------------------------------------------------------------------
void CNPC_CombineGunship::MakeTracer(const Vector &vecTracerSrc,
  ...
  switch (iTracerType) {
    case TRACER_LINE: {
      ...
      UTIL_Tracer(vecTracerSrc, tr.endpos, 0, TRACER_DONT_USE_ATTACHMENT, 8000,
                  true, "GunshipTracer");
    } break;
...
// Output : int
//-----------------------------------------------------------------------------
void CNPC_CombineGunship::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (info.GetDamageType() & DMG_BULLET) {
    if (random->RandomInt(0, 2) == 0) {
      ...
      UTIL_Tracer(ptr->endpos, end, entindex(), TRACER_DONT_USE_ATTACHMENT,
                  3500);
    }
...
//-----------------------------------------------------------------------------
// Purpose: This is necessary to ensure that the game doesn't break if a
// mapmaker has outputs that
//			must be fired on gunships, and the player switches skill
// levels 			midway through a gunship battle.
// Input  : iDamageNumber -
//-----------------------------------------------------------------------------
...
int CNPC_CombineGunship::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (m_bInvulnerable == false) {
    ...
    int healthIncrement =
        iHealthIncrements -
        (GetHealth() / (float)((GetMaxHealth() / (float)iHealthIncrements)));
    switch (healthIncrement) {
      case 1:
        // If we're on Easy, we're half dead now, so fire the rest of our
        // outputs too This is done in case the mapmaker's connected those
        // inputs to something important that has to happen before the gunship
        // dies.
        if (g_pGameRules->IsSkillLevel(SKILL_EASY)) {
...
void CNPC_CombineGunship::StopCannonBurst(void) {
  ...
  float flDelay = std::clamp(flPerc * m_flBurstDelay, 0.5f, m_flBurstDelay);
  ...
  if (m_pCannonSound != NULL) {
    CSoundEnvelopeController::GetController().SoundChangeVolume(m_pCannonSound,
                                                                0.0, 0.05);
  }
...
bool CNPC_CombineGunship::IsValidEnemy(CBaseEntity *pEnemy) {
  ...
  if (pEnemy->IsAlive() && !pEnemy->MyNPCPointer() &&
      FClassnameIs(pEnemy, "rpg_missile"))
    return true;
