...
BEGIN_DATADESC(CSpeaker)
  ...
  DEFINE_KEYFIELD(m_delayMin, FIELD_FLOAT, "delaymin"),
      ...
      // DEFINE_CUSTOM_FIELD( m_pInstancedResponseSystem,
...
void CSpeaker::Spawn(void) {
  ...
  //	{
  //		Warning( "'speaker' entity using rule set %s with empty concept
  // string\n", soundfile );
  //	}
...
void CSpeaker::Precache(void) {
  if (!FBitSet(m_spawnflags.operator const int &(), SF_SPEAKER_START_SILENT)) {
  ...
  if (!m_pInstancedResponseSystem) {
    m_pInstancedResponseSystem =
        PrecacheCustomResponseSystem(STRING(m_iszRuleScriptFile));
  }
}

//-----------------------------------------------------------------------------
// Purpose: Need a custom save restore so we can restore the instanced response
// system by name
...
int CSpeaker::Save(ISave &save) {
  ...
  if (iret) {
    bool doSave =
        (m_pInstancedResponseSystem && (m_iszRuleScriptFile != NULL_STRING))
            ? true
            : false;
    ...
    if (doSave) {
      ...
      {
        SaveRestoreFieldInfo_t fieldInfo = {&m_pInstancedResponseSystem, 0,
                                            NULL};
...
void CSpeaker::SpeakerThink(void) {
  ...
  if (!g_AIFriendliesTalkSemaphore.IsAvailable(this) ||
      !g_AIFoesTalkSemaphore.IsAvailable(this)) {
    float releaseTime = std::max(g_AIFriendliesTalkSemaphore.GetReleaseTime(),
                                 g_AIFoesTalkSemaphore.GetReleaseTime());
...
//
// If an announcement is pending, cancel it.  If no announcement is pending,
// start one.
//
