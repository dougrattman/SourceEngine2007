...
//
//------------------------------------------------------------------------------
bool CNPC_EnemyFinder::FVisible(CBaseEntity *pTarget, int traceMask,
                                CBaseEntity **ppBlocker) {
  float flTargetDist = GetAbsOrigin().DistTo(pTarget->GetAbsOrigin());
  if (flTargetDist < m_flMinSearchDist) return false;

  if (m_flMaxSearchDist && flTargetDist > m_flMaxSearchDist) return false;

  if (!FBitSet(m_spawnflags.operator const int &(), SF_ENEMY_FINDER_CHECK_VIS))
    return true;
  ...
  CTraceFilterSkipTwoEntities traceFilter(pTarget, pVehicle,
...
void CNPC_EnemyFinder::StartNPC(void) {
  AddSpawnFlags(SF_NPC_FALL_TO_GROUND);  // this prevents CAI_BaseNPC from
                                         // slamming the finder to the ground
                                         // just because it's not MOVETYPE_FLY
  BaseClass::StartNPC();
...
void CNPC_EnemyFinder::PrescheduleThink() {
  ...
  if (ai_debug_enemyfinders.GetBool()) {
    ...
    if (IsInSquad() && GetSquad()->NumMembers() > 1) {
      ...
      while (pSquadmate) {
        NDebugOverlay::Line(WorldSpaceCenter(), pSquadmate->EyePosition(), 255,
                            255, 0, false, 0.1f);
...
bool CNPC_EnemyFinder::ShouldAlwaysThink() {
  if (BaseClass::ShouldAlwaysThink()) return true;
  ...
  if (pPlayer && IRelationType(pPlayer) == D_HT) {
    ...
    if (!m_flMaxSearchDist || playerDistSqr <= Square(m_flMaxSearchDist)) {
      if (!FBitSet(m_spawnflags.operator const int &(),
                   SF_ENEMY_FINDER_CHECK_VIS))
        return true;

      if (playerDistSqr <= Square(50 * 12)) return true;
    }
  }
...
class CNPC_EnemyFinderCombineCannon : public CNPC_EnemyFinder {
 ...
 public:
  ...
  float m_flTimeWideFOV;  // If this is > gpGlobals->curtime, we have 180 degree
                          // viewcone.
  string_t m_iszSnapToEnt;
};
LINK_ENTITY_TO_CLASS(npc_enemyfinder_combinecannon,
                     CNPC_EnemyFinderCombineCannon);
...
BEGIN_DATADESC(CNPC_EnemyFinderCombineCannon)
  DEFINE_FIELD(m_flTimeNextHateNPC, FIELD_TIME),
      ...
      DEFINE_INPUTFUNC(FIELD_FLOAT, "SetWideFOVForSeconds",
...
void CNPC_EnemyFinderCombineCannon::Spawn() {
  ...
  if (m_iszSnapToEnt != NULL_STRING) {
    ...
    if (pSnapToEnt != NULL) {
      //!!!HACKHACK - this eight-inch offset puts this enemyfinder perfectly
      //! on-bore
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_EnemyFinderCombineCannon::FVisible(CBaseEntity *pEntity,
                                             int traceMask,
                                             CBaseEntity **ppBlocker) {
#if 1
  ...
  if (pEntity->IsPlayer() && result == false) {
    ...
    if (pBlocker != NULL) {
      ...
      {
        // Anytime the line of sight is blocked by something I can hurt, I have
        // line of sight. This will make the func_tank_combinecannon shoot the
        // blocking object. This will continue until the gun bores through to
        // the player or clears all interposing breakables and finds its
        // progress impeded by something truly solid. So lie, and say we CAN see
        // the player.
        result = true;
      }
    }
...
//			Go through short periods of time where NPCs may distract
// me
//
//			ALSO- ignore NPC's (focus only on the player) when I'm
// in 			wide viewcone mode.
//-----------------------------------------------------------------------------
bool CNPC_EnemyFinderCombineCannon::IsValidEnemy(CBaseEntity *pTarget) {
  if (m_flTimeWideFOV > gpGlobals->curtime && !pTarget->IsPlayer()) {
    ...
    // kill zombies most of the time, but immediately turn their attention to
    // the player when necessary (by ignoring everything else)
  ...
  if (bResult && !pTarget->IsPlayer()) {
    ...
    for (i = 0; i < s_ListEnemyfinders.Count(); i++) {
      if (s_ListEnemyfinders[i] == this) continue;
  ...
  if( bResult == true && pTarget->IsNPC() && pPlayer != NULL && FInViewCone(
  pPlayer ) )
  {
          if( HasCondition(COND_SEE_PLAYER) )
          {
                  // The player is visible! Immediately ignore all NPCs as
  enemies. return false;
          }

          // The base class wants to call this a valid enemy. We may choose to
  interfere
          // If the player is in my viewcone. That means that my func_tank could
  potentially
          // harass the player. This means I should meter the time I spend
  shooting at npcs
          ...
          if( m_flTimeStopHateNPC != EF_COMBINE_CANNON_HATE_TIME_INVALID )
          {
                  ...
                  if( gpGlobals->curtime > m_flTimeStopHateNPC )
                  {
                          ...
                          m_flTimeStopHateNPC =
  EF_COMBINE_CANNON_HATE_TIME_INVALID; m_flTimeNextHateNPC = gpGlobals->curtime
  + ai_ef_hate_npc_frequency.GetFloat(); return bResult;
                  }
          }
          else
          {
                  // We do not hate NPCs at the moment. Is it time to turn it
  on? if( gpGlobals->curtime > m_flTimeNextHateNPC )
                  {
                          m_flTimeStopHateNPC = gpGlobals->curtime +
  ai_ef_hate_npc_duration.GetFloat();
                  }
                  else
                  {
                          // Stop harassing player to attack something else
  higher priority. if( IRelationPriority(pTarget) > iPlayerRelationPriority )
                                  return bResult;
                  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_EnemyFinderCombineCannon::InputSetWideFOVForSeconds(
