...
void CHLMachineGun::PrimaryAttack(void) {
  ...
  if (UsesClipsForAmmo1()) {
    if (iBulletsToFire > m_iClip1) iBulletsToFire = m_iClip1;
  ...
  CSoundEnt::InsertSound(SOUND_COMBAT, GetAbsOrigin(),
                         SOUNDENT_VOLUME_MACHINEGUN, 0.2, pPlayer);
...
int CHLMachineGun::WeaponRangeAttack1Condition(float flDot, float flDist) {
  ...
  } else if (flDot < 0.5f)  // UNDONE: Why check this here? Isn't the AI
                            // checking this already?
  {
...
// Purpose:
//-----------------------------------------------------------------------------
void CHLMachineGun::DoMachineGunKick(CBasePlayer *pPlayer, float dampEasy,
...
#define KICK_MIN_Z 0.1f  // Degrees
  ...
  if (random->RandomInt(-1, 1) >= 0) vecScratch.y *= -1;

  // Wobble up and down
  if (random->RandomInt(-1, 1) >= 0) vecScratch.z *= -1;
...
int CHLMachineGun::WeaponSoundRealtime(WeaponSound_t shoot_type) {
  ...
  float dt = std::clamp(m_flAnimTime - m_flPrevAnimTime, 0.0f, 0.2f);
...
void CHLSelectFireMachineGun::PrimaryAttack(void) {
  ...
  switch (m_iFireMode) {
    ...
    case FIREMODE_3RNDBURST:
      ...
      // Call the think function directly so that the first round gets fired
      // immediately.
      BurstThink();
...
//
//-----------------------------------------------------------------------------
void CHLSelectFireMachineGun::WeaponSound(WeaponSound_t shoot_type,
    ...
    switch (m_iFireMode) {
      ...
      case FIREMODE_3RNDBURST:
        if (m_iBurstSize == GetBurstSize() && m_iClip1 >= m_iBurstSize) {
          // First round of a burst, and enough bullets remaining in the clip to
          // fire the whole burst
          BaseClass::WeaponSound(BURST, soundtime);
        } else if (m_iClip1 < m_iBurstSize) {
          // Not enough rounds remaining in the magazine to fire a burst, so
          // play the gunshot sounds individually as each round is fired.
          BaseClass::WeaponSound(SINGLE, soundtime);
        }
...
//-----------------------------------------------------------------------------
int CHLSelectFireMachineGun::WeaponRangeAttack1Condition(float flDot,
                                                         float flDist) {
  if (m_iClip1 <= 0) {
    return COND_NO_PRIMARY_AMMO;
  } else if (flDist < m_fMinRange1) {
