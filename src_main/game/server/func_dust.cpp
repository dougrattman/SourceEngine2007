...
SendPropFloat(SENDINFO(m_flSize), -1, SPROP_COORD),
    ...
    SendPropVector(SENDINFO(m_vecDirection), 4, 0, -1.0f,
                   1.0f),  // cheap normal
    END_SEND_TABLE()
...
//			&angles -
//-----------------------------------------------------------------------------
void TE_Dust(IRecipientFilter &filter, float delay, const Vector &pos,
             const Vector &dir, float size, float speed) {
