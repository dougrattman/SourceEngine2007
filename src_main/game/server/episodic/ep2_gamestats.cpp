...
const char *CEP2GameStats::GetStatSaveFileName(void) {
  // overriding the default for backwards compatibility with release stat
  // tracking code
...
static char const *ep2Maps[] = {
    "ep2_outland_01", "ep2_outland_02",  "ep2_outland_03",  "ep2_outland_04",
    "ep2_outland_05", "ep2_outland_06",  "ep2_outland_06a", "ep2_outland_07",
    "ep2_outland_08", "ep2_outland_09",  "ep2_outland_10",  "ep2_outland_10a",
...
void CEP2GameStats::Event_LevelInit(void) {
  ...
  m_pCurrentMap->Init(STRING(gpGlobals->mapname), gpGlobals->curtime, pchTag,
                      gpGlobals->mapversion);
}

Ep2LevelStats_t::EntityDeathsLump_t *CEP2GameStats::FindDeathsLump(
    char const *npcName) {
  if (!m_pCurrentMap) return NULL;
  ...
  if (Q_strnicmp(name, "npc_", 4)) return NULL;
...
Ep2LevelStats_t::WeaponLump_t *CEP2GameStats::FindWeaponsLump(
    char const *pchWeaponName, bool bPrimary) {
  if (!m_pCurrentMap) return NULL;
  ...
  char lookup[512];
  Q_snprintf(lookup, sizeof(lookup), "%s_%s", pchWeaponName,
...
Ep2LevelStats_t::GenericStatsLump_t *CEP2GameStats::FindGenericLump(
    char const *pchStatName) {
  if (!m_pCurrentMap) return NULL;
  if (!pchStatName || !*pchStatName) return NULL;
...
// Purpose:
//-----------------------------------------------------------------------------
void CEP2GameStats::Event_PlayerDamage(CBasePlayer *pBasePlayer,
                                       const CTakeDamageInfo &info) {
  BaseClass::Event_PlayerDamage(pBasePlayer, info);

  m_pCurrentMap->m_FloatCounters[Ep2LevelStats_t::COUNTER_DAMAGETAKEN] +=
      info.GetDamage();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CEP2GameStats::Event_PlayerKilledOther(CBasePlayer *pAttacker,
                                            CBaseEntity *pVictim,
                                            const CTakeDamageInfo &info) {
  ...
  if (lump) {
    ...
    CPropVehicleDriveable *veh =
        dynamic_cast<CPropVehicleDriveable *>(pAttacker);
    if (!veh) veh = dynamic_cast<CPropVehicleDriveable *>(info.GetInflictor());
    if (veh) {
      ...
      if (driver && driver->IsPlayer()) {
        ...
        StatsLog(
            "  Vehicular homicide [%I64d] of %s's\n",
            m_pCurrentMap
                ->m_IntCounters[Ep2LevelStats_t::COUNTER_VEHICULARHOMICIDES],
            pchVictim);
      }
...
// Purpose:
//-----------------------------------------------------------------------------
void CEP2GameStats::Event_PlayerKilled(CBasePlayer *pPlayer,
                                       const CTakeDamageInfo &info) {
  ...
  StatsLog("CEP2GameStats::Event_PlayerKilled at location [%d %d %d]\n",
           (int)death.nPosition[0], (int)death.nPosition[1],
           (int)death.nPosition[2]);
  ...
  Ep2LevelStats_t::SaveGameInfoRecord2_t *rec =
      map->m_SaveGameInfo.m_pCurrentRecord;
  if (rec) {
...
void CEP2GameStats::Event_PlayerTraveled(CBasePlayer *pBasePlayer,
                                         float distanceInInches,
                                         bool bInVehicle, bool bSprinting) {
  BaseClass::Event_PlayerTraveled(pBasePlayer, distanceInInches, bInVehicle,
                                  bSprinting);
  ...
  if (intPart > 0) {
    if (bInVehicle) {
      m_pCurrentMap
          ->m_IntCounters[Ep2LevelStats_t::COUNTER_DISTANCE_INVEHICLE] +=
          intPart;
    } else {
      if (bSprinting) {
        m_pCurrentMap->m_IntCounters
            [Ep2LevelStats_t::COUNTER_DISTANCE_ONFOOTSPRINTING] += intPart;
      } else {
        m_pCurrentMap
            ->m_IntCounters[Ep2LevelStats_t::COUNTER_DISTANCE_ONFOOT] +=
            intPart;
      }
    }
  }
...
void CEP2GameStats::Event_WeaponFired(CBasePlayer *pShooter, bool bPrimary,
                                      char const *pchWeaponName) {
  BaseClass::Event_WeaponFired(pShooter, bPrimary, pchWeaponName);

  Ep2LevelStats_t::WeaponLump_t *lump =
      FindWeaponsLump(pchWeaponName, bPrimary);
  if (lump) {
    ++lump->m_nShots;
  }
}

void CEP2GameStats::Event_WeaponHit(CBasePlayer *pShooter, bool bPrimary,
                                    char const *pchWeaponName,
                                    const CTakeDamageInfo &info) {
...
void CEP2GameStats::Event_SaveGame(void) {
  ...
  if (!pchSaveFile || !pchSaveFile[0]) return;
  ...
  if (pPlayer) {
    ...
    rec->m_SaveType =
        Q_stristr(pchSaveFile, "autosave")
            ...
            : Ep2LevelStats_t::SaveGameInfoRecord2_t::TYPE_USERSAVE;

    StatsLog("save pos %i %i %i w/ health %d\n", rec->m_nSavePos[0],
             rec->m_nSavePos[1], rec->m_nSavePos[2], rec->m_nSaveHealth);
  }
}
...
void CEP2GameStats::Event_LoadGame(void) {
  ...
  if (pSaveGameInfo->m_nCurrentSaveFileTime == 0 ||
    ...
    // Latch off previous
    StatsLog(
        "Relatching save game file due to time or filename change (%s : %u)\n",
        name, uFileTime);
    pSaveGameInfo->Latch(name, uFileTime);
  }
}

void CEP2GameStats::Event_FlippedVehicle(CBasePlayer *pDriver,
...
void CEP2GameStats::Event_PreSaveGameLoaded(char const *pSaveName,
                                            bool bInGame) {
  ...
  // We're loading a saved game while the player is still alive (are they
  // stuck?)
  if (pPlayer->IsAlive()) {
    ++m_pCurrentMap
...
void CEP2GameStats::Event_DecrementPlayerEnteredNoClip(
...
void CEP2GameStats::Event_IncrementCountedStatistic(
    const Vector &vecAbsOrigin, char const *pchStatisticName,
    float flIncrementAmount) {
  BaseClass::Event_IncrementCountedStatistic(vecAbsOrigin, pchStatisticName,
                                             flIncrementAmount);
...
static void CC_ListDeaths(const CCommand &args) {
  Ep2LevelStats_t *map =
      s_CEP2GameStats_Singleton.FindOrAddMapStats(STRING(gpGlobals->mapname));
  if (!map) return;
  ...
  for (int i = map->m_aPlayerDeaths.Count() - 1; i >= 0; --i, ++nRendered) {
    ...
    */
    ...
    if (nRendered > 150) break;
  }
  Msg("\nlisted %d deaths\n", map->m_aPlayerDeaths.Count());
}

static ConCommand listDeaths("listdeaths", CC_ListDeaths, "lists player deaths",
                             0);
