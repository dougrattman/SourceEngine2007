...
class CNPC_Puppet : public CAI_BaseNPC {
 ...
 private:
  ...
  CNetworkVar(
      EHANDLE,
      m_hAnimationTarget);  // NPC that will drive what animation we're playing
  CNetworkVar(
...
BEGIN_DATADESC(CNPC_Puppet)
  DEFINE_KEYFIELD(m_sAnimTargetname, FIELD_STRING, "animationtarget"),
      ...
      DEFINE_INPUTFUNC(FIELD_STRING, "SetAnimationTarget",
                       InputSetAnimationTarget),
END_DATADESC()
...
void CNPC_Puppet::Spawn(void) {
  ...
  if (pTarget) {
    ...
    if (pAnimating) {
      m_nTargetAttachment =
          pAnimating->LookupAttachment(STRING(m_sAnimAttachmentName));
    }
  }
