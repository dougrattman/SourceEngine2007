...
//-----------------------------------------------------------------------------
bool CAI_PassengerBehaviorZombie::CanEnterVehicle(void) { return false; }
...
Activity CAI_PassengerBehaviorZombie::NPC_TranslateActivity(Activity activity) {
  ...
  if (activity == ACT_IDLE) return (Activity)ACT_PASSENGER_IDLE;
...
bool CAI_PassengerBehaviorZombie::EnemyInVehicle(void) {
  ...
  if (GetOuter()->GetEnemy() == NULL) return false;

  // See if they're in a vehicle, currently
  CBaseCombatCharacter *pCCEnemy =
      GetOuter()->GetEnemy()->MyCombatCharacterPointer();
  if (pCCEnemy && pCCEnemy->IsInAVehicle()) return true;
...
int CAI_PassengerBehaviorZombie::SelectSchedule(void) {
  ...
  if (nState == PASSENGER_STATE_INSIDE) {
    ...
    if (nNewSchedule != SCHED_NONE) return nNewSchedule;
...
bool CAI_PassengerBehaviorZombie::CanJumpToAttachToVehicle(void) {
  ...
  float flDist =
      UTIL_DistApprox(vecPredictedPosition, GetOuter()->GetAbsOrigin());

  // If we're facing them enough, allow the jump
  if ((flDist < JUMP_ATTACH_DIST_THRESHOLD) &&
      UTIL_IsFacingWithinTolerance(GetOuter(), pEnemy,
                                   JUMP_ATTACH_FACING_THRESHOLD))
    return true;
...
inline bool CAI_PassengerBehaviorZombie::CanBeOnEnemyVehicle(void) {
  ...
  if (pEnemy != NULL) {
    ...
    if (pVehicle && pVehicle->NPC_HasAvailableSeat(GetRoleName())) return true;
  }
...
void CAI_PassengerBehaviorZombie::GatherConditions(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_OUTSIDE) {
    ...
    if (pPlayer != NULL && GetOuter()->GetEnemy() == pPlayer &&
      ...
      float flDistSqr =
          (pPlayer->GetAbsOrigin() - GetOuter()->GetAbsOrigin()).LengthSqr();
...
void CAI_PassengerBehaviorZombie::StartDismount(void) {
  ...
  // This removes the NPC from the vehicle's handling and fires all necessary
  // outputs
  m_hVehicle->RemovePhysicsChild(GetOuter());
  m_hVehicle->NPC_RemovePassenger(GetOuter());
...
void CAI_PassengerBehaviorZombie::GatherVehicleStateConditions(void) {
  ...
  if (GetPassengerState() != PASSENGER_STATE_INSIDE) return;

  // Calculate how our body is leaning
  CalculateBodyLean();

  // The forward delta of the vehicle
  float flLateralDelta = (m_vehicleState.m_vecDeltaVelocity.x +
                          m_vehicleState.m_vecDeltaVelocity.y);
  ...
  if (flLateralDelta < -350.0f) {
    if (m_hVehicle) {
      ...
      // NDebugOverlay::HorzArrow( GetAbsOrigin(), GetAbsOrigin() + (
      // vecDamageForce * 256.0f ), 16.0f, 255, 0, 0, 16, true, 2.0f );

      // Fake it!
      CTakeDamageInfo info(m_hVehicle, m_hVehicle, vecDamageForce,
                           GetOuter()->WorldSpaceCenter(), 200,
                           (DMG_CRUSH | DMG_VEHICLE));
...
bool CAI_PassengerBehaviorZombie::AttachToVehicle(void) {
  ...
  if (m_hVehicle->NPC_CanEnterVehicle(GetOuter(), false) == false) return false;

  // Reserve the seat
  if (ReserveEntryPoint(VEHICLE_SEAT_ANY) == false) return false;
  ...
  // Find Alyx
  // TODO: Iterate through the list of passengers in the vehicle and find one we
  // can interact with
  CNPC_Alyx *pAlyx = CNPC_Alyx::GetAlyx();
  if (pAlyx) {
    // Tell Alyx to play along!
    pAlyx->ForceVehicleInteraction(GetOuter()->GetSequenceName(nSequence),
                                   GetOuter());
  }
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_PassengerBehaviorZombie) {
  ...
  DEFINE_SCHEDULE(SCHED_PASSENGER_ZOMBIE_ENTER_VEHICLE,
  ...
  DEFINE_SCHEDULE(SCHED_PASSENGER_ZOMBIE_MELEE_ATTACK1,
  ...
  DEFINE_SCHEDULE(SCHED_PASSENGER_ZOMBIE_RANGE_ATTACK1,

                  "	Tasks"
                  "		TASK_PLAY_SEQUENCE			"
                  "	ACTIVITY:ACT_PASSENGER_RANGE_ATTACK1"
                  "		TASK_SET_ACTIVITY			"
                  "	ACTIVITY:ACT_PASSENGER_ZOMBIE_LEAP_LOOP"
  ...
  DEFINE_SCHEDULE(SCHED_PASSENGER_ZOMBIE_ATTACH,
