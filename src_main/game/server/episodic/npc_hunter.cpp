...
static const char *HUNTER_FLECHETTE_MODEL =
...
ConVar sk_hunter_flechette_explode_dmg("sk_hunter_flechette_explode_dmg",
                                       "12.0");
ConVar sk_hunter_flechette_explode_radius("sk_hunter_flechette_explode_radius",
                                          "128.0");
...
ConVar hunter_flechette_max_concurrent_volleys(
    "hunter_flechette_max_concurrent_volleys", "2");
ConVar hunter_flechette_volley_start_min_delay(
    "hunter_flechette_volley_start_min_delay", ".25");
ConVar hunter_flechette_volley_start_max_delay(
    "hunter_flechette_volley_start_max_delay", ".95");
ConVar hunter_flechette_volley_end_min_delay(
    "hunter_flechette_volley_end_min_delay", "1");
ConVar hunter_flechette_volley_end_max_delay(
...
ConVar sk_hunter_buckshot_damage_scale("sk_hunter_buckshot_damage_scale",
                                       "0.5");
ConVar sk_hunter_vehicle_damage_scale("sk_hunter_vehicle_damage_scale", "2.2");
ConVar sk_hunter_dmg_from_striderbuster("sk_hunter_dmg_from_striderbuster",
                                        "150");
...
ConVar hunter_show_weapon_los_condition("hunter_show_weapon_los_condition",
                                        "0");
...
ConVar hunter_jostle_car_min_speed(
    "hunter_jostle_car_min_speed",
    "100");  // If hit by a car going at least this fast, jostle the car
ConVar hunter_jostle_car_max_speed("hunter_jostle_car_max_speed",
                                   "600");  // Used for determining jostle scale
...
ConVar hunter_hate_thrown_striderbusters("hunter_hate_thrown_striderbusters",
                                         "1");
ConVar hunter_hate_attached_striderbusters(
    "hunter_hate_attached_striderbusters", "1");
ConVar hunter_hate_held_striderbusters_delay(
    "hunter_hate_held_striderbusters_delay", "0.5");
ConVar hunter_hate_held_striderbusters_tolerance(
    "hunter_hate_held_striderbusters_tolerance", "2000.0");
ConVar hunter_hate_thrown_striderbusters_tolerance(
    "hunter_hate_thrown_striderbusters_tolerance", "300.0");
ConVar hunter_seek_thrown_striderbusters_tolerance(
    "hunter_seek_thrown_striderbusters_tolerance", "400.0");
ConVar hunter_retreat_striderbusters(
    "hunter_retreat_striderbusters", "1", FCVAR_NONE,
...
ConVar hunter_stand_still(
    "hunter_stand_still",
    "0");  // used for debugging, keeps them rooted in place
...
#define HUNTER_IGNORE_ENEMY_TIME \
  5  // How long the hunter will ignore another enemy when distracted by the
     // player.

#define HUNTER_FACING_DOT 0.8  // The angle within which we start shooting
#define HUNTER_SHOOT_MAX_YAW_DEG \
  60.0f  // Once shooting, clamp to +/- these degrees of yaw deflection as our
         // target moves
...
#define HUNTER_FOLLOW_DISTANCE_SQR \
  (HUNTER_FOLLOW_DISTANCE * HUNTER_FOLLOW_DISTANCE)
...
bool HateThisStriderBuster(CBaseEntity *pTarget) {
  if (StriderBuster_WasKnockedOffStrider(pTarget)) return false;
  ...
  if (pTarget->VPhysicsGetObject()) {
    if (hunter_hate_held_striderbusters.GetBool() ||
      ...
      if ((pTarget->VPhysicsGetObject()->GetGameFlags() &
           (FVPHYSICS_PLAYER_HELD | FVPHYSICS_WAS_THROWN))) {
        return true;
      }
...
static const char *s_szHunterFlechetteDangerSoundThink =
    "HunterFlechetteDangerSoundThink";
...
class CHunterFlechette : public CPhysicsProp, public IParentPropInteraction {
 ...
 public:
  ...
  static CHunterFlechette *FlechetteCreate(const Vector &vecOrigin,
                                           const QAngle &angAngles,
                                           CBaseEntity *pentOwner = NULL);

  // IParentPropInteraction
  void OnParentCollisionInteraction(parentCollisionInteraction_t eType,
...
BEGIN_DATADESC(CHunterFlechette)
  ...
  DEFINE_THINKFUNC(BubbleThink), DEFINE_THINKFUNC(DangerSoundThink),
      DEFINE_THINKFUNC(ExplodeThink), DEFINE_THINKFUNC(DopplerThink),
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CHunterFlechette *CHunterFlechette::FlechetteCreate(const Vector &vecOrigin,
                                                    const QAngle &angAngles,
                                                    CBaseEntity *pentOwner) {
  // Create a new entity with CHunterFlechette private data
  CHunterFlechette *pFlechette =
      (CHunterFlechette *)CreateEntityByName("hunter_flechette");
...
void CC_Hunter_Shoot_Flechette(const CCommand &args) {
  ...
  CHunterFlechette *entity = CHunterFlechette::FlechetteCreate(
      pPlayer->EyePosition(), angEye, pPlayer);
  if (entity) {
    entity->Precache();
...
static ConCommand ent_create(
    "hunter_shoot_flechette", CC_Hunter_Shoot_Flechette,
    "Fires a hunter flechette where the player is looking.",
    FCVAR_GAMEDLL | FCVAR_CHEAT);

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CHunterFlechette::CHunterFlechette() { UseClientSideAnimation(); }
...
void CHunterFlechette::SetSeekTarget(CBaseEntity *pTargetEntity) {
  if (pTargetEntity) {
    ...
    SetContextThink(&CHunterFlechette::SeekThink, gpGlobals->curtime,
                    s_szHunterFlechetteSeekThink);
  }
}
...
//-----------------------------------------------------------------------------
void CHunterFlechette::OnParentCollisionInteraction(
    parentCollisionInteraction_t eType, int index,
...
void CHunterFlechette::OnParentPhysGunDrop(CBasePlayer *pPhysGunUser,
...
bool CHunterFlechette::CreateSprites(bool bBright) {
  if (bBright) {
    DispatchParticleEffect("hunter_flechette_trail_striderbuster",
                           PATTACH_ABSORIGIN_FOLLOW, this);
  } else {
    DispatchParticleEffect("hunter_flechette_trail", PATTACH_ABSORIGIN_FOLLOW,
                           this);
  }
...
void CHunterFlechette::StickTo(CBaseEntity *pOther, trace_t &tr) {
  ...
  if (!bAttachedToBuster) {
    ...
    SetNextThink(gpGlobals->curtime +
                 (hunter_flechette_explode_delay.GetFloat() -
                  HUNTER_FLECHETTE_WARN_TIME));
  } else {
    DangerSoundThink();
  }
...
void CHunterFlechette::FlechetteTouch(CBaseEntity *pOther) {
  if (pOther->IsSolidFlagSet(FSOLID_VOLUME_CONTENTS | FSOLID_TRIGGER)) {
    // Some NPCs are triggers that can take damage (like antlion grubs). We
    // should hit them.
    if ((pOther->m_takedamage == DAMAGE_NO) ||
        (pOther->m_takedamage == DAMAGE_EVENTS_ONLY))
      return;
  }

  if (FClassnameIs(pOther, "hunter_flechette")) return;
  ...
  if (pOther->m_takedamage != DAMAGE_NO) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + vForward * 128,
                   MASK_BLOCKLOS, pOther, COLLISION_GROUP_NONE, &tr2);

    if (tr2.fraction != 1.0f) {
      // NDebugOverlay::Box( tr2.endpos, Vector( -16, -16, -16 ), Vector( 16,
      // 16, 16 ), 0, 255, 0, 0, 10 ); NDebugOverlay::Box( GetAbsOrigin(),
      // Vector( -16, -16, -16 ), Vector( 16, 16, 16 ), 0, 0, 255, 0, 10 );
    ...
    if (((pOther->GetMoveType() == MOVETYPE_VPHYSICS) ||
         (pOther->GetMoveType() == MOVETYPE_PUSH)) &&
        ((pOther->GetHealth() > 0) ||
         (pOther->m_takedamage == DAMAGE_EVENTS_ONLY))) {
    ...
    if (pOther->GetMoveType() == MOVETYPE_NONE &&
      ...
      // We hit a physics object that survived the impact. Stick to it.
      StickTo(pOther, tr);
    } else if (pOther->GetMoveType() == MOVETYPE_PUSH &&
               FClassnameIs(pOther, "func_breakable")) {
      // We hit a func_breakable, stick to it.
      // The MOVETYPE_PUSH is a micro-optimization to cut down on the classname
      // checks.
      StickTo(pOther, tr);
    } else {
...
void CHunterFlechette::SeekThink() {
  if (m_hSeekTarget) {
    ...
    Vector vecClosest;
    CalcClosestPointOnLineSegment(GetAbsOrigin(), m_vecShootPosition,
                                  vecBodyTarget, vecClosest, NULL);
...
void CHunterFlechette::Shoot(Vector &vecVelocity, bool bBrightFX) {
  ...
  SetContextThink(&CHunterFlechette::BubbleThink, gpGlobals->curtime + 0.1,
                  s_szHunterFlechetteBubbles);
}
...
void CHunterFlechette::DangerSoundThink() {
  ...
  CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_EXCLUDE_COMBINE,
                         GetAbsOrigin(), 150.0f, 0.5, this);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CHunterFlechette::ExplodeThink() { Explode(); }
...
void CHunterFlechette::Explode() {
  ...
  DispatchParticleEffect("hunter_projectile_explosion_1", vecFuse,
                         GetAbsAngles(), NULL);

  int nDamageType = DMG_DISSOLVE;

  // Perf optimization - only every other explosion makes a physics force. This
  // is hardly noticeable since flechettes usually explode in clumps.
  ...
  RadiusDamage(
      CTakeDamageInfo(this, GetOwnerEntity(),
                      sk_hunter_flechette_explode_dmg.GetFloat(), nDamageType),
      GetAbsOrigin(), sk_hunter_flechette_explode_radius.GetFloat(), CLASS_NONE,
      NULL);
...
//-----------------------------------------------------------------------------
void Hunter_ApplyChargeDamage(CBaseEntity *pHunter, CBaseEntity *pTarget,
                              float flDamage) {
  Vector attackDir =
      (pTarget->WorldSpaceCenter() - pHunter->WorldSpaceCenter());
...
class CHunterTraceFilterSkipPhysics : public CTraceFilter {
 public:
  ...
  CHunterTraceFilterSkipPhysics(const IHandleEntity *passentity,
                                int collisionGroup, float minMass)
      : m_pPassEnt(passentity),
        m_collisionGroup(collisionGroup),
        m_minMass(minMass) {}
  virtual bool ShouldHitEntity(IHandleEntity *pHandleEntity, int contentsMask) {
    if (!StandardFilterRules(pHandleEntity, contentsMask)) return false;
    ...
    if (pEntity) {
      if (!pEntity->ShouldCollide(m_collisionGroup, contentsMask)) return false;

      if (!g_pGameRules->ShouldCollide(m_collisionGroup,
                                       pEntity->GetCollisionGroup()))
        return false;
      ...
      if (!pEntity->IsNPC() && pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
        ...
        if (entMass < m_minMass) {
          if (entMass < m_minMass * 0.666f ||
              pEntity->CollisionProp()->BoundingRadius() <
                  (assert_cast<const CAI_BaseNPC *>(
                       EntityFromEntityHandle(m_pPassEnt)))
                      ->GetHullHeight()) {
            return false;
          }
        }
      ...
      if (pEntity->Classify() == CLASS_ANTLION) {
        CBaseEntity *pHunter =
            (CBaseEntity *)EntityFromEntityHandle(m_pPassEnt);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
inline void HunterTraceHull_SkipPhysics(
    const Vector &vecAbsStart, const Vector &vecAbsEnd, const Vector &hullMin,
 ...
 public:
  ...
  bool FarFromFollowTarget() {
    return (GetFollowTarget() &&
            (GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin()).LengthSqr() >
                HUNTER_FOLLOW_DISTANCE_SQR);
  }
  ...
  static float gm_flLastDefendSound;  // not saved and loaded, it's okay to yell
                                      // again after a load
...
impactdamagetable_t s_HunterImpactDamageTable = {
    ...
    HUNTER_MIN_PHYSICS_DAMAGE,  // never take more than 10 pts of damage from
                                // anything under 10kg
    36 * 36,  // <10kg objects must go faster than 36 in/s to do damage

    VPHYSICS_LARGE_OBJECT_MASS,  // large mass in kg
    4,  // large mass scale (anything over 500kg does 4X as much energy to read
        // from damage table)
    5,  // large mass falling scale (emphasize falling/crushing damage over
        // sideways impacts since the stress will kill you anyway)
    0.0f,  // min vel
};
...
class CNPC_Hunter : public CAI_BaseActor {
 ...
 public:
  ...
  bool HandleInteraction(int interactionType, void *data,
                         CBaseCombatCharacter *pSourceEnt);
  ...
  int MeleeAttack1ConditionsVsEnemyInVehicle(CBaseCombatCharacter *pEnemy,
                                             float flDot);
  ...
  CBaseEntity *MeleeAttack(float flDist, int iDamage, QAngle &qaViewPunch,
  ...
  void DelayRangedAttackers(float minDelay, float maxDelay,
                            bool bForced = false);
 ...
 private:
  ...
  // Charge attack.
  bool ShouldCharge(const Vector &startPos, const Vector &endPos, bool useTime,
                    bool bCheckForCancel);
  ...
  void GetShootDir(Vector &vecDir, const Vector &vecSrc,
                   CBaseEntity *pTargetEntity, bool bStriderbuster,
                   int nShotNum, bool bSingleShot);
  ...
  float m_flNextChargeTime;  // Prevents us from doing our threat display too
                             // often.
  ...
  CSimpleSimTimer m_EyeSwitchTimer;  // Controls how often we switch which eye
                                     // is focusing on our enemy.

  bool m_bTopMuzzle;  // Used to alternate between top muzzle FX and bottom
                      // muzzle FX.
  ...
  bool m_bDisableShooting;  // Range attack disabled via an input. Used for
                            // scripting melee attacks.

  bool
      m_bFlashlightInEyes;  // The player is shining the flashlight on our eyes.
  float m_flPupilDilateTime;  // When to dilate our pupils if the flashlight is
                              // no longer on our eyes.
  ...
  int m_nClampedShots;  // The number of consecutive shots fired at an
                        // out-of-max yaw target.

  float m_flNextRangeAttack2Time;  // Time when we can fire another volley of
                                   // flechettes.
  float
      m_flNextFlechetteTime;  // Time to fire the next flechette in this volley.
  ...
  float m_fCorneredTimer;  ///< hunter was cornered when fleeing player; it
                           ///< won't flee again until this time
...
BEGIN_DATADESC(CNPC_Hunter)
  ...
  DEFINE_KEYFIELD(m_iszFollowTarget, FIELD_STRING, "FollowTarget"),
      ...
      DEFINE_EMBEDDED(m_HeavyDamageDelay), DEFINE_EMBEDDED(m_FlinchTimer),
      ...
      DEFINE_EMBEDDED(m_RundownDelay), DEFINE_EMBEDDED(m_IgnoreVehicleTimer),
      ...
      DEFINE_EMBEDDED(m_BeginFollowDelay), DEFINE_EMBEDDED(m_EyeSwitchTimer),
      ...
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableSquadShootDelay",
                       InputEnableSquadShootDelay),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableSquadShootDelay",
                       InputDisableSquadShootDelay),
      DEFINE_INPUTFUNC(FIELD_VOID, "EnableUnplantedShooting",
                       InputEnableUnplantedShooting),
      DEFINE_INPUTFUNC(FIELD_VOID, "DisableUnplantedShooting",
                       InputDisableUnplantedShooting),

      // Function Pointers
      DEFINE_THINKFUNC(TeslaThink), DEFINE_THINKFUNC(BleedThink),
...
void CNPC_Hunter::Spawn() {
  ...
  CapabilitiesAdd(bits_CAP_MOVE_GROUND | bits_CAP_SQUAD |
  ...
  // CSoundEnvelopeController::GetController(); 	CPASAttenuationFilter
  // filter( this );
  //
  //	m_pGunFiringSound = controller.SoundCreate( filter, entindex(),
  //"NPC_Hunter.FlechetteShootLoop" ); 	controller.Play( m_pGunFiringSound, 0.0,
  // 100 );
  //}
}
...
void CNPC_Hunter::SetupGlobalModelData() {
  if (gm_nBodyYawPoseParam != -1) return;
...
void CNPC_Hunter::StopLoopingSounds() {
  ...
  //{
  //	CSoundEnvelopeController &controller =
  // CSoundEnvelopeController::GetController(); 	controller.SoundDestroy(
  // m_pGunFiringSound ); 	m_pGunFiringSound = NULL;
  //}
}
...
bool CNPC_Hunter::ShouldPlayIdleSound() {
  if (random->RandomInt(0, 99) == 0 && !HasSpawnFlags(SF_NPC_GAG)) return true;
...
//-----------------------------------------------------------------------------
bool CNPC_Hunter::OverrideMoveFacing(const AILocalMoveGoal_t &move,
                                     float flInterval) {
  ...
  if (GetEnemy() &&
      (bSideStepping ||
       (((GetNavigator()->GetMovementActivity() == ACT_RUN) ||
         (GetNavigator()->GetMovementActivity() == ACT_WALK)) &&
        !IsCurSchedule(SCHED_HUNTER_TAKE_COVER_FROM_ENEMY, false)))) {
    Vector vecEnemyLKP = GetEnemyLKP();

    // Face my enemy if we're close enough
    if (bSideStepping ||
        UTIL_DistApprox(vecEnemyLKP, GetAbsOrigin()) < HUNTER_FACE_ENEMY_DIST) {
...
void CNPC_Hunter::Activate() {
  ...
  // If no one has initialized the hunters to run over counter, just zero it
  // out.
  ...
  CMissile::AddCustomDetonator(
      this,
      (GetHullMaxs().AsVector2D() - GetHullMins().AsVector2D()).Length() * 0.5,
      GetHullHeight());
  ...
  if (gm_flMinigunDistZ == 0) {
    ...
    VectorITransform(position, pHunter->EntityToWorldTransform(),
                     gm_vecLocalRelativePositionMinigun);
    UTIL_Remove(pHunter);
  }
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
Class_T CNPC_Hunter::Classify() { return CLASS_COMBINE_HUNTER; }

//-----------------------------------------------------------------------------
// Compensate for the hunter's long legs by moving the bodytarget up to his
// head.
//-----------------------------------------------------------------------------
Vector CNPC_Hunter::BodyTarget(const Vector &posSrc, bool bNoisy /*= true*/) {
  ...
  if (bNoisy) {
    float rand1 = random->RandomFloat(0, gm_flHeadRadius) +
                  random->RandomFloat(0, gm_flHeadRadius);
...
int CNPC_Hunter::DrawDebugTextOverlays() {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    EntityText(text_offset, CFmtStr("%s", m_bPlanted ? "Planted" : "Unplanted"),
               0);
    text_offset++;
...
void CNPC_Hunter::UpdateEyes() {
  ...
  if (m_EyeSwitchTimer.Expired()) {
    ...
    if ((m_eEyeState == HUNTER_EYE_STATE_BOTTOM_LOCKED) ||
        (m_eEyeState == HUNTER_EYE_STATE_BOTH_UNLOCKED)) {
      SetExpression("scenes/npc/hunter/hunter_eyedarts_top.vcd");
    }

    if ((m_eEyeState == HUNTER_EYE_STATE_TOP_LOCKED) ||
        (m_eEyeState == HUNTER_EYE_STATE_BOTH_UNLOCKED)) {
...
void CNPC_Hunter::PrescheduleThink() {
  ...
  if (m_bFlashlightInEyes) {
    if (m_flPupilDilateTime < gpGlobals->curtime) {
      ...
      if (pPlayer && !pPlayer->IsIlluminatedByFlashlight(this, NULL) ||
          !PlayerFlashlightOnMyEyes(pPlayer)) {
        // Msg( "NOT SHINING FLASHLIGHT ON ME\n" );

        // Remove the actor from the flashlight scene
        RemoveActorFromScriptedScenes(this, true, false,
...
void CNPC_Hunter::GatherChargeConditions() {
  ...
  if (!hunter_charge.GetBool()) return;

  if (!GetEnemy()) return;

  if (GetHintGroup() != NULL_STRING) return;

  if (!HasCondition(COND_SEE_ENEMY)) return;

  if (!hunter_charge_test.GetBool() && gpGlobals->curtime < m_flNextChargeTime)
    return;

  // No charging Alyx or Barney
  if (GetEnemy()->Classify() == CLASS_PLAYER_ALLY_VITAL) return;

  if (m_EscortBehavior.GetEscortTarget() &&
      GetEnemy()->MyCombatCharacterPointer() &&
      !GetEnemy()->MyCombatCharacterPointer()->FInViewCone(this))
    return;
...
void CNPC_Hunter::GatherConditions() {
  ...
  if (m_IgnoreVehicleTimer.Expired() && GetEnemy() &&
    ...
    if ((pVehicle) && (GlobalEntity_GetCounter(s_iszHuntersToRunOver) <= 0)) {
      ...
      // Extrapolate the position of the vehicle and see if it's heading toward
      // us.
      float predictTime = hunter_dodge_warning.GetFloat();
      Vector2D vecFuturePos =
          pVehicle->GetAbsOrigin().AsVector2D() +
          pVehicle->GetSmoothedVelocity().AsVector2D() * predictTime;
      if (pVehicle->GetSmoothedVelocity().LengthSqr() > Square(200)) {
        ...
        if (hunter_dodge_debug.GetBool()) {
          NDebugOverlay::Line(
              ...
              pVehicle->GetAbsOrigin() + pVehicle->GetSmoothedVelocity(), 255,
              255, 255, true, .1);
        }
        vDirMovement.NormalizeInPlace();
        Vector2D vDirToHunter =
            GetAbsOrigin().AsVector2D() - pVehicle->GetAbsOrigin().AsVector2D();
        vDirToHunter.NormalizeInPlace();
        if (DotProduct2D(vDirMovement, vDirToHunter) >
                hunter_dodge_warning_cone.GetFloat() &&
            CalcDistanceSqrToLine2D(GetAbsOrigin().AsVector2D(),
                                    pVehicle->GetAbsOrigin().AsVector2D(),
                                    vecFuturePos, &t) <
                Square(hunter_dodge_warning_width.GetFloat() * .5) &&
            t > 0.0 && t < 1.0) {
          if (fabs(predictTime - hunter_dodge_warning.GetFloat()) < .05 ||
          ...
          if (hunter_dodge_debug.GetBool()) {
            ...
            if (timeDrawnArrow != gpGlobals->curtime) {
              ...
              Vector vEndpoint(
                  vecFuturePos.x, vecFuturePos.y,
                  UTIL_GetLocalPlayer()->WorldSpaceCenter().z - 24);
              NDebugOverlay::HorzArrow(
                  UTIL_GetLocalPlayer()->WorldSpaceCenter() - Vector(0, 0, 24),
                  vEndpoint, hunter_dodge_warning_width.GetFloat(), 255, 0, 0,
                  64, true, .1);
            }
          }
      ...
      if (hunter_dodge_debug.GetBool()) {
        if (timeDrawnArrow != gpGlobals->curtime) {
          timeDrawnArrow = gpGlobals->curtime;
          Vector vEndpoint(vecFuturePos.x, vecFuturePos.y,
                           UTIL_GetLocalPlayer()->WorldSpaceCenter().z - 24);
          NDebugOverlay::HorzArrow(
              UTIL_GetLocalPlayer()->WorldSpaceCenter() - Vector(0, 0, 24),
              vEndpoint, hunter_dodge_warning_width.GetFloat(), 127, 127, 127,
              64, true, .1);
        }
      }
    }
...
void CNPC_Hunter::ManageSiegeTargets() {
  if (gpGlobals->curtime < m_flTimeNextSiegeTargetAttack) return;
  ...
  if (m_pSiegeTargets.Count() == 0) {
    ...
    if (m_pSiegeTargets.Count() == 0) return;
  }

  m_flTimeNextSiegeTargetAttack =
      gpGlobals->curtime +
      (hunter_siege_frequency.GetFloat() * RandomFloat(0.8f, 1.2f));
  CBasePlayer *pPlayer = AI_GetSinglePlayer();
  ...
  if (bCreateSiegeTarget) {
    // We've decided that the situation calls for a siege target. So, we dig
    // through all of my siege targets and take the closest one to the player
    // that the player can see! (Obey they bullseye's FOV)
    float flClosestDistSqr =
        Square(1200.0f);  // Only use siege targets within 100 feet of player
    ...
    for (int i = 0; i < m_pSiegeTargets.Count(); i++) {
      ...
      float flDistSqr =
          pCandidate->GetAbsOrigin().DistToSqr(pPlayer->GetAbsOrigin());

      if (flDistSqr < flClosestDistSqr) {
        // CollectSiegeTargets() guarantees my list is populated only with
        // bullseye entities.
        CNPC_Bullseye *pBullseye = dynamic_cast<CNPC_Bullseye *>(pCandidate);
        if (!pBullseye->FInViewCone(this)) continue;
    ...
    if (pSiegeTargetLocation != NULL) {
      ...
      // Create a bullseye that will live for 20 seconds. If we can't attack it
      // within 20 seconds, it's probably out of reach anyone, so have it clean
      // itself up after that long.
      CBaseEntity *pSiegeTarget =
          CreateCustomTarget(pSiegeTargetLocation->GetAbsOrigin(), 20.0f);
      ...
      GetEnemies()->UpdateMemory(GetNavigator()->GetNetwork(), pSiegeTarget,
                                 pSiegeTarget->GetAbsOrigin(), 0.0f, true);
      AI_EnemyInfo_t *pMemory = GetEnemies()->Find(pSiegeTarget);

      if (pMemory) {
        // Pretend we've known about this target longer than we really have so
        // that our AI doesn't waste time running ALERT schedules.
...
bool CNPC_Hunter::QueryHearSound(CSound *pSound) {
  ...
  if (pSound->SoundContext() & SOUND_CONTEXT_PLAYER_VEHICLE) return false;
...
//					Use volumes in the levels? (yucky for
// designers)
//-----------------------------------------------------------------------------
...
void CNPC_Hunter::GatherIndoorOutdoorConditions() {
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, 40.0f * 12.0f),
...
void CNPC_Hunter::BuildScheduleTestBits() {
  ...
  if (IsCurSchedule(SCHED_HUNTER_RANGE_ATTACK2, false) &&
      (gpGlobals->curtime < m_flShootAllowInterruptTime)) {
    ClearCustomInterruptConditions();
    SetCustomInterruptCondition(COND_HEAVY_DAMAGE);
  } else if (IsCurSchedule(SCHED_HUNTER_RANGE_ATTACK2, false) &&
  ...
  } else if (!IsInLargeOutdoorMap() &&
             IsCurSchedule(SCHED_HUNTER_FLANK_ENEMY, false) &&
             GetEnemy() != NULL) {
    if (HasCondition(COND_CAN_RANGE_ATTACK2) &&
        m_flTimeSawEnemyAgain != HUNTER_SEE_ENEMY_TIME_INVALID) {
      if ((gpGlobals->curtime - m_flTimeSawEnemyAgain) >= 2.0f) {
        // When we're running flank behavior, wait a moment AFTER being able to
        // see the enemy before breaking my schedule to range attack. This helps
        // assure that the hunter gets well inside the room before stopping to
        // attack. Otherwise the Hunter may stop immediately in the doorway and
        // stop the progress of any hunters behind it.
        SetCustomInterruptCondition(COND_CAN_RANGE_ATTACK2);
      }
    }
  }

  // If our enemy is anything but a striderbuster, drop everything if we see
  // one.
...
static bool IsMovablePhysicsObject(CBaseEntity *pEntity) {
  return pEntity && pEntity->GetMoveType() == MOVETYPE_VPHYSICS &&
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::ShouldCharge(const Vector &startPos, const Vector &endPos,
                               bool useTime, bool bCheckForCancel) {
  // Must have a target
  ...
  if (!bCheckForCancel && !hunter_charge_test.GetBool()) {
    ...
    if ((distance < Square(HUNTER_CHARGE_MIN)) ||
        (distance > Square(HUNTER_CHARGE_MAX)))
      return false;
  }
  ...
  GetMoveProbe()->MoveLimit(NAV_GROUND, startPos, vecTargetPos,
                            MASK_NPCSOLID_BRUSHONLY, GetEnemy(), &moveTrace);
  ...
  if (g_debug_hunter_charge.GetInt() == 1) {
    ...
    NDebugOverlay::BoxDirection(startPos, GetHullMins(),
                                GetHullMaxs() + Vector(enemyDist, 0, 0),
                                enemyDir, 0, 255, 0, 8, 1.0f);
  }

  // If we're not blocked, charge
  if (IsMoveBlocked(moveTrace)) {
    // Don't allow it if it's too close to us
    if (UTIL_DistApprox(WorldSpaceCenter(), moveTrace.vEndPosition) <
        HUNTER_CHARGE_MIN)
      return false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::HandleInteraction(int interactionType, void *data,
                                    CBaseCombatCharacter *pSourceEnt) {
  if ((pSourceEnt != this) &&
      (interactionType == g_interactionHunterFoundEnemy)) {
...
int CNPC_Hunter::SelectCombatSchedule() {
  ...
  if (bStriderBuster) {
    if (gpGlobals->curtime - CAI_HunterEscortBehavior::gm_flLastDefendSound >
        10.0) {
  ...
  // Certain behaviors, like flanking and melee attacks, only make sense on
  // visible, corporeal enemies (NOT bullseyes).
  ...
  if (bIsCorporealEnemy && HasCondition(COND_NEW_ENEMY)) {
    ...
    if (GetSquad() && pEnemyInfo &&
        (pEnemyInfo->timeFirstSeen == pEnemyInfo->timeAtFirstHand)) {
      GetSquad()->BroadcastInteraction(g_interactionHunterFoundEnemy, NULL,
                                       this);
  ...
  // Fire a flechette volley. Ignore squad slots if we're attacking a
  // striderbuster. See if there is an opportunity to charge.
  if (!bStriderBuster && bIsCorporealEnemy &&
      HasCondition(COND_HUNTER_CAN_CHARGE_ENEMY)) {
    if (hunter_charge_test.GetBool() ||
        random->RandomInt(1, 100) < hunter_charge_pct.GetInt()) {
      if (hunter_charge_test.GetBool() ||
          OccupyStrategySlot(SQUAD_SLOT_HUNTER_CHARGE)) {
  ...
  if (HasCondition(COND_CAN_RANGE_ATTACK2)) {
    if (bStriderBuster ||
        CountRangedAttackers() <
            hunter_flechette_max_concurrent_volleys.GetInt()) {
      DelayRangedAttackers(hunter_flechette_volley_start_min_delay.GetFloat(),
                           hunter_flechette_volley_start_max_delay.GetFloat(),
                           true);
      return SCHED_HUNTER_RANGE_ATTACK2;
    }
  }
  ...
  /*
     ...
     UTIL_DistApprox( GetEnemy()->GetAbsOrigin(), GetAbsOrigin() ) <
     HUNTER_FACE_ENEMY_DIST ) )
                  {
                          m_flNextSideStepTime = gpGlobals->curtime +
     random->RandomFloat( 1.0f, 3.0f ); return SCHED_HUNTER_SIDESTEP;
                  }
          }
  */
  if (HasCondition(COND_HEAVY_DAMAGE) &&
      (gpGlobals->curtime > m_flNextSideStepTime)) {
  ...
  if (!bStriderBuster && bIsCorporealEnemy) {
    ...
    // Try to be a flanker.
    if ((NumHuntersInMySquad() > 1) &&
        OccupyStrategySlotRange(SQUAD_SLOT_HUNTER_FLANK_FIRST,
                                SQUAD_SLOT_HUNTER_FLANK_LAST)) {
      return SCHED_HUNTER_FLANK_ENEMY;
    }
  }

  // Can't see my enemy.
  if (HasCondition(COND_ENEMY_OCCLUDED) || HasCondition(COND_ENEMY_TOO_FAR) ||
  ...
  // if ( HasCondition( COND_ENEMY_OCCLUDED ) && IsCurSchedule(
  // SCHED_RANGE_ATTACK1, false ) )
  //{
...
int CNPC_Hunter::SelectSiegeSchedule() {
  ...
  if (bHasEnemy) {
    ...
    if (HasCondition(COND_CAN_RANGE_ATTACK2)) return SCHED_HUNTER_RANGE_ATTACK2;

    return SCHED_HUNTER_SIEGE_STAND;
  } else {
    // Otherwise we are loitering in siege mode. Break line of sight with the
    // player if they expose our position.
...
int CNPC_Hunter::SelectSchedule() {
  ...
  if (HasCondition(COND_HUNTER_FORCED_DODGE)) return SCHED_HUNTER_DODGE;

  if (HasCondition(COND_HUNTER_NEW_HINTGROUP) ||
      (GetHintGroup() != NULL_STRING && m_CheckHintGroupTimer.Expired())) {
    CAI_Hint *pHint;
    ...
    if (HasCondition(COND_HUNTER_NEW_HINTGROUP)) {
      ...
      if (GetEnemy()) {
        pHint = CAI_HintManager::FindHint(NULL, GetEnemy()->GetAbsOrigin(),
                                          criteria);
      } else {
      ...
      pHint = CAI_HintManager::FindHint(GetAbsOrigin(), criteria);
      if (pHint) {
        if ((pHint->GetAbsOrigin() - GetAbsOrigin()).Length2DSqr() <
            Square(20 * 12)) {
  ...
  if (HasCondition(COND_HUNTER_INCOMING_VEHICLE)) {
    if (m_RundownDelay.Expired()) {
      ...
      if (iRundownCounter % 2 == 0) {
        ...
        if (GetSquad()) {
          GetSquad()->SetSquadData(HUNTER_RUNDOWN_SQUADDATA,
                                   iRundownCounter + 1);
        }
      }
    }
    ...
    if (pBestSound &&
        (pBestSound->SoundContext() & SOUND_CONTEXT_PLAYER_VEHICLE)) {
  ...
  // Now that we're past all of the forced reactions to things, if we're running
  // the siege behavior, go pick an appropriate siege schedule UNLESS we have an
  // enemy. If we have an enemy, we should focus on attacking that enemy.
  if (IsUsingSiegeTargets()) {
    return SelectSiegeSchedule();
  }

  // back away if there's a magnade glued to my head.
  if (hunter_retreat_striderbusters
          .GetBool() /*&& GetEnemy() && ( GetEnemy()->IsPlayer() )*/
  ...
  if (!BehaviorSelectSchedule()) {
    switch (GetState()) {
      ...
      case NPC_STATE_ALERT: {
        if (HasCondition(COND_HUNTER_SHOULD_PATROL)) return SCHED_HUNTER_PATROL;
...
int CNPC_Hunter::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    ...
    case SCHED_ALERT_STAND: {
      if (!hunter_disable_patrol.GetBool()) return SCHED_HUNTER_PATROL_RUN;
      break;
    }
...
void CNPC_Hunter::TaskFail(AI_TaskFailureCode_t code) {
  if (IsCurSchedule(SCHED_HUNTER_TAKE_COVER_FROM_ENEMY, false) &&
      (code == FAIL_NO_ROUTE_BLOCKED)) {
    // cornered!
...
//-----------------------------------------------------------------------------
// The player is speeding toward us in a vehicle! Find a good activity for
// dodging.
//-----------------------------------------------------------------------------
void CNPC_Hunter::TaskFindDodgeActivity() {
  ...
  if (pVehicle) {
    ...
    enemyRay.Init(pVehicle->GetAbsOrigin(),
                  pVehicle->GetAbsOrigin() + pVehicle->GetSmoothedVelocity());
    ...
    perpendicularRay.m_Start.z = perpendicularRay.m_Delta.z =
        perpendicularRay.m_StartOffset.z;
  ...
  while (!bFoundDir && (nTries < 2)) {
    ...
    trace_t tr;
    HunterTraceHull_SkipPhysics(GetAbsOrigin(), GetAbsOrigin() + vecDelta,
                                testHullMins, GetHullMaxs(), MASK_NPCSOLID,
                                this, GetCollisionGroup(), &tr,
                                VPhysicsGetObject()->GetMass() * 0.5f);

    // TODO: dodge anyway if we'll make it a certain percentage of the way
    // through the dodge?
    if (tr.fraction == 1.0f) {
      // NDebugOverlay::SweptBox( GetAbsOrigin(), GetAbsOrigin() + vecDelta,
      // testHullMins, GetHullMaxs(), QAngle( 0, 0, 0 ), 0, 255, 0, 128, 5 );
      bFoundDir = true;
      TaskComplete();
    } else {
      // NDebugOverlay::SweptBox( GetAbsOrigin(), GetAbsOrigin() + vecDelta,
      // testHullMins, GetHullMaxs(), QAngle( 0, 0, 0 ), 255, 0, 0, 128, 5 );
      nTries++;
  ...
  switch (pTask->iTask) {
    case TASK_HUNTER_FINISH_RANGE_ATTACK: {
      if (GetEnemy() != NULL &&
        ...
        // Just finished shooting at Alyx! So forget her for a little while and
        // get back on the player
        // !!!LATER - make sure there's someone else in enemy memory to go
        // bother.
    ...
    case TASK_HUNTER_SHOOT_COMMIT: {
      // We're committing to shooting. Don't allow interrupts until after we've
      // shot a bit (see TASK_RANGE_ATTACK1).
    ...
    case TASK_RANGE_ATTACK2: {
      if (GetEnemy()) {
        ...
        if (bIsBuster) {
          AddFacingTarget(GetEnemy(),
                          GetEnemy()->GetAbsOrigin() +
                              GetEnemy()->GetSmoothedVelocity() * .5,
                          1.0, 0.8);
        }
        ...
        // controller.SoundChangeVolume( m_pGunFiringSound, 1.0,
        // hunter_first_flechette_delay.GetFloat() );
        ...
        LockBothEyes(initialDelay +
                     (nShots * hunter_flechette_delay.GetFloat()));
      } else {
    ...
    case TASK_HUNTER_STAGGER: {
      ...
      Vector vecLocalStaggerDir =
          worldToLocalRotation.InverseTR().ApplyRotation(m_vecStaggerDir);
    ...
    case TASK_HUNTER_FIND_SIDESTEP_POSITION: {
      if (GetEnemy() == NULL) {
        ...
        Vector vecPos = GetAbsOrigin() + Vector(0, 0, 64) +
                        random->RandomFloat(120, 200) * vecDir;

        // Try to find the ground at the sidestep position.
        trace_t tr;
        UTIL_TraceLine(vecPos, vecPos + Vector(0, 0, -128), MASK_NPCSOLID, NULL,
...
void CNPC_Hunter::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_RANGE_ATTACK2: {
      if (!hunter_hate_thrown_striderbusters.GetBool() && GetEnemy() != NULL &&
          IsStriderBuster(GetEnemy())) {
      ...
      bool bIsBuster = IsStriderBuster(GetEnemy());
      if (bIsBuster) {
        Vector vFuturePosition =
            GetEnemy()->GetAbsOrigin() + GetEnemy()->GetSmoothedVelocity() * .3;
        AddFacingTarget(GetEnemy(), vFuturePosition, 1.0, 0.8);

        Vector2D vToFuturePositon =
            (vFuturePosition.AsVector2D() - GetAbsOrigin().AsVector2D());
      ...
      if (gpGlobals->curtime >= m_flNextFlechetteTime) {
        ...
        if (GetEnemy() != NULL && m_nFlechettesQueued > 0) {
          ...
          // If we fired three or more clamped shots in a row, call it quits so
          // we don't look dumb.
          if ((m_nClampedShots >= 3) || (m_nFlechettesQueued == 0)) {
            ...
            m_flNextFlechetteTime =
                gpGlobals->curtime + hunter_flechette_delay.GetFloat();
          }
        } else {
          bDone = true;
        }
        ...
        if (bDone) {
          ...
          DelayRangedAttackers(hunter_flechette_volley_end_min_delay.GetFloat(),
                               hunter_flechette_volley_end_max_delay.GetFloat(),
                               true);
          TaskComplete();
        }
      }
    ...
    case TASK_HUNTER_CHARGE: {
      ...
      if (eActivity == ACT_HUNTER_CHARGE_STOP ||
        ...
        if (IsActivityFinished()) {
          m_flNextChargeTime =
              gpGlobals->curtime + hunter_charge_min_delay.GetFloat() +
              random->RandomFloat(0, 2.5) + random->RandomFloat(0, 2.5);
          float delayMultiplier =
              (g_pGameRules->IsSkillLevel(SKILL_EASY)) ? 1.5 : 1.0;
          float groupDelay =
              gpGlobals->curtime +
              (2.0 + random->RandomFloat(0, 2)) * delayMultiplier;
          for (int i = 0; i < g_Hunters.Count(); i++) {
            if (g_Hunters[i] != this &&
      ...
      if (eActivity == ACT_HUNTER_CHARGE_RUN ||
          eActivity == ACT_HUNTER_CHARGE_START) {
        if (HasCondition(COND_NEW_ENEMY) || HasCondition(COND_LOST_ENEMY) ||
      ...
      if (AutoMovement(GetEnemy(), &moveTrace) == false) {
        ...
        if (HandleChargeImpact(moveTrace.vEndPosition,
          ...
          if (eActivity != ACT_HUNTER_CHARGE_STOP) {
            if (moveTrace.fStatus == AIMR_BLOCKED_WORLD &&
                moveTrace.vHitNormal == vec3_origin) {
              SetIdealActivity(ACT_HUNTER_CHARGE_STOP);
            } else {
              // Shake the screen
              if (moveTrace.fStatus != AIMR_BLOCKED_NPC) {
                EmitSound("NPC_Hunter.ChargeHitWorld");
                UTIL_ScreenShake(GetAbsOrigin(), 16.0f, 4.0f, 1.0f, 400.0f,
                                 SHAKE_START);
              }
          ...
          // We never have hunters and antlions together, but you never know.
            ...
            if (FClassnameIs(moveTrace.pObstruction, "npc_antlionguard")) {
              ...
              Hunter_ApplyChargeDamage(this, moveTrace.pObstruction,
                                       moveTrace.pObstruction->GetHealth());
            }
          }
        }
      }
...
bool CNPC_Hunter::EnemyIsRightInFrontOfMe(CBaseEntity **pEntity) {
  ...
  if ((GetEnemy()->WorldSpaceCenter() - WorldSpaceCenter()).LengthSqr() <
      (156 * 156)) {
    ...
    if (DotProduct(vecLOS, vBodyDir) > 0.8) {
      ...
      trace_t tr;
      UTIL_TraceHull(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(),
                     GetHullMins() * 0.5, GetHullMaxs() * 0.5, MASK_SOLID, this,
                     COLLISION_GROUP_NONE, &tr);
...
float CNPC_Hunter::ChargeSteer() {
  ...
  // Probe
  HunterTraceHull_SkipPhysics(
      GetAbsOrigin(), testPos, testHullMins, GetHullMaxs(), MASK_NPCSOLID, this,
      COLLISION_GROUP_NONE, &tr, VPhysicsGetObject()->GetMass() * 0.5f);

  // Debug info
  if (g_debug_hunter_charge.GetInt() == 1) {
    if (tr.fraction == 1.0f) {
      NDebugOverlay::BoxDirection(GetAbsOrigin(), testHullMins,
                                  GetHullMaxs() + Vector(testLength, 0, 0),
                                  forward, 0, 255, 0, 8, 0.1f);
    } else {
      NDebugOverlay::BoxDirection(GetAbsOrigin(), testHullMins,
                                  GetHullMaxs() + Vector(testLength, 0, 0),
                                  forward, 255, 0, 0, 8, 0.1f);
    }
  }
  ...
  // Debug
  if (g_debug_hunter_charge.GetInt() == 1) {
    NDebugOverlay::Line(GetAbsOrigin(), GetAbsOrigin() + (steer * 512.0f), 255,
                        255, 0, true, 0.1f);
    NDebugOverlay::Cross3D(GetAbsOrigin() + (steer * 512.0f), Vector(2, 2, 2),
                           -Vector(2, 2, 2), 255, 255, 0, true, 0.1f);

    NDebugOverlay::Line(GetAbsOrigin(),
                        GetAbsOrigin() + (BodyDirection3D() * 256.0f), 255, 0,
                        255, true, 0.1f);
    NDebugOverlay::Cross3D(GetAbsOrigin() + (BodyDirection3D() * 256.0f),
                           Vector(2, 2, 2), -Vector(2, 2, 2), 255, 0, 255, true,
                           0.1f);
  }
...
bool CNPC_Hunter::HandleChargeImpact(Vector vecImpact, CBaseEntity *pEntity) {
  // Cause a shock wave from this point which will disrupt nearby physics
  // objects
  ...
  if (!pEntity || pEntity->IsWorld()) {
    ...
    // Did we manage to find him? If not, increment our charge miss count and
    // abort.
    if (pEntity->IsWorld()) {
      return true;
    }
  ...
  if (IRelationType(pEntity) == D_HT &&
      (GetNextAttack() < gpGlobals->curtime)) {
    ...
    if (!pEntity->IsNPC()) {
      pEntity->ApplyAbsVelocityImpulse((BodyDirection2D() * 400) +
                                       Vector(0, 0, 200));
    }
  ...
  if (pEntity->GetMoveType() == MOVETYPE_NONE ||
      pEntity->GetMoveType() == MOVETYPE_PUSH) {
    CBreakable *pBreakable = dynamic_cast<CBreakable *>(pEntity);
    if (pBreakable && pBreakable->IsBreakable() &&
        pBreakable->m_takedamage == DAMAGE_YES && pBreakable->GetHealth() > 0) {
      ChargeDamage(pEntity);
    }
    return true;
  }
  ...
  if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
    ...
    if (pPhysics) {
      ...
      if (!pPhysics->IsMoveable()) return true;

      float entMass = PhysGetEntityMass(pEntity);
      float minMass = VPhysicsGetObject()->GetMass() * 0.5f;
      if (entMass < minMass) {
        if (entMass < minMass * 0.666f ||
            pEntity->CollisionProp()->BoundingRadius() < GetHullHeight()) {
          if (pEntity->GetHealth() > 0) {
            CBreakableProp *pBreakable =
                dynamic_cast<CBreakableProp *>(pEntity);
            if (pBreakable && pBreakable->m_takedamage == DAMAGE_YES &&
                pBreakable->GetHealth() > 0 && pBreakable->GetHealth() <= 50) {
              ChargeDamage(pEntity);
            }
          }
...
void CNPC_Hunter::Explode() {
  ...
  PropBreakableCreateAll(GetModelIndex(), NULL, EyePosition(), GetAbsAngles(),
                         velocity, angVelocity, 1.0, 150, COLLISION_GROUP_NPC,
                         this);

  ExplosionCreate(EyePosition(), GetAbsAngles(), this, 500, 256,
                  (SF_ENVEXPLOSION_NOPARTICLES | SF_ENVEXPLOSION_NOSPARKS |
                   SF_ENVEXPLOSION_NODLIGHTS | SF_ENVEXPLOSION_NODAMAGE |
...
Activity CNPC_Hunter::NPC_TranslateActivity(Activity baseAct) {
  if ((baseAct == ACT_WALK) || (baseAct == ACT_RUN)) {
    if (GetEnemy()) {
      ...
      if (UTIL_DistApprox(vecEnemyLKP, GetAbsOrigin()) <
          HUNTER_FACE_ENEMY_DIST) {
    ...
    if (!m_bPlanted &&
        (m_bEnableUnplantedShooting || IsStriderBuster(GetEnemy()))) {
...
void CNPC_Hunter::HandleAnimEvent(animevent_t *pEvent) {
  ...
  if (pEvent->event == AE_HUNTER_MELEE_ATTACK_LEFT) {
    ...
    MeleeAttack(HUNTER_MELEE_REACH, sk_hunter_dmg_one_slash.GetFloat(),
                QAngle(25, 30, -20), right + forward, HUNTER_BLOOD_LEFT_FOOT);
    return;
  }
  ...
  if (pEvent->event == AE_HUNTER_SPRAY_BLOOD) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      trace_t tr;
      AI_TraceLine(vecOrigin, vecOrigin + (vecTraceDir * 192.0f),
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::AddEntityRelationship(CBaseEntity *pEntity,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::ScheduledMoveToGoalEntity(int scheduleType,
...
int CNPC_Hunter::NumHuntersInMySquad() {
  ...
  while (pSquadmate) {
    if (pSquadmate->m_iClassname == m_iClassname) count++;
...
void CAI_HunterEscortBehavior::SetEscortTarget(CNPC_Strider *pStrider,
  ...
  if (!GetOuter()->GetSquad()) {
    GetOuter()->AddToSquad(AllocPooledString(
        CFmtStr("%s_hunter_squad", STRING(pStrider->GetEntityName()))));
  }
...
void CNPC_Hunter::InputUseSiegeTargets(inputdata_t &inputdata) {
  ...
  m_flTimeNextSiegeTargetAttack =
      gpGlobals->curtime +
      random->RandomFloat(1, hunter_siege_frequency.GetFloat());
  ...
  if (m_iszSiegeTargetName == NULL_STRING) {
    ...
    m_flDistTooFar =
        hunter_flechette_max_range.GetFloat() * HUNTER_SIEGE_MAX_DIST_MODIFIER;
  }
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::FVisible(CBaseEntity *pEntity, int traceMask,
  ...
  if (IsStriderBuster(pTarget)) {
    ...
    if (pTarget->VPhysicsGetObject()) {
      if ((pTarget->VPhysicsGetObject()->GetGameFlags() &
        ...
        if (gpGlobals->curtime - StriderBuster_GetPickupTime(pTarget) >
            hunter_hate_held_striderbusters_delay.GetFloat()) {
          if (StriderBuster_NumFlechettesAttached(pTarget) <= 2) {
            if (m_EscortBehavior.GetEscortTarget() &&
                (m_EscortBehavior.GetEscortTarget()
                     ->GetAbsOrigin()
                     .AsVector2D() -
                 pTarget->GetAbsOrigin().AsVector2D())
                        .LengthSqr() <
                    Square(
                        hunter_hate_held_striderbusters_tolerance.GetFloat())) {
              return true;
            }
          }
        }
      ...
      bool bThrown = (pTarget->VPhysicsGetObject()->GetGameFlags() &
                      FVPHYSICS_WAS_THROWN) != 0;
      bool bAttached = StriderBuster_IsAttachedStriderBuster(pTarget);

      if ((bThrown && !bAttached) &&
          hunter_hate_thrown_striderbusters.GetBool()) {
        float t;
        float dist = CalcDistanceSqrToLineSegment2D(
            m_EscortBehavior.GetEscortTarget()->GetAbsOrigin().AsVector2D(),
            pTarget->GetAbsOrigin().AsVector2D(),
            pTarget->GetAbsOrigin().AsVector2D() +
                pTarget->GetSmoothedVelocity().AsVector2D(),
            &t);

        if (t > 0 &&
            dist <
                Square(
                    hunter_hate_thrown_striderbusters_tolerance.GetFloat())) {
          return true;
        }
        return false;
      }
      ...
      if (bAttached &&
          StriderBuster_IsAttachedStriderBuster(
              pTarget, m_EscortBehavior.GetEscortTarget()) &&
          hunter_hate_attached_striderbusters.GetBool()) {
        return true;
      }
    }
...
Disposition_t CNPC_Hunter::IRelationType(CBaseEntity *pTarget) {
  if (!pTarget) return D_NU;

  if (IsStriderBuster(pTarget)) {
    if (HateThisStriderBuster(pTarget)) return D_HT;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::UpdateEnemyMemory(CBaseEntity *pEnemy, const Vector &position,
...
bool CNPC_Hunter::CanPlantHere(const Vector &vecPos) {
  ...
  trace_t tr;
  UTIL_TraceHull(vecPos, vecPos, vecMins, vecMaxs, MASK_NPCSOLID, this,
                 COLLISION_GROUP_NONE, &tr);
  if (tr.startsolid) {
    ...
    UTIL_TraceHull(vecStart, vecPos, vecMins, vecMaxs, MASK_NPCSOLID, this,
                   COLLISION_GROUP_NONE, &tr);
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
int CNPC_Hunter::MeleeAttack1ConditionsVsEnemyInVehicle(
    CBaseCombatCharacter *pEnemy, float flDot) {
  if (!IsCorporealEnemy(GetEnemy())) return COND_NONE;
  ...
  VectorNormalize(vecDelta);
  trace_t tr;
  AI_TraceHull(WorldSpaceCenter(), WorldSpaceCenter() + (vecDelta * 64),
               -Vector(8, 8, 8), Vector(8, 8, 8), MASK_SHOT, this,
               COLLISION_GROUP_NONE, &tr);
...
int CNPC_Hunter::MeleeAttack1Conditions(float flDot, float flDist) {
  ...
  if ((gpGlobals->curtime <
       m_flNextMeleeTime) &&  // allow berzerk bashing if cornered
      !(m_hAttachedBusters.Count() > 0 &&
        gpGlobals->curtime < m_fCorneredTimer)) {
    return COND_NONE;
  }
  ...
  trace_t tr;
  AI_TraceHull(WorldSpaceCenter(),
               WorldSpaceCenter() + forward * HUNTER_MELEE_REACH, vecMins,
               vecMaxs, MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);

  if (tr.fraction == 1.0 || !tr.m_pEnt) {
    // This attack would miss completely. Trick the hunter into moving around
    // some more.
    return COND_TOO_FAR_TO_ATTACK;
  }

  if (tr.m_pEnt == GetEnemy() || tr.m_pEnt->IsNPC() ||
      (tr.m_pEnt->m_takedamage == DAMAGE_YES &&
       (dynamic_cast<CBreakableProp *>(tr.m_pEnt)))) {
    // Let the hunter swipe at his enemy if he's going to hit them.
    // Also let him swipe at NPC's that happen to be between the hunter and the
    // enemy. This makes mobs of hunters seem more rowdy since it doesn't leave
    // guys in the back row standing around. Also let him swipe at things that
    // takedamage, under the assumptions that they can be broken.
    return COND_CAN_MELEE_ATTACK1;
  }
  ...
  {
          // The trace hit something solid, but it's not the enemy. If this item
  is closer to the hunter than
          // the enemy is, treat this as an obstruction.
          Vector vecToEnemy = GetEnemy()->WorldSpaceCenter() -
  WorldSpaceCenter(); Vector vecTrace = tr.endpos - tr.startpos;
  ...
  if (!tr.m_pEnt->IsWorld() && GetEnemy() &&
      GetEnemy()->GetGroundEntity() == tr.m_pEnt) {
    // Try to swat whatever the player is standing on instead of acting like a
    // dill.
    return COND_CAN_MELEE_ATTACK1;
  }
...
bool CNPC_Hunter::IsCorporealEnemy(CBaseEntity *pEnemy) {
  ...
  if (pEnemy->IsEffectActive(EF_NODRAW)) return false;

  // Don't flank, melee attack striderbusters.
  if (IsStriderBuster(pEnemy)) return false;
...
int CNPC_Hunter::RangeAttack2Conditions(float flDot, float flDist) {
  bool bIsBuster = IsStriderBuster(GetEnemy());
  bool bIsPerfectBullseye =
      (GetEnemy() && dynamic_cast<CNPC_Bullseye *>(GetEnemy()) &&
       ((CNPC_Bullseye *)GetEnemy())->UsePerfectAccuracy());

  if (!bIsPerfectBullseye && !bIsBuster && !hunter_flechette_test.GetBool() &&
      (gpGlobals->curtime < m_flNextRangeAttack2Time)) {
    return COND_NONE;
  }
  ...
  if (!bIsBuster && (flDist > flMaxFlechetteRange)) {
    return COND_TOO_FAR_TO_ATTACK;
             ...
             (!GetEnemy() || !GetEnemy()->ClassMatches("npc_bullseye")) &&
             flDist < hunter_flechette_min_range.GetFloat()) {
    return COND_TOO_CLOSE_TO_ATTACK;
  } else if (flDot < HUNTER_FACING_DOT) {
    return COND_NOT_FACING_ATTACK;
  }

  if (!bIsBuster && !m_bEnableUnplantedShooting &&
      !hunter_flechette_test.GetBool() && !CanPlantHere(GetAbsOrigin())) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::WeaponLOSCondition(const Vector &ownerPos,
  ...
  if ((pTargetEnt && tr.m_pEnt == pTargetEnt) || tr.fraction == 1.0 ||
      CanShootThrough(tr, targetPos)) {
    static Vector vMins(-2.0, -2.0, -2.0);
    static Vector vMaxs(-vMins);
    // Hit the enemy, or hit nothing (traced all the way to a nonsolid enemy
    // like a bullseye)
    AI_TraceHull(vFrom - Vector(0, 0, 18), targetPos, vMins, vMaxs, MASK_SHOT,
    ...
    if ((pTargetEnt && tr.m_pEnt == pTargetEnt) || tr.fraction == 1.0 ||
      ...
      if (hunter_show_weapon_los_condition.GetBool()) {
        ...
        NDebugOverlay::Line(vFrom - Vector(0, 0, 18), targetPos, 0, 0, 255,
                            false, 0.1);
      }
      return true;
    }
...
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_Hunter::MeleeAttack(float flDist, int iDamage,
                                      QAngle &qaViewPunch,
                                      Vector &vecVelocityPunch,
                                      int BloodOrigin) {
  // Added test because claw attack anim sometimes used when for cases other
  // than melee
  if (GetEnemy()) {
    trace_t tr;
    AI_TraceHull(WorldSpaceCenter(), GetEnemy()->WorldSpaceCenter(),
                 -Vector(8, 8, 8), Vector(8, 8, 8), MASK_SOLID_BRUSHONLY, this,
                 COLLISION_GROUP_NONE, &tr);
  ...
  CBaseEntity *pHurt =
      CheckTraceHullAttack(flDist, vecMins, vecMaxs, iDamage, DMG_SLASH);
  ...
  if (pHurt) {
    ...
    if (pPlayer != NULL && !(pPlayer->GetFlags() & FL_GODMODE)) {
      ...
      // Shake the screen
      UTIL_ScreenShake(pPlayer->GetAbsOrigin(), 100.0, 1.5, 1.0, 2,
                       SHAKE_START);
      ...
      /*if ( UTIL_ShouldShowBlood( pPlayer->BloodColor() ) )
      {
              ...
              QAngle angDiscard;
              GetBonePosition( LookupBone( "MiniStrider.top_eye_bone" ),
...
COLLISION_GROUP_NONE, &tr );
      ...
      if (IsMovablePhysicsObject(pHurt)) {
        ...
        if (pPhysics) {
          // If the object is being held by the player, break it or make them
          // drop it.
          if (pPhysics->GetGameFlags() & FVPHYSICS_PLAYER_HELD) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::TestShootPosition(const Vector &vecShootPos,
                                    const Vector &targetPos) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::MakeTracer(const Vector &vecTracerSrc, const trace_t &tr,
                             int iTracerType) {
  float flTracerDist;
  Vector vecDir;
  Vector vecEndPos;
  ...
  UTIL_Tracer(vecTracerSrc, tr.endpos, nAttachment, TRACER_FLAG_USEATTACHMENT,
              5000, true, "HunterTracer");
}
...
bool CNPC_Hunter::CanShootThrough(const trace_t &tr, const Vector &vecTarget) {
  ...
  AI_TraceLine(tr.endpos, vecTarget, MASK_SHOT, tr.m_pEnt, COLLISION_GROUP_NONE,
               &continuedTrace);
...
int CNPC_Hunter::GetSoundInterests() {
  return SOUND_WORLD | SOUND_COMBAT | SOUND_PLAYER | SOUND_DANGER |
         SOUND_PHYSICS_DANGER | SOUND_PLAYER_VEHICLE | SOUND_BULLET_IMPACT |
         SOUND_MOVE_AWAY;
}
...
//-----------------------------------------------------------------------------
bool CNPC_Hunter::IsInLargeOutdoorMap() { return m_bInLargeOutdoorMap; }

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::AlertSound() { EmitSound("NPC_Hunter.Alert"); }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::TraceAttack(const CTakeDamageInfo &inputInfo,
  ...
  if ((info.GetDamageType() & DMG_BULLET) ||
    ...
    if (info.GetDamageType() & DMG_BUCKSHOT) {
      flScale = sk_hunter_buckshot_damage_scale.GetFloat();
               ...
               (info.GetDamageType() & DMG_NEVERGIB)) {
      // Hunters resist most bullet damage, but they are actually vulnerable to
      // .357 rounds, since players regard that weapon as one of the game's
    ...
    DispatchParticleEffect("blood_impact_synth_01_arc_parent",
                           PATTACH_POINT_FOLLOW, this,
                           gm_nHeadCenterAttachment);
  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::PhysicsDamageEffect(const Vector &vecPos,
                                      const Vector &vecDir) {
  CEffectData data;
...
void CNPC_Hunter::TeslaThink() {
  ...
  if (gpGlobals->curtime < m_flTeslaStopTime) {
    SetContextThink(&CNPC_Hunter::TeslaThink,
                    gpGlobals->curtime + random->RandomFloat(0.1f, 0.3f),
                    HUNTER_ZAP_THINK);
  }
}
...
void CNPC_Hunter::BleedThink() {
  ...
  SetNextThink(gpGlobals->curtime + random->RandomFloat(0.6, 1.5),
               HUNTER_BLEED_THINK);
}
...
void CNPC_Hunter::JostleVehicleThink() {
  ...
  if (DotProduct(vecForce, GetAbsOrigin()) <
      DotProduct(vecForce, pInflictor->GetAbsOrigin())) {
  ...
  float flForceScale =
      RemapValClamped(flSpeed, hunter_jostle_car_min_speed.GetFloat(),
                      hunter_jostle_car_max_speed.GetFloat(), 50.0f, 150.0f);

  vecForce *= (flForceScale * pInflictor->VPhysicsGetObject()->GetMass());

  pInflictor->VPhysicsGetObject()->ApplyForceOffset(vecForce,
                                                    WorldSpaceCenter());
}
...
int CNPC_Hunter::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (pInflictor) {
    if (IsStriderBuster(pInflictor)) {
      ...
      SetContextThink(&CNPC_Hunter::TeslaThink, gpGlobals->curtime,
                      HUNTER_ZAP_THINK);
    ...
    } else if (pInflictor->ClassMatches(GetClassname()) &&
               !(info.GetDamageType() == DMG_GENERIC)) {
      return 0;
    } else if (pInflictor->ClassMatches("hunter_flechette")) {
      if (!((CHunterFlechette *)pInflictor)->WasThrownBack()) {
        // Flechettes only hurt us if they were thrown back at us by the player.
        // This prevents hunters from hurting themselves when they walk into
        // their own flechette clusters.
        return 0;
      }
    }
  }
  ...
  if (m_EscortBehavior.GetFollowTarget() &&
      m_EscortBehavior.GetFollowTarget() == pAttacker) {
    return 0;
  }

  bool bHitByUnoccupiedCar = false;
  if (((info.GetDamageType() & DMG_CRUSH) &&
       (pAttacker && pAttacker->IsPlayer())) ||
      (info.GetDamageType() & DMG_VEHICLE)) {
    ...
    if (flDamage < HUNTER_MIN_PHYSICS_DAMAGE) {
      ...
      if ((info.GetDamageType() & DMG_VEHICLE) ||
          (pInflictor && pInflictor->GetServerVehicle() &&
           ((bHitByUnoccupiedCar =
                 (dynamic_cast<CPropVehicleDriveable *>(pInflictor) &&
                  static_cast<CPropVehicleDriveable *>(pInflictor)
                          ->GetDriver() == NULL)) == false))) {
        ...
        // Pick a force direction based on which side we're on relative to the
        // vehicle's motion.
        Vector vecVelDir = pInflictor->GetSmoothedVelocity();
        if (vecVelDir.Length() >= hunter_jostle_car_min_speed.GetFloat()) {
          EmitSound("NPC_Hunter.HitByVehicle");
          m_hHitByVehicle = pInflictor;
          SetContextThink(&CNPC_Hunter::JostleVehicleThink, gpGlobals->curtime,
                          HUNTER_JOSTLE_VEHICLE_THINK);
        }
      }
  ...
  if ((myInfo.GetDamageType() & (DMG_CRUSH | DMG_BLAST)) &&
      (myInfo.GetDamage() > 0)) {
    if (!bHitByUnoccupiedCar) SetCondition(COND_HUNTER_STAGGERED);
  }
  ...
  // Take less damage from citizens and Alyx, otherwise hunters go down too
  // easily.
  float flScale = 1.0;

  if (pAttacker && ((pAttacker->Classify() == CLASS_CITIZEN_REBEL) ||
  ...
  if (flScale != 0) {
    ...
    if (pAttacker && pAttacker->IsPlayer()) {
      ...
      // This is achieved by updating the Hunter's enemy memory of the player
      // and making the Hunter's current enemy invalid for a short time.
      if (!GetEnemy() || !GetEnemy()->IsPlayer()) {
        UpdateEnemyMemory(pAttacker, pAttacker->GetAbsOrigin(), this);

        if (GetEnemy()) {
          // Gotta forget about this person for a little bit.
          GetEnemies()->SetTimeValidEnemy(
              GetEnemy(), gpGlobals->curtime + HUNTER_IGNORE_ENEMY_TIME);
        }
      }
    }
  ...
  if (IsUsingSiegeTargets() && info.GetAttacker() != NULL &&
...
void CNPC_Hunter::Event_Killed(const CTakeDamageInfo &info) {
  ...
  if (m_EscortBehavior.GetFollowTarget()) {
    if (AIGetNumFollowers(m_EscortBehavior.GetFollowTarget(), m_iClassname) ==
        1) {
      m_EscortBehavior.GetEscortTarget()->AlertSound();
      if (info.GetAttacker() && info.GetAttacker()->IsPlayer()) {
        m_EscortBehavior.GetEscortTarget()->UpdateEnemyMemory(
            UTIL_GetLocalPlayer(), UTIL_GetLocalPlayer()->GetAbsOrigin(), this);
      }
    }
  }
...
void CNPC_Hunter::StartBleeding() {
  ...
  DispatchParticleEffect("blood_drip_synth_01", PATTACH_POINT_FOLLOW, this,
                         gm_nHeadBottomAttachment);

  // Emit spurts of our blood
  SetContextThink(&CNPC_Hunter::BleedThink, gpGlobals->curtime + 0.1,
                  HUNTER_BLEED_THINK);
}
...
float CNPC_Hunter::MaxYawSpeed() {
  ...
  if (GetActivity() == ACT_HUNTER_ANGRY) return 0;

  if (GetActivity() == ACT_HUNTER_CHARGE_RUN) return 5;

  if (GetActivity() == ACT_HUNTER_IDLE_PLANTED) return 0;

  if (GetActivity() == ACT_HUNTER_RANGE_ATTACK2_UNPLANTED) return 180;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_Hunter::IsJumpLegal(const Vector &startPos, const Vector &apex,
                              const Vector &endPos) const {
  ...
  trace_t tr;
  UTIL_TraceHull(startPos, startPos, GetHullMins(), GetHullMaxs(),
                 MASK_NPCSOLID, this, COLLISION_GROUP_NONE, &tr);
  if (tr.startsolid) {
    // Trying to start a jump in solid! Consider checking for this in
    // CAI_MoveProbe::JumpMoveLimit.
    Assert(0);
    return false;
  }

  if (BaseClass::IsJumpLegal(startPos, apex, endPos, MAX_JUMP_RISE,
                             MAX_JUMP_DROP, MAX_JUMP_DISTANCE)) {
    return true;
  }
  return false;
}
...
void CNPC_Hunter::DoMuzzleFlash(int nAttachment) {
  ...
  DispatchParticleEffect("hunter_muzzle_flash", PATTACH_POINT_FOLLOW, this,
                         nAttachment);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Hunter::DelayRangedAttackers(float minDelay, float maxDelay,
                                       bool bForced) {
  ...
  if (!m_bEnableSquadShootDelay && !bForced) {
    m_flNextRangeAttack2Time =
        gpGlobals->curtime +
        random->RandomFloat(minDelay, maxDelay) * delayMultiplier;
    return;
  }
...
//-----------------------------------------------------------------------------
void CNPC_Hunter::GetShootDir(Vector &vecDir, const Vector &vecSrc,
                              CBaseEntity *pTargetEntity, bool bStriderBuster,
                              int nShotNum, bool bSingleShot) {
  ...
  if (pTargetEntity->Classify() == CLASS_PLAYER_ALLY_VITAL) {
    // Shooting at Alyx, most likely (in EP2). The attack is designed to
    // displace her, not necessarily actually harm her. So shoot at the area
    // around her feet.
  ...
  if (!bStriderBuster) {
    // If we're not firing at a strider buster, miss in an entertaining way for
    // the first three shots of each volley.
    if ((nShotNum < 3) && (flDist > 200)) {
      ...
      if (flDot < -0.8f) {
        ...
        vecTarget = GetAbsOrigin() +
                    (flPerc * (pTargetEntity->GetAbsOrigin() - GetAbsOrigin()));
      } else if (flDot > 0.8f) {
        ...
        vecTarget = pTargetEntity->EyePosition() +
                    (36.0f * (3 - nShotNum)) * vecMissDir;
      } else {
        // Our target is facing vaguely perpendicular to us, shoot across their
        // view.
        vecTarget = pTargetEntity->EyePosition() +
                    (36.0f * (3 - nShotNum)) * vecTargetForward;
      }
    }
...
bool CNPC_Hunter::ShouldSeekTarget(CBaseEntity *pTargetEntity,
  ...
  if (bStriderBuster) {
    ...
    if (pTargetEntity->VPhysicsGetObject() &&
        (pTargetEntity->VPhysicsGetObject()->GetGameFlags() &
         FVPHYSICS_PLAYER_HELD)) {
      bSeek = true;
      ...
      if (StriderBuster_IsAttachedStriderBuster(pTargetEntity)) {
        ...
        if (pEscortTarget &&
            (pEscortTarget->GetAbsOrigin() - pTargetEntity->GetAbsOrigin())
                    .LengthSqr() <
                Square(
                    hunter_seek_thrown_striderbusters_tolerance.GetFloat())) {
          bSeek = true;
        }
      }
    }
...
bool CNPC_Hunter::ShootFlechette(CBaseEntity *pTargetEntity, bool bSingleShot) {
  ...
  Vector vecDir;
  GetShootDir(vecDir, vecSrc, pTargetEntity, bStriderBuster, nShotNum,
              bSingleShot);
  ...
  if (IsUsingSiegeTargets() && nShotNum >= 2 && (nShotNum % 2) == 0) {
    // Near perfect accuracy for these three shots, so they are likely to fly
    // right into the windows. NOTE! In siege behavior in the map that this
    // behavior was designed for (ep2_outland_10), the Hunters will only ever
    // shoot at siege targets in siege mode. If you allow Hunters in siege mode
    // to attack players or other NPCs, this accuracy bonus will apply unless we
    // apply a bit more logic to it.
  ...
  CHunterFlechette *pFlechette =
      CHunterFlechette::FlechetteCreate(vecSrc, angShoot, this);
  ...
  if (nShotNum == 1 && pTargetEntity->Classify() == CLASS_PLAYER_ALLY_VITAL) {
    ...
    CSoundEnt::InsertSound(SOUND_DANGER | SOUND_CONTEXT_REACT_TO_SOURCE |
                           ...
                           pTargetEntity->EyePosition(), 180.0f, 2.0f, this);
  }
...
Vector CNPC_Hunter::LeftFootHit(float eventtime) {
  ...
  EmitSound(filter, entindex(), "NPC_Hunter.Footstep", &footPosition,
            eventtime);
...
Vector CNPC_Hunter::BackFootHit(float eventtime) {
  ...
  CPASAttenuationFilter filter(this);
  EmitSound(filter, entindex(), "NPC_Hunter.BackFootstep", &footPosition,
            eventtime);
  FootFX(footPosition);
...
void CNPC_Hunter::FootFX(const Vector &origin) {
  ...
  /*trace_t tr;
  AI_TraceLine( origin, origin - Vector(0,0,100), MASK_SOLID_BRUSHONLY, this,
  COLLISION_GROUP_NONE, &tr ); float yaw = random->RandomInt(0,120); for ( int i
  = 0; i < 3; i++ )
  {
...
CBaseEntity *CNPC_Hunter::GetEnemyVehicle() {
  ...
  CBaseCombatCharacter *pCCEnemy = GetEnemy()->MyCombatCharacterPointer();
  if (pCCEnemy != NULL) return pCCEnemy->GetVehicleEntity();
...
void CNPC_Hunter::DrawDebugGeometryOverlays() {
  if (m_debugOverlays & OVERLAY_BBOX_BIT) {
    ...
    if ((GetEntryNode(nSeq) == gm_nPlantedNode) &&
        (GetExitNode(nSeq) == gm_nPlantedNode)) {
      // planted - green
      NDebugOverlay::Box(GetAbsOrigin(), GetHullMins(), GetHullMaxs(), 0, 255,
                         0, 128, 0);
    } else if ((GetEntryNode(nSeq) == gm_nUnplantedNode) &&
               (GetExitNode(nSeq) == gm_nUnplantedNode)) {
      // unplanted - blue
      NDebugOverlay::Box(GetAbsOrigin(), GetHullMins(), GetHullMaxs(), 0, 0,
                         255, 128, 0);
    } else if ((GetEntryNode(nSeq) == gm_nUnplantedNode) &&
               (GetExitNode(nSeq) == gm_nPlantedNode)) {
      ...
      NDebugOverlay::Box(GetAbsOrigin(), GetHullMins(), GetHullMaxs(), 0, 255,
                         255, 128, 0);
    } else if ((GetEntryNode(nSeq) == gm_nPlantedNode) &&
               (GetExitNode(nSeq) == gm_nUnplantedNode)) {
      // unplanting transition - purple
      NDebugOverlay::Box(GetAbsOrigin(), GetHullMins(), GetHullMaxs(), 255, 0,
                         255, 128, 0);
    } else {
    ...
    NDebugOverlay::BoxDirection(EyePosition(), Vector(0, 0, -1),
                                Vector(200, 0, 1), vLeftDir, 255, 0, 0, 50, 0);
    NDebugOverlay::BoxDirection(EyePosition(), Vector(0, 0, -1),
                                Vector(200, 0, 1), vRightDir, 255, 0, 0, 50, 0);
    NDebugOverlay::BoxDirection(EyePosition(), Vector(0, 0, -1),
                                Vector(200, 0, 1), vEyeDir, 0, 255, 0, 50, 0);
...
const char *CNPC_Hunter::SelectRandomExpressionForState(NPC_STATE state) {
  if (m_bFlashlightInEyes) return NULL;

  if (!hunter_random_expressions.GetBool()) return NULL;

  char *szExpressions[4] = {"scenes/npc/hunter/hunter_scan.vcd",
                            "scenes/npc/hunter/hunter_eyeclose.vcd",
...
void CNPC_Hunter::UpdateAim() {
  if (!GetModelPtr() || !GetModelPtr()->SequencesAvailable()) return;
  ...
  if (GetEnemy() && GetState() != NPC_STATE_SCRIPT &&
      (eActivity != ACT_HUNTER_CHARGE_CRASH) &&
...
bool CNPC_Hunter::CanBecomeRagdoll() {
  return (m_nKillingDamageType & DMG_CRUSH) ||
         IsCurSchedule(
             SCHED_DIE,
             false) ||  // Finished playing death anim, time to ragdoll
         IsCurSchedule(
             SCHED_HUNTER_CHARGE_ENEMY,
             false) ||  // While moving, it looks better to ragdoll instantly
...
//-----------------------------------------------------------------------------
Activity CNPC_Hunter::GetDeathActivity() { return ACT_DIESIMPLE; }
...
void CAI_HunterEscortBehavior::OnDamage(const CTakeDamageInfo &info) {
  if (info.GetDamage() > 0 && info.GetAttacker()->IsPlayer() &&
      ...
      (GetOuter()->GetSquad()->GetSquadSoundWaitTime() <=
       gpGlobals->curtime))  // && !FarFromFollowTarget()
  {
    // Start the clock ticking. We'll return the the strider when the timer
    // elapses.
    m_flTimeEscortReturn =
        gpGlobals->curtime + random->RandomFloat(15.0f, 25.0f);
    GetOuter()->GetSquad()->SetSquadSoundWaitTime(
...
void CAI_HunterEscortBehavior::BuildScheduleTestBits() {
  ...
  if ((m_flTimeEscortReturn != 0) &&
      (gpGlobals->curtime > m_flTimeEscortReturn)) {
...
void CAI_HunterEscortBehavior::CheckBreakEscort() {
  if (m_flTimeEscortReturn != 0 &&
      (FarFromFollowTarget() || gpGlobals->curtime >= m_flTimeEscortReturn)) {
    ...
    if (GetOuter()->GetSquad()) {
      GetOuter()->GetSquad()->SetSquadSoundWaitTime(
          gpGlobals->curtime + random->RandomFloat(5.0f, 12.0f));
    }
  }
}
...
void CAI_HunterEscortBehavior::GatherConditions(void) {
  ...
  if (GetEnemy() && GetEnemy()->IsPlayer() && HasCondition(COND_SEE_ENEMY)) {
    if (GetOuter()->GetSquad()->GetSquadSoundWaitTime() <= gpGlobals->curtime &&
        ((CBasePlayer *)GetEnemy())->IsInAVehicle()) {
      m_flTimeEscortReturn =
          gpGlobals->curtime + random->RandomFloat(15.0f, 25.0f);
      GetOuter()->GetSquad()->SetSquadSoundWaitTime(
          m_flTimeEscortReturn + 1.0);  // prevent others from breaking escort
...
bool CAI_HunterEscortBehavior::ShouldFollow() {
  if (IsStriderBuster(GetEnemy())) return false;

  if (HasCondition(COND_HEAR_PHYSICS_DANGER)) return false;
    ...
    case TASK_MOVE_TO_FOLLOW_POSITION: {
      if (GetEnemy()) {
        if (GetOuter()->OccupyStrategySlot(SQUAD_SLOT_RUN_SHOOT)) {
          if (GetOuter()->GetSquad()->GetSquadMemberNearestTo(
                  GetEnemy()->GetAbsOrigin()) == GetOuter()) {
            GetOuter()->BeginVolley(NUM_FLECHETTE_VOLLEY_ON_FOLLOW,
                                    gpGlobals->curtime + 1.0 +
                                        random->RandomFloat(0, .25) +
                                        random->RandomFloat(0, .25));
          } else {
      ...
      if (!GetFollowTarget()) {
        ...
        if (GetEnemy()) {
          ...
          if (HasCondition(COND_SEE_ENEMY)) {
            ...
            float distSq =
                (pHunter->GetAbsOrigin() - pHunter->GetEnemy()->GetAbsOrigin())
                    .Length2DSqr();
            ...
            if (distSq < Square(maxDist)) {
              if (gpGlobals->curtime >= pHunter->m_flNextFlechetteTime) {
                ...
                if (bHasSlot) {
                  // Start the firing sound.
                  // CSoundEnvelopeController &controller =
                  // CSoundEnvelopeController::GetController(); if (
                  // controller.SoundGetVolume( pHunter->m_pGunFiringSound ) ==
                  // 0.0f )
                  //{
                  //	controller.SoundChangeVolume(
                  // pHunter->m_pGunFiringSound, 1.0f, 0.0f );
                  //}

                  pHunter->ShootFlechette(GetEnemy(), true);

                  if (--pHunter->m_nFlechettesQueued > 0) {
                    pHunter->m_flNextFlechetteTime =
                        gpGlobals->curtime + hunter_flechette_delay.GetFloat();
                  } else {
                    // Stop the firing sound.
                    // CSoundEnvelopeController &controller =
                    // CSoundEnvelopeController::GetController();
                    // controller.SoundChangeVolume( pHunter->m_pGunFiringSound,
                    // 0, 0.01f );

                    bVacate = true;
                    pHunter->BeginVolley(NUM_FLECHETTE_VOLLEY_ON_FOLLOW,
                                         gpGlobals->curtime + 1.0 +
                                             random->RandomFloat(0, .25) +
                                             random->RandomFloat(0, .25));
                  }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAI_HunterEscortBehavior::FindFreeHunters(
    CUtlVector<CNPC_Hunter *> *pFreeHunters) {
  ...
  for (i = 0; i < g_Hunters.Count(); i++) {
    ...
    if (pHunter->IsAlive() && pHunter->m_EscortBehavior.m_bEnabled) {
      if (pHunter->m_EscortBehavior.GetFollowTarget() == NULL ||
          !pHunter->m_EscortBehavior.GetFollowTarget()->IsAlive()) {
...
void CAI_HunterEscortBehavior::DistributeFreeHunters() {
  if (g_TimeLastDistributeFreeHunters != -1 &&
      gpGlobals->curtime - g_TimeLastDistributeFreeHunters <
  ...
  for (i = 0; i < g_AI_Manager.NumAIs() && freeHunters.Count(); i++) {
    ...
    CNPC_Strider *pStrider =
        (ppNPCs[i]->IsAlive()) ? dynamic_cast<CNPC_Strider *>(ppNPCs[i]) : NULL;
...
void CAI_HunterEscortBehavior::DrawDebugGeometryOverlays() {
  if (!GetFollowTarget()) return;

  Vector vecFollowPos = GetGoalPosition();
  if (FarFromFollowTarget()) {
    if (gpGlobals->curtime >= m_flTimeEscortReturn) {
      NDebugOverlay::HorzArrow(GetOuter()->GetAbsOrigin(), vecFollowPos, 16.0f,
                               255, 0, 0, 0, true, 0);
    } else {
      NDebugOverlay::HorzArrow(GetOuter()->GetAbsOrigin(), vecFollowPos, 16.0f,
                               255, 255, 0, 0, true, 0);
    ...
    NDebugOverlay::HorzArrow(GetOuter()->GetAbsOrigin(), vecFollowPos, 16.0f, 0,
                             255, 0, 0, true, 0);
  }
}
...
void Hunter_StriderBusterLaunched(CBaseEntity *pBuster) {
  ...
  for (int i = 0; i < nAIs; i++) {
    ...
    if (pNPC && (pNPC->Classify() == CLASS_COMBINE_HUNTER) &&
        pNPC->m_lifeState == LIFE_ALIVE) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void Hunter_StriderBusterAttached(CBaseEntity *pHunter,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void Hunter_StriderBusterDetached(CBaseEntity *pHunter,
                                  CBaseEntity *pAttached) {
  Assert(dynamic_cast<CNPC_Hunter *>(pHunter));
 ...
 public:
  void MakeMultipleNPCS(int nNPCs) {
    ...
    for (i = 0; i < candidates.Count() && freeHunters.Count() < 3; i++) {
      if (candidates[i]->GetHealth() >
          candidates[i]->GetMaxHealth() * MIN_HEALTH_PCT) {
    ...
    for (i = 0; i < freeHunters.Count() && nNPCs; i++) {
      freeHunters[i]->m_EscortBehavior.SetFollowTarget(
          this);  // this will make them not "free"
      freeHunters[i]->SetName(
          m_iszTemplateName);  // this will force the hunter to get the
                               // FollowStrider input
      nNPCs--;
      nSummoned++;
    }
    ...
    DevMsg(
        "Requested %d to spawn, Summoning %d free hunters, spawning %d new "
        "hunters\n",
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_HUNTER_FAIL_CHARGE_ENEMY"
    "		TASK_STOP_MOVING				0"
    "		TASK_FACE_ENEMY					0"
...
DEFINE_SCHEDULE(SCHED_HUNTER_FAIL_CHARGE_ENEMY,
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_ESTABLISH_LINE_OF_FIRE"
    "		TASK_STOP_MOVING				0"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_HUNTER_MELEE_ATTACK1,
                ...
                //"		TASK_SET_SCHEDULE
                // SCHEDULE:SCHED_HUNTER_POST_MELEE_WAIT"
                ""
                "	Interrupts")
...
//=========================================================
DEFINE_SCHEDULE(SCHED_HUNTER_CHANGE_POSITION,

                "	Tasks"
                "		TASK_STOP_MOVING			"
                "	0"
                "		TASK_WANDER				"
                "		720432"  // 6 feet to 36 feet
                "		TASK_RUN_PATH				"
                "	0"
                "		"
                ...
                "	0"
                "		TASK_SET_SCHEDULE			"
                "	SCHEDULE:SCHED_HUNTER_CHANGE_POSITION_FINISH"
                ""
                "	Interrupts"
                "		COND_ENEMY_DEAD"
                "		COND_CAN_MELEE_ATTACK1"
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_HUNTER_FAIL_IMMEDIATE"  // used because sched_fail includes
                                            // a one second pause. ick!
    "		TASK_STOP_MOVING					"
    "	0"
...
//=========================================================
//=========================================================
DEFINE_SCHEDULE(SCHED_HUNTER_PATROL,
...
//=========================================================
DEFINE_SCHEDULE(SCHED_HUNTER_STAGGER,
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_HUNTER_CHASE_ENEMY_MELEE"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_FACE_ENEMY					0"
    "		TASK_SET_ACTIVITY				"
    "ACTIVITY:ACT_IDLE"  // Translated to cover
    "		TASK_WAIT						1"
    ""
    "	Interrupts"
    "		COND_NEW_ENEMY"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_HUNTER_FAIL_IMMEDIATE,
...
DEFINE_SCHEDULE(
    ...
    "		TASK_WAIT_RANDOM				2"
    "		TASK_SET_SCHEDULE				"
    "SCHEDULE:SCHED_HUNTER_CHANGE_POSITION_SIEGE"
    ""
    ""
    "	Interrupts"
    "		COND_SEE_PLAYER"
