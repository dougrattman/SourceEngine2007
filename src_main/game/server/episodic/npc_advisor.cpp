...
// this file contains the definitions for the message ID constants (eg
...
ConVar advisor_use_impact_table(
    "advisor_use_impact_table", "1", FCVAR_NONE,
    "If true, advisor will use her custom impact damage table.");

#if NPC_ADVISOR_HAS_BEHAVIOR
ConVar advisor_throw_velocity("advisor_throw_velocity", "1100");
ConVar advisor_throw_rate("advisor_throw_rate",
                          "4");  // Throw an object every 4 seconds.
ConVar advisor_throw_warn_time(
    "advisor_throw_warn_time",
    "1.0");  // Warn players one second before throwing an object.
ConVar advisor_throw_lead_prefetch_time(
    "advisor_throw_lead_prefetch_time", "0.66", FCVAR_NONE,
    "Save off the player's velocity this many seconds before throwing.");
ConVar advisor_throw_stage_distance("advisor_throw_stage_distance", "180.0",
                                    FCVAR_NONE,
                                    "Advisor will try to hold an object this "
                                    "far in front of him just before throwing "
                                    "it at you. Small values will clobber the "
                                    "shield and be very bad.");
// ConVar advisor_staging_num("advisor_staging_num","1",FCVAR_NONE,"Advisor will
// queue up this many objects to throw at Gordon.");
ConVar advisor_throw_clearout_vel(
    "advisor_throw_clearout_vel", "200", FCVAR_NONE,
    "TEMP: velocity with which advisor clears things out of a throwable's way");
...
DEFINE_FIELD(m_flFloat, FIELD_FLOAT),
 ...
 public:
  ...
  void InputWrenchImmediate(
      inputdata_t &inputdata);  ///< immediately wrench an object into the air
 ...
 protected:
#if NPC_ADVISOR_HAS_BEHAVIOR
  Vector GetThrowFromPos(CBaseEntity *pEnt);  ///< Get the position in which we
                                              ///< shall hold an object prior to
                                              ///< throwing it
#endif
  ...
  void AddToThrownObjects(
      CBaseEntity *pEnt);  ///< add to the recently thrown objects array
  ...
  CBaseEntity *PickThrowable(bool bRequireInView);  ///< choose an object to
                                                    ///< throw at the player (so
                                                    ///< it can get stuffed in
                                                    ///< the handle array)

  /// push everything out of the way between an object I'm about to throw and
  /// the player.
  ...
  // todo: write accessor functions for m_hvStagedEnts so that it doesn't have
  // members added and removed willy nilly throughout code (will make the
  // networking below more reliable)

  void Write_BeamOn(CBaseEntity *pEnt);  ///< write a message turning a beam on
  void Write_BeamOff(
      CBaseEntity *pEnt);        ///< write a message turning a beam off
  ...
  // keep track of up to four objects after we have thrown them, to prevent
  // oscillation or levitation of recently thrown ammo.
  ...
  // unsigned char m_pickFailures; // the number of times we have tried to pick
  // a throwable and failed

  Vector m_vSavedLeadVel;  ///< save off player velocity for leading a bit
                           ///< before actually pelting them.
#endif
};
...
DEFINE_KEYFIELD(m_iszLevitateGoal1, FIELD_STRING, "levitategoal_bottom"),
    ...
    DEFINE_KEYFIELD(
        m_iszLevitationArea, FIELD_STRING,
        "levitationarea"),  ///< we will float all the objects in this volume

    DEFINE_PHYSPTR(m_pLevitateController), DEFINE_EMBEDDED(m_levitateCallback),
...
#if NPC_ADVISOR_HAS_BEHAVIOR
    DEFINE_KEYFIELD(m_iszStagingEntities, FIELD_STRING,
                    "staging_ent_names"),  ///< entities named this constitute
                                           ///< the positions to which we stage
                                           ///< objects to be thrown
    DEFINE_KEYFIELD(m_iszPriorityEntityGroupName, FIELD_STRING,
                    "priority_grab_name"),

    DEFINE_UTLVECTOR(m_hvStagedEnts, FIELD_EHANDLE),
    DEFINE_UTLVECTOR(m_hvStagingPositions, FIELD_EHANDLE),
    DEFINE_ARRAY(m_haRecentlyThrownObjects, FIELD_EHANDLE,
                 CNPC_Advisor::kMaxThrownObjectsTracked),
    DEFINE_ARRAY(m_flaRecentlyThrownObjectTimes, FIELD_TIME,
                 CNPC_Advisor::kMaxThrownObjectsTracked),
  ...
  SetViewOffset(
      Vector(0, 0, 80));  // Position of the eyes relative to NPC's origin.
...
void CNPC_Advisor::Activate() {
  ...
  m_hLevitateGoal1 =
      gEntList.FindEntityGeneric(NULL, STRING(m_iszLevitateGoal1), this);
  m_hLevitateGoal2 =
      gEntList.FindEntityGeneric(NULL, STRING(m_iszLevitateGoal2), this);
  m_hLevitationArea =
      gEntList.FindEntityGeneric(NULL, STRING(m_iszLevitationArea), this);
  ...
  // conditional assignment: find an entity by name and save it into pEnt. Bail
  // out when none are left.
  ...
  // positions loaded, 0 out the m_hvStagedEnts array with exactly as many
  // 0 spaces
  m_hvStagedEnts.SetCount(m_hvStagingPositions.Count());

  m_iStagingNum = 1;

  AssertMsg(m_hvStagingPositions.Count() > 0,
            "You did not specify any staging positions in the advisor's "
            "staging_ent_names !");
#endif
}
...
//-----------------------------------------------------------------------------
Class_T CNPC_Advisor::Classify() { return CLASS_COMBINE; }
...
void CNPC_Advisor::StartLevitatingObjects() {
  if (!m_pLevitateController) {
    m_pLevitateController =
        physenv->CreateMotionController(&m_levitateCallback);
  }
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    // NDebugOverlay::Box( pEnt->GetAbsOrigin(),
    // pEnt->CollisionProp()->OBBMins(), pEnt->CollisionProp()->OBBMaxs(), 0,
    // 255, 0, 1, 0.1 );
...
bool CNPC_Advisor::CanLevitateEntity(CBaseEntity *pEntity, int minMass,
                                     int maxMass) {
  if (!pEntity || pEntity->IsNPC()) return false;
...
#if NPC_ADVISOR_HAS_BEHAVIOR
// find an object to throw at the player and start the warning on it. Return
// object's pointer if we got something. Otherwise, return NULL if nothing left
// to throw. Will always leave the prepared object at the head of m_hvStagedEnts
CBaseEntity *CNPC_Advisor::ThrowObjectPrepare() {
  ...
  if (pThrowable) {
    ...
    m_flThrowPhysicsTime =
        gpGlobals->curtime + advisor_throw_warn_time.GetFloat();
...
void CNPC_Advisor::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    // DVS: TODO: if this gets expensive we can start caching the results and
    // doing it less often.
    case TASK_ADVISOR_FIND_OBJECTS: {
      // if we have a trigger volume, use the contents of that. If not, use a
      // hardcoded box (for debugging purposes) in both cases we validate the
      // objects using the same helper funclet just above. When we can count on
      // the trigger vol being there, we can elide the else{} clause here.

      CBaseEntity *pVolume = m_hLevitationArea;
      AssertMsg(pVolume,
                "Combine advisor needs 'levitationarea' key pointing to a "
                "trigger volume.");
      ...
      if (touchroot) {
        ...
        for (touchlink_t *link = touchroot->nextLink; link != touchroot;
             link = link->nextLink) {
          CBaseEntity *pTouch = link->entityTouched;
          if (CanLevitateEntity(pTouch, 10, 220)) {
            if (pTouch->GetMoveType() == MOVETYPE_VPHYSICS) {
              // Msg( "   %d added %s\n", m_physicsObjects.Count(), STRING(
              // list[i]->GetModelName() ) );
    ...
    // we're about to pelt the player with everything. Start the warning effect
    // on the first object.
...
//-----------------------------------------------------------------------------
// todo: find a way to guarantee that objects are made pickupable again when
// bailing out of a task
//-----------------------------------------------------------------------------
void CNPC_Advisor::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_ADVISOR_LEVITATE_OBJECTS: {
      ...
      // set the top and bottom on the levitation volume from the entities. If
      // we don't have both, zero it out so that we can use the old-style
      // simpler mechanism.
      if (m_hLevitateGoal1 && m_hLevitateGoal2) {
        ...
        if (m_levitateCallback.m_vecGoalPos1.z >
            m_levitateCallback.m_vecGoalPos2.z) {
          swap(m_levitateCallback.m_vecGoalPos1,
               m_levitateCallback.m_vecGoalPos2);
        }

        m_levitateCallback.m_flFloat =
            0.06f;  // this is an absolute accumulation upon gravity
      } else {
        m_levitateCallback.m_vecGoalPos1.Invalidate();
        m_levitateCallback.m_vecGoalPos2.Invalidate();

        // the below two stanzas are used for old-style floating, which is
        // linked to float up before thrown and down after
    ...
    // Pick a random object that we are levitating. If we have a clear LOS from
    // that object to our enemy's eyes, choose that one to throw. Otherwise,
    // keep looking.
    case TASK_ADVISOR_STAGE_OBJECTS: {
      if (m_iStagingNum > m_hvStagingPositions.Count()) {
        Warning(
            "Advisor tries to stage %d objects but only has %d positions named "
            "%s! Overriding.\n",
      ...
      // in the future i'll distribute the staging chronologically. For now,
      // yank all the objects at once.
      if (m_hvStagedEnts.Count() < m_iStagingNum) {
        // pull another object
        bool bDesperate = m_flStagingEnd - gpGlobals->curtime <
                          0.50f;  // less than one half second left
        ...
        if (pThrowable) {
          ...
          if (pPhys) {
            // no pickup!
            pPhys->SetGameFlags(pPhys->GetGameFlags() |
          ...
          DispatchParticleEffect("advisor_object_charge",
                                 PATTACH_ABSORIGIN_FOLLOW, pThrowable, 0,
                                 false);
        }
      }
      ...
      for (int ii = m_hvStagedEnts.Count() - 1; ii >= 0; --ii) {
        // just ignore lost objects (if the player destroys one, that's fine,
        // leave a hole)
        CBaseEntity *pThrowable = m_hvStagedEnts[ii];
        if (pThrowable) {
          PullObjectToStaging(pThrowable,
                              m_hvStagingPositions[ii]->GetAbsOrigin());
        }
      }
    ...
    case TASK_ADVISOR_BARRAGE_OBJECTS: {
      ...
      while (!pThrowable) {  // player has destroyed whatever I planned to hit
                             // him with, get something else
      ...
      if ((gpGlobals->curtime >
           m_flThrowPhysicsTime -
               advisor_throw_lead_prefetch_time.GetFloat()) &&
          !m_vSavedLeadVel.IsValid()) {
        // save off the velocity we will use to lead the player a little early,
        // so that if he jukes at the last moment he'll have a better shot of
        // dodging the object.
    ...
    case TASK_ADVISOR_PIN_PLAYER: {
      ...
      if ( gpGlobals->curtime > m_playerPinFailsafeTime )
      {
              GetEnemy()->SetGravity(1.0f);
              GetEnemy()->SetMoveType( MOVETYPE_WALK );
              Warning( "Advisor did not leave PIN PLAYER mode. Aborting due to
      ten second failsafe!\n" ); TaskFail("Advisor did not leave PIN PLAYER
      mode. Aborting due to ten second failsafe!\n"); break;
      }
      ...
      float desiredDisplacementLen =
      ExponentialDecay(0.250f,gpGlobals->frametime);// * sqrt(displacementLen);

      Vector nuPos = playerPos + (displacement * (1.0f -
      desiredDisplacementLen));
...
#if NPC_ADVISOR_HAS_BEHAVIOR
// helper function for testing whether or not an avisor is allowed to grab an
// object
...
// param bRequireInView : if true, only accept objects that are in the player's
// fov.
//
...
CBaseEntity *CNPC_Advisor::PickThrowable(bool bRequireInView) {
  ...
  const int numObjs =
      m_physicsObjects
          .Count();  ///< total number of physics objects in my system
  if (numObjs < 1) return NULL;  // bail out if nothing available
  ...
  Assert(numObjs < 128);  /// I'll come back and utlvector this shortly --
                          /// wanted easier debugging
  unsigned int candidates[128];
  unsigned int numCandidates = 0;

  if (!!m_iszPriorityEntityGroupName)  // if the string isn't 0
  {
    // first look to see if we have any priority objects.
    for (int ii = 0; ii < numObjs; ++ii) {
      CBaseEntity *pThrowEnt = m_physicsObjects[ii];
      // Assert(pThrowEnt);
      if (!pThrowEnt) continue;

      if (!pThrowEnt->NameMatches(m_iszPriorityEntityGroupName))  // if this is
                                                                  // not a
                                                                  // priority
                                                                  // object
        continue;

      bool bCanPick = AdvisorCanPickObject(pPlayer, pThrowEnt) &&
                      !m_hvStagedEnts.HasElement(m_physicsObjects[ii]);
      if (!bCanPick) continue;
  ...
  // if we found no priority objects (or don't have a priority), just grab
  // whatever
  if (numCandidates == 0) {
    for (int ii = 0; ii < numObjs; ++ii) {
      CBaseEntity *pThrowEnt = m_physicsObjects[ii];
  ...
  if (numCandidates == 0) return NULL;  // must have at least one candidate
...
/*! \TODO
        ...
        into the throw speed. This is temporarily fixed here by using
   SetVelocity instead of AddVelocity, but the proper fix is to pin the object
   to its staging point during the warn period. That will require maintaining a
   map of throwables to their staging points during the throw task.
*/
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::HurlObjectAtPlayer(CBaseEntity *pEnt,
                                      const Vector &leadVel) {
  IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
  ...
  //	const Vector &leadVel = pLeadVelocity ? *pLeadVelocity :
  // GetEnemy()->GetAbsVelocity();
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::PreHurlClearTheWay(CBaseEntity *pThrowable,
                                      const Vector &toPos) {
  ...
  AssertMsg(nFoundCt < 128,
            "Found more than 128 obstructions between advisor and Gordon while "
            "throwing. (safe to continue)\n");
  ...
  for (int i = 0; i < nFoundCt; ++i) {
        ...
        !DidThrow(obstruction))  // and I didn't just throw it
    {
      ...
      // this is an object we want to push out of the way. Compute a vector
      // perpendicular to the path of the throwables's travel, and thrust the
      // object along that vector.
      Vector thrust;
      CalcClosestPointOnLine(obstruction->GetAbsOrigin(),
                             pThrowable->GetAbsOrigin(), toPos, thrust);
  ...
  /*
          ...
          Ray_t ray;
          ray.Init( vecStartPoint, vecStartPoint + 2048 * vecVelDir, -extents,
     extents ); int nCount = UTIL_EntitiesAlongRay( list, 1024, ray, FL_NPC |
     FL_CLIENT ); for ( int i = 0; i < nCount; i++ )
          {
...
bool CNPC_Advisor::DidThrow(const CBaseEntity *pEnt) {
  // look through all my objects and see if they match this entity. Incidentally
  // if they're more than three seconds old, purge them.
...
void CNPC_Advisor::AddToThrownObjects(CBaseEntity *pEnt) {
  ...
  for (int ii = 0; ii < kMaxThrownObjectsTracked; ++ii) {
    if (m_haRecentlyThrownObjects[ii].IsValid()) {
      if (m_flaRecentlyThrownObjectTimes[ii] <
          m_flaRecentlyThrownObjectTimes[oldestThrownObject]) {
...
//-----------------------------------------------------------------------------
void CNPC_Advisor::PullObjectToStaging(CBaseEntity *pEnt,
                                       const Vector &stagingPos) {
  IPhysicsObject *pPhys = pEnt->VPhysicsGetObject();
  Assert(pPhys);
  ...
  float desiredDisplacementLen =
      ExponentialDecay(STAGING_OBJECT_FALLOFF_TIME,
                       gpGlobals->frametime);  // * sqrt(displacementLen);

  Vector vel;
  AngularImpulse angimp;
  pPhys->GetVelocity(&vel, &angimp);

  vel = (1.0f / gpGlobals->frametime) *
        (displacement * (1.0f - desiredDisplacementLen));
...
int CNPC_Advisor::SelectSchedule() {
  if (IsInAScript()) return SCHED_ADVISOR_IDLE_STAND;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CNPC_Advisor::IdleSound() { EmitSound("NPC_Advisor.Idle"); }

void CNPC_Advisor::AlertSound() { EmitSound("NPC_Advisor.Alert"); }
...
bool CNPC_Advisor::QueryHearSound(CSound *pSound) {
  ...
  if (pOwner && pSound->IsSoundType(SOUND_COMBAT) &&
      pSound->SoundChannel() != SOUNDENT_CHANNEL_NPC_FOOTSTEP &&
      pSound->m_hOwner.IsValid() && pOwner->IsPlayer()) {
...
void CNPC_Advisor::InputPinPlayer(inputdata_t &inputdata) {
  ...
  // 0 string means designer is trying to unpin the player
  if (!targetname) {
...
void CNPC_Advisor::GatherConditions(void) {
  ...
  if ((m_bWasScripting && bInScript == false) ||
      (m_bWasScripting == false && bInScript)) {
...
void CNPC_Advisor::InputWrenchImmediate(inputdata_t &inputdata) {
  ...
  // for all entities with that name that aren't floating, punt them at me and
  // add them to the levitation
  ...
  while ((pEnt = gEntList.FindEntityByName(pEnt, groupname)) != NULL) {
    ...
    if (!m_physicsObjects.HasElement(pEnt)) {
      ...
      if (pPhys) {
        ...
        Warning("Advisor tried to wrench %s, but it is not moveable!",
                pEnt->GetEntityName().ToCStr());
      }
    }
  }
}
...
void CNPC_Advisor::InputTurnBeamOn(inputdata_t &inputdata) {
  ...
  if (pTarget) {
    ...
    Warning("InputTurnBeamOn could not find object %s",
...
// MOTION CALLBACK
//==============================================================================================
CAdvisorLevitate::simresult_e CAdvisorLevitate::Simulate(
    IPhysicsMotionController *pController, IPhysicsObject *pObject,
    float deltaTime, Vector &linear, AngularImpulse &angular) {
  // this function can be optimized to minimize branching if necessary (PPE
  // branch prediction)
  CNPC_Advisor *pAdvisor = static_cast<CNPC_Advisor *>(m_Advisor.Get());
  Assert(pAdvisor);

  if (!OldStyle()) {  // independent movement of all objects
    // if an object was recently thrown, just zero out its gravity.
    if (pAdvisor->DidThrow(
            static_cast<CBaseEntity *>(pObject->GetGameData()))) {
      ...
      // if above top limit and moving up, move down. if below bottom limit and
      // moving down, move up.
      if (bMovingUp) {
        if (pos.z > m_vecGoalPos2.z) {
          // turn around move down
          linear =
              Vector(0, 0, Square((1.0f - m_flFloat)) * sv_gravity.GetFloat());
  ...
  } else  // old stateless technique
  {
    ...
    */
    ...
    linear = Vector(0, 0, m_flFloat * sv_gravity.GetFloat());  // + m_flFloat *
                                                               // 0.5 * (
                                                               // vecDir1 +
                                                               // vecDir2 );
    angular = Vector(0, 0, 10);
...
static impactentry_t advisorLinearTable[] = {
    {100 * 100, 10}, {250 * 250, 25},    {350 * 350, 50},
    {500 * 500, 75}, {1000 * 1000, 100},
};
...
static impactdamagetable_t gAdvisorImpactDamageTable = {
    ...
    45,  // large mass in kg
    2,   // large mass scale (anything over 500kg does 4X as much energy to read
         // from damage table)
...
const impactdamagetable_t &CNPC_Advisor::GetPhysicsImpactDamageTable(void) {
  return advisor_use_impact_table.GetBool()
             ? gAdvisorImpactDamageTable
...
DECLARE_TASK(TASK_ADVISOR_STAGE_OBJECTS)    // haul all the objects into the
                                            // throw-from slots
