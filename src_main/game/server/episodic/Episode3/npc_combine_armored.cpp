...
class CArmorPiece : public CBaseAnimating {
 ...
 public:
  ...
  void Precache(void) { PrecacheModel(STRING(GetModelName())); }
...
void CNPC_Combine_Armored::SpawnArmorPieces(void) {
  ...
  for (int i = 0; i < std::size(ArmorPiecesPositions); i++) {
    CArmorPiece *pArmor = (CArmorPiece *)CBaseEntity::CreateNoSpawn(
        "combine_armor_piece", GetAbsOrigin(), GetAbsAngles(), this);
    pArmor->SetModelName(MAKE_STRING(ArmorPiecesPositions[i].pszModel));
    pArmor->SetParent(this,
                      LookupAttachment(ArmorPiecesPositions[i].pszAttachment));
