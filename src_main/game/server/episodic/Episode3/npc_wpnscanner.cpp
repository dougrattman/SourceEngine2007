...
void CNPC_WpnScanner::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_WPNSCANNER_ATTACK: {
      ...
      vecToTarget = vecToTarget + x * VECTOR_CONE_20DEGREES * vecRight +
                    y * VECTOR_CONE_20DEGREES * vecUp;
...
float CNPC_WpnScanner::MinGroundDist(void) {
  if (m_nFlyMode == SCANNER_FLY_ATTACK) return 16;
...
void CNPC_WpnScanner::MoveToAttack(float flInterval) {
  ...
  Vector idealPos =
      IdealGoalForMovement(vTargetPos, GetAbsOrigin(), GetGoalDistance(), 128);

  NDebugOverlay::Box(idealPos, -Vector(4, 4, 4), Vector(4, 4, 4), 0, 255, 0, 8,
                     0.1);
