...
void CAI_BaseNPC::InitSchedulingTables() {
  CAI_BaseNPC::gm_ClassScheduleIdSpace.Init(
      "CAI_BaseNPC", CAI_BaseNPC::GetSchedulingSymbols());
...
// UNDONE: enable this after the schedules are all loaded (right now some load
// in monster spawns)
#if 0
...
// Output :
//-----------------------------------------------------------------------------
CAI_Schedule *CAI_SchedulesManager::CreateSchedule(char *name,
                                                   int schedule_id) {
  // Allocate schedule
  CAI_Schedule *pSched =
      new CAI_Schedule(name, schedule_id, CAI_SchedulesManager::allSchedules);
...
int CAI_SchedulesManager::GetPathID(const char *token) {
  ...
  //	else if ( !stricmp( token, "FLANK" ) )		{	return
  // PATH_FLANK;
  //}
  //	else if ( !stricmp( token, "FLANK_LOS" ) )	{	return
  // PATH_FLANK_LOS;	}
  else if (!stricmp(token, "COVER")) {
    return PATH_COVER;
  }
  //	else if ( !stricmp( token, "COVER_LOS" ) )	{	return
  // PATH_COVER_LOS;	}
...
//			As I'm parsing a human generated file, give a lot of
// error output
// Output:  true  - if data successfully read
//			false - if data load fails
//-----------------------------------------------------------------------------

bool CAI_SchedulesManager::LoadSchedulesFromBuffer(
    const char *prefix, char *pStartFile, CAI_ClassScheduleIdSpace *pIdSpace) {
  char token[1024];
  ...
  while (!stricmp("Schedule", token)) {
    ...
    if (GetScheduleByName(token)) {
      DevMsg(
          "ERROR: file contains a schedule (%s) that has already been "
          "defined!\n",
          token);
    ...
    if (stricmp(token, "Tasks")) {
      DevMsg(
          "ERROR: LoadSchd (%s): (%s) Malformed AI Schedule.  Expecting "
          "'Tasks' keyword.\n",
    ...
    while ((token[0] != NULL) && (stricmp("Interrupts", token))) {
      ...
      tempTask[taskNum].iTask = (pIdSpace) ? pIdSpace->TaskGlobalToLocal(taskID)
                                           : AI_RemapFromGlobal(taskID);

      // If not a valid condition, send a warning message
      if (tempTask[taskNum].iTask == -1) {
        DevMsg("ERROR: LoadSchd (%s): (%s) Unknown task %s!\n", prefix,
               new_schedule->GetName(), token);
        Assert(0);
        return false;
      }
      ...
      if (!stricmp("Activity", token)) {
        ...
        // Convert generic ID to sub-class specific enum
        int taskID = CAI_BaseNPC::GetTaskID(token);
        tempTask[taskNum].flTaskData = (pIdSpace)
                                           ? pIdSpace->TaskGlobalToLocal(taskID)
                                           : AI_RemapFromGlobal(taskID);
        ...
        tempTask[taskNum].flTaskData =
            (pIdSpace) ? pIdSpace->ScheduleGlobalToLocal(schedID)
                       : AI_RemapFromGlobal(schedID);
      ...
      } else if (!stricmp("Interrupts", token) ||
                 !strnicmp("TASK_", token, 5)) {
        // a parse error.  Interrupts is the next section, TASK_ is probably the
        // next task, missing task argument?
        Warning(
            "ERROR: LoadSchd (%s): (%s) Bad syntax at task #%d (wasn't "
            "expecting %s)\n",
    ...
    while ((token[0] != NULL) && (stricmp("Schedule", token))) {
      ...
      // Otherwise, add to this schedules list of conditions
        ...
        Assert(AI_IdIsGlobal(condID) && interrupt >= 0 &&
               interrupt < MAX_CONDITIONS);
...
bool CAI_SchedulesManager::LoadSchedules(const char *prefix,
...
CAI_Schedule *CAI_SchedulesManager::GetScheduleFromID(int schedID) {
  for (CAI_Schedule *schedule = CAI_SchedulesManager::allSchedules;
       schedule != NULL; schedule = schedule->nextSchedule) {
    if (schedule->m_iScheduleID == schedID) return schedule;
  }
...
CAI_Schedule *CAI_SchedulesManager::GetScheduleByName(const char *name) {
  for (CAI_Schedule *schedule = CAI_SchedulesManager::allSchedules;
       schedule != NULL; schedule = schedule->nextSchedule) {
    if (FStrEq(schedule->GetName(), name)) return schedule;
  }
...
void CAI_SchedulesManager::DeleteAllSchedules(void) {
  ...
  if (m_CurLoadSig < 0) m_CurLoadSig = 0;
