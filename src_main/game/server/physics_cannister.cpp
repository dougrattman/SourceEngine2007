...
BEGIN_DATADESC(CPhysicsCannister)
  ...
  DEFINE_OUTPUT(m_onActivate, "OnActivate"),
      ...
      DEFINE_FIELD(m_thrustOrigin,
                   FIELD_VECTOR),  // this is a position, but in local space
      DEFINE_EMBEDDED(m_thruster), DEFINE_PHYSPTR(m_pController),
...
bool CPhysicsCannister::TestCollision(const Ray_t &ray, unsigned int mask,
                                      trace_t &trace) {
...
int CPhysicsCannister::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  if (info.GetDamageType() &
      (DMG_BULLET | DMG_BUCKSHOT | DMG_BURN | DMG_BLAST)) {
    Explode(info.GetAttacker());
  }
...
void CPhysicsCannister::TraceAttack(const CTakeDamageInfo &info,
                                    const Vector &dir, trace_t *ptr) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysicsCannister::CannisterActivate(CBaseEntity *pActivator,
  ...
  m_pJet = dynamic_cast<CSteamJet *>(
      CBaseEntity::Create("env_steam", m_thrustOrigin, angles, this));
...
//-----------------------------------------------------------------------------
// Purpose: The cannister's been fired by a weapon, so it should stay pretty
// accurate
//-----------------------------------------------------------------------------
...
void CPhysicsCannister::Explode(CBaseEntity *pAttacker) {
  ...
  PropBreakableCreateAll(GetModelIndex(), pPhysics, GetAbsOrigin(),
                         GetAbsAngles(), velocity, angVelocity, 1.0, 20,
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysicsCannister::VPhysicsCollision(int index,
...
//-----------------------------------------------------------------------------
void CPhysicsCannister::OnPhysGunPickup(CBasePlayer *pPhysGunUser,
...
// Purpose:
//-----------------------------------------------------------------------------
void CPhysicsCannister::OnPhysGunDrop(CBasePlayer *pPhysGunUser,
...
//-----------------------------------------------------------------------------
// Purpose: Update the visible representation of the physic system's
