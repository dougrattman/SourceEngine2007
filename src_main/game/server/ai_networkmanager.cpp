...
#define DebuggingConnect(node1, node2)                               \
  ((node1 == g_DebugConnectNode1 && node2 == g_DebugConnectNode2) || \
...
CON_COMMAND(ai_debug_node_connect,
...
BEGIN_DATADESC(CAI_NetworkManager)
  ...
  DEFINE_FIELD(m_bNeedGraphRebuild, FIELD_BOOLEAN),
      ...
      // Function pointers
      DEFINE_FUNCTION(DelayedInit), DEFINE_FUNCTION(RebuildThink),
...
//			 going to rebuild the network, before we hang during the
// rebuild
//------------------------------------------------------------------------------
...
void CAI_NetworkManager::RebuildNetworkGraph(void) {
  if (m_pfnThink != (void (CBaseEntity::*)()) &
...
//-----------------------------------------------------------------------------
// Purpose: Called by save restore code if no valid load graph was loaded at
// restore time.
//  Prevents writing out of a "bogus" node graph...
// Input  :  -
//-----------------------------------------------------------------------------
void CAI_NetworkManager::MarkDontSaveGraph() { m_bDontSaveGraph = true; }
...
void CAI_NetworkManager::SaveNetworkGraph(void) {
  if (m_bDontSaveGraph) return;

  if (!m_bNeedGraphRebuild) return;
  ...
  Q_strncat(szNrpFilename, STRING(gpGlobals->mapname), sizeof(szNrpFilename),
            COPY_ALL_CHARACTERS);
  Q_strncat(szNrpFilename, IsX360() ? ".360.ain" : ".ain",
            sizeof(szNrpFilename), COPY_ALL_CHARACTERS);
  ...
  for (node = 0; node < m_pNetwork->m_iNumNodes; node++) {
    ...
    for (int link = 0; link < pNode->NumLinks(); link++) {
      ...
      if (node == pLink->m_iSrcID) {
        ...
        buf.Put(pLink->m_iAcceptedMoveTypes,
                sizeof(pLink->m_iAcceptedMoveTypes));
      }
    }
  }
  ...
  for (node = 0; node < m_pNetwork->m_iNumNodes; node++) {
    int iPreviousNodeBinding =
        wcIDs.Find(GetEditOps()->m_pNodeIndexTable[node]);
    if (iPreviousNodeBinding != wcIDs.InvalidIndex()) {
      if (!bCheckForProblems) {
        DevWarning(
            "******* MAP CONTAINS DUPLICATE HAMMER NODE IDS! CHECK FOR "
            "PROBLEMS IN HAMMER TO CORRECT *******\n");
        bCheckForProblems = true;
      }
      DevWarning(
          "   AI node %d is associated with Hammer node %d, but %d is already "
          "bound to node %d\n",
          node, GetEditOps()->m_pNodeIndexTable[node],
          GetEditOps()->m_pNodeIndexTable[node], wcIDs[iPreviousNodeBinding]);
    } else {
...
void CAI_NetworkManager::SaveNetworkGraph( void )
{
        ...
        Q_strncat( szNrpFilename, STRING( gpGlobals->mapname ),
COPY_ALL_CHARACTERS ); Q_strncat( szNrpFilename, ".ain", COPY_ALL_CHARACTERS );
        ...
        {
                filesystem->FPrintf ( file, "Location
%4f,%4f,%4f\n",m_pAInode[node]->GetOrigin().x, m_pAInode[node]->GetOrigin().y,
m_pAInode[node]->GetOrigin().z ); for (int hull =0;hull<NUM_HULLS;hull++)
                {
...
m_pAInode[node]->GetYaw() ); filesystem->FPrintf ( file, "NodeType
%4d\n",m_pAInode[node]->GetType()); filesystem->FPrintf ( file, "NodeInfo
%4d\n",m_pAInode[node]->m_eNodeInfo); filesystem->FPrintf ( file, "Neighbors ");
                ...
                filesystem->FPrintf ( file, "NumLinks
...
m_pAInode[node]->GetLinkByIndex(link)->m_iSrcID); filesystem->FPrintf ( file,
"LinkDestID      %4d\n", m_pAInode[node]->GetLinkByIndex(link)->m_iDestID);

                                for (int hull =0;hull<NUM_HULLS;hull++)
                                {
                                        filesystem->FPrintf ( file, "Hulls
...
void CAI_NetworkManager::LoadNetworkGraph(void) {
  ...
  if (printedHeader)
    DevMsg(
        "\n** Should run \"Check For Problems\" on the VMF then verify dynamic "
        "links\n");
#endif
...
COPY_ALL_CHARACTERS ); Q_strncat( szNrpFilename, ".ain", COPY_ALL_CHARACTERS );
        ...
        //
------------------------------------------------------------------------
        // If in wc_edit mode allocate extra space for nodes that might be
created
        //
------------------------------------------------------------------------ if (
engine->IsInEditMode() )
        ...
        if ( !m_pAInode )
        {
                Warning( "LoadNetworkGraph:  Not enough memory to create %i
nodes\n", numNodes ); Assert(0); return;
        }
        ...
        for (int node = 0; node < numNodes; node++)
        {
                ...
                Vector origin;
                fscanf(file,"%255s",&temps);
                fscanf(file, "%f,%f,%f\n", &new_node->GetOrigin().x,
&new_node->GetOrigin().y, &new_node->GetOrigin().z ); for (int hull
=0;hull<NUM_HULLS;hull++)
                {
                        fscanf(file,"%255s",&temps);
                ...
                //
------------------------------------------------------------------------
                // If in wc_edit mode allocate extra space for nodes that might
be created
                //
------------------------------------------------------------------------ if (
engine->IsInEditMode() )
...
void CAI_NetworkManager::InitializeAINetworks() {
  ...
  g_pAINetworkManager = pNetwork =
      CREATE_ENTITY(CAI_NetworkManager, "ai_network");
...
void CAI_NetworkManager::DelayedInit(void) {
  ...
  if (!g_ai_norebuildgraph.GetInt()) {
    ...
    // --------------------------------------------
    else if (!m_bDontSaveGraph && (!CAI_NetworkManager::NetworksLoaded() ||
                                   engine->IsInEditMode())) {
#ifdef _WIN32
      // --------------------------------------------------------
      ...
      // --------------------------------------------------------
      if (engine->IsInEditMode()) {
        int status = Editor_BeginSession(STRING(gpGlobals->mapname),
                                         gpGlobals->mapversion, false);
...
CAI_Node *CAI_NetworkEditTools::m_pLastDeletedNode =
    NULL;  // For undo in wc edit mode
...
void CAI_NetworkEditTools::OnInit() {
  ...
  // --------------------------------------------
  if (!engine->IsInEditMode()) {
    //		delete[] m_pNodeIndexTable;	// For now only one AI Network
    // called "BigNet" 		m_pNodeIndexTable = NULL;
  }
}
...
int CAI_NetworkEditTools::GetWCIdFromNodeId(int nNodeId) {
  if (nNodeId == -1 || nNodeId >= m_pNetwork->NumNodes()) return -1;
...
CAI_Node *CAI_NetworkEditTools::FindAINodeNearestFacing(const Vector &origin,
                                                        const Vector &facing,
                                                        float threshold,
                                                        int nNodeType) {
  ...
  for (int node = 0; node < aiNet->NumNodes(); node++) {
    if (aiNet->GetNode(node)->GetType() != NODE_DELETED) {
      ...
      if (nNodeType == NODE_ANY ||
        ...
        // Make vector to node
        Vector to_node =
            (aiNet->GetNode(node)->GetPosition(m_iHullDrawNum) - origin);
        ...
        if (dot > bestDot) {
          ...
          trace_t tr;
          AI_TraceLine(origin,
                       aiNet->GetNode(node)->GetPosition(m_iHullDrawNum),
...
CAI_Link *CAI_NetworkEditTools::FindAILinkNearestFacing(const Vector &vOrigin,
      ...
      if ((m_bAirEditMode && aiNet->GetNode(node)->GetType() == NODE_AIR) ||
        ...
        for (int link = 0; link < aiNet->GetNode(node)->NumLinks(); link++) {
          ...
          float fIntersectDist =
              vOriginToNearest.Length() * tan(fAngleToNearest);
          ...
          if (lookDot > bestDot) {
            // Make sure I have a line of sight to it
            trace_t tr;
            AI_TraceLine(vOrigin, vIntersection, MASK_BLOCKLOS, NULL,
...
//			 rebuild and turns of any displays that have been
// invalidated 			 as the network is now out of date
// Input  :
// Output :
//-----------------------------------------------------------------------------
...
void CAI_NetworkEditTools::RecalcUsableNodesForHull(void) {
  ...
  for (int node = 0; node < m_pNetwork->NumNodes(); node++) {
    if ((m_pNetwork->GetNode(node)->m_eNodeInfo &
         (HullToBit((Hull_t)m_iHullDrawNum) << NODE_ENT_FLAGS_SHIFT)) ||
...
void CAI_NetworkEditTools::DrawEditInfoOverlay(void) {
  ...
  Q_snprintf(hullTypeTxt, sizeof(hullTypeTxt), "  %s",
             NAI_Hull::Name(m_iHullDrawNum));
  Q_snprintf(outTxt, sizeof(outTxt), "Displaying:\n%s\n\n", hullTypeTxt);

  if (engine->IsInEditMode()) {
    char outTxt2[255];
    Q_snprintf(nodeTypeTxt, sizeof(nodeTypeTxt), "  %s (l)",
               m_bLinkEditMode ? "Links" : "Nodes");
    Q_snprintf(editTypeTxt, sizeof(editTypeTxt), "  %s (m)",
               m_bAirEditMode ? "Air" : "Ground");
...
void CAI_NetworkEditTools::DrawAINetworkOverlay(void) {
  ...
  if (flDrawDuration < .1) flDrawDuration = .1;
  ...
  if (m_debugNetOverlays & bits_debugOverlayGrid) {
    ...
    if (pPlayer) {
      ...
      trace_t tr;
      AI_TraceLine(vSource, vSource + vForward * 2048, MASK_SOLID, pPlayer,
                   COLLISION_GROUP_NONE, &tr);
    ...
    if (m_debugNetOverlays & bits_debugNeedRebuild) {
      ...
      for (int node = 0; node < m_pNetwork->NumNodes(); node++) {
        if (m_pNetwork->IsConnected(m_iGConnectivityNode, node)) {
          Vector srcPos =
              pAINode[m_iGConnectivityNode]->GetPosition(m_iHullDrawNum);
      ...
      for (int node = startDrawNode; node < endDrawNode; node++) {
        for (int link = 0; link < pAINode[node]->NumLinks(); link++) {
          ...
          if (pAINode[node]->GetLinkByIndex(link)->DestNodeID(node) < node) {
            Vector srcPos =
                pAINode[pAINode[node]->GetLinkByIndex(link)->m_iSrcID]
                    ->GetPosition(m_iHullDrawNum);
            Vector desPos =
                pAINode[pAINode[node]->GetLinkByIndex(link)->m_iDestID]
                    ->GetPosition(m_iHullDrawNum);
            ...
            if (pAINode[node]
                    ->GetLinkByIndex(link)
                    ->m_iAcceptedMoveTypes[m_iHullDrawNum] &
                bits_CAP_MOVE_FLY) {
              NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction,
                                          100, 255, 255, 20, flDrawDuration);
            }
            ...
            if (pAINode[node]
                    ...
                    ->m_iAcceptedMoveTypes[m_iHullDrawNum] &
                bits_CAP_MOVE_CLIMB) {
              // Display as a vertical slice up the climbing surface unless
              // dismount node
              if (pAINode[pAINode[node]->GetLinkByIndex(link)->m_iSrcID]
                      ->GetOrigin() !=
                  pAINode[pAINode[node]->GetLinkByIndex(link)->m_iDestID]
                      ->GetOrigin()) {
            ...
            if (pAINode[node]
              ...
              NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction,
                                          0, 255, 50, 20, flDrawDuration);
            }

            else if (pAINode[node]
                         ->GetLinkByIndex(link)
                         ->m_iAcceptedMoveTypes[m_iHullDrawNum] &
                     bits_CAP_MOVE_JUMP) {
              NDebugOverlay::BoxDirection(srcPos, hullMins, hullMaxs, direction,
                                          0, 0, 255, 20, flDrawDuration);
            }
          }
        }
      }
  ...
  // -------------------------------
  if (m_debugNetOverlays &
      (bits_debugOverlayNodes | bits_debugOverlayConnections)) {
    for (int node = startDrawNode; node < endDrawNode; node++) {
      ...
      if (pAINode[node]->GetType() != NODE_DELETED) {
        ...
        if (m_debugNetOverlays & bits_debugOverlayConnections) {
          ...
          if (m_debugNetOverlays & bits_debugNeedRebuild) {
            ...
            for (int link = 0; link < pAINode[node]->NumLinks(); link++) {
              // Only draw link once
              if (pAINode[node]->GetLinkByIndex(link)->DestNodeID(node) <
                  node) {
                ...
                int moveTypes = pAINode[node]
                                    ->GetLinkByIndex(link)
                                    ->m_iAcceptedMoveTypes[m_iHullDrawNum];

                // when rendering, raise NODE_GROUND off the floor slighty as
                // they seem to clip too much
                ...
                if (linkInfo & bits_LINK_STALE_SUGGESTED) {
                  NDebugOverlay::Line(srcPos, desPos, 255, 0, 0, false,
                                      flDrawDuration);
                }
                // Draw in grey if link turned off
                else if (linkInfo & bits_LINK_OFF) {
                  NDebugOverlay::Line(srcPos, desPos, 100, 100, 100, false,
                                      flDrawDuration);
                } else if ((m_debugNetOverlays &
                            bits_debugOverlayFlyConnections) &&
                           (moveTypes & bits_CAP_MOVE_FLY)) {
                  NDebugOverlay::Line(srcPos, desPos, 100, 255, 255, false,
                                      flDrawDuration);
                } else if (moveTypes & bits_CAP_MOVE_CLIMB) {
                  NDebugOverlay::Line(srcPos, desPos, 255, 0, 255, false,
                                      flDrawDuration);
                } else if (moveTypes & bits_CAP_MOVE_GROUND) {
                  NDebugOverlay::Line(srcPos, desPos, 0, 255, 50, false,
                                      flDrawDuration);
                } else if ((m_debugNetOverlays &
                            bits_debugOverlayJumpConnections) &&
                           (moveTypes & bits_CAP_MOVE_JUMP)) {
                  NDebugOverlay::Line(srcPos, desPos, 0, 0, 255, false,
                                      flDrawDuration);
                } else {  // Dark red if this hull can't use
                  bool isFly = (srcType == NODE_AIR || desType == NODE_AIR);
                  bool isJump = true;
                  for (int i = HULL_HUMAN; i < NUM_HULLS; i++) {
                    if (pAINode[node]
                            ->GetLinkByIndex(link)
                            ->m_iAcceptedMoveTypes[i] &
                        ~bits_CAP_MOVE_JUMP) {
                      isJump = false;
                      break;
                    }
                  }
                  if ((isFly && (m_debugNetOverlays &
                                 bits_debugOverlayFlyConnections)) ||
                      (isJump && (m_debugNetOverlays &
                                  bits_debugOverlayJumpConnections)) ||
                      (!isFly && !isJump)) {
                    NDebugOverlay::Line(srcPos, desPos, 100, 25, 25, false,
                                        flDrawDuration);
                  }
                }
              }
        ...
        if (m_debugNetOverlays & bits_debugOverlayNodes) {
          ...
          NDebugOverlay::Box(nodePos, Vector(-5, -5, -5), Vector(5, 5, 5), r, g,
                             b, 0, flDrawDuration);

          // If climb node draw line in facing direction
          if (pAINode[node]->GetType() == NODE_CLIMB) {
            Vector offsetDir =
                12.0 * Vector(cos(DEG2RAD(pAINode[node]->GetYaw())),
                              sin(DEG2RAD(pAINode[node]->GetYaw())),
                              flDrawDuration);
            NDebugOverlay::Line(nodePos, nodePos + offsetDir, r, g, b, false,
                                flDrawDuration);
          }

          if (pAINode[node]->GetHint()) {
            NDebugOverlay::Box(nodePos, Vector(-7, -7, -7), Vector(7, 7, 7),
                               255, 255, 0, 0, flDrawDuration);
          }
          ...
          if (m_debugNetOverlays & bits_debugOverlayNodesLev2) {
            ...
            if (m_pNodeIndexTable)
              msg.sprintf("%i (wc:%i; z:%i)", node,
                          m_pNodeIndexTable[pAINode[node]->GetId()],
                          pAINode[node]->GetZone());
            else
...
CAI_NetworkEditTools::CAI_NetworkEditTools(
  ...
  for (int i = 0; i < MAX_NODES; i++) m_pNodeIndexTable[i] = NO_NODE;
  m_nNextWCIndex = 0;
...
void CAI_NetworkBuilder::FloodFillZone(CAI_Node **ppNodes, CAI_Node *pNode,
                                       int zone) {
  pNode->SetZone(zone);

  for (int link = 0; link < pNode->NumLinks(); link++) {
    CAI_Link *pLink = pNode->GetLinkByIndex(link);
    CAI_Node *pLinkedNode = (pLink->m_iDestID == pNode->GetId())
                                ? ppNodes[pLink->m_iSrcID]
                                : ppNodes[pLink->m_iDestID];
    if (pLinkedNode->GetZone() == AI_NODE_ZONE_UNKNOWN)
      FloodFillZone(ppNodes, pLinkedNode, zone);

    Assert(pLinkedNode->GetZone() == pNode->GetZone() &&
           pNode->GetZone() == zone);
  }
}
...
void CAI_NetworkBuilder::InitZones(CAI_Network *pNetwork) {
  ...
  // Mark solo nodes
  for (i = 0; i < nNodes; i++) {
    if (ppNodes[i]->NumLinks() == 0) ppNodes[i]->SetZone(AI_NODE_ZONE_SOLO);
  }
    ...
    if (ppNodes[i]->m_eNodeInfo & bits_NODE_WC_CHANGED) {
      ...
      for (int node = 0; node < nNodes; node++) {
        if (ppNodes[node]->GetType() == NODE_AIR) {
          if ((ppNodes[node]->GetOrigin() - vRebuildPos).LengthSqr() <
              MAX_AIR_NODE_LINK_DIST_SQ) {
            ppNodes[node]->SetNeedsRebuild();
            ppNodes[node]->SetZone(AI_NODE_ZONE_UNIVERSAL);
          }
        } else {
          if ((ppNodes[node]->GetOrigin() - vRebuildPos).LengthSqr() <
              MAX_NODE_LINK_DIST_SQ) {
            ppNodes[node]->SetNeedsRebuild();
            ppNodes[node]->SetZone(AI_NODE_ZONE_UNIVERSAL);
          }
        }
...
void CAI_NetworkBuilder::Build(CAI_Network *pNetwork) {
  ...
  CAI_NetworkBuildHelper *pHelper =
      (CAI_NetworkBuildHelper *)CreateEntityByName("ai_network_build_helper");
  ...
  for (i = 0; i < nNodes; i++) {
    InitNodePosition(pNetwork, ppNodes[i]);
    if (pHelper) pHelper->PostInitNodePosition(pNetwork, ppNodes[i]);
  }
  nNodes = pNetwork->NumNodes();  // InitNodePosition can create nodes
  timer.End();
  ...
  if (pHelper) UTIL_Remove(pHelper);
}

//------------------------------------------------------------------------------
// Purpose : Forces testing of a connection between src and dest IDs for all
// dynamic links
//
// Input   :
// Output  :
...
void CAI_NetworkBuilder::ForceDynamicLinkNeighbors(void) {
  ...
  while (pDynamicLink) {
    ...
    // -------------------------------------------------------------
    int nSrcID = g_pAINetworkManager->GetEditOps()->GetNodeIdFromWCId(
    ...
    int nDestID = g_pAINetworkManager->GetEditOps()->GetNodeIdFromWCId(
    ...
    if (nSrcID != -1 && nDestID != -1) {
      if (nSrcID < g_pBigAINet->NumNodes() &&
          nDestID < g_pBigAINet->NumNodes()) {
...
//			set to be just above the floor or at the same level at
// the 			dismount point for the node
//
// Input  :
// Output :
//-----------------------------------------------------------------------------
void CAI_NetworkBuilder::InitClimbNodePosition(CAI_Network *pNetwork,
                                               CAI_Node *pNode) {
  AI_PROFILE_SCOPE(CAI_Node_InitClimbNodePosition);

  // If this is a node for mounting/dismounting the climb skip it
  if (pNode->m_eNodeInfo &
      (bits_NODE_CLIMB_OFF_FORWARD | bits_NODE_CLIMB_OFF_LEFT |
  ...
  AI_TraceHull(posOnLadder, posOnLadder + Vector(0, 0, -37),
               NAI_Hull::Mins(HULL_SMALL_CENTERED),
               NAI_Hull::Maxs(HULL_SMALL_CENTERED), MASK_NPCSOLID_BRUSHONLY,
               NULL, COLLISION_GROUP_NONE, &trace);
  ...
  for (int i = 0; i < 3; i++) {
    ...
    //  Check outward
    // ----------------
    AI_TraceLine(posOnLadder, origin, MASK_NPCSOLID_BRUSHONLY, NULL,
                 COLLISION_GROUP_NONE, &trace);

    // DevMsg( "to %f %f %f : %d %f", origin.x, origin.y, origin.z,
    ...
    if (!trace.startsolid && trace.fraction == 1.0) {
      ...
      if (abs(pNode->GetOrigin().z - floorZ) < 36) {
        CAI_Node *new_node =
            pNetwork->AddNode(pNode->GetOrigin(), pNode->m_flYaw);
        ...
        if (numExits == 0) {
          ...
          for (int hull = 0; hull < NUM_HULLS; hull++) {
            if (fabs(pNode->m_flVOffset[hull] - new_node->m_flVOffset[hull]) >
                1) {
              DevMsg(2,
                     "Warning: Climb Node %i has different exit heights for "
                     "hull %s\n",
...
// Output :
//-----------------------------------------------------------------------------
void CAI_NetworkBuilder::InitGroundNodePosition(CAI_Network *pNetwork,
                                                CAI_Node *pNode) {
  AI_PROFILE_SCOPE(CAI_Node_InitGroundNodePosition);

  if (pNode->m_eNodeInfo & bits_DONT_DROP) return;
  ...
  for (int hull = 0; hull < NUM_HULLS; hull++) {
    ...
    AI_TraceHull(origin, origin + Vector(0, 0, -384), mins, maxs,
...
// Output :
//-----------------------------------------------------------------------------
void CAI_NetworkBuilder::InitNodePosition(CAI_Network *pNetwork,
                                          CAI_Node *pNode) {
  ...
  // Otherwise mark as a land node and drop to the floor
    ...
    if (pNode->m_flVOffset[HULL_SMALL_CENTERED] < -100) {
      ...
      DevWarning(
          "ERROR: Node %.0f %.0f %.0f, WC ID# %i, is either too low (fell "
          "through floor) or too high (>100 units above floor)\n",
  ...
  /*	// If under water, not that the node is in water	<<TODO>>  when
  we get water else if ( UTIL_PointContents(GetOrigin()) & MASK_WATER )
  {
...
// Output :
//-----------------------------------------------------------------------------
void CAI_NetworkBuilder::InitVisibility(CAI_Network *pNetwork,
                                        CAI_Node *pNode) {
  ...
  for (int testnode = 0; testnode < pNetwork->NumNodes(); testnode++) {
    ...
    if (testNode->GetOrigin() == pNode->GetOrigin() &&
        testNode->GetType() != NODE_CLIMB) {
    ...
    float flDistToCheckNode =
        (testNode->GetOrigin() - pNode->GetOrigin()).LengthSqr();

    if (testNode->GetType() == NODE_AIR) {
      if (flDistToCheckNode > MAX_AIR_NODE_LINK_DIST_SQ) continue;
    } else {
      if (flDistToCheckNode > MAX_NODE_LINK_DIST_SQ) continue;
    }
    ...
    // position using the smallest hull to make sure were not in geometry
    Vector destPos =
        pNetwork->GetNode(testnode)->GetPosition(HULL_SMALL_CENTERED);
    ...
    //  Bottom to bottom
    // ------------------
    AI_TraceLine(srcPos, destPos, MASK_NPCWORLDSTATIC, NULL,
                 COLLISION_GROUP_NONE, &tr);
    ...
    // ------------------
    if (!isVisible) {
      AI_TraceLine(srcPos + Vector(0, 0, 70), destPos + Vector(0, 0, 70),
                   MASK_NPCWORLDSTATIC, NULL, COLLISION_GROUP_NONE, &tr);
      if (!tr.startsolid && tr.fraction == 1.0) {
        isVisible = true;
      }
    }
    ...
    // ------------------
    if (!isVisible) {
      AI_TraceLine(srcPos + Vector(0, 0, 70), destPos, MASK_NPCWORLDSTATIC,
    ...
    // ------------------
    if (!isVisible) {
      AI_TraceLine(srcPos, destPos + Vector(0, 0, 70), MASK_NPCWORLDSTATIC,
                   NULL, COLLISION_GROUP_NONE, &tr);
      if (!tr.startsolid && tr.fraction == 1.0) {
        isVisible = true;
      }
    ...
    // trace hit a brush ent, trace backwards to make sure that this ent is the
    only thing in the way. if ( tr.fraction != 1.0 )
    {
            pTraceEnt = tr.u.ent;// store the ent that the trace hit, for
    comparison
    ...
    must record several things about in order to keep
            // track of it in the pathfinding code, as well as through save and
    restore of the node graph. ANY data that is manipulated
            // as part of the process of adding a LINKENT to a connection here
    must also be done in CGraph::SetGraphPointers, where reloaded
    ...
    ) )
            {
                    ...
                    memcpy( pLinkPool [ cTotalLinks ].m_szLinkEntModelname,
    STRING( tr.u.ent->model ), 4 );

                    // set the flag for this ent that indicates that it is
    attached to the world graph
                    // if this ent is removed from the world, it must also be
    removed from the connections
            ...
            // even if the ent wasn't there, these nodes couldn't be connected.
    Skip. else
            {
                    continue;
            }
...
void CAI_NetworkBuilder::InitNeighbors(CAI_Network *pNetwork, CAI_Node *pNode) {
  ...
  for (int checknode = 0; checknode < pNetwork->NumNodes(); checknode++) {
    ...
    for (int testnode = 0; testnode < pNetwork->NumNodes(); testnode++) {
      ...
      // ----------------------------------------------------------
      if ((pCheckNode->GetType() == NODE_AIR &&
           pTestNode->GetType() != NODE_AIR) ||
          (pCheckNode->GetType() != NODE_AIR &&
           pTestNode->GetType() == NODE_AIR)) {
        continue;
      }
      ...
      if (pNode->GetType() == NODE_CLIMB &&
          (pCheckNode->GetType() == NODE_CLIMB ||
           pTestNode->GetType() == NODE_CLIMB)) {
        continue;
      }
      ...
      // ----------------------------------------------------------
      if ((pCheckNode->GetOrigin() == pNode->GetOrigin() &&
           pNode->GetType() == NODE_CLIMB &&
           pCheckNode->GetType() == NODE_CLIMB) ||
          (pTestNode->GetOrigin() == pNode->GetOrigin() &&
           pNode->GetType() == NODE_CLIMB &&
           pTestNode->GetType() == NODE_CLIMB) ||
          (pTestNode->GetOrigin() == pCheckNode->GetOrigin() &&
           pCheckNode->GetType() == NODE_CLIMB &&
           pTestNode->GetType() == NODE_CLIMB)) {
        continue;
      }

      // @HACKHACK (toml 02-25-04): Ignore redundancy if both nodes are air
      // nodes with hint type "strider node". Really, really should do this in a
      // clean manner
      bool nodeIsStrider = (pNode->GetHint() &&
                            pNode->GetHint()->HintType() == HINT_STRIDER_NODE);
      bool other1IsStrider =
          (pCheckNode->GetHint() &&
           pCheckNode->GetHint()->HintType() == HINT_STRIDER_NODE);
      bool other2IsStrider =
          (pTestNode->GetHint() &&
...
static bool IsInLineForClimb(const Vector &srcPos, const Vector &srcFacing,
                             const Vector &destPos, const Vector &destFacing) {
#ifdef DEBUG
  ...
  Assert(VectorsAreEqual(srcFacing, normSrcFacing, 0.01) &&
         VectorsAreEqual(destFacing, normDestFacing, 0.01));
#endif

  // If they are not facing the same way...
  if (1 - srcFacing.Dot(destFacing) > 0.01) return false;

  // If they aren't in line along the facing...
  if (CalcDistanceToLine2D(destPos.AsVector2D(), srcPos.AsVector2D(),
                           srcPos.AsVector2D() + srcFacing.AsVector2D()) > 0.01)
    return false;

  // Check that the angle between them is either staight up, or on at angle of
  // ladder-stairs
  ...
  if (fabsCos > 0.05 && fabs(fabsCos - CosAngLadderStairs) > 0.05) return false;
...
int CAI_NetworkBuilder::ComputeConnection(CAI_Node *pSrcNode,
                                          CAI_Node *pDestNode, Hull_t hull) {
  ...
  // ==============================================================
  // @Note (toml 02-10-03): this should be optimized, caching the results of
  // CanFitAtNode()
  ...
  if (pSrcNode->m_eNodeType == NODE_AIR || pDestNode->GetType() == NODE_AIR) {
    ...
    if (pSrcNode->m_eNodeType == NODE_AIR && pDestNode->GetType() == NODE_AIR) {
      AI_TraceHull(pSrcNode->GetOrigin(), pDestNode->GetOrigin(),
  ...
  // and there is room for the hull to pass between them
  else if ((pSrcNode->m_eNodeType == NODE_CLIMB) &&
           (pDestNode->GetType() == NODE_CLIMB)) {
    ...
    if (pSrcNode->GetOrigin() == pDestNode->GetOrigin()) {
      AI_TraceHull(srcPos, destPos, NAI_Hull::Mins(hull), NAI_Hull::Maxs(hull),
                   MASK_NPCWORLDSTATIC, m_pTestHull, COLLISION_GROUP_NONE, &tr);
      if (!tr.startsolid && tr.fraction == 1.0) {
      ...
      if (!IsInLineForClimb(srcPos, UTIL_YawToVector(pSrcNode->m_flYaw),
                            destPos, UTIL_YawToVector(pDestNode->m_flYaw))) {
        Assert(!IsInLineForClimb(destPos, UTIL_YawToVector(pDestNode->m_flYaw),
                                 srcPos, UTIL_YawToVector(pSrcNode->m_flYaw)));
        DebugConnectMsg(srcId, destId,
  ...
  // > TWO LAND NODES
  // =====================================================
  else if ((pSrcNode->m_eNodeType == NODE_GROUND) ||
           (pDestNode->GetType() == NODE_GROUND)) {
    // BUG: this could use GroundMoveLimit, except there's no version of world
    // but not brushes (doors open, etc).

    // ====================================================
    // > WALKING : walk the space between the nodes
    // =====================================================

    // in this loop we take tiny steps from the current node to the nodes that
    // it links to, one at a time.
    ...
    if (!m_pTestHull->GetMoveProbe()->CheckStandPosition(srcPos,
                                                         MASK_NPCWORLDSTATIC)) {
      DebugConnectMsg(srcId, destId, "      Failed to stand at %d\n", srcId);
      fStandFailed = true;
    }

    if (!m_pTestHull->GetMoveProbe()->CheckStandPosition(destPos,
                                                         MASK_NPCWORLDSTATIC)) {
      DebugConnectMsg(srcId, destId, "      Failed to stand at %d\n", destId);
      fStandFailed = true;
    }
    ...
    // srcPos.z, destPos.x, destPos.y, destPos.z );

    if (!fStandFailed) {
      fWalkFailed = !m_pTestHull->GetMoveProbe()->TestGroundMove(
          srcPos, destPos, MASK_NPCWORLDSTATIC, AITGM_IGNORE_INITIAL_STAND_POS,
          NULL);
      if (fWalkFailed)
    ...
    // =============================================================================
    if (!fStandFailed && fWalkFailed &&
        (pSrcNode->m_eNodeType == NODE_GROUND) &&
        (pDestNode->GetType() == NODE_GROUND)) {
      ...
      // Jumps aren't bi-directional.  We can jump down further than we can jump
      // up so we have to test for either one
      ...
      if (canDestJump || canSrcJump) {
        ...
        pMoveProbe->MoveLimit(NAV_JUMP, srcPos, destPos, MASK_NPCWORLDSTATIC,
                              NULL, &moveTrace);
        if (!IsMoveBlocked(moveTrace)) {
          fJumpLegal = true;
        }
        pMoveProbe->MoveLimit(NAV_JUMP, destPos, srcPos, MASK_NPCWORLDSTATIC,
                              NULL, &moveTrace);
        if (!IsMoveBlocked(moveTrace)) {
          fJumpLegal = true;
        }
...
void CAI_NetworkBuilder::InitLinks(CAI_Network *pNetwork, CAI_Node *pNode) {
  ...
  for (int i = 0; i < pNetwork->NumNodes(); i++) {
    ...
    if (m_NeighborsTable[pNode->m_iID].IsBitSet(pDestNode->m_iID)) {
      ...
      if (!(pNode->m_eNodeInfo & bits_NODE_FALLEN) &&
          !(pDestNode->m_eNodeInfo & bits_NODE_FALLEN)) {
        for (int hull = 0; hull < NUM_HULLS; hull++) {
          DebugConnectMsg(pNode->m_iID, i, "   Testing for hull %s\n",
                          NAI_Hull::Name((Hull_t)hull));

          acceptedMotions[hull] =
              ComputeConnection(pNode, pDestNode, (Hull_t)hull);
          if (acceptedMotions[hull] != 0) bAllFailed = false;
        }
      } else
        DebugConnectMsg(pNode->m_iID, i,
