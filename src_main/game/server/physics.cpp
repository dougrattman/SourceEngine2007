...
void PortalPhysFrame(float deltaTime);  // small wrapper for PhysFrame that
                                        // simulates all 3 environments at once
#endif
...
ConVar phys_timescale("phys_timescale", "1", 0, "Scale time for physics",
                      TimescaleChanged);

#if _DEBUG
ConVar phys_dontprintint("phys_dontprintint", "1", FCVAR_NONE,
                         "Don't print inter-penetration warnings.");
#endif
...
class CPhysicsHook : public CBaseGameSystemPerFrame {
 public:
  ...
  bool FindOrAddVehicleScript(const char *pScriptName,
                              vehicleparams_t *pVehicle,
                              vehiclesounds_t *pSounds);
  void FlushVehicleScripts() { m_vehicleScripts.RemoveAll(); }

  bool ShouldSimulate() { return (physenv && !m_bPaused) ? true : false; }
...
// Singleton access
//-----------------------------------------------------------------------------
IGameSystem *PhysicsGameSystem() { return &g_PhysicsHook; }
...
bool CPhysicsHook::Init(void) {
  ...
  if (!factories.physicsFactory) return false;

  if ((physics = (IPhysics *)factories.physicsFactory(
           VPHYSICS_INTERFACE_VERSION, NULL)) == NULL ||
      (physcollision = (IPhysicsCollision *)factories.physicsFactory(
           VPHYSICS_COLLISION_INTERFACE_VERSION, NULL)) == NULL ||
      (physprops = (IPhysicsSurfaceProps *)factories.physicsFactory(
           VPHYSICS_SURFACEPROPS_INTERFACE_VERSION, NULL)) == NULL)
    return false;
...
void CPhysicsHook::LevelInitPreEntity() {
  ...
  physenv->EnableConstraintNotify(true);  // callback when an object gets
                                          // deleted that is attached to a
                                          // constraint
...
void CPhysicsHook::LevelInitPostEntity() { m_bPaused = false; }
...
bool CPhysicsHook::FindOrAddVehicleScript(const char *pScriptName,
                                          vehicleparams_t *pVehicle,
                                          vehiclesounds_t *pSounds) {
  ...
  if (index < 0) {
    ...
    if (pFile) {
      ...
      IVPhysicsKeyParser *pParse =
          physcollision->VPhysicsKeyParserCreate((char *)pFile);
  ...
  if (index >= 0) {
    ...
    if (pSounds) {
      ...
      if (bLoadedSounds == false) return false;
...
void CPhysicsHook::FrameUpdatePostEntityThink() {
  VPROF_BUDGET("CPhysicsHook::FrameUpdatePostEntityThink",
               VPROF_BUDGETGROUP_PHYSICS);

  // Tracker 24846:  If game is paused, don't simulate vphysics
  float interval = (gpGlobals->frametime > 0.0f) ? TICK_INTERVAL : 0.0f;

  // update the physics simulation, not we don't use gpGlobals->frametime, since
  // that can be 30 msec or 15 msec depending on whether
  // IsSimulatingOnAlternateTicks is true or not
...
bool PhysIsFinalTick() { return g_PhysicsHook.m_isFinalTick; }

IPhysicsObject *PhysCreateWorld(CBaseEntity *pWorld) {
  staticpropmgr->CreateVPhysicsRepresentations(physenv, &g_SolidSetup, pWorld);
  return PhysCreateWorld_Shared(pWorld, modelinfo->GetVCollide(1),
                                g_PhysDefaultObjectParams);
}

// vehicle wheels can only collide with things that can't get stuck in them
// during game physics because they aren't in the game physics world at present
static bool WheelCollidesWith(IPhysicsObject *pObj, CBaseEntity *pEntity) {
#if defined(INVASION_DLL)
  if (pEntity->GetCollisionGroup() == TFCOLLISION_GROUP_OBJECT) return false;
#endif
  ...
  if (pEntity->GetMoveType() == MOVETYPE_PUSH ||
      pEntity->GetMoveType() == MOVETYPE_VPHYSICS || pObj->IsStatic())
    return true;
...
int CCollisionEvent::ShouldCollide(IPhysicsObject *pObj0, IPhysicsObject *pObj1,
...
#endif
{
  ...
  if (pEntity0 == pEntity1) {
    ...
    IPhysicsCollisionSet *pSet =
        physics->FindCollisionSet(pEntity0->GetModelIndex());
    if (pSet)
  ...
  // objects that are both constrained to the world don't collide with each
  // other
  ...
  if (pObj0->GetCallbackFlags() & CALLBACK_IS_VEHICLE_WHEEL) {
    if (!WheelCollidesWith(pObj1, pEntity1)) return false;
  }
  if (pObj1->GetCallbackFlags() & CALLBACK_IS_VEHICLE_WHEEL) {
    if (!WheelCollidesWith(pObj0, pEntity0)) return false;
  }

  if (pEntity0->ForceVPhysicsCollide(pEntity1) ||
      pEntity1->ForceVPhysicsCollide(pEntity0))
    return 1;

  if (pEntity0->edict() && pEntity1->edict()) {
    // don't collide with your owner
    if (pEntity0->GetOwnerEntity() == pEntity1 ||
        pEntity1->GetOwnerEntity() == pEntity0)
      return 0;
  }
  ...
  if (pEntity0->GetMoveParent() || pEntity1->GetMoveParent()) {
    ...
    // NOTE: Don't let siblings/parents collide.  If you want this behavior, do
    // it with constraints, not hierarchy!
    if (pParent0 == pParent1) return 0;
    ...
    if (p0 && p1) {
      if (g_EntityCollisionHash->IsObjectPairInHash(p0, p1)) return 0;
    }
  }
  ...
  if (pEntity0->GetMoveParent()) {
    // if the object & its parent are both MOVETYPE_VPHYSICS, then this must be
    // a special case like a prop_ragdoll_attached
    if (!(movetype0 == MOVETYPE_VPHYSICS &&
          pEntity0->GetRootMoveParent()->GetMoveType() == MOVETYPE_VPHYSICS)) {
      aiMove0 = true;
    }
  }
  if (pEntity1->GetMoveParent()) {
    // if the object & its parent are both MOVETYPE_VPHYSICS, then this must be
    // a special case.
    if (!(movetype1 == MOVETYPE_VPHYSICS &&
          pEntity1->GetRootMoveParent()->GetMoveType() == MOVETYPE_VPHYSICS)) {
      aiMove1 = true;
    }
  }

  // AI movers don't collide with the world/static/pinned objects or other AI
  // movers
  if ((aiMove0 && !pObj1->IsMoveable()) || (aiMove1 && !pObj0->IsMoveable()) ||
      (aiMove0 && aiMove1))
    return 0;

  // two objects under shadow control should not collide.  The AI will figure it
  // out
  if (pObj0->GetShadowController() && pObj1->GetShadowController()) return 0;
  ...
  if ((solid0 == SOLID_NONE) || (solid1 == SOLID_NONE)) return 0;
  ...
  if ((nSolidFlags0 | nSolidFlags1) & FSOLID_NOT_SOLID) {
    ...
    if (pObj0->IsTrigger() && !(nSolidFlags1 & FSOLID_NOT_SOLID)) return 1;
    if (pObj1->IsTrigger() && !(nSolidFlags0 & FSOLID_NOT_SOLID)) return 1;

    return 0;
  }

  if ((nSolidFlags0 & FSOLID_TRIGGER) &&
      !(solid1 == SOLID_VPHYSICS || solid1 == SOLID_BSP ||
        movetype1 == MOVETYPE_VPHYSICS))
    return 0;

  if ((nSolidFlags1 & FSOLID_TRIGGER) &&
      !(solid0 == SOLID_VPHYSICS || solid0 == SOLID_BSP ||
        movetype0 == MOVETYPE_VPHYSICS))
    return 0;

  if (!g_pGameRules->ShouldCollide(pEntity0->GetCollisionGroup(),
                                   pEntity1->GetCollisionGroup()))
    return 0;
...
bool FindMaxContact(IPhysicsObject *pObject, float minForce,
                    IPhysicsObject **pOtherObject, Vector *contactPos,
                    Vector *pForce) {
...
bool CCollisionEvent::ShouldFreezeObject(IPhysicsObject *pObject) {
  ...
  // NOTE: If this becomes a problem (too many collision checks this tick) we
  // should add a path to inform the logic in VPhysicsUpdatePusher() about the
  // limit being applied so that it doesn't falsely block the object when it's
  // simply been temporarily frozen for performance reasons
  CBaseEntity *pEntity = static_cast<CBaseEntity *>(pObject->GetGameData());
  if (pEntity) {
    if (pEntity->GetMoveType() == MOVETYPE_PUSH) return false;

    // don't limit vehicle collisions either, limit can make breaking through a
    // pile of breakable props very hitchy
    if (pEntity->GetServerVehicle() &&
        !(pObject->GetCallbackFlags() & CALLBACK_IS_VEHICLE_WHEEL))
      return false;
  }

  // if we're freezing a debris object, then it's probably due to some kind of
  // solver issue usually this is a large object resting on the debris object in
  // question which is not very stable. After doing the experiment of
  // constraining the dynamic range of mass while solving friction contacts, I
  // like the results of this tradeoff better.  So damage or remove the debris
  // object wherever possible once we hit this case:
  if (IsDebris(pEntity->GetCollisionGroup()) && !pEntity->IsNPC()) {
    ...
    if (FindMaxContact(pObject, pObject->GetMass() * 10, &pOtherObject,
                       &contactPos, &force)) {
      CBaseEntity *pOther =
          static_cast<CBaseEntity *>(pOtherObject->GetGameData());
      // this object can take damage, crush it
      if (pEntity->m_takedamage > DAMAGE_EVENTS_ONLY) {
        CTakeDamageInfo dmgInfo(pOther, pOther, force, contactPos,
                                force.Length() * 0.1f, DMG_CRUSH);
        PhysCallbackDamage(pEntity, dmgInfo);
      } else {
        // can't be damaged, so do something else:
        if (PropIsGib(pEntity)) {
          // it's always safe to delete gibs, so kill this one to avoid
          // simulation problems
          PhysCallbackRemove(pEntity->NetworkProp());
        } else {
          // not a gib, create a solver:
          // UNDONE: Add a property to override this in gameplay critical
          // scenarios?
          g_PostSimulationQueue.QueueCall(EntityPhysics_CreateSolver, pOther,
                                          pEntity, true, 1.0f);
        }
      }
    }
...
bool CCollisionEvent::ShouldFreezeContacts(IPhysicsObject **pObjectList,
                                           int objectCount) {
  if (m_lastTickFrictionError > gpGlobals->tickcount ||
      m_lastTickFrictionError < (gpGlobals->tickcount - 1)) {
    DevWarning("Performance Warning: large friction system (%d objects)!!!\n",
               objectCount);
#if _DEBUG
    for (int i = 0; i < objectCount; i++) {
      CBaseEntity *pEntity =
          static_cast<CBaseEntity *>(pObjectList[i]->GetGameData());
...
bool PhysShouldCollide(IPhysicsObject *pObj0, IPhysicsObject *pObj1) {
  ...
  if (!pGameData0 || !pGameData1) return false;
...
static void ReportPenetration(CBaseEntity *pEntity, float duration) {
  if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
    ...
    pEntity->AddTimedOverlay(UTIL_VarArgs("VPhysics Penetration Error (%s)!",
...
static void UpdateEntityPenetrationFlag(CBaseEntity *pEntity,
                                        bool isPenetrating) {
  if (!pEntity) return;
...
void CCollisionEvent::GetListOfPenetratingEntities(
    CBaseEntity *pSearch, CUtlVector<CBaseEntity *> &list) {
  for (int i = m_penetrateEvents.Count() - 1; i >= 0; --i) {
    if (m_penetrateEvents[i].hEntity0 == pSearch &&
        m_penetrateEvents[i].hEntity1.Get() != NULL) {
      list.AddToTail(m_penetrateEvents[i].hEntity1);
    } else if (m_penetrateEvents[i].hEntity1 == pSearch &&
               m_penetrateEvents[i].hEntity0.Get() != NULL) {
...
void CCollisionEvent::UpdatePenetrateEvents(void) {
  for (int i = m_penetrateEvents.Count() - 1; i >= 0; --i) {
    ...
    } else if (m_penetrateEvents[i].collisionState ==
               COLLSTATE_TRYENTITYSOLVER) {
      if (pEntity0 && pEntity1) {
        if (!IsDebris(pEntity1->GetCollisionGroup()) ||
            pEntity1->GetMoveType() != MOVETYPE_VPHYSICS) {
...
penetrateevent_t &CCollisionEvent::FindOrAddPenetrateEvent(
    CBaseEntity *pEntity0, CBaseEntity *pEntity1) {
  int index = -1;
  for (int i = m_penetrateEvents.Count() - 1; i >= 0; --i) {
    if (m_penetrateEvents[i].hEntity0.Get() == pEntity0 &&
        m_penetrateEvents[i].hEntity1.Get() == pEntity1) {
      index = i;
      break;
    }
  }
...
static ConVar phys_penetration_error_time(
    "phys_penetration_error_time", "10", 0,
    "Controls the duration of vphysics penetration error boxes.");

static bool CanResolvePenetrationWithNPC(CBaseEntity *pEntity,
                                         IPhysicsObject *pObject) {
  if (pEntity->GetMoveType() == MOVETYPE_VPHYSICS) {
    // hinged objects won't be able to be pushed out anyway, so don't try the
    // npc solver
    if (!pObject->IsHinged() && !pObject->IsAttachedToConstraint(true)) {
      if (pObject->IsMoveable() || pEntity->GetServerVehicle()) return true;
    }
  }
  return false;
}

int CCollisionEvent::ShouldSolvePenetration(IPhysicsObject *pObj0,
  ...
  // this can get called as entities are being constructed on the other side of
  // a game load or level transition Some entities may not be fully constructed,
  // so don't call into their code until the level is running
  if (g_PhysicsHook.m_bPaused) return true;

  // solve it yourself here and return 0, or have the default implementation do
  // it
  if (pEntity0 > pEntity1) {
    // swap sort
    CBaseEntity *pTmp = pEntity0;
  ...
  if (pEntity0 == pEntity1) {
    if (pObj0->GetGameFlags() & FVPHYSICS_PART_OF_RAGDOLL) {
      DevMsg(2, "Solving ragdoll self penetration! %s (%s) (%d v %d)\n",
             pObj0->GetName(), pEntity0->GetDebugName(), pObj0->GetGameIndex(),
             pObj1->GetGameIndex());
  ...
  if ((pEntity0->MyNPCPointer() &&
       CanResolvePenetrationWithNPC(pEntity1, pObj1)) ||
      (pEntity1->MyNPCPointer() &&
       CanResolvePenetrationWithNPC(pEntity0, pObj0))) {
    event.collisionState = COLLSTATE_TRYNPCSOLVER;
  }

  if ((IsDebris(pEntity0->GetCollisionGroup()) && !pObj1->IsStatic()) ||
      (IsDebris(pEntity1->GetCollisionGroup()) && !pObj0->IsStatic())) {
    if (eventTime > 0.5f) {
  ...
  if (phys_dontprintint.GetBool() == false) {
    ...
    if (pEntity0 == pEntity1) {
      ...
      DevMsg(1, "***Inter-penetration on %s (%d & %d) (%.0f, %.0f)\n",
             pName1 ? pName1 : "(0)", index0, index1, gpGlobals->curtime,
             eventTime);
    } else {
      DevMsg(1, "***Inter-penetration between %s(%s) AND %s(%s) (%.0f, %.0f)\n",
             pName1 ? pName1 : "(0)", pEntity0->GetDebugName(),
             pName2 ? pName2 : "(0)", pEntity1->GetDebugName(),
             gpGlobals->curtime, eventTime);
    }
  }
#endif

  if (eventTime > 3) {
    // don't report penetrations on ragdolls with themselves, or outside of
    // developer mode
    ...
    if (!pEntity0->IsPlayer() && !pEntity1->IsPlayer() &&
        !pObj0->GetShadowController() && !pObj1->GetShadowController()) {
      // two objects have been stuck for more than 3 seconds, try disabling
      // simulation
...
void CCollisionEvent::FluidStartTouch(IPhysicsObject *pObject,
                                      IPhysicsFluidController *pFluid) {
  CallbackContext check(this);
  if ((pObject == NULL) || (pFluid == NULL)) return;

  CBaseEntity *pEntity = static_cast<CBaseEntity *>(pObject->GetGameData());
  if (!pEntity) return;

  pEntity->AddEFlags(EFL_TOUCHING_FLUID);
  pEntity->OnEntityEvent(ENTITY_EVENT_WATER_TOUCH,
                         (void *)pFluid->GetContents());

  float timeSinceLastCollision = DeltaTimeSinceLastFluid(pEntity);
  if (timeSinceLastCollision < 0.5f) return;

  // UNDONE: Use this for splash logic instead?
  // UNDONE: Use angular term too - push splashes in rotAxs cross normal
  // direction?
  Vector normal;
  float dist;
  ...
  // surface and scale angScale by that For now assume 25%
...
void CCollisionEvent::FluidEndTouch(IPhysicsObject *pObject,
  ...
  pEntity->OnEntityEvent(ENTITY_EVENT_WATER_UNTOUCH,
                         (void *)pFluid->GetContents());
}
...
void PhysSolidOverride(solid_t &solid, string_t overrideScript) {
  if (overrideScript != NULL_STRING) {
    ...
    // parse that sucker
    IVPhysicsKeyParser *pParse =
        physcollision->VPhysicsKeyParserCreate(pTmpString);
    CSkipKeys tmp;
  ...
  if (index >= 0) {
    ...
    switch (override.alignType) {
      case masscenteroverride_t::ALIGN_POINT:
        VectorITransform(massCenterWS, pEntity->EntityToWorldTransform(),
                         solidOut.massCenterOverride);
        break;
      case masscenteroverride_t::ALIGN_AXIS: {
        Vector massCenterLocal, defaultMassCenterWS;
        physcollision->CollideGetMassCenter(pCollide->solids[solidOut.index],
                                            &massCenterLocal);
        VectorTransform(massCenterLocal, pEntity->EntityToWorldTransform(),
                        defaultMassCenterWS);
        massCenterWS +=
            override.axis * (DotProduct(defaultMassCenterWS, override.axis) -
                             DotProduct(override.axis, override.center));
        VectorITransform(massCenterWS, pEntity->EntityToWorldTransform(),
                         solidOut.massCenterOverride);
      } break;
    }
...
void IterateActivePhysicsEntities(EntityCallbackFunction func) {
  ...
  if (activeCount) {
    pActiveList =
        (IPhysicsObject **)stackalloc(sizeof(IPhysicsObject *) * activeCount);
    physenv->GetActiveObjects(pActiveList);
    for (int i = 0; i < activeCount; i++) {
      CBaseEntity *pEntity =
          reinterpret_cast<CBaseEntity *>(pActiveList[i]->GetGameData());
      if (pEntity) {
        func(pEntity);
      }
    }
...
CON_COMMAND(physics_highlight_active,
...
CON_COMMAND_F(surfaceprop, "Reports the surface properties at the cursor",
              FCVAR_CHEAT) {
  ...
  UTIL_TraceLine(pPlayer->EyePosition(),
                 ...
                 MASK_SHOT_HULL | CONTENTS_GRATE | CONTENTS_DEBRIS, pPlayer,
  ...
  if (tr.DidHit()) {
    ...
    if (tr.DidHitWorld() && tr.hitbox > 0) {
      ICollideable *pCollide =
          staticpropmgr->GetStaticPropByIndex(tr.hitbox - 1);
    ...
    if (pModel) {
      modelStuff.sprintf(
          "%s.%s ", modelinfo->IsTranslucent(pModel) ? "Translucent" : "Opaque",
    ...
    Msg("Hit surface \"%s\" (entity %s, model \"%s\" %s), texture \"%s\"\n",
        physprops->GetPropName(tr.surface.surfaceProps),
        tr.m_pEnt->GetClassname(), pModelName, modelStuff.Access(),
        tr.surface.name);
...
class CConstraintFloodList {
 public:
  ...
  bool IsWorldEntity(CBaseEntity *pEnt) {
    if (pEnt->edict()) return pEnt->IsWorld();
    return false;
  }
  ...
  void BuildGraphFromEntity(CBaseEntity *pEntity,
                            CUtlVector<CBaseEntity *> &constraintList) {
    int listIndex = m_list.Find(pEntity);
    if (listIndex != m_list.InvalidIndex()) {
      int entryIndex = m_list.Element(listIndex);
      CConstraintFloodEntry &entry = m_entryList.Element(entryIndex);
...
// traverses the graph of attachments (currently supports springs & constraints)
// starting at an entity Then turns on debug info for each link in the graph
// (springs/constraints are links)
static void DebugConstraints(CBaseEntity *pEntity) {
  extern bool GetSpringAttachments(CBaseEntity * pEntity,
                                   CBaseEntity * pAttach[2],
                                   IPhysicsObject * pAttachVPhysics[2]);
  extern bool GetConstraintAttachments(CBaseEntity * pEntity,
                                       CBaseEntity * pAttach[2],
                                       IPhysicsObject * pAttachVPhysics[2]);
  ...
  for (CBaseEntity *pList = gEntList.FirstEnt(); pList != NULL;
       pList = gEntList.NextEnt(pList)) {
    if (GetConstraintAttachments(pList, pAttach, pAttachVPhysics) ||
        GetSpringAttachments(pList, pAttach, pAttachVPhysics)) {
  ...
  for (int i = 0; i < constraints.Count(); i++) {
    ...
    Msg("**********************\n%s connects %s(%s:%d) to %s(%s:%d)\n",
        constraints[i]->GetClassname(), pName0, pModel0, index0, pName1,
        pModel1, index1);
...
CON_COMMAND(physics_constraints,
...
CON_COMMAND(physics_budget, "Times the cost of each active object") {
  ...
  if (activeCount) {
    ...
    // NOTE: This is just a heuristic.  Attempt to estimate cost by putting each
    // object to sleep in turn.
    //	note that simulation may wake the objects again and some costs scale
    // with sets of objects/constraints/etc 	so these are only generally
    // useful for broad questions, not real metrics!
    ...
    for (i = 0; i < ents.Count(); i++) {
      ...
      Msg("%s (%s): %.3fms (%.3f%%) @ %s\n", ents[i]->GetClassname(),
          ents[i]->GetDebugName(), fraction * totalTime * 1000.0f,
          fraction * 100.0f, VecToString(ents[i]->GetAbsOrigin()));
    }
...
void PortalPhysFrame(float deltaTime)  // small wrapper for PhysFrame that
                                       // simulates all environments at once
{
  CPortalSimulator::PrePhysFrame();

  if (sv_fullsyncclones.GetBool()) CPhysicsShadowClone::FullSyncAllClones();
...
void PhysFrame(float deltaTime) {
  ...
  if (!g_PhysicsHook.ShouldSimulate()) return;
  ...
  // !!!HACKHACK -- hard limit scaled time to avoid spending too much time in
  // here Limit to 100 ms
  if (deltaTime > 0.100f) deltaTime = 0.100f;
...
#ifndef PORTAL  // instead of wrapping 1 simulation with this, portal needs to
                // wrap 3
  g_Collisions.BufferTouchEvents(true);
#endif
  ...
  if (bProfile) {
    ...
    if (simRealTime < 0) simRealTime = 0;
    ...
    if (lastObjectCount != 0 || activeCount != 0) {
      Msg("Physics: %3d objects, %4.1fms / AVG: %4.1fms\n", activeCount,
          simRealTime * 1000, g_PhysAverageSimTime * 1000);
    }
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (pObject) {
      if (pObject->GetGameFlags() & FVPHYSICS_PLAYER_HELD) {
        CBaseEntity *pOtherEntity = reinterpret_cast<CBaseEntity *>(
            pEvent->pObjects[!i]->GetGameData());
        if (pOtherEntity && !pOtherEntity->IsPlayer()) {
          Vector velocity;
          AngularImpulse angVel;
          // HACKHACK: If we totally clear this out, then Havok will think the
          // objects are penetrating and generate forces to separate them so
      ...
      pObject->GetVelocity(&m_gameEvent.preVelocity[i],
                           &m_gameEvent.preAngularVelocity[i]);
    }
  }
}
...
void CCollisionEvent::PostCollision(vcollisionevent_t *pEvent) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (pObject) {
      CBaseEntity *pEntity =
          reinterpret_cast<CBaseEntity *>(pObject->GetGameData());
      if (!pEntity) return;

      // UNDONE: This is here to trap crashes due to NULLing out the game data
      // on delete
...
void PhysForceEntityToSleep(CBaseEntity *pEntity, IPhysicsObject *pObject) {
  ...
  if (!pObject || !pObject->IsMoveable()) return;
...
void CCollisionEvent::Friction(IPhysicsObject *pObject, float energy,
...
void CCollisionEvent::FlushQueuedOperations() {
  int loopCount = 0;
  while (loopCount < 20) {
    int count = m_triggerEvents.Count() + m_touchEvents.Count() +
                m_damageEvents.Count() + m_removeObjects.Count() +
                g_PostSimulationQueue.Count();
    if (!count) break;
...
void CCollisionEvent::FrameUpdate(void) {
  ...
  UpdateFluidEvents();
  UpdateDamageEvents();  // if there was no PSI in physics, we'll still need to
                         // do some of these because collisions are solved in
                         // between PSIs
  g_PostSimulationQueue.CallQueued();
  UpdateRemoveObjects();

  // There are some queued operations that must complete each frame, iterate
...
void CCollisionEvent::DispatchStartTouch(CBaseEntity *pEntity0,
  ...
  // NOTE: This sets up the touch list for both entities, no call to pEntity1 is
  // needed
  pEntity0->PhysicsMarkEntitiesAsTouchingEventDriven(pEntity1, trace);
}

void CCollisionEvent::DispatchEndTouch(CBaseEntity *pEntity0,
                                       CBaseEntity *pEntity1) {
...
void CCollisionEvent::UpdateTouchEvents(void) {
  ...
  for (i = 0; i < m_touchEvents.Count(); i++) {
    ...
    if (event.touchType == TOUCH_START) {
      DispatchStartTouch(event.pEntity0, event.pEntity1, event.endPoint,
                         event.normal);
    } else {
      // TOUCH_END
  ...
  for (i = 0; i < m_triggerEvents.Count(); i++) {
    ...
    if (m_currentTriggerEvent.bStart) {
      m_currentTriggerEvent.pTriggerEntity->StartTouch(
          m_currentTriggerEvent.pEntity);
    } else {
      m_currentTriggerEvent.pTriggerEntity->EndTouch(
          m_currentTriggerEvent.pEntity);
    }
  }
...
void CCollisionEvent::UpdateDamageEvents(void) {
  for (int i = 0; i < m_damageEvents.Count(); i++) {
    ...
    iEntBits |=
        (event.pEntity->GetSolidFlags() & FSOLID_NOT_SOLID) ? 0x0004 : 0;
#if 0
    ...
    if (event.bRestoreVelocity && iEntBits != iEntBits2) {
      // UNDONE: Use ratio of masses to blend in a little of the collision
      // response? UNDONE: Damage for future events is already computed - it
      // would be nice to
      //			go back and recompute it now that the values
      // have 			been adjusted
...
void CCollisionEvent::RestoreDamageInflictorState(int inflictorStateIndex,
                                                  float velocityBlend) {
  ...
  if (velocityBlend > 0) {
    ...
    state.savedVelocity =
        state.savedVelocity * velocityBlend + velocity * (1 - velocityBlend);
    state.savedAngularVelocity = state.savedAngularVelocity * velocityBlend +
                                 angVel * (1 - velocityBlend);
    state.pInflictorPhysics->SetVelocity(&state.savedVelocity,
                                         &state.savedAngularVelocity);
  }
...
void CCollisionEvent::RestoreDamageInflictorState(IPhysicsObject *pInflictor) {
  ...
  if (index >= 0) {
    ...
    if (!state.restored) {
      ...
      if (inflictorMass < VPHYSICS_LARGE_OBJECT_MASS &&
...
bool CCollisionEvent::GetInflictorVelocity(IPhysicsObject *pInflictor,
                                           Vector &velocity,
...
void CCollisionEvent::AddTouchEvent(CBaseEntity *pEntity0,
                                    CBaseEntity *pEntity1, int touchType,
                                    const Vector &point, const Vector &normal) {
  if (!pEntity0 || !pEntity1) return;
...
void CCollisionEvent::AddDamageEvent(CBaseEntity *pEntity,
                                     const CTakeDamageInfo &info,
                                     IPhysicsObject *pInflictorPhysics,
                                     bool bRestoreVelocity,
                                     const Vector &savedVel,
                                     const AngularImpulse &savedAngVel) {
  if (pEntity->IsMarkedForDeletion()) return;

  int iTimeBasedDamage = g_pGameRules->Damage_GetTimeBased();
  if (!(info.GetDamageType() & (DMG_BURN | DMG_DROWN | iTimeBasedDamage |
                                DMG_PREVENT_PHYSICS_FORCE))) {
    Assert(info.GetDamageForce() != vec3_origin &&
           info.GetDamagePosition() != vec3_origin);
  }
  ...
  if (event.bRestoreVelocity) {
    ...
    if (inflictorIndex >= 0) {
      ...
      AddDamageInflictor(pInflictorPhysics, otherMass, savedVel, savedAngVel,
                         true);
    }
  }
}
...
// Impulse events
//-----------------------------------------------------------------------------
static void PostSimulation_ImpulseEvent(IPhysicsObject *pObject,
                                        const Vector &centerForce,
                                        const AngularImpulse &centerTorque) {
  pObject->ApplyForceCenter(centerForce);
  pObject->ApplyTorqueCenter(centerTorque);
}

void PostSimulation_SetVelocityEvent(IPhysicsObject *pPhysicsObject,
...
int CCollisionEvent::FindDamageInflictor(IPhysicsObject *pInflictorPhysics) {
  ...
  for (int i = m_damageInflictors.Count() - 1; i >= 0; --i) {
    ...
    if (state.pInflictorPhysics == pInflictorPhysics) return i;
  }

  return -1;
}

int CCollisionEvent::AddDamageInflictor(IPhysicsObject *pInflictorPhysics,
                                        float otherMass, const Vector &savedVel,
                                        const AngularImpulse &savedAngVel,
                                        bool addList) {
  ...
  // NOTE: we can't save a delta because there could be subsequent post-fatal
  // collisions
      ...
      if (physCount > 1) {
        ...
        for (int i = 0; i < physCount; i++) {
          if (pList[i] != pInflictorPhysics) {
            ...
            int next =
                AddDamageInflictor(pList[i], otherMass, vel, angVel, false);
...
void CCollisionEvent::StartTouch(IPhysicsObject *pObject1,
...
static int CountPhysicsObjectEntityContacts(IPhysicsObject *pObject,
                                            CBaseEntity *pEntity) {
  IPhysicsFrictionSnapshot *pSnapshot = pObject->CreateFrictionSnapshot();
  int count = 0;
  while (pSnapshot->IsValid()) {
...
void CCollisionEvent::EndTouch(IPhysicsObject *pObject1,
                               IPhysicsObject *pObject2,
                               IPhysicsCollisionData *pTouchData) {
  CallbackContext check(this);
  CBaseEntity *pEntity1 = static_cast<CBaseEntity *>(pObject1->GetGameData());
...
void CCollisionEvent::ObjectEnterTrigger(IPhysicsObject *pTrigger,
                                         IPhysicsObject *pObject) {
  CBaseEntity *pTriggerEntity =
      static_cast<CBaseEntity *>(pTrigger->GetGameData());
  CBaseEntity *pEntity = static_cast<CBaseEntity *>(pObject->GetGameData());
  if (pTriggerEntity && pEntity) {
    // UNDONE: Don't buffer these until we can solve generating touches at
    // object creation time
    if (0 && m_bBufferTouchEvents) {
      int index = m_triggerEvents.AddToTail();
      m_triggerEvents[index].Init(pTriggerEntity, pTrigger, pEntity, pObject,
                                  true);
    } else {
      CallbackContext check(this);
      m_currentTriggerEvent.Init(pTriggerEntity, pTrigger, pEntity, pObject,
                                 true);
...
void CCollisionEvent::ObjectLeaveTrigger(IPhysicsObject *pTrigger,
                                         IPhysicsObject *pObject) {
  CBaseEntity *pTriggerEntity =
      static_cast<CBaseEntity *>(pTrigger->GetGameData());
  CBaseEntity *pEntity = static_cast<CBaseEntity *>(pObject->GetGameData());
...
bool CCollisionEvent::GetTriggerEvent(triggerevent_t *pEvent,
                                      CBaseEntity *pTriggerEntity) {
...
void PhysicsImpactSound(CBaseEntity *pEntity, IPhysicsObject *pPhysObject,
  ...
  physicssound::AddImpactSound(
      g_PhysicsHook.m_impactSounds, pEntity, pEntity->entindex(), channel,
      pPhysObject, surfaceProps, surfacePropsHit, volume, impactSpeed);
}

void PhysCollisionSound(CBaseEntity *pEntity, IPhysicsObject *pPhysObject,
                        int channel, int surfaceProps, int surfacePropsHit,
                        float deltaTime, float speed) {
  if (deltaTime < 0.05f || speed < 70.0f) return;

  float volume =
      speed * speed * (1.0f / (320.0f * 320.0f));  // max volume at 320 in/s
  if (volume > 1.0f) volume = 1.0f;

  PhysicsImpactSound(pEntity, pPhysObject, channel, surfaceProps,
                     surfacePropsHit, volume, speed);
}

void PhysBreakSound(CBaseEntity *pEntity, IPhysicsObject *pPhysObject,
                    Vector vecOrigin) {
  if (!pPhysObject) return;

  physicssound::AddBreakSound(g_PhysicsHook.m_breakSounds, vecOrigin,
                              pPhysObject->GetMaterialIndex());
}
...
void PhysCollisionScreenShake(gamevcollisionevent_t *pEvent, int index) {
  ...
  if (mass >= VPHYSICS_LARGE_OBJECT_MASS &&
    ...
    if (pEvent->collisionSpeed > 30 && pEvent->deltaCollisionTime > 0.25f) {
      ...
      float amplitude = impulse * (collision_shake_amp.GetFloat() /
                                   (30.0f * VPHYSICS_LARGE_OBJECT_MASS));
      UTIL_ScreenShake(vecPos, amplitude, collision_shake_freq.GetFloat(),
                       collision_shake_time.GetFloat(), amplitude * 60,
                       SHAKE_START);
    }
  }
}
...
#if HL2_EPISODIC
// Uses DispatchParticleEffect because, so far as I know, that is the new means
// of kicking off flinders for this kind of collision. Should this be in
// g_pEffects instead?
...
void PhysCollisionDust(gamevcollisionevent_t *pEvent, surfacedata_t *phit) {
  switch (phit->game.material) {
    ...
    case CHAR_TEX_DIRT:
      ...
      if (pEvent->collisionSpeed < 200.0f) return;

      break;

    case CHAR_TEX_CONCRETE:

      if (pEvent->collisionSpeed < 340.0f) return;

      break;

#if HL2_EPISODIC
      // this is probably redundant because BaseEntity::VHandleCollision should
      // have already dispatched us elsewhere
...
void PhysFrictionSound(CBaseEntity *pEntity, IPhysicsObject *pObject,
  ...
  if (flVolume > (1.0f / 128.0f)) {
    ...
    if (!pFriction->pObject) {
      ...
      if (params.volume * flVolume <= 0.1f) return;
      ...
      pFriction->patch = CSoundEnvelopeController::GetController().SoundCreate(
          filter, pEntity->entindex(), CHAN_BODY, pSoundName,
          params.soundlevel);
      CSoundEnvelopeController::GetController().Play(
          pFriction->patch, params.volume * flVolume, params.pitch);
    } else {
      float pitch =
          (flVolume * (params.pitchhigh - params.pitchlow)) + params.pitchlow;
      CSoundEnvelopeController::GetController().SoundChangeVolume(
          pFriction->patch, params.volume * flVolume, 0.1f);
      CSoundEnvelopeController::GetController().SoundChangePitch(
          pFriction->patch, pitch, 0.1f);
    }
...
void PhysCallbackImpulse(IPhysicsObject *pPhysicsObject,
  ...
  g_PostSimulationQueue.QueueCall(PostSimulation_ImpulseEvent, pPhysicsObject,
                                  RefToVal(vecCenterForce),
                                  RefToVal(vecCenterTorque));
}

void PhysCallbackSetVelocity(IPhysicsObject *pPhysicsObject,
                             const Vector &vecVelocity) {
  Assert(physenv->IsInSimulation());
  g_PostSimulationQueue.QueueCall(PostSimulation_SetVelocityEvent,
                                  pPhysicsObject, RefToVal(vecVelocity));
}
...
void PhysCallbackDamage(CBaseEntity *pEntity, const CTakeDamageInfo &info,
  ...
  g_Collisions.AddDamageEvent(pEntity, info, event.pObjects[otherIndex], true,
                              event.preVelocity[otherIndex],
                              event.preAngularVelocity[otherIndex]);
}
...
void PhysCallbackDamage(CBaseEntity *pEntity, const CTakeDamageInfo &info) {
  if (PhysIsInCallback()) {
    ...
    IPhysicsObject *pInflictorPhysics =
        (pInflictor) ? pInflictor->VPhysicsGetObject() : NULL;
    g_Collisions.AddDamageEvent(pEntity, info, pInflictorPhysics, false,
    ...
    if (pEntity->VPhysicsGetObject()) {
      if (pBestObject) {
        const char *pErrorName =
            pErrorEntity ? pErrorEntity->GetClassname() : "Unknown";
        Vector origin =
            pErrorEntity ? pErrorEntity->GetAbsOrigin() : vec3_origin;
        DevWarning(
            "entity %s at %s has physics attachment to more than one entity "
            "with the name %s!!!\n",
...
static ConCommand air_density(
    "air_density", CC_AirDensity,
    "Changes the density of air for drag computations.", FCVAR_CHEAT);
...
void DebugDrawContactPoints(IPhysicsObject *pPhysics) {
  ...
  while (pSnapshot->IsValid()) {
    ...
    CFmtStr str("%s (%s): %s [%0.2f]", pEntity0->GetClassname(),
                STRING(pEntity0->GetModelName()), pEntity0->GetDebugName(),
                pSnapshot->GetFrictionCoefficient());
...
// examples:
// world:
//	DumpCollideToGlView( pWorldCollide->solids[0], vec3_origin, vec3_origin,
//"jaycollide.txt" );
// static_prop:
//	DumpCollideToGlView( info.m_pCollide->solids[0], info.m_Origin,
//info.m_Angles, "jaycollide.txt" );
//
//-----------------------------------------------------------------------------
void DumpCollideToGlView(CPhysCollide *pCollide, const Vector &origin,
                         const QAngle &angles, const char *pFilename) {
  if (!pCollide) return;
  ...
  for (i = 0; i < triCount; i++) {
    ...
    filesystem->FPrintf(fp, "%6.3f %6.3f %6.3f 1 0 0\n", outVerts[vert].x,
                        outVerts[vert].y, outVerts[vert].z);
    vert++;
