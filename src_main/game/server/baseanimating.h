...
#define BCF_NO_ANIMATION_SKIP \
  (1 << 0)  // Do not allow PVS animation skipping (mostly for attachments being
...
class CBaseAnimating : public CBaseEntity {
 public:
  ...
  // Then the interpolation will be done correctly on the client. This defaults
  // to off.
  ...
  // inline void SetSequence(int nSequence) { Assert( GetModelPtr( ) &&
  // nSequence >= 0 && nSequence < GetModelPtr( )->GetNumSeq() );  m_nSequence =
  // nSequence; }
  void SetSequence(int nSequence);
  /* inline */ void ResetSequence(int nSequence);
  // TODO(d.rattman): push transitions support down into CBaseAnimating?
  ...
  virtual void GetSkeleton(CStudioHdr *pStudioHdr, Vector pos[], Quaternion q[],
                           int boneMask);
 ...
 protected:
  // The modus operandi for pose parameters is that you should not use the const
  // char * version of the functions in general code -- it causes many many
  // string comparisons, which is slower than you think. Better is to save off
  // your pose parameters in member variables in your derivation of this
  // function:
 ...
 public:
  ...
  bool GotoSequence(int iCurrentSequence, float flCurrentCycle,
                    float flCurrentRate, int iGoalSequence, int &iNextSequence,
                    float &flCycle, int &iDir);
  ...
  bool GetAttachment(const char *szName, Vector &absOrigin,
                     ...
                     Vector *up = NULL);
  bool GetAttachment(int iAttachment, Vector &absOrigin, Vector *forward = NULL,
  ...
  void GetInputDispatchEffectPosition(const char *sInputString, Vector &pOrigin,
                                      QAngle &pAngles);
 ...
 private:
  ...
  // TODO(d.rattman): necessary so that cyclers can hack m_bSequenceFinished
...
#define ANIMATION_SKIN_BITS \
  10  // 1024 body skin selections TODO(d.rattman): this seems way high
...
#define ANIMATION_PLAYBACKRATE_BITS \
  8  // default playback rate, only used on leading edge detect sequence changes
