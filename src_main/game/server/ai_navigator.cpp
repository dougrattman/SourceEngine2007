...
const char *g_ppszGoalTypes[] = {
    "GOALTYPE_NONE",     "GOALTYPE_TARGETENT",
    "GOALTYPE_ENEMY",    "GOALTYPE_PATHCORNER",
    "GOALTYPE_LOCATION", "GOALTYPE_LOCATION_NEAREST_NODE",
    "GOALTYPE_FLANK",    "GOALTYPE_COVER",
};
...
#define ShouldTestFailPath() \
  (ai_test_nav_failure_handling.GetBool() && g_PathFailureCounter++ % 20 == 0)
#define ShouldTestFailMove() \
  (ai_test_nav_failure_handling.GetBool() && g_MoveFailureCounter++ % 100 == 0)
#else
...
#define FLIER_CUT_CORNER_DIST \
  16  // 8 means the npc's bounding box is contained without the box of the node
      // in WC

#define NAV_STOP_MOVING_TOLERANCE \
  6  // Goal tolerance for TASK_STOP_MOVING stopping paths
...
BEGIN_SIMPLE_DATADESC(CAI_Navigator)
  ...
  DEFINE_FIELD(m_navType, FIELD_INTEGER),
      ...
      //								m_pClippedWaypoints
      //(not saved)
      //m_flTimeClipped		(not saved) 								m_PreviousMoveActivity (not saved)
      ...
      // (think transient)
      //								m_hPeerWaitingOn
      //(peer move fields do not need to be saved, tied to current schedule and
      // path, which are not saved)
      // m_PeerWaitMoveTimer	(ibid) 								m_PeerWaitClearTimer(ibid)
      //								m_NextSidestepTimer
      //(ibid)
      DEFINE_FIELD(m_hBigStepGroundEnt, FIELD_EHANDLE),
      DEFINE_FIELD(m_hLastBlockingEnt, FIELD_EHANDLE),
  //								m_vPosBeginFailedSteer (reset
  //on
  // load) 								m_timeBeginFailedSteer (reset on load)
...
CAI_Navigator::CAI_Navigator(CAI_BaseNPC *pOuter) : BaseClass(pOuter) {
...
void CAI_Navigator::Restore(IRestore &restore) {
  ...
  if (version != AI_NAVIGATOR_SAVE_VERSION) return;
  ...
  if (minPathArray.Count()) {
    for (int i = 0; i < minPathArray.Count(); i++) {
      m_pClippedWaypoints->PrependWaypoint(
          minPathArray[i].GetPos(), minPathArray[i].NavType(),
          (minPathArray[i].Flags() & ~bits_WP_TO_PATHCORNER),
          minPathArray[i].flYaw);
    }
    m_flTimeClipped =
        gpGlobals->curtime + 1000;  // time passes between restore and onrestore
  }
}
...
float CAI_Navigator::MaxYawSpeed() { return GetOuter()->MaxYawSpeed(); }

//-----------------------------------------------------------------------------

void CAI_Navigator::SetSpeed(float fl) { GetOuter()->m_flSpeed = fl; }
...
bool CAI_Navigator::FindPath(const AI_NavGoal_t &goal, unsigned flags) {
  ...
  if (goal.activity != AIN_DEF_ACTIVITY)
    ...
    pPath->SetMovementActivity(
        (GetOuter()->GetState() == NPC_STATE_COMBAT) ? ACT_RUN : ACT_WALK);
  ...
  pPath->SetGoalType(GOALTYPE_NONE);  // avoids a spurious warning about setting
                                      // the goal type twice
  ...
  if ((goal.type == GOALTYPE_TARGETENT) || (goal.type == GOALTYPE_ENEMY)) {
    ...
    // Set the goal offset
    if (goal.dest != AIN_NO_DEST) pPath->SetTargetOffset(goal.dest);
  ...
  if (result == false) {
    ...
    if (goal.arrivalActivity != AIN_DEF_ACTIVITY &&
        goal.arrivalActivity > ACT_RESET) {
...
ConVar ai_navigator_generate_spikes_strength(
...
bool CAI_Navigator::SetGoal(const AI_NavGoal_t &goal, unsigned flags) {
  ...
  if (PostFrameNavigationSystem()->IsGameFrameRunning()) {
    ...
    PostFrameNavigationSystem()->EnqueueEntityNavigationQuery(
        GetOuter(),
    ...
    // For now, always succeed -- we need to deal with failures on the next
    // frame
    return true;
  }
  ...
  // Clear out previous state
  if (flags & AIN_CLEAR_PREVIOUS_STATE) ClearPath();

  if (GetOuter()->IsCurTaskContinuousMove() ||
      ai_post_frame_navigation.GetBool())
  ...
  if (result == false) {
    ...
    if (flags & AIN_DISCARD_IF_FAIL) ClearPath();
  } else {
...
bool CAI_Navigator::SetRadialGoal(const Vector &destination,
                                  const Vector &center, float radius, float arc,
                                  float stepDist, bool bClockwise,
                                  bool bAirRoute) {
  DbgNavMsg(GetOuter(), "Set radial goal\n");
  OnNewGoal();
  GetPath()->SetGoalType(GOALTYPE_LOCATION);

  GetPath()->SetWaypoints(
      GetPathfinder()->BuildRadialRoute(
          GetLocalOrigin(), center, destination, radius, arc, stepDist,
          bClockwise, GetPath()->GetGoalTolerance(), bAirRoute),
      true);
...
bool CAI_Navigator::SetRandomGoal(const Vector &from, float minPathLength,
                                  const Vector &dir) {
  DbgNavMsg(GetOuter(), "Set random goal\n");
  OnNewGoal();
  if (GetNetwork()->NumNodes() <= 0) return false;
  ...
  int fromNodeID =
      GetNetwork()->NearestNodeToPoint(GetOuter(), from, true, pFilter);

  if (fromNodeID == NO_NODE) return false;

  AI_Waypoint_t *pRoute =
      GetPathfinder()->FindShortRandomPath(fromNodeID, minPathLength, dir);
...
bool CAI_Navigator::SetDirectGoal(const Vector &goalPos, Navigation_t navType) {
  ...
  GetPath()->SetGoalType(GOALTYPE_LOCATION);
  GetPath()->SetWaypoints(
      new AI_Waypoint_t(goalPos, 0, navType, bits_WP_TO_GOAL, NO_NODE));
  GetPath()->SetGoalTolerance(GetOuter()->GetDefaultNavGoalTolerance());
...
static bool SetWanderGoalByRandomVector(CAI_Navigator *pNav, float minRadius,
                                        float maxRadius, int numTries) {
  while (--numTries >= 0) {
    float dist = random->RandomFloat(minRadius, maxRadius);
    Vector dir = UTIL_YawToVector(random->RandomFloat(0, 359.99));

    if (pNav->SetVectorGoal(dir, dist, minRadius)) return true;
  }
...
void CAI_Navigator::CalculateDeflection(const Vector &start, const Vector &dir,
...
bool CAI_Navigator::SetVectorGoal(const Vector &dir, float targetDist,
                                  float minDist, bool fShouldDeflect) {
...
bool CAI_Navigator::SetVectorGoalFromTarget(const Vector &goalPos,
                                            float minDist,
                                            bool fShouldDeflect) {
  Vector vDir = goalPos;
  float dist =
      ComputePathDirection(GetNavType(), GetLocalOrigin(), goalPos, &vDir);
  return SetVectorGoal(vDir, dist, minDist, fShouldDeflect);
}

//-----------------------------------------------------------------------------

bool CAI_Navigator::FindVectorGoal(Vector *pResult, const Vector &dir,
                                   float targetDist, float minDist,
                                   bool fShouldDeflect) {
  ...
  GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), testLoc,
  ...
  if (moveTrace.fStatus != AIMR_OK) {
    ...
    if (fShouldDeflect && moveTrace.vHitNormal != vec3_origin) {
      ...
      if (GetNavType() == NAV_GROUND) vecNormal.z = 0;

      CalculateDeflection(moveTrace.vEndPosition, dir, vecNormal, &vecDeflect);

      testLoc =
          moveTrace.vEndPosition + (vecDeflect * (targetDist - distAchieved));

      Vector temp = moveTrace.vEndPosition;
      GetMoveProbe()->MoveLimit(GetNavType(), temp, testLoc, MASK_NPCSOLID,
                                NULL, &moveTrace);

      distAchieved += (targetDist - distAchieved) - moveTrace.flDistObstructed;
    }

    if (distAchieved < minDist + 0.01) return false;
  }
...
bool CAI_Navigator::PrependLocalAvoidance(float distObstacle,
  ...
  if (AIStrongOpt()) return false;

  if (GetOuter()->IsFlaggedEfficient()) return false;
  ...
  if (distObstacle < GetHullWidth() * 0.5) {
    ...
    if (GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), vTestBackaway,
                                  MASK_NPCSOLID, GetNavTargetEntity(), 100.0,
                                  flags, &backawayTrace)) {
      vStart = backawayTrace.vEndPosition;
      pAvoidanceRoute = new AI_Waypoint_t(vStart, 0, GetNavType(),
                                          bits_WP_TO_DETOUR, NO_NODE);
    }
  }

  AI_Waypoint_t *pTriangulation = GetPathfinder()->BuildTriangulationRoute(
      vStart, GetCurWaypointPos(), GetNavTargetEntity(), bits_WP_TO_DETOUR,
      NO_NODE, 0.0, distObstacle, GetNavType());
  ...
  // @TODO (toml 02-04-04): it would be better to do this on each triangulation
  // test to improve the odds of success. however, difficult in current
  // structure
...
void CAI_Navigator::PrependWaypoint(const Vector &newPoint,
...
CBaseEntity *CAI_Navigator::GetGoalTarget() { return GetPath()->GetTarget(); }
...
bool CAI_Navigator::UpdateGoalPos(const Vector &goalPos) {
  ...
  if (PostFrameNavigationSystem()->IsGameFrameRunning()) {
    ...
    PostFrameNavigationSystem()->EnqueueEntityNavigationQuery(
        GetOuter(),
        CreateFunctor(this, &CAI_Navigator::UpdateGoalPos, RefToVal(goalPos)));
  ...
  Assert((GetPath()->GoalType() != GOALTYPE_ENEMY) &&
         (GetPath()->GoalType() != GOALTYPE_TARGETENT));
...
int CAI_Navigator::GetMovementSequence() {
  ...
  if (sequence == ACT_INVALID) {
    ...
    sequence = GetOuter()->SelectWeightedSequence(
        GetOuter()->TranslateActivity(activity));
    if (sequence == ACT_INVALID) {
      DevMsg(GetOuter(),
             "No appropriate sequence for movement activity %s (%d)\n",
             GetOuter()->GetActivityName(GetPath()->GetArrivalActivity()),
             GetPath()->GetArrivalActivity());

      if (activity == ACT_SCRIPT_CUSTOM_MOVE) {
        sequence = GetOuter()->GetScriptCustomMoveSequence();
      } else {
        sequence = GetOuter()->SelectWeightedSequence(
            GetOuter()->TranslateActivity(ACT_WALK));
      }
    }
...
int CAI_Navigator::GetArrivalSequence(int curSequence) {
  ...
  if (sequence == ACT_INVALID) {
    ...
    sequence = GetOuter()->SelectWeightedSequence(
        GetOuter()->TranslateActivity(activity), curSequence);
    ...
    if (sequence == ACT_INVALID) {
      DevMsg(GetOuter(),
             ...
             GetPath()->GetArrivalActivity());
      sequence = GetOuter()->SelectWeightedSequence(
          GetOuter()->TranslateActivity(ACT_IDLE), curSequence);
    }
    Assert(sequence != ACT_INVALID);
...
GoalType_t CAI_Navigator::GetGoalType() const { return GetPath()->GoalType(); }

//-----------------------------------------------------------------------------

int CAI_Navigator::GetGoalFlags() const { return GetPath()->GoalFlags(); }
...
bool CAI_Navigator::GetPointAlongPath(Vector *pResult, float distance,
                                      bool fReducibleOnly) {
  if (!GetPath()->GetCurWaypoint()) return false;
  ...
  while (pEndPoint->GetNext()) {
    distToNext =
        ComputePathDistance(GetNavType(), vPosPrev, pEndPoint->GetPos());

    if (distToNext > distRemaining) break;

    distRemaining -= distToNext;
    vPosPrev = pEndPoint->GetPos();
    if (fReducibleOnly && !pEndPoint->IsReducible()) break;
...
void CAI_Navigator::OnNavFailed(bool bMovement) {
  ...
  if (bMovement) GetOuter()->OnMovementFailed();

#ifdef DEBUG
  if (CurWaypointIsGoal()) {
    float flWaypointDist = ComputePathDistance(GetNavType(), GetLocalOrigin(),
                                               GetPath()->ActualGoalPosition());
    ...
    if (GetOuter()->ShouldBruteForceFailedNav()) {
      if (bMovement) {
        ...
        if (pBlocker && !pBlocker->IsWorld() && !pBlocker->IsPlayer() &&
          ...
          if (NPCPhysics_CreateSolver(GetOuter(), pBlocker, true, 10.0f) !=
              NULL) {
            ClearNavFailCounter();
          }
        }
...
int CAI_Navigator::GetNavFailCounter() const { return m_nNavFailCounter; }

//-----------------------------------------------------------------------------

void CAI_Navigator::ClearNavFailCounter() { m_nNavFailCounter = 0; }

//-----------------------------------------------------------------------------

float CAI_Navigator::GetLastNavFailTime() const { return m_flLastNavFailTime; }
...
bool CAI_Navigator::TeleportAlongPath() {
  while (GetPath()->GetCurWaypoint()) {
    ...
    GetOuter()->GetMoveProbe()->FloorPoint(
        vecStart, MASK_NPCSOLID, GetOuter()->StepHeight(), -64, &vTestPoint);

    if (CanFitAtPosition(vTestPoint, MASK_NPCSOLID, false, false)) {
      if (GetOuter()->GetMoveProbe()->CheckStandPosition(vTestPoint,
                                                         MASK_NPCSOLID)) {
        GetOuter()->Teleport(&vTestPoint, NULL, NULL);
        // clear ground entity, let normal fall code reestablish what the npc is
        // now standing on
...
//-----------------------------------------------------------------------------
void CAI_Navigator::SetNavType(Navigation_t navType) { m_navType = navType; }
...
AIMoveResult_t CAI_Navigator::MoveClimb() {
  ...
  if (GetNavType() != NAV_CLIMB) {
    ...
    GetMotor()->MoveClimbStart(climbDest, climbDir, climbDist,
                               GetPath()->CurWaypointYaw());
  }
  ...
  if (climbDist > 0.01 &&
    ...
    if (pOther) {
      ...
      if (!pOther->IsMoving())
        ...
        const Vector &otherClimbDest =
            pOther->GetNavigator()->GetPath()->CurWaypointPos();
  ...
  AIMoveResult_t result =
      GetMotor()->MoveClimbExecute(climbDest, climbDir, climbDist,
                                   GetPath()->CurWaypointYaw(), climbNodesLeft);
  ...
  if (result == AIMR_CHANGE_TYPE) {
    ...
    if (!GetPath()->GetCurWaypoint() ||
        !GetPath()->GetCurWaypoint()->GetNext() ||
        !(GetPath()->CurWaypointNavType() == NAV_CLIMB)) {
...
AIMoveResult_t CAI_Navigator::MoveJump() {
  ...
  // --------------------------------------------------
    ...
    // DevMsg( "jump to %f %f %f landed %f %f %f",
    // GetPath()->CurWaypointPos().x, GetPath()->CurWaypointPos().y,
    // GetPath()->CurWaypointPos().z, GetLocalOrigin().x, GetLocalOrigin().y,
    // GetLocalOrigin().z );
...
void CAI_Navigator::MoveCalcBaseGoal(AILocalMoveGoal_t *pMoveGoal) {
  ...
  pMoveGoal->maxDist = ComputePathDirection(GetNavType(), GetLocalOrigin(),
                                            pMoveGoal->target, &pMoveGoal->dir);
  ...
  if (GetPath()->CurWaypointIsGoal()) {
    ...
    if (pCurWaypoint->GetNext() &&
        pCurWaypoint->GetNext()->NavType() != pCurWaypoint->NavType())
...
bool CAI_Navigator::OnCalcBaseMove(AILocalMoveGoal_t *pMoveGoal,
...
bool CAI_Navigator::OnObstructionPreSteer(AILocalMoveGoal_t *pMoveGoal,
                                          float distClear,
  ...
  if (fTargetIsGoal) {
    ...
    if (pMoveGoal->maxDist < 4 * 12 &&
        pMoveGoal->maxDist - distClear < tolerance)
  ...
  if (fShouldAttemptHit) {
    if (distClear > pMoveGoal->maxDist) {
      ...
      if (distClear < AI_EPS_CASTS)  // needed because vphysics can pull us back
                                     // up to this far
      {
  ...
  if (ai_vehicle_avoidance.GetBool() &&
    ...
    AI_Waypoint_t *pOBB = GetPathfinder()->BuildOBBAvoidanceRoute(
        ...
        pMoveGoal->directTrace.pObstruction, GetNavTargetEntity(),
        GetNavType());
    ...
    if (pOBB == NULL) {
      ...
      if ( GetOuter()->ShouldFailNav( true ) == false )
      {
              ...
              NPCPhysics_CreateSolver( GetOuter(),
      pMoveGoal->directTrace.pObstruction, true, 5.0f ); return true;
      }
      */
    } else {
  ...
  if (!m_hBigStepGroundEnt.Get() && pMoveGoal->directTrace.pObstruction &&
...
bool CAI_Navigator::OnInsufficientStopDist(AILocalMoveGoal_t *pMoveGoal,
                                           float distClear,
                                           AIMoveResult_t *pResult) {
...
#ifdef PHYSICS_NPC_SHADOW_DISCREPENCY
  if (distClear <
      AI_EPS_CASTS)  // needed because vphysics can pull us back up to this far
  {
    DebugNoteMovementFailure();
    *pResult = pMoveGoal->directTrace.fStatus;
    pMoveGoal->maxDist = 0;
  ...
  if (!IsMovingOutOfWay(*pMoveGoal, distClear)) {
    float goalDist = ComputePathDistance(GetNavType(), GetAbsOrigin(),
                                         GetPath()->ActualGoalPosition());
    ...
    if (m_NextSidestepTimer.Expired()) {
      ...
      AIMoveTrace_t moveTrace;
      Vector vDeflection;
      CalculateDeflection(GetLocalOrigin(), pMoveGoal->dir,
                          pMoveGoal->directTrace.vHitNormal, &vDeflection);

      for (int i = 1; i > -2; i -= 2) {
        Vector testLoc =
            GetLocalOrigin() + (vDeflection * GetHullWidth() * 2.0) * i;
        GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), testLoc,
                                  MASK_NPCSOLID, NULL, &moveTrace);
        if (moveTrace.fStatus == AIMR_OK) {
          Vector vNewWaypoint = moveTrace.vEndPosition;
          GetMoveProbe()->MoveLimit(GetNavType(), vNewWaypoint,
                                    pMoveGoal->target, MASK_NPCSOLID_BRUSHONLY,
                                    NULL, &moveTrace);
          if (moveTrace.fStatus == AIMR_OK) {
...
bool CAI_Navigator::OnFailedSteer(AILocalMoveGoal_t *pMoveGoal, float distClear,
                                  AIMoveResult_t *pResult) {
  // Allow the NPC to override this behavior
  ...
  if (pMoveGoal->flags & AILMG_TARGET_IS_GOAL) {
    ...
    if (distClear > pMoveGoal->maxDist - GetPath()->GetGoalTolerance()) {
      Assert(CurWaypointIsGoal() &&
             fabs(pMoveGoal->maxDist - GetPathDistToCurWaypoint()) < 0.01);

      if (pMoveGoal->maxDist > distClear) pMoveGoal->maxDist = distClear;

      if (distClear < 0.125) OnNavComplete();
  ...
  if (!TestingSteering() &&
      pMoveGoal->directTrace.fStatus == AIMR_BLOCKED_NPC &&
      pMoveGoal->directTrace.vHitNormal != vec3_origin) {
    AIMoveTrace_t moveTrace;
    Vector vDeflection;
    CalculateDeflection(GetLocalOrigin(), pMoveGoal->dir,
                        pMoveGoal->directTrace.vHitNormal, &vDeflection);
    ...
    if (pMoveGoal->dir.AsVector2D().Dot(vDeflection.AsVector2D()) > 0.7) {
      Vector testLoc =
          GetLocalOrigin() + (vDeflection * pMoveGoal->curExpectedDist);
      GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), testLoc,
                                MASK_NPCSOLID, NULL, &moveTrace);
      if (moveTrace.fStatus == AIMR_OK) {
  ...
  // If fail steer more than once after a second with no measurable progres,
  // fail completely This usually means a sucessful triangulation was not
  // actually a valid avoidance.
  const float MOVE_TOLERANCE = 12.0;
  const float TIME_TOLERANCE = 1.0;

  if (m_vPosBeginFailedSteer == vec3_invalid ||
      (m_vPosBeginFailedSteer - GetAbsOrigin()).LengthSqr() >
          Square(MOVE_TOLERANCE)) {
  ...
  if (!(pMoveGoal->flags & AILMG_NO_AVOIDANCE_PATHS) &&
      distClear < pMoveGoal->maxDist && !TestingSteering()) {
...
bool CAI_Navigator::OnFailedLocalNavigation(AILocalMoveGoal_t *pMoveGoal,
                                            float distClear,
                                            AIMoveResult_t *pResult) {
  // Allow the NPC to override this behavior
...
bool CAI_Navigator::DelayNavigationFailure(const AIMoveTrace_t &trace) {
  // This code only handles the case of a group of AIs in close proximity,
  // preparing to move mostly as a group, but on slightly different think
  // schedules. Without this patience factor, in the middle or at the rear might
  // fail just because it happened to have its think function called a half
  // cycle before the one in front of it.

  CAI_BaseNPC *pBlocker =
      trace.pObstruction ? trace.pObstruction->MyNPCPointer() : NULL;
  Assert(m_fPeerMoveWait == false ||
         pBlocker == m_hPeerWaitingOn);  // expected to be cleared each frame,
                                         // and never call this function twice
    ...
    if (pBlocker) {
      if (m_hPeerWaitingOn != pBlocker || m_PeerWaitClearTimer.Expired()) {
        ...
        if (pBlocker->GetGroundEntity() == GetOuter()) {
          trace_t bumpTrace;
          pBlocker->GetMoveProbe()->TraceHull(
              pBlocker->GetAbsOrigin(),
              pBlocker->GetAbsOrigin() + Vector(0, 0, 2.0), MASK_NPCSOLID,
              &bumpTrace);
          if (bumpTrace.fraction == 1.0) {
            UTIL_SetOrigin(pBlocker, bumpTrace.endpos, true);
          }
        }
      } else if (m_hPeerWaitingOn == pBlocker &&
                 !m_PeerWaitMoveTimer.Expired()) {
        m_fPeerMoveWait = true;
      }
    }
  }
...
// @TODO (toml 11-12-02): right now, physics can pull the box back pretty far
// even though a hull trace said we could make the move. Jay is looking into it.
// For now, if the NPC physics shadow is active, allow for a bugger tolerance
...
bool CAI_Navigator::MoveUpdateWaypoint(AIMoveResult_t *pResult) {
  ...
  AI_Waypoint_t *pCurWaypoint = GetPath()->GetCurWaypoint();
  float waypointDist = ComputePathDistance(GetNavType(), GetLocalOrigin(),
                                           pCurWaypoint->GetPos());
  ...
  if (waypointDist <= tolerance) {
    if (test_it) {
      if (pCurWaypoint->GetNext() &&
          pCurWaypoint->GetNext()->NavType() != pCurWaypoint->NavType()) {
        if (waypointDist < 0.001) fHit = true;
      } else
        fHit = true;
    } else
      fHit = true;
...
bool CAI_Navigator::OnMoveExecuteFailed(const AILocalMoveGoal_t &move,
  ...
  if (!m_hBigStepGroundEnt.Get() && trace.pObstruction &&
  ...
  if (fMotorResult == AIM_PARTIAL_HIT_TARGET) {
    OnNavComplete();
    *pResult = AIMR_OK;
  } else if (fMotorResult == AIM_PARTIAL_HIT_NPC &&
             DelayNavigationFailure(trace)) {
    *pResult = AIMR_OK;
  }
...
bool CAI_Navigator::OnMoveBlocked(AIMoveResult_t *pResult) {
  if (*pResult == AIMR_BLOCKED_NPC && GetPath()->GetCurWaypoint() &&
    ...
    CBasePropDoor *pDoor = (CBasePropDoor *)(CBaseEntity *)GetPath()
                               ->GetCurWaypoint()
                               ->GetEHandleData();
    if (pDoor != NULL) {
  ...
  // Allow the NPC to override this behavior
  if (GetOuter()->OnMoveBlocked(pResult)) return true;

  float flWaypointDist;

  if (!GetPath()->CurWaypointIsGoal() &&
      GetPath()->GetCurWaypoint()->IsReducible()) {
    flWaypointDist = ComputePathDistance(GetNavType(), GetLocalOrigin(),
                                         GetCurWaypointPos());
...
AIMoveResult_t CAI_Navigator::MoveEnact(const AILocalMoveGoal_t &baseMove) {
  ...
  result = GetLocalNavigator()->MoveCalc(
      &move, (m_flLastSuccessfulSimplifyTime == gpGlobals->curtime));
...
AIMoveResult_t CAI_Navigator::MoveNormal() {
  if (!PreMove()) return AIMR_ILLEGAL;

  if (ShouldTestFailMove()) return AIMR_ILLEGAL;

  // --------------------------------

  AIMoveResult_t result = AIMR_ILLEGAL;

  if (MoveUpdateWaypoint(&result)) return result;
  ...
  int nPreMoveSequence =
      GetOuter()->GetSequence();  // this is an unfortunate necessity to ensure
                                  // setting back the activity picks the right
                                  // one if it had been sticky
  ...
  if (m_bValidateActivitySpeed && GetIdealSpeed() <= 0.0f) {
    if (GetActivity() == ACT_TRANSITION) return AIMR_OK;
    DevMsg("%s moving with speed <= 0 (%s)\n", GetEntClassname(),
           GetOuter()->GetSequenceName(GetSequence()));
  }
...
bool CAI_Navigator::PreMove() {
  ...
  if (goalType == NAV_GROUND && curType != NAV_GROUND) {
    DevMsg(
        "Warning: %s(%s) appears to have wrong nav type in "
        "CAI_Navigator::MoveGround()\n",
...
bool CAI_Navigator::IsMovingOutOfWay(const AILocalMoveGoal_t &moveGoal,
  ...
  CAI_BaseNPC *pBlocker =
      moveGoal.directTrace.pObstruction
          ? moveGoal.directTrace.pObstruction->MyNPCPointer()
          : NULL;

  // if it's the world, it ain't moving
  if (!pBlocker) return false;

  // if they're doing something, assume it'll work out
  if (pBlocker->IsMoving()) {
    if (distClear > moveGoal.curExpectedDist * 0.75) return true;
...
bool CAI_Navigator::Move(float flInterval) {
  ...
  if (!GetOuter()->OverrideMove(flInterval)) {
    ...
    if (bHasGoal) {
      ...
      if (ActivityIsLocomotive(m_PreviousMoveActivity) &&
          !ActivityIsLocomotive(GetMovementActivity())) {
        SetMovementActivity(
            GetOuter()->TranslateActivity(m_PreviousMoveActivity));
      }
    } else {
    ...
    if (fShouldMove) {
      ...
      if (result == AINR_OK) {
        ...
        while (moveResult >= AIMR_OK && !m_fNavComplete) {
          ...
          if (m_hBigStepGroundEnt &&
              m_hBigStepGroundEnt != GetOuter()->GetGroundEntity())
            m_hBigStepGroundEnt = NULL;
          ...
          if (moveResult == AIMR_OK) bMoved = true;

          ++nLoopCount;
          if (nLoopCount > 16) {
            DevMsg(
                "ERROR: %s navigation not terminating. Possibly bad cyclical "
                "solving?\n",
        ...
        if (IsMoveBlocked(moveResult)) {
          ...
          if (moveResult != AIMR_BLOCKED_NPC || GetNavType() == NAV_CLIMB ||
              GetNavType() == NAV_JUMP ||
              GetPath()->CurWaypointNavType() == NAV_JUMP) {
          ...
          if (!bRecovered) {
            OnNavFailed((moveResult == AIMR_ILLEGAL) ? FAIL_NO_ROUTE_ILLEGAL
      ...
      // @TODO (toml 10-30-02): the climb part of this is unacceptable, but
      // needed until navigation can handle commencing
...
float CAI_Navigator::CalcYawSpeed(void) {
  ...
  if (flNPCYaw >= 0.0f) return flNPCYaw;
  ...
  if (IsGoalSet() && GetIdealSpeed() != 0.0) {
    ...
    if (GetIdealSpeed() > 0) {
      ...
      // -----------------------------------------------------------------
      float waypointDist =
          (GetPath()->CurWaypointPos() - GetLocalOrigin()).Length();
...
void CAI_Navigator::AdvancePath() {
  ...
  AI_Waypoint_t *pCurWaypoint = GetPath()->GetCurWaypoint();
  bool bPassingPathcorner =
      ((pCurWaypoint->Flags() & bits_WP_TO_PATHCORNER) != 0);

  if (bPassingPathcorner) {
    Assert(!pCurWaypoint->GetNext() ||
           (pCurWaypoint->GetNext()->Flags() &
            (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL)) ==
               (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL));
  ...
  if (GetPath()->CurWaypointIsGoal()) return;

  if (pCurWaypoint->Flags() & bits_WP_TO_DOOR) {
    CBasePropDoor *pDoor =
        (CBasePropDoor *)(CBaseEntity *)pCurWaypoint->GetEHandleData();
    if (pDoor != NULL) {
      GetOuter()->OpenPropDoorBegin(pDoor);
    } else {
  ...
  if (bPassingPathcorner) {
    ...
    if (pCurWaypoint) {
      Assert(
          (pCurWaypoint->Flags() & (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL)) ==
          (bits_WP_TO_PATHCORNER | bits_WP_TO_GOAL));
...
bool CAI_Navigator::ShouldAttemptSimplifyTo(const Vector &pos) {
  if (m_bForcedSimplify) return true;

  Vector vecToPos = (pos - GetLocalOrigin());
  vecToPos.z = 0;
  VectorNormalize(vecToPos);

  Vector vecCurrentDirectionOfMovement =
      (GetCurWaypointPos() - GetLocalOrigin());
  vecCurrentDirectionOfMovement.z = 0;
  VectorNormalize(vecCurrentDirectionOfMovement);

  float dot =
      vecCurrentDirectionOfMovement.AsVector2D().Dot(vecToPos.AsVector2D());
...
bool CAI_Navigator::ShouldSimplifyTo(bool passedDetour, const Vector &pos) {
  ...
  if (!passedDetour) flags |= AIMLF_IGNORE_TRANSIENTS;
#endif

  AIMoveTrace_t moveTrace;
  GetMoveProbe()->MoveLimit(GetNavType(), GetLocalOrigin(), pos, MASK_NPCSOLID,
...
void CAI_Navigator::SimplifyPathInsertSimplification(
    AI_Waypoint_t *pSegmentStart, const Vector &point) {
  if (point != pSegmentStart->GetPos()) {
    ...
    AI_Waypoint_t *pNewWaypoint =
        new AI_Waypoint_t(point, 0, pSegmentStart->NavType(), 0, NO_NODE);
...
bool CAI_Navigator::SimplifyPathForwardScan(
    const CAI_Navigator::SimplifyForwardScanParams &params,
    AI_Waypoint_t *pCurWaypoint, const Vector &curPoint, float distRemaining,
    bool skipTest, bool passedDetour, int *pTestCount) {
  ...
  if (distRemaining > 0) {
    if (pCurWaypoint->IsReducible()) {
      ...
      float distToNext =
          ComputePathDistance(GetNavType(), curPoint, pNextWaypoint->GetPos());
      ...
      bool skipTestNext =
          (ComputePathDistance(GetNavType(), GetLocalOrigin(), nextPoint) >
           params.radius + 0.1);

      if (SimplifyPathForwardScan(params, pRecursionWaypoint, nextPoint,
                                  distRemaining, skipTestNext, passedDetour,
                                  pTestCount))
        return true;
    }
  }

  if (!skipTest && *pTestCount < params.maxSamples &&
      ShouldAttemptSimplifyTo(curPoint)) {
    (*pTestCount)++;
...
bool CAI_Navigator::SimplifyPathForwardScan(
  ...
  if (distRemaining < 0.1) return false;

  if (SimplifyPathForwardScan(params, pCurWaypoint, pCurWaypoint->GetPos(),
                              distRemaining, true, false, &testCount))
    return true;
...
bool CAI_Navigator::SimplifyPathForward(float maxDist) {
  ...
  if (SimplifyPathForwardScan(scanParams)) return true;

  if (ShouldAttemptSimplifyTo(pNextWaypoint->GetPos()) &&
      ComputePathDistance(GetNavType(), GetLocalOrigin(),
                          pNextWaypoint->GetPos()) < scanParams.scanDist &&
      ShouldSimplifyTo(
          ((pCurWaypoint->Flags() & bits_WP_TO_DETOUR) != 0),
          pNextWaypoint
              ->GetPos()))  // @TODO (toml 04-25-03): need to handle this
                            // better. this is here because forward scan may
                            // look out so far that a close obvious solution is
                            // skipped (due to test limiting)
  {
    delete pCurWaypoint;
...
bool CAI_Navigator::SimplifyPathBacktrack() {
  ...
  // ------------------------------------------------------------------------
  if (pCurWaypoint->GetNext() && (pNextWaypoint->Flags() & bits_WP_TO_NODE) &&
    ...
    if (firstProj > 0.5 * firstDist) {
      ...
      if (CapabilitiesGet() & bits_CAP_MOVE_JUMP) buildFlags |= bits_BUILD_JUMP;

      // Make sure I can get to the new point
      AI_Waypoint_t *route1 = GetPathfinder()->BuildLocalRoute(
          GetLocalOrigin(), targetPos, GetPath()->GetTarget(),
          bits_WP_TO_DETOUR, NO_NODE, buildFlags, goalTolerance);
      if (!route1) return false;

      // Make sure the new point gets me to the target location
      AI_Waypoint_t *route2 = GetPathfinder()->BuildLocalRoute(
          targetPos, pNextWaypoint->GetPos(), GetPath()->GetTarget(),
          bits_WP_TO_DETOUR, NO_NODE, buildFlags, goalTolerance);
      if (!route2) {
        DeleteAll(route1);
        return false;
      ...
      // Now advance the route linked list, putting the finished waypoints back
      // in the waypoint pool
...
bool CAI_Navigator::SimplifyPathQuick() {
  ...
  if (SimplifyPathForwardScan(quickScanParams[AIStrongOpt()])) return true;
...
bool CAI_Navigator::SimplifyPath(bool bFirstForPath, float scanDist) {
  ...
  if (TestingSteering() || IsSimplifyPathDisabled()) return false;
  ...
  if (navType == NAV_GROUND || navType == NAV_FLY) {
    AI_Waypoint_t *pCurWaypoint = GetPath()->GetCurWaypoint();
    if (!pCurWaypoint || !pCurWaypoint->IsReducible()) return false;
    ...
    if (bFullSimplify) {
      float simplifyDelay =
          (bInPVS) ? ROUTE_SIMPLIFY_TIME_DELAY[AIStrongOpt()]
                   : NO_PVS_ROUTE_SIMPLIFY_TIME_DELAY[AIStrongOpt()];

      if (GetOuter()->GetMoveEfficiency() > AIME_NORMAL) simplifyDelay *= 2;
      ...
      if (SimplifyPathForward(scanDist))
        ...
        bRetVal = true;
    } else if (bFirstForPath ||
               (bInPVS && GetOuter()->GetMoveEfficiency() == AIME_NORMAL)) {
      if (!AIStrongOpt() ||
          gpGlobals->curtime - m_flLastSuccessfulSimplifyTime >
              QUICK_SIMPLIFY_TIME_DELAY[AIStrongOpt()]) {
        if (SimplifyPathQuick()) bRetVal = true;
      }
    }
  }
...
AI_NavPathProgress_t CAI_Navigator::ProgressFlyPath(
    const AI_ProgressFlyPathParams_t &params) {
  if (IsGoalActive()) {
    ...
    if (CurWaypointIsGoal()) {
      ...
      bool bIsStrictWaypoint =
          ((GetPath()->CurWaypointFlags() &
            (bits_WP_TO_PATHCORNER | bits_WP_DONT_SIMPLIFY)) != 0);
      float tolerance = (bIsStrictWaypoint) ? params.strictPointTolerance
                                            : params.waypointTolerance;
      if (waypointDist <= tolerance) {
        trace_t tr;
        AI_TraceLine(GetAbsOrigin(),
                     GetPath()->GetCurWaypoint()->GetNext()->GetPos(),
                     MASK_NPCSOLID, GetOuter(), COLLISION_GROUP_NONE, &tr);
        if (tr.fraction == 1.0f) {
          AdvancePath();
      ...
      if (SimplifyFlyPath(params)) return AINPP_ADVANCED;
    }
  }
...
void CAI_Navigator::SimplifyFlyPath(unsigned collisionMask,
  ...
  AI_ProgressFlyPathParams_t params(collisionMask, strictPointTolerance,
                                    blockTolerance, 0, 0, blockHandling);
...
bool CAI_Navigator::SimplifyFlyPath(const AI_ProgressFlyPathParams_t &params) {
  ...
  if (m_flNextSimplifyTime > gpGlobals->curtime) return false;
  ...
  if (!bIsStrictWaypoint || length < params.strictPointTolerance) {
    ...
    AIMoveTrace_t moveTrace;
    GetMoveProbe()->MoveLimit(NAV_FLY, GetLocalOrigin(),
                              GetPath()->NextWaypointPos(),
                              params.collisionMask, params.pTarget, &moveTrace);

    if (moveTrace.flDistObstructed - params.blockTolerance < 0.01 ||
        ((params.blockHandling == AISF_IGNORE) &&
         (moveTrace.fStatus == AIMR_BLOCKED_NPC))) {
      AdvancePath();
      return true;
    } else if (moveTrace.pObstruction && params.blockHandling == AISF_AVOID) {
      PrependLocalAvoidance(params.blockTolerance - moveTrace.flDistObstructed,
                            moveTrace);
    }
  }
...
// Output :
//-----------------------------------------------------------------------------
float CAI_Navigator::MovementCost(int moveType, Vector &vecStart,
                                  Vector &vecEnd) {
  float cost;
...
bool CAI_Navigator::CanFitAtNode(int nodeNum, unsigned int collisionMask) {
  ...
  if ((pTestNode->GetType() == NODE_CLIMB) &&
      (pTestNode->m_eNodeInfo &
       (bits_NODE_CLIMB_BOTTOM | bits_NODE_CLIMB_EXIT))) {
    if (!GetMoveProbe()->CheckStandPosition(startPos, collisionMask)) {
      return false;
    }
  }
  ...
  if (!CanFitAtPosition(startPos, collisionMask)) {
    ...
    if (!CanFitAtPosition(startPos, collisionMask)) return false;
  }
...
// Output :
//-----------------------------------------------------------------------------
bool CAI_Navigator::CanFitAtPosition(const Vector &vStartPos,
                                     unsigned int collisionMask,
                                     bool bIgnoreTransients,
                                     bool bAllowPlayerAvoid) {
  CTraceFilterNav traceFilter(const_cast<CAI_BaseNPC *>(GetOuter()),
                              bIgnoreTransients, GetOuter(),
                              COLLISION_GROUP_NONE, bAllowPlayerAvoid);

  Vector vEndPos = vStartPos;
  vEndPos.z += 0.01;
  trace_t tr;
  AI_TraceHull(vStartPos, vEndPos, GetHullMins(), GetHullMaxs(), collisionMask,
               &traceFilter, &tr);
...
//-----------------------------------------------------------------------------
// Computes the distance to our goal, rebuilding waypoint distances if
// necessary. Returns -1 if we still don't have a valid path length after
// rebuilding.
//
...
float CAI_Navigator::BuildAndGetPathDistToGoal() {
  ...
  if (GetPath()->GetCurWaypoint() &&
      (GetPath()->GetCurWaypoint()->flPathDistGoal >= 0))
...
float CAI_Navigator::GetPathDistToGoal() const {
  return (GetPath()->GetCurWaypoint())
             ? (GetPathDistToCurWaypoint() +
                GetPath()->GetCurWaypoint()->flPathDistGoal)
             : 0;
}
...
bool CAI_Navigator::FindPath(bool fSignalTaskStatus, bool bDontIgnoreBadLinks) {
  ...
  if (ai_navigator_generate_spikes.GetBool()) {
    unsigned int nLargeCount =
        (INT_MAX >> (ai_navigator_generate_spikes_strength.GetInt()));
    while (nLargeCount--) {
    }
  }

  bool bRetrying =
      (HasMemory(bits_MEMORY_PATH_FAILED) && m_timePathRebuildMax != 0);
  if (bRetrying) {
  ...
  if (!bDontIgnoreBadLinks && !bFindResult &&
      GetOuter()->IsNavigationUrgent()) {
...
//			occured as stale so when then next node route is build
//that link 			will be explictly checked for blockages
// Input  :
// Output :
//-----------------------------------------------------------------------------
bool CAI_Navigator::MarkCurWaypointFailedLink(void) {
  if (TestingSteering()) return false;

  if (!m_fRememberStaleNodes) return false;

  // Prevent a crash in release
  if (!GetPath() || !GetPath()->GetCurWaypoint()) return false;
  ...
  if (endID != NO_NODE) {
    ...
    if (m_hLastBlockingEnt != NULL && !m_hLastBlockingEnt->IsPlayer() &&
        !m_hLastBlockingEnt->IsNPC() &&
      ...
      if (bFoundLarge && !bFoundSmall) {
        Vector vStartPos =
            GetNetwork()->GetNode(endID)->GetPosition(GetHullType());
        Vector vEndPos = vStartPos;
        vEndPos.z += 0.01;
        trace_t tr;

        UTIL_TraceModel(vStartPos, vEndPos, GetHullMins(), GetHullMaxs(),
                        m_hLastBlockingEnt, COLLISION_GROUP_NONE, &tr);

        if (tr.startsolid) bBlockAll = true;
      }
    }
...
bool CAI_Navigator::DoFindPathToPos(void) {
  ...
  AI_Waypoint_t *pFirstWaypoint =
      pPathfinder->BuildRoute(origin, actualGoalPos, pTarget, tolerance,
  ...
  if (pFirstWaypoint->IsReducible() && pFirstWaypoint->GetNext() &&
    ...
    // If we're seemingly beyond the waypoint, and our hull is over the line,
    // move on
    const float EPS = 0.1;
    Vector vClosest;
    CalcClosestPointOnLineSegment(origin, pFirstWaypoint->GetPos(),
...
CBaseEntity *CAI_Navigator::GetNextPathcorner(CBaseEntity *pPathCorner) {
  ...
  if (pPathCorner) {
    ...
    CAI_Hint *pHint;
    if (!pNextPathCorner &&
        (pHint = dynamic_cast<CAI_Hint *>(pPathCorner)) != NULL) {
...
bool CAI_Navigator::DoFindPathToPathcorner(CBaseEntity *pPathCorner) {
  ...
  // UNDONE: Remove this and change the pathing code to be able to refresh
  // itself and support circular paths, etc.
  ...
  if (pPathCorner != NULL) {
    ...
    if ((GetGoalFlags() & AIN_NO_PATHCORNER_PATHFINDING) ||
        m_bNoPathcornerPathfinds) {
      // HACKHACK: If the starting path_corner has a speed, copy that to the
      // entity
      if (pPathCorner->m_flSpeed != 0) SetSpeed(pPathCorner->m_flSpeed);

      GetPath()->ClearWaypoints();

      AI_Waypoint_t *pRoute =
          new AI_Waypoint_t(pPathCorner->GetLocalOrigin(), 0, GetNavType(),
                            bits_WP_TO_PATHCORNER, NO_NODE);
      pRoute->hPathCorner = pPathCorner;
      AI_Waypoint_t *pLast = pRoute;
      pPathCorner = GetNextPathcorner(pPathCorner);
      if (pPathCorner) {
        pLast = new AI_Waypoint_t(pPathCorner->GetLocalOrigin(), 0,
                                  GetNavType(), bits_WP_TO_PATHCORNER, NO_NODE);
      ...
      if ((returnCode = DoFindPathToPos()) != false) {
        ...
        pPathCorner = GetNextPathcorner(
            pPathCorner);  // first already accounted for in pathfind
        if (pPathCorner) {
          // Place a dummy node in that will be used to signal the next
          // pathfind, also prevents animation system from decellerating when
...
bool CAI_Navigator::DoFindPath(void) {
  ...
  switch (GetPath()->GoalType()) {
    ...
    case GOALTYPE_LOCATION_NEAREST_NODE: {
      ...
      if (myNodeID != NO_NODE) {
        destNodeID = GetPathfinder()->NearestNodeToPoint(
            GetPath()->ActualGoalPosition());
        if (destNodeID != NO_NODE) {
          AI_Waypoint_t *pRoute =
              GetPathfinder()->FindBestPath(myNodeID, destNodeID);
...
void CAI_Navigator::ClearPath(void) {
  ...
  m_timePathRebuildMax =
      0;  // How long to try rebuilding path before failing task
  m_timePathRebuildFail =
      0;  // Current global time when should fail building path
  m_timePathRebuildNext = 0;  // Global time to try rebuilding again
  m_timePathRebuildDelay =
      0;  // How long to wait before trying to rebuild again
  ...
  if (pWaypoint) {
    ...
    while (pWaypoint) {
      if (pWaypoint->iNodeID != NO_NODE) {
        ...
        if (pNode) {
          if (pNode->IsLocked()) pNode->Unlock();
        }
      }
      pWaypoint = pWaypoint->GetNext();
    }
...
bool CAI_Navigator::GetStoppingPath(CAI_WaypointList *pClippedWaypoints) {
  ...
  AI_Waypoint_t *pCurWaypoint = GetPath()->GetCurWaypoint();
  if (pCurWaypoint) {
    bool bMustCompleteCurrent = (pCurWaypoint->NavType() == NAV_CLIMB ||
                                 pCurWaypoint->NavType() == NAV_JUMP);
    float distRemaining = GetMotor()->MinStoppingDist(0);

    if (bMustCompleteCurrent) {
      float distToCurrent = ComputePathDistance(
          pCurWaypoint->NavType(), GetLocalOrigin(), pCurWaypoint->GetPos());
      if (pCurWaypoint->NavType() == NAV_CLIMB) {
        if (pCurWaypoint->GetNext() &&
            pCurWaypoint->GetNext()->NavType() == NAV_CLIMB)
          distToCurrent +=
              ComputePathDistance(NAV_CLIMB, pCurWaypoint->GetPos(),
                                  pCurWaypoint->GetNext()->GetPos());
        distToCurrent += GetHullWidth() * 2.0;
      }

      if (distToCurrent > distRemaining) distRemaining = distToCurrent;
    }
    ...
    if (bMustCompleteCurrent || distRemaining > 0.1) {
      ...
      while (distRemaining > 0.01 && pNextPoint) {
        if ((pNextPoint->NavType() == NAV_CLIMB ||
             pNextPoint->NavType() == NAV_JUMP) &&
        ...
        if (pNextPoint->NavType() != NAV_CLIMB || !pNextPoint->GetNext() ||
            pNextPoint->GetNext()->NavType() != NAV_CLIMB)
          bMustCompleteCurrent = false;

        float distToNext = ComputePathDistance(pNextPoint->NavType(), vPosPrev,
                                               pNextPoint->GetPos());
        ...
        if (distToNext <= distRemaining + 0.01) {
          ...
          //					NDebugOverlay::Cross3D( vPosPrev, 16, 255,
          //255, 0, false, 10.0f );

          pNextPoint = pNextPoint->GetNext();
          distRemaining -= distToNext;
        } else {
          Assert(!(pNextPoint->NavType() == NAV_CLIMB ||
                   pNextPoint->NavType() == NAV_JUMP));
          ...
          AIMoveTrace_t trace;
          if (GetMoveProbe()->MoveLimit(pNextPoint->NavType(), vPosPrev,
                                        remainder, MASK_NPCSOLID, NULL, 100,
                                        AIMLF_DEFAULT | AIMLF_2D, &trace)) {
            pNewWaypoint =
                new AI_Waypoint_t(trace.vEndPosition, yaw,
                                  pNextPoint->NavType(), bits_WP_TO_GOAL, 0);
...
bool CAI_Navigator::SetGoalFromStoppingPath() {
  if (m_pClippedWaypoints && m_pClippedWaypoints->IsEmpty()) SaveStoppingPath();
  if (m_pClippedWaypoints && !m_pClippedWaypoints->IsEmpty()) {
    if (m_PreviousMoveActivity <= ACT_RESET &&
        GetMovementActivity() <= ACT_RESET) {
      m_pClippedWaypoints->RemoveAll();
      DevWarning(2,
                 "%s has a stopping path and no valid. Movement activity: %s "
                 "(prev %s)\n",
                 GetOuter()->GetDebugName(),
                 ActivityList_NameForIndex(GetMovementActivity()),
                 ActivityList_NameForIndex(m_PreviousMoveActivity));
      return false;
    }

    if ((m_pClippedWaypoints->GetFirst()->NavType() == NAV_CLIMB ||
         m_pClippedWaypoints->GetFirst()->NavType() == NAV_JUMP)) {
      const Task_t *pCurTask = GetOuter()->GetTask();
      if (pCurTask && pCurTask->iTask == TASK_STOP_MOVING) {
        // Clipped paths are used for 2 reasons: Prepending movement that must
        // be finished in the case of climbing/jumping, and bringing an NPC to a
        // stop. In the second case, we should never be starting climb or jump
        // movement.
        m_pClippedWaypoints->RemoveAll();
        return false;
      }
    ...
    if (!GetPath()->IsEmpty()) GetPath()->ClearWaypoints();
...
void CAI_Navigator::DrawDebugRouteOverlay(void) {
  ...
  if (waypoint) {
    ...
    NDebugOverlay::Line(GetLocalOrigin(), waypoint->GetPos(), RGB[0], RGB[1],
                        RGB[2], true, 0);
  }

  while (waypoint) {
    Vector RGB = GetWaypointColor(waypoint->NavType());
    NDebugOverlay::Box(waypoint->GetPos(), Vector(-3, -3, -3), Vector(3, 3, 3),
                       RGB[0], RGB[1], RGB[2], true, 0);

    if (waypoint->GetNext()) {
      Vector RGB = GetRouteColor(waypoint->GetNext()->NavType(),
                                 waypoint->GetNext()->Flags());
      NDebugOverlay::Line(waypoint->GetPos(), waypoint->GetNext()->GetPos(),
                          RGB[0], RGB[1], RGB[2], true, 0);
    }
  ...
  if (GetPath()->GoalType() != GOALTYPE_NONE) {
    ...
    NDebugOverlay::Line(vecGoalPos, vecGoalPos + vecGoalDir * 32, 0, 0, 255,
                        true, 2.0);
