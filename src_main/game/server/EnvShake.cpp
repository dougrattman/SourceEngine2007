...
//
// Purpose: Implements a screen shake effect that can also shake physics
// objects.
//
...
class CPhysicsShake : public IMotionEvent {
 ...
 public:
  virtual simresult_e Simulate(IPhysicsMotionController *pController,
    ...
    Vector contact;
    if (!pObject->GetContactPoint(&contact, NULL)) return SIM_NOTHING;

    // fudge the force a bit to make it more dramatic
    pObject->CalculateForceOffset(m_force * (1.0f + pObject->GetMass() * 0.4f),
                                  contact, &linear, &angular);
...
BEGIN_DATADESC(CEnvShake)
  ...
  DEFINE_KEYFIELD(m_Amplitude, FIELD_FLOAT, "amplitude"),
      ...
      DEFINE_PHYSPTR(m_pShakeController), DEFINE_EMBEDDED(m_shakeCallback),
...
#define SF_SHAKE_NO_VIEW \
  0x0020  // DON'T shake the view (only ropes and/or physics objects)
...
float CEnvShake::Radius(bool bPlayers) {
  ...
  if (bPlayers && HasSpawnFlags(SF_SHAKE_EVERYONE)) return 0;
  return m_Radius;
}
...
void CEnvShake::Spawn(void) {
  ...
  if (HasSpawnFlags(SF_SHAKE_NO_VIEW) && !HasSpawnFlags(SF_SHAKE_PHYSICS) &&
      !HasSpawnFlags(SF_SHAKE_ROPES)) {
    DevWarning(
        "env_shake %s with \"Don't shake view\" spawnflag set without \"Shake "
        "physics\" or \"Shake ropes\" spawnflags set.",
        GetDebugName());
  }
}
...
void CEnvShake::ApplyShake(ShakeCommand_t command) {
  if (!HasSpawnFlags(SF_SHAKE_NO_VIEW) || !HasSpawnFlags(SF_SHAKE_NO_RUMBLE)) {
    ...
    UTIL_ScreenShake(GetAbsOrigin(), Amplitude(), Frequency(), Duration(),
                     Radius(), command, air);
  }
  ...
  if (GetSpawnFlags() & SF_SHAKE_PHYSICS) {
    ...
    switch (command) {
      ...
      case SHAKE_START_RUMBLEONLY: {
        ...
        extents.z = std::max(extents.z, 100.0f);
        ...
        for (int i = 0; i < count; i++) {
          //
          // Only shake physics entities that players can see. This is one frame
          // out of date so it's possible that we could miss objects if a player
          // changed PVS this frame.
          //
...
void CEnvShake::Think(void) {
  ...
  if (gpGlobals->curtime > m_nextShake) {
    ...
    m_maxForce *=
        m_currentAmp * 400;  // amplitude is the acceleration of a 100kg object
  }
  ...
  m_currentAmp -=
      m_currentAmp * (gpGlobals->frametime / (Duration() * Frequency()));
  SetNextThink(gpGlobals->curtime);
}
...
void CC_Shake(void) {
  ...
  if (pPlayer) {
    UTIL_ScreenShake(pPlayer->WorldSpaceCenter(), 25.0, 150.0, 1.0, 750,
                     SHAKE_START);
  }
}
