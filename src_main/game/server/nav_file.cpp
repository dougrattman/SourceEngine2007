...
class PlaceDirectory {
 public:
  ...
  void Reset(void) { m_directory.RemoveAll(); }
  ...
  IndexType GetIndex(Place place) const {
    if (place == UNDEFINED_PLACE) return 0;
  ...
  void AddPlace(Place place) {
    if (place == UNDEFINED_PLACE) return;
  ...
  Place IndexToPlace(IndexType entry) const {
    if (entry == 0) return UNDEFINED_PLACE;
...
char *GetBspFilename(const char *navFilename) {
  ...
  Q_snprintf(bspFilename, sizeof(bspFilename), "maps\\%s.bsp",
...
{
        fprintf( fp, "v  %f %f %f\n", m_extent.lo.x, m_extent.lo.y,
m_extent.lo.z ); fprintf( fp, "v  %f %f %f\n", m_extent.hi.x, m_extent.lo.y,
m_neZ ); fprintf( fp, "v  %f %f %f\n", m_extent.hi.x, m_extent.hi.y,
m_extent.hi.z ); fprintf( fp, "v  %f %f %f\n", m_extent.lo.x, m_extent.hi.y,
m_swZ );

        static int base = 1;
        fprintf( fp, "\n\ng %04dArea%s%s%s%s\n", m_id,
                                (GetAttributes() & BOT_NAV_CROUCH) ? "CROUCH" :
"", (GetAttributes() & BOT_NAV_JUMP) ? "JUMP" : "", (GetAttributes() &
BOT_NAV_PRECISE) ? "PRECISE" : "", (GetAttributes() & BOT_NAV_NO_JUMP) ?
"NO_JUMP" : "" ); fprintf( fp, "f %d %d %d %d\n\n", base, base+1, base+2, base+3
); base += 4;
}
*/
...
void CNavArea::Save(FileHandle_t file, unsigned int version) const {
  ...
  for (int a = 0; a < m_approachCount; ++a) {
    if (m_approach[a].here.area)
      filesystem->Write(&m_approach[a].here.area->m_id, sizeof(unsigned int),
                        file);
    else
      filesystem->Write(&zero, sizeof(unsigned int), file);

    if (m_approach[a].prev.area)
      filesystem->Write(&m_approach[a].prev.area->m_id, sizeof(unsigned int),
                        file);
    else
      filesystem->Write(&zero, sizeof(unsigned int), file);
    ...
    if (m_approach[a].next.area)
      filesystem->Write(&m_approach[a].next.area->m_id, sizeof(unsigned int),
                        file);
    else
      filesystem->Write(&zero, sizeof(unsigned int), file);
  ...
  //
  {
    ...
    FOR_EACH_LL(m_spotEncounterList, it) {
      ...
      FOR_EACH_LL(e->spotList, sit) {
        ...
        // order->spot may be NULL if we've loaded a nav mesh that has been
        // edited but not re-analyzed
  ...
  for (i = 0; i < MAX_NAV_TEAMS; ++i) {
    ...
    filesystem->Write(&m_earliestOccupyTime[i], sizeof(m_earliestOccupyTime[i]),
                      file);
  }
}
...
void CNavArea::Load(FileHandle_t file, unsigned int version) {
  ...
  for (int i = 0; i < MAX_NAV_TEAMS; ++i) {
    // no spot in the map should take longer than this to reach
    filesystem->Read(&m_earliestOccupyTime[i], sizeof(m_earliestOccupyTime[i]),
                     file);
  }
}
...
NavErrorType CNavArea::PostLoad(void) {
  ...
  for (int dir = 0; dir < CNavLadder::NUM_LADDER_DIRECTIONS; ++dir) {
    FOR_EACH_LL(m_ladder[dir], it) {
      ...
      if (TheNavMesh->GetLadders().Find(connect.ladder) ==
          TheNavMesh->GetLadders().InvalidIndex()) {
        connect.ladder = TheNavMesh->GetLadderByID(id);
      }

      if (id && connect.ladder == NULL) {
        Msg("CNavArea::PostLoad: Corrupt navigation ladder data. Cannot "
            "connect Navigation Areas.\n");
  ...
  for (int d = 0; d < NUM_DIRECTIONS; d++) {
    FOR_EACH_LL(m_connect[d], it) {
      ...
      if (id && connect->area == NULL) {
        Msg("CNavArea::PostLoad: Corrupt navigation data. Cannot connect "
            "Navigation Areas.\n");
        error = NAV_CORRUPT_DATA;
      }
    }
  ...
  for (int a = 0; a < m_approachCount; ++a) {
    ...
    if (m_approach[a].here.id && m_approach[a].here.area == NULL) {
      Msg("CNavArea::PostLoad: Corrupt navigation data. Missing Approach Area "
          "(here).\n");
      error = NAV_CORRUPT_DATA;
    }
  ...
  FOR_EACH_LL(m_spotEncounterList, it) {
    ...
    if (e->from.area == NULL) {
      Msg("CNavArea::PostLoad: Corrupt navigation data. Missing \"from\" "
          "Navigation Area for Encounter Spot.\n");
      error = NAV_CORRUPT_DATA;
    }

    e->to.area = TheNavMesh->GetNavAreaByID(e->to.id);
    if (e->to.area == NULL) {
      Msg("CNavArea::PostLoad: Corrupt navigation data. Missing \"to\" "
          "Navigation Area for Encounter Spot.\n");
      error = NAV_CORRUPT_DATA;
    }
    ...
    FOR_EACH_LL(e->spotList, sit) {
      ...
      if (order->spot == NULL) {
        Msg("CNavArea::PostLoad: Corrupt navigation data. Missing Hiding "
            "Spot\n");
        error = NAV_CORRUPT_DATA;
      }
    }
  ...
  FOR_EACH_LL(TheNavAreaList, nit) {
    ...
    if (IsOverlapping(area)) m_overlapList.AddToTail(area);
  }
...
void CNavArea::ComputeEarliestOccupyTimes(void) {
  ...
  for (spot = gEntList.FindEntityByClassname(NULL, "info_player_terrorist");
    ...
    float travelDistance =
        NavAreaTravelDistance(spot->GetAbsOrigin(), m_center, cost);
    if (travelDistance < 0.0f) continue;
  ...
  for (spot =
           gEntList.FindEntityByClassname(NULL, "info_player_counterterrorist");
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Determine if this area is a "battlefront" area - where two rushing teams
 * first meet.
 */
...
const char *CNavMesh::GetFilename(void) const {
  // filename is local to game dir for Steam, so we need to prepend game dir for
  // regular file save
  char gamePath[256];
  engine->GetGameDir(gamePath, 256);

  static char filename[256];
  Q_snprintf(filename, sizeof(filename), "%s\\maps\\%s.nav", gamePath,
             STRING(gpGlobals->mapname));
...
static void WarnIfMeshNeedsAnalysis(void) {
  // Quick check to warn about needing to analyze: nav_strip, nav_delete, etc
  // set every CNavArea's m_approachCount to 0, and delete their
  // m_spotEncounterList. So, if no area has either, odds are good we need an
  // analyze.
  {
...
bool CNavMesh::Save(void) const {
  ...
  // get size of source bsp file for later (before we open the nav file for
  // writing, in case of failure)
...
static NavErrorType CheckNavFile(const char *bspFilename) {
  if (!bspFilename) return NAV_CANT_ACCESS_FILE;
  ...
  FileHandle_t file = filesystem->Open(
      filename, "rb",
      "MOD");  // this ignores .nav files embedded in the .bsp ...
  if (!file) {
    navIsInBsp = true;
    file = filesystem->Open(
        filename, "rb",
        "GAME");  // ... and this looks for one if it's the only one around.
  }
...
void CommandNavCheckFileConsistency(void) {
  ...
  const char *bspFilename =
      filesystem->FindFirstEx("maps/*.bsp", "MOD", &findHandle);
  while (bspFilename) {
...
static ConCommand nav_check_file_consistency(
    "nav_check_file_consistency", CommandNavCheckFileConsistency,
    "Scans the maps directory and reports any missing/out-of-date navigation "
    "files.",
...
NavErrorType CNavMesh::Load(void) {
  ...
  char filename[256];
  Q_snprintf(filename, sizeof(filename), "maps\\%s.nav",
             STRING(gpGlobals->mapname));
  ...
  if (version >= 4) {
    ...
    if (bspSize != saveBspSize && !navIsInBsp) {
      if (engine->IsDedicatedServer()) {
        // Warning doesn't print to the dedicated server console, so we'll use
        // Msg instead
        Msg("The Navigation Mesh was built using a different version of this "
            "map.\n");
      } else {
        Warning(
            "The Navigation Mesh was built using a different version of this "
            "map.\n");
      }
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (areaExtent.lo.x >= areaExtent.hi.x ||
        areaExtent.lo.y >= areaExtent.hi.y)
      Warning("WARNING: Degenerate Navigation Area #%d at ( %g, %g, %g )\n",
              area->GetID(), area->m_center.x, area->m_center.y,
              area->m_center.z);

    if (areaExtent.lo.x < extent.lo.x) extent.lo.x = areaExtent.lo.x;
    if (areaExtent.lo.y < extent.lo.y) extent.lo.y = areaExtent.lo.y;
    if (areaExtent.hi.x > extent.hi.x) extent.hi.x = areaExtent.hi.x;
    if (areaExtent.hi.y > extent.hi.y) extent.hi.y = areaExtent.hi.y;
  }
