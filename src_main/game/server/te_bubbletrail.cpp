...
// Input  : *name -
//-----------------------------------------------------------------------------
CTEBubbleTrail::CTEBubbleTrail(const char* name) : CBaseTempEntity(name) {
  m_vecMins.Init();
  m_vecMaxs.Init();
  m_flWaterZ = 0.0;
  m_nModelIndex = 0;
...
//			*current_angles -
//-----------------------------------------------------------------------------
void CTEBubbleTrail::Test(const Vector& current_origin,
...
SendPropVector(SENDINFO(m_vecMins), -1, SPROP_COORD),
    ...
    SendPropFloat(SENDINFO(m_flWaterZ), 17, 0, MIN_COORD_INTEGER,
                  MAX_COORD_INTEGER),
    SendPropInt(SENDINFO(m_nCount), BUBBLE_TRAIL_COUNT_BITS, SPROP_UNSIGNED),
    SendPropFloat(SENDINFO(m_fSpeed), 17, 0, MIN_COORD_INTEGER,
                  MAX_COORD_INTEGER),
    END_SEND_TABLE()
...
//			speed -
//-----------------------------------------------------------------------------
void TE_BubbleTrail(IRecipientFilter& filter, float delay, const Vector* mins,
                    const Vector* maxs, float flWaterZ, int modelindex,
  ...
  g_TEBubbleTrail.m_nCount = std::min(count, (int)BUBBLE_TRAIL_MAX_COUNT);
