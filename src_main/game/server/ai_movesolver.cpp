...
//-----------------------------------------------------------------------------
inline float simpleRound(float f) { return (float)((int)(f + 0.5)); }
...
void CAI_MoveSolver::VisualizeRegulations(const Vector &origin) {
  if (m_Regulations.Count()) {
    ...
    NormalizeSuggestions(&regulations[0],
                         (&regulations[0]) + regulations.Count());

    Vector side1, mid, side2;
    for (int i = regulations.Count(); --i >= 0;) {
      // Compute the positions of the angles...
      float flMinAngle =
          regulations[i].arc.center - regulations[i].arc.span * 0.5f;
      float flMaxAngle =
          regulations[i].arc.center + regulations[i].arc.span * 0.5f;
...
//-------------------------------------
bool CAI_MoveSolver::Solve(const AI_MoveSuggestion_t *pSuggestions,
                           int nSuggestions, AI_MoveSolution_t *pResult) {
  //---------------------------------
  //
  // Quick out
  //
  ...
  if (nSuggestions == 1 && m_Regulations.Count() == 0 &&
      pSuggestions->type == AIMST_MOVE) {
  ...
  // The first thing we do is reweight and normalize the weights into a range of
  // [-1..1], where a negative weight is a repulsion. This becomes a bias for
  // the solver.
  // @TODO (toml 06-18-02): this can be made sligtly more optimal by
  // precalculating regulation adjusted weights
  Assert(suggestions.Count() >= 1);
  NormalizeSuggestions(&suggestions[0],
                       (&suggestions[0]) + suggestions.Count());
  ...
  for (int iSuggestion = 0; iSuggestion < suggestions.Count(); ++iSuggestion) {
    ...
    // Convert arc values to solution indices relative to right post. Right is
    // angle down, left is angle up.
    float halfSpan = current.arc.span * 0.5;
    int center = simpleRound((halfSpan * NUM_SOLUTIONS) / 360);
    ...
    const float positiveDegradePer180 =
        0.05;  // i.e., lose 5% of weight by the time hit 180 degrees off center
    const float positiveDegrade =
        (positiveDegradePer180 / (NUM_SOLUTIONS * 0.5));
...
//-------------------------------------
// Purpose: Adjusts the suggestion weights according to the type of the
// suggestion,
//			apply the appropriate sign, ensure values are in
// expected  ranges
//-------------------------------------
...
static AI_MoveSuggWeights g_AI_MoveSuggWeights[] =  // @TODO (toml 06-18-02):
                                                    // these numbers need tuning
    {
...
void CAI_MoveSolver::NormalizeSuggestions(AI_MoveSuggestion_t *pBegin,
...
CON_COMMAND(ai_test_move_solver, "Tests the AI move solver system") {
  ...
  //
  {
    ...
    for (i = 0; i < 360; ++i) {
      Assert(
          solver.Solve(AI_MoveSuggestion_t(AIMST_MOVE, 1, i, 180), &solution));
