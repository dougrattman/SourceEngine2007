...
ConVar ai_follow_use_points_when_moving("ai_follow_use_points_when_moving",
                                        "1");
#define FollowMsg(s)                             \
...
//
//			Right now, this is in a very preliminary sketch state.
//(toml 03-03-03)
//-----------------------------------------------------------------------------
...
class CAI_FollowManager {
 public:
  ~CAI_FollowManager() {
    for (int i = 0; i < m_groups.Count(); i++) delete m_groups[i];
  }

  bool AddFollower(CBaseEntity *pTarget, CAI_BaseNPC *pFollower,
                   AI_Formations_t formation,
                   AI_FollowManagerInfoHandle_t *pHandle);
  void ChangeFormation(AI_FollowManagerInfoHandle_t &handle,
                       AI_Formations_t formation);
  ...
  int CountFollowers(CBaseEntity *pFollowTarget, string_t iszClassname) {
    ...
    if (iszClassname == NULL_STRING) {
      ...
      int result = 0;
      for (int i = pGroup->followers.Head();
           i != pGroup->followers.InvalidIndex();
           i = pGroup->followers.Next(i)) {
        if (pGroup->followers[i].hFollower &&
            pGroup->followers[i].hFollower->ClassMatches(iszClassname)) {
          result++;
        }
      }
      return result;
 ...
 private:
  ...
  void CalculateFieldsFromSlot(AI_FollowSlot_t *pSlot,
                               AI_FollowNavInfo_t *pFollowerInfo);
...
BEGIN_SIMPLE_DATADESC(AI_FollowNavInfo_t)
  DEFINE_FIELD(flags, FIELD_INTEGER),
      ...
      DEFINE_FIELD(range, FIELD_FLOAT), DEFINE_FIELD(Zrange, FIELD_FLOAT),
...
BEGIN_DATADESC(CAI_FollowBehavior)
  DEFINE_FIELD(m_hFollowTarget, FIELD_EHANDLE),
      ...
      DEFINE_EMBEDDED(m_FollowDelay), DEFINE_EMBEDDED(m_RepathOnFollowTimer),
      ...
      DEFINE_EMBEDDED(m_params), DEFINE_FIELD(m_hFollowGoalEnt, FIELD_EHANDLE),
...
int CAI_FollowBehavior::DrawDebugTextOverlays(int text_offset) {
  ...
  if (GetOuter()->m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    if (followEnt != NULL) {
      Q_snprintf(tempstr, sizeof(tempstr), "Follow: (%d) %s (%s)",
                 followEnt->entindex(), followEnt->GetDebugName(),
                 followEnt->GetClassname());
    } else {
...
void CAI_FollowBehavior::SetFollowTarget(CBaseEntity *pLeader,
                                         bool fFinishCurSchedule) {
  if (pLeader == m_hFollowTarget) return;
  ...
  if (m_hFollowTarget) {
    ...
    if (IsRunning()) {
      if (GetNavigator()->GetGoalType() == GOALTYPE_TARGETENT) {
        GetNavigator()
            ->StopMoving();  // Stop him from walking toward the player
      }
  ...
  if (pLeader) {
    if (g_AIFollowManager.AddFollower(pLeader, GetOuter(), m_params.formation,
                                      &m_hFollowManagerInfo)) {
...
bool CAI_FollowBehavior::SetFollowGoal(CAI_FollowGoal *pGoal,
                                       bool fFinishCurSchedule) {
  if (GetOuter()->ShouldAcceptGoal(this, pGoal)) {
    ...
    Assert(pGoal->m_iFormation == AIF_SIMPLE ||
           pGoal->m_iFormation == AIF_WIDE ||
           pGoal->m_iFormation == AIF_MEDIUM ||
           pGoal->m_iFormation == AIF_SIDEKICK ||
           pGoal->m_iFormation == AIF_VORTIGAUNT);
...
bool CAI_FollowBehavior::UpdateFollowPosition() {
  ...
  if (m_hFollowTarget == NULL) return false;

  if (!g_AIFollowManager.CalcFollowPosition(m_hFollowManagerInfo,
                                            &m_FollowNavGoal)) {
    return false;
  }
  ...
  if (pFollowTarget->GetParent()) {
    if (pFollowTarget->GetParent()->GetServerVehicle()) {
      ...
      m_FollowNavGoal.range +=
          pFollowTarget->GetParent()->BoundingRadius() * 0.333;
    }
  }

#if TODO
  // @TODO (toml 07-27-03): this is too simplistic. fails when the new point is
...
bool CAI_FollowBehavior::IsMovingToFollowTarget() {
  return (IsRunning() &&
          (IsCurSchedule(SCHED_FOLLOW, false) ||
           IsCurSchedule(SCHED_FOLLOWER_GO_TO_WAIT_POINT, false)));
}

//-------------------------------------

bool CAI_FollowBehavior::CanSelectSchedule() {
  if (!GetOuter()->IsInterruptable()) return false;
...
bool CAI_FollowBehavior::PlayerIsPushing() {
  return (m_hFollowTarget && m_hFollowTarget->IsPlayer() &&
          HasCondition(COND_PLAYER_PUSHING));
}
...
bool CAI_FollowBehavior::IsFollowTargetInRange(float rangeMultiplier) {
  ...
  if (GetNpcState() == NPC_STATE_COMBAT) {
    if (IsFollowGoalInRange(std::max(m_FollowNavGoal.coverTolerance,
                                     m_FollowNavGoal.enemyLOSTolerance) *
                                rangeMultiplier,
                            GetGoalZRange(), GetGoalFlags())) {
      return true;
    }
  } else {
    if (IsFollowGoalInRange(
            std::max(m_FollowNavGoal.tolerance, GetGoalRange()) *
                rangeMultiplier,
            GetGoalZRange(), GetGoalFlags())) {
...
bool CAI_FollowBehavior::IsFollowGoalInRange(float tolerance, float zTolerance,
                                             int flags) {
  const Vector &origin = WorldSpaceCenter();
  const Vector &goal = GetGoalPosition();
  if (zTolerance == -1) zTolerance = GetHullHeight();
  ...
  zTolerance +=
      zTolerance * std::clamp((distanceSq / flIncreaseRange), 0.0f, 1.0f);
  if (fabs(origin.z - goal.z) > zTolerance) return false;

  if (distanceSq > flToleranceSq) return false;

  if (flags & AIFF_REQUIRE_LOS_OUTSIDE_COMBAT && m_hFollowTarget.Get()) {
    if (!GetOuter()->GetSenses()->DidSeeEntity(m_hFollowTarget)) return false;
  }
...
bool CAI_FollowBehavior::IsChaseGoalInRange() {
  if (GetEnemy() &&
      (GetEnemy()->WorldSpaceCenter() - m_FollowNavGoal.position).LengthSqr() >
          Square(m_FollowNavGoal.chaseEnemyTolerance))
    return false;
...
void CAI_FollowBehavior::BeginScheduleSelection() {
  if (GetOuter()->m_hCine) GetOuter()->m_hCine->CancelScript();
  ...
  if (!m_params.bNormalMemoryDiscard) {
    ...
    m_flOriginalEnemyDiscardTime =
        GetOuter()->GetEnemies()->GetEnemyDiscardTime();
...
void CAI_FollowBehavior::CleanupOnDeath(CBaseEntity *pCulprit,
...
void CAI_FollowBehavior::Precache() {
  if (m_hFollowTarget != NULL && m_hFollowManagerInfo.m_pGroup == NULL) {
    // Post load fixup
    if (!g_AIFollowManager.AddFollower(m_hFollowTarget, GetOuter(),
                                       m_params.formation,
                                       &m_hFollowManagerInfo)) {
      m_hFollowTarget = NULL;
    }
...
void CAI_FollowBehavior::GatherConditions(void) {
  ...
  if (GetHintNode() == NULL) {
    if (ShouldUseFollowPoints() && m_TimeBlockUseWaitPoint.Expired() &&
        m_TimeCheckForWaitPoint.Expired()) {
      m_TimeCheckForWaitPoint.Reset();
      m_pInterruptWaitPoint = FindFollowPoint();
      if (m_pInterruptWaitPoint) SetCondition(COND_FOUND_WAIT_POINT);
    }
  }

  if (m_flTimeUpdatedFollowPosition == 0 ||
      gpGlobals->curtime - m_flTimeUpdatedFollowPosition > 2.0)
  ...
  if ((GetFollowTarget()->IsPlayer() && HasCondition(COND_SEE_PLAYER)) ||
      GetOuter()->FVisible(GetFollowTarget())) {
  ...
  if (HasFollowPoint() &&
      (m_flTimeFollowTargetVisible != 0 &&
       gpGlobals->curtime - m_flTimeFollowTargetVisible > 5.0))
...
int CAI_FollowBehavior::SelectFailSchedule(int failedSchedule, int failedTask,
                                           AI_TaskFailureCode_t taskFailCode) {
  if (failedTask == TASK_MOVE_TO_FOLLOW_POSITION ||
      failedTask == TASK_GET_PATH_TO_FOLLOW_POSITION) {
    if (m_hFollowTarget) {
      m_TargetMonitor.SetMark(m_hFollowTarget,
                              m_FollowNavGoal.targetMoveTolerance * 0.5);
      m_FollowDelay.Start();
      NoteFailedFollow();
    }
  }
...
bool CAI_FollowBehavior::ShouldFollow() {
  if (!GetFollowTarget()) return false;
  ...
  if (m_bFollowNavFailed && m_FollowDelay.IsRunning() &&
      !m_FollowDelay.Expired()) {
    return false;
  }
...
bool CAI_FollowBehavior::ShouldMoveToFollowTarget() {
  if (GetFollowTarget() == NULL) return false;
  ...
  if (HL2GameRules()->IsAlyxInDarknessMode()) {
    ...
    if (HasCondition(COND_FOLLOW_PLAYER_IS_NOT_LIT)) return false;
  }
#endif

  if (HasFollowPoint()) {
    if (IsFollowPointInRange()) return false;
  } else if (IsFollowTargetInRange())
    return false;

  if (m_FollowDelay.IsRunning() && !m_FollowDelay.Expired() &&
      !HasCondition(COND_TARGET_MOVED_FROM_MARK))
    return false;
...
int CAI_FollowBehavior::SelectScheduleManagePosition() {
  if (PlayerIsPushing()) return SCHED_MOVE_AWAY;
...
bool CAI_FollowBehavior::ShouldUseFollowPoints() {
  if (!ai_follow_use_points.GetBool() || GetEnemy() != NULL) return false;
...
CAI_Hint *CAI_FollowBehavior::FindFollowPoint() {
  if (!m_TimeBlockUseWaitPoint.Expired()) return NULL;
  ...
  hintCriteria.AddIncludePosition(
      GetGoalPosition(),
      std::max(m_FollowNavGoal.followPointTolerance, GetGoalRange()));
  hintCriteria.AddExcludePosition(
      GetGoalPosition(), (GetFollowTarget()->WorldAlignMins().AsVector2D() -
                          GetFollowTarget()->WorldAlignMaxs().AsVector2D())
                             .Length());
...
bool CAI_FollowBehavior::IsFollowPointInRange() {
  return (GetHintNode() &&
          ...
          (GetHintNode()->GetAbsOrigin() - GetFollowTarget()->GetAbsOrigin())
                  .LengthSqr() <
              Square(std::max(m_FollowNavGoal.followPointTolerance,
                              GetGoalRange())));
}
...
void CAI_FollowBehavior::SetFollowPoint(CAI_Hint *pHintNode) {
  ...
  if (GetHintNode() == pHintNode) return;
...
int CAI_FollowBehavior::SelectScheduleFollowPoints() {
  bool bShouldUseFollowPoints =
      (ShouldUseFollowPoints() &&
       IsFollowGoalInRange(m_FollowNavGoal.followPointTolerance + 0.1,
                           GetGoalZRange(), GetGoalFlags()));
  ...
  if (bHasFollowPoint) {
    distSqToPoint =
        (GetHintNode()->GetAbsOrigin() - GetAbsOrigin()).LengthSqr();
  ...
  if (bShouldUseFollowPoints) {
    ...
    if (!GetHintNode()) {
      bNewHint = true;
      SetFollowPoint((m_pInterruptWaitPoint) ? m_pInterruptWaitPoint
                                             : FindFollowPoint());
...
int CAI_FollowBehavior::SelectScheduleMoveToFormation() {
  if ((GetNpcState() != NPC_STATE_COMBAT &&
    ...
    if (pSquad) {
      for (CAI_BaseNPC *pSquadMember = pSquad->GetFirstMember(&iter);
           pSquadMember; pSquadMember = pSquad->GetNextMember(&iter)) {
    ...
    if (ShouldMoveToFollowTarget() || m_bFirstFacing) {
      return SCHED_TARGET_FACE;  // Code for "SCHED_MOVE_TO_FACE_FOLLOW_TARGET".
...
int CAI_FollowBehavior::SelectSchedule() {
  ...
  if (hl2_episodic.GetBool()) {
    // Range attack
    if (GetOuter()->ShouldMoveAndShoot() == false &&
  ...
  if (GetFollowTarget()) {
    if (!GetFollowTarget()->IsAlive()) {
      ...
      if (result != SCHED_NONE) return result;
      ...
      if (HasCondition(COND_NO_PRIMARY_AMMO) &&
          HaveSequenceForActivity(GetOuter()->TranslateActivity(ACT_RUN_AIM)))
...
int CAI_FollowBehavior::TranslateSchedule(int scheduleType) {
  switch (scheduleType) {
    ...
    case SCHED_IDLE_STAND: {
      if (ShouldMoveToFollowTarget() &&
          !IsFollowGoalInRange(GetGoalRange(), GetGoalZRange(),
                               GetGoalFlags())) {
    ...
    case SCHED_TARGET_FACE: {
      ...
      return SCHED_FACE_FOLLOW_TARGET;  // @TODO (toml 03-03-03): should select
                                        // a facing sched
    }
    ...
    // SCHED_ESTABLISH_LINE_OF_FIRE_FALLBACK just tells the NPC to chase their
    // enemy, so forbid this unless the destination is acceptable within the
    // parameters of the follow behavior.
    case SCHED_CHASE_ENEMY:
    case SCHED_ESTABLISH_LINE_OF_FIRE_FALLBACK: {
      if (IsChaseGoalInRange() == false) return SCHED_FOLLOWER_IDLE_STAND;
      break;
    }
    ...
    case SCHED_RANGE_ATTACK1: {
      if (GetOuter()->GetShotRegulator()->IsInRestInterval()) {
        ...
        return SCHED_FOLLOWER_IDLE_STAND;  // @TODO (toml 07-02-03): Should do
                                           // something more tactically sensible
      }
      break;
    }
    ...
    case SCHED_CHASE_ENEMY_FAILED: {
      if (HasMemory(bits_MEMORY_INCOVER)) {
        ...
        if (GetFollowTarget()) {
          float fDist =
              (GetLocalOrigin() - GetFollowTarget()->GetAbsOrigin()).Length();
          if (fDist > 500) {
            return SCHED_FOLLOW;
          }
        }
...
bool CAI_FollowBehavior::ValidateFaceTarget(Vector *pFaceTarget) {
  ...
  trace_t tr;
  AI_TraceLine(GetOuter()->EyePosition(), testPoint, MASK_BLOCKLOS,
               m_hFollowTarget, COLLISION_GROUP_NONE, &tr);
...
bool CAI_FollowBehavior::FindCoverFromEnemyAtFollowTarget(float coverRadius,
                                                          Vector *pResult) {
  CBaseEntity *pEntity = GetEnemy();

  return GetOuter()->FindCoverPosInRadius(pEntity, m_FollowNavGoal.position,
                                          coverRadius, pResult);
}
...
void CAI_FollowBehavior::StartTask(const Task_t *pTask) {
  ...
  switch (pTask->iTask) {
    ...
    case TASK_FACE_FOLLOW_TARGET: {
      ...
      bool bFollowingPoint =
          (dynamic_cast<CPointEntity *>(m_hFollowTarget.Get()) != NULL);
      if (GetNpcState() == NPC_STATE_COMBAT) {
        ...
        if (m_bFirstFacing && m_hFollowTarget->IsPlayer()) {
          ...
          if (bIsEpisodicVitalAlly) {
            ...
            if (roll == 1) {
              ...
              if (count > 0) {
                // Slice up the directions among followers and leader. ( +1
                // because we count the leader!)
    ...
    case TASK_MOVE_TO_FOLLOW_POSITION: {
      if (m_hFollowTarget == NULL) {
        TaskFail(FAIL_NO_TARGET);
      } else if ((m_hFollowTarget->GetAbsOrigin() - GetAbsOrigin()).Length() <
                 1) {
        TaskComplete();
    ...
    case TASK_FIND_COVER_FROM_ENEMY: {
      ...
      if (pLeader) {
        ...
        float coverRadius =
            std::min(GetOuter()->CoverRadius(), m_FollowNavGoal.coverTolerance);
        ...
        if (FindCoverFromEnemyAtFollowTarget(coverRadius, &coverPos)) {
          ...
          GetOuter()->m_flMoveWaitFinished =
              gpGlobals->curtime + pTask->flTaskData;
          TaskComplete();
        } else
  ...
  switch (pTask->iTask) {
      ...
      switch (GetOuter()->GetTaskInterrupt()) {
        case 0: {
          if (GetEnemy()) {
            ...
            Vector coverPos = vec3_invalid;
            float coverRadius =
                std::min((float)12 * 12, m_FollowNavGoal.coverTolerance);
            if (FindCoverFromEnemyAtFollowTarget(coverRadius, &coverPos)) {
        ...
        case 1: {
          if (GetOuter()->m_vInterruptSavePosition != vec3_invalid) {
            AI_NavGoal_t goal(GOALTYPE_COVER,
                              GetOuter()->m_vInterruptSavePosition, ACT_RUN,
                              AIN_HULL_TOLERANCE, AIN_DEF_FLAGS);
        ...
        case 2: {
          ...
          AI_NavGoal_t goal(vGoalPosition, AIN_DEF_ACTIVITY,
                            GetGoalTolerance());
          if (!m_hFollowTarget->GetParent() ||
              !m_hFollowTarget->GetParent()->GetServerVehicle()) {
    ...
    case TASK_MOVE_TO_FOLLOW_POSITION: {
      if (m_hFollowTarget == NULL) {
        ...
        // Re-evaluate when you think your finished, or the target has moved too
        // far
        if (!UpdateFollowPosition()) {
          TaskFail(FAIL_NO_TARGET);
          break;
        ...
        if (ShouldUseFollowPoints() &&
            ai_follow_use_points_when_moving.GetBool()) {
          if (HasFollowPoint()) {
          ...
          if (GetNavigator()->GetNavType() != NAV_JUMP && !HasFollowPoint() &&
        ...
        if (!GetNavigator()->IsGoalActive()) {
          // What this probably means is that the navigation failed but within
          // tolerance So for now, just call it good and block another attempt
          // for a bit
          TaskComplete();
          if (!IsFollowPointInRange()) ClearFollowPoint();
          if (!IsFollowGoalInRange(m_FollowNavGoal.tolerance, GetGoalZRange(),
                                   GetGoalFlags()))
            m_FollowDelay.Start(0.25, 0.75);
          else {
            m_TargetMonitor.SetMark(GetFollowTarget(),
                                    m_FollowNavGoal.targetMoveTolerance);
            m_bTargetUnreachable = false;
          }
          break;
        ...
        if (!HasFollowPoint()) {
          ...
          if (m_RepathOnFollowTimer.Expired()) {
            if ((GetNavigator()->GetGoalPos() - GetGoalPosition()).LengthSqr() >
                Square(m_FollowNavGoal.repathOnRouteTolerance)) {
              if (GetNavigator()->GetNavType() != NAV_JUMP) {
                ...
                if (!GetNavigator()->UpdateGoalPos(GetGoalPosition())) {
                  ...
                  if (vToGoal.Length2DSqr() > 6 * 12) {
                    if (GetNavigator()->UpdateGoalPos(vTarget +
                                                      vToGoal * 0.5)) {
                      bSuccess = true;
                    }
                  }
          ...
          if (GetNavigator()->GetGoalPos() != vFollowPoint) {
            ...
            if (!ShouldIgnoreFollowPointFacing())
              GetNavigator()->SetArrivalDirection(
                  GetHintNode()->GetDirection());
            if (GetHintNode()->HintActivityName() != NULL_STRING) {
              ...
              if (hintActivity != ACT_INVALID) {
                GetNavigator()->SetArrivalActivity(GetOuter()->GetHintActivity(
                    GetHintNode()->HintType(), hintActivity));
              } else {
                int iSequence = GetOuter()->LookupSequence(
                    STRING(GetHintNode()->HintActivityName()));
        ...
        if (m_CurrentFollowActivity != ACT_RUN) {
          float distToTargetSq =
              (GetNavigator()->GetGoalPos() - GetLocalOrigin()).Length2DSqr();

          // Pick the right movement activity.
          Activity followActivity =
              (distToTargetSq < Square(m_FollowNavGoal.walkTolerance) &&
               GetOuter()->GetState() != NPC_STATE_COMBAT)
                  ? ACT_WALK
                  : ACT_RUN;
        ...
        if ((m_vFollowMoveAnchor - GetAbsOrigin()).LengthSqr() >
            Square(15.0 * 12.0)) {
          m_vFollowMoveAnchor = GetAbsOrigin();
          NoteSuccessfulFollow();
        }
...
void CAI_FollowBehavior::TaskComplete(bool fIgnoreSetFailedCondition) {
  ...
  if (pTask->iTask == TASK_MOVE_TO_FOLLOW_POSITION ||
      pTask->iTask == TASK_GET_PATH_TO_FOLLOW_POSITION)
    NoteSuccessfulFollow();
...
void CAI_FollowBehavior::BuildScheduleTestBits() {
  ...
  if ((GetOuter()->ConditionInterruptsCurSchedule(COND_GIVE_WAY) ||
       ...
       IsCurSchedule(SCHED_COMBAT_STAND) || IsCurSchedule(SCHED_ALERT_STAND)) ||
  ...
  if (hl2_episodic.GetBool()) {
    if (IsCurScheduleFollowSchedule() &&
        GetOuter()->ShouldMoveAndShoot() == false) {
    ...
    if (HL2GameRules()->IsAlyxInDarknessMode()) {
      if (IsCurSchedule(SCHED_FOLLOW, false) ||
          IsCurSchedule(SCHED_MOVE_TO_FACE_FOLLOW_TARGET, false) ||
...
Activity CAI_FollowBehavior::NPC_TranslateActivity(Activity activity) {
  if (activity == ACT_IDLE && HasFollowPoint() &&
      GetHintNode()->HintActivityName() != NULL_STRING) {
    return GetOuter()->GetHintActivity(
        GetHintNode()->HintType(), (Activity)CAI_BaseNPC::GetActivityID(STRING(
                                       GetHintNode()->HintActivityName())));
  }
...
bool CAI_FollowBehavior::IsCurScheduleFollowSchedule() {
  int curScheduleId = (GetOuter()->GetCurSchedule())
                          ? GetOuter()->GetCurSchedule()->GetId()
                          : SCHED_NONE;
  if (curScheduleId >= GetClassScheduleIdSpace()->ScheduleLocalToGlobal(
...
bool CAI_FollowBehavior::IsCurTaskContinuousMove() {
  ...
  if (pCurTask && pCurTask->iTask == TASK_MOVE_TO_FOLLOW_POSITION) return true;
...
bool CAI_FollowBehavior::FValidateHintType(CAI_Hint *pHint) {
  if (pHint->HintType() == HINT_FOLLOW_WAIT_POINT) {
    if (GetFollowTarget() &&
        GetFollowTarget()->FVisible(pHint->GetAbsOrigin() + Vector(0, 0, 0.1)))
      return true;
    else
      return false;
  }
...
bool CAI_FollowBehavior::IsValidCover(const Vector &vLocation,
  ...
  if ((vLocation - m_FollowNavGoal.position).LengthSqr() >
      Square(m_FollowNavGoal.coverTolerance + 0.1))
    return false;
...
bool CAI_FollowBehavior::IsValidShootPosition(const Vector &vLocation,
                                              ...
                                              CAI_Hint const *pHint) {
  if ((vLocation - m_FollowNavGoal.position).LengthSqr() >
      Square(m_FollowNavGoal.enemyLOSTolerance + 0.1))
    return false;
...
void CAI_FollowGoal::DisableGoal(CAI_BaseNPC *pAI) {
  CAI_FollowBehavior *pBehavior;
  if (!pAI || !pAI->GetBehavior(&pBehavior)) return;
...
// @TODO (toml 11-21-03): rework follow so we don't have to have class specifc
// formations in this file
...
static AI_FollowSlot_t g_SimpleFollowFormationSlots[] = {
    {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
    {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
    {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
    {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
    {1, {0, 0, 0}, 0, 96, 120, -1, 128}, {1, {0, 0, 0}, 0, 96, 120, -1, 128},
...
static AI_FollowSlot_t g_WideFollowFormationSlots[] = {
    {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
    {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
    {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
    {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
    {1, {0, 0, 0}, 0, 120, 240, -1, 128}, {1, {0, 0, 0}, 0, 120, 240, -1, 128},
...
static AI_FollowSlot_t g_AntlionFollowFormationSlots[] = {
    {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
    {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
    {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
    {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
    {1, {0, 0, 0}, 0, 150, 250, -1, 128}, {1, {0, 0, 0}, 0, 150, 250, -1, 128},
...
static AI_FollowSlot_t g_MediumFollowFormationSlots[] = {
    {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
    {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
    {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
    {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
    {1, {0, 0, 0}, 0, 156, 156, -1, 128}, {1, {0, 0, 0}, 0, 156, 156, -1, 128},
...
//-----------------------------------------------------------------------------
// NOTE: these must correspond with the AI_Formations_t enumeration in
// AI_Behavior_Follow.h!!
//-----------------------------------------------------------------------------
AI_FollowFormation_t *g_AI_Formations[] = {
    &g_SimpleFollowFormation,     &g_WideFollowFormation,
    &g_AntlionFollowFormation,    &g_CommanderFollowFormation,
    &g_TightFollowFormation,      &g_MediumFollowFormation,
    &g_SidekickFollowFormation,   &g_HunterFollowFormation,
...
bool CAI_FollowManager::AddFollower(CBaseEntity *pTarget,
...
bool CAI_FollowManager::CalcFollowPosition(AI_FollowManagerInfoHandle_t &hInfo,
                                           AI_FollowNavInfo_t *pNavInfo) {
  if (hInfo.m_pGroup && hInfo.m_hFollower) {
    ...
    if (iterNode->navInfo.position != vec3_origin) {
      ...
      VectorRotate(iterNode->navInfo.position, fRotateMatrix,
                   pNavInfo->position);
      pNavInfo->position += pTarget->WorldSpaceCenter();
    } else {
      pNavInfo->position =
          iterNode->navInfo.position + pTarget->WorldSpaceCenter();
    }
    ...
    pNavInfo->repathOnRouteTolerance =
        pGroup->pFormation->repathOnRouteTolerance;
...
bool CAI_FollowManager::RedistributeSlots(AI_FollowGroup_t *pGroup) {
  ...
  while ((bestSlot = FindBestSlot(pGroup)) != -1 &&
         ((int)movedFollowers.Count() < pGroup->followers.Count())) {
    ...
    while (h != pGroup->followers.InvalidIndex()) {
      ...
      if (movedFollowers.Find(p->hFollower) == movedFollowers.InvalidIndex() &&
          (p->slot == -1 ||
           pSlot->priority > pGroup->pFormation->pSlots[p->slot].priority)) {
    ...
    if (hBest == pGroup->followers.InvalidIndex()) break;
...
void CAI_FollowManager::ChangeFormation(AI_FollowManagerInfoHandle_t &hInfo,
                                        AI_Formations_t formation) {
  if (!hInfo.m_pGroup || !hInfo.m_hFollower) return;

  AI_FollowGroup_t *pGroup = hInfo.m_pGroup;
  AI_FollowFormation_t *pNewFormation = AIGetFormation(formation);
  if (pNewFormation == pGroup->pFormation) return;
  ...
  while (h != pGroup->followers.InvalidIndex()) {
    ...
    if (pFollowBehavior) {
      ...
      pFollowBehavior->SetCondition(
          CAI_FollowBehavior::COND_TARGET_MOVED_FROM_MARK);
...
int CAI_FollowManager::FindBestSlot(AI_FollowGroup_t *pGroup) {
  ...
  for (int i = 0; i < nSlots; i++) {
    if (!pGroup->slotUsage.IsBitSet(i) &&
        pGroup->pFormation->pSlots[i].priority > bestPriority) {
...
void CAI_FollowManager::CalculateFieldsFromSlot(
    AI_FollowSlot_t *pSlot, AI_FollowNavInfo_t *pFollowerInfo) {
  // @TODO (toml 02-28-03): placeholder. Force break if someone tries to
  // actually use
...
AI_FollowGroup_t *CAI_FollowManager::FindCreateGroup(
    CBaseEntity *pTarget, AI_Formations_t formation) {
...
AI_FollowGroup_t *CAI_FollowManager::FindGroup(CBaseEntity *pTarget) {
  for (int i = 0; i < m_groups.Count(); i++) {
    if (m_groups[i]->hFollowTarget == pTarget) return m_groups[i];
  }
  return NULL;
}
...
AI_FollowGroup_t *CAI_FollowManager::FindFollowerGroup(CBaseEntity *pFollower) {
  for (int i = 0; i < m_groups.Count(); i++) {
    ...
    while (h != m_groups[i]->followers.InvalidIndex()) {
      ...
      if (p->hFollower.Get() == pFollower) return m_groups[i];
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_FOLLOWER_MOVE_AWAY_FAIL "
    "		 TASK_STOP_MOVING					"
    "	0"
...
DEFINE_SCHEDULE(
    ...
    "ACTIVITY:ACT_IDLE"
    "		TASK_SET_SCHEDULE					"
    "SCHEDULE:SCHED_FOLLOWER_IDLE_STAND "
    ""
    "	Interrupts"
    "		COND_NEW_ENEMY"
    "		COND_LIGHT_DAMAGE"
...
DEFINE_SCHEDULE(SCHED_FOLLOWER_GO_TO_WAIT_POINT,
                ...
                "	Tasks"
                "		TASK_LOCK_HINTNODE			"
                "0		"  // this will fail the schedule if no hint
                                   // node or not already lockable
                "		TASK_SET_FAIL_SCHEDULE		"
                "SCHEDULE:SCHED_FOLLOWER_GO_TO_WAIT_POINT_FAIL"
...
DEFINE_SCHEDULE(SCHED_FOLLOWER_STAND_AT_WAIT_POINT,
                ...
                "		TASK_SET_SCHEDULE			"
                "SCHEDULE:SCHED_FOLLOWER_STAND_AT_WAIT_POINT "
                ""
                "	Interrupts"
                "		COND_NEW_ENEMY"
                "		COND_LIGHT_DAMAGE"
