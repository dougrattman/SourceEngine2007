...
//
// Purpose: A link that can be turned on and off.  Unlike normal links dynamic
// links must be entities so they can receive messages. They update the state of
// the actual links. Allows us to save a lot of memory by not making all links
// into entities
      ...
      for (int j = 0; j < nLinks; j++) {
        ...
        if (iLinkDest > i) {
          ...
          if (origin.DistToSqr(originOther) < MinDistCareSq) {
            if (IsBoxIntersectingRay(vAbsMins, vAbsMaxs, nodeOrigin,
                                     originOther - nodeOrigin)) {
              Assert(IsBoxIntersectingRay(vAbsMins, vAbsMaxs, originOther,
                                          nodeOrigin - originOther));
              ...
              pLink->m_nSrcEditID =
                  g_pAINetworkManager->GetEditOps()->GetWCIdFromNodeId(
                      pLink->m_nSrcID);
              pLink->m_nDestEditID =
                  g_pAINetworkManager->GetEditOps()->GetWCIdFromNodeId(
                      pLink->m_nDestID);
...
void CAI_DynamicLinkController::InputTurnOn(inputdata_t &inputdata) {
  for (int i = 0; i < m_ControlledLinks.Count(); i++) {
    if (m_ControlledLinks[i] == NULL) {
      ...
      if (i >= m_ControlledLinks.Count()) break;
    }
...
void CAI_DynamicLink::InitDynamicLinks(void) {
  ...
  while (pDynamicLink) {
    ...
    if (pDynamicLink->m_nSrcID != NO_NODE &&
        pDynamicLink->m_nDestID != NO_NODE) {
      if ((pDynamicLink->GetSpawnFlags() & bits_HULL_BITS_MASK) != 0) {
        ...
        if (!pLink) {
          ...
          if (pNode1 && pNode2) {
            pLink = g_pBigAINet->CreateLink(pDynamicLink->m_nSrcID,
...
//			 there is still a corresponding node link, if not
// removes it
// Input   :
// Output  :
//------------------------------------------------------------------------------
...
#ifdef _WIN32
      int nWCSrcID = g_pAINetworkManager->GetEditOps()
                         ->m_pNodeIndexTable[pDynamicLink->m_nSrcID];
      int nWCDstID = g_pAINetworkManager->GetEditOps()
                         ->m_pNodeIndexTable[pDynamicLink->m_nDestID];
...
CAI_Link *CAI_DynamicLink::FindLink() {
  ...
  if (pSrcNode) {
    ...
    for (int i = 0; i < numLinks; i++) {
      ...
      if (((pLink->m_iSrcID == m_nSrcID) && (pLink->m_iDestID == m_nDestID)) ||

          ((pLink->m_iSrcID == m_nDestID) && (pLink->m_iDestID == m_nSrcID))) {
        return pLink;
      }
    }
  }
...
int CAI_DynamicLink::ObjectCaps() {
  ...
  if (m_bNotSaved) caps |= FCAP_DONT_SAVE;
...
void CAI_DynamicLink::SetLinkState(void) {
  if (!gm_bInitialized) {
    // Safe to quietly return. Consistency will be enforced when
    // InitDynamicLinks() is called
    return;
  }
...
CAI_DynamicLink *CAI_DynamicLink::GetDynamicLink(int nSrcID, int nDstID) {
  ...
  while (pDynamicLink) {
    if ((nSrcID == pDynamicLink->m_nSrcID &&
         nDstID == pDynamicLink->m_nDestID) ||
        (nSrcID == pDynamicLink->m_nDestID &&
         nDstID == pDynamicLink->m_nSrcID)) {
      return pDynamicLink;
    }
  ...
  if (pDynamicLink == this) {
    ...
    while (pDynamicLink) {
      if (pDynamicLink->m_pNextDynamicLink == this) {
        pDynamicLink->m_pNextDynamicLink =
            pDynamicLink->m_pNextDynamicLink->m_pNextDynamicLink;
        break;
      }
      pDynamicLink = pDynamicLink->m_pNextDynamicLink;
    }
...
void CAI_RadialLinkController::Activate() {
  ...
  if (GetParent() != NULL) {
    ...
    if (flDist > 200.0f) {
      // Warn at the console if a link controller is far away from its parent.
      // This most likely means that a level designer has copied an entity
      // without researching its hierarchy.
  ...
  if (pParent) {
    if (pParent->VPhysicsGetObject()->IsAsleep()) {
      ...
      if (m_bAtRest) {
        ...
        if (flDist < 18.0f) {
          // Ignore movement If moved less than 18 inches from the place where
          // we came to rest.
          // Msg("Reject.\n");
          return;
        }
    ...
    if (m_vecAtRestOrigin.DistToSqr(nodeOrigin) < MinDistCareSq) {
      ...
      for (int j = 0; j < nLinks; j++) {
        ...
        if (iLinkDest > i) {
          ...
          if (((pLink->m_iAcceptedMoveTypes[HULL_HUMAN] ||
                pLink->m_iAcceptedMoveTypes[HULL_WIDE_HUMAN]) &
               bits_CAP_MOVE_GROUND) == 0) {
            // Micro-optimization: Ignore any connection that's not a walking
            // connection for humans.(sjb)
            bQualify = false;
          }

          const Vector &originOther = ppNodes[iLinkDest]->GetOrigin();
          if (bQualify &&
              m_vecAtRestOrigin.DistToSqr(originOther) < MinDistCareSq) {
            if (IsRayIntersectingSphere(nodeOrigin, originOther - nodeOrigin,
                                        m_vecAtRestOrigin, m_flRadius)) {
              if (bMakeStale) {
