...
bool NWCEdit::IsWCVersionValid(void) {
  int status = Editor_CheckVersion(STRING(gpGlobals->mapname),
...
//			 looking.  Keep distance from player constant but adjust
// height 			 based on viewing angle
// Input   :
// Output  :
//------------------------------------------------------------------------------
Vector NWCEdit::AirNodePlacementPosition(void) {
  ...
  float lookDist =
      g_pAINetworkManager->GetEditOps()->m_flAirEditDistance / cosAngle;
...
void NWCEdit::CreateAINode(CBasePlayer *pPlayer) {
  ...
  // -------------------------------------------------------------
  if (!IsWCVersionValid() || !pPlayer) return;
  ...
  if (g_pAINetworkManager->GetEditOps()->m_bAirEditMode) {
    ...
    trace_t tr;
    UTIL_TraceLine(pPlayer->EyePosition(), vNewNodePos, MASK_NPCSOLID_BRUSHONLY,
    ...
    trace_t tr;
    UTIL_TraceLine(startTrace, endTrace, MASK_NPCSOLID, pPlayer,
                   COLLISION_GROUP_NONE, &tr);
    if (tr.fraction != 1.0) {
      // Raise the end position up off the floor, place the node and drop him
      // down
      tr.endpos.z += 48;
  ...
  if (bPositionValid) {
    ...
    UTIL_TraceHull(testHull->GetAbsOrigin(), vUpBit, NAI_Hull::Mins(hullType),
                   NAI_Hull::Maxs(hullType), MASK_NPCSOLID, testHull,
                   COLLISION_GROUP_NONE, &tr);
    ...
    int status;
    if (g_pAINetworkManager->GetEditOps()->m_bAirEditMode) {
      status = Editor_CreateNode(
          "info_node_air", g_pAINetworkManager->GetEditOps()->m_nNextWCIndex,
          testHull->GetLocalOrigin().x, testHull->GetLocalOrigin().y,
          testHull->GetLocalOrigin().z, false);
    } else {
      ...
      status = Editor_CreateNode(
          "info_node", g_pAINetworkManager->GetEditOps()->m_nNextWCIndex,
          testHull->GetLocalOrigin().x, testHull->GetLocalOrigin().y,
          testHull->GetLocalOrigin().z, false);
    }
    if (status == Editor_BadCommand) {
      ...
      pNodeEnt->m_NodeData.nWCNodeID =
          g_pAINetworkManager->GetEditOps()->m_nNextWCIndex;
...
DoneCreate:
  ...
  // ----------------------------------------------------------
  if (!bPositionValid) {
    NDebugOverlay::Box(vNewNodePos, NAI_Hull::Mins(hullType),
                       NAI_Hull::Maxs(hullType), 255, 0, 0, 0, 0.1);
  }
...
void NWCEdit::UndoDestroyAINode(void) {
  ...
  if (g_pAINetworkManager->GetEditOps()->m_pLastDeletedNode) {
    Vector nodePos =
        g_pAINetworkManager->GetEditOps()->m_pLastDeletedNode->GetOrigin();

    int status;
    int nOldWCID = g_pAINetworkManager->GetEditOps()
                       ->m_pNodeIndexTable[g_pAINetworkManager->GetEditOps()
                                               ->m_pLastDeletedNode->GetId()];

    if (g_pAINetworkManager->GetEditOps()->m_bAirEditMode) {
      status = Editor_CreateNode("info_node_air", nOldWCID, nodePos.x,
                                 nodePos.y, nodePos.z, false);
    } else {
      status = Editor_CreateNode("info_node", nOldWCID, nodePos.x, nodePos.y,
                                 nodePos.z, false);
    }
    if (status == Editor_BadCommand) {
      Msg("Worldcraft failed on creation...\n");
    } else if (status == Editor_OK) {
      g_pAINetworkManager->GetEditOps()->m_pLastDeletedNode->SetType(
          NODE_GROUND);
      //@ tofo
      // g_pAINetworkManager->GetEditOps()->m_pLastDeletedNode->m_pNetwork->BuildNetworkGraph();
...
void NWCEdit::DestroyAINode(CBasePlayer *pPlayer) {
  ...
  if (pAINode) {
    int status = Editor_DeleteNode(
        g_pAINetworkManager->GetEditOps()->m_pNodeIndexTable[pAINode->GetId()],
        false);
    ...
    if (status == Editor_BadCommand) {
      ...
      // Now go through at delete any dynamic links that were attached to this
      // node
      for (int link = 0; link < pAINode->NumLinks(); link++) {
        ...
        if (CAI_DynamicLink::GetDynamicLink(nSrcID, nDstID)) {
          int nWCSrcID =
              g_pAINetworkManager->GetEditOps()->m_pNodeIndexTable[nSrcID];
          int nWCDstID =
              g_pAINetworkManager->GetEditOps()->m_pNodeIndexTable[nDstID];
...
void NWCEdit::CreateAILink(CBasePlayer *pPlayer) {
  ...
  if (pAILink && (pAILink->m_LinkInfo & bits_LINK_OFF)) {
    int nWCSrcID =
        g_pAINetworkManager->GetEditOps()->m_pNodeIndexTable[pAILink->m_iSrcID];
    int nWCDstID = g_pAINetworkManager->GetEditOps()
                       ->m_pNodeIndexTable[pAILink->m_iDestID];
    int status = Editor_DeleteNodeLink(nWCSrcID, nWCDstID, false);

    if (status == Editor_BadCommand) {
      Msg("Worldcraft failed on node link creation...\n");
    } else if (status == Editor_OK) {
      // Don't actually destroy the dynamic link while editing.  Just mark the
      // link
      pAILink->m_LinkInfo &= ~bits_LINK_OFF;

      CAI_DynamicLink *pDynamicLink = CAI_DynamicLink::GetDynamicLink(
          pAILink->m_iSrcID, pAILink->m_iDestID);
...
void NWCEdit::DestroyAILink(CBasePlayer *pPlayer) {
  ...
  if (pAILink) {
    ...
    if (status == Editor_BadCommand) {
      ...
      CAI_DynamicLink *pNewLink =
          (CAI_DynamicLink *)CreateEntityByName("info_node_link");
      ;
...
void NWCEdit::RememberEntityPosition(CBaseEntity *pEntity) {
  if (!(pEntity->ObjectCaps() & FCAP_WCEDIT_POSITION)) return;

  if (!g_EntityPositions) {
    g_EntityPositions = new Vector[NUM_ENT_ENTRIES];
    g_EntityOrientations = new QAngle[NUM_ENT_ENTRIES];
    // have to save these too because some entities change the classname on
    // spawn (e.g. prop_physics_override, physics_prop)
...
void NWCEdit::UpdateEntityPosition(CBaseEntity *pEntity) {
  ...
  DevMsg(1, "%s\n   origin %f %f %f\n   angles %f %f %f\n",
         pEntity->GetClassname(), newPos.x, newPos.y, newPos.z, newAng.x,
         newAng.y, newAng.z);
  ...
  if (pEntity->GetModel() &&
      modelinfo->GetModelType(pEntity->GetModel()) == mod_brush) {
    QAngle xformAngles;
    RotationDelta(g_EntityOrientations[entIndex], newAng, &xformAngles);
    if (xformAngles.Length() > 1e-4) {
      result =
          Editor_RotateEntity(pClassname, pos.x, pos.y, pos.z, xformAngles);
    } else {
    ...
    if (Ragdoll_IsPropRagdoll(pEntity)) {
      char tmp[2048];
      Ragdoll_GetAngleOverrideString(tmp, sizeof(tmp), pEntity);
      result = Editor_SetKeyValue(pClassname, pos.x, pos.y, pos.z,
                                  "angleOverride", tmp);
      if (result != Editor_OK) goto error;
    }
    result =
        Editor_SetKeyValue(pClassname, pos.x, pos.y, pos.z, "angles",
                           CFmtStr("%f %f %f", newAng.x, newAng.y, newAng.z));
  }
  if (result != Editor_OK) goto error;

  result =
      Editor_SetKeyValue(pClassname, pos.x, pos.y, pos.z, "origin",
                         CFmtStr("%f %f %f", newPos.x, newPos.y, newPos.z));
  if (result != Editor_OK) goto error;
...
static ConCommand wc_create(
    "wc_create", CC_WC_Create,
    "When in WC edit mode, creates a node where the player is looking if a "
    "node is allowed at that location for the currently selected hull size "
    "(see ai_next_hull)",
    FCVAR_CHEAT);
...
void CC_WC_Destroy(void) {
  ...
  if (engine->IsInEditMode()) {
    ...
    // CBaseEntity* pEntity = FindEntity( pEdict, ""); - use when generalize
    // this to any class int status = Editor_DeleteEntity("info_node",
    // pEdict->origin.x, pEdict->origin.y, pEdict->origin.z, false);
...
static ConCommand wc_destroy(
    "wc_destroy", CC_WC_Destroy,
    "When in WC edit mode, destroys the node that the player is nearest to "
    "looking at.  (The node will be highlighted by a red box).",
    FCVAR_CHEAT);
...
static ConCommand wc_destroy_undo(
    "wc_destroy_undo", CC_WC_DestroyUndo,
    "When in WC edit mode restores the last deleted node", FCVAR_CHEAT);
...
static ConCommand wc_air_node_edit("wc_air_node_edit", CC_WC_AirNodeEdit,
                                   "When in WC edit mode, toggles laying down "
                                   "or air nodes instead of ground nodes",
                                   FCVAR_CHEAT);
...
void CC_WC_AirNodeEditFurther(void) {
  // Only allowed in wc_edit_mode
  if (engine->IsInEditMode() &&
      g_pAINetworkManager->GetEditOps()->m_bAirEditMode) {
    g_pAINetworkManager->GetEditOps()->m_flAirEditDistance += 10.0;
  }
}
static ConCommand wc_air_edit_further(
    "wc_air_edit_further", CC_WC_AirNodeEditFurther,
    "When in WC edit mode and editing air nodes,  moves position of air node "
    "crosshair and placement location further away from player",
    FCVAR_CHEAT);
...
static ConCommand wc_air_edit_nearer(
    "wc_air_edit_nearer", CC_WC_AirNodeEditNearer,
    "When in WC edit mode and editing air nodes,  moves position of air node "
    "crosshair and placement location nearer to from player",
    FCVAR_CHEAT);
...
void CC_WC_LinkEdit(void) {
  ...
  if (engine->IsInEditMode()) {
    ...
    else if (!(g_pAINetworkManager->GetEditOps()->m_debugNetOverlays &
               bits_debugNeedRebuild)) {
...
/// This is an entity used by the hammer_update_safe_entities command. It allows
/// designers to specify objects that should be ignored. It stores an array of
/// sixteen strings which may correspond to names. Designers may ignore more
/// than sixteen objects by placing more than one of these in a level.
...
CON_COMMAND(hammer_update_entity,
...
CON_COMMAND(
    ...
    "Updates entities in the map that can safely be updated (don't have "
    "parents or are affected by constraints). Also excludes entities mentioned "
    "in any hammer_updateignorelist objects in this map.") {
  int iCount = 0;
  CBaseEntity *pEnt = NULL;
  ...
  // first look for any exclusion objects -- these are entities that list
  // specific things to be ignored. All the names that are inside them, we store
  // into a hash table (here implemented through a local CUtlSymbolTable)

  CUtlSymbolTable ignoredNames(
      16, 32, true);  // grow 16 strings at a time. Case insensitive.
  while ((pEnt = gEntList.FindEntityByClassname(
              pEnt, "hammer_updateignorelist")) != NULL) {
  ...
  for (pEnt = gEntList.FirstEnt(); pEnt != NULL;
       pEnt = gEntList.NextEnt(pEnt)) {
    if (!(pEnt->ObjectCaps() & FCAP_WCEDIT_POSITION)) continue;

    // If we have a parent, or any children, we're not safe to update
    if (pEnt->GetMoveParent() || pEnt->FirstMoveChild()) continue;

    IPhysicsObject *pPhysics = pEnt->VPhysicsGetObject();
    if (!pPhysics) continue;
    // If we are affected by any constraints, we're not safe to update
    if (pPhysics->IsAttachedToConstraint(Ragdoll_IsPropRagdoll(pEnt))) continue;
    // Motion disabled?
    if (!pPhysics->IsMoveable()) continue;

    // ignore brush models (per bug 61318)
    if (dynamic_cast<CPhysBox *>(pEnt)) continue;

    // explicitly excluded by designer?
    if (ignoredNames.Find(pEnt->GetEntityName().ToCStr()).IsValid()) continue;
