...
SendPropInt(SENDINFO(m_nSequence), ANIMATION_SEQUENCE_BITS, SPROP_UNSIGNED),
    SendPropFloat(SENDINFO(m_flCycle), ANIMATION_CYCLE_BITS, SPROP_ROUNDDOWN,
                  0.0f, 1.0f),
    SendPropFloat(SENDINFO(m_flPrevCycle), ANIMATION_CYCLE_BITS,
    ...
    END_SEND_TABLE()
        ...
        BEGIN_SEND_TABLE_NOBASE(CBaseAnimatingOverlay, DT_OverlayVars)
            SendPropUtlVector(
                ...
                SendPropDataTable(NULL, 0,
                                  &REFERENCE_SEND_TABLE(DT_Animationlayer)))
                END_SEND_TABLE()

                    IMPLEMENT_SERVERCLASS_ST(CBaseAnimatingOverlay,
...
void CAnimationLayer::StudioFrameAdvance(float flInterval,
...
bool CAnimationLayer::IsAbandoned(void) {
  if (IsActive() && !IsAutokill() && !IsKillMe() && m_flLastAccess > 0.0 &&
      (gpGlobals->curtime - m_flLastAccess > 0.2))
    return true;
  else
    return false;
}
...
void CBaseAnimatingOverlay::VerifyOrder(void) {
  ...
  for (i = 0; i < m_AnimOverlay.Count(); i++) {
    if (m_AnimOverlay[i].m_nOrder < MAX_OVERLAYS) {
      ...
      layer[j] = i;
      if (j > maxOrder) maxOrder = j;
    }
  }

  // make sure they're sequential
  // Aim layers are allowed to have gaps, and we are moving aim blending to
  // server
  ...
  for ( i = 0; i < MAX_OVERLAYS; i++ )
  {
          ...
          {
                  char tempstr[512];
                  Q_snprintf( tempstr, sizeof( tempstr ),"%d : %d :%.2f
  :%d:%d:%.1f", j, m_AnimOverlay[ j ].m_nSequence, m_AnimOverlay[ j
  ].m_flWeight, m_AnimOverlay[ j ].IsActive(), m_AnimOverlay[ j ].IsKillMe(),
...
//			 if an flInterval is passed in, only advance animation that
//number of seconds
// Input   :
// Output  :
//------------------------------------------------------------------------------
    ...
    if (pLayer->IsActive()) {
      ...
      if (pLayer->IsKillMe()) {
        if (pLayer->m_flKillDelay > 0) {
          ...
          pLayer->m_flKillDelay = std::clamp(pLayer->m_flKillDelay, 0.0f, 1.0f);
          ...
          pLayer->m_flWeight =
              std::clamp(static_cast<float>(pLayer->m_flWeight), 0.0f, 1.0f);
        } else {
          ...
          if (ai_sequence_debug.GetBool() == true &&
            ...
            Msg("removing %d (%d): %s : %5.3f (%.3f)\n", i,
                pLayer->m_nOrder.Get(), GetSequenceName(pLayer->m_nSequence),
                pLayer->m_flCycle.Get(), pLayer->m_flWeight.Get());
          }
          FastRemoveLayer(i);
      ...
      // Having a weight left over when you're no longer marked as active is now
      // harmless and commonplace.  Just clean up.
      pLayer->Init(this);
      pLayer->Dying();
    }
  }
  ...
  if (ai_sequence_debug.GetBool() == true &&
    ...
    for (int i = 0; i < m_AnimOverlay.Count(); i++) {
      if (m_AnimOverlay[i].IsActive()) {
        ...
        m_AnimOverlay[ i ].m_flLastAccess );
        }
        */
        Msg(" %d (%d): %s : %5.3f (%.3f)\n", i, m_AnimOverlay[i].m_nOrder.Get(),
            GetSequenceName(m_AnimOverlay[i].m_nSequence),
            m_AnimOverlay[i].m_flCycle.Get(),
            m_AnimOverlay[i].m_flWeight.Get());
      }
    }
  }
...
void CAnimationLayer::DispatchAnimEvents(CBaseAnimating *eventHandler,
                                         CBaseAnimating *pOwner) {
  animevent_t event;
  ...
  if (m_nSequence >= pstudiohdr->GetNumSeq()) return;
  ...
  float flCycleRate =
      pOwner->GetSequenceCycleRate(m_nSequence) * m_flPlaybackRate;
  float flStart = m_flLastEventCheck;
  float flEnd = m_flCycle;

  if (!m_bLooping) {
    // fire off events early
    float flLastVisibleCycle =
        1.0f - (pstudiohdr->pSeqdesc(m_nSequence).fadeouttime) * flCycleRate;
  ...
  STRING(pOwner->GetModelName()), pstudiohdr->pSeqdesc( m_nSequence
  ).pszLabel(), flStart, flEnd, m_bSequenceFinished );
  }
  */
  ...
  while ((index = GetAnimationEvent(pstudiohdr, m_nSequence, &event, flStart,
    ...
    if (flCycleRate > 0.0) {
      ...
      event.eventtime = pOwner->m_flAnimTime +
                        (flCycle - m_flCycle) / flCycleRate +
                        pOwner->GetAnimTimeInterval();
    }
...
void CBaseAnimatingOverlay::GetSkeleton(CStudioHdr *pStudioHdr, Vector pos[],
  ...
  for (i = 0; i < m_AnimOverlay.Count(); i++) {
    CAnimationLayer &pLayer = m_AnimOverlay[i];
    if ((pLayer.m_flWeight > 0) && pLayer.IsActive() && pLayer.m_nOrder >= 0 &&
        pLayer.m_nOrder < m_AnimOverlay.Count()) {
  ...
  for (i = 0; i < m_AnimOverlay.Count(); i++) {
    if (layer[i] >= 0 && layer[i] < m_AnimOverlay.Count()) {
      ...
      AccumulatePose(pStudioHdr, m_pIk, pos, q, pLayer.m_nSequence,
                     pLayer.m_flCycle, GetPoseParameterArray(), boneMask,
                     pLayer.m_flWeight, gpGlobals->curtime);
    }
  }
...
void CBaseAnimatingOverlay::OnRestore() {
  ...
  // and Tracker 48843 (Alyx skating after restore) restore the owner entity ptr
  // (otherwise the network layer won't get NetworkStateChanged signals until
  // the layer is re-Init()'ed
  for (i = 0; i < m_AnimOverlay.Count(); i++) {
  ...
  for (i = 0; i < m_AnimOverlay.Count(); i++) {
    if ((m_AnimOverlay[i].IsActive() &&
         (m_AnimOverlay[i].m_fFlags & ANIM_LAYER_DONTRESTORE)) ||
...
// Output : int
//-----------------------------------------------------------------------------
int CBaseAnimatingOverlay::AddGestureSequence(int sequence,
                                              bool autokill /*= true*/) {
...
// Output : int
//-----------------------------------------------------------------------------
int CBaseAnimatingOverlay::AddGestureSequence(int nSequence, float flDuration,
                                              bool autokill /*= true*/) {
  int iLayer = AddGestureSequence(nSequence, autokill);
  Assert(iLayer != -1);

  if (iLayer >= 0 && flDuration > 0) {
    m_AnimOverlay[iLayer].m_flPlaybackRate =
        SequenceDuration(nSequence) / flDuration;
  }
  return iLayer;
}
...
// Output  :
//------------------------------------------------------------------------------
int CBaseAnimatingOverlay::AddGesture(Activity activity,
                                      bool autokill /*= true*/) {
...
int CBaseAnimatingOverlay::AddGesture(Activity activity, float flDuration,
                                      bool autokill /*= true*/) {
...
void CBaseAnimatingOverlay::SetLayerDuration(int iLayer, float flDuration) {
  if (IsValidLayer(iLayer) && flDuration > 0) {
    m_AnimOverlay[iLayer].m_flPlaybackRate =
        SequenceDuration(m_AnimOverlay[iLayer].m_nSequence) / flDuration;
  }
}
...
float CBaseAnimatingOverlay::GetLayerDuration(int iLayer) {
  if (IsValidLayer(iLayer)) {
    if (m_AnimOverlay[iLayer].m_flPlaybackRate != 0.0f) {
      return (1.0 - m_AnimOverlay[iLayer].m_flCycle) *
             SequenceDuration(m_AnimOverlay[iLayer].m_nSequence) /
             m_AnimOverlay[iLayer].m_flPlaybackRate;
    }
...
int CBaseAnimatingOverlay::AddLayeredSequence(int sequence, int iPriority) {
  ...
  if (IsValidLayer(i)) {
    ...
    m_AnimOverlay[i].m_bLooping =
        ((GetSequenceFlags(GetModelPtr(), sequence) & STUDIO_LOOPING) != 0);
    if (ai_sequence_debug.GetBool() == true &&
        m_debugOverlays & OVERLAY_NPC_SELECTED_BIT) {
      Msg("%5.3f : adding %d (%d): %s : %5.3f (%.3f)\n", gpGlobals->curtime, i,
          m_AnimOverlay[i].m_nOrder.Get(),
          GetSequenceName(m_AnimOverlay[i].m_nSequence),
          m_AnimOverlay[i].m_flCycle.Get(), m_AnimOverlay[i].m_flWeight.Get());
    }
  }
...
bool CBaseAnimatingOverlay::IsValidLayer(int iLayer) {
  return (iLayer >= 0 && iLayer < m_AnimOverlay.Count() &&
          m_AnimOverlay[iLayer].IsActive());
}
...
int CBaseAnimatingOverlay::AllocateLayer(int iPriority) {
  ...
  for (i = 0; i < m_AnimOverlay.Count(); i++) {
    if (m_AnimOverlay[i].IsActive()) {
      if (m_AnimOverlay[i].m_nPriority <= iPriority) {
        iNewOrder = std::max(iNewOrder, m_AnimOverlay[i].m_nOrder + 1);
      }
  ...
  // make sure there's always an empty unused layer so that history slots will
  // be available on the client when it is used
  if (iNumOpen == 0) {
    if (m_AnimOverlay.Count() < MAX_OVERLAYS) {
...
int CBaseAnimatingOverlay::FindGestureLayer(Activity activity) {
  for (int i = 0; i < m_AnimOverlay.Count(); i++) {
    if (!(m_AnimOverlay[i].IsActive())) continue;
...
// Input  : activity -
//-----------------------------------------------------------------------------
void CBaseAnimatingOverlay::RestartGesture(Activity activity,
...
void CBaseAnimatingOverlay::SetLayerCycle(int iLayer, float flCycle,
  ...
  if (!m_AnimOverlay[iLayer].m_bLooping) {
    flCycle = std::clamp(flCycle, 0.0f, 1.0f);
    flPrevCycle = std::clamp(flPrevCycle, 0.0f, 1.0f);
  }
  m_AnimOverlay[iLayer].m_flCycle = flCycle;
...
float CBaseAnimatingOverlay::GetLayerCycle(int iLayer) {
  if (!IsValidLayer(iLayer)) return 0.0;
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseAnimatingOverlay::SetLayerPlaybackRate(int iLayer,
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseAnimatingOverlay::RemoveLayer(int iLayer, float flKillRate,
                                        float flKillDelay) {
  if (!IsValidLayer(iLayer)) return;

  if (flKillRate > 0) {
    m_AnimOverlay[iLayer].m_flKillRate =
        m_AnimOverlay[iLayer].m_flWeight / flKillRate;
  } else {
...
void CBaseAnimatingOverlay::FastRemoveLayer(int iLayer) {
  ...
  for (int j = 0; j < m_AnimOverlay.Count(); j++) {
    if ((m_AnimOverlay[j].IsActive()) &&
        m_AnimOverlay[j].m_nOrder > m_AnimOverlay[iLayer].m_nOrder) {
...
CAnimationLayer *CBaseAnimatingOverlay::GetAnimOverlay(int iIndex) {
  iIndex = std::clamp(iIndex, 0, m_AnimOverlay.Count() - 1);
...
bool CBaseAnimatingOverlay::HasActiveLayer(void) {
  for (int j = 0; j < m_AnimOverlay.Count(); j++) {
    if (m_AnimOverlay[j].IsActive()) return true;
  }
