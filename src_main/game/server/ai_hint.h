...
SOURCE_DECLARE_POINTER_HANDLE(AIHintIter_t);
...
class CAI_HintManager {
 ...
 public:
  ...
  static CAI_Hint *FindHint(CAI_BaseNPC *pNPC, Hint_e nHintType, int nFlags,
                            float flMaxDist, const Vector *pMaxDistFrom = NULL);
  ...
  static int FindAllHints(CAI_BaseNPC *pNPC, const CHintCriteria &hintCriteria,
                          CUtlVector<CAI_Hint *> *pResult) {
...
class CAI_Hint : public CServerOnlyEntity {
 ...
 public:
  ...
  bool HintMatchesCriteria(CAI_BaseNPC *pNPC, const CHintCriteria &hintCriteria,
 ...
 private:
  ...
  int m_nTargetNodeID;
  EHANDLE m_hHintOwner;   // Is hint locked (being used by NPC / NPC en-route to
                          // use it)
