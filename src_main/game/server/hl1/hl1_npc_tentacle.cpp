...
BEGIN_DATADESC(CNPC_Tentacle)
  DEFINE_FIELD(m_flInitialYaw, FIELD_FLOAT),
      ...
      DEFINE_THINKFUNC(Start), DEFINE_THINKFUNC(Cycle),
...
bool CNPC_Tentacle::HeardAnything(void) {
  if (HasCondition(COND_HEAR_DANGER) ||  // I remove a bunch of sounds from here
                                         // on purpose. Talk to me if you're
                                         // changing this!(sjb)
      HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_WORLD) ||
...
void CNPC_Tentacle::Cycle(void) {
  ...
  // pev->origin.z, m_MonsterState, m_IdealMonsterState );
  SetNextThink(gpGlobals->curtime + 0.1);

  // ALERT( at_console, "%s %d %d %d %f %f\n", STRING( pev->targetname ),
  // pev->sequence, m_iGoalAnim, m_iDir, pev->framerate, pev->health );
  ...
  if (HeardAnything())
    pSound = GetSenses()->GetClosestSound(
        false, (SOUND_DANGER | SOUND_COMBAT | SOUND_WORLD | SOUND_PLAYER));
  else
    pSound = NULL;

  if (pSound) {
    // NDebugOverlay::Line( EarPosition(), pSound->GetSoundOrigin(), 0, 255, 0,
    // false, 0.2 );

    Vector vecDir;
    if (gpGlobals->curtime - m_flPrevSoundTime < 0.5) {
      float dt = gpGlobals->curtime - m_flPrevSoundTime;
      vecDir = pSound->GetSoundOrigin() +
               (pSound->GetSoundOrigin() - m_vecPrevSound) / dt -
               GetAbsOrigin();
    } else {
    ...
    if (m_flSoundYaw < -180) m_flSoundYaw += 360;
    if (m_flSoundYaw > 180) m_flSoundYaw -= 360;
    ...
    if (m_flSoundTime < gpGlobals->curtime) {
      ...
      UTIL_EmitAmbientSound(GetSoundSourceIndex(),
                            GetAbsOrigin() + Vector(0, 0, MyHeight()),
                            "Tentacle.Alert", 1.0, SNDLVL_GUNFIRE, 0, 100);
    }
  ...
  switch (GetSequence()) {
    ...
    case TENTACLE_ANIM_Lev3_Rear:
    case TENTACLE_ANIM_Lev3_Rear_Idle:
      if (dy < 0 && dy > -m_flMaxYaw) dy = -m_flMaxYaw;
      if (dy > 0 && dy < m_flMaxYaw) dy = m_flMaxYaw;
      break;
    default:
      if (dy < -m_flMaxYaw) dy = -m_flMaxYaw;
      if (dy > m_flMaxYaw) dy = m_flMaxYaw;
  }
  ...
  if (IsSequenceFinished()) {
    ...
    if (m_iHealth <= 1) {
      ...
      if (m_flSoundYaw >= -(m_flMaxYaw + 30) &&
          m_flSoundYaw <= (m_flMaxYaw + 30)) {
        // strike
      ...
      } else if (m_flSoundYaw >= -m_flMaxYaw * 2 &&
                 m_flSoundYaw <= m_flMaxYaw * 2) {
        // tap
        switch (m_iSoundLevel) {
          case 0:
    ...
    switch (GetSequence()) {
      ...
      case TENTACLE_ANIM_Lev3_Tap: {
        ...
        UTIL_TraceLine(vecSrc, vecSrc + v_forward * 512,
        ...
        // ALERT( at_console, "%f %f\n", tr1.flFraction * 512, tr2.flFraction *
        // 512 );

        m_flTapRadius = SetPoseParameter(
            0, random->RandomFloat(tr1.fraction * 512, tr2.fraction * 512));
      } break;
      default:
  ...
  if (m_flPrevSoundTime + 2.0 > gpGlobals->curtime) {
    ...
    // slowdown to normal
    m_flPlaybackRate =
        m_iDir + m_iDir * (5 - (gpGlobals->curtime - m_flPrevSoundTime)) / 2 +
        m_flFramerateAdj;
  }
}
...
void CNPC_Tentacle::HandleAnimEvent(animevent_t *pEvent) {
  switch (pEvent->event) {
    case 1:  // bang
    {
      ...
      // Vector vecSrc = GetAbsOrigin() + m_flTapRadius * Vector( cos(
      // GetAbsAngles().y * (3.14192653 / 180.0) ), sin( GetAbsAngles().y *
      // (M_PI / 180.0) ), 0.0 );
      ...
      switch (m_iTapSound) {
        case TE_SILO:
          UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc,
                                "Tentacle.HitSilo", 1.0, SNDLVL_GUNFIRE, 0,
                                100);
          break;
        case TE_NONE:
          break;
        case TE_DIRT:
          UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc,
                                "Tentacle.HitDirt", 1.0, SNDLVL_GUNFIRE, 0,
                                100);
          break;
        case TE_WATER:
          UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc,
                                "Tentacle.HitWater", 1.0, SNDLVL_GUNFIRE, 0,
                                100);
          break;
      }
    } break;
    ...
    case 6:  // light tap
    {
      Vector vecSrc =
          GetAbsOrigin() +
          m_flTapRadius * Vector(cos(GetAbsAngles().y * (M_PI / 180.0)),
                                 sin(GetAbsAngles().y * (M_PI / 180.0)), 0.0);
      ...
      switch (m_iTapSound) {
        case TE_SILO:
          UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc,
                                "Tentacle.HitSilo", flVol, SNDLVL_GUNFIRE, 0,
                                100);
          break;
        case TE_NONE:
          break;
        case TE_DIRT:
          UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc,
                                "Tentacle.HitDirt", flVol, SNDLVL_GUNFIRE, 0,
                                100);
          break;
        case TE_WATER:
          UTIL_EmitAmbientSound(GetSoundSourceIndex(), vecSrc,
                                "Tentacle.HitWater", flVol, SNDLVL_GUNFIRE, 0,
                                100);
          break;
      }
    } break;
    ...
    case 7:  // roar
      UTIL_EmitAmbientSound(GetSoundSourceIndex(),
                            GetAbsOrigin() + Vector(0, 0, MyHeight()),
                            "Tentacle.Roar", 1.0, SNDLVL_GUNFIRE, 0, 100);
      break;

    case 8:  // search
      UTIL_EmitAmbientSound(GetSoundSourceIndex(),
                            GetAbsOrigin() + Vector(0, 0, MyHeight()),
                            "Tentacle.Search", 1.0, SNDLVL_GUNFIRE, 0, 100);
      break;

    case 9:  // swing
      UTIL_EmitAmbientSound(GetSoundSourceIndex(),
                            GetAbsOrigin() + Vector(0, 0, MyHeight()),
                            "Tentacle.Swing", 1.0, SNDLVL_GUNFIRE, 0, 100);
      break;
    default:
...
void CNPC_Tentacle::HitTouch(CBaseEntity *pOther) {
  if (m_flHitTime > gpGlobals->curtime) return;

  // only look at the ones where the player hit me
  if (pOther == NULL || pOther->GetModelIndex() == GetModelIndex() ||
...
int CNPC_Tentacle::OnTakeDamage(const CTakeDamageInfo &info) {
  ...
  // Don't allow the tentacle to die. Instead set health to 1, so we can do our
...
bool CNPC_Tentacle::CreateVPhysics(void) {
  ...
  m_BoneFollowerManager.InitBoneFollowers(
      this, std::size(pTentacleFollowerBoneNames), pTentacleFollowerBoneNames);
