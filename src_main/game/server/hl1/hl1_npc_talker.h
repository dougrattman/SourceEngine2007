...
#define SF_NPC_PREDISASTER \
  (1 << 16)  // This is a predisaster scientist or barney. Influences how they
             // speak.
...
class CHL1NPCTalker : public CNPCSimpleTalker {
 ...
 public:
  ...
  void IdleHeadTurn(CBaseEntity *pTarget, float flDuration = 0.0,
                    float flImportance = 1.0f);
  ...
  float PickLookTarget(bool bExcludePlayers = false, float minTime = 1.5,
                       float maxTime = 2.5);

  bool OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor,
