...
class CNPC_BaseTurret : public CAI_BaseNPC {
 ...
 public:
  ...
  void EXPORT TurretUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  float m_flSpinUpTime;  // Amount of time until the barrel should spin down
                         // when searching
...
BEGIN_DATADESC(CNPC_BaseTurret)
  ...
  DEFINE_FIELD(m_flMaxSpin, FIELD_FLOAT), DEFINE_FIELD(m_iSpin, FIELD_INTEGER),
      ...
      // Thinks
      DEFINE_THINKFUNC(ActiveThink), DEFINE_THINKFUNC(SearchThink),
      DEFINE_THINKFUNC(AutoSearchThink), DEFINE_THINKFUNC(TurretDeath),
      DEFINE_THINKFUNC(SpinDownCall), DEFINE_THINKFUNC(SpinUpCall),
      DEFINE_THINKFUNC(Deploy), DEFINE_THINKFUNC(Retire),
...
Class_T CNPC_BaseTurret::Classify(void) {
  if (m_iOn || m_iAutoStart) return CLASS_MACHINE;
  return CLASS_NONE;
}
...
//=========================================================
void CNPC_BaseTurret::TraceAttack(const CTakeDamageInfo &info,
  ...
  if (ptr->hitgroup == 10) {
    ...
    if (m_flDamageTime != gpGlobals->curtime ||
        (random->RandomInt(0, 10) < 1)) {
      g_pEffects->Ricochet(ptr->endpos, ptr->plane.normal);
      m_flDamageTime = gpGlobals->curtime;
    }

    ainfo.SetDamage(0.1);  // don't hurt the monster much, but allow
                           // bits_COND_LIGHT_DAMAGE to be generated
  }
...
int CNPC_BaseTurret::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  ...
  if (m_iHealth <= 0) {
    ...
    //		ClearBits (pev->flags, FL_MONSTER); // why are they set in the
    //first  place???
...
void CNPC_BaseTurret::SetTurretAnim(TURRET_ANIM anim) {
  ...
  if (GetSequence() != anim)
  {
          ...
          case TURRET_ANIM_SPIN:
                  if (GetSequence() != TURRET_ANIM_FIRE && GetSequence() !=
  TURRET_ANIM_SPIN)
                  {
                          m_flCycle = 0;
                  }
          ...
          case TURRET_ANIM_RETIRE:
                  m_flCycle			= 255;
                  m_flPlaybackRate		= -1.0;	//play the animation
  backwards break; case TURRET_ANIM_DIE: m_flPlaybackRate		= 1.0;
                  break;
          }
  ...
  */
  ...
  if (GetSequence() != anim) {
    ...
    switch (anim) {
      case TURRET_ANIM_FIRE:
      case TURRET_ANIM_SPIN:
        if (GetSequence() != TURRET_ANIM_FIRE &&
            GetSequence() != TURRET_ANIM_SPIN) {
          SetCycle(0);
        }
        break;
      case TURRET_ANIM_RETIRE:
...
void CNPC_BaseTurret::ActiveThink(void) {
  ...
  // forward
  //	NDebugOverlay::Line(vecMuzzle, vecMid + ( vecMuzzleDir * 200 ), 255,0,0,
  // false, 0.1);  LOS
  //	NDebugOverlay::Line(vecMuzzle, vecMid + ( vecLOS * 200 ), 0,0,255,
  // false, 0.1);
  ...
  if (m_fBeserk) {
    ...
    if (vecAnglesToEnemy.y > 360) vecAnglesToEnemy.y -= 360;
...
//=========================================================
// SearchThink
// This search function will sit with the turret deployed and look for a new
// target. After a set amount of time, the barrel will spin down. After
// m_flMaxWait, the turret will retact.
//=========================================================
void CNPC_BaseTurret::SearchThink(void) {
  ...
  if (pEnemy != NULL) {
    ...
    if (m_vecGoalAngles.y >= 360) m_vecGoalAngles.y -= 360;
    MoveTurret();
  }
...
void CNPC_BaseTurret::TurretDeath(void) {
  ...
  if (IsSequenceFinished() && !MoveTurret() &&
      m_flDamageTime + 5 < gpGlobals->curtime) {
    m_flPlaybackRate = 0;
    SetThink(NULL);
  }
}
...
void CNPC_BaseTurret::TurretUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                                USE_TYPE useType, float value) {
  if (!ShouldToggle(useType, m_iOn)) return;
  ...
  if (m_iOn) {
    ...
    // if the turret is flagged as an autoactivate turret, re-enable it's
    // ability open self.
    if (m_spawnflags & SF_MONSTER_TURRET_AUTOACTIVATE) {
      m_iAutoStart = TRUE;
    }
...
void CNPC_Turret::Spawn() {
  ...
  m_iDeployHeight = 32;
  m_iMinPitch = -90;
  UTIL_SetSize(this, Vector(-32, -32, -m_iRetractHeight),
               Vector(32, 32, m_iRetractHeight));

  SetThink(&CNPC_BaseTurret::Initialize);

  m_pEyeGlow = CSprite::SpriteCreate(TURRET_GLOW_SPRITE, GetAbsOrigin(), FALSE);
  m_pEyeGlow->SetTransparency(kRenderGlow, 255, 0, 0, 0,
...
void CNPC_Turret::Shoot(Vector &vecSrc, Vector &vecDirToEnemy) {
  ...
  FireBullets(1, vecSrc, vecDirToEnemy, TURRET_SPREAD, TURRET_RANGE,
              m_iAmmoType, 1);
...
void CNPC_MiniTurret::Spawn() {
  ...
  m_iDeployHeight = 32;
  m_iMinPitch = -90;
  UTIL_SetSize(this, Vector(-16, -16, -m_iRetractHeight),
               Vector(16, 16, m_iRetractHeight));
...
BEGIN_DATADESC(CNPC_Sentry)
  DEFINE_ENTITYFUNC(SentryTouch), DEFINE_FIELD(m_bStartedDeploy, FIELD_BOOLEAN),
END_DATADESC()
...
void CNPC_Sentry::Event_Killed(const CTakeDamageInfo &info) {
  ...
  te->Smoke(filter, 0.0, &vecSrc, g_sModelIndexSmoke, 2.5, 10);
