...
#define SQUID_SPRINT_DIST \
  256  // how close the squid has to get before starting to sprint and refusing
       // to swerve
...
void CSquidSpit::Shoot(CBaseEntity *pOwner, Vector vecStart,
                       Vector vecVelocity) {
  ...
  if (pSprite) {
    ...
    pSprite->SetTransparency(pSpit->m_nRenderMode, pSpit->m_clrRender->r,
                             pSpit->m_clrRender->g, pSpit->m_clrRender->b,
                             pSpit->m_clrRender->a, pSpit->m_nRenderFX);
  }

  CPVSFilter filter(vecStart);

  VectorNormalize(vecVelocity);
  te->SpriteSpray(filter, 0.0, &vecStart, &vecVelocity,
                  pSpit->m_nSquidSpitSprite, 210, 25, 15);
}
...
void CSquidSpit::Touch(CBaseEntity *pOther) {
  ...
  if (!pOther->m_takedamage) {
    ...
    UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + GetAbsVelocity() * 10,
    ...
    te->SpriteSpray(filter, 0.0, &tr.endpos, &tr.plane.normal,
                    m_nSquidSpitSprite, 30, 8, 5);

  } else {
    CTakeDamageInfo info(this, this, sk_bullsquid_dmg_spit.GetFloat(),
                         DMG_BULLET);
    CalculateBulletDamageForce(&info, GetAmmoDef()->Index("9mmRound"),
...
void CNPC_Bullsquid::Spawn() {
  ...
  m_flFieldOfView = 0.2;  // indicates the width of this monster's forward view
  ...
  switch (pEvent->event) {
    case BSQUID_AE_SPIT: {
      if (GetEnemy()) {
        ...
        // !!!HACKHACK - the spot at which the spit originates (in front of the
        // mouth) was measured in 3ds and hardcoded here. we should be able to
        // read the position of bones at runtime for this info.
    ...
    case BSQUID_AE_BITE: {
      // SOUND HERE!
      CBaseEntity *pHurt =
          CheckTraceHullAttack(70, Vector(-16, -16, -16), Vector(16, 16, 16),
                               sk_bullsquid_dmg_bite.GetFloat(), DMG_SLASH);
      if (pHurt) {
        Vector forward, up;
    ...
    case BSQUID_AE_TAILWHIP: {
      CBaseEntity *pHurt = CheckTraceHullAttack(
          70, Vector(-16, -16, -16), Vector(16, 16, 16),
          sk_bullsquid_dmg_whip.GetFloat(), DMG_SLASH | DMG_ALWAYSGIB);
      if (pHurt) {
        Vector right, up;
    ...
    case BSQUID_AE_THROW: {
      ...
      CBaseEntity *pHurt = CheckTraceHullAttack(70, Vector(-16, -16, -16),
                                                Vector(16, 16, 16), 0, 0);
      ...
      if (pHurt) {
        ...
        // screeshake transforms the viewmodel as well as the viewangle. No
        // problems with seeing the ends of the viewmodels.
        ...
        if (pHurt->IsPlayer()) {
          ...
          pHurt->SetAbsVelocity(pHurt->GetAbsVelocity() + forward * 300 +
                                up * 300);
        }
      }
    } break;
...
int CNPC_Bullsquid::RangeAttack1Conditions(float flDot, float flDist) {
  if (IsMoving() && flDist >= 512) {
    // squid will far too far behind if he stops running to spit at this
    // distance from the enemy.
    return (COND_NONE);
  }

  if (flDist > 85 && flDist <= 784 && flDot >= 0.5 &&
      gpGlobals->curtime >= m_flNextSpitTime) {
    if (GetEnemy() != NULL) {
...
int CNPC_Bullsquid::MeleeAttack1Conditions(float flDot, float flDist) {
  if (GetEnemy()->m_iHealth <= sk_bullsquid_dmg_whip.GetFloat() &&
      flDist <= 85 && flDot >= 0.7) {
...
int CNPC_Bullsquid::MeleeAttack2Conditions(float flDot, float flDist) {
  if (flDist <= 85 && flDot >= 0.7 &&
      !HasCondition(COND_CAN_MELEE_ATTACK1))  // The player & bullsquid can be
                                              // as much as their bboxes
...
bool CNPC_Bullsquid::FValidateHintType(CAI_Hint *pHint) {
  if (pHint->HintType() == HINT_HL1_WORLD_HUMAN_BLOOD) return true;
...
Disposition_t CNPC_Bullsquid::IRelationType(CBaseEntity *pTarget) {
  if (gpGlobals->curtime - m_flLastHurtTime < 5 &&
      FClassnameIs(pTarget, "monster_headcrab")) {
    // if squid has been hurt in the last 5 seconds, and is getting relationship
    // for a headcrab, tell squid to disregard crab.
    return D_NU;
  }
...
int CNPC_Bullsquid::GetSoundInterests(void) {
  return SOUND_WORLD | SOUND_COMBAT | SOUND_CARCASS | SOUND_MEAT |
         SOUND_GARBAGE | SOUND_PLAYER;
}
...
void CNPC_Bullsquid::OnListened(void) {
  ...
  ClearConditions(conditionsToClear, std::size(conditionsToClear));
...
void CNPC_Bullsquid::RunAI(void) {
  ...
  if (GetEnemy() != NULL && GetActivity() == ACT_RUN) {
    ...
    if ((GetAbsOrigin() - GetEnemy()->GetAbsOrigin()).Length2D() <
        SQUID_SPRINT_DIST) {
  ...
  switch (m_NPCState) {
    case NPC_STATE_ALERT: {
      ...
      if (HasCondition(COND_SQUID_SMELL_FOOD)) {
        ...
        if (pSound && (!FInViewCone(pSound->GetSoundOrigin()) ||
                       !FVisible(pSound->GetSoundOrigin()))) {
      ...
      if (HasCondition(COND_SMELL)) {
        ...
        pSound = GetBestScent();
        if (pSound) return SCHED_SQUID_WALLOW;
      }
    ...
    case NPC_STATE_COMBAT: {
      ...
      if (HasCondition(COND_ENEMY_DEAD)) {
        // call base class, all code to handle dead enemies is centralized
        // there.
      ...
      if (HasCondition(COND_NEW_ENEMY)) {
        if (m_fCanThreatDisplay && IRelationType(GetEnemy()) == D_HT &&
          ...
          m_fCanThreatDisplay =
              FALSE;  // only do the headcrab dance once per lifetime.
...
bool CNPC_Bullsquid::FVisible(Vector vecOrigin) {
  ...
  UTIL_TraceLine(vecLookerOrigin, vecOrigin, MASK_BLOCKLOS, this /*pentIgnore*/,
...
NPC_STATE CNPC_Bullsquid::SelectIdealState(void) {
  // If no schedule conditions, the new ideal state is probably the reason we're
  // in here.
  switch (m_NPCState) {
    case NPC_STATE_COMBAT:
      /*
      ...
      */
      {
        if (GetEnemy() != NULL &&
            ...
            FClassnameIs(GetEnemy(), "monster_headcrab")) {
          // if the squid has a headcrab enemy and something hurts it, it's
          // going to forget about the crab for a while.
          SetEnemy(NULL);
...
// > SCHED_SQUID_HURTHOP
//=========================================================
DEFINE_SCHEDULE(SCHED_SQUID_HURTHOP,
...
// > SCHED_SQUID_SEECRAB
//=========================================================
DEFINE_SCHEDULE(SCHED_SQUID_SEECRAB,

                "	Tasks"
                "		TASK_STOP_MOVING			0"
                "		TASK_SOUND_WAKE				0"
                "		TASK_PLAY_SEQUENCE			"
                "ACTIVITY:ACT_SQUID_EXCITED"
                "		TASK_FACE_ENEMY				0"
                "	"
                "	Interrupts"
                "		COND_LIGHT_DAMAGE"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SQUID_EAT						10"
    "		TASK_PLAY_SEQUENCE					"
    "ACTIVITY:ACT_SQUID_DETECT_SCENT"
    "		TASK_STORE_LASTPOSITION				0"
    "		TASK_GET_PATH_TO_BESTSCENT			0"
    "		TASK_WALK_PATH						0"
    "		TASK_WAIT_FOR_MOVEMENT				0"
...
DEFINE_SCHEDULE(
    ...
    "		TASK_WAIT_FOR_MOVEMENT			0"
    "		TASK_PLAY_SEQUENCE				"
    "ACTIVITY:ACT_SQUID_INSPECT_FLOOR"
    "		TASK_SQUID_EAT					50"
    "		TASK_GET_PATH_TO_LASTPOSITION	0"
    "		TASK_WALK_PATH					0"
    "		TASK_WAIT_FOR_MOVEMENT			0"
