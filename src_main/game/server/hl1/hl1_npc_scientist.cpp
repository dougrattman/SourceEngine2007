...
void CNPC_Scientist::ModifyOrAppendCriteria(AI_CriteriaSet &criteriaSet) {
  ...
  criteriaSet.AppendCriteria(
      "disaster", predisaster ? "[disaster::pre]" : "[disaster::post]");
}
...
void CNPC_Scientist::TalkInit() {
  ...
  switch (m_nBody % 3) {
    default:
    case HEAD_GLASSES:
      GetExpresser()->SetVoicePitch(105);
      break;  // glasses
    case HEAD_EINSTEIN:
      GetExpresser()->SetVoicePitch(100);
      break;  // einstein
    case HEAD_LUTHER:
      GetExpresser()->SetVoicePitch(95);
      break;  // luther
    case HEAD_SLICK:
      GetExpresser()->SetVoicePitch(100);
      break;  // slick
...
//
//-----------------------------------------------------------------------------
void CNPC_Scientist::Spawn() {
  // Select the body first if it's going to be random cause we set his voice
  // pitch in Precache.
  if (m_nBody == -1)
    m_nBody =
        random->RandomInt(0, NUM_SCIENTIST_HEADS - 1);  // pick a head, any head
  ...
  if (m_nBody == HEAD_LUTHER) m_nSkin = 1;
...
// Output :
//-----------------------------------------------------------------------------
Class_T CNPC_Scientist::Classify() { return CLASS_HUMAN_PASSIVE; }

int CNPC_Scientist::GetSoundInterests() {
  return SOUND_WORLD | SOUND_COMBAT | SOUND_DANGER | SOUND_PLAYER;
}
...
void CNPC_Scientist::DeclineFollowing() {
  if (CanSpeakAfterMyself()) {
    Speak(SC_POK);
  }
}

bool CNPC_Scientist::CanBecomeRagdoll() {
...
void CNPC_Scientist::SUB_LVFadeOut() {
...
Activity CNPC_Scientist::GetStoppedActivity() {
  if (GetEnemy() != NULL) return (Activity)ACT_EXCITED;

  return BaseClass::GetStoppedActivity();
}

float CNPC_Scientist::MaxYawSpeed() {
  switch (GetActivity()) {
    case ACT_TURN_LEFT:
    case ACT_TURN_RIGHT:
      return 160;
...
void CNPC_Scientist::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    ...
    case TASK_RANDOM_SCREAM:
      if (random->RandomFloat(0, 1) < pTask->flTaskData) Scream();
      TaskComplete();
      break;
...
void CNPC_Scientist::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_RUN_PATH_SCARED:
      if (!IsMoving()) TaskComplete();
      if (random->RandomInt(0, 31) < 8) Scream();
      break;
    ...
    case TASK_MOVE_TO_TARGET_RANGE_SCARED: {
      ...
      if (GetTarget() == NULL) {
        ...
        distance =
            (GetNavigator()->GetPath()->ActualGoalPosition() - GetAbsOrigin())
                .Length2D();
        // Re-evaluate when you think your finished, or the target has moved too
        // far
        if ((distance < pTask->flTaskData) ||
            (GetNavigator()->GetPath()->ActualGoalPosition() -
             GetTarget()->GetAbsOrigin())
                    .Length() > pTask->flTaskData * 0.5) {
          GetNavigator()->GetPath()->ResetGoalPosition(
              GetTarget()->GetAbsOrigin());
          distance =
              (GetNavigator()->GetPath()->ActualGoalPosition() - GetAbsOrigin())
                  .Length2D();
        ...
        if (distance < pTask->flTaskData) {
          ...
          if (distance < 190 &&
              GetNavigator()->GetMovementActivity() != ACT_WALK_SCARED)
            GetNavigator()->SetMovementActivity(ACT_WALK_SCARED);
          else if (distance >= 270 &&
                   GetNavigator()->GetMovementActivity() != ACT_RUN_SCARED)
            GetNavigator()->SetMovementActivity(ACT_RUN_SCARED);
        }
      }
    } break;
    ...
    case TASK_HEAL:
      if (IsSequenceFinished()) {
        ...
        if (TargetDistance() > 90) TaskComplete();

        if (GetTarget())
          GetMotor()->SetIdealYaw(
              UTIL_VecToYaw(GetTarget()->GetAbsOrigin() - GetAbsOrigin()));
...
int CNPC_Scientist::OnTakeDamage_Alive(const CTakeDamageInfo &inputInfo) {
  if (inputInfo.GetInflictor() &&
      inputInfo.GetInflictor()->GetFlags() & FL_CLIENT) {
...
bool CNPC_Scientist::CanHeal() {
  CBaseEntity *pTarget = GetFollowTarget();

  if (pTarget == NULL) return false;

  if (pTarget->IsPlayer() == false) return false;

  if ((m_flHealTime > gpGlobals->curtime) ||
      (pTarget->m_iHealth > (pTarget->m_iMaxHealth * 0.5)))
    return false;
  ...
  switch (scheduleType) {
    ...
    case SCHED_TARGET_FACE: {
      ...
      if (baseType == SCHED_IDLE_STAND)
        return SCHED_TARGET_FACE;  // override this for different target face
                                   // behavior
      else
        return baseType;
    } break;
    ...
    case SCHED_IDLE_STAND: {
      ...
      if (baseType == SCHED_IDLE_STAND)
        return SCHED_SCI_IDLESTAND;  // override this for different target face
                                     // behavior
      else
        return baseType;
    } break;
...
Activity CNPC_Scientist::NPC_TranslateActivity(Activity newActivity) {
  if (GetFollowTarget() && GetEnemy()) {
    ...
    if (pEnemy != NULL) relationship = IRelationType(pEnemy);
...
int CNPC_Scientist::SelectSchedule() {
  if (m_NPCState == NPC_STATE_PRONE) {
    // Immediately call up to the talker code. Barnacle death is priority
    // schedule.
      ...
      if (TargetDistance() <= 128) {
        if (CanHeal())  // Heal opportunistically
        {
          ...
          return SCHED_SCI_HEAL;
             ...
             !(GetSpawnFlags() &
               SF_NPC_PREDISASTER)) {  // Player wants me to move
  ...
  switch (m_NPCState) {
    ...
    case NPC_STATE_IDLE:
      ...
      if (pEnemy) {
        ...
        else if (DisregardEnemy(pEnemy))  // After 15 seconds of being hidden,
                                          // return to alert
        {
          SetEnemy(NULL);
          pEnemy = NULL;
      ...
      if (HasCondition(COND_HEAR_DANGER) || HasCondition(COND_HEAR_COMBAT)) {
        ...
        if (pSound) {
          if (pSound->IsSoundType(SOUND_DANGER | SOUND_COMBAT)) {
            if (gpGlobals->curtime - m_flFearTime >
                3)  // Only cower every 3 seconds or so
            {
      ...
      if (GetFollowTarget()) {
        ...
        } else  // UNDONE: When afraid, scientist won't move out of your way.
                // Keep This?  If not, write move away scared
        {
          if (HasCondition(
                  COND_NEW_ENEMY))  // I just saw something new and scary, react
            return SCHED_SCI_FEAR;  // React to something scary
          return SCHED_SCI_FACETARGETSCARED;  // face and follow, but I'm
                                              // scared!
        }
      }
    ...
    case NPC_STATE_COMBAT:
      ...
      if (HasCondition(COND_SEE_ENEMY)) return SCHED_SCI_COVER;  // Take Cover

      if (HasCondition(COND_HEAR_COMBAT) || HasCondition(COND_HEAR_DANGER))
        return SCHED_TAKE_COVER_FROM_BEST_SOUND;  // Cower and panic from the
                                                  // scary sound!
...
NPC_STATE CNPC_Scientist::SelectIdealState() {
  switch (m_NPCState) {
    case NPC_STATE_ALERT:
    case NPC_STATE_IDLE:
      if (HasCondition(COND_NEW_ENEMY)) {
        if (GetFollowTarget() && GetEnemy()) {
          ...
          if (relationship != D_FR ||
              relationship != D_HT && (!HasCondition(COND_LIGHT_DAMAGE) ||
        ...
        if (GetFollowTarget()) StopFollowing();
      }
      break;
    ...
    case NPC_STATE_COMBAT: {
      ...
      if (pEnemy != NULL) {
        if (DisregardEnemy(
                pEnemy))  // After 15 seconds of being hidden, return to alert
        {
...
int CNPC_Scientist::FriendNumber(int arrayNumber) {
  ...
  if (arrayNumber < 3) return array[arrayNumber];
  return arrayNumber;
}

float CNPC_Scientist::TargetDistance() {
  CBaseEntity *pFollowTarget = GetFollowTarget();

  // If we lose the player, or he dies, return a really large distance
  if (pFollowTarget == NULL || !pFollowTarget->IsAlive()) return 1e6;
...
char *CNPC_DeadScientist::m_szPoses[] = {
    "lying_on_back", "lying_on_stomach", "dead_sitting", "dead_hang",
...
void CNPC_DeadScientist::Spawn() {
  ...
  if (m_nBody == -1) {  // -1 chooses a random head
    m_nBody =
        random->RandomInt(0, NUM_SCIENTIST_HEADS - 1);  // pick a head, any head
  }
  // Luther is black, make his hands black
...
void CNPC_SittingScientist::Spawn() {
  ...
  m_bloodColor = BLOOD_COLOR_RED;
  m_flFieldOfView =
      VIEW_FIELD_WIDE;  // indicates the width of this monster's forward view
  ...
  // Luther is black, make his hands black
  if (m_nBody == HEAD_LUTHER) m_nBody = 1;
...
void CNPC_SittingScientist::Precache() {
  m_baseSequence = LookupSequence("sitlookleft");
  TalkInit();
}
...
// sit, do stuff
//=========================================================
void CNPC_SittingScientist::SittingThink() {
  CBaseEntity *pent;
  ...
  if (0 && GetExpresser()->CanSpeakConcept(TLK_HELLO)) {
    ...
    if (pent) {
      float yaw =
          VecToYaw(pent->GetAbsOrigin() - GetAbsOrigin()) - GetAbsAngles().y;
...
//=========================================================
int CNPC_SittingScientist::FIdleSpeak() {
  ...
  GetExpresser()->BlockSpeechUntil(gpGlobals->curtime +
                                   random->RandomFloat(4.8, 5.2));
...
DEFINE_SCHEDULE(
    ...
    "SCHEDULE:SCHED_SCI_FOLLOWTARGET"
    "		TASK_FACE_IDEAL						0"
    "		TASK_SAY_HEAL						0"
...
//=========================================================
DEFINE_SCHEDULE(SCHED_SCI_FOLLOWTARGET,

                "	Tasks"
                //		"		TASK_SET_FAIL_SCHEDULE
                // SCHEDULE:SCHED_SCI_STOPFOLLOWING"
                "		TASK_GET_PATH_TO_TARGET			0"
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_SET_FAIL_SCHEDULE			"
    "SCHEDULE:SCHED_SCI_PANIC"
    "		TASK_STOP_MOVING				0"
    "		TASK_FIND_COVER_FROM_ENEMY		0"
    "		TASK_RUN_PATH_SCARED			0"
    "		TASK_TURN_LEFT					179"
    "		TASK_SET_SCHEDULE				"
    "SCHEDULE:SCHED_SCI_HIDE"
    "	"
    "	Interrupts"
    "		COND_NEW_ENEMY")
...
DEFINE_SCHEDULE(
    ...
    "		TASK_SET_ACTIVITY			ACTIVITY:ACT_CROUCHIDLE"
    "		TASK_SET_SCHEDULE			"
    "SCHEDULE:SCHED_SCI_FOLLOWSCARED"
    "	"
    "	Interrupts"
    "		COND_NEW_ENEMY"
    "		COND_HEAR_DANGER")
...
// > SCHED_FEAR
//=========================================================
DEFINE_SCHEDULE(SCHED_SCI_FEAR,
