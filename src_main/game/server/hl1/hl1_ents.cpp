...
/*********************
        TRIGGERS: trigger_auto, trigger_relay, multimanager: replaced in src by
logic_auto and logic_relay
*********************/
...
void CAutoTrigger::Spawn(void) { Precache(); }

void CAutoTrigger::Precache(void) { SetNextThink(gpGlobals->curtime + 0.1f); }

//-----------------------------------------------------------------------------
// Purpose: Checks the global state and fires targets if the global state is
// set.
//-----------------------------------------------------------------------------
...
void CTriggerRelay::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
class CMultiManager : public CPointEntity {
 ...
 public:
  ...
  void ManagerUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                  USE_TYPE useType, float value);
  ...
  string_t m_iTargetName[MAX_MULTI_TARGETS];  // list if indexes into global
                                              // string array
  float m_flTargetDelay[MAX_MULTI_TARGETS];   // delay (in seconds) from time of
                                              // manager fire to target fire
...
BEGIN_DATADESC(CMultiManager)
  DEFINE_KEYFIELD(m_flWait, FIELD_FLOAT, "wait"),
      ...
      // Function Pointers
      DEFINE_FUNCTION(ManagerThink), DEFINE_FUNCTION(ManagerUse),
#if _DEBUG
...
bool CMultiManager::KeyValue(const char *szKeyName, const char *szValue) {
  ...
  {
    // this assumes that additional fields are targetnames and their values are
    // delay values.
...
// so I changed it to use the standard target fire code, made it a little
// simpler.
...
void CMultiManager::ManagerUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                               USE_TYPE useType, float value) {
  ...
  // Calculate the time to re-enable the multimanager - just after the last
  // output is fired. dvsents2: need to disable multimanager until last output
  // is fired
...
BEGIN_DATADESC(CPendulum)
  DEFINE_FIELD(m_flAccel, FIELD_FLOAT), DEFINE_FIELD(m_flTime, FIELD_TIME),
      ...
      DEFINE_FUNCTION(PendulumUse), DEFINE_FUNCTION(Swing),
      DEFINE_FUNCTION(Stop), DEFINE_FUNCTION(RopeTouch),
...
void CPendulum::Spawn(void) {
  ...
  if (m_flMoveDistance != 0) {
    ...
    m_flAccel = (m_flSpeed * m_flSpeed) /
                (2 * fabs(m_flMoveDistance));  // Calculate constant
                                               // acceleration from speed and
                                               // distance
...
void CPendulum::PendulumUse(CBaseEntity *pActivator, CBaseEntity *pCaller,
                            USE_TYPE useType, float value) {
...
void CPendulum::Blocked(CBaseEntity *pOther) { m_flTime = gpGlobals->curtime; }
...
void CPendulum::RopeTouch(CBaseEntity *pOther) {
  if (!pOther->IsPlayer()) {  // not a player!
    DevMsg(2, "Not a client\n");
    return;
  }

  if (pOther == GetEnemy()) return;
...
class CFuncMortarField : public CBaseToggle {
 ...
 public:
  ...
  // void EXPORT FieldUse( CBaseEntity *pActivator, CBaseEntity *pCaller,
  // USE_TYPE useType, float value );
...
void CFuncMortarField::InputTrigger(inputdata_t &inputdata) {
  Vector vecStart;
  CollisionProp()->RandomPointInBounds(Vector(0, 0, 1), Vector(1, 1, 1),
                                       &vecStart);
  ...
  switch (m_fControl) {
    ...
    case 2:  // table
    {
      ...
      if (m_iszXController != NULL_STRING) {
        ...
        if (pController != NULL) {
          if (FClassnameIs(pController, "momentary_rot_button")) {
            CMomentaryRotButton *pXController =
                static_cast<CMomentaryRotButton *>(pController);
            Vector vecNormalizedPos(
                pXController->GetPos(pXController->GetLocalAngles()), 0.0f,
                0.0f);
            Vector vecWorldSpace;
            CollisionProp()->NormalizedToWorldSpace(vecNormalizedPos,
                                                    &vecWorldSpace);
            vecStart.x = vecWorldSpace.x;
          } else {
            DevMsg(
                "func_mortarfield has X controller that isn't a "
      ...
      if (m_iszYController != NULL_STRING) {
        ...
        if (pController != NULL) {
          if (FClassnameIs(pController, "momentary_rot_button")) {
            CMomentaryRotButton *pYController =
                static_cast<CMomentaryRotButton *>(pController);
            Vector vecNormalizedPos(
                0.0f, pYController->GetPos(pYController->GetLocalAngles()),
                0.0f);
            Vector vecWorldSpace;
            CollisionProp()->NormalizedToWorldSpace(vecNormalizedPos,
                                                    &vecWorldSpace);
            ...
            DevMsg(
                "func_mortarfield has Y controller that isn't a "
                "momentary_rot_button.\n");
          }
        }
      }
  ...
  for (int i = 0; i < m_iCount; i++) {
    ...
    trace_t tr;
    UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -1) * MAX_TRACE_LENGTH,
                   MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);

    CBaseEntity *pMortar = Create("monster_mortar", tr.endpos, QAngle(0, 0, 0),
                                  inputdata.pActivator);
    pMortar->SetNextThink(gpGlobals->curtime + t);
    t += random->RandomFloat(0.2, 0.5);

    if (i == 0) CSoundEnt::InsertSound(SOUND_DANGER, tr.endpos, 400, 0.3);
  }
}
...
void CMortar::MortarExplode(void) {
  ...
  UTIL_Beam(vecStart, vecEnd, m_spriteTexture, 0, 0, 0, 0.5, 4.0, 4.0, 100, 0,
            255, 160, 100, 128, 0);

  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin() + Vector(0, 0, 1024),
                 GetAbsOrigin() - Vector(0, 0, 1024), MASK_ALL, this,
...
char *CNPC_DeadHEV::m_szPoses[] = {"deadback", "deadsitting", "deadstomach",
                                   "deadtable"};
...
//
// This entity will copy its render parameters (renderfx, rendermode,
// rendercolor, renderamt) to its targets when triggered.
//

// Flags to indicate masking off various render parameters that are normally
...
void CRenderFxManager::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
                           USE_TYPE useType, float value) {
  if (m_target != NULL_STRING) {
    CBaseEntity *pEntity = NULL;
    while ((pEntity = gEntList.FindEntityByName(pEntity, STRING(m_target))) !=
           NULL) {
      if (!HasSpawnFlags(SF_RENDER_MASKFX)) pEntity->m_nRenderFX = m_nRenderFX;
...
BEGIN_DATADESC(CXenPLight)
  DEFINE_FIELD(m_pGlow, FIELD_CLASSPTR), DEFINE_FIELD(m_flDmgTime, FIELD_FLOAT),
END_DATADESC()
...
void CXenPLight::Spawn(void) {
  ...
  m_pGlow = CSprite::SpriteCreate(
      ...
      GetLocalOrigin() +
          Vector(0, 0, (WorldAlignMins().z + WorldAlignMaxs().z) * 0.5),
      FALSE);
  m_pGlow->SetTransparency(kRenderGlow, GetRenderColor().r, GetRenderColor().g,
                           GetRenderColor().b, GetRenderColor().a, m_nRenderFX);
...
void CXenPLight::Think(void) {
  ...
  switch (GetActivity()) {
    ...
    case ACT_STAND:
      if (IsSequenceFinished()) SetActivity(ACT_IDLE);
      break;
...
void CXenPLight::LightOn(void) {
  ...
  if (m_pGlow) m_pGlow->RemoveEffects(EF_NODRAW);
}
...
void CXenPLight::LightOff(void) {
  ...
  if (m_pGlow) m_pGlow->AddEffects(EF_NODRAW);
}
...
void CXenHair::Spawn(void) {
  ...
  SetMoveType(MOVETYPE_NONE);
  SetNextThink(gpGlobals->curtime +
               random->RandomFloat(0.1, 0.4));  // Load balance these a bit
}
...
void CXenHair::Precache(void) { PrecacheModel("models/hair.mdl"); }
...
class CXenTreeTrigger : public CBaseEntity {
 ...
 public:
  void Touch(CBaseEntity *pOther);
  static CXenTreeTrigger *TriggerCreate(CBaseEntity *pOwner,
                                        const Vector &position);
};
LINK_ENTITY_TO_CLASS(xen_ttrigger, CXenTreeTrigger);

CXenTreeTrigger *CXenTreeTrigger::TriggerCreate(CBaseEntity *pOwner,
                                                const Vector &position) {
...
void CXenTree::Touch(CBaseEntity *pOther) {
  if (!pOther->IsPlayer() && FClassnameIs(pOther, "monster_bigmomma")) return;
  ...
  switch (pEvent->event) {
    case TREE_AE_ATTACK: {
      ...
      int count = UTIL_EntitiesInBox(
          pList, 8, m_pTrigger->GetAbsOrigin() + m_pTrigger->WorldAlignMins(),
          m_pTrigger->GetAbsOrigin() + m_pTrigger->WorldAlignMaxs(),
          FL_NPC | FL_CLIENT);
      ...
      for (int i = 0; i < count; i++) {
        if (pList[i] != this) {
          if (pList[i]->GetOwnerEntity() != this) {
            sound = true;
            pList[i]->TakeDamage(
                CTakeDamageInfo(this, this, 25, DMG_CRUSH | DMG_SLASH));
            pList[i]->ViewPunch(QAngle(15, 0, 18));

            pList[i]->SetAbsVelocity(pList[i]->GetAbsVelocity() +
                                     forward * 100);
          }
        }
      }
...
class CXenHull : public CPointEntity {
 ...
 public:
  static CXenHull *CreateHull(CBaseEntity *source, const Vector &mins,
                              const Vector &maxs, const Vector &offset);
  Class_T Classify(void) { return CLASS_ALIEN_PREDATOR; }
};

CXenHull *CXenHull::CreateHull(CBaseEntity *source, const Vector &mins,
                               const Vector &maxs, const Vector &offset) {
...
const Vector CXenSporeLarge::m_hullSizes[] = {
    Vector(90, -25, 0),  Vector(25, 75, 0),  Vector(-15, -100, 0),
    Vector(-90, -35, 0), Vector(-90, 60, 0),
};
...
void CXenSporeLarge::Spawn(void) {
  ...
  for (int i = 0; i < std::size(m_hullSizes); i++) {
    CXenHull::CreateHull(
        this, Vector(-12, -12, 0), Vector(12, 12, 120),
...
void CXenSpore::Spawn(void) {
  ...
  ResetSequenceInfo();
  SetNextThink(gpGlobals->curtime +
               random->RandomFloat(0.1f, 0.4f));  // Load balance these a bit
}
...
void CXenSpore::Precache(void) { PrecacheModel((char *)pModelNames[m_nSkin]); }
...
void CHL1Gib::WaitTillLand(void) {
  ...
  if (GetAbsVelocity() == vec3_origin) {
    ...
    /*	if ( m_bloodColor != DONT_BLEED )
            {
                    // ok, start stinkin!
                    CSoundEnt::InsertSound ( bits_SOUND_MEAT, pev->origin, 384,
       25 );
            }*/
  } else {
...
void CHL1Gib::BounceGibTouch(CBaseEntity *pOther) {
  ...
  if (GetFlags() & FL_ONGROUND) {
    ...
    if (m_cBloodDecals > 0 && m_bloodColor != DONT_BLEED) {
      vecSpot =
          GetAbsOrigin() + Vector(0, 0, 8);  // move up a bit, and trace down.
      UTIL_TraceLine(vecSpot, vecSpot + Vector(0, 0, -24), MASK_SOLID_BRUSHONLY,
    ...
    if (m_material != matNone && random->RandomInt(0, 2) == 0) {
      ...
      CBreakable::MaterialSoundRandom(entindex(), (Materials)m_material,
                                      volume);
    }
  }
}
...
void CHL1Gib::StickyGibTouch(CBaseEntity *pOther) {
  ...
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + GetAbsVelocity() * 32,
                 MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &tr);
...
void CHL1Gib::Spawn(const char *szGibModel) {
  ...
  m_material = matNone;
  m_cBloodDecals = 5;  // how many blood decals this gib can place (1 per bounce
                       // until none remain).
}
...
BEGIN_DATADESC(CHL1Gib)
  // Function Pointers
  DEFINE_FUNCTION(BounceGibTouch), DEFINE_FUNCTION(StickyGibTouch),
