...
// HL1_DMG_SHOWNHUD: Add DMG_VEHICLE because HL2 hijacked those bits from
// DMG_FREEZE, which is what they are in HL1 HL1_DMG_SHOWNHUD: Add DMG_DISSOLVE
// because HL2 hijacked those bits from DMG_SLOWFREEZE, which is what they are
// in HL1 See Halflife1 GameRules - Damage_GetShowOnHud()
//#define HL1_DMG_SHOWNHUD	(DMG_POISON | DMG_ACID | DMG_FREEZE |
// DMG_SLOWFREEZE | DMG_DROWN | DMG_BURN | DMG_SLOWBURN | DMG_NERVEGAS |
...
//-----------------------------------------------------------------------------
bool CHL1_Player::HandleInteraction(int interactionType, void *data,
                                    CBaseCombatCharacter *sourceEnt) {
  if (interactionType == g_interactionBarnacleVictimDangle) {
    TakeDamage(CTakeDamageInfo(sourceEnt, sourceEnt, m_iHealth + ArmorValue(),
...
void CHL1_Player::StartPullingObject(CBaseEntity *pObject) {
  ...
  ballsocket.InitWithCurrentObjectState(
      VPhysicsGetObject(), pObject->VPhysicsGetObject(), WorldSpaceCenter());
  m_pPullConstraint = physenv->CreateBallsocketConstraint(
      VPhysicsGetObject(), pObject->VPhysicsGetObject(), NULL, ballsocket);
...
// Purpose:
//-----------------------------------------------------------------------------
void CHL1_Player::InitVCollision(const Vector &vecAbsOrigin,
...
void CHL1_Player::SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs,
                                  int pvssize) {
...
int CHL1_Player::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (ArmorValue() &&
      !(info.GetDamageType() &
        (DMG_FALL | DMG_DROWN |
         DMG_POISON)) &&  // armor doesn't protect against fall or drown damage!
      !(GetFlags() & FL_GODMODE)) {
  ...
  fTookDamage =
      CBaseCombatCharacter::OnTakeDamage(info);  // Bypass CBasePlayer's
  ...
  while (fTookDamage && (!ftrivial || bTimeBasedDamage) && ffound &&
         bitsDamage) {
    ffound = false;
...
void CHL1_Player::FindMissTargets(void) {
  if (m_flTargetFindTime > gpGlobals->curtime) return;
  ...
  int numEnts = UTIL_EntitiesInBox(pEnts, 256, GetAbsOrigin() - radius,
                                   GetAbsOrigin() + radius, 0);
  ...
  for (int i = 0; i < numEnts; i++) {
    ...
    if (FClassnameIs(pEnts[i], "prop_dynamic") ||
      ...
      // NDebugOverlay::Cross3D( pEnts[i]->WorldSpaceCenter(), -Vector(4,4,4),
      // Vector(4,4,4), 0, 255, 0, true, 1.0f );
...
bool CHL1_Player::GetMissPosition(Vector *position) {
  if (m_nNumMissPositions == 0) return false;

  (*position) =
      m_vecMissPositions[random->RandomInt(0, m_nNumMissPositions - 1)];
  ...
  if ((m_flFlashLightTime) && (m_flFlashLightTime <= gpGlobals->curtime)) {
    if (FlashlightIsOn()) {
      if (m_nFlashBattery) {
        ...
        if (!m_nFlashBattery) FlashlightTurnOff();
      }
    } else {
...
class CGrabController : public IMotionEvent {
 ...
 public:
  ...
  void AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity,
                    IPhysicsObject *pPhys, bool bIsMegaPhysCannon = false);
  void DetachEntity();
  void OnRestore();
...
void CGrabController::AttachEntity(CBasePlayer *pPlayer, CBaseEntity *pEntity,
  ...
  PhysicsImpactSound(pPlayer, pPhys, CHAN_STATIC, pPhys->GetMaterialIndex(),
                     pPlayer->VPhysicsGetObject()->GetMaterialIndex(), 1.0, 64);
  Vector position;
  QAngle angles;
  ...
  int count = pEntity->VPhysicsGetObjectList(pList, std::size(pList));
  m_flLoadWeight = 0;
  float damping = 10;
...
float CGrabController::GetSavedMass(IPhysicsObject *pObject) {
  ...
  if (pHeld) {
    if (pObject->GetGameData() == (void *)pHeld) {
      IPhysicsObject *pList[VPHYSICS_MAX_OBJECT_LIST_COUNT];
      int count = pHeld->VPhysicsGetObjectList(pList, std::size(pList));
      for (int i = 0; i < count; i++) {
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError) {
  ...
  /*
          ...
          {
                  NDebugOverlay::Box( end, -Vector( 2,2,2 ), Vector(2,2,2), 0,
     255, 0, true, 0 );

                  NDebugOverlay::Box( GetAttached()->WorldSpaceCenter(),
                                                          -Vector( radius,
     radius, radius), Vector( radius, radius, radius ), 255, 0, 0, true, 0.0f );
          }
  */
...
void CPlayerPickupController::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  if (ToBasePlayer(pActivator) == m_pPlayer) {
    ...
    if (m_pPlayer->m_nButtons & IN_ATTACK) {
      ...
      float massFactor =
          std::clamp(pAttached->VPhysicsGetObject()->GetMass(), 0.5, 15);
