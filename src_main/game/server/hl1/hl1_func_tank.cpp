...
class CFuncTank : public CBaseEntity {
 ...
 public:
  ...
  void TraceAttack(CBaseEntity *pAttacker, float flDamage, const Vector &vecDir,
                   trace_t *ptr, int bitsDamageType);
 ...
 protected:
  ...
  float m_yawRange;  // Range of turning motion (one-sided: 30 is +/- 30 degress
                     // from center) Zero is full rotation
  ...
  float m_persist;   // Persistence of firing (how long do I shoot when I can't
                     // see)
  float m_persist2;  // Secondary persistence of firing (randomly shooting when
                     // I can't see)
...
BEGIN_DATADESC(CFuncTank)
  ...
  DEFINE_KEYFIELD(m_yawRate, FIELD_FLOAT, "yawrate"),
      ...
      DEFINE_INPUTFUNC(FIELD_VECTOR, "SetTargetPosition",
                       InputSetTargetPosition),
      DEFINE_INPUTFUNC(FIELD_STRING, "SetTargetEntityName",
...
int CFuncTank::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    if (m_spawnflags & SF_TANK_AIM_AT_POS) {
      Q_snprintf(tempstr, sizeof(tempstr), "Aim Pos: %3.0f %3.0f %3.0f",
...
//			bitsDamageType -
//-----------------------------------------------------------------------------
void CFuncTank::TraceAttack(CBaseEntity *pAttacker, float flDamage,
...
//			pActivator -
//-----------------------------------------------------------------------------
CBaseEntity *CFuncTank::FindTarget(string_t targetName,
                                   CBaseEntity *pActivator) {
  return gEntList.FindEntityGenericNearest(STRING(targetName), GetAbsOrigin(),
                                           0, this, pActivator);
}
...
void CFuncTank::Activate(void) {
  ...
  if (m_iszControlVolume != NULL_STRING) {
    m_hControlVolume = dynamic_cast<CBaseTrigger *>(
        gEntList.FindEntityByName(NULL, m_iszControlVolume));
  }

  if ((!m_hControlVolume) && (m_spawnflags & SF_TANK_CANCONTROL)) {
    Msg("ERROR: Couldn't find control volume for player-controllable func_tank "
        "%s.\n",
...
void CFuncTank::Precache(void) {
  ...
  if (m_iszSpriteSmoke != NULL_STRING) PrecacheModel(STRING(m_iszSpriteSmoke));
  if (m_iszSpriteFlash != NULL_STRING) PrecacheModel(STRING(m_iszSpriteFlash));
...
bool CFuncTank::OnControls(CBaseEntity *pTest) {
  if (!(m_spawnflags & SF_TANK_CANCONTROL)) return FALSE;
...
bool CFuncTank::StartControl(CBasePlayer *pController) {
  ...
  if (m_iszMaster != NULL_STRING) {
    if (!UTIL_IsMasterTriggered(m_iszMaster, pController)) return FALSE;
  }
...
void CFuncTank::ControllerPostFrame(void) {
  ...
  if (m_pController->m_nButtons & IN_ATTACK) {
    m_fireLast = gpGlobals->curtime - (1 / m_fireRate) -
                 0.01;  // to make sure the gun doesn't fire too many bullets
    ...
    CSoundEnt::InsertSound(SOUND_COMBAT, WorldSpaceCenter(),
                           FUNCTANK_FIREVOLUME, 0.2);

    m_flNextAttack = gpGlobals->curtime + (1 / m_fireRate);
  }
}

void CFuncTank::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
  ...
  if (m_spawnflags & SF_TANK_CANCONTROL) {
    ...
    if (value == 2 && useType == USE_SET) {
      ...
      if (!m_hControlVolume->IsTouching(pPlayer)) return;
    ...
    if (!ShouldToggle(useType, IsActive())) return;
...
bool CFuncTank::InRange(float range) {
  if (range < m_minRange) return FALSE;
  if (m_maxRange > 0 && range > m_maxRange) return FALSE;
...
void CFuncTank::Think(void) {
  ...
  if (fabs(GetLocalAngularVelocity().x) > 1 ||
      fabs(GetLocalAngularVelocity().y) > 1)
    StartRotSound();
  else
    StopRotSound();
}
...
QAngle CFuncTank::AimBarrelAt(const Vector &parentTarget) {
  ...
  if (quadTarget <= m_barrelPos.LengthSqr()) {
    ...
    float centerToGunYaw = atan2(
        m_barrelPos.y, sqrt(quadTarget - (m_barrelPos.y * m_barrelPos.y)));

    float targetToCenterPitch = atan2(target.z, sqrt(quadTargetXY));
    float centerToGunPitch = atan2(
        -m_barrelPos.z, sqrt(quadTarget - (m_barrelPos.z * m_barrelPos.z)));
    return QAngle(-RAD2DEG(targetToCenterPitch + centerToGunPitch),
...
void CFuncTank::TrackTarget(void) {
  ...
  if (m_pController) {
    ...
    if (m_spawnflags & SF_TANK_AIM_AT_POS) {
      ...
      if (!pEntity || (pEntity->GetFlags() & FL_NOTARGET)) {
        ...
        {
          m_hTarget =
              ToBasePlayer(GetContainingEntity(UTIL_FindClientInPVS(edict())));
        }
    ...
    UTIL_TraceLine(barrelEnd, worldTargetPosition, MASK_SHOT, this,
    ...
    if (m_spawnflags & SF_TANK_AIM_AT_POS) {
      ...
      if (tr.fraction == 1.0 || tr.m_pEnt == pTarget) {
        ...
        if (InRange(range) && pInstance && pInstance->IsAlive()) {
          ...
          // Sight position is BodyTarget with no noise (so gun doesn't bob up
          // and down)
  ...
  if (bOutsidePitchRange || bOutsideYawRange ||
      (vecToTarget.Length() < (barrelEnd - GetAbsOrigin()).Length())) {
  ...
  angles.x = std::clamp(angles.x, m_pitchCenter - m_pitchRange,
                        m_pitchCenter + m_pitchRange);
  ...
  if (CanFire() &&
      ((fabs(distX) < m_pitchTolerance && fabs(distY) < m_yawTolerance) ||
       (m_spawnflags & SF_TANK_LINEOFSIGHT))) {
    bool fire = FALSE;
    Vector forward;
    ...
    if (m_spawnflags & SF_TANK_LINEOFSIGHT) {
      ...
      UTIL_TraceLine(barrelEnd, barrelEnd + forward * length, MASK_SHOT, this,
                     COLLISION_GROUP_NONE, &tr);

      if (tr.m_pEnt == pTarget) fire = TRUE;
    } else
      fire = TRUE;
...
//			*pAttacker -
//-----------------------------------------------------------------------------
void CFuncTank::FiringSequence(const Vector &barrelEnd, const Vector &forward,
                               CBaseEntity *pAttacker) {
  if (m_fireLast != 0) {
    int bulletCount = (gpGlobals->curtime - m_fireLast) * m_fireRate;
...
//			pAttacker -
//-----------------------------------------------------------------------------
void CFuncTank::Fire(int bulletCount, const Vector &barrelEnd,
                     const Vector &forward, CBaseEntity *pAttacker) {
  if (m_iszSpriteSmoke != NULL_STRING) {
    CSprite *pSprite =
        CSprite::SpriteCreate(STRING(m_iszSpriteSmoke), barrelEnd, TRUE);
    pSprite->AnimateAndDie(random->RandomFloat(15.0, 20.0));
    pSprite->SetTransparency(kRenderTransAlpha, m_clrRender->r, m_clrRender->g,
                             m_clrRender->b, 255, kRenderFxNone);
  ...
  if (m_iszSpriteFlash != NULL_STRING) {
    CSprite *pSprite =
        CSprite::SpriteCreate(STRING(m_iszSpriteFlash), barrelEnd, TRUE);
...
void CFuncTank::TankTrace(const Vector &vecStart, const Vector &vecForward,
                          const Vector &vecSpread, trace_t &tr) {
  ...
  Vector vecDir = vecForward + x * vecSpread.x * right + y * vecSpread.y * up;
  Vector vecEnd;
...
void CFuncTank::StartRotSound(void) {
  if (m_spawnflags & SF_TANK_SOUNDON) return;
...
class CFuncTankGun : public CFuncTank {
 public:
  ...
  void Fire(int bulletCount, const Vector &barrelEnd, const Vector &forward,
            CBaseEntity *pAttacker);
};
LINK_ENTITY_TO_CLASS(func_tank, CFuncTankGun);

void CFuncTankGun::Fire(int bulletCount, const Vector &barrelEnd,
                        const Vector &forward, CBaseEntity *pAttacker) {
  int i;
  ...
  for (i = 0; i < bulletCount; i++) {
    switch (m_bulletType) {
      case TANK_BULLET_SMALL:
        FireBullets(1, barrelEnd, forward, gTankSpread[m_spread],
                    MAX_TRACE_LENGTH, m_iSmallAmmoType, 1, -1, -1,
                    m_iBulletDamage, pAttacker);
        break;

      case TANK_BULLET_MEDIUM:
        FireBullets(1, barrelEnd, forward, gTankSpread[m_spread],
                    MAX_TRACE_LENGTH, m_iMediumAmmoType, 1, -1, -1,
                    m_iBulletDamage, pAttacker);
        break;

      case TANK_BULLET_LARGE:
        FireBullets(1, barrelEnd, forward, gTankSpread[m_spread],
                    MAX_TRACE_LENGTH, m_iLargeAmmoType, 1, -1, -1,
                    m_iBulletDamage, pAttacker);
        break;
      default:
...
// Output  :
//------------------------------------------------------------------------------
void CFuncTankPulseLaser::Fire(int bulletCount, const Vector &barrelEnd,
  ...
  for (int i = 0; i < bulletCount; i++) {
    ...
    Vector vecDir = vecForward + x * gTankSpread[m_spread].x * vecRight +
                    y * gTankSpread[m_spread].y * vecUp;

    CGrenadeBeam *pPulse = CGrenadeBeam::Create(pAttacker, barrelEnd);
    pPulse->Format(m_flPulseColor, m_flPulseWidth);
    pPulse->Shoot(vecDir, m_flPulseSpeed, m_flPulseLife, m_flPulseLag,
                  m_iBulletDamage);
...
void CFuncTankLaser::Think(void) {
  if (m_pLaser && (gpGlobals->curtime > m_laserTime)) m_pLaser->TurnOff();

  CFuncTank::Think();
}

void CFuncTankLaser::Fire(int bulletCount, const Vector &barrelEnd,
                          const Vector &forward, CBaseEntity *pAttacker) {
  int i;
  trace_t tr;
...
void CFuncTankRocket::Fire(int bulletCount, const Vector &barrelEnd,
                           const Vector &forward, CBaseEntity *pAttacker) {
  int i;

  for (i = 0; i < bulletCount; i++) {
    CBaseEntity *pRocket =
        CBaseEntity::Create("rpg_rocket", barrelEnd, GetAbsAngles(), this);
...
class CFuncTankMortar : public CFuncTank {
 public:
  ...
  void Precache(void);
  void FiringSequence(const Vector &barrelEnd, const Vector &forward,
                      CBaseEntity *pAttacker);
  void Fire(int bulletCount, const Vector &barrelEnd, const Vector &vecForward,
            CBaseEntity *pAttacker);
  void ShootGun(void);
...
void CFuncTankMortar::FiringSequence(const Vector &barrelEnd,
  ...
  if (m_fireLast != 0) {
    ...
    if (bulletCount > 0) {
      ...
      if (m_fireDelay != 0) {
        g_EventQueue.AddEvent(this, "ShootGun", m_fireDelay, pAttacker, this,
                              0);
      } else {
        ShootGun();
      }
...
void CFuncTankMortar::Fire(int bulletCount, const Vector &barrelEnd,
                           const Vector &vecForward, CBaseEntity *pAttacker) {
  if (m_fireEndSound != NULL_STRING) {
    CPASAttenuationFilter filter(this);
...
void CFuncTankPhysCannister::Activate(void) {
  ...
  if (m_iszBarrelVolume != NULL_STRING) {
    m_hBarrelVolume = dynamic_cast<CBaseTrigger *>(
        gEntList.FindEntityByName(NULL, m_iszBarrelVolume));
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CFuncTankPhysCannister::Fire(int bulletCount, const Vector &barrelEnd,
                                  const Vector &forward,
                                  CBaseEntity *pAttacker) {
  Assert(m_hBarrelVolume);

  // Do we have a cannister in our barrel volume?
  CPhysicsCannister *pCannister =
      (CPhysicsCannister *)m_hBarrelVolume->GetTouchedEntityOfType(
          "physics_cannister");
  if (!pCannister) {
