...
class CNPC_Osprey : public CBaseHelicopter {
 ...
 public:
  ...
  /*
          ...
          void CrashTouch( CBaseEntity *pOther );
          void DyingThink( void );
          void CommandUse( CBaseEntity *pActivator, CBaseEntity *pCaller,
...
BEGIN_DATADESC(CNPC_Osprey)
  DEFINE_FIELD(m_startTime, FIELD_TIME),
      ...
      DEFINE_THINKFUNC(FindAllThink), DEFINE_THINKFUNC(DeployThink),
...
void CNPC_Osprey::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                              trace_t *ptr) {
  ...
  if (flDamage > 50 || ptr->hitgroup == 1 || ptr->hitgroup == 2 ||
      ptr->hitgroup == 3) {
...
void CNPC_Osprey::InitializeRotorSound(void) {
  ...
  m_pRotorSound = controller.SoundCreate(filter, entindex(), CHAN_STATIC,
                                         "Apache.RotorSpinup", 0.2);
...
void CNPC_Osprey::FindAllThink(void) {
  ...
  while ((pEntity = gEntList.FindEntityByClassname(
              pEntity, "monster_human_grunt")) != NULL) {
    if (m_iUnits > MAX_CARRY) break;
...
void CNPC_Osprey::DeployThink(void) {
  ...
  trace_t tr;
  UTIL_TraceLine(GetAbsOrigin(), GetAbsOrigin() + Vector(0, 0, -4096.0),
...
bool CNPC_Osprey::HasDead() {
  for (int i = 0; i < m_iUnits; i++) {
    if (m_hGrunt[i] == NULL || !m_hGrunt[i]->IsAlive()) {
      ...
      m_vecOrigin[i] =
          m_hGrunt[i]->GetAbsOrigin();  // send them to where they died
    }
  }
  return FALSE;
}
...
void CNPC_Osprey::HoverThink(void) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (pRepel != NULL && pRepel->m_iHealth > 0 &&
        pRepel->GetMoveType() == MOVETYPE_FLYGRAVITY) {
      break;
    }
  }

  if (i == 4) m_iRepelState = LOADED_WITH_GRUNTS;
...
CAI_BaseNPC *CNPC_Osprey::MakeGrunt(Vector vecSrc) {
  ...
  trace_t tr;
  UTIL_TraceLine(vecSrc, vecSrc + Vector(0, 0, -4096.0), MASK_NPCSOLID, this,
                 COLLISION_GROUP_NONE, &tr);

  if (tr.m_pEnt && tr.m_pEnt->GetSolid() != SOLID_BSP) return NULL;
  ...
  for (int i = 0; i < m_iUnits; i++) {
    if (m_hGrunt[i] == NULL || !m_hGrunt[i]->IsAlive()) {
      ...
      pBeam->SetNextThink(gpGlobals->curtime +
                          -4096.0 * tr.fraction / pGrunt->GetAbsVelocity().z +
                          0.5);

      // ALERT( at_console, "%d at %.0f %.0f %.0f\n", i, m_vecOrigin[i].x,
      // m_vecOrigin[i].y, m_vecOrigin[i].z );
...
void CNPC_Osprey::Flight(void) {
  if (m_iRepelState == LOADED_WITH_GRUNTS) {
    ...
    if (m_angleVelocity > 0) {
      m_iRotorAngle =
          UTIL_Approach(-45, m_iRotorAngle, 5 * (m_angleVelocity / 10));
    } else {
...
void CNPC_Osprey::PrescheduleThink(void) {
  ...
  if (m_startTime != 0.0 && m_startTime <= gpGlobals->curtime) FindAllThink();
...
void CNPC_Osprey::DyingThink(void) {
  ...
  if (gpGlobals->curtime > m_flNextCrashExplosion) {
    ...
    te->Explosion(filter, 0.0, &pos, g_sModelIndexFireball, 10, 15,
                  TE_EXPLFLAG_NONE, 100, 0);
    ...
    switch (m_iNextCrashModel) {
      case 0: {
        te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize,
                       vec3_origin, m_iTailGibs, 100, 0, 2.5, BREAK_METAL);
        break;
      }
      case 1: {
        te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize,
                       vec3_origin, m_iBodyGibs, 100, 0, 2.5, BREAK_METAL);
        break;
      }
      case 2: {
        te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize,
                       vec3_origin, m_iEngineGibs, 100, 0, 2.5, BREAK_METAL);
        break;
      }
      case 3: {
        te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize,
                       vec3_origin, m_nDebrisModel, 100, 0, 2.5, BREAK_METAL);
        break;
      }
    }
...
void CNPC_Osprey::CrashTouch(CBaseEntity *pOther) {
  ...
  te->BreakModel(filter, 0.0, GetAbsOrigin(), vec3_angle, vecSize, vec3_origin,
                 m_iTailGibs, 100, 0, 2.5, BREAK_METAL);
...
BEGIN_DATADESC(CBaseHelicopter)
  ...
  DEFINE_THINKFUNC(HelicopterThink), DEFINE_THINKFUNC(CallDyingThink),
...
void CBaseHelicopter::Hunt(void) {
  ...
  if (m_fHelicopterFlags & BITS_HELICOPTER_GUN_ON) {
    if ((m_flLastSeen + 1 > gpGlobals->curtime) &&
        (m_flPrevSeen + 2 < gpGlobals->curtime)) {
      if (FireGun()) {
...
void CBaseHelicopter::FlyPathCorners(void) {
  if (GetGoalEnt() == NULL &&
      m_target != NULL_STRING)  // this monster has a target
  {
  ...
  if (GetGoalEnt()) {
    // ALERT( at_console, "%.0f\n", flLength );
    if (HasReachedTarget()) {
      // If we get this close to the desired position, it's assumed that we've
      // reached the desired position, so move on.
      ...
      if (GetGoalEnt()) {
        ...
        // NDebugOverlay::Box( m_vecDesiredPosition, Vector( -16, -16, -16 ),
        // Vector( 16, 16, 16 ), 0,255,0, false, 30.0);
      }
    }
  } else {
...
void CBaseHelicopter::Flight(void) {
  ...
  AngleVectors(GetLocalAngles() + GetLocalAngularVelocity() * 2, &forward,
               &right, &up);
...
#define FORCE_NEGDELTA 8
  ...
  if (m_flForce < MAX_FORCE && vecEst.z < m_vecDesiredPosition.z) {
    ...
    if (vecEst.z > m_vecDesiredPosition.z) m_flForce -= FORCE_NEGDELTA;
  }
...
void CBaseHelicopter::FlyTouch(CBaseEntity *pOther) {
  ...
  if (pOther->GetSolid() == SOLID_BSP) {
    ...
    ApplyAbsVelocityImpulse(tr.plane.normal *
                            (GetAbsVelocity().Length() + 200));
  }
}
...
void CBaseHelicopter::CrashTouch(CBaseEntity *pOther) {
  ...
  if (pOther->GetSolid() == SOLID_BSP) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      pos.y += random->RandomFloat(-150, 150);
      pos.z += random->RandomFloat(-150, -50);
      te->Explosion(filter, std::min(0.99, i * 0.2), &pos,
                    g_sModelIndexFireball, 10, 15, TE_EXPLFLAG_NONE, 100, 0);
    }
...
void CBaseHelicopter::TraceAttack(const CTakeDamageInfo &info,
  ...
  // ignore blades
  //	if (ptr->hitgroup == 6 && (info.GetDamageType() &
  //(DMG_ENERGYBEAM|DMG_BULLET|DMG_CLUB))) 		return;
...
void CBaseHelicopter::ChangePathCorner(const char *pszName) {
  ...
  if (GetGoalEnt()) {
    ...
    // I don't think we need to do this. The FLIGHT() code will do it for us
    // (sjb)
    if (GetGoalEnt()) {
      m_vecDesiredPosition = GetGoalEnt()->GetLocalOrigin();
      AngleVectors(GetGoalEnt()->GetLocalAngles(), &m_vecGoalOrientation);
