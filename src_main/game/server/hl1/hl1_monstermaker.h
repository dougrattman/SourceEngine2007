...
class CNPCMaker : public CBaseEntity {
 public:
  ...
  void DeathNotice(CBaseEntity *pChild);  // NPC maker children use this to tell
  ...
  int m_iMaxLiveChildren;  // max number of NPCs that this maker may have out at
                           // one time.
  string_t m_iszNPCClassname;  // classname of the NPC(s) that will be created.

  COutputEvent m_OnSpawnNPC;

  int m_cLiveChildren;  // how many NPCs made by this NPC maker that are
                        // currently alive

  float m_flGround;  // z coord of the ground under me, used to make sure no
                     // NPCs are under the maker when it drops a new child
  bool m_bDisabled;
};
