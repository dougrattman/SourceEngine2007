...
BEGIN_DATADESC(CRopeAnchor)
  DEFINE_FIELD(m_hRope, FIELD_EHANDLE),
      ...
      DEFINE_THINKFUNC(FallThink), DEFINE_THINKFUNC(RemoveThink),
END_DATADESC();
...
void CRopeAnchor::Spawn() {
  ...
  m_hRope = CRopeKeyframe::CreateWithSecondPointDetached(
      this, -1, flDist, RAPPEL_ROPE_WIDTH, "cable/cable.vmt", 5, true);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define RAPPEL_MAX_SPEED 600.0f  // Go this fast if you're really high.
#define RAPPEL_MIN_SPEED 60.0f   // Go no slower than this.
#define RAPPEL_DECEL_DIST \
  (20.0f * 12.0f)  // Start slowing down when you're this close to the ground.
void CAI_RappelBehavior::SetDescentSpeed() {
  // Trace to the floor and see how close we're getting. Slow down if we're
  // close. STOP if there's an NPC under us.
  trace_t tr;
  AI_TraceLine(GetOuter()->GetAbsOrigin(),
               GetOuter()->GetAbsOrigin() - Vector(0, 0, 8192), MASK_SHOT,
               GetOuter(), COLLISION_GROUP_NONE, &tr);
  ...
  if (flDist <= RAPPEL_DECEL_DIST) {
    ...
    speed = std::max(RAPPEL_MIN_SPEED, speed * factor);
  }
...
void CAI_RappelBehavior::CleanupOnDeath(CBaseEntity *pCulprit,
                                        bool bFireDeathOutput) {
  BaseClass::CleanupOnDeath(pCulprit, bFireDeathOutput);

  // This will remove the beam and create a rope if the NPC dies while rappeling
  // down.
  if (m_hLine) {
...
void CAI_RappelBehavior::StartTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_MOVE_AWAY_PATH:
      GetOuter()->GetMotor()->SetIdealYaw(
          UTIL_AngleMod(GetOuter()->GetLocalAngles().y - 180.0f));
    ...
    case TASK_HIT_GROUND:
      ...
      if (GetOuter()->GetGroundEntity() != NULL &&
          GetOuter()->GetGroundEntity()->IsNPC() &&
          GetOuter()->GetGroundEntity()->m_iClassname ==
              GetOuter()->m_iClassname) {
        // Although I tried to get NPC's out from under me, I landed on one.
        // Kill it, so long as it's the same type of character as me.
        variant_t val;
        val.SetFloat(0);
        g_EventQueue.AddEvent(GetOuter()->GetGroundEntity(), "sethealth", val,
                              0, GetOuter(), GetOuter());
      }
...
void CAI_RappelBehavior::RunTask(const Task_t *pTask) {
  switch (pTask->iTask) {
    case TASK_RAPPEL: {
      // If we don't do this, the beam won't show up sometimes. Ideally, all
      // beams would update their bboxes correctly, but we're close to shipping
      ...
      if (GetOuter()->GetFlags() & FL_ONGROUND) {
        ...
        if (pGroundEnt && pGroundEnt->IsPlayer()) {
          // try to shove the player in the opposite direction as they are
          // facing (so they'll see me)
          Vector vecForward;
...
void CAI_RappelBehavior::BeginRappel() {
  ...
  UTIL_TraceEntity(GetOuter(), GetAbsOrigin(),
                   GetAbsOrigin() - Vector(0, 0, 4096), MASK_SHOT, GetOuter(),
  ...
  if (tr.m_pEnt != NULL && tr.m_pEnt->IsNPC()) {
    ...
    CSoundEnt::InsertSound(SOUND_DANGER,
                           tr.m_pEnt->EarPosition() - forward * 12.0f, 32.0f,
                           0.2f, GetOuter());
  }
}
...
DEFINE_SCHEDULE(
    ...
    "		TASK_RAPPEL				0"
    "		TASK_SET_SCHEDULE		"
    "SCHEDULE:SCHED_CLEAR_RAPPEL_POINT"
    ""
    "	Interrupts"
    ""
    "		COND_NEW_ENEMY"  // Only so the enemy selection code will pick
                                 // an enemy!
);
...
DEFINE_SCHEDULE(
    ...
    "	Tasks"
    "		TASK_HIT_GROUND			0"
    "		TASK_MOVE_AWAY_PATH		128"  // Clear this spot for
                                                      // other rappellers
    "		TASK_RUN_PATH			0"
