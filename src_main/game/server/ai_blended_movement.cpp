...
void CAI_BlendedMotor::ResetGoalSequence(void) {
  ...
  m_nSavedTranslatedGoalActivity =
      GetOuter()->NPC_TranslateActivity(m_nSavedGoalActivity);

  m_nGoalSequence = GetNavigator()->GetArrivalSequence(m_nPrimarySequence);
  // Msg("Start %s end %s\n", GetOuter()->GetSequenceName( m_nPrimarySequence ),
  // GetOuter()->GetSequenceName( m_nGoalSequence ) );
...
void CAI_BlendedMotor::MoveStop() {
  ...
  // 	int nNextSequence = FindTransitionSequence(GetSequence(),
  // m_nIdealSequence, NULL);
}
...
void CAI_BlendedMotor::MoveContinue() {
  ...
  if (m_nPrimarySequence == ACT_INVALID) return;
...
//-----------------------------------------------------------------------------
// Purpose: for the MoveInterval, interpolate desired speed, calc actual
// distance traveled
//-----------------------------------------------------------------------------
float CAI_BlendedMotor::GetMoveScriptDist(float &flNewSpeed) {
  ...
  for (i = 0; i < m_scriptMove.Count() - 1; i++) {
    if (t < m_scriptMove[i].flTime) {
      ...
      flNewSpeed = m_scriptMove[i].flMaxVelocity * (1 - a) +
                   m_scriptMove[i + 1].flMaxVelocity * a;
...
float CAI_BlendedMotor::GetMoveScriptYaw(void) {
  ...
  for (i = 0; i < m_scriptTurn.Count() - 1; i++) {
    if (t < m_scriptTurn[i].flTime) {
      ...
      float deltaYaw =
          UTIL_AngleDiff(m_scriptTurn[i + 1].flYaw, m_scriptTurn[i].flYaw);
...
void CAI_BlendedMotor::SetMoveScriptAnim(float flNewSpeed) {
  ...
  if (flWeight < 0.0) {
    ...
    m_flCurrRate = std::clamp(m_flCurrRate, 0.0f, 1.0f);
    SetPlaybackRate(m_flCurrRate);
    flWeight = 0.0;
  }
  // Msg("weight %.3f rate %.3f\n", flWeight, m_flCurrRate );
  m_flCurrRate = std::min(m_flCurrRate + (1.0 - m_flCurrRate) * 0.8, 1.0);
  ...
  if (m_flSecondaryWeight == 1.0 ||
      (m_iSecondaryLayer != -1 && m_nPrimarySequence == m_nSecondarySequence)) {
    // secondary layer at full strength last time, delete the primary and shift
    // down
  ...
  if (m_nSecondarySequence == ACT_INVALID) {
    ...
    if (m_nGoalSequence != ACT_INVALID &&
        m_nPrimarySequence != m_nGoalSequence) {
      // Msg("From %s to %s\n", GetOuter()->GetSequenceName( m_nPrimarySequence
      // ), GetOuter()->GetSequenceName( m_nGoalSequence ) );
      m_nSecondarySequence = GetOuter()->FindTransitionSequence(
          m_nPrimarySequence, m_nGoalSequence, NULL);
      if (m_nSecondarySequence == ACT_INVALID)
  ...
  if (m_nSecondarySequence != ACT_INVALID) {
    ...
    if (m_flSecondaryWeight < 1.0) {
      SetLayerWeight(m_iPrimaryLayer,
                     (flWeight - m_flSecondaryWeight * flWeight) /
                         (1.0f - m_flSecondaryWeight * flWeight));
    ...
    if (m_iSecondaryLayer != -1) {
      // secondary layer at full strength last time, delete the primary and
      // shift down
    ...
    // debounce
    // flWeight = flWeight * 0.5 + 0.5 * GetOuter()->GetLayerWeight(
    // m_iPrimaryLayer );
...
//-----------------------------------------------------------------------------
// Purpose: get the "idle" animation to play as the compliment to the movement
// animation
//-----------------------------------------------------------------------------
int CAI_BlendedMotor::GetInteriorSequence(int fromSequence) {
  ...
  if (m_nInteriorSequence != ACT_INVALID &&
      sequence == m_nPrevMovementSequence) {
  ...
  // Msg("sequence %d : %s (%d)\n", sequence,  GetOuter()->GetSequenceName(
  // sequence ), seqKeyValues != NULL );
  if (seqKeyValues) {
    ...
    if (pkvInterior) {
      ...
      if (activity != ACT_INVALID) {
        m_nInteriorSequence = GetOuter()->SelectWeightedSequence(
            GetOuter()->TranslateActivity(activity), fromSequence);
      } else {
...
AIMotorMoveResult_t CAI_BlendedMotor::MoveGroundExecute(
    const AILocalMoveGoal_t &move, AIMoveTrace_t *pTraceResult) {
  AI_PROFILE_SCOPE(CAI_BlendedMotor_MoveGroundExecute);

  if (move.curExpectedDist < 0.001) {
    AIMotorMoveResult_t result =
        BaseClass::MoveGroundExecute(move, pTraceResult);
  ...
  gpGlobals->curtime, flNewSpeed, move.maxDist, move.transitionDist,
  GetOuter()->m_pHintNode != NULL );
          // DevMsg( "%6.2f : Speed %.1f : %.1f\n", gpGlobals->curtime,
  flNewSpeed, GetIdealSpeed() );
  }
  */
  ...
  flNewSpeed, GetIdealSpeed(), flNewSpeed / GetIdealSpeed() );
  }
  */

  AIMotorMoveResult_t result =
      MoveGroundExecuteWalk(move, flNewSpeed, flTotalDist, pTraceResult);

  return result;
}

AIMotorMoveResult_t CAI_BlendedMotor::MoveFlyExecute(
    const AILocalMoveGoal_t &move, AIMoveTrace_t *pTraceResult) {
  ...
  GetMoveProbe()->MoveLimit(NAV_FLY, vecStart, vecEnd, MASK_NPCSOLID, NULL,
                            &moveTrace);
  if (pTraceResult) *pTraceResult = moveTrace;
...
void CAI_BlendedMotor::RecalculateYawSpeed() {
  ...
  if (GetOuter()->HasMemory(bits_MEMORY_TURNING)) return;
...
void CAI_BlendedMotor::MoveClimbStart(const Vector &climbDest,
                                      const Vector &climbDir, float climbDist,
                                      float yaw) {
...
void CAI_BlendedMotor::BuildMoveScript(const AILocalMoveGoal_t &move,
  ...
  /*
          if (GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT)
          {
                  ...
                  {
                          NDebugOverlay::Line( m_scriptMove[i-1].vecLocation,
  m_scriptMove[i].vecLocation, 255,255,255, true, 0.1 );

                          NDebugOverlay::Box( m_scriptMove[i].vecLocation,
  Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 0,255,255, 0, 0.1 );

                          //NDebugOverlay::Line( m_scriptMove[i].vecLocation,
  m_scriptMove[i].vecLocation + Vector( 0,0,m_scriptMove[i].flMaxVelocity),
  0,255,255, true, 0.1 );

                          Vector vecMidway = m_scriptMove[i].vecLocation +
  ((m_scriptMove[i-1].vecLocation - m_scriptMove[i].vecLocation) * 0.5);
                          NDebugOverlay::Text( vecMidway, UTIL_VarArgs( "%d", i
  ), false, 0.1 );
                  }
  #endif
  #if 0
                  ...
                  {
                          NDebugOverlay::Line( m_scriptTurn[i-1].vecLocation,
  m_scriptTurn[i].vecLocation, 255,255,255, true, 0.1 );

                          NDebugOverlay::Box( m_scriptTurn[i].vecLocation,
  Vector( -2, -2, -2 ), Vector( 2, 2, 2 ), 255,0,0, 0, 0.1 );

                          NDebugOverlay::Line( m_scriptTurn[i].vecLocation +
  Vector( 0,0,1), m_scriptTurn[i].vecLocation + Vector( 0,0,1) +
  UTIL_YawToVector( m_scriptTurn[i].flYaw ) * 32, 255,0,0, true, 0.1 );
                  }
  #endif
          }
  */
...
void CAI_BlendedMotor::BuildTurnScript(const AILocalMoveGoal_t &move) {
  ...
  // insert default turn parameters, try to turn 80% to goal at all corners
  ...
  for (i = 0; i < m_scriptMove.Count(); i++) {
    ...
    if (pCurWaypoint) {
      ...
      m_scriptTurn[prev].flTime =
          script.flElapsedTime - m_scriptTurn[prev].flElapsedTime;
      ...
      if (pCurWaypoint->GetNext()) {
        ...
        if (deltaYaw > 0.1) {
          ...
          m_scriptTurn.AddToTail(script);
          // DevMsg("turn waypoint %.1f %.1f %.1f\n", script.vecLocation.x,
          // script.vecLocation.y, script.vecLocation.z );
          prev++;
        }
      } else {
  ...
  for (i = m_scriptTurn.Count() - 1; i > 1; i--) {
    ...
    if (fabs(deltaYaw) > maxYaw) {
      m_scriptTurn[i - 1].flYaw = UTIL_ApproachAngle(
          m_scriptTurn[i - 1].flYaw, m_scriptTurn[i].flYaw, maxYaw);
    }
  }
...
int CAI_BlendedMotor::BuildTurnScript(int i, int j) {
  ...
  float totalTime =
      m_scriptTurn[j].flElapsedTime - m_scriptTurn[i].flElapsedTime;
  ...
  if (t1 < 0.01) {
    ...
    // we're already aiming close enough to the interior yaw, set the point
    // where we need to blend out
    ...
    // we'll finish up aiming close enough to the interior yaw, set the point
    // where we need to blend in
    ...
    m_scriptTurn[k].flYaw = UTIL_ApproachAngle(
        m_scriptTurn[j].flYaw, m_scriptTurn[i].flYaw, deltaYaw * (1 - a));
...
int CAI_BlendedMotor::BuildInsertNode(int i, float flTime) {
  ...
  for (i; i < m_scriptTurn.Count() - 1; i++) {
    if (m_scriptTurn[i].flTime < flTime) {
      ...
      script.flElapsedTime = m_scriptTurn[i].flElapsedTime * (1 - a) +
                             m_scriptTurn[i + 1].flElapsedTime * a;

      script.vecLocation = m_scriptTurn[i].vecLocation * (1 - a) +
                           m_scriptTurn[i + 1].vecLocation * a;
...
ConVar ai_path_insert_pause_at_obstruction(
    "ai_path_insert_pause_at_obstruction", "1");
ConVar ai_path_adjust_speed_on_immediate_turns(
...
void CAI_BlendedMotor::BuildVelocityScript(const AILocalMoveGoal_t &move) {
  ...
  if (npc_height_adjust.GetBool() && move.bHasTraced &&
      move.directTrace.flTotalDist != move.thinkTrace.flTotalDist) {
    float flDist = (move.directTrace.vEndPosition - m_scriptMove[0].vecLocation)
                       .Length2D();
    float flHeight =
        move.directTrace.vEndPosition.z - m_scriptMove[0].vecLocation.z;
    float flDelta;
    ...
    m_flPredictiveSpeedAdjust = std::clamp(
        m_flPredictiveSpeedAdjust, (flHeight > 0.0f) ? 0.5f : 0.8f, 1.0f);

    /*
    if ((GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
    {
            Msg("m_flPredictiveSpeedAdjust %.3f  %.1f %.1f\n",
    m_flPredictiveSpeedAdjust, flHeight, flDist ); NDebugOverlay::Box(
    move.directTrace.vEndPosition, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ),
    0,255,255, 0, 0.12 );
    }
    */
  }
  if (npc_height_adjust.GetBool()) {
    ...
    newSpeedAdjust =
        std::clamp(newSpeedAdjust, (flHeight > 0.0f) ? 0.5f : 0.8f, 1.0f);

    // debounce speed adjust
    if (newSpeedAdjust < m_flReactiveSpeedAdjust) {
      m_flReactiveSpeedAdjust =
          m_flReactiveSpeedAdjust * 0.2 + newSpeedAdjust * 0.8;
    } else {
      m_flReactiveSpeedAdjust =
          m_flReactiveSpeedAdjust * 0.5 + newSpeedAdjust * 0.5;
    }
    ...
    if ((GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
    {
            NDebugOverlay::Box( m_vecPrevOrigin2, Vector( -2, -2, -2 ), Vector(
    2, 2, 2 ), 255,0,255, 0, 0.12 ); NDebugOverlay::Box(
    move.thinkTrace.vEndPosition, Vector( -2, -2, -2 ), Vector( 2, 2, 2 ),
    255,0,255, 0, 0.12 ); Msg("m_flReactiveSpeedAdjust %.3f  %.1f %.1f\n",
  ...
  idealVelocity = idealVelocity *
                  std::min(m_flReactiveSpeedAdjust, m_flPredictiveSpeedAdjust);
  ...
  while (
      ...
      (pCurWaypoint->NavType() == NAV_GROUND ||
       pCurWaypoint->NavType() ==
           NAV_FLY) /*&& flTotalDist / idealVelocity < 3.0*/)  // limit
                                                               // lookahead to 3
                                                               // seconds
  {
    script.Init();
    ...
    if (pNext) {
      switch (pNext->NavType()) {
        ...
        case NAV_JUMP:
          ...
          // to get how far/fast the jump will be, but this is also stateless,
          // so it'd call it per frame. So far it's not clear that the moveprobe
          // doesn't also call this.....

          {
            ...
            Vector rawJumpVel = GetMoveProbe()->CalcJumpLaunchVelocity(
                script.vecLocation, pNext->vecLocation, gravity, &minJumpHeight,
                maxHorzVel, &vecApex);
        ...
        case NAV_CLIMB: {
          /*
          CAI_Node *pClimbNode =
          GetNavigator()->GetNetwork()->GetNode(pNext->iNodeID);
  ...
  for (i = 0; i < m_scriptMove.Count() - 1; i++) {
    flTotalDist += m_scriptMove[i].flDist =
        (m_scriptMove[i + 1].vecLocation - m_scriptMove[i].vecLocation)
            .Length2D();
  }
  ...
  if (!m_bDeceleratingToGoal && m_scriptMove.Count() && flTotalDist > 0) {
    float flNeededAccel = DeltaV(
        m_scriptMove[0].flMaxVelocity,
        m_scriptMove[m_scriptMove.Count() - 1].flMaxVelocity, flTotalDist);
  ...
  if (ai_path_insert_pause_at_obstruction.GetBool() &&
      move.directTrace.pObstruction) {
    float distToObstruction =
        (move.directTrace.vEndPosition - m_scriptMove[0].vecLocation)
            .Length2D();

    // HACK move obstruction out "stepsize" to account for it being based on
    // stand position and not a trace
    distToObstruction = distToObstruction + 16;

    InsertSlowdown(distToObstruction, idealAccel, false);
  }

  if (ai_path_insert_pause_at_est_end.GetBool() &&
      GetNavigator()->GetArrivalDistance() > 0.0) {
    InsertSlowdown(flTotalDist - GetNavigator()->GetArrivalDistance(),
                   idealAccel, true);
  }

  // calc initial velocity based on immediate direction changes
  if (ai_path_adjust_speed_on_immediate_turns.GetBool() &&
      m_scriptMove.Count() > 1) {
    /*
    if ((GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT))
    {
            Vector tmp = m_scriptMove[1].vecLocation -
    m_scriptMove[0].vecLocation; VectorNormalize( tmp ); NDebugOverlay::Line(
    ...
    ), m_scriptMove[1].vecLocation + Vector( 0, 0, 10 ), 255,0,0, true, 0.1 );

            tmp = GetCurVel();
            VectorNormalize( tmp );
            NDebugOverlay::Line( m_scriptMove[0].vecLocation + Vector( 0, 0, 10
    ), m_scriptMove[0].vecLocation + tmp * 32 + Vector( 0, 0, 10 ), 0,0,255,
    true, 0.1 );
    }
    */
  ...
  for (i = 0; i < m_scriptMove.Count() - 1; i++) {
    ...
    float dv =
        m_scriptMove[i + 1].flMaxVelocity - m_scriptMove[i].flMaxVelocity;

    if (dv > 0.0) {
      // find time, distance to accel to next max vel
      float t1 = dv / idealAccel;
      float d1 =
          m_scriptMove[i].flMaxVelocity * t1 + 0.5 * (idealAccel)*t1 * t1;
      ...
      if (d1 > m_scriptMove[i].flDist) {
        ...
        if (SolveQuadratic(0.5 * idealAccel, m_scriptMove[i].flMaxVelocity,
                           -m_scriptMove[i].flDist, r1, r2)) {
          m_scriptMove[i + 1].flMaxVelocity =
              m_scriptMove[i].flMaxVelocity + idealAccel * r1;
        }
      }
    }
  }
  ...
  for (i = m_scriptMove.Count() - 1; i > 0; i--) {
    ...
    float dv =
        m_scriptMove[i].flMaxVelocity - m_scriptMove[i - 1].flMaxVelocity;
  ...
  for (i = 0; i < m_scriptMove.Count(); i++)
  {
          NDebugOverlay::Text( m_scriptMove[i].vecLocation, (const char
  *)CFmtStr( "%.2f ", m_scriptMove[i].flMaxVelocity  ), false, 0.1 );
  ...
  */
  ...
  for (i = 0; i < m_scriptMove.Count() - 1;) {
    ...
    float d2 =
        m_scriptMove[i + 1].flMaxVelocity * t2 + 0.5 * (idealAccel)*t2 * t2;

    m_scriptMove[i].flDist =
        (m_scriptMove[i + 1].vecLocation - m_scriptMove[i].vecLocation)
            .Length2D();
    ...
    if (d1 + d2 < m_scriptMove[i].flDist) {
      ...
      // check to see if the amount of change needed to reach target is less
      // than the ideal acceleration
      float flNeededAccel = fabs(DeltaV(m_scriptMove[i].flMaxVelocity,
                                        m_scriptMove[i + 1].flMaxVelocity,
                                        m_scriptMove[i].flDist));
      if (flNeededAccel < idealAccel) {
        ...
        // if this assert hits, write down the v1, v2, dist, and idealAccel
        // numbers and send them to me (Ken). go ahead the comment it out, it's
        // safe, but I'd like to know a test case where it's happening
        // Assert( t1 > 0 && t2 > 0 );
        ...
        if (t1 > 0.0 && t2 > 0.0) {
          ...
          */
          ...
          script.Init();
          script.vecLocation = m_scriptMove[i + 1].vecLocation * a +
                               m_scriptMove[i].vecLocation * (1 - a);
          script.flMaxVelocity =
              m_scriptMove[i].flMaxVelocity + idealAccel * t1;

          if (script.flMaxVelocity < idealVelocity) {
            // DevMsg("insert %.2f %.2f %.2f\n", m_scriptMove[i].flMaxVelocity,
            // script.flMaxVelocity, m_scriptMove[i+1].flMaxVelocity );
            m_scriptMove.InsertAfter(i, script);
            i += 1;
          }
        }
  ...
  for (i = 0; i < m_scriptMove.Count(); i++) {
    m_scriptMove[i].flMaxVelocity =
        std::max(m_scriptMove[i].flMaxVelocity, MIN_VELOCITY);
  }
  ...
  for (i = 0; i < m_scriptMove.Count() - 1;) {
    ...
    if (m_scriptMove[i].flMaxVelocity == 0 &&
        m_scriptMove[i + 1].flMaxVelocity == 0) {
    ...
    float t =
        m_scriptMove[i].flDist / (0.5 * (m_scriptMove[i].flMaxVelocity +
                                         m_scriptMove[i + 1].flMaxVelocity));
    ...
    */
    ...
    m_scriptMove[i + 1].flElapsedTime =
        m_scriptMove[i].flElapsedTime + m_scriptMove[i].flTime;
  ...
  for (i = 0; i < m_scriptMove.Count(); i++)
  {
          DevMsg("(%.2f : %.2f : %.2f)", m_scriptMove[i].flMaxVelocity,
  m_scriptMove[i].flDist, m_scriptMove[i].flTime );
          // DevMsg("(%.2f:%.2f)", m_scriptMove[i].flTime,
  m_scriptMove[i].flElapsedTime );
  }
  DevMsg("\n");
  */
}

void CAI_BlendedMotor::InsertSlowdown(float distToObstruction, float idealAccel,
                                      bool bAlwaysSlowdown) {
  int i;
  AI_Movementscript_t script;

  if (distToObstruction <= 0.0) return;

  for (i = 0; i < m_scriptMove.Count() - 1; i++) {
    if (m_scriptMove[i].flDist > 0 &&
        distToObstruction - m_scriptMove[i].flDist < 0) {
      float a = distToObstruction / m_scriptMove[i].flDist;
      Assert(a >= 0 && a <= 1);
      script.vecLocation = (1 - a) * m_scriptMove[i].vecLocation +
                           a * m_scriptMove[i + 1].vecLocation;

      // NDebugOverlay::Line( m_scriptMove[i].vecLocation + Vector( 0, 0, 5 ),
      // script.vecLocation + Vector( 0, 0, 5 ), 0,255,0, true, 0.1 );
      // NDebugOverlay::Line( script.vecLocation + Vector( 0, 0, 5 ),
      // m_scriptMove[i+1].vecLocation + Vector( 0, 0, 5 ), 0,0,255, true, 0.1
      // );
      ...
      // clamp the next velocity to the possible accel in the given distance
      if (!bAlwaysSlowdown &&
          SolveQuadratic(-0.5 * idealAccel, m_scriptMove[0].flMaxVelocity,
                         -distToObstruction, r1, r2)) {
        script.flMaxVelocity =
            std::max(10.0f, m_scriptMove[0].flMaxVelocity - idealAccel * r1);
      } else {
...
//-----------------------------------------------------------------------------
// Purpose: issues turn gestures when it detects that the body has turned but
// the feet haven't compensated
//-----------------------------------------------------------------------------
...
void CAI_BlendedMotor::MaintainTurnActivity(void) {
  ...
  if (m_flNextTurnGesture > gpGlobals->curtime ||
      m_flNextTurnAct > gpGlobals->curtime || GetOuter()->IsMoving()) {
  ...
  if (m_doTurn > 15.0f) {
    ...
    if (iSeq != ACT_INVALID) {
      ...
      if (iLayer != -1) {
        ...
        m_flNextTurnAct =
            gpGlobals->curtime + GetOuter()->GetLayerDuration(iLayer);
      } else {
...
bool CAI_BlendedMotor::AddTurnGesture(float flYD) {
  ...
  if (scene_flatturn.GetBool() &&
      GetOuter()->IsCurSchedule(SCHED_SCENE_GENERIC)) {
  ...
  if (seq != ACTIVITY_NOT_AVAILABLE) {
    ...
    if (iLayer != -1) {
      ...
      speed = std::clamp(speed, 15.0f, 35.0f);
      ...
      // don't overlap the turn portion of the gestures, and don't play them too
      // often
      m_flNextTurnGesture =
          gpGlobals->curtime + std::max(turnCompletion * actualDuration, 0.3f);

      /*
      if ( GetOuter()->m_debugOverlays & OVERLAY_NPC_SELECTED_BIT )
      {
              Msg( "%.1f : %.2f %.2f : %.2f (%.2f)\n", flYD, weight, speed,
      actualDuration, turnCompletion * actualDuration );
      }
      */
      return true;
    } else {
...
//-------------------------------------
// Purpose:	return a velocity that should be hit at the end of the interval
// to match goal Input  : flInterval - time interval to consider
