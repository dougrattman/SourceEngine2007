...
DEFINE_FIELD(timeSpoken, FIELD_TIME),  // Relative to server time
                                       // DEFINE_EMBEDDED( response,
                                       // FIELD_??? ),	// This is manually
                                       // saved/restored by the ConceptHistory
                                       // saverestore ops below
    END_DATADESC()
 ...
 public:
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    CUtlDict<ConceptHistory_t, int> *ch =
        ((CUtlDict<ConceptHistory_t, int> *)fieldInfo.pField);
    int count = ch->Count();
...
DEFINE_CUSTOM_FIELD(m_ConceptHistories, &g_ConceptHistoriesSaveDataOps),
  ...
  if (!GetSink()->UseSemaphore()) return true;

  CAI_TimedSemaphore *pSemaphore =
      GetMySpeechSemaphore(pTalker->MyNPCPointer());
...
CAI_Expresser::~CAI_Expresser() {
  ...
  if (pSemaphore) {
    if (pSemaphore->GetOwner() == GetOuter()) pSemaphore->Release();
...
static const int LEN_SPECIFIC_SCENE_MODIFIER =
    strlen(AI_SPECIFIC_SCENE_MODIFIER);
...
// Output : AI_Response
//-----------------------------------------------------------------------------
AI_Response *CAI_Expresser::SpeakFindResponse(
  ...
  // Append local player criteria to set, but not if this is a player doing the
  // talking
  ...
  if (result->GetOdds() < 100 &&
      random->RandomInt(1, 100) <= result->GetOdds()) {
    delete result;
    return NULL;
  }
...
// Input  : *response -
//-----------------------------------------------------------------------------
bool CAI_Expresser::SpeakDispatchResponse(
    AIConcept_t concept, AI_Response *result,
  ...
  if (IsSpeaking() && concept[0] != 0) {
    DevMsg(
        "SpeakDispatchResponse:  Entity ( %i/%s ) already speaking, forcing "
        "'%s'\n",
        GetOuter()->entindex(), STRING(GetOuter()->GetEntityName()), concept);

    // Tracker 15911:  Can break the game if we stop an imported map placed lcs
    // here, so only
    ...
    if (IsRunningScriptedScene(GetOuter())) {
      DevMsg(
          "SpeakDispatchResponse:  Entity ( %i/%s ) refusing to speak due to "
          "scene entity, tossing '%s'\n",
          GetOuter()->entindex(), STRING(GetOuter()->GetEntityName()), concept);
      delete result;
      return false;
    }
  ...
  switch (result->GetType()) {
    ...
    case RESPONSE_SPEAK: {
      if (!result->ShouldntUseScene()) {
        ...
        DevMsg("SpeakDispatchResponse:  Entity ( %i/%s ) playing sound '%s'\n",
               GetOuter()->entindex(), STRING(GetOuter()->GetEntityName()),
               response);
    ...
    case RESPONSE_PRINT: {
      if (g_pDeveloper->GetInt() > 0) {
        Vector vPrintPos;
        GetOuter()->CollisionProp()->NormalizedToWorldSpace(
            Vector(0.5, 0.5, 1.0f), &vPrintPos);
  ...
  if (spoke) {
    ...
    if (DebuggingSpeech() && g_pDeveloper->GetInt() > 0 && response &&
        result->GetType() != RESPONSE_PRINT) {
      Vector vPrintPos;
      GetOuter()->CollisionProp()->NormalizedToWorldSpace(
          Vector(0.5, 0.5, 1.0f), &vPrintPos);
      NDebugOverlay::Text(vPrintPos, CFmtStr("%s: %s", concept, response), true,
                          1.5);
    }
...
bool CAI_Expresser::Speak(AIConcept_t concept, const char *modifiers /*= NULL*/,
  ...
  SpeechMsg(GetOuter(), "%s (%x) spoke %s (%f)\n",
            STRING(GetOuter()->GetEntityName()), GetOuter(), concept,
            gpGlobals->curtime);
...
bool CAI_Expresser::SpeakRawScene(const char *pszScene, float delay,
  ...
  if (sceneLength > 0) {
    ...
    GetOuter()->GenderExpandString(pszScene, szInstanceFilename,
                                   sizeof(szInstanceFilename));
...
bool CAI_Expresser::SpeakAutoGeneratedScene(char const *soundname,
                                            float delay) {
...
int CAI_Expresser::SpeakRawSentence(const char *pszSentence, float delay,
  ...
  if (pszSentence[0] == AI_SP_SPECIFIC_SENTENCE) {
    ...
    CBaseEntity::EmitSentenceByIndex(filter, GetOuter()->entindex(), CHAN_VOICE,
                                     sentenceIndex, volume, soundlevel, 0,
                                     GetVoicePitch());
  } else {
    sentenceIndex =
        SENTENCEG_PlayRndSz(GetOuter()->NetworkProp()->edict(), pszSentence,
                            volume, soundlevel, 0, GetVoicePitch());
  }

  SpeechMsg(GetOuter(), "SpeakRawSentence( %s, %f) %f\n", pszSentence, delay,
            engine->SentenceLength(sentenceIndex));
...
//			The base CanSpeak() includes the default speech delay,
//and  won't 			return true until that delay time has passed
// after finishing the
//			speech. This returns true as soon as the speech
// finishes.
//-----------------------------------------------------------------------------
...
bool CAI_Expresser::CanSpeakConcept(AIConcept_t concept) {
  ...
  if (respeakDelay != 0.0f) {
    if (history->timeSpoken != -1 &&
        (gpGlobals->curtime < history->timeSpoken + respeakDelay))
      return false;
  }
...
float CAI_Expresser::GetTimeSpokeConcept(AIConcept_t concept) {
  int iter = m_ConceptHistories.Find(concept);
  if (iter == m_ConceptHistories.InvalidIndex()) return -1;
...
void CAI_Expresser::DumpHistories() {
  int c = 1;
  for (int i = m_ConceptHistories.First();
       i != m_ConceptHistories.InvalidIndex(); i = m_ConceptHistories.Next(i)) {
    ConceptHistory_t *h = &m_ConceptHistories[i];

    DevMsg("%i: %s at %f\n", c++, m_ConceptHistories.GetElementName(i),
           h->timeSpoken);
  }
}
...
bool CAI_Expresser::IsValidResponse(ResponseType_t type, const char *pszValue) {
  if (type == RESPONSE_SCENE) {
    char szInstanceFilename[256];
    GetOuter()->GenderExpandString(pszValue, szInstanceFilename,
                                   sizeof(szInstanceFilename));
...
CAI_TimedSemaphore *CAI_Expresser::GetMySpeechSemaphore(CBaseEntity *pNpc) {
  if (!pNpc->MyNPCPointer()) return false;

  return (pNpc->MyNPCPointer()->IsPlayerAlly() ? &g_AIFriendliesTalkSemaphore
...
void CAI_ExpresserHost_NPC_DoModifyOrAppendCriteria(CAI_BaseNPC *pSpeaker,
                                                    AI_CriteriaSet &set) {
  // Append current activity name
  const char *pActivityName =
      pSpeaker->GetActivityName(pSpeaker->GetActivity());
  if (pActivityName) {
  ...
  if (pSpeaker->GetEnemy()) {
    ...
    if (pSpeaker->GetLastEnemyTime() == 0.0)
      ...
      set.AppendCriteria("timesincecombat",
                         UTIL_VarArgs("%f", gpGlobals->curtime -
                                                pSpeaker->GetLastEnemyTime()));
  }

  set.AppendCriteria(
      "speed", UTIL_VarArgs("%.3f", pSpeaker->GetSmoothedVelocity().Length()));
  ...
  if (pPlayer) {
    ...
    set.AppendCriteria("distancetoplayer",
                       UTIL_VarArgs("%f", distance.Length()));
  ...
  if (pPlayer && pPlayer->FInViewCone(pSpeaker) &&
      pPlayer->FVisible(pSpeaker)) {
