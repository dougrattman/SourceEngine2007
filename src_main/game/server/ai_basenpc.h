...
#define bits_MEMORY_SUSPICIOUS \
  (1 << 2)  // Ally is suspicious of the player, and will move to provoked more
            // easily
#define bits_MEMORY_TASK_EXPENSIVE \
  (1 << 3)  // NPC has completed a task which is considered costly, so don't do
            // another task this frame
...
enum DesiredWeaponState_t {
  ...
  DESIREDWEAPONSTATE_HOLSTERED_DESTROYED,  // Put the weapon away, then destroy
                                           // it.
...
struct ScriptedNPCInteraction_t {
  ...
  string_t iszTheirWeapon;  // Classname of the weapon my interaction partner is
                            // holding, if any
  ScriptedNPCInteraction_Phases_t sPhases[SNPCINT_NUM_PHASES];

  // These will be filled out for you in AddScriptedNPCInteraction
  VMatrix matDesiredLocalToWorld;  // Desired relative position / angles of the
                                   // other NPC
...
class CAI_BaseNPC : public CBaseCombatCharacter, public CAI_DefMovementSink {
 ...
 public:
  ...
  // Notification that the current schedule, if any, is ending and a new one is
  // being selected
  ...
  // After the schedule has been selected, it will be processed by this function
  // so child NPC classes can remap base schedules into child-specific behaviors
 ...
 private:
  // This function maps the type through TranslateSchedule() and then retrieves
  // the pointer to the actual CAI_Schedule from the database of schedules
 ...
 private:
  ...
  CAI_ScheduleBits
      m_CustomInterruptConditions;  // Bit string assembled by the schedule
                                    // running, then modified by leaf classes to
                                    // suit their needs
 ...
 public:
  ...
  void ResetActivity() { m_Activity = ACT_RESET; }
 ...
 private:
  void *CheckEnemy(CBaseEntity *pEnemy) {
    return NULL;
  }  // OBSOLETE, replaced by GatherEnemyConditions(), left here to make derived
     // code not compile
 ...
 public:
  ...
  virtual void AddFacingTarget(const Vector &vecPosition, float flImportance,
                               float flDuration, float flRamp = 0.0);
  virtual void AddFacingTarget(CBaseEntity *pTarget, const Vector &vecPosition,
                               float flImportance, float flDuration,
                               float flRamp = 0.0);
  ...
  virtual bool OverrideMove(
      float flInterval);  // Override to take total control of movement (return
                          // true if done so)
  ...
  virtual bool OnObstructingDoor(AILocalMoveGoal_t *pMoveGoal, CBaseDoor *pDoor,
                                 float distClear, AIMoveResult_t *pResult);
  ...
  CHandle<CBasePropDoor>
      m_hOpeningDoor;  // The CBasePropDoor that we are in the midst of opening
                       // for navigation.
 ...
 public:
  ...
  enum SCRIPTSTATE {
    ...
    SCRIPT_WAIT,  // Waiting on everyone in the script to be ready. Plays the
                  // pre idle animation if there is one.
    ...
    SCRIPT_CUSTOM_MOVE_TO_MARK,  // Moving to the scripted sequence position
                                 // while playing a custom movement animation.
  };
  ...
  static void ForceSelectedGoRandom();
 ...
 protected:
  ...
  float
      m_flLastPlayerDamageTime;  // Last time I received damage from the player
 ...
 public:
  ...
  virtual bool FindCoverPosInRadius(CBaseEntity *pEntity, const Vector &goalPos,
                                    float coverRadius, Vector *pResult);
 ...
 public:
  ...
  virtual void AddEntityRelationship(CBaseEntity *pEntity,
                                     Disposition_t nDisposition, int nPriority);
  ...
  float ThrowLimit(const Vector &vecStart, const Vector &vecEnd, float fGravity,
                   float fArcSize, const Vector &mins, const Vector &maxs,
                   CBaseEntity *pTarget, Vector *jumpVel,
                   CBaseEntity **pBlocker);

  // these functions will survey conditions and set appropriate conditions bits
  // for attack types.
  ...
  // origin is at chasePosition by default, leave this alone to make your origin
  // coincide with his.
  ...
  virtual bool PlayerInSpread(const Vector &sourcePos, const Vector &targetPos,
  ...
  CBaseEntity *DropItem(char *pszItemName, Vector vecPos,
  ...
  //---------------------------------
  // Utility methods
  static Vector CalcThrowVelocity(const Vector &startPos, const Vector &endPos,
  ...
  float m_flNextDodgeTime;  // Time at which I can dodge again. Used so that the
                            // behavior doesn't happen over and over.
 ...
 public:
  int m_cAmmoLoaded;  // how much ammo is in the weapon (used to trigger reload
                      // anim sequences)
  float m_flDistTooFar;  // if enemy farther away than this,
 ...
 private:
  bool m_bDeferredNavigation;  // This NPCs has a navigation query that's being
                               // deferred until later in the frame
 ...
 private:
  ...
  static bool LoadDefaultSchedules();

  static void InitDefaultScheduleSR();
  static void InitDefaultTaskSR();
  static void InitDefaultConditionSR();
  static void InitDefaultActivitySR();
  static void InitDefaultSquadSlotSR();
...
inline bool ValidateConditionLimits(const char *pszNewCondition) {
  ...
  if (nGlobalConditions >= MAX_CONDITIONS) {
    AssertMsg2(
        0, "Exceeded max number of conditions (%d), ignoring condition %s\n",
...
#define DEFINE_CUSTOM_SCHEDULE_PROVIDER                                 \
  ...
  virtual const char *GetSchedulingErrorName() {                        \
    return gm_pszErrorClassName;                                        \
                                                                        ...
                                                                        \
  static void InitCustomSchedules();                                    \
                                                                        \
  static bool LoadSchedules();                                          \
  virtual bool LoadedSchedules();                                       \
                                                                        \
