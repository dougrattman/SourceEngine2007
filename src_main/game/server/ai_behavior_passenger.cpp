...
ConVar passenger_impact_response_threshold(
    "passenger_impact_response_threshold", "-350.0");
...
BEGIN_DATADESC(CAI_PassengerBehavior)
  ...
  DEFINE_EMBEDDED(m_vehicleState), DEFINE_FIELD(m_bEnabled, FIELD_BOOLEAN),
...
void CAI_PassengerBehavior::Enable(CPropJeepEpisodic *pVehicle,
                                   bool bImmediateEnter /*= false*/) {
  if (m_bEnabled && m_hVehicle.Get()) return;
...
void CAI_PassengerBehavior::EnterVehicle(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_INSIDE ||
      GetPassengerState() == PASSENGER_STATE_ENTERING) {
    // Clear out an exit
...
void CAI_PassengerBehavior::ExitVehicle(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_OUTSIDE ||
      GetPassengerState() == PASSENGER_STATE_EXITING) {
  ...
  //
  // Everything below this point will still attempt to exit the vehicle, once
  // able
  //
...
bool CAI_PassengerBehavior::IsPassengerHostile(void) {
  ...
  if (GetOuter()->IRelationType(pPlayer) == D_HT ||
      GetOuter()->IRelationType(pPlayer) == D_FR)
    return true;
...
void CAI_PassengerBehavior::InitVehicleState(void) {
  ...
  m_vehicleState.m_bPlayerInVehicle =
      (pPlayer && pPlayer->IsInAVehicle() &&
       pPlayer->GetServerVehicle() == m_hVehicle->GetServerVehicle());

  // Update our vehicle state so we don't confuse our previous velocity on the
  // first frame!
...
void CAI_PassengerBehavior::FinishEnterVehicle(void) {
  ...
  m_hVehicle->NPC_FinishedEnterVehicle(GetOuter(),
                                       (IsPassengerHostile() == false));
}
...
void CAI_PassengerBehavior::FinishExitVehicle(void) {
  ...
  m_hVehicle->NPC_FinishedExitVehicle(GetOuter(),
                                      (IsPassengerHostile() == false));
...
void CAI_PassengerBehavior::BuildScheduleTestBits(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_OUTSIDE ||
      GetPassengerState() == PASSENGER_STATE_INSIDE) {
...
bool CAI_PassengerBehavior::CanExitVehicle(void) {
  ...
  if (vecVelocity.LengthSqr() > Square(8.0f)) return false;
...
int CAI_PassengerBehavior::SelectTransitionSchedule(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_INSIDE) {
    ...
    if (HasCondition(COND_PASSENGER_EXITING) ||
      ...
      if (CanExitVehicle()) {
        ...
        return SCHED_PASSENGER_EXIT_VEHICLE;
             ...
             GetPassengerState() == PASSENGER_STATE_EXITING) {
    // The following code attempts to fix up a passenger being interrupted
    // mid-transition
    Warning("SelectSchedule() called on transitioning passenger!\n");
    ADD_DEBUG_HISTORY(
        HISTORY_AI_DECISIONS,
        UTIL_VarArgs(
            "%s(%d):  SelectSchedule() called on transitioning passenger!\n",
    ...
    if (GetPassengerState() == PASSENGER_STATE_EXITING) {
      ...
      // The teleport function is overridden for passengers, meaning that they
      // will clean up properly when called to do so
...
int CAI_PassengerBehavior::SelectSchedule(void) {
  ...
  if (nSchedule != SCHED_NONE) return nSchedule;
...
// Purpose:
//-----------------------------------------------------------------------------
int CAI_PassengerBehavior::SelectFailSchedule(
...
//-----------------------------------------------------------------------------
// Purpose: Finds a ground position at a given location with some delta up and
// down to check Input  : &in - position to check at
//			delta - amount of distance up and down to check
//			*out - ground position
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::FindGroundAtPosition(const Vector &in,
                                                 float flUpDelta,
                                                 float flDownDelta,
                                                 Vector *out) {
  ...
  CTraceFilterVehicleTransition ignoreFilter(m_hVehicle, GetOuter(),
                                             COLLISION_GROUP_NONE);

  trace_t tr;
  UTIL_TraceHull(startPos, endPos, hullMin, hullMax, MASK_NPCSOLID,
                 &ignoreFilter, &tr);
  ...
  if (tr.allsolid) {
    ...
    if (passenger_debug_transition.GetBool()) {
      NDebugOverlay::SweptBox(tr.startpos, tr.endpos, hullMin, hullMax,
                              vec3_angle, 255, 255, 0, 255, 1.0f);
    }
...
bool CAI_PassengerBehavior::PointIsNavigable(const Vector &vecTargetPos) {
  // Attempt to local move between this point and the nearest node, ignoring
  // anything but world architecture
  AIMoveTrace_t moveTrace;
  int iNearestNode =
      GetOuter()->GetPathfinder()->NearestNodeToPoint(vecTargetPos);
  if (iNearestNode != NO_NODE) {
    ...
    GetOuter()->GetMoveProbe()->MoveLimit(
        ...
        vecTargetPos, MASK_SOLID_BRUSHONLY, NULL, 0, &moveTrace);

    // See if we got close enough to call it arrived
    if ((moveTrace.vEndPosition - vecTargetPos).LengthSqr() <
            Square(GetHullWidth()) &&
        GetOuter()->GetMoveProbe()->CheckStandPosition(moveTrace.vEndPosition,
                                                       MASK_SOLID_BRUSHONLY)) {
      // NDebugOverlay::HorzArrow( vecTargetPos, moveTrace.vEndPosition, 8.0f,
      // 255, 0, 0, 16, true, 4.0f ); NDebugOverlay::HorzArrow( vecTargetPos,
      // g_pBigAINet->GetNodePosition(GetOuter()->GetHullType(), iNearestNode
      // ), 8.0f, 255, 255, 0, 16, true, 4.0f );
      return true;
    }
  }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::GetExitPoint(int nSequence, Vector *vecExitPoint,
                                         QAngle *vecExitAngles) {
  bool bSucceeded = true;

  // Get the delta to the final position as will be dictated by this animation's
  // auto movement
  Vector vecDeltaPos;
  QAngle vecDeltaAngles;
  GetOuter()->GetSequenceMovement(nSequence, 0.0f, 1.0f, vecDeltaPos,
                                  vecDeltaAngles);
  ...
  if (vecExitPoint != NULL) {
    ...
    bool bFoundGround =
        FindGroundAtPosition(GetOuter()->GetAbsOrigin() + vecDeltaPos,
                             flUpDelta, flDownDelta, &vecGroundPos);
    if (bFoundGround) {
...
//-----------------------------------------------------------------------------
// Purpose: Determines whether the NPC can move between a start and end position
// of a transition Input  : &vecStartPos - start position
//			&vecEndPos - end position
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::IsValidTransitionPoint(const Vector &vecStartPos,
                                                   const Vector &vecEndPos) {
  // Now sweep a hull through space to see if we can validly exit there
  Vector vecHullMins =
      GetOuter()->GetHullMins() + Vector(0, 0, GetOuter()->StepHeight() * 2.0f);
  Vector vecHullMaxs =
      GetOuter()->GetHullMaxs() - Vector(0, 0, GetOuter()->StepHeight());

  trace_t tr;
  CTraceFilterVehicleTransition skipFilter(GetOuter(), m_hVehicle,
                                           COLLISION_GROUP_NONE);
  UTIL_TraceHull(vecStartPos, vecEndPos, vecHullMins, vecHullMaxs,
                 MASK_NPCSOLID, &skipFilter, &tr);

  // If we're blocked, we can't get out there
  if (tr.fraction < 1.0f || tr.allsolid) {
    if (passenger_debug_transition.GetBool()) {
      NDebugOverlay::SweptBox(vecStartPos, vecEndPos, vecHullMins, vecHullMaxs,
                              vec3_angle, 255, 0, 0, 64, 2.0f);
    }
    return false;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Find the proper sequence to use (weighted by priority or distance
// from current position)
//			to enter the vehicle.
// Input  : bNearest - Use distance as the criteria for a "best" sequence.
...
int CAI_PassengerBehavior::FindEntrySequence(bool bNearest /*= false*/) {
  ...
  const PassengerSeatAnims_t *pEntryAnims =
      m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatAnims(
          GetOuter(), PASSENGER_SEAT_ENTRY);
  if (pEntryAnims == NULL) return -1;

  // Get the ultimate position we'll end up at
  Vector vecStartPos, vecEndPos;
  if (m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatPosition(
          GetOuter(), &vecEndPos, NULL) == false)
    return -1;
  ...
  for (int i = 0; i < pEntryAnims->Count(); i++) {
    ...
    nSequence =
        GetOuter()->LookupSequence(STRING(pTransition->GetAnimationName()));
    if (nSequence == -1) continue;
...
int CAI_PassengerBehavior::FindExitSequence(void) {
  // Get a list of all our animations
  const PassengerSeatAnims_t *pExitAnims =
      m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatAnims(
          GetOuter(), PASSENGER_SEAT_EXIT);
  if (pExitAnims == NULL) return -1;

  // Get the ultimate position we'll end up at
  Vector vecStartPos, vecEndPos;
  if (m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatPosition(
          GetOuter(), &vecStartPos, NULL) == false)
    return -1;

  // Test each animation (sorted by priority) for the best match
  for (int i = 0; i < pExitAnims->Count(); i++) {
    // Find the activity for this animation name
    int nSequence = GetOuter()->LookupSequence(
        STRING(pExitAnims->Element(i).GetAnimationName()));
    if (nSequence == -1) continue;
...
bool CAI_PassengerBehavior::ReserveExitPoint(void) {
  ...
  Vector hullMax = GetOuter()->GetHullMaxs();
  m_hBlocker =
      CEntityBlocker::Create(vecGroundPos, hullMin, hullMax, GetOuter(), true);
  ...
  if (passenger_debug_transition.GetBool()) {
    ...
    NDebugOverlay::HorzArrow(m_vecTargetPosition, vecArrowEnd, 8.0f, 255, 255,
                             0, 64, true, 4.0f);
  }
...
//			the vehicle.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::GetEntryPoint(int nSequence, Vector *vecEntryPoint,
  ...
  Vector vecDeltaPos;
  QAngle vecDeltaAngles;
  GetOuter()->GetSequenceMovement(nSequence, 1.0f, 0.0f, vecDeltaPos,
                                  vecDeltaAngles);
  ...
  if (vecEntryPoint != NULL) {
    ...
    Vector vecGroundPos;
    if (FindGroundAtPosition((*vecEntryPoint), 16.0f, 64.0f, &vecGroundPos) ==
        false) {
      // We failed
      if (passenger_debug_transition.GetBool()) {
        NDebugOverlay::SweptBox(
            (*vecEntryPoint), vecGroundPos, GetOuter()->GetHullMins(),
            GetOuter()->GetHullMaxs(), vec3_angle, 255, 0, 0, 64, 2.0f);
      }
  ...
  if (vecEntryAngles != NULL) {
    ...
    *vecEntryAngles = vecTargetAngles;
    ...
    ConcatTransforms(m_hVehicle->EntityToWorldTransform(), angToParent,
                     angToWorld);
  ...
  if (passenger_debug_transition.GetBool() && vecEntryPoint && vecEntryAngles) {
    ...
    if (vecEntryAngles != NULL) {
      ...
      NDebugOverlay::HorzArrow((*vecEntryPoint), vecArrowEnd, 8.0f, 0, 255, 0,
                               64, true, 4.0f);
    }
  }
...
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::GetSequenceBlendAmount(float flCycle,
                                                   float *posBlend,
                                                   float *angBlend) {
  // Find positional blend, if requested
  if (posBlend != NULL) {
    float flFrac = RemapValClamped(flCycle, m_flOriginStartFrame,
                                   m_flOriginEndFrame, 0.0f, 1.0f);
    (*posBlend) = SimpleSpline(flFrac);
  }

  // Find angular blend, if requested
  if (angBlend != NULL) {
    float flFrac = RemapValClamped(flCycle, m_flAnglesStartFrame,
                                   m_flAnglesEndFrame, 0.0f, 1.0f);
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::GetEntryTarget(Vector *vecOrigin,
                                           QAngle *vecAngles) {
  // Get the ultimate position we'll end up at
  m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatPositionLocal(
      GetOuter(), vecOrigin, vecAngles);
}
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::GetTransitionAnimationIdeal(
    float flCycle, const Vector &vecTargetPos, const QAngle &vecTargetAngles,
    Vector *idealOrigin, QAngle *idealAngles) {
  // Get the position in time working backwards from our goal
  Vector vecDeltaPos;
  QAngle vecDeltaAngles;
  GetOuter()->GetSequenceMovement(GetSequence(), 1.0f, flCycle, vecDeltaPos,
                                  vecDeltaAngles);
...
bool CAI_PassengerBehavior::LocalIntervalMovement(float flInterval,
  ...
  if (pstudiohdr == NULL) return false;
  ...
  if ((GetOuter()->SequenceLoops() == false) && flNextCycle > 1.0f) {
    flInterval = GetOuter()->GetCycle() /
                 (GetOuter()->GetSequenceCycleRate(GetSequence()) *
  ...
  if (Studio_SeqMovement(pstudiohdr, GetOuter()->GetSequence(),
                         GetOuter()->GetCycle(), flNextCycle,
                         GetOuter()->GetPoseParameterArray(), deltaPos,
                         deltaAngles)) {
...
//-----------------------------------------------------------------------------
float CAI_PassengerBehavior::GetNextCycleForInterval(int nSequence,
                                                     float flInterval) {
  return GetOuter()->GetCycle() +
         flInterval * GetOuter()->GetSequenceCycleRate(GetSequence()) *
             GetOuter()->GetPlaybackRate();
}
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::DrawDebugTransitionInfo(
    const Vector &vecIdealPos, const QAngle &vecIdealAngles,
    const Vector &vecAnimPos, const QAngle &vecAnimAngles) {
  // Debug info
...
bool CAI_PassengerBehavior::DoTransitionMovement(void) {
  ...
  if (LocalIntervalMovement(flInterval, bSequenceFinished, vecAnimPos,
                            vecAnimAngles)) {
    ...
    QAngle vecDeltaAngles;
    vecDeltaAngles.x =
        AngleDiff(vecIdealAngles.x, vecAnimAngles.x) * flAngBlend;
    vecDeltaAngles.y =
        AngleDiff(vecIdealAngles.y, vecAnimAngles.y) * flAngBlend;
    vecDeltaAngles.z =
        AngleDiff(vecIdealAngles.z, vecAnimAngles.z) * flAngBlend;
    ...
    if (passenger_debug_transition.GetBool()) {
      DrawDebugTransitionInfo(vecIdealPos, vecIdealAngles, vecAnimPos,
                              vecAnimAngles);
    }
...
//-----------------------------------------------------------------------------
// Purpose: Gather conditions we can comment on or react to while riding in the
// vehicle
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::GatherVehicleStateConditions(void) {
  ...
  if (pPlayer) {
    if (pPlayer->IsInAVehicle() &&
        pPlayer->GetVehicle() == m_hVehicle->GetServerVehicle()) {
  ...
  Vector deltaVelocity =
      (localVelocity - m_vehicleState.m_vecLastLocalVelocity);
  ...
  if (deltaVelocity.y < passenger_impact_response_threshold.GetFloat()) {
    ...
    // The X axis represents lateral movement and the Z axis represents vertical
    // movement{
  ...
  if (m_vehicleState.m_flLastSpeedSqr > STOPPED_VELOCITY_THRESHOLD_SQR &&
      flSpeedSqr < STOPPED_VELOCITY_THRESHOLD_SQR) {
    SetCondition(COND_PASSENGER_VEHICLE_STOPPED);
  } else if (m_vehicleState.m_flLastSpeedSqr < STARTED_VELOCITY_THRESHOLD_SQR &&
             flSpeedSqr > STARTED_VELOCITY_THRESHOLD_SQR) {
  ...
  m_vehicleState.m_vecDeltaVelocity =
      (localVelocity - m_vehicleState.m_vecLastLocalVelocity);
  ...
  m_vehicleState.m_vecLastAngles =
      (m_vehicleState.m_vecLastAngles * 0.2f) + (angVel * 0.8f);
}
...
void CAI_PassengerBehavior::PrescheduleThink(void) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_OUTSIDE &&
      HasCondition(COND_PASSENGER_CANCEL_ENTER)) {
  ...
  if (GetPassengerState() == PASSENGER_STATE_INSIDE) {
    ...
    if (m_hVehicle &&
        m_hVehicle->GetServerVehicle()->NPC_GetPassengerSeatPositionLocal(
            GetOuter(), &vecSeatOrigin, &vecSeatAngles)) {
      ...
      Warning(
          "Passenger is in vehicle without a valid seat position! -- "
          "EJECTED\n");
      GetOuter()->SetParent(NULL);
      Disable();
...
void CAI_PassengerBehavior::GatherConditions(void) {
  if (IsEnabled() == false) return BaseClass::GatherConditions();
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::ModifyOrAppendCriteria(
  ...
  criteriaSet.AppendCriteria("vehicle_speed",
                             UTIL_VarArgs("%f", flVehicleSpeed));

  // Whether or not the passenger is currently able to enter the vehicle (only
  // accounts for locking really)
  ...
  // Whether or not the passenger is currently able to exit the vehicle (only
  // accounts for locking really)
...
void CAI_PassengerBehavior::CacheBlendTargets(void) {
  ...
  KeyValues *seqValues =
      GetOuter()->GetSequenceKeyValues(m_nTransitionSequence);
  if (seqValues == NULL) {
    Assert(0);
    return;
  }
  ...
  int nMaxFrames =
      Studio_MaxFrame(GetOuter()->GetModelPtr(), m_nTransitionSequence,
                      GetOuter()->GetPoseParameterArray());
  ...
  if (subKeys) {
    ...
    m_flOriginStartFrame =
        RemapValClamped(m_flOriginStartFrame, 0, nMaxFrames, 0.0f, 1.0f);
    m_flOriginEndFrame =
        RemapValClamped(m_flOriginEndFrame, 0, nMaxFrames, 0.0f, 1.0f);
  }
  ...
  if (subKeys) {
    ...
    m_flAnglesStartFrame =
        RemapValClamped(m_flAnglesStartFrame, 0, nMaxFrames, 0.0f, 1.0f);
    m_flAnglesEndFrame =
        RemapValClamped(m_flAnglesEndFrame, 0, nMaxFrames, 0.0f, 1.0f);
  }
}
...
void CAI_PassengerBehavior::SetTransitionSequence(int nSequence) {
  // We need to use the ACT_SCRIPT_CUSTOM_MOVE scenario for this type of custom
  // anim
  m_nTransitionSequence = nSequence;
  GetOuter()->m_iszSceneCustomMoveSeq =
      AllocPooledString(GetOuter()->GetSequenceName(m_nTransitionSequence));
...
// Purpose:
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::SpeakIfAllowed(
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::ForceVehicleInteraction(
    const char *lpszInteractionName, CBaseCombatCharacter *pOther) {
...
//-----------------------------------------------------------------------------
void CAI_PassengerBehavior::Teleport(const Vector *newPosition,
...
void CAI_PassengerBehavior::ClearSchedule(const char *szReason) {
  // Cannot do this while we're transitioning, but it's also a bug because the
  // code that called it was probably relying on this to work!
  if (GetPassengerState() == PASSENGER_STATE_ENTERING ||
      GetPassengerState() == PASSENGER_STATE_EXITING) {
  ...
  // TODO: Even this will probably need more crafting depending on what we're
  // doing in the vehicle Otherwise allow it
  GetOuter()->ClearSchedule(szReason);
}

//-----------------------------------------------------------------------------
// Purpose: Dictate the terms for being interrupted by scripted schedules or
// scenes
//-----------------------------------------------------------------------------
bool CAI_PassengerBehavior::IsInterruptable(void) {
  // NOTE: We should never be interrupted this way when in a car. This would
  // effectively makes us go comatose if we
...
AI_BEGIN_CUSTOM_SCHEDULE_PROVIDER(CAI_PassengerBehavior) {
  ...
  DEFINE_SCHEDULE(
      ...
      "		TASK_WAIT 0.1"  // We must wait one tick for us to start being
                                // updated
  ...
  DEFINE_SCHEDULE(SCHED_PASSENGER_PLAY_SCRIPTED_ANIM,
