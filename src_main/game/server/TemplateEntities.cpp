...
//			that were configured by the level designer. This allows
//us
// to spawn 			entities with arbitrary sets of key/value data and
// entity I/O 			connections.
//
//			Template entities are marked with a special spawnflag
//which causes 			them not to spawn, but to be saved as a string
// containing all the 			map data (keyvalues and I/O connections)
// from the BSP. Template 			entities are looked up by name
// by the spawner, which copies the 			map data into a local
// string (that's how the template data is saved 			and restored).
// Once all the
// entities in the map have been activated, 			the template database
// is freed.
...
// This is appended to key's values that will need to be unique in template
// instances
...
struct TemplateEntityData_t {
  ...
  string_t iszMapData;
  int iMapDataLength;
  bool bNeedsEntityIOFixup;  // If true, this template has entity I/O in its
                             // mapdata that needs fixup before spawning.
  char *pszFixedMapData;  // A single copy of this template that we used to fix
                          // up the Entity I/O whenever someone wants a fixed
...
BEGIN_SIMPLE_DATADESC(TemplateEntityData_t)
  // DEFINE_FIELD( pszName,			FIELD_STRING	),		// Saved custom,
  // see below DEFINE_FIELD( pszMapData,			FIELD_STRING
  // ),
  // // Saved custom, see below
  ...
  // DEFINE_FIELD( pszFixedMapData,	FIELD_STRING	),		// Not saved
  // at all
END_DATADESC()
...
int Templates_Add(CBaseEntity *pEntity, const char *pszMapData, int nLen) {
  ...
  if ((!pszName) || (!strlen(pszName))) {
    DevWarning(
        1, "RegisterTemplateEntity: template entity with no name, class %s\n",
        pEntity->GetClassname());
    return -1;
  }

  TemplateEntityData_t *pEntData =
      (TemplateEntityData_t *)malloc(sizeof(TemplateEntityData_t));
  pEntData->pszName = strdup(pszName);

  // We may modify the values of the keys in this mapdata chunk later on to fix
  // Entity I/O connections. For this reason, we need to ensure we have enough
  // memory to do that.
...
//-----------------------------------------------------------------------------
// Purpose: Returns true if the specified index needs to be fixed up to be
// unique
...
//			Used by point_templates because they often have multiple
...
string_t Templates_FindByIndex(int iIndex) {
  ...
  // It's safe to do it now because this isn't called until post Entity I/O
  // cleanup.
  if (g_Templates[iIndex]->iszMapData == NULL_STRING) {
    g_Templates[iIndex]->iszMapData =
        AllocPooledString(g_Templates[iIndex]->pszMapData);
  }
...
//			NOTE: This can't handle multiple templates with the same
// targetname.
//-----------------------------------------------------------------------------
string_t Templates_FindByTargetName(const char *pszName) {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (!stricmp(pTemplate->pszName, pszName)) return Templates_FindByIndex(i);
  }
...
//			inside it's templates. Go through the keys and add look
//for
// values 			that match a name within the group's entity	names.
// Append %d to any 			found values, which will later be filled out by a unique
// identifier 			whenever the template is instanced.
//-----------------------------------------------------------------------------
void Templates_ReconnectIOForGroup(CPointTemplate *pGroup) {
  ...
  for (i = 0; i < iCount; i++) {
    ...
    newGroupTemplate.pMapDataParser = new CEntityMapData(
        g_Templates[newGroupTemplate.iIndex]->pszMapData,
        g_Templates[newGroupTemplate.iIndex]->iMapDataLength);
    Assert(newGroupTemplate.pMapDataParser);
    newGroupTemplate.pMapDataParser->ExtractValue("targetname",
                                                  newGroupTemplate.pszName);
  ...
  if (pGroup->AllowNameFixup()) {
    ...
    // Now go through all the entities in the group and parse their mapdata
    // keyvalues. We're looking for any values that match targetnames of any of
    // the group entities.
    for (i = 0; i < iCount; i++) {
      ...
      if (!mapData->GetFirstKey(keyName, value)) continue;

      do {
        // Ignore targetnames
        if (!stricmp(keyName, "targetname")) continue;
        ...
        for (int iTName = 0; iTName < iCount; iTName++) {
          ...
          if (stricmp(pName, sValue)) continue;

          if (template_debug.GetInt()) {
            Msg("Template Connection Found: Key %s (\"%s\") in entity named "
                "\"%s\"(%d) matches entity %d's targetname\n",
                keyName, sValue, GroupTemplates[i].pszName, i, iTName);
          }

          char newvalue[MAPKEY_MAXLENGTH];

          // Get the current key instance. (-1 because it's this one we're
          // changing)
          int nKeyInstance = KeyInstanceCount[idx] - 1;

          // Add our IO value to the targetname
          // We need to append it if this isn't an Entity I/O value, or prepend
          // it to the ',' if it is
          if (s) {
            Q_strncpy(newvalue, valueclipped, MAPKEY_MAXLENGTH);
            Q_strncat(newvalue, ENTITYIO_FIXUP_STRING, sizeof(newvalue),
                      COPY_ALL_CHARACTERS);
          ...
          // Set both entity's flags telling them their template needs fixup
          // when it's spawned
          g_Templates[GroupTemplates[i].iIndex]->bNeedsEntityIOFixup = true;
          g_Templates[GroupTemplates[iTName].iIndex]->bNeedsEntityIOFixup =
              true;
        }
...
void Templates_StartUniqueInstance(void) {
  ...
  int iMax =
      pow(10.0f, (int)((strlen(ENTITYIO_FIXUP_STRING) - 1)));  // -1 for the &
...
//			entity IO fixup. Fill out the pMapData with a copy of
//the
// template 			with unique key/values where the template requires
// them.
//-----------------------------------------------------------------------------
char *Templates_GetEntityIOFixedMapData(int iIndex) {
  ...
  if (!g_Templates[iIndex]->pszFixedMapData) {
    g_Templates[iIndex]->pszFixedMapData =
        new char[g_Templates[iIndex]->iMapDataLength];
    Q_strncpy(g_Templates[iIndex]->pszFixedMapData,
              g_Templates[iIndex]->pszMapData,
              g_Templates[iIndex]->iMapDataLength);
  }

  int iFixupSize = strlen(ENTITYIO_FIXUP_STRING);
  char *sOurFixup = new char[iFixupSize];
  Q_snprintf(sOurFixup, iFixupSize, "%c%.4d", ENTITYIO_FIXUP_STRING[0],
             g_iCurrentTemplateInstance);

  // Now rip through the map data string and replace any instances of the fixup
  // string with our unique identifier
...
class CTemplatesHook : public CAutoGameSystem {
 public:
  ...
  virtual void LevelShutdownPostEntity(void) { Templates_RemoveAll(); }
};
...
class CTemplate_SaveRestoreBlockHandler : public CDefSaveRestoreBlockHandler {
 public:
  const char *GetBlockName() { return "Templates"; }
  ...
  void Restore(IRestore *pRestore, bool createPlayers) {
    if (m_fDoLoad) {
      ...
      while (iTemplates--) {
        TemplateEntityData_t *pNewTemplate =
            (TemplateEntityData_t *)malloc(sizeof(TemplateEntityData_t));
        ...
        pRestore->ReadString(pNewTemplate->pszMapData,
                             pNewTemplate->iMapDataLength, sizeData);
