...
//
// Purpose:		A gib is a chunk of a body, or a piece of
// wood/metal/rocks/etc.
...
BEGIN_DATADESC(CGib)
  ...
  // Function pointers
  DEFINE_ENTITYFUNC(BounceGibTouch), DEFINE_ENTITYFUNC(StickyGibTouch),
      DEFINE_THINKFUNC(WaitTillLand), DEFINE_THINKFUNC(DieThink),
...
void CGib::LimitVelocity(void) {
  ...
  // ceiling at 1500.  The gib velocity equation is not bounded properly. Rather
  // than tune it in 3 separate places again, I'll just limit it here.
  if (length > 1500.0) {
    vecNewVelocity *=
        1500;  // This should really be sv_maxvelocity * 0.75 or something
...
void CGib::SpawnStickyGibs(CBaseEntity *pVictim, Vector vecOrigin, int cGibs) {
  ...
  for (i = 0; i < cGibs; i++) {
    ...
    if (pVictim) {
      pGib->SetLocalOrigin(Vector(vecOrigin.x + random->RandomFloat(-3, 3),
      ...
      QAngle vecAngVelocity(random->RandomFloat(250, 400),
                            random->RandomFloat(250, 400), 0);
...
void CGib::SpawnHeadGib(CBaseEntity *pVictim) {
  ...
  if (pVictim) {
    ...
    if (random->RandomInt(0, 100) <= 5 && pentPlayer) {
      ...
      vecNewVelocity =
          Vector(random->RandomFloat(-100, 100), random->RandomFloat(-100, 100),
                 random->RandomFloat(200, 300));
    }
...
// Output  :
//------------------------------------------------------------------------------
void CGib::InitGib(CBaseEntity *pVictim, float fMinVelocity,
                   float fMaxVelocity) {
  // ------------------------------------------------------------------------
  // If have a pVictim spawn the gib somewhere in the pVictim's bounding volume
  // ------------------------------------------------------------------------
  if (pVictim) {
    // Find a random position within the bounding box (add 1 to Z to get it out
    // of the ground)
    Vector vecOrigin;
    pVictim->CollisionProp()->RandomPointInBounds(vec3_origin, Vector(1, 1, 1),
                                                  &vecOrigin);
    vecOrigin.z += 1.0f;
...
// Output  :
//------------------------------------------------------------------------------
void CGib::SpawnSpecificGibs(CBaseEntity *pVictim, int nNumGibs,
                             float vMinVelocity, float vMaxVelocity,
                             const char *cModelName, float flLifetime) {
...
// Output  :
//------------------------------------------------------------------------------
void CGib::SpawnRandomGibs(CBaseEntity *pVictim, int cGibs,
                           GibType_e eGibType) {
  int cSplat;
    ...
    if (g_Language.GetInt() == LANGUAGE_GERMAN) {
      ...
      switch (eGibType) {
        case GIB_HUMAN:
          ...
          pGib->m_nBody = random->RandomInt(
              1, HUMAN_GIB_COUNT - 1);  // start at one to avoid throwing random
                                        // amounts of skulls (0th gib)
          break;
        case GIB_ALIEN:
          // alien pieces
...
// Purpose:
//-----------------------------------------------------------------------------
void CGib::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType,
               float value) {
...
void CGib::BounceGibTouch(CBaseEntity *pOther) {
  ...
  if (GetFlags() & FL_ONGROUND) {
    ...
    if (g_Language.GetInt() != LANGUAGE_GERMAN && m_cBloodDecals > 0 &&
    ...
    if (m_material != matNone && random->RandomInt(0, 2) == 0) {
      ...
      volume = 0.8f * std::min(1.0f, ((float)zvel) / 450.0f);
...
CBaseEntity *CreateRagGib(const char *szModel, const Vector &vecOrigin,
                          const QAngle &vecAngles, const Vector &vecForce,
                          float flFadeTime, bool bShouldIgnite) {
  CRagGib *pGib;
...
void CRagGib::Spawn(const char *szModel, const Vector &vecOrigin,
                    const Vector &vecForce, float flFadeTime = 0.0) {
  SetSolid(SOLID_BBOX);
  AddSolidFlags(FSOLID_NOT_SOLID);
  SetModel(szModel);
  UTIL_SetSize(this, vec3_origin, vec3_origin);
