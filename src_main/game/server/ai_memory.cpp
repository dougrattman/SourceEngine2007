...
class CAI_EnemiesListSaveRestoreOps : public CDefSaveRestoreOps {
 public:
  ...
  virtual void Save(const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave) {
    ...
    for (CAI_Enemies::CMemMap::IndexType_t i = pMemMap->FirstInorder();
         i != pMemMap->InvalidIndex(); i = pMemMap->NextInorder(i)) {
...
DEFINE_CUSTOM_FIELD(m_Map, &g_AI_MemoryListSaveRestoreOps),
    ...
    DEFINE_FIELD(hEnemy, FIELD_EHANDLE), DEFINE_FIELD(timeLastSeen, FIELD_TIME),
...
CAI_Enemies::~CAI_Enemies() {
  for (CMemMap::IndexType_t i = m_Map.FirstInorder(); i != m_Map.InvalidIndex();
...
AI_EnemyInfo_t *CAI_Enemies::GetFirst(AIEnemiesIter_t *pIter) {
  ...
  *pIter = (AIEnemiesIter_t)(unsigned)i;
  ...
  if (i == m_Map.InvalidIndex()) return NULL;

  if (m_Map[i]->hEnemy == NULL) return GetNext(pIter);
...
AI_EnemyInfo_t *CAI_Enemies::Find(CBaseEntity *pEntity, bool bTryDangerMemory) {
  if (pEntity == AI_UNKNOWN_ENEMY) pEntity = NULL;
...
bool CAI_Enemies::ShouldDiscardMemory(AI_EnemyInfo_t *pMemory) {
  ...
  if (pEnemy) {
    ...
    if (pEnemyNPC && pEnemyNPC->GetState() == NPC_STATE_DEAD) return true;
  } else {
    if (!pMemory->bDangerMemory) return true;
  }
  ...
  while (i != m_Map.InvalidIndex()) {
    ...
    if (ShouldDiscardMemory(pMemory)) {
      ...
      if (gpGlobals->curtime <=
          pMemory->timeLastSeen + m_flFreeKnowledgeDuration) {
...
bool CAI_Enemies::UpdateMemory(CAI_Network *pAINet, CBaseEntity *pEnemy,
                               const Vector &vPosition, float reactionDelay,
                               bool firstHand) {
  if (pEnemy == AI_UNKNOWN_ENEMY) pEnemy = NULL;
  ...
  if (pMemory) {
    ...
    if (firstHand) pMemory->timeLastSeen = gpGlobals->curtime;
    pMemory->bEludedMe = false;

    float deltaDist = (pMemory->vLastKnownLocation - vPosition).LengthSqr();

    if (deltaDist > DIST_TRIGGER_REACQUIRE_SQ ||
        (deltaDist > MIN_DIST_TIME_TRIGGER_REACQUIRE_SQ &&
         (gpGlobals->curtime - pMemory->timeLastSeen) >
             TIME_TRIGGER_REACQUIRE)) {
  ...
  if (firstHand) {
    pAddMemory->timeLastReacquired = pAddMemory->timeFirstSeen =
        pAddMemory->timeLastSeen = pAddMemory->timeAtFirstHand =
            gpGlobals->curtime;
  } else {
    // Block free knowledge
    pAddMemory->timeLastReacquired = pAddMemory->timeFirstSeen =
        pAddMemory->timeLastSeen =
            (gpGlobals->curtime - (m_flFreeKnowledgeDuration + 0.01));
...
void CAI_Enemies::OnTookDamageFrom(CBaseEntity *pEnemy) {
  ...
  if (pMemory) pMemory->timeLastReceivedDamageFrom = gpGlobals->curtime;
}
...
const Vector &CAI_Enemies::LastKnownPosition(CBaseEntity *pEnemy) {
  ...
  if (pMemory) {
    ...
    DevWarning(
        2, "Asking LastKnownPosition for enemy that's not in my memory!!\n");
  }
...
float CAI_Enemies::FirstTimeSeen(CBaseEntity *pEnemy) {
  ...
  AI_EnemyInfo_t *pMemory = Find(pEnemy, true);
  if (pMemory) return pMemory->timeFirstSeen;
...
bool CAI_Enemies::HasFreeKnowledgeOf(CBaseEntity *pEnemy) {
  ...
  if (pEnemy != AI_UNKNOWN_ENEMY)
    DevWarning(
        2, "Asking HasFreeKnowledgeOf for enemy that's not in my memory!!\n");
  return false;
}
...
float CAI_Enemies::LastTimeTookDamageFrom(CBaseEntity *pEnemy) {
  ...
  AI_EnemyInfo_t *pMemory = Find(pEnemy, true);
  if (pMemory) return pMemory->timeLastReceivedDamageFrom;
...
float CAI_Enemies::TimeAtFirstHand(CBaseEntity *pEnemy) {
  ...
  AI_EnemyInfo_t *pMemory = Find(pEnemy, true);
  if (pMemory) return pMemory->timeAtFirstHand;
