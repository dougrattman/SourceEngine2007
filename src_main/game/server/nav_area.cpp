...
ConVar nav_coplanar_slope_limit("nav_coplanar_slope_limit", "0.99",
                                FCVAR_GAMEDLL);
ConVar nav_split_place_on_ground(
    "nav_split_place_on_ground", "0", FCVAR_GAMEDLL,
    "If true, nav areas will be placed flush with the ground when split.");
ConVar nav_area_bgcolor(
    "nav_area_bgcolor", "0 0 0 30", FCVAR_GAMEDLL,
...
ConVar nav_corner_adjust_adjacent("nav_corner_adjust_adjacent", "18",
                                  FCVAR_GAMEDLL,
                                  "radius used to raise/lower corners in "
                                  "nearby areas when raising/lowering "
                                  "corners.");
...
void CNavArea::Initialize(void) {
  ...
  // set an ID for splitting and other interactive editing - loads will
  // overwrite this
  m_id = m_nextID++;
 ...
 */
CNavArea::CNavArea(void) { Initialize(); }
 ...
 */
CNavArea::CNavArea(CNavNode *nwNode, CNavNode *neNode, CNavNode *seNode,
                   CNavNode *swNode) {
  Initialize();
...
CNavArea::~CNavArea() {
  ...
  // if we are resetting the system, don't bother cleaning up - all areas are
  // being destroyed
  if (m_isReset) return;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * This is invoked at the start of an incremental nav generation on pre-existing
 * areas.
 */
...
void CNavArea::OnDestroyNotify(CNavArea *dead) {
  NavConnect con;
  con.area = dead;
  for (int d = 0; d < NUM_DIRECTIONS; ++d) m_connect[d].FindAndRemove(con);
 ...
 * Remove any references we have to it.
 */
void CNavArea::OnDestroyNotify(CNavLadder *dead) { Disconnect(dead); }
...
void CNavArea::ConnectTo(CNavArea *area, NavDirType dir) {
  ...
  FOR_EACH_LL(m_connect[dir], it) {
    if (m_connect[dir][it].area == area) return;
  }
...
void CNavArea::MergeAdjacentConnections(CNavArea *adjArea) {
  ...
  // remove any references from this area to the adjacent area, since it is now
  // part of us
  for (dir = 0; dir < NUM_DIRECTIONS; dir++) {
    NavConnect connect;
  ...
  // We can't just replace existing connections, as several adjacent areas may
  // have been merged into one, resulting in a large area adjacent to all of
  // them ending up with multiple redunandant connections into the merged area,
  // one for each of the adjacent subsumed smaller ones. If an area has a
  // connection to the merged area, we must remove all references to adjArea,
  // and add a single connection to us.
  FOR_EACH_LL(TheNavAreaList, it) {
    CNavArea *area = TheNavAreaList[it];
  ...
  FOR_EACH_LL(TheNavMesh->GetLadders(), lit) {
    ...
    if (ladder->m_topForwardArea == adjArea) ladder->m_topForwardArea = this;

    if (ladder->m_topRightArea == adjArea) ladder->m_topRightArea = this;

    if (ladder->m_topLeftArea == adjArea) ladder->m_topLeftArea = this;

    if (ladder->m_topBehindArea == adjArea) ladder->m_topBehindArea = this;

    if (ladder->m_bottomArea == adjArea) ladder->m_bottomArea = this;
  }
}
...
void CNavArea::AssignNodes(CNavArea *area) {
  ...
  for (CNavNode *vertNode = m_node[NORTH_WEST]; vertNode != m_node[SOUTH_WEST];
...
bool CNavArea::SplitEdit(bool splitAlongX, float splitEdge, CNavArea **outAlpha,
  ...
  if (splitAlongX) {
    ...
    if (splitEdge <= m_extent.lo.y + 1.0f) return false;

    if (splitEdge >= m_extent.hi.y - 1.0f) return false;
    ...
    // don't do split if at edge of area
    if (splitEdge <= m_extent.lo.x + 1.0f) return false;

    if (splitEdge >= m_extent.hi.x - 1.0f) return false;
  ...
  for (dir = 0; dir < CNavLadder::NUM_LADDER_DIRECTIONS; ++dir) {
    FOR_EACH_LL(m_ladder[dir], it) {
      ...
      Vector ladderPos =
          ladder->m_top;  // doesn't matter if we choose top or bottom
  ...
  // For every ladder that pointed to us, connect that ladder to the closer of
  // alpha and beta
  FOR_EACH_LL(TheNavMesh->GetLadders(), lit) {
    CNavLadder *ladder = TheNavMesh->GetLadders()[lit];
  ...
  if (dir == NUM_DIRECTIONS) {
    ...
    for (int d = 0; d < NUM_DIRECTIONS; ++d) {
      FOR_EACH_LL(m_connect[d], it) {
        if (area == m_connect[d][it].area) return true;
      }
    }
    ...
    FOR_EACH_LL(m_ladder[CNavLadder::LADDER_UP], it) {
      ...
      if (ladder->m_topBehindArea == area || ladder->m_topForwardArea == area ||
          ladder->m_topLeftArea == area || ladder->m_topRightArea == area)
        return true;
    }
    ...
    FOR_EACH_LL(m_ladder[CNavLadder::LADDER_DOWN], dit) {
      ...
      if (ladder->m_bottomArea == area) return true;
    }
  } else {
    // check specific direction
    FOR_EACH_LL(m_connect[dir], it) {
      if (area == m_connect[dir][it].area) return true;
    }
  }
 ...
 * The "ignoreEdge" direction defines the side of the original area that the new
 * area does not include
 */
void CNavArea::FinishSplitEdit(CNavArea *newArea, NavDirType ignoreEdge) {
  newArea->m_center.x =
      (newArea->m_extent.lo.x + newArea->m_extent.hi.x) / 2.0f;
  newArea->m_center.y =
      (newArea->m_extent.lo.y + newArea->m_extent.hi.y) / 2.0f;
  newArea->m_center.z =
      (newArea->m_extent.lo.z + newArea->m_extent.hi.z) / 2.0f;
  ...
  if (HasNodes()) {
    ...
    while (!newArea->IsOverlapping(*newArea->m_node[corner[0]]->GetPosition(),
                                   GenerationStepSize / 2)) {
      for (int i = 0; i < 2; ++i) {
        Assert(newArea->m_node[corner[i]]);
        Assert(newArea->m_node[corner[i]]->GetConnectedNode(dir));
        newArea->m_node[corner[i]] =
            newArea->m_node[corner[i]]->GetConnectedNode(dir);
      }
    }
...
void CNavArea::Strip(void) {
  m_approachCount = 0;
  m_spotEncounterList
      .PurgeAndDeleteElements();  // this calls delete on each element
}
...
bool CNavArea::IsOverlapping(const Vector &pos, float tolerance) const {
  if (pos.x + tolerance >= m_extent.lo.x &&
      pos.x - tolerance <= m_extent.hi.x &&
...
bool CNavArea::IsOverlapping(const CNavArea *area) const {
  if (area->m_extent.lo.x < m_extent.hi.x &&
      area->m_extent.hi.x > m_extent.lo.x &&
      area->m_extent.lo.y < m_extent.hi.y &&
      area->m_extent.hi.y > m_extent.lo.y)
    return true;
...
bool CNavArea::IsOverlappingY(const CNavArea *area) const {
  if (area->m_extent.lo.y < m_extent.hi.y &&
      area->m_extent.hi.y > m_extent.lo.y)
    return true;
...
bool CNavArea::Contains(const Vector &pos) const {
  ...
  // the point overlaps us, check that it is above us, but not above any areas
  // that overlap us
  float myZ = GetZ(pos);

  // if the nav area is above the given position, fail
  if (myZ > pos.z) return false;
  ...
  FOR_EACH_LL(m_overlapList, it) {
    ...
    // check 2D overlap
    if (!area->IsOverlapping(pos)) continue;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return true if the area is approximately flat, using normals computed from
 * opposite corners
 */
...
bool CNavArea::IsCoplanar(const CNavArea *area) const {
  ...
  // can only merge areas that are nearly planar, to ensure areas do not differ
  // from underlying geometry much
  const float tolerance =
      nav_coplanar_slope_limit
          .GetFloat();  // 0.99f; // 0.7071f;		// 0.9
  if (DotProduct(normal, otherNormal) > tolerance) return true;
...
float CNavArea::GetZ(const Vector &pos) const {
  ...
  if (dx == 0.0f || dy == 0.0f) return m_neZ;
...
CNavArea *CNavArea::GetRandomAdjacentArea(NavDirType dir) const {
  ...
  int i = 0;
  FOR_EACH_LL(m_connect[dir], it) {
    if (i == which) return m_connect[dir][it].area;
 ...
 * Return center of portal opening, and half-width defining sides of portal from
 * center. NOTE: center->z is unset.
 */
void CNavArea::ComputePortal(const CNavArea *to, NavDirType dir, Vector *center,
...
bool CNavArea::GetCornerHotspot(NavCornerType corner,
  ...
  float dist =
      IntersectRayWithTriangle(ray, hotspot[0], hotspot[1], hotspot[2], false);
  if (dist > 0) {
    return true;
  }

  dist =
      IntersectRayWithTriangle(ray, hotspot[2], hotspot[3], hotspot[0], false);
  if (dist > 0) {
    return true;
  }
...
void CNavArea::Draw(void) const {
  ...
  if (IsDegenerate()) {
    ...
    NDebugOverlay::Text(
        GetCenter(), UTIL_VarArgs("Degenerate area %d", GetID()), true, 0.1f);
  }
  ...
  if (4 == sscanf(nav_area_bgcolor.GetString(), "%d %d %d %d", &(bgcolor[0]),
    ...
    if (bgcolor[3] > 0) {
      ...
      NDebugOverlay::Triangle(nw + offset, se + offset, ne + offset, bgcolor[0],
                              bgcolor[1], bgcolor[2], bgcolor[3], true, 0.15f);
      NDebugOverlay::Triangle(se + offset, nw + offset, sw + offset, bgcolor[0],
                              bgcolor[1], bgcolor[2], bgcolor[3], true, 0.15f);
    }
  }
  ...
  if (this != TheNavMesh->GetMarkedArea() &&
      ...
      TheNavMesh->m_navEditMode == CNavMesh::NAV_EDIT_NORMAL) {
  ...
  if (GetAttributes() & NAV_MESH_CROUCH) {
    if (useAttributeColors) color = NavAttributeCrouchColor;

    NavDrawLine(nw, se, color);
  }

  if (GetAttributes() & NAV_MESH_JUMP) {
    if (useAttributeColors) color = NavAttributeJumpColor;
  ...
  if (GetAttributes() & NAV_MESH_PRECISE) {
    if (useAttributeColors) color = NavAttributePreciseColor;
  ...
  if (GetAttributes() & NAV_MESH_NO_JUMP) {
    if (useAttributeColors) color = NavAttributeNoJumpColor;
  ...
  if (GetAttributes() & NAV_MESH_STOP) {
    if (useAttributeColors) color = NavAttributeStopColor;
  ...
  if (GetAttributes() & NAV_MESH_WALK) {
    if (useAttributeColors) color = NavAttributeWalkColor;
  ...
  if (GetAttributes() & NAV_MESH_RUN) {
    if (useAttributeColors) color = NavAttributeRunColor;
  ...
  if (GetAttributes() & NAV_MESH_AVOID) {
    if (useAttributeColors) color = NavAttributeAvoidColor;
...
void CNavArea::DrawConnectedAreas(void) const {
  ...
  if (!ladder->IsConnected(this, CNavLadder::LADDER_DOWN)) {
    NavDrawLine(m_center, ladder->m_bottom + Vector(0, 0, GenerationStepSize),
...
void CNavArea::AddToOpenList(void) {
  ...
  for (area = m_openList; area; area = area->m_nextOpen) {
    if (this->GetTotalCost() < area->GetTotalCost()) break;
 ...
 * @todo "bubbling" does unnecessary work, since the order of all other nodes
 * will be unchanged - only this node is altered
 */
void CNavArea::UpdateOnOpenList(void) {
  ...
  while (m_prevOpen && this->GetTotalCost() < m_prevOpen->GetTotalCost()) {
    ...
    if (after) after->m_prevOpen = other;
  }
}
...
void CNavArea::RemoveFromOpenList(void) {
  ...
  if (m_nextOpen) m_nextOpen->m_prevOpen = m_prevOpen;
...
bool IsHidingSpotInCover(const Vector &spot) {
  ...
  UTIL_TraceLine(from, to, MASK_PLAYERSOLID_BRUSHONLY, NULL,
  ...
  for (float angle = 0.0f; angle < 2.0f * M_PI; angle += inc) {
    to = from + Vector(coverRange * (float)cos(angle),
                       coverRange * (float)sin(angle), HalfHumanHeight);
  ...
  if (coverCount < halfCover) return false;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Finds the hiding spot position in a corner's area.  If the typical inset is
 * off the nav area (small hand-constructed areas), it tries to fit the position
 * inside the area.
 */
static Vector FindPositionInArea(CNavArea *area, NavCornerType corner) {
  ...
  if (!area->IsOverlapping(pos)) {
    ...
    if (!area->IsOverlapping(pos)) {
      ...
      pos = cornerPos +
            Vector(area->GetSizeX() * 0.5f * multX, offset * multY, 0.0f);
      if (!area->IsOverlapping(pos)) {
        // Try pulling the X and Y offsets to the area's center
        pos = cornerPos + Vector(area->GetSizeX() * 0.5f * multX,
                                 area->GetSizeY() * 0.5f * multY, 0.0f);
        if (!area->IsOverlapping(pos)) {
          AssertMsg(false, UTIL_VarArgs("A Hiding Spot can't be placed on its "
                                        "area at (%.0f %.0f %.0f)",
          ...
          if (!area->IsOverlapping(pos)) {
            // Nothing is working (degenerate area?), so just put it directly on
            // the corner
            pos = cornerPos;
          }
        }
...
void CNavArea::ComputeHidingSpots(void) {
  ...
  for (int i = 0; i < NUM_CORNERS; ++i) cornerCount[i] = 0;
  ...
  for (int d = 0; d < NUM_DIRECTIONS; ++d) {
    ...
    FOR_EACH_LL(m_connect[d], it) {
      ...
      // if connection is only one-way, it's a "jump down" connection (ie: a
      // discontinuity that may mean cover) ignore it
      if (connect.area->IsConnected(
              this, OppositeDirection(static_cast<NavDirType>(d))) == false)
        continue;
    ...
    switch (d) {
      case NORTH:
        if (extent.lo - m_extent.lo.x >= cornerSize) ++cornerCount[NORTH_WEST];

        if (m_extent.hi.x - extent.hi >= cornerSize) ++cornerCount[NORTH_EAST];
        break;

      case SOUTH:
        if (extent.lo - m_extent.lo.x >= cornerSize) ++cornerCount[SOUTH_WEST];

        if (m_extent.hi.x - extent.hi >= cornerSize) ++cornerCount[SOUTH_EAST];
        break;

      case EAST:
        if (extent.lo - m_extent.lo.y >= cornerSize) ++cornerCount[NORTH_EAST];

        if (m_extent.hi.y - extent.hi >= cornerSize) ++cornerCount[SOUTH_EAST];
        break;

      case WEST:
        if (extent.lo - m_extent.lo.y >= cornerSize) ++cornerCount[NORTH_WEST];

        if (m_extent.hi.y - extent.hi >= cornerSize) ++cornerCount[SOUTH_WEST];
        break;
    }
  }
  ...
  for (int c = 0; c < NUM_CORNERS; ++c) {
    ...
    if (cornerCount[c] == 2) {
      ...
      if (!c || !IsHidingSpotCollision(pos)) {
        ...
        spot->SetFlags(IsHidingSpotInCover(pos) ? HidingSpot::IN_COVER
                                                : HidingSpot::EXPOSED);
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Determine how much walkable area we can see from the spot, and how far away
 * we can see.
 */
void ClassifySniperSpot(HidingSpot *spot) {
  ...
  FOR_EACH_LL(TheNavAreaList, it) {
    ...
    // scan this area
    for (walkable.y = extent.lo.y + GenerationStepSize / 2.0f;
         walkable.y < extent.hi.y; walkable.y += GenerationStepSize) {
      for (walkable.x = extent.lo.x + GenerationStepSize / 2.0f;
           walkable.x < extent.hi.x; walkable.x += GenerationStepSize) {
        ...
        if (result.fraction == 1.0f && !result.startsolid) {
          ...
          if (rangeSq > farthestRangeSq) {
            ...
            if (rangeSq >= minSniperRangeSq) {
              ...
              // determine how good of a sniper spot it is by keeping track of
              // the snipable area
              if (found) {
  ...
  if (found) {
    ...
    if (snipableArea >= minIdealSniperArea ||
...
void CNavArea::AddSpotEncounters(const CNavArea *from, NavDirType fromDir,
  ...
  for (float along = 0.0f; !done; along += stepSize) {
    ...
    FOR_EACH_LL(TheHidingSpotList, it) {
      ...
      // only look at spots with cover (others are out in the open and easily
      // seen)
      ...
      UTIL_TraceLine(
          eye, Vector(spotPos.x, spotPos.y, spotPos.z + HalfHumanHeight),
          MASK_PLAYERSOLID_BRUSHONLY, NULL, COLLISION_GROUP_NONE, &result);
      if (result.fraction != 1.0f) continue;
    ...
    FOR_EACH_LL(m_connect[fromDir], it) {
      ...
      for (int toDir = 0; toDir < NUM_DIRECTIONS; ++toDir) {
        FOR_EACH_LL(m_connect[toDir], ot) {
          ...
          AddSpotEncounters(fromCon->area, (NavDirType)fromDir, toCon->area,
                            (NavDirType)toDir);
        }
      }
    }
...
void CNavArea::DecayDanger(void) {
  ...
  for (int i = 0; i < MAX_NAV_TEAMS; ++i) {
    ...
    if (m_danger[i] < 0.0f) m_danger[i] = 0.0f;
...
void CNavArea::RaiseCorner(NavCornerType corner, int amount,
  ...
  for (int x = gridX - shift; x <= gridX + shift; ++x) {
    if (x < 0 || x >= TheNavMesh->m_gridSizeX) continue;

    for (int y = gridY - shift; y <= gridY + shift; ++y) {
      if (y < 0 || y >= TheNavMesh->m_gridSizeY) continue;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * FindGroundZFromPoint walks from the start position to the end position in
 * GenerationStepSize increments, checking the ground height along the way.
 */
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Finds the Z value for a corner given two other corner points.  This walks
 * along the edges of the nav area in GenerationStepSize increments, to increase
 * accuracy.
 */
  ...
  if (TheNavMesh->GetMarkedArea()) {
    ...
    if (area->IsBlocked()) {
      DevMsg("Area #%d %s is blocked\n", area->GetID(),
             VecToString(area->GetCenter() + Vector(0, 0, HalfHumanHeight)));
    }
  } else {
    ...
    if (blockedArea) {
      ...
      if (player) {
        if ((player->IsDead() || player->IsObserver()) &&
          ...
          Vector origin =
              blockedArea->GetCenter() + Vector(0, 0, 0.75f * HumanHeight);
...
static ConCommand nav_update_blocked(
    "nav_update_blocked", CommandNavUpdateBlocked,
    "Updates the blocked/unblocked status for every nav area.", FCVAR_GAMEDLL);
 ...
 public:
  ...
  virtual IterationRetval_t EnumElement(IHandleEntity *pHandleEntity) {
    ...
    if (pEnt->GetCollisionGroup() != COLLISION_GROUP_PUSHAWAY &&
        pEnt->GetCollisionGroup() != COLLISION_GROUP_BREAKABLE_GLASS &&
        pEnt->GetCollisionGroup() != COLLISION_GROUP_PLAYER &&
        pEnt->GetCollisionGroup() != COLLISION_GROUP_NONE)
    ...
    if (pInterface) {
      ...
      if ((FClassnameIs(pEnt, "func_breakable") ||
        ...
        if (!isBreakable) return ITERATION_CONTINUE;
...
void CNavArea::UpdateBlocked(void) {
  ...
  trace_t tr;
  UTIL_TraceHull(origin, origin, mins, maxs, MASK_PLAYERSOLID_BRUSHONLY,
                 &filter, &tr);
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Checks if there is a floor under the nav area, in case a breakable floor is
 * gone
 */
void CNavArea::CheckFloor(CBaseEntity *ignore) {
  ...
  // See if spot is valid
  trace_t tr;
  UTIL_TraceHull(origin, origin, mins, maxs, MASK_PLAYERSOLID_BRUSHONLY, ignore,
...
static ConCommand nav_check_floor(
    "nav_check_floor", CommandNavCheckFloor,
    "Updates the blocked/unblocked status for every nav area.", FCVAR_GAMEDLL);
