...
CBasePlayer *BotPutInServer(bool bFrozen, int iTeam) {
  ...
  if (iTeam == -1) iTeam = (pPlayer->entindex() & 1) ? TEAM_TERRORIST : TEAM_CT;
...
bool IsTempBot(CBaseEntity *pEnt) {
  ...
  if (!(pEnt->GetFlags() & FL_FAKECLIENT)) return false;
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove(CCSPlayer *fakeclient, const QAngle &viewangles,
...
void Bot_Think(CCSPlayer *pBot) {
  ...
  if (pBot->GetTeamNumber() == TEAM_UNASSIGNED &&
    ...
    pBot->HandleCommand_JoinTeam(botdata->m_WantedTeam);
             ...
             pBot->PlayerClass() == CS_CLASS_NONE) {
    ...
    if (pBot->IsAlive() && (pBot->GetSolid() == SOLID_BBOX)) {
      ...
      if (!pBot->IsEFlagSet(EFL_BOT_FROZEN) && pBot->m_iHealth == 100) {
        ...
        while (--maxtries >= 0) {
          ...
          botdata->lastturntoright =
              random->RandomInt(0, 1) == 0 ? true : false;
        ...
        if ( gpGlobals->curtime >= botdata->nextstrafetime )
        {
                ...
                {
                        botdata->sidemove = -600.0f + 1200.0f *
        random->RandomFloat( 0, 2 );
                }
                else
                {
                        botdata->sidemove = 0;
...
void PossessBot_f(const CCommand &args) {
  ...
  if (iBotClient < 0 || iBotClient >= gpGlobals->maxClients ||
...
ConCommand cc_PossessBot(
    "PossessBot", PossessBot_f,
    "Toggle. Possess a bot.\n\tArguments: <bot client number>", FCVAR_CHEAT);
