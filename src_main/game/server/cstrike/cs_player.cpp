...
ConVar cs_ShowStateTransitions("cs_ShowStateTransitions", "-2", FCVAR_CHEAT,
                               "cs_ShowStateTransitions <ent index or -1 for "
                               "all>. Show player state transitions.");
ConVar sv_max_usercmd_future_ticks("sv_max_usercmd_future_ticks", "8", 0,
                                   "Prevents clients from running usercmds too "
                                   "far in the future. Prevents speed hacks.");
...
SendPropFloat(SENDINFO(m_flStamina), 14, 0, 0, 1400),
    ...
    // cs_playeranimstate and clientside animation takes care of these on the
    // client
    SendPropExclude("DT_ServerAnimationData", "m_flCycle"),
    SendPropExclude("DT_AnimTimeMustBeFirst", "m_flAnimTime"),

    // We need to send a hi-res origin to the local player to avoid prediction
    // errors sliding along walls
    SendPropExclude("DT_BaseEntity", "m_vecOrigin"),

    // Data that only gets sent to the local player.
    SendPropDataTable("cslocaldata", 0,
                      &REFERENCE_SEND_TABLE(DT_CSLocalPlayerExclusive),
                      SendProxy_SendLocalDataTable),
    SendPropDataTable("csnonlocaldata", 0,
                      &REFERENCE_SEND_TABLE(DT_CSNonLocalPlayerExclusive),
                      SendProxy_SendNonLocalDataTable),

    SendPropInt(SENDINFO(m_iThrowGrenadeCounter), THROWGRENADE_COUNTER_BITS,
                SPROP_UNSIGNED),
    ...
    SendPropBool(SENDINFO(m_bNightVisionOn)),  // send as int so we can use a
                                               // RecvProxy on the client
...
ConCommand cc_CreatePredictionError("CreatePredictionError",
                                    cc_CreatePredictionError_f,
                                    "Create a prediction error", FCVAR_CHEAT);
...
void CCSPlayer::Precache() {
  ...
  // Sigh - have to force identical VMTs for the player models.  I'm just going
  // to hard-code these strings here, rather than have char***'s or the
  // CUtlVector<CUtlVector<>> equivalent.
  engine->ForceSimpleMaterial("materials/models/player/ct_urban/ct_urban.vmt");
  engine->ForceSimpleMaterial(
...
void CCSPlayer::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper) {
  ...
  if (!IsEngineThreaded() &&
      (ucmd->tick_count >
       (gpGlobals->tickcount + sv_max_usercmd_future_ticks.GetInt()))) {
    DevMsg("Client cmd out of sync (delta %i).\n",
           ucmd->tick_count - gpGlobals->tickcount);
    return;
  }

  // If they use a negative bot_mimic value, then don't process their usercmds,
  // but have bots process them instead (so they can stay still and have the bot
  // move around).
  CUserCmd tempCmd;
...
bool CCSPlayer::RunMimicCommand(CUserCmd &cmd) {
  ...
  if (iMimic > gpGlobals->maxClients) return false;
...
//-----------------------------------------------------------------------------
void CCSPlayer::RunPlayerMove(const QAngle &viewangles, float forwardmove,
                              float sidemove, float upmove,
                              unsigned short buttons, byte impulse,
                              float frametime) {
  CUserCmd cmd;
...
void CCSPlayer::InitialSpawn(void) {
  ...
  // FinishClientPutInServer() because of the bots' timing for purchasing
  // weapons/items.
  if (IsBot()) {
    m_iAccount = CSGameRules()->GetStartMoney();
  }

  if (!engine->IsDedicatedServer() && !TheNavMesh->IsFromCurrentMap() &&
      this == UTIL_GetListenServerHost()) {
    ClientPrint(
        this, HUD_PRINTCENTER,
...
void CCSPlayer::SetModelFromClass(void) {
  if (GetTeamNumber() == TEAM_TERRORIST) {
    ...
    if (index < 0 || index >= TerroristPlayerModels.Count()) {
      ...
      m_iClass = index + FIRST_T_CLASS;  // clean up players who selected a
                                         // higher class than we support yet
    }
    ...
    if (index < 0 || index >= CTPlayerModels.Count()) {
      ...
      m_iClass = index + FIRST_CT_CLASS;  // clean up players who selected a
                                          // higher class than we support yet
    }
...
void CCSPlayer::Spawn() {
  ...
  AddFlag(
      FL_ONGROUND);  // set the player on the ground at the start of the round.
  ...
  // Special-case here. A bunch of things happen in CBasePlayer::Spawn(), and we
  // really want the player states to control these things, so give whatever
  // player state we're in a chance to reinitialize itself.
  ...
  if (!engine->IsDedicatedServer() && CSGameRules()->m_iTotalRoundsPlayed < 2 &&
      !TheNavMesh->IsFromCurrentMap() && this == UTIL_GetListenServerHost()) {
    ClientPrint(
        this, HUD_PRINTCENTER,
        "The Navigation Mesh was built using a different version of this map.");
  }
  ...
  SetContextThink(&CCSPlayer::PushawayThink,
                  gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL,
                  CS_PUSHAWAY_THINK_CONTEXT);

  if (GetActiveWeapon() && !IsObserver()) {
    GetActiveWeapon()->Deploy();
    m_flNextAttack = gpGlobals->curtime;  // Allow reloads to finish, since
                                          // we're playing the deploy anim
                                          // instead.  This mimics goldsrc
                                          // behavior, anyway.
  }
...
void CCSPlayer::ShowViewPortPanel(const char *name, bool bShow,
                                  KeyValues *data) {
  if (CSGameRules()->IsLogoMap()) return;
...
int CCSPlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info) {
  ...
  if (event) {
    ...
    if (strncmp(weaponName, "weapon_", 7) == 0) {
      weaponName += 7;
    } else if (strncmp(weaponName, "hegrenade", 9) ==
               0)  //"hegrenade_projectile"
    {
      weaponName = "hegrenade";
    } else if (strncmp(weaponName, "flashbang", 9) ==
               0)  //"flashbang_projectile"
    {
      weaponName = "flashbang";
    } else if (strncmp(weaponName, "smokegrenade", 12) ==
               0)  //"smokegrenade_projectile"
    {
...
// Purpose:
//-----------------------------------------------------------------------------
void CCSPlayer::InitVCollision(const Vector &vecAbsOrigin,
...
void CCSPlayer::SetupVisibility(CBaseEntity *pViewEntity, unsigned char *pvs,
                                int pvssize) {
  BaseClass::SetupVisibility(pViewEntity, pvs, pvssize);

  int area = pViewEntity ? pViewEntity->NetworkProp()->AreaNum()
                         : NetworkProp()->AreaNum();
...
void CCSPlayer::UpdateAddonBits() {
  ...
  if (weapon && weapon != GetActiveWeapon()) {
    ...
    // The active weapon is weapon_elite.  Set ADDON_PISTOL2 without
    // ADDON_PISTOL, so we know to display the empty holster.
    iNewBits |= ADDON_PISTOL2;
    m_iSecondaryAddon = weapon->GetWeaponID();
  } else {
...
void CCSPlayer::UpdateRadar() {
  ...
  engine->Message_DetermineMulticastRecipients(false, EyePosition(),
                                               playerbits);
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (playerbits.Get(i) && bSameTeam == true)
      continue;  // this player is in my PVS and not in my team, don't update
                 // radar pos
...
void CCSPlayer::UpdateMouseoverHints() {
  if (IsBlind() || IsObserver()) return;

  Vector forward, up;
  EyeVectors(&forward, NULL, &up);

  trace_t tr;
  // Search for objects in a sphere (tests for entities that are not solid, yet
  // still useable)
  ...
  UTIL_TraceLine(searchStart, searchEnd, useableContents, this,
  ...
  if (tr.fraction != 1.0f) {
    if (tr.DidHitNonWorldEntity() && tr.m_pEnt) {
      ...
      switch (pObject->Classify()) {
        case CLASS_PLAYER: {
          const float grenadeBloat = 1.2f;  // Be conservative in estimating
                                            // what a player can distinguish
          if (!TheBots->IsLineBlockedBySmoke(
                  EyePosition(), pObject->EyePosition(), grenadeBloat)) {
            if (g_pGameRules->PlayerRelationship(this, pObject) ==
                GR_TEAMMATE) {
        ...
        case CLASS_PLAYER_ALLY:
          switch (GetTeamNumber()) {
            case TEAM_CT:
              ...
              } else if (!(m_iDisplayHistoryBits & DHF_HOSTAGE_SEEN_NEAR) &&
                         tr.fraction <= 0.1f) {
                m_iDisplayHistoryBits |= DHF_HOSTAGE_SEEN_FAR;
...
void CCSPlayer::PostThink() {
  ...
  if (GetActiveWeapon() && !(m_iDisplayHistoryBits & DHF_AMMO_EXHAUSTED)) {
    ...
    if (!pWeapon->HasAnyAmmo() &&
        !(pWeapon->GetWpnData().iFlags & ITEM_FLAG_EXHAUSTIBLE)) {
  ...
  if ((m_applyDeafnessTime != 0.0f) &&
      (m_applyDeafnessTime <= gpGlobals->curtime)) {
    ApplyDeafnessEffect();
  }
  ...
  if (IsAlive() && m_cycleLatchTimer.IsElapsed()) {
    ...
    // Cycle is a float from 0 to 1.  We don't need to transmit a whole float
    // for that.  Compress it in to a small fixed point
...
void CCSPlayer::PushawayThink() {
  ...
  SetNextThink(gpGlobals->curtime + PUSHAWAY_THINK_INTERVAL,
               CS_PUSHAWAY_THINK_CONTEXT);
}
...
bool CCSPlayer::Weapon_CanSwitchTo(CBaseCombatWeapon *pWeapon) {
  ...
  if (GetActiveWeapon()) {
    if (!GetActiveWeapon()->CanHolster()) return false;
  }
...
bool CCSPlayer::ShouldDoLargeFlinch(int nHitGroup, CBaseEntity *pAttacker) {
  if (FBitSet(GetFlags(), FL_DUCKING)) return FALSE;
...
int CCSPlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo) {
  ...
  if (bFriendlyFire && pInflictor->GetTeamNumber() == GetTeamNumber() &&
      pInflictor != this && info.GetAttacker() != this) {
    CCSPlayer *pCSAttacker = ToCSPlayer(pInflictor);
    if (!pCSAttacker) pCSAttacker = ToCSPlayer(info.GetAttacker());
      ...
      if ((pCSAttacker->m_flLastAttackedTeammate + 0.6f) < gpGlobals->curtime) {
        ...
        for (int i = 1; i <= gpGlobals->maxClients; ++i) {
          ...
          if (pPlayer && pPlayer->GetTeamNumber() == GetTeamNumber()) {
            ClientPrint(pPlayer, HUD_PRINTTALK, "#Game_teammate_attack",
                        pCSAttacker->GetPlayerName());
          }
        }
      }
  ...
  if (bFriendlyFire || pInflictor->GetTeamNumber() != GetTeamNumber() ||
    ...
    if (ArmorValue() && !(info.GetDamageType() & (DMG_FALL | DMG_DROWN)) &&
      ...
      if (!(info.GetDamageType() & DMG_FALL)) Pain(true /*has armor*/);
    } else {
      m_lastDamageArmor = 0;
      if (!(info.GetDamageType() & DMG_FALL)) Pain(false /*no armor*/);
    }
...
void CCSPlayer::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir,
                            trace_t *ptr) {
  ...
  if (bHitShield) {
    ...
    if (ArmorValue() > 0) bShouldBleed = false;
  ...
  // Since this code only runs on the server, make sure it shows the tempents it
  // creates.
  ...
  if ((ptr->hitgroup == HITGROUP_HEAD || bHitShield) &&
      bShouldSpark)  // they hit a helmet
  {
...
//-----------------------------------------------------------------------------
void CCSPlayer::HintMessage(const char *pMessage, bool bDisplayIfDead,
...
void CCSPlayer::ObserverRoundRespawn() {
  ...
  if (m_szNewName[0] != 0) {
    // ... and force the name change now.  After this happens, the gamerules
    // will get a ClientSettingsChanged callback from the above ClientCommand,
    // but the name matches what we're setting here, so it will do nothing.
    ChangeName(m_szNewName);
    m_szNewName[0] = 0;
  }
}
...
void CCSPlayer::RoundRespawn() {
  ...
  m_receivesMoneyNextRound =
      true;  // reset this variable so they can receive their cash next round.
...
void CCSPlayer::PreThink() {
  ...
  if (mp_autokick.GetBool() && !IsBot() && !IsHLTV() && !IsAutoKickDisabled()) {
    if (m_flLastMovement + CSGameRules()->GetRoundLength() * 2 <
        gpGlobals->curtime) {
...
#ifndef _XBOX
  if (!IsAlive()) {
    // CS would like their players to continue to update their LastArea even
    // when dead since it is displayed in the observer screen now. But we won't
    // do the population tracking while dead.
...
void CCSPlayer::MoveToNextIntroCamera() {
  ...
  // if m_pIntroCamera is NULL we just were at end of list, start searching from
  // start again
  if (!m_pIntroCamera)
    m_pIntroCamera =
        gEntList.FindEntityByClassname(m_pIntroCamera, "point_viewcontrol");
  ...
  if (!m_pIntroCamera)
    m_pIntroCamera =
        gEntList.FindEntityByClassname(m_pIntroCamera, "info_player_terrorist");

  SetViewOffset(vec3_origin);                    // no view offset
  UTIL_SetSize(this, vec3_origin, vec3_origin);  // no bbox

  if (!Target)  // if there are no cameras(or the camera has no target, find a
                // spawn point and black out the screen
  {
...
// Expose the VIP selection to plugins, since we don't have an official VIP
// mode.  This allows plugins to access the (limited) VIP functionality already
// present (scoreboard identification and radar color).
CON_COMMAND(cs_make_vip, "Marks a player as the VIP") {
  ...
  CCSPlayer *player =
      static_cast<CCSPlayer *>(UTIL_PlayerByIndex(atoi(args[1])));
  if (!player) {
...
void CCSPlayer::DropShield(void) {
  ...
  CBaseAnimating *pShield = (CBaseAnimating *)CBaseEntity::Create(
      "item_shield", WorldSpaceCenter(), GetLocalAngles());
  pShield->ApplyAbsVelocityImpulse(vForward * 200 +
                                   vRight * random->RandomFloat(-50, 50));
...
bool CCSPlayer::CSWeaponDrop(CBaseCombatWeapon *pWeapon, bool bDropShield,
                             bool bThrowForward) {
  bool bSuccess = false;
  ...
  if (pWeapon) {
    ...
    pWeapon->SetSolidFlags(FSOLID_NOT_STANDABLE | FSOLID_TRIGGER |
        ...
        if (IsAlive() && GetHealth() > 0) {
          for (int i = 0; i < MAX_WEAPONS; ++i) {
            ...
            if (pOtherWeapon && pOtherWeapon != pWeapon &&
                pOtherWeapon->GetPrimaryAmmoType() == iAmmoIndex) {
              bAmmoTypeInUse = true;
              break;
            }
          }
    ...
    if (hdr && hdr->numbones() > 0) {
      ...
      for (iWeaponBoneIndex = 0; iWeaponBoneIndex < hdr->numbones();
           ++iWeaponBoneIndex) {
...
bool CCSPlayer::CanPlayerBuy(bool display) {
  ...
  if (m_bIsVIP) {
    if (display == true) ClientPrint(this, HUD_PRINTCENTER, "#VIP_cant_buy");

    return false;
  }

  if (mp->m_bCTCantBuy && (GetTeamNumber() == TEAM_CT)) {
    if (display == true) ClientPrint(this, HUD_PRINTCENTER, "#CT_cant_buy");
...
BuyResult_e CCSPlayer::AttemptToBuyAssaultSuit(void) {
  ...
  if (CSGameRules()->IsBlackMarket()) {
    iKevlarPrice = CSGameRules()->GetBlackMarketPriceForWeapon(WEAPON_KEVLAR);
    iAssaultSuitPrice =
        CSGameRules()->GetBlackMarketPriceForWeapon(WEAPON_ASSAULTSUIT);
...
// Handles the special "buy" alias commands we're creating to accommodate the
// buy scripts players use (now that we've rearranged the buy menus and broken
// the scripts)
BuyResult_e CCSPlayer::HandleCommand_Buy(const char *item) {
  char itemNameUsed[256];

  BuyResult_e result = CanPlayerBuy(false)
                           ? BUY_PLAYER_CANT_BUY
                           : BUY_INVALID_ITEM;  // set some defaults

  // translate the new weapon names to the old ones that are actually being
  // used.
  ...
  CCSWeaponInfo *pWeaponInfo =
      GetWeaponInfo((CSWeaponID)ClassnameToWeaponID(wpnName));
  if (pWeaponInfo == NULL) {
    ...
    // MIKETODO: assasination maps have a specific set of weapons that can be
    // used in them.
    if (pWeaponInfo->m_iTeam != TEAM_UNASSIGNED &&
        GetTeamNumber() != pWeaponInfo->m_iTeam) {
      result = BUY_NOT_ALLOWED;
      if (pWeaponInfo->m_WrongTeamMsg[0] != 0) {
        ClientPrint(this, HUD_PRINTCENTER, "#Alias_Not_Avail",
                    pWeaponInfo->m_WrongTeamMsg);
      }
    } else if (pWeaponInfo->GetWeaponPrice() <= 0) {
      // ClientPrint( this, HUD_PRINTCENTER, "#Cant_buy_this_item",
      // pWeaponInfo->m_WrongTeamMsg );
    ...
    } else if (!Weapon_OwnsThisType(
                   pWeaponInfo->szClassName))  // don't buy duplicate weapons
    {
...
BuyResult_e CCSPlayer::BuyGunAmmo(CBaseCombatWeapon *pWeapon,
                                  bool bBlinkMoney) {
...
BuyResult_e CCSPlayer::BuyAmmo(int nSlot, bool bBlinkMoney) {
  ...
  if (!pSlot) return BUY_INVALID_ITEM;
...
RadioType NameToRadioEvent(const char *name) {
  for (int i = 0; RadioEventName[i]; ++i)
    if (!stricmp(RadioEventName[i], name)) return static_cast<RadioType>(i);

  return RADIO_INVALID;
}

void CCSPlayer::HandleMenu_Radio1(int slot) {
  if (m_iRadioMessages < 0) return;
...
void UTIL_CSRadioMessage(IRecipientFilter &filter, int iClient, int msg_dest,
                         const char *msg_name, const char *param1 = NULL,
                         const char *param2 = NULL, const char *param3 = NULL,
  ...
  for (i = 1; i <= gpGlobals->maxClients; ++i) {
    ...
    if (player->GetTeamNumber() == TEAM_SPECTATOR) {
      ...
      if (player->m_iObserverMode == OBS_MODE_IN_EYE ||
          player->m_iObserverMode == OBS_MODE_CHASE) {
...
void CCSPlayer::Radio(const char *pszRadioSound, const char *pszRadioText) {
  ...
  if (pszRadioText) {
    ...
    if (pszLocationText && *pszLocationText) {
      UTIL_CSRadioMessage(filter, entindex(), HUD_PRINTTALK,
                          "#Game_radio_location", GetPlayerName(),
                          pszLocationText, pszRadioText);
    } else {
      UTIL_CSRadioMessage(filter, entindex(), HUD_PRINTTALK, "#Game_radio",
                          GetPlayerName(), pszRadioText);
    }
  }
...
void CCSPlayer::ListPlayers() {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pPlayer && !pPlayer->IsDormant()) {
      if (pPlayer->IsBot()) {
        Q_snprintf(buf, sizeof(buf), "B %d : %s", pPlayer->GetUserID(),
                   pPlayer->GetPlayerName());
      } else {
        Q_snprintf(buf, sizeof(buf), "  %d : %s", pPlayer->GetUserID(),
                   pPlayer->GetPlayerName());
      }
...
void CCSPlayer::ApplyDeafnessEffect() {
  // what's happening here is that the low-pass filter and the oscillator
  // frequency effects need to fade in and out slowly.  So we have several
  // filters that we switch between to achieve this effect.  The first 3rd of
  // the total effect will be the "fade in" of the effect.  Which means going
  // from filter to filter from the first to the last.  Then we keep on the
  // "last" filter for another third of the total effect time.  Then the last
  // third of the time we go back from the last filter to the first.  Clear as
  // mud?

  // glossary:
  //  filter: an individual filter as defined in dsp_presets.txt
  //  section: one of the sections for the total effect, fade in, full, fade out
  //  are the possible sections effect: the total effect of combining all the
  //  sections, the whole of what the player hears from start to finish.
  ...
  const float timeForEachFilterInFadeIn =
      fadeInSectionTime / (lastGrenadeFilterIndex - firstGrenadeFilterIndex);
  const float timeForEachFilterInFadeOut =
      fadeOutSectionTime / (lastGrenadeFilterIndex - firstGrenadeFilterIndex);
  ...
  if (timeIntoEffect < fadeInSectionTime) {
    section = 0;
  } else if (timeIntoEffect <
             (grenadeEffectLengthInSecs - fadeOutSectionTime)) {
    section = 1;
  } else {
    section = 2;
  }
  ...
  if (section == 0) {
    ...
    float timeIntoSection =
        timeIntoEffect - (grenadeEffectLengthInSecs - fadeOutSectionTime);
...
bool CCSPlayer::WantsLagCompensationOnEntity(
  ...
  if (!(pCmd->buttons & IN_ATTACK) &&
    ...
    if ((pCmd->buttons & IN_ATTACK2) == 0) return false;
...
// Handles the special "radio" alias commands we're creating to accommodate the
// scripts players use
...
bool CCSPlayer::ClientCommand(const CCommand &args) {
  ...
  /*
          if ( bot_mimic.GetInt() && !( GetFlags() & FL_FAKECLIENT ) )
          {
                  for ( int i=1; i <= gpGlobals->maxClients; i++ )
                  {
                          CCSPlayer *pPlayer = dynamic_cast< CCSPlayer* >(
     UTIL_PlayerByIndex( i ) ); if ( pPlayer && pPlayer != this && (
  ...
  */
  ...
  if (FStrEq(pcmd, "bot_cmd")) {
    CCSPlayer *pPlayer =
        dynamic_cast<CCSPlayer *>(UTIL_PlayerByIndex(atoi(args[1])));
...
bool CCSPlayer::HandleCommand_JoinTeam(int team) {
  ...
  // If this player is a VIP, don't allow him to switch teams/appearances unless
  // the following conditions are met : a) There is another TEAM_CT player who
  // is in the queue to be a VIP b) This player is dead
  ...
  */
  ...
  if (mp->TeamFull(team)) {
    ...
    bool madeRoom = false;
    if (cv_bot_auto_vacate.GetBool() && !IsBot()) {
      if (UTIL_KickBotFromTeam(team)) madeRoom = true;
    }
  ...
  if (mp->TeamStacked(team, GetTeamNumber()))  // players are allowed to change
                                               // to their own team so they can
                                               // just change their model
  {
    // attempt to kick a bot to make room for this player
    bool madeRoom = false;
    if (cv_bot_auto_vacate.GetBool() && !IsBot()) {
  ...
  // This must come before ClientKill() for CheckWinConditions() to function
  // properly
...
bool CCSPlayer::HandleCommand_JoinClass(int iClass) {
  if (iClass == CS_CLASS_NONE) {
    ...
    switch (GetTeamNumber()) {
      case TEAM_TERRORIST:
        iClass = RandomInt(FIRST_T_CLASS, LAST_T_CLASS);
        break;

      case TEAM_CT:
        iClass = RandomInt(FIRST_CT_CLASS, LAST_CT_CLASS);
        break;
  ...
  if (!IsAlive()) {
    GetIntoGame();
  } else if (!IsBot())  // Bots are responsible about only switching classes
                        // when they join.
  {
    // Kill player if switching classes while alive.
    // This mimics goldsrc CS 1.6, and prevents a player from hiding, and
    // switching classes to make the opposing team think there are more enemies
...
void CCSPlayer::GetIntoGame() {
  // Set their model and if they're allowed to spawn right now, put them into
  // the world.
  ...
  if (!MPRules->FPlayerCanRespawn(this)) {
    // This player is joining in the middle of a round or is an observer. Put
    // them directly into observer mode.
    // pev->deadflag		= DEAD_RESPAWNABLE;
    // pev->classname		= MAKE_STRING("player");
    // pev->flags		   &= ( FL_PROXY | FL_FAKECLIENT );	// clear flags, but
    // keep proxy and bot flags that might already be set  pev->flags
    // |= FL_CLIENT | FL_SPECTATOR;  SetThink(PlayerDeathThink);
    if (!(m_iDisplayHistoryBits & DHF_SPEC_DUCK)) {
      m_iDisplayHistoryBits |= DHF_SPEC_DUCK;
      HintMessage("#Spec_Duck", true, true);
    }
  ...
  } else  // else spawn them right in
  {
    ...
    if (MPRules->m_flRestartRoundTime == 0.0f) {
      ...
      if (MPRules->IsBombDefuseMap() && !MPRules->IsThereABomber() &&
          !MPRules->IsThereABomb())
        MPRules->GiveC4();  // Checks for terrorists.
    }

    // If a new terrorist is entering the fray, then up the # of potential
    // escapers.
...
bool CCSPlayer::SelectSpawnSpot(const char *pEntClassName,
                                CBaseEntity *&pSpot) {
...
void CCSPlayer::State_Enter(CSPlayerState newState) {
  ...
  if (cs_ShowStateTransitions.GetInt() == -1 ||
      cs_ShowStateTransitions.GetInt() == entindex()) {
    if (m_pCurStateInfo)
...
CCSPlayerStateInfo *CCSPlayer::State_LookupInfo(CSPlayerState state) {
  ...
  static CCSPlayerStateInfo playerStateInfos[] = {
      {STATE_ACTIVE, "STATE_ACTIVE", &CCSPlayer::State_Enter_ACTIVE, NULL,
       &CCSPlayer::State_PreThink_ACTIVE},
      {STATE_WELCOME, "STATE_WELCOME", &CCSPlayer::State_Enter_WELCOME, NULL,
       &CCSPlayer::State_PreThink_WELCOME},
      {STATE_PICKINGTEAM, "STATE_PICKINGTEAM",
       &CCSPlayer::State_Enter_PICKINGTEAM, NULL,
       &CCSPlayer::State_PreThink_OBSERVER_MODE},
      {STATE_PICKINGCLASS, "STATE_PICKINGCLASS",
       &CCSPlayer::State_Enter_PICKINGCLASS, NULL,
       &CCSPlayer::State_PreThink_OBSERVER_MODE},
      {STATE_DEATH_ANIM, "STATE_DEATH_ANIM", &CCSPlayer::State_Enter_DEATH_ANIM,
       NULL, &CCSPlayer::State_PreThink_DEATH_ANIM},
      {STATE_DEATH_WAIT_FOR_KEY, "STATE_DEATH_WAIT_FOR_KEY",
       &CCSPlayer::State_Enter_DEATH_WAIT_FOR_KEY, NULL,
...
void CCSPlayer::State_Enter_WELCOME() {
  ...
  // Important to set MOVETYPE_NONE or our physics object will fall while we're
  // sitting at one of the intro cameras.
  ...
  if (!CSGameRules()->IsLogoMap()) {
    if (CommandLine()->FindParm(
            "-makereslists"))  // don't show the MOTD when making reslists
    {
...
void CCSPlayer::State_PreThink_DEATH_ANIM() {
  // If the anim is done playing, go to the next state (waiting for a keypress
  // to either respawn the guy or put him into observer mode).
  ...
  if (gpGlobals->curtime >=
      (m_flDeathTime +
       DEATH_ANIMATION_TIME))  // allow 3 seconds death animation
  {
...
void CCSPlayer::State_PreThink_DEATH_WAIT_FOR_KEY() {
  // once we're done animating our death and we're on the ground, we want to set
  // movetype to None so our dead body won't do collisions and stuff anymore
  // this prevents a bug where the dead body would go to a player's head if he
  ...
  // after a certain amount of time switch to observer mode even if they don't
  // press a key.
...
void CCSPlayer::State_Enter_PICKINGCLASS() {
  if (CommandLine()->FindParm(
          "-makereslists"))  // don't show the menu when making reslists
  {
...
void CCSPlayer::State_PreThink_ACTIVE() {
  ...
  // It would be nice if the noclip command set some flag so we could tell that
...
bool CCSPlayer::Weapon_CanUse(CBaseCombatWeapon *pBaseWeapon) {
  ...
  if (pWeapon) {
    ...
    if (pWeapon->GetCSWpnData().m_WeaponType == WEAPONTYPE_C4 &&
...
bool CCSPlayer::BumpWeapon(CBaseCombatWeapon *pBaseWeapon) {
  ...
  // Even if we already have a grenade in this slot, we can pickup another one
  // if we don't already own this type of grenade.
  bool bPickupGrenade =
      (pWeapon->GetCSWpnData().m_WeaponType == WEAPONTYPE_GRENADE);
  ...
  if ( !bPickupGrenade && Weapon_SlotOccupied( pWeapon ) )
  {
          ...
          // Can't just check HasAnyAmmo because if I don't use clips, I want to
  be removed, if ( pWeapon->UsesClipsForAmmo1() && pWeapon->HasPrimaryAmmo() )
                  return false;
  ...
  */
  ...
  // Check ammo counts for grenades, and don't try to pick up more grenades than
  // we can carry
  if (bPickupGrenade) {
    CBaseCombatWeapon *pOwnedGrenade =
        Weapon_OwnsThisType(pWeapon->GetClassname());
...
void CCSPlayer::EmitPrivateSound(const char *soundName) {
  ...
  if (!GetParametersForSound(soundName, params, NULL)) return;
...
void CCSPlayer::AutoBuy() {
  ...
  const char *autobuyString =
      engine->GetClientConVarValue(engine->IndexOfEdict(edict()), "cl_autobuy");
...
void CCSPlayer::ParseAutoBuyString(const char *string, bool &boughtPrimary,
  ...
  while (*c != NULL) {
    ...
    if (ShouldExecuteAutoBuyCommand(commandInfo, boughtPrimary,
                                    boughtSecondary)) {
      BuyResult_e result = HandleCommand_Buy(command);

      overallResult = CombineBuyResults(overallResult, result);

      // check to see if we actually bought a primary or secondary weapon this
      // time.
...
BuyResult_e CCSPlayer::CombineBuyResults(BuyResult_e prevResult,
                                         BuyResult_e newResult) {
  if (newResult == BUY_BOUGHT) {
    prevResult = BUY_BOUGHT;
  } else if (prevResult != BUY_BOUGHT &&
             (newResult == BUY_CANT_AFFORD || newResult == BUY_INVALID_ITEM ||
              newResult == BUY_PLAYER_CANT_BUY)) {
...
//==============================================
void CCSPlayer::PostAutoBuyCommandProcessing(
    const AutoBuyInfoStruct *commandInfo, bool &boughtPrimary,
    bool &boughtSecondary) {
  if (commandInfo == NULL) {
    return;
  }
  ...
  if ((pPrimary != NULL) &&
      (stricmp(pPrimary->GetClassname(), commandInfo->m_classname) == 0)) {
    // I just bought the gun I was trying to buy.
    boughtPrimary = true;
  } else if ((pPrimary == NULL) &&
             ((commandInfo->m_class & AUTOBUYCLASS_SHIELD) ==
              AUTOBUYCLASS_SHIELD) &&
             HasShield()) {
    // the shield is a primary weapon even though it isn't a "real" weapon.
    boughtPrimary = true;
  } else if ((pSecondary != NULL) && (stricmp(pSecondary->GetClassname(),
                                              commandInfo->m_classname) == 0)) {
    // I just bought the pistol I was trying to buy.
    boughtSecondary = true;
  }
}

bool CCSPlayer::ShouldExecuteAutoBuyCommand(
    const AutoBuyInfoStruct *commandInfo, bool boughtPrimary,
    bool boughtSecondary) {
  if (commandInfo == NULL) {
    return false;
  ...
  if ((boughtPrimary) && ((commandInfo->m_class & AUTOBUYCLASS_PRIMARY) != 0) &&
      ((commandInfo->m_class & AUTOBUYCLASS_AMMO) == 0)) {
    // this is a primary weapon and we already have one.
    return false;
  }

  if ((boughtSecondary) &&
      ((commandInfo->m_class & AUTOBUYCLASS_SECONDARY) != 0) &&
      ((commandInfo->m_class & AUTOBUYCLASS_AMMO) == 0)) {
...
//==============================================
// PostAutoBuyCommandProcessing
//- reorders the tokens in autobuyString based on the order of tokens in the
// priorityString.
//==============================================
void CCSPlayer::PrioritizeAutoBuyString(char *autobuyString,
  ...
  // now just copy anything left in the autobuyString to the new string in the
  // order it's in already.
...
void CCSPlayer::BuildRebuyStruct() {
  ...
  if (primary == NULL) {
    ...
    if (HasShield()) {
      ...
      Q_strncpy(m_rebuyStruct.m_szPrimaryWeapon, "shield",
                sizeof(m_rebuyStruct.m_szPrimaryWeapon));
      m_rebuyStruct.m_primaryAmmo = 0;  // shields don't have ammo.
    } else {
      m_rebuyStruct.m_szPrimaryWeapon[0] = 0;  // if we don't have a shield and
                                               // we don't have a primary
                                               // weapon, we got nuthin.
      m_rebuyStruct.m_primaryAmmo =
          0;  // can't have ammo if we don't have a gun right?
    }
  } else {
    ...
    Q_strncpy(m_rebuyStruct.m_szPrimaryWeapon, wpnName + 7,
              sizeof(m_rebuyStruct.m_szPrimaryWeapon));
  ...
  if (secondary == NULL) {
    ...
    m_rebuyStruct.m_secondaryAmmo =
        0;  // can't have ammo if we don't have a gun right?
  } else {
    const char *wpnName = secondary->GetClassname();

    Q_strncpy(m_rebuyStruct.m_szSecondaryWeapon, wpnName + 7,
              sizeof(m_rebuyStruct.m_szSecondaryWeapon));

    if (secondary->GetPrimaryAmmoType() != -1) {
      m_rebuyStruct.m_secondaryAmmo =
          GetAmmoCount(secondary->GetPrimaryAmmoType());
    }
  }
  ...
  if (pGrenade /*&& pGrenade->GetPrimaryAmmoType() != -1*/) {
    m_rebuyStruct.m_smokeGrenade =
        1;  // GetAmmoCount(pGrenade->GetPrimaryAmmoType());
  } else
  ...
  // night vision
  m_rebuyStruct.m_nightVision =
      m_bHasNightVision.Get();  // cast to avoid strange compiler warning
...
void CCSPlayer::Rebuy(void) {
  ...
  const char *rebuyString =
      engine->GetClientConVarValue(engine->IndexOfEdict(edict()), "cl_rebuy");
  ...
  // after we're done buying, the user is done with their equipment purchasing
  // experience. so we are effectively out of the buy zone.
  //	if (TheTutor != NULL)
  //	{
...
BuyResult_e CCSPlayer::RebuyPrimaryWeapon() {
  CBaseCombatWeapon *primary = Weapon_GetSlot(WEAPON_SLOT_RIFLE);
  if (primary != NULL) {
    return BUY_ALREADY_HAVE;  // don't drop primary weapons via rebuy - if the
                              // player picked up a different weapon, he wants
                              // to keep it.
  }
...
BuyResult_e CCSPlayer::RebuySecondaryWeapon() {
  ...
  if (pistol != NULL && !m_bUsingDefaultPistol) {
    return BUY_ALREADY_HAVE;  // don't drop pistols via rebuy if we've bought
                              // one other than the default pistol
  }
...
bool CCSPlayer::IsUseableEntity(CBaseEntity *pEntity,
  ...
  CBaseCSGrenadeProjectile *pGrenade =
      dynamic_cast<CBaseCSGrenadeProjectile *>(pEntity);
  if (pGrenade) {
...
CBaseEntity *CCSPlayer::FindUseEntity() {
  ...
  if (entity == NULL) {
    // If we didn't find anything, check to see if the bomb is close enough to
    // use. This is done separately since there might be something blocking our
    // LOS to it but we might want to use it anyway if it's close enough.  This
    // should eliminate the vast majority of bomb placement exploits (places
    // where the bomb can be planted but can't be "used".  This also mimics
    // goldsrc cstrike behavior.

    if (CSGameRules()->IsBombDefuseMap()) {
      CBaseEntity *bomb =
          gEntList.FindEntityByClassname(NULL, PLANTED_C4_CLASSNAME);
      if (bomb != NULL) {
        Vector bombPos = bomb->GetAbsOrigin();
        Vector vecLOS = EyePosition() - bombPos;

        if (vecLOS.LengthSqr() < (96 * 96))  // 64 is the distance in Goldsrc.
                                             // However since Goldsrc did
                                             // distance from the player's
                                             // origin and we're doing distance
                                             // from the player's eye, make the
                                             // radius a bit bigger.
        {
...
void CCSPlayer::StockPlayerAmmo(CBaseCombatWeapon *pNewWeapon) {
  ...
  if (pWeapon) {
    if (pWeapon->GetWpnData().iFlags & ITEM_FLAG_EXHAUSTIBLE) return;
...
CBaseEntity *CCSPlayer::GiveNamedItem(const char *pszName, int iSubType) {
  ...
  if (!Q_stricmp(pszName, "weapon_shield")) return NULL;
#endif
...
void CCSPlayer::DoAnimationEvent(PlayerAnimEvent_t event, int nData) {
  if (event == PLAYERANIMEVENT_THROW_GRENADE) {
    // Grenade throwing has to synchronize exactly with the player's grenade
    // weapon going away, and events get delayed a bit, so we let
    // CCSPlayerAnimState pickup the change to this variable.
    m_iThrowGrenadeCounter =
        (m_iThrowGrenadeCounter + 1) % (1 << THROWGRENADE_COUNTER_BITS);
  } else {
...
void CCSPlayer::DropWeapons(void) {
  ...
  if (HasDefuser()) {
    ...
    CBaseAnimating *pDefuser = (CBaseAnimating *)CBaseEntity::Create(
        "item_defuser", WorldSpaceCenter(), GetLocalAngles(), this);
    pDefuser->ApplyAbsVelocityImpulse(vForward * 200 +
                                      vRight * random->RandomFloat(-50, 50));
  ...
  if (HasShield()) {
    ...
    if (!DropRifle()) DropPistol();
  }

  // drop any live grenades so they explode
  CBaseCSGrenade *pGrenade =
      dynamic_cast<CBaseCSGrenade *>(Weapon_OwnsThisType("weapon_hegrenade"));
  if (pGrenade && (pGrenade->IsPinPulled() || pGrenade->IsBeingThrown())) {
    pGrenade->DropGrenade();
    pGrenade->DecrementAmmo(this);
  } else {
    pGrenade =
        dynamic_cast<CBaseCSGrenade *>(Weapon_OwnsThisType("weapon_flashbang"));
    if (pGrenade && (pGrenade->IsPinPulled() || pGrenade->IsBeingThrown())) {
      pGrenade->DropGrenade();
      pGrenade->DecrementAmmo(this);
    } else {
      pGrenade = dynamic_cast<CBaseCSGrenade *>(
          Weapon_OwnsThisType("weapon_smokegrenade"));
      if (pGrenade && (pGrenade->IsPinPulled() || pGrenade->IsBeingThrown())) {
        pGrenade->DropGrenade();
        pGrenade->DecrementAmmo(this);
      }
...
void CCSPlayer::ChangeTeam(int iTeamNum) {
  ...
  // Always allow a change to spectator, and don't count it as one of our team
  // changes. We now store the old team, so if a player changes once to one
  // team, then to spectator, they won't be able to change back to their old old
  // team, but will still be able to join the team they initially changed to.
  ...
  } else  // active player
  {
    if (iOldTeam == TEAM_SPECTATOR) {
      ...
      m_iAccount = CSGameRules()->GetStartMoney();  // all players start with
                                                    // "mp_startmoney" bucks
    }

    // bots get to this state on TEAM_UNASSIGNED, yet they are marked alive.
    // Don't kill them.
  ...
  CSGameRules()->InitializePlayerCounts(NumAliveTerrorist, NumAliveCT,
                                        NumDeadTerrorist, NumDeadCT);
}
...
void CCSPlayer::SwitchTeam(int iTeamNum) {
  if (!GetGlobalTeam(iTeamNum) ||
      (iTeamNum != TEAM_CT && iTeamNum != TEAM_TERRORIST)) {
...
void CCSPlayer::RecordDamageGiven(const char *szDamageTaker, int iDamageGiven) {
  FOR_EACH_LL(m_DamageGivenList, i) {
    if (Q_strncmp(szDamageTaker, m_DamageGivenList[i]->GetPlayerName(),
  ...
  CDamageRecord *record =
      new CDamageRecord(szDamageTaker, iDamageGiven, s_BulletGroupCounter);
...
//=======================================================
void CCSPlayer::RecordDamageTaken(const char *szDamageDealer,
                                  int iDamageTaken) {
  FOR_EACH_LL(m_DamageTakenList, i) {
    if (Q_strncmp(szDamageDealer, m_DamageTakenList[i]->GetPlayerName(),
                  MAX_PLAYER_NAME_LENGTH) == 0) {
  ...
  CDamageRecord *record =
      new CDamageRecord(szDamageDealer, iDamageTaken, s_BulletGroupCounter);
  ...
  FOR_EACH_LL(m_DamageTakenList, i) {
    ...
    if (pRecord) {
      if (pRecord->GetNumHits() == 1) {
        Q_snprintf(buf, sizeof(buf), "%d in %d hit", pRecord->GetDamage(),
                   pRecord->GetNumHits());
      } else {
        Q_snprintf(buf, sizeof(buf), "%d in %d hits", pRecord->GetDamage(),
                   pRecord->GetNumHits());
      }
...
int CCSPlayer::GetNextObserverSearchStartPoint(bool bReverse) {
  // If we are currently watching someone who is dead, they must have died while
  // we were watching (since a dead guy is not a valid pick to start watching).
  // He was given his killer as an observer target when he died, so let's start
  // by trying to observe his killer.  If we fail, we'll use the normal way. And
  // this is just the start point anyway, but we want to start the search here
  // in case it is okay.
...
void CCSPlayer::PlayStepSound(Vector &vecOrigin, surfacedata_t *psurface,
...
void CCSPlayer::SelectDeathPose(const CTakeDamageInfo &info) {
  ...
  SelectDeathPoseActivityAndFrame(this, info, m_LastHitGroup, aActivity,
                                  iDeathFrame);
...
void CCSPlayer::HandleAnimEvent(animevent_t *pEvent) {
  if (pEvent->event == 4001 || pEvent->event == 4002) {
    // Ignore these for now - soon we will be playing footstep sounds based on
    // these events that mark footfalls in the anims.
  } else {
...
bool CCSPlayer::CanChangeName(void) {
  ...
  if ((m_flNameChangeHistory[0] + MIN_NAME_CHANGE_INTERVAL) >=
      gpGlobals->curtime) {
    return false;
  }
  ...
  if ((m_flNameChangeHistory[NAME_CHANGE_HISTORY_SIZE - 1] +
       NAME_CHANGE_HISTORY_INTERVAL) >= gpGlobals->curtime) {
    return false;
  }
...
void CCSPlayer::ChangeName(const char *pszNewName) {
  ...
  UTIL_SayText2Filter(filter, this, false, "#Cstrike_Name_Change", pszOldName,
                      trimmedName);
...
bool CCSPlayer::StartReplayMode(float fDelay, float fDuration, int iEntity) {
  if (!BaseClass::StartReplayMode(fDelay, fDuration, iEntity)) return false;
...
void CCSPlayer::PlayUseDenySound() {
  // Don't do a sound here because it can mute your footsteps giving you an
  // advantage. The CS:S content for this sound is silent anyways.
