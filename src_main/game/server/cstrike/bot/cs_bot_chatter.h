...
#define COUNT_CURRENT_ENEMIES \
  0xFF                // use the number of enemies we see right when we speak
...
class BotMeme {
 public:
  ...
  virtual void Interpret(CCSBot *sender, CCSBot *receiver)
      const = 0;  ///< cause the given bot to act on this meme
};

//----------------------------------------------------------------------------------------------------
class BotHelpMeme : public BotMeme {
 public:
  BotHelpMeme(Place place = UNDEFINED_PLACE) { m_place = place; }

  virtual void Interpret(CCSBot *sender, CCSBot *receiver)
      const;  ///< cause the given bot to act on this meme
...
class BotBombsiteStatusMeme : public BotMeme {
 ...
 private:
  ...
  StatusType
      m_status;  ///< whether it is cleared or the bomb is there (planted)
};
...
class BotDefendHereMeme : public BotMeme {
 public:
  BotDefendHereMeme(const Vector &pos) { m_pos = pos; }
 ...
 * They represent a specific wav file of a phrase, and the criteria for which it
 * is useful
 */
class BotSpeakable {
 public:
...
//----------------------------------------------------------------------------------------------------
/**
 * The BotPhrase class is a collection of Speakables associated with a name, ID,
 * and criteria
 */
class BotPhrase {
 public:
  char *GetSpeakable(int bankIndex,
                     float *duration = NULL) const;  ///< return a random
                                                     ///< speakable and its
                                                     ///< duration in seconds
                                                     ///< that meets the current
                                                     ///< criteria

  // NOTE: Criteria must be set just before the GetSpeakable() call, since they
  // are shared among all bots
  void ClearCriteria(void) const;
  void SetPlaceCriteria(PlaceCriteria place)
      const;  ///< all returned phrases must have this place criteria
  void SetCountCriteria(CountCriteria count)
      const;  ///< all returned phrases must have this count criteria

  const char *GetName(void) const { return m_name; }
  const unsigned int GetPlace(void) const { return m_place; }
  RadioType GetRadioEquivalent(void) const {
    return m_radioEvent;
  }  ///< return equivalent "standard radio" event
  bool IsImportant(void) const {
    return m_isImportant;
  }  ///< return true if this phrase is part of an important statement
 ...
 private:
  ...
  mutable BotVoiceBankVector
      m_voiceBank;          ///< array of voice banks (arrays of speakables)
  ...
  void InitVoiceBank(int bankIndex);  ///< sets up the vector of voice banks for
                                      ///< the first bankIndex voice banks
...
enum BotChatterOutputType { BOT_CHATTER_RADIO, BOT_CHATTER_VOICE };
typedef CUtlVector<BotChatterOutputType> BotOutputList;

//----------------------------------------------------------------------------------------------------
/**
 * The BotPhraseManager is a singleton that provides an interface to all
 * BotPhrase collections
 */
class BotPhraseManager {
 public:
  ...
  bool Initialize(const char *filename,
                  int bankIndex);  ///< initialize phrase system from database
                                   ///< file for a specific voice bank (0 is the
                                   ///< default voice bank)
  ...
  const BotPhrase *GetPhrase(const char *name)
      const;  ///< given a name, return the associated phrase collection
  const BotPhrase *GetPainPhrase(void) const {
    return m_painPhrase;
  }  ///< optimization, replaces a static pointer to the phrase
  const BotPhrase *GetAgreeWithPlanPhrase(void) const {
    return m_agreeWithPlanPhrase;
  }  ///< optimization, replaces a static pointer to the phrase

  const BotPhrase *GetPlace(const char *name)
      const;  ///< given a name, return the associated Place phrase collection
  const BotPhrase *GetPlace(unsigned int id)
      const;  ///< given an id, return the associated Place phrase collection

  const BotPhraseList *GetPlaceList(void) const { return &m_placeList; }

  float GetPlaceStatementInterval(Place where) const;  ///< return time last
                                                       ///< statement of given
                                                       ///< type was emitted by
                                                       ///< a teammate for the
                                                       ///< given place
  void ResetPlaceStatementInterval(Place where);       ///< set time of last
                                                  ///< statement of given type
                                                  ///< was emitted by a teammate
                                                  ///< for the given place
...
inline int BotPhraseManager::FindPlaceIndex(Place where) const {
  for (int i = 0; i < m_placeCount; ++i)
    if (m_placeStatementHistory[i].placeID == where) return i;
...
/**
 * Return time last statement of given type was emitted by a teammate for the
 * given place
 */
...
/**
 * Set time of last statement of given type was emitted by a teammate for the
 * given place
 */
...
class BotStatement {
 public:
  BotStatement(BotChatterInterface *chatter, BotStatementType type,
  ...
  BotStatementType GetType(void) const {
    return m_type;
  }                              ///< return the type of statement this is
  bool IsImportant(void) const;  ///< return true if this statement is
                                 ///< "important" and not personality chatter

  bool HasSubject(void) const {
    return (m_subject == UNDEFINED_SUBJECT) ? false : true;
  }
  ...
  Place GetPlace(void) const;  ///< if this statement refers to a specific
                               ///< place, return that place
  void SetPlace(Place where) { m_place = where; }  ///< explicitly set place

  bool HasCount(
      void) const;  ///< return true if this statement has an associated count

  bool IsRedundant(const BotStatement *say)
      const;  ///< return true if this statement is the same as the given one
  bool IsObsolete(void)
      const;  ///< return true if this statement is no longer appropriate to say
  void Convert(const BotStatement *say);  ///< possibly change what were going
                                          ///< to say base on what teammate is
                                          ///< saying
  ...
  }  ///< define the earliest time this statement can be spoken
  ...
  void AddCondition(ConditionType condition);  ///< conditions must be true for
                                               ///< the statement to be spoken
  ...
  void AppendPhrase(ContextType contextPhrase);  ///< special phrases that
                                                 ///< depend on the context

  bool Update(
      void);  ///< emit statement over time, return false if statement is done
  bool IsSpeaking(void) const {
    return m_isSpeaking;
  }  ///< return true if this statement is currently being spoken
  float GetTimestamp(void) const {
    return m_timestamp;
  }  ///< get time statement was created (but not necessarily started talking)

  void AttachMeme(BotMeme *meme);  ///< attach a meme to this statement, to be
                                   ///< transmitted to other friendly bots when
                                   ///< spoken
 ...
 private:
  ...
  BotChatterInterface
      *m_chatter;  ///< the chatter system this statement is part of
  ...
  Place m_place;    ///< explicit place - note some phrases have implicit places
                    ///< as well
  ...
  ConditionType m_condition[MAX_BOT_CONDITIONS];  ///< conditions that must be
                                                  ///< true for the statement to
                                                  ///< be said
  int m_conditionCount;

  int m_index;  ///< m_index refers to the phrase currently being spoken, or -1
...
class BotChatterInterface {
 public:
  ...
  VerbosityType GetVerbosity(
      void) const;  ///< return our current level of verbosity

  CCSBot *GetOwner(void) const { return m_me; }

  bool IsTalking(void) const;  ///< return true if we are currently talking
  float GetRadioSilenceDuration(
      void);  ///< return time since any teammate said anything
  void ResetRadioSilenceDuration(void);

  enum { MUST_ADD = 1 };
  void AddStatement(
      BotStatement *statement,
      bool mustAdd = false);  ///< register a statement for speaking
  void RemoveStatement(BotStatement *statement);  ///< remove a statement

  BotStatement *GetActiveStatement(void);  ///< returns the statement that is
                                           ///< being spoken, or is next to be
                                           ///< spoken if no-one is speaking now
  BotStatement *GetStatement(void)
      const;  ///< returns our current statement, or NULL if we aren't speaking
...
#define IS_PLAN true
  ...
  void Encourage(
      const char *phraseName, float repeatInterval = 10.0f,
 ...
 private:
  BotStatement
      *m_statementList;  ///< list of all active/pending messages for this bot

  void ReportEnemies(void);  ///< track nearby enemy count and generate enemy
                             ///< activity statements
  ...
  bool m_requestedBombLocation;  ///< true if we already asked where the bomb
                                 ///< has been planted
  ...
  static CountdownTimer m_encourageTimer;  ///< timer to know when we can
                                           ///< "encourage" the human player
                                           ///< again - shared by all bots
};

inline BotChatterInterface::VerbosityType BotChatterInterface::GetVerbosity(
    void) const {
...
inline bool BotChatterInterface::IsTalking(void) const {
  if (m_statementList) return m_statementList->IsSpeaking();
...
inline void BotChatterInterface::Say(const char *phraseName, float lifetime,
                                     float delay) {
