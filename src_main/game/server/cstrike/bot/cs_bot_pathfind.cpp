...
//--------------------------------------------------------------------------------------------------------------
/**
 * Finds a point from which we can approach a descending ladder.  First it tries
 * behind the ladder, then in front of ladder, based on LOS.  Once we know the
 * direction, we snap to the aproaching nav area.  Returns true if we're
 * approaching from behind the ladder.
 */
static bool FindDescendingLadderApproachPoint(const CNavLadder *ladder,
                                              const CNavArea *area,
                                              Vector *pos) {
  *pos = ladder->m_top - ladder->GetNormal() * 2.0f * HalfHumanWidth;

  trace_t result;
  UTIL_TraceLine(ladder->m_top, *pos, MASK_PLAYERSOLID_BRUSHONLY, NULL,
...
bool CCSBot::ComputePathPositions(void) {
  ...
  for (int i = 1; i < m_pathLength; ++i) {
    ...
    if (to->how <= GO_WEST)  // walk along the floor to the next area
    {
      ...
      from->area->ComputeClosestPointInPortal(to->area, (NavDirType)to->how,
                                              from->pos, &to->pos);

      // move goal position into the goal area a bit
      const float stepInDist = 5.0f;  // how far to "step into" an area - must
                                      // be less than min area size
      ...
      // if this is a "jump down" connection, we must insert an additional point
      // on the path
      if (to->area->IsConnected(from->area, NUM_DIRECTIONS) == false) {
        ...
        if (m_pathLength < MAX_PATH_LENGTH - 1) {
          // copy nodes down
          for (int j = m_pathLength; j > i; --j) m_path[j] = m_path[j - 1];
          ...
          m_path[i].pos.z = to->area->GetZ(m_path[i].pos);
               ...
               GO_LADDER_UP)  // to get to next area, must go up a ladder
    {
      // find our ladder
      const NavLadderConnectList *list =
          from->area->GetLadderList(CNavLadder::LADDER_UP);
      int it;
      for (it = list->Head(); it != list->InvalidIndex(); it = list->Next(it)) {
        ...
        if (ladder->m_topForwardArea == to->area ||
          ...
          to->ladder = ladder;
          to->pos =
              ladder->m_bottom + ladder->GetNormal() * 2.0f * HalfHumanWidth;
          break;
        }
      }
      ...
      if (it == list->InvalidIndex()) {
        ...
        return false;
               ...
               GO_LADDER_DOWN)  // to get to next area, must go down a ladder
    {
      // find our ladder
      const NavLadderConnectList *list =
          from->area->GetLadderList(CNavLadder::LADDER_DOWN);
      int it;
      for (it = list->Head(); it != list->InvalidIndex(); it = list->Next(it)) {
        CNavLadder *ladder = (*list)[it].ladder;
  ...
  if (to->ladder) {
    ...
    if (to->how == GO_LADDER_UP) {
      ...
      m_goalPosition = m_pathLadder->m_bottom +
                       m_pathLadder->GetNormal() * 2.0f * HalfHumanWidth;
      m_lookAheadAngle = AngleNormalizePositive(ladderAngles[YAW] + 180.0f);
    } else {
      // try to mount ladder "face out" first
      bool behind = FindDescendingLadderApproachPoint(m_pathLadder, from->area,
                                                      &m_goalPosition);
...
void CCSBot::ComputeLadderEndpoint(bool isAscending) {
  ...
  UTIL_TraceLine(from, m_pathLadder->m_bottom, MASK_PLAYERSOLID_BRUSHONLY, NULL,
                 COLLISION_GROUP_NONE, &result);
...
bool CCSBot::UpdateLadderMovement(void) {
  ...
  if (gpGlobals->curtime - m_pathLadderTimestamp > ladderTimeoutDuration &&
      !cv_bot_debug.GetBool()) {
  ...
  switch (m_pathLadderState) {
    ...
    case DESCEND_LADDER: {
      ...
      const Vector &ladderPos = (m_pathLadderState == MOUNT_ASCENDING_LADDER ||
                                 m_pathLadderState == ASCEND_LADDER)
                                    ? m_pathLadder->m_bottom
                                    : m_pathLadder->m_top;
      if ((ladderPos.AsVector2D() - myOrigin.AsVector2D())
              .IsLengthGreaterThan(farAway)) {
  ...
  switch (m_pathLadderState) {
    case APPROACH_ASCENDING_LADDER: {
      ...
      if (d.x * m_pathLadder->GetNormal().x +
              d.y * m_pathLadder->GetNormal().y <
          0.0f) {
        Vector2D perp(-m_pathLadder->GetNormal().y,
                      m_pathLadder->GetNormal().x);

        if (fabs(d.x * perp.x + d.y * perp.y) < tolerance &&
            d.Length() < closeToGoal)
          approached = true;
      }

      // small radius will just slow them down a little for more accuracy in
      // hitting their spot
      ...
      if (d.IsLengthLessThan(100.0f)) {
        if (!IsOnLadder() &&
            (m_pathLadder->m_bottom.z - GetAbsOrigin().z > JumpCrouchHeight)) {
    ...
    case APPROACH_DESCENDING_LADDER: {
      ...
      if (GetFeetZ() <= m_pathLadder->m_bottom.z + HalfHumanHeight) {
        ...
        m_pathLadderState = MOVE_TO_DESTINATION;
        m_path[m_pathIndex].area->GetClosestPointOnArea(m_pathLadder->m_bottom,
                                                        &m_goalPosition);
        ...
        Vector2D d(myOrigin.x - m_goalPosition.x,
                   myOrigin.y - m_goalPosition.y);
        ...
        if (m_pathLadder->m_topBehindArea != m_lastKnownArea) {
          ...
          if (!IsCrouching() && d.IsLengthLessThan(walkRange)) Walk();
        }
    ...
    case FACE_ASCENDING_LADDER: {
      ...
      if (AnglesAreEqual(EyeAngles().y, idealAngle.y, angleTolerance)) {
        ...
        switch (m_pathLadderDismountDir) {
          case LEFT:
            PrintIfWatched("MOUNT_ASCENDING_LADDER LEFT\n");
            break;
          case RIGHT:
            PrintIfWatched("MOUNT_ASCENDING_LADDER RIGHT\n");
            break;
          default:
            PrintIfWatched("MOUNT_ASCENDING_LADDER FORWARD\n");
            break;
        }
      }
      break;
    ...
    case ASCEND_LADDER:
      ...
      if (m_path[m_pathIndex].area->GetAttributes() & NAV_MESH_CROUCH) Crouch();
    ...
    case DISMOUNT_ASCENDING_LADDER: {
      if (gpGlobals->curtime - m_pathLadderDismountTimestamp >= 0.4f) {
        m_pathLadderState = MOVE_TO_DESTINATION;
        m_path[m_pathIndex].area->GetClosestPointOnArea(myOrigin,
                                                        &m_goalPosition);
        PrintIfWatched("MOVE_TO_DESTINATION\n");
      }
 ...
 * NOTE: This does not do line-of-sight tests, so closest point may be thru the
 * floor, etc
 */
bool CCSBot::FindClosestPointOnPath(const Vector &worldPos, int startIndex,
                                    int endIndex, Vector *close) const {
  if (!HasPath() || close == NULL) return false;
 ...
 * If "local" is true, only check the portion of the path surrounding
 * m_pathIndex.
 */
int CCSBot::FindOurPositionOnPath(Vector *close, bool local) const {
  ...
  Vector eyes =
      feet + Vector(0, 0, HalfHumanHeight);  // in case we're crouching
  Vector pos;
  const Vector *from, *to;
  float length;
  float closeLength;
  ...
  if (local) {
    ...
    if (end > m_pathLength) end = m_pathLength;
  } else {
    start = 1;
    end = m_pathLength;
  }
  ...
  for (int i = start; i < end; ++i) {
    ...
    if (distSq < closeDistSq) {
      ...
      if (!IsWalkableTraceLineClear(eyes, probe,
                                    WALK_THRU_DOORS | WALK_THRU_BREAKABLES))
        continue;

      // don't use points we cant reach
      if (!IsStraightLinePathWalkable(pos)) continue;
...
bool CCSBot::IsStraightLinePathWalkable(const Vector &goal) const {
  // this is causing hang-up problems when crawling thru ducts/windows that drop
  // off into rooms (they fail the "falling" check)
  return true;
  ...
  while (!done) {
    ...
    if (!TheNavMesh->GetSimpleGroundHeight(pos, &ground)) return false;

    // check for falling
    if (ground - lastGround < -StepHeight) return false;

    // check for unreachable jump
    // use slightly shorter jump limit, to allow for some fudge room
    if (ground - lastGround > JumpHeight) return false;
...
int CCSBot::FindPathPoint(float aheadRange, Vector *point, int *prevIndex) {
  ...
  // finds the closest point on local area of path, and returns the path index
  // just prior to it
  Vector close;
  ...
  if (IsCrouching()) {
    // we want to move to the immediately next point along the path from where
    // we are now
    int index = startIndex + 1;
    if (index >= m_pathLength) index = m_pathLength - 1;

    *point = m_path[index].pos;

    // if we are very close to the next point in the path, skip ahead to the
    // next one to avoid wiggling we must do a 2D check here, in case the goal
    // point is floating in space due to jump down, etc
  ...
  while (startIndex < m_pathLength - 1) {
    ...
    // we must do a 2D check here, in case the goal point is floating in space
    // due to jump down, etc
  ...
  // if we hit a ladder, stop, or jump area, must stop (dont use ladder behind
  // us)
  if (startIndex > m_pathIndex && startIndex < m_pathLength &&
      (m_path[startIndex].ladder || m_path[startIndex].area->GetAttributes() &
                                        (NAV_MESH_JUMP | NAV_MESH_STOP))) {
  ...
  ++startIndex;
  if (startIndex >= m_pathLength) startIndex = m_pathLength - 1;
  ...
  for (i = startIndex; i < m_pathLength; ++i) {
    ...
    // don't allow path to double-back from our starting direction (going
    // upstairs, down curved passages, etc)
    ...
    // if the path turns a corner, we want to move towards the corner, not into
    // the wall/stairs/etc
  ...
  if (afterIndex == 0) {
    ...
    *point = *beforePoint + t * to;
    ...
    // if afterPoint wasn't visible, slide point backwards towards beforePoint
    // until it is
    if (!visible) {
      ...
      while (t > 0.0f &&
             !IsWalkableTraceLineClear(eyes, probe, WALK_THRU_BREAKABLES)) {
        t -= dt;
        *point = *beforePoint + t * to;
      }
  ...
  // if position found is too close to us, or behind us, force it farther down
  // the path so we don't stop and wiggle
  if (!isCorner) {
    const float epsilon = 50.0f;
    Vector2D toPoint;
    toPoint.x = point->x - myOrigin.x;
    toPoint.y = point->y - myOrigin.y;
    if (DotProduct2D(toPoint, initDir.AsVector2D()) < 0.0f ||
        toPoint.IsLengthLessThan(epsilon)) {
      int i;
      for (i = startIndex; i < m_pathLength; ++i) {
        ...
        if (m_path[i].ladder ||
            ...
            toPoint.IsLengthGreaterThan(epsilon)) {
          *point = m_path[i].pos;
          startIndex = i;
          break;
        }
  ...
  // m_pathIndex should always be the next point on the path, even if we're not
...
void CCSBot::SetPathIndex(int newIndex) {
  ...
  if (m_path[m_pathIndex].ladder) {
    ...
    if (m_pathIndex < m_pathLength && m_pathIndex >= 2)
      m_spotEncounter = m_path[m_pathIndex - 1].area->GetSpotEncounter(
          m_path[m_pathIndex - 2].area, m_path[m_pathIndex].area);
    else
      m_spotEncounter = NULL;
...
bool CCSBot::IsNearJump(void) const {
  if (m_pathIndex == 0 || m_pathIndex >= m_pathLength) return false;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return approximately how much damage will will take from the given fall
 * height
 */
...
bool CCSBot::IsFriendInTheWay(const Vector &goalPos) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; ++i) {
    ...
    if (toFriend.IsLengthGreaterThan(personalSpace)) continue;
    ...
    if (friendDistAlong <= 0.0f) continue;
    ...
    if ((pos - GetCentroid(player)).IsLengthLessThan(friendRadius)) {
      // friend is in our personal space and overlaps our intended line of
      // movement
...
void CCSBot::FeelerReflexAdjustment(Vector *goalPosition) {
  ...
  const float feelerLengthRun =
      50.0f;  // 100 - too long for tight hallways (cs_747)
  const float feelerLengthWalk = 30.0f;
  const float feelerHeight =
      StepHeight +
      0.1f;  // if obstacle is lower than StepHeight, we'll walk right over it
  ...
  if (GetSimpleGroundHeightWithFloor(eye, &ground, &normal) == false) return;
  ...
  bool leftClear = IsWalkableTraceLineClear(
      from, to, WALK_THRU_DOORS | WALK_THRU_BREAKABLES);

  // avoid ledges, too
  // use 'from' so it doesn't interfere with legitimate gap jumping (its at our
  // feet)
  /// @todo Rethink this - it causes lots of wiggling when bots jump down from
  /// vents, etc
  /*
          float ground;
  ...
  */
  ...
  bool rightClear = IsWalkableTraceLineClear(
      from, to, WALK_THRU_DOORS | WALK_THRU_BREAKABLES);
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Allows the current nav area to make us run/walk without messing with our
 * state
 */
...
CCSBot::PathResult CCSBot::UpdatePathMovement(bool allowSpeedChange) {
  ...
  //
  // If we are navigating a ladder, it overrides all other path movement until
  // complete
  //
  if (UpdateLadderMovement()) return PROGRESSING;

  // ladder failure can destroy the path
  if (m_pathLength == 0) return PATH_FAILURE;

  // we are not supposed to be on a ladder - if we are, jump off
  if (IsOnLadder()) Jump(MUST_JUMP);
  ...
  if (!IsUsingLadder()) {
    ...
    // if the m_isStopping flag is set and movement is stopped, clear
    // m_isStopping
  ...
  if (m_pathIndex >= m_pathLength - 1) {
    ...
    Vector d =
        toEnd;  // can't use 2D because path end may be below us (jump down)
    ...
    if (d.IsLengthLessThan(walkRange)) {
      ...
      if (allowSpeedChange && !IsCrouching()) Walk();

      // note if we are near the end of the path
      const float nearEndRange = 50.0f;
      if (d.IsLengthLessThan(nearEndRange)) nearEndOfPath = true;
      ...
      if (d.IsLengthLessThan(closeEpsilon)) {
        ...
        /// @todo We should push and pop walk state here, in case we want to
        /// continue walking after reaching goal
        if (allowSpeedChange) Run();
  ...
  // if goal position is near to us, we must be about to go around a corner - so
  // look ahead!
  Vector myOrigin = GetCentroid(this);
  const float nearCornerRange = 100.0f;
  if (m_pathIndex < m_pathLength - 1 &&
      (m_goalPosition - myOrigin).IsLengthLessThan(nearCornerRange)) {
  ...
  if (!IsUsingLadder()) {
    ...
    for (int i = prevIndex; i < m_pathLength; ++i) {
      ...
      // if there is a jump area on the way to the crouch area, don't crouch as
      // it messes up the jump unless we are already higher than the jump area -
      // we must've jumped already but not moved into next area
  ...
  // if we are crouching, look towards where we are moving to negotiate tight
  // corners
  if (IsCrouching()) {
    ...
    if (m_pathIndex == 0) {
      ...
      if (!m_path[m_pathIndex].ladder && !IsNearJump() &&
        ...
        for (i = m_pathIndex + 1; i < m_pathLength; ++i) {
          ...
          if (along + segmentLength >= lookAheadRange) {
            // interpolate between points to keep look ahead point at fixed
            // distance
        ...
        if (i == m_pathLength) toGoal = GetPathEndpoint() - myOrigin;
      }
    } else {
  ...
  // Feelers come from our ankles, just above StepHeight, so we avoid short
  // walls, too Don't use feelers if very near the end of the path, or about to
  // jump
  //
  /// @todo Consider having feelers at several heights to deal with overhangs,
  /// etc.
  ...
  if (cv_bot_traceview.GetInt() == 1 && IsLocalPlayerWatchingMe() ||
    ...
    UTIL_DrawBeamPoints(adjustedGoal, adjustedGoal + Vector(0, 0, 50), 1, 255,
                        0, 255);
    UTIL_DrawBeamPoints(myOrigin, adjustedGoal + Vector(0, 0, 50), 1, 255, 0,
                        255);
  }
  ...
  //
  if (!isWaitingForLadder &&
      (!m_isWaitingBehindFriend || m_politeTimer.IsElapsed())) {
    //
    // Move along path
    //
  ...
  if (m_goalPosition.z - GetFeetZ() > JumpCrouchHeight) {
    ...
    if (to.IsLengthLessThan(closeRange)) {
      ...
      // check if we can reach the next node, in case this was a "jump down"
      // situation
      if (m_pathIndex < m_pathLength - 1) {
        if (m_path[m_pathIndex + 1].pos.z - GetFeetZ() > JumpCrouchHeight) {
          ...
          for (int i = m_pathIndex; i <= m_pathIndex + 1; ++i) {
            if (m_path[i].how == GO_LADDER_UP) {
              // if we're going up a ladder, and we're within reach of the
              // ladder bottom, we haven't fallen
  ...
  if (didFall || gpGlobals->curtime - m_areaEnteredTimestamp > giveUpDuration) {
    if (didFall) {
      ...
      if (IsLocalPlayerWatchingMe() && cv_bot_debug.GetBool() &&
    ...
    CNavArea *area =
        TheNavMesh->GetNearestNavArea(GetAbsOrigin(), false, 500.0f, true);
...
bool CCSBot::ComputePath(const Vector &goal, RouteType route) {
  ...
  // Throttle re-pathing
  //
  if (!m_repathTimer.IsElapsed()) return false;
  ...
  bool pathToGoalExists =
      NavAreaBuildPath(startArea, goalArea, &goal, cost, &closestArea);
  ...
  int count = 0;
  CNavArea *area;
  for (area = effectiveGoalArea; area; area = area->GetParent()) ++count;

  // save room for endpoint
  if (count > MAX_PATH_LENGTH - 1) count = MAX_PATH_LENGTH - 1;
  ...
  // find initial encounter area along this path, if we are in the early part of
  // the round
  if (IsSafe()) {
    ...
    for (i = 0; i < m_pathLength; ++i) {
      if (m_path[i].area->GetEarliestOccupyTime(myTeam) >
          m_path[i].area->GetEarliestOccupyTime(enemyTeam)) {
        break;
      }
    }
...
void CCSBot::DrawPath(void) {
  ...
  if (FindOurPositionOnPath(&close, true) >= 0) {
    ...
    UTIL_DrawBeamPoints(close + Vector(25, 0, 0), close + Vector(-25, 0, 0), 1,
                        0, 255, 0);
    UTIL_DrawBeamPoints(close + Vector(0, 25, 0), close + Vector(0, -25, 0), 1,
                        0, 255, 0);
  }
}
