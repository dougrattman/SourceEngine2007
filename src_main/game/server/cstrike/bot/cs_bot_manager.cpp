...
ConVar bot_show_occupy_time(
    "bot_show_occupy_time", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
    "Show when each nav area can first be reached by each team.");

void DrawBattlefront(void);
ConVar bot_show_battlefront(
    "bot_show_battlefront", "0", FCVAR_GAMEDLL | FCVAR_CHEAT,
...
ConVar bot_join_delay("bot_join_delay", "0", FCVAR_GAMEDLL,
                      "Prevents bots from joining the server for this many "
...
inline bool AreBotsAllowed() {
  // If they pass in -nobots, don't allow bots.  This is for people who host
  // servers, to allow them to disallow bots to enforce CPU limits.
...
//--------------------------------------------------------------------------------------------------------------
CBasePlayer *ClientPutInServerOverride_Bot(edict_t *pEdict,
...
void CCSBotManager::RestartRound() {
  ...
  SetLooseBomb(NULL);
  m_isBombPlanted = false;
  m_earliestBombPlantTimestamp =
      gpGlobals->curtime + RandomFloat(10.0f, 30.0f);  // 60
  m_bombDefuser = NULL;
  ...
  m_isDefenseRushing =
      (RandomFloat(0.0f, 100.0f) <= defenseRushChance) ? true : false;
...
void UTIL_DrawBox(Extent *extent, int lifetime, int red, int green, int blue) {
  ...
  static int edge[] = {1, 2,  3, 4,  -1, 5,  6, 7,  8, -5,
                       1, -5, 2, -6, 3,  -7, 4, -8, 0};
...
void CCSBotManager::EnableEventListeners(bool enable) {
  ...
  // enable/disable the most frequent event listeners, to improve performance
  // when no bots are present.
  for (int i = 0; i < m_commonEventListeners.Count(); ++i) {
    if (enable) {
      gameeventmanager->AddListener(m_commonEventListeners[i],
                                    m_commonEventListeners[i]->GetEventName(),
                                    true);
    } else {
...
void CCSBotManager::ServerActivate() {
  ...
  TheBotProfiles->FindVoiceBankIndex(
      "BotChatter.db");  // make sure default voice bank is first
  const char *filename;
  ...
  // Now that we've parsed all the profiles, we have a list of the voice banks
  // they're using. Go back and parse the custom voice speakables.
  const BotProfileManager::VoiceBankList *voiceBanks =
      TheBotProfiles->GetVoiceBanks();
 ...
 */
void BotArgumentsFromArgv(const CCommand &args, const char **name,
                          CSWeaponType *weaponType,
                          BotDifficultyType *difficulty, int *team = NULL,
                          bool *all = NULL) {
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_add,
              "bot_add <t|ct> <type> <difficulty> <name> - Adds a bot matching "
              "the given criteria.",
              FCVAR_GAMEDLL) {
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_add_t,
              "bot_add_t <type> <difficulty> <name> - Adds a terrorist bot "
  ...
  TheCSBots()->BotAddCommand(TEAM_TERRORIST, FROM_CONSOLE, name, weaponType,
                             difficulty);
}

//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_add_ct,
              "bot_add_ct <type> <difficulty> <name> - Adds a "
              "Counter-Terrorist bot matching the given criteria.",
              FCVAR_GAMEDLL) {
  if (!UTIL_IsCommandIssuedByServerAdmin()) return;
  ...
  CSWeaponType weaponType;
  BotArgumentsFromArgv(args, &name, &weaponType, &difficulty);
  TheCSBots()->BotAddCommand(TEAM_CT, FROM_CONSOLE, name, weaponType,
                             difficulty);
}

//--------------------------------------------------------------------------------------------------------------
/**
 * Collects all bots matching the given criteria (player name, profile template
 * name, difficulty, and team)
 */
class CollectBots {
 public:
  CollectBots(const char *name, CSWeaponType weaponType,
              BotDifficultyType difficulty, int team) {
    m_name = name;
  ...
  bool operator()(CBasePlayer *player) {
    ...
    if (m_weaponType != WEAPONTYPE_UNKNOWN) {
      ...
      if (m_weaponType !=
          WeaponClassFromWeaponID(
              (CSWeaponID)bot->GetProfile()->GetWeaponPreference(0))) {
        return true;
      }
    }
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_kill,
              "bot_kill <all> <t|ct> <type> <difficulty> <name> - Kills a "
              "specific bot, or all bots, matching the given criteria.",
              FCVAR_GAMEDLL) {
  if (!UTIL_IsCommandIssuedByServerAdmin()) return;
  ...
  if ((!name || !*name) && team == TEAM_UNASSIGNED &&
      difficulty == NUM_DIFFICULTY_LEVELS) {
    all = true;
  }
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_kick,
              "bot_kick <all> <t|ct> <type> <difficulty> <name> - Kicks a "
              "specific bot, or all bots, matching the given criteria.",
              FCVAR_GAMEDLL) {
  if (!UTIL_IsCommandIssuedByServerAdmin()) return;
  ...
  // adjust bot quota so kicked bot is not immediately added back in
  if (all && (!name || !*name) && team == TEAM_UNASSIGNED &&
      difficulty == NUM_DIFFICULTY_LEVELS) {
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_snipers_only,
              "Restricts the bots to only using sniper rifles", FCVAR_GAMEDLL) {
  if (!UTIL_IsCommandIssuedByServerAdmin()) return;
...
//--------------------------------------------------------------------------------------------------------------
CON_COMMAND_F(bot_goto_mark,
              "Sends a bot to the selected nav area (useful for testing "
              "navigation meshes)",
 ...
 */
bool CCSBotManager::BotAddCommand(int team, bool isFromConsole,
  ...
  if (profileName && *profileName) {
    ...
    if (!profile) {
      ...
      profile = TheBotProfiles->GetProfileMatchingTemplate(profileName, team,
                                                           difficulty);
      if (!profile) {
        if (isFromConsole) {
  ...
  if (CSGameRules()->TeamStacked(team, TEAM_UNASSIGNED)) {
    if (isFromConsole) {
      Msg("Could not add bot to the game: Team is stacked (to disable this "
          "check, set mp_autoteambalance to zero, increase mp_limitteams, and "
          "restart the round).\n");
    }
    return false;
  }
...
void CCSBotManager::MaintainBotQuota() {
  ...
  // don't add bots until local player has been registered, to make sure he's
  // player ID #1
  if (!engine->IsDedicatedServer() && totalHumansInGame == 0) return;

  // new players can't spawn immediately after the round has been going for some
  // time
  ...
  /// isRoundInProgress is true if the round has progressed far enough that new
  /// players will join as dead.
  ...
  if (FStrEq(cv_bot_quota_mode.GetString(), "fill")) {
    // If bot_quota_mode is 'fill', we want the number of bots and humans
    ...
    // If bot_quota_mode is 'match', we want the number of bots to be bot_quota
    // * total humans unless the round is already in progress, in which case we
  ...
  if (cv_bot_join_after_player.GetBool()) {
    if (humanPlayersInGame == 0) desiredBotCount = 0;
  }

  // wait until the map has been loaded for a bit, to allow players to
  // transition across the transition without missing the pistol round
  if (bot_join_delay.GetInt() > CSGameRules()->GetMapElapsedTime()) {
    desiredBotCount = 0;
  }

  // if bots will auto-vacate, we need to keep one slot open to allow players to
  // join
  ...
  if (botsInGame > 0 && desiredBotCount == botsInGame &&
      CSGameRules()->m_bFirstConnected) {
    if (CSGameRules()->GetRoundElapsedTime() <
        20.0f)  // new bots can still spawn during this time
    {
      if (mp_autoteambalance.GetBool()) {
        ...
        if (!FStrEq(cv_bot_join_team.GetString(), "T") &&
          ...
          if (numAliveTerrorist > CSGameRules()->m_iNumCT + 1) {
            if (UTIL_KickBotFromTeam(TEAM_TERRORIST)) return;
          } else if (numAliveCT > CSGameRules()->m_iNumTerrorist + 1) {
            if (UTIL_KickBotFromTeam(TEAM_CT)) return;
          }
        }
      }
    }
  ...
  if (desiredBotCount > botsInGame) {
    ...
    if (!CSGameRules()->TeamFull(TEAM_TERRORIST) ||
        !CSGameRules()->TeamFull(TEAM_CT))
    ...
    // remove from the team that's winning
      ...
      kickTeam = TEAM_TERRORIST;
    } else if (CSGameRules()->m_iNumCTWins >
...
class CollectOverlappingAreas {
 public:
  ...
  bool operator()(CNavArea *area) {
    ...
    if (areaExtent.hi.x >= m_zone->m_extent.lo.x &&
        areaExtent.lo.x <= m_zone->m_extent.hi.x &&
        areaExtent.hi.y >= m_zone->m_extent.lo.y &&
        areaExtent.lo.y <= m_zone->m_extent.hi.y &&
        areaExtent.hi.z >= m_zone->m_extent.lo.z &&
        areaExtent.lo.z <= m_zone->m_extent.hi.z) {
  ...
  for (i = 1; i < gpGlobals->maxEntities; ++i) {
    ...
    if (found) {
      if (m_zoneCount < MAX_ZONES) {
        ...
        m_zone[m_zoneCount].m_center =
            (isLegacy) ? entity->GetAbsOrigin() : (absmin + absmax) / 2.0f;
 ...
 */
const CCSBotManager::Zone *CCSBotManager::GetClosestZone(
    const Vector &pos) const {
  const Zone *close = NULL;
  float closeRangeSq = 999999999.9f;

  for (int z = 0; z < m_zoneCount; ++z) {
    if (m_zone[z].m_isBlocked) continue;
...
const Vector *CCSBotManager::GetRandomPositionInZone(const Zone *zone) const {
  ...
  if (zone->m_isLegacy) {
    /// @todo It is possible that the radius might not overlap this area at
    /// all...
...
CNavArea *CCSBotManager::GetRandomAreaInZone(const Zone *zone) const {
  ...
  if (areaCount == 0) {
    assert(false &&
           "CCSBotManager::GetRandomAreaInZone: No areas for this zone");
    return NULL;
  }

  // Random, but weighted.  Jump areas score zero, since you aren't ever meant
  // to stop on one of those. Avoid areas score 1 to a normal area's 20 because
  // pathfinding treats Avoid as a 20x penalty.
  int totalWeight = 0;
  ...
  if (totalWeight == 0) {
    assert(false &&
           "CCSBotManager::GetRandomAreaInZone: No real areas for this zone");
    return NULL;
  }
...
void CCSBotManager::OnServerShutdown(IGameEvent *event) {
  if (!engine->IsDedicatedServer()) {
    // Since we're a listenserver, save some config info for the next time we
    // start up
    static const char *botVars[] = {"bot_quota",
                                    "bot_difficulty",
                                    "bot_chatter",
                                    "bot_prefix",
                                    "bot_join_team",
...
void CCSBotManager::OnBombDefuseBegin(IGameEvent *event) {
  m_bombDefuser =
      static_cast<CCSPlayer *>(UTIL_PlayerByUserId(event->GetInt("userid")));
...
void CCSBotManager::CheckForBlockedZones() {
  ...
  for (int i = 0; i < m_zoneCount; ++i) {
    ...
    float dist =
        NavAreaTravelDistance(spawnArea, m_zone[i].m_area[0], costFunc);
    m_zone[i].m_isBlocked = (dist < 0.0f);

    if (cv_bot_debug.GetInt() == 5) {
      if (m_zone[i].m_isBlocked)
        DevMsg(
            "%.1f: Zone %d, area %d (%.0f %.0f %.0f) is blocked from spawn "
            "area %d (%.0f %.0f %.0f)\n",
            gpGlobals->curtime, i, m_zone[i].m_area[0]->GetID(),
            m_zone[i].m_area[0]->GetCenter().x,
            m_zone[i].m_area[0]->GetCenter().y,
            m_zone[i].m_area[0]->GetCenter().z, spawnArea->GetID(), spawnPos.x,
...
void CCSBotManager::OnRoundFreezeEnd(IGameEvent *event) {
  ...
  m_NavBlockedEvent.Enable(false);  // don't listen to nav_blocked events -
                                    // there could be several, and we don't have
                                    // bots pathing
  CUtlVector<CNavArea *> &transientAreas = TheNavMesh->GetTransientAreas();
  for (int i = 0; i < transientAreas.Count(); ++i) {
    CNavArea *area = transientAreas[i];
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Check all nav areas inside the breakable's extent to see if players would now
 * fall through
 */
class CheckAreasOverlappingBreakable {
 public:
  CheckAreasOverlappingBreakable(CBaseEntity *breakable) {
    ...
    collideable->WorldSpaceSurroundingBounds(&m_breakableExtent.lo,
                                             &m_breakableExtent.hi);
  ...
  bool operator()(CNavArea *area) {
    ...
    if (areaExtent.hi.x >= m_breakableExtent.lo.x &&
        areaExtent.lo.x <= m_breakableExtent.hi.x &&
        areaExtent.hi.y >= m_breakableExtent.lo.y &&
        areaExtent.lo.y <= m_breakableExtent.hi.y &&
        areaExtent.hi.z >= m_breakableExtent.lo.z &&
        areaExtent.lo.z <= m_breakableExtent.hi.z) {
...
void CCSBotManager::OnBreakBreakable(IGameEvent *event) {
  CheckAreasOverlappingBreakable collector(
      UTIL_EntityByIndex(event->GetInt("entindex")));
  TheNavMesh->ForAllAreas(collector);
...
bool CCSBotManager::IsImportantPlayer(CCSPlayer *player) const {
  switch (GetScenario()) {
    ...
    case SCENARIO_ESCORT_VIP: {
      if (player->GetTeamNumber() == TEAM_CT && player->IsVIP()) return true;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Returns a random spawn point for the given team (no arg means use both team
 * spawnpoints)
 */
CBaseEntity *CCSBotManager::GetRandomSpawn(int team) const {
  ...
  if (team == TEAM_CT || team == TEAM_MAXCOUNT) {
    // collect CT spawns
    for (spot = gEntList.FindEntityByClassname(NULL,
                                               "info_player_counterterrorist");
         spot; spot = gEntList.FindEntityByClassname(
                   spot, "info_player_counterterrorist")) {
      spawnSet.AddToTail(spot);
    }
  }
 ...
 */
float CCSBotManager::GetRadioMessageTimestamp(RadioType event,
                                              int teamID) const {
 ...
 */
float CCSBotManager::GetRadioMessageInterval(RadioType event,
                                             int teamID) const {
  int i = (teamID == TEAM_TERRORIST) ? 0 : 1;
...
void DrawOccupyTime() {
  FOR_EACH_LL(TheNavAreaList, it) {
    ...
    if (TheCSBots()->GetElapsedRoundTime() >
        area->GetEarliestOccupyTime(TEAM_TERRORIST)) {
      if (TheCSBots()->GetElapsedRoundTime() >
          area->GetEarliestOccupyTime(TEAM_CT)) {
        r = 255;
        g = 0;
        b = 255;
      } else {
    ...
    } else if (TheCSBots()->GetElapsedRoundTime() >
               area->GetEarliestOccupyTime(TEAM_CT)) {
      r = 0;
      g = 0;
      b = 255;
...
void DrawBattlefront() {
  ...
  FOR_EACH_LL(TheNavAreaList, it) {
    ...
    if (fabs(area->GetEarliestOccupyTime(TEAM_TERRORIST) -
             area->GetEarliestOccupyTime(TEAM_CT)) > epsilon) {
      continue;
    }
...
static bool CheckAreaAgainstAllZoneAreas(CNavArea *queryArea) {
  ...
  for (int zoneIndex = 0; zoneIndex < goalZoneCount; zoneIndex++) {
    ...
    for (int areaIndex = 0; areaIndex < zoneAreaCount; areaIndex++) {
      ...
      // We need to be connected to every area in the zone, since we don't know
      // what other code might pick for an area
      ShortestPathCost cost;
      if (NavAreaTravelDistance(queryArea, zoneArea, cost) == -1.0f) {
        Msg("Area #%d is disconnected from goal area #%d.\n",
            queryArea->GetID(), zoneArea->GetID());
        return false;
      }
    }
  }
...
CON_COMMAND_F(nav_check_connectivity,
              "Checks to be sure every (or just the marked) nav area can get "
              "to every goal area for the map (hostages or bomb site).",
              FCVAR_CHEAT) {
  ...
  if (TheNavMesh->GetMarkedArea()) {
    ...
    // Otherwise, loop through every area, and make sure they can all get to the
    // goal.
