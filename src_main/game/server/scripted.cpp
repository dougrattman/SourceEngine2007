...
//
// targetname "me" - there can be more than one with the same name, and they act
// in concert target "the_entity_I_want_to_start_playing" or "class
// entity_classname" will pick the closest inactive scientist play
// "name_of_sequence" idle "name of idle sequence to play before starting"
...
BEGIN_DATADESC(CAI_ScriptedSequence)
  ...
  DEFINE_KEYFIELD(m_iszEntry, FIELD_STRING, "m_iszEntry"),
      ...
      DEFINE_KEYFIELD(m_bDisableNPCCollisions, FIELD_BOOLEAN,
...
void CAI_ScriptedSequence::ScriptEntityCancel(CBaseEntity *pentCine,
  ...
  if (FClassnameIs(pentCine, CLASSNAME)) {
    ...
    if (pEntity) pTarget = pEntity->MyNPCPointer();
    ...
    if (pTarget) {
      ...
      if (pTarget->m_NPCState == NPC_STATE_SCRIPT) {
        ...
        // We have to save off the flags here, because the NPC's m_hCine is
        // cleared in CineCleanup()
        int iSavedFlags =
            (pTarget->m_hCine ? pTarget->m_hCine->m_savedFlags : 0);
    ...
    if (bPretendSuccess) {
      ...
      if (pCineTarget->m_startTime == 0) {
        // If start time is 0, this sequence never actually ran. Fire the failed
        // output.
...
bool CAI_ScriptedSequence::FCanOverrideState(void) {
  if (m_spawnflags & SF_SCRIPT_OVERRIDESTATE) return true;
  return false;
}
...
void CAI_ScriptedSequence::InputMoveToPosition(inputdata_t &inputdata) {
  if (m_bInitiatedSelfDelete) return;
  ...
  if (pTarget != NULL) {
    ...
    if (pTarget->m_scriptState == CAI_BaseNPC::SCRIPT_PLAYING ||
        pTarget->m_scriptState == CAI_BaseNPC::SCRIPT_POST_IDLE) {
    ...
    // No, presumably they are moving to position or waiting for the
    // BeginSequence input.
  } else {
    // No, grab the NPC but make them wait until BeginSequence is fired. They'll
    // play their pre-action idle animation until BeginSequence is fired.
    StartThink();
...
void CAI_ScriptedSequence::InputBeginSequence(inputdata_t &inputdata) {
  ...
  if (pTarget) {
    ...
    // Because we are directly calling the new "think" function ScriptThink,
    // assume we're done This fixes the following bug (along with the
    // WokeThisTick() code herein:
    //  A zombie is created in the asleep state and then, the mapper fires both
    //  Wake and BeginSequence messages to have it jump up out of the slime,
    //  e.g.  What happens before this change is that the Wake code removed
    //  EF_NODRAW, but so the zombie is transmitted to the client, but the
    //  script hasn't started and won't start until the next Think time (2 ticks
    //  on xbox) at which time the actual sequence starts causing the zombie to
    //  quickly lie down.
    // The changes here are to track what tick we "awoke" on and get rid of the
    // lag between Wake and ScriptThink by actually calling ScriptThink directly
    // on the same frame and checking for the
    //  zombie having woken up and been instructed to play a sequence in the
    //  same frame.
...
//-----------------------------------------------------------------------------
// Purpose: This doesn't really make sense since only MOVETYPE_PUSH get
// 'Blocked' events Input  : pOther - The entity blocking us.
//-----------------------------------------------------------------------------
...
void CAI_ScriptedSequence::Touch(CBaseEntity *pOther) {
  /*
          ...
          {
                  CAI_BaseNPC *pTarget = GetClassPtr((CAI_BaseNPC
     *)VARS(m_pentTarget)); pTarget->m_NPCState == NPC_STATE_SCRIPT;
          }
  */
}
...
CAI_BaseNPC *CAI_ScriptedSequence::FindScriptEntity() {
  ...
  if (m_hForcedTarget) {
    ...
    pEntity = gEntList.FindEntityByNameWithin(
        m_hLastFoundEntity, STRING(m_iszEntity), GetAbsOrigin(), m_flRadius);
    if (!pEntity) {
  ...
  while (pEntity != NULL) {
    ...
    if (pNPC) {
      ...
      //
      CanPlaySequence_t eCanPlay =
          pNPC->CanPlaySequence(FCanOverrideState(), interrupt);
    ...
    if (m_hForcedTarget) {
      Warning(
          "Code forced %s(%s), to be the target of scripted sequence %s, but "
          "it can't play it.\n",
      ...
      if (interrupt == SS_INTERRUPT_BY_NAME)
        pEntity = gEntList.FindEntityByNameWithin(pEntity, STRING(m_iszEntity),
                                                  GetAbsOrigin(), m_flRadius);
      else
        pEntity = gEntList.FindEntityByClassnameWithin(
            pEntity, STRING(m_iszEntity), GetAbsOrigin(), m_flRadius);
    }
  }
  ...
  if (pTarget) {
    ...
    if (pTarget->m_hCine != NULL) {
      if (pTarget->m_hCine->m_hNextCine != NULL) {
        ...
        //
        CAI_ScriptedSequence *pCine =
            (CAI_ScriptedSequence *)pTarget->m_hCine->m_hNextCine.Get();
    ...
    //
    // If no next script is specified, clear it out so other scripts can enqueue
    // themselves after us.
    //
    if (!m_iszNextScript) {
      m_hNextCine = NULL;
    ...
    {
      m_bTargetWasAsleep =
          (pTarget->GetSleepState() != AISS_AWAKE) ? true : false;
      bool justAwoke = pTarget->WokeThisTick();
      if (m_bTargetWasAsleep || justAwoke) {
        // Note, Wake() will remove the EF_NODRAW flag, but if we are starting a
        // seq on a hidden entity
        //  we don't want it to draw on the client until the sequence actually
        //  starts to play
        ...
        // Even if awakened this frame, temporarily keep the entity hidden for
        // now
        pTarget->AddEffects(EF_NODRAW);
      }
    }

    // If the entity was asleep at the start, make sure we don't make it
    // invisible AFTER the script finishes (can't think of a case where you'd
    // want that to happen)
    ...
    switch (m_fMoveTo) {
      ...
      case CINE_MOVETO_TELEPORT:
        ...
        // UNDONE: Add a flag to do this so people can fixup physics after
        // teleporting NPCs
        // pTarget->SetGroundEntity( NULL );
        break;
    }
    // DevMsg( 2,  "\"%s\" found and used (INT: %s)\n", STRING( pTarget->m_iName
    // ), FBitSet(m_spawnflags, SF_SCRIPT_NOINTERRUPT)?"No":"Yes" );
    ...
    if (pTarget->IsEFlagSet(EFL_NO_THINK_FUNCTION) &&
      ...
      DevWarning(
          "scripted_sequence %d:%s - restarting dormant entity %d:%s : "
          "%.1f:%.1f\n",
          entindex(), GetDebugName(), pTarget->entindex(),
          pTarget->GetDebugName(), gpGlobals->curtime, pTarget->GetNextThink());
      pTarget->SetNextThink(gpGlobals->curtime);
    }
  }
}
...
void CAI_ScriptedSequence::ScriptThink(void) {
  if (g_pAINetworkManager && !g_pAINetworkManager->IsInitialized()) {
    ...
    DevMsg(2, "scripted_sequence %d:\"%s\" using NPC %d:\"%s\"(%s)\n",
           entindex(), GetDebugName(), GetTarget()->entindex(),
           GetTarget()->GetEntityName().ToCStr(), STRING(m_iszEntity));
  } else {
    CancelScript();
...
//-----------------------------------------------------------------------------
// Purpose: Callback for firing the begin sequence output. Called by the NPC
// that
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_ScriptedSequence::StartSequence(CAI_BaseNPC *pTarget, string_t iszSeq,
  ...
  if (act == ACT_INVALID) act = ACT_IDLE;

  pTarget->SetActivityAndSequence(act, nSequence, act, act);

  // If the target was hidden even though we woke it up, only make it drawable
  // if we're not still on the preidle seq...
  if (m_bTargetWasAsleep && iszSeq != m_iszPreIdle) {
  ...
  // pTarget->GetClassname(), STRING( iszSeq), (m_spawnflags &
  // SF_SCRIPT_NOINTERRUPT) ? "No" : "Yes" );

  return true;
}

//-----------------------------------------------------------------------------
// Purpose: Called when a scripted sequence is ready to start playing the
// sequence Input  : pNPC - Pointer to the NPC that the sequence possesses.
//-----------------------------------------------------------------------------
...
bool CAI_ScriptedSequence::FinishedActionSequence(CAI_BaseNPC *pNPC) {
  ...
  if (ShouldLoopActionSequence()) {
    // If the NPC has reached post idle state, we need to stop looping the
    // action sequence
    if (pNPC->m_scriptState == CAI_BaseNPC::SCRIPT_POST_IDLE) return true;
...
void CAI_ScriptedSequence::SequenceDone(CAI_BaseNPC *pNPC) {
  ...
  // If we're part of a synchronised post-idle sequence, we need to do things
  // differently
  if (m_bSynchPostIdles && GetEntityName() != NULL_STRING) {
    ...
    if ((m_iszPostIdle != NULL_STRING) && (m_hNextCine == NULL)) {
      //
      // First time we've gotten here for this script. Start playing the post
      // idle if one is specified.
      //
      pNPC->m_scriptState = CAI_BaseNPC::SCRIPT_POST_IDLE;
      StartSequence(pNPC, m_iszPostIdle,
                    false);  // false to prevent recursion here!
    } else {
      PostIdleDone(pNPC);
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void CAI_ScriptedSequence::SynchNewSequence(CAI_BaseNPC::SCRIPTSTATE newState,
  ...
  if (bSynchOtherScenes) {
    ...
    // GetTarget()->GetDebugName(), GetDebugName(), iszSequence,
    // gpGlobals->curtime);

    CBaseEntity *pentCine =
        gEntList.FindEntityByName(NULL, GetEntityName(), NULL);
    while (pentCine) {
      CAI_ScriptedSequence *pScene =
          dynamic_cast<CAI_ScriptedSequence *>(pentCine);
  ...
  // GetDebugName(), iszSequence, gpGlobals->curtime);
...
void CAI_ScriptedSequence::PostIdleDone(CAI_BaseNPC *pNPC) {
  //
  // If we're set to keep the NPC in a scripted idle, hack them back into the
  // script, but allow another scripted sequence to take control of the NPC if
  // it wants to, unless another script has stolen them from us.
  //
  if ((m_iszPostIdle != NULL_STRING) &&
      (m_spawnflags & SF_SCRIPT_LOOP_IN_POST_IDLE) && (m_hNextCine == NULL)) {
    //
    ...
    //
    // This is done so that another sequence can take over the NPC when
    // triggered by the first.
    //
    pNPC->CineCleanup();

    // We have to pass in the flags here, because the NPC's m_hCine was cleared
    // in CineCleanup()
    ...
    //
    if (m_hNextCine != NULL) {
      CAI_ScriptedSequence *pNextCine =
          (CAI_ScriptedSequence *)m_hNextCine.Get();
...
void CAI_ScriptedSequence::FixFlyFlag(CAI_BaseNPC *pNPC, int iSavedCineFlags) {
  // Adrian: We NEED to clear this or the NPC's FL_FLY flag will never be
  // removed cause of ClearSchedule!
  if (pNPC->GetTask() &&
      (pNPC->GetTask()->iTask == TASK_PLAY_SCRIPT ||
       pNPC->GetTask()->iTask == TASK_PLAY_SCRIPT_POST_IDLE)) {
...
void CAI_ScriptedSequence::AllowInterrupt(bool fAllow) {
  if (m_spawnflags & SF_SCRIPT_NOINTERRUPT) return;
...
bool CAI_ScriptedSequence::CanInterrupt(void) {
  if (!m_interruptable) return false;

  CBaseEntity *pTarget = GetTarget();

  if (pTarget != NULL && pTarget->IsAlive()) return true;
...
bool CAI_ScriptedSequence::CanEnqueueAfter(void) {
  ...
  if (m_iszNextScript != NULL_STRING) {
    DevMsg(2,
           "%s is specified as the 'Next Script' and cannot be kicked out of "
           "the queue\n",
...
void CAI_ScriptedSequence::StopActionLoop(bool bStopSynchronizedScenes) {
  ...
  if (!bStopSynchronizedScenes || GetEntityName() == NULL_STRING) return;
...
void CAI_ScriptedSequence::ForceSetTargetEntity(
  ...
  m_iszEntity =
      m_hForcedTarget->GetEntityName();  // Not guaranteed to be unique
...
void CAI_ScriptedSequence::SetupInteractionPosition(
    CBaseEntity *pRelativeEntity, VMatrix &matDesiredLocalToWorld) {
...
void CAI_ScriptedSequence::ModifyScriptedAutoMovement(Vector *vecNewPos) {
  if (m_hInteractionRelativeEntity) {
    ...
    if (m_iszEntry != NULL_STRING && !m_bIsPlayingEntry) return;
    ...
    if (bDebug) {
      Msg("--\n%s current org: %f %f\n", m_hTargetEnt->GetDebugName(),
          m_hTargetEnt->GetAbsOrigin().x, m_hTargetEnt->GetAbsOrigin().y);
      Msg("%s current org: %f %f", m_hInteractionRelativeEntity->GetDebugName(),
          vecRelativeOrigin.x, vecRelativeOrigin.y);
    }

    CBaseAnimating *pAnimating =
        dynamic_cast<CBaseAnimating *>(m_hInteractionRelativeEntity.Get());
    if (pAnimating) {
      Vector vecDeltaPos;
      QAngle vecDeltaAngles;
      pAnimating->GetSequenceMovement(pAnimating->GetSequence(), 0.0f,
                                      pAnimating->GetCycle(), vecDeltaPos,
                                      vecDeltaAngles);
      VectorYawRotate(vecDeltaPos, pAnimating->GetLocalAngles().y, vecDeltaPos);

      if (bDebug) {
        NDebugOverlay::Box(vecRelativeOrigin, -Vector(2, 2, 2), Vector(2, 2, 2),
                           0, 255, 0, 8, 0.1);
      }
      ...
      if (bDebug) {
        Msg(", relative to sequence start: %f %f\n", vecRelativeOrigin.x,
            vecRelativeOrigin.y);
        NDebugOverlay::Box(vecRelativeOrigin, -Vector(3, 3, 3), Vector(3, 3, 3),
                           255, 0, 0, 8, 0.1);
      }
    }

    // We've been asked to maintain a specific position relative to the other
    // NPC we're interacting with. Lerp towards the relative position.
...
void CAI_ScriptedSequence::CancelScript(void) {
  ...
  // Don't cancel matching sequences if we're asked not to, unless we didn't
  // actually succeed in starting, in which case we should always cancel. This
  // fixes dynamic interactions where an NPC was killed the same frame another
  // NPC started a dynamic interaction with him.
  bool bDontCancelOther =
      ((m_bDontCancelOtherSequences || HasSpawnFlags(SF_SCRIPT_ALLOW_DEATH)) &&
       (m_startTime != 0));
  ...
  CBaseEntity *pentCineTarget =
      gEntList.FindEntityByName(NULL, GetEntityName());
  ...
  while (pentCine) {
    if (FClassnameIs(pentCine, "scripted_sequence")) {
      ...
      if (bDelay) {
        ...
        // Msg("SSEQ: (%d) Found matching SS (%d). Incrementing MY m_iDelay to
        ...
        // Msg("SSEQ: (%d) Found matching SS (%d). Decrementing THEIR m_iDelay
        ...
        if (pTarget->m_iDelay == 0) {
          ...
          // Msg("SSEQ: STARTING SEQUENCE for \"%s\" (%d) (m_iDelay reached
...
void CAI_ScriptedSequence::DrawDebugGeometryOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    if (GetTarget()) {
      NDebugOverlay::HorzArrow(GetAbsOrigin(), GetTarget()->GetAbsOrigin(), 16,
                               0, 255, 0, 64, true, 0.0f);
    }
  }
}
...
int CAI_ScriptedSequence::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Target: %s",
               GetTarget() ? GetTarget()->GetDebugName() : "None");
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Thinking: %s",
               m_bThinking ? "Yes" : "No");
    EntityText(text_offset, tempstr, 0);
    text_offset++;
    ...
    Q_snprintf(tempstr, sizeof(tempstr), "Sequence has started: %s",
               m_sequenceStarted ? "Yes" : "No");
    EntityText(text_offset, tempstr, 0);
    text_offset++;

    Q_snprintf(tempstr, sizeof(tempstr), "Cancel Other Sequences: %s",
               m_bDontCancelOtherSequences ? "No" : "Yes");
    EntityText(text_offset, tempstr, 0);
    text_offset++;
...
void CAI_ScriptedSchedule::ScriptThink(void) {
  ...
  if (!m_bGrabAll) {
    pTarget =
        FindScriptEntity((m_spawnflags & SF_SCRIPT_SEARCH_CYCLICALLY) != 0);
    if (pTarget) {
      DevMsg(2, "scripted_schedule \"%s\" using NPC \"%s\"(%s)\n",
             GetDebugName(), STRING(m_iszEntity),
             pTarget->GetEntityName().ToCStr());
    ...
    while ((pTarget = FindScriptEntity(true)) != NULL) {
      DevMsg(2, "scripted_schedule \"%s\" using NPC \"%s\"(%s)\n",
             GetDebugName(), pTarget->GetEntityName().ToCStr(),
             STRING(m_iszEntity));
      StartSchedule(pTarget);
      success = true;
    }
...
CAI_BaseNPC *CAI_ScriptedSchedule::FindScriptEntity(bool bCyclic) {
  CBaseEntity *pEntity = gEntList.FindEntityGenericWithin(
      m_hLastFoundEntity, STRING(m_iszEntity), GetAbsOrigin(), m_flRadius, this,
      m_hActivator);
  ...
  while (pEntity != NULL) {
    ...
    if (pNPC && pNPC->IsAlive() && pNPC->IsInterruptable()) {
      if (bCyclic) {
        // next time this is called, start searching from the one found last
        // time
    ...
    pEntity = gEntList.FindEntityGenericWithin(
        pEntity, STRING(m_iszEntity), GetAbsOrigin(), m_flRadius, this, NULL);
  }
...
void CAI_ScriptedSchedule::StartSchedule(CAI_BaseNPC *pTarget) {
  ...
  CBaseEntity *pGoalEnt =
      gEntList.FindEntityGeneric(NULL, STRING(m_sGoalEnt), this, NULL);
  ...
  if (!pGoalEnt) {
    ...
    CAI_Hint *pHint =
        CAI_HintManager::FindHint(pTarget->GetAbsOrigin(), hintCriteria);
    if (!pHint) {
  ...
  static NPC_STATE forcedStatesMap[] = {NPC_STATE_NONE, NPC_STATE_IDLE,
                                        NPC_STATE_ALERT, NPC_STATE_COMBAT};

  if (pTarget->GetSleepState() > AISS_AWAKE) pTarget->Wake();
  ...
  if (forcedState != NPC_STATE_NONE) pTarget->SetState(forcedState);

  //
  // Set enemy and make the NPC aware of the enemy's current position.
  //
  if (m_nSchedule == SCHED_SCRIPT_ENEMY_IS_GOAL ||
      m_nSchedule == SCHED_SCRIPT_ENEMY_IS_GOAL_AND_RUN_TO_GOAL) {
  ...
  switch (m_nSchedule) {
    ...
    case SCHED_SCRIPT_ENEMY_IS_GOAL_AND_RUN_TO_GOAL: {
      Activity movementActivity =
          (m_nSchedule == SCHED_SCRIPT_WALK_TO_GOAL) ? ACT_WALK : ACT_RUN;
      bool bIsFlying = (pTarget->GetMoveType() == MOVETYPE_FLY) ||
                       (pTarget->GetMoveType() == MOVETYPE_FLYGRAVITY);
      if (bIsFlying) {
        movementActivity = ACT_FLY;
      }

      if (!pTarget->ScheduledMoveToGoalEntity(SCHED_IDLE_WALK, pGoalEnt,
                                              movementActivity)) {
    ...
    case SCHED_SCRIPT_RUN_PATH_GOAL: {
      Activity movementActivity =
          (m_nSchedule == SCHED_SCRIPT_WALK_PATH_GOAL) ? ACT_WALK : ACT_RUN;
      bool bIsFlying = (pTarget->GetMoveType() == MOVETYPE_FLY) ||
                       (pTarget->GetMoveType() == MOVETYPE_FLYGRAVITY);
      if (bIsFlying) {
        movementActivity = ACT_FLY;
      }
      if (!pTarget->ScheduledFollowPath(SCHED_IDLE_WALK, pGoalEnt,
                                        movementActivity)) {
        if (!(m_spawnflags & SF_SCRIPT_NO_COMPLAINTS)) {
          DevMsg(1,
  ...
  if (bDidSetSchedule) {
    // Chain this to the target so that it can add the base and any custom
    // interrupts to this
...
void CAI_ScriptedSchedule::InputStartSchedule(inputdata_t &inputdata) {
  ...
  if (!m_bDidFireOnce || (m_spawnflags & SF_SCRIPT_REPEATABLE)) {
    ...
    DevMsg(2,
           "aiscripted_schedule - not playing schedule again: not flagged to "
           "repeat\n");
  }
}
...
void CAI_ScriptedSchedule::InputStopSchedule(inputdata_t &inputdata) {
  if (!m_bDidFireOnce) {
    DevMsg(2,
           "aiscripted_schedule - StopSchedule called, but schedule's never "
           "started.\n");
    return;
  }
...
//-----------------------------------------------------------------------------
// Purpose: If the target entity appears to be running this scripted schedule
// break it
//-----------------------------------------------------------------------------
...
BEGIN_DATADESC(CAI_ScriptedSentence)
  ...
  DEFINE_KEYFIELD(m_iszSentence, FIELD_STRING, "sentence"),
      ...
      // Function Pointers
      DEFINE_FUNCTION(FindThink), DEFINE_FUNCTION(DelayThink),
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CAI_ScriptedSentence::KeyValue(const char *szKeyName,
                                    const char *szValue) {
...
void CAI_ScriptedSentence::Spawn(void) {
  ...
  if (m_flVolume <= 0) m_flVolume = 1.0;
}
...
void CAI_ScriptedSentence::FindThink(void) {
  ...
  if (pNPC) {
    ...
    if (m_spawnflags & SF_SENTENCE_ONCE) UTIL_Remove(this);
    ...
    // add 0.1 because the sound engine mixes ahead -- the sentence will
    // actually start ~0.1 secs from now
...
void CAI_ScriptedSentence::DelayThink(void) {
  m_active = true;
  if (!GetEntityName()) SetNextThink(gpGlobals->curtime + 0.1f);
  SetThink(&CAI_ScriptedSentence::FindThink);
}
...
bool CAI_ScriptedSentence::AcceptableSpeaker(CAI_BaseNPC *pNPC) {
  if (pNPC) {
    ...
    if (m_spawnflags & SF_SENTENCE_INTERRUPT)
      ...
      override = false;
    if (pNPC->CanPlaySentence(override)) return true;
  }
  return false;
}
...
CAI_BaseNPC *CAI_ScriptedSentence::FindEntity(void) {
  ...
  while (pentTarget) {
    ...
    if (pNPC != NULL) {
      if (AcceptableSpeaker(pNPC)) return pNPC;
  ...
  for (CEntitySphereQuery sphere(GetAbsOrigin(), m_flRadius, FL_NPC);
...
int CAI_ScriptedSentence::StartSentence(CAI_BaseNPC *pTarget) {
  ...
  if (!(m_spawnflags & SF_SENTENCE_CONCURRENT)) bConcurrent = true;
  ...
  if (m_spawnflags & SF_SENTENCE_SPEAKTOACTIVATOR) {
    ...
    pListener = gEntList.FindEntityGenericNearest(
        STRING(m_iszListener), pTarget->GetAbsOrigin(), radius, this, NULL);
  }

  int sentenceIndex = pTarget->PlayScriptedSentence(
      STRING(m_iszSentence), m_flDelay, m_flVolume, m_iSoundLevel, bConcurrent,
      pListener);
...
// HACKHACK: This is a little expensive with the dynamic_cast<> and all, but it
// lets us solve the problem of matching scripts back to entities without new
// state.
const char *CAI_ScriptedSequence::GetSpawnPreIdleSequenceForScript(
    CBaseEntity *pEntity) {
  CAI_ScriptedSequence *pScript =
      gEntList.NextEntByClass((CAI_ScriptedSequence *)NULL);
  while (pScript) {
    if (pScript->HasSpawnFlags(SF_SCRIPT_START_ON_SPAWN) &&
        pScript->m_iszEntity == pEntity->GetEntityName()) {
