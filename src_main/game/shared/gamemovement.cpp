...
// and gpGlobals->frametime are. We should probably set tickcount (to
// player->m_nTickBase), but we're REALLY close to shipping, so we can change
// that later and people can use player->CurrentCommandNumber() in the meantime.
#define tickcount USE_PLAYER_CURRENT_COMMAND_NUMBER__INSTEAD_OF_TICKCOUNT

#if defined(HL2_DLL)
ConVar xc_uncrouch_on_jump("xc_uncrouch_on_jump", "1", FCVAR_ARCHIVE,
                           "Uncrouch when jump occurs");
#endif

#if defined(HL2_DLL) || defined(HL2_CLIENT_DLL)
ConVar player_limit_jump_speed("player_limit_jump_speed", "1",
                               FCVAR_REPLICATED);
#endif

// option_duck_method is a carrier convar. Its sole purpose is to serve an
// easy-to-flip convar which is ONLY set by the X360 controller menu to tell us
// which way to bind the duck controls. Its value is meaningless anytime we
// don't have the options window open.
ConVar option_duck_method(
    "option_duck_method", "1",
    FCVAR_REPLICATED |
        FCVAR_ARCHIVE);  // 0 = HOLD to duck, 1 = Duck is a toggle

// [MD] I'll remove this eventually. For now, I want the ability to A/B the
// optimizations.
bool g_bMovementOptimizations = true;

// Roughly how often we want to update the info about the ground surface we're
// on. We don't need to do this very often.
#define CATEGORIZE_GROUND_SURFACE_INTERVAL 0.3f
#define CATEGORIZE_GROUND_SURFACE_TICK_INTERVAL \
  ((int)(CATEGORIZE_GROUND_SURFACE_INTERVAL / TICK_INTERVAL))
...
#define CHECK_STUCK_TICK_INTERVAL_SP \
  ((int)(CHECK_STUCK_INTERVAL_SP / TICK_INTERVAL))

#define CHECK_LADDER_INTERVAL 0.2f
#define CHECK_LADDER_TICK_INTERVAL \
  ((int)(CHECK_LADDER_INTERVAL / TICK_INTERVAL))
...
// If you ever get stuck walking around, then you can run this code to find the
// code which would leave the player in a bad spot
void CMoveData::SetAbsOrigin(const Vector &vec) {
  CGameMovement *gm = dynamic_cast<CGameMovement *>(g_pGameMovement);
  if (gm && gm->GetMoveData() && gm->player && gm->player->entindex() == 1 &&
      gm->player->GetMoveType() == MOVETYPE_WALK) {
    trace_t pm;
    gm->TracePlayerBBox(vec, vec, gm->PlayerSolidMask(),
...
class IDiffMgr {
 public:
  ...
  virtual void AddToDiff(bool bServer, int nCommandNumber,
                         char const *string) = 0;
...
class CDiffStr {
 public:
  ...
  CDiffStr(char const *str) { Q_strncpy(m_str, str, sizeof(m_str)); }

  CDiffStr(const CDiffStr &src) { Q_strncpy(m_str, src.m_str, sizeof(m_str)); }
...
class CDiffManager : public IDiffMgr {
 public:
  ...
  EMismatched IsMismatched(int svidx) {
    ...
    int clidx = m_Client.Find(clsearch);
    if (clidx == m_Client.InvalidIndex()) return DIFFCHECK_NOTREADY;

    // Now compare them
    CDiffInfo *clientslot = &m_Client[clidx];

    bool bSpew = false;
    if (serverslot->m_Lines.Count() != clientslot->m_Lines.Count()) {
    ...
    if (!bSpew) {
      for (int i = 0; i < maxSlot; ++i) {
        ...
        if (Q_stricmp(sv ? sv->String() : "(missing)",
                      cl ? cl->String() : "(missing)")) {
          return DIFFCHECK_DIFFERS;
        }
      }
    }
  ...
  void SpewRecords(char const *prefix, int svidx) {
    ...
    clsearch.m_nCommandNumber = serverslot->m_nCommandNumber;
    int clidx = m_Client.Find(clsearch);
    if (clidx == m_Client.InvalidIndex()) return;
    ...
    for (int i = 0; i < maxSlot; ++i) {
      ...
      Msg("%s%s%d:  sv[%50.50s] cl[%50.50s]\n", prefix,
          bDiffers ? "+++" : "   ", serverslot->m_nCommandNumber, sv, cl);
    }
  }
...
void _CheckV(int tick, char const *ctx, const Vector &vel) {
  DiffPrint(CBaseEntity::IsServer(), tick, "%20.20s %f %f %f", ctx, vel.x,
            vel.y, vel.z);
}
...
void CGameMovement::DiffPrint(char const *fmt, ...) {
  ...
  ::DiffPrint(CBaseEntity::IsServer(), player->CurrentCommandNumber(), "%s",
              string);
}
...
void DrawDispCollPlane(CBaseTrace *pTrace) {
  ...
  Vector vecImpactPoint =
      pTrace->startpos + pTrace->fraction * (pTrace->endpos - pTrace->startpos);
  vecImpactPoint += vecNormal;

  // Generate a quad to represent the plane.
  Vector vecPlanePoints[4];
  vecPlanePoints[0] =
      vecImpactPoint + (vecBasisU * -flLength) + (vecBasisV * -flLength);
  vecPlanePoints[1] =
      vecImpactPoint + (vecBasisU * -flLength) + (vecBasisV * flLength);
  vecPlanePoints[2] =
      vecImpactPoint + (vecBasisU * flLength) + (vecBasisV * flLength);
  ...
  // Draw the plane.
  NDebugOverlay::Triangle(vecPlanePoints[0], vecPlanePoints[1],
                          vecPlanePoints[2], 125, 125, 125, 125, false, 5.0f);
  NDebugOverlay::Triangle(vecPlanePoints[0], vecPlanePoints[2],
                          vecPlanePoints[3], 125, 125, 125, 125, false, 5.0f);

  NDebugOverlay::Line(vecPlanePoints[0], vecPlanePoints[1], 255, 255, 255,
                      false, 5.0f);
  NDebugOverlay::Line(vecPlanePoints[1], vecPlanePoints[2], 255, 255, 255,
                      false, 5.0f);
  NDebugOverlay::Line(vecPlanePoints[2], vecPlanePoints[3], 255, 255, 255,
                      false, 5.0f);
  NDebugOverlay::Line(vecPlanePoints[3], vecPlanePoints[0], 255, 255, 255,
                      false, 5.0f);

  // Draw the normal.
  NDebugOverlay::Line(vecImpactPoint, vecImpactPoint + (vecNormal * flLength),
                      255, 0, 0, false, 5.0f);
}
#endif
...
int CGameMovement::GetCheckInterval(IntervalType_t type) {
  ...
  switch (type) {
    ...
    case GROUND:
      ...
      break;
    case STUCK:
      // If we are in the process of being "stuck", then try a new position
      // every command tick until m_StuckLast gets reset back down to zero
...
bool CGameMovement::CheckInterval(IntervalType_t type) {
  ...
  if (g_bMovementOptimizations) {
    return (player->CurrentCommandNumber() + player->entindex()) %
               tickInterval ==
           0;
  } else {
    return true;
...
CBaseHandle CGameMovement::TestPlayerPosition(const Vector &pos,
...
//			}
                ...
                if (footprintDecal != -1)
                {
                        ...
                        Vector hipOrigin;
                        VectorMA( pev->origin,
                                m_IsFootprintOnLeft ? -PLAYER_HALFWIDTH :
PLAYER_HALFWIDTH, right, hipOrigin );

                        // Find where that leg hits the ground
                        UTIL_TraceLine( hipOrigin, hipOrigin + Vector(0, 0,
-COORD_EXTENT * 1.74), MASK_SOLID_BRUSHONLY, edict(), COLLISION_GROUP_NONE,
&tr);
                        ...
                        te->FootprintDecal( filter, 0.0f, &tr.endpos, &right,
ENTINDEX(tr.u.ent), gDecals[footprintDecal].index, mType );
...
#define WET_TIME			    5.f	// how many seconds till
we're completely wet/dry #define DRY_TIME			   20.f	//
how many seconds till we're completely wet/dry
...
void CBasePlayer::UpdateWetness()
{
        ...
        trace_t tr;
        UTIL_TraceLine( pev->origin, pev->origin + Vector(0, 0, -COORD_EXTENT
* 1.74), MASK_SOLID_BRUSHONLY, edict(), COLLISION_GROUP_NONE, &tr); if
(tr.surface.flags & SURF_WET)
        {
                if (! (pev->flags & FL_INRAIN) )
                {
                        // Transition...
                        // Figure out how wet we are now (we were drying off...)
                        float wetness = (m_WetTime - gpGlobals->time) /
DRY_TIME; if (wetness < 0.0f) wetness = 0.0f;

                        // Here, wet time represents the time at which we get
totally wet m_WetTime = gpGlobals->time + (1.0 - wetness) * WET_TIME;
        ...
        else
        {
                ...
                {
                        // Transition...
                        // Figure out how wet we are now (we were getting more
wet...) float wetness = 1.0f + (gpGlobals->time - m_WetTime) / WET_TIME; if
(wetness > 1.0f) wetness = 1.0f;

                        // Here, wet time represents the time at which we get
totally dry m_WetTime = gpGlobals->time + wetness * DRY_TIME;
...
void CGameMovement::CategorizeGroundSurface(trace_t &pm) {
  ...
  physprops->GetPhysicsProperties(player->m_surfaceProps, NULL, NULL,
                                  &player->m_surfaceFriction, NULL);

  // HACKHACK: Scale this to fudge the relationship between vphysics friction
  // values and player friction values. A value of 0.8f feels pretty normal for
  // vphysics, whereas 1.0f is normal for players. This scaling trivially makes
  // them equivalent.  REVISIT if this affects low friction surfaces too much.
  player->m_surfaceFriction *= 1.25f;
  if (player->m_surfaceFriction > 1.0f) player->m_surfaceFriction = 1.0f;
...
//-----------------------------------------------------------------------------
float CGameMovement::ComputeConstraintSpeedFactor() {
  ...
  if (DotProduct(vecDelta, vecDesired) < 0.0f) return 1.0f;

  float flFrac =
      (sqrt(flDistSq) - (mv->m_flConstraintRadius - mv->m_flConstraintWidth)) /
...
void CGameMovement::CheckParameters() {
  ...
  if (!IsDead()) {
    ...
    if (player->GetMoveType() != MOVETYPE_ISOMETRIC &&
        player->GetMoveType() != MOVETYPE_NOCLIP) {
      mv->m_vecAngles[ROLL] =
          CalcRoll(v_angle, mv->m_vecVelocity, sv_rollangle.GetFloat(),
                   sv_rollspeed.GetFloat());
    } else {
...
void CGameMovement::ProcessMovement(CBasePlayer *pPlayer, CMoveData *pMove) {
  ...
  DiffPrint("start %f %f %f", mv->GetAbsOrigin().x, mv->GetAbsOrigin().y,
            mv->GetAbsOrigin().z);
  ...
  DiffPrint("end %f %f %f", mv->GetAbsOrigin().x, mv->GetAbsOrigin().y,
            mv->GetAbsOrigin().z);
...
//-----------------------------------------------------------------------------
void CGameMovement::FinishMove() { mv->m_nOldButtons = mv->m_nButtons; }

#define PUNCH_DAMPING \
  9.0f  // bigger number makes the response more damped, smaller is less damped
        // currently the system will overshoot, with larger damping values it
        // won't
#define PUNCH_SPRING_CONSTANT \
  65.0f  // bigger number increases the speed at which the view corrects

//-----------------------------------------------------------------------------
// Purpose: Decays the punchangle toward 0,0,0.
//			Modelled as a damped spring
//-----------------------------------------------------------------------------
void CGameMovement::DecayPunchAngle() {
  if (player->m_Local.m_vecPunchAngle->LengthSqr() > 0.001 ||
      player->m_Local.m_vecPunchAngleVel->LengthSqr() > 0.001) {
    player->m_Local.m_vecPunchAngle +=
...
void CGameMovement::CheckWaterJump() {
  ...
  // Are we backing into water from steps or something?  If so, don't pop
  // forward
  if (curspeed != 0.0 && (DotProduct(flatvelocity, flatforward) < 0.0)) return;

  Vector vecStart;
  // Start line trace at waist height (using the center of the player for this
  // here)
  ...
  trace_t tr;
  TracePlayerBBox(vecStart, vecEnd, PlayerSolidMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, tr);
  if (tr.fraction < 1.0)  // solid at waist
  {
    IPhysicsObject *pPhysObj = tr.m_pEnt->VPhysicsGetObject();
    if (pPhysObj) {
      if (pPhysObj->GetGameFlags() & FVPHYSICS_PLAYER_HELD) return;
    }

    vecStart.z =
        mv->GetAbsOrigin().z + player->GetViewOffset().z + WATERJUMP_HEIGHT;
    VectorMA(vecStart, 24.0f, flatforward, vecEnd);
...
void CGameMovement::WaterJump() {
  if (player->m_flWaterJumpTime > 10000) player->m_flWaterJumpTime = 10000;
...
void CGameMovement::WaterMove() {
  ...
  AngleVectors(mv->m_vecViewAngles, &forward, &right,
               &up);  // Determine movement angles
  ...
  if (speed) {
    newspeed = speed - gpGlobals->frametime * speed * sv_friction.GetFloat() *
  ...
  if (wishspeed >= 0.1f)  // old !
  {
    ...
    if (addspeed > 0) {
      ...
      accelspeed = sv_accelerate.GetFloat() * wishspeed * gpGlobals->frametime *
                   player->m_surfaceFriction;
  ...
  if (pm.fraction == 1.0f) {
    ...
    TracePlayerBBox(start, dest, PlayerSolidMask(),
                    COLLISION_GROUP_PLAYER_MOVEMENT, pm);
...
void CGameMovement::StepMove(Vector &vecDestination, trace_t &trace) {
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), vecEndPos, PlayerSolidMask(),
...
void CGameMovement::Friction() {
  ...
  if (player->GetGroundEntity() != NULL)  // On an entity that is the ground
  {
    ...
    control =
        (speed < sv_stopspeed.GetFloat()) ? sv_stopspeed.GetFloat() : speed;
...
void CGameMovement::FinishGravity() {
  ...
  mv->m_vecVelocity[2] -=
      (ent_gravity * sv_gravity.GetFloat() * gpGlobals->frametime * 0.5);
...
//			accel -
//-----------------------------------------------------------------------------
void CGameMovement::AirAccelerate(Vector &wishdir, float wishspeed,
                                  float accel) {
  int i;
  ...
  accelspeed =
      accel * wishspeed * gpGlobals->frametime * player->m_surfaceFriction;
...
void CGameMovement::AirMove() {
  ...
  // Now pull the base velocity back out.   Base velocity is set if you are on a
  // moving object, like a conveyor (or maybe another monster?)
  VectorSubtract(mv->m_vecVelocity, player->GetBaseVelocity(),
                 mv->m_vecVelocity);
}
...
void CGameMovement::Accelerate(Vector &wishdir, float wishspeed, float accel) {
  ...
  // This gets overridden because some games (CSPort) want to allow dead
  // (observer) players to be able to move around.
  ...
  accelspeed =
      accel * gpGlobals->frametime * wishspeed * player->m_surfaceFriction;
...
//-----------------------------------------------------------------------------
// Purpose: Try to keep a walking player on the ground when running down slopes
// etc
//-----------------------------------------------------------------------------
void CGameMovement::StayOnGround() {
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), start, PlayerSolidMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, trace);
  start = trace.endpos;
  ...
  TracePlayerBBox(start, end, PlayerSolidMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, trace);
  if (trace.fraction > 0.0f &&  // must go somewhere
      trace.fraction < 1.0f &&  // must hit something
      !trace.startsolid &&      // can't be embedded in a solid
      trace.plane.normal[2] >=
          0.7)  // can't hit a steep slope that we can't stand on anyway
  {
    float flDelta = fabs(mv->GetAbsOrigin().z - trace.endpos.z);

    // This is incredibly hacky. The real problem is that trace returning that
    // strange value we can't network over.
...
void CGameMovement::WalkMove() {
  ...
  if (spd < 1.0f) {
    ...
    // Now pull the base velocity back out.   Base velocity is set if you are on
    // a moving object, like a conveyor (or maybe another monster?)
    VectorSubtract(mv->m_vecVelocity, player->GetBaseVelocity(),
                   mv->m_vecVelocity);
    return;
  }
...
void CGameMovement::FullWalkMove() {
  ...
  // can jump up out
  //  of, and, if so, start out jump.  Otherwise, if we are not moving up, then
  //  reset jump timer to 0
  if (player->GetWaterLevel() >= WL_Waist) {
    if (player->GetWaterLevel() == WL_Waist) {
      CheckWaterJump();
    }

    // If we are falling again, then we must not trying to jump out of water any
    // more.
  ...
  // Not fully underwater
  {
    ...
    // Fricion is handled before we add in any base velocity. That way, if we
    // are on a conveyor,
...
bool CGameMovement::CheckJumpButton() {
  ...
  if (player->m_flWaterJumpTime) {
    ...
    if (player->m_flWaterJumpTime < 0) player->m_flWaterJumpTime = 0;
  ...
  if (player->GetWaterLevel() >= 2) {
    ...
    if (player->GetWaterType() ==
        CONTENTS_WATER)  // We move up a certain amount
  ...
  player->PlayStepSound((Vector &)mv->GetAbsOrigin(), player->m_pSurfaceData,
                        1.0, true);
  ...
  if (gpGlobals->maxClients == 1) {
    ...
    // We give a certain percentage of the current forward movement as a bonus
    // to the jump speed.  That bonus is clipped to not accumulate over time.
    float flSpeedBoostPerc =
        (!pMoveData->m_bIsSprinting && !player->m_Local.m_bDucked) ? 0.5f
                                                                   : 0.1f;
    float flSpeedAddition = fabs(mv->m_flForwardMove * flSpeedBoostPerc);
    float flMaxSpeed = mv->m_flMaxSpeed + (mv->m_flMaxSpeed * flSpeedBoostPerc);
    float flNewSpeed = (flSpeedAddition + mv->m_vecVelocity.Length2D());

    // If we're over the maximum, we want to only boost as much as will get us
    // to the goal speed
    ...
    // Add it on
    VectorAdd((vecForward * flSpeedAddition), mv->m_vecVelocity,
              mv->m_vecVelocity);
  }
#endif
...
int CGameMovement::TryPlayerMove(Vector *pFirstDest, trace_t *pFirstTrace) {
  ...
  for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
    if (mv->m_vecVelocity.Length() == 0.0) break;
    ...
    if (g_bMovementOptimizations) {
      // If their velocity Z is 0, then we can avoid an extra trace here during
      // WalkMove.
...
#if defined(PLAYER_GETTING_STUCK_TESTING)
        trace_t foo;
        TracePlayerBBox(mv->GetAbsOrigin(), mv->GetAbsOrigin(),
...
//-----------------------------------------------------------------------------
// Purpose: Determine whether or not the player is on a ladder (physprop or
// world).
//-----------------------------------------------------------------------------
inline bool CGameMovement::OnLadder(trace_t &trace) {
  ...
  if (pPhysProps) {
    const surfacedata_t *pSurfaceData =
        pPhysProps->GetSurfaceData(trace.surface.surfaceProps);
    if (pSurfaceData) {
      if (pSurfaceData->game.climbable != 0) return true;
    }
  }
...
bool CGameMovement::LadderMove() {
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), end, LadderMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, pm);
  ...
  if (mv->m_nButtons & IN_BACK) forwardSpeed -= climbSpeed;

  if (mv->m_nButtons & IN_FORWARD) forwardSpeed += climbSpeed;

  if (mv->m_nButtons & IN_MOVELEFT) rightSpeed -= climbSpeed;

  if (mv->m_nButtons & IN_MOVERIGHT) rightSpeed += climbSpeed;
  ...
  if (mv->m_nButtons & IN_JUMP) {
    ...
    if (forwardSpeed != 0 || rightSpeed != 0) {
      ...
      // Vector velocity = (forward * gpGlobals->v_forward) + (right *
      // gpGlobals->v_right);
      ...
      // because the velocity is a sum of the directional velocity and the
      // converted velocity through the face of the ladder -- by design.
      CrossProduct(pm.plane.normal, perp, tmp);
      VectorMA(lateral, -normal, tmp, mv->m_vecVelocity);

      if (onFloor && normal > 0)  // On ground moving away from the ladder
      {
        VectorMA(mv->m_vecVelocity, MAX_CLIMB_SPEED, pm.plane.normal,
                 mv->m_vecVelocity);
      }
      // pev->velocity = lateral - (CrossProduct( trace.vecPlaneNormal, perp ) *
      // normal);
    } else {
      mv->m_vecVelocity.Init();
    }
...
// Purpose:
//-----------------------------------------------------------------------------
void CGameMovement::CheckVelocity() {
  int i;
...
void CGameMovement::AddGravity() {
  ...
  mv->m_vecVelocity[2] -=
      (ent_gravity * sv_gravity.GetFloat() * gpGlobals->frametime);
  mv->m_vecVelocity[2] += player->GetBaseVelocity()[2] * gpGlobals->frametime;
  Vector temp = player->GetBaseVelocity();
  temp[2] = 0;
  player->SetBaseVelocity(temp);
...
void CGameMovement::PushEntity(Vector &push, trace_t *pTrace) {
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), end, PlayerSolidMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, *pTrace);
...
// Output : int
//-----------------------------------------------------------------------------
int CGameMovement::ClipVelocity(Vector &in, Vector &normal, Vector &out,
                                float overbounce) {
  float backoff;
  float change;
  float angle;
...
// Output : float
//-----------------------------------------------------------------------------
float CGameMovement::CalcRoll(const QAngle &angles, const Vector &velocity,
...
int CGameMovement::CheckStuck() {
  ...
  hitent = TestPlayerPosition(mv->GetAbsOrigin(),
                              COLLISION_GROUP_PLAYER_MOVEMENT, traceresult);
  ...
  if (developer.GetBool()) {
    ...
    engine->Con_NPrintf(isServer, "%s stuck on object %i/%s",
                        isServer ? "server" : "client", hitent.GetEntryIndex(),
  ...
  if (!player->IsServer()) {
    if (MoveHelper()->IsWorldEntity(hitent)) {
      ...
      do {
        ...
        if (TestPlayerPosition(test, COLLISION_GROUP_PLAYER_MOVEMENT,
                               traceresult) == INVALID_ENTITY_HANDLE) {
          ResetStuckOffsets(player);
  ...
  fTime = engine->Time();
  // Too soon?
  if (m_flStuckCheckTime[player->entindex()][idx] >=
      fTime - CHECKSTUCK_MINTIME) {
    return 1;
  }
  ...
  if (TestPlayerPosition(test, COLLISION_GROUP_PLAYER_MOVEMENT, traceresult) ==
...
// Output : bool
//-----------------------------------------------------------------------------
bool CGameMovement::InWater() { return (player->GetWaterLevel() > WL_Feet); }
...
int CGameMovement::GetPointContentsCached(const Vector &point, int slot) {
  if (g_bMovementOptimizations) {
    ...
    if (m_CachedGetPointContents[idx][slot] == -9999 ||
        point.DistToSqr(m_CachedGetPointContentsPoint[idx][slot]) > 1) {
      m_CachedGetPointContents[idx][slot] =
          enginetrace->GetPointContents(point);
...
bool CGameMovement::CheckWater() {
  ...
  point[0] =
      mv->GetAbsOrigin()[0] + (GetPlayerMins()[0] + GetPlayerMaxs()[0]) * 0.5;
  point[1] =
      mv->GetAbsOrigin()[1] + (GetPlayerMins()[1] + GetPlayerMaxs()[1]) * 0.5;
  ...
  if (cont & MASK_WATER) {
    ...
    point[2] =
        mv->GetAbsOrigin()[2] + (GetPlayerMins()[2] + GetPlayerMaxs()[2]) * 0.5;
    ...
    if (cont & MASK_CURRENT) {
      Vector v;
      VectorClear(v);
      if (cont & CONTENTS_CURRENT_0) v[0] += 1;
      if (cont & CONTENTS_CURRENT_90) v[1] += 1;
      if (cont & CONTENTS_CURRENT_180) v[0] -= 1;
      if (cont & CONTENTS_CURRENT_270) v[1] -= 1;
      if (cont & CONTENTS_CURRENT_UP) v[2] += 1;
      if (cont & CONTENTS_CURRENT_DOWN) v[2] -= 1;

      // BUGBUG -- this depends on the value of an unspecified enumerated type
      // The deeper we are, the stronger the current.
      Vector temp;
      VectorMA(player->GetBaseVelocity(), 50.0 * player->GetWaterLevel(), v,
               temp);
      player->SetBaseVelocity(temp);
    }
  }

  // if we just transitioned from not in water to in water, record the time it
  // happened
...
void CGameMovement::SetGroundEntity(trace_t *pm) {
  ...
  if (newGround) {
    ...
    // Standing on an entity other than the world, so signal that we are
    // touching something.
    if (!pm->DidHitWorld()) {
      MoveHelper()->AddToTouched(*pm, mv->m_vecVelocity);
    }
...
//-----------------------------------------------------------------------------
void TracePlayerBBoxForGround(const Vector &start, const Vector &end,
                              const Vector &minsSrc, const Vector &maxsSrc,
                              IHandleEntity *player, unsigned int fMask,
                              int collisionGroup, trace_t &pm) {
...
void CGameMovement::CategorizePosition() {
  ...
  // Reset this each time we-recategorize, otherwise we have bogus friction when
  // we jump into water and plunge downward really quickly
  ...
  // the amount we move up is less than the 1 pixel 'threshold' we're about to
  // snap to.	Also, we'll call this several times per frame, so we really need
  // to avoid sticking to the bottom of water on each call, and the converse
  // case will correct itself if called twice.
  CheckWater();
...
#define NON_JUMP_VELOCITY 140.0f
  ...
  if (bMovingUpRapidly) {
    ...
    // After save/restore (and maybe at other times), we can get a case where we
    // were saved on a lift and
    //  after restore we'll have a high local velocity due to the lift making
    //  our abs velocity appear high.
    // We need to account for standing on a moving ground object in that case in
    // order to determine if we really
    //  are moving away from the object we are standing on at too rapid a speed.
    //  Note that CheckJump already sets ground entity to NULL, so this wouldn't
    //  have any effect unless we are moving up rapidly not from the jump
    //  button.
  ...
  if (bMovingUpRapidly ||
    ...
    // Try and move down.
    TracePlayerBBox(bumpOrigin, point, MASK_PLAYERSOLID,
                    COLLISION_GROUP_PLAYER_MOVEMENT, pm);

    // Was on ground, but now suddenly am not.  If we hit a steep plane, we are
    // not on ground
    if (!pm.m_pEnt || pm.plane.normal[2] < 0.7) {
      // Test four sub-boxes, to see if any of them would have found shallower
      // slope we could actually stand on
      TracePlayerBBoxForGround(bumpOrigin, point, GetPlayerMins(),
                               GetPlayerMaxs(), mv->m_nPlayerHandle.Get(),
                               MASK_PLAYERSOLID,
                               COLLISION_GROUP_PLAYER_MOVEMENT, pm);
      if (!pm.m_pEnt || pm.plane.normal[2] < 0.7) {
        SetGroundEntity(NULL);
    ...
    if (player->IsInAVehicle() == false) {
      // If our gamematerial has changed, tell any player surface triggers that
      // are watching
      IPhysicsSurfaceProps *physprops = MoveHelper()->GetSurfaceProps();
      surfacedata_t *pSurfaceProp =
          physprops->GetSurfaceData(pm.surface.surfaceProps);
  ...
  if (player->GetGroundEntity() != NULL && !IsDead() &&
    ...
    if (player->GetWaterLevel() > 0) {
      ...
      if (player->GetGroundEntity()->GetAbsVelocity().z < 0.0f) {
        // Player landed on a descending object. Subtract the velocity of the
        // ground entity.
      ...
      if (player->m_Local.m_flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED) {
        //
        // If they hit the ground going this fast they may take damage (and
        // die).
        //
        bAlive = MoveHelper()->PlayerFallingDamage();
        fvol = 1.0;
      } else if (player->m_Local.m_flFallVelocity >
                 PLAYER_MAX_SAFE_FALL_SPEED / 2) {
        fvol = 0.85;
...
void CGameMovement::PlayerRoughLandingEffects(float fvol) {
  if (fvol > 0.0) {
    ...
    player->m_Local.m_vecPunchAngle.Set(
        ROLL, player->m_Local.m_flFallVelocity * 0.013);
...
#if !defined(CLIENT_DLL)
    player->RumbleEffect(
        (fvol > 0.85f) ? (RUMBLE_FALL_LONG) : (RUMBLE_FALL_SHORT), 0,
        RUMBLE_FLAGS_NONE);
#endif
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: Use for ease-in, ease-out style interpolation (accel/decel)  Used by
// ducking code. Input  : value -
//			scale -
// Output : float
//-----------------------------------------------------------------------------
...
void CGameMovement::FixPlayerCrouchStuck(bool upward) {
  ...
  hitent = TestPlayerPosition(mv->GetAbsOrigin(),
                              COLLISION_GROUP_PLAYER_MOVEMENT, dummy);
  if (hitent == INVALID_ENTITY_HANDLE) return;
...
bool CGameMovement::CanUnduck() {
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), newOrigin, PlayerSolidMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, trace);
  ...
  if (player->m_Local.m_flDuckJumpTime != 0.0f) {
    ...
    if (flDuckSeconds > TIME_TO_UNDUCK) {
      ...
      float flDuckFraction =
          SimpleSpline(1.0f - (flDuckSeconds / TIME_TO_UNDUCK));
...
bool CGameMovement::CanUnDuckJump(trace_t &trace) {
  ...
  TracePlayerBBox(mv->GetAbsOrigin(), vecEnd, PlayerSolidMask(),
                  COLLISION_GROUP_PLAYER_MOVEMENT, trace);
  if (trace.fraction < 1.0f) {
    ...
    player->m_Local.m_bDucked = false;
    TracePlayerBBox(vecEnd, vecEnd, PlayerSolidMask(),
                    COLLISION_GROUP_PLAYER_MOVEMENT, traceUp);
    player->m_Local.m_bDucked = bWasDucked;
    if (!traceUp.startsolid) return true;
  }
...
void CGameMovement::Duck() {
  int buttonsChanged =
      (mv->m_nOldButtons ^
       mv->m_nButtons);  // These buttons have changed this frame
  int buttonsPressed =
      buttonsChanged &
      mv->m_nButtons;  // The changed ones still down are "pressed"
  int buttonsReleased =
      buttonsChanged & mv->m_nOldButtons;  // The changed ones which were
                                           // previously down are "released"
  ...
  // If the player is holding down the duck button, the player is in duck
  // transition, ducking, or duck-jumping.
  if ((mv->m_nButtons & IN_DUCK) || player->m_Local.m_bDucking || bInDuck ||
      bDuckJump) {
    // DUCK
    if ((mv->m_nButtons & IN_DUCK) || bDuckJump) {
      // Have the duck button pressed, but the player currently isn't in the
      // duck position.
      if ((buttonsPressed & IN_DUCK) && !bInDuck && !bDuckJump &&
          !bDuckJumpTime) {
      ...
      if (player->m_Local.m_bDucking && !bDuckJump && !bDuckJumpTime) {
        ...
        // Finish in duck transition when transition time is over, in "duck", in
        // air.
    ...
    // UNDUCK (or attempt to...)
      ...
      if (player->m_Local.m_bAllowAutoMovement || bInAir ||
        ...
        // We released the duck button, we aren't in "duck" and we are not in
        // the air - start unduck transition.
        ...
        if (CanUnduck()) {
          ...
          // Still under something where we can't unduck, so make sure we reset
          // this timer so
  ...
  // HACK: (jimd 5/25/2006) we have a reoccuring bug (#50063 in Tracker) where
  // the player's view height gets left at the ducked height while the player is
  // standing, but we haven't been  able to repro it to find the cause.  It may
  // be fixed now due to a change I'm also making in UpdateDuckJumpEyeOffset but
  // just in case, this code will sense the problem and restore the eye to the
  // proper position.  It doesn't smooth the transition, but it is preferable to
  // leaving the player's view too low.
  //
  ...
  // his view height is at the standing height.
    ...
    if ((player->m_Local.m_flDuckJumpTime == 0.0f) &&
        (fabs(player->GetViewOffset().z - GetPlayerViewOffset(false).z) >
         0.1)) {
      // we should rarely ever get here, so assert so a coder knows when it
      // happens
      Assert(0);
...
static ConVar sv_optimizedmovement("sv_optimizedmovement", "1",
...
void CGameMovement::PlayerMove() {
  ...
  AngleVectors(mv->m_vecViewAngles, &m_vecForward, &m_vecRight,
               &m_vecUp);  // Determine movement angles

  // Always try and unstick us unless we are using a couple of the movement
  // modes
  ...
  // Now that we are "unstuck", see where we are (player->GetWaterLevel() and
  // type, player->GetGroundEntity()).
  ...
  player->UpdateStepSound(player->m_pSurfaceData, mv->GetAbsOrigin(),
                          mv->m_vecVelocity);
...
void CGameMovement::PerformFlyCollisionResolution(trace_t &pm, Vector &move) {
  ...
  if (pm.plane.normal[2] > 0.7) {
    ...
    if (vel < (30 * 30) ||
        (player->GetMoveCollide() != MOVECOLLIDE_FLY_BOUNCE)) {
      SetGroundEntity(&pm);
      mv->m_vecVelocity.Init();
    } else {
      VectorScale(mv->m_vecVelocity,
                  (1.0 - pm.fraction) * gpGlobals->frametime * 0.9, move);
      PushEntity(move, &pm);
    }
...
void CGameMovement::IsometricMove() {
  int i;
  Vector wishvel;
  float fmove, smove;
  Vector forward, right, up;
  ...
  // Zero out the velocity so that we don't accumulate a huge downward velocity
  // from
  //  gravity, etc.
  mv->m_vecVelocity.Init();
}
