...
void SetActivityForSequence(CStudioHdr *pstudiohdr, int i) {
  ...
  if (pszActivityName[0] != '\0') {
    ...
    if (iActivityIndex == -1) {
      // Allow this now.  Animators can create custom activities that are
      // referenced only on the client or by scripts, etc.
      // Warning( "***\nModel %s tried to reference unregistered activity: %s
      // \n***\n", pstudiohdr->name, pszActivityName );  Assert(0);
      // HACK: the client and server don't share the private activity list so
      // registering it on the client would hose the server
#ifdef CLIENT_DLL
      seqdesc.flags &= ~STUDIO_ACTIVITY;
#else
...
int SelectWeightedSequence(CStudioHdr *pstudiohdr, int activity,
  ...
  for (int i = 0; i < pstudiohdr->GetNumSeq(); i++) {
    ...
    if (curActivity == activity) {
      ...
      if (IsInPrediction())
        randomValue =
            SharedRandomInt("SelectWeightedSequence", 0, weighttotal - 1, i);
      else
        randomValue = RandomInt(0, weighttotal - 1);

      if (!weighttotal || randomValue < iabs(weight)) seq = i;
    }
  }
...
// Pick a sequence for the given activity. If the current sequence is
// appropriate for the current activity, and its stored weight is negative
// (whatever that means), always select it. Otherwise perform a weighted
// selection -- imagine a large roulette wheel, with each sequence having a
// number of spaces corresponding to its weight.
int CStudioHdr::CActivityToSequenceMapping::SelectWeightedSequence(
    CStudioHdr *pstudiohdr, int activity, int curSequence) {
  if (!ValidateAgainst(pstudiohdr)) {
    AssertMsg1(false,
               "CStudioHdr %s has changed its vmodel pointer without "
  ...
  // a 0 m_pSequenceTuples just means that this studio header has no
  // activities.
  ...
  // chug through the entries in the list (they are sequential therefore
  // cache-coherent) until we run out of random juice
  SequenceTuple *__restrict sequenceInfo =
      m_pSequenceTuples + actData->startingIdx;

  const SequenceTuple *const stopHere =
      sequenceInfo + actData->count;  // this is a backup
                                      // in case the weights are somehow
                                      // miscalculated -- we don't read or write
                                      // through it (because it aliases the
                                      // restricted pointer above); it's only
...
int LookupSequence(CStudioHdr *pstudiohdr, const char *label) {
  ...
  for (int i = 0; i < pstudiohdr->GetNumSeq(); i++) {
    mstudioseqdesc_t &seqdesc = pstudiohdr->pSeqdesc(i);
    if (stricmp(seqdesc.pszLabel(), label) == 0) return i;
  }
...
void GetSequenceLinearMotion(CStudioHdr *pstudiohdr, int iSequence,
  ...
  if (iSequence < 0 || iSequence >= pstudiohdr->GetNumSeq()) {
    ...
    if (pstudiohdr->GetNumSeq() > 0) {
      ...
      while (++msgCount < 10) {
        Msg("Bad sequence (%i out of %i max) in GetSequenceLinearMotion() for "
            "model '%s'!\n",
  ...
  QAngle vecAngles;
  Studio_SeqMovement(pstudiohdr, iSequence, 0, 1.0, poseParameter, (*pVec),
                     vecAngles);
}
#endif
...
int GetSequenceFlags(CStudioHdr *pstudiohdr, int sequence) {
  if (!pstudiohdr || !pstudiohdr->SequencesAvailable() || sequence < 0 ||
...
bool HasAnimationEventOfType(CStudioHdr *pstudiohdr, int sequence, int type) {
  if (!pstudiohdr || sequence >= pstudiohdr->GetNumSeq()) return false;

  mstudioseqdesc_t &seqdesc = pstudiohdr->pSeqdesc(sequence);
  if (!&seqdesc) return false;

  mstudioevent_t *pevent = GetEventIndexForSequence(seqdesc);
  if (!pevent) return false;

  if (seqdesc.numevents == 0) return false;
...
int GetAnimationEvent(CStudioHdr *pstudiohdr, int sequence,
  ...
  mstudioseqdesc_t &seqdesc = pstudiohdr->pSeqdesc(sequence);
  if (seqdesc.numevents == 0 || index >= (int)seqdesc.numevents) return 0;

  // Msg( "flStart %f flEnd %f (%d) %s\n", flStart, flEnd, seqdesc.numevents,
  // seqdesc.label );
  mstudioevent_t *pevent = GetEventIndexForSequence(seqdesc);
  for (; index < (int)seqdesc.numevents; index++) {
    // Don't send client-side events to the server AI
    if (pevent[index].type & AE_TYPE_NEWEVENTSYSTEM) {
      if (!(pevent[index].type & AE_TYPE_SERVER)) continue;
    } else if (pevent[index].event >=
               EVENT_CLIENT)  // Adrian - Support the old event system
      continue;
...
int FindTransitionSequence(CStudioHdr *pstudiohdr, int iCurrentSequence,
  ...
  if (!pstudiohdr) return iGoalSequence;
  ...
  if (pstudiohdr->EntryNode(iCurrentSequence) == 0 ||
      pstudiohdr->EntryNode(iGoalSequence) == 0) {
    *piDir = 1;
    return iGoalSequence;
  }
  ...
  int iInternNode =
      pstudiohdr->GetTransition(iEndNode, pstudiohdr->EntryNode(iGoalSequence));
  ...
  for (i = 0; i < pstudiohdr->GetNumSeq(); i++) {
    mstudioseqdesc_t &seqdesc = pstudiohdr->pSeqdesc(i);
    if (pstudiohdr->EntryNode(i) == iEndNode &&
        pstudiohdr->ExitNode(i) == iInternNode) {
      *piDir = 1;
      return i;
    }
    if (seqdesc.nodeflags) {
      if (pstudiohdr->ExitNode(i) == iEndNode &&
          pstudiohdr->EntryNode(i) == iInternNode) {
        *piDir = -1;
        return i;
      }
    }
  ...
  DevMsg(2, "error in transition graph: %s to %s\n",
         pstudiohdr->pszNodeName(iEndNode),
         pstudiohdr->pszNodeName(pstudiohdr->EntryNode(iGoalSequence)));
...
bool GotoSequence(CStudioHdr *pstudiohdr, int iCurrentSequence,
  ...
  if (pstudiohdr->EntryNode(iCurrentSequence) !=
      pstudiohdr->ExitNode(iCurrentSequence)) {
    // are we done with it?
...
int GetBodygroup(CStudioHdr *pstudiohdr, int body, int iGroup) {
  ...
  if (pbodypart->nummodels <= 1) return 0;
...
int GetSequenceActivity(CStudioHdr *pstudiohdr, int sequence, int *pweight) {
  ...
  if (pweight) *pweight = seqdesc.actweight;
  return seqdesc.activity;
}
...
void GetAttachmentLocalSpace(CStudioHdr *pstudiohdr, int attachIndex,
  ...
  if (attachIndex >= 0) {
    const mstudioattachment_t &pAttachment =
        pstudiohdr->pAttachment(attachIndex);
...
int FindHitboxSetByName(CStudioHdr *pstudiohdr, const char *name) {
  ...
  for (int i = 0; i < pstudiohdr->numhitboxsets(); i++) {
    ...
    if (!stricmp(set->pszName(), name)) return i;
  }
