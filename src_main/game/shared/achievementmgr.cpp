...
ConVar cc_achievement_debug("achievement_debug", "0",
...
ISteamUserStats *SteamUserStats() { return NULL; }
#endif
...
class CAchievementSaveThread : public CWorkerThread {
 public:
  CAchievementSaveThread() : m_pKV(NULL) { SetName("AchievementSaveThread"); }
...
CAchievementMgr::CAchievementMgr()
    : CAutoGameSystem("CAchievementMgr")
#if !defined(NO_STEAM)
      ,
      m_CallbackUserStatsReceived(this,
...
bool CAchievementMgr::Init() {
  ...
  // (for single player), so register ourselves with the engine so UI has a
  // uniform place to go get the pointer to us

#ifdef _DEBUG
  // There can be only one achievement manager instance; no one else should be
  // registered
...
//			do this in PostInit because the Steam app ID is not
// available earlier
//-----------------------------------------------------------------------------
void CAchievementMgr::PostInit() {
  ...
  while (pAchievementHelper) {
    ...
    // only add an achievement if it does not have a game filter (only compiled
    // into the game it applies to, or truly cross-game) or, if it does have a
    // game filter, the filter matches current game. (e.g. EP 1/2/...
    // achievements are in shared binary but are game specific, they have a game
    // filter for runtime check.)
...
void CAchievementMgr::Shutdown() {
  ...
  FOR_EACH_MAP(m_mapAchievement, iter) { delete m_mapAchievement[iter]; }
...
void CAchievementMgr::LevelInitPreEntity() {
  // load global state if we haven't already; X360 users may not have had a
  // storage device available or selected at boot time
  EnsureGlobalStateLoaded();

#ifdef GAME_DLL
  // For single-player games, achievement mgr must live on the server.  (Only
  // the server has detailed knowledge of game state.)
  Assert(!GameRules()->IsMultiplayer());
#else
  // For multiplayer games, achievement mgr must live on the client.  (Only the
  // client can read/write player state from Steam/XBox Live.)
  Assert(GameRules()->IsMultiplayer());
#endif
  ...
  // client and server have map names available in different forms (full path on
  ...
  FOR_EACH_MAP(m_mapAchievement, iAchievement) {
    ...
    // if the achievement only applies to a specific map, and it's not the
    // current map, skip it
    const char *pMapNameFilter = pAchievement->m_pMapNameFilter;
    if (pMapNameFilter && (0 != Q_strcmp(m_szMap, pMapNameFilter))) continue;
...
//-----------------------------------------------------------------------------
// Purpose: returns achievement with specified name.  NOTE: this iterates
// through
//			all achievements to find the name, intended for
// debugging  purposes. 			Use GetAchievementByID for fast
// lookup.
//-----------------------------------------------------------------------------
...
#ifndef NO_STEAM
  if (SteamUserStats()) {
    // request stat download; will get called back at OnUserStatsReceived when
    // complete
...
#ifndef NO_STEAM
    if (SteamUserStats()) {
      // Upload current Steam client achievements & stats state to Steam.  Will
      // get called back at OnUserStatsStored when complete. Only values
      // previously set via SteamUserStats() get uploaded
      ...
      if (iAchievementID > 0) {
        ...
        if (pAchievement) {
          ...
          if (pAchievement->HasComponents()) {
            int64_t iComponentBits = pNode->GetUint64("data");
...
void CAchievementMgr::AwardAchievement(int iAchievementID) {
  ...
  // save state at next good opportunity.  (Don't do it immediately, may hitch
  // at bad time.)
  m_bDirty = true;
  ...
  if (SteamUserStats()) {
    ...
    CGameID gameID(engine->GetAppID());
    bool bRet =
        SteamUserStats()->SetAchievement(gameID, pAchievement->GetName());
    //		Assert( bRet );
    if (bRet) {
...
bool CAchievementMgr::CheckAchievementsEnabled() {
  ...
  if (IsPC()) {
    ...
    if (sv_cheats.GetBool()) {
      // Cheats get turned on automatically if you run with -dev which many
      // people do internally, so allow cheats if developer is turned on and
      // we're not running on Steam public
...
//			on her friends list, and if there are at least the
// specified # of 			teammates.  Involves cross-process calls
// to Steam so this is  mildly 			expensive, don't call this every
// frame.
//-----------------------------------------------------------------------------
bool CalcPlayersOnFriendsList(int iMinFriends) {
  ...
  if (!g_pGameRules->IsMultiplayer()) return false;

  // Do a cheap rejection: check teammate count first to see if we even need to
  // bother checking w/Steam Subtract 1 for the local player.
  if (CalcPlayerCount() - 1 < iMinFriends) return false;
  ...
  for (int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++) {
    ...
    if ((iPlayerIndex != iLocalPlayerIndex) &&
        (g_PR->IsConnected(iPlayerIndex))) {
      player_info_t pi;
      if (!engine->GetPlayerInfo(iPlayerIndex, &pi)) continue;
      if (!pi.friendsID) continue;
#ifndef NO_STEAM
      // check and see if they're on the local player's friends list
      CSteamID steamID(pi.friendsID, 1, SteamUtils()->GetConnectedUniverse(),
                       k_EAccountTypeIndividual);
      if (!SteamFriends()->HasFriend(steamID,
...
//			to same clan as local player. Involves cross-process
// calls  to Steam 			so this is mildly expensive, don't call
// this every  frame.
//-----------------------------------------------------------------------------
bool CalcHasNumClanPlayers(int iClanTeammates) {
  Assert(g_pGameRules->IsMultiplayer());
#ifndef NO_STEAM
  // Do a cheap rejection: check teammate count first to see if we even need
  // to bother checking w/Steam Subtract 1 for the local player.
  if (CalcPlayerCount() - 1 < iClanTeammates) return false;
    ...
    for (int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++) {
      if ((iPlayerIndex != iLocalPlayerIndex) &&
        ...
        if (engine->GetPlayerInfo(iPlayerIndex, &pi) && (pi.friendsID)) {
          // check and see if they're on the local player's friends list
          CSteamID steamID(pi.friendsID, 1,
                           SteamUtils()->GetConnectedUniverse(),
                           k_EAccountTypeIndividual);
          if (SteamFriends()->IsUserInSource(steamID, clanID)) {
            iClanMembersOnTeam++;
            if (iClanMembersOnTeam == iClanTeammates) return true;
          }
        }
      }
    }
...
int CalcTeammateCount() {
  ...
  for (int iPlayerIndex = 1; iPlayerIndex <= MAX_PLAYERS; iPlayerIndex++) {
    ...
    if ((iPlayerIndex != iLocalPlayerIndex) &&
        (g_PR->IsConnected(iPlayerIndex)) &&
        (g_PR->GetTeam(iPlayerIndex) == iLocalPlayerTeam)) {
      iNumTeammates++;
    }
  }
...
void CAchievementMgr::ResetAchievements() {
  if (!LoggedIntoSteam()) {
    Msg("Steam not running, achievements disabled. Cannot reset "
        "achievements.\n");
    return;
  }
...
void CAchievementMgr::PrintAchievementStatus() {
  ...
  FOR_EACH_MAP(m_mapAchievement, i) {
    ...
    CFailableAchievement *pFailableAchievement =
        dynamic_cast<CFailableAchievement *>(pAchievement);
    if (pAchievement->IsAchieved()) {
      Msg("%-20s", "ACHIEVED");
...
void CAchievementMgr::FireGameEvent(IGameEvent *event) {
  VPROF_("CAchievementMgr::FireGameEvent", 1, VPROF_BUDGETGROUP_STEAM, false,
         0);
  const char *name = event->GetName();
  if (0 == Q_strcmp(name, "entity_killed")) {
#ifdef GAME_DLL
    CBaseEntity *pVictim =
        UTIL_EntityByIndex(event->GetInt("entindex_killed", 0));
    CBaseEntity *pAttacker =
        UTIL_EntityByIndex(event->GetInt("entindex_attacker", 0));
    CBaseEntity *pInflictor =
        UTIL_EntityByIndex(event->GetInt("entindex_inflictor", 0));
    OnKillEvent(pVictim, pAttacker, pInflictor, event);
#endif  // GAME_DLL
  } else if (0 == Q_strcmp(name, "game_init")) {
#ifdef GAME_DLL
    // clear all state as though we were loading a saved game, but without
    // loading the game
    PreRestoreSavedGame();
    ...
    CBaseEntity *pVictim = ClientEntityList().GetEnt(
        engine->GetPlayerForUserID(event->GetInt("userid")));
    CBaseEntity *pAttacker = ClientEntityList().GetEnt(
        engine->GetPlayerForUserID(event->GetInt("attacker")));
    ...
    if (pLocalPlayer) {
      ...
      if (iTeam > TEAM_SPECTATOR) {
        if (0 == m_flTeamplayStartTime) {
          // player transitioned from no/spectator team to a game team, mark the
          // time
          m_flTeamplayStartTime = gpGlobals->curtime;
        }
      } else {
        // player transitioned to no/spectator team, clear the teamplay start
        // time
        m_flTeamplayStartTime = 0;
      }
    }
    ...
    if (false == event->GetBool("full_round", true)) {
      // we just finished a miniround but the round is continuing, increment
      // miniround count
...
//-----------------------------------------------------------------------------
void CAchievementMgr::OnKillEvent(CBaseEntity *pVictim, CBaseEntity *pAttacker,
                                  CBaseEntity *pInflictor, IGameEvent *event) {
  // can have a NULL victim on client if victim has never entered local player's
  // PVS
  if (!pVictim) return;

  // if single-player game, calculate if the attacker is the local player and if
  // the victim is the player enemy
  ...
  if (!g_pGameRules->IsMultiplayer()) {
    ...
    if (pLocalPlayer) {
      ...
      CBaseCombatCharacter *pBCC =
          dynamic_cast<CBaseCombatCharacter *>(pVictim);
  ...
  // look through all the kill event listeners and notify any achievements whose
  // filters we pass
  FOR_EACH_VEC(m_vecKillEventListeners, iAchievement) {
    CBaseAchievement *pAchievement = m_vecKillEventListeners[iAchievement];

    if (!pAchievement->IsActive()) continue;

    // if this achievement only looks for kills where attacker is player and
    // that is not the case here, skip this achievement
    if ((pAchievement->GetFlags() & ACH_FILTER_ATTACKER_IS_PLAYER) &&
        !bAttackerIsPlayer)
      continue;

    // if this achievement only looks for kills where victim is killer enemy and
    // that is not the case here, skip this achievement
    if ((pAchievement->GetFlags() & ACH_FILTER_VICTIM_IS_PLAYER_ENEMY) &&
        !bVictimIsPlayerEnemy)
      continue;

#if GAME_DLL
    // if this achievement only looks for a particular victim class name and
    // this victim is a different class, skip this achievement
    const char *pVictimClassNameFilter = pAchievement->m_pVictimClassNameFilter;
    if (pVictimClassNameFilter &&
        !pVictim->ClassMatches(pVictimClassNameFilter))
      continue;

    // if this achievement only looks for a particular inflictor class name and
    // this inflictor is a different class, skip this achievement
    const char *pInflictorClassNameFilter =
        pAchievement->m_pInflictorClassNameFilter;
    if (pInflictorClassNameFilter &&
        ((NULL == pInflictor) ||
         !pInflictor->ClassMatches(pInflictorClassNameFilter)))
      continue;

    // if this achievement only looks for a particular attacker class name and
    // this attacker is a different class, skip this achievement
    const char *pAttackerClassNameFilter =
        pAchievement->m_pAttackerClassNameFilter;
    if (pAttackerClassNameFilter &&
        ((NULL == pAttacker) ||
         !pAttacker->ClassMatches(pAttackerClassNameFilter)))
      continue;

    // if this achievement only looks for a particular inflictor entity name and
    // this inflictor has a different name, skip this achievement
    const char *pInflictorEntityNameFilter =
        pAchievement->m_pInflictorEntityNameFilter;
    if (pInflictorEntityNameFilter &&
        ((NULL == pInflictor) ||
         !pInflictor->NameMatches(pInflictorEntityNameFilter)))
      continue;
#endif  // GAME_DLL

    // we pass all filters for this achievement, notify the achievement of the
    // kill
...
void CAchievementMgr::OnMapEvent(const char *pchEventName) {
  ...
  if (!pchEventName || !*pchEventName) return;
  ...
  FOR_EACH_VEC(m_vecComponentListeners, iAchievement) {
    ...
    if (0 == Q_strncmp(pchEventName, pAchievement->m_pszComponentPrefix,
                       pAchievement->m_iComponentPrefixLen)) {
...
//-----------------------------------------------------------------------------
// Purpose: Returns an achievement as it's abstract object. This interface is
// used by gameui.dll for getting achievement info. Input  : index - Output :
// IBaseAchievement*
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Returns total achievement count. This interface is used by
// gameui.dll for getting achievement info. Input  :  - Output : Count of
// achievements in manager's vector.
//-----------------------------------------------------------------------------
int CAchievementMgr::GetAchievementCount() { return m_vecAchievement.Count(); }

#if !defined(NO_STEAM)
//-----------------------------------------------------------------------------
// Purpose: called when stat download is complete
//-----------------------------------------------------------------------------
void CAchievementMgr::Steam_OnUserStatsReceived(
    UserStatsReceived_t *pUserStatsReceived) {
  ...
  // run through the achievements and set their achieved state according to
  // Steam data
  FOR_EACH_MAP(m_mapAchievement, i) {
    CBaseAchievement *pAchievement = m_mapAchievement[i];
    bool bAchieved = false;
    bool bRet = SteamUserStats()->GetAchievement(
        gameID, pAchievement->GetName(), &bAchieved);
    if (bRet) {
...
//-----------------------------------------------------------------------------
void CAchievementMgr::Steam_OnUserStatsStored(
    UserStatsStored_t *pUserStatsStored) {
    ...
    if (m_AchievementsAwarded.Count() > 0) {
      ...
      if (g_pGameRules && g_pGameRules->IsMultiplayer()) {
        ...
        if (pLocalPlayer) {
          ...
          Q_snprintf(cmd, sizeof(cmd), "achievement_earned %d %d",
                     m_AchievementsAwarded[0] ^ mask,
                     (iPlayerID ^ m_AchievementsAwarded[0]) ^ mask);
...
void CAchievementMgr::ResetAchievement_Internal(
...
void MsgFunc_AchievementEvent(bf_read &msg) {
  ...
  if (!pAchievementMgr) return;
  pAchievementMgr->OnAchievementEvent(iAchievementID);
}

CON_COMMAND_F(achievement_reset_all, "Clears all achievements",
...
CON_COMMAND_F(achievement_reset, "<internal name> Clears specified achievement",
              FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
...
CON_COMMAND_F(achievement_status, "Shows status of all achievement",
              FCVAR_CHEAT) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
...
CON_COMMAND_F(achievement_unlock, "<internal name> Unlocks achievement",
              FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
...
CON_COMMAND_F(achievement_unlock_all, "Unlocks all achievements",
              FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
...
CON_COMMAND_F(achievement_evaluate,
              "<internal name> Causes failable achievement to be evaluated",
              FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
...
CON_COMMAND_F(achievement_test_friend_count,
              "Counts the # of teammates on local player's friends list",
...
CON_COMMAND_F(
    achievement_test_clan_count,
    "Determines if specified # of teammates belong to same clan w/local player",
    FCVAR_DEVELOPMENTONLY) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
...
CON_COMMAND_F(achievement_mark_dirty, "Mark achievement data as dirty",
              FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT) {
  CAchievementMgr *pAchievementMgr =
      dynamic_cast<CAchievementMgr *>(engine->GetAchievementMgr());
  if (!pAchievementMgr) return;
