...
static ConVar soundpatch_captionlength(
    "soundpatch_captionlength", "2.0", FCVAR_REPLICATED,
...
BEGIN_SIMPLE_DATADESC(CSoundEnvelope)
  DEFINE_FIELD(m_current, FIELD_FLOAT), DEFINE_FIELD(m_target, FIELD_FLOAT),
      DEFINE_FIELD(m_rate, FIELD_FLOAT),
      DEFINE_FIELD(m_forceupdate, FIELD_BOOLEAN),
END_DATADESC()

//-----------------------------------------------------------------------------
// Purpose: Set the new target value for this ramp.  Reach this target in
// deltaTime
//			seconds from now
 ...
 public:
  ...
  void Init(IRecipientFilter *pSrc) {
    ...
    for (int i = 0; i < pSrc->GetRecipientCount(); i++) {
      ...
      if (index >= 0) m_Recipients.AddToTail(index);
    }
  }

  bool IsActive() const { return (m_Flags & FLAG_ACTIVE) != 0; }

  virtual bool IsReliable(void) const { return (m_Flags & FLAG_RELIABLE) != 0; }

  virtual int GetRecipientCount(void) const { return m_Recipients.Count(); }

  virtual int GetRecipientIndex(int slot) const { return m_Recipients[slot]; }
  ...
  virtual bool AddRecipient(CBasePlayer *player) {
    ...
    // Already in list
    if (m_Recipients.Find(index) != m_Recipients.InvalidIndex()) return false;
...
class CSoundPatch {
 public:
  ...
  void Init(IRecipientFilter *pFilter, CBaseEntity *pEnt, int channel,
            const char *pSoundName, soundlevel_t iSoundLevel);
 ...
 private:
  // SoundPatches take volumes between 0 & 1, and use that to multiply the
  // sounds.txt specified volume. This function is an internal method of
  // accessing the real volume passed into the engine (i.e. post multiply)
...
BEGIN_SIMPLE_DATADESC(CSoundPatch)
  ...
  DEFINE_EMBEDDED(m_pitch), DEFINE_EMBEDDED(m_volume),
      ...
      DEFINE_FIELD(m_flScriptVolume, FIELD_FLOAT), DEFINE_EMBEDDED(m_Filter),
...
//			channel - This is a sound channel (CHAN_ITEM,
//CHAN_STATIC) 			*pSoundName - sound script string name 			attenuation - attenuation
//of this sound (not animated)
//-----------------------------------------------------------------------------
void CSoundPatch::Init(IRecipientFilter *pFilter, CBaseEntity *pEnt,
  ...
  if (!Q_stristr(pSoundName, ".wav") && !Q_stristr(pSoundName, ".mp3") &&
    ...
    // This has to be the actual .wav because rndwave would cause a bunch of new
    // .wavs to play... bad...
...
void CSoundPatch::ChangeVolume(float volumeTarget, float deltaTime) {
  ...
  if (volumeTarget > 1.0) volumeTarget = 1.0;
...
//-----------------------------------------------------------------------------
float CSoundPatch::GetPitch(void) { return m_pitch.Value(); }

//-----------------------------------------------------------------------------
// Purpose: Get the sound's current volume
//-----------------------------------------------------------------------------
float CSoundPatch::GetVolume(void) { return m_volume.Value(); }
...
//-----------------------------------------------------------------------------
// Purpose: SoundPatches take volumes between 0 & 1, and use that to multiply
// the sounds.txt specified volume. This function is an internal method of
// accessing the real volume passed into the engine (i.e. post multiply) Output
// : float
//-----------------------------------------------------------------------------
...
bool CSoundPatch::Update(float time, float deltaTime) {
  ...
  if (m_flags && m_Filter.IsActive()) {
    // SoundPatches take volumes between 0 & 1, and use that to multiply the
    // sounds.txt specified volume. Because of this, we need to always set the
    // SND_CHANGE_VOL flag when we emit sound, or it'll use the scriptfile's
    // instead.
    m_flags |= SND_CHANGE_VOL;
...
//-----------------------------------------------------------------------------
void CSoundPatch::Reset(void) { m_shutdownTime = 0; }
...
void CSoundPatch::StartSound(float flStartTime) {
  ...
  if (m_Filter.IsActive()) {
    ...
    CBaseEntity::EmitCloseCaption(
        m_Filter, EntIndex(), STRING(m_iszSoundScriptName),
        ep.m_UtlVecSoundOrigin, m_flCloseCaptionDuration, true);
  }
  m_isPlaying = true;
}
...
void CSoundPatch::ResumeSound(void) {
  if (IsPlaying() && m_Filter.IsActive()) {
    if (EntIndex() >= 0) {
      ...
      DevWarning(
          "CSoundPatch::ResumeSound: Lost EHAndle on restore - destroy the "
          "sound patch in your entity's StopLoopingSounds! (%s)\n",
          STRING(m_iszSoundName));
    }
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: A new player's entered the game. See if we need to restart our
// sound.
//-----------------------------------------------------------------------------
...
// This is an entry in the command queue.  It's used to queue up various pitch
// and volume changes so you can define an envelope without writing timing code
// in an entity.  Existing queued commands can be deleted later if the envelope
// changes dynamically.
#include "tier0/include/memdbgoff.h"
struct SoundCommand_t {
  SoundCommand_t(void) { memset(this, 0, sizeof(*this)); }
  SoundCommand_t(CSoundPatch *pSound, float executeTime,
                 soundcommands_t command, float deltaTime, float value)
      : m_pPatch(pSound),
        m_time(executeTime),
...
BEGIN_SIMPLE_DATADESC(SoundCommand_t)
  ...
  // NOTE: This doesn't need to be saved, sound commands are saved right after
  // the patch they are associated with
  //	DEFINE_FIELD( m_pPatch, FIELD_????? )
  DEFINE_FIELD(m_time, FIELD_TIME), DEFINE_FIELD(m_deltaTime, FIELD_FLOAT),
...
bool SoundCommandLessFunc(const SOUNDCOMMANDPTR &lhs,
                          const SOUNDCOMMANDPTR &rhs) {
  // NOTE: A greater time means "less" priority
  return (lhs->m_time > rhs->m_time);
}

// This implements the sound controller
class CSoundControllerImp : public CSoundEnvelopeController,
 ...
 public:
  ...
  void Play(CSoundPatch *pSound, float volume, float pitch,
            float flStartTime = 0);
  void CommandAdd(CSoundPatch *pSound, float executeDeltaTime,
  ...
  CSoundPatch *SoundCreate(IRecipientFilter &filter, int nEntIndex,
                           const char *pSoundName);
  CSoundPatch *SoundCreate(IRecipientFilter &filter, int nEntIndex, int channel,
  ...
  void SoundChangePitch(CSoundPatch *pSound, float pitchTarget,
                        float deltaTime);
  void SoundChangeVolume(CSoundPatch *pSound, float volumeTarget,
                         float deltaTime);
  ...
  float SoundPlayEnvelope(CSoundPatch *pSound, soundcommands_t soundCommand,
                          envelopePoint_t *points, int numPoints);
  float SoundPlayEnvelope(CSoundPatch *pSound, soundcommands_t soundCommand,
...
#ifdef CLIENT_DLL
  // CAutoClientSystem
  virtual void Update(float frametime) { SystemUpdate(); }
#else
  virtual void PreClientUpdate() { SystemUpdate(); }
#endif

  virtual void LevelShutdownPreEntity() { SystemReset(); }
...
//-----------------------------------------------------------------------------
// Purpose: Remove this sound from the sound list & shutdown (not in external
// interface) Input  : *pSound - patch to remove
//-----------------------------------------------------------------------------
...
//			commandTime - commands have 2 parameters, a time and a
//value 			value -
// Output : 	void
//-----------------------------------------------------------------------------
void CSoundControllerImp::CommandAdd(CSoundPatch *pSound,
                                     float executeDeltaTime,
                                     soundcommands_t command, float commandTime,
                                     float commandValue) {
  SoundCommand_t *pCommand =
      new SoundCommand_t(pSound, g_pEffects->Time() + executeDeltaTime, command,
                         commandTime, commandValue);
...
//-----------------------------------------------------------------------------
void CSoundControllerImp::SaveSoundPatch(CSoundPatch *pSoundPatch,
                                         ISave *pSave) {
  int i;
...
//-----------------------------------------------------------------------------
void CSoundControllerImp::RestoreSoundPatch(CSoundPatch **ppSoundPatch,
                                            IRestore *pRestore) {
...
CSoundPatch *CSoundControllerImp::SoundCreate(IRecipientFilter &filter,
                                              int nEntIndex,
                                              const char *pSoundName) {
...
CSoundPatch *CSoundControllerImp::SoundCreate(IRecipientFilter &filter,
                                              int nEntIndex, int channel,
                                              const char *pSoundName,
                                              float attenuation) {
  CSoundPatch *pSound = new CSoundPatch;
  EHANDLE hEnt =
      (nEntIndex != -1) ? g_pEntityList->GetNetworkableHandle(nEntIndex) : NULL;
  pSound->Init(&filter, hEnt.Get(), channel, pSoundName,
               ATTN_TO_SNDLVL(attenuation));
...
void CSoundControllerImp::SoundChangePitch(CSoundPatch *pSound,
                                           float pitchTarget, float deltaTime) {
  pSound->ChangePitch(pitchTarget, deltaTime);
}

void CSoundControllerImp::SoundChangeVolume(CSoundPatch *pSound,
                                            float volumeTarget,
                                            float deltaTime) {
...
//-----------------------------------------------------------------------------
float CSoundControllerImp::SoundPlayEnvelope(CSoundPatch *pSound,
  ...
  for (int i = 0; i < numPoints; i++) {
    ...
    if ((points[i].amplitudeMin != -1.0f) ||
        (points[i].amplitudeMax != -1.0f)) {
      amplitude =
          random->RandomFloat(points[i].amplitudeMin, points[i].amplitudeMax);
    } else if (i == 0) {
    ...
    if ((points[i].durationMin != -1.0f) || (points[i].durationMax != -1.0f)) {
      duration =
          random->RandomFloat(points[i].durationMin, points[i].durationMax);
...
float CSoundControllerImp::SoundPlayEnvelope(CSoundPatch *pSound,
                                             soundcommands_t soundCommand,
                                             envelopeDescription_t *envelope) {
  return SoundPlayEnvelope(pSound, soundCommand, envelope->pPoints,
                           envelope->nNumPoints);
}

//-----------------------------------------------------------------------------
// Purpose: Looping sounds are often started in entity spawn/activate functions.
//			In singleplayer, the player's not ready to receive sounds
//then, so restart 			and SoundPatches that are active and have no receivers.
//-----------------------------------------------------------------------------
