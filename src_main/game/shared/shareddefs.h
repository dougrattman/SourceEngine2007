...
class CViewVectors {
 public:
  ...
  CViewVectors(Vector vView, Vector vHullMin, Vector vHullMax,
               Vector vDuckHullMin, Vector vDuckHullMax, Vector vDuckView,
               Vector vObsHullMin, Vector vObsHullMax, Vector vDeadViewHeight) {
    m_vView = vView;
    m_vHullMin = vHullMin;
    m_vHullMax = vHullMax;
...
#define WEAPON_NOCLIP \
  -1  // clip sizes set to this tell the weapon it doesn't use a clip
...
#define HIDEHUD_NEEDSUIT \
  (1 << 5)  // Hide when the local player doesn't have the HEV suit
#define HIDEHUD_MISCSTATUS \
  (1 << 6)  // Hide miscellaneous status elements (trains, pickup history, death
            // notices, etc)
#define HIDEHUD_CHAT \
  (1 << 7)  // Hide all communication elements (saytext, voice icon, etc)
#define HIDEHUD_CROSSHAIR (1 << 8)          // Hide crosshairs
#define HIDEHUD_VEHICLE_CROSSHAIR (1 << 9)  // Hide vehicle crosshair
#define HIDEHUD_INVEHICLE (1 << 10)
#define HIDEHUD_BONUS_PROGRESS \
  (1 << 11)  // Hide bonus progress display (for bonus map challenges)
...
// ) ) The Source engine is really designed for 32 or less players.  If you
// raise this number above 32, you better know what you are doing
//  and have a good answer for a bunch of perf question related to player
//  simulation, thinking logic, tracelines, networking overhead, etc.
// But if you are brave or are doing something interesting, go for it...   ywb
// 9/22/03
...
#define WEAPON_IS_ACTIVE \
  2  // This client is carrying this weapon and it's the currently held weapon
...
#define ITEM_FLAG_EXHAUSTIBLE \
  (1 << 4)  // A player can totally exhaust their ammo supply and lose this
            // weapon
#define ITEM_FLAG_DOHITLOCATIONDMG \
  (1 << 5)  // This weapon take hit location into account when applying damage
#define ITEM_FLAG_NOAMMOPICKUPS \
  (1 << 6)  // Don't draw ammo pickup sprites/sounds when ammo is received
#define ITEM_FLAG_NOITEMPICKUP \
  (1 << 7)  // Don't draw weapon pickup when this weapon is picked up by the
            // player
...
// Muzzle flash definitions (for the flags field of the "MuzzleFlash"
// DispatchEffect)
enum {
  MUZZLEFLASH_AR2 = 0,
  MUZZLEFLASH_SHOTGUN,
...
#define HITGROUP_GEAR \
  10  // alerts NPC, but doesn't do damage or bleed (1/100th damage)
...
// NOTE: The discrete ticks can have quantization error, so these numbers are
// biased a little to make the heights more exact
#define PLAYER_FATAL_FALL_SPEED \
  922.5f  // approx 60 feet sqrt( 2 * gravity * 60 * 12 )
#define PLAYER_MAX_SAFE_FALL_SPEED \
  526.5f  // approx 20 feet sqrt( 2 * gravity * 20 * 12 )
#define PLAYER_LAND_ON_FLOATING_OBJECT \
  173  // Can fall another 173 in/sec without getting hurt
#define PLAYER_MIN_BOUNCE_SPEED 173
#define PLAYER_FALL_PUNCH_THRESHOLD \
  303.0f  // won't punch player's screen/make scrape noise unless player falling
          // at least this fast - at least a 76" fall (sqrt( 2 * g * 76))
#else
#define PLAYER_FATAL_FALL_SPEED 1024    // approx 60 feet
#define PLAYER_MAX_SAFE_FALL_SPEED 580  // approx 20 feet
#define PLAYER_LAND_ON_FLOATING_OBJECT \
  200  // Can go another 200 units without getting hurt
#define PLAYER_MIN_BOUNCE_SPEED 200
#define PLAYER_FALL_PUNCH_THRESHOLD \
  (float)350  // won't punch player's screen/make scrape noise unless player
              // falling at least this fast.
#endif
#define DAMAGE_FOR_FALL_SPEED         \
  100.0f / (PLAYER_FATAL_FALL_SPEED - \
            PLAYER_MAX_SAFE_FALL_SPEED)  // damage per unit per second.
...
// CTakeDamageInfo::DebugGetDamageTypeString(unsigned int DamageType, char
// *outbuf, unsigned int outbuflength )
#define DMG_GENERIC 0       // generic damage was done
#define DMG_CRUSH (1 << 0)  // crushed by falling or moving object.
// NOTE: It's assumed crush damage is occurring as a result of physics
// collision, so no extra physics force is generated by crush damage. DON'T use
// DMG_CRUSH when damaging entities unless it's the result of a physics
...
#define DMG_NEVERGIB \
  (1 << 12)  // with this bit OR'd in, no damage type will be able to gib
             // victims upon death
#define DMG_ALWAYSGIB \
  (1 << 13)  // with this bit OR'd in, any damage type can be made to gib
             // victims upon death.
...
#define DMG_POISON \
  (1 << 17)  // blood poisoning - heals over time like drowning damage
#define DMG_RADIATION (1 << 18)     // radiation exposure
#define DMG_DROWNRECOVER (1 << 19)  // drowning recovery
#define DMG_ACID (1 << 20)          // toxic chemicals or acid burns
#define DMG_SLOWBURN (1 << 21)      // in an oven

#define DMG_REMOVENORAGDOLL \
  (1 << 22)  // with this bit OR'd in, no ragdoll will be created, and the
             // target will be quietly removed.
// use this to kill an entity that you've already got a server-side ragdoll for

#define DMG_PHYSGUN \
  (1 << 23)  // Hit by manipulator. Usually doesn't do any damage.
#define DMG_PLASMA (1 << 24)   // Shot by Cremator
#define DMG_AIRBOAT (1 << 25)  // Hit by the airboat's gun

#define DMG_DISSOLVE (1 << 26)  // Dissolving!
#define DMG_BLAST_SURFACE \
  (1 << 27)  // A blast on the surface of water that cannot harm things
             // underwater
#define DMG_DIRECT (1 << 28)
#define DMG_BUCKSHOT \
  (1 << 29)  // not quite a bullet. Little, rounder, different.

// NOTE: DO NOT ADD ANY MORE CUSTOM DMG_ TYPES. MODS USE THE DMG_LASTGENERICFLAG
// BELOW, AND
//		 IF YOU ADD NEW DMG_ TYPES, THEIR TYPES WILL BE HOSED. WE NEED A
// BETTER SOLUTION.

// TODO: keep this up to date so all the mod-specific flags don't overlap
// anything.
...
#define OBS_ALLOW_NONE \
  2  // don't allow any spectating after death (fixed & fade to black)
#define OBS_ALLOW_TEAM_ALL \
  3  // allow only own team, but any mode (currently only used by TF2)
...
enum : int {
  EFL_KILLME = (1 << 0),   // This entity is marked for death -- This allows the
                           // game to actually delete ents at a safe time
  EFL_DORMANT = (1 << 1),  // Entity is dormant, no updates to client
  EFL_NOCLIP_ACTIVE =
      (1 << 2),  // Lets us know when the noclip command is active.
  EFL_SETTING_UP_BONES =
      (1 << 3),  // Set while a model is setting up its bones.
  EFL_KEEP_ON_RECREATE_ENTITIES =
      (1 << 4),  // This is a special entity that should not be deleted when we
                 // restart entities only

  EFL_HAS_PLAYER_CHILD = (1 << 4),  // One of the child entities is a player.

  EFL_DIRTY_SHADOWUPDATE =
      (1 << 5),  // Client only- need shadow manager to update the shadow...
  EFL_NOTIFY = (1 << 6),  // Another entity is watching events on this entity
                          // (used by teleport)

  // The default behavior in ShouldTransmit is to not send an entity if it
  // doesn't have a model. Certain entities want to be sent anyway because all
  // the drawing logic is in the client DLL. They can set this flag and the
  // engine will transmit them even if they don't have a model.
  EFL_FORCE_CHECK_TRANSMIT = (1 << 7),

  EFL_BOT_FROZEN = (1 << 8),   // This is set on bots that are frozen.
  EFL_SERVER_ONLY = (1 << 9),  // Non-networked entity.
  EFL_NO_AUTO_EDICT_ATTACH =
      (1 << 10),  // Don't attach the edict; we're doing it explicitly
  ...
  EFL_IN_SKYBOX =
      (1 << 17),  // This is set if the entity detects that it's in the skybox.
                  // This forces it to pass the "in PVS" for transmission.
  EFL_USE_PARTITION_WHEN_NOT_SOLID =
      (1 << 18),  // Entities with this flag set show up in the partition even
                  // when not solid
  ...
  EFL_NO_MEGAPHYSCANNON_RAGDOLL =
      (1 << 28),  // Mega physcannon can't ragdoll these guys.
  EFL_NO_WATER_VELOCITY_CHANGE =
      (1 << 29),  // Don't adjust this entity's velocity when transitioning into
                  // water
  EFL_NO_PHYSCANNON_INTERACTION =
      (1 << 30),  // Physcannon can't pick these up or punt them
  EFL_NO_DAMAGE_FORCES =
      (1 << 31),  // Doesn't accept forces from physics damage
};
...
// These are the types of data that hang off of CBaseEntities and the flag bits
// used to mark their presence
enum {
  GROUNDLINK = 0,
  TOUCHLINK,
  STEPSIMULATION,
...
enum FireBulletsFlags_t {
  FIRE_BULLETS_FIRST_SHOT_ACCURATE =
      0x1,  // Pop the first shot with perfect accuracy
  FIRE_BULLETS_DONT_HIT_UNDERWATER =
      0x2,  // If the shot hits its target underwater, don't damage it
  FIRE_BULLETS_ALLOW_WATER_SURFACE_IMPACTS =
      0x4,  // If the shot hits water surface, still call DoImpactEffect
  FIRE_BULLETS_TEMPORARY_DANGER_SOUND =
      0x8,  // Danger sounds added from this impact can be stomped immediately
            // if another is queued
};
...
struct FireBulletsInfo_t {
  ...
  FireBulletsInfo_t(int nShots, const Vector &vecSrc, const Vector &vecDir,
                    const Vector &vecSpread, float flDistance, int nAmmoType,
  ...
  int m_iTracerFreq;
  int m_iDamage;
  int m_iPlayerDamage;  // Damage to be used instead of m_iDamage if we hit a
                        // player
...
// Purpose: Data for making the MOVETYPE_STEP entities appear to simulate every
// frame
//  We precompute the simulation and then meter it out each tick during
//  networking of the entities origin and orientation.  Uses a bit more
//  bandwidth, but it solves the NPCs interacting with elevators/lifts bugs.
...
struct StepSimulationData {
  ...
  // This is a potential mid-think position, orientation (Quaternion) and tick
  // count Used to mark motion discontinuities that happen between thinks
  StepSimulationStep m_Discontinuity;

  // This is the goal or post-Think position and orientation (and Quaternion for
  // blending) and next think time tick
  StepSimulationStep m_Next;
  QAngle m_angNextRotation;

  // This variable is used so that we only compute networked origin/angles once
  // per tick
...
// Purpose: Simple state tracking for changing model sideways shrinkage during
// barnacle swallow
...
struct EmitSound_t {
  ...
  float m_flSoundTime;  ///< NOT DURATION, but rather, some absolute time in the
                        ///< future until which this sound should be delayed
  float *m_pflSoundDuration;
  bool m_bEmitCloseCaption;
  bool m_bWarnOnMissingCloseCaption;
  bool m_bWarnOnDirectWaveReference;
  int m_nSpeakerEntity;
  mutable CUtlVector<Vector>
      m_UtlVecSoundOrigin;  ///< Actual sound origin(s) (can be multiple if
                            ///< sound routed through speaker entity(ies) )
...
// YWB:  3/12/2007
// Changing the following #define for Prediction Error checking (See
// gamemovement.cpp for overview) will to 1 or 2 enables the system, 0 turns it
// off Level 1 enables it, but doesn't force "full precision" networking, so you
// can still get lots of errors in position/velocity/etc. Level 2 enables it but
// also forces origins/angles to be sent full precision, so other fields can be
// error / tolerance checked NOTE:  This stuff only works on a listen server
// since it punches a hole from the client .dll to server .dll!!!
