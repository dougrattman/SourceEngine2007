...
CSceneCache::CSceneCache() { msecs = 0; }
...
int CSceneCache::GetSoundCount() const { return sounds.Count(); }
...
//			soundlist -
//-----------------------------------------------------------------------------
void CSceneCache::PrecacheSceneEvent(CChoreoEvent *event,
                                     CUtlVector<unsigned short> &soundlist) {
  if (!event || event->GetType() != CChoreoEvent::SPEAK) return;
