...
//--------------------------------------------------------------------------------------------------------------
/**
 *  Returns the client's ID_* value for the currently owned weapon, or ID_NONE
 * if no weapon is owned
 */
...
class CWeaponCSBase : public CBaseCombatWeapon {
 public:
  ...
  virtual float GetMaxSpeed()
      const;  // What's the player's max speed while holding this weapon.
  ...
  // return true if this weapon is a kinf of the given weapon type (ie:
  // "IsKindOf" WEAPONTYPE_RIFLE )
  bool IsKindOf(CSWeaponType type) const {
    return GetCSWpnData().m_WeaponType == type;
  }
 ...
 public:
  ...
  bool m_bDelayFire;  // This variable is used to delay the time between
