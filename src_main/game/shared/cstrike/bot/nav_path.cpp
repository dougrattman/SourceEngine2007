...
#define DrawLine(from, to, duration, red, green, blue) \
  NDebugOverlay::Line(from, to, red, green, blue, true, 0.1f)
...
bool CNavPath::ComputePathPositions() {
  if (m_segmentCount == 0) return false;
...
//--------------------------------------------------------------------------------------------------------------
/**
 * Return point a given distance along the path - if distance is out of path
 * bounds, point is clamped to start/end
...
bool CNavPath::GetPointAlongPath(float distAlong, Vector *pointOnPath) const {
  if (!IsValid() || pointOnPath == NULL) return false;
 ...
 * floor, etc
 */
bool CNavPath::FindClosestPointOnPath(const Vector *worldPos, int startIndex,
                                      int endIndex, Vector *close) const {
  if (!IsValid() || close == NULL) return false;
...
void CNavPath::Draw(const Vector &color) {
  ...
  for (int i = 1; i < m_segmentCount; ++i) {
    DrawLine(m_path[i - 1].pos + Vector(0, 0, HalfHumanHeight),
             m_path[i].pos + Vector(0, 0, HalfHumanHeight), 2, 255 * color.x,
             255 * color.y, 255 * color.z);
  }
}
...
void CNavPathFollower::Update(float deltaT, bool avoidObstacles) {
  if (m_path == NULL || m_path->IsValid() == false) return;

  const CNavPath::PathSegment *node = (*m_path)[m_segmentIndex];

  if (node == NULL) {
    m_improv->OnMoveToFailure(m_path->GetEndpoint(),
                              CImprovLocomotor::FAIL_INVALID_PATH);
    m_path->Invalidate();
    return;
  }
  ...
  if (node->ladder)
  {
          ...
          if (!m_isLadderStarted)
          {
                  ...
                  m_improv->StartLadder( node->ladder, node->how, approachPos,
  departPos ); m_isLadderStarted = true;
          }

          // move improv along ladder
          if (m_improv->TraverseLadder( node->ladder, node->how, approachPos,
  departPos, deltaT ))
          {
                  // completed ladder
                  ++m_segmentIndex;
  ...
  */
  ...
  if (!m_improv->IsUsingLadder()) {
    ...
    for (int i = m_segmentIndex; i < m_path->GetSegmentCount(); ++i) {
      ...
      if ((close - m_improv->GetFeet())
              .AsVector2D()
              .IsLengthGreaterThan(crouchRange))
        break;
  ...
  if (m_isDebug) {
    m_path->Draw();
    UTIL_DrawBeamPoints(m_improv->GetCentroid(),
                        m_goal + Vector(0, 0, StepHeight), 1, 255, 0, 255);
    UTIL_DrawBeamPoints(m_goal + Vector(0, 0, StepHeight),
                        m_improv->GetCentroid(), 1, 255, 0, 255);
  }
  ...
  if (m_goal.z - m_improv->GetFeet().z > JumpCrouchHeight) {
    ...
    Vector2D to(m_improv->GetFeet().x - m_goal.x,
                m_improv->GetFeet().y - m_goal.y);
    if (to.IsLengthLessThan(closeRange)) {
      ...
      if (m_behindIndex >= 0 && nextNode) {
        if (nextNode->pos.z - m_improv->GetFeet().z > JumpCrouchHeight) {
          ...
          m_improv->OnMoveToFailure(m_path->GetEndpoint(),
                                    CImprovLocomotor::FAIL_FELL_OFF);
          m_path->Invalidate();
          return;
        }
      } else {
  ...
  if (avoidObstacles && !isApproachingJumpArea && !m_improv->IsJumping() &&
      m_segmentIndex < m_path->GetSegmentCount() - 1) {
    FeelerReflexAdjustment(&m_goal);

    // currently, this is only used for hostages, and their collision physics
    // stinks do more feeler checks to avoid short obstacles
    /*
...
int CNavPathFollower::FindOurPositionOnPath(Vector *close, bool local) const {
  ...
  if (local) {
    ...
    if (end > m_path->GetSegmentCount()) end = m_path->GetSegmentCount();
  } else {
    start = 1;
 ...
 */
int CNavPathFollower::FindPathPoint(float aheadRange, Vector *point,
                                    int *prevIndex) {
  ...
  if (startIndex > m_segmentIndex && startIndex < m_path->GetSegmentCount() &&
      ((*m_path)[startIndex]->ladder ||
       (*m_path)[startIndex]->area->GetAttributes() & NAV_MESH_JUMP)) {
  ...
  for (i = startIndex; i < m_path->GetSegmentCount(); ++i) {
    ...
    Vector along =
        (i == startIndex) ? (pos - feet) : (pos - (*m_path)[i - 1]->pos);
 ...
 */
void CNavPathFollower::FeelerReflexAdjustment(Vector *goalPosition,
                                              float height) {
  // if we are in a "precise" area, do not do feeler adjustments
  if (m_improv->GetLastKnownArea() &&
      m_improv->GetLastKnownArea()->GetAttributes() & NAV_MESH_PRECISE)
    return;
  ...
  const float feelerOffset =
      (m_improv->IsCrouching()) ? 15.0f : 20.0f;  // 15, 20
  ...
  const float feelerHeight =
      (height > 0.0f) ? height : StepHeight + 0.1f;  // if obstacle is lower
                                                     // than StepHeight, we'll
                                                     // walk right over it

  float feelerLength =
      (m_improv->IsRunning()) ? feelerLengthRun : feelerLengthWalk;
  ...
  float ground;
  Vector normal;
  if (m_improv->GetSimpleGroundHeightWithFloor(m_improv->GetEyes(), &ground,
                                               &normal) == false)
    return;
...
void CStuckMonitor::Update(CImprovLocomotor *improv) {
  if (m_isStuck) {
    ...
    if ((improv->GetCentroid() - m_stuckSpot)
            .IsLengthGreaterThan(unstuckRange)) {
      // no longer stuck
      Reset();
    ...
    if (m_avgVelCount < MAX_VEL_SAMPLES) {
      ...
      float avgVel = 0.0f;
      for (int t = 0; t < m_avgVelCount; ++t) avgVel += m_avgVel[t];

      avgVel /= m_avgVelCount;

      // cannot make this velocity too high, or actors will get "stuck" when
      // going down ladders
