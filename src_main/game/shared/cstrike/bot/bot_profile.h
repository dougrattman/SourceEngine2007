...
class BotProfile {
 public:
  ...
  bool HasPrimaryPreference(void)
      const;  ///< return true if this profile has a primary weapon preference
  bool HasPistolPreference(void)
      const;  ///< return true if this profile has a pistol weapon preference

  int GetCost(void) const { return m_cost; }
  int GetSkin(void) const { return m_skin; }
  bool IsDifficulty(BotDifficultyType diff) const;  ///< return true if this
                                                    ///< profile can be used for
 ...
 private:
  ...
  void Inherit(const BotProfile *parent,
               const BotProfile *baseline);  ///< copy values from parent if
                                             ///< they differ from baseline

  char *m_name;        ///< the bot's name
  float m_aggression;  ///< percentage: 0 = coward, 1 = berserker
  float m_skill;       ///< percentage: 0 = terrible, 1 = expert
  float m_teamwork;  ///< percentage: 0 = rogue, 1 = complete obeyance to team,
                     ///< lots of comm

  enum { MAX_WEAPON_PREFS = 16 };
  int m_weaponPreference[MAX_WEAPON_PREFS];  ///< which weapons this bot likes
  ...
  unsigned char m_difficultyFlags;  ///< bits set correspond to difficulty
                                    ///< levels this is valid for
  int m_voicePitch;      ///< the pitch shift for bot chatter (100 = normal)
  float m_reactionTime;  //< our reaction time in seconds
  float m_attackDelay;   ///< time in seconds from when we notice an enemy to
                         ///< when we open fire
  int m_teams;           ///< teams for which this profile is valid

  bool m_prefersSilencer;  ///< does the bot prefer to use silencers?

  int m_voiceBank;  ///< Index of the BotChatter.db voice bank this profile uses
                    ///< (0 is the default)

  CUtlVector<const BotProfile *>
      m_templates;  ///< List of templates we inherit from
};
 ...
 */
inline void BotProfile::Inherit(const BotProfile *parent,
                                const BotProfile *baseline) {
...
class BotProfileManager {
 public:
  ...
  const BotProfile *GetProfileMatchingTemplate(
      const char *profileName, int team, BotDifficultyType difficulty) const {
    FOR_EACH_LL(m_profileList, it) {
      BotProfile *profile = m_profileList[it];
  ...
  const BotProfile *GetRandomProfile(
      BotDifficultyType difficulty, int team,
      CSWeaponType weaponType) const;  ///< return random unused profile that
                                       ///< matches the given difficulty level

  const char *GetCustomSkin(
      int index);  ///< Returns custom skin name at a particular index
  const char *GetCustomSkinModelname(
      int index);  ///< Returns custom skin modelname at a particular index
  const char *GetCustomSkinFname(
      int index);  ///< Returns custom skin filename at a particular index
  int GetCustomSkinIndex(
      const char *name,
      const char *filename = NULL);  ///< Looks up a custom skin index by name

  typedef CUtlVector<char *> VoiceBankList;
  const VoiceBankList *GetVoiceBanks(void) const { return &m_voiceBanks; }
  int FindVoiceBankIndex(const char *filename);  ///< return index of the
                                                 ///< (custom) bot phrase db,
