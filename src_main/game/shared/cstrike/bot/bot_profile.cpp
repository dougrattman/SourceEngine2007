...
const char *BotProfile::GetWeaponPreferenceAsString(int i) const {
  if (i < 0 || i >= m_weaponPreferenceCount) return NULL;
...
bool BotProfile::HasPrimaryPreference(void) const {
  for (int i = 0; i < m_weaponPreferenceCount; ++i) {
    if (IsPrimaryWeapon(m_weaponPreference[i])) return true;
  }
...
bool BotProfile::HasPistolPreference(void) const {
  for (int i = 0; i < m_weaponPreferenceCount; ++i)
    if (IsSecondaryWeapon(m_weaponPreference[i])) return true;
...
bool BotProfile::IsValidForTeam(int team) const {
  return (team == TEAM_UNASSIGNED || m_teams == TEAM_UNASSIGNED ||
          team == m_teams);
}
...
bool BotProfile::InheritsFrom(const char *name) const {
  if (WildcardMatch(name, GetName())) return true;
...
void BotProfileManager::Init(const char *filename, unsigned int *checksum) {
  ...
  if (checksum) {
    *checksum = 0;  // ComputeSimpleChecksum( (const unsigned char
  ...
  while (true) {
    ...
    if (isCustomSkin) {
      ...
      if (m_nextSkin < NumCustomSkins && !skinExists) {
        ...
        m_skinFilenames[m_nextSkin] =
            new char[strlen(token) * 2 + strlen("models/player//.mdl") + 1];
        Q_snprintf(m_skinFilenames[m_nextSkin],
                   sizeof(m_skinFilenames[m_nextSkin]),
                   "models/player/%s/%s.mdl", token, token);
        ++m_nextSkin;
      }
      ...
      continue;  // it's just a custom skin - no need to do inheritance on a bot
                 // profile, etc.
    }
    ...
    if (!isDefault) {
      ...
      /**
       * HACK HACK
       * Until we have a generalized means of storing bot preferences, we're
       * going to hardcode the bot's preference towards silencers based on his
       * name.
       */
      ...
      if (!stricmp("Aggression", attributeName)) {
        ...
        if (!stricmp(token, "none")) {
          ...
          if (profile->m_weaponPreferenceCount < BotProfile::MAX_WEAPON_PREFS) {
            profile->m_weaponPreference[profile->m_weaponPreferenceCount++] =
                AliasToWeaponID(token);
          }
        }
...
//--------------------------------------------------------------------------------------------------------------
BotProfileManager::~BotProfileManager() { Reset(); }
...
//--------------------------------------------------------------------------------------------------------
/**
 * Looks up a custom skin index by filename-decorated name (will decorate the
 * name if filename is given)
 */
int BotProfileManager::GetCustomSkinIndex(const char *name,
                                          const char *filename) {
 ...
 */
const BotProfile *BotProfileManager::GetRandomProfile(
    BotDifficultyType difficulty, int team, CSWeaponType weaponType) const {
  ...
  FOR_EACH_LL(m_profileList, it) {
    ...
    if (weaponType != WEAPONTYPE_UNKNOWN) {
      if (!profile->GetWeaponPreferenceCount()) continue;

      if (weaponType !=
          WeaponClassFromWeaponID((CSWeaponID)profile->GetWeaponPreference(0)))
        continue;
    }
