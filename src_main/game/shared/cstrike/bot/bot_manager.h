...
const int g_BotUpdateSkipCount =
    2;  ///< number of upkeep periods to skip update
...
class ActiveGrenade {
 public:
  ...
  void OnEntityGone();   ///< called when the grenade in the world goes away
  void Update();         ///< called every frame
  bool IsValid(void) const;  ///< return true if this grenade is valid

  bool IsEntity(CBaseGrenade *grenade) const {
  ...
  CBaseGrenade *GetGrenade() { return m_entity; }
  float GetRadius(void) const { return m_radius; }
  void SetRadius(float radius) { m_radius = radius; }

 private:
  CBaseGrenade *m_entity;       ///< the entity
  Vector m_detonationPosition;  ///< the location where the grenade detonated
                                ///< (smoke)
...
//--------------------------------------------------------------------------------------------------------------
/**
 * This class manages all active bots, propagating events to them and updating
 * them.
 */
class CBotManager {
 public:
  ...
  CBasePlayer *AllocateAndBindBotEntity(edict_t *ed);  ///< allocate the
                                                       ///< appropriate entity
                                                       ///< for the bot and bind
                                                       ///< it to the given
                                                       ///< edict
  virtual CBasePlayer *AllocateBotEntity(void) = 0;    ///< factory method to
                                                       ///< allocate the
  ...
  virtual void RestartRound(
      void);                      ///< (EXTEND) invoked when a new round begins
  virtual void StartFrame();  ///< (EXTEND) called each frame

  virtual unsigned int GetPlayerPriority(CBasePlayer *player)
      const = 0;  ///< return priority of player (0 = max pri)

  void AddGrenade(
      CBaseGrenade *grenade);  ///< add an active grenade to the bot's awareness
  void RemoveGrenade(CBaseGrenade *grenade);  ///< the grenade entity in the
                                              ///< world is going away
  void SetGrenadeRadius(CBaseGrenade *grenade,
                        float radius);  ///< the radius of the grenade entity
                                        ///< (or associated smoke cloud)
  ...
  bool IsLineBlockedBySmoke(const Vector &from, const Vector &to,
                            float grenadeBloat = 1.0f);  ///< return true if
                                                         ///< line intersects
                                                         ///< smoke volume, with
                                                         ///< grenade radius
                                                         ///< increased by the
                                                         ///< grenadeBloat
                                                         ///< factor
  bool IsInsideSmokeCloud(
      const Vector *pos);  ///< return true if position is inside a smoke cloud
  ...
  int GetDebugMessageCount(
      void) const;  ///< get number of debug messages in history
  const DebugMessage *GetDebugMessage(int which = 0)
      const;  ///< return the debug message emitted by the bot (0 = most recent)
  void ClearDebugMessages();
  void AddDebugMessage(const char *msg);

 private:
  ActiveGrenadeList m_activeGrenadeList;  ///< the list of active grenades the
                                          ///< bots are aware of
...
inline const CBotManager::DebugMessage *CBotManager::GetDebugMessage(
    int which) const {
  if (which >= m_debugMessageCount) return NULL;

  int i = m_currentDebugMessage - which;
  if (i < 0) i += MAX_DBG_MSGS;
