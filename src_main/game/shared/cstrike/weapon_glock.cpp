...
class CWeaponGlock : public CWeaponCSBase {
 ...
 private:
  ...
  int m_iGlock18ShotsFired;  // used to keep track of the shots fired during the
                             // Glock18 burst fire mode.
  float m_flGlock18Shoot;  // time to shoot the remaining bullets of the glock18
                           // burst fire
  float m_flLastFire;
};
...
CWeaponGlock::CWeaponGlock() { m_flLastFire = gpGlobals->curtime; }
...
void CWeaponGlock::GlockFire(float flSpread, bool bFireBurst) {
  ...
  if (bFireBurst) {
    ...
    if (pPlayer->m_iShotsFired > 1) return;
  }

  // Mark the time of this shot and determine the accuracy modifier based on the
  // last shot fired...
...
bool CWeaponGlock::Reload() {
  if (m_flGlock18Shoot != 0) return true;
