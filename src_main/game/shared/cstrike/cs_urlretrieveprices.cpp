...
#else
#define WEEKLY_PRICE_URL \
  "https://store.steampowered.com/stats/csmarket/weeklyprices.dat"
#endif
...
bool SendHTTPRequest(const char *pchRequestURL, SOCKET socketHTML) {
  ...
  return ::send(socketHTML, szHeader, Q_strlen(szHeader) + 1, 0) !=
         SOCKET_ERROR;
}

//-----------------------------------------------------------------------------
// Purpose: Given a previous HTTP request parse the response into a key values
// buffer
//-----------------------------------------------------------------------------
  ...
  while (!bFinishedHeaderRead && iRecvPosition < sizeof(szHeaderBuf)) {
    ...
    switch (szHeaderBuf[iRecvPosition]) {
      case '\r':
        break;
      case '\n':
        if (cCharsInLine == 0) bFinishedHeaderRead = true;
...
//-----------------------------------------------------------------------------
bool ProcessURL(const char *pchURL, void *pSockAddrIn, char *pchRequest,
                int cchRequest) {
  ...
  if (pchColon) {
    ...
    if (!pchForwardSlash) return false;
    Q_strncpy(rgchRequest, pchColon + 1, pchForwardSlash - (pchColon + 1) + 1);
    iPort = atoi(rgchRequest);
    Q_strncpy(rgchRequest, pchForwardSlash,
              (pchURL + Q_strlen(pchURL)) - pchForwardSlash + 1);
  } else {
...
bool BlackMarket_DownloadPrices(void) {
  ...
  if (ProcessURL(WEEKLY_PRICE_URL, &server, szRequest, sizeof(szRequest))) {
    ...
    if (socketHTML != INVALID_SOCKET) {
      int iRet =
          ::connect(socketHTML, (LPSOCKADDR)&server, sizeof(SOCKADDR_IN));
