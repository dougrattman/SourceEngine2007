...
CSheetSimulator::CSheetSimulator(TraceLineFunc_t traceline,
    ...
    : m_pFixedPoint(0),
      m_ControlPoints(0),
      m_TraceLine(traceline),
      m_TraceHull(traceHull) {}
...
void CSheetSimulator::AddFixedPointSpring(int fixedPoint, int p,
                                          float restLength) {
...
void CSheetSimulator::ComputeForces() {
  ...
  for (i = 0; i < m_Springs.Size(); ++i) {
    ...
    // got two particles, a and b with positions xa and xb and velocities va and
    // vb and l = xa - xb fa = -( ks * (|l| - r) + kd * (va - vb) dot (l) / |l|)
    // * l/|l|
  ...
  for (i = 0; i < m_Gravity.Count(); ++i) {
    m_Particle[m_Gravity[i]].m_Force.z -=
        m_Particle[m_Gravity[i]].m_Mass * m_GravityConstant;
  }
  ...
  for (i = 0; i < NumParticles(); ++i) {
    ...
    if ((m_Particle[i].m_CollisionPlane) >= 0) {
      const Vector& planeNormal =
          m_pCollisionPlanes[m_Particle[i].m_CollisionPlane].normal;
      float perp = DotProduct(m_Particle[i].m_Force, planeNormal);
      if (perp < 0) m_Particle[i].m_Force -= planeNormal * perp;
    }
...
void CSheetSimulator::TestVertAgainstPlane(int vert, int plane, bool bFarTest) {
  if (!m_pValidCollisionPlane[plane]) return;
...
void CSheetSimulator::InitPosition(int i) {
  ...
  if (tr.fraction - 1.0 < 0) {
    ...
    float dist = (m_pCollisionPlanes[i].dist -
                  DotProduct(m_Origin, m_pCollisionPlanes[i].normal)) /
...
void CSheetSimulator::DetectCollision(int i, float flPlaneOffset) {
  ...
  if (tr.fraction - 1.0 < 0) {
    ...
    VectorNormalize(m_pCollisionPlanes[i].normal);
    m_pCollisionPlanes[i].dist =
        DotProduct(m_Origin, m_pCollisionPlanes[i].normal) - flPlaneOffset;
    m_pCollisionPlanes[i].type = 3;
  } else {
    m_pValidCollisionPlane[i] = false;
  }
...
void CSheetSimulator::DetermineBestCollisionPlane(bool bFarTest) {
  ...
  for (int i = 0; i < NumVertical(); ++i) {
    for (int j = 0; j < NumHorizontal(); ++j) {
      ...
      if (i > 0) TestVertAgainstPlane(idx, idx - NumHorizontal(), bFarTest);
      if (i < NumVertical() - 1)
...
void CSheetSimulator::SatisfyCollisionConstraints() {
  ...
  for (int i = 0; i < NumParticles(); ++i) {
    ...
    if (m_Particle[i].m_CollisionPlane >= 0) {
      ...
      m_Particle[i].m_Position =
          m_Origin + delta * m_Particle[i].m_CollisionDist;

      float perp = DotProduct(m_Particle[i].m_Velocity, pPlane->normal);
      if (perp < 0) m_Particle[i].m_Velocity -= pPlane->normal * perp;
    }
  }
}
...
void CSheetSimulator::EulerStep(float dt) {
  ...
  for (int i = 0; i < NumParticles(); ++i) {
    ...
    m_Particle[i].m_Velocity +=
        m_Particle[i].m_Force * dt / m_Particle[i].m_Mass;
...
void CSheetSimulator::ClampPointsToCollisionPlanes() {
  ...
  for (int i = 0; i < NumParticles(); ++i) {
    ...
    if (m_Particle[i].m_CollisionPlane >= 0) {
      ...
      float flPerpDist = fabs(
          DotProduct(m_Particle[i].m_Position, pPlane->normal) - pPlane->dist);
      if (flPerpDist >= CLAMP_DIST) continue;

      // Drop it along the perp
      VectorMA(m_Particle[i].m_Position, -flPerpDist, pPlane->normal,
               m_Particle[i].m_Position);
    }
  }
}
...
//-----------------------------------------------------------------------------
CIterativeSheetSimulator::CIterativeSheetSimulator(TraceLineFunc_t traceline,
                                                   TraceHullFunc_t traceHull)
    : CSheetSimulator(traceline, traceHull), m_SimulationSteps(0) {}

void CIterativeSheetSimulator::BeginSimulation(float dt, int steps,
                                               int substeps,
                                               int collisionCount) {
...
void CIterativeSheetSimulator::DetectCollisions(void) {
  for (int i = 0; i < m_CollisionCount; ++i) {
    if (m_InitialPass) {
      ...
      float flOffset =
          COLLISION_PLANE_OFFSET *
          ((float)(m_SimulationSteps - 1) / (float)(m_TotalSteps - 1));
