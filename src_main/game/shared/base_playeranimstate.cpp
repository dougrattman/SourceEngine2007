...
ConVar cl_showanimstate("cl_showanimstate", "-1",
                        FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
                        "Show the (client) animation state for the specified "
                        "entity (-1 for none).");
ConVar showanimstate_log("cl_showanimstate_log", "0",
                         FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
                         "1 to output cl_showanimstate to Msg(). 2 to store in "
                         "AnimStateClient.log. 3 for both.");
#else
#include "player.h"
ConVar sv_showanimstate("sv_showanimstate", "-1",
                        FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
                        "Show the (server) animation state for the specified "
                        "entity (-1 for none).");
ConVar showanimstate_log("sv_showanimstate_log", "0",
                         FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
                         "1 to output sv_showanimstate to Msg(). 2 to store in "
                         "AnimStateServer.log. 3 for both.");
#endif
...
ConVar mp_feetyawrate(
    "mp_feetyawrate", "720", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "How many degrees per second that we can turn our feet or upper body.");

ConVar mp_facefronttime("mp_facefronttime", "3",
                        FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
                        "After this amount of time of standing in place but "
                        "aiming to one side, go ahead and move feet to face "
                        "upper body.");

ConVar mp_ik("mp_ik", "1", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
...
void CBasePlayerAnimState::Init(CBaseAnimatingOverlay *pPlayer,
...
void CBasePlayerAnimState::Release() { delete this; }
...
void CBasePlayerAnimState::Update(float eyeYaw, float eyePitch) {
  ...
  // Clear animation overlays because we're about to completely reconstruct
  // them.
  ClearAnimationLayers();

  // Some mods don't want to update the player's animation state if they're dead
  // and ragdolled.
  ...
  ComputePoseParam_MoveYaw(
      pStudioHdr);  // What direction his legs are running in.
  ...
  if (cl_showanimstate.GetInt() == m_pOuter->entindex()) {
    ...
    if (targetPlayer && (targetPlayer->GetObserverMode() == OBS_MODE_IN_EYE ||
                         targetPlayer->GetObserverMode() == OBS_MODE_CHASE)) {
...
bool CBasePlayerAnimState::ShouldUpdateAnimState() {
  // By default, don't update their animation state when they're dead because
  // they're either a ragdoll or they're not drawn.
  return GetOuter()->IsAlive();
}

bool CBasePlayerAnimState::ShouldChangeSequences(void) const { return true; }

void CBasePlayerAnimState::SetOuterPoseParameter(int iParam, float flValue) {
  // Make sure to set all the history values too, otherwise the server can
  // overwrite them.
...
void CBasePlayerAnimState::ComputeSequences(CStudioHdr *pStudioHdr) {
  ...
  UpdateInterpolators();  // The groundspeed interpolator uses the main sequence
                          // info.
...
void CBasePlayerAnimState::ComputeMainSequence() {
  ...
  // Have our class or the mod-specific class determine what the current
  // activity is.
  ...
  if (pPlayer->GetSequenceActivity(pPlayer->GetSequence()) ==
      pPlayer->GetSequenceActivity(animDesired))
    return;
#endif
...
void CBasePlayerAnimState::UpdateAimSequenceLayers(
  ...
  int iAimSequence =
      CalcAimLayerSequence(&flCycle, &flAimSequenceWeight, bForceIdle);
  if (iAimSequence == -1) iAimSequence = 0;

  // Feed the current state of the animation parameters to the sequence
  // transitioner. It will hand back either 1 or 2 animations in the queue to
  // set, depending on whether it's transitioning or not. We just dump those
  // into the animation layers.
  pTransitioner->CheckForSequenceChange(
      m_pOuter->GetModelPtr(), iAimSequence,
      false,  // don't force transitions on the same anim
      true    // yes, interpolate when transitioning
  );

  pTransitioner->UpdateCurrent(m_pOuter->GetModelPtr(), iAimSequence, flCycle,
  ...
  if (pTransitioner->m_animationQueue.Count() == 1) {
    ...
    *pDest1 = *pSource1;
    ...
    pDest1->m_flWeight = 1 - pDest0->m_flWeight;  // This layer just mirrors the
                                                  // other layer's weight (one
                                                  // fades in while the other
                                                  // fades out).
...
void CBasePlayerAnimState::OptimizeLayerWeights(int iFirstLayer, int nLayers) {
  ...
  // Find the total weight of the blended layers, not including the idle layer
  // (iFirstLayer)
  ...
  // weighted on top of the idle animations, all this does is disable the idle
  // animations if the walk/runs are at full weighting, which is whenever a guy
  // is at full speed.
  //
  // So it saves us blending a couple animation layers whenever a guy is walking
  // or running full speed.
  int iLastOne = -1;
  for (i = 0; i < nLayers; i++) {
    CAnimationLayer *pLayer = m_pOuter->GetAnimOverlay(iFirstLayer + i);
    if (pLayer->IsActive() && pLayer->m_flWeight > 0.99) iLastOne = i;
  }
...
bool CBasePlayerAnimState::ShouldBlendAimSequenceToIdle() {
  ...
  return (act == ACT_RUN || act == ACT_WALK || act == ACT_RUNTOIDLE ||
          act == ACT_RUN_CROUCH);
}
...
void CBasePlayerAnimState::ComputeAimSequence() {
  ...
  UpdateAimSequenceLayers(flCycle, AIMSEQUENCE_LAYER, true,
                          &m_IdleSequenceTransitioner, 1);

  if (ShouldBlendAimSequenceToIdle()) {
    // What we do here is blend between the idle upper body animation (like
    // where he's got the dual elites held out in front of him but he's not
    // moving) and his walk/run/crouchrun upper body animation, weighting it
    // based on how fast he's moving. That way, when he's moving slowly, his
    // upper body doesn't jiggle all around.
    bool bIsMoving;
    float flPlaybackRate = CalcMovementPlaybackRate(&bIsMoving);
    if (bIsMoving)
      UpdateAimSequenceLayers(flCycle, AIMSEQUENCE_LAYER + 2, false,
                              &m_SequenceTransitioner, flPlaybackRate);
  }
...
bool CBasePlayerAnimState::CanThePlayerMove() { return true; }
...
void CBasePlayerAnimState::ComputePlaybackRate() {
  ...
  if (m_AnimConfig.m_LegAnimType != LEGANIM_9WAY &&
    ...
    // When using a 9-way blend, playback rate is always 1 and we just scale the
    // pose params to speed up or slow down the animation.
    bool bIsMoving;
...
void CBasePlayerAnimState::ComputePoseParam_MoveYaw(CStudioHdr *pStudioHdr) {
  ...
  // If using goldsrc-style animations where he's moving in the direction that
  // his feet are facing, we don't use move yaw.
  if (m_AnimConfig.m_LegAnimType != LEGANIM_9WAY &&
      m_AnimConfig.m_LegAnimType != LEGANIM_8WAY)
    return;
  ...
  if (m_AnimConfig.m_LegAnimType == LEGANIM_9WAY) {
    ...
    if (iMoveYaw >= 0) {
      ...
      CAnimationLayer *pLayer =
          m_pOuter->GetAnimOverlay(MAIN_IDLE_SEQUENCE_LAYER);
      ...
      if (ShouldChangeSequences()) {
        // Whenever this layer stops blending, we can choose a new idle sequence
        // to blend to, so he doesn't always use the same idle.
        if (pLayer->m_flWeight < 0.02f || m_iCurrent8WayIdleSequence == -1) {
          m_iCurrent8WayIdleSequence =
              m_pOuter->SelectWeightedSequence(ACT_IDLE);
          m_iCurrent8WayCrouchIdleSequence =
              m_pOuter->SelectWeightedSequence(ACT_CROUCHIDLE);
        }
      ...
      pLayer->m_flCycle +=
          m_pOuter->GetSequenceCycleRate(pStudioHdr, pLayer->m_nSequence) *
          gpGlobals->frametime;
...
// Output : int
//-----------------------------------------------------------------------------
int CBasePlayerAnimState::ConvergeAngles(float goal, float maxrate,
                                         float maxgap, float dt,
                                         float &current) {
...
void CBasePlayerAnimState::ComputePoseParam_BodyYaw() {
  ...
  // If we just initialized this guy (maybe he just came into the PVS), then
  // immediately set his feet in the right direction, otherwise they'll spin
  // around from 0 to the right direction every time someone switches spectator
  // targets.
  if (!m_bCurrentFeetYawInitialized) {
    ...
    if (m_AnimConfig.m_LegAnimType == LEGANIM_9WAY ||
        m_AnimConfig.m_LegAnimType == LEGANIM_8WAY) {
      // His feet point in the direction his eyes are, but they can run in any
      // direction.
      ...
      Vector vFeetYaw(cos(DEG2RAD(m_flGoalFeetYaw)),
                      sin(DEG2RAD(m_flGoalFeetYaw)), 0);
      if (vEyeYaw.Dot(vFeetYaw) < -0.01) {
        m_flGoalFeetYaw += 180;
      }
    }

  } else if ((gpGlobals->curtime - m_flLastTurnTime) >
             mp_facefronttime.GetFloat()) {
    // player didn't move & turn for quite some time
    m_flGoalFeetYaw = m_flEyeYaw;
  } else {
    // If he's rotated his view further than the model can turn, make him face
    // forward.
  ...
  if (m_flCurrentFeetYaw != m_flGoalFeetYaw) {
    ConvergeAngles(m_flGoalFeetYaw, mp_feetyawrate.GetFloat(),
                   m_AnimConfig.m_flMaxBodyYawDegrees, gpGlobals->frametime,
                   m_flCurrentFeetYaw);
...
void CBasePlayerAnimState::DebugShowAnimState(int iStartLine) {
  ...
  AnimStatePrintf(
      iLine++, "main: %s(%d), cycle: %.2f cyclerate: %.2f playbackrate: %.2f\n",
      GetSequenceName(m_pOuter->GetModelPtr(), m_pOuter->GetSequence()),
      m_pOuter->GetSequence(), m_pOuter->GetCycle(),
      m_pOuter->GetSequenceCycleRate(m_pOuter->GetModelPtr(),
...
#ifdef CLIENT_DLL
    AnimStatePrintf(
        iLine++, "%s(%d), weight: %.2f, cycle: %.2f, order (%d), aim (%d)",
        !pLayer->IsActive() ? "-- "
                            : (pLayer->m_nSequence == 0
                                   ? "-- "
                                   : GetSequenceName(m_pOuter->GetModelPtr(),
                                                     pLayer->m_nSequence)),
        ...
        !pLayer->IsActive() ? 0 : (int)pLayer->m_nOrder, i);
#else
    AnimStatePrintf(
        iLine++,
        ...
        !pLayer->IsActive() ? 0 : (int)pLayer->m_nSequence,
        !pLayer->IsActive() ? 0
                            : (int)pLayer->m_fFlags,  // Doesn't exist on client
        !pLayer->IsActive() ? 0 : (float)pLayer->m_flWeight,
        !pLayer->IsActive() ? 0 : (float)pLayer->m_flCycle,
        !pLayer->IsActive() ? 0 : (int)pLayer->m_nOrder, i);
#endif
  ...
  AnimStatePrintf(iLine++, "vel: %.2f, time: %.2f, max: %.2f, animspeed: %.2f",
                  vOuterVel.Length2D(), gpGlobals->curtime,
                  GetInterpolatedGroundSpeed(),
                  m_pOuter->GetSequenceGroundSpeed(m_pOuter->GetSequence()));

  if (m_AnimConfig.m_LegAnimType == LEGANIM_8WAY) {
    AnimStatePrintf(iLine++,
                    "ent yaw: %.2f, body_yaw: %.2f, move_yaw: %.2f, gait_yaw: "
                    "%.2f, body_pitch: %.2f",
                    m_angRender[YAW], g_flLastBodyYaw, m_flLastMoveYaw,
                    m_flGaitYaw, g_flLastBodyPitch);
  } else {
    AnimStatePrintf(iLine++,
                    "ent yaw: %.2f, body_yaw: %.2f, body_pitch: %.2f, move_x: "
                    "%.2f, move_y: %.2f",
                    m_angRender[YAW], g_flLastBodyYaw, g_flLastBodyPitch,
                    m_vLastMovePose.x, m_vLastMovePose.y);
  }
