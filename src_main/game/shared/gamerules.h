...
enum {
  ...
  AUTOAIM_ON_CONSOLE,  // Autoaim is on, including enhanced features for Console
                       // gaming (more assistance, etc)
};
...
abstract_class CGameRules : public CAutoGameSystemPerFrame {
 public:
  ...
  virtual CBaseCombatWeapon *GetNextBestWeapon(
      CBaseCombatCharacter * pPlayer,
      CBaseCombatWeapon * pCurrentWeapon);  // I can't use this weapon anymore,
                                            // get me the next best one.
 ...
 public:
  ...
  virtual void Think(
      void) = 0;  // GR_Think - runs every server frame, should handle any timer
                  // tasks, periodic events, etc.
  virtual bool IsAllowedToSpawn(
      CBaseEntity *
      pEntity) = 0;  // Can this item spawn (eg NPCs don't spawn in deathmatch).
  ...
  virtual CBaseEntity *GetPlayerSpawnSpot(
      CBasePlayer * pPlayer);  // Place this player on their spawnspot and face
  ...
  virtual void ClientSettingsChanged(CBasePlayer *
                                     pPlayer);  // the player has changed cvars
  ...
  virtual int WeaponShouldRespawn(CBaseCombatWeapon *
                                  pWeapon) = 0;  // should this weapon respawn?
  ...
  virtual float FlWeaponTryRespawn(
      CBaseCombatWeapon *
      pWeapon) = 0;  // can i respawn now,  and if not, when should i try again?
  ...
  virtual void PlayerGotItem(
      CBasePlayer * pPlayer,
      CItem * pItem) = 0;  // call each time a player picks up an item (battery,
                           // healthkit)
  ...
  virtual int PlayerRelationship(
      CBaseEntity * pPlayer,
      CBaseEntity *
          pTarget) = 0;  // What is the player's relationship with this entity?
  ...
  // trace line rules
  virtual float WeaponTraceEntity(CBaseEntity * pEntity, const Vector &vecStart,
