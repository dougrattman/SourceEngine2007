...
// info targets are like point entities except you can force them to spawn on
// the client
...
bool IsStaticPointEntity(CBaseEntity *pEnt) {
  if (pEnt->GetMoveParent()) return false;

  if (!pEnt->GetModelIndex()) return 1;

  if (FClassnameIs(pEnt, "info_target") ||
      FClassnameIs(pEnt, "info_landmark") || FClassnameIs(pEnt, "path_corner"))
    return true;
...
#if defined(CLIENT_DLL)
extern bool ComputeBeamEntPosition(CBaseEntity *pEnt, int nAttachment,
                                   bool bInterpretAttachmentIndexAsHitboxIndex,
                                   Vector &pt);

void RecvProxy_Beam_ScrollSpeed(const CRecvProxyData *pData, void *pStruct,
                                void *pOut) {
  C_Beam *beam;
  float val;
...
static void *SendProxy_SendPredictableId(const SendProp *pProp,
                                         const void *pStruct,
                                         const void *pVarData,
  ...
  if (!pEntity->GetOwnerEntity()) return NULL;
...
RecvPropPredictableId(RECVINFO(m_PredictableID)),
...
#if !defined(CLIENT_DLL)
            SendPropInt(SENDINFO(m_nBeamType), Q_log2(NUM_BEAM_TYPES) + 1,
                        SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_nBeamFlags), NUM_BEAM_FLAGS, SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_nNumBeamEnts), 5, SPROP_UNSIGNED),
    SendPropArray3(SENDINFO_ARRAY3(m_hAttachEntity),
                   SendPropEHandle(SENDINFO_ARRAY(m_hAttachEntity))),
    SendPropArray3(SENDINFO_ARRAY3(m_nAttachIndex),
                   SendPropInt(SENDINFO_ARRAY(m_nAttachIndex),
                               ATTACHMENT_INDEX_BITS, SPROP_UNSIGNED)),
    ...
    SendPropFloat(SENDINFO(m_fEndWidth), 10, SPROP_ROUNDUP, 0.0f,
                  MAX_BEAM_WIDTH),
    SendPropFloat(SENDINFO(m_fFadeLength), 0, SPROP_NOSCALE),
    SendPropFloat(SENDINFO(m_fAmplitude), 8, SPROP_ROUNDDOWN, 0.0f,
                  MAX_BEAM_NOISEAMPLITUDE),
    SendPropFloat(SENDINFO(m_fStartFrame), 8, SPROP_ROUNDDOWN, 0.0f, 256.0f),
    SendPropFloat(SENDINFO(m_fSpeed), 8, SPROP_NOSCALE, 0.0f,
                  MAX_BEAM_SCROLLSPEED),
    ...
    SendPropFloat(SENDINFO(m_flFrame), 20,
                  SPROP_ROUNDDOWN | SPROP_CHANGES_OFTEN, 0.0f, 256.0f),
    SendPropInt(SENDINFO(m_clrRender), 32,
    ...
    SendPropVector(SENDINFO(m_vecOrigin), 19, SPROP_CHANGES_OFTEN,
                   MIN_COORD_INTEGER, MAX_COORD_INTEGER),
    SendPropEHandle(SENDINFO_NAME(m_hMoveParent, moveparent)),
    SendPropInt(SENDINFO(m_nMinDXLevel), 8, SPROP_UNSIGNED),
#if !defined(NO_ENTITY_PREDICTION)
    SendPropDataTable("beampredictable_id", 0,
                      &REFERENCE_SEND_TABLE(DT_BeamPredictableId),
                      SendProxy_SendPredictableId),
#endif

#else
RecvPropInt(RECVINFO(m_nBeamType)), RecvPropInt(RECVINFO(m_nBeamFlags)),
    ...
    RecvPropArray3(RECVINFO_ARRAY(m_nAttachIndex),
                   RecvPropInt(RECVINFO(m_nAttachIndex[0]))),
    RecvPropInt(RECVINFO(m_nHaloIndex)), RecvPropFloat(RECVINFO(m_fHaloScale)),
    RecvPropFloat(RECVINFO(m_fWidth)), RecvPropFloat(RECVINFO(m_fEndWidth)),
    ...
    RecvPropInt(RECVINFO(m_clrRender)), RecvPropInt(RECVINFO(m_nRenderFX)),
    RecvPropInt(RECVINFO(m_nRenderMode)), RecvPropFloat(RECVINFO(m_flFrame)),
...
#if !defined(NO_ENTITY_PREDICTION)
    RecvPropDataTable("beampredictable_id", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_BeamPredictableId)),
#endif
...
#if !defined(CLIENT_DLL)
        BEGIN_DATADESC(CBeam) DEFINE_FIELD(m_nHaloIndex, FIELD_MODELINDEX),
    ...
    DEFINE_FIELD(m_fWidth, FIELD_FLOAT), DEFINE_FIELD(m_fEndWidth, FIELD_FLOAT),
...
()
        ...
        BEGIN_PREDICTION_DATA(CBeam)
            ...
            DEFINE_PRED_FIELD(m_nBeamType, FIELD_INTEGER,
                              FTYPEDESC_INSENDTABLE),

    DEFINE_PRED_FIELD(m_nNumBeamEnts, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_ARRAY(m_hAttachEntity, FIELD_EHANDLE, MAX_BEAM_ENTS,
                      FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_ARRAY(m_nAttachIndex, FIELD_INTEGER, MAX_BEAM_ENTS,
                      FTYPEDESC_INSENDTABLE),
    ...
    DEFINE_PRED_FIELD_TOL(m_vecEndPos, FIELD_VECTOR, FTYPEDESC_INSENDTABLE,
                          0.125f),
#ifdef PORTAL
    DEFINE_PRED_FIELD(m_bDrawInMainRender, FIELD_BOOLEAN,
                      FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_FIELD(m_bDrawInPortalRender, FIELD_BOOLEAN,
                      FTYPEDESC_INSENDTABLE),
#endif
    DEFINE_PRED_FIELD(m_nModelIndex, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
    DEFINE_PRED_FIELD_TOL(m_vecOrigin, FIELD_VECTOR, FTYPEDESC_INSENDTABLE,
                          0.125f),
...
void CBeam::SetBeamFlag(int flag) { m_nBeamFlags |= flag; }

int CBeam::GetType(void) const { return m_nBeamType; }

int CBeam::GetBeamFlags(void) const { return m_nBeamFlags; }
...
const Vector &CBeam::GetAbsEndPos(void) const {
  if (GetType() != BEAM_POINTS && GetType() != BEAM_HOSE && GetEndEntity()) {
    ...
    CBaseEntity *ent = CBaseEntity::Instance(pent);
    if (ent) return ent->GetAbsOrigin();
  }
...
const Vector &C_Beam::GetAbsStartPos(void) const {
  ...
  if (GetType() != BEAM_POINTS && GetType() != BEAM_HOSE) {
    if (ComputeBeamEntPosition(m_hAttachEntity[0], m_nAttachIndex[0], false,
                               vecStartAbsPosition))
...
const Vector &C_Beam::GetAbsEndPos(void) const {
  ...
  if (GetType() != BEAM_POINTS && GetType() != BEAM_HOSE) {
    if (ComputeBeamEntPosition(m_hAttachEntity[m_nNumBeamEnts - 1],
                               m_nAttachIndex[m_nNumBeamEnts - 1], false,
                               vecEndAbsPosition))
...
// Output : CSprite
//-----------------------------------------------------------------------------
CBeam *CBeam::BeamCreatePredictable(const char *module, int line, bool persist,
                                    const char *pSpriteName, float width,
                                    CBasePlayer *pOwner) {
#if !defined(NO_ENTITY_PREDICTION)
  CBeam *pBeam = (CBeam *)CBaseEntity::CreatePredictedEntityByName(
      "beam", module, line, persist);
  if (pBeam) {
    pBeam->BeamInit(pSpriteName, width);
...
void CBeam::RelinkBeam(void) {
  ...
  if (pStartEntity) pEntityBeamTraceFilter = pStartEntity->GetBeamTraceFilter();

  CTraceFilterChain traceFilterChain(&traceFilter, pEntityBeamTraceFilter);

  bUseExtraPoints = UTIL_Portal_Trace_Beam(this, startPos, endPos, vecAbsExtra1,
                                           vecAbsExtra2, &traceFilterChain);
#endif
...
CBaseEntity *CBeam::RandomTargetname(const char *szName) {
  ...
  while ((pNewEntity = gEntList.FindEntityByName(pNewEntity, szName)) != NULL) {
    total++;
    if (random->RandomInt(0, total - 1) < 1) pEntity = pNewEntity;
  }
  return pEntity;
#else
  return NULL;
...
void CBeam::BeamDamage(trace_t *ptr) {
  ...
  if (ptr->fraction != 1.0 && ptr->m_pEnt != NULL) {
    ...
    if (pHit) {
      ...
      CTakeDamageInfo info(this, this,
                           m_flDamage * (gpGlobals->curtime - m_flFireTime),
                           nDamageType);
...
void CBeam::InputColorRedValue(inputdata_t &inputdata) {
  int nNewColor = std::clamp(inputdata.value.Float(), 0.0f, 255.0f);
...
int CBeam::UpdateTransmitState(void) {
  ...
  if (GetMoveParent()) return SetTransmitState(FL_EDICT_FULLCHECK);
...
int CBeam::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  if (IsEffectActive(EF_NODRAW)) return FL_EDICT_DONTSEND;
...
int CBeam::DrawDebugTextOverlays(void) {
  ...
  if (m_debugOverlays & OVERLAY_TEXT_BIT) {
    // Print state
    char tempstr[512];
    Q_snprintf(tempstr, sizeof(tempstr), "start: (%.2f,%.2f,%.2f)",
               GetAbsOrigin().x, GetAbsOrigin().y, GetAbsOrigin().z);
    EntityText(text_offset, tempstr, 0);
    text_offset++;

    Q_snprintf(tempstr, sizeof(tempstr), "end  : (%.2f,%.2f,%.2f)",
               m_vecEndPos.GetX(), m_vecEndPos.GetY(), m_vecEndPos.GetZ());
    EntityText(text_offset, tempstr, 0);
    text_offset++;
  }
...
//-----------------------------------------------------------------------------
bool CBeam::OnPredictedEntityRemove(bool isbeingremoved,
...
int CBeam::DrawModel(int flags) {
  ...
  if (g_bRenderingScreenshot || !r_drawviewmodel.GetBool()) {
    ...
    for (int i = 0; i < MAX_BEAM_ENTS; i++) {
      C_BaseViewModel *vm =
          dynamic_cast<C_BaseViewModel *>(m_hAttachEntity[i].Get());
      if (vm) {
        return 0;
      }
    }
...
void CBeam::OnDataChanged(DataUpdateType_t updateType) {
  ...
  // Convert weapon world models to viewmodels if they're weapons being carried
  // by the local player
  for (int i = 0; i < MAX_BEAM_ENTS; i++) {
...
bool CBeam::IsTransparent(void) { return true; }
...
void CBeam::ComputeBounds(Vector &mins, Vector &maxs) {
  ...
  bUseExtraPoints =
      UTIL_Portal_Trace_Beam(this, vecAbsStart, vecAbsEnd, vecAbsExtra1,
                             vecAbsExtra2, &traceFilterChain);
#endif
      ...
      for (int i = 0; i < m_nNumBeamEnts; ++i) {
        ...
        if (pTestEnt) {
          if (pTestEnt == this) {
            ...
            // We do this so we don't have to calculate attachments (and do
            // expensive bone-setup calculations) on our attachments.
