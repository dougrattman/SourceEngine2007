...
ConVar props_break_max_pieces(
    "props_break_max_pieces", "-1", 0,
    "Maximum prop breakable piece count (-1 = model default)");
ConVar props_break_max_pieces_perframe(
    "props_break_max_pieces_perframe", "-1", FCVAR_REPLICATED,
    "Maximum prop breakable piece count per frame (-1 = model default)");
#ifdef GAME_DLL
extern ConVar breakable_multiplayer;
#endif  // GAME_DLL

extern bool PropBreakableCapEdictsOnCreateAll(
    int modelindex, IPhysicsObject *pPhysics,
    const breakablepropparams_t &params, CBaseEntity *pEntity,
    int iPrecomputedBreakableCount = -1);
extern CBaseEntity *BreakModelCreateSingle(
...
//-----------------------------------------------------------------------------
// Purpose: Given two symmetric boxes, rotate the coordinate frame by the
// necessary
...
//			&boxExtents2 -
//-----------------------------------------------------------------------------
static void AlignBoxes(matrix3x4_t *pInOutMatrix, const Vector &boxExtents1,
                       const Vector &boxExtents2) {
  int rotCount = 0;
  struct {
    int axis0;
    int axis1;
  ...
  while (rotCount > 0) {
    rotCount--;
    MatrixRot90(*pInOutMatrix, rotations[rotCount].axis0,
                rotations[rotCount].axis1);
  }
}
...
propdata_interaction_s
    sPropdataInteractionSections[PROPINTER_NUM_INTERACTIONS] = {
        {"physgun_interactions", "onworldimpact",
         "stick"},  // PROPINTER_PHYSGUN_WORLD_STICK,
        {"physgun_interactions", "onfirstimpact",
         "break"},  // PROPINTER_PHYSGUN_FIRST_BREAK,
        {"physgun_interactions", "onfirstimpact",
         "paintsplat"},  // PROPINTER_PHYSGUN_FIRST_PAINT,
        {"physgun_interactions", "onfirstimpact",
         "impale"},  // PROPINTER_PHYSGUN_FIRST_IMPALE,
        {"physgun_interactions", "onlaunch",
         "spin_none"},  // PROPINTER_PHYSGUN_LAUNCH_SPIN_NONE,
        {"physgun_interactions", "onlaunch",
         "spin_zaxis"},  // PROPINTER_PHYSGUN_LAUNCH_SPIN_Z,
        {"physgun_interactions", "onbreak",
         "explode_fire"},  // PROPINTER_PHYSGUN_BREAK_EXPLODE,
        {"physgun_interactions", "damage",
         "none"},  // PROPINTER_PHYSGUN_DAMAGE_NONE,

        {"fire_interactions", "flammable", "yes"},  // PROPINTER_FIRE_FLAMMABLE,
        {"fire_interactions", "explosive_resist",
         "yes"},  // PROPINTER_FIRE_EXPLOSIVE_RESIST,
        {"fire_interactions", "ignite",
         "halfhealth"},  // PROPINTER_FIRE_IGNITE_HALFHEALTH,

        {"physgun_interactions", "onpickup",
         "create_flare"},  // PROPINTER_PHYSGUN_CREATE_FLARE,

        {"physgun_interactions", "allow_overhead",
         "yes"},  // 	PROPINTER_PHYSGUN_ALLOW_OVERHEAD,

        {"world_interactions", "onworldimpact",
         "bloodsplat"},  // PROPINTER_WORLD_BLOODSPLAT,
};
#else
extern propdata_interaction_s
    sPropdataInteractionSections[PROPINTER_NUM_INTERACTIONS];
#endif
...
void CPropData::ParsePropDataFile(void) {
  ...
  if (pBreakableSection) {
    ...
    while (pChunkSection) {
      ...
      pBreakableChunk->iszChunkType =
          AllocPooledString(pChunkSection->GetName());
...
int CPropData::ParsePropFromKV(CBaseEntity *pProp, KeyValues *pSection,
  ...
  if (!pBreakableInterface) return PARSE_FAILED_BAD_DATA;
  ...
  if (pszBase && pszBase[0]) {
    ...
    if ((iBaseResult != PARSE_SUCCEEDED) &&
        (iBaseResult != PARSE_SUCCEEDED_ALLOWED_STATIC))
      return iBaseResult;
  }
  ...
  if (pBreakableInterface->GetPhysicsDamageTable() == NULL_STRING) {
    ...
    pszTableName = pSection->GetString(
        "damage_table", STRING(pBreakableInterface->GetPhysicsDamageTable()));
  }
  ...
  pBreakableInterface->SetPhysicsMode(
      pSection->GetInt("physicsmode", pBreakableInterface->GetPhysicsMode()));

  const char *multiplayer_break =
      pSection->GetString("multiplayer_break", NULL);
  ...
  // Get damage modifiers, but only if they're specified, because our base may
  // have already overridden them.
  pBreakableInterface->SetDmgModBullet(pSection->GetFloat(
      "dmg.bullets", pBreakableInterface->GetDmgModBullet()));
  pBreakableInterface->SetDmgModClub(
      pSection->GetFloat("dmg.club", pBreakableInterface->GetDmgModClub()));
  pBreakableInterface->SetDmgModExplosive(pSection->GetFloat(
      "dmg.explosive", pBreakableInterface->GetDmgModExplosive()));

  // Get the health (unless this is an override prop)
  if (!FClassnameIs(pProp, "prop_physics_override") &&
      !FClassnameIs(pProp, "prop_dynamic_override")) {
    pProp->SetHealth(pSection->GetInt("health", pProp->GetHealth()));

    // Explosive?
    pBreakableInterface->SetExplosiveDamage(pSection->GetFloat(
        "explosive_damage", pBreakableInterface->GetExplosiveDamage()));
    pBreakableInterface->SetExplosiveRadius(pSection->GetFloat(
        "explosive_radius", pBreakableInterface->GetExplosiveRadius()));
  ...
  if (pBreakableInterface->GetBreakableModel() == NULL_STRING) {
    ...
    pszBreakableModel = pSection->GetString(
        "breakable_model", STRING(pBreakableInterface->GetBreakableModel()));
  }
  if (pszBreakableModel && pszBreakableModel[0]) {
    pBreakableInterface->SetBreakableModel(
        AllocPooledString(pszBreakableModel));
  } else {
    pBreakableInterface->SetBreakableModel(NULL_STRING);
  }
  pBreakableInterface->SetBreakableSkin(pSection->GetInt(
      "breakable_skin", pBreakableInterface->GetBreakableSkin()));
  pBreakableInterface->SetBreakableCount(pSection->GetInt(
      "breakable_count", pBreakableInterface->GetBreakableCount()));
  ...
  for (int i = 0; i < PROPINTER_NUM_INTERACTIONS; i++) {
    // If we hit this assert, we have too many interactions for our current
    // storage solution to handle
    Assert(i < 32);

    propdata_interaction_s *pInteraction = &sPropdataInteractionSections[i];

    KeyValues *pkvCurrentInter =
        pInteractionSection->FindKey(pInteraction->pszSectionName);
    if (pkvCurrentInter) {
      char const *pszInterBase =
          pkvCurrentInter->GetString(pInteraction->pszKeyName);
      if (pszInterBase && pszInterBase[0] &&
          !stricmp(pszInterBase, pInteraction->pszValue)) {
  ...
  if (pSection->GetInt("allowstatic", 0)) return PARSE_SUCCEEDED_ALLOWED_STATIC;
...
int CPropData::ParsePropFromBase(CBaseEntity *pProp, const char *pszPropData) {
  if (!m_bPropDataLoaded) return PARSE_FAILED_NO_DATA;
  ...
  if (!pSection) {
    Warning(
        "%s '%s' has a base specified as '%s', but there is no matching entry "
        "in propdata.txt.\n",
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CPropData::GetRandomChunkModel(const char *pszBreakableSection,
                                           int iMaxSize) {
  if (!m_bPropDataLoaded) return NULL;

  // Find the right section
  int iCount = m_BreakableChunks.Count();
  int i;
  for (i = 0; i < iCount; i++) {
    if (!Q_strncmp(STRING(m_BreakableChunks[i].iszChunkType),
                   pszBreakableSection, strlen(pszBreakableSection)))
      break;
  }
  ...
  if (iMaxSize == -1) {
    ...
    iRandom = RandomInt(
        0, std::min(iMaxSize, m_BreakableChunks[i].iszChunkModels.Count() - 1));
  }
...
static const char *FixupModelName(char *pOut, int sizeOut,
                                  const char *pModelNameIn) {
  char tmp[1024];
 ...
 public:
  ...
  virtual void ParseKeyValue(void *pData, const char *pKey,
    ...
    if (!strcmpi(pKey, "model")) {
      ...
      pModel->collisionGroup = atoi(pValue) > 0 ? COLLISION_GROUP_DEBRIS
...
void BreakModelList(CUtlVector<breakmodel_t> &list, int modelindex,
  ...
  IVPhysicsKeyParser *pParse =
      physcollision->VPhysicsKeyParserCreate(pCollide->pKeyValues);
...
int GetAutoMultiplayerPhysicsMode(Vector size, float mass) {
  ...
  if (mass < 8.0) return PHYSICS_MULTIPLAYER_NON_SOLID;
...
bool CGameGibManager::AllowedToSpawnGib(void) {
  if (m_bAllowNewGibs) return true;

  // We're not tracking gibs at the moment
  if (m_iCurrentMaxPieces < 0) return true;

  if (m_iCurrentMaxPieces == 0) return false;
...
void CGameGibManager::AddGibToLRU(CBaseAnimating *pEntity) {
  ...
  // We're not tracking gibs at the moment
  if (m_iCurrentMaxPieces <= 0) return;
...
CGameGibManager *GetGibManager(void) {
  ...
  if (g_hGameGibManager == NULL) {
    g_hGameGibManager = (CGameGibManager *)gEntList.FindEntityByClassname(
        NULL, "game_gib_manager");
  }
...
void PropBreakableCreateAll(int modelindex, IPhysicsObject *pPhysics,
                            ...
                            bool bIgnoreGibLimit, bool defaultLocation) {
  ...
  if (nPropCount != -1) {
    ...
    if (nPropBreakablesPerFrameCount >= nPropCount) return;
  }
  ...
  if (iMaxBreakCount != -1) {
    if (iPrecomputedBreakableCount != -1) {
      iPrecomputedBreakableCount =
          std::min(iMaxBreakCount, iPrecomputedBreakableCount);
    } else {
  ...
  if (!PropBreakableCapEdictsOnCreateAll(modelindex, pPhysics, params, pEntity,
                                         iPrecomputedBreakableCount)) {
  ...
  int parentAttachment =
      Studio_FindAttachment(&studioHdr, "placementOrigin") + 1;
  ...
  if (list.Count()) {
    for (int i = 0; i < list.Count(); i++) {
      ...
      {
#ifdef GAME_DLL
        if (list[i].mpBreakMode == MULTIPLAYER_BREAK_CLIENTSIDE) continue;
#else
        if (list[i].mpBreakMode == MULTIPLAYER_BREAK_SERVERSIDE) continue;
#endif

        if (!defaultLocation &&
            list[i].mpBreakMode == MULTIPLAYER_BREAK_DEFAULT)
          continue;
      }
      ...
      if (pOwnerAnim && list[i].placementName[0]) {
        if (list[i].placementIsBone) {
          ...
          int attachmentIndex =
              Studio_FindAttachment(&studioHdr, list[i].placementName) + 1;
      ...
      if (nActualSkin > studioHdr.numskinfamilies()) nActualSkin = 0;
...
#endif
      {
        pBreakable = BreakModelCreateSingle(
            pOwnerEntity, &list[i], position, angles, objectVelocity,
            params.angularVelocity, nActualSkin, params);
      }
      ...
      for (int i = 0; i < pBreakableInterface->GetBreakableCount(); i++) {
        ...
        Q_strncpy(breakModel.modelName,
                  g_PropDataSystem.GetRandomChunkModel(
                      STRING(pBreakableInterface->GetBreakableModel()),
        ...
        pEntity->CollisionProp()->RandomPointInBounds(vecMins, vecMaxs,
                                                      &breakModel.offset);
        ...
        {
          pBreakable = BreakModelCreateSingle(
              pOwnerEntity, &breakModel, breakModel.offset, vecAngles,
              vecVelocity, vec3_origin /*params.angularVelocity*/, iSkin,
              params);
          if (!pBreakable) {
...
void PropBreakableCreateAll(int modelindex, IPhysicsObject *pPhysics,
                            ...
                            float impactEnergyScale, float defBurstScale,
                            int defCollisionGroup, CBaseEntity *pEntity,
...
void PrecacheGibsForModel(int iModel) {
  ...
  // The scale and group doesn't really matter at the moment, we are just using
  // the parser to get the model name to cache.
...
//			defaultLocation -
//-----------------------------------------------------------------------------
CBaseEntity *CreateGibsFromList(CUtlVector<breakmodel_t> &list, int modelindex,
                                IPhysicsObject *pPhysics,
                                const breakablepropparams_t &params,
                                CBaseEntity *pEntity,
                                int iPrecomputedBreakableCount,
                                bool bIgnoreGibLimit, bool defaultLocation,
                                CUtlVector<EHANDLE> *pGibList) {
  // Check for prop breakable count reset.
  int nPropCount = props_break_max_pieces_perframe.GetInt();
  if (nPropCount != -1) {
    if (nFrameNumber != gpGlobals->framecount) {
    ...
    // Check for max breakable count for the frame.
    if (nPropBreakablesPerFrameCount >= nPropCount) return NULL;
  }
