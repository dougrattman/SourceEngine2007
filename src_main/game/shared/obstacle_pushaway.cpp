...
ConVar sv_pushaway_force(
    "sv_pushaway_force", "30000", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "How hard physics objects are pushed away from the players on the server.");
ConVar sv_pushaway_min_player_speed(
    "sv_pushaway_min_player_speed", "75",
    FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "If a player is moving slower than this, don't push away physics objects "
    "(enables ducking behind things).");
ConVar sv_pushaway_max_force(
    "sv_pushaway_max_force", "1000", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "Maximum amount of force applied to physics objects by players.");
ConVar sv_pushaway_clientside(
    "sv_pushaway_clientside", "0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "Clientside physics push away (0=off, 1=only localplayer, 1=all players)");

ConVar sv_pushaway_player_force(
    "sv_pushaway_player_force", "200000",
    FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
    "How hard the player is pushed away from physics objects (falls off with "
    "inverse square of distance).");
ConVar sv_pushaway_max_player_force(
...
bool IsPushAwayEntity(CBaseEntity *pEnt) {
  ...
  if (pEnt->GetCollisionGroup() != COLLISION_GROUP_PUSHAWAY) {
    // Try backing away from doors that are currently rotating, to prevent
    // blocking them
#ifndef CLIENT_DLL
...
bool IsBreakableEntity(CBaseEntity *pEnt) {
  ...
  if (pEnt->m_takedamage != DAMAGE_YES) return false;
  ...
  if (pEnt->m_iHealth > 200) return false;

  IMultiplayerPhysics *pPhysicsInterface =
      dynamic_cast<IMultiplayerPhysics *>(pEnt);
  if (pPhysicsInterface) {
    if (pPhysicsInterface->GetMultiplayerPhysicsMode() !=
        PHYSICS_MULTIPLAYER_SOLID)
      return false;
  } else {
    if ((FClassnameIs(pEnt, "func_breakable") ||
      ...
      if (FClassnameIs(pEnt, "func_breakable_surf")) {
        ...
        if (surf->m_bIsBroken) return false;
      }
    } else if (pEnt->PhysicsSolidMaskForEntity() & CONTENTS_PLAYERCLIP) {
      // hostages and players use CONTENTS_PLAYERCLIP, so we can use it to
      // ignore them
      return false;
    }
  }
  ...
  IBreakableWithPropData *pBreakableInterface =
      dynamic_cast<IBreakableWithPropData *>(pEnt);
...
int GetPushawayEnts(CBaseCombatCharacter *pPushingEntity, CBaseEntity **ents,
  ...
  Ray_t ray;
  ray.Init(pPushingEntity->GetAbsOrigin(), pPushingEntity->GetAbsOrigin(),
           pPushingEntity->GetCollideable()->OBBMins() - vExpand,
           pPushingEntity->GetCollideable()->OBBMaxs() + vExpand);
  ...
  if (physPropEnum) delete physPropEnum;

  return numHit;
}

void AvoidPushawayProps(CBaseCombatCharacter *pPlayer, CUserCmd *pCmd) {
  // Figure out what direction we're moving and the extents of the box we're
  // going to sweep against physics objects.
  Vector currentdir;
  Vector rightdir;
  ...
  for (int i = 0; i < nEnts; i++) {
    // Don't respond to this entity on the client unless it has
    // PHYSICS_MULTIPLAYER_FULL set.
    IMultiplayerPhysics *pInterface =
        dynamic_cast<IMultiplayerPhysics *>(props[i]);
    if (pInterface &&
        pInterface->GetMultiplayerPhysicsMode() != PHYSICS_MULTIPLAYER_SOLID)
      continue;
    ...
    mass = std::clamp(mass, minMass, maxMass);

    mass = std::max(mass, 0.0f);
    mass /= maxMass;  // bring into a 0..1 range

    // Push away from the collision point. The closer our center is to the
    // collision point, the harder we push away.
    props[i]->CollisionProp()->CalcNearestPoint(ourCenter, &nearestPropPoint);
    pPlayer->CollisionProp()->CalcNearestPoint(nearestPropPoint,
                                               &nearestPlayerPoint);
    Vector vPushAway = (nearestPlayerPoint - nearestPropPoint);
    float flDist = VectorNormalize(vPushAway);

    const float MaxPushawayDistance = 5.0f;
    if (flDist > MaxPushawayDistance &&
        !pPlayer->CollisionProp()->IsPointInBounds(nearestPropPoint)) {
      continue;
    }
    ...
    flDist = std::max(flDist, 1.0f);

    float flForce = sv_pushaway_player_force.GetFloat() / flDist * mass;
    flForce = std::min(flForce, sv_pushaway_max_player_force.GetFloat());

#ifndef CLIENT_DLL
    pPlayer->PushawayTouch(props[i]);

    // We can get right up next to rotating doors before they start to move, so
    // scale back our force so we don't go flying
    if (FClassnameIs(props[i], "func_door_rotating") ||
        FClassnameIs(props[i], "prop_door_rotating"))
#endif
    {
      flForce *= 0.25f;
    }
...
void PerformObstaclePushaway(CBaseCombatCharacter *pPushingEntity) {
  if (pPushingEntity->m_lifeState != LIFE_ALIVE) return;
...
#ifdef CLIENT_DLL
  // if sv_pushaway_clientside is disabled, clientside phys objects don't bounce
  // away
  if (sv_pushaway_clientside.GetInt() == 0) return;

  // if sv_pushaway_clientside is 1, only local player can push them
  CBasePlayer *pPlayer = pPushingEntity->IsPlayer()
                             ? (dynamic_cast<CBasePlayer *>(pPushingEntity))
                             : NULL;
  if ((sv_pushaway_clientside.GetInt() == 1) &&
  ...
  for (int i = 0; i < nEnts; i++) {
    // If this entity uas PHYSICS_MULTIPLAYER_FULL set (ie: it's not just
    // debris), and we're moving too slow, don't push it away. Instead, let the
    // client bounce off it. This allows players to get close to and duck behind
    // things without knocking them over.
    IMultiplayerPhysics *pInterface =
        dynamic_cast<IMultiplayerPhysics *>(props[i]);

    if (pInterface &&
        pInterface->GetMultiplayerPhysicsMode() == PHYSICS_MULTIPLAYER_SOLID) {
      if (pPushingEntity->GetAbsVelocity().Length2D() <
          sv_pushaway_min_player_speed.GetFloat())
        continue;
    }
    ...
    if (pObj) {
      Vector vPushAway =
          (props[i]->WorldSpaceCenter() - pPushingEntity->WorldSpaceCenter());
      vPushAway.z = 0;
      ...
      flForce = std::min(flForce, sv_pushaway_max_force.GetFloat());

      pObj->ApplyForceOffset(vPushAway * flForce,
                             pPushingEntity->WorldSpaceCenter());
    }
  }
}
