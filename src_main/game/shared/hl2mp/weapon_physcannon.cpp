...
bool PlayerHasMegaPhysCannon() { return false; }

bool PhysCannonAccountableForObject(CBaseCombatWeapon *pPhysCannon,
                                    CBaseEntity *pObject) {
  // TODO(d.rattman) : This can't be implemented trivially, so I'm leaving it to
  // Steve or Adrian
  Assert(0);
  return false;
}
...
CGrabController::~CGrabController() { DetachEntity(false); }
...
class CPhysCannonEffect {
 public:
  ...
  CMaterialReference &GetMaterial() { return m_hMaterial; }

  CInterpolatedValue &GetAlpha() { return m_Alpha; }
  CInterpolatedValue &GetScale() { return m_Scale; }
...
#define NUM_PHYSCANNON_BEAMS 3
  ...
  void GetEffectParameters(EffectType_t effectID, color32 &color, float &scale,
                           IMaterial **pMaterial, Vector &vecAttachment);
  ...
  CInterpolatedValue m_ElementParameter;  // Used to interpolate the position of
                                          // the articulated elements
  CPhysCannonEffect
...
acttable_t CWeaponPhysCannon::m_acttable[] = {
    ...
    {ACT_HL2MP_GESTURE_RANGE_ATTACK, ACT_HL2MP_GESTURE_RANGE_ATTACK_PHYSGUN,
     false},
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::SetViewModel() { BaseClass::SetViewModel(); }

//-----------------------------------------------------------------------------
// Purpose: Force the cannon to drop anything it's carrying
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::ForceDrop() {
  CloseElements();
  DetachObject();
  StopEffects();
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::DryFire() {
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::PrimaryFireEffect() {
  CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
void CWeaponPhysCannon::PuntVPhysics(CBaseEntity *pEntity,
  ...
  if (Pickup_OnAttemptPhysGunPickup(pEntity, pOwner, PUNTED_BY_CANNON)) {
    IPhysicsObject *pList[VPHYSICS_MAX_OBJECT_LIST_COUNT];
    int listCount =
        pEntity->VPhysicsGetObjectList(pList, std::size(pList));
    if (!listCount) {
...
void CWeaponPhysCannon::ApplyVelocityBasedForce(CBaseEntity *pEntity,
  ...
  // TODO(d.rattman): Josh needs to put a real value in for PHYSGUN_FORCE_PUNTED
  AngularImpulse aVel =
...
void CWeaponPhysCannon::PrimaryAttack() {
  ...
  if (m_bActive) {
    ...
    if (pHeld != NULL) {
      float heldDist =
          (pHeld->WorldSpaceCenter() - pOwner->WorldSpaceCenter()).Length();
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::WeaponIdle() {
...
bool CWeaponPhysCannon::AttachObject(CBaseEntity *pObject,
  ...
  /*	if( pOwner )
     ...
     maxSpeed, hl2_normspeed.GetFloat() ); pOwner->SetMaxSpeed( maxSpeed );
          }*/
...
CWeaponPhysCannon::FindObjectResult_t CWeaponPhysCannon::FindObject() {
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError) {
  ...
  if (ComputeError() > flError) return false;
  if (pPlayer->GetGroundEntity() == pEntity) return false;
  ...
  if (g_debug_physcannon.GetBool()) {
    ...
    debugoverlay->AddBoxOverlay(end, -Vector(2, 2, 2), Vector(2, 2, 2), angles,
                                0, 255, 255, true, 0);
    ...
    NDebugOverlay::Box(GetAttached()->WorldSpaceCenter(),
                       ...
                       Vector(radius + 5, radius + 5, radius + 5), 255, 0, 0,
                       true, 0.0f);
#endif
  }
...
void CWeaponPhysCannon::UpdateObject() {
  CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
  Assert(pPlayer);
...
#ifdef CLIENT_DLL
void CWeaponPhysCannon::ManagePredictedObject() {
    ...
    if (pAttachedObject != GetGrabController().GetAttached()) {
      ...
      if (pPhysics) {
        ...
        GetGrabController().AttachEntity(ToBasePlayer(GetOwner()),
                                         pAttachedObject, pPhysics, false,
                                         vec3_origin, false);
...
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::UpdateElementPosition() {
  CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
void CWeaponPhysCannon::ClientThink() {
...
void CWeaponPhysCannon::ItemPreFrame() {
  ...
  if (localplayer && !localplayer->IsObserver()) ManagePredictedObject();
#endif
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::CheckForTarget() {
#ifndef CLIENT_DLL
  ...
  UTIL_TraceHull(startPos, endPos, -Vector(4, 4, 4), Vector(4, 4, 4),
                 MASK_SHOT | CONTENTS_GRATE, pOwner, COLLISION_GROUP_NONE, &tr);

  if ((tr.fraction != 1.0f) && (tr.m_pEnt != NULL)) {
    // TODO(d.rattman): Try just having the elements always open when pointed at
    // a physics object
...
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::DoEffectIdle() {
#ifdef CLIENT_DLL
  ...
  {
          // Turn on the glow sprites
          for ( int i = PHYSCANNON_GLOW1; i <
  (PHYSCANNON_GLOW1+NUM_GLOW_SPRITES); i++ )
          {
                  m_Parameters[i].GetScale().SetAbsolute( random->RandomFloat(
  0.075f, 0.05f ) * SPRITE_SCALE ); m_Parameters[i].GetAlpha().SetAbsolute(
  random->RandomInt( 24, 32 ) );
          }

          // Turn on the glow sprites
          for ( i = PHYSCANNON_ENDCAP1; i <
  (PHYSCANNON_ENDCAP1+NUM_ENDCAP_SPRITES); i++ )
          {
                  m_Parameters[i].GetScale().SetAbsolute( random->RandomFloat(
  3, 5 ) ); m_Parameters[i].GetAlpha().SetAbsolute( random->RandomInt( 200, 255
  ) );
          }
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::OpenElements() {
  if (m_bOpen) return;
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::CloseElements() {
...
// Output : float
//-----------------------------------------------------------------------------
float CWeaponPhysCannon::GetLoadPercentage() {
...
// Output : CSoundPatch
//-----------------------------------------------------------------------------
CSoundPatch *CWeaponPhysCannon::GetMotorSound() {
  if (m_sndMotor == NULL) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::DestroyEffects() {
#ifdef CLIENT_DLL
...
// Purpose:
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::StartEffects() {
#ifdef CLIENT_DLL
  ...
  if (m_Parameters[PHYSCANNON_BLAST].GetMaterial() == NULL) {
    ...
    if (m_Parameters[PHYSCANNON_BLAST].SetMaterial(PHYSCANNON_BLAST_SPRITE) ==
        false) {
      // This means the texture was not found
      Assert(0);
    }
  ...
  for (int i = PHYSCANNON_GLOW1; i < (PHYSCANNON_GLOW1 + NUM_GLOW_SPRITES);
       i++) {
    if (m_Parameters[i].GetMaterial() != NULL) continue;
    ...
    if (IsCarriedByLocalPlayer()) {
      ...
      m_Parameters[i].SetAttachment(
          LookupAttachment(attachNamesGlowThirdPerson[i - PHYSCANNON_GLOW1]));
    }
...
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::DoEffectClosed() {
#ifdef CLIENT_DLL
...
void CWeaponPhysCannon::DoEffectLaunch(Vector *pos) {
  ...
  if (pos == NULL) {
    ...
    if (m_hAttachedObject) {
      ...
      trace_t tr;
      UTIL_TraceLine(endPos, endPos + (shotDir * MAX_TRACE_LENGTH), MASK_SHOT,
...
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::DoEffectNone() {
#ifdef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CWeaponPhysCannon::DrawEffects() {
...
//-----------------------------------------------------------------------------
bool CWeaponPhysCannon::IsTransparent() { return true; }
