	...
	void RestoreBlock( IRestore *pRestore, const PhysObjectHeader_t &header ) 
	{
		...
		if ( iQueued != m_QueuedRestores.InvalidIndex() )
		{
			...
			if ( pOwner->ShouldSavePhysics() && header.nObjects > 0 )
			{
				...
				if ( pItem )
				{
					int nObjects = std::min( header.nObjects, pItem->header.nObjects );
		...
		if ( nObjects == 1 )
		{
			...
			if ( pPhysCollide )
			{
				if ( !(*pItem->ppPhysObj) )
				{
					...
					DevMsg( "Physics object pointer unexpectedly non-0 before restore. Should be creating physics object in CreatePhysics()?\n" );
			}
			else
