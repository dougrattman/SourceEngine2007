...
static void VMatrixOffset(VMatrix &dest, const VMatrix &matrixIn,
                          const Vector &offset) {
  dest = matrixIn;
  dest.PostTranslate(offset);
}

static void Matrix3x4Offset(matrix3x4_t &dest, const matrix3x4_t &matrixIn,
                            const Vector &offset) {
...
// This does the necessary casting / extract to grab a pointer to a member
// function as a void * UNDONE: Cast to BASEPTR or something else here?
...
CSave::CSave(CSaveRestoreData *pdata)
    : m_pData(pdata), m_pGameInfo(pdata), m_bAsync(pdata->bAsync) {
...
// Purpose: Log data.
//-----------------------------------------------------------------------------
void CSave::Log(const char *pName, fieldtype_t fieldType, void *value,
                int count) {
  ...
  for (int iCount = 0; iCount < count; ++iCount) {
    switch (fieldType) {
      ...
      case FIELD_VECTOR: {
        ...
        Q_snprintf(szTempBuf, sizeof(szTempBuf), "(%f %f %f)", vecValue.x,
                   vecValue.y, vecValue.z);
        Q_strncat(szBuf, szTempBuf, sizeof(szTempBuf), COPY_ALL_CHARACTERS);
        break;
      }
      case FIELD_QUATERNION: {
        ...
        Q_snprintf(szTempBuf, sizeof(szTempBuf), "(%f %f %f %f)", q[0], q[1],
                   q[2], q[3]);
        Q_strncat(szBuf, szTempBuf, sizeof(szTempBuf), COPY_ALL_CHARACTERS);
        break;
      }
      ...
      case FIELD_COLOR32: {
        ...
        Q_snprintf(szTempBuf, sizeof(szTempBuf), "(%d %d %d %d)",
                   (int)pColor[0], (int)pColor[1], (int)pColor[2],
                   (int)pColor[3]);
        Q_strncat(szBuf, szTempBuf, sizeof(szTempBuf), COPY_ALL_CHARACTERS);
      }
      case FIELD_EMBEDDED:
...
void CSave::WriteString(const char *pname, const string_t *stringId,
                        int count) {
  int i, size;

  size = 0;
  for (i = 0; i < count; i++) size += strlen(STRING(stringId[i])) + 1;
...
void CSave::WriteVMatrixWorldspace(const char *pname, const VMatrix *value,
                                   int count) {
  WriteHeader(pname, sizeof(VMatrix) * count);
...
void CSave::WriteMatrix3x4Worldspace(const char *pname,
                                     const matrix3x4_t *value, int count) {
  WriteHeader(pname, sizeof(matrix3x4_t) * count);
  WriteMatrix3x4Worldspace(value, count);
}

void CSave::WriteInterval(const char *pname, const interval_t *value,
                          int count) {
  ...
  switch (pField->fieldType) {
    case FIELD_EMBEDDED: {
      if (pField->flags & FTYPEDESC_PTR) {
        AssertMsg(pField->fieldSize == 1,
                  "Arrays of embedded pointer types presently unsupported by "
                  "save/restore");
        if (pField->fieldSize != 1) return false;
      }

      AssertMsg(
          pField->td != NULL,
          "Embedded type appears to have not had type description implemented");
      if (pField->td == NULL) return false;
      ...
      char *pTestData =
          (char *)((!(pField->flags & FTYPEDESC_PTR)) ? pData
                                                      : *((void **)pData));
      while (--nFieldCount >= 0) {
        typedescription_t *pTestField = pField->td->dataDesc;
        typedescription_t *pLimit =
            pField->td->dataDesc + pField->td->dataNumFields;

        for (; pTestField < pLimit; ++pTestField) {
          if (ShouldSaveField(
                  pTestData + pTestField->fieldOffset[TD_OFFSET_NORMAL],
                  pTestField))
            return true;
        }
    ...
    case FIELD_CUSTOM: {
      ...
      SaveRestoreFieldInfo_t fieldInfo = {
          ...
          ((char *)pData) - pField->fieldOffset[TD_OFFSET_NORMAL], pField};
      if (pField->pSaveRestoreOps->IsEmpty(fieldInfo)) return false;
    }
      return true;

    case FIELD_EHANDLE: {
      if ((pField->fieldSizeInBytes !=
           pField->fieldSize * gSizes[pField->fieldType])) {
        Warning(
      ...
      for (int i = 0; i < pField->fieldSize; ++i, ++pEHandle) {
        if ((*pEHandle) != 0xFFFFFFFF) return true;
      }
    }
      return false;
    ...
    default: {
      ...
      // old byte-by-byte 0 check
      if (DataEmpty((const char *)pData,
                    pField->fieldSize * gSizes[pField->fieldType]))
        return false;
    }
      return true;
...
bool CSave::WriteBasicField(const char *pname, void *pData, datamap_t *pRootMap,
  ...
  switch (pField->fieldType) {
    ...
    case FIELD_CUSTOM: {
      ...
      SaveRestoreFieldInfo_t fieldInfo = {
          pData, ((char *)pData) - pField->fieldOffset[TD_OFFSET_NORMAL],
          pField};
...
bool CSave::WriteField(const char *pname, void *pData, datamap_t *pRootMap,
                       typedescription_t *pField) {
#ifdef _DEBUG
...
int CSave::WriteFields(const char *pname, const void *pBaseData,
                       datamap_t *pRootMap, typedescription_t *pFields,
                       int fieldCount) {
  ...
  for (int i = 0; i < fieldCount; i++) {
    pTest = &pFields[i];
    void *pOutputData =
        ((char *)pBaseData + pTest->fieldOffset[TD_OFFSET_NORMAL]);

    if (!ShouldSaveField(pOutputData, pTest)) continue;

    if (!WriteField(pname, pOutputData, pRootMap, pTest)) break;
    count++;
  }
...
int CSave::DoWriteAll(const void *pLeafObject, datamap_t *pLeafMap,
                      datamap_t *pCurMap) {
  ...
  return WriteFields(pCurMap->dataClassName, pLeafObject, pLeafMap,
                     pCurMap->dataDesc, pCurMap->dataNumFields);
}
...
void CSave::EndBlock() {
  ...
  // Move to the the location where the size of the block was written & rewrite
  // the size
...
void CSave::BufferString(char *pdata, int len) {
  ...
  BufferData(&c, 1);       // Write a 0 terminator
}
...
int CSave::EntityFlagsSet(int entityIndex, int flags) {
  if (!m_pGameInfo || entityIndex < 0) return 0;
...
void CSave::WriteTime(const char *pname, const float *data, int count) {
  ...
  for (i = 0; i < count; i++) {
    // Always encode time as a delta from the current time so it can be re-based
    // if loaded in a new level Times of 0 are never written to the file, so
    // they will be restored as 0, not a relative time
...
void CSave::WriteFunction(datamap_t *pRootMap, const char *pname,
...
void CSave::WriteEntityPtr(const char *pname, CBaseEntity **ppEntity,
  ...
  AssertMsg(
      ...
      "Array of entities or ehandles exceeds limit supported by save/restore");
...
void CSave::WriteEHandle(const char *pname, const EHANDLE *pEHandle,
                         int count) {
  AssertMsg(
      count <= MAX_ENTITYARRAY,
      "Array of entities or ehandles exceeds limit supported by save/restore");
  ...
  for (int i = 0; i < count && i < MAX_ENTITYARRAY; i++) {
    entityArray[i] =
        EntityIndex((CBaseEntity *)(const_cast<EHANDLE *>(pEHandle)[i]));
  }
  WriteInt(pname, entityArray, count);
}
...
bool CSave::WriteGameField(const char *pname, void *pData, datamap_t *pRootMap,
  ...
  switch (pField->fieldType) {
    case FIELD_CLASSPTR:
      WriteEntityPtr(pField->fieldName, (CBaseEntity **)pData,
                     pField->fieldSize);
      break;
    ...
    case FIELD_POSITION_VECTOR:
      WritePositionVector(pField->fieldName, (Vector *)pData,
                          pField->fieldSize);
      break;
    ...
    case FIELD_MODELINDEX: {
      ...
      WriteString(pField->fieldName, (string_t *)&strModelName,
                  pField->fieldSize);
    } break;
    ...
    case FIELD_MATERIALINDEX: {
      ...
      WriteString(pField->fieldName, (string_t *)&strMaterialName,
                  pField->fieldSize);
    } break;
    ...
    // For now, just write the address out, we're not going to change memory
    // while doing this yet!
    case FIELD_FUNCTION:
      WriteFunction(pRootMap, pField->fieldName, (int *)(char *)pData,
                    pField->fieldSize);
      break;
    ...
    case FIELD_VMATRIX_WORLDSPACE:
      WriteVMatrixWorldspace(pField->fieldName, (VMatrix *)pData,
                             pField->fieldSize);
      break;

    case FIELD_MATRIX3X4_WORLDSPACE:
      WriteMatrix3x4Worldspace(pField->fieldName, (const matrix3x4_t *)pData,
                               pField->fieldSize);
      break;
...
CRestore::CRestore(CSaveRestoreData *pdata)
    : m_pData(pdata), m_pGameInfo(pdata), m_global(0), m_precache(true) {
  ...
  switch (pField->fieldType) {
    ...
    case FIELD_EMBEDDED: {
      ...
      if (!(pField->flags & FTYPEDESC_PTR) || *((void **)pDest)) {
        int nFieldCount = pField->fieldSize;
        char *pFieldData =
            (char *)((!(pField->flags & FTYPEDESC_PTR)) ? pDest
                                                        : *((void **)pDest));
        while (--nFieldCount >= 0) {
          // No corresponding "block" (see write) as it was used as the header
          // of the field
        ...
        Warning(
            "Attempted to restore FIELD_EMBEDDEDBYREF %s but there is no "
            "destination memory\n",
            pField->fieldName);
      }
      break;
    ...
    case FIELD_CUSTOM: {
      // No corresponding "block" (see write) as it was used as the header of
      // the field
      int posNextField = GetReadPos() + header.size;

      SaveRestoreFieldInfo_t fieldInfo = {
          pDest, ((char *)pDest) - pField->fieldOffset[TD_OFFSET_NORMAL],
          pField};
...
void CRestore::ReadField(const SaveRestoreRecordHeader_t &header, void *pDest,
...
bool CRestore::ShouldReadField(typedescription_t *pField) {
  if ((pField->flags & FTYPEDESC_SAVE) == 0) return false;
...
typedescription_t *CRestore::FindField(const char *pszFieldName,
  ...
  if (pszFieldName) {
    ...
    for (int i = 0; i < fieldCount; i++) {
      ...
      if (stricmp(pTest->fieldName, pszFieldName) == 0) return pTest;
    }
  }
...
void CRestore::EmptyFields(void *pBaseData, typedescription_t *pFields,
                           int fieldCount) {
  int i;
  for (i = 0; i < fieldCount; i++) {
    typedescription_t *pField = &pFields[i];
    if (!ShouldEmptyField(pField)) continue;

    void *pFieldData =
        (char *)pBaseData + pField->fieldOffset[TD_OFFSET_NORMAL];
    switch (pField->fieldType) {
      case FIELD_CUSTOM: {
        SaveRestoreFieldInfo_t fieldInfo = {pFieldData, pBaseData, pField};
        pField->pSaveRestoreOps->MakeEmpty(fieldInfo);
      } break;

      case FIELD_EMBEDDED: {
        if ((pField->flags & FTYPEDESC_PTR) && !*((void **)pFieldData)) break;

        int nFieldCount = pField->fieldSize;
        char *pFieldMemory = (char *)((!(pField->flags & FTYPEDESC_PTR))
                                          ? pFieldData
                                          : *((void **)pFieldData));
        while (--nFieldCount >= 0) {
          EmptyFields(pFieldMemory, pField->td->dataDesc,
                      pField->td->dataNumFields);
      ...
      default:
        ...
        memset(pFieldData, (pField->fieldType != FIELD_EHANDLE) ? 0 : 0xFF,
               pField->fieldSize * gSizes[pField->fieldType]);
        break;
    }
  }
...
void CRestore::StartBlock(char szBlockName[]) {
  ...
  StartBlock(&header);
  Q_strncpy(szBlockName, StringFromHeaderSymbol(header.symbol),
            SIZE_BLOCK_NAME_BUF);
}
...
int CRestore::ReadFields(const char *pname, void *pBaseData,
                         datamap_t *pRootMap, typedescription_t *pFields,
                         int fieldCount) {
  ...
  if (curSym != symName)  // Field Set marker
  {
    ...
    Msg("Expected %s found %s ( raw '%s' )! (prev: %s)\n", pname, pCurName,
        BufferPointer(), pLastName);
    Msg("Field type name may have changed or inheritance graph changed, save "
        "file is suspect\n");
  ...
  int searchCookie =
      0;  // Make searches faster, most data is read/written in the same order
  SaveRestoreRecordHeader_t header;
  ...
  for (i = 0; i < nFieldsSaved; i++) {
    ...
    typedescription_t *pField =
        FindField(m_pData->StringFromSymbol(header.symbol), pFields, fieldCount,
                  &searchCookie);
...
int CRestore::DoReadAll(void *pLeafObject, datamap_t *pLeafMap,
                        datamap_t *pCurMap) {
  ...
  return ReadFields(pCurMap->dataClassName, pLeafObject, pLeafMap,
                    pCurMap->dataDesc, pCurMap->dataNumFields);
}
...
void CRestore::ReadString(char *pDest, int nSizeDest, int nBytesAvailable) {
  ...
  if (!nBytesAvailable) nBytesAvailable = strlen(pString) + 1;
...
int CRestore::ReadString(string_t *pValue, int nElems, int nBytesAvailable) {
  AssertMsg(nBytesAvailable > 0,
            "CRestore::ReadString() implementation does not currently support "
            "unspecified bytes available");
...
int CRestore::ReadQuaternion(Quaternion *pValue, int nElems,
                             int nBytesAvailable) {
  return ReadSimple(pValue, nElems, nBytesAvailable);
}
...
int CRestore::ReadVMatrixWorldspace(VMatrix *pValue, int nElems,
                                    int nBytesAvailable) {
...
int CRestore::ReadMatrix3x4Worldspace(matrix3x4_t *pValue, int nElems,
                                      int nBytesAvailable) {
  Vector basePosition = m_pGameInfo->GetLandmark();
  matrix3x4_t tmp;
...
int CRestore::ReadInterval(interval_t *interval, int count,
                           int nBytesAvailable) {
...
CBaseEntity *CRestore::EntityFromIndex(int entityIndex) {
  if (!m_pGameInfo || entityIndex < 0) return NULL;
  ...
  for (i = 0; i < m_pGameInfo->NumEntities(); i++) {
    ...
    if (pTable->id == entityIndex) return pTable->hEnt;
  }
  return NULL;
}
...
int CRestore::ReadEntityPtr(CBaseEntity **ppEntity, int count,
                            int nBytesAvailable) {
  AssertMsg(
      count <= MAX_ENTITYARRAY,
      "Array of entities or ehandles exceeds limit supported by save/restore");
...
void CRestore::ReadGameField(const SaveRestoreRecordHeader_t &header,
  ...
  switch (pField->fieldType) {
    ...
    case FIELD_MODELINDEX: {
      ...
      string_t *pModelName =
          (string_t *)stackalloc(pField->fieldSize * sizeof(string_t));
    ...
    case FIELD_SOUNDNAME: {
      ...
      if (m_precache) {
#if !defined(CLIENT_DLL)
        // HACKHACK: Rewrite the .bsp models to match the map name in case the
    ...
    case FIELD_MATRIX3X4_WORLDSPACE:
      ReadMatrix3x4Worldspace((matrix3x4_t *)pDest, pField->fieldSize,
                              header.size);
      break;
...
int CRestore::ReadPositionVector(Vector *pValue, int count,
                                 int nBytesAvailable) {
  Vector basePosition = m_pGameInfo->GetLandmark();
  int nRead = ReadVector(pValue, count, nBytesAvailable);

  for (int i = nRead - 1; i >= 0; i--) {
    if (pValue[i] != vec3_invalid) pValue[i] += basePosition;
  }
...
int CRestore::ReadFunction(datamap_t *pMap, void **pValue, int count,
                           int nBytesAvailable) {
  AssertMsg(nBytesAvailable > 0,
            "CRestore::ReadFunction() implementation does not currently "
            "support unspecified bytes available");
...
BEGIN_SIMPLE_DATADESC(entitytable_t)
  DEFINE_FIELD(id, FIELD_INTEGER), DEFINE_FIELD(edictindex, FIELD_INTEGER),
...
//-----------------------------------------------------------------------------
int CEntitySaveUtils::GetEntityDependencies(CBaseEntity *pEntity, int nCount,
                                            CBaseEntity **ppEntList) {
  return m_pLevelAdjacencyDependencyHash->GetPairListForObject(
      pEntity, nCount, (void **)ppEntList);
}

//-----------------------------------------------------------------------------
// Methods of IEntitySaveUtils
//-----------------------------------------------------------------------------
void CEntitySaveUtils::AddLevelTransitionSaveDependency(CBaseEntity *pEntity1,
                                                        CBaseEntity *pEntity2) {
...
class CEntitySaveRestoreBlockHandler : public ISaveRestoreBlockHandler {
 ...
 private:
  friend int CreateEntityTransitionList(CSaveRestoreData *pSaveData,
                                        int levelMask);
  ...
  int RestoreEntity(CBaseEntity *pEntity, IRestore *pRestore,
                    entitytable_t *pEntInfo);

#if !defined(CLIENT_DLL)
  // Find the matching global entity.  Spit out an error if the designer made
  // entities of different classes with the same global name
  CBaseEntity *FindGlobalEntity(string_t classname, string_t globalname);

  int RestoreGlobalEntity(CBaseEntity *pEntity, CSaveRestoreData *pSaveData,
                          entitytable_t *pEntInfo);
#endif
...
#else
  // Do this because it'll force entities to figure out their origins, and that
  // requires SetupBones in the case of aiments.
  {
...
void CEntitySaveRestoreBlockHandler::Save(ISave *pSave) {
  ...
  for (int i = 0; i < pSaveData->NumEntities(); i++) {
    ...
    if (pEnt && !(pEnt->ObjectCaps() & FCAP_DONT_SAVE)) {
      MDLCACHE_CRITICAL_SECTION();
#if !defined(CLIENT_DLL)
      AssertMsg(!pEnt->edict() ||
                    (pEnt->m_iClassname != NULL_STRING &&
      ...
      pEntInfo->size =
          pSave->GetWritePos() -
          pEntInfo
              ->location;  // Size of entity block is data size written to block

      pEntInfo->classname =
          pEnt->m_iClassname;  // Remember entity class for respawn
      ...
      bool bIsPlayer =
          ((nEntIndex >= 1) && (nEntIndex <= gpGlobals->maxClients)) ? true
                                                                     : false;
      if (bIsPlayer) {
...
void CEntitySaveRestoreBlockHandler::WriteSaveHeaders(ISave *pSave) {
  ...
  for (int i = 0; i < pSaveData->NumEntities(); i++)
    pSave->WriteFields("ETABLE", pSaveData->GetEntityInfo(i), NULL,
                       entitytable_t::m_DataMap.dataDesc,
                       entitytable_t::m_DataMap.dataNumFields);
}
...
void CEntitySaveRestoreBlockHandler::ReadRestoreHeaders(IRestore *pRestore) {
  ...
  entitytable_t *pEntityTable = (entitytable_t *)engine->SaveAllocMemory(
      (sizeof(entitytable_t) * nEntities), sizeof(char));
  if (!pEntityTable) {
    return;
  }
  ...
  for (int i = 0; i < pSaveData->NumEntities(); i++)
    pRestore->ReadFields("ETABLE", pSaveData->GetEntityInfo(i), NULL,
                         entitytable_t::m_DataMap.dataDesc,
                         entitytable_t::m_DataMap.dataNumFields);
}
...
void CEntitySaveRestoreBlockHandler::Restore(IRestore *pRestore,
                                             bool createPlayers) {
  entitytable_t *pEntInfo;
  CBaseEntity *pent;
  ...
  for (i = 0; i < pSaveData->NumEntities(); i++) {
    ...
    if (pEntInfo->classname != NULL_STRING && pEntInfo->size &&
      ...
      } else if ((pEntInfo->edictindex > 0) &&
                 (pEntInfo->edictindex <= gpGlobals->maxClients)) {
...
void CEntitySaveRestoreBlockHandler::Restore(IRestore *pRestore,
  ...
  for (i = 0; i < pSaveData->NumEntities(); i++) {
    ...
    // BUGBUG: JAY: Disable ragdolls across transitions until PVS/solid check &
    // client entity patch file are implemented
  ...
  // Tell hud elements to modify behavior based on game restoration, if
  // applicable
  gHUD.OnRestore();
}
#endif
...
void SaveEntityOnTable(CBaseEntity *pEntity, CSaveRestoreData *pSaveData,
                       int &iSlot) {
...
bool CEntitySaveRestoreBlockHandler::SaveInitEntities(
...
#endif
  entitytable_t *pEntityTable = (entitytable_t *)engine->SaveAllocMemory(
      (sizeof(entitytable_t) * number_of_entities), sizeof(char));
  if (!pEntityTable) return false;
  ...
  // if an entity requires a certain edict number upon restore, save that as
  // well
  CBaseEntity *pEnt = NULL;
  int i = 0;
...
// Find the matching global entity.  Spit out an error if the designer made
// entities of different classes with the same global name
CBaseEntity *CEntitySaveRestoreBlockHandler::FindGlobalEntity(
    string_t classname, string_t globalname) {
  CBaseEntity *pReturn = NULL;

  while ((pReturn = gEntList.NextEnt(pReturn)) != NULL) {
    if (FStrEq(STRING(pReturn->m_iGlobalname), STRING(globalname))) break;
  }
...
bool CEntitySaveRestoreBlockHandler::DoRestoreEntity(CBaseEntity *pEntity,
                                                     IRestore *pRestore) {
  MDLCACHE_CRITICAL_SECTION();
...
// changes in model space for global brush entities (designer models them in
// different coords!)
Vector CEntitySaveRestoreBlockHandler::ModelSpaceLandmark(int modelIndex) {
  ...
  if (modelinfo->GetModelType(pModel) != mod_brush) return vec3_origin;
...
int CEntitySaveRestoreBlockHandler::RestoreEntity(CBaseEntity *pEntity,
                                                  IRestore *pRestore,
                                                  entitytable_t *pEntInfo) {
  if (!DoRestoreEntity(pEntity, pRestore)) return 0;
...
int CEntitySaveRestoreBlockHandler::RestoreGlobalEntity(
    CBaseEntity *pEntity, CSaveRestoreData *pSaveData,
    entitytable_t *pEntInfo) {
  Vector oldOffset;
  ...
  if (!FStrEq(pSaveData->levelInfo.szCurrentMapName,
              GlobalEntity_GetMap(globalIndex))) {
    return 0;
  }
  ...
  if (pNewEntity) {
    ...
    pSaveData->modelSpaceOffset =
        pEntInfo->landmarkModelSpace -
        ModelSpaceLandmark(pNewEntity->GetModelIndex());

    UTIL_Remove(pEntity);
    pEntity =
        pNewEntity;  // we're going to restore this data OVER the old entity
    pEntInfo->hEnt = pEntity;
    // HACKHACK: Do we need system-wide support for removing non-global spawn
    // allocated resources?
    pEntity->VPhysicsDestroyObject();
    Assert(pEntInfo->edictindex == -1);
    // Update the global table to say that the global definition of this entity
    // should come from this level
    GlobalEntity_SetMap(globalIndex, gpGlobals->mapname);
  } else {
    // This entity will be freed automatically by the engine->  If we don't do a
    // restore on a matching entity (below) or call EntityUpdate() to move it to
    // this level, we haven't changed global state at all.
    DevMsg(
...
CSaveRestoreData *SaveInit(int size) {
  ...
  if (size <= 0)
    size = 2 * 1024 * 1024;  // Reserve 2048K for now, UNDONE: Shrink this after
                             // compressing strings
#else
  if (size <= 0)
    size = 3 * 1024 * 1024;  // Reserve 3096K for now, UNDONE: Shrink this after
                             // compressing strings
#endif
  ...
  void *pSaveMemory = engine->SaveAllocMemory(
      sizeof(CSaveRestoreData) + (sizeof(entitytable_t) * numentities) + size,
      sizeof(char));
  if (!pSaveMemory) {
    return NULL;
  }
  ...
  const int nTokens = 0xfff;  // Assume a maximum of 4K-1 symbol table
                              // entries(each of some length)
 ...
 public:
  ...
  void PreSave(CSaveRestoreData *pData) {
    ...
    for (int i = 0; i < m_Handlers.Count(); i++) {
      Q_strncpy(m_BlockHeaders[i].szName, m_Handlers[i]->GetBlockName(),
                MAX_BLOCK_NAME_LEN + 1);
  ...
  void WriteSaveHeaders(ISave *pSave) {
    ...
    memset(&dummyArr[0], 0xff,
           dummyArr.Count() * sizeof(SaveRestoreBlockHeader_t));
  ...
  void CallBlockHandlerRestore(ISaveRestoreBlockHandler *pHandler,
  ...
  void AddBlockHandler(ISaveRestoreBlockHandler *pHandler) {
    // Grody, but... while this class is still isolated in saverestore.cpp, this
    // seems like a fine time to assert:
    AssertMsg(pHandler == &g_EntitySaveRestoreBlockHandler ||
                  (m_Handlers.Count() >= 1 &&
                   m_Handlers[0] == &g_EntitySaveRestoreBlockHandler),
...
//------------------------------------------------------------------------------
void CreateEntitiesInTransitionList(CSaveRestoreData *pSaveData,
                                    int levelMask) {
  CBaseEntity *pent;
  int i;
  for (i = 0; i < pSaveData->NumEntities(); i++) {
...
int CreateEntityTransitionList(CSaveRestoreData *pSaveData, int levelMask) {
  ...
  for (i = 0; i < pSaveData->NumEntities(); i++) {
    ...
    if (pent && (pEntInfo->flags &
                 levelMask))  // Screen out the player if he's not to be spawned
    {
      if (pEntInfo->flags & FENTTABLE_GLOBAL) {
        ...
        // -------------------------------------------------------------------------
        // Pass the "global" flag to the DLL to indicate this entity should only
        // override a matching entity, not be spawned
        if (g_EntitySaveRestoreBlockHandler.RestoreGlobalEntity(pent, pSaveData,
                                                                pEntInfo) > 0) {
          movedCount++;
        ...
        DevMsg(2, "Transferring %s (%d)\n", STRING(pEntInfo->classname),
               pent->edict() ? ENTINDEX(pent->edict()) : -1);
        CRestore restoreHelper(pSaveData);
        if (g_EntitySaveRestoreBlockHandler.RestoreEntity(pent, &restoreHelper,
                                                          pEntInfo) < 0) {
          UTIL_RemoveImmediate(pent);
        } else {
          // needs to be checked.  Do this in a separate pass so that pointers &
          // hierarchy can be traversed
          checkList.AddToTail(i);
        }
      }

      // Remove any entities that were removed using UTIL_Remove() as a result
      // of the above calls to UTIL_RemoveImmediate()
  ...
  for (i = checkList.Count() - 1; i >= 0; --i) {
    ...
    // NOTE: pent can be NULL because UTIL_RemoveImmediate (called below)
    // removes all in hierarchy
    if (!pent) continue;

    MDLCACHE_CRITICAL_SECTION();

    if (!(pEntInfo->flags & FENTTABLE_PLAYER) && UTIL_EntityInSolid(pent)) {
      // this can happen during normal processing - PVS is just a guess, some
      // map areas won't exist in the new map
