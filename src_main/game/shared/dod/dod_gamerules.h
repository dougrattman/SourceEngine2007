...
class CDODGamePlayRules {
 public:
  ...
  CDODGamePlayRules::CDODGamePlayRules() { Reset(); }
...
class CDODViewVectors : public CViewVectors {
 public:
  CDODViewVectors(Vector vView, Vector vHullMin, Vector vHullMax,
                  Vector vDuckHullMin, Vector vDuckHullMax, Vector vDuckView,
                  Vector vObsHullMin, Vector vObsHullMax,
                  Vector vDeadViewHeight, Vector vProneHullMin,
...
class CDODGameRules : public CTeamplayRules {
 public:
  ...
  virtual void RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn,
                            float flRadius, int iClassIgnore,
                            CBaseEntity *pEntityIgnore,
                            bool bIgnoreWorld = false);
  void RadiusStun(const CTakeDamageInfo &info, const Vector &vecSrc,
                  float flRadius);
  virtual void Think();
 ...
 private:
  ...
  void RadiusDamage(const CTakeDamageInfo &info, const Vector &vecSrcIn,
                    float flRadius, int iClassIgnore, bool bIgnoreWorld);
  float GetExplosionDamageAdjustment(
      Vector &vecSrc, Vector &vecEnd, CBaseEntity *pTarget,
      CBaseEntity *pEntityToIgnore);  // returns multiplier between 0.0 and 1.0
                                      // that is the percentage of any damage
                                      // done from vecSrc to vecEnd that
                                      // actually makes it.
  float GetAmountOfEntityVisible(Vector &src, CBaseEntity *pTarget,
                                 CBaseEntity *pEntityToIgnore);  // returns a
                                                                 // value from 0
                                                                 // to 1 that is
                                                                 // the
                                                                 // percentage
...
#define MAX_PLAYERCLASSES_PER_TEAM 16
  ...
  m_hPlayerClassInfoHandles[2][MAX_PLAYERCLASSES_PER_TEAM];
  ...
  bool m_bInitialSpawn;  // first time activating? longer wait time for people
                         // to join
...
#define DOD_RESPAWN_QUEUE_SIZE 10
 ...
 public:
  ...
  int m_iWeaponDistanceBuckets[WEAPON_MAX]
                              [DOD_NUM_WEAPON_DISTANCE_BUCKETS];  // distances
                                                                  // of buckets
                                                                  // are defined
                                                                  // per-weapon
