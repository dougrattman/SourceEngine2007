...
// careful when reusing HL2 DMG_ flags, some of them cancel out the damage, eg
// if they are in DMG_TIMEBASED
...
#define VEC_PRONE_HULL_MAX  \
  DODGameRules()            \
      ->GetDODViewVectors() \
      ->m_vProneHullMax  // MUST be shorter than duck hull for deploy check
...
enum DODPlayerState {
  ...
  // This state can jump to a bunch of other states like STATE_PICKINGCLASS or
  // STATE_DEATH_ANIM.
  STATE_ACTIVE = 0,

  // This is the state you're in when you first enter the server.
  // It's switching between intro cameras every few seconds, and there's a level
  // info screen up.
  ...
  // Either way, you can't move while choosing team or class (or while any menu
  // is up).
  STATE_PICKINGTEAM,   // Choosing team.
  STATE_PICKINGCLASS,  // Choosing class.

  STATE_DEATH_ANIM,       // Playing death anim, waiting for that to finish.
  STATE_DEATH_OBSERVING,  // Done playing death anim. Waiting for keypress to go
                          // into observer mode.
...
#define ALTWPN_CRITERIA_PRONE_DEPLOYED_RELOAD \
  (1 << 5)  // player should use special alt model when prone deployed reloading
...
enum { DOD_COLLISIONGROUP_SHELLS = LAST_SHARED_COLLISION_GROUP };
...
// other magic numbers exist inside the achievements themselves in
