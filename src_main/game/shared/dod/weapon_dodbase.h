...
#define DOD_AMMO_HANDGRENADE_EX \
  "DOD_AMMO_HANDGRENADE_EX"  // the EX is for EXploding! :)
...
void FindHullIntersection(const Vector &vecSrc, trace_t &tr, const Vector &mins,
                          const Vector &maxs, CBaseEntity *pEntity);
...
class CWeaponDODBase : public CBaseCombatWeapon {
 public:
  ...
  // return true if this weapon is an instance of the given weapon type (ie:
  // "IsA" WEAPON_GLOCK)
  ...
  void KickBack(float up_base, float lateral_base, float up_modifier,
                float lateral_modifier, float up_max, float lateral_max,
 ...
 public:
  ...
  bool m_bUseAltWeaponModel;  // use alternate left handed world model? reset on
                              // new sequence
#else
  ...
  bool m_bInAttack;  // True after a semi-auto weapon fires - will not fire a
                     // second time on the same button press
  ...
  virtual CBaseEntity *MeleeAttack(int iDamageAmount, int iDamageType,
                                   float flDmgDelay, float flAttackDelay);
  ...
  virtual Activity GetStrongMeleeActivity(void) {
