...
#ifdef CLIENT_DLL
void RecvProxy_RoundState(const CRecvProxyData *pData, void *pStruct,
                          void *pOut) {
...
RecvPropInt(RECVINFO(m_iRoundState), 0, RecvProxy_RoundState),
    ...
    RecvPropArray3(RECVINFO_ARRAY(m_AlliesRespawnQueue),
                   RecvPropTime(RECVINFO(m_AlliesRespawnQueue[0]))),
    RecvPropArray3(RECVINFO_ARRAY(m_AxisRespawnQueue),
                   RecvPropTime(RECVINFO(m_AxisRespawnQueue[0]))),
...
SendPropInt(SENDINFO(m_iRoundState), 5), SendPropBool(SENDINFO(m_bInWarmup)),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_AlliesRespawnQueue),
                   SendPropTime(SENDINFO_ARRAY(m_AlliesRespawnQueue))),
    SendPropArray3(SENDINFO_ARRAY3(m_AxisRespawnQueue),
                   SendPropTime(SENDINFO_ARRAY(m_AxisRespawnQueue))),
    SendPropInt(SENDINFO(m_iAlliesRespawnHead),
                Q_log2(DOD_RESPAWN_QUEUE_SIZE) + 1, SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_iAlliesRespawnTail),
                Q_log2(DOD_RESPAWN_QUEUE_SIZE) + 1, SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_iAxisRespawnHead),
                Q_log2(DOD_RESPAWN_QUEUE_SIZE) + 1, SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_iAxisRespawnTail),
                Q_log2(DOD_RESPAWN_QUEUE_SIZE) + 1, SPROP_UNSIGNED),
...
#ifndef CLIENT_DLL
        ConVar dod_flagrespawnbonus(
            "dod_flagrespawnbonus", "1.0", FCVAR_GAMEDLL | FCVAR_CHEAT,
            "How many seconds per advantage flag to decrease the respawn time");

ConVar mp_warmup_time("mp_warmup_time", "0", FCVAR_GAMEDLL,
                      "Warmup time length in seconds");
ConVar mp_restartwarmup("mp_restartwarmup", "0", FCVAR_GAMEDLL,
                        "Set to 1 to start or restart the warmup period.");
ConVar mp_cancelwarmup("mp_cancelwarmup", "0", FCVAR_GAMEDLL,
                       "Set to 1 to end the warmup period.");
#endif

ConVar dod_enableroundwaittime("dod_enableroundwaittime", "1", FCVAR_REPLICATED,
                               "Enable timers to wait between rounds.");
ConVar mp_allowrandomclass("mp_allowrandomclass", "1", FCVAR_REPLICATED,
                           "Allow players to select random class");

ConVar dod_bonusroundtime("dod_bonusroundtime", "15", FCVAR_REPLICATED,
                          "Time after round win until round restarts", true, 5,
                          true, 15);
...
#ifdef CLIENT_DLL
void RecvProxy_DODGameRules(const RecvProp *pProp, void **pOut, void *pData,
                            int objectID) {
...
RecvPropDataTable("dod_gamerules_data", 0, 0,
                  &REFERENCE_RECV_TABLE(DT_DODGameRules),
                  RecvProxy_DODGameRules)
END_RECV_TABLE
()
#else
void *SendProxy_DODGameRules(const SendProp *pProp, const void *pStructBase,
                             const void *pData,
...
SendPropDataTable("dod_gamerules_data", 0,
                  &REFERENCE_SEND_TABLE(DT_DODGameRules),
                  SendProxy_DODGameRules)
END_SEND_TABLE
()
#endif
...
void Load_EntText(void) {
  ...
  char szFullName[512];
  Q_snprintf(szFullName, sizeof(szFullName), "maps/%s.ent",
...
// NOTE: the indices here must match TEAM_TERRORIST, TEAM_ALLIES, TEAM_AXIS,
// etc.
char *sTeamNames[] = {"Unassigned", "Spectator", "Allies", "Axis"};
...
CON_COMMAND_F(bot, "Add a bot.", FCVAR_CHEAT) {
  ...
  // -class <index> selects the bot's class. Default is -1 which chooses
...
void RestartRound_f() { DODGameRules()->State_Transition(STATE_RESTART); }
ConCommand cc_Restart("restartround", RestartRound_f, "Restart the round",
                      FCVAR_CHEAT);
...
CDODGameRules::CDODGameRules() {
  ...
  memset(m_iStatsSpawnsPerClass_Allies, 0,
         sizeof(m_iStatsSpawnsPerClass_Allies));
  ...
  memset(m_iStatsDefensesPerClass_Allies, 0,
         sizeof(m_iStatsDefensesPerClass_Allies));
  memset(m_iStatsDefensesPerClass_Axis, 0,
         sizeof(m_iStatsDefensesPerClass_Axis));
  ...
  memset(&m_flSecondsPlayedPerClass_Allies, 0,
         sizeof(m_flSecondsPlayedPerClass_Allies));
  memset(&m_flSecondsPlayedPerClass_Axis, 0,
         sizeof(m_flSecondsPlayedPerClass_Axis));
...
void CDODGameRules::UploadLevelStats(void) {
  if (Q_strlen(STRING(gpGlobals->mapname)) > 0) {
    ...
    Q_strncpy(stats.header.szMapName, STRING(gpGlobals->mapname),
              sizeof(stats.header.szMapName));
    ...
    stats.iMinutesPlayed =
        std::clamp((short)(gpGlobals->curtime / 60), 0, MY_USHRT_MAX);
    ...
    for (i = 1; i <= MAX_PLAYERS; i++) {
      ...
      if (pPlayer) {
        ...
        // players who disconnected had their connect time added in
        // ClientDisconnected

        // Tally the latest time for this player
        pPlayer->TallyLatestTimePlayedPerClass(
            pPlayer->GetTeamNumber(), pPlayer->m_Shared.DesiredPlayerClass());

        for (j = 0; j < 7; j++) {
          m_flSecondsPlayedPerClass_Allies[j] +=
              pPlayer->m_flTimePlayedPerClass_Allies[j];
          m_flSecondsPlayedPerClass_Axis[j] +=
              pPlayer->m_flTimePlayedPerClass_Axis[j];
        }
      }
    }
    ...
    // convert to minutes
    for (j = 0; j < 7; j++) {
      stats.iMinutesPlayedPerClass_Allies[j] = std::clamp(
          (short)(m_flSecondsPlayedPerClass_Allies[j] / 60), 0, MY_USHRT_MAX);
      stats.iMinutesPlayedPerClass_Axis[j] = std::clamp(
          (short)(m_flSecondsPlayedPerClass_Axis[j] / 60), 0, MY_USHRT_MAX);
    }

    for (i = 0; i < 6; i++) {
      stats.iKillsPerClass_Allies[i] =
          std::clamp((short)m_iStatsKillsPerClass_Allies[i], 0, MY_USHRT_MAX);
      stats.iKillsPerClass_Axis[i] =
          std::clamp((short)m_iStatsKillsPerClass_Axis[i], 0, MY_USHRT_MAX);

      stats.iSpawnsPerClass_Allies[i] =
          std::clamp((short)m_iStatsSpawnsPerClass_Allies[i], 0, MY_USHRT_MAX);
      stats.iSpawnsPerClass_Axis[i] =
          std::clamp((short)m_iStatsSpawnsPerClass_Axis[i], 0, MY_USHRT_MAX);

      stats.iCapsPerClass_Allies[i] =
          std::clamp((short)m_iStatsCapsPerClass_Allies[i], 0, MY_USHRT_MAX);
      stats.iCapsPerClass_Axis[i] =
          std::clamp((short)m_iStatsCapsPerClass_Axis[i], 0, MY_USHRT_MAX);

      stats.iDefensesPerClass_Allies[i] =
          std::clamp(m_iStatsDefensesPerClass_Allies[i], 0, MY_UCHAR_MAX);
      stats.iDefensesPerClass_Axis[i] =
          std::clamp(m_iStatsDefensesPerClass_Axis[i], 0, MY_UCHAR_MAX);
    }

    // Server Settings
    stats.iClassLimits_Allies[0] =
        std::clamp(mp_limitAlliesRifleman.GetInt(), -1, 254);
    stats.iClassLimits_Allies[1] =
        std::clamp(mp_limitAlliesAssault.GetInt(), -1, 254);
    stats.iClassLimits_Allies[2] =
        std::clamp(mp_limitAlliesSupport.GetInt(), -1, 254);
    stats.iClassLimits_Allies[3] =
        std::clamp(mp_limitAlliesSniper.GetInt(), -1, 254);
    stats.iClassLimits_Allies[4] =
        std::clamp(mp_limitAlliesMachinegun.GetInt(), -1, 254);
    stats.iClassLimits_Allies[5] =
        std::clamp(mp_limitAlliesRocket.GetInt(), -1, 254);

    stats.iClassLimits_Axis[0] =
        std::clamp(mp_limitAxisRifleman.GetInt(), -1, 254);
    stats.iClassLimits_Axis[1] =
        std::clamp(mp_limitAxisAssault.GetInt(), -1, 254);
    stats.iClassLimits_Axis[2] =
        std::clamp(mp_limitAxisSupport.GetInt(), -1, 254);
    stats.iClassLimits_Axis[3] =
        std::clamp(mp_limitAxisSniper.GetInt(), -1, 254);
    stats.iClassLimits_Axis[4] =
        std::clamp(mp_limitAxisMachinegun.GetInt(), -1, 254);
    stats.iClassLimits_Axis[5] =
        std::clamp(mp_limitAxisRocket.GetInt(), -1, 254);
    ...
    for (i = 0; i < DOD_NUM_DISTANCE_STAT_WEAPONS; i++) {
      ...
      stats.weaponStatsDistance[i].iNumHits =
          std::clamp(m_iWeaponShotsHit[weaponId], 0, MY_USHRT_MAX);
      stats.weaponStatsDistance[i].iNumAttacks =
          std::clamp(m_iWeaponShotsFired[weaponId], 0, MY_USHRT_MAX);
      for (int j = 0; j < DOD_NUM_WEAPON_DISTANCE_BUCKETS; j++) {
        stats.weaponStatsDistance[i].iDistanceBuckets[j] =
            std::clamp(m_iWeaponDistanceBuckets[weaponId][j], 0, MY_USHRT_MAX);
      }
    }
    ...
    for (i = 0; i < DOD_NUM_NODIST_STAT_WEAPONS; i++) {
      ...
      stats.weaponStats[i].iNumHits =
          std::clamp(m_iWeaponShotsHit[weaponId], 0, MY_USHRT_MAX);
      stats.weaponStats[i].iNumAttacks =
          std::clamp(m_iWeaponShotsFired[weaponId], 0, MY_USHRT_MAX);
    }
    ...
    if (gamestatsuploader) {
      gamestatsuploader->UploadGameStats(STRING(gpGlobals->mapname),
                                         DOD_STATS_BLOB_VERSION, uBlobSize,
                                         pvBlobData);
    }
  }
}
  ...
  if (FStrEq(pcmd, "teamwin")) {
...
#endif
      // Handle some player commands here as they relate more directly to
      // gamerules state
    ...
    if (pDODPlayer->m_flNextTimeCheck < gpGlobals->curtime) {
      ...
      if (nextlevel.GetString() && *nextlevel.GetString() &&
          engine->IsMapValid(nextlevel.GetString())) {
    ...
    if (pDODPlayer->m_flNextTimeCheck < gpGlobals->curtime) {
      if (mp_timelimit.GetInt() > 0) {
        ...
        ClientPrint(pPlayer, HUD_PRINTTALK, "#game_time_left1", szMinutes,
                    szSeconds);
      } else {
...
void CDODGameRules::CheckChatForReadySignal(CDODPlayer *pPlayer,
                                            const char *chatmsg) {
  if (m_bAwaitingReadyRestart &&
      FStrEq(chatmsg, mp_clan_ready_signal.GetString())) {
...
// return a multiplier that should adjust the damage done by a blast at position
// vecSrc to something at the position vecEnd.  This will take into account the
// density of an entity that blocks the line of sight from one position to the
// other.
//
// this algorithm was taken from the HL2 version of RadiusDamage.
float CDODGameRules::GetExplosionDamageAdjustment(
    Vector &vecSrc, Vector &vecEnd, CBaseEntity *pTarget,
    CBaseEntity *pEntityToIgnore) {
  float retval = 0.0;
  trace_t tr;

  UTIL_TraceLine(vecSrc, vecEnd, MASK_SHOT, pEntityToIgnore,
                 COLLISION_GROUP_NONE, &tr);

  Assert(pTarget);

  // its a hit if we made it to the dest, or if we hit another part of the
  // target on the way
  if (tr.fraction == 1.0 || tr.m_pEnt == pTarget) {
    retval = 1.0;
  } else if (!(tr.DidHitWorld()) && (tr.m_pEnt != NULL) &&
             (tr.m_pEnt->GetOwnerEntity() != pEntityToIgnore)) {
    // if we didn't hit world geometry perhaps there's still damage to be done
    // here.

    CBaseEntity *blockingEntity = tr.m_pEnt;

    // check to see if this part of the player is visible if entities are
    // ignored.
    UTIL_TraceLine(vecSrc, vecEnd, CONTENTS_SOLID, NULL, COLLISION_GROUP_NONE,
                   &tr);

    if (tr.fraction == 1.0) {
      if ((blockingEntity != NULL) &&
          (blockingEntity->VPhysicsGetObject() != NULL)) {
        int nMaterialIndex =
            blockingEntity->VPhysicsGetObject()->GetMaterialIndex();
        ...
        retval = 0.75;  // we're blocked by something that isn't an entity with
                        // a physics module or world geometry, just cut damage
                        // in half for now.
      }
    }
  }
...
// returns the percentage of the player that is visible from the given point in
// the world. return value is between 0 and 1.
float CDODGameRules::GetAmountOfEntityVisible(Vector &vecSrc,
                                              CBaseEntity *entity,
  ...
  */
  ...
  float damageAdjustment =
      GetExplosionDamageAdjustment(vecSrc, vecRHand, player, pIgnoreEntity);
  retval += (damagePercentageHand * damageAdjustment);

  /*
                  Msg( "right hand: %.1f\n", damageAdjustment );
                  NDebugOverlay::Line( vecSrc, vecRHand,
                                                  (int)(damageAdjustment *
     255.0), (int)((1.0 - damageAdjustment) * 255.0), 0, true, 10 );*/

  // left hand
  damageAdjustment =
      GetExplosionDamageAdjustment(vecSrc, vecLHand, player, pIgnoreEntity);
  retval += (damagePercentageHand * damageAdjustment);
  ...
  // head
  damageAdjustment =
      GetExplosionDamageAdjustment(vecSrc, vecHead, player, pIgnoreEntity);
  ...
  // chest
  damageAdjustment =
      GetExplosionDamageAdjustment(vecSrc, vecChest, player, pIgnoreEntity);
  ...
  // right foot
  damageAdjustment =
      GetExplosionDamageAdjustment(vecSrc, vecRFoot, player, pIgnoreEntity);
...
void CDODGameRules::RadiusDamage(const CTakeDamageInfo &info,
  ...
  for (CEntitySphereQuery sphere(vecSrc, flRadius);
    ...
    if (pEntity->m_takedamage != DAMAGE_NO) {
      ...
      if (bIgnoreWorld) {
        ...
        // explosion position.
        flDamagePercentage =
            GetAmountOfEntityVisible(vecSrc, pEntity, info.GetInflictor());
      }
...
void CDODGameRules::RadiusStun(const CTakeDamageInfo &info,
  ...
  // Iterate through all players that made it into playerbits, that are inside
  // the radius and give them stun damage
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (playerbits.Get(i) == false) continue;

    pEntity = UTIL_EntityByIndex(i + 1);

    if (!pEntity || !pEntity->IsPlayer()) continue;
...
void CDODGameRules::CheckWarmup(void) {
  ...
  if (m_bInWarmup) {
    ...
    if (gpGlobals->curtime > m_flWarmupTimeEnds && m_flRestartRoundTime < 0 &&
...
void CDODGameRules::CheckRestartRound(void) {
  ...
  if (iRestartDelay > 0) {
    if (iRestartDelay > 60) iRestartDelay = 60;

    m_flRestartRoundTime = gpGlobals->curtime + iRestartDelay;

    IGameEvent *event =
        gameeventmanager->CreateEvent("dod_round_restart_seconds");
    if (event) {
...
void CDODGameRules::CheckPlayerPositions() {
  ...
  for (i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    engine->Message_DetermineMulticastRecipients(false, pPlayer->EyePosition(),
                                                 playerbits);
    ...
    for (int i = 0; i < gpGlobals->maxClients; i++) {
      ...
      if (!bUpdatePlayer[i]) continue;

      CBasePlayer *pOtherPlayer = UTIL_PlayerByIndex(i + 1);

      if (!pOtherPlayer) continue;  // nothing there

      if (pOtherPlayer == pPlayer) continue;  // dont update himself

      if (!pOtherPlayer->IsAlive() || pOtherPlayer->IsObserver() ||
          !pOtherPlayer->IsConnected())
        continue;  // don't update spectators or dead players

      if (pPlayer->GetTeamNumber() > TEAM_SPECTATOR) {
        // update only team mates if not a pure spectator
        if (pPlayer->GetTeamNumber() != pOtherPlayer->GetTeamNumber()) continue;
      }
...
Vector DropToGround(CBaseEntity *pMainEnt, const Vector &vPos,
                    const Vector &vMins, const Vector &vMaxs) {
  trace_t trace;
  UTIL_TraceHull(vPos, vPos + Vector(0, 0, -500), vMins, vMaxs, MASK_SOLID,
                 pMainEnt, COLLISION_GROUP_NONE, &trace);
  return trace.endpos;
}
...
void TestSpawnPointType(const char *pEntClassName) {
  ...
  while (pSpot) {
    ...
    if (g_pGameRules->IsSpawnPointValid(pSpot, NULL)) {
      ...
      NDebugOverlay::Box(pSpot->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX, 0,
                         255, 0, 100, 60);

      // drop down to ground
      Vector GroundPos =
          DropToGround(NULL, pSpot->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX);

      // the location the player will spawn at
      NDebugOverlay::Box(GroundPos, VEC_HULL_MIN, VEC_HULL_MAX, 0, 0, 255, 100,
                         60);
      ...
      NDebugOverlay::HorzArrow(pSpot->GetAbsOrigin(),
                               pSpot->GetAbsOrigin() + vecForward * 32, 10, 255,
                               0, 0, 255, true, 60);
    } else {
      // failed spawn point location
      NDebugOverlay::Box(pSpot->GetAbsOrigin(), VEC_HULL_MIN, VEC_HULL_MAX, 255,
                         0, 0, 100, 60);
    }
...
ConCommand cc_TestSpawns("map_showspawnpoints", TestSpawns,
...
CBaseEntity *CDODGameRules::GetPlayerSpawnSpot(CBasePlayer *pPlayer) {
  ...
  // drop down to ground
  Vector GroundPos = DropToGround(pPlayer, pSpawnSpot->GetAbsOrigin(),
...
bool CDODGameRules::IsSpawnPointValid(CBaseEntity *pSpot,
                                      CBasePlayer *pPlayer) {
...
void CDODGameRules::PlayerSpawn(CBasePlayer *p) {
  ...
  if (team == TEAM_ALLIES || team == TEAM_AXIS) {
    if (pPlayer->m_Shared.DesiredPlayerClass() == PLAYERCLASS_RANDOM) {
      ...
      ClientPrint(pPlayer, HUD_PRINTTALK, "#game_now_as",
                  GetPlayerClassName(pPlayer->m_Shared.PlayerClass(), team));
    } else {
    ...
    if (playerclass != PLAYERCLASS_UNDEFINED) {
      // Assert( PLAYERCLASS_UNDEFINED < playerclass && playerclass <
      // NUM_PLAYERCLASSES );

      int i;

      CDODTeam *pTeam = GetGlobalDODTeam(team);
      const CDODPlayerClassInfo &pClassInfo =
          pTeam->GetPlayerClassInfo(playerclass);
      ...
      // Primary weapon
      Q_snprintf(buf, bufsize, "weapon_%s",
                 WeaponIDToAlias(pClassInfo.m_iPrimaryWeapon));
      ...
      if (pClassInfo.m_iSecondaryWeapon != WEAPON_NONE) {
        Q_snprintf(buf, bufsize, "weapon_%s",
                   WeaponIDToAlias(pClassInfo.m_iSecondaryWeapon));
      ...
      if (pClassInfo.m_iMeleeWeapon) {
        Q_snprintf(buf, bufsize, "weapon_%s",
                   WeaponIDToAlias(pClassInfo.m_iMeleeWeapon));
      ...
      if (pWpn) {
        int iNumClip = pWpn->GetDODWpnData().m_iDefaultAmmoClips -
                       1;  // account for one clip in the gun
      ...
      if (pSecondaryWpn) {
        ...
        if (pWpn) {
          int iNumClip = pWpn->GetDODWpnData().m_iDefaultAmmoClips -
                         1;  // account for one clip in the gun
          int iClipSize = pWpn->GetDODWpnData().iMaxClip1;
          pPlayer->GiveAmmo(iNumClip * iClipSize,
                            pWpn->GetDODWpnData().szAmmo1);
        }
      }
      ...
      if (pClassInfo.m_iGrenType1 != WEAPON_NONE) {
        Q_snprintf(buf, bufsize, "weapon_%s",
                   WeaponIDToAlias(pClassInfo.m_iGrenType1));
      ...
      if (pClassInfo.m_iGrenType2 != WEAPON_NONE) {
        Q_snprintf(buf, bufsize, "weapon_%s",
                   WeaponIDToAlias(pClassInfo.m_iGrenType2));
...
void CDODGameRules::ChooseRandomClass(CDODPlayer *pPlayer) {
  ...
  if (numChoices == 0) {
    Msg("Random class found that all classes were full - ignoring class limits "
        "for this spawn\n");

    pPlayer->m_Shared.SetPlayerClass(
        random->RandomFloat(firstclass, lastclass));
  } else {
...
//-----------------------------------------------------------------------------
// Purpose: This function can be used to find a valid placement location for an
// entity.
//			Given an origin to start looking from and a minimum radius
//to place the entity at,
//			it will sweep out a circle around vOrigin and try to find a
//valid spot (on the ground) 			where mins and maxs will fit.
...
//-----------------------------------------------------------------------------
bool EntityPlacementTest(CBaseEntity *pMainEnt, const Vector &vOrigin,
                         Vector &outPos, bool bDropToGround) {
  // This function moves the box out in each dimension in each step trying to
  // find empty space like this:
  //
  //											  X
  //							   X X
    ...
    for (int iDim = 0; iDim < 2; iDim++) {
      ...
      for (int iSign = 0; iSign < 2; iSign++) {
        ...
        if (UTIL_IsSpaceEmpty(pMainEnt, vBase + vTestMins, vBase + vTestMaxs)) {
          // Ensure that there is a clear line of sight from the spawnpoint
          // entity to the actual spawn point. (Useful for keeping things from
          // spawning behind walls near a spawn point)
          trace_t tr;
          UTIL_TraceLine(vOrigin, vBase, MASK_SOLID, pMainEnt,
                         COLLISION_GROUP_NONE, &tr);
...
bool CDODGameRules::CanHavePlayerItem(CBasePlayer *pPlayer,
  ...
  if (pWpn) {
    ...
    switch (type) {
      case WPN_TYPE_MELEE: {
#ifdef DEBUG
        CWeaponDODBase *pMeleeWeapon =
            (CWeaponDODBase *)pPlayer->Weapon_GetSlot(WPN_SLOT_MELEE);
...
#ifdef DEBUG
        CWeaponDODBase *pSecondaryWeapon =
            (CWeaponDODBase *)pPlayer->Weapon_GetSlot(WPN_SLOT_SECONDARY);
      ...
      case WPN_TYPE_BAZOOKA: {
        ...
        CWeaponDODBase *pPrimaryWeapon =
            (CWeaponDODBase *)pPlayer->Weapon_GetSlot(WPN_SLOT_PRIMARY);
        bool bHasPrimary = (pPrimaryWeapon != NULL);

        if (bHasPrimary) return false;
      } break;
...
int CDODGameRules::GetSubTeam(int team) { return SUBTEAM_NORMAL; }
...
int CDODGameRules::GetTimeLeft(void) {
  ...
  Assert(flTimeLimit > 0 &&
         "Should not call this function when !IsGameUnderTimeLimit");
  ...
  if (m_bUsingTimer && m_pRoundTimer) {
    ...
    // If the map time for any reason goes beyond the end of the round, remove
    // the flag
    if (flMapChangeSeconds > flTimerSeconds) {
      ...
      // or else the round will end when a team captures an objective and adds
      // time to above 120
...
int CDODGameRules::GetReinforcementTimerSeconds(int team,
...
bool CDODGameRules::IsInBonusRound(void) {
  return (dod_bonusround.GetBool() == true &&
          (State_Get() == STATE_ALLIES_WIN || State_Get() == STATE_AXIS_WIN));
}

ConVar dod_showroundtransitions("dod_showroundtransitions", "0", 0,
...
CDODRoundStateInfo *CDODGameRules::State_LookupInfo(DODRoundState state) {
  static CDODRoundStateInfo playerStateInfos[] = {
      {STATE_INIT, "STATE_INIT", &CDODGameRules::State_Enter_INIT, NULL,
       &CDODGameRules::State_Think_INIT},
      {STATE_PREGAME, "STATE_PREGAME", &CDODGameRules::State_Enter_PREGAME,
       NULL, &CDODGameRules::State_Think_PREGAME},
      {STATE_STARTGAME, "STATE_STARTGAME",
       &CDODGameRules::State_Enter_STARTGAME, NULL,
       &CDODGameRules::State_Think_STARTGAME},
      {STATE_PREROUND, "STATE_PREROUND", &CDODGameRules::State_Enter_PREROUND,
       NULL, &CDODGameRules::State_Think_PREROUND},
      {STATE_RND_RUNNING, "STATE_RND_RUNNING",
       &CDODGameRules::State_Enter_RND_RUNNING, NULL,
       &CDODGameRules::State_Think_RND_RUNNING},
      {STATE_ALLIES_WIN, "STATE_ALLIES_WIN",
       &CDODGameRules::State_Enter_ALLIES_WIN, NULL,
       &CDODGameRules::State_Think_ALLIES_WIN},
      {STATE_AXIS_WIN, "STATE_AXIS_WIN", &CDODGameRules::State_Enter_AXIS_WIN,
       NULL, &CDODGameRules::State_Think_AXIS_WIN},
      {STATE_RESTART, "STATE_RESTART", &CDODGameRules::State_Enter_RESTART,
       NULL, &CDODGameRules::State_Think_RESTART},
...
void CDODGameRules::State_Think_INIT(void) { State_Transition(STATE_PREGAME); }
...
void CDODGameRules::State_Think_PREGAME(void) {
  ...
  if (CountActivePlayers() > 0) State_Transition(STATE_STARTGAME);
}
...
void CDODGameRules::State_Enter_STARTGAME(void) {
  m_flStateTransitionTime =
      gpGlobals->curtime + 5 * dod_enableroundwaittime.GetFloat();
...
void CDODGameRules::State_Enter_PREROUND(void) {
  ...
  if (m_bInitialSpawn) {
    m_flStateTransitionTime =
        gpGlobals->curtime + 10 * dod_enableroundwaittime.GetFloat();
  ...
  // Game rules may change, if a new one becomes mastered at the end of the last
  // round
  DetectGameRules();
  ...
  // reset this now! If its reset at round restart, we lose all the players that
  // died during the preround
  m_iAlliesRespawnHead = 0;
  m_iAlliesRespawnTail = 0;
  m_iAxisRespawnHead = 0;
  m_iAxisRespawnTail = 0;
  ...
  CBaseEntity *pEnt =
      gEntList.FindEntityByClassname(NULL, "dod_control_point_master");

  if (!pEnt) {
    Warning(
        "No dod_control_point_master found in level - control points will not "
        "work as expected.\n");
  }
    ...
    if (pMaster && pMaster->IsActive()) {
      if (pMaster->IsUsingRoundTimer()) {
        ...
        if (!m_pRoundTimer.Get()) {
          m_pRoundTimer =
              (CDODRoundTimer *)CreateEntityByName("dod_round_timer");
        }
...
void CDODGameRules::State_Enter_RND_RUNNING(void) {
  ...
  if (!IsInWarmup()) PlayStartRoundVoice();
...
void CDODGameRules::State_Think_RND_RUNNING(void) {
  ...
  if (m_bUsingTimer && m_pRoundTimer) {
    ...
    if (flSecondsRemaining <= 0) {
      ...
      // find all the control points, init the timer
      CBaseEntity *pEnt =
          gEntList.FindEntityByClassname(NULL, "dod_bomb_target");
      ...
      while (pEnt) {
        ...
        if (pBomb && pBomb->State_Get() == BOMB_TARGET_ARMED) {
          ...
          if (pPoint && pPoint->GetBombsRemaining() <= 1) {
            // find active dod_control_point_masters, ask them if this flag
            // capping would end the game
            CBaseEntity *pEnt = gEntList.FindEntityByClassname(
                NULL, "dod_control_point_master");

            while (pEnt) {
              CControlPointMaster *pMaster =
                  dynamic_cast<CControlPointMaster *>(pEnt);

              if (pMaster->IsActive()) {
                // Check TeamOwnsAllPoints, while overriding this particular
                // flag's owner
              ...
              pEnt = gEntList.FindEntityByClassname(pEnt,
      ...
      if (bBombBlocksWin == false) {
        ...
        // tell the dod_control_point_master to fire its outputs for the winning
        // team! minor hackage - dod_gamerules should be responsible for team
        // win events, not dod_cpm
        ...
        while (pEnt) {
          ...
          pEnt =
              gEntList.FindEntityByClassname(pEnt, "dod_control_point_master");
        }
      }
    } else if (flSecondsRemaining < 60.0 &&
               m_bPlayTimerWarning_1Minute == true) {
    ...
    } else if (flSecondsRemaining < 120.0 &&
               m_bPlayTimerWarning_2Minute == true) {
...
void CDODGameRules::CheckRespawnWaves(void) {
  ...
  if (m_iNumAlliesRespawnWaves > 0 &&
      m_AlliesRespawnQueue[m_iAlliesRespawnHead] < gpGlobals->curtime) {
  ...
  if (m_iNumAxisRespawnWaves > 0 &&
      m_AxisRespawnQueue[m_iAxisRespawnHead] < gpGlobals->curtime) {
...
void CDODGameRules::State_Enter_ALLIES_WIN(void) {
  ...
  m_flStateTransitionTime =
      gpGlobals->curtime + flTime * dod_enableroundwaittime.GetFloat();
...
void CDODGameRules::State_Think_RESTART(void) {
  Assert(0);  // should never get here, State_Enter_RESTART sets us into a
              // different state
}
...
void CDODGameRules::ResetScores(void) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pDODPlayer == NULL) continue;

    if (FNullEnt(pDODPlayer->edict())) continue;

    pDODPlayer->ResetScores();
  }
}

ConVar dod_showcleanedupents("dod_showcleanedupents", "0", 0,
   ...
   public:
    virtual bool ShouldCreateEntity(const char *pClassname) {
      ...
      if (!FindInList(s_PreserveEnts, pClassname)) {
        ...
        // Increment our iterator since it's not going to call CreateNextEntity
        // for this ent.
    ...
    virtual CBaseEntity *CreateNextEntity(const char *pClassname) {
      if (m_iIterator == g_MapEntityRefs.InvalidIndex()) {
        // This shouldn't be possible. When we loaded the map, it should have
        // used CDODMapLoadEntityFilter, which should have built the
        // g_MapEntityRefs list with the same list of entities we're referring
        // to here.
        Assert(false);
        return NULL;
      } else {
        ...
        m_iIterator =
            g_MapEntityRefs.Next(m_iIterator);  // Seek to the next entity.

        if (ref.m_iEdict == -1 || engine->PEntityOfEntIndex(ref.m_iEdict)) {
          // Doh! The entity was delete and its slot was reused.
          // Just use any old edict slot. This case sucks because we lose the
          // baseline.
          return CreateEntityByName(pClassname);
        } else {
          // Cool, the slot where this entity was is free again (most likely,
          // the entity was freed above). Now create an entity with this
          // specific index.
...
int PlayerScoreInfoSort(const playerscore_t *p1, const playerscore_t *p2) {
  // check frags
  if (p1->iScore > p2->iScore) return -1;
  if (p2->iScore > p1->iScore) return 1;
...
void FillEventCategory(IGameEvent *event, int side, int category,
                       CUtlVector<playerscore_t> &pList) {
  switch (side) {
    case 0:
  ...
  static const char *pCategoryNames[2][6] = {
      {"left_1", "left_score_1", "left_2", "left_score_2", "left_3",
       "left_score_3"},
      {"right_1", "right_score_1", "right_2", "right_score_2", "right_3",
       "right_score_3"}};
...
void CDODGameRules::SetWinningTeam(int team) {
  ...
  if (FStrEq(STRING(gpGlobals->mapname), "dod_colmar")) {
    CControlPointMaster *pMaster = dynamic_cast<CControlPointMaster *>(
        gEntList.FindEntityByClassname(NULL, "dod_control_point_master"));

    if (pMaster) {
      // 1. losing team must not own any control points
      // 2. for each point that the winning team owns, that takes bombs, it must
      // still have 2 bombs required
      ...
      for (int i = 0; i < iNumCP; i++) {
        ...
        if (!pPoint || !pPoint->PointIsVisible()) continue;
        ...
        // 1 bomb remaining means we own it, but the other team blew it up a
        // bit.
        else if (pPoint->GetBombsRequired() > 0 &&
                 pPoint->GetBombsRemaining() == 1) {
          bFlawlessVictory = false;
        }
      }

      if (bFlawlessVictory) {
        GetGlobalDODTeam(team)->AwardAchievement(
  ...
  if (winEvent) {
    ...
    if (m_bUsingTimer) {
      if (team == m_iTimerWinTeam) {
        ...
        int iTimeElapsed = m_pRoundTimer->GetTimerMaxLength() -
                           (int)m_pRoundTimer->GetTimeRemaining();
    ...
    int iLastEvent =
        (team == TEAM_ALLIES) ? m_iLastAlliesCapEvent : m_iLastAxisCapEvent;
    ...
    for (i = 0; i < iNumPlayers; i++) {
      CDODPlayer *pPlayer =
          dynamic_cast<CDODPlayer *>(pWinningTeam->GetPlayer(i));
  ...
  if (winEvent) {
    if (1) {
      if (0 /*team == m_iTimerWinTeam */) {
        ...
        // show total time that was defended
        winEvent->SetBool("show_timer_defend", true);
        winEvent->SetInt("timer_time",
                         0 /*m_pRoundTimer->GetTimerMaxLength() */);
      } else {
        // attackers win
        // show time it took for them to win
        winEvent->SetBool("show_timer_attack", true);

        int iTimeElapsed = 90;  // m_pRoundTimer->GetTimerMaxLength() -
                                // (int)m_pRoundTimer->GetTimeRemaining();
        winEvent->SetInt("timer_time", iTimeElapsed);
      }
    } else {
...
ConCommand dod_test_winpanel("dod_test_winpanel", TestWinpanel, "",
                             FCVAR_CHEAT);

// bForceRespawn - respawn player even if dead or dying
// bTeam - if true, only respawn the passed team
// iTeam  - team to respawn
void CDODGameRules::RespawnPlayers(bool bForceRespawn, bool bTeam /* = false */,
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (!bForceRespawn) {
      ...
      if (State_Get() != STATE_PREROUND &&
          pPlayer->State_Get() == STATE_DEATH_ANIM)
        continue;
    }
...
bool CDODGameRules::IsPlayerClassOnTeam(int cls, int team) {
  if (cls == PLAYERCLASS_RANDOM) return true;
...
bool CDODGameRules::CanPlayerJoinClass(CDODPlayer *pPlayer, int cls) {
  ...
  if (ReachedClassLimit(pPlayer->GetTeamNumber(), cls)) return false;
...
int CDODGameRules::CountPlayerClass(int team, int cls) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (pDODPlayer->GetTeamNumber() != team) continue;

    if (pDODPlayer->m_Shared.DesiredPlayerClass() == cls) num++;
  }
...
void CDODGameRules::CheckLevelInitialized() {
  if (!m_bLevelInitialized) {
    ...
    while ((ent = gEntList.FindEntityByClassname(ent, "info_player_allies")) !=
           NULL) {
      if (IsSpawnPointValid(ent, NULL)) {
        m_iSpawnPointCount_Allies++;
      } else {
        Warning("Invalid allies spawnpoint at (%.1f,%.1f,%.1f)\n",
                ent->GetAbsOrigin()[0], ent->GetAbsOrigin()[2],
                ent->GetAbsOrigin()[2]);
      }
    }

    while ((ent = gEntList.FindEntityByClassname(ent, "info_player_axis")) !=
           NULL) {
      if (IsSpawnPointValid(ent, NULL)) {
...
void CDODGameRules::CreateStandardEntities() {
  ...
  g_pPlayerResource = (CPlayerResource *)CBaseEntity::Create(
      "dod_player_manager", vec3_origin, vec3_angle);

  // Create the objective resource
  g_pObjectiveResource = (CDODObjectiveResource *)CBaseEntity::Create(
      "dod_objective_resource", vec3_origin, vec3_angle);
...
ConVar dod_waverespawnfactor("dod_waverespawnfactor", "1.0",
                             FCVAR_REPLICATED | FCVAR_CHEAT,
                             "Factor for respawn wave timers");
...
float CDODGameRules::GetWaveTime(int iTeam) {
  ...
  switch (iTeam) {
    case TEAM_ALLIES:
      flRespawnTime = (m_iNumAlliesRespawnWaves > 0)
                          ? m_AlliesRespawnQueue[m_iAlliesRespawnHead]
                          : -1;
      break;
    case TEAM_AXIS:
      flRespawnTime = (m_iNumAxisRespawnWaves > 0)
                          ? m_AxisRespawnQueue[m_iAxisRespawnHead]
                          : -1;
      break;
    default:
...
float CDODGameRules::GetMaxWaveTime(int nTeam) {
  ...
  // Minimum 5 seconds
  if (fTime <= DEATH_CAM_TIME) fTime = DEATH_CAM_TIME;
...
bool CDODGameRules::InRoundRestart(void) {
  if (State_Get() == STATE_PREROUND) return true;

  return false;
}

void CDODGameRules::PlayerKilled(CBasePlayer *pVictim,
  ...
  if (pVictim == pScorer) {
    ...
    if (bScoring) pVictim->IncrementFragCount(-1);
  } else if (pScorer) {
    // if a player dies in a deathmatch game and the killer is a client, award
    // the killer some points
    if (bScoring) pScorer->IncrementFragCount(DODPointsForKill(pVictim, info));
    ...
    // dvsents2: uncomment when removing all FireTargets
    // variant_t value;
    // g_EventQueue.AddEvent( "game_playerkill", "Use", value, 0, pScorer,
    // pScorer );
    ...
    if (pDODVictim->m_signals.GetState() & SIGNAL_CAPTUREAREA) {
      ...
      CAreaCapture *pArea = dynamic_cast<CAreaCapture *>(
          gEntList.FindEntityByClassname(NULL, "dod_capture_area"));
      while (pArea) {
        if (pArea->CheckIfDeathCausesBlock(pDODVictim, pScorer)) {
          break;
        }

        pArea = dynamic_cast<CAreaCapture *>(
            gEntList.FindEntityByClassname(pArea, "dod_capture_area"));
      }
    }
    if (pDODVictim->m_bIsDefusing && pDODVictim->m_pDefuseTarget &&
        pScorer->GetTeamNumber() != pDODVictim->GetTeamNumber()) {
...
void CDODGameRules::PlaySpawnSoundToTeam(const char *sound, int team) {
  ...
  while (!bBreakLoop) {
    iCurrent++;
    if (iCurrent > gpGlobals->maxClients) iCurrent = 1;
...
void CDODGameRules::DeathNotice(CBasePlayer *pVictim,
                                const CTakeDamageInfo &info) {
  ...
  CBaseEntity *pKiller = info.GetAttacker();
  CDODPlayer *pScorer =
      dynamic_cast<CDODPlayer *>(GetDeathScorer(pKiller, pInflictor));
    ...
    if (pInflictor) {
      if (pInflictor == pScorer) {
        ...
        if (pWeapon) {
          ...
          if (pWeapon->GetDODWpnData().m_iDefaultTeam !=
                  pScorer->GetTeamNumber() &&
              pScorer->GetTeamNumber() != pVictim->GetTeamNumber()) {
            pScorer->HandleEnemyWeaponsAchievement(1);
          }

          // achievement for getting kills with several different weapon types
          // in one life
        ...
        killer_weapon_name =
            STRING(pInflictor->m_iClassname);  // it's just that easy
      }
    }
  } else {
  ...
  if (strncmp(killer_weapon_name, "weapon_", 7) == 0) {
    ...
    // achievement for getting kills with several different weapon types in one
    // life
    if (pScorer->GetTeamNumber() != pVictim->GetTeamNumber()) {
...
CDODDetect::CDODDetect() { m_GamePlayRules.Reset(); }
...
bool CDODGameRules::TeamStacked(int iNewTeam, int iCurTeam) {
  ...
  if (iNewTeam == iCurTeam) return false;
...
#define DOD_DAMAGE_FOR_FALL_SPEED \
  ((float)100 /                   \
   (DOD_PLAYER_FATAL_FALL_SPEED - \
    DOD_PLAYER_MAX_SAFE_FALL_SPEED))  // damage per unit per second.

/*
#define PLAYER_FALL_PUNCH_THRESHHOLD (float)350 // won't punch player's
screen/make scrape noise unless player falling at least this fast.
*/
...
CAmmoDef *GetAmmoDef() {
  ...
  if (!bInitted) {
    ...
    // pistol ammo
    def.AddAmmoType(DOD_AMMO_COLT, DMG_BULLET, TRACER_NONE, 0, 0, 21, 5000, 10,
                    14);
    def.AddAmmoType(DOD_AMMO_P38, DMG_BULLET, TRACER_NONE, 0, 0, 24, 5000, 10,
                    14);
    def.AddAmmoType(DOD_AMMO_C96, DMG_BULLET, TRACER_NONE, 0, 0, 60, 5000, 10,
                    14);

    // rifles
    def.AddAmmoType(DOD_AMMO_GARAND, DMG_BULLET, TRACER_NONE, 0, 0, 88, 9000,
                    10, 14);
    def.AddAmmoType(DOD_AMMO_K98, DMG_BULLET, TRACER_NONE, 0, 0, 65, 9000, 10,
                    14);
    def.AddAmmoType(DOD_AMMO_M1CARBINE, DMG_BULLET, TRACER_NONE, 0, 0, 165,
                    9000, 10, 14);
    def.AddAmmoType(DOD_AMMO_SPRING, DMG_BULLET, TRACER_NONE, 0, 0, 55, 9000,
                    10, 14);

    // submg
    def.AddAmmoType(DOD_AMMO_SUBMG, DMG_BULLET, TRACER_NONE, 0, 0, 210, 7000,
                    10, 14);
    def.AddAmmoType(DOD_AMMO_BAR, DMG_BULLET, TRACER_LINE_AND_WHIZ, 0, 0, 260,
                    9000, 10, 14);

    // mg
    def.AddAmmoType(DOD_AMMO_30CAL, DMG_BULLET | DMG_MACHINEGUN,
                    TRACER_LINE_AND_WHIZ, 0, 0, 300, 9000, 10, 14);
    def.AddAmmoType(DOD_AMMO_MG42, DMG_BULLET | DMG_MACHINEGUN,
                    TRACER_LINE_AND_WHIZ, 0, 0, 500, 9000, 10, 14);

    // rockets
    def.AddAmmoType(DOD_AMMO_ROCKET, DMG_BLAST, TRACER_NONE, 0, 0, 5, 9000, 10,
                    14);

    // grenades
    def.AddAmmoType(DOD_AMMO_HANDGRENADE, DMG_BLAST, TRACER_NONE, 0, 0, 2, 1, 4,
                    8);
    def.AddAmmoType(DOD_AMMO_STICKGRENADE, DMG_BLAST, TRACER_NONE, 0, 0, 2, 1,
                    4, 8);
    def.AddAmmoType(DOD_AMMO_HANDGRENADE_EX, DMG_BLAST, TRACER_NONE, 0, 0, 1, 1,
                    4, 8);
    def.AddAmmoType(DOD_AMMO_STICKGRENADE_EX, DMG_BLAST, TRACER_NONE, 0, 0, 1,
                    1, 4, 8);

    // smoke grenades
    def.AddAmmoType(DOD_AMMO_SMOKEGRENADE_US, DMG_BLAST, TRACER_NONE, 0, 0, 2,
                    1, 4, 8);
    def.AddAmmoType(DOD_AMMO_SMOKEGRENADE_GER, DMG_BLAST, TRACER_NONE, 0, 0, 2,
                    1, 4, 8);
    def.AddAmmoType(DOD_AMMO_SMOKEGRENADE_US_LIVE, DMG_BLAST, TRACER_NONE, 0, 0,
                    2, 1, 4, 8);
    def.AddAmmoType(DOD_AMMO_SMOKEGRENADE_GER_LIVE, DMG_BLAST, TRACER_NONE, 0,
                    0, 2, 1, 4, 8);

    // rifle grenades
    def.AddAmmoType(DOD_AMMO_RIFLEGRENADE_US, DMG_BLAST, TRACER_NONE, 0, 0, 2,
                    1, 4, 8);
    def.AddAmmoType(DOD_AMMO_RIFLEGRENADE_GER, DMG_BLAST, TRACER_NONE, 0, 0, 2,
                    1, 4, 8);
    def.AddAmmoType(DOD_AMMO_RIFLEGRENADE_US_LIVE, DMG_BLAST, TRACER_NONE, 0, 0,
                    2, 1, 4, 8);
    def.AddAmmoType(DOD_AMMO_RIFLEGRENADE_GER_LIVE, DMG_BLAST, TRACER_NONE, 0,
                    0, 2, 1, 4, 8);
  }
...
void CDODGameRules::AddWaveTime(int team, float flTime) {
  switch (team) {
    case TEAM_ALLIES: {
      ...
      m_AlliesRespawnQueue.Set(m_iAlliesRespawnTail,
                               gpGlobals->curtime + flTime);
      m_iNumAlliesRespawnWaves++;

      m_iAlliesRespawnTail =
          (m_iAlliesRespawnTail + 1) % DOD_RESPAWN_QUEUE_SIZE;

      DevMsg(1, "AddWaveTime ALLIES head %d tail %d numtotal %d time %.1f\n",
             m_iAlliesRespawnHead, m_iAlliesRespawnTail,
             m_iNumAlliesRespawnWaves, gpGlobals->curtime + flTime);
    } break;
    case TEAM_AXIS: {
      ...
      DevMsg(1, "AddWaveTime AXIS head %d tail %d numtotal %d time %.1f\n",
             m_iAxisRespawnHead, m_iAxisRespawnTail, m_iNumAxisRespawnWaves,
             gpGlobals->curtime + flTime);
    } break;
    default:
      Assert(0);
  ...
  switch (team) {
    case TEAM_ALLIES: {
      ...
      m_iAlliesRespawnHead =
          (m_iAlliesRespawnHead + 1) % DOD_RESPAWN_QUEUE_SIZE;
      m_iNumAlliesRespawnWaves--;
...
void CDODGameRules::ClientSettingsChanged(CBasePlayer *pPlayer) {
  ...
  pDODPlayer->SetAutoReload(Q_atoi(engine->GetClientConVarValue(
                                pPlayer->entindex(), "cl_autoreload")) > 0);
  pDODPlayer->SetShowHints(Q_atoi(engine->GetClientConVarValue(
                               pPlayer->entindex(), "cl_showhelp")) > 0);
  pDODPlayer->SetAutoRezoom(Q_atoi(engine->GetClientConVarValue(
                                pPlayer->entindex(), "cl_autorezoom")) > 0);

  BaseClass::ClientSettingsChanged(pPlayer);
}

int CDODGameRules::DODPointsForKill(CBasePlayer *pVictim,
                                    const CTakeDamageInfo &info) {
  if (IsInWarmup()) return 0;
...
//-----------------------------------------------------------------------------
// Purpose: Returns the weapon in the player's inventory that would be better
// than
//			the given weapon.
// Note, this version allows us to switch to a weapon that has no ammo as a last
// resort.
//-----------------------------------------------------------------------------
CBaseCombatWeapon *CDODGameRules::GetNextBestWeapon(
    CBaseCombatCharacter *pPlayer, CBaseCombatWeapon *pCurrentWeapon) {
  CBaseCombatWeapon *pCheck;
  CBaseCombatWeapon *pBest;  // this will be used in the event that we don't
                             // find a weapon in the same category.
  ...
  if (pCurrentWeapon) {
    if (!pCurrentWeapon->AllowsAutoSwitchFrom() ||
        !pCurrentWeapon->CanHolster()) {
  ...
  for (int i = 0; i < pPlayer->WeaponCount(); ++i) {
    ...
    // If we have an active weapon and this weapon doesn't allow autoswitching
    // away from another weapon, skip it.
    if (pCurrentWeapon && !pCheck->AllowsAutoSwitchTo()) continue;
    ...
    } else if (iWeight > iBestWeight &&
               pCheck != pCurrentWeapon)  // don't reselect the weapon we're
                                          // trying to get rid of
    {
      // Msg( "Considering %s\n", STRING( pCheck->GetClassname() );
      // we keep updating the 'best' weapon just in case we can't find a weapon
      // of the same weight that the player was using. This will end up leaving
      // the player with his heaviest-weighted weapon.
  ...
  // if pBest is 0, we didn't find ANYTHING. Shouldn't be possible- should
  // always at least get the crowbar, but ya never know.
  return pBest;
}

char *szHitgroupNames[] = {"generic",  "head",      "chest",    "stomach",
...
void CDODGameRules::WriteStatsFile(const char *pszLogName) {
  ...
  for (i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (pPlayer) {
      ...
      filesystem->FPrintf(hFile, "\t\t<playername>%s</playername>",
      ...
      /*
      pPlayer->TallyLatestTimePlayedPerClass( pPlayer->GetTeamNumber(),
      pPlayer->m_Shared.DesiredPlayerClass() );
      ...
      */
      ...
      filesystem->FPrintf(hFile,
                          "\t\t<bonus_round_kills>%i</bonus_round_kills>\n",
...
bool CDODGameRules::IsBombingTeam(int team) {
  if (team == TEAM_ALLIES) return m_bAlliesAreBombing;

  if (team == TEAM_AXIS) return m_bAxisAreBombing;
...
ConVar showblockerwalls("showblockerwalls", "0", FCVAR_CHEAT,
                        "Set to 1 to visualize blocker walls");
