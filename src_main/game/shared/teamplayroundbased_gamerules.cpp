...
#ifdef CLIENT_DLL
void RecvProxy_TeamplayRoundState(const CRecvProxyData *pData, void *pStruct,
                                  void *pOut) {
...
RecvPropInt(RECVINFO(m_iRoundState), 0, RecvProxy_TeamplayRoundState),
    ...
    RecvPropInt(RECVINFO(m_iWinningTeam)), RecvPropInt(RECVINFO(m_bInOvertime)),
    ...
    RecvPropArray3(RECVINFO_ARRAY(m_flNextRespawnWave),
                   RecvPropTime(RECVINFO(m_flNextRespawnWave[0]))),
    RecvPropArray3(RECVINFO_ARRAY(m_TeamRespawnWaveTimes),
                   RecvPropFloat(RECVINFO(m_TeamRespawnWaveTimes[0]))),
#else
SendPropInt(SENDINFO(m_iRoundState), 5),
    ...
    SendPropBool(SENDINFO(m_bInOvertime)), SendPropBool(SENDINFO(m_bInSetup)),
    ...
    SendPropArray3(SENDINFO_ARRAY3(m_flNextRespawnWave),
                   SendPropTime(SENDINFO_ARRAY(m_flNextRespawnWave))),
    SendPropArray3(SENDINFO_ARRAY3(m_TeamRespawnWaveTimes),
                   SendPropFloat(SENDINFO_ARRAY(m_TeamRespawnWaveTimes))),
#endif
    END_NETWORK_TABLE()

        IMPLEMENT_NETWORKCLASS_ALIASED(TeamplayRoundBasedRulesProxy,
                                       DT_TeamplayRoundBasedRulesProxy)

#ifdef CLIENT_DLL
            void RecvProxy_TeamplayRoundBasedRules(const RecvProp *pProp,
                                                   void **pOut, void *pData,
                                                   int objectID) {
  CTeamplayRoundBasedRules *pRules =
      dynamic_cast<CTeamplayRoundBasedRules *>(GameRules());
  Assert(pRules);
  *pOut = pRules;
}

BEGIN_RECV_TABLE(CTeamplayRoundBasedRulesProxy, DT_TeamplayRoundBasedRulesProxy)
RecvPropDataTable("teamplayroundbased_gamerules_data", 0, 0,
                  &REFERENCE_RECV_TABLE(DT_TeamplayRoundBasedRules),
                  RecvProxy_TeamplayRoundBasedRules)
END_RECV_TABLE
()

    void CTeamplayRoundBasedRulesProxy::OnPreDataChanged(
...
#else
            void *SendProxy_TeamplayRoundBasedRules(
                const SendProp *pProp, const void *pStructBase,
...
SendPropDataTable("teamplayroundbased_gamerules_data", 0,
                  &REFERENCE_SEND_TABLE(DT_TeamplayRoundBasedRules),
                  SendProxy_TeamplayRoundBasedRules) END_SEND_TABLE()

    BEGIN_DATADESC(CTeamplayRoundBasedRulesProxy)
    // Inputs.
    DEFINE_INPUTFUNC(FIELD_BOOLEAN, "SetStalemateOnTimelimit",
...
()
    ...
    // Purpose:
    //-----------------------------------------------------------------------------
    void CTeamplayRoundBasedRulesProxy::InputSetStalemateOnTimelimit(
...
ConVar mp_capstyle(
    "mp_capstyle", "1", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "Sets the style of capture points used. 0 = Fixed players required to cap. "
    "1 = More players cap faster, but longer cap times.");
ConVar mp_blockstyle(
    "mp_blockstyle", "1", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "Sets the style of capture point blocking used. 0 = Blocks break captures "
    "completely. 1 = Blocks only pause captures.");
ConVar mp_respawnwavetime("mp_respawnwavetime", "10.0",
                          FCVAR_NOTIFY | FCVAR_REPLICATED,
                          "Time between respawn waves.");
ConVar mp_capdeteriorate_time(
    "mp_capdeteriorate_time", "90.0", FCVAR_REPLICATED | FCVAR_DEVELOPMENTONLY,
    "Time it takes for a full capture point to deteriorate.");

ConVar mp_teams_unbalance_limit(
    "mp_teams_unbalance_limit", "1", FCVAR_REPLICATED | FCVAR_NOTIFY,
...
#ifdef GAME_DLL
ConVar mp_showroundtransitions("mp_showroundtransitions", "0",
                               FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
                               "Show gamestate round transitions.");
ConVar mp_enableroundwaittime("mp_enableroundwaittime", "1", FCVAR_REPLICATED,
                              "Enable timers to wait between rounds.");
ConVar mp_showcleanedupents("mp_showcleanedupents", "0",
                            FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY,
                            "Show entities that are removed on round respawn.");
ConVar mp_bonusroundtime("mp_bonusroundtime", "15", FCVAR_REPLICATED,
                         "Time after round win until round restarts", true, 5,
                         true, 15);
ConVar mp_restartround("mp_restartround", "0", FCVAR_GAMEDLL,
                       "If non-zero, the current round will restart in the "
                       "specified number of seconds");
ConVar mp_winlimit("mp_winlimit", "0", FCVAR_REPLICATED,
                   "Max score one team can reach before server changes maps",
                   true, 0, false, 0);
ConVar mp_stalemate_timelimit("mp_stalemate_timelimit", "240", FCVAR_REPLICATED,
                              "Timelimit (in seconds) of the stalemate round.");
ConVar mp_autoteambalance("mp_autoteambalance", "1", FCVAR_NOTIFY);
ConVar mp_maxrounds("mp_maxrounds", "0", FCVAR_REPLICATED | FCVAR_NOTIFY,
                    "max number of rounds to play before server changes maps",
                    true, 0, false, 0);
ConVar mp_stalemate_enable("mp_stalemate_enable", "0",
...
static ConCommand mp_switchteams("mp_switchteams", cc_SwitchTeams,
...
static ConCommand mp_scrambleteams("mp_scrambleteams", cc_ScrambleTeams,
...
CON_COMMAND_F(mp_forcewin, "Forces team to win", FCVAR_CHEAT) {
  ...
  if (pRules) {
    ...
    int iWinReason = (TEAM_UNASSIGNED == iTeam ? WINREASON_STALEMATE
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::SetTeamRespawnWaveTime(int iTeam,
                                                      float flValue) {
  if (flValue < 0) {
    flValue = 0;
  }

  // initialized to -1 so we can try to determine if this is the first spawn
  // time we have received for this team
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::AddTeamRespawnWaveTime(int iTeam,
                                                      float flValue) {
...
//-----------------------------------------------------------------------------
// Purpose: don't let us spawn before our freezepanel time would have ended,
// even if we skip it
//-----------------------------------------------------------------------------
float CTeamplayRoundBasedRules::GetNextRespawnWave(int iTeam,
                                                   CBasePlayer *pPlayer) {
  if (State_Get() == GR_STATE_STALEMATE) return 0;
...
float CTeamplayRoundBasedRules::GetMinTimeWhenPlayerMaySpawn(
  ...
  float flDeathAnimLength =
      2.0 + spec_freeze_traveltime.GetFloat() + spec_freeze_time.GetFloat();

  float fMinDelay = flDeathAnimLength +
                    GetRespawnWaveMaxLength(pPlayer->GetTeamNumber(), false);
  ...
  if (g_fGameOver)  // someone else quit the game already
  {
    ...
    if (m_flIntermissionEndTime &&
      ...
      if (!IsX360()) {
        ...
        IGameEvent *event =
            gameeventmanager->CreateEvent("player_stats_updated");
        if (event) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::CheckChatText(CBasePlayer *pPlayer,
                                             char *pText) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::CheckChatForReadySignal(CBasePlayer *pPlayer,
                                                       const char *chatmsg) {
...
void CTeamplayRoundBasedRules::SetInWaitingForPlayers(bool bWaitingForPlayers) {
  ...
  if (m_bInWaitingForPlayers) {
    m_flWaitingForPlayersTimeEnds =
        gpGlobals->curtime + mp_waitingforplayers_time.GetFloat();
  } else {
...
void CTeamplayRoundBasedRules::SetOvertime(bool bOvertime) {
  if (m_bInOvertime == bOvertime) return;
...
void CTeamplayRoundBasedRules::CheckWaitingForPlayers(void) {
  ...
  if (mp_waitingforplayers_restart.GetBool()) {
    if (m_bInWaitingForPlayers) {
      ...
      if (m_hWaitingForPlayersTimer) {
        ...
        m_hWaitingForPlayersTimer->AcceptInput("SetTime", NULL, NULL, sVariant,
                                               0);
      }
    } else {
  ...
  if (mp_waitingforplayers_cancel.GetBool()) {
    ...
    if (m_hPreviousActiveTimer.Get()) {
      CTeamRoundTimer *pTimer =
          dynamic_cast<CTeamRoundTimer *>(m_hPreviousActiveTimer.Get());
  ...
  if (m_bInWaitingForPlayers) {
    // only exit the waitingforplayers if the time is up, and we are not in a
    // round restart countdown already, and we are not waiting for a ready
    // restart
    if (gpGlobals->curtime > m_flWaitingForPlayersTimeEnds &&
        m_flRestartRoundTime < 0 && !m_bAwaitingReadyRestart) {
      m_flRestartRoundTime = gpGlobals->curtime;  // reset asap

      // if "waiting for players" is ending and we're restarting...
      // keep the current round that we're already running around in as the
      // first round after the restart
      CTeamControlPointMaster *pMaster =
          g_hControlPointMasters.Count() ? g_hControlPointMasters[0] : NULL;
      if (pMaster && pMaster->PlayingMiniRounds() &&
          pMaster->GetCurrentRound()) {
      ...
      if (!m_hWaitingForPlayersTimer) {
        ...
        variant_t sVariant;
        m_hWaitingForPlayersTimer = (CTeamRoundTimer *)CBaseEntity::Create(
            "team_round_timer", vec3_origin, vec3_angle);
        m_hWaitingForPlayersTimer->SetName(
            MAKE_STRING("zz_teamplay_waiting_timer"));
        m_hWaitingForPlayersTimer->KeyValue("show_in_hud", "1");
        sVariant.SetInt(m_flWaitingForPlayersTimeEnds - gpGlobals->curtime);
        m_hWaitingForPlayersTimer->AcceptInput("SetTime", NULL, NULL, sVariant,
                                               0);
        m_hWaitingForPlayersTimer->AcceptInput("Resume", NULL, NULL, sVariant,
                                               0);
        m_hWaitingForPlayersTimer->AcceptInput("Enable", NULL, NULL, sVariant,
                                               0);
      }
    }
  }
...
void CTeamplayRoundBasedRules::CheckRestartRound(void) {
  ...
  if (iRestartDelay > 0) {
    ...
    IGameEvent *event =
        gameeventmanager->CreateEvent("teamplay_round_restart_seconds");
    if (event) {
    ...
    if (mp_restartgame.GetInt() > 0) {
      if (ShouldSwitchTeams()) {
        pFormat = (iRestartDelay > 1) ? "#game_switch_in_secs"
                                      : "#game_switch_in_sec";
      } else if (ShouldScrambleTeams()) {
        pFormat = (iRestartDelay > 1) ? "#game_scramble_in_secs"
                                      : "#game_scramble_in_sec";
      } else {
        pFormat = (iRestartDelay > 1) ? "#game_restart_in_secs"
                                      : "#game_restart_in_sec";
      }
    } else if (mp_restartround.GetInt() > 0) {
      pFormat = (iRestartDelay > 1) ? "#round_restart_in_secs"
...
bool CTeamplayRoundBasedRules::CheckTimeLimit(void) {
  if ((mp_timelimit.GetInt() > 0 && CanChangelevelBecauseOfTimeLimit()) ||
      m_bChangelevelAfterStalemate) {
    // If there's less than 5 minutes to go, just switch now. This avoids the
    // problem of sudden death modes starting shortly after a new round starts.
    const int iMinTime = 5;
    bool bSwitchDueToTime =
        (mp_timelimit.GetInt() > iMinTime && GetTimeLeft() < (iMinTime * 60));

    if (GetTimeLeft() <= 0 || m_bChangelevelAfterStalemate ||
        bSwitchDueToTime) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::State_Transition(
    gamerules_roundstate_t newState) {
  State_Leave();
...
// Purpose:
//-----------------------------------------------------------------------------
CGameRulesRoundStateInfo *CTeamplayRoundBasedRules::State_LookupInfo(
    gamerules_roundstate_t state) {
  static CGameRulesRoundStateInfo playerStateInfos[] = {
      {GR_STATE_INIT, "GR_STATE_INIT",
       &CTeamplayRoundBasedRules::State_Enter_INIT, NULL,
       &CTeamplayRoundBasedRules::State_Think_INIT},
      {GR_STATE_PREGAME, "GR_STATE_PREGAME",
       &CTeamplayRoundBasedRules::State_Enter_PREGAME, NULL,
       &CTeamplayRoundBasedRules::State_Think_PREGAME},
      {GR_STATE_STARTGAME, "GR_STATE_STARTGAME",
       &CTeamplayRoundBasedRules::State_Enter_STARTGAME, NULL,
       &CTeamplayRoundBasedRules::State_Think_STARTGAME},
      {GR_STATE_PREROUND, "GR_STATE_PREROUND",
       &CTeamplayRoundBasedRules::State_Enter_PREROUND, NULL,
       &CTeamplayRoundBasedRules::State_Think_PREROUND},
      {GR_STATE_RND_RUNNING, "GR_STATE_RND_RUNNING",
       &CTeamplayRoundBasedRules::State_Enter_RND_RUNNING, NULL,
       &CTeamplayRoundBasedRules::State_Think_RND_RUNNING},
      {GR_STATE_TEAM_WIN, "GR_STATE_TEAM_WIN",
       &CTeamplayRoundBasedRules::State_Enter_TEAM_WIN, NULL,
       &CTeamplayRoundBasedRules::State_Think_TEAM_WIN},
      {GR_STATE_RESTART, "GR_STATE_RESTART",
       &CTeamplayRoundBasedRules::State_Enter_RESTART, NULL,
       &CTeamplayRoundBasedRules::State_Think_RESTART},
      {GR_STATE_STALEMATE, "GR_STATE_STALEMATE",
...
void CTeamplayRoundBasedRules::State_Enter_PREROUND(void) {
  m_flStateTransitionTime =
      gpGlobals->curtime + 5 * mp_enableroundwaittime.GetFloat();
...
void CTeamplayRoundBasedRules::State_Think_RND_RUNNING(void) {
  ...
  // See if we're coming up to the server timelimit, in which case force a
  // stalemate immediately.
  if (State_Get() == GR_STATE_RND_RUNNING && m_bAllowStalemateAtTimelimit &&
      mp_timelimit.GetInt() > 0) {
    if (GetTimeLeft() <= 5.0) {
      int iDrawScoreCheck = -1;
      bool bTeamsAreDrawn = true;
      for (int i = FIRST_GAME_TEAM; (i < GetNumberOfTeams()) && bTeamsAreDrawn;
           i++) {
...
void CTeamplayRoundBasedRules::State_Enter_TEAM_WIN(void) {
  float flTime = std::max(5, mp_bonusroundtime.GetFloat());

  m_flStateTransitionTime =
      gpGlobals->curtime + flTime * mp_enableroundwaittime.GetFloat();

  // if we're forcing the map to reset it must be the end of a "full" round not
  // a mini-round
  if (m_bForceMapReset) {
    m_nRoundsPlayed++;
  }
...
void CTeamplayRoundBasedRules::State_Think_TEAM_WIN(void) {
  if (gpGlobals->curtime > m_flStateTransitionTime) {
    // check the win limit, max rounds, time limit and nextlevel cvar before
...
void CTeamplayRoundBasedRules::State_Enter_STALEMATE(void) {
  ...
  if (!m_hStalemateTimer) {
    m_hStalemateTimer = (CTeamRoundTimer *)CBaseEntity::Create(
        "team_round_timer", vec3_origin, vec3_angle);
  }
  ...
  if (iDeadTeam && iAliveTeam) {
    ...
    if (!m_bForceMapReset) {
      ...
      for (int i = 0; i < g_hControlPointMasters.Count(); i++) {
        if (g_hControlPointMasters[i]) {
          ...
          g_hControlPointMasters[i]->AcceptInput("SetWinnerAndForceCaps", NULL,
                                                 NULL, sVariant, 0);
          bMasterHandled = true;
        }
      }
    ...
    if (!bMasterHandled) {
      SetWinningTeam(iAliveTeam, WINREASON_OPPONENTS_DEAD, m_bForceMapReset);
             ...
             (m_hStalemateTimer && TimerMayExpire() &&
              m_hStalemateTimer->GetTimeRemaining() <= 0)) {
...
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::SetWinningTeam(int team, int iWinReason,
  ...
  if ((team != TEAM_UNASSIGNED) &&
      (team <= LAST_SHARED_TEAM || team >= GetNumberOfTeams())) {
  ...
  // only reward the team if they have won the map and we're going to do a full
  // reset or the time has run out and we're changing maps
  bool bRewardTeam =
      bForceMapReset || (IsGameUnderTimeLimit() && (GetTimeLeft() <= 0));
  ...
  if (team == TEAM_UNASSIGNED) {
    for (int i = 1; i <= gpGlobals->maxClients; i++) {
      CBaseMultiplayerPlayer *pPlayer =
          ToBaseMultiplayerPlayer(UTIL_PlayerByIndex(i));
      if (!pPlayer) continue;
...
//			round timer expired
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::SetStalemate(int iReason,
                                            bool bForceMapReset /* = true */,
                                            bool bSwitchTeams /* = false */) {
  if (!mp_stalemate_enable.GetBool()) {
    SetWinningTeam(TEAM_UNASSIGNED, WINREASON_STALEMATE, bForceMapReset,
                   bSwitchTeams);
    return;
  }
...
static ConCommand mp_forcerespawnplayers("mp_forcerespawnplayers",
                                         CC_CH_ForceRespawn,
...
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::RespawnPlayers(
    bool bForceRespawn, bool bTeam /* = false */,
    int iTeam /* = TEAM_UNASSIGNED */) {
  if (bTeam) {
  ...
  for (int i = 1; i <= gpGlobals->maxClients; i++) {
    ...
    if (!bForceRespawn) {
      ...
      if (m_iRoundState != GR_STATE_PREROUND) {
        ...
        if (bTeam && !HasPassedMinRespawnTime(pPlayer)) continue;
...
//-----------------------------------------------------------------------------
// Purpose: Recreate all the map entities from the map data (preserving their
// indices),
   ...
   public:
    ...
    virtual CBaseEntity *CreateNextEntity(const char *pClassname) {
      if (m_iIterator == g_MapEntityRefs.InvalidIndex()) {
        ...
        // used CTeamplayMapEntityFilter, which should have built the
...
bool FindInList(const char **pStrings, const char *pToFind) {
  ...
  while (pStrings[i][0] != 0) {
    if (Q_stricmp(pStrings[i], pToFind) == 0) return true;
    i++;
  }
...
// )
//-----------------------------------------------------------------------------
static int SwitchPlayersSort(CBaseMultiplayerPlayer *const *p1,
                             CBaseMultiplayerPlayer *const *p2) {
  // sort by score
...
void CTeamplayRoundBasedRules::CheckRespawnWaves(void) {
  for (int team = LAST_SHARED_TEAM + 1; team < GetNumberOfTeams(); team++) {
    if (m_flNextRespawnWave[team] > gpGlobals->curtime) continue;

    RespawnTeam(team);

    m_flNextRespawnWave.Set(team,
                            gpGlobals->curtime + GetRespawnWaveMaxLength(team));
  }
}
...
void CTeamplayRoundBasedRules::BalanceTeams(bool bRequireSwitcheesToBeDead) {
  ...
  if (!m_bPrintedUnbalanceWarning &&
      ((gpGlobals->curtime - m_flFoundUnbalancedTeamsTime) > 1.0)) {
  ...
  int iNumSwitchesRequired =
      (pHeavyTeam->GetNumPlayers() - pLightTeam->GetNumPlayers()) / 2;
  ...
  for (int i = 0; i < vecPlayers.Count() && iNumSwitchesRequired > 0 &&
                  i < iNumEligibleSwitchees;
       i++) {
    ...
    if (bRequireSwitcheesToBeDead == false || !pPlayer->IsAlive()) {
      ...
      IGameEvent *event =
          gameeventmanager->CreateEvent("teamplay_teambalanced_player");
      if (event) {
      ...
      UTIL_ClientPrintAll(HUD_PRINTTALK, "#game_player_was_team_balanced",
...
void CTeamplayRoundBasedRules::ResetMapTime(void) {
  ...
  IGameEvent *event =
      gameeventmanager->CreateEvent("teamplay_map_time_remaining");
  if (event) {
...
void CTeamplayRoundBasedRules::AddPlayedRound(string_t strName) {
  if (strName != NULL_STRING) {
    ...
    if (m_iszPreviousRounds.Count() > 2) {
      // remove all but two of the entries (should only ever have to remove 1
      // when we're at 3)
...
bool CTeamplayRoundBasedRules::IsPreviouslyPlayedRound(string_t strName) {
  return (m_iszPreviousRounds.Find(strName) !=
          m_iszPreviousRounds.InvalidIndex());
}
...
//-----------------------------------------------------------------------------
float CTeamplayRoundBasedRules::GetRespawnWaveMaxLength(
    int iTeam, bool bScaleWithNumPlayers /* = true */) {
  if (State_Get() != GR_STATE_RND_RUNNING) return 0;

  float flTime =
      ((m_TeamRespawnWaveTimes[iTeam] >= 0) ? m_TeamRespawnWaveTimes[iTeam]
                                            : mp_respawnwavetime.GetFloat());

  // For long respawn times, scale the time as the number of players drops
  if (bScaleWithNumPlayers && flTime > 5) {
    flTime = std::max(5.0f, flTime * GetRespawnTimeScalar(iTeam));
  }
...
//-----------------------------------------------------------------------------
bool CTeamplayRoundBasedRules::WouldChangeUnbalanceTeams(int iNewTeam,
                                                         int iCurrentTeam) {
  ...
  for (pTeam = GetGlobalTeam(i); pTeam != NULL; pTeam = GetGlobalTeam(++i)) {
    if (pTeam == pNewTeam) continue;

    int iNumPlayers = pTeam->GetNumPlayers();

    if (i == iCurrentTeam) {
      iNumPlayers = std::max(0, iNumPlayers - 1);
    }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CTeamplayRoundBasedRules::AreTeamsUnbalanced(int &iHeaviestTeam,
                                                  int &iLightestTeam) {
  ...
  for (CTeam *pTeam = GetGlobalTeam(i); pTeam != NULL;
...
void CTeamplayRoundBasedRules::OnDataChanged(DataUpdateType_t updateType) {
  ...
  if (updateType == DATA_UPDATE_CREATED) {
    if (State_Get() == GR_STATE_STALEMATE) {
      IGameEvent *event =
          gameeventmanager->CreateEvent("teamplay_round_stalemate");
      if (event) {
