...
//	SendPropInt( SENDINFO( m_bReflectViewModelAnimations ), 1,
//SPROP_UNSIGNED ),
#else
...
bool CBaseHLCombatWeapon::Deploy(void) {
  ...
  // We have to ask the player if the last time it checked, the weapon was
  // lowered
  if (GetOwner() && GetOwner()->IsPlayer()) {
    CHL2_Player *pPlayer = assert_cast<CHL2_Player *>(GetOwner());
    if (pPlayer->IsWeaponLowered()) {
      if (SelectWeightedSequence(ACT_VM_IDLE_LOWERED) !=
          ACTIVITY_NOT_AVAILABLE) {
        if (DefaultDeploy((char *)GetViewModel(), (char *)GetWorldModel(),
                          ACT_VM_IDLE_LOWERED, (char *)GetAnimPrefix())) {
          m_bLowered = true;

          // Stomp the next attack time to fix the fact that the lower idles are
          // long
...
bool CBaseHLCombatWeapon::WeaponShouldBeLowered(void) {
  ...
  if (GetIdealActivity() != ACT_VM_IDLE_LOWERED &&
      ...
      GetIdealActivity() != ACT_VM_IDLE_TO_LOWERED &&
      GetIdealActivity() != ACT_VM_LOWERED_TO_IDLE)
    return false;

  if (m_bLowered) return true;
...
void CBaseHLCombatWeapon::WeaponIdle(void) {
  ...
  if (WeaponShouldBeLowered()) {
    ...
    if (m_flRaiseTime < gpGlobals->curtime &&
...
float CBaseHLCombatWeapon::CalcViewmodelBob(void) {
  ...
  // NOTENOTE: For now, let this cycle continue when in the air, because it
  // snaps badly without it

  if ((!gpGlobals->frametime) || (player == NULL)) {
    // NOTENOTE: We don't use this return value in our case (need to restructure
    // the calculation function setup!)
  ...
  cycle =
      bobtime - (int)(bobtime / HL2_BOB_CYCLE_MAX * 2) * HL2_BOB_CYCLE_MAX * 2;
...
//			viewmodelindex -
//-----------------------------------------------------------------------------
void CBaseHLCombatWeapon::AddViewmodelBob(CBaseViewModel *viewmodel,
...
CBaseHLCombatWeapon::GetDefaultProficiencyValues() {
  // Weapon proficiency table. Keep this in sync with WeaponProficiency_t enum
  // in the header!!
