...
static CMemoryPool g_EdictTouchLinks(sizeof(touchlink_t), MAX_EDICTS,
                                     CMemoryPool::GROW_NONE,
                                     "g_EdictTouchLinks");
static CMemoryPool g_EntityGroundLinks(sizeof(groundlink_t), MAX_EDICTS,
                                       CMemoryPool::GROW_NONE,
                                       "g_EntityGroundLinks");
...
class CWatcherList {
 public:
  // CWatcherList(); NOTE: Dataobj doesn't support constructors - it zeros the
  // memory
  ~CWatcherList();  // frees the positionwatcher_t's to the pool
  void Init();

  void NotifyPositionChanged(CBaseEntity *pEntity);
  void NotifyVPhysicsStateChanged(IPhysicsObject *pPhysics,
                                  CBaseEntity *pEntity, bool bAwake);
...
// Prints warnings if any entity think functions take longer than this many
// milliseconds
#ifdef _DEBUG
...
ConVar think_limit("think_limit", DEF_THINK_LIMIT, FCVAR_REPLICATED,
                   "Maximum think time in milliseconds, warning is printed if "
                   "this is exceeded.");
#ifndef CLIENT_DLL
...
CCallQueue *GetPortalCallQueue() {
  return (CPortalTouchScope::m_nDepth > 0) ? &CPortalTouchScope::m_CallQueue
                                           : NULL;
}
...
class CDataObjectAccessSystem : public CAutoGameSystem {
 public:
  ...
  virtual bool Init() {
    ...
    AddDataAccessor(STEPSIMULATION,
                    new CEntityDataInstantiator<StepSimulationData>);
    AddDataAccessor(MODELWIDTHSCALE,
                    new CEntityDataInstantiator<ModelWidthScale>);
    AddDataAccessor(POSITIONWATCHER, new CEntityDataInstantiator<CWatcherList>);
    AddDataAccessor(PHYSICSPUSHLIST,
                    new CEntityDataInstantiator<physicspushlist_t>);
    AddDataAccessor(VPHYSICSUPDATEAI,
                    new CEntityDataInstantiator<vphysicsupdateai_t>);
 ...
 private:
  bool IsValidType(int type) const {
    if (type < 0 || type >= MAX_ACCESSORS) return false;

    if (m_Accessors[type] == NULL) return false;
    return true;
  }
...
void *CBaseEntity::GetDataObject(int type) {
  Assert(type >= 0 && type < NUM_DATAOBJECT_TYPES);
  if (!HasDataObjectType(type)) return NULL;
...
void CBaseEntity::DestroyDataObject(int type) {
  Assert(type >= 0 && type < NUM_DATAOBJECT_TYPES);
  if (!HasDataObjectType(type)) return;
...
int CWatcherList::GetCallbackObjects(IWatcherCallback **pList, int listMax) {
  ...
  for (unsigned short node = g_WatcherList.Head(m_list);
       node != g_WatcherList.InvalidIndex(); node = next) {
...
void CWatcherList::NotifyPositionChanged(CBaseEntity *pEntity) {
  IWatcherCallback
      *pCallbacks[1024];  // HACKHACK: Assumes this list is big enough
...
void CWatcherList::NotifyVPhysicsStateChanged(IPhysicsObject *pPhysics,
                                              CBaseEntity *pEntity,
                                              bool bAwake) {
  IWatcherCallback
      *pCallbacks[1024];  // HACKHACK: Assumes this list is big enough!
...
static void AddWatcherToEntity(CBaseEntity *pWatcher, CBaseEntity *pEntity,
                               int watcherType) {
...
static void RemoveWatcherFromEntity(CBaseEntity *pWatcher, CBaseEntity *pEntity,
                                    int watcherType) {
  CWatcherList *pList = (CWatcherList *)pEntity->GetDataObject(watcherType);
  if (pList) {
...
void ReportPositionChanged(CBaseEntity *pMovedEntity) {
  CWatcherList *pList =
      (CWatcherList *)pMovedEntity->GetDataObject(POSITIONWATCHER);
  if (pList) {
    pList->NotifyPositionChanged(pMovedEntity);
  }
}

void WatchVPhysicsStateChanges(CBaseEntity *pWatcher,
                               CBaseEntity *pPhysicsEntity) {
  AddWatcherToEntity(pWatcher, pPhysicsEntity, VPHYSICSWATCHER);
}

void RemoveVPhysicsStateWatcher(CBaseEntity *pWatcher,
                                CBaseEntity *pPhysicsEntity) {
  AddWatcherToEntity(pWatcher, pPhysicsEntity, VPHYSICSWATCHER);
}

void ReportVPhysicsStateChanged(IPhysicsObject *pPhysics, CBaseEntity *pEntity,
                                bool bAwake) {
      ...
      if (root) {
        ...
        for (touchlink_t *link = root->nextLink; link != root;
          ...
          Msg("[%d] (%d) Link %d (%s) -> %d (%s)\n", nCount,
              pClass->IsDormant(), pClass->entindex(), pClass->GetClassname(),
              link->entityTouched->entindex(),
              link->entityTouched->GetClassname());
        }
      }
    }
  }
...
inline touchlink_t *AllocTouchLink(void) {
  touchlink_t *link =
      (touchlink_t *)g_EdictTouchLinks.Alloc(sizeof(touchlink_t));
  if (link) {
    ++linksallocated;
  } else {
...
inline groundlink_t *AllocGroundLink(void) {
  groundlink_t *link =
      (groundlink_t *)g_EntityGroundLinks.Alloc(sizeof(groundlink_t));
  if (link) {
...
//			Called at the end of a frame, after all the entities
// have run
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsCheckForEntityUntouch(void) {
  ...
  if (root) {
    ...
    while (link != root) {
      ...
      // these touchlinks are not polled.  The ents are touching due to an
      // outside system that will add/delete them as necessary (vphysics in this
      // case)
...
//-----------------------------------------------------------------------------
// Purpose: notifies an entity than another touching entity has moved out of
// contact. Input  : *other - the entity to be acted upon
//-----------------------------------------------------------------------------
  ...
  if (root) {
    ...
    while (link != root) {
      if (link->entityTouched == ent) {
        ...
        if (g_bCleanupDatObject && root->nextLink == root &&
            root->prevLink == root) {
...
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsRemoveToucher(CBaseEntity *otherEntity,
                                       touchlink_t *link) {
  ...
  if (DebugTouchlinks())
    Msg("remove %p: %s-%s (%d-%d) [%d in play, %d max]\n", link,
        link->entityTouched->GetDebugName(), otherEntity->GetDebugName(),
        link->entityTouched->entindex(), otherEntity->entindex(),
...
void CBaseEntity::PhysicsRemoveTouchedList(CBaseEntity *ent) {
  ...
  if (root) {
    ...
    while (link && link != root) {
      ...
      if (DebugTouchlinks())
        Msg("remove %p: %s-%s (%d-%d) [%d in play, %d max]\n", link,
            ent->GetDebugName(), link->entityTouched->GetDebugName(),
            ent->entindex(), link->entityTouched->entindex(), linksallocated,
...
// contact. Input  : *other - the entity to be acted upon
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsNotifyOtherOfGroundRemoval(CBaseEntity *ent,
                                                    CBaseEntity *other) {
  if (!other) return;
...
//			Stamps the touch link with the new time so that when we
// check for 			untouch we know things haven't changed.
...
touchlink_t *CBaseEntity::PhysicsMarkEntityAsTouched(CBaseEntity *other) {
  ...
  if (IsSolidFlagSet(FSOLID_TRIGGER) && other->IsSolidFlagSet(FSOLID_TRIGGER)) {
    if (!IsSolid() && !other->IsSolid()) return NULL;
  }
  ...
  link = AllocTouchLink();
  if (DebugTouchlinks())
    Msg("add %p: %s-%s (%d-%d) [%d in play, %d max]\n", link, GetDebugName(),
        other->GetDebugName(), entindex(), other->entindex(), linksallocated,
        g_EdictTouchLinks.PeakCount());
  if (!link) return NULL;
  ...
  bool bShouldTouch = (IsSolid() && !IsSolidFlagSet(FSOLID_VOLUME_CONTENTS)) ||
                      IsSolidFlagSet(FSOLID_TRIGGER);
...
const trace_t &CBaseEntity::GetTouchTrace(void) { return g_TouchTrace; }

//-----------------------------------------------------------------------------
// Purpose: Marks the fact that two edicts are in contact
// Input  : *other - other entity
//-----------------------------------------------------------------------------
void CBaseEntity::PhysicsMarkEntitiesAsTouching(CBaseEntity *other,
                                                trace_t &trace) {
  g_TouchTrace = trace;
...
void CBaseEntity::PhysicsMarkEntitiesAsTouchingEventDriven(CBaseEntity *other,
                                                           trace_t &trace) {
  g_TouchTrace = trace;
...
void CBaseEntity::UpdateWaterState() {
  // TODO(d.rattman): This computation is nonsensical for rigid child
  // attachments Should we just grab the type + level of the parent? Probably
  // for rigid children anyways...
...
bool CBaseEntity::PhysicsCheckWater(void) {
  if (GetMoveParent()) return GetWaterLevel() > 1;
...
void CBaseEntity::PhysicsAddGravityMove(Vector &move) {
  ...
  float newZVelocity =
      vecAbsVelocity.z - GetActualGravity(this) * gpGlobals->frametime;

  move.z = ((vecAbsVelocity.z + newZVelocity) / 2.0 + GetBaseVelocity().z) *
           gpGlobals->frametime;
...
//-----------------------------------------------------------------------------
// Purpose: Slide off of the impacting object.  Returns the blocked flags (1 =
// floor, 2 = step / wall) Input  : in -
//			normal -
//			out -
//			overbounce -
// Output : int
//-----------------------------------------------------------------------------
int CBaseEntity::PhysicsClipVelocity(const Vector &in, const Vector &normal,
                                     Vector &out, float overbounce) {
  float backoff;
  float change;
  float angle;
  int i, blocked;
...
void CBaseEntity::ResolveFlyCollisionBounce(trace_t &trace, Vector &vecVelocity,
  ...
  physprops->GetPhysicsProperties(trace.surface.surfaceProps, NULL, NULL, NULL,
                                  &flSurfaceElasticity);
  ...
  PhysicsClipVelocity(GetAbsVelocity(), trace.plane.normal, vecAbsVelocity,
                      2.0f);
  ...
  if (trace.plane.normal.z > 0.7f)  // Floor
  {
    ...
    if (flSpeedSqr < (30 * 30)) {
      ...
      VectorScale(vecAbsVelocity,
                  (1.0f - trace.fraction) * gpGlobals->frametime, vecVelocity);
      VectorMA(vecVelocity, (1.0f - trace.fraction) * gpGlobals->frametime,
               GetBaseVelocity() * flScale, vecVelocity);
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseEntity::ResolveFlyCollisionSlide(trace_t &trace,
                                           Vector &vecVelocity) {
  // Get the impact surface's friction.
  float flSurfaceFriction;
  physprops->GetPhysicsProperties(trace.surface.surfaceProps, NULL, NULL,
                                  &flSurfaceFriction, NULL);
  ...
  if (flSpeedSqr < (30 * 30)) {
    ...
    vecAbsVelocity *=
        (1.0f - trace.fraction) * gpGlobals->frametime * flSurfaceFriction;
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseEntity::ResolveFlyCollisionCustom(trace_t &trace,
                                            Vector &vecVelocity) {
  // Stop if on ground.
  if (trace.plane.normal.z > 0.7)  // Floor
  {
...
void CBaseEntity::PerformFlyCollisionResolution(trace_t &trace, Vector &move) {
  switch (GetMoveCollide()) {
    ...
    case MOVECOLLIDE_DEFAULT:
      // NOTE: The default fly collision state is the same as a slide (for
      // backward capatability).
      {
...
//-----------------------------------------------------------------------------
// Purpose: Checks if an object has passed into or out of water and sets water
// info, alters velocity, plays splash sounds, etc.
//-----------------------------------------------------------------------------
...
void CBaseEntity::PhysicsToss(void) {
  ...
  // FL_ONGROUND
  if (GetAbsVelocity()[2] > 0 || !GetGroundEntity() ||
  ...
  if (GetFlags() & FL_ONGROUND) {
    if (VectorCompare(GetAbsVelocity(), vec3_origin)) {
      ...
      SetLocalAngularVelocity(vec3_angle);
      if (VectorCompare(GetBaseVelocity(), vec3_origin)) return;
    }
  }
  ...
  if (VPhysicsGetObject()) {
    VPhysicsGetObject()->UpdateShadow(GetAbsOrigin(), vec3_angle, true,
                                      gpGlobals->frametime);
  }
#endif
...
void CBaseEntity::PhysicsRigidChild(void) {
  ...
  if (VPhysicsGetObject()) {
    ...
    bool bAxisAligned =
        (solidType == SOLID_BBOX || solidType == SOLID_NONE) ? true : false;
    VPhysicsGetObject()->UpdateShadow(
        GetAbsOrigin(), bAxisAligned ? vec3_angle : GetAbsAngles(), true,
        gpGlobals->frametime);
  }
#endif
...
void CBaseEntity::PhysicsSimulate(void) {
  ...
  // NOTE:  Players override PhysicsSimulate and drive through their CUserCmds
  // at that point instead of
  //  processng through this function call!!!  They shouldn't chain to here
  //  ever.
  // Make sure not to simulate this guy twice per frame
  if (m_nSimulationTick == gpGlobals->tickcount) return;
  ...
  if ((GetMoveType() == MOVETYPE_NONE && !pMoveParent) ||
  ...
  if (pMoveParent) {
    ...
    if (((GetFlags() & FL_BASEVELOCITY) == 0) &&
        (GetBaseVelocity() != vec3_origin)) {
      // Apply momentum (add in half of the previous frame of velocity first)
      // BUGBUG: This will break with PhysicsStep() because of the timestep
      // difference
      Vector vecAbsVelocity;
      VectorMA(GetAbsVelocity(), 1.0 + (gpGlobals->frametime * 0.5),
               GetBaseVelocity(), vecAbsVelocity);
      SetAbsVelocity(vecAbsVelocity);
  ...
  switch (GetMoveType()) {
    ...
    default:
      Warning("PhysicsSimulate: %s bad movetype %d", GetClassname(),
              GetMoveType());
      Assert(0);
      break;
  }
...
bool CBaseEntity::PhysicsRunThink(thinkmethods_t thinkMethod) {
  if (IsEFlagSet(EFL_NO_THINK_FUNCTION)) return true;
...
#if !defined(CLIENT_DLL)
static ConVar sv_thinktimecheck("sv_thinktimecheck", "0", 0,
...
class CThinkSyncTester {
 public:
  ...
  void EntityThinking(int framecount, CBaseEntity *ent, float thinktime,
                      int thinktick) {
#if !defined(CLIENT_DLL)
 ...
 private:
  ...
  void Report() {
    if (m_Thinkers.Count() == 0) return;

    Msg("-----------------\nThink report frame %i\n", gpGlobals->tickcount);

    for (int i = m_Thinkers.FirstInorder(); i != m_Thinkers.InvalidIndex();
      ...
      for (int j = 0; j < ecount; j++) {
        ...
        Msg("  %p : %30s (last %5i/next %5i)\n", h.Get(),
            h.Get() ? h->GetClassname() : "NULL", lastthinktick, nextthinktick);
      }
    }
  }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseEntity::PhysicsRunSpecificThink(int nContextIndex,
                                          BASEPTR thinkFunc) {
  int thinktick = GetNextThinkTick(nContextIndex);

  if (thinktick <= 0 || thinktick > gpGlobals->tickcount) return true;
...
#if !defined(CLIENT_DLL)
  g_ThinkChecker.EntityThinking(gpGlobals->tickcount, this, thinktime,
                                m_nNextThinkTick);
#endif
...
void CBaseEntity::SetGroundEntity(CBaseEntity *ground) {
  if (m_hGroundEntity.Get() == ground) return;

#ifdef GAME_DLL
  // this can happen in-between updates to the held object controller
  // (physcannon, +USE) so trap it here and release held objects when they
  // become player ground
  ...
  // HACK/PARANOID:  This is redundant with the code above, but in case we get
  // out of sync groundlist entries ever,
...
bool CBaseEntity::HasNPCsOnIt(void) {
  ...
  if (root) {
    for (link = root->nextLink; link != root; link = link->nextLink) {
      if (link->entity && link->entity->MyNPCPointer()) return true;
    }
  }
