...
CEventRelativeTag::CEventRelativeTag(CChoreoEvent *owner, const char *name,
                                     float percentage) {
  Assert(owner);
  Assert(name);
...
// Purpose: Returns the corrected time based on the owner's length and start
// time Output : float
...
CFlexTimingTag::CFlexTimingTag(CChoreoEvent *owner, const char *name,
                               float percentage, bool locked)
...
CFlexTimingTag::CFlexTimingTag(const CFlexTimingTag &src) : BaseClass(src) {
...
CEventAbsoluteTag::CEventAbsoluteTag(CChoreoEvent *owner, const char *name,
                                     float t) {
  Assert(owner);
  Assert(name);
  Assert(t >= 0.0f);
...
CFlexAnimationTrack::CFlexAnimationTrack(CChoreoEvent *event) {
  ...
  m_nFlexControllerIndexRaw[0] = m_nFlexControllerIndexRaw[1] =
      LocalFlexController_t(-1);
...
CExpressionSample *CFlexAnimationTrack::GetSample(int index, int type /*=0*/) {
  ...
  if (index < 0 || index >= GetNumSamples(type)) return NULL;
...
void CFlexAnimationTrack::SetEdgeInfo(bool leftEdge, int curveType,
                                      float zero) {
...
void CFlexAnimationTrack::GetEdgeInfo(bool leftEdge, int &curveType,
                                      float &zero) const {
  int idx = leftEdge ? 0 : 1;
...
CExpressionSample *CFlexAnimationTrack::GetBoundedSample(int number,
                                                         bool &bClamped,
                                                         int type /*=0*/) {
  Assert(type == 0 || type == 1);
...
float CFlexAnimationTrack::GetFracIntensity(float time, int type) {
  ...
  Interpolator_CurveInterpolatorsForType(esStart->GetCurveType(), dummy,
                                         earlypart);
  Interpolator_CurveInterpolatorsForType(esEnd->GetCurveType(), laterpart,
                                         dummy);
  ...
  if (earlypart == INTERPOLATE_HOLD) {
    ...
    if (sameCurveType) {
      Interpolator_CurveInterpolate(laterpart, vPre, vStart, vEnd, vNext, f2,
                                    vOut);
    } else  // curves differ, sigh
    {
      Vector vOut1, vOut2;

      Interpolator_CurveInterpolate(earlypart, vPre, vStart, vEnd, vNext, f2,
                                    vOut1);
      Interpolator_CurveInterpolate(laterpart, vPre, vStart, vEnd, vNext, f2,
                                    vOut2);
...
CExpressionSample *CFlexAnimationTrack::AddSample(float time, float value,
                                                  int type /*=0*/) {
  Assert(type == 0 || type == 1);
...
LocalFlexController_t CFlexAnimationTrack::GetRawFlexControllerIndex(
    int side /*= 0*/) {
  Assert(side == 0 || side == 1);
...
void CFlexAnimationTrack::SetFlexControllerIndex(LocalFlexController_t raw,
                                                 int index, int side /*= 0*/) {
  Assert(side == 0 || side == 1);
...
CChoreoEvent::CChoreoEvent(CChoreoScene *scene, EVENTTYPE type,
                           const char *name) {
  Init(scene);
  SetType(type);
  SetName(name);
...
CChoreoEvent &CChoreoEvent::operator=(const CChoreoEvent &src) {
  ...
  for (i = 0; i < src.m_FlexAnimationTracks.Size(); i++) {
    CFlexAnimationTrack *newtrack =
        new CFlexAnimationTrack(src.m_FlexAnimationTracks[i]);
...
void CChoreoEvent::SetType(EVENTTYPE type) {
  ...
  if (m_fType == SPEAK || m_fType == SUBSCENE) {
    m_bFixedLength = true;
  } else {
    m_bFixedLength = false;
  }
...
const char *CChoreoEvent::GetDescription(void) {
  ...
  if (!GetActor()) {
    ...
    Assert(m_pChannel);
    Q_snprintf(description, sizeof(description), "%s : %s : %s -- %s \"%s\"",
               m_pActor->GetName(), m_pChannel->GetName(), GetName(),
               NameForType(GetType()), GetParameters());
...
float CCurveData::GetIntensityArea(ICurveDataAccessor *data, float time) {
  ...
  // vStart.x) * f2 );
  flTotal = flTotal + std::clamp(vOut.y, 0.0f, 1.0f) * (vEnd.x - vStart.x);
  return flTotal;
}
...
void CCurveData::UpdateIntensityArea(ICurveDataAccessor *data) {
  ...
  CExpressionSample *esEnd =
      GetBoundedSample(data, std::min(i + 1, rampCount), dummy);
  ...
  for (i = -1; i < rampCount; i++) {
    CExpressionSample *esNext =
        GetBoundedSample(data, std::min(i + 2, rampCount), dummy);
    Vector vNext(esNext->time, esNext->value, 0);

    Catmull_Rom_Spline_Integral_Normalize(vPre, vStart, vEnd, vNext, 1.0f,
                                          vOut);

    m_RampAccumulator[i + 1] =
        std::clamp(vOut.y, 0.0f, 1.0f) * (vEnd.x - vStart.x);
 ...
 public:
  CCheckEventNames() {
    ...
    for (int i = 0; i < CChoreoEvent::NUM_TYPES; ++i) {
      ...
      Error("g_NameMap:  Event type at %i has wrong value (%i)!", i,
...
CChoreoEvent::EVENTTYPE CChoreoEvent::TypeForName(const char *name) {
  for (int i = 0; i < NUM_TYPES; ++i) {
    ...
    if (!Q_stricmp(name, slot->name)) return slot->type;
  }
 ...
 public:
  CCheckCCNames() {
    ...
    for (int i = 0; i < CChoreoEvent::NUM_CC_TYPES; ++i) {
      ...
      Error("g_CCNameMap:  Event type at %i has wrong value (%i)!", i,
...
// Purpose:
// Input  : set if the gesture should sync its exit tag with the following
// gestures entry tag
...
// Purpose:
// Output  : get if the gesture should sync its exit tag with the following
// gestures entry tag
...
int CChoreoEvent::GetNumRelativeTags(void) { return m_RelativeTags.Size(); }
...
int CChoreoEvent::GetNumTimingTags(void) { return m_TimingTags.Size(); }
...
void CChoreoEvent::AddTimingTag(const char *tagname, float percentage,
                                bool locked) {
...
CFlexAnimationTrack *CChoreoEvent::GetFlexAnimationTrack(int index) {
  if (index < 0 || index >= GetNumFlexAnimationTracks()) return NULL;
...
void CChoreoEvent::StartProcessing(IChoreoEventCallback *cb,
                                   CChoreoScene *scene, float t) {
...
void CChoreoEvent::ContinueProcessing(IChoreoEventCallback *cb,
                                      CChoreoScene *scene, float t) {
  Assert(m_bProcessing);
  if (cb) {
...
void CChoreoEvent::StopProcessing(IChoreoEventCallback *cb, CChoreoScene *scene,
                                  float t) {
  Assert(m_bProcessing);
  if (cb) {
...
bool CChoreoEvent::CheckProcessing(IChoreoEventCallback *cb,
                                   CChoreoScene *scene, float t) {
  // Assert( !m_bProcessing );
  if (cb) {
...
void CChoreoEvent::SnapTimes() {
  ...
  // Don't snap end time for fixed length events, just set based on new start
  // time
  if (IsFixedLength()) {
...
CChoreoEvent::AbsTagType CChoreoEvent::TypeForAbsoluteTagName(
    const char *name) {
...
CEventAbsoluteTag *CChoreoEvent::FindAbsoluteTag(AbsTagType type,
                                                 const char *tagname) {
...
void CChoreoEvent::AddAbsoluteTag(AbsTagType type, const char *tagname,
                                  float t) {
...
float CChoreoEvent::GetBoundedAbsoluteTagPercentage(AbsTagType type,
                                                    int tagnum) {
  if (tagnum <= -2) {
    /*
...
float CChoreoEvent::GetOriginalPercentageFromPlaybackPercentage(float t) {
  ...
  if (pStartTag && pEndTag) {
    if (pStartTag->GetLinear() && pEndTag->GetLinear()) {
      ...
      if (pOrigStartTag && pOrigEndTag) {
        s = (t - pStartTag->GetPercentage()) /
            (pEndTag->GetPercentage() - pStartTag->GetPercentage());
        return (1 - s) * pOrigStartTag->GetPercentage() +
               s * pOrigEndTag->GetPercentage();
      }
    }
  }
  ...
  Vector vPre(GetBoundedAbsoluteTagPercentage(PLAYBACK, prev),
              GetBoundedAbsoluteTagPercentage(ORIGINAL, prev), 0);
  Vector vStart(GetBoundedAbsoluteTagPercentage(PLAYBACK, start),
                GetBoundedAbsoluteTagPercentage(ORIGINAL, start), 0);
  Vector vEnd(GetBoundedAbsoluteTagPercentage(PLAYBACK, end),
              GetBoundedAbsoluteTagPercentage(ORIGINAL, end), 0);
  Vector vNext(GetBoundedAbsoluteTagPercentage(PLAYBACK, next),
               GetBoundedAbsoluteTagPercentage(ORIGINAL, next), 0);

  // simulate sections of either side of "linear" portion of ramp as linear
  // slope
  if (pStartTag && pStartTag->GetLinear()) {
    vPre.Init(vStart.x - (vEnd.x - vStart.x), vStart.y - (vEnd.y - vStart.y),
              0);
  }
  ...
  Vector vOut;
  Catmull_Rom_Spline_NormalizeX(vPre, vStart, vEnd, vNext, f2, vOut);
  ...
  float retval = std::clamp( vOut.y, 0.0f, duration );
  return retval;
  */
}
...
float CChoreoEvent::GetPlaybackPercentageFromOriginalPercentage(float t) {
  ...
  if (pStartTag && pEndTag) {
    if (pStartTag->GetLinear() && pEndTag->GetLinear()) {
      ...
      if (pPlaybackStartTag && pPlaybackEndTag) {
        s = (t - pStartTag->GetPercentage()) /
            (pEndTag->GetPercentage() - pStartTag->GetPercentage());
        return (1 - s) * pPlaybackStartTag->GetPercentage() +
               s * pPlaybackEndTag->GetPercentage();
      }
    }
  }
  ...
  Vector vPre(GetBoundedAbsoluteTagPercentage(ORIGINAL, prev),
              GetBoundedAbsoluteTagPercentage(PLAYBACK, prev), 0);
  Vector vStart(GetBoundedAbsoluteTagPercentage(ORIGINAL, start),
                GetBoundedAbsoluteTagPercentage(PLAYBACK, start), 0);
  Vector vEnd(GetBoundedAbsoluteTagPercentage(ORIGINAL, end),
              GetBoundedAbsoluteTagPercentage(PLAYBACK, end), 0);
  Vector vNext(GetBoundedAbsoluteTagPercentage(ORIGINAL, next),
               GetBoundedAbsoluteTagPercentage(PLAYBACK, next), 0);
...
CExpressionSample *CCurveData::GetBoundedSample(ICurveDataAccessor *data,
                                                int number, bool &bClamped) {
  // Search for two samples which span time f
  if (number < 0) {
    static CExpressionSample nullstart;
    nullstart.time = 0.0f;
...
void CChoreoEvent::RescaleGestureTimes(float newstart, float newend,
  ...
  if (GetType() != CChoreoEvent::GESTURE) return;
  ...
  if (bMaintainAbsoluteTagPositions) {
    ...
    for (i = 0; i < count; i++) {
      ...
      tagtime = std::clamp(tagtime / newduration, 0.0f, 1.0f);
...
bool CChoreoEvent::GetPlaybackCloseCaptionToken(char *dest, int destlen) {
  ...
  switch (m_ccType) {
    ...
    case CC_MASTER: {
      // Always use the override if we're the master, otherwise always use the
      // default
      //  parameter
      if (m_CCToken[0] != 0) {
        Q_strncpy(dest, m_CCToken.Get(), destlen);
...
static void CleanupTokenName(char const *in, char *dest, int destlen) {
  ...
  while (*in && (out - dest) < destlen) {
    if (isalnum(
            *in) ||  // lowercase, uppercase, digits and underscore are valid
        *in == '_') {
      *out++ = *in;
    } else {
...
bool CChoreoEvent::ComputeCombinedBaseFileName(char *dest, int destlen,
                                               bool creategenderwildcard) {
  if (m_fType != SPEAK) return false;
  ...
  if (creategenderwildcard) {
    Q_snprintf(dest, destlen, "sound/combined/%s%s_$gender.wav", pvcd,
               cleanedtoken);
  } else {
...
int CChoreoEvent::ValidateCombinedFile() { return 0; }
  ...
  for (tagtype = 0; tagtype < CChoreoEvent::NUM_ABS_TAG_TYPES; tagtype++) {
    ...
    for (int i = 0; i < num; ++i) {
      CEventAbsoluteTag *abstag =
          GetAbsoluteTag((CChoreoEvent::AbsTagType)tagtype, i);
      Assert(abstag);
      buf.PutShort(pStringPool->FindOrAddString(abstag->GetName()));

      // save as u4.12
      Assert(abstag->GetPercentage() >= 0.0f &&
             abstag->GetPercentage() <= 15.0f);
...
bool CCurveData::RestoreFromBuffer(CUtlBuffer &buf,
                                   IChoreoStringPool *pStringPool) {
...
bool CChoreoEvent::RestoreFlexAnimationsFromBuffer(
    CUtlBuffer &buf, IChoreoStringPool *pStringPool) {
  ...
  for (int i = 0; i < numTracks; i++) {
    ...
    if (track->IsComboType()) {
      int combo_s = buf.GetUnsignedShort();
      for (int j = 0; j < combo_s; ++j) {
