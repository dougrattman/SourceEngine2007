...
// away before this, then teh hud hint counter will be deremented so the hint
// will be shown again, as if it had never been seen. The total display time for
// a hud hint is specified in client script HudAnimations.txt (which I can't
// read here).
...
void CBaseCombatWeapon::Spawn(void) {
  ...
  // Use more efficient bbox culling on the client. Otherwise, it'll setup bones
  // for most characters even when they're not in the frustum.
...
void CBaseCombatWeapon::Precache(void) {
  ...
  if (ReadWeaponDataFromFileForSlot(filesystem, GetClassname(),
                                    &m_hWeaponFileInfo, GetEncryptionKey())) {
...
// Purpose:
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetViewModel(
    int /*viewmodelindex = 0 -- this is ignored in the base class here*/)
    const {
...
// Purpose:
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetWeight(void) const { return GetWpnData().iWeight; }

//-----------------------------------------------------------------------------
// Purpose: Whether this weapon can be autoswitched to when the player runs out
//			of ammo in their current weapon or they pick this weapon
// up.
//-----------------------------------------------------------------------------
...
//			runs out of ammo in this weapon or picks up another
// weapon or ammo.
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetSlot(void) const { return GetWpnData().iSlot; }
...
bool CBaseCombatWeapon::HasAmmo(void) {
  ...
  if (m_iPrimaryAmmoType == -1 && m_iSecondaryAmmoType == -1) return true;
  if (GetWeaponFlags() & ITEM_FLAG_SELECTONEMPTY) return true;

  CBasePlayer *player = ToBasePlayer(GetOwner());
  if (!player) return false;
  return (m_iClip1 > 0 || player->GetAmmoCount(m_iPrimaryAmmoType) ||
          m_iClip2 > 0 || player->GetAmmoCount(m_iSecondaryAmmoType));
}

//-----------------------------------------------------------------------------
// Purpose: Return true if this weapon should be seen, and hence be selectable,
// in the weapon selection
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::VisibleInWeaponSelection(void) { return true; }
...
bool CBaseCombatWeapon::HasWeaponIdleTimeElapsed(void) {
  if (gpGlobals->curtime > m_flTimeWeaponIdle) return true;
...
// Output : float
//-----------------------------------------------------------------------------
float CBaseCombatWeapon::GetWeaponIdleTime(void) { return m_flTimeWeaponIdle; }
...
void CBaseCombatWeapon::OnPickedUp(CBaseCombatCharacter *pNewOwner) {
  ...
  if (pNewOwner->IsPlayer()) {
    ...
    if (filter.GetRecipientCount()) {
      CBaseEntity::EmitSound(filter, pNewOwner->entindex(),
                             "Player.PickupWeapon");
    }
...
//			iTracerType -
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::MakeTracer(const Vector &vecTracerSrc,
  ...
  switch (iTracerType) {
    case TRACER_LINE:
      UTIL_Tracer(vNewSrc, tr.endpos, iEntIndex, iAttachment, 0.0f, true,
                  pszTracerName);
      break;
...
void CBaseCombatWeapon::DefaultTouch(CBaseEntity *pOther) {
  ...
  if (HasSpawnFlags(SF_WEAPON_NO_PLAYER_PICKUP)) return;
...
bool CBaseCombatWeapon::ShouldDisplayAltFireHUDHint() {
  if (m_iAltFireHudHintCount >= WEAPON_RELOAD_HUD_HINT_COUNT) return false;
...
bool CBaseCombatWeapon::ShouldDisplayReloadHUDHint() {
  if (m_iReloadHudHintCount >= WEAPON_RELOAD_HUD_HINT_COUNT) return false;

  CBaseCombatCharacter *pOwner = GetOwner();

  if (pOwner != NULL && pOwner->IsPlayer() && UsesClipsForAmmo1() &&
      m_iClip1 < (GetMaxClip1() / 2)) {
    // I'm owned by a player, I use clips, I have less then half a clip loaded.
    // Now, does the player have more ammo?
    if (pOwner) {
      if (pOwner->GetAmmoCount(m_iPrimaryAmmoType) > 0) return true;
    }
  }
...
bool CBaseCombatWeapon::IsViewModelSequenceFinished(void) {
  ...
  if (GetActivity() == ACT_RESET || GetActivity() == ACT_INVALID) return true;
...
bool CBaseCombatWeapon::HasPrimaryAmmo(void) {
  ...
  if (pOwner) {
    ...
    if (GetPrimaryAmmoCount() > 0) return true;
  }
...
bool CBaseCombatWeapon::HasSecondaryAmmo(void) {
  // If I use a clip, and have some ammo in it, then I have ammo
  if (UsesClipsForAmmo2()) {
    if (m_iClip2 > 0) return true;
  }

  // Otherwise, I have ammo if I have some in my ammo counts
  CBaseCombatCharacter *pOwner = GetOwner();
  if (pOwner) {
    if (pOwner->GetAmmoCount(m_iSecondaryAmmoType) > 0) return true;
  }
...
bool CBaseCombatWeapon::IsWeaponVisible(void) {
  ...
  if (pOwner) {
    vm = pOwner->GetViewModel(m_nViewModelIndex);
    if (vm) return (!vm->IsEffectActive(EF_NODRAW));
  }
...
//			to the next best weapon we've got. Returns true if it
// took any action.
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::ReloadOrSwitchWeapons(void) {
  ...
  if (!HasAnyAmmo() && m_flNextPrimaryAttack < gpGlobals->curtime &&
    ...
    // Weapon is useable. Reload if empty and weapon has waited as long as it
    // has to after firing
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::DefaultDeploy(char *szViewModel, char *szWeaponModel,
  ...
  if (!HasAnyAmmo() && AllowsAutoSwitchFrom()) return false;

  CBasePlayer *pOwner = ToBasePlayer(GetOwner());
  if (pOwner) {
    // Dead men deploy no weapons
    if (pOwner->IsAlive() == false) return false;
  ...
  This code is disabled for now, because moving through the weapons in the
  carousel selects and deploys each weapon as you pass it. (sjb)
...
bool CBaseCombatWeapon::Deploy() {
  ...
  return DefaultDeploy((char *)GetViewModel(), (char *)GetWorldModel(),
                       GetDrawActivity(), (char *)GetAnimPrefix());
}

Activity CBaseCombatWeapon::GetDrawActivity(void) { return ACT_VM_DRAW; }
...
bool CBaseCombatWeapon::Holster(CBaseCombatWeapon *pSwitchingTo) {
  ...
  if (!flSequenceDuration) {
    ...
    SetContextThink(&CBaseCombatWeapon::HideThink,
                    gpGlobals->curtime + flSequenceDuration,
                    HIDEWEAPON_THINK_CONTEXT);
  }

  // if we were displaying a hud hint, squelch it.
  if (m_flHudHintMinDisplayTime &&
      gpGlobals->curtime < m_flHudHintMinDisplayTime) {
    if (m_bAltFireHudHintDisplayed) RescindAltFireHudHint();

    if (m_bReloadHudHintDisplayed) RescindReloadHudHint();
  }
...
void CBaseCombatWeapon::BoneMergeFastCullBloat(
    Vector &localMins, Vector &localMaxs, const Vector &thisEntityMins,
    const Vector &thisEntityMaxs) const {
  // The default behavior pushes it out by BONEMERGE_FASTCULL_BBOX_EXPAND in all
  // directions, but we can do better since we know the weapon will never point
  // behind him.

  localMaxs.x += 20;  // Leaves some space in front for long weapons.

  localMins.y -=
      20;  // Fatten it to his left and right since he can rotate that way.
  localMaxs.y += 20;
...
void CBaseCombatWeapon::ItemPreFrame(void) {
...
#endif
  {
    ...
    // display the hint, and the player is not standing still, try to show a hud
    // hint. If the player IS standing still, assume they could change away from
    // this weapon at any second.
    if ((!m_bAltFireHudHintDisplayed || !m_bReloadHudHintDisplayed) &&
        gpGlobals->curtime > m_flHudHintMinDisplayTime &&
        gpGlobals->curtime > m_flHudHintPollTime && GetOwner() &&
        GetOwner()->IsPlayer()) {
      CBasePlayer *pPlayer = (CBasePlayer *)(GetOwner());

      if (pPlayer && pPlayer->GetStickDist() > 0.0f) {
        // If the player is moving, they're unlikely to switch away from the
        // current weapon the moment this weapon displays its HUD hint.
...
void CBaseCombatWeapon::ItemPostFrame(void) {
  ...
  if ((pOwner->m_nButtons & IN_ATTACK2) &&
    ...
    if (UsesSecondaryAmmo() &&
      ...
      if (m_flNextEmptySoundTime < gpGlobals->curtime) {
        WeaponSound(EMPTY);
        m_flNextSecondaryAttack = m_flNextEmptySoundTime =
  ...
  if (!bFired && (pOwner->m_nButtons & IN_ATTACK) &&
    ...
    if (!IsMeleeWeapon() && ((UsesClipsForAmmo1() && m_iClip1 <= 0) ||
                             (!UsesClipsForAmmo1() &&
      ...
      // NOTENOTE: There is a bug with this code with regards to the way machine
      // guns catch the leading edge trigger 			on the player
      // hitting the attack
      // key.  It relies on the gun catching that case in the same frame.
      //			However, because the player can also be doing a
      // secondary attack, the edge trigger may be missed.
      // We really need to hold onto the edge trigger and only clear the
      // condition when the gun has
      // fired its 			first shot.  Right now that's too much
      // of an architecture change -- jdw

      // If the firing button was just pressed, or the alt-fire just released,
  ...
  if (pOwner->m_nButtons & IN_RELOAD && UsesClipsForAmmo1() && !m_bInReload) {
    // reload when reload is pressed, or if no buttons are down and weapon is
    // empty.
    Reload();
...
// Output :
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetBulletType(void) { return 0; }
...
// Output :
//-----------------------------------------------------------------------------
float CBaseCombatWeapon::GetFireRate(void) { return 0; }
...
// Output :
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::WeaponSound(WeaponSound_t sound_type,
  ...
  if (!shootsound || !shootsound[0]) return;
...
#if !defined(CLIENT_DLL)
      if (sound_type == EMPTY) {
        CSoundEnt::InsertSound(SOUND_COMBAT, GetOwner()->GetAbsOrigin(),
                               SOUNDENT_VOLUME_EMPTY, 0.2, GetOwner());
      }
#endif
    }
...
// Purpose:
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::DefaultReload(int iClipSize1, int iClipSize2,
                                      int iActivity) {
  CBaseCombatCharacter *pOwner = GetOwner();
  ...
  if (UsesClipsForAmmo1()) {
    ...
    int primary = std::min(iClipSize1 - m_iClip1,
  ...
  if (UsesClipsForAmmo2()) {
    ...
    int secondary = std::min(iClipSize2 - m_iClip2,
...
void CBaseCombatWeapon::FinishReload(void) {
  ...
  if (pOwner) {
    ...
    if (UsesClipsForAmmo1()) {
      int primary = std::min(GetMaxClip1() - m_iClip1,
                             pOwner->GetAmmoCount(m_iPrimaryAmmoType));
      m_iClip1 += primary;
    ...
    if (UsesClipsForAmmo2()) {
      int secondary = std::min(GetMaxClip2() - m_iClip2,
                               pOwner->GetAmmoCount(m_iSecondaryAmmoType));
      m_iClip2 += secondary;
...
void CBaseCombatWeapon::PrimaryAttack(void) {
  ...
  // bullet here on low-framerate systems, especially if the weapon we're firing
  // has a really fast rate of fire.
  info.m_iShots = 0;
  ...
  if (UsesClipsForAmmo1()) {
    info.m_iShots = std::min(info.m_iShots, static_cast<int>(m_iClip1));
    m_iClip1 -= info.m_iShots;
  } else {
    info.m_iShots =
        std::min(info.m_iShots, pPlayer->GetAmmoCount(m_iPrimaryAmmoType));
...
//-----------------------------------------------------------------------------
// Purpose: Called every frame to check if the weapon is going through
...
void CBaseCombatWeapon::MaintainIdealActivity(void) {
  ...
  if (IsViewModelSequenceFinished() == false) return;
...
//-----------------------------------------------------------------------------
// Purpose: Sets the ideal activity for the weapon to be in, allowing for
// transitional animations inbetween Input  : ideal - activity to end up at,
// ideally
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::GetControlPanelInfo(int nPanelIndex,
...
// Returns information about the various control panels
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::GetControlPanelClassName(int nPanelIndex,
                                                 const char *&pPanelName) {
...
DEFINE_PRED_FIELD(m_nNextThinkTick, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
    ...
    DEFINE_PRED_FIELD(m_iViewModelIndex, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
    DEFINE_PRED_FIELD(m_iWorldModelIndex, FIELD_INTEGER,
                      FTYPEDESC_INSENDTABLE | FTYPEDESC_MODELINDEX),
    DEFINE_PRED_FIELD_TOL(m_flNextPrimaryAttack, FIELD_FLOAT,
                          FTYPEDESC_INSENDTABLE, TD_MSECTOLERANCE),
    DEFINE_PRED_FIELD_TOL(m_flNextSecondaryAttack, FIELD_FLOAT,
                          FTYPEDESC_INSENDTABLE, TD_MSECTOLERANCE),
    DEFINE_PRED_FIELD_TOL(m_flTimeWeaponIdle, FIELD_FLOAT,
                          FTYPEDESC_INSENDTABLE, TD_MSECTOLERANCE),

    DEFINE_PRED_FIELD(m_iPrimaryAmmoType, FIELD_INTEGER, FTYPEDESC_INSENDTABLE),
    DEFINE_PRED_FIELD(m_iSecondaryAmmoType, FIELD_INTEGER,
    ...
    // Special hack since we're aliasing the name C_BaseCombatWeapon with a
    // macro on the client
    ...
    // Function pointers
    DEFINE_FUNCTION(DefaultTouch), DEFINE_FUNCTION(FallThink),
    DEFINE_FUNCTION(Materialize), DEFINE_FUNCTION(AttemptToMaterialize),
    DEFINE_FUNCTION(DestroyItem), DEFINE_FUNCTION(SetPickupTouch),
...
RecvPropTime(RECVINFO(m_flNextPrimaryAttack)),
    ...
    //-----------------------------------------------------------------------------
    // Purpose: Propagation data for weapons. Only sent when a player's holding
    // it.
    //-----------------------------------------------------------------------------
...
#if !defined(CLIENT_DLL)
            SendPropDataTable("LocalWeaponData", 0,
                              &REFERENCE_SEND_TABLE(DT_LocalWeaponData),
                              SendProxy_SendLocalWeaponDataTable),
    SendPropDataTable("LocalActiveWeaponData", 0,
                      &REFERENCE_SEND_TABLE(DT_LocalActiveWeaponData),
                      SendProxy_SendActiveLocalWeaponDataTable),
...
#else
            RecvPropDataTable("LocalWeaponData", 0, 0,
                              &REFERENCE_RECV_TABLE(DT_LocalWeaponData)),
    RecvPropDataTable("LocalActiveWeaponData", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_LocalActiveWeaponData)),
    RecvPropInt(RECVINFO(m_iViewModelIndex)),
    RecvPropInt(RECVINFO(m_iWorldModelIndex)), RecvPropInt(RECVINFO(m_iState)),
