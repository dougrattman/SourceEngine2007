...
CPredictionCopy::CPredictionCopy(int type, void *dest, bool dest_packed,
                                 void const *src, bool src_packed,
                                 bool counterrors /*= false*/,
                                 bool reporterrors /*= false*/,
                                 bool performcopy /*= true*/,
                                 bool describefields /*= false*/,
...
void CPredictionCopy::ReportFieldsDiffer(const char *fmt, ...) {
  ...
  if (!m_bShouldReport) return;

  if (m_bDescribeFields && m_FieldCompareFunc) return;
  ...
  if (m_pCurrentField) {
    ...
    fieldname =
        m_pCurrentField->fieldName ? m_pCurrentField->fieldName : "NULL";
  }
  ...
  Msg("%03i %s::%s - %s", m_nErrorCount, m_pCurrentClassName, fieldname, data);
...
void CPredictionCopy::DescribeFields(difftype_t dt, const char *fmt, ...) {
  ...
  (*m_FieldCompareFunc)(m_pCurrentClassName, fieldname,
                        g_FieldTypes[m_pCurrentField->fieldType], isnetworked,
                        isnoterrorchecked, dt != IDENTICAL ? true : false,
                        dt == WITHINTOLERANCE ? true : false, data);
...
//-----------------------------------------------------------------------------
/*
void CPredictionCopy::CopyData( difftype_t dt, int size, char *outdata, const
char *indata )
{
        if ( !m_bPerformCopy )
                return;
...
CPredictionCopy::difftype_t CPredictionCopy::CompareData(int size,
                                                         char *outdata,
                                                         const char *indata) {
  if (!m_bErrorCheck) return DIFFERS;
...
void CPredictionCopy::DescribeData(difftype_t dt, int size, char *outdata,
                                   const char *indata) {
  if (!m_bErrorCheck) return;
...
void CPredictionCopy::WatchData(difftype_t dt, int size, char *outdata,
                                const char *indata) {
  if (m_pWatchField != m_pCurrentField) return;

  WatchMsg("binary (%i bytes)", size);
}

void CPredictionCopy::DescribeShort(difftype_t dt, short *outvalue,
                                    const short *invalue, int count) {
  if (!m_bErrorCheck) return;

  if (dt == DIFFERS) {
    int i = 0;
    ReportFieldsDiffer("short differs (net %i pred %i) diff(%i)\n",
                       (int)(invalue[i]), (int)(outvalue[i]),
                       (int)(outvalue[i] - invalue[i]));
  }

  DescribeFields(dt, "short (%i)\n", (int)(outvalue[0]));
}

void CPredictionCopy::WatchShort(difftype_t dt, short *outvalue,
                                 const short *invalue, int count) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::DescribeInt(difftype_t dt, int *outvalue,
                                  const int *invalue, int count) {
  if (!m_bErrorCheck) return;

  if (dt == DIFFERS) {
    int i = 0;
    ReportFieldsDiffer("int differs (net %i pred %i) diff(%i)\n", invalue[i],
                       outvalue[i], outvalue[i] - invalue[i]);
  }
...
void CPredictionCopy::WatchInt(difftype_t dt, int *outvalue, const int *invalue,
                               int count) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::DescribeBool(difftype_t dt, bool *outvalue,
                                   const bool *invalue, int count) {
  if (!m_bErrorCheck) return;
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer("bool differs (net %s pred %s)\n",
                       ...
                       (outvalue[i]) ? "true" : "false");
  }

  DescribeFields(dt, "bool (%s)\n", (outvalue[0]) ? "true" : "false");
}

void CPredictionCopy::WatchBool(difftype_t dt, bool *outvalue,
                                const bool *invalue, int count) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::DescribeFloat(difftype_t dt, float *outvalue,
                                    const float *invalue, int count) {
  if (!m_bErrorCheck) return;
...
void CPredictionCopy::WatchFloat(difftype_t dt, float *outvalue,
                                 const float *invalue, int count) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::DescribeString(difftype_t dt, char *outstring,
                                     const char *instring) {
  if (!m_bErrorCheck) return;
...
void CPredictionCopy::WatchString(difftype_t dt, char *outstring,
                                  const char *instring) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::DescribeVector(difftype_t dt, Vector &outValue,
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer(
        "vec differs (net %f %f %f - pred %f %f %f) delta(%f %f %f)\n",
        inValue.x, inValue.y, inValue.z, outValue.x, outValue.y, outValue.z,
...
void CPredictionCopy::DescribeVector(difftype_t dt, Vector *outValue,
                                     const Vector *inValue, int count) {
  if (!m_bErrorCheck) return;
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer(
        "vec[] differs (1st diff) (net %f %f %f - pred %f %f %f) delta(%f %f "
        "%f)\n",
        inValue[i].x, inValue[i].y, inValue[i].z, outValue[i].x, outValue[i].y,
        outValue[i].z, delta.x, delta.y, delta.z);
  }

  DescribeFields(dt, "vector (%f %f %f)\n", outValue[0].x, outValue[0].y,
                 outValue[0].z);
}

void CPredictionCopy::WatchVector(difftype_t dt, Vector &outValue,
                                  const Vector &inValue) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::DescribeQuaternion(difftype_t dt, Quaternion &outValue,
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer(
        "quaternion differs (net %f %f %f %f - pred %f %f %f %f) delta(%f %f "
        "%f %f)\n",
        inValue[0], inValue[1], inValue[2], inValue[3], outValue[0],
        outValue[1], outValue[2], outValue[3], delta[0], delta[1], delta[2],
        delta[3]);
  }
...
void CPredictionCopy::DescribeQuaternion(difftype_t dt, Quaternion *outValue,
                                         const Quaternion *inValue, int count) {
  if (!m_bErrorCheck) return;
  ...
  if (dt == DIFFERS) {
    ...
    ReportFieldsDiffer(
        "quaternion[] differs (1st diff) (net %f %f %f %f - pred %f %f %f %f) "
        "delta(%f %f %f %f)\n",
        inValue[0], inValue[1], inValue[2], inValue[3], outValue[0],
        outValue[1], outValue[2], outValue[3], delta[0], delta[1], delta[2],
        delta[3]);
  ...
  DescribeFields(dt, "quaternion (%f %f %f %f)\n", outValue[0][0],
                 outValue[0][1], outValue[0][2], outValue[3]);
}

void CPredictionCopy::WatchQuaternion(difftype_t dt, Quaternion &outValue,
                                      const Quaternion &inValue) {
  if (m_pWatchField != m_pCurrentField) return;

  WatchMsg("quaternion (%f %f %f %f)", outValue[0], outValue[1], outValue[2],
           outValue[3]);
}
...
void CPredictionCopy::WatchQuaternion(difftype_t dt, Quaternion *outValue,
  ...
  WatchMsg("quaternion (%f %f %f %f)", outValue[0][0], outValue[0][1],
           outValue[0][2], outValue[0][3]);
}

void CPredictionCopy::DescribeEHandle(difftype_t dt, EHANDLE *outvalue,
...
void CPredictionCopy::WatchEHandle(difftype_t dt, EHANDLE *outvalue,
                                   EHANDLE const *invalue, int count) {
  if (m_pWatchField != m_pCurrentField) return;
...
void CPredictionCopy::CopyShort(difftype_t dt, short *outvalue,
                                const short *invalue, int count) {
...
CPredictionCopy::difftype_t CPredictionCopy::CompareShort(short *outvalue,
                                                          const short *invalue,
                                                          int count) {
  if (!m_bErrorCheck) return DIFFERS;
  ...
  if (CanCheck()) {
    for (int i = 0; i < count; i++) {
      if (outvalue[i] == invalue[i]) continue;
...
void CPredictionCopy::CopyInt(difftype_t dt, int *outvalue, const int *invalue,
                              int count) {
  if (!m_bPerformCopy) return;
...
CPredictionCopy::difftype_t CPredictionCopy::CompareInt(int *outvalue,
                                                        const int *invalue,
                                                        int count) {
  if (!m_bErrorCheck) return DIFFERS;
...
void CPredictionCopy::CopyBool(difftype_t dt, bool *outvalue,
                               const bool *invalue, int count) {
  if (!m_bPerformCopy) return;
...
CPredictionCopy::difftype_t CPredictionCopy::CompareBool(bool *outvalue,
                                                         const bool *invalue,
                                                         int count) {
  if (!m_bErrorCheck) return DIFFERS;
...
void CPredictionCopy::CopyFloat(difftype_t dt, float *outvalue,
                                const float *invalue, int count) {
  if (!m_bPerformCopy) return;
...
CPredictionCopy::difftype_t CPredictionCopy::CompareFloat(float *outvalue,
                                                          const float *invalue,
                                                          int count) {
  if (!m_bErrorCheck) return DIFFERS;
  ...
  if (CanCheck()) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (usetolerance && (fabs(outvalue[i] - invalue[i]) <= tolerance)) {
...
void CPredictionCopy::CopyString(difftype_t dt, char *outstring,
                                 const char *instring) {
  if (!m_bPerformCopy) return;

  if (dt == IDENTICAL) return;

  CopyData(dt, Q_strlen(instring) + 1, (char *)outstring,
           (const char *)instring);
}

CPredictionCopy::difftype_t CPredictionCopy::CompareString(
    char *outstring, const char *instring) {
  if (!m_bErrorCheck) return DIFFERS;
...
void CPredictionCopy::CopyVector(difftype_t dt, Vector &outValue,
                                 const Vector &inValue) {
  CopyVector(dt, &outValue, &inValue, 1);
}

void CPredictionCopy::CopyQuaternion(difftype_t dt, Quaternion &outValue,
                                     const Quaternion &inValue) {
  CopyQuaternion(dt, &outValue, &inValue, 1);
}

CPredictionCopy::difftype_t CPredictionCopy::CompareVector(
    Vector &outValue, const Vector &inValue) {
  if (!m_bErrorCheck) return DIFFERS;
  ...
  if (CanCheck()) {
    ...
    if (outValue != inValue && (tolerance > 0.0f)) {
      ...
      if (fabs(delta.x) <= tolerance && fabs(delta.y) <= tolerance &&
...
CPredictionCopy::difftype_t CPredictionCopy::CompareQuaternion(
    Quaternion &outValue, const Quaternion &inValue) {
  if (!m_bErrorCheck) return DIFFERS;
  ...
  if (CanCheck()) {
    ...
    if (QuaternionCompare(outValue, inValue) == 0 && (tolerance > 0.0f)) {
      ...
      if (fabs(delta[0]) <= tolerance && fabs(delta[1]) <= tolerance &&
          fabs(delta[2]) <= tolerance && fabs(delta[3]) <= tolerance) {
        return WITHINTOLERANCE;
      }
    }
...
void CPredictionCopy::CopyQuaternion(difftype_t dt, Quaternion *outValue,
  ...
  CopyData(dt, sizeof(Quaternion) * count, (char *)outValue,
           (const char *)inValue);
}

CPredictionCopy::difftype_t CPredictionCopy::CompareVector(
    Vector *outValue, const Vector *inValue, int count) {
  if (!m_bErrorCheck) return DIFFERS;
...
CPredictionCopy::difftype_t CPredictionCopy::CompareQuaternion(
    Quaternion *outValue, const Quaternion *inValue, int count) {
  if (!m_bErrorCheck) return DIFFERS;
  ...
  if (CanCheck()) {
    ...
    for (int i = 0; i < count; i++) {
      if (QuaternionCompare(outValue[i], inValue[i])) continue;
...
void CPredictionCopy::CopyEHandle(difftype_t dt, EHANDLE *outvalue,
                                  EHANDLE const *invalue, int count) {
  if (!m_bPerformCopy) return;
...
CPredictionCopy::difftype_t CPredictionCopy::CompareEHandle(
    EHANDLE *outvalue, EHANDLE const *invalue, int count) {
  if (!m_bErrorCheck) return DIFFERS;

  int i;
  if (CanCheck()) {
    for (i = 0; i < count; i++) {
      if (outvalue[i].Get() == invalue[i].Get()) continue;
...
void CPredictionCopy::CopyFields(int chain_count, datamap_t *pRootMap,
  ...
  for (i = 0; i < fieldCount; i++) {
    ...
    if (m_pCurrentField->fieldType != FIELD_EMBEDDED) {
      ...
      if (flags & FTYPEDESC_PRIVATE) continue;

      // For PC_NON_NETWORKED_ONLYs skip any fields that are present in the
      // network send tables
      if (m_nType == PC_NON_NETWORKED_ONLY && (flags & FTYPEDESC_INSENDTABLE))
        continue;

      // For PC_NETWORKED_ONLYs skip any fields that are not present in the
      // network send tables
    ...
    switch (m_pCurrentField->fieldType) {
      case FIELD_EMBEDDED: {
        ...
        CopyFields(chain_count, pRootMap, m_pCurrentField->td->dataDesc,
                   m_pCurrentField->td->dataNumFields);
      ...
      case FIELD_FLOAT: {
        difftype = CompareFloat((float *)pOutputData, (float const *)pInputData,
                                fieldSize);
        CopyFloat(difftype, (float *)pOutputData, (float const *)pInputData,
                  fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeFloat(difftype, (float *)pOutputData,
                        (float const *)pInputData, fieldSize);
        if (bShouldWatch)
          WatchFloat(difftype, (float *)pOutputData, (float const *)pInputData,
                     fieldSize);
      } break;
      ...
      case FIELD_STRING: {
        ...
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeString(difftype, (char *)pOutputData,
                         (char const *)pInputData);
        if (bShouldWatch)
      ...
      case FIELD_VECTOR: {
        difftype = CompareVector((Vector *)pOutputData,
                                 (Vector const *)pInputData, fieldSize);
        CopyVector(difftype, (Vector *)pOutputData, (Vector const *)pInputData,
                   fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeVector(difftype, (Vector *)pOutputData,
                         (Vector const *)pInputData, fieldSize);
        if (bShouldWatch)
          WatchVector(difftype, (Vector *)pOutputData,
                      (Vector const *)pInputData, fieldSize);
      } break;

      case FIELD_QUATERNION: {
        difftype = CompareQuaternion((Quaternion *)pOutputData,
                                     (Quaternion const *)pInputData, fieldSize);
        CopyQuaternion(difftype, (Quaternion *)pOutputData,
                       (Quaternion const *)pInputData, fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeQuaternion(difftype, (Quaternion *)pOutputData,
                             (Quaternion const *)pInputData, fieldSize);
        if (bShouldWatch)
          WatchQuaternion(difftype, (Quaternion *)pOutputData,
                          (Quaternion const *)pInputData, fieldSize);
      } break;

      case FIELD_COLOR32: {
        difftype = CompareData(4 * fieldSize, (char *)pOutputData,
                               (const char *)pInputData);
        CopyData(difftype, 4 * fieldSize, (char *)pOutputData,
                 (const char *)pInputData);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeData(difftype, 4 * fieldSize, (char *)pOutputData,
                       (const char *)pInputData);
        if (bShouldWatch)
          WatchData(difftype, 4 * fieldSize, (char *)pOutputData,
                    (const char *)pInputData);
      } break;

      case FIELD_BOOLEAN: {
        difftype = CompareBool((bool *)pOutputData, (bool const *)pInputData,
                               fieldSize);
        CopyBool(difftype, (bool *)pOutputData, (bool const *)pInputData,
                 fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeBool(difftype, (bool *)pOutputData, (bool const *)pInputData,
                       fieldSize);
        if (bShouldWatch)
          WatchBool(difftype, (bool *)pOutputData, (bool const *)pInputData,
                    fieldSize);
      } break;

      case FIELD_INTEGER: {
        difftype =
            CompareInt((int *)pOutputData, (int const *)pInputData, fieldSize);
        CopyInt(difftype, (int *)pOutputData, (int const *)pInputData,
                fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeInt(difftype, (int *)pOutputData, (int const *)pInputData,
                      fieldSize);
        if (bShouldWatch)
          WatchInt(difftype, (int *)pOutputData, (int const *)pInputData,
                   fieldSize);
      } break;

      case FIELD_SHORT: {
        difftype = CompareShort((short *)pOutputData, (short const *)pInputData,
                                fieldSize);
        CopyShort(difftype, (short *)pOutputData, (short const *)pInputData,
                  fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeShort(difftype, (short *)pOutputData,
                        (short const *)pInputData, fieldSize);
        if (bShouldWatch)
          WatchShort(difftype, (short *)pOutputData, (short const *)pInputData,
                     fieldSize);
      } break;

      case FIELD_CHARACTER: {
        difftype = CompareData(fieldSize, ((char *)pOutputData),
                               (const char *)pInputData);
        CopyData(difftype, fieldSize, ((char *)pOutputData),
                 (const char *)pInputData);
        ...
        if (bShouldWatch)
          WatchData(difftype, fieldSize, ((char *)pOutputData),
                    (const char *)pInputData);
      } break;
      case FIELD_EHANDLE: {
        difftype = CompareEHandle((EHANDLE *)pOutputData,
                                  (EHANDLE const *)pInputData, fieldSize);
        CopyEHandle(difftype, (EHANDLE *)pOutputData,
                    (EHANDLE const *)pInputData, fieldSize);
        if (m_bErrorCheck && m_bShouldDescribe)
          DescribeEHandle(difftype, (EHANDLE *)pOutputData,
                          (EHANDLE const *)pInputData, fieldSize);
        if (bShouldWatch)
          WatchEHandle(difftype, (EHANDLE *)pOutputData,
                       (EHANDLE const *)pInputData, fieldSize);
      } break;
      case FIELD_FUNCTION: {
        Assert(0);
...
static typedescription_t *FindFieldByName_R(const char *fieldname,
                                            datamap_t *dmap) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (td->fieldType == FIELD_EMBEDDED) {
      // TODO:  this will only find the first subclass with the variable of the
      // specified name
      //  At some point we might want to support multiple levels of overriding
      //  automatically
...
void ValidateChains_R(datamap_t *dmap) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (!(td->flags & FTYPEDESC_OVERRIDE)) continue;

    if (dmap->baseMap) {
      typedescription_t *basefield =
          FindFieldByName_R(td->fieldName, dmap->baseMap);
      if (basefield) {
...
static ConVar pwatchent("pwatchent", "-1", FCVAR_CHEAT,
                        "Entity to watch for prediction system changes.");
static ConVar pwatchvar(
    "pwatchvar", "", FCVAR_CHEAT,
...
void CPredictionCopy::WatchMsg(const char *fmt, ...) {
  ...
  Msg("%i %s %s : %s\n", gpGlobals->tickcount, m_pOperation,
      m_pCurrentField->fieldName, data);
}
...
//			*dmap -
//-----------------------------------------------------------------------------
void CPredictionCopy::DetermineWatchField(const char *operation, int entindex,
                                          datamap_t *dmap) {
  m_pWatchField = NULL;
  m_pOperation = operation;
  if (!m_pOperation || !m_pOperation[0]) return;

  int enttowatch = pwatchent.GetInt();
  if (enttowatch < 0) return;
...
// Output : int
//-----------------------------------------------------------------------------
int CPredictionCopy::TransferData(const char *operation, int entindex,
                                  datamap_t *dmap) {
  ++g_nChainCount;
...
public:
        ...
        void	DescribeFields( datamap_t *pMap, typedescription_t *pFields,
int fieldCount );
...
CPredictionDescribeData::CPredictionDescribeData(
    void const *src, bool src_packed, FN_FIELD_DESCRIPTION func /*= 0*/) {
  m_pSrc = src;
  m_nSrcOffsetIndex = src_packed ? TD_OFFSET_PACKED : TD_OFFSET_NORMAL;
...
void CPredictionDescribeData::Describe(const char *fmt, ...) {
  ...
  if (m_FieldDescFunc) {
    (*m_FieldDescFunc)(m_pCurrentClassName, m_pCurrentField->fieldName,
                       g_FieldTypes[m_pCurrentField->fieldType], isnetworked,
                       data);
  } else {
    char suffix[128];
    ...
    if (suffix[0]) {
      Msg("%s::%s(%s) - %s", m_pCurrentClassName, fieldname, suffix, data);
    } else {
      Msg("%s::%s - %s", m_pCurrentClassName, fieldname, data);
    }
  }
...
void CPredictionDescribeData::DescribeQuaternion(const Quaternion *inValue,
                                                 int count) {
  Describe("quaternion (%f %f %f %f)\n", inValue[0][0], inValue[0][1],
           inValue[0][2], inValue[0][3]);
}

void CPredictionDescribeData::DescribeEHandle(EHANDLE const *invalue,
                                              int count) {
  Describe("EHandle (%p)\n", invalue->operator C_BaseEntity *());
}

void CPredictionDescribeData::DescribeFields_R(int chain_count,
                                               datamap_t *pRootMap,
  ...
  for (i = 0; i < fieldCount; i++) {
    ...
    switch (m_pCurrentField->fieldType) {
      case FIELD_EMBEDDED: {
        ...
        DescribeFields_R(chain_count, pRootMap, m_pCurrentField->td->dataDesc,
                         m_pCurrentField->td->dataNumFields);
...
CValueChangeTracker::CValueChangeTracker()
    : m_bActive(false), m_bTracking(false) {
...
void CValueChangeTracker::GetValue(char *buf, size_t bufsize) {
  ...
  if (!m_hEntityToTrack.Get()) return;
  ...
  for (int i = 0; i < m_FieldStack.Count(); ++i) {
    ...
    if ((td->flags & FTYPEDESC_PTR) && (td->fieldType & FIELD_EMBEDDED)) {
  ...
  switch (fieldType) {
    ...
    case FIELD_COLOR32: {
      ...
      Q_snprintf(buf, bufsize, "%d %d %d %d", color->r(), color->g(),
                 color->b(), color->a());
    } break;

    case FIELD_BOOLEAN:
      Q_snprintf(buf, bufsize, "%s",
                 (*(const bool *)pInputData) ? "true" : "false");
      break;
    case FIELD_INTEGER:
    case FIELD_TICK:
    case FIELD_MODELINDEX:
    ...
    case FIELD_EHANDLE:
      Q_snprintf(buf, bufsize, "eh 0x%p",
                 (void const *)((const EHANDLE *)pInputData)->Get());
      break;
  }
}
...
void CValueChangeTracker::EndTrack() {
  ...
  if (Q_stricmp(final, m_OrigValueBuf)) {
    history->Set(CFmtStr("+++ %-20.20s:  %s (was %s)", m_strContext.String(),
                         final, m_OrigValueBuf));
  } else {
...
static bool FindFieldStackByName_R(const char *fieldname, datamap_t *dmap,
                                   CUtlVector<typedescription_t *> &stack) {
  int c = dmap->dataNumFields;
  for (int i = 0; i < c; i++) {
    typedescription_t *td = &dmap->dataDesc[i];
...
void CValueChangeTracker::SetupTracking(C_BaseEntity *ent,
                                        char const *pchFieldName) {
  ClearTracking();
...
void CValueChangeTracker::Reset() { m_History.RemoveAll(); }

bool CValueChangeTracker::IsActive() const { return m_bActive; }
...
CON_COMMAND_F(cl_pred_track,
              "<entindex> <fieldname>:  Track changes to entity index "
...
DEFINE_FIELD(CCopyTesterData2, m_CharValue, FIELD_CHARACTER),
...

  {
    ...
    CPredictionCopy tester(PC_NON_NETWORKED_ONLY, foo1, false, foo3, false,
                           true);
    int diff_count = 0;
    diff_count = tester.TransferData(foo3->GetPredDescMap(),
                                     foo1->GetPredDescMap()->dataDesc,
                                     foo1->GetPredDescMap()->dataNumFields);
  ...
  {
    Msg("Simple compare of != objects, should spew and have non-zero "
        "diffcount\n");

    CPredictionCopy tester(PC_NON_NETWORKED_ONLY, foo1, false, foo2, false,
                           true, false);
    int diff_count = 0;
    diff_count = tester.TransferData(foo2->GetPredDescMap(),
                                     foo1->GetPredDescMap()->dataDesc,
                                     foo1->GetPredDescMap()->dataNumFields);
  ...
  {
    Msg("Comparing and coyping same != objects, should spew and have non-zero "
        "diffcount\n");
  ...
  {
    Msg("Comparing and copying objects which were just made to coincide, "
        "should have zero diffcount\n");
...

  {
    ...
    describe.DumpDescription(foo1->GetPredDescMap(),
                             foo1->GetPredDescMap()->dataDesc,
                             foo1->GetPredDescMap()->dataNumFields);
  }
