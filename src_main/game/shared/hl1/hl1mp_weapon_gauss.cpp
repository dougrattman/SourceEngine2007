...
#ifdef CLIENT_DLL
RecvPropInt(RECVINFO(m_nAttackState)), RecvPropBool(RECVINFO(m_bPrimaryFire)),
#else
SendPropInt(SENDINFO(m_nAttackState)), SendPropBool(SENDINFO(m_bPrimaryFire)),
#endif
    END_NETWORK_TABLE()

        BEGIN_PREDICTION_DATA(CWeaponGauss)
#ifdef CLIENT_DLL
            DEFINE_PRED_FIELD(m_nAttackState, FIELD_INTEGER,
...
void CWeaponGauss::PrimaryAttack() {
  ...
  // TODO(d.rattman): pPlayer->m_iWeaponVolume = GAUSS_PRIMARY_FIRE_VOLUME;
  m_bPrimaryFire = true;
...
void CWeaponGauss::SecondaryAttack() {
  ...
  if (m_nAttackState == 0) {
    ...
    pPlayer->RemoveAmmo(
        1, m_iPrimaryAmmoType);  // take one ammo just to start the spin
    pPlayer->m_flNextAmmoBurn = gpGlobals->curtime;

    // spin up
    // TODO(d.rattman): pPlayer->m_iWeaponVolume = GAUSS_PRIMARY_CHARGE_VOLUME;
    ...
    if (m_sndCharge == NULL) {
      ...
      m_sndCharge = (CSoundEnvelopeController::GetController())
                        .SoundCreate(filter, entindex(), CHAN_WEAPON,
                                     "Weapon_Gauss.Spin", ATTN_NORM);
    }
    ...
    if (gpGlobals->curtime >= pPlayer->m_flNextAmmoBurn &&
    ...
    int pitch = (gpGlobals->curtime - pPlayer->m_flStartCharge) *
                    (150 / GetFullChargeTime()) +
                100;
    ...
    if (m_sndCharge != NULL) {
      (CSoundEnvelopeController::GetController())
          .SoundChangePitch(m_sndCharge, pitch, 0);
    }

    // TODO(d.rattman): m_pPlayer->m_iWeaponVolume =
    ...
    if (pPlayer->m_flStartCharge < gpGlobals->curtime - 10) {
      ...
      pPlayer->TakeDamage(
          CTakeDamageInfo(this, this, 50, DMG_SHOCK | DMG_CRUSH));
      ...
      StopSpinSound();
      // Player may have been killed and this weapon dropped, don't execute any
...
void CWeaponGauss::StartFire() {
  ...
  if (m_bPrimaryFire) {
    flDamage =
        sk_plr_dmg_gauss.GetFloat() * g_pGameRules->GetDamageMultiplier();
  }

  // ALERT ( at_console, "Time:%f Damage:%f\n", gpGlobals->curtime -
  // m_pPlayer->m_flStartCharge, flDamage );
  ...
  pPlayer->m_flPlayAftershock =
      gpGlobals->curtime + random->RandomFloat(0.3, 0.8);
...
void CWeaponGauss::Fire(Vector vecOrigSrc, Vector vecDir, float flDamage) {
  ...
  while (flDamage > 10 && nMaxHits > 0) {
    ...
    // ALERT( at_console, "." );
    UTIL_TraceLine(vecSrc, vecDest, MASK_SHOT, pIgnore, COLLISION_GROUP_NONE,
                   &tr);
...
// pPlayer->GetViewModel()->entindex();
      ...
      data6.m_fFlags = m_bPrimaryFire;
      te->DispatchEffect(filter, 0.0, data6.m_vOrigin, "HL1GaussBeamReflect",
                         data6);
    }
    ...
    if (pEntity->IsBSPModel() && !fShouldDamageEntity) {
      ...
      if (n < 0.5)  // 60 degrees
      {
...
#if !defined(CLIENT_DLL)
        // explode a bit
        RadiusDamage(CTakeDamageInfo(this, pPlayer, flDamage * n, DMG_BLAST),
                     tr.endpos, flDamage * n * 2.5, CLASS_NONE, NULL);
#endif
        ...
        // try punching through wall if secondary attack (primary is incapable
        // of breaking through)
        if (!m_bPrimaryFire) {
          trace_t punch_tr;

          UTIL_TraceLine(tr.endpos + vecDir * 8, vecDest, MASK_SHOT, pIgnore,
                         COLLISION_GROUP_NONE, &punch_tr);
          if (!punch_tr.allsolid) {
            trace_t exit_tr;
            // trace backwards to find exit point
            UTIL_TraceLine(punch_tr.endpos, tr.endpos, MASK_SHOT, pIgnore,
                           COLLISION_GROUP_NONE, &exit_tr);
...
#if !defined(CLIENT_DLL)
              RadiusDamage(CTakeDamageInfo(this, pPlayer, flDamage, DMG_BLAST),
                           exit_tr.endpos + vecDir * 8, flDamageRadius,
                           CLASS_NONE, NULL);
...
void CWeaponGauss::WeaponIdle() {
  ...
  if (pPlayer->m_flPlayAftershock &&
      pPlayer->m_flPlayAftershock < gpGlobals->curtime) {
