...
CChoreoScene &CChoreoScene::operator=(const CChoreoScene &src) {
  ...
  for (i = 0; i < (int)src.m_TimeZoomLookup.Count(); i++) {
    m_TimeZoomLookup.Insert(src.m_TimeZoomLookup.GetElementName(i),
                            src.m_TimeZoomLookup[i]);
  }
...
CChoreoEvent *CChoreoScene::AllocEvent() {
...
CChoreoChannel *CChoreoScene::AllocChannel() {
  MEM_ALLOC_CREDIT_CLASS();
...
CChoreoActor *CChoreoScene::AllocActor() {
  MEM_ALLOC_CREDIT_CLASS();
...
CChoreoEvent *CChoreoScene::GetEvent(int event) {
  if (event < 0 || event >= m_Events.Size()) return NULL;
...
CChoreoActor *CChoreoScene::GetActor(int actor) {
  if (actor < 0 || actor >= GetNumActors()) return NULL;
...
CChoreoChannel *CChoreoScene::GetChannel(int channel) {
  if (channel < 0 || channel >= GetNumChannels()) return NULL;
...
void CChoreoScene::ParseSceneRamp(ISceneTokenProcessor *tokenizer,
                                  CChoreoScene *scene) {
  scene->m_SceneRamp.Parse(tokenizer, scene);
}

void CCurveData::Parse(ISceneTokenProcessor *tokenizer,
                       ICurveDataAccessor *data) {
  Clear();
  ...
  while (1) {
    ...
    if (!Q_stricmp(tokenizer->CurrentToken(), "}")) break;
...
void CChoreoScene::ParseEdgeInfo(ISceneTokenProcessor *tokenizer,
                                 EdgeInfo_t *edgeinfo) {
  Assert(edgeinfo);
  Assert(tokenizer);
  ...
  edgeinfo->m_CurveType =
      Interpolator_CurveTypeForName(tokenizer->CurrentToken());
  tokenizer->GetToken(false);
...
void CChoreoScene::ParseFlexAnimations(ISceneTokenProcessor *tokenizer,
                                       CChoreoEvent *e,
                                       bool removeold /*= true*/) {
  Assert(e);
  ...
  // if samples_use_time is true, then samples are actually offsets of time from
  // starttime
  ...
  while (1) {
    ...
    Q_strncpy(flexcontroller, tokenizer->CurrentToken(),
              sizeof(flexcontroller));
    ...
    for (int samplecount = 0; samplecount < (combo ? 2 : 1); samplecount++) {
      ...
      while (1) {
        ...
        // If there are more tokens on this line, then it's a new format curve
        // name
        if (tokenizer->TokenAvailable()) {
          tokenizer->GetToken(false);
          int curveType =
    ...
    if (active || samples[0].Size() >= 1) {
      ...
      for (int edge = 0; edge < 2; ++edge) {
        if (!edgeinfo[edge].m_bActive) continue;

        track->SetEdgeActive(edge == 0 ? true : false, true);
        track->SetEdgeInfo(edge == 0 ? true : false, edgeinfo[edge].m_CurveType,
                           edgeinfo[edge].m_flZeroPos);
      }
...
CChoreoEvent *CChoreoScene::ParseEvent(CChoreoActor *actor,
  ...
  while (1) {
    ...
    if (!Q_stricmp(m_pTokenizer->CurrentToken(), "time")) {
    ...
    } else if (!Q_stricmp(m_pTokenizer->CurrentToken(),
      ...
      tagtype =
          CChoreoEvent::TypeForAbsoluteTagName(m_pTokenizer->CurrentToken());
      ...
      e->SetCloseCaptionType(
          CChoreoEvent::CCTypeForName(m_pTokenizer->CurrentToken()));
  ...
  if (hadramp && !e->GetRampCount()) {
    ...
    if (attack != 1.0f || sustain != 1.0f || decay != 1.0f) {
...
CChoreoActor *CChoreoScene::ParseActor() {
...
void CChoreoScene::ParseMapname() {
  m_szMapname[0] = 0;
...
void CChoreoScene::ParseIgnorePhonemes() {
  m_pTokenizer->GetToken(true);
  m_bIgnorePhonemes =
      !Q_stricmp(m_pTokenizer->CurrentToken(), "on") ? true : false;
}
...
bool CChoreoScene::ParseFromBuffer(const char *pFilename,
                                   ISceneTokenProcessor *tokenizer) {
...
float CChoreoScene::FindStopTime() {
...
void CCurveData::FileSave(CUtlBuffer &buf, int level, const char *name) {
  // Nothing to save?
  int c = GetCount();
  if (c <= 0 && !IsEdgeActive(true) && !IsEdgeActive(false)) return;
  ...
  if (IsEdgeActive(true) || IsEdgeActive(false)) {
    if (IsEdgeActive(true)) {
      char sz[256];
      Q_snprintf(sz, sizeof(sz), " leftedge %s %.3f",
                 Interpolator_NameForCurveType(GetEdgeCurveType(true), false),
                 GetEdgeZeroValue(true));
      Q_strncat(line, sz, sizeof(line), COPY_ALL_CHARACTERS);
    }
    if (IsEdgeActive(false)) {
      char sz[256];
      Q_snprintf(sz, sizeof(sz), " rightedge %s %.3f",
                 Interpolator_NameForCurveType(GetEdgeCurveType(false), false),
                 GetEdgeZeroValue(false));
      Q_strncat(line, sz, sizeof(line), COPY_ALL_CHARACTERS);
    }
  }
...
void CChoreoScene::FileSaveScaleSettings(CUtlBuffer &buf, int level,
                                         CChoreoScene *scene) {
  // Nothing to save?
...
void CChoreoScene::FileSaveFlexAnimationTrack(CUtlBuffer &buf, int level,
  ...
  if (track->GetMin() != 0.0f || track->GetMax() != 1.0f) {
    char sz[256];
    Q_snprintf(sz, sizeof(sz), "range %.1f %.1f ", track->GetMin(),
               track->GetMax());
    Q_strncat(line, sz, sizeof(line), COPY_ALL_CHARACTERS);
  }
  if (track->IsEdgeActive(true) || track->IsEdgeActive(false)) {
    ...
    if (track->IsEdgeActive(true)) {
      char sz[256];
      Q_snprintf(
          sz, sizeof(sz), "leftedge %s %.3f ",
          Interpolator_NameForCurveType(track->GetEdgeCurveType(true), false),
    ...
    if (track->IsEdgeActive(false)) {
      char sz[256];
      Q_snprintf(
          sz, sizeof(sz), "rightedge %s %.3f ",
          Interpolator_NameForCurveType(track->GetEdgeCurveType(false), false),
  ...
  for (int j = 0; j < track->GetNumSamples(0); j++) {
    ...
    if (s->GetCurveType() != nDefaultCurveType &&
        s->GetCurveType() != CURVE_DEFAULT) {
...
void CChoreoScene::FileSaveFlexAnimations(CUtlBuffer &buf, int level,
                                          CChoreoEvent *e) {
  // Nothing to save
  ...
  if (e->GetDefaultCurveType() != CURVE_DEFAULT) {
    FilePrintf(buf, level + 1,
               "flexanimations samples_use_time defaultcurvetype=%s\n",
  ...
  if (e->GetNumTimingTags() > 0) {
    ...
    for (int t = 0; t < e->GetNumTimingTags(); t++) {
      ...
      FilePrintf(buf, level + 2, "\"%s\" %f %i\n", tt->GetName(),
                 tt->GetPercentage(), tt->GetLocked() ? 1 : 0);
    }
    FilePrintf(buf, level + 1, "}\n");
  }
  int tagtype;
  for (tagtype = 0; tagtype < CChoreoEvent::NUM_ABS_TAG_TYPES; tagtype++) {
    if (e->GetNumAbsoluteTags((CChoreoEvent::AbsTagType)tagtype) > 0) {
      FilePrintf(buf, level + 1, "absolutetags %s\n",
                 CChoreoEvent::NameForAbsoluteTagType(
                     (CChoreoEvent::AbsTagType)tagtype));
      FilePrintf(buf, level + 1, "{\n");
      for (int t = 0;
           t < e->GetNumAbsoluteTags((CChoreoEvent::AbsTagType)tagtype); t++) {
        CEventAbsoluteTag *abstag =
            e->GetAbsoluteTag((CChoreoEvent::AbsTagType)tagtype, t);
        Assert(abstag);
        FilePrintf(buf, level + 2, "\"%s\" %f\n", abstag->GetName(),
                   abstag->GetPercentage());
      }
      FilePrintf(buf, level + 1, "}\n");
    }
  }
...
void CChoreoScene::FileSaveChannel(CUtlBuffer &buf, int level,
                                   CChoreoChannel *c) {
  if (!c->IsMarkedForSave()) return;
...
float CChoreoScene::FindAdjustedStartTime() {
...
float CChoreoScene::FindAdjustedEndTime() {
...
void CChoreoScene::ResetSimulation(bool forward /*= true*/,
...
bool CChoreoScene::CheckEventCompletion() {
  ...
  for (int i = 0; i < m_ActiveResumeConditions.Size(); i++) {
    ...
    bAllCompleted = bAllCompleted && e->CheckProcessing(m_pIChoreoEventCallback,
                                                        this, m_flCurrentTime);
  }
...
bool CChoreoScene::SimulationFinished() {
...
CChoreoEvent *CChoreoScene::FindPauseBetweenTimes(float starttime,
                                                  float endtime) {
  CChoreoEvent *e;
...
int CChoreoScene::EventThink(CChoreoEvent *e, float frame_start_time,
  ...
  switch (e->GetType()) {
    ...
    case CChoreoEvent::SPEAK:
      // If it's a wav file, pre-queue the starting/endtime time by the sound
      // system's
      //  current latency
      {
        if (playing_forward) {
          ...
          //  adjusted start time, but make sure that the pause event hasn't
          //  already triggered...
          CChoreoEvent *pauseEvent = FindPauseBetweenTimes(
              starttime, starttime + m_flSoundSystemLatency);
          if (pauseEvent && (frame_start_time <= pauseEvent->GetStartTime())) {
        ...
        // Don't bother if playing backward!!!
        {
                ...
                // Search for pause condition in between the original time and
        the
                //  adjusted start time
                CChoreoEvent *pauseEvent = FindPauseBetweenTimes( endtime -
        m_flSoundSystemLatency, endtime ); if ( pauseEvent )
                {
                        pauseEvent->AddEventDependency( e );
  ...
  if (e->IsProcessing()) {
    ...
    // See if it's a single fire event which should occur during this frame
      ...
      where_is_event =
          IsTimeInRange(starttime, frame_start_time, frame_end_time);
      if (IN_RANGE == where_is_event) {
        disposition = PROCESSING_TYPE_START;
        iret = 1;
      }
...
bool CChoreoScene::EventLess(const CChoreoScene::ActiveList &al0,
...
void CChoreoScene::AddPauseEventDependency(CChoreoEvent *pauseEvent,
...
void CChoreoScene::Think(float curtime) {
  ...
  for (i = 0; i < m_Events.Size(); i++) {
    ...
    PROCESSING_TYPE disposition;
    m_nActiveEvents +=
        EventThink(e, m_flCurrentTime, curtime, playing_forward, disposition);
  ...
  // Events are sorted start time and then by channel and actor slot or by name
  // if those aren't equal
  bool dump = false;
  ...
  while (i != pending.InvalidIndex()) {
    ...
    if (dump) {
      Msg("%f == %s starting at %f (actor %p channel %p)\n", m_flCurrentTime,
          entry->e->GetName(), entry->e->GetStartTime(), entry->e->GetActor(),
          entry->e->GetChannel());
    }
    ...
    switch (entry->pt) {
      ...
      case PROCESSING_TYPE_START_RESUMECONDITION: {
        entry->e->StartProcessing(m_pIChoreoEventCallback, this,
                                  m_flCurrentTime);
        ...
        // This event can "pause" the scene, so we need to remember who "paused"
        // the scene so that
        //  when we resume we can resume any suppressed events dependent on this
        //  pauser...
        if (entry->e->GetType() == CChoreoEvent::SECTION) {
          // So this event should be in the pauseevents list, otherwise this'll
          // be -1
          m_nLastPauseEvent = m_PauseEvents.Find(entry->e);
        }
      } break;
      case PROCESSING_TYPE_CONTINUE: {
        entry->e->ContinueProcessing(m_pIChoreoEventCallback, this,
                                     m_flCurrentTime);
      } break;
      case PROCESSING_TYPE_STOP: {
        entry->e->StopProcessing(m_pIChoreoEventCallback, this,
                                 m_flCurrentTime);
      } break;
    }
...
void CChoreoScene::ResumeSimulation() {
  // If the thing that paused us was a SECTION pause event, then this will be
  // set
  if (m_nLastPauseEvent >= 0 && m_nLastPauseEvent < m_PauseEvents.Count()) {
    // Start any suppressed dependencies immediately, should only be .wav
    // files!!! These are .wav files which are placed at or just after the
    // SECTION pause event
    //  in the .vcd, but due to the user's sound system latency, they would have
    //  triggered before the pause (we pre-queue sounds).  Since we suppressed
    //  that, we need to unsupress / start these sounds now that the SECTION
    //  pause is being resumed from
    ...
    for (int j = 0; j < deps.Count(); ++j) {
      ...
      Assert(startEvent);
      // Start them now.  Yes, they won't pre-queue, but it's better than
      // totally skipping the sound!!!
      startEvent->StartProcessing(m_pIChoreoEventCallback, this,
                                  m_flCurrentTime);
    }
  }
...
void CChoreoScene::ReconcileTags() {
  for (int i = 0; i < m_Actors.Size(); i++) {
    CChoreoActor *a = m_Actors[i];
    if (!a) continue;
    ...
    for (int j = 0; j < a->GetNumChannels(); j++) {
      ...
      for (int k = 0; k < c->GetNumEvents(); k++) {
        ...
        CEventRelativeTag *tag =
            FindTagByName(e->GetRelativeWavName(), e->GetRelativeTagName());
        ...
        if (tag) {
          ...
          choreoprintf(0, "Event %s was missing tag %s for wav %s\n",
                       e->GetName(), e->GetRelativeWavName(),
...
CChoreoEvent *CChoreoScene::FindTargetingEvent(const char *wavname,
                                               const char *name) {
  for (int i = 0; i < m_Actors.Size(); i++) {
    CChoreoActor *a = m_Actors[i];
    if (!a) continue;
...
CEventRelativeTag *CChoreoScene::FindTagByName(const char *wavname,
                                               const char *name) {
  for (int i = 0; i < m_Actors.Size(); i++) {
    CChoreoActor *a = m_Actors[i];
    if (!a) continue;
...
void CChoreoScene::ExportEvents(const char *filename,
...
void CChoreoScene::ImportEvents(ISceneTokenProcessor *tokenizer,
                                CChoreoActor *actor, CChoreoChannel *channel) {
  m_pTokenizer = tokenizer;
...
int CChoreoScene::TimeZoomNext(int i) { return m_TimeZoomLookup.Next(i); }
...
void CChoreoScene::ParseScaleSettings(ISceneTokenProcessor *tokenizer,
                                      CChoreoScene *scene) {
  tokenizer->GetToken(true);
...
bool CChoreoScene::SaveBinary(char const *pszBinaryFileName,
                              char const *pPathID, unsigned int nTextVersionCRC,
...
void CChoreoScene::SaveToBinaryBuffer(CUtlBuffer &buf,
  ...
  */
  ...
  /*
  // compiled version strips out scale settings fps and snap, only used by
  editor FileSaveScaleSettings( buf, 0, this ); FilePrintf( buf, 0, "fps %i\n",
...
bool CChoreoScene::RestoreFromBinaryBuffer(CUtlBuffer &buf,
                                           char const *filename,
                                           IChoreoStringPool *pStringPool) {
  Q_strncpy(m_szFileName, filename, sizeof(m_szFileName));

  int tag = buf.GetInt();
  if (tag != SCENE_BINARY_TAG) return false;

  uint8_t ver = buf.GetChar();
  if (ver != SCENE_BINARY_VERSION) return false;
