...
int GetWeaponId( const char *pszWeaponName )
{
	...
	assert( std::size( g_aWeaponNames ) == ( TF_WEAPON_COUNT + 1 ) );

	for ( int iWeapon = 0; iWeapon < std::size( g_aWeaponNames ); ++iWeapon )
	{
...
const char *WeaponIdToAlias( int iWeapon )
{
	...
	if ( ( iWeapon >= std::size( g_aWeaponNames ) ) || ( iWeapon < 0 ) )
		return NULL;
...
int GetWeaponFromDamage( const CTakeDamageInfo &info )
{
	...
	{
		for( int i = 0; i < std::size( g_szProjectileNames ); i++ )
		{
