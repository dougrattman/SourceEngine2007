...
ConVar tf_spy_invis_time("tf_spy_invis_time", "1.0",
                         FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
                         "Transition time in and out of spy invisibility", true,
                         0.1, true, 5.0);
ConVar tf_spy_invis_unstealth_time(
    "tf_spy_invis_unstealth_time", "2.0",
    FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
    "Transition time in and out of spy invisibility", true, 0.1, true, 5.0);

ConVar tf_spy_max_cloaked_speed("tf_spy_max_cloaked_speed", "999",
                                FCVAR_DEVELOPMENTONLY |
                                    FCVAR_REPLICATED);  // no cap
ConVar tf_max_health_boost(
    "tf_max_health_boost", "1.5", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
    "Max health factor that players can be boosted to by healers.", true, 1.0,
    false, 0);
ConVar tf_invuln_time("tf_invuln_time", "1.0",
                      FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
                      "Time it takes for invulnerability to wear off.");

#ifdef GAME_DLL
ConVar tf_boost_drain_time(
    "tf_boost_drain_time", "15.0", FCVAR_DEVELOPMENTONLY,
    "Time is takes for a full health boost to drain away from a player.", true,
    0.1, false, 0);
ConVar tf_debug_bullets("tf_debug_bullets", "0", FCVAR_DEVELOPMENTONLY,
                        "Visualize bullet traces.");
ConVar tf_damage_events_track_for("tf_damage_events_track_for", "30",
                                  FCVAR_DEVELOPMENTONLY);
#endif

ConVar tf_useparticletracers("tf_useparticletracers", "1",
                             FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
                             "Use particle tracers instead of old style ones.");
ConVar tf_spy_cloak_consume_rate(
    "tf_spy_cloak_consume_rate", "10.0",
    FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
    "cloak to use per second while cloaked, from 100 max )");  // 10 seconds of
                                                               // invis
ConVar tf_spy_cloak_regen_rate(
    "tf_spy_cloak_regen_rate", "3.3", FCVAR_DEVELOPMENTONLY | FCVAR_REPLICATED,
    "cloak to regen per second, up to 100 max");  // 30 seconds to full charge
...
// ConVar tf_spy_stealth_blink_time( "tf_spy_stealth_blink_time", "0.3",
// FCVAR_DEVELOPMENTONLY, "time after being hit the spy blinks into view" );
// ConVar tf_spy_stealth_blink_scale( "tf_spy_stealth_blink_scale", "0.85",
// FCVAR_DEVELOPMENTONLY, "percentage visible scalar after being hit the spy
// blinks into view" );
...
const char *g_pszBDayGibs[22] = {"models/effects/bday_gib01.mdl",
...
RecvPropInt(RECVINFO(m_nDesiredDisguiseTeam)),
    ...
    RecvPropArray3(RECVINFO_ARRAY(m_bPlayerDominated),
                   RecvPropBool(RECVINFO(m_bPlayerDominated[0]))),
    RecvPropArray3(RECVINFO_ARRAY(m_bPlayerDominatingMe),
                   RecvPropBool(RECVINFO(m_bPlayerDominatingMe[0]))),
    END_RECV_TABLE()

        BEGIN_RECV_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerShared)
            RecvPropInt(RECVINFO(m_nPlayerCond)),
    RecvPropInt(RECVINFO(m_bJumping)), RecvPropInt(RECVINFO(m_nNumHealers)),
    RecvPropInt(RECVINFO(m_iCritMult)), RecvPropInt(RECVINFO(m_bAirDash)),
    ...
    // Local Data.
    RecvPropDataTable("tfsharedlocaldata", 0, 0,
                      &REFERENCE_RECV_TABLE(DT_TFPlayerSharedLocal)),
    END_RECV_TABLE()

        BEGIN_PREDICTION_DATA_NO_BASE(CTFPlayerShared)
            DEFINE_PRED_FIELD(m_nPlayerState, FIELD_INTEGER,
    ...
    DEFINE_PRED_FIELD(m_flInvisChangeCompleteTime, FIELD_FLOAT,
                      FTYPEDESC_INSENDTABLE),
    END_PREDICTION_DATA()
// Server specific.
#else
...
SendPropInt(SENDINFO(m_nDesiredDisguiseTeam), 3, SPROP_UNSIGNED),
    ...
    SendPropFloat(SENDINFO(m_flCloakMeter), 0,
                  SPROP_NOSCALE | SPROP_CHANGES_OFTEN, 0.0, 100.0),
    SendPropArray3(SENDINFO_ARRAY3(m_bPlayerDominated),
                   SendPropBool(SENDINFO_ARRAY(m_bPlayerDominated))),
    SendPropArray3(SENDINFO_ARRAY3(m_bPlayerDominatingMe),
                   SendPropBool(SENDINFO_ARRAY(m_bPlayerDominatingMe))),
    END_SEND_TABLE()

        BEGIN_SEND_TABLE_NOBASE(CTFPlayerShared, DT_TFPlayerShared)
            SendPropInt(SENDINFO(m_nPlayerCond), TF_COND_LAST,
                        SPROP_UNSIGNED | SPROP_CHANGES_OFTEN),
    SendPropInt(SENDINFO(m_bJumping), 1, SPROP_UNSIGNED | SPROP_CHANGES_OFTEN),
    SendPropInt(SENDINFO(m_nNumHealers), 5,
                SPROP_UNSIGNED | SPROP_CHANGES_OFTEN),
    ...
    SendPropInt(SENDINFO(m_nPlayerState), Q_log2(TF_STATE_COUNT) + 1,
                SPROP_UNSIGNED),
    SendPropInt(SENDINFO(m_iDesiredPlayerClass), Q_log2(TF_CLASS_COUNT_ALL) + 1,
                SPROP_UNSIGNED),
    // Spy
    ...
    // Local Data.
    SendPropDataTable("tfsharedlocaldata", 0,
                      &REFERENCE_SEND_TABLE(DT_TFPlayerSharedLocal),
...
//-----------------------------------------------------------------------------
void CTFPlayerShared::AddCond(int nCond,
                              float flDuration /* = PERMANENT_CONDITION */) {
...
void CTFPlayerShared::ConditionGameRulesThink(void) {
  ...
  // Our health will only decay ( from being medic buffed ) if we are not being
  // healed by a medic Dispensers can give us the TF_COND_HEALTH_BUFF, but will
  // not maintain or give us health above 100%s
  bool bDecayHealth = true;

  // If we're being healed, heal ourselves
  if (InCond(TF_COND_HEALTH_BUFF)) {
    // Heal faster if we haven't been in combat for a while
    float flTimeSinceDamage =
        gpGlobals->curtime - m_pOuter->GetLastDamageTime();
    ...
    for (int i = 0; i < m_aHealers.Count(); i++) {
      ...
      {
        m_flHealFraction +=
            gpGlobals->frametime * m_aHealers[i].flAmount * flScale;
      }
    ...
    if (nHealthToAdd > 0) {
      ...
      for (int i = 0; i < m_aHealers.Count(); i++) {
        ...
        if (m_aHealers[i].pPlayer.IsValid()) {
          CTFPlayer *pPlayer = static_cast<CTFPlayer *>(
              static_cast<CBaseEntity *>(m_aHealers[i].pPlayer));
          if (IsAlly(pPlayer)) {
            CTF_GameStats.Event_PlayerHealedOther(
                pPlayer,
                nHealthToAdd * (m_aHealers[i].flAmount / fTotalHealAmount));
          } else {
            CTF_GameStats.Event_PlayerLeachedHealth(
  ...
  if (bDecayHealth) {
    ...
    if (m_pOuter->GetHealth() > m_pOuter->GetMaxHealth()) {
      ...
      m_flHealFraction += (gpGlobals->frametime *
                           (flBoostMaxAmount / tf_boost_drain_time.GetFloat()));
    ...
    if (InCond(TF_COND_DISGUISED) &&
        m_iDisguiseHealth > m_pOuter->GetMaxHealth()) {
      float flBoostMaxAmount = GetMaxBuffedHealth() - m_pOuter->GetMaxHealth();
      m_flDisguiseHealFraction +=
          (gpGlobals->frametime *
           (flBoostMaxAmount / tf_boost_drain_time.GetFloat()));
  ...
  if (InCond(TF_COND_BURNING) &&
      (m_pOuter->m_flPowerPlayTime < gpGlobals->curtime)) {
    // If we're underwater, put the fire out
    if (gpGlobals->curtime > m_flFlameRemoveTime ||
        m_pOuter->GetWaterLevel() >= WL_Waist) {
      RemoveCond(TF_COND_BURNING);
    } else if ((gpGlobals->curtime >= m_flFlameBurnTime) &&
               (TF_CLASS_PYRO != m_pOuter->GetPlayerClass()->GetClassIndex())) {
      // Burn the player (if not pyro, who does not take persistent burning
      // damage)
      CTakeDamageInfo info(m_hBurnAttacker, m_hBurnAttacker, TF_BURNING_DMG,
  ...
  if (m_pOuter->IsPlayerClass(TF_CLASS_MEDIC)) {
    CWeaponMedigun *pWeapon =
        (CWeaponMedigun *)m_pOuter->Weapon_OwnsThisID(TF_WEAPON_MEDIGUN);
  ...
  if (InCond(TF_COND_INVULNERABLE)) {
    ...
    if ((TFGameRules()->State_Get() == GR_STATE_TEAM_WIN) &&
        (TFGameRules()->GetWinningTeam() != m_pOuter->GetTeamNumber())) {
      bRemoveInvul = true;
    }
  ...
  if (InCond(TF_COND_STEALTHED_BLINK)) {
    if (TF_SPY_STEALTH_BLINKTIME /*tf_spy_stealth_blink_time.GetFloat()*/ <
        (gpGlobals->curtime - m_flLastStealthExposeTime)) {
...
void CTFPlayerShared::ConditionThink(void) {
  ...
  if (m_pOuter->IsPlayerClass(TF_CLASS_SPY) && bIsLocalPlayer) {
    if (InCond(TF_COND_STEALTHED)) {
      m_flCloakMeter -=
          gpGlobals->frametime * tf_spy_cloak_consume_rate.GetFloat();
      ...
      m_flCloakMeter +=
          gpGlobals->frametime * tf_spy_cloak_regen_rate.GetFloat();
...
void CTFPlayerShared::OnAddDisguising(void) {
  ...
  if (!m_pOuter->IsLocalPlayer() &&
      (!InCond(TF_COND_STEALTHED) || !m_pOuter->IsEnemyPlayer())) {
    const char *pEffectName = (m_pOuter->GetTeamNumber() == TF_TEAM_RED)
                                  ? "spy_start_disguise_red"
                                  : "spy_start_disguise_blue";
    m_pOuter->m_pDisguisingEffect =
        m_pOuter->ParticleProp()->Create(pEffectName, PATTACH_ABSORIGIN_FOLLOW);
...
void CTFPlayerShared::OnAddInvulnerable(void) {
  ...
  if (m_pOuter->IsLocalPlayer()) {
    ...
    IMaterial *pMaterial = materials->FindMaterial(
        pEffectName, TEXTURE_GROUP_CLIENT_EFFECTS, false);
...
void CTFPlayerShared::Burn(CTFPlayer *pAttacker) {
  ...
  if (!m_pOuter->IsAlive()) return;

  // pyros don't burn persistently or take persistent burning damage, but we
  // show brief burn effect so attacker can tell they hit
  bool bVictimIsPyro =
      (TF_CLASS_PYRO == m_pOuter->GetPlayerClass()->GetClassIndex());
  ...
  float flFlameLife =
      bVictimIsPyro ? TF_BURNING_FLAME_LIFE_PYRO : TF_BURNING_FLAME_LIFE;
...
void CTFPlayerShared::OnAddStealthed(void) {
  ...
  m_flInvisChangeCompleteTime =
      gpGlobals->curtime + tf_spy_invis_time.GetFloat();
  ...
  for (i = 0; i < m_pOuter->WeaponCount(); i++) {
    ...
    if (pWpn->GetWeaponID() != TF_WEAPON_INVIS) continue;
...
void CTFPlayerShared::OnAddBurning(void) {
  ...
  if (!m_pOuter->m_pBurningEffect) {
    const char *pEffectName = (m_pOuter->GetTeamNumber() == TF_TEAM_RED)
                                  ? "burningplayer_red"
                                  : "burningplayer_blue";
    m_pOuter->m_pBurningEffect =
        m_pOuter->ParticleProp()->Create(pEffectName, PATTACH_ABSORIGIN_FOLLOW);

    m_pOuter->m_flBurnEffectStartTime = gpGlobals->curtime;
    m_pOuter->m_flBurnEffectEndTime =
        gpGlobals->curtime + TF_BURNING_FLAME_LIFE;
  }
  // set the burning screen overlay
  if (m_pOuter->IsLocalPlayer()) {
    IMaterial *pMaterial = materials->FindMaterial(
        "effects/imcookin", TEXTURE_GROUP_CLIENT_EFFECTS, false);
    if (!IsErrorMaterial(pMaterial)) {
      view->SetScreenOverlayMaterial(pMaterial);
    }
  }
...
//-----------------------------------------------------------------------------
void CTFPlayerShared::SetPlayerDominatingMe(CTFPlayer *pPlayer,
                                            bool bDominated) {
  int iPlayerIndex = pPlayer->entindex();
...
bool CTFPlayerShared::IsPlayerDominated(int iPlayerIndex) {
  ...
  // As a result, it's only valid to ask for dominations related to the local
  // player
...
void CTFPlayerShared::FadeInvis(float flInvisFadeTime) {
  ...
  if (flInvisFadeTime < 0.15) {
    ...
    m_flStealthNoAttackExpire =
        gpGlobals->curtime + tf_spy_cloak_no_attack_time.GetFloat();
  }
...
void CTFPlayerShared::InvisibilityThink(void) {
  ...
  if (InCond(TF_COND_STEALTHED_BLINK)) {
    ...
    flTargetInvisScale =
        TF_SPY_STEALTH_BLINKSCALE; /*tf_spy_stealth_blink_scale.GetFloat();*/
  }
  ...
  if (m_flInvisChangeCompleteTime > gpGlobals->curtime) {
    if (InCond(TF_COND_STEALTHED)) {
      flTargetInvis =
          1.0f - ((m_flInvisChangeCompleteTime - gpGlobals->curtime));
    } else {
      flTargetInvis =
          ((m_flInvisChangeCompleteTime - gpGlobals->curtime) * 0.5f);
    }
  } else {
    if (InCond(TF_COND_STEALTHED)) {
      flTargetInvis = 1.0f;
...
void CTFPlayerShared::FindDisguiseTarget(void) {
  ...
  if (m_hDisguiseTarget) {
    ...
    Assert(m_iDisguiseTargetIndex >= 1 &&
           m_iDisguiseTargetIndex <= MAX_PLAYERS);
  } else {
...
void CTFPlayerShared::RecalcDisguiseWeapon(void) {
  ...
  for (i = 0; i < TF_PLAYER_WEAPON_COUNT; i++) {
    if (pData->m_aWeapons[i] != TF_WEAPON_NONE) {
      ...
      CTFWeaponInfo *pWeaponInfo =
          dynamic_cast<CTFWeaponInfo *>(GetFileWeaponInfoFromHandle(hWpnInfo));
  ...
  Assert(pDisguiseWeaponInfo != NULL &&
         "Cannot find slot 0 primary weapon for desired disguise class\n");

  m_pDisguiseWeaponInfo = pDisguiseWeaponInfo;
  m_iDisguiseWeaponModelIndex = -1;

  if (pDisguiseWeaponInfo) {
    m_iDisguiseWeaponModelIndex =
        modelinfo->GetModelIndex(pDisguiseWeaponInfo->szWorldModel);
  }
}
...
//-----------------------------------------------------------------------------
void CTFPlayerShared::Heal(CTFPlayer *pPlayer, float flAmount,
                           bool bDispenserHeal /* = false */) {
...
bool CTFPlayerShared::IsProvidingInvuln(CTFPlayer *pPlayer) {
  if (!pPlayer->IsPlayerClass(TF_CLASS_MEDIC)) return false;

  CTFWeaponBase *pWpn = pPlayer->GetActiveTFWeapon();
  if (!pWpn) return false;

  CWeaponMedigun *pMedigun = dynamic_cast<CWeaponMedigun *>(pWpn);
  if (pMedigun && pMedigun->IsReleasingCharge()) return true;
...
void CTFPlayerShared::RecalculateInvuln(bool bInstantRemove) {
  ...
  if (!m_pOuter->HasTheFlag()) {
    if (IsProvidingInvuln(m_pOuter)) {
      ...
      for (int i = 0; i < m_aHealers.Count(); i++) {
        if (!m_aHealers[i].pPlayer) continue;
...
void CTFPlayerShared::SetInvulnerable(bool bState, bool bInstant) {
  ...
  if (bState) {
    ...
    CSingleUserRecipientFilter filter(m_pOuter);
    m_pOuter->EmitSound(filter, m_pOuter->entindex(),
    ...
    if (bInstant) {
      ...
      if (m_flInvulnerableOffTime) return;
...
int CTFPlayerShared::FindHealerIndex(CTFPlayer *pPlayer) {
  for (int i = 0; i < m_aHealers.Count(); i++) {
    if (m_aHealers[i].pPlayer == pPlayer) return i;
  }
...
EHANDLE CTFPlayerShared::GetFirstHealer() {
  if (m_aHealers.Count() > 0) return m_aHealers.Head().pPlayer;
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayerShared::SetJumping(bool bJumping) { m_bJumping = bJumping; }

void CTFPlayerShared::SetAirDash(bool bAirDash) { m_bAirDash = bAirDash; }
...
void CTFPlayerShared::UpdateCritMult(void) {
  ...
  // Go through the damage multipliers and remove expired ones, while summing
  ...
  for (int i = m_DamageEvents.Count() - 1; i >= 0; i--) {
    ...
    if (flDelta > tf_damage_events_track_for.GetFloat()) {
      ...
      // m_DamageEvents[i].flTime, gpGlobals->curtime );
      m_DamageEvents.Remove(i);
      continue;
    }

    // Ignore damage we've just done. We do this so that we have time to get
    // those damage events to the client in time for using them in prediction in
    // this code.
    ...
    if (flDelta > TF_DAMAGE_CRITMOD_MAXTIME) continue;

    // Msg( "      Added %.2f (%d: time %.2f, now %.2f)\n",
    // m_DamageEvents[i].flDamage, i, m_DamageEvents[i].flTime,
    // gpGlobals->curtime );

    flTotalDamage += m_DamageEvents[i].flDamage;
  }

  float flMult = RemapValClamped(flTotalDamage, 0, TF_DAMAGE_CRITMOD_DAMAGE,
                                 flMinMult, flMaxMult);
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayerShared::RecordDamageEvent(const CTakeDamageInfo &info,
                                        bool bKill) {
...
int CTFPlayerShared::GetNumKillsInTime(float flTime) {
  if (tf_damage_events_track_for.GetFloat() < flTime) {
    Warning(
        "Player asking for damage events for time %.0f, but "
        "tf_damage_events_track_for is only tracking events for %.0f\n",
...
//-----------------------------------------------------------------------------
void CTFPlayer::FireBullet(const FireBulletsInfo_t &info, bool bDoEffects,
                           int nDamageType,
  ...
  trace_t trace;
  UTIL_TraceLine(vecStart, vecEnd, (MASK_SOLID | CONTENTS_HITBOX), this,
  ...
  if (trace.fraction < 1.0) {
    ...
    if (bDoEffects) {
      ...
      if (!(enginetrace->GetPointContents(trace.startpos) &
            (CONTENTS_WATER | CONTENTS_SLIME)) &&
          (enginetrace->GetPointContents(trace.endpos) &
           (CONTENTS_WATER | CONTENTS_SLIME))) {
      ...
      if ((info.m_iTracerFreq != 0) &&
          (tracerCount++ % info.m_iTracerFreq) == 0) {
        ...
        } else if (pLocalPlayer && pLocalPlayer->GetObserverTarget() == this &&
        ...
        if (tf_useparticletracers.GetBool()) {
          ...
          if (pszTracerEffect && pszTracerEffect[0]) {
            ...
            if (nDamageType & DMG_CRITICAL) {
              Q_snprintf(szTracerEffect, sizeof(szTracerEffect), "%s_crit",
                         pszTracerEffect);
              pszTracerEffect = szTracerEffect;
            }

            UTIL_ParticleTracer(pszTracerEffect, vecStart, trace.endpos,
                                entindex(), iUseAttachment, true);
          }
        } else {
          UTIL_Tracer(vecStart, trace.endpos, entindex(), iUseAttachment, 5000,
                      true, GetTracerType());
        }
      }
#endif
    ...
    CTakeDamageInfo dmgInfo(this, info.m_pAttacker, info.m_iDamage,
                            nDamageType);
    dmgInfo.SetDamageCustom(nCustomDamageType);
    CalculateBulletDamageForce(&dmgInfo, info.m_iAmmoType,
                               info.m_vecDirShooting, trace.endpos,
                               1.0);  // MATTTODO bullet forces
...
#ifdef CLIENT_DLL
static ConVar tf_impactwatertimeenable("tf_impactwatertimeenable", "0",
                                       FCVAR_CHEAT,
                                       "Draw impact debris effects.");
static ConVar tf_impactwatertime("tf_impactwatertime", "1.0f", FCVAR_CHEAT,
                                 "Draw impact debris effects.");
#endif
...
void CTFPlayer::ImpactWaterTrace(trace_t &trace, const Vector &vecStart) {
  ...
  if (tf_impactwatertimeenable.GetBool()) {
    if (m_flWaterImpactTime > gpGlobals->curtime) return;
  }
#endif

  trace_t traceWater;
  UTIL_TraceLine(vecStart, trace.endpos,
                 (MASK_SHOT | CONTENTS_WATER | CONTENTS_SLIME), this,
                 COLLISION_GROUP_NONE, &traceWater);
  if (traceWater.fraction < 1.0f) {
    ...
    if (pWeapon && (TF_WEAPON_MINIGUN == pWeapon->GetWeaponID())) {
      // for the minigun, use a different, cheaper splash effect because it can
...
void CTFPlayer::TeamFortress_SetSpeed() {
  ...
  if (HasItem() && GetItem()->GetItemID() == TF_ITEM_CAPTURE_FLAG) {
    ...
    if (pFlag) {
      if (pFlag->GetGameType() == TF_FLAGTYPE_ATTACK_DEFEND ||
          pFlag->GetGameType() == TF_FLAGTYPE_TERRITORY_CONTROL) {
        maxfbspeed *= 0.5;
      }
    }
  }
  ...
  if (m_Shared.InCond(TF_COND_AIMING)) {
    ...
    if (playerclass == TF_CLASS_PYRO) {
      if (maxfbspeed > 200) maxfbspeed = 200;
    } else {
      if (maxfbspeed > 80) maxfbspeed = 80;
    }
  }
...
//-----------------------------------------------------------------------------
// Purpose: Return true if this player's allowed to build another one of the
// specified object
//-----------------------------------------------------------------------------
int CTFPlayer::CanBuild(int iObjectType) {
  if (iObjectType < 0 || iObjectType >= OBJ_LAST) return CB_UNKNOWN_OBJECT;
...
//-----------------------------------------------------------------------------
// Purpose: Return true if we should record our last weapon when switching
// between the two specified weapons
//-----------------------------------------------------------------------------
bool CTFPlayer::Weapon_ShouldSetLast(CBaseCombatWeapon *pOldWeapon,
...
// Purpose:
//-----------------------------------------------------------------------------
void CTFPlayer::SetStepSoundTime(stepsoundtimes_t iStepSoundTime,
                                 bool bWalking) {
  float flMaxSpeed = MaxSpeed();
...
bool CTFPlayer::CanAttack(void) {
  ...
  if (m_Shared.GetStealthNoAttackExpireTime() > gpGlobals->curtime ||
      m_Shared.InCond(TF_COND_STEALTHED)) {
#ifdef CLIENT_DLL
  ...
  if ((pRules->State_Get() == GR_STATE_TEAM_WIN) &&
...
bool CTFPlayer::DoClassSpecialSkill(void) {
  ...
  switch (GetPlayerClass()->GetClassIndex()) {
      ...
      if (m_Shared.m_flStealthNextChangeTime <= gpGlobals->curtime) {
        ...
        if (m_Shared.InCond(TF_COND_STEALTHED)) {
          ...
          bDoSkill = true;
                   ...
                   (m_Shared.GetSpyCloakMeter() >
                    8.0f))  // must have over 10% cloak to start
        {
    ...
    case TF_CLASS_DEMOMAN: {
      CTFPipebombLauncher *pPipebombLauncher =
