...
struct RoundStats_t {
  ...
  void Reset() {
    for (usize i = 0; i < std::size(m_iStat); i++) {
      m_iStat[i] = 0;
    }
  };
...
struct TF_Gamestats_LevelStats_t {
 public:
  ...
  // Level start and end
  void Init(const char *pszMapName, int nIPAddr, short nPort,
            float flStartTime);
  ...
  void Accumulate(TF_Gamestats_LevelStats_t *pOther) {
    ...
    for (usize i = 0; i < std::size(m_aClassStats); i++) {
      m_aClassStats[i].Accumulate(pOther->m_aClassStats[i]);
    }
    for (usize i = 0; i < std::size(m_aWeaponStats); i++) {
 ...
 public:
  ...
  struct LevelHeader_t {
    ...
    unsigned int
        m_nIPAddr;  // IP Address of the server - 4 bytes stored as an int.
  ...
  struct PlayerDeathsLump_t {
    ...
    u8 iAttackClass;           // Class that killed the player.
    u8 iTargetClass;           // Class of the player killed.
  };
  ...
  struct PlayerDamageLump_t {
    ...
    u8 iAttackClass;             // Class of the attacker
    u8 iTargetClass;             // Class of the target
    u8 iCrit;                    // was the shot a crit?
    u8 iKill;                    // did the shot kill the target?
  };
  ...
  TF_Gamestats_ClassStats_t
      m_aClassStats[TF_CLASS_COUNT_ALL];  // Vector of class data
  TF_Gamestats_WeaponStats_t
      m_aWeaponStats[TF_WEAPON_COUNT];  // Vector of weapon data
  // Temporary data.
...
struct KillStats_t {
  ...
  int iNumKilled[MAX_PLAYERS + 1];    // how many times this player has killed
                                      // every other player
  int iNumKilledBy[MAX_PLAYERS + 1];  // how many times this player has been
                                      // killed by every other player
  int iNumKilledByUnanswered[MAX_PLAYERS + 1];  // how many unanswered kills
                                                // this player has been dealt by
                                                // every other player
};
...
// reported stats structure that contains all stats data uploaded from TF server
// to Steam.  Note that this code is shared between TF server and
// processgamestats, which cracks the data file on the back end
