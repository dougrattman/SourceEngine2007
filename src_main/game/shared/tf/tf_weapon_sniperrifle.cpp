...
void CTFSniperRifle::ItemPostFrame( void )
{
	...
	else if ( m_flNextSecondaryAttack <= gpGlobals->curtime )
	{
		...
		{
			m_flChargedDamage = std::min( m_flChargedDamage + gpGlobals->frametime * TF_WEAPON_SNIPERRIFLE_CHARGE_PER_SEC, TF_WEAPON_SNIPERRIFLE_DAMAGE_MAX );
		}
		else
		{
			m_flChargedDamage = std::max( 0, m_flChargedDamage - gpGlobals->frametime * TF_WEAPON_SNIPERRIFLE_UNCHARGE_PER_SEC );
		}
	}
...
void CTFSniperRifle::Zoom( void )
{
	...
	m_flNextPrimaryAttack = std::max( m_flNextPrimaryAttack, gpGlobals->curtime + 0.1 );
...
float CTFSniperRifle::GetProjectileDamage( void )
{
	...
	return std::max( m_flChargedDamage, TF_WEAPON_SNIPERRIFLE_DAMAGE_MIN );
}
