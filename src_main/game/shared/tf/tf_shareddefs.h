...
// Teams.
//-----------------------------------------------------------------------------
enum { TF_TEAM_RED = LAST_SHARED_TEAM + 1, TF_TEAM_BLUE, TF_TEAM_COUNT };
...
enum {
  ...
  TF_CLASS_CIVILIAN,  // Civilians are a special class. It is not a player
                      // class.
  TF_CLASS_COUNT_ALL,
...
extern const char
    *g_aPlayerClassNames_NonLocalized[];  // non-localized class names
...
//-----------------------------------------------------------------------------
enum {
  TF_AMMO_DUMMY = 0,  // Dummy index to make the CAmmoDef indices correct for
                      // the other ammo types.
  TF_AMMO_PRIMARY,
  TF_AMMO_SECONDARY,
  TF_AMMO_METAL,
  TF_AMMO_GRENADES1,
...
#define TF_BURNING_FLAME_LIFE_PYRO \
  0.25  // pyro only displays burning effect momentarily
...
enum {
  ...
  // If you add a new condition that shouldn't have this behavior, add it before
  // this section.
  TF_COND_HEALTH_BUFF,
  TF_COND_BURNING,
...
#define TF_TIME_ASSIST_KILL \
  3.0f  // Time window for a recent damager to get credit for an assist for a
        // kill
#define TF_TIME_SUICIDE_KILL_CREDIT \
  10.0f  // Time window for a recent damager to get credit for a kill if target
         // suicides
...
#define TFSTATE_ALTKILL \
  0x000004  // #TRUE if killed with a weapon not in self.weapon: NOT USED
            // ANYMORE
...
// This can only ever be used on a TakeHealth call, since it re-uses a dmg flag
...
enum {
  ...
  TFCOLLISION_GROUP_ROCKETS,  // Solid to players, but not player movement.
                              // ensures touch calls are originating from rocket
...
//--------------------------------------------------------------------------
enum { BS_IDLE = 0, BS_SELECTING, BS_PLACING, BS_PLACING_INVALID };
...
enum {
  ...
  CB_LIMIT_REACHED,   // Player has reached the limit of the number of these
                      // objects allowed
  CB_NEED_RESOURCES,  // Player doesn't have enough resources to build this
                      // object
  CB_NEED_ADRENALIN,  // Commando doesn't have enough adrenalin to build a rally
                      // flag
...
class CObjectInfo {
 public:
  ...
  char *m_pBuilderWeaponName;       // Names shown for each object onscreen when
                                    // using the builder weapon
  char *m_pBuilderPlacementString;  // String shown to player during placement
                                    // of this object
  ...
  int m_iDisplayPriority;  // Priority for ordering in the hud display ( higher
                           // is closer to top )
  bool m_bVisibleInWeaponSelection;  // should show up and be selectable via the
                                     // weapon selection?
  char *m_pExplodeSound;             // gamesound to play when object explodes
  char *m_pExplosionParticleEffect;  // particle effect to play when object
                                     // explodes
  bool m_bAutoSwitchTo;  // should we let players switch back to the builder
                         // weapon representing this?
...
int CalculateObjectCost(
    int iObjectType /*, int iNumberOfObjects, int iTeam, bool bLast = false*/);
