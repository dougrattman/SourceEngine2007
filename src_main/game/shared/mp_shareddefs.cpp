...
int GetMPConceptIndexFromString(const char *pszConcept) {
  ...
  static_assert(SOURCE_ARRAYSIZE(g_pszMPConcepts) == MP_TF_CONCEPT_COUNT);

  for (int iConcept = 0; iConcept < SOURCE_ARRAYSIZE(g_pszMPConcepts);
       ++iConcept) {
    if (!Q_stricmp(pszConcept, g_pszMPConcepts[iConcept])) return iConcept;
  }
