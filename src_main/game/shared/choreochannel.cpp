...
void CChoreoChannel::ReconcileGestureTimes() {
  ...
  for (i = sortedGestures.FirstInorder(); i != sortedGestures.InvalidIndex();
       i = sortedGestures.NextInorder(i)) {
    ...
    if (!previous) {
      ...
      CEventAbsoluteTag *pEntryTag =
          event->FindEntryTag(CChoreoEvent::PLAYBACK);
      CEventAbsoluteTag *pExitTag =
          previous->FindExitTag(CChoreoEvent::PLAYBACK);
      ...
      if (pEntryTag && pExitTag) {
        ...
        // adjust the previous gestures end time to current apex + existing
        // decay rate
        previous->RescaleGestureTimes(previous->GetStartTime(),
                                      entryTime + decayTime, true);
      ...
      // -------------------------
      // When copying and pasteing posture and gesture clips in face poser the
      // beginings of the clips stretch
      //  to the begining of the scene even if there is a 0 gesture in place at
      //  the begining.
      // -------------------------
      /*
      else if ( pEntryTag && !Q_stricmp( previous->GetName(), "NULL" ) )
      {
              // If the previous was a 0 event, then do a bit of fixup
      ...
      */
      ...
      // The previous event decays from it's end dispaly end time to the current
      // event's display start time The next event starts just after the display
  ...
  if (previous) {
    ...
    if (scene) {
      // HACK:  Could probably do better by allowing user to drag the blue "end
      // time" bar
  ...
  /*
  c = 0;
  for ( i = sortedGestures.FirstInorder(); i != sortedGestures.InvalidIndex(); i
  = sortedGestures.NextInorder( i ) )
  {
          CChoreoEvent *event = sortedGestures[ i ];
...
struct EventGroup {
  EventGroup() : timeSortedEvents(0, 0, ChoreEventStartTimeLessFunc) {}
...
bool CChoreoChannel::GetSortedCombinedEventList(
    char const *cctoken, CUtlRBTree<CChoreoEvent *> &events) {
  events.RemoveAll();
  ...
  for (i = 0; i < c; i++) {
    ...
    if (Q_stricmp(token, cctoken)) continue;
...
bool CChoreoChannel::RestoreFromBuffer(CUtlBuffer &buf, CChoreoScene *pScene,
                                       CChoreoActor *pActor,
                                       IChoreoStringPool *pStringPool) {
  char sz[256];
