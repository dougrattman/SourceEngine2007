...
LINK_ENTITY_TO_CLASS(
    env_glow,
    CSprite);  // For backwards compatibility, remove when no longer needed.
#endif
...
BEGIN_DATADESC(CSprite)
  ...
  DEFINE_FIELD(m_flLastTime, FIELD_TIME),
      ...
      // Function Pointers
      DEFINE_FUNCTION(AnimateThink), DEFINE_FUNCTION(ExpandThink),
      DEFINE_FUNCTION(AnimateUntilDead), DEFINE_FUNCTION(BeginFadeOutThink),
...
DEFINE_PRED_FIELD(m_hAttachedToEntity, FIELD_EHANDLE, FTYPEDESC_INSENDTABLE),
    ...
    //	DEFINE_FIELD( m_flDestScale, FIELD_FLOAT ),
    ////Destination scale 	DEFINE_FIELD( m_flScaleTimeStart, FIELD_FLOAT ),
    ////Real time for start of scale 	DEFINE_FIELD( m_nStartBrightness,
    // FIELD_INTEGER ),
    ////Starting brightness 	DEFINE_FIELD( m_nDestBrightness, FIELD_INTEGER
    ///), /Destination brightness 	DEFINE_FIELD( m_flBrightnessTimeStart,
    // FIELD_FLOAT ),	//Real time for brightness
...
static void RecvProxy_SpriteScale(const CRecvProxyData *pData, void *pStruct,
                                  void *pOut) {
...
#else
    SendPropFloat(SENDINFO(m_flSpriteScale), 8, SPROP_ROUNDUP, 0.0f,
                  MAX_SPRITE_SCALE),
#endif
    SendPropFloat(SENDINFO(m_flGlowProxySize), 6, SPROP_ROUNDUP, 0.0f,
                  MAX_GLOW_PROXY_SIZE),
...
#endif
    scale =
        std::clamp(static_cast<float>(m_flSpriteScale), 0.0f, MAX_SPRITE_SCALE);
  }
...
// Purpose: Initialize absmin & absmax to the appropriate box
//-----------------------------------------------------------------------------
void CSprite::ComputeWorldSpaceSurroundingBox(Vector *pVecWorldMins,
...
int CSprite::ShouldTransmit(const CCheckTransmitInfo *pInfo) {
  ...
  if (GetMoveParent()) {
    CBaseViewModel *pViewModel =
        dynamic_cast<CBaseViewModel *>(GetMoveParent());
...
// Output : CSprite
//-----------------------------------------------------------------------------
CSprite *CSprite::SpriteCreatePredictable(const char *module, int line,
                                          const char *pSpriteName,
                                          const Vector &origin, bool animate) {
  CSprite *pSprite = (CSprite *)CBaseEntity::CreatePredictedEntityByName(
      "env_sprite", module, line);
  if (pSprite) {
    pSprite->SpriteInit(pSpriteName, origin);
    pSprite->SetSolid(SOLID_NONE);
...
#endif
    {
      if (m_flMaxFrame > 0) m_flFrame = fmod(m_flFrame, m_flMaxFrame);
    }
  }
}
...
void CSprite::SetBrightness(int brightness, float time) {
  m_nBrightness =
      brightness;  // Take our current position as our starting position
...
void CSprite::SetSpriteScale(float scale) {
  if (scale != m_flSpriteScale) {
    m_flSpriteScale =
        scale;  // Take our current position as our new starting position
...
void CSprite::Use(CBaseEntity *pActivator, CBaseEntity *pCaller,
...
//-----------------------------------------------------------------------------
void CSprite::InputHideSprite(inputdata_t &inputdata) { TurnOff(); }

//-----------------------------------------------------------------------------
// Purpose: Input handler that hides the sprite.
//-----------------------------------------------------------------------------
void CSprite::InputShowSprite(inputdata_t &inputdata) { TurnOn(); }
...
float CSprite::GetRenderScale(void) {
  ...
  if ((m_flScaleTime == 0) ||
      ((m_flScaleTimeStart + m_flScaleTime) < gpGlobals->curtime))
...
void CSprite::GetRenderBounds(Vector &vecMins, Vector &vecMaxs) {
  ...
  if (m_bWorldSpaceScale == false) {
    CEngineSprite *psprite =
        (CEngineSprite *)modelinfo->GetModelExtraData(GetModel());
    if (psprite) {
      float flSize = std::max(psprite->GetWidth(), psprite->GetHeight());
      flScale *= flSize;
    }
  }
...
int CSprite::GetRenderBrightness(void) {
  // See if we're done scaling
  if ((m_flBrightnessTime == 0) ||
      ((m_flBrightnessTimeStart + m_flBrightnessTime) < gpGlobals->curtime)) {
    return m_nBrightness;
  }

  // Get our percentage
  float timeDelta =
      (gpGlobals->curtime - m_flBrightnessTimeStart) / m_flBrightnessTime;

  float brightness =
      ((float)m_nStartBrightness +
       ((float)(m_nDestBrightness - m_nStartBrightness) * timeDelta));
...
int CSprite::DrawModel(int flags) {
  ...
  // Draw it
  int drawn = DrawSprite(this, GetModel(), GetAbsOrigin(), GetAbsAngles(),
...
//			CSprites swap the roll and yaw angle inputs, and rotate
// the yaw 180 degrees
//-----------------------------------------------------------------------------
...
void CSpriteOriented::Spawn(void) {
  ...
  BaseClass::Spawn();
  // ORIENTED sprites "forward" vector points in the players "view" direction,
  // not the direction "out" from the sprite (gah)
...
bool CSpriteOriented::IsTransparent(void) { return true; }
