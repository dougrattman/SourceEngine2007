...
CPlayerBitVec
    g_PlayerModEnable;  // Set to 1 for each player if the player wants to use
                        // voice in this mod. (If it's zero, then the server
                        // reports that the game rules are saying the player
                        // can't hear anyone).

CPlayerBitVec
    g_BanMasks[VOICE_MAX_PLAYERS];  // Tells which players don't want to hear
                                    // each other. These are indexed as clients
                                    // and each bit represents a client (so
                                    // player entity is bit+1).

CPlayerBitVec
    g_SentGameRulesMasks[VOICE_MAX_PLAYERS];  // These store the masks we last
                                              // sent to each client so we can
                                              // determine if
...
ConVar sv_alltalk("sv_alltalk", "0", FCVAR_NOTIFY,
...
CVoiceGameMgr *GetVoiceGameMgr() { return &g_VoiceGameMgr; }
...
bool CVoiceGameMgr::Init(IVoiceGameMgrHelper *pHelper, int maxClients) {
  m_pHelper = pHelper;
  m_nMaxPlayers =
      VOICE_MAX_PLAYERS < maxClients ? VOICE_MAX_PLAYERS : maxClients;
...
void CVoiceGameMgr::Update(double frametime) {
  ...
  if (m_UpdateInterval < UPDATE_INTERVAL) return;
...
void CVoiceGameMgr::UpdateMasks() {
  ...
  for (int iClient = 0; iClient < m_nMaxPlayers; iClient++) {
    ...
    if (g_PlayerModEnable[iClient]) {
      ...
      for (int iOtherClient = 0; iOtherClient < m_nMaxPlayers; iOtherClient++) {
        ...
        if (pEnt && pEnt->IsPlayer() &&
            (bAllTalk || m_pHelper->CanPlayerHearPlayer(
                             pPlayer, (CBasePlayer *)pEnt, bProximity))) {
    ...
    for (int iOtherClient = 0; iOtherClient < m_nMaxPlayers; iOtherClient++) {
      bool bCanHear =
          gameRulesMask[iOtherClient] && !g_BanMasks[iClient][iOtherClient];
      g_pVoiceServer->SetClientListening(iClient + 1, iOtherClient + 1,
                                         bCanHear);

      if (bCanHear) {
        g_pVoiceServer->SetClientProximity(iClient + 1, iOtherClient + 1,
                                           !!ProximityMask[iOtherClient]);
      }
    }
  }
}
...
bool CVoiceGameMgr::CheckProximity(int iDistance) {
  if (m_iProximityDistance >= iDistance) return true;
