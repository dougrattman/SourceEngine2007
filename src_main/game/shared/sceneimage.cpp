...
class CSceneImage : public ISceneImage {
 public:
  virtual bool CreateSceneImageFile(CUtlBuffer &targetBuffer,
                                    char const *pchModPath, bool bLittleEndian,
                                    bool bQuiet, ISceneCompileStatus *Status);
};
...
// the final pool contains all the unique strings seperated by a 0
...
bool CreateTargetFile_VCD(const char *pSourceName, const char *pTargetName,
  ...
  CChoreoScene *pChoreoScene =
      ChoreoLoadScene(pSourceName, NULL, &g_SceneTokenProcessor, Msg);
  if (!pChoreoScene) {
    return false;
  }
  ...
  // calc duration
  g_SceneFiles[iScene].msecs =
      (unsigned int)(pChoreoScene->FindStopTime() * 1000.0f + 0.5f);
...
class CSceneImageEntryLessFunc {
 public:
  bool Less(const SceneImageEntry_t &entryLHS,
            const SceneImageEntry_t &entryRHS, void *pCtx) {
...
bool CSceneImage::CreateSceneImageFile(CUtlBuffer &targetBuffer,
  ...
  int stringPoolStart =
      sizeof(SceneImageHeader_t) + stringOffsets.Count() * sizeof(int);
  // then directory
  int sceneEntryStart = stringPoolStart + stringPool.TellMaxPut();
  // then variable sized summaries
  int sceneSummaryStart =
      sceneEntryStart + g_SceneFiles.Count() * sizeof(SceneImageEntry_t);
  ...
  for (int i = 0; i < g_SceneFiles.Count(); i++) {
    ...
    // temp store an index to its file, fixup later, necessary to access post
    // sort
    imageEntry.nDataOffset = i;
    if (imageDirectory.Find(imageEntry) != imageDirectory.InvalidIndex()) {
      // filename checksums must be unique or runtime binary search would be
      // bogus
  ...
  for (int i = 0; i < imageDirectory.Count(); i++) {
    ...
    sceneDataStart +=
        sizeof(SceneImageSummary_t) + (numSounds - 1) * sizeof(int);
  }
  ...
  for (int i = 0; i < imageDirectory.Count(); i++) {
    ...
    if (!bLittleEndian) {
      ...
      imageDirectory[i].nSceneSummaryOffset =
          BigLong(imageDirectory[i].nSceneSummaryOffset);
    }
  ...
  for (int i = 0; i < imageDirectory.Count(); i++) {
    int iScene = writeOrder[i];
    targetBuffer.Put(g_SceneFiles[iScene].compiledBuffer.Base(),
                     g_SceneFiles[iScene].compiledBuffer.TellMaxPut());
  }
