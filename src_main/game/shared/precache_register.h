...
#define PRECACHE_REGISTER_FN(functionName) \
  static CPrecacheRegister precache_function_##functionName(functionName, 0);
#define PRECACHE_WEAPON_REGISTER(className)             \
  static CPrecacheRegister precache_weapon_##className( \
      &CPrecacheRegister::PrecacheFn_Other, #className)
#define PRECACHE_REGISTER(className)                   \
  static CPrecacheRegister precache_other_##className( \
      &CPrecacheRegister::PrecacheFn_Other, #className)

class CPrecacheRegister {
 public:
  typedef void (*PrecacheFn)(
      void *pUser);  // Prototype for a custom precache function.
