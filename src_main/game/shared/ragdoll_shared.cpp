...
void CRagdollLowViolenceManager::SetLowViolence(const char *pMapName) {
  ...
  // Turn the low violence ragdoll stuff off if we're in the HL2 Citadel maps
  // because the player has the super gravity gun and fading ragdolls will break
  // things.
...
class CRagdollAnimatedFriction : public IVPhysicsKeyHandler {
 public:
  CRagdollAnimatedFriction(ragdoll_t *ragdoll) { m_ragdoll = ragdoll; }
  virtual void ParseKeyValue(void *pData, const char *pKey,
                             const char *pValue) {
...
void RagdollSetupAnimatedFriction(IPhysicsEnvironment *pPhysEnv,
                                  ragdoll_t *ragdoll, int iModelIndex) {
...
static void RagdollAddSolid(IPhysicsEnvironment *pPhysEnv, ragdoll_t &ragdoll,
                            const ragdollparams_t &params, solid_t &solid) {
  if (solid.index >= 0 && solid.index < params.pCollide->solidCount) {
    ...
    if (boneIndex >= 0) {
      ...
      if (surfaceData < 0) surfaceData = physprops->GetSurfaceIndex("default");

      solid.params.pName = params.pStudioHdr->pszName();
      ragdoll.list[ragdoll.listCount].pObject = pPhysEnv->CreatePolyObject(
          params.pCollide->solids[solid.index], surfaceData, vec3_origin,
          vec3_angle, &solid.params);
      ragdoll.list[ragdoll.listCount].pObject->SetPositionMatrix(
          params.pCurrentBones[boneIndex], true);
...
static void RagdollAddConstraint(IPhysicsEnvironment *pPhysEnv,
                                 ragdoll_t &ragdoll,
  ...
  if (constraint.childIndex >= 0 && constraint.parentIndex >= 0) {
    ...
    // this parent/child pair is not usually a parent/child pair in the
    // skeleton.  There are often bones in between that are collapsed for
    // simulation.  So we need to compute the transform.
    ...
    MatrixGetColumn(constraint.constraintToAttached, 3,
                    childElement.originParentSpace);
    // UNDONE: We could transform the constraint limit axes relative to the bone
    // space using this data.  Do we need that feature?
    SetIdentityMatrix(constraint.constraintToReference);
    childElement.pConstraint = pPhysEnv->CreateRagdollConstraint(
        childElement.pObject, ragdoll.list[constraint.parentIndex].pObject,
        ragdoll.pGroup, constraint);
  }
}

static void RagdollCreateObjects(IPhysicsEnvironment *pPhysEnv,
                                 ragdoll_t &ragdoll,
                                 const ragdollparams_t &params) {
  ragdoll.listCount = 0;
  ragdoll.pGroup = NULL;
  ...
  IVPhysicsKeyParser *pParse =
      physcollision->VPhysicsKeyParserCreate(params.pCollide->pKeyValues);
  while (!pParse->Finished()) {
    const char *pBlock = pParse->GetCurrentBlockName();
    if (!strcmpi(pBlock, "solid")) {
      ...
      IPhysicsCollisionSet *pSet = physics->FindOrCreateCollisionSet(
          params.modelIndex, ragdoll.listCount);
...
void RagdollSetupCollisions(ragdoll_t &ragdoll, vcollide_t *pCollide,
                            int modelIndex) {
  Assert(pCollide);
  if (!pCollide) return;
    ...
    while (!pParse->Finished()) {
      ...
      if (!strcmpi(pBlock, "collisionrules")) {
        IPhysicsCollisionSet *pSet =
            physics->FindOrCreateCollisionSet(modelIndex, ragdoll.listCount);
        CRagdollCollisionRules rules(pSet);
        pParse->ParseCustom((void *)&rules, &rules);
        bFoundRules = true;
      } else {
...
void RagdollActivate(ragdoll_t &ragdoll, vcollide_t *pCollide, int modelIndex,
                     bool bForceWake) {
...
bool RagdollCreate(ragdoll_t &ragdoll, const ragdollparams_t &params,
  ...
  if (!ragdoll.listCount) return false;
  ...
  if (forcePosition != vec3_origin) {
    for (i = 0; i < ragdoll.listCount; i++) {
      if (forceBone != i) {
        ...
        ragdoll.list[i].pObject->ApplyForceOffset(scale * nudgeForce,
                                                  forcePosition);
      }
    }
  }
...
void RagdollApplyAnimationAsVelocity(ragdoll_t &ragdoll,
  ...
  for (int i = 0; i < ragdoll.listCount; i++) {
    ...
    CalcBoneDerivatives(velocity, angVel, pPrevBones[boneIndex],
                        pCurrentBones[boneIndex], dt);
...
void RagdollApplyAnimationAsVelocity(ragdoll_t &ragdoll,
  ...
  for (int i = 0; i < ragdoll.listCount; i++) {
    ...
    // move those bone-local coords back to world space using the ragdoll
    // transform
...
void RagdollDestroy(ragdoll_t &ragdoll) {
  ...
  for (i = 0; i < ragdoll.listCount; i++) {
    // during level transitions these can get temporarily loaded without physics
    // objects purely for the purpose of testing for PVS of transition.  If they
    // fail they get deleted before the physics objects are loaded.  The list
    // count will be nonzero since that is saved separately.
...
// Parse the ragdoll and obtain the mapping from each physics element index to a
// bone index returns num phys elements
int RagdollExtractBoneIndices(int *boneIndexOut, CStudioHdr *pStudioHdr,
                              vcollide_t *pCollide) {
  int elementCount = 0;
  ...
  while (!pParse->Finished()) {
    ...
    if (!strcmpi(pBlock, "solid")) {
      ...
      if (elementCount < RAGDOLL_MAX_ELEMENTS) {
        boneIndexOut[elementCount] =
            Studio_BoneIndexByName(pStudioHdr, solid.name);
        elementCount++;
      }
    } else {
      pParse->SkipBlock();
...
bool RagdollGetBoneMatrix(const ragdoll_t &ragdoll, CBoneAccessor &pBoneToWorld,
                          int objectIndex) {
  ...
  // during restore if a model has changed since the file was saved, this could
  // be NULL
  ...
  if (element.parentIndex >= 0 && !ragdoll.allowStretch) {
    ...
    // UNDONE: If we support other types of constraints (or multiple constraints
    // per object) make sure these don't fight !
    int parentBoneIndex = ragdoll.boneIndex[element.parentIndex];
    Vector out;
    VectorTransform(element.originParentSpace,
                    pBoneToWorld.GetBone(parentBoneIndex), out);
    MatrixSetColumn(out, 3, pBoneToWorld.GetBoneForWrite(boneIndex));
  }
  return true;
}

void RagdollComputeExactBbox(const ragdoll_t &ragdoll, const Vector &origin,
                             Vector &outMins, Vector &outMaxs) {
  outMins = origin;
  outMaxs = origin;
  ...
  for (int i = 0; i < ragdoll.listCount; i++) {
    ...
    physcollision->CollideGetAABB(&mins, &maxs, pObject->GetCollide(),
                                  objectOrg, objectAng);
    for (int j = 0; j < 3; j++) {
      if (mins[j] < outMins[j]) {
        outMins[j] = mins[j];
...
void RagdollSolveSeparation(ragdoll_t &ragdoll, CBaseEntity *pEntity) {
  ...
  for (int i = 0; i < ragdoll.listCount; i++) {
    ...
    if (element.pConstraint && element.parentIndex >= 0) {
      ...
      ragdoll.list[element.parentIndex].pObject->LocalToWorld(
          &target, element.originParentSpace);
      ...
      if (dir.LengthSqr() > 1.0f) {
        // this fixes a bug in ep2 with antlion grubs, but causes problems in
        // TF2 - revisit, but disable for TF now
#if !defined(TF_CLIENT_DLL)
        // heuristic: guess that anything separated and small mass ratio is in
        // some state that's keeping the solver from fixing it
        ...
        if (mass * 2.0f < massParent) {
          // if this is <0.5 mass of parent and still separated it's attached to
          // something heavy or in a bad state
          needsFix[i] = 1;
          ++fixCount;
          continue;
        }
  ...
  if (fixCount) {
    for (int i = 0; i < ragdoll.listCount; i++) {
      ...
      ragdoll.list[element.parentIndex].pObject->LocalToWorld(
          &target, element.originParentSpace);
      ragdoll.list[element.parentIndex].pObject->GetVelocityAtPoint(target,
                                                                    &velocity);
      matrix3x4_t xform;
...
#endif
ConVar g_debug_ragdoll_removal("g_debug_ragdoll_removal", "0",
...
bool ShouldRemoveThisRagdoll(CBaseAnimating *pRagdoll) {
  ...
  /* we no longer ignore enemies just because they are on fire -- a ragdoll in
front of me is always a higher priority for retention than a flaming zombie
behind me. At the time I put this in, the ragdolls do clean up their own effects
if culled via SUB_Remove(). If you're encountering trouble with ragdolls leaving
effects behind, try renabling the code below.
/////////////////////
  ...
  */
  ...
  if (engine->IsBoxInViewCluster(vMins + origin, vMaxs + origin) == false) {
    if (g_debug_ragdoll_removal.GetBool()) {
      debugoverlay->AddBoxOverlay(origin, vMins, vMaxs, QAngle(0, 0, 0), 0, 255,
                                  0, 16, 5);
      debugoverlay->AddLineOverlay(origin, origin + Vector(0, 0, 64), 0, 255, 0,
                                   true, 5);
    }
    ...
    if (g_debug_ragdoll_removal.GetBool()) {
      debugoverlay->AddBoxOverlay(origin, vMins, vMaxs, QAngle(0, 0, 0), 0, 0,
                                  255, 16, 5);
      debugoverlay->AddLineOverlay(origin, origin + Vector(0, 0, 64), 0, 0, 255,
                                   true, 5);
    }
  ...
  if (!UTIL_FindClientInPVS(pRagdoll->edict())) {
    if (g_debug_ragdoll_removal.GetBool())
      NDebugOverlay::Line(pRagdoll->GetAbsOrigin(),
                          pRagdoll->GetAbsOrigin() + Vector(0, 0, 64), 0, 255,
                          0, true, 5);
    ...
    if (g_debug_ragdoll_removal.GetBool())
      NDebugOverlay::Line(pRagdoll->GetAbsOrigin(),
                          pRagdoll->GetAbsOrigin() + Vector(0, 0, 64), 0, 0,
                          255, true, 5);
...
void CRagdollLRURetirement::Update(float frametime)  // EPISODIC VERSION
{
  ...
  // First, find ragdolls that are good candidates for deletion because they are
  // not visible at all, or are in a culled visibility box
  ...
  if (pPlayer &&
      m_LRU.Count() > iMaxRagdollCount)  // find the furthest one algorithm
  {
    ...
    for (i = m_LRU.Head(); i < m_LRU.InvalidIndex(); i = next) {
      ...
      IPhysicsObject *pObject = pRagdoll->VPhysicsGetObject();
      if (pRagdoll &&
          (pRagdoll->GetEffectEntity() || (pObject && !pObject->IsAsleep())))
        continue;

      if (pRagdoll) {
        // float distToPlayer = (pPlayer->GetAbsOrigin() -
        // pRagdoll->GetAbsOrigin()).LengthSqr();
        float distToPlayer =
            (PlayerOrigin - pRagdoll->GetAbsOrigin()).LengthSqr();
  ...
  {
    for (i = m_LRU.Head(); i < m_LRU.InvalidIndex(); i = next) {
      if (m_LRU.Count() <= iMaxRagdollCount) break;
...
void CRagdollLRURetirement::Update(float frametime)  // Non-episodic version
{
  ...
  for (i = m_LRU.Head(); i < m_LRU.InvalidIndex(); i = next) {
    ...
    // Just ignore it until we're done burning/dissolving.
    if (pRagdoll && pRagdoll->GetEffectEntity()) continue;
...
// This is pretty hacky, it's only called on the server so it just calls the
// update method.
void CRagdollLRURetirement::FrameUpdatePostEntityThink(void) { Update(0); }

ConVar g_ragdoll_important_maxcount("g_ragdoll_important_maxcount", "2",
                                    FCVAR_REPLICATED);
...
void CRagdollLRURetirement::MoveToTopOfLRU(CBaseAnimating *pRagdoll,
  ...
  if (bImportant) {
    ...
    if (m_LRUImportantRagdolls.Count() >
        g_ragdoll_important_maxcount.GetInt()) {
...
C_EntityDissolve *DissolveEffect(C_BaseAnimating *pTarget, float flTime) {
  ...
  if (pDissolve->InitializeAsClientEntity(
          "sprites/blueglow1.vmt", RENDER_GROUP_TRANSLUCENT_ENTITY) == false) {
    pDissolve->Release();
    return NULL;
  }
...
C_EntityFlame *FireEffect(C_BaseAnimating *pTarget, C_BaseEntity *pServerFire,
  ...
  if (pFire->InitializeAsClientEntity(NULL, RENDER_GROUP_TRANSLUCENT_ENTITY) ==
      false) {
    pFire->Release();
    return NULL;
  }
  ...
  if (pFire != NULL) {
    ...
    pTarget->EmitSound(filter, pTarget->GetSoundSourceIndex(),
...
void C_BaseAnimating::IgniteRagdoll(C_BaseAnimating *pSource) {
  ...
  if (pChild) {
    ...
    if (pFireChild) {
      pRagdoll->SetEffectEntity(
          FireEffect(pRagdoll, pFireChild, NULL, NULL, NULL));
    }
  }
}
  ...
  if (pChild) {
    ...
    if (pDissolveChild) {
      ...
      if (pRagdoll) {
        ...
        C_EntityDissolve *pDissolve =
            DissolveEffect(pRagdoll, pRagdoll->m_flEffectTime);
...
void CBaseAnimating::TransferDissolveFrom(CBaseAnimating *pAnim) {
  if (!pAnim || !pAnim->IsDissolving()) return;
  ...
  if (pDissolve) {
    ...
    CEntityDissolve *pDissolveFrom =
        dynamic_cast<CEntityDissolve *>(pAnim->GetEffectEntity());
