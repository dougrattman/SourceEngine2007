...
//-----------------------------------------------------------------------------
// Game systems are singleton objects in the client + server codebase
// responsible for various tasks The order in which the server systems appear in
// this list are the order in which they are initialized and updated. They are
// shut down in reverse order from which they are initialized.
//-----------------------------------------------------------------------------
...
abstract_class IGameSystem {
 public:
  ...
  // Called every frame. It's safe to remove an igamesystem from within this
  // callback.
...
class CAutoGameSystem : public CBaseGameSystem {
 public:
  CAutoGameSystem(char const *name =
                      NULL);  // hooks in at startup, no need to explicitly add
