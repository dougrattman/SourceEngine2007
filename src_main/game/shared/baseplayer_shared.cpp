...
void CBasePlayer::ItemPreFrame() {
  ...
  if (!GetActiveWeapon()->IsPredicted()) return;
#endif
...
#ifdef PORTAL
  // In Portal we MUST play footstep sounds even when the player is moving very
  // slowly This is used to count the number of footsteps they take in the
  // challenge mode -Jeep
...
void CBasePlayer::PlayStepSound(Vector &vecOrigin, surfacedata_t *psurface,
  ...
  if (gpGlobals->maxClients > 1 && !sv_footsteps.GetFloat()) return;

#if defined(CLIENT_DLL)
  // during prediction play footstep sounds only once
  if (prediction->InPrediction() && !prediction->IsFirstTimePredicted()) return;
#endif
  ...
  if (m_StepSoundCache[nSide].m_usSoundNameIndex == stepSoundName) {
    ...
    // Only cache if there's one option.  Otherwise we'd never here any other
    // sounds
    if (params.count == 1) {
...
void CBasePlayer::UpdateButtonState(int nUserCmdButtonMask) {
  ...
  m_afButtonPressed =
      buttonsChanged & m_nButtons;  // The changed ones still down are "pressed"
  m_afButtonReleased =
...
bool CBasePlayer::Weapon_Switch(CBaseCombatWeapon *pWeapon,
  ...
  if (BaseClass::Weapon_Switch(pWeapon, viewmodelindex)) {
    ...
    Assert(pViewModel);
    if (pViewModel) pViewModel->RemoveEffects(EF_NODRAW);
    ResetAutoaim();
    return true;
  }
  return false;
...
void CBasePlayer::SelectLastItem(void) {
  if (m_hLastWeapon.Get() == NULL) return;
...
void CBasePlayer::SelectItem(const char *pstr, int iSubType) {
  ...
  if (GetActiveWeapon()) {
    if (!GetActiveWeapon()->CanHolster()) return;
...
CBaseEntity *CBasePlayer::FindUseEntity() {
  ...
  // A button, etc. can be made out of clip brushes, make sure it's +useable via
  // a traceline, too.
  ...
  //							forward, 45 deg, 30 deg, 20 deg, 15 deg, 10 deg,
  //-10, -15
  ...
  for (int i = 0; i < NUM_TANGENTS; i++) {
    if (i == 0) {
      ...
      UTIL_TraceHull(searchCenter, searchCenter + down * 72,
                     -Vector(16, 16, 16), Vector(16, 16, 16), useableContents,
    ...
    if (bUsable) {
      ...
      delta.z =
          IntervalDistance(tr.endpos.z, centerZ + CollisionProp()->OBBMins().z,
  ...
  // if you've got a useable ground entity, then shrink the cone of this search
  // to 45 degrees otherwise, search out in a 90 degree cone (hemisphere)
  if (GetGroundEntity() &&
  ...
  if (!pNearest) {
    ...
    // Check to see if the player is trying to select an NPC through a rail,
    // fence, or other 'see-though' volume.
    trace_t trAllies;
...
void CBasePlayer::CalcViewRoll(QAngle &eyeAngles) {
  if (GetMoveType() == MOVETYPE_NOCLIP) return;
...
//			zoomRate - Amount of time (in seconds) to move between old
//and new FOV
//-----------------------------------------------------------------------------
bool CBasePlayer::SetFOV(CBaseEntity *pRequester, int FOV, float zoomRate,
  ...
  if (pRequester == NULL) return false;
...
bool fogparams_t::operator!=(const fogparams_t &other) const {
  if (this->enable != other.enable || this->blend != other.blend ||
      ...
      this->endLerpTo != other.endLerpTo || this->lerptime != other.lerptime ||
