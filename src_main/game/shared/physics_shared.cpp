...
const char *SURFACEPROP_MANIFEST_FILE =
...
void CSolidSetDefaults::ParseKeyValue(void *pData, const char *pKey,
                                      const char *pValue) {
...
CPhysCollide *PhysCreateBbox(const Vector &minsIn, const Vector &maxsIn) {
  // UNDONE: Track down why this causes errors for the player controller and
  // adjust/enable
...
IPhysicsObject *PhysModelCreateBox(CBaseEntity *pEntity, const Vector &mins,
  ...
  return PhysModelCreateCustom(pEntity, pCollide, origin, vec3_angle,
                               STRING(pEntity->GetModelName()), isStatic,
                               &solid);
}
...
IPhysicsObject *PhysModelCreateOBB(CBaseEntity *pEntity, const Vector &mins,
  ...
  return PhysModelCreateCustom(pEntity, pCollide, origin, angle,
                               STRING(pEntity->GetModelName()), isStatic,
                               &solid);
}
...
//-----------------------------------------------------------------------------
bool PhysModelParseSolidByIndex(solid_t &solid, CBaseEntity *pEntity,
                                vcollide_t *pCollide, int solidIndex) {
  bool parsed = false;
...
IPhysicsObject *PhysModelCreate(CBaseEntity *pEntity, int modelIndex,
  ...
  IPhysicsObject *pObject =
      physenv->CreatePolyObject(pCollide->solids[pSolid->index], surfaceProp,
                                origin, angles, &pSolid->params);
  ...
  if (pObject) {
    if (modelinfo->GetModelType(modelinfo->GetModel(modelIndex)) == mod_brush) {
      ...
      // HACKHACK: So keep solid on for water brushes since they should pass
      // collision rules (as triggers)
...
IPhysicsObject *PhysModelCreateUnmoveable(CBaseEntity *pEntity, int modelIndex,
  ...
  if (!PhysModelParseSolidByIndex(solid, pEntity, pCollide, -1)) return NULL;
  ...
  solid.params.pGameData = static_cast<void *>(pEntity);
  solid.params.pName = STRING(pEntity->GetModelName());
  IPhysicsObject *pObject = physenv->CreatePolyObjectStatic(
...
IPhysicsObject *PhysModelCreateCustom(CBaseEntity *pEntity,
  ...
  if (isStatic) {
    pObject = physenv->CreatePolyObjectStatic(pModel, surfaceProp, origin,
                                              angles, &pSolid->params);
  } else {
    pObject = physenv->CreatePolyObject(pModel, surfaceProp, origin, angles,
                                        &pSolid->params);
  }

  if (pObject) g_pPhysSaveRestoreManager->AssociateModel(pObject, pModel);
...
IPhysicsObject *PhysSphereCreate(CBaseEntity *pEntity, float radius,
  ...
  solid.params.pGameData = static_cast<void *>(pEntity);
  IPhysicsObject *pObject = physenv->CreateSphereObject(
      radius, surfaceProp, origin, vec3_angle, &solid.params, false);
...
void PhysDestroyObject(IPhysicsObject *pObject, CBaseEntity *pEntity) {
  ...
  if (pObject) pObject->SetGameData(NULL);
...
void AddSurfacepropFile(const char *pFileName, IPhysicsSurfaceProps *pProps,
  ...
  if (file) {
    ...
    Error(
        "Unable to load surface prop file '%s' (referenced by manifest file "
        "'%s')\n",
        pFileName, SURFACEPROP_MANIFEST_FILE);
  }
}

void PhysParseSurfaceData(IPhysicsSurfaceProps *pProps,
                          IFileSystem *pFileSystem) {
  ...
  if (manifest->LoadFromFile(pFileSystem, SURFACEPROP_MANIFEST_FILE, "GAME")) {
    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
      ...
      Warning(
          "surfaceprops::Init:  Manifest '%s' with bogus file type '%s', "
          "expecting 'file'\n",
...
void PhysCreateVirtualTerrain(CBaseEntity *pWorld,
  ...
  for (int i = 0; i < MAX_MAP_DISPINFO; i++) {
    ...
    if (pCollide) {
      ...
      pObject->SetCallbackFlags(pObject->GetCallbackFlags() |
...
IPhysicsObject *PhysCreateWorld_Shared(CBaseEntity *pWorld,
  ...
  pWorldPhysics->SetCallbackFlags(pWorldPhysics->GetCallbackFlags() |
                                  CALLBACK_NEVER_DELETED);

  // PhysCheckAdd( world, "World" );
  // walk the world keys in case there are some fluid volumes to create
  IVPhysicsKeyParser *pParse =
      physcollision->VPhysicsKeyParserCreate(pWorldCollide->pKeyValues);
  ...
  while (!pParse->Finished()) {
    ...
    if (!strcmpi(pBlock, "solid") || !strcmpi(pBlock, "staticsolid")) {
      ...
      if (solid.index == 0) continue;

      if (!pWorldCollide->solids[solid.index]) {
        // this implies that the collision model is a mopp and the physics DLL
      ...
      // create this as part of the world
      IPhysicsObject *pObject = physenv->CreatePolyObjectStatic(
          pWorldCollide->solids[solid.index], surfaceData, vec3_origin,
      ...
      if (fluid.index > 0) {
        ...
        // create this as part of the world
        IPhysicsObject *pWater = physenv->CreatePolyObjectStatic(
            pWorldCollide->solids[fluid.index], surfaceData, vec3_origin,
            vec3_angle, &solid.params);

        pWater->SetCallbackFlags(pWater->GetCallbackFlags() |
                                 CALLBACK_NEVER_DELETED);
...
class CPhysicsGameTrace : public IPhysicsGameTrace {
 public:
  ...
  void VehicleTraceRayWithWater(const Ray_t &ray, void *pVehicle,
                                trace_t *pTrace);
...
//-----------------------------------------------------------------------------
void CPhysicsGameTrace::VehicleTraceRay(const Ray_t &ray, void *pVehicle,
                                        trace_t *pTrace) {
...
// Purpose: Game ray-traces in vphysics.
//-----------------------------------------------------------------------------
void CPhysicsGameTrace::VehicleTraceRayWithWater(const Ray_t &ray,
                                                 void *pVehicle,
                                                 trace_t *pTrace) {
  CBaseEntity *pBaseEntity = static_cast<CBaseEntity *>(pVehicle);
  UTIL_TraceRay(ray, MASK_SOLID | MASK_WATER, pBaseEntity, COLLISION_GROUP_NONE,
                pTrace);
}
...
void PhysEnableEntityCollisions(IPhysicsObject *pObject0,
  ...
  if (!pObject0 || !pObject1) return;

  g_EntityCollisionHash->RemoveObjectPair(pObject0->GetGameData(),
                                          pObject1->GetGameData());
  PhysRecheckObjectPair(pObject0, pObject1);
}

// disables collisions between entities (each entity may contain multiple
// objects)
void PhysDisableEntityCollisions(IPhysicsObject *pObject0,
                                 IPhysicsObject *pObject1) {
  if (!pObject0 || !pObject1) return;

  g_EntityCollisionHash->AddObjectPair(pObject0->GetGameData(),
                                       pObject1->GetGameData());
  PhysRecheckObjectPair(pObject0, pObject1);
}
...
void PhysFrictionEffect(Vector &vecPos, Vector vecVel, float energy,
  ...
  switch (phit->game.material) {
    case CHAR_TEX_DIRT:
      ...
      if (energy < MASS10_SPEED2ENERGY(15)) break;
    ...
    case CHAR_TEX_CONCRETE:
      ...
      if (energy < MASS10_SPEED2ENERGY(28)) break;
...
void PhysFrictionSound(CBaseEntity *pEntity, IPhysicsObject *pObject,
  ...
  if (!pEntity || energy < 75.0f || surfaceProps < 0) return;

  // don't make noise for hidden/invisible/sky materials
  surfacedata_t *phit = physprops->GetSurfaceData(surfacePropsHit);
  surfacedata_t *psurf = physprops->GetSurfaceData(surfaceProps);

  if (phit->game.material == 'X' || psurf->game.material == 'X') return;

  // rescale the incoming energy
  energy *= ENERGY_VOLUME_SCALE;

  // volume of scrape is proportional to square of energy (steeper rolloff at
  // low energies)
  ...
  if (psurf->sounds.scrapeSmooth &&
      phit->audio.roughnessFactor < psurf->audio.roughThreshold) {
