...
//			view depending on this vehicle's preferences. When vehicles
//wreck 			catastrophically, it's often better to lock the view for a little 			while
//until things settle down than to keep trying to clamp/flatten 			the view
//artificially because we can never really catch up with 			the chaotic flipping.
//-----------------------------------------------------------------------------
float ApplyViewLocking(float flAngleRaw, float flAngleClamped,
                       ViewLockData_t &lockData,
                       RemapAngleRange_CurvePart_t eCurvePart) {
  // If we're set up to never lock this degree of freedom, return the clamped
  // value.
  ...
  if ((eCurvePart == RemapAngleRange_CurvePart_Linear) ||
    ...
    if (!lockData.bLocked) {
      if (lockData.flUnlockTime != 0) {
        ...
        float flBlend =
            RemapValClamped(gpGlobals->curtime - lockData.flUnlockTime, 0,
                            lockData.flUnlockBlendInterval, 0, 1);
        // Msg( "BLEND %f\n", flBlend );
...
void RemapViewAngles(ViewSmoothingData_t *pData, QAngle &vehicleEyeAngles) {
  ...
  vecEyeAnglesRemapped.x =
      RemapAngleRange(pData->flPitchCurveZero, pData->flPitchCurveLinear,
                      vehicleEyeAngles.x, &ePitchCurvePart);

  vehicleEyeAngles.z = vecEyeAnglesRemapped.z =
      AngleNormalize(vehicleEyeAngles.z);

  // Blend out the roll dampening as our pitch approaches 90 degrees, to avoid
  ...
  float flRollDamped =
      RemapAngleRange(pData->flRollCurveZero, pData->flRollCurveLinear,
                      vecEyeAnglesRemapped.z, &eRollCurvePart);
  vecEyeAnglesRemapped.z =
      Lerp(flBlendRoll, vecEyeAnglesRemapped.z, flRollDamped);

  // Msg("PITCH ");
  vehicleEyeAngles.x =
      ApplyViewLocking(vehicleEyeAngles.x, vecEyeAnglesRemapped.x,
                       pData->pitchLockData, ePitchCurvePart);

  // Msg("ROLL ");
  vehicleEyeAngles.z =
      ApplyViewLocking(vehicleEyeAngles.z, vecEyeAnglesRemapped.z,
                       pData->rollLockData, eRollCurvePart);
}
...
void SharedVehicleViewSmoothing(CBasePlayer *pPlayer, Vector *pAbsOrigin,
  ...
  int eyeAttachmentIndex =
      pData->pVehicle->LookupAttachment("vehicle_driver_eyes");
  matrix3x4_t vehicleEyePosToWorld;
  Vector vehicleEyeOrigin;
  QAngle vehicleEyeAngles;
  pData->pVehicle->GetAttachment(eyeAttachmentIndex, vehicleEyeOrigin,
                                 vehicleEyeAngles);
  ...
  *pAbsAngles = pPlayer->EyeAngles();
  if (r_VehicleViewDampen.GetInt() && pData->bDampenEyePosition) {
    CPropVehicleDriveable *pDriveable =
        assert_cast<CPropVehicleDriveable *>(pData->pVehicle);
  ...
  if (bRunningAnim && !pData->bWasRunningAnim) {
    ...
    pData->flEnterExitDuration =
        pData->pVehicle->SequenceDuration(pData->pVehicle->GetSequence());
    ...
    // Save our initial angular error, which we will blend out over the length
    // of the animation.
    pData->vecAngleDiffSaved.x =
        AngleDiff(vehicleEyeAngles.x, pData->vecAnglesSaved.x);
    pData->vecAngleDiffSaved.y =
        AngleDiff(vehicleEyeAngles.y, pData->vecAnglesSaved.y);
    pData->vecAngleDiffSaved.z =
        AngleDiff(vehicleEyeAngles.z, pData->vecAnglesSaved.z);
  ...
  // If we're in an enter/exit animation, blend the player's eye angles to the
  // attachment's
  if (bRunningAnim || pData->bRunningEnterExit) {
    *pAbsAngles = vehicleEyeAngles;
    ...
    frac = (gpGlobals->curtime - pData->flEnterExitStartTime) /
           pData->flEnterExitDuration;
    frac = std::clamp(frac, 0.0f, 1.0f);

    flFracFOV = (gpGlobals->curtime - pData->flEnterExitStartTime) /
                (pData->flEnterExitDuration * 0.85f);
    flFracFOV = std::clamp(flFracFOV, 0.0f, 1.0f);
    ...
    if (frac < 1.0) {
      ...
      // vehicleEyeOrigin = PrevMainViewOrigin() + (vecToView *
      // SimpleSpline(frac)); debugoverlay->AddBoxOverlay( vehicleEyeOrigin,
      // -Vector(1,1,1), Vector(1,1,1), vec3_angle, 0,255,255, 64, 10 );
    } else {
      ...
      if (!bExitAnimOn) {
        ...
        pData->pVehicle->GetAttachmentLocal(eyeAttachmentIndex, localEyeOrigin,
                                            localEyeAngles);
#ifdef CLIENT_DLL
  ...
  // Compute the relative rotation between the unperturbed eye attachment + the
  // eye angles
  ...
  // Now treat the relative eye angles as being relative to this new, perturbed
  // view position...
  ...
  if (bRunningAnim || pData->bRunningEnterExit) {
    float flSplineFrac = std::clamp(SimpleSpline(frac), 0.0f, 1.0f);

    // Blend out the error between the player's initial eye angles and the
    // animation's initial eye angles over the duration of the animation.
    ...
    // In either case, never increase the error, so track the minimum error and
    // clamp to that.
    for (int i = 0; i < 3; i++) {
    ...
    *pAbsAngles -= pData->vecAngleDiffMin;
    ...
    //}
    // Msg("Prev: %.2f %.2f %.2f\n", pData->vecAnglesSaved.x,
    // pData->vecAnglesSaved.y, pData->vecAnglesSaved.z );

    Vector vecAbsOrigin = *pAbsOrigin;

    // If we're exiting, our desired position is the server-sent exit position
    if (bExitAnimOn) {
      // debugoverlay->AddBoxOverlay( vecEyeExitEndpoint, -Vector(1,1,1),
      // Vector(1,1,1), vec3_angle, 255,255,255, 64, 10 );
