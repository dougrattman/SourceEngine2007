...
void CMultiPlayerAnimState::DebugShowAnimStateForPlayer(bool bIsServer)
{
  ...
  for (int iAnim = 0; iAnim < GetBasePlayer()->GetNumAnimOverlays(); ++iAnim)
  {
    ...
    {
      Anim_StatePrintf(
        iLine++, "Layer %s: Weight: %.2f, Cycle: %.2f",
        GetSequenceName(GetBasePlayer()->GetModelPtr(),
          pLayer->m_nSequence),
        pLayer->m_flWeight.operator const float &(),
        pLayer->m_flCycle.operator const float &());
    }
#else
    ...
    if (pGesture)
    {
      if (pGesture->m_bActive)
      {
        Anim_StatePrintf(iLine++, "Gesture Slot %d(%s): %s %s(A:%s, C:%f P:%f)\n",
          ...
          pGesture->m_pAnimLayer->m_flCycle.operator const float &(),
          pGesture->m_pAnimLayer->m_flPlaybackRate);
      }
      else
      {
