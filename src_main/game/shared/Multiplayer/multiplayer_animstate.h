...
class CMultiPlayerAnimState {
 public:
  ...
  // This is called by both the client and the server in the same way to trigger
  // events for players firing, jumping, throwing grenades, etc.
 ...
 protected:
  ...
  void AddToGestureSlot(int iGestureSlot, Activity iGestureActivity,
                        bool bAutoKill);
  void RestartGesture(int iGestureSlot, Activity iGestureActivity,
                      bool bAutoKill = true);
...
// Client specific.
 ...
 protected:
  ...
  // Last activity we've used on the lower body. Used to determine if animations
  // should restart.
