...
#define IMPLEMENT_ACTTABLE(className)                              \
  ...
  int className::ActivityListCount(void) {                         \
    return SOURCE_ARRAYSIZE(m_acttable);                           \
  }
...
class CBaseCombatWeapon : public CBaseAnimating {
 public:
  ...
  // A derived weapon class should return true here so that weapon sounds, etc,
  // can
  ...
  virtual void RescindAltFireHudHint();  ///< undisplay the hud hint and pretend
                                         ///< it never showed.
  ...
  bool IsViewModelSequenceFinished(
      void);  // Returns if the viewmodel's current animation is finished
  ...
  virtual void ItemBusyFrame(
      void);  // called each frame by the player PostThink, if the player's not
              // ready to attack yet
  ...
  HandleFireOnEmpty();  // Called when they have the attack button down
                        ...
                        // either reloads, switches weapons, or plays an empty
                        // sound.
#ifdef CLIENT_DLL
  ...
  virtual float GetSpreadBias(WeaponProficiency_t proficiency) { return 1.0f; }
  ...
  virtual float WeaponAutoAimScale() {
    return 1.0f;
  }  // allows a weapon to influence the perceived size of the target's autoaim
     // radius.
  ...
  // TF Injury functions
  virtual float GetDamage(float flDistance, int iLocation) { return 0.0f; };
...
// Server Only Methods
  ...
  bool RepositionWeapon(
      void);  // Attempts to reposition the weapon in a location where it can be
  virtual void Materialize(void);   // make a weapon visible and tangible
  void AttemptToMaterialize(void);  // see if the game rules will let the weapon
...
// Client only methods
 ...
 public:
  ...
  // Weapon firing
  CNetworkVar(float, m_flNextPrimaryAttack);  // soonest time ItemPostFrame will
                                              // call PrimaryAttack
 ...
 public:
  ...
  CNetworkVar(
      int,
      m_iSecondaryAmmoType);  // "secondary" ammo index into the ammo info array
 ...
 private:
  ...
  bool m_bReloadHudHintDisplayed;   // Have we displayed a reload HUD hint since
                                    // this weapon was deployed?
  float m_flHudHintPollTime;  // When to poll the weapon again for whether it
                              // should display a hud hint.
  float
      m_flHudHintMinDisplayTime;  // if the hint is squelched before this, reset
                                  // my counter so we'll display it again.
  ...
  COutputEvent
      m_OnCacheInteraction;  // For awarding lambda cache achievements in HL2 on
                             // 360. See .FGD file for details
