...
void CPointBonusMapsAccessor::InputUnlock(inputdata_t &inputdata) {
  if (m_pGameUI)
    m_pGameUI->BonusMapUnlock(m_String_tFileName.ToCStr(),
                              m_String_tMapName.ToCStr());
}

void CPointBonusMapsAccessor::InputComplete(inputdata_t &inputdata) {
  if (m_pGameUI) {
    m_pGameUI->BonusMapComplete(m_String_tFileName.ToCStr(),
                                m_String_tMapName.ToCStr());
...
void CPointBonusMapsAccessor::InputSave(inputdata_t &inputdata) {
  if (m_pGameUI) m_pGameUI->BonusMapDatabaseSave();
}
...
void BonusMapChallengeUpdate(const char *pchFileName, const char *pchMapName,
  ...
  if (gameUIFactory) {
    ...
    if (pGameUI) {
      pGameUI->BonusMapChallengeUpdate(pchFileName, pchMapName,
      ...
      IGameEvent *event =
          gameeventmanager->CreateEvent("challenge_map_complete");
      if (event) {
...
void BonusMapChallengeNames(char *pchFileName, size_t file_name_size,
  ...
  if (gameUIFactory) {
    IGameUI *pGameUI = (IGameUI *)gameUIFactory(GAMEUI_INTERFACE_VERSION, NULL);
    if (pGameUI) {
      pGameUI->BonusMapChallengeNames(pchFileName, file_name_size, pchMapName,
                                      map_name_size, pchChallengeName,
                                      challenge_name_size);
    }
  }
}
