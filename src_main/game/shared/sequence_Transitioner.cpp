...
void CSequenceTransitioner::CheckForSequenceChange(CStudioHdr *hdr,
                                                   int nCurSequence,
  ...
  // TODO(d.rattman): this should detect that what's been asked to be drawn
  // isn't what was expected due to not only sequence change, by frame index,
  // rate, or whatever.  When that happens, it should insert the previous rules.
  if (m_animationQueue.Count() == 0) {
    m_animationQueue.AddToTail();
  }

  CAnimationLayer *currentblend =
      &m_animationQueue[m_animationQueue.Count() - 1];
  ...
  if (currentblend->m_flLayerAnimtime &&
    ...
    if ((seqdesc.flags & STUDIO_SNAP) || !bInterpolate) {
      ...
      currentblend->m_flLayerFadeOuttime =
          std::min(prevseqdesc.fadeouttime, seqdesc.fadeintime);
      /*
      // clip blends to time remaining
      if ( !IsSequenceLooping(hdr, currentblend->m_nSequence) )
      {
              float length = Studio_Duration( hdr, currentblend->m_nSequence,
      flPoseParameter ) / currentblend->m_flPlaybackRate; float timeLeft = (1.0
      - currentblend->m_flCycle) * length; if (timeLeft <
      currentblend->m_flLayerFadeOuttime) currentblend->m_flLayerFadeOuttime =
      timeLeft;
      }
      */
    }
...
void CSequenceTransitioner::UpdateCurrent(CStudioHdr *hdr, int nCurSequence,
                                          float flCurCycle,
