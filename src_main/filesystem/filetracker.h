...
enum EFileFlags {
  ...
  k_eFileFlagsHasCRC = 0x0002,  // m_CRC represents the most recently-loaded
                                // version of the file. This might be
  // unset but m_CRC (and k_eFileFlagsGotCRCOnce) could be set, signifying that
  // the file has been opened since last time we calculated the CRC but we
  // didn't calculate a CRC for that file version.

  k_eFileFlagsForcedLoadFromSteam =
      0x0004,  // Set if k_eFileFlagsLoadedFromSteam is set AND we forced Steam
               // to not check the disk for this file.

  k_eFileFlagsGotCRCOnce = 0x0008,  // This is set if we EVER had
                                    // k_eFileFlagsHasCRC set.. m_CRC will still
                                    // be set in that case,
  // but it'll be the last CRC we calculated and not necessarily

  k_eFileFlagsFailedToLoadLastTime = 0x0010  // This is used if we had a record
                                             // of the file and the game tried
                                             // to open it, but
  // it couldn't be opened. This will happen if the file was loaded from disk,
  // then sv_pure is turned on and the file is forced to come from Steam, but
  // there is no Steam version. In that case, the game should be told to retry
  // the file next time sv_pure is changed because if sv_pure goes back to 0 it
  // -would- load the file legitimately.
};
...
class FileInfo {
 ...
 public:
  ...
  int m_iNeedsVerificationListIndex;  // Index into m_NeedsVerificationList or
...
class CFileTracker {
 public:
  ...
  // If this is true, then we'll calculate CRCs for each file that came off
  // disk.
  void SetWantFileCRCs(bool bWantCRCs);

  // As files are opened, if it is calculating CRCs, it will add those files and
  // their CRCs to the "unverified CRC" list. The client can then ask the server
  // to verify those CRCs to make sure the client is "pure".
  void MarkAllCRCsUnverified();
  void MarkAllCRCsVerified(bool bLockMutex = true);

  // Cache a file's CRC. Loads the file and calculates the CRC if we don't have
  // it yet.
  void CacheFileCRC(const char *pPathID, const char *pRelativeFilename);
  EFileCRCStatus CheckCachedFileCRC(const char *pPathID,
                                    const char *pRelativeFilename,
                                    CRC32_t *pCRC);

  // This is like CacheFileCRC, but it assumes that the same file would be found
  // by pPathIDToCopyFrom, so it just copies the CRC record from that path ID
  // into the one in pPathID and avoids a redundant CRC calculation.
  void CacheFileCRC_Copy(const char *pPathID, const char *pRelativeFilename,
                         const char *pPathIDToCopyFrom);

  // When we don't have a whitelist, it loads files without bothering to
  // calculate their CRCs (we'd only need them on a pure server), but when we
  // get a whitelist, we'll want the CRCs, so it goes back, opens those files,
  // and calculates the CRCs for them.
  ...
  void NoteFileLoadedFromDisk(const char *pFilename, const char *pPathID,
                              FileHandle_t fp);
  void NoteFileLoadedFromSteam(const char *pFilename, const char *pPathID,
