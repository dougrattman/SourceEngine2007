...
//
// The CBaseFileSystem methods implement the IFileSystem asynchronous entry
// points. The model for reads currently is a callback model where the callback
// can take place either in the context of the main thread or the worker thread.
// It would be easy to do a polled model later. Async operations return a handle
// which is used to refer to the operation later. The handle is actually a
// pointer to a reference counted "job" object that holds all the context,
// status, and results of an operation.
...
//
//-----------------------------------------------------------------------------
ConVar async_mode("async_mode", "0", 0,
...
ConVar async_simulate_delay(
    "async_simulate_delay", "0", 0,
    "Simulate a delay of up to a set msec per file operation");
ConVar async_allow_held_files("async_allow_held_files", "1", 0,
                              "Allow AsyncBegin/EndRead()");

#define SimulateDelay()                                           \
...
CON_COMMAND(async_suspend, "") { BaseFileSystem()->AsyncSuspend(); }

CON_COMMAND(async_resume, "") { BaseFileSystem()->AsyncResume(); }
...
//-----------------------------------------------------------------------------
// Need to support old external. New implementation has less granular priority
// for efficiency
//-----------------------------------------------------------------------------
...
struct AsyncOpenedFile_t
    : CRefCounted<CRefCountServiceNoDelete>  // no mutex needed, under control
                                             // of CAsyncOpenedFiles
{
...
class CAsyncOpenedFiles {
 public:
  CAsyncOpenedFiles() { m_map.SetLessFunc(CaselessStringLessThan); }
...
class CFileAsyncJob : public CJob {
 public:
  CFileAsyncJob(JobPriority_t priority = JP_NORMAL) : CJob(priority) {
...
//---------------------------------------------------------
class CFileAsyncReadJob : public CFileAsyncJob, protected FileAsyncRequest_t {
 public:
  ...
  const FileAsyncRequest_t *GetRequest() const { return this; }
...
#if (defined(_DEBUG) || defined(USE_MEM_DEBUG))
    if (m_pszAllocCreditFile) MemAlloc_PopAllocDbgInfo();
#endif

#if defined(TRACK_BLOCKING_IO)
    m_Timer.End();
    FileBlockingItem item(FILESYSTEM_BLOCKING_ASYNCHRONOUS, Describe(),
                          m_Timer.GetDuration().GetSeconds(),
                          FileBlockingItem::FB_ACCESS_READ);
  ...
  static void InterceptCallback(const FileAsyncRequest_t &request,
    ...
    if (pJob->m_pfnRealCallback) {
      // Going to slam the values. Not used after this point. Make temps if that
      // changes
...
class CFileAsyncWriteJob : public CFileAsyncJob {
 public:
  CFileAsyncWriteJob(const char *pszFilename, const void *pData,
                     unsigned nBytes, bool bFreeMemory, bool bAppend)
  ...
  virtual JobStatus_t DoExecute() {
    ...
    bool oldState = BaseFileSystem()->SetAllowSynchronousLogging(false);
#endif
    JobStatus_t retval = BaseFileSystem()->SyncWrite(
        m_pszFilename, m_pData, m_nBytes, false, m_bAppend);
...
class CFileAsyncWriteFileJob : public CFileAsyncWriteJob {
 public:
  CFileAsyncWriteFileJob(const char *pszFilename, const CUtlBuffer *pData,
                         unsigned nBytes, bool bFreeMemory, bool bAppend)
      : CFileAsyncWriteJob(pszFilename, pData->Base(), nBytes, bFreeMemory,
                           bAppend),
        m_pBuffer(pData) {}
...
class CFileAsyncAppendFileJob : public CFileAsyncJob {
 public:
  ...
  ~CFileAsyncAppendFileJob() { g_nAsyncWriteJobs--; }
...
#endif
    JobStatus_t retval =
        BaseFileSystem()->SyncAppendFile(m_pszAppendTo, m_pszAppendFrom);

#if defined(TRACK_BLOCKING_IO)
    m_Timer.End();
    FileBlockingItem item(FILESYSTEM_BLOCKING_ASYNCHRONOUS, Describe(),
                          m_Timer.GetDuration().GetSeconds(),
                          FileBlockingItem::FB_ACCESS_APPEND);
    BaseFileSystem()->RecordBlockingFileAccess(false, item);
    BaseFileSystem()->SetAllowSynchronousLogging(oldState);
#endif
...
class CFileAsyncFileSizeJob : public CFileAsyncReadJob {
 public:
  ...
  virtual JobStatus_t DoExecute() {
    ...
    FileBlockingItem item(FILESYSTEM_BLOCKING_ASYNCHRONOUS, Describe(),
                          m_Timer.GetDuration().GetSeconds(),
                          FileBlockingItem::FB_ACCESS_SIZE);
    BaseFileSystem()->RecordBlockingFileAccess(false, item);
    BaseFileSystem()->SetAllowSynchronousLogging(oldState);
#endif
    return retval;
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::AsyncReadMultipleCreditAlloc(
    const FileAsyncRequest_t *pRequests, int nRequests, const char *pszFile,
    int line, FSAsyncControl_t *phControls) {
  bool bAsyncMode = (GetAsyncMode() == FSAM_ASYNC);
  bool bSynchronous =
      (!bAsyncMode || (pRequests[0].flags & FSASYNC_FLAGS_SYNC) ||
       !m_pThreadPool);
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::AsyncWrite(const char *pFileName,
  ...
  CJob *pJob =
      new CFileAsyncWriteJob(pFileName, pSrc, nSrcBytes, bFreeMemory, bAppend);
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::AsyncWriteFile(const char *pFileName,
                                                const CUtlBuffer *pBuff,
  ...
  CJob *pJob = new CFileAsyncWriteFileJob(pFileName, pBuff, nSrcBytes,
                                          bFreeMemory, bAppend);
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::AsyncAppendFile(
  ...
  CJob *pJob =
      new CFileAsyncAppendFileJob(pAppendToFileName, pAppendFromFileName);
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::AsyncFinish(FSAsyncControl_t hControl,
                                             bool wait) {
  if (wait) {
...
#if defined(TRACK_BLOCKING_IO)
    timer.End();
    FileBlockingItem item(FILESYSTEM_BLOCKING_ASYNCHRONOUS_BLOCK,
                          pJob->Describe(), timer.GetDuration().GetSeconds(),
                          FileBlockingItem::FB_ACCESS_READ);
    BaseFileSystem()->RecordBlockingFileAccess(false, item);
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::AsyncSetPriority(FSAsyncControl_t hControl,
                                                  int newPriority) {
  if (m_pThreadPool) {
    CJob *pJob = (CJob *)hControl;
...
FSAsyncStatus_t CBaseFileSystem::SyncRead(const FileAsyncRequest_t &request) {
  ...
  if (hFile) {
    // ------------------------------------------------------
    int nBytesToRead =
        (request.nBytes) ? request.nBytes : Size(hFile) - request.nOffset;
    int nBytesBuffer;
    void *pDest;
    ...
    SetBufferSize(hFile,
                  0);  // TODO: what if it's a pack file? restore buffer size?
...
//
//-----------------------------------------------------------------------------
FSAsyncStatus_t CBaseFileSystem::SyncGetFileSize(
...
void CBaseFileSystem::DoAsyncCallback(const FileAsyncRequest_t &request,
  ...
  if (request.pfnCallback) {
    ...
    if (pData && request.pData != pData) {
      ...
      temp.pData = pData;
      {
        AUTOBLOCKREPORTER_FN(DoAsyncCallback, this, false, temp.pszFilename,
      ...
      {
        AUTOBLOCKREPORTER_FN(DoAsyncCallback, this, false, request.pszFilename,
                             FILESYSTEM_BLOCKING_CALLBACKTIMING,
                             FileBlockingItem::FB_ACCESS_READ);
