...
CFileTracker::~CFileTracker() { Clear(); }

void CFileTracker::NoteFileLoadedFromDisk(const char *pFilename,
                                          const char *pPathID,
                                          FileHandle_t fp) {
  AUTO_LOCK(m_Mutex);
  ...
  if (pInfo->m_iNeedsVerificationListIndex == -1)
    pInfo->m_iNeedsVerificationListIndex =
        m_NeedsVerificationList.AddToTail(pInfo);
...
void CFileTracker::NoteFileLoadedFromSteam(const char *pFilename,
                                           const char *pPathID,
  ...
  if (bForcedLoadFromSteam) pInfo->m_Flags |= k_eFileFlagsForcedLoadFromSteam;
}
  ...
  for (int i = m_PathIDs.First(); i != m_PathIDs.InvalidIndex();
    ...
    for (j = pPath->m_Files.First(); j != pPath->m_Files.InvalidIndex();
      ...
      if (!(pInfo->m_Flags & k_eFileFlagsLoadedFromSteam) &&
        ...
        // If the new "force match" list doesn't care whether the file has a CRC
        // or not, then don't bother to calculate it.
        if (!pWantCRCList->IsFileInList(pInfo->GetFilename())) continue;
...
void CFileTracker::CacheFileCRC(const char *pPathID,
...
void CFileTracker::CacheFileCRC_Copy(const char *pPathID,
                                     const char *pRelativeFilename,
...
EFileCRCStatus CFileTracker::CheckCachedFileCRC(const char *pPathID,
                                                const char *pRelativeFilename,
                                                CRC32_t *pCRC) {
  Assert(ThreadInMainThread());
...
void CFileTracker::CalculateMissingCRC(const char *pFilename,
                                       const char *pPathID) {
...
void CFileTracker::MarkAllCRCsVerified(bool bLockMutex) {
  ...
  if (bLockMutex) m_Mutex.Unlock();
}

int CFileTracker::GetUnverifiedCRCFiles(CUnverifiedCRCFile *pFiles,
                                        int nMaxFiles) {
  Assert(nMaxFiles > 0);
  ...
  int iOutFile = 0;
  int iNext = 0;
  for (int i = m_NeedsVerificationList.Head();
    ...
    // This can happen if a file that was in this list was loaded from Steam
    // since it got added to the list. In that case, just act like it's not in
    // the list.
    if (pInfo->m_Flags & k_eFileFlagsLoadedFromSteam) continue;
    ...
    ++iOutFile;
    if (iOutFile >= nMaxFiles) return iOutFile;
  }
...
CPathIDFileList *CFileTracker::GetPathIDFileList(const char *pPathID,
                                                 bool bAutoAdd) {
  AUTO_LOCK(m_Mutex);
