...
void CheckError(SteamHandle_t fp, TSteamError &steamError) {
  ...
  if (fp) {
    if (steamError.eSteamError != eSteamErrorNone ||
        g_tLastError.eSteamError != eSteamErrorNone) {
      g_pLastErrorFile = fp;
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CFileSystem_Steam, IFileSystem,
                                  FILESYSTEM_INTERFACE_VERSION,
                                  g_FileSystem_Steam);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CFileSystem_Steam, IBaseFileSystem,
...
void MountDependencies(int iAppId, CUtlVector<unsigned int> &depList) {
  ...
  for (int i = 0; i < (int)steamApp.uNumDependencies; i++) {
    ...
    if (steamError.eSteamError != eSteamErrorNone)
      Error("EnumerateAppDependency( %d, %d ) failed: %s", iAppId, i,
            steamError.szDesc);
    ...
    if (depList.Find(appDependencyInfo.uAppId) == -1) {
      ...
      if (steamError.eSteamError != eSteamErrorNone &&
          steamError.eSteamError != eSteamErrorNotSubscribed)
        Error("MountFilesystem( %d ) failed: %s", appDependencyInfo.uAppId,
              steamError.szDesc);
    }
  }
}
...
void CFileSystem_Steam::Shutdown() {
  ...
  // If we're not running Steam in local mode, remove all mount points from the
  // STEAM VFS.
...
void CFileSystem_Steam::LoadAndStartSteam() {
  ...
  if (m_hSteamDLL) {
    ...
    PFSteamCreateInterface pfnSteamCreateInterface =
        (PFSteamCreateInterface)GetProcAddress(m_hSteamDLL, "_f");
    if (pfnSteamCreateInterface)
      steam =
          (ISteamInterface *)pfnSteamCreateInterface(STEAM_INTERFACE_VERSION);
  }
  ...
  TSteamError steamError;
  if (!steam->Startup(STEAM_USING_FILESYSTEM | STEAM_USING_LOGGING |
                          STEAM_USING_USERID | STEAM_USING_ACCOUNT,
                      &steamError)) {
...
FilesystemMountRetval_t CFileSystem_Steam::MountSteamContent(int nExtraAppId) {
  ...
  // this enables both the game and the platform to use this same code, even
  // though they mount caches at different times
  int loggedIn = 0;
  TSteamError steamError;
  ...
  if (!result || loggedIn) {
    if (nExtraAppId != -1) {
      ...
      if (nExtraAppId < -1) {
...
#ifdef _WIN32
        // Use GetEnvironmentVariable instead of getenv because getenv doesn't
        // pick up changes to the process environment after the DLL was loaded.
        char szMainAppId[128];
        if (GetEnvironmentVariable("steamappid", szMainAppId,
                                   sizeof(szMainAppId)) != 0) {
        ...
        // swapping this mount order ensures the most current engine binaries
        // are used by tools
        MountDependencies(nExtraAppId, depList);
...
FILE *CFileSystem_Steam::FS_fopen(const char *filename, const char *options,
  ...
  if (m_bAssertFilesImmediatelyAvailable && !m_bCurrentlyLoading) {
    if (!IsFileImmediatelyAvailable(filename)) {
      Msg("Steam FS: '%s' not immediately available when not in loading dialog",
          filename);
    }
  }
  ...
  int bLocal = 0;
  SteamHandle_t f =
      steam->OpenFileEx(filename, options, pInfo->m_bSteamCacheOnly, &fileSize,
                        &bLocal, &steamError);
...
WaitForResourcesHandle_t CFileSystem_Steam::WaitForResources(
  ...
  // cancel any old call
  TSteamError steamError;
  m_hWaitForResourcesCallHandle =
      steam->WaitForResources(szResourceList, &steamError);
  if (steamError.eSteamError == eSteamErrorNone) {
...
// Purpose: steam call, unnecessary in stdio
//-----------------------------------------------------------------------------
bool CFileSystem_Steam::GetWaitForResourcesProgress(
  ...
  *complete = true;
  ...
  if (m_iCurrentReturnedCallHandle != handle) return false;
  if (m_hWaitForResourcesCallHandle == STEAM_INVALID_CALL_HANDLE) return false;

  // get the progress
  TSteamError steamError;
  TSteamProgress steamProgress;
  int result = steam->ProcessCall(m_hWaitForResourcesCallHandle, &steamProgress,
                                  &steamError);
  ...
  if (steamProgress.bValid) {
    *progress = (float)steamProgress.uPercentDone /
                (100.0f * STEAM_PROGRESS_PERCENT_SCALE);
  } else {
    *progress = 0;
  }
  *complete = false;
...
// Purpose: steam call, unnecessary in stdio
//-----------------------------------------------------------------------------
void CFileSystem_Steam::CancelWaitForResources(
    WaitForResourcesHandle_t handle) {
  // check to see if they're using an old handle
  if (m_iCurrentReturnedCallHandle != handle) return;
  if (m_hWaitForResourcesCallHandle == STEAM_INVALID_CALL_HANDLE) return;
...
void CFileSystem_Steam::FS_fseek(FILE *fp, __int64 pos, int seekType) {
  TSteamError steamError;
  int result;
  result = steam->SeekFile((SteamHandle_t)fp, (int32_t)pos,
                           (ESteamSeekMethod)seekType, &steamError);
  CheckError((SteamHandle_t)fp, steamError);
}
...
int CFileSystem_Steam::FS_feof(FILE *fp) {
  ...
  if (g_tLastError.eSteamError != eSteamErrorNone) return 0;

  // Jump to the end...
  FS_fseek(fp, 0L, SEEK_END);

  // If we were already at the end, return true
  if (orig_pos == FS_ftell(fp)) return 1;
...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
size_t CFileSystem_Steam::FS_fread(void *dest, size_t destSize, size_t size,
                                   FILE *fp) {
  TSteamError steamError;
  int blocksRead =
      steam->ReadFile(dest, 1, size, (SteamHandle_t)fp, &steamError);
  CheckError((SteamHandle_t)fp, steamError);
...
int CFileSystem_Steam::FS_stat(const char *path, struct _stat *buf) {
  ...
  if (returnVal == 0) {
    if (Info.bIsDir) {
      ...
      // Now we want to know if it's writable or not. First see if there is a
      // local copy.
      struct _stat testBuf;
      int rt = _stat(path, &testBuf);
      if (rt == 0) {
        // Ok, there's a local copy. Now check if the copy on our HD is
        // writable.
...
// Purpose: low-level filesystem wrapper
//-----------------------------------------------------------------------------
HANDLE CFileSystem_Steam::FS_FindFirstFile(const char *findname,
                                           WIN32_FIND_DATA *dat) {
  ...
  if (steamResult == STEAM_INVALID_HANDLE) {
    ...
    // ***NOTE*** we are hijacking the FILE_ATTRIBUTE_OFFLINE bit and using it
    // in a different
    //            (but similar) manner than the WIN32 documentation indicates
    //            ***NOTE***
...
#ifdef _WIN32
      if (IsDebuggerPresent() != FALSE) bGetLocalCopy = false;
#endif
      if (bGetLocalCopy) GetLocalCopy(newPathName);
...
extern "C" SteamHandle_t SteamFindFirst(const char *cszPattern,
...
int CFileSystem_Steam::HintResourceNeed(const char *hintlist,
