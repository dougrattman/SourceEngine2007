...
// loading process.
//
// Not designed for application startup or gameplay time. Layered on top of
// async i/o. Queued loading is allowed during the map load process until full
// connection only, but can complete the remaining low priority jobs during the
// game render. The normal loading path can run in units of seconds if it does
// not have to do I/O, which is why this system runs first and gets all the data
// in memory unhindered by dependency blocking. The I/O delivery process
// achieves its speed by having all the I/O requests at once, performing the
// I/O, and handing the actual consumption of the I/O buffer to another
// available core/thread (via job pool) for computation work. The I/O (should be
// all unbuffered) is then only throttled by physical transfer rates.
//
// The Load process is broken into three phases. The first phase build up I/O
// requests. The second phase fulfills only the high priority I/O requests. This
// gets the critical data in memory, that has to be there for the normal load
// path to query, or the renderer to run (i.e. models and shaders). The third
// phase is the normal load process. The low priority jobs run concurrently with
// the normal load process.  Low priority jobs are those that have been
// specially built such that the game or loading can operate unblocked without
// the actual data (i.e. d3d texture bits).
//
// Phase 1: The reslist is parsed into seperate lists based on handled
// extensions. Each list call its own loader which in turn generates its own
// dictionaries and I/O requests through "AddJob". A single reslist entry could
// cause a laoder to request multiple jobs. ( i.e. models ) A loader marks its
// jobs as high or low priority. Phase 2: The I/O requests are sorted (which
// achieves seek offset order) and async i/o commences. Phase 2 does not end
// until all the high priority jobs are complete. This ensures critical data is
// resident. Phase 3: The !!!NORMAL!!! loading path can commence. The legacy
// loading path then is not expected to do I/O (it can, but that's a hole in the
// reslist), as all of the data that it queries, should be resident.
//
...
struct FileJob_t {
  FileJob_t() { Q_memset(this, 0, sizeof(FileJob_t)); }
...
class CQueuedLoader : public CTier2AppSystem<IQueuedLoader> {
 ...
 public:
  ...
  char *GetFilename(const FileNameHandle_t hFilename, char *pBuff,
                    int nBuffSize);
  FileNameHandle_t FindFilename(const char *pFilename);
  void SpewInfo();

  // submit any queued jobs to the async loader, called by main or async thread
  // to get more work
 ...
 private:
  class CFileJobsLessFunc {
   public:
    ...
    bool Less(FileJob_t *const &pFileJobLHS, FileJob_t *const &pFileJobRHS,
              void *pCtx);
  };
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CQueuedLoader, IQueuedLoader,
...
class CResourcePreloadAnonymous : public IResourcePreload {
  virtual bool CreateResource(const char *pName) {
    // create an anonymous job to get the data in memory, claimed during load,
    // or auto-freed
...
void IOComputationJob(FileJob_t *pFileJob, void *pData, int nSize,
  ...
  if (spewDetail &
      (LOADER_DETAIL_COMPLETIONS | LOADER_DETAIL_LATECOMPLETIONS)) {
  ...
  if (!pFileJob->m_pCallback) {
    // absent callback means resource loader want this system to delay buffer
    // until ready for it
    ...
    // context
    pFileJob->m_pCallback(pFileJob->m_pContext, pFileJob->m_pContext2, pData,
                          nSize, loaderError);
...
void FinishAnonymousJob(FileJob_t *pFileJob, QueuedLoaderCallback_t pCallback,
  ...
  // regardless of error, call job callback so caller can do cleanup of their
  // context
  pCallback(pContext, pContext2, pFileJob->m_pTargetData,
            pFileJob->m_nActualBytesRead, pFileJob->m_LoaderError);
...
void IOAsyncCallback(const FileAsyncRequest_t &asyncRequest, int numReadBytes,
  ...
  g_pThreadPool
      ->QueueCall(IOComputationJob, pFileJob, asyncRequest.pData, numReadBytes,
                  loaderError)
      ->Release();

  // don't let the i/o starve, possibly get some more work from the pending
  // queue
...
void CQueuedLoader::SubmitPendingJobs() {
  ...
  if (ThreadInMainThread() && g_nActiveJobs != 0) {
    ...
    // once the I/O is kicked off, the I/O thread is responsible for continual
    // draining
    return;
  } else if (!ThreadInMainThread() && g_nActiveJobs != 1) {
    // I/O thread requests more work, but will only fall through and get some
    // when it expects to go idle I/O thread still has jobs and doesn't need any
    // more yet
    return;
  }
  ...
  while (pNode) {
    ...
    CTSList<FileJob_t *>::Node_t *pNext =
        (CTSList<FileJob_t *>::Node_t *)pNode->Next;
    delete pNode;
    pNode = pNext;
  }
  ...
  for (int i = 0; i < sortedFiles.Count(); i++) {
    ...
    if (pFileJob->m_bFileExists) {
      ...
      // still need to do callback so subsystems can do the right thing based on
      // file absence
      g_pThreadPool
          ->QueueCall(IOComputationJob, pFileJob, pFileJob->m_pTargetData, 0,
                      LOADERERROR_FILEOPEN)
          ->Release();
    }
  }
}
...
bool CQueuedLoader::AddJob(const LoaderJob_t *pLoaderJob) {
  ...
  // anonymous jobs lack callbacks and are heavily restricted to ensure their
  // stability the caller is expected to claim these before load ends (which
  // auto-purges them)
  if (!pLoaderJob->m_pCallback &&
      pLoaderJob->m_Priority == LOADERPRIORITY_ANYTIME) {
    Assert(0);
  ...
  // all bsp based files get forced to a higher priority in order to achieve a
  // clustered sort the bsp files are not going to be anywhere near the zips,
  // thus we don't want head thrashing
  bool bFileIsFromBSP;
  bool bExists = false;
  ...
  if (V_IsAbsolutePath(pLoaderJob->m_pFilename)) {
    ...
    // which achieves seek linearization a resolved absolute file ensures its
    // existence
    PathTypeFilter_t pathFilter = FILTER_NONE;

    PathTypeQuery_t pathType;
    g_pFullFileSystem->RelativePathToFullPath(
        pLoaderJob->m_pFilename, pLoaderJob->m_pPathID, szFullPath,
        sizeof(szFullPath), pathFilter, &pathType);
  ...
  pFileJob->m_Priority =
      bFileIsFromBSP ? LOADERPRIORITY_DURINGPRELOAD : pLoaderJob->m_Priority;
  ...
  if (pLoaderJob->m_pTargetData) {
    ...
    pFileJob->m_bFreeTargetAfterIO =
        (pLoaderJob->m_bPersistTargetData == false);
  }
...
bool CQueuedLoader::ClaimAnonymousJob(const char *pFilename,
  ...
  if (!pFileJob->m_bFinished) {
    ...
    // anonymous jobs and their claims are very restrictive in such a way to
    // provide stability this dead job will get auto-cleaned at end of map
    // loading
    Assert(0);
    return false;
  }
  ...
  g_pThreadPool
      ->QueueCall(FinishAnonymousJob, pFileJob, pCallback, pContext, pContext2)
      ->Release();
...
bool CQueuedLoader::ClaimAnonymousJob(const char *pFilename, void **pData,
  ...
  *pDataSize = pFileJob->m_LoaderError == LOADERERROR_NONE
...
void CQueuedLoader::SubmitBatchedJobsAndWait() {
  ...
  if (GetSpewDetail()) {
    Msg("QueuedLoader: High Priority Jobs: %d\n",
        static_cast<int>(g_nHighPriorityJobs));
  }

  // finish only the high priority jobs
  // high priority jobs are expected to be complete at the conclusion of
  // batching
...
void CQueuedLoader::SpewInfo() {
    ...
    while (iIndex != m_SubmittedJobs.InvalidIndex()) {
      ...
      if (!pFileJob->m_pCallback) {
        V_snprintf(szAnonymousString, sizeof(szAnonymousString), "(%s) ",
                   pFileJob->m_bClaimed ? "Claimed" : "Unclaimed");
      ...
      Msg("Submit:%5dms AsyncDuration:%5dms Tag:%d Thread:%8.8x Size:%7d "
          ...
          pFileJob->m_SubmitTag, pFileJob->m_ThreadId,
          pFileJob->m_nActualBytesRead, pAnonymousStatus,
          GetFilename(pFileJob->m_hFilename, szFilename, sizeof(szFilename)));
  ...
  Msg("%d Queued Jobs\n", static_cast<int>(g_nQueuedJobs));
  Msg("%d Active Jobs\n", static_cast<int>(g_nActiveJobs));
...
void CQueuedLoader::PurgeUnreferencedResources() {
  ...
  // the purge operations require a specific order (models and cubemaps before
  // materials)
  int numPurges = 0;
...
void CQueuedLoader::GetJobRequests() {
  ...
  // materials are built (and precache the cubmeap textures) cannot be
  // overlapped, must run serially
  BuildResources(m_pLoaders[RESOURCEPRELOAD_CUBEMAP],
                 &m_ResourceNames[RESOURCEPRELOAD_CUBEMAP],
                 &m_LoaderTimes[RESOURCEPRELOAD_CUBEMAP]);

  // Overlapping these is not critical in any way, total time is currently < 2
  // seconds. These operations flood calls (AddJob) back into the queued loader
  // (which has to mutex its lists), so in fact it's slightly slower to queue
  // these at this stage. As these routines age they may become more
  // heavyweight.
  jobs[0] = g_pThreadPool->QueueCall(BuildResources,
                                     m_pLoaders[RESOURCEPRELOAD_SOUND],
                                     &m_ResourceNames[RESOURCEPRELOAD_SOUND],
                                     &m_LoaderTimes[RESOURCEPRELOAD_SOUND]);
  jobs[1] = g_pThreadPool->QueueCall(BuildMaterialResources,
                                     m_pLoaders[RESOURCEPRELOAD_MATERIAL],
                                     &m_ResourceNames[RESOURCEPRELOAD_MATERIAL],
                                     &m_LoaderTimes[RESOURCEPRELOAD_MATERIAL]);
  jobs[2] = g_pThreadPool->QueueCall(
      BuildResources, m_pLoaders[RESOURCEPRELOAD_STATICPROPLIGHTING],
      &m_ResourceNames[RESOURCEPRELOAD_STATICPROPLIGHTING],
      &m_LoaderTimes[RESOURCEPRELOAD_STATICPROPLIGHTING]);
  jobs[3] = g_pThreadPool->QueueCall(BuildResources,
                                     m_pLoaders[RESOURCEPRELOAD_MODEL],
                                     &m_ResourceNames[RESOURCEPRELOAD_MODEL],
                                     &m_LoaderTimes[RESOURCEPRELOAD_MODEL]);
  jobs[4] = g_pThreadPool->QueueCall(
      BuildResources, m_pLoaders[RESOURCEPRELOAD_ANONYMOUS],
      &m_ResourceNames[RESOURCEPRELOAD_ANONYMOUS],
...
void CQueuedLoader::ParseResourceList(CUtlBuffer &resourceList) {
  ...
  // duplicates don't need to be culled, loaders are supposed to handle
  ...
  if (m_bActive) {
    ...
    if (g_nIOMemory || g_nAnonymousIOMemory) {
      ...
      DevWarning("CQueuedLoader: Unclaimed I/O memory: total:%d anonymous:%d\n",
                 static_cast<int>(g_nIOMemory),
                 static_cast<int>(g_nAnonymousIOMemory));
      g_nIOMemory = 0;
