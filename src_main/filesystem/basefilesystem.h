...
enum FileType_t { FT_NORMAL, FT_PACK_BINARY, FT_PACK_TEXT };
...
class CFileHandle {
 ...
 protected:
  ...
  enum { MAGIC = 'CFHa', FREE_MAGIC = 'FreM' };
...
class CPackFileHandle {
 public:
  inline CPackFileHandle(CPackFile *pOwner, int64_t nBase, unsigned int nLength,
                         unsigned int nIndex = -1,
                         unsigned int nFilePointer = 0);
  ...
  inline void SetBufferSize(unsigned int nBytes);
  inline int GetSectorSize();
  inline int64_t AbsoluteBaseOffset();

 protected:
  int64_t m_nBase;              // Base offset of the file inside the pack file.
  unsigned int m_nFilePointer;  // Current seek pointer (0 based from the
...
class CPackFile : public CRefCounted<CRefCountServiceMT> {
 public:
  ...
  virtual CFileHandle *OpenFile(const char *pFileName,
                                const char *pOptions = "rb");

  // The two functions a pack file must provide
  virtual bool Prepare(int64_t fileLen = -1, int64_t nFileOfs = 0) = 0;
  virtual bool FindFile(const char *pFilename, int &nIndex, int64_t &nPosition,
                        int &nLength) = 0;

  // This is the core IO routine for reading anything from a pack file,
  // everything should go through here at some point
  virtual int ReadFromPack(int nIndex, void *buffer, int nDestBytes, int nBytes,
                           int64_t nOffset);

  // Returns the filename for a given file in the pack. Returns true if a
  // filename is found, otherwise buffer is filled with "unknown"
  ...
  const CUtlSymbol &GetPath() const {
    Assert(m_Path != UTL_INVAL_SYMBOL);
    return m_Path;
  }
  CUtlSymbol m_Path;

  // possibly embedded pack
  int64_t m_nBaseOffset;
...
class CZipPackFile : public CPackFile {
 public:
  ...
  // Loads the pack file
  virtual bool Prepare(int64_t fileLen = -1, int64_t nFileOfs = 0);
  virtual bool FindFile(const char *pFilename, int &nIndex, int64_t &nOffset,
                        int &nLength);
  virtual int ReadFromPack(int nIndex, void *buffer, int nDestBytes, int nBytes,
                           int64_t nOffset);

  int64_t GetPackFileBaseOffset() { return m_nBaseOffset; }
...
#pragma pack()
  ...
  class CPackFileLessFunc {
   public:
    bool Less(CPackFileEntry const &src1, CPackFileEntry const &src2,
              void *pCtx);
  };
  ...
  bool GetOffsetAndLength(const char *FileName, int &nBaseIndex,
  ...
  int64_t m_nPreloadSectionOffset;
...
class CFileLoadInfo {
 public:
  bool m_bSteamCacheOnly;  // If Steam and this is true, then the file is only
                           // looked for in the Steam caches.
  bool m_bLoadedFromSteamCache;  // If Steam, this tells whether the file was
...
the_interface CBaseFileSystem : public CTier1AppSystem<IFileSystem> {
 ...
 public:
  ...
  virtual void CacheFileCRCs(const char *pPathname, ECacheCRCType eType,
                             IFileList *pFilter);
  void CacheFileCRCs_R(const char *pPathname, ECacheCRCType eType,
                       IFileList *pFilter, CUtlDict<int, int> &searchPathNames);
  ...
  virtual FSAsyncStatus_t AsyncAppend(const char *pFileName, const void *pSrc,
    ...
    return AsyncWrite(pFileName, pSrc, nSrcBytes, bFreeMemory, true, pControl);
  }
  ...
  virtual FSAsyncStatus_t AsyncWriteFile(
      ...
      bool bFreeMemory, bool bAppend, FSAsyncControl_t *pControl);
  ...
  bool AddPackFileFromPath(const char *pPath, const char *pakfile,
                           bool bCheckForAppendedPack, const char *pathID);

  // converts a partial path into a full path
  // can be filtered to restrict path types and can provide info about resolved
  // path
  ...
  virtual void RecordBlockingFileAccess(bool synchronous,
  ...
  virtual bool FullPathToRelativePathEx(
      const char *pFullpath, const char *pPathId, char *pRelative, int maxlen);

  FSAsyncStatus_t SyncRead(const FileAsyncRequest_t &request);
  FSAsyncStatus_t SyncWrite(const char *pszFilename, const void *pSrc,
                            int nSrcBytes, bool bFreeMemory, bool bAppend);
  FSAsyncStatus_t SyncAppendFile(const char *pAppendToFileName,
                                 const char *pAppendFromFileName);
  FSAsyncStatus_t SyncGetFileSize(const FileAsyncRequest_t &request);
  void DoAsyncCallback(const FileAsyncRequest_t &request, void *pData,
                       int nBytesRead, FSAsyncStatus_t result);
  ...
  FSDirtyDiskReportFunc_t GetDirtyDiskReportFunc() {
    return m_DirtyDiskReportFunc;
  }
  ...
  class CSearchPathsVisits {
   ...
   private:
    CUtlVector<int> m_Visits;  // This is a copy of IDs for the search paths
                               // we've visited, so
  };

  class CSearchPathsIterator {
   public:
    CSearchPathsIterator(CBaseFileSystem *pFileSystem,
      ...
      if (*ppszFilename && !Q_IsAbsolutePath(*ppszFilename)) {
        ...
        // If it's an absolute path, it isn't worth using the paths at all.
        // Simplify client logic by pretending there's a search path of 1
  ...
  struct FindData_t {
    ...
    HANDLE findHandle;
    CSearchPathsVisits m_VisitedSearchPaths;  // This is a copy of IDs for the
                                              // search paths we've visited, so
                                              // avoids searching duplicate
                                              // paths.
    int m_CurrentStoreID;  // CSearchPath::m_storeId of the current search path.

    CUtlSymbol m_FilterPathID;  // What path ID are we looking at? Ignore all
                                // others. (Only set by FindFirstEx).

    CUtlDict<int, int> m_VisitedFiles;  // We go through the search paths in
                                        // priority order, and we use this to
                                        // make sure
 ...
 protected:
  ...
  // NOTE:  The symbol table could take up memory that we don't want to eat
  // here. In that case, we shouldn't store them in a table, or we should store
  // them as locally allocates stings
  ...
  static bool OpenedFileLessFunc(COpenedFile const &src1,
 ...
 public:
  void LogAccessToFile(char const *accesstype, char const *fullpath,
                       char const *options);
  void Warning(FileWarningLevel_t level, const char *fmt, ...);

 protected:
  // Note: if pFoundStoreID is passed in, then it will set that to the
  // CSearchPath::m_storeId value of the search path it found the file in.
  const char *FindFirstHelper(const char *pWildCard, const char *pPathID,
                              FileFindHandle_t *pHandle, int *pFoundStoreID);
  bool FindNextFileHelper(FindData_t * pFindData, int *pFoundStoreID);

  void RemoveAllMapSearchPaths();
  void AddMapPackFile(const char *pPath, const char *pPathID,
                      SearchPathAdd_t addType);
  void AddPackFiles(const char *pPath, const CUtlSymbol &pathID,
                    SearchPathAdd_t addType);
  bool PreparePackFile(CPackFile & packfile, int offsetofpackinmetafile,
                       int64_t filelen);

  // Goes through all the search paths (or just the one specified) and calls
  ...
  void ComputeFullWritePath(char *pDest, int maxlen, const char *pWritePathID,
                            char const *pRelativePath);

  void AddSearchPathInternal(const char *pPath, const char *pathID,
                             SearchPathAdd_t addType, bool bAddPackFiles);
  ...
  bool LookupKeyValuesRootKeyName(char const *filename, char const *pPathID,
                                  char *rootName, size_t bufsize);
  void UnloadCompiledKeyValues();

  // If bByRequestOnly is -1, then it will default to false if it doesn't
  // already exist, and it won't change it if it does already exist. Otherwise,
  // it will be set to the value of bByRequestOnly.
  CPathIDInfo *FindOrAddPathIDInfo(const CUtlSymbol &id, int bByRequestOnly);
  static bool FilterByPathID(const CSearchPath *pSearchPath,
                             const CUtlSymbol &pathID);

  // Global/shared filename/path table
  CUtlFilenameSymbolTable m_FileNames;

  // This manages most of the info we use for pure servers (whether files came
  // from Steam caches or off-disk, their CRCs, which ones are unverified, etc).
  CFileTracker m_FileTracker;
  int m_WhitelistFileTrackingEnabled;  // -1 if unset, 0 if disabled (single
                                       // player), 1 if enabled (multiplayer).
  FSDirtyDiskReportFunc_t m_DirtyDiskReportFunc;

  struct CompiledKeyValuesPreloaders_t {
    CompiledKeyValuesPreloaders_t() : m_CacheFile(0), m_pReader(0) {}
  ...
  // Pack exclude paths are strictly for 360 to allow holes in search paths and
  // pack files which fall through to support new or dynamic data on the host
  // pc.
...
inline bool CBaseFileSystem::FilterByPathID(const CSearchPath *pSearchPath,
                                            const CUtlSymbol &pathID) {
  if ((UtlSymId_t)pathID == UTL_INVAL_SYMBOL) {
    // They didn't specify a specific search path, so if this search path's path
    // ID is by request only, then ignore it.
    return pSearchPath->m_pPathIDInfo->m_bByRequestOnly;
  } else {
    // Bit of a hack, but specifying "BSP" as the search path will search in
    // "GAME" for only the map/.bsp pack file path
    if (pathID == m_BSPPathID) {
      if (pSearchPath->GetPathID() != m_GamePathID) return true;

      if (!pSearchPath->GetPackFile()) return true;

      if (!pSearchPath->GetPackFile()->m_bIsMapPath) return true;
...
inline CPackFileHandle::CPackFileHandle(CPackFile *pOwner, int64_t nBase,
                                        unsigned int nLength,
                                        unsigned int nIndex,
                                        unsigned int nFilePointer) {
  m_pOwner = pOwner;
  m_nBase = nBase;
  m_nLength = nLength;
  m_nIndex = nIndex;
...
inline void CPackFileHandle::SetBufferSize(unsigned int nBytes) {
...
inline int64_t CPackFileHandle::AbsoluteBaseOffset() {
...
class CAutoBlockReporter {
 public:
  CAutoBlockReporter(CBaseFileSystem *fs, bool synchronous,
                     char const *filename, int eBlockType, int nTypeOfAccess)
      : m_pFS(fs),
...
#define AUTOBLOCKREPORTER_FN(name, fs, sync, filename, blockType, accessType) \
