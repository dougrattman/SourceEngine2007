...
Color CGraphPanel::CGraphsImage::NetInColor =
    Color(255, 255, 0, 255);  // yellow
Color CGraphPanel::CGraphsImage::NetOutColor =
    Color(0, 255, 255, 255);  // light blue
Color CGraphPanel::CGraphsImage::PlayersColor =
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CGraphPanel::CGraphPanel(vgui::Panel *parent, const char *name)
  ...
  m_pGraphs->SetDraw(m_pCPUButton->IsSelected(), m_pFPSButton->IsSelected(),
                     m_pInButton->IsSelected(), m_pOutButton->IsSelected(),
                     m_pPINGButton->IsSelected(),
                     m_pPlayerButton->IsSelected());
...
//-----------------------------------------------------------------------------
void CGraphPanel::OnPageShow() { BaseClass::OnPageShow(); }

//-----------------------------------------------------------------------------
// Purpose: Hides the page
//-----------------------------------------------------------------------------
void CGraphPanel::OnPageHide() { BaseClass::OnPageHide(); }

//-----------------------------------------------------------------------------
// Purpose: called every frame to update stats page
//-----------------------------------------------------------------------------
void CGraphPanel::OnTick() {
  if (m_flNextStatsUpdateTime > system()->GetFrameTime()) return;
...
void CGraphPanel::OnServerDataResponse(const char *value,
  ...
  if (!_stricmp(value, "stats")) {
    ...
    char timeText[64];
    sprintf_s(timeText, "%i", (int)p.players);
...
//-----------------------------------------------------------------------------
// Purpose: the paint routine for the graph image. Handles the layout and
// drawing of the graph image
//-----------------------------------------------------------------------------
void CGraphPanel::CGraphsImage::Paint() {
  int x, y;
  float distPoints;  // needs to be a float, rounding errors cause problems with
                     // lots of points
  ...
  if (!cpu && !fps && !net_i && !net_o && !ping &&
      !players)  // no graphs selected
    return;

  if (points.Count() < 2) return;  // not enough points yet...

  if (x <= 200 || y <= 100) return;  // to small
  ...
  if (ping) {
    RangePing += static_cast<float>(
        maxPing *
        0.1);  // don't let the top of the range touch the top of the panel

    if (RangePing <= 1) {  // don't let the zero be at the top of the screen
      RangePing = 1.0;
    }
  ...
  if (fps) {
    RangeFPS += static_cast<float>(
        maxFPS *
        0.1);  // don't let the top of the range touch the top of the panel

    if (RangeFPS <= 1) {  // don't let the zero be at the top of the screen
      RangeFPS = 1.0;
    }
  ...
  if (net_i) {
    ...
    if (Range <= 1) {  // don't let the zero be at the top of the screen
      Range = 1.0;
    }
  ...
  if (players) {
    RangePlayers += static_cast<float>(
        maxPlayers *
        0.1);  // don't let the top of the range touch the top of the panel
  ...
  // draw the graphs, left to right
  {
    ...
    if (net_o) {
      ...
      pOutY[i] =
          static_cast<int>(((Range - points[i].out) / Range) * y - bottom);
    }

    if (fps) {
      pFPSX[i] = left + static_cast<int>(i * distPoints);
      pFPSY[i] = static_cast<int>(((RangeFPS - points[i].fps) / RangeFPS) * y -
                                  bottom);
    }

    if (ping) {
      pPingX[i] = left + static_cast<int>(i * distPoints);
      pPingY[i] = static_cast<int>(
          ((RangePing - points[i].ping) / RangePing) * y - bottom);
    }

    if (players) {
      pPlayersX[i] = left + static_cast<int>(i * distPoints);
      pPlayersY[i] = static_cast<int>(
          ((RangePlayers - points[i].players) / RangePlayers) * y - bottom);
    }
  }
  // we use DrawPolyLine, its much, much, much more efficient than calling lots
  // of DrawLine()'s
...
//-----------------------------------------------------------------------------
void CGraphPanel::CGraphsImage::SetDraw(bool cpu_in, bool fps_in, bool net_in,
                                        bool net_out, bool ping_in,
                                        bool players_in) {
  cpu = cpu_in;
  fps = fps_in;
  net_i = net_in;
...
bool CGraphPanel::CGraphsImage::AddPoint(Points_t p) {
  ...
  // there are more points than pixels so thin them out
  {
    while (points.Count() > x / 2) {
      ...
      if (points[0].in == maxIn || points[0].out == maxOut ||
          points[0].fps == maxFPS || points[0].ping == maxPing ||
...
void CGraphPanel::CGraphsImage::SetScale(intervals time) {
  ...
  // and reset the maxes
  maxIn = maxOut = minIn = minOut = minFPS = maxFPS = minPing = maxPing =
...
//-----------------------------------------------------------------------------
void CGraphPanel::OnClearButton() { m_pGraphs->RemovePoints(); }

//-----------------------------------------------------------------------------
// Purpose: passes the state of the check buttons (for graph line display)
// through to the graph image
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose:Handles the scale radio buttons, passes the scale to use through to
// the graph image
//-----------------------------------------------------------------------------
...
// Purpose:
//-----------------------------------------------------------------------------
void CGraphPanel::SetAxisLabels(Color c, const char *max, const char *mid,
                                const char *min) {
