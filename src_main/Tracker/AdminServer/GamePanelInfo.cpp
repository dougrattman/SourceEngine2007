...
static const long MAP_CHANGE_TIME =
    20000;  // refresh 20 seconds after a map change
static const long RESTART_TIME =
    60000;  // refresh 60 seconds after a "_restart"
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CGamePanelInfo::CGamePanelInfo(vgui::Panel *parent, const char *name,
                               const char *mod)
    : Frame(parent, name) {
  SetSize(560, 420);
  ...
  // Each image file is named ss1, ss2, ss3... ss20, one image for each frame of
  // the animation. This loads the 20 images in to the Animation class.
  ...
  if (m_pBudgetPanel) m_pDetailsSheet->AddPage(m_pBudgetPanel, "#Game_Budgets");
...
//-----------------------------------------------------------------------------
void CGamePanelInfo::SetNewTitle(bool connectionFailed,
  ...
  g_pVGuiLocalize->ConvertANSIToUnicode(additional_text, serverName,
                                        sizeof(serverName));
  wchar_t title[256];
  g_pVGuiLocalize->ConstructString(title, sizeof(title),
                                   g_pVGuiLocalize->Find(localized_title), 1,
                                   serverName);
...
void CGamePanelInfo::OnStop() {
  QueryBox *box =
      new QueryBox("#Game_Stop_Server_Title", "#Game_Restart_Server");
...
void CGamePanelInfo::ActivateBuildMode() {
  ...
  EditablePanel *pg =
      dynamic_cast<EditablePanel *>(m_pDetailsSheet->GetActivePage());
  if (pg) {
...
void CGamePanelInfo::OnMasterRequestRestart() {
  ...
  if (!m_hRestartQueryBox.Get()) {
    m_hRestartQueryBox = new QueryBox(
