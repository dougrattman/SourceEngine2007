...
//-----------------------------------------------------------------------------
// Purpose: Configures and installs the connection to the game server (remote or
// local)
//-----------------------------------------------------------------------------
class CRemoteServer {
 public:
  ...
  // remote connection
  void ConnectRemoteGameServer(unsigned int ip, unsigned short port,
                               const char *password);
  ...
  void ChangeAccessPassword(IServerDataResponse *requester,
  ...
  // information channels from the server->client (map changed, player joined,
  // etc.)
...
class IServerDataResponse {
 public:
  ...
  virtual void OnServerDataResponse(const char *value,
