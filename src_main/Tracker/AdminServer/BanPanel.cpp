...
void CBanPanel::RemoveBan() {
  ...
  if (kv != nullptr) {
    ...
    wchar_t message[256];
    g_pVGuiLocalize->ConstructString(message, sizeof(message),
                                     g_pVGuiLocalize->Find("#Ban_Remove_Msg"),
                                     1, id);

    // activate the confirmation dialog
    QueryBox *box =
        new QueryBox(g_pVGuiLocalize->Find("#Ban_Title_Remove"), message);
    box->SetOKCommand(
        new KeyValues("removebanbyid", "id", kv->GetString("id")));
    box->AddActionSignalTarget(this);
    box->DoModal();
  }
}
...
void CBanPanel::ChangeBan() {
  ...
  if (kv != nullptr) {
    ...
    box->Activate(kv->GetString("id"), timeText, "changeban",
                  "#Ban_Change_Time");
  }
}
...
void CBanPanel::ChangeBanTimeByID(const char *id, const char *newtime) {
  ...
  // send down the command
  char cmd[512];
  sprintf_s(cmd, "%s %s %s\n", IsIPAddress(id) ? "addip" : "banid", newtime,
            id);
  RemoteServer().SendCommand(cmd);
  if (IsIPAddress(id)) {
    RemoteServer().SendCommand("writeip");
...
void CBanPanel::ImportBanList() {
  ...
  FileOpenDialog *pFileDialog =
      new FileOpenDialog(this, "#Ban_Find_Ban_File", true);
