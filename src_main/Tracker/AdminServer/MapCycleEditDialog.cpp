...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMapCycleEditDialog::CMapCycleEditDialog(vgui::Panel *parent, const char *name)
...
//-----------------------------------------------------------------------------
void CMapCycleEditDialog::Activate(vgui::Panel *updateTarget,
  ...
  for (int i = 0; i < availableMaps.Count(); i++) {
    ...
    if (!inMapCycle) {
      m_pAvailableMapList->AddItem(
          new KeyValues("MapItem", "Map", availableMaps[i].String()), 0, false,
          false);
    }
  }
  for (int i = 0; i < mapCycle.Count(); i++) {
    m_pMapCycleList->AddItem(
        new KeyValues("MapItem", "Map", mapCycle[i].String()), 0, false, false);
  }
...
void CMapCycleEditDialog::OnItemSelected(vgui::Panel *panel) {
  if (panel == m_pAvailableMapList &&
      m_pAvailableMapList->GetSelectedItemsCount() > 0) {
    m_pMapCycleList->ClearSelectedItems();
  } else if (panel == m_pMapCycleList &&
             m_pMapCycleList->GetSelectedItemsCount() > 0) {
...
void CMapCycleEditDialog::OnCommand(const char *command) {
  if (!_stricmp(command, "ArrowLeft")) {
    ...
    while (m_pMapCycleList->GetSelectedItemsCount() > 0) {
      ...
      m_pAvailableMapList->AddItem(new KeyValues("MapItem", "Map", map), 0,
                                   true, false);
    ...
    while (m_pAvailableMapList->GetSelectedItemsCount() > 0) {
      ...
      const char *map = data->GetString("Map");
      m_pMapCycleList->AddItem(new KeyValues("MapItem", "Map", map), 0, true,
                               false);
    ...
    int row = m_pMapCycleList->GetItemCurrentRow(itemID);
    int nextRow = row + 1;
    if (nextRow + 1 > m_pMapCycleList->GetItemCount()) return;
