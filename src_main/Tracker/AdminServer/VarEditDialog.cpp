...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CVarEditDialog::CVarEditDialog(vgui::Panel *parent, const char *name)
    : Frame(parent, name) {
  SetSize(280, 180);
  SetSizeable(false);
...
//-----------------------------------------------------------------------------
void CVarEditDialog::Activate(vgui::Panel *actionSignalTarget,
                              KeyValues *rules) {
  // configure
  ...
  if (!_stricmp(type, "enumeration")) {
    ...
    while (*parse) {
      ...
      // copy in the name
      strncpy_s(customString, parse, nameSize);
      ...
      // activate the current item
      if (!_stricmp(customString, currentValue)) {
