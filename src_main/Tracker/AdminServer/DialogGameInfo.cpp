...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDialogGameInfo::CDialogGameInfo(IGameList *gameList, unsigned int serverID,
                                 int serverIP, int serverPort)
  ...
  m_pAutoRetryAlert =
      new RadioButton(this, "AutoRetryAlert",
                      "A&lert me when a player slot is available on server.");
  m_pAutoRetryJoin = new RadioButton(
      this, "AutoRetryJoin",
      "J&oin the server as soon as a player slot is available.");
...
void CDialogGameInfo::PerformLayout() {
  ...
  if (server.ip[0] && server.port) {
    char buf[64];
    sprintf(buf, "%d.%d.%d.%d:%d", server.ip[0], server.ip[1], server.ip[2],
  ...
  if (m_pAutoRetry->IsSelected()) {
    if (server.players < server.maxPlayers) {
      ...
      m_pInfoLabel->SetText(
          "You will join the server as soon as a player slot is free.");
    } else {
      m_pInfoLabel->SetText(
          "You will be alerted as soon player slot is free on the server.");
    }
  } else if (m_bServerFull) {
    m_pInfoLabel->SetText("Could not connect - server is full.");
  } else if (m_bServerNotResponding) {
    char text[100];
    _snprintf(text, 100, "Server is not responding.%d.%d.%d.%d:%d",
...
void CDialogGameInfo::OnConnect() {
  ...
  // need to refresh server before attempting to connect, to make sure there is
  // enough room on the server
  RequestInfo();
}
...
//-----------------------------------------------------------------------------
// Purpose: Forces the whole dialog to redraw when the auto-retry button is
// toggled
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CDialogGameInfo::SetControlText(const char *textEntryName,
                                     const char *text) {
...
MessageMapItem_t CDialogGameInfo::m_MessageMap[] = {
    ...
    MAP_MESSAGE_PTR(CDialogGameInfo, "RadioButtonChecked", OnButtonToggled,
                    "panel"),

    MAP_MESSAGE_CONSTCHARPTR(CDialogGameInfo, "JoinServerWithPassword",
                             OnJoinServerWithPassword, "password"),
};
