...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CPlayerPanel::CPlayerPanel(vgui::Panel *parent, const char *name)
    : vgui::PropertyPage(parent, name) {
  m_pPlayerListPanel = new vgui::ListPanel(this, "Players list");

  m_pPlayerListPanel->AddColumnHeader(0, "name", "#Player_Panel_Name", 200,
...
static const char *FormatSeconds(int seconds) {
  ...
  if (hours > 0) {
    Q_snprintf(string, sizeof(string), "%2i:%02i:%02i", hours, minutes,
               seconds);
  } else {
  ...
  if (!_stricmp(value, "UpdatePlayers")) {
    ...
    while (parse && *parse) {
      ...
      ivgui()->DPrintf2("pars:  \"%s\" %s %s %d %d %d %s\n", name, authID,
                        netAdr, ping, packetLoss, frags, timeStr);
...
void CPlayerPanel::OnKickButtonPressed() {
  if (m_pPlayerListPanel->GetSelectedItemsCount() < 1) return;

  // open a message box to ask the user if they want to follow through on this
  QueryBox *box;
  if (m_pPlayerListPanel->GetSelectedItemsCount() > 1) {
    box = new QueryBox("#Kick_Multiple_Players_Title",
                       "#Kick_Multiple_Players_Question");
  } else {
    // show the player name in the message box if only one player is being
    // booted
    KeyValues *kv =
        m_pPlayerListPanel->GetItem(m_pPlayerListPanel->GetSelectedItem(0));
    Assert(kv != NULL);
    if (!kv) return;
    ...
    g_pVGuiLocalize->ConstructString(
        msg, sizeof(msg), g_pVGuiLocalize->Find("Kick_Single_Player_Question"),
        1, playerName);
