...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CVarListPropertyPage::CVarListPropertyPage(vgui::Panel *parent,
                                           const char *name)
...
bool CVarListPropertyPage::LoadVarList(const char *varfile) {
  ...
  if (dat->LoadFromFile(g_pFullFileSystem, varfile, NULL)) {
    // enter into list
    for (KeyValues *rule = dat->GetFirstSubKey(); rule != NULL;
         rule = rule->GetNextKey()) {
...
//-----------------------------------------------------------------------------
void CVarListPropertyPage::SetVarString(const char *varName,
                                        const char *value) {
...
//-----------------------------------------------------------------------------
void CVarListPropertyPage::SetCustomStringList(const char *varName,
...
void CVarListPropertyPage::OnVarChanged(const char *var) {
  ...
  // process the queue immediately, since if we're running a local server there
  // will already be a reply
  RemoteServer().ProcessServerResponse();
}
...
//-----------------------------------------------------------------------------
void CVarListPropertyPage::OnServerDataResponse(const char *value,
                                                const char *response) {
