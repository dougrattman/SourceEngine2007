...
void CRemoteServer::Initialize() {
  ...
  m_ListenerID = g_pGameServerData->GetNextListenerID(
      false);  // don't require auth on this connection
...
//-----------------------------------------------------------------------------
void CRemoteServer::ConnectRemoteGameServer(unsigned int ip,
                                            unsigned short port,
                                            const char *password) {
...
//-----------------------------------------------------------------------------
void CRemoteServer::RequestValue(IServerDataResponse *requester,
                                 const char *variable) {
...
bool CRemoteServer::ProcessServerResponse() {
  ...
  while (1) {
    ...
    int bytesRead = g_pGameServerData->ReadDataResponse(m_ListenerID, charbuf,
                                                        sizeof(charbuf));
    ...
    switch (responseType) {
      case SERVERDATA_RESPONSE_VALUE: {
        ...
        for (int i = m_ResponseHandlers.Head();
             m_ResponseHandlers.IsValidIndex(i);
             i = m_ResponseHandlers.Next(i)) {
          if (m_ResponseHandlers[i].requestID == requestID) {
            // found, call
            m_ResponseHandlers[i].handler->OnServerDataResponse(
                variable, (const char *)value.Base());
...
//-----------------------------------------------------------------------------
void CRemoteServer::AddServerMessageHandler(IServerDataResponse *handler,
                                            const char *watch) {
...
//-----------------------------------------------------------------------------
// Purpose: removes a requester from the list to guarantee the pointer won't be
// used
//-----------------------------------------------------------------------------
void CRemoteServer::RemoveServerDataResponseTarget(
