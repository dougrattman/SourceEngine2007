...
class CBudgetPanelAdmin : public CBudgetPanelShared {
 ...
 public:
  ...
  void DrawColoredText(vgui::HFont font, int x, int y, int r, int g, int b,
                       int a, const char *pText, ...);

 private:
  Color m_budgetTextColor;
};

CBudgetPanelAdmin::CBudgetPanelAdmin(vgui::Panel *pParent,
...
void CBudgetPanelAdmin::DrawColoredText(vgui::HFont font, int x, int y, int r,
...
void CBudgetPanelAdmin::ApplySchemeSettings(vgui::IScheme *pScheme) {
  ...
  m_budgetTextColor =
      pScheme->GetColor("BrightControlText", Color(0, 255, 0, 255));
}

void CBudgetPanelAdmin::PostChildPaint() {
  DrawColoredText(m_hFont, 0, 0, m_budgetTextColor[0], m_budgetTextColor[1],
                  m_budgetTextColor[2], m_budgetTextColor[3],
                  "%i fps (showbudget 3D driver time included)",
                  RoundFloatToInt(g_fFrameRate));
  DrawColoredText(m_hFont, 0, 16, m_budgetTextColor[0], m_budgetTextColor[1],
                  m_budgetTextColor[2], m_budgetTextColor[3], "%.1f ms",
...
CBudgetPanelContainer::CBudgetPanelContainer(vgui::Panel *parent,
                                             const char *name)
...
void CBudgetPanelContainer::OnServerDataResponse(const char *value,
                                                 const char *response) {}
...
void CBudgetPanelContainer::OnPageShow() {
  if (g_pVProfExport) g_pVProfExport->AddListener();
...
void CBudgetPanelContainer::OnPageHide() {
  if (g_pVProfExport) g_pVProfExport->RemoveListener();
