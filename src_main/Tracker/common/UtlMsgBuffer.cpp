...
enum {
  ...
  PACKBIT_INTNAME = 0x02,     // if this is set then it's an int named variable,
                              // instead of a string
  PACKBIT_BINARYDATA =
      0x04,  // signifies the data in this variable is binary, it's not a string
};

CUtlMsgBuffer::CUtlMsgBuffer(unsigned short msgID, int initialSize)
    : m_Memory(0, initialSize) {
  m_iMsgID = msgID;
  m_iWritePos = 0;
  m_iReadPos = 0;
  m_iNextVarPos = 0;
...
CUtlMsgBuffer::CUtlMsgBuffer(unsigned short msgID, void const *data,
                             int dataSize)
    : m_Memory(0, dataSize) {
  m_iMsgID = msgID;
...
void CUtlMsgBuffer::WriteBlob(const char *name, const void *data,
                              int dataSize) {
  // write out the variable type
  unsigned char vtype =
      PACKBIT_CONTROLBIT | PACKBIT_BINARYDATA;  // stringname var, binary data
  Write(&vtype, 1);
...
bool CUtlMsgBuffer::Read(void *buffer, int readAmount) {
  if (m_iReadPos + readAmount >= m_iWritePos) return false;
...
//			assumes enough space has already been allocated for the
// write
...
int CUtlMsgBuffer::ReadBlob(const char *name, void *data, int dataBufferSize) {
  ...
  if (dataSize > dataBufferSize) dataSize = dataBufferSize;
...
bool CUtlMsgBuffer::ReadNextVar(char varname[32], bool &stringData, void *data,
                                int &dataSize) {
  // read the type
  unsigned char vtype = 1;
  if (!Read(&vtype, 1)) return false;
  ...
  if (!ReadUntilNull(varname, 31)) return false;
  ...
  // store of the next position, since that is probably where the next read
  // needs to occur
  m_iReadPos += size;
...
//			returns pointer to buffer position on success, NULL if not
//found
void *CUtlMsgBuffer::FindVar(const char *name, int &dataSize) {
  ...
  while (loopCount--) {
    ...
    while (Read(&vtype, 1)) {
      ...
      // read the variable name
      char varname[32];
      if (!ReadUntilNull(varname, 31)) break;
      ...
      if (!stricmp(varname, name)) {
        ...
        // store of the next position, since that is probably where the next
        // read needs to occur
        m_iReadPos += size;
        m_iNextVarPos = m_iReadPos;
        return data;
      ...
      m_iReadPos += size;
      if (m_iReadPos >= m_iWritePos) break;
    }
