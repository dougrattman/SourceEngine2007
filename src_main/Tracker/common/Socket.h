...
#define MAX_RETRIES \
  2  // the number of fragments from other packets to drop before we declare the
     // outstanding fragment lost :)

// Purpose: Instances a message handler for incoming messages.
class CMsgHandler {
 public:
  enum { MAX_HANDLER_STRING = 64 };
 ...
 private:
  // Internal message received, crack type info and check it before calling
  // process
...
class CSocket {
 public:
  ...
  // Send the message to the recipient, if msg == NULL, use the internal message
  // buffer
  virtual int SendMessage(netadr_t *to, CMsgBuffer *msg = NULL);
  // Broadcast the message on the specified port, if msg == NULL use the
 ...
 private:
  ...
  // Socket to which non Broadcast SendMessage was directed.  The socket will
  // wait for a response
  //  from that exact address
  netadr_t m_ToAddress;
  // Set to true if the send was a Broadcast, and therefore from != to address
  // is okay
