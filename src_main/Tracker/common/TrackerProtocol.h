...
// Purpose: List of all the tracker messages used msgID's are 32bits big
enum TrackerMsgID_t {
  // generic messages
  ...
  TSVC_CONNECTIONKEEPALIVE,  // information that an attemption connect is taking
                             // time, and the user should wait
  TSVC_ROUTEMESSAGEFAILED,   // chat message failed to be routed through the
                             // servers
  TSVC_REDIRECTLOGIN,  // tells the client to redirect their login attempt to a
                       // different server
...
enum TrackerLogoffReason_t {
  ...
  TRACKER_LOGOFF_LOGGEDINELSEWHERE,  // user has logged into friends at a
                                     // different location
  TRACKER_LOGOFF_SERVERWORK,      // server needs to do work (like lock the user
                                  // range)
  ...
  TRACKER_LOGOFF_FIREWALL,      // users' firewall won't allow enough packets
                                // through
  TRACKER_LOGOFF_NOTCONNECTED,  // user sent server a packet that implied they
                                // think they're logged in but they're not
  ...
  TRACKER_LOGOFF_SERVERMESSAGEFAIL,  // a message to the server was not
                                     // successfully transmitted

  TRACKER_LOGOFF_TOOMANYATTEMPTS,  // too many login attempts have been
                                   // performed
