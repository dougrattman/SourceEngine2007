...
class CUtlMsgBuffer {
 public:
  ...
  // returns a pointer to the data buffer, and its size, of the specified
  // variable
 ...
 private:
  ...
  short m_iNextVarPos;  // a guess at which variable is most likely to be read
                        // next
};
...
inline int CUtlMsgBuffer::DataSize() const {
  ...
  if (m_iWritePos > m_iReadPos) return m_iWritePos;

  return m_iReadPos;
}

// Purpose: returns a pointer to the base data
inline void *CUtlMsgBuffer::Base() { return &m_Memory[0]; }

// Purpose: returns a pointer to the base data
inline const void *CUtlMsgBuffer::Base() const { return &m_Memory[0]; }
...
// Purpose: Reads in a named 4-byte int, returns true on sucess, false on
// failure
inline bool CUtlMsgBuffer::ReadInt(const char *name, int &data) {
  return (ReadBlob(name, &data, 4) == 4);
}

// Purpose: Reads in a named 4-byte unsigned int, returns true on sucess, false
// on failure
...
inline bool CUtlMsgBuffer::ReadString(const char *name, char *data,
                                      int dataBufferSize) {
...
inline void CUtlMsgBuffer::SetWritePos(int size) { m_iWritePos = size; }
