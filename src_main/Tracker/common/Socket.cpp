...
static u32 WINAPI SocketThreadFunc(void *threadobject) {
  ...
  while (1) {
    ...
    if (WAIT_OBJECT_0 ==
        VCRHook_WaitForSingleObject(socketthread->GetShutdownHandle(), 0)) {
      break;
    }
    ...
    count = 0;
    for (sockets = socketthread->GetSocketList(); sockets;
...
void CSocketThread::AddSocketToThread(CSocket *socket) {
  ...
  if (!m_hThread) {
    m_hThread = VCRHook_CreateThread(NULL, 0, SocketThreadFunc, (void *)this, 0,
                                     &m_nThreadId);
    assert(m_hThread);
  }
...
CMsgHandler::CMsgHandler(HANDLERTYPE type, void *typeinfo /*=NULL*/) {
  ...
  switch (m_Type) {
    ...
    case MSGHANDLER_STRING:
      strcpy_s(m_szString, (char *)typeinfo);
      break;
  }
}
...
// Input  : *next -
//-----------------------------------------------------------------------------
void CMsgHandler::SetNext(CMsgHandler *next) { m_pNext = next; }
...
// Input  : *socket -
//-----------------------------------------------------------------------------
void CMsgHandler::SetSocket(CSocket *socket) { m_pSocket = socket; }

//-----------------------------------------------------------------------------
// Purpose: Creates a non-blocking, broadcast capable, UDP socket.  If port is
//  specified, binds it to listen on that port, otherwise, chooses a random
//  port.
//-----------------------------------------------------------------------------
CSocket::CSocket(const char *socketname, int port /*= -1*/)
    : m_SendBuffer(socketname) {
  ...
  if (setsockopt(m_Socket, SOL_SOCKET, SO_BROADCAST, (char *)&i, sizeof(i)) ==
      -1) {
    closesocket(m_Socket);
    m_Socket = 0;
    return;
  ...
  // if (!net_interface || !net_interface[0] || !_stricmp(net_interface,
  // "localhost"))
...
int CSocket::SendMessage(netadr_t *to, CMsgBuffer *msg /*= NULL*/) {
  ...
  int bytessent = sendto(m_Socket, (const char *)msg->GetData(),
                         msg->GetCurSize(), 0, &addr, sizeof(addr));
...
void CSocket::Frame() {
  ...
  for (int i = 0; i < m_MsgBuffers.Size(); i++) {
    ...
    while (handler) {
      // Swallow message?
      if (handler->ProcessMessage(&addr, &m_MsgBuffers[i])) break;
...
// Input  : userData -
//-----------------------------------------------------------------------------
void CSocket::SetUserData(unsigned int userData) { m_nUserData = userData; }
...
bool CSocket::ReceiveData() {
  ...
  fromlen = sizeof(from);
  bytes = VCRHook_recvfrom(m_Socket, (char *)buffer, CMsgBuffer::NET_MAXMESSAGE,
  ...
  // Mark the time no matter what since FD_SET said there was data and we should
  // have it now
  ...
  if (*(int *)&buffer[0] == -2)  // its a split packet :)
  {
    ...
    if (m_iTotalPackets == 0)  // this is the first in the series
    {
      ...
      m_iCurrentPackets = 1;  // packet numbers start at zero, total is the
                              // total number (i.e =2 for packet 0,1)
      ...
      m_iRetries++;
      if (m_iRetries >
          MAX_RETRIES)  // make sure we give up eventually on fragments
      {
        m_iTotalPackets = 0;
      }
    ...
    if (m_iCurrentPackets == m_iTotalPackets) {
      ...
      for (int i = 0; i < m_iTotalPackets; i++) {
        // buffer must be big enough for us to use, that is where the data
