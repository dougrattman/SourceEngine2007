...
void DecommitDMXMemory() { s_DMXAllocator.FreeAll(true); }
...
class CDmxSerializer {
 ...
 private:
  ...
  bool ShouldWriteAttribute(const char *pAttributeName,
                            CDmxAttribute *pAttribute);
  void SerializeElementIndex(CUtlBuffer &buf, CDmxSerializationDictionary &list,
                             CDmxElement *pElement);
  void SerializeElementAttribute(CUtlBuffer &buf,
                                 CDmxSerializationDictionary &list,
                                 CDmxAttribute *pAttribute);
  void SerializeElementArrayAttribute(CUtlBuffer &buf,
                                      CDmxSerializationDictionary &list,
                                      CDmxAttribute *pAttribute);
  bool SaveElementDict(CUtlBuffer &buf, CUtlRBTree<const char *> &stringTable,
                       CDmxElement *pElement);
  ...
  CDmxElement *UnserializeElementIndex(CUtlBuffer &buf,
                                       CUtlVector<CDmxElement *> &elementList);
  void UnserializeElementAttribute(CUtlBuffer &buf, CDmxAttribute *pAttribute,
                                   CUtlVector<CDmxElement *> &elementList);
  void UnserializeElementArrayAttribute(CUtlBuffer &buf,
                                        CDmxAttribute *pAttribute,
                                        CUtlVector<CDmxElement *> &elementList);
  bool UnserializeAttributes(CUtlBuffer &buf, CDmxElement *pElement,
                             CUtlVector<CDmxElement *> &elementList,
                             int nStrings, int *offsetTable, char *stringTable);
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::ShouldWriteAttribute(const char *pAttributeName,
                                          CDmxAttribute *pAttribute) {
  if (!pAttribute) return false;

  // These are already written in the initial element dictionary
  if (!Q_stricmp(pAttributeName, "name")) return false;
...
//-----------------------------------------------------------------------------
void CDmxSerializer::SerializeElementIndex(CUtlBuffer &buf,
                                           CDmxSerializationDictionary &list,
                                           CDmxElement *pElement) {
  if (!pElement) {
...
//-----------------------------------------------------------------------------
void CDmxSerializer::SerializeElementAttribute(
    CUtlBuffer &buf, CDmxSerializationDictionary &list,
    CDmxAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
void CDmxSerializer::SerializeElementArrayAttribute(
    CUtlBuffer &buf, CDmxSerializationDictionary &list,
    CDmxAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::SaveElement(CUtlBuffer &buf,
                                 CDmxSerializationDictionary &list,
                                 CUtlRBTree<const char *> &stringTable,
                                 CDmxElement *pElement) {
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::Serialize(CUtlBuffer &buf, CDmxElement *pRoot,
  ...
  for (i = dict.FirstRootElement(); i != DMX_SERIALIZATION_HANDLE_INVALID;
  ...
  if (nStrings > 65535) return false;
...
//-----------------------------------------------------------------------------
CDmxElement *CDmxSerializer::UnserializeElementIndex(
    CUtlBuffer &buf, CUtlVector<CDmxElement *> &elementList) {
  ...
  if (nElementIndex < 0 || !elementList[nElementIndex]) return NULL;
...
//-----------------------------------------------------------------------------
void CDmxSerializer::UnserializeElementAttribute(
...
//-----------------------------------------------------------------------------
void CDmxSerializer::UnserializeElementArrayAttribute(
    CUtlBuffer &buf, CDmxAttribute *pAttribute,
    CUtlVector<CDmxElement *> &elementList) {
  int nElementCount = buf.GetInt();
  CUtlVector<CDmxElement *> &elementArray =
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::UnserializeAttributes(
    CUtlBuffer &buf, CDmxElement *pElement,
    CUtlVector<CDmxElement *> &elementList, int nStrings, int *offsetTable,
    char *stringTable) {
...
int CDmxSerializer::GetStringOffsetTable(CUtlBuffer &buf, int *offsetTable,
                                         int nStrings) {
  ...
  for (int i = 0; i < nStrings; ++i) {
    ...
    do {
      // grow/shift utlbuffer if it hasn't loaded the entire string table into
      // memory
...
//-----------------------------------------------------------------------------
bool CDmxSerializer::Unserialize(CUtlBuffer &buf, int nEncodingVersion,
                                 CDmxElement **ppRoot) {
  ...
  *ppRoot = elementList[0];
  ...
  for (int i = 0; i < nElementCount; ++i) {
    UnserializeAttributes(buf, elementList[i], elementList, nStrings,
                          offsetTable, stringTable);
  }
...
bool SerializeDMX(CUtlBuffer &buf, CDmxElement *pRoot, const char *pFileName) {
  ...
  int nEncodingVersion =
      buf.IsText() ? 1 : 2;  // HACK - we should have some way of automatically
                             // updating this when the encoding version changes!
  const char *pFormatName = GENERIC_DMX_FORMAT;
  int nFormatVersion = 1;  // HACK - we should have some way of automatically
                           // updating this when the encoding version changes!
...
bool SerializeDMX(const char *pFileName, const char *pPathID, bool bTextMode,
  ...
  CUtlStreamBuffer buf(pFullPath, pPathID,
                       bTextMode ? CUtlBuffer::TEXT_BUFFER : 0);
  if (!buf.IsValid()) {
...
//-----------------------------------------------------------------------------
bool ReadDMXHeader(CUtlBuffer &buf, char *pEncodingName, int nEncodingNameLen,
                   int &nEncodingVersion, char *pFormatName, int nFormatNameLen,
                   int &nFormatVersion) {
  ...
  bool bOk = buf.ParseToken(DMX_VERSION_STARTING_TOKEN,
                            DMX_VERSION_ENDING_TOKEN, header, sizeof(header));
  if (bOk) {
#ifdef _WIN32
  ...
  if (!bOk) {
    ...
    bOk = buf.ParseToken(DMX_LEGACY_VERSION_STARTING_TOKEN,
                         DMX_LEGACY_VERSION_ENDING_TOKEN, pFormatName,
                         sizeof(pFormatName));
    if (bOk) {
      nEncodingVersion = 0;
      nFormatVersion =
          0;  // format version is encoded in the format name string

      if (!V_stricmp(pFormatName, "binary_v1") ||
          !V_stricmp(pFormatName, "binary_v2")) {
        bOk = !bBufIsText;
        V_strncpy(pEncodingName, "binary", nEncodingNameLen);
      } else if (!V_stricmp(pFormatName, "keyvalues2_v1") ||
                 !V_stricmp(pFormatName, "keyvalues2_flat_v1")) {
        bOk = bBufIsText;
...
bool UnserializeDMX(CUtlBuffer &buf, CDmxElement **ppRoot,
                    const char *pFileName) {
  // NOTE: Checking the format name string for a version check here is how you'd
  // do it
  *ppRoot = NULL;
  ...
  if (!ReadDMXHeader(buf, pEncodingName, sizeof(pEncodingName),
                     nEncodingVersion, pFormatName, sizeof(pFormatName),
                     nFormatVersion))
    return false;
