...
//-----------------------------------------------------------------------------
CUtlSymbol CDmxElement::GetType() const { return m_Type; }
...
const DmObjectId_t &CDmxElement::GetId() const { return m_Id; }

//-----------------------------------------------------------------------------
// Sets the object id
//-----------------------------------------------------------------------------
void CDmxElement::SetId(const DmObjectId_t &id) { CopyUniqueId(id, &m_Id); }
...
void CDmxElement::Resort() const {
  if (m_bResortNeeded) {
    ...
    for (int i = nCount; --i >= 1;) {
      if (m_Attributes[i]->GetNameSymbol() ==
          m_Attributes[i - 1]->GetNameSymbol()) {
...
CDmxAttribute *CDmxElement::AddAttribute(const char *pAttributeName) {
  ...
  if (nIndex >= 0) return m_Attributes[nIndex];
...
void CDmxElement::RemoveAttributeByPtr(CDmxAttribute *pAttribute) {
  ...
  for (int i = 0; i < nCount; ++i) {
    if (m_Attributes[i] != pAttribute) continue;
...
// Rename an attribute
//-----------------------------------------------------------------------------
void CDmxElement::RenameAttribute(const char *pAttributeName,
                                  const char *pNewName) {
  CDmxElementModifyScope modify(this);

  // No change...
  if (!Q_stricmp(pAttributeName, pNewName)) return;
...
CDmxAttribute *CDmxElement::GetAttribute(const char *pAttributeName) {
  int idx = FindAttribute(pAttributeName);
  if (idx >= 0) return m_Attributes[idx];
  return NULL;
}

const CDmxAttribute *CDmxElement::GetAttribute(
...
//-----------------------------------------------------------------------------
int CDmxElement::AttributeCount() const { return m_Attributes.Count(); }
...
//-----------------------------------------------------------------------------
void CDmxElement::AddElementsToDelete(
    CUtlVector<CDmxElement *> &elementsToDelete) {
  if (m_bIsMarkedForDeletion) return;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (pAttribute->GetType() == AT_ELEMENT_ARRAY) {
      const CUtlVector<CDmxElement *> &elements =
...
//-----------------------------------------------------------------------------
void CDmxElement::UnpackIntoStructure(
    void *pData, const DmxElementUnpackStructure_t *pUnpack) const {
  for (; pUnpack->m_AttributeType != AT_UNKNOWN; ++pUnpack) {
    ...
    if (IsArrayType(pUnpack->m_AttributeType)) {
      AssertMsg(0, ("CDmxElement::UnpackIntoStructure: Array attribute types "
                    "not currently supported!\n"));
      continue;
    }

    if (pUnpack->m_AttributeType == AT_VOID) {
      AssertMsg(0, ("CDmxElement::UnpackIntoStructure: Binary blob attribute "
    ...
    if (pUnpack->m_AttributeType != pAttribute->GetType()) {
      Warning(
          "CDmxElement::UnpackIntoStructure: Mismatched attribute type in "
          "attribute \"%s\"!\n",
    ...
    if (pAttribute->GetType() == AT_STRING) {
      // Strings get special treatment: they are stored as in-line arrays of
      // chars
      Q_strncpy(pDest, pAttribute->GetValueString(), pUnpack->m_nSize);
      continue;
    }

    // special case - if data type is float, but dest size == 16, we are
    // unpacking into simd by replication
    if ((pAttribute->GetType() == AT_FLOAT) &&
        (pUnpack->m_nSize == sizeof(fltx4))) {
      ...
      AssertMsg(pUnpack->m_nSize ==
                    CDmxAttribute::AttributeDataSize(pAttribute->GetType()),
                ("CDmxElement::UnpackIntoStructure: Incorrect size to unpack "
                 "data into in attribute \"%s\"!\n",
                 pUnpack->m_pAttributeName));
...
//-----------------------------------------------------------------------------
void CDmxElement::AddAttributesFromStructure(
    const void *pData, const DmxElementUnpackStructure_t *pUnpack) {
  for (; pUnpack->m_AttributeType != AT_UNKNOWN; ++pUnpack) {
    const char *pSrc = (const char *)pData + pUnpack->m_nOffset;

    // NOTE: This does not work with array data at the moment
    if (IsArrayType(pUnpack->m_AttributeType)) {
      AssertMsg(0, ("CDmxElement::AddAttributesFromStructure: Array attribute "
                    "types not currently supported!\n"));
      continue;
    }

    if (pUnpack->m_AttributeType == AT_VOID) {
      AssertMsg(0, ("CDmxElement::AddAttributesFromStructure: Binary blob "
                    "attribute types not currently supported!\n"));
      continue;
    }

    if (HasAttribute(pUnpack->m_pAttributeName)) {
      AssertMsg(0, ("CDmxElement::AddAttributesFromStructure: Attribute %s "
                    "already exists!\n",
                    pUnpack->m_pAttributeName));
      continue;
    }

    {
      ...
      if (pUnpack->m_AttributeType == AT_STRING) {
        ...
        if ((pUnpack->m_AttributeType == AT_FLOAT) &&
            (nSize == sizeof(fltx4))) {
          nSize = sizeof(float);
        }

        AssertMsg(
            nSize == CDmxAttribute::AttributeDataSize(pUnpack->m_AttributeType),
            ("CDmxElement::UnpackIntoStructure: Incorrect size to unpack data "
             "into in attribute \"%s\"!\n",
             pUnpack->m_pAttributeName));
