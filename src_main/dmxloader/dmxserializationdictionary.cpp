...
//-----------------------------------------------------------------------------
bool CDmxSerializationDictionary::LessFunc(const DmxElementInfo_t &lhs,
                                           const DmxElementInfo_t &rhs) {
...
//-----------------------------------------------------------------------------
DmxSerializationHandle_t CDmxSerializationDictionary::Find(
...
//-----------------------------------------------------------------------------
void CDmxSerializationDictionary::BuildElementList_R(CDmxElement *pElement,
                                                     bool bFlatMode,
                                                     bool bIsRoot) {
  if (!pElement) return;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    switch (pAttribute->GetType()) {
      ...
      case AT_ELEMENT_ARRAY: {
        ...
        int array_items_count = array.Count();
        for (int j = 0; j < array_items_count; ++j) {
...
void CDmxSerializationDictionary::BuildElementList(CDmxElement *pElement,
                                                   bool bFlatMode) {
...
//-----------------------------------------------------------------------------
void CDmxSerializationDictionary::Clear() { m_Dict.RemoveAll(); }
...
DmxSerializationHandle_t CDmxSerializationDictionary::NextRootElement(
    DmxSerializationHandle_t h) const {
  ++h;
  int nCount = m_Dict.Count();
  for (; h < nCount; ++h) {
    if (m_Dict[h].m_bRoot) return h;
...
CDmxElement *CDmxSerializationDictionary::GetRootElement(
    DmxSerializationHandle_t h) {
