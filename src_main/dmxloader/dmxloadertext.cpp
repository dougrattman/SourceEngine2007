...
// Current line control
//-----------------------------------------------------------------------------
void CDmxKeyValues2ErrorStack::IncrementCurrentLine() { ++m_nFileLine; }

void CDmxKeyValues2ErrorStack::SetCurrentLine(int nLine) {
  m_nFileLine = nLine;
}

int CDmxKeyValues2ErrorStack::GetCurrentLine() const { return m_nFileLine; }
...
void CDmxKeyValues2ErrorStack::ReportError(const char *pFmt, ...) {
  ...
  char temp2[2048];
  Q_snprintf(temp2, sizeof(temp2), "%s(%d) : %s\n", m_pFilename, m_nFileLine,
             temp);
  Warning(temp2);

  for (int i = 0; i < m_maxErrorIndex; i++) {
    if (!m_errorStack[i].IsValid()) continue;
...
class CKeyValues2ErrorContext {
 public:
  ...
  CKeyValues2ErrorContext(CUtlSymbol symName) { Init(symName); }

  ~CKeyValues2ErrorContext() { g_KeyValues2ErrorStack.Pop(); }
...
enum { ELEMENT_DICT_HANDLE_INVALID = (DmxElementDictHandle_t)~0 };
...
class CDmxElementDictionary {
 public:
  ...
  void AddArrayAttribute(CDmxAttribute *pAttribute,
                         DmxElementDictHandle_t hChild);
  void AddArrayAttribute(CDmxAttribute *pAttribute,
                         const DmObjectId_t &pElementId);
  ...
  void SetElementId(DmxElementDictHandle_t hElement,
                    const DmObjectId_t &objectId);
...
//-----------------------------------------------------------------------------
DmxElementDictHandle_t CDmxElementDictionary::InsertElement(
...
// Sets the element id for an element
//-----------------------------------------------------------------------------
void CDmxElementDictionary::SetElementId(DmxElementDictHandle_t hElement,
                                         const DmObjectId_t &objectId) {
...
CDmxElement *CDmxElementDictionary::GetElement(DmxElementDictHandle_t handle) {
  if (handle == ELEMENT_DICT_HANDLE_INVALID) return NULL;
...
//-----------------------------------------------------------------------------
void CDmxElementDictionary::AddAttribute(CDmxAttribute *pAttribute,
                                         const DmObjectId_t &objectId) {
...
//-----------------------------------------------------------------------------
void CDmxElementDictionary::AddArrayAttribute(CDmxAttribute *pAttribute,
                                              DmxElementDictHandle_t hElement) {
...
// Finds an element into the table
//-----------------------------------------------------------------------------
DmxElementDictHandle_t CDmxElementDictionary::FindElement(
    CDmxElement *pElement) {
  int nCount = m_Dict.Count();
  for (int i = 0; i < nCount; ++i) {
    if (pElement == m_Dict[i].m_pElement) return i;
  }
...
DmxElementDictHandle_t CDmxElementDictionary::FindElement(
  ...
  int nCount = m_Dict.Count();
  for (int i = 0; i < nCount; ++i) {
    if (IsUniqueIdEqual(objectId, m_Dict[i].m_Id)) return i;
  }
  return ELEMENT_DICT_HANDLE_INVALID;
}
...
void CDmxElementDictionary::HookUpElementArrayAttributes() {
  ...
  for (int i = 0; i < n; ++i) {
    CUtlVector<CDmxElement *> &array =
        m_ArrayAttributes[i].m_pAttribute->GetArrayForEdit<CDmxElement *>();
    ...
    if (m_ArrayAttributes[i].m_nType == AT_ELEMENT) {
      ...
      // search id->handle table (both loaded and unloaded) for id, and if not
      // found, create a new handle, map it to the id and return it
      DmxElementDictHandle_t hElement =
          FindElement(m_ArrayAttributes[i].m_ObjectId);
      CDmxElement *pElement = GetElement(hElement);
      array.AddToTail(pElement);
    }
  }
...
class CDmxSerializerKeyValues2 {
 public:
  bool Unserialize(const char *pFileName, CUtlBuffer &buf,
                   CDmxElement **ppRoot);
 ...
 private:
  ...
  bool UnserializeAttributeValueFromToken(CDmxAttribute *pAttribute,
                                          DmAttributeType_t type,
                                          CUtlBuffer &tokenBuf);
  bool UnserializeElementAttribute(CUtlBuffer &buf,
                                   DmxElementDictHandle_t hElement,
                                   const char *pAttributeName,
                                   const char *pElementType);
  bool UnserializeElementArrayAttribute(CUtlBuffer &buf,
                                        DmxElementDictHandle_t hElement,
                                        const char *pAttributeName);
  bool UnserializeArrayAttribute(CUtlBuffer &buf,
                                 DmxElementDictHandle_t hElement,
                                 const char *pAttributeName,
                                 DmAttributeType_t nAttrType);
  bool UnserializeAttribute(CUtlBuffer &buf, DmxElementDictHandle_t hElement,
                            const char *pAttributeName,
                            DmAttributeType_t nAttrType);
  bool UnserializeElement(CUtlBuffer &buf, const char *pElementType,
                          DmxElementDictHandle_t *pHandle);
  ...
  void SerializeElementArrayAttribute(CUtlBuffer &buf,
                                      CDmxSerializationDictionary &dict,
                                      CDmxAttribute *pAttribute);
  bool SerializeAttributes(CUtlBuffer &buf, CDmxSerializationDictionary &dict,
                           CDmxElement *pElement);
  bool SaveElement(CUtlBuffer &buf, CDmxSerializationDictionary &dict,
                   CDmxElement *pElement, bool bWriteDelimiters = true);
...
//-----------------------------------------------------------------------------
void CDmxSerializerKeyValues2::SerializeElementAttribute(
    CUtlBuffer &buf, CDmxSerializationDictionary &dict,
    CDmxAttribute *pAttribute) {
...
//-----------------------------------------------------------------------------
void CDmxSerializerKeyValues2::SerializeElementArrayAttribute(
    CUtlBuffer &buf, CDmxSerializationDictionary &dict,
    CDmxAttribute *pAttribute) {
  const CUtlVector<CDmxElement *> &array =
      pAttribute->GetArray<CDmxElement *>();
...
//-----------------------------------------------------------------------------
void CDmxSerializerKeyValues2::SerializeArrayAttribute(
    CUtlBuffer &buf, CDmxAttribute *pAttribute) {
...
bool CDmxSerializerKeyValues2::SerializeAttributes(
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (nAttrType != AT_ELEMENT) {
      ...
      // Elements either serialize their type name or "element" depending on
      // whether they are inlined
      buf.Printf("\"%s\" ", pName);
    }
...
bool CDmxSerializerKeyValues2::SaveElement(CUtlBuffer &buf,
                                           CDmxSerializationDictionary &dict,
                                           CDmxElement *pElement,
...
bool CDmxSerializerKeyValues2::Serialize(CUtlBuffer &outBuf, CDmxElement *pRoot,
...
void CDmxSerializerKeyValues2::EatWhitespacesAndComments(CUtlBuffer &buf) {
  ...
  while (nOffset < nMaxPut) {
    ...
    const char *pPeek = (const char *)buf.PeekGet(sizeof(char), nOffset);

    while (pPeek) {
      if (!isspace(*pPeek)) break;

      if (*pPeek == '\n') {
        g_KeyValues2ErrorStack.IncrementCurrentLine();
      }

      if (++nOffset >= nMaxPut) break;

      pPeek = (const char *)buf.PeekGet(sizeof(char), nOffset);
    }

    // If we don't have a a c++ style comment next, we're done
    pPeek = (const char *)buf.PeekGet(2 * sizeof(char), nOffset);
    if ((nOffset >= nMaxPut) || !pPeek || (pPeek[0] != '/') ||
        (pPeek[1] != '/'))
      break;
...
//-----------------------------------------------------------------------------
CDmxSerializerKeyValues2::TokenType_t CDmxSerializerKeyValues2::ReadToken(
    CUtlBuffer &buf, CUtlBuffer &token) {
  ...
  if (!buf.IsValid() || (buf.TellGet() == buf.TellMaxPut())) return TOKEN_EOF;
  ...
  switch (c) {
    ...
    case '\"':
      ...
      nLength =
          buf.PeekDelimitedStringLength(GetCStringCharConversion(), false) - 1;
  ...
  token.SetExternalBuffer((void *)buf.PeekGet(), nLength, nLength,
...
//-----------------------------------------------------------------------------
DmxElementDictHandle_t CDmxSerializerKeyValues2::CreateDmxElement(
    const char *pElementType) {
...
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeElementAttribute(
    CUtlBuffer &buf, DmxElementDictHandle_t hElement,
  ...
  if (pElement->HasAttribute(pAttributeName)) {
    g_KeyValues2ErrorStack.ReportError(
...
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeElementArrayAttribute(
    CUtlBuffer &buf, DmxElementDictHandle_t hElement,
    const char *pAttributeName) {
  CDmxElement *pElement = m_ElementDict.GetElement(hElement);
  if (pElement->HasAttribute(pAttributeName)) {
  ...
  while (buf.IsValid()) {
    ...
    if (token == TOKEN_CLOSE_BRACKET) break;
...
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeAttributeValueFromToken(
    CDmxAttribute *pAttribute, DmAttributeType_t type, CUtlBuffer &tokenBuf) {
...
// Reads an attribute for an element array
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeArrayAttribute(
    CUtlBuffer &buf, DmxElementDictHandle_t hElement,
  ...
  while (buf.IsValid()) {
    ...
    if (!UnserializeAttributeValueFromToken(pAttribute, nAttrType, tokenBuf)) {
      g_KeyValues2ErrorStack.ReportError(
          "Error reading in array attribute \"%s\" element %d", pAttributeName,
          nElementIndex);
      return false;
    }
...
// Reads an attribute for an element
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeAttribute(
    CUtlBuffer &buf, DmxElementDictHandle_t hElement,
    const char *pAttributeName, DmAttributeType_t nAttrType) {
  ...
  if (token != TOKEN_DELIMITED_STRING) {
    g_KeyValues2ErrorStack.ReportError(
        "Expecting quoted attribute value for attribute \"%s\", didn't find "
        "one!",
        pAttributeName);
    return false;
  }
  ...
  switch (nAttrType) {
    case AT_ELEMENT: {
      ...
      if (!pAttributeValue[0]) return true;
    ...
    default:
      ...
      g_KeyValues2ErrorStack.ReportError("Error reading attribute \"%s\"",
                                         pAttributeName);
      return false;
  }
}
...
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeElement(
    CUtlBuffer &buf, const char *pElementType,
    DmxElementDictHandle_t *pHandle) {
  *pHandle = ELEMENT_DICT_HANDLE_INVALID;
  ...
  while (buf.IsValid()) {
    ...
    // Then, keep reading until we hit a '}'
    if (token == TOKEN_CLOSE_BRACE) break;
    ...
    switch (nAttrType) {
      case AT_UNKNOWN:
        bOk = UnserializeElementAttribute(buf, hElement, pAttributeName,
                                          pAttributeType);
        break;
      ...
      default:
        if (nAttrType >= AT_FIRST_ARRAY_TYPE) {
          bOk = UnserializeArrayAttribute(buf, hElement, pAttributeName,
                                          nAttrType);
        } else {
...
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::UnserializeElement(
    CUtlBuffer &buf, DmxElementDictHandle_t *pHandle) {
  *pHandle = ELEMENT_DICT_HANDLE_INVALID;
  ...
  if (token == TOKEN_EOF) return true;
...
//-----------------------------------------------------------------------------
bool CDmxSerializerKeyValues2::Unserialize(const char *pFileName,
                                           CUtlBuffer &buf,
                                           CDmxElement **ppRoot) {
  *ppRoot = NULL;
  ...
  while (buf.IsValid()) {
    ...
    if (!bOk || (h == ELEMENT_DICT_HANDLE_INVALID)) break;
  ...
  // do this *before* getting the root, since the first element might be deleted
  // due to id conflicts
...
//-----------------------------------------------------------------------------
// Unserialization entry point for text files (assumes version has been
// stripped)
//-----------------------------------------------------------------------------
bool UnserializeTextDMX(const char *pFileName, CUtlBuffer &buf,
                        CDmxElement **ppRoot) {
  CDmxSerializerKeyValues2 dmxUnserializer;
  return dmxUnserializer.Unserialize(pFileName, buf, ppRoot);
}

bool SerializeTextDMX(const char *pFileName, CUtlBuffer &buf,
                      CDmxElement *pRoot) {
