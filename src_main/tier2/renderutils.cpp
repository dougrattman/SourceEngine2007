...
void RenderSphere(const Vector &vCenter, float flRadius, int nTheta, int nPhi,
  ...
  for (i = 0; i < nPhi - 1; ++i) {
    ...
    // Emit a degenerate triangle to skip to the next row without a connecting
    // triangle
    if (i < nPhi - 2) {
      meshBuilder.Index(idx);
      meshBuilder.AdvanceIndex();
...
static void GenerateBoxVertices(const Vector &vOrigin, const QAngle &angles,
...
void RenderBox(const Vector &vOrigin, const QAngle &angles, const Vector &vMins,
  ...
  for (int i = 0; i < 6; i++) {
    ...
    int *ppFaceIndices =
        bInsideOut ? s_pBoxFaceIndicesInsideOut[i] : s_pBoxFaceIndices[i];
    for (int j = 1; j < 3; ++j) {
...
void RenderBox(const Vector &vOrigin, const QAngle &angles, const Vector &vMins,
               const Vector &vMaxs, Color c, bool bZBuffer, bool bInsideOut) {
  IMaterial *pMaterial = bZBuffer ? s_pVertexColor : s_pVertexColorIgnoreZ;
  Color cActual(c.r(), c.g(), c.b(), c.a());
...
static void DrawExtrusionFace(const Vector &start, const Vector &end,
                              Vector *pts, int idx1, int idx2, Color c,
                              CMeshBuilder &meshBuilder) {
  Vector temp;
