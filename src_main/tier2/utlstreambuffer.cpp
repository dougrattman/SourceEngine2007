...
int CUtlStreamBuffer::ReadBytesFromFile(int nBytesToRead, int nReadOffset) {
  ...
  int nBytesRead =
      g_pFullFileSystem->Read(pReadPoint, nBytesToRead, m_hFileHandle);
...
bool CUtlStreamBuffer::StreamGetOverflow(int nSize) {
  ...
  int nUnreadBytes;
  bool bHasPartialOverlap = (nSize >= 0) && (TellGet() >= m_nOffset) &&
                            (TellGet() <= m_nOffset + Size());
  if (bHasPartialOverlap) {
    nUnreadBytes = Size() - (TellGet() - m_nOffset);
    if ((TellGet() != m_nOffset) && (nUnreadBytes > 0)) {
      memmove(Base(), (const char *)Base() + TellGet() - m_nOffset,
              nUnreadBytes);
    }
  } else {
    m_nOffset = TellGet();
