...
void CGameListPanel::OnKeyCodeTyped(vgui::KeyCode code) {
  ...
  if (code == KEY_ENTER && m_pOuter->OnGameListEnterPressed()) return;
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CBaseGamesPage::CBaseGamesPage(vgui::Panel *parent, const char *name,
...
#endif
{
  ...
  m_pRefreshQuick =
      new Button(this, "RefreshQuickButton", "#ServerBrowser_RefreshQuick");
  m_pAddServer =
      new Button(this, "AddServerButton", "#ServerBrowser_AddServer");
  m_pAddCurrentServer = new Button(this, "AddCurrentServerButton",
  ...
  m_pGameList->AddColumnHeader(
      0, "Password", "#ServerBrowser_Password", 16,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
  m_pGameList->AddColumnHeader(
      1, "Bots", "#ServerBrowser_Bots", 16,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_HIDDEN);
  m_pGameList->AddColumnHeader(
      2, "Secure", "#ServerBrowser_Secure", 16,
      ListPanel::COLUMN_FIXEDSIZE | ListPanel::COLUMN_IMAGE);
  m_pGameList->AddColumnHeader(
      3, "Name", "#ServerBrowser_Servers", 50,
      ListPanel::COLUMN_RESIZEWITHWINDOW | ListPanel::COLUMN_UNHIDABLE);
  m_pGameList->AddColumnHeader(4, "IPAddr", "#ServerBrowser_IPAddress", 64,
                               ListPanel::COLUMN_HIDDEN);
  m_pGameList->AddColumnHeader(5, "GameDesc", "#ServerBrowser_Game", 112,
                               112,  // minwidth
                               300,  // maxwidth
                               0     // flags
  );
  m_pGameList->AddColumnHeader(6, "Players", "#ServerBrowser_Players", 55,
  ...
  m_pGameList->AddColumnHeader(8, "Ping", "#ServerBrowser_Latency", 55,
                               ListPanel::COLUMN_FIXEDSIZE);

  m_pGameList->SetColumnHeaderTooltip(0,
...
void CBaseGamesPage::PerformLayout() {
  ...
  if (SupportsItem(IGameList::ADDSERVER)) {
    m_pFilterString->SetWide(
        90);  // shrink the filter label to fix the add current server button
...
void CBaseGamesPage::ApplySchemeSettings(IScheme *pScheme) {
  ...
  int passwordColumnImage = imageList->AddImage(
      scheme()->GetImage("servers/icon_password_column", false));
  int botColumnImage = imageList->AddImage(
      scheme()->GetImage("servers/icon_bots_column", false));
  int secureColumnImage = imageList->AddImage(
      scheme()->GetImage("servers/icon_robotron_column", false));

  m_pGameList->SetImageList(imageList, true);
  m_hFont = pScheme->GetFont("ListSmall", IsProportional());
  if (!m_hFont) m_hFont = pScheme->GetFont("DefaultSmall", IsProportional());
...
#ifndef NO_STEAM
  if (!SteamMatchmakingServers()) return NULL;

  if (serverID >= 0) {
    return SteamMatchmakingServers()->GetServerDetails(m_eMatchMakingType,
                                                       serverID);
  } else {
    Assert(!"Unable to return a useful entry");
    return NULL;  // bugbug Alfred: temp Favorites/History objects won't return
                  // a good value here...
  }
#else
  return NULL;
...
void CBaseGamesPage::CreateFilters() {
  ...
  m_pNoEmptyServersFilterCheck =
      new CheckButton(this, "ServerEmptyFilterCheck", "");
...
void CBaseGamesPage::LoadFilterSettings() {
  ...
  if (ServerBrowserDialog().GetActiveModName()) {
    Q_strncpy(m_szGameFilter, ServerBrowserDialog().GetActiveModName(),
              sizeof(m_szGameFilter));
    m_iLimitToAppID = ServerBrowserDialog().GetActiveAppID();
  } else {
    Q_strncpy(m_szGameFilter, filter->GetString("game"),
              sizeof(m_szGameFilter));
...
void CBaseGamesPage::UpdateGameFilter() {
  ...
  for (int i = 0; i < m_pGameFilter->GetItemCount(); i++) {
    ...
    if (iAppID == m_iLimitToAppID &&
        (!m_szGameFilter[0] ||
         !Q_strncmp(pchGameDir, m_szGameFilter, Q_strlen(pchGameDir)))) {
...
void CBaseGamesPage::ServerResponded(gameserveritem_t &server) {
  int nIndex = -1;  // start at -1 and work backwards to find the next free slot
                    // for this adhoc query
  while (m_mapServers.Find(nIndex) != m_mapServers.InvalidIndex()) nIndex--;
...
void CBaseGamesPage::ServerResponded(int iServer) {
  ...
  gameserveritem_t *pServerItem =
      SteamMatchmakingServers()->GetServerDetails(m_eMatchMakingType, iServer);
  if (!pServerItem) {
...
void CBaseGamesPage::ServerResponded(int iServer,
  ...
  if (iServerMap == m_mapServers.InvalidIndex()) {
    netadr_t netAdr(pServerItem->m_NetAdr.GetIP(),
                    pServerItem->m_NetAdr.GetQueryPort());
  ...
  if (!CheckPrimaryFilters(*pServerItem)) {
    ...
    // we still ping this server in the future; however it is removed from UI
    // list
    removeItem = true;
  }
  ...
  if (pServerItem->m_bSecure) {
    // show the denied icon if banned from secure servers, the secure icon
    // otherwise
    kv->SetInt(
        "secure",
        ServerBrowser().IsVACBannedFromGame(pServerItem->m_nAppID) ? 4 : 3);
  } else {
    kv->SetInt("secure", 0);
  }
  ...
  int nAdjustedForBotsPlayers =
      std::max(0, pServerItem->m_nPlayers - pServerItem->m_nBotPlayers);
  int nAdjustedForBotsMaxPlayers =
  ...
  if (pServer->m_bDoNotRefresh) {
    // clear out the vars
    kv->SetString("Ping", "");
    kv->SetWString("GameDesc",
                   g_pVGuiLocalize->Find("#ServerBrowser_NotResponding"));
  ...
  if (!m_pGameList->IsValidItemID(pServer->m_iListID)) {
    ...
    pServer->m_iListID =
        m_pGameList->AddItem(kv, pServer->m_iServerID, false, false);
...
void CBaseGamesPage::OnButtonToggled(Panel *panel, int state) {
  if (panel == m_pFilter) {
    ...
    InvalidateLayout();
  } else if (panel == m_pNoFullServersFilterCheck ||
             panel == m_pNoEmptyServersFilterCheck ||
             panel == m_pNoPasswordFilterCheck) {
...
void CBaseGamesPage::UpdateDerivedLayouts(void) {
  ...
  if (m_pCustomResFilename) {
    Q_snprintf(rgchControlSettings, sizeof(rgchControlSettings), "%s",
               m_pCustomResFilename);
  } else {
    if (m_pFilter->IsSelected()) {
      // drop down
      Q_snprintf(rgchControlSettings, sizeof(rgchControlSettings),
                 "servers/%sPage_Filters.res", "InternetGames");
    } else {
      // hide filter area
      Q_snprintf(rgchControlSettings, sizeof(rgchControlSettings),
                 "servers/%sPage.res", "InternetGames");
    }
  }
...
void CBaseGamesPage::OnTextChanged(Panel *panel, const char *text) {
  ...
  if (m_bFiltersVisible &&
      (panel == m_pGameFilter || panel == m_pLocationFilter)) {
    // if they changed games and/or region then cancel the refresh because the
    // old list they are getting will be for the wrong game, so stop and start a
    // refresh
    StopRefresh();
    GetNewServerList();
  }
...
#ifndef NO_STEAM
  if (!SteamMatchmakingServers()) return;
  // loop through all the servers checking filters
  FOR_EACH_MAP_FAST(m_mapServers, i) {
    serverdisplay_t &server = m_mapServers[i];
    gameserveritem_t *pServer = SteamMatchmakingServers()->GetServerDetails(
        m_eMatchMakingType, server.m_iServerID);
    if (!pServer) continue;
    ...
    if (!CheckPrimaryFilters(*pServer) || !CheckSecondaryFilters(*pServer)) {
      ...
      if (m_pGameList->IsValidItemID(server.m_iListID)) {
        // don't remove the server from list, just hide since this is a lot
        // faster
      ...
      if (!m_pGameList->IsValidItemID(server.m_iListID)) {
        ...
        if (pServer->m_szGameDescription[0]) {
          ...
          kv->SetWString("GameDesc",
                         g_pVGuiLocalize->Find("#ServerBrowser_PendingPing"));
        }

        int nAdjustedForBotsPlayers =
            std::max(0, pServer->m_nPlayers - pServer->m_nBotPlayers);
        int nAdjustedForBotsMaxPlayers =
            std::max(0, pServer->m_nMaxPlayers - pServer->m_nBotPlayers);
        ...
        server.m_iListID =
            m_pGameList->AddItem(kv, server.m_iServerID, false, false);
        kv->deleteThis();
      }
...
void CBaseGamesPage::UpdateStatus() {
  if (m_pGameList->GetItemCount() > 1) {
    ...
    m_pGameList->SetColumnHeaderText(
        3, g_pVGuiLocalize->Find("#ServerBrowser_Servers"));
  }
}
...
void CBaseGamesPage::UpdateFilterSettings() {
  ...
  if (ServerBrowserDialog().GetActiveModName()) {
    ...
    if (data) {
      Q_strncpy(m_szGameFilter, data->GetString("gamedir"),
                sizeof(m_szGameFilter));
  ...
  if (m_szGameFilter[0] &&
      m_iLimitToAppID !=
          1002)  // HACKHACK: Alfred - don't use a dir filter for RDKF
  {
    m_vecServerFilters.AddToTail(
        MatchMakingKeyValuePair_t("gamedir", m_szGameFilter));
  }
  ...
  if (regCode > 0) {
    ...
    m_vecServerFilters.AddToTail(
        MatchMakingKeyValuePair_t("region", szRegCode));
  }
...
//-----------------------------------------------------------------------------
// Purpose: reconstructs the filter description string from the current filter
// settings
//-----------------------------------------------------------------------------
...
bool CBaseGamesPage::CheckPrimaryFilters(gameserveritem_t &server) {
  if (m_szGameFilter[0] && (server.m_szGameDir[0] || server.m_nPing) &&
      Q_stricmp(m_szGameFilter, server.m_szGameDir) &&
      server.m_nAppID !=
          1002)  // HACKHACK Alfred - don't apply gamedir filter to rdkf
  {
    return false;
  }

  if (server.m_nAppID < 200 ||
      (server.m_nAppID > 900 && server.m_nAppID != 4000))
    return false;
...
//			server will be continued to be pinged if it fails the
// filter, since 			the relvent server data is dynamic
//-----------------------------------------------------------------------------
bool CBaseGamesPage::CheckSecondaryFilters(gameserveritem_t &server) {
  if (m_bFilterNoEmptyServers &&
      (server.m_nPlayers - server.m_nBotPlayers) < 1) {
    return false;
  }
...
//-----------------------------------------------------------------------------
bool CBaseGamesPage::OnGameListEnterPressed() { return false; }
...
void CBaseGamesPage::OnAddToFavorites() {
  ...
  for (int i = 0; i < m_pGameList->GetSelectedItemsCount(); i++) {
    int serverID =
        m_pGameList->GetItemUserData(m_pGameList->GetSelectedItem(i));

    gameserveritem_t *pServer = SteamMatchmakingServers()->GetServerDetails(
        m_eMatchMakingType, serverID);
    if (pServer) {
  ...
  switch (m_eMatchMakingType) {
    case eFavoritesServer:
      SteamMatchmakingServers()->RequestFavoritesServerList(
          GetFilterAppID(), &pFilters, nFilters, this);
      break;
    case eHistoryServer:
    ...
    case eInternetServer:
      SteamMatchmakingServers()->RequestInternetServerList(
          GetFilterAppID(), &pFilters, nFilters, this);
      break;
...
//-----------------------------------------------------------------------------
void CBaseGamesPage::GetNewServerList() { StartRefresh(); }
...
bool CBaseGamesPage::IsRefreshing() {
  ...
  return SteamMatchmakingServers() &&
         SteamMatchmakingServers()->IsRefreshing(m_eMatchMakingType);
#else
  return false;
#endif
}
...
//-----------------------------------------------------------------------------
void CBaseGamesPage::OnPageShow() { StartRefresh(); }

//-----------------------------------------------------------------------------
// Purpose: Called on page hide, stops any refresh
//-----------------------------------------------------------------------------
void CBaseGamesPage::OnPageHide() { StopRefresh(); }

//-----------------------------------------------------------------------------
// Purpose: initiates server connection
//-----------------------------------------------------------------------------
void CBaseGamesPage::OnBeginConnect() {
  if (!m_pGameList->GetSelectedItemsCount()) return;
...
void CBaseGamesPage::OnFavoritesMsg(FavoritesListChanged_t *pFavListChanged) {
  switch (m_eMatchMakingType) {
    ...
    case eFavoritesServer:
    case eHistoryServer: {
      int iIPServer = m_mapServerIP.Find(
          netadr_t(pFavListChanged->m_nIP, pFavListChanged->m_nQueryPort));
      if (iIPServer == m_mapServerIP.InvalidIndex()) {
        if (pFavListChanged->m_bAdd) {
#ifndef NO_STEAM
          if (SteamMatchmakingServers())
            SteamMatchmakingServers()->PingServer(
                pFavListChanged->m_nIP, pFavListChanged->m_nQueryPort, this);
#endif
        }
