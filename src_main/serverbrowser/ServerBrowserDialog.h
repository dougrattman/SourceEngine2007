...
class CServerBrowserDialog : public vgui::Frame {
 ...
 public:
  ...
  CDialogGameInfo *OpenGameInfoDialog(IGameList *gameList,
                                      unsigned int serverIndex);

  // opens a game info dialog by a specified IP, not attached to any game list
  CDialogGameInfo *OpenGameInfoDialog(int serverIP, uint16 connPort,
                                      uint16 queryPort);
  ...
  // gets the name of the mod directory we're restricted to accessing, NULL if
  // none
 ...
 private:
  ...
  // receives a specified game is active, so no other game types can be
...
// Used by the LAN tab and the add server dialog when trying to find servers
// without having been given any ports to look for servers on.
