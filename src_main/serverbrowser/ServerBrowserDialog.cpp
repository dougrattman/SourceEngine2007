...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CServerBrowserDialog::CServerBrowserDialog(vgui::Panel *parent)
...
void CServerBrowserDialog::LoadUserData() {
  ...
  if (!m_pSavedData->LoadFromFile(g_pFullFileSystem, "ServerBrowser.vdf",
                                  "CONFIG")) {
    // doesn't matter if the file is not found, defaults will work successfully
    // and file will be created on exit
  }
  ...
  if (m_pHistory) {
    ...
    if (m_pHistory->IsVisible()) m_pHistory->StartRefresh();
  }
  ...
  if (m_pFavorites) {
    ...
    if (m_pFavorites->IsVisible()) m_pFavorites->StartRefresh();
  }
...
void CServerBrowserDialog::SaveUserData() {
  ...
  m_pSavedData->RemoveSubKey(m_pSavedData->FindKey(
      "Filters"));  // remove the saved subkey and add our subkey
...
void CServerBrowserDialog::UpdateStatusText(const char *fmt, ...) {
  ...
  if (fmt && strlen(fmt) > 0) {
    ...
    va_list argptr;
    va_start(argptr, fmt);
    _vsnprintf_s(str, sizeof(str), fmt, argptr);
    va_end(argptr);
...
void CServerBrowserDialog::AddServerToFavorites(gameserveritem_t &server) {
  ...
  if (SteamMatchmaking()) {
    SteamMatchmaking()->AddFavoriteGame2(
        server.m_nAppID, server.m_NetAdr.GetIP(),
        server.m_NetAdr.GetConnectionPort(), server.m_NetAdr.GetQueryPort(),
        k_unFavoriteFlagFavorite, time(NULL));
  }
#endif
}
...
//			the game info dialog it opens will also update the game
//list
//-----------------------------------------------------------------------------
CDialogGameInfo *CServerBrowserDialog::JoinGame(IGameList *gameList,
...
CDialogGameInfo *CServerBrowserDialog::JoinGame(int serverIP, int serverPort) {
  ...
  CDialogGameInfo *gameDialog =
      OpenGameInfoDialog(serverIP, serverPort, serverPort);
...
//-----------------------------------------------------------------------------
CDialogGameInfo *CServerBrowserDialog::OpenGameInfoDialog(
    IGameList *gameList, unsigned int serverIndex) {
  gameserveritem_t *pServer = gameList->GetServer(serverIndex);
  if (!pServer) return NULL;

  CDialogGameInfo *gameDialog = new CDialogGameInfo(
      NULL, pServer->m_NetAdr.GetIP(), pServer->m_NetAdr.GetQueryPort(),
      pServer->m_NetAdr.GetConnectionPort());
...
//-----------------------------------------------------------------------------
// Purpose: opens a game info dialog by a specified IP, not attached to any game
// list
//-----------------------------------------------------------------------------
CDialogGameInfo *CServerBrowserDialog::OpenGameInfoDialog(int serverIP,
                                                          uint16 connPort,
                                                          uint16 queryPort) {
  CDialogGameInfo *gameDialog =
      new CDialogGameInfo(NULL, serverIP, queryPort, connPort);
  gameDialog->AddActionSignalTarget(this);
  gameDialog->SetParent(GetVParent());
  gameDialog->Run("");
  int i = m_GameInfoDialogs.AddToTail();
...
void CServerBrowserDialog::CloseAllGameInfoDialogs() {
  for (int i = 0; i < m_GameInfoDialogs.Count(); i++) {
    ...
    if (dlg) {
      vgui::ivgui()->PostMessage(dlg->GetVPanel(), new KeyValues("Close"),
                                 NULL);
    }
  }
}
...
//-----------------------------------------------------------------------------
CDialogGameInfo *CServerBrowserDialog::GetDialogGameInfoForFriend(
    uint64 ulSteamIDFriend) {
...
//-----------------------------------------------------------------------------
// Purpose: gets the name of the mod directory we're restricted to accessing,
// NULL if none
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: return the app id to limit game queries to, set by Source/HL1
// engines (NOT by filter settings, that is per page)
//-----------------------------------------------------------------------------
int CServerBrowserDialog::GetActiveAppID() { return m_iLimitAppID; }

//-----------------------------------------------------------------------------
// Purpose: receives a specified game is active, so no other game types can be
// displayed in server list
//-----------------------------------------------------------------------------
...
void CServerBrowserDialog::OnConnectToGame(KeyValues *pMessageValues) {
  ...
  m_CurrentConnection.m_NetAdr.SetConnectionPort(
      (unsigned short)connectionPort);
#ifndef NO_STEAM
  if (m_pHistory && SteamMatchmaking()) {
    SteamMatchmaking()->AddFavoriteGame2(0, ::htonl(ip), connectionPort,
                                         queryPort, k_unFavoriteFlagHistory,
                                         time(NULL));
  ...
  for (int i = 0; i < m_GameInfoDialogs.Count(); i++) {
    vgui::Panel *dlg = m_GameInfoDialogs[i];
    if (dlg) {
      KeyValues *kv = new KeyValues("ConnectedToGame", "ip", ip,
...
void CServerBrowserDialog::ActivateBuildMode() {
  ...
  EditablePanel *panel =
      dynamic_cast<EditablePanel *>(m_pTabPanel->GetActivePage());
  if (!panel) return;
...
// time
//-----------------------------------------------------------------------------
bool CServerBrowserDialog::GetDefaultScreenPosition(int &x, int &y, int &wide,
                                                    int &tall) {
  int wx, wy, ww, wt;
