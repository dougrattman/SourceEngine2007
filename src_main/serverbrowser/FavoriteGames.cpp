...
void CFavoriteGames::OnOpenContextMenu(int itemID) {
  ...
  if (m_pGameList->GetSelectedItemsCount()) {
    // get the server
    int serverID =
        m_pGameList->GetItemUserData(m_pGameList->GetSelectedItem(0));

    // Activate context menu
    menu->ShowMenu(this, serverID, true, true, true, false);
    menu->AddMenuItem("RemoveServer",
  ...
  menu->AddMenuItem("AddServerByName", "#ServerBrowser_AddServerByIP",
                    new KeyValues("AddServerByName"), this);
}
...
void CFavoriteGames::OnRemoveFromFavorites() {
#ifndef NO_STEAM
  if (!SteamMatchmakingServers() || !SteamMatchmaking()) return;
  ...
  for (int iGame = 0; iGame < m_pGameList->GetSelectedItemsCount(); iGame++) {
    ...
    gameserveritem_t *pServer =
        SteamMatchmakingServers()->GetServerDetails(eFavoritesServer, serverID);

    if (pServer) {
      SteamMatchmaking()->RemoveFavoriteGame2(
          pServer->m_nAppID, pServer->m_NetAdr.GetIP(),
...
#ifndef NO_STEAM
  gameserveritem_t *pConnected =
      ServerBrowserDialog().GetCurrentConnectedServer();

  if (pConnected && SteamMatchmaking()) {
    SteamMatchmaking()->AddFavoriteGame2(
        pConnected->m_nAppID, pConnected->m_NetAdr.GetIP(),
        pConnected->m_NetAdr.GetConnectionPort(),
        pConnected->m_NetAdr.GetQueryPort(), k_unFavoriteFlagFavorite,
        time(NULL));
