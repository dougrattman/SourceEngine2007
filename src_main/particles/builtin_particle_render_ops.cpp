...
//-----------------------------------------------------------------------------
static inline int GetMaxParticlesPerBatch(IMatRenderContext *pRenderContext,
                                          IMaterial *pMaterial,
                                          bool bWithInstancing) {
...
DEFINE_PARTICLE_OPERATOR(C_OP_RenderPoints, "render_points",
                         OPERATOR_SINGLETON);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_RenderPoints)
END_PARTICLE_OPERATOR_UNPACK(C_OP_RenderPoints)

void C_OP_RenderPoints::Render(IMatRenderContext *pRenderContext,
  ...
  size_t xyz_stride;
  const fltx4 *xyz =
      pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_XYZ, &xyz_stride);
...
struct SpriteRenderInfo_t {
  ...
  void Init(CParticleCollection *pParticles, int nVertexOffset,
            float flAgeScale, float flAgeScale2, CSheet *pSheet) {
    m_pParticles = pParticles;
    m_pXYZ =
        pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_XYZ, &m_nXYZStride);
    m_pRot = pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_ROTATION,
                                             &m_nRotStride);
    m_pYaw =
        pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_YAW, &m_nYawStride);
    m_pRadius = pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_RADIUS,
                                                &m_nRadStride);
    m_pRGB = pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_TINT_RGB,
                                             &m_nRGBStride);
    m_pAlpha = pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_ALPHA,
                                               &m_nAlphaStride);
    m_pCreationTimeStamp = pParticles->GetM128AttributePtr(
        PARTICLE_ATTRIBUTE_CREATION_TIME, &m_nCreationTimeStride);
    m_pSequenceNumber = pParticles->GetM128AttributePtr(
        PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER, &m_nSequenceStride);
    m_pSequence1Number = pParticles->GetM128AttributePtr(
        PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER1, &m_nSequence1Stride);
...
class C_OP_RenderSprites : public C_OP_RenderPoints {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    C_OP_RenderSpritesContext_t *pCtx =
        reinterpret_cast<C_OP_RenderSpritesContext_t *>(pContext);
  ...
  uint32_t GetReadAttributes(void) const {
    return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_ROTATION_MASK |
           ...
           PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER_MASK |
  ...
  void RenderSpriteCard(CMeshBuilder &meshBuilder, SpriteRenderInfo_t &info,
                        int hParticle) const;
  void RenderTwoSequenceSpriteCard(CMeshBuilder &meshBuilder,
                                   SpriteRenderInfo_t &info,
                                   int hParticle) const;

  void RenderNonSpriteCardCameraFacing(CParticleCollection *pParticles,
                                       void *pContext,
                                       IMatRenderContext *pRenderContext,
                                       IMaterial *pMaterial) const;

  void RenderNonSpriteCardZRotating(CMeshBuilder &meshBuilder,
                                    SpriteRenderInfo_t &info, int hParticle,
                                    const Vector &vecCameraPos) const;
  void RenderNonSpriteCardZRotating(CParticleCollection *pParticles,
                                    void *pContext,
                                    IMatRenderContext *pRenderContext,
                                    IMaterial *pMaterial) const;
  void RenderUnsortedNonSpriteCardZRotating(
      CParticleCollection *pParticles, void *pContext,
      IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
      int nVertexOffset, int nFirstParticle, int nParticleCount) const;

  void RenderNonSpriteCardOriented(CMeshBuilder &meshBuilder,
                                   SpriteRenderInfo_t &info, int hParticle,
                                   const Vector &vecCameraPos) const;
  void RenderNonSpriteCardOriented(CParticleCollection *pParticles,
                                   void *pContext,
                                   IMatRenderContext *pRenderContext,
                                   IMaterial *pMaterial) const;
...
DMXELEMENT_UNPACK_FIELD("animation_fit_lifetime", "0", bool,
                        m_bFitCycleToLifetime)
DMXELEMENT_UNPACK_FIELD("orientation_type", "0", int, m_nOrientationType)
DMXELEMENT_UNPACK_FIELD("second sequence animation rate", "0", float,
                        m_flAnimationRate2)
DMXELEMENT_UNPACK_FIELD("use animation rate as FPS", "0", bool, m_bAnimateInFPS)
END_PARTICLE_OPERATOR_UNPACK(C_OP_RenderSprites)

void C_OP_RenderSprites::InitParams(CParticleSystemDefinition *pDef,
                                    CDmxElement *pElement) {}

const SheetSequenceSample_t *GetSampleForSequence(CSheet *pSheet,
...
int C_OP_RenderSprites::GetParticlesToRender(CParticleCollection *pParticles,
                                             ...
                                             int *pVertsUsed,
                                             int *pIndicesUsed) const {
  int nMaxParticles = ((nRemainingVertices / 4) > (nRemainingIndices / 6))
                          ? nRemainingIndices / 6
                          : nRemainingVertices / 4;
...
void C_OP_RenderSprites::RenderNonSpriteCardCameraFacing(
    CParticleCollection *pParticles, void *pContext,
    IMatRenderContext *pRenderContext, IMaterial *pMaterial) const {
  // generate the sort list before this code starts messing with the matrices
  int nParticles;
  const ParticleRenderData_t *pSortList =
      pParticles->GetRenderList(pRenderContext, true, &nParticles);

  // NOTE: This is interesting to support because at first we won't have all the
  // various pixel-shader versions of SpriteCard, like modulate, twotexture,
  // etc. etc.
  VMatrix tempView;
  ...
  size_t rot_stride;
  const fltx4 *pRot =
      pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_ROTATION, &rot_stride);

  size_t rad_stride;
  const fltx4 *pRadius =
      pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_RADIUS, &rad_stride);

  size_t rgb_stride;
  const fltx4 *pRGB =
      pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_TINT_RGB, &rgb_stride);

  size_t a_stride;
  const fltx4 *pAlpha =
      pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_ALPHA, &a_stride);

  size_t ct_stride;
  const fltx4 *pCreationTimeStamp = pParticles->GetM128AttributePtr(
      PARTICLE_ATTRIBUTE_CREATION_TIME, &ct_stride);

  size_t seq_stride;
  const fltx4 *pSequenceNumber = pParticles->GetM128AttributePtr(
      PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER, &seq_stride);

  size_t ld_stride;
  const fltx4 *pLifeDuration = pParticles->GetM128AttributePtr(
      PARTICLE_ATTRIBUTE_LIFE_DURATION, &ld_stride);
    ...
    for (int i = 0; i < nParticlesInBatch; i++) {
      ...
      if (pSheet) {
        if (m_bFitCycleToLifetime) {
          float flLifetime =
              SubFloat(pLifeDuration[nGroup * ld_stride], nOffset);
          flAgeScale = (flLifetime > 0.0f)
                           ? (1.0f / flLifetime) * SEQUENCE_SAMPLE_COUNT
                           : 0.0f;
        } else {
        ...
        pSample = GetSampleForSequence(
            pSheet, SubFloat(pCreationTimeStamp[nGroup * ct_stride], nOffset),
            pParticles->m_flCurTime, flAgeScale,
            SubFloat(pSequenceNumber[nGroup * seq_stride], nOffset));
      }
      const SequenceSampleTextureCoords_t *pSample0 =
          &(pSample->m_TextureCoordData[0]);

      meshBuilder.Position3f(vecViewPos.x + (-ca + sa) * rad,
                             vecViewPos.y + (-sa - ca) * rad, vecViewPos.z);
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderSprites::RenderNonSpriteCardZRotating(
    CMeshBuilder &meshBuilder, SpriteRenderInfo_t &info, int hParticle,
    const Vector &vecCameraPos) const {
  Assert(hParticle != -1);
  int nGroup = hParticle / 4;
  int nOffset = hParticle & 0x3;
  ...
  if (info.m_pSheet) {
    pSample = GetSampleForSequence(
        info.m_pSheet,
        SubFloat(info.m_pCreationTimeStamp[nGroup * info.m_nCreationTimeStride],
                 nOffset),
        info.m_pParticles->m_flCurTime, info.m_flAgeScale,
        SubFloat(info.m_pSequenceNumber[nGroup * info.m_nSequenceStride],
                 nOffset));
  }
...
void C_OP_RenderSprites::RenderNonSpriteCardZRotating(
  ...
  while (nParticles) {
    ...
    IMesh *pMesh = pRenderContext->GetDynamicMesh(true);
    CMeshBuilder meshBuilder;
    meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, nParticlesInBatch * 4,
                      nParticlesInBatch * 6);
...
void C_OP_RenderSprites::RenderUnsortedNonSpriteCardOriented(
    CParticleCollection *pParticles, void *pContext,
    IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
    int nVertexOffset, int nFirstParticle, int nParticleCount) const {
  // NOTE: This is interesting to support because at first we won't have all the
...
void C_OP_RenderSprites::RenderSpriteCard(CMeshBuilder &meshBuilder,
                                          SpriteRenderInfo_t &info,
                                          int hParticle) const {
  Assert(hParticle != -1);
  int nGroup = hParticle / 4;
  ...
  if (info.m_pSheet) {
    ...
    // 		}
    if (m_bAnimateInFPS) {
      int nSequence = SubFloat(
          info.m_pSequenceNumber[nGroup * info.m_nSequenceStride], nOffset);
      flAgeScale =
          flAgeScale / info.m_pParticles->m_Sheet()->m_flFrameSpan[nSequence];
    }
    pSample = GetSampleForSequence(
        info.m_pSheet,
        SubFloat(info.m_pCreationTimeStamp[nGroup * info.m_nCreationTimeStride],
                 nOffset),
        info.m_pParticles->m_flCurTime, flAgeScale,
        SubFloat(info.m_pSequenceNumber[nGroup * info.m_nSequenceStride],
                 nOffset));
  }
  ...
  const SequenceSampleTextureCoords_t *pSecondTexture0 =
      &(pSample->m_TextureCoordData[1]);

  // Submit 1 (instanced) or 4 (non-instanced) verts (if we're instancing, we
  // don't produce indices either)
  meshBuilder.Position3f(x, y, z);
  meshBuilder.Color4ub(rc, gc, bc, ac);
  meshBuilder.TexCoord4f(0, pSample0->m_fLeft_U0, pSample0->m_fTop_V0,
                         pSample0->m_fRight_U0, pSample0->m_fBottom_V0);
  ...
  meshBuilder.TexCoord4f(
      ...
      pSecondTexture0->m_fRight_U0, pSecondTexture0->m_fBottom_V0);
  meshBuilder.AdvanceVertex();
...
void C_OP_RenderSprites::RenderTwoSequenceSpriteCard(CMeshBuilder &meshBuilder,
                                                     SpriteRenderInfo_t &info,
                                                     int hParticle) const {
  Assert(hParticle != -1);
  int nGroup = hParticle / 4;
  ...
  if (info.m_pSheet) {
    ...
    pSample1 = GetSampleForSequence(
        ...
        SubFloat(info.m_pCreationTimeStamp[nGroup * info.m_nCreationTimeStride],
                 nOffset),
        info.m_pParticles->m_flCurTime, info.m_flAgeScale2,
        SubFloat(info.m_pSequence1Number[nGroup * info.m_nSequence1Stride],
                 nOffset));
  }
  ...
  const SequenceSampleTextureCoords_t *pSample1Frame =
      &(pSample1->m_TextureCoordData[0]);
  ...
  meshBuilder.TexCoord4f(
      4, pSecondTexture0->m_fLeft_U0, pSecondTexture0->m_fTop_V0,
      pSecondTexture0->m_fRight_U0, pSecondTexture0->m_fBottom_V0);
  meshBuilder.TexCoord4f(5, pSample1Frame->m_fLeft_U0, pSample1Frame->m_fTop_V0,
                         pSample1Frame->m_fRight_U0,
                         pSample1Frame->m_fBottom_V0);
  meshBuilder.TexCoord4f(6, pSample1Frame->m_fLeft_U1, pSample1Frame->m_fTop_V1,
                         pSample1Frame->m_fRight_U1,
                         pSample1Frame->m_fBottom_V1);
  ...
  if (!bUseInstancing) {
    ...
    meshBuilder.TexCoord4f(
        4, pSecondTexture0->m_fLeft_U0, pSecondTexture0->m_fTop_V0,
        pSecondTexture0->m_fRight_U0, pSecondTexture0->m_fBottom_V0);
    meshBuilder.TexCoord4f(5, pSample1Frame->m_fLeft_U0,
                           pSample1Frame->m_fTop_V0, pSample1Frame->m_fRight_U0,
                           pSample1Frame->m_fBottom_V0);
    meshBuilder.TexCoord4f(6, pSample1Frame->m_fLeft_U1,
                           pSample1Frame->m_fTop_V1, pSample1Frame->m_fRight_U1,
                           pSample1Frame->m_fBottom_V1);
    meshBuilder.TexCoord4f(7, pSample1->m_fBlendFactor, 0, 0, 0);
    meshBuilder.AdvanceVertex();
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderSprites::Render(IMatRenderContext *pRenderContext,
                                CParticleCollection *pParticles,
                                void *pContext) const {
  IMaterial *pMaterial = pParticles->m_pDef->GetMaterial();
  C_OP_RenderSpritesContext_t *pCtx =
  ...
  IMaterialVar *pVar =
      pMaterial->FindVarFast("$orientation", &pCtx->m_nOrientationVarToken);
  if (pVar) {
  ...
  if (!pMaterial->IsSpriteCard()) {
    switch (m_nOrientationType) {
      case 0:
        RenderNonSpriteCardCameraFacing(pParticles, pContext, pRenderContext,
                                        pMaterial);
        break;

      case 1:
        RenderNonSpriteCardZRotating(pParticles, pContext, pRenderContext,
                                     pMaterial);
        break;

      case 2:
        RenderNonSpriteCardOriented(pParticles, pContext, pRenderContext,
                                    pMaterial);
        break;
    }
  ...
  MaterialPrimitiveType_t primType =
      bUseInstancing ? MATERIAL_INSTANCED_QUADS : MATERIAL_TRIANGLES;
  int nMaxParticlesInBatch =
      GetMaxParticlesPerBatch(pRenderContext, pMaterial, bUseInstancing);
  ...
  while (nParticles) {
    ...
    int vertexCount =
        bUseInstancing ? nParticlesInBatch : nParticlesInBatch * 4;
...
void C_OP_RenderSprites::RenderUnsorted(CParticleCollection *pParticles,
                                        void *pContext,
                                        IMatRenderContext *pRenderContext,
                                        CMeshBuilder &meshBuilder,
                                        int nVertexOffset, int nFirstParticle,
                                        int nParticleCount) const {
  if (!pParticles->m_pDef->GetMaterial()->IsSpriteCard()) {
    switch (m_nOrientationType) {
      ...
      case 1:
        RenderUnsortedNonSpriteCardZRotating(
            pParticles, pContext, pRenderContext, meshBuilder, nVertexOffset,
            nFirstParticle, nParticleCount);
        break;

      case 2:
        RenderUnsortedNonSpriteCardOriented(
            pParticles, pContext, pRenderContext, meshBuilder, nVertexOffset,
            nFirstParticle, nParticleCount);
        break;
    }
...
struct SpriteTrailRenderInfo_t : public SpriteRenderInfo_t {
  ...
  void Init(CParticleCollection *pParticles, int nVertexOffset,
    ...
    m_pParticles = pParticles;
    m_pPrevXYZ = pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_PREV_XYZ,
                                                 &m_nPrevXYZStride);
    m_pLength = pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_TRAIL_LENGTH,
                                                &length_stride);
    m_pCreationTime = pParticles->GetM128AttributePtr(
        PARTICLE_ATTRIBUTE_CREATION_TIME, &m_nCreationTimeStride);
  }
};
...
class C_OP_RenderSpritesTrail : public CParticleOperatorInstance {
  ...
  uint32_t GetReadAttributes(void) const {
    return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK |
           ...
           PARTICLE_ATTRIBUTE_CREATION_TIME_MASK |
           PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER_MASK |
           PARTICLE_ATTRIBUTE_TRAIL_LENGTH_MASK;
  }
  ...
  void RenderSpriteTrail(CMeshBuilder &meshBuilder,
...
DEFINE_PARTICLE_OPERATOR(C_OP_RenderSpritesTrail, "render_sprite_trail",
                         OPERATOR_SINGLETON);
...
int C_OP_RenderSpritesTrail::GetParticlesToRender(
    CParticleCollection *pParticles, void *pContext, int nFirstParticle,
    int nRemainingVertices, int nRemainingIndices, int *pVertsUsed,
    int *pIndicesUsed) const {
  int nMaxParticles = ((nRemainingVertices / 4) > (nRemainingIndices / 6))
                          ? nRemainingIndices / 6
                          : nRemainingVertices / 4;
...
void C_OP_RenderSpritesTrail::RenderSpriteTrail(CMeshBuilder &meshBuilder,
  ...
  // Get the delta direction and find the magnitude, then scale the length by
  // the desired length amount
  Vector vecDelta;
  VectorSubtract(vecPrevViewPos, vecViewPos, vecDelta);
  float flMag = VectorNormalize(vecDelta);
  float flLength =
      flLengthScale * flMag * flOODt *
  ...
  Assert(verts[0].IsValid() && verts[1].IsValid() && verts[2].IsValid() &&
         verts[3].IsValid());
...
void C_OP_RenderSpritesTrail::Render(IMatRenderContext *pRenderContext,
                                     CParticleCollection *pParticles,
                                     void *pContext) const {
  IMaterial *pMaterial = pParticles->m_pDef->GetMaterial();
  ...
  if (pMaterial->IsSpriteCard()) return;
...
void C_OP_RenderSpritesTrail::RenderUnsorted(
    CParticleCollection *pParticles, void *pContext,
    IMatRenderContext *pRenderContext, CMeshBuilder &meshBuilder,
    int nVertexOffset, int nFirstParticle, int nParticleCount) const {
  // NOTE: This is interesting to support because at first we won't have all the
...
struct RopeRenderInfo_t {
  ...
  void GenerateSeg(int hParticle, BeamSeg_t &seg) {
    ...
    seg.m_vColor.Init(SubFloat(m_pRGB[nColorIndex], nOffset),
                      SubFloat(m_pRGB[nColorIndex + 1], nOffset),
                      SubFloat(m_pRGB[nColorIndex + 2], nOffset));
...
class C_OP_RenderRope : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
    ...
    // NOTE: Has to happen here, and not in InitParams, since the material isn't
    // set up yet
    const_cast<C_OP_RenderRope *>(this)->m_flTextureScale =
        1.0f / (pParticles->m_pDef->GetMaterial()->GetMappingHeight() *
  ...
  virtual void Render(IMatRenderContext *pRenderContext,
                      CParticleCollection *pParticles, void *pContext) const;
  virtual void RenderSpriteCard(CParticleCollection *pParticles, void *pContext,
                                IMaterial *pMaterial) const;
  virtual void RenderUnsorted(CParticleCollection *pParticles, void *pContext,
                              IMatRenderContext *pRenderContext,
                              CMeshBuilder &meshBuilder, int nVertexOffset,
...
DMXELEMENT_UNPACK_FIELD("texture_scroll_rate", "0.0f", float,
                        m_flTextureScrollRate)
...
int C_OP_RenderRope::GetParticlesToRender(CParticleCollection *pParticles,
                                          ...
                                          int *pVertsUsed,
                                          int *pIndicesUsed) const {
  if ((nFirstParticle >= pParticles->m_nActiveParticles - 1) ||
      (pParticles->m_nActiveParticles <= 1)) {
    *pVertsUsed = 0;
    *pIndicesUsed = 0;
    return 0;
  }
  ...
  // We can't choose a max particle count so that we only have 1 particle to
  // render next time
...
#define OUTPUT_2SPLINE_VERTS(t)                                     \
  meshBuilder.Color4ub(FastFToC(vecColor.x), FastFToC(vecColor.y),  \
                       FastFToC(vecColor.z), FastFToC(vecColor.w)); \
...
void C_OP_RenderRope::RenderSpriteCard(CParticleCollection *pParticles,
                                       void *pContext,
                                       IMaterial *pMaterial) const {
  int nParticles = pParticles->m_nActiveParticles;

  int nSegmentsToRender = nParticles - 1;
  if (!nSegmentsToRender) return;
  ...
  const float *pColor =
      pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_TINT_RGB, 0);

  const float *pRadius =
      pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_RADIUS, 0);
  const float *pAlpha =
      pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_ALPHA, 0);
...
void C_OP_RenderRope::Render(IMatRenderContext *pRenderContext,
  ...
  while (nParticles) {
    ...
    int nParticlesInBatch =
        GetParticlesToRender(pParticles, pContext, nFirstParticle, nMaxVertices,
                             nMaxIndices, &nVertCount, &nIndexCount);
    if (nParticlesInBatch == 0) break;
    ...
    RenderUnsorted(pParticles, pContext, pRenderContext, meshBuilder, 0,
...
// Purpose:
//-----------------------------------------------------------------------------
void C_OP_RenderRope::RenderUnsorted(CParticleCollection *pParticles,
                                     void *pContext,
                                     IMatRenderContext *pRenderContext,
                                     CMeshBuilder &meshBuilder,
                                     int nVertexOffset, int nFirstParticle,
  ...
  beamSegment.Start(pRenderContext, (nParticleCount - 1) * m_nSubdivCount + 1,
                    pMaterial, &meshBuilder, nVertexOffset);

  Vector vecCatmullRom[4];
  BeamSeg_t seg[2];
  info.GenerateSeg(nFirstParticle, seg[0]);
  seg[0].m_flTexCoord =
      (pCtx->m_flRenderedRopeLength + flTexOffset) * m_flTextureScale;
  ...
  for (int i = 1; i < nParticleCount; ++i, ++hParticle) {
    ...
    if (m_nSubdivCount > 1) {
      ...
      Vector vecColorInc =
          (seg[nCurr].m_vColor - seg[nPrev].m_vColor) * flOOSubDivCount;
      float flAlphaInc =
          (seg[nCurr].m_flAlpha - seg[nPrev].m_flAlpha) * flOOSubDivCount;
      float flTexcoordInc =
          (seg[nCurr].m_flTexCoord - seg[nPrev].m_flTexCoord) * flOOSubDivCount;
      float flWidthInc =
      ...
      for (int iSubdiv = 1; iSubdiv < m_nSubdivCount; ++iSubdiv) {
        ...
        Catmull_Rom_Spline(vecCatmullRom[(i + 3) & 0x3], vecCatmullRom[i & 0x3],
                           vecCatmullRom[(i + 1) & 0x3],
                           vecCatmullRom[(i + 2) & 0x3], pSubdivList[iSubdiv],
                           subDivSeg.m_vPos);
...
void C_OP_RenderBlobs::Render(IMatRenderContext *pRenderContext,
                              CParticleCollection *pParticles,
                              void *pContext) const {
  ...
  static SmartArray<ImpParticle>
      imp_particles_sa;  // This doesn't specify alignment, might have problems
                         // with SSE
  ...
  // TODO: I don't need to load this as a sorted list. See Lennard Jones forces
  // for better way!
  int nParticles;
  const ParticleRenderData_t *pSortList =
      pParticles->GetRenderList(pRenderContext, false, &nParticles);
  size_t xyz_stride;
  const fltx4 *xyz =
      pParticles->GetM128AttributePtr(PARTICLE_ATTRIBUTE_XYZ, &xyz_stride);
  ...
  tiler
      ->drawSurface();  // NOTE: need to call drawSurfaceSorted for transparency
  tiler->endFrame();
}
...
DEFINE_PARTICLE_OPERATOR(C_OP_RenderScreenVelocityRotate,
                         "render_screen_velocity_rotate", OPERATOR_SINGLETON);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_RenderScreenVelocityRotate)
DMXELEMENT_UNPACK_FIELD("rotate_rate(dps)", "0.0f", float,
...
void C_OP_RenderScreenVelocityRotate::Render(IMatRenderContext *pRenderContext,
                                             CParticleCollection *pParticles,
                                             void *pContext) const {
  // NOTE: This is interesting to support because at first we won't have all the
  // various pixel-shader versions of SpriteCard, like modulate, twotexture,
  ...
  const fltx4 *prev_xyz = pParticles->GetM128AttributePtr(
      PARTICLE_ATTRIBUTE_PREV_XYZ, &prev_xyz_stride);

  size_t rot_stride;
  // const fltx4 *pRot = pParticles->GetM128AttributePtr(
  // PARTICLE_ATTRIBUTE_ROTATION, &rot_stride );
  fltx4 *pRot = pParticles->GetM128AttributePtrForWrite(
      PARTICLE_ATTRIBUTE_ROTATION, &rot_stride);
  ...
  for (int i = 0; i < nParticles; i++) {
    ...
    float rot = atan2(vecViewPos.y - vecPrevViewPos.y,
                      vecViewPos.x - vecPrevViewPos.x) +
                flForwardRadians;
