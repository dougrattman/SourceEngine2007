...
//-----------------------------------------------------------------------------
// Destroys an existing element, returns if this element should be added to the
// name list
//-----------------------------------------------------------------------------
void CParticleSystemDictionary::DestroyExistingElement(CDmxElement *pElement) {
  ...
  bool bPreventNameBasedLookup =
      pElement->GetValue<bool>("preventNameBasedLookup");
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!IsUniqueIdEqual(m_ParticleIdMap[i]->GetId(), id)) continue;
...
// Adds a destructor
//-----------------------------------------------------------------------------
CParticleSystemDefinition *CParticleSystemDictionary::AddParticleSystem(
    CDmxElement *pParticleSystem) {
  if (Q_stricmp(pParticleSystem->GetTypeString(),
  ...
  // Must add the def to the maps before Read() because Read() may create new
  // child particle systems
  bool bPreventNameBasedLookup =
      pParticleSystem->GetValue<bool>("preventNameBasedLookup");
  if (!bPreventNameBasedLookup) {
...
int CParticleSystemDictionary::Count() const { return m_ParticleIdMap.Count(); }

CParticleSystemDefinition *CParticleSystemDictionary::GetParticleSystem(int i) {
  return m_ParticleIdMap[i];
}

ParticleSystemHandle_t CParticleSystemDictionary::FindParticleSystemHandle(
    const char *pName) {
  return m_ParticleNameMap.Find(pName);
}

CParticleSystemDefinition *CParticleSystemDictionary::FindParticleSystem(
    ParticleSystemHandle_t h) {
...
CParticleSystemDefinition *CParticleSystemDictionary::FindParticleSystem(
    const char *pName) {
  if (m_ParticleNameMap.Defined(pName)) return m_ParticleNameMap[pName];
  return nullptr;
}
...
DMXELEMENT_UNPACK_FIELD_STRING_USERDATA("material", "vgui/white",
                                        m_pszMaterialName, "vmtPicker")
DMXELEMENT_UNPACK_FIELD("bounding_box_min", "-10 -10 -10", Vector,
                        m_BoundingBoxMin)
DMXELEMENT_UNPACK_FIELD("bounding_box_max", "10 10 10", Vector,
                        m_BoundingBoxMax)
...
DMXELEMENT_UNPACK_FIELD_STRING("cull_replacement_definition", "",
...
DMXELEMENT_UNPACK_FIELD("sequence_number 1", "0", int,
...
DMXELEMENT_UNPACK_FIELD("maximum sim tick rate", "0.0", float,
                        m_flMaximumSimTime)
DMXELEMENT_UNPACK_FIELD("minimum sim tick rate", "0.0", float,
                        m_flMinimumSimTime)
DMXELEMENT_UNPACK_FIELD("minimum rendered frames", "0", int, m_nMinimumFrames)
DMXELEMENT_UNPACK_FIELD(
    "control point to disable rendering if it is the camera", "-1", int,
    m_nSkipRenderControlPoint)
DMXELEMENT_UNPACK_FIELD("maximum draw distance", "100000.0", float,
                        m_flMaxDrawDistance)
DMXELEMENT_UNPACK_FIELD("time to sleep when not drawn", "8", float,
...
END_DMXELEMENT_UNPACK(CParticleSystemDefinition,
...
const char *CParticleSystemDefinition::GetName() const { return m_Name; }
...
//-----------------------------------------------------------------------------
bool CParticleSystemDefinition::IsPrecached() { return m_bIsPrecached; }
...
void CParticleSystemDefinition::ParseOperators(
    ...
    CDmxElement *pElement, CUtlVector<CParticleOperatorInstance *> &outList) {
  const CDmxAttribute *pAttribute = pElement->GetAttribute(pszOpKey);
  if (!pAttribute || pAttribute->GetType() != AT_ELEMENT_ARRAY) return;

  const CUtlVector<IParticleOperatorDefinition *> &flist =
      g_pParticleSystemMgr->GetAvailableParticleOperatorList(nFunctionType);
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    for (int j = 0; j < nFunctionCount; ++j) {
      if (Q_stricmp(pOpName, flist[j]->GetName())) continue;

      // found it!
      bFound = true;

      CParticleOperatorInstance *pNewRef =
          flist[j]->CreateInstance(ops[i]->GetId());
      const DmxElementUnpackStructure_t *pUnpack =
          flist[j]->GetUnpackStructure();
      if (pUnpack) {
      ...
      // Special case: Reading particle ID means we're reading the initial
      // particle id
      if ((pNewRef->GetReadAttributes() | pNewRef->GetReadInitialAttributes()) &
...
void CParticleSystemDefinition::ParseChildren(CDmxElement *pElement) {
  const CUtlVector<CDmxElement *> &children =
      pElement->GetArray<CDmxElement *>("children");
  int nCount = children.Count();
  for (int i = 0; i < nCount; ++i) {
    CDmxElement *pChild = children[i]->GetValue<CDmxElement *>("child");
    if (!pChild ||
        Q_stricmp(pChild->GetTypeString(), "DmeParticleSystemDefinition"))
      continue;

    int j = m_Children.AddToTail();
    children[i]->UnpackIntoStructure(&m_Children[j], s_ChildrenInfoUnpack);
    m_Children[j].m_bUseNameBasedLookup =
        !pChild->GetValue<bool>("preventNameBasedLookup");
...
//	m_Material.Init( "debug/particleerror", TEXTURE_GROUP_OTHER, true );
  ...
  ParseOperators("initializers", FUNCTION_INITIALIZER, pElement,
                 m_Initializers);
  ParseOperators("emitters", FUNCTION_EMITTER, pElement, m_Emitters);
  ParseChildren(pElement);
  ParseOperators("forces", FUNCTION_FORCEGENERATOR, pElement,
                 m_ForceGenerators);
...
IMaterial *CParticleSystemDefinition::GetMaterial() {
  // NOTE: This has to be this way to ensure we don't load every freaking
  // material @ startup
  Assert(IsPrecached());
  if (!IsPrecached()) return nullptr;
  return m_Material;
}
...
bool CParticleSystemDefinition::UsesPowerOfTwoFrameBufferTexture() {
  ...
  // material @ startup
  Assert(IsPrecached());
  return m_Material->NeedsPowerOfTwoFrameBufferTexture(
      false);  // The false checks if it will ever need the frame buffer, not
               // just this frame
}
...
bool CParticleSystemDefinition::UsesFullFrameBufferTexture() {
  ...
  // material @ startup
  Assert(IsPrecached());
  return m_Material->NeedsFullFrameBufferTexture(false);  // The false checks if
                                                          // it will ever need
                                                          // the frame buffer,
                                                          // not just this frame
}
...
//-----------------------------------------------------------------------------
void CParticleSystemDefinition::WriteOperators(
    CDmxElement *pElement, const char *pOpKeyName,
...
void CParticleSystemDefinition::WriteChildren(CDmxElement *pElement) {
  ...
  CUtlVector<CDmxElement *> &children =
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    children[i]->AddAttributesFromStructure(&m_Children[i],
                                            s_ChildrenInfoUnpack);
    CDmxElement *pChildParticleSystem;
    if (m_Children[i].m_bUseNameBasedLookup) {
      pChildParticleSystem =
          g_pParticleSystemMgr->CreateParticleDmxElement(m_Children[i].m_Name);
    } else {
      pChildParticleSystem =
          g_pParticleSystemMgr->CreateParticleDmxElement(m_Children[i].m_Id);
    }
...
void CParticleSystemDefinition::SetupContextData() {
  ...
  CUtlVector<CParticleOperatorInstance *> *olists[] = {
      ...
      &m_Emitters,  &m_ForceGenerators, &m_Constraints};
...
//-----------------------------------------------------------------------------
CUtlVector<CParticleOperatorInstance *>
    *CParticleSystemDefinition::GetOperatorList(ParticleFunctionType_t type) {
  switch (type) {
    case FUNCTION_EMITTER:
      return &m_Emitters;
    case FUNCTION_RENDERER:
...
// Finds an operator by id
//-----------------------------------------------------------------------------
CParticleOperatorInstance *CParticleSystemDefinition::FindOperatorById(
    ParticleFunctionType_t type, const DmObjectId_t &id) {
  CUtlVector<CParticleOperatorInstance *> *pVec = GetOperatorList(type);
  if (!pVec) return nullptr;

  int nCount = pVec->Count();
  for (int i = 0; i < nCount; ++i) {
    if (IsUniqueIdEqual(id, pVec->Element(i)->GetId())) return pVec->Element(i);
  }
  return nullptr;
}
...
//
//-----------------------------------------------------------------------------
void CParticleOperatorInstance::InitNewParticles(
    CParticleCollection *pParticles, int nFirstParticle, int nParticleCount,
  ...
  if (nBlockCount) {
    InitNewParticlesBlock(pParticles, nFirstParticle / 4, nBlockCount,
                          nAttributeWriteMask, pContext);
  ...
  // do tail
  if (nParticleCount) {
    InitNewParticlesScalar(pParticles, nFirstParticle, nParticleCount,
                           nAttributeWriteMask, pContext);
  }
}
...
void *CParticleCollection::operator new(size_t nSize, int nBlockUse,
...
void CParticleCollection::operator delete(void *pData, int nBlockUse,
                                          const char *pFileName, int nLine) {
  if (pData) {
    MemAlloc_FreeAligned(pData);
  }
...
void *CWorldCollideContextData::operator new(size_t nSize, int nBlockUse,
                                             const char *pFileName, int nLine) {
  return MemAlloc_AllocAligned(nSize, 16, pFileName, nLine);
}
...
void CWorldCollideContextData::operator delete(void *pData, int nBlockUse,
...
CParticleCollection::~CParticleCollection() {
  ...
  for (usize i = 0; i < std::size(m_pCollisionCacheData); i++) {
...
// Initialization
//-----------------------------------------------------------------------------
void CParticleCollection::Init(CParticleSystemDefinition *pDef, float flDelay,
                               int nRandomSeed) {
  m_pDef = pDef;
  ...
  if (m_bIsScrubbable) {
    ...
    m_nRandomSeed = (int)(intptr_t)this;
#ifndef _DEBUG
  ...
  SetAttributeToConstant(PARTICLE_ATTRIBUTE_ROTATION,
                         pDef->m_flConstantRotation);
  SetAttributeToConstant(PARTICLE_ATTRIBUTE_ROTATION_SPEED,
                         pDef->m_flConstantRotationSpeed);
  SetAttributeToConstant(
      PARTICLE_ATTRIBUTE_TINT_RGB, pDef->m_ConstantColor.r() / 255.0f,
      pDef->m_ConstantColor.g() / 255.0f, pDef->m_ConstantColor.g() / 255.0f);
  SetAttributeToConstant(PARTICLE_ATTRIBUTE_ALPHA,
                         pDef->m_ConstantColor.a() / 255.0f);
  SetAttributeToConstant(PARTICLE_ATTRIBUTE_CREATION_TIME, 0.0f);
  SetAttributeToConstant(PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER,
                         pDef->m_nConstantSequenceNumber);
  SetAttributeToConstant(PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER1,
                         pDef->m_nConstantSequenceNumber1);
  ...
  if (m_pDef->m_nContextDataSize) {
    m_pOperatorContextData = reinterpret_cast<uint8_t *>(
        MemAlloc_AllocAligned(m_pDef->m_nContextDataSize, 16));
  }

  m_flNextSleepTime = g_pParticleSystemMgr->GetLastSimulationTime() +
                      pDef->m_flNoDrawTimeToGoToSleep;

  // now, init context data
  CUtlVector<CParticleOperatorInstance *> *olists[] = {
      &(m_pDef->m_Operators),       &(m_pDef->m_Renderers),
      &(m_pDef->m_Initializers),    &(m_pDef->m_Emitters),
      &(m_pDef->m_ForceGenerators), &(m_pDef->m_Constraints),
  };
  CUtlVector<size_t> *offsetlists[] = {
      &(m_pDef->m_nOperatorsCtxOffsets),
      &(m_pDef->m_nRenderersCtxOffsets),
      &(m_pDef->m_nInitializersCtxOffsets),
      &(m_pDef->m_nEmittersCtxOffsets),
  ...
  for (usize i = 0; i < std::size(olists); i++) {
    ...
    for (int j = 0; j < nOperatorCount; j++) {
      (*olists[i])[j]->InitializeContextData(
          this, m_pOperatorContextData + (*offsetlists)[i][j]);
    }
  }
  ...
  for (int i = 0; i < nChildCount; ++i) {
    ...
    if (pDef->m_Children[i].m_bUseNameBasedLookup) {
      pChild = g_pParticleSystemMgr->CreateParticleCollection(
          pDef->m_Children[i].m_Name,
          -m_flCurTime + pDef->m_Children[i].m_flDelay, nRandomSeed);
    } else {
      pChild = g_pParticleSystemMgr->CreateParticleCollection(
          pDef->m_Children[i].m_Id,
          -m_flCurTime + pDef->m_Children[i].m_flDelay, nRandomSeed);
    }
    if (pChild) {
  ...
  LabelTextureUsage();
  m_bAnyUsesPowerOfTwoFrameBufferTexture =
      ComputeUsesPowerOfTwoFrameBufferTexture();
...
bool CParticleCollection::Init(CParticleSystemDefinition *pDef) {
  if (!pDef || !pDef->IsPrecached()) {
    ...
    CParticleSystemDefinition *pErrorDef =
        g_pParticleSystemMgr->FindParticleSystem("error");
    if (pErrorDef) pDef = pErrorDef;
  }
...
bool CParticleCollection::Init(const char *pParticleSystemName) {
  if (!pParticleSystemName) return false;
...
void CParticleCollection::InitStorage(CParticleSystemDefinition *pDef) {
  ...
  int nConstantMemorySize =
      3 * 4 * MAX_PARTICLE_ATTRIBUTES * sizeof(float) + 16;
  ...
  m_nPerParticleInitializedAttributeMask =
      pDef->m_nPerParticleInitializedAttributeMask;
  m_nPerParticleUpdatedAttributeMask = pDef->m_nPerParticleUpdatedAttributeMask;

  // Only worry about initial attributes that are per-particle *and* are updated
  // at a later time If they aren't updated at a later time, then we can just
  // point the initial + current pointers at the same memory
  m_nPerParticleReadInitialAttributeMask =
      pDef->m_nInitialAttributeReadMask &
      (pDef->m_nPerParticleInitializedAttributeMask &
       pDef->m_nPerParticleUpdatedAttributeMask);

  // This is the mask of attributes which are initialized per-particle, but
  // never updated *and* where operators want to read initial particle state
  int nPerParticleReadConstantAttributeMask =
      pDef->m_nInitialAttributeReadMask &
      (pDef->m_nPerParticleInitializedAttributeMask &
       (~pDef->m_nPerParticleUpdatedAttributeMask));
  ...
  int nPerParticleAttributeMask = m_nPerParticleInitializedAttributeMask |
                                  m_nPerParticleUpdatedAttributeMask;
  ...
  if (nInitialAttributeSize != 0) {
    int nInitialAllocationSize =
        m_nAllocatedParticles * nInitialAttributeSize * sizeof(float) + 16;
  ...
  for (int bit = 0; bit < MAX_PARTICLE_ATTRIBUTES; bit++) {
    ...
    if (pDef->m_nInitialAttributeReadMask & (1 << bit)) {
      ...
      m_pParticleInitialAttributes[bit] = nullptr;
      m_nParticleInitialFloatStrides[bit] = 0;
    }
  }
}
...
//-----------------------------------------------------------------------------
bool CParticleCollection::UsesPowerOfTwoFrameBufferTexture(
    bool bThisFrame) const {
  if (!m_bAnyUsesPowerOfTwoFrameBufferTexture)  // quick out if neither us or
                                                // our children ever use
  {
    return false;
  }
  if (bThisFrame) {
    return SystemContainsParticlesWithBoolSet(
        &CParticleCollection::m_bUsesPowerOfTwoFrameBufferTexture);
  }
  return true;
}
//-----------------------------------------------------------------------------
...
bool CParticleCollection::UsesFullFrameBufferTexture(bool bThisFrame) const {
  if (!m_bAnyUsesFullFrameBufferTexture)  // quick out if neither us or our
                                          // children ever use
  {
    return false;
  }
  if (bThisFrame) {
    return SystemContainsParticlesWithBoolSet(
        &CParticleCollection::m_bUsesFullFrameBufferTexture);
  }
  return true;
}

bool CParticleCollection::SystemContainsParticlesWithBoolSet(
    bool CParticleCollection::*pField) const {
  if (m_nActiveParticles && (this->*pField)) return true;
  for (CParticleCollection *p = m_Children.m_pHead; p; p = p->m_pNext) {
    if (p->SystemContainsParticlesWithBoolSet(pField)) return true;
  }
  return false;
}

void CParticleCollection::LabelTextureUsage() {
  if (m_pDef) {
    m_bUsesPowerOfTwoFrameBufferTexture =
        m_pDef->UsesPowerOfTwoFrameBufferTexture();
...
bool CParticleCollection::ComputeUsesPowerOfTwoFrameBufferTexture() {
  if (!m_pDef) return false;

  if (m_pDef->UsesPowerOfTwoFrameBufferTexture()) return true;

  for (CParticleCollection *p = m_Children.m_pHead; p; p = p->m_pNext) {
    if (p->UsesPowerOfTwoFrameBufferTexture(false)) return true;
  }
...
bool CParticleCollection::ComputeUsesFullFrameBufferTexture() {
  ...
  if (m_pDef->UsesFullFrameBufferTexture()) return true;

  for (CParticleCollection *p = m_Children.m_pHead; p; p = p->m_pNext) {
    if (p->UsesFullFrameBufferTexture(false)) return true;
  }
...
bool CParticleCollection::ContainsOpaqueCollections() {
  ...
  if (!m_pDef->GetMaterial()->IsTranslucent()) return true;

  for (CParticleCollection *p = m_Children.m_pHead; p; p = p->m_pNext) {
    if (p->ContainsOpaqueCollections()) return true;
  }
...
// Is the particle system two-pass?
//-----------------------------------------------------------------------------
bool CParticleCollection::IsTwoPass() const { return m_bIsTwoPass; }

bool CParticleCollection::ComputeIsTwoPass() {
  if (!ComputeIsTranslucent()) return false;
...
//-----------------------------------------------------------------------------
bool CParticleCollection::IsTranslucent() const { return m_bIsTranslucent; }
...
bool CParticleCollection::ComputeIsTranslucent() {
  ...
  for (CParticleCollection *p = m_Children.m_pHead; p; p = p->m_pNext) {
    if (p->IsTranslucent()) return true;
  }
...
void *CParticleCollection::GetRendererContext(int i) {
  return IsValid() ? m_pOperatorContextData + m_pDef->m_nRenderersCtxOffsets[i]
                   : nullptr;
}
...
float FadeInOut(float flFadeInStart, float flFadeInEnd, float flFadeOutStart,
...
bool CParticleCollection::CheckIfOperatorShouldRun(
    CParticleOperatorInstance const *pOp, float *pflCurStrength) {
  ...
  float flStrength =
      FadeInOut(pOp->m_flOpStartFadeInTime, pOp->m_flOpEndFadeInTime,
                pOp->m_flOpStartFadeOutTime, pOp->m_flOpEndFadeOutTime, flTime);
  if (pflCurStrength) *pflCurStrength = flStrength;
...
void CParticleCollection::Restart() {
  ...
  for (i = 0; i < nEmitterCount; i++) {
    m_pDef->m_Emitters[i]->Restart(
        this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i]);
  }

  // Update all children
  CParticleCollection *pChild;
  for (i = 0, pChild = m_Children.m_pHead; pChild != nullptr;
       pChild = pChild->m_pNext, i++) {
...
//-----------------------------------------------------------------------------
void CParticleCollection::Render(IMatRenderContext *pRenderContext,
      ...
      for (int i = 0; i < nCount; i++) {
        if (CheckIfOperatorShouldRun(m_pDef->m_Renderers[i]))
          m_pDef->m_Renderers[i]->Render(
              pRenderContext, this,
              m_pOperatorContextData + m_pDef->m_nRenderersCtxOffsets[i]);
      }
    }
  }
...
#define END_OP                                        \
  if (1) {                                            \
    ...
    IParticleOperatorDefinition *pDef =               \
        (IParticleOperatorDefinition *)pOp->m_pDef;   \
...
void CParticleCollection::InitializeNewParticles(int nFirstParticle,
                                                 int nParticleCount,
  ...
  for (int i = 0; i < nInitializerCount; i++) {
    int nInitializerAttrMask =
        m_pDef->m_Initializers[i]->GetWrittenAttributes();
    if ((nInitializerAttrMask & nAttrsLeftToInit) &&
        !m_pDef->m_Initializers[i]->InitMultipleOverride()) {
      CParticleOperatorInstance *pOp = m_pDef->m_Initializers[i];
      START_OP;
      pOp->InitNewParticles(
          this, nFirstParticle, nParticleCount, nAttrsLeftToInit,
          m_pOperatorContextData + m_pDef->m_nInitializersCtxOffsets[i]);
      END_OP;
      nAttrsLeftToInit &= ~nInitializerAttrMask;
    }
  }
  // always run second tier initializers (modifiers) after first tier - this
  // ensures they don't get stomped.
  for (int i = 0; i < nInitializerCount; i++) {
    int nInitializerAttrMask =
        m_pDef->m_Initializers[i]->GetWrittenAttributes();
...
void CParticleCollection::SkipToTime(float t) {
  if (t > m_flCurTime) {
    ...
    int i;
    for (i = 0; i < nEmitterCount; i++) {
      m_pDef->m_Emitters[i]->SkipToTime(
          t, this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i]);
    }
...
static void CHECKSYSTEM(CParticleCollection *pParticles) {
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    const float *rad =
        pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_RADIUS, i);
...
void CParticleCollection::SimulateFirstFrame() {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (pOp->ShouldRunBeforeEmitters() &&
      ...
      pOp->Operate(this, flStrength,
                   m_pOperatorContextData + m_pDef->m_nOperatorsCtxOffsets[i]);
      CHECKSYSTEM(this);
      UpdatePrevControlPoints(0.05f);
    }
...
void CParticleCollection::Simulate(float dt) {
  VPROF_BUDGET("CParticleCollection::Simulate",
  ...
  // and destroyed before being rendered.
  // if ( m_pDef->m_flMaximumSimTime != 0.0 && !m_bHasDrawnOnce )
  if (m_pDef->m_flMaximumSimTime != 0.0 &&
      (m_nDrawnFrames <= m_pDef->m_nMinimumFrames)) {
  ...
  while (flRemainingDt > 0.0) {
    ...
    for (int i = 0; i < nEmitterCount; i++) {
      ...
      if (CheckIfOperatorShouldRun(m_pDef->m_Emitters[i], &flEmitStrength)) {
        uint32_t nInittedMask = m_pDef->m_Emitters[i]->Emit(
            this, flEmitStrength,
            m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i]);
        if (nOldParticleCount != m_nActiveParticles) {
          // init newly emitted particles
          InitializeNewParticles(nOldParticleCount,
                                 m_nActiveParticles - nOldParticleCount,
                                 nInittedMask);
          CHECKSYSTEM(this);
        }
      }
    ...
    if (m_nActiveParticles)
      for (int i = 0; i < nCount; i++) {
        ...
        if ((!pOp->ShouldRunBeforeEmitters()) &&
          ...
          if (m_nNumParticlesToKill) {
            ApplyKillList();
            if (!m_nActiveParticles) break;  // don't run any more operators
          }
          m_nOperatorRandomSampleOffset += 17;
        }
      }
  ...
  if (bAttachedKillList) g_pParticleSystemMgr->DetachKillList(this);
  UpdatePrevControlPoints(dt);
...
//-----------------------------------------------------------------------------
void CParticleCollection::InitParticleAttributes(int nStartParticle,
                                                 int nNumParticles,
                                                 int nAttrsLeftToInit) {
  if (nAttrsLeftToInit == 0) return;
  ...
  for (int i = nStartParticle; i < nStartParticle + nNumParticles; i++) {
    for (int nAttr = 0; nAttr < MAX_PARTICLE_ATTRIBUTES; ++nAttr) {
      if ((nAttrsLeftToInit & (1 << nAttr)) == 0) continue;
      ...
      if (nAttr == PARTICLE_ATTRIBUTE_PARTICLE_ID) {
        *((int *)pAttrData) =
            (m_nRandomSeed + m_nUniqueParticleId) & RANDOM_FLOAT_MASK;
        m_nUniqueParticleId++;
        continue;
      }
      ...
      // If this assertion fails, it means we're writing into constant memory,
      // which is a nono
...
void CParticleCollection::CopyInitialAttributeValues(int nStartParticle,
                                                     int nNumParticles) {
  ...
  for (int i = nStartParticle; i < nStartParticle + nNumParticles; i++) {
    for (int nAttr = 0; nAttr < MAX_PARTICLE_ATTRIBUTES; ++nAttr) {
      if (m_nPerParticleReadInitialAttributeMask & (1 << nAttr)) {
        ...
        Assert(m_nParticleFloatStrides[nAttr] ==
...
//-----------------------------------------------------------------------------
float CParticleCollection::RandomVectorInUnitSphere(int nRandomSampleId,
                                                    Vector *pVector) {
  // Guarantee uniform random distribution within a sphere
  // Graphics gems III contains this algorithm ("Nonuniform random point sets
  // via warping")
...
//-----------------------------------------------------------------------------
void CParticleCollection::GetControlPointAtTime(int nControlPoint, float flTime,
                                                Vector *pControlPoint) {
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
  if (nControlPoint > GetHighestControlPoint()) {
    DevWarning(
        2, "Warning : Particle system (%s) using unassigned ControlPoint %d!\n",
  ...
  VectorLerp(m_ControlPoints[nControlPoint].m_PrevPosition,
             m_ControlPoints[nControlPoint].m_Position, t, *pControlPoint);
  Assert(IsFinite(pControlPoint->x) && IsFinite(pControlPoint->y) &&
         IsFinite(pControlPoint->z));
}
...
//
//-----------------------------------------------------------------------------
void CParticleCollection::GetControlPointAtPrevTime(int nControlPoint,
                                                    Vector *pControlPoint) {
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
  *pControlPoint = m_ControlPoints[nControlPoint].m_PrevPosition;
}

void CParticleCollection::GetControlPointOrientationAtTime(int nControlPoint,
                                                           float flTime,
                                                           Vector *pForward,
                                                           Vector *pRight,
                                                           Vector *pUp) {
...
void CParticleCollection::GetControlPointTransformAtTime(int nControlPoint,
                                                         float flTime,
                                                         matrix3x4_t *pMat) {
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
  ...
  pMat->Init(m_ControlPoints[nControlPoint].m_ForwardVector, left,
             m_ControlPoints[nControlPoint].m_UpVector, vecControlPoint);
}

void CParticleCollection::GetControlPointTransformAtTime(
    int nControlPoint, float flTime, CParticleSIMDTransformation *pXForm) {
  Assert(m_pDef->ReadsControlPoint(nControlPoint));
  Vector vecControlPoint;
  GetControlPointAtTime(nControlPoint, flTime, &vecControlPoint);

  pXForm->m_v4Fwd.DuplicateVector(
      m_ControlPoints[nControlPoint].m_ForwardVector);
  pXForm->m_v4Up.DuplicateVector(m_ControlPoints[nControlPoint].m_UpVector);
  // Vector left;
  // VectorMultiply( m_ControlPoints[nControlPoint].m_RightVector, -1.0f, left
  // );
  pXForm->m_v4Right.DuplicateVector(
      m_ControlPoints[nControlPoint].m_RightVector);
}

int CParticleCollection::GetHighestControlPoint() const { return m_nHighestCP; }
...
void CParticleCollection::BloatBoundsUsingControlPoint() {
  // more specifically, some particle systems were using "start" as an input, so
  // it got set as control point 1, so other particle systems had an extra point
  // in their bounding box, that generally remained at the world origin
  RecomputeBounds();

  // Deal with children
  // NOTE: Bounds have been recomputed for children prior to this call in
  // Simulate
...
//-----------------------------------------------------------------------------
void CParticleCollection::RecomputeBounds() {
  ...
  float fsum_x = SubFloat(sum_x, 0) + SubFloat(sum_x, 1) + SubFloat(sum_x, 2) +
                 SubFloat(sum_x, 3);
  float fsum_y = SubFloat(sum_y, 0) + SubFloat(sum_y, 1) + SubFloat(sum_y, 2) +
                 SubFloat(sum_y, 3);
  float fsum_z = SubFloat(sum_z, 0) + SubFloat(sum_z, 1) + SubFloat(sum_z, 2) +
                 SubFloat(sum_z, 3);
...
bool CParticleCollection::IsFinished() {
  if (!m_pDef) return true;
  if (m_nParticleFlags & PCFLAGS_FIRST_FRAME) return false;
  if (m_nActiveParticles) return false;
  if (m_bDormant) return false;

  // no particles. See if any emmitters intead to create more particles
  int nEmitterCount = m_pDef->m_Emitters.Count();
  for (int i = 0; i < nEmitterCount; i++) {
    if (m_pDef->m_Emitters[i]->MayCreateMoreParticles(
            this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i]))
      return false;
  }
...
//-----------------------------------------------------------------------------
void CParticleCollection::StopEmission(bool bInfiniteOnly,
                                       bool bRemoveAllParticles,
                                       bool bWakeOnStop) {
  if (!m_pDef) return;

  // Whenever we call stop emission, we clear out our dormancy. This ensures we
  // get deleted if we're told to stop emission while dormant. SetDormant()
  // ensures dormancy is set to true after stopping out emission.
  m_bDormant = false;
  ...
  for (int i = 0; i < m_pDef->m_Emitters.Count(); i++) {
    m_pDef->m_Emitters[i]->StopEmission(
        this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i],
        bInfiniteOnly);
  }
...
void CParticleCollection::StartEmission(bool bInfiniteOnly) {
  ...
  for (int i = 0; i < m_pDef->m_Emitters.Count(); i++) {
    m_pDef->m_Emitters[i]->StartEmission(
        this, m_pOperatorContextData + m_pDef->m_nEmittersCtxOffsets[i],
        bInfiniteOnly);
  }
...
//			new ones. Unlike particle systems that have
// StopEmission()  called 			dormant particle systems don't
// remove themselves  when they're
...
void CParticleCollection::SetDormant(bool bDormant) {
  ...
  if (bDormant == m_bDormant) return;

  // If emissions have already been stopped, don't go dormant, we're supposed to
  // be dying.
  if (m_bEmissionStopped && bDormant) return;
...
void CParticleCollection::MoveParticle(int nInitialIndex, int nNewIndex) {
  ...
  for (int p = 0; p < MAX_PARTICLE_ATTRIBUTES; ++p) {
    switch (m_nParticleFloatStrides[p]) {
      case 4:  // move a float
        m_pParticleAttributes[p][nNewIndex] =
            m_pParticleAttributes[p][nInitialIndex];
        break;
      ...
      case 12:  // move a vec3
      {
        ...
        m_pParticleAttributes[p][4 + oldofs] =
            m_pParticleAttributes[p][4 + lastofs];
        m_pParticleAttributes[p][8 + oldofs] =
            m_pParticleAttributes[p][8 + lastofs];
        break;
      }
    }
    ...
    switch (m_nParticleInitialFloatStrides[p]) {
      case 4:  // move a float
        m_pParticleInitialAttributes[p][nNewIndex] =
            m_pParticleInitialAttributes[p][nInitialIndex];
        break;
      ...
      case 12:  // move a vec3
      {
        ...
        m_pParticleInitialAttributes[p][oldofs] =
            m_pParticleInitialAttributes[p][lastofs];
        m_pParticleInitialAttributes[p][4 + oldofs] =
            m_pParticleInitialAttributes[p][4 + lastofs];
        m_pParticleInitialAttributes[p][8 + oldofs] =
            m_pParticleInitialAttributes[p][8 + lastofs];
        break;
      }
    }
  }
...
static int32_t *g_pKillBuffers[MAX_SIMULTANEOUS_KILL_LISTS];
    ...
    for (usize i = 0; i < std::size(g_nKillBufferInUse); i++) {
      if (!g_nKillBufferInUse[i])  // available?
      {
        ...
        if (ThreadInterlockedAssignIf(&(g_nKillBufferInUse[i]), 1, 0)) {
          if (!g_pKillBuffers[i]) {
            g_pKillBuffers[i] = new int32_t[MAX_PARTICLES_IN_A_SYSTEM];
          }
...
void CParticleCollection::ApplyKillList() {
  int nLeftInKillList = m_nNumParticlesToKill;
  if (nLeftInKillList == 0) return;

  int nParticlesActiveNow = m_nActiveParticles;
  const int *pCurKillListSlot = m_pParticleKillList;

#ifdef _DEBUG
  // This algorithm assumes the particles listed in the kill list are in
  // ascending order
  ...
  while (nLeftInKillList &&
         pCurKillListSlot[nLeftInKillList - 1] >= nParticlesActiveNow) {
    nLeftInKillList--;
  }
...
void CParticleCollection::CalculatePathValues(CPathParameters const &PathIn,
                                              ...
                                              Vector *pEndPnt) {
  Vector StartPnt;
  GetControlPointAtTime(PathIn.m_nStartControlPointNumber, flTimeStamp,
                        &StartPnt);
  Vector EndPnt;
  ...
  if (PathIn.m_nBulgeControl) {
    ...
    if (PathIn.m_nBulgeControl == 2) nCP = PathIn.m_nEndControlPointNumber;
    Vector Fwd = m_ControlPoints[nCP].m_ForwardVector;
    float len = VectorLength(vTarget);
    if (len > 1.0e-6) {
      vTarget *= (1.0 / len);  // normalize
      flBulgeScale =
          1.0 - fabs(DotProduct(vTarget, Fwd));  // bulge inversely scaled
    }
...
CParticleSystemMgr::CParticleSystemMgr()
    : m_VisualizedOperatorId{}  // m_SheetList( DefLessFunc( ITexture * ) )
{
...
bool CParticleSystemMgr::Init(IParticleSystemQuery *pQuery) {
  ...
  if (!m_bDidInit) {
    ...
    if (g_pMaterialSystemHardwareConfig->GetDXSupportLevel() >= 90) {
      ...
      m_pShadowDepthMaterial = g_pMaterialSystem->CreateMaterial(
          "__particlesDepthWrite", pVMTKeyValues);
    }
...
void CParticleSystemMgr::UncacheAllParticleSystems() {
  if (!m_pParticleSystemDictionary) return;
...
static const char *s_pParticleFieldNames[MAX_PARTICLE_ATTRIBUTES] = {
    ...
    nullptr,          // PREV_XYZ is for internal use only
    "Radius",         // RADIUS, 3 );
...
//-----------------------------------------------------------------------------
void CParticleSystemMgr::AddParticleOperator(
    ParticleFunctionType_t nOpType, IParticleOperatorDefinition *pOpFactory) {
  m_ParticleOperators[nOpType].AddToTail(pOpFactory);
}

CUtlVector<IParticleOperatorDefinition *>
    &CParticleSystemMgr::GetAvailableParticleOperatorList(
        ParticleFunctionType_t nWhichList) {
  return m_ParticleOperators[nWhichList];
}

const DmxElementUnpackStructure_t *
CParticleSystemMgr::GetParticleSystemDefinitionUnpackStructure() {
...
void *CParticleOperatorInstance::operator new(size_t nSize, int nBlockUse,
                                              const char *pFileName,
                                              int nLine) {
  return MemAlloc_AllocAligned(nSize, 16, pFileName, nLine);
}
...
void CParticleOperatorInstance::operator delete(void *pData, int nBlockUse,
                                                const char *pFileName,
                                                int nLine) {
  if (pData) {
    MemAlloc_FreeAligned(pData);
...
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::ReadParticleDefinitions(CUtlBuffer &buf,
                                                 const char *pFileName,
                                                 bool bPrecache,
                                                 bool bDecommitTempMemory) {
  ...
  if (!Q_stricmp(pRoot->GetTypeString(), "DmeParticleSystemDefinition")) {
    CParticleSystemDefinition *pDef =
        m_pParticleSystemDictionary->AddParticleSystem(pRoot);
    if (pDef && bPrecache) {
  ...
  const CDmxAttribute *pDefinitions =
      pRoot->GetAttribute("particleSystemDefinitions");
  if (!pDefinitions || pDefinitions->GetType() != AT_ELEMENT_ARRAY) {
    CleanupDMX(pRoot);
    return false;
  }

  const CUtlVector<CDmxElement *> &definitions =
      pDefinitions->GetArray<CDmxElement *>();
  int nCount = definitions.Count();
  for (int i = 0; i < nCount; ++i) {
    CParticleSystemDefinition *pDef =
        m_pParticleSystemDictionary->AddParticleSystem(definitions[i]);
    if (pDef && bPrecache) {
      pDef->m_bAlwaysPrecache = true;
      pDef->Precache();
    }
...
//-----------------------------------------------------------------------------
void CParticleSystemMgr::DecommitTempMemory() { DecommitDMXMemory(); }
...
CParticleSystemDefinition *CParticleSystemMgr::FindParticleSystem(
    const char *pName) {
...
// Read the particle config file from a utlbuffer
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::ReadParticleConfigFile(CUtlBuffer &buf, bool bPrecache,
                                                bool bDecommitTempMemory,
                                                const char *pFileName) {
  return ReadParticleDefinitions(buf, pFileName, bPrecache,
                                 bDecommitTempMemory);
}
...
// Read the particle config file from a utlbuffer
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::ReadParticleConfigFile(const char *pFileName,
                                                bool bPrecache,
                                                bool bDecommitTempMemory) {
  ...
  if (g_pFullFileSystem->ReadFile(pFileName, "GAME", buf)) {
    return ReadParticleConfigFile(buf, bPrecache, bDecommitTempMemory,
                                  pFileName);
  }
...
//-----------------------------------------------------------------------------
bool CParticleSystemMgr::WriteParticleConfigFile(
    const char *pParticleSystemName, CUtlBuffer &buf,
    bool bPreventNameBasedLookup) {
...
bool CParticleSystemMgr::WriteParticleConfigFile(const DmObjectId_t &id,
                                                 CUtlBuffer &buf,
                                                 bool bPreventNameBasedLookup) {
  // Create DMX elements representing the particle system definition
...
ParticleSystemHandle_t CParticleSystemMgr::GetParticleSystemIndex(
    const char *pParticleSystemName) {
  if (!pParticleSystemName) return UTL_INVAL_SYMBOL;

  return m_pParticleSystemDictionary->FindParticleSystemHandle(
      pParticleSystemName);
}

const char *CParticleSystemMgr::GetParticleSystemNameFromIndex(
    ParticleSystemHandle_t iIndex) {
  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(iIndex);
...
//-----------------------------------------------------------------------------
CParticleCollection *CParticleSystemMgr::CreateParticleCollection(
    const char *pParticleSystemName, float flDelay, int nRandomSeed) {
  if (!pParticleSystemName) return nullptr;

  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(pParticleSystemName);
  if (!pDef) {
    Warning("Attempted to create unknown particle system type %s\n",
            pParticleSystemName);
    return nullptr;
...
CParticleCollection *CParticleSystemMgr::CreateParticleCollection(
    const DmObjectId_t &id, float flDelay, int nRandomSeed) {
  if (!IsUniqueIdValid(id)) return nullptr;

  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(id);
  if (!pDef) {
    char pBuf[256];
...
bool CParticleSystemMgr::IsParticleSystemDefined(const DmObjectId_t &id) {
  if (!IsUniqueIdValid(id)) return false;
...
bool CParticleSystemMgr::IsParticleSystemDefined(const char *pName) {
  ...
  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(pName);
  return (pDef != nullptr);
}
...
//--------------------------------------------------------------------------------
CDmxElement *CParticleSystemMgr::CreateParticleDmxElement(
    const DmObjectId_t &id) {
  CParticleSystemDefinition *pDef =
      m_pParticleSystemDictionary->FindParticleSystem(id);
...
// Particle sheets
//--------------------------------------------------------------------------------
CSheet *CParticleSystemMgr::FindOrLoadSheet(char const *pszFname,
                                            ITexture *pTexture) {
  if (!m_bShouldLoadSheets) return nullptr;

  if (m_SheetList.Defined(pszFname)) return m_SheetList[pszFname];
...
CSheet *CParticleSystemMgr::FindOrLoadSheet(IMaterial *pMaterial) {
  ...
  if (bFoundVar && pVar && pVar->IsDefined()) {
    ...
    if (pTex && !pTex->IsError()) return FindOrLoadSheet(pTex->GetName(), pTex);
  }

  return nullptr;
}

void CParticleSystemMgr::FlushAllSheets() {
...
// Render cache
//-----------------------------------------------------------------------------
void CParticleSystemMgr::ResetRenderCache() {
...
void CParticleSystemMgr::AddToRenderCache(CParticleCollection *pParticles) {
  ...
  for (; iRenderCache < nRenderCacheCount; ++iRenderCache) {
    if ((pParticles->m_pDef->GetMaterial() ==
  ...
  for (CParticleCollection *p = pParticles->m_Children.m_pHead; p;
       p = p->m_pNext) {
    AddToRenderCache(p);
  }
}

void CParticleSystemMgr::BuildBatchList(int iRenderCache,
  ...
  for (int iCache = 0; iCache < nCacheCount; ++iCache) {
    CParticleCollection *pParticles =
        m_RenderCache[iRenderCache].m_ParticleCollections[iCache];
    if (!pParticles->IsValid()) continue;
    ...
    for (int j = 0; j < nRenderCount; ++j) {
      ...
      while (nFirstParticle < pParticles->m_nActiveParticles) {
        ...
        step.m_nParticleCount = step.m_pRenderer->GetParticlesToRender(
            pParticles, step.m_pContext, nFirstParticle, nRemainingVertices,
            nRemainingIndices, &step.m_nVertCount, &indexes_used);
        nFirstParticle += step.m_nParticleCount;

        if (step.m_nParticleCount > 0) {
          pBatch->m_nVertCount += step.m_nVertCount;
          pBatch->m_nIndexCount += indexes_used;
          pBatch->m_BatchStep.AddToTail(step);
          Assert(pBatch->m_nVertCount <= nMaxVertices &&
                 pBatch->m_nIndexCount <= nMaxIndices);
        } else {
          if (pBatch->m_nVertCount == 0) break;

          // Not enough room
          Assert(pBatch->m_nVertCount > 0 && pBatch->m_nIndexCount > 0);
          int last_batch_idx = batches.AddToTail();
          pBatch = &batches[last_batch_idx];
          pBatch->m_nVertCount = 0;
          pBatch->m_nIndexCount = 0;
...
void CParticleSystemMgr::DumpProfileInformation() {
#if MEASURE_PARTICLE_PERF
  ...
  g_pFullFileSystem->FPrintf(
      fh, "name, total time, max time, max particles, allocated particles\n");
  for (int i = 0; i < m_pParticleSystemDictionary->NameCount(); i++) {
    ...
    if (p->m_nMaximumActiveParticles)
      g_pFullFileSystem->FPrintf(fh, "%s,%f,%f,%d,%d\n", p->m_Name.Get(),
                                 p->m_flTotalSimTime, p->m_flMaxMeasuredSimTime,
                                 p->m_nMaximumActiveParticles,
                                 p->m_nMaxParticles);
  }
  g_pFullFileSystem->FPrintf(fh, "\n\nopname, total time, max time\n");
  for (usize i = 0; i < std::size(m_ParticleOperators); i++) {
    for (int j = 0; j < m_ParticleOperators[i].Count(); j++) {
      ...
      if (fltotal > 0.0)
        g_pFullFileSystem->FPrintf(fh, "%s,%f,%f\n",
                                   m_ParticleOperators[i][j]->GetName(),
                                   fltotal, flmax);
    }
  }
...
#if MEASURE_PARTICLE_PERF
  if (1) {
    if (bCommit) m_nNumFramesMeasured++;
    for (int i = 0; i < m_pParticleSystemDictionary->NameCount(); i++) {
      CParticleSystemDefinition *p = (*m_pParticleSystemDictionary)[i];
      ...
      for (int j = 0; j < m_ParticleOperators[i].Count(); j++) {
        if (bCommit)
          m_ParticleOperators[i][j]->m_flTotalExecutionTime +=
              m_ParticleOperators[i][j]->m_flUncomittedTime;
...
void CParticleSystemMgr::DrawRenderCache(bool bShadowDepth) {
  ...
  VPROF_BUDGET("CParticleSystemMgr::DrawRenderCache",
  ...
  for (int iRenderCache = 0; iRenderCache < nRenderCacheCount; ++iRenderCache) {
    ...
    for (int i = 0; i < nBatchCount; ++i) {
      ...
      meshBuilder.Begin(pMesh, MATERIAL_TRIANGLES, batch.m_nVertCount,
                        batch.m_nIndexCount);
      ...
      for (int j = 0; j < nBatchStepCount; ++j) {
        ...
        step.m_pRenderer->RenderUnsorted(
            step.m_pParticles, step.m_pContext, pRenderContext, meshBuilder,
            nVertexOffset, step.m_nFirstParticle, step.m_nParticleCount);
...
void IParticleSystemQuery::GetRandomPointsOnControllingObjectHitBox(
    CParticleCollection *pParticles, int nControlPointNumber, int nNumPtsOut,
    float flBBoxScale, int nNumTrysToGetAPointInsideTheModel, Vector *pPntsOut,
    Vector vecDirectionalBias, Vector *pHitBoxRelativeCoordOut,
    int *pHitBoxIndexOut) const {
  for (int i = 0; i < nNumPtsOut; i++) {
    ...
    if (pHitBoxIndexOut) pHitBoxIndexOut[i] = -1;
  }
}
...
void CParticleCollection::UpdateHitBoxInfo(int nControlPointNumber) {
  ...
  if (hb.m_flLastUpdateTime == m_flCurTime) return;  // up to date

  hb.m_flLastUpdateTime = m_flCurTime;

  // make sure space allocated
  if (!hb.m_pHitBoxes) hb.m_pHitBoxes = new ModelHitBoxInfo_t[MAXSTUDIOBONES];
  if (!hb.m_pPrevBoxes) hb.m_pPrevBoxes = new ModelHitBoxInfo_t[MAXSTUDIOBONES];
  ...
  // issue hitbox query
  hb.m_nNumHitBoxes =
      g_pParticleSystemMgr->Query()->GetControllingObjectHitBoxInfo(
