...
// classes for keeping a dictionary of sheet files in memory.  A sheet is a
// bunch of frames packewd within one texture. Each sheet has 1 or more frame
// sequences stored for it.

// for fast lookups to retrieve sequence data, we store the sequence information
// discretized into a fixed # of frames. If this discretenesss is a visual
// problem, you can lerp the blend values to get it perfect.
#define SEQUENCE_SAMPLE_COUNT 1024u
...
struct SheetSequenceSample_t {
  ...
  SequenceSampleTextureCoords_t
      m_TextureCoordData[MAX_IMAGES_PER_FRAME_IN_MEMORY];
