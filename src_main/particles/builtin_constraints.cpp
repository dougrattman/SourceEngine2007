...
bool C_OP_ConstrainDistance::EnforceConstraint(int nStartBlock, int nNumBlocks,
  ...
  if (m_bGlobalCenter)
    ...
    vecCenter +=
        pParticles->TransformAxis(m_CenterOffset, true, m_nControlPointNumber);
  }
  FourVectors Center;
  ...
  do {
    ...
    if (IsAnyNegative(NeedAdjust))  // any out of bounds?
    {
      ...
      clamp_near += Center;
      pts.x = MaskedAssign(TooCloseMask, clamp_near.x,
                           MaskedAssign(TooFarMask, clamp_far.x, pXYZ->x));
      pts.y = MaskedAssign(TooCloseMask, clamp_near.y,
                           MaskedAssign(TooFarMask, clamp_far.y, pXYZ->y));
      pts.z = MaskedAssign(TooCloseMask, clamp_near.z,
                           MaskedAssign(TooFarMask, clamp_far.z, pXYZ->z));
      *(pXYZ) = pts;
...
DEFINE_PARTICLE_OPERATOR(C_OP_ConstrainDistance,
...
bool C_OP_ConstrainDistanceToPath::EnforceConstraint(
    int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
    void *pContext) const {
  ...
  fltx4 TimeScale = ReplicateX4(1.0 / (std::max(0.001f, m_flTravelTime)));
  ...
  do {
    ...
    // calculate radius at the point. !!speed!! - use speical case for constant
    // radius

    fltx4 dist_squared = pts * pts;
    fltx4 TooFarMask = CmpGtSIMD(dist_squared, SIMDMaxDist2);
    if ((!bConstantRadius) && (!IsAnyNegative(TooFarMask))) {
      // need to calculate and adjust for true radius =- we've only trivilally
      // rejected note voodoo here - we update simdmaxdist for true radius, but
      // not max dist^2, since that's used only for the trivial reject case,
...
DEFINE_PARTICLE_OPERATOR(
    ...
    "Constrain distance to path between two control points", OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("maximum distance middle", "-1", float,
                        m_flMaxDistanceMid)
...
bool C_OP_PlanarConstraint::EnforceConstraint(int nStartBlock, int nNumBlocks,
                                              CParticleCollection *pParticles,
                                              void *pContext) const {
  C4VAttributeWriteIterator pXYZ(PARTICLE_ATTRIBUTE_XYZ, pParticles);
  pXYZ += nStartBlock;
  ...
  PlaneNormal.DuplicateVector(pParticles->TransformAxis(
      m_PlaneNormal, !m_bGlobalNormal, m_nControlPointNumber));
  ...
  if (m_bGlobalOrigin) {
    ...
    Vector ofs =
        pParticles->TransformAxis(m_PointOnPlane, true, m_nControlPointNumber);
    Vector vecCenter;
    pParticles->GetControlPointAtTime(m_nControlPointNumber,
                                      pParticles->m_flCurTime, &vecCenter);
...
DEFINE_PARTICLE_OPERATOR(C_OP_PlanarConstraint,
                         "Prevent passing through a plane", OPERATOR_GENERIC);
...
void CWorldCollideContextData::CalculatePlanes(CParticleCollection *pParticles,
                                               int nCollisionMode,
                                               int nCollisionGroup,
                                               Vector const *pCPOffset,
  ...
  if (m_nActivePlanes && (nCollisionMode == COLLISION_MODE_INITIAL_TRACE_DOWN))
    return;
  Vector RayStart =
      pParticles->GetControlPointAtCurrentTime(0);  // allow config + offset

  if (pCPOffset) RayStart += *pCPOffset;

  if ((m_flLastUpdateTime > 0.) &&
      ((RayStart - m_vecLastUpdateOrigin).LengthSqr() <
       Square(flDistanceTolerance)))
    return;
  ...
  for (int i = -1; i <= 1; i++)
    for (int j = -1; j <= 1; j++)
      for (int k = -1; k <= 1; k++) {
        if (((nCollisionMode == COLLISION_MODE_PER_FRAME_PLANESET) &&
             (i || j || k)) ||
          ...
          if (tr.fraction < 1.0) {
            ...
            m_PointOnPlane[n].DuplicateVector(
                RayStart + (tr.fraction * flRadius) * TraceDir);
...
void C_OP_WorldCollideConstraint::SetupConstraintPerFrameData(
    CParticleCollection *pParticles, void *pContext) const {
  CWorldCollideContextData *pCtx =
      reinterpret_cast<CWorldCollideContextData *>(pContext);
  pCtx->CalculatePlanes(pParticles, COLLISION_MODE_PER_FRAME_PLANESET,
                        COLLISION_GROUP_NONE);
}

bool C_OP_WorldCollideConstraint::EnforceConstraint(
    int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
    void *pContext) const {
  C4VAttributeWriteIterator pXYZ(PARTICLE_ATTRIBUTE_XYZ, pParticles);
  pXYZ += nStartBlock;
...
DEFINE_PARTICLE_OPERATOR(C_OP_WorldCollideConstraint,
...
static void WorldIntersectT(FourVectors const *pStartPnt,
                            FourVectors const *pEndPnt, int nCollisionGroup,
                            int nMask, ISectData_t *pISectData,
                            CWorldCollideContextData *pCtx) {
  ...
  if (pCtx) {
    ...
    for (int i = 0; i < pCtx->m_nActivePlanes; i++) {
      ...
      fltx4 fl4CrossMask = AndSIMD(CmpGeSIMD(fl4StartDist, Four_Zeros),
                                   CmpLtSIMD(fl4EndDist, Four_Zeros));
      if (IsAnyNegative(fl4CrossMask)) {
        // a hit!
        fltx4 fl4T = DivSIMD(fl4StartDist, SubSIMD(fl4StartDist, fl4EndDist));
        fl4CrossMask =
            AndSIMD(fl4CrossMask, CmpLtSIMD(fl4T, pISectData->m_ISectT));
        if (IsAnyNegative(fl4CrossMask)) {
          pISectData->m_ISectT =
              MaskedAssign(fl4CrossMask, fl4T, pISectData->m_ISectT);
          pISectData->m_ISectNormal.x =
              MaskedAssign(fl4CrossMask, pCtx->m_PlaneNormal[i].x,
                           pISectData->m_ISectNormal.x);
          pISectData->m_ISectNormal.y =
              MaskedAssign(fl4CrossMask, pCtx->m_PlaneNormal[i].y,
                           pISectData->m_ISectNormal.y);
          pISectData->m_ISectNormal.z =
              MaskedAssign(fl4CrossMask, pCtx->m_PlaneNormal[i].z,
                           pISectData->m_ISectNormal.z);
        }
      }
    }
    pISectData->m_LeftOverT =
        MaxSIMD(Four_Zeros, SubSIMD(Four_Ones, pISectData->m_ISectT));
  } else {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      CBaseTrace tr;
      g_pParticleSystemMgr->Query()->TraceLine(start, end, nMask, NULL,
                                               nCollisionGroup, &tr);
...
bool C_OP_WorldTraceConstraint::EnforceConstraint(
    int nStartBlock, int nNumBlocks, CParticleCollection *pParticles,
    void *pContext) const {
  ...
  bool bBouncingOrSliding =
      (m_flBounceAmount != 0.0) || (m_flSlideAmount != 0.0);
  ...
  if (m_bBrushOnly) nMask = MASK_SOLID_BRUSHONLY;
  ...
  if ((m_nCollisionMode == COLLISION_MODE_PER_FRAME_PLANESET) ||
    ...
    if (pCtx->m_flLastUpdateTime != pParticles->m_flCurTime) {
      pCtx->CalculatePlanes(pParticles, m_nCollisionMode,
                            m_nCollisionGroupNumber, &m_vecCpOffset,
  ...
  do {
    ...
    WorldIntersectT(pPrevXYZ, &endPnt, m_nCollisionGroupNumber, nMask, &iData,
                    pCtx);
    ...
    if (IsAnyNegative(didhit))  // any penetration?
    {
      ...
      if (bBouncingOrSliding) {
        // need to compute movement due to sliding and bouncing, and add it to
        // the point, and also compute the new velocity, adjust prev pnt to
        // reflect that new velocity

        FourVectors bouncePart =
            VectorReflect(deltanormalized, iData.m_ISectNormal);
...
DEFINE_PARTICLE_OPERATOR(C_OP_WorldTraceConstraint, "Collision via traces",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("control point offset for fast collisions", "0 0 0",
                        Vector, m_vecCpOffset)
DMXELEMENT_UNPACK_FIELD("control point movement distance tolerance", "5", float,
                        m_flCpMovementTolerance)
