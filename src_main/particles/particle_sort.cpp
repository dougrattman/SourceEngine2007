...
struct SortBucket_t {
  ParticleRenderData_t
      *m_pBucketData[(MAX_PARTICLES_IN_A_SYSTEM + BUCKET_BLOCK_SIZE - 1) /
                     BUCKET_BLOCK_SIZE];
};
...
#define NUM_BUCKETS_NEEDED_IN_POOL \
  (SORT_BUCKET_SIZE +              \
   ((MAX_PARTICLES_IN_A_SYSTEM + BUCKET_BLOCK_SIZE)) / BUCKET_BLOCK_SIZE)

static ParticleRenderData_t
    s_BlockPool[BUCKET_BLOCK_SIZE * NUM_BUCKETS_NEEDED_IN_POOL];
...
void CParticleCollection::GenerateFirstSortPassData(Vector CameraPos) {
  ...
  const FourVectors *pXYZ =
      Get4VAttributePtr(PARTICLE_ATTRIBUTE_XYZ, &nXYZ_stride);

  size_t ct_stride;
  const fltx4 *pCreationTimeStamp =
      GetM128AttributePtr(PARTICLE_ATTRIBUTE_CREATION_TIME, &ct_stride);

  // 	size_t seq_stride;
  // 	const fltx4 *pSequenceNumber = GetM128AttributePtr(
  // PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER, &seq_stride );

  // 	size_t ld_stride;
  // 	const fltx4 *pLifeDuration = GetM128AttributePtr(
  ...
  while (nParticles > 0) {
    ...
    // the 2.0 is added to the squared distance to prevent negative exponents so
    // I can ignore exponent high bits in my ieee hackery.
    fltx4 Distance_squared =
        AddSIMD(Four_Twos,
                AddSIMD(MulSIMD(Xdiff, Xdiff),
                        AddSIMD(MulSIMD(Ydiff, Ydiff), MulSIMD(Zdiff, Zdiff))));
    ...
    for (int nSub = 0; nSub < std::min(4, nParticles); nSub++) {
      uint32_t nDistanceKey = SubInt(Distance_squared, nSub);
      // now, do some bit-twiddling to get it the way we like the addition
      // of 2.0 to the distance^2 allows us to remove the exponent bias because
      // now we will have no negative exponents
      nDistanceKey -= (127 << 23);
      // 			int exponent=nDistanceKey>>23;
      // 			int mantissa=nDistanceKey & ((1<<23)-1);
      nDistanceKey >>= 12;  // move most exponent bits down
      nDistanceKey = std::min(nDistanceKey, 0x3ffffu);

      int nBucket = nDistanceKey & (SORT_BUCKET_SIZE - 1);
      int nBucketBlockIndex =
          (s_nNumParticlesInBucket[nBucket] >> BUCKET_BLOCK_SHIFT);
      ...
      s_nNumParticlesToGoInBucketInPassTwo
          [new_record->m_nPass2SortKey]++;  // update histogram for pass 2
...
void CParticleCollection::GenerateSortedIndexList(Vector vecCamera) {
  VPROF_BUDGET("CParticleCollection::GenerateSortedIndexList",
               VPROF_BUDGETGROUP_PARTICLE_RENDERING);
  memset(s_nNumParticlesInBucket, 0, sizeof(s_nNumParticlesInBucket));
  memset(s_nNumParticlesToGoInBucketInPassTwo, 0,
         sizeof(s_nNumParticlesToGoInBucketInPassTwo));

  // perform distance caluclation, first pass radix sort, and histogram
  // calculation for 2nd pass.

  s_AvailableBlockPtr = s_BlockPool;
  GenerateFirstSortPassData(vecCamera);

  // now, we will perform the last radix sort pass, serializing the buckets in
  // groups based upon high bits
  ...
  for (int i = 0; i < SORT_BUCKET_SIZE; i++) {
    ...
    while (nCount) {
      ...
      int nNumInBlock = std::min(nCount, BUCKET_BLOCK_SIZE);
      nCount -= nNumInBlock;
      do {
  ...
  // magically, we should now be done. lets check in case the code doesn't work
  // the first time
#if DEBUG_PARTICLE_SORT
  for (int i = 0; i < nTotalParticles - 1; i++) {
    Assert(s_SortedIndexList[i].m_nSortKey <=
           s_SortedIndexList[i + 1].m_nSortKey);
    // 		if ( fabs( s_SortedIndexList[i].m_flDistance -
    // s_SortedIndexList[i+1].m_flDistance )>10)
    // 		{
    // 			Assert( s_SortedIndexList[i].m_flDistance <=
    // s_SortedIndexList[i+1].m_flDistance );
    // 		}
  }
#endif
}

const ParticleRenderData_t *CParticleCollection::GetRenderList(
