...
CSheet::CSheet(CUtlBuffer &buf) {
  memset(m_pSamples, 0, sizeof(m_pSamples));
  memset(m_bClamp, 0, sizeof(m_bClamp));
  memset(m_bSequenceIsCopyOfAnotherSequence, 0,
         sizeof(m_bSequenceIsCopyOfAnotherSequence));
  ...
  while (nNumSequences--) {
    ...
    m_pSamples[nSequenceNumber] = new SheetSequenceSample_t[nTimeSamples];
    ...
    for (int nFrm = 0; nFrm < nFrameCount; nFrm++) {
      ...
      InterpKnot[nFrm] =
          SEQUENCE_SAMPLE_COUNT * (fCurTime / fTotalSequenceTime);
      ...
      if (nNumCoordsPerFrame == 1) seq.CopyFirstFrameToOthers();
    ...
    for (int nIdx = 0; nIdx < nTimeSamples; nIdx++) {
      ...
      GetInterpolationData(
          InterpKnot, InterpValue, nFrameCount, SEQUENCE_SAMPLE_COUNT, nIdx,
          !(m_bClamp[nSequenceNumber]), &flIdxA, &flIdxB, &flInterp);
  ...
  // now, fill in all unseen sequences with copies of the first seen sequence to
  // prevent crashes while editing
