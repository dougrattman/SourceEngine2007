  ...
  for (int i = 0; i < nBlocks; i++) {
    pAccumulatedForces->x =
        AddSIMD(pAccumulatedForces->x,
                AddSIMD(box_min.x, MulSIMD(box_max.x, RandSIMD(nContext))));
    pAccumulatedForces->y =
        AddSIMD(pAccumulatedForces->y,
                AddSIMD(box_min.y, MulSIMD(box_max.y, RandSIMD(nContext))));
    pAccumulatedForces->z =
        AddSIMD(pAccumulatedForces->z,
                AddSIMD(box_min.z, MulSIMD(box_max.z, RandSIMD(nContext))));
    pAccumulatedForces++;
  }
...
void C_OP_TwistAroundAxis::AddForces(FourVectors *pAccumulatedForces,
                                     ...
                                     int nBlocks, float flStrength,
                                     void *pContext) const {
  FourVectors Twist_AxisInWorldSpace;
  Twist_AxisInWorldSpace.DuplicateVector(
      pParticles->TransformAxis(m_TwistAxis, m_bLocalSpace));
  ...
  const FourVectors *pPos =
      pParticles->Get4VAttributePtr(PARTICLE_ATTRIBUTE_XYZ, &nPosStride);
  for (int i = 0; i < nBlocks; i++) {
...
DEFINE_PARTICLE_OPERATOR(C_OP_TwistAroundAxis, "twist around axis",
                         OPERATOR_GENERIC);
...
void C_OP_AttractToControlPoint::AddForces(FourVectors *pAccumulatedForces,
  ...
  int power_frac =
      -4.0 *
      m_fFalloffPower;  // convert to what pow_fixedpoint_exponent_simd wants
  ...
  for (int i = 0; i < nBlocks; i++) {
    ...
    ofs *= Pow_FixedPoint_Exponent_SIMD(len,
                                        power_frac);  // * 1/pow(dist, exponent)
...
DEFINE_PARTICLE_OPERATOR(C_OP_AttractToControlPoint,
                         "Pull towards control point", OPERATOR_GENERIC);
...
class C_OP_LennardJonesForce : public CParticleOperatorInstance {
 ...
 private:
  // virtual void addParticleForce(ImpParticle* a, ImpParticleCacheNode* bcn,
  // float flStrength, float ts) const;
  virtual void addParticleForce(ImpParticle *a, ImpParticle *b, float distSq,
                                float flStrength, float ts) const;
};

// TODO: I should make sure I don't have divide by zero errors.
// TODO: ts is not used
void C_OP_LennardJonesForce::addParticleForce(ImpParticle *a, ImpParticle *b,
                                              float distSq, float flStrength,
                                              float ts) const {
  ...
  if (a->group == b->group)  // In the same group
  {
    ...
    // I tried this, and discovered that rather than averaging, I can take maybe
    // take the larger of the two neighbor counts, so the attraction between two
    // particles on the surface will be strong, but the attraction between a
    // particle inside and a particle on the surface will be weak. I can also
    // try taking the min so that the attraction between a particle on the
    // surface and a particle inside the fluid will be strong, but the
    // attraction between two particles completely on the inside will be weak.
    //
    // int symmetric_neighbor_count = std::min(a->neighbor_count,
    // b->neighbor_count);
    //
    // Can try having neighbors only cause stronger attraction (no repulsion)
    // Can try lower exponents for the LennardJones forces.

    // This is a trick to prevent single particles from floating off... the less
    // neighbors a particle has.. the more it sticks This also tends to simulate
    // surface tension
    float surface_tension_modifier =
        ((24.0f * m_fSurfaceTension) /
         (a->neighbor_count + b->neighbor_count + 0.1f)) +
        1.0f;
    // float lennard_jones_force = fLennardJones * 2.0f * (p2 - (p4 * p4));
    float lennard_jones_force =
        m_fLennardJonesAttraction * p2 - m_fLennardJonesRepulsion * p4;
    f = surface_tension_modifier * lennard_jones_force;

    // This is some older code:
    // f = ((35.0f * LampScene::simulationSurfaceTension) / (a->neighbor_count +
    // 0.1f)) * (p2 - (p4 * p4));
    // used to be 68'

    // float factor = (b->neighbor_count < 13 && neighbor_count < 13 ? 4.0f :
    // 0.5f); f = factor * (p2 - (p2 * p2 * p2 * p2));
  } else {
  ...
  Point3D scaledr =
      ...
      (f /
       (d +
        FLT_EPSILON));  // Dividing by d scales distance down to a unit vector
  a->force.add(scaledr);
...
void C_OP_LennardJonesForce::AddForces(FourVectors *pAccumulatedForces,
                                       ...
                                       void *pContext) const {
  int nParticles =
      pParticles->m_nActiveParticles;  // Not sure if this is correct!
  ...
  const FourVectors *pPrevPos=pParticles->Get4VAttributePtr(
  PARTICLE_ATTRIBUTE_PREV_XYZ, &nPrevPosStride );
  */
  ...
  float nearNeighborInteractionRadiusSq =
      nearNeighborInteractionRadius * nearNeighborInteractionRadius;
  ...
  for (int i = 0; i < nParticles; i++) {
    ...
    while (node->particle != NULL) {
      ...
      // Compare addresses of the two particles. This makes sure we apply a
      // force only once between a pair of particles.
  ...
  for(ParticleListNode* bit3 = particles; bit3; bit3 = bit3->next)
  {
          ...
          //b1->addDirDragForce();
          b->move(ts); // Move the particle (it should never be used again until
  next iteration)
  }
  */
...
DEFINE_PARTICLE_OPERATOR(C_OP_LennardJonesForce, "lennard jones force",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("lennard jones attractive force", "1", float,
                        m_fLennardJonesAttraction)
DMXELEMENT_UNPACK_FIELD("lennard jones repulsive force", "1", float,
                        m_fLennardJonesRepulsion)
