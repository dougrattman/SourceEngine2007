...
class C_OP_InstantaneousEmitter : public CParticleOperatorInstance {
  ...
  virtual void InitParams(CParticleSystemDefinition *pDef,
    ...
    m_nScaleControlPointField = std::clamp(m_nScaleControlPointField, 0, 2);
  }

  virtual void StopEmission(CParticleCollection *pParticles, void *pContext,
                            bool bInfiniteOnly) const {
    InstantaneousEmitterContext_t *pCtx =
        reinterpret_cast<InstantaneousEmitterContext_t *>(pContext);
    if (!bInfiniteOnly) {
  ...
  virtual void SkipToTime(float flTime, CParticleCollection *pParticles,
                          void *pContext) const {
    // NOTE: This is a bit of a hack. We're saying that if we're skipping more
    // than two seconds, that we're
    //		 probably not going to bother emitting at all.  Really, this
    // would have to know the maximum 		 lifetime of the child particles
    // and only skip if past that.
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
    ...
    InstantaneousEmitterContext_t *pCtx =
        reinterpret_cast<InstantaneousEmitterContext_t *>(pContext);
    if (m_nMinParticlesToEmit >= 0) {
      pCtx->m_ActualParticlesToEmit =
          pParticles->RandomInt(m_nMinParticlesToEmit, m_nParticlesToEmit);
    } else {
...
DEFINE_PARTICLE_OPERATOR(C_OP_InstantaneousEmitter, "emit_instantaneously",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("emission count scale control point", "-1", int,
                        m_nScaleControlPoint)
DMXELEMENT_UNPACK_FIELD("emission count scale control point field", "0", int,
                        m_nScaleControlPointField)
END_PARTICLE_OPERATOR_UNPACK(C_OP_InstantaneousEmitter)

uint32_t C_OP_InstantaneousEmitter::Emit(CParticleCollection *pParticles,
                                         float flCurStrength,
                                         void *pContext) const {
  // Don't emit any more if the particle system has emitted all it's supposed
  // to.
  InstantaneousEmitterContext_t *pCtx =
      reinterpret_cast<InstantaneousEmitterContext_t *>(pContext);
  if (pCtx->m_nRemainingParticles <= 0) return 0;

  // Wait until we're told to start emitting
  float flStartTime = m_flStartTime + pCtx->m_flTimeOffset;
  if (pParticles->m_flCurTime < flStartTime) return 0;
  ...
  if ((m_nScaleControlPoint >= 0) && !pCtx->m_bReadScaleFactor) {
    ...
    if (flStartTime <= pParticles->m_flCurTime &&
      ...
      pParticles->GetControlPointAtTime(m_nScaleControlPoint, flStartTime,
                                        &vecScale);
    } else {
  ...
  pCtx->m_nRemainingParticles = std::max(pCtx->m_nRemainingParticles, 0);

  // NOTE: Applying the scale here because I don't believe we can sample the
  // control point values inside We're only allowed to emit so many particles,
  // though.. If we run out of room, only emit the last N particles
  ...
  int nParticlesThisFrame =
      std::min(m_nPerFrameNum, pCtx->m_nRemainingParticles);
  nAllowedParticlesToEmit =
      std::min(nAllowedParticlesToEmit, nParticlesThisFrame);
  int nActualParticlesToEmit =
      std::min(nAllowedParticlesToEmit,
               pCtx->m_ActualParticlesToEmit * g_nParticle_Multiplier);
  ...
  pParticles->SetNActiveParticles(nActualParticlesToEmit +
                                  pParticles->m_nActiveParticles);

  // !! speed!! do sse init here
  for (int i = nStartParticle; i < nStartParticle + nActualParticlesToEmit;
       i++) {
    float *pTimeStamp = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_CREATION_TIME, i);
...
class C_OP_ContinuousEmitter : public CParticleOperatorInstance {
  ...
  virtual void StopEmission(CParticleCollection *pParticles, void *pContext,
                            bool bInfiniteOnly) const {
    ContinuousEmitterContext_t *pCtx =
        reinterpret_cast<ContinuousEmitterContext_t *>(pContext);
  ...
  // Called when the SFM wants to skip forward in time
  // Currently hacked for save/load pre-sim - correct solution is to serialize
  // rather than skip-to-time and simulate
  virtual void SkipToTime(float flTime, CParticleCollection *pParticles,
                          void *pContext) const {
    ContinuousEmitterContext_t *pCtx =
        reinterpret_cast<ContinuousEmitterContext_t *>(pContext);
    float flStartTime = m_flStartTime + pCtx->m_flTimeOffset;
    if (flTime <= flStartTime) return;
    ...
    flDeltaTime = std::min(flDeltaTime, 4.0f);
    ...
    if (pParticles->m_nMaxAllowedParticles <
        nStartParticle + nActualParticlesToEmit) {
      nActualParticlesToEmit =
          pParticles->m_nMaxAllowedParticles - nStartParticle;
    }
    ...
    // Set the particle creation time to the exact sub-frame particle emission
    // time
    // !! speed!! do sse init here
    for (int i = nStartParticle; i < nStartParticle + nActualParticlesToEmit;
         i++) {
...
DEFINE_PARTICLE_OPERATOR(C_OP_ContinuousEmitter, "emit_continuously",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("scale emission to used control points", "0.0", float,
                        m_flEmissionScale)
DMXELEMENT_UNPACK_FIELD("use parent particles for emission scaling", "0", bool,
                        m_bScalePerParticle)
END_PARTICLE_OPERATOR_UNPACK(C_OP_ContinuousEmitter)

uint32_t C_OP_ContinuousEmitter::Emit(CParticleCollection *pParticles,
                                      float flCurStrength,
                                      void *pContext) const {
  ...
  float flControlPointScale = pParticles->GetHighestControlPoint();
  // The emission scale here allows for a scalar value per controlpoint, like 2
  // or .25...
  flControlPointScale *= m_flEmissionScale;
  ...
  if (flControlPointScale != 0.0f || m_bScalePerParticle) {
    if (m_bScalePerParticle) {
      if (pParticles->m_pParent) {
        flControlPointScale =
            pParticles->m_pParent->m_nActiveParticles * m_flEmissionScale;
      } else {
        flControlPointScale = m_flEmissionScale;
      }
    }
  ...
  // determine our previous and current draw times and clamp them to start time
  // and emission duration
  ...
  // Calculate emission rate by delta time from last frame to determine number
  // of particles to emit this frame as a fractional float
  ...
  // Floor float accumulated value and subtract whole int emitted so far from
  // the result to determine total whole particles to emit this frame
  int nParticlesToEmit =
      floor(pCtx->m_flTotalActualParticlesSoFar) - pCtx->m_nTotalEmittedSoFar;
  ...
  if (nAllowedParticlesToEmit < nParticlesToEmit) {
    ...
    // flStartEmissionTime = pCtx->m_flNextEmitTime - flTimePerEmission *
    // nActualParticlesToEmit;
  }
  if (nActualParticlesToEmit == 0) return 0;
...
class C_OP_NoiseEmitter : public CParticleOperatorInstance {
  ...
  virtual void StopEmission(CParticleCollection *pParticles, void *pContext,
                            bool bInfiniteOnly) const {
    NoiseEmitterContext_t *pCtx =
        reinterpret_cast<NoiseEmitterContext_t *>(pContext);
    if (!bInfiniteOnly || IsInfinitelyEmitting()) {
      pCtx->m_bStoppedEmission = true;
    }
  }
  ...
  virtual void SkipToTime(float flTime, CParticleCollection *pParticles,
    ...
    pCtx->m_flTotalActualParticlesSoFar =
        (pParticles->m_flCurTime - flStartTime) * flEmissionRate + 1;

    // if ( !IsInfinitelyEmitting() )
    //	pCtx->m_flTotalActualParticlesSoFar = std::min(
    // pCtx->m_ActualParticlesToEmit, pCtx->m_flTotalActualParticlesSoFar );
    pCtx->m_nTotalEmittedSoFar = 0;
  }
...
DMXELEMENT_UNPACK_FIELD("time noise coordinate scale", "0.1", float,
                        m_flNoiseScale)
// DMXELEMENT_UNPACK_FIELD( "spatial noise coordinate
// scale","0.001",float,m_flNoiseScaleLoc)
...
uint32_t C_OP_NoiseEmitter::Emit(CParticleCollection *pParticles,
  ...
  Coord = Vector((pParticles->m_flCurTime + Offset),
                 (pParticles->m_flCurTime + Offset),
                 (pParticles->m_flCurTime + Offset));
  Coord *= CoordScale;
  ...
  *((int *)&flNoise) &= nAbsVal;
  ...
  flInitialNoise =
      std::clamp(flInitialNoise, 0.0f, static_cast<float>(INT_MAX));
  ...
  float flTimeStampStep =
      (flCurrDrawTime - flPrevDrawTime) / (nActualParticlesToEmit);
  float flTimeStep = flPrevDrawTime + flTimeStampStep;
