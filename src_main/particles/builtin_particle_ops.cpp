...
#if VPROF_LEVEL > 0
#define START_OP                          \
  float flOpStartTime = Plat_FloatTime(); \
  VPROF_ENTER_SCOPE(pOp->GetDefinition()->GetName())
#else
...
#define END_OP                                               \
  if (1) {                                                   \
    ...
    IParticleOperatorDefinition *pDef =                      \
        (IParticleOperatorDefinition *)pOp->GetDefinition(); \
...
DEFINE_PARTICLE_OPERATOR(C_OP_BasicMovement, "Movement Basic",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("max constraint passes", "3", int,
...
static void CHECKSYSTEM(CParticleCollection *pParticles) {
  //	Assert( pParticles->m_nActiveParticles <=
  // pParticles->m_pDef->m_nMaxParticles );
...
void C_OP_BasicMovement::Operate(CParticleCollection *pParticles,
  ...
  // fltx4 adj_dt = ReplicateX4( (1.0-m_fDrag) * ( pParticles->m_flDt /
  // pParticles->m_flPreviousDt ) );
  fltx4 adj_dt =
      ReplicateX4((pParticles->m_flDt / pParticles->m_flPreviousDt) *
                  ExponentialDecay((1.0f - std::max(0.0f, m_fDrag)),
                                   (1.0f / 30.0f), pParticles->m_flDt));
  ...
  do {
    fltx4 accX = MulSIMD(pAccIn->x, DtSquared);
    fltx4 accY = MulSIMD(pAccIn->y, DtSquared);
    fltx4 accZ = MulSIMD(pAccIn->z, DtSquared);

    // we will write prev xyz, and swap prev and cur at the end
    prev_xyz->x = AddSIMD(
        xyz->x, AddSIMD(accX, MulSIMD(adj_dt, SubSIMD(xyz->x, prev_xyz->x))));
    prev_xyz->y = AddSIMD(
        xyz->y, AddSIMD(accY, MulSIMD(adj_dt, SubSIMD(xyz->y, prev_xyz->y))));
    prev_xyz->z = AddSIMD(
        xyz->z, AddSIMD(accZ, MulSIMD(adj_dt, SubSIMD(xyz->z, prev_xyz->z))));
    CHECKSYSTEM(pParticles);
    ++prev_xyz;
    ++xyz;
    pAccIn += nForceStride;
  ...
  if (nConstraints && pParticles->m_nPaddedActiveParticles) {
    ...
    for (int i = 0; i < nConstraints; i++) {
      bFinalConstraint[i] =
          pParticles->m_pDef->m_Constraints[i]->IsFinalConstaint();
          ...
          if ((!bFinalConstraint[i]) &&
            ...
            if (bDidSomething) {
              ...
              for (int j = 0; j < nConstraints; j++)
                if (i != j) bConstraintSatisfied[j] = false;
            }
          }
        }
      }
    ...
    for (int i = 0; i < nConstraints; i++) {
      ...
      if ((bFinalConstraint[i]) &&
          (pParticles->CheckIfOperatorShouldRun(pOp))) {
        START_OP;
...
class C_OP_FadeAndKill : public CParticleOperatorInstance {
  ...
  uint32_t GetReadInitialAttributes() const {
    return PARTICLE_ATTRIBUTE_ALPHA_MASK;
  }
...
DEFINE_PARTICLE_OPERATOR(C_OP_FadeAndKill, "Alpha Fade and Decay",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("start_fade_out_time", "0.5", float,
...
void C_OP_FadeAndKill::Operate(CParticleCollection *pParticles,
                               float flStrength, void *pContext) const {
  CM128AttributeIterator pCreationTime(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                       pParticles);
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
                                       pParticles);
  CM128InitialAttributeIterator pInitialAlpha(PARTICLE_ATTRIBUTE_ALPHA,
                                              pParticles);
  ...
  fltx4 fl4FadeInDuration =
      ReplicateX4(m_flEndFadeInTime - m_flStartFadeInTime);
  fltx4 fl4OOFadeInDuration = ReciprocalEstSIMD(fl4FadeInDuration);

  fltx4 fl4FadeOutDuration =
      ReplicateX4(m_flEndFadeOutTime - m_flStartFadeOutTime);
  ...
  for (int i = 0; i < nLimit; i += 4) {
    ...
    fltx4 fl4KillMask = CmpGeSIMD(
        fl4Age, *pLifeDuration);  // takes care of lifeduration = 0 div 0
    fl4Age =
        MulSIMD(fl4Age, ReciprocalEstSIMD(fl4ParticleLifeTime));  // age 0..1
    fltx4 fl4FadingInMask =
        AndNotSIMD(fl4KillMask, AndSIMD(CmpLeSIMD(fl4StartFadeInTime, fl4Age),
                                        CmpGtSIMD(fl4EndFadeInTime, fl4Age)));
    fltx4 fl4FadingOutMask =
        AndNotSIMD(fl4KillMask, AndSIMD(CmpLeSIMD(fl4StartFadeOutTime, fl4Age),
                                        CmpGtSIMD(fl4EndFadeOutTime, fl4Age)));
    if (IsAnyNegative(fl4FadingInMask)) {
      fltx4 fl4Goal = MulSIMD(*pInitialAlpha, fl4StartAlpha);
      fltx4 fl4NewAlpha = SimpleSplineRemapValWithDeltasClamped(
          fl4Age, fl4StartFadeInTime, fl4FadeInDuration, fl4OOFadeInDuration,
    ...
    if (IsAnyNegative(fl4FadingOutMask)) {
      ...
      fltx4 fl4NewAlpha = SimpleSplineRemapValWithDeltasClamped(
          fl4Age, fl4StartFadeOutTime, fl4FadeOutDuration, fl4OOFadeOutDuration,
    ...
    if (IsAnyNegative(fl4KillMask)) {
      ...
      if (nMask & 1) pParticles->KillParticle(i);
      if (nMask & 2) pParticles->KillParticle(i + 1);
      if (nMask & 4) pParticles->KillParticle(i + 2);
      if (nMask & 8) pParticles->KillParticle(i + 3);
    }
    ++pCreationTime;
    ++pLifeDuration;
    ++pInitialAlpha;
...
void C_OP_FadeIn::Operate(CParticleCollection *pParticles, float flStrength,
                          void *pContext) const {
  CM128AttributeIterator pCreationTime(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                       pParticles);
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
  ...
  do {
    ...
    if (IsAnyNegative(ApplyMask)) {
      // Fading in
      fltx4 NewAlpha = SimpleSplineRemapValWithDeltasClamped(
          flLifeTime, Four_Zeros, FadeInTime, ReciprocalEstSIMD(FadeInTime),
...
DMXELEMENT_UNPACK_FIELD("fade out time exponent", "1", float,
                        m_flFadeOutTimeExp)
DMXELEMENT_UNPACK_FIELD("proportional 0/1", "1", bool, m_bProportional)
END_PARTICLE_OPERATOR_UNPACK(C_OP_FadeOut)

void C_OP_FadeOut::Operate(CParticleCollection *pParticles, float flStrength,
                           void *pContext) const {
  CM128AttributeIterator pCreationTime(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                       pParticles);
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
  ...
  do {
    fltx4 fl4FadeOutTime = Pow_FixedPoint_Exponent_SIMD(
        ...
        nSSEFixedExponent);
    fl4FadeOutTime =
        AddSIMD(FadeTimeMin, MulSIMD(FadeTimeWidth, fl4FadeOutTime));
    ...
    if (m_bProportional) {
      fl4LifeTime = MulSIMD(SubSIMD(fl4CurTime, *pCreationTime),
                            ReciprocalEstSIMD(fl4LifeDuration));
    ...
    if (IsAnyNegative(ApplyMask)) {
      // Fading out
      fltx4 NewAlpha = SimpleSplineRemapValWithDeltasClamped(
          fl4LifeTime, fl4FadeOutTime, fl4Lifespan,
          ReciprocalEstSIMD(fl4Lifespan), *pInitialAlpha,
          SubSIMD(Four_Zeros, *pInitialAlpha));
...
class C_OP_OscillateScalar : public CParticleOperatorInstance {
  ...
  uint32_t GetWrittenAttributes() const { return 1 << m_nField; }
...
DEFINE_PARTICLE_OPERATOR(C_OP_OscillateScalar, "Oscillate Scalar",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_OscillateScalar)
DMXELEMENT_UNPACK_FIELD_USERDATA("oscillation field", "7", int, m_nField,
...
void C_OP_OscillateScalar::Operate(CParticleCollection *pParticles,
                                   float flStrength, void *pContext) const {
  CM128AttributeIterator pCreationTime(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                       pParticles);
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
  ...
  fltx4 fl4CosFactor =
      AddSIMD(MulSIMD(fl4CosFactorMultiplier, fl4CurTime), fl4CosFactorAdd);
  ...
  do {
    ...
    if (m_bProportionalOp) {
      fl4LifeTime = MulSIMD(
          SubSIMD(fl4CurTime, *pCreationTime),
          ReciprocalEstSIMD(fl4LifeDuration));  // maybe need accurate div here?
    } else {
      fl4LifeTime = SubSIMD(fl4CurTime, *pCreationTime);
    }

    fltx4 fl4StartTime =
        pParticles->RandomFloat(*pParticleId, nRandomOffset + 11);
    fl4StartTime =
        AddSIMD(fl4StartTimeMin, MulSIMD(fl4StartTimeWidth, fl4StartTime));
    fltx4 fl4EndTime =
        pParticles->RandomFloat(*pParticleId, nRandomOffset + 12);
    ...
    if (IsAnyNegative(fl4GoodMask)) {
      ...
      fl4Frequency =
          AddSIMD(fl4FrequencyMin, MulSIMD(fl4FrequencyWidth, fl4Frequency));
      ...
      if (m_bProportional) {
        fl4LifeTime = MulSIMD(SubSIMD(fl4CurTime, *pCreationTime),
                              ReciprocalEstSIMD(fl4LifeDuration));
        fl4Cos = AddSIMD(
            MulSIMD(fl4CosFactorProp, MulSIMD(fl4LifeTime, fl4Frequency)),
            fl4CosFactorAdd);
      } else {
      ...
      fl4OscVal =
          AddSIMD(*pOscField, MulSIMD(fl4OscMultiplier, SinEst01SIMD(fl4Cos)));
      if (m_nField == 7) {
        *pOscField = MaskedAssign(
            fl4GoodMask, MaxSIMD(MinSIMD(fl4OscVal, Four_Ones), Four_Zeros),
            *pOscField);
      } else {
...
DEFINE_PARTICLE_OPERATOR(C_OP_OscillateVector, "Oscillate Vector",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_OscillateVector)
DMXELEMENT_UNPACK_FIELD_USERDATA("oscillation field", "0", int, m_nField,
...
DMXELEMENT_UNPACK_FIELD("oscillation frequency min", "1 1 1", Vector,
                        m_FrequencyMin)
DMXELEMENT_UNPACK_FIELD("oscillation frequency max", "1 1 1", Vector,
                        m_FrequencyMax)
DMXELEMENT_UNPACK_FIELD("proportional 0/1", "1", bool, m_bProportional)
DMXELEMENT_UNPACK_FIELD("start time min", "0", float, m_flStartTime_min)
DMXELEMENT_UNPACK_FIELD("start time max", "0", float, m_flStartTime_max)
...
void C_OP_OscillateVector::Operate(CParticleCollection *pParticles,
                                   float flStrength, void *pContext) const {
  CM128AttributeIterator pCreationTime(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                       pParticles);
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
    ...
    if (IsAnyNegative(fl4GoodMask)) {
      ...
      if (m_bProportional) {
        fl4LifeTime = MulSIMD(SubSIMD(fl4CurTime, *pCreationTime),
                              ReciprocalEstSIMD(fl4LifeDuration));
        fvCos.x = AddSIMD(
            MulSIMD(fl4CosFactorProp, MulSIMD(fvFrequency.x, fl4LifeTime)),
            fl4CosFactorAdd);
        fvCos.y = AddSIMD(
            MulSIMD(fl4CosFactorProp, MulSIMD(fvFrequency.y, fl4LifeTime)),
            fl4CosFactorAdd);
        fvCos.z = AddSIMD(
            MulSIMD(fl4CosFactorProp, MulSIMD(fvFrequency.z, fl4LifeTime)),
            fl4CosFactorAdd);
      } else {
      ...
      fvOscVal.x = AddSIMD(fvOutput.x,
                           MulSIMD(fvOscMultiplier.x, SinEst01SIMD(fvCos.x)));
      fvOscVal.y = AddSIMD(fvOutput.y,
                           MulSIMD(fvOscMultiplier.y, SinEst01SIMD(fvCos.y)));
      fvOscVal.z = AddSIMD(fvOutput.z,
                           MulSIMD(fvOscMultiplier.z, SinEst01SIMD(fvCos.z)));

      if (m_nField == 6) {
        pOscField->x = MaskedAssign(
            fl4GoodMask, MaxSIMD(MinSIMD(fvOscVal.x, Four_Ones), Four_Zeros),
            fvOutput.x);
        pOscField->y = MaskedAssign(
            fl4GoodMask, MaxSIMD(MinSIMD(fvOscVal.y, Four_Ones), Four_Zeros),
            fvOutput.y);
        pOscField->z = MaskedAssign(
            fl4GoodMask, MaxSIMD(MinSIMD(fvOscVal.z, Four_Ones), Four_Zeros),
            fvOutput.z);
      } else {
...
DMXELEMENT_UNPACK_FIELD_USERDATA("input field", "7", int, m_nFieldInput,
                                 "intchoice particlefield_scalar")
DMXELEMENT_UNPACK_FIELD("input minimum", "0", float, m_flInputMin)
DMXELEMENT_UNPACK_FIELD("input maximum", "1", float, m_flInputMax)
DMXELEMENT_UNPACK_FIELD_USERDATA("output field", "3", int, m_nFieldOutput,
                                 "intchoice particlefield_scalar")
...
void C_OP_RemapScalar::Operate(CParticleCollection *pParticles,
                               float flStrength, void *pContext) const {
  ...
  if (ATTRIBUTES_WHICH_ARE_0_TO_1 & (1 << m_nFieldOutput)) {
    flMin = std::clamp(m_flOutputMin, 0.0f, 1.0f);
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    float flOutput =
        RemapValClamped(*pInput, m_flInputMin, m_flInputMax, flMin, flMax);
...
void C_OP_Noise::Operate(CParticleCollection *pParticles, float flStrength,
                         void *pContext) const {
...
DMXELEMENT_UNPACK_FLTX4("noise coordinate scale", "0.1", m_fl4NoiseScale)
DMXELEMENT_UNPACK_FIELD_USERDATA("output field", "6", int, m_nFieldOutput,
                                 "intchoice particlefield_vector")
...
void C_OP_VectorNoise::Operate(CParticleCollection *pParticles,
                               float flStrength, void *pContext) const {
  ...
  fltx4 ValueBaseX = ReplicateX4(m_vecOutputMin.x +
                                 0.5 * (m_vecOutputMax.x - m_vecOutputMin.x));

  fltx4 ValueScaleY = ReplicateX4(0.5 * (m_vecOutputMax.y - m_vecOutputMin.y));
  fltx4 ValueBaseY = ReplicateX4(m_vecOutputMin.y +
                                 0.5 * (m_vecOutputMax.y - m_vecOutputMin.y));

  fltx4 ValueScaleZ = ReplicateX4(0.5 * (m_vecOutputMax.z - m_vecOutputMin.z));
  fltx4 ValueBaseZ = ReplicateX4(m_vecOutputMin.z +
...
void C_OP_Decay::Operate(CParticleCollection *pParticles, float flStrength,
                         void *pContext) const {
  fltx4 fl4CurTime = pParticles->m_fl4CurTime;
...
void C_OP_Cull::Operate(CParticleCollection *pParticles, float flStrength,
                        void *pContext) const {
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    int nParticleId =
        *pParticles->GetIntAttributePtr(PARTICLE_ATTRIBUTE_PARTICLE_ID, i);
    float flCullRank =
        pParticles->RandomFloat(nParticleId + nRandomOffset + 15, 0.0f, 1.0f);
    float flCullTime =
        pParticles->RandomFloatExp(nParticleId + nRandomOffset + 12,
                                   m_flCullStart, m_flCullEnd, m_flCullExp);

    if (flCullRank > (m_flCullPerc * flStrength)) {
      continue;
    }
    // Find our life percentage
    flLifeTime = std::clamp(
        (pParticles->m_flCurTime - *pCreationTime) / (*pLifeDuration), 0.0f,
        1.0f);
    if (flLifeTime >= m_flCullStart && flLifeTime <= m_flCullEnd &&
        flLifeTime >= flCullTime) {
...
class CGeneralSpin : public CParticleOperatorInstance {
 protected:
  ...
  uint32_t GetWrittenAttributes() const {
    if (m_nSpinRateDegrees != 0.0) return (1 << GetAttributeToSpin());
    return 0;
  }
...
void CGeneralSpin::Operate(CParticleCollection *pParticles, float flStrength,
  ...
  CM128AttributeIterator pCreationTimeStamp(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                            pParticles);
  ...
  int nActive = pParticles->m_nActiveParticles;
  for (int i = 0; i < nActive; i += 4) {
    // HACK: Rather than redo this, I'm simply remapping the stop time into the
    // percentage of lifetime, rather than seconds
    ...
    fltx4 RScale =
        MaxSIMD(Four_Zeros, SubSIMD(Four_Ones, MulSIMD(Age, OOSpinFadeRate)));

    // Cap the rotation at a minimum speed
    fltx4 deltaRot = MulSIMD(Rot_Add, RScale);
    fltx4 Tooslow = CmpLeSIMD(deltaRot, minSpeedRadians);
    deltaRot = OrSIMD(AndSIMD(Tooslow, minSpeedRadians),
                      AndNotSIMD(Tooslow, deltaRot));
...
class CSpinUpdateBase : public CParticleOperatorInstance {
 protected:
  ...
  uint32_t GetWrittenAttributes() const { return (1 << GetAttributeToSpin()); }
...
void CSpinUpdateBase::Operate(CParticleCollection *pParticles, float flStrength,
                              void *pContext) const {
  CM128AttributeIterator pCreationTimeStamp(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                            pParticles);
...
class C_OP_Spin : public CGeneralSpin {
  ...
  int GetAttributeToSpin() const { return PARTICLE_ATTRIBUTE_ROTATION; }
};
...
class C_OP_SpinUpdate : public CSpinUpdateBase {
  ...
  virtual int GetAttributeToSpin() const { return PARTICLE_ATTRIBUTE_ROTATION; }

  virtual int GetSpinSpeedAttribute() const {
...
class C_OP_SpinYaw : public CGeneralSpin {
  ...
  int GetAttributeToSpin() const { return PARTICLE_ATTRIBUTE_YAW; }
};
...
DEFINE_PARTICLE_OPERATOR(C_OP_InterpolateRadius, "Radius Scale",
                         OPERATOR_GENERIC);
...
void C_OP_InterpolateRadius::Operate(CParticleCollection *pParticles,
                                     float flStrength, void *pContext) const {
  if (m_flEndTime <= m_flStartTime) return;
  CM128AttributeIterator pCreationTime(PARTICLE_ATTRIBUTE_CREATION_TIME,
                                       pParticles);
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
                                       pParticles);
  CM128AttributeWriteIterator pRadius(PARTICLE_ATTRIBUTE_RADIUS, pParticles);
  CM128InitialAttributeIterator pInitialRadius(PARTICLE_ATTRIBUTE_RADIUS,
                                               pParticles);
  ...
  if (m_bEaseInAndOut) {
    do {
      ...
      if (IsAnyNegative(fl4GoodMask)) {
        fltx4 fl4FadeWindow =
            MulSIMD(SubSIMD(fl4LifeTime, fl4StartTime), fl4OOTimeWidth);
        fl4FadeWindow = AddSIMD(
            fl4StartScale, MulSIMD(SimpleSpline(fl4FadeWindow), fl4ScaleWidth));
        // !!speed!! - can anyone really tell the diff between spline and lerp
        // here?
    ...
    if (flBias == 0.5)  // no bias case
    {
      do {
        ...
        fltx4 fl4GoodMask = CmpGtSIMD(fl4LifeDuration, Four_Zeros);
        fltx4 fl4LifeTime =
            MulSIMD(SubSIMD(fl4CurTime, *pCreationTime),
                    ReciprocalEstSIMD(
                        fl4LifeDuration));  // maybe need accurate div here?
        fl4GoodMask =
            AndSIMD(fl4GoodMask, CmpGeSIMD(fl4LifeTime, fl4StartTime));
        fl4GoodMask = AndSIMD(fl4GoodMask, CmpLtSIMD(fl4LifeTime, fl4EndTime));
        if (IsAnyNegative(fl4GoodMask)) {
          fltx4 fl4FadeWindow =
              MulSIMD(SubSIMD(fl4LifeTime, fl4StartTime), fl4OOTimeWidth);
          fl4FadeWindow =
              AddSIMD(fl4StartScale, MulSIMD(fl4FadeWindow, fl4ScaleWidth));
          *pRadius = MaskedAssign(
              fl4GoodMask, MulSIMD(*pInitialRadius, fl4FadeWindow), *pRadius);
        }
        ++pCreationTime;
      ...
      do {
        ...
        if (IsAnyNegative(fl4GoodMask)) {
          ...
          fl4FadeWindow = AddSIMD(
              fl4StartScale,
              MulSIMD(BiasSIMD(fl4FadeWindow, fl4BiasParam), fl4ScaleWidth));
          *pRadius = MaskedAssign(
              fl4GoodMask, MulSIMD(*pInitialRadius, fl4FadeWindow), *pRadius);
        }
        ++pCreationTime;
...
void C_OP_ColorInterpolate::Operate(CParticleCollection *pParticles,
                                    float flStrength, void *pContext) const {
  ...
  CM128AttributeIterator pLifeDuration(PARTICLE_ATTRIBUTE_LIFE_DURATION,
                                       pParticles);
  C4VInitialAttributeIterator pInitialColor(PARTICLE_ATTRIBUTE_TINT_RGB,
                                            pParticles);
  if (m_flFadeEndTime == m_flFadeStartTime) return;
  ...
  if (m_bEaseInOut) {
    do {
      ...
      if (IsAnyNegative(goodMask)) {
        fltx4 flLifeTime =
            DivSIMD(SubSIMD(curTime, *pCreationTime), *pLifeDuration);

        fltx4 T = MulSIMD(SubSIMD(flLifeTime, lowRange), ooInRange);
        T = MinSIMD(Four_Ones, MaxSIMD(Four_Zeros, T));
        T = SimpleSpline(T);
        pColor->x = MaskedAssign(
            goodMask,
            AddSIMD(pInitialColor->x,
                    MulSIMD(T, SubSIMD(targetR, pInitialColor->x))),
            pColor->x);
        pColor->y = MaskedAssign(
            goodMask,
            AddSIMD(pInitialColor->y,
                    MulSIMD(T, SubSIMD(targetG, pInitialColor->y))),
            pColor->y);
        pColor->z = MaskedAssign(
            goodMask,
            AddSIMD(pInitialColor->z,
                    MulSIMD(T, SubSIMD(targetB, pInitialColor->z))),
            pColor->z);
      }
      ++pColor;
      ++pCreationTime;
...
class C_OP_PositionLock : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    C_OP_PositionLockContext_t *pCtx =
        reinterpret_cast<C_OP_PositionLockContext_t *>(pContext);
    pCtx->m_vPrevPosition = vec3_origin;
    pParticles->GetControlPointTransformAtTime(m_nControlPointNumber,
                                               pParticles->m_flCurTime,
                                               &pCtx->m_matPrevTransform);
  }
...
DEFINE_PARTICLE_OPERATOR(C_OP_PositionLock, "Movement Lock to Control Point",
                         OPERATOR_GENERIC);
...
void C_OP_PositionLock::Operate(CParticleCollection *pParticles,
                                float flStrength, void *pContext) const {
  Vector vecControlPoint =
      pParticles->GetControlPointAtCurrentTime(m_nControlPointNumber);

  // At initialization, set prevposition to the control point to prevent random
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    float flLifeTime =
        *pLifeDuration != 0.0f
            ? std::clamp(
                  (pParticles->m_flCurTime - *pCreationTime) / (*pLifeDuration),
                  0.0f, 1.0f)
            : 0.0f;
    ...
    int nParticleId =
        *pParticles->GetIntAttributePtr(PARTICLE_ATTRIBUTE_PARTICLE_ID, i);
    float flStartTime = pParticles->RandomFloatExp(
        nParticleId + nRandomOffset + 9, m_flStartTime_min, m_flStartTime_max,
        m_flStartTime_exp);
    float flEndTime = pParticles->RandomFloatExp(
        nParticleId + nRandomOffset + 10, m_flEndTime_min, m_flEndTime_max,
        m_flEndTime_exp);

    // bias attachedness by fadeout
    float flLockScale = SimpleSplineRemapValClamped(flLifeTime, flStartTime,
                                                    flEndTime, 1.0f, 0.0f);

    float *xyz =
        pParticles->GetFloatAttributePtrForWrite(PARTICLE_ATTRIBUTE_XYZ, i);
    float *xyz_prev = pParticles->GetFloatAttributePtrForWrite(
    ...
    if (m_flRange != 0) {
      ...
      flDampenAmount =
          SimpleSplineRemapValClamped(flDistance, 0, m_flRange, 1.0f, 0.0f);
  ...
  if (m_flStartTime_min >= 1.0f)  // always locked on
  {
    do {
      ...
      if (bUseRange) {
        ...
        fl4Dist = BiasSIMD(MinSIMD(Four_Ones, MulSIMD(fl4Dist, fl4OORange)),
                           fl4BiasParm);
    ...
    do {
      ...
      fltx4 fl4StartTime =
          Pow_FixedPoint_Exponent_SIMD(RandSIMD(nContext), nSSEStartExponent);
      fl4StartTime =
          AddSIMD(fl4StartBias, MulSIMD(fl4StartTime, fl4StartRange));

      fltx4 fl4EndTime =
          Pow_FixedPoint_Exponent_SIMD(RandSIMD(nContext), nSSEEndExponent);
      fl4EndTime = AddSIMD(fl4EndBias, MulSIMD(fl4EndTime, fl4EndRange));

      // now, determine "lockedness"
      fltx4 fl4LockScale = DivSIMD(SubSIMD(fl4LifeTime, fl4StartTime),
                                   SubSIMD(fl4EndTime, fl4StartTime));
      fl4LockScale = SubSIMD(
          Four_Ones, MaxSIMD(Four_Zeros, MinSIMD(Four_Ones, fl4LockScale)));
...
class C_OP_ControlpointLight : public CParticleOperatorInstance {
  ...
  float m_LightFiftyDist1, m_LightZeroDist1, m_LightFiftyDist2,
      m_LightZeroDist2, m_LightFiftyDist3, m_LightZeroDist3, m_LightFiftyDist4,
      m_LightZeroDist4;
  Color m_LightColor1, m_LightColor2, m_LightColor3, m_LightColor4;
  bool m_bLightType1, m_bLightType2, m_bLightType3, m_bLightType4,
      m_bLightDynamic1, m_bLightDynamic2, m_bLightDynamic3, m_bLightDynamic4,
      m_bUseNormal, m_bUseHLambert, m_bLightActive1, m_bLightActive2,
      m_bLightActive3, m_bLightActive4, m_bClampLowerRange, m_bClampUpperRange;
...
DEFINE_PARTICLE_OPERATOR(C_OP_ControlpointLight,
                         "Color Light From Control Point", OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("Light 1 Control Point Offset", "0 0 0", Vector,
                        m_vecCPOffset1)
...
DMXELEMENT_UNPACK_FIELD("Light 1 Direction", "0 0 0", Vector,
                        m_LightNode1.m_Direction)
...
DMXELEMENT_UNPACK_FIELD("Light 1 Spot Inner Cone", "30.0", float,
                        m_LightNode1.m_Theta)
DMXELEMENT_UNPACK_FIELD("Light 1 Spot Outer Cone", "45.0", float,
                        m_LightNode1.m_Phi)
DMXELEMENT_UNPACK_FIELD("Light 2 Control Point", "0", int, m_nControlPoint2)
DMXELEMENT_UNPACK_FIELD("Light 2 Control Point Offset", "0 0 0", Vector,
                        m_vecCPOffset2)
...
DMXELEMENT_UNPACK_FIELD("Light 2 Direction", "0 0 0", Vector,
                        m_LightNode2.m_Direction)
...
DMXELEMENT_UNPACK_FIELD("Light 2 Spot Inner Cone", "30.0", float,
                        m_LightNode2.m_Theta)
DMXELEMENT_UNPACK_FIELD("Light 2 Spot Outer Cone", "45.0", float,
                        m_LightNode2.m_Phi)
DMXELEMENT_UNPACK_FIELD("Light 3 Control Point", "0", int, m_nControlPoint3)
DMXELEMENT_UNPACK_FIELD("Light 3 Control Point Offset", "0 0 0", Vector,
                        m_vecCPOffset3)
...
DMXELEMENT_UNPACK_FIELD("Light 3 Direction", "0 0 0", Vector,
                        m_LightNode3.m_Direction)
...
DMXELEMENT_UNPACK_FIELD("Light 3 Spot Inner Cone", "30.0", float,
                        m_LightNode3.m_Theta)
DMXELEMENT_UNPACK_FIELD("Light 3 Spot Outer Cone", "45.0", float,
                        m_LightNode3.m_Phi)
DMXELEMENT_UNPACK_FIELD("Light 4 Control Point", "0", int, m_nControlPoint4)
DMXELEMENT_UNPACK_FIELD("Light 4 Control Point Offset", "0 0 0", Vector,
                        m_vecCPOffset4)
...
DMXELEMENT_UNPACK_FIELD("Light 4 Direction", "0 0 0", Vector,
                        m_LightNode4.m_Direction)
...
DMXELEMENT_UNPACK_FIELD("Light 4 Spot Inner Cone", "30.0", float,
                        m_LightNode4.m_Theta)
DMXELEMENT_UNPACK_FIELD("Light 4 Spot Outer Cone", "45.0", float,
                        m_LightNode4.m_Phi)
DMXELEMENT_UNPACK_FIELD("Initial Color Bias", "0.0", float, m_flScale)
DMXELEMENT_UNPACK_FIELD("Clamp Minimum Light Value to Initial Color", "0", bool,
                        m_bClampLowerRange)
DMXELEMENT_UNPACK_FIELD("Clamp Maximum Light Value to Initial Color", "0", bool,
                        m_bClampUpperRange)
DMXELEMENT_UNPACK_FIELD("Compute Normals From Control Points", "0", bool,
                        m_bUseNormal)
DMXELEMENT_UNPACK_FIELD("Half-Lambert Normals", "1", bool, m_bUseHLambert)
END_PARTICLE_OPERATOR_UNPACK(C_OP_ControlpointLight)

void C_OP_ControlpointLight::Operate(CParticleCollection *pParticles,
                                     float flStrength, void *pContext) const {
  // Set up location of each light - this needs to be done every time as the
  // CP's can move
  ...
  if (m_bLightDynamic1) {
    ...
    Color lc;
    g_pParticleSystemMgr->Query()->GetLightingAtPoint(LightNode1.m_Position,
                                                      lc);
  ...
  if (m_bLightDynamic2) {
    // Get the color and luminosity at this position
    Color lc;
    g_pParticleSystemMgr->Query()->GetLightingAtPoint(LightNode2.m_Position,
                                                      lc);
  ...
  if (m_bLightDynamic3) {
    // Get the color and luminosity at this position
    Color lc;
    g_pParticleSystemMgr->Query()->GetLightingAtPoint(LightNode3.m_Position,
                                                      lc);
  ...
  if (m_bLightDynamic4) {
    // Get the color and luminosity at this position
    Color lc;
    g_pParticleSystemMgr->Query()->GetLightingAtPoint(LightNode4.m_Position,
                                                      lc);
  ...
  if (m_bUseNormal) {
    ...
    do {
      ...
      LightNode1.ComputeLightAtPoints(*pXYZ, vNormal, vLighting,
                                      m_bUseHLambert);
      vNormal = *pXYZ;
      vNormal -= vCPPosition2;
      vNormal.VectorNormalizeFast();
      LightNode2.ComputeLightAtPoints(*pXYZ, vNormal, vLighting,
                                      m_bUseHLambert);
      vNormal = *pXYZ;
      vNormal -= vCPPosition3;
      vNormal.VectorNormalizeFast();
      LightNode3.ComputeLightAtPoints(*pXYZ, vNormal, vLighting,
                                      m_bUseHLambert);
      vNormal = *pXYZ;
      vNormal -= vCPPosition4;
      vNormal.VectorNormalizeFast();
      LightNode4.ComputeLightAtPoints(*pXYZ, vNormal, vLighting,
                                      m_bUseHLambert);
...
void C_OP_ControlpointLight::Render(CParticleCollection *pParticles) const {
  Vector vecOrigin1 =
      pParticles->GetControlPointAtCurrentTime(m_nControlPoint1);
  vecOrigin1 += m_vecCPOffset1;
  Vector vecOrigin2 =
      pParticles->GetControlPointAtCurrentTime(m_nControlPoint2);
  vecOrigin2 += m_vecCPOffset2;
  Vector vecOrigin3 =
      pParticles->GetControlPointAtCurrentTime(m_nControlPoint3);
  vecOrigin3 += m_vecCPOffset3;
  Vector vecOrigin4 =
      pParticles->GetControlPointAtCurrentTime(m_nControlPoint4);
  ...
  if (m_bLightActive1) {
    RenderWireframeSphere(vecOrigin1, m_LightFiftyDist1, 16, 8, m_LightColor1,
                          false);
    RenderWireframeSphere(vecOrigin1, m_LightZeroDist1, 16, 8, LightColor1Outer,
                          false);
  }
  if (m_bLightActive2) {
    RenderWireframeSphere(vecOrigin2, m_LightFiftyDist2, 16, 8, m_LightColor2,
                          false);
    RenderWireframeSphere(vecOrigin2, m_LightZeroDist2, 16, 8, LightColor2Outer,
                          false);
  }
  if (m_bLightActive3) {
    RenderWireframeSphere(vecOrigin3, m_LightFiftyDist3, 16, 8, m_LightColor3,
                          false);
    RenderWireframeSphere(vecOrigin3, m_LightZeroDist3, 16, 8, LightColor3Outer,
                          false);
  }
  if (m_bLightActive4) {
    RenderWireframeSphere(vecOrigin4, m_LightFiftyDist4, 16, 8, m_LightColor4,
                          false);
    RenderWireframeSphere(vecOrigin4, m_LightZeroDist4, 16, 8, LightColor4Outer,
                          false);
  }
}

// set child controlpoints - copy the positions of our particles to the control
// points of a child
class C_OP_SetChildControlPoints : public CParticleOperatorInstance {
  ...
  uint32_t GetReadAttributes() const { return PARTICLE_ATTRIBUTE_XYZ; }

  virtual void Operate(CParticleCollection *pParticles, float flStrength,
                       void *pContext) const;
};

DEFINE_PARTICLE_OPERATOR(C_OP_SetChildControlPoints,
...
DMXELEMENT_UNPACK_FIELD("First control point to set", "0", int,
                        m_nFirstControlPoint)
DMXELEMENT_UNPACK_FIELD("# of control points to set", "1", int,
                        m_nNumControlPoints)
...
void C_OP_SetChildControlPoints::Operate(CParticleCollection *pParticles,
                                         float flStrength,
                                         void *pContext) const {
  int nFirst = std::max(
      0, std::min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nFirstControlPoint));
  int nToSet = std::min(pParticles->m_nActiveParticles - m_nFirstSourcePoint,
                        m_nNumControlPoints);
  nToSet = std::min(nToSet, MAX_PARTICLE_CONTROL_POINTS - nFirst);
  if (nToSet) {
...
class C_OP_SetControlPointPositions : public CParticleOperatorInstance {
  ...
  bool ShouldRunBeforeEmitters() const { return true; }

  virtual void Operate(CParticleCollection *pParticles, float flStrength,
                       void *pContext) const;
};

DEFINE_PARTICLE_OPERATOR(C_OP_SetControlPointPositions,
                         "Set Control Point Positions", OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("First Control Point Location", "128 0 0", Vector,
                        m_vecCP1Pos)
...
DMXELEMENT_UNPACK_FIELD("Second Control Point Location", "0 128 0", Vector,
                        m_vecCP2Pos)
...
DMXELEMENT_UNPACK_FIELD("Third Control Point Location", "-128 0 0", Vector,
                        m_vecCP3Pos)
...
DMXELEMENT_UNPACK_FIELD("Fourth Control Point Location", "0 -128 0", Vector,
                        m_vecCP4Pos)
DMXELEMENT_UNPACK_FIELD("Set positions in world space", "0", bool,
                        m_bUseWorldLocation)
DMXELEMENT_UNPACK_FIELD("Control Point to offset positions from", "0", int,
                        m_nHeadLocation)
END_PARTICLE_OPERATOR_UNPACK(C_OP_SetControlPointPositions)

void C_OP_SetControlPointPositions::Operate(CParticleCollection *pParticles,
                                            float flStrength,
                                            void *pContext) const {
  if (!m_bUseWorldLocation) {
    Vector vecControlPoint =
        pParticles->GetControlPointAtCurrentTime(m_nHeadLocation);
...
DEFINE_PARTICLE_OPERATOR(C_OP_DampenToCP,
...
void C_OP_DampenToCP::Operate(CParticleCollection *pParticles, float flStrength,
                              void *pContext) const {
  if (m_flRange <= 0.0f) return;
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    VectorLerp(vecParticlePositionOrg, vecParticlePosition, flStrength,
               vecParticlePosition);
    SetVectorAttribute(xyz, vecParticlePosition);
  }
};
...
class C_OP_DistanceBetweenCPs : public CParticleOperatorInstance {
  ...
  uint32_t GetReadAttributes() const { return 0; }
  uint32_t GetReadInitialAttributes() const { return 1 << m_nFieldOutput; }
  ...
  void InitParams(CParticleSystemDefinition *pDef, CDmxElement *pElement) {
    m_nCollisionGroupNumber =
        g_pParticleSystemMgr->Query()->GetCollisionGroupFromName(
            m_CollisionGroupName);
    m_nStartCP =
        std::max(0, std::min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nStartCP));
    m_nEndCP = std::max(0, std::min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nEndCP));
  }
...
DEFINE_PARTICLE_OPERATOR(C_OP_DistanceBetweenCPs,
...
DMXELEMENT_UNPACK_FIELD_STRING("LOS collision group", "NONE",
                               m_CollisionGroupName)
...
DMXELEMENT_UNPACK_FIELD("output is scalar of initial random range", "0", bool,
                        m_bScaleInitialRange)
END_PARTICLE_OPERATOR_UNPACK(C_OP_DistanceBetweenCPs)

void C_OP_DistanceBetweenCPs::Operate(CParticleCollection *pParticles,
                                      float flStrength, void *pContext) const {
  // clamp the result to 0 and 1 if it's alpha
  float flMin = m_flOutputMin;
  float flMax = m_flOutputMax;
  ...
  if (m_bLOS) {
    ...
    CBaseTrace tr;
    g_pParticleSystemMgr->Query()->TraceLine(vecControlPoint1, vecEndPoint,
                                             MASK_OPAQUE_AND_NPCS, NULL,
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    if (m_bScaleInitialRange) {
      const float *pInitialOutput =
          pParticles->GetInitialFloatAttributePtr(m_nFieldOutput, i);
...
DEFINE_PARTICLE_OPERATOR(C_OP_DistanceToCP,
...
DMXELEMENT_UNPACK_FIELD("output is scalar of initial random range", "0", bool,
                        m_bScaleInitialRange)
DMXELEMENT_UNPACK_FIELD("only active within specified distance", "0", bool,
                        m_bActiveRange)
END_PARTICLE_OPERATOR_UNPACK(C_OP_DistanceToCP)

void C_OP_DistanceToCP::Operate(CParticleCollection *pParticles,
                                float flStrength, void *pContext) const {
  // clamp the result to 0 and 1 if it's alpha
  float flMin = m_flOutputMin;
  float flMax = m_flOutputMax;
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    float flDistance = vecDelta.Length();
    if (m_bActiveRange &&
        (flDistance < m_flInputMin || flDistance > m_flInputMax)) {
      continue;
    }
    if (m_bLOS) {
...
class C_OP_SetControlPointToPlayer : public CParticleOperatorInstance {
  ...
  void InitParams(CParticleSystemDefinition *pDef, CDmxElement *pElement) {
    m_nCP1 = std::max(0, std::min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nCP1));
  }
...
DEFINE_PARTICLE_OPERATOR(C_OP_SetControlPointToPlayer,
                         "Set Control Point To Player", OPERATOR_GENERIC);
...
void C_OP_SetControlPointToPlayer::Operate(CParticleCollection *pParticles,
                                           float flStrength,
                                           void *pContext) const {
  ...
  Vector vecClientForwardVector =
      g_pParticleSystemMgr->Query()->GetLocalPlayerFacing();
...
class C_OP_PerParticleEmitter : public CParticleOperatorInstance {
  ...
  virtual bool MayCreateMoreParticles(CParticleCollection *pParticles,
                                      void *pContext) const {
    C_OP_PerParticleEmitterContext_t *pCtx =
        reinterpret_cast<C_OP_PerParticleEmitterContext_t *>(pContext);
    if (pCtx->m_bStoppedEmission) return false;

    if (m_flEmitRate <= 0.0f) return false;

    if (m_flEmissionDuration != 0.0f &&
        (pParticles->m_flCurTime - pParticles->m_flDt) >
            (m_flStartTime + m_flEmissionDuration))
      return false;
...
DEFINE_PARTICLE_OPERATOR(C_OP_PerParticleEmitter, "Per Particle Emitter",
                         OPERATOR_GENERIC);
...
void C_OP_PerParticleEmitter::Operate(CParticleCollection *pParticles,
                                      float flStrength, void *pContext) const {
  for (CParticleCollection *pChild = pParticles->m_Children.m_pHead; pChild;
       pChild = pChild->m_pNext) {
    if (pChild->GetGroupID() == m_nChildGroupID) {
      for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
        ...
        vecParticlePosition_prev =
            Vector(pXYZ_Prev[0], pXYZ_Prev[4], pXYZ_Prev[8]);
        vParticleDelta = vecParticlePosition - vecParticlePosition_prev;
        ...
        if (flEmissionRate == 0.0f) continue;
        ...
        // determine our previous and current draw times and clamp them to start
        // time and emission duration
        ...
        // Calculate emission rate by delta time from last frame to determine
        // number of particles to emit this frame as a fractional float
        float flActualParticlesToEmit = flEmissionRate * flDeltaTime;
        int nParticlesEmitted = pCtx->m_nTotalEmittedSoFar;
        // Add emitted particle to float counter to allow for fractional
        // emission
        pCtx->m_flTotalActualParticlesSoFar += flActualParticlesToEmit;

        // Floor float accumulated value and subtract whole int emitted so far
        // from the result to determine total whole particles to emit this frame
        int nParticlesToEmit = floor(pCtx->m_flTotalActualParticlesSoFar) -
                               pCtx->m_nTotalEmittedSoFar;

        // Add emitted particles to running int total.
        pCtx->m_nTotalEmittedSoFar += nParticlesToEmit;

        if (nParticlesToEmit == 0) continue;
        ...
        int nAllowedParticlesToEmit =
            pChild->m_nMaxAllowedParticles - pParticles->m_nActiveParticles;
        if (nAllowedParticlesToEmit < nParticlesToEmit) {
          nActualParticlesToEmit = nAllowedParticlesToEmit;
        }
        if (nActualParticlesToEmit == 0) continue;

        int nStartParticle = pChild->m_nActiveParticles;
        pChild->SetNActiveParticles(nActualParticlesToEmit +
                                    pChild->m_nActiveParticles);
        ...
        if (nParticlesEmitted != pChild->m_nActiveParticles) {
          uint32_t nInittedMask =
              (PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_PREV_XYZ_MASK |
               PARTICLE_ATTRIBUTE_CREATION_TIME_MASK);
          // init newly emitted particles
          pChild->InitializeNewParticles(
              nParticlesEmitted, pChild->m_nActiveParticles - nParticlesEmitted,
              nInittedMask);
          // CHECKSYSTEM( this );
        }

        // Set the particle creation time to the exact sub-frame particle
        // emission time
        // !! speed!! do sse init here
        for (int j = nStartParticle;
             j < nStartParticle + nActualParticlesToEmit; j++) {
          float *pTimeStamp = pChild->GetFloatAttributePtrForWrite(
              PARTICLE_ATTRIBUTE_CREATION_TIME, j);
          flTimeStep = std::min(flTimeStep, flCurrDrawTime);
          *pTimeStamp = flTimeStep;
          flTimeStep += flTimeStampStep;
          float *pXYZ_Child =
              pChild->GetFloatAttributePtrForWrite(PARTICLE_ATTRIBUTE_XYZ, j);
          float *pXYZ_Prev_Child = pChild->GetFloatAttributePtrForWrite(
              PARTICLE_ATTRIBUTE_PREV_XYZ, j);
          Vector vecChildXYZ;
...
DEFINE_PARTICLE_OPERATOR(C_OP_LockToBone, "Movement Lock to Bone",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_LockToBone)
DMXELEMENT_UNPACK_FIELD("control_point_number", "0", int, m_nControlPointNumber)
DMXELEMENT_UNPACK_FIELD("lifetime fade start", "0", float,
...
void C_OP_LockToBone::Operate(CParticleCollection *pParticles, float flStrength,
                              void *pContext) const {
  ...
  if (pParticles->m_ControlPointHitBoxes[m_nControlPointNumber]
    ...
    for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
      float *pXYZ =
          pParticles->GetFloatAttributePtrForWrite(PARTICLE_ATTRIBUTE_XYZ, i);
      float *pPrevXYZ = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_PREV_XYZ, i);
      const float *pUVW = pParticles->GetFloatAttributePtr(
          PARTICLE_ATTRIBUTE_HITBOX_RELATIVE_XYZ, i);
      const int *pIdx =
          pParticles->GetIntAttributePtr(PARTICLE_ATTRIBUTE_HITBOX_INDEX, i);
      float const *pCreationTime =
          pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_CREATION_TIME, i);
      ...
      if (flAge < flAgeThreshold) {
        int nBoxIndex = *pIdx;
        if ((nBoxIndex <
             pParticles->m_ControlPointHitBoxes[m_nControlPointNumber]
                 .m_nNumHitBoxes) &&
            (nBoxIndex <
             pParticles->m_ControlPointHitBoxes[m_nControlPointNumber]
                 .m_nNumPrevHitBoxes)) {
          Vector vecParticlePosition;
          ModelHitBoxInfo_t const &hb =
              pParticles->m_ControlPointHitBoxes[m_nControlPointNumber]
                  .m_pHitBoxes[nBoxIndex];
          vecParticlePosition.x =
              Lerp(pUVW[0], hb.m_vecBoxMins.x, hb.m_vecBoxMaxes.x);
          vecParticlePosition.y =
              Lerp(pUVW[4], hb.m_vecBoxMins.y, hb.m_vecBoxMaxes.y);
          vecParticlePosition.z =
              Lerp(pUVW[8], hb.m_vecBoxMins.z, hb.m_vecBoxMaxes.z);
          Vector vecWorldPosition;
          VectorTransform(vecParticlePosition, hb.m_Transform,
                          vecWorldPosition);

          Vector vecPrevParticlePosition;
          ModelHitBoxInfo_t phb =
              pParticles->m_ControlPointHitBoxes[m_nControlPointNumber]
                  .m_pPrevBoxes[nBoxIndex];
          vecPrevParticlePosition.x =
              Lerp(pUVW[0], phb.m_vecBoxMins.x, phb.m_vecBoxMaxes.x);
          vecPrevParticlePosition.y =
              Lerp(pUVW[4], phb.m_vecBoxMins.y, phb.m_vecBoxMaxes.y);
          vecPrevParticlePosition.z =
              Lerp(pUVW[8], phb.m_vecBoxMins.z, phb.m_vecBoxMaxes.z);
          Vector vecPrevWorldPosition;
          VectorTransform(vecPrevParticlePosition, phb.m_Transform,
                          vecPrevWorldPosition);

          Vector Delta = vecWorldPosition - vecPrevWorldPosition;

          if (flAge > m_flLifeTimeFadeStart)
            Delta *= flStrength *
                     (1.0 - ((flAge - m_flLifeTimeFadeStart) * flIScale));
...
DEFINE_PARTICLE_OPERATOR(C_OP_PlaneCull, "Cull when crossing plane",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_PlaneCull)
DMXELEMENT_UNPACK_FIELD("Control Point for point on plane", "0", int,
...
void C_OP_PlaneCull::Operate(CParticleCollection *pParticles, float flStrength,
                             void *pContext) const {
  C4VAttributeIterator pXYZ(PARTICLE_ATTRIBUTE_XYZ, pParticles);
  int nLimit = pParticles->m_nPaddedActiveParticles << 2;
  ...
  v4N.VectorNormalize();
  FourVectors v4Pnt;
  v4Pnt.DuplicateVector(
      pParticles->GetControlPointAtCurrentTime(m_nPlaneControlPoint));
  FourVectors ofs = v4N;
...
DEFINE_PARTICLE_OPERATOR(C_OP_SetControlPointToCenter,
...
void C_OP_SetControlPointToCenter::Operate(CParticleCollection *pParticles,
                                           float flStrength,
                                           void *pContext) const {
  Vector vecMinBounds;
  Vector vecMaxBounds;
...
class C_OP_VelocityMatchingForce : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    VelocityMatchingForceContext_t *pCtx =
        reinterpret_cast<VelocityMatchingForceContext_t *>(pContext);
...
DEFINE_PARTICLE_OPERATOR(C_OP_VelocityMatchingForce,
                         "Movement Match Particle Velocities",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_VelocityMatchingForce)
DMXELEMENT_UNPACK_FIELD("Direction Matching Strength", "0.25", float,
                        m_flDirScale)
DMXELEMENT_UNPACK_FIELD("Speed Matching Strength", "0.25", float, m_flSpdScale)
DMXELEMENT_UNPACK_FIELD("Control Point to Broadcast Speed and Direction To",
                        "-1", int, m_nCPBroadcast)
END_PARTICLE_OPERATOR_UNPACK(C_OP_VelocityMatchingForce)

void C_OP_VelocityMatchingForce::Operate(CParticleCollection *pParticles,
                                         float flStrength,
                                         void *pContext) const {
  VelocityMatchingForceContext_t *pCtx =
      reinterpret_cast<VelocityMatchingForceContext_t *>(pContext);
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    if (pCtx->m_vecAvgVelocity != vec3_origin) {
      ...
      VectorLerp(vecVelocityCur, pCtx->m_vecAvgVelocity, m_flDirScale,
                 vecScaledXYZ);
  ...
  if (m_nCPBroadcast != -1) {
    pParticles->SetControlPoint(
        m_nCPBroadcast,
        Vector(pCtx->m_flAvgSpeed, pCtx->m_flAvgSpeed, pCtx->m_flAvgSpeed));
    pParticles->SetControlPointForwardVector(m_nCPBroadcast,
                                             pCtx->m_vecAvgVelocity);
  }
};
...
DEFINE_PARTICLE_OPERATOR(C_OP_OrientTo2dDirection,
                         "Rotation Orient to 2D Direction", OPERATOR_GENERIC);
...
DEFINE_PARTICLE_OPERATOR(C_OP_MaxVelocity, "Movement Max Velocity",
                         OPERATOR_GENERIC);
...
void C_OP_MaxVelocity::Operate(CParticleCollection *pParticles,
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    vecVelocityCur *= std::min(flSpeed, flMaxVelocityNormalized);
...
class C_OP_MaintainSequentialPath : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    SequentialPositionContext_t *pCtx =
        reinterpret_cast<SequentialPositionContext_t *>(pContext);
...
DEFINE_PARTICLE_OPERATOR(C_OP_MaintainSequentialPath,
...
DMXELEMENT_UNPACK_FIELD("start control point number", "0", int,
                        m_PathParams.m_nStartControlPointNumber)
DMXELEMENT_UNPACK_FIELD("end control point number", "0", int,
                        m_PathParams.m_nEndControlPointNumber)
DMXELEMENT_UNPACK_FIELD(
    "bulge control 0=random 1=orientation of start pnt 2=orientation of end "
    "point",
    "0", int, m_PathParams.m_nBulgeControl)
DMXELEMENT_UNPACK_FIELD("mid point position", "0.5", float,
                        m_PathParams.m_flMidPoint)
DMXELEMENT_UNPACK_FIELD("particles to map from start to end", "100", float,
                        m_flNumToAssign)
DMXELEMENT_UNPACK_FIELD("restart behavior (0 = bounce, 1 = loop )", "1", bool,
                        m_bLoop)
...
void C_OP_MaintainSequentialPath::Operate(CParticleCollection *pParticles,
                                          float flStrength,
                                          void *pContext) const {
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    ...
    pParticles->CalculatePathValues(m_PathParams, pParticles->m_flCurTime,
                                    &StartPnt, &MidP, &EndPnt);
    if (pCtx->m_nParticleCount >= m_flNumToAssign ||
        pCtx->m_nParticleCount < 0) {
      if (m_bLoop) {
        pCtx->m_nParticleCount = 0;
      } else {
        pCtx->m_nCountAmount *= -1;
        pCtx->m_nParticleCount =
            std::min(pCtx->m_nParticleCount * 1.0f, (m_flNumToAssign - 1));
        pCtx->m_nParticleCount = std::max(pCtx->m_nParticleCount, 1);
      }
    }
    ...
    float flXYZOffset = std::min(vecXYZ.Length(), m_fMaxDistance);
    float flPXYZOffset = std::min(vecPXYZ.Length(), m_fMaxDistance);
...
DEFINE_PARTICLE_OPERATOR(C_OP_RemapDotProductToScalar,
                         "remap dot product to scalar", OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_RemapDotProductToScalar)
DMXELEMENT_UNPACK_FIELD("use particle velocity for first input", "0", bool,
...
DMXELEMENT_UNPACK_FIELD("output is scalar of initial random range", "0", bool,
                        m_bScaleInitialRange)
DMXELEMENT_UNPACK_FIELD("only active within specified input range", "0", bool,
                        m_bActiveRange)
END_PARTICLE_OPERATOR_UNPACK(C_OP_RemapDotProductToScalar)

void C_OP_RemapDotProductToScalar::Operate(CParticleCollection *pParticles,
                                           float flStrength,
                                           void *pContext) const {
  // clamp the result to 0 and 1 if it's alpha
  float flMin = m_flOutputMin;
  ...
  pParticles->GetControlPointTransformAtTime(m_nInputCP1,
                                             pParticles->m_flCurTime, &pXForm1);
  pParticles->GetControlPointTransformAtTime(m_nInputCP2,
                                             pParticles->m_flCurTime, &pXForm2);
  ...
  for (int i = 0; i < pParticles->m_nActiveParticles; ++i) {
    if (m_bUseParticleVelocity) {
      const float *xyz =
          pParticles->GetFloatAttributePtrForWrite(PARTICLE_ATTRIBUTE_XYZ, i);
      const float *pxyz = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_PREV_XYZ, i);
      Vector vecXYZ;
      Vector vecPXYZ;
    ...
    float *pOutput =
        pParticles->GetFloatAttributePtrForWrite(m_nFieldOutput, i);
    float flOutput =
        RemapValClamped(flInput, m_flInputMin, m_flInputMax, flMin, flMax);
    if (m_bScaleInitialRange) {
      flOutput *= *pOutput;
    }
