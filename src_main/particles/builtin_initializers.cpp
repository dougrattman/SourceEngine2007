...
void CParticleOperatorInstance::InitScalarAttributeRandomRangeBlock(
    int attr_num, float fMin, float fMax, CParticleCollection *pParticles,
    int start_block, int n_blocks) const {
  size_t attr_stride;
  fltx4 *pAttr =
      pParticles->GetM128AttributePtrForWrite(attr_num, &attr_stride);
...
void CParticleOperatorInstance::AddScalarAttributeRandomRangeBlock(
    ...
    CParticleCollection *pParticles, int nStartBlock, int nBlockCount,
    bool bRandomlyInvert) const {
  size_t nAttrStride;
  fltx4 *pAttr =
      pParticles->GetM128AttributePtrForWrite(nAttributeId, &nAttrStride);
  ...
  if (!bRandomlyInvert) {
    if (fExp != 1.0f) {
      ...
      while (nBlockCount--) {
        *pAttr = AddSIMD(*pAttr,
                         AddSIMD(val0, MulSIMD(RandSIMD(nRandContext), val_d)));
        pAttr += nAttrStride;
      }
    }
  } else {
    ...
    if (fExp != 1.0f) {
      while (nBlockCount--) {
        fltx4 fl4RandVal = AddSIMD(
            val0,
            MulSIMD(Pow_FixedPoint_Exponent_SIMD(RandSIMD(nRandContext), fExp),
                    val_d));
        fltx4 fl4Sign =
            MaskedAssign(CmpGeSIMD(RandSIMD(nRandContext), Four_PointFives),
                         Four_Ones, fl4NegOne);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateOnModel, "Position on Model Random",
...
void C_INIT_CreateOnModel::InitNewParticlesScalar(
  ...
  while (nParticleCount) {
    ...
    for (usize i = 0; i < nToDo; i++) {
      ...
      float *pHitboxRelXYZ = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_HITBOX_RELATIVE_XYZ, start_p);
      int *pHitboxIndex = pParticles->GetIntAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_HITBOX_INDEX, start_p);
      start_p++;
      ...
      if (pHitboxRelXYZ &&
          (nAttributeWriteMask & PARTICLE_ATTRIBUTE_HITBOX_RELATIVE_XYZ_MASK)) {
        pHitboxRelXYZ[0] = vecUVW[i].x;
        pHitboxRelXYZ[4] = vecUVW[i].y;
        pHitboxRelXYZ[8] = vecUVW[i].z;
      }
      if (pHitboxIndex &&
          (nAttributeWriteMask & PARTICLE_ATTRIBUTE_HITBOX_INDEX_MASK)) {
...
class C_INIT_CreateWithinSphere : public CParticleOperatorInstance {
  ...
  virtual uint64_t GetReadControlPointMask() const {
    if (!m_bUseHighestEndCP) return 1ULL << m_nControlPointNumber;
  ...
  void InitParams(CParticleSystemDefinition *pDef, CDmxElement *pElement) {
    ...
    m_bDistanceBias = (m_vecDistanceBias.x != 1.0f) ||
                      (m_vecDistanceBias.y != 1.0f) ||
                      (m_vecDistanceBias.z != 1.0f);
    m_bDistanceBiasAbs = (m_vecDistanceBiasAbs.x != 0.0f) ||
                         (m_vecDistanceBiasAbs.y != 0.0f) ||
                         (m_vecDistanceBiasAbs.z != 0.0f);
  }

  void Render(CParticleCollection *pParticles) const;
};

DEFINE_PARTICLE_OPERATOR(C_INIT_CreateWithinSphere,
                         "Position Within Sphere Random", OPERATOR_PI_POSITION);
...
DMXELEMENT_UNPACK_FIELD("distance_bias_absolute_value", "0 0 0", Vector,
...
DMXELEMENT_UNPACK_FIELD("speed_in_local_coordinate_system_min", "0 0 0", Vector,
                        m_LocalCoordinateSystemSpeedMin)
DMXELEMENT_UNPACK_FIELD("speed_in_local_coordinate_system_max", "0 0 0", Vector,
                        m_LocalCoordinateSystemSpeedMax)
DMXELEMENT_UNPACK_FIELD("create in model", "0", int, m_nCreateInModel)
DMXELEMENT_UNPACK_FIELD("randomly distribute to highest supplied Control Point",
                        "0", bool, m_bUseHighestEndCP)
DMXELEMENT_UNPACK_FIELD("randomly distribution growth time", "0", float,
                        m_flEndCPGrowthTime)
...
void C_INIT_CreateWithinSphere::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    float *xyz = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_XYZ, start_p);
    const float *ct = pParticles->GetFloatAttributePtr(
        PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
    float *pxyz = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_PREV_XYZ, start_p);
    int nCurrentControlPoint = m_nControlPointNumber;
    if (m_bUseHighestEndCP) {
      // hack for growth time instead of using strength as currenly initializers
      // don't support it.
      float flStrength = 1.0;
      if (m_flEndCPGrowthTime != 0.0f) {
        flStrength = std::min(pParticles->m_flCurTime, m_flEndCPGrowthTime) /
                     m_flEndCPGrowthTime;
      }
      int nHighestControlPoint =
          floor(pParticles->GetHighestControlPoint() * flStrength);
      nCurrentControlPoint =
          pParticles->RandomInt(m_nControlPointNumber, nHighestControlPoint);
    }
    ...
    for (int nTryCtr = 0; nTryCtr < 10; nTryCtr++) {
      ...
      if (!m_bDistanceBias || !m_bLocalCoords) {
        ...
        pParticles->GetControlPointAtTime(nCurrentControlPoint, *ct,
                                          &vecControlPoint);
        randpos += vecControlPoint;
      } else {
        matrix3x4_t mat;
        pParticles->GetControlPointTransformAtTime(nCurrentControlPoint, *ct,
                                                   &mat);
      ...
      if ((m_nCreateInModel == 0) ||
          (g_pParticleSystemMgr->Query()->MovePointInsideControllingObject(
              pParticles,
              pParticles->m_ControlPoints[nCurrentControlPoint].m_pObject,
              &randpos)))
        break;
    }
    ...
    if (pxyz && (nAttributeWriteMask & PARTICLE_ATTRIBUTE_PREV_XYZ_MASK)) {
      ...
      if (m_fSpeedMax > 0.0) {
        float rand_speed = pParticles->RandomFloatExp(m_fSpeedMin, m_fSpeedMax,
                                                      m_fSpeedRandExp);
      ...
      poffset -=
          pParticles->RandomFloat(m_LocalCoordinateSystemSpeedMin.x,
                                  m_LocalCoordinateSystemSpeedMax.x) *
          pParticles->m_ControlPoints[nCurrentControlPoint].m_ForwardVector;
      poffset -=
          pParticles->RandomFloat(m_LocalCoordinateSystemSpeedMin.y,
                                  m_LocalCoordinateSystemSpeedMax.y) *
          pParticles->m_ControlPoints[nCurrentControlPoint].m_RightVector;
      poffset -= pParticles->RandomFloat(m_LocalCoordinateSystemSpeedMin.z,
                                         m_LocalCoordinateSystemSpeedMax.z) *
...
void C_INIT_CreateWithinSphere::Render(CParticleCollection *pParticles) const {
  ...
  pParticles->GetControlPointAtTime(m_nControlPointNumber,
                                    pParticles->m_flCurTime, &vecOrigin);
  RenderWireframeSphere(vecOrigin, m_fRadiusMin, 16, 8, Color(192, 192, 0, 255),
                        false);
  RenderWireframeSphere(vecOrigin, m_fRadiusMax, 16, 8, Color(128, 128, 0, 255),
                        false);
}
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateWithinBox, "Position Within Box Random",
                         OPERATOR_PI_POSITION);
...
void C_INIT_CreateWithinBox::InitNewParticlesScalar(
    CParticleCollection *pParticles, int start_p, int nParticleCount,
    int nAttributeWriteMask, void *pContext) const {
  int nControlPointNumber = std::max(
      0, std::min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nControlPointNumber));
  for (; nParticleCount--; start_p++) {
    float *xyz = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_XYZ, start_p);
    ...
    Vector vecControlPoint;
    pParticles->GetControlPointAtTime(nControlPointNumber, *ct,
                                      &vecControlPoint);
    randpos += vecControlPoint;
...
void C_INIT_CreateWithinBox::Render(CParticleCollection *pParticles) const {
  ...
  pParticles->GetControlPointAtTime(m_nControlPointNumber,
                                    pParticles->m_flCurTime, &vecOrigin);
  RenderWireframeBox(vecOrigin, vec3_angle, m_vecMin, m_vecMax,
                     Color(192, 192, 0, 255), false);
}
...
DEFINE_PARTICLE_OPERATOR(C_INIT_PositionOffset, "Position Modify Offset Random",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("offset proportional to radius 0/1", "0", bool,
                        m_bProportional)
...
void C_INIT_PositionOffset::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    const float *ct = pParticles->GetFloatAttributePtr(
        PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
    const float *radius =
        pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_RADIUS, start_p);

    Vector randpos;

    if (m_bProportional) {
      pParticles->RandomVector((m_OffsetMin * *radius), (m_OffsetMax * *radius),
                               &randpos);
    } else {
    ...
    if (m_bLocalCoords) {
      matrix3x4_t mat;
      pParticles->GetControlPointTransformAtTime(m_nControlPointNumber, *ct,
                                                 &mat);
      Vector vecTransformLocal = vec3_origin;
...
void C_INIT_PositionOffset::Render(CParticleCollection *pParticles) const {
  ...
  RenderWireframeBox(vecOrigin, vec3_angle, vecMinExtent, vecMaxExtent,
                     Color(192, 192, 0, 255), false);
}
...
class C_INIT_VelocityRandom : public CParticleOperatorInstance {
  ...
  virtual uint64_t GetReadControlPointMask() const {
    if (m_bHasLocalSpeed) return 1ULL << m_nControlPointNumber;
    return 0;
  }
  ...
  void InitParams(CParticleSystemDefinition *pDef, CDmxElement *pElement) {
    m_nControlPointNumber = std::max(
        0, std::min(MAX_PARTICLE_CONTROL_POINTS - 1, m_nControlPointNumber));
    m_bHasLocalSpeed = (m_LocalCoordinateSystemSpeedMin != vec3_origin) ||
                       (m_LocalCoordinateSystemSpeedMax != vec3_origin);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_VelocityRandom, "Velocity Random",
                         OPERATOR_GENERIC);
...
void C_INIT_VelocityRandom::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if (m_bHasLocalSpeed) {
      ...
      pParticles->RandomVector(m_LocalCoordinateSystemSpeedMin,
                               m_LocalCoordinateSystemSpeedMax,
                               &vecRandomSpeed);
...
class C_INIT_InitialVelocityNoise : public CParticleOperatorInstance {
  ...
  void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block,
                             int n_blocks, int nAttributeWriteMask,
                             void *pContext) const;
...
DEFINE_PARTICLE_OPERATOR(C_INIT_InitialVelocityNoise, "Velocity Noise",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_InitialVelocityNoise)
DMXELEMENT_UNPACK_FIELD("Control Point Number", "0", int, m_nControlPointNumber)
DMXELEMENT_UNPACK_FIELD("Time Noise Coordinate Scale", "1", float,
                        m_flNoiseScale)
DMXELEMENT_UNPACK_FIELD("Spatial Noise Coordinate Scale", "0.01", float,
                        m_flNoiseScaleLoc)
DMXELEMENT_UNPACK_FIELD("Time Coordinate Offset", "0", float, m_flOffset)
DMXELEMENT_UNPACK_FIELD("Spatial Coordinate Offset", "0 0 0", Vector,
                        m_vecOffsetLoc)
...
DMXELEMENT_UNPACK_FIELD("Apply Velocity in Local Space (0/1)", "0", bool,
                        m_bLocalSpace)
END_PARTICLE_OPERATOR_UNPACK(C_INIT_InitialVelocityNoise);

void C_INIT_InitialVelocityNoise::InitNewParticlesBlock(
    CParticleCollection *pParticles, int start_block, int n_blocks,
    int nAttributeWriteMask, void *pContext) const {
  ...
  bool m_bNoiseAbs = (m_vecAbsValInv.x != 0.0f) || (m_vecAbsValInv.y != 0.0f) ||
                     (m_vecAbsValInv.z != 0.0f);
  ...
  float ValueScaleX, ValueScaleY, ValueScaleZ, ValueBaseX, ValueBaseY,
      ValueBaseZ;

  ValueScaleX = (flAbsScaleX * (m_vecOutputMax.x - m_vecOutputMin.x));
  ValueBaseX = (m_vecOutputMin.x +
                ((1.0 - flAbsScaleX) * (m_vecOutputMax.x - m_vecOutputMin.x)));

  ValueScaleY = (flAbsScaleY * (m_vecOutputMax.y - m_vecOutputMin.y));
  ValueBaseY = (m_vecOutputMin.y +
                ((1.0 - flAbsScaleY) * (m_vecOutputMax.y - m_vecOutputMin.y)));

  ValueScaleZ = (flAbsScaleZ * (m_vecOutputMax.z - m_vecOutputMin.z));
  ValueBaseZ = (m_vecOutputMin.z +
                ((1.0 - flAbsScaleZ) * (m_vecOutputMax.z - m_vecOutputMin.z)));
  ...
  const FourVectors *xyz =
      pParticles->Get4VAttributePtr(PARTICLE_ATTRIBUTE_XYZ, &attr_stride);
  xyz += attr_stride * start_block;
  FourVectors *pxyz = pParticles->Get4VAttributePtrForWrite(
      PARTICLE_ATTRIBUTE_PREV_XYZ, &attr_stride);
  pxyz += attr_stride * start_block;
  const fltx4 *pCreationTime = pParticles->GetM128AttributePtr(
      PARTICLE_ATTRIBUTE_CREATION_TIME, &attr_stride);
...
void C_INIT_InitialVelocityNoise::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    const float *xyz =
        pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_XYZ, start_p);
    float *pxyz = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_PREV_XYZ, start_p);
    ...
    Coord = Vector((*pCreationTime + Offset), (*pCreationTime + Offset),
                   (*pCreationTime + Offset));
...
class C_INIT_RandomLifeTime : public CParticleOperatorInstance {
  ...
  void InitNewParticlesBlock(CParticleCollection *pParticles, int start_block,
    ...
    if (m_fLifetimeRandExponent != 1.0f) {
      InitScalarAttributeRandomRangeExpBlock(
          PARTICLE_ATTRIBUTE_LIFE_DURATION, m_fLifetimeMin, m_fLifetimeMax,
          m_fLifetimeRandExponent, pParticles, start_block, n_blocks);
    } else {
      InitScalarAttributeRandomRangeBlock(PARTICLE_ATTRIBUTE_LIFE_DURATION,
                                          m_fLifetimeMin, m_fLifetimeMax,
                                          pParticles, start_block, n_blocks);
    }
  }
};

DEFINE_PARTICLE_OPERATOR(C_INIT_RandomLifeTime, "Lifetime Random",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("lifetime_random_exponent", "1", float,
...
void C_INIT_RandomLifeTime::InitNewParticlesScalar(
    ...
    int nAttributeWriteMask, void *pContext) const {
  for (; nParticleCount--; start_p++) {
    float *dtime = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_LIFE_DURATION, start_p);
    *dtime = pParticles->RandomFloatExp(m_fLifetimeMin, m_fLifetimeMax,
                                        m_fLifetimeRandExponent);
  }
}
...
class C_INIT_RandomRadius : public CParticleOperatorInstance {
  ...
  virtual void InitNewParticlesBlock(CParticleCollection *pParticles,
                                     ...
                                     int nAttributeWriteMask,
                                     void *pContext) const {
    if (m_flRadiusRandExponent != 1.0f) {
      InitScalarAttributeRandomRangeExpBlock(
          PARTICLE_ATTRIBUTE_RADIUS, m_flRadiusMin, m_flRadiusMax,
          m_flRadiusRandExponent, pParticles, start_block, n_blocks);
    } else {
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomRadius, "Radius Random",
                         OPERATOR_PI_RADIUS);
...
DMXELEMENT_UNPACK_FIELD("radius_random_exponent", "1", float,
...
void C_INIT_RandomRadius::InitNewParticlesScalar(
    ...
    int nAttributeWriteMask, void *pContext) const {
  for (; nParticleCount--; start_p++) {
    float *r = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_RADIUS, start_p);
    *r = pParticles->RandomFloatExp(m_flRadiusMin, m_flRadiusMax,
                                    m_flRadiusRandExponent);
  }
}
...
class C_INIT_RandomAlpha : public CParticleOperatorInstance {
  ...
  virtual void InitNewParticlesBlock(CParticleCollection *pParticles,
    ...
    if (m_flAlphaRandExponent != 1.0f) {
      InitScalarAttributeRandomRangeExpBlock(
          PARTICLE_ATTRIBUTE_ALPHA, m_flAlphaMin, m_flAlphaMax,
          m_flAlphaRandExponent, pParticles, start_block, n_blocks);
    } else {
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
                              ...
                              void *pContext) const {
    for (; nParticleCount--; start_p++) {
      float *pAlpha = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_ALPHA, start_p);
      *pAlpha = pParticles->RandomFloatExp(m_flAlphaMin, m_flAlphaMax,
...
DMXELEMENT_UNPACK_FIELD("alpha_random_exponent", "1", float,
                        m_flAlphaRandExponent)
 ...
 protected:
  ...
  virtual void InitNewParticlesBlock(CParticleCollection *pParticles,
    ...
    if (m_flRotationRandExponent != 1.0f) {
      InitScalarAttributeRandomRangeExpBlock(
          GetAttributeToInit(), m_flRadiansMin, m_flRadiansMax,
          m_flRotationRandExponent, pParticles, start_block, n_blocks);
    } else {
      InitScalarAttributeRandomRangeBlock(GetAttributeToInit(), m_flRadiansMin,
                                          m_flRadiansMax, pParticles,
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    for (; nParticleCount--; start_p++) {
      float *drot = pParticles->GetFloatAttributePtrForWrite(
          GetAttributeToInit(), start_p);
      *drot = m_flRadians +
              pParticles->RandomFloatExp(m_flRadiansMin, m_flRadiansMax,
...
class CAddGeneralRandomRotation : public CParticleOperatorInstance {
 protected:
  ...
  virtual void InitNewParticlesBlock(CParticleCollection *pParticles,
                                     ...
                                     int nAttributeWriteMask,
                                     void *pContext) const {
    AddScalarAttributeRandomRangeBlock(GetAttributeToInit(), m_flRadiansMin,
                                       m_flRadiansMax, m_flRotationRandExponent,
                                       pParticles, start_block, n_blocks,
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    if (!m_bRandomlyFlipDirection) {
      for (; nParticleCount--; start_p++) {
        float *pAttr = pParticles->GetFloatAttributePtrForWrite(
            GetAttributeToInit(), start_p);
        *pAttr += m_flRadians +
                  pParticles->RandomFloatExp(m_flRadiansMin, m_flRadiansMax,
                                             m_flRotationRandExponent);
      ...
      for (; nParticleCount--; start_p++) {
        float *pAttr = pParticles->GetFloatAttributePtrForWrite(
            GetAttributeToInit(), start_p);
        float flSpeed = m_flRadians + pParticles->RandomFloatExp(
...
DMXELEMENT_UNPACK_FIELD("rotation_random_exponent", "1", float,
                        m_flRotationRandExponent)
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomRotationSpeed, "Rotation Speed Random",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("rotation_speed_random_max", "360", float,
                        m_flDegreesMax)
DMXELEMENT_UNPACK_FIELD("rotation_speed_random_exponent", "1", float,
                        m_flRotationRandExponent)
DMXELEMENT_UNPACK_FIELD("randomly_flip_direction", "1", bool,
                        m_bRandomlyFlipDirection)
...
DMXELEMENT_UNPACK_FIELD("yaw_random_exponent", "1", float,
                        m_flRotationRandExponent)
...
class C_INIT_RandomColor : public CParticleOperatorInstance {
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    float *pColor;
    for (; nParticleCount--; start_p++) {
      pColor = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_TINT_RGB, start_p);

      randomPerc = pParticles->RandomFloat(0.0f, 1.0f);

      // Randomly choose a range between the two colors
      pColor[0] = m_flNormColorMin[0] +
                  ((m_flNormColorMax[0] - m_flNormColorMin[0]) * randomPerc);
      pColor[4] = m_flNormColorMin[1] +
                  ((m_flNormColorMax[1] - m_flNormColorMin[1]) * randomPerc);
      pColor[8] = m_flNormColorMin[2] +
                  ((m_flNormColorMax[2] - m_flNormColorMin[2]) * randomPerc);

      // Tint the particles
      if (m_flTintPerc) {
        pColor[0] =
            Lerp(m_flTintPerc, (float)pColor[0], (float)tint.r() / 255.0f);
        pColor[4] =
            Lerp(m_flTintPerc, (float)pColor[4], (float)tint.g() / 255.0f);
        pColor[8] =
            Lerp(m_flTintPerc, (float)pColor[8], (float)tint.b() / 255.0f);
      }
    }
  }
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomColor, "Color Random",
...
class C_INIT_RandomTrailLength : public CParticleOperatorInstance {
  ...
  virtual void InitNewParticlesBlock(CParticleCollection *pParticles,
    ...
    if (m_flLengthRandExponent != 1.0f) {
      InitScalarAttributeRandomRangeExpBlock(
          PARTICLE_ATTRIBUTE_TRAIL_LENGTH, m_flMinLength, m_flMaxLength,
          m_flLengthRandExponent, pParticles, start_block, n_blocks);
    } else {
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    float *pLength;
    for (; nParticleCount--; start_p++) {
      pLength = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_TRAIL_LENGTH, start_p);
      *pLength = pParticles->RandomFloatExp(m_flMinLength, m_flMaxLength,
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomTrailLength, "Trail Length Random",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("length_random_exponent", "1", float,
                        m_flLengthRandExponent)
...
class C_INIT_RandomSequence : public CParticleOperatorInstance {
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    float *pSequence;
    for (; nParticleCount--; start_p++) {
      pSequence = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER, start_p);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomSequence, "Sequence Random",
                         OPERATOR_GENERIC);
...
// Scales initial position and velocity of particles within a random vector
// range
...
DMXELEMENT_UNPACK_FIELD(
    "warp transition time (treats min/max as start/end sizes)", "0", float,
    m_flWarpTime)
DMXELEMENT_UNPACK_FIELD("warp transition start time", "0", float,
                        m_flWarpStartTime)
...
void C_INIT_PositionWarp::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if (m_flWarpTime != 0.0f) {
      ...
      float flPercentage =
          RemapValClamped(*ct, m_flWarpStartTime, flWarpEnd, 0.0, 1.0);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreationNoise, "Remap Noise to Scalar",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_CreationNoise)
DMXELEMENT_UNPACK_FIELD("time noise coordinate scale", "0.1", float,
                        m_flNoiseScale)
DMXELEMENT_UNPACK_FIELD("spatial noise coordinate scale", "0.001", float,
                        m_flNoiseScaleLoc)
...
DMXELEMENT_UNPACK_FIELD("world time noise coordinate scale", "0", float,
                        m_flWorldTimeScale)
END_PARTICLE_OPERATOR_UNPACK(C_INIT_CreationNoise);

void C_INIT_CreationNoise::InitNewParticlesBlock(
    CParticleCollection *pParticles, int start_block, int n_blocks,
    int nAttributeWriteMask, void *pContext) const {
  float flAbsScale;
  fltx4 fl4AbsVal;
  ...
  fltx4 *pAttr =
      pParticles->GetM128AttributePtrForWrite(m_nFieldOutput, &attr_stride);
  pAttr += attr_stride * start_block;
  const FourVectors *pxyz =
      pParticles->Get4VAttributePtr(PARTICLE_ATTRIBUTE_XYZ, &attr_stride);
  pxyz += attr_stride * start_block;
  const fltx4 *pCreationTime = pParticles->GetM128AttributePtr(
      PARTICLE_ATTRIBUTE_CREATION_TIME, &attr_stride);
...
void C_INIT_CreationNoise::InitNewParticlesScalar(
  ...
  const float *pCreationTime = pParticles->GetFloatAttributePtr(
      PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
  float Offset = m_flOffset;
  CoordBase = Vector((*pCreationTime + Offset), (*pCreationTime + Offset),
                     (*pCreationTime + Offset));
  CoordBase *= CoordScale;
  CoordWorldTime = Vector((Plat_MSTime() * m_flWorldTimeScale),
                          (Plat_MSTime() * m_flWorldTimeScale),
                          (Plat_MSTime() * m_flWorldTimeScale));
  ...
  for (; nParticleCount--; start_p++) {
    const float *pxyz =
        pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_XYZ, start_p);
    float *pAttr =
        pParticles->GetFloatAttributePtrForWrite(m_nFieldOutput, start_p);
    ...
    *((int *)&flNoise) &= nAbsVal;
    ...
    if (ATTRIBUTES_WHICH_ARE_0_TO_1 & (1 << m_nFieldOutput)) {
      flInitialNoise = std::clamp(flInitialNoise, 0.0f, 1.0f);
    }
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateAlongPath, "Position Along Path Random",
                         OPERATOR_PI_POSITION);
...
void C_INIT_CreateAlongPath::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    pParticles->CalculatePathValues(m_PathParams, *ct, &StartPnt, &MidP,
                                    &EndPnt);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_MoveBetweenPoints,
...
void C_INIT_MoveBetweenPoints::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    pParticles->GetControlPointAtTime(m_nEndControlPointNumber, *ct,
                                      &vecControlPoint);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RemapScalar, "remap initial scalar",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_RemapScalar)
DMXELEMENT_UNPACK_FIELD("emitter lifetime start time (seconds)", "-1", float,
                        m_flStartTime)
DMXELEMENT_UNPACK_FIELD("emitter lifetime end time (seconds)", "-1", float,
                        m_flEndTime)
DMXELEMENT_UNPACK_FIELD_USERDATA("input field", "8", int, m_nFieldInput,
                                 "intchoice particlefield_scalar")
...
void C_INIT_RemapScalar::InitNewParticlesScalar(CParticleCollection *pParticles,
                                                int start_p, int nParticleCount,
                                                int nAttributeWriteMask,
                                                void *pContext) const {
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if (ATTRIBUTES_WHICH_ARE_INTS & (1 << m_nFieldInput)) {
      const int *pInput =
          pParticles->GetIntAttributePtr(m_nFieldInput, start_p);
      flInput = float(*pInput);
    } else {
      const float *pInput =
          pParticles->GetFloatAttributePtr(m_nFieldInput, start_p);
      flInput = *pInput;
    }
    ...
    if ((((flLifeTime < m_flStartTime) || (flLifeTime >= m_flEndTime)) &&
        ...
        (m_bActiveRange && (flInput < m_flInputMin || flInput > m_flInputMax)))
      continue;
...
void C_INIT_InheritVelocity::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    Vector vecControlPoint;
    pParticles->GetControlPointAtTime(m_nControlPointNumber, *ct,
                                      &vecControlPoint);
    Vector vecControlPointPrev;
    pParticles->GetControlPointAtPrevTime(m_nControlPointNumber,
                                          &vecControlPointPrev);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_AgeNoise, "Lifetime Pre-Age Noise",
                         OPERATOR_GENERIC);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_AgeNoise)
DMXELEMENT_UNPACK_FIELD("time noise coordinate scale", "1.0", float,
                        m_flNoiseScale)
DMXELEMENT_UNPACK_FIELD("spatial noise coordinate scale", "1.0", float,
                        m_flNoiseScaleLoc)
DMXELEMENT_UNPACK_FIELD("time coordinate offset", "0", float, m_flOffset)
DMXELEMENT_UNPACK_FIELD("spatial coordinate offset", "0 0 0", Vector,
                        m_vecOffsetLoc)
...
void C_INIT_AgeNoise::InitNewParticlesScalar(CParticleCollection *pParticles,
                                             int start_p, int nParticleCount,
                                             int nAttributeWriteMask,
                                             void *pContext) const {
  float flAbsScale;
  ...
  for (; nParticleCount--; start_p++) {
    ...
    const float *pCreationTime = pParticles->GetFloatAttributePtr(
        PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
    const float *pLifespan = pParticles->GetFloatAttributePtr(
        PARTICLE_ATTRIBUTE_LIFE_DURATION, start_p);
    float *pAttr = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_SequenceLifeTime, "lifetime from sequence",
                         OPERATOR_GENERIC);
...
void C_INIT_SequenceLifeTime::InitNewParticlesScalar(
  ...
  if ((m_flFramerate != 0.0f) && (pParticles->m_Sheet())) {
    for (; nParticleCount--; start_p++) {
      const float *flSequence = pParticles->GetFloatAttributePtr(
          PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER, start_p);
      float *dtime = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_LIFE_DURATION, start_p);
      ...
      if (pParticles->m_Sheet()->m_flFrameSpan[nSequence] != 0) {
        *dtime =
            pParticles->m_Sheet()->m_flFrameSpan[nSequence] / m_flFramerate;
      } else {
        *dtime = 1.0;
      }
    }
...
class C_INIT_CreateInHierarchy : public CParticleOperatorInstance {
  ...
  virtual uint64_t GetReadControlPointMask() const {
    uint64_t nStartMask = (1ULL << m_nDesiredStartPoint) - 1;
    uint64_t nEndMask = m_bUseHighestEndCP
                            ? 0xFFFFFFFFFFFFFFFF
                            : (1ULL << (m_nDesiredEndPoint + 1)) - 1;
    return nEndMask & (~nStartMask);
  }
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateInHierarchy, "Position In CP Hierarchy",
                         OPERATOR_PI_POSITION);
...
DMXELEMENT_UNPACK_FIELD("start control point number", "0", int,
                        m_nDesiredStartPoint)
DMXELEMENT_UNPACK_FIELD("end control point number", "1", int,
                        m_nDesiredEndPoint)
DMXELEMENT_UNPACK_FIELD(
    "bulge control 0=random 1=orientation of start pnt 2=orientation of end "
    "point",
    "0", int, m_nOrientation)
...
// DMXELEMENT_UNPACK_FIELD( "trace distance for optional culling", "0.0", float,
// m_flTraceDist )
DMXELEMENT_UNPACK_FIELD("use highest supplied end point", "0", bool,
                        m_bUseHighestEndCP)
DMXELEMENT_UNPACK_FIELD("distance_bias", "1 1 1", Vector, m_vecDistanceBias)
DMXELEMENT_UNPACK_FIELD("distance_bias_absolute_value", "0 0 0", Vector,
                        m_vecDistanceBiasAbs)
...
void C_INIT_CreateInHierarchy::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if ((pParticles->m_flCurTime <= m_flGrowthTime) && (nRealEndPoint > 0)) {
      float nCurrentEndCP = RemapValClamped(
          *ct, 0.0f, m_flGrowthTime,
          std::min(m_nDesiredStartPoint + 1, nRealEndPoint), nRealEndPoint);
      nEndCP = pParticles->RandomInt(
          std::min(m_nDesiredStartPoint + 1.0f, nCurrentEndCP), nCurrentEndCP);
      ...
      int nLowestStartPoint = std::min(m_nDesiredStartPoint + 1, nRealEndPoint);
      nEndCP = pParticles->RandomInt(nLowestStartPoint, nRealEndPoint);
      flGrowth = 1.0;
    }

    PathParams.m_nStartControlPointNumber =
        pParticles->m_ControlPoints[nEndCP].m_nParent;
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RemapScalarToVector, "remap scalar to vector",
                         OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("input minimum", "0", float, m_flInputMin)
DMXELEMENT_UNPACK_FIELD("input maximum", "1", float, m_flInputMax)
DMXELEMENT_UNPACK_FIELD_USERDATA("output field", "0", int, m_nFieldOutput,
...
void C_INIT_RemapScalarToVector::InitNewParticlesScalar(
  ...
  for (; nParticleCount--; start_p++) {
    ...
    vecOutput.x = RemapValClamped(*pInput, m_flInputMin, m_flInputMax,
                                  m_vecOutputMin.x, m_vecOutputMax.x);
    vecOutput.y = RemapValClamped(*pInput, m_flInputMin, m_flInputMax,
                                  m_vecOutputMin.y, m_vecOutputMax.y);
    vecOutput.z = RemapValClamped(*pInput, m_flInputMin, m_flInputMax,
    ...
    if (m_nFieldOutput == 0) {
      ...
      if (!m_bLocalCoords) {
        Vector vecControlPoint;
        pParticles->GetControlPointAtTime(m_nControlPointNumber, *pCreationTime,
                                          &vecControlPoint);
...
class C_INIT_CreateSequentialPath : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    SequentialPathContext_t *pCtx =
        reinterpret_cast<SequentialPathContext_t *>(pContext);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateSequentialPath,
...
DEFINE_PARTICLE_OPERATOR(C_INIT_InitialRepulsionVelocity,
                         "Velocity Repulse from World", OPERATOR_GENERIC);
...
DMXELEMENT_UNPACK_FIELD("control_point_number", "0", int, m_nControlPointNumber)
DMXELEMENT_UNPACK_FIELD("Per Particle World Collision Tests", "0", bool,
                        m_bPerParticle)
DMXELEMENT_UNPACK_FIELD("Use radius for Per Particle Trace Length", "0", bool,
                        m_bPerParticleTR)
...
DMXELEMENT_UNPACK_FIELD("control points to broadcast to children (n + 1)", "-1",
                        int, m_nChildCP)
...
void C_INIT_InitialRepulsionVelocity::InitNewParticlesScalar(
  ...
  if (m_bPerParticle) {
    for (; nParticleCount--; start_p++) {
      float *pxyz = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_XYZ, start_p);
      float *pxyz_prev = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_PREV_XYZ, start_p);
      const float *radius =
          pParticles->GetFloatAttributePtr(PARTICLE_ATTRIBUTE_RADIUS, start_p);
      ...
      for (int i = 0; i < 6; i++) {
        ...
        CBaseTrace tr;
        g_pParticleSystemMgr->Query()->TraceLine(vecCurrentPos, endpos,
                                                 CONTENTS_SOLID, NULL,
      ...
      vecRepulsionAmount.x =
          Lerp(resultForce, m_vecOutputMin.x, m_vecOutputMax.x);
      vecRepulsionAmount.y =
          Lerp(resultForce, m_vecOutputMin.y, m_vecOutputMax.y);
      vecRepulsionAmount.z =
          Lerp(resultForce, m_vecOutputMin.z, m_vecOutputMax.z);
    ...
    if (m_bInherit) {
      float *ct = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_CREATION_TIME, start_p);
      pParticles->GetControlPointAtTime(m_nControlPointNumber, *ct,
                                        &resultDirection);
      Vector vecPassedForce;
      pParticles->GetControlPointAtTime(m_nControlPointNumber + 1, *ct,
                                        &vecPassedForce);

      vecRepulsionAmount.x =
          Lerp(vecPassedForce.x, m_vecOutputMin.x, m_vecOutputMax.x);
      vecRepulsionAmount.y =
          Lerp(vecPassedForce.x, m_vecOutputMin.y, m_vecOutputMax.y);
      vecRepulsionAmount.z =
          Lerp(vecPassedForce.x, m_vecOutputMin.z, m_vecOutputMax.z);
      ...
      if (m_nChildCP != -1) {
        for (CParticleCollection *pChild = pParticles->m_Children.m_pHead;
             pChild; pChild = pChild->m_pNext) {
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomYawFlip, "Rotation Yaw Flip Random",
                         OPERATOR_GENERIC);
...
class C_INIT_RandomSecondSequence : public CParticleOperatorInstance {
  ...
  void InitNewParticlesScalar(CParticleCollection *pParticles, int start_p,
    ...
    for (; nParticleCount--; start_p++) {
      pSequence = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER1, start_p);
      *pSequence = pParticles->RandomInt(m_nSequenceMin, m_nSequenceMax);
    }
  }
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RandomSecondSequence, "Sequence Two Random",
                         OPERATOR_GENERIC);
...
class C_INIT_RemapCPtoScalar : public CParticleOperatorInstance {
  ...
  virtual void InitParams(CParticleSystemDefinition *pDef,
                          CDmxElement *pElement) {
    m_nField = std::clamp(m_nField, 0, 2);
  }
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RemapCPtoScalar,
                         "remap control point to scalar", OPERATOR_GENERIC);
...
class C_INIT_RemapCPtoVector : public CParticleOperatorInstance {
  ...
  virtual uint64_t GetReadControlPointMask() const {
    uint64_t nMask = (1ULL << m_nCPInput);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_RemapCPtoVector,
                         "remap control point to Vector", OPERATOR_GENERIC);
...
void C_INIT_RemapCPtoVector::InitNewParticlesScalar(
    ...
    int nAttributeWriteMask, void *pContext) const {
  Vector vecControlPoint;
  pParticles->GetControlPointAtTime(m_nCPInput, pParticles->m_flCurTime,
                                    &vecControlPoint);
  Vector vOutputMinLocal = m_vOutputMin;
  Vector vOutputMaxLocal = m_vOutputMax;
  if (m_nLocalSpaceCP != -1) {
    matrix3x4_t mat;
    pParticles->GetControlPointTransformAtTime(m_nLocalSpaceCP,
                                               pParticles->m_flCurTime, &mat);
    Vector vecTransformLocal = vec3_origin;
  ...
  for (; nParticleCount--; start_p++) {
    ...
    Vector vOutput;
    vOutput.x = RemapValClamped(vecControlPoint.x, m_vInputMin.x, m_vInputMax.x,
                                vOutputMinLocal.x, vOutputMaxLocal.x);
    vOutput.y = RemapValClamped(vecControlPoint.y, m_vInputMin.y, m_vInputMax.y,
                                vOutputMinLocal.y, vOutputMaxLocal.y);
    vOutput.z = RemapValClamped(vecControlPoint.z, m_vInputMin.z, m_vInputMax.z,
                                vOutputMinLocal.z, vOutputMaxLocal.z);
    ...
    if (m_nFieldOutput == 6) {
      pOutput[0] = std::max(0.0f, std::min(vOutput.x, 1.0f));
      pOutput[4] = std::max(0.0f, std::min(vOutput.y, 1.0f));
      pOutput[8] = std::max(0.0f, std::min(vOutput.z, 1.0f));
    } else {
      float *pXYZ_Prev = pParticles->GetFloatAttributePtrForWrite(
          PARTICLE_ATTRIBUTE_PREV_XYZ, start_p);
      Vector vXYZPrev;
...
class C_INIT_CreateFromParentParticles : public CParticleOperatorInstance {
  ...
  virtual void InitializeContextData(CParticleCollection *pParticles,
                                     void *pContext) const {
    ParentParticlesContext_t *pCtx =
        reinterpret_cast<ParentParticlesContext_t *>(pContext);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_CreateFromParentParticles,
                         "Position From Parent Particles",
                         OPERATOR_PI_POSITION);

BEGIN_PARTICLE_OPERATOR_UNPACK(C_INIT_CreateFromParentParticles)
DMXELEMENT_UNPACK_FIELD("Inherited Velocity Scale", "0", float,
                        m_flVelocityScale)
DMXELEMENT_UNPACK_FIELD("Random Parent Particle Distribution", "0", bool,
  ...
  for (; nParticleCount--; start_p++) {
    if (m_bRandomDistribution) {
      pCtx->m_nCurrentParentParticle =
          pParticles->RandomInt(0, nActiveParticles);
    ...
    float *pxyz = pParticles->GetFloatAttributePtrForWrite(
        PARTICLE_ATTRIBUTE_PREV_XYZ, start_p);
    const float *pParent_xyz = pParticles->m_pParent->GetFloatAttributePtr(
        PARTICLE_ATTRIBUTE_XYZ, pCtx->m_nCurrentParentParticle);
    const float *pParent_pxyz = pParticles->m_pParent->GetFloatAttributePtr(
        PARTICLE_ATTRIBUTE_PREV_XYZ, pCtx->m_nCurrentParentParticle);
...
DEFINE_PARTICLE_OPERATOR(C_INIT_DistanceToCPInit,
  ...
  for (; nParticleCount--; start_p++) {
    ...
    if (m_bScaleInitialRange) {
      const float *pInitialOutput =
          pParticles->GetFloatAttributePtr(m_nFieldOutput, start_p);
...
void AddBuiltInParticleInitializers() {
  ...
  REGISTER_PARTICLE_OPERATOR(FUNCTION_INITIALIZER,
