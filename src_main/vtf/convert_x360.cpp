...
// if the entire vtf file is smaller than this threshold, add entirely to
// preload
...
//-----------------------------------------------------------------------------
bool ComputeTargetDimensions(const char *pDebugName, IVTFTexture *pVTFTexture,
                             int picmip, int &width, int &height, int &mipCount,
                             int &mipSkipCount, bool &bNoMip) {
  ...
  int nClampX = 1 << 30;
  int nClampY = 1 << 30;
  TextureLODControlSettings_t const *pLODInfo =
      reinterpret_cast<TextureLODControlSettings_t const *>(
...
//-----------------------------------------------------------------------------
bool ConvertImageFormatEx(unsigned char *pSourceImage, int sourceImageSize,
                          ImageFormat sourceFormat, unsigned char *pTargetImage,
                          int targetImageSize, ImageFormat targetFormat,
                          int width, int height, bool bSrgbGammaConvert) {
  // format conversion expects pc oriented data
  // but, formats that are >8 bits per channels need to be element pre-swapped
  ImageLoader::PreConvertSwapImageData(pSourceImage, sourceImageSize,
                                       sourceFormat);

  bool bRetVal = ImageLoader::ConvertImageFormat(
      pSourceImage, sourceFormat, pTargetImage, targetFormat, width, height);
  if (!bRetVal) {
    return false;
  }

  // convert to proper channel order for 360 d3dformats
  ImageLoader::PostConvertSwapImageData(pTargetImage, targetImageSize,
                                        targetFormat);

  // Convert colors from sRGB gamma space into 360 piecewise linear gamma space
  if (bSrgbGammaConvert == true) {
    if (targetFormat == IMAGE_FORMAT_BGRA8888 ||
        targetFormat == IMAGE_FORMAT_BGRX8888) {
      // Msg( "   Converting 8888 texture from sRGB gamma to 360 PWL gamma ***
      // %dx%d\n", width, height );
      for (int i = 0; i < (targetImageSize / 4);
           i++)  // targetImageSize is the raw data length in bytes
      {
        ...
        for (int j = 0; j < 3; j++)  // For red, green, blue
        {
          ...
          *(pRGB[j]) = (unsigned char)(std::clamp(
              ((fl360Gamma * 255.0f) + 0.5f), 0.0f, 255.0f));
               ...
               (targetFormat == IMAGE_FORMAT_DXT3) ||
               (targetFormat == IMAGE_FORMAT_DXT5)) {
      // Msg( "   Converting DXT texture from sRGB gamma to 360 PWL gamma ***
      // %dx%d\n", width, height );
      int nStrideBytes = 8;
      int nOffsetBytes = 0;
      if ((targetFormat == IMAGE_FORMAT_DXT3) ||
          (targetFormat == IMAGE_FORMAT_DXT5)) {
        nOffsetBytes = 8;
        nStrideBytes = 16;
      }

      for (int i = 0; i < (targetImageSize / nStrideBytes);
           i++)  // For each color or color/alpha block
      {
        // Get 16bit 565 colors into an unsigned short
        unsigned short n565Color0 = 0;
        n565Color0 |=
            ((unsigned short)(unsigned char)(pTargetImage[(i * nStrideBytes) +
                                                          nOffsetBytes + 0]))
            << 8;
        n565Color0 |=
            ((unsigned short)(unsigned char)(pTargetImage[(i * nStrideBytes) +
        ...
        // Since we have one bit less of red and blue, add some of the error
        // back in
        ...
        for (int j = 0; j < 3; j++)  // For red, green, blue
        {
          for (int k = 0; k < 2; k++)  // For color0 and color1
          {
            ...
            unsigned char *p8BitValue =
                (k == 0) ? &(v888Color0New[j]) : &(v888Color1New[j]);
        ...
        unsigned short n565Color0New = ((unsigned short)v888Color0New[0] << 8) |
                                       ((unsigned short)v888Color0New[1] << 3) |
                                       ((unsigned short)v888Color0New[2] >> 3);

        v888Color1New[0] &= 0xf8;  // 5 bits
        v888Color1New[1] &= 0xfc;  // 6 bits
        v888Color1New[2] &= 0xf8;  // 5 bits
        unsigned short n565Color1New = ((unsigned short)v888Color1New[0] << 8) |
                                       ((unsigned short)v888Color1New[1] << 3) |
                                       ((unsigned short)v888Color1New[2] >> 3);

        // If we're targeting DXT1, make sure we haven't made a non transparent
        // color block transparent
        if ((targetFormat == IMAGE_FORMAT_DXT1) ||
          ...
          if ((n565Color0New <= n565Color1New) && (n565Color0 > n565Color1)) {
            if ((v888Color0New[0] == v888Color1New[0]) &&
                (v888Color0[0] != v888Color1[0])) {
            ...
            if ((v888Color0New[1] == v888Color1New[1]) &&
                (v888Color0[1] != v888Color1[1])) {
            ...
            if ((v888Color0New[2] == v888Color1New[2]) &&
                (v888Color0[2] != v888Color1[2])) {
            ...
            n565Color0New = ((unsigned short)v888Color0New[0] << 8) |
                            ((unsigned short)v888Color0New[1] << 3) |
                            ((unsigned short)v888Color0New[2] >> 3);
            n565Color1New = ((unsigned short)v888Color1New[0] << 8) |
                            ((unsigned short)v888Color1New[1] << 3) |
                            ((unsigned short)v888Color1New[2] >> 3);
          }
        }
        ...
        pTargetImage[(i * nStrideBytes) + nOffsetBytes + 0] =
            (unsigned char)((n565Color0New >> 8) & 0x00ff);
        pTargetImage[(i * nStrideBytes) + nOffsetBytes + 1] =
            (unsigned char)(n565Color0New & 0x00ff);

        pTargetImage[(i * nStrideBytes) + nOffsetBytes + 2] =
            (unsigned char)((n565Color1New >> 8) & 0x00ff);
        pTargetImage[(i * nStrideBytes) + nOffsetBytes + 3] =
            (unsigned char)(n565Color1New & 0x00ff);
      }
    }
  }
...
//-----------------------------------------------------------------------------
bool SerializeImageData(IVTFTexture *pSourceVTF, int frame, int face, int mip,
                        ImageFormat targetFormat, CUtlBuffer &targetBuf) {
  int width;
  int height;
  int targetImageSize;
  uint8_t *pSourceImage;
  int sourceImageSize;
  int targetSize;
  CUtlMemory<uint8_t> targetImage;
  ...
  sourceImageSize = ImageLoader::GetMemRequired(width, height, 1,
                                                pSourceVTF->Format(), false);
  pSourceImage = pSourceVTF->ImageData(frame, face, mip);

  targetImageSize =
      ImageLoader::GetMemRequired(width, height, 1, targetFormat, false);
  targetImage.EnsureCapacity(targetImageSize);
  uint8_t *pTargetImage = (uint8_t *)targetImage.Base();
  ...
  bool bRetVal = ConvertImageFormatEx(
      pSourceImage, sourceImageSize, pSourceVTF->Format(), pTargetImage,
      targetImageSize, targetFormat, width, height,
      (pSourceVTF->Flags() & TEXTUREFLAGS_SRGB) ? true : false);
  if (!bRetVal) {
    return false;
  }

  // X360TBD: incorrect uint8_t order
  //	// fixup mip dependent data
  //	if ( ( pSourceVTF->Flags() & TEXTUREFLAGS_ONEOVERMIPLEVELINALPHA ) && (
  // targetFormat == IMAGE_FORMAT_BGRA8888 ) )
  //	{
  //		unsigned char ooMipLevel = ( unsigned char )( 255.0f * ( 1.0f /
  //(  float )( 1 << mip ) ) ); 		int i;
  //
...
bool ConvertVTFTo360Format(const char *pDebugName, CUtlBuffer &sourceBuf,
  ...
  // Only need to uint8_t swap writes if we are running the coversion on the PC,
  // and data will be read from 360
  ...
  if (!pSourceVTF->Unserialize(sourceBuf, true, 0)) goto cleanUp;
  ...
  if (!ComputeTargetDimensions(pDebugName, pSourceVTF, 0, targetWidth,
                               targetHeight, targetMipCount, mipSkipCount,
                               bNoMip))
    goto cleanUp;
  ...
  // unserialize the vtf with all the data configured with the desired starting
  // mip
  ...
  numTypes =
      pSourceVTF->GetResourceTypes(resourceTypes, MAX_RSRC_DICTIONARY_ENTRIES);
  for (int i = 0; i < numTypes; i++) {
    ...
    switch (resourceTypes[i] & ~RSRCF_MASK) {
      ...
      default:
        pResourceData =
            pSourceVTF->GetResourceData(resourceTypes[i], &resourceLength);
        if (pResourceData) {
  ...
  targetFormat = PreferredFormat(pSourceVTF, pSourceVTF->Format(), targetWidth,
                                 targetHeight, targetMipCount, targetFaceCount);
  ...
  bHasLowResData = (pSourceVTF->LowResFormat() != -1) &&
  ...
  targetBuf.EnsureCapacity(sizeof(VTFFileHeaderX360_t) +
                           targetResources.Count() * sizeof(ResourceEntryInfo));
  targetBuf.SeekPut(CUtlBuffer::SEEK_CURRENT,
                    sizeof(VTFFileHeaderX360_t) +
                        targetResources.Count() * sizeof(ResourceEntryInfo));

  // serialize low res
  unsigned char targetLowResSample[4];
  if (targetLowResWidth && targetLowResHeight) {
    CUtlMemory<uint8_t> targetLowResImage;

    int sourceLowResImageSize = ImageLoader::GetMemRequired(
        pSourceVTF->LowResWidth(), pSourceVTF->LowResHeight(), 1,
        pSourceVTF->LowResFormat(), false);
    int targetLowResImageSize = ImageLoader::GetMemRequired(
        targetLowResWidth, targetLowResHeight, 1, IMAGE_FORMAT_RGB888, false);

    // conversion may skip bytes, ensure all bits initialized
    targetLowResImage.EnsureCapacity(targetLowResImageSize);
    uint8_t *pTargetLowResImage = (uint8_t *)targetLowResImage.Base();
    ...
    bRetVal = ConvertImageFormatEx(
        pSourceVTF->LowResImageData(), sourceLowResImageSize,
        pSourceVTF->LowResFormat(), pTargetLowResImage, targetLowResImageSize,
        IMAGE_FORMAT_RGB888, targetLowResWidth, targetLowResHeight, false);
    if (!bRetVal) {
      goto cleanUp;
    }
    ...
    for (int j = 0; j < targetLowResWidth * targetLowResHeight; j++) {
      linearColor.x +=
          SrgbGammaToLinear(pTargetLowResImage[j * 3 + 0] * 1.0f / 255.0f);
      linearColor.y +=
          SrgbGammaToLinear(pTargetLowResImage[j * 3 + 1] * 1.0f / 255.0f);
      linearColor.z +=
          SrgbGammaToLinear(pTargetLowResImage[j * 3 + 2] * 1.0f / 255.0f);
    }
    VectorScale(linearColor, 1.0f / (targetLowResWidth * targetLowResHeight),
                linearColor);
  ...
  for (int i = 0; i < targetResources.Count(); i++) {
    ...
    if (resourceDataLength == 4) {
      ...
      V_memcpy(&targetResources[i].m_EntryInfo.resData,
               targetResources[i].m_pData, 4);
    } else if (resourceDataLength != 0) {
      targetResources[i].m_EntryInfo.resData = targetBuf.TellPut();
      int swappedLength;
      byteSwapWriter.SwapBufferToTargetEndian(&swappedLength,
                                              &resourceDataLength);
  ...
  // preload data might be updated and pushed to extend into the image data mip
  // chain
  ...
  for (mip = targetMipCount - 1; mip >= 0; mip--) {
    for (frame = 0; frame < targetFrameCount; frame++) {
      for (face = 0; face < targetFaceCount; face++) {
        if (!SerializeImageData(pSourceVTF, frame, face, mip, targetFormat,
                                targetBuf)) {
          goto cleanUp;
        }
      }
  ...
  if (preloadDataSize <
      VTFFileHeaderSize(VTF_X360_MAJOR_VERSION, VTF_X360_MINOR_VERSION)) {
  ...
  if (preloadDataSize >= 65536) {
    // possible overflow due to large frames, faces, and format, may have to
    // offset to 32 bits
  ...
  targetHeader.headerSize = sizeof(VTFFileHeaderX360_t) +
                            targetResources.Count() * sizeof(ResourceEntryInfo);
  ...
  for (int i = 0; i < targetResources.Count(); i++) {
    ...
    if (!(targetResources[i].m_EntryInfo.eType & RSRCF_HAS_NO_DATA_CHUNK)) {
      ...
      byteSwapWriter.SwapBufferToTargetEndian(
          &targetResources[i].m_EntryInfo.resData);
    }
...
//-----------------------------------------------------------------------------
// Copy the 360 preload data into a buffer. Used by tools to request the
// preload, as part of the preload build process. Caller doesn't have to know
// cracking details. Not to be used at gametime.
//-----------------------------------------------------------------------------
