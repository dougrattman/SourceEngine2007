...
#ifndef SOURCE_VTF_CVTF_H_
#define SOURCE_VTF_CVTF_H_
...
class CVTFTexture : public IVTFTexture {
 public:
  ...
  virtual void *SetResourceData(uint32_t eType, void const *pData,
                                size_t nDataSize);
  virtual void *GetResourceData(uint32_t eType, size_t *pDataSize) const;
  ...
  virtual void ComputeMipLevelDimensions(int iMipLevel, int *pWidth,
                                         int *pHeight, int *pMipDepth) const;
 ...
 private:
  ...
  void BlendCubeMapEdgePalettes(int iFrame, int iMipLevel,
                                const CEdgeMatch *pMatch);

  void BlendCubeMapCornerPalettes(int iFrame, int iMipLevel,
                                  const CCornerMatch *pMatch);

  void MatchCubeMapS3TCPalettes(CEdgeMatch edgeMatches[NUM_EDGE_MATCHES],
  ...
  void SetupTextureEdgeIncrements(int iMipLevel, int iFace1Edge, int iFace2Edge,
                                  bool bFlipFace2Edge, CEdgeIncrements *incs);

  void BlendCubeMapFaceEdges(int iFrame, int iMipLevel,
                             const CEdgeMatch *pMatch);

  void BlendCubeMapFaceCorners(int iFrame, int iMipLevel,
                               const CCornerMatch *pMatch);

  void BuildCubeMapMatchLists(CEdgeMatch edgeMatches[NUM_EDGE_MATCHES],
                              CCornerMatch cornerMatches[NUM_CORNER_MATCHES],
                              bool bSkybox);
  ...
  bool LoadImageData(CUtlBuffer &buf, const VTFFileHeader_t &header,
                     int nSkipMipLevels);
  ...
  void ComputeSpheremapFrame(unsigned char **ppCubeFaces,
                             unsigned char *pSpheremap, LookDir_t lookDir);

  // Makes a single frame of spheremap
  void ComputeHemispheremapFrame(unsigned char **ppCubeFaces,
                                 unsigned char *pSpheremap, LookDir_t lookDir);
  ...
  int GetImageOffset(int iFrame, int iFace, int iMipLevel,
                     ImageFormat fmt) const;

  // Determines if the vtf or vtfx file needs to be swapped to the current
  // platform
