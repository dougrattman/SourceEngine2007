...
BEGIN_BYTESWAP_DATADESC_(VTFFileHeaderV7_1_t, VTFFileBaseHeader_t)
  DEFINE_FIELD(width, FIELD_SHORT), DEFINE_FIELD(height, FIELD_SHORT),
      DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(numFrames, FIELD_SHORT),
...
BEGIN_BYTESWAP_DATADESC_(VTFFileHeaderX360_t, VTFFileBaseHeader_t)
  DEFINE_FIELD(flags, FIELD_INTEGER), DEFINE_FIELD(width, FIELD_SHORT),
      DEFINE_FIELD(height, FIELD_SHORT), DEFINE_FIELD(depth, FIELD_SHORT),
...
void S3TC_MergeBlocks(char **blocks, S3RGBA **pOriginals, int nBlocks,
                      int lPitch,  // (in BYTES)
                      ImageFormat format) {}
...
// We use this to match the verts and determine which edges need to be blended
// together. The vert ordering is lower-left, top-left, top-right, bottom-right.
...
int *g_FaceVerts[6] = {g_rightFaceVerts, g_leftFaceVerts, g_backFaceVerts,
                       g_frontFaceVerts, g_upFaceVerts,   g_downFaceVerts};
...
int *g_skybox_FaceVerts[6] = {g_skybox_rightFaceVerts, g_skybox_leftFaceVerts,
                              g_skybox_backFaceVerts,  g_skybox_frontFaceVerts,
                              g_skybox_upFaceVerts,    g_skybox_downFaceVerts};

//-----------------------------------------------------------------------------
// Class factory
//-----------------------------------------------------------------------------
IVTFTexture *CreateVTFTexture() { return new CVTFTexture; }
  ...
  switch (nMajorVersion) {
    case VTF_MAJOR_VERSION:
      switch (nMinorVersion) {
        ...
        case 3:
          return sizeof(VTFFileHeaderV7_3_t) +
                 sizeof(ResourceEntryInfo) * MAX_RSRC_DICTIONARY_ENTRIES;
    ...
    case VTF_X360_MAJOR_VERSION:
      return sizeof(VTFFileHeaderX360_t) +
             sizeof(ResourceEntryInfo) * MAX_X360_RSRC_DICTIONARY_ENTRIES;
  }
...
CVTFTexture::~CVTFTexture() { Shutdown(); }
...
static void GenericAllocateReusableData(unsigned char **ppData,
                                        int *pNumAllocated, int numRequested) {
...
void CVTFTexture::AllocateLowResImageData(int nMemorySize) {
  GenericAllocateReusableData(&m_pLowResImageData, &m_nLowResImageAllocSize,
                              nMemorySize);
}
...
// Initialization
//-----------------------------------------------------------------------------
bool CVTFTexture::Init(int nWidth, int nHeight, int nDepth, ImageFormat fmt,
                       int iFlags, int iFrameCount, int nForceMipCount) {
  if (nDepth == 0) {
    nDepth = 1;
  }
  ...
  if (!IsMultipleOf4(nWidth) || !IsMultipleOf4(nHeight) ||
      !IsMultipleOf4(nDepth)) {
  ...
  // THIS CAUSED A BUG!!!  We want all of the mip levels in the vtf file even
  // with nomip in case we have lod. NOTE: But we don't want more than 1 mip
  // level for procedural textures
  if ((iFlags & (TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_PROCEDURAL)) ==
      (TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_PROCEDURAL)) {
    nForceMipCount = 1;
  }
...
void CVTFTexture::InitLowResImage(int nWidth, int nHeight, ImageFormat fmt) {
  ...
  int iLowResImageSize = ImageLoader::GetMemRequired(
      m_nLowResImageWidth, m_nLowResImageHeight, 1, m_LowResImageFormat, false);
...
//-----------------------------------------------------------------------------
void CVTFTexture::SetBumpScale(float flScale) { m_flBumpScale = flScale; }
...
void CVTFTexture::ReleaseResources() {
  ...
  for (ResourceMemorySection *pRms = m_arrResourcesData.Base(),
                             *pRmsEnd = pRms + m_arrResourcesData.Count();
       pRms < pRmsEnd; ++pRms) {
    delete[] pRms->m_pData;
  }
  m_arrResourcesData.RemoveAll();

  for (ResourceMemorySection *
           pRms = m_arrResourcesData_ForReuse.Base(),
          *pRmsEnd = pRms + m_arrResourcesData_ForReuse.Count();
       pRms < pRmsEnd; ++pRms) {
    delete[] pRms->m_pData;
  }
...
#if defined(_X360)
  // must be first to ensure X360 aliased pointers are unhooked, otherwise
  // memory corruption
  ReleaseImageMemory();
#endif
...
void CVTFTexture::LowResFileInfo(int *pStartLocation, int *pSizeInBytes) const {
  ...
  if (ResourceEntryInfo const *pLowResData =
          FindResourceEntryInfo(VTF_LEGACY_RSRC_LOW_RES_IMAGE)) {
    *pStartLocation = pLowResData->resData;
    *pSizeInBytes =
        ImageLoader::GetMemRequired(m_nLowResImageWidth, m_nLowResImageHeight,
                                    1, m_LowResImageFormat, false);
  } else {
    *pStartLocation = 0;
    *pSizeInBytes = 0;
  }
...
void CVTFTexture::ImageFileInfo(int nFrame, int nFace, int nMipLevel,
  ...
  ResourceEntryInfo const *pImageDataInfo =
      FindResourceEntryInfo(VTF_LEGACY_RSRC_IMAGE);
  ...
  for (i = m_nMipCount - 1; i > nMipLevel; --i) {
    ...
    int iMipLevelSize = ImageLoader::GetMemRequired(iMipWidth, iMipHeight,
                                                    iMipDepth, m_Format, false);
  ...
  int nFaceSize = ImageLoader::GetMemRequired(iMipWidth, iMipHeight, iMipDepth,
                                              m_Format, false);
...
bool CVTFTexture::LoadLowResData(CUtlBuffer &buf) {
  // Allocate low-res bits
  InitLowResImage(m_nLowResImageWidth, m_nLowResImageHeight,
                  m_LowResImageFormat);
  int nLowResImageSize = ImageLoader::GetMemRequired(
      m_nLowResImageWidth, m_nLowResImageHeight, 1, m_LowResImageFormat, false);
...
//-----------------------------------------------------------------------------
bool CVTFTexture::LoadImageData(CUtlBuffer &buf, const VTFFileHeader_t &header,
                                int nSkipMipLevels) {
  ...
  int nFacesToRead = m_nFaceCount;
  if (IsCubeMap()) {
    if ((header.version[0] == 7) && (header.version[1] < 1)) nFacesToRead = 6;
  }

  AllocateImageData(iImageSize);

  // NOTE: The mip levels are stored ascending from smallest (1x1) to largest
  // (NxN) in order to allow for truncated reads of the minimal required data
  for (int iMip = m_nMipCount; --iMip >= 0;) {
    // NOTE: This is for older versions...
    if (header.numMipLevels - nSkipMipLevels <= iMip) continue;
...
void *CVTFTexture::SetResourceData(uint32_t eType, void const *pData,
                                   size_t nNumBytes) {
  Assert((eType & RSRCF_MASK) == 0);
  eType &= ~RSRCF_MASK;

  // Very inefficient to set less than 4 bytes of data
  Assert(!nNumBytes || (nNumBytes >= sizeof(uint32_t)));
  ...
  if (nNumBytes) {
    ...
    if (nNumBytes == sizeof(pInfo->resData)) {
      ...
      if (pData) pInfo->resData = reinterpret_cast<const int *>(pData)[0];
      return &pInfo->resData;
    } else {
      rms.AllocateData(nNumBytes);
      if (pData) memcpy(rms.m_pData, pData, nNumBytes);
      return rms.m_pData;
    }
  } else {
...
void *CVTFTexture::GetResourceData(uint32_t eType, size_t *pDataSize) const {
  Assert((eType & RSRCF_MASK) == 0);
  eType &= ~RSRCF_MASK;
...
bool CVTFTexture::HasResourceEntry(uint32_t eType) const {
  return (FindResourceEntryInfo(eType) != NULL);
}

unsigned int CVTFTexture::GetResourceTypes(unsigned int *arrTypesBuffer,
...
// Unserialization
//-----------------------------------------------------------------------------
static bool ReadHeaderFromBufferPastBaseHeader(CUtlBuffer &buf,
                                               VTFFileHeader_t &header) {
  unsigned char *pBuf =
      (unsigned char *)(&header) + sizeof(VTFFileBaseHeader_t);
...
// Unserialization
//-----------------------------------------------------------------------------
bool CVTFTexture::Unserialize(CUtlBuffer &buf, bool bHeaderOnly,
                              int nSkipMipLevels) {
  ...
  if (int(header.numResources) < m_arrResourcesData.Count()) {
    m_arrResourcesData_ForReuse.EnsureCapacity(
        m_arrResourcesData_ForReuse.Count() + m_arrResourcesData.Count() -
        header.numResources);
    for (ResourceMemorySection const *
             pRms = &m_arrResourcesData[header.numResources],
            *pRmsEnd = m_arrResourcesData.Base() + m_arrResourcesData.Count();
         pRms < pRmsEnd; ++pRms) {
      if (pRms->m_pData) {
        int idxReuse = m_arrResourcesData_ForReuse.AddToTail(*pRms);
        m_arrResourcesData_ForReuse[idxReuse].m_nDataLength =
            0;  // Data for reuse shouldn't have length set
      }
    }
  }
  ...
  if (header.numResources > 0) {
    ...
    buf.Get(m_arrResourcesInfo.Base(),
            m_arrResourcesInfo.Count() * sizeof(ResourceEntryInfo));
  ...
  // Caller wants the header component only, avoids reading large image data
  // sets
  if (bHeaderOnly) return true;

  // Load the low res image
  if (ResourceEntryInfo const *pLowResDataInfo =
          FindResourceEntryInfo(VTF_LEGACY_RSRC_LOW_RES_IMAGE)) {
    buf.SeekGet(CUtlBuffer::SEEK_HEAD, pLowResDataInfo->resData);
    if (!LoadLowResData(buf)) return false;
  }
  ...
  // Load the image data
  if (ResourceEntryInfo const *pImageDataInfo =
          FindResourceEntryInfo(VTF_LEGACY_RSRC_IMAGE)) {
    buf.SeekGet(CUtlBuffer::SEEK_HEAD, pImageDataInfo->resData);
    if (!LoadImageData(buf, header, nSkipMipLevels)) return false;
  } else {
    // No image data
    return false;
  }
    ...
    if ((rei.eType & RSRCF_HAS_NO_DATA_CHUNK) == 0) {
      switch (rei.eType) {
        ...
        default:
          ...
          if (!rms.LoadData(buf, m_Swap)) return false;
      }
    }
  }
...
ResourceEntryInfo const *CVTFTexture::FindResourceEntryInfo(
  ...
  find_routine:
    ...
    for (; pRange[0] < pRange[1]; ++pRange[0]) {
      if ((pRange[0]->eType & ~RSRCF_MASK) == eType) return pRange[0];
    }
  }
...
ResourceEntryInfo *CVTFTexture::FindResourceEntryInfo(uint32_t eType) {
  return const_cast<ResourceEntryInfo *>(
      ((CVTFTexture const *)this)->FindResourceEntryInfo(eType));
}

ResourceEntryInfo *CVTFTexture::FindOrCreateResourceEntryInfo(uint32_t eType) {
  Assert((eType & RSRCF_MASK) == 0);

  int k = 0;
  for (; k < m_arrResourcesInfo.Count(); ++k) {
    uint32_t rsrcType = (m_arrResourcesInfo[k].eType & ~RSRCF_MASK);
    if (rsrcType == eType) {
      // found
  ...
  if (m_arrResourcesData_ForReuse.Count()) {
    m_arrResourcesData.InsertBefore(
        k,
        m_arrResourcesData_ForReuse[m_arrResourcesData_ForReuse.Count() - 1]);
    m_arrResourcesData_ForReuse.FastRemove(m_arrResourcesData_ForReuse.Count() -
                                           1);
  } else {
...
bool CVTFTexture::RemoveResourceEntryInfo(uint32_t eType) {
  Assert((eType & RSRCF_MASK) == 0);
  ...
  for (int k = 0; k < m_arrResourcesInfo.Count(); ++k) {
    if ((m_arrResourcesInfo[k].eType & ~RSRCF_MASK) == eType) {
      ...
      if (m_arrResourcesData[k].m_pData) {
        int idxReuse =
            m_arrResourcesData_ForReuse.AddToTail(m_arrResourcesData[k]);
        m_arrResourcesData_ForReuse[idxReuse].m_nDataLength =
            0;  // Data for reuse shouldn't have length set
      }
...
bool CVTFTexture::Serialize(CUtlBuffer &buf) {
  ...
  for (int iRsrc = 0; iRsrc < m_arrResourcesInfo.Count(); ++iRsrc) {
    ...
    switch (rei.eType) {
      ...
      default: {
        if (rei.eType & RSRCF_HAS_NO_DATA_CHUNK) continue;
        rei.resData = buf.TellPut();
        ResourceMemorySection &rms = m_arrResourcesData[iRsrc];
        if (!rms.WriteData(buf)) return false;
      }
    }
  }
...
// Attributes...
//-----------------------------------------------------------------------------
int CVTFTexture::Width() const { return m_nWidth; }

int CVTFTexture::Height() const { return m_nHeight; }

int CVTFTexture::Depth() const { return m_nDepth; }

int CVTFTexture::MipCount() const { return m_nMipCount; }

ImageFormat CVTFTexture::Format() const { return m_Format; }

int CVTFTexture::FaceCount() const { return m_nFaceCount; }

int CVTFTexture::FrameCount() const { return m_nFrameCount; }

int CVTFTexture::Flags() const { return m_nFlags; }
...
bool CVTFTexture::IsVolumeTexture() const { return (m_nDepth > 1); }

float CVTFTexture::BumpScale() const { return m_flBumpScale; }

const Vector &CVTFTexture::Reflectivity() const { return m_vecReflectivity; }

unsigned char *CVTFTexture::ImageData() { return m_pImageData; }

int CVTFTexture::LowResWidth() const { return m_nLowResImageWidth; }

int CVTFTexture::LowResHeight() const { return m_nLowResImageHeight; }

ImageFormat CVTFTexture::LowResFormat() const { return m_LowResImageFormat; }

unsigned char *CVTFTexture::LowResImageData() { return m_pLowResImageData; }
...
unsigned char *CVTFTexture::ImageData(int iFrame, int iFace, int iMipLevel,
  ...
  Assert((x >= 0) && (x <= nWidth) && (y >= 0) && (y <= nHeight) && (z >= 0) &&
         (z <= nDepth));
#endif
...
//-----------------------------------------------------------------------------
// Computes the size (in bytes) of a single mipmap of a single face of a single
// frame
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
inline int CVTFTexture::ComputeFaceSize(int iStartingMipLevel,
...
//-----------------------------------------------------------------------------
int CVTFTexture::GetImageOffset(int iFrame, int iFace, int iMipLevel,
                                ImageFormat fmt) const {
...
//-----------------------------------------------------------------------------
void CVTFTexture::ComputeMipLevelDimensions(int iMipLevel, int *pMipWidth,
                                            int *pMipHeight,
                                            int *pMipDepth) const {
  Assert(iMipLevel < m_nMipCount);
...
//-----------------------------------------------------------------------------
void CVTFTexture::ComputeMipLevelSubRect(Rect_t *pSrcRect, int nMipLevel,
  ...
  pSubRect->width =
      (int)ceil((pSrcRect->x + pSrcRect->width) * flInvShrink) - pSubRect->x;
  pSubRect->height =
      (int)ceil((pSrcRect->y + pSrcRect->height) * flInvShrink) - pSubRect->y;
}
...
void CVTFTexture::ConvertImageFormat(ImageFormat fmt, bool bNormalToDUDV) {
  ...
  if (bNormalToDUDV &&
      !(fmt == IMAGE_FORMAT_UV88 || fmt == IMAGE_FORMAT_UVWQ8888 ||
        fmt == IMAGE_FORMAT_UVLX8888)) {
    Assert(0);
    return;
  }
  ...
  unsigned char *pConvertedImage =
      (unsigned char *)MemAllocScratch(iConvertedSize);
  for (int iMip = 0; iMip < m_nMipCount; ++iMip) {
    ...
    int nSrcFaceStride =
        ImageLoader::GetMemRequired(nMipWidth, nMipHeight, 1, m_Format, false);
    int nDstFaceStride =
        ImageLoader::GetMemRequired(nMipWidth, nMipHeight, 1, fmt, false);

    for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
      for (int iFace = 0; iFace < m_nFaceCount; ++iFace) {
        unsigned char *pSrcData = ImageData(iFrame, iFace, iMip);
        unsigned char *pDstData =
            pConvertedImage + GetImageOffset(iFrame, iFace, iMip, fmt);

        for (int z = 0; z < nMipDepth;
             ++z, pSrcData += nSrcFaceStride, pDstData += nDstFaceStride) {
          if (bNormalToDUDV) {
            if (fmt == IMAGE_FORMAT_UV88) {
              ImageLoader::ConvertNormalMapRGBA8888ToDUDVMapUV88(
                  pSrcData, nMipWidth, nMipHeight, pDstData);
            } else if (fmt == IMAGE_FORMAT_UVWQ8888) {
              ImageLoader::ConvertNormalMapRGBA8888ToDUDVMapUVWQ8888(
                  pSrcData, nMipWidth, nMipHeight, pDstData);
            ...
            ImageLoader::ConvertImageFormat(pSrcData, m_Format, pDstData, fmt,
                                            nMipWidth, nMipHeight);
          }
        }
      }
    }
  ...
  if (!ImageLoader::IsCompressed(fmt)) {
    ...
    if ((fmt == IMAGE_FORMAT_DXT1) || (fmt == IMAGE_FORMAT_ATI2N) ||
...
//
//-----------------------------------------------------------------------------
static void CalcInit(SphereCalc_t *pCalc, int iSize,
                     unsigned char **ppCubeFaces,
...
static int CalcFaceIndex(const Vector &normal) {
  ...
  if (absx > absy) {
    if (absx > absz) {
      // left/right
      if (normal[0] >= 0) return CUBEMAP_FACE_RIGHT;
      return CUBEMAP_FACE_LEFT;
    }
  } else {
    if (absy > absz) {
      // front / back
      if (normal[1] >= 0) return CUBEMAP_FACE_BACK;
      return CUBEMAP_FACE_FRONT;
    }
  }

  // top / bottom
  if (normal[2] >= 0) return CUBEMAP_FACE_UP;
  return CUBEMAP_FACE_DOWN;
}

static void CalcColor(SphereCalc_t *pCalc, int iFace, const Vector &normal,
                      unsigned char *color) {
  float x, y, w;
  ...
  if ((iFace == CUBEMAP_FACE_LEFT) || (iFace == CUBEMAP_FACE_RIGHT)) {
    ...
    x = -normal[2];
    y = -normal[1];
    if (iFace == CUBEMAP_FACE_LEFT) y = -y;
  } else if ((iFace == CUBEMAP_FACE_FRONT) || (iFace == CUBEMAP_FACE_BACK)) {
    w = hw / normal[1];
    x = normal[0];
    y = normal[2];
    if (iFace == CUBEMAP_FACE_FRONT) x = -x;
  } else {
    w = hw / normal[2];
    x = -normal[0];
    y = -normal[1];
    if (iFace == CUBEMAP_FACE_UP) x = -x;
  }
...
//-----------------------------------------------------------------------------
void CVTFTexture::ComputeSpheremapFrame(unsigned char **ppCubeFaces,
                                        unsigned char *pSpheremap,
                                        LookDir_t lookDir) {
  SphereCalc_t sphere;
...
void CVTFTexture::ComputeHemispheremapFrame(unsigned char **ppCubeFaces,
                                            unsigned char *pSpheremap,
                                            LookDir_t lookDir) {
  SphereCalc_t sphere;
  CalcInit(&sphere, m_nWidth, ppCubeFaces, lookDir);
...
void CVTFTexture::GenerateSpheremap(LookDir_t lookDir) {
  ...
  unsigned char *pSphereMapBits =
      (unsigned char *)MemAllocScratch(iMemRequired);
  ...
  for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
    ...
    // Compute the mip levels of the spheremap, converting from RGBA8888 to our
    // format
    unsigned char *pFinalSphereMapBits =
...
void CVTFTexture::GenerateHemisphereMap(unsigned char *pSphereMapBitsRGBA,
...
// format from DX8.
//-----------------------------------------------------------------------------
static void FixCubeMapFacing(unsigned char *pImage, int cubeFaceID, int size,
                             ImageFormat fmt) {
  int retVal;
  switch (cubeFaceID) {
    case CUBEMAP_FACE_RIGHT:  // +x
      ...
      Assert(retVal);
      retVal =
          ImageLoader::FlipImageVertically(pImage, pImage, size, size, fmt);
      Assert(retVal);
      break;
    ...
    case CUBEMAP_FACE_LEFT:  // -x
      ...
      Assert(retVal);
      retVal =
          ImageLoader::FlipImageHorizontally(pImage, pImage, size, size, fmt);
      Assert(retVal);
      break;
...
//-----------------------------------------------------------------------------
// Fixes the cubemap faces orientation from our standard to what the material
// system needs
//-----------------------------------------------------------------------------
void CVTFTexture::FixCubemapFaceOrientation() {
  ...
  for (int iMipLevel = 0; iMipLevel < m_nMipCount; ++iMipLevel) {
    ...
    for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
      for (int iFace = 0; iFace < 6; ++iFace) {
        FixCubeMapFacing(ImageData(iFrame, iFace, iMipLevel), iFace, iMipSize,
                         m_Format);
      }
    }
  }
...
void CVTFTexture::NormalizeTopMipLevel() {
  if (!(m_nFlags & TEXTUREFLAGS_NORMAL)) return;
  ...
  for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
    for (int iFace = 0; iFace < m_nFaceCount; ++iFace) {
      ...
      ImageLoader::NormalizeNormalMapRGBA8888(
          pSrcLevel, nSrcWidth * nSrcHeight * nSrcDepth);
    }
  }
}
...
void CVTFTexture::GenerateMipmaps() {
  ...
  //	if( ( Flags() & ( TEXTUREFLAGS_NOMIP | TEXTUREFLAGS_NOLOD ) ) == (
  ...
  info.m_nFlags = 0;
  bool bNormalMap = (Flags() & TEXTUREFLAGS_NORMAL) ||
                    (m_Options.flags0 & VtfProcessingOptions::OPT_NORMAL_DUDV);
  bool bAlphaTest =
      ((m_Options.flags0 & VtfProcessingOptions::OPT_MIP_ALPHATEST) != 0);
  ...
  for (int iMipLevel = 1; iMipLevel < m_nMipCount; ++iMipLevel) {
    ComputeMipLevelDimensions(iMipLevel, &info.m_nDestWidth,
                              &info.m_nDestHeight, &info.m_nDestDepth);
    ...
    if (nSrcMipLevel < 0) nSrcMipLevel = 0;

    // Decay options
    bool bMipBlendActive = false;
    char chChannels[4] = {'R', 'G', 'B', 'A'};
    for (int ch = 0; ch < 4; ++ch) {
      int iLastNonDecayMip =
          numMipsClampedLod + int(m_Options.numNotDecayMips[ch]);
      if (iLastNonDecayMip > m_nMipCount) iLastNonDecayMip = m_nMipCount - 1;
      int numDecayMips = m_nMipCount - iLastNonDecayMip - 1;
      if (numDecayMips < 1) numDecayMips = 1;

      // Decay is only active starting from numDecayMips
      if (!(((iMipLevel == m_nMipCount - 1) ||
             (iMipLevel > iLastNonDecayMip)) &&  // last 1x1 mip  or  past
                                                 // clamped and skipped
            (m_Options.flags0 & (VtfProcessingOptions::OPT_DECAY_R
                                 << ch))))  // the channel has decay
        continue;
      ...
      } else if (m_Options.flags0 &
                 (VtfProcessingOptions::OPT_DECAY_EXP_R << ch)) {
        info.m_flColorScale[ch] =
            pow(m_Options.fDecayExponentBase[ch], iMipLevel - iLastNonDecayMip);
      } else {
        info.m_flColorScale[ch] =
            1.0f - float(iMipLevel - iLastNonDecayMip) / float(numDecayMips);
      }

      if (!bMipBlendActive) {
        bMipBlendActive = true;
        printf("Blending mip%d %dx%d to", iMipLevel, info.m_nDestWidth,
               info.m_nDestHeight);
      }

      printf("   %c=%d ~%d%%", chChannels[ch], m_Options.clrDecayGoal[ch],
             int((1.f - info.m_flColorScale[ch]) * 100.0f + 0.5f));
    }
    if (bMipBlendActive) printf("\n");

    if (bNormalMap) {
      info.m_nFlags |= ImageLoader::RESAMPLE_NORMALMAP;
      // Normal maps xyz decays to 127.f
      for (int ch = 0; ch < 3; ++ch) info.m_flColorGoal[ch] = 127.0f;
    }
    ...
    for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
      for (int iFace = 0; iFace < m_nFaceCount; ++iFace) {
        ...
        ComputeMipLevelDimensions(nSrcMipLevel, &info.m_nSrcWidth,
                                  &info.m_nSrcHeight, &info.m_nSrcDepth);
        ...
        if (Flags() & TEXTUREFLAGS_NORMAL) {
          ImageLoader::NormalizeNormalMapRGBA8888(
              pDstLevel,
              info.m_nDestWidth * info.m_nDestHeight * info.m_nDestDepth);
        }
      }
    }
  }
...
void CVTFTexture::PutOneOverMipLevelInAlpha() {
  ...
  for (int iMipLevel = 0; iMipLevel < m_nMipCount; ++iMipLevel) {
    ...
    unsigned char ooMipLevel =
        (unsigned char)(255.0f * (1.0f / (float)(1 << iMipLevel)));
...
void CVTFTexture::PostProcess(bool bGenerateSpheremap, LookDir_t lookDir,
                              bool bAllowFixCubemapOrientation) {
  // HDRFIXME: Make sure that all of the below functions check for the proper
  // formats if we get rid of this assert.
  //	Assert( m_Format == IMAGE_FORMAT_RGBA8888 );
  ...
  if (IsCubeMap()) {
    ...
    // Compute the spheremap fallback for cubemaps if we weren't able to load up
    // one...
    if (bGenerateSpheremap) GenerateSpheremap(lookDir);
  }
...
bool CVTFTexture::ConstructLowResImage() {
  ...
  lowResSizeImage.EnsureCapacity(m_nLowResImageWidth * m_nLowResImageHeight *
                                 4);
  ...
  if (!ImageLoader::ResampleRGBA8888(info)) return false;

  // convert to the low-res size version with the correct image format
  unsigned char *tmpImage = lowResSizeImage.Base();
  return ImageLoader::ConvertImageFormat(
      tmpImage, IMAGE_FORMAT_RGBA8888, m_pLowResImageData, m_LowResImageFormat,
      m_nLowResImageWidth, m_nLowResImageHeight);
}
...
void CVTFTexture::SetupEdgeIncrement(CEdgePos &start, CEdgePos &end,
                                     CEdgePos &inc) {
  inc.x = inc.y = 0;
  if (start.x != end.x) {
...
void CVTFTexture::SetupTextureEdgeIncrements(int iMipLevel, int iFace1Edge,
                                             int iFace2Edge,
                                             bool bFlipFace2Edge,
...
void CVTFTexture::BlendCubeMapFaceEdges(int iFrame, int iMipLevel,
                                        const CEdgeMatch *pMatch) {
  int nMipWidth, nMipHeight, nMipDepth;
  ComputeMipLevelDimensions(iMipLevel, &nMipWidth, &nMipHeight, &nMipDepth);
  Assert(nMipDepth == 1);
  if (nMipWidth <= 1 || nMipHeight <= 1) return;
  ...
  CEdgeIncrements incs;
  SetupTextureEdgeIncrements(iMipLevel, pMatch->m_iEdges[0],
                             pMatch->m_iEdges[1], pMatch->m_bFlipFace2Edge,
                             &incs);

  // Do all pixels but the first and the last one (those will be handled when
  // blending corners).
  ...
  if (m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5) {
    if (iFace1Cur != incs.iFace1End) {
      while (iFace1Cur != incs.iFace1End) {
        ...
        S3PaletteIndex paletteIndex = S3TC_GetPaletteIndex(
            pFace1Data, m_Format, nMipWidth, iFace1Cur.x, iFace1Cur.y);
        S3TC_SetPaletteIndex(pFace2Data, m_Format, nMipWidth, iFace2Cur.x,
                             iFace2Cur.y, paletteIndex);
    ...
    if (iFace1Cur != incs.iFace1End) {
      while (iFace1Cur != incs.iFace1End) {
        // Now we know the 2 pixels. Average them and copy the averaged value to
        // both pixels.
    ...
    Error("BlendCubeMapFaceEdges: unsupported image format (%d)",
          (int)m_Format);
  }
}

void CVTFTexture::BlendCubeMapFaceCorners(int iFrame, int iMipLevel,
  ...
  if (m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5) {
    if (nMipWidth < 4 || nMipHeight < 4) return;

    // Copy the first palette index to the other blocks.
    S3PaletteIndex paletteIndex = S3TC_GetPaletteIndex(
        pImageData[0], m_Format, nMipWidth, texelPos[0].x, texelPos[0].y);
    S3TC_SetPaletteIndex(pImageData[1], m_Format, nMipWidth, texelPos[1].x,
                         texelPos[1].y, paletteIndex);
    S3TC_SetPaletteIndex(pImageData[2], m_Format, nMipWidth, texelPos[2].x,
                         texelPos[2].y, paletteIndex);
  } else if (m_Format == IMAGE_FORMAT_RGBA8888) {
...
void CVTFTexture::BuildCubeMapMatchLists(
    ...
    CCornerMatch cornerMatches[NUM_CORNER_MATCHES], bool bSkybox) {
  ...
  for (int iFace = 0; iFace < 6; iFace++) {
    for (int iEdge = 0; iEdge < 4; iEdge++) {
      ...
      // Only look for faces with indices < what we have so we don't do each
      // edge twice.
...
void CVTFTexture::BlendCubeMapEdgePalettes(int iFrame, int iMipLevel,
                                           const CEdgeMatch *pMatch) {
  ...
  ComputeMipLevelDimensions(iMipLevel, &nMipWidth, &nMipHeight, &nMipDepth);
  Assert(nMipDepth == 1);
  if (nMipWidth <= 8 || nMipHeight <= 8) return;

  unsigned char *pFace1Data = ImageData(iFrame, pMatch->m_iFaces[0], iMipLevel);
  unsigned char *pFace2Data = ImageData(iFrame, pMatch->m_iFaces[1], iMipLevel);
  S3RGBA *pFace1Original =
      &m_OriginalData[GetImageOffset(iFrame, pMatch->m_iFaces[0], iMipLevel,
                                     IMAGE_FORMAT_RGBA8888) /
                      4];
  S3RGBA *pFace2Original =
      &m_OriginalData[GetImageOffset(iFrame, pMatch->m_iFaces[1], iMipLevel,
                                     IMAGE_FORMAT_RGBA8888) /
                      4];
  ...
  CEdgePos iFace1Cur = incs.iFace1Start + incs.iFace1Inc;
  CEdgePos iFace2Cur = incs.iFace2Start + incs.iFace2Inc;
  while (iFace1Cur != incs.iFace1End)  // We intentionally want to not process
                                       // the last block here..
  {
    // Merge the palette of these two blocks.
    char *blocks[2] = {S3TC_GetBlock(pFace1Data, m_Format, nMipWidth >> 2,
                                     iFace1Cur.x, iFace1Cur.y),
                       S3TC_GetBlock(pFace2Data, m_Format, nMipWidth >> 2,
                                     iFace2Cur.x, iFace2Cur.y)};

    S3RGBA *originals[2] = {
        &pFace1Original[(iFace1Cur.y * 4 * nMipWidth) + iFace1Cur.x * 4],
        &pFace2Original[(iFace2Cur.y * 4 * nMipWidth) + iFace2Cur.x * 4]};

    S3TC_MergeBlocks(blocks, originals, 2, nMipWidth * 4, m_Format);
...
void CVTFTexture::BlendCubeMapCornerPalettes(int iFrame, int iMipLevel,
                                             const CCornerMatch *pMatch) {
  int nMipWidth, nMipHeight, nMipDepth;
  ComputeMipLevelDimensions(iMipLevel, &nMipWidth, &nMipHeight, &nMipDepth);
  Assert(nMipDepth == 1);
  ...
  for (int iEdge = 0; iEdge < 3; iEdge++) {
    ...
    S3RGBA *pFaceOriginal =
        &m_OriginalData[GetImageOffset(iFrame, iFaceIndex, iMipLevel,
                                       IMAGE_FORMAT_RGBA8888) /
                        4];

    blocks[iEdge] = S3TC_GetBlock(pFaceData, m_Format, nMipWidth >> 2,
                                  facePos.x, facePos.y);
    originals[iEdge] =
        &pFaceOriginal[(facePos.y * 4 * nMipWidth) + facePos.x * 4];
  }

  S3TC_MergeBlocks(blocks, originals, 3, nMipWidth * 4, m_Format);
}
...
void CVTFTexture::MatchCubeMapS3TCPalettes(
  ...
  for (int iMipLevel = 0; iMipLevel < m_nMipCount; ++iMipLevel) {
    for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
      // First, match all the edge palettes (this part skips the first and last
      // 4 texels along the edge since those S3 blocks are handled in the corner
      // case).
      for (int iEdgeMatch = 0; iEdgeMatch < NUM_EDGE_MATCHES; iEdgeMatch++) {
        BlendCubeMapEdgePalettes(iFrame, iMipLevel, &edgeMatches[iEdgeMatch]);
      }

      for (int iCornerMatch = 0; iCornerMatch < NUM_CORNER_MATCHES;
           iCornerMatch++) {
        BlendCubeMapCornerPalettes(iFrame, iMipLevel,
...
void CVTFTexture::MatchCubeMapBorders(int iStage, ImageFormat finalFormat,
  ...
  if (iStage == 1) {
    // Stage 1 is while the image is still RGBA8888. If we're not going to S3
    // compress the image, then it is easiest to match the borders now.
    Assert(m_Format == IMAGE_FORMAT_RGBA8888);
    if (finalFormat == IMAGE_FORMAT_DXT1 || finalFormat == IMAGE_FORMAT_DXT5) {
      // If we're going to S3 compress the image eventually, then store off the
      // original version because we can use that while matching the S3
      // compressed edges (we have to do some tricky repalettizing).
      int nTotalBytes = ComputeTotalSize();
      m_OriginalData.SetSize(nTotalBytes / 4);
      memcpy(m_OriginalData.Base(), ImageData(), nTotalBytes);

      // Swap R and B in these because IMAGE_FORMAT_RGBA8888 is swapped from the
      // way S3RGBAs are.
    ...
    if (finalFormat == IMAGE_FORMAT_DXT1 || finalFormat == IMAGE_FORMAT_DXT5) {
      ...
      // If we're not winding up S3 compressed, then we already fixed the
      // cubemap borders.
      return;
    }
  }
  ...
  // If we're S3 compressed, then during the first pass, we need to match the
  // palettes of all bordering S3 blocks.
  if (m_Format == IMAGE_FORMAT_DXT1 || m_Format == IMAGE_FORMAT_DXT5) {
  ...
  for (int iMipLevel = 0; iMipLevel < m_nMipCount; ++iMipLevel) {
    for (int iFrame = 0; iFrame < m_nFrameCount; ++iFrame) {
      for (int iEdgeMatch = 0; iEdgeMatch < NUM_EDGE_MATCHES; iEdgeMatch++) {
        BlendCubeMapFaceEdges(iFrame, iMipLevel, &edgeMatches[iEdgeMatch]);
      }

      for (int iCornerMatch = 0; iCornerMatch < NUM_CORNER_MATCHES;
           iCornerMatch++) {
        BlendCubeMapFaceCorners(iFrame, iMipLevel,
                                &cornerMatches[iCornerMatch]);
      }
    }
  }
