...
// Output : int
//-----------------------------------------------------------------------------
int CSoundEmitterSystemBase::First() const { return 0; }
...
void CSoundEmitterSystemBase::Disconnect() { filesystem = NULL; }
...
// Init, shutdown
//-----------------------------------------------------------------------------
InitReturnVal_t CSoundEmitterSystemBase::Init() { return INIT_OK; }

void CSoundEmitterSystemBase::Shutdown() {}

//-----------------------------------------------------------------------------
// Purpose: Helper for checksuming script files and manifest to determine if
// soundname caches
...
// Output : static void
//-----------------------------------------------------------------------------
static void AccumulateFileNameAndTimestampIntoChecksum(CRC32_t *crc,
                                                       char const *filename) {
...
bool CSoundEmitterSystemBase::ModInit() {
  ...
  */
  ...
  if (filesystem->LoadKeyValues(*manifest, IFileSystem::TYPE_SOUNDEMITTER,
                                MANIFEST_FILE, "GAME")) {
    AccumulateFileNameAndTimestampIntoChecksum(&crc, MANIFEST_FILE);

    for (KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL;
         sub = sub->GetNextKey()) {
      ...
      Warning(
          "CSoundEmitterSystemBase::BaseInit:  Manifest '%s' with bogus file "
          "type '%s', expecting 'declare_file' or 'precache_file'\n",
...
int CSoundEmitterSystemBase::GetSoundIndex(const char *pName) const {
  ...
  if (idx == m_Sounds.InvalidIndex()) return -1;
...
// Output : int
//-----------------------------------------------------------------------------
int CSoundEmitterSystemBase::GetSoundCount(void) { return m_Sounds.Count(); }

void CSoundEmitterSystemBase::EnsureAvailableSlotsForGender(
    SoundFile *pSoundnames, int c, gender_t gender) {
  int i;
  if (c <= 0) {
    return;
  }
  ...
  for (i = 0; i < c; i++) {
    if (pSoundnames[i].gender != gender) continue;

    // There was at least one match for the gender
    needsreset = true;

    // This sound is unavailable
    if (!pSoundnames[i].available) continue;
...
//			soundnames -
//-----------------------------------------------------------------------------
int CSoundEmitterSystemBase::FindBestSoundForGender(SoundFile *pSoundnames,
                                                    int c, gender_t gender) {
  ...
  for (int i = 0; i < c; i++) {
    if (pSoundnames[i].gender == gender && pSoundnames[i].available) {
      slots.AddToTail(i);
    }
  }
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSoundEmitterSystemBase::GetParametersForSound(
    const char *soundname, CSoundParameters &params, gender_t gender,
    bool isbeingemitted /*= false*/) {
  ...
  if (index == SOUNDEMITTER_INVALID_HANDLE) {
    ...
    if (UTL_INVAL_SYMBOL == soundWarnings.Find(key)) {
      ...
      DevMsg(
          "CSoundEmitterSystemBase::GetParametersForSound:  No such sound %s\n",
          soundname);
    }
    return false;
  }

  return GetParametersForSoundEx(soundname, index, params, gender,
                                 isbeingemitted);
}

CSoundParametersInternal *
CSoundEmitterSystemBase::InternalGetParametersForSound(int index) {
...
static void SplitName(char const *input, int splitchar, int splitlen,
                      char *before, int beforelen, char *after, int afterlen) {
  char const *in = input;
  char *out = before;
...
//			gender -
//-----------------------------------------------------------------------------
void CSoundEmitterSystemBase::AddSoundName(CSoundParametersInternal &params,
                                           char const *wavename,
                                           gender_t gender) {
...
//			*wavename -
//-----------------------------------------------------------------------------
void CSoundEmitterSystemBase::ExpandSoundNameMacros(
    CSoundParametersInternal &params, char const *wavename) {
  ...
  SplitName(wavename, offset, duration, before, sizeof(before), after,
            sizeof(after));
...
void CSoundEmitterSystemBase::GenderExpandString(gender_t gender,
                                                 char const *in, char *out,
                                                 int maxlen) {
  // Assume the worst
...
//			maxlen -
//-----------------------------------------------------------------------------
void CSoundEmitterSystemBase::GenderExpandString(char const *actormodel,
                                                 char const *in, char *out,
                                                 int maxlen) {
...
void CSoundEmitterSystemBase::LoadGlobalActors() {
  ...
  if (allActors->LoadFromFile(filesystem, "scripts/global_actors.txt", NULL)) {
    KeyValues *pvkActor;
    for (pvkActor = allActors->GetFirstSubKey(); pvkActor != NULL;
         pvkActor = pvkActor->GetNextKey()) {
      ...
      if (idx == m_ActorGenders.InvalidIndex()) {
        if (m_ActorGenders.Count() + 1 == m_ActorGenders.InvalidIndex()) {
          Warning(
              "Exceeded max number of actors in scripts/global_actors.txt\n");
          break;
        }
...
gender_t CSoundEmitterSystemBase::GetActorGender(char const *actormodel) {
  ...
  if (idx == m_ActorGenders.InvalidIndex()) return GENDER_NONE;
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CSoundEmitterSystemBase::InitSoundInternalParameters(
    const char *soundname, KeyValues *kv, CSoundParametersInternal &params) {
    ...
    if (!Q_strcasecmp(pKey->GetName(), "channel")) {
      ...
      while (pWaves) {
        ...
        pWaves = pWaves->GetNextKey();
               ...
               !Q_strcasecmp(pKey->GetName(), "CompatibilityAttenuation")) {
      if (!Q_strncasecmp(pKey->GetString(), "SNDLVL_", strlen("SNDLVL_"))) {
        DevMsg(
            "CSoundEmitterSystemBase::GetParametersForSound:  sound %s has "
            "\"attenuation\" with %s value!\n",
            soundname, pKey->GetString());
      }

      if (!Q_strncasecmp(pKey->GetString(), "ATTN_", strlen("ATTN_"))) {
        params.SetSoundLevel(
            ATTN_TO_SNDLVL(TranslateAttenuation(pKey->GetString())));
      } else {
        interval_t interval;
        ...
        params.SetSoundLevel(ATTN_TO_SNDLVL(start),
                             ATTN_TO_SNDLVL(end) - ATTN_TO_SNDLVL(start));
      }

      // Goldsrc compatibility mode.. feed the sndlevel value through the sound
      // engine interface in such a way that it can reconstruct the original
      // sndlevel value and flag the sound as using Goldsrc attenuation.
      bool bCompatibilityAttenuation =
          !Q_strcasecmp(pKey->GetName(), "CompatibilityAttenuation");
      if (bCompatibilityAttenuation) {
        if (params.GetSoundLevel().range != 0) {
          Warning(
              "CompatibilityAttenuation for sound %s must have same start and "
              "end values.\n",
              soundname);
        }

        params.SetSoundLevel(
            SNDLEVEL_TO_COMPATIBILITY_MODE(params.GetSoundLevel().start));
      }
    } else if (!Q_strcasecmp(pKey->GetName(), "soundlevel")) {
      if (!Q_strncasecmp(pKey->GetString(), "ATTN_", strlen("ATTN_"))) {
        DevMsg(
            "CSoundEmitterSystemBase::GetParametersForSound:  sound %s has "
            "\"soundlevel\" with %s value!\n",
            soundname, pKey->GetString());
      }
...
// Output : char const
//-----------------------------------------------------------------------------
const char *CSoundEmitterSystemBase::GetWavFileForSound(
    const char *soundname, char const *actormodel) {
  gender_t gender = GetActorGender(actormodel);
...
// Input  : *filename -
//-----------------------------------------------------------------------------
void CSoundEmitterSystemBase::AddSoundsFromFile(const char *filename,
                                                bool bPreload,
  ...
  if (filesystem->LoadKeyValues(*kv, IFileSystem::TYPE_SOUNDEMITTER, filename,
                                "GAME")) {
      ...
      if (pKeys->GetFirstSubKey()) {
        ...
        if (lookup != m_Sounds.InvalidIndex()) {
          ...
          if (bIsOverride) {
            ...
            // Store off the old sound if it's not already an "override" from
            // another file!!! Otherwise, just whack it again!!!
            ...
            InitSoundInternalParameters(pKeys->GetName(), pKeys,
                                        pEntry->m_SoundParams);
            pEntry->m_SoundParams.SetShouldPreload(
                bPreload);  // this gets handled by game code after initting.
  ...
  if (bIsOverride) {
    Warning(
        "SoundEmitter:  adding map sound overrides from %s [%i total, %i "
        "replacements, %i duplicated replacements]\n",
        filename, newOverrideCount, replaceCount, duplicatedReplacements);
  }
...
int CSoundEmitterSystemBase::CheckForMissingWavFiles(bool verbose) {
  ...
  for (i = 0; i < c; i++) {
    ...
    for (int wave = 0; wave < waveCount; wave++) {
      ...
      // Skip ! sentence stuff
      if (name[0] == CHAR_SENTENCE) continue;
      Q_snprintf(testfile, sizeof(testfile), "sound/%s", PSkipSoundChars(name));
      if (filesystem->FileExists(testfile)) continue;
...
float CSoundEmitterSystemBase::TranslateAttenuation(const char *key) {
  ...
  if (!Q_strcasecmp(key, "ATTN_NONE")) return ATTN_NONE;

  if (!Q_strcasecmp(key, "ATTN_NORM")) return ATTN_NORM;

  if (!Q_strcasecmp(key, "ATTN_IDLE")) return ATTN_IDLE;

  if (!Q_strcasecmp(key, "ATTN_STATIC")) return ATTN_STATIC;

  if (!Q_strcasecmp(key, "ATTN_RICOCHET")) return ATTN_RICOCHET;

  if (!Q_strcasecmp(key, "ATTN_GUNFIRE")) return ATTN_GUNFIRE;
...
bool CSoundEmitterSystemBase::AddSound(const char *soundname,
  ...
  if (i == m_SoundKeyValues.InvalidIndex()) {
    Warning(
        "CSoundEmitterSystemBase::AddSound( '%s', '%s', ... ), script file not "
        "list in manifest '%s'\n",
...
void CSoundEmitterSystemBase::MoveSound(const char *soundname,
  ...
  if (newscriptindex == m_SoundKeyValues.InvalidIndex()) {
    Warning(
        "CSoundEmitterSystemBase::MoveSound( '%s', '%s' ), script file not "
        "list in manifest '%s'\n",
...
const char *CSoundEmitterSystemBase::GetSoundScriptName(int index) const {
  if (index < 0 || index >= m_SoundKeyValues.Count()) return NULL;
...
bool CSoundEmitterSystemBase::IsSoundScriptDirty(int index) const {
  if (index < 0 || index >= m_SoundKeyValues.Count()) return false;
...
void CSoundEmitterSystemBase::SaveChangesToSoundScript(int scriptindex) {
  ...
  if (!outfile) {
    Msg("CSoundEmitterSystemBase::SaveChangesToSoundScript:  No script file "
        "for index %i\n",
        scriptindex);
    return;
  }

  if (filesystem->FileExists(outfile) && !filesystem->IsFileWritable(outfile)) {
  ...
  int c = GetSoundCount();
  for (int i = 0; i < c; i++) {
    if (Q_stricmp(outfile, GetSourceFileForSound(i))) continue;

    // It's marked for deletion, just skip it
    if (m_Sounds[i]->m_bRemoved) continue;
    ...
    if (totalCount > 0) {
      ...
      if (waveCount == 1) {
        ...
        for (wave = 0; wave < waveCount; wave++) {
          ...
          if (p->GetSoundNames()[wave].gender != GENDER_NONE) continue;
  ...
  if (fh) {
    ...
    Warning(
        "SceneManager_SaveSoundsToScriptFile:  Unable to write file %s!!!\n",
        outfile);
  }
}
...
void CSoundEmitterSystemBase::RenameSound(const char *soundname,
                                          const char *newname) {
  // Same name?
...
void CSoundEmitterSystemBase::UpdateSoundParameters(
    const char *soundname, const CSoundParametersInternal &params) {
  int idx = GetSoundIndex(soundname);
  if (!IsValidIndex(idx)) {
...
bool CSoundEmitterSystemBase::GetParametersForSoundEx(
    const char *soundname, HSOUNDSCRIPTHANDLE &handle, CSoundParameters &params,
    gender_t gender, bool isbeingemitted /*= false*/) {
  if (handle == SOUNDEMITTER_INVALID_HANDLE) {
    handle = GetSoundIndex(soundname);
    if (handle == SOUNDEMITTER_INVALID_HANDLE) return false;
  }

  CSoundParametersInternal *internal =
      InternalGetParametersForSound((int)handle);
  if (!internal) {
    Assert(0);
    DevMsg(
        "CSoundEmitterSystemBase::GetParametersForSound:  No such sound %s\n",
  ...
  int bestIndex = FindBestSoundForGender(internal->GetSoundNames(),
                                         internal->NumSoundNames(), gender);

  if (bestIndex >= 0) {
    Q_strncpy(params.soundname,
              GetWaveName(internal->GetSoundNames()[bestIndex].symbol),
              sizeof(params.soundname));
  ...
  if (!params.soundname[0]) {
    DevMsg(
        "CSoundEmitterSystemBase::GetParametersForSound:  sound %s has no wave "
        "or rndwave key!\n",
        soundname);
    return false;
  }

  if (internal->HadMissingWaveFiles() && params.soundname[0] != CHAR_SENTENCE) {
    char testfile[256];
    Q_snprintf(testfile, sizeof(testfile), "sound/%s",
               PSkipSoundChars(params.soundname));
    if (!filesystem->FileExists(testfile)) {
      ...
      if (UTL_INVAL_SYMBOL == soundWarnings.Find(key)) {
        ...
        DevMsg(
            "CSoundEmitterSystemBase::GetParametersForSound:  sound '%s' "
            "references wave '%s' which doesn't exist on disk!\n",
...
soundlevel_t CSoundEmitterSystemBase::LookupSoundLevelByHandle(
    char const *soundname, HSOUNDSCRIPTHANDLE &handle) {
  if (handle == SOUNDEMITTER_INVALID_HANDLE) {
    handle = (HSOUNDSCRIPTHANDLE)GetSoundIndex(soundname);
    if (handle == SOUNDEMITTER_INVALID_HANDLE) return SNDLVL_NORM;
  }
...
void CSoundEmitterSystemBase::AddSoundOverrides(char const *scriptfile) {
  ...
  if (m_OverrideFiles.Find(handle) != m_OverrideFiles.InvalidIndex()) return;
...
void CSoundEmitterSystemBase::ClearSoundOverrides() {
  ...
  // Build a list of names to remove.  I'm reluctant to try and remove while
  // walking CUtlDict since it's an RB tree and there's
  //  no telling what will happen to the tree traversal if I'm removing while
  //  traversing it...  or I haven't dug into it at least...
  for (i = 0; i < m_Sounds.Count(); ++i) {
  ...
  if (toRemove.Count() > 0 || m_SavedOverrides.Count() > 0) {
    Warning(
        "SoundEmitter:  removing map sound overrides [%i to remove, %i to "
        "restore]\n",
        toRemove.Count(), m_SavedOverrides.Count());
  }
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSoundEmitterSystemBase,
