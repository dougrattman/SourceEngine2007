...
struct DataCacheItem_t : DataCacheItemData_t {
  DataCacheItem_t(const DataCacheItemData_t &data)
      : DataCacheItemData_t(data), hLRU(INVALID_MEMHANDLE) {
...
typedef CDataManager<DataCacheItem_t, DataCacheItemData_t, DataCacheItem_t *,
                     CThreadFastMutex>
    CDataCacheLRU;
...
class CDataCacheSection : public IDataCacheSection {
 public:
  CDataCacheSection(CDataCache *pSharedCache, IDataCacheClient *pClient,
                    const char *pszName);
  ~CDataCacheSection();
  ...
  virtual void GetStatus(DataCacheStatus_t *pStatus,
                         DataCacheLimits_t *pLimits = NULL);
  ...
  inline unsigned GetNumBytesUnlocked() {
    return m_status.nBytes - m_status.nBytesLocked;
  }
  inline unsigned GetNumItemsUnlocked() {
    return m_status.nItems - m_status.nItemsLocked;
  }
  ...
  virtual bool AddEx(DataCacheClientID_t clientId, const void *pItemData,
                     unsigned size, unsigned flags, DataCacheHandle_t *pHandle);
  virtual DataCacheHandle_t Find(DataCacheClientID_t clientId);
  virtual DataCacheRemoveResult_t Remove(DataCacheHandle_t handle,
                                         const void **ppItemData = NULL,
                                         unsigned *pItemSize = NULL,
                                         bool bNotify = false);
 ...
 private:
  ...
  virtual void OnAdd(DataCacheClientID_t clientId,
...
class CDataCacheSectionFastFind : public CDataCacheSection {
 public:
  CDataCacheSectionFastFind(CDataCache *pSharedCache, IDataCacheClient *pClient,
                            const char *pszName)
...
// Note: if status updates are moved out of a mutexed section, will need to
// change these to use interlocked instructions
