...
#define MakeCacheID(handle, type) \
  (((uintptr_t)(handle) << 16) | (uintptr_t)(type))
...
struct studiodata_t {
  ...
  // vertex data is usually compressed to save memory (model decal code only
  // needs some data)
...
static ConVar mod_forcedata(
    "mod_forcedata", (AsyncMdlCache()) ? "0" : "1", 0,
...
inline int MakeAsyncInfoKey(MDLHandle_t hModel, MDLCacheDataType_t type,
                            int iAnimBlock) {
  Assert(type <= 7 && iAnimBlock < 8 * 1024);
  return ((((int)hModel) << 16) | ((int)type << 13) | iAnimBlock);
}

inline int GetAsyncInfoIndex(MDLHandle_t hModel, MDLCacheDataType_t type,
                             int iAnimBlock = 0) {
...
inline int SetAsyncInfoIndex(MDLHandle_t hModel, MDLCacheDataType_t type,
                             int iAnimBlock, int index) {
  Assert(index == NO_ASYNC ||
         GetAsyncInfoIndex(hModel, type, iAnimBlock) == NO_ASYNC);
  int key = MakeAsyncInfoKey(hModel, type, iAnimBlock);
  if (index == NO_ASYNC) {
...
struct ModelParts_t {
  ...
  bool DoFinalProcessing() {
    ...
    // when all parts are present, returns true ( guaranteed once ), and marked
    // as completed
...
//-----------------------------------------------------------------------------
class CMDLCache : public CTier3AppSystem<IMDLCache>,
                  public IStudioDataCache,
                  public CDefaultDataCacheClient {
 ...
 public:
  ...
  virtual vcollide_t *GetVCollide(MDLHandle_t handle) {
    return GetVCollideEx(handle, true);
  }
  virtual vcollide_t *GetVCollideEx(MDLHandle_t handle,
                                    bool synchronousLoad = true);
  ...
  void CacheData(DataCacheHandle_t *c, void *pData, int size, const char *name,
                 MDLCacheDataType_t type,
                 DataCacheClientID_t id = (DataCacheClientID_t)-1);
  ...
  void UncacheData(DataCacheHandle_t c, MDLCacheDataType_t type,
                   bool bLockedOk = false);

  void DisableAsync() {
    mod_load_mesh_async.SetValue(0);
    mod_load_anims_async.SetValue(0);
  }
  ...
  static void QueuedLoaderCallback_MDL(void *pContext, void *pContext2,
 ...
 private:
  ...
  // Returns the *actual* name of the model (could be an error model if the
  // requested model didn't load)
  const char *GetActualModelName(MDLHandle_t handle);

  // Attempts to load a MDL file, validates that it's ok.
  bool ReadMDLFile(MDLHandle_t handle, const char *pMDLFileName,
                   CUtlBuffer &buf);

  // Unserializes the VCollide file associated w/ models (the vphysics
  // representation)
  ...
  studiohdr_t *UnserializeMDL(MDLHandle_t handle, void *pData, int nDataSize,
                              bool bDataValid);
  ...
  void UnloadHardwareData(MDLHandle_t handle, bool bCacheRemove = true,
                          bool bLockedOk = false);
  ...
  FSAsyncStatus_t LoadData(const char *pszFilename, const char *pszPathID,
                           bool bAsync, FSAsyncControl_t *pControl) {
    return LoadData(pszFilename, pszPathID, NULL, 0, 0, bAsync, pControl);
  }
  FSAsyncStatus_t LoadData(const char *pszFilename, const char *pszPathID,
                           void *pDest, int nBytes, int nOffset, bool bAsync,
                           FSAsyncControl_t *pControl);
  vertexFileHeader_t *LoadVertexData(studiohdr_t *pStudioHdr);
  vertexFileHeader_t *BuildAndCacheVertexData(studiohdr_t *pStudioHdr,
                                              vertexFileHeader_t *pRawVvdHdr);
  ...
  bool ClearAsync(MDLHandle_t handle, MDLCacheDataType_t type, int iAnimBlock,
                  bool bAbort = false);
  ...
  int UpdateOrCreate(studiohdr_t *pHdr, const char *pFilename,
                     char *pX360Filename, int maxLen, const char *pPathID,
                     bool bForce = false);

  // Attempts to read the platform native file - on 360 it can read and swap
  // Win32 file as a fallback
  bool ReadFileNative(char *pFileName, const char *pPath, CUtlBuffer &buf,
                      int nMaxBytes = 0);

  // Creates a thin cache entry (to be used for model decals) from fat vertex
  // data
  vertexFileHeader_t *CreateThinVertexes(vertexFileHeader_t *originalData,
                                         const studiohdr_t *pStudioHdr,
                                         int *cacheLength);

  // Processes raw data (from an I/O source) into the cache. Sets the cache
  // state as expected for bad data.
  bool ProcessDataIntoCache(MDLHandle_t handle, MDLCacheDataType_t type,
                            int iAnimBlock, void *pData, int nDataSize,
                            bool bDataValid);
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMDLCache, IMDLCache,
                                  MDLCACHE_INTERFACE_VERSION, g_MDLCache);
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CMDLCache, IStudioDataCache,
...
bool CMDLCache::Connect(CreateInterfaceFn factory) {
  ...
  if (!g_pMaterialSystemHardwareConfig || !g_pPhysicsCollision ||
      !g_pStudioRender || !g_pMaterialSystem)
    return false;
...
InitReturnVal_t CMDLCache::Init() {
  ...
  if (!m_pModelCacheSection) {
    m_pModelCacheSection =
        g_pDataCache->AddSection(this, MODEL_CACHE_MODEL_SECTION_NAME);
  }
  ...
  if (!m_pMeshCacheSection) {
    ...
    m_pMeshCacheSection =
        g_pDataCache->AddSection(this, MODEL_CACHE_MESH_SECTION_NAME, limits);
  }

  if (!m_pAnimBlockCacheSection) {
    // 360 tuned to worst case, ep_outland_12a, less than 6 MB is not a viable
    // working set
    unsigned int animBlockLimit = IsX360() ? 6 * 1024 * 1024 : (unsigned)-1;
    DataCacheLimits_t limits(animBlockLimit, (unsigned)-1, 0, 0);
    m_pAnimBlockCacheSection = g_pDataCache->AddSection(
        this, MODEL_CACHE_ANIMBLOCK_SECTION_NAME, limits);
  }
...
const char *CMDLCache::GetModelName(MDLHandle_t handle) {
  if (handle == MDLHANDLE_INVALID) return ERROR_MODEL;
...
int CMDLCache::Release(MDLHandle_t handle) {
  // Deal with shutdown order issues (i.e. datamodel shutting down after
  // mdlcache)
  if (!m_bInitialized) return 0;

  // NOTE: It can be 0 during shutdown because multiple studiomdls
...
void CMDLCache::UnserializeVCollide(MDLHandle_t handle, bool synchronousLoad) {
  ...
  if (iAsync == NO_ASYNC) {
    ...
    memset(&pStudioData->m_VCollisionData, 0,
           sizeof(pStudioData->m_VCollisionData));
    {
      virtualmodel_t *pVirtualModel = GetVirtualModel(handle);
      if (pVirtualModel) {
        for (int i = 1; i < pVirtualModel->m_group.Count(); i++) {
          MDLHandle_t sharedHandle =
    ...
    LoadData(pFileName, "GAME", bAsyncLoad, &info.hControl);
    {
      ...
      iAsync = SetAsyncInfoIndex(handle, MDLCACHE_VCOLLIDE,
                                 m_PendingAsyncs.AddToTail(info));
    }
...
// Unserializes the PHY file associated w/ models (the vphysics representation)
//-----------------------------------------------------------------------------
vcollide_t *CMDLCache::GetVCollideEx(MDLHandle_t handle,
                                     bool synchronousLoad /*= true*/) {
  if (mod_test_not_available.GetBool()) return NULL;
...
void CMDLCache::AllocateAnimBlocks(studiodata_t *pStudioData, int nCount) {
  ...
  pStudioData->m_pAnimBlock =
      new DataCacheHandle_t[pStudioData->m_nAnimBlockCount];

  memset(pStudioData->m_pAnimBlock, 0,
         sizeof(DataCacheHandle_t) * pStudioData->m_nAnimBlockCount);

  pStudioData->m_iFakeAnimBlockStall =
      new unsigned long[pStudioData->m_nAnimBlockCount];
  memset(pStudioData->m_iFakeAnimBlockStall, 0,
         sizeof(unsigned long) * pStudioData->m_nAnimBlockCount);
}
...
unsigned char *CMDLCache::UnserializeAnimBlock(MDLHandle_t handle, int nBlock) {
  ...
  if (iAsync == NO_ASYNC) {
    ...
    info.hControl = NULL;
    LoadData(pFileName, "GAME", NULL, nSize, pBlock->datastart,
             mod_load_anims_async.GetBool(), &info.hControl);
    {
      AUTO_LOCK_FM(m_AsyncMutex);
      iAsync = SetAsyncInfoIndex(handle, MDLCACHE_ANIMBLOCK, nBlock,
                                 m_PendingAsyncs.AddToTail(info));
    }
  }
  ...
  return (unsigned char *)CheckData(pStudioData->m_pAnimBlock[nBlock],
                                    MDLCACHE_ANIMBLOCK);
}
...
unsigned char *CMDLCache::GetAnimBlock(MDLHandle_t handle, int nBlock) {
  ...
  unsigned char *pData = (unsigned char *)CheckData(
      pStudioData->m_pAnimBlock[nBlock], MDLCACHE_ANIMBLOCK);
  if (!pData) {
    pStudioData->m_pAnimBlock[nBlock] = NULL;
  ...
  if (mod_load_fakestall.GetInt()) {
    ...
    if (pStudioData->m_iFakeAnimBlockStall[nBlock] == 0 ||
        pStudioData->m_iFakeAnimBlockStall[nBlock] > t) {
      pStudioData->m_iFakeAnimBlockStall[nBlock] = t;
    }

    if ((int)(t - pStudioData->m_iFakeAnimBlockStall[nBlock]) <
        mod_load_fakestall.GetInt()) {
      return NULL;
    }
  }
  return pData;
...
// Allocates/frees autoplay sequence list
//-----------------------------------------------------------------------------
void CMDLCache::AllocateAutoplaySequences(studiodata_t *pStudioData,
                                          int nCount) {
  FreeAutoplaySequences(pStudioData);
...
void CMDLCache::FreeVirtualModel(MDLHandle_t handle) {
  ...
  if (pStudioData && pStudioData->m_pVirtualModel) {
    ...
    Assert((nGroupCount >= 1) &&
           pStudioData->m_pVirtualModel->m_group[0].cache == (void *)handle);

    // NOTE: Start at *1* here because the 0th element contains a reference to
    // *this* handle
    for (int i = 1; i < nGroupCount; ++i) {
      MDLHandle_t h =
          (MDLHandle_t)(uintptr_t)pStudioData->m_pVirtualModel->m_group[i]
              .cache;
      FreeVirtualModel(h);
      Release(h);
    }
...
virtualmodel_t *CMDLCache::GetVirtualModelFast(const studiohdr_t *pStudioHdr,
                                               MDLHandle_t handle) {
  if (pStudioHdr->numincludemodels == 0) return NULL;
...
//-----------------------------------------------------------------------------
// Purpose: Pulls all submodels/.ani file models into the cache
// to avoid runtime hitches and load animations at load time, set mod_forcedata
// to be 1
//-----------------------------------------------------------------------------
void CMDLCache::UnserializeAllVirtualModelsAndAnimBlocks(MDLHandle_t handle) {
  if (handle == MDLHANDLE_INVALID) return;

  // might be re-loading, discard old virtualmodel to force rebuild
  // unfortunately, the virtualmodel does build data into the cacheable
  // studiohdr
  FreeVirtualModel(handle);
...
bool CMDLCache::LoadHardwareData(MDLHandle_t handle) {
  ...
  if (iAsync == NO_ASYNC) {
    ...
    {
      AUTO_LOCK_FM(m_AsyncMutex);
      iAsync = SetAsyncInfoIndex(handle, MDLCACHE_STUDIOHWDATA,
                                 m_PendingAsyncs.AddToTail(info));
    }
  }
        ...
        for (int l = 0; l < pMesh->numflexes; l++) {
          ...
          for (int m = 0; m < pFlex->numverts; m++) {
            mstudiovertanim_t *pVAnim =
                bIsWrinkleAnim ? pFlex->pVertanimWrinkle(m)
                               : pFlex->pVertanim(m);  // Memory respresentation
            dstudiovertanim_t *pDAnim =
                (dstudiovertanim_t *)pVAnim;  // On-disk representation
...
//
//-----------------------------------------------------------------------------
bool CMDLCache::BuildHardwareData(MDLHandle_t handle, studiodata_t *pStudioData,
                                  studiohdr_t *pStudioHdr,
                                  OptimizedModel::FileHeader_t *pVtxHdr) {
  if (pVtxHdr) {
    ...
    if (pVtxHdr->version != OPTIMIZED_MODEL_FILE_VERSION) {
      Warning("Error Index File for '%s' version %d should be %d\n",
              pStudioHdr->pszName(), pVtxHdr->version,
  ...
  BeginLock();
  bool bLoaded = g_pStudioRender->LoadModel(pStudioHdr, pVtxHdr,
                                            &pStudioData->m_HardwareData);
  EndLock();
...
#if defined(USE_HARDWARE_CACHE)
  GetCacheSection(MDLCACHE_STUDIOHWDATA)
      ->Add(MakeCacheID(handle, MDLCACHE_STUDIOHWDATA),
            &pStudioData->m_HardwareData,
            ComputeHardwareDataSize(&pStudioData->m_HardwareData),
            &pStudioData->m_HardwareDataCache);
#endif
  return true;
}
...
// Loads the static meshes
//-----------------------------------------------------------------------------
void CMDLCache::UnloadHardwareData(MDLHandle_t handle, bool bCacheRemove,
                                   bool bLockedOk) {
  if (handle == MDLHANDLE_INVALID) return;
  ...
  if (pStudioData->m_nFlags & STUDIODATA_FLAGS_STUDIOMESH_LOADED) {
#if defined(USE_HARDWARE_CACHE)
    if (bCacheRemove) {
      if (GetCacheSection(MDLCACHE_STUDIOHWDATA)
              ->BreakLock(pStudioData->m_HardwareDataCache) &&
          !bLockedOk) {
      ...
      GetCacheSection(MDLCACHE_STUDIOHWDATA)
          ->Remove(pStudioData->m_HardwareDataCache);
    }
#endif
    ...
    memset(&pStudioData->m_HardwareData, 0,
           sizeof(pStudioData->m_HardwareData));
    pStudioData->m_nFlags &= ~STUDIODATA_FLAGS_STUDIOMESH_LOADED;
  }
}
...
studiohwdata_t *CMDLCache::GetHardwareData(MDLHandle_t handle) {
  ...
  if ((pStudioData->m_nFlags & (STUDIODATA_FLAGS_STUDIOMESH_LOADED |
                                STUDIODATA_FLAGS_NO_STUDIOMESH)) == 0) {
...
void CMDLCache::RestoreMaterialSystemObjects(int nChangeFlags) {
  ...
  while (i != m_MDLDict.InvalidIndex()) {
    ...
    bool bIsMDLInMemory =
        GetCacheSection(MDLCACHE_STUDIOHDR)->IsPresent(pStudioData->m_MDLCache);

    // If the vertex format changed, we have to free the data because we may be
    // using different .vtx files.
    ...
    // Only restore the hardware data of those studiohdrs which are currently in
    // memory
    if (bIsMDLInMemory) {
      GetHardwareData(i);
    }
...
//-----------------------------------------------------------------------------
static bool MdlcacheCreateCallback(const char *pSourceName,
  ...
  if (bOk) {
    ...
    int bytes = StudioByteSwap::ByteswapStudioFile(
        pTargetName, targetBuf.Base(), sourceBuf.Base(), sourceBuf.TellPut(),
        (studiohdr_t *)pHdr);
    if (bytes) {
      ...
      if (Q_stristr(pSourceName, ".mdl")) {
        ...
        Q_strncat(szANISourceName, ".ani", sizeof(szANISourceName),
                  COPY_ALL_CHARACTERS);
        UpdateOrCreate(szANISourceName, NULL, 0, pPathID,
                       MdlcacheCreateCallback, true, targetBuf.Base());
      }
...
//-----------------------------------------------------------------------------
int CMDLCache::UpdateOrCreate(studiohdr_t *pHdr, const char *pSourceName,
                              char *pTargetName, int targetLen,
                              const char *pPathID, bool bForce) {
  return ::UpdateOrCreate(pSourceName, pTargetName, targetLen, pPathID,
                          MdlcacheCreateCallback, bForce, pHdr);
}

//-----------------------------------------------------------------------------
// Purpose: Attempts to read a file native to the current platform
//-----------------------------------------------------------------------------
bool CMDLCache::ReadFileNative(char *pFileName, const char *pPath,
...
// Purpose:
//-----------------------------------------------------------------------------
studiohdr_t *CMDLCache::UnserializeMDL(MDLHandle_t handle, void *pData,
                                       int nDataSize, bool bDataValid) {
  CTempAllocHelper pOriginalData;
  ...
  studiohdr_t *pHdr =
      (studiohdr_t *)AllocData(MDLCACHE_STUDIOHDR, pStudioHdrIn->length);
  MemAlloc_PopAllocDbgInfo();
  if (!pHdr) return NULL;

  CacheData(&m_MDLDict[handle]->m_MDLCache, pHdr, pStudioHdrIn->length,
            GetModelName(handle), MDLCACHE_STUDIOHDR,
            MakeCacheID(handle, MDLCACHE_STUDIOHDR));
...
// Attempts to load a MDL file, validates that it's ok.
//-----------------------------------------------------------------------------
bool CMDLCache::ReadMDLFile(MDLHandle_t handle, const char *pMDLFileName,
                            CUtlBuffer &buf) {
...
studiohdr_t *CMDLCache::GetStudioHdr(MDLHandle_t handle) {
  if (handle == MDLHANDLE_INVALID) return NULL;
    ...
    // Returning a pointer to data inside the cache when it's unlocked is just a
    // bad idea. It's technically legal, but the pointer can get invalidated if
    // anything else looks at the cache. Don't do that. Assert(
    // m_pModelCacheSection->IsFrameLocking() ); Assert(
    // m_pMeshCacheSection->IsFrameLocking() );

#if _DEBUG
  VPROF_INCREMENT_COUNTER("GetStudioHdr", 1);
#endif
  studiohdr_t *pHdr = (studiohdr_t *)CheckData(m_MDLDict[handle]->m_MDLCache,
                                               MDLCACHE_STUDIOHDR);
  if (!pHdr) {
    ...
    if (!ReadMDLFile(handle, pModelName, buf)) {
      ...
      if (!bOk) {
        Error("Model %s not found and %s couldn't be loaded", pModelName,
              ERROR_MODEL);
        return NULL;
      }
    }
    ...
    // put it in the cache
    if (ProcessDataIntoCache(handle, MDLCACHE_STUDIOHDR, 0, buf.Base(),
                             buf.TellMaxPut(), true)) {
      pHdr = (studiohdr_t *)CheckData(m_MDLDict[handle]->m_MDLCache,
                                      MDLCACHE_STUDIOHDR);
    }
  }
...
bool CMDLCache::HandleCacheNotification(
    const DataCacheNotification_t &notification) {
  switch (notification.type) {
    ...
    case DC_REMOVED: {
      MdlCacheMsg("MDLCache: Data cache discard %s %s\n",
                  g_ppszTypes[TypeFromCacheID(notification.clientId)],
      ...
      if ((DataCacheClientID_t)notification.pItemData ==
        ...
        FreeData(TypeFromCacheID(notification.clientId),
                 (void *)notification.pItemData);
      } else {
...
bool CMDLCache::GetItemName(DataCacheClientID_t clientId, const void *pItem,
                            char *pDest, unsigned nMaxLen) {
  ...
  Q_snprintf(pDest, nMaxLen, "%s - %s", g_ppszTypes[type],
             GetModelName(handle));
...
bool CMDLCache::IsDataLoaded(MDLHandle_t handle, MDLCacheDataType_t type) {
  ...
  switch (type) {
    ...
    case MDLCACHE_ANIMBLOCK: {
      if (!pData->m_pAnimBlock) return false;

      for (int i = 0; i < pData->m_nAnimBlockCount; ++i) {
        if (!pData->m_pAnimBlock[i]) return false;
...
bool CMDLCache::VerifyHeaders(studiohdr_t *pStudioHdr) {
  ...
  if (!ReadFileNative(pFileName, "GAME", vvdHeader,
                      sizeof(vertexFileHeader_t))) {
    return false;
  }
  ...
  if (!ReadFileNative(pFileName, "GAME", vtxHeader,
                      sizeof(OptimizedModel::FileHeader_t))) {
    return false;
  }

  // check
  OptimizedModel::FileHeader_t *pVtxHdr =
      (OptimizedModel::FileHeader_t *)vtxHeader.PeekGet();
  if ((pVtxHdr->version != OPTIMIZED_MODEL_FILE_VERSION) ||
...
vertexFileHeader_t *CMDLCache::CacheVertexData(studiohdr_t *pStudioHdr) {
  ...
  handle = (MDLHandle_t)(uintptr_t)pStudioHdr->virtualModel;
  Assert(handle != MDLHANDLE_INVALID);

  pVvdHdr = (vertexFileHeader_t *)CheckData(m_MDLDict[handle]->m_VertexCache,
                                            MDLCACHE_VERTEXES);
  if (pVvdHdr) {
    return pVvdHdr;
  }
...
//-----------------------------------------------------------------------------
FSAsyncStatus_t CMDLCache::LoadData(const char *pszFilename,
                                    const char *pszPathID, void *pDest,
                                    int nBytes, int nOffset, bool bAsync,
                                    FSAsyncControl_t *pControl) {
  if (!*pControl) {
...
//-----------------------------------------------------------------------------
// Determine the maximum number of 'real' bone influences used by any vertex in
// a model (100% binding to bone zero doesn't count)
//-----------------------------------------------------------------------------
  ...
  for (int i = 0; i < numVerts; i++) {
    if (verts[i].m_BoneWeights.numbones > 0) {
      ...
      for (int j = 0; j < MAX_NUM_BONES_PER_VERT; j++) {
        if (verts[i].m_BoneWeights.weight[j] > 0) numWeights = j + 1;
      }
      if ((numWeights == 1) && (verts[i].m_BoneWeights.bone[0] == 0)) {
        // 100% binding to first bone - not really skinned (the first bone is
...
vertexFileHeader_t *CMDLCache::CreateThinVertexes(
  ...
  int vertexSize = 2 * sizeof(Vector) +
                   ...
                   numStoredWeights * sizeof(float);
  *cacheLength = sizeof(vertexFileHeader_t) + sizeof(thinModelVertices_t) +
                 numVerts * vertexSize;

  // Allocate cache space for the thin data
  MemAlloc_PushAllocDbgInfo("Models:Vertex data", 0);
  vertexFileHeader_t *pNewVvdHdr =
      (vertexFileHeader_t *)AllocData(MDLCACHE_VERTEXES, *cacheLength);
  MemAlloc_PopAllocDbgInfo();

  Assert(pNewVvdHdr);
  if (pNewVvdHdr) {
    // Copy the header and set it up to hold thin vertex data
    memcpy((void *)pNewVvdHdr, (void *)originalData,
           sizeof(vertexFileHeader_t));
    ...
    thinModelVertices_t *pNewThinVerts =
        (thinModelVertices_t *)(pNewVvdHdr + 1);
    ...
    if (numBoneInfluences == 0) pBoneIndices = NULL;
    pNewThinVerts->Init(numBoneInfluences, pPositions, pNormals, pBoneWeights,
                        pBoneIndices);
...
//-----------------------------------------------------------------------------
bool CMDLCache::ProcessDataIntoCache(MDLHandle_t handle,
                                     MDLCacheDataType_t type, int iAnimBlock,
                                     void *pData, int nDataSize,
                                     bool bDataValid) {
  studiohdr_t *pStudioHdrCurrent = NULL;
  if (type != MDLCACHE_STUDIOHDR) {
    // can only get the studiohdr once the header has been processed
    // successfully into the cache causes a ProcessDataIntoCache() with the
    // studiohdr data
  ...
  switch (type) {
    case MDLCACHE_STUDIOHDR: {
      ...
      if (pStudioHdrCurrent->numincludemodels == 0) {
        // perf optimization, calculate once and cache off the autoplay
        // sequences
        int nCount = pStudioHdrCurrent->CountAutoplaySequences();
        if (nCount) {
          AllocateAutoplaySequences(m_MDLDict[handle], nCount);
          pStudioHdrCurrent->CopyAutoplaySequences(
              m_MDLDict[handle]->m_pAutoplaySequenceList, nCount);
        }
      }
    ...
    case MDLCACHE_STUDIOHWDATA: {
      if (bDataValid) {
        BuildHardwareData(handle, pStudioDataCurrent, pStudioHdrCurrent,
                          (OptimizedModel::FileHeader_t *)pData);
      } else {
    ...
    case MDLCACHE_ANIMBLOCK: {
      ...
      if (bDataValid) {
        ...
        CacheData(&pStudioDataCurrent->m_pAnimBlock[iAnimBlock], pData,
                  nDataSize, pCacheName, MDLCACHE_ANIMBLOCK,
                  MakeCacheID(handle, MDLCACHE_ANIMBLOCK));
      } else {
    ...
    case MDLCACHE_VCOLLIDE: {
      ...
      if (bDataValid) {
        ...
        if ((header.size == sizeof(header)) && header.solidCount > 0) {
          ...
          g_pPhysicsCollision->VCollideLoad(pCollide, header.solidCount,
                                            (const char *)buf.PeekGet(),
                                            nBufSize);
          if (m_pCacheNotify) {
...
int CMDLCache::ProcessPendingAsync(int iAsync) {
  ...
  FSAsyncStatus_t status =
      g_pFullFileSystem->AsyncGetResult(pInfo->hControl, &pData, &nBytesRead);
  ...
  switch (pInfo->type) {
    ...
    case MDLCACHE_STUDIOHWDATA:
    case MDLCACHE_VCOLLIDE: {
      ProcessDataIntoCache(pInfo->hModel, pInfo->type, 0, pData, nBytesRead,
                           status == FSASYNC_OK);
    ...
    case MDLCACHE_ANIMBLOCK: {
      ...
      if (!ProcessDataIntoCache(pInfo->hModel, MDLCACHE_ANIMBLOCK,
                                pInfo->iAnimBlock, pData, nBytesRead,
                                status == FSASYNC_OK)) {
        g_pFullFileSystem->FreeOptimalReadBuffer(pData);
      }
      break;
...
void CMDLCache::ProcessPendingAsyncs(MDLCacheDataType_t type) {
  ...
  // Process all of the completed loads that were requested before a new one.
  // This ensures two things -- the LRU is in correct order, and it catches
  // precached items lurking in the async queue that have only been requested
  // once (thus aren't being cached and might lurk forever, e.g., wood gibs in
  // the citadel)
...
// Cache model's specified dynamic data
//-----------------------------------------------------------------------------
bool CMDLCache::ClearAsync(MDLHandle_t handle, MDLCacheDataType_t type,
                           int iAnimBlock, bool bAbort) {
  ...
  if (iAsyncInfo != NO_ASYNC) {
    ...
    if (pInfo->hControl) {
      if (bAbort) {
        ...
        void *pData;
        int ignored;
        if (g_pFullFileSystem->AsyncGetResult(pInfo->hControl, &pData,
                                              &ignored) == FSASYNC_OK) {
          g_pFullFileSystem->FreeOptimalReadBuffer(pData);
        }
      }
...
// Cache model's specified dynamic data
//-----------------------------------------------------------------------------
vertexFileHeader_t *CMDLCache::BuildAndCacheVertexData(
    studiohdr_t *pStudioHdr, vertexFileHeader_t *pRawVvdHdr) {
  MDLHandle_t handle = (MDLHandle_t)(uintptr_t)pStudioHdr->virtualModel;
  vertexFileHeader_t *pVvdHdr;
  ...
  if (pRawVvdHdr->version != MODEL_VERTEX_FILE_VERSION) {
    Warning("Error Vertex File for '%s' version %d should be %d\n",
            pStudioHdr->pszName(), pRawVvdHdr->version,
  ...
  CacheData(&m_MDLDict[handle]->m_VertexCache, pVvdHdr, cacheLength,
            pStudioHdr->pszName(), MDLCACHE_VERTEXES,
            MakeCacheID(handle, MDLCACHE_VERTEXES));

  // expected 32 byte alignment
  Assert(((int64_t)pVvdHdr & 0x1F) == 0);
...
vertexFileHeader_t *CMDLCache::LoadVertexData(studiohdr_t *pStudioHdr) {
  ...
  if (iAsync == NO_ASYNC) {
    ...
    {
      AUTO_LOCK_FM(m_AsyncMutex);
      iAsync = SetAsyncInfoIndex(handle, MDLCACHE_VERTEXES,
                                 m_PendingAsyncs.AddToTail(info));
    }
  }
  ...
  return (vertexFileHeader_t *)CheckData(m_MDLDict[handle]->m_VertexCache,
                                         MDLCACHE_VERTEXES);
}
...
// Caches an item
//-----------------------------------------------------------------------------
void CMDLCache::CacheData(DataCacheHandle_t *c, void *pData, int size,
                          const char *name, MDLCacheDataType_t type,
                          DataCacheClientID_t id) {
  if (!pData) {
    return;
  }
...
//-----------------------------------------------------------------------------
void *CMDLCache::CheckDataNoTouch(DataCacheHandle_t c,
                                  MDLCacheDataType_t type) {
...
// Frees a cache item
//-----------------------------------------------------------------------------
void CMDLCache::UncacheData(DataCacheHandle_t c, MDLCacheDataType_t type,
                            bool bLockedOk) {
  if (c == DC_INVALID_HANDLE) return;
...
void CMDLCache::ProcessQueuedData(ModelParts_t *pModelParts) {
  ...
  if (pModelParts->nLoadedParts & (1 << ModelParts_t::BUFFER_MDL)) {
    ...
    ProcessDataIntoCache(handle, MDLCACHE_STUDIOHDR, 0, pData, nSize,
                         nSize != 0);
    g_pFullFileSystem->FreeOptimalReadBuffer(pData);
  }

  bool bAbort = false;
  pStudioHdr = (studiohdr_t *)CheckDataNoTouch(m_MDLDict[handle]->m_MDLCache,
                                               MDLCACHE_STUDIOHDR);
  if (!pStudioHdr) {
    // huh?, the header is expected to be loaded and locked, everything depends
    // on it!
    Assert(0);
    DevWarning("CMDLCache:: Error MDLCACHE_STUDIOHDR not present for '%s'\n",
               GetModelName(handle));

    // cannot unravel any of this model's dependant data, abort any further
    // processing
    bAbort = true;
  }
  ...
  if (pModelParts->nLoadedParts & (1 << ModelParts_t::BUFFER_PHY)) {
    ...
    nSize = bAbort
                ? 0
                : pModelParts->Buffers[ModelParts_t::BUFFER_PHY].TellMaxPut();
    ProcessDataIntoCache(handle, MDLCACHE_VCOLLIDE, 0, pData, nSize,
                         nSize != 0);
    g_pFullFileSystem->FreeOptimalReadBuffer(pData);
  }
  ...
  if (pModelParts->nLoadedParts & (1 << ModelParts_t::BUFFER_VVD)) {
    ...
    nSize = bAbort
                ? 0
                : pModelParts->Buffers[ModelParts_t::BUFFER_VVD].TellMaxPut();
    ProcessDataIntoCache(handle, MDLCACHE_VERTEXES, 0, pData, nSize,
                         nSize != 0);
    g_pFullFileSystem->FreeOptimalReadBuffer(pData);
  }
  ...
  if (pModelParts->nLoadedParts & (1 << ModelParts_t::BUFFER_VTX)) {
    ...
    nSize = bAbort
                ? 0
                : pModelParts->Buffers[ModelParts_t::BUFFER_VTX].TellMaxPut();
    ...
    GetCacheSection(MDLCACHE_STUDIOHWDATA)->Lock(pStudioData->m_VertexCache);
    {
      ...
      ProcessDataIntoCache(handle, MDLCACHE_STUDIOHWDATA, 0, pData, nSize,
                           nSize != 0);
    }
    g_pFullFileSystem->FreeOptimalReadBuffer(pData);
  }
...
// Not all components exist, but that information is not known at job
// submission.
//-----------------------------------------------------------------------------
void CMDLCache::QueuedLoaderCallback_MDL(void *pContext, void *pContext2,
                                         const void *pData, int nSize,
                                         LoaderError_t loaderError) {
...
//-----------------------------------------------------------------------------
// Build a queued loader job to get the MDL ant all of its components into the
// cache.
//-----------------------------------------------------------------------------
bool CMDLCache::PreloadModel(MDLHandle_t handle) {
  ...
  // actual necessity is not established here, allowable absent files need their
  // i/o error to occur
...
virtualmodel_t *studiohdr_t::GetVirtualModel(void) const {
  ...
  return g_MDLCache.GetVirtualModelFast(this,
                                        (MDLHandle_t)(uintptr_t)virtualModel);
}
