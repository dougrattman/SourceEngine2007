...
static RGBA8888ToUserFormatFunc_t GetRGBA8888ToUserFormatFunc_t(
...
static inline void DecodeColorBlock(CDestPixel *pOutputImage,
...
bool ConvertToATIxN(const uint8_t *src, ImageFormat srcImageFormat,
  ...
  if (srcImageFormat != IMAGE_FORMAT_ARGB8888) return false;
  ...
  destTexture.format =
      dstImageFormat == IMAGE_FORMAT_ATI2N
          ? ATI_TC_FORMAT_ATI2N
          : ATI_TC_FORMAT_ATI1N;  // Assume it can only be one of these two...
  ...
  if ((dstImageFormat == IMAGE_FORMAT_DXT1 ||
    ...
    return true;
             ...
             (dstImageFormat == IMAGE_FORMAT_DXT1 ||       //
              ...
              dstImageFormat == IMAGE_FORMAT_DXT5)) {
    return ConvertToDXT(src, srcImageFormat, dst, dstImageFormat, width, height,
                        srcStride, dstStride);
  } else if ((srcImageFormat == IMAGE_FORMAT_ARGB8888) &&  // RGBA source and
             (dstImageFormat == IMAGE_FORMAT_ATI1N ||
              dstImageFormat == IMAGE_FORMAT_ATI2N))  // ATI compressed dest
  {
    return ConvertToATIxN(src, srcImageFormat, dst, dstImageFormat, width,
                          height, srcStride, dstStride);
...
#define DST(x, y) dst[((x) + (y)*widthHeight) * sizeInBytes]
  ...
  for (y = 0; y < halfWidthHeight; y++) {
    for (x = 0; x < halfWidthHeight; x++) {
      ...
      memcpy(tmp[2], &SRC(widthHeight - x - 1, widthHeight - y - 1),
             sizeInBytes);
      ...
      memcpy(&DST(widthHeight - x - 1, widthHeight - y - 1), tmp[1],
             sizeInBytes);
