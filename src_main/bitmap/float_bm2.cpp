...
void FloatBitMap_t::CompressTo8Bits(float overbright) {
  ...
  // I haven't been able to come up with a filter which eleiminates
  // objectionable artifacts on all source textures. So, I've gone to doing the
  // lerping in the shader.
  int pass = 0;
  while (pass < 1) {
    ...
    for (int y = 0; y < Height; y++) {
      for (int x = 0; x < Width; x++) {
        ...
        MaybeSetScaleVaue(*this, temp_filtered, x, y, new_trial_scale,
                          overbright);
      }
    }
    pass++;
