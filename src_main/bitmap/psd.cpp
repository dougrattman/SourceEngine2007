...
//	byte			palette[ numBytesPalette ]; = { (all red palette
...
bool IsPSDFile(CUtlBuffer &buf) {
  ...
  if (BigLong(header.m_nSignature) != PSD_SIGNATURE) return false;
  if (BigShort(header.m_nVersion) != 1) return false;
...
bool PSDGetInfo(CUtlBuffer &buf, int *pWidth, int *pHeight,
  ...
  if (BigLong(header.m_nSignature) != PSD_SIGNATURE) return false;
  if (BigShort(header.m_nVersion) != 1) return false;
  if (BigShort(header.m_nDepth) != 8) return false;

  *pWidth = BigLong(header.m_nColumns);
  *pHeight = BigLong(header.m_nRows);
  *pImageFormat = BigShort(header.m_nChannels) == 3 ? IMAGE_FORMAT_RGB888
...
static void PSDReadCompressedChannels(CUtlBuffer &buf, int nChannelsCount,
  ...
  for (int i = 0; i < nChannelsCount; ++i) {
    ...
    for (int j = 0; j < bitmap.m_nHeight; ++j) {
      u8 *pSrc = pChannelRow;
      int nPixelsRemaining = bitmap.m_nWidth;
      while (nPixelsRemaining > 0) {
        int nCount = buf.GetChar();
        if (nCount >= 0) {
          // If nCount is between 0 + 7F, it means copy the next nCount+1 bytes
          // directly
          ++nCount;
          Assert(nCount <= nPixelsRemaining);
          buf.Get(pSrc, nCount);
        } else {
          // If nCount is between 80 and FF, it means replicate the next byte
          // -Count+1 times
          nCount = -nCount + 1;
          Assert(nCount <= nPixelsRemaining);
...
bool PSDReadFileRGBA8888(CUtlBuffer &buf, Bitmap_t &bitmap) {
  ...
  switch (mode) {
    case MODE_RGBA:
      if (nChannelsCount < 3) return false;
      break;

    case MODE_GREYSCALE:
    case MODE_PALETTIZED:
      if (nChannelsCount != 1 && nChannelsCount != 2) return false;
      break;

    case MODE_CMYK:
      if (nChannelsCount < 4) return false;
      break;
  ...
  if (nCompressionType == 0) {
    PSDReadUncompressedChannels(buf, (nChannelsCount > 4) ? 4 : nChannelsCount,
                                mode, palette, bitmap);
  } else {
...
PSDResFileInfo::ResFileInfoElement PSDResFileInfo::FindElement(
    ResFileInfo eType) const {
