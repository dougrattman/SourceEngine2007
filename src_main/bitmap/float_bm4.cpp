...
static unsigned SSBumpCalculationThreadFN(void *ctx1) {
  ...
  RayTracingSingleResult *rslts =
      new RayTracingSingleResult[ctx->ret_bm->Width *
                                 ctx->nrays_to_trace_per_pixel];

  for (int y = ctx->min_y; y <= ctx->max_y; y++) {
    if (ctx->thread_number == 0)
      ReportProgress("Computing output", (1 + ctx->max_y - ctx->min_y),
                     y - ctx->min_y);
    ...
    // now, all ray tracing results are in the results buffer. Determine the
    // visible self-shadowed bump map lighting at each vertex in each basis
    // direction
    for (int x = 0; x < ctx->src_bm->Width; x++) {
          ...
          if (ctx->nrays_to_trace_per_pixel) {
            ...
            for (int r = 0; r < ctx->nrays_to_trace_per_pixel; r++) {
              ...
              if (dot > 0) {
                ...
                if (this_rslt[r].HitID == -1) sum_dots += dot;
              }
            }
          } else {
      ...
      if (ctx->m_nOptionFlags & SSBUMP_OPTION_NONDIRECTIONAL) {
        ctx->ret_bm->Pixel(x, y, 1) =
            ctx->ret_bm->Pixel(x, y, 0);  // copy height
        ctx->ret_bm->Pixel(x, y, 2) =
            ctx->ret_bm->Pixel(x, y, 0);  // copy height
        ctx->ret_bm->Pixel(x, y, 3) =
            ctx->ret_bm->Pixel(x, y, 0);  // copy height
      } else {
        ctx->ret_bm->Pixel(x, y, 3) =
            ctx->src_bm->Pixel(x, y, 3);  // copy height
      }
    }
  }
  delete[] rslts;
...
FloatBitMap_t::ComputeSelfShadowedBumpmapFromHeightInAlphaChannel(
  ...
  if (nrays_to_trace_per_pixel) {
    rtEnv.MakeRoomForTriangles((1 + 2 * NREPS_TILE) * (1 + 2 * NREPS_TILE) * 2 *
                               Height * Width);
    ...
    for (int tilex = -NREPS_TILE; tilex <= NREPS_TILE; tilex++)
      for (int tiley = -NREPS_TILE; tiley <= NREPS_TILE; tiley++) {
        ...
        int min_y = 0;
        int max_y = Height - 1;
        if (tilex < 0) min_x = Width / 2;
        if (tilex > 0) max_x = Width / 2;
        if (tiley < 0) min_y = Height / 2;
        if (tiley > 0) max_y = Height / 2;
    ...
    // ok, now we have built a structure for ray intersection. we will take
    // advantage of the SSE ray tracing code by intersecting rays as a batch.
  }

  // We need to calculate for each vertex (i.e. pixel) of the heightmap, how
  // "much" of the world it can see in each basis direction. we will do this by
  // sampling a sphere of rays around the vertex, and using dot-product
  // weighting to measure the lighting contribution in each basis direction.
  // note that the surface normal is not used here. The surface normal will end
  // up being reflected in the result because of rays being blocked when they
  // try to pass through the planes of the triangles touching the vertex.

  // note that there is no reason inter-bounced lighting could not be folded
  ...
  for (int t = 0; t < nthreads; t++) {
    if (t) memcpy((void *)(&ctxs[t]), (void *)&ctxs[0], sizeof(ctxs[0]));
    ...
    waithandles[t] = CreateSimpleThread(SSBumpCalculationThreadFN,
                                        (SSBumpCalculationContext *)&ctxs[t]);
    starty += ystep;
  }
  for (int t = 0; t < nthreads; t++) {
