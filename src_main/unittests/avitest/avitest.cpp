...
bool CAVITestApp::PreInit() {
  ...
  if (!g_pFullFileSystem || !g_pMaterialSystem || !g_pVGui || !g_pVGuiSurface ||
      !g_pAVI || !g_pBIK)
    return false;
...
int CAVITestApp::Main() {
  ...
  AVIMaterial_t hAVIMaterial =
      g_pAVI->CreateAVIMaterial("avitest", "c:\\temp\\test.avi", "MOD");
