...
class CShaderUtilTemp : public CBaseAppSystem<IShaderUtil> {
 public:
  ...
  virtual bool ConvertImageFormat(unsigned char *src,
                                  enum ImageFormat srcImageFormat,
                                  unsigned char *dst,
                                  enum ImageFormat dstImageFormat, int width,
                                  int height, int srcStride = 0,
                                  int dstStride = 0) {
    return true;
  }
  ...
  virtual void DrawClearBufferQuad(unsigned char r, unsigned char g,
  ...
  virtual bool OnDrawMesh(IMesh *pMesh, CPrimList *pLists, int nLists) {
    return false;
  }
  ...
  virtual bool OnSetPrimitiveType(IMesh *pMesh, MaterialPrimitiveType_t type) {
    return false;
  }
...
class CShaderAPITestApp : public CTier2SteamApp {
 ...
 private:
  ...
  void CreateSimpleBuffers(ShaderBufferType_t nVBType,
                           ShaderBufferType_t nIBType, bool bBuffered);

  // Destroys the buffers
  void DestroyBuffers();

  // Creates shaders
  void CreateShaders(const char *pVShader, int nVBufLen, const char *pGShader,
                     int nGBufLen, const char *pPShader, int nPBufLen);

  // Destroys the buffers
  void DestroyShaders();

  // DrawUsingShaders
  void TestColoredQuad(ShaderBufferType_t nVBType, ShaderBufferType_t nIBType,
                       bool bBuffered);
  ...
  if (module == APP_MODULE_INVALID) {
    if (module == APP_MODULE_INVALID) {
      ...
      if (module == APP_MODULE_INVALID) {
        ...
        module = LoadModule(pShaderDLL);
        if (module == APP_MODULE_INVALID) return false;
      }
    }
  }

  g_pShaderDeviceMgr = (IShaderDeviceMgr *)AddSystem(
      module, SHADER_DEVICE_MGR_INTERFACE_VERSION);
...
// Window callback
//-----------------------------------------------------------------------------
static LRESULT CALLBACK ShaderAPITestWndProc(HWND hWnd, UINT message,
...
// Window management
//-----------------------------------------------------------------------------
bool CShaderAPITestApp::CreateAppWindow(const char *pTitle, bool bWindowed,
                                        int w, int h) {
  WNDCLASSEX wc;
...
bool CShaderAPITestApp::PreInit() {
  ...
  if (!g_pFullFileSystem || !g_pShaderDeviceMgr) return false;
...
void CShaderAPITestApp::PostShutdown() { BaseClass::PostShutdown(); }
...
void CShaderAPITestApp::DisplayAdapterInfo() {
  ...
  for (int i = 0; i < nAdapterCount; ++i) {
    ...
    Msg("\tName: %s\n\tVendor: 0x%X\n\tDevice: 0x%X\n\tSubSystem: "
        ...
        "%d\n",
        info.m_pDriverName, info.m_VendorID, info.m_DeviceID, info.m_SubSysID,
        info.m_Revision, info.m_nDXSupportLevel, info.m_nMaxDXSupportLevel);

    CUtlBuffer buf(0, 0, CUtlBuffer::TEXT_BUFFER);
    KeyValues *pConfiguration = new KeyValues("Config");
    g_pShaderDeviceMgr->GetRecommendedConfigurationInfo(
        i, info.m_nDXSupportLevel, pConfiguration);
    ...
    for (int j = 0; j < nModeCount; ++j) {
      ...
      Msg("\t\tH: %5d W: %5d Format: %3d Refresh %3d/%3d\n", mode.m_nWidth,
          mode.m_nHeight, mode.m_Format, mode.m_nRefreshRateNumerator,
...
bool CShaderAPITestApp::SetMode() {
  ...
  CreateInterfaceFn shaderFactory =
      g_pShaderDeviceMgr->SetMode(m_HWnd, nAdapter, mode);
  if (!shaderFactory) {
...
// Creates really simple vertex + index buffers
//-----------------------------------------------------------------------------
void CShaderAPITestApp::CreateSimpleBuffers(ShaderBufferType_t nVBType,
  ...
  if (IsDynamicBufferType(nVBType)) {
    ...
    m_pVertexBuffer =
        m_pShaderDevice->CreateVertexBuffer(nVBType, fmt, 4, "test");
  }
  ...
  if (IsDynamicBufferType(nIBType)) {
    m_pIndexBuffer = m_pShaderDevice->CreateIndexBuffer(
        nIBType, MATERIAL_INDEX_FORMAT_UNKNOWN, 64, "test");
  } else {
    m_pIndexBuffer = m_pShaderDevice->CreateIndexBuffer(
        nIBType, MATERIAL_INDEX_FORMAT_16BIT, 6, "test");
  }
  ...
  m_pShaderAPI->BindVertexBuffer(0, m_pVertexBuffer, vb.Offset(), 0,
                                 vb.TotalVertexCount(), fmt);
...
//-----------------------------------------------------------------------------
void CShaderAPITestApp::CreateShaders(const char *pVShader, int nVBufLen,
                                      const char *pGShader, int nGBufLen,
                                      const char *pPShader, int nPBufLen) {
  const char *pVertexShaderVersion =
      g_pMaterialSystemHardwareConfig->GetDXSupportLevel() == 100 ? "vs_4_0"
                                                                  : "vs_2_0";
  const char *pPixelShaderVersion =
      g_pMaterialSystemHardwareConfig->GetDXSupportLevel() == 100 ? "ps_4_0"
                                                                  : "ps_2_0";

  // Compile shaders
  m_hVertexShader = m_pShaderDevice->CreateVertexShader(pVShader, nVBufLen,
                                                        pVertexShaderVersion);
...
// DrawQuad
//-----------------------------------------------------------------------------
void CShaderAPITestApp::TestColoredQuad(ShaderBufferType_t nVBType,
                                        ShaderBufferType_t nIBType,
                                        bool bBuffered) {
  // clear (so that we can make sure that we aren't getting results from the
  // previous quad)
  m_pShaderAPI->ClearColor3ub(RandomInt(0, 100), RandomInt(0, 100),
                              RandomInt(190, 255));
  ...
  // Draw a quad!
  CreateShaders(s_pSimpleVertexShader, sizeof(s_pSimpleVertexShader), NULL, 0,
                s_pSimplePixelShader, sizeof(s_pSimplePixelShader));
...
void CShaderAPITestApp::TestDynamicBuffers() {
  ...
  for (int i = 0; i < nLoopCount; ++i) {
    ...
    m_pShaderAPI->BindVertexBuffer(0, m_pVertexBuffer, vb.Offset(),
                                   vb.GetFirstVertex(), vb.TotalVertexCount(),
                                   fmt);
    m_pShaderAPI->BindIndexBuffer(m_pIndexBuffer, ib.Offset());
    m_pShaderAPI->Draw(MATERIAL_TRIANGLES, ib.GetFirstIndex(),
                       ib.TotalIndexCount());
  }
...
bool CShaderAPITestApp::CreateDynamicCombos_Ver5(uint8 *pComboBuffer,
                                                 bool bVertexShader) {
...
void CShaderAPITestApp::LoadShaderFile(const char *pName, bool bVertexShader) {
  ...
  Q_snprintf(pFileName, SOURCE_MAX_PATH, "..\\hl2\\shaders\\fxc\\%s.vcs",
             pName);

  FileHandle_t hFile =
      g_pFullFileSystem->Open(pFileName, "rb", "EXECUTABLE_PATH");
  if (hFile == FILESYSTEM_INVALID_HANDLE) {
    Warning("Couldn't load %s shader %s\n", bVertexShader ? "vertex" : "pixel",
            pName);
    return;
  }
...
int CShaderAPITestApp::Main() {
  ...
  if (!WaitForKeypress()) return 1;

  // Test viewport
  int nMaxViewports = g_pMaterialSystemHardwareConfig->MaxViewports();
  ShaderViewport_t *pViewports =
      (ShaderViewport_t *)_alloca(nMaxViewports * sizeof(ShaderViewport_t));
  ...
  // Test drawing a full-screen quad with interpolated vertex colors for every
  // combo of static/dynamic VB, static dynamic IB, buffered/non-buffered.
  char buf[1024];
  for (int nVBType = 0; nVBType < SHADER_BUFFER_TYPE_COUNT; ++nVBType) {
    // TODO(d.rattman): Remove
    if (nVBType > SHADER_BUFFER_TYPE_DYNAMIC) continue;

    for (int nIBType = 0; nIBType < SHADER_BUFFER_TYPE_COUNT; ++nIBType) {
      // TODO(d.rattman): Remove
      if (nIBType > SHADER_BUFFER_TYPE_DYNAMIC) continue;

      // MESHFIXME: make buffered vertex buffers/index buffers work.
      int nBuffered = 0;
      //			for( nBuffered = 0; nBuffered < 2; nBuffered++ )
      {
        TestColoredQuad((ShaderBufferType_t)nVBType,
                        (ShaderBufferType_t)nIBType, nBuffered != 0);

        sprintf(buf,
                "TestColoredQuad results VB: %d IB: %d Buffered: %d HIT A KEY!",
