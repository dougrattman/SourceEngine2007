...
class CModalPreserveMessageBox : public vgui::MessageBox {
 public:
  CModalPreserveMessageBox(const char *title, const char *text,
                           vgui::Panel *parent)
...
//-----------------------------------------------------------------------------
// Purpose: Called during intialization to setup the initial state of the VGUI
// controls
//-----------------------------------------------------------------------------
...
CBrowseButton::CBrowseButton(vgui::Panel *pParent)
    : BaseClass(pParent, "Browse Button", "...", pParent, "browse") {
  SetParent(pParent);
...
void CBrowseButton::InitBrowseInfo(int x, int y, const char *pszName,
                                   const char *pszDir, const char *pszFilter,
                                   const char *pszField) {
  SetSize(24, 24);
  SetPos(x, y);
  SetName(pszName);
...
void CBrowseButton::SetActionMessage() {
  KeyValues *newActionMessage = new KeyValues(
      "browse", "directory", pszStartingDirectory, "filter", pszFileFilter);
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CQCGenerator::CQCGenerator(vgui::Panel *pParent, const char *pszPath,
                           const char *pszScene)
    : BaseClass(pParent, "QCGenerator") {
  ...
  m_pCollisionBrowseButton->InitBrowseInfo(
  ...
  Q_strncpy(m_QCInfo_t.pszMaterialPath, pszMaterialsStart,
            pszMaterialsEnd - pszMaterialsStart + 1);
  ...
  if (!fp) {
    // the set game configuration didn't have a surfaceproperties file; we are
    // grabbing it from hl2
    // TODO:  This only works if they are in a subdirectory that is a peer to an
  ...
  vgui::Panel *pSurfacePropDropDown =
      FindChildByName("surfacePropertyDropDown");
...
{
        DirectorySelectDialog *pDialog = new DirectorySelectDialog( this,
"Select Directory" ); pDialog->AddActionSignalTarget( this );
        pDialog->DoModal();
...
bool CQCGenerator::GenerateQCFile() {
  // TODO: clean this up.  Consider creating a datatype that includes the string
  ...
  for (int i = 0; i < m_QCInfo_t.LODs.Count(); i++) {
    ...
    g_pFullFileSystem->FPrintf(
        pSaveFile, "$lod %d\n{\n\treplacemodel \"%s\" \"%s\"\n}\n\n",
        thisLOD.iLOD, strrchr(m_QCInfo_t.pszSMDPath, '\\') + 1,
        thisLOD.pszFilename);
  }
  ...
  if (m_QCInfo_t.bDisableCollision != true) {
    ...
    if (m_QCInfo_t.bReferenceAsPhys == true) {
      g_pFullFileSystem->FPrintf(pSaveFile, "$collisionmodel \"%s\"",
                                 strrchr(m_QCInfo_t.pszSMDPath, '\\') + 1);
    } else {
      if (Q_strcmp(m_QCInfo_t.pszCollisionPath, "")) {
        g_pFullFileSystem->FPrintf(
            pSaveFile, "$collisionmodel \"%s\"",
            strrchr(m_QCInfo_t.pszCollisionPath, '\\') + 1);
      }
    }
  ...
  g_pFullFileSystem->RelativePathToFullPath(
      "studiomdl.bat", NULL, studiomdlPath, sizeof(studiomdlPath));
...
void CQCGenerator::EditLOD() {
  ...
  if (numSelected == 1 && !m_pLODPanel->IsInEditMode()) {
    ...
    m_pLODPanel->EnterEditMode(m_nSelectedSequence, m_nSelectedColumn,
                               m_pLODEdit);
  }
}
