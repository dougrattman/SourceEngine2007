...
// Constructor
//-----------------------------------------------------------------------------
CGameFileTreeView::CGameFileTreeView(Panel *parent, const char *name,
...
bool CGameFileTreeView::DoesDirectoryHaveSubdirectories(const char *pFilePath) {
  ...
  const char *pszFileName =
      g_pFullFileSystem->FindFirstEx(pSearchString, "GAME", &findHandle);
  while (pszFileName) {
    bool bIsDirectory = g_pFullFileSystem->FindIsDirectory(findHandle);
    if (bIsDirectory && Q_strnicmp(pszFileName, ".", 2) &&
        Q_strnicmp(pszFileName, "..", 3))
      return true;
...
// Populate the root node with directories
//-----------------------------------------------------------------------------
void CGameFileTreeView::AddDirectoriesOfNode(int itemIndex,
...
//-----------------------------------------------------------------------------
void CGameFileTreeView::AddFilesOfNode(int itemIndex, const char *pFilePath,
                                       const char *pExt) {
  char pSearchString[SOURCE_MAX_PATH];
  ...
  // add all the items
  const char *pszFileName =
      g_pFullFileSystem->FindFirst(pSearchString, &findHandle);
  while (pszFileName) {
...
void CGameFileTreeView::GenerateContextMenu(int itemIndex, int x, int y) {
  ...
  pContext->AddMenuItem("Cloak folder", new KeyValues("CloakFolder", "item",
  itemIndex), GetParent(), NULL);
