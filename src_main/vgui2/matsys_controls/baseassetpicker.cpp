...
//-----------------------------------------------------------------------------
// sorting function, should return true if node1 should be displayed before
// node2
//-----------------------------------------------------------------------------
...
class CAssetTreeView : public vgui::TreeView {
 ...
 public:
  CAssetTreeView(vgui::Panel *parent, const char *name,
                 const char *pRootFolderName, const char *pRootDir);
 ...
 private:
  ...
  void AddDirectoryToTreeView(int nParentItemIndex, const char *pFullParentPath,
                              DirHandle_t hPath);
...
DirHandle_t CAssetTreeView::AddSubDirectory(DirHandle_t hParent,
                                            const char *pDirName) {
  ...
  Q_strlower(m_DirectoryStructure[hSubdir].Get(),
             m_DirectoryStructure[hSubdir].Length() + 1);
...
// Add a directory into the treeview
//-----------------------------------------------------------------------------
void CAssetTreeView::AddDirectoryToTreeView(int nParentItemIndex,
  ...
  bool bHasSubdirectories = m_DirectoryStructure.FirstChild(hPath) !=
                            m_DirectoryStructure.InvalidIndex();
...
void CAssetTreeView::GenerateChildrenOfNode(int nItemIndex) {
  ...
  if (!pFullParentPath) return;

  DirHandle_t hPath = (DirHandle_t)pkv->GetInt(
      "dirHandle", m_DirectoryStructure.InvalidIndex());
  if (hPath == m_DirectoryStructure.InvalidIndex()) return;
...
DirHandle_t CAssetTreeView::RefreshTreeViewItem(int nItemIndex) {
  if (nItemIndex < 0) return m_DirectoryStructure.InvalidIndex();
  ...
  if (!bIsExpanded) return hPath;

  // Check all children + build a list of children we've already got
  int nChildCount = GetNumChildren(nItemIndex);
  DirHandle_t *pFoundHandles =
      (DirHandle_t *)_alloca(nChildCount * sizeof(DirHandle_t));
  ...
  DirHandle_t hChild = m_DirectoryStructure.FirstChild(hPath);
  for (; hChild != m_DirectoryStructure.InvalidIndex();
       hChild = m_DirectoryStructure.NextSibling(hChild)) {
...
bool CAssetTreeView::SelectFolder_R(int nItemID, const char *pPath) {
  ...
  str += '\\';
  if (Q_strnicmp(str, pPath, str.Length())) return false;
  ...
  for (int i = 0; i < nChildCount; ++i) {
    ...
    if (SelectFolder_R(nChildItemID, pPath)) return true;
  }
  return false;
}
...
class CAssetCache {
 public:
  ...
  AssetList_t FindAssetList(const char *pAssetType, const char *pSubDir,
                            int nExtCount, const char **ppExt);
 ...
 private:
  ...
  struct CachedAssetList_t {
    CachedAssetList_t() {}
    CachedAssetList_t(const char *pSearchSubDir, int nExtCount,
 ...
 private:
  bool AddFilesInDirectory(CachedAssetList_t &list, const char *pStartingFile,
                           const char *pFilePath, DirHandle_t hDirHandle,
                           float flStartTime, float flDuration);
  bool DoesExtensionMatch(CachedAssetList_t &list, const char *pFileName);
  void AddAssetToList(CachedAssetList_t &list, const char *pAssetName,
                      int nModIndex);
 ...
 private:
  ...
  static bool CachedAssetLessFunc(const CachedAssetList_t &src1,
                                  const CachedAssetList_t &src2);
};
...
//-----------------------------------------------------------------------------
void CAssetCache::AddAssetToList(CachedAssetList_t &list,
                                 const char *pAssetName, int nModIndex) {
...
bool CAssetCache::DoesExtensionMatch(CachedAssetList_t &info,
  ...
  for (int i = 0; i < nCount; ++i) {
    if (!Q_stricmp(info.m_Ext[i], pChildExt)) return true;
  }
...
//-----------------------------------------------------------------------------
bool CAssetCache::AddFilesInDirectory(CachedAssetList_t &list,
                                      const char *pStartingFile,
                                      const char *pFilePath,
                                      DirHandle_t hCurrentDir,
                                      float flStartTime, float flDuration) {
  ...
  while (pszFileName) {
    ...
    if (g_pFullFileSystem->FindIsDirectory(list.m_hFind)) {
      if (Q_strnicmp(pszFileName, ".", 2) && Q_strnicmp(pszFileName, "..", 3)) {
        DirHandle_t hDirHandle =
            list.m_pFileTree->AddSubDirectory(hCurrentDir, pszFileName);
      ...
      if (DoesExtensionMatch(list, pszFileName)) {
        ...
        for (int i = 0; i < nModCount; ++i) {
          if (!Q_strnicmp(pFullAssetPath, m_ModList[i].m_Path,
                          m_ModList[i].m_Path.Length())) {
            nModIndex = i;
            break;
          }
        }
...
bool CAssetCache::ContinueSearchForAssets(AssetList_t hList, float flDuration) {
  ...
  while (list.m_DirectoriesToCheck.Count()) {
    const char *pFilePath =
        list.m_DirectoriesToCheck[list.m_DirectoriesToCheck.Head()].m_DirName;
    DirHandle_t hCurrentDir =
        list.m_DirectoriesToCheck[list.m_DirectoriesToCheck.Head()]
            .m_hDirHandle;
    ...
    if (list.m_hFind == FILESYSTEM_INVALID_FIND_HANDLE) {
      ...
      pStartingFile =
          g_pFullFileSystem->FindFirstEx(pSearchString, "GAME", &list.m_hFind);
    } else {
      pStartingFile = g_pFullFileSystem->FindNext(list.m_hFind);
    }

    if (!AddFilesInDirectory(list, pStartingFile, pFilePath, hCurrentDir,
...
bool CAssetCache::BeginAssetScan(AssetList_t hList, bool bForceRescan) {
  ...
  // This case occurs if we stopped the picker previously while in the middle of
  // a scan
  ...
  list.m_DirectoriesToCheck[i].m_hDirHandle =
      list.m_pFileTree->GetRootDirectory();
  return true;
}
...
AssetList_t CAssetCache::FindAssetList(const char *pAssetType,
                                       const char *pSubDir, int nExtCount,
                                       const char **ppExt) {
  ...
  if (nIndex == m_CachedAssets.InvalidIndex()) {
    ...
    list.m_bAssetScanComplete = false;
    list.m_pFileTree =
        new CAssetTreeView(NULL, "FolderFilter", pAssetType, pSubDir);
  }
...
CAssetTreeView *CAssetCache::GetFileTree(AssetList_t hList) {
  if (hList == ASSET_LIST_INVALID) return NULL;
  return m_CachedAssets[(int)hList].m_pFileTree;
}

int CAssetCache::GetAssetCount(AssetList_t hList) const {
  if (hList == ASSET_LIST_INVALID) return 0;
  return m_CachedAssets[(int)hList].m_AssetList.Count();
}

const CAssetCache::CachedAssetInfo_t &CAssetCache::GetAsset(AssetList_t hList,
                                                            int nIndex) const {
...
static int __cdecl AssetBrowserModSortFunc(vgui::ListPanel *pPanel,
                                           const ListPanelItem &item1,
                                           const ListPanelItem &item2) {
  int nMod1 = item1.kv->GetInt("modIndex", -1);
  int nMod2 = item2.kv->GetInt("modIndex", -1);
  if (nMod1 != nMod2) return nMod1 - nMod2;
...
//-----------------------------------------------------------------------------
void CBaseAssetPicker::CreateStandardControls(vgui::Panel *pParent,
  ...
  m_hAssetList = s_AssetCache.FindAssetList(m_pAssetType, m_pAssetSubDir,
                                            nExtCount, ppExt);

  m_pAssetSplitter =
      new vgui::Splitter(pParent, "AssetSplitter", SPLITTER_MODE_HORIZONTAL, 1);
  ...
  // Rescan button
  m_pRescanButton = new Button(pSplitterTopSide, "RescanButton", "Rescan", this,
                               "AssetRescan");
  ...
  m_pSubDirCheck = new CheckButton(pSplitterTopSide, "SubDirCheck",
  ...
  m_pAssetBrowser->AddColumnHeader(1, "asset", m_pAssetType, 128,
...
void CBaseAssetPicker::ApplyUserConfigSettings(KeyValues *pUserConfig) {
  ...
  if (pMod && pMod[0]) {
    ...
    for (int i = 0; i < nCount; ++i) {
      ...
      if (Q_stricmp(pMod, modInfo.m_ModName)) continue;
...
void CBaseAssetPicker::GetUserConfigSettings(KeyValues *pUserConfig) {
  ...
  pUserConfig->SetString("mod", (m_nCurrentModFilter >= 0)
                                    ? s_AssetCache.ModInfo(m_nCurrentModFilter)
                                          .m_ModName.
                                          operator const char *()
                                    : "");
}
...
void CBaseAssetPicker::SetInitialSelection(const char *pAssetName) {
  // This makes it so the background list filling code will automatically select
  // this asset when it gets to it.
  m_SelectedAsset = pAssetName;

  if (pAssetName) {
    // Sometimes we've already refreshed our list with a bunch of cached
    // resources and the item is already in the list, so in that case just
    // select it here.
...
void CBaseAssetPicker::OnKeyCodeTyped(KeyCode code) {
  if ((code == KEY_UP) || (code == KEY_DOWN) || (code == KEY_PAGEUP) ||
      (code == KEY_PAGEDOWN)) {
    KeyValues *pMsg = new KeyValues("KeyCodeTyped", "code", code);
    vgui::ipanel()->SendMessage(m_pAssetBrowser->GetVPanel(), pMsg,
                                GetVPanel());
    pMsg->deleteThis();
  } else {
...
bool CBaseAssetPicker::IsAssetVisible(int nAssetIndex) {
  const CAssetCache::CachedAssetInfo_t &info =
      s_AssetCache.GetAsset(m_hAssetList, nAssetIndex);
  ...
  if (!Q_strcmp(pAssetName, m_SelectedAsset)) return true;

  if (m_Filter.Length() && !Q_stristr(pAssetName, m_Filter.Get())) return false;

  // Filter based on folder
  if (m_FolderFilter.Length() &&
      Q_strnicmp(pAssetName, m_FolderFilter.Get(), m_FolderFilter.Length()))
    return false;
...
void CBaseAssetPicker::AddAssetToList(int nAssetIndex) {
  ...
  bool bInRootDir =
      !strchr(info.m_AssetName, '\\') && !strchr(info.m_AssetName, '/');
  ...
  if (m_pAssetBrowser->GetSelectedItemsCount() == 0 &&
      !Q_strcmp(m_SelectedAsset, info.m_AssetName)) {
...
void CBaseAssetPicker::OnTick() {
  ...
  float flTime = m_bFirstAssetScan ? ASSET_LIST_DIRECTORY_INITIAL_SEARCH_TIME
                                   : ASSET_LIST_DIRECTORY_SEARCH_TIME;
  ...
  if (bFinished) {
    ...
    // and this point, this should still work since m_FolderFilter should be
    // updated
...
void CBaseAssetPicker::UpdateAssetColumnHeader() {
  ...
  Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)%s", m_pAssetType,
             m_nMatchingAssets, m_AssetList.Count(),
             m_bFinishedAssetListScan ? "" : " ...");
...
void CBaseAssetPicker::RefreshAssetList() {
  ...
  if ((m_pAssetBrowser->GetSelectedItemsCount() == 0) &&
      (m_pAssetBrowser->GetItemCount() > 0)) {
...
const char *CBaseAssetPicker::GetSelectedAsset(int nAssetIndex) {
  ...
  if (nSelectedAssetCount <= nAssetIndex || nAssetIndex < 0) return NULL;
...
int CBaseAssetPicker::GetSelectedAssetModIndex() {
  if (m_pAssetBrowser->GetSelectedItemsCount() == 0) return NULL;
...
CBaseAssetPickerFrame::CBaseAssetPickerFrame(vgui::Panel *pParent)
  ...
  m_pOpenButton =
      new Button(this, "OpenButton", "#FileOpenDialog_Open", this, "Open");
