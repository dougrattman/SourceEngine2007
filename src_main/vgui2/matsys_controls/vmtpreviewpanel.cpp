...
// Draw a sphere
//-----------------------------------------------------------------------------
void CVMTPreviewPanel::RenderSphere(const Vector &vCenter, float flRadius,
                                    int nTheta, int nPhi) {
  ...
  bool bIsUsingLightmap =
      m_Material->GetPropertyFlag(MATERIAL_PROPERTY_NEEDS_LIGHTMAP);
  bool bIsUsingBumpedLightmap =
      m_Material->GetPropertyFlag(MATERIAL_PROPERTY_NEEDS_BUMPED_LIGHTMAPS);
...
//-----------------------------------------------------------------------------
void CVMTPreviewPanel::DrawRectangle() {
...
//-----------------------------------------------------------------------------
void CVMTPreviewPanel::DrawSphere() {
...
void CVMTPreviewPanel::LookAt(const Vector &vecLookAt, float flRadius) {
  ...
  pRenderContext->Translate(-vecCameraOrigin[0], -vecCameraOrigin[1],
                            -vecCameraOrigin[2]);
}
...
void CVMTPreviewPanel::SetupProjectionMatrix(int nWidth, int nHeight) {
  ...
  float flZFar = ZFAR;
  float flApsectRatio =
      (nHeight != 0.0f) ? (float)nWidth / (float)nHeight : 100.0f;
...
static ITexture *GetPowerOfTwoFrameBufferTexture() {
  if (!s_pPowerOfTwoFrameBufferTexture) {
    s_pPowerOfTwoFrameBufferTexture.Init(vgui::MaterialSystem()->FindTexture(
        "_rt_PowerOfTwoFB", TEXTURE_GROUP_RENDER_TARGET));
  }
