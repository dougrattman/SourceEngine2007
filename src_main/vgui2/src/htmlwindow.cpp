...
// Explorer control.
//
// Abandon all hope ye who enter here, there be dragons... (in other words,
// hacks below)
...
class FS_IDispatch : public IDispatch {
 public:
  FS_IDispatch(FrameSite *fs) {
    m_fs = fs;
    m_bNewURL = 0;
    m_bOnLink = false;
    m_bAllowJavaScript = true;
  }
  ~FS_IDispatch() {}
...
class FS_DWebBrowserEvents2 : public DWebBrowserEvents2 {
 public:
  ...
  ULONG STDMETHODCALLTYPE Release() { return m_fs->Release(); }
  // IDispatch
  STDMETHODIMP GetIDsOfNames(REFIID r, OLECHAR **o, unsigned int i, LCID l,
                             DISPID *d) {
...
class FS_IAdviseSinkEx : public IAdviseSinkEx {
 public:
  ...
  void STDMETHODCALLTYPE OnSave() { m_fs->m_IAdviseSink2->OnSave(); }
  void STDMETHODCALLTYPE OnClose() { m_fs->m_IAdviseSink2->OnClose(); }
  // IAdviseSinkEx
...
//-----------------------------------------------------------------------------
// Purpose: forcefully destruct any HTML windows that are left around, so we can
// unhook from IE
//-----------------------------------------------------------------------------
  ...
  if (win) {
    switch (uMsg) {
      ...
      case WM_SETFOCUS:
        ...
        // send a message (rather than SetFocus() ) so that the IE hwnd has
        // focus for a couple frames and can process the incoming mouse click/kb
        // event
      ...
      default:
        return ::CallWindowProc((WNDPROC)win->GetIEWndProc(), hwnd, uMsg,
                                wParam, lParam);
        break;
    }
  }
  ...
  if (msg && win) {
    switch (msg->message) {
      ...
      default:
        /*	{
                        char dbtxt[200];
                        Q_snprintf(dbtxt,sizeof(dbtxt),"$$$$$$$$$$$$$$$ Got msg
...
BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM lParam) {
  ...
  if (win) {
    ...
    // work out the coords for this event in the CHILD WINDOWS space (not the
    // parents)
    ...
    if (::PtInRect(&win_rect, pt))  // if point is within client control
    {
      ...
      if (::PostMessage(hwnd, WM_SETFOCUS,
                        reinterpret_cast<uint>(win->GetIEHWND()),
                        lParam) == 0) {
        // DWORD err= GetLastError();
        DEBUG("msg2 not delivered");
        return true;
...
HtmlWindow::HtmlWindow(vgui::IHTMLEvents *events, vgui::VPANEL c, HWND parent,
                       bool AllowJavaScript, bool DirectToHWND) {
  m_oleObject = NULL;
  ...
  if (!DirectToHWND) {
    ...
    m_parent = CreateWindowEx(0, tmp, "", WS_CHILD | WS_DISABLED, 0, 0, 1, 1,
...
static const CLSID CLSID_MozillaBrowser = {
    ...
    0x3453,
    0x11D2,
    {0x93, 0xB9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};
...
void HtmlWindow::CreateBrowser(bool AllowJavaScript) {
  ...
  m_viewObject->SetAdvise(DVASPECT_CONTENT, ADVF_PRIMEFIRST,
                          c->m_IAdviseSinkEx);
...
//-----------------------------------------------------------------------------
// Purpose: called when the VGUI panel changes size, updates the IE component
// and the
...
void HtmlWindow::OnSize(int x, int y, int w_in, int h_in) {
  ...
  ::SetWindowPos(m_parent, HWND_TOP, panel_x, panel_y, w, h,
...
//-----------------------------------------------------------------------------
// Purpose: determines whether the control is visible or not, DEPRECATED - Use
// SetVisible()
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: generic function used to inform IE of its size and return the
// current size
//-----------------------------------------------------------------------------
...
void HtmlWindow::OnPaint(HDC hDC) {
  ...
  m_viewObject->Draw(DVASPECT_CONTENT, -1, NULL, NULL, hDC, hdcMem, prc, NULL,
                     NULL, 0);
  ...
  int chk = GetDIBits(hdcMem, hBitmap, 0L, (DWORD)h, (LPBYTE)NULL,
  ...
  if (chk != 0) {
    ...
    if (lpvBits) {
      ...
      chk = GetDIBits(hdcMem, hBitmap, 0L, (DWORD)h, (LPBYTE)lpvBits,
                      (LPBITMAPINFO)lpbi, (DWORD)DIB_RGB_COLORS);
      ...
      if (chk != 0) {
        ...
        // we could get windows to do it via CreateDIBitmap() but this is
        // quicker :) (and win95 can have problems with RGB format...)
        for (unsigned int i = 0; i < lpbi->bmiHeader.biSizeImage; i += 4) {
          unsigned char tmp = bmpBits[i];
...
//-----------------------------------------------------------------------------
// Purpose: handler for mouse click events, passes the message through to the IE
// component
//-----------------------------------------------------------------------------
void HtmlWindow::OnMouse(vgui::MouseCode code, MOUSE_STATE s, int x, int y) {
  ...
  // also tell it that it got focus
  if (m_ieHWND &&
      ::PostMessage(m_ieHWND, WM_SETFOCUS,
                    reinterpret_cast<uint>(m_oleObjectHWND), lParam) == 0) {
    // DWORD err= GetLastError();
    DEBUG("msg2 not delivered");
    return;
  }
...
//-----------------------------------------------------------------------------
// Purpose: empties the current HTML container of any HTML text (used in
// conjunction with AddText)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: passes a keydown event to the engine, used mainly for left and right
...
//-----------------------------------------------------------------------------
// Purpose: called when a new URL is requested, can stop the engine processing
// it.
//			This calls the m_events->OnStartURL callback if
// available.
//-----------------------------------------------------------------------------
...
void HtmlWindow::OnUpdate() {
  DEBUG("onupdate");
  // You need to query the IHTMLElement interface here to get it later, I have
  // no idea why...
  IDispatch *pDisp;
  m_webBrowser->get_Document(&pDisp);
  ...
  if (pDisp) {
    ...
    if (hret == S_OK) {
      ...
      assert(SUCCEEDED(hret));
      // connect the object to our sink
      m_connectionPoint->Advise(c->m_DHTMLDocumentEvents2,
                                &m_HtmlEventsAdviseCookie);
      cpContainer->Release();
...
//-----------------------------------------------------------------------------
// Purpose: called when the cursor enters a new HTML Element (i.e link,
// image,etc)
//-----------------------------------------------------------------------------
bool HtmlWindow::OnMouseOver() {
  ...
  if (hr == S_OK) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (SUCCEEDED(hr) &&
          pEvtObj)  // sometimes it can succeed BUT pEvtObj is NULL, nice eh?
      {
        IHTMLElement *el;
...
STDMETHODIMP FrameSite::QueryInterface(REFIID riid, void **ppv) {
  ...
  *ppv = NULL;
  ...
  else if (riid == IID_IOleItemContainer || riid == IID_IOleContainer ||
...
HRESULT FS_IDispatch::GetTypeInfo(unsigned int iTInfo, LCID lcid,
                                  ITypeInfo **ppTInfo) {
...
HRESULT FS_IDispatch::Invoke(DISPID dispIdMember, REFIID riid, LCID lcid,
  ...
  if (wFlags & DISPATCH_PROPERTYGET) {
    ...
    switch (dispIdMember) {
      ...
      case DISPID_AMBIENT_DLCONTROL:  // turn off javascript
        pVarResult->vt = VT_I4;
        pVarResult->lVal =
            (long)(DLCTL_DLIMAGES | DLCTL_VIDEOS | DLCTL_BGSOUNDS);
  ...
  switch (dispIdMember) {
    case DISPID_BEFORENAVIGATE2: {
      ...
      if (m_fs->m_window->OnStartURL((char *)_bstr_t(vurl->bstrVal),
                                     (char *)_bstr_t(targetFrame->bstrVal),
                                     !m_bNewURL)) {
        *pDispParams->rgvarg->pboolVal = VARIANT_FALSE;
      } else {  // this cancels the navigation
    ...
    case DISPID_DOCUMENTCOMPLETE: {
      // DON'T do this in DISPID_NAVIGATECOMPLETE2, the DHTML stuff is
      // incomplete in that event
      ...
      if (m_bNewURL == 0)  // only allow the first url to get through :)
      {
        ...
        m_fs->m_window
            ->OnUpdate();  // not finished the page, but a major "chunk" is done
      }
    }
...
#else
      if (!vgui::g_pSurface->SupportsFeature(
              vgui::ISurface::OPENING_NEW_HTML_WINDOWS))
#endif
      {
        m_bNewURL++;
...
HRESULT FS_IOleInPlaceFrame::SetActiveObject(
...
HRESULT FS_IOleInPlaceFrame::InsertMenus(HMENU hmenuShared,
                                         LPOLEMENUGROUPWIDTHS lpMenuWidths) {
  DEBUG("IOleInPlaceFrame::InsertMenus");
  return S_OK;
}

HRESULT FS_IOleInPlaceFrame::SetMenu(HMENU hmenuShared, HOLEMENU holemenu,
...
HRESULT FS_IOleInPlaceSiteWindowless::GetWindowContext(
...
HRESULT FS_IOleInPlaceSiteWindowless::OnInPlaceActivateEx(BOOL *pfNoRedraw,
                                                          DWORD dwFlags) {
...
HRESULT FS_IOleInPlaceSiteWindowless::GetDC(LPCRECT pRect, DWORD grfFlags,
                                            HDC *phDC) {
...
HRESULT FS_IOleInPlaceSiteWindowless::InvalidateRect(LPCRECT pRect,
                                                     BOOL fErase) {
...
HRESULT FS_IOleInPlaceSiteWindowless::OnDefWindowMessage(UINT, WPARAM, LPARAM,
                                                         LRESULT *) {
...
HRESULT FS_IOleItemContainer::ParseDisplayName(IBindCtx *pbc,
...
HRESULT FS_IOleItemContainer::EnumObjects(DWORD grfFlags,
                                          IEnumUnknown **ppenum) {
...
HRESULT FS_IOleControlSite::TransformCoords(POINTL *pPtlHimetric,
...
HRESULT FS_IOleControlSite::TranslateAccelerator(LPMSG pMsg,
                                                 DWORD grfModifiers) {
  DEBUG("IOleControlSite::TranslateAccelerator");
  return E_NOTIMPL;
}

HRESULT FS_IOleControlSite::ShowContextMenu(DWORD dwID, POINT *ppt,
...
HRESULT FS_IOleCommandTarget::QueryStatus(const GUID *pguidCmdGroup,
...
void STDMETHODCALLTYPE FS_IAdviseSink2::OnDataChange(FORMATETC *pFormatEtc,
                                                     STGMEDIUM *pgStgMed) {
  DEBUG("IAdviseSink::OnDataChange");
}

void STDMETHODCALLTYPE FS_IAdviseSink2::OnViewChange(DWORD dwAspect,
                                                     LONG lIndex) {
...
FS_IAdviseSinkEx::OnViewStatusChange(DWORD dwViewStatus) {
