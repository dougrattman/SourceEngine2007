...
class CInputWin32 : public IInputInternal {
 public:
  ...
  virtual bool InternalCursorMoved(int x,
                                   int y);  // expects input in surface space
  ...
  virtual void AssociatePanelWithInputContext(HInputContext context,
                                              VPANEL pRoot);
  ...
  // conversely whether it's being excluded from receiving messages
 ...
 private:
  ...
  struct InputContext_t {
    ...
    VPanel *_mouseFocus;  // the panel that has the current mouse focus - same
                          // as _mouseOver unless _mouseCapture is set
    VPanel *_mouseOver;   // the panel that the mouse is currently over, NULL if
                          // not over any vgui item

    VPanel *_mouseCapture;  // the panel that has currently captured mouse focus
    MouseCode m_MouseCaptureStartCode;  // The Mouse button which was pressed to
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(
    CInputWin32, IInput, VGUI_INPUT_INTERFACE_VERSION,
...
CInputWin32::CInputWin32() {
  ...
  _keyTrans[KEY_LCONTROL] =
      "\0\0KEY_LCONTROL"
      ", L";
  _keyTrans[KEY_RCONTROL] =
      "\0\0KEY_RCONTROL"
      ", L";
...
CInputWin32::~CInputWin32() { DestroyCandidateList(); }
...
void CInputWin32::InitInputContext(InputContext_t *pContext) {
  ...
  memset(pContext->_mouseDoublePressed, 0,
         sizeof(pContext->_mouseDoublePressed));
...
//-----------------------------------------------------------------------------
CInputWin32::InputContext_t *CInputWin32::GetInputContext(
    HInputContext context) {
  if (context == DEFAULT_INPUT_CONTEXT) return &m_DefaultInputContext;
...
// Associating NULL is valid; it disconnects the panel from the context
//-----------------------------------------------------------------------------
void CInputWin32::AssociatePanelWithInputContext(HInputContext context,
                                                 VPANEL pRoot) {
...
void CInputWin32::ActivateInputContext(HInputContext context) {
  Assert((context == DEFAULT_INPUT_CONTEXT) ||
         m_Contexts.IsValidIndex(context));
  m_hContext = context;
}
...
void CInputWin32::RunFrame() {
  ...
  if (pContext->_keyFocus) {
    ...
    if (IsChildOfModalPanel((VPANEL)pContext->_keyFocus)) {
      g_pIVgui->PostMessage((VPANEL)pContext->_keyFocus,
                            new KeyValues("KeyFocusTicked"), NULL);
    }
  }
  ...
  if (pContext->_mouseFocus) {
    ...
    if (IsChildOfModalPanel((VPANEL)pContext->_mouseFocus)) {
      g_pIVgui->PostMessage((VPANEL)pContext->_mouseFocus,
                            new KeyValues("MouseFocusTicked"), NULL);
    }
  }
  // Mouse has wandered "off" the modal panel, just force a regular arrow cursor
  // until it wanders back within the proper bounds
  ...
  if (pContext->_keyFocus != wantedKeyFocus) {
    if (pContext->_keyFocus != NULL) {
      ...
      // there may be out of order operations here, since we're directly calling
      // SendMessage, but we need to have focus messages happen immediately,
      // since otherwise mouse events happen out of order - more specifically,
      // they happen before the focus changes
...
VPanel *CInputWin32::CalculateNewKeyFocus() {
  ...
  if (g_pSurface->GetPopupCount() > 0) {
    ...
    while (nIndex) {
      ...
      if (top && top->IsPopup() && top->IsVisible() &&
        ...
        if (bIsVisible && !g_pSurface->IsMinimized((VPANEL)top)) break;
      }
  ...
  if (context._mouseOver == focus) {
    ...
    if ( m_nDebugMessages > 0 )
    {
            g_pIVgui->DPrintf2( "removing kb focus %s\n",
                    context._keyFocus ? pcontext._keyFocus->GetName() : "(no
    name)" );
    }
    */
...
void CInputWin32::PanelDeleted(VPANEL focus) {
  HInputContext i;
  for (i = m_Contexts.Head(); i != m_Contexts.InvalidIndex();
       i = m_Contexts.Next(i)) {
...
void CInputWin32::SetMouseFocus(VPANEL newMouseFocus) {
  ...
  if (newMouseFocus) {
    ...
    } while (wantsMouse && !isPopup && panel &&
             panel->GetParent());  // only consider panels that want mouse input
  }
  ...
  if ((VPANEL)pContext->_mouseOver != newMouseFocus ||
      (!pContext->_mouseCapture &&
       (VPANEL)pContext->_mouseFocus != newMouseFocus)) {
    ...
    if (pContext->_oldMouseFocus != NULL) {
      // only notify of entry if the mouse is not captured or we're the captured
      // panel
      if (!pContext->_mouseCapture ||
          pContext->_oldMouseFocus == pContext->_mouseCapture) {
        g_pIVgui->PostMessage((VPANEL)pContext->_oldMouseFocus,
                              new KeyValues("CursorExited"), NULL);
      }
    }
    ...
    if (pContext->_mouseOver != NULL) {
      ...
      // panel
      if (!pContext->_mouseCapture ||
          pContext->_mouseOver == pContext->_mouseCapture) {
        g_pIVgui->PostMessage((VPANEL)pContext->_mouseOver,
                              new KeyValues("CursorEntered"), NULL);
      }
    }

    // set where the mouse is currently over
    // mouse capture overrides destination
    VPanel *newFocus = pContext->_mouseCapture ? pContext->_mouseCapture
                                               : pContext->_mouseOver;

    if (m_nDebugMessages > 0) {
      g_pIVgui->DPrintf2("changing mouse focus from %s to %s\n",
                         pContext->_mouseFocus
                             ? pContext->_mouseFocus->GetName()
                             : "(no name)",
...
VPanel *CInputWin32::GetMouseFocusIgnoringModalSubtree() {
  ...
  if (!pContext->_rootPanel) {
    if (g_pSurface->IsCursorVisible() && g_pSurface->IsWithin(x, y)) {
      ...
      if (!focus) {
        focus = (VPanel *)((VPanel *)g_pSurface->GetEmbeddedPanel())
                    ->Client()
                    ->IsWithinTraverse(x, y, false);
      }
    }
  } else {
    focus = (VPanel *)((VPanel *)(pContext->_rootPanel))
                ->Client()
                ->IsWithinTraverse(x, y, false);
  }
  ...
  if (!pContext->_rootPanel) {
    if (g_pSurface->IsCursorVisible() && g_pSurface->IsWithin(x, y)) {
      ...
      for (int i = g_pSurface->GetPopupCount() - 1; i >= 0; i--) {
        ...
        VPanel *panel = popup;
        bool wantsMouse = panel->IsMouseInputEnabled() &&
                          IsChildOfModalSubTree((VPANEL)panel);
        bool isVisible = !g_pSurface->IsMinimized((VPANEL)panel);
...
// Passes in a keycode which allows hitting other mouse buttons w/o cancelling
// capture mode
void CInputWin32::SetMouseCaptureEx(VPANEL panel,
                                    MouseCode captureStartMouseCode) {
...
//			a NULL panel means that you want to clear the
// mouseCapture
//			MouseCaptureLost is sent to the panel that loses the
// mouse  capture
//-----------------------------------------------------------------------------
void CInputWin32::SetMouseCapture(VPANEL panel) {
  ...
  if (pContext->_mouseCapture && panel != (VPANEL)pContext->_mouseCapture) {
    g_pIVgui->PostMessage((VPANEL)pContext->_mouseCapture,
                          new KeyValues("MouseCaptureLost"), NULL);
  }
...
bool CInputWin32::IsChildOfModalSubTree(VPANEL panel) {
  ...
  if (pContext->m_pModalSubTree) {
    ...
    bool isChildOfModal =
        ((VPanel *)panel)->HasParent(pContext->m_pModalSubTree);
    if (isChildOfModal) {
...
bool CInputWin32::IsChildOfModalPanel(VPANEL panel,
  ...
  if (!checkModalSubTree) return true;
...
bool CInputWin32::WasKeyReleased(KeyCode code) {
  // changed from: only return true if the key was released and the passed in
  // panel matches the keyFocus
...
void CInputWin32::UpdateCursorPosInternal(int x, int y) {
  ...
  InputContext_t *pContext = GetInputContext(m_hContext);
  if (pContext->m_nCursorX == x && pContext->m_nCursorY == y) return;
...
void CInputWin32::SurfaceSetCursorPos(int x, int y) {
  if (g_pSurface->HasCursorPosFunctions())  // does the surface export cursor
...
void CInputWin32::SetCursorOveride(HCursor cursor) { _cursorOverride = cursor; }

HCursor CInputWin32::GetCursorOveride() { return _cursorOverride; }
...
bool CInputWin32::InternalCursorMoved(int x, int y) {
  g_pIVgui->PostMessage(
      (VPANEL)-1, new KeyValues("SetCursorPosInternal", "xpos", x, "ypos", y),
      NULL);
  return true;
}
...
void CInputWin32::PostCursorMessage() {
  ...
  if (pContext->m_bSetCursorExplicitly) {
    // NOTE m_bSetCursorExplicitly will be reset to false in
    // HandleExplicitSetCursor
    pContext->m_nCursorX = pContext->m_nExternallySetCursorX;
    pContext->m_nCursorY = pContext->m_nExternallySetCursorY;
  }

  if (pContext->m_nLastPostedCursorX == pContext->m_nCursorX &&
  ...
  if (pContext->_mouseCapture) {
    if (!IsChildOfModalPanel((VPANEL)pContext->_mouseCapture)) return;

    // the panel with mouse capture gets all messages
    g_pIVgui->PostMessage(
        (VPANEL)pContext->_mouseCapture,
        new KeyValues("CursorMoved", "xpos", pContext->m_nCursorX, "ypos",
                      pContext->m_nCursorY),
        NULL);
...
bool CInputWin32::InternalMousePressed(MouseCode code) {
  // True means we've processed the message and other code shouldn't see this
  // message
  bool bFilter = false;
  ...
  if (pContext->_mouseCapture &&
      IsChildOfModalPanel((VPANEL)pContext->_mouseCapture)) {
    ...
    bool captureLost = code == pContext->m_MouseCaptureStartCode ||
                       pContext->m_MouseCaptureStartCode == (MouseCode)-1;

    // the panel with mouse capture gets all messages
    g_pIVgui->PostMessage((VPANEL)pContext->_mouseCapture,
                          new KeyValues("MousePressed", "code", code), NULL);
    pTargetPanel = pContext->_mouseCapture;

    if (captureLost) {
      // this has to happen after MousePressed so the panel doesn't Think it got
      // a mouse press after it lost capture
      SetMouseCapture(NULL);
    }
  } else if ((pContext->_mouseFocus != NULL) &&
             IsChildOfModalPanel((VPANEL)pContext->_mouseFocus)) {
    // The faked mouse wheel button messages are specifically ignored by vgui
    if (code == MOUSE_WHEEL_DOWN || code == MOUSE_WHEEL_UP) return true;
  ...
  } else if (pContext->m_pModalSubTree &&
             pContext->m_pUnhandledMouseClickListener) {
    ...
    if (p) {
      ...
      if (isUnRestricted != isChildOfModal) {
        // The faked mouse wheel button messages are specifically ignored by
        // vgui
        if (code == MOUSE_WHEEL_DOWN || code == MOUSE_WHEEL_UP) return true;

        g_pIVgui->PostMessage(
            (VPANEL)pContext->m_pUnhandledMouseClickListener,
...
bool CInputWin32::InternalMouseReleased(MouseCode code) {
  ...
  if (pContext->_mouseCapture &&
    ...
    // the panel with mouse capture gets all messages
    g_pIVgui->PostMessage((VPANEL)pContext->_mouseCapture,
                          new KeyValues("MouseReleased", "code", code), NULL);
    bFilter = true;
  } else if ((pContext->_mouseFocus != NULL) &&
             IsChildOfModalPanel((VPANEL)pContext->_mouseFocus)) {
    // The faked mouse wheel button messages are specifically ignored by vgui
...
bool CInputWin32::InternalMouseWheeled(int delta) {
  ...
  if ((pContext->_mouseFocus != NULL) &&
    ...
    g_pIVgui->PostMessage((VPANEL)pContext->_mouseFocus,
                          new KeyValues("MouseWheeled", "delta", delta), NULL);
    bFilter = true;
  }
  return bFilter;
}
...
//-----------------------------------------------------------------------------
// Updates the internal key/mouse state associated with the current input
// context without sending messages
//-----------------------------------------------------------------------------
  ...
  switch (event.m_nType) {
    ...
    case IE_ButtonDoubleClicked: {
      ...
      if (IsMouseCode(code)) {
        ...
        state = (event.m_nType == IE_ButtonReleased) ? vgui::BUTTON_RELEASED
                                                     : vgui::BUTTON_PRESSED;
...
bool CInputWin32::PostKeyMessage(KeyValues *message) {
  InputContext_t *pContext = GetInputContext(m_hContext);
  if ((pContext->_keyFocus != NULL) &&
      IsChildOfModalPanel((VPANEL)pContext->_keyFocus)) {
...
struct IMESettingsTransform {
  ...
  bool ConvMatches(DWORD convFlags) {
    // To match, the active flags have to have none of the remove flags and have
    // to have all of the "add" flags
...
static IMESettingsTransform g_ConversionMode_CHT_ToChinese(
    IME_CMODE_ALPHANUMERIC, IME_CMODE_NATIVE | IME_CMODE_LANGUAGE, 0, 0);
static IMESettingsTransform g_ConversionMode_CHT_ToEnglish(
    IME_CMODE_NATIVE | IME_CMODE_LANGUAGE, IME_CMODE_ALPHANUMERIC, 0, 0);
...
static IMESettingsTransform g_SentenceMode_JP_BiasNames(
    0, 0,
    IME_SMODE_NONE | IME_SMODE_PHRASEPREDICT | IME_SMODE_SINGLECONVERT |
        IME_SMODE_AUTOMATIC | IME_SMODE_CONVERSATION,
    IME_SMODE_PLAURALCLAUSE);
  ...
  */
  ...
  if (hIMC) {
    if (flags & VGUI_GCS_RESULTSTR) {
      ...
      int len = ImmGetCompositionStringW(hIMC, GCS_RESULTSTR, (LPVOID)tempstr,
                                         sizeof(tempstr));
      if (len > 0) {
        if ((len % 2) != 0) len++;
    ...
    if (flags & VGUI_GCS_COMPSTR) {
      ...
      int len = ImmGetCompositionStringW(hIMC, GCS_COMPSTR, (LPVOID)tempstr,
                                         sizeof(tempstr));
      if (len > 0) {
        if ((len % 2) != 0) len++;
        int numchars = len / sizeof(wchar_t);
...
void CInputWin32::RegisterKeyCodeUnhandledListener(VPANEL panel) {
  ...
  if (pContext->m_KeyCodeUnhandledListeners.Find(listener) ==
      pContext->m_KeyCodeUnhandledListeners.InvalidIndex()) {
...
void CInputWin32::OnKeyCodeUnhandled(int keyCode) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    g_pIVgui->PostMessage((VPANEL)listener,
                          new KeyValues("KeyCodeUnhandled", "code", keyCode),
                          NULL);
  }
}
...
void CInputWin32::PostModalSubTreeMessage(VPanel *subTree, bool state) {
  InputContext_t *pContext = GetInputContext(m_hContext);
  if (pContext->m_pModalSubTree == NULL) return;
...
//  if restrictMessagesToSubTree is true, then mouse and kb messages are only
//  routed to the subTree and it's children and mouse/kb focus
//   can only be on one of the subTree children, if a mouse click occurs outside
//   of the subtree, and "UnhandledMouseClick" message is sent to
//   unhandledMouseClickListener panel if it's set
//  if restrictMessagesToSubTree is false, then mouse and kb messages are routed
//  as normal except that they are not routed down into the subtree
//   however, if a mouse click occurs outside of the subtree, and
//   "UnhandleMouseClick" message is sent to unhandledMouseClickListener panel
//   if it's set
void CInputWin32::SetModalSubTree(VPANEL subTree,
                                  VPANEL unhandledMouseClickListener,
                                  bool restrictMessagesToSubTree /*= true*/) {
  InputContext_t *pContext = GetInputContext(m_hContext);
  if (!pContext) return;
...
void CInputWin32::SetModalSubTreeReceiveMessages(bool state) {
  ...
  if (!pContext->m_pModalSubTree) return;

  pContext->m_bRestrictMessagesToModalSubTree = state;
}

bool CInputWin32::ShouldModalSubTreeReceiveMessages() const {
  InputContext_t *pContext =
      const_cast<CInputWin32 *>(this)->GetInputContext(m_hContext);
