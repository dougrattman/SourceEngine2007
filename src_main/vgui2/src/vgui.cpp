...
static const int WARN_PANEL_NUMBER = 32768;  // in DEBUG if more panels than
                                             // this are created then throw an
                                             // Assert, helps catch panel leaks
...
struct MessageItem_t {
  ...
  float _arrivalTime;  // time at which the message should be passed on to the
                       // recipient
...
class CVGui : public CTier3AppSystem<IVGui> {
 ...
 public:
  ...
  bool IsDispatchingMessages() { return m_InDispatcher; }
 ...
 private:
  ...
  inline bool IsReentrant() const { return m_nReentrancyCount > 0; }
  ...
  // timing queue, holds all the messages that have to arrive at a specified
  // time
  CUtlPriorityQueue<MessageItem_t> m_DelayedMessageQueue;
};

CVGui g_VGui;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CVGui, IVGui, VGUI_IVGUI_INTERFACE_VERSION,
                                  g_VGui);

bool IsDispatchingMessageQueue() { return g_VGui.IsDispatchingMessages(); }
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (m_HandleTable.IsHandleValid(h)) {
      ...
      Msg("\tpanel '%s' of type '%s' leaked\n",
          g_pIPanel->GetName((VPANEL)pPanel),
          ((VPanel *)pPanel)->GetClassName());
    }
  }
}
...
CVGui::Context_t *CVGui::GetContext(HContext context) {
  if (context == DEFAULT_VGUI_CONTEXT) return &m_DefaultContext;
...
void CVGui::AssociatePanelWithContext(HContext context, VPANEL pRoot) {
  Assert(context != DEFAULT_VGUI_CONTEXT);
  g_pInput->AssociatePanelWithInputContext(GetContext(context)->m_hInputContext,
                                           pRoot);
}
...
HPanel CVGui::PanelToHandle(VPANEL panel) {
  if (panel) return ((VPanel *)panel)->GetHPanel();
      ...
      for (panelIndex = 0; panelIndex < nCount; ++panelIndex) {
        ...
        if (ipanel)
          fprintf(file1, "panel %d: name: %s   classname: %s\n", panelIndex,
                  ipanel->GetName(), ipanel->GetClassName());
        else
...
//-----------------------------------------------------------------------------
// Purpose: Adds the panel to a tick signal list, so the panel receives a
// message every frame
//-----------------------------------------------------------------------------
...
bool CVGui::DispatchMessages() {
  ...
  int nPassCount = 0;
  while (nPassCount < 2) {
    while (m_MessageQueue.Count() > 0 || (m_SecondaryQueue.Count() > 0) ||
           bUsingDelayedQueue) {
      // get the first message
      MessageItem_t *messageItem = NULL;
      int messageIndex = 0;

      // use the secondary queue until it empties. empty it after each message
      // in the primary queue. this makes primary messages completely resolve
      ...
      if (bUsingSecondaryQueue) {
        ...
        if (!messageItem || messageItem->_arrivalTime > time) {
          // no more items in the delayed message queue, move to the system
          // queue
      ...
      if (messageItem->_messageTo == 0xFFFFFFFF) {
        ...
        VPanel *vto =
            (VPanel *)g_pIVgui->HandleToPanel(messageItem->_messageTo);
        if (vto) {
          //			Msg("Sending message: %s to %s\n", params ?
          // params->GetName() : "\"\"", vto->GetName() ? vto->GetName() :
          //"\"\"");
      ...
      // we can't reference the messageItem pointer anymore since the queue
      // might have moved in memory
      if (params) {
        params->deleteThis();
      }
...
//-----------------------------------------------------------------------------
void CVGui::PostMessage(VPANEL target, KeyValues *params, VPANEL from,
                        float delay) {
  ...
  messageItem._messageTo =
      (target != (VPANEL)-1) ? g_pIVgui->PanelToHandle(target) : 0xFFFFFFFF;
...
void CVGui::ShutdownMessage(unsigned int shutdownID) {
  ...
  for (int i = 0; i < ((VPanel *)panel)->GetChildCount(); i++) {
    g_pIVgui->PostMessage((VPANEL)((VPanel *)panel)->GetChild(i),
  ...
  g_pIVgui->PostMessage(
      panel, new KeyValues("ShutdownRequest", "id", shutdownID), NULL);
}
