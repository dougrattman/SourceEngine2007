...
class CWin32Surface : public ISurface {
 public:
  ...
  virtual void DrawPrintText(const wchar_t *, int textLen,
                             FontDrawType_t drawType = FONT_DRAW_DEFAULT);
  virtual void DrawUnicodeChar(wchar_t wch,
                               FontDrawType_t drawType = FONT_DRAW_DEFAULT);
  virtual void DrawUnicodeString(const wchar_t *pwString,
                                 FontDrawType_t drawType = FONT_DRAW_DEFAULT);
  ...
  virtual void DrawSetTextureFile(int id, const char *filename,
                                  int hardwareFilter, bool forceReload = false);
  ...
  virtual void DrawSetTextureRGBAEx(int id, const unsigned char *rgba, int wide,
                                    int tall, ImageFormat imageFormat);
  ...
  virtual void SetNotifyIcon(VPANEL context, HTexture icon,
                             VPANEL panelToReceiveMessages, const char *text);
  ...
  virtual void CreatePopup(VPANEL panel, bool minimised, bool showTaskbarIcon,
                           bool disabled, bool mouseInput, bool kbInput);
  ...
  virtual void ClearTemporaryFontCache();
  ...
  virtual void DrawTexturedPolyLine(
      const Vertex_t *p,
      int n);  // (Note: this connects the first and last points).
  virtual void DrawTexturedSubRect(int x0, int y0, int x1, int y1, float texs0,
                                   float text0, float texs1, float text1);
  ...
  // (NOT SUPPORTED BY DEFAULT SURFACE )!!!
 ...
 private:
  ...
  void InternalSchemeSettingsTraverse(VPANEL panel,
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CWin32Surface, ISurface,
                                  VGUI_SURFACE_INTERFACE_VERSION, g_Surface);
...
static LRESULT CALLBACK staticProc(HWND hwnd, UINT msg, WPARAM wparam,
                                   LPARAM lparam);
...
class CSurfaceDragDropTarget : public IDropTarget {
 ...
 private:
  ...
  virtual ULONG STDMETHODCALLTYPE AddRef() { return ++_refCount; }

  virtual ULONG STDMETHODCALLTYPE Release() { return --_refCount; }

  virtual HRESULT STDMETHODCALLTYPE DragEnter(IDataObject *pDataObject,
...
static void staticGenerateIconForTexture(Texture *texture, HDC hdc) {
  ...
  texture->_icon = (HICON)::LoadImage(NULL, buf, IMAGE_ICON, 16, 16,
                                      LR_LOADFROMFILE | LR_DEFAULTSIZE);
  if (texture->_icon) return;
...
CWin32Surface::~CWin32Surface() {
  ...
  for (int i = 0; i < m_VGuiSurfaceTextures.MaxElement(); i++) {
    if (!m_VGuiSurfaceTextures.IsValidIndex(i)) continue;
...
void CWin32Surface::DrawTexturedSubRect(int x0, int y0, int x1, int y1,
...
void CWin32Surface::DrawTexturedPolygon(int n, Vertex_t *pVertices) {
  ...
  if (pt) {
    ...
    // Set the pen color to the current brush color to avoid strange outlines on
    // our polygons
    DrawSetTextColor(GetRValue(brushcolor), GetGValue(brushcolor),
                     GetBValue(brushcolor), 255);
...
//-----------------------------------------------------------------------------
void *CWin32Surface::QueryInterface(const char *pInterfaceName) { return NULL; }

//-----------------------------------------------------------------------------
// Initialization and shutdown...
//-----------------------------------------------------------------------------
InitReturnVal_t CWin32Surface::Init() { return INIT_FAILED; }
...
void CWin32Surface::SetEmbeddedPanel(VPANEL panel) {
  ...
  // send a message to ourselves every 50ms (20Hz) so that we don't block in the
  // message queue too long
  ::SetTimer(PLAT(_currentContextPanel)->hwnd, 0, 50, (TIMERPROC)NULL);

  // fonts initialization
  char language[64];
  if (g_pSystem->GetRegistryString(
...
// Lower level char drawing code, call DrawGet then pass in info to DrawRender
bool CWin32Surface::DrawGetUnicodeCharRenderInfo(wchar_t ch,
                                                 CharRenderInfo &info) {
...
void CWin32Surface::PushMakeCurrent(VPANEL panel, bool useInsets) {
  ...
  if (_currentContextPanel == panel) {
    ...
    ::SetViewportOrgEx(PLAT(_currentContextPanel)->hdc, 0 + inset[0],
                       0 + inset[1], 0);
  } else {
    // child window, so set win32 up so all subsequent drawing calls are done in
    // local space
    ::SetViewportOrgEx(PLAT(_currentContextPanel)->hdc,
                       (absPanel[0] + inset[0]) - absThis[0],
                       (absPanel[1] + inset[1]) - absThis[1], 0);
  }
  ...
  ::SelectObject(PLAT(_currentContextPanel)->hdc,
                 PLAT(_currentContextPanel)->clipRgn);
}
...
// Purpose:
//-----------------------------------------------------------------------------
void CWin32Surface::SetNotifyIcon(VPANEL context, HTexture iconID,
...
void CWin32Surface::DrawFilledRect(int x0, int y0, int x1, int y1) {
  ...
  ExtTextOut(PLAT(_currentContextPanel)->hdc, 0, 0, ETO_OPAQUE, &rect, NULL, 0,
             NULL);
}
...
void CWin32Surface::DrawPrintText(
  ...
  //	ExtTextOut(_currentContextPanel->Plat()->hdc, 0, 0, 0, NULL, text,
  // textLen, NULL);
}

void CWin32Surface::DrawUnicodeString(
    const wchar_t *pwString, FontDrawType_t drawType /*= FONT_DRAW_DEFAULT*/) {
  Assert(pwString);
  if (!pwString) return;
...
//			current font & color
//-----------------------------------------------------------------------------
void CWin32Surface::DrawUnicodeChar(
    wchar_t wch, FontDrawType_t drawType /*= FONT_DRAW_DEFAULT*/) {
  ...
  if (m_bSupportsUnicode) {
    ...
    ExtTextOutA(PLAT(_currentContextPanel)->hdc, 0, 0, 0, NULL, mbcs,
                strlen(mbcs), NULL);
  }
}
...
IHTML *CWin32Surface::CreateHTMLWindow(vgui::IHTMLEvents *events,
                                       VPANEL context) {
  ...
  HtmlWindow *IE =
      new HtmlWindow(events, context, PLAT(parent)->hwnd, m_bAllowJavaScript,
                     SupportsFeature(DIRECT_HWND_RENDER));
...
void CWin32Surface::PaintHTMLWindow(IHTML *htmlwin) {
  /*	HtmlWindow *IE = dynamic_cast<HtmlWindow *>(htmlwin);
          ...
          if(IE)
          {
                  ...
                  //BOOL r = BitBlt(_currentContextPanel->Plat()->hdc, 0, 0, w,
     h, bit, 0, 0, SRCCOPY);
                  // don't release the bitmap's HDC, its "cached" by the IE
     control
...
//-----------------------------------------------------------------------------
void CWin32Surface::DrawSetTextureRGBA(int id, const unsigned char *rgba,
                                       int wide, int tall, int hardwareFilter,
                                       bool forceReload) {
...
// Purpose: maps a texture from memory to an id, and uploads it into the engine
//-----------------------------------------------------------------------------
void CWin32Surface::DrawSetTextureRGBAEx(int id, const unsigned char *rgba,
                                         int wide, int tall,
...

  {
    ...
    texture->_icon = NULL;
    texture->_dib = NULL;
    texture->_bitmap = staticCreateBitmapHandle(texture->_wide, texture->_tall,
...
int CWin32Surface::DrawGetTextureId(char const *filename) {
  ...
  while (i != m_VGuiSurfaceTextures.InvalidIndex()) {
    Texture *texture = &m_VGuiSurfaceTextures[i];
    if (!Q_stricmp(filename, texture->_filename)) return texture->_id;
...
//-----------------------------------------------------------------------------
// Purpose: Maps a texture file to an id, and makes it the current drawing
// texture
//			tries to load as a .tga first, and if not found as a
//.bmp
//-----------------------------------------------------------------------------
void CWin32Surface::DrawSetTextureFile(int id, const char *filename,
                                       int hardwareFilter,
                                       bool forceReload /*= false*/) {
  Texture *texture = GetTextureById(id);
...
void CWin32Surface::DrawTexturedRect(int x0, int y0, int x1, int y1) {
  ...
  if (m_pCurrentTexture->_bMask) {
    ...
    oldObject =
        ::SelectObject(PLAT(_currentContextPanel)->textureDC, bitmap_mask);
    ::StretchBlt(PLAT(_currentContextPanel)->hdc, x0, y0, x1 - x0, y1 - y0,
                 PLAT(_currentContextPanel)->textureDC, 0, 0, wide, tall,
                 SRCAND);
...
HBITMAP staticCreateBitmapHandle(int wide, int tall, HDC hdc, int bpp,
  ...
  HBITMAP hRet;
  hRet = CreateDIBSection(hdc, (BITMAPINFO *)&bitmapInfoHeader, DIB_RGB_COLORS,
                          dib, 0, 0);
  if (!hRet) Error("staticCreateBitmapHandle: can't create DIB");
...
bool CWin32Surface::LoadBMP(Texture *texture, const char *filename) {
  ...
  if (bmfHeader.bfType == DIB_HEADER_MARKER) {
    ...
    char *pDIB = (LPSTR)::GlobalLock((HGLOBAL)hDIB);
    {
      ...
      unsigned char *rgba = (unsigned char *)(pDIB + sizeof(BITMAPINFOHEADER) +
                                              256 * sizeof(RGBQUAD));
...
bool CWin32Surface::LoadTGA(Texture *texture, const char *filename) {
  ...
  texture->_icon = NULL;
  texture->_bitmap = staticCreateBitmapHandle(tgaHeader.width, tgaHeader.height,
                                              PLAT(_currentContextPanel)->hdc,
                                              32, &texture->_dib);
  texture->_bMask = bMask;
  if (bMask)
    texture->_maskBitmap = staticCreateBitmapHandle(
        tgaHeader.width, tgaHeader.height, PLAT(_currentContextPanel)->hdc, 32,
        &texture->_maskDib);
  else
...
void CWin32Surface::BringToFront(VPANEL panel) {
  ...
  if (panel && PLAT(panel)) {
    // this should trigger a WM_SETFOCUS message which will move the window to
    // the top
...
//-----------------------------------------------------------------------------
// Purpose: puts the thread that created the specified window into the
// foreground
...
// behind all topmost windows). This flag has no effect if the window is already
...
void CWin32Surface::SetTitle(VPANEL panel, const wchar_t *title) {
  ...
  if (panel) {
    if (m_bSupportsUnicode) {
      ...
      char mbcs[512];
      ::WideCharToMultiByte(CP_ACP, 0, title, -1, mbcs, sizeof(mbcs), NULL,
                            NULL);
...
// Purpose:
//-----------------------------------------------------------------------------
void CWin32Surface::CreatePopup(VPANEL panel, bool minimised,
  ...
  // WS_SYSMENU and WS_MINIMIZEBOX flags are there simply to make icon appear in
  // taskbar WS_EX_TOOLWINDOW hides the taskbar/ALT-TAB button
  ...
  plat->hwnd = CreateWindowEx(style_ex, "Surface", "", style, x, y, wide, tall,
  ...
  // the first time PlaySound is called there is 300ms hang, so this forces it
  // to happen at startup instead of while the user is trying to do something we
  // can't do this before a window created else it'll hang
...
bool CWin32Surface::RecreateContext(VPANEL panel) {
  if (panel && PLAT(panel)) {
    ...
    if ((wide > plat->bitmapSize[0]) || (tall > plat->bitmapSize[1]) ||
      ...
      plat->bitmap =
          ::CreateCompatibleBitmap(plat->hwndDC, wide + 100, tall + 100);
...
bool CWin32Surface::ShouldPaintChildPanel(VPANEL childPanel) {
  // don't Paint children as part of the normal process, handle them in with
  // WM_PAINT messages instead
...
void CWin32Surface::ApplyChanges() {
  for (int i = 0; i < GetPopupCount(); i++) {
    ...
    if ((x != sx) || (y != sy) || (wide != swide) || (tall != stall)) {
      ::SetWindowPos(Plat->hwnd, 0, x, y, wide, tall,
    ...
    {
      // check to see if embedded VPanel is visible, if so then show the win32
      // window
      if (((VPanel *)panel)->IsVisible()) {
    ...
    // if the win32 window changed size and is visible, then the context needs
    // to be recreated
    if (((wide != swide) || (tall != stall)) &&
        ((VPanel *)panel)->IsVisible()) {
      RecreateContext(panel);
    }
  }
}
...
//			must be done child before parent
//-----------------------------------------------------------------------------
void CWin32Surface::InternalSchemeSettingsTraverse(
...
void CWin32Surface::SolveTraverse(VPANEL panel, bool forceApplySchemeSettings) {
  ...
  InternalSchemeSettingsTraverse(
      panel,
      forceApplySchemeSettings);  // do apply scheme settings, child to parent
...
void CWin32Surface::SetCursor(HCursor cursor) {
  switch (cursor) {
    ...
    case dc_blank:  // don't touch the cursor, just stick with what windows is
                    // currently using
    {
      return;
      break;
...
bool CWin32Surface::HasFocus() {
  ...
  for (int i = 0; i < GetPopupCount(); i++) {
    VPANEL panel = GetPopup(i);
    if (focus == PLAT(panel)->hwnd) return true;
  }
...
//			Uses the windows call to do this, instead of doing it
// procedurally
//-----------------------------------------------------------------------------
  ...
  if (panel && PLAT(panel)) {
    ...
    if (focus && ((VPanel *)focus)->HasParent((VPanel *)panel)) {
      ...
      if (g_pInput->GetAppModalSurface() &&
          !((VPanel *)focus)
               ->HasParent((VPanel *)g_pInput->GetAppModalSurface())) {
        // trying to set focus to something that's not the modal panel, set it
        // back
...
//-----------------------------------------------------------------------------
bool CWin32Surface::SetFontGlyphSet(HFont font, const char *windowsFontName,
...
//-----------------------------------------------------------------------------
void CWin32Surface::GetTextSize(HFont font, const wchar_t *text, int &wide,
                                int &tall) {
...
//-----------------------------------------------------------------------------
// Purpose: Pre-compiled bitmap font support for game engine - not implemented
// for GDI
//-----------------------------------------------------------------------------
...
void CWin32Surface::SetBitmapFontName(const char *pName,
...
bool CWin32Surface::SetBitmapFontGlyphSet(HFont font,
...
void CWin32Surface::ClearTemporaryFontCache() { Assert(0); }

void CWin32Surface::DrawSetTextScale(float sx, float sy) { Assert(0); }
void CWin32Surface::SetPanelForInput(VPANEL vpanel) { Assert(0); }

void CWin32Surface::DrawFilledRectFade(int x0, int y0, int x1, int y1,
                                       unsigned int alpha0, unsigned int alpha1,
                                       bool bHorizontal) {
  Assert(0);
}
...
//-----------------------------------------------------------------------------
void CWin32Surface::GetAbsoluteWindowBounds(int &x, int &y, int &wide,
                                            int &tall) {
...
void CWin32Surface::RunFrame() {
  ...
  if (!g_pIVgui->GetShouldVGuiControlSleep()) {
    // if vgui doesn't control sleeping, then make sure we don't block in this
    // loop
  ...
  // there is a timer set to force this loop to run at least 20Hz, which should
  // be fine for the desktop
  do {
  ...
  switch (lparam) {
    case WM_LBUTTONDOWN: {
      ...
      if (g_Surface.GetNotifyPanel()) {
        g_pIVgui->PostMessage(
            g_pSurface->GetNotifyPanel(),
            new KeyValues("NotifyIconMsg", "msg", "WM_LBUTTONDOWN"), NULL);
      }
      break;
    }
    ...
    case WM_LBUTTONDBLCLK: {
      ...
      if (g_pSurface->GetNotifyPanel()) {
        g_pIVgui->PostMessage(
            g_pSurface->GetNotifyPanel(),
            new KeyValues("NotifyIconMsg", "msg", "LBUTTONDBLCLK"), NULL);
      }
      break;
    }
    ...
    case WM_CONTEXTMENU:  // win2k/ME
    {
      ...
      if (g_pSurface->GetNotifyPanel()) {
        g_pIVgui->PostMessage(
            g_pSurface->GetNotifyPanel(),
            new KeyValues("NotifyIconMsg", "msg", "CONTEXTMENU"), NULL);
      }
      break;
    }
    default:
...
static LRESULT CALLBACK staticProc(HWND hwnd, UINT msg, WPARAM wparam,
  ...
  if (msg == WM_ENDSESSION && wparam == (WPARAM)TRUE) {
    ...
    if (g_pSurface->GetEmbeddedPanel()) {
      g_pIPanel->SendMessage(g_pSurface->GetEmbeddedPanel(),
                             new KeyValues("WindowsEndSession"), NULL);
    }
    return 0;
  }
  ...
  switch (msg) {
      ...
      if (!bRepeating) {
        if (code == VK_CAPITAL || code == VK_SCROLL || code == VK_NUMLOCK) {
          ...
          switch (code) {
            default:
            case VK_CAPITAL:
              toggleCode = KEY_CAPSLOCKTOGGLE;
              break;
            case VK_SCROLL:
              toggleCode = KEY_SCROLLLOCKTOGGLE;
              break;
            case VK_NUMLOCK:
              toggleCode = KEY_NUMLOCKTOGGLE;
              break;
          };
    ...
    case WM_ERASEBKGND: {
      ...
      // simply this means.. this call only happens when windows wants the
      // Repaint  and WM_PAINT gets called just after this to do the real
      // painting
    ...
    case WM_PAINT: {
      // surface was by repainted vgui or by windows itself, do the repainting
      // all repainting  will goes through here and nowhere else
      ...
      //);
      //			g_pInput->PostMessage( panel, new
      // KeyValues("Paint"), NULL );

      // this relies on platTick() being called BEFORE dispatchMessages(), so
      // that painting occurs
      ...
      /*	char dbtxt[200];
              Q_snprintf(dbtxt,200,"paint:%p --
         %p\n",hwnd,g_Surface.GetHTMLWindow(0)->GetIEHWND()); OutputDebugString(
         dbtxt );
      */
