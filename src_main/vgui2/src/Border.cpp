...
//-----------------------------------------------------------------------------
void Border::Paint(int x, int y, int wide, int tall, int breakSide,
                   int breakStart, int breakEnd) {
  // iterate through and draw all lines
  // draw left
  int i;
  for (i = 0; i < _sides[SIDE_LEFT].count; i++) {
    line_t *line = &(_sides[SIDE_LEFT].lines[i]);
    g_pSurface->DrawSetColor(line->col[0], line->col[1], line->col[2],
                             line->col[3]);
    ...
    if (breakSide == SIDE_LEFT) {
      ...
      if (breakStart > 0) {
        ...
        g_pSurface->DrawFilledRect(x + i, y + line->startOffset, x + i + 1,
                                   y + breakStart);
      }

      if (breakEnd < (tall - line->endOffset)) {
        // draw after break end
        g_pSurface->DrawFilledRect(x + i, y + breakEnd + 1, x + i + 1,
                                   tall - line->endOffset);
      }
    } else {
      g_pSurface->DrawFilledRect(x + i, y + line->startOffset, x + i + 1,
  ...
  for (i = 0; i < _sides[SIDE_TOP].count; i++) {
    ...
    if (breakSide == SIDE_TOP) {
      ...
      if (breakStart > 0) {
        // draw before the break Start
        g_pSurface->DrawFilledRect(x + line->startOffset, y + i, x + breakStart,
                                   y + i + 1);
      }

      if (breakEnd < (wide - line->endOffset)) {
        // draw after break end
        g_pSurface->DrawFilledRect(x + breakEnd + 1, y + i,
                                   wide - line->endOffset, y + i + 1);
      }
    } else {
      g_pSurface->DrawFilledRect(x + line->startOffset, y + i,
                                 wide - line->endOffset, y + i + 1);
    }
  }
  ...
  for (i = 0; i < _sides[SIDE_RIGHT].count; i++) {
    ...
    g_pSurface->DrawSetColor(line->col[0], line->col[1], line->col[2],
                             line->col[3]);
    g_pSurface->DrawFilledRect(wide - (i + 1), y + line->startOffset,
                               (wide - (i + 1)) + 1, tall - line->endOffset);
  }
  ...
  for (i = 0; i < _sides[SIDE_BOTTOM].count; i++) {
    ...
    g_pSurface->DrawSetColor(line->col[0], line->col[1], line->col[2],
                             line->col[3]);
    g_pSurface->DrawFilledRect(x + line->startOffset, tall - (i + 1),
                               wide - line->endOffset, (tall - (i + 1)) + 1);
  }
}
...
void Border::ApplySchemeSettings(IScheme *pScheme, KeyValues *inResourceData) {
  ...
  m_eBackgroundType =
      (backgroundtype_e)inResourceData->GetInt("backgroundtype");
}

//-----------------------------------------------------------------------------
// Purpose: parses scheme data
//-----------------------------------------------------------------------------
void Border::ParseSideSettings(int side_index, KeyValues *inResourceData,
                               IScheme *pScheme) {
  ...
  int count = 0;
  KeyValues *kv;
  for (kv = inResourceData->GetFirstSubKey(); kv != NULL;
       kv = kv->GetNextKey()) {
    count++;
  }
