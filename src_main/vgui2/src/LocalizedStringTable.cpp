...
class CLocalizedStringTable : public vgui::ILocalize {
 public:
  ...
  virtual bool AddFile(const char *fileName, const char *pPathID,
  ...
  void AddString(const char *tokenName, wchar_t *unicodeString,
                 const char *fileName);
 ...
 private:
  ...
  struct localizedstring_t {
    ...
    union {
      ...
      char const
          *pszValueString;  // Used only if nameIndex == INVALID_STRING_INDEX
    };
    CUtlSymbol filename;
  };
  ...
  struct LocalizationFileInfo_t {
    ...
    static bool LessFunc(const LocalizationFileInfo_t &lhs,
                         const LocalizationFileInfo_t &rhs) {
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CLocalizedStringTable, ILocalize,
...
CLocalizedStringTable::CLocalizedStringTable()
    : m_Lookup(0, 0, SymLess),
      m_Names(1024),
      m_Values(2048),
      m_FastValueLookup(0, 0, FastValueLessFunc) {
...
bool CLocalizedStringTable::AddFile(const char *szFileName, const char *pPathID,
  ...
  if (langptr) {
    ...
    Q_strncat(fileName, szFileName + offs, sizeof(fileName),
              COPY_ALL_CHARACTERS);
  ...
  for (int sp = 0; sp < searchList.Count(); ++sp) {
    ...
    // this is an optimization so that the filename string doesn't have to get
    // converted to a symbol for each key/value
    m_CurrentFile = fullpath;

    // read into a memory block
    int fileSize = g_pFullFileSystem->Size(file);
    int bufferSize =
        g_pFullFileSystem->GetOptimalReadSize(file, fileSize + sizeof(wchar_t));
    wchar_t *memBlock =
        (wchar_t *)g_pFullFileSystem->AllocOptimalReadBuffer(file, bufferSize);
    bool bReadOK =
        (g_pFullFileSystem->ReadEx(memBlock, bufferSize, fileSize, file) != 0);
      ...
      if (state == STATE_BASE) {
        ...
        if (!_stricmp(key, "}")) {
          ...
          if ((bEnglishFile) ||
            ...
            wchar_t *tempData = ReadUnicodeToken(data, conditional,
                                                 MAX_LOCALIZED_CHARS, bQuoted);
...
//-----------------------------------------------------------------------------
// Purpose: Load all the localized language strings, and uses the longest string
// from each language
//-----------------------------------------------------------------------------
...
bool CLocalizedStringTable::SaveToFile(const char *szFileName) {
  ...
  int strLength =
      ConvertANSIToUnicode(startStr, unicodeString, sizeof(unicodeString));
  if (!strLength) return false;

  g_pFullFileSystem->Write(unicodeString,
                           wcslen(unicodeString) * sizeof(wchar_t), file);
  ...
  for (StringIndex_t idx = GetFirstStringIndex(); idx != INVALID_STRING_INDEX;
       idx = GetNextStringIndex(idx)) {
...
//-----------------------------------------------------------------------------
bool CLocalizedStringTable::SymLess(localizedstring_t const &i1,
                                    localizedstring_t const &i2) {
...
wchar_t *CLocalizedStringTable::Find(const char *pName) {
  ...
  if (idx == INVALID_STRING_INDEX) return NULL;
...
StringIndex_t CLocalizedStringTable::FindIndex(const char *pName) {
  ...
  // strip the pound character (which is used elsewhere to indicate that it's a
...
//-----------------------------------------------------------------------------
void CLocalizedStringTable::AddString(const char *pString, wchar_t *pValue,
                                      const char *fileName) {
  if (!pString) return;
...
//-----------------------------------------------------------------------------
// Purpose: returns the next index, or INVALID_STRING_INDEX if no more strings
// available
//-----------------------------------------------------------------------------
StringIndex_t CLocalizedStringTable::GetNextStringIndex(StringIndex_t index) {
  StringIndex_t idx = m_Lookup.NextInorder(index);
  if (idx == m_Lookup.InvalidIndex()) return INVALID_STRING_INDEX;
  return idx;
}
...
wchar_t *CLocalizedStringTable::GetValueByIndex(StringIndex_t index) {
  if (index == INVALID_STRING_INDEX) return NULL;
...
bool CLocalizedStringTable::FastValueLessFunc(const fastvalue_t &lhs,
                                              const fastvalue_t &rhs) {
  Assert(s_pTable);

  const wchar_t *w1 =
      lhs.search ? lhs.search : &s_pTable->m_Values[lhs.valueindex];
  const wchar_t *w2 =
      rhs.search ? rhs.search : &s_pTable->m_Values[rhs.valueindex];
...
int CLocalizedStringTable::FindExistingValueIndex(const wchar_t *value) {
  if (!s_pTable) return INVALID_STRING_INDEX;
...
//-----------------------------------------------------------------------------
void CLocalizedStringTable::SetValueByIndex(StringIndex_t index,
                                            wchar_t *newValue) {
  ...
  if (newLen > oldLen) {
    ...
    memcpy(&m_Values[lstr.valueIndex], newValue,
...
bool CLocalizedStringTable::LocalizationFileIsLoaded(const char *name) {
  ...
  for (int i = 0; i < c; ++i) {
    if (!Q_stricmp(m_LocalizationFiles[i].symName.String(), name)) return true;
  }
...
//-----------------------------------------------------------------------------
int CLocalizedStringTable::ConvertANSIToUnicode(const char *ansi,
                                                wchar_t *unicode,
                                                int unicodeBufferSizeInBytes) {
  int chars = ::MultiByteToWideChar(CP_UTF8, 0, ansi, -1, unicode,
...
//-----------------------------------------------------------------------------
int CLocalizedStringTable::ConvertUnicodeToANSI(const wchar_t *unicode,
                                                char *ansi,
                                                int ansiBufferSize) {
...
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput,
  ...
  ConstructString(unicodeOutput, unicodeBufferSizeInBytes, formatString,
                  numFormatParameters, argList);
  va_end(argList);
}
        ...
        if (!param) {
          Assert(
              !("CLocalizedStringTable::ConstructString - Found a %s# escape "
                "sequence who's index was more than the number of args."));
          *outputPos = '\0';
        }
...
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput,
  ...
  if (index != INVALID_STRING_INDEX) {
    ConstructString(unicodeOutput, unicodeBufferSizeInBytes, index,
...
void CLocalizedStringTable::ConstructString(wchar_t *unicodeOutput,
                                            ...
                                            StringIndex_t unlocalizedTextSymbol,
                                            KeyValues *localizationVariables) {
  if (unicodeBufferSizeInBytes < 1) return;
    ...
    if (*searchPos == '%') {
      ...
      if (searchPos[1] == 's' && searchPos[2] >= '0' && searchPos[2] <= '9') {
        ...
        if (varEnd && *varEnd == '%') {
          ...
          for (const wchar_t *pws = varStart;
               pws < varEnd && (vset < variableName + sizeof(variableName) - 1);
               ++pws, ++vset) {
            *vset = (char)*pws;
          }
          *vset = 0;
          ...
          const wchar_t *value =
              localizationVariables->GetWString(variableName, L"[unknown]");
