...
static BOOL CALLBACK GetMainApplicationWindowHWND_EnumProc(HWND hWnd,
                                                           LPARAM lParam) {
  ...
  // Foreign process
  if (dwOurProcessId != dwProcessId) return TRUE;
  // Service window
...
static HWND GetMainApplicationWindowHWND() {
  ...
  hWnd = gti.hwndActive;
  for (HWND hParent = hWnd ? GetParent(hWnd) : hWnd; hParent;
       hWnd = hParent, hParent = GetParent(hWnd))
    continue;
...
class CSystem : public ISystem {
 public:
  ...
  virtual KeyValues *GetUserConfigFileData(const char *dialogName,
                                           int dialogID);
  ...
  virtual bool GetCommandLineParamValue(const char *paramName, char *value,
                                        int valueBufferSize);
  ...
  virtual bool CreateShortcut(const char *linkFileName, const char *targetPath,
                              const char *arguments,
                              const char *workingDirectory,
                              const char *iconFile);
  virtual bool GetShortcutTarget(const char *linkFileName, char *targetPath,
                                 char *arguments, int destBufferSizes);
  virtual bool ModifyShortcutTarget(const char *linkFileName,
  ...
  virtual void ShellExecuteEx(const char *command, const char *file,
                              const char *pParams);
...
CSystem g_System;
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSystem, ISystem,
                                  VGUI_SYSTEM_INTERFACE_VERSION, g_System);
...
void CSystem::RunFrame() {
  ...
  if (m_bStaticWatchForComputerUse) {
    ...
    // allow a little slack for jittery mice, don't reset until it's moved more
    // than fifty pixels
...
//-----------------------------------------------------------------------------
double CSystem::GetFrameTime() { return m_flFrameTime; }

//-----------------------------------------------------------------------------
// Purpose: returns the current time
//-----------------------------------------------------------------------------
double CSystem::GetCurrentTime() { return Plat_FloatTime(); }

//-----------------------------------------------------------------------------
// Purpose: returns the current time in milliseconds
//-----------------------------------------------------------------------------
long CSystem::GetTimeMillis() { return (long)(GetCurrentTime() * 1000); }
...
void CSystem::ShellExecuteEx(const char *command, const char *file,
                             const char *pParams) {
#ifndef _X360
...
void CSystem::SetClipboardImage(void *pWnd, int x1, int y1, int x2, int y2) {
  ...
  // Main app window
  HWND hWnd = (HWND)(pWnd);
  if (!hWnd) hWnd = GetMainApplicationWindowHWND();
  if (!hWnd) return;
...
static bool staticSplitRegistryKey(const char *key, char *key0, int key0Len,
                                   char *key1, int key1Len) {
  if (key == 0) {
    return false;
  }
...
bool CSystem::SetRegistryString(const char *key, const char *value) {
  ...
  if (VCRHook_RegCreateKeyEx(hSlot, key0, 0, 0, REG_OPTION_NON_VOLATILE,
                             value ? KEY_WRITE : KEY_ALL_ACCESS, 0, &hKey,
                             0) != ERROR_SUCCESS) {
    return false;
  }

  if (VCRHook_RegSetValueEx(hKey, key1, NULL, REG_SZ, (uchar *)value,
...
bool CSystem::GetRegistryString(const char *key, char *value, int valueLen) {
  ...
  ulong len = valueLen;
  if (VCRHook_RegQueryValueEx(hKey, key1, 0, 0, (uchar *)value, &len) ==
      ERROR_SUCCESS) {
    VCRHook_RegCloseKey(hKey);
    return true;
  }
...
bool CSystem::SetRegistryInteger(const char *key, int value) {
  ...
  if (VCRHook_RegSetValueEx(hKey, key1, 0, REG_DWORD, (uchar *)&value, 4) ==
      ERROR_SUCCESS) {
    VCRHook_RegCloseKey(hKey);
    return true;
  }
...
bool CSystem::SetWatchForComputerUse(bool state) {
  if (state == m_bStaticWatchForComputerUse) return true;
...
//-----------------------------------------------------------------------------
// Purpose: returns the amount of available disk space, in bytes, on the
// specified path
//-----------------------------------------------------------------------------
...
// Purpose: user config
//-----------------------------------------------------------------------------
KeyValues *CSystem::GetUserConfigFileData(const char *dialogName,
                                          int dialogID) {
  if (!m_pUserConfigData) return NULL;
...
//-----------------------------------------------------------------------------
// Purpose: sets the name of the config file to save/restore from.  Settings are
// loaded immediately.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool CSystem::GetCommandLineParamValue(const char *paramName, char *value,
                                       int valueBufferSize) {
  const char *cmdLine = GetFullCommandLine();
  const char *loc = strstr(cmdLine, paramName);
  if (!loc) return false;
...
//-----------------------------------------------------------------------------
const char *CSystem::GetFullCommandLine() { return VCRHook_GetCommandLine(); }
...
//-----------------------------------------------------------------------------
bool CSystem::GetCurrentTimeAndDate(int *year, int *month, int *dayOfWeek,
                                    int *day, int *hour, int *minute,
                                    int *second) {
  SYSTEMTIME time;
  GetLocalTime(&time);
...
//-----------------------------------------------------------------------------
bool CSystem::CreateShortcut(const char *linkFileName, const char *targetPath,
                             const char *arguments,
                             const char *workingDirectory,
                             const char *iconFile) {
  bool bSuccess = false;
  ...
  if (_stat(linkFileName, &statBuf) != -1) return false;
...
//-----------------------------------------------------------------------------
bool CSystem::ModifyShortcutTarget(const char *linkFileName,
                                   const char *targetPath,
                                   const char *arguments,
                                   const char *workingDirectory) {
#ifndef _X360
...
const char *CSystem::GetDesktopFolderPath() {
  ...
  if (GetRegistryString("HKEY_CURRENT_"
                        "USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Ex"
                        "plorer\\Shell Folders\\Desktop",
