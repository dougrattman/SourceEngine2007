...
class CScheme : public IScheme {
 public:
  ...
  void LoadFromFile(VPANEL sizingPanel, const char *filename, const char *tag,
                    KeyValues *inKeys);
 ...
 private:
  ...
  const char *GetMungedFontName(const char *fontName, const char *scheme,
                                bool proportional);
  void LoadFonts();
  void LoadBorders();
...
class CSchemeManager : public ISchemeManager {
 public:
  ...
  // scheme are derivitives of that tag is friendly string representing the name
  // of the loaded scheme
 ...
 private:
  int GetProportionalScaledValue_(int rootWide, int rootTall,
                                  int normalizedValue);
  int GetProportionalNormalizedValue_(int rootWide, int rootTall,
                                      int scaledValue);
  ...
  static bool BitmapHandleSearchFunc(const CachedBitmapHandle_t &,
...
EXPOSE_SINGLE_INTERFACE_GLOBALVAR(CSchemeManager, ISchemeManager,
                                  VGUI_SCHEME_INTERFACE_VERSION, g_Scheme);
...
// first scheme loaded becomes the default scheme, and all subsequent loaded
// scheme are derivitives of that
HScheme CSchemeManager::LoadSchemeFromFileEx(VPANEL sizingPanel,
                                             const char *fileName,
                                             const char *tag) {
...
// Purpose: loads a scheme from disk
//-----------------------------------------------------------------------------
HScheme CSchemeManager::LoadSchemeFromFile(const char *fileName,
                                           const char *tag) {
  return LoadSchemeFromFileEx(0, fileName, tag);
}

//-----------------------------------------------------------------------------
// Purpose: Table of scheme file entries, translation from old goldsrc schemes
// to new src schemes
//-----------------------------------------------------------------------------
...
SchemeEntryTranslation_t g_SchemeTranslation[] = {
    {"Border.Bright", "BorderBright",
     "200 200 200 196"},  // the lit side of a control
    {"Border.Dark"
     "BorderDark",
     "40 40 40 196"},  // the dark/unlit side of a control
    {"Border.Selection"
     "BorderSelection",
     "0 0 0 196"},  // the additional border color for displaying the
...
//-----------------------------------------------------------------------------
void CScheme::LoadFromFile(VPANEL sizingPanel, const char *inFilename,
                           const char *inTag, KeyValues *inKeys) {
  ...
  for (int i = 0; i < ARRAYSIZE(g_SchemeTranslation); i++) {
    if (!m_pkvBaseSettings->FindKey(g_SchemeTranslation[i].pchNewEntry,
      ...
      m_pkvBaseSettings->SetString(g_SchemeTranslation[i].pchNewEntry,
                                   pchColor);
    }
  }
...
void CScheme::LoadFonts() {
  ...
  for (KeyValues *kv = m_pData->FindKey("Fonts", true)->GetFirstSubKey();
    ...
    for (int i = 0; i < 2; i++) {
      ...
      const char *fontName = GetMungedFontName(
          kv->GetName(), tag, proportionalFont);  // first time it adds a normal
                                                  // font, and then a
                                                  // proportional one
...
void CScheme::ReloadFontGlyphs() {
  ...
  for (int i = 0; i < m_FontAliases.Count(); i++) {
    ...
    for (KeyValues *fontdata = kv->GetFirstSubKey(); fontdata != NULL;
      ...
      if (fontdata->GetInt("antialias") &&
          g_pSurface->SupportsFeature(ISurface::ANTIALIASED_FONTS)) {
        flags |= ISurface::FONTFLAG_ANTIALIAS;
      }
      if (fontdata->GetInt("dropshadow") &&
          g_pSurface->SupportsFeature(ISurface::DROPSHADOW_FONTS)) {
        flags |= ISurface::FONTFLAG_DROPSHADOW;
      }
      if (fontdata->GetInt("outline") &&
          g_pSurface->SupportsFeature(ISurface::OUTLINE_FONTS)) {
      ...
      if ((!fontYResMin && !fontYResMax) &&
          m_FontAliases[i].m_bProportional)  // only grow this font if it
                                             // doesn't have a resolution filter
                                             // specified
      {
      ...
      if (flags & ISurface::FONTFLAG_BITMAP) {
        ...
        g_pSurface->SetBitmapFontGlyphSet(
            ...
            fontdata->GetFloat("scaley", 1.0f), flags);
      } else {
        // add the new set
        g_pSurface->SetFontGlyphSet(
            m_FontAliases[i]._font, fontdata->GetString("name"), tall,
            fontdata->GetInt("weight"), blur, scanlines, flags);
      }
...
//-----------------------------------------------------------------------------
HScheme CSchemeManager::GetDefaultScheme() { return 1; }
...
int CSchemeManager::GetProportionalScaledValue_(int rootWide, int rootTall,
                                                int normalizedValue) {
  int proH, proW;
...
int CSchemeManager::GetProportionalNormalizedValue_(int rootWide, int rootTall,
                                                    int scaledValue) {
  int proH, proW;
  g_pSurface->GetProportionalBase(proW, proH);
...
// layouts use these for font, image and panel size scaling (they all use the
// pixel height of the display for scaling)
int CSchemeManager::GetProportionalScaledValueEx(CScheme *pScheme,
                                                 int normalizedValue) {
...
int CSchemeManager::GetProportionalNormalizedValueEx(CScheme *pScheme,
                                                     int scaledValue) {
  VPANEL sizing = pScheme->GetSizingPanel();
  if (!sizing) {
...
int CSchemeManager::GetProportionalScaledValueEx(HScheme scheme,
                                                 int normalizedValue) {
...
int CSchemeManager::GetProportionalNormalizedValueEx(HScheme scheme,
                                                     int scaledValue) {
  IScheme *pscheme = GetIScheme(scheme);
  if (!pscheme) {
    Assert(0);
...
IImage *CSchemeManager::GetImage(const char *imageName, bool hardwareFiltered) {
  if (!imageName || strlen(imageName) <= 0)  // frame icons and the like are in
                                             // the scheme file and may not be
...
char const *CScheme::GetFontName(const HFont &font) {
  for (int i = m_FontAliases.Count(); --i >= 0;) {
    ...
    if (fnt == font) return m_FontAliases[i]._trueFontName.String();
  }
