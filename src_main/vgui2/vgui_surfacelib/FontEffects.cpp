...
void ApplyRotaryEffectToTexture(int rgbaWide, int rgbaTall, unsigned char *rgba,
  ...
  unsigned char *line = &rgba[(rgbaTall / 2 * rgbaWide) * 4];
  ...
  for (int y = rgbaTall - 1; y >= iDropShadowOffset; y--) {
    for (int x = rgbaWide - 1; x >= iDropShadowOffset; x--) {
      ...
      if (dest[3] == 0) {
        ...
        unsigned char *src = &rgba[(x - iDropShadowOffset +
                                    ((y - iDropShadowOffset) * rgbaWide)) *
                                   4];
        dest[0] = 0;
        dest[1] = 0;
        dest[2] = 0;
  ...
  for (y = 0; y < rgbaTall; y++) {
    for (x = 0; x < rgbaWide; x++) {
      ...
      if (src[3] == 0) {
        ...
        int shadowX, shadowY;
        for (shadowX = -(int)iOutlineSize; shadowX <= (int)iOutlineSize;
             shadowX++) {
          for (shadowY = -(int)iOutlineSize; shadowY <= (int)iOutlineSize;
               shadowY++) {
            ...
            testX = shadowX + x;
            testY = shadowY + y;
            if (testX < 0 || testX >= rgbaWide || testY < 0 ||
                testY >= rgbaTall) {
              continue;
            }
...
FORCEINLINE void GetBlurValueForPixel(unsigned char *src, int blur,
  ...
  int maxX = std::min(srcX + blur, rgbaWide);
  int minX = std::max(srcX - blur, 0);
  for (int x = minX; x <= maxX; x++) {
    int maxY = std::min(srcY + blur, rgbaTall - 1);
    int minY = std::max(srcY - blur, 0);
  ...
  // leave ALL pixels white or we get black backgrounds mixed in
  dest[0] = dest[1] = dest[2] = 255;
  // blur occurs entirely in the alpha
  dest[3] = std::min((int)accum, 255);
}
...
void ApplyGaussianBlurToTexture(int rgbaWide, int rgbaTall, unsigned char *rgba,
  ...
  for (int x = 0; x <= (iBlur * 2); x++) {
    int val = x - iBlur;
    pGaussianDistribution[x] = (float)(1.0f / sqrt(2 * 3.14 * sigma * sigma)) *
                               pow(2.7, -1 * (val * val) / (2 * sigma * sigma));
  }
  ...
  for (int y = 0; y < rgbaTall; y++) {
    for (int x = 0; x < rgbaWide; x++) {
      ...
      GetBlurValueForPixel(src, iBlur, pGaussianDistribution, x, y, rgbaWide,
                           rgbaTall, dest);
