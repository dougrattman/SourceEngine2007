...
//			used to determine whether or not a font exists on the
// system
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: creates the font from windows.  returns false if font does not exist
// in the OS.
//-----------------------------------------------------------------------------
bool CWin32Font::Create(const char *windowsFontName, int tall, int weight,
  ...
  int charset =
      (flags & vgui::ISurface::FONTFLAG_SYMBOL) ? SYMBOL_CHARSET : ANSI_CHARSET;

  // hack for japanese win98 support
  if (!_stricmp(windowsFontName, "win98japanese")) {
  ...
  m_hFont = ::CreateFontA(
      tall, 0, 0, 0, m_iWeight, flags & vgui::ISurface::FONTFLAG_ITALIC,
      flags & vgui::ISurface::FONTFLAG_UNDERLINE,
      flags & vgui::ISurface::FONTFLAG_STRIKEOUT, charset, OUT_DEFAULT_PRECIS,
      CLIP_DEFAULT_PRECIS,
      m_bAntiAliased ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY,
      DEFAULT_PITCH | FF_DONTCARE, windowsFontName);
  if (!m_hFont) {
  ...
  m_hDIB = ::CreateDIBSection(m_hDC, (BITMAPINFO *)&header, DIB_RGB_COLORS,
                              (void **)(&m_pBuf), NULL, 0);
  ...
  if (::GetCharABCWidthsW(m_hDC, 0, ABCWIDTHS_CACHE_SIZE - 1, &abc[0]) ||
    ...
    for (int i = 0; i < ABCWIDTHS_CACHE_SIZE; i++) {
      ...
      m_ABCWidthsCache[i].b =
          abc[i].abcB + ((m_iBlur + m_iOutlineSize) * 2) + m_iDropShadowOffset;
      m_ABCWidthsCache[i].c =
          abc[i].abcC - m_iBlur - m_iDropShadowOffset - m_iOutlineSize;
    }
  } else {
    Warning("GetCharABCWidths() failed for windows font '%s'\n",
            windowsFontName);

    // since that failed, it must be fixed width, zero everything so a and c
    // will be zeros, then fill b with the value from TEXTMETRIC
    for (int i = 0; i < ABCWIDTHS_CACHE_SIZE; i++) {
...
//-----------------------------------------------------------------------------
void CWin32Font::GetCharRGBA(wchar_t ch, int rgbaWide, int rgbaTall,
                             unsigned char *rgba) {
  int a, b, c;
  ...
  if (!s_bSupportsUnicode) {
    // win98 hack, don't antialias some characters that ::GetGlyphOutline()
    // produces bad results for
    if (ch == 'I' || ch == '1') {
      bShouldAntialias = false;
    }
  ...
  // only antialias latin characters, since it essentially always fails for
  // asian characters
  if (bShouldAntialias) {
    // try and get the glyph directly
    ::SelectObject(m_hDC, m_hFont);
    bytesNeeded = ::GetGlyphOutline(m_hDC, ch, GGO_GRAY8_BITMAP, &glyphMetrics,
                                    0, NULL, &mat2);
  }
  ...
  if (bytesNeeded > 0) {
    ...
    ::GetGlyphOutline(m_hDC, ch, GGO_GRAY8_BITMAP, &glyphMetrics, bytesNeeded,
                      lpbuf, &mat2);
    ...
    // don't copy the first set of pixels if the antialiased bmp is bigger than
    // the char width
    ...
    if (s_bSupportsUnicode) {
      ...
      // clear the background first (it may not get done automatically in
      // win98/ME
    ...
    for (int j = (int)m_iOutlineSize; j < tall - (int)m_iOutlineSize; j++) {
      // only copy from within the dib, ignore the outline border we are
      // artificially adding
      for (int i = (int)m_iOutlineSize;
           i < wide - (int)m_iDropShadowOffset - (int)m_iOutlineSize; i++) {
    ...
    // this is because of a bug in winME that writes noise to them, only on the
    // first time the game is run after a reboot the bottom row should
    // guaranteed to be empty to fit the dropshadow
...
//-----------------------------------------------------------------------------
bool CWin32Font::IsEqualTo(const char *windowsFontName, int tall, int weight,
                           int blur, int scanlines, int flags) {
  if (!_stricmp(windowsFontName, m_szName.String()) && m_iTall == tall &&
      m_iWeight == weight && m_iBlur == blur && m_iFlags == flags)
    return true;
...
bool CWin32Font::IsValid() {
  if (m_szName.IsValid() && m_szName.String()[0]) return true;
...
//-----------------------------------------------------------------------------
// Purpose: returns the ascent of the font, in pixels (ascent=units above the
// base line)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: returns the flags used to make this font, used by the dynamic
// resizing code
//-----------------------------------------------------------------------------
int CWin32Font::GetFlags() { return m_iFlags; }

//-----------------------------------------------------------------------------
// Purpose: Comparison function for abc widths storage
//-----------------------------------------------------------------------------
bool CWin32Font::ExtendedABCWidthsCacheLessFunc(const abc_cache_t &lhs,
