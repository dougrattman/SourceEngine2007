...
//-----------------------------------------------------------------------------
static int __cdecl SequenceSortFunc(vgui::ListPanel *pPanel,
...
static int __cdecl ActivitySortFunc(vgui::ListPanel *pPanel,
                                    const ListPanelItem &item1,
                                    const ListPanelItem &item2) {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CSequencePicker::CSequencePicker(vgui::Panel *pParent, int nFlags)
    : BaseClass(pParent, "SequencePicker") {
  m_hSelectedMDL = MDLHANDLE_INVALID;

  // property sheet - revisions, changes, etc.
  m_pPreviewSplitter =
      new Splitter(this, "PreviewSplitter", SPLITTER_MODE_HORIZONTAL, 1);
...
void CSequencePicker::RefreshActivitiesAndSequencesList() {
  ...
  if (m_hSelectedMDL == MDLHANDLE_INVALID) return;
...
void CSequencePicker::OnPageChanged() {
  if (m_pSequencesPage &&
    ...
    if (pSequenceName) {
      ...
      PostActionSignal(
          new KeyValues("SequencePreviewChanged", "sequence", pSequenceName));
    }
    return;
  }
  ...
  if (m_pActivitiesPage &&
    ...
    if (pActivityName) {
      ...
      PostActionSignal(
          new KeyValues("SequencePreviewChanged", "activity", pActivityName));
    }
    return;
  }
}
...
//
//-----------------------------------------------------------------------------
CSequencePickerFrame::CSequencePickerFrame(vgui::Panel *pParent, int nFlags)
    : BaseClass(pParent, "SequencePickerFrame") {
...
void CSequencePickerFrame::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "Open")) {
    ...
    if ((type == CSequencePicker::PICK_SEQUENCES) ||
        (type == CSequencePicker::PICK_ACTIVITIES)) {
      const char *pSequenceName = m_pPicker->GetSelectedSequenceName();
      if (pSequenceName) {
        if (type == CSequencePicker::PICK_SEQUENCES) {
          PostActionSignal(
              new KeyValues("SequenceSelected", "sequence", pSequenceName));
        } else {
          PostActionSignal(
              new KeyValues("SequenceSelected", "activity", pSequenceName));
        }
        CloseModal();
        return;
      }
