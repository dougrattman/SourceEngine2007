...
//-----------------------------------------------------------------------------
CParticleSystemPanel::CParticleSystemPanel(vgui::Panel *pParent,
                                           const char *pName)
  ...
  m_pLightmapTexture.Init("//platform/materials/debug/defaultlightmap",
                          "editor");
...
void CParticleSystemPanel::OnTick() {
  ...
  for (int i = 0; i < MAX_PARTICLE_CONTROL_POINTS; ++i) {
    if (!m_pParticleSystem->ReadsControlPoint(i)) continue;

    m_pParticleSystem->SetControlPoint(i, m_pControlPointValue[i]);
    m_pParticleSystem->SetControlPointOrientation(
    ...
    if (m_bPerformNameBasedLookup) {
      if (m_ParticleSystemName.Length()) {
        CParticleCollection *pNewParticleSystem =
            g_pParticleSystemMgr->CreateParticleCollection(
...
//-----------------------------------------------------------------------------
void CParticleSystemPanel::SetParticleSystem(
    CDmeParticleSystemDefinition *pDef) {
  ...
  if (pDef) {
    ...
    if (m_bPerformNameBasedLookup) {
      ...
      Assert(
          g_pParticleSystemMgr->IsParticleSystemDefined(m_ParticleSystemName));
      m_pParticleSystem =
...
void CParticleSystemPanel::DrawBounds() {
  ...
  RenderWireframeBox(vec3_origin, vec3_angle, vecMins, vecMaxs,
                     Color(0, 255, 255, 255), true);
}
...
void CParticleSystemPanel::DrawCullBounds() {
  Vector vecCenter;
  m_pParticleSystem->GetControlPointAtTime(
      m_pParticleSystem->m_pDef->GetCullControlPoint(),
      m_pParticleSystem->m_flCurTime, &vecCenter);
  RenderWireframeSphere(vecCenter, m_pParticleSystem->m_pDef->GetCullRadius(),
                        32, 16, Color(0, 255, 255, 255), true);
}
...
void CParticleSystemPanel::OnPaint3D() {
  ...
  if (m_bRenderBounds) {
    ...
    Vector vP1;
    Vector vP2;
    m_pParticleSystem->GetControlPointAtTime(0, m_pParticleSystem->m_flCurTime,
                                             &vP1);
    m_pParticleSystem->GetControlPointAtTime(1, m_pParticleSystem->m_flCurTime,
                                             &vP2);
...
class CControlPointPage : public vgui::PropertyPage {
 ...
 public:
  // constructor, destructor
  CControlPointPage(vgui::Panel *pParent, const char *pName,
...
void CControlPointPage::OnTextChanged(KeyValues *pParams) {
  ...
  for (int i = 0; i < MAX_PARTICLE_CONTROL_POINTS; ++i) {
    if (pPanel != m_pControlPointValue[i]) continue;

    char pBuf[512];
    m_pControlPointValue[i]->GetText(pBuf, sizeof(pBuf));

    Vector vecValue(0, 0, 0);
    sscanf_s(pBuf, "%f %f %f", &vecValue.x, &vecValue.y, &vecValue.z);
...
void CControlPointPage::OnNewLine(KeyValues *pParams) {
  ...
  for (int i = 0; i < MAX_PARTICLE_CONTROL_POINTS; ++i) {
    ...
    Q_snprintf(pBuf, sizeof(pBuf), "%.3f %.3f %.3f", vecValue.x, vecValue.y,
               vecValue.z);
...
void CControlPointPage::CreateControlPointControls() {
  ...
  CParticleCollection *pParticleSystem =
      m_pParticleSystemPanel->GetParticleSystem();
  if (!pParticleSystem) return;

  for (int i = 0; i < MAX_PARTICLE_CONTROL_POINTS; ++i) {
    if (!pParticleSystem->ReadsControlPoint(i)) continue;
    ...
    Q_snprintf(pName, sizeof(pName), "%.3f %.3f %.3f", vecValue.x, vecValue.y,
               vecValue.z);
...
void CControlPointPage::LayoutControlPointControls() {
  ...
  for (int i = 0; i < MAX_PARTICLE_CONTROL_POINTS; ++i) {
    if (!m_pControlPointName[i]) continue;
...
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CParticleSystemPreviewPanel,
                           DmeParticleSystemDefinition,
                           "DmeParticleSystemDefinitionViewer",
                           "Particle System Viewer", false);

//-----------------------------------------------------------------------------
// constructor, destructor
//-----------------------------------------------------------------------------
CParticleSystemPreviewPanel::CParticleSystemPreviewPanel(vgui::Panel *pParent,
                                                         const char *pName)
    : BaseClass(pParent, pName) {
  ...
  m_pParticleSystemPanel =
      new CParticleSystemPanel(pSplitterRightSide, "ParticlePreview");
  m_pParticleSystemPanel->AddActionSignalTarget(this);
  m_pParticleSystemPanel->SetBackgroundColor(0, 0, 0);

  m_pParticleCount =
      new vgui::Label(pSplitterRightSide, "ParticleCountLabel", "");
  ...
  m_pRenderBounds = new vgui::CheckButton(m_pRenderPage, "RenderBounds",
                                          "Render Bounding Box");
  m_pRenderBounds->AddActionSignalTarget(this);

  m_pRenderCullBounds = new vgui::CheckButton(m_pRenderPage, "RenderCullBounds",
                                              "Render Culling Bounds");
  m_pRenderCullBounds->AddActionSignalTarget(this);

  m_pRenderHelpers =
      new vgui::CheckButton(m_pRenderPage, "RenderHelpers", "Render Helpers");
  m_pRenderHelpers->AddActionSignalTarget(this);

  m_pBackgroundColor =
      new CColorPickerButton(m_pRenderPage, "BackgroundColor", this);
  m_pBackgroundColor->SetColor(m_pParticleSystemPanel->GetBackgroundColor());

  m_pRenderPage->LoadControlSettingsAndUserConfig(
      "resource/particlesystempreviewpanel_renderpage.res");

  m_pControlPointPage = new CControlPointPage(
      m_pControlSheet, "ControlPointPage", m_pParticleSystemPanel);
...
void CParticleSystemPreviewPanel::OnThink() {
  ...
  if (!pParticleSystem) {
    ...
    Q_snprintf(buf, sizeof(buf), "Particle Count: %5d/%5d",
               pParticleSystem->m_nActiveParticles,
...
// Set the particle system to draw
//-----------------------------------------------------------------------------
void CParticleSystemPreviewPanel::SetParticleSystem(
    CDmeParticleSystemDefinition *pDef) {
  m_pParticleSystemPanel->SetParticleSystem(pDef);
}

void CParticleSystemPreviewPanel::SetDmeElement(
    CDmeParticleSystemDefinition *pDef) {
...
// Indicates which helper to draw
//-----------------------------------------------------------------------------
void CParticleSystemPreviewPanel::SetParticleFunction(
...
void CParticleSystemPreviewPanel::OnBackgroundColorCancel(KeyValues *pParams) {
  m_pParticleSystemPanel->SetBackgroundColor(
      pParams->GetColor("startingColor"));
}
