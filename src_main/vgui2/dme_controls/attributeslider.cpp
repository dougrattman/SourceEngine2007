...
static ConVar ifm_attributeslider_sensitivity("ifm_attributeslider_sensitivity",
                                              "3.0", 0);
...
//-----------------------------------------------------------------------------
static void BlendFlexValues(AttributeValue_t *pResult,
                            const AttributeValue_t &src,
                            const AttributeValue_t &dest, float flBlend,
                            float flBalanceFilter = 0.5f) {
  // Apply the left-right balance to the target
  float flLeftFilter, flRightFilter;
  ValueBalanceToLeftRight(&flLeftFilter, &flRightFilter, flBlend,
                          flBalanceFilter);

  // Do the math in 'left-right' space because we filter in that space
  float flSrcLeft, flSrcRight;
  ValueBalanceToLeftRight(&flSrcLeft, &flSrcRight,
                          src.m_pValue[ANIM_CONTROL_VALUE],
                          src.m_pValue[ANIM_CONTROL_BALANCE]);

  float flDestLeft, flDestRight;
  ValueBalanceToLeftRight(&flDestLeft, &flDestRight,
                          dest.m_pValue[ANIM_CONTROL_VALUE],
                          dest.m_pValue[ANIM_CONTROL_BALANCE]);
  ...
  LeftRightToValueBalance(
      &pResult->m_pValue[ANIM_CONTROL_VALUE],
      &pResult->m_pValue[ANIM_CONTROL_BALANCE], flTargetLeft, flTargetRight,
      (flBlend <= 0.5f) ? src.m_pValue[ANIM_CONTROL_BALANCE]
                        : dest.m_pValue[ANIM_CONTROL_BALANCE]);

  pResult->m_pValue[ANIM_CONTROL_MULTILEVEL] =
      src.m_pValue[ANIM_CONTROL_MULTILEVEL] +
      (dest.m_pValue[ANIM_CONTROL_MULTILEVEL] -
       src.m_pValue[ANIM_CONTROL_MULTILEVEL]) *
          flBlend;
}
...
CAttributeSlider::CAttributeSlider(CBaseAnimSetAttributeSliderPanel *parent,
                                   const char *panelName, CDmElement *pControl)
  ...
  m_pCircleImage =
      new CSubRectImage("tools/ifm/icon_balance", false, 7, 8, 19, 15);
...
void CAttributeSlider::ActivateControl(AnimationControlType_t type,
                                       bool bActive) {
  if (m_bIsControlActive[type] != bActive) {
    m_bIsControlActive[type] = bActive;
    if (bActive) {
      PostActionSignal(new KeyValues(s_pChangeMessage[type],
                                     s_pChangeKeyValue[type],
                                     m_Control.m_pValue[type]));
    }
  }
}
...
void CAttributeSlider::SetValue(AnimationControlType_t type, float flValue) {
  if (m_Control.m_pValue[type] != flValue) {
    ...
    if (m_bIsControlActive[type]) {
      PostActionSignal(new KeyValues(s_pChangeMessage[type],
                                     s_pChangeKeyValue[type], flValue));
    }
  }
}
...
//-----------------------------------------------------------------------------
float CAttributeSlider::GetControlDefaultValue(
    AnimationControlType_t type) const {
  if (IsTransform()) return 0.0f;

  Assert(m_hControl.Get());
  if (!m_hControl.Get()) return 0.0f;
...
//-----------------------------------------------------------------------------
// Given a mouse position in (x,y) in local coordinates, which animation control
// is it over?
//-----------------------------------------------------------------------------
...
void CAttributeSlider::SetSelected(bool state) { m_bSelected = state; }

bool CAttributeSlider::IsSelected() const { return m_bSelected; }
...
void CAttributeSlider::OnMousePressed(MouseCode code) {
  if (!IsEnabled() || IsInTextEntry() || IsDragging()) return;
...
void CAttributeSlider::OnCursorMoved(int x, int y) {
  if (!IsEnabled() || !IsDragging() || m_bTransform) return;

  // NOTE: This works because we always slam the mouse to be back at the start
  // position at the end of this function
  ...
  if (GetDragControl() == ANIM_CONTROL_VALUE &&
      IsControlActive(ANIM_CONTROL_BALANCE)) {
    // do the hacky conversion from the ui's left/right to the underlying
    // value/balance
    float flLeftValue, flRightValue;
    ValueBalanceToLeftRight(&flLeftValue, &flRightValue, m_flDragStartValue,
                            m_flDragStartBalance);

    float flLeftDelta, flRightDelta;
    ValueBalanceToLeftRight(&flLeftDelta, &flRightDelta, flDelta,
                            m_pParent->GetBalanceSliderValue());
    ...
    SetValue(ANIM_CONTROL_BALANCE,
             flBalance);  // TODO - add balance for multi control as well
  } else {
    float flValue =
        std::clamp(m_flDragStartValue + flDelta, flMinVal, flMaxVal);
    SetValue(GetDragControl(), flValue);
  }
...
//-----------------------------------------------------------------------------
void CAttributeSlider::StampValueIntoLogs(AnimationControlType_t type,
                                          float flValue) {
  Assert(!m_bTransform);
...
//-----------------------------------------------------------------------------
void CAttributeSlider::EnterTextEntryMode(AnimationControlType_t type,
                                          bool bRelatchValues) {
...
void CAttributeSlider::AcceptTextEntryValue() {
  ...
  if (m_pRightTextField && GetTextEntryControl() == ANIM_CONTROL_VALUE &&
      IsControlActive(ANIM_CONTROL_BALANCE)) {
...
void CAttributeSliderTextEntry::OnKillFocus(KeyValues *pParams) {
  ...
  if (hPanel != INVALID_PANEL &&
      vgui::ipanel()->GetParent(hPanel) == m_pSlider->GetVPanel())
    return;
...
void CAttributeSliderTextEntry::OnMouseWheeled(int delta) {
  if (m_pSlider->m_bTransform) return;
  ...
  CUndoScopeGuard guard(UNDO_CHAIN_MOUSEWHEEL_ATTRIBUTE_SLIDER,
                        "Set Slider Value");
...
bool CAttributeSlider::IsPreviewEnabled() const { return m_bPreviewEnabled; }

bool CAttributeSlider::IsSimplePreview() const { return m_bSimplePreviewOnly; }

#define ATTRIBUTE_SLIDER_RAMP_TIME 0.5f

bool CAttributeSlider::IsRampingTowardPreview() const {
  if (m_flPreviewGoalTime == -1.0f) return false;

  return true;
}

void CAttributeSlider::RampDown() {
  if (m_flPreviewGoalTime == -1.0f) return;
...
void CAttributeSlider::UpdateTime(float dt) {
  ...
  if (m_flPreviewGoalTime >= ATTRIBUTE_SLIDER_RAMP_TIME) {
    ...
    BlendFlexValues(&m_Preview.m_Current, m_Previous.m_Current,
                    m_Next.m_Current, frac);
    BlendFlexValues(&m_Preview.m_Full, m_Previous.m_Full, m_Next.m_Full, frac);
  }
}

void CAttributeSlider::SetPreview(const AttributeValue_t &value,
                                  const AttributeValue_t &full,
                                  bool instantaneous, bool startfromcurrent) {
  m_bRampUp = true;
  ...
  if (instantaneous) {
    ...
    // Current becomes previous, next becomes goal and preview starts moving
    // toward that goal
    if (startfromcurrent) {
      m_Previous.m_Current = GetValue();
      m_Previous.m_Full = GetValue();
...
float CAttributeSlider::EstimateValueAtPos(int nLocalX, int nLocalY) const {
  ...
  float flFactor = rect.width > 1
                       ? (float)(nLocalX - rect.x) / (float)(rect.width - 1)
                       : 0.5f;
...
void CAttributeSlider::PerformLayout() {
  ...
  if (m_pRightTextField && GetTextEntryControl() == ANIM_CONTROL_VALUE &&
    ...
    m_pRightTextField->SetBounds(rect.x + rect.width / 2, rect.y,
...
void CAttributeSlider::GetControlRect(Rect_t *pRect,
                                      AnimationControlType_t type) const {
  int sw, sh;
...
//-----------------------------------------------------------------------------
void CAttributeSlider::DrawTick(const Color &clr, float frac, int width,
                                int inset) {
...
//-----------------------------------------------------------------------------
void CAttributeSlider::DrawCircularTick(const Color &clr, float flValue,
...
//-----------------------------------------------------------------------------
void CAttributeSlider::DrawCircularPreview(AnimationControlType_t type,
                                           bool bMainTick, float flRadius) {
  Rect_t rect;
  ...
  if (m_bSimplePreviewOnly && !m_bFaderBeingDragged) return;
...
void CAttributeSlider::Paint() {
  ...
  if (m_bPreviewEnabled) {
    ...
    if (IsControlActive(ANIM_CONTROL_MULTILEVEL)) {
      DrawCircularPreview(ANIM_CONTROL_MULTILEVEL, true,
                          CIRCULAR_CONTROL_RADIUS);
    }
  }
}
...
void CAttributeSlider::DrawValueLabel(float flValue) {
  ...
  m_pValues[2]->SetPos(rect.x + rect.width - cw - 5,
                       rect.y + (rect.height - ch) * 0.5f);
  m_pValues[2]->Paint();
  ...
  m_pValues[1]->SetPos(rect.x + (rect.width - cw) * 0.5f,
                       rect.y + (rect.height - ch) * 0.5f);
  m_pValues[1]->Paint();
}

//-----------------------------------------------------------------------------
// Draws the text for the slider. It's either the slider name, or its value if
...
void CAttributeSlider::DrawNameLabel() {
  ...
  m_pName->SetPos(rect.x + (rect.width - cw) * 0.5f,
                  rect.y + (rect.height - ch) * 0.5f);
  m_pName->Paint();
}
...
void CAttributeSlider::PaintBackground() {
  ...
  surface()->DrawOutlinedRect(rect.x + rect.width - 1, rect.y,
                              rect.x + rect.width, rect.y + rect.height);
  // bottom
  surface()->DrawSetColor(Color(56, 56, 56, 255));
  surface()->DrawOutlinedRect(rect.x, rect.y + rect.height - 1,
                              rect.x + rect.width, rect.y + rect.height);
  ...
  bool bUsePreview =
      m_bPreviewEnabled && (!m_bSimplePreviewOnly || m_bFaderBeingDragged);

  float flMidPoint = GetControlDefaultValue(viewType);
  int nMidPoint =
      (int)((float)rect.width * std::clamp(flMidPoint, 0.0f, 1.0f) + 0.5f);

  float flValue =
      bUsePreview ? m_Preview.m_Current.m_pValue[viewType] : GetValue(viewType);
  if (viewType == ANIM_CONTROL_VALUE && IsControlActive(ANIM_CONTROL_BALANCE)) {
    float flBalance = bUsePreview
                          ? m_Preview.m_Current.m_pValue[ANIM_CONTROL_BALANCE]
                          : GetValue(ANIM_CONTROL_BALANCE);
    float flLeftValue, flRightValue;
    ValueBalanceToLeftRight(&flLeftValue, &flRightValue, flValue, flBalance);

    int nLeftValue =
        (int)((float)rect.width * std::clamp(flLeftValue, 0.0f, 1.0f) + 0.5f);
    int nRightValue =
        (int)((float)rect.width * std::clamp(flRightValue, 0.0f, 1.0f) + 0.5f);

    // Draw the current value as a bar from the midpoint
    surface()->DrawSetColor(
        IsDragging() ? s_DraggingBarColor
                     : s_BarColor[m_bIsLogPreviewControl][IsSelected()]);
    surface()->DrawFilledRect(rect.x + std::min(nLeftValue, nMidPoint), y0,
                              rect.x + std::max(nLeftValue, nMidPoint), y1);
    surface()->DrawFilledRect(rect.x + std::min(nRightValue, nMidPoint), y1,
                              rect.x + std::max(nRightValue, nMidPoint), y2);
  } else {
    Assert(viewType != ANIM_CONTROL_BALANCE);

    int nValue =
        (int)((float)rect.width * std::clamp(flValue, 0.0f, 1.0f) + 0.5f);

    // Draw the current value as a bar from the midpoint
    surface()->DrawSetColor(
        IsDragging() ? s_DraggingBarColor
                     : s_BarColor[m_bIsLogPreviewControl][IsSelected()]);
    surface()->DrawFilledRect(rect.x + std::min(nValue, nMidPoint), y0,
                              rect.x + std::max(nValue, nMidPoint), y2);
  }
  ...
  if (IsControlActive(ANIM_CONTROL_MULTILEVEL)) {
    float flMultiValue =
        bUsePreview ? m_Preview.m_Current.m_pValue[ANIM_CONTROL_MULTILEVEL]
                    : GetValue(ANIM_CONTROL_MULTILEVEL);
    GetControlRect(&rect, ANIM_CONTROL_MULTILEVEL);
    ...
    int nCenterX = rect.x + rect.width / 2;
    int nCenterY = rect.y + rect.height / 2;
    DrawCircularTick(s_MidpointColor,
                     GetControlDefaultValue(ANIM_CONTROL_MULTILEVEL), nCenterX,
                     nCenterY, CIRCULAR_CONTROL_RADIUS);
  }
}
