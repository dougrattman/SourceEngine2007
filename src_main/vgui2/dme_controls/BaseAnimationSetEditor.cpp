...
static AnimSetLayout_t g_AnimSetLayout[] = {
    ...
    {CBaseAnimationSetEditor::LAYOUT_VERTICAL, "vertical",
     "#BxAnimSetVerticalLayout"},
    {CBaseAnimationSetEditor::LAYOUT_HORIZONTAL, "horizontal",
     "#BxAnimSetHorizontalLayout"},
};

CBaseAnimationSetEditor::CBaseAnimationSetEditor(vgui::Panel *parent,
                                                 const char *className,
                                                 bool bShowGroups)
    : BaseClass(parent, className), m_Layout(LAYOUT_SPLIT), m_Images(false) {
  const char *modes[] = {
      "AS_OFF",
      "AS_PREVIEW",
      "AS_RECORD",
  ...
  //	m_Images.AddImage( scheme()->GetImage( "tools/ifm/icon_eyedropper",
  //false ) ); 	m_Images.AddImage( scheme()->GetImage(
  //"tools/ifm/icon_selectionmodeactive", false ) );
  m_Images.AddImage(
      scheme()->GetImage("tools/ifm/icon_selectionmodeattached", false));
  for (i = 0; i < NUM_AS_RECORDING_STATES; ++i) {
...
void CBaseAnimationSetEditor::CreateToolsSubPanels() {
  m_hControlGroup = new CBaseAnimSetControlGroupPanel(
      (Panel *)NULL, "AnimSetControlGroup", this);
  m_hPresetFader = new CBaseAnimSetPresetFaderPanel((Panel *)NULL,
                                                    "AnimSetPresetFader", this);
  m_hAttributeSlider = new CBaseAnimSetAttributeSliderPanel(
      (Panel *)NULL, "AnimSetAttributeSliderPanel", this);
}
...
void CBaseAnimationSetEditor::OnButtonToggled(KeyValues *params) {
  ...
  {
  // TODO(d.rattman): could do this with MESSAGE_FUNC_PARAMS and look up "panel"
  ptr and compare to figure out which button was manipulated...
  g_pMovieMaker->SetTimeSelectionModeType( !m_pSelectionModeType->IsSelected() ?
  CIFMTool::MODE_DETACHED : CIFMTool::MODE_ATTACHED );
  }
  else
  */
  {
...
void CBaseAnimationSetEditor::ChangeLayout(EAnimSetLayout_t newLayout) {
  ...
  switch (m_Layout) {
    default:
    case LAYOUT_SPLIT: {
      m_Splitter = new Splitter(this, "AnimSetEditorMainSplitter",
                                SPLITTER_MODE_VERTICAL, 1);
      m_Splitter->SetAutoResize(Panel::PIN_TOPLEFT,
                                Panel::AUTORESIZE_DOWNANDRIGHT, 0,
                                ANIMATION_SET_EDITOR_BUTTONTRAY_HEIGHT, 0, 0);
      m_Splitter->SetBounds(0, ANIMATION_SET_EDITOR_BUTTONTRAY_HEIGHT,
                            GetWide(),
                            GetTall() - ANIMATION_SET_EDITOR_BUTTONTRAY_HEIGHT);
      ...
      m_hControlGroup->SetAutoResize(
          Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);

      sub = new Splitter(m_Splitter->GetChild(1), "AnimSetEditorSubSplitter",
                         SPLITTER_MODE_HORIZONTAL, 1);
      sub->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 0,
                         0, 0, 0);

      m_hPresetFader->SetParent(sub->GetChild(0));
      m_hPresetFader->SetAutoResize(Panel::PIN_TOPLEFT,
                                    Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
    ...
    case LAYOUT_VERTICAL: {
      ...
      for (i = 0; i < list.Count(); ++i) {
        ...
        list[i]->SetSize(m_Splitter->GetChild(i)->GetWide(),
                         m_Splitter->GetChild(i)->GetTall());
        list[i]->SetAutoResize(Panel::PIN_TOPLEFT,
                               Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
      }
...
//-----------------------------------------------------------------------------
CDmeChannel *CBaseAnimationSetEditor::FindImportChannel(
    CDmeChannel *pChannel, CDmeChannelsClip *pChannelsClip) {
  CDmElement *pTargetElement = pChannel->GetToElement();
  if (!pTargetElement) return NULL;

  CDmAttribute *pTargetAttribute = pChannel->GetToAttribute();
  if (!pTargetAttribute) return NULL;
  ...
  for (int j = 0; j < nCount; ++j) {
    ...
    if (!Q_stricmp(pTargetName, pImportName)) return pImportChannel;
    ...
    if (!pParen2) continue;
    *pParen2 = 0;
    if (!Q_stricmp(pImportName, pTemp)) return pImportChannel;
  }
  return NULL;
}
...
void CBaseAnimationSetEditor::TransformImportedChannel(CDmeChannel *pChannel) {
  ...
  CDmeGameModel *pGameModel =
      FindReferringElement<CDmeGameModel>(pTarget, symBones);
  if (!pGameModel) return;

  int nBoneIndex = pGameModel->FindBone(CastElement<CDmeTransform>(pTarget));
  if (nBoneIndex < 0) return;
  ...
  if (pGameModel->GetSrcBoneTransforms(&preTransform, &postTransform,
                                       nBoneIndex)) {
  ...
  for (int i = 0; i < nLayerCount; ++i) {
    if (logType == AT_VECTOR3) {
      CDmeVector3LogLayer *pPositionLog =
          CastElement<CDmeVector3LogLayer>(pChannel->GetLog()->GetLayer(i));
      if (bHasPreTransform) {
      ...
      if (bHasPostTransform) {
        Assert(fabs(postTransform[0][3]) < 1e-3 &&
               fabs(postTransform[1][3]) < 1e-3 &&
               fabs(postTransform[2][3]) < 1e-3);
      }
#endif
    } else {
      CDmeQuaternionLogLayer *pOrientationLog =
          CastElement<CDmeQuaternionLogLayer>(pChannel->GetLog()->GetLayer(i));
      if (bHasPreTransform) {
...
//-----------------------------------------------------------------------------
void CBaseAnimationSetEditor::FixupChannelsClipTime(
    CDmeChannelsClip *pChannelsClip, CDmeLog *pLog) {
...
// Expands channels clip time to encompass log
//-----------------------------------------------------------------------------
void CBaseAnimationSetEditor::FixupChannelsClipTime(CDmeChannel *pChannel,
                                                    CDmeLog *pLog) {
...
void CBaseAnimationSetEditor::OnImportConfirmed(KeyValues *pParams) {
  ...
  CDmeChannelsClip *pChannelsClip =
      GetElementKeyValue<CDmeChannelsClip>(pImportParams, "channelsClip");
  ...
  int nCount = bVisibleOnly ? BuildVisibleControlList(controls)
                            : BuildFullControlList(controls);
...
// Imports a specific channels clip into the animation set
//-----------------------------------------------------------------------------
void CBaseAnimationSetEditor::ImportAnimation(CDmeChannelsClip *pChannelsClip,
                                              bool bVisibleOnly) {
  CUtlVector<LogPreview_t> controls;
  int nCount = bVisibleOnly ? BuildVisibleControlList(controls)
                            : BuildFullControlList(controls);

  COperationFileListFrame *pStatusFrame = new COperationFileListFrame(
      this, "Import the Following Channels?", "Target Control", false);
  ...
  CDmeChannel **ppFoundChannels =
      (CDmeChannel **)_alloca(nSrcCount * sizeof(CDmeChannel *));
  int nFoundCount = 0;

  for (int i = 0; i < nCount; ++i) {
    for (int k = 0; k < LOG_PREVIEW_MAX_CHANNEL_COUNT; ++k) {
      CDmeChannel *pChannel = controls[i].m_hChannels[k];
      if (!pChannel || pChannel->GetToElement() == NULL) continue;

      char pChannelInfo[512];
      Q_snprintf(pChannelInfo, sizeof(pChannelInfo), "\"%s\" : %s",
                 pChannel->GetToElement()->GetName(),
                 pChannel->GetToAttribute()->GetName());

      CDmeChannel *pImportChannel = FindImportChannel(pChannel, pChannelsClip);
      if (!pImportChannel) {
        pStatusFrame->AddOperation("No source channel", pChannelInfo,
                                   Color(255, 0, 0, 255));
        continue;
      }
      ...
      Q_snprintf(pImportInfo, sizeof(pImportInfo), "\"%s\" : %s",
                 pImportChannel->GetToElement()->GetName(),
                 pImportChannel->GetToAttribute()->GetName());
      pStatusFrame->AddOperation(pImportInfo, pChannelInfo,
                                 Color(0, 255, 0, 255));
    }
  }
  ...
  for (int i = 0; i < nSrcCount; ++i) {
    ...
    for (j = 0; j < nFoundCount; ++j) {
      if (ppFoundChannels[j] == pMissingChannel) break;
    }

    if (j != nFoundCount) continue;

    char pImportInfo[512];
    Q_snprintf(pImportInfo, sizeof(pImportInfo), "\"%s\" : %s",
               pMissingChannel->GetToElement()->GetName(),
               pMissingChannel->GetToAttribute()->GetName());
    pStatusFrame->AddOperation(pImportInfo, "No destination control",
                               Color(255, 255, 0, 255));
  }
...
void CBaseAnimationSetEditor::OnImportAnimationSelected(KeyValues *pParams) {
  ...
  CDmeChannelsClip *pChannelsClip =
      GetElementKeyValue<CDmeChannelsClip>(pParams, "dme");
  if (pChannelsClip) {
...
void CBaseAnimationSetEditor::OnImportAnimationCancelled(KeyValues *pParams) {
  KeyValues *pContextKeyValues = pParams->FindKey("context");
  CDmElement *pAnimationList =
      GetElementKeyValue<CDmElement>(pContextKeyValues, "animationList");
...
//-----------------------------------------------------------------------------
void CBaseAnimationSetEditor::SelectImportAnimation(
    CDmeAnimationList *pAnimationList, bool bVisibleOnly) {
  ...
  CDmePickerFrame *pAnimationPicker =
      new CDmePickerFrame(this, "Select Animation To Import");
...
void CBaseAnimationSetEditor::OnFileSelected(KeyValues *kv) {
  ...
  if (!pAnimationList || pAnimationList->GetAnimationCount() == 0) {
    ...
    vgui::MessageBox *pMessageBox =
        new vgui::MessageBox("Error Importing Animations\n", pBuf, GetParent());
    pMessageBox->DoModal();
...
void CBaseAnimationSetEditor::OnImportAnimation(KeyValues *pParams) {
  ...
  CDmeGameModel *pGameModel =
      m_AnimSet->GetValueElement<CDmeGameModel>("gameModel");
  ...
  if (!pGameModel) {
    ...
    Q_snprintf(pRelativePath, sizeof(pRelativePath), "models/%s/animations/dmx",
               pModelName);
  ...
  KeyValues *pContextKeyValues = new KeyValues("ImportAnimation", "visibleOnly",
                                               pParams->GetInt("visibleOnly"));
  FileOpenDialog *pDialog = new FileOpenDialog(
      this, "Select Animation File Name", true, pContextKeyValues);
  pDialog->SetStartDirectoryContext("animation_set_import_animation",
                                    pStartingDir);
...
//-----------------------------------------------------------------------------
void CBaseAnimationSetEditor::SetupFileOpenDialog(
  ...
  pDialog->AddFilter("*.*", "All Files (*.*)", false);
  pDialog->AddFilter("*.dmx", "Facial animation file (*.dmx)", true,
                     pFileFormat);
}

bool CBaseAnimationSetEditor::OnReadFileFromDisk(const char *pFileName,
...
//-----------------------------------------------------------------------------
bool CBaseAnimationSetEditor::OnWriteFileToDisk(const char *pFileName,
                                                const char *pFileFormat,
                                                KeyValues *pContextKeyValues) {
  ...
  CDisableUndoScopeGuard guard;
  bool bOk = ExportFacialAnimation(pFileName, GetRootClip(),
                                   GetAnimationSetClip(), m_AnimSet);
  return bOk;
}
...
void CBaseAnimationSetEditor::OnExportFacialAnimation() {
  ...
  m_hFileOpenStateMachine->SaveFile(pContextKeyValues, NULL, "facial_animation",
...
void CBaseAnimationSetEditor::OnOpenContextMenu(KeyValues *params) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    m_hContextMenu->AddMenuItem(
        data.contextmenulabel,
        new KeyValues("OnChangeLayout", "value", (int)data.type), this);
  }

  if (m_AnimSet.Get()) {
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#ImportAnimation",
                                new KeyValues("ImportAnimation"), this);
    m_hContextMenu->AddMenuItem("#ReattachToModel",
                                new KeyValues("ReattachToModel"), this);
    m_hContextMenu->AddMenuItem("#ExportFacialAnimation",
                                new KeyValues("ExportFacialAnimation"), this);
  }
...
CBaseAnimationSetEditor::GetAttributeSlider() {
...
void CBaseAnimationSetEditor::SetRecordingState(RecordingState_t state,
                                                bool /*updateSettings*/) {
...
int CBaseAnimationSetEditor::BuildVisibleControlList(
    CUtlVector<LogPreview_t> &list) {
  return m_hAttributeSlider->BuildVisibleControlList(list);
}

int CBaseAnimationSetEditor::BuildFullControlList(
    CUtlVector<LogPreview_t> &list) {
