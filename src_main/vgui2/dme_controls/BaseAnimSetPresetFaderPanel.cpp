...
class CAddPresetDialog : public vgui::BaseInputDialog {
 ...
 public:
  ...
  void DoModal(CDmeAnimationSet *pAnimationSet,
               KeyValues *pContextKeyValues = NULL);
...
// Constructor
//-----------------------------------------------------------------------------
CAddPresetDialog::CAddPresetDialog(vgui::Panel *parent)
    : BaseClass(parent, "Enter Preset Name") {
...
void CAddPresetDialog::DoModal(CDmeAnimationSet *pAnimationSet,
...
void CAddPresetDialog::OnCommand(const char *command) {
  if (!Q_stricmp(command, "OK")) {
    ...
    CDmePresetGroup *pPresetGroup =
        pCurrentGroup
            ? GetElementKeyValue<CDmePresetGroup>(pCurrentGroup, "presetGroup")
            : NULL;
...
//-----------------------------------------------------------------------------
//
// CPresetSliderEdgeButton: The buttons that lie on either side of the
// PresetSlider
//
//-----------------------------------------------------------------------------
class CPresetSliderEdgeButton : public Button {
 ...
 public:
  CPresetSliderEdgeButton(CPresetSlider *parent, const char *panelName,
                          const char *text);
...
class CPresetSlider : public Slider {
 ...
 public:
  ...
  CPresetSlider(CBaseAnimSetPresetFaderPanel *parent, const char *panelName,
                CDmePreset *pPreset);
  ~CPresetSlider();
...
//
//-----------------------------------------------------------------------------
CPresetSliderEdgeButton::CPresetSliderEdgeButton(CPresetSlider *parent,
...
void CPresetSliderEdgeButton::OnMousePressed(vgui::MouseCode code) {
  ...
  PostMessage(m_pSlider->GetVPanel(),
              new KeyValues("MousePressed", "code", code));
  PostMessage(m_pSlider->GetVPanel(),
              new KeyValues("MouseReleased", "code", code), 0.001f);
}
...
//-----------------------------------------------------------------------------
CPresetSlider::CPresetSlider(CBaseAnimSetPresetFaderPanel *parent,
                             const char *panelName, CDmePreset *preset)
  ...
  m_pEdgeButtons[0] =
      new CPresetSliderEdgeButton(this, "PresetSliderLeftEdge", "");
  m_pEdgeButtons[1] =
      new CPresetSliderEdgeButton(this, "PresetSliderRightEdge", "");
...
CPresetSlider::~CPresetSlider() { delete m_pName; }

CDmePreset *CPresetSlider::GetPreset() { return m_hSelf; }

// #define PRORCEDURAL_PRESET_TIMING

void CPresetSlider::UpdateProceduralValues() {
  if (!m_hSelf->IsProcedural()) return;
  ...
  switch (nPresetType) {
    ...
    case PROCEDURAL_PRESET_STAGGER:
      // These are handled elsewhere right now... at some point we'll copy in
      // values at head position in order to do ctrl-key preview mode
      break;
...
void CPresetSlider::OnDragCompleted(float flValue) {
  if (m_bSuppressCompletion) return;
...
void CPresetSlider::OnRenameCompleted(const char *pText,
                                      KeyValues *pContextKeyValues) {
  if (!pText || !*pText) {
...
void CPresetSlider::OnDelete() {
  if (m_hConfirm.Get()) delete m_hConfirm.Get();
...
void CPresetSlider::OnShowContextMenu() {
  ...
  if (bCanOverwrite) {
    m_hContextMenu->AddMenuItem("Overwrite", new KeyValues("OnOverwrite"),
                                this);
  ...
  m_hContextMenu->AddMenuItem("Change Crossfade Speed...",
                              new KeyValues("SetPresetCrossfadeSpeed"),
                              m_pParent);

  m_hContextMenu->AddSeparator();

  m_hContextMenu->AddMenuItem("Manage...", new KeyValues("ManagePresets"),
                              m_pParent);
...
void CPresetSlider::UpdateTickPos(int x, int y) {
  ...
  bool bIsCtrlKeyDown = vgui::input()->IsKeyDown(KEY_LCONTROL) ||
                        vgui::input()->IsKeyDown(KEY_RCONTROL);
  bool bIsAltKeyDown =
...
void CPresetSlider::OnCursorMoved(int x, int y) { UpdateTickPos(x, y); }

bool CPresetSlider::IsDragging() { return _dragging; }

bool CPresetSlider::IsPreviewSlider() {
  VPANEL capturePanel = input()->GetMouseCapture();
  if (capturePanel) {
    if (capturePanel != GetVPanel()) return false;
    return true;
  }

  VPANEL appModal = input()->GetAppModalSurface();
  if (appModal) return false;
...
float CPresetSlider::GetCurrent() { return GetValue() * 0.001f; }
...
void CPresetSlider::Paint() {
  if (!IsPreviewSlider()) return;

  bool bIsCtrlKeyDown = vgui::input()->IsKeyDown(KEY_LCONTROL) ||
                        vgui::input()->IsKeyDown(KEY_RCONTROL);
  if (!IsDragging() && !bIsCtrlKeyDown) return;
...
void CPresetSlider::PaintBackground() {
  ...
  if (hasFocus && (IsDragging() || bIsCtrlKeyDown)) {
    ...
    int curAlpha = std::max(GetCurrent() * alphaTarget, (float)minAlpha);

    if (_dragging) {
      surface()->DrawFilledRectFade(tx, ty, tx + fillw, ty + th, minAlpha,
                                    curAlpha, true);
      surface()->DrawSetColor(m_ZeroColor);
...
class CSliderListPanel : public PanelListPanel {
 ...
 public:
  CSliderListPanel(CBaseAnimSetPresetFaderPanel *parent, vgui::Panel *pParent,
                   const char *panelName);
...
// Constructor
//-----------------------------------------------------------------------------
CSliderListPanel::CSliderListPanel(CBaseAnimSetPresetFaderPanel *pFader,
                                   vgui::Panel *pParent, const char *panelName)
...
// Constructor
//-----------------------------------------------------------------------------
CBaseAnimSetPresetFaderPanel::CBaseAnimSetPresetFaderPanel(
    vgui::Panel *parent, const char *className, CBaseAnimationSetEditor *editor)
  ...
  m_pSliders->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT,
                            0, 24, 0, 0);
  ...
  m_pFilter->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_RIGHT, 2, 2, 2,
                           22);
  m_pFilter->SetPos(0, 0);

  m_pWorkspace->SetAutoResize(Panel::PIN_TOPLEFT,
...
void CBaseAnimSetPresetFaderPanel::OnPresetNameSelected(KeyValues *pParams) {
  const char *pText = pParams->GetString("text", NULL);
  if (!pText || !*pText) {
    vgui::MessageBox *pError = new MessageBox(
        "Add Preset Error", "Can't add preset with an empty name\n", this);
  ...
  CDmePresetGroup *pPresetGroup =
      GetElementKeyValue<CDmePresetGroup>(pParams, "presetGroup");
  if (!pPresetGroup) {
    const char *pGroupName = pParams->GetString("presetGroupName");
    if (!pGroupName) return;
  ...
  if (pPresetGroup->m_bIsReadOnly) {
    vgui::MessageBox *pError =
        new MessageBox("Add Preset Error",
                       "Can't add preset to a read-only preset group!\n", this);
    pError->SetDeleteSelfOnClose(true);
    pError->DoModal();
    return;
  }
  ...
  if (pPresetGroup->FindPreset(pText)) {
    vgui::MessageBox *pError = new MessageBox(
        "Add Preset Error", "A preset with that name already exists!\n", this);
    pError->SetDeleteSelfOnClose(true);
    pError->DoModal();
    return;
  }
...
void CBaseAnimSetPresetFaderPanel::OnSetCrossfadeSpeed() {
  ...
  m_hInputDialog =
      new InputDialog(this, "Crossfade Speed", "Fader Crossfade Rate:", sz);
  if (m_hInputDialog.Get()) {
...
void CBaseAnimSetPresetFaderPanel::GetPreviewFader(FaderPreview_t &fader) {
  ...
  for (int i = m_pSliders->FirstItem(); i != m_pSliders->InvalidItemID();
       i = m_pSliders->NextItem(i)) {
    CPresetSlider *slider =
        static_cast<CPresetSlider *>(m_pSliders->GetItemPanel(i));
    if (!slider || !slider->IsPreviewSlider()) continue;
  ...
  if (mouseOver) {
    ...
    if (fader.holdingctrl || fader.isbeingdragged) {
...
void CBaseAnimSetPresetFaderPanel::PopulateList(bool bChanged) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    Assert(pPresetGroup);
    if (!pPresetGroup || !pPresetGroup->m_bIsVisible) continue;
  ...
  if (bNeedRebuild) {
    ...
    for (int i = 0; i < c; ++i) {
      ...
      for (int j = 0; j < cp; ++j) {
        ...
        CPresetSlider *pSlider =
            new CPresetSlider(this, pPreset->GetName(), pPreset);
        pSlider->SetPos(0);
...
void CBaseAnimSetPresetFaderPanel::ChangeAnimationSet(
    CDmeAnimationSet *newAnimSet) {
...
void CBaseAnimSetPresetFaderPanel::ApplyPreset(float flScale,
                                               AttributeDict_t &dict) {
  CBaseAnimSetAttributeSliderPanel *sliderPanel =
      m_hEditor->GetAttributeSlider();
  if (sliderPanel) {
...
void CBaseAnimSetPresetFaderPanel::SetPresetFromSliders(CDmePreset *pPreset) {
  ...
  CBaseAnimSetAttributeSliderPanel *pSliderPanel =
      m_hEditor->GetAttributeSlider();
  if (!pSliderPanel) return;
  ...
  for (int i = 0; i < numControls; ++i) {
    ...
    bool bIsDefault =
        !pSliderPanel->GetAttributeSliderValue(&value, pControl->GetName());
    if (!bIsDefault) {
      float flDefaultValue = pControl->GetValue<float>("defaultValue");
      float flDefaultBalance = pControl->GetValue<float>("defaultBalance");
      float flDefaultMultilevel =
          pControl->GetValue<float>("defaultMultilevel");
      bIsDefault =
          (value.m_pValue[ANIM_CONTROL_VALUE] == flDefaultValue) &&
    ...
    CDmElement *pControlValue =
        pPreset->FindOrAddControlValue(pControl->GetName());
    pControlValue->SetValue<float>("value", value.m_pValue[ANIM_CONTROL_VALUE]);
    if (bIsCombo) {
      pControlValue->SetValue<float>("balance",
                                     value.m_pValue[ANIM_CONTROL_BALANCE]);
    } else {
      pControlValue->RemoveAttribute("balance");
    }

    if (bIsMulti) {
      pControlValue->SetValue<float>("multilevel",
                                     value.m_pValue[ANIM_CONTROL_MULTILEVEL]);
    } else {
...
void CBaseAnimSetPresetFaderPanel::AddNewPreset(const char *pGroupName,
                                                const char *pName) {
  if (!m_AnimSet.Get()) return;
...
void CBaseAnimSetPresetFaderPanel::ProceduralPreset_UpdateCrossfade(
    CDmePreset *pPreset, bool bFadeIn) {
