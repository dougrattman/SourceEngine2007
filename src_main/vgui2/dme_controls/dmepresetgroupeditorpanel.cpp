...
DEFINE_SOURCE_NOTIFY_SCOPE_GUARD(PresetGroup,
...
// Constructor
//-----------------------------------------------------------------------------
static int __cdecl DestPresetNameSortFunc(vgui::ListPanel *pPanel,
...
static int __cdecl SrcPresetNameSortFunc(vgui::ListPanel *pPanel,
                                         const vgui::ListPanelItem &item1,
                                         const vgui::ListPanelItem &item2) {
...
CDmePresetRemapPanel::CDmePresetRemapPanel(vgui::Panel *pParent,
                                           const char *pTitle)
...
CDmePresetRemapPanel::~CDmePresetRemapPanel() { CleanupContextMenu(); }
...
void CDmePresetRemapPanel::RefreshPresetList() {
  ...
  CDmaElementArray<CDmePreset> *pPresetList =
      m_hDestGroup.Get() ? &m_hDestGroup->GetPresets() : NULL;
  if (!pPresetList) return;
  ...
  bool bUseRemap =
      (pRemap && m_hSourceGroup.Get() &&
       !Q_stricmp(pRemap->m_SourcePresetGroup, m_hSourceGroup->GetName()));
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (bUseRemap) {
      ...
      CDmePreset *pSrcPreset =
          pSource ? m_hSourceGroup->FindPreset(pSource) : NULL;
...
void CDmePresetRemapPanel::OnPresetPicked(KeyValues *pParams) {
  ...
  if (nSelectedItemCount != 1) return;
...
void CDmePresetRemapPanel::OnSelectPreset() {
  ...
  CPresetPickerFrame *pPresetPicker =
      new CPresetPickerFrame(this, "Select Source Preset", false);
...
void CDmePresetRemapPanel::OnOpenContextMenu(KeyValues *kv) {
  ...
  m_hContextMenu->AddMenuItem("#DmePresetRemapPanel_SelectPreset",
                              new KeyValues("SelectPreset"), this);
  m_hContextMenu->AddMenuItem("#DmePresetRemapPanel_RemovePreset",
                              new KeyValues("RemovePreset"), this);
...
void CDmePresetRemapPanel::OnTextChanged() {
  ...
  m_hSourceGroup = pCurrentGroup ? GetElementKeyValue<CDmePresetGroup>(
                                       pCurrentGroup, "presetGroup")
                                 : NULL;
  RefreshPresetList();
}

void CDmePresetRemapPanel::DoModal(CDmeAnimationSet *pAnimationSet,
                                   CDmePresetGroup *pDestGroup) {
  ...
  const CDmaElementArray<CDmePresetGroup> &presetGroupList =
      pAnimationSet->GetPresetGroups();
  int nCount = presetGroupList.Count();
  for (int i = 0; i < nCount; ++i) {
    CDmePresetGroup *pPresetGroup = presetGroupList[i];
    if (pPresetGroup == m_hDestGroup.Get()) continue;

    KeyValues *kv = new KeyValues("entry");
    SetElementKeyValue(kv, "presetGroup", pPresetGroup);
    int nItemID = m_pSourcePresetGroup->AddItem(pPresetGroup->GetName(), kv);
    if (!bSelected || (pRemap && !Q_stricmp(pRemap->m_SourcePresetGroup,
                                            pPresetGroup->GetName()))) {
...
void CDmePresetRemapPanel::ApplyChangesToPresetRemap() {
  ...
  for (int nItemID = m_pPresetRemapList->FirstItem();
    ...
    CDmePreset *pSrcPreset =
        GetElementKeyValue<CDmePreset>(pKeyValues, "srcPreset");
    CDmePreset *pDestPreset =
        GetElementKeyValue<CDmePreset>(pKeyValues, "destPreset");
...
class CDmePresetGroupListPanel : public vgui::ListPanel {
 ...
 public:
  ...
  CDmePresetGroupListPanel(vgui::Panel *pParent, const char *pName,
...
class CDmePresetListPanel : public vgui::ListPanel {
 ...
 public:
  // constructor, destructor
  CDmePresetListPanel(vgui::Panel *pParent, const char *pName,
                      CDmePresetGroupEditorPanel *pComboPanel);
...
//-----------------------------------------------------------------------------
static int __cdecl IndexSortFunc(vgui::ListPanel *pPanel,
                                 const vgui::ListPanelItem &item1,
                                 const vgui::ListPanelItem &item2) {
...
// constructor, destructor
//-----------------------------------------------------------------------------
CDmePresetGroupEditorPanel::CDmePresetGroupEditorPanel(vgui::Panel *pParent,
                                                       const char *pName)
    : BaseClass(pParent, pName) {
  m_pSplitter = new vgui::Splitter(this, "PresetGroupSplitter",
                                   vgui::SPLITTER_MODE_VERTICAL, 1);
  vgui::Panel *pSplitterLeftSide = m_pSplitter->GetChild(0);
  vgui::Panel *pSplitterRightSide = m_pSplitter->GetChild(1);

  m_pPresetGroupList =
      new CDmePresetGroupListPanel(pSplitterLeftSide, "PresetGroupList", this);
  ...
  m_pPresetList =
      new CDmePresetListPanel(pSplitterRightSide, "PresetList", this);
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::SetAnimationSet(
...
void CDmePresetGroupEditorPanel::RefreshAnimationSet() {
  ...
  if (!m_hAnimationSet.Get()) return;

  const CDmaElementArray<CDmePresetGroup> &presetGroupList =
      m_hAnimationSet->GetPresetGroups();
  int nCount = presetGroupList.Count();
  for (int i = 0; i < nCount; ++i) {
    CDmePresetGroup *pPresetGroup = presetGroupList[i];
    Assert(pPresetGroup);
    ...
    SetElementKeyValue(kv, "presetGroup", pPresetGroup);
    kv->SetColor("cellcolor", pPresetGroup->m_bIsReadOnly
                                  ? Color(255, 0, 0, 255)
...
CDmePreset *CDmePresetGroupEditorPanel::GetSelectedPreset() {
  if (!m_hAnimationSet.Get()) return NULL;

  int nSelectedPresetCount = m_pPresetList->GetSelectedItemsCount();
  if (nSelectedPresetCount != 1) return NULL;
...
void CDmePresetGroupEditorPanel::SetSelectedPreset(CDmePreset *pPreset) {
  ...
  for (int nItemID = m_pPresetList->FirstItem();
    ...
    KeyValues *pKeyValues = m_pPresetList->GetItem(nItemID);
    CDmePreset *pItemPreset =
        GetElementKeyValue<CDmePreset>(pKeyValues, "preset");
...
CDmePresetGroup *CDmePresetGroupEditorPanel::GetSelectedPresetGroup() {
  ...
  if (nSelectedItemCount != 1) return NULL;
  ...
  CDmePresetGroup *pPresetGroup =
      GetElementKeyValue<CDmePresetGroup>(pKeyValues, "presetGroup");
  return pPresetGroup;
}
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::SetSelectedPresetGroup(
    CDmePresetGroup *pPresetGroup) {
  ...
  for (int nItemID = m_pPresetGroupList->FirstItem();
    ...
    KeyValues *pKeyValues = m_pPresetGroupList->GetItem(nItemID);
    CDmePresetGroup *pItemPresetGroup =
        GetElementKeyValue<CDmePresetGroup>(pKeyValues, "presetGroup");
...
//-----------------------------------------------------------------------------
// If it finds a duplicate preset name, reports an error message and returns it
// found one
//-----------------------------------------------------------------------------
bool CDmePresetGroupEditorPanel::HasDuplicatePresetName(
    const char *pPresetName, CDmePreset *pIgnorePreset) {
  CDmePresetGroup *pPresetGroup = GetSelectedPresetGroup();
  ...
  if (pMatch && pMatch != pIgnorePreset) {
    vgui::MessageBox *pError = new vgui::MessageBox(
        ...
        "#DmePresetGroupEditor_DuplicatePresetNameText", this);
    pError->DoModal();
    return true;
  }
...
void CDmePresetGroupEditorPanel::PerformAddPreset(const char *pNewPresetName) {
  ...
  if (HasDuplicatePresetName(pNewPresetName)) return;
...
// Called by OnInputCompleted after we get a new group name
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::PerformRenamePreset(
    const char *pNewPresetName) {
  CDmePreset *pPreset = GetSelectedPreset();
  if (!pPreset) return;

  if (HasDuplicatePresetName(pNewPresetName, pPreset)) return;
...
void CDmePresetGroupEditorPanel::OnAddPreset() {
  vgui::InputDialog *pInput =
      new vgui::InputDialog(this, "Add Preset", "Enter name of new preset");
...
void CDmePresetGroupEditorPanel::OnRenamePreset() {
  ...
  vgui::InputDialog *pInput =
      new vgui::InputDialog(this, "Rename Preset", "Enter new name of preset");
  pInput->SetMultiline(false);
...
void CDmePresetGroupEditorPanel::OnMovePresetUp() {
  CDmePresetGroup *pPresetGroup = GetSelectedPresetGroup();
  CDmePreset *pPreset = GetSelectedPreset();
  if (!pPresetGroup || !pPreset) return;
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::MovePresetInFrontOf(CDmePreset *pDragPreset,
                                                     CDmePreset *pDropPreset) {
  CDmePresetGroup *pPresetGroup = GetSelectedPresetGroup();
  if (!pPresetGroup) return;
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::OnFileStateMachineFinished(
    KeyValues *pParams) {
  KeyValues *pContextKeyValues = pParams->GetFirstTrueSubKey();
  if (Q_stricmp(pContextKeyValues->GetName(), "ImportPresets")) return;

  CDmElement *pRoot =
      GetElementKeyValue<CDmElement>(pContextKeyValues, "presets");
  if (!pRoot) return;

  if (pParams->GetInt("completionState", 0) != 0) {
    CPresetPickerFrame *pPresetPicker =
        new CPresetPickerFrame(this, "Select Preset(s) to Import");
    pPresetPicker->AddActionSignalTarget(this);
...
// Fileopen state machine
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::SetupFileOpenDialog(
    vgui::FileOpenDialog *pDialog, bool bOpenFile, const char *pFileFormat,
  ...
  if (!Q_stricmp(pFileFormat, PRESET_FILE_FORMAT)) {
    ...
    pDialog->AddFilter("*.pre", "Preset File (*.pre)", true,
                       PRESET_FILE_FORMAT);
    ...
    pDialog->SetStartDirectoryContext("preset_exportvfe", pPresetPath);
    pDialog->AddFilter("*.*", "All Files (*.*)", false);
    pDialog->AddFilter("*.txt", "Faceposer Expression File (*.txt)", true,
                       "txt");
  }
}

bool CDmePresetGroupEditorPanel::OnReadFileFromDisk(
    const char *pFileName, const char *pFileFormat,
    KeyValues *pContextKeyValues) {
  CDmElement *pRoot;
  CDisableUndoScopeGuard sg;
  DmFileId_t fileId = g_pDataModel->RestoreFromFile(
      pFileName, NULL, pFileFormat, &pRoot, CR_FORCE_COPY);
  sg.Release();

  if (fileId == DMFILEID_INVALID) return false;
  ...
  if (!Q_stricmp(pContextKeyValues->GetName(), "ImportPresetGroup")) {
    ...
    if (pPresetRemap) {
      if (m_hAnimationSet->FindPresetGroup(pPresetRemap->m_SourcePresetGroup) ==
          NULL) {
        char pBuf[512];
        Q_snprintf(pBuf, sizeof(pBuf),
                   "Import contains a remap which refers to an unknown preset "
                   "group \"%s\"!\n",
                   pPresetRemap->m_SourcePresetGroup.Get());
        vgui::MessageBox *pError =
            new vgui::MessageBox("Bad source remap name!", pBuf, this);
        pError->DoModal();
      }
    }
  ...
  if (!pPresets) return false;

  SetElementKeyValue(pContextKeyValues, "presets", pRoot);
  return true;
}

bool CDmePresetGroupEditorPanel::OnWriteFileToDisk(
    const char *pFileName, const char *pFileFormat,
    KeyValues *pContextKeyValues) {
  // Used when exporting an entire preset group
  if (!Q_stricmp(pContextKeyValues->GetName(), "ExportPresetGroup")) {
    CDmePresetGroup *pPresetGroup =
        GetElementKeyValue<CDmePresetGroup>(pContextKeyValues, "presetGroup");
    if (!pPresetGroup) return false;

    bool bOk = g_pDataModel->SaveToFile(
        pFileName, NULL, g_pDataModel->GetDefaultEncoding(pFileFormat),
        pFileFormat, pPresetGroup);
    return bOk;
  }
  ...
  const char *pPresetGroupName =
      pPresetGroup ? pPresetGroup->GetName() : "root";

  CDisableUndoScopeGuard sg;
  CDmePresetGroup *pRoot =
      CreateElement<CDmePresetGroup>(pPresetGroupName, DMFILEID_INVALID);
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CDmePreset *pPreset =
        GetElementKeyValue<CDmePreset>(pContextKeyValues, pBuf);
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::ImportPresets(
    const CUtlVector<CDmePreset *> &presets) {
  CDmePresetGroup *pPresetGroup = GetSelectedPresetGroup();
  if (!pPresetGroup) return;
  ...
  for (int i = 0; i < nPresetCount; ++i) {
    ...
    const CDmaElementArray<CDmElement> &srcValues =
        presets[i]->GetControlValues();
...
void CDmePresetGroupEditorPanel::OnPresetPicked(KeyValues *pParams) {
  ...
  if (pParams->FindKey("ExportPicked")) {
    ...
    m_hFileOpenStateMachine->SaveFile(pContextKeyValues, NULL,
                                      PRESET_FILE_FORMAT,
...
void CDmePresetGroupEditorPanel::OnExportPresets() {
  ...
  CPresetPickerFrame *pPresetPicker =
      new CPresetPickerFrame(this, "Select Preset(s) to Export");
  pPresetPicker->AddActionSignalTarget(this);
...
void CDmePresetGroupEditorPanel::OnExportPresetGroupToVFE() {
  ...
  m_hFileOpenStateMachine->SaveFile(pContextKeyValues, NULL, "vfe",
                                    vgui::FOSM_SHOW_PERFORCE_DIALOGS);
}
...
void CDmePresetGroupEditorPanel::OnExportPresetGroupToTXT() {
  ...
  SetElementKeyValue(pContextKeyValues, "presetGroup", pPresetGroup);
  m_hFileOpenStateMachine->SaveFile(pContextKeyValues, NULL, "txt",
                                    vgui::FOSM_SHOW_PERFORCE_DIALOGS);
}
...
void CDmePresetGroupEditorPanel::OnExportPresetGroups() {
  ...
  SetElementKeyValue(pContextKeyValues, "presetGroup", pPresetGroup);
  m_hFileOpenStateMachine->SaveFile(pContextKeyValues, NULL, PRESET_FILE_FORMAT,
                                    vgui::FOSM_SHOW_PERFORCE_DIALOGS);
}
...
void CDmePresetGroupEditorPanel::OnRemoveDefaultControls() {
  ...
  for (int i = 0; i < nPresetCount; ++i) {
    ...
    for (int j = nControlCount; --j >= 0;) {
      ...
      CDmElement *pControl =
          m_hAnimationSet->FindControl(pControlValue->GetName());
      if (!pControl) {
        controls.Remove(j);
        continue;
      }
      ...
      if (pControl->GetValue<float>("defaultValue") !=
          pControlValue->GetValue<float>("value")) {
        bIsDefault = false;
      }

      bool bIsStereo = pControl->GetValue<bool>("combo");
      if (bIsStereo) {
        if (pControl->GetValue<float>("defaultBalance") !=
            pControlValue->GetValue<float>("balance")) {
          bIsDefault = false;
        }
      }
      ...
      if (bIsMulti) {
        if (pControl->GetValue<float>("defaultMultilevel") !=
            pControlValue->GetValue<float>("multilevel")) {
          bIsDefault = false;
        }
      }
...
void CDmePresetGroupEditorPanel::OnEditPresetRemapping() {
  ...
  CDmePresetRemapPanel *pPresetRemapPanel =
      new CDmePresetRemapPanel(this, "Manage Preset Remapping");
...
void CDmePresetGroupEditorPanel::OnRemapPresets() {
  ...
  if (!pDestPresetGroup || pDestPresetGroup->m_bIsReadOnly) return;

  CDmePresetRemap *pPresetRemap = pDestPresetGroup->GetPresetRemap();
  if (!pPresetRemap) return;

  CDmePresetGroup *pSourcePresetGroup =
      m_hAnimationSet->FindPresetGroup(pPresetRemap->m_SourcePresetGroup);
  if (!pSourcePresetGroup) {
    char pBuf[512];
    Q_snprintf(pBuf, sizeof(pBuf),
               "Unable to find preset group name %s in animation set %s!\n",
               pPresetRemap->m_SourcePresetGroup.Get(),
               m_hAnimationSet->GetName());
    vgui::MessageBox *pError =
        new vgui::MessageBox("Bad source remap name!", pBuf, this);
    pError->DoModal();
    return;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    if (!pSourcePreset || !pDestPreset) continue;
...
void CDmePresetGroupEditorPanel::OnOpenPresetContextMenu() {
  ...
  if (pPresetGroup->m_bIsReadOnly) {
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ExportPresets",
                                new KeyValues("ExportPresets"), this);
    vgui::Menu::PlaceContextMenu(this, m_hContextMenu.Get());
    return;
  }

  m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_AddPreset",
                              new KeyValues("AddPreset"), this);
  if (pPreset) {
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_RenamePreset",
                                new KeyValues("RenamePreset"), this);
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_RemovePreset",
                                new KeyValues("RemovePreset"), this);
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_MoveUp",
                                new KeyValues("MovePresetUp"), this);
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_MoveDown",
                                new KeyValues("MovePresetDown"), this);
  }

  m_hContextMenu->AddSeparator();
  m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ImportPresets",
                              new KeyValues("ImportPresets"), this);
  m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ExportPresets",
                              new KeyValues("ExportPresets"), this);
...
void CDmePresetGroupEditorPanel::OnOpenContextMenu(KeyValues *kv) {
  ...
  if (pPanel != m_pPresetGroupList) return;

  m_hContextMenu = new vgui::Menu(this, "ActionMenu");
  m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_AddGroup",
                              new KeyValues("AddGroup"), this);
  m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_AddPhonemeGroup",
                              new KeyValues("AddPhonemeGroup"), this);

  CDmePresetGroup *pPresetGroup = GetSelectedPresetGroup();
  if (pPresetGroup) {
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_RenameGroup",
                                new KeyValues("RenameGroup"), this);
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_RemoveGroup",
                                new KeyValues("RemoveGroup"), this);
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ToggleVisibility",
                                new KeyValues("ToggleGroupVisibility"), this);
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ToggleSharing",
                                new KeyValues("ToggleGroupSharing"), this);
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_MoveUp",
                                new KeyValues("MoveGroupUp"), this);
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_MoveDown",
                                new KeyValues("MoveGroupDown"), this);
    ...
    if (!pPresetGroup->m_bIsReadOnly) {
      m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_RemoveDefaultControls",
                                  new KeyValues("RemoveDefaultControls"), this);
      m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_EditPresetRemapping",
                                  new KeyValues("EditPresetRemapping"), this);
    }
    if (pPresetRemap) {
      m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_RemapPresets",
                                  new KeyValues("RemapPresets"), this);
    }
  }
  m_hContextMenu->AddSeparator();
  m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ImportPresets",
                              new KeyValues("ImportPresetGroups"), this);
  if (pPresetGroup) {
    m_hContextMenu->AddMenuItem("#DmePresetGroupEditor_ExportPresets",
                                new KeyValues("ExportPresetGroups"), this);
    m_hContextMenu->AddMenuItem(
        "#DmePresetGroupEditor_ExportPresetsToFaceposer",
        new KeyValues("ExportPresetGroupsToTXT"), this);
    m_hContextMenu->AddMenuItem(
        "#DmePresetGroupEditor_ExportPresetsToExpression",
        new KeyValues("ExportPresetGroupsToVFE"), this);
  }
...
//-----------------------------------------------------------------------------
// If it finds a duplicate control name, reports an error message and returns it
// found one
//-----------------------------------------------------------------------------
bool CDmePresetGroupEditorPanel::HasDuplicateGroupName(
    const char *pGroupName, CDmePresetGroup *pIgnorePreset) {
  if (!m_hAnimationSet) return false;

  CDmePresetGroup *pMatch = m_hAnimationSet->FindPresetGroup(pGroupName);
  if (pMatch && pMatch != pIgnorePreset) {
    vgui::MessageBox *pError =
        new vgui::MessageBox("#DmePresetGroupEditor_DuplicateNameTitle",
                             "#DmePresetGroupEditor_DuplicateNameText", this);
    pError->DoModal();
    return true;
  }
...
void CDmePresetGroupEditorPanel::PerformAddGroup(const char *pNewGroupName) {
  if (!m_hAnimationSet) return;

  if (HasDuplicateGroupName(pNewGroupName)) return;

  CPresetGroupUndoScopeGuard sg(NOTIFY_SETDIRTYFLAG, "Add Preset Group");
  CDmePresetGroup *pPresetGroup =
      m_hAnimationSet->FindOrAddPresetGroup(pNewGroupName);
  sg.Release();
...
// Called by OnInputCompleted after we get a new group name
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::PerformAddPhonemeGroup(
    const char *pNewGroupName) {
  if (!m_hAnimationSet) return;

  if (HasDuplicateGroupName(pNewGroupName)) return;

  CPresetGroupUndoScopeGuard sg(NOTIFY_SETDIRTYFLAG,
                                "Add Phoneme Preset Group");
  CDmePresetGroup *pPresetGroup =
      m_hAnimationSet->FindOrAddPresetGroup(pNewGroupName);

  int nPhonemeCount = NumPhonemes();
  for (int i = 0; i < nPhonemeCount; ++i) {
    if (!IsStandardPhoneme(i)) continue;
...
void CDmePresetGroupEditorPanel::PerformRenameGroup(const char *pNewGroupName) {
  ...
  if (HasDuplicateGroupName(pNewGroupName, pPresetGroup)) return;
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::ToggleGroupVisibility(
    CDmePresetGroup *pPresetGroup) {
  if (pPresetGroup) {
    CPresetGroupUndoScopeGuard sg(NOTIFY_SETDIRTYFLAG,
                                  "Toggle Preset Group Visibility");
...
void CDmePresetGroupEditorPanel::OnAddGroup() {
  vgui::InputDialog *pInput = new vgui::InputDialog(
      this, "Add Preset Group", "Enter name of new preset group");
  pInput->SetMultiline(false);
...
void CDmePresetGroupEditorPanel::OnAddPhonemeGroup() {
  vgui::InputDialog *pInput =
      new vgui::InputDialog(this, "Add Phoneme Preset Group",
                            "Enter name of new preset group", "phoneme");
  pInput->SetMultiline(false);
...
void CDmePresetGroupEditorPanel::OnRenameGroup() {
  ...
  vgui::InputDialog *pInput = new vgui::InputDialog(
      this, "Rename Preset Group", "Enter new name of preset group");
  pInput->SetMultiline(false);
...
void CDmePresetGroupEditorPanel::OnRemoveGroup() {
  ...
  if (!Q_stricmp(pPresetGroup->GetName(), "procedural")) {
    vgui::MessageBox *pError = new vgui::MessageBox(
        ...
        "#DmePresetGroupEditor_CannotRemovePresetGroupText", this);
    pError->DoModal();
    return;
  }
...
void CDmePresetGroupEditorPanel::OnMoveGroupUp() {
  CDmePresetGroup *pPresetGroup = GetSelectedPresetGroup();
  if (!pPresetGroup || !m_hAnimationSet.Get()) return;
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::MovePresetGroupInFrontOf(
    CDmePresetGroup *pDragGroup, CDmePresetGroup *pDropGroup) {
  if (!m_hAnimationSet.Get()) return;
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorPanel::MovePresetIntoGroup(CDmePreset *pPreset,
                                                     CDmePresetGroup *pGroup) {
  if (!m_hAnimationSet.Get() || !pPreset || !pGroup) return;
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmePresetGroupListPanel::CDmePresetGroupListPanel(
    vgui::Panel *pParent, const char *pName,
    CDmePresetGroupEditorPanel *pComboPanel)
...
bool CDmePresetGroupListPanel::IsDroppable(CUtlVector<KeyValues *> &msgList) {
  if (msgList.Count() > 0) {
    ...
    if (m_pPresetGroupPanel) {
      CDmePresetGroup *pPresetGroup =
          GetElementKeyValue<CDmePresetGroup>(pData, "presetGroup");
      if (pPresetGroup) return true;
      ...
      if (pPreset) {
        ...
        CDmePresetGroup *pDropGroup =
            pKeyValues
                ? GetElementKeyValue<CDmePresetGroup>(pKeyValues, "presetGroup")
                : NULL;

        if (pDropGroup && !pDropGroup->m_bIsReadOnly) return true;
      }
    }
  }
...
// Called when a list panel's selection changes
//-----------------------------------------------------------------------------
void CDmePresetGroupListPanel::OnPanelDropped(
    CUtlVector<KeyValues *> &msgList) {
  if (msgList.Count() == 0) return;

  KeyValues *pData = msgList[0];
  if (!m_pPresetGroupPanel) return;
...
//-----------------------------------------------------------------------------
void CDmePresetGroupListPanel::OnDroppablePanelPaint(
    CUtlVector<KeyValues *> &msglist, CUtlVector<Panel *> &dragPanels) {
  // Discover the cell the panel is over
...
// Constructor, destructor
//-----------------------------------------------------------------------------
CDmePresetListPanel::CDmePresetListPanel(
    vgui::Panel *pParent, const char *pName,
    CDmePresetGroupEditorPanel *pComboPanel)
    : BaseClass(pParent, pName), m_pPresetGroupPanel(pComboPanel) {}
...
void CDmePresetListPanel::OnCreateDragData(KeyValues *msg) {
  CDmePresetGroup *pPresetGroup = m_pPresetGroupPanel->GetSelectedPresetGroup();
  if (pPresetGroup->m_bIsReadOnly) return;
...
void CDmePresetListPanel::OnPanelDropped(CUtlVector<KeyValues *> &msgList) {
  ...
  KeyValues *pData = msgList[0];
  if (pData->GetPtr("panel", NULL) != this || !m_pPresetGroupPanel) return;
  ...
  if (pDragPreset) {
    CDmePreset *pDropPreset =
        pKeyValues ? GetElementKeyValue<CDmePreset>(pKeyValues, "preset")
                   : NULL;
...
// Mouse is now over a droppable panel
//-----------------------------------------------------------------------------
void CDmePresetListPanel::OnDroppablePanelPaint(
    CUtlVector<KeyValues *> &msglist, CUtlVector<Panel *> &dragPanels) {
  // Discover the cell the panel is over
  int x, y, w, h, row, column;
  vgui::input()->GetCursorPos(x, y);
...
//
//-----------------------------------------------------------------------------
CDmePresetGroupEditorFrame::CDmePresetGroupEditorFrame(vgui::Panel *pParent,
                                                       const char *pTitle)
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorFrame::SetAnimationSet(
    CDmeAnimationSet *pComboSystem) {
...
//-----------------------------------------------------------------------------
void CDmePresetGroupEditorFrame::NotifyDataChanged(const char *pReason,
                                                   int nNotifySource,
                                                   int nNotifyFlags) {
  if (!IsVisible()) return;

  if (nNotifySource == NOTIFY_SOURCE_PRESET_GROUP_EDITOR) return;
