...
// Sort by sound name
//-----------------------------------------------------------------------------
static int __cdecl GameSoundSortFunc(vgui::ListPanel *pPanel,
...
CSoundPicker::CSoundPicker(vgui::Panel *pParent, int nFlags)
  ...
  if (nFlags & PICK_GAMESOUNDS) {
    ...
    m_pGameSoundList->SetAutoResize(Panel::PIN_TOPLEFT,
    ...
    m_pGameSoundPage->LoadControlSettings(
        "resource/soundpickergamesoundpage.res");
...
void CSoundPicker::SetSelectedSound(PickType_t type, const char *pSoundName) {
  if (type == PICK_NONE || !pSoundName) return;
...
void CSoundPicker::OnKeyCodeTyped(KeyCode code) {
  if (m_pGameSoundPage &&
      (m_pViewsSheet->GetActivePage() == m_pGameSoundPage)) {
    if ((code == KEY_UP) || (code == KEY_DOWN) || (code == KEY_PAGEUP) ||
        (code == KEY_PAGEDOWN)) {
      KeyValues *pMsg = new KeyValues("KeyCodeTyped", "code", code);
      vgui::ipanel()->SendMessage(m_pGameSoundList->GetVPanel(), pMsg,
                                  GetVPanel());
      pMsg->deleteThis();
      return;
    }
...
bool CSoundPicker::IsGameSoundVisible(int hGameSound) {
  ...
  return (!m_GameSoundFilter.Length() ||
          Q_stristr(pSoundName, m_GameSoundFilter.Get()));
}

//-----------------------------------------------------------------------------
// Updates the column header in the chooser
//-----------------------------------------------------------------------------
void CSoundPicker::UpdateGameSoundColumnHeader(int nMatchCount,
                                               int nTotalCount) {
  char pColumnTitle[512];
  Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)", "Game Sound",
             nMatchCount, nTotalCount);
...
void CSoundPicker::RefreshGameSoundList() {
  ...
  int nTotalCount = 0;
  for (int nItemID = m_pGameSoundList->FirstItem();
       nItemID != m_pGameSoundList->InvalidItemID();
       nItemID = m_pGameSoundList->NextItem(nItemID)) {
    KeyValues *kv = m_pGameSoundList->GetItem(nItemID);
    int hGameSound =
        kv->GetInt("gameSoundHandle", SoundEmitterSystem()->InvalidIndex());
    if (hGameSound == SoundEmitterSystem()->InvalidIndex()) continue;
  ...
  if ((m_pGameSoundList->GetSelectedItemsCount() == 0) &&
      (m_pGameSoundList->GetItemCount() > 0)) {
    int nItemID = m_pGameSoundList->GetItemIDFromRow(0);
    // This prevents the refreshing of the sound list from playing the sound
    ++m_nSoundSuppressionCount;
    m_pGameSoundList->SetSelectedCell(nItemID, 0);
...
void CSoundPicker::PlayGameSound(const char *pSoundName) {
  ...
  if (SoundEmitterSystem()->GetParametersForSound(pSoundName, params,
                                                  GENDER_NONE)) {
    m_nPlayingSound = EngineTool()->StartSound(
        0, true, -1, CHAN_STATIC, params.soundname, params.volume,
        params.soundlevel, vec3_origin, vec3_origin, 0, params.pitch, false,
        params.delay_msec / 1000.0f);
  }
}
...
void CSoundPicker::PlayWavSound(const char *pSoundName) {
  StopSoundPreview();
  m_nPlayingSound = EngineTool()->StartSound(
      0, true, -1, CHAN_STATIC, pSoundName, VOL_NORM, SNDLVL_NONE, vec3_origin,
      vec3_origin, 0, PITCH_NORM, false, 0);
}
...
void CSoundPicker::OnItemSelected(KeyValues *kv) {
  ...
  if (m_pGameSoundList && (pPanel == m_pGameSoundList)) {
    ...
    if (pGameSoundName && bPlaySounds) {
      usize len = strlen(pGameSoundName) + 2;
      char *soundname = stack_alloc<char>(len);
      soundname[0] = '#';  // mark sound to bypass the dsp
      strcpy_s(soundname + 1, len - 1, pGameSoundName);
...
//
//-----------------------------------------------------------------------------
CSoundPickerFrame::CSoundPickerFrame(vgui::Panel *pParent, const char *pTitle,
                                     int nFlags)
    : BaseClass(pParent) {
...
//-----------------------------------------------------------------------------
void CSoundPickerFrame::DoModal(CSoundPicker::PickType_t initialType,
...
void CSoundPickerFrame::OnCommand(const char *pCommand) {
  ...
  if (!Q_stricmp(pCommand, "Open")) {
    ...
    if ((type == CSoundPicker::PICK_GAMESOUNDS) ||
        (type == CSoundPicker::PICK_WAVFILES)) {
      const char *pSoundName = pPicker->GetSelectedSoundName();

      usize len = strlen(pSoundName);
      char *soundname = stack_alloc<char>(len + 2);
      soundname[0] = '#';  // mark sound to bypass the dsp
