...
IMPLEMENT_DMEPANEL_FACTORY(CMDLPanel, DmeMDLMakefile,
                           "DmeMakeFileOutputPreview",
                           "MDL MakeFile Output Preview", false);

//-----------------------------------------------------------------------------
// Constructor, destructor
//-----------------------------------------------------------------------------
CMDLPanel::CMDLPanel(vgui::Panel *pParent, const char *pName)
  ...
  ITexture *pCubemapTexture =
      vgui::MaterialSystem()->FindTexture("editor/cubemap", NULL, true);
  m_DefaultEnvCubemap.Init(pCubemapTexture);
  pCubemapTexture =
      vgui::MaterialSystem()->FindTexture("editor/cubemap.hdr", NULL, true);
...
void CMDLPanel::SetGroundGrid(bool bVisible) { m_bGroundGrid = bVisible; }

void CMDLPanel::SetWireFrame(bool bVisible) { m_bWireFrame = bVisible; }

void CMDLPanel::SetLockView(bool bLocked) { m_bLockView = bLocked; }
...
void CMDLPanel::DrawCollisionModel() {
  ...
  if (!pCollide || pCollide->solidCount <= 0) return;

  static color32 color = {255, 0, 0, 0};

  IVPhysicsKeyParser *pParser =
      g_pPhysicsCollision->VPhysicsKeyParserCreate(pCollide->pKeyValues);
  CStudioHdr studioHdr(g_pMDLCache->GetStudioHdr(m_hMDL->GetMDL()),
                       g_pMDLCache);
  ...
  // PERFORMANCE: Just parse the script each frame.  It's fast enough for tools.
  // If you need this to go faster then cache off the bone index mapping in an
  // array like HLMV does
  while (!pParser->Finished()) {
    ...
    if (!_stricmp(pBlock, "solid")) {
      ...
      Vector *outVerts;
      int vertCount = g_pPhysicsCollision->CreateDebugMesh(
          pCollide->solids[solid.index], &outVerts);
      ...
      if (vertCount) {
        ...
        // this code wouldn't work unless you draw the model this frame before
        // calling this routine.  CMDLPanel always does this, but it's worth
        // noting. A better solution would be to move the ragdoll visulization
        // into the CDmeMdl and either draw it there or make it queryable and
        // query/draw here.
        matrix3x4_t xform;
        ...
        IMesh *pMesh =
            pRenderContext->GetDynamicMesh(true, NULL, NULL, m_Wireframe);
