...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CMDLSequencePicker::CMDLSequencePicker(vgui::Panel *pParent)
    : BaseClass(pParent, "MDLSequencePicker"), m_Images(false) {
  ...
  m_pFileTree = new CGameFileTreeView(pSplitterLeftSide, "FileTree",
                                      "All .MDLs", "models", "mdl");
  ...
  m_pSequenceSplitter = new Splitter(pSplitterRightSide, "SequenceSplitter",
  ...
  m_pSequencesList->SetAutoResize(Panel::PIN_TOPLEFT,
  ...
  m_pActivitiesList->SetAutoResize(Panel::PIN_TOPLEFT,
                                   Panel::AUTORESIZE_DOWNANDRIGHT, 0, 0, 0, 0);
...
//-----------------------------------------------------------------------------
// Purpose: This is a bit of a hack to make sure that the ToolWindow containing
// this picker punches a hold for the rendering viewport, too Input : -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void CMDLSequencePicker::OnClose() { BaseClass::OnClose(); }
...
//-----------------------------------------------------------------------------
void CMDLSequencePicker::RefreshFileList() { m_pFileTree->RefreshFileList(); }
...
//-----------------------------------------------------------------------------
int SelectWeightedSequence( studiohdr_t *pstudiohdr, int activity, int
curSequence )
{
        if (! pstudiohdr)
                return 0;
        ...
        for (int i = 0; i < pstudiohdr->GetNumSeq(); i++)
        {
                ...
                if (curActivity == activity)
                {
                        ...
                        int randomValue;
                        if ( IsInPrediction() )
                                randomValue = SharedRandomInt(
"SelectWeightedSequence", 0, weighttotal - 1, i ); else randomValue = RandomInt(
0, weighttotal - 1 );
...
void CMDLSequencePicker::SelectMDL(const char *pMDLName) {
  m_hSelectedMDL =
      pMDLName ? vgui::MDLCache()->FindMDL(pMDLName) : MDLHANDLE_INVALID;
...
void CMDLSequencePicker::OnFileSelected() {
  ...
  if (pkv->GetInt("dir") || pkv->GetInt("root")) return;
  ...
  g_pFullFileSystem->FullPathToRelativePath(pFullPathName, pRelativePathName,
                                            sizeof(pRelativePathName));
...
//-----------------------------------------------------------------------------
CMDLSequencePickerFrame::CMDLSequencePickerFrame(vgui::Panel *parent,
                                                 char const *title)
...
void CMDLSequencePickerFrame::OnTick() {
  ...
  bool bHasSequence =
      m_pMDLSequencePicker->GetSequenceNumber() != -1 ? true : false;
...
void CMDLSequencePickerFrame::OnCancel() {
  ...
  pActionKeys->SetString("SequenceName",
                         m_pMDLSequencePicker->GetSequenceName());
  pActionKeys->SetInt("SequenceNumber",
                      m_pMDLSequencePicker->GetSequenceNumber());
...
void CMDLSequencePickerFrame::OnOK() { CloseModal(); }
