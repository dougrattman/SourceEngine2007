...
CFloatAttributeManipulator::CFloatAttributeManipulator(CDmAttribute *pAttr)
    : m_pAttr(pAttr), m_startingValue(0.0f), m_lastx(-1), m_lasty(-1) {
  if (m_pAttr) {
...
void CFloatAttributeManipulator::OnMousePressed(vgui::MouseCode code, int x,
                                                int y) {
  m_lastx = x;
  m_lasty = y;
}

void CFloatAttributeManipulator::OnMouseReleased(vgui::MouseCode code, int x,
                                                 int y) {
  m_lastx = x;
  m_lasty = y;
}
...
CTranslationManip::CTranslationManip(CDmeTransform *transform,
                                     CDmeCamera *camera)
    : CTransformManipulator(transform),
      m_pCamera(camera),
      m_lastx(-1),
      m_lasty(-1),
      m_offset(vec3_origin),
      m_prevOffset(vec3_origin) {}
...
void CTranslationManip::OnCursorMoved(int x, int y) {
  ...
  VMatrix viewprojinv;
  m_pCamera->GetViewProjectionInverse(viewprojinv, m_nViewport[0],
                                      m_nViewport[1]);
