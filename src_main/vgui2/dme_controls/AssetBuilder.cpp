...
//-----------------------------------------------------------------------------
CCompileStatusBar::CCompileStatusBar(vgui::Panel *pParent,
  ...
  vgui::surface()->DrawSetTextureFile(m_CompilingId, "vgui/progressbar", true,
                                      false);
}
...
// Sets compile status
//-----------------------------------------------------------------------------
void CCompileStatusBar::SetStatus(CompileStatus_t status,
                                  const char *pMessage) {
  m_Status = status;
...
//
//-----------------------------------------------------------------------------
IMPLEMENT_DMEPANEL_FACTORY(CAssetBuilder, DmeMakefile, "DmeMakeFileDefault",
                           "MakeFile Editor", true);
...
void BuildAssetTypeList() {
  if (s_bAssetTypeListBuilt) return;
  ...
  while (g_pDataModel->IsValidFactory(hFactory)) {
    ...
    CDmElement *pElement = GetElement<CDmElement>(
        g_pDataModel->CreateElement(pFactoryName, "temp"));
    ...
    if (pMakeFile && pMakeFile->GetMakefileType()) {
      ...
      s_AssetTypes[i].m_pChoiceString =
          pMakeFile->GetMakefileType()->m_pHumanReadableName;
...
// Builds the list of asset types
//-----------------------------------------------------------------------------
static PickerList_t &BuildAssetSubTypeList(const char **ppSubTypes,
                                           PickerList_t &pickerList) {
  if (!ppSubTypes) return s_AssetTypes;
  ...
  for (int i = 0; i < nCount; ++i) {
    // Add all DmeElements that inherit from DmeMakefile
    CDmElement *pElement = GetElement<CDmElement>(
        g_pDataModel->CreateElement(s_AssetTypes[i].m_pChoiceValue, "temp"));
    CDmeMakefile *pMakeFile = CastElement<CDmeMakefile>(pElement);

    for (int j = 0; ppSubTypes[j]; ++j) {
      if (!pElement->IsA(ppSubTypes[j])) continue;

      int k = pickerList.AddToTail();
      pickerList[k].m_pChoiceString =
          pMakeFile->GetMakefileType()->m_pHumanReadableName;
...
//-----------------------------------------------------------------------------
static void OverwriteFileDialog(vgui::Panel *pActionTarget,
                                const char *pFileName, KeyValues *pOkCommand) {
  ...
  vgui::MessageBox *pMessageBox =
      new vgui::MessageBox("Overwrite Existing File?", pBuf, pActionTarget);
...
//-----------------------------------------------------------------------------
static CDmeMakefile *ReadMakefile(const char *pFileName,
                                  CDmElement **ppRoot = NULL) {
  if (ppRoot) {
    *ppRoot = NULL;
  }

  CDmElement *pRoot;
  DmFileId_t fileid = g_pDataModel->RestoreFromFile(pFileName, NULL, NULL,
                                                    &pRoot, CR_DELETE_OLD);
...
// Sort by MDL name
//-----------------------------------------------------------------------------
static int __cdecl TypeSortFunc(vgui::ListPanel *pPanel,
...
static int __cdecl FileSortFunc(vgui::ListPanel *pPanel,
                                const ListPanelItem &item1,
                                const ListPanelItem &item2) {
...
CAssetBuilder::CAssetBuilder(vgui::Panel *pParent, const char *pPanelName)
  ...
  m_pOutputPreviewPage =
      new PropertyPage(m_pInputOutputSheet, "OutputPreviewPage");

  m_pPropertiesSplitter = new vgui::Splitter(m_pInputPage, "PropertiesSplitter",
                                             SPLITTER_MODE_VERTICAL, 1);
  ...
  m_pOututPreviewPanel =
      new CDmePanel(m_pOutputPreviewPage, "OutputPreview", false);
  ...
  m_pCompile =
      new vgui::Button(this, "CompileButton", "Compile", this, "OnCompile");
  m_pPublish =
      new vgui::Button(this, "PublishButton", "Publish", this, "OnPublish");
  m_pAbortCompile = new vgui::Button(this, "AbortCompileButton", "AbortCompile",
                                     this, "OnAbortCompile");
  m_pCompileStatusBar = new CCompileStatusBar(this, "CompileStatus");

  m_pInputPage->LoadControlSettingsAndUserConfig(
      "resource/assetbuilderinputpage.res");
  m_pOutputPage->LoadControlSettingsAndUserConfig(
      "resource/assetbuilderoutputpage.res");
  m_pCompilePage->LoadControlSettingsAndUserConfig(
      "resource/assetbuildercompilepage.res");
  m_pOutputPreviewPage->LoadControlSettingsAndUserConfig(
      "resource/assetbuilderoutputpreviewpage.res");
...
//-----------------------------------------------------------------------------
void CAssetBuilder::BuildFileIDList(CDmeMakefile *pMakeFile,
...
void CAssetBuilder::CleanupMakefile() {
  ...
  int nCount = fileIds.Count();
  for (int i = 0; i < nCount; ++i) {
    if (fileIds[i] != DMFILEID_INVALID &&
        g_pDataModel->GetFileName(fileIds[i])[0]) {
...
//-----------------------------------------------------------------------------
CDmeMakefile *CAssetBuilder::GetMakeFile() { return m_hMakefile.Get(); }

CDmeMakefile *CAssetBuilder::GetRootMakeFile() { return m_hRootMakefile.Get(); }
...
void CAssetBuilder::SetCurrentMakefile(CDmeMakefile *pMakeFile) {
  ...
  m_pDmePanel->SetDmeElement(NULL);
  m_pOututPreviewPanel->SetDmeElement(pMakeFile, true,
                                      "DmeMakeFileOutputPreview");
  RefreshSourceList();
  RefreshOutputList();
...
void CAssetBuilder::RefreshSourceList() {
  ...
  for (int i = 0; pSourceTypes[i].m_pTypeName; ++i) {
    ...
    for (int j = 0; j < nCount; ++j) {
      ...
      KeyValues *pItemKeys =
          new KeyValues("node", "type", pSourceTypes[i].m_pHumanReadableName);
...
void CAssetBuilder::SelectSource(CDmeSource *pSource) {
  ...
  for (; nItemID != m_pSourcesList->InvalidItemID();
       nItemID = m_pSourcesList->NextItem(nItemID)) {
    KeyValues *kv = m_pSourcesList->GetItem(nItemID);
    if (GetElementKeyValue<CDmeSource>(kv, "dmeSource") != pSource) continue;
...
void CAssetBuilder::OnPicked(KeyValues *kv) {
  ...
  CDmeMakefile *pMakeFile =
      GetElement<CDmeMakefile>(g_pDataModel->CreateElement(pValue, "unnamed"));
  if (!pMakeFile) return;
  ...
  KeyValues *pDialogKeys =
      new KeyValues("NewSourceFileSelected", "makefileType", pValue);
  FileOpenDialog *pDialog = new FileOpenDialog(
      this, "Select Asset Builder File Name", false, pDialogKeys);
...
void CAssetBuilder::OnNewSourceFile() {
  ...
  PickerList_t &pickerList =
      BuildAssetSubTypeList(pSource->GetSourceMakefileTypes(), typePickerList);

  // Create a list indicating which type of asset to create
  CPickerFrame *pPicker = new CPickerFrame(this, "Select Sub-Asset Type",
...
void CAssetBuilder::OnAddSource() {
  ...
  for (int i = 0; pSourceTypes[i].m_pTypeName; ++i) {
    if (pSourceTypes[i].m_bIsSingleton) {
      if (m_hMakefile->HasSourceOfType(pSourceTypes[i].m_pTypeName)) continue;
    }
  ...
  CPickerFrame *pPicker =
      new CPickerFrame(this, "Select Source Type", "Source Type", "sourceType");
...
void CAssetBuilder::SelectSourceListRow(int nRow) {
  ...
  if (nVisibleRowCount == 0 || nRow < 0) return;
...
void CAssetBuilder::OnRemoveSource() {
  ...
  if (nCount == 0 || !m_hMakefile.Get()) return;
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CDmeSource *pSource =
        GetElementKeyValue<CDmeSource>(pKeyValues, "dmeSource");
    if (pSource) {
...
void CAssetBuilder::OnZoomOutSource() {
  // Called to zoom into the currently selected source
  if (m_hMakefileStack.Count() <= 1) return;
...
//-----------------------------------------------------------------------------
void CAssetBuilder::OnSourceFileAdded(const char *pFileName,
...
//-----------------------------------------------------------------------------
void CAssetBuilder::OnNewSourceFileSelected(const char *pFileName,
                                            KeyValues *kv) {
  int nCount = m_pSourcesList->GetSelectedItemsCount();
  if (nCount != 1 || !m_hMakefile.Get()) return;
...

  {
    ...
    CDmeMakefile *pSourceMakeFile =
        CreateElement<CDmeMakefile>(pMakeFileType, pFileName, fileid);
...
void CAssetBuilder::ShowSourceFileBrowser(const char *pTitle,
  ...
  pDialog->SetStartDirectoryContext(pContext, pStartingDir);
  pDialog->AddFilter(pSourceType->m_pFileFilter,
                     pSourceType->m_pFileFilterString, true);
  pDialog->SetDeleteSelfOnClose(true);
  pDialog->AddActionSignalTarget(this);
  pDialog->DoModal(false);
}
...
void CAssetBuilder::OnBrowseSourceFile() {
  ...
  DmeMakefileType_t &sourceType =
      m_hMakefile->GetSourceTypes()[nSourceTypeIndex];
...
void CAssetBuilder::OnOpenContextMenu(KeyValues *kv) {
  ...
  if (pPanel != m_pSourcesList) return;
  ...
  if (bShowZoomIn || bShowZoomOut) {
    ...
    if (bShowZoomIn) {
      m_hContextMenu->AddMenuItem("Zoom In", new KeyValues("ZoomInSource"),
                                  this);
    }
    if (bShowZoomOut) {
      m_hContextMenu->AddMenuItem("Zoom Out", new KeyValues("ZoomOutSource"),
                                  this);
    }
  }
  ...
  if (nCount == 1) {
    m_hContextMenu->AddSeparator();
    m_hContextMenu->AddMenuItem("New Source File...",
                                new KeyValues("NewSourceFile"), this);
    m_hContextMenu->AddMenuItem("Select Source File...",
                                new KeyValues("BrowseSourceFile"), this);
    if (bShowLoadSourceFile) {
      m_hContextMenu->AddMenuItem("Load Source File",
                                  new KeyValues("LoadSourceFile"), this);
    }
    if (bHasValidSourceFile) {
      m_hContextMenu->AddMenuItem("Edit Source File",
                                  new KeyValues("EditSourceFile"), this);
    }
  }
...
CDmeSource *CAssetBuilder::GetSelectedSource() {
  int nCount = m_pSourcesList->GetSelectedItemsCount();
  if (nCount != 1 || !m_hMakefile.Get()) return NULL;
...
void CAssetBuilder::FinishCompilation(CompilationState_t state) {
  ...
  m_pOututPreviewPanel->SetDmeElement(m_hMakefile, true,
                                      "DmeMakeFileOutputPreview");
  m_bIsCompiling = false;

  // NOTE: Sort of side-effecty. These two things must be done after
  // m_pOututPreviewPanel->SetDmeElement, since that's what reloads the output
  // element, which is also what can cause a reload of the makefile
  RefreshSourceList();
  SelectSourceListRow(nRow);

  // Lets the asset builder update the title bar
  // (compilation could have changed the dirty state if the makefile is in the
  // file)
  PostActionSignal(new KeyValues("UpdateFileName"));

  if (state == COMPILATION_FAILED) {
    char pBuf[256];
    Q_snprintf(pBuf, sizeof(pBuf), "Compilation Error (return code %d)",
               g_pDmeMakefileUtils->GetExitCode());
    m_pCompileStatusBar->SetStatus(CCompileStatusBar::COMPILATION_FAILED, pBuf);
  } else {
    m_pCompileStatusBar->SetStatus(CCompileStatusBar::COMPILATION_SUCCESSFUL,
                                   "Compile Successful!");
  }
}
...
void CAssetBuilder::OnTick() {
  ...
  if (m_bIsCompiling) {
    ...
    CompilationState_t state =
        g_pDmeMakefileUtils->UpdateCompilation(pBuf, nLen);
    if (nLen > 0) {
...
void CAssetBuilder::OnAbortCompile() {
  if (m_bIsCompiling) {
    ...
    m_bIsCompiling = false;
    m_pAbortCompile->SetEnabled(false);
    m_pCompileStatusBar->SetStatus(CCompileStatusBar::COMPILATION_FAILED,
                                   "Compile Aborted");
  }
}
...
void CAssetBuilder::OnCompile() {
  ...
  m_pCompileStatusBar->SetStatus(CCompileStatusBar::CURRENTLY_COMPILING,
                                 "Compiling...");
...
CAssetBuilderFrame::CAssetBuilderFrame(vgui::Panel *pParent, const char *pTitle)
  ...
  pMenu->AddMenuItem("new", "#AssetBuilder_FileNew", new KeyValues("FileNew"),
                     this);
  pMenu->AddMenuItem("open", "#AssetBuilder_FileOpen",
                     new KeyValues("FileOpen"), this);
  pMenu->AddMenuItem("save", "#AssetBuilder_FileSave",
                     new KeyValues("FileSave"), this);
  pMenu->AddMenuItem("saveas", "#AssetBuilder_FileSaveAs",
                     new KeyValues("FileSaveAs"), this);
  SetSysMenu(pMenu);
...
//-----------------------------------------------------------------------------
void CAssetBuilderFrame::SetupFileOpenDialog(vgui::FileOpenDialog *pDialog,
                                             bool bOpenFile,
...
bool CAssetBuilderFrame::OnReadFileFromDisk(const char *pFileName,
                                            const char *pFileFormat,
                                            KeyValues *pContextKeyValues) {
  CDmElement *pRoot;
  CDmeMakefile *pMakeFile = ReadMakefile(pFileName, &pRoot);
  if (!pMakeFile) return false;

  Reset(pMakeFile);
  return true;
}

bool CAssetBuilderFrame::OnWriteFileToDisk(const char *pFileName,
                                           const char *pFileFormat,
                                           KeyValues *pContextKeyValues) {
  ...
  if (!bOk) {
    vgui::MessageBox *pError =
        new vgui::MessageBox("#AssetBuilder_CannotRenameSourceFiles",
  ...
  CDmElement *pRoot =
      GetElement<CDmElement>(g_pDataModel->GetFileRoot(pMakefile->GetFileId()));
  if (!pRoot) {
    pRoot = pMakefile;
  }
  bOk = g_pDataModel->SaveToFile(pFileName, NULL,
                                 g_pDataModel->GetDefaultEncoding(pFileFormat),
                                 pFileFormat, pRoot);
...
void CAssetBuilderFrame::UpdateFileName() {
  ...
  const char *pFileName = (fileId != DMFILEID_INVALID)
                              ? g_pDataModel->GetFileName(fileId)
                              : "<unnamed>";
...
//-----------------------------------------------------------------------------
void CAssetBuilderFrame::OnDmeElementChanged() { SetDirty(true); }
...
void CAssetBuilderFrame::OnFileSelected(KeyValues *kv) {
  ...
  if (pDialogKeys) {
    KeyValues *pOkCommand =
        new KeyValues("PerformFileNew", "makefileType",
                      pDialogKeys->GetString("makefileType"));
...
void CAssetBuilderFrame::OnPicked(KeyValues *kv) {
  ...
  FileOpenDialog *pDialog =
      new FileOpenDialog(this, pTitle, false, pDialogKeys);
  pDialog->SetStartDirectoryContext(pContext, pStartingDir);
  pDialog->AddFilter(pType->m_pFileFilter, pType->m_pFileFilterString, true);
  pDialog->SetDeleteSelfOnClose(true);
  pDialog->AddActionSignalTarget(this);
...
void CAssetBuilderFrame::OnFileStateMachineFinished(KeyValues *pKeyValues) {
  ...
  if (pNewFile) {
    ...
    if (pKeyValues->GetInt("completionState",
                           FileOpenStateMachine::IN_PROGRESS) ==
        FileOpenStateMachine::SUCCESSFUL) {
      ShowNewAssetPicker();
    }
    return;
  }
...
void CAssetBuilderFrame::ShowNewAssetPicker() {
  ...
  // Create a list indicating which type of asset to create
  CPickerFrame *pPicker =
      new CPickerFrame(this, "Select Asset Type", "Asset Type", "assetType");
...
void CAssetBuilderFrame::OnFileNew() {
  ...
  if (pMakeFile && pMakeFile->IsDirty()) {
    ...
    m_pFileOpenStateMachine->SaveFile(
        pContextKeyValues, pFileName, ASSET_FILE_FORMAT,
...
void CAssetBuilderFrame::OnFileOpen() {
  ...
  m_pFileOpenStateMachine->OpenFile(ASSET_FILE_FORMAT, pContextKeyValues,
                                    pFileName, NULL, nFlags);
}
...
void CAssetBuilderFrame::OnFileSave() {
  ...
  const char *pFileName = g_pDataModel->GetFileName(pMakeFile->GetFileId());
  m_pFileOpenStateMachine->SaveFile(pContextKeyValues, pFileName,
                                    ASSET_FILE_FORMAT,
                                    FOSM_SHOW_PERFORCE_DIALOGS);
}
...
void CAssetBuilderFrame::OnFileSaveAs() {
  ...
  KeyValues *pContextKeyValues = new KeyValues("FileSave");
  m_pFileOpenStateMachine->SaveFile(pContextKeyValues, NULL, ASSET_FILE_FORMAT,
                                    FOSM_SHOW_PERFORCE_DIALOGS);
}
