...
CMDLPicker::CMDLPicker(vgui::Panel *pParent, int nFlags)
  ...
  m_pFileBrowserSplitter =
      new Splitter(this, "FileBrowserSplitter", SPLITTER_MODE_VERTICAL, 1);
  ...
  m_pPreviewSplitter = new Splitter(pSplitterRightSide, "PreviewSplitter",
  ...
  if (nFlags & PAGE_RENDER) {
    ...
    m_pRenderPage->LoadControlSettingsAndUserConfig(
        "resource/mdlpickerrender.res");

    RefreshRenderSettings();

    // ground
    Button *pSelectProbe =
        (Button *)m_pRenderPage->FindChildByName("ChooseLightProbe");
  ...
  if (nFlags & PAGE_SEQUENCES) {
    ...
    m_pSequencesList->SetAutoResize(
        Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6, -6, -6);
  }
  ...
  if (nFlags & PAGE_ACTIVITIES) {
    ...
    m_pActivitiesList =
        new vgui::ListPanel(m_pActivitiesPage, "ActivitiesList");
    ...
    m_pActivitiesList->SetAutoResize(
        Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6, -6, -6);
  }
  ...
  if (nFlags & PAGE_SKINS) {
    ...
    m_pSkinsList->SetAutoResize(Panel::PIN_TOPLEFT,
                                Panel::AUTORESIZE_DOWNANDRIGHT, 6, 6, -6, -6);
  }
  ...
  if (nFlags & PAGE_INFO) {
    ...
    CheckButton *pTempCheck =
        (CheckButton *)m_pInfoPage->FindChildByName("PhysicsObject");
    ...
    m_pPropDataList->AddColumnHeader(0, "key", "key", 250,
    ...
    m_pPropDataList->SetAutoResize(
        Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT, 6, 72, -6, -6);
...
void CMDLPicker::OnAssetSelected(KeyValues *pParams) {
  ...
  if (!pLightProbe) {
    ...
    vgui::MessageBox *pMessageBox =
        new vgui::MessageBox("Error Loading File!\n", pBuf, GetParent());
    pMessageBox->DoModal();
    return;
  }
...
void CMDLPicker::OnCommand(const char *pCommand) {
  if (!Q_stricmp(pCommand, "ChooseLightProbe")) {
    CAssetPickerFrame *pPicker = new CAssetPickerFrame(
        this, "Select Light Probe (.prb) File", "Light Probe", "prb",
        "materials/lightprobes", "lightprobe");
    pPicker->DoModal();
    return;
  }
...
void CMDLPicker::RefreshActivitiesAndSequencesList() {
  ...
  for (int j = 0; j < hdr->GetNumSeq(); j++) {
    if (/*g_viewerSettings.showHidden ||*/ !(hdr->pSeqdesc(j).flags &
                                             STUDIO_HIDDEN)) {
      const char *pActivityName = hdr->pSeqdesc(j).pszActivityName();
      if (pActivityName && pActivityName[0]) {
        // Multiple sequences can have the same activity name; only add unique
        // activity names
        if (activityNames.Find(pActivityName) == activityNames.InvalidIndex()) {
          ...
          pDrag->SetString("mdl",
                           vgui::MDLCache()->GetModelName(m_hSelectedMDL));
...
void CMDLPicker::SelectMDL(const char *pRelativePath) {
  MDLHandle_t hSelectedMDL = pRelativePath
                                 ? vgui::MDLCache()->FindMDL(pRelativePath)
                                 : MDLHANDLE_INVALID;

  // We didn't change models after all...
  if (hSelectedMDL == m_hSelectedMDL) {
    // vgui::MDLCache()->FindMDL adds a reference by default we don't use,
    // release it again
  ...
  // vgui::MDLCache()->FindMDL adds a reference by default we don't use, release
  // it again
  if (hSelectedMDL != MDLHANDLE_INVALID) {
    vgui::MDLCache()->Release(hSelectedMDL);
  }

  PostActionSignal(new KeyValues("MDLPreviewChanged", "mdl",
...
int CMDLPicker::GetSelectedPage() {
  ...
  if (m_pActivitiesPage &&
      (m_pViewsSheet->GetActivePage() == m_pActivitiesPage))
...
const char *CMDLPicker::GetSelectedSequenceName() {
  if (!m_pSequencesPage) return NULL;
...
const char *CMDLPicker::GetSelectedActivityName() {
  if (!m_pActivitiesPage) return NULL;
...
void CMDLPicker::SelectActivity(const char *pActivityName) {
  ...
  for (int i = 0; i < pstudiohdr->GetNumSeq(); i++) {
    ...
    if (_stricmp(seqdesc.pszActivityName(), pActivityName) == 0) {
  ...
  PostActionSignal(
      new KeyValues("SequenceSelectionChanged", "activity", pActivityName));
}
...
void CMDLPicker::SelectSequence(const char *pSequenceName) {
  ...
  PostActionSignal(
      new KeyValues("SequenceSelectionChanged", "sequence", pSequenceName));
}
...
void CMDLPicker::OnPageChanged() {
  if (m_pSequencesPage &&
      (m_pViewsSheet->GetActivePage() == m_pSequencesPage)) {
...
//
//-----------------------------------------------------------------------------
CMDLPickerFrame::CMDLPickerFrame(vgui::Panel *pParent, const char *pTitle,
                                 int nFlags)
    : BaseClass(pParent) {
...
void CMDLPicker::UpdateInfoTab() {
  ...
  CheckButton *pTempCheck =
      (CheckButton *)m_pInfoPage->FindChildByName("StaticObject");
...
int CMDLPicker::UpdatePropDataList(const char *pszPropData, bool &bIsStatic) {
  ...
  if (m_pPropDataList) {
    ...
    while (endChunk) {
      ...
      KeyValues *pkv =
          new KeyValues("node", "key", keyText, "value", valueText);
