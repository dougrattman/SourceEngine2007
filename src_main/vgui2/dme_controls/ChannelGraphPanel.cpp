...
// input methods
//-----------------------------------------------------------------------------
void CChannelGraphPanel::OnSizeChanged(
    int newWide,
...
void CChannelGraphPanel::OnCursorMoved(int mx, int my) {
  ...
  if (!vgui::input()->IsMouseDown(MOUSE_LEFT)) return;
  ...
  if (bInTimeLegend) {
    if (abs(dy) > abs(dx)) {
      ...
      m_flTimeToPixel = (GetWide() - m_nGraphOriginX) /
                        (m_graphMaxTime - m_graphMinTime).GetSeconds();
    ...
    if (abs(dx) > abs(dy)) {
      ...
      m_flValueToPixel =
          (GetTall() - m_nGraphOriginY) / (m_graphMaxValue - m_graphMinValue);
...
float GetDisplayIncrement(int windowpixels, int fontpixels, float valuerange,
                          int *pDecimalPlaces = NULL) {
...
int CChannelGraphPanel::TimeToPixel(DmeTime_t time) {
  return m_nGraphOriginX +
         (int)floor(m_flTimeToPixel * (time - m_graphMinTime).GetSeconds() +
                    0.5f);
}

int CChannelGraphPanel::ValueToPixel(float flValue) {
  return m_nGraphOriginY +
         (int)floor(m_flValueToPixel * (flValue - m_graphMinValue) + 0.5f);
}
...
void CChannelGraphPanel::Paint() {
  ...
  surface()->DrawLine(m_nGraphOriginX, GetTall() - m_nGraphOriginY - 1,
                      GetWide(), GetTall() - m_nGraphOriginY - 1);
  surface()->DrawLine(m_nGraphOriginX, GetTall() - m_nGraphOriginY - 1,
                      m_nGraphOriginX, 0);
  ...
  float flValueIncrement =
      GetDisplayIncrement(tall, (int)(1.5f * textheight),
                          m_graphMaxValue - m_graphMinValue, &nDecimalPlaces);
  ...
  for (int i = nMinValueIndex; i <= nMaxValueIndex;
       ++i, flValue += flValueIncrement) {
    wchar_t pFormat[32];
    _snwprintf_s(pFormat, sizeof(pFormat), L"%%.%df", nDecimalPlaces);

    wchar_t wstring[32];
    _snwprintf_s(wstring, sizeof(wstring), pFormat, flValue);

    int tw = 0, th = 0;
    surface()->GetTextSize(m_font, wstring, tw, th);

    int y = GetTall() - ValueToPixel(flValue) - 1;
    surface()->DrawSetTextPos(m_nGraphOriginX - m_nTextBorder - tw,
                              y - textheight / 2);
  ...
  float flTimeIncrement = GetDisplayIncrement(
      wide, textwidth, (m_graphMaxTime - m_graphMinTime).GetSeconds(),
      &nDecimalPlaces);
  ...
  for (int i = nMinTimeIndex; i <= nMaxTimeIndex;
    ...
    surface()->DrawSetTextPos(x - tw / 2,
                              GetTall() - m_nGraphOriginY + m_nTextBorder - 1);
    surface()->DrawPrintText(wstring, wcslen(wstring));

    surface()->DrawLine(x, GetTall() - m_nGraphOriginY + m_nTextBorder - 1, x,
                        GetTall() - m_nGraphOriginY - 1);
  }
...
CChannelGraphFrame::CChannelGraphFrame(Panel *parent, const char *pTitle)
  ...
  SetScheme(vgui::scheme()->LoadSchemeFromFile("Resource/BoxRocket.res",
                                               "BoxRocket"));
}
...
void CChannelGraphFrame::PerformLayout() {
  ...
  m_pChannelGraph->SetSize(iWidth - 2 * border,
                           iHeight - GetCaptionHeight() - 2 * border);
}
