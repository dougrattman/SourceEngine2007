...
// Sort by MDL name
//-----------------------------------------------------------------------------
static int __cdecl DmeBrowserSortFunc(vgui::ListPanel *pPanel,
...
void CDmePicker::Activate(const CUtlVector<DmePickerInfo_t> &vec) {
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    const char *pElementName =
        pElement ? pElement->GetName() : "<null element>";
    const char *pItemName =
        vec[i].m_pChoiceString ? vec[i].m_pChoiceString : pElementName;
...
void CDmePicker::RefreshDmeList() {
  ...
  int nTotalCount = 0;
  for (int nItemID = m_pDmeBrowser->FirstItem();
       nItemID != m_pDmeBrowser->InvalidItemID();
       nItemID = m_pDmeBrowser->NextItem(nItemID)) {
    KeyValues *kv = m_pDmeBrowser->GetItem(nItemID);
    const char *pElementName = kv->GetString("dme");
    bool bIsVisible =
        !m_Filter.Length() || Q_stristr(pElementName, m_Filter.Get());
  ...
  Q_snprintf(pColumnTitle, sizeof(pColumnTitle), "%s (%d/%d)", "Dme Elements",
             nMatchingElements, nTotalCount);
  m_pDmeBrowser->SetColumnHeaderText(0, pColumnTitle);

  if ((m_pDmeBrowser->GetItemCount() > 0) &&
      (m_pDmeBrowser->GetSelectedItemsCount() == 0)) {
...
CDmElement *CDmePicker::GetSelectedDme() {
  if (m_pDmeBrowser->GetSelectedItemsCount() == 0) return NULL;
...
CDmePickerFrame::~CDmePickerFrame() { CleanUpMessage(); }
...
//-----------------------------------------------------------------------------
void CDmePickerFrame::DoModal(const CUtlVector<DmePickerInfo_t> &vec,
