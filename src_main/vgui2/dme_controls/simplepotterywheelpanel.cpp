...
class CTranslationManipulator : public CTransformManipulator {
 public:
  CTranslationManipulator(CDmeTransform *transform, CDmeCamera *pCamera)
...
class CRotationManipulator : public CTransformManipulator {
 public:
  CRotationManipulator(CDmeTransform *transform, CDmeCamera *pCamera)
      : CTransformManipulator(transform) {
    m_hCamera = pCamera;
  ...
  virtual void OnCursorMoved(int x, int y) {
    ...
    m_altitude =
        std::max((float)-M_PI / 2, std::min((float)M_PI / 2, m_altitude));
...
//-----------------------------------------------------------------------------
CSimplePotteryWheelPanel::CSimplePotteryWheelPanel(vgui::Panel *pParent,
                                                   const char *pName)
  ...
  m_pCameraRotate =
      new CRotationManipulator(m_pCameraRootDag->GetTransform(), m_pCamera);
  m_pCameraTranslate =
      new CTranslationManipulator(m_pCameraRootDag->GetTransform(), m_pCamera);
...
void CSimplePotteryWheelPanel::CreateDefaultLights() {
  ...
  CDmeDirectionalLight *pLight =
      CreateElement<CDmeDirectionalLight>("dir light", DMFILEID_INVALID);
...
void CSimplePotteryWheelPanel::SetLightProbe(CDmElement *pLightProbe) {
  ...
  const char *pCubemapHDR = pLightProbe->HasAttribute("cubemapHdr")
                                ? pLightProbe->GetValueString("cubemapHdr")
                                : pCubemap;
  ...
  if (ambientCube.Count() == 6) {
    for (int i = 0; i < 6; ++i) {
      m_vecAmbientCube[i].Init(ambientCube[i].x, ambientCube[i].y,
                               ambientCube[i].z, 0.0f);
    }
  }
  ...
  for (int i = 0; i < nLightCount; ++i) {
    ...
    Color c(255 * vecColor.x / flIntensity + 0.5f,
            255 * vecColor.y / flIntensity + 0.5f,
            255 * vecColor.z / flIntensity + 0.5f, 255);
    if (!Q_stricmp(pType, "directional")) {
      CDmeDirectionalLight *pLight =
          CreateElement<CDmeDirectionalLight>("light");
    ...
    if (!Q_stricmp(pType, "spot")) {
      ...
      pLight->SetAttenuation(vecAtten.x, vecAtten.y, vecAtten.z);
      pLight->SetMaxDistance(pLocalLight->GetValue<float>("maxDistance"));
      pLight->SetAngles(RAD2DEG(pLocalLight->GetValue<float>("theta")),
...
ITexture *CSimplePotteryWheelPanel::GetLightProbeCubemap(bool bHDR) {
  if (!m_bHasLightProbe) return NULL;
...
void CSimplePotteryWheelPanel::UseEngineCoordinateSystem(bool bEnable) {
  static_cast<CRotationManipulator *>(m_pCameraRotate)
      ->SetUsingEngineCoordinateSystem(bEnable);
  static_cast<CTranslationManipulator *>(m_pCameraTranslate)
      ->SetUsingEngineCoordinateSystem(bEnable);
}

bool CSimplePotteryWheelPanel::IsUsingEngineCoordinateSystem() const {
  return static_cast<CRotationManipulator *>(m_pCameraRotate)
      ->IsUsingEngineCoordinateSystem();
}
...
void CSimplePotteryWheelPanel::LookAt(float flRadius) {
  ...
  // since tan( fov/2 ) = f/d
  // cos( fov/2 ) = r / r' where r = sphere radius, r' = perp distance from
  // sphere center to max extent of camera d/f = r'/d' where d' is distance of
  // camera to sphere d' = r' / tan( fov/2 ) * r' = r / ( cos (fov/2) * tan(
  // fov/2 ) ) = r / sin( fov/2 )
...
//-----------------------------------------------------------------------------
void CSimplePotteryWheelPanel::ComputeCameraTransform(
    matrix3x4_t *pWorldToCamera) {
...
//-----------------------------------------------------------------------------
void CSimplePotteryWheelPanel::ComputePanelPosition(const Vector &vecPosition,
                                                    Vector2D *pPanelPos) {
  int w, h;
  GetSize(w, h);
...
void CSimplePotteryWheelPanel::Paint() {
...

  {
    ...
    m_pCamera->SetupRenderState(iWidth, iHeight,
                                IsUsingEngineCoordinateSystem());
  ...
  pRenderContext->ClearColor4ub(m_ClearColor.r(), m_ClearColor.g(),
  ...
  if (HasLightProbe()) {
    ...
    IMaterial *pMaterial =
        (vgui::MaterialSystemHardwareConfig()->GetHDRType() == HDR_TYPE_NONE)
            ? m_LightProbeBackground
            : m_LightProbeHDRBackground;

    RenderBox(vecCameraOrigin, vec3_angle, Vector(-100, -100, -100),
              Vector(100, 100, 100), Color(255, 255, 255, 255), pMaterial,
              true);
  }
...
void CSimplePotteryWheelPanel::EnterManipulationMode(
    ManipulationMode_t manipMode, bool bMouseCapture,
    vgui::MouseCode mouseCode /* = -1 */) {
  switch (manipMode) {
    case CAMERA_ROTATE:
...
void CSimplePotteryWheelPanel::EnableMouseCapture(
    bool enable, vgui::MouseCode mouseCode /* = -1 */) {
  if (enable) {
