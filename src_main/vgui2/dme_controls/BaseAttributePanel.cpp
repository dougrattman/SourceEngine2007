...
//-----------------------------------------------------------------------------
bool CBaseAttributePanel::ColInfoLessFunc(
    const CBaseAttributePanel::colinfo_t &lhs,
    const CBaseAttributePanel::colinfo_t &rhs) {
...
//-----------------------------------------------------------------------------
CBaseAttributePanel::CBaseAttributePanel(vgui::Panel *parent,
  ...
  Q_strncpy(m_szAttributeName, info.m_pAttributeName,
            sizeof(m_szAttributeName));

  m_pType = new Label(this, "AttributeType", "");
  SetColumnSize(m_pType, 100);

  CDmAttribute *pAttribute =
      info.m_pElement->GetAttribute(info.m_pAttributeName);
...
void CBaseAttributePanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (GetDirty()) {
    SetBgColor(pScheme->GetColor("AttributeWidget.DirtyBgColor",
                                 Color(100, 100, 200, 63)));
  } else {
...
//-----------------------------------------------------------------------------
inline CDmElement *CBaseAttributePanel::GetPanelElement() { return m_hObject; }
...
const char *CBaseAttributePanel::GetAttributeValueAsString(char *pBuf,
                                                           int nLength) {
  if (m_nArrayIndex < 0) {
...
void CBaseAttributePanel::OnCreateDragData(KeyValues *msg) {
  if (GetPanelElement()) {
    msg->SetInt("root", GetPanelElement() ? GetPanelElement()->GetHandle()
                                          : DMELEMENT_HANDLE_INVALID);
    msg->SetString("type",
                   g_pDataModel->GetAttributeNameForType(m_AttributeType));
