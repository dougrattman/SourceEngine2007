...
static int ListFileNameSortFunc([[maybe_unused]] ListPanel *pPanel,
  ...
  // if they're both not directories of files, return if dir1 is a directory
  // (before files)
  if (dir1 != dir2) {
    return dir1 ? -1 : 1;
  }
  ...
  // YWB:  Mimic windows behavior where filenames starting with numbers are
  // sorted based on numeric part
...
static int ListBaseStringSortFunc(ListPanel *pPanel, const ListPanelItem &item1,
                                  const ListPanelItem &item2,
...
static int ListBaseIntegerSortFunc(ListPanel *pPanel,
                                   const ListPanelItem &item1,
                                   const ListPanelItem &item2,
                                   char const *fieldName) {
  bool dir1 = item1.kv->GetInt("directory") == 1;
...
static int ListBaseInteger64SortFunc(ListPanel *pPanel,
                                     const ListPanelItem &item1,
                                     const ListPanelItem &item2,
                                     char const *lowfield,
                                     char const *highfield) {
  bool dir1 = item1.kv->GetInt("directory") == 1;
  bool dir2 = item2.kv->GetInt("directory") == 1;
...
static int ListFileSizeSortFunc(ListPanel *pPanel, const ListPanelItem &item1,
                                const ListPanelItem &item2) {
  return ListBaseIntegerSortFunc(pPanel, item1, item2, "filesizeint");
}

static int ListFileModifiedSortFunc(ListPanel *pPanel,
                                    const ListPanelItem &item1,
                                    const ListPanelItem &item2) {
  // NOTE: Backward order to get most recent files first
  return ListBaseInteger64SortFunc(pPanel, item2, item1, "modifiedint_low",
                                   "modifiedint_high");
}
static int ListFileCreatedSortFunc(ListPanel *pPanel,
                                   const ListPanelItem &item1,
                                   const ListPanelItem &item2) {
  // NOTE: Backward order to get most recent files first
  return ListBaseInteger64SortFunc(pPanel, item2, item1, "createdint_low",
                                   "createdint_high");
}
static int ListFileAttributesSortFunc(ListPanel *pPanel,
                                      const ListPanelItem &item1,
                                      const ListPanelItem &item2) {
  return ListBaseStringSortFunc(pPanel, item1, item2, "attributes");
}
static int ListFileTypeSortFunc(ListPanel *pPanel, const ListPanelItem &item1,
                                const ListPanelItem &item2) {
...
int FileCompletionEdit::AddItem(const char *itemText, KeyValues *userData) {
  ...
  return m_pDropDown->AddMenuItem(
      itemText, new KeyValues("SetText", "text", itemText), this, userData);
}
...
int FileCompletionEdit::GetRowFromItemID(int itemID) {
  ...
  for (i = 0; i < GetItemCount(); i++) {
    if (m_pDropDown->GetMenuID(i) == itemID) return i;
  }
  return -1;
}
...
void FileCompletionEdit::ShowMenu(void) {
  ...
  // make sure we're at the top of the draw order (and therefore our children as
  // well) this important to make sure the menu will be drawn in the foreground
  MoveToFront();
...
static ColumnInfo_t g_ColInfo[] = {
    {"text", "#FileOpenDialog_Col_Name", 175, 20, 10000,
     ListPanel::COLUMN_UNHIDABLE, &ListFileNameSortFunc, Label::a_west},
    {"filesize", "#FileOpenDialog_Col_Size", 100, 20, 10000, 0,
     &ListFileSizeSortFunc, Label::a_east},
    {"type", "#FileOpenDialog_Col_Type", 150, 20, 10000, 0,
     &ListFileTypeSortFunc, Label::a_west},
    {"modified", "#FileOpenDialog_Col_DateModified", 125, 20, 10000, 0,
     &ListFileModifiedSortFunc, Label::a_west},
    {"created", "#FileOpenDialog_Col_DateCreated", 125, 20, 10000,
     ListPanel::COLUMN_HIDDEN, &ListFileCreatedSortFunc, Label::a_west},
    {"attributes", "#FileOpenDialog_Col_Attributes", 50, 20, 10000,
     ListPanel::COLUMN_HIDDEN, &ListFileAttributesSortFunc, Label::a_west},
};

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
FileOpenDialog::FileOpenDialog(Panel *parent, const char *title, bool bOpenOnly,
...
FileOpenDialog::FileOpenDialog(Panel *parent, const char *title,
                               FileOpenDialogType_t type,
                               KeyValues *pContextKeyValues)
    : Frame(parent, "FileOpenDialog") {
  m_DialogType = type;
  Init(title, pContextKeyValues);
...
void FileOpenDialog::Init(const char *title, KeyValues *pContextKeyValues) {
  ...
  for (int i = 0; i < SOURCE_ARRAYSIZE(g_ColInfo); ++i) {
    ...
    m_pFileList->AddColumnHeader(i, info.columnName, info.columnText,
                                 info.startingWidth, info.minWidth,
                                 info.maxWidth, info.flags);
  ...
  switch (m_DialogType) {
    case FOD_OPEN:
      m_pOpenButton =
          new Button(this, "OpenButton", "#FileOpenDialog_Open", this);
      break;
    case FOD_SAVE:
      m_pOpenButton =
          new Button(this, "OpenButton", "#FileOpenDialog_Save", this);
      break;
    case FOD_SELECT_DIRECTORY:
      m_pOpenButton =
          new Button(this, "OpenButton", "#FileOpenDialog_Select", this);
      m_pFileTypeCombo->SetVisible(false);
      break;
  }

  m_pCancelButton =
      new Button(this, "CancelButton", "#FileOpenDialog_Cancel", this);
  ...
  Label *fileName = new Label(this, "FileNameLabel",
                              (m_DialogType != FOD_SELECT_DIRECTORY)
  ...
  m_pFullPathEdit->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_RIGHT,
                                 67, 32, -100, 0);
  m_pFileNameEdit->SetAutoResize(Panel::PIN_BOTTOMLEFT, Panel::AUTORESIZE_RIGHT,
                                 nFileEditLeftSide, -42, -104, 0);
  m_pFileTypeCombo->SetAutoResize(Panel::PIN_BOTTOMLEFT,
                                  Panel::AUTORESIZE_RIGHT, nFileEditLeftSide,
                                  -12, -104, 0);
  m_pFileList->SetAutoResize(Panel::PIN_TOPLEFT, Panel::AUTORESIZE_DOWNANDRIGHT,
                             10, 60, -10, -70);
  ...
  if (m_DialogType != FOD_SELECT_DIRECTORY) {
    Label *fileType =
        new Label(this, "FileTypeLabel", "#FileOpenDialog_File_type");
  ...
  m_pOpenButton->SetCommand((m_DialogType != FOD_SELECT_DIRECTORY)
                                ? new KeyValues("OnOpen")
                                : new KeyValues("SelectFolder"));
  ...
  // Because these call through virtual functions, we can't issue them in the
  // constructor, so we post a message to ourselves instead!!
...
void FileOpenDialog::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pNewFolderButton->AddImage(
      scheme()->GetImage("resource/icon_newfolder", false), -3);
  m_pOpenInExplorerButton->AddImage(
      scheme()->GetImage("resource/icon_play_once", false), -3);
  ...
  imageList->AddImage(
      scheme()->GetImage("resource/icon_folder_selected", false));
...
void FileOpenDialog::OnNewFolder() {
  if (m_hInputDialog.Get()) delete m_hInputDialog.Get();

  m_hInputDialog = new InputDialog(this, "#FileOpenDialog_NewFolder_InputTitle",
...
//-----------------------------------------------------------------------------
// Sets the start directory context (and resets the start directory in the
// process)
//-----------------------------------------------------------------------------
void FileOpenDialog::SetStartDirectoryContext(const char *pStartDirContext,
  ...
  if (pStartDirContext) {
    ...
    if (m_nStartDirContext == s_StartDirContexts.InvalidIndex()) {
      m_nStartDirContext =
          s_StartDirContexts.Insert(pStartDirContext, pDefaultDir);
...
//-----------------------------------------------------------------------------
void FileOpenDialog::AddFilter(const char *filter, const char *filterName,
                               bool bActive, const char *pFilterInfo) {
...
void FileOpenDialog::NewFolder(char const *folderName) {
  ...
  int i = 2;
  do {
    Q_MakeAbsolutePath(pFullPath, sizeof(pFullPath), pNewFolderName,
                       pCurrentDirectory);
...
void FileOpenDialog::PopulateFileList() {
  ...
  if (m_DialogType != FOD_SELECT_DIRECTORY) {
    while ((filterPtr != NULL) && (*filterPtr != 0)) {
      ...
      while (
          (filterPtr != NULL) &&
          ((*filterPtr == ',') || (*filterPtr == ';') || (*filterPtr <= ' '))) {
        ++filterPtr;
      }
      while ((filterPtr != NULL) && (*filterPtr != ',') &&
             (*filterPtr != ';') && (*filterPtr > ' ')) {
      ...
      while (findHandle != INVALID_HANDLE_VALUE) {
        if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
          ...
          kv->SetString("filesize",
                        Q_pretifymem(findData.nFileSizeLow, 0, true));
          Q_FixSlashes(fullpath);
          wchar_t fileType[80];
          g_pFullFileSystem->GetFileTypeForFullPath(fullpath, fileType,
                                                    sizeof(fileType));
          kv->SetWString("type", fileType);
          kv->SetString("attributes",
                        GetAttributesAsString(findData.dwFileAttributes));
  ...
  while (findHandle != INVALID_HANDLE_VALUE) {
    if (findData.cFileName[0] != '.' &&
      ...
      kv->SetString("filesize", "");
      // char extension[ 4 ];
      // Q_ExtractFileExtension( findData.cFileName, extension, sizeof(
      // extension ) );
...
bool FileOpenDialog::ExtensionMatchesFilter(const char *pExt) {
  ...
  while ((filterPtr != NULL) && (*filterPtr != 0)) {
    ...
    curFilter[0] = 0;
    int i = 0;
    while ((filterPtr != NULL) && ((*filterPtr == ',') || (*filterPtr == ';') ||
                                   (*filterPtr <= ' '))) {
      ++filterPtr;
    }
    while ((filterPtr != NULL) && (*filterPtr != ',') && (*filterPtr != ';') &&
...
void FileOpenDialog::SaveFileToStartDirContext(const char *pFullPath) {
  if (m_nStartDirContext == s_StartDirContexts.InvalidIndex()) return;
...
void FileOpenDialog::PostFileSelectedMessage(const char *pFileName) {
  ...
  const char *pFilterInfo =
      pFilterKeys ? pFilterKeys->GetString("filterinfo", NULL) : NULL;
  if (pFilterInfo) {
...
//-----------------------------------------------------------------------------
// Purpose: using the file edit box as a prefix, create a menu of all possible
// files
//-----------------------------------------------------------------------------
