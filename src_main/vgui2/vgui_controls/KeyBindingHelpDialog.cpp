...
// If the user holds the key bound to help down for this long, then the dialog
// will stay on automatically
...
CKeyBindingHelpDialog::CKeyBindingHelpDialog(Panel *parent, Panel *panelToView,
                                             KeyBindingContextHandle_t handle,
                                             KeyCode code, int modifiers)
...
void CKeyBindingHelpDialog::OnCommand(char const *cmd) {
  if (!Q_stricmp(cmd, "OK") || !Q_stricmp(cmd, "cancel") ||
      !Q_stricmp(cmd, "Close")) {
    MarkForDeletion();
...
void CKeyBindingHelpDialog::GetMappingList(
    Panel *panel, CUtlVector<PanelKeyBindingMap *> &maps) {
...
void CKeyBindingHelpDialog::AnsiText(char const *token, char *out,
                                     size_t buflen) {
  out[0] = 0;
...
void CKeyBindingHelpDialog::PopulateList() {
  ...
  while (pPanel->IsKeyBindingChainToParentAllowed()) {
    ...
    while (map) {
      ...
      int c = maps.Count();
      for (k = 0; k < c; ++k) {
        if (maps[k].m_pMap == map) break;
      }
      if (k == c) {
        int z = maps.AddToTail();
        maps[z].m_pMap = map;
        maps[z].m_pPanel = pPanel;
      }
      map = map->baseMap;
    }
  ...
  for (i = 0; i < c; ++i) {
    ...
    for (j = 0; j < bindings; ++j) {
      ...
      item->SetWString("Binding",
                       Panel::KeyCodeModifiersToDisplayString(
                           (KeyCode)kbMap->keycode, kbMap->modifiers));

      // Find the binding
      KeyBindingMap_t *bindingMap = pPanel->LookupBinding(kbMap->bindingname);
      if (bindingMap && bindingMap->helpstring) {
  ...
  for (j = sorted.FirstInorder(); j != sorted.InvalidIndex();
       j = sorted.NextInorder(j)) {
