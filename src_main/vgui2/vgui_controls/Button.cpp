...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Button::Button(Panel *parent, const char *panelName, const char *text,
               Panel *pActionSignalTarget, const char *pCmd)
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Button::Button(Panel *parent, const char *panelName, const wchar_t *wszText,
               Panel *pActionSignalTarget, const char *pCmd)
...
//-----------------------------------------------------------------------------
// Purpose:	Set button depressed state with respect to the force depressed
// state.
//-----------------------------------------------------------------------------
void Button::RecalculateDepressedState(void) {
  ...
  if (!IsEnabled()) {
    ...
    newState = _buttonFlags.IsFlagSet(FORCE_DEPRESSED)
                   ? true
                   : (_buttonFlags.IsFlagSet(ARMED) &&
                      _buttonFlags.IsFlagSet(SELECTED));
  }
...
//-----------------------------------------------------------------------------
// Purpose: Sets whether or not the button captures all mouse input when
// depressed
//			Defaults to true
//			Should be set to false for things like menu items where
//there is a higher-level mouse capture
//-----------------------------------------------------------------------------
...
void Button::PerformLayout() {
  // reset our border
  SetBorder(GetBorder(_buttonFlags.IsFlagSet(DEPRESSED),
                      _buttonFlags.IsFlagSet(ARMED),
                      _buttonFlags.IsFlagSet(SELECTED), HasFocus()));
...
Color Button::GetButtonFgColor() {
  if (!_buttonFlags.IsFlagSet(BLINK)) {
    if (_buttonFlags.IsFlagSet(DEPRESSED)) return _depressedFgColor;
    if (_buttonFlags.IsFlagSet(ARMED)) return _armedFgColor;
  ...
  if (_buttonFlags.IsFlagSet(BLINK)) {
    cBlendedColor[0] = (float)cBlendedColor[0] * fBlink +
                       (float)_blinkFgColor[0] * (1.0f - fBlink);
    cBlendedColor[1] = (float)cBlendedColor[1] * fBlink +
                       (float)_blinkFgColor[1] * (1.0f - fBlink);
    cBlendedColor[2] = (float)cBlendedColor[2] * fBlink +
                       (float)_blinkFgColor[2] * (1.0f - fBlink);
    cBlendedColor[3] = (float)cBlendedColor[3] * fBlink +
                       (float)_blinkFgColor[3] * (1.0f - fBlink);
  }
...
Color Button::GetButtonBgColor() {
  if (_buttonFlags.IsFlagSet(DEPRESSED)) return _depressedBgColor;
  if (_buttonFlags.IsFlagSet(ARMED)) return _armedBgColor;
...
void Button::ApplySchemeSettings(IScheme *pScheme) {
  ...
  _defaultFgColor =
      GetSchemeColor("Button.TextColor", Color(255, 255, 255, 255), pScheme);
  _defaultBgColor =
      GetSchemeColor("Button.BgColor", Color(0, 0, 0, 255), pScheme);

  _armedFgColor =
      GetSchemeColor("Button.ArmedTextColor", _defaultFgColor, pScheme);
  _armedBgColor =
      GetSchemeColor("Button.ArmedBgColor", _defaultBgColor, pScheme);

  _depressedFgColor =
      GetSchemeColor("Button.DepressedTextColor", _defaultFgColor, pScheme);
  _depressedBgColor =
      GetSchemeColor("Button.DepressedBgColor", _defaultBgColor, pScheme);
  _keyboardFocusColor =
      GetSchemeColor("Button.FocusBorderColor", Color(0, 0, 0, 255), pScheme);

  _blinkFgColor =
      GetSchemeColor("Button.BlinkColor", Color(255, 155, 0, 255), pScheme);
  InvalidateLayout();
}
...
//-----------------------------------------------------------------------------
IBorder *Button::GetBorder(bool depressed, bool armed, bool selected,
                           bool keyfocus) {
  if (_buttonFlags.IsFlagSet(BUTTON_BORDER_ENABLED)) {
    // raised buttons with no armed state
    if (depressed) return _depressedBorder;
    if (keyfocus) return _keyFocusBorder;
    ...
    if (depressed) return _depressedBorder;
    if (armed) return _defaultBorder;
  }
...
void Button::SetAsCurrentDefaultButton(int state) {
  if (_buttonFlags.IsFlagSet(DEFAULT_BUTTON) != (bool)state) {
    ...
    if (state) {
      // post a message up notifying our nav group that we're now the default
      // button
      if (GetVParent()) {
...
void Button::FireActionSignal() {
  ...
  if (_actionMessage) {
    ...
    if (!_stricmp(_actionMessage->GetName(), "command") &&
        !_strnicmp(_actionMessage->GetString("command", ""), "url ",
                   std::size("url ") - 1) &&
        strstr(_actionMessage->GetString("command", ""), "://")) {
      // it's a command to launch a url, run it
      system()->ShellExecute(
          "open", _actionMessage->GetString("command", "      ") + 4);
    }
  ...
  if (!_stricmp(outputData->GetName(), "CanBeDefaultButton")) {
    ...
    if (_actionMessage) {
      outputData->SetString("command",
                            _actionMessage->GetString("command", ""));
    } else {
      outputData->SetString("command", "");
    }
    return true;
...
void Button::GetSettings(KeyValues *outResourceData) {
  ...
  if (_actionMessage) {
    outResourceData->SetString("command",
                               _actionMessage->GetString("command", ""));
  }
...
const char *Button::GetDescription(void) {
  ...
  Q_snprintf(buf, sizeof(buf), "%s, string command, int default",
...
void Button::OnMousePressed(MouseCode code) {
  ...
  if (IsUseCaptureMouseEnabled() &&
      _activationType == ACTIVATE_ONPRESSEDANDRELEASED) {
    {
      if (IsKeyBoardInputEnabled()) {
        RequestFocus();
      }
...
void Button::OnMouseReleased(MouseCode code) {
  ...
  // it has to be both enabled and (mouse over the button or using a key) to
  // fire
  if (IsEnabled() && (GetVPanel() == input()->GetMouseOver() ||
                      _buttonFlags.IsFlagSet(BUTTON_KEY_DOWN))) {
    DoClick();
  } else {
    SetSelected(false);
  }
...
void Button::OnKeyCodePressed(KeyCode code) {
  if (code == KEY_SPACE || code == KEY_ENTER) {
    ...
    if (IsUseCaptureMouseEnabled())  // undo the mouse capture since its a fake
                                     // mouse click!
    {
      input()->SetMouseCapture(NULL);
    }
...
//-----------------------------------------------------------------------------
// Purpose: Size the object to its button and text.  - only works from in
// ApplySchemeSettings or PerformLayout()
//-----------------------------------------------------------------------------
