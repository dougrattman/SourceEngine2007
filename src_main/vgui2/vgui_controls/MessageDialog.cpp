...
// CMessageDialog
//-----------------------------------------------------------------------------
CMessageDialog::CMessageDialog(vgui::Panel *pParent, const uint32_t nType,
                               const char *pTitle, const char *pMsg,
                               const char *pCmdA, const char *pCmdB,
                               vgui::Panel *pCreator, bool bShowActivity)
...
//-----------------------------------------------------------------------------
void CMessageDialog::CreateButtonLabel(ButtonLabel_s *pButton,
                                       const char *pIcon, const char *pText) {
...
//-----------------------------------------------------------------------------
// Purpose: Create and arrange the panel button labels according to the dialog
// type
//-----------------------------------------------------------------------------
void CMessageDialog::ApplySchemeSettings(vgui::IScheme *pScheme) {
  BaseClass::ApplySchemeSettings(pScheme);

  LoadControlSettings("resource/UI/MessageDialog.res", "GAME",
                      m_pControlSettings);
  ...
  if (m_nType & MD_OK) {
    CreateButtonLabel(&m_Buttons[BTN_A], "#GameUI_Icons_A_BUTTON",
                      "#GameUI_OK");
  } else if (m_nType & MD_CANCEL) {
    CreateButtonLabel(&m_Buttons[BTN_B], "#GameUI_Icons_B_BUTTON",
                      "#GameUI_Cancel");
  ...
  m_clrNotSimpleBG = pScheme->GetColor("MessageDialog.MatchmakingBG",
  ...
  if (m_bShowActivity && m_ActivityIndent) {
    ...
    m_pMsg->SetBounds(iX + m_ActivityIndent, iY,
                      std::max(0, iW - m_ActivityIndent), iH);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
uint32_t CMessageDialog::GetType(void) { return m_nType; }
