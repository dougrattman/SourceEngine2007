...
BuildGroup::BuildGroup(Panel *parentPanel, Panel *contextPanel) {
  ...
  for (int i = 0; i < 4; ++i) _rulerNumber[i] = NULL;
...
BuildGroup::~BuildGroup() {
  if (m_hBuildDialog) delete m_hBuildDialog.Get();
  m_hBuildDialog = NULL;
...
void BuildGroup::DrawRulers() {
  ...
  surface()->DrawFilledRect(0, cy + ctall - 1, cx,
                            cy + ctall);  // bottom horiz left
  surface()->DrawFilledRect(cx + cwide, cy + ctall - 1, wide,
                            cy + ctall);  // bottom	 horiz right

  surface()->DrawFilledRect(cx, 0, cx + 1, cy);  // top vert left
  surface()->DrawFilledRect(cx + cwide - 1, 0, cx + cwide,
                            cy);  // top vert right

  surface()->DrawFilledRect(cx, cy + ctall, cx + 1, tall);  // bottom vert left
  surface()->DrawFilledRect(cx + cwide - 1, cy + ctall, cx + cwide,
                            tall);  // bottom vert right
  ...
  _rulerNumber[2]->SetPos(cx + cwide + (wide - cx - cwide) / 2 - twide / 2,
                          cy + ctall - 3);
  ...
  _rulerNumber[3]->SetPos(cx + cwide,
                          cy + ctall + (tall - cy - ctall) / 2 - ttall / 2);
}
...
bool BuildGroup::CursorMoved(int x, int y, Panel *panel) {
  ...
  if (_dragging) {
    ...
    if (_dragMouseCode == MOUSE_RIGHT) {
      ...
      bool ctrl = (input()->IsKeyDown(KEY_LCONTROL) ||
      ...
      for (int i = 0; i < _controlGroup.Count(); ++i) {
        // now fix offset of member panels with respect to the one we are
        // dragging
        Panel *groupMember = _controlGroup[i].Get();
        groupMember->SetPos(_dragStartPanelPos[0] + _groupDeltaX[i] +
                                (x - _dragStartCursorPos[0]),
                            _dragStartPanelPos[1] + _groupDeltaY[i] +
                                (y - _dragStartCursorPos[1]));
...
bool BuildGroup::MousePressed(MouseCode code, Panel *panel) {
  ...
  if (panel == m_hBuildDialog) {
    ...
    ivgui()->PostMessage(m_hBuildDialog->GetVPanel(),
                         new KeyValues("HideNewControlMenu"), NULL);
    return true;
  }
  ...
  if (code == MOUSE_RIGHT && panel == GetContextPanel()) {
    ...
    // it is the last panel we clicked in because if we move the panels  as a
    // group it would be from that one
    ...
    // if this panel wasn't already selected update the buildmode dialog
    // controls to show its info
    if (_currentPanel != panel) {
      _currentPanel = panel;

      if (m_hBuildDialog) {
        // think this is taken care of by SetActiveControl.
        // ivgui()->PostMessage(m_hBuildDialog->GetVPanel(), new
        // KeyValues("ApplyDataToControls"), NULL);
...
bool BuildGroup::KeyCodeTyped(KeyCode code, Panel *panel) {
  ...
  switch (code) {
    ...
    case KEY_DELETE: {
      ...
      ivgui()->PostMessage(m_hBuildDialog->GetVPanel(),
                           new KeyValues("DeletePanel"), NULL);
      break;
    }
  }
  ...
  if (ctrl) {
    switch (code) {
      case KEY_Z: {
        ivgui()->PostMessage(m_hBuildDialog->GetVPanel(), new KeyValues("Undo"),
                             NULL);
        break;
      }

      case KEY_C: {
        ivgui()->PostMessage(m_hBuildDialog->GetVPanel(), new KeyValues("Copy"),
                             NULL);
        break;
      }
      case KEY_V: {
        ivgui()->PostMessage(m_hBuildDialog->GetVPanel(),
                             new KeyValues("Paste"), NULL);
        break;
      }
    }
  }
  ...
  if (dx || dy) {
    ...
    if (m_hBuildDialog) {
      ...
      ivgui()->PostMessage(m_hBuildDialog->GetVPanel(),
                           new KeyValues("PanelMoved"), NULL);
    }
  }

  // If holding key while dragging, simulate moving cursor so shift/ctrl key
  // changes take effect
...
//-----------------------------------------------------------------------------
void BuildGroup::LoadControlSettings(const char *controlResourceName,
                                     const char *pathID,
  ...
  if (!rDat) {
    ...
    if (!pathID) {
      bSuccess =
          rDat->LoadFromFile(g_pFullFileSystem, controlResourceName, "SKIN");
    }
    if (!bSuccess) {
      bSuccess =
          rDat->LoadFromFile(g_pFullFileSystem, controlResourceName, pathID);
    }
...
//			use when the dialog may have multiple states and the
// editor  will need to be able to switch between them
//-----------------------------------------------------------------------------
void BuildGroup::RegisterControlSettingsFile(const char *controlResourceName,
                                             const char *pathID) {
  // add the file into a list for build mode
  CUtlSymbol sym(controlResourceName);
  if (!m_RegisteredControlSettingsFiles.IsValidIndex(
          m_RegisteredControlSettingsFiles.Find(sym))) {
...
void BuildGroup::ReloadControlSettings() {
  ...
  for (int i = 1; i < _panelDar.Count(); i++) {
    if (!_panelDar[i].Get())  // this can happen if we had two of the same
                              // handle in the list
    {
      _panelDar.Remove(i);
      --i;
...
bool BuildGroup::SaveControlSettings(void) {
  ...
  if (m_pResourceName) {
    ...
    g_pFullFileSystem->RelativePathToFullPath(
        m_pResourceName, m_pResourcePathID, fullpath, sizeof(fullpath));
...
void BuildGroup::ApplySettings(KeyValues *resourceData) {
  ...
  for (KeyValues *controlKeys = resourceData->GetFirstSubKey();
    ...
    for (int i = 0; i < _panelDar.Count(); i++) {
      ...
      if (!panel)  // this can happen if we had two of the same handle in the
                   // list
      {
        _panelDar.Remove(i);
        --i;
    ...
    if (!bFound) {
      // the key was not found in the registered list, check to see if we should
      // create it
...
Panel *BuildGroup::NewControl(const char *name, int x, int y) {
  ...
  newPanel =
      static_cast<EditablePanel *>(m_pParentPanel)->CreateControlByName(name);
...
//-----------------------------------------------------------------------------
// Purpose: Create a new control in the context panel
// Input:	controlKeys: keyvalues of settings for the panel only works when
...
Panel *BuildGroup::NewControl(KeyValues *controlKeys, int x, int y) {
  ...
  Panel *newPanel = NULL;
  if (controlKeys) {
    KeyValues *keyVal = new KeyValues("ControlFactory", "ControlName",
                                      controlKeys->GetString("ControlName"));
...
//-----------------------------------------------------------------------------
void BuildGroup::GetNewFieldName(char *newFieldName, int newFieldNameSize,
                                 Panel *newPanel) {
  ...
  while (1) {
    Q_snprintf(newFieldName, newFieldNameSize, "%s%d", defaultName,
               fieldNameNumber);
    if (FieldNameTaken(newFieldName) == NULL) break;
    ++fieldNameNumber;
  }
}
...
//-----------------------------------------------------------------------------
// Purpose: sets the panel from which the build group gets all it's object
// creation info
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: gets the panel from which the build group gets all it's object
// creation info
//-----------------------------------------------------------------------------
