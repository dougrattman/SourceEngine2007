...
// enable this define if you want unlocalized strings logged to files
// unfound.txt and unlocalized.txt #define LOG_UNLOCALIZED_STRINGS
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
TextImage::~TextImage() { delete[] _utext; }

//-----------------------------------------------------------------------------
// Purpose: takes the string and looks it up in the localization file to convert
// it to unicode
//-----------------------------------------------------------------------------
void TextImage::SetText(const char *text) {
  ...
  if (*text == '#') {
    ...
    if (_unlocalizedTextSymbol != INVALID_STRING_INDEX) {
      wchar_t *unicode =
          g_pVGuiLocalize->GetValueByIndex(_unlocalizedTextSymbol);
      SetText(unicode);
      return;
    } else {
...
#if defined(LOG_UNLOCALIZED_STRINGS)
    if (text[0]) {
      // setting a label to be ANSI text, write out error to unlocalized.txt log
      // file
      static bool first = true;
      FILE *f;
      if (first) {
...
//-----------------------------------------------------------------------------
void TextImage::GetDrawWidth(int &width) { width = _drawWidth; }
...
void TextImage::SetText(const wchar_t *unicode, bool bClearUnlocalizedSymbol) {
  ...
  wcscpy_s(_utext, (short)(_textLen + 1), unicode);
  m_bRecalculateTruncation = true;
}
...
void TextImage::GetText(wchar_t *buffer, int bufLenInBytes) {
  wcscpy_s(buffer, bufLenInBytes / sizeof(wchar_t), _utext);
}
...
void TextImage::Paint() {
  ...
  if (!_utext || GetFont() == INVALID_FONT) return;
  ...
  for (wchar_t *wsz = _utext; *wsz != 0; wsz++) {
    ...
    if (ch == '\r') {
      ...
      if (wsz[1] == '&') {
        ...
        // draw the underline, then continue to the next character without
        // moving forward
...
void TextImage::GetTextSize(int &wide, int &tall) {
  ...
  HFont font = _font;
  if (font == INVALID_FONT) return;

  if (m_bWrap) RecalculateNewLinePositions();
  ...
  for (wchar_t *wsz = &_utext[startChar]; *wsz != 0; wsz++) {
    ...
    if ((x + charWidth) > _drawWidth || ch == '\r' || ch == '\n') {
      ...
      if (ch == '\r' || ch == '\n') {
        ...
        // ywb 8/1/07:  Back off one extra char since the 'continue' will
        // increment wsz for us by one in the for loop
  ...
  for (int check = 0; check < (m_bUseFallbackFont ? 2 : 1); ++check) {
    ...
    for (wchar_t *wsz = _utext; *wsz != 0; wsz++) {
      ...
      if (ch == '\r') {
        ...
        // "&&" means draw a single ampersand, single one is a shortcut
        // character
        if (wsz[1] == '&') {
          // just move on and draw the second ampersand
          wsz++;
    ...
    if (!m_pwszEllipsesPosition) break;
  }
}

void TextImage::SetWrap(bool bWrap) { m_bWrap = bWrap; }
...
void TextImage::ResizeImageToContentMaxWidth(int nMaxWidth) {
  ...
  // Since we might have to use the "fallback" font, go ahead and recalc the
  // ellipses state first to see if that's the case NOTE:  I think there may be
  // a race condition lurking here, but this seems to work.
  if (m_bRecalculateTruncation) {
    RecalculateEllipsesPosition();
  }
