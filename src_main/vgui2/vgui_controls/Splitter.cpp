...
class SplitterHandle : public Panel {
 ...
 public:
  SplitterHandle(Splitter *parent, const char *name, SplitterMode_t mode,
                 int nIndex);
  ~SplitterHandle();
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
SplitterHandle::SplitterHandle(Splitter *parent, const char *name,
                               SplitterMode_t mode, int nIndex)
...
void SplitterHandle::OnCursorMoved(int x, int y) {
  if (m_bDragging) {
    ...
    pSplitter->SetSplitterPosition(
        m_nIndex, (m_nMode == SPLITTER_MODE_HORIZONTAL) ? y : x);
  }
}
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Splitter::Splitter(Panel *parent, const char *name, SplitterMode_t mode,
                   int nCount)
    : BaseClass(parent, name) {
  Assert(nCount >= 1);
  m_Mode = mode;
...
void Splitter::RecreateSplitters(int nCount) {
  ...
  for (i = 0; i < nCount; ++i) {
    SplitterHandle *pHandle =
        new SplitterHandle(this, "SplitterHandle", m_Mode, i);
...
void Splitter::LockChildSize(int nChildIndex, int nSize) {
  ...
  if (!info.m_bLocked || (info.m_nLockedSize != nSize)) {
    float flPrevPos =
        (nChildIndex > 0) ? m_Splitters[nChildIndex - 1].m_flPos : 0.0f;
