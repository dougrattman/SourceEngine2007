...
// Sort by asset name
//-----------------------------------------------------------------------------
static int __cdecl OperationSortFunc(vgui::ListPanel *pPanel,
...
static int __cdecl FileBrowserSortFunc(vgui::ListPanel *pPanel,
                                       const vgui::ListPanelItem &item1,
                                       const vgui::ListPanelItem &item2) {
...
COperationFileListFrame::COperationFileListFrame(
    vgui::Panel *pParent, const char *pTitle, const char *pColumnHeader,
    bool bShowDescription, bool bShowOkOnly, int nDialogID)
  ...
  if (bShowDescription) {
    m_pSplitter =
        new vgui::Splitter(this, "Splitter", vgui::SPLITTER_MODE_HORIZONTAL, 1);
  ...
  if (bShowDescription) {
    LoadControlSettingsAndUserConfig("resource/perforcefilelistdescription.res",
                                     nDialogID);
  } else {
    LoadControlSettingsAndUserConfig("resource/perforcefilelist.res",
                                     nDialogID);
  }
...
//-----------------------------------------------------------------------------
void COperationFileListFrame::AddOperation(const char *pOperation,
...
// Adds the strings to the list panel
//-----------------------------------------------------------------------------
void COperationFileListFrame::DoModal(KeyValues *pContextKeyValues,
                                      const char *pMessage) {
...
//-----------------------------------------------------------------------------
const char *COperationFileListFrame::GetDescription() { return m_pText; }
...
void COperationFileListFrame::OnCommand(const char *pCommand) {
  ...
  if (!Q_stricmp(pCommand, "No")) {
    KeyValues *pActionKeys =
        new KeyValues(CompletionMessage(), "operationPerformed", 0);
    if (m_pContextKeyValues) {
      pActionKeys->AddSubKey(m_pContextKeyValues);
      m_pContextKeyValues = NULL;
    }
...
//
//-----------------------------------------------------------------------------
CPerforceFileListFrame::CPerforceFileListFrame(vgui::Panel *pParent,
                                               const char *pTitle,
                                               const char *pColumnHeader,
                                               PerforceAction_t action)
    : BaseClass(pParent, pTitle, pColumnHeader,
                (action == PERFORCE_ACTION_FILE_SUBMIT), false,
                OPERATION_DIALOG_ID_PERFORCE) {
  m_Action = action;
}
...
//-----------------------------------------------------------------------------
void CPerforceFileListFrame::DoModal(KeyValues *pContextKeys,
                                     const char *pMessage) {
  BaseClass::DoModal(pContextKeys,
                     pMessage ? pMessage : "PerforceActionConfirmed");
}
...
//-----------------------------------------------------------------------------
void CPerforceFileListFrame::AddFileForSubmit(const char *pFullPath,
                                              P4FileState_t state) {
  if (state == P4FILE_UNOPENED) return;

  char pBuf[128];
  const char *pPrefix =
      (m_Action == PERFORCE_ACTION_FILE_REVERT) ? "Revert" : "Submit";
  switch (state) {
...
//-----------------------------------------------------------------------------
void CPerforceFileListFrame::AddFile(const char *pRelativePath,
                                     const char *pPathId) {
  ...
  // Get the list of opened files, cache it off so we aren't continually
  // reasking
  ...
  for (k = 0; k < nOpenedFileCount; ++k) {
    if (m_OpenedFiles[k].m_eOpenState != P4FILE_OPENED_FOR_DELETE) continue;
    ...
    if (!g_pFullFileSystem->FullPathToRelativePathEx(pLocalFile, pPathId, pTemp,
                                                     sizeof(pTemp)))
      continue;

    // The relative paths had better be the same
    if (Q_stricmp(pTemp, pSearchString)) continue;
...
bool CPerforceFileListFrame::PerformOperation() {
  ...
  const char **ppFileNames =
      (const char **)_alloca(nFileCount * sizeof(char *));
  ...
  if (!bSuccess) {
    vgui::MessageBox *pError =
        new vgui::MessageBox("Perforce Error!", pErrorString, GetParent());
    pError->SetSmallCaption(true);
    pError->DoModal();
  }
#if 0
...
//-----------------------------------------------------------------------------
void ShowPerforceQuery(vgui::Panel *pParent, const char *pFileName,
                       vgui::Panel *pActionSignalTarget, KeyValues *pKeyValues,
  ...
  if ((action == PERFORCE_ACTION_NONE) ||
      ((actionFilter != PERFORCE_ACTION_NONE) && (actionFilter != action))) {
    // Spoof a completion event
    KeyValues *pSpoofKeys =
        new KeyValues("PerforceQueryCompleted", "operationPerformed", 1);
    if (pKeyValues) {
  ...
  CPerforceFileListFrame *pQuery =
      new CPerforceFileListFrame(pParent, pTitle, "File", action);
