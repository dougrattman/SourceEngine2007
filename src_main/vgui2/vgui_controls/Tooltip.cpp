...
// just the ok button of a message box. We use a message box class for the ok
// button and implement another button here.
...
//-----------------------------------------------------------------------------
const char *Tooltip::GetText() { return m_Text.Base(); }
...
void Tooltip::ApplySchemeSettings(IScheme *pScheme) {
  if (s_TooltipWindow.Get()) {
    s_TooltipWindow->SetFont(
        pScheme->GetFont("DefaultSmall", s_TooltipWindow->IsProportional()));
  }
}
...
//-----------------------------------------------------------------------------
int Tooltip::GetTooltipDelay() { return _tooltipDelay; }
...
void Tooltip::PerformLayout() {
  if (!_makeVisible) return;
  if (_delay > system()->GetTimeMillis()) return;
  ...
  s_TooltipWindow->SetBgColor(s_TooltipWindow->GetSchemeColor(
      "Tooltip.BgColor", s_TooltipWindow->GetBgColor(), pScheme));
  s_TooltipWindow->SetFgColor(s_TooltipWindow->GetSchemeColor(
      "Tooltip.TextColor", s_TooltipWindow->GetFgColor(), pScheme));
