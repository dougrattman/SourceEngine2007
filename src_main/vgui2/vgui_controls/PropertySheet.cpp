...
class ContextLabel : public Label {
 ...
 public:
  ContextLabel(Button *parent, char const *panelName, char const *text)
      : BaseClass((Panel *)parent, panelName, text), m_pTabButton(parent) {
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    if (GetParent()) {
      SetFgColor(
          pScheme->GetColor("Button.TextColor", GetParent()->GetFgColor()));
...
static PropertySheet *IsDroppingSheet(CUtlVector<KeyValues *> &msglist) {
  ...
  PropertySheet *sheet =
      reinterpret_cast<PropertySheet *>(data->GetPtr("propertysheet"));
...
class PageTab : public Button {
 ...
 public:
  PageTab(PropertySheet *parent, const char *panelName, const char *text,
          char const *imageName, int maxTabWidth, Panel *page,
          bool showContextButton, long hoverActivatePageTime = -1)
    ...
    m_pContextLabel =
        m_bShowContextLabel ? new ContextLabel(this, "Context", "9") : NULL;
  }
  ...
  virtual void OnThink() {
    if (m_bAttemptingDrop && m_hoverActivatePageTime >= 0 &&
        m_dropHoverTime >= 0) {
  ...
  virtual bool IsDroppable(CUtlVector<KeyValues *> &msglist) {
    ...
    if (sheet) return GetParent()->IsDroppable(msglist);
  ...
  virtual void OnPanelDropped(CUtlVector<KeyValues *> &msglist) {
    ...
    if (!active || !active->IsDroppable(msglist)) return;
        ...
        if (!m_pParent->IsWithin(screenx, screeny)) {
          ...
          if (tw) {
            ...
            if (factory) {
              ...
              factory->InstanceToolWindow(tw->GetParent(),
                                          sheet->ShouldShowContextButtons(),
                                          page, title, hasContextMenu);
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    _textColor = GetSchemeColor("PropertySheet.SelectedTextColor", GetFgColor(),
                                pScheme);
    _dimTextColor =
        GetSchemeColor("PropertySheet.TextColor", GetFgColor(), pScheme);
    ...
    if (m_pImage) {
      ...
      wide = std::max(m_bMaxTabWidth,
                      contentWide + 10);  // 10 = 5 pixels margin on each side
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
PropertySheet::PropertySheet(Panel *parent, const char *panelName,
...
//-----------------------------------------------------------------------------
// Purpose: ToolWindow uses this to drag tools from container to container by
// dragging the tab Input  :  - Output : Returns true on success, false on
// failure.
//-----------------------------------------------------------------------------
...
void PropertySheet::SetSmallTabs(bool state) {
  ...
  m_tabFont = scheme()
                  ...
                  ->GetFont(m_bSmallTabs ? "DefaultVerySmall" : "Default");
...
int PropertySheet::FindPage(Panel *page) const {
  int c = m_Pages.Count();
  for (int i = 0; i < c; ++i) {
    if (m_Pages[i].page == page) return i;
  }
...
void PropertySheet::AddPage(Panel *page, const char *title,
  ...
  PageTab *tab =
      new PageTab(this, "tab", title, imageName, _tabWidth, page,
                  m_bContextButton && bHasContextMenu, hoverActivatePageTime);
  if (m_bDraggableTabs) {
...
void PropertySheet::SetActivePage(Panel *page) {
  ...
  if (!m_Pages.IsValidIndex(index)) return;
...
void PropertySheet::ResetAllData() {
  ...
  for (int i = 0; i < m_Pages.Count(); i++) {
    ipanel()->SendMessage(m_Pages[i].page->GetVPanel(),
                          new KeyValues("ResetData"), GetVPanel());
  }
}
...
void PropertySheet::ApplyChanges() {
  ...
  for (int i = 0; i < m_Pages.Count(); i++) {
    ipanel()->SendMessage(m_Pages[i].page->GetVPanel(),
                          new KeyValues("ApplyChanges"), GetVPanel());
  }
}
...
void PropertySheet::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetBorder(pBorder);
  m_flPageTransitionEffectTime =
      atof(pScheme->GetResourceString("PropertySheet.TransitionEffectTime"));
...
void PropertySheet::PaintBorder() {
  ...
  int wide, tall;
  GetSize(wide, tall);
  border->Paint(0, py + ptall, wide, tall, IBorder::SIDE_TOP, px + 1,
                px + pwide - 1);
}
...
void PropertySheet::RemovePage(Panel *panel) {
  ...
  if (location == m_Pages.InvalidIndex()) return;
...
void PropertySheet::ChangeActiveTab(int index) {
  ...
  // notify old page
  if (_activePage) {
    ivgui()->PostMessage(_activePage->GetVPanel(), new KeyValues("PageHide"),
                         GetVPanel());
  ...
  if (m_flPageTransitionEffectTime) {
    if (m_hPreviouslyActivePage.Get()) {
      ...
      GetAnimationController()->RunAnimationCommand(
          m_hPreviouslyActivePage, "Alpha", 0.0f, 0.0f,
          m_flPageTransitionEffectTime / 2,
    ...
    GetAnimationController()->RunAnimationCommand(
        _activePage, "Alpha", 255.0f, m_flPageTransitionEffectTime / 2,
        m_flPageTransitionEffectTime / 2,
        AnimationController::INTERPOLATOR_LINEAR);
  } else {
    if (m_hPreviouslyActivePage.Get()) {
  ...
  // notify
  ivgui()->PostMessage(_activePage->GetVPanel(), new KeyValues("PageShow"),
                       GetVPanel());
...
//-----------------------------------------------------------------------------
// Purpose: Called by the associated combo box (if in that mode), changes the
// current panel
//-----------------------------------------------------------------------------
...
bool PropertySheet::PageHasContextMenu(Panel *page) const {
  ...
  if (pageNum == m_Pages.InvalidIndex()) return false;
...
bool PropertySheet::IsDroppable(CUtlVector<KeyValues *> &msglist) {
  if (!m_bDraggableTabs) return false;
...
void PropertySheet::OnDroppablePanelPaint(CUtlVector<KeyValues *> &msglist,
                                          CUtlVector<Panel *> &dragPanels) {
  ...
  PageTab *fakeTab =
      new PageTab(this, "FakeTab", text, NULL, _tabWidth, NULL, false);
