...
// for the ok button and implement another button here.
...
enum { WINDOW_BORDER_WIDTH = 1 };

vgui::Panel *HTML_NoJavascript_Factory() { return new HTML(NULL, NULL, false); }

vgui::Panel *HTML_Javascript_Factory() { return new HTML(NULL, NULL, true); }

// DECLARE_BUILD_FACTORY_CUSTOM_ALIAS( HTML, HTML_NoJavascript,
// HTML_NoJavascript_Factory ); DECLARE_BUILD_FACTORY_CUSTOM_ALIAS( HTML,
// HTML_Javascript, HTML_Javascript_Factory );

//-----------------------------------------------------------------------------
// Purpose: Constructor
//-----------------------------------------------------------------------------
HTML::HTML(Panel *parent, const char *name, bool allowJavaScript)
    : Panel(parent, name) {
...
// Purpose: Destructor
//-----------------------------------------------------------------------------
HTML::~HTML() { surface()->DeleteHTMLWindow(browser); }
...
//-----------------------------------------------------------------------------
// Purpose: Causes the HTML window to repaint itself every 100ms, to allow
// animaited gifs and the like
//-----------------------------------------------------------------------------
void HTML::StartAnimate(int time) {
  // a tick signal to let the web page re-render itself, in case of animated
  // images
...
//-----------------------------------------------------------------------------
// Purpose: overrides panel class, paints a texture of the HTML window as a
// background
//-----------------------------------------------------------------------------
void HTML::PaintBackground() {
  ...
  if (surface()->SupportsFeature(ISurface::DIRECT_HWND_RENDER)) {
    ...
    // If we have scrollbars, we need to draw the bg color under them, since the
    // browser bitmap is a checkerboard under them, and they are transparent in
    // the in-game client
    if (m_iScrollBorderX > 0 || m_iScrollBorderY > 0) {
      ...
      surface()->DrawSetColor(GetBgColor());
      if (m_iScrollBorderX) {
        surface()->DrawFilledRect(w - m_iScrollBorderX - right, top, w - right,
                                  h - bottom);
      }
      if (m_iScrollBorderY) {
        surface()->DrawFilledRect(left, h - m_iScrollBorderY - bottom,
                                  w - m_iScrollBorderX - right, h - bottom);
      }
    }
  }
}
...
void HTML::SetVisible(bool state) {
  ...
  if (IsLayoutInvalid() &&
      state)  // only set visible on performlayout IF you are setting it visible
  {
...
//-----------------------------------------------------------------------------
// Purpose: calculates the need for and position of both horizontal and vertical
// scroll bars
//-----------------------------------------------------------------------------
void HTML::CalcScrollBars(int w, int h) {
  ...
  if (m_bScrollBarEnabled) {
    ...
    if (img_h > h) {
      ...
      _vbar->SetPos(w - (_vbar->GetWide() + WINDOW_BORDER_WIDTH),
                    WINDOW_BORDER_WIDTH);
      if (img_w > w) {
        _vbar->SetSize(_vbar->GetWide(),
                       h - _vbar->GetWide() - 1 - WINDOW_BORDER_WIDTH);
      } else {
    ...
    if (img_w > w) {
      ...
      _hbar->SetPos(WINDOW_BORDER_WIDTH,
                    h - (_vbar->GetWide() + WINDOW_BORDER_WIDTH));
      if (img_h > h) {
        _hbar->SetSize(w - _vbar->GetWide() - WINDOW_BORDER_WIDTH,
                       _vbar->GetWide());
      } else {
...
// Purpose: opens the URL, will accept any URL that IE accepts
//-----------------------------------------------------------------------------
bool HTML::StopLoading() { return browser->StopLoading(); }

//-----------------------------------------------------------------------------
// Purpose: refreshes the current page
//-----------------------------------------------------------------------------
bool HTML::Refresh() { return browser->Refresh(); }

//-----------------------------------------------------------------------------
// Purpose: empties the current HTML container of any HTML text (used in
// conjunction with AddText)
//-----------------------------------------------------------------------------
void HTML::Clear() { browser->Clear(); }

//-----------------------------------------------------------------------------
// Purpose: appends "text" to the end of the current page. "text" should be a
// HTML formatted string
//-----------------------------------------------------------------------------
void HTML::AddText(const char *text) { browser->AddText(text); }
...
//-----------------------------------------------------------------------------
void HTML::OnTick() {
  if (IsVisible() && m_iAnimTime &&
      system()->GetTimeMillis() >= m_iNextFrameTime) {
...
//-----------------------------------------------------------------------------
void HTML::AddCustomURLHandler(const char *customProtocolName,
                               vgui::Panel *target) {
...
bool HTML::OnStartURL(const char *url, const char *target, bool first) {
  if (IsVisible() &&
      !surface()->SupportsFeature(ISurface::DIRECT_HWND_RENDER)) {
    SetCursor(dc_arrow);
  }
...
void HTML::BrowserResize() {
  ...
  if (browser) {
    browser->OnSize(m_iScrollX + left, m_iScrollY + top,
                    w - m_iScrollBorderX - right,
                    h - m_iScrollBorderY - bottom);
  }
}
...
void HTML::OnFinishURL(const char *url) {
  ...
  m_bRegenerateHTMLBitmap =
      true;  // repaint the window, as we have a new picture to show
  if (!surface()->SupportsFeature(ISurface::DIRECT_HWND_RENDER)) {
    Repaint();
  }
}
...
//-----------------------------------------------------------------------------
void HTML::OnUpdate() {
  // only let it redraw every m_iAnimTime milliseconds, so stop it sucking up
  // all the CPU time
...
//-----------------------------------------------------------------------------
void HTML::SetScrollbarsEnabled(bool state) { m_bScrollBarEnabled = state; }

//-----------------------------------------------------------------------------
// Purpose: data accessor
//-----------------------------------------------------------------------------
void HTML::SetContextMenuEnabled(bool state) { m_bContextMenuEnabled = state; }

//-----------------------------------------------------------------------------
// Purpose: data accessor
//-----------------------------------------------------------------------------
void HTML::NewWindowsOnly(bool state) { m_bNewWindowsOnly = state; }
