...
// Purpose: Destructor
//-----------------------------------------------------------------------------
AnalogBar::~AnalogBar() { delete[] m_pszDialogVar; }
...
//-----------------------------------------------------------------------------
// Purpose: returns the total number of segment blocks drawn (active and
// inactive)
//-----------------------------------------------------------------------------
...
void AnalogBar::PaintSegment(int &x, int &y, int tall, int wide, Color color,
                             bool bHome) {
  switch (m_iAnalogValueDirection) {
    case PROGRESS_EAST:
      ...
      if (bHome) {
        ...
        surface()->DrawFilledRect(x, y, x + _segmentWide,
                                  y + ANALOG_BAR_HOME_SIZE);
        surface()->DrawFilledRect(x, y + tall - (y * 2) - ANALOG_BAR_HOME_SIZE,
                                  x + _segmentWide, y + tall - (y * 2));
      }
    ...
    case PROGRESS_NORTH:
      ...
      if (bHome) {
        surface()->DrawSetColor(GetHomeColor());
        surface()->DrawFilledRect(x, y, x + ANALOG_BAR_HOME_SIZE,
                                  y + _segmentWide);
        surface()->DrawFilledRect(x + wide - (x * 2) - ANALOG_BAR_HOME_SIZE, y,
                                  x + wide - (x * 2), y + _segmentWide);
      }

      surface()->DrawSetColor(color);
      surface()->DrawFilledRect(x + ANALOG_BAR_LESS_TALL, y,
                                x + wide - (x * 2) - ANALOG_BAR_LESS_TALL,
                                y + _segmentWide);
      break;
...
void AnalogBar::ApplySchemeSettings(IScheme *pScheme) {
  ...
  SetBgColor(Color(255 - GetFgColor().r(), 255 - GetFgColor().g(),
                   255 - GetFgColor().b(), GetFgColor().a()));
}

//-----------------------------------------------------------------------------
// Purpose: utility function for calculating a time remaining string
//-----------------------------------------------------------------------------
bool AnalogBar::ConstructTimeRemainingString(wchar_t *output,
                                             int outputBufferSizeInBytes,
                                             float startTime, float currentTime,
  ...
  if (lastAnalogValueUpdateTime < currentTime) {
    ...
    float extrapolatedTotalTime =
        (currentTime - startTime) / extrapolatedAnalogValue;
  ...
  if (minutesRemaining > 0) {
    ...
    g_pVGuiLocalize->ConstructString(output, outputBufferSizeInBytes,
                                     g_pVGuiLocalize->Find(unlocString), 2,
    ...
    g_pVGuiLocalize->ConstructString(output, outputBufferSizeInBytes,
                                     g_pVGuiLocalize->Find(unlocString), 1,
                                     unicodeSeconds);
  } else {
    return false;
  }
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
void AnalogBar::SetBarInset(int pixels) { m_iBarInset = pixels; }
...
void AnalogBar::ApplySettings(KeyValues *inResourceData) {
  ...
  if (dialogVar && *dialogVar) {
    usize size{strlen(dialogVar) + 1};
    m_pszDialogVar = new char[size];
    strcpy_s(m_pszDialogVar, size, dialogVar);
  }
...
void ContinuousAnalogBar::Paint() {
  ...
  switch (m_iAnalogValueDirection) {
    ...
    case PROGRESS_WEST:
      surface()->DrawFilledRect(x + (int)(wide * (1.0f - _analogValue)), y,
                                x + wide, y + tall);
      break;

    case PROGRESS_NORTH:
      surface()->DrawFilledRect(x, y + (int)(tall * (1.0f - _analogValue)),
                                x + wide, y + tall);
      break;
