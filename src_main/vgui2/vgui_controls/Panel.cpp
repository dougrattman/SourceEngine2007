...
struct vgui::DragDrop_t {
  ...
  bool m_bDragging;
  bool m_bDragStarted;
  // How many pixels the dragged box must move before showing the outline
  // rect...
  ...
  // Drop related data
  bool m_bDropEnabled;
  // A droppable panel can have a hover context menu, which will show up after
  // m_flHoverContextTime of hovering
...
KeyBindingMap_t::KeyBindingMap_t()
    : bindingname(0), func(0), helpstring(0), docstring(0), passive(false) {}
...
class CKeyBindingsMgr {
 public:
  ...
  static bool KeyBindingContextHandleLessFunc(const KBContext_t &lhs,
                                              const KBContext_t &rhs) {
    return lhs.m_Handle < rhs.m_Handle;
  }

  KeyBindingContextHandle_t CreateContext(char const *filename,
                                          char const *pathID) {
    KBContext_t entry;
...
//-----------------------------------------------------------------------------
KeyBindingContextHandle_t Panel::CreateKeyBindingsContext(
...
// Output : Panel
//-----------------------------------------------------------------------------
Panel *Panel::GetPanelWithKeyBindings(KeyBindingContextHandle_t handle,
                                      int index) {
...
//-----------------------------------------------------------------------------
// Purpose: static method.  Reverts key bindings for all registered panels
// (panels with keybindings actually
//  loaded from file
// Input  :  -
//-----------------------------------------------------------------------------
...
// Input  :  -
//-----------------------------------------------------------------------------
void Panel::SaveKeyBindingsToFile(KeyBindingContextHandle_t handle,
                                  char const *filename,
                                  char const *pathID /*= 0*/) {
...
//			*panelOfInterest -
//-----------------------------------------------------------------------------
void Panel::LoadKeyBindingsForOnePanel(KeyBindingContextHandle_t handle,
                                       Panel *panelOfInterest) {
  if (!panelOfInterest) return;
  if (!panelOfInterest->GetName()) return;
  if (!panelOfInterest->GetName()[0]) return;
...
void Panel::Init(int x, int y, int wide, int tall) {
  ...
  SetPos(x, y);
  SetSize(wide, tall);
  _flags.SetFlag(NEEDS_LAYOUT | NEEDS_SCHEME_UPDATE |
  ...
  _buildModeFlags =
      0;  // not editable or deletable in buildmode dialog by default
...
//-----------------------------------------------------------------------------
// Purpose: fully construct this panel so its ready for use right now (i.e fonts
// loaded, colors set, default label text set, ...)
//-----------------------------------------------------------------------------
...
void Panel::SetName(const char *panelName) {
  // No change?
  if (_panelName && panelName && !Q_strcmp(_panelName, panelName)) {
    return;
  }
...
//-----------------------------------------------------------------------------
// Purpose: returns the name of the module that this instance of panel was
// compiled into
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
VPANEL Panel::GetVParent() { return ipanel()->GetParent(GetVPanel()); }
...
Panel *Panel::GetParent() {
  ...
  // this is OK, the hierarchy is guaranteed to be all from the same module,
  // except for the root node the root node always returns NULL when a
  // GetParent() is done so everything is OK
  VPANEL parent = ipanel()->GetParent(GetVPanel());
  if (parent) {
    Panel *pParent = ipanel()->GetPanel(parent, GetControlsModuleName());
    Assert(!pParent ||
           !strcmp(pParent->GetModuleName(), GetControlsModuleName()));
    return pParent;
  }
...
void Panel::OnScreenSizeChanged(int nOldWide, int nOldTall) {
  ...
  for (int i = 0; i < ipanel()->GetChildCount(GetVPanel()); i++) {
    ...
    PostMessage(child,
                new KeyValues("OnScreenSizeChanged", "oldwide", nOldWide,
                              "oldtall", nOldTall),
                NULL);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
bool Panel::IsVisible() { return ipanel()->IsVisible(GetVPanel()); }
...
void Panel::PaintTraverse(bool repaint, bool allowForce) {
  ...
  int clipRect[4];
  ipanel()->GetClipRect(vpanel, clipRect[0], clipRect[1], clipRect[2],
                        clipRect[3]);
  ...
  if (repaint && _flags.IsFlagSet(PAINT_BORDER_ENABLED) && (_border != 0)) {
  ...
  if (repaint && IsBuildGroupEnabled())  //&& HasFocus() )
  {
    ...
    for (int i = 0; i < controlGroup->Size(); ++i) {
      surface()->PushMakeCurrent(((*controlGroup)[i].Get())->GetVPanel(),
                                 false);
...
void Panel::PaintBackground() {
  ...
  if (m_SkipChild.Get() && m_SkipChild->IsVisible()) {
    if (GetPaintBackgroundType() == 2) {
      ...
      FillRectSkippingPanel(GetBgColor(), cornerWide, cornerTall, wide, tall,
                            m_SkipChild.Get());
    } else {
...
void Panel::SetParent(VPANEL newParent) {
  ...
  if (GetVParent() && !IsPopup()) {
    ...
    if (ipanel()->IsKeyBoardInputEnabled(GetVParent()) !=
...
//-----------------------------------------------------------------------------
// Purpose: if set to true, panel automatically frees itself when parent is
// deleted
//-----------------------------------------------------------------------------
...
void Panel::InternalCursorEntered() {
  if (IsCursorNone() || !IsMouseInputEnabled()) return;
...
//-----------------------------------------------------------------------------
// Purpose: Checks to see if message is being subverted due to modal subtree
// logic Input  :  - Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
...
KeyBindingMap_t *Panel::LookupBinding(char const *bindingName) {
  ...
  while (map) {
    ...
    for (int i = 0; i < c; ++i) {
      ...
      if (!Q_stricmp(binding->bindingname, bindingName)) return binding;
    }
...
PanelKeyBindingMap *Panel::LookupMapForBinding(char const *bindingName) {
  ...
  while (map) {
    ...
    for (int i = 0; i < c; ++i) {
      KeyBindingMap_t *binding = &map->entries[i];
      if (!Q_stricmp(binding->bindingname, bindingName)) return map;
    }
...
void Panel::LookupBoundKeys(char const *bindingName,
...
static void AddModifierToString(char const *modifiername, char *buf,
                                size_t bufsize) {
  char add[32];
  if (Q_strlen(buf) > 0) {
...
wchar_t const *Panel::KeyCodeModifiersToDisplayString(KeyCode code,
                                                      int modifiers) {
  char sz[256];
  sz[0] = 0;
...
static void WriteKeyBindingToBuffer(CUtlBuffer &buf, int level,
                                    const BoundKey_t &binding) {
...
bool Panel::ParseKeyBindings(KeyValues *kv) {
  Assert(IsValidKeyBindingsContext());
  if (!IsValidKeyBindingsContext()) return false;

  // To have KB the panel must have a name
  Assert(GetName() && GetName()[0]);
  if (!GetName() || !GetName()[0]) return false;
  ...
  for (KeyValues *binding = kv->GetFirstSubKey(); binding != NULL;
       binding = binding->GetNextKey()) {
    char const *bindingName = binding->GetName();
    if (!bindingName || !bindingName[0]) continue;
    ...
    if (b) {
      ...
      KeyBindingMap_t *bound =
          LookupBindingByKeyCode(StringToKeyCode(keycode), modifiers);
      if (!bound) {
  ...
  // Now for each binding which is currently "unbound" to any key, use the
  // default binding
  PanelKeyBindingMap *map = GetKBMap();
  while (map) {
    int c = map->entries.Count();
    for (int i = 0; i < c; ++i) {
      ...
      if (list.Count() == 0) {
        ...
        if (defaultKey) {
          KeyBindingMap_t *alreadyBound = LookupBindingByKeyCode(
              (KeyCode)defaultKey->keycode, defaultKey->modifiers);
          if (alreadyBound) {
            Warning("No binding for '%s', defautl key already bound to '%s'\n",
                    binding->bindingname, alreadyBound->bindingname);
          } else {
            AddKeyBinding(defaultKey->bindingname, defaultKey->keycode,
...
//-----------------------------------------------------------------------------
// Purpose: Set this to false to disallow IsKeyRebound chaining to GetParent()
// Panels... Input  : state -
//-----------------------------------------------------------------------------
...
bool Panel::IsKeyRebound(KeyCode code, int modifiers) {
  ...
  if (IsKeyBindingChainToParentAllowed() && pParent &&
      !IsKeyOverridden(code, modifiers))
...
void Panel::InternalSetCursor() {
  ...
  if (visible) {
    ...
    if (m_pDragDrop->m_bDragging || g_DragDropCapture.Get() != NULL) return;
#endif
    ...
    // only change the cursor if this panel is visible, and if its part of the
    // main VGUI tree
...
void Panel::OnThink() {
  ...
  if (IsPC() && m_pDragDrop->m_bDragEnabled && m_pDragDrop->m_bDragging &&
    ...
    if (m_pDragDrop->m_hCurrentDrop != NULL) {
      ...
      // allow the cursor to change based upon things like changing keystate,
      // etc.
      surface()->SetCursor(
          m_pDragDrop->m_hCurrentDrop->GetDropCursor(m_pDragDrop->m_DragData));

      if (!m_pDragDrop->m_bDropMenuShown) {
        // See if the hover time has gotten larger
        float hoverSeconds =
            (system()->GetTimeMillis() - m_pDragDrop->m_lDropHoverTime) *
            0.001f;
        ...
        if (dropInfo->m_flHoverContextTime != 0.0f) {
          if (hoverSeconds >= dropInfo->m_flHoverContextTime) {
            ...
            Menu *menu =
                new Menu(m_pDragDrop->m_hCurrentDrop.Get(), "DropContext");

            bool useMenu =
                m_pDragDrop->m_hCurrentDrop->GetDropContextMenu(menu, data);
            if (useMenu) {
...
// base implementation forwards Key messages to the Panel's parent - override to
// 'swallow' the input
...
bool Panel::IsWithin(int x, int y) {
  ...
  int clipRect[4];
  ipanel()->GetClipRect(GetVPanel(), clipRect[0], clipRect[1], clipRect[2],
                        clipRect[3]);
...
VPANEL Panel::IsWithinTraverse(int x, int y, bool traversePopups) {
  ...
  if (traversePopups) {
    ...
    // this checks in backwards order so the last child drawn for this panel is
    // chosen which coincides to how it would be visibly displayed
    for (i = GetChildCount() - 1; i >= 0; i--) {
      VPANEL panel = ipanel()->GetChild(GetVPanel(), i);
    ...
    // since we're not checking popups, it must be within us, so we can check
    // ourself first
    if (IsWithin(x, y)) {
      // check children recursive, if you find one, just return first one
      // this checks in backwards order so the last child drawn for this panel
      // is chosen which coincides to how it would be visibly displayed
...
// Purpose:
//-----------------------------------------------------------------------------
int Panel::GetChildCount() { return ipanel()->GetChildCount(GetVPanel()); }
...
Panel *Panel::GetChild(int index) {
  ...
  // this assumes that the child is from the same module as the this
  // (precondition)
  return ipanel()->GetPanel(ipanel()->GetChild(GetVPanel(), index),
                            GetControlsModuleName());
}
...
// Input  : direction - the direction in which focus travelled to arrive at this
// panel; forward = 1, back = -1
//-----------------------------------------------------------------------------
...
void Panel::OnRequestFocus(VPANEL subFocus, VPANEL defaultPanel) {
  CallParentFunction(new KeyValues("OnRequestFocus", "subFocus", subFocus,
...
void Panel::InternalFocusChanged(bool lost) {
  /*
          //if focus is gained tell the focusNavGroup about it so its current
     can be correct if( (!lost) && (_focusNavGroup!=0) )
          {
...
void Panel::RemoveActionSignalTarget(Panel *oldTarget) {
  _actionSignalTargetDar.RemoveElement(
      ivgui()->PanelToHandle(oldTarget->GetVPanel()));
}
...
bool Panel::IsBuildGroupEnabled() {
  if (!_buildGroup.IsValid()) return false;
...
void Panel::MarkForDeletion() {
  if (_flags.IsFlagSet(MARKED_FOR_DELETION)) return;
...
void Panel::SetAutoResize(PinCorner_e pinCorner, AutoResize_e resizeDir,
                          ...
                          int nUnpinnedCornerOffsetX,
                          int nUnpinnedCornerOffsetY) {
...
#if defined(VGUI_USEDRAGDROP)
  m_clrDragFrame =
      pScheme->GetColor("DragDrop.DragFrame", Color(255, 255, 255, 192));
  m_clrDropFrame =
      pScheme->GetColor("DragDrop.DropFrame", Color(150, 255, 150, 255));
...
void Panel::PerformApplySchemeSettings() {
  ...
  if (_flags.IsFlagSet(NEEDS_SCHEME_UPDATE)) {
    ...
    AssertOnce(pScheme);
    if (pScheme)  // this should NEVER be 0, but if it is bad things would
                  // happen in ApplySchemeSettings...
    {
...
void Panel::ApplyAutoResizeSettings(KeyValues *inResourceData) {
  ...
  AutoResize_e autoResize =
      (AutoResize_e)inResourceData->GetInt("AutoResize", AUTORESIZE_NO);
  PinCorner_e pinCorner =
      (PinCorner_e)inResourceData->GetInt("PinCorner", PIN_TOPLEFT);
  ...
  if (IsProportional()) {
    if (inResourceData->FindKey("PinnedCornerOffsetX")) {
      nPinnedCornerOffsetX = scheme()->GetProportionalScaledValueEx(
          GetScheme(), inResourceData->GetInt("PinnedCornerOffsetX"));
    }
    if (inResourceData->FindKey("PinnedCornerOffsetY")) {
      nPinnedCornerOffsetY = scheme()->GetProportionalScaledValueEx(
          GetScheme(), inResourceData->GetInt("PinnedCornerOffsetY"));
    }
    if (inResourceData->FindKey("UnpinnedCornerOffsetX")) {
      nUnpinnedCornerOffsetX = scheme()->GetProportionalScaledValueEx(
          GetScheme(), inResourceData->GetInt("UnpinnedCornerOffsetX"));
    }
    if (inResourceData->FindKey("UnpinnedCornerOffsetY")) {
      nUnpinnedCornerOffsetY = scheme()->GetProportionalScaledValueEx(
          GetScheme(), inResourceData->GetInt("UnpinnedCornerOffsetY"));
    }
  } else {
    nPinnedCornerOffsetX =
        inResourceData->GetInt("PinnedCornerOffsetX", nPinnedCornerOffsetX);
    nPinnedCornerOffsetY =
        inResourceData->GetInt("PinnedCornerOffsetY", nPinnedCornerOffsetY);
    nUnpinnedCornerOffsetX =
        inResourceData->GetInt("UnpinnedCornerOffsetX", nUnpinnedCornerOffsetX);
    nUnpinnedCornerOffsetY =
        inResourceData->GetInt("UnpinnedCornerOffsetY", nUnpinnedCornerOffsetY);
  }
  ...
  SetAutoResize(pinCorner, autoResize, nPinnedCornerOffsetX,
                nPinnedCornerOffsetY, nUnpinnedCornerOffsetX,
                nUnpinnedCornerOffsetY);
}
...
void Panel::ApplySettings(KeyValues *inResourceData) {
  ...
  _buildModeFlags &=
      ~(BUILDMODE_SAVE_XPOS_RIGHTALIGNED | BUILDMODE_SAVE_XPOS_CENTERALIGNED |
        BUILDMODE_SAVE_YPOS_BOTTOMALIGNED | BUILDMODE_SAVE_YPOS_CENTERALIGNED |
...
const char *Panel::GetDescription(void) {
  static const char *panelDescription =
      "string fieldName, int xpos, int ypos, int wide, int tall, bool visible, "
      "bool enabled, int tabPosition, corner pinCorner, autoresize autoResize, "
      "string tooltiptext";
...
//			this is used for any control details the user wants
// saved  between sessions 			eg. dialog positions, last
// directory opened, list column  width
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: optimization, return true if this control has any user config
// settings
//-----------------------------------------------------------------------------
  ...
  while (panelMap != NULL && !panelMap->processed) {
    ...
    for (int i = 0; i < panelMap->entries.Count(); i++) {
      ...
      if (item->firstParamName) {
        item->firstParamSymbol =
            KeyValuesSystem()->GetSymbolForString(item->firstParamName);
      } else {
        item->firstParamSymbol = INVALID_KEY_SYMBOL;
      }
      if (item->secondParamName) {
        item->secondParamSymbol =
            KeyValuesSystem()->GetSymbolForString(item->secondParamName);
      } else {
      ...
      if (iMessageName == pMap->nameSymbol) {
        ...
        switch (pMap->numParams) {
          ...
          case 1: {
            ...
            switch (pMap->firstParamType) {
              ...
              case DATATYPE_UINT64:
                typedef void (Panel::*MessageFunc_Uin64_t)(uint64_t);
              ...
              case DATATYPE_KEYVALUES:
                ...
                if (pMap->firstParamName) {
                  (this->*((MessageFunc_KeyValues_t)pMap->func))(
                      (KeyValues *)param1->GetPtr());
                } else {
                  // no param set, so pass in the whole thing
                  (this->*((MessageFunc_KeyValues_t)pMap->func))(
                      const_cast<KeyValues *>(params));
                }
                break;
          ...
          case 2: {
            ...
            if ((DATATYPE_INT == pMap->firstParamType) &&
                (DATATYPE_INT == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_IntInt_t)(int, int);
              (this->*((MessageFunc_IntInt_t)pMap->func))(param1->GetInt(),
                                                          param2->GetInt());
            } else if ((DATATYPE_PTR == pMap->firstParamType) &&
                       (DATATYPE_INT == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_PtrInt_t)(void *, int);
              (this->*((MessageFunc_PtrInt_t)pMap->func))(param1->GetPtr(),
                                                          param2->GetInt());
            } else if ((DATATYPE_CONSTCHARPTR == pMap->firstParamType) &&
                       (DATATYPE_INT == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_ConstCharPtrInt_t)(const char *,
                                                                   int);
              (this->*((MessageFunc_ConstCharPtrInt_t)pMap->func))(
                  param1->GetString(), param2->GetInt());
            } else if ((DATATYPE_CONSTCHARPTR == pMap->firstParamType) &&
                       (DATATYPE_CONSTCHARPTR == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_ConstCharPtrConstCharPtr_t)(
                  const char *, const char *);
              (this->*((MessageFunc_ConstCharPtrConstCharPtr_t)pMap->func))(
                  param1->GetString(), param2->GetString());
            } else if ((DATATYPE_INT == pMap->firstParamType) &&
                       (DATATYPE_CONSTCHARPTR == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_IntConstCharPtr_t)(
                  int, const char *);
              (this->*((MessageFunc_IntConstCharPtr_t)pMap->func))(
                  param1->GetInt(), param2->GetString());
            } else if ((DATATYPE_PTR == pMap->firstParamType) &&
                       (DATATYPE_CONSTCHARPTR == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_PtrConstCharPtr_t)(
                  void *, const char *);
              (this->*((MessageFunc_PtrConstCharPtr_t)pMap->func))(
                  param1->GetPtr(), param2->GetString());
            } else if ((DATATYPE_PTR == pMap->firstParamType) &&
                       (DATATYPE_CONSTWCHARPTR == pMap->secondParamType)) {
              typedef void (Panel::*MessageFunc_PtrConstCharPtr_t)(
                  void *, const wchar_t *);
              (this->*((MessageFunc_PtrConstCharPtr_t)pMap->func))(
                  param1->GetPtr(), param2->GetWString());
            } else {
    ...
    for (int i = 0; i < panelMap->dataNumFields; i++) {
      if (iMessageName == pMessageMap[i].nameSymbol) {
        ...
        switch (pMessageMap[i].numParams) {
          case 2:
            if ((DATATYPE_INT == pMessageMap[i].firstParamType) &&
                (DATATYPE_INT == pMessageMap[i].secondParamType)) {
              typedef void (Panel::*MessageFunc_IntInt_t)(int, int);
              (this->*((MessageFunc_IntInt_t)pMessageMap[i].func))(
                  params->GetInt(pMessageMap[i].firstParamName),
                  params->GetInt(pMessageMap[i].secondParamName));
            } else if ((DATATYPE_PTR == pMessageMap[i].firstParamType) &&
                       (DATATYPE_INT == pMessageMap[i].secondParamType)) {
              typedef void (Panel::*MessageFunc_PtrInt_t)(void *, int);
              (this->*((MessageFunc_PtrInt_t)pMessageMap[i].func))(
                  params->GetPtr(pMessageMap[i].firstParamName),
                  params->GetInt(pMessageMap[i].secondParamName));
            } else if ((DATATYPE_CONSTCHARPTR ==
                        pMessageMap[i].firstParamType) &&
                       (DATATYPE_INT == pMessageMap[i].secondParamType)) {
              typedef void (Panel::*MessageFunc_ConstCharPtrInt_t)(const char *,
                                                                   int);
              (this->*((MessageFunc_ConstCharPtrInt_t)pMessageMap[i].func))(
                  params->GetString(pMessageMap[i].firstParamName),
                  params->GetInt(pMessageMap[i].secondParamName));
            } else if ((DATATYPE_CONSTCHARPTR ==
                        pMessageMap[i].firstParamType) &&
                       (DATATYPE_CONSTCHARPTR ==
                        pMessageMap[i].secondParamType)) {
              typedef void (Panel::*MessageFunc_ConstCharPtrConstCharPtr_t)(
                  const char *, const char *);
              (this->*((MessageFunc_ConstCharPtrConstCharPtr_t)pMessageMap[i]
                           .func))(
                  params->GetString(pMessageMap[i].firstParamName),
                  params->GetString(pMessageMap[i].secondParamName));
            } else if ((DATATYPE_INT == pMessageMap[i].firstParamType) &&
                       (DATATYPE_CONSTCHARPTR ==
                        pMessageMap[i].secondParamType)) {
              typedef void (Panel::*MessageFunc_IntConstCharPtr_t)(
                  int, const char *);
              (this->*((MessageFunc_IntConstCharPtr_t)pMessageMap[i].func))(
                  params->GetInt(pMessageMap[i].firstParamName),
                  params->GetString(pMessageMap[i].secondParamName));
            } else if ((DATATYPE_PTR == pMessageMap[i].firstParamType) &&
                       (DATATYPE_CONSTCHARPTR ==
              ...
              typedef void (Panel::*MessageFunc_PtrConstCharPtr_t)(
                  void *, const char *);
              (this->*((MessageFunc_PtrConstCharPtr_t)pMessageMap[i].func))(
                  params->GetPtr(pMessageMap[i].firstParamName),
                  params->GetString(pMessageMap[i].secondParamName));
            } else if ((DATATYPE_PTR == pMessageMap[i].firstParamType) &&
              ...
              (this->*((MessageFunc_PtrConstCharPtr_t)pMessageMap[i].func))(
                  params->GetPtr(pMessageMap[i].firstParamName),
                  params->GetWString(pMessageMap[i].secondParamName));
            } else {
              // the message isn't handled
              ivgui()->DPrintf(
                  "Message '%s', sent to '%s', has invalid parameter types\n",
          ...
          case 1:
            switch (pMessageMap[i].firstParamType) {
              case DATATYPE_BOOL:
                ...
                (this->*((MessageFunc_Bool_t)pMessageMap[i].func))(
                    (bool)params->GetInt(pMessageMap[i].firstParamName));
                break;

              case DATATYPE_CONSTCHARPTR:
                typedef void (Panel::*MessageFunc_ConstCharPtr_t)(const char *);
                (this->*((MessageFunc_ConstCharPtr_t)pMessageMap[i].func))(
                    (const char *)params->GetString(
              ...
              case DATATYPE_CONSTWCHARPTR:
                typedef void (Panel::*MessageFunc_ConstCharPtr_t)(const char *);
                (this->*((MessageFunc_ConstCharPtr_t)pMessageMap[i].func))(
                    (const char *)params->GetWString(
                        pMessageMap[i].firstParamName));
                break;

              case DATATYPE_INT:
                typedef void (Panel::*MessageFunc_Int_t)(int);
                (this->*((MessageFunc_Int_t)pMessageMap[i].func))(
                    params->GetInt(pMessageMap[i].firstParamName));
                break;

              case DATATYPE_FLOAT:
                typedef void (Panel::*MessageFunc_Float_t)(float);
                (this->*((MessageFunc_Float_t)pMessageMap[i].func))(
                    params->GetFloat(pMessageMap[i].firstParamName));
                break;

              case DATATYPE_PTR:
                typedef void (Panel::*MessageFunc_Ptr_t)(void *);
                (this->*((MessageFunc_Ptr_t)pMessageMap[i].func))(
                    (void *)params->GetPtr(pMessageMap[i].firstParamName));
                break;

              case DATATYPE_KEYVALUES:
                typedef void (Panel::*MessageFunc_KeyValues_t)(KeyValues *);
                if (pMessageMap[i].firstParamName) {
                  (this->*((MessageFunc_KeyValues_t)pMessageMap[i].func))(
                      (KeyValues *)params->GetPtr(
                          pMessageMap[i].firstParamName));
                } else {
                  (this->*((MessageFunc_KeyValues_t)pMessageMap[i].func))(
                      params);
              ...
              default:
                // the message isn't handled
                ivgui()->DPrintf(
                    "Message '%s', sent to '%s', has an invalid parameter "
                    "type\n",
                    params->GetName(), GetName());
                break;
            }
...
//			Returns a pointer to a valid panel, NULL if the panel
// has  been deleted
//-----------------------------------------------------------------------------
Panel *PHandle::Get() {
...
//-----------------------------------------------------------------------------
// Purpose: Returns a handle to a valid panel, NULL if the panel has been
// deleted
//-----------------------------------------------------------------------------
...
class CProportionalFloatProperty
    : public vgui::IPanelAnimationPropertyConverter {
 public:
  virtual void GetData(Panel *panel, KeyValues *kv,
                       PanelAnimationMapEntry *entry) {
    void *data = (void *)((*entry->m_pfnLookup)(panel));
 ...
 public:
  ...
  virtual void InitFromDefault(Panel *panel, PanelAnimationMapEntry *entry) {
    ...
    if (scheme) {
      void *data = (void *)((*entry->m_pfnLookup)(panel));
      *(Color *)data =
          scheme->GetColor(entry->defaultvalue(), Color(0, 0, 0, 0));
    }
  }
};
 ...
 public:
  ...
  virtual void InitFromDefault(Panel *panel, PanelAnimationMapEntry *entry) {
    ...
    if (scheme) {
      void *data = (void *)((*entry->m_pfnLookup)(panel));
      *(HFont *)data =
          scheme->GetFont(entry->defaultvalue(), panel->IsProportional());
    }
  }
};
...
class CTextureIdProperty : public vgui::IPanelAnimationPropertyConverter {
 public:
  virtual void GetData(Panel *panel, KeyValues *kv,
    ...
    if (currentId != -1 && surface()->DrawGetTextureFile(currentId, texturename,
                                                         sizeof(texturename))) {
...
static CUtlDict<IPanelAnimationPropertyConverter *, int>
...
static IPanelAnimationPropertyConverter *FindConverter(char const *typeName) {
  ...
  if (lookup == g_AnimationPropertyConverters.InvalidIndex()) return NULL;

  IPanelAnimationPropertyConverter *converter =
      g_AnimationPropertyConverters[lookup];
  return converter;
}

void Panel::AddPropertyConverter(char const *typeName,
                                 IPanelAnimationPropertyConverter *converter) {
  int lookup = g_AnimationPropertyConverters.Find(typeName);
...
PanelAnimationMapEntry *Panel::FindPanelAnimationEntry(char const *scriptname,
...
void Panel::InternalApplySettings(PanelAnimationMap *map,
  ...
  for (kv = inResourceData->GetFirstSubKey(); kv; kv = kv->GetNextKey()) {
    ...
    PanelAnimationMapEntry *entry =
        FindPanelAnimationEntry(varname, GetAnimMap());
    if (entry) {
      // Set value to value from script
      IPanelAnimationPropertyConverter *converter =
          FindConverter(entry->type());
      if (converter) {
...
//-----------------------------------------------------------------------------
void Panel::DrawBox(int x, int y, int wide, int tall, Color color,
                    float normalizedAlpha, bool hollow /*=false*/) {
  if (m_nBgTextureId1 == -1 || m_nBgTextureId2 == -1 || m_nBgTextureId3 == -1 ||
      m_nBgTextureId4 == -1) {
    return;
  }
  ...
  surface()->DrawFilledRect(x + cornerWide, y, x + wide - cornerWide,
                            y + cornerTall);
  if (!hollow) {
    surface()->DrawFilledRect(x, y + cornerTall, x + wide,
                              y + tall - cornerTall);
  } else {
    surface()->DrawFilledRect(x, y + cornerTall, x + cornerWide,
                              y + tall - cornerTall);
    surface()->DrawFilledRect(x + wide - cornerWide, y + cornerTall, x + wide,
                              y + tall - cornerTall);
  }
  surface()->DrawFilledRect(x + cornerWide, y + tall - cornerTall,
  ...
  surface()->DrawTexturedRect(x + wide - cornerWide, y, x + wide,
                              y + cornerTall);
  surface()->DrawSetTexture(m_nBgTextureId3);
  surface()->DrawTexturedRect(x + wide - cornerWide, y + tall - cornerTall,
                              x + wide, y + tall);
  surface()->DrawSetTexture(m_nBgTextureId4);
  surface()->DrawTexturedRect(x + 0, y + tall - cornerTall, x + cornerWide,
                              y + tall);
}

void Panel::DrawBoxFade(int x, int y, int wide, int tall, Color color,
                        float normalizedAlpha, unsigned int alpha0,
                        unsigned int alpha1, bool bHorizontal,
                        bool hollow /*=false*/) {
  if (m_nBgTextureId1 == -1 || m_nBgTextureId2 == -1 || m_nBgTextureId3 == -1 ||
      m_nBgTextureId4 == -1 || surface()->DrawGetAlphaMultiplier() == 0) {
    return;
  }
  ...
  if (!bHorizontal) {
    ...
    surface()->DrawFilledRectFade(x + cornerWide, y, x + wide - cornerWide,
                                  y + cornerTall, alpha0, alpha0, bHorizontal);
    if (!hollow) {
      surface()->DrawFilledRectFade(x, y + cornerTall, x + wide,
                                    y + tall - cornerTall, alpha0, alpha1,
                                    bHorizontal);
    } else {
      surface()->DrawFilledRectFade(x, y + cornerTall, x + cornerWide,
                                    y + tall - cornerTall, alpha0, alpha1,
                                    bHorizontal);
      surface()->DrawFilledRectFade(x + wide - cornerWide, y + cornerTall,
                                    x + wide, y + tall - cornerTall, alpha0,
                                    alpha1, bHorizontal);
    }
    surface()->DrawFilledRectFade(x + cornerWide, y + tall - cornerTall,
                                  x + wide - cornerWide, y + tall, alpha1,
                                  alpha1, bHorizontal);
  } else {
    // draw the background in the areas not occupied by the corners
    // draw it in three horizontal strips
    surface()->DrawSetColor(color);
    surface()->DrawFilledRectFade(x, y + cornerTall, x + cornerWide,
                                  y + tall - cornerTall, alpha0, alpha0,
                                  bHorizontal);
    if (!hollow) {
      surface()->DrawFilledRectFade(x + cornerWide, y, x + wide - cornerWide,
      ...
      // + wide, y + tall - cornerTall, alpha0, alpha1, bHorizontal);
    }
    surface()->DrawFilledRectFade(x + wide - cornerWide, y + cornerTall,
                                  x + wide, y + tall - cornerTall, alpha1,
                                  alpha1, bHorizontal);
  }
...
//			normalizedAlpha -
//-----------------------------------------------------------------------------
void Panel::DrawHollowBox(int x, int y, int wide, int tall, Color color,
...
// Purpose: draws a selection box
//-----------------------------------------------------------------------------
void Panel::DrawTexturedBox(int x, int y, int wide, int tall, Color color,
                            float normalizedAlpha) {
  if (m_nBgTextureId1 == -1) return;
...
//-----------------------------------------------------------------------------
// Purpose: Marks this panel as draggable (note that children will chain to
// their parents to see if any parent is draggable) Input  : enabled -
//-----------------------------------------------------------------------------
void Panel::SetDragEnabled(bool enabled) {
#if defined(VGUI_USEDRAGDROP)
  // If turning it off, quit dragging if mid-drag
  if (!enabled && m_pDragDrop->m_bDragging) {
    OnFinishDragging(false, (MouseCode)-1);
  }
...
// functionality if you don't want to chain up from a child panel to the best
//  draggable parent.
...
//-----------------------------------------------------------------------------
// Purpose: Marks this panel as droppable ( note that children will chain to
// their parents to see if any parent is droppable) Input  : enabled -
//-----------------------------------------------------------------------------
void Panel::SetDropEnabled(bool enabled,
                           float flHoverContextTime /* = 0.0f */) {
#if defined(VGUI_USEDRAGDROP)
...
void Panel::OnStartDragging() {
  ...
  input()->GetCursorPos(m_pDragDrop->m_nStartPos[0],
                        m_pDragDrop->m_nStartPos[1]);
...
//-----------------------------------------------------------------------------
// Purpose: Called if drag drop is started but not dropped on top of droppable
// panel... Input  :  -
//-----------------------------------------------------------------------------
...
// Input  :  -
//-----------------------------------------------------------------------------
void Panel::OnFinishDragging(bool mousereleased, MouseCode code,
                             bool abort /*= false*/) {
#if defined(VGUI_USEDRAGDROP)
  ...
  if (m_pDragDrop->m_bDragStarted) {
    ...
    if (mousereleased && m_pDragDrop->m_hCurrentDrop != NULL &&
  ...
  // Copy data ptrs out of data because OnPanelDropped might cause this panel to
  // be deleted and our this ptr will be hosed...
...
bool Panel::CanStartDragging(int startx, int starty, int mx, int my) {
  ...
  if (IsStartDragWhenMouseExitsPanel()) {
    ...
    if (mx > GetWide() || my > GetTall()) return true;
...
bool IsSelfDroppable(CUtlVector<KeyValues *> &dragData) {
  if (dragData.Count() == 0) return false;

  KeyValues *pKeyValues(dragData[0]);
  if (!pKeyValues) return false;
...
void Panel::OnContinueDragging() {
  ...
  if (!m_pDragDrop->m_bDragStarted) {
    if (CanStartDragging(m_pDragDrop->m_nStartPos[0],
                         m_pDragDrop->m_nStartPos[1], x, y)) {
  ...
  if (m_pDragDrop->m_hCurrentDrop.Get() != oldDrop.Get()) {
    ...
    if (m_pDragDrop->m_hCurrentDrop.Get()) {
      m_pDragDrop->m_hCurrentDrop->OnPanelEnteredDroppablePanel(
          m_pDragDrop->m_DragData);
      m_pDragDrop->m_hCurrentDrop->OnDropContextHoverHide(
          m_pDragDrop->m_DragData);
...
void Panel::OnCreateDragData(KeyValues *msg) {
  ...
  // "x", "y" - drop coordinates relative to this window (the window being
  // dropped upon)
}

// Called if m_flHoverContextTime was non-zero, allows droppee to preview the
// drop data and show an appropriate menu
...
VPANEL CDragDropHelperPanel::IsWithinTraverse(int x, int y,
                                              bool traversePopups) {
  return (VPANEL)0;
}
...
void CDragDropHelperPanel::AddPanel(Panel *current) {
  ...
  int c = m_PaintList.Count();
  for (int i = 0; i < c; ++i) {
    if (m_PaintList[i].m_hPanel.Get() == current) return;
  }
...
//			check -
//-----------------------------------------------------------------------------
void Panel::FindDropTargetPanel_R(CUtlVector<VPANEL> &panelList, int x, int y,
                                  VPANEL check) {
#if defined(VGUI_USEDRAGDROP)
  if (!ipanel()->IsFullyVisible(check)) return;

  if (::ShouldHandleInputMessage(check) &&
      ipanel()->IsWithinTraverse(check, x, y, false)) {
...
//-----------------------------------------------------------------------------
// Purpose: Mouse is on draggable panel and has started moving, but is not over
// a droppable panel yet Input  :  -
//-----------------------------------------------------------------------------
...
// Input  : *dragPanel -
//-----------------------------------------------------------------------------
void Panel::OnDroppablePanelPaint(CUtlVector<KeyValues *> &msglist,
...
void Panel::DragDropStartDragging() {
  ...
  if (g_DragDropCapture.Get()) {
    if (HasParent(g_DragDropCapture.Get()->GetVPanel())) return;
...
void Panel::FillRectSkippingPanel(Color clr, int x, int y, int w, int h,
                                  Panel *skipPanel) {
  int sx = 0, sy = 0, sw, sh;
...
class CPanelMessageMapDictionary {
 public:
  CPanelMessageMapDictionary()
      : m_PanelMessageMapPool(
            sizeof(PanelMessageMap), 32, CMemoryPool::GROW_FAST,
...
PanelMessageMap *CPanelMessageMapDictionary::FindOrAddPanelMessageMap(
  ...
  entry.map = new (m_PanelMessageMapPool.Alloc(sizeof(PanelMessageMap)))
      PanelMessageMap;
  Construct(entry.map);
...
class CPanelKeyBindingMapDictionary {
 public:
  CPanelKeyBindingMapDictionary()
      : m_PanelKeyBindingMapPool(
            sizeof(PanelKeyBindingMap), 32, CMemoryPool::GROW_FAST,
...
PanelKeyBindingMap *CPanelKeyBindingMapDictionary::FindOrAddPanelKeyBindingMap(
  ...
  // use the alloc in place method of new
  entry.map = new (m_PanelKeyBindingMapPool.Alloc(sizeof(PanelKeyBindingMap)))
      PanelKeyBindingMap;
  Construct(entry.map);
  m_MessageMaps.Insert(StripNamespace(className), entry);
  return entry.map;
