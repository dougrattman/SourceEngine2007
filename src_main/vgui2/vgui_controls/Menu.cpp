...
class vgui::MenuSeparator : public Panel {
 public:
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    SetFgColor(
        pScheme->GetColor("Menu.SeparatorColor", Color(142, 142, 142, 255)));
...
Menu::Menu(Panel *parent, const char *panelName) : Panel(parent, panelName) {
  ...
  if (IsProportional()) {
    m_iMenuItemHeight = scheme()->GetProportionalScaledValueEx(
        GetScheme(), DEFAULT_MENU_ITEM_HEIGHT);
  } else {
...
void Menu::DeleteAllItems() {
  FOR_EACH_LL(m_MenuItems, i) { m_MenuItems[i]->MarkForDeletion(); }
...
// Purpose: Add a menu item to the menu.
// Input  : *item - MenuItem
//			*command -  Command text to be sent when menu item is
// selected 			*target - Target panel of the command
// *userData - any user data associated with this menu item
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItemCharCommand(MenuItem *item, const char *command,
...
//			*itemText - Name of item text that will appear in the
// manu. 			*message - pointer to the message to send when the item
// is selected 			*target - Target panel of the command
//          *cascadeMenu - if the menu item opens a cascading menu, this is a
//							ptr to the menu that opens
//on selecting the item
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItemKeyValuesCommand(MenuItem *item, KeyValues *message,
                                      Panel *target,
...
// Input  : *itemName - Name of item
//			*itemText - Name of item text that will appear in the
// manu. 			*command -  Command text to be sent when menu item is
// selected 			*target - Target panel of the command
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem(const char *itemName, const char *itemText,
...
int Menu::AddMenuItem(const char *itemName, const wchar_t *wszItemText,
                      const char *command, Panel *target,
                      const KeyValues *userData) {
...
//						This will also be used as the name of
//the
// menu item panel. 			*command -  Command text to be sent when menu
// item is selected 			*target - Target panel of the command
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem(const char *itemText, const char *command, Panel *target,
                      const KeyValues *userData) {
...
//						This will also be used as the name of
//the
// menu item panel. 			*message - pointer to the message to send
// when the item is selected 			*target - Target panel of the
// command
//          *cascadeMenu - if the menu item opens a cascading menu, this is a
//							ptr to the menu that opens
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem(const char *itemText, KeyValues *message, Panel *target,
                      const KeyValues *userData) {
...
// Purpose: Add a menu item to the menu.
// Input  : *itemText - Name of item text that will appear in the manu.
//						This will also be the text of the
//command
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem(const char *itemText, Panel *target,
                      const KeyValues *userData) {
...
// selected 			*target - Target panel of the command
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem(const char *itemName, const char *itemText,
                               const char *command, Panel *target,
                               const KeyValues *userData) {
...
int Menu::AddCheckableMenuItem(const char *itemName, const wchar_t *wszItemText,
                               const char *command, Panel *target,
                               const KeyValues *userData) {
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem(const char *itemText, const char *command,
                               Panel *target, const KeyValues *userData) {
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem(const char *itemText, KeyValues *message,
                               Panel *target, const KeyValues *userData) {
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem(const char *itemText, Panel *target,
                               const KeyValues *userData) {
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem(const char *itemName, const char *itemText,
                               const char *command, Panel *target,
                               Menu *cascadeMenu, const KeyValues *userData) {
  MenuItem *item = new MenuItem(this, itemName, itemText, cascadeMenu);
  return AddMenuItemCharCommand(item, command, target, userData);
}

int Menu::AddCascadingMenuItem(const char *itemName, const wchar_t *wszItemText,
                               const char *command, Panel *target,
                               Menu *cascadeMenu, const KeyValues *userData) {
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem(const char *itemText, const char *command,
                               Panel *target, Menu *cascadeMenu,
                               const KeyValues *userData) {
  return AddCascadingMenuItem(itemText, itemText, command, target, cascadeMenu,
                              userData);
}
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem(const char *itemText, KeyValues *message,
                               Panel *target, Menu *cascadeMenu,
                               const KeyValues *userData) {
  return AddCascadingMenuItem(itemText, itemText, message, target, cascadeMenu,
                              userData);
}
...
//on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem(const char *itemText, Panel *target,
                               Menu *cascadeMenu, const KeyValues *userData) {
  return AddCascadingMenuItem(itemText, itemText, target, cascadeMenu,
                              userData);
}
...
//			*message - pointer to the message to send when the item
//is selected
//-----------------------------------------------------------------------------
void Menu::UpdateMenuItem(int itemID, const char *itemText, KeyValues *message,
                          const KeyValues *userData) {
...
// Purpose: Sets the values of a menu item at the specified index
//-----------------------------------------------------------------------------
void Menu::UpdateMenuItem(int itemID, const wchar_t *wszItemText,
                          KeyValues *message, const KeyValues *userData) {
  Assert(m_MenuItems.IsValidIndex(itemID));
  if (m_MenuItems.IsValidIndex(itemID)) {
    MenuItem *menuItem = dynamic_cast<MenuItem *>(m_MenuItems[itemID]);
    // make sure its enabled since disabled items get highlighted.
...
void Menu::SetFixedWidth(int width) {
  // the padding makes it so the menu has the label padding on each side of the
  // menu. makes the menu items look centered.
  m_iFixedWidth = width;
  InvalidateLayout(false);
}
...
int Menu::CountVisibleItems() {
  ...
  for (int i = 0; i < c; ++i) {
    if (m_MenuItems[m_SortedItems[i]]->IsVisible()) ++count;
  }
  return count;
}
...
void Menu::PositionRelativeToPanel(Panel *relative, MenuDirection_e direction,
...
int Menu::ComputeFullMenuHeightWithInsets() {
  ...
  int i;
  for (i = 0; i < m_SortedItems.Count();
       i++)  // use sortedItems instead of MenuItems due to SetPos()
  {
...
void Menu::PerformLayout() {
  ...
  if (m_iNumVisibleLines > 0 && maxVisibleItems > m_iNumVisibleLines) {
    bNeedScrollbar = true;
  ...
  if (bNeedScrollbar) {
    ...
    MakeItemsVisibleInScrollRange(m_iNumVisibleLines,
                                  std::min(fullHeightWouldRequire, workTall));
  } else {
    RemoveScrollBar();
  ...
  int totalTall = itop + ibottom;
  int i;
  for (i = 0; i < m_VisibleSortedItems.Count();
       i++)  // use sortedItems instead of MenuItems due to SetPos()
  {
    ...
    // this will make all the menuitems line up in a column with space for the
    // checks to the left.
    if ((!child->IsCheckable()) && (m_iCheckImageWidth > 0)) {
      ...
      child->SetTextInset(
          0, 0);  // TODO: for some reason I can't comment this out.
    }
  ...
  FOR_EACH_LL(m_MenuItems, j) {
    m_MenuItems[j]
        ->InvalidateLayout();  // cause each menu item to redo its apply
                               // settings now we have sized ourselves
  }
...
//-----------------------------------------------------------------------------
// Purpose: Figure out how wide the menu should be if the menu is not fixed
// width
//-----------------------------------------------------------------------------
void Menu::CalculateWidth() {
  if (!_recalculateWidth) return;
...
//			Also size the menu items with cascading menus so the
//arrow fits in there.
//-----------------------------------------------------------------------------
void Menu::SizeMenuItems() {
  ...
  FOR_EACH_LL(m_MenuItems, i) {
    ...
    if (child) {
      // labels do thier own sizing. this will size the label to the width of
      // the menu, this will put the cascading menu arrow on the right side
      // automatically.
...
//-----------------------------------------------------------------------------
void Menu::MakeItemsVisibleInScrollRange(int maxVisibleItems,
                                         int nNumPixelsAvailable) {
  // Detach all items from tree
  int i;
  FOR_EACH_LL(m_MenuItems, item) { m_MenuItems[item]->SetBounds(0, 0, 0, 0); }
  ...
  Assert(startItem >= 0);
  do {
    if (startItem >= m_SortedItems.Count()) break;
    ...
    if (maxVisibleItems > 0) {
      if (m_VisibleSortedItems.Count() >= maxVisibleItems) break;
    }
...
void Menu::Paint() {
  if (m_pScroller->IsVisible()) {
    ...
    if (IsProportional()) {
      surface()->DrawFilledRect(wide - m_pScroller->GetWide(), -1,
                                wide - m_pScroller->GetWide() + 1, tall);
    } else {
      surface()->DrawFilledRect(wide - m_pScroller->GetWide(), -1,
                                wide - m_pScroller->GetWide() + 1, tall);
    }
...
//-----------------------------------------------------------------------------
// Purpose:	sets the max number of items visible (scrollbar appears with
// more) Input  : numItems -
//-----------------------------------------------------------------------------
...
MenuItem *Menu::GetMenuItem(int itemID) {
  if (!m_MenuItems.IsValidIndex(itemID)) return NULL;
...
void Menu::CloseOtherMenus(MenuItem *item) {
  FOR_EACH_LL(m_MenuItems, i) {
    if (m_MenuItems[i] == item) continue;
...
void Menu::OnKeyCodeTyped(KeyCode code) {
  ...
  switch (code) {
    ...
    // for now left and right arrows just open or close submenus if they are
    // there.
    case KEY_RIGHT:
...
void Menu::OnKeyTyped(wchar_t unichar) {
  //
  // NOTE - if hotkeys are ever enabled you need to work out a way to
  // differentiate between combo box menus (which can't have hot keys) and
  // system style menus (which do have hot keys).
  //
  //
  /*	if (unichar)
          {
                  // iterate the menu items looking for one with the matching
     hotkey FOR_EACH_LL( m_MenuItems, i )
                  {
                          ...
                          if (panel->IsVisible())
                          {
                                  ...
                                  if (hot)
                                  {
                                          // post a message to the menuitem
     telling it it's hotkey was pressed PostMessage(hot, new
     KeyValues("Hotkey")); return;
                                  }
                                  // if the menuitem is a cascading menuitem and
     it is open, check its hotkeys too Menu *cascadingMenu = panel->GetMenu();
                                  if (cascadingMenu &&
     cascadingMenu->IsVisible())
                                  {
...
void Menu::OnMouseWheeled(int delta) {
  if (!m_pScroller->IsVisible()) return;
...
void Menu::OnKillFocus() {
  ...
  if (!input()->GetFocus() ||
    ...
    // if we don't accept keyboard input, then we have to ignore the killfocus
    // if it's not actually being stolen
    ...
    // if the panel that is getting focus is the parent menu, don't close this
    // menu.
  ...
  bool IsWithinMenuOrRelative(Panel *panel, int x, int y) {
    ...
    if (panel->GetParent()) {
      ...
      if (topMost) {
        ...
        /*
        // NOTE:  this check used to not cast to MenuButton, but it seems wrong
        to me
        //  since if the mouse is over another child of the parent panel to the
        menu then
...
//-----------------------------------------------------------------------------
// Purpose: Static method called on mouse released to see if Menu objects should
// be aborted Input  : *other -
//			code -
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: returns a pointer to a MenuItem that is this menus parent, if it has
// one
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Activate the n'th item in the menu list, as if that menu item had
...
int Menu::GetMenuID(int index) {
  if (!m_SortedItems.IsValidIndex(index)) return m_MenuItems.InvalidIndex();
...
void Menu::SetItemEnabled(int itemID, bool state) {
  if (!m_MenuItems.IsValidIndex(itemID)) return;
...
//			also make items visible or invisible in the list as
// appropriate
//-----------------------------------------------------------------------------
...
void Menu::OnKeyCodePressed(KeyCode code) {
  ...
  if (GetVParent()) {
    ivgui()->PostMessage(GetVParent(), new KeyValues("KeyModeSet"),
                         GetVPanel());
  }
}
...
void Menu::OnCursorEnteredMenuItem(int VPanel) {
  ...
  if (m_iInputMode == MOUSE) {
    MenuItem *item =
        static_cast<MenuItem *>(ipanel()->GetPanel(menuItem, GetModuleName()));
    // arm the menu
    item->ArmItem();
...
//			pchName -		Our name (typically a member var in
//our container)
//-----------------------------------------------------------------------------
