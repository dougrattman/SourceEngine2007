...
CTreeViewListControl::CTreeViewListControl(vgui::Panel *pParent,
                                           const char *pName)
...
vgui::TreeView *CTreeViewListControl::GetTree() { return m_pTree; }

int CTreeViewListControl::GetTitleBarHeight() { return m_TitleBarHeight; }

void CTreeViewListControl::SetTitleBarInfo(vgui::HFont hFont,
                                           int titleBarHeight) {
...
void CTreeViewListControl::SetBorderColor(Color clr) { m_BorderColor = clr; }
...
int CTreeViewListControl::GetNumColumns() const { return m_Columns.Count(); }

void CTreeViewListControl::SetColumnInfo(int iColumn, const char *pTitle,
                                         int width, int ciFlags) {
...
int CTreeViewListControl::GetNumRows() { return m_Rows.Count(); }
...
void CTreeViewListControl::GetGridElementBounds(int iColumn, int iRow,
                                                int &left, int &top, int &right,
                                                int &bottom) {
  left = m_Columns[iColumn].m_Left;
  right = m_Columns[iColumn].m_Right;

  // vgui doesn't seem to be drawing things exactly right. Like it you draw a
  // line at (0,0) to (100,0), then a rectangle from (1,1) to (100,100), it'll
  // overwrite the line at the top.
  int treeTopBorder = 0;
...
void CTreeViewListControl::RecalculateRows() {
  ...
  if (!m_pTree || m_pTree->GetRootItemIndex() == -1) return;
...
void CTreeViewListControl::RecalculateRows_R(int index) {
  ...
  if (!m_pTree->IsItemExpanded(index)) return;
...
int CTreeViewListControl::GetScrollBarSize() { return 0; }
