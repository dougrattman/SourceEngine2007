...
// Purpose: Destructor
//-----------------------------------------------------------------------------
ProgressBar::~ProgressBar() { delete[] m_pszDialogVar; }
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
float ProgressBar::GetProgress() { return _progress; }
...
//-----------------------------------------------------------------------------
bool ProgressBar::ConstructTimeRemainingString(
    wchar_t *output, int outputBufferSizeInBytes, float startTime,
    float currentTime, float currentProgress, float lastProgressUpdateTime,
  ...
  if (lastProgressUpdateTime < currentTime) {
    ...
    float extrapolatedTotalTime =
        (currentTime - startTime) / extrapolatedProgress;
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
void ProgressBar::SetBarInset(int pixels) { m_iBarInset = pixels; }
...
const char *ProgressBar::GetDescription(void) {
  ...
  _snprintf_s(buf, sizeof(buf), "%s, string progress, string variable",
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
ContinuousProgressBar::ContinuousProgressBar(Panel *parent,
...
void ContinuousProgressBar::Paint() {
  ...
  switch (m_iProgressDirection) {
    ...
    case PROGRESS_WEST:
      surface()->DrawFilledRect(x + (int)(wide * (1.0f - _progress)), y,
                                x + wide, y + tall);
      break;

    case PROGRESS_NORTH:
      surface()->DrawFilledRect(x, y + (int)(tall * (1.0f - _progress)),
                                x + wide, y + tall);
      break;
