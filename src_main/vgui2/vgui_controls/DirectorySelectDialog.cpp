...
class CreateDirectoryDialog : public Frame {
 ...
 public:
  CreateDirectoryDialog(Panel *parent, const char *defaultCreateDirName)
    ...
    // If some other window was hogging the input focus, then we have to hog it
    // or else we'll never get input.
    m_PrevAppFocusPanel = vgui::input()->GetAppModalSurface();
    if (m_PrevAppFocusPanel) vgui::input()->SetAppModalSurface(GetVPanel());
  }
  ...
  virtual void OnCommand(const char *command) {
    if (!_stricmp(command, "OK")) {
      PostActionSignal(new KeyValues("CreateDirectory", "dir",
                                     GetControlString("NameEntry")));
      Close();
    } else {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
DirectorySelectDialog::DirectorySelectDialog(vgui::Panel *parent,
                                             const char *title)
    : Frame(parent, NULL) {
  SetTitle(title, true);
  SetSize(320, 360);
...
inline const char *MoveToNextSubDir(const char *pStart, int *nCount) {
  ...
  // Move past the current block of text until we've hit the next path seperator
  // (or end)
...
//-----------------------------------------------------------------------------
// Purpose: Walk through our directory structure given a path as our guide,
// while expanding
//			and populating the nodes of the tree view to match
// Input  : *path - path (with drive letter) to show
//-----------------------------------------------------------------------------
void DirectorySelectDialog::ExpandTreeToPath(
    const char *lpszPath, bool bSelectFinalDirectory /*= true*/) {
  ...
  if (workPath[0] == '\0' ||
      DoesDirectoryHaveSubdirectories(m_szCurrentDrive, "") == false) {
    // Failing, start in C:
    SetStartDirectory("C:\\");
  }
  ...
  // While there are subdirectory names present, expand and populate the tree
  // with their subdirectories
  while (lpszSubDirName[0] != '\0') {
    // Move our string pointer forward while keeping where our last subdir
    // started off
    ...
    for (int i = 0; i < m_pDirTree->GetNumChildren(nItemIndex); i++) {
      ...
      if (Q_stricmp(pValues->GetString("Text"), subDirName) == 0) {
        ...
        // Get the full path (starting from the drive letter) up to our current
        // subdir
  ...
  if (bSelectFinalDirectory && m_pDirTree->IsItemIDValid(nItemIndex)) {
    // If we don't call this once before selecting an item, the tree will not be
    // properly expanded before it calculates how to show the selected item in
    // the view
    PerformLayout();
...
//-----------------------------------------------------------------------------
// Purpose: sets what name should show up by default in the create directory
// dialog
//-----------------------------------------------------------------------------
void DirectorySelectDialog::SetDefaultCreateDirectoryName(
    const char *defaultCreateDirName) {
  strcpy_s(m_szDefaultCreateDirName, defaultCreateDirName);
}
...
void DirectorySelectDialog::BuildDirTree() {
  ...
  // add in a root
  int rootIndex =
      m_pDirTree->AddItem(new KeyValues("root", "Text", m_szCurrentDrive), -1);
...
//-----------------------------------------------------------------------------
void DirectorySelectDialog::ExpandTreeNode(const char *path,
                                           int parentNodeIndex) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool DirectorySelectDialog::DoesDirectoryHaveSubdirectories(const char *path,
                                                            const char *dir) {
  char searchString[512];
  sprintf_s(searchString, "%s%s\\*.*", path, dir);
...
//-----------------------------------------------------------------------------
void DirectorySelectDialog::GenerateFullPathForNode(int nodeIndex, char *path,
                                                    int pathBufferSize) {
  // get all the nodes
  ...
  FOR_EACH_LL(nodes, i) {
    ...
    strcat_s(path, pathBufferSize, kv->GetString("Text"));
...
void DirectorySelectDialog::OnCreateDirectory(const char *dir) {
  ...
  if (m_pDirTree->IsItemIDValid(selectedIndex)) {
    ...
    if (_mkdir(fullPath) == 0) {
      ...
      // print error message
      MessageBox *box = new MessageBox("#vgui_CreateDirectoryFail_Title",
  ...
  if (!_stricmp(command, "Cancel")) {
    ...
    if (m_pDirTree->IsItemIDValid(selectedIndex)) {
      CreateDirectoryDialog *dlg =
          new CreateDirectoryDialog(this, m_szDefaultCreateDirName);
