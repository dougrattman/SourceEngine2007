...
ColumnButton::ColumnButton(vgui::Panel *parent, const char *name,
                           const char *text)
...
void Dragger::OnMouseDoublePressed(MouseCode code) {
  if (m_bMovable) {
    ...
    PostMessage(GetParent(),
                new KeyValues("ResizeColumnToContents", "column", m_iDragger));
  }
}
...
void Dragger::SetMovable(bool state) {
  ...
  if (IsVisible()) {
    if (state) {
      ...
      // if parent windows Start getting fancy cursors we should probably
      // retrive a parent cursor and set it to that
      SetCursor(dc_sizewe);
    } else {
      SetCursor(dc_arrow);
    }
...
//-----------------------------------------------------------------------------
static int __cdecl DefaultSortFunc(ListPanel *pPanel,
  ...
  {
    const ImagePanel *s1 = (ImagePanel *)p1->kv->GetPtr(col, nullptr);
    const ImagePanel *s2 = (ImagePanel *)p2->kv->GetPtr(col, nullptr);
...
//-----------------------------------------------------------------------------
static int __cdecl FastSortFunc(ListPanel *pPanel, const ListPanelItem &item1,
                                const ListPanelItem &item2) {
...
//-----------------------------------------------------------------------------
bool ListPanel::RBTreeLessFunc(vgui::ListPanel::IndexItem_t &item1,
                               vgui::ListPanel::IndexItem_t &item2) {
  int result =
      s_pSortFunc(s_pCurrentSortingListPanel, *item1.dataItem, *item2.dataItem);
  if (result == 0) {
...
ListPanel::~ListPanel() {
  ...
  // free column headers
  unsigned char i;
  for (i = m_ColumnsData.Head(); i != m_ColumnsData.InvalidIndex();
       i = m_ColumnsData.Next(i)) {
...
// Purpose:
//-----------------------------------------------------------------------------
void ListPanel::SetImageList(ImageList *imageList,
                             bool deleteImageListWhenDone) {
  // get rid of existing list image if there's one and we're supposed to get rid
  // of it
...
// Purpose:
//-----------------------------------------------------------------------------
void ListPanel::SetColumnHeaderHeight(int height) { m_iHeaderHeight = height; }

//-----------------------------------------------------------------------------
// Purpose: adds a column header.
//			this->FindChildByName(columnHeaderName) can be used to
// retrieve a pointer to a header panel by name
//
// if minWidth and maxWidth are BOTH NOTRESIZABLE or RESIZABLE
// the min and max size will be calculated automatically for you with that
// attribute columns are resizable by default if min and max size are specified
// column is resizable
//
// A small note on passing numbers for minWidth and maxWidth,
// If the initial window size is larger than the sum of the original widths of
// the columns, you can wind up with the columns "snapping" to size after the
// first window focus This is because the dxPerBar being calculated in
// PerformLayout() is making resizable bounded headers exceed thier maxWidths at
// the Start. Solution is to either put in support for redistributing the extra
// dx being truncated and therefore added to the last column on window opening,
// which is what causes the snapping. OR to ensure the difference between the
// starting sum of widths is not too much smaller/bigger than the starting
// window size so the starting dx doesn't cause snapping to occur. The easiest
// thing is to simply set it so your column widths add up to the starting size
// of the window on opening.
//
// Another note: Always give bounds for the last column you add or make it not
// resizable.
//
// Columns can have text headers or images for headers (e.g. password icon)
//-----------------------------------------------------------------------------
void ListPanel::AddColumnHeader(int index, const char *columnName,
                                const char *columnText, int width,
                                int columnFlags) {
  if (columnFlags & COLUMN_FIXEDSIZE &&
      !(columnFlags & COLUMN_RESIZEWITHWINDOW)) {
    // for fixed size columns, set the min & max widths to be the same as the
    // initial width
    AddColumnHeader(index, columnName, columnText, width, width, width,
                    columnFlags);
  } else {
    AddColumnHeader(index, columnName, columnText, width, 20, 10000,
                    columnFlags);
  }
}
...
//-----------------------------------------------------------------------------
void ListPanel::AddColumnHeader(int index, const char *columnName,
                                const char *columnText, int width, int minWidth,
                                int maxWidth, int columnFlags) {
  ...
  // put this index on the tail, so all item's m_SortedTreeIndexes have a
  // consistent mapping
  ...
  Button *pButton = SETUP_PANEL(new ColumnButton(
      this, columnName,
      columnText));  // the cell rendering mucks with the button visibility
                     // during the solvetraverse loop,
  ...
  m_ColumnsData[columnDataIndex].m_SortedTree.SetLessFunc(
      (IndexRBTree_t::LessFunc_t)RBTreeLessFunc);

  // go through all the headers and make sure their Command has the right column
  // ID
...
void ListPanel::ResortColumnRBTree(int col) {
  ...
  s_currentSortingColumnTypeIsText =
      column.m_bTypeIsText;  // type of data in the column
  ...
  FOR_EACH_LL(m_DataItems, i) {
    ...
    // if can only be because this is the brand new column, so add it to the
    // SortedTreeIndexes
...
//-----------------------------------------------------------------------------
// Purpose: Resets the "SetSortColumn" command for each column - in case columns
...
void ListPanel::SetColumnHeaderImage(int column, int imageListIndex) {
  ...
  m_ColumnsData[m_CurrentColumns[column]].m_pHeader->SetImageAtIndex(
      0, m_pImageList->GetImage(imageListIndex), 0);
}
...
void ListPanel::SetColumnHeaderTooltip(int column, const char *tooltipText) {
  m_ColumnsData[m_CurrentColumns[column]].m_pHeader->GetTooltip()->SetText(
      tooltipText);
  m_ColumnsData[m_CurrentColumns[column]]
      .m_pHeader->GetTooltip()
...
int ListPanel::GetNumColumnHeaders() const { return m_CurrentColumns.Count(); }
...
void ListPanel::SetColumnSortable(int col, bool sortable) {
  if (sortable) {
    m_ColumnsData[m_CurrentColumns[col]].m_pHeader->SetCommand(
        new KeyValues("SetSortColumn", "column", col));
  } else {
    m_ColumnsData[m_CurrentColumns[col]].m_pHeader->SetCommand(
        (const char *)NULL);
  }
...
void ListPanel::SetColumnVisible(int col, bool visible) {
  ...
  if (column.m_bHidden == bHidden) return;

  if (column.m_bUnhidable) return;
...
void ListPanel::RemoveColumn(int col) {
  if (!m_CurrentColumns.IsValidIndex(col)) return;
...
int ListPanel::AddItem(const KeyValues *item, unsigned int userData,
  ...
  newitem->m_pIcon =
      reinterpret_cast<IImage *>(newitem->kv->GetPtr("iconImage"));
...
void ListPanel::SetUserData(int itemID, unsigned int userData) {
  if (!m_DataItems.IsValidIndex(itemID)) return;
...
int ListPanel::GetItemIDFromUserData(unsigned int userData) {
  FOR_EACH_LL(m_DataItems, itemID) {
    if (m_DataItems[itemID]->userData == userData) return itemID;
  }
  // not found
...
KeyValues *ListPanel::GetItem(int itemID) {
  if (!m_DataItems.IsValidIndex(itemID)) return NULL;
...
int ListPanel::GetItemIDFromRow(int currentRow) {
  if (!m_VisibleItems.IsValidIndex(currentRow)) return -1;

  return m_VisibleItems[currentRow];
}

int ListPanel::FirstItem() const { return m_DataItems.Head(); }

int ListPanel::NextItem(int iItem) const { return m_DataItems.Next(iItem); }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
int ListPanel::InvalidItemID() const { return m_DataItems.InvalidIndex(); }
...
unsigned int ListPanel::GetItemUserData(int itemID) {
  if (!m_DataItems.IsValidIndex(itemID)) return 0;
...
void ListPanel::IndexItem(int itemID) {
  ...
  for (int i = 0; i < m_ColumnsHistory.Count(); i++) {
    ...
    if (m_ColumnsHistory[i] == m_ColumnsData.InvalidIndex()) continue;
    ...
    s_pCurrentSortingColumn =
        column.m_pHeader->GetName();  // name of current column for sorting
    s_currentSortingColumnTypeIsText =
        column.m_bTypeIsText;  // type of data in the column
...
//-----------------------------------------------------------------------------
void ListPanel::DeleteAllItems() { RemoveAll(); }
...
//-----------------------------------------------------------------------------
int ListPanel::GetSelectedItemsCount() { return m_SelectedItems.Count(); }
...
// Purpose:
//-----------------------------------------------------------------------------
int ListPanel::GetSelectedColumn() { return m_iSelectedColumn; }
...
//-----------------------------------------------------------------------------
void ListPanel::GetCellText(int itemID, int col, wchar_t *wbuffer,
                            int bufferSize) {
...
Panel *ListPanel::GetCellRenderer(int itemID, int col) {
  ...
  if (column.m_bTypeIsText) {
    ...
    if (m_SelectedItems.HasElement(itemID) &&
        (!m_bCanSelectIndividualCells || col == m_iSelectedColumn)) {
      selected = true;
      VPANEL focus = input()->GetFocus();
      // if one of the children of the SectionedListPanel has focus, then 'we
      // have focus' if we're selected
      if (HasFocus() || (focus && ipanel()->HasParent(focus, GetVParent()))) {
        m_pLabel->SetBgColor(
            GetSchemeColor("ListPanel.SelectedBgColor", pScheme));
        // selection
      } else {
        m_pLabel->SetBgColor(
            GetSchemeColor("ListPanel.SelectedOutOfFocusBgColor", pScheme));
      }
    ...
    if (col == 0 && listItem->m_bImage && m_pImageList) {
      ...
      if (listItem->m_pIcon) {
        ...
        int imageIndex = selected ? listItem->m_nImageIndexSelected
                                  : listItem->m_nImageIndex;
        if (m_pImageList->IsValidIndex(imageIndex)) {
...
void ListPanel::PerformLayout() {
  if (m_CurrentColumns.Count() == 0) return;
  ...
  int numToResize = 0;
  if (m_iColumnDraggerMoved !=
      -1)  // we're resizing in response to a column dragger
  {
    numToResize = 1;  // only one column will change size, the one we dragged
  } else              // we're resizing in response to a window resize
  {
    for (int i = 0; i < nColumns; i++) {
      if (m_ColumnsData[m_CurrentColumns[i]]
              .m_bResizesWithWindow  // column is resizable in response to
                                     // window
  ...
  if (numToResize == 0) {
    ...
    // if all the minimum widths cannot fit in the space given, then we will
    // shrink ALL columns an equal amount
    ...
    m_lastBarWidth = buttonMaxXPos;
             ...
             0)  // make sure this isnt the first time we opened the window
  {
    int dx =
        buttonMaxXPos - m_lastBarWidth;  // this is how much we grew or shrank.

    // see how many bars we have and now much each should grow/shrink
    dxPerBar = (int)((float)dx / (float)numToResize);
    m_lastBarWidth = buttonMaxXPos;
  } else  // this is the first time we've opened the window, make sure all our
          // colums fit! resize if needed
  {
    int startingBarWidth = 0;
    for (int i = 0; i < nColumns; i++) {
      if (!m_ColumnsData[m_CurrentColumns[i]].m_bHidden) {
        startingBarWidth +=
            m_ColumnsData[m_CurrentColumns[i]].m_pHeader->GetWide();
      }
    }
    int dx = buttonMaxXPos -
             startingBarWidth;  // this is how much we grew or shrank.
    // see how many bars we have and now much each should grow/shrink
    dxPerBar = (int)((float)dx / (float)numToResize);
    m_lastBarWidth = buttonMaxXPos;
  }

  // Make sure nothing is smaller than minwidth to start with or else we'll get
  // into trouble below.
  for (int i = 0; i < nColumns; i++) {
  ...
  // This was a while(1) loop and we hit an infinite loop case, so now we max
  // out the # of times it can loop.
  for (int iLoopSanityCheck = 0; iLoopSanityCheck < 1000; iLoopSanityCheck++) {
    // try and place headers as is - before we have to force items to be minimum
    // width
    int x = -1;
    int i;
    for (i = 0; i < nColumns; i++) {
      ...
      // if we couldn't fit this column - then we need to force items to be
      // minimum width
      ...
      {
        // either this column is allowed to resize OR we are forcing it because
        // we're shrinking all columns
    ...
    // we do this AFTER trying first, to let as many columns as possible try and
    // get to their desired width before we forcing the minimum width on them
    ...
    // shrink from the most right column to minimum width until we can fit them
    // all
    ...
    // If we don't allow this to shrink, then as we resize, it can get stuck in
    // an infinite loop.
    dxPerBar -= 5;
    if (dxPerBar < 0) dxPerBar = 0;
  ...
  if (m_hEditModePanel.Get()) {
    ...
    for (int i = nStartItem; i < nTotalRows && !bDone; i++) {
      int x = 0;
      if (!m_VisibleItems.IsValidIndex(i)) continue;
      ...
      for (int j = 0; j < m_CurrentColumns.Count(); j++) {
        ...
        if (itemID == m_iEditModeItemID && j == m_iEditModeColumn) {
          m_hEditModePanel->SetPos(x + m_iTableStartX + 2,
                                   (drawcount * m_iRowHeight) + m_iTableStartY);
  ...
  for (int i = nStartItem; i < nTotalRows && !bDone; i++) {
    ...
    for (int j = 0; j < m_CurrentColumns.Count(); j++) {
      ...
      if (render) {
        ...
        // mark the panel to draw immediately (since it will probably be
        // recycled to draw other cells)
        render->Repaint();
...
// Purpose:
//-----------------------------------------------------------------------------
void ListPanel::PaintBackground() { BaseClass::PaintBackground(); }
...
void ListPanel::HandleMultiSelection(int itemID, int row, int column) {
  ...
  // convert the last item selected to a row so we can multiply select by rows
  // NOT items
  int lastSelectedRow = (m_LastItemSelected != -1)
                            ? m_VisibleItems.Find(m_LastItemSelected)
                            : row;
  int startRow, endRow;
  ...
  // clear the selection if neither control key was down - we are going to readd
  // ALL selected items in case the user changed the 'direction' of the shift
  // add
...
// Purpose:
//-----------------------------------------------------------------------------
void ListPanel::UpdateSelection(MouseCode code, int x, int y, int row,
                                int column) {
  ...
  if (code == MOUSE_RIGHT && m_SelectedItems.HasElement(itemID)) return;

  if (m_bCanSelectIndividualCells) {
    if (input()->IsKeyDown(KEY_LCONTROL) || input()->IsKeyDown(KEY_RCONTROL)) {
      // we're ctrl selecting the same cell, clear it
      if ((m_LastItemSelected == itemID) && (m_iSelectedColumn == column) &&
...
void ListPanel::OnMousePressed(MouseCode code) {
  ...
  if (code == MOUSE_RIGHT) {
    if (m_SelectedItems.Count() > 0) {
      PostActionSignal(
          new KeyValues("OpenContextMenu", "itemID", m_SelectedItems[0]));
    } else {
...
void ListPanel::OnKeyCodeTyped(KeyCode code) {
  ...
  switch (code) {
    ...
    case KEY_LEFT:
      if (m_bCanSelectIndividualCells && (GetSelectedItemsCount() == 1) &&
          (nSelectedColumn >= 0)) {
        nSelectedColumn--;
  ...
  // This will select the cell if in single select mode, or the row in
  // multiselect mode
  if ((row != m_LastItemSelected) || (nSelectedColumn != m_iSelectedColumn) ||
      (m_SelectedItems.Count() > 1)) {
...
// Purpose:
//-----------------------------------------------------------------------------
bool ListPanel::GetCellBounds(int row, int col, int &x, int &y, int &wide,
                              int &tall) {
  if (col < 0 || col >= m_CurrentColumns.Count()) return false;

  if (row < 0 || row >= m_VisibleItems.Count()) return false;

  // Is row on screen?
  int startitem = GetStartItem();
  if (row < startitem || row >= (startitem + GetRowsPerPage())) return false;
...
bool ListPanel::GetCellAtPos(int x, int y, int &row, int &col) {
  ...
  if (x >= 0 && y >= 0) {
    ...
    // NOTE: if we do height independent rows, we will need to change
    // GetCellBounds as well
    for (row = startitem; row < m_VisibleItems.Count(); row++) {
      if (y < (((row - startitem) + 1) * m_iRowHeight)) break;
    }
...
void ListPanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_DisabledColor =
      GetSchemeColor("ListPanel.DisabledTextColor", m_LabelFgColor, pScheme);
  m_SelectionFgColor =
      GetSchemeColor("ListPanel.SelectedTextColor", m_LabelFgColor, pScheme);
  m_DisabledSelectionFgColor = GetSchemeColor(
      "ListPanel.DisabledSelectedTextColor", m_LabelFgColor, pScheme);

  m_pEmptyListText->SetColor(
      GetSchemeColor("ListPanel.EmptyListInfoTextColor", pScheme));
...
void ListPanel::SetSortFunc(int col, SortFunc *func) {
  ...
  if (!m_ColumnsData[dataColumnIndex].m_bTypeIsText && func != NULL) {
    m_ColumnsData[dataColumnIndex].m_pHeader->SetMouseClickEnabled(MOUSE_LEFT,
                                                                   1);
  }
...
// Purpose:
//-----------------------------------------------------------------------------
void ListPanel::SetSortColumn(int column) { m_iSortColumn = column; }

int ListPanel::GetSortColumn() const { return m_iSortColumn; }

void ListPanel::SetSortColumnEx(int iPrimarySortColumn,
                                int iSecondarySortColumn, bool bSortAscending) {
  m_iSortColumn = iPrimarySortColumn;
  m_iSortColumnSecondary = iSecondarySortColumn;
  m_bSortAscending = bSortAscending;
}

void ListPanel::GetSortColumnEx(int &iPrimarySortColumn,
                                int &iSecondarySortColumn,
                                bool &bSortAscending) const {
...
void ListPanel::SortList(void) {
  ...
  // check if the last selected item is on the screen - if so, we should try to
  // maintain it on screen
  int startItem = GetStartItem();
  int rowsperpage = (int)GetRowsPerPage();
  ...
  if (m_LastItemSelected != -1 && m_SelectedItems.Count() > 0) {
    ...
    if (selectedItemRow >= startItem &&
        selectedItemRow <= (startItem + rowsperpage)) {
  ...
  if (m_CurrentColumns.IsValidIndex(m_iSortColumn)) {
    IndexRBTree_t &rbtree =
        m_ColumnsData[m_CurrentColumns[m_iSortColumn]].m_SortedTree;
    unsigned int index = rbtree.FirstInorder();
    unsigned int lastIndex = rbtree.LastInorder();
    int prevDuplicateIndex = 0;
    int sortValue = 1;
    while (1) {
      FastSortListPanelItem *dataItem =
          (FastSortListPanelItem *)rbtree[index].dataItem;
      if (dataItem->visible) {
        // only increment the sort value if we're a different token from the
        // previous
        if (!prevDuplicateIndex ||
  ...
  if (m_CurrentColumns.IsValidIndex(m_iSortColumnSecondary)) {
    IndexRBTree_t &rbtree =
        m_ColumnsData[m_CurrentColumns[m_iSortColumnSecondary]].m_SortedTree;
    unsigned int index = rbtree.FirstInorder();
    unsigned int lastIndex = rbtree.LastInorder();
    int sortValue = 1;
    int prevDuplicateIndex = 0;
  ...
  // quick sort the list
  qsort(m_VisibleItems.Base(), (size_t)m_VisibleItems.Count(),
        (size_t)sizeof(int), AscendingSortFunc);

  if (screenPosition != -1) {
    int selectedItemRow = m_VisibleItems.Find(m_LastItemSelected);

    // if we can put the last selected item in exactly the same spot, put it
    // there, otherwise we need to be at the top of the list
...
void ListPanel::OnColumnResized(int col, int delta) {
  ...
  panelWide -=
      (x + restColumnsMinWidth + m_vbar->GetWide() + WINDOW_BORDER_WIDTH);
  if (wide > panelWide) {
    wide = panelWide;
  }
...
bool ListPanel::IsItemVisible(int itemID) {
  if (!m_DataItems.IsValidIndex(itemID)) return false;
...
float ListPanel::GetRowsPerPage() {
  float rowsperpage =
      (float)(GetTall() - m_iHeaderHeight) / (float)m_iRowHeight;
  return rowsperpage;
}
...
bool ListPanel::IsMultiselectEnabled() const { return m_bMultiselectEnabled; }
...
void ListPanel::OpenColumnChoiceMenu() {
  if (!m_bAllowUserAddDeleteColumns) return;
  ...
  for (int i = 0; i < m_CurrentColumns.Count(); i++) {
    ...
    int itemID = menu->AddCheckableMenuItem(
        name, new KeyValues("ToggleColumnVisible", "col", m_CurrentColumns[i]),
        this);
...
void ListPanel::ApplyUserConfigSettings(KeyValues *userConfig) {
  // We save/restore m_lastBarWidth because all of the column widths are saved
  // relative to that size. If we don't save it, you can run into this case:
  //    - Window width is 500, load sizes setup relative to a 1000-width window
  //	  - Set window size to 1000
  //    - In PerformLayout, it thinks the window has grown by 500 (since
  //    m_lastBarWidth is 500 and new window width is 1000)
  //      so it pushes out any COLUMN_RESIZEWITHWINDOW columns to their max
  //      extent and shrinks everything else to its min extent.
  m_lastBarWidth = userConfig->GetInt("lastBarWidth", 0);

  // read which columns are hidden
  for (int i = 0; i < m_CurrentColumns.Count(); i++) {
    char name[64];
    sprintf_s(name, "%d_hidden", i);
...
// settings
//-----------------------------------------------------------------------------
bool ListPanel::HasUserConfigSettings() { return true; }
...
//-----------------------------------------------------------------------------
bool ListPanel::IsInEditMode() { return (m_hEditModePanel.Get() != NULL); }
