...
class BuildModeDialog::PanelList {
 public:
  ...
  void AddItem(Panel *label, TextEntry *edit, ComboBox *combo, Button *button,
...
BuildModeDialog::~BuildModeDialog() {
  ...
  if (_undoSettings) _undoSettings->deleteThis();
  if (_copySettings) _copySettings->deleteThis();
}
...
class CBuildModeNavCombo : public ComboBox {
 ...
 public:
  CBuildModeNavCombo(Panel *parent, const char *panelName, int numLines,
                     bool allowEdit, bool getParents, Panel *context)
    ...
    if (m_bParents) {
      ...
      while (p) {
        ...
        if (ep && ep->GetBuildGroup()) {
          ...
          menu->AddMenuItem(text, new KeyValues("SetText", "text", text),
                            GetParent(), kv);
        }
        p = p->GetParent();
      }
      ...
      for (i = 0; i < c; ++i) {
        EditablePanel *ep =
            dynamic_cast<EditablePanel *>(m_hContext->GetChild(i));
...
void BuildModeDialog::CreateControls() {
  ...
  for (i = 0; i < m_pBuildGroup->GetRegisteredControlSettingsFileCount(); i++) {
    m_pFileSelectionCombo->AddItem(
        m_pBuildGroup->GetRegisteredControlSettingsFileByIndex(i), NULL);
  }
  ...
  m_pStatusLabel =
      new Label(this, "StatusLabel", "[nothing currently selected]");
  ...
  m_pEditableParents = new CBuildModeNavCombo(this, NULL, 15, false, true,
                                              m_pBuildGroup->GetContextPanel());
  m_pEditableParents->SetSize(116, buttonH);
  m_pEditableParents->SetOpenDirection(Menu::DOWN);

  m_pEditableChildren = new CBuildModeNavCombo(
      this, NULL, 15, false, false, m_pBuildGroup->GetContextPanel());
  ...
  for (i = sorted.FirstInorder(); i != sorted.InvalidIndex();
  ...
  m_pReloadLocalization =
      new Button(this, "Localization", "&Reload Localization");
...
void BuildModeDialog::PerformLayout() {
  ...
  // layout parameters
  const int BORDER_GAP = 16, YGAP_SMALL = 4, YGAP_LARGE = 8, TITLE_HEIGHT = 24,
            BOTTOM_CONTROLS_HEIGHT = 145, XGAP = 6;
  ...
  // selection combo
  m_pFileSelectionCombo->SetBounds(xpos, ypos, wide - (BORDER_GAP * 2),
                                   m_pStatusLabel->GetTall());
  ypos += (m_pStatusLabel->GetTall() + YGAP_SMALL);

  // status
  m_pStatusLabel->SetBounds(xpos, ypos, wide - (BORDER_GAP * 2),
                            m_pStatusLabel->GetTall());
  ypos += (m_pStatusLabel->GetTall() + YGAP_SMALL);

  // center control
  m_pPanelList->m_pControls->SetPos(xpos, ypos);
  m_pPanelList->m_pControls->SetSize(wide - (BORDER_GAP * 2),
                                     tall - (ypos + BOTTOM_CONTROLS_HEIGHT));

  // controls from bottom-right
  ypos = tall - BORDER_GAP;
  xpos = BORDER_GAP + m_pVarsButton->GetWide() + m_pDeleteButton->GetWide() +
         m_pAddNewControlCombo->GetWide() + (XGAP * 2);
...
// Input  : char **string - pointer to the string pointer, which will be
// incremented Output : const char * - pointer to the token
//-----------------------------------------------------------------------------
...
void BuildModeDialog::SetActiveControl(Panel *controlToEdit) {
  ...
  while (1) {
    ...
    HFont smallFont =
        scheme()->GetIScheme(GetScheme())->GetFont("DefaultVerySmall");
...
void BuildModeDialog::UpdateControlData(Panel *control) {
  KeyValues *dat =
      m_pPanelList->m_pResourceData->FindKey(control->GetName(), true);
  ...
  char statusText[512];
  Q_snprintf(statusText, sizeof(statusText), "%s: \'%s\'",
             control->GetClassName(), control->GetName());
...
//-----------------------------------------------------------------------------
void BuildModeDialog::UpdateEditControl(PanelItem_t &panelItem,
                                        const char *datstring) {
...
void BuildModeDialog::ApplyDataToControls() {
  ...
  if (panel) {
    if (panel != m_pCurrentPanel)  // make sure name is taken by some other
                                   // panel not this one
    {
      char messageString[255];
      Q_snprintf(messageString, sizeof(messageString),
                 "Fieldname is not unique: %s\nRename it and try again.",
                 fieldName);
      MessageBox *errorBox =
          new MessageBox("Cannot Apply", messageString, false);
      errorBox->DoModal();
      UpdateControlData(m_pCurrentPanel);
      m_pApplyButton->SetEnabled(false);
      return;
  ...
  for (int i = 0; i < m_pPanelList->m_PanelList.Size(); i++) {
    ...
    switch (m_pPanelList->m_PanelList[i].m_iType) {
      case TYPE_CORNER:
      case TYPE_AUTORESIZE:
        // the integer value is assumed to be the first part of the string for
        // these items
...
//-----------------------------------------------------------------------------
// Purpose: Revert to the stored the settings of the current panel in a
// keyValues
//-----------------------------------------------------------------------------
...
void BuildModeDialog::DoCopy() {
  ...
  Q_strncpy(_copyClassName, m_pCurrentPanel->GetClassName(),
            sizeof(_copyClassName));
}
...
void BuildModeDialog::ShowHelp() {
  char helpText[] =
      ...
      "Delete button - deletes the currently selected panel if it is "
      "deletable.\n"
      ...
      "Shift clicking panels allows multiple panels to be selected into a "
      "group.\n"
      "Ctrl-c copies the settings of the last selected panel.\n"
      "Ctrl-v creates a new panel with the copied settings at the location of "
      "the mouse pointer.\n"
      "Arrow keys slowly move panels, holding shift + arrow will slowly resize "
      "it.\n"
...
void BuildModeDialog::OnShowNewControlMenu() {
  ...
  m_hContextMenu->AddMenuItem(
      "None", "None", new KeyValues("CreateNewControl", "text", "None"), this);
  ...
  for (i = sorted.FirstInorder(); i != sorted.InvalidIndex();
    ...
    m_hContextMenu->AddMenuItem(
        sorted[i], sorted[i],
        new KeyValues("CreateNewControl", "text", sorted[i]), this);
  }
