...
//-----------------------------------------------------------------------------
// Purpose: forwards tab key presses up from the text entry so we can do
// autocomplete
//-----------------------------------------------------------------------------
 ...
 public:
  ...
  virtual void OnKeyCodeTyped(KeyCode code) {
    if (code == KEY_TAB) {
      ...
      PostMessage(GetParent()->GetVPanel(),
                  new KeyValues("Command", "command", "submit"));
    } else {
  ...
  virtual void OnKillFocus() {
    if (input()->GetFocus() !=
        m_pCompletionList)  // if its not the completion window trying to steal
                            // our focus
    {
...
CConsolePanel::CompletionItem &CConsolePanel::CompletionItem::operator=(
    const CompletionItem &src) {
...
const char *CConsolePanel::CompletionItem::GetName() const {
  if (m_bIsCommand) return m_pCommand->GetName();
...
const char *CConsolePanel::CompletionItem::GetItemText(void) {
  ...
  if (m_pText) {
    if (m_pText->HasExtra()) {
      Q_snprintf(text, sizeof(text), "%s %s", m_pText->GetText(),
                 m_pText->GetExtra());
    } else {
...
//-----------------------------------------------------------------------------
CConsolePanel::CConsolePanel(vgui::Panel *pParent, const char *pName,
                             bool bStatusVersion)
    ...
    while (cmd) {
      ...
      if (!_strnicmp(text, cmd->GetName(), len)) {
        ...
        if (!cmd->IsCommand()) {
          ...
          ConVar_ServerBounded *pBounded =
              dynamic_cast<ConVar_ServerBounded *>(var);
...
void CConsolePanel::OnTextChanged(Panel *panel) {
  if (panel != m_pEntry) return;

  Q_strncpy(m_szPreviousPartialText, m_szPartialText,
            sizeof(m_szPreviousPartialText));
  ...
  bool hitTilde =
      (m_szPartialText[len - 1] == '~' || m_szPartialText[len - 1] == '`')
          ? true
          : false;
  ...
  bool altKeyDown =
      (vgui::input()->IsKeyDown(KEY_LALT) || vgui::input()->IsKeyDown(KEY_RALT))
          ? true
          : false;
  bool ctrlKeyDown = (vgui::input()->IsKeyDown(KEY_LCONTROL) ||
...
void CConsolePanel::PerformLayout() {
  ...
  if (!m_bStatusVersion) {
    ...
    const int submitInset =
        7;  // x inset to pull the submit button away from the frame grab

    m_pHistory->SetPos(inset, inset + topHeight);
    m_pHistory->SetSize(wide - (inset * 2), tall - (entryInset * 2 + inset * 2 +
                                                    topHeight + entryHeight));
    ...
    m_pHistory->SetBounds(inset + entryWidth + inset, inset,
                          (wide - entryWidth) - inset, tall - 2 * inset);
  }
...
void CConsolePanel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  m_pCompletionList->SetFont(
      pScheme->GetFont("DefaultSmall", IsProportional()));
  InvalidateLayout();
}
...
void CConsolePanel::AddToHistory(const char *commandText,
  ...
  if (extraText) {
    ...
    if (extra) {
      ...
      while (
          i >= 0 &&  // Check I before referencing i == -1 into the extra array!
          extra[i] == ' ') {
        extra[i] = '\0';
        i--;
      }
...
void CConsolePanel::DumpConsoleTextToFile() {
  ...
  for (int i = 0; i < CONDUMP_FILES_MAX_NUM; ++i) {
    sprintf_s(szfile, "condump%03d.txt", i);
  ...
  if (!found) {
    Print(
        "Can't condump! Too many existing condump output files in the "
        "gamedir!\n");
    return;
  }
...
//
//-----------------------------------------------------------------------------
CConsoleDialog::CConsoleDialog(vgui::Panel *pParent, const char *pName,
                               bool bStatusVersion)
