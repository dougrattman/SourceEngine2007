...
class ClickPanel : public Panel {
 ...
 public:
  ...
  void SetTextIndex(int index) { _textIndex = index; }
...
class RichTextInterior : public Panel {
 ...
 public:
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    //		m_pAppearanceScrollbar = FindSchemeAppearance( pScheme,
    //"scrollbar_visible" );
  }
...
RichText::RichText(Panel *parent, const char *panelName)
  ...
  if (IsProportional()) {
    ...
    _drawOffsetX =
        static_cast<int>(static_cast<float>(DRAW_OFFSET_X) *
                         (static_cast<float>(sw) / static_cast<float>(width)));
    _drawOffsetY =
        static_cast<int>(static_cast<float>(DRAW_OFFSET_Y) *
                         (static_cast<float>(sw) / static_cast<float>(width)));
  } else {
...
//-----------------------------------------------------------------------------
// Purpose: sets it as drawing text only - used for embedded RichText control
// into other text drawing situations
//-----------------------------------------------------------------------------
...
void RichText::ApplySchemeSettings(IScheme *pScheme) {
  ...
  _selectionTextColor =
      GetSchemeColor("RichText.SelectedTextColor", GetFgColor(), pScheme);
  _selectionColor = GetSchemeColor("RichText.SelectedBgColor", pScheme);

  if (Q_strlen(pScheme->GetResourceString("RichText.InsetX"))) {
    SetDrawOffsets(atoi(pScheme->GetResourceString("RichText.InsetX")),
                   atoi(pScheme->GetResourceString("RichText.InsetY")));
  }
}
...
void RichText::SetFgColor(Color color) {
  ...
  if (m_FormatStream.Size() == 1 &&
      (m_FormatStream[0].color == _defaultTextColor ||
       m_FormatStream[0].color == GetFgColor())) {
...
const wchar_t *RichText::ResolveLocalizedTextAndVariables(
    char const *pchLookup, wchar_t *outbuf, size_t outbufsizeinbytes) {
  if (pchLookup[0] == '#') {
    ...
    if (index == INVALID_STRING_INDEX) {
      /*			// if it's not found, maybe it's a special
         expanded variable - look for an expansion char rgchT[SOURCE_MAX_PATH];
                              ...
                              if ( variables )
                              {
                                      // see if any are any special vars to put
         in for ( KeyValues *pkv = variables->GetFirstSubKey(); pkv != NULL; pkv
         = pkv->GetNextKey() )
                                      {
                                              if ( !Q_strncmp( pkv->GetName(),
         "$", 1 ) )
                                              {
                                                      // make a new lookup, with
         this key appended Q_snprintf( rgchT, sizeof( rgchT ), "%s%s=%s",
         pchLookup, pkv->GetName(), pkv->GetString() ); index =
         localize()->FindIndex( rgchT ); break;
                                              }
                                      }
                              }
                              */
    ...
    if (index != INVALID_STRING_INDEX) {
      ...
      if (format) {
        ...
        if ( variables )
        {
                localize()->ConstructString( outbuf, outbufsizeinbytes, index,
...
int RichText::PixelToCursorSpace(int cx, int cy) {
  ...
  for (i = startIndex; i < m_TextStream.Count(); i++) {
    ...
    if (onRightLine) {
      ...
      {
      } else if (cx < (_drawOffsetX + renderState.pixelsIndent) ||
...
int RichText::DrawString(int iFirst, int iLast, TRenderState &renderState,
  ...
  if (iFirst >= selection0 && iFirst < selection1) {
    ...
    surface()->DrawFilledRect(renderState.x, renderState.y,
                              renderState.x + charWide,
...
void RichText::FinishingURL(int x, int y) {
  ...
  ClickPanel *clickPanel =
      _clickableTextPanels.IsValidIndex(_clickableTextIndex)
          ? _clickableTextPanels[_clickableTextIndex]
          : NULL;
  if (clickPanel) {
    int px, py;
    clickPanel->GetPos(px, py);
    int fontTall = surface()->GetFontTall(_font);
    clickPanel->SetSize(std::max(x - px, 6), y - py + fontTall);
...
void RichText::Paint() {
  ...
  for (int i = startIndex; i < m_TextStream.Count() && renderState.y < tall;) {
    ...
    if (i < selection0 && iLast >= selection0) iLast = selection0 - 1;
    ...
    if (iLast < i) {
      if (m_TextStream[i] == '\t') {
        ...
        dxTabWidth = std::max(1, dxTabWidth);
...
int RichText::GetClickableTextIndexStart(int startIndex) {
  // cycle to the right url panel	for what is visible	after the
  // startIndex.
...
bool RichText::UpdateRenderState(int textStreamPos, TRenderState &renderState) {
  ...
  if (m_FormatStream.IsValidIndex(renderState.formatStreamIndex) &&
    ...
    renderState.textColor = m_FormatStream[renderState.formatStreamIndex].color;
    renderState.textClickable =
        m_FormatStream[renderState.formatStreamIndex].textClickable;

    CalculateFade(renderState);

    int indentChange =
        m_FormatStream[renderState.formatStreamIndex].pixelsIndent -
        renderState.pixelsIndent;
    renderState.pixelsIndent =
        m_FormatStream[renderState.formatStreamIndex].pixelsIndent;
...
//-----------------------------------------------------------------------------
// Purpose: Returns the index in the format stream for the specified text stream
// index
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void RichText::GenerateRenderStateForTextStreamIndex(
    int textStreamIndex, TRenderState &renderState) {
  // find where in the format stream we need to be given the specified place in
  // the text stream
  renderState.formatStreamIndex =
      FindFormatStreamIndexForTextStreamPos(textStreamIndex);
...
void RichText::ResetAllFades(bool bHold, bool bOnlyExpired,
                             float flNewSustain) {
  m_bResetFades = bHold;
  ...
  if (m_bResetFades == false) {
    for (int i = 1; i < m_FormatStream.Count(); i++) {
      if (bOnlyExpired == true) {
        if (m_FormatStream[i].fade.flFadeStartTime >=
            system()->GetCurrentTime())
          continue;
      }
      ...
      m_FormatStream[i].fade.flFadeStartTime =
          system()->GetCurrentTime() + flNewSustain;
    }
  }
}
...
void RichText::RecalculateLineBreaks() {
  ...
  for (int i = startChar; i < m_TextStream.Count(); ++i) {
    ...
    if ((x + charWidth) >= wide || ch == '\r' || ch == '\n') {
      ...
      } else if (wordStartedOnNewLine || iswspace(ch))  // catch the "blah
                                                        // " case wrapping
                                                        // around a line
      {
        ...
        if (renderState.textClickable) {
          ...
          int oldIndex =
              _clickableTextPanels[clickableTextNum - 1]->GetTextIndex();
...
void RichText::LayoutVerticalScrollBarSlider() {
  ...
  if (rmax &&
      (previousValue + rmin + _vertScrollBar->GetRangeWindow() == rmax)) {
...
void RichText::CopySelected() {
  ...
  if (GetSelectedRange(x0, x1)) {
    ...
    for (int i = x0; i <= x1; i++) {
      if (m_TextStream.IsValidIndex(i) == false) continue;
...
int RichText::GetStartDrawIndex(int &lineBreakIndexIndex) {
  ...
  if (startLine >= m_LineBreaks.Count())  // incase the line breaks got reset
                                          // and the scroll bar hasn't
  {
...
void RichText::OnClickPanel(int index) {
  ...
  for (int i = index; i < (sizeof(wBuf) - 1) && i < m_TextStream.Count(); i++) {
    ...
    if (!renderState.textClickable) break;
  ...
  if (m_FormatStream[iFormatSteam].m_sClickableTextAction) {
    Q_UTF8ToUnicode(
        m_FormatStream[iFormatSteam].m_sClickableTextAction.String(), wBuf,
        sizeof(wBuf));
  }
...
const char *RichText::GetDescription() {
  ...
  Q_snprintf(buf, sizeof(buf), "%s, string text, bool scrollbar",
...
void RichText::SetToFullHeight() {
  ...
  tall = GetNumLines() * (surface()->GetFontTall(_font) + _drawOffsetY) +
         _drawOffsetY + 2;
  SetSize(wide, tall);
  PerformLayout();
}
...
//-----------------------------------------------------------------------------
void RichText::InsertPossibleURLString(const char *text, Color URLTextColor,
...
//-----------------------------------------------------------------------------
int RichText::ParseTextStringForUrls(const char *text, int startPos,
                                     char *pchURLText, int cchURLText,
                                     char *pchURL, int cchURL,
                                     bool &clickable) {
