...
static ColumnInfo_t g_ColInfo[] = {
    {"text", "#PerforceFileList_Col_Name", 175, 20, 10000,
     ListPanel::COLUMN_UNHIDABLE, &ListFileNameSortFunc, Label::a_west},
    {"type", "#PerforceFileList_Col_Type", 150, 20, 10000, 0,
     &ListFileTypeSortFunc, Label::a_west},
    {"in_perforce", "#PerforceFileList_Col_InPerforce", 50, 20, 10000,
     ListPanel::COLUMN_UNHIDABLE, &ListFileAttributesSortFunc, Label::a_west},
    {"synched", "#PerforceFileList_Col_Synched", 50, 20, 10000,
     ListPanel::COLUMN_UNHIDABLE, &ListFileAttributesSortFunc, Label::a_west},
    {"checked_out", "#PerforceFileList_Col_Checked_Out", 50, 20, 10000,
     ListPanel::COLUMN_UNHIDABLE, &ListFileAttributesSortFunc, Label::a_west},
    {"attributes", "#PerforceFileList_Col_Attributes", 50, 20, 10000,
...
PerforceFileList::PerforceFileList(Panel *pParent, const char *pPanelName)
  ...
  for (int i = 0; i < SOURCE_ARRAYSIZE(g_ColInfo); ++i) {
    ...
    AddColumnHeader(i, info.columnName, info.columnText, info.startingWidth,
                    info.minWidth, info.maxWidth, info.flags);
...
void PerforceFileList::ShowDeletedFiles(bool bShowDeletedFiles) {
  if (m_bShowDeletedFiles != bShowDeletedFiles) {
    ...
    for (int i = FirstItem(); i != InvalidItemID(); i = NextItem(i)) {
      ...
      if (!pKeyValues->GetInt("deleted", 0)) continue;
...
//-----------------------------------------------------------------------------
int PerforceFileList::AddFileToFileList(const char *pFullPath,
                                        bool bExistsOnDisk) {
  bool bIsFileWriteable =
      bExistsOnDisk ? g_pFullFileSystem->IsFileWritable(pFullPath, NULL) : true;
  ...
  g_pFullFileSystem->GetFileTypeForFullPath(pFullPath, pFileType,
                                            sizeof(pFileType));
...
//-----------------------------------------------------------------------------
int PerforceFileList::AddDirectoryToFileList(const char *pFullPath,
                                             bool bExistsOnDisk) {
  KeyValues *kv = new KeyValues("item");
...
//-----------------------------------------------------------------------------
int PerforceFileList::AddFile(const char *pFullPath, int nFileExists,
                              int nIsDirectory) {
  if (!pFullPath) return InvalidItemID();
...
//-----------------------------------------------------------------------------
static P4File_t *FindFileInPerforceList(const char *pFileName,
                                        CUtlVector<P4File_t> &fileList,
                                        bool *pFound) {
  int nCount = fileList.Count();
  for (int i = 0; i < nCount; ++i) {
    if (pFound[i]) continue;
...
//-----------------------------------------------------------------------------
void PerforceFileList::RefreshPerforceState(int nItemID, bool bFileExists,
                                            P4File_t *pFileInfo) {
  ...
  if (bIsFileInPerforce) {
    if (pFileInfo->m_bDeleted != bFileExists) {
      bIsSynched = (pFileInfo->m_bDeleted ||
                    (pFileInfo->m_iHeadRevision == pFileInfo->m_iHaveRevision));
    }
  } else {
  ...
  kv->SetInt("checked_out",
             bIsFileInPerforce && (pFileInfo->m_eOpenState != P4FILE_UNOPENED));
...
void PerforceFileList::Refresh() {
  ...
  for ( int i = FirstItem(); i != InvalidItemID(); i = NextItem( i ) )
  {
          ...
          RefreshPerforceState( i, bFileExists, bIsFileInPerforce ? &fileInfo :
  NULL );
  }
  */
  ...
  for (int i = 0; i < nCount; ++i) {
    ...
    CUtlVector<P4File_t> &fileList =
        p4->GetFileListUsingClientSpec(pDirectory, pInfo->m_ClientSpec);
...
int PerforceFileList::FindFile(const char *pFullPath) {
  for (int i = FirstItem(); i != InvalidItemID(); i = NextItem(i)) {
    const char *pFile = GetFile(i);
    if (!Q_stricmp(pFile, pFullPath)) return i;
  }
  return InvalidItemID();
}
