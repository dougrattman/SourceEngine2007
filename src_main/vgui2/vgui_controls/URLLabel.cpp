...
// Purpose: constructor
//-----------------------------------------------------------------------------
URLLabel::URLLabel(Panel *parent, const char *panelName, const char *text,
                   const char *pszURL)
...
//-----------------------------------------------------------------------------
URLLabel::URLLabel(Panel *parent, const char *panelName, const wchar_t *wszText,
                   const char *pszURL)
...
void URLLabel::SetURL(const char *pszURL) {
  ...
  strcpy_s(m_pszURL, iNewURLSize + 1, pszURL);
...
const char *URLLabel::GetDescription(void) {
  ...
  _snprintf_s(buf, sizeof(buf), "%s, string URLText",
...
void URLLabel::ApplySchemeSettings(IScheme *pScheme) {
  ...
  // the Label::ApplySchemeSettings() will override it if override set in scheme
  // file
