...
//-----------------------------------------------------------------------------
bool lessFunc(const int &lhs, const int &rhs) { return lhs < rhs; }
...
//			the button map.  If the keycode isn't handled, it gets
...
void CControllerMap::ApplySettings(KeyValues *inResourceData) {
  ...
  for (KeyValues *dat = inResourceData->GetFirstSubKey(); dat != NULL;
       dat = dat->GetNextKey()) {
