...
//-----------------------------------------------------------------------------
CSubRectImage::CSubRectImage(const char *filename, bool hardwareFiltered,
  ...
  //			 This needs to be revisited once GoldSRC is
  // grandfathered  off.
  //!! need to make this work with goldsrc
  usize size = strlen(filename) + std::size("vgui/");
  _filename = (char *)malloc(size);
  Assert(_filename);

  sprintf_s(_filename, size, "vgui/%s", filename);
...
CSubRectImage::~CSubRectImage() { heap_free(_filename); }
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
void CSubRectImage::SetColor(Color col) { _color = col; }

//-----------------------------------------------------------------------------
// Purpose: returns the file name of the bitmap
//-----------------------------------------------------------------------------
const char *CSubRectImage::GetName() { return _filename; }
...
void CSubRectImage::Paint() {
  ...
  surface()->DrawTexturedSubRect(_pos[0], _pos[1], _pos[0] + _wide,
                                 _pos[1] + _tall, s[0], t[0], s[1], t[1]);
}
...
// Purpose: data accessor
//-----------------------------------------------------------------------------
HTexture CSubRectImage::GetID() { return _id; }

bool CSubRectImage::IsValid() { return _valid; }
