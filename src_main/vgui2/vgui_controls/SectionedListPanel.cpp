...
class CSectionHeader : public Label {
 ...
 public:
  CSectionHeader(SectionedListPanel *parent, const char *name, int sectionID)
      : Label(parent, name, "") {
    m_pListPanel = parent;
  ...
  CSectionHeader(SectionedListPanel *parent, const wchar_t *name, int sectionID)
  ...
  void ApplySchemeSettings(IScheme *pScheme) {
    ...
    m_SectionDividerColor =
        GetSchemeColor("SectionedListPanel.DividerColor", pScheme);
    SetBgColor(GetSchemeColor("SectionedListPanelHeader.BgColor", GetBgColor(),
                              pScheme));
  ...
  void PerformLayout() {
    ...
    if (colCount != GetImageCount()) {
      ...
      for (int i = 0; i < colCount; i++) {
        int columnFlags =
            m_pListPanel->GetColumnFlagsBySection(m_iSectionID, i);
        IImage *image = NULL;
        if (columnFlags & SectionedListPanel::HEADER_IMAGE) {
          ...
          HFont fallback =
              m_pListPanel->GetColumnFallbackFontBySection(m_iSectionID, i);
    ...
    for (int repeat = 0; repeat <= 1; repeat++) {
      ...
      for (int i = 0; i < colCount; i++) {
        int columnFlags =
            m_pListPanel->GetColumnFlagsBySection(m_iSectionID, i);
        int columnWidth =
            m_pListPanel->GetColumnWidthBySection(m_iSectionID, i);
        ...
        // see if we can draw over the next few column headers (if we're
        // left-aligned)
        if (!(columnFlags & SectionedListPanel::COLUMN_RIGHT)) {
          for (int j = i + 1; j < colCount; j++) {
            ...
            if (iwide == 0) {
              ...
              maxWidth +=
                  m_pListPanel->GetColumnWidthBySection(m_iSectionID, j);
            }
          }
        }
        if (maxWidth >= 0) {
        ...
        if (columnFlags & SectionedListPanel::COLUMN_RIGHT) {
          SetImageBounds(i, xpos + wide - contentWide,
                         contentWide - COLUMN_DATA_GAP);
        } else {
        ...
        if (!(columnFlags & SectionedListPanel::HEADER_IMAGE)) {
          ...
          textImage->SetText(
              m_pListPanel->GetColumnTextBySection(m_iSectionID, i));
...
class CItemButton : public Label {
 ...
 public:
  ...
  virtual void PerformLayout() {
    ...
    if (!m_pData || colCount < 1) {
      ...
      if (colCount != GetImageCount()) {
        ...
        {
          for (int i = GetImageCount(); i < colCount;
               i++)  // make sure we have enough image slots
          {
            AddImage(NULL, 0);
          }
        }
      ...
      for (int i = 0; i < colCount; i++) {
        const char *keyname =
            m_pListPanel->GetColumnNameBySection(m_iSectionID, i);
        ...
        if (columnFlags & SectionedListPanel::COLUMN_IMAGE) {
          ...
          if (m_pListPanel->m_pImageList) {
            ...
            if (m_pListPanel->m_pImageList->IsValidIndex(imageIndex)) {
              ...
              // this is mildly valid (CGamesList hits it because of the way it
              // uses the image indices) Assert(!("Image index out of range for
              // ImageList in SectionedListPanel"));
            }
          } else {
            Assert(
                !("Images columns used in SectionedListPanel with no ImageList "
                  "set"));
          }
        } else {
          ...
          if (textImage) {
            ...
            // set the text color based on the selection state - if one of the
            // children of the SectionedListPanel has focus, then 'we have
            // focus' if we're selected
            VPANEL focus = input()->GetFocus();
            if (!m_bOverrideColors) {
              if (IsSelected() && !m_pListPanel->IsInEditMode()) {
                if (HasFocus() ||
                    (focus && ipanel()->HasParent(focus, GetVParent()))) {
        ...
        if (i == 0 && !(columnFlags & SectionedListPanel::COLUMN_IMAGE)) {
          ...
          SetImageBounds(i, xpos + COLUMN_DATA_INDENT,
                         wide - (COLUMN_DATA_INDENT + COLUMN_DATA_GAP));
        } else {
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    m_ArmedFgColor1 =
        GetSchemeColor("SectionedListPanel.BrightTextColor", pScheme);
    m_ArmedFgColor2 =
        GetSchemeColor("SectionedListPanel.SelectedTextColor", pScheme);
    m_OutOfFocusSelectedTextColor = GetSchemeColor(
        "SectionedListPanel.OutOfFocusSelectedTextColor", pScheme);
    m_ArmedBgColor =
        GetSchemeColor("SectionedListPanel.SelectedBgColor", pScheme);

    m_FgColor2 = GetSchemeColor("SectionedListPanel.TextColor", pScheme);

    m_BgColor =
        GetSchemeColor("SectionedListPanel.BgColor", GetBgColor(), pScheme);
    m_SelectionBG2Color =
        GetSchemeColor("SectionedListPanel.OutOfFocusSelectedBgColor", pScheme);
  ...
  virtual void OnMousePressed(MouseCode code) {
    if (code == MOUSE_LEFT) {
      m_pListPanel->PostActionSignal(
          new KeyValues("ItemLeftClick", "itemID", m_iID));
    }
    if (code == MOUSE_RIGHT) {
  ...
  virtual void OnSetFocus() {
    InvalidateLayout();  // force the layout to be redone so we can change text
                         // color according to focus
  ...
  virtual void OnMouseDoublePressed(MouseCode code) {
    if (code == MOUSE_LEFT) {
      m_pListPanel->PostActionSignal(
          new KeyValues("ItemDoubleLeftClick", "itemID", m_iID));
...
void SectionedListPanel::ReSortList() {
  ...
  for (int sectionIndex = 0; sectionIndex < m_Sections.Size(); sectionIndex++) {
    ...
    for (int i = m_Items.Head(); i != m_Items.InvalidIndex();
         i = m_Items.Next(i)) {
      if (m_Items[i]->GetSectionID() == m_Sections[sectionIndex].m_iID) {
        ...
        if (section.m_pSortFunc) {
          ...
          for (; insertionPoint < m_SortedItems.Count(); insertionPoint++) {
            if (section.m_pSortFunc(this, i,
                                    m_SortedItems[insertionPoint]->GetID()))
              break;
          }
...
//-----------------------------------------------------------------------------
// Purpose: iterates through and sets up the position of all the sections and
// items
//-----------------------------------------------------------------------------
void SectionedListPanel::PerformLayout() {
  ...
  if (m_iContentHeight > ctall && m_bVerticalScrollbarEnabled) {
    ...
    // since we're just about to make the scrollbar visible, we need to
    // re-layout the buttons since they depend on the scrollbar size
    LayoutPanels(m_iContentHeight);
  } else {
...
void SectionedListPanel::ApplySettings(KeyValues *inResourceData) {
  ...
  if (IsProportional()) {
    m_iLineSpacing =
        scheme()->GetProportionalScaledValueEx(GetScheme(), m_iLineSpacing);
  }
}
...
void SectionedListPanel::SetProportional(bool state) {
  ...
  FOR_EACH_LL(m_Items, j) { m_Items[j]->SetProportional(state); }
}
...
//-----------------------------------------------------------------------------
void SectionedListPanel::AddSection(int sectionID, const char *name,
                                    SectionSortFunc_t sortFunc) {
  CSectionHeader *header =
      SETUP_PANEL(new CSectionHeader(this, name, sectionID));
...
// Purpose: adds a new section
//-----------------------------------------------------------------------------
void SectionedListPanel::AddSection(int sectionID, const wchar_t *name,
                                    SectionSortFunc_t sortFunc) {
  CSectionHeader *header =
      SETUP_PANEL(new CSectionHeader(this, name, sectionID));
  AddSectionHelper(sectionID, header, sortFunc);
...
//-----------------------------------------------------------------------------
void SectionedListPanel::AddSectionHelper(int sectionID, CSectionHeader *header,
                                          SectionSortFunc_t sortFunc) {
...
void SectionedListPanel::RemoveAllSections() {
  for (int i = 0; i < m_Sections.Count(); i++) {
    if (!m_Sections.IsValidIndex(i)) continue;
...
//-----------------------------------------------------------------------------
bool SectionedListPanel::AddColumnToSection(
    int sectionID, const char *columnName, const char *columnText,
    int columnFlags, int width, HFont fallbackFont /*= INVALID_FONT*/) {
  wchar_t wtext[64];
...
//-----------------------------------------------------------------------------
bool SectionedListPanel::AddColumnToSection(
    int sectionID, const char *columnName, const wchar_t *columnText,
    int columnFlags, int width, HFont fallbackFont /*= INVALID_FONT*/) {
  ...
  strcpy_s(column.m_szColumnName, columnName);
  wcscpy_s(column.m_szColumnText, columnText);
...
//-----------------------------------------------------------------------------
bool SectionedListPanel::ModifyColumn(int sectionID, const char *columnName,
  ...
  int columnIndex;
  for (columnIndex = 0; columnIndex < section.m_Columns.Count();
       columnIndex++) {
    if (!_stricmp(section.m_Columns[columnIndex].m_szColumnName, columnName))
      break;
  }
  if (!section.m_Columns.IsValidIndex(columnIndex)) return false;
...
//-----------------------------------------------------------------------------
bool SectionedListPanel::ModifyItem(int itemID, int sectionID,
                                    const KeyValues *data) {
  if (!m_Items.IsValidIndex(itemID)) return false;
...
void SectionedListPanel::SetItemFgColor(int itemID, Color color) {
  ...
  if (!m_Items.IsValidIndex(itemID)) return;
...
void SectionedListPanel::SetSectionFgColor(int sectionID, Color color) {
  if (!m_Sections.IsValidIndex(sectionID)) return;
...
//-----------------------------------------------------------------------------
// Purpose: removes an item from the list; returns false if the item does not
// exist or is already removed
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: returns the name of a column by section and column index; returns
// NULL if there are no more columns
//			valid range of columnIndex is [0,
// GetColumnCountBySection)
//-----------------------------------------------------------------------------
const char *SectionedListPanel::GetColumnNameBySection(int sectionID,
                                                       int columnIndex) {
  int index = FindSectionIndexByID(sectionID);
...
//-----------------------------------------------------------------------------
const wchar_t *SectionedListPanel::GetColumnTextBySection(int sectionID,
                                                          int columnIndex) {
  int index = FindSectionIndexByID(sectionID);
  if (index < 0 || columnIndex >= m_Sections[index].m_Columns.Size())
    return NULL;
...
//-----------------------------------------------------------------------------
int SectionedListPanel::GetColumnFlagsBySection(int sectionID,
                                                int columnIndex) {
  int index = FindSectionIndexByID(sectionID);
  if (index < 0) return 0;
...
// Purpose:
//-----------------------------------------------------------------------------
int SectionedListPanel::GetColumnWidthBySection(int sectionID,
                                                int columnIndex) {
  int index = FindSectionIndexByID(sectionID);
  if (index < 0) return 0;
...
//-----------------------------------------------------------------------------
void SectionedListPanel::OnMousePressed(MouseCode code) { ClearSelection(); }
...
void SectionedListPanel::MoveSelectionDown() {
  ...
  for (i = 0; i < m_SortedItems.Count(); i++) {
    if (m_SortedItems[i]->GetID() == itemID) break;
  }
...
void SectionedListPanel::SetSelectedItem(CItemButton *item) {
  if (m_hSelectedItem.Get() == item) return;
  ...
  Repaint();
  PostActionSignal(
      new KeyValues("ItemSelected", "itemID",
                    m_hSelectedItem.Get() ? m_hSelectedItem->GetID() : -1));
}
...
KeyValues *SectionedListPanel::GetItemData(int itemID) {
  Assert(m_Items.IsValidIndex(itemID));
  if (!m_Items.IsValidIndex(itemID)) return NULL;
...
int SectionedListPanel::GetItemSection(int itemID) {
  if (!m_Items.IsValidIndex(itemID)) return -1;
...
int SectionedListPanel::GetItemIDFromRow(int row) {
  if (!m_SortedItems.IsValidIndex(row)) return -1;

  return m_SortedItems[row]->GetID();
}

//-----------------------------------------------------------------------------
// Purpose: returns the row that this itemID occupies. -1 if the itemID is
// invalid
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
bool SectionedListPanel::GetCellBounds(int itemID, int column, int &x, int &y,
                                       int &wide, int &tall) {
  x = y = wide = tall = 0;
  if (!IsItemIDValid(itemID)) return false;
...
//-----------------------------------------------------------------------------
void SectionedListPanel::EnterEditMode(int itemID, int column,
...
//-----------------------------------------------------------------------------
void SectionedListPanel::SetImageList(ImageList *imageList,
...
//-----------------------------------------------------------------------------
HFont SectionedListPanel::GetColumnFallbackFontBySection(int sectionID,
                                                         int columnIndex) {
  int index = FindSectionIndexByID(sectionID);
  if (index < 0) return INVALID_FONT;
