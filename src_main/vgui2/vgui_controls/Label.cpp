...
void Label::GetContentSize(int &wide, int &tall) {
  if (GetFont() ==
      INVALID_FONT)  // we haven't loaded our font yet, so load it now
  {
  ...
  for (int i = 0; i < _imageDar.Size(); i++) wide += _imageDar[i].offset;
...
Panel *Label::HasHotkey(wchar_t key) {
  ...
  if (iswalnum(key)) key = towlower(key);

  if (_hotkey == key) return this;
#endif
...
//-----------------------------------------------------------------------------
void Label::SetHotkey(wchar_t ch) { _hotkey = ch; }

//-----------------------------------------------------------------------------
// Purpose: Handle a hotkey by passing on focus to associate
//-----------------------------------------------------------------------------
void Label::OnHotkeyPressed() {
  // we can't accept focus, but if we are associated to a control give it to
  // that
...
//-----------------------------------------------------------------------------
// Purpose: Additional offset at the Start of the text (from whichever side it
// is aligned)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void Label::SetEnabled(bool state) { Panel::SetEnabled(state); }
...
void Label::ComputeAlignment(int &tx0, int &ty0, int &tx1, int &ty1) {
  ...
  for (int i = 0; i < _imageDar.Count(); i++) {
    ...
    if (!image) continue;  // skip over 0 images

    // add up the bounds
    int iWide, iTall;
    image->GetSize(iWide, iTall);
    if (iWide >
        wide)  // if the image is larger than the label just do a west alignment
...
void Label::Paint() {
  ...
  int imageYPos = 0;  // a place to save the y offset for when we draw the
                      // disable version of the image
  ...
  for (int i = 0; i < _imageDar.Count(); i++) {
    ...
    // if this is the text image then add its inset to the left or from the
    // right
    ...
    if (_contentAlignment == Label::a_west ||
        _contentAlignment == Label::a_center ||
        _contentAlignment == Label::a_east) {
      int iw, it;
      image->GetSize(iw, it);
      if (it < (ty1 - ty0)) {
    ...
    if (image == _textImage) {
      if (IsEnabled()) {
        if (_associate.Get() &&
            ipanel()->HasParent(input()->GetFocus(), _associate->GetVPanel())) {
...
//-----------------------------------------------------------------------------
void Label::DrawDashedLine(int x0, int y0, int x1, int y1, int dashLen,
                           int gapLen) {
...
//-----------------------------------------------------------------------------
// Purpose: Size the width of the label to its contents - only works from in
...
//-----------------------------------------------------------------------------
HFont Label::GetFont() { return _textImage->GetFont(); }
...
//-----------------------------------------------------------------------------
void Label::SetDisabledFgColor1(Color color) { _disabledFgColor1 = color; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
void Label::SetDisabledFgColor2(Color color) { _disabledFgColor2 = color; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
Color Label::GetDisabledFgColor1() { return _disabledFgColor1; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
Color Label::GetDisabledFgColor2() { return _disabledFgColor2; }

//-----------------------------------------------------------------------------
// Purpose:
//-----------------------------------------------------------------------------
TextImage *Label::GetTextImage() { return _textImage; }
...
IImage *Label::GetImageAtIndex(int index) {
  if (_imageDar.IsValidIndex(index)) return _imageDar[index].image;
  return NULL;
}
...
//-----------------------------------------------------------------------------
int Label::GetImageCount() { return _imageDar.Count(); }
...
int Label::SetTextImageIndex(int newIndex) {
  if (newIndex == _textImageIndex) return _textImageIndex;
...
//-----------------------------------------------------------------------------
// Purpose: Labels can be associated with controls, and alter behaviour based on
...
void Label::ApplySchemeSettings(IScheme *pScheme) {
  ...
  if (m_bWrap) {
    ...
    // if you don't set the size of the image, many, many buttons will break -
    // we might want to look into fixing this all over the place later
    int wide, tall;
...
const char *Label::GetDescription(void) {
  ...
  Q_snprintf(buf, sizeof(buf),
             ...
             "int wrap, int dulltext, int brighttext, string font",
