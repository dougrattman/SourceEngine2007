...
AnimationController *GetAnimationController() {
  static AnimationController *s_pAnimationController =
      new AnimationController(NULL);
...
//-----------------------------------------------------------------------------
bool AnimationController::SetScriptFile(VPANEL sizingPanel,
                                        const char *fileName,
                                        bool wipeAll /*=false*/) {
  ...
  // Store off this filename for reloading later on (if we don't have it
  // already)
...
AnimationController::RelativeAlignmentLookup
    AnimationController::g_AlignmentLookup[] = {
...
//-----------------------------------------------------------------------------
void AnimationController::SetupPosition(AnimCmdAnimate_t &cmd, float *output,
                                        char const *psz, int screendimension) {
...
bool AnimationController::ParseScriptFile(char *pMem, int length) {
  ...
  while (token[0]) {
    ...
    while (token && token[0]) {
      ...
      if (!_stricmp(token, "animate")) {
        ...
        if (cmdAnimate.variable == m_sSize) {
          if (IsProportional()) {
            cmdAnimate.target.a =
                static_cast<float>(vgui::scheme()->GetProportionalScaledValueEx(
                    GetScheme(), cmdAnimate.target.a));
            cmdAnimate.target.b =
                static_cast<float>(vgui::scheme()->GetProportionalScaledValueEx(
                    GetScheme(), cmdAnimate.target.b));
          }
        ...
        Warning(
            "Couldn't parse script sequence '%s': expected <anim command>, "
            "found '%s'\n",
            g_ScriptSymbols.String(seq.name), token);
        return false;
      }
    ...
    if (bAccepted) {
      // Attempt to find a collision in the sequences, replacing the old one if
      // found
      int seqIterator;
      for (seqIterator = 0; seqIterator < m_Sequences.Count() - 1;
           seqIterator++) {
...
void AnimationController::UpdatePostedMessages(bool bRunToCompletion) {
  ...
  for (int i = 0; i < m_PostedMessages.Count(); i++) {
    ...
    if (m_flCurrentTime < msgRef.startTime && !bRunToCompletion) continue;

    // take a copy of th message
    PostedMessage_t msg = msgRef;

    // remove the event
    // do this before handling the message because the message queue may be
    // messed with
    ...
    switch (msg.commandType) {
      case CMD_RUNEVENT: {
        ...
        // run the event, but only if we haven't already run it this frame, for
        // this parent
...
void AnimationController::UpdateActiveAnimations(bool bRunToCompletion) {
  ...
  for (int i = 0; i < m_ActiveAnimations.Count(); i++) {
    ...
    if (m_flCurrentTime < anim.startTime && !bRunToCompletion) continue;
    ...
    if (!anim.started && !bRunToCompletion) {
      ...
      // g_ScriptSymbols.String(anim.variable), anim.endValue.a,
      // g_ScriptSymbols.String(anim.seqName), anim.panel->GetName());
    }
    ...
    if (m_flCurrentTime >= anim.endTime || bRunToCompletion) {
      ...
      // get the interpolated value
      val = GetInterpolatedValue(anim.interpolator, anim.interpolatorParam,
                                 m_flCurrentTime, anim.startTime, anim.endTime,
...
bool AnimationController::UpdateScreenSize() {
  ...
  bool changed = m_nScreenBounds[0] != sx || m_nScreenBounds[1] != sy ||
...
//-----------------------------------------------------------------------------
AnimationController::Value_t AnimationController::GetInterpolatedValue(
    int interpolator, float interpolatorParam, float currentTime,
    float startTime, float endTime, Value_t &startValue, Value_t &endValue) {
...
//-----------------------------------------------------------------------------
// Purpose: sets that the script file should be reloaded each time a script is
// ran
...
// Purpose: starts an animation sequence script
//-----------------------------------------------------------------------------
bool AnimationController::StartAnimationSequence(Panel *pWithinParent,
  ...
  if (seqName == UTL_INVAL_SYMBOL) return false;
  ...
  for (i = 0; i < m_Sequences.Count(); i++) {
    if (m_Sequences[i].name == seqName) break;
  }
  if (i >= m_Sequences.Count()) return false;

  // execute the sequence
  for (int cmdIndex = 0; cmdIndex < m_Sequences[i].cmdList.Count();
       cmdIndex++) {
...
//-----------------------------------------------------------------------------
void AnimationController::RunAnimationCommand(
    vgui::Panel *panel, const char *variable, float targetValue,
    float startDelaySeconds, float duration, Interpolators_e interpolator,
...
//-----------------------------------------------------------------------------
void AnimationController::RunAnimationCommand(
    vgui::Panel *panel, const char *variable, Color targetValue,
    float startDelaySeconds, float duration, Interpolators_e interpolator,
    float animParameter /* = 0 */) {
  // clear any previous animations of this variable
  UtlSymId_t var = g_ScriptSymbols.AddString(variable);
...
//-----------------------------------------------------------------------------
float AnimationController::GetAnimationSequenceLength(
    const char *sequenceName) {
  // lookup the symbol for the name
  UtlSymId_t seqName = g_ScriptSymbols.Find(sequenceName);
  if (seqName == UTL_INVAL_SYMBOL) return 0.0f;
  ...
  if (i >= m_Sequences.Count()) return 0.0f;
...
void AnimationController::RemoveQueuedAnimationCommands(UtlSymId_t seqName,
  ...
  for (int i = 0; i < m_ActiveAnimations.Count(); i++) {
    if (m_ActiveAnimations[i].seqName != seqName) continue;

    // panel this anim is on, m_ActiveAnimations[i].panel
    if (pWithinParent) {
      Panel *animPanel = m_ActiveAnimations[i].panel;

      if (!animPanel) continue;

      Panel *foundPanel =
          pWithinParent->FindChildByName(animPanel->GetName(), true);

      if (foundPanel != animPanel) continue;
    }
...
//-----------------------------------------------------------------------------
void AnimationController::RemoveQueuedAnimationByType(
    vgui::Panel *panel, UtlSymId_t variable, UtlSymId_t sequenceToIgnore) {
  for (int i = 0; i < m_ActiveAnimations.Count(); i++) {
    if (m_ActiveAnimations[i].panel == panel &&
        m_ActiveAnimations[i].variable == variable &&
        m_ActiveAnimations[i].seqName != sequenceToIgnore) {
      // Msg("Removing queued anim %s::%s::%s\n",
      // g_ScriptSymbols.String(m_ActiveAnimations[i].seqName),
      // panel->GetName(), g_ScriptSymbols.String(variable));
      m_ActiveAnimations.Remove(i);
      break;
    }
  }
...
//-----------------------------------------------------------------------------
void AnimationController::ExecAnimationCommand(UtlSymId_t seqName,
                                               AnimCommand_t &animCommand,
                                               Panel *pWithinParent) {
...
//-----------------------------------------------------------------------------
void AnimationController::StartCmd_Animate(UtlSymId_t seqName,
  ...
  Panel *panel =
      pWithinParent->FindChildByName(g_ScriptSymbols.String(cmd.panel), true);
  if (!panel) {
    // Check the parent
...
//-----------------------------------------------------------------------------
void AnimationController::StartCmd_Animate(Panel *panel, UtlSymId_t seqName,
                                           AnimCmdAnimate_t &cmd) {
...
//-----------------------------------------------------------------------------
// Purpose: a posted message to stop all animations relevant to a specified
// panel
//-----------------------------------------------------------------------------
void AnimationController::RunCmd_StopPanelAnimations(PostedMessage_t &msg) {
  ...
  for (int i = 0; i < m_ActiveAnimations.Count(); i++) {
    if (m_ActiveAnimations[i].panel == panel &&
        m_ActiveAnimations[i].seqName != msg.seqName) {
      m_ActiveAnimations.Remove(i);
      --i;
    }
  }
...
void AnimationController::RunCmd_SetFont(PostedMessage_t &msg) {
  ...
  Panel *panel =
      parent->FindChildByName(g_ScriptSymbols.String(msg.event), true);
  Assert(panel != NULL);
  if (!panel) return;

  KeyValues *inputData = new KeyValues(g_ScriptSymbols.String(msg.variable));
  inputData->SetString(g_ScriptSymbols.String(msg.variable),
                       g_ScriptSymbols.String(msg.variable2));
...
int AnimationController::GetRelativeOffset(AnimAlign_t &align, bool xcoord) {
  if (!align.relativePosition) return 0;

  Panel *panel = GetParent()->FindChildByName(
      g_ScriptSymbols.String(align.alignPanel), true);
  if (!panel) return 0;
...
//-----------------------------------------------------------------------------
AnimationController::Value_t AnimationController::GetValue(
    ActiveAnimation_t &anim, Panel *panel, UtlSymId_t var) {
  Value_t val = {0, 0, 0, 0};
  if (var == m_sPosition) {
    int x, y;
    panel->GetPos(x, y);
...
//-----------------------------------------------------------------------------
void AnimationController::SetValue(ActiveAnimation_t &anim, Panel *panel,
                                   UtlSymId_t var, Value_t &value) {
  if (var == m_sPosition) {
    ...
    if (value.b == 0.0f && value.c == 0.0f && value.d == 0.0f) {
      ...
      Color col((unsigned char)value.a, (unsigned char)value.b,
                (unsigned char)value.c, (unsigned char)value.d);
...
class CPanelAnimationDictionary {
 public:
  ...
  ~CPanelAnimationDictionary() { m_PanelAnimationMapPool.Clear(); }
...
//-----------------------------------------------------------------------------
PanelAnimationMap *CPanelAnimationDictionary::FindPanelAnimationMap(
...
// Purpose:
//-----------------------------------------------------------------------------
PanelAnimationMap *CPanelAnimationDictionary::FindOrAddPanelAnimationMap(
    char const *className) {
...
// Purpose:
//-----------------------------------------------------------------------------
void CPanelAnimationDictionary::PanelAnimationDumpMap(PanelAnimationMap *map,
                                                      bool recursive) {
  if (map->pfnClassName) {
    Msg("%s\n", (*map->pfnClassName)());
  }
