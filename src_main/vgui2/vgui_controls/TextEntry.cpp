...
TextEntry::TextEntry(Panel *parent, const char *panelName)
  ...
  // If keyboard focus is in an edit control, don't chain keyboard mappings up
  // to parents since it could mess with typing in text.
...
void TextEntry::ApplySchemeSettings(IScheme *pScheme) {
  ...
  _selectionTextColor =
      GetSchemeColor("TextEntry.SelectedTextColor", GetFgColor(), pScheme);
  _selectionColor = GetSchemeColor("TextEntry.SelectedBgColor", pScheme);
  _defaultSelectionBG2Color =
      GetSchemeColor("TextEntry.OutOfFocusSelectedBgColor", pScheme);
  _focusEdgeColor =
      GetSchemeColor("TextEntry.FocusEdgeColor", Color(0, 0, 0, 0), pScheme);
...
//          Turns off any selected text in the window if we are not using the
//          edit menu
//-----------------------------------------------------------------------------
void TextEntry::OnKillFocus() {
  ...
  if (mouseRightClicked || mouseRightDown || mouseRightUp) {
    ...
    if (IsWithin(cursorX, cursorY)) return;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Set the text array - convert ANSI text to unicode and pass to
// unicode function
//-----------------------------------------------------------------------------
...
void TextEntry::CursorToPixelSpace(int cursorPos, int &cx, int &cy) {
  ...
  for (int i = GetStartDrawIndex(lineBreakIndexIndex); i < m_TextStream.Count();
       i++) {
    ...
    // if we've passed a line break go to that
    if (m_LineBreaks.Count() && lineBreakIndexIndex < m_LineBreaks.Count() &&
...
int TextEntry::PixelToCursorSpace(int cx, int cy) {
  ...
  for (i = startIndex; i < m_TextStream.Count(); i++) {
    ...
    // if we are on the right line but off the end of if put the cursor at the
    // end of the line
    if (m_LineBreaks[lineBreakIndexIndex] == i) {
      // add another line
      AddAnotherLine(x, y);
    ...
    if (onRightLine) {
      ...
      {
      } else if (cx < (DRAW_OFFSET_X + _pixelsIndent) ||
                 cy < yStart)  // off left side of window
      {
...
void TextEntry::PaintBackground() {
  ...
  bool highlight_composition =
      (nCompStart != -1 && nCompEnd != -1) ? true : false;
  ...
  if ((!_multiline) && (!_horizScrollingAllowed)) {
    ...
    if ((!HasFocus() && (IsEditable())) || (!IsEditable())) {
      ...
      if (addEllipses && !IsEditable() && m_bUseFallbackFont &&
      ...
      if (m_TextStream.Count() - endIndex < 3 &&
          m_TextStream.Count() - endIndex > 0) {
  ...
  // custom border
  //!! need to replace this with scheme stuff
  //!(TextEntryBorder/TextEntrySelectedBorder)
...
void TextEntry::RecalculateLineBreaks() {
  if (!_multiline || _hideText) return;

  if (m_TextStream.Count() < 1) return;
...
void TextEntry::CreateEditMenu() {
  ...
  if (_editable && !_hideText) {
    m_pEditMenu->AddMenuItem("#TextEntry_Cut", new KeyValues("DoCutSelected"),
                             this);
  }

  if (!_hideText) {
    m_pEditMenu->AddMenuItem("#TextEntry_Copy", new KeyValues("DoCopySelected"),
                             this);
  }

  if (_editable) {
    m_pEditMenu->AddMenuItem("#TextEntry_Paste", new KeyValues("DoPaste"),
                             this);
  }
  ...
  if (m_bAllowNonAsciiCharacters) {
    ...
    if (count > 0) {
      ...
      for (int i = 0; i < count; ++i) {
        int id = subMenu->AddCheckableMenuItem(
            "Language", UnlocalizeUnicode(langs[i].menuname),
            new KeyValues("DoLanguageChanged", "handle", langs[i].handleValue),
            this);
        if (langs[i].active) {
      ...
      m_pEditMenu->AddCascadingMenuItem("Language", "#TextEntry_Language", "",
                                        this, subMenu);
    ...
    if (count > 0) {
      ...
      for (int i = 0; i < count; ++i) {
        int id = subMenu->AddCheckableMenuItem(
            "ConversionMode", UnlocalizeUnicode(modes[i].menuname),
      ...
      m_pEditMenu->AddCascadingMenuItem(
          "ConversionMode", "#TextEntry_ConversionMode", "", this, subMenu);
    ...
    if (count > 0) {
      ...
      m_pEditMenu->AddCascadingMenuItem(
          "SentenceMode", "#TextEntry_SentenceMode", "", this, subMenu);
...
void TextEntry::OnMouseFocusTicked() {
  ...
  if (_mouseDragSelection)  // text is being selected via mouse clicking and
                            // dragging
  {
...
void TextEntry::OnCursorEntered() {
  _mouseDragSelection =
      false;  // outside of window dont recieve drag scrolling ticks
}
...
void TextEntry::OnCursorMoved(int x, int y) {
  if (_mouseSelection) {
    ...
    if (_cursorPos == 0) _putCursorAtEnd = false;
...
void TextEntry::OnMousePressed(MouseCode code) {
  if (code == MOUSE_LEFT) {
    ...
    _cursorIsAtEnd =
        _putCursorAtEnd;  // save this off before calling PixelToCursorSpace()
    _cursorPos = PixelToCursorSpace(x, y);
    // if we are at Start of buffer don't put cursor at end, this will keep
    // window from scrolling up to a blank line
    if (_cursorPos == 0) _putCursorAtEnd = false;
  ...
  if (ctrl) {
    ...
    switch (code) {
      ...
      case KEY_PAGEUP: {
        ...
        if (_multiline) {
          int displayLines =
              GetTall() / (surface()->GetFontTall(_font) + DRAW_OFFSET_Y);
      ...
      default: {
        ...
        // and we don't want select[1] changed unless a char was pressed that
        // this fxn handles
        return;
      }
    }
...
void TextEntry::OnKeyTyped(wchar_t unichar) {
  ...
  if (iswcntrl(unichar) ||
      unichar == 9)  // tab key (code 9) is printable but handled elsewhere
    return;
...
bool TextEntry::IsLineBreak(int index) {
  for (int i = 0; i < m_LineBreaks.Count(); ++i) {
    if (index == m_LineBreaks[i]) return true;
  }
  return false;
}
...
void TextEntry::GotoLeft() {
  ...
  if (IsLineBreak(_cursorPos)) {
    ...
    if (!_cursorIsAtEnd) _putCursorAtEnd = true;
  }
...
int TextEntry::GetCursorLine() {
  ...
  for (cursorLine = 0; cursorLine < m_LineBreaks.Count(); cursorLine++) {
    if (_cursorPos < m_LineBreaks[cursorLine]) break;
  }

  if (_putCursorAtEnd)  // correct for when cursor is at end of line rather than
                        // Start of next
  {
    // we are not at end of buffer, in which case there is no next line to be at
    // the Start of
    if (_cursorPos != m_TextStream.Count()) cursorLine--;
  }
...
//-----------------------------------------------------------------------------
// Purpose: Move the cursor to a line, need to know how many pixels are in a
// line
//-----------------------------------------------------------------------------
void TextEntry::MoveCursor(int line, int pixelsAcross) {
  // clamp to a valid line
  if (line < 0) line = 0;
  if (line >= m_LineBreaks.Count()) line = m_LineBreaks.Count() - 1;
  ...
  // of the text buffer, to let's just slap it on the end of the text buffer
  // then.
...
//		    Toggling this will disable it in single line windows as
// well.
//-----------------------------------------------------------------------------
...
void TextEntry::ScrollLeftForResize() {
  ...
  while (_currentStartIndex > 0)  // go until we hit leftmost
  {
    ...
    if (nVal != _currentStartIndex) break;
  }
  LayoutVerticalScrollBarSlider();
}
...
//-----------------------------------------------------------------------------
// Purpose: Horizontal scrolling function, not used in multiline windows
//			Scroll one char right until the cursor is visible in the
// window. 			We do this one char at a time because char width
// isn't a constant.
//-----------------------------------------------------------------------------
void TextEntry::ScrollRight() {
  if (!_horizScrollingAllowed) return;
...
//			just compare cursor's pixel coords with the window size
// coords.
// Input:	an integer cursor Position, if you pass _cursorPos fxn will tell
// you
...
void TextEntry::GotoWordRight() {
  ...
  while (++_cursorPos < m_TextStream.Count()) {
    if (iswspace(m_TextStream[_cursorPos])) break;
  }
  ...
  if (_cursorPos > m_TextStream.Count()) _cursorPos = m_TextStream.Count();
...
void TextEntry::GotoFirstOfLine() {
  ...
  //_cursorPos = 0; //TODO: this is wrong, should go to first non-whitespace
  // first, then to zero
...
int TextEntry::GetCurrentLineStart() {
  ...
  if (IsLineBreak(_cursorPos)) {
    for (i = 0; i < m_LineBreaks.Count(); ++i) {
      if (_cursorPos == m_LineBreaks[i]) break;
    }
    if (_cursorIsAtEnd) {
      if (i > 0) {
  ...
  // if there were no line breaks, the first char in the line is the Start of
  // the buffer
  return 0;
}
...
void TextEntry::GotoEndOfLine() {
  SelectCheck();
  // to get to the end of the line you have to take into account line wrap in
  // the buffer we have to figure out at which point the line wraps given the
  // current cursor position, select[1], find the index that is the line end to
  // the right of the cursor
  //_cursorPos=m_TextStream.Count(); //TODO: this is wrong, should go to last
  // non-whitespace, then to true EOL
...
void TextEntry::InsertChar(wchar_t ch) {
    ...
    if (_maxCharCount > 0 && _multiline && _wrap) {
      ...
      while (m_TextStream.Count() > _maxCharCount) {
        ...
        if (m_LineBreaks[0] + 1 < m_TextStream.Count()) {
          ...
          if (_cursorPos > m_TextStream.Count()) {
            _cursorPos = m_TextStream.Count();
          } else {  // shift the cursor up. don't let it wander past zero
  ...
  if (_wrap) {
    ...
    // move chars right 1 starting from cursor, then replace cursorPos with char
  ...
  // if its a newline char we can't do the slider until we recalc the line
  // breaks
  if (ch == '\n') {
    RecalculateLineBreaks();
  }
  ...
  if (m_bAutoProgressOnHittingCharLimit &&
      m_TextStream.Count() == _maxCharCount) {
...
//			note _recalculateBreaksIndex < 0 flags
// RecalculateLineBreaks 			to figure it all out from
// scratch
//-----------------------------------------------------------------------------
void TextEntry::CalcBreakIndex() {
  ...
  if (_cursorPos == m_TextStream.Count()) {
    // we know m_LineBreaks array always has at least one element in it (99999
    // sentinel) when there is just one line this will make recalc = -1 which is
    // ok.
...
//-----------------------------------------------------------------------------
// Purpose: Converts an ansi string to unicode and inserts it into the text
// stream
//-----------------------------------------------------------------------------
...
void TextEntry::Backspace() {
  ...
  // shift chars left one, starting at the cursor position, then make the line
  // one smaller
  ...
  // As we hit the Start of the window, expose more chars so we can see what we
  // are deleting
  if (_cursorPos == _currentStartIndex) {
    // windows tabs over 6 chars
    if (_currentStartIndex - 6 >=
        0)  // dont scroll if there are not enough chars to scroll
    {
  ...
  // recalculate linebreaks (the fast incremental linebreak function doesn't
  // work in this case)
  _recalculateBreaksIndex = 0;
  m_LineBreaks.RemoveAll();
  m_LineBreaks.AddToTail(BUFFER_SIZE);
...
//-----------------------------------------------------------------------------
// Purpose: Deletes the current selection, if any, moving the cursor to the
// Start
//			of the selection
//-----------------------------------------------------------------------------
void TextEntry::DeleteSelected() {
  ...
  // if the line is empty, don't do anything
  if (m_TextStream.Count() == 0) return;
  ...
  // shift chars left one starting after cursor position, then make the line one
  // smaller
  int dif = x1 - x0;
  for (int i = 0; i < dif; ++i) {
...
void TextEntry::Delete() {
  ...
  if (!GetSelectedRange(x0, x1)) {
    ...
    if (_cursorPos >= m_TextStream.Count()) return;
  }
...
void TextEntry::Paste() {
  ...
  if (!m_bAutoProgressOnHittingCharLimit) {
    bufferSize = _maxCharCount > 0
                     ? _maxCharCount + 1
                     : system()->GetClipboardTextCount();  // +1 for terminator
  }

  buf.AddMultipleToTail(bufferSize);
  int len =
      system()->GetClipboardText(0, buf.Base(), bufferSize * sizeof(wchar_t));
  if (len < 1) return;
  ...
  for (int i = 0; i < len && buf[i] != 0; i++) {
    if (m_bAutoProgressOnHittingCharLimit) {
      ...
      if (m_TextStream.Count() == _maxCharCount) {
        ...
        if (GetVParent() &&
            ipanel()->GetCurrentKeyFocus(GetVParent()) != GetVPanel()) {
          bHaveMovedFocusAwayFromCurrentEntry = true;
          ipanel()->SendMessage(ipanel()->GetCurrentKeyFocus(GetVParent()),
                                new KeyValues("DoPaste"), GetVPanel());
        }
        break;
      }
    }
...
int TextEntry::GetStartDrawIndex(int &lineBreakIndexIndex) {
  ...
  if (_vertScrollBar && !_mouseDragSelection) {
    ...
    HFont font = _font;
    int displayLines =
        GetTall() / (surface()->GetFontTall(font) + DRAW_OFFSET_Y);
    if (displayLines < 1) {
      displayLines = 1;
    }
    if (numLines > displayLines) {
  ...
  if (!_horizScrollingAllowed) return 0;
...
void TextEntry::GetText(char *buf, int bufLen) {
  if (m_TextStream.Count()) {
    // temporarily 0 terminate the text stream so we can use the conversion
    // function
...
const char *TextEntry::GetDescription() {
  ...
  Q_snprintf(buf, sizeof(buf),
             "%s, bool textHidden, bool editable, bool unicode, bool "
             "NumericInputOnly, int maxchars",
...
void TextEntry::SetToFullHeight() {
  ...
  tall = GetNumLines() * (surface()->GetFontTall(_font) + DRAW_OFFSET_Y) +
         DRAW_OFFSET_Y + 2;
  SetSize(wide, tall);
  PerformLayout();
}
...
void TextEntry::SetToFullWidth() {
  ...
  int tall =
      (surface()->GetFontTall(_font) + DRAW_OFFSET_Y) + DRAW_OFFSET_Y + 2;
...
bool TextEntry::GetDropContextMenu(Menu *menu,
...
void TextEntry::OnPanelDropped(CUtlVector<KeyValues *> &msglist) {
  ...
  if (!newText || newText[0] == L'\0') return;

  char const *cmd = data->GetString("command");
  if (!Q_stricmp(cmd, "replace") || !Q_stricmp(cmd, "default")) {
    SetText(newText);
    _dataChanged = true;
    FireActionSignal();
