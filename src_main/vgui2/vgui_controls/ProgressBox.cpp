...
// Purpose: Constructor
//-----------------------------------------------------------------------------
ProgressBox::ProgressBox(const char *title, const char *text,
                         const char *pszUnknownTimeString, Panel *parent)
    : Frame(parent, NULL, parent ? false : true) {
  // save off the non-localized title, since we may need to dynamically localize
  // it (on progress updates)
  const wchar_t *ws = g_pVGuiLocalize->Find(title);
  if (ws) {
    wcscpy_s(m_wszTitleString, ws);
  } else {
    g_pVGuiLocalize->ConvertANSIToUnicode(title, m_wszTitleString,
                                          sizeof(m_wszTitleString));
  }
  ...
  if (ws) {
    wcscpy_s(m_wszUnknownTimeString, ws);
  } else {
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
ProgressBox::ProgressBox(const wchar_t *wszTitle, const wchar_t *wszText,
                         const wchar_t *wszUnknownTimeString, Panel *parent)
    : Frame(parent, NULL, parent ? false : true) {
  wcscpy_s(m_wszTitleString, wszTitle);
  m_pMessageLabel = new Label(this, NULL, wszUnknownTimeString);
  wcscpy_s(m_wcsInfoString, wszText);
  wcscpy_s(m_wszUnknownTimeString, wszUnknownTimeString);
  Init();
}
...
//			Does not suspend execution - use addActionSignal to get
// return value
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void ProgressBox::SetText(const char *text) { m_pMessageLabel->SetText(text); }
...
void ProgressBox::UpdateTitle() {
  ...
  g_pVGuiLocalize->ConstructString(unicode, sizeof(unicode), m_wszTitleString,
                                   1, completion);
...
void ProgressBox::OnThink() {
  ...
  if (m_flFirstProgressUpdate >= 0.0f && m_wcsInfoString[0]) {
    ...
    if (ProgressBar::ConstructTimeRemainingString(
            timeRemaining, sizeof(timeRemaining), m_flFirstProgressUpdate,
      ...
      g_pVGuiLocalize->ConstructString(unicode, sizeof(unicode),
                                       m_wcsInfoString, 1, timeRemaining);
...
//-----------------------------------------------------------------------------
void ProgressBox::OnCloseFrameButtonPressed() { OnCancel(); }
