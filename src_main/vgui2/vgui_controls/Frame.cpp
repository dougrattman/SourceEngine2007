...
static const int DEFAULT_SNAP_RANGE =
    10;  // number of pixels distance before the frame will snap to an edge
...
class GripPanel : public Panel {
 public:
  ...
  // Input- dx, dy, the offet of the mouse pointer from where we started
  // dragging
  virtual void moved(int dx, int dy) {
    if (!_frame->IsSizeable()) return;
...
class CaptionGripPanel : public GripPanel {
 public:
  ...
  void tryToDock(VPANEL window, int &newX, int &newY) {
    ...
    if (window == _frame->GetVPanel()) return;
  ...
  // Input: boundX boundY, position of candidate window we are seeing if we snap
  // to
  //        boundWide, boundTall, width and height of window we are seeing if we
  //        snap to
  ...
  bool getInsideSnapPosition(int boundX, int boundY, int boundWide,
                             int boundTall, int &snapToX, int &snapToY) {
    int wide, tall;
  ...
  // Purpose: To calculate the windows new x,y position if it snaps
  //          Will snap to the OUTSIDE edges of a window (i.e. will stick peers
  //          together
  ...
  bool getOutsideSnapPosition(int left, int top, int boundWide, int boundTall,
    ...
    // want to make it so that if any part of the window can dock to the
    // candidate, it will
...
class FrameButton : public Button {
 ...
 public:
  ...
  FrameButton(Panel *parent, const char *name, const char *text)
    ...
    SetSize(FrameButton::GetButtonSide((Frame *)parent),
            FrameButton::GetButtonSide((Frame *)parent));
    _brightBorder = NULL;
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    _disabledFgColor =
        GetSchemeColor("FrameTitleButton.DisabledFgColor", pScheme);
    _disabledBgColor =
        GetSchemeColor("FrameTitleButton.DisabledBgColor", pScheme);
...
class FrameSystemButton : public MenuButton {
 ...
 public:
  ...
  virtual void ApplySchemeSettings(IScheme *pScheme) {
    ...
    const char *pEnabledImage =
        m_EnabledImage.Length()
            ? m_EnabledImage.Get()
            : pScheme->GetResourceString("FrameSystemButton.Icon");
    const char *pDisabledImage =
        m_DisabledImage.Length()
            ? m_DisabledImage.Get()
  ...
  void SetResponsive(bool state) { _respond = state; }
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Frame::Frame(Panel *parent, const char *panelName, bool showTaskbarIcon)
...
//-----------------------------------------------------------------------------
// Purpose: Bring the frame to the front and requests focus, ensures it's not
// minimized
//-----------------------------------------------------------------------------
void Frame::Activate() {
  MoveToFront();
...
//			if dialog is not currently visible it starts it minimized
//and flashing in the taskbar
//-----------------------------------------------------------------------------
void Frame::ActivateMinimized() {
  if (IsVisible() && !IsMinimized() ||
      !surface()->SupportsFeature(ISurface::FRAME_MINIMIZE_MAXIMIZE)) {
    Activate();
  } else {
...
void Frame::LayoutProportional(FrameButton *bt) {
  ...
  bt->SetSize((int)(FrameButton::GetButtonSide(this) * scale),
              (int)(FrameButton::GetButtonSide(this) * scale));
...
void Frame::OnThink() {
  ...
  if (IsVisible() && m_flTransitionEffectTime > 0) {
    if (m_bFadingOut) {
      ...
      GetAnimationController()->RunAnimationCommand(
          this, "alpha", 255.0f, 0.0f, m_flTransitionEffectTime,
  ...
  if (hasFocus != m_bHasFocus) {
    // Because vgui focus is message based, and focus gets reset to NULL when a
    // focused panel is deleted, we defer the flashing/transition
    //  animation for an extra frame in case something is deleted, a message is
    //  sent, and then we become the focused panel again on the next frame
    if (!m_bPrimed) {
      m_bPrimed = true;
      return;
    }
...
void Frame::OnFrameFocusChanged(bool bHasFocus) {
  ...
  if (bHasFocus) {
    if (m_flFocusTransitionEffectTime) {
      GetAnimationController()->RunAnimationCommand(
          this, "BgColor", m_InFocusBgColor, 0.0f, m_flTransitionEffectTime,
          AnimationController::INTERPOLATOR_LINEAR);
    } else {
    ...
    if (m_flFocusTransitionEffectTime) {
      GetAnimationController()->RunAnimationCommand(
          this, "BgColor", m_OutOfFocusBgColor, 0.0f, m_flTransitionEffectTime,
          AnimationController::INTERPOLATOR_LINEAR);
    } else {
...
void Frame::PerformLayout() {
  ...
  _bottomGrip->SetBounds(CORNER_SIZE, tall - DRAGGER_SIZE,
                         wide - (CORNER_SIZE + BOTTOMRIGHTSIZE), DRAGGER_SIZE);
  _rightGrip->SetBounds(wide - DRAGGER_SIZE, CORNER_SIZE, DRAGGER_SIZE,
                        tall - (CORNER_SIZE + BOTTOMRIGHTSIZE));

  _bottomRightGrip->SetBounds(wide - BOTTOMRIGHTSIZE, tall - BOTTOMRIGHTSIZE,
                              BOTTOMRIGHTSIZE, BOTTOMRIGHTSIZE);
  ...
  _menuButton->SetBounds(5 + 2, 5 + 3, GetCaptionHeight() - 5,
                         GetCaptionHeight() - 5);
  ...
  if (_closeButton->IsVisible()) {
    _closeButton->SetPos((wide - side_border_offset) - offset,
                         top_border_offset);
    offset += offset_start;
    LayoutProportional(_closeButton);
  }
  if (_minimizeToSysTrayButton->IsVisible()) {
    _minimizeToSysTrayButton->SetPos((wide - side_border_offset) - offset,
                                     top_border_offset);
    offset += offset_start;
    LayoutProportional(_minimizeToSysTrayButton);
  }
  if (_maximizeButton->IsVisible()) {
    _maximizeButton->SetPos((wide - side_border_offset) - offset,
                            top_border_offset);
    offset += offset_start;
    LayoutProportional(_maximizeButton);
  }
  if (_minimizeButton->IsVisible()) {
    _minimizeButton->SetPos((wide - side_border_offset) - offset,
                            top_border_offset);
    offset += offset_start;
...
void Frame::SetTitle(const char *title, bool surfaceTitle) {
  ...
  // see if the combobox text has changed, and if so, post a message detailing
  // the new text
  ...
  if (*newTitle == '#') {
    ...
    StringIndex_t unlocalizedTextSymbol =
        g_pVGuiLocalize->FindIndex(newTitle + 1);
    if (unlocalizedTextSymbol != INVALID_STRING_INDEX) {
      // we have a new text value
      wcscpy_s(unicodeText,
               g_pVGuiLocalize->GetValueByIndex(unlocalizedTextSymbol));
    }
  } else {
...
// When moving via caption, don't let any part of window go outside parent's
// bounds
...
void Frame::GetClientArea(int &x, int &y, int &wide, int &tall) {
  ...
  if (_drawTitleBar) {
    ...
    int border =
        m_bSmallCaption ? CAPTION_TITLE_BORDER_SMALL : CAPTION_TITLE_BORDER;
...
//-----------------------------------------------------------------------------
// Purpose: gets the default position and size on the screen to appear the first
// time (defaults to centered)
//-----------------------------------------------------------------------------
...
void Frame::ApplySchemeSettings(IScheme *pScheme) {
  ...
  _titleBarDisabledFgColor =
      GetSchemeColor("FrameTitleBar.DisabledTextColor", pScheme);
  _titleBarDisabledBgColor =
      GetSchemeColor("FrameTitleBar.DisabledBgColor", pScheme);
  ...
  _title->SetFont(
      pScheme->GetFont((font && *font) ? font : "Default", IsProportional()));
  ...
  m_flTransitionEffectTime =
      atof(pScheme->GetResourceString("Frame.TransitionEffectTime"));
  m_flFocusTransitionEffectTime =
      atof(pScheme->GetResourceString("Frame.FocusTransitionEffectTime"));

  m_InFocusBgColor = pScheme->GetColor("Frame.BgColor", GetBgColor());
  m_OutOfFocusBgColor =
      pScheme->GetColor("Frame.OutOfFocusBgColor", m_InFocusBgColor);
...
void Frame::ApplySettings(KeyValues *inResourceData) {
  // Don't change the frame's visibility, remove that setting from the config
  // data
  inResourceData->SetInt("visible", -1);
  BaseClass::ApplySettings(inResourceData);
  if (!inResourceData->GetInt(
          "settitlebarvisible",
          1))  // if "title" is "0" then don't draw the title bar
  {
...
//-----------------------------------------------------------------------------
void Frame::OnClose() {
  // if we're modal, release that before we hide the window else the wrong
  ...
  if (m_flTransitionEffectTime) {
    ...
    GetAnimationController()->RunAnimationCommand(
        this, "alpha", 0.0f, 0.0f, m_flTransitionEffectTime,
        AnimationController::INTERPOLATOR_LINEAR);
    m_bFadingOut = true;
    // move us to the back of the draw order (so that fading out over the top of
    // other dialogs doesn't look wierd)
...
//-----------------------------------------------------------------------------
void Frame::SetMenuButtonVisible(bool state) { _menuButton->SetVisible(state); }

//-----------------------------------------------------------------------------
// Purpose: Toggle respond of the system menu button
//			it will look enabled or disabled in response to the title
//bar 			but may not activate.
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: Toggles visibility of the minimize-to-systray icon (defaults to
// false)
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Purpose: load the control settings - should be done after all the children
// are added to the dialog
//-----------------------------------------------------------------------------
void Frame::LoadControlSettings(const char *dialogResourceName,
                                const char *pathID,
                                KeyValues *pPreloadedKeyValues) {
...
void Frame::OnKeyCodeTyped(KeyCode code) {
  ...
  if (ctrl && shift && alt && code == KEY_B) {
    ...
    if (top) {
      ...
      if (panel) {
        // make the top-level panel reload it's scheme, it will chain down to
    ...
    if (panel && ipanel()->IsVisible(panel) && ipanel()->IsEnabled(panel)) {
      ...
      PostMessage(panel, new KeyValues("Hotkey"));
             ...
             input()->GetAppModalSurface() == GetVPanel()) {
    // ESC cancels, unless we're in the engine - in the engine ESC flips between
    // the UI and the game
    CloseModal();
  }
  // Usually don't chain back as Frames are the end of the line for key presses,
  // unless m_bChainKeysToParent is set
