...
void LightDesc_t::RecalculateDerivedValues(void) {
  ...
  if (m_Attenuation2) m_Flags |= LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION2;

  if (m_Type == MATERIAL_LIGHT_SPOT) {
    m_ThetaDot = cos(m_Theta);
    m_PhiDot = cos(m_Phi);
    f32 spread = m_ThetaDot - m_PhiDot;
    if (spread > 1.0e-10) {
...
void LightDesc_t::ComputeLightAtPointsForDirectional(const FourVectors &pos,
...
void LightDesc_t::ComputeLightAtPoints(const FourVectors &pos,
                                       const FourVectors &normal,
                                       FourVectors &color,
                                       bool DoHalfLambert) const {
  FourVectors delta;
  Assert((m_Type == MATERIAL_LIGHT_POINT) || (m_Type == MATERIAL_LIGHT_SPOT) ||
  ...
  if (m_Flags & LIGHTTYPE_OPTIMIZATIONFLAGS_HAS_ATTENUATION1) {
    falloff = AddSIMD(falloff,
                      MulSIMD(ReplicateX4(m_Attenuation1), SqrtEstSIMD(dist2)));
  }
  ...
  switch (m_Type) {
    ...
    case MATERIAL_LIGHT_SPOT: {
      fltx4 dot2 = SubSIMD(Four_Zeros, delta * m_Direction);  // dot position
                                                              // with spot light
                                                              // dir for cone
                                                              // falloff

      fltx4 cone_falloff_scale =
          MulSIMD(ReplicateX4(OneOver_ThetaDot_Minus_PhiDot),
                  SubSIMD(dot2, ReplicateX4(m_PhiDot)));
      cone_falloff_scale = MinSIMD(cone_falloff_scale, Four_Ones);

      if ((m_Falloff != 0.0) && (m_Falloff != 1.0)) {
        // !!speed!! could compute integer exponent needed by powsimd and store
        // in light
      ...
      fltx4 OutsideMask =
          CmpGtSIMD(dot2, ReplicateX4(m_PhiDot));  // outside light cone?
...
void LightDesc_t::ComputeNonincidenceLightAtPoints(const FourVectors &pos,
...
void LightDesc_t::SetupOldStyleAttenuation(f32 fQuadraticAttn,
  ...
  if (fLinearAttn < EQUAL_EPSILON) fLinearAttn = 0;

  if (fConstantAttn < EQUAL_EPSILON) fConstantAttn = 0;

  if ((fConstantAttn < EQUAL_EPSILON) && (fLinearAttn < EQUAL_EPSILON) &&
...
void LightDesc_t::SetupNewStyleAttenuation(f32 fFiftyPercentDistance,
                                           f32 fZeroPercentDistance) {
  // new style storing 50% and 0% distances
  f32 d50 = fFiftyPercentDistance;
  f32 d0 = fZeroPercentDistance;
  if (d0 < d50) {
    // !!warning in lib code???!!!
    Warning(
        "light has _fifty_percent_distance of %f but no "
        "zero_percent_distance\n",
        d50);
    d0 = 2.0 * d50;
  }
  f32 a = 0, b = 1, c = 0;
