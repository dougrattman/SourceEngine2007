...
static struct QuantizedValue *AllocQValue(void) {
  ...
  ret->ErrorMeasure = new f64[current_ndims];
  ret->Mean = new u8[current_ndims];
  ret->Mins = new u8[current_ndims];
  ret->Maxs = new u8[current_ndims];
...
static int QNumSort(void const *a, void const *b) {
  i32 as = ((struct Sample *)a)->QNum;
  i32 bs = ((struct Sample *)b)->QNum;
  if (as == bs) return 0;
  return (as > bs) ? 1 : -1;
}
...
static int samplesort(void const *a, void const *b) {
  u8 as = ((struct Sample *)a)->Value[current_sort_dim];
  u8 bs = ((struct Sample *)b)->Value[current_sort_dim];
  if (as == bs) return 0;
  return (as > bs) ? 1 : -1;
}
#endif
...
static int sortlong(void const *a, void const *b) {
  ...
  return memcmp(((struct Sample *)a)->Value, ((struct Sample *)b)->Value,
                current_ndims);
}

#define NEXTSAMPLE(s) ((struct Sample *)(((u8 *)s) + current_ssize))
...
void PrintSamples(struct Sample const *s, int nsamples, int ndims) {
  ...
  while (nsamples--) {
    ...
    for (int d = 0; d < ndims; d++) printf("%02x,", s->Value[d]);
    printf("}\n");
    s = NEXTSAMPLE(s);
  }
}
...
void PrintQTree(struct QuantizedValue const *p, int idlevel) {
  ...
  if (p) {
    ...
    for (i = 0; i < current_ndims; i++) printf("%x,", p->Mean[i]);
    printf("}\n");
    for (i = 0; i < idlevel; i++) printf(" ");
    printf("Errors={");
    for (i = 0; i < current_ndims; i++) printf("%f,", p->ErrorMeasure[i]);
    printf("}\n");
    for (i = 0; i < idlevel; i++) printf(" ");
    printf("Mins={");
...
static void UpdateStats(struct QuantizedValue *v) {
  ...
  for (i = 0; i < v->NSamples; i++) {
    ...
    f64 c = s->Count;
    for (j = 0; j < current_ndims; j++) {
      f64 diff = SQ(s->Value[j] - v->Mean[j]);
      Errors[j] += c * diff;  // charles uses abs not sq()
      if (diff > WorstError[j]) WorstError[j] = diff;
    }
  }
  v->TotalError = 0.;
  f64 ErrorScale = 1.;  // /sqrt((f64) (N));
  for (j = 0; j < current_ndims; j++) {
  ...
  if (q->Children[0]) {
    ...
    if (q->TotalError > ErrorVal) {
      ...
      for (int d = 0; d < current_ndims; d++)
        if (q->ErrorMeasure[d] > q->ErrorMeasure[ErrorDim]) ErrorDim = d;
    }
  }
}
...
static void SubdivideNode(struct QuantizedValue *n, int whichdim) {
  ...
  for (int i = 0; i < n->NSamples; i++) {
    int whichside = 1;
    struct Sample *sl = SAMPLE(n->Samples, i);
    u8 v = sl->Value[whichdim];
    if (v < minv) {
      minv = v;
      minS = sl;
    }
    ...
    if (v < n->Mean[whichdim]) whichside = 0;
    totsamps[whichside] += sl->Count;
    for (int d = 0; d < current_ndims; d++)
      LocalMean[d][whichside] += sl->Count * sl->Value[d];
  }
  ...
  if (totsamps[0] && totsamps[1])
    ...
    // value.
    for (int i = 0; i < current_ndims; i++) {
      LocalMean[i][0] = minS ? minS->Value[i] : 0;
      LocalMean[i][1] = maxS ? maxS->Value[i] : 0;
    }
  }
  ...
  for (int i = 0; i < n->NSamples; i++, NAdded++)
    if (SAMPLE(n->Samples, i)->QNum) break;
  ...
  int TotAdded = 0;
  for (;;) {
    if (NAdded == n->NSamples - 1) break;
    if (TotAdded >= n->TotSamples / 2) break;
...
void CheckInRange(struct QuantizedValue *q, u8 *max, u8 *min) {
  if (q) {
    if (q->Children[0]) {
      // non-leaf node
...
struct QuantizedValue *Quantize(struct Sample *s, int nsamples, int ndims,
                                int nvalues, u8 *weights, int firstvalue) {
  SetNDims(ndims);
  ...
  while (--nvalues) {
    if (!FindWorst()) break;  // if <n unique ones, stop now
...
f64 MaximumError(struct QuantizedValue const *q, u8 const *sample, int ndims,
  ...
  for (int i = 0; i < ndims; i++) {
    int val2 = sample[i];
    int val1 = (abs(val2 - q->Mins[i]) > abs(val2 - q->Maxs[i])) ? q->Mins[i]
                                                                 : q->Maxs[i];
...
void InitHeap(struct FHeap *h) { h->heap_n = 0; }
...
void HeapInsert(struct FHeap *h, f64 *elem) {
  h->heap_n++;
...
struct QuantizedValue *FindMatch(u8 const *sample, int ndims, u8 *weights,
  ...
  PUSHNODE(q);
  for (;;) {
    struct QuantizedValue *test =
        (struct QuantizedValue *)RemoveHeapItem(&TheQueue);
    ...
    if (test->Children[0]) {
      ...
      if (c1->MinError < besterror) HeapInsert(&TheQueue, &(c1->MinError));
      c2->MinError = MinimumError(c2, sample, ndims, weights);
      if (c2->MinError < besterror) HeapInsert(&TheQueue, &(c2->MinError));
    } else {
  ...
  if (bestmatch) {
    ...
    bestmatch->NQuant++;
    for (int i = 0; i < ndims; i++) bestmatch->Sums[i] += sample[i];
  }
  return bestmatch;
}
