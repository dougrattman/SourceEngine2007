...
fltx4 NoiseSIMD(const fltx4 &x, const fltx4 &y, const fltx4 &z) {
  ...
  fltx4 lattice000 = Four_Zeros, lattice001 = Four_Zeros,
        lattice010 = Four_Zeros, lattice011 = Four_Zeros;
...
#define DOPASS(i)                                                           \
  ...
  // now, we have 8 lattice values for each of four points as m128s, and
  // interpolant values for each axis in m128 form in [xyz]frac. Perfom the
  // trilinear interpolation as SIMD ops

  // first, do x interpolation
  fltx4 l2d00 =
      AddSIMD(lattice000, MulSIMD(xfrac, SubSIMD(lattice100, lattice000)));
  fltx4 l2d01 =
      AddSIMD(lattice001, MulSIMD(xfrac, SubSIMD(lattice101, lattice001)));
  fltx4 l2d10 =
      AddSIMD(lattice010, MulSIMD(xfrac, SubSIMD(lattice110, lattice010)));
  fltx4 l2d11 =
      AddSIMD(lattice011, MulSIMD(xfrac, SubSIMD(lattice111, lattice011)));
