...
//			angles[PITCH, YAW, ROLL]. Receives right-handed
// counterclockwise
...
// transform a set of angles in the output space of parentMatrix to the input
// space
...
// transform a set of angles in the input space of parentMatrix to the output
// space
...
// Matrix is right-handed x=forward, y=left, z=up.  We a left-handed convention
// for vectors in the game code (forward, right, up)
...
//			the basis vectors for the rotations will be in the
...
bool SolveInverseQuadraticMonotonic(f32 x1, f32 y1, f32 x2, f32 y2, f32 x3,
  ...
  // this code is not fast. what it does is when the curve would be
...
bool SolveInverseReciprocalQuadratic(f32 x1, f32 y1, f32 x2, f32 y2, f32 x3,
  ...
  b = (x2 * x2 * y2 * (y1 - y3) + x3 * x3 * (-y1 + y2) * y3 +
       x1 * x1 * y1 * (-y2 + y3)) /
      det;

  c = (x2 * (x2 - x3) * x3 * y2 * y3 + x1 * x1 * y1 * (x2 * y2 - x3 * y3) +
       x1 * (-(x2 * x2 * y1 * y2) + x3 * x3 * y1 * y3)) /
      det;
...
f32 QuaternionNormalize(Quaternion &q) {
  ...
  if (radius)  // > FLT_EPSILON && ((radius < 1.0f - 4*FLT_EPSILON) || (radius
               // > 1.0f + 4*FLT_EPSILON))
  {
    radius = sqrt(radius);
    iradius = 1.0f / radius;
...
void Catmull_Rom_Spline(const Vector &p1, const Vector &p2, const Vector &p3,
  ...
  // matrix row 1
  VectorScale(p1, -tSqrSqr,
              a);  // 0.5 t^3 * [ (-1*p1) + ( 3*p2) + (-3*p3) + p4 ]
  ...
  // matrix row 2
  VectorScale(p1, tSqr * 2,
              a);  // 0.5 t^2 * [ ( 2*p1) + (-5*p2) + ( 4*p3) - p4 ]
...
bool CalcLineToLineIntersectionSegment(const Vector &p1, const Vector &p2,
  ...
  if (fabs(p43.x) < LINE_EPS && fabs(p43.y) < LINE_EPS &&
      fabs(p43.z) < LINE_EPS)
    return false;
  p21.x = p2.x - p1.x;
  p21.y = p2.y - p1.y;
  p21.z = p2.z - p1.z;
  if (fabs(p21.x) < LINE_EPS && fabs(p21.y) < LINE_EPS &&
      fabs(p21.z) < LINE_EPS)
    return false;
  ...
  if (fabs(denom) < LINE_EPS) return false;
...
int PolyFromPlane(Vector *outVerts, const Vector &normal, f32 dist,
  ...
  // Calculate the third orthonormal basis vector for our plane space (this one
  // and vup are in the plane)
  CrossProduct(vup, normal, vright);

  // Make the plane's basis vectors big (these are the half-sides of the polygon
  // we're making)
...
int ClipPolyToPlane_Precise(f64 *inVerts, int vertCount, f64 *outVerts,
  ...
  for (i = 0; i < vertCount; i++) {
    // dot = DotProduct( inVerts[i], normal) - dist;
    dot = ((inVerts[i * 3 + 0] * normal[0]) + (inVerts[i * 3 + 1] * normal[1]) +
           (inVerts[i * 3 + 2] * normal[2])) -
          dist;
    dists[i] = dot;
    if (dot > fOnPlaneEpsilon) {
      sides[i] = SIDE_FRONT;
...
void GeneratePerspectiveFrustum(const Vector &origin, const Vector &forward,
  ...
  frustum.SetPlane(FRUSTUM_BOTTOM, PLANE_ANYZ, normalPos,
                   normalPos.Dot(origin));
...
void GetInterpolationData(f32 const *pKnotPositions, f32 const *pKnotValues,
  ...
  if (idx == 0) {
    if (bWrap) {
      ...
      flOffsetFromStartOfGap = flPositionToInterpolateAt +
                               (nInterpolationRange - pKnotPositions[nKnot1]);
    } else {
  ...
  } else if (idx == nNumValuesinList)  // ran out of values
  {
    if (bWrap) {
      ...
      flOffsetFromStartOfGap =
          flPositionToInterpolateAt - pKnotPositions[nKnot1];
    } else {
